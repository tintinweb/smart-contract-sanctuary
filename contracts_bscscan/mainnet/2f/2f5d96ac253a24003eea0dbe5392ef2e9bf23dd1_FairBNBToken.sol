...
/**
 20% Redistribution In BNB
Auto Paid Every 1 HR
 4% Liquidity 
 1% Marketing 

BNBFAIR  Official Community
🌐Website:  www.bnbfair.online
✅Telegram: t.me/bnbfair
✅Twitter: twitter.com/bnb_fa1r
*/
...
contract FairBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("FairBNB_Dividend_Tracker", "FairBNB_Dividend_Tracker", 9) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000 * (10**9); //must hold 10000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "FairBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "FairBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FairBNB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FairBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FairBNB_Dividend_Tracker: Cannot update claimWait to same value");
...
contract FairBNBToken is BEP20, Ownable {
    using SafeMath for uint256;

    IPancakeswapV2Router02 public pancakeswapV2Router;
    address public immutable pancakeswapV2Pair;
    address public marketingWallet = 0x08a2d060217B4be7b3F25c224Fc1928AF118c35a;

    bool private swapping;

    FairBNBDividendTracker public dividendTracker;
    ...
    constructor() public BEP20("FairBNB", "BNF", 9) {
    	...
    	dividendTracker = new FairBNBDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FairBNB: The dividend tracker already has that address");

        FairBNBDividendTracker newDividendTracker = FairBNBDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FairBNB: The new dividend tracker must be owned by the FairBNB token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "FairBNB: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FairBNB: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "FairBNB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FairBNB: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "FairBNB: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FairBNB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FairBNB: Cannot update gasForProcessing to same value");
