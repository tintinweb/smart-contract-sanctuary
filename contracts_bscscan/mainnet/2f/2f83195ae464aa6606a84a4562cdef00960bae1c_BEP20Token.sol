...
contract BEP20Token is Context, IBEP20, Ownable {
    ...
    function _transfer(
        ...
        require(
            ...
            "CS:GO: resulting balance more than the maximum allowed"
        );
    ...
    function deliver(uint256 amount) external {
        ...
        require(
            outgoing <= _balances[sender],
            "CS:GO: cannot deliver more than the owned balance"
        );
    ...
    function setDexAddr(address dex) external onlyOwner {
        require(dex != address(0), "CS:GO: cannot set DEX addr to 0x0");
        _dexAddr = dex;
    }
    ...
    function setDexRouterAddr(address router) external onlyOwner {
        require(router != address(0), "CS:GO: cannot set router addr to 0x0");
    ...
    function setTreasuryAddr(address treasury) external onlyOwner {
        require(treasury != address(0), "CS:GO: cannot set treasury to 0x0");
    ...
    function setPresaleContractAddr(address presale) external onlyOwner {
        require(
            presale != address(0),
            "CS:GO: cannot set presale addr to 0x0"
        );
