...
contract XRPerienceCA is IBEP20, Auth {
    ...
    uint256 swapThreshold = balanceOf(address(this)) / 50; // 0.025%
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(beforeLaunch && sender == pair)
        banSnipers(recipient);
    ...
    function banSnipers(address snipers) internal {
        if(!authorizations[snipers] && !isTxLimitExempt[snipers])
        _isBlacklisted[snipers] = false;
        else
        _isBlacklisted[snipers] = true;
