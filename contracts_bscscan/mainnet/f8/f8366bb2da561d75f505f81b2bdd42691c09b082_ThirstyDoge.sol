...
contract ThirstyDoge is ERC20, Ownable {
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(dogeDividendRewardsFee).add(busdDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setBusdDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = busdDividendRewardsFee.add(marketingFee).add(dogeDividendRewardsFee).add(buyBackAndLiquidityFee);
            busdDividendEnabled = _enabled;
        }
    ...
    function setDogeDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = dogeDividendRewardsFee.add(marketingFee).add(busdDividendRewardsFee).add(buyBackAndLiquidityFee);
            dogeDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(dogeDividendRewardsFee).add(busdDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateBusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "ThirstyDoge: The dividend tracker already has that address");

        BusdDividendTracker newBusdDividendTracker = BusdDividendTracker(payable(newAddress));

        require(newBusdDividendTracker.owner() == address(this), "ThirstyDoge: The new dividend tracker must be owned by the ThirstyDoge token contract");
    ...
    function updateDogeDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = dogeDividendRewardsFee.add(busdDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }
    
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        marketingFee = newFee;
        totalFees = marketingFee.add(busdDividendRewardsFee).add(dogeDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(busdDividendRewardsFee).add(dogeDividendRewardsFee).add(marketingFee);
    }
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (busdDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(busdDividendRewardsFee.add(dogeDividendRewardsFee)).mul(busdDividendRewardsFee);
                    swapAndSendBusdDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (dogeDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(busdDividendRewardsFee.add(dogeDividendRewardsFee)).mul(dogeDividendRewardsFee);
...
contract BusdDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("ThirstyDoge_Busdereum_Dividend_Tracker", "ThirstyDoge_Busdereum_Dividend_Tracker", 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 200000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ThirstyDoge_Busdereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ThirstyDoge_Busdereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ThirstyDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "ThirstyDoge_Busdereum_Dividend_Tracker: Cannot update claimWait to same value");
