...
/*
BGATE is built upon the fundamentals of Buyback and increasing the investor's value
    
Main features are
    
1) 4% tax is collected and distributed to holders for HODLing
2) 8% liquidity, buyback and marketing tax is collected and 3% of it is sent for marketing fund and othe 3% is used to buyback the tokens and 2% automated liquidity
*/
...
contract CoinToken is Context, IERC20, Ownable {
    ...
    uint liquifyFee = 2; // percent
    uint marketingFee = 3; // percent
    uint buyBackFee = 3; // percent
    bool private liquifyEnabled = true;
    uint private stakeDuration = 3;//days
    ...
    uint private botKillerLimitMinutes = 1;
    uint presaleDate;
    bool presaleMode = true;
    bool isLive = false;
    uint goLiveTime;

    uint256 public buyLimit = 2 * 10 ** 12 * 10 ** _decimals;
    uint256 public sellLimit= 500 * 10 ** 9 * 10 ** _decimals;
    ...
    constructor (address _marketingWallet, address _buyBackWallet, address _stakeWallet) {
        ...
        buyBackAddress = payable(_buyBackWallet);
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()) {
            ...
            if(!presaleMode) _botKiller(from, to, amount);
    ...
    function _botKiller(address from, address to, uint256 amount) private {
        if (botKillerLimitMinutes == 0) return;
        ...
        if (isSellOrder(from, to)) {
         ...
         if (isBuyOrder(from, to)) {
            ...
            if (goLiveTime > (NOW - 5*60*60)) {
                require(amount.mod(300) == 0, "Are you a bot");
            }
        }
    ...
    function _validateStakeTransfer(address from, address to, uint256 amount) private {
        uint NOW = block.timestamp;
        uint stakeEndTime = (NOW - stakeDuration * 24 * 60 * 60);
        if (presaleMode || from == stakeWallet) {
            if (_isExcludedFromFee[to]) return;
            require(presaleMode || stakeBalance[to] == 0 || stakeStartTime[to] == 0 || stakeStartTime[to] <= stakeEndTime, "Transfer to already staking address not allowed");
            stakeBalance[to] = amount;
            stakeStartTime[to] = presaleMode? presaleDate : NOW;
        }
        if (stakeBalance[from] > 0) {
            if(stakeStartTime[from] > stakeEndTime) 
                require(balanceOf(from) > (amount + stakeBalance[from]), "Cannot transfer staked balance");
            else {
                stakeStartTime[from] = 0;
                stakeBalance[from] = 0;
            }
        }
    }
    function setStakeWallet (address account) public onlyOwner {
        stakeWallet = account;
    }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (liquifyEnabled)  {
            uint256 amountToLiquify = contractTokenBalance.mul(liquifyFee).div(8);
            swapAndLiquify(amountToLiquify);
            // contractTokenBalance = contractTokenBalance.sub(amountToLiquify);
        }
        //  uint256 initialBalance = address(this).balance;
        IERC20 usdContract = IERC20(uniswapUSD);
        uint256 initialBalance = usdContract.balanceOf(address(this));
        swapTokensForUSD(address(this), contractTokenBalance.mul(8-liquifyFee).div(8));

        //uint256 transferredBalance = address(this).balance.sub(initialBalance);

        uint256 transferredBalance = usdContract.balanceOf(address(this)).sub(initialBalance);


        //Send to Marketing address
        // transferToAddressETH(marketingAddress, transferredBalance.div(_liquidityFee).mul(marketingDivisor));
        usdContract.transfer(marketingAddress, transferredBalance.mul(marketingFee).div(marketingFee+buyBackFee));
        usdContract.transfer(buyBackAddress, transferredBalance.mul(buyBackFee).div(marketingFee+buyBackFee));
    ...
    function afterPreSale() external onlyOwner {
        ...
        _taxFee = 4;
        _liquidityFee = 8;
        buyLimit = 2 * 10 ** 12 * 10 ** _decimals;
        sellLimit= 500 * 10 ** 9 * 10 ** _decimals;
    }
    ...
    function setBuyLimit(uint256 amount) public onlyOwner() {
        buyLimit = amount * 10 ** _decimals;
    }
    
    function setSellLimit(uint256 amount) public onlyOwner() {
        sellLimit = amount * 10 ** _decimals;
    }
    ...
    function _checkBuySellLimits(address from, address to, uint256 amount) private {
        ...
        if (isSellOrder(from, to)) {
            ...
            Sale memory _sale = dailySales[from];
            uint NOW = block.timestamp;
            _sale._total = _sale._total + amount;
            if (_sale._time < (NOW - 24 * 3600)) {
                _sale._total = amount;
            }
            _sale._time = NOW;
            dailySales[from] = _sale;
            require(_sale._total <= dailySellLimit, "Daily Sell limit exceeded");
        }
    } 
