...
contract InfinityRise is Context, IBEP20, Ownable { // contract name
    ...
    address payable public _charityWallet1 = payable(0x430F49FB39d24AF0EDdF1f0b5bb6dfa2A7AeBfDC); // charity1 address
    address payable public _burnAddress    = payable(0x000000000000000000000000000000000000dEaD); // Burn Address

    string private _name = "INFINITY RISE"; // token name
    string private _symbol = "INFINITY"; // token symbol
    ...
    uint256 private _tTotal = 100000 * 10**3 * 10**_decimals;
    ...
    uint256 private _collectedLiquidityTokens;
    ...
    uint256 public _burnFee = 50; // burn fee 5%
    uint256 private _previousBurnFee = _burnFee; // burn fee

    uint256 public _reflectionFee = 0; //reflection fee 0%
    uint256 private _previousReflectionFee = _reflectionFee; //reflection fee
    
    uint256 public _liquidityFee = 50; // actual liquidity fee 5%
    ...
    bool public swapToCharityEnabled = true; // set auto swap to BNB and send to charity.
    
    uint256 public _maxTxAmount = 5000 * 10**3 * 10**_decimals; // max allowed tokens tranfer per transaction
    uint256 public _minTokensSwapToAndTransferTo = 500 * 10**3 * 10**_decimals; // min token liquidity fee collected before swapandLiquify
    uint256 public _maxTokensPerAddress            = 100000 * 10**3 * 10**_decimals; // Max number of tokens that an address can hold
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        {   
            ...
            if(_isExcludedFromReflection[_burnAddress])
	...
	*/
    ...
    function setSwapToCharityEnabled(bool _enabled) public onlyOwner {
        swapToCharityEnabled = _enabled;
    }
    ...
    function setCharityWallets(address account1) public onlyOwner {
        _charityWallet1 = payable(account1);
    }
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        _collectedLiquidityTokens = _collectedLiquidityTokens.add(tLiquidity);
    }
    ...
    function removeAllFee() private {
        if(_burnFee == 0 && _reflectionFee == 0
    ...
    function _transfer(
        ...
        require(_isExcludedFromAntiWhale[from] || balanceOf(to) + amount <= _maxTokensPerAddress,
        ...
        uint256 liquidityTokenBalance = _collectedLiquidityTokens;
        uint256 charityTokenBalance = contractTokenBalance.sub(_collectedLiquidityTokens);
        
        if(liquidityTokenBalance >= _maxTxAmount)
        {
            liquidityTokenBalance = _maxTxAmount;
        }
        if(swapToCharityEnabled &&
            !inSwapAndLiquify &&
            from != pancakePair &&
            charityTokenBalance >= amount
            ) {
                charityTokenBalance = amount;
                swapAndSendToCharity(charityTokenBalance);
            }
        
        bool overMinTokenBalance = liquidityTokenBalance >=_minTokensSwapToAndTransferTo;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            from != pancakePair &&
            ...
            liquidityTokenBalance =_minTokensSwapToAndTransferTo;
            //add liquidity
    ...
    function swapAndLiquify(uint256 tokenBalance) private lockTheSwap {
        ...
        // swap half liquidity tokens for BNB
        ...
        _collectedLiquidityTokens = _collectedLiquidityTokens.sub(tokenBalance);
    ...
    function swapAndSendToCharity(uint256 charityTokens) private lockTheSwap {
        address[] memory path = new address[](2);
            path[0] = address(this);
            path[1] = pancakeRouter.WETH();

        _approve(address(this), address(pancakeRouter), charityTokens);
        
        uint256 swapAmount = charityTokens.div(2);
        ...
        _burn(address(this), charityTokens);
    }
