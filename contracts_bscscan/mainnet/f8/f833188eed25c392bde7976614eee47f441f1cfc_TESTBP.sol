...
contract TESTBP is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    TESTBPDividendTracker public dividendTracker;
    ...
    constructor() ERC20("TESTBP", "TBP") {
        ...
        dividendTracker = new TESTBPDividendTracker(
            payable(this),
            router,
...
contract TESTBPDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        TESTBPContract = TESTBP(mainContract);
    ...
    function _transfer(
        ...
        require(false, "TESTBP_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "TESTBP_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TESTBP contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(TESTBPContract);

            uint256 prevBalance = TESTBPContract.balanceOf(address(this));
            ...
            {
                uint256 received = TESTBPContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    TESTBPContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
