...
library SafeMath {
    ...
    function div(uint256 a, uint256 b, string memory errorMessage) internal pure returns (uint256) {
        //rue Solidity only automatically asserts when dividing by 0
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 FIL = IBEP20(0x0D8Ce2A99Bb6e3B7Db580eD848240e4a0F9aE153);
    ...
    //SETMEUP, change this to 1 MINUTES instead of 10mins
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = FIL.balanceOf(address(this));
        ...
        uint256 amount = FIL.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            FIL.transfer(shareholder, amount);
...
contract DogeShibaCake is IBEP20, Auth {
    using SafeMath for uint256;

    address FIL = 0x0D8Ce2A99Bb6e3B7Db580eD848240e4a0F9aE153;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "Doge ShibaCake";
    string constant _symbol = "DSC";
    uint8 constant _decimals = 4;

    uint256 _totalSupply = 1 * 10**9 * (10 ** _decimals);
    uint256 public _maxTxAmount = ( _totalSupply * 35 )  / 100;

    //max wallet holding of 3%
    uint256 public _maxWalletToken = ( _totalSupply * 35 ) / 100;
    ...
    uint256 public swapThreshold = _totalSupply * 10 / 1000000; // 0.01% of supply
    bool inSwap;
