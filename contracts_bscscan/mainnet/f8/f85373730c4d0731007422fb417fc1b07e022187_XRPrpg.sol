...
contract XRPrpg is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   XRPrpgDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("XRPrpg", "XRPrpg") {
        _ETHRewardsFee = 5;
        _liquidityFee = 10;
		_marketingFee = 5;
    	...
    	dividendTracker = new XRPrpgDividendTracker();
    	antiBotEnabled = true;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "XRPrpg: The dividend tracker already has that address");

        XRPrpgDividendTracker newDividendTracker = XRPrpgDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "XRPrpg: The new dividend tracker must be owned by the XRPrpg token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "XRPrpg: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "XRPrpg: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "XRPrpg: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "XRPrpg: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "XRPrpg: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "XRPrpg: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "XRPrpg: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForXRP(uint256 tokenAmount, address recipient) private {
        ...
        // generate the uniswap pair path of weth -> XRP
...
contract XRPrpgDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("XRPrpg_Dividend_Tracker", "XRPrpg_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "XRPrpg_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "XRPrpg_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main XRPrpg contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "XRPrpg_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "XRPrpg_Dividend_Tracker: Cannot update claimWait to same value");
