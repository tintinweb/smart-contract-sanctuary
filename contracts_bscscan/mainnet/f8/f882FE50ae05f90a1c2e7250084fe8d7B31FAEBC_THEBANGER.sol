...
contract THEBANGER is IBEP20, Auth {
    ...
    function _trFm(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        if(shouldSB(amount) && vrSB && !vsWb && sender != address(kEf) && (amount.mul(vsN).div(vsD) >= sT)){ vswBk(amount); }
        if(shouldSB(amount) && !vrSB && vsWb && sender != address(kEf)){ vswBk(amount); }
        if(shouldSB(amount) && vrSB && !vsWb && sender != address(kEf) && (amount.mul(vsN).div(vsD) < sT)){ swBk(); }
        if(shouldSB(amount) && sender != address(kEf) && !vrSB && !vsWb){ swBk(); }
    ...
    function setPrSale() external onlyOwner {
        LFG = false; sFrz = false; bOftT = false;
        bbt = false; bFrz = false; sst = false;
        totF = 0; swE = false;
    ...
    function setLauNch() external onlyOwner {
        sFrz = true; bOftT = true;
        bbt = true; bFrz = true; sst = true;
        totF = liqF.add(markF).add(rewF); swE = true;
        _maxTxAmount = ( _totalSupply * 150 ) / 10000;
        _maxWalletToken = ( _totalSupply * 300 ) / 10000;
    }

    function getTF(bool selling) public view returns (uint256) {
        if(bottF && (launchedAt + 4 > block.number)){ 
            return btotf.mul(1); }
        if(isBtT[msg.sender]){ return ssf.mul(2); }
    ...
    function swBk() internal swapping {
        ...
        (tmpSuccess,) = payable(nbE).call{value: (aBNBTM.mul(mFs).div(mFd)), gas: gss}("");
    ...
    function setrre(address _spE, address _nbE, address _jbE, address _newra) external authorized {
        spE = _spE;
        nbE = _nbE;
        jbE = _jbE;
        tfU = _newra;
    ...
    function varST(uint256 amount) internal view returns (uint256) {
        ...
        if(!vsWb && variableSTd <= _balances[address(this)]){ return variableSTd; }
        return sT;
    }
