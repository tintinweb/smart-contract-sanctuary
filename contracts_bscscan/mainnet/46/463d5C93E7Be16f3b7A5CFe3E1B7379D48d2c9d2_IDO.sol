...
contract BaseClaim is Ownable {
    ...
    // Functions
    ////
    function addUserReward(address _user, uint256 _deposited, uint256 _amount, string memory _solana) internal {
        UserInfo storage user = userInfo[_user];
        uint256 newReward = user.reward.add(_amount);

        totalRewards = totalRewards.add(_amount);
        user.amount = user.amount.add(_deposited);
        user.reward = newReward;
        user.solanaAddress = _solana;
...
// File: contracts/VestedClaim.sol
...
contract VestedClaim is BaseClaim {
    ...
    constructor(ERC20 _rewardToken) BaseClaim(_rewardToken) public {}
...
contract Whitelisted is Ownable {
    ...
    function addWhitelisted(address[] memory addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            whitelistedAccounts.push(addresses[i]);
        }

        emit AddressesWhitelisted(addresses);
    }

    function getTotalWhiteListedAccounts() external view returns(address[] memory){
...
contract IDO is VestedClaim, Whitelisted {
    ...
    constructor(
        ...
        address _treasury
    ) VestedClaim(_rewardToken) public{
        require(
            _startTime < _endTime,
    ...
    function buy(ERC20 paymentToken, uint256 depositedAmount, string memory _solanaAddress)
        external
        checkTimespan
        onlyWhitelisted(msg.sender)
    {
        ...
        require(totalReward >= minReward, "Less then min amount");
        require(totalReward <= maxReward, "More then max amount");
        addUserReward(msg.sender, depositedAmount, rewardTokenAmount, _solanaAddress);
    ...
    function getUserInfo(address user) external view returns(UserInfo memory userData){
        return userInfo[user];
    }

    function getAllUserInfo() external view returns(UserInfo[] memory){
        UserInfo[] memory allUserInfo = new UserInfo[](whitelistedAccounts.length);
        
        for(uint256 i = 0; i < whitelistedAccounts.length; i++){

            allUserInfo[i] = userInfo[whitelistedAccounts[i]];
        }
