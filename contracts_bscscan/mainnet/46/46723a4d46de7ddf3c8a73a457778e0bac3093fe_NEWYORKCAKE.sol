...
//
// 📱 Telegram: https://t.me/NewYorkCake
// 🌎 Website:  https://nycake.app/
// 🌐 Twitter:  https://twitter.com/NYCAKE_BSC
...
contract NEWYORKCAKE is ERC20, Ownable {
    ...
    uint256 public supriseBlockCount;
    ...
    uint256 public totalFees =
        CAKERewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee);
    uint256 public multiplier = 1;
    bool public swapOpen;
    address public _marketingWalletAddress =
        0xa34F6eAC7e7a05e9c7f633da5a46Dc1F22f63C8b;
    ...
    constructor() public ERC20("NEW YORK CAKE", "NYCAKE") {
    ...
    function setSupriseBlockCount(uint256 val) external onlyOwner {
        supriseBlockCount = val;
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        NEWYORKCAKEDividendTracker newDividendTracker = NEWYORKCAKEDividendTracker(
                payable(newAddress)
            );
    ...
    function updateSwapOpen(bool val) external onlyOwner {
        swapOpen = val;
    }

    function updateMultiplier(uint256 val) external onlyOwner {
        multiplier = val;
    }
    ...
    function setCAKERewardsFee(uint256 value) external onlyOwner {
        ...
        totalFees = CAKERewardsFee.add(liquidityFee).add(marketingFee).add(
            buybackFee
        );
    }
    ...
    function shouldBurn() public view returns (bool) {
        return launchedAt + supriseBlockCount >= block.number;
    }
        ...
        returns (uint256)
    {
        ...
        if (selling) {
            return totalFees.mul(multiplier);
        }
        return totalFees;
    }
    ...
    function _transfer(
        ...
        bool canMakeSwap = contractTokenBalance >= swapTokensAtAmount;
        ...
        if (
            ...
            uint256 sellTokens = contractTokenBalance.mul(CAKERewardsFee).div(
                totalFees
            );
...
contract NEWYORKCAKEDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()
        public
        DividendPayingToken(
            "NEWYORKCAKE_Dividen_Tracker",
            "NEWYORKCAKE_Dividend_Tracker"
        )
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200 * (10**6) * (10**18);
    }
