...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 BMARS = IBEP20(0x46880afc2E6FA41bBbE9787c082f7c23F795465E);
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = BMARS.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(BMARS);
        ...
        uint256 amount = BMARS.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            BMARS.transfer(shareholder, amount);
...
contract BabyBinamars is IBEP20, Auth {
    using SafeMath for uint256;

    address BMARS = 0x46880afc2E6FA41bBbE9787c082f7c23F795465E;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "BabyBinamars";
    string constant _symbol = "BBmars";
    ...
    bool public restrictWalletMax = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isTxLimitExempt[recipient] && restrictWalletMax)
        {
            require(_balances[recipient].add(amount) <= (getCirculatingSupply().mul(maxWalletPercent).div(100)));
        }
    ...
    function setMaxWalletPercent(uint256 newValue) external authorized {
       maxWalletPercent = newValue;
    }
    
    function setRestrictWalletMax(bool newValue) external authorized {
       restrictWalletMax = newValue;
    }
