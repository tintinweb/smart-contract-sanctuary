...
/*
 Website : https://flokibeastsanta.cc
 Telegram: https://t.me/flokibeastsanta
 */
...
contract FlokiBeastSanta is Context, IERC20, Ownable {
    ...
    string private _name = "FLOKIBEASTSANTA";
    string private _symbol = "$FBS";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000000 * 10**9 * 10**_decimals;
    
    address payable public marketingAddress = payable(0xc8b38992FD0692527E86D8DBe7dc90EC50BC5bD4);
    address public marketingWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _FBSisExcludedFromFees;
    mapping (address => bool) private _FBSisExcludedFromMaxBalance;

    uint256 private constant _FBSmaxFees = 11;
    uint256 private _FBStotalFees;
    uint256 private _FBStotalFeesContract;
    uint256 private _FBSburnFees;
    uint256 private _FBSliquidityFees;
    uint256 private _FBSmarketingFees;
    
    uint256 private _FBSmaxBalanceWallet;
    ...
    constructor () {
        ...
        _FBSisExcludedFromFees[owner()] = true;
        _FBSisExcludedFromFees[address(this)] = true;
        
        _FBSisExcludedFromMaxBalance[owner()] = true;
        _FBSisExcludedFromMaxBalance[address(this)] = true;
        _FBSisExcludedFromMaxBalance[uniswapV2Pair] = true;

        _FBSliquidityFees = 1;
        _FBSmarketingFees = 10;
        _FBSburnFees = 0;
        _FBStotalFees = _FBSliquidityFees.add(_FBSmarketingFees).add(_FBSburnFees);
        _FBStotalFeesContract = _FBSliquidityFees.add(_FBSmarketingFees);

        _liquidityThreshhold = 3000 * 10**9 * 10**_decimals;
        _FBSmaxBalanceWallet = 1000000 * 10**9 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_FBSburnFees).add(_FBSmarketingFees) <= _FBSmaxFees, "Fees are too high.");
        _FBSliquidityFees = newLiquidityFee;
        _FBStotalFees = _FBSliquidityFees.add(_FBSmarketingFees).add(_FBSburnFees);
        _FBStotalFeesContract = _FBSliquidityFees.add(_FBSmarketingFees);
        emit SetLiquidityFee(_FBSliquidityFees);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_FBSliquidityFees.add(_FBSburnFees).add(newMarketingFee) <= _FBSmaxFees, "Fees are too high.");
        _FBSmarketingFees = newMarketingFee;
        _FBStotalFees = _FBSliquidityFees.add(_FBSmarketingFees).add(_FBSburnFees);
        _FBStotalFeesContract = _FBSliquidityFees.add(_FBSmarketingFees);
        emit SetMarketingFee(_FBSmarketingFees);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_FBSliquidityFees.add(newBurnFee).add(_FBSmarketingFees) <= _FBSmaxFees, "Fees are too high.");
        _FBSburnFees = newBurnFee;
        _FBStotalFees = _FBSliquidityFees.add(_FBSmarketingFees).add(_FBSburnFees);
        emit SetBurnFee(_FBSburnFees);
    }
    ...
    function _transfer(
        ...
        // Make sure that: Balance + Buy Amount <= _FBSmaxBalanceWallet
        if(
            from != owner() &&              // Not from Owner
            to != owner() &&                // Not to Owner
            !_FBSisExcludedFromMaxBalance[to]  // is excludedFromMaxBalance
        ){
            require(
                balanceOf(to).add(amount) <= _FBSmaxBalanceWallet,
        ...
        if(
            ...
            _FBStotalFeesContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_FBSisExcludedFromFees[from] || _FBSisExcludedFromFees[to])
            && _FBStotalFees > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_FBStotalFeesContract).div(100);
            uint256 toBurnAmount = amount.mul(_FBSburnFees).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_FBSliquidityFees).div(_FBStotalFeesContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_FBSmarketingFees).div(_FBStotalFeesContract);
