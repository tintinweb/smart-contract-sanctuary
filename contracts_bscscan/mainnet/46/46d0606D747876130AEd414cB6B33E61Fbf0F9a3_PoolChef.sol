...
contract BTBar is ERC20('BTBar Token', 'FBAR'), Ownable {
    using SafeMath for uint256;
    ...
    constructor(BTToken _bt) {
        require(address(_bt) != address(0), "illegal bt");
        bt = _bt;
    }
    ...
    function safeBTTransfer(address _to, uint256 _amount) external onlyOwner {
        uint256 btBal = bt.balanceOf(address(this));
        if (_amount > btBal) {
            bt.transfer(_to, btBal);
        } else {
            bt.transfer(_to, _amount);
        }
    }
...
contract PoolChef is SafeOwnable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accBTPerShare; // Accumulated CAKEs per share, times 1e12. See below.
    }
    ...
    function pendingBT(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && tokenSupply != 0) {
            ...
            accBTPerShare = accBTPerShare.add(reward.mul(1e12).div(tokenSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accBTPerShare = pool.accBTPerShare.add(reward.mul(1e12).div(tokenSupply));
        pool.lastRewardBlock = block.number;
    }

    function safeBTTransfer(address _to, uint256 _amount) internal {
        bar.safeBTTransfer(_to, _amount);
    }
