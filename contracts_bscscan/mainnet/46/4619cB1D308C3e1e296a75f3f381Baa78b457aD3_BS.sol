...
contract BS is ERC20, Ownable {
    ...
    BSDividendTracker public dividendTracker;
    ...
    address payable public  _projectAddress = 0x38aC07a1d108A8Ae9AF80caa778Ee380d5DaA287;
    address payable public  _buybackAddress = 0x11d10260ED83597Ee214bd6de5eCEbd607bE8771;
    address payable public liquidityWallet = 0x8f151fb8416De8E65d45c6BC8ABb5dA54376298a;
    ...
    constructor() public ERC20("Babysuncoin", "BS") {
        dividendTracker = new BSDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BS: The dividend tracker already has that address");

        BSDividendTracker newDividendTracker = BSDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BS: The new dividend tracker must be owned by the BS token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BS: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BS: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }

    function BSexcludeFromDividends(address addr) public onlyOwner {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BS: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BS: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address payable newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BS: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BS: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BS: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(!blacklist[from], "BS: from in blacklist");
        require(!blacklist[to], "BS: to in blacklist");
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 h8 = newBalance.mul(8).div(96);
        _projectAddress.transfer(h8);
        _buybackAddress.transfer(h8);

        h8 = newBalance.mul(65).div(96);
...
contract BSDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("BS_Dividend_Tracker", "BS_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "BS_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BS contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BS_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BS_Dividend_Tracker: Cannot update claimWait to same value");
