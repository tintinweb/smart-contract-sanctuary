...
// SpaceEther @ https://t.me/SpaceEther
...
// File: contracts/SpaceEtherDividendPayingToken.sol
...
contract SpaceEther is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable BUSD = address(0x2170Ed0880ac9A755fd29B2688956BD959F933F8);
    address private _marketingWallet = address(0x88D17eDAc08C7111833D92B77ddfbdAE2449adf3);
    // address private _devWallet = address(0x028b567F883f4a54A763b8426cEFdd32545862D2);

    bool private swapping;
    bool public airdrop;

    SpaceEtherDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("SpaceEther", "SpaceEther") {
    	...
    	dividendTracker = new SpaceEtherDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SpaceEther: The dividend tracker already has that address");

        SpaceEtherDividendTracker newDividendTracker = SpaceEtherDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SpaceEther: The new dividend tracker must be owned by the SpaceEther token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SpaceEther: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SpaceEther: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SpaceEther: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SpaceEther: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SpaceEther: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SpaceEther: Cannot update gasForProcessing to same value");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SpaceEtherBSC: Account is already the value of 'excluded'");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "SpaceEther: This account cannot send tokens until trading is enabled");
        }
...
contract SpaceEtherDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("SpaceEther_Dividend_Tracker", "SpaceEther_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "SpaceEther_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "SpaceEther_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SpaceEther contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SpaceEther_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SpaceEther_Dividend_Tracker: Cannot update claimWait to same value");
