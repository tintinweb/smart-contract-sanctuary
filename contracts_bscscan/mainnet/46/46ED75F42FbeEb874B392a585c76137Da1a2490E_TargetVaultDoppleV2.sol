...
import "../../../interfaces/targetVaults/protocols/IDoppleFarmV2.sol";
import "../../../interfaces/targetVaults/protocols/IDoppleSwap.sol";
import "../TargetVault.sol";

contract TargetVaultDoppleV2 is TargetVault {
     ...
     * @dev Dopple Pool
     */
    IDoppleSwap public dopPool;

    /**
     * @dev Dopple Configurations
     */
    uint8 public dopPoolLength;

    /**
     * @dev Dopple Staking address
     */
    IDoppleFarmV2 public doppleStaking;
    ...
    /**
     * @dev RewardToken to Token Path
     */
    address[] public rewardTokenToTokenPath;
    ...
    constructor(
        ...
        address _dopLP,
        address _dopPool,
        uint8 _dopPoolLength,
        address _doppleStaking,
        uint256 _pid,
        address _feesCollector,
        address _swapRouterAddress,
        address _rewardToken,
        address[] memory _tokenToBuyBackPath,
        address[] memory _rewardTokenToTokenPath
    ) public {
        token = IERC20(_token);
        feedVault = _feedVault;
        dopLP = IERC20(_dopLP);
        dopPool = IDoppleSwap(_dopPool);
        dopPoolLength = _dopPoolLength;
        doppleStaking = IDoppleFarmV2(_doppleStaking);
        pid = _pid;
        ...
        rewardTokenToTokenPath = _rewardTokenToTokenPath;
    }

    /**
     * @dev Events
     */
    event TokenToBuyBackPathChanged(address[] _tokenToBuyBackPath);
    event RewardTokenToTokenPathChanged(address[] _rewardTokenToTokenPath);
    ...
    function _deposit() internal virtual {
        ...
        if (_balance > 0) {
            ...
            doppleStaking.deposit(pid, _balance);
    ...
    function withdraw(uint256 _amount) external virtual onlyVault {
        if (_amount > 0) {
            ...
            doppleStaking.withdraw(pid, _amount);
    ...
    function _harvest() internal virtual {
        doppleStaking.deposit(pid, 0);
        uint256 swapAmt = IERC20(rewardToken).balanceOf(address(this));
        if (swapAmt > 0) {
            IERC20(rewardToken).safeIncreaseAllowance(swapRouterAddress, swapAmt);
            uint256[] memory amounts = IPancakeRouter02(swapRouterAddress).getAmountsOut(swapAmt, rewardTokenToTokenPath);
            uint256 amountOut = amounts[amounts.length.sub(1)];
            IPancakeRouter02(swapRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(
                swapAmt,
                amountOut.mul(slippageFactor).div(1000),
                rewardTokenToTokenPath,
                address(this),
                block.timestamp + 120
            );
    ...
    function emergencyWithdrawAll() external virtual onlyOwner {
        doppleStaking.emergencyWithdraw(pid);
    ...
    function balanceOfToken() public view virtual returns (uint256) {
        uint256 _tokenInVault = 0;
        if (vaultBalance() > 0) {
            _tokenInVault = dopPool.calculateRemoveLiquidityOneToken(address(this), vaultBalance(), getDoppleTokenIndex());
        }
        return availableBalance().add(_tokenInVault);
    }
    ...
    function vaultBalance() public view virtual returns (uint256) {
        (uint256 _amount, uint256 _rewardDebt) = doppleStaking.userInfo(pid, address(this));
    ...
    function vaultToken() public view virtual returns (address) {
        return address(dopPool.getToken(getDoppleTokenIndex()));
    }

    /**
     * @dev Get Dopple token index
     */
    function getDoppleTokenIndex() public view returns (uint8) {
        return dopPool.getTokenIndex(address(token));
    }
     ...
     */
    function setTokenToBuyBackPath(address[] memory _tokenToBuyBackPath) external virtual onlyOwner {
        tokenToBuyBackPath = _tokenToBuyBackPath;

        emit TokenToBuyBackPathChanged(tokenToBuyBackPath);
    }

    /**
     * @dev Set reward token to token path
     */
    function setRewardTokenToTokenPath(address[] memory _rewardTokenToTokenPath) external virtual onlyOwner {
        rewardTokenToTokenPath = _rewardTokenToTokenPath;

        emit RewardTokenToTokenPathChanged(rewardTokenToTokenPath);
    }
}
