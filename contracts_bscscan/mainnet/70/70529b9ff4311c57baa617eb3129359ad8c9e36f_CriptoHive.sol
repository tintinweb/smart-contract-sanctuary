...
contract CriptoHive is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x9078835148AE1f412722008F2386f15251A21071);
    address payable private _DevAddress;

    uint256 public CTHive_MAX = 2881;
    ...
    Counters.Counter private _publicCTHive;

    constructor() ERC721("CriptoHive", "CH") {
        _DevAddress = payable(msg.sender);
    }

    function PRICE() public view returns(uint256){
        if (_publicCTHive.current() < 288) return 2_000_000_000_000_000;//0.002BNB
        else if (_publicCTHive.current() < 1000) return 10_000_000_000_000_000;//0.01BNB
        else return 20_000_000_000_000_000;//0.02BNB
    }
    ...
    function setCTHive_MAX(uint256 newCTHive_MAX) external onlyOwner {
		require(newCTHive_MAX < CTHive_MAX,
				"Supply can only be increased"
				);
        CTHive_MAX = newCTHive_MAX;
    }

    function gift(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicCTHive.current();

            if (_publicCTHive.current() < CTHive_MAX) {
                _publicCTHive.increment();
                _safeMint(to, tokenId);
            }
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(
            _publicCTHive.current() < CTHive_MAX,
            "Purchase would exceed CTHive_MAX"
        );
        require(
