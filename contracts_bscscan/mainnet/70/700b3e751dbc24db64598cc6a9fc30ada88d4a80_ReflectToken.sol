...
contract ReflectToken is IERC20, Ownable
{
    ...
    address public constant LIQUIDITY_ADDRESS = 0x000000000000000000000000000000000000dEaD;    
    uint256 public constant PERCENT_FACTOR = 10000; //100%

    //========================
    // ATTRIBUTES
    //========================
    
    IERC20 public immutable wBNB; //wrapped BNB
    ...
    //fees    
    uint256 public maxFee = 3500; //max fee percent (35%)
    ...
    FeeInfo public emptyFees; //all 0
    ...
    uint256 private swapTokensAtHigh; //liquify trigger high
    uint256 private swapTokensAtLow; //liquify trigger high
    ...
    {
        //dummy
        IUniRouterV2 _router = IUniRouterV2(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        ...
        decimals = 18;
        tTotal = 100000 ether;        
        minRemainingTokens = 0 ether;
        swapTokensAtLow = 0.05 ether;
        swapTokensAtHigh = swapTokensAtLow.mul(3);        
        ...
        address newOwner = 0x597aDd1B26e5F8311FEc6863e12c99fD65634747;//msg.sender;
        UserInfo storage user = userMap[newOwner];
        user.rOwned = rTotal;
        user.excludedFromFee = true;
        emit Transfer(address(0), newOwner, tTotal);

        //exclude from fee or reward        
    ...
    function processFees(uint256 _amount) private
    {
        ...
        accumulatedMarketingFee = accumulatedLiquidityFee.sub(tokenForMarketing);
        accumulatedTeamFee = accumulatedLiquidityFee.sub(tokenForTeam);
    ...
    function swapForBNB(uint256 _amount) private lockTheSwap
    {
        uint256 initialBalance = address(this).balance;
    ...
    function getTValues(uint256 _tAmount, FeeInfo memory _fees) private pure returns (tFeeValues memory)
    {
        tFeeValues memory tValues = tFeeValues(
            0,
            calculateFee(_tAmount, _fees.reflectionFee),
            calculateFee(_tAmount, _fees.liquidityFee),
            calculateFee(_tAmount, _fees.teamFee),
            calculateFee(_tAmount, _fees.marketingFee),
            calculateFee(_tAmount, _fees.burnFee)
        );
    ...
    function getTransferAmount(uint256 _amount, address _from) private view returns (uint256)
    {
        ...
        if (_from != address(router)
            ...
            && _from != address(this))
        {
            if (_amount > minRemainingTokens)
            {
                transferAmount = transferAmount.sub(minRemainingTokens);
            }
            else
            {
                transferAmount = 0;
    ...
    function _transfer(address _from, address _to, uint256 _amount) private
    {
        ...
        bool overMinTokenBalance = contractTokenBalance >= swapTokensAtHigh;
        ...
        if (takeFeeOnTransfer)        
        {
            ...
            else if (_from == address(routerPair))
            {
                fees = sellFees;
            }
            else
