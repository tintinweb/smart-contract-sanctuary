...
*/
//
// Any buy might trigger the BitBomb!
// Weekly GiveAway as well!
//
// $BitBomb has 12% tax
//
// BitBomb      2%: Automatic purchase lottery
// Reflections  2%: Reflections
// Weekly       2%: Weekly GiveAway
// LP           3%: LP
...
/**
 * @dev The official BitBomb smart contract
 */
contract BitBomb is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    // General Info
    string private _name     = "BitBombTEST";
    string private _symbol   = "TESTBMB";
    uint8  private _decimals = 9;

    // Liquidity Settings
    IUniswapV2Router02 public _pancakeswapV2Router; // The address of the PancakeSwap V2 Router
    address public _pancakeswapV2LiquidityPair;     // The address of the PancakeSwap V2 liquidity pairing for BitBomb/WBNB
    ...
    // Addresses
    address payable public _marketingAddress = payable(0x7DC02BEd715A79B9444fB30DC48743529cd8120C); // Marketing address used to pay for marketing
    ...
    uint256 public _tokenSwapThreshold = 0.1 * 10**9 * 10**9; // 100 million

    // Timer Constants
    uint private constant DAY = 86400; // How many seconds in a day

    // Anti-Whale Settings
    uint256 public _whaleSellThreshold = 0.05 * 10**9 * 10**9; // 50 million
    ...
    uint256 public _buybackUpperLimit    = 1 * 10**9 * 10**9; // 1 billion $BitBomb will be used to buy at maximum in any one trade
    ...
    // LOTTERY
    bool    public  _enableLottery       = true;              // Disabled on deployment
    ...
    uint256 public  _lotteryThreshold    = 0.01 * 10**6 * 10**9; // 10K tokens required to be in the pool before lottery can be triggered
    uint256 public  _lotteryMinimumSpend = 0.01 * 10**6 * 10**9; // 10K tokens required to buy before before lottery can be triggered
    ...
    /**
     * @notice Withdraws non-BitBomb tokens that are stuck as to not interfere with the liquidity
     */
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // Gets the contracts BitBomb balance for buybacks, charity, liquidity and marketing
    ...
    /**
     * @notice Burns BitBomb tokens straight to the burn address
     */
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockSwapping {
        ...
        // Swap 7/8ths of BitBomb tokens for BNB
        ...
        // We now have 6/7ths left of BNB (converted from BitBomb)
    ...
    function swapBNBForTokens(uint256 amount) private {
        ...
        _pancakeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(
            0, // Accept any amount of BitBomb
            path,
