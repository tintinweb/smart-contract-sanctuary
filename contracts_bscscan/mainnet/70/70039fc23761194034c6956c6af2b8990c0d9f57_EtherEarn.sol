...
/**
  ...
  TG: https://t.me/EtherEarn
 
 * lp is locked for 7 days 
 * 6% (auto)reward Eth
 * 5% max wallet
 * 3% max BUY 
 * 4% lp
...
contract EtherEarn is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    EtherEarnDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("EtherEarn", "$Ethern") {
        _ETHRewardsFee = 6;
        _liquidityFee = 4;
		_marketingFee = 3;
        ...
        maxBuyTranscationAmount = 4000000000 * (10**18);
        maxSellTransactionAmount = 4000000000 * (10**18);
        swapTokensAtAmount = 20000000 * (10**18);
        _maxWalletToken = 5000000000 * (10**18); // 5% of total supply


    	dividendTracker = new EtherEarnDividendTracker();
    	antiBotEnabled = true;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "EtherEarn: The dividend tracker already has that address");

        EtherEarnDividendTracker newDividendTracker = EtherEarnDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "EtherEarn: The new dividend tracker must be owned by the EtherEarn token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "EtherEarn: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "EtherEarn: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "EtherEarn: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "EtherEarn: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "EtherEarn: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "EtherEarn: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "EtherEarn: Cannot update gasForProcessing to same value");
...
contract EtherEarnDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("EtherEarnDividend_Tracker", "EtherEarn_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "EtherEarn_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "EtherEarnDividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherEarn contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "EtherEarn_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EtherEarn_Dividend_Tracker: Cannot update claimWait to same value");
