...
// https://t.me/MiniDickbsc
// For the first time you don't have to be ashamed for haaving a MiniDick!
//
//  ..... ▄▄ ▄▄
//  ......▄▌▒▒▀▒▒▐▄
//  .... ▐▒▒▒▒▒▒▒▒▒▌
...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public immutable INCH = 0x111111111117dC0aa78b770fA6A738034120C302;
  ...
  function distributeINCHDividends(uint256 amount) public onlyOwner{
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      bool success = IERC20(INCH).transfer(user, _withdrawableDividend);
...
contract MiniDick is ERC20, Ownable {
	using SafeMath for uint256;
	...
	MINIDDividendTracker public dividendTracker;
	MINIDTransferHelper private transferHelper;
	...
	constructor(address routerAddress, address payable marketingWalletAddress, address payable devWalletAddress) public ERC20("MiniDick", "MINID") {
		dividendTracker = new MINIDDividendTracker();
		transferHelper = new MINIDTransferHelper(routerAddress);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "MINID: The dividend tracker already has that address");

		MINIDDividendTracker newDividendTracker = MINIDDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "MINID: The new dividend tracker must be owned by the MINID token contract");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "MINID: The router already has that address");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "MINID: Account is already the value of 'excluded'");
	...
	function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
		require(pair != uniswapV2Pair, "MINID: The PanINCHSwap pair cannot be removed from automatedMarketMakerPairs");
	...
	function setMaxWalletAmount(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "MINID: Amount cannot be over the total supply.");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "MINID: Automated market maker pair is already set to that value");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "MINID: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "MINID: Cannot update gasForProcessing to same value");
	...
	function swapAndSendDividends(uint256 amountIn) private {
		uint256 dividends = swapExactETHForTokens(amountIn, INCH);
...
contract MINIDDividendTracker is Ownable, DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() public DividendPayingToken("MINID_Dividend_Tracker", "MINID_Dividend_Tracker") {

	}

	function _transfer(address, address, uint256) internal override {
		require(false, "MINID_Dividend_Tracker: No transfers allowed");
	}

	function withdrawDividend() public override {
		require(false, "MINID_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MINID contract.");
	}
	...
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "MINID_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "MINID_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MINIDTransferHelper is Ownable {
