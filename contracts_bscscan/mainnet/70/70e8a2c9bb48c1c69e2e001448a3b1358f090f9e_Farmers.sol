...
// File: src/contracts/Farmers.sol
...
contract Farmers is ERC721, ERC721Enumerable, ERC721URIStorage {
  using SafeMath for uint256;

  // Initiatialize
  constructor() ERC721("Farmers", "Farmers") {
    nextGrow = 0;
    mintPrice = 1000; //Binaf
    growPrice = 10; //Seed
  ...
  function setBinafAddr(address a) public {
    require(msg.sender == deployerWallet);
    BinafAddr = a;
  }
  function setSeedAddr(address a) public {
    require(msg.sender == deployerWallet);
    SeedAddr = a;
  }
  ...
  function mintWithFee(string memory _name, string memory _tokenURI) public {
    ...
    address FarmersAddr = address(this);
    uint256 senderBalance = Binaf.balanceOf(senderAddr);
    uint256 allowance = Binaf.allowance(senderAddr, FarmersAddr);
    ...
    Binaf.transferFrom(senderAddr, deadWallet, onePercent); // <= Hasta aca OK!
    // // send 49% to rewards wallet
    Binaf.transferFrom(senderAddr, rewardsWallet, restOtherHalf); // <=
    // // send 50% to liquidity
    Binaf.transferFrom(senderAddr, liquidityWallet, half);
  ...
  function grow(uint256 _tokenIdx) public {
    ...
    IBinaf Seed = IBinaf(SeedAddr);
    address FarmersAddr = address(this);
    address senderAddr = address(msg.sender);
    uint256 tokenId = _tokenIdx.add(1);
    uint256 senderBalance = Seed.balanceOf(senderAddr);
    uint256 allowance = Seed.allowance(senderAddr, FarmersAddr);
    ...
    Seed.transferFrom(senderAddr, deadWallet, growPrice);
  }
  ...
  function buy(uint256 _tokenId) public {
    ...
    address FarmersAddr = address(this);
    uint256 buyerBalance = Binaf.balanceOf(buyerAddr);
    uint256 allowance = Binaf.allowance(buyerAddr, FarmersAddr);
    ...
    Binaf.transferFrom(buyerAddr, initialOwner, price);
  }
