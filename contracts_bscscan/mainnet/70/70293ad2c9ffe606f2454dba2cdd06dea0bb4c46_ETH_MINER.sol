...
contract ETH_MINER {
    ...
    function sellEggs() public {
        ...
        ERC20(eth).transfer(ceoAddress, fee2);
        ERC20(eth).transfer(ceoAddress2, fee-fee2);
        ERC20(eth).transfer(address(msg.sender), SafeMath.sub(eggValue,fee));
    }
    function buyEggs(address ref, uint256 amount) public {
        ...
        uint256 balance = ERC20(eth).balanceOf(address(this));
    ...
    function calculateEggSell(uint256 eggs) public view returns(uint256) {
        return calculateTrade(eggs,marketEggs,ERC20(eth).balanceOf(address(this)));
    }
    function calculateEggBuy(uint256 ethr,uint256 contractBalance) public view returns(uint256) {
        return calculateTrade(ethr,contractBalance,marketEggs);
    }
    function calculateEggBuySimple(uint256 ethr) public view returns(uint256){
        return calculateEggBuy(ethr,ERC20(eth).balanceOf(address(this)));
    }
