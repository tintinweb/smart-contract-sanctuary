...
// File: contracts/BIFI/strategies/degens/ZefiCard.sol
...
contract ZefiCard is Ownable {
    ...
    */
    ...
    // address[] public cbrlToBtcRoute = [cbrl, btcb];
    // address[] public wbnbTocbrlRoute = [wbnb, btcb, cbrl];
    address[] tokenToWbnbRoute;
    address[] wbnbTocbrlRoute;
    address[] wbnbToZefiRoute;
    ...
    function tokenToReais(
        ...
        // _tokeToWbnbRoute = [_token, wbnb];
        IUniswapRouter(_dexrouter).swapExactTokensForTokens(_toReais, 0, _tokenToWbnbRoute, address(this), now.add(600));
        
        uint256 wbnbBal = IERC20(wbnb).balanceOf(address(this));        

        uint256 wbnbBalUser = wbnbBal.mul(950).div(1000);
        IUniswapRouter(unirouter).swapExactTokensForTokens(wbnbBalUser, 0, wbnbTocbrlRoute, _touser, now.add(600));
        
        uint256 wbnbBalBuyback = wbnbBal.mul(40).div(1000);
        IUniswapRouter(unirouter).swapExactTokensForTokens(wbnbBalBuyback, 0, wbnbToZefiRoute, dead, now.add(600));
        
        uint256 wbnbBalReferral = wbnbBal.mul(10).div(1000);
        IUniswapRouter(unirouter).swapExactTokensForTokens(wbnbBalReferral, 0, wbnbToZefiRoute, _referral, now.add(600));        


/* more fee

        wantToCbrlRoute = [_token, wbnb, btcb, cbrl];
        wantToZefiRoute = [_token, wbnb, zefi];
        
        uint256 tokenBal = IERC20(_token).balanceOf(address(this));

        uint256 tokenBalUser = tokenBal.mul(950).div(1000);
        IUniswapRouter(unirouter).swapExactTokensForTokens(tokenBalUser, 0, wantToCbrlRoute, msg.sender, now.add(600));
        
        uint256 tokenBalBuyback = tokenBal.mul(50).div(1000);
        IUniswapRouter(unirouter).swapExactTokensForTokens(tokenBalBuyback, 0, wantToZefiRoute, dead, now.add(600));
*/
    }
