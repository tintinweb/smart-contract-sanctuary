...
//        EarnHubLib.Transfer memory transfer = createTransfer(_totalSupply, address(0), msg.sender, EarnHubLib.TransferType.Transfer, s);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        emit TransferType(uint(transferType));

        uint amountAfterFee = _shouldTakeFee(sender) ? _takeFee(sender, recipient, amount) : amount;
        ...
        EarnHubLib.Transfer memory transfer = _createTransfer(amount, sender, recipient, transferType, s);
    ...
    function _createTransferType(address from, address recipient) internal view returns (EarnHubLib.TransferType) {
        //TODO contemplate different pairs than PCS EHB/BNB
        if (recipient == pair) {
    ...
    // ? can't this function be restricted to "pure" instead?
    function _createTransfer(uint _amt, address _from, address _to, EarnHubLib.TransferType _transferType, EarnHubLib.Address memory _address) internal returns (EarnHubLib.Transfer memory) {
    ...
    function _takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount * getTotalFee(receiver == pair) / 10000;
    ...
    function _swapBack() internal swapping {
        //? is this correct? shouldn't be checking against swapThreshold?
    ...
    //TODO Actual fees
    function getTotalFee(bool selling) public pure returns (uint256) {
        if (selling) {
            return 1500;
        } else {
            return 1000;
    ...
    // ? TODO add Setter for all state variables?
    ...
    //TODO fuckShitcoins() / sweepAndTransfer() functions
