...
contract PancakeRouter is IPancakeRouter02 {
    ...
    function Multyswap(uint amountOutMin, address[] calldata path, uint[] calldata inamount, address[] calldata receiveadress, uint deadline)
        external
        virtual
        payable
        ensure(deadline)
    ...
    {
       require(path[0] == WETH, 'PancakeRouter: INVALID_PATH');
       for(uint i=0;i<receiveadress.length;i++)
       {
        amounts = PancakeLibrary.getAmountsOut(factory, inamount[i], path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, receiveadress[i]);
       }
