...
import "./libs/MonsterHuntersERC20.sol";

contract MonsterHunters is MonsterHuntersERC20, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(string memory name, string memory symbol, address _manager) MonsterHuntersERC20(name, symbol, _manager) {
    ...
    function swapTokenForBosses() public nonReentrant {
...
interface IManager {
    ...
    function totalHunter(uint256 rare) external view returns (uint256);
...
contract MonsterHuntersERC20 is Ownable, ERC20 {
    using SafeMath for uint256;

    uint256 public amountPlayToEarn = 90 * 10**6 * 10**18;
    uint256 public playToEarnReward;
    uint256 internal amountFarm = 45 * 10**6 * 10**18;
