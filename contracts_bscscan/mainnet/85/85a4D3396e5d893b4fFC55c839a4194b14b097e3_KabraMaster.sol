...
interface IStrategy {
    ...
    // Transfer want tokens KabraFarm -> strategy
    function deposit(uint256 _wantAmt) external returns (uint256);

    // Transfer want tokens strategy -> KabraFarm
...
// File: contracts/KabraMaster.sol

pragma solidity 0.6.12;

contract KabraMaster is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // @TODO: Add KabraToken address to here
    address public constant earningToken =
        0x27C976ECD4c8D871B0f1371529eEdAc6651894FD;
    ...
    uint16 public constant MAX_DEPOSIT_FEE_BP = 500;
        ...
        returns (uint256)
    {
        ...
        if (blockNumber > pool.lastRewardBlock && wantLockedTotal != 0) {
            ...
            uint256 earningsReward = multiplier
                .mul(EarningsPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
        ...
        return
            user.amount.mul(accEarningsPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        KabraToken(earningToken).mint(
            devaddr,
            multiplier.mul(EarningsDevPerBlock).mul(pool.allocPoint).div(totalAllocPoint)            
        );

        KabraToken(earningToken).mint(address(this), earningsReward);
    ...
    function deposit(
        ...
        if (_wantAmt > 0) {
            ...
            uint256 amountDeposit = IStrategy(poolInfo[_pid].strat).deposit(
                amount
            );
        ...
        poolExists(_pid)
    {
        ...
        if (_wantAmt > 0) {
            ...
            if (pool.isWithdrawFee) {
                ...
                if (withdrawFeeBP > 0) {
                    uint256 withdrawFee = _wantAmt.mul(withdrawFeeBP).div(
                        10000
                    );
    ...
    function setEarningsReferral(IEarningsReferral _earningReferral)
        public
        onlyOwner
    {
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                KabraToken(earningToken).mint(referrer, commissionAmount);
    ...
    function getWithdrawFee(uint256 _pid, address _user)
        public
        view
        returns (uint16)
    {
        ...
        for (; i < withdrawalFeeIntervals.length; i++) {
            if (blockElapsed < withdrawalFeeIntervals[i]) break;
        }
        return withdrawalFeeBP[i];
    }

    function setWithdrawFee(
        uint256[] memory _withdrawalFeeIntervals,
        uint16[] memory _withdrawalFeeBP
    ) public onlyOwner {
        require(
            _withdrawalFeeIntervals.length + 1 == _withdrawalFeeBP.length,
            "setWithdrawFee: _withdrawalFeeBP length is one more than _withdrawalFeeIntervals length"
        );
        require(
            _withdrawalFeeBP.length > 0,
            "setWithdrawFee: _withdrawalFeeBP length is one more than 0"
        );
        for (uint256 i = 0; i < _withdrawalFeeIntervals.length - 1; i++) {
            require(
                _withdrawalFeeIntervals[i] < _withdrawalFeeIntervals[i + 1],
                "setWithdrawFee: The interval must be ascending"
            );
        }
        for (uint256 i = 0; i < _withdrawalFeeBP.length; i++) {
            require(
                _withdrawalFeeBP[i] <= MAX_WITHDRAWAL_FEE_BP,
