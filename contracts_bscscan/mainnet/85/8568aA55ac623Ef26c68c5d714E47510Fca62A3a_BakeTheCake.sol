...
contract BakeTheCake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("BakeTheCake 2.0", "BAKECAKE") {
    ...
    receive() external payable {
  	...
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "BakeTheCake: The team wallet is already this address");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "BakeTheCake: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "BakeTheCake: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "BakeTheCake: The new dividend tracker must be owned by the BakeTheCake token contract");
    ...
    function updateBakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(bakeDividendTracker), "BakeTheCake: The dividend tracker already has that address");

        BakeDividendTracker newBakeDividendTracker = BakeDividendTracker(payable(newAddress));

        require(newBakeDividendTracker.owner() == address(this), "BakeTheCake: The new dividend tracker must be owned by the BakeTheCake token contract");
    ...
    function updateCakeDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "BakeTheCake: Fee must be less than 6%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "BakeTheCake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "BakeTheCake: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BakeTheCake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "BakeTheCake: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "BakeTheCake: Cannot update gasForProcessing to same value");
	...
	function processDividendTracker(uint256 gas) external onlyOwner {
		...
		(uint256 dogeBackIterations, uint256 dogeBackClaims, uint256 dogeBackLastProcessedIndex) = bakeDividendTracker.process(gas);
		emit ProcessedBakeDividendTracker(dogeBackIterations, dogeBackClaims, dogeBackLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "BakeTheCake: Trading has not started yet");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BakeTheCake_Cakeereum_Dividend_Tracker", "BakeTheCake_Cakeereum_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BakeTheCake_Cakeereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BakeTheCake_Cakeereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BakeTheCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "BakeTheCake_Cakeereum_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BakeTheCake_Bake_Dividend_Tracker", "BakeTheCake_Bake_Dividend_Tracker", 0xE02dF9e3e622DeBdD69fb838bB799E3F168902c5) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BakeTheCake_Bake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BakeTheCake_Bake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BakeTheCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "BakeTheCake_Bake_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
