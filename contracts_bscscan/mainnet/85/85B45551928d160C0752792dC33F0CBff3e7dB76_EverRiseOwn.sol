...
contract EverRiseOwn is Context, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public contractAddress = payable(0xC7D43F2B51F44f09fBB8a691a0451E8FFCF36c0a);

    address public immutable uniswapV2Pair;
    mapping (address => address) private _ownerShips;
    string private _name = "EverRiseOwn";
    string private _symbol = "RISEOWN";
    ...
    constructor () {
        ...
        token = EverRise(contractAddress);
    ...
    function setMaxTxAmount(uint256 maxTxAmount) public onlyOwner {
        token.setMaxTxAmount(maxTxAmount);
    }
    
    function setMarketingDivisor(uint256 divisor) public onlyOwner {
        token.setMarketingDivisor(divisor);
    }
    
    function setNumTokensSellToAddToLiquidity(uint256 _minimumTokensBeforeSwap) public onlyOwner {
        token.setNumTokensSellToAddToLiquidity(_minimumTokensBeforeSwap);
    }
    
    function setBuybackUpperLimit(uint256 buyBackLimit) public onlyBuybackOwner {
        token.setBuybackUpperLimit(buyBackLimit);
    }
    
    function setMarketingAddress(address _marketingAddress) public onlyOwner {
        token.setMarketingAddress(_marketingAddress);
    }
    
    function setBuyBackEnabled(bool _enabled) public onlyBuybackOwner {
        token.setBuyBackEnabled(_enabled);
    }
    
    function buyBack(uint256 amount, uint256 chunks) public onlyBuybackOwner {
