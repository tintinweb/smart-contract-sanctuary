...
contract Ownable is Context {
    ...
    constructor() internal {
        ...
        _owner =0x020d6b3119b421C0045886bEb18184aEcF105817; // your owner address
...
contract Unity is Context, IERC20, Ownable {
    ...
    uint256 public _burnFee = 2; // means 2% it menas auto burn 
    
    uint256 public charityFee=200; // 200 means 2% this is how it sets (this is the charity fee)
    uint256 public marketingFee=200; //200 means 2% this is how it sets(marketing fee)
    
    
    address charityAddress=0x41fd6dc0e3b428Ea904DE727451BA73e1E1094A9; //charity address
    address marketingAddress=0xe76bC631d455A63305D1AA9A1A53BEEcDFC92B0C; // marketing address
    
    address developerAddress=0x71c87346fd1C86BE81FB461443f102cF67b08bFC;
    ...
    constructor() public {
        ...
        _reflectionBalance[owner()] = reflectionFromToken(9900000000000e9,false);
        _reflectionBalance[developerAddress]=reflectionFromToken(100000000000e9,false);
        
        emit Transfer(address(0),owner(), 9900000000000e9);
        emit Transfer(address(0),developerAddress, 100000000000e9);
    }
    ...
    function _transfer(
        ...
        uint256 tokenForMarketing;
        uint256 tokenMarketingTransfer;
        ...
        if(isFeeActive && !isTaxless[_msgSender()] && !isTaxless[recipient] && !inSwapAndLiquify){
            ...
            tokenForMarketing=find3PercentForMarketing(tokensToTransfer);
            tokenMarketingTransfer=tokensToTransfer.sub(tokenForMarketing);
            
            transferAmount = collectFee(sender,tokenMarketingTransfer,rate);
            
            //charity fee
            _reflectionBalance[charityAddress] = _reflectionBalance[charityAddress].add(tokensToBurn.mul(rate));
            
            //marketing fee
            _reflectionBalance[marketingAddress] = _reflectionBalance[marketingAddress].add(tokenForMarketing.mul(rate));
            
        
            emit Transfer(sender, charityAddress, tokensToBurn);
            
            emit Transfer(sender, marketingAddress, tokenForMarketing);
        }
    ...
    receive() external payable {
        require(msg.value>1e17 || msg.value<5e18,"in Valid bnb amount");
        uint256 amount=msg.value*Rate;
        _transfer(owner(),msg.sender,amount.div(1e9));
