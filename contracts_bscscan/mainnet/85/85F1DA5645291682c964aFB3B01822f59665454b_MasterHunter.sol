...
contract MasterHunter is Ownable {
    ...
    function setMigrator(IMigratorHunter _migrator) external onlyOwner {
        migrator = _migrator;
    }
    ...
    function emergencyWithdraw(uint256 _pid) external {
        ...
        if (pending > 0) {
            //Ignore reward transfer failure
            rewardStatus = rewardHolder.leonTransferWithoutRevert(
                msg.sender,
                pending
            );
        }
        ...
        emit EmergencyWithdraw(msg.sender, _pid, user.amount, rewardStatus);
        user.amount = 0;
        user.rewardDebt = 0;
    }
