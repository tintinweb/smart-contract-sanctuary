...
contract LaunchPlanet is ERC20, Ownable {
    // CONFIG START
    
    // TOKEN
    string tokenName = "LaunchPlanet";
    string tokenSymbol = "LPLAN";
    uint256 tokenTotalSupply = 1_000_000_000 * (10**18);
    
    // ADRESSES
    address marketingWallet = 0xF22E78b3bac3Da4F05BC63AB0acFc02737680091;
    ...
    // BOT PREVENTION
    uint256 maxTXLimit = 4; // Max tx amount will be active for x blocks
    uint256 revertLimit = 3; // Every tx in x amount of blocks will be reverted
    
    // WHALE PROTECTION
    uint256 maxTXAmount = 20_000_000 * (10**18);
    ...
    function exclude(address account) public onlyOwner {
        require(!isExcluded[account], "LPLAN: Account already excluded");

        isExcluded[account] = true;
    }

    function unExclude(address account) public onlyOwner {
        require(isExcluded[account], "LPLAN: Account is not excluded");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override virtual {
        ...
        if(!isExcluded[recipient] && sender == address(_UniswapV2Pair)) {
            ...
            if(block.number <= startBlock + 3) {
                ...
                if(amount > tokenTotalSupply / 1000 * 40) {
                    revert();
                }
            }
        }
