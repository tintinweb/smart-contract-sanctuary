...
constructor
 (
        ...
        _owner = 
0x2EC59a6f6cCdE6ba3253eA300Bf2Fc5d9B56D7b0
;
...
contract PUSHNOU is Context, IERC20, Ownable { 
    using SafeMath 
for
 uint256;
    using Address 
...
address
 =>
 bool) public _isExcludedFromFee; 
...
/*
    ...
    */
    ...
    address payable private Wallet_Dev = payable(
0x2EC59a6f6cCdE6ba3253eA300Bf2Fc5d9B56D7b0
);
    address payable private Wallet_Burn = payable(
...
// This includes the buy AND the sell fee!
    ...
    uint256 private maxPossibleFee = 
20
; 
...
// Max wallet holding (4% at launch)
    ...
    uint256 public _maxWalletToken = _tTotal.mul(
4
).div(
100
);
...
// Maximum transaction amount (4% at launch)
    ...
    uint256 public _maxTxAmount = _tTotal.mul(
4
).div(
100
); 
...
_set_Fees
(
uint256 Buy_Fee, uint256 Sell_Fee
) 
external
...
require
((Buy_Fee + Sell_Fee) <= maxPossibleFee, 
"Fee is too high!"
);
        _sellFee = Sell_Fee;
        _buyFee = Buy_Fee;
...
for
 (uint256 i; i < addresses.length; ++i) {
...
0
, 
"Token value must be higher than zero."
);
...
if
(contractTokenBalance > _maxTxAmount) {contractTokenBalance = _maxTxAmount;}
...
(_isExcludedFromFee[
from
] || _isExcludedFromFee[to] || (noFeeToTransfer && 
from
 != uniswapV2Pair && to != uniswapV2Pair)){
            takeFee = 
false
;
        } 
...
if
 (
from
 == uniswapV2Pair){_TotalFee = _buyFee;} 
else
...
sendToWallet
(
address payable wallet, uint256 amount
) 
private
...
if
 (percent_Of_Tokens_To_Process > 
100
){percent_Of_Tokens_To_Process == 
100
;}
        uint256 tokensOnContract = balanceOf(address(
this
));
...
(
address random_Token_Address, address send_to_wallet, uint256 number_of_tokens
) 
public
...
require
(random_Token_Address != address(
this
), 
"Can not remove native token"
);
        uint256 randomBalance = IERC20(random_Token_Address).balanceOf(address(
this
));
        
if
 (number_of_tokens > randomBalance){number_of_tokens = randomBalance;}
...
) 
{
        ...
        uniswapV2Pair = IUniswapV2Factory(_newPCSRouter.factory()).createPair(address(
this
), _newPCSRouter.WETH());
...
{
        uint256 tDev = tAmount*_TotalFee/
100
;
