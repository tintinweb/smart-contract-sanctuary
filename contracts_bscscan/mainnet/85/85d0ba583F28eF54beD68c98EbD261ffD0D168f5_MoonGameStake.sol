...
contract MoonGameStake is IReflectionPool, Auth, Pausable {
    ...
    constructor (address _stakingToken, address _rewardsToken) Auth (msg.sender) {
    ...
    function leaveStaking(uint amt) external {
        ...
        require(block.timestamp >= stakingTime[msg.sender] + stakeMinDuration, "Staking min duration require");
