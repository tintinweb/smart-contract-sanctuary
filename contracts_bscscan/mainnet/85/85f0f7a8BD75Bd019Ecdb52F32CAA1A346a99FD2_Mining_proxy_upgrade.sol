...
contract Mining_proxy_upgrade is Template_mining_plus{
    
    // consume amount of BCL or SYNC
    function upgrade(uint amount ,uint mode) external{
        require(mode == 1 || mode ==2,"BCL or SYNC2");
        
        address token = mode == 1 ? bcl_addr : SYNC; 
        
        uint value_u = amount * getPrice(token,base) * getPrice(base, USDT) *10** IBCL(USDT).decimals() / 10**IBCL(token).decimals()  / 1e36; 
        
        upgradeCost[msg.sender] += value_u;

        IBCL(token).transferFrom(msg.sender, address(this), amount);
        
        if(upgradeCost[msg.sender]>=2000*1e18){
            userInfo[msg.sender].level = 5;
        }else if(upgradeCost[msg.sender]>=1500*1e18){
            userInfo[msg.sender].level = 4;
        }else if(upgradeCost[msg.sender]>=1000*1e18){
            userInfo[msg.sender].level = 3;
        }else if(upgradeCost[msg.sender]>=500*1e18){
            userInfo[msg.sender].level = 2;
        }else if(upgradeCost[msg.sender]>=100*1e18){
            userInfo[msg.sender].level = 1;
        }
        if(mode==2){
            
            Address.functionCall(token,abi.encodeWithSignature("burn(uint256)",amount*15/100));
        }
        else{
            IBCL(token).transfer(address(0), amount*15/100);
            
        }
        
        IBCL(token).transfer(platform, amount*5/100);
        IBCL(token).transfer(taibao, amount*7/100);
        IBCL(token).transfer(sub_node, amount*5/100);
        IBCL(token).transfer(node, amount*3/100);
        
        // dynamic rewrds for group 
        uint reward = amount*65/100;
        
        // iterate 20 layers dynamic reference
        ...
        for(uint i=1; i<=n; ++i){
            ...
            if(lev>0 && userInfo[temp_user].value >= 100 * 1e18){
                ...
                uint256 dynamicReward = reward * dynamicRatio[layer][lev-1] / 100;
                if(mode==1){
                   bclReward_toClaim[temp_user] += dynamicReward;
                }else{
                    syncReward_toClaim[temp_user] += dynamicReward;
                }
            }
        }
