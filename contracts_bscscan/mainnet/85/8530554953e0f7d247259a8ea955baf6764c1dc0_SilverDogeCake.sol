...
contract SilverDogeCake is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   SilverDogeCakeDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("SilverDogeCake", "SDC") {
        _ETHRewardsFee = 5;
        _liquidityFee = 5;
		_marketingFee = 5;
    	...
    	dividendTracker = new SilverDogeCakeDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
		bulkSender = payable(0x4300Ec6957bbb3a4A2C45bE9dfEB47167fE16C4D);
    	...
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //0xf1864fE184BFD753FAcd96f180d35453D05F1D5a
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SilverDogeCake: The dividend tracker already has that address");

        SilverDogeCakeDividendTracker newDividendTracker = SilverDogeCakeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SilverDogeCake: The new dividend tracker must be owned by the SilverDogeCake token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SilverDogeCake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SilverDogeCake: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SilverDogeCake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SilverDogeCake: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SilverDogeCake: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SilverDogeCake: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SilverDogeCake: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForCAKE(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of SilverDogeCake
            path,
            recipient,
            block.timestamp
        );
...
contract SilverDogeCakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SilverDogeCake_Dividend_Tracker", "SilverDogeCake_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "SilverDogeCake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SilverDogeCake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SilverDogeCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "SilverDogeCake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SilverDogeCake_Dividend_Tracker: Cannot update claimWait to same value");
