...
contract USDCalculator {
    ...
    function get() external view returns(uint) {
        ...
        return (fcb_amt_fcb*1e18) / wbnb_amt_fcb * wbnb_amt_busd / busd_amt_busd;
    }
    ...
    function getWBNBAmountInUSDWorth(uint amount_) external view returns(uint) {
        uint busd_amt_busd = BUSD.balanceOf(address(BUSDBNB));
        uint wbnb_amt_busd = WBNB.balanceOf(address(BUSDBNB));

        return ((wbnb_amt_busd * 1e18) / (busd_amt_busd)) * amount_;
    }
