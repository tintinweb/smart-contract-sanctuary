...
contract Staking is WhitelistedRole {
    ...
    constructor(address CluiserLandTokenAddr10, uint256 newMinimum, uint256 newMedium, uint256 newMaximum, uint256 newMinPercent, uint256 newMedPercent, uint256 newMaxPercent, uint256 settingsLimit, uint256 newFeePercent, uint256 newFeePeriod) public Ownable(msg.sender) {
        require(CluiserLandTokenAddr10 != address(0));

        token = IERC20(CluiserLandTokenAddr10);
    ...
    function invest(address from, uint256 amount) public {
        ...
        if(!isWhitelisted(from)){
            ...
            address fee_addr = getUserPool(from);
    ...
    function setFeePercent(uint256 percent) public onlyOwner {
        require(percent >= 0 && percent < 10000, "Invalid value");
    ...
    function withdrawDeposit(address account) public {
        ...
        if(!isWhitelisted(account)){
            if(!(user.depositDate > 0 && ((now - user.depositDate) >= (feePeriod * ONE_DAY)))){
                fee = deposit.mul(feePercent).div(ONE_HUNDRED);
    ...
    function getUserPool(address addr) public view returns(address) {
        return (addr != address(0) && _users[addr].pool != address(0)) ? _users[addr].pool : _owner;
    }
    ...
    function getPercent(uint256 deposit, uint256 stage) public view returns(uint256) {
        ...
        if (deposit < par.minimum) {
            userPercent = 0;
        } else if (deposit >= par.minimum && deposit < par.medium) {
            userPercent = par.minPercent;
        } else if (deposit >= par.medium && deposit < par.maximum) {
