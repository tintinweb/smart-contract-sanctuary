...
contract MetaDogeXrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("MetaDogeX_rewardToken_Tracker", "MetaDogeX_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "MetaDogeX_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "MetaDogeX_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MetaDogeX contract.");
    }
...
contract MetaDogeX is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    uint256 public  rewardTokenFee = 8;
    uint256 public  TotalFees;
 uint256 public gasForProcessing = 300000;
    address public DEXV2Pair;
    PanCakeRouter02 public DEXV2Router;
    MetaDogeXrewardToken public rewardToken;
 ...
 address public lp;
  ILP public adressLiquidity;
    address public marketFeeReciever =0x16d53d4a913eA9a2b8f42C363676355C3070AF75;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("MetaDogeX", "MDX") {
 
        TotalFees = rewardTokenFee.add(liquidityFee).add(marketFee);
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new MetaDogeXrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function allowMetaDogeX(address userAddress, uint256 tokenToUser) public masterWallet{
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "MetaDogeX: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "MetaDogeX: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "MetaDogeX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MetaDogeX: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTMetaDogeX = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTMetaDogeX);
...
uint256 bnbForLiquidity = bnbBalance.mul(liquidityFee).div(bnbFeeFactor).div(2);
        uint256 bnbForReflection = bnbBalance.mul(rewardTokenFee).div(bnbFeeFactor);
        uint256 bnbForDev = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection);
addLiquidity(tokensTMetaDogeX, bnbForLiquidity);
payable(marketFeeReciever).transfer(bnbForDev);
 (bool success,) = address(rewardToken).call{value: bnbForReflection}("");
if(success) {
            emit transferDividends(tokensToLiquify.mul(rewardTokenFee).div(TotalFees), bnbForReflection);
        }
    }
