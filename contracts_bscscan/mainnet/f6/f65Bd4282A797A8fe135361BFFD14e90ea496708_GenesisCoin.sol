...
import './IGenesisCoin.sol';

contract GenesisCoin is
  IGenesisCoin,
  Context,
  ...
  ReentrancyGuard
{
  ...
  uint256 public gasForProcessing = 30000000;
  ...
  function _setupPancakeswap(address _routerAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    require(
      ...
      'Genesiscoin: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs'
    );
  ...
  function _setAutomatedMarketMakerPair(address pair, bool value) private {
    require(
      ...
      'Genesiscoin: Automated market maker pair is already set to that value'
    );
  ...
  //dividedRewardToken
  function updateDividendRewardToken(address dividendTokenAddress)
    public 
    onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _dividendRewardToken = dividendTokenAddress;
    }
...
// dividend swap
   function setBnbAsReward () public onlyRole(DEFAULT_ADMIN_ROLE){
        _isBnbReward = !_isBnbReward;
   }
   
   function setNTSAL(uint256 liquidityThresholdPercentage_) public onlyRole(DEFAULT_ADMIN_ROLE) {
       _numTokensSellToAddToLiquidity = (_totalSupply * liquidityThresholdPercentage_) / 10000;
   }
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    require(
      ...
      'GenesisCoin: The dividend tracker already has that address'
    );
    ...
    require(
      ...
      'GenesisCoin: The new dividend tracker must be owned by the token contract'
    );
...
interface IGenesisCoin is IERC20, IERC20Metadata {
