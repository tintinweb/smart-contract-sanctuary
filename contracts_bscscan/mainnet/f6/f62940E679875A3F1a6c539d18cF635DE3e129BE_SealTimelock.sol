...
/**
 * @dev ISealFarm functions that do not require less than the min timelock
 */
interface ISealFarm {
    function add(
        uint256 _allocPoint,
 ...
 * to position this {SealTimelock} as the owner of a smart contract, with
 * a multisig or a DAO as the sole proposer.
 *
 * _Available since v3.3._
 */
contract SealTimelock is AccessControl, ReentrancyGuard {
    ...
    modifier onlyRole(bytes32 role) {
        require(
            ...
            "SealTimelock: sender requires permission"
        );
        _;
    }
    ...
    function _schedule(bytes32 id, uint256 delay) private {
        require(
            _timestamps[id] == 0,
            "SealTimelock: operation already scheduled"
        );
        require(delay >= minDelay, "SealTimelock: insufficient delay");
        // solhint-disable-next-line not-rely-on-time
    ...
    function cancel(bytes32 id) public virtual onlyRole(PROPOSER_ROLE) {
        require(
            ...
            "SealTimelock: operation cannot be cancelled"
        );
    ...
    function _beforeCall(bytes32 predecessor) private view {
        require(
            ...
            "SealTimelock: missing dependency"
        );
    }
    ...
    function _afterCall(bytes32 id) private {
        require(
            isOperationReady(id),
            "SealTimelock: operation is not ready"
        );
        _timestamps[id] = _DONE_TIMESTAMP;
    }
    ...
    function _call(
        ...
        require(success, "SealTimelock: underlying transaction reverted");
    ...
    function updateMinDelay(uint256 newDelay) external virtual {
        require(
            ...
            "SealTimelock: caller must be timelock"
        );
    ...
    function executeSet(
        ...
        _beforeCall(predecessor);
        ISealFarm(_sealFarmAddress).set(_pid, _allocPoint, _depositFeeBP, _withUpdate);
        _afterCall(id);
    }
    ...
    function add(
        ...
        ISealFarm(_sealFarmAddress).add(0, _lpToken, _depositFeeBP, _withUpdate, _strat); // allocPoint = 0. Schedule set (timelocked) to increase allocPoint.
    }
