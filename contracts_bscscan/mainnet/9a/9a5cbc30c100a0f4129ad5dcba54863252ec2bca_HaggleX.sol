 ...
 */
contract BEP20Upgradeable is Initializable, ContextUpgradeable, IBEP20Upgradeable {
     ...
     * construction.
     */
    function __BEP20_init(string memory name_, string memory symbol_) internal initializer {
        __Context_init_unchained();
        __BEP20_init_unchained(name_, symbol_);
    }

    function __BEP20_init_unchained(string memory name_, string memory symbol_) internal initializer {
        _name = name_;
        _symbol = symbol_;
    }
 ...
 */
abstract contract BEP20BurnableUpgradeable is Initializable, ContextUpgradeable, BEP20Upgradeable {
...
contract HaggleX is Initializable, BEP20Upgradeable, BEP20BurnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    struct StakeType {
        ...
        uint maxWithdrawals; //The maximum amount of times an account can withdraw from a particular stake
    ...
    uint private stakedSupply;
    uint private lastOtherRewardWithdrawalTime;
    ...
    function initialize() initializer public {
        ...
        //tracks the total staked supply
        stakedSupply = 0;
          
        //tracks the last time a withdrawal was made on the other rewards
        lastOtherRewardWithdrawalTime = deploymentTime + 1 days;
        ...
        _stakingOptions[1].lockedTime = 90 days;
        _stakingOptions[1].totalStaked = 0;
        _stakingOptions[1].maxWithdrawals = 90;

        
        //staking for 6months 
        _stakingOptions[2].rewardPercent = 30;
        _stakingOptions[2].lockedTime = 180 days;
        _stakingOptions[2].totalStaked = 0;
        _stakingOptions[2].maxWithdrawals = 180;


        
        //staking for 12months 
        _stakingOptions[3].rewardPercent = 55;
        _stakingOptions[3].lockedTime = 360 days;
        _stakingOptions[3].totalStaked = 0;
        _stakingOptions[3].maxWithdrawals = 360;
    ...
    function set_LB_UBI_DEV_Address(address LB, address UBI, address DEV) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(LB != address(0), "HaggleX: approve to the zero address");
        require(UBI != address(0), "HaggleX: approve to the zero address");
        require(DEV != address(0), "HaggleX: approve to the zero address");
     ...
     * or free to withdraw.
     */
    function isStakeLocked(uint stake_) internal view returns (bool) {
    ...
    *
    ...
    function calculateDailyStakeReward(uint stake_) internal view returns (uint) {
        uint reward = getStakeAmount(stake_) * (getStakePercentageReward(_staking[_msgSender()][stake_].stakeType));
        return reward/(getTotalStakedAmount(_staking[_msgSender()][stake_].stakeType));
    }
    ...
    */
     ...
     */
    function getStakePercentageReward(uint stakeType_) internal view returns (uint) {
        uint rewardPerc = getHalvedReward() * (STAKERS_PERCENTAGE) * (_stakingOptions[stakeType_].rewardPercent);
        return  rewardPerc/(10000);
    }
    
    function getLeadershipBoardPercentageReward() internal view returns (uint) {
        uint rewardPerc = getHalvedReward() * (LEADERSHIP_BOARD_PERCENTAGE);
        return  rewardPerc/(100);
    }
    
    function getUBIPercentageReward() internal view returns (uint) {
        uint rewardPerc = getHalvedReward() * (UNIVERSAL_BASIC_INCOME_PERCENTAGE);
        return  rewardPerc/(100);
    }
    
    function getDevelopmentPercentageReward() internal view returns (uint) {
        uint rewardPerc = getHalvedReward() * (DEVELOPMENT_PERCENTAGE);
        return  rewardPerc/(100);
    }
    ...
    *
    ...
    function withdrawStakeReward(uint stake_) public {
        require(isStakeLocked(stake_) || (_staking[_msgSender()][stake_].noOfWithdrawals < _stakingOptions[_staking[_msgSender()][stake_].stakeType].maxWithdrawals), "HaggleX: Stake withdrawal no longer available!");
        require(block.timestamp >= _staking[_msgSender()][stake_].lastWithdrawTime + 1 days, "HaggleX: Not yet time to withdraw Stake reward");
    ...
    //Withdraw Rewards for the Leadership Board, 
    function withdrawOtherRewards() public onlyRole(MINTER_ROLE){
        require(block.timestamp >= lastOtherRewardWithdrawalTime + 1 days, "HaggleX: Not yet time to withdraw Other Board rewards");
     ...
     *
     */
    function stake(uint _amount, uint stakeType_) public {
        require(stakeType_ == 1 || stakeType_ == 2 || stakeType_ == 3, "HaggleX: incorrect stake type ");
    ...
    function unstake(uint stake_) public  {
        require(!isStakeLocked(stake_), "HaggleX:Stake still locked!");
        require(_staking[_msgSender()][stake_].stakeActive, "HaggleX:Stake is no longer active");
        require(_staking[_msgSender()][stake_].noOfWithdrawals == _stakingOptions[_staking[_msgSender()][stake_].stakeType].maxWithdrawals, "HaggleX: All stake rewards are not yet withdrawn");
