...
contract BabyBullTerrier is Context, IERC20, Ownable {
    ...
    uint256 numTokensSellToAddToLiquidity = _tTotal.div(1000);
    ...
    string private constant _name = "Baby Bull Terrier";
    ...
    modifier onlyMarketing() {
        require(MarketingAddress == _msgSender(), "caller is not the Marketing");
        _;
    }
    ...
    constructor () {
        ...
        MarketingAddress= 0x52B6e6AEc458d0Ec2aFf69aF25D62ea710b871d5;
    ...
    function _getValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rBurn, to_return.rMarketing, to_return.rLiquidity) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount.sub(s.tRfi).sub(s.tBurn).sub(s.tMarketing).sub(s.tLiquidity);
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rBurn,uint256 rMarketing, uint256 rLiquidity) {
        ...
        rLiquidity = s.tLiquidity.mul(currentRate);
        rTransferAmount =  rAmount.sub(rRfi).sub(rBurn).sub(rMarketing).sub(rLiquidity);
        return (rAmount, rTransferAmount, rRfi,rBurn,rMarketing,rLiquidity);
    }
