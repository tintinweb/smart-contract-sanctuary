...
contract KimCardano is IERC20 {
    ...
    uint256 public _buyReflectionFee = 800;
    ...
    uint256 public maxBurnFee = 500;
    uint256 public maxCharityFee = 500;
    ...
    uint256 private previousCharityFee = _charityFee;
    ...
    uint256 private charityRatio = _sellCharityFee;
    ...
    address payable private _marketingWallet = payable(0xfAEb76A74A43B8f6C9719A636a22CFA4faea7E71);
    address payable private _charityWallet = payable(0xbD399687ce5182d75f990320E034f8E1708c4095);

    // Max TX amount is 0.75% of the total supply.
    ...
    function setBuyTaxes(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee, uint256 burnFee, uint256 charityFee) external onlyOwner {
        require(liquidityFee <= maxLiquidityFee
                && reflectionFee <= maxReflectionFee
                && marketingFee <= maxMarketingFee
                && charityFee <= maxCharityFee
                && burnFee <= maxBurnFee);
        require(liquidityFee + reflectionFee + marketingFee + charityFee + burnFee <= 5000);
    ...
    function setSellTaxes(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee, uint256 burnFee, uint256 charityFee) external onlyOwner {
        require(liquidityFee <= maxLiquidityFee
                && reflectionFee <= maxReflectionFee
                && marketingFee <= maxMarketingFee
                && charityFee <= maxCharityFee
    ...
    function setRatios(uint256 reflection, uint256 liquidity, uint256 marketing, uint256 charity) external onlyOwner {
        reflectionRatio = reflection;
        liquidityRatio = liquidity;
        marketingRatio = marketing;
        charityRatio = charity;
    }
    ...
    function getTotalFee() public view returns (uint256) {
        return _reflectionFee + _liquidityFee + _marketingFee + _charityFee;
    }
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        uint256 swapTotalFee = reflectionRatio + liquidityRatio + marketingRatio + charityRatio;
    ...
    function transferBNBOut(uint256 amount) internal {
        uint256 amountBNBMarketing = (amount * marketingRatio) / (marketingRatio + charityRatio);
        uint256 amountBNBCharity = amount - amountBNBMarketing;
