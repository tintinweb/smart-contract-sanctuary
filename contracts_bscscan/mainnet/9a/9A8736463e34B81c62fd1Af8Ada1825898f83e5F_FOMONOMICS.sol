...
contract FOMONOMICS is ERC20Detailed, Ownable {
  ...
  // TOTAL_FMN is a multiple of INITIAL_FRAGMENTS_SUPPLY so that _FMNPerFragment is an integer.
  // Use the highest value that fits in a uint256 for max granularity.
  uint256 private constant TOTAL_FMN = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

  // MAX_SUPPLY = maximum integer < (sqrt(4*TOTAL_FMN + 1) - 1) / 2
  uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

  uint256 private _totalSupply;
  uint256 private _FMNPerFragment;
  mapping(address => uint256) private _FMNBalances;
  ...
  ERC20Detailed("FOMONOMICS", "FMN", uint8(DECIMALS))
  payable
  public
  {
    ...
    _FMNBalances[msg.sender] = TOTAL_FMN;
    _FMNPerFragment = TOTAL_FMN.div(_totalSupply);
...
*/
  ...
  returns (uint256)
  {
    return _FMNBalances[who].div(_FMNPerFragment);
  }
  ...
  function _transferStandard(address sender, address recipient, uint256 amount) private {
    ...
    uint256 FMNDeduct = amount.mul(_FMNPerFragment);
    uint256 FMNValue = tTransferAmount.mul(_FMNPerFragment);
    _FMNBalances[sender] = _FMNBalances[sender].sub(FMNDeduct);
    _FMNBalances[recipient] = _FMNBalances[recipient].add(FMNValue);
    _takeFee(sender, tFee);
    emit Transfer(sender, recipient, amount);
  }

  function _transferExcluded(address sender, address recipient, uint256 amount) private {
    uint256 FMNValue = amount.mul(_FMNPerFragment);
    _FMNBalances[sender] = _FMNBalances[sender].sub(FMNValue);
    _FMNBalances[recipient] = _FMNBalances[recipient].add(FMNValue);
    emit Transfer(sender, recipient, amount);
  }
  ...
  function _takeFee(address sender, uint256 tFee) private {
    uint256 rFee = tFee.mul(_FMNPerFragment);
    _FMNBalances[address(this)] = _FMNBalances[address(this)].add(rFee);
