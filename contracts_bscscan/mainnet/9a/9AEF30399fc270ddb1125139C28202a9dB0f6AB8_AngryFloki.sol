...
contract AngryFloki is ERC20, Ownable {
    ...
    function utilizeFee(uint256 amount) internal {
        ...
        uint256 tokenForLiqudity = amount.mul(liquidityTotal.div(2)).div(currentTotal);
        uint256 tokenForReward = amount.mul(rewardTotal).div(currentTotal);

        uint256 currentBNB = swapTokensForBNB(amount.sub(tokenForLiqudity));

        {
            uint256 BNBForMarketing = currentBNB.mul(tokenForMarketing).div(amount);
            payable(_marketingAddress).transfer(BNBForMarketing);
        }

        {
            uint256 BNBForCharity = currentBNB.mul(tokenForCharity).div(amount);
            payable(_charityAddress).transfer(BNBForCharity);
        }

        {
            uint256 BNBForBuyback = currentBNB.mul(tokenForBuyback).div(amount);
            payable(_buybackAddress).transfer(BNBForBuyback);
        }

        {
            uint256 BNBForReward = currentBNB.mul(tokenForReward).div(amount);
            swapAndSendDividends(BNBForReward);
        }

        addToLiquidity(tokenForLiqudity, address(this).balance);
       ...
       _liquidityFeeTotal = liquidityTotal.sub(tokenForLiqudity.mul(2));
