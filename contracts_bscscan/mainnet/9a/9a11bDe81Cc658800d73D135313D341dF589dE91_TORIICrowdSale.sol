...
contract TORIICrowdSale is Ownable {
    ...
    constructor() {
        token = IBEP20(0xfD053F9A52d2D74Aae83c28323CC7d2e2Fefb263); // TORII
        wallet = payable(0xA2436f912F114cdcf889A554442622ce04c70Cae);
        minBuyValue = 100000000000000000; // 0,1 BNB
        maxBuyValue = 5000000000000000000; // 5 BNB
        
        rateMultiplier = 100;
        // 0 ... r1 ... 600 ... r2 ... 1200 ... r3 ... 2400 ... r4 ... 3200 ... r4 .....
        rounds.push(Round({
			amount: 600 * 10**18, 
			rate: 800
		}));   
    ...
    function buyTokens() public payable {
        ...
        if (whiteListSell) {
			require(whiteList[msg.sender], "TORII-CrowdSale buyTokens: buy allowed only for whitelisted addresses!");
		} 
		
        require(msg.value >= minBuyValue, "TORII-CrowdSale buyTokens: you can't buy for less than min buy value!");
                
        UserInfo storage user = users[msg.sender];

        require(user.bnbSpent + msg.value <= maxBuyValue, "TORII-CrowdSale buyTokens: you can't buy for more than max buy value!");

		(uint256 purchaseAmount, uint256 roundResult) = buyCalculate(msg.value);
		
		require(purchaseAmount <= balanceOfToken(), "TORII-buyTokens buyTokens: not enough tokens in contract for sell");		
    ...
    function setRound(uint256 _index, uint256 _amount, uint256 _rate) external onlyOwner {
        ...
        if (_index > 0) {
            ...
            require((_amount * rateMultiplier) > ((prevRound.amount * rateMultiplier) + prevRound.rate), "TORII-CrowdSale setRound: amount must be grater than previous round amount + previous round rate!");
        }		
    ...
    function addRound(uint256 _amount, uint256 _rate) external onlyOwner {
        ...
        if (rounds.length > 0) {
            ...
            require((_amount * rateMultiplier) > ((lastRound.amount * rateMultiplier) + lastRound.rate), "TORII-CrowdSale addRound: amount must be grater than last round amount + last round rate!");
        }
    ...
    function getRound()  public view returns (uint256 index, uint256 amount, uint256 rate, uint256 previousRoundAmount, bool isLastRound, uint256 roundRateMultiplier) {
        require(rounds.length > 0, "TORII-CrowdSale getRound: no rounds added!");
    ...
    function setStartTime(uint256 _startTimestamp) external onlyOwner {
		require(totalTokensPurchased == 0, "TORII-CrowdSale setStartTime: you can start only at 0 purchsed, or reset it first!");
        require(_startTimestamp >= blockTimestamp(), "TORII-CrowdSale setStartTime: start time can't be in past!");
        require(rounds.length > 0, "TORII-CrowdSale setStartTime: at least one round should be added!");
		startTime = _startTimestamp;		      
    }

    // 
    function startNow() external onlyOwner {
		require(totalTokensPurchased == 0, "TORII-CrowdSale startNow: you can start only at 0 purchsed, or reset it first!");
        require(rounds.length > 0, "TORII-CrowdSale startNow: at least one round should be added!");
        startTime = blockTimestamp();		      
    }
    ...
    function setWallet(address payable _wallet) external onlyOwner {
        require(_wallet != address(0), "TORII-CrowdSale setWallet: wrong wallet address!");
		wallet = _wallet;		     
    }
