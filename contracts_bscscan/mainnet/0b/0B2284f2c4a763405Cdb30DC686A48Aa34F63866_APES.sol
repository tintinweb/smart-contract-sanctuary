...
contract APES is IERC20, Ownable {
  ...
  string private _name = "APES.TEAM";
  ...
  bool private teamAllocated = false;
  ...
  constructor(address _crowdfundingWallet, address _teamWallet) {
  ...
  function _transfer(address sender, address recipient, uint256 amount ) internal virtual {
      ...
      if ( amount < 1 * 10**9 || isExcludedFromFee[sender] || recipient == liquidityPool ) {
        ...
        if (toLiquidity > 1000 * 10**9 && swapAndLiquifyEnabled) {
  ...
  function toggleDiscount() public onlyOwner {
      if ( discountEnabled ) { discountEnabled = false; } else { discountEnabled = true; }
  }
  ...
  function getPrice() public view returns(uint) {
    if ( presale ) {
      ...
      uint currentPrice = router.getAmountIn(1000000000, reserve1, reserve0);
      if ( discountEnabled ) {
        uint discountPrice = currentPrice.div(100).mul(90); // 10% discount
  ...
  function allocateTokens() public onlyOwner {
    ...
    (bool sent) = IERC20(address(this)).transfer(crowdfundingWallet, tAmount);
    teamAllocated = sent;
  }
  ...
  function addLiquidityETH(uint256 tokenAmount, uint256 ethAmount) external payable onlyOwner returns (uint amountToken, uint amountETH, uint liquidity) {
      ...
      (amountToken, amountETH, liquidity) = router.addLiquidityETH{value: ethAmount}(apesAddress, tokenAmount, 0, 0, owner(), block.timestamp.add(300));
  }
