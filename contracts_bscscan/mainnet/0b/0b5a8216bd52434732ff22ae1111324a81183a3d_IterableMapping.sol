...
// MicroPay - The Decentralised Payment service for E-Commerce Ecosystem
// Website: https://micropay.finance/
...
contract Micropay is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MicroPayDividendTracker public dividendTracker;
    ...
    constructor() ERC20("MICROPAY", "MIP") {
        dividendTracker = new MicroPayDividendTracker();
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "MicroPay: The router already has that address");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MicroPay: The dividend tracker already has that address");

        MicroPayDividendTracker newDividendTracker = MicroPayDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MicroPay: The new dividend tracker must be owned by the MicroPay token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MicroPay: Account is already the value of 'excluded'");
    ...
    function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPeriodLimit[account] != excluded, "MicroPay: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "MicroPay: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MicroPay: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MicroPay: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "MicroPay: This account cannot send tokens until trading is enabled");
        }
...
contract MicroPayDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MicroPayDividendTracker", "MIP_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MicroPayDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MicroPayDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main MicroPay contract.");
    }
