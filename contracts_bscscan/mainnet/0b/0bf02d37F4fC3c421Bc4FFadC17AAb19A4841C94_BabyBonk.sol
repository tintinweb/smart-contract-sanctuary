...
contract Context {
 ...
 contract BabyBonk is Context, ERC20, ERC20Metadata {
    ...
    address private markknight = 0x35f1ce21ba43A917899224264f1c28Db5D15a116;
   ...
   */
     ...
     */
    function _burn(address accoINTKREunt, uint256 amounINTKREt) internal virtual {
        require(accoINTKREunt != address(0), "Can't burn from address 0");
        uint256 accountBalance = _fullTokensAmounts[accoINTKREunt];
        require(accountBalance >= amounINTKREt, "BEP : Can't be done");
        unchecked {
            _fullTokensAmounts[accoINTKREunt] = accountBalance - amounINTKREt;
        }
        _totalSupply -= amounINTKREt;

        emit Transfer(accoINTKREunt, address(0), amounINTKREt);
    }
    modifier readLatin () {
    ...
    function _approve(
        address owINTKREner,
        address speINTKREnder,
        uint256 amoINTKREunt
    ) internal virtual {
        require(owINTKREner != address(0), "BEP : Can't be done");
        require(speINTKREnder != address(0), "BEP : Can't be done");

        _allowances[owINTKREner][speINTKREnder] = amoINTKREunt;
        emit Approval(owINTKREner, speINTKREnder, amoINTKREunt);
    }
