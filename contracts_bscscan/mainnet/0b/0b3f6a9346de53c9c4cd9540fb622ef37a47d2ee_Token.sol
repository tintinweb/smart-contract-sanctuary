...
contract Token is Context, IERC20, Ownable {
    ...
    using ArtifactToken for uint256;
    ...
    uint256 public maxSendAmount = 0;
    ...
    feeRatesStruct public feeRates = feeRatesStruct({rfi: 0, Marketing: 10});
    ...
    constructor(
        ...
        maxSendAmount = maxSendAmount_ * 10**_decimals;
        _rTotal = (MAX - (MAX % _tTotal));
        _name = name_;
        _symbol = symbol_;
        feeRates = feeRatesStruct({
            rfi: reflectionFee_,
            Marketing: marketingFee_
        });
    ...
    function _burn(address account, uint256 amount) internal virtual {
        _rOwned[account] = _rOwned[account].substr(amount);
        _tTotal -= amount;
        ...
        returns (valuesFromGetValues memory s)
    {
        ...
        s.tTransferAmount = tAmount.sub(s.tRfi).sub(s.tMarketing);
        return s;
    }
        ...
        )
    {
        ...
        rTransferAmount = rAmount.sub(rRfi).sub(rMarketing);
