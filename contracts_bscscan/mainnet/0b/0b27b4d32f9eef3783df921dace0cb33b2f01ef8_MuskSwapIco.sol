...
contract MuskSwapIco is Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    IERC20Upgradeable public muskSwapToken;
    ...
    address public oldSellContract;
    ...
    function initialize(
        ...
        address _muskSwapTokenaAddress,
        address _priceFeedAddress,
        address _oldSellContract
    ) public initializer {
        ...
        require(
            _muskSwapTokenaAddress != address(0),
            "invalid ShibaMusk contract address"
        );
        require(
            _oldSellContract != address(0),
            "invalid ShibaMusk contract address"
        );
        ...
        muskSwapToken = IERC20Upgradeable(_muskSwapTokenaAddress);
     ...
     * callable by admin
     */
    function setIcoContract(address _contractAddress) external onlyAdmin {
        require(_contractAddress != address(0), "Cannot be zero address");
        oldSellContract = _contractAddress;
    }
    ...
    function buyICOByBUSD(uint256 amount) external payable {
        ...
        uint256 buyAmountToken = amount.mul(totalAmountPerBUSD);

        address sender = _msgSender();
        _lockAmountToken(buyAmountToken, sender);
        tokenBUSD.safeTransferFrom(sender, address(this), amount);
        // muskSwapToken.safeTransferFrom(address(this), sender, buyAmountToken);
    }
    ...
    function buyICO() external payable {
        ...
        uint256 totalBUSDConverted = msg.value.mul(10**18).div(
            uint256(busdBNBPrice)
        );
        uint256 buyAmountToken = totalBUSDConverted.mul(totalAmountPerBUSD);
        _lockAmountToken(buyAmountToken, _msgSender());
        muskSwapToken.safeTransferFrom(address(this), _msgSender(), buyAmountToken);
    }
    ...
    function claimBUSD() external onlyAdmin {
    ...
    function claimToken() external onlyAdmin {
        address sender = _msgSender();
        uint256 remainAmountToken = muskSwapToken.balanceOf(address(this));
        muskSwapToken.safeTransfer(sender, remainAmountToken);
    }

    function setContractToken(address contractToken) external onlyOwner {
        muskSwapToken = IERC20Upgradeable(contractToken);
    }
    ...
    function receivedAmount(address recipient) external view returns (uint256) {
        if (lockInfos[recipient].amountLocked == 0) {
            uint256 lockedAmount = ISellToken(oldSellContract).receivedAmount(recipient);
            return lockedAmount;
        } else {
            return lockInfos[recipient].amountLocked;
        }
    }

    function _lockAmountToken(uint256 _lockedAmount, address _address) internal {
        if (lockInfos[_address].amountLocked == 0) {
            uint256 lockedAmount = ISellToken(oldSellContract).receivedAmount(_address);
            lockInfos[_address].amountLocked += lockedAmount + _lockedAmount;
        } else {
            lockInfos[_address].amountLocked += _lockedAmount;
        }
    }
}
