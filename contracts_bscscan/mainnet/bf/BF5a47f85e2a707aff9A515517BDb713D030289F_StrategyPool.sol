...
// File: contracts/BIFI/strategies/degens/StrategyPool.sol
 ...
 */
contract StrategyPool is Ownable, Pausable, GasThrottler {
     ...
     * {want} - Token that the strategy maximizes. The same token that users deposit in the vault. ZEFI
     * {bifi} - BeefyFinance token, used to send funds to the treasury.
     */
    address constant public wbnb = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    address constant public want = address(0x7D5bc7796fD62a9A27421198fc3c349B96cDD9Dc); // token do pool MEL
    ...
    // address constant public unirouter  = address(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F); // v1
    address constant public unirouterv2  = address(0x10ED43C718714eb63d5aA57B78B54704E256024E); // v2
    address constant public unirouter  = address(0x1C0a81Cc2383E2f28DF42d7E4b47a09dD9526157); // MEL
    address constant public masterchef = address(0xc41E3944367814C947C5c53cb1b3FDce1C7BC286);
    uint8 constant public poolId = 13;
    ...
    address constant public treasury = address(0x000000000000000000000000000000000000dEaD);
    ...
    constructor(address _vault, address _strategist) public {
        ...
        IERC20(bifi).safeApprove(masterchef, uint(-1));
        IERC20(want).safeApprove(unirouter, uint(-1));
        IERC20(wbnb).safeApprove(unirouter, uint(-1));
        IERC20(want).safeApprove(unirouterv2, uint(-1));
        IERC20(wbnb).safeApprove(unirouterv2, uint(-1));
    }
    ...
    function deposit() public whenNotPaused {
        ...
        uint256 toWbnb = IERC20(want).balanceOf(address(this));
        IUniswapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toWbnb, 0, wantToWbnbRoute, address(this), now.add(600));

        uint256 tobifi = IERC20(wbnb).balanceOf(address(this));
        IUniswapRouter(unirouterv2).swapExactTokensForTokensSupportingFeeOnTransferTokens(tobifi, 0, wbnbToBifiRoute, address(this), now.add(600));        

        uint256 wantBal = IERC20(bifi).balanceOf(address(this));
    ...
    function harvest() public whenNotPaused gasThrottle {
        ...
        IMasterChef(masterchef).deposit(poolId, 0, strategist);
        uint256 wantBal = IERC20(want).balanceOf(address(this));
        if (wantBal > 0) {
            chargeFees();
            deposit();
    ...
    /**
     * @dev Takes out 3.0% as system fees from the rewards.
     * 3.0% -> Strategist fee
     */
    function chargeFees() internal {
        
        uint256 toWbnb = IERC20(want).balanceOf(address(this)).mul(porcFee).div(1000);
        IUniswapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toWbnb, 0, wantToWbnbRoute, strategist, now.add(600));

        // uint256 wbnbBal = IERC20(wbnb).balanceOf(address(this));
        // IERC20(wbnb).safeTransfer(rewards, wbnbBal);  // all

        // uint256 callFeeAmount = wbnbBal.mul(callFee).div(MAX_FEE); // 5,5
        // IERC20(wbnb).safeTransfer(msg.sender, callFeeAmount);

        // uint256 treasuryHalf = wbnbBal.mul(TREASURY_FEE).div(MAX_FEE).div(2);
        // IERC20(wbnb).safeTransfer(treasury, treasuryHalf);  // 5,6
        // IUniswapRouter(unirouter).swapExactTokensForTokens(treasuryHalf, 0, wbnbToBifiRoute, treasury, now.add(600));  // 5,6

        // uint256 treasuryHalf = wbnbBal.mul(TREASURY_FEE).div(MAX_FEE);
        // IUniswapRouter(unirouter).swapExactTokensForTokens(treasuryHalf, 0, wbnbToBifiRoute, treasury, now.add(600));  // 11,2

        // uint256 rewardsFeeAmount = wbnbBal.mul(rewardsFee).div(MAX_FEE); // 72,1
        // IERC20(wbnb).safeTransfer(rewards, rewardsFeeAmount);

        // uint256 strategistFee = wbnbBal.mul(STRATEGIST_FEE).div(MAX_FEE); // 11,2
    ...
    function pause() public onlyOwner {
        ...
        IERC20(bifi).safeApprove(masterchef, 0);
        IERC20(want).safeApprove(unirouter, 0);
        IERC20(wbnb).safeApprove(unirouter, 0);
        IERC20(want).safeApprove(unirouterv2, 0);
        IERC20(wbnb).safeApprove(unirouterv2, 0);        
    }
