...
contract WonkaTreasury is Ownable {
    using SafeMath for uint256;
    ...
    address public sWONKA;
    uint256 public sWONKAQueue; // Delays change to sWONKA address
    ...
    constructor(
        ...
        isLiquidityToken[_WONKABUSD] = true;
        liquidityTokens.push(_WONKABUSD);

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for WONKA
        @param _amount uint
    ...
    function deposit(
        ...
        // mint WONKA needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IERC20Mintable(WONKA).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn WONKA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf(_token, _amount);
        IWONKAERC20(WONKA).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sWONKA).balanceOf(msg.sender); // Can only borrow against sWONKA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with WONKA
        @param _amount uint
     */
    function repayDebtWithWONKA(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");

        IWONKAERC20(WONKA).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, WONKA, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(WONKA).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(WONKA).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns WONKA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    ...
    {
        if (isReserveToken[_token]) {
            // convert amount to match WONKA decimals
            value_ = _amount.mul(10**IERC20(WONKA).decimals()).div(
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.SWONKA) {
            // 9
            sWONKAQueue = block.number.add(blocksNeededForQueue);
        } else return false;
