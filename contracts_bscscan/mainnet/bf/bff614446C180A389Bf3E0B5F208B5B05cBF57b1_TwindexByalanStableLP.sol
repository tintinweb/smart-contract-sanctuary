...
contract TwindexByalanStableLP is ByalanIsland, Sailor, IByalan, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using Address for address payable;

    // Tokens used
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant KUSD = 0x940Ff63e82d15fb47371BFE5a4ed7D7D183dE1A5;
    ...
    uint8 public immutable kusdIndex;

    // Routes
    address[] public dopToWbnbRoute = [DOPX, WBNB];
    address[] public dopToKusdRoute = [DOPX, KUSD];
        ...
        )
    {
        ...
        kusdIndex = ISwap(swapToken).getTokenIndex(KUSD);
    ...
    function addLiquidity() internal {
        uint256 dopxBal = IERC20(DOPX).balanceOf(address(this));
        ...
        uint256 kusdBal = IERC20(KUSD).balanceOf(address(this));
        uint256[] memory amounts = new uint256[](swapSize);
        amounts[kusdIndex] = kusdBal;
    ...
    function panic() public override onlyHydra {
        pause();
        IMasterChef(MASTERCHEF).emergencyWithdraw(pid);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(KUSD).safeApprove(swapToken, type(uint256).max);
    }

    function _removeAllowances() internal {
        IERC20(want).safeApprove(MASTERCHEF, 0);
        IERC20(DOPX).safeApprove(unirouter, 0);
        IERC20(KUSD).safeApprove(swapToken, 0);
    }
