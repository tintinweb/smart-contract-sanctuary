...
contract RoaringElon is Context, IERC20, Ownable {
    ...
    string private _name = "Roaring Elon";
    string private _symbol = "ROARINGELON";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000000 * 10**9 * 10**_decimals;
    
    address payable public marketingAddress = payable(0xAae2d1bD09eb6cd424eBdD0660042C4d578607c1);
    address public marketingWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromMaxBalanceRE;
    mapping (address => bool) private _isExcludedFromFeeRE;

    uint256 private constant _maxFeeRE = 15;
    uint256 private _totalFeeRE;
    uint256 private _totalFeeREContract;
    uint256 private _liquidityFeeRE;
    uint256 private _marketingFeeRE;
    uint256 private _burnFeeRE;
    
    uint256 private _maxBalanceWalletRE;
    ...
    constructor () {
        ...
        _isExcludedFromFeeRE[owner()] = true;
        _isExcludedFromFeeRE[address(this)] = true;
        
        _isExcludedFromMaxBalanceRE[owner()] = true;
        _isExcludedFromMaxBalanceRE[address(this)] = true;
        _isExcludedFromMaxBalanceRE[uniswapV2Pair] = true;

        _liquidityFeeRE = 1;
        _marketingFeeRE = 14;
        _burnFeeRE = 0;
        _totalFeeRE = _liquidityFeeRE.add(_marketingFeeRE).add(_burnFeeRE);
        _totalFeeREContract = _liquidityFeeRE.add(_marketingFeeRE);

        _liquidityThreshhold = 3000 * 10**9 * 10**_decimals;
        _maxBalanceWalletRE = 1000000 * 10**9 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFeeRE).add(_marketingFeeRE) <= _maxFeeRE, "Fees are too high.");
        _liquidityFeeRE = newLiquidityFee;
        _totalFeeRE = _liquidityFeeRE.add(_marketingFeeRE).add(_burnFeeRE);
        _totalFeeREContract = _liquidityFeeRE.add(_marketingFeeRE);
        emit SetLiquidityFee(_liquidityFeeRE);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFeeRE.add(_burnFeeRE).add(newMarketingFee) <= _maxFeeRE, "Fees are too high.");
        _marketingFeeRE = newMarketingFee;
        _totalFeeRE = _liquidityFeeRE.add(_marketingFeeRE).add(_burnFeeRE);
        _totalFeeREContract = _liquidityFeeRE.add(_marketingFeeRE);
        emit SetMarketingFee(_marketingFeeRE);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFeeRE.add(newBurnFee).add(_marketingFeeRE) <= _maxFeeRE, "Fees are too high.");
        _burnFeeRE = newBurnFee;
        _totalFeeRE = _liquidityFeeRE.add(_marketingFeeRE).add(_burnFeeRE);
        emit SetBurnFee(_burnFeeRE);
    }
    ...
    function setMaxBalance(uint256 newMaxBalance) external onlyOwner(){
        // Minimum _maxBalanceWalletRE is 0.5% of _totalSupply 
        require(newMaxBalance >= _totalSupply.mul(5).div(1000));
        _maxBalanceWalletRE = newMaxBalance;
    }

    function isExcludedFromFees(address account) public view returns(bool) {
        return _isExcludedFromFeeRE[account];
    }
    
    function excludeFromFees(address account) public onlyOwner {
        _isExcludedFromFeeRE[account] = true;
    }
    
    function includeInFees(address account) public onlyOwner {
        _isExcludedFromFeeRE[account] = false;
    }

    function isExcludedFromMaxBalance(address account) public view returns(bool) {
        return _isExcludedFromMaxBalanceRE[account];
    }
    
    function excludeFromMaxBalance(address account) public onlyOwner {
        _isExcludedFromMaxBalanceRE[account] = true;
    }
    
    function includeInMaxBalance(address account) public onlyOwner {
        _isExcludedFromMaxBalanceRE[account] = false;
    }
    ...
    function _transfer(
        ...
        // Make sure that: Balance + Buy Amount <= _maxBalanceWalletRE
        if(
            from != owner() &&              // Not from Owner
            to != owner() &&                // Not to Owner
            !_isExcludedFromMaxBalanceRE[to]  // is excludedFromMaxBalance
        ){
            require(
                balanceOf(to).add(amount) <= _maxBalanceWalletRE,
        ...
        if(
            ...
            _totalFeeREContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromFeeRE[from] || _isExcludedFromFeeRE[to])
            && _totalFeeRE > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_totalFeeREContract).div(100);
            uint256 toBurnAmount = amount.mul(_burnFeeRE).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFeeRE).div(_totalFeeREContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeeRE).div(_totalFeeREContract);
