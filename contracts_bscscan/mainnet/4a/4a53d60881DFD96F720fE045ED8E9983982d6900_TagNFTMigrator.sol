...
import "../Tokens/TagNFT.sol";

contract TagNFTMigrator is AccessControl {
    ...
    constructor( address _TAddr ) AccessControl() {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);

        TNFT = TagNFT(_TAddr);
    }

    function mintNFT ( string[] memory NFT , address[] memory addr, uint256[] memory ctr )
        onlyRole(DEFAULT_ADMIN_ROLE)
        public
    {
        require( NFT.length == addr.length , "TagNFT Migrator :  InCorrect Length");
        require( NFT.length == ctr.length , "TagNFT Migrator :  InCorrect Length");
        require( addr.length == ctr.length , "TagNFT Migrator :  InCorrect Length");
        uint256 lx = 0;
        while (lx < NFT.length) {
            require( ctr[lx] == TNFT.totalSupply() + 1 , "TagNFT Migrator : Incorrect CTR");
            TNFT.mint(addr[lx], NFT[lx]);
            lx = lx + 1;
        }
    }
