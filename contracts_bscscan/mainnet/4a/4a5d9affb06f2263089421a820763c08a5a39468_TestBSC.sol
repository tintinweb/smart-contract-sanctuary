...
contract TestBSC is ERC20, Ownable {
    ...
    constructor() ERC20("Test BSC", "Test") {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            0x10ED43C718714eb63d5aA57B78B54704E256024E //TraderJoe AVAX Router
        );
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(contractETHBalance > 0 && !swapping) {
                uint256 tokensBefore = balanceOf(address(this));
                if(buyBackEnabled){
                    swapETHforThisToken(contractETHBalance);
                }
                uint256 tokensAfter = balanceOf(address(this));
                uint256 tokensBurn = tokensAfter.sub(tokensBefore);
                if(burnEnabled) {
                    super._burn(address(this), tokensBurn);
                }
        }
        else if (swapEnabled && !swapping && from != uniswapV2Pair) {
            if (overMinimumTokenBalance) {
                uint256 swapTokens = contractTokenBalance;
                swapTokensDistribute(swapTokens);
            }
        }
    ...
    function swapETHforThisToken(uint ethAmount) private lockTheSwap {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(this);

        _approve(address(this), address(uniswapV2Router), ethAmount);
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: ethAmount}(ethAmount,path,address(this),block.timestamp);
    }
    ...
    function swapTokensDistribute(uint256 tokens) private lockTheSwap {
        ...
        uint256 dividends = newBalance.div(2);
        uint256 teamFee = dividends.div(4);
        _marketingWallet.transfer(teamFee);
        _devWallet.transfer(teamFee);
        if(sendDividendsEnabled) {
            sendDividends(dividends);
        }
    }
