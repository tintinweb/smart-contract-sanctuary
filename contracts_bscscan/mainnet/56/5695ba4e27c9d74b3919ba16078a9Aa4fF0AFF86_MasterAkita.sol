...
/*
    The AKITA and AMM on Binance Smart Chain

    🐶🐶 Website: https://akitainuswap.finance/#/
    🐶🐶 Telegram: https://t.me/AkitaSwapOfficial
    🐶🐶 Channel: https://t.me/AkitaSwapAnn
    🐶🐶 Twitter: https://twitter.com/AkitaSwap
    🐶🐶 Medium: https://akitaswap.medium.com/
 ...
 * functions have added to mitigate the well-known issues around setting
...
contract AkitaSwap is BEP20('Akita Swap', 'AKITAS') {
...
contract MasterAkita is Ownable {
    using SafeMath for uint256;
    ...
    AkitaSwap public akita;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public akitaPerBlock;
    // Bonus muliplier for early akita makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _akitaPerBlock,
        uint256 _startBlock
    ) public {
        akita = _akita;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        akitaPerBlock = _akitaPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 akitaReward = multiplier.mul(akitaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(akitaReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 akitaReward = multiplier.mul(akitaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        akita.mint(devaddr, akitaReward.div(10));
        akita.mint(address(this), akitaReward);
        pool.accEggPerShare = pool.accEggPerShare.add(akitaReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe akita transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 akitaBal = akita.balanceOf(address(this));
        if (_amount > akitaBal) {
            akita.transfer(_to, akitaBal);
        } else {
            akita.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _akitaPerBlock) public onlyOwner {
        massUpdatePools();
        akitaPerBlock = _akitaPerBlock;
    }
