...
contract DeHubRewards is DeHubRewardsUpgradeable {
	...
	*/
	function hasAlreadyClaimed(address holderAddr) public view returns(bool) {
		...
		return timeSinceLastClaim < claimCycleHours * (60 * 60);
	}
	...
	*/
	function calcClaimableShare(
		...
		uint totalHoldingAmount = circulatingSupply - lpTokens;
		uint holderAmount = dehubToken.balanceOf(holderAddr);
		uint bnbShare = totalHoldingAmount.percent(holderAmount, 18);
		uint bnbToSend = bnbShare.percentOf(claimableDistribution, 1 * 10 ** 18);
	...
	*/
	function calcRealClaimableShare(address holderAddr) public view returns(uint) {
		uint circulatingSupply = dehubToken.totalCirculatingSupply();
		uint lpTokens = dehubToken.balanceOf(uniswapV2Pair);
		uint bnbToSend = calcClaimableShare(holderAddr, circulatingSupply, lpTokens);
		return bnbToSend;
	}
	...
	*/
	function resetClaimDistributionCycle(uint cycleHours) 
		internal
	{
		require(cycleHours > 0, "Cycle hours can't be 0.");
		...
		returns(uint)
	{
		...
		uint bnbShare = calcRealClaimableShare(claimant);
