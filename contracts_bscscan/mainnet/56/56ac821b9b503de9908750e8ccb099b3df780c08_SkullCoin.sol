...
// File: contracts/SkullCoinDividendPayingToken.sol
...
contract SkullCoin is ERC20, Ownable {
    ...
    SkullCoinDividendTracker public dividendTracker;
    ...
    address _marketingWalletAddress = 0x26d07BE620752EEA04F87419Bd38D4990AcA125c; //change this
    ...
    constructor() ERC20("SkullCoin", "Skull") {
    	...
    	dividendTracker = new SkullCoinDividendTracker();
    ...
    function changeSellFee(uint256 amount) external onlyOwner {
	    require(amount >= 100 && amount <= 990, "SkullCoin: SellFee must be updated to between 100 and 150.");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SkullCoin: The dividend tracker already has that address");

        SkullCoinDividendTracker newDividendTracker = SkullCoinDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SkullCoin: The new dividend tracker must be owned by the SkullCoin token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SkullCoin: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SkullCoin: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SkullCoin: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SkullCoin: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SkullCoin: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SkullCoin: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SkullCoin: Cannot update gasForProcessing to same value");
    ...
    function updateRewardsFee(uint256 RewardFeePercent) external onlyOwner{
        require(RewardFeePercent <= 20, "SkullCoin: updateRewardsFee must be between 0 and 20.");
        TokenRewardsFee = RewardFeePercent;
        totalFees = TokenRewardsFee.add(liquidityFee).add(marketingFee);
    }
    
    function updateLiquidityFee(uint256 liquidityFeePercent) external onlyOwner{
        require(liquidityFeePercent <= 5, "SkullCoin: updateRewardsFee must be between 0 and 5.");
        liquidityFee = liquidityFeePercent;
        totalFees = TokenRewardsFee.add(liquidityFee).add(marketingFee);
    }
    
    function updateMarketingFee(uint256 marketingFeePercent) external onlyOwner{
        require(marketingFeePercent <= 10, "SkullCoin: updateRewardsFee must be between 0 and 10.");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "SkullCoin: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        uint256 initialSkullBalance = IERC20(DividendToken).balanceOf(address(this));
 
        swapTokensForBusd(tokens, address(this));
        uint256 newBalance = (IERC20(DividendToken).balanceOf(address(this))).sub(initialSkullBalance);
...
contract SkullCoinDividendTracker is Ownable, DividendPayingToken{
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SkullCoin_Dividend_Tracker", "SkullCoin_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "SkullCoin: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SkullCoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SkullCoin_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }

    function updateMinimunForRewards(uint256 newValueMinimunForRewards) external onlyOwner {
        require(newValueMinimunForRewards >= 10000 && newValueMinimunForRewards <= 500000, "SkullCoin_Dividend_Tracker: newValueMinimunForRewards must be updated to between 10000 and 500000");
        require(newValueMinimunForRewards != minimunForRewards, "SkullCoin_Dividend_Tracker: Cannot update claimWait to same value");
