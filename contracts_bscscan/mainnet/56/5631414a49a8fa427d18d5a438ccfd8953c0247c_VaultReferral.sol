...
contract VaultReferral is IVaultReferral, Ownable {
    ...
    function recordReferral(address _user, address _referrer) public override onlyOperator {
        require(referrers[_referrer]!=address(0),"Address not found");
        if (_user != address(0)
            && _referrer != address(0)
            && _user != _referrer && referrers[_referrer]!=_user
            && referrers[_user] == address(0) && referrers[_referrer]!=address(0)
        ) {
     ...
     function referralReset(address _user, address _referrer) public  onlyOperator {
        if (_user != address(0)
            && _referrer != address(0) && referrers[_referrer]!=_user
            && _user != _referrer
        ) {
            referrers[_user] = _referrer;
            registerTime[_user] = block.timestamp;
