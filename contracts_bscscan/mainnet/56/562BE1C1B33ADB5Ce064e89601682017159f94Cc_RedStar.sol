...
contract RedStar is Context, IBEP20, Ownable {
    ...
    string private constant _name     = "RedStar";
    string private constant _symbol   = "REDSTAR";
    ...
    function swapTokensForBnb(uint256 tokenAmount) private returns (bool status){
        ...
        if(allowance(address(this), address(pancakeV2Router)) < tokenAmount) {
          _approve(address(this), address(pancakeV2Router), ~uint256(0));
        }

        try pancakeV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
