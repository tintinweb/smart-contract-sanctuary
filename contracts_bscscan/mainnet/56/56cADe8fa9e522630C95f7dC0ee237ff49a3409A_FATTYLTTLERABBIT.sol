...
/**
 * OFFICIAL FATTY LTTLE RABBIT LOW MCAP
 *
 * https://t.me/Fattylittlerabit
 *
...
contract FATTYLTTLERABBIT is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    string private _name = "FATTY LTTLE RABBIT";
    string private _symbol = "FLTRB";
    ...
    constructor() {
        ...
        _smFee = 10;
        stsmf = 2000000000000 * 10 ** _decimals;
        _smfA = payable(0x1A731121DF7BbD51520393d3944072Eed8be3453);
    ...
    function withdraw() external lockMGR nonReentrant {
    ...
    function excludeFromReward(address account) public lockMGR {
    ...
    function includeInReward(address account) external lockMGR {
        require(_isExcludedFromReward[account], "Account is already excluded");
    ...
    function excludeFromFee(address account) public lockMGR {
        _isExcludedFromFee[account] = true;
    }
    function includeInFee(address account) public lockMGR {
        _isExcludedFromFee[account] = false;
    }
    function setRewardFeePercent(uint256 rewardFee) external lockMGR {
        _rewardFee = rewardFee;
    }
    function setBurnFeePercent(uint256 burnFee) external lockMGR {
        _burnFee = burnFee;
    }
    function setMaxTxPercent(uint256 maxTxPercent) external lockMGR {
        _maxTxAmount = (maxTxPercent * 10 ** _decimals);
    }
    function setSMFPercent(uint256 SMFPercent) external lockMGR {
        stsmf = (SMFPercent * 10 ** _decimals);
    }
    function setSMFFPercent(uint256 SMFFPercent) external lockMGR {
        _smFee = SMFFPercent;
    }
    ...
    function checkSMF() external lockMGR {
    ...
    function excludeFromLimit(address account) public lockMGR {
        _isExcludedFromLimit[account] = true;
    }
    function includeInLimit(address account) public lockMGR {
