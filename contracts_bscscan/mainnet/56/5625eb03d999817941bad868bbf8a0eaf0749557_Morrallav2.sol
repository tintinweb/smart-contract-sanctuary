...
contract Morrallav2 is ERC20('Morrallav2.1', 'MORRALLA'), Ownable {
    // burn counter
    ...
    function totalMorrallaBurned() public view returns (uint256) {
        return _burnTotal;
    }
...
// File contracts/Micheladasv2.sol

pragma solidity ^0.8.4;

contract Micheladasv2 is ERC20('Micheladav2', 'MICHELADA'), Ownable {
    ...
    // Morralla
    Morrallav2 public morralla;

    constructor(
        Morrallav2 _morralla
    )  {
        morralla = _morralla;
    }
...
contract Tianguisv2 is Ownable {
    ...
    //  Tokens
    Morrallav2 public morralla;
    Micheladasv2 public micheladas;
    // farm parameters
    ...
    function deposit(uint256 _pid, uint256 _amount) public lock {
        require (_pid != 0, 'Deposita MORRALLA con enterMicheladas!');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public lock {
        require (_pid != 0, 'Retira MORRALLA con leaveMicheladas!');

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "Cantidad excede lo depositado!");
    ...
    function leavMicheladas(uint256 _amount) public lock {
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[0][msg.sender];
        require(user.amount >= _amount, "No tienes bastante micheladas!");
        updatePool(0);
        uint256 pending = user.amount * pool.accMorrallaPerShare / 1e12 - user.rewardDebt;
        if(pending > 0) {
            safeMorrallaTransfer(msg.sender, pending);
