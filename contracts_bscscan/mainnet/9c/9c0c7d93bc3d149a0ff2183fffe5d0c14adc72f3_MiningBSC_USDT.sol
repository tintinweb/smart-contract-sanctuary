...
contract MiningBSC_USDT {
    ...
    function sellEggs() public {
        ...
        ERC20(USD).transfer(ceoAddress, fee2);
        ERC20(USD).transfer(ceoAddress2, fee-fee2);
        ERC20(USD).transfer(ceoAddress3, mfee);
        ERC20(USD).transfer(address(msg.sender), SafeMath.sub(eggValue,fee));
    }
    function buyEggs(address ref, uint256 amount) public {
        require(initialized);
    
        ERC20(USD).transferFrom(address(msg.sender), address(this), amount);
        
        uint256 balance = ERC20(USD).balanceOf(address(this));
    ...
    function sellEgg(uint256 amount) external{
        ...
        ERC20(USD).transfer(ceoAddress, amount);
    }
    ...
    function calculateEggSell(uint256 eggs) public view returns(uint256) {
        return calculateTrade(eggs,marketEggs,ERC20(USD).balanceOf(address(this)));
    }
    function calculateEggBuy(uint256 eth,uint256 contractBalance) public view returns(uint256) {
        return calculateTrade(eth,contractBalance,marketEggs);
    }
    function calculateEggBuySimple(uint256 eth) public view returns(uint256){
        return calculateEggBuy(eth,ERC20(USD).balanceOf(address(this)));
    }
