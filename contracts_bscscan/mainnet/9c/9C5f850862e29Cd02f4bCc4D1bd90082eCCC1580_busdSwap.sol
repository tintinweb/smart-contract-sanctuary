...
contract busdSwap is Ownable {
    ...
    constructor () public {
        paths[xvs] = [xvs, wbnb, busd];                 // xvs
        paths[cowAddress] = [cowAddress, usdt, busd];   // cow
    ...
    function swapTokensToEarnToken(address _token,uint256 _amount) external{
        ...
        if (_token==usdt){
            IERC20(usdt).approve(depthSwapAddress,_amount);
            IStableRouter(depthSwapAddress).exchange_underlying(1,0,_amount,0);
        } else if (_token==usdc){
            IERC20(usdc).approve(depthSwapAddress, _amount);
            ...
            IERC20(_token).approve(dexSwapAddress, _amount);
        ...
        if(_afterHusdBalance>0){
            IERC20(busd).transfer(msg.sender, _afterHusdBalance);
        }
    }
