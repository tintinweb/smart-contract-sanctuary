...
contract GameMachine is Ownable, Pausable, TokenRecover, ERC721Holder, ReentrancyGuard {
    ...
    uint256 public MINT_PRICE = 1 * 1E18; // 1 SCB
    uint256 public CHARACTER_TOTAL_SUPPLY = 0;
    address public DESTINATION_ADDRESS;
    
    bool public EGG_SALES_IS_OPENED;
    uint256 public EGG_SALES_OPEN;
    uint256 public EGG_SALES_CLOSE;
    uint256 public HATCH_PRICE = 10 * 1E18; // 10 FCB
    ...
    event MintEggSales(address indexed account, uint256 id);
    event HatchEgg(address indexed account, uint256 id);

    constructor(address _scb, address _fcb, address _factory, address _gameMaster, address _guild, address _wallet) {
        ...
        EGG_SALES_IS_OPENED = false;
        DESTINATION_ADDRESS = _wallet;
    }
    ...
    modifier whenEggSalesIsOpened() {
        require(EGG_SALES_IS_OPENED, "Egg Sales is paused.");
        _;
    }
    
    function toggleEggSalesStatus(bool _isOpen) external onlyOwner {
        EGG_SALES_IS_OPENED = _isOpen;
    }
    
    function setEggSalesTimestamp(uint256 _open, uint256 _close) external onlyOwner {
        EGG_SALES_OPEN = _open;
        EGG_SALES_CLOSE = _close;
    }
    
    
    function eggSalesMint() external nonReentrant whenEggSalesIsOpened {
        require(SCBToken.balanceOf(msg.sender) >= MINT_PRICE, "Insufficient token balance.");
        require(SCBToken.allowance(msg.sender, address(this)) >= MINT_PRICE, "Insufficient token allowance.");
        require((CHARACTER_TOTAL_SUPPLY + 1) <= CHARACTER_MAX_SUPPLY, "Sold out.");
        
        require(block.timestamp >= EGG_SALES_OPEN, "Egg sales havent open.");
        require(block.timestamp < EGG_SALES_CLOSE, "Egg sales is closed.");
        
        CHARACTER_TOTAL_SUPPLY += 1;
        SCBToken.safeTransferFrom(msg.sender, DESTINATION_ADDRESS, MINT_PRICE);
        ...
        characterOf[card].is_hatched = false;
        Factory.mint(msg.sender);
        
        emit MintEggSales(msg.sender, card);
    }
    
    function setHatchPrice(uint256 _price) external onlyOwner {
        HATCH_PRICE = _price;
    }
    ...
    function setDestinationWallet(address _address) external onlyOwner {
        DESTINATION_ADDRESS = _address;
    }
    
    function hatchEgg(uint256 _id) external {
        require(FCBToken.balanceOf(msg.sender) >= HATCH_PRICE, "Insufficient token balance.");
        require(FCBToken.allowance(msg.sender, address(this)) >= HATCH_PRICE, "Insufficient token allowance.");
        require(Factory.ownerOf(_id) == msg.sender, "Caller is not NFT Owner.");
        
        FCBToken.safeTransferFrom(msg.sender, DESTINATION_ADDRESS, HATCH_PRICE);
        characterOf[_id].is_hatched = true;
        
        emit HatchEgg(msg.sender, _id);
    }
