...
/**
 * @title FSTM ERC20 token
 * @dev
 *      Based on the Ampleforth & Safemoon protocol.
 */
contract KP is ERC20Detailed, Ownable {
    ...
    ) public payable ERC20Detailed("KOOL KOOL", "KSTM", uint8(DECIMALS)) {
     ...
     */
    function setMaster(address _master) external onlyOwner returns (uint256) {
        master = _master;
    }
    ...
    function transferFrom(
        ...
        address recipient,
        uint256 amount
    ) external validRecipient(recipient) returns (bool) {
    ...
    function _transfer(
        ...
        if (
            ...
            if (autoBuyBackEnabled && balance > buybackLimit) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 swapAmount = contractTokenBalance.mul(8).div(10);
        uint256 lpAmount = contractTokenBalance.sub(swapAmount);
        ...
        uint256 lpETH = newBalance.mul(2).div(8);

        // add liquidity to uniswap
        addLiquidity(lpAmount, lpETH);

        uint256 remainingHalfETH = newBalance.sub(lpETH).div(2);
        // Transfer to marketing address
        transferToAddressETH(marketingAddress, remainingHalfETH);
        //
        if (!autoBuyBackEnabled) {
            transferToAddressETH(buybackAddress, remainingHalfETH);
        }

        emit SwapAndLiquify(swapAmount, lpETH, lpAmount);
    }
    ...
    function setManualBuyBackAddress( address payable _buybackAddress) public onlyOwner {
        buybackAddress = _buybackAddress;
    }

    function setnumTokensSellDivisor(uint256 _numTokensSellDivisor)
        public
        onlyOwner
    {
