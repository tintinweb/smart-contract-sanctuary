...
contract KIPSLockedWallet is Ownable {
    ...
    address public MBAToken = 0x1c57eEed03F36d8954c6a54b0624E2c094841eF1;
    address public lpToken = 0x08398A04C1E4289dB4cAB1A9B78Fc9B40A5e9A25;

    /**
     * @notice Transfers tokens held by timelock to owner.
     */
    function releaseMBAToken() external onlyOwner {
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp >= releaseTime, "TokenTimelock: current time is before release time");

        uint256 amount = IERC20(MBAToken).balanceOf(address(this));
        require(amount > 0, "TokenTimelock: no tokens to release");

        IERC20(MBAToken).safeTransfer(owner(), amount);
    }
    ...
    function getMBATokenBalance() external view returns(uint256) {
        return IERC20(MBAToken).balanceOf(address(this));
    }
    ...
    function setMBAToken(address _MBAToken) external onlyOwner {
        MBAToken = _MBAToken;
    }
