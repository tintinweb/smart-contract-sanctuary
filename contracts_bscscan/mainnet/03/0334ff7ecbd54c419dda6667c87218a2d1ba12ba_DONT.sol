...
Welcome To the DONT Community!
 

Website: https://www.DONT.org/
TG: https://t.me/DONTbsc
Twitter:https://twitter.com/DONTBsc
...
abstract contract DONTBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	// MAIN TOKEN PROPERTIES
	string private constant NAME = "DONT";
	string private constant SYMBOL = "DONT";
	...
	constructor (address routerAddress) {
		...
		// Initialize PancakeSwap V2 router and DONT <-> BNB pair.  Router address will be: 0x10ed43c718714eb63d5aa57b78b54704e256024e or for testnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
	...
	// This function swaps a {tokenAmount} of DONT tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for DONT -> WBNB
	...
	// Returns how many more $DONT tokens are needed in the contract before triggering a swap
...
contract DONT is DONTBase {
	...
	uint256 private _totalBNBAsDONTClaimed; // The total number of BNB that was converted to DONT and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsDONTClaimed; // The amount of BNB converted to DONT and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy DONT automatically
	...
	bool private _reimburseAfterDONTClaimFailure; // If true, and DONT reward claim portion fails, the portion will be given as BNB instead
	...
	constructor (address routerAddress) DONTBase(routerAddress) {
	...
	function onActivated() internal override {
		...
		setReimburseAfterDONTClaimFailure(true);
	...
	function onBeforeTransfer(address sender, address recipient, uint256 amount) internal override {
		...
		bool isSelling = isPancakeSwapPair(recipient);
		if (!isSelling) {
			// Wait for a dip, DONT diamond hands
			return;
		}
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough DONT balance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim DONT tokens
		if (!claimDONT(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterDONTClaimFailure) {
	...
	function claimDONT(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsDONTClaimed[user] += bnbAmount;
		_totalBNBAsDONTClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $DONT on a zero balance address:
		...
		// Depending on the % of $DONT tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsDONT(address byAddress) public view returns (uint256) {
		return _bnbAsDONTClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsDONT() public view returns (uint256) {
		return _totalBNBAsDONTClaimed;
	}
	...
	function reimburseAfterDONTClaimFailure() public view returns(bool) {
		return _reimburseAfterDONTClaimFailure;
	}


	function setReimburseAfterDONTClaimFailure(bool value) public onlyOwner {
		_reimburseAfterDONTClaimFailure = value;
	}
