...
contract FlyingShibaNFT is ERC721Enumerable, Ownable{
    ...
    constructor(address _beneficiary) ERC721("FlyingShiba", "FlyingShiba") {
        _baseTokenURI = "https://flyingshiba.findmynft.net/api/";
    ...
    function mint_FlyingShiba() public payable returns (uint256){
        require(totalSupply() <= 255, "enough minting");

        _tokenIds.increment();
        uint256 newFlyingShiba = _tokenIds.current();
        uint256 price = getPrice(newFlyingShiba);
        require(msg.value >= price, "not enough");

        _mint(msg.sender, newFlyingShiba);
        emit minting(newFlyingShiba);

        return newFlyingShiba;
    }
