...
contract EtBUSD is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "EternalBUSD"; //token name #4 
    string constant _symbol = "E-BUSD"; // token tika #5
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100 * 10**9 * (10 ** _decimals);
    uint256 public _maxTxAmount = ( _totalSupply * 1 )  / 200; //max transaction % #6
    ...
    uint256 liquidityFee    = 2; // ammount to LP #7
    uint256 reflectionFee   = 7; // ammount to rewards
    uint256 marketingFee    = 6; // ammount to marketing wallet
    uint256 public totalFee = 20; //Total Fee
    uint256 feeDenominator  = 100;// must be 100

    uint256 sellfeeMult = 33; 
    uint256 sellfeeDenom = 100; //33% sell fee increase
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xB62d119D7b75C67494542Eb3c5Fd129B9A9fDA2a   ;
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        uint256 sellfeeadd = feeAmount.mul(sellfeeMult).div(sellfeeDenom);
        uint256 sellfeeAmount = feeAmount.add(sellfeeadd);
