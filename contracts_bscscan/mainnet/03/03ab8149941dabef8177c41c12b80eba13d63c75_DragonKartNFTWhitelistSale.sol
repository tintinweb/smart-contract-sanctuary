...
contract DragonKartNFTWhitelistSale is AccessControlEnumerable {
    ...
    function buy(uint256 batchNumber_) external batchExisted(batchNumber_) {
        require(
            ...
            "DragonKartNFTWhitelistSale: The batch is unavailable"
        );
        require(
        ...
        onlyClaimable(batchNumber_, _msgSender())
    {
        ...
        ExtraReward storage _reward = _extraRewards[batchNumber_];
        if (_reward.sponsor != address(0)) {
    ...
    function whitelistBatches() external view returns (uint256[] memory) {
        return _whitelistBatches.values();
    }
