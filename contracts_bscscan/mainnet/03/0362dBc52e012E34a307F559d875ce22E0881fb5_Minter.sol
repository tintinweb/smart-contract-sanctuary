...
contract Minter is AccessControlEnumerable, ReentrancyGuard {
    bytes32 public constant OWNER_ROLE = keccak256("OWNER_ROLE");
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    bool isEnabled = false;

    mapping(uint256 => uint256) mintedOnSwap;

    event EnableMinter(bool isEnabled);
    event DisableMinter(bool isEnabled);
    event Mint(address from, address to, address token, uint256 amount, uint256 mintedOnSwap);
    ...
    function getMinted(uint256 swapId_) external view returns(uint256) {
        return mintedOnSwap[swapId_];
    }

    function enableMinter() external onlyRole(OWNER_ROLE) {
        isEnabled = true;
        emit EnableMinter(isEnabled);
    }
    
    function disableMinter() external onlyRole(OWNER_ROLE) {
        isEnabled = false;
        emit DisableMinter(isEnabled);
    }

    function mint(address to_, address tokenAddress_, uint256 amount_, uint256 swapId_) external onlyRole(ADMIN_ROLE) {
        PackNFT token = PackNFT(tokenAddress_);
        
        require(isEnabled == true, "Minting is disabled");
        require(amount_ > 0, "Invalid amount");
        token.mint(to_, swapId_, amount_, true, 0, amount_ + 1);
        mintedOnSwap[swapId_] += amount_;

        emit Mint(msg.sender, to_, tokenAddress_, amount_, mintedOnSwap[swapId_]);
    }
}
