...
LotteryHelper {
    ...
    string constant _name = "CVN";
    string constant _symbol = "CVN";
    ...
    address public separateBuybackReceiver = 0x7532af0c82d1bdd6F551781bD43176ACe132085a;
    address public marketingAndOperationsFeeReceiver = 0x86a1E1EcD8766c4aB1075D0834C1747b4926B192;
    ...
    function callAfterDeployAndSetOwnersOnLotteryContract() external onlyOwner {
        ...
        setLotteryDrawThreshold(10**17, 10**17); // set the threshold for hourly and daily lottery contracts
    }

    function callAfterLiquidityIsAdded() external onlyOwner {
        require(!liquidityIsAdded);
        liquidityIsAdded = true;
        setSwapEnabledAndThreshold(true, _totalSupply / 20000); // 0.005 %
        setLiquidityEnabledAndThreshold(true, _totalSupply / 1000000);
        setAutoBuybackEnabledAndThreshold(true, 10**10); // 0.1 BNB
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (liquidityIsAdded) {
            ...
            if (!done) {
                tryAutoBuyback(sender, recipient);
                trySwapAndSendBUSDToLottery(sender);
            ...
            if (!done) {
                if (shouldAddLiquidity(sender, accumulatedForLiquidity)) {
                    swapAndAddLiquidity(accumulatedForLiquidity);
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _separateBuybackFee, uint256 _marketingFee, uint256 _lotteryFee) external onlyOwner {
