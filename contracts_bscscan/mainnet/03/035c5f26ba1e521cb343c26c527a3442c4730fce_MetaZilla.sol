 ...
 */
contract MetaZilla is Context, IBEP20, Ownable, ReentrancyGuard {
     ...
     * construction.
     */
    string private constant _NAME = "MetaZillaToken";
    string private constant _SYMBOL = "MetaZilla";
    ...
    address public developmentWallet =
        0x7523B1E63584a69408194E93B176C81C1798ABEE;
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private {
        (uint256 tokenAmountAdded, uint256 bnbAmountAdded, ) = pancakeswapV2Router
    ...
    function _tokenTransfer(
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(
                sender,
                recipient,
                (amount.sub(burnAmt).sub(developmentAmt))
            );
     ...
     */
    function migrateBalanceBnb(address payable recipient, uint256 amount)
        external
        onlyOwner
        nonReentrant
    {
    ...
    function setRouterAddress(address newRouter) external onlyOwner {
        ...
        if (get_pair == address(0)) {
            pancakeswapV2Pair = IPancakeFactory(_newRouter.factory())
                .createPair(address(this), _newRouter.WETH());
        } else {
    ...
    function approveDistributorAddress(address approvedAddress)
        public
        onlyOwner
    {
