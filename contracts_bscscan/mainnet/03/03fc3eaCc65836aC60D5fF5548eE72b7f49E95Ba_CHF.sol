...
contract CHF is IBEP20, Ownable {
    ...
    uint256 private _burnTokensPooled = 0;
   ...
   * - `spender` cannot be the zero address.
    ...
    */
    ...
    /**
     * @notice Send tokens to the dead address
     *
    ...
    function burnTokens(uint256 tokenAmount) private {
        ...
        _balances[_deadAddress] += tokenAmount;
    ...
    function _swapAndLiquify(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 liquidityCut = (_liquidityTokensPooled * tokenAmount) / (_liquidityTokensPooled + _marketingTokensPooled + _burnTokensPooled);
        if (liquidityCut > _liquidityTokensPooled) { liquidityCut = _liquidityTokensPooled; }

        uint256 burnCut = (_burnTokensPooled * tokenAmount) / (_liquidityTokensPooled + _marketingTokensPooled + _burnTokensPooled);
        if (burnCut > _burnTokensPooled) { burnCut = _burnTokensPooled; }

        uint256 marketingCut = tokenAmount - liquidityCut - burnCut;
    ...
    */
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (takeFee) {
            ...
            if ((tx.gasprice > _maxGasPrice && ((block.number - _tradingEnabledBlock) < 10))  || block.number == _tradingEnabledBlock) {
