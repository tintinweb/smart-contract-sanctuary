...
contract BPManager is AccessControl{
    ...
    function setAutoBlacklistCount(uint256 count) external onlyRole(BOT_MANAGEMENT_ROLE){
        _botProtectionContract.setAutoBlacklistCount(count);
    }

    function setBuyCoolDown(uint256 buyCoolDown) external onlyRole(BOT_MANAGEMENT_ROLE){
        _botProtectionContract.setBuyCoolDown(buyCoolDown);
    }

    function setSellCoolDown(uint256 sellCooldown) external onlyRole(BOT_MANAGEMENT_ROLE){
        _botProtectionContract.setSellCoolDown(sellCooldown);
    }

    function setMaxBuyAmount (uint256 maxBuyAmount) external onlyRole(BOT_MANAGEMENT_ROLE){
        _botProtectionContract.setMaxBuyAmount(maxBuyAmount);

    }

    function setMaxSellAmount(uint256 maxSellAmount) external onlyRole(BOT_MANAGEMENT_ROLE){
        _botProtectionContract.setMaxSellAmount(maxSellAmount);
    }

    function setBuyCoolDownFastestBuyers(uint256 buyCoolDown) external onlyRole(BOT_MANAGEMENT_ROLE){
        _botProtectionContract.setBuyCoolDownFastestBuyers(buyCoolDown);

    }

    function setSellCoolDownFastestBuyers(uint256 sellCooldown) external onlyRole(BOT_MANAGEMENT_ROLE){
        _botProtectionContract.setSellCoolDownFastestBuyers(sellCooldown);
    }

    function setMaxBuyAmountFastestBuyers(uint256 maxBuyAmount)  external onlyRole(BOT_MANAGEMENT_ROLE){
        _botProtectionContract.setMaxBuyAmountFastestBuyers(maxBuyAmount);
    }

    function setMaxSellAmountFastestBuyers(uint256 maxSellAmount) external onlyRole(BOT_MANAGEMENT_ROLE){
        _botProtectionContract.setMaxSellAmountFastestBuyers(maxSellAmount);
    }


    function excludeFromTransferLimitations(address addressToExclude) external onlyRole(BOT_MANAGEMENT_ROLE){
        _botProtectionContract.excludeFromTransferLimitations(addressToExclude);
    }

    function includedAddressesFromTransferLimitations (address addressToRemove) external onlyRole(BOT_MANAGEMENT_ROLE){
        _botProtectionContract.includedAddressesFromTransferLimitations(addressToRemove);
    }

    function multiBlacklist(address [] memory addressesToBlacklist) external onlyRole(BOT_MANAGEMENT_ROLE){
        for(uint256 i=0; i<addressesToBlacklist.length; i++){
            _botProtectionContract.manualBlackList(addressesToBlacklist[i]);
        }
    }

    function multiRemoveFromBlacklist(address [] memory addressesToBlacklist) external onlyRole(BOT_MANAGEMENT_ROLE){
        for(uint256 i=0; i<addressesToBlacklist.length; i++){
    ...
    function getSellCooldown() external view returns (uint256){
        return _botProtectionContract._sellCooldown();
    }

    function getBuyCooldown() external view returns (uint256){
