...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable Dividendstoken = address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43);
...
contract BullishFloki is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BullishFlokiDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 public maxBuyTranscationAmount = 1000000000 * (10**18); // 1%
    uint256 public maxSellTransactionAmount = 500000000 * (10**18); // 0.5%
    uint256 public swapTokensAtAmount = 500000000 * (10**18);
    uint256 public _maxWalletToken = 2000000000 * (10**18); // 2% of total supply
    ...
    address public _marketingWalletAddress = 0x7Df1cCD11e094D01bd1325483bCBa25c00357fcB;

    // sells have fees of 8 and 8 (10 * 0.8 and 10 * 0.8)
    ...
    uint256 private constant DefaultLiquidityLockTime=1 days; //check tg group for more info
    ...
    constructor() public ERC20("BullishFloki", "BF") {
    	...
    	dividendTracker = new BullishFlokiDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BullishFloki: The dividend tracker already has that address");

        BullishFlokiDividendTracker newDividendTracker = BullishFlokiDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BullishFloki: The new dividend tracker must be owned by the main token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BullishFloki: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BullishFloki: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BullishFloki: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BullishFloki: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BullishFloki: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BullishFloki: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BullishFloki: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BullishFloki: This account cannot send tokens until trading is enabled");
        }
...
contract BullishFlokiDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BullishFloki_Dividend_Tracker", "BullishFloki_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 100000 * (10**18); //must hold  tokens
    }



    function _transfer(address, address, uint256) internal override {
        require(false, "BullishFloki_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BullishFloki_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main  contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BullishFloki_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BullishFloki_Dividend_Tracker: Cannot update claimWait to same value");
