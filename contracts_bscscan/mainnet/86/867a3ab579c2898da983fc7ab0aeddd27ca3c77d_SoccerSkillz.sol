...
// Play Soccer Skillz on: www.soccerskillz.football
// P2E Launching: 11/03/2021
// Telegram: https://t.me/SoccerSkillz
...
contract SoccerSkillz is Context, IERC20, Ownable {
    ...
    string private _name = "Soccer Skillz";
    string private _symbol = "SOCC";
    ...
    uint256 private _previousTaxFee = _burnTax;
    ...
    uint256 public _maxWalletAmount = 1 * 10**5 * 10**9; 
    uint256 public numTokensSellToAddToLiquidity = 5 * 10**3 * 10**9;
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_burnTax).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_burnTax == 0 && _liquidityFee == 0 && _burnFee == 0) return;
        
        _previousTaxFee = _burnTax;
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && to != _addressUniswapV2Router02 && to != uniswapV2Pair && to != address(this))
    ...
    function _setBurnFee(uint256 burnFee) external onlyOwner() {
        require(burnFee < _burnTax, 'burnFee should be less than taxFee');
        _burnFee = burnFee;
    }
}