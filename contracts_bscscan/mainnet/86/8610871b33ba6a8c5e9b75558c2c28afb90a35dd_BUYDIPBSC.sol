...
contract BUYDIPBSC is ERC20, Ownable {
    ...
    address public _investmentWalletAddress = address(0x85140d508f6A4691417D5d85D1677641b3C6407a); //Investment wallet
    ...
    constructor() ERC20("BUY DIP", "BDBSC") {
        ...
        investmentFee = 40; //4% investment fee
        liquidityFee = 40;  // 4% liquidity fee
    ...
    function setinvestmentFee(uint16 investment) external onlyOwner {
        investmentFee = investment;
    }

    function setLiquidityFee(uint16 liquidity) external onlyOwner {
    ...
    function _transfer(
        ...
        require(!_isBlackListed[from] && !_isBlackListed[to],"This account is blacklisted");
        ...
        if(takeFee) {
            require(isTradingEnabled,"TOKEN: Trading not enabled yet");

            uint16 totalFee = investmentFee + liquidityFee;
    ...
    function swapAndSend(uint256 tokens) private {
        ...
        uint256 totalFees = investmentFee + liquidityFee;

        uint256 forLiquidity = tokens.mul(
            liquidityFee).div(totalFees);

        uint256 forinvestment = tokens.mul(
            investmentFee).div(totalFees);    

        uint256 half = forLiquidity.div(2);

        // swap tokens for ETH
        swapTokensForEth(half + forinvestment); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        ...
        payable(_investmentWalletAddress).transfer(address(this).balance);

        emit SwapAndSend(forLiquidity, forinvestment, liquidityShare);
    }
