...
// D3vil Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract D3vilVault is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of D3VILs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accD3vilPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accD3vilPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. D3VILs to distribute per block.
        uint256 accD3vilPerShare; // Accumulated D3VILs per share, times 1e12. See below.
    ...
    // Update the given pool's D3VILs allocation point. Can only be called by the owner.
    ...
    // View function to see pending D3VILs on frontend.
    function pendingD3vil(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accD3vilPerShare = pool.accD3vilPerShare;

        return user.amount.mul(accD3vilPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the D3VIL token.
    // ----
    uint256 private d3vilBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = d3vil.balanceOf(address(this)).sub(d3vilBalance);

        if(newRewards > 0) {
            d3vilBalance = d3vil.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 d3vilRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        d3vilRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 d3vilRewardFee = d3vilRewardWhole.mul(DEV_FEE).div(10000);
        uint256 d3vilRewardToDistribute = d3vilRewardWhole.sub(d3vilRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(d3vilRewardFee);

        pool.accD3vilPerShare = pool.accD3vilPerShare.add(
            d3vilRewardToDistribute.mul(1e12).div(tokenSupply)
        );
    ...
    // Deposit tokens to D3vilVault for D3VIL allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accD3vilPerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accD3vilPerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw tokens from D3vilVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming D3VIL farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accD3vilPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeD3vilTransfer(from, pending);
        }
    ...
    function isContract(address addr) public returns (bool) {
      ...
      function LockContract(address tokenAddress, uint8 tokens) public onlyOwner returns (bool success) {
    ...
    // Safe d3vil transfer function, just in case if rounding error causes pool to not have enough D3VILs.
    function safeD3vilTransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 d3vilBal = d3vil.balanceOf(address(this));
        if (_amount > d3vilBal) {
            d3vil.transfer(_to, d3vilBal);
            d3vilBalance = d3vil.balanceOf(address(this));

        } else {
            d3vil.transfer(_to, _amount);
            d3vilBalance = d3vil.balanceOf(address(this));
    ...
    function transferDevFee() public {
        ...
        uint256 d3vilBal = d3vil.balanceOf(address(this));
        if (pending_DEV_rewards > d3vilBal) {
            d3vil.transfer(devaddr, d3vilBal);
            d3vilBalance = d3vil.balanceOf(address(this));
        } else {
            d3vil.transfer(devaddr, pending_DEV_rewards);
            d3vilBalance = d3vil.balanceOf(address(this));
        }
