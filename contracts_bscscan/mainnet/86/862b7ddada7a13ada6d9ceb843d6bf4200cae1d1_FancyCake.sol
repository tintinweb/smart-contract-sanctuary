...
Telegram : https://t.me/FancyCakeOfficial
Website : fancycake.xyz
...
//SPDX-License-Identifier: MIT
	 ...
	 * This allows applications to reconstruct the allowance for all acFancyCake just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also FancyCakeed.
	...
	// File: contracts/FancyCakeDividendPayingToken.sol
	...
	contract FancyCake is ERC20, Ownable {
		...
		address public CAKECoin = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); 
	
		bool private swapping;

		FancyCakeDividendTracker private dividendTracker;
		...
		event ExcludeMultipleAcFancyCakeFromFees(address[] acFancyCake, bool isExcluded);
		...
		constructor() ERC20("FancyCake", "FANCY"){
			...
			dividendTracker = new FancyCakeDividendTracker();
		...
		function excludeFromFees(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "FancyCake: Account is already the value of 'excluded'");
		...
		function excludeMultipleAcFancyCakeFromFees(address[] calldata acFancyCake, bool excluded) public onlyOwner {
			for(uint256 i = 0; i < acFancyCake.length; i++) {
				_isExcludedFromFees[acFancyCake[i]] = excluded;
			}

			emit ExcludeMultipleAcFancyCakeFromFees(acFancyCake, excluded);
		}

		function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
			require(pair != uniswapV2Pair, "FancyCake: The uniswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "FancyCake: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "FancyCake: The liquidity wallet is already this address");
		...
		function updateGasForProcessing(uint256 newValue) private onlyOwner {
			require(newValue >= 200000 && newValue <= 500000, "FancyCake: gasForProcessing must be between 200,000 and 500,000");
			require(newValue != gasForProcessing, "FancyCake: Cannot update gasForProcessing to same value");
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if(!hasLaunched) {
				require(canTransferBeforehasLaunched[from], "FancyCake: This account cannot send tokens until trading is enabled");
			}
	...
	contract FancyCakeDividendTracker is DividendPayingToken, Ownable {
		using SafeMath for uint256;
		using SafeMathInt for int256;
		...
		constructor() public DividendPayingToken("FancyCake_Dividend_Tracker", "FancyCake_Dividend_Tracker") {
			claimWait = 3600;
			minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
		}

		function _transfer(address, address, uint256) internal override {
			require(true, "FancyCake_Dividend_Tracker: No transfers allowed");
		}
		...
		function updateClaimWait(uint256 newClaimWait) external onlyOwner {
			require(newClaimWait >= 1 && newClaimWait <= 86400, "FancyCake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
			require(newClaimWait != claimWait, "FancyCake_Dividend_Tracker: Cannot update claimWait to same value");
