...
abstract contract CandleGenieDiceBruteShield
{
     function CheckRoll(uint256 requestId) external virtual returns (bool passed);
     function GetResult(CandleGenieDice.DiceRoll roll) external view virtual returns (int result);
}
...
contract CandleGenieDiceRNG is VRFConsumerBase, ReentrancyGuard, Ownable 
{
    ...
    // Game
    address constant public gameContractAddress = 0xC4AfEbB71fBEF942A44f1120DC65c77cc18DA060;
    ...
    // Brute Shield
    address public bruteShieldAddress;
    CandleGenieDiceBruteShield internal bruteShieldContract;
    ...
    function setBruteShield(address _bruteShieldAddress) external onlyOwner {
        
        bruteShieldAddress = _bruteShieldAddress;
        bruteShieldContract = CandleGenieDiceBruteShield(bruteShieldAddress);
    }
    ...
    function RollWithVRF() external onlyGameContract returns (uint256 requestId) 
    {
        ...
        require(bruteShieldAddress != address(0), "Brute shield contract not set");  
    ...
    function RollWithPSEUDO() external onlyGameContract returns (uint256 requestId) 
    {
        ...
        uint256 hash = uint256(keccak256(abi.encodePacked(rollCount + block.timestamp + block.number)));
        rollCount++;
        return hash;
    }
    ...
    {
        finalizeRoll(uint256(requestId), randomness);  
    }
    ...
    function finalizeRoll(uint256 requestId, uint256 randomness) internal
    {
        
        CandleGenieDice.Bet memory currentBet = gameContract.getBet(requestId);
        require(currentBet.betAmount != 0 ,"RandomSource: Bet not found !");

        
        // -----------------------------------DISCLAMER-------------------------------------------
        // In order to protect game reserve and to keep game running we have to deal with lots of 
        // bot or hacker attacks. We have defined several rules that must any placed roll have to
        // pass it before count as valid one. These securtiy checks prevent contract from bankrupt
        // and malicious operations by attackers.
        //----------------------------------------------------------------------------------------

        // CHECKING CURRENT ROLL
        if (!bruteShieldContract.CheckRoll(requestId))
        {
            // ILLEGAL ROLL
            gameContract.Drop(requestId, bruteShieldContract.GetResult(currentBet.guess));  
            return;
        }
        ...
        // PASSED ROLL
        uint256 dropResult =  randomness % 6 + 1;
        gameContract.Drop(requestId, int(dropResult)); 

    }
    
    function getRollToPosition(CandleGenieDice.DiceRoll roll) internal pure returns (int result)
    {
         if (roll == CandleGenieDice.DiceRoll.One) return 1;
         if (roll == CandleGenieDice.DiceRoll.Two) return 2;
         if (roll == CandleGenieDice.DiceRoll.Three) return 3;
         if (roll == CandleGenieDice.DiceRoll.Four) return 4;
         if (roll == CandleGenieDice.DiceRoll.Five) return 5;
         if (roll == CandleGenieDice.DiceRoll.Six) return 6;
         return 0;
    }
    
    function getPositionToRoll(int position) internal pure returns (CandleGenieDice.DiceRoll result)
    {
         if (position == 1) return CandleGenieDice.DiceRoll.One;
         if (position == 2) return CandleGenieDice.DiceRoll.Two;
         if (position == 3) return CandleGenieDice.DiceRoll.Three;
         if (position == 4) return CandleGenieDice.DiceRoll.Four;
         if (position == 5) return CandleGenieDice.DiceRoll.Five;
         if (position == 6) return CandleGenieDice.DiceRoll.Six;
         
         return CandleGenieDice.DiceRoll.None;
    }
