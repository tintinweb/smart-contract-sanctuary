...
contract BondMaster is Ownable {
  event AddBond(uint256 _amount, address _erc20, uint256 _releaseTime);
  event ClaimBond(address owner, uint256 tokenId, uint256 _amount, address _erc20);
  ...
  function conceiveBond(uint256 _amount, address _erc20, uint256 _releaseTime, string memory _tokenURI) external returns (bool) {
    require(_releaseTime > block.timestamp, "release time need to be a future time");
    require(_amount > 0, "amount not valid");
    require(_erc20.code.length > 0,"not a valid contract address");
    ...
    nftBond.safeMint(msg.sender, numOfBonds, _tokenURI);
  ...
  function _addBond(uint256 _tokenId, uint256 _amount, address _erc20, uint256 _releaseTime) internal {
    ...
    emit AddBond(_amount,_erc20,_releaseTime);
  }
  ...
  function claimBond(uint tokenId) external isReleased(tokenId) isOwner(tokenId) returns (bool) {
    ...
    emit ClaimBond(msg.sender, bond.amount, tokenId, bond.erc20);
    return true;
  }
}
...
contract BondNFT is ERC721, AccessControl {
    ...
    constructor() ERC721("Crypto Bearer Bond", "CBB") {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "BondNFT: URI query for nonexistent token");
    ...
    function burn(uint tokenId) public {
      ...
      require(_exists(tokenId), "BondNFT: approved query for nonexistent token");
      _burn(tokenId);
    }
