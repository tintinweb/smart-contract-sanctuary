...
/* 
  ...
  ctrl+f CHANGEME to find all the places to change
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0xF8A0BF9cF54Bb92F17374d9e9A321E6a111a51bD;  //CHANGEME  to whatever contract. This is ADA
...
contract TestLink is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public _dividendToken = 0xF8A0BF9cF54Bb92F17374d9e9A321E6a111a51bD;  // CHANGEME to whatever contract you picked
    ...
    bool public liquidityIsEnabled = false;

    TESTLINKDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 public maxBuyTranscationAmount = 500000000 * (10**18);  //0.5%  CHANGEME for max buy
    uint256 public maxSellTransactionAmount = 250000000 * (10**18); //0.25%  CHANGEME for max sell
    uint256 public swapTokensAtAmount = 20000000 * (10**18);
    uint256 public maxWalletToken = 2000000000 * (10**18); // 2.0% of total supply   CHANGEME for max wallet
    ...
    uint256 public sellFeeIncreaseFactor = 120;  //CHANGEME for sell %
    ...
    constructor() ERC20("TestLink","TESTLINK") {
        uint256 _dividendRewardsFee = 10;  //CHANGEME for dividends %
        uint256 _marketingFee = 5;  //CHANGEME for marketing wallet %

        dividendRewardsFee = _dividendRewardsFee;
        marketingFee = _marketingFee;
        totalFees = _dividendRewardsFee.add(_marketingFee);

    	dividendTracker = new TESTLINKDividendTracker();

    	buyBackWallet = 0x4A892893270f6aB4DF54840Cf3405Ae34FDB4EAf; //CHANGEME unless you want your money to go to me ;)
    ...
    receive() external payable {
  	...
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "TESTLINK: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function afterPreSale() external onlyOwner {
        dividendRewardsFee = 10; //CHANGEME since you'll be running this function, choose whatever you picked earier
        marketingFee = 5; //CHANGEME
        maxBuyTranscationAmount = 500000000 * (10**18); //CHANGEME
        maxWalletToken = 2000000000 * (10**18); //CHANGEME
    }
    ...
    function setLiquidityIsEnabled(bool _enabled) public onlyOwner {
        liquidityIsEnabled = _enabled;
    }
    
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "TESTLINK: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "TESTLINK: The dividend tracker already has that address");

        TESTLINKDividendTracker newDividendTracker = TESTLINKDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TESTLINK: The new dividend tracker must be owned by the TESTLINK token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "TESTLINK: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TESTLINK: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TESTLINK: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TESTLINK: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TESTLINK: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "TESTLINK: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TESTLINK: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TESTLINK: Cannot update gasForProcessing to same value");
...
contract TESTLINKDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("TESTLINK_Dividend_Tracker", "TESTLINK_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "TESTLINK_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "TESTLINK_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TESTLINK contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TESTLINK_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TESTLINK_Dividend_Tracker: Cannot update claimWait to same value");
