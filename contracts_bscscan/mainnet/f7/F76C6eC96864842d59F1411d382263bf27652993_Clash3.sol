...
contract Clash3 is Ownable {
    using SafeMath for uint;
    using EnumerableSet for EnumerableSet.AddressSet;
    
    /* Constants and Mains */
    uint constant tournament_type = 1 ;
    string public constant tournament_name = 'Clash Royale #3';
    string public constant start_date = '14th Jan - 15:00 UTC';
    uint public prizepool = 0;
    uint public poolPerUser = 15000000000000000000;
    string private tourney_pass = "";
    
    uint public constant timeLocked = 30 days;
    uint public constant timePaying = 5 days;
    uint public constant amountToLock = 1500000000000000000000;
    uint public pointsPerRegistration = 6;
    
    uint public constant percentage = 450;
    uint public constant playersPerTeam = 1;
    uint public maxParticipants = 500;
    uint public numParticipants;
    
    /* UNIX date when register finish: 13rd Jan 20:00 UTC */
    uint public register_date = 1642104000; 
    ...
    function setPass(string memory _new) public onlyReferee returns(bool){
        tourney_pass = _new;
        return true;
    }

    function getPass() public view returns(string memory){
        if((getIsSponsor(msg.sender) || getIsGamer(msg.sender)) && block.timestamp > register_date){
            return tourney_pass;
        }else{
            return "";
        }
