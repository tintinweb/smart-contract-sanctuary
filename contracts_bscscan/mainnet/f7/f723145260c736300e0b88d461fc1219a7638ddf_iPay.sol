...
Sell tax will be increased for the inital period after launch. 
Please check the Telegram Group or Website to see the current fees. 
...
contract iPay is Context, IERC20, Ownable { 
    ...
    uint256 public _maxTxAmount_BUY = _tTotal; 
    uint256 public _maxTxAmount_SELL = _tTotal; 
    ...
    */
    ...
    function set_Wallet_Max_Percent(uint256 max_Transaction_Percent_BUY, uint256 max_Transaction_Percent_SELL, uint256 max_Wallet_Holding_Percent) external onlyOwner() {

        _maxTxAmount_BUY = _tTotal*max_Transaction_Percent_BUY/100;
        _maxTxAmount_SELL = _tTotal*max_Transaction_Percent_SELL/100;
    ...
    function set_Wallet_Max_Tokens(uint256 max_Transaction_Tokens_BUY, uint256 max_Transaction_Tokens_SELL, uint256 max_Wallet_Holding_Tokens) external onlyOwner() {

        _maxTxAmount_BUY = max_Transaction_Tokens_BUY *10**_decimals;
        _maxTxAmount_SELL = max_Transaction_Tokens_SELL *10**_decimals;
    ...
    function _transfer(
        ...
        */
        ...
        if (!_limitExempt[to] && _isPair[from] && from != owner())
            require(amount <= _maxTxAmount_BUY, "You are trying to buy more than the max transaction limit.");


        // Limit the maximum number of tokens that can be bought or sold in one transaction
        if (!_limitExempt[from] && _isPair[to] && from != owner())
            require(amount <= _maxTxAmount_SELL, "You can not sell that much in one transaction.");
        ...
        {  
            ...
            if(contractTokenBalance > _maxTxAmount_SELL) {contractTokenBalance = _maxTxAmount_SELL;}
            txCount = 0;
