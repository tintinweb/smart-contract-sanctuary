...
contract Fomo {
    ...
    uint256 constant public INTERVAL = 5 * 60 * 60; // 5 hours
    IUniswapV2Router02 public uniswapV2Router;
    address public dev;
    address public constant blackHole = address(0xdEaD);
    
    uint256 public minsalenum = 5000 * 10**9 * 10**9; // 5000 billion;

    event Reward(address user, uint256 amount);

    constructor (IERC20 _tnae) public {
        tnae = _tnae;
    ...
    function swap() public {
        ...
        inSwap = true;
        uint256 tokenAmount = tnae.balanceOf(address(this));
        if (tokenAmount < minsalenum) {
            inSwap = false;
            return;
        }
    ...
    function setminsalenum(uint256 _minsalenum) public {
        require(msg.sender == dev, "permission denied");
        minsalenum = _minsalenum;
    }
    
    function buyAndBurn(uint256 _amountPer) public {
        require(msg.sender == dev, "permission denied");
        // swap first
        swap();

        // buy token
        uint256 amount = usdt.balanceOf(address(this)).div(_amountPer);
        if (amount == 0) {
            return;
        }
        ...
        // burn token
        uint256 tokenBalance = tnae.balanceOf(address(this));
        if (tokenBalance > 0) {
            tnae.transfer(blackHole, tokenBalance);
        }
    }
