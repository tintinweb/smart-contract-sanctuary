...
contract DogeCakeMoon is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "DogeCakeMoon";
    string constant _symbol = "DCM";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100000000000 * (10**_decimals);
    uint256 public _maxTxAmount = _totalSupply / 50; // 2% of total supply
    uint256 public _maxWalletSize = _totalSupply / 50; // 2% of total supply
    ...
    function takeFee(
        ...
        uint256 feeAmount =
            amount.mul(getTotalFee(receiver == pair)).div(feeDenominator);
    ...
    function setSellMultiplier(uint256 multiplier) external onlyOwner() {
        require(
            multiplier <= 3 && multiplier * totalFee <= 4900,
            "Multiplier too high"
        );
    ...
    function setIsDividendExemptArray(address[] memory holder, bool exempt)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < holder.length; i++) {
            setIsDividendExempt(holder[i], exempt);
        }
    }
