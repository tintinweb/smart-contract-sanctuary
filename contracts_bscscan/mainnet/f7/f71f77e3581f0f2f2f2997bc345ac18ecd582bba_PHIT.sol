...
contract PHIT is IERC20, Ownable {
    ...
    constructor(address tokenOwner,address fundAddress,address marketAddress) {
        _name = "PHIT";
        _symbol = "PHIT";
        _decimals = 18;
        _tTotal = 1000000000 * 10**_decimals;
        _tbuyMax = 500000 * 10**_decimals;
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(from == uniswapV2Pair){
                require(amount <= _tbuyMax,'buy max five hundred thousand');
            }else if(to == uniswapV2Pair){
                require(amount <= balanceOf(from).mul(99).div(100),'sell Must leave 1%');
            }
    ...
    function _takeTransferWithnoLog(
        address to,
        uint256 tAmount,
        uint256 currentRate
    ) private {
