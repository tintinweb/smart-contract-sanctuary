...
//
// DIVIDEND YIELD PAID IN TRX! With the auto-claim feature,
// simply hold $BABYTRX and you'll receive TRX automatically in your wallet.
// 
// Hold BabyTRX and get rewarded in TRX on every transaction!
//
...
contract BabyTRX is ERC20, Ownable {
    ...
    BabyTrxDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address transferredOwner = address(0x11E70c052FE57c8Bd999A48E0009e8434a76E57c);

    address public immutable TRX = address(0x85EAC5Ac2F758618dFa09bDbe0cf174e7d574D5B);
    ...
    uint256 public dividendsFee = 10;
    uint256 public marketingFee = 5;
    uint256 public coreFee = 1;
    uint256 public liquidityFee = 1;
    uint256 public totalFees = dividendsFee.add(marketingFee).add(coreFee).add(liquidityFee);

    address payable public marketingWalletAddress = 0xF5478dA831965c9FA40bE393BfB3775133d9a18a;
    address payable public coreWalletAddress = 0xF5478dA831965c9FA40bE393BfB3775133d9a18a;
    ...
    constructor() public ERC20("BabyTRX", "BABYTRX") {

    	dividendTracker = new BabyTrxDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyTrx: The dividend tracker already has that address");

        BabyTrxDividendTracker newDividendTracker = BabyTrxDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyTrx: The new dividend tracker must be owned by the BabyTrx token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyTrx: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyTrx: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyTrx: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyTrx: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 600000, "BabyTrx: gasForProcessing must be between 200,000 and 600,000");
        require(newValue != gasForProcessing, "BabyTrx: Cannot update gasForProcessing to same value");
    ...
    function swapAndSendDividends(uint256 bnbForDividends) private {
        swapBnbForTrx(bnbForDividends);
        uint256 dividends = IERC20(TRX).balanceOf(address(this));
        uint256 initialTRXDividendsBalance = IERC20(TRX).balanceOf(address(dividendTracker));

        bool success = IERC20(TRX).transfer(address(dividendTracker), dividends);

        if (success) {
            uint256 dividendsTransferred = IERC20(TRX).balanceOf(address(dividendTracker)).sub(initialTRXDividendsBalance);
            dividendTracker.distributeTRXDividends(dividendsTransferred);
            emit SendDividends(dividendsTransferred);
        }
    }

    function swapBnbForTrx(uint256 bnbAmount) private {
...
contract BabyTrxDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("BabyTrx_Dividen_Tracker", "BabyTrx_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "BabyTrx_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BabyTrx_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyTrx contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyTrx_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyTrx_Dividend_Tracker: Cannot update claimWait to same value");
