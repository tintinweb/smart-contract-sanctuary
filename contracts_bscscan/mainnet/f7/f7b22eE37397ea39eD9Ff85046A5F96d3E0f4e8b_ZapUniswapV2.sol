...
contract ZapUniswapV2 {
    ...
    function _returnAssetsInEth(address[] memory tokens, IUniswapV2Router02 router) private {
        ...
        uint256 balance = IERC20(path[1]).balanceOf(this_);
        if (balance > 0) {
            _approveTokenIfNeeded(path[0], address(router));
            router.swapExactTokensForTokens(balance, 0, path, this_, time);
        }

        path[0] = path[1];
        path[1] = WETH;
        balance = IERC20(path[0]).balanceOf(this_);
        if (balance > 0) {
            _approveTokenIfNeeded(path[0], address(router));
            router.swapExactTokensForTokens(balance, 0, path, this_, time);
        }
        
        balance = IERC20(path[1]).balanceOf(this_);
        if (balance > 0) IERC20(path[1]).safeTransfer(caller, balance);
    }
