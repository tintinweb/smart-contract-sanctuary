...
contract ANFTBeam is Context, IERC20, Ownable {
    ...
    uint private startingSupply = 387386386; //963 million
    ...
    string private _name = "1ANFTBeam";
    string private _symbol = "NFTBeam";
    ...
    uint256 public _sellReflectFee = 1000;
    uint256 private maxReflectFee = 1600;
    ...
    uint256 private maxBurnFee = 800;
    ...
    uint256 public maxSellTransactionAmount = (_tTotal * 5) / 10000;
    ...
    function setBuyTaxes(uint256 reflectFee, uint256 liquidityFee,  uint256 burnFee) external onlyOwner() {
    ...
    function setSellTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 burnFee) external onlyOwner() {
        require(reflectFee <= maxReflectFee); // Prevents owner from abusing fees.
        require(liquidityFee <= maxLiquidityFee); // Prevents owner from abusing fees.
        require(burnFee <= maxBurnFee); // Prevents owner from abusing fees.
...
,        address to,
        ...
        if(_hasLimits(from, to) && !inSwapAndLiquify
            && to == lpPair
            && swapAndLiquifyEnabled &&
        	from != address(dexRouter) && //router -> pair is removing liquidity which shouldn't have max
