...
// File interface/IThoreumITOv2.sol

pragma solidity ^0.6.12;

interface IThoreumITOv2 {
    /**
...
contract ThoreumITOv2 is IThoreumITOv2, ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    ...
    event PoolParametersSet(uint256 offeringAmountPool, uint256 raisingAmountPool, uint8 pid, address lpToken);
     ...
     * @param _numberPools: number pools
     */
    constructor(
        uint256 _startBlock,
        uint256 _endBlock,
    ...
    function finalWithdraw() external onlyOwner {
        for (uint8 i = 0; i < numberPools; i++) {
            uint256 amount = _poolInformation[i].lpToken.balanceOf(address(this));
            if(amount > 0){
                _poolInformation[i].lpToken.safeTransfer(address(this), amount);
            }
        }
    ...
    function setPool(
        ...
        emit PoolParametersSet(_offeringAmountPool, _raisingAmountPool, _pid, address(_lpToken));
    }
