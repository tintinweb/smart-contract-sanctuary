...
contract Exchange is OrderState, TransferSafe {
	...
	IPicnicStore private _PicnicStore;
	address public weth;
		...
		if (
			...
			if (order.orderType == 1 || order.orderType == 3) {
				if (!isStore) {
					...
					require(
						order.key.sellAsset.token == address(_PicnicStore),
						"invalid sell asset"
					);
					erc1155safeMintTransferFrom(
						IPicnicStore(order.key.sellAsset.token),
						order.key.owner,
						buyer,
						order.key.sellAsset.tokenId,
						1,
				...
				if (!isStore) {
					...
					require(
						order.key.buyAsset.token == address(_PicnicStore),
						"invalid buy asset"
					);
					erc1155safeMintTransferFrom(
						IPicnicStore(order.key.buyAsset.token),
						buyer,
						order.key.owner,
						order.key.buyAsset.tokenId,
						1,
	...
	function setMintableStore(address newMintableStore) external onlyOwner {
		...
		_PicnicStore = IPicnicStore(newMintableStore);
	...
	function PicnicStore() external view returns (address) {
		return address(_PicnicStore);
	}
}