...
Its finally here! FUCK INFLUENCERS 2K22. 
...
We might turn on liquidity fee and reflections fees later - IF COMMUNITY SO DESIRES
...
contract FuckInfluencers2K22 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address public deadAddress = 0x000000000000000000000000000000000000dEaD; // dead address
    ...
    string private constant _name = "FuckInfluencers2K22";
    string private constant _symbol = "FuckInfluencers2K22";
    ...
    uint256 private gasPriceLimit = 6 * 1 gwei;
    uint256 private gasMaxLimit =  5000000; // gasLeft limit
    ...
    constructor() {
        ...
        maxTransactionAmount = _tTotal * 200 / 1000; // 2% maxTransactionAmountTxn
        maxWalletAmount = _tTotal * 200 / 1000; // 3% maxWalletToken
    ...
    function enableTrading() external onlyOwner {
        ...
        earlyBuyPenaltyEnd = block.timestamp + 272 hours;
    }
    ...
    function setLimits(uint256 gasLim, uint256 gasMax) external onlyOwner {
         gasPriceLimit = gasLim * 1 gwei;
         gasMaxLimit = gasMax * 10**5;
    }
    ...
    function _transfer(
     ...
     if(limitsInEffect){
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            // Sell
            else if (automatedMarketMakerPairs[to] || boughtEarly[from]) {
                removeAllFee();
                _taxFee = _sellTaxFee;
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 bnbForLiquidity = address(this).balance.sub(initialBNBBalance);
    ...
    function checkLimits(address to) private{
        if (gasLimitActive){
    ...
    function setSwapAndLiquidify(bool _enabled) external onlyOwner() {
    ...
    function transferBNB() external onlyOwner{
        ...
        payable(liquidityAddress).transfer(_contractBalance);
    }
