...
contract SwapMining is SafeOwnable {
    ...
    // BT tokens created per block
    uint256 public rewardPerBlock;
    // The block number when BT mining starts.
    ...
    // factory address
    IBTFactory public factory;
    // token address
    BTToken public vdrToken;
    ...
    struct PoolInfo {
        ...
        uint256 allocBTAmount; // How many BTs
    ...
    function setBTPerBlock(uint256 _newPerBlock) public onlyOwner {
        massMintPools();
        rewardPerBlock = _newPerBlock;
    }

    // Only tokens in the whitelist can be mined BT
    ...
    function getBTReward(uint256 _lastRewardBlock) public view returns (uint256) {
    ...
    function mint(uint256 _pid) public returns (bool) {
        ...
        uint256 blockReward = getBTReward(pool.lastRewardBlock);
        if (blockReward <= 0) {
            return false;
        }
        // Calculate the rewards obtained by the pool based on the allocPoint
        uint256 btReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
        // Increase the number of tokens in the current pool
        pool.allocBTAmount = pool.allocBTAmount.add(btReward);
    ...
    function swap(address account, address input, address output, uint256 amount) public onlyRouter returns (bool) {
        ...
        address pair = BTLibrary.pairFor(address(factory), input, output);
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        } else if (IBTFactory(factory).getPair(outputToken, anchorToken) != address(0)) {
    ...
    function takerWithdraw() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (user.quantity > 0) {
                ...
                uint256 userReward = pool.allocBTAmount.mul(user.quantity).div(pool.quantity);
                pool.quantity = pool.quantity.sub(user.quantity);
                pool.allocBTAmount = pool.allocBTAmount.sub(userReward);
                user.quantity = 0;
                user.blockNumber = block.number;
    ...
    function getUserReward(uint256 _pid, address _user) public view returns (uint256, uint256){
        ...
        if (user.quantity > 0) {
            uint256 blockReward = getBTReward(pool.lastRewardBlock);
            uint256 btReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
            userSub = userSub.add((pool.allocBTAmount.add(btReward)).mul(user.quantity).div(pool.quantity));
        }
        //BT available to users, User transaction amount
    ...
    function getPoolInfo(uint256 _pid) public view returns (address, address, uint256, uint256, uint256, uint256){
        require(_pid <= poolInfo.length - 1, "SwapMining: Not find this pool");
        PoolInfo memory pool = poolInfo[_pid];
        address token0 = IBTPair(pool.pair).token0();
        address token1 = IBTPair(pool.pair).token1();
        uint256 btAmount = pool.allocBTAmount;
        uint256 blockReward = getBTReward(pool.lastRewardBlock);
        uint256 btReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
        btAmount = btAmount.add(btReward);
        //token0,token1,Pool remaining reward,Total /Current transaction volume of the pool
        return (token0, token1, btAmount, pool.totalQuantity, pool.quantity, pool.allocPoint);
    }

    function ownerWithdraw(address _to, uint256 _amount) public onlyOwner {
        safeBTTransfer(_to, _amount);
    }

    function safeBTTransfer(address _to, uint256 _amount) internal {
