...
contract ButterVotingNFT is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function setCuttoffs(uint256 t1, uint256 t2) public onlyOwner()
    {
        tier1cuttoff = t1;
        tier2cuttoff = t2;
    }
...
contract ButterVoting is Context, Ownable, IButterVoting {
    ...
    address mbAddress = 0x0962840397B0ebbFbb152930AcB0583e94F49B5c;
    ...
    function setURIs(string memory tier1uri, string memory tier2uri, string memory tier3uri) public onlyOwner()
    {
        votingNFT.setURIs(tier1uri, tier2uri, tier3uri);
    }

    function setCuttoffs(uint256 t1, uint256 t2) public onlyOwner()
    {
        votingNFT.setCuttoffs(t1, t2);
    }
    ...
    {
        require(mb.balanceOf(msg.sender) > 0, "You have no Milk and Butter to vote with!");
        require(voteCast[msg.sender][pollNumber] == false, "You have already voted in this poll");
        charities[charityIndex].votes = charities[charityIndex].votes.add(mb.balanceOf(msg.sender));
        voteCast[msg.sender][pollNumber] = true;
        vote[msg.sender][pollNumber] = charityIndex;
        voteWeight[msg.sender][pollNumber] = mb.balanceOf(msg.sender);
    ...
    function boostVote() public
    {
        ...
        require(mb.balanceOf(msg.sender) > voteWeight[msg.sender][pollNumber], "You have to have more Milk and Butter than your initial vote to boost your vote");
        
        
        //Add the boost amount to their charity
        uint256 boostAmount = mb.balanceOf(msg.sender) - voteWeight[msg.sender][pollNumber];
    ...
    function getSeriesNumber() public view returns (uint256)
    {
        return series;
    }
    ...
    function canBoost() public view returns (bool)
    {
        if(hasVoted())
        {
            if(mb.balanceOf(msg.sender) > voteWeight[msg.sender][pollNumber]) return true;
            return false;
        }
        return false;
    }
