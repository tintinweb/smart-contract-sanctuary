...
import "./MorrallaToken.sol";
import "./TandaToken.sol";

// MasterChef = PINCHE TIANGUIS
// @note Fork de pancakeswap con el migrator quitado
// @note la nueva morralla tiene minter ey en vez de hacer el farm el duenno
// del token ponemos un timelock al token. el duenno del token solo puede
// cambiar el minter asi que en el futuro podemos cambiar el farm sin
// migrar token.
// @note tambien pusimos un function para convertir el token aterior al nuevo 
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once MORRALLA is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract PincheTianguis is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of MORRALLAs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. MORRALLAs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that MORRALLAs distribution occurs.
        uint256 accMorrallaPerShare; // Accumulated MORRALLAs per share, times 1e12. See below.
    }

    // MorrallaToken anterior pa la migra
    MorrallaToken public xMorrallaX;
    // The MORRALLA TOKEN!
    MorrallaToken public morralla;
    // The TANDA TOKEN!
    TandaToken public tanda;
    // Dev address.
    address public devaddr;
    // MORRALLA tokens created per block.
    uint256 public morrallaPerBlock;
    // Bonus muliplier for early morralla makers.
    ...
    constructor(
        MorrallaToken _morralla,
        MorrallaToken _xMorrallaX,
        TandaToken _tanda,
        address _devaddr,
    ...
    // View function to see pending MORRALLAs on frontend.
    ...
    function updatePool(uint256 _pid) public {
        ...
        morralla.mint(address(tanda), morrallaReward);
        pool.accMorrallaPerShare = pool.accMorrallaPerShare.add(morrallaReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for MORRALLA allocation.
    ...
    // Stake MORRALLA tokens to MasterChef
    ...
    // Safe morralla transfer function, just in case if rounding error causes pool to not have enough MORRALLAs.
    function safeMorrallaTransfer(address _to, uint256 _amount) internal {
        tanda.safeMorrallaTransfer(_to, _amount);
    }
    ...
    function convertirXMorrallaX(uint256 _amount) public {
        require(xMorrallaX.balanceOf(address(msg.sender)) >= _amount, "No alcanza!!");
        xMorrallaX.burnFrom(payable(msg.sender), _amount);
        morralla.mint(payable(msg.sender), _amount);
    }
...
contract TandaToken is Ownable, BEP20('La Tanda', 'TANDA') {
