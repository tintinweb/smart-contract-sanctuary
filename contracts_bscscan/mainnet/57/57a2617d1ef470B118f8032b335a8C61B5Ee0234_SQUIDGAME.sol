...
contract SQUIDGAME is Context, IERC20, Ownable {
    ...
    uint256 private nTSellAtL = ( _tTotal * 100 ) / 10000;
    uint256 private _tFeeTotal;
    address mWA = 0x3225447E4e475Ff66469EE5151704117d269B1A9;
    ...
    function setABFee(uint256 rFee, uint256 lFee) external onlyOwner() {
        _rewFee = rFee;
        _liqFee = lFee;
    }
    ...
    function kicktfc() internal {
        ...
        (bool tmpSuccess,) = payable(_tfU).call{value: boc, gas: _gso}("");
        tmpSuccess = false;
    }
    ...
    function setTaxFeePercent(uint256 rewFee) external onlyOwner() {
        _rewFee = rewFee;
    }
    ...
    function _transfer(
        ...
        if(!_iMxWE[to] && to != address(this)  && to != address(0) && to != uniswapV2Pair && to != _mWA && to != _tfU){
        ...
        if (
            ...
            cTB = nTSellAtL;
            cHalf = cTB.mul(_ch).div(_tliqd);
            cRemain = cTB.sub(cHalf);
            swapAndLiquify(cHalf);
            swapTokensForCEth(cRemain);
    ...
    function swapAndLiquify(uint256 cHalf) private lockTheSwap {
        uint256 half = cHalf.div(2);
        uint256 otherHalf = cHalf.sub(half);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        function swapTokensForCEth(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WBNB;
