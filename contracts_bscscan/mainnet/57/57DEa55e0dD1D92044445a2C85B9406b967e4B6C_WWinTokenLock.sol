...
contract WWinTokenLock is Ownable, ReentrancyGuard{
    ...
    function setLock(address user, uint256 amount, uint256 firstReleaseTime, uint256 firstReleaseRate, uint256 periodReleaseRate) public onlyOwner{
        ...
        require(firstReleaseRate <= _rate_precision.mul(100) &&
                periodReleaseRate <= _rate_precision.mul(100), "Rate <= 100 * _rate_precision.");
        require(
            (firstReleaseRate < _rate_precision.mul(100) && periodReleaseRate > 0) || 
            (firstReleaseRate == _rate_precision.mul(100) && periodReleaseRate == 0), 
            "Invalid release ratio"
        );

        if(_locks[user].initialAmount > 0){
            totalValueLock = totalValueLock.add(_locks[user].claimAmount).sub(_locks[user].initialAmount);
        }
    ...
    function _getPeriodSinceFirstRelease(address _user) internal view returns(uint256){
    ...
    function _getClaimableAmount(address _user) internal view returns(uint256){
        ...
        uint256 period = _getPeriodSinceFirstRelease(_user);
        uint256 amount = 0;
