...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    ...
    string private _name = "Multi-Meta World";
    ...
    address public BurnWallet =0xBe62d49B18BdA07963D77dCE19E6Fb52eb9F3f06 ;
    uint256 private _previousBurnFee = _burnFee;

    uint256 public _marketingFee = 2;
    address public MarketingWallet = 0x6E49C632B7498C7c4391F13887Aabad94b5133BC; //5% for marketing 
    uint256 private _previousMarketingFee = _marketingFee;

    uint256 public _devFee = 2;
     address public Devwallet=  0x8aBF4b96A8eBA70353921CaefEE710Cfcd1e0Cd2;
    ...
    uint256 public _maxTxAmount = 50000000000 * 10**9; //1% of total supply   //ANTIDUMP
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(burnAmt).sub(MarketingAmt).sub(DevAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(burnAmt).sub(MarketingAmt).sub(DevAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(MarketingAmt).sub(DevAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(burnAmt).sub(MarketingAmt).sub(DevAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(MarketingAmt).sub(DevAmt)));
        }
