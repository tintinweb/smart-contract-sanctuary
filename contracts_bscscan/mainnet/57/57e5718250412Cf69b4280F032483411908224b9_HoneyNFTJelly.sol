...
contract HoneyNFTJelly is Ownable, ReentrancyGuard, VRFConsumerBase {
    ...
    function initialize(
        ...
        require(_winnerCount > 0, "Winner must be larger than 0");
    ...
    function winners() external view returns (address[] memory) {
        address[] memory candidateList = userList;
        ...
        for (uint i = 0; i < userList.length; i++) {
            totalShareAmount = totalShareAmount.add( pendingReward(userList[i]) );
        }
        ...
        for (uint j = 0; j < winnerCount; j++) {
            ...
            for (uint i = 0; i < candidateList.length; i++) {
                uint256 share = pendingReward(candidateList[i]);
                if (rewardSharePoint < accShareAmount.add(share)) {
                    winnerList[j] = candidateList[i];
                    candidateList[i] = candidateList[candidateList.length-1];
                    totalShareAmount = totalShareAmount.sub(share);
                    break;
                }
    ...
    // function winners2() external view returns (address[] memory) {
    //     address[] memory candidateList = userList;
    //     address[] memory winnerList = new address[](winnerCount);
