...
contract BisharesZapUniswapRouterMinter is BMath {
  ...
  function swapETHForVaultsAndMint(
    ...
    uint256 amountInMax = initialAmount;
    ...
    for (uint256 i = 0; i < vaults.length; i++) {
      uint256 amountIn = initialAmount.mul(
      ...
      );
      {
        ...
        IStrategy strategy = IStrategy(IVault(vaults[i]).strategy());
        tokens[0] = strategy.lpToken0();
        tokens[1] = strategy.lpToken1();
        _returnAssets(tokens);
      }
    }
    poolAmountOut = poolAmountOut.mul(99).div(100);
