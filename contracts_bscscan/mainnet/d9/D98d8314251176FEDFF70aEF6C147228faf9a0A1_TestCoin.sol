...
contract TestCoin is Context, IERC20, Ownable {
    ...
    uint256 private constant _totalSupply = 1e9 * 1e9;
    uint256 public _maxTxAmount = 1e9 * 1e9; // max transaction amount
    ...
    function activate() public onlyOwner {
        require(!tradingEnabled, "TestCoin: Trading is already enabled");
        tradingEnabled = true;
    }

    function setMaxTxPercent(uint256 numerator, uint256 denominator) public virtual onlyOwner() {
        require(numerator < denominator);
        _maxTxAmount = _totalSupply.mul(numerator).div(denominator);
    }
    ...
    function _approve(address _owner, address _spender, uint256 amount) private {
        ...
        if (_msgSender() != owner()) {
            require(tradingEnabled, "TestCoin: Trading is not enabled");
        }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if ((sender != owner() && recipient != owner()) && (!_isExcluded[sender] && !_isExcluded[recipient])) {
            require(tradingEnabled, "TestCoin: Trading is not enabled");
