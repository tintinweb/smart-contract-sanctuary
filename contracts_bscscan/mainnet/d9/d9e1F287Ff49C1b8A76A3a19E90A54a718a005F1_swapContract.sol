...
contract swapContract is AccessControl, Pausable, ECDSAOffsetRecovery{
    ...
    event TransferCryptoToOtherBlockchainUser(uint128 blockchain, address sender, uint256 RBCAmountIn, uint256 amountSpent,
    ...
    event TransferTokensToOtherBlockchainUser(uint128 blockchain, address sender, uint256 RBCAmountIn, uint256 amountSpent,
    ...
    /**
      ...
      * @param amountWithFee Amount of tokens with included fees // "RBCAmountIn" from event
    ...
    modifier TransferTo(
        ...
        if (params.swapToCrypto){
            emit TransferCryptoToOtherBlockchainUser(params.blockchain, _msgSender(), params.exactRBCtokenOut, params.tokenInAmount, params.newAddress, params.tokenOutMin, params.secondPath);
        }
        else{
            emit TransferTokensToOtherBlockchainUser(params.blockchain, _msgSender(), params.exactRBCtokenOut, params.tokenInAmount, params.newAddress, params.tokenOutMin, params.secondPath);
        }
    }
    ...
    )
    {
        ...
        require(IERC20(RubicAddresses[numOfThisBlockchain]).approve(address(blockchainRouter), type(uint256).max),
    ...
    )
    {
        ...
        if (params.firstPath.length > 1){
            ...
            require(tokenIn.approve(address(blockchainRouter), params.tokenInAmount), 'swapContract: tokeinIn approve failed.');
    ...
    {
        require(params.path.length > 1, "swapContract: path length must be greater than 1");

        IERC20 RBCToken = IERC20(params.path[0]);

        uint256[] memory amountIn = blockchainRouter.getAmountsIn(params.amountOutMin, params.path);

        require(RBCToken.transferFrom(blockchainPool, address(this), amountIn[0]),
                                                        "swapContract: transfer from pool failed");

        blockchainRouter.swapTokensForExactTokens(
            params.amountOutMin,
            amountIn[0],
            params.path,
            params.user,
        ...
        emit TransferFromOtherBlockchain(params.user, amountIn[0], amountIn[0], params.originalTxHash);
    }
