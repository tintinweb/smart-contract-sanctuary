...
contract NalaDistribution is Multimanager {
    ...
    constructor (IERC20 _token, uint8 _decimalsToken){
        ...
        //_startFirstClaim = block.timestamp + (5*60); // DEV 15-12-2021 13.00 GMT 
    ...
    function isClaimAllowed(address account) public view returns(bool){
        ...
        if(NalaDistributionLib.isLastClaimOverYear(lastRedeem, dateTime, _startFirstClaim)) return false;
    ...
    function Claim() public payable returns(uint256, uint256){
        require((msg.value >= 500000000000000 ), 'The payment must be greater or equals than 0,0005 bnb');
        uint256 dateTime = block.timestamp;
        ...
        require (!NalaDistributionLib.isLastClaimOverYear(lastRedeem, dateTime, _startFirstClaim), "The redeem cannot be done because you have exceeded one year from the previous claim");
        
        
        // da questo momento in poi si puo' passare al trasferimento
...
library NalaDistributionLib {
    ...
    function isLastClaimOverYear (uint256 lastClaim, uint256 dateTimeNow, uint256 startFirstClaim) internal pure returns(bool){ // true deve cancellare i balance utente
        if (lastClaim == 0){ // se il lastClaim è 0, non è mai stato fatto, devo solo controllare che non sia passato un anno dalla prima data utile di redeem
            ...
            //if(dateTimeNow >= (startFirstClaim + (30*60))) return true; // DEV remove ogni giorno
            return false;
        }
            ...
            //if(dateTimeNow >= (lastClaim + (30*60))) return true; // DEV remove ogni giorno
            return false;
        }
    }
