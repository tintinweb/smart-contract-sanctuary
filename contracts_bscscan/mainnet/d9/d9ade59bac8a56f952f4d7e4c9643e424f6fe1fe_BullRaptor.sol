...
Telegram : https://t.me/bullraptor
...
//SPDX-License-Identifier: MIT
	 ...
	 * This allows applications to reconstruct the allowance for all acBullRaptor just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also BullRaptored.
	...
	// File: contracts/BullRaptorDividendPayingToken.sol
	...
	// File: contracts/BullRaptor.sol

	// 

	pragma solidity ^0.7.0;

	contract BullRaptor is ERC20, Ownable {
		using SafeMath for uint256;

		IuniswapV2Router02 private uniswapV2Router;
		address private immutable uniswapV2Pair;

		address payable public _MarketingAddress = payable(0x90EDe2c80f6Ee2A79E71324ad7f23c4A7d9EeDB6); 
	
		bool private swapping;

		BullRaptorDividendTracker private dividendTracker;

		address public liquidityWallet;

		uint256 private _swapTokensAtAmount = 200000 * (10**18);
		uint256 public _maxTxAmount = 20000 * (10**18);
		uint256 public _maxWalletToken = 20000 * (10**18);
		...
		event ExcludeMultipleAcBullRaptorFromFees(address[] acBullRaptor, bool isExcluded);
		...
		constructor() ERC20("BullRaptor", "BULLRAPTOR"){
			...
			dividendTracker = new BullRaptorDividendTracker();
		...
		function excludeFromFees(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "BullRaptor: Account is already the value of 'excluded'");
		...
		function excludeMultipleAcBullRaptorFromFees(address[] calldata acBullRaptor, bool excluded) public onlyOwner {
			for(uint256 i = 0; i < acBullRaptor.length; i++) {
				_isExcludedFromFees[acBullRaptor[i]] = excluded;
			}

			emit ExcludeMultipleAcBullRaptorFromFees(acBullRaptor, excluded);
		}

		function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
			require(pair != uniswapV2Pair, "BullRaptor: The uniswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "BullRaptor: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "BullRaptor: The liquidity wallet is already this address");
		...
		function updateGasForProcessing(uint256 newValue) private onlyOwner {
			require(newValue >= 200000 && newValue <= 500000, "BullRaptor: gasForProcessing must be between 200,000 and 500,000");
			require(newValue != gasForProcessing, "BullRaptor: Cannot update gasForProcessing to same value");
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if(!hasLaunched) {
				require(canTransferBeforehasLaunched[from], "BullRaptor: This account cannot send tokens until trading is enabled");
			}
	...
	contract BullRaptorDividendTracker is DividendPayingToken, Ownable {
		using SafeMath for uint256;
		using SafeMathInt for int256;
		...
		constructor() public DividendPayingToken("BullRaptor_Dividend_Tracker", "BullRaptor_Dividend_Tracker") {
			claimWait = 3600;
			minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
		}

		function _transfer(address, address, uint256) internal override {
			require(true, "BullRaptor_Dividend_Tracker: No transfers allowed");
		}
		...
		function updateClaimWait(uint256 newClaimWait) external onlyOwner {
			require(newClaimWait >= 1 && newClaimWait <= 86400, "BullRaptor_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
			require(newClaimWait != claimWait, "BullRaptor_Dividend_Tracker: Cannot update claimWait to same value");
