...
contract TeamDividend is AdminRole, Emergency {
    using SafeERC20 for IERC20;

    IERC20[] public tokens;

    // b,s,m,a,k
    address[5] public contributors = [
        0x3e119d34ed3f22cd7eC4292FeeB83fa4591b8888,
        0xE05C387b68D5DbF8Dc98dFa8ff7fa4Efd66135F9,
        0x22f214eCa4b6a4DF554e00b16D25614997F9Cc2A,
        0x7121Ba4396A16C16650fBc7Da3F84A7b62B6CcBb,
        0x61687668885D5FE3424A914c49d223378520875A
    ];

    uint8[5] public rates = [30, 30, 20, 10, 10];
    ...
    function distributeBNB() public {
        ...
        for (uint8 i = 0; i < contributors.length; i++) {
            uint256 _amount = (_totalAmount * rates[i]) / 100;
            if (_amount > 0) {
                transfer(contributors[i], _amount);
            }
        }
    }

    function distributeToken(uint8 _tokenIndex) public {
        IERC20 _token = tokens[_tokenIndex];
        uint256 _totalAmount = _token.balanceOf(address(this));
        if (_totalAmount == 0) {
            return;
        }
        ...
        for (uint8 i = 0; i < contributors.length; i++) {
            uint256 _amount = (_totalAmount * rates[i]) / 100;
            if (_amount > 0) {
                _token.safeTransfer(contributors[i], _amount);
            }
        }
    }

    function setTokens(IERC20[] memory _tokens) external onlyOwner {
        tokens = _tokens;
    }

    function setContributors(address[5] memory _contributors) external onlyOwner {
        contributors = _contributors;
    }

    function setRates(uint8[5] memory _rates) external onlyOwner {
        rates = _rates;
    }
}
...
contract Emergency is AdminRole {
    event EmergencyWithdraw(IERC20 _token, uint256 _amount);
    event EmergencyWithdrawBNB(uint256 _amount);

    function emergencyWithdraw(IERC20 token) external onlyOwner {
        uint256 _amount = token.balanceOf(address(this));
        token.transfer(_msgSender(), _amount);
        emit EmergencyWithdraw(token, _amount);
    }

    function emergencyWithdrawBNB() external onlyOwner {
        uint256 _amount = address(this).balance;
        transfer(_msgSender(), _amount);
        emit EmergencyWithdrawBNB(_amount);
    }
