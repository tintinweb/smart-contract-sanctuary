...
// File contracts/Interfaces/ManualSplitterInterface.sol
...
interface ManualSplitterInterface is IAccessControl {

    /////////////////////
    //* PUBLIC REGION *//
    /////////////////////
    function splitFunds(uint participantId_, uint amount_) external returns (bool);

    ///////////////////////////
    //* MANAGER_ROLE REGION *//
    ///////////////////////////
    function addParticipant(address destinationAddress_) external returns (bool);
    ...
    event ParticipantAdded(uint id, address destinationAddress);
    ...
    event ParticipantRemoved(uint id, address destinationAddress);
...
// File contracts/ManualSplitterContract.sol

pragma solidity 0.8.10;


contract ManualSplitterContract is AccessControl, ManualSplitterInterface {
    ...
    constructor() {
        ...
        _varableNameArray = new string[](1);
        _varableNameArray[0] = "destinationAddress";

        _variableHashArray = new bytes8[](1);
    ...
    //* PUBLIC REGION *//
    /////////////////////
    function splitFunds(uint participantId_, uint amount_) public onlyRole(MANAGER_ROLE) override returns (bool) {
        uint currentBalance = getTokenBalance();
        require(currentBalance > 0, "Balance is 0!");
        require(currentBalance >= amount_, "Amount is to big!");
        ParticipantData memory currentParticipant = getParticipantData(participantId_);
        _tokenInstance.transfer(currentParticipant.destinationAddress, amount_);
        emit TokensSent(currentParticipant.destinationAddress, amount_);
        return true;
    }
    ...
    //* MANAGER_ROLE REGION *//
    ///////////////////////////
    function addParticipant(address destinationAddress_) public onlyRole(MANAGER_ROLE) override returns (bool) {
        ParticipantData memory newParticipant = ParticipantData(0, destinationAddress_);
    ...
    function _appendParticipant(ParticipantData memory participant_) internal {
        ...
        emit ParticipantAdded(participant_.id, participant_.destinationAddress);
    }
    function _removeParticipant(uint participantId_) internal {
        ...
        emit ParticipantRemoved(currentParticipant.id, currentParticipant.destinationAddress);
    ...
    function getParticipantsData(uint startIndex_, uint endIndex_) public view override returns (ParticipantData[] memory) {
        ...
        for (uint i=0; i<endIndex_ - startIndex_; i++) {
            participantsDataArray[i] = getParticipantData(i + startIndex_);
        }
