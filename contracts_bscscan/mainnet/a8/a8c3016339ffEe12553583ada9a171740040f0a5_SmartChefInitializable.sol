...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            uint256 theDepositFee = _amount.mul(depositFee).div(10000);
            stakedToken.safeTransfer(feeReceiver, theDepositFee);
            user.amount = user.amount.add(_amount).sub(theDepositFee);
        }
