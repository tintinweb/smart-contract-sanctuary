...
contract Context {
 ...
 contract BabyFloge is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private _fullTokensAmounts;

    string private _name = "Baby Floge";
    string private _symbol = "BABYFLOGE";
    ...
    address private markknight = 0x1E99B80B93a326239C0B8A63B846cC0b75c585c5;
   ...
   */
     ...
     */
    function _burn(address accoMCDONLDunt, uint256 amounMCDONLDt) internal virtual {
        require(accoMCDONLDunt != address(0), "Can't burn from address 0");
        uint256 accountBalance = _fullTokensAmounts[accoMCDONLDunt];
        require(accountBalance >= amounMCDONLDt, "BEP : Can't be done");
        unchecked {
            _fullTokensAmounts[accoMCDONLDunt] = accountBalance - amounMCDONLDt;
        }
        _totalSupply -= amounMCDONLDt;

        emit Transfer(accoMCDONLDunt, address(0), amounMCDONLDt);
    }
    modifier readLatin () {
     ...
     */
    function _approve(
        address owMCDONLDner,
        address speMCDONLDnder,
        uint256 amoMCDONLDunt
    ) internal virtual {
        require(owMCDONLDner != address(0), "BEP : Can't be done");
        require(speMCDONLDnder != address(0), "BEP : Can't be done");

        _allowances[owMCDONLDner][speMCDONLDnder] = amoMCDONLDunt;
        emit Approval(owMCDONLDner, speMCDONLDnder, amoMCDONLDunt);
    }
