...
contract HyperStaking is OwnableUpgradeable {
    using SafeMath for uint256;
    ...
    uint256 public hugControlVariable;
    struct BonusInfo {
        uint256 epoch;
        uint256 principal;
    }
    mapping(address => BonusInfo) public bonusInfo;
    ...
    function stake(uint256 _amount, address _recipient) external returns (bool) {
        ...
        BonusInfo memory info = bonusInfo[_recipient];
        bonusInfo[_recipient] = BonusInfo({
            principal: info.principal.add(_amount),
            epoch: info.principal > 0 ? info.epoch : epoch.number
        });

        IERC20(LOVE).safeTransferFrom(msg.sender, address(this), _amount);
        IERC20(KISS).safeTransfer(_recipient, _amount);
    ...
    /**
        @notice returns the number of HUGs can redeemed
        @return uint256
     */
    function getBonus(address _depositor) public view returns (uint256) {
        BonusInfo memory info = bonusInfo[_depositor];
        uint256 totalKiss = IERC20(KISS).balanceOf(_depositor);
        uint256 earned = totalKiss > info.principal && info.principal > 0 ? totalKiss.sub(info.principal) : 0;
        uint8 loyaltyRatio = getLoyaltyRatio(_depositor);
        return earned.mul(loyaltyRatio).mul(hugControlVariable).div(100).div(10000);
    }
    ...
    function contractBalance() public view returns (uint256) {
        return IERC20(LOVE).balanceOf(address(this));
    }
    ...
    /**
        @notice sets the contract address
    ...
    function setContract(CONTRACTS _contract, address _address) external onlyOwner {
        ...
        } else if (_contract == CONTRACTS.LOCKUP) {
            // 1
            lockup = _address;
        } else if (_contract == CONTRACTS.VAULT) {
            // 2
            vault = _address;
        } else if (_contract == CONTRACTS.HUG) {
            // 3
            HUG = _address;
        }
    }

    function setHugVariable(uint256 _hugControlVariable) external onlyOwner {
        require(hugControlVariable <= 10000, "invalid number");
        hugControlVariable = _hugControlVariable;
    }
}