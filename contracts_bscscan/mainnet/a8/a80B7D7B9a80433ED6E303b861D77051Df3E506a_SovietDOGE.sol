...
/*
 ...
 | (___     ___   __   __  _    ___  | |_  | |  | | | |  | | | |  __  | |__   
  \___ \   / _ \  \ \ / / | |  / _ \ | __| | |  | | | |  | | | | |_ | |  __|  
  ____) | | (_) |  \ V /  | | |  __/ | |_  | |__| | | |__| | | |__| | | |____ 
 |_____/   \___/    \_/   |_|  \___|  \__| |_____/   \____/   \_____| |______|

*/

// Telegram Group => https://t.me/soviet_doge
// Website        => https://sovietdoge.com
...
contract SovietDOGEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("SovietDOGE_Dividend_Tracker", "SovietDOGE_Dividend_Tracker", 9) {
    	claimWait = 14400;
        minimumTokenBalanceForDividends = 1000000 * (10**9); //must hold 10000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "SovietDOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "SovietDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SovietDOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SovietDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SovietDOGE_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SovietDOGE is BEP20, Ownable {
    using SafeMath for uint256;

    IPancakeswapV2Router02 public pancakeswapV2Router;
    address public immutable pancakeswapV2Pair;
    address public marketingWallet = 0x70968A58Cc7FA0FF0f33CE1fd30Be1B94285fbb4;

    bool private swapping;

    SovietDOGEDividendTracker public dividendTracker;
    ...
    constructor() public BEP20("SovietDOGE", "SDOGE", 9) {
    	...
    	dividendTracker = new SovietDOGEDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SovietDOGE: The dividend tracker already has that address");

        SovietDOGEDividendTracker newDividendTracker = SovietDOGEDividendTracker(payable(newAddress));
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "SovietDOGE: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SovietDOGE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "SovietDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SovietDOGE: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SovietDOGE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SovietDOGE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SovietDOGE: Cannot update gasForProcessing to same value");
        emit GasForProcessingUpdated(newValue, gasForProcessing);
        gasForProcessing = newValue;
    }
    
    function initializeSovietDOGE(uint256 initialTokenAmount) public onlyOwner {
        /*
            _mint is an internal function in BEP20.sol that is only called here,
            and CANNOT be called ever again as ownership will be renounced
        */
