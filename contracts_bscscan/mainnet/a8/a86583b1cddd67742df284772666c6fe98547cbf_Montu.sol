...
contract Ownable is Context {
    ...
    constructor() internal {
        _owner =0xcB8ABc1A7dfAbCC9bb9D262182e83dFd87b2d747;
...
contract Montu is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Montu";
    string private _symbol = "MON2";
    ...
    address feeWallet=0x6a363a53D5CF85965Dc735c01ad80fD7d5DFa88f;//
     
    uint256 public _feeDecimal = 2; // do not change this value...
    uint256 public _taxFee = 100; // means 2%(distributed to all holders)
    uint256 public _buyFee=200; //200 means 2% this is how it sets(marketing fee)
    uint256 public _sellFee=30000; // means 30% 
    uint256 public _burnFee=300; // menase 5%
    ...
    function collectBuyFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(_buyFee != 0){
            ...
            _reflectionBalance[address(this)] = _reflectionBalance[address(this)].add(buyFee.mul(rate));
            if(_isExcluded[address(this)]){
                _tokenBalance[address(this)] = _tokenBalance[address(this)].add(buyFee);
            }
            emit Transfer(account,feeWallet,buyFee);
        }
    ...
    function collectSellFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(_sellFee != 0){
            ...
            _reflectionBalance[address(this)] = _reflectionBalance[address(this)].add(sellFee.mul(rate));
            if(_isExcluded[address(this)]){
                _tokenBalance[address(this)] = _tokenBalance[address(this)].add(sellFee);
            }
            emit Transfer(account,feeWallet,sellFee);
        }
