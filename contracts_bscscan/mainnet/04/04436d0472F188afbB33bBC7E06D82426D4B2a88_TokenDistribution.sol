    ...
    PausableUpgradeable
{
    ...
    function getAMOUNT_TOKEN_SALE_PER_ROUND(uint round) public view returns (uint256){
    ...
    function getTokenAmountLeftInRound(uint256 round)
        ...
        returns (uint256)
    {
        return getAMOUNT_TOKEN_SALE_PER_ROUND(round).sub(rounds[round].totalWithdraw);
    }
    ...
    function getLastRoundPrice(uint256 price) public view returns (uint256) {
    ...
    function initialPriceInRound(uint256 round) public view returns (uint256) {
        ...
        if (round >=4 ){
            return
            lastRoundPrice.sub(firstRoundPrice).div(MAX_ROUND.sub(MIN_ROUND)) +
            initialPriceInRound(round - 1);
        } else {
            uint _oldRoundPrice = (575 * 10**12);
            return _oldRoundPrice.sub(firstRoundPrice).div(MAX_ROUND.sub(MIN_ROUND)) + initialPriceInRound(round - 1);
        }
    ...
    function maxVolumeInRound(uint256 round) public view returns (uint256) {
        return
            getAMOUNT_TOKEN_SALE_PER_ROUND(round).mul(initialPriceInRound(round)).div(
                10**18
            );
    }
    ...
    function calculateExactTokenAmount(uint256 round, uint256 amountBnb)
        ...
        returns (uint256)
    {
        return amountBnb.div(initialPriceInRound(round)).mul(10**18); //todo: check why .mul(10**18)
    }
