...
library Base64 {
    ...
    function encode64(string memory _str) internal pure returns (string memory) {
    ...
    IERC721Enumerable
{
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        bytes memory image = abi.encodePacked('<svg viewBox="0 0 200 200" style="background-color:#000;">\n  <text x="100" y="80" text-anchor="middle" fill="#ffda00">Token #', uint2str(tokenId));
        image = abi.encodePacked(image, '</text>\n  <text x="100" y="120" text-anchor="middle" fill="#ffda00">Block #');
        image = abi.encodePacked(image, "Visit HunterDoge.com for more Informations");
        image = abi.encodePacked(image, "</text>\n</svg>");
        image = abi.encodePacked("data:image/svg+xml;base64,", string(image).encode64());

        // concat data and return the json as a string
        bytes memory json = abi.encodePacked(
            '{"description":"This was minted for hunterdoge.com.","name": "HunterDoge.com","external_link": "https://t.me/hunterdogeofficial","image": "'
        );
