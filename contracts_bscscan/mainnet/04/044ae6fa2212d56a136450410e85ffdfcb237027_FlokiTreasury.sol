...
contract FlokiTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SFOHM }
    ...
    address public sFOHM;
    uint public sFOHMQueue; // Delays change to sFOHM address
    ...
    /**
        @notice allow approved address to deposit an asset for FOHM
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint FOHM needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( FOHM ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn FOHM for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IFOHMERC20( FOHM ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sFOHM ).balanceOf( msg.sender ); // Can only borrow against sFOHM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with FOHM
        @param _amount uint
     */
    function repayDebtWithFOHM( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IFOHMERC20( FOHM ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, FOHM, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( FOHM ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( FOHM ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns FOHM valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match FOHM decimals
            value_ = _amount.mul( 10 ** IERC20( FOHM ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SFOHM ) { // 9
            sFOHMQueue = block.number.add( blocksNeededForQueue );
        } else return false;
