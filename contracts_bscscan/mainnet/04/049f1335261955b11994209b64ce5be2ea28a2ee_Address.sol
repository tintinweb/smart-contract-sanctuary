...
contract SnoopCoin is Context, IBEP20, Ownable {
    ...
    string  private constant _NAME = "Snoop Coin";
    ...
    uint256 private _tTotal = 20674854 * _DECIMALFACTOR;
    ...
    constructor (uint256 _txFee,uint256 _burnFee,address tokenOwner) {

		_TAX_FEE = _txFee.mul(100); 
        _BURN_FEE = _burnFee.mul(100);
	...
	function _burn(address _who, uint256 rAmount) internal {

        require(rAmount <= _rOwned[_who]);
        uint256 tAmount = tokenFromReflection(rAmount);
        _rOwned[_who] = _rOwned[_who].sub(rAmount);
        
        if(_isExcluded[_who]){
            _tOwned[_who] = _tOwned[_who].sub(tAmount);
        }
        ...
        emit Transfer(_who, address(0), rAmount);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        bool takeFee = true;
        if (_isExcluded[recipient] || _isExcluded[sender]) {
            takeFee = false;
            removeAllFee();
        }
