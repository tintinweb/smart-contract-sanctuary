...
contract UniswapV2ERC20 {
    ...
    string public constant name = 'Doge Swap LP Token';
...
contract UniswapV2Pair is UniswapV2ERC20 {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    uint256 liquidity = rootK.sub(rootKLast);
    ...
    function swap(
        ...
        require(amount0In > 0 || amount1In > 0, 'UniswapV2: INSUFFICIENT_INPUT_AMOUNT');
        {
            ...
            uint256 balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(IUniswapV2Factory(factory).feeRate()));
            uint256 balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(IUniswapV2Factory(factory).feeRate()));
            require(
...
contract UniswapV2Factory is IUniswapV2Factory {
    ...
    function getReserves(address tokenA, address tokenB) public override view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = UniswapV2Pair(pairFor(tokenA, tokenB)).getReserves();
