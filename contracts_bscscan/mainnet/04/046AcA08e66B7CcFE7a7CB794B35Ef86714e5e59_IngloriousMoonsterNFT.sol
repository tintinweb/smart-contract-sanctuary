...
contract IngloriousMoonsterNFT is ERC721Enumerable, Ownable{
    ...
    constructor() ERC721("Moonster", "Moonster") {
        _baseTokenURI = "https://ingloriousmoonster.findmynft.net/api/";
    ...
    function mint_IngloriousMoonster() public payable returns (uint256){
        require(totalSupply() <= 82, "enough minting");

        _tokenIds.increment();
        uint256 newIngloriousMoonster = _tokenIds.current();
        uint256 price = getPrice(newIngloriousMoonster);
        require(msg.value >= price, "not enough");

        _mint(msg.sender, newIngloriousMoonster);
        emit minting(newIngloriousMoonster);

        return newIngloriousMoonster;
    }
