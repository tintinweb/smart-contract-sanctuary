...
contract NUTGAIN is ERC20, Ownable {
	using SafeMath for uint256;
	...
	NUTGAINDividendTracker public dividendTracker;
	NUTGAINTransferHelper private transferHelper;
	...
	uint256 public _maxWalletAmount = 50000000 * (10 ** 18);
	...
	constructor(address routerAddress, address payable devWalletAddress) public ERC20("NUTGAIN", "NUTG") {
		dividendTracker = new NUTGAINDividendTracker();
		transferHelper = new NUTGAINTransferHelper(routerAddress);
		...
		// set default fees (dividends, liquidity)
		setBuyFees(0, 0, 0);
		setSellFees(0, 0, 0);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "NUTGAIN: The dividend tracker already has that address");

		NUTGAINDividendTracker newDividendTracker = NUTGAINDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "NUTGAIN: The new dividend tracker must be owned by the NUTGAIN token contract");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "NUTGAIN: Account is already the value of 'excluded'");
	...
	function setDevWallet(address payable wallet) public onlyOwner {
		require(wallet != owner(), "Dev wallet cannot be the owner");
		_devWalletAddress = wallet;
		excludeFromFees(_devWalletAddress, true);
		excludeFromMaxWallet(wallet, true);
	}
	
	function setBuyFees(uint256 _dividendsFee, uint256 _liquidityFee, uint256 _devFee) public onlyOwner {
		buyFees = FeeSet({
	...
	function setSellFees(uint256 _dividendsFee, uint256 _liquidityFee, uint256 _devFee) public onlyOwner {
		sellFees = FeeSet({
			dividendsFee: _dividendsFee,
			liquidityFee: _liquidityFee,
			devFee: _devFee
	...
	function getSumOfFeeSet(FeeSet memory set) private pure returns (uint256) {
		return set.dividendsFee.add(set.liquidityFee).add(set.devFee);
	}
	...
	function setMaxWalletAmount(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "NUTGAIN: Amount cannot be over the total supply.");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "NUTGAIN: gasForProcessing must be between 200,000 and 500,000");
	...
	function processFees(uint256 amountIn) private {
		...
		uint256 ethForLiquidity = amountOut.sub(ethForDividends).sub(ethForDev);
...
contract NUTGAINDividendTracker is Ownable, DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() public DividendPayingToken("NUTGAIN_Dividend_Tracker", "NUTGAIN_Dividend_Tracker") {

	}

	function _transfer(address, address, uint256) internal override {
		require(false, "NUTGAIN_Dividend_Tracker: No transfers allowed");
	}

	function withdrawDividend() public override {
		require(false, "NUTGAIN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NUTGAIN contract.");
	}
	...
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "NUTGAIN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "NUTGAIN_Dividend_Tracker: Cannot update claimWait to same value");
...
contract NUTGAINTransferHelper is Ownable {
