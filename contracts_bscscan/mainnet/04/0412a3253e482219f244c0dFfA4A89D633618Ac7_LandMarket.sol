...
contract MarketStore{
    ...
    // LandMarket
    mapping (uint => LandSales) numToLandSales;
}
...
abstract contract MarketService is MarketStore, ILandMarket{
    ...
    uint public constant freezeTime = 30 minutes;
    ...
    modifier onlyCEO() {
        ...
        require(msg.sender == landCoreInter.getCEO(), "only ceoAddress");
        _;
    }
    ...
    modifier unlockThawing(uint _landId){
        require( numToLandSales[_landId].thawingTime <= block.timestamp, "is freeze time");
        _;
    }
    ...
    function delLandSalesInfo(uint _landId) external override onlyLandMarketDep{
        if (numToLandSales[_landId].landId != 0) {
            LandSales memory _ls = numToLandSales[_landId];
            delete numToLandSales[_landId];
            shopLandCount = shopLandCount.sub(1);
            emit GetOffShopLand(_landId,_ls.seller);
        }
    }

    function addLandSalesInfo(uint _landId, address _seller, uint _sellPrice) external override onlyLandMarketDep{
        if (numToLandSales[_landId].landId == 0) {
            numToLandSales[_landId] = LandSales(_landId, _seller, _sellPrice, block.timestamp, block.timestamp + freezeTime);
...
contract LandMarket is MarketService{
    ...
    function putShopLand(uint _landId,uint _sellPrice) external override landOnlyOwnerOf(_landId) returns(uint){
        require(numToLandSales[_landId].landId == 0, "The land on the shop");
        IRichToken richTokenInter = IRichToken(farmDep.richTokenAddress());
        require(_sellPrice >= farmDep.minPrice() && _sellPrice < richTokenInter.getCapValue(), 'Your price must > minPrice and < capValue');
        numToLandSales[_landId] = LandSales(_landId, msg.sender,_sellPrice, block.timestamp, block.timestamp + freezeTime);
        shopLandCount = shopLandCount.add(1);
        emit PutShopLand(_landId, msg.sender, _sellPrice, block.timestamp);
        return _landId;
    }

    function getOffShopLand(uint _landId) external override landOnlyOwnerOf(_landId) unlockThawing(_landId) returns(uint){
        require(numToLandSales[_landId].landId != 0, "The land is not on the shop");
        delete numToLandSales[_landId];
        shopLandCount = shopLandCount.sub(1);
        emit GetOffShopLand(_landId,msg.sender);
        return _landId;
    }

    function buyShopLand(uint _landId) public {
        LandSales memory _landSale = numToLandSales[_landId];
        ...
        richTokenInter.transferAccount(msg.sender,_landSale.seller,_landSale.price);
        ILandCore landCoreInter = ILandCore(farmDep.landCoreAddress());
        landCoreInter.safeTransferByMainContract(_landSale.seller,msg.sender, _landId);
        emit BuyShopLand(_landId, msg.sender, _landSale.seller, _landSale.price);
    }

    function getShopLand() external view returns(uint[] memory) {
        ILandCore landCoreInter = ILandCore(farmDep.landCoreAddress());
        uint[] memory result = new uint[](shopLandCount);
        uint counter = 0;
        for (uint i = 0; i < landCoreInter.getOpenedLandCount(); i++) {
            if (numToLandSales[i].price != 0) {
                result[counter] = numToLandSales[i].landId;
                counter++;
            }
        }
        return result;
    ...
    function getShopByLandId(uint _landId) external view override returns(address, uint, uint, uint) {
        return (numToLandSales[_landId].seller, numToLandSales[_landId].price, numToLandSales[_landId].timestamp, numToLandSales[_landId].thawingTime);
    }
}