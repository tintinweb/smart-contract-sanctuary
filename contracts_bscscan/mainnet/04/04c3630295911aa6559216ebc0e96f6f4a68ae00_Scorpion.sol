...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also Scorpioned.
...
// File: contracts/ScorpionDividendPayingToken.sol
...
contract Scorpion is ERC20, Ownable {
	...
	address payable public _MarketingWalletAddress = payable(0x3D4C9a3C70409E4ae96760F6716D73291f1423Ef); // Marketing Address

	
    bool private swapping;

    ScorpionDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 private _setmaxSell = 100000000000000 * (10**18); 
    ...
    constructor() ERC20("Scorpion", "SCORPION")  {
        ...
        dividendTracker = new ScorpionDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Scorpion: The dividend tracker already has that address");

        ScorpionDividendTracker newDividendTracker = ScorpionDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Scorpion: The new dividend tracker must be owned by the Scorpion token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Scorpion: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PanCAKESwapV2Pair, "Scorpion: The PanCAKESwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Scorpion: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Scorpion: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Scorpion: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Scorpion: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!tradingEnabled) {
            require(canTransferBeforetradingEnabled[from], "Scorpion: This account cannot send tokens until trading is enabled");
        }
...
contract ScorpionDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("Scorpion_Dividend_Tracker", "Scorpion_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**2); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "Scorpion_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "Scorpion_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Scorpion_Dividend_Tracker: Cannot update claimWait to same value");
