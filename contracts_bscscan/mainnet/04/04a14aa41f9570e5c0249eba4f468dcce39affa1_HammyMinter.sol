...
// Dependency file: contracts/hammy/PancakeSwap.sol
...
abstract contract PancakeSwap is OwnableUpgradeable {
    ...
    function tokenToHammy(address token, uint amount) internal returns(uint hammyAmount) {
        if (token == cake) {
            ...
            hammyAmount = _bunnyToHammy(amount);
        } else {
    ...
    function _bunnyToHammy(uint amount) private returns(uint hammyAmount) {
        uint256 hammyBefore = IBEP20(hammy).balanceOf(address(this));
        swapToken(bunny, amount, hammy);
        hammyAmount = IBEP20(hammy).balanceOf(address(this)).sub(hammyBefore);
    }
...
// Root file: contracts/hammy/HammyMinter.sol
...
// import "contracts/hammy/PancakeSwap.sol";
