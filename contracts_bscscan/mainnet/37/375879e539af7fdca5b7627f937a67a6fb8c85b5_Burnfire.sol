...
contract ExtendedReflections is Context, IERC20, Ownable {
    ...
    */
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if( // BUY
            AMMPairs[sender] &&
            recipient != address(_uniswapV2Router) //pair -> router is adding liquidity
        ) {
        ...
        if ( // SELL
            AMMPairs[recipient] &&
            sender != address(_uniswapV2Router) && //router -> pair is removing liquidity
            !_isExcludedFromFee[sender]
        ) {
            selling = true;
        }

        ValuesFromAmount memory values = _getValues(amount, _isExcludedFromFee[sender], selling);
        ...
        if (!_isExcludedFromFee[sender]) {
            _afterTokenTransfer(values, selling);
        }
    ...
    /**
      ...
      */
    function _afterTokenTransfer(ValuesFromAmount memory values, bool selling) internal virtual {
        // Burn
        if (_autoBurnEnabled) {
        ...
        // 10% burn tax when selling
        if (selling) { 
            _tokenBalances[address(this)] += values.tSellBurnFee;
            _reflectionBalances[address(this)] += values.rSellBurnFee;
            _approve(address(this), _msgSender(), values.tSellBurnFee);
            burnFrom(address(this), values.tSellBurnFee);
        }
    ...
    function _getValues(uint256 amount, bool deductTransferFee, bool selling) private view returns (ValuesFromAmount memory) {
        ...
        _getRValues(values, deductTransferFee, selling);
        return values;
    }
    ...
    function _getTValues(ValuesFromAmount memory values, bool deductTransferFee, bool selling) view private {
        ...
        if (deductTransferFee) {
            ...
            if (selling) {
                values.tSellBurnFee = _calculateTax(values.amount, taxSellBurn, 0);
            }
            
            // amount after fee
            values.tTransferAmount = values.amount - values.tBurnFee - values.tRewardFee - values.tLiquifyFee - values.tMarketingFee - values.tSellBurnFee;
        }
     ...
     * More details can be found at comments for ValuesForAmount Struct.
     */
    function _getRValues(ValuesFromAmount memory values, bool deductTransferFee, bool selling) view private {
        uint256 currentRate = _getRate();
        ...
        if (deductTransferFee) {
            ...
            if (selling) {
                values.rSellBurnFee = values.tSellBurnFee * currentRate;
            }

            values.rTransferAmount = values.rAmount - values.rBurnFee - values.rRewardFee - values.rLiquifyFee - values.rMarketingFee - values.rSellBurnFee;
        }
    ...
    */
    ...
    function _setAMMPair(address pair, bool value) private onlyOwner {
...
contract Burnfire is ExtendedReflections {

    uint256 private _tokenSupply = 10000000000; // 10 billion

    uint32 private _taxBurn = 3;
    ...
    uint256 private _maxBuyAmount = 200000000; // 2% of tokenSupply
    ...
    constructor () ExtendedReflections("Burnfire", "BF", _decimals, _tokenSupply) {
