...
contract BlindBoxLive {
  ...
  constructor() public{
    ...
    // xanaliaDex = 0x1c06d9295f9EBa99F51f4d0c143Baac4eC42A6b2;
    // LPAlia=IERC20(0x3581735ce4d03f65e57614f521383e50e1678d98);
    // LPBNB=IERC20(0x1B96B92314C44b159149f7E0303511fB2Fc4774f);
    // isInitialized1=true;
    // isInitialized2=true;
    // chainRan = IERC20(0x538B8357FBDaFf50e54eD9751e8BD4dAFA4dD67B);
  }
  ...
  function init3() public  adminAdd{
    require(isInitialized1 && !isInitialized3, "already initialized");
    isInitialized3=true;
    deployTime = now;
  ...
  // function initializeBox4(string memory name) adminAdd public {
  //   uint256 counter;
  //     for(uint256 i = 0; i < 2; i++){
  //      for (uint256 j = 0; j < 100; j++) {
  //         randomValueMain = chainRan.getRandomVal();
  //         uint256 randNumber =  (uint256(keccak256(abi.encode(randomValueMain, (block.timestamp + counter++ )))) % 1000);
  ...
  //     boxDetails[name].usedCountByday[index].count = 10;
      
  //   }
  // }

  function initializeBox4(string memory name, uint256 startingIndex) adminAdd public {
    uint256 counter = 100 * startingIndex;
      for(uint256 i = startingIndex; i < startingIndex + 2; i++){
       for (uint256 j = 0; j < 100 || (i == 9 && j < 101); j++) {
          randomValueMain = chainRan.getRandomVal();
          uint256 randNumber =  (uint256(keccak256(abi.encode(randomValueMain, (block.timestamp + counter++ )))) % (i == 9 ? 1100 : 1000));
          boxDetails[name].luckyBox[i][randNumber] = true;
        }
      }
    for (uint256 index = startingIndex; index < startingIndex + 2; index++) {
        boxDetails[name].usedCountByday[index].isSold = false;
        boxDetails[name].usedCountByday[index].count = startingIndex == 9 ? 11 : 10;
    }
  }
  ...
  function buyBox(address seller, string memory ownerId, string memory name, uint256 selectedTime, bool isPaid) public {
    ...
    require(boxDetails[name].initialize < block.timestamp && (boxDetails[name].initialize + (24 hours * dayCount)) + 16 hours > block.timestamp , "Box not available");
    ...
    for(uint256 i =0; i<Category[name].countNFT; i++)
    {
     ...
     if(boxDetails[name].tokenUriUsedCount[Category[name].tokenUris[randNumber]] >= 100){
    ...
    !isLucky && ALIA.transferFrom(msg.sender, 0xb5CDA0299b18C0f92cf48138530ca1506bbb64a6, price);
  }
  ...
  function isBoxSold(uint256 selectedTime, string memory name) public view returns(bool) {
    ...
    uint256 dayCount = ( currentTime - boxDetails[name].initialize)/(60 * 60 * 24);
    
    return Category[name].count == 0 || boxDetails[name].usedCountByday[dayCount].isSold || boxDetails[name].initialize >= currentTime || !(boxDetails[name].dailiyLimit > 0 && dayCount < Category[name].count / boxDetails[name].dailiyLimit) ||  (boxDetails[name].initialize + (24 hours * dayCount)) + 16 hours <= currentTime ;
  }
