...
contract LakeMarketplace is Ownable, ReentrancyGuard {
    ...
    function _buyItem(
        ...
        address _treasury = collectionRoyalties[_nftAddress].treasury;
        uint16 royalty = collectionRoyalties[_nftAddress].royalty;
        if (_treasury != address(0) && royalty != 0) {
            uint256 royaltyFee = price.sub(feeAmount).mul(royalty).div(
                10000
            );
            if (_payToken == address(0)) {
                (bool royaltyTransferSuccess, ) = payable(_treasury).call{
                    value: royaltyFee
                }("");
                require(
    ...
    function acceptOffer(
        ...
        if (_treasury != address(0) && royalty != 0) {
            ...
            offer.payToken.safeTransferFrom(_creator, _treasury, royaltyFee);
            feeAmount = feeAmount.add(royaltyFee);
        }
