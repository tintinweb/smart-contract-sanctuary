...
contract test11 is Context, IBEP20, Ownable {
    ...
    uint256 public _numOfTokensToExchangeForBNB = _tTotal  * 5 / (10**5);  // 0.005%
    //buy
    ...
    address payable public _gameWalletAddress;
 
    uint256 public _resAndDevFee;
    uint256 private _previousResAndDevFee = _resAndDevFee;
    address payable public _resAndDevWalletAddress;
...
constructor (string memory Name, string memory Symbol, uint256 taxFee, uint256 marketFee, uint256 gameFee, uint256 resAndDevFee, uint256 nftFee,
    ...
    function setNumOfTokensToExchangeForBNB(uint256 amount, uint256 percent) external onlyOwner() {
        _numOfTokensToExchangeForBNB = _tTotal.mul(amount).div(percent);
    }
