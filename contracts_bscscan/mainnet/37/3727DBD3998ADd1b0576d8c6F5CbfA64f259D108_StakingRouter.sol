...
contract StakingRouter is RecoverableByOwner, ITokenSupplier {
    ...
    mapping(address => uint256) public pendingSupplies; /// @dev If there is more than one StakingService it is necessary to store supplied amount of Token between the invocations of particular service to return correct amount of supplied tokens
    ...
    function changeStakingServiceShares(
        ...
        require(
            ...
            "StakingRouter: addresses must be the same length as shares"
        );
        int128 cumulativeShare = 0;
        for (uint256 i = 0; i < shares.length; i++) {
            require(
                addresses[i] != address(0),
                "StakingRouter: zero address is invalid"
            );
            require(shares[i] > 0, "StakingRouter: shares must be positive");
            cumulativeShare += shares[i];
            for (uint256 j = i + 1; j < shares.length; j++) {
                require(
                    addresses[i] != addresses[j],
                    "StakingRouter: duplicate addresses are not possible"
                );
            }
        }
        require(
            cumulativeShare <= ABDKMath64x64.fromInt(1),
            "StakingRouter: shares must be le 1<<64 in total"
        );
        ...
        returns (uint256 supply)
    {
        ...
        require(transferred, "StakingRouter: TOKEN_FAILED_TRANSFER");
        return supply;
    }
    ...
    {
        uint256 supply = ITokenSupplier(token).supplyToken(maxTime);
        uint256 activeServicesLength = activeServices.length;
        for (
