...
contract AIRD is Context, IERC20, Ownable {
    ...
    uint private startingSupply = 1500000000000000; // 1.5 quadrillion
    ...
    string private _name = "Air Doge";
    string private _symbol = "AIRD ";
    
    // ALL FEES ARE DIVIDED BY 10**5 FOR MORE ACCURACY
    // EXAMPLE: 400 / 10**5 = 4%
    uint256 private _reflectFee = 200; // Set dynamically
    uint256 public _buyReflectFee = 200;
    uint256 public _sellReflectFee = 400;
    uint256 private maxReflectFee = 500;
    
    uint256 private _liquidityFee = 200; // Set dynamically
    uint256 public _buyLiquidityFee = 200;
    uint256 public _sellLiquidityFee = 400;
    uint256 private maxLiquidityFee = 500;

    uint256 private _marketingFee = 200; // Set dynamically
    uint256 public _buyMarketingFee = 200;
    uint256 public _sellMarketingFee = 300;
    uint256 private maxMarketingFee = 500; 

    uint256 private _burnFee = 200; // Set dynamically
    uint256 public _buyBurnFee = 200;
    uint256 public _sellBurnFee = 200;
    uint256 private maxBurnFee = 500; 
    
    uint256 private _airDropFee = 200; // set dynamically
    uint256 public _buyairDropFee = 200;
    uint256 public _sellairDropFee = 200;
    uint256 private maxairDropFee = 500;
    ...
    address payable private _marketingWallet = payable(0xb97DC510A9249fcE089D9624F187bC93e7927607);//select your marketing address
    address payable private _airDropWallet = payable(0xD2e62456200Ba3b3D143681FA307Ef3f38F740bF); //airDrop address
   ...
   // uint256 public maxWalletSizeUI = (startingSupply * maxWalletPercent) / maxWalletDivisor; // Actual amount for UI's
    ...
    function setBuyTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee, uint256 burnFee, uint airDropFee) external onlyOwner() {
        ...
        require(airDropFee<=maxairDropFee); // Prevents owner from abusing fees.
...
/*
    ...
    */
    ...
    function setAirDropWallet(address payable aWallet)external onlyOwner{
        require(_airDropWallet != aWallet, "Wallet already set!");
        _airDropWallet = payable(aWallet);
    }
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwner {
     ...
     function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(_marketingWallet, transferredBalance.div(_buyLiquidityFee).mul(_buyMarketingFee));
        
        //send to airdrop address
        transferToAddressETH(_airDropWallet, transferredBalance.div(_buyLiquidityFee).mul(_buyairDropFee));
