...
// File: IDividendPayingTokenOptional.sol
...
10% Dex Token reflection fee 
5% Liquidity fee 

Dex Token Contract - 0xe91a8D2c584Ca93C7405F15c22CdFE53C29896E3
Uni Token Price: $.22
Uni Token MCAP: $60,000,000
*/
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable DividendsToken = address(0xe91a8D2c584Ca93C7405F15c22CdFE53C29896E3);
...
// File: BabyLiteDividendTracker.sol
...
contract BabyUniswapDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BabyUniswap_Dividend_Tracker", "BabyUniswap_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "BabyUniswap_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BabyUniswap_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyUniswap contract.");
    }
    ...
    function setBuyBackWalletAddress(address _newAddress) external onlyOwner {
        require(buyBackWallet != _newAddress, "old address so the same as the new one");
        buyBackWallet = _newAddress;
    }

    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyUniswap_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyUniswap_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BabyDex is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BabyUniswapDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 public maxBuyTranscationAmount = 1500000 * (10**18);
    uint256 public maxSellTransactionAmount = 100000000 * (10**18);
    uint256 public swapTokensAtAmount = 500 * (10**18);
    uint256 public _maxWalletToken = 7000000 * (10**18); // 7% of total supply
    ...
    constructor() public ERC20("BabyDex", "$BABYDEX") {
        uint256 _ETHRewardsFee = 10;
    	...
    	dividendTracker = new BabyUniswapDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyUniswap: The dividend tracker already has that address");

        BabyUniswapDividendTracker newDividendTracker = BabyUniswapDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyUniswap: The new dividend tracker must be owned by the BabyUniswap token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyUniswap: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyUniswap: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyUniswap: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyUniswap: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyUniswap: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyUniswap: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyUniswap: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BabyUniswap: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of BabyUniswap
            path,
            recipient,
            block.timestamp
        );
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        swapTokensForEth(tokens.div(100*(10**18)).mul(3*(10**18)));
        payable(dividendTracker.buyBackWallet.address).transfer(tokens.div(100*(10**18)).mul(3*(10**18)));
    }
