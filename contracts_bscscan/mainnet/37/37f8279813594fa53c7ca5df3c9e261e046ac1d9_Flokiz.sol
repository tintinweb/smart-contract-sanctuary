...
interface IDEXRouter {
    ...
    function FLOKI() external pure returns (address);
    ...
    function addLiquidityFLOKI(
        ...
        address to,
        uint deadline
    ) external payable returns (uint amountToken, uint amountFLOKI, uint liquidity);
    ...
    function swapExactFLOKIForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ...
    function swapExactTokensForFLOKISupportingFeeOnTransferTokens(
        uint amountIn,
        uint amountOutMin,
        address[] calldata path,
        address to,
...
contract DividendDistributor is IDividendDistributor {
    ...
    function deposit() external payable override onlyToken {
        ...
        router.swapExactFLOKIForTokensSupportingFeeOnTransferTokens{value: msg.value}(
            0,
            path,
            address(this),
            block.timestamp
...
contract Flokiz is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Flokiz";
    string constant _symbol = "Flokiz";
    ...
    constructor () Auth(msg.sender) {
        ...
        address _presaler = 0x3D64fC7977349cD742524b69721eCdddB664aA82;
    ...
    function swapBack() internal swapping {
        ...
        router.swapExactTokensForFLOKISupportingFeeOnTransferTokens(
            amountToSwap,
            0,
            path,
            address(this),
        ...
        require(success, "receiver rejected FLOKI transfer");

        if(amountToLiquify > 0){
            router.addLiquidityFLOKI{value: amountBNBLiquidity}(
                address(this),
                amountToLiquify,
                0,
                0,
    ...
    function buyTokens(uint256 amount, address to) internal swapping {
        ...
        router.swapExactFLOKIForTokensSupportingFeeOnTransferTokens{value: amount}(
            0,
            path,
            to,
            block.timestamp
    ...
    function manualSend() external authorized {
        ...
        payable(marketingFeeReceiver).transfer(contractFLOKIBalance);
    }
