...
contract DogggoGirlNFT is ERC721Enumerable, Ownable{
    ...
    constructor() ERC721() {
        _baseTokenURI = "https://doggga.findmynft.net/api/";
        beneficiary = 0x7ecd2f10E58a01bE9713Ed2A537f63554b0f289E;
    }
    ...
    {
        require(
            totalSupply()  + _addresses.length <= 500,
            "Doggga: max total supply exceeded"
        );

        require(
            giftCount + _addresses.length <= maxGiftSupply,
            "Doggga: max gift supply exceeded"
        );
        uint256 _newItemId;
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                _addresses[ind] != address(0),
                "DogggaGirl recepient is the null address"
            );
            _tokenIds.increment();
    ...
    function mint_DogggaGirl()  external
        onlyOwner payable returns (uint256) {
        require(totalSupply() <= 500, "enough minting");

        _tokenIds.increment();
        uint256 newDogggaGirl = _tokenIds.current();
        uint256 price = getPrice();
        require(msg.value >= price, "not enough");

        _mint(msg.sender, newDogggaGirl);
        emit minting(newDogggaGirl);

        return newDogggaGirl;
    }
