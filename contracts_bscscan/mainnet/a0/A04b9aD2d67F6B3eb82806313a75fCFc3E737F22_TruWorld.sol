...
contract TruWorld is IBEP20, Context {
    using SafeMath for uint256;
    ...
    string constant _name = "TruWorld";
    string constant _symbol = "$TruWorld";
    uint8 constant _decimals = 18;

    address private _owner;

    uint256 _totalSupply = 20 * 10**15 * (10**_decimals); // 20 Quadrillion
    ...
    constructor() {
        ...
        fundFeeReceiver = address(0xBb514c372B86FD9a92e4A3404475E19c654e6070);

        marketingFeeReceiver = address(
            0x6a3e35a34a9a8dda94A160e6F680736c45FD233c
        );
        charityFeeReceiver = address(
            0x474B9D510453065d65c579544e07b425e773Bc04
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToBurn = amountSwapBackBefore.mul(burnFee).div(
            feeDenominator
        );
        _burn(address(this), amountToBurn);

        uint256 amountSwapBack = amountSwapBackBefore - amountToBurn;
        try
            ...
            )
        {
            ...
            uint256 amountBNBfund = amountBNB.mul(fundFee).div(totalBuyFee);
            ...
            payable(fundFeeReceiver).call{value: amountBNBfund, gas: 30000}("");
            ...
            emit SwapBackSuccess(amountSwapBack);
    ...
    function setFees(
        ...
        fundFee = _fundFee;
        totalBuyFee = _fundFee.add(_reflectionFee).add(_marketingFee);
        feeDenominator = _feeDenominator;
        totalSellFee = _fundFee.add(_reflectionFee).add(_marketingFee).add(
            _extraSellFee
        );
