...
// File: contracts/doggyDAIDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable BUSD = address(0x1AF3F329e8BE154074D8769D1FFa4eE058B1DBc3);
...
contract doggyDAI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable BUSD = address(0x1AF3F329e8BE154074D8769D1FFa4eE058B1DBc3);
    address private _marketingWallet = address(0xDEC2be2714AB087D07f7A9d6DeFAF55bDf605D6d);
    address private _devWallet = address(0x3D8333eBA0896109F9828F463FbCDA9aFd00a4e7);
    ...
    doggyDAIDividendTracker public dividendTracker;
    ...
    uint256 public immutable tradingEnabledTimestamp = 1627258511;
    ...
    constructor() public ERC20("doggyDAI", "doggyDAI") {
    	...
    	dividendTracker = new doggyDAIDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "doggyDAI: The dividend tracker already has that address");

        doggyDAIDividendTracker newDividendTracker = doggyDAIDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "doggyDAI: The new dividend tracker must be owned by the doggyDAI token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "doggyDAI: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "doggyDAI: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "doggyDAI: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "doggyDAI: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "doggyDAI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "doggyDAI: Cannot update gasForProcessing to same value");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "doggyDAIBSC: Account is already the value of 'excluded'");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "doggyDAI: This account cannot send tokens until trading is enabled");
        }
        ...
        if(
            ...
            uint256 BNBTokens = contractTokenBalance.mul(5).div(totalFees);
...
contract doggyDAIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("doggyDAI_Dividend_Tracker", "doggyDAI_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1 * (10**18); //must hold 1+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "doggyDAI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "doggyDAI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main doggyDAI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "doggyDAI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "doggyDAI_Dividend_Tracker: Cannot update claimWait to same value");
