 ...
 * ! Max wallet:        2 POT  
 * ! Max TX:            0.2 POT
 * ! Sell cooldown:     1 MINUTE (!)
 * ! Fees:              2% reflections // 5% burn // 5% liquidity
 */
...
contract MicroPot is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner())  require(tradingEnabled,"trading not yet enabled");
        ...
        if (isBuy && from != owner() && to != owner()){
            if(!_excludedFromBuyLock.contains(from)){   
                require(_buyLock[from]<=block.timestamp||buyLockDisabled,"Address in buyLock");
                _buyLock[from]=block.timestamp+buyLockTime;
            }
        }
        bool takeFee = true;
