...
interface iALPACAibBNB is IERC20{
...
contract InvestinStrategyToken is ERC20("ibBNB-ALPACA", "IVNs-23"), ReentrancyGuard, Ownable, iStrategy {
    
    uint constant MAX_UINT_VALUE = type(uint).max;
    iALPACAibBNB constant ibBNB = iALPACAibBNB(0xd7D069493685A581d27824Fc46EdA46B7EfC0063);
    iWBNB public constant baseToken = iWBNB(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    ...
    constructor(address[] memory _fundFactories) public {
        fundFactories = _fundFactories;
        baseToken.approve(address(ibBNB), MAX_UINT_VALUE);
    }
    ...
    function enter(uint256 amount, bool isFund, uint256 amountOutMin) public nonReentrant{
        ...
        if(isFund==false){
            ...
            baseToken.transfer(IVNy.investinVault(), _entryFee);
            amount -= _entryFee;
        }
    ...
    function enterHelper(uint amount, uint amountOutMin) internal{
        ibBNB.deposit(amount);
        
        uint ibBNBAmount = ibBNB.balanceOf(address(this));
        
        require(ibBNBAmount >= amountOutMin, "DFL");
        
        _mint(msg.sender, ibBNBAmount);
    ...
    function getStrategyTokenPrice() public view override returns (uint) {
        ...
        if(status != true)
            ...
            return exchangeRouter.getAmountsOut((ibBNB.totalToken()).mul(1e18).div(ibBNB.totalSupply()), path)[1];
    }
    
    fallback() external payable {
        uint value = msg.value;
        baseToken.deposit(value);
        if(msg.sender != address(ibBNB)){
            enterHelper(value, 0);
        }
    }
