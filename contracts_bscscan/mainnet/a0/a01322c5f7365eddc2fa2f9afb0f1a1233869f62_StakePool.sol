...
contract StakePool is Ownable {
    ...
    /**
     * @dev stake
     * @param _stakeType type of stake rate 1: 9/1, 2: 8/2, 3: 7/3, 4: 5/5, 5: 1/9
     * @param _amount    aToken amount
     */
    function stake(address _payToken, uint _stakeType, uint _amount) external payable {
        require(isOpen(_stakeType, msg.sender), "stake: isOpen(_stakeType, msg.sender)");

        uint payTokenAmount = _amount.mul(100).div(STAKE_PER[_stakeType - 1]).sub(_amount);

        if (0 < msg.value) { // pay with ETH
            require(_payToken == WETH_ADDRESS, "stake: _payToken = WETH_ADDRESS");
            // transfer to this
            require(payTokenAmount == msg.value, "stake: payTokenAmount == msg.value");
            IERC20(aToken).safeTransferFrom(msg.sender, address(this), _amount);
        } else { // pay with payToken
            // transfer to this
            IERC20(_payToken).safeTransferFrom(msg.sender, address(this), payTokenAmount);
            IERC20(aToken).safeTransferFrom(msg.sender, address(this), _amount);
        }
        // calculate power
        uint aTokenValue = getUSDTPrice(aToken).mul(_amount).div(DECIMALS);
        uint payTokenValue = getUSDTPrice(_payToken).mul(payTokenAmount).div(DECIMALS);
        ...
        item.aTokenAmount = _amount;
        item.payTokenAmount = payTokenAmount;
        item.payTokenAddr = _payToken;
        item.power = power;
        _stakeOf[msg.sender].add(item);
        _stakeOf[address(0)].add(item);

        emit Stake(msg.sender, _payToken, _stakeType, item.id, payTokenAmount, _amount);
    }
    ...
    function withdrawReward(uint _withdrawRewardId, address _to, uint _amount, uint8 _v, bytes32 _r, bytes32 _s) external {
        ...
        // transfer fee
        IERC20(aToken).safeTransfer(_to, fee);
    ...
    function getPrice(address _lp) public view returns (uint) {
        ...
        (bool success, bytes memory returnData) = _lp.staticcall(abi.encodeWithSignature("getReserves()"));
        if (success) {
            (uint112 reserve0, uint112 reserve1, ) = abi.decode(returnData, (uint112, uint112, uint32));
            return uint(reserve0).mul(DECIMALS).div(uint(reserve1));
        }
