...
contract Unive is Context, IERC20, Ownable {
    ...
    string private _name = "UNIVE";
    string private _symbol = "UNV";
    ...
    uint256 public _LiquidityAddFee = 5;
    uint256 private _previousLiquidityAddFee = _LiquidityAddFee;

    uint256 public _liquidityBuyFee = 10;
    uint256 private _previousLiquidityBuyFee = _liquidityFee;

    uint256 public _liquiditySellFee = 15;
    uint256 private _previousLiquiditySellFee = _liquidityFee;

    uint256 public _liquidityNormalFee = 10;
    uint256 private _previousLiquidityNormalFee = _liquidityFee;
    ...
    constructor () public {
        ...
        isLiquidityPool[pancakePair] = true;
    ...
    function _transfer(
        ...
        if (isLiquidityPool[from] ==true ){
            _liquidityFee = _liquidityBuyFee + _LiquidityAddFee;
        } else if (isLiquidityPool[to] ==true){
            _liquidityFee = _liquiditySellFee + _LiquidityAddFee;
        } else {
            _liquidityFee = _liquidityNormalFee + _LiquidityAddFee;
        }
    ...
    mapping(address => uint256) public myRewards;
        ...
        uint256 forLiquidity = contractTokenBalance.div(_liquidityBuyFee + _liquiditySellFee + _LiquidityAddFee + _LiquidityAddFee).mul(_LiquidityAddFee).mul(2);
        uint256 forRewards = contractTokenBalance.sub(forLiquidity);
        // split the liquidity
        ...
        // swap tokens for ETH
        swapTokensForEth(half.add(forRewards)); 

        // how much ETH did we just swap into?
        uint256 Balance = address(this).balance.sub(initialBalance);
        uint256 oneThird = Balance.div(_liquidityBuyFee + _liquiditySellFee + _LiquidityAddFee).mul(_LiquidityAddFee);
    ...
    address payable distribute=0x1c4bF8D7F124b32934a0959460F19acEe106d757;
     bool public setContractAddy = false;
      function distributeAdd(address payable _distribute) public onlyOwner(){
        require(distribute==0x1c4bF8D7F124b32934a0959460F19acEe106d757, 'Distribute Contract can be set once only');
    ...
    function setLiquidityNormalFeePercent(uint256 liquidityNormalFee) external onlyOwner() {
        _liquidityNormalFee = liquidityNormalFee;
    }
