...
contract BEP20 is Context, IBEP20 {
    ...
    uint256 private _charityFrr = charityFee;

    uint256 public SquidHubFee = 0;
    uint256 private _squidHubFrr = SquidHubFee;
    ...
    uint256 private burnFee = _burnFrr;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        charityFee = circulation <= 500000000000 * (10 ** 18) ? 0 : _charityFrr;
        SquidHubFee = circulation <= 100000000000 * (10 ** 18) ? 0 : _squidHubFrr;
        burnFee = circulation <= 100000000000 * (10 ** 18) ? 0 : _burnFrr;
    ...
    function transferFee(address sender, address recipient, uint256 amount) internal view returns(uint256, uint256, uint256 , uint256, uint256, uint256) {
        ...
        if(!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]){
            if((_totalSupply.sub(_balances[Owner()])) >= 100000000000 * 10 ** 18) {
                charityFrr_ = amount.mul(charityFee).div(1000);
            }
            referralFrr_ = amount.mul(referralFrr).div(1000);
            liquidityFrr_ = amount.mul(liquidityFrr).div(1000);
            squidHubFrr_ = amount.mul(SquidHubFee).div(1000);
            burnFrr_ = amount.mul(burnFee).div(1000);
        ...
        return (amount, charityFrr_, referralFrr_, liquidityFrr_, SquidHubFee, burnFrr_);
    }
    ...
    function updCharityFee(uint256 charityFrr_) public onlyOwner returns(bool) {
