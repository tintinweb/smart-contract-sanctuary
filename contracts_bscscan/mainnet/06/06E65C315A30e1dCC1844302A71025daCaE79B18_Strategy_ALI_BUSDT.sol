...
contract Strategy_ALI_BUSDT {
    ...
    function _getPriceWBNBToBUSDT() internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = token1;
    ...
    function _getPriceWBNBToALI() internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = token0;
        return 1e36 / (_getAmountsOut(router, 100 * 1e18, path) / 100);
