...
contract PoppyShiba is IERC20Metadata, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address public marketingWallet = 0xCDA523e9fF879aCe6788DC2cEF060FA5F2309A54; // Marketing Address
    address public charityWallet = 0x3f130199a44A3756d8C9b2E19Ca7B07F5b4DfE6d; // Charity Address
    ...
    string constant _name = "PoppyShiba";
    string constant _symbol = "POPPY";
    uint8 constant _decimals = 9;
    
    uint256 private constant MAX = ~uint256(0);
    uint256 internal constant _totalSupply = 1000000000 * 10**9;
    ...
    uint256 public charityPortionOfSwap = 30;
    uint256 public devPortionOfSwap = 40;
    ...
    uint256 public sellingCharityFee = 3;
    uint256 public buyingCharityFee = 3;
    ...
    uint256 public buyersTotalFees = buyingMarketingFee.add(buyingRfiFee).add(buyingCharityFee).add(buyingDevFee);
    uint256 public sellersTotalFees = sellingMarketingFee.add(sellingRfiFee).add(sellingCharityFee).add(sellingDevFee);
    ...
    function calcBuyersTotalFees() private {
        buyersTotalFees = buyingMarketingFee.add(buyingRfiFee).add(buyingCharityFee).add(buyingDevFee);
    }
    
    function calcSellersTotalFees() private {
        sellersTotalFees = sellingMarketingFee.add(sellingRfiFee).add(sellingCharityFee).add(sellingDevFee);
    }
    ...
    function updateSellingCharityFee (uint256 newFee) external onlyOwner {
        sellingCharityFee = newFee;
        calcSellersTotalFees();
    }
    ...
    function updateBuyingCharityFee (uint256 newFee) external onlyOwner {
        buyingCharityFee = newFee;
        calcBuyersTotalFees();
    }
    ...
    function updateCharityWallet(address newCharityWallet) external onlyOwner {
        require(newCharityWallet != charityWallet, "The Charity wallet is already this address");
    ...
    function updatePortionsOfSwap(uint256 marketingPortion, uint256 devPortion, uint256 charityPortion) 
    external onlyOwner {
        
        uint256 totalPortion = marketingPortion.add(devPortion).add(charityPortion);
    ...
    function _takeFees(uint256 amount, uint256 currentRate, uint256 sumOfFees, bool isBuying) private {
        uint256 theRfi = isBuying ? buyingRfiFee : sellingRfiFee;
        ...
        if ( sumOfFees > 0 && !isInPresale ){
            if(antiBotEnabled && block.timestamp <= _startTimeForSwap) {
                ...
                uint256 otherFees = sumOfFees.sub(theRfi);
                if(isBuying) {
