...
contract WrcpPresale2 {
    using SafeMath for uint256;

    uint256 public constant _minimumDepositBNBAmount = .38 ether; // Equivalent to .38 BNB minimum deposit
    uint256 public constant _maximumDepositBNBAmount = 19 ether; // Equivalent to 14 BNB maximum deposit

    uint256 public constant _bnbAmountCap = 8400 ether; // Allow cap at 8400 BNB
    uint256 public _presale2DateStarts = 1637884800;
    uint256 public _presale2DateEnds = 1638662400;
    uint256 public _tokenValuePerBNB = 5263;

    bool public _isPresale2Ended; // Has admin decided to end presale2? 
    ...
    //Adjust presale2 date starts
    function setPresale2DateStarts(uint256 _newDate) external onlyAdmin {
        _presale2DateStarts = _newDate;
    }
    //Adjust presale2 date ends
    function setPresale2DateEnds(uint256 _newDate) external onlyAdmin {
        _presale2DateEnds = _newDate;
    }
    ...
    function removeAllDepositAddress(uint256 number) external onlyAdmin {
        ...
        require(block.timestamp < _presale2DateStarts,"Presale2 already started");
    ...
    function Buy() external payable {
        ...
        require(block.timestamp >= _presale2DateStarts && block.timestamp <= _presale2DateEnds, "Deposit rejected, presale2 has either not yet started or not yet overed");
        require(!_isPresale2Ended, 'Admin has ended presale2 earlier');
        ...
        if (_totalAddressesDepositAmount.add(msg.value) > _bnbAmountCap) {
            ...
            erc20Contract.transferPresale2(msg.sender, value2.mul(_tokenValuePerBNB));
            ...
            erc20Contract.transferPresale2(msg.sender,value.mul(_tokenValuePerBNB));
        }
    }

    // Allow admin to end Presale2 earlier
    function endPresale2() external onlyAdmin {
        _isPresale2Ended = true;
    }
