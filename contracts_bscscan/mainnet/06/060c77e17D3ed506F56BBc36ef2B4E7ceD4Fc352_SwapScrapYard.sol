...
contract SwapScrapYard {
    ...
    IPancakeFactory factoryContract; // = IPancakeFactory(0xca143ce32fe78f1f7019d7d551a6402fc5350c73);
    AggregatorV3Interface priceFeedUsd = AggregatorV3Interface(0x87Ea38c9F24264Ec1Fff41B04ec94a97Caf99941);
    AggregatorV3Interface priceFeedBnb = AggregatorV3Interface(0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE);
    ...
    constructor (address _owner, address _payoutWallet, address _scrapPair, address _factory, IBEP20 _scrap, IBEP20 _busd) {
        owner = payable(_owner);
        payoutWallet = payable(_payoutWallet);
        scrap = _scrap;
        busd = _busd;
        scrapPairAddress = IPancakePair(_scrapPair);
        factoryContract = IPancakeFactory(_factory);
        scrapSupply = 10000e9;
    }
    ...
    function swapNoneWorthyToken(IBEP20 _token)public returns(bool){
        ...
        _token.transferFrom(msg.sender, payoutWallet, tokenAmount);
        scrapedTokens.push(address(_token));

        uint256 reward=busd.balanceOf(payoutWallet).div(1e3);
    ...
    function swapAndLiquifyBNB(uint256 tokenBalance,IBEP20 _token) private {
        ...
        scrap.transferFrom(payoutWallet,msg.sender,amountToSend);
