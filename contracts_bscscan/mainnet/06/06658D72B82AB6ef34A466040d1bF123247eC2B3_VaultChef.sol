...
contract VaultChef is Ownable, ReentrancyGuard, Operators {
    ...
    uint16 public depositFeeBP = 300;   // 300 / 10000 = 3% deposit fee
    address public feeAddress = 0x70aDCAa34c21D1934cf7041a7aF70e74aEa0c519;
    ...
    function _deposit(uint256 _pid, uint256 _wantAmt, address _to) internal {
        ...
        if (_wantAmt > 0) {
            ...
            if (depositFeeBP > 0) {
                uint feeAmount = _wantAmt.mul(depositFeeBP).div(10000);
                pool.want.safeTransfer(feeAddress, feeAmount);
    ...
    function withdrawAll(uint256 _pid) external {
        _withdraw(_pid, ~uint256(0), msg.sender);
    }
    ...
    function resetAllowances() external onlyOwner {
        for (uint256 i=0; i<poolInfo.length; i++) {
            ...
            pool.want.safeIncreaseAllowance(pool.strat, ~uint256(0));
        }
    }
    ...
    function setDepositFee(uint16 _depositFeeBP) public onlyOwner {
        require(_depositFeeBP <= 500, 'VaultChef: exceeds maximum deposit fee');
