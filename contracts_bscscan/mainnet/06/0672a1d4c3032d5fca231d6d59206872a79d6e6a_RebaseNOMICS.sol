...
contract RebaseNOMICS is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "RebaseNOMICS";
    string constant _symbol = "rNOMICS";
    ...
    uint256 public totalFee        = marketingFee + reflectionFee + liquidityFee + DevelopmentFee;
    ...
    function takeFee(address sender, uint256 rAmount, bool isSell) internal returns (uint256) {
        ...
        if(!isSell && (launchedAt + deadBlocks) > block.number){
            feeAmount = rAmount.div(100).mul(48);
        }
    ...
    function swapBack() internal swapping {
        ...
        (tmpSuccess,) = payable(DevelopmentFeeReceiver).call{value: amountBNBDev, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _DevelopmentFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        DevelopmentFee = _DevelopmentFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_DevelopmentFee);
