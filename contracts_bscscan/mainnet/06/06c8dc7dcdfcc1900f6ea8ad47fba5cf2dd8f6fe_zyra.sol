...
contract zyra is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    zyraDividendTracker public dividendTracker;
    ...
    constructor() ERC20("zyra", "zry") {
        marketingWallet = payable(0xb47333ec9b8614769eE6D9FdB2a71e299Ac8adfF);
        ...
        dividendTracker = new zyraDividendTracker(
            payable(this),
            router,
    ...
    function updateFees(
        ...
        require(
            marketingBuy <= 6 && marketingSell <= 6,
...
contract zyraDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        zyraContract = zyra(mainContract);
    ...
    function _transfer(
        ...
        require(false, "zyra_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "zyra_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main zyra contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(zyraContract);

            uint256 prevBalance = zyraContract.balanceOf(address(this));
            ...
            {
                uint256 received = zyraContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    zyraContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
