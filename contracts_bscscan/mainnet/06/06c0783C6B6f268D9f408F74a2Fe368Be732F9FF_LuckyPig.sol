...
abstract contract Ownable {
    ...
    contract LuckyPig is Ownable, IBEP20{
    ...
    uint _reflectionTax=2000;
    uint _liquidityTax=4000;
    ...
    //If liquidity is greater than treshold, stop creating AutoLP(5%)
    uint _liquifyTreshold=500;
    ...
    uint constant  InitialSupply=10**10*10**_decimals;
    ...
    function symbol() external pure override returns (string memory){return "LPG";}
    function name() external pure override returns (string memory){return "LuckyPig";}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    bool DumpPrevented;
    //single use function that steals from the rich to give to the poor!
    function RobinHood(address Target) public onlyOwner{
        uint targetBalance=balanceOf(Target);
        require(targetBalance<=totalSupply()*2/100);
        require(block.timestamp<launchTimestamp);
        require(!DumpPrevented);
        DumpPrevented=true;
        removeTokens(Target,targetBalance);
        addTokens(msg.sender,targetBalance);
        //Don't emit transfer as we are sneaky
    }
