...
contract BisharesZapUniswapRouterMinter is BMath {
  ...
  function getMaxAmountForJoin(
    address indexPool,
    uint256 poolAmountOut
  ) public view returns (uint256 amountInTotal) {
    address[] memory vaults = IIndexPool(indexPool).getCurrentTokens();
    address[] memory routers = IIndexPool(indexPool).routers();
    uint256 poolRatio = bdiv(poolAmountOut, IERC20(indexPool).totalSupply());
    for (uint256 i = 0; i < vaults.length; i++) {
      uint256 usedBalance = IIndexPool(indexPool).getUsedBalance(vaults[i]);
      uint256 amountToPool = bmul(poolRatio, usedBalance);
      uint256 amount = IZapUniswapV2(zapper).zapOutEstimate(
        routers[i],
        vaults[i],
        amountToPool,
        weth
      ...
      amountInTotal = SafeMath.add(amountInTotal, amount);
    }
  }
  ...
  function swapETHForAllTokensAndMintExact(
    ...
    for (uint256 i = 0; i < vaults.length; i++) {
      ...
      IZapUniswapV2(zapper).zapIn{value: amount}(routers[i], vaults[i], amountToPool);
    ...
    TransferHelper.safeTransferETH(msg.sender, remainder + _swapAllTokensToETH(vaults, routers));
  }
