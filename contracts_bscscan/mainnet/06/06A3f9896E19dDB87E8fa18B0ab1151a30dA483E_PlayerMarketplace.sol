...
contract PlayerMarketplace is Ownable {
    ...
    function removeFromActiveAuctions(uint256 index) internal {
        ...
        for(uint256 i = index; i < activeAuctions.length - 1; i++)
        {
    ...
    function massUpdateAuctions() public {
        for(uint256 i = 0; i < auctions.length; i++)
        {
            ...
            {
                if(auctions[i].endBlock <= block.timestamp)
                {
                    auctions[i].status = "Expired";
                    removeFromActiveAuctions(getIndexOfActiveAuction(auctions[i].auctionID));
                }
            }
        }
    }
