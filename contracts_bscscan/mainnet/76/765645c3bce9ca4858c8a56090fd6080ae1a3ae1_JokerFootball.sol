...
contract JokerFootball is Context, IERC20, Ownable {
    ...
    string private _name = "JokerFootball";
    string private _symbol = "JKF";
    ...
    constructor (address treasuryAddress, address marketingAddress) public {
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        uint256 tLiquidity = tOtherFee.mul(_liquidityFee).div(otherFee);
        uint256 tmarketing = tOtherFee.mul(_marketingFee).div(otherFee);
        uint256 tTreasury = tOtherFee.mul(_treasuryFee).div(otherFee);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tmarketing, uint256 tTreasury) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tmarketing, tTreasury, _getRate());

        uint256 tOtherFee = tLiquidity.add(tmarketing).add(tTreasury);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        tTransferAmount = tTransferAmount.sub(tmarketing).sub(tTreasury);
        return (tTransferAmount, tFee, tLiquidity, tmarketing, tTreasury);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tmarketing, uint256 tTreasury, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rBurn = tmarketing.mul(currentRate);
        uint256 rTreasury = tTreasury.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity);
        rTransferAmount = rTransferAmount.sub(rBurn).sub(rTreasury);
    ...
    function _getOtherFee() private view returns (uint256) {
        uint256  otherFee = _previousLiquidityFee.add(_previousmarketingFee).add(_previousTreasuryFee);
        return otherFee;
    }

    function _withdrawBNB(uint256 amount) external onlyOwner() {
