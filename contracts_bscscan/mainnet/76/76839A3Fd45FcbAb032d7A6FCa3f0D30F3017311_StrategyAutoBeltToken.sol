...
// File: contracts/M7Finance/interfaces/auto/IAutoFarmV2.sol
...
// File: contracts/M7Finance/interfaces/belt/IBeltToken.sol
...
contract StrategyAutoBeltToken is  Ownable, Pausable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Tokens used
    address constant public wbnb = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c); // mainnet
    address constant public Auto = address(0xa184088a740c695E156F91f5cC086a06bb78b827); // mainnet
    
    address public want;       // beltBNB
    address public wantToken;  //wBNB
    /**
     ...
     * 1.5% -> VAULT holders Fee
     * 0.5% -> Trading comp fee
     * 1% -> Treasury fee
     */
    function chargeFees() internal {
        uint256 toWbnb = IERC20(Auto).balanceOf(address(this));

        uint256 vaultHoldersFee = toWbnb.mul(VAULT_HOLDERS_FEE).div(MAX_FEE);
        pancakeswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(vaultHoldersFee, 0, AutoToWbnbRoute, vaultRewardsPool, block.timestamp.add(300));

        uint256 tradingCompFee = toWbnb.mul(TRADING_COMP_FEE).div(MAX_FEE);
        pancakeswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(tradingCompFee, 0, AutoToWbnbRoute, tradingCompWallet, block.timestamp.add(300));

        uint256 treasuryFee = toWbnb.mul(TREASURY_FEE).div(MAX_FEE);
        pancakeswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(treasuryFee, 0, AutoToWbnbRoute, treasuryWallet, block.timestamp.add(300));
    ...
    function addLiquidity() internal {
        ...
        pancakeswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(autoBal, 0, AutoToWantTokenRoute,  address(this), block.timestamp.add(300));
        ...
        dividendsCollected = dividendsCollected.add(autoBal);
    }
    ...
    function _retireStrat() internal {
        IAutoFarmV2(autofarm).withdraw(poolId, 2**256 - 1);
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(autofarm, 2**256 - 1);
        IERC20(Auto).safeApprove(address(pancakeswapV2Router), 2**256 - 1);
        IERC20(wantToken).safeApprove(want, 2**256 - 1);
    }

    function _removeAllowances() internal {
        IERC20(want).safeApprove(autofarm, 0);
        IERC20(Auto).safeApprove(address(pancakeswapV2Router), 0);
