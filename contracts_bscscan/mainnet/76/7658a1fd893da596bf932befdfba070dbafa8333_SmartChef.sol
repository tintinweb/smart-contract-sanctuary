...
contract SmartChef is Ownable {
    ...
    function deposit(uint256 _pid,uint256 _amount, address _referrer, uint256 _type) public {
        ...
        if (user.amount > 0) {
            ...
            uint256 pending = pendingReward(_pid,msg.sender)*1000000000000000000;
            if(pending > 0) {
                pool.rewardToken.transfer(address(msg.sender), pending.mul(8).div(10));
    ...
    /*function testTransfer() public{
        ...
        pool.rewardToken.transfer(address(msg.sender), 1);
    }*/
    ...
    function payReferralCommission(IBEP20 _rewardToken,address _user, uint256 _pending) internal {
        ...
        if(commissionAmount>0){
            ...
            address lead = userInfo[_user].lead;
            if(lead != address(0)){
                _rewardToken.transfer(lead, commissionAmountLead);
                emit ReferralCommissionPaid(_user, lead, commissionAmountLead);
            }
