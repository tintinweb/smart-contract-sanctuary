...
/**
 *GYSR was submitted for verification at Etherscan.io on 2020-11-04 Bailey Building and Loans (BBL) was submitted June 2021 
*/
...
This Contract is used for Farming - Checks our Docs for contracts 
...
contract Geyser is IGeyser, ReentrancyGuard {
    ...
    //Fees 
    uint256 public withdrawalFeeInBips = 65; //Default is 0.65% 

    address public TREASURY_ADDR = address(msg.sender) ;  //default is master Dev Address  // to-do Get address 


    //locking 
    uint256 public lockUpInSeconds = 0; // 86400-day 604800-week 2629743-month
    ...
    function unstake(
        ...
        _unstake(amount, 0); //The Paying-to-earn-more feature has been disabled, any USDC sent to this contract during Unstake will be lost
    }
        ...
        returns (uint256)
    {
        ...
        require(  secondsWaited()  >= lockUpInSeconds , "Funds have not unlocked yet");
        ...
        //Withdrawal Fees  - to-do 
        uint256 fee = amount.mul(withdrawalFeeInBips).div(10000); //to-do add fee parameter  
        uint256 amountAfterFees = amount.sub(fee); 


        // transactions 
        _stakingPool.transfer(msg.sender, amountAfterFees); //to-do add the fee here.
        emit Unstaked(msg.sender, amountAfterFees, totalStakedFor(msg.sender), "");
        _stakingPool.transfer( TREASURY_ADDR, fee );
    ...
    //Dev Function to modify Fees as appropriate 
    function updateFee(uint256 feeInBips) external onlyOwner {  //to-do check external 
        require( feeInBips >= 0, "Withdrawal Fee Can't Be Negative Fee"  );
        require( feeInBips <= 500, "Withdrawal Fee Can't be Higher than 5%"  ) ;   // 1 = .01%, 10 = .1% , 100 = 1%, 500 = 5% & 500 Max    
        withdrawalFeeInBips = feeInBips ; 

    }

    //Dev Function to modify treasury addr 
    function updateTreasuryAddr( address newTreasuryAddr ) external onlyOwner { 
        require( newTreasuryAddr != address(0), "Provide a valid Address for TREASURY_ADDR") ; 
        TREASURY_ADDR = newTreasuryAddr; 
    }

    //Dev function to adjust lock time
    function updatelockUpInSeconds(uint256 lockTime ) external onlyOwner {
        uint256 month = 2629743 ;
        require( lockTime >= 0, "Time lock needs to be non-negative"  );  // non-negative lock on funds 
        require( lockTime <= month.mul(3), "Needs to be less than 3 months"  );  // max lock is 3 months
        lockUpInSeconds = lockTime ;
    }

    //remaining time lock up 
    function secondsWaited( ) public view returns(uint256) {
        uint256 lastStake = userStakes[msg.sender].length.sub(1);
        return ( block.timestamp.sub( userStakes[msg.sender][lastStake].timestamp )) ; 
    } 
