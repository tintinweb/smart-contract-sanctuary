...
contract BSCWinBulls is Context, ERC20, Ownable, Proxiable, DataLayout {
    ...
    function isTimeForLottery() public view returns(bool) {
        ...
        for (uint i; i < lotteryTimes.length; i++) {
            require(lotteryTimes[timeIndex] > 0);
        }
    ...
    function didUserWin(uint32 winningId) internal view returns(bool) {
        if (_isExcluded[idAddress[winningId]]) {
            return false;
        }
    ...
    receive() external payable {
        (bool sent, bytes memory data) = DAOWallet.call{value: address(this).balance}("");
    }
    ...
    function setIsExcluded(address user, bool _value) external onlyOwner {
        _isExcluded[user] = _value;
        if (!_value && userId[user] == 0) {
    ...
    function _transfer(
        ...
        if(from != owner()) {
            require(isLive, "Trading has not started.");
        }
        ERCStorage(tokenStorage).sendToken1();
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = cakeRouter.WETH();

        _approve(address(this), address(cakeRouter), tokenAmount);

        // make the swap
        cakeRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of ETH
            path,
            DAOWallet, // The contract
            block.timestamp
        );
