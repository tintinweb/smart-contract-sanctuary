...
/**
 * Developed by snipe.finance for MST Team
 */
...
// File: contracts/MSTDividendPayingToken.sol
...
contract MST is ERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 45000 * (10**18);
    ...
    address public charityTokenAddressForFee = address(0x0000000000000000000000000000000000000000);
    address public buyBackTokenAddressForFee = address(0x0000000000000000000000000000000000000000);

    // transfer fees
    uint256 public marketFee = 2;
    uint256 public developmentFee = 3; //development 
    uint256 public devFee = 3;         //development total 6%
    uint256 public tokenRewardsFee = 1;
    uint256 public liquidityFee = 1;
    uint256 public totalFees = tokenRewardsFee.add(liquidityFee).add(marketFee).add(developmentFee).add(devFee);
    // buy fees
    uint256 public buyMarketFee = 2;
    uint256 public buyDevelopmentFee = 3; //development
    uint256 public buyDevFee = 3;         //development total 6%
    uint256 public buyTokenRewardsFee = 1;
    uint256 public buyLiquidityFee = 1;
    uint256 public buyTotalFees = buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketFee).add(buyDevelopmentFee).add(buyDevFee);
    // sell fees
    uint256 public sellMarketFee = 3;
    uint256 public sellDevelopmentFee = 3; //development
    uint256 public sellDevFee = 3;         //development total 6%
    uint256 public sellTokenRewardsFee = 3;
    uint256 public sellLiquidityFee = 1;
    uint256 public sellTotalFees = sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketFee).add(sellDevelopmentFee).add(sellDevFee);
    // fee tracker
    ...
    event LPwalletUpdated(address indexed newLPwallet, address indexed oldLPwallet);
    ...
    constructor() public ERC20("Metastake", "MST") {
        // LP address
        burnAddress = address(0x7D6b68e3E3dcdC34EB53Cc602AAe4e5925FE7FeE);
        uint256 _processDividendTime = block.timestamp;
        processDividendTime = _processDividendTime;

        dividendTracker = new MSTDividendTracker();
    ...
    function updateFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _developmentFee, uint256 _devFee) external onlyOwner {
        tokenRewardsFee = _tokenRewardsFee;
        liquidityFee = _liquidityFee;
        marketFee = _marketFee;
        developmentFee = _developmentFee;
        devFee = _devFee;
        totalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_developmentFee).add(_devFee);
    }    
    
    function updateBuyFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _developmentFee, uint256 _devFee) external onlyOwner {
        buyTokenRewardsFee = _tokenRewardsFee;
        buyLiquidityFee = _liquidityFee;
        buyMarketFee = _marketFee;
        buyDevelopmentFee = _developmentFee;
        buyDevFee = _devFee;
        buyTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_developmentFee).add(_devFee);
    }    
    
    function updateSellFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _developmentFee, uint256 _devFee) external onlyOwner {
        sellTokenRewardsFee = _tokenRewardsFee;
        sellLiquidityFee = _liquidityFee;
        sellMarketFee = _marketFee;
        sellDevelopmentFee = _developmentFee;
        sellDevFee = _devFee;
        sellTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_developmentFee).add(_devFee);
    }
    ...
    function swapAndLiquify(uint256 tokens) private  {
        ...
        uint256 developmentFeeFull = tokens.mul(developmentFee).div(100);
        uint256 devFeeFull = tokens.mul(devFee).div(100);
        uint256 tokensAfterFee = tokens - marketFeeFull - developmentFeeFull - devFeeFull;
        ...
        uint256 developmentFeeBalance = newBalance.mul(developmentFee).div(100);
        uint256 devFeeBalance = newBalance.mul(devFee).div(100);
        uint256 finalBalance = newBalance - marketFeeBalance - developmentFeeBalance - devFeeBalance;
        ...
        if(charityTokenAddressForFee != address(0)){
            swapEthForTokens(developmentFeeBalance, charityTokenAddressForFee, charityAddress);
        }else{
            (bool sent,) = charityAddress.call{value: developmentFeeBalance}("");
        }        
        if(isBuyBackActive){
            swapEthForTokens(devFeeBalance, address(this), buyBackAddress);
        }else{
            (bool sent,) = buyBackAddress.call{value: devFeeBalance}("");
        }
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "MST: This account cannot send tokens until trading is enabled");
        }
...
contract MSTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("MST_Dividend_Tracker", "MST_Dividend_Tracker") {
        claimWait = 3600;
