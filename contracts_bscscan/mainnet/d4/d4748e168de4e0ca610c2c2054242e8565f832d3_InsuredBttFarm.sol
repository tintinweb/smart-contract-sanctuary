...
contract InsuredBttFarm {
    ...
    ERC20 constant btt = ERC20(0x8595F9dA7b868b1822194fAEd312235E43007b49);
    ...
    BttSyrupPool constant cakePool = BttSyrupPool(0x3b804460C3C62f0F565aF593984159f13B1AC976);
    ...
    constructor() public {
        ...
        btt.approve(address(cakeV2), 2 ** 255);
    }
    ...
    function claimYield() public {
        ...
        if (dividends > 0 && dividends <= btt.balanceOf(address(this))) {
            payoutsTo[farmer] += (int256) (dividends * magnitude);
            btt.transfer(farmer, dividends);
        }
    ...
    function pullOutstandingDivs() internal {
        uint256 beforeBalance = btt.balanceOf(address(this));
        address(cakePool).call(abi.encodePacked(cakePool.deposit.selector, abi.encode(0)));

        uint256 divsGained = btt.balanceOf(address(this)) - beforeBalance;
        if (divsGained > 0) {
    ...
    function cashout(uint256 amount) external {
        ...
        address(cakePool).call(abi.encodePacked(cakePool.withdraw.selector, abi.encode(amount)));
    ...
    function dividendsOf(address farmer) view public returns (uint256) {
        uint256 unClaimedDivs = cakePool.pendingReward(address(this));
