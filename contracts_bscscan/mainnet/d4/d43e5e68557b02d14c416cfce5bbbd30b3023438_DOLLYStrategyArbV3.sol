...
// File: contracts/strategies/DOLLYStrategyArbV3.sol
...
contract DOLLYStrategyArbV3 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public percentFlashDepositor = 10000; // 10% to depositors when flash loans are used
    ...
    address constant DOP_ADDRESS = address(0x844FA82f1E54824655470970F7004Dd90546bB28); // DOP
    address constant DOPPLE_TWINDEX = address(0xF0AEA510a29DDAEbe4Ac8b198BF2f984c7A74aDC);
    address constant PANCAKE_ROUTER_V2 = address(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    address constant DOPPLE_STABLESWAP = address(0x61f864a7dFE66Cc818a4Fd0baabe845323D70454);
    ...
    function setupWithdrawTokens() internal {
        // Start with DOLLY
        IERC20 _token = IERC20(address(0xfF54da7CAF3BC3D34664891fC8f3c9B6DeA6c7A5));
        tokenList.push(
            TokenInfo({
                token: _token,
    ...
    function simulateExchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal view returns (uint256) {
        if(_idOut == WBNB_TOKEN_ID){
            ...
            if(_idIn == 0){
                // Convert DOLLY to BUSD first
                ValueLikeExchange swap = ValueLikeExchange(DOPPLE_STABLESWAP);
                _amount = swap.calculateSwap(tokenList[0].swapID, tokenList[1].swapID, _amount); // Get BUSD amount
            }
            // Now use Pancake to convert to WBNB
            TradeRouter router = TradeRouter(PANCAKE_ROUTER_V2);
            path = new address[](2);
            ...
            ValueLikeExchange swap = ValueLikeExchange(DOPPLE_STABLESWAP);
            _amount = swap.calculateSwap(tokenList[_idIn].swapID, tokenList[_idOut].swapID, _amount);
            return _amount;
        }
    }
    ...
    function exchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal {
        if(_idOut == WBNB_TOKEN_ID){
            ...
            if(_idIn == 0){
                // Convert DOLLY to BUSD first
                ValueLikeExchange swap = ValueLikeExchange(DOPPLE_STABLESWAP);
                tokenList[_idIn].token.safeApprove(DOPPLE_STABLESWAP, 0);
                tokenList[_idIn].token.safeApprove(DOPPLE_STABLESWAP, _amount);
                uint256 _bal = tokenList[1].token.balanceOf(address(this));
                swap.swap(tokenList[0].swapID, tokenList[1].swapID, _amount, 1, now.add(60));
            ...
            path[0] = address(tokenList[1].token);
            path[1] = WBNB_ADDRESS;
            tokenList[1].token.safeApprove(PANCAKE_ROUTER_V2, 0);
            tokenList[1].token.safeApprove(PANCAKE_ROUTER_V2, _amount); 
            router.swapExactTokensForTokens(_amount, 1, path, address(this), now.add(60));
        }else{
            ValueLikeExchange swap = ValueLikeExchange(DOPPLE_STABLESWAP);
            tokenList[_idIn].token.safeApprove(DOPPLE_STABLESWAP, 0);
            tokenList[_idIn].token.safeApprove(DOPPLE_STABLESWAP, _amount);
            swap.swap(tokenList[_idIn].swapID, tokenList[_idOut].swapID, _amount, 1, now.add(60));
        }
        return;
    }
    ...
    function checkAndSwapToken(address _executor, uint256 targetID, bool doFlash) internal {
        ...
        bool _expectIncrease = false;
        {
            if(doFlash == false){
                ...
                // It will perform a flash loan and return the profit to this strategy as BUSD if successful
                _expectIncrease = true;
                DOLLYFlashModule(flashModuleAddress).doFlashTrade();
    ...
    function expectedProfit(bool inWBNBForExecutor, bool checkFlash) external view returns (uint256,uint256,uint256) {
...

        {
            if(checkFlash == false){
                ...
                _normalizedGain = DOLLYFlashModule(flashModuleAddress).checkFlashTrade(); // Gain in BUSD
                _allocatedStipend = _allocatedStipend.mul(3);
                targetID = 1;
            }
        }
