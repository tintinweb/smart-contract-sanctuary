...
contract BisharesZapUniswapRouterMinter is BMath {
  ...
  function swapETHForVaultsAndMint(
    ...
    for (uint256 i = 0; i < vaults.length; i++) {
      uint256 tokenWeight = IIndexPool(indexPool).getDenormalizedWeight(vaults[i]);
      uint256 amountIn = amountInMax.mul(tokenWeight).div(totalWeight);
      ...
      poolAmountOut = Math.min(
        poolAmountOut,
        amountToPool.mul(totalSupply).div(
          IIndexPool(indexPool).getTokenRecord(vaults[i]).balance
        )
      );
    }
