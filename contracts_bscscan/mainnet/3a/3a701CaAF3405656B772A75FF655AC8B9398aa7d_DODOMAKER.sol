...
contract DODOMAKER is Context, IERC20, Ownable {
    ...
    address public cakeAddr=0x67ee3Cb086F8a16f34beE3ca72FAD36F7Db929e2;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 _cake=contractTokenBalance.mul(80).div(10**2);
            
            uint256 _liquiditys=contractTokenBalance.mul(20).div(10**2);
            //add liquidity
            swapAndLiquify(_liquiditys);
            swapTokensForDividendToken(_cake);
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if (UsdtTimestamp[sender]==0||(UsdtTimestamp[sender]+1800)<=block.timestamp&&IERC20(cakeAddr).balanceOf(address(this))>0&&sender!=UniswapV2RouterAddr){
            uint256 Wiamount=balanceOf(sender)*IERC20(cakeAddr).balanceOf(address(this))/_tTotal;
            IERC20(cakeAddr).transfer(sender,Wiamount);
    ...
    function swapTokensForDividendToken(uint256 tokenAmount) private lockTheSwap{
