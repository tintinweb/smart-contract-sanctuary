...
contract SmartChef is Ownable {
    ...
    function withdraw(uint256 _amount) public {
        if(_amount<=getTotalAmountCanRemove(msg.sender)*10000000000){
            PoolInfo storage pool = poolInfo[0];
            UserInfo storage user = userInfo[msg.sender];
            updatePool(0);
            uint256 pending = pendingReward(msg.sender);
            ...
            if(_amount > 0) {
                ...
                //delete array
                for(uint i=0; i<idTodelete.length; i++){
                    //remove array
                    delete user.amountDetail[idTodelete[i]];
                    delete user.createDates[idTodelete[i]];
                }
