...
contract EddaNFTMarket is ERC1155Holder, Ownable, Pausable, ReentrancyGuard {
	...
	address public immutable eddaPriceFetch;
	uint256 private BUFFER = 1050;
	uint256 private taxFee;
	uint256 private totalOrder;
	...
	constructor(address _eddaNFT, address _taxReceiver, address _eddaPriceFetch) public {
		taxReceiver = _taxReceiver;
		taxFee = 60;
		eddaNFT = IERC1155(_eddaNFT);
		paymentMethod[address(0)] = true;
		eddaPriceFetch = _eddaPriceFetch;
	}

	function updateTax(address _taxReceiver, uint256 _taxFee) external onlyOwner() {
		taxReceiver = _taxReceiver;
		taxFee = _taxFee;
	}

	function updateBuffer(uint256 _buffer) external onlyOwner() {
		BUFFER = _buffer;
	}
	...
	function whiteListPaymentMethod(address[] calldata _tokens, bool[] calldata _isWhiteList) external onlyOwner() {
		for(uint i = 0; i < _tokens.length; i++) {
			paymentMethod[_tokens[i]] = _isWhiteList[i];
			if(_isWhiteList[i] == true) { 
				IERC20(_tokens[i]).safeApprove(dexRouter, uint256(-1));
			} else { 
				IERC20(_tokens[i]).safeApprove(dexRouter, 0);
			}
		}
	}

	function getPath(address _tokenIn, address _tokenOut) private returns (address[] memory path) {
		address pair = IDexFactory(dexFactory).getPair(_tokenIn, _tokenOut);
		if (pair == address(0)) {
			path = new address[](3);
			path[0] = _tokenIn;
			path[1] = WBNB;
	...
	function _swapTokenForToken(
		...
		uint256[] memory amounts = IDexRouter(dexRouter).swapExactTokensForTokens(amount, 0, getPath(_tokenIn, _tokenOut), address(this), deadline);
		return amounts[1];
	}

	function _swapBNBForToken(address _tokenOut) private returns (uint256) {
		uint256[] memory amounts = IDexRouter(dexRouter).swapExactETHForTokens{ value: msg.value }(0, getPath(WBNB, _tokenOut), address(this), deadline); // amounts[0] = WETH, amounts[1] = DAI
		return amounts[1];
	}
	...
	function _swapTokenForBNB(
		...
		uint256[] memory amounts = IDexRouter(dexRouter).swapExactTokensForETH(amount, 0, getPath(_tokenIn, WBNB), address(this), deadline);
		return amounts[1];
	}
	...
	function buy(
		...
		require(amount > 0, 'Invalid-amount');
		require(paymentMethod[paymentToken], 'Invalid-payment-method');
		Order memory order = orders[orderId];
		require(amount <= order.amount, 'Invalid-amount');
		
		uint256 orderAmount = order.pricePerToken.mul(amount);
		uint256 taxAmount = orderAmount.mul(taxFee).div(1000);

		if (paymentToken == order.paymentToken && paymentToken == address(0)) {
			// paid in BNB
			require(msg.value >= orderAmount);
			payable(taxReceiver).sendValue(taxAmount);
			payable(order.orderOwner).sendValue(orderAmount.sub(taxAmount));
			if (msg.value > orderAmount) {
				uint256 amountReturn = msg.value.sub(orderAmount);
				payable(msg.sender).sendValue(amountReturn);
			}
		} else if (paymentToken == order.paymentToken && paymentToken != address(0)) {
			IERC20(paymentToken).safeTransferFrom(msg.sender, taxReceiver, taxAmount);
			IERC20(paymentToken).safeTransferFrom(msg.sender, order.orderOwner, orderAmount.sub(taxAmount));
		} else if (paymentToken != order.paymentToken && paymentToken == address(0)) {
			uint256 amountTokenSwap = _swapBNBForToken(order.paymentToken);
			require(amountTokenSwap >= orderAmount, 'Invalid-amount-payment');
			IERC20(order.paymentToken).safeTransfer(taxReceiver, taxAmount);
			IERC20(order.paymentToken).safeTransfer(order.orderOwner, orderAmount.sub(taxAmount));
			if (amountTokenSwap > orderAmount) {
				uint256 amountReturn = amountTokenSwap.sub(orderAmount);
				IERC20(order.paymentToken).safeTransfer(msg.sender, amountReturn);
			}
		} else {
			uint paymentAmount; 
			// uint bufferAmount;
			uint amountTokenSwap;
			if (order.paymentToken == address (0)) {
				paymentAmount = IEddaPriceFetch(eddaPriceFetch).estimateSwapAmount(paymentToken, WBNB, orderAmount);
				// bufferAmount = paymentAmount.mul(BUFFER).div(1000);
				IERC20(paymentToken).safeTransferFrom(msg.sender, address(this), paymentAmount);
				amountTokenSwap = _swapTokenForBNB(paymentToken, paymentAmount);
			} else { 
				paymentAmount = IEddaPriceFetch(eddaPriceFetch).estimateSwapAmount(paymentToken, order.paymentToken, orderAmount);
				// bufferAmount = paymentAmount.mul(BUFFER).div(1000);
				IERC20(paymentToken).safeTransferFrom(msg.sender, address(this), paymentAmount);
