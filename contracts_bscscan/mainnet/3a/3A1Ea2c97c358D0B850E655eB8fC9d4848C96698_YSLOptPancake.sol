...
contract YSLOptPancake is YSLOpt {
    ...
    function setPath(address[] memory _path0, address[] memory _path1) external onlyRole(STRAT_ROLE) {
        require(_path0.length > 0 && _path1.length > 0, "Incorrect paths");
        if (path0.length > 0) {
            delete path0;
        }
    ...
    function currentRewardUSD(address _user) public view returns (uint256) {
        ...
        uint256 usersUSD = IStrategySwap(stratSwap).getBusdAmount(userCompounds, lpToken, pancakeSwap, path0, path1);
        // Optimisation
