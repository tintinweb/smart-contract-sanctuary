...
contract DogeSon is Context, IBEP20, Ownable {
    ...
    uint256 public _liquidityFee = 1400; // Percentages for liquidity, buyback, marketing and dev (NOTE: liquidity is 4%, buyback is 2%, marketing is 4% and dev is 4%)
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _buyBackFee = 200; // 4%
    uint256 public _marketingFee = 400; // 4%
    uint256 public _devFee = 400; // 4%
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
     ...
     function setBuyBackFeePercent(uint256 value) external onlyOwner() {
         _liquidityFee = _liquidityFee.sub(_buyBackFee);
        _buyBackFee = value;
        _liquidityFee = _liquidityFee.add(_buyBackFee);
    }
    
    function setMarketingFeePercent(uint256 value) external onlyOwner() {
         _liquidityFee = _liquidityFee.sub(_marketingFee);
        _marketingFee = value;
        _liquidityFee = _liquidityFee.add(_marketingFee);
    }
    
    function setDevFeePercent(uint256 value) external onlyOwner() {
         _liquidityFee = _liquidityFee.sub(_devFee);
        _devFee = value;
