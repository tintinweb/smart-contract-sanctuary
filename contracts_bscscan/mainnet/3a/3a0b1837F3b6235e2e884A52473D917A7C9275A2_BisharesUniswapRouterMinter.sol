...
contract BisharesUniswapRouterMinter is BMath {
  ...
  constructor(address factory_, address weth_, address multisig_) public {
    factory = factory_;
    weth = weth_;
    multisig = multisig_;
  }
  ...
  function getMaxAmountForJoin(
    ...
    address indexPool,
    uint256 poolAmountOut
  ) public view returns (uint amountInMax) {
    ...
    amountInMax = SafeMath.div(SafeMath.mul(amountInMax, 101), 100);
  }
  ...
  function _collectFee(
    ...
    uint256 amountInMax,
    bool isWeth
  ) private returns (uint256 newAmountInMax, uint256 fee) {
    fee = SafeMath.div(amountInMax, 100);
    if (isWeth) {
      TransferHelper.safeTransfer(tokenIn, multisig, fee);
    } else {
      TransferHelper.safeTransferFrom(tokenIn, msg.sender, multisig, fee);
    }
    newAmountInMax = SafeMath.sub(amountInMax, fee);
  }
