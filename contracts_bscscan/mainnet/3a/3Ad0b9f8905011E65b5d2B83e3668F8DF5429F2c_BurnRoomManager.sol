...
contract BurnRoomManager is SafeOwnable, Random {
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public openedNum;
    mapping(uint256 => mapping(uint256 => bool)) public allOpened;
    mapping(uint256 => uint256) public openMaxLoop;
    ...
    function beginLoop(uint _rid) public {
        ...
        if (room.currentLoop >= room.totalLoop) {
            if (room.currentLoop == room.totalLoop && room.openNum == room.totalNum) {
    ...
    function finishRandom(bytes32 _requestId) internal override {
        ...
        openedNum[random.rid][random.loop] = openedNum[random.rid][random.loop] + 1;
        if (openedNum[random.rid][random.loop] == room.totalNum) {
        ...
        for (uint i = random.loop; i < room.currentLoop; i ++) {
            if (allOpened[random.rid][i]) {
                if (roomReward[random.rid][i] > 0 && winerNum[random.rid][i] == 0) {
                    roomReward[random.rid][i + 1] = roomReward[random.rid][i + 1].add(roomReward[random.rid][i]);
    ...
    function claim(uint256 _rid, uint256 _loop, address _to) external {
        ...
        uint maxLoop = openMaxLoop[_rid];
        if (maxLoop == 0) maxLoop = 1;
        if (maxLoop < _loop) {
            for (uint i = maxLoop; i <= _loop; i ++) {
                require (openedNum[_rid][_loop] == room.totalNum, "loop not ready");
            }
            openMaxLoop[_rid] = _loop;
        }
