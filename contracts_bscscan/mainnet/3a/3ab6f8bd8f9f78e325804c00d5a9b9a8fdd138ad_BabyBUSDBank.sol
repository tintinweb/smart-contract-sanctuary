...
// File: contracts/BabyBUSDBankDividendPayingToken.sol
...
// File: contracts/BabyBUSDBank.sol
...
contract BabyBUSDBank is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BabyBUSDBankDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("BabyBUSDBank", "BabyBUSDBank") {
    	...
    	dividendTracker = new BabyBUSDBankDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyBUSDBank: The dividend tracker already has that address");

        BabyBUSDBankDividendTracker newDividendTracker = BabyBUSDBankDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyBUSDBank: The new dividend tracker must be owned by the BabyBUSDBank token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyBUSDBank: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyBUSDBank: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyBUSDBank: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyBUSDBank: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyBUSDBank: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyBUSDBank: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyBUSDBank: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BabyBUSDBank: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of BabyBUSDBank
            path,
            recipient,
            block.timestamp
        );
...
contract BabyBUSDBankDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BabyBUSDBank_Dividend_Tracker", "BabyBUSDBank_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "BabyBUSDBank_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BabyBUSDBank_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyBUSDBank contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyBUSDBank_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyBUSDBank_Dividend_Tracker: Cannot update claimWait to same value");
