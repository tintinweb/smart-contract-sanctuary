...
contract TimeLock is Ownable {
    ...
    address public CYOToken = 0x0B3E1B1D6EA46D60879b96F3bddfFCb7dAc491d5;
    address public lpToken = 0x09516D131152A48d0cddCDa9Dd817b508d0947Bc;

    /**
     * @notice Transfers tokens held by timelock to owner.
     */
    function releaseCYOToken() external onlyOwner {
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp >= releaseTime, "TokenTimelock: current time is before release time");

        uint256 amount = IERC20(CYOToken).balanceOf(address(this));
        require(amount > 0, "TokenTimelock: no tokens to release");

        IERC20(CYOToken).safeTransfer(owner(), amount);
    }
    ...
    function getCYOTokenBalance() external view returns(uint256) {
        return IERC20(CYOToken).balanceOf(address(this));
    }
    ...
    function setCYOToken(address _CYOToken) external onlyOwner {
        CYOToken = _CYOToken;
    }
