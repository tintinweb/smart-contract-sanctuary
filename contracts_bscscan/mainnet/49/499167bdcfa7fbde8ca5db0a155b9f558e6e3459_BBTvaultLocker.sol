...
contract BBTvaultLocker is Ownable{
    using SafeMath for uint256;
    ...
    address public taxMarkAddress;
    
    event Withdraw(address withdrawer, uint256 amount);
    
    constructor(address _taxMarkAddress) {
        taxMarkAddress = _taxMarkAddress;
    }
    ...
    function lockTokens(IERC20 _token, address _withdrawer, uint256 _amount, uint256 _unlockTimestamp) external returns (uint256 _id) {
        ...
        _token.safeTransfer(taxMarkAddress, tax);
    ...
    function customLockTokens(uint256 _id) external {
        ...
        lockedToken[_id].token.safeTransfer(taxMarkAddress, tax);
    ...
    function settaxMarkAddress(address _taxMarkAddress) external onlyOwner {
        taxMarkAddress = _taxMarkAddress;
    }
