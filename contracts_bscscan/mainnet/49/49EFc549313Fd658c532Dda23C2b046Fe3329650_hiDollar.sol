...
abstract contract FrozenableToken is Ownable
{
    ...
    function setApprover(address wallet, bool approve) public onlyOwner {
        Approvers[wallet] = approve;

        if (!approve)
          delete Approvers[wallet];
    }

    function freezeAccount(address to, bool freeze) public {
        require(Approvers[msg.sender], "Non-approver not allowed");
...
contract hiDollar is ERC20, ERC20Burnable, Pausable, FrozenableToken {
    ...
    constructor() ERC20("bi Dollar", "BI") {
        holder = msg.sender;
    ...
    function setDefaultFeeRatio(uint256 newRatio) public onlyOwner {
        require(newRatio <= MAX_FEE_RATIO, "hiDollar: new fee ratio exceeds MAX_FEE_RATIO");
        defaultFeeRatio = newRatio;
    }

    function setUserFeeRatio(address user, uint256 ratio, bool enabled) public onlyOwner {
    ...
    function setHolder(address newHolder) public onlyOwner {
        require(newHolder != address(0), "hiDollar: zero address not allowed for mint holder");
        holder = newHolder;
    }
    ...
    function setProposer(address wallet, bool on) public onlyOwner {
        Proposers[wallet] = on;

        if (!on)
          delete Proposers[wallet];
    }
    ...
    */
    function proposeMint(uint256 amount) public returns(bool) {
        require(Proposers[msg.sender], "hiDollar: non-proposer not allowed");
        require(amount > 0, "hiDollar: zero amount not allowed" );

        //mint by a proposer for once only otherwise would be overwritten
        Proposals[msg.sender] = amount;
        return true;
    }

    function approveMint(address proposer, uint256 amount, bool approve) public returns(bool) {
        require( Approvers[msg.sender], "hiDollar: none-approver not allowed" );
        ...
        require( amount > 0, "hiDollar: non-positive amount not allowed" );
        require( Proposals[proposer] >= amount, "hiDollar: over-approve mint amount not allowed" );

        _mint(holder, amount);

        Proposals[proposer] -= amount;
        if (Proposals[proposer] == 0) {
            delete Proposals[proposer];
        }
