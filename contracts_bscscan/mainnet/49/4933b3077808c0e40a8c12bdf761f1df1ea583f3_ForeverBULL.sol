...
contract ForeverBULL is Context, IERC20, Ownable {
    ...
    address payable public _marketingAddress = payable(0x2f894734aF7B84f65D1911eB0A031dB2548d41D4); //Marketing Address
    ...
    string private _name = "ForeverBULL"; // token name
    string private _symbol = "FORBULL"; // token symbol
    ...
    uint256 public _burnFee = 1; // burn fee 0.5%
    uint256 private _previousBurnFee = _burnFee; // burn fee

    uint256 public _reflectionFee = 5; //reflection fee 2%
    ...
    uint256 public _marketingFee = 50; // marketing fee 4%
    uint256 private _previousMarketingFee = _marketingFee; // marketing fee
    
    uint256 public _liquidityFee = 10; // actual liquidity fee 5%
    uint256 private _previousLiquidityFee = _liquidityFee; // restore actual liquidity fee

    uint256 private _totalLiquidityFee = _liquidityFee.add(_marketingFee); // liquidity + marketing fee on each transaction
    ...
    uint256 public _maxTxAmount = 1000000000 * 10**6 * 10**9; // max allowed tokens tranfer per transaction
    uint256 public  numTokensSellToAddToLiquidity = 500000000 * 10**3 * 10**9; // min token liquidity fee collected before swapandLiquify
    uint256 public _maxTokensPerAddress            = 1000000000 * 10**6 * 10**9; // Max number of tokens that an address can hold
     ...
     */
    function setMinTokensSendToLiquidity(uint256 minLiquidityValue) public onlyOwner()
    {
        numTokensSellToAddToLiquidity = minLiquidityValue;
    }
