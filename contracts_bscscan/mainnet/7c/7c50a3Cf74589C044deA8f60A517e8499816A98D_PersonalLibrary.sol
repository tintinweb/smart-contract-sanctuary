...
contract PersonalLibrary {
    ...
    */
    function claimRewardNativeTokens(address[] memory _tokens) strategistOrInvestor public {
        ...
        for (uint256 i; i < _tokens.length; i++){
            ...
            require(yieldBurn.add(ethToDevelopment).add(amountToInvestor) == rewardAmount, 'wrong math');
    ...
    */
    function claimRewardYIELDTokens(address[] memory _tokens) strategistOrInvestor public {
        ...
        uint256 yieldToBurn = rewardAmount.sub(rewardAmount.mul(percentageDecimals.sub(_factory.onRewardYieldBurn())).div(percentageDecimals));
        uint256 yieldToDevelopmentFund = rewardAmount.sub(rewardAmount.mul(percentageDecimals.sub(_factory.onRewardYieldDevelopmentFund())).div(percentageDecimals));
        uint256 yieldToInvestor = rewardAmount.sub(yieldToBurn).sub(yieldToDevelopmentFund);
        
        /*uint256 ethToDevelopmentFund = */convertTokenToETH(_factory.developmentFund(), yieldToken, yieldToDevelopmentFund);
        require(rewardAmount == yieldToBurn.add(yieldToDevelopmentFund).add(yieldToInvestor), 'wrong math');
    ...
    */
    function stakeReward(address[] memory _tokens) strategistOrInvestor public returns (uint256) {
        ...
        uint256 gotRewards = IERC20(yieldStakeRewardToken).balanceOf(address(this)).sub(rewardBalanceBefore);
        if(gotRewards > 0){
        ...
        for(uint256 c = 0; c < stakeArraySize; c++){
            ...
            if(element.unlockAt <= block.timestamp){
                if(_amountToRemove <= element.amount){
                    element.amount = element.amount.sub(_amountToRemove);
                    _amountToRemove = 0;
                    break;
                }else{
                    _amountToRemove = _amountToRemove.sub(element.amount);
                    element.amount = 0;
                }
            }
        }
