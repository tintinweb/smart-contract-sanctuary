...
interface ITokenMasterNFT is IERC721 {
...
contract TokenFarmingBank is IERC721Locker, OwnableUpgradeable, ReentrancyGuard, ContractGuard {
    ...
    address public nft; // TOKENMASTER
    ...
    //   TOTAL:                    17,850,000
    //   =============================================
    //   > LP Incentive (to Farm):  7,875,000 (44.12%)
    //   > Dev + MKT:               4,725,000 (26.47%)
    //   > In-game Treasury:        3,150,000 (17.65%)
    //   > Jackpot Treasury:        1,050,000  (5.88%)
    //   > Insurance Fund:          1,050,000  (5.88%)
    ...
    uint256 public totalJackpotFundAdded;
    ...
    function _updateRewardPerSecond() internal {
        ...
        uint256 _totalRate = devRate + gameTreasuryRate + jackpotRate + insuranceRate;
    ...
    function initialize(
        ...
        devRate = 1000; // 10%
        gameTreasuryRate = 6000; // 60%
        jackpotRate = 0; // 0%
        insuranceRate = 0; // 0%

        totalRewardPerSecond = 162037037030000000; // 0.16203703703 TOKEN/seconds
        _updateRewardPerSecond();
        ...
        rewardHalvingRate = 9000; // 90% - no halving
        halvingChecked = true;

        noStakeFee = 7000; // 70%: to burn if not staking any NFT
    ...
    function replacePoolLpToken(uint256 _pid, IERC20 _lpToken) external onlyOwner {
        ...
        require(pool.accTokenPerShare == 0 && pool.totalLpSupply == 0, "Cant replace running pool");
        massUpdatePools();
    ...
    function setJackpotRate(uint256 _jackpotRate) external onlyOwner {
    ...
    function setJackpotFund(address _jackpotFund) external onlyOwner {
        require(_jackpotFund != address(0), "zero");
        jackpotFund = _jackpotFund;
    }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            if (totalAllocPoint > 0) {
                uint256 _tokenReward = (_seconds * rewardPerSecond * pool.allocPoint) / totalAllocPoint;
                accTokenPerShare += (_tokenReward * 1e18) / lpSupply;
            }
        }
        uint256 _totalReward = (_getAccountBoostedAmount(_user, user.amount) * accTokenPerShare) / 1e18;
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _seconds = block.timestamp - pool.lastRewardTime;
            uint256 _tokenReward = (_seconds * rewardPerSecond * pool.allocPoint) / totalAllocPoint;
            pool.accTokenPerShare += (_tokenReward * 1e18) / lpSupply;
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function _harvestReward(uint256 _pid, address _account) internal {
        ...
        if (_amount > 0) {
            ...
            PoolInfo storage pool = poolInfo[_pid];
            uint256 _totalReward = (_amount * pool.accTokenPerShare) / 1e18;
            ...
            if (_claimableAmount > 0) {
                ...
                _safeTokenMint(address(this), _claimableAmount);
                ...
                _safeTokenTransfer(_account, _claimableAmount);
            }
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public notContract onlyOneBlock nonReentrant checkHalving {
        ...
        user.rewardDebt = (_getAccountBoostedAmount(msg.sender, user.amount) * pool.accTokenPerShare) / 1e18;
    ...
    function _withdraw(
        ...
        user.rewardDebt = (_getAccountBoostedAmount(_account, user.amount) * pool.accTokenPerShare) / 1e18;
    ...
    function _safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 _tokenBal = IERC20(token).balanceOf(address(this));
        if (_tokenBal > 0) {
            if (_amount > _tokenBal) {
                IERC20(token).safeTransfer(_to, _tokenBal);
            } else {
    ...
    function _safeTokenMint(address _to, uint256 _amount) internal {
        address _token = token;
        if (ICappedMintableBurnableERC20(_token).minter(address(this)) && _to != address(0)) {
            ICappedMintableBurnableERC20(_token).mint(_to, _amount);
        }
    }

    function _safeTokenBurn(uint256 _amount) internal {
        uint256 _tokenBal = IERC20(token).balanceOf(address(this));
        if (_tokenBal > 0) {
            if (_amount > _tokenBal) {
                ICappedMintableBurnableERC20(token).burn(_tokenBal);
            } else {
                ICappedMintableBurnableERC20(token).burn(_amount);
            }
        }
    }
    ...
    function _topupFunds(uint256 _claimableAmount) internal {
        ...
        uint256 _jackpotAmount = (_totalAmount * jackpotRate) / 10000;
        uint256 _insuranceAmount = (_totalAmount * insuranceRate) / 10000;
        if (
            ICappedMintableBurnableERC20(_token).minter(address(this))
        ) {
            ICappedMintableBurnableERC20(_token).mint(devFund, _devAmount);
            ICappedMintableBurnableERC20(_token).mint(gameTreasuryFund, _gameTreasuryAmount);
            ICappedMintableBurnableERC20(_token).mint(jackpotFund, _jackpotAmount);
            ICappedMintableBurnableERC20(_token).mint(insuranceFund, _insuranceAmount);
            totalDevFundAdded = totalDevFundAdded + _devAmount;
            totalGameTreasuryFundAdded = totalGameTreasuryFundAdded + _gameTreasuryAmount;
            totalJackpotFundAdded = totalJackpotFundAdded + _jackpotAmount;
    ...
    function getBoostedPercent(uint256 _tokenId) public view returns (uint256 _boosted) {
        if (_tokenId > 0) {
            (, , uint16 productivityStat, ) = ITokenMasterNFT(nft).getStats(_tokenId);
    ...
    function _massUpdatePoolsWithNftLocked(address _account, uint256 _newLockedTokenId) internal {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (_amount > 0) {
                ...
                updatePool(pid);
                user.rewardDebt = (_getBoostedAmount(_amount, _newLockedTokenId) * pool.accTokenPerShare) / 1e18;
            }
        }
    }
    ...
    function unlock(uint256 tokenId) external override nonReentrant {
        ...
        ITokenMasterNFT(nft).unlock(tokenId);
    }
    ...
    function rescueStuckErc20(address _token) external onlyTimelock {
