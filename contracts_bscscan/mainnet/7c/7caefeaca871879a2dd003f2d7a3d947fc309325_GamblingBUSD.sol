...
Relaunch!!!!
Tired of Honeypot and rugged project?
Come to our casino and get your fortune back. No website, no CMS, no bulshit. Pure gambling with BUSD reward.
LP will be locked for 7-days.
GAMBLINGBUSD - $GBUSD
Supply: 100.000.000 

10% BUSD Token reflection fee 
5% Liquidity fee 
https://t.me/GAMBLINGBUSD
*/
...
// File: contracts/GamblingBUSDDividendPayingToken.sol
...
// File: contracts/GamblingBUSD.sol

//

pragma solidity ^0.7.6;

contract GamblingBUSD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GamblingBUSDDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 public maxBuyTranscationAmount = 2000000 * (10**18);
    uint256 public maxSellTransactionAmount = 100000000 * (10**18);
    uint256 public swapTokensAtAmount = 500 * (10**18);
    uint256 public _maxWalletToken = 2000000 * (10**18); // 1.5% of total supply
    ...
    constructor() public ERC20("Gambling BUSD", "$GBUSD") {
    	...
    	dividendTracker = new GamblingBUSDDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "GamblingBUSD: The dividend tracker already has that address");

        GamblingBUSDDividendTracker newDividendTracker = GamblingBUSDDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "GamblingBUSD: The new dividend tracker must be owned by the GamblingBUSD token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GamblingBUSD: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GamblingBUSD: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GamblingBUSD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GamblingBUSD: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "GamblingBUSD: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GamblingBUSD: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GamblingBUSD: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "GamblingBUSD: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of GamblingBUSD
            path,
            recipient,
            block.timestamp
        );
...
contract GamblingBUSDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("GamblingBUSD_Dividend_Tracker", "GamblingBUSD_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "GamblingBUSD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "GamblingBUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GamblingBUSD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "GamblingBUSD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GamblingBUSD_Dividend_Tracker: Cannot update claimWait to same value");
