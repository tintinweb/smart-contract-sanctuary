...
contract MetastroCake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("MetastroCake", "METASTRO") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0x5DA35ab9DF8F51CD7A7D1749ef1972aCb28F107D;
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "MetastroCake: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "MetastroCake: The new dividend tracker must be owned by the MetastroCake token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("MetastroCake_Dividend_Tracker", "MetastroCake_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 150000 * (10**18); //must hold 150,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "MetastroCake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "MetastroCake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "MetastroCake_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "MetastroCake_Dividend_Tracker: Cannot update claimWait to same value");
