...
Infinity Moons
Main token of Infinity Moonshots team

t.me/infinitymoons
t.me/infinitymoonshots
...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x24c5B2CFd81FdEd497b3104171FA6dF2fa7557d2;
...
contract MoonsHot is Context, IERC20, Ownable { 
    ...
    address payable private Wallet_InfBurn = payable(0x000000000000000000000000000000000000dEaD); 

    address private infinityAddress = 0x93848210b98b446FA8fF3e2cDE4368A740457E23;
    ...
    uint256 private _previousFeeInfBurn = _FeeInfBurn; 
    ...
    uint256 private _promoFee = _FeeMarketing+_FeeInfBurn;
    uint256 public _FeesTotal = _FeeMarketing+_FeeInfBurn+_FeeLiquidity+_FeeReflection+_FeeBurnTokens;

    // Fee for the auto LP and the all bnb wallets - used to process fees 
    uint256 private _liquidityAndPromoFee = _FeeMarketing+_FeeInfBurn+_FeeLiquidity;
    ...
    constructor () {
        ...
        _isExcludedFromFee[Wallet_InfBurn] = true;
    ...
    function set_infinityToken_address(address _infinityAddress) public onlyOwner(){
        infinityAddress = _infinityAddress;
    }
    ...
    */
    ...
    function _set_Fees(uint256 Liquidity, uint256 Marketing, uint256 Reflection, uint256 TokenBurn, uint256 InfinityBurn) external onlyOwner() {
        ...
        // Set the fees
          ...
          _FeeInfBurn = InfinityBurn;

        // For calculations and processing 

          _promoFee = _FeeMarketing+_FeeInfBurn;
          _liquidityAndPromoFee = _FeeLiquidity+_promoFee;
          _FeesTotal = _FeeMarketing+_FeeInfBurn+_FeeLiquidity+_FeeBurnTokens+_FeeReflection;
    ...
    */
    ...
    function Wallet_Update_This_Token_Burn(address payable wallet) public onlyOwner() {
        Wallet_Token_Burn = wallet;
        _isExcludedFromFee[Wallet_Token_Burn] = true;
    }

    //Update infinityBurn Wallet  
        function Wallet_Update_InfinityToken_Burn(address payable wallet) public onlyOwner() {
        Wallet_InfBurn = wallet;
        _isExcludedFromFee[Wallet_Token_Burn] = true;
    }
    ...
    */
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tInfBurn) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tInfBurn, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tInfBurn);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tFee = calculateFeeReflection(tAmount);
        uint256 tLiquidity = calculateLiquidityAndPromoFee(tAmount);
        uint256 tInfBurn = calculateFeeInfBurnTokens(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tInfBurn);
        return (tTransferAmount, tFee, tLiquidity, tInfBurn);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tInfBurn, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rInfBurn = tInfBurn.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rInfBurn);
    ...
    function _takeInfBurn(uint256 tInfBurn) private {
        uint256 currentRate =  _getRate();
        uint256 rInfBurn = tInfBurn.mul(currentRate);
        _rOwned[Wallet_Token_Burn] = _rOwned[Wallet_Token_Burn].add(rInfBurn);
        if(_isExcluded[Wallet_Token_Burn])
            _tOwned[Wallet_Token_Burn] = _tOwned[Wallet_Token_Burn].add(tInfBurn);
    }
    ...
    function calculateFeeInfBurnTokens(uint256 _amount) private view returns (uint256) {
    ...
    function removeAllFee() private {
        if(_FeeReflection == 0 && _FeeLiquidity == 0 && _FeeMarketing == 0 && _FeeInfBurn == 0 && _FeeBurnTokens == 0) return;
        
        _previousFeeReflection = _FeeReflection;
        _previousFeeLiquidity = _FeeLiquidity;
        _previousFeeMarketing = _FeeMarketing;
        _previousFeeInfBurn = _FeeInfBurn;
    ...
    function restoreAllFee() private {
        ...
        _FeeInfBurn = _previousFeeInfBurn;
        _FeeBurnTokens = _previousFeeBurnTokens;


        _FeesTotal = _FeeMarketing+_FeeInfBurn+_FeeLiquidity+_FeeReflection+_FeeBurnTokens;
        _promoFee = _FeeMarketing+_FeeInfBurn;
        _liquidityAndPromoFee = _FeeMarketing+_FeeInfBurn+_FeeLiquidity;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
       ...
       uint256 totalBNB;
        ...
        if (_promoFee != 0 && _FeeLiquidity != 0){
            ...
            // Calculate the correct ratio splits for marketing and infinityBurn
        ...
        sendToWallet(Wallet_InfBurn, (totalBNB-marketingBNB));
    ...
    */
    ...
    function process_Purge_BNBFromContract() public onlyOwner {
        ...
        // Send BNB from infinityBurn to marketing wallet
        uint256 infinityBurnBNB = bnbAmount-marketingBNB;
        if (infinityBurnBNB > 0){sendToWallet(Wallet_Marketing, infinityBurnBNB);}
    }
   ...
   function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tInfBurn) = _getValues(tAmount);
                
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _takeInfBurn(tInfBurn);
