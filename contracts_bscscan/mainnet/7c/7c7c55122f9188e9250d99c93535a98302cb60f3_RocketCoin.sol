...
/*
WELCOME TO ROCKET, THE FUTURE OF DEFI
CREATING A DIGITAL COMMUNITY FOR THE MODERN WORLD.

Total supply: 1,000,000,000
• 47% Burned at launch
• 125 M tokens for marketing

• Starting price: Approximately $0.001

• 11% Tax - (Subject to change based on future governance decisions from the community)
• 4% Marketing
• 3% Buy Back
• 2% Reflection
• 2% Development Team

Website: https://rocketcoin.in/#
WhitePaper: https://rocketcoin.in/assets/RocketCoinWhitePaper.pdf
Facebook: https://m.facebook.com/rocketcoinbnb
Twitter: https://twitter.com/therocketcoin
Telegram: https://t.me/TheRocketCoin
Instagram: https://instagram.com/officialrocketcoin
...
interface IERC20 {
  ...
  */
   ...
   * https://github.com/ethereum/EIPs/te/th/er/bl/ack/20#issuecomment-263524729
   *
...
contract RocketCoin is Context, IERC20 {
    ...
    uint256 private _sellMarketingFee = 400;
    uint256 private _sellBuyBackFee = 300;
    uint256 private _sellEcosystemFee = 300;
    ...
    uint256 private _transferBuybackFee = (_buyBuybackFee + _sellBuyBackFee) / 2;
    uint256 private _transferEcosystemFee = (_buyEcosystemFee + _sellEcosystemFee) / 2;

    uint256 public _buyFee = _buyLiquidityFee + _buyMarketingFee + _buyBuybackFee + _buyEcosystemFee;
    uint256 public _sellFee = _sellLiquidityFee + _sellMarketingFee + _sellBuyBackFee + _sellEcosystemFee;
    uint256 public _transferFee = _transferLiquidityFee + _transferMarketingFee + _transferBuybackFee + _transferEcosystemFee;
    ...
    uint256 private _buybackRatio = (_buyBuybackFee + _sellBuyBackFee) / 2;
    uint256 private _ecosystemRatio = (_buyEcosystemFee + _sellEcosystemFee) / 2;

    uint256 constant public maxBuyTaxes = 5000;
    uint256 constant public maxSellTaxes = 9900;
    ...
    address payable private _marketingWallet = payable(0xE575608222755834b5D0E3C591E5e30eF94823A7);
    address payable private _ecosystemWallet = payable(0x7A67F9366149B710fB2Ecd4C4cd97aEb7B91A849);
    address payable private _buybackWallet = payable(0xB61bA8E60B99360D60F79e58847eA896de9d17F2);
    
    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = false;
    
    uint256 private maxTxPercent = 500;
    uint256 private maxTxDivisor = 100000;
    uint256 private _maxTxAmount = (_tTotal * maxTxPercent) / maxTxDivisor;
    uint256 private _previousMaxTxAmount = _maxTxAmount;
    uint256 public maxTxAmountUI = (startingSupply * maxTxPercent) / maxTxDivisor;

    uint256 private maxWalletPercent = 9500;
    uint256 private maxWalletDivisor = 100000;
...
//===============================================================================================================
    ...
    function setProtectionSettings(bool antiGas, bool antiBlock) external onlyOwner() {
    ...
    function setRatios(uint256 liquidity, uint256 marketing, uint256 buyBack, uint256 ecosystem) external onlyOwner {
        require (liquidity + marketing + buyBack + ecosystem == 100, "Must add up to 100%");
        if (marketing > 0 || ecosystem > 0) {
            require(marketing <= 50
                    && ecosystem <= 50
                    && (liquidity > 0 || buyBack > 0)
                    );
        }
    ...
    function setMaxTxPercent(uint256 percent, uint256 divisor) external onlyOwner {
        ...
        require(check >= (_tTotal / 100000), "Max Transaction amt must be above 0.001% of total supply.");
        _maxTxAmount = check;
    ...
    function setMaxWalletSize(uint256 percent, uint256 divisor) external onlyOwner {
        uint256 check = (_tTotal * percent) / divisor;
        require(check >= (_tTotal / 100000), "Max Wallet amt must be above 0.001% of total supply.");
    ...
    function setWallets(address payable newMarketingWallet, address payable newEcosystemWallet, address payable newBuybackWallet) external onlyOwner {
        _marketingWallet = payable(newMarketingWallet);
        _ecosystemWallet = payable(newEcosystemWallet);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 totalFee = _liquidityRatio + _marketingRatio + _buybackRatio + _ecosystemRatio;
        if (totalFee == 0)
            return;
        ...
        uint256 toEcosystem = (contractTokenBalance * _ecosystemRatio) / (totalFee);
        uint256 toBuyback = contractTokenBalance - toLiquify - toMarketing - toEcosystem;
        ...
        uint256 toSwapForEth = half + toMarketing + toBuyback + toEcosystem;
        ...
        if (toEcosystem > 0) {
            _ecosystemWallet.transfer((fromSwap * toEcosystem) / toSwapForEth);
        }
    }
    ...
    function buyback(uint256 _ethAmount) external onlyOwner {
