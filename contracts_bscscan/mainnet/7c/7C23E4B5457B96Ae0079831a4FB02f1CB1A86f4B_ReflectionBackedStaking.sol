...
import "./interfaces/IAnyflect.sol";
import "./ReflectionBackedStaking.sol";
import "./interfaces/IAnyshareOracle.sol";//Fixes anyflect shares + changes reflection for an user

contract AnyShareOracle is IAnyShareOracle, Auth {
    IAnyflect public anyflect = IAnyflect(0x8e3Ad8D73EE2439c3ce0A293e59C19563C2C56F5);

    ReflectionBackedStaking[] public stakingFeeds;
    mapping (uint => ReflectionBackedStaking) public stakingFeedsIndexed;
    uint feedLength;
    IBEP20 public shareToken; // EH

    constructor (IAnyflect _anyflect, IBEP20 _shareToken) Auth(msg.sender) {
        anyflect = _anyflect;
        shareToken = _shareToken;
    }

    function getAnyflectShares(address _address) public view override returns (uint) {
        uint a; // Unused variable, needed for unpacking tuples in efficient way
        uint aggregatedShares = 0;
        uint currentFeedAmount = 0;
        for (uint i = 0; i < feedLength; i++) {
            (currentFeedAmount, a, a) = stakingFeedsIndexed[i].shares(_address); // Only using amt
            aggregatedShares += currentFeedAmount;
        }
        return shareToken.balanceOf(_address) + aggregatedShares;
    }

    function addStakingFeed(ReflectionBackedStaking _feed) public authorized {
        stakingFeeds.push(_feed);
        stakingFeedsIndexed[feedLength] = _feed;
        feedLength++;
    }

    function addStakingFeeds(ReflectionBackedStaking[] memory _feed, uint length) external authorized {
        for (uint i = 0; i < length; i++) {
            addStakingFeed(_feed[i]);
        }
    }
