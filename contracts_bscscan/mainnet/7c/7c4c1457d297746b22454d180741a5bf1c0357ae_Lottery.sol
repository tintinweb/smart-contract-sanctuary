...
contract Lottery is LotteryOwnable, Initializable {
    ...
    function initialize(
        ...
        allocation = [50, 15, 5, 25]; //(1st prize, 2nd prize, 3rd prize, burn)
        initOwner(_owner);
        devAddress = address(0x000000000000000000000000000000000000dEaD);// edit to dead wallet for burn
    }
        ...
        if (matchingNumber > 1) {
            ...
            uint256 poolAmount =
                getTotalRewards(_issueIndex)
                    .mul(allocation[4 - matchingNumber]) // [50,20,10,20]
                    .div(100);
            reward = amount
                .mul(1e12)
