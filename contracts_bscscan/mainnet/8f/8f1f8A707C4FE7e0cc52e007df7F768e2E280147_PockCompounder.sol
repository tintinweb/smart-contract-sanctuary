...
contract PockCompounder is Ownable, ReentrancyGuard, Pausable {
    // Maximize yield in Pock Finance: https://pocketfi.io
    // Fork of AUTO StratX contract, https://bscscan.com/address/0x65168c89a16fbed4e2e418d5245ff626bd66874b#code, modified for POCK Compounding

    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public farmContractAddress; // address of Lion's den main contract
    ...
    address public lionsDenAddress; // lion's den
    address public POCKAddress; // pock token address
    ...
    address[] public earnedToPOCKPath;
    ...
    constructor(
        address _govAddress,
        address _lionsDenAddress,
        address _POCKAddress,
        address _farmContractAddress,
        uint256 _pid,
        address _wantAddress, // POCK-WBNB pancakeswap pool
        address _token0Address, // POCK (or some other token)
        address _token1Address, // WBNB
        address _earnedAddress, 
        address _uniRouterAddress // pancakeswap router
    ) public {
        govAddress = _govAddress;
        lionsDenAddress = _lionsDenAddress;
        POCKAddress = _POCKAddress;
        ...
        earnedToPOCKPath = [earnedAddress, wbnbAddress, POCKAddress];
        if (wbnbAddress == earnedAddress) {
            earnedToPOCKPath = [wbnbAddress, POCKAddress];
        }
        ...
        transferOwnership(lionsDenAddress);
    }
    ...
    function _farm() internal {
        ...
        IPancakeswapFarm(farmContractAddress).deposit(pid, wantAmt, address(0));
    }
        ...
        returns (uint256)
    {
        ...
        IERC20(wantAddress).safeTransfer(lionsDenAddress, _wantAmt);
    ...
    function enableBuyBack(bool _enable) public {        
        require(msg.sender == govAddress, "!gov");
        isBuyBackEnabled = _enable;
    }
}