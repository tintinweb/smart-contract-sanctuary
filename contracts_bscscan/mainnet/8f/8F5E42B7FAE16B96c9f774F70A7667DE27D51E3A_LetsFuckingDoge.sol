...
TG: https://t.me/LetsFucking_Doge
...
contract LetsFuckingDoge is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "LetsFuckingDoge";
    ...
    uint256 public totalFee        = marketingFee + reflectionFee + liquidityFee + communityFee;
    ...
    uint8 public cooldownTimerInterval = 18;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != communityReceiver  && recipient != autoLiquidityReceiver){
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBSPrice = amountBNB.mul(communityFee).div(totalBNBFee);
        
        if(reflectionFee>0) {try distributor.deposit{value: amountBNBReflection}() {} catch {}}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(communityReceiver).call{value: amountBNBSPrice, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _communityFee, uint256 _sellFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        communityFee = _communityFee;
        sellFee = _sellFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_communityFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/3, "Fees cannot be more than 33%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver,  address _communityReceiver) external authorized {
