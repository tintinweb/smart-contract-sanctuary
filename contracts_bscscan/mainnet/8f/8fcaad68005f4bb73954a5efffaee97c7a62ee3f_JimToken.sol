...
contract JimToken is IERC20, IERC20Metadata, Ownable {
    ...
    uint256 private MAX_SALE_AMOUNT = 5000000 * 10 ** _decimals;

    uint256 private MAX_START_BUY = 200000000 * 10 ** _decimals;

    uint256[4] public _saleFees = [5,25,30,40];
    ...
    function _transfer(
        ...
        if (isUser(sender) && isUser(recipient)) {
            ...
            if (_dailySales[sender].length == 4) {
                ...
                require(_dailySales[sender][_dailySales[sender].length-1] < block.timestamp-1200, "no delay");
