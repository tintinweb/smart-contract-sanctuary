...
contract XMASINU is Context, IERC20, Ownable {
    ...
    string private _name = "100XMAS INU";
    string private _symbol = "100XMASINU";
    ...
    mapping (address => bool) private _isExcludedFromFeesXinu;
    mapping (address => bool) private _isExcludedFromMaxBalanceXinu;

    uint256 private constant _maxFeesXinu = 12;
    uint256 private _totalFeesXinu;
    uint256 private _totalFeesXinuContract;
    uint256 private _burnFeesXinu;
    uint256 private _liquidityFeesXinu;
    uint256 private _marketingFeeXinu;
    
    uint256 private _maxBalanceWalletXinu;
    ...
    constructor () {
        ...
        _isExcludedFromFeesXinu[owner()] = true;
        _isExcludedFromFeesXinu[address(this)] = true;
        
        _isExcludedFromMaxBalanceXinu[owner()] = true;
        _isExcludedFromMaxBalanceXinu[address(this)] = true;
        _isExcludedFromMaxBalanceXinu[uniswapV2Pair] = true;

        _liquidityFeesXinu = 2;
        _marketingFeeXinu = 10;
        _burnFeesXinu = 0;
        _totalFeesXinu = _liquidityFeesXinu.add(_marketingFeeXinu).add(_burnFeesXinu);
        _totalFeesXinuContract = _liquidityFeesXinu.add(_marketingFeeXinu);

        _liquidityThreshhold = 3000 * 10**9 * 10**_decimals;
        _maxBalanceWalletXinu = 1000000 * 10**9 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFeesXinu).add(_marketingFeeXinu) <= _maxFeesXinu, "Fees are too high.");
        _liquidityFeesXinu = newLiquidityFee;
        _totalFeesXinu = _liquidityFeesXinu.add(_marketingFeeXinu).add(_burnFeesXinu);
        _totalFeesXinuContract = _liquidityFeesXinu.add(_marketingFeeXinu);
        emit SetLiquidityFee(_liquidityFeesXinu);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFeesXinu.add(_burnFeesXinu).add(newMarketingFee) <= _maxFeesXinu, "Fees are too high.");
        _marketingFeeXinu = newMarketingFee;
        _totalFeesXinu = _liquidityFeesXinu.add(_marketingFeeXinu).add(_burnFeesXinu);
        _totalFeesXinuContract = _liquidityFeesXinu.add(_marketingFeeXinu);
        emit SetMarketingFee(_marketingFeeXinu);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFeesXinu.add(newBurnFee).add(_marketingFeeXinu) <= _maxFeesXinu, "Fees are too high.");
        _burnFeesXinu = newBurnFee;
        _totalFeesXinu = _liquidityFeesXinu.add(_marketingFeeXinu).add(_burnFeesXinu);
        emit SetBurnFee(_burnFeesXinu);
    }
    ...
    function _transfer(
        ...
        // Make sure that: Balance + Buy Amount <= _maxBalanceWalletXinu
        if(
            from != owner() &&              // Not from Owner
            to != owner() &&                // Not to Owner
            !_isExcludedFromMaxBalanceXinu[to]  // is excludedFromMaxBalance
        ){
            require(
                balanceOf(to).add(amount) <= _maxBalanceWalletXinu,
        ...
        if(
            ...
            _totalFeesXinuContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromFeesXinu[from] || _isExcludedFromFeesXinu[to])
            && _totalFeesXinu > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_totalFeesXinuContract).div(100);
            uint256 toBurnAmount = amount.mul(_burnFeesXinu).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFeesXinu).div(_totalFeesXinuContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeeXinu).div(_totalFeesXinuContract);
