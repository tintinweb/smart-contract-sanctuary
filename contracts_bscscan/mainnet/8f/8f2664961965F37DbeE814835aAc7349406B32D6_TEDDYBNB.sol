...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also TEDDYBNBed.
...
// File: contracts/TEDDYBNBDividendPayingToken.sol
...
contract TEDDYBNB is ERC20, Ownable {
    using SafeMath for uint256;
	...
	address payable public MarketingWallet = payable(0xBB1c9298e493cCBE7bF6368fdfc6616309983A87); // Marketing Address

    bool private swapping;

    TEDDYBNBDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 private _FeeFactor = 2000000000 * (10**18); 
    ...
    constructor() ERC20("TEDDY BNB", "TEDDYBNB") {
        ...
        dividendTracker = new TEDDYBNBDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "TEDDYBNB: The dividend tracker already has that address");

        TEDDYBNBDividendTracker newDividendTracker = TEDDYBNBDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TEDDYBNB: The new dividend tracker must be owned by the TEDDYBNB token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TEDDYBNB: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PancakeSwapV2Pair, "TEDDYBNB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TEDDYBNB: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "TEDDYBNB: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TEDDYBNB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TEDDYBNB: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!tradingOpen) {
            require(canTransferBeforetradingOpen[from], "TEDDYBNB: This account cannot send tokens until trading is enabled");
        }
...
contract TEDDYBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("TEDDYBNB_Dividend_Tracker", "TEDDYBNB_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "TEDDYBNB_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "TEDDYBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TEDDYBNB_Dividend_Tracker: Cannot update claimWait to same value");
