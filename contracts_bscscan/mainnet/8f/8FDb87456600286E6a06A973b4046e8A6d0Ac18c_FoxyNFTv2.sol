...
specialid=1000;
   ...
   function setFoxy(address _Foxytokenaddr) public onlyOwner {
     ...
     function setConAll(uint _cons,uint _conrewd,uint _conchchance,uint _conatk,uint _condef) public onlyOwner {
...
function randomattack() internal returns (uint) {
   ...
   struct Foods {
    ...
    struct Ownerdata {
        ...
        uint256 LastClaim;
        uint crates;
        uint256 LastimeHospital;
        
    } 
    
      struct Bossdata {
        uint256 id;
        IBEP20 rewardaddress;
        uint bossreward;
        uint256 bossfee;
        uint256 bosshp;
        uint256 bossmax;
 ...
 struct Adobepets {
    ...
    mapping (address => Ownerdata) public Ownerdatamap;
    mapping (uint => Bossdata) public Bossdatamap;
    mapping(address => mapping(uint => dungeonstats)) public dungeonstatsmap;
...
event battleresult(address indexed from,uint256 diceattacker,uint256 dicetarget,string pveresult,uint256 pvereward,uint256 exp);
event battleboss(uint256 diceattacker,uint256 diceboss,string pveresult,uint256 attackhit,uint256 crates);
    ...
    function Specialadopt() public onlyMinter{
        ...
        PetView[_tokenIdTracker.current()] = Adobepets({
            ...
            PetStaminatime:uint256(block.timestamp.sub(getStaminaMaxWait())),
...
modifier BattlePaused() {
    ...
    modifier Gamepaused() {
        require(!nftgame, "Game Paused!!!");
        _;
    }
    
     modifier Adoptpaused() {
        require(!adoptedpauset, "Closed!!!");
        _;
    }
...
function AdoptPet() public Adoptpaused {
 ...
 uint256  priceautoadopt=adoptprice;    
 ...
 uint256   fordev = priceautoadopt.mul(devfee).div(10000); 
    ...
    Adobepets storage petnft = PetView[_petid];
  uint petxps=petnft.PetXP;
//Petlevel
  uint levelup = sqrt(petxps.div(cons)); 
  petnft.Petlevel=levelup;
  
  uint petlevel=petnft.Petlevel;
  uint pettype= petnft.PetType;
  uint petrarity=petnft.PetRarity;
//PetAtk&def
  uint calcatk= sqrt((petlevel.mul(pettype)).mul(petrarity).add(conatk)).mul(conatk);
  uint caldef= sqrt(petlevel.mul(pettype).mul(petrarity)).mul(condef);
//changeatk&def
petnft.PetAttack=calcatk;
petnft.PetDefence=caldef;
...
uint256 petlvl=petnft.Petlevel;
uint256 petrary=petnft.PetRarity;
uint256 petreward=sqrt(petlvl.mul(petrary));
...
function feedPet(uint _idfood,uint _petid) public Gamepaused {
 require(_isApprovedOrOwner(_msgSender(), _petid), "not owner nor approved");
      //send token here
...
emit Efeed(msg.sender,_petid,_idfood);
 ...
 function feedAll(uint256[] calldata ids,uint256 _idfood) public  {
    ...
    function KillPets(uint256 PetKill) public Gamepaused{
        
uint256 sicktime=PetView[PetKill].PetDeath; 
require(block.timestamp > Ownerdatamap[msg.sender].LastimeHospital,"1 per days");
require(block.timestamp > sicktime,"Pet Not Sick");
require(PetView[PetKill].PetHospital == 0,"Pet already on hospital");
require(tx.origin == msg.sender,"Caller Only non contract");
uint256 hospitalreward=5e9;

PetView[PetKill].PetHospitaldate=block.timestamp;
PetView[PetKill].PetHospital=1;
Ownerdatamap[msg.sender].LastimeHospital=block.timestamp+86400;
FoxyToken.transfer(msg.sender,hospitalreward);
 
    }
    
function Releasesickpet(uint _petid) public Gamepaused{
 require(_isApprovedOrOwner(_msgSender(), _petid), "not owner nor approved");
 
uint256 maxcost=Hospitalcost(_petid);


FoxyToken.safeTransferFrom(msg.sender, burnaddr,maxcost);

PetView[_petid].PetDeath=block.timestamp+172800;
PetView[_petid].PetHospital=0;  

    }

function Hospitalcost(uint _petid) public view returns(uint256){
 require(_isApprovedOrOwner(_msgSender(), _petid), "not owner nor approved");
  Adobepets storage petnft = PetView[_petid];
uint256 hospitalcostpaid=0;
uint256 hospitalcost=petnft.PetRarity.mul(10e9);
uint256 secondcost=hospitalcost.div(86400);
uint256 hospitalrent=block.timestamp.sub(petnft.PetHospitaldate);
uint256 costrent=hospitalrent.mul(secondcost);
uint256 maxcost=costrent.mul(604800);

if(petnft.PetHospital ==1){

if(block.timestamp <= petnft.PetHospitaldate+86400){

  hospitalcostpaid=hospitalcost;
    
}else if(block.timestamp < petnft.PetHospitaldate+604800){
    
   
      hospitalcostpaid=costrent;
}else{
    
  
    hospitalcostpaid=maxcost;
}
}
 return(hospitalcostpaid);
  }
    
function PVEchance(uint _petid) public view returns(uint256){
          Adobepets storage petnft = PetView[_petid];
         uint petAtk=petnft.PetAttack;
         uint petDef =petnft.PetDefence;
        
       
         uint petchancewin=petAtk.add(petDef).mul(conchance);
    ...
    function PVEBossbattle(uint256[] calldata ids,uint256 _bossid) public  Gamepaused BattlePaused {
       require(tx.origin == msg.sender,"Caller Only non contract");
       
        Bossdata storage boss = Bossdatamap[_bossid];
        dungeonstats storage playerfee = dungeonstatsmap[msg.sender][_bossid];
        Ownerdata storage playerdata = Ownerdatamap[msg.sender];
        
        uint256 bosshp=boss.bosshp;
        uint256 totaldame=playerfee.totaldamage;
       
          require(bosshp >0 ,"Boss Die!!");
          require(PetView[ids[0]].PetType == 1,"Need Type Novice");
          require(PetView[ids[1]].PetType == 2,"Need Type Mage");
          require(PetView[ids[2]].PetType == 3,"Need Type Assasin");
          require(PetView[ids[3]].PetType == 4,"Need Type Berserker");
           FoxyToken.safeTransferFrom(msg.sender, burnaddr,boss.bossfee);
          
       uint256 allpetattack; 
       uint256 allpetpower; 
             for (uint256 i = 0; i < ids.length; i++) {
         
          require(_isApprovedOrOwner(_msgSender(), ids[i]), "not owner nor approved");
          require(PetView[ids[i]].PetHospital != 1,"Sick Pet Can't Battle");
           
          
         uint256 staminaPoints = getStaminaPointsFromTimestamp(PetView[ids[i]].PetStaminatime);
        require(staminaPoints >= 100, "Not enough stamina!");

        uint256 drainTime = uint256(100 * staminaregen);
        if(staminaPoints >= staminamax) { 
            PetView[ids[i]].PetStaminatime = uint256(block.timestamp - getStaminaMaxWait() + drainTime);
        }
        else {
            PetView[ids[i]].PetStaminatime = uint256(PetView[ids[i]].PetStaminatime  + drainTime);
        }
        
         allpetattack+= PetView[ids[i]].PetAttack;
         allpetpower+= PVEchance(ids[i]);
         
             }
    
     uint playerbossdice=diceplayer(allpetpower);
     uint diceboss=diceplayer(boss.bosspower);
       string memory pveresult;
         uint256 cratesget=0;
        
        if(playerbossdice>diceboss){
             pveresult="win";
            
              uint256 chance=randomIntl();
                 if(chance<=5){
                   uint totalcrates=playerdata.crates;
                cratesget=1;
                 playerdata.crates=totalcrates.add(1);
                  
                 } 
            
            if(allpetattack >= bosshp){
            
           playerfee.totaldamage=totaldame.add(bosshp);
            boss.bosshp=0;
           ...
           boss.bosshp=bosshp.sub(allpetattack);
           playerfee.totaldamage=totaldame.add(allpetattack);
            }
       ...
       emit battleboss(playerbossdice,diceboss,pveresult,allpetattack,cratesget);
       
      
      
    }
    
    function PVEbattle(uint _petid,uint multi) public  Gamepaused BattlePaused {
        require(_isApprovedOrOwner(_msgSender(), _petid), "not owner nor approved");
         require(tx.origin == msg.sender,"Caller Only non contract");
         require(PetView[_petid].PetHospital != 1,"Sick Pet Can't Battle");
          Adobepets storage petnft = PetView[_petid];
          Ownerdata storage player = Ownerdatamap[msg.sender];
        uint256 playerbalance=player.RewardBalance;
        
        uint256 staminaPoints = getStaminaPointsFromTimestamp(petnft.PetStaminatime);
        require(staminaPoints >= multi.mul(40), "Not enough stamina!");

        uint256 drainTime = uint256(multi.mul(40) * staminaregen);
        if(staminaPoints >= staminamax) { 
            petnft.PetStaminatime = uint256(block.timestamp - getStaminaMaxWait() + drainTime);
        }
        else {
            petnft.PetStaminatime = uint256(petnft.PetStaminatime  + drainTime);
        }
      
       uint botdice=dicebot();
       uint dicepet=diceplayer(PVEchance(_petid));
       string memory pveresult;
        uint256 multireward;
       uint256 xpbonus= PetView[_petid].PetType.mul(10*multi);
...
uint256 pvereward=CalcReward(_petid);
           pveresult="win";
     multireward=multi.mul(pvereward);       
         player.RewardBalance=playerbalance.add(multireward);
       ...
       emit battleresult(msg.sender,dicepet,botdice,pveresult,multireward,xpbonus);
    }



function ClaimReward() public Gamepaused{
     Ownerdata storage playerdata = Ownerdatamap[msg.sender];
     require(playerdata.RewardBalance > 0,"Your Reward is ZERO");
      
      
      if(block.timestamp < playerdata.LastClaim){
          
        uint256 allreward = playerdata.RewardBalance;
     uint256 taxreward =allreward.mul(2000).div(10000);
     uint256 rewardclaim=allreward.sub(taxreward);
     
     FoxyToken.transfer(msg.sender,rewardclaim);
     playerdata.RewardBalance=0;
    // playerdata.LastClaim=block.timestamp+604800;
     
    emit EClaim(msg.sender,rewardclaim);
          
      }else{
          
          uint256 rewardclaim = playerdata.RewardBalance;
     
     FoxyToken.transfer(msg.sender,rewardclaim);
     playerdata.RewardBalance=0;
     playerdata.LastClaim=block.timestamp+604800;
     
     EClaim(msg.sender,rewardclaim);
...
function Createboss(uint _bossimg,IBEP20 _tokenaddr,uint256 _bossreward,uint _bossfee,uint _bosshp,uint _bosspower) public onlyMinter {
         Bossdatamap[bossid] = Bossdata({
            id: bossid,
            rewardaddress: _tokenaddr,
            bossreward:_bossreward,
            bossfee:_bossfee,
            bosshp:_bosshp,
            bossmax:_bosshp, 
...
function claimbossreward(uint256 _bossid) public Gamepaused {
 Bossdata storage boss = Bossdatamap[_bossid];
dungeonstats storage playerfee = dungeonstatsmap[msg.sender][_bossid];
require(playerfee.rewardclaim==false,"Reward already Claim");
require(playerfee.totaldamage > 0,"No Damage");
require(boss.bosshp ==0 ,"Kill boss first!!");

uint256 totaldamage= playerfee.totaldamage;
uint256 bosshp= boss.bossmax;
uint256 percentreward= totaldamage.mul(10000).div(bosshp);
uint256 totalreward= boss.bossreward;
uint256 rewardbossclaim= percentreward.mul(totalreward).div(10000);
 
boss.rewardaddress.transfer(msg.sender,rewardbossclaim);

playerfee.rewardclaim=true;
...
uint256 public staminaregen;
function setregensecond(uint256 _staminaregen) public onlyOwner{
    
     staminaregen=_staminaregen; 
    

}
uint256 public staminamax;
function setStaminamax(uint256 _staminamax) public onlyOwner{
 ...
 function getStaminaTimestamp(uint256 _petid) public view  returns (uint256) {
        return PetView[_petid].PetStaminatime;
    }

    function setStaminaTimestamp(uint256 _petid, uint256 timestamp) public onlyOwner {
        PetView[_petid].PetStaminatime = timestamp;
    }

    function getStaminaPoints(uint256 _petid) public view  returns (uint256) {
        return getStaminaPointsFromTimestamp(PetView[_petid].PetStaminatime);
    }

    function getStaminaPointsFromTimestamp(uint256 timestamp) public view returns (uint256) {
        if(timestamp  > block.timestamp)
            return 0;

        uint256 points = (block.timestamp - timestamp) / staminaregen;
        if(points > staminamax) {
            points = staminamax;
        }
        return uint8(points);
    }
    ...
    function isStaminaFull(uint256 id) public view returns (bool) {
        return getStaminaPoints(id) >= staminamax;
    }

    function getStaminaMaxWait() public view returns (uint256) {
        return uint256(staminamax * staminaregen);
    }
...
function Breeding(uint256 parentid1,uint256 parentid2) public {
   ...
   require(_isApprovedOrOwner(_msgSender(), parentid2), "BEP721: Breed caller is not owner nor approved");
    ...
    uint256 intelparient1=PetView[parentid1].PetType;
    uint256 intelparient2=PetView[parentid2].PetType;
...
function battleready(uint256 _filterlvl,uint256 _filterint,uint256 _filterary) public view returns(uint256[] memory ) {
   ...
   uint256 _tokenCount = LastPets();
    ...
    for (uint256 i = 1; i < _tokenCount; i++) {
        ...
        if(PetView[i].Petlevel ==_filterlvl && PetView[i].PetType == _filterint  && PetView[i].PetRarity  == _filterary){
...
function sickpet(uint256 _hospitalpet) public view returns(uint256[] memory ) {
   ...
   uint256 _tokenCount = LastPets();
    ...
    for (uint256 i = 1; i < _tokenCount; i++) {
            ...
            if(block.timestamp > PetView[i].PetDeath   && PetView[i].PetHospital == _hospitalpet && PetView[i].id != 0 ){
