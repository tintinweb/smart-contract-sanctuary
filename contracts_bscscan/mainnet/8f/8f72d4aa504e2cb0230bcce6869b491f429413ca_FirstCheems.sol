...
https://t.me/RoadToOneDollar

Daily Degen Play by @DetectiveCalls

Goal is to get the token to $1.00. We'll use everything in our arsenal to get there. We don't GIVE A F*CK!
...
contract DividendDistributor is IDividendDistributor {
    ...
    // EARN
    IBEP20 CAKE = IBEP20(0xF69897644694BC36A4Ee03779f04142510141040);
...
/* Token contract */
contract FirstCheems is IBEP20, Auth {
    using SafeMath for uint256;

    // Addresses
    address CAKE = 0xF69897644694BC36A4Ee03779f04142510141040;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address DEV = 0x940C9A0C69A85e9f79214198506dB6Eb00D70011;
    ...
    // Name and symbol
    string private _name = "FirstCheems";
    string private _symbol = "FirstCheems";
    uint8 constant _decimals = 18;

    // Total supply
    uint256 _totalSupply = 100000 * (10 ** _decimals); // 100m

    // Max wallet and TX
    uint256 public _maxBuyTxAmount = _totalSupply * 20 / 10000; // 1.0% on launch or 1M tokens
    uint256 public _maxSellTxAmount = _totalSupply * 20 / 10000; // 0.20% or 250k tokens
    uint256 public _maxWalletToken = _totalSupply * 250 / 10000; // 3% or 3T tokens
    ...
    uint256 marketingFeeBuy = 600;
    uint256 devFeeBuy = 200;
    uint256 totalFeeBuy = 1200;
    ...
    uint256 discountFactor = 2;  // 2 means all fees are divided by 2.
    ...
    // Dead blocks
    uint256 deadBlocks = 500;

    // Perks
    bool public initialLiquidityAdded;  // if the initial liquidity is added
    uint256 public liquidityAddedBlock;
    uint256 public earlybuyBlocks = 0;  // for $APL holders to buy early
    uint256 public holdingThreshold = 30;  // 1 basis point, i.e., 1 is 0.01%

    // Sell amount of tokens when a sell takes place
    uint256 public swapThreshold = _totalSupply * 5 / 10000; // 0.1% of supply
    ...
    constructor () Auth(msg.sender) {
        ...
        apl = IBEP20(0xF69897644694BC36A4Ee03779f04142510141040);  // APL contract
    ...
    function setHoldingThreshold(uint256 threshold_) external onlyOwner {
        require(threshold_ < 100, "Exceeding limit");
        holdingThreshold = threshold_;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!authorizations[sender] && !authorizations[recipient]){
            ...
            if (!_hasDiscount(sender) && !_hasDiscount(recipient)) {
                require(!_inEarlyBuy(), "Not allowed in early buy");
            }
        }
        ...
        _setEarlyBuy(isSell);  // Set the starting block from adding liquidity.
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, recipient, amount, isSell, GREEDMode) : amount;
    ...
    // Set early buy limit
    function _setEarlyBuy(bool isSell) private {
    ...
    function _inEarlyBuy() private view returns (bool) {
        return block.number <= liquidityAddedBlock + earlybuyBlocks;
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount, bool isSell, bool GREEDMode) internal returns (uint256) {
        uint256 feeAmount;

        uint256 den;

        if (isSell) {
            den = _hasDiscount(sender) ? discountFactor : 1;
        } else {
            den = _hasDiscount(recipient) ? discountFactor : 1;
        }
        ...
        if (GREEDMode){
            if(isSell){
                ...
                feeAmount = amount.mul(totalFee).mul(3).div(2).div(den).div(feeDenominator);
            } else {
                // We are buying so cut our taxes in half
                feeAmount = amount.mul(totalFee).div(2).div(den).div(feeDenominator);
            }
        } else {
            feeAmount = amount.mul(totalFee).div(den).div(feeDenominator);
        }
    ...
    function hasDiscount(address account) public view returns (bool) {
        return _hasDiscount(account);
    }

    function _hasDiscount(address sender) internal view returns (bool) {
        return apl.balanceOf(sender) >= apl.totalSupply() * holdingThreshold / 10000;
    }
