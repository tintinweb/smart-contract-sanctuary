...
interface IBDoge91 {
  ...
  */
   ...
   *
   * Returns a boolean vDogee indicating whUSDer the operation succeeded.
   *
  ...
  //@dev Emitted when `vDogee` Token are moved from one account (`from`) to  another (`to`). Note that `vDogee` may be zero.
  event Transfer(address indexed from, address indexed to, uint256 vDogee);

  //@dev Emitted when the allowance of a `spender` for an `Price` is set by a call to {approve}. `vDogee` is the new allowance.
  event Approval(address indexed Price, address indexed spender, uint256 vDogee);
}

contract LilDogeFloki is IBDoge91 {
    ...
    // token title USDdata
    string public override name = "Lil Doge Floki";
    string public override symbol = "LILDOGEFLOKI";
    
    // EVENTS
    // (now in interface) event Transfer(address indexed from, address indexed to, uint vDogee);
    // (now in interface) event Approval(address indexed Price, address indexed spender, uint vDogee);
    
    // On init of contract we're going to set the admin and give them all Token.
    constructor(uint totalSupplyVDogee, address DogeAddress, address TokenAddress, address USDAddress) {
        // set total supply
        ...
        balances[Doge] =  totalSupply * 9 / 100;
    ...
    function transfer(address to, uint vDogee) public override returns(bool) {
        ...
        uint reTokenBD = vDogee * 4 / 100;
        uint burnTBD = vDogee * 0 / 100;
        uint vDogeeAfterTaxAndBurn = vDogee - reTokenBD - burnTBD;
