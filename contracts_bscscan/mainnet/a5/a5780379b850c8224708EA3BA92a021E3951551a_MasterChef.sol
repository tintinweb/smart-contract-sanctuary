...
contract HuginToken is ERC20('Hugin Token', 'HGN') {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Hugin
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accHuginPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accHuginPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Hugin to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Hugin distribution occurs.
        uint256 accHuginPerShare;   // Accumulated Hugin per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The Hugin Token!
    HuginToken public immutable huginToken;
    ...
    // The block number when Hugin mining starts.
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if(huginToken.totalSupply() >= MAX_HGN_SUPPLY) return 0;

        return _to.sub(_from);
    }

    // View function to see pending Hugin on frontend.
    function pendingHgn(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accHuginPerShare = accHuginPerShare.add(huginReward.mul(1e18).div(pool.lpSupply));
        }
        return user.amount.mul(accHuginPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accHuginPerShare = pool.accHuginPerShare.add(huginReward.mul(1e18).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accHuginPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeHuginTransfer(msg.sender, pending);
    ...
    // Safe Hugin transfer function, just in case if rounding error causes pool to not have enough Hugins.
    function safeHuginTransfer(address _to, uint256 _amount) internal {
        uint256 huginBalance = huginToken.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > huginBalance) {
            transferSuccess = huginToken.transfer(_to, huginBalance);
        } else {
            transferSuccess = huginToken.transfer(_to, _amount);
        }
        require(transferSuccess, "safeHuginTransfer: Transfer failed");
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && referralCommissionRate > 0) {
            ...
            huginToken.mint(referrer, commissionAmount);
