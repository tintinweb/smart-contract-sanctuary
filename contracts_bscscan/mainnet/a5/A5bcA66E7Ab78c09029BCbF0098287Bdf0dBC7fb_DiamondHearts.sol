...
contract DiamondHearts {
    ...
    uint private _affiliateBonus = 10; //Reward is divided by this amount, i.e. 1==100%, 10==10%, 100=1%
    ...
    */
    ...
    function createStake(StakeType stakeType, uint amount, address affiliate) public returns (bool) {
        require(amount >= 1000 * 10**8, "Minimum stake of 1000 Lovecoin.");
        _tokenContract.transferFrom(msg.sender, address(this), amount);
        Stake memory stake = Stake(stakeType, false, block.timestamp, amount, _interestRates[uint(stakeType)], affiliate);
    ...
    function claimStake(uint stakeID) public returns (uint) {
        ...
        uint affiliateReward = reward/_affiliateBonus;
        if (stake.affiliate == address(0)) affiliateReward = 0;
        bool rewardsWithdrawn = false;

        if (reward + (affiliateReward*2) <= rewardsPool) {
            rewardsPool -= reward + (affiliateReward * 2);
            if (affiliateReward > 0) _tokenContract.transfer(stake.affiliate, affiliateReward);
