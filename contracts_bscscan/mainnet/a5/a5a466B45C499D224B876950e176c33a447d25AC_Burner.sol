...
/*
 *
 * Burner - The token that loves to BURN!
 * Burner is here for the people for real! 100% Safe - check the contract :)
 * Burner is here  TO GIVE LOVE FOR THE PEOPLE OF THE WORLD! <3
 *
 * Buying - 0% tax
 * Selling - 15%  tax
 *
 * On Selling -
 * Auto Liqudity - 100% Goes to Burn to the Dead Address address(0xdead)!
 * Auto Burn
 * Auto Holders Reflection
 *
 * Total Supply: 100,000,000,000,000
 * Max Buy: (1.1% of Total Supply)
 * Max Hold: (5.1% of Total Supply)
 *
 *
 */
...
contract Burner is Context, IBEP20, Ownable {
    ...
    uint256 public _maxTxAmount = 2.1 * 10**12 * 10**18; // Max Transaction: 1.5 Trillion (1.5%)
    uint256 public _numTokensSellToAddToLiquidity = 200 * 10**9 * 10**18; // 200 Billion
    uint256 public _maxWalletToken = 5.5 * 10**12 * 10**18; // Max Wallet: 5.1 Trillion (5.1%)

    // Fees
    uint256 public _maxTax = 15 * 10**2; // 1500 = 15.00%
    uint256 public _burnFee = 2 * 10**2; // 200 = 2.00%
    uint256 public _liquidityFee = 7 * 10**2; // 700 = 7.00%
    uint256 public _holderFee = 6 * 10**2; // 600 = 6.00%
    ...
    // This section allows adjusting tokenomics

    function _getRouterAddress() public view returns (address) {
    ...
    function setBurnFee(uint256 newBurnFee) public onlyOwner {
        uint256 newTotal = newBurnFee + _liquidityFee + _holderFee;
        require(newTotal <= _maxTax, "Max Tax is 15%");
        _burnFee = newBurnFee;
    }

    /*
     * newLiquidityFee: 700 = 7.00%
     */
    function setLiquidityFee(uint256 newLiquidityFee) public onlyOwner {
        uint256 newTotal = newLiquidityFee + _burnFee + _holderFee;
        require(newTotal <= _maxTax, "Max Tax is 15%");
        _liquidityFee = newLiquidityFee;
    }

    /*
     * newHolderFee: 500 = 5.00%
     */
    function setHolderFee(uint256 newHolderFee) public onlyOwner {
        uint256 newTotal = newHolderFee + _burnFee + _liquidityFee;
        require(newTotal <= _maxTax, "Max Tax is 15%");
        _holderFee = newHolderFee;
    }

    /*
     * newNum: 200 * 10**9 * 10**18 = 200 Billion
     */
    ...
    function _burn(uint256 tBurn) public onlyOwner {
        require(balanceOf(owner()) >= tBurn, "Not enough balance");
        _rOwned[address(owner())] = _rOwned[address(owner())].sub(tBurn);
        _rOwned[address(0xdead)] = _rOwned[address(0xdead)].add(tBurn);

        _tTotal = _tTotal.sub(tBurn);
        _rTotal = _rTotal.sub(tBurn);
        emit Transfer(owner(), address(0xdead), tBurn);
    }
        ...
        )
    {
        ...
        uint256 tLiquidity = tAmount.div(10**2).mul(_getCurrentLiqFee()).div(
            10**2
        );
        uint256 tFee = tAmount.div(10**2).mul(_getCurrentHolderFee()).div(
            10**2
        );
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 q = contractTokenBalance.div(4);
        uint256 afterQ = contractTokenBalance.sub(q);
        uint256 half = afterQ.div(2);
