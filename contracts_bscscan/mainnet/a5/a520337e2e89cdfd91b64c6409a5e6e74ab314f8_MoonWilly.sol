...
// File: contracts/MoonWillyDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable DAIToken = address(0x1AF3F329e8BE154074D8769D1FFa4eE058B1DBc3);
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      bool success = IERC20(DAIToken).transfer(user, _withdrawableDividend);
...
contract MoonWilly is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable DAIToken = address(0x1AF3F329e8BE154074D8769D1FFa4eE058B1DBc3);
	address public constant Treasury = 0x0331AcC9120d88aeF48F251b29BbA1Bc00764f0e;	
    bool private swapping;

    MoonWillyDividendTracker public dividendTracker;
    ...
    bool public AntiSniperBotEnabled = true;
    ...
    constructor() ERC20("MoonWilly", "MOONWILLY") {
        ...
        dividendTracker = new MoonWillyDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MoonWilly: The dividend tracker already has that address");

        MoonWillyDividendTracker newDividendTracker = MoonWillyDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MoonWilly: The new dividend tracker must be owned by the MoonWilly token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MoonWilly: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MoonWilly: Account is already the value of 'excluded'");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "MoonWilly: This account cannot send tokens until trading is enabled");
        }
...
contract MoonWillyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("MoonWilly_Dividend_Tracker", "MoonWilly_Dividend_Tracker") {
        claimWait = 3600;
