...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    */
     ...
     * e.g. set automatic allowances for certain subappleems, etc.
     *
     * Emits an {Approval} event.
     *
     * Requirements:
...
contract AppleFarm is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock;    //APPLE Last block number that APPLEs distribution occurs.
    ...
    function safeAppleTransfer(address _to, uint256 _amount) internal {
        ...
        uint256 amount= _amount.div(1000000000);
        if (amount > appleBal) {
            apple.transfer(_to, appleBal);
        } else {
            apple.transfer(_to, amount);
        }
    ...
    function updateEmissionRate(uint256 _applePerBlock) public onlyOwner {
        massUpdatePools();
        applePerBlock = _applePerBlock;
        emit UpdateEmissionRate(msg.sender, _applePerBlock);
    }
}