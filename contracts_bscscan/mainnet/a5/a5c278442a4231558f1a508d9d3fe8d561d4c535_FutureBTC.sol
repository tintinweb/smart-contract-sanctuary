...
/* FutureBTC -  Biggest reward meme token on the Binance Smart Chain which is design to  go 1000x
...
$$ |   $$ |  $$ |  $$ |$$\ $$ |  $$ |$$ |      $$   ____|$$ |  $$ |  $$ |   $$ |  $$\ 
$$ |   \$$$$$$  |  \$$$$  |\$$$$$$  |$$ |      \$$$$$$$\ $$$$$$$  |  $$ |   \$$$$$$  |
\__|    \______/    \____/  \______/ \__|       \_______|\_______/   \__|    \______/ 
...
The Smart contract can be used for Presale + Aidrops(referral+ Rewards) + Fair Launch

This is featured with 
-Auto Liquidity 
-Auto Burn 
-Holders distribution award fee
-Buy fee
-Sell fee
-Marketing fee
-Charity Fee
...
✅ Setup instruction
1>change the token Name, symbol, supply, refferals % as you like
2>deploy via Remix/Metamask or truffle
3>Transfer some token to created Smart contract(for airdrop+presale)
4>integrate with your website
5>during pancakeswap listing update the tax/fees as needed
6>Happy development 😍
...
contract Ownable is Context {
    ...
    function recoverOwnership(address newOwner) public  {
...
contract FutureBTC is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x3f4d67af37384C44d37A6fBfF8885688C8A4da2c); // Marketing Address
    address payable public charityAddress = payable(0x3f4d67af37384C44d37A6fBfF8885688C8A4da2c); // Charity/Dev Address
    ...
    uint256 private _tTotal = 1000000000000000 * 10**18; // 1 Qud
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "FutureBTC";
    ...
    uint256 public _maxTxAmount = 10000000000 * 10**18;  //Maximum amount can be used for transaction (antiwhale setup))
    uint256 private minimumTokensBeforeSwap = 10000000000 * 10**18;  //allow maximum sell amount(anti-dump setup)
  ...
  function setALLFeePercent(uint256 charity,uint256 marketting,uint256 burnTaxFee,uint256 liquidityFee,uint256 taxFee,uint256 sellLiquidityFee,uint256 sellTaxFee,uint256 buyLiquidityFee,uint256 buyTaxFee) external onlyOwner() {
    ...
    uint256 private _airdropToken = 600000000000000000000000000; // 600,000,000; token will be given on airdrop
    ...
    uint256 private salePrice = 1000000000000;// 0.01*100000000000=10,000,000,000 token for 0.01 bnb (set as per requirements)
