...
contract EarnUSDC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    earnusdcDividendTracker public dividendTracker;

    address public earnusdcWallet;

    uint256 public maxBuyTranscationAmount = 30 * (10**18);
    uint256 public maxSellTransactionAmount = 10 * (10**18);
    uint256 public swapTokensAtAmount = 30 * (10**18);
    uint256 public maxWalletToken = 30 * (10**18); // 1.5% of total supply

    uint256 public dividendRewardsFee;
    uint256 public earnusdcFee;
    ...
    constructor() ERC20("BabyBishokuInuUSDC", "BBIUSDC") {
        uint256 _dividendRewardsFee = 12;
        uint256 _earnusdcFee = 1;

        dividendRewardsFee = _dividendRewardsFee;
        earnusdcFee = _earnusdcFee;
        totalFees = _dividendRewardsFee.add(_earnusdcFee);

        dividendTracker = new earnusdcDividendTracker();

        earnusdcWallet = 0xD35F471Be7347210110fb88c8b379eEa2332479b;
        ...
        excludeFromFees(earnusdcWallet, true);
        ...
        onlyOwner
    {
        require(
            ...
            "earnusdc: Sell transaction multipler must be between 100 (1x) and 200 (2x)"
        );
    ...
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(
            ...
            "earnusdc: Marketing divisor must be between 0 (0%) and 100 (100%)"
        );
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(
            !swapping,
            "earnusdc: A swapping process is currently running, wait till that is complete"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "earnusdc: The dividend tracker already has that address"
        );

        earnusdcDividendTracker newDividendTracker = earnusdcDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "earnusdc: The new dividend tracker must be owned by the FLOKIBUSD token contract"
        );
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "earnusdc: Dividend reward tax must be between 0 and 10"
        );
        dividendRewardsFee = newFee;
    }

    function updateearnusdcFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "earnusdc: Dividend reward tax must be between 0 and 10"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "earnusdc: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "earnusdc: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "earnusdc: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "earnusdc: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateearnusdcWallet(address newearnusdcWallet) public onlyOwner {
        require(
            newearnusdcWallet != earnusdcWallet,
            "earnusdc: The liquidity wallet is already this address"
        );
        excludeFromFees(newearnusdcWallet, true);
        earnusdcWallet = newearnusdcWallet;
        emit earnusdcWalletUpdated(newearnusdcWallet, earnusdcWallet);
    }

    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            newValue >= 200000 && newValue <= 500000,
            "earnusdc: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "earnusdc: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[from]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 swapTokens = contractTokenBalance.mul(earnusdcFee).div(
                    totalFees
                );
                swapTokensForBNB(swapTokens);
                transferToearnusdcWallet(
                    payable(earnusdcWallet),
    ...
    function transferToearnusdcWallet(address payable recipient, uint256 amount)
        private
    {
...
contract earnusdcDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "earnusdc_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "earnusdc_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main earnusdc contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "earnusdc_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "earnusdc_Dividend_Tracker: Cannot update claimWait to same value"
        );
