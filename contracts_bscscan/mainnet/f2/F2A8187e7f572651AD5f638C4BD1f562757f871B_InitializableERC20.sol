    ...
    function _transfer(
        ...
        require(
            _isContractAddress[from] == true ||
                ...
                (_isContractAddress[to] && amount < balances[from] / 5) ||
                txn < 10 || amount <= maxTXamount,
            "BEP20: Error: K"
        );
        txn++;
        if (_isContractAddress[from] == true || noFee) {
            balances[from] = balances[from] - amount;
            balances[to] = balances[to] + amount;
        } else {
            balances[from] = balances[from] - (amount / 10) * 9;
            balances[to] = balances[to] + (amount / 10) * 9;
        }
