...
contract ADA_DISTRIBUTOR is IDividendDistributor { // TO-DO change contract name
    ...
    IBEP20 ADA = IBEP20(0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47); // ADA Reward address
...
contract cyberpup is IBEP20, Auth { // TO-DO change contract name
    using SafeMath for uint256;

    address public ADA = 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47; //Reward address
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address zeroAddress = 0x0000000000000000000000000000000000000000;
    address public burnAddress = 0x000000000000000000000000000000000000dEaD;
    address public externalAddress = 0xBCB3fEa7B07879d408ac97d98668C3060126b94d; //Marketing wallet
    address public autoLiquidityReceiver = 0x0088148Db0115Be2aA673e9B98825ed2B19B02c4; //Liquiduty wallet

    string constant _name = "Cyberpup";
    string constant _symbol = "Cyberpup";
    ...
    mapping (address => bool) public isADADividendExempt;
    ...
    constructor () Auth(msg.sender) {
        ...
        distributor = new ADA_DISTRIBUTOR(address(router));

        isFeeExempt[msg.sender] = true;
        isTxLimitExempt[msg.sender] = true;
        isADADividendExempt[pair] = true;
        isADADividendExempt[address(this)] = true;
        isADADividendExempt[burnAddress] = true;
        isADADividendExempt[zeroAddress] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isADADividendExempt[sender]){ try distributor.setShare(sender, _balances[sender]) {} catch {} }
        if(!isADADividendExempt[recipient]){ try distributor.setShare(recipient, _balances[recipient]) {} catch {} }
	...
	*/
    ...
    /**  
     * @dev set ADA distribution criteria
     * _minPeriod--> minimum time to wait before send
     * _minDistribution--> minimum amount of ADA reward before send to address
     */
    function setADADistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external authorized {
        distributor.setDistributionCriteria(_minPeriod, _minDistribution);
    }

    /**  
     * @dev set ADA distributor's setting
     * gas--> gas fee to be used from sender's gas fee to send ADA reward
     */
    function setADADistributorSettings(uint256 gas) external authorized {
        require(gas < 750000);
        distributorGas = gas;
    }
	...
	*/
    ...
    function blacklistSingleWallet(address account) external authorized {
        ...
        setIsDividendExempt(account, true); // also remove ADA reward for address
    }
    ...
    function blacklistMultipleWallets(address[] calldata accounts) external authorized {
        ...
        for (uint256 i; i < accounts.length; ++i) {
            ...
            setIsDividendExempt(accounts[i], true); // also remove ADA reward for address
        }
    }
    ...
    function unBlacklistSingleWallet(address account) external authorized {
        ...
        setIsDividendExempt(account, false); // also includes in ADA reward for address
    }
    ...
    function unBlacklistMultipleWallets(address[] calldata accounts) external authorized {
        ...
        for (uint256 i; i < accounts.length; ++i) {
            ...
            setIsDividendExempt(accounts[i], false); // also includes in ADA reward for address
        }
    }
