...
// MasterChef is the master of TestCake Token (FLUX). He can make FLUX and he is a fair guy.
//
...
//     `\<<<<</'                            `\>>>>>/'
    ...
    struct PoolInfo {
        ...
        uint256 accTestPerShare; // Accumulated FLUXs per share, times 1e12. See below.
    ...
    // The swap router, modifiable. Will be changed to Test's router when our own AMM release
    IUniswapV2Router02 public testRouter;
    ...
    uint256 public constant INITIAL_HARVEST_TIME = 1 hours;
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 testReward = multiplier.mul(testPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accTestPerShare = accTestPerShare.add(
                testReward.mul(1e12).div(lpSupply)
            );
        }
        uint256 pending = user.amount.mul(accTestPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 testReward =
            multiplier.mul(testPerBlock).mul(pool.allocPoint).div(
                totalAllocPoint
            );
    ...
    function payOrLockupPendingTest(uint256 _pid) internal {
        ...
        if (block.timestamp >= startTimeHarvest && block.timestamp <= lastTimeHarvest) {
            ...
            if (pending > 0 || user.rewardLockedUp > 0) {        
                ...
                test.mint(msg.sender, totalRewards);
    ...
    // Safe test transfer function, just in case if rounding error causes pool to not have enough FLUXs.
    ...
    function updateEmissionRate(uint256 _testPerBlock) external onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, testPerBlock, _testPerBlock);
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            test.mint(referrer, refBonusEarned);
...
contract TestToken is BEP20 {
    ...
    // Transfer tax 7.5%, capped at 7.5%
    // Antiwhale is at 5% at start, lower bound 0.3%
    // Router is initially set to address(0) and is then swappable for the PCS one,
    // but nothing else - we can't prevent holders to swap
    // Please ctrl+f updatetestRouter() to verify it yourself

    // Owner, which is the Masterchef, is excluded from transaction fees (send and receive)
    // This prevents the hack that killed ketchupswap and others.
    // Besides, it removes tx fee for staking, which is better for holders.

    // We added a withdraw method to remove the eth that may get stuck in the contract
    // We noticed it may happens in similar contracts for unknown reasons

    // There's quotes from the Back to The Future movies to keep you entertained while reading it

    // Transfer tax rate in basis points. (default 7.5%)
    uint16 public transferTaxRate = 750;
    // Burn rate % of transfer tax. (default 20% x 7.5% = 1.5% of total amount).
    uint16 public burnRate = 20;
    // Max transfer tax rate: 7.5%
    uint16 public constant MAXIMUM_TRANSFER_TAX_RATE = 750;
    // Burn address
    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    // Fee address
    address public constant FEE_ADDRESS = 0x56979E38cBa51c9658e842e7a7f0AaaD7D06FC95;
    ...
    // Min amount to liquify. (default 20 FARMs)
    uint256 public minAmountToLiquify = 20 ether;
    // The swap router, modifiable. Will be changed to FARM's router when our own AMM release
    IUniswapV2Router02 public testRouter;
    // The trading pair
    address public testPair;
    // In swap and liquify
    ...
    event TestRouterUpdated(address indexed operator, address indexed router, address indexed pair);
     ...
     */
    constructor() public BEP20("Test Token", "FARM") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (
            ...
            && _inSwapAndLiquify == false
            && address(testRouter) != address(0)
            && testPair != address(0)
            && sender != testPair
            // We whitelist the Masterchef
            && sender != owner()
            && recipient != owner()
        ...
        if (recipient == BURN_ADDRESS
            ...
            require(taxAmount == burnAmount + feeAmount, "FARM::transfer: Burn value invalid");

            // default 92.5% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "FARM::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the FARM pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = testRouter.WETH();

        _approve(address(this), address(testRouter), tokenAmount);

        // make the swap
        testRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        testRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from testRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "FARM::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "FARM::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "FARM::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxTransferAmountRate >= 30, "FARM::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the minimum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateTestRouter() public onlyOperator {
        address _router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        testRouter = IUniswapV2Router02(_router);
        testPair = IUniswapV2Factory(testRouter.factory()).getPair(address(this), testRouter.WETH());

        require(testPair != address(0), "FARM::updateTestRouter: Invalid pair address.");
        emit TestRouterUpdated(msg.sender, address(testRouter), testPair);
    }

    // In case some bnbs stay trapped in the contract
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "FARM::transferOperator: new operator is the zero address");
