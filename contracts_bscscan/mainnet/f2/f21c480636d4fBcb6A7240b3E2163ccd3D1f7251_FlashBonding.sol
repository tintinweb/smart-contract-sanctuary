...
contract FlashBonding is Ownable {
    ...
    uint256 public totalPurchased; // total value of outstanding bonds; used for pricing
    ...
    struct Terms {
        ...
        uint256 maxDebt; // 9 decimal debt ratio, total LOVE issue for this flash sale
    }
    ...
    function deposit(uint256 _amount, address _depositor) external returns (uint256) {
        ...
        require(totalPurchased <= terms.maxDebt, "Max capacity reached");
        ...
        require(payout <= maxPayoutOf(_depositor), "Bond too large"); // size protection because there is no slippage
        ...
        totalPurchased = totalPurchased.add(payout);
    ...
    /**
     *  @notice determine maximum bond size, limit per address
     *  @return uint
     */

    function maxPayoutOf(address account) public view returns (uint256) {
        return terms.maxPayout.sub(bondInfo[account].payout);
    }
