...
contract CroxNextGenInitializable is Ownable, ReentrancyGuard {
    ...
    // The minimum amount of first deposit
    ...
    event NewMinFirstDepositAmount(uint256 amount);
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (user.withdrawFrom <= block.timestamp) {
                if (pending.add(user.rewardLocked) > 0) rewardToken.safeTransfer(address(msg.sender), pending.add(user.rewardLocked));
                if (secRewardToken != address(0)) {
                    ...
                    if (secPending.add(user.secRewardLocked) > 0) IBEP20(secRewardToken).safeTransfer(address(msg.sender), secPending.add(user.secRewardLocked));
                }
                user.rewardLocked = 0;
        ...
        if (_amount > 0) {
            if (user.withdrawFrom == 0 && _amount < minFirstDeposit) {
                revert("Invalid first deposit amount");
            }

            uint256 feeStakedToken = _amount.mul(depositFee).div(10000);
            stakedToken.safeTransferFrom(address(msg.sender), feeAddress, feeStakedToken);
            stakedToken.safeTransferFrom(address(msg.sender), address(this), _amount.sub(feeStakedToken));
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (pending.add(user.rewardLocked) > 0) {
            if (user.withdrawFrom <= block.timestamp) {
                rewardToken.safeTransfer(address(msg.sender), pending.add(user.rewardLocked));
        ...
        if (_amount > 0) {
            ...
            if (block.number >= bonusEndBlock) {
                ...
                stakedToken.safeTransfer(feeAddress, penalty);
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        if (amountToTransfer > 0) {
            if (block.number >= bonusEndBlock) {
                ...
                stakedToken.safeTransfer(feeAddress, penalty);
                stakedToken.safeTransfer(address(msg.sender), amountToTransfer.sub(penalty));
            }
        }
    ...
    function emergencyRewardWithdraw(uint256 _amount1, uint256 _amount2) external onlyOwner {
        rewardToken.safeTransfer(address(msg.sender), _amount1);
        IBEP20(secRewardToken).safeTransfer(address(msg.sender), _amount2);
    }
    ...
    /*
     * @notice Update the minFirstDeposit
     * @dev Only callable by owner.
     * @param _amount: the deposit fee of first deposit
     */
    function updateMinFirstDeposit(uint256 _amount) external onlyOwner {
        minFirstDeposit = _amount;
        emit NewMinFirstDepositAmount(_amount);
    }
