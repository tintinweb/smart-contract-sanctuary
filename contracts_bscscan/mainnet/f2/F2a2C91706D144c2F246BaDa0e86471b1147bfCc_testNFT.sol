...
contract testBase is testAccessControl,ERC1155{
    ...
    IERC20 public FDOTA = IERC20(0x55d398326f99059fF775485246999027B3197955);
    bool allowNFTFlag = false;
    bool allowMintFlag = false;
    ...
    function setFlags(bool _nftFlag, bool _mintFlag) external onlyCEO{
        allowNFTFlag = _nftFlag;
        allowMintFlag = _mintFlag;
    }
    
    function setFDOTA(IERC20 token) external onlyCEO {
        FDOTA = token;
    }
    ...
    function createNFT(address account, uint256 amount, uint16 itemType,
    string memory itemName,uint16 gameId,bool isSingle,bool isCopyright,bytes memory data)public virtual onlyPartner{
        ...
        ItemAttr memory attr = 
            ItemAttr(itemName,gameId,itemType,isSingle,isCopyright,1,genes,genes.mod(100),intelligence,agility,health);
    ...
    function mint(address account, uint256 id, uint256 amount,bytes memory data)public onlyPartner{
        require(allowMintFlag == true, "not allow mint now");
        uint256 newItemId = _tokenIds.current();
        if(newItemId > id){
            require(creators[id] == _msgSender(),"Base: caller is not minter of the token");
            ItemAttr memory attr = tokenIdToItemAttr[id];
            require(attr.isSingle == false,"Base: this item isOnly");
    ...
    function levelUpItem(uint256 _tokenId1, uint256 _tokenId2) public returns(uint256 _rand,uint256 _genes){
        ...
        require(token1.isCopyright == false && token2.isCopyright == false && token2.isSingle == true,"Items cannot be leveled up");
        require(token1.itemType == token2.itemType,"Must be of the same type");
        if (msg.sender != ceoAddress) {
            require(FDOTA.transferFrom(msg.sender, address(this), levelUpAmount), "you need pay fee to upgrade");
        }
        ...
        tokenIdToItemAttr[newItemId] = 
            ItemAttr(attr.name,attr.gameId,attr.itemType,attr.isSingle,false,1,newGenes,strength,intelligence,agility,health);
        creators[newItemId] = _msgSender();
        _tokenIds.increment();
    ...
    function upgradeStar(uint256 _tokenId1, uint256 _tokenId2, uint16 choice) public {
        ItemAttr storage token1 = tokenIdToItemAttr[_tokenId1];
        ItemAttr storage token2 = tokenIdToItemAttr[_tokenId2];
        ...
        require(balanceOf(_msgSender(),_tokenId1) == 1 && balanceOf(msg.sender,_tokenId2) == 1,"You must have this item");
        require(token1.isCopyright == false && token2.isCopyright == false && token2.isSingle == true,"Items cannot be upgraded");
        if (msg.sender != ceoAddress) {
            require(FDOTA.transferFrom(msg.sender, address(this), upgradeStarAmount), "you need pay fee to upgrade");
        }
        
        uint tempAttrPoints = 0;
        if (token1.star == 1) {
            tempAttrPoints = 20;
            token1.health += 30;
    ...
    function itemShop(uint256 _tokenId) public payable{
        ...
        require(msg.value >= shop.price,"be short of bnb coins");
        ...
        tokenIdToItemAttr[newItemId] = 
            ItemAttr(attr.name,attr.gameId,attr.itemType,attr.isSingle,false,1,
            genes,strength,intelligence,agility,health);
        creators[newItemId] = _msgSender();
        _tokenIds.increment();
...
contract testNFT is testBase {
    ...
    function setOwnerCut(uint256 _ownerCut) external onlyCFO {
        ownerCut = _ownerCut;    
    }
