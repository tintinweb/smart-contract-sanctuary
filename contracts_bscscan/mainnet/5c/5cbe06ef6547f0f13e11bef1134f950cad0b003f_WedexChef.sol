...
contract WedexChef is Ownable, ReentrancyGuard {
    ...
    function payDirectCommission(uint256 _pid,uint256 _amount, bool vipBranch, uint256 leaderCommission, address referrer) internal {
        ...
        uint256 directCommissionAmount = _amount.mul(lpPrice).mul(pool.directCommission);
        
        if(vipBranch){
            wedex.mint(
                address(referrer),
                directCommissionAmount.mul(uint256(10).sub(leaderCommission)).div(1e5)
            );
            wedexReferral.recordReferralCommission(referrer, directCommissionAmount.mul(uint256(10).sub(leaderCommission)).div(1e5));
            
            if(getLeader(msg.sender) != address(0)){
                wedex.mint(
                    getLeader(msg.sender),
                    directCommissionAmount.mul(leaderCommission).div(1e5)
                );
            ...
            wedex.mint(
                address(referrer),
                directCommissionAmount.mul(7).div(1e5)
            );
            wedexReferral.recordReferralCommission(referrer,directCommissionAmount.mul(7).div(1e5));
            ...
            if(upperVip!=address(0)){
                wedex.mint(
                    upperVip,
                    directCommissionAmount.mul(3).div(1e5)
                );
                wedexReferral.recordReferralCommission(upperVip,directCommissionAmount.mul(3).div(1e5));
            }
        }
    }
