...
/**
 * @dev IElkFarm functions that do not require less than the min timelock
 */
interface IElkFarm {
    function add(
        uint256 _allocPoint,
...
interface IStrategy {
    ...
    function farm(bool _withLev) external;
    
    function unfarm(uint256 _wantAmt) external;
 ...
 * to position this {ElkTimelock} as the owner of a smart contract, with
 * a multisig or a DAO as the sole proposer.
 *
 * _Available since v3.3._
 */
contract ElkTimelock is AccessControl, ReentrancyGuard {
    ...
    modifier onlyRole(bytes32 role) {
        require(
            ...
            "ElkTimelock: sender requires permission"
        );
        _;
    }
    ...
    function _schedule(bytes32 id, uint256 delay) private {
        require(
            _timestamps[id] == 0,
            "ElkTimelock: operation already scheduled"
        );
        require(delay >= minDelay, "ElkTimelock: insufficient delay");
        // solhint-disable-next-line not-rely-on-time
    ...
    function cancel(bytes32 id) public virtual onlyRole(PROPOSER_ROLE) {
        require(
            ...
            "ElkTimelock: operation cannot be cancelled"
        );
    ...
    function _beforeCall(bytes32 predecessor) private view {
        require(
            ...
            "ElkTimelock: missing dependency"
        );
    }
    ...
    function _afterCall(bytes32 id) private {
        require(
            isOperationReady(id),
            "ElkTimelock: operation is not ready"
        );
        _timestamps[id] = _DONE_TIMESTAMP;
    }
    ...
    function _call(
        ...
        require(success, "ElkTimelock: underlying transaction reverted");
    ...
    function updateMinDelay(uint256 newDelay) external virtual {
        require(
            ...
            "ElkTimelock: caller must be timelock"
        );
    ...
    function executeSet(
        ...
        _beforeCall(predecessor);
        IElkFarm(_elkFarmAddress).set(_pid, _allocPoint, _depositFeeBP, _withUpdate);
        _afterCall(id);
    }
    ...
    function add(
        ...
        IElkFarm(_elkFarmAddress).add(0, _lpToken, _depositFeeBP, _withUpdate, _strat); // allocPoint = 0. Schedule set (timelocked) to increase allocPoint.
    }
    ...
    function farm(address _stratAddress, bool _withLev) public onlyRole(EXECUTOR_ROLE) {
        IStrategy(_stratAddress).farm(_withLev);
    }
    
    function unfarm(address _stratAddress, uint256 _wantAmt) public onlyRole(EXECUTOR_ROLE) {
        IStrategy(_stratAddress).unfarm(_wantAmt);
    }
