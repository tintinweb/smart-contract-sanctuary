...
contract DiwaliToken is Context, IERC20, Ownable,IERC20Metadata {
    ...
    address payable public _developmentWallet = payable(0x991Ae23C8eC0FFe1fC11fEad6c90C3A5Bc405e8A); 
    ...
    uint256 public  _totalFees      = _taxFee.add(_liquidityFee).add(_developmentFee).add(_burnFee);
   ...
   function name() external pure override returns (string memory) {
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner {
        _taxFee = taxFee;
        _totalFees      = _taxFee.add(_liquidityFee).add(_developmentFee).add(_burnFee);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 _totalFeesForBNB = _liquidityFee.add(_developmentFee);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 developmentPart = contractTokenBalance.sub(lpPart);
        ...
        if( developmentPart > 0)
        {
            ...
            swapTokensForBnb(developmentPart);
