...
contract Tysondoge is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
   
    address payable public marketingAddress = payable(0xaC333009e4E587E2D8d9DC7Cc1A722996059463B); // Marketing Address
    ...
    string private _name = "Tysondoge";
    string private _symbol = "Tysondoge";
   ...
   _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function prepareForDxSale(address dxRouter, address presaleAddress) public onlyOwner {
        ...
        _isExcludedFromMaxWallet[presaleAddress] = true;
    }
    ...
    function setMaxWalletPercent(uint256 _maxWalletPercent) external onlyOwner {
        _maxWalletAmount = _tTotal.mul(_maxWalletPercent).div(
          10**3 // Division by 1000, set to 20 for 2%, set to 2 for 0.2%
        );
    ...
    function setNumTokensSellToAddToBuyBack(uint256 amountForSwap) external onlyOwner {
        numTokensSellToAddToLiquidity = amountForSwap;
    }
