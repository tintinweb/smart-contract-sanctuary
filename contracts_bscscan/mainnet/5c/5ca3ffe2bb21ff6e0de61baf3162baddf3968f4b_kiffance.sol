...
contract kiffance is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    //change that
    uint8 public maxLiqFee = 1;                // Transaction fee % that would be used to add liquidity (0-10)
    uint8 public maxTaxFee = 15;                // Transaction fee % that would be distributed to holders (0-10)
    ...
    uint8 public minMxTxPercentage = 3;         // Transaction allowed as % of supply (1-100). Anti-Whale mechanics
    uint8 public minMxWalletPercentage = 3;     // The maximum supply % that a wallet can hold (1-100). Anti-Whale mechanics
