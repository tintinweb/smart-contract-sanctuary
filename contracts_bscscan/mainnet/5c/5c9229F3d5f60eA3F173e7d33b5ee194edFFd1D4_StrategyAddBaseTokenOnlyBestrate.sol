...
// File: contracts/StrategyAddBaseTokenOnlyBestrate.sol
...
contract StrategyAddBaseTokenOnlyBestrate is Ownable, ReentrancyGuard, Strategy {
    ...
    constructor(IUniswapV2Router02 _router, INarwhalSwapBestRateQuery _query, uint256[] memory _routerIndex) public {
        ...
        nonReentrant
    {
        ...
        (uint256 routeIndex,) = query.oneRoute(baseToken, fToken, aIn, routerIndex);
        baseToken.safeApprove(address(bestrate), 0);
        baseToken.safeApprove(address(bestrate), uint(-1));
        require(bestrate.trade.value(0)(routeIndex, baseToken, aIn, fToken, 0, partnerIndex) > 0, "cannot trade.");
        // 4. Mint more LP tokens and return all LP tokens to the sender.
        baseToken.safeApprove(address(router), 0);
        baseToken.safeApprove(address(router), uint(-1));
        fToken.safeApprove(address(router), 0);
        fToken.safeApprove(address(router), uint(-1));
        (,, uint256 moreLPAmount) = router.addLiquidity(
            baseToken, fToken, baseToken.myBalance(), fToken.myBalance(), 0, 0, address(this), now
        );
    ...
    function setRouterIndex(uint256[] calldata value) external onlyOwner nonReentrant {
        routerIndex = value;
    }
