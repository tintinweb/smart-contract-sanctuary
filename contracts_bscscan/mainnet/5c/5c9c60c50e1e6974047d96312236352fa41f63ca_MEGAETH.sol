...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also MEGAETHed.
...
contract MEGAETH is ERC20, Ownable {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PancakeSwapV2Pair, "MEGAETH: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MEGAETH: gasForProcessing must be between 200,000 and 500,000");
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!tradingOpen) {
            require(canTransferBeforetradingOpen[from], "MEGAETH: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForETH(uint256 tokenAmount, address recipient) private {
        ...
        // generate the PancakeSwap pair path of weth -> ETH
...
contract MEGAETHDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("MEGAETH_Dividend_Tracker", "MEGAETH_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "MEGAETH_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "MEGAETH_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
