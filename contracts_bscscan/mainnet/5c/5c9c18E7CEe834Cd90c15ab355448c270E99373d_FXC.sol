...
5% rewards Fees in ADA - directly auto claimed into your wallet just by holding FlokiX Cardano Tokens !
5% to marketing and 5% to liquidity pool.
Diamond hands will be rewarded !!

Official Telegram group : https://t.me/FlokiXcardano
...
import "./FXCDividendTracker.sol";

contract FXC is ERC20, Ownable {
    ...
    FXCDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("FlokiX Cardano", "FXC") {

    	dividendTracker = new FXCDividendTracker();

        _marketingWalletAddress = 0x2d5D07f10cC5F34d5Fe0742fc849Eb8997a39977;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FXC: The dividend tracker already has that address");

        FXCDividendTracker newDividendTracker = FXCDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FXC: The new dividend tracker must be owned by the FXC token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FXC: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FXC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FXC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FXC: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FXC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FXC: Cannot update gasForProcessing to same value");
