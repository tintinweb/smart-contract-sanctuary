...
contract Baby is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = 8000000 * 10**9 * 10**9;  //1% (0.005x) of Total circulating supply, after burn
    uint256 private numTokensSellToAddToLiquidity = 8000000 * 10**9 * 10**9; //1 order of magnitude smaller than the max tx amount (so, div by 10, sort of)
    ...
    constructor(address param_addr, address opt_multiSig) {
        ...
        BabyUniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());
    ...
    */
    ...
    function _transfer(address from, address to, uint256 amount
        ...
        if( (from != owner() && to != owner()) || !(_isExcludedFromTxLimit[from]) ) {
            require(amount <= _maxTxAmount, "BabyMooner: Transfer amount exceeds the maxTxAmount.");
        }
        ...
        if(to == BabyUniswapV2Pair){ //It's a sell, boys!
            purchaseOrSale = true;
        }
