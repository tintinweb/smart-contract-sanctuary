...
contract CommunityRewardBnb is Ownable {
    using SafeBEP20 for IBEP20;
    using SafeBEP20 for IWBNB;
    ...
    receive() external payable {
        assert(msg.sender == address(rewardToken)); // only accept BNB via fallback from the WBNB contract
    }

    constructor(
        IWBNB _wbnb, //token which will be staked
        IBEP20 _stakeToken, //token which will be a reward for staking
    ...
    // Stake stakeToken and Harvest BNB
    function deposit(uint256 _amount, address referrer) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                ...
                IWBNB(rewardToken).withdraw(pending);
    ...
    // Withdraw stakeTokens tokens from farm.
    ...
    function payRefFees(uint256 pending) internal
    {
        ...
        if (referrer != address(0)) {// send commission to referrer
            ...
            IWBNB(rewardToken).withdraw(toReferral);
            safeTransferBNB(referrer, toReferral);
    ...
    function rewardWithdraw() public onlyOwner {
        ...
        IWBNB(rewardToken).withdraw(balance);
        safeTransferBNB(divPoolAddress, balance);
