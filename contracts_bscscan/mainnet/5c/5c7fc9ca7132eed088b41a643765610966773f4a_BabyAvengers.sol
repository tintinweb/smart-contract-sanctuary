...
contract BabyAvengers is IBEP20, Auth {
    ...
    uint256 public _maxTx1 = (_totalSupply * 1) / 200; //Max Tx for the first 30 seconds is 0.5%
    uint256 public _maxTx2 = (_totalSupply * 2) / 100; //Max Tx for after 30 seconds is 2%
    uint256 public _maxTxAmount = _maxTx1; 
    ...
    address public marketingFeeReceiver = address(0x1BBb762296Aea5bc47274103AcD01C7f0a7d5b1b);
    ...
    bool public tradingOpen; //Once the trading is open, I can not stop it
    ...
    bool public swapDividendEnabled = true;
    uint256 public swapThreshold = _totalSupply / 30000; // 0.01% of supply
    bool inSwap;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!authorizations[sender] && !authorizations[recipient] && !tradingOpen){
            if(launchAt <= block.timestamp)
                tradingOpen = true;
            require(tradingOpen,"Trading not open yet");
        }

        if(!Done && launchAt + 30 seconds <= block.timestamp){
            buyCooldownEnabled = false;
            _maxTxAmount = _maxTx2;
            Done = true;
        }
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isBuying, isSelling) : amount;
    ...
    function takeFee(address sender, uint256 amount, bool isBuying, bool isSelling) internal returns (uint256) {
    	uint256 finalFee = totalFee;

        if(isBuying){
            if(launchAt + 2 seconds >= block.timestamp)
