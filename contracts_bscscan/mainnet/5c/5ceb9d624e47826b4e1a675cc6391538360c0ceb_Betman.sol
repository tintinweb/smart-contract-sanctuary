...
/**
BetMan
 Telegram : https://t.me/Betmanbsc
 Games on www.betman.cc
...
BetMan
is based on a brand new tokenomics that we have mentioned on our mediums and website.
www.Betman.cc
The Project is based on the upcoming trade game mentioned on the websites of BET.
*/
...
contract Betman is Context, IERC20, IERC20Extented, Ownable {
    using SafeMath for uint256;
    string private constant _name = "BetMan";
    ...
    function getTokenPrice() external view returns(uint256) {
        IERC20Extented token0 = IERC20Extented(IUniswapV2Pair(uniswapV2Pair).token0());//Betman
        ...
        if(pairSwapped) {
            token0 = IERC20Extented(IUniswapV2Pair(uniswapV2Pair).token1());//Betman
