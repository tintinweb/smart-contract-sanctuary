...
contract StakingContract is Ownable, ReentrancyGuard {
    ...
    mapping(address => Counters.Counter) public userDepositId;
    ...
    uint256 public immutable claimingTimelock;
    uint256 public immutable prestakeTimestamp;
    uint256 public immutable bonusRewardTimestamp;

    uint256[] public milestones; // We can make both mappings immutable.
    ...
    event DepositMade(address indexed user, address indexed recipient, uint256 depositId, uint256 amount);
    event DepositRewardUpdated(address indexed user, uint256 indexed depositId, uint256 newRewardLevel);
    event AutoCompoundSet(address user, bool authorized);
    event InterestClaimed(address indexed user, uint256[] depositIds, uint256 totalAmount);
    event InterestStaked(address indexed user, uint256[] depositIds, uint256 totalAmount);
    event PartialWithdrawal(address indexed user, uint256 depositId, uint256 amount);
    event Withdrawal(address indexed user, uint256[] depositIds, uint256 amount);



    // Contract constructor.
    constructor(address _token, address _distributor, address _DAO, uint256 _initRewards, uint256 _claimTimelock, uint256 _prestakeTimestamp, uint256 _bonusRewardTimestamp) {
    ...
    function setTokenRewardsPerSecond(uint256 _tokenRewardsPerSecond) external onlyOwner {
        tokenRewardsPerSecond = _tokenRewardsPerSecond;
    }

    function setBonusRewardsPerSecond(uint256 _bonusTokenRewardsPerSecond) external onlyOwner {
        bonusTokenRewardsPerSecond = _bonusTokenRewardsPerSecond;
    }

    function setRewardStructure(uint256[] calldata timestamps, uint256[] calldata changes) external onlyOwner {
        // Array of mock milestones: [0, 60, 300, 600, 3600]
        // Array of mock rate changes: [0, 1500, 2500, 3000, 3200]
        require(milestonesSet == false, 'Error: milestones have already been set');
        require(timestamps.length == changes.length, 'Error: arrays with different lengths');
    ...
    function getTokenRewardsPerDepositedUnit() public view returns(uint256) {
        require(adjTokenDepositedAmount != 0, 'Error: Calculation not available');

        uint256 unitReward = tokenRewardsPerSecond * TOKEN_UNIT / adjTokenDepositedAmount;
        return unitReward;
    }

    function getBonusTokenRewardsPerDepositedUnit() public view returns(uint256) {
        require(adjTokenDepositedAmount != 0, 'Error: Calculation not available');

        uint256 unitReward = bonusTokenRewardsPerSecond * TOKEN_UNIT / adjTokenDepositedAmount;
        return unitReward;
    }

    function updateDepositRewardLevel(address depositor, uint256 depositId) public returns(bool) {
        require(userActiveDeposits[depositor][depositId] == true, 'Error: Deposit ID is not active');
        ...
        if (newRewardLevel > depoRewardLevel) {
            ...
            uint256 depoAdjustment = depo.amount * rewardAdjustments[newRewardLevel] / PERCENTAGE_FACTOR;
    ...
    function deposit(address recipient, uint256 amount) public {
        ...
        if (operator != address(this) && operator != owner()) {
            IERC20(token).safeTransferFrom(operator, address(this), amount); 
        }
        ...
        userInfo[recipient][depositId] = Deposit(amount, amount, depoTimestamp, 0, lastClaimedTimestamp, depoClaimTimelock);
    ...
    function claimInterest(uint256[] calldata depositIds, bool compound) external nonReentrant returns(uint256) {
    ...
    function autoCompoundInterest(address depositor, uint256[] calldata depositIds) external onlyOwner {
        require(autoCompound[depositor] == true, 'Error: User has not allowed autoCompound');
        ...
        uint256 netAmount = amountClaimed.sub(compoundFee);
        deposit(depositor, netAmount);

        IERC20(token).safeTransfer(DAO, compoundFee);

        emit InterestStaked(depositor, depositIds, netAmount);
    }

    function withdraw(uint256[] calldata depositIds) external nonReentrant {
        address depositor = msg.sender;
        ...
        for (uint256 i = 0; i < depositIds.length; i++) {
            require(userActiveDeposits[depositor][depositIds[i]] == true, 'Error: Deposit is not active'); 
            ...
            if (newRewardLevel > depo.rewardLevel) {
                ...
                emit DepositRewardUpdated(depositor, depositIds[i], newRewardLevel);
            }

            // Withdrawing process.
            require(block.timestamp >= prestakeTimestamp, 'Error: Not allowed to withdraw');

            uint256 accruedTime = block.timestamp - depo.lastTimestampClaimed;
            uint256 tokenUnitReward = getTokenRewardsPerDepositedUnit();

            amountInterest += depo.adjAmount * accruedTime * tokenUnitReward / TOKEN_UNIT;

            if(depo.lastTimestampClaimed < bonusRewardTimestamp ) {
                uint256 bonusTokenUnitReward = getBonusTokenRewardsPerDepositedUnit();
                uint256 bonusTime = bonusRewardTimestamp - depo.lastTimestampClaimed;
                amountInterest += depo.amount * bonusTime * bonusTokenUnitReward / TOKEN_UNIT;
            }
        ...
        IDistributor(distributor).sendTokenAmount(address(this), amountInterest);

        IERC20(token).safeTransfer(depositor, totalAmount);
    ...
    function partialWithdraw(uint256 depositId, uint256 amount) external nonReentrant {
        address depositor = msg.sender;

        Deposit storage depo = userInfo[depositor][depositId];

        require(userActiveDeposits[depositor][depositId] == true, 'Error: Deposit is not active'); 
        ...
        uint256 amountInterest = depo.adjAmount * accruedTime * tokenUnitReward / TOKEN_UNIT;
        ...
        uint256 adjAmountPrincipal = amount + amount * rewardAdjustments[depo.rewardLevel] / PERCENTAGE_FACTOR;
    ...
    // Internal functions.
    function _getDepositRewardLevel(uint256 depositLifetime) internal view returns(uint256) {
        uint256 rewardLevel;
    ...
    function updateAdjustedTokenDepositedAmount(uint256 amount, bool add) internal {
        if (add == true) {
    ...
    function _claimInterest(address depositor, uint256[] calldata depositIds) internal returns(uint256) {
        ...
        for(uint256 i = 0; i < depositIds.length; i++) {
            ...
            require(block.timestamp >= depo.timelockTimestamp, 'Error: Not allowed to claim yet');
            ...
            amountClaimed += depo.adjAmount * accruedTime * tokenUnitReward / TOKEN_UNIT;

            if(depo.lastTimestampClaimed < bonusRewardTimestamp ) {
                uint256 bonusTokenUnitReward = getBonusTokenRewardsPerDepositedUnit();
                uint256 bonusTime = bonusRewardTimestamp - depo.lastTimestampClaimed;
                amountClaimed += depo.amount * bonusTime * bonusTokenUnitReward / TOKEN_UNIT;
            }
