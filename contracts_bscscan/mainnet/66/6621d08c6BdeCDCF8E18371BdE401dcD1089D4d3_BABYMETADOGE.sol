...
// File: contracts/BABYMETADOGEDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, Ownable, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public DividendToken = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //Change dividend
...
// File: contracts/BABYMETADOGE.sol
...
contract BABYMETADOGE is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public DividendToken = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //change dividend token

    bool private swapping;

    BABYMETADOGEDividendTracker public dividendTracker;

    address public liquidityWallet;
    uint256 public maxBuyTranscationAmount = 5000000 * (10**18);
    uint256 public maxSellTransactionAmount = 2000000 * (10**18);
    uint256 public swapTokensAtAmount = 2000000 * (10**18);
    uint256 public _maxWalletToken = 20000000 * (10**18); // 2% of total supply
    ...
    address _marketingWalletAddress = 0xdc2e3FF58a2e8871e917397D3977182Ef6ED5AF0; //change this
    ...
    constructor() ERC20("BABYMETADOGE", "BBYMTDG") {
    	...
    	dividendTracker = new BABYMETADOGEDividendTracker();
    ...
    receive() external payable {
  	...
  	function whitelistDxSale(address _presaleAddress, address _routerAddress) public onlyOwner {
        ...
        canJumpLimitOnWallet[_presaleAddress] = true;
        canJumpLimitOnWallet[_routerAddress] = true;
  	}

    function changeSellFee(uint256 amount) external onlyOwner {
	    require(amount >= 100 && amount <= 150, "BABYMETADOGE: SellFee must be updated to between 100 and 150.");
		sellFeeIncreaseFactor = amount;
	}
	
	 function changeDividend(address TokenAddress) external onlyOwner {
		emit UpdateDividendToken(DividendToken, address(TokenAddress));
		DividendToken = TokenAddress;
        dividendTracker.changeDividend(TokenAddress);
	}

    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BABYMETADOGE: The dividend tracker already has that address");

        BABYMETADOGEDividendTracker newDividendTracker = BABYMETADOGEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BABYMETADOGE: The new dividend tracker must be owned by the BABYMETADOGE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BABYMETADOGE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BABYMETADOGE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BABYMETADOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BABYMETADOGE: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BABYMETADOGE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BABYMETADOGE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BABYMETADOGE: Cannot update gasForProcessing to same value");
    ...
    function updateRewardsFee(uint256 RewardFeePercent) external onlyOwner{
        require(RewardFeePercent <= 20, "BABYMETADOGE: updateRewardsFee must be between 0 and 20.");
        TokenRewardsFee = RewardFeePercent;
        totalFees = TokenRewardsFee.add(liquidityFee).add(marketingFee);
    }
    
    function updateLiquidityFee(uint256 liquidityFeePercent) external onlyOwner{
        require(liquidityFeePercent <= 5, "BABYMETADOGE: updateRewardsFee must be between 0 and 5.");
        liquidityFee = liquidityFeePercent;
        totalFees = TokenRewardsFee.add(liquidityFee).add(marketingFee);
    }
    
    function updateMarketingFee(uint256 marketingFeePercent) external onlyOwner{
        require(marketingFeePercent <= 10, "BABYMETADOGE: updateRewardsFee must be between 0 and 10.");
    ...
    function setTradingIsEnabled() public onlyOwner {
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BABYMETADOGE: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        uint256 initialBBYMTDGBalance = IERC20(DividendToken).balanceOf(address(this));
 
        swapTokensForBusd(tokens, address(this));
        uint256 newBalance = (IERC20(DividendToken).balanceOf(address(this))).sub(initialBBYMTDGBalance);
...
contract BABYMETADOGEDividendTracker is Ownable, DividendPayingToken{
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    uint256 public minimunForRewards = 50000;
    ...
    constructor() DividendPayingToken("BABYMETADOGE_Dividend_Tracker", "BABYMETADOGE_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = minimunForRewards * (10**18); //must hold 50000+ tokens
    }

    // Prevent bug :)
    function _transfer(address, address, uint256) internal pure override {
        require(false, "BABYMETADOGE: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BABYMETADOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BABYMETADOGE_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }

    function updateMinimunForRewards(uint256 newValueMinimunForRewards) external onlyOwner {
        require(newValueMinimunForRewards >= 10000 && newValueMinimunForRewards <= 500000, "BABYMETADOGE_Dividend_Tracker: newValueMinimunForRewards must be updated to between 10000 and 500000");
        require(newValueMinimunForRewards != minimunForRewards, "BABYMETADOGE_Dividend_Tracker: Cannot update claimWait to same value");
