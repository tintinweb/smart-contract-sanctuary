...
contract Number is IERC20, Ownable {
    ...
    constructor(address tokenOwner,address root) {
        _name = "Digital Metaverse DAO";
        _symbol = "Number";
        _decimals = 18;

        _tTotal = 75969341790000 * 10**_decimals;
    ...
    function transferFrom(
        ...
        if(recipient == uniswapV2Pair && swapStart == 0){
    ...
    function getIsIn5minter() public view returns (bool) {
        return block.timestamp.sub(300) <= swapStart;
    }
    ...
    function _transfer(
        ...
        bool takeFee = true;
        bool isMin = _tTotal.sub(balanceOf(_destroyAddress)) <= 1000000000* 10**_decimals;
        if(_isExcludedFromFee[from]||_isExcludedFromFee[to] || isMin){
            takeFee = false;
        }else{
            bool isIn5minter = block.timestamp.sub(300) <= swapStart;
            if(from == uniswapV2Pair && isIn5minter) {
                require(amount <= 500000000* 10**_decimals, "amount no't more than 1");
            }
        }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            _takeTransfer(
                sender,
                _destroyAddress,
                tAmount.div(10000).mul(_destroyFee.add(500).sub(rateSum)),
                currentRate
            );
