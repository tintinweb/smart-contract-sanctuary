...
abstract contract DealBase is AdminableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct TierInfo {
        uint256 gmpdAmount; //min amount of GMPD
    ...
    function updateTier(uint256 _index, uint256 _gmpdAmount, uint256 _ticketSize, uint256 _allocation) external onlyOwnerOrAdmin {
    ...
    function addTier(uint256 _gmpdAmount, uint256 _ticketSize, uint256 _allocation) external onlyOwnerOrAdmin {
        allTiers.push(TierInfo(_gmpdAmount, _ticketSize, _allocation, 0));
        checkExceedTiers();
    }
