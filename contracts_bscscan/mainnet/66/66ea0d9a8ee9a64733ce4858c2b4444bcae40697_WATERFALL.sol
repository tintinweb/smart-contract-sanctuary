...
contract WATERFALL is Context, BEP20, Ownable {
    ...
    uint256 public FeeTotal = (_rFee.add(_mFee).add(_lFee));
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _rFeeTotal;
    uint256 private _srFeeTotal;
    ...
    function setABFee(uint256 bmFee, uint256 brFee, uint256 blFee) external onlyOwner() {
        ...
        _rFee = brFee;
        _lFee = blFee;
        FeeTotal = (_rFee.add(_mFee).add(_lFee));
    }
    ...
    function _reflectsFee(uint256 rFee, uint256 tFee) private {
        _rTotal = _rTotal.sub(rFee);
        _rFeeTotal = _rFeeTotal.add(tFee);
    }
    ...
    function _getsValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing) = _getsTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getsRValues(tAmount, tFee, tLiquidity, tMarketing, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tMarketing);
    }

    function _getsTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tFee = calculatesReflectionFee(tAmount);
        uint256 tLiquidity = calculatesLiquidityFee(tAmount);
        uint256 tMarketing = calculatesMarketingFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tMarketing);
        return (tTransferAmount, tFee, tLiquidity, tMarketing);
    }

    function _getsRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rMarketing = tMarketing.mul(currentRate);
    ...
    function _takesLiquidity(uint256 tLiquidity) private {
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rLiquidity);
        if(_isExcluded[address(this)])
    ...
    function _takesMarketing(uint256 tMarketing) private {
        uint256 currentRate =  _getRate();
        uint256 rMarketing = tMarketing.mul(currentRate);
        _rOwned[_mWA] = _rOwned[_mWA].add(rMarketing);
        if(_isExcluded[_mWA])
    ...
    function _transfersStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing) = _getsValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takesLiquidity(tLiquidity);
        _takesMarketing(tMarketing);
        _reflectsFee(rFee, tFee);
        emit Transfer(sender, recipient, tTransferAmount);
    }
