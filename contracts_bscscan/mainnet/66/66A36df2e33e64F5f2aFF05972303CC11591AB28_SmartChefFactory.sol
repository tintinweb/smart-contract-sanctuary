...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    uint public numberOfClaimSavedPendingRewards;
    uint public numberOfClaimCurrentAndTotalPendingRewards;

    struct UserInfo {
        uint256 amount; // How many staked tokens the user has provided
        uint256 rewardDebt; // Reward debt
        uint256 savedPendingRewards;
    ...
    function updateRewardPerBlock(uint256 _rewardPerBlock) external onlyOwner {
        ...
        numberOfClaimSavedPendingRewards = 0;
        numberOfClaimCurrentAndTotalPendingRewards = 0;
        rewardPerBlock = _rewardPerBlock;
        emit NewRewardPerBlock(_rewardPerBlock);
    }

    function updateSavedPendingRewards(uint256 num) external onlyOwner {
        require(num > 0, 'The no should be greater than 0');
        //require(hasRewardPerBlockUpdated, 'Unable to claim reward, reward per block has not yet updated by admin.');

        uint _numberOfClaim = numberOfClaimSavedPendingRewards;

        for (;
            _numberOfClaim < (numberOfClaimSavedPendingRewards.add(num)) && _numberOfClaim < userArr.length;
            _numberOfClaim++ // Not using add of safemath to reduce gas
        ) {   
            UserInfo storage user = userInfo[userArr[_numberOfClaim]];
            user.savedPendingRewards = user.totalPendingRewards;
        }
        ...
        numberOfClaimSavedPendingRewards = _numberOfClaim;
    }


    function updateCurrentAndTotalPendingRewards(uint256 num) external onlyOwner {
        require(num > 0, 'The no should be greater than 0');
        //require(hasRewardPerBlockUpdated, 'Unable to claim reward, reward per block has not yet updated by admin.');

        uint _numberOfClaim = numberOfClaimCurrentAndTotalPendingRewards;

        for (;
            _numberOfClaim < (numberOfClaimCurrentAndTotalPendingRewards.add(num)) && _numberOfClaim < userArr.length;
            _numberOfClaim++ // Not using add of safemath to reduce gas
        ) {   
            UserInfo storage user = userInfo[userArr[_numberOfClaim]];
            user.currentPendingRewards = pendingReward(userArr[_numberOfClaim]);
            user.totalPendingRewards = user.currentPendingRewards.add(user.savedPendingRewards);
        }
        ...
        numberOfClaimCurrentAndTotalPendingRewards = _numberOfClaim;
    }
