...
interface SurfDoge {
   ...
   function wasLotteryDrawn() external pure returns (bool) ;
...
contract HelperContract is Ownable {
    ...
    event TimeBetweenDrawsUpdated(uint256 indexed oldTime, uint256 indexed newTime);
    event BalanceMinimumUpdated(uint256 indexed oldBalance, uint256 indexed newBalance);
    
	uint256 public timeBetweenDraws = 3600;
	uint256 private timeOfLastLottery;
	uint256 public balanceMinimum = 40000000000000000; //0.04 BNB Minimum for getting randomness
	
	address surfDogeAddress = 0x5ea5cD406dBB5e7344DA9912C7fF07aCC4a92243;
    ...
    function emptySurplusOfBNB () public onlyOwner {
        uint256 surplus = address(this).balance - balanceMinimum; //0,04 BNB for sending TX
    ...
    function timeToNextLottery () public view returns (string memory) {
        if (timeOfLastLottery == 0) {
            uint256 minutesToLottery = 0;
            return append(uint2str(minutesToLottery), " minutes");
        }
        
        else {
            uint256 minutesToLottery = (timeOfLastLottery + timeBetweenDraws - block.timestamp) / 60;
            return append(uint2str(minutesToLottery), " minutes");
        }
    }
    ...
    receive() external payable {
	    ...
	    if (surfDogeAddress.balance > balanceMinimum) {
    	   ...
    	   if ((timeOfLastLottery + timeBetweenDraws) < block.timestamp && !surfdoge.wasLotteryDrawn()) {
