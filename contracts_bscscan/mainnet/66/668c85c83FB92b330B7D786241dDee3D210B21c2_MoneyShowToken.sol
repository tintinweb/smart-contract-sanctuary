...
contract MoneyShowToken is BEP20('MoneyShow Token', 'MYS') {
    ...
    */
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying LIFE (not scaled);
...
// MasterChef is the master of MoneyShow. He can make MoneyShow and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once MoneyShow is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of MoneyShows
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accLifePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accLifePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. MoneyShows to distribute per block.
        uint256 lastRewardBlock;  // Last block number that MoneyShows distribution occurs.
        uint256 accLifePerShare;   // Accumulated Lifes per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The LIFE TOKEN!
    MoneyShowToken public life;
    // Dev address.
    address public devaddr;
    // MoneyShowToken tokens created per block.
    uint256 public myfperblock;
    // Bonus muliplier for early MoneyShowToken makers.
    ...
    // The block number when MoneyShowToken mining starts.
    ...
    constructor(
        MoneyShowToken _life,
        address _devaddr,
        address _feeAddress,
        uint256 _myfperblock,
        uint256 _startBlock
    ) public {
        life = _life;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        myfperblock = _myfperblock;
    ...
    // Update the given pool's cMoneyShowToken allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending MoneyShowToken on frontend.
    function pendingMyf(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 lifeReward = multiplier.mul(myfperblock).mul(pool.allocPoint).div(totalAllocPoint);
            accLifePerShare = accLifePerShare.add(lifeReward.mul(1e12).div(lpSupply));
        }
    ...
    // Deposit LP tokens to MasterChef for MoneyShow allocation.
    ...
    // Safe Life transfer function, just in case if rounding error causes pool to not have enough Lifes.
    function safeLifeTransfer(address _to, uint256 _amount) internal {
        uint256 lifeBal = life.balanceOf(address(this));
        if (_amount > lifeBal) {
            life.transferWithLock(_to, lifeBal);
        } else {
            life.transferWithLock(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _myfperblock) public onlyOwner {
        massUpdatePools();
        myfperblock = _myfperblock;
    }
    
    // Setup life Token Reward Release 
    function setLifeRewardLock(uint256 lock) public onlyOwner {
        life.setRewardLock(lock);
    }
    
    function setLifeTotalBlockRelease(uint256 totalBlockRelease) public onlyOwner {
        life.setTotalBlockRelease(totalBlockRelease);
    }
}