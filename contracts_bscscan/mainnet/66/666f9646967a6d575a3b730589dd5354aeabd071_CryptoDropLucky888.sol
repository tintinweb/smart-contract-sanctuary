...
// Root file: contracts/CryptoDropLucky888.sol
...
/// @notice Lucky888
contract CryptoDropLucky888 is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(address _storage, address _team) {
        require(_storage != address(0x0) && _team != address(0x0), "Lucky888: address is zero");
    ...
    function placeBet() external payable virtual nonReentrant whenNotPaused {
        require(!msg.sender.isContract(), "Lucky888: sender cannot be a contract");
        require(tx.origin == msg.sender, "Lucky888: msg sender is not original user");
        require(
            msg.value >= betLimits.min && msg.value <= betLimits.max,
            string(
                abi.encodePacked(
                    "Lucky888: Bet amount should be greater or equal than ",
        ...
        Bet memory bet = Bet({blockNumber: block.number - 1, amount: msg.value, reward: 0, hash: ""});

        bets[msg.sender].push(bet);

        _processBet(msg.sender, bets[msg.sender].length - 1);
    }
    ...
    function _beforeProcessBet(uint256 _amount) internal virtual {
        ...
        _deliverFunds(storageAddress, storageFee, "Lucky888: failed transfer BNB to Staking Storage");

        _deliverFunds(teamAddress, teamFee, "Lucky888: failed transfer BNB to Team");
    }
    ...
    function _processBet(address _gamer, uint256 _id) internal virtual {
        ...
        if (isLucky) {
            ...
            //sent to gamer prize
            _deliverFunds(_gamer, prize, "Lucky888: failed transfer BNB to Gamer");

            emit PayoutBet(_id, prize, _gamer);
        }

        emit ProcessBet(_id, _gamer, bet.amount, bet.reward, bet.hash);
    }

    /// @notice get block hash
    function _getBlockHash() internal view virtual returns (bytes32 _hash) {
        return keccak256(abi.encode(blockhash(block.number - 1), block.timestamp, block.coinbase, msg.sender));
    }
    ...
    function withdrawToken(address _token, address _recipient) external virtual onlyOwner {
    ...
    function _withdraw(address _recipient, uint256 _amount) internal virtual {
        require(_recipient != address(0x0), "Lucky888: address is zero");
        require(_amount <= address(this).balance, "Lucky888: not enought BNB balance");
        ...
        _deliverFunds(_recipient, _amount, "Lucky888: Can't send BNB");
    ...
    function _withdrawToken(
        ...
        require(_recipient != address(0x0), "Lucky888: address is zero");
        require(_amount <= IERC20(_token).balanceOf(address(this)), "Lucky888: not enought token balance");
