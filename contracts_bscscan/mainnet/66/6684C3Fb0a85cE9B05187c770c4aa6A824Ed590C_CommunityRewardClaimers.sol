...
contract CommunityRewardClaimers is Ownable {
    ...
    mapping(address => uint256) public pearlAwardeeRewards;
    mapping(address => uint256) public clamAwardeeRewards;
    mapping(address => uint256) public clamMaximaAwardeeRewards;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < claimers.length; i++) {
            pearlAwardeeRewards[claimers[i]] = 1;
        }
    }
    ...
    function collectReward() external {
        require(
            rngRequestHashFromRewardBeneficiary[msg.sender] != bytes32(0),
            "CommunityRewardClaimers: Can't collect reward without claiming it first. You may have also already claimed your reward(s)"
        );
        ...
        if (pearlAwardeeRewards[msg.sender] > 0) {
            rewardType = address(pearl);
            pearl.mint(msg.sender, rand, PEARLS_REMAINING, PEARL_BODY_COLOR_NUMBER, PEARL_SHAPE_NUMBER);
            pearlAwardeeRewards[msg.sender] = pearlAwardeeRewards[msg.sender].sub(1);
        } else if (clamMaximaAwardeeRewards[msg.sender] > 0) {
            isMaxima = true;
            rewardType = address(clam);
            clam.mint(msg.sender, rand, isMaxima);
            clamMaximaAwardeeRewards[msg.sender] = clamMaximaAwardeeRewards[msg.sender].sub(1);
        } else {
            rewardType = address(clam);
            clam.mint(msg.sender, rand, isMaxima);
            clamAwardeeRewards[msg.sender] = clamAwardeeRewards[msg.sender].sub(1);
        }
    ...
    function isAwardee(address claimer) public view returns (bool) {
        return pearlAwardeeRewards[claimer].add(clamAwardeeRewards[claimer]).add(clamMaximaAwardeeRewards[claimer]) > 0;
    }

    function userRewards(address claimer) public view returns (uint256) {
        return pearlAwardeeRewards[claimer].add(clamAwardeeRewards[claimer]).add(clamMaximaAwardeeRewards[claimer]);
    }

    // *** owner mode *** //
    function addPearlAwardee(address[] memory accounts) external onlyOwner {
        for (uint256 index; index < accounts.length; index++) {
            pearlAwardeeRewards[accounts[index]] = pearlAwardeeRewards[accounts[index]].add(1);
        }
    }

    function addClamAwardee(address[] memory accounts) external onlyOwner {
        for (uint256 index; index < accounts.length; index++) {
            clamAwardeeRewards[accounts[index]] = clamAwardeeRewards[accounts[index]].add(1);
        }
    }

    function addClamMaximaAwardee(address[] memory accounts) external onlyOwner {
        for (uint256 index; index < accounts.length; index++) {
            clamMaximaAwardeeRewards[accounts[index]] = clamMaximaAwardeeRewards[accounts[index]].add(1);
        }
    }

    function changeUserPearlRewards(address account, uint256 rewardAmount) external onlyOwner {
        pearlAwardeeRewards[account] = rewardAmount;
    }

    function changeUserClamRewards(address account, uint256 rewardAmount) external onlyOwner {
        clamAwardeeRewards[account] = rewardAmount;
    }

    function changeUserClamMaximaRewards(address account, uint256 rewardAmount) external onlyOwner {
        clamMaximaAwardeeRewards[account] = rewardAmount;
    }
