...
// V1.2 testnet contract for DogeBets.
...
//PREDICTIONS
contract DogeBetsPredictionV1 is Ownable, Pausable, ReentrancyGuard {
    struct Round {
        uint256 epoch;
        uint256 bullAmount;
        uint256 bearAmount;
    ...
    function RoundExecute(int256 price, uint32 timestamp, uint256 betOnBull, uint256 betOnBear) external onlyOwnerOrOperator whenNotPaused 
    {
        require(startedOnce && lockedOnce, "Can only execute after StartRound and LockRound is triggered");
        require(Rounds[currentEpoch - 2].closeTimestamp != 0, "Can only execute after round n-2 has ended");
    ...
    {
        return (_minHouseBetRatio <= ((betBull * 100) / betBear) &&
            ((betBull * 100) / betBear) <= ((100 * 100) / _minHouseBetRatio));
    }
