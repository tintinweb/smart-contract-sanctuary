...
https://t.me/DADFEGCOINBSC

https://www.babyfegcoin.net/

Since 2021, Hold DAD FEG COIN Earn BABY FEG COIN and MUM FEG COIN
...
contract DividendDistributor is IDividendDistributor {
    ...
    /* MAINNET ADDRESSES */
    address BABYFEG_TOKEN_ADDR = 0x7038A3E5db841021C4Ebfe2f605E43739eBd5887;
    address MUMFEG_TOKEN_ADDR = 0x7038A3E5db841021C4Ebfe2f605E43739eBd5887;
    ...
    IBEP20 BABYFEG = IBEP20(BABYFEG_TOKEN_ADDR);
    IBEP20 MUMFEG = IBEP20(MUMFEG_TOKEN_ADDR);
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore1 = BABYFEG.balanceOf(address(this));
        uint256 balanceBefore2 = MUMFEG.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(BABYFEG);
        ...
        uint256 amount = BABYFEG.balanceOf(address(this)).sub(balanceBefore1);
        ...
        path[0] = router.WETH();
        path[1] = address(MUMFEG);

        router.swapExactETHForTokensSupportingFeeOnTransferTokens{
            value: half
        }(0, path, address(this), block.timestamp);

        amount = MUMFEG.balanceOf(address(this)).sub(balanceBefore2);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if (amount > 0) {
            ...
            BABYFEG.transfer(shareholder, amount.div(2));
            MUMFEG.transfer(shareholder, amount.div(2));
...
contract DadFEG is IBEP20, RSunAuth {
    using SafeMath for uint256;
    ...
    string constant _name = "DAD FEG COIN";
    string constant _symbol = "$DADFEGCOIN";
