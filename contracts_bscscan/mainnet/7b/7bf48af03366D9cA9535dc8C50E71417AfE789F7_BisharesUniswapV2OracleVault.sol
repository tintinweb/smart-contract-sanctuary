...
/* ==========  Mutative Functions  ========== */
  ...
  function updatePrice(address factory, address token) external returns (bool);

  function updatePrices(address[] memory factories, address[] memory tokens) external returns (bool[] memory);

/* ==========  Meta Price Queries  ========== */

  function hasPriceObservationInWindow(address factory, address token, uint256 priceKey) external view returns (bool);
...
/* ==========  Price Update Queries  ========== */
  ...
  function canUpdatePrice(address factory, address token) external view returns (bool);
...
/* ========= View Functions ========= */
  ...
  function _calculateVaultLPReservesInQuote(
    ...
    if (_isLPReady(token0, token1, quote)) {
      lpReservesInQuote = uint256(vaultLP.token0() == quote
        ? vaultLPReserve0
        : vaultLPReserve1
      ).mul(2);
    } else {
      (uint256 token0Reserve, uint256 quoteReserve) = UniswapV2Library.getReserves(
        factory,
        token0,
        quote
      );
      uint256 tokenToQuotePrice = quoteReserve.mul(1e18).div(token0Reserve);
      lpReservesInQuote = uint256(vaultLPReserve0).mul(tokenToQuotePrice.mul(2)).div(1e18);
    }
  }
  ...
  function _calculatePriceCumulativeLast(
    ...
    uint256 rate = rateNominator.mul(1e18).div(rateDenominator);
    priceCumulativeLast = uint224(rate.mul(rateMultiplier).div(rateDivider).mul(2 ** 112).div(1e18));
  }
    ...
    returns (uint32 timestamp, uint224 priceCumulativeLast)
  {
    ...
    priceCumulativeLast = reversed
      ? _calculatePriceCumulativeLast(vaultLPTotalSupply, lpReservesInQuote, 1e18, vaultPrice)
      : _calculatePriceCumulativeLast(lpReservesInQuote, vaultLPTotalSupply, vaultPrice, 1e18);
  }
...
/* ========= Utility Functions ========= */
   ...
   */
  function computeTwoWayAveragePrice(
    PriceLibrary.PriceObservation memory observation1,
    PriceLibrary.PriceObservation memory observation2
  ) internal pure returns (PriceLibrary.TwoWayAveragePrice memory) {
...
library UniswapV2OracleLibrary {
  ...
  function computeAveragePrice(
    ...
    // overflow is desired.
    if (priceCumulativeStart > priceCumulativeEnd) {
      (priceCumulativeStart, priceCumulativeEnd) = (priceCumulativeEnd, priceCumulativeStart);
    }
...
contract BisharesUniswapV2OracleVault is IBisharesUniswapV2Oracle {
...
/* ==========  Storage  ========== */
  ...
  mapping(address => mapping(address => BisharesPriceMapLibrary.BisharesPriceMap)) internal _tokenPriceMaps;
...
/* ==========  Mutative Functions  ========== */
   ...
   */
  function updatePrice(address factory, address token) public override returns (bool) {
    if (token == _weth) return true;
    PriceLibrary.PriceObservation memory observation = factory.observeTwoWayPrice(token, _weth);
    return _tokenPriceMaps[factory][token].writePriceObservation(observation);
  }
  ...
  function updatePrices(
      address[] memory factories,
      address[] memory tokens
  ) external override returns (bool[] memory pricesUpdated) {
    uint256 len = factories.length;
    require(len == tokens.length, "Invalid tokens count");
    pricesUpdated = new bool[](len);
    for (uint256 i = 0; i < len; i++) {
      pricesUpdated[i] = updatePrice(factories[i], tokens[i]);
    }
  }
...
/* ==========  Meta Price Queries  ========== */
   ...
   */
  function hasPriceObservationInWindow(
		address factory,
		address token,
		uint256 priceKey
	) external view override returns (bool) {
    return _tokenPriceMaps[factory][token].hasPriceInWindow(priceKey);
  }
  ...
  function getPriceObservationInWindow(
		...
		address token,
		uint256 priceKey
	) external view override returns (PriceLibrary.PriceObservation memory observation) {
    observation = _tokenPriceMaps[factory][token].getPriceInWindow(priceKey);
    require(
  ...
  function getPriceObservationsInRange(
		...
		uint256 timeFrom,
		uint256 timeTo
	) external view override returns (PriceLibrary.PriceObservation[] memory prices) {
    prices = _tokenPriceMaps[factory][token].getPriceObservationsInRange(timeFrom, timeTo);
  }
...
/* ==========  Price Update Queries  ========== */
   ...
   * and at least 30 minutes have passed since the last observation.
   */
  function canUpdatePrice(address factory, address token) external view override returns (bool) {
    if (!factory.pairInitialized(token, _weth)) return false;
    return _tokenPriceMaps[factory][token].canUpdatePrice(uint32(block.timestamp));
  }
  ...
  function canUpdatePrices(
		address[] memory factories,
		address[] memory tokens
	) external view override returns (bool[] memory canUpdateArr) {
    uint256 len = factories.length;
    require(len == tokens.length, "Invalid tokens count");
    ...
    for (uint256 i = 0; i < len; i++) {
      ...
      bool timeAllowed = _tokenPriceMaps[factory][token].canUpdatePrice(uint32(block.timestamp));
      canUpdateArr[i] = timeAllowed && factory.pairInitialized(token, _weth);
    }
  }
...
/* ==========  Price Queries: Singular  ========== */
  ...
  function computeTwoWayAveragePrice(
    ...
    uint256 minTimeElapsed,
    uint256 maxTimeElapsed
  ) external view override validMinMax(minTimeElapsed, maxTimeElapsed) returns (
		PriceLibrary.TwoWayAveragePrice memory
	) {
    return _getTwoWayPrice(factory, token, minTimeElapsed, maxTimeElapsed);
  }
  ...
  function computeAverageTokenPrice(
    ...
    return _getTokenPrice(factory, token, minTimeElapsed, maxTimeElapsed);
  }
  ...
  function computeAverageEthPrice(
    ...
    return _getEthPrice(factory, token, minTimeElapsed, maxTimeElapsed);
  }
...
/* ==========  Price Queries: Multiple  ========== */
  ...
  function computeTwoWayAveragePrices(
    ...
    uint256 maxTimeElapsed
		...
		uint256 len = factories.length;
		require(tokens.length == len, "Invalid tokens count");
    prices = new PriceLibrary.TwoWayAveragePrice[](len);
    for (uint256 i = 0; i < len; i++) {
      prices[i] = _getTwoWayPrice(factories[i], tokens[i], minTimeElapsed, maxTimeElapsed);
    }
  }
  ...
  function computeAverageTokenPrices(
    ...
    for (uint256 i = 0; i < len; i++) {
      averagePrices[i] = _getTokenPrice(factories[i], tokens[i], minTimeElapsed, maxTimeElapsed);
    }
  }
  ...
  function computeAverageEthPrices(
    ...
    averagePrices = new FixedPoint.uq112x112[](len);
    for (uint256 i = 0; i < len; i++) {
      averagePrices[i] = _getEthPrice(factories[i], tokens[i], minTimeElapsed, maxTimeElapsed);
    }
  }
...
/* ==========  Value Queries: Singular  ========== */
  ...
  function computeAverageEthForTokens(
    ...
    uint256 minTimeElapsed,
    uint256 maxTimeElapsed
  ) external view override validMinMax(minTimeElapsed, maxTimeElapsed) returns (uint144) {
    FixedPoint.uq112x112 memory tokenPrice = _getTokenPrice(factory, token, minTimeElapsed, maxTimeElapsed);
  ...
  {
    FixedPoint.uq112x112 memory ethPrice = _getEthPrice(factory, token, minTimeElapsed, maxTimeElapsed);
...
/* ==========  Value Queries: Multiple  ========== */
  ...
  function computeAverageEthForTokens(
    ...
    uint256 minTimeElapsed,
    uint256 maxTimeElapsed
  ) external view override validMinMax(minTimeElapsed, maxTimeElapsed) returns (uint144[] memory averageValuesInWETH) {
		uint256 len = factories.length;
		require(tokens.length == len, "Invalid tokens count");
		require(tokenAmounts.length == len, "Invalid tokenAmounts count");
  ...
  function computeAverageTokensForEth(
    ...
    uint256 maxTimeElapsed
		...
		uint256 len = factories.length;
		require(tokens.length == len, "Invalid tokens count");
		require(wethAmounts.length == len, "Invalid wethAmounts count");
    averageValuesInWETH = new uint144[](len);
    for (uint256 i = 0; i < len; i++) {
...
/* ==========  Internal Functions  ========== */
  function _getTwoWayPrice(
    ...
    (bool foundPrice, uint256 lastPriceKey) = _tokenPriceMaps[factory][token].getLastPriceObservation(
      current.timestamp,
      minTimeElapsed,
      maxTimeElapsed
    );
    require(foundPrice, "BisharesUniswapV2Oracle::_getTwoWayPrice: No price found in provided range.");
    PriceLibrary.PriceObservation memory previous = _tokenPriceMaps[factory][token].priceMap[lastPriceKey];
  ...
  function _getTokenPrice(
    ...
    (uint32 timestamp, uint224 priceCumulativeEnd) = factory.observePrice(token, _weth, false);
    (bool foundPrice, uint256 lastPriceKey) = _tokenPriceMaps[factory][token].getLastPriceObservation(
      timestamp,
      minTimeElapsed,
      maxTimeElapsed
    ...
    PriceLibrary.PriceObservation storage previous = _tokenPriceMaps[factory][token].priceMap[lastPriceKey];
  ...
  function _getEthPrice(
    ...
    (uint32 timestamp, uint224 priceCumulativeEnd) = factory.observePrice(_weth, token, true);
    (bool foundPrice, uint256 lastPriceKey) = _tokenPriceMaps[factory][token].getLastPriceObservation(
      timestamp,
      minTimeElapsed,
      maxTimeElapsed
