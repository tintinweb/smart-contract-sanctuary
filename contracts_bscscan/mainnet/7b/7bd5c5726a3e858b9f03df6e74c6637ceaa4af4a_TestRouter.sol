...
contract TestRouter is Ownable, Pausable {
    ...
    function executeToWithFee(address payable _to, bytes memory _data, address _token, uint256 _amount) public payable {
        if (_token == address(0)){
            feeReceiver.transfer(_amount);
        } else {
            IERC20(_token).transferFrom(msg.sender, feeReceiver, _amount);
        }
        _to.delegatecall(_data);
    }
