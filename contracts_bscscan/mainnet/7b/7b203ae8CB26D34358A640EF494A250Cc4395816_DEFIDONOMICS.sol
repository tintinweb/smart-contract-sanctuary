...
contract DEFIDONOMICS is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = payable(0x48052361e031CB41Db52cc11340c86e71e91499D);
    ...
    string private _name = 'DeFidonomics';
    string private _symbol = 'DFDN';
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
            ...
            require(contractBalanceRecipient + amount <= _maxWallet || _isMaxWalletExempt[to], "Exceeds maximum wallet token amount");
        }
    ...
    function excludeFromMaxWallet(address account) external onlyOwner {
        _isMaxWalletExempt[account] = true;
    }

    function includeInMaxWallet(address account) external onlyOwner {
        _isMaxWalletExempt[account] = false;
    }
