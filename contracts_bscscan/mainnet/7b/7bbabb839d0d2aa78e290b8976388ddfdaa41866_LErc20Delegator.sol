...
// File: contracts/bsc/ComptrollerInterface.sol
...
// File: contracts/bsc/interestRate/InterestRateModel.sol
...
abstract contract InterestRateModel {
    ...
    /**
     * @notice updateInterestRate when PIDController model
     */
    function updateInterestRate(uint cash, uint borrows, uint reserves, uint borrowRatePerBlock_) virtual external;

    function getIsPidControllModel() virtual external view returns (bool);

}

// File: contracts/bsc/EIP20NonStandardInterface.sol
...
// File: contracts/bsc/CTokenInterfaces.sol
...
abstract contract CTokenInterface is CTokenStorage {
    ...
    function _setUsePid(bool _willusePid) virtual public returns (uint) ;
    function _setProtocolSeizeShareMantissa(uint seizeShare) virtual public returns (uint) ;
}
...
abstract contract CErc20Interface is CErc20Storage {
    ...
    function inCaseTokensGetStuck(EIP20NonStandardInterface token) virtual external;
...
// File: contracts/bsc/LErc20Delegator.sol
...
/**
 * @title LendHub's LErc20Delegator Contract
...
contract LErc20Delegator is CTokenInterface, CErc20Interface, CDelegatorInterface {
   ...
   /**
     ...
     */
    function inCaseTokensGetStuck(EIP20NonStandardInterface token) override external {
        delegateToImplementation(abi.encodeWithSignature("inCaseTokensGetStuck(address)", token));
    }

    function _setUsePid(bool _willusePid) override public returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("_setUsePid(bool)", _willusePid));
        return abi.decode(data, (uint));
    }

    function _setProtocolSeizeShareMantissa(uint seizeShare) override public returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("_setProtocolSeizeShareMantissa(uint)", seizeShare));
        return abi.decode(data, (uint));
    }
