...
contract Strategy_BANANA_BUSD {
    // This strategy is for BSC network. BANANA - BUSD pair. RewardToken = "BANANA"
    ...
    address public constant lpToken = 0x7Bd46f6Da97312AC2DBD1749f82E202764C0B914;
    ...
    function _getPriceWBNBToBANANA() internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = token0;
        return 1e36 / (_getAmountsOut(router, 100 * 1e18, path) / 100);
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        ...
        if(_amount > 0) {
            ...
            tCI = _getPriceWBNBToBUSD() * t1V / d + _getPriceWBNBToBANANA() * t0V / d;
        }
        ...
        if(_balance != 0) {
            tCI += _getPriceWBNBToBANANA() * _balance / d;
        }
