...
abstract contract Ownable is Context {
    ...
    function lockOwnership(uint256 time) public virtual onlyOwner {
        ...
        _prevowner = _owner;
        _locked = true;
        _unlocktime = block.timestamp + time;
        _owner = address(0);
    }

    function unlockOwnership() public virtual {
        require(_msgSender() == _prevowner);
        require(_locked);
        _owner = _prevowner;
        _locked = false;
...
contract GamerShiba is ERC20, Ownable {
    ...
    uint256 public constant maxLiquidityFee = 15;

    uint256 public previousLiquidityFee;

    uint256 public firstDividendRewardsFee = 8;
    uint256 public constant maxFirstDividendRewardsFee = 15;
    ...
    uint256 public constant maxMarketingFee = 15;
    ...
    constructor() ERC20("GamerShiba", "ADAC") {
        firstDividendTracker = new FIRSTDividendTracker();

        marketingWallet = address(0x04B836364124426CE4128c7E576675a263145764);
        liquidityWallet = address(0x04B836364124426CE4128c7E576675a263145764);
    ...
    function updateMarketingWallet(address _newWallet) external onlyOwner {
        require(_newWallet != marketingWallet, "GamerShiba: The marketing wallet is already this address");
    ...
    function updateFirstDividendRewardFee(uint8 newFee) external onlyOwner {
        if(newFee > maxFirstDividendRewardsFee) {
            firstDividendRewardsFee = maxFirstDividendRewardsFee;
        }
        else {
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "GamerShiba: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "GamerShiba: Account is already exluded from fees");
    ...
    function excludeFromBuyFees(address account, bool excludefromBuyFees) public onlyOwner {
        require(isExcludedFromBuyFees[account] != excludefromBuyFees, "GamerShiba: Account is already exluded from buy fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GamerShiba: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "GamerShiba: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "GamerShiba: Cannot update gasForProcessing to same value");
...
contract FIRSTDividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("GamerShiba_Dividend_Tracker", "GamerShiba_Dividend_Tracker", 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56) {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) pure internal override {
        require(false, "GamerShiba_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "GamerShiba_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GamerShiba contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GamerShiba_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GamerShiba_Dividend_Tracker: Cannot update claimWait to same value");
