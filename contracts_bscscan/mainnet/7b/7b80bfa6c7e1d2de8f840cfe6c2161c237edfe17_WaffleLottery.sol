...
// File: contracts/library/IWaffleLottery.sol
...
interface IWaffleLottery {
     ...
     * @param percentageToKeepOnNotEnoughTokens: If someone doesn't have tokens at the time of winning, this will define the 
    ...
    event LotteryCreated(
        ...
        uint256 percentageToKeepOnNotEnoughTokens
    );
    ...
    /**
     * @return The amount of tokens that someone needs to hold to buy lottery tickets
...
// File: contracts/WaffleLottery.sol
...
contract WaffleLottery is
    VRFConsumerBase(
        0x404460C6A5EdE2D891e8297795264fDe62ADBB75,
        0x747973a5A2a4Ae1D3a8fDF5479f1514F65Db9C31
    ),
    IWaffleLottery,
    ERC721("Waffle Lottery", "WAFL")
{
    using Counters for Counters.Counter;

    IERC20 private _token;  // The ERC20 token contract
    IWETH private _WETH;    // The WETH contract used to wrap ETH
    ...
    address private _feeReceiver;   // Where all the ticket sale fees will be sent to
    ...
    bool private _rewardClaimed;    // Used to store if the reward has been claimed for the current lottery, resets per lottery
    ...
    Counters.Counter private _lotteryID;    // A counter of how much lotteries there have been, increases by 1 each new lottery
    mapping(uint256 => Lottery) private _lottery;   // A mapping of Lottery ID => The lottery struct that stores information
    mapping(address => mapping(uint256 => uint256)) private _ticketsBought; // A mapping of Address => Lottery ID => Amount of tickets bought


    // Requires that there is no lottery ongoing (ended)
    modifier onlyEnded {
        require(
            _lottery[_lotteryID.current()].endTimestamp <= block.timestamp,
            "WaffleLottery: Lottery needs to have ended for this"
        );
        _;
    }

    // Requires that there is a lottery in progress (on)
    modifier onlyOn {
        require(
            _lottery[_lotteryID.current()].endTimestamp > block.timestamp,
            "WaffleLottery: No lottery is on right now"
        );
        _;
    }

    constructor() {
        _token = IERC20(0x650febe5aD8A17A8f27F17EC80fD208E23E9246C);
        _feeReceiver = address(0xc1029a72220a3eE06E17Cb1d4C686e162C86F3a1);
        _WETH = IWETH(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
        _chainlinkKeyHash = 0xc251acd21ec4fb7f31bb8868288bfdbaeb4fbfec2df3735ddbd4f7dc8d60103c;
        _chainlinkFee = 2e17;
        _rewardClaimed = true;  // The owner can be able to start the lottery
        _tokenID = 1;
        _lastClaimedTokenID = 1;
    }

    // Transfer ETH and, if failed, wrap the ETH and send WETH.
    ...
    // Make the lottery tickets untransferable
    function _beforeTokenTransfer(
        address from,
        address to,
        uint256
        ...
        require(
            ...
            "WaffleLottery: Cannot transfer tickets"
        );
    }

    // Amount of tokens that someone needs to hold to buy lottery tickets
    function tokensNeededForTickets(address user, uint256 amount)
        public
        view
        override
        returns (uint256)
    {
        uint256 minimumTokensBalance = _lottery[_lotteryID.current()].minimumTokensBalance;
        uint256 tokensNeededForEachTicket = _lottery[_lotteryID.current()].tokensNeededForEachTicket;
    ...
    function buyTickets() external payable override onlyOn {
        ...
        require(amountOfTickets != 0, "WaffleLottery: Not enough BNB");
        require(
            _token.balanceOf(_msgSender()) >=
                tokensNeededForTickets(_msgSender(), amountOfTickets),
            "WaffleLottery: You don't have enough tokens"
        );
        uint256 neededETH = amountOfTickets * ticketPrice;
        
        // Refunds unneeded eth
    ...
    // How much fees that will be taken
    function calculateFees(
        uint256 currentTokenID,
        uint256 ticketPrice,
        uint256 ticketFee,
    ...
    // Amount of fees taken for the current lottery
    ...
    {
        require(!_rewardClaimed, "WaffleLottery: Reward already claimed");
        require(!_inClaimReward, "WaffleLottery: Reward is being claimed");
        ...
        require(
            ...
            "WaffleLottery: Put some LINK into the contract"
        );
    ...
    function fulfillRandomness(bytes32, uint256 randomness) internal override {
        // Get a random number in range of the token IDs
        ...
        uint256 tokensNeeded = tokensNeededForTickets(winner, 0);
        if (_token.balanceOf(winner) < tokensNeeded) {
            // He sold his tokens, so give the jackpot to the fee receiver and deduct based on the percentage to keep
            uint256 amountToKeepForNextLottery = (amountWon *
                _lottery[_lotteryID.current()]
                .percentageToKeepOnNotEnoughTokens) / 10000;
    ...
    function createLottery(
        uint256 lotteryLength,  // How long the lottery will be in seconds
        uint256 ticketPrice,    // The price of a ticket in BNB
        uint256 ticketFee,  // The percentage of the ticket price that is sent to the fee receiver
        uint256 percentageToKeepForNextLottery, // The percentage that will be kept as reward for the lottery after
        ...
        uint256 percentageToKeepOnNotEnoughTokens    // If someone doesn't have tokens at the time of winning, this will define the percentage of the reward that will be kept in the contract for the next lottery
    ) external onlyOwner onlyEnded {
        require(
            _rewardClaimed,
            "WaffleLottery: Claim the reward before starting a new lottery"
        );
        require(
            percentageToKeepForNextLottery + ticketFee < 10000,
            "WaffleLottery: You can not take everything or more as a fee"
        );
        require(
            lotteryLength <= 2629744,
            "WaffleLottery: Must be under or equal to 1 month"
        );
        ...
        _lottery[newLotteryID] = Lottery(
            ...
            percentageToKeepOnNotEnoughTokens
        );
    ...
    // Amount of tokens to keep for the next lottery
    ...
    // Anyone can contribute to the jackpot when there is a lottery ongoing
    ...
    // Get info of a lottery (A struct)
    ...
    // In case the token gets changed, this can change it
    function setToken(IERC20 newToken) external onlyOwner {
        _token = newToken;
    }

    // Sets the address that receives all the fees
    ...
    // Extends the duration of the current lottery. Max 1 month.
