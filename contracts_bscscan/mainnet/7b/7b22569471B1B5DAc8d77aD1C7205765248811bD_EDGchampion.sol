...
contract EDGchampion is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x27225d44ac2196F78360Dd54D85118Bfa6eC971C); // Marketing Address
    ...
    uint256 private _maxWalletTotal = 10 * 10**10 * 10**9;
    uint256 private _tFeeTotal;

    string private _name = "EDGchampion";
    string private _symbol = "EDGchampion";
    ...
    function closeTrding() external onlyOwner() {
        tradingOpen = false;
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to])
            require(balanceOf(address(to))+amount <= _maxWalletTotal, "Transfer amount exceeds the maxTxAmount.");
        ...
        if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
            ...
            //antibot
            if (block.timestamp <= launchTime+1) {
                _isSniper[to] = true;
    ...
    function setMaxWalletAmt(uint256 _amount) external onlyOwner() {
        _maxWalletTotal = _amount;
    }
