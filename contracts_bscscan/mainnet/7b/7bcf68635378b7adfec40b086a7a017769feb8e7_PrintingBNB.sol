...
/**
PrintingBNB
As the name suggests, we are pritning BNB for all holders.
Join us and get more BNB just by holding.

TG: https://t.me/PrintingBNB
...
// File: contracts/PrintingBNBividendPayingToken.sol
...
// File: contracts/PrintingBNB.sol
...
contract PrintingBNB is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PrintingBNBDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("PrintingBNB", "PrintingBNB") {
    	...
    	dividendTracker = new PrintingBNBDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PrintingBNB: The dividend tracker already has that address");

        PrintingBNBDividendTracker newDividendTracker = PrintingBNBDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "PrintingBNB: The new dividend tracker must be owned by the PrintingBNB token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PrintingBNB: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PrintingBNB: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PrintingBNB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PrintingBNB: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "PrintingBNB: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PrintingBNB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PrintingBNB: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "PrintingBNB: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of PrintingBNB
            path,
            recipient,
            block.timestamp
        );
...
contract PrintingBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("PrintingBNB_Dividend_Tracker", "PrintingBNBDividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "PrintingBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "PrintingBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PrintingBNB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PrintingBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PrintingBNB_Dividend_Tracker: Cannot update claimWait to same value");
