...
contract ShiboshiDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Shiboshi_Dividend_Tracker", "Shiboshi_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100 * (10**10); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Shiboshi_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Shiboshi_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Shiboshi contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Shiboshi_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "Shiboshi_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Shiboshi is BEP20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    ShiboshiDividendTracker public dividendTracker;

    uint256 public _totalSupply = 100_000_000 * (10**10);
    ...
    BEP20("Shiboshi", "SBSH") 
    {
        ...
        marketingWallet = payable(0xC6E8365B566C6cDd0EcceE21dfF2b6d42685bA58);
    
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new ShiboshiDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Shiboshi: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Shiboshi: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Shiboshi: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Shiboshi: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Shiboshi: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Shiboshi: Cannot update gasForProcessing to same value");
