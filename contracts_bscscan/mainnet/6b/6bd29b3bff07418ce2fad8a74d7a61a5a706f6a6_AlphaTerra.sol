...
contract AlphaTerra is Context, IBEP20, LockToken {
    using SafeMath for uint256;
    using Address for address;
    address payable public marketingAddress = payable(0x158260445a4aa568FD1E1BC162049a7B6cD66705); // Marketing Address
    ...
    string private _name = "Alpha Terra";
    string private _symbol = "Alfat";
    ...
    uint256 private _maxTxAmount = _tTotal.div(1000);//1%
    uint256 private maxLimit =  _tTotal.div(100).mul(2);//2%

    uint256 private minimumTokensBeforeSwap = 1_000_0 * 10**18; 
    ...
    {   
        ...
        uint256 halfLiquidityTokens = contractTokenBalance.div(totalFee).mul(_liquidityFee).div(2);
        uint256 swapableTokens = contractTokenBalance.sub(halfLiquidityTokens);
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(swapableTokens);
        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 ethForLiquidity = newBalance.div(totalFee).mul(_liquidityFee).div(2); 
        addLiquidity(halfLiquidityTokens, ethForLiquidity);
        //Send to Marketing address
        transferToAddressETH(marketingAddress, newBalance.sub(ethForLiquidity));
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        uint256 burnTokens = amount.div(100).mul(_burnFee); 
    ...
    function setAllFeesPercent(uint256 taxFee, uint256 liquidityFee,
    uint256 marketingFee, uint256 burnFee) public onlyOwner() 
    {
        _taxFee = taxFee;
    ...
    function setSaleFeesPercent(uint256 taxFee, uint256 liquidityFee,
    uint256 marketingFee, uint256 burnFee) public onlyOwner() 
    {
        _saleTaxFee = taxFee;
    ...
    function prepareForPreslae() public onlyOwner
    {
        setSwapAndLiquifyEnabled(false);
        setAllFeesPercent(0,0,0,0);
        setSaleFeesPercent(0,0,0,0);
        _maxTxAmount = _tTotal;
    }    

    function afterPreslae() public onlyOwner
    {
        setSwapAndLiquifyEnabled(false);
        setAllFeesPercent(3,3,2,1);
        setSaleFeesPercent(4,3,4,2);
