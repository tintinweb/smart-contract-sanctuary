...
Telegram Group : https://t.me/EthNinja Website: www.ethninja.net  Passively earn ETH on every transaction. Be A holder today
...
contract EthNinja is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public sellFeeIncreaseFactor = 160;
    ...
    constructor() ERC20("EthNinja", "ENINJA") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0x629D02536Ea5de2660bed2dBa78F935EAde4aebF;

    	cakeDividendToken = 0x2170Ed0880ac9A755fd29B2688956BD959F933F8; //
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "EthNinja: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "EthNinja: The new dividend tracker must be owned by the EthNinja token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("EthNinja_Dividend_Tracker", "EthNinja_Dividend_Tracker", 0x2170Ed0880ac9A755fd29B2688956BD959F933F8) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "EthNinja_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "EthNinja_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "EthNinja_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "EthNinja_Dividend_Tracker: Cannot update claimWait to same value");
