...
/**
stealthed low mc . push it 

5% Liquidity
3% Burn
2% Reward
max wallet 20 tokens 
max tx 4 tokens 
*/
...
contract xstealth is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    string private _name = "100Xstealth";
    string private _symbol = "XSTLT";
    ...
    uint256 public _startingMaxWalletTxPercent = 10;
    uint256 public _startingMaxWalletTxTimer = 0;
    ...
    function setMaxStartingTxPercent(uint256 maxStartingTxPercent) external onlyOwner() {
    	_startingMaxWalletTxPercent = maxStartingTxPercent;
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
    	    if (_startingMaxWalletTxTimer + (30 minutes) < block.timestamp)
    	        require(amount <= balanceOf(from).mul(_startingMaxWalletTxPercent).div(100), "Transfer amount exceeds the starting maxTxAmount.");
        ...
        } else if (to == uniswapV2Pair && _startingMaxWalletTxTimer == 0)
		    _startingMaxWalletTxTimer = block.timestamp;
