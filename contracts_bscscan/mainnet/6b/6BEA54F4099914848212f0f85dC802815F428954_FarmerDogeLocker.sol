...
contract FarmerDogeLocker is Ownable{
    ...
    struct Distribution {
        ...
        uint256 alloc;              // allocation point of token supply
    ...
    function initialize(IERC20 _token, address _reflectionToken, uint256 _supply) external onlyOwner {
    ...
    function setSupply(uint256 _supply) external onlyOwner {
        require(isActive == false, "Not update");

        tokenSupply = _supply;

        for(uint i = 0; i < distributors.length; i++) {
            Distribution storage _distribution = distributions[distributors[i]];

            uint256 amount = tokenSupply.mul(_distribution.alloc).div(10000).sub(_distribution.tokenDebt);
    ...
    function addDistribution(address distributor, uint256 allocation, uint256 duration, uint256 unlockRate) external onlyOwner {
        require(totalAllocated.add(allocation) <= 10000, "total allocations exceed 100%");
        ...
        uint256 amount = tokenSupply.mul(_distribution.alloc).div(10000);
        _distribution.reflectionDebt = amount.mul(accReflectionPerShare).div(PRECISION_FACTOR);
    ...
    function removeDistribution(address distributor) external onlyOwner {
        ...
        totalAllocated = totalAllocated.sub(distributions[distributor].alloc);
    ...
    function updateDistribution(address distributor, uint256 allocation, uint256 duration, uint256 unlockRate) external onlyOwner {
        ...
        require(totalAllocated.sub(_distribution.alloc).add(allocation) <= 10000, "total allocations exceed 100%");
        totalAllocated = totalAllocated.sub(_distribution.alloc).add(allocation);
    ...
    function claim() external onlyActive {
        ...
        token.transfer(_distribution.distributor, claimAmt);
