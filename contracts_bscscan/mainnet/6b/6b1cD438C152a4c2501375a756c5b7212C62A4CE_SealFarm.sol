...
// File: contracts/SealToken.sol

pragma solidity 0.6.12;

// SealToken with Governance.
contract SealToken is BEP20 {
    ...
    // Min amount to liquify. (default 10 SEALs)
    uint256 public minAmountToLiquify = 10 ether;
    // The swap router, modifiable. Will be changed to Seal's router when our own AMM release
    IPancakeRouter02 public sealRouter;
    // The trading pair
    address public sealPair;
    // In swap and liquify
    ...
    event SealRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "SEAL::antiWhale: Transfer amount exceeds the maxTransferAmount");
                require(swapEnabled == true, "SEAL::swap: Cannot transfer at the moment");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the SealToken contract.
     */
    constructor() public BEP20("HarpSeal Token", "SEAL") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "SEAL::transfer: Burn value invalid");

            // default 100% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "SEAL::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the seal pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = sealRouter.WETH();

        _approve(address(this), address(sealRouter), tokenAmount);

        // make the swap
        sealRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        sealRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from sealRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "SEAL::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "SEAL::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "SEAL::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateSealRouter(address _router) public onlyOperator {
        sealRouter = IPancakeRouter02(_router);
        sealPair = IPancakeV2Factory(sealRouter.factory()).getPair(address(this), sealRouter.WETH());
        require(sealPair != address(0), "SEAL::updateSealRouter: Invalid pair address.");
        emit SealRouterUpdated(msg.sender, address(sealRouter), sealPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "SEAL::transferOperator: new operator is the zero address");
...
// SealFarm is the master of HarpSeal Token (SEAL). He can make SEAL and he is a fair guy.
//
...
// contract once $SEAL is sufficiently distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract SealFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function pendingSeal(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 sealReward = multiplier.mul(sealPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accSealPerShare = accSealPerShare.add(
                sealReward.mul(1e12).div(lpSupply)
            );
        }
        uint256 pending = user.amount.mul(accSealPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 sealReward =
            multiplier.mul(sealPerBlock).mul(pool.allocPoint).div(
                totalAllocPoint
            );
        seal.mint(devAddr, sealReward.div(10));
        seal.mint(address(this), sealReward);
        pool.accSealPerShare = pool.accSealPerShare.add(
            sealReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to HarpSeal for SEAL allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);		
		payOrLockupPendingSeal(_pid);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(seal)) {
                uint256 transferTax = _amount.mul(seal.transferTaxRate()).div(10000);
    ...
    // Deposit LP tokens to HarpSeal for SEAL allocation with referral.
    ...
    // Withdraw LP tokens from HarpSeal.
	...
	// Pay or lockup pending SEALs.
    function payOrLockupPendingSeal(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (block.number >= startBlockHarvest && block.number <= lastBlockHarvest) {
            if (pending > 0 || user.rewardLockedUp > 0) {        
                ...
                // send rewards
                safeSealTransfer(msg.sender, totalRewards);
    ...
    function updateEmissionRate(uint256 _sealPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, sealPerBlock, _sealPerBlock);
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            seal.mint(referrer, refBonusEarned);
