...
interface ISwap {
  function swapQuery(uint256 amount, address src, address dest) external;
}

// File: contracts/StrategyAddTwoSidesOptimalV2.sol
...
contract StrategyAddTwoSidesOptimalV2 is Ownable, ReentrancyGuard, Strategy {
    ...
    constructor(IUniswapV2Router02 _router, address _goblin, ISwap _swap) public {
        ...
        nonReentrant 
    {
        ...
        }
        {
            ...
            (uint256 swapAmt, bool isReversed) = optimalDeposit(baseToken.myBalance(), fToken.myBalance(), ethReserve, fReserve);
            (address src, address dest) = isReversed ? (fToken, baseToken) : (baseToken, fToken);

            src.safeApprove(address(swap), 0);
            src.safeApprove(address(swap), uint(-1));
            swap.swapQuery(swapAmt, src, dest);
        }
        uint256 swapAmt;
        bool isReversed;
        {
    ...
    function setSwap(ISwap _swap) external onlyOwner nonReentrant {
        swap = _swap;
    }
