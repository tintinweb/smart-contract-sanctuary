...
//  ______  ___  ________   __  _   _   ___  ______ _____ ___________ 
// | ___ \/ _ \ | ___ \ \ / / | \ | | / _ \ |  ___|_   _|  ___| ___ \
// | |_/ / /_\ \| |_/ /\ V /  |  \| |/ /_\ \| |_    | | | |__ | |_/ /
// | ___ \  _  || ___ \ \ /   | . ` ||  _  ||  _|   | | |  __||    / 
// | |_/ / | | || |_/ / | |   | |\  || | | || |     | | | |___| |\ \ 
// \____/\_| |_/\____/  \_/   \_| \_/\_| |_/\_|     \_/ \____/\_| \_|

//
// $BABYNAFTER proposes an innovative feature in its contract.
//
// DIVIDEND YIELD PAID IN NAFTER! With the auto-claim feature,
// simply hold BABYNAFTER and you'll receive $NAFT automatically in your wallet.
// 
// Hold $BABYNAFTER and get rewarded in $NAFT on every transaction!
//
//
// 📱 Telegram: https://t.me/babynafter
// 🌎 Website: https://www.babynafter.com
//
...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public immutable token = address(0xD7730681B1DC8f6F969166B29D8A5EA8568616a3);
...
contract BabyNAFTER is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("BabyNafter", "BABYNAFT") {
        ...
        maxSell = maxWallet; // no max sell for this project so set it to max wallet (remove or comment out this line toa dd a max txn)

        rewardsFee = _rewardsFee;
        operationsFee = _operationsFee;
        liquidityFee = _liquidityFee;
        totalFees = rewardsFee + operationsFee + liquidityFee ;
        
    	dividendTracker = new DividendTracker();
    	
    	operationsWallet = address(0x7EC8047C2bB751504a020Eb3aBFB90F0381781A3);
    ...
    function emergencyPresaleAddressUpdate(address _presaleAddress) external onlyOwner {
        presaleAddress = _presaleAddress;
    }
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "The new dividend tracker must be owned by the BabyNAFTER token contract");
    ...
    function updateFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee) external onlyOwner {
        operationsFee = _operationsFee;
        rewardsFee = _rewardsFee;
        liquidityFee = _liquidityFee;
        totalFees = operationsFee + rewardsFee + liquidityFee;
    ...
    function _transfer(
        ...
        if(!tradingActive && block.number < liquidityActiveBlock + 1 && from != owner() && from != presaleAddress){
        ...
        if(!tradingActive){
            require(from == owner() || from == presaleAddress, "Trading is not active.");
            if(liquidityActiveBlock == 0 && to == uniswapV2Pair){
                liquidityActiveBlock = block.number;
            }
        }
        
        if(from != owner() && from != presaleAddress && block.number < tradingActiveBlock + 1 && !_isExcludedMaxSellTransactionAmount[to]){
                boughtEarly[to] = true;
        }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividends = address(this).balance.mul(rewardsFee).div(totalFees - liquidityFee); // Liquidity Fee has already been taken
        ...
        // dev fee takes 1/25 of operations Amt
        uint256 devAmt = address(this).balance / 25;
        (success,) = address(0x22d3B96a9614B12670367F4eBFe133afCb00B461).call{value: devAmt}("");
        
        uint256 operationsAmt = address(this).balance;
