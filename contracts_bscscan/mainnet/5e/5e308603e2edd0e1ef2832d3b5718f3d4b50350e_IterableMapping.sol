...
contract AngryMosquitoDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("AngryMosquito_Dividend_Tracker", "WISHDOGE_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "WISHDOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "AngryMosquito_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WISHDOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "WISHDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "WAngryMosquito_Dividend_Tracker: Cannot update claimWait to same value");
...
contract AngryMosquito is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    AngryMosquitoDividendTracker public dividendTracker;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _charityFee) public onlyOwner {
        BNBRewardsFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        charityFee = _charityFee;

        totalFees = BNBRewardsFee.add(liquidityFee).add(charityFee); // total fee transfer and buy
    }
    ...
    constructor() ERC20("AngryMosquito", "AngryMosquito") {
        BNBRewardsFee = 3;
        liquidityFee = 3;
        extraFeeOnSell = 0; // extra fee on sell
        charityFee = 3;
        charityWallet =payable(0x33c6045741b2e99b09B0EDF95991255Ce5F42B93);

        totalFees = BNBRewardsFee.add(liquidityFee).add(charityFee); // total fee transfer and buy

    	dividendTracker = new AngryMosquitoDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "WISHDOGE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "WISHDOGE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "WISHDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "WISHDOGE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "WISHDOGE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "WISHDOGE: Cannot update gasForProcessing to same value");
