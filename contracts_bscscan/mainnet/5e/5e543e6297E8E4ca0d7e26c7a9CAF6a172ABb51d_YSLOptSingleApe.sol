...
contract YSLOptSingleApe is YSLOpt {
    ...
    function currentRewardUSD(address _user) public view returns (uint256) {
        ...
        // Get pending
        uint256 pendingBanana = IPancakeMaster(apeMaster).pendingCake(0, address(this));
        uint256 userPendingBanana = (pendingBanana * userDeposited[_user]) / totalDeposited;
        if (userPendingBanana + userCompounds <= DUST) {
            return 0;
        }

        uint256 usersUSD = getPrice(address(want), BUSD, userPendingBanana + userCompounds, apeSwap);
        // Optimisation
