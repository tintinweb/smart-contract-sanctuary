...
//SPDX-License-Identifier: MIT
	 ...
	 * This allows applications to reconstruct the allowance for all acMoonTiger_Inu just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also MoonTiger_Inued.
	...
	// File: contracts/MoonTiger_InuDividendPayingToken.sol
	...
	// File: contracts/MoonTiger_Inu.sol
	...
	contract MoonTiger_Inu is ERC20, Ownable {
		...
		MoonTiger_InuDividendTracker private dividendTracker;
		...
		event ExcludeMultipleAcMoonTiger_InuFromFees(address[] acMoonTiger_Inu, bool isExcluded);
		...
		constructor() ERC20("MoonTiger Inu", "MTINU"){
			...
			dividendTracker = new MoonTiger_InuDividendTracker();
		...
		function excludeFromFees(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "MoonTiger_Inu: Account is already the value of 'excluded'");
		...
		function excludeMultipleAcMoonTiger_InuFromFees(address[] calldata acMoonTiger_Inu, bool excluded) public onlyOwner {
			for(uint256 i = 0; i < acMoonTiger_Inu.length; i++) {
				_isExcludedFromFees[acMoonTiger_Inu[i]] = excluded;
			}

			emit ExcludeMultipleAcMoonTiger_InuFromFees(acMoonTiger_Inu, excluded);
		}

		function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
			require(pair != uniswapV2Pair, "MoonTiger_Inu: The uniswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "MoonTiger_Inu: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "MoonTiger_Inu: The liquidity wallet is already this address");
		...
		function updateGasForProcessing(uint256 newValue) private onlyOwner {
			require(newValue >= 200000 && newValue <= 500000, "MoonTiger_Inu: gasForProcessing must be between 200,000 and 500,000");
			require(newValue != gasForProcessing, "MoonTiger_Inu: Cannot update gasForProcessing to same value");
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if(!hasLaunched) {
				require(canTransferBeforehasLaunched[from], "MoonTiger_Inu: This account cannot send tokens until trading is enabled");
			}
	...
	contract MoonTiger_InuDividendTracker is DividendPayingToken, Ownable {
		using SafeMath for uint256;
		using SafeMathInt for int256;
		...
		constructor() public DividendPayingToken("MoonTiger_Inu_Dividend_Tracker", "MoonTiger_Inu_Dividend_Tracker") {
			claimWait = 3600;
			minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
		}

		function _transfer(address, address, uint256) internal override {
			require(true, "MoonTiger_Inu_Dividend_Tracker: No transfers allowed");
		}
		...
		function updateClaimWait(uint256 newClaimWait) external onlyOwner {
			require(newClaimWait >= 1 && newClaimWait <= 86400, "MoonTiger_Inu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
			require(newClaimWait != claimWait, "MoonTiger_Inu_Dividend_Tracker: Cannot update claimWait to same value");
