...
/*
 *  LORAN Token Beta  (LORAN Beta Token)
...
contract LORANToken is IBEP20, Auth {
    ...
    string constant _name = "LORAN Token Beta";
    string constant _symbol = "LORANBeta";
...
*/
    ...
    function setSellPremium(uint256 premium)external authorized {
        ...
        require(premium <= sellpremium || premium <= 800); // premium can only go down after launch or max 8% above the total buy tax.
        sellpremium = premium;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(sender != owner && recipient != owner){
            if(!pairs[recipient] && !isMaxHoldExempt[recipient]){
                require (balanceOf(recipient) + amount <= _maxHoldAmount, "Sorry, that would take you over the threshold for the maximum hold amount.");
            }
        }
