...
contract SwapsSocks is ERC721, Ownable {
  ...
  constructor(string memory name, string memory symbol, address sockContract, address[] memory blacklisted) ERC721(name, symbol) public {
  ...
  function claimSocks(uint256 _amount) external payable {
      ...
      require(whitelist[msg.sender] >= _amount, "ERC721: You cannot mint!");
      for(uint256 x = 0; x < _amount; x++){
        _mint(msg.sender, ERC721.totalSupply());
