...
contract StorageImpl is IStorageInterface, Ownable {
    constructor(string memory _name, address _assetUnderlying, address _pTokenUnderlying, IControllerInterface _controller) public {
        name = _name;
    ...
    function deleteBailAddress(address _account, uint256 _id, address _remove) public returns (bool){
        mustInSuperList(msg.sender);
        if (_remove == assetUnderlying) {
            //不允许删除债务资产，用于清算时统计借款数量
            // console.log("no need to remove assetUnderlying!");
            return true;
        }
    ...
    function setSupplyConfig(address _account, uint256 _id, address _supplyToken, MSPStruct.supplyConfig memory _config) public {
        mustInSuperList(msg.sender);
        checkId(_account, _id);
        MSPStruct.BailConfig storage bailConfig = bailConfigs[_account][_id];
