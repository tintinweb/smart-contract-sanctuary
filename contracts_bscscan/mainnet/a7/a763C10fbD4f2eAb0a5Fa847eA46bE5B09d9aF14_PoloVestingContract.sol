...
contract PoloVestingContract is ReentrancyGuard {
    ...
    //event to recover
    event RecoverToken(address token, uint256 indexed _amount);
    ...
    constructor() public {
        ...
        start = 1633651200 ;        // 2021-10-08T00:00:00.000Z
        end = 1665187200;        // 2022-10-08T00:00:00.000Z
        cliffDuration = 86400;   // 24*60*60, emit every day
        _revocable = false;       //false cant be revoked once initiated
    }
    ...
    function _availableDrawDownAmount(address _beneficiary) internal view returns (uint256 _amount) {
        ...
        uint256 drawDownRate = vestedAmount[_beneficiary].div(335*24*60*60);
    ...
    function recoverToken(uint256 amount) external {
        require(msg.sender == owner, "VestingContract::recoverToken: Only owner");
        token.transfer(owner, amount);
        emit RecoverToken(address(token), amount);
    }
}