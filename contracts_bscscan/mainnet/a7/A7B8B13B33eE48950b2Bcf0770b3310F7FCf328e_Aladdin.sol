...
contract Aladdin is BEP20{
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "ALADDIN::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    // Constructs the Aladdin contract.
    
    constructor() public BEP20('Aladdin', 'ALADDIN') {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        if (recipient == burnAddr || sender == devAddr || recipient == feeAddr ) {
            ...
            // 1.25% of every transfer ist sent to feeAddr
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= MAXIMUM_TRANSFER_RATE, "ALADDIN::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxTransferAmountRate >= MINIMUM_TRANSFER_RATE, "ALADDIN::updateMaxTransferAmountRate: Max transfer amount rate can not be bleow minimum rate.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "ALADDIN::transferOperator: new operator is the zero address");
