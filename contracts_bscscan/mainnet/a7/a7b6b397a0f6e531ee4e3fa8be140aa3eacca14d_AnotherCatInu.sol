...
contract AnotherCatInu is Context, IERC20, Ownable {
    ...
    string private _name = "AnotherCatInu Cat Inu";
    string private _symbol = "ACATINU";
    ...
    function setBot(address account) external onlyOwner{
        _isBot[account] = true;
    }
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(from != uniswapV2Pair && to == uniswapV2Pair){
                uint256 timePassed = block.timestamp.sub(_lastTrade[from]);
                require(timePassed >= 30 * 1 seconds, 'You must wait 30 seconds between trades');
                require(amount <= maxTxAmount, 'you are exceeding maxTxAmount');
                _lastTrade[from] = block.timestamp;
            }
            if(to != uniswapV2Pair && from == uniswapV2Pair){
                uint256 timePassed = block.timestamp.sub(_lastTrade[to]);
                require(timePassed >= 30 * 1 seconds, 'You must wait 30 seconds between trades');
                require(amount <= maxTxAmount, 'you are exceeding maxTxAmount');
