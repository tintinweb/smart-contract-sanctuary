...
// @dev tg: @Maiktrading
// @dev yt: https://www.libertates.org
...
contract libertates is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;


    string private _name = "Libertates";
    string private _symbol = "libertates";
    ...
    uint256 internal _tokenTotal = 1_000_000_000_000_000e8;
    ...
    // thats the distribution to holders (1000 = 10%)
    ...
    uint256 public _activismoFee = 300;

    uint256 public _taxFeeTotal;
    uint256 public _burnFeeTotal;
    uint256 public _liquidityFeeTotal;
    uint256 public _activismoFeeTotal;

    address public activismoWallet;
    ...
    constructor() public {
        ...
        //paste the wallet adress, that earns the activismoFee here
        activismoWallet = 0x27664E3cBFb609325F0CC6c9f5Fbb86b5452c0d2;
        //paste the wallet ^^^^^^^^^^^^^^^^^^^^^ adress, that earns the activismoFee here
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        //@dev activismo fee
        if(_activismoFee != 0){
            uint256 activismoFee = amount.mul(_activismoFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(activismoFee);
            _reflectionBalance[activismoWallet] = _reflectionBalance[activismoWallet].add(activismoFee.mul(rate));
            if (_isExcluded[activismoWallet]) {
                _tokenBalance[activismoWallet] = _tokenBalance[activismoWallet].add(activismoFee);
            }
            _activismoFeeTotal = _activismoFeeTotal.add(activismoFee);
            emit Transfer(account,activismoWallet,activismoFee);
        }
    ...
    function setactivismoWallet(address account) external onlyOwner {
        activismoWallet = account;
    }
    ...
    function setactivismoFee(uint256 fee) external onlyOwner {
        _activismoFee = fee;
    }
