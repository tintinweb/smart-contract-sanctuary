...
contract BasedZilla is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address private marketingWallet = 0xfBE7d2F32F98dB5d9a0969978bC307a268B97162;

    bool public tradingIsEnabled = false;
    
    bool private swapping;

    BasedZillaDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    uint256 public maxBuyTranscationAmount = 100000000000000 * (10**18); // No limit on buy side
    uint256 public maxSellTransactionAmount = 100000000000000 * (10**18); // No Max Sell
    uint256 public swapTokensAtAmount = 70000000 * (10**18); // Swap to add liquidity
    uint256 public _maxWalletToken = 5000000000000 * (10**18); // 5% of total supply
    ...
    constructor() public ERC20("BasedZilla", "BasedZilla") {

    	dividendTracker = new BasedZillaDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BasedZilla: The dividend tracker already has that address");

        BasedZillaDividendTracker newDividendTracker = BasedZillaDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BasedZilla: The new dividend tracker must be owned by the BasedZilla token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BasedZilla: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BasedZilla: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BasedZilla: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BasedZilla: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BasedZilla: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BasedZilla: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BasedZilla: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BasedZilla: This account cannot send tokens until trading is enabled");
        }
...
contract BasedZillaDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BasedZilla_Dividend_Tracker", "BasedZilla_Dividend_Tracker") {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "BasedZilla_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BasedZilla_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BasedZilla contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 60 && newClaimWait <= 86400, "BasedZilla_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BasedZilla_Dividend_Tracker: Cannot update claimWait to same value");
