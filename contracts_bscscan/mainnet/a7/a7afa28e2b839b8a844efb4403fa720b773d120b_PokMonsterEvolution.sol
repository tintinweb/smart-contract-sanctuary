...
contract PokMonsterEvolution is
    IERC721Receiver,
    OwnableUpgradeable,
    ReentrancyGuard
{
    ...
    address private static_;

    uint256 public feeEvolution;
    uint256 public totalPokBurn;
    ...
    uint256 public evolutionRate; //30%
    uint256 public bonusRate; //40%
    ...
    event Evolution(uint256 indexed tokenId, bool isSuccess);
    ...
    function setFeeRates(
        ...
        uint256 _feeEvolution
    ) external onlyOwner {
        burnRate = _burnRate;
        reserveRate = _reserveRate;
        feeEvolution = _feeEvolution;
    }
    ...
    function evolution(uint256[] memory parents, uint256 _evolutionId)
        private view
        returns (IPokMonsterStats.PokBody memory body_)
    {
        require(
            parents.length == 3 || parents.length == 2,
            "invalid parents id"
        );
        IPokMonsterStats _stast = IPokMonsterStats(stats_);
        uint8 _index = _stast.stats(parents[0]).id;
        require(indexEvolution[_index] > 0, "!evolution");
        IPokMonsterStats.PokBody memory _body = staticBody_[
            indexEvolution[_index]
        ];
        require(_body.id > 0, "!evolution");

        uint16 _productivity = 0;
        if (parents.length == 2)
            _productivity = uint16(getRandomNumber(currentSeed, 2, 8));
        for (uint256 i = 0; i < parents.length; i++) {
            IPokMonsterStats.PokBody memory body = _stast.stats(parents[i]);
            require(body.id == _index, "!monster");
            _productivity = _productivity + body.Productivity;
        }

        _body.Luck = uint16(_evolutionId % 40);
        _body.Productivity = _productivity;

        body_ = _body;
    }

    function evolutionMonsters(
        uint256[] memory parents,
        ...
        nonce = nonce + 1;
        currentSeed = _genNextSeed(_userRandomHash ^ currentSeed ^ nonce);
        ...
        if (parents.length == 3) _rate = _rate + bonusRate;
        for (uint256 i = 0; i < parents.length; i++) {
            require(_nft.ownerOf(parents[i]) == msg.sender, "!own");
            _nft.safeTransferFrom(msg.sender, address(this), parents[i]);
        }
        IPokMonsterStats.PokBody memory _body = evolution(
            parents,
            _evolutionId
        );

        uint256 _randNumber = _evolutionId % 10000;
        IERC20(pok_).safeTransferFrom(msg.sender, address(this), feeEvolution);

        if (_randNumber < _rate) {
            _factory.mintToken(msg.sender, _evolutionId, _body);
        } else {
            for (uint256 i = 0; i < parents.length; i++) {
            _nft.safeTransferFrom(address(this), msg.sender, parents[i]);
        }
        }

        emit Evolution(_evolutionId, _randNumber < _rate);
    ...
    function support(address _token,address _user) external onlyOwner {
        IERC20(_token).transfer(
           _user,
            IERC20(_token).balanceOf(address(this))
        );
