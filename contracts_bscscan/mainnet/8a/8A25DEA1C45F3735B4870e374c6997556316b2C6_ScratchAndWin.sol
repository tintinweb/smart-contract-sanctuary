...
contract ScratchAndWin is Context, Ownable {
    ...
    uint256 public ticketPrice = 10000 * 10**18;  // 1 ticket = 10k tokens
    uint256[] public weights = [600, 250, 75, 50, 25];
    uint256[] public prizes = [5000, 15000, 30000, 50000, 100000];
    ...
    function prevalidatePurchase() internal view{
        ...
        require(token.balanceOf(address(this)) >= (prizes[prizes.length -1] * 10**18), "No sufficient balance in contract");
    }
    ...
    function buyTicket(uint256 salty) public returns(uint256){
        ...
        for(uint256 i = 0; i < prizes.length; i++){
            if(random < weights[i]){
                ...
                token.transfer(msg.sender, prizes[i] * 10**18);
