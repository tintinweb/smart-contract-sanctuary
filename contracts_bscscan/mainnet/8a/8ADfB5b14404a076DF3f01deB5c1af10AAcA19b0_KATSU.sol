...
contract KATSU is Context, IERC20, Ownable {
    ...
    event PayoutReward(address indexed from, address indexed to, uint value);
    ...
    constructor() payable {
        ...
        _burnFee = 5 * 10 ** 2;                     // 5%
        _liquidityFee = 2 * 10 ** 2;                // 2%
        _communityMarketingFee = 2 * 10 ** 2;       // 2%
        _totalFee = _burnFee.add(_liquidityFee).add(_communityMarketingFee);
        _extraSellFee = 3 * 10 ** 2;                // 1 devFee + 1 _rewardFee + 1 _communityMarketingFee
    
        _reduceBurnFee = 10 * 10 ** 1;               // reduction by 20% of initialFee
        _reduceLiquidiyFee = 4 * 10 ** 1;           // reduction by 20% of initialFee
        _reduceCommunityMarketingFee = 2 * 10 * 1;  // reduction by 10% of initialFee
    ...
    function addContract(address _contractaddress) onlyAdmin_ external {
        require(!_isEcoContract[_contractaddress], 'address already part of Eco');
        _isEcoContract[_contractaddress] = true;
    }
    
    function removeContract(address _contractaddress) onlyAdmin_ external {
    ...
    function rewardMember(address to, uint256 reward) isContractOrAdminCall_ public returns (bool){
        require(_communityMarketingReserve >= reward, 'Not enough Balance');
        _communityMarketingReserve = _communityMarketingReserve.sub(reward);
        _transfer(address(this), to, reward);
        emit Transfer(address(this), to, reward);
        
        return true;
    }
    
    function increaseComMarkReserve(uint256 amount) isContractOrAdminCall_ public returns (bool) {
        require(balanceOf(_msgSender()) >= amount, 'not enough balance');
        
        _balances[_msgSender()] = _balances[_msgSender()].sub(amount);
        _balances[address(this)] = _balances[address(this)].add(amount);
        
        _communityMarketingReserve = _communityMarketingReserve.add(amount);
    ...
    function checkDeadWalletforFeeUpdate() private returns (bool) {
        ...
        uint256 init_liquidityFee = _liquidityFee;
        uint256 init_communityMarketingFee = _communityMarketingFee;
 
        if (                                            
            _reduceFeeCounter == 1 &&
            _amountBurned >= _totalSupply.div(5)
            ) { 
                _burnFee = _burnFee.sub(_reduceBurnFee);
                _liquidityFee = _liquidityFee.sub(_reduceLiquidiyFee);
                _communityMarketingFee = _communityMarketingFee.sub(_reduceCommunityMarketingFee);
                _reduceFeeCounter = _reduceFeeCounter.add(1);
                emit AutomatedFeeUpdate('burnFee', init_burnFee, _burnFee);
                emit AutomatedFeeUpdate('liquidityFee', init_liquidityFee, _liquidityFee);
                emit AutomatedFeeUpdate('communityMarketingFee', init_communityMarketingFee, _communityMarketingFee);
            }
        if (                                            
            _reduceFeeCounter == 2 &&
            _amountBurned >= _totalSupply.div(10).mul(4)
            ) { 
                _burnFee = _burnFee.sub(_reduceBurnFee);
                _liquidityFee = _liquidityFee.sub(_reduceLiquidiyFee);
                _communityMarketingFee = _communityMarketingFee.sub(_reduceCommunityMarketingFee);
        ...
        if (                                            
            _reduceFeeCounter == 3 &&
            _amountBurned >= _totalSupply.div(10).mul(6)
            ) { 
                _burnFee = _burnFee.sub(_reduceBurnFee);
                _liquidityFee = _liquidityFee.sub(_reduceLiquidiyFee);
                _communityMarketingFee = _communityMarketingFee.sub(_reduceCommunityMarketingFee);
        ...
        if (                                            
            _reduceFeeCounter == 4 &&
            _amountBurned >= _totalSupply.div(10).mul(8)
            ) { 
                _burnFee = _burnFee.sub(_reduceBurnFee);
                _liquidityFee = _liquidityFee.sub(_reduceLiquidiyFee);
                _communityMarketingFee = _communityMarketingFee.sub(_reduceCommunityMarketingFee);
        ...
        if (                                            
            _reduceFeeCounter == 4 &&
            _amountBurned >= _totalSupply.mul(21).div(100000)) {
                _burnFee = 0;
            }
    ...
    function checkAndActionLiquify(address sender) private returns (bool) {
        uint256 contractTokenBalance = balanceOf(address(this)).sub(_devFeeAmountReserve).sub(_communityMarketingReserve);
        uint256 contractMaxTxBalance = (_totalSupply.sub(_amountBurned)).div(10000);
        uint256 contractMinTokentoSell = (_totalSupply.sub(_amountBurned)).div(100000);
        uint256 contractDevMaxTxBalance = _devFeeAmountReserve;
        uint256 contractDevMinTokentoSell = 10000 * 10 ** 9;
        uint256 swapamount;
        uint256 swapamount2;
        ...
        bool overMinDevTokenBalance = contractDevMaxTxBalance >= contractMinTokentoSell;
        
        bool canswap = overMinTokenBalance && !inSwapAndLiquify && sender != uniswapV2Pair;
        bool canswap2 = overMinDevTokenBalance && !inSwapandPay && sender != uniswapV2Pair;
        ...
        if (canswap2) {
            swapamount2 = contractDevMinTokentoSell;
            swapAndSendtoFee(swapamount2);
        }
    ...
    function _calculateAndCollectFee(address sender, address recipient, uint256 _amount) private returns(uint256) { 
            
        uint256 burnAmount = _amount.mul(_burnFee).div(100 * 10 ** 2);
        uint256 LiquidityFee = _amount.mul(_liquidityFee).div(100 * 10 ** 2);
        uint256 CommunityMarketingFee = _amount.mul(_communityMarketingFee).div(100 * 10 ** 2);
        uint256 TotalFee = LiquidityFee.add(CommunityMarketingFee);
        uint256 TxAmount = _amount.sub(TotalFee).sub(burnAmount);
        uint256 ExtraFee = _amount.mul(_extraSellFee).div(100 * 10 ** 2);
        
    	if(automatedMarketMakerPairs[recipient]){
    	   TxAmount = TxAmount.sub(ExtraFee);
        ...
        if (TxAmount != _amount) {
            _balances[sender] = _balances[sender].sub(TotalFee).sub(burnAmount);
            _balances[address(this)] = _balances[address(this)].add(TotalFee).add(burnAmount);
            
            tokenburn(burnAmount);

            _liquidityFeeAmountReserve = _liquidityFeeAmountReserve.add(LiquidityFee);
            _communityMarketingReserve = _communityMarketingReserve.add(CommunityMarketingFee);
            if (automatedMarketMakerPairs[recipient]) {
                uint256 ExtraCommunityMarketingFee = ExtraFee.div(3).mul(2);
                uint256 DevFee = ExtraFee.sub(ExtraCommunityMarketingFee);
                _devFeeAmountReserve = _devFeeAmountReserve.add(DevFee);
                _communityMarketingReserve = _communityMarketingReserve.add(ExtraCommunityMarketingFee);
            }
    ...
    function swapAndSendtoFee(uint256 swapTokenAmount) private lockTheSwap2 {
        uint256 initialETHBalance = address(this).balance;
        swapTokensForEth(swapTokenAmount);
        uint256 ETHBalanceToPayout = address(this).balance.sub(initialETHBalance);
        (bool success, /*bytes memory data*/) = payable(admin).call{value: ETHBalanceToPayout, gas: 30000}("");
