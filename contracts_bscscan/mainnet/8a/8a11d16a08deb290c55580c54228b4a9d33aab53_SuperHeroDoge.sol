...
contract SuperHeroDoge is Context, IERC20, Ownable {
    ...
    address payable public charityAddress = payable(0xD0e67540A237549F9C5310fFFd46E543EBd71350);
    ...
    string private _name = "SaveGamer";
    string private _symbol = "SGAMER";
    ...
    uint256 public _totalFee = _burnFee + _charityFee + _liquidityFee + _taxFee + _buybackFee;

    uint256 public _maxTxAmount = _tTotal.div(1000); // 0.1%
    ...
    function swapTokens(uint256 tokensToSwap) private lockTheSwap {
        ...
        uint256 bnbForCharity = onePart.mul(_charityFee);

        transferToCharity(bnbForCharity);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        _distributeFee(vs.rCharity, vs.tCharity);
    ...
    function _getValues(uint256 tAmount) private view returns (Values memory) {
        ...
        RValues memory rvs = _getRValues(tAmount, tvs.tFee, tvs.tBurn, tvs.tCharity, tvs.tBuyback, tvs.tLiquidity, _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (TValues memory) {
        ...
        uint256 allFee = tFee.add(tBurn).add(tCharity).add(tLiquidity).add(tBuyback);
        uint256 tTransferAmount = tAmount.sub(allFee);
        return TValues(tTransferAmount, tFee, tBurn, tCharity, tBuyback, tLiquidity);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tBurn, uint256 tCharity, uint256 tBuyback, uint256 tLiquidity, uint256 currentRate) private pure returns (RValues memory) {
        ...
        uint256 allFee = rFee.add(rBurn).add(rCharity).add(rLiquidity).add(rBuyback);
        uint256 rTransferAmount = rAmount.sub(allFee);
        return RValues(rAmount, rTransferAmount, rFee, rBurn, rCharity, rBuyback, rLiquidity);
    }
    ...
    function _distributeFee(uint256 tCharity, uint256 rCharity) private {
    ...
    function restoreAllFee() private {
        ...
        _totalFee = _burnFee + _charityFee + _liquidityFee + _taxFee + _buybackFee;
    }
    ...
    function setFeePercents(uint256 burnFee, uint256 charityFee, uint256 liquidityFee, uint256 taxFee, uint256 buybackFee) external onlyOwner() {
        _burnFee = burnFee;
        _charityFee = charityFee;
        _liquidityFee = liquidityFee;
        _taxFee = taxFee;
        _buybackFee = buybackFee;
        uint256 newTotalFee = _burnFee + _charityFee + _liquidityFee + _taxFee + _buybackFee;
    ...
    function transferToCharity(uint256 amount) private {
