...
import "./interfaces/IEluniumReferral.sol";
import "./interfaces/IStrategy.sol";
import "./interfaces/IElunium.sol";
import "./interfaces/IRock.sol";
import "./interfaces/IEluniumFarm.sol";
import "./interfaces/IEluniumFarmCallback.sol";
import "./interfaces/IStandardFarm.sol";

/// @notice EluniumFarm is a smart contract for distributing ELUNIUM by asking user to stake the BEP20-based token.
contract EluniumFarm is
  IEluniumFarm,
  OwnableUpgradeable,
  ReentrancyGuardUpgradeable
{
  ...
  struct PoolInfo {
    ...
    uint256 lastRewardBlock; // Last block number that ELUNIUM distribution occurs.
    uint256 accEluniumPerShare; // Accumulated ELUNIUM per share, times 1e12. See below.
    uint256 accEluniumPerShareTilBonusEnd; // Accumated ELUNIUM per share until Bonus End.
    uint256 allocBps; // Pool allocation in BPS, if it's not a fixed bps pool, leave it 0
    address strat; // Strategy address that will auto compound want tokens
  }

  // Elunium referral
  IEluniumReferral public eluniumReferral;

  // ELUNIUM token.
  IElunium public elunium;
  // Rock token.
  IRock public rock;
  // Dev address.
  ...
  IElunium public activeElunium; // active Elunium token to be used as a reward. (eluniumv1)
  IRock public activeRock; // active Rock to be used as a rock token
  ...
  event SetEluniumPerBlock(
    uint256 prevEluniumPerBlock,
    uint256 currentEluniumPerBlock
  );
  event Harvest(
    address indexed caller,
  ...
  /// @dev Initializer to create EluniumEluniumFarm instance + add pool(0)
  /// @param _elunium The address of ELUNIUM
  /// @param _devAddr The address that will ELUNIUM dev fee
  /// @param _eluniumPerBlock The initial emission rate
  /// @param _startBlock The block that ELUNIUM will start to release
  function initialize(
    IElunium _elunium,
    IRock _rock,
    address _devAddr,
    ...
    // add ELUNIUM->ELUNIUM pool
    pools.add(address(_elunium));
    poolInfo[address(_elunium)] = PoolInfo({
      allocPoint: 0,
  ...
  modifier onlyPermittedTokenFunder(address _beneficiary, address _stakeToken) {
    require(
      ...
      "EluniumFarm::onlyPermittedTokenFunder: caller is not permitted"
    );
    _;
  }
  ...
  modifier onlyStakeTokenCallerContract(address _stakeToken) {
    require(
      ...
      "EluniumFarm::onlyStakeTokenCallerContract: bad caller"
    );
    _;
  }

  /// @notice set elunium referral implement
  /// @param _referral new elunium referral address
  function setReferral(address _referral) external onlyOwner {
    eluniumReferral = IEluniumReferral(_referral);
  }
    ...
    onlyOwner
  {
    require(
      ...
      "EluniumFarm::addStakeTokenCallerContract: the pool doesn't allow a contract caller"
    );
  ...
  /// @param _caller a stake token contract caller
  function removeStakeTokenCallerContract(address _stakeToken, address _caller)
    external
    onlyOwner
  {
    require(
      stakeTokenCallerAllowancePool[_stakeToken],
      "EluniumFarm::removeStakeTokenCallerContract: the pool doesn't allow a contract caller"
    );
    LinkList.List storage list = stakeTokenCallerContracts[_stakeToken];
  ...
  /// @dev Set lockUpBps.
  /// @param _lockUpBps The new lockUpBps.
  ...
  function setDev(address _devAddr) external {
    require(
      _msgSender() == devAddr,
      "EluniumFarm::setDev::only prev dev can changed dev address"
    );
    devAddr = _devAddr;
  }

  /// @dev Set ELUNIUM per block.
  /// @param _eluniumPerBlock The new emission rate for ELUNIUM
  function setEluniumPerBlock(uint256 _eluniumPerBlock) external onlyOwner {
    massUpdatePools();
    emit SetEluniumPerBlock(eluniumPerBlock, _eluniumPerBlock);
    eluniumPerBlock = _eluniumPerBlock;
  }
    ...
    onlyOwner
  {
    require(
      ...
      "EluniumFarm::setPoolAllocBps::_stakeToken must not be address(0) or address(1)"
    );
    require(
      pools.has(_stakeToken),
      "EluniumFarm::setPoolAllocBps::pool hasn't been set"
    );
    ...
    require(
      ...
      "EluniumFarm::setPoolallocBps::accumAllocBps must < 10000"
    );
    massUpdatePools();
    if (_allocBps == 0) {
  ...
  function setBonus(
    ...
    require(
      ...
      "EluniumFarm::setBonus::bad bonusEndBlock"
    );
    require(_bonusMultiplier > 1, "EluniumFarm::setBonus::bad bonusMultiplier");
    require(
      _bonusLockUpBps <= 10000,
      "EluniumFarm::setBonus::bad bonusLockUpBps"
    );
  ...
  /// @param _stakeToken The token that needed to be staked to earn ELUNIUM.
  /// @param _allocPoint The allocation point of a new pool.
  /// @param _strat The strategy of this pool.
  function addPool(
    address _stakeToken,
    uint256 _allocPoint,
    address _strat
    ...
    require(
      _stakeToken != address(0) && _stakeToken != address(1),
      "EluniumFarm::addPool::_stakeToken must not be address(0) or address(1)"
    );
    require(
      !pools.has(_stakeToken),
      "EluniumFarm::addPool::_stakeToken duplicated"
    );
    ...
    // Add pool to elunium referral
    eluniumReferral.addPool(_stakeToken);

    emit AddPool(_stakeToken, _allocPoint, totalAllocPoint, _strat);
  }

  /// @dev Update the given pool's ELUNIUM allocation point. Can only be called by the owner.
    ...
    onlyOwner
  {
    require(
      _stakeToken != address(0) && _stakeToken != address(1),
      "EluniumFarm::setPool::_stakeToken must not be address(0) or address(1)"
    );
    require(
      pools.has(_stakeToken),
      "EluniumFarm::setPool::_stakeToken not in the list"
    );

    massUpdatePools();

    totalAllocPoint = totalAllocPoint.sub(poolInfo[_stakeToken].allocPoint).add(
        _allocPoint
      );
  ...
  function removePool(address _stakeToken) external override onlyOwner {
    require(
      _stakeToken != address(elunium),
      "EluniumFarm::removePool::can't remove ELUNIUM pool"
    );
    require(
      pools.has(_stakeToken),
      "EluniumFarm::removePool::pool not add yet"
    );
    require(
      IERC20Upgradeable(_stakeToken).balanceOf(address(this)) == 0,
      "EluniumFarm::removePool::pool not empty"
    );
    ...
    poolInfo[_stakeToken].accEluniumPerShare = 0;
    poolInfo[_stakeToken].accEluniumPerShareTilBonusEnd = 0;
  ...
  function _updatePoolAlloc(
    ...
    while (curr != LinkList.end) {
      ...
      totalAllocPoint = totalAllocPoint.sub(poolInfo[curr].allocPoint).add(
        poolPoints
      );
  ...
  /// @dev View function to see pending ELUNIUMs on frontend.
  /// @param _stakeToken The stake token
  /// @param _user The address of a user
  function pendingElunium(address _stakeToken, address _user)
    external
    view
    override
    returns (uint256)
  {
    ...
    if (pool.strat == address(0)) {
      totalStakeToken = IERC20Upgradeable(_stakeToken).balanceOf(address(this));
    } else {
      (uint256 stratAmt, ) = IStandardFarm(
        IStrategy(pool.strat).farmContractAddress()
      ).userInfo(IStrategy(pool.strat).pid(), address(pool.strat));
      totalStakeToken = stratAmt;
    }
    if (block.number > pool.lastRewardBlock && totalStakeToken != 0) {
      uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
      uint256 eluniumReward = multiplier
        .mul(eluniumPerBlock)
        .mul(pool.allocPoint)
        .div(totalAllocPoint);
      accEluniumPerShare = accEluniumPerShare.add(
        eluniumReward.mul(1e12).div(totalStakeToken)
      );
    }
  ...
  function updatePool(address _stakeToken) public override {
    ...
    uint256 eluniumReward = multiplier
      ...
      .mul(pool.allocPoint)
      .div(totalAllocPoint);
    activeElunium.mint(devAddr, eluniumReward.mul(devFeeBps).div(10000));
    activeElunium.mint(address(activeRock), eluniumReward);
    pool.accEluniumPerShare = pool.accEluniumPerShare.add(
      eluniumReward.mul(1e12).div(totalStakeToken)
    );

    // Clear bonus & update accEluniumPerShareTilBonusEnd.
    if (block.number <= bonusEndBlock) {
      activeElunium.lock(
        devAddr,
        eluniumReward.mul(bonusLockUpBps).mul(15).div(1000000)
      );
    ...
    if (block.number > bonusEndBlock && pool.lastRewardBlock < bonusEndBlock) {
      uint256 eluniumBonusPortion = bonusEndBlock
      ...
      activeElunium.lock(
        devAddr,
        eluniumBonusPortion.mul(bonusLockUpBps).mul(15).div(1000000)
      );
      pool.accEluniumPerShareTilBonusEnd = pool
        .accEluniumPerShareTilBonusEnd
        .add(eluniumBonusPortion.mul(1e12).div(totalStakeToken));
    }

    pool.lastRewardBlock = block.number;
  }

  /// @dev Deposit token to get ELUNIUM.
  /// @param _stakeToken The stake token to be deposited
  /// @param _amount The amount to be deposited
  /// @param _referrer The referrer of for
  function deposit(
    address _for,
    address _stakeToken,
    uint256 _amount,
    ...
    require(
      _stakeToken != address(elunium),
      "EluniumFarm::deposit::use depositElunium instead"
    );
    require(pools.has(_stakeToken), "EluniumFarm::deposit::no pool");
    ...
    if (
      _amount > 0 &&
      address(eluniumReferral) != address(0) &&
      _referrer != address(0) &&
      _referrer != _for
    ) {
      eluniumReferral.addReferral(_for, _referrer);
    }

    if (user.fundedBy != address(0))
      require(user.fundedBy == _msgSender(), "EluniumFarm::deposit::bad sof");
    ...
    if (_amount > 0 && pool.strat != address(0)) {
      ...
      IERC20Upgradeable(_stakeToken).safeIncreaseAllowance(pool.strat, _amount);
      uint256 sharesAdded = IStrategy(pool.strat).deposit(msg.sender, _amount);
    ...
    user.bonusDebt = user.amount.mul(pool.accEluniumPerShareTilBonusEnd).div(
      1e12
    );
  ...
  /// @dev Withdraw token from EluniumEluniumFarm.
  ...
  function withdraw(
    ...
    require(
      _stakeToken != address(elunium),
      "EluniumFarm::withdraw::use withdrawElunium instead"
    );
    require(pools.has(_stakeToken), "EluniumFarm::withdraw::no pool");
    ...
    require(
      ...
      "EluniumFarm::withdraw::only funder"
    );
    require(user.amount >= _amount, "EluniumFarm::withdraw::not good");
    ...
    if (_amount > 0 && pool.strat != address(0)) {
      ...
      uint256 wantBal = IERC20Upgradeable(_stakeToken).balanceOf(address(this));
      if (wantBal < _amount) {
        _amount = wantBal;
      }
    } else {
  ...
  /// @dev Deposit ELUNIUM to get even more ELUNIUM.
  /// @param _amount The amount to be deposited
  /// @param _referrer The referrer of for
  function depositElunium(
    address _for,
    uint256 _amount,
    address _referrer
  )
    external
    override
    onlyPermittedTokenFunder(_for, address(elunium))
    nonReentrant
  {
    if (
      _amount > 0 &&
    ...
    _assignActiveToken();
    PoolInfo storage pool = poolInfo[address(elunium)];
    UserInfo storage user = userInfo[address(elunium)][_for];

    if (user.fundedBy != address(0))
      require(
        user.fundedBy == _msgSender(),
        "EluniumFarm::depositElunium::bad sof"
      );

    uint256 lastRewardBlock = pool.lastRewardBlock;
    updatePool(address(elunium));

    if (user.amount > 0) _harvest(_for, address(elunium), lastRewardBlock);
    if (user.fundedBy == address(0)) user.fundedBy = _msgSender();
    if (_amount > 0) {
      IERC20Upgradeable(address(elunium)).safeTransferFrom(
        address(_msgSender()),
        address(this),
        _amount
      );
    ...
    rock.mint(_for, _amount);

    emit Deposit(_msgSender(), _for, address(elunium), _amount);
  }

  /// @dev Withdraw ELUNIUM
  /// @param _amount The amount to be withdrawn
  function withdrawElunium(address _for, uint256 _amount)
    external
    override
    nonReentrant
  {
    ...
    require(
      user.fundedBy == _msgSender(),
      "EluniumFarm::withdrawElunium::only funder"
    );
    require(user.amount >= _amount, "EluniumFarm::withdrawElunium::not good");

    uint256 lastRewardBlock = pool.lastRewardBlock;
    updatePool(address(elunium));
    _harvest(_for, address(elunium), lastRewardBlock);

    if (_amount > 0) {
      user.amount = user.amount.sub(_amount);
      IERC20Upgradeable(address(elunium)).safeTransfer(
        address(_msgSender()),
        _amount
      );
    }
    ...
    rock.burn(_for, _amount);

    emit Withdraw(_msgSender(), _for, address(elunium), user.amount);
  }

  /// @dev Harvest ELUNIUM earned from a specific pool.
  ...
  /// @dev Harvest ELUNIUM earned from pools.
  ...
  /// @dev Internal function to harvest ELUNIUM
  ...
  function _harvest(
    ...
    require(
      user.fundedBy == _msgSender(),
      "EluniumFarm::_harvest::only funder"
    );
    require(user.amount > 0, "EluniumFarm::_harvest::nothing to harvest");
    uint256 pending = user.amount.mul(pool.accEluniumPerShare).div(1e12).sub(
      user.rewardDebt
    );
    require(
      pending <= activeElunium.balanceOf(address(activeRock)),
      "EluniumFarm::_harvest::wait what.. not enough ELUNIUM"
    );
    uint256 bonus = user
      .amount
      .mul(pool.accEluniumPerShareTilBonusEnd)
      .div(1e12)
      .sub(user.bonusDebt);
    activeRock.safeEluniumTransfer(_for, pending);
    uint256 lockAmount = pending.mul(lockUpBps).div(1000);
    activeElunium.lock(_for, lockAmount);
    ...
    if (bonus > 0) {
      activeElunium.lock(_for, bonus.mul(bonusLockUpBps).div(10000));
    }
    if (address(eluniumReferral) != address(0)) {
      address referrer = eluniumReferral.getReferrer(_for);
      if (referrer != address(0)) {
        eluniumReferral.updateCommission(_for, _stakeToken, pending);
      }
    }
  ...
  function _eluniumFarmCallee(
    ...
    (bool success, ) = _caller.call(
      abi.encodeWithSelector(
        IEluniumFarmCallback.eluniumFarmCall.selector,
        _stakeToken,
        _for,
        _pending,
        _lastRewardBlock
    ...
    require(
      success,
      "EluniumFarm::_eluniumFarmCallee:: failed to execute eluniumFarmCall"
    );
  }
    ...
    nonReentrant
  {
    ...
    require(
      user.fundedBy == _msgSender(),
      "EluniumFarm::emergencyWithdraw::only funder"
    );
    IERC20Upgradeable(_stakeToken).safeTransfer(address(_for), user.amount);

    emit EmergencyWithdraw(_for, _stakeToken, user.amount);

    // Burn Rock if user emergencyWithdraw ELUNIUM
    if (_stakeToken == address(elunium)) {
      rock.burn(_for, user.amount);
    }
  ...
  /// @dev This is a function for mining an extra amount of elunium, should be called only by stake token caller contract (boosting purposed)
  ...
  function mintExtraReward(
    ...
    _assignActiveToken();
    uint256 lockAmount = _amount.mul(lockUpBps).div(1000);

    // mint & lock(if any) an extra reward
    activeElunium.mint(_to, _amount);
    activeElunium.lock(_to, lockAmount);

    emit MintExtraReward(_msgSender(), _stakeToken, _to, _amount);
  }

  /// if reward token hasn't been set, set it as a elunium
  function _assignActiveToken() internal {
    if (address(activeElunium) == address(0)) activeElunium = elunium;
    if (address(activeRock) == address(0)) activeRock = rock;
  }
}
...
interface IEluniumReferral {
  ...
  function updateNftCommission(address _user, uint256 _amount) external;

  function addReferral(address _user, address _referrer) external;

  function addNftReferral(address _user, address _referrer) external;
...
interface IRock {
  // ROCK specific functions
  function safeEluniumTransfer(address _account, uint256 _amount) external;
...
interface IEluniumFarm {
  ...
  function withdrawElunium(address _for, uint256 _amount) external;
...
interface IEluniumFarmCallback {
  function eluniumFarmCall(
    address stakeToken,
    address userAddr,
