...
/**
 Bull ecosystem, Contract Bullrise.
*/
...
library Address {
    ...
    function isContract(address account) internal view returns (bool) {
        ...
        // and 0xc5d24606f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470 is returned
...
contract BullRise is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(address(this)); // Marketing Address
	...
	uint256 private buyBackMinimum = 1 * 10**9;
	uint256 private _previousbuyBackMinimum = buyBackMinimum;
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
            ...
            if (buyBackEnabled && balance > buyBackMinimum) {
        ...
        if(_isExcludedFromFundFee[from]){	
         ...
         //send tax to owner	
        uint256 amountSentToOwner = amount.mul(_fundFee).div(100);	
        uint256 remainingAmount =  amount.sub(amountSentToOwner);	
            	

        _tokenTransfer(from,_fundWallet,amountSentToOwner,takeFee);
        //transfer amount, it will take tax, burn, liquidity fee	
        _tokenTransfer(from,to,remainingAmount,takeFee);	
        }
	...
	function setMinimumBuyBack(uint256 buyBackMinimumNew) external onlyOwner() {
        buyBackMinimum  = buyBackMinimumNew;
    }
