...
contract PresaleBEP20 is Ownable {
    ...
    address payable public presale = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    uint256 public tokenPrice = 10 ether;   // 10 BNB = 1 token
    ...
    constructor(
        ...
        presaleStartTimestamp = now.add(5 days + 1 hours + 30 minutes);
    ...
    function deposit() public payable {
        ...
        uint256 tokenAmount = msg.value.div(1e18).mul(tokenDecimal).div(tokenPrice);
        // token.mint(msg.sender, tokenAmount);
        totalDepositedEthBalance = totalDepositedEthBalance.add(msg.value);
        deposits[msg.sender] = deposits[msg.sender].add(tokenAmount);
        emit Deposited(msg.sender, msg.value);
    }
    function claim() public {
        require(isClaimable, "Not claimable now");
        require(deposits[msg.sender] != 0, "Deposited 0");
        require(deposits[msg.sender] <= IERC20(token).balanceOf(address(this)), "Lack of token on presale contract");
        IERC20(token).transfer(msg.sender, deposits[msg.sender]);
