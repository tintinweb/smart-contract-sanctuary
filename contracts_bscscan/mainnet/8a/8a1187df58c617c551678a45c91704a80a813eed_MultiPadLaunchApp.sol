...
contract MultiPadLaunchApp is IBEP20 {
    ...
    */
     ...
     */
     function whitelistUserAdress(address[]  _userAddresses) external onlyOwner returns(bool){
     ...
     */
     ...
     function _trasferInLockingState(
        ...
        _thisSaleContribution[_userAddress][saleId] = _thisSaleContribution[_userAddress][saleId].add(msg.value);
    ...
    function claimPurchasedTokens(uint256 _lockNumber) external validateClaim(msg.sender,_lockNumber) returns (bool){
        uint256 calculation = (_finalSoldAmount[msg.sender].mul(20)).div(100);
    ...
    modifier validateClaim(address _userAddress, uint256 _lockNumber)
    {
        ...
        require(_lockNumber == 1);
        require(block.timestamp > saleEndTime + 150 && reEntrance[_userAddress][_lockNumber] != true, "Insufficient Unlocked Tokens");
        _;
    }
    ...
    function getUserTokensByAdd(address _userAddress) external view returns(uint256 _div1){
        _div1 =  (_finalSoldAmount[_userAddress].mul(20)).div(100);
        return(_div1);
    }
    ...
    function getClaimDates() view external returns(uint256){
        return(saleEndTime.add(150));
    }
    
    /*
     * @dev Get claimed tokens by user address
     */
    function getClaimedTokensHistory(address _userAddress) view external returns(uint256){
        return(_claimedByUser[_userAddress][1]);
    }
