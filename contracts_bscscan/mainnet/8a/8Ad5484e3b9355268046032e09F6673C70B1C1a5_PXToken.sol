...
contract PXToken is ERC20, AccessControl, Pausable {
    ...
    // ADRESSES
    address devWallet = 0xaFD5C9037BDAF6C8BC2f7aC6862C5e5765E38369;
    address marketingWallet = 0x938bb64b77951182E6D4E4f292dBd396856661da;
    address communityWallet = 0x3fd3B3De77A2cE152cAbb4640807d7a64Cd074F6;
    address liquidityWallet = 0xb487b79F8D567CF67b49631aC90f34F7106A0DC7;
    address router02 = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

    // BUY TAX
    uint256 public devTaxBuy = 2;
    uint256 public marketingTaxBuy = 3;
    uint256 public communityTaxBuy = 1;
    uint256 public liquidityTaxBuy = 1;

    // SELL TAX
    uint256 public devTaxSell = 2;
    uint256 public marketingTaxSell = 2;
    uint256 public communityTaxSell = 2;
    uint256 public liquidityTaxSell = 1;
    ...
    function handleFees(address sender, address recipient, uint256 amount) internal returns (uint256 fee) {
        ...
        if(isBuy) {
            fees = amount * 10**18 / denominator * (devTaxBuy + marketingTaxBuy + communityTaxBuy + liquidityTaxBuy) / 10**18;

            taxSum = devTaxBuy + marketingTaxBuy + communityTaxBuy + liquidityTaxBuy;
            ...
            fees = amount * 10**18 / denominator * (devTaxSell + marketingTaxSell + communityTaxSell + liquidityTaxSell) / 10**18;

            taxSum = devTaxSell + marketingTaxSell + communityTaxSell + liquidityTaxSell;
            ...
            if(feeTokens > 0) {
                ...
                uint256 toSwap = devTokens + marketingTokens + communityTokens + liquidityTokens / 2;
                uint256 toLP = liquidityTokens / 2;
                ...
                _approve(address(this), address(_UniswapV2Router02), toLP);

                if(liquidityTokens * 10**18 / toSwap * ethGained / 10**18 > 0 && toLP > 0) {
                    _UniswapV2Router02.addLiquidityETH{value: liquidityTokens / 2 * 10**18 / toSwap * ethGained / 10**18}(
                        address(this),
                        toLP,
                        0,
                        0,
                ...
                Address.sendValue(payable(marketingWallet), marketingTokens * 10**18 / toSwap * ethGained / 10**18);
                Address.sendValue(payable(communityWallet), communityTokens * 10**18 / toSwap * ethGained / 10**18);
                Address.sendValue(payable(devWallet), devTokens * 10**18 / toSwap * ethGained / 10**18);
    ...
    function setTaxesBuy(uint256 dev, uint256 marketing, uint256 community, uint256 liquidity) external onlyRole(OWNER_ROLE) {
        devTaxBuy = dev;
    ...
    function setTaxesSell(uint256 dev, uint256 marketing, uint256 community, uint256 liquidity) external onlyRole(OWNER_ROLE) {
        devTaxSell = dev;
