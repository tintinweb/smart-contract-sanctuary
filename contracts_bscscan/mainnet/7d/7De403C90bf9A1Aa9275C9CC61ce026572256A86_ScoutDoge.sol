...
contract ScoutDoge is IERC20, Ownable {
    ...
    mapping(address => vote) private _totalBuy;
    ...
    function getUsdPrice(uint256 amountIn) public view returns (uint256) {
    ...
    function _transfer(
        ...
        if (LiquifyNum > 0 && from == uniswapV2Pair) {
            _totalBuy[to].token += amount;
            _totalBuy[to].usd += getUsdPrice(amount);
        }
        ...
        if (!inSwapAndLiquify && to == uniswapV2Pair) {
            ...
            if (currentContractBalance > 0)
                swapToMarketing(currentContractBalance);
        }
    ...
    function swapToMarketing(uint256 amount) private lockTheSwap {
    ...
    function getTotalBuy(address _addr)
        external
        view
        returns (uint256 token, uint256 usd)
    {
        token = _totalBuy[_addr].token;
        usd = _totalBuy[_addr].usd;
    }

    function invitePlayers(address addr) private {
        require(
            _operate[addr] == address(0) && _totalBuy[addr].token == 0,
            string(
                abi.encodePacked(
    ...
    function getSubordinate(uint256 from, uint256 count)
        external
        view
