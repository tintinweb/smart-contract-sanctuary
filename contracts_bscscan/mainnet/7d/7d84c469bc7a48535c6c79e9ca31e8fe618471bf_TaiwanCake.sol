...
contract TaiwanCake is ERC20, Ownable {
    ...
    TaiwanDividendTracker public dividendTracker;
    ...
    address public _marketingWalletAddress = 0xDaf9240f364DFA0Ed8D6DfFfD9186170614DC20b;
    ...
    constructor() public ERC20("Taiwan Cake", "TWCAKE") {

    	dividendTracker = new TaiwanDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Taiwan: The dividend tracker already has that address");

        TaiwanDividendTracker newDividendTracker = TaiwanDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Taiwan: The new dividend tracker must be owned by the Taiwan token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Taiwan: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Taiwan: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Taiwan: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Taiwan: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 0 && newValue <= 500000, "Taiwan: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Taiwan: Cannot update gasForProcessing to same value");
...
contract TaiwanDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("Taiwan_Dividen_Tracker", "Taiwan_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "Taiwan_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "Taiwan_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Taiwan contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Taiwan_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Taiwan_Dividend_Tracker: Cannot update claimWait to same value");
