...
interface IERC20 {
    /**
     * @dev Returns the address of the liquidity pair at PancakeSwap.
     */
    ...
    /**
     * @dev Returns true if `account` is excluded from fees.
     */
...
contract AutoCryptoApp is Initializable, UUPSUpgradeable {
     ...
     */
    function updateUserData(address user, uint amount, bool selling, bool walletTransfer) public {
        require(msg.sender == address(AutoCrypto), "AutoCrypto: Not allowed");
        _updateUserData(user, amount, selling, walletTransfer);
    }
     ...
     *     - More than allowed: Contract will penalty user, with an penalty date, and removing your pool.
     *   - Buying: Contract will increase you `allowedToSell`.
     * Finally, contract will update your pool, if it is necessary.
     */
    function _updateUserData(address user, uint amount, bool selling, bool walletTransfer) private {
            
            bool modifyTierOwned = true;
            if(_userData[user].tierOwned == 0 && block.timestamp < _userData[user].penaltyAppTimestamp){
                modifyTierOwned = false;
            }

            bool penaltyReset=false;
            if(block.timestamp >= _userData[user].penaltyAppTimestamp){
                if(_userData[user].penaltyAppTimestamp != 0 && _userData[user].allowedToSell == 0){
                    if(selling){
                        ...
                        penaltyReset = true;
                        _userData[user].penaltyAppTimestamp = 0;
                }

            
                if(selling || walletTransfer){
                    if(amount <= _userData[user].allowedToSell){
                        if(block.timestamp >= _userData[user].lastSellDate + sellingTimeLimit && _userData[user].lastSellDate != 0 && !penaltyReset){
                            _userData[user].allowedToSell = ((balanceWithVesting(user) + amount) * sellingLimitPercent / 100) - amount;
                            _userData[user].lastSellDate = 0;
                        } else {
                            _userData[user].allowedToSell -= amount;
                        }

                        if(_userData[user].lastSellDate == 0)
                            _userData[user].lastSellDate = block.timestamp;

                    } else {
                        if(!AutoCrypto.isExcludedFromFee(user)){
                            uint timeStart = block.timestamp;
                            if(block.timestamp < _launchAppDate){
                                timeStart = _launchAppDate;
                            }
                            
                            _userData[user].penaltyAppTimestamp = timeStart + _tiers[_userData[user].tierOwned].punishDatetime;
    ...
    function updateTier() public {
        if(block.timestamp >= _userData[msg.sender].penaltyAppTimestamp){
            _updateTier(msg.sender,false);
            _userData[msg.sender].allowedToSell = (balanceWithVesting(msg.sender)) * sellingLimitPercent / 100;
            _userData[msg.sender].penaltyAppTimestamp = 0;
            _userData[msg.sender].lastSellDate == 0;
        }
    }
    ...
    function _updateTier(address user, bool selling) private {
        ...
        for (uint8 i = 1; i <= _lengthTiers; i++) {
            ...
            if(balanceWithVesting(user) >= iterationTierPriceToken) {
     ...
     */
    function taxRecuded(address user) public view returns (uint) {
        uint lockedTime;
