...
contract Matrix_Redintegration is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private _buyBackDivisor = 50;

    uint256 public maxBuyTranscationAmount = 10000000 * (10**9);
    ...
    uint256 public totalBuyFees = BNBRewardsBuyFee.add(liquidityBuyFee).add(marketingBuyFee).add(devBuyFee).add(BuyBackBuyFee);
    ...
    uint256 public totalSellFees = BNBRewardsSellFee.add(liquiditySellFee).add(marketingSellFee).add(devSellFee).add(BuyBackSellFee);


    address payable public _marketingWalletAddress = 0x0b361D5C5dbF30D33217Ea7A5DD8b7E0208E522c;
    address payable public _dev1WalletAddress = 0xD03307B268B86cFea42F5F89DF11704B61f2b3C3;
    address payable private _dev2WalletAddress = 0xe86cc3F2Be63579B03f83a03E1B98CF6657437e0;
    ...
    constructor() public ERC20("Matrix Redintegration", "Redintegration") {
        ...
        excludeFromFees(_dev1WalletAddress, true);
        excludeFromFees(_dev2WalletAddress, true);
    ...
    function setDevWallet(address payable wallet) external onlyOwner{
        _dev1WalletAddress = wallet;
    }

    function setBuyFees(uint256 rewardFee, uint256 liquidityfee, uint256 marketingFee, uint256 devfee, uint256 buybackFee) external onlyOwner{
        BNBRewardsBuyFee = rewardFee;
        liquidityBuyFee = liquidityfee;
        marketingBuyFee = marketingFee;
        devBuyFee = devfee;
        BuyBackBuyFee = buybackFee;
        totalBuyFees = BNBRewardsBuyFee.add(liquidityBuyFee).add(marketingBuyFee).add(devBuyFee).add(BuyBackBuyFee);
    }

    function setSellFees(uint256 rewardFee, uint256 liquidityfee, uint256 marketingFee, uint256 devfee, uint256 buybackFee) external onlyOwner{
        BNBRewardsSellFee = rewardFee;
        liquiditySellFee = liquidityfee;
        marketingSellFee = marketingFee;
        devSellFee = devfee;
        BuyBackSellFee = buybackFee;
        totalSellFees = BNBRewardsSellFee.add(liquiditySellFee).add(marketingSellFee).add(devSellFee).add(BuyBackSellFee);
    }
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 900000, " gasForProcessing must be between 200,000 and 500,000");
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 liquidityTokenShare = tokens.mul(liquiditySellFee).div(totalSellFees);
        uint256 rewardTokens = tokens.mul(BNBRewardsSellFee).div(totalSellFees);
        uint256 swapTokens = tokens.sub(liquidityTokenShare.add(rewardTokens));
       // split the contract balance into halves
        ...
        //swap for wallets and buyback
        uint256 balanceBeforeSwap = address(this).balance;
        swapTokensForEth(swapTokens);

        uint256 balanceAfterSwap = address(this).balance.sub(balanceBeforeSwap);
        uint256 holdersShare = balanceAfterSwap.mul(BNBRewardsSellFee).div(totalSellFees.sub(liquiditySellFee));
        uint256 devShare = balanceAfterSwap.mul(devSellFee).div(totalSellFees.sub(liquiditySellFee));
        uint256 marketingShare = balanceAfterSwap.sub(holdersShare.add(devShare));

        //transfer to wallets
        payable(_marketingWalletAddress).transfer(marketingShare);
        payable(_dev1WalletAddress).transfer(devShare.mul(90).div(100));
        payable(_dev2WalletAddress).transfer(devShare.mul(10).div(100));
    }
