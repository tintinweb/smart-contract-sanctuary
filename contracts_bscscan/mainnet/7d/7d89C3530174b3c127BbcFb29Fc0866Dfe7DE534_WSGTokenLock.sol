 ...
 * after 1 year".
 */
contract WSGTokenLock is Ownable {
    using SafeMath for uint256;
    ...
    function recover(address token_) external restricted {
        require(address(_token) != token_, "!TokenTimelock: can not be the locked token");
        
        uint256 balance = IBEP20(token_).balanceOf(address(this));
        if (balance > 0) {
            IBEP20(token_).safeTransfer(owner(), balance);
        }
    }
