...
contract SBVJetV2 is IStakingRewards, Ownable , ReentrancyGuard {
    using SafeMath for uint256;
    ...
    IMasterChef public constant MASTER_CHEF = IMasterChef(0x63d6EC1cDef04464287e2af710FFef9780B6f9F5);
    address private constant WINGS = 0x0487b824c8261462F88940f97053E65bDb498446;
    ...
    address[] public wingsToBusdPath = [WINGS,WBNB,BUSD];
    address[] public busdToXBlzdPath = [BUSD,xBLZD];

    IPancakeRouter02 public constant ROUTER = IPancakeRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    IPancakeRouter02 public constant ROUTER_OTHER = IPancakeRouter02(0xBe65b8f75B9F20f4C522e0067a3887FADa714800);
    ...
    constructor(
        ...
        (address _stakingToken,,,) = MASTER_CHEF.poolInfo(_pid);
        ...
        IERC20(_stakingToken).safeApprove(address(MASTER_CHEF), uint256(-1));
        stakingToken = IERC20(_stakingToken);

        IERC20(WINGS).safeApprove(address(ROUTER_OTHER), uint256(-1));
    ...
    function withdraw(uint256 amount) public override nonReentrant updateReward(msg.sender,0) updateReward(msg.sender,1) {
        ...
        if(emergencyStop != true){
            MASTER_CHEF.withdraw(pid, amount);
        }
    ...
    function _harvest() private {
        ...
        uint256 cakeAmount = IERC20(WINGS).balanceOf(address(this));

        // 47% for TEMPEST
        uint256 cakeAmountForTEMPEST = cakeAmount.mul(470).div(1000);
        if(cakeAmountForTEMPEST > 0){
          // 100% for BUSD
          uint256 beforeBNNswap = IERC20(BUSD).balanceOf(address(this));
          _safeSwap(
              address(ROUTER_OTHER),
              cakeAmountForTEMPEST,
              slippageFactor,
              wingsToBusdPath,
              address(this),
              deadline
          );
          uint256 afterBNNswap = (IERC20(BUSD).balanceOf(address(this))).sub(beforeBNNswap);
          // 50% for XBLZD
          _safeSwap(
              address(ROUTER),
              afterBNNswap.div(2),
