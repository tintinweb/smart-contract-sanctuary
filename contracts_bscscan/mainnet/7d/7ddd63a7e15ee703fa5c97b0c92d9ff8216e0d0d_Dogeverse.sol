...
contract Dogeverse is ERC20, Authorizable {
    ...
    uint256 public activatedBlockTimestamp;
    ...
    function activate() public onlyAuthorized {
        ...
        activatedBlockTimestamp = block.timestamp;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        // Only whitelisted addresses can send or receive before go-live.
        if (!tradingEnabled) {
            require(whitelisted(from) || whitelisted(to), "Dogeverse: Cannot transfer tokens until trading is enabled");
        }
        ...
        // Enforce max wallet size during the first 24-hours.
        if (activatedBlockTimestamp != 0 && block.timestamp <= activatedBlockTimestamp + 24 * 60 * 60) {
            if (!liquidating && to != dexPair && from != liquidityWallet && to != liquidityWallet) {
                require(balanceOf(to) <= maxWalletSize(), "Dogeverse: Max wallet size is enforced during the first 24 hours");
            }
        }
    }
