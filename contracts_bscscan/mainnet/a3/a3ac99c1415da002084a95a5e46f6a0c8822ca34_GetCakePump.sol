...
TG = https://t.me/GetCakePump            //         Stealth launch under 15 min   
...
contract GetCakePump is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   GetCakePumpDividendTracker public dividendTracker;
    ...
    uint256 public maxWalletToken = 350000000 * (10**18);
    ...
    constructor() ERC20("GetCakePump", "GCP") {
    	...
    	dividendTracker = new GetCakePumpDividendTracker();
		marketingWallet = payable(0x3E93358CC576aC567Cfc45A90AdCa2894eF077e7);

    	buyBackWallet = 0xca9fc13D8ca6A8e2cCa5C77d9ED971a2F9B7D8DC;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "GetCakePump: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "GetCakePump: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "GetCakePump: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "GetCakePump: The dividend tracker already has that address");

       GetCakePumpDividendTracker newDividendTracker = GetCakePumpDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "GetCakePump: The new dividend tracker must be owned by the GetCakePump token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "GetCakePump: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GetCakePump: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GetCakePump: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GetCakePump: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GetCakePump: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "GetCakePump: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GetCakePump: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GetCakePump: Cannot update gasForProcessing to same value");
...
contract GetCakePumpDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("GetCakePump_Dividend_Tracker", "GetCakePump_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 100000000 * (10**18); //must hold 100000000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "GetCakePump_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "GetCakePump_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GetCakePump contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 3600, "GetCakePump_Dividend_Tracker: claimWait must be updated to between 30 Minutes and 1 Hour");
        require(newClaimWait != claimWait, "GetCakePump_Dividend_Tracker: Cannot update claimWait to same value");
