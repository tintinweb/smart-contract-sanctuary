...
contract NFCSpa is Ownable {
    ...
    constructor(
        ...
        // VIP nft
        nft = address(0x69799454f8c742690E20a34B953521fdA013159E);
    }
    ...
    function avlNft () internal view returns (uint256) {
        return IERC721(nft).balanceOf(msg.sender);
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        require(avlNft() > 0, "deposit: you need NFCs to stake in this pool");
        ...
        if (_amount > 0) {
            if (avlNft() * 500 * 10 ** 18 < user.amount.add(_amount)){
                // set new _amount to not exceed the limit (does not consider reflect fees)
                _amount = (avlNft() * 500 * 10 ** 18).sub(user.amount);
            }
    ...
    function withdraw(uint256 _amount) public {
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[msg.sender];
        require(avlNft() > 0, "withdraw: you need NFC VIP status (at least 1 NFL) to withdraw");
