...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accMomoPerShare;   // Accumulated BALTOs per share, times 1e12. See below.
    ...
    function pendingMomo(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accMomoPerShare = accMomoPerShare.add(baltoReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accMomoPerShare = pool.accMomoPerShare.add(baltoReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
