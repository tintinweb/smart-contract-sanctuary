...
contract SonOfWitcher is Context, IERC20, Ownable {
    ...
    string private _name = "SonOfWitcher";
    string private _symbol = "SOW";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000000 * 10**9 * 10**_decimals;
    
    address payable public marketingAddress = payable(0xAE78F46EBA23a14615aFe305faf8a19e45d40E64);
    address public marketingWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFeesSow;
    mapping (address => bool) private _isExcludedFromMaxBalanceSow;

    uint256 private constant _maxFeesSow = 13;
    uint256 private _totalFeesSow;
    uint256 private _totalFeesSowContract;
    uint256 private _burnFeesSow;
    uint256 private _liquidityFeesSow;
    uint256 private _marketingFeeSow;
    
    uint256 private _maxBalanceWalletSow;
    ...
    constructor () {
        ...
        _isExcludedFromFeesSow[owner()] = true;
        _isExcludedFromFeesSow[address(this)] = true;
        
        _isExcludedFromMaxBalanceSow[owner()] = true;
        _isExcludedFromMaxBalanceSow[address(this)] = true;
        _isExcludedFromMaxBalanceSow[uniswapV2Pair] = true;

        _liquidityFeesSow = 2;
        _marketingFeeSow = 11;
        _burnFeesSow = 0;
        _totalFeesSow = _liquidityFeesSow.add(_marketingFeeSow).add(_burnFeesSow);
        _totalFeesSowContract = _liquidityFeesSow.add(_marketingFeeSow);

        _liquidityThreshhold = 3000 * 10**9 * 10**_decimals;
        _maxBalanceWalletSow = 1000000 * 10**9 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFeesSow).add(_marketingFeeSow) <= _maxFeesSow, "Fees are too high.");
        _liquidityFeesSow = newLiquidityFee;
        _totalFeesSow = _liquidityFeesSow.add(_marketingFeeSow).add(_burnFeesSow);
        _totalFeesSowContract = _liquidityFeesSow.add(_marketingFeeSow);
        emit SetLiquidityFee(_liquidityFeesSow);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFeesSow.add(_burnFeesSow).add(newMarketingFee) <= _maxFeesSow, "Fees are too high.");
        _marketingFeeSow = newMarketingFee;
        _totalFeesSow = _liquidityFeesSow.add(_marketingFeeSow).add(_burnFeesSow);
        _totalFeesSowContract = _liquidityFeesSow.add(_marketingFeeSow);
        emit SetMarketingFee(_marketingFeeSow);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFeesSow.add(newBurnFee).add(_marketingFeeSow) <= _maxFeesSow, "Fees are too high.");
        _burnFeesSow = newBurnFee;
        _totalFeesSow = _liquidityFeesSow.add(_marketingFeeSow).add(_burnFeesSow);
        emit SetBurnFee(_burnFeesSow);
    }
    ...
    function _transfer(
        ...
        // Make sure that: Balance + Buy Amount <= _maxBalanceWalletSow
        if(
            from != owner() &&              // Not from Owner
            to != owner() &&                // Not to Owner
            !_isExcludedFromMaxBalanceSow[to]  // is excludedFromMaxBalance
        ){
            require(
                balanceOf(to).add(amount) <= _maxBalanceWalletSow,
        ...
        if(
            ...
            _totalFeesSowContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromFeesSow[from] || _isExcludedFromFeesSow[to])
            && _totalFeesSow > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_totalFeesSowContract).div(100);
            uint256 toBurnAmount = amount.mul(_burnFeesSow).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFeesSow).div(_totalFeesSowContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeeSow).div(_totalFeesSowContract);
