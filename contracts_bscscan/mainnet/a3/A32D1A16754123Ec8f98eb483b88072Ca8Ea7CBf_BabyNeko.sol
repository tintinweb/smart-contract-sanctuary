...
contract BabyNeko  is Context, IERC20, Ownable {
    ...
    string private _name = "Baby Neko";
    string private _symbol = "Baby Neko";
    ...
    uint256 public _maxTxAmountBuy = 30000000* 10**9;
    uint256 public _maxTxAmountSell = 10000000* 10**9;
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
            ...
            require(amount <= _maxTxAmountBuy || _isTxLimitExempt[from], "TX Limit Exceeded");
        ...
        if (!inSwapAndLiquify && tradingOpen && to == uniswapV2Pair) {
            require(amount <= _maxTxAmountSell || _isTxLimitExempt[from], "TX Limit Exceeded");
    ...
    function setIsTxLimitExempt(address account) public onlyOwner {
        _isTxLimitExempt[account] = true;
    }

    function setTxLimitBuy(uint256 amount) external authorized {
        require(amount >= _tTotal / 10000000000000000);
