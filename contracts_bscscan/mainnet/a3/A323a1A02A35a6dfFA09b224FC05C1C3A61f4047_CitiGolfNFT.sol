...
contract CitiGolfNFT is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint public constant max_mintable = 5000;
    uint256 public LATEST_CLAIMED_ID;
    uint256 public NEXT_STOP_ID = 0;
    uint public chestLoot = 1500000000000000000000 wei; //1500 LPDi
    uint public claimPrice = 300000000000000000000 wei; //300 LPDi

    address public LDRToken = address(0);
    address public token = 0x20b1290DEdb94328705b21986485A447C63B8506;
    ...
    function startClaim(uint256 _nextStop) external onlyOwner {
    ...
    function claim(uint n) public nonReentrant{
        ...
        uint excess = 0;
        IERC20(token).transferFrom(_msgSender(), address(this), total_cost);
        ...
        IERC20(token).transfer(_msgSender(), excess);
    ...
    function burn(uint256 tokenId) public virtual {
        ...
        _burn(tokenId);
        if (tokenId % 27==0){
            IERC20(token).transfer(_msgSender(), chestLoot);
        } else{ 
            MyToken(LDRToken).mint(_msgSender(), 1500000000000000000000); // 1500
        }
    }
   ...
   function getBalance() external view returns(uint){
    ...
    constructor() ERC721("CitiGolf NFT", "CGN") Ownable() {}
}
