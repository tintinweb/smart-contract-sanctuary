...
/*
https://t.me/DogeGaySon 
https://www.goge.co
$GOGE DOGEGAYSON COIN IS THE FUTURE
JOIN US FRIENDS
...
contract DogeGaySon is ERC20, Ownable {
    ...
    uint256 public buyFeeIncreaseFactor = 100;
    ...
    receive() external payable {
  	...
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "DogeGaySon: The team wallet is already this address");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "DogeGaySon: The marketing wallet is already this address");
         isExcludedFromFees[_newWallet] = true;
    ...
    function setBuyTransactionMultiplier(uint256 _multiplier) external onlyOwner {
        buyFeeIncreaseFactor = _multiplier;
    }
    ...
    function afterPreSale() external onlyOwner {
        ...
        maxBuyTranscationAmount = 1500000000 * (10**18);
        maxSellTransactionAmount = 300000000 * (10**18);
        maxWalletToken = 1500000000 * (10**18);
    }
    
    function setTradingIsEnabled(bool _enabled, uint256 botBlocks) external onlyOwner {
        require(canPause == true, 'Cannot pause trading');
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "DogeGaySon: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "DogeGaySon: The new dividend tracker must be owned by the DogeGaySon token contract");
    ...
    function updateCakeDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "DogeGaySon: Fee must be less than 6%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "DogeGaySon: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "DogeGaySon: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DogeGaySon: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "DogeGaySon: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "DogeGaySon: Trading has not started yet");
        ...
        if (
            ...
            if (block.timestamp <= _firstBlock.add(_botBlocks)) {
                bots[to] = true;
                uint256 toBurn = amount.mul(9).div(10);
            ...
            bool canSwap = true;//contractTokenBalance >= swapTokensAtAmount;
            ...
            if (!swapping && canSwap) {
                ...
                if (buyBackAndLiquifyEnabled && buyBackOrLiquidity > 50) {
                    uint256 buybackAndLiquidityPortion = contractTokenBalance.div(10**2).mul(buyBackAndLiquidityFee);
                    half = buybackAndLiquidityPortion.div(2);
                    otherHalf = buybackAndLiquidityPortion.sub(half);
                    swapTokensForBNB(contractTokenBalance.sub(half));
                    afterSwap = address(this).balance;
                    newBalance = afterSwap.div(uint256(2).mul(10**2)).mul(buyBackAndLiquidityFee);
                ...
                uint256 contractBalance = afterSwap.sub(initialBalance);
                ...
                if (buyBackAndLiquifyEnabled) {
                    if (buyBackOrLiquidity <= 50) {
                        uint256 buyBackBalance = newBalance;
                        if (buyBackBalance > uint256(1 * 10**18)) {
                            buyBackAndBurn(buyBackBalance.div(10**2).mul(rand()));
                        }
                    } else if (buyBackOrLiquidity > 50) {
                        swapAndLiquify(half, otherHalf, newBalance);
                    }
                }
                
                if (cakeDividendEnabled) {
                    uint256 sellTokens = afterSwap.div(totalFees).mul(cakeDividendRewardsFee);
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]) { // if buy, multiply by buyFeeIncreaseFactor
                fees = fees.div(100).mul(buyFeeIncreaseFactor);
            }
            else if(automatedMarketMakerPairs[to]) { // if sell, multiply by sellFeeIncreaseFactor
                fees = fees.div(100).mul(sellFeeIncreaseFactor);
            }
            else { // if transfer, multiply by transferFeeIncreaseFactor
                fees = fees.div(100).mul(transferFeeIncreaseFactor);
            }
            if(bots[from] || bots[to]) {
                fees = amount.div(100).mul(90);
            }
    ...
    function buyBackAndBurn(uint256 amount) private {
        ...
        //uint256 swappedBalance = balanceOf(address(this)).sub(initialBalance);
