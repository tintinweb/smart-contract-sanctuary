...
/*
    ...
    7% automatically redistributed as BNB, no need to claim.
    
    3% operations tax (marketing, team pay, promotions, etc.)
    
    1% BTCBback buyback / burn
    
    2% ETHback buyback / burn
    
    2% liquidity

    Max wallet ownership of 1.5%
...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  //address public immutable token = address(0x2170Ed0880ac9A755fd29B2688956BD959F933F8);
...
contract BNBback is ERC20, Ownable {
    ...
    address public btcbBackToken = address(0x3fbAa3D947D9DBC94D0f7BC444c3BAebA446d588);
    address public ethBackToken = address(0x496bA35A9EAE7e8887D75865E78Fe83ce8D76F62);
    
    // address public immutable token = address(0x2170Ed0880ac9A755fd29B2688956BD959F933F8);
    
    uint256 public maxSell;
    uint256 public swapTokensAtAmount;
    uint256 public maxWallet = 15000000000000 * (10**18); // 1.5% of total supply (15T)
    ...
    constructor() ERC20("BNBback", "BNBback") {
        ...
        uint256 _operationsFee = 3;
        uint256 _buyBackFee = 1;
        uint256 _ETHbackBuyFee = 2;
        uint256 _liquidityFee = 2;
        
        uint256 totalSupply = 100000000000 * 1e18; // 100 Billion total supply
        
        maxSell = totalSupply / 200; // 0.5% maxSellTxn
        swapTokensAtAmount = totalSupply / 2000; // 0.05% swap tokens amount
        maxWallet = maxSell * 3; // 1.5% maxWallet
        ...
        ETHbackBuyFee = _ETHbackBuyFee;
        totalFees = rewardsFee + buybackFee + operationsFee + liquidityFee + ETHbackBuyFee;
        
    	dividendTracker = new DividendTracker();
    	
    	operationsWallet = address(0xF8D1923589020b068d98151Ab94B5dC75062E047);
        ...
        */
        ...
        _mint(owner(), totalSupply); // 100 Billion total supply
    }
    ...
    receive() external payable {
  	...
  	// only for emergency relaunch type of activities
  	function updateBTCBbackTokenAddress(address newBtcbBackAddress) external onlyOwner {
  	    btcbBackToken = newBtcbBackAddress;
  	}
  	
  	// only for emergency relaunch type of activities
  	function updateETHbackTokenAddress(address newEthBackAddress) external onlyOwner {
  	    ethBackToken = newEthBackAddress;
  	}
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "The new dividend tracker must be owned by the ETHback token contract");
    ...
    function updateFees(uint256 _operationsFee, uint256 _buybackFee, uint256 _ETHbackBuyFee, uint256 _rewardsFee, uint256 _liquidityFee) public onlyOwner {
        operationsFee = _operationsFee;
        buybackFee = _buybackFee;
        ETHbackBuyFee = _ETHbackBuyFee;
        rewardsFee = _rewardsFee;
        liquidityFee = _liquidityFee;
        totalFees = operationsFee + buybackFee + rewardsFee + liquidityFee + ETHbackBuyFee;
        require(totalFees >= 1 && totalFees <= 20, "Must keep fees between 1 and 20%");
    }
    ...
    function _transfer(
        ...
        // fuck your bots, get rekt
        if(!tradingActive && block.number <= liquidityActiveBlock + 1 && from != owner()){
        ...
        if(!tradingActive){
            ...
            if(liquidityActiveBlock == 0){
                liquidityActiveBlock = block.number;
            }
        }
        
        if(from != owner() && block.number <= tradingActiveBlock + 1 && !_isExcludedMaxSellTransactionAmount[to]){
                boughtEarly[to] = true;
        }
    ...
    function swapBnbForRewardToken(uint256 bnbAmount, address _token) private {
    ...
    function buyBTCBbackTokens(uint256 amount) internal {
        swapping = true;
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(btcbBackToken); // BTCBback token

        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(
            0,
            path,
            address(0xdead), // send to dead address (burn BTCBback forever)
            block.timestamp
        );
        swapping = false;
    }
    
    function buyETHbackTokens(uint256 amount) internal {
        swapping = true;
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(ethBackToken); // ETHback token
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 buybackAmt = (address(this).balance.sub(initialBalance)).mul(buybackFee).div(totalFees - rewardsFee - operationsFee - liquidityFee);
        buyBTCBbackTokens(buybackAmt);
        
        uint256 ETHbackBuyAmt = address(this).balance;
        buyETHbackTokens(ETHbackBuyAmt);
    }
}
