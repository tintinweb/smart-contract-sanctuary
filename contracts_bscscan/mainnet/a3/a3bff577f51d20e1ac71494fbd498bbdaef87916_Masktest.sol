...
contract Masktest is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 45600000000 *  10**9;
    ...
    string private _name = "masktest";
    string private _symbol = "maskt";
    ...
    uint256 public _maxTxAmount = 456000000 * 10**9;
    ...
    function _getdevelopmentWallet() public view virtual returns (address) {
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        ...
        function setWallets(address payable development, address payable marketing) public onlyOwner() {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tMarketing, data.tDevelopment);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        return (tTransferAmount, TData(0, tFee, tLiquidity, tMarketing, tDevelopment, 0));
    }
    ...
    function _getRValues( TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rDevelopment = _data.tDevelopment.mul(_data.currentRate);
    ...
    function _takeDevelopment(uint256 tDevelopment) private {
        ...
        if (_shouldSwapToBNB) {
            swapTokensForEth(rDevelopment);
            _developmentWallet.transfer(address(this).balance);
        } else {
            _rOwned[_developmentWallet] = _rOwned[_developmentWallet].add(rDevelopment);
            if(_isExcluded[_developmentWallet])
                _tOwned[_developmentWallet] = _tOwned[_developmentWallet].add(tDevelopment);
        }
    }
