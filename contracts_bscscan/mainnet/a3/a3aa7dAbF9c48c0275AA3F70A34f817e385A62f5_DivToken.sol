    ...
    AccessControl 
{
    ...
    constructor(
        ...
        uint256 _cap,
        address _admin
    ) public ERC20(name, symbol) ERC20Capped(_cap) {
        _setupRole(DEFAULT_ADMIN_ROLE, _admin);
        _setupRole(MINTER_ROLE, _admin);
        _setupRole(BURNER_ROLE, _admin);
        dividend = IERC20(_dividend);
    }
    ...
    function claimDividend(uint256 _snapshotId) external {
        ...
        uint256 referralFee = transferReferralFee(dividend, dividendAmount);
        claimStatus[_snapshotId][msg.sender] = true;
        dividend.safeTransfer(msg.sender, dividendAmount.sub(referralFee));
        emit ClaimDividend(msg.sender, _snapshotId, dividendAmount.sub(referralFee));
    }
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 amount
    ) internal override(ERC20, ERC20Snapshot, ERC20Capped) {
        super._beforeTokenTransfer(from, to, amount);
    }
}
...
contract ReferralManager is Ownable {
    ...
    constructor() public {        
        referralPercent = 10e18; // 10%
    }

    function setReferralPercent(uint256 _referralPercent) external onlyOwner {
        require(_referralPercent <= 100e18, "> 100%");
        require(_referralPercent != referralPercent, "Same value");
        referralPercent = _referralPercent;
    }

    function setReferral(address _user, address _referral) external onlyOwner {
        require(referrals[_user] == address(0), "Referral exists");
        referrals[_user] = _referral;
    }

    function myReferral(address _referral) external  {
        require(referrals[msg.sender] == address(0), "Referral exists");
        require(referrals[_referral] != address(0), "Referral not exists");
        referrals[msg.sender] = _referral;
    }

    function transferReferralFee(IERC20 _token, uint256 _amount)
        internal
        returns (uint256 totalFeeAmount)
    {
        uint256 _feeAmount = _amount.mul(referralPercent).div(
        ...
        for (uint256 i = 0; i < LEVEL; i++) {
            ...
            totalFeeAmount = totalFeeAmount.add(_feeAmount);
            _token.safeTransfer(_referral, _feeAmount);
...
// Example class - a mock class using delivering from TokenineERC20Dividend
// Only use for testing
contract DivToken is ERC20Dividend {
    constructor(address _dividend, address _admin)
        public
        ERC20Dividend("Divine Token", "DIVI", _dividend, 1e25, _admin)
    {}
}
