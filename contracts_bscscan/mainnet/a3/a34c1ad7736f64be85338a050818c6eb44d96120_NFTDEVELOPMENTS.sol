...
contract NFTDEVELOPMENTS is IERC20Upgradeable, OwnableUpgradeable {
    ...
    function initialize (
        ...
        require(MarketingFeeBps_ >= 0 && MarketingFeeBps_ <= 10**4, "Invalid charity fee");
        require(maxTxBps_ > 0 && maxTxBps_ <= 10**4, "Invalid max tx amount");
        if (MarketingAddress_ == address(0)) {
            require(MarketingFeeBps_ == 0, "Cant set both charity address to address 0 and charity percent more than 0");
        }
        require(taxFeeBps_ + liquidityFeeBps_ + MarketingFeeBps_ <= 10**4, "Total fee is over 100% of transfer amount");
    ...
    function _reflectFee(uint256 rFee, uint256 tFee) public onlyOwner {
    ...
    function _takeMarketingFee(uint256 tMarketing) public onlyOwner {
        if (tMarketing > 0 ) {
            uint256 currentRate =  _getRate();
            uint256 rMarketing = tMarketing.mul(currentRate);
