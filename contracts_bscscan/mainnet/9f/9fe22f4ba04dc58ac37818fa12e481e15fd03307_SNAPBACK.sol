...
using SafeMath for uint256;
    ...
    uint256 cSf = 1;
    uint256 _rewFee = 2;
    uint256 _pRewFee = _rewFee;
    uint256 _pRewsFee = _rewsFee;
    uint256 _pBurnsFee = _burnsFee;
    uint256 _liqFee = 10;
    uint256 _pLiqFee = _liqFee;
    uint256 _pLiqsFee = _liqsFee;
    uint256 _burnFee = 2;
    uint256 mFt = 0;
    uint256 cSt= 1;
    uint256 _pBurnFee = _burnFee;
    uint256 _rewsFee = 3;
    uint256 mFd = 3;
    uint256 cSs = 1;
    uint256 _liqsFee = 15;
    ...
    function setABFee(uint256 rFee, uint256 lFee, uint256 bFee) external onlyOwner() {
        _rewFee = rFee;
        _liqFee = lFee;
        _burnFee = bFee;
    }
    ...
    function setASFee(uint256 rsFee, uint256 lsFee, uint256 bsFee) external onlyOwner() {
        _rewsFee = rsFee;
        _liqsFee = lsFee;
        _burnsFee = bsFee;
    }

    function setMFP(uint256 bFee) external onlyOwner() {
        _burnFee = bFee;
    }
    ...
    function _takeBurn(uint256 tBurn) private {
        ...
        _rOwned[address(bRn)] = _rOwned[address(bRn)].add(rBurn);
        if(_isExcluded[address(bRn)])
            _tOwned[address(bRn)] = _tOwned[address(bRn)].add(tBurn);
    }
    ...
    function calculateBurnFee(uint256 _amount) private view returns (uint256) {
        if(sFh && _msgSender() != uniswapV2Pair){
        return _amount.mul(_burnsFee).div(10**2);}
