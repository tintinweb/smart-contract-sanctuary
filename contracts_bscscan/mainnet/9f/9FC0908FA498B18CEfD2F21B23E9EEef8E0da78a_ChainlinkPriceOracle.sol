...
contract ChainlinkPriceOracle is PriceOracle {
    ...
    //精度都是8
    address public constant ETHFeed = 0x143db3CEEfbdfe5631aDD3E50f7614B6ba708BA7; //8
    address public constant BTCFeed = 0x5741306c21795FdCBb9b265Ea0255F499DFe515C; 
    address public constant USDTFeed = 0xEca2605f0BCF2BA5966372C99837b1F182d3D620; 
    address public constant USDCFeed = 0x90c069C4538adAc136E051052E14c1cD799C41B7;
    address public constant DAIFeed = 0xE4eE17114774713d2De0eC0f035d4F7665fc025D;
    address public constant BUSDFeed = 	0x9331b55D9830EF609A2aBCfAc0FBCE050A52fdEa;

    mapping(address => uint) prices; //价格map
    mapping(address => address) aggregators; //Aggregators map, 底层资产地址, chainlinkFeed地址


    constructor() public {
        //priceFeed = AggregatorV3Interface();
	aggregators[ETH] = ETHFeed;
	aggregators[BTC] = BTCFeed;
	aggregators[USDT] = USDTFeed;
	aggregators[USDC] = USDCFeed;
	aggregators[DAI] = DAIFeed;
	aggregators[BUSD] = BUSDFeed;
    }

    event PricePosted(address asset, uint previousPriceMantissa, uint requestedPriceMantissa, uint newPriceMantissa);

    function getUnderlyingPrice(EToken eToken) public view returns (uint) {
        if (compareStrings(eToken.symbol(), "eETH")){
            return 1e18;
        } else {
	    address feedAddr = aggregators[address(CErc20(address(eToken)).underlying())];
	    ...
	    }else
	    {
		    ...
		    return uint(price*1e10); //保持精度为18
	    }
        }
    }

    function setUnderlyingPrice(EToken eToken, uint underlyingPriceMantissa) public {
