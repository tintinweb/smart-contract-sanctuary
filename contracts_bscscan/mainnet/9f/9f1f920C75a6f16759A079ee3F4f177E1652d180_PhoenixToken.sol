...
contract PhoenixToken is Context, IERC20, Ownable {
    ...
    uint256 private  _tTotal = 500000000000000 * 10**uint256(_decimals);
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    
    uint256 public  MAX_STOP_FEE_TOTAL = 50000000 * 10**uint256(_decimals);
   ...
   function calculateBurnFee(uint256 _amount) private view returns (uint256) {
    ...
    function cancelAllFee() public onlyOwner {
        if(_taxFee == 0 && _burnFee == 0) return;
        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _taxFee = 0;
