...
// HuginToken with Governance.
contract HuginToken is BEP20 {
    // Transfer tax rate in basis points. (default 6%)
    uint16 public transferTaxRate = 600;
    // Burn rate % of transfer tax. (default 20% x 6% = 1.2% of total amount).
    ...
    // Min amount to liquify. (default 500 HGN)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to KrustyHugin's router when our own AMM release
    IUniswapV2Router02 public huginSwapRouter;
    // The trading pair
    address public huginSwapPair;
    // In swap and liquify
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "HGN::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the HuginToken contract.
     */
    constructor() public BEP20("Hugin Token", "HGN") {
        ...
        huginSwapPair = IUniswapV2Factory(_uniswapV2Router.factory())
    ...
    /// @dev overrides transfer function to meet tokenomics of HGN
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(huginSwapRouter) != address(0)
            && huginSwapPair != address(0)
            && sender != huginSwapPair
            && sender != owner()
        ) {
            swapAndLiquify();
        }
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "HGN::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "HGN::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the huginSwap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = huginSwapRouter.WETH();

        _approve(address(this), address(huginSwapRouter), tokenAmount);

        // make the swap
        huginSwapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        huginSwapRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from huginSwapRouter when swapping
    ...
    */
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "HGN::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying KPs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "HGN::_writeCheckpoint: block number exceeds 32 bits");
