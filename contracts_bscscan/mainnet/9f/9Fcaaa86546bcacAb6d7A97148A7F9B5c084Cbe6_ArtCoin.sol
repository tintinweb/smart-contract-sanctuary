...
contract ArtCoin is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    // : In Reflection contracts like TCGCoin 2.0, _rTotal is like the money shown in the balance sheets 
    // of the central bank, while _tTotal is like the currency in circulation. When someone sells and 
    // a fee is to be distributed to all holders, instead of doing it with a for loop, _rTotal is 
    // shrunk so everyone’s money is worth more, just like deflation.
    
    // In Reflect, you can understand _rTotal as the total money supply in the central bank, and _tTotal 
    ...
    // This is done by changing the _tTotal and _rTotal in _reflectFee function, which is called every time 
    ...
    // more/less. The mechanism is illustrated in the figure below:
    ...
    uint256 private _tTotal = 280 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "ArtCoin v26";
    ...
    uint256 public _maxTxAmount = 140 * 10**4 * 10**9 ;
    ...
    // \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ Rewards calculation and managing functions \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ 
    ...
    // The ‘deliver’ function allows any wallet to donate directly to the funds distributed as rewards.
    ...
    // Removing tokens from reflection.
    ...
    //  By using the excludeFromReward function in the SafeMoon (or other RFI forks) smart contract, 
    //  the desired accounts can be excluded from receiving holder rewards. For example, 
    //  by excluding the contract owner or the address where the exchange liquidity is allocated, 
    //  these accounts can be prevented from benefiting from the holder rewards.
    ...
    function setWithFeeFlag(bool value) external onlyOwner() {
        _withFee = value;
    }
    ...
    // \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ Fee calculation and managing functions \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ 
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax and a liquidity fee
        if(!takeFee)
            removeAllFee();
