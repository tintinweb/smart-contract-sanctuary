...
// File: contracts/utils/GeometryProfile.sol
...
contract BunnySpecialV1 is Ownable {
    ...
    GeometryProfile public geometryProfile;
    ...
    function mintNFT(uint8 _bunnyId) external {
        ...
        (userId, , , , , isUserActive) = geometryProfile.getUserProfile(
            senderAddress
        );
        ...
        if (bunnyCharacteristics[_bunnyId].gxoCost > 0) {
            gxoToken.safeTransferFrom(
                senderAddress,
                address(this),
                bunnyCharacteristics[_bunnyId].gxoCost
            );
        }
    ...
    function addBunny(
        ...
        emit BunnyAdd(_bunnyId, _thresholdUser, _gxoCost);
    }
    ...
    function updateBunny(
        ...
        bunnyCharacteristics[_bunnyId].gxoCost = _gxoCost;
        bunnyCharacteristics[_bunnyId].isActive = _isActive;

        emit BunnyChange(_bunnyId, _thresholdUser, _gxoCost, _isActive);
    }
    ...
    function canClaimSingle(address _userAddress, uint8 _bunnyId)
        ...
        returns (bool)
    {
        if (!geometryProfile.hasRegistered(_userAddress)) {
            return false;
        } else {
            uint256 userId;
            bool userStatus;

            (userId, , , , , userStatus) = geometryProfile.getUserProfile(
                _userAddress
            );
