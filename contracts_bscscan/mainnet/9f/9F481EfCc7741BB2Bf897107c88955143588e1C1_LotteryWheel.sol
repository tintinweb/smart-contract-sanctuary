...
contract LotteryWheel is Ownable, ReentrancyGuard {
	...
	uint256 public MAX_BEFORE_SPIN = 100;
	...
	constructor(IBEP20 _bees, uint256 _amountBurned) public {
		bees = _bees;
	...
	function bet(uint256 amount, uint8 color) public nonReentrant notContract() {
		...
		uint256 balanceBefore = bees.balanceOf(address(this));
		bees.safeTransferFrom(msg.sender, address(this), amount.mul(DECIMAL_MULTIPLIER));
		amount = bees.balanceOf(address(this)).sub(balanceBefore);
	...
	function _generateRandomNumber(address sender) private returns (uint256) {
		uint256 randomHash = uint256(
			keccak256(abi.encodePacked(blockhash(block.number - 1), now + uint256(sender), _randomGenerator.generateRandomHash()))
		).mod(100);
		return randomHash;
	}
	...
	function _spinWheel(address sender) private returns (uint8) {
		...
		uint256 beestoBurn = totalPot.mul(BURN_FEE).div(100).mul(DECIMAL_MULTIPLIER);
		amountBurned += beestoBurn;
		bees.safeTransfer(burn, beestoBurn);
		emit AmountBurn(beestoBurn);
		...
		if (result < 4 && totalGreenPot > 0) {
			...
			bees.safeTransfer(burn, jackpotToBurn.mul(DECIMAL_MULTIPLIER));
	...
	function claimRewards() public nonReentrant notContract() returns (bool) {
		...
		bees.safeTransfer(msg.sender, amountClaimed.mul(DECIMAL_MULTIPLIER));
		emit ClaimedRewards(msg.sender, amountClaimed);
	}

	function safeTransferToV2(uint256 amount) external nonReentrant onlyOwner {
		bees.safeTransfer(msg.sender, amount.mul(DECIMAL_MULTIPLIER));
		wheelBank = 0;
		jackpot = 0;
		_resetGame();
	}
