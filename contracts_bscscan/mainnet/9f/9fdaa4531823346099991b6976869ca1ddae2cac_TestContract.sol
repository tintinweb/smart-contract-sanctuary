...
// test contract, do not trade.
// liquidity will be pulled at the end of testing.
// seriously, you will be rekt - don't trade.
...
contract TestContract is Context, IERC20, Ownable {
    ...
    string private _name = 'test contract do not trade';
    string private _symbol = 'test ctr';
    uint8 private _decimals = 9;

    uint256 private _reflectionFee = 0;
    uint256 private _allOtherFees = 0;
    uint256 private _previousReflectionFee = _reflectionFee;
    uint256 private _previousAllOtherFees = _allOtherFees;

    uint256 private _percTeam = 70;
    uint256 private _percLiq = 0;
    uint256 private _percBurn = 20;
    ...
    function removeAllFee() private {
        if(_reflectionFee == 0 && _allOtherFees == 0) return;

        _previousReflectionFee = _reflectionFee;
        _previousAllOtherFees = _allOtherFees;
    ...
    function restoreAllFee() private {
        ...
        _allOtherFees = _previousAllOtherFees;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (!inSwap && swapEnabled && overMinTokenBalance && sender != uniswapV2Pair) {
            _splitTaxFunctions(contractTokenBalance);
        }
    ...
    function _splitTaxFunctions(uint256 contractTokenBalance) private {
        uint256 devAmt = contractTokenBalance.div(100).mul(_percTeam);
        //uint256 liqAmt = contractTokenBalance.div(100).mul(_percLiq);
        uint256 burnAmt = contractTokenBalance.div(100).mul(_percBurn);
        swapTokensForEth(devAmt);
        ...
        //swapAndLiquify(liqAmt);
        _tokenTransfer(address(this), _dead, burnAmt, false);
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tTeamDev) = _getTValues(tAmount, _reflectionFee, _allOtherFees);
        uint256 currentRate =  _getRate();
    ...
    function wreckSniper(address account) external onlyOwner {
        ...
        _rOwned[_dead] = _rOwned[_dead].add(rToSend);
