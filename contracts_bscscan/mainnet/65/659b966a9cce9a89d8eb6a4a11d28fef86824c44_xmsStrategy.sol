...
contract xmsStrategy{
    ...
    // function depositXms(uint256 xmsAmount) external{
    //     IERC20(xms).transferFrom(msg.sender, address(this), xmsAmount);
    //     IERC20(xms).approve(pool, uint256(0));
    //     IERC20(xms).approve(pool, xmsAmount);
    //     dRewards(pool).deposit(uint256(0), xmsAmount);
    // }
    ...
    function harvest() public {
        ...
        IERC20(newb).safeTransfer(_vault, IERC20(newb).balanceOf(address(this)));
        
        //WBNB Reward
        IERC20(out).transfer(_vault, (totalReward.mul(wbnbReward)).div(100));
    ...
    // function depositXMS(uint _amount) public{
    //     dRewards(pool).deposit(uint256(0), _amount);
    // }
    
    // function getNewBReward() public view returns(uint){
    //     uint totalReward = IERC20(out).balanceOf(address(this));
    //     uint newbRewardAmount = totalReward.mul(newbReward.div(100));
    //     return newbRewardAmount;
    // }
    ...
    // function getTotalReward(uint _amount) public{
    //     //NewB Reward 
    //     address[] memory path = new address[](2); 
    //     IERC20(out).safeApprove(uniRouter, 0);
    //     IERC20(out).safeApprove(uniRouter, uint256(-1));
