...
contract ShibArmy is ERC20, Ownable {
    ...
    ShibArmyDividendTracker public dividendTracker;
    ...
    uint256 public buyShibRewardsFee = 2;
    uint256 public sellShibRewardsFee = 2;
    uint256 public deadFee = 5;
    uint256 public AmountLiquidityFee = 0;
    uint256 public AmountShibRewardsFee = 0;
    ...
    constructor() ERC20("ShibArmy", "ShibArmy") {
        ...
        dividendTracker = new ShibArmyDividendTracker();
        ...
        swapTokensAtAmount = 1000000000000 * (10**18);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "ShibArmy: The dividend tracker already has that address"
        );

        ShibArmyDividendTracker newDividendTracker = ShibArmyDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "ShibArmy: The new dividend tracker must be owned by the ShibArmy token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "ShibArmy: The router already has that address"
        );
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "ShibArmy: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function multiWhitelistAddress(address[] calldata addresses, bool value)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            _isWhitelisted[addresses[i]] = value;
        }
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "ShibArmy: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "ShibArmy: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "ShibArmy: Cannot update gasForProcessing to same value"
        );
    ...
    function setbuyShibRewardsFee(uint256 amount) public onlyOwner {
        buyShibRewardsFee = amount;
    }

    function setsellShibRewardsFee(uint256 amount) public onlyOwner {
        sellShibRewardsFee = amount;
    }
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            super._transfer(owner(), addresses[i], tokens);
        }
    }
    ...
    function _transfer(
        ...
        if (
            ...
            require(_isWhitelisted[from] || _isWhitelisted[to], "Disabled");
        }
        ...
        if (
            ...
            swapAndSendDividends(AmountShibRewardsFee);
            swapping = false;
        }
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                ...
                CFee = amount.mul(buyShibRewardsFee).div(100);
                AmountShibRewardsFee += CFee;
                DFee = amount.mul(deadFee).div(100);
                fees = LFee.add(CFee).add(DFee);
            }
            if (automatedMarketMakerPairs[to]) {
                LFee = amount.mul(sellLiquidityFee).div(100);
                AmountLiquidityFee += LFee;
                CFee = amount.mul(sellShibRewardsFee).div(100);
                AmountShibRewardsFee += CFee;
                DFee = amount.mul(deadFee).div(100);
                fees = LFee.add(CFee).add(DFee);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        AmountShibRewardsFee = AmountShibRewardsFee - tokens;
...
contract ShibArmyDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken(
            "ShibArmy_Dividen_Tracker",
            "ShibArmy_Dividend_Tracker"
        )
    {
        claimWait = 86400;
        minimumTokenBalanceForDividends = 100000000000 * (10**18); //must hold tokens for dividen
    }
    ...
    function _transfer(
        ...
        require(false, "ShibArmy_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ShibArmy_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ShibArmy contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "ShibArmy_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "ShibArmy_Dividend_Tracker: Cannot update claimWait to same value"
        );
