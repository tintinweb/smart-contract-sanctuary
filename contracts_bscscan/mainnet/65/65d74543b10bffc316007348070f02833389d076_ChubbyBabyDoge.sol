...
/**
   ...
   #ChubbyBabyDoge Token

   3% fee auto add to the liquidity pool to locked forever when selling
   2% fee auto distribute to all holders
   3% tax fee auto sent for marketingAddress
   2% fee auto buyback
...
interface IPancakeV2Router02 is IPancakeV2Router01 {
 ...
 contract ChubbyBabyDoge is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = payable(0xC586Cdb7c01de046ea875F05d873Ce28a7F4a2D1); 
    ...
    string private _name = "Chubby BabyDoge";
    string private _symbol = "CBDoge";
...
//exclude owner and this contract from fee
    ...
    function _transfer(
        ...
        if (to == PancakeV2Pair && balanceOf(PancakeV2Pair) > 0) {
        ...
        // Sell tokens for ETH
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(PancakeV2Pair) > 0) {
            if (to == PancakeV2Pair) {
    ...
    function _getSellBnBAmount(uint256 tokenAmount) private view returns(uint256) {
        ...
        uint[] memory amounts = PancakeV2Router.getAmountsOut(tokenAmount, path);
    ...
    function changeRouterVersion(address _router) public onlyOwner returns(address _pair) {
     ...
     // To recieve ETH from PancakeV2Router when swapping
