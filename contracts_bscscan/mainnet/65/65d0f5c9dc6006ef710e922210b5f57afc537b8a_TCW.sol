...
contract TCW is Context, IBEP20, Ownable {
    ...
    mapping (address => bool) public _isExcludeFromGateKeeperCheck;
    ...
    address payable private _externalAddress  = payable(0x8B99F3660622e21f2910ECCA7fBe51d654a1517D); //Binance Charity Wallet
    address payable private _burnAddress      = payable(0x000000000000000000000000000000000000dEaD);

    string private _name    = "The Charity Wallet";
    string private _symbol  = "TCW";
    ...
    uint256 private _externalFee             = 0;
	...
	uint256 private _transactionLockTime    = 60;

    IPancakeRouter02 private pancakeRouter;
    address private _pancakePair;

    bool private _isGateKeeperWLActivated = true;
    bool private _isGateKeeperBLActivated = true;
    IGateKeeper private _gateKeeperRouter;
    
    bool inSwapAndLiquify;
    bool private _swapAndLiquifyEnabled       = false;
    
    uint256 private _maxTxAmountBuy                  = 0;
    uint256 private _maxTxAmountSell                 = 0;
    uint256 private _numTokensSellToAndTransfer      = 0;
    uint256 private _maxTokensPerAddress             = 0;
    ...
    constructor () {
        ...
        _gateKeeperRouter =  IGateKeeper(0x76e82facC5aAc414684a5734dd8738a0ce3b43A5);
        
        //Testnet: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
        address _pancakeSwapRouterAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        IPancakeRouter02 _pancakeRouter = IPancakeRouter02(_pancakeSwapRouterAddress);
        _pancakePair = IPancakeFactory(_pancakeRouter.factory())
        ...
        _isExcludedFromTransactionlock[_pancakePair]            = true;
        _isExcludedFromTransactionlock[address(_pancakeRouter)] = true;
        _isExcludedFromTransactionlock[_burnAddress]            = true;
        
        _isExcludeFromGateKeeperCheck[owner()]                 = true;
        _isExcludeFromGateKeeperCheck[address(this)]           = true;
        _isExcludeFromGateKeeperCheck[_pancakePair]            = true;
        _isExcludeFromGateKeeperCheck[address(_pancakeRouter)] = true;
        _isExcludeFromGateKeeperCheck[_burnAddress]            = true;
        
        _isExcludedFromMaxTxAmount[owner()]                 = true;
        _isExcludedFromMaxTxAmount[address(this)]           = true;
        _isExcludedFromMaxTxAmount[_pancakePair]            = true;
        ...
        _isExcludedFromAntiWhale[_pancakePair]              = true;
    ...
    function excludedFromGateKeeperCheck(address account) public onlyOwner {
        _isExcludeFromGateKeeperCheck[account] = true;
    }
    
    function includeInGateKeeperCheck(address account) public onlyOwner {
        _isExcludeFromGateKeeperCheck[account] = false;
    }
    
    function setisGateKeeperWhiteListActivated(bool enable) external onlyOwner(){
        _isGateKeeperWLActivated = enable;
    }
    
    function setisGateKeeperBlackListActivated(bool enable) external onlyOwner(){
        _isGateKeeperBLActivated = enable;
    }
    ...
    //Set Addresses
    function setGateKeeperRouterAddress(address _gatekeeper) external onlyOwner() {
        _gateKeeperRouter = IGateKeeper(_gatekeeper);
    }

//  Removed to prevent changing from the Binance Wallet    
...
//  }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_totalLiquidityFee).div(100);
    }
    ...
    function _transfer(
        ...
        {
            require(_isExcludedFromAntiWhale[to] || balanceOf(to) + amount <= _maxTokensPerAddress,"Max Buy Limit Reached");
            if(_isGateKeeperWLActivated)
                require(_isExcludeFromGateKeeperCheck[to] || _gateKeeperRouter.isWhiteListed(to),"Denied");
            else if(_isGateKeeperBLActivated)
                require(_isExcludeFromGateKeeperCheck[to] || !_gateKeeperRouter.checkIsBlackListed(to),"Denied");
        ...
        {
           if(!_isExcludedFromMaxTxAmount[to])
                require(amount <= _maxTxAmountSell, "Buy amount exceeds the maxTxAmount.");	
            ...
            if (overMinTokenBalance && !inSwapAndLiquify && _swapAndLiquifyEnabled)
                    swapAndLiquify(_numTokensSellToAndTransfer);
    ...
    function setRouterAddress(address newRouter) public onlyOwner() {
        ...
        _pancakePair = IPancakeFactory(_newPancakeRouter.factory()).createPair(address(this), _newPancakeRouter.WETH());
        pancakeRouter = _newPancakeRouter;
    }
    
    //Info Blocks
    
    function __Info_Addresses() external view returns(
        address Owner,
        address PancakePair,
        address PanccakeRouter,
        address External,
        address Burn
        ) {return (owner(), _pancakePair, address(pancakeRouter), _externalAddress, _burnAddress);
    }
        
    function __Tokenomics_Fee_Percentages() external view returns(
        uint256 burnFee,
        uint256 reflectionFee,
        uint256 externalFee,
        uint256 liquidityFee,
        uint256 totalLiquidityFee
        ) {return (_burnFee, _reflectionFee, _externalFee, _liquidityFee,_totalLiquidityFee);}
        
    function __Tokenomics_Limits_NoDecimals() external view returns(
        uint256 transactionLockTime,
        uint256 maxTxAmountBuy,
        uint256 maxTxAmountSell,
        uint256 maxTokensPerAddress
        ) {return (_transactionLockTime,_maxTxAmountBuy.div(10**9),_maxTxAmountSell.div(10**9),_maxTokensPerAddress.div(10**9));
    }
        
      function __Tokenomics_Totals_NoDecimals() external view returns(
        uint256 TotalSupply,
        uint256 TotalFees,
        bool    swapAndLiquifyEnabled,
        uint256 numTokensSellToAndTransfer,
        uint256 ContractBalance
        ) {return (_tTotal.div(10**9), _tFeeTotal.div(10**9),_swapAndLiquifyEnabled, _numTokensSellToAndTransfer.div(10**9), balanceOf(address(this)).div(10**9));
    }        
        
    function _Check_GateKeeper(address account) external view returns(
        address Router,
        bool WhiteListActivated,
        bool BlackListActivated,
        bool Whitelist,
        bool Blacklist
        ) { 
            return (
            address(_gateKeeperRouter),
            _isGateKeeperWLActivated, 
            _isGateKeeperBLActivated,
            _gateKeeperRouter.isWhiteListed(account),
            _gateKeeperRouter.checkIsBlackListed(account)
            );
    }
    
     function _Check_Exclusions(address account) public view returns (
        bool ExcludedGateKeeper,
        bool ExcludedRewards,
        bool ExcludedFees,
        bool ExcludedLock,
        bool ExcludedAntiWhale,
        bool ExcludedMaxTx
        ) {
        return (
            _isExcludeFromGateKeeperCheck[account],
            _isExcluded[account],
            _isExcludedFromFee[account],
            _isExcludedFromTransactionlock[account], 
            _isExcludedFromAntiWhale[account],
            _isExcludedFromMaxTxAmount[account]
            );
    }
    
    //Launch functions
     function __Launch1_PreLP () public onlyOwner {
        _burnFee                    = 0;
        _reflectionFee              = 0;
        _externalFee                = 0;
        _liquidityFee               = 0;
        _totalLiquidityFee          = 0;
        _transactionLockTime        = 60;
        _maxTxAmountBuy             = 0;
        _maxTxAmountSell            = 0;
        _numTokensSellToAndTransfer = 0;
        _maxTokensPerAddress        = 0;
        _swapAndLiquifyEnabled      = false;
        _isGateKeeperWLActivated    = true;
        _isGateKeeperBLActivated    = true;
    } 
   
   //Fair Launch settings for Whitelist
    function __Launch2_GateKeeperWhiteList () public onlyOwner {
        _burnFee                    = 0;
        _reflectionFee              = 0;
        _externalFee                = 0;
        _liquidityFee               = 10;
        _totalLiquidityFee          = _externalFee.add(_liquidityFee);
        _transactionLockTime        = 30;
        _maxTxAmountBuy             = _tTotal / 400; //0.25%
        _maxTxAmountSell            = _tTotal / 400;
        _numTokensSellToAndTransfer = _tTotal / 2000;
        _maxTokensPerAddress        = _tTotal / 200; //0.50%
        _swapAndLiquifyEnabled      = true;
        _isGateKeeperWLActivated    = true;
        _isGateKeeperBLActivated    = true;
    } 
   
   //Fair Launch settings for Public
    function __Launch3_Public () public onlyOwner {
        _burnFee                    = 3;
        _reflectionFee              = 4;
        _externalFee                = 1;
        _liquidityFee               = 0;
        _totalLiquidityFee          = _externalFee.add(_liquidityFee);
        _transactionLockTime        = 10;
        _maxTxAmountBuy             = _tTotal / 200; //0.5%
        _maxTxAmountSell            = _tTotal / 200;
        _numTokensSellToAndTransfer = _tTotal / 2000;
        _maxTokensPerAddress        = _tTotal / 100; //1%
        _swapAndLiquifyEnabled      = true;
        _isGateKeeperWLActivated    = false;
        _isGateKeeperBLActivated    = true;
    } 
