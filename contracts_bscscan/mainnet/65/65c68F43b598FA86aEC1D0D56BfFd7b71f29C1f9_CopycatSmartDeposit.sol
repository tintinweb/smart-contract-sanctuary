...
contract CopycatSmartDeposit is Ownable, ReentrancyGuard {
  ...
  event AdminRecoverBnb(address indexed admin, uint256 amount);
  function adminRecoverBnb() external onlyOwner {
    emit AdminRecoverBnb(msg.sender, address(this).balance);
    payable(msg.sender).transfer(address(this).balance);
  }

  event AdminRecoverToken(address indexed admin, address indexed token, uint256 amount);
  function adminRecoverToken(IERC20 token) external onlyOwner {
    uint256 balance = token.balanceOf(address(this));
    token.transfer(msg.sender, balance);
    emit AdminRecoverToken(msg.sender, address(token), balance);
  }
  ...
  function depositUsingToken(ICopycatLeader leader, uint256 tokenAmount, address router, uint256 shareMin, address[] calldata path) public nonReentrant returns (uint256 shareOutputAmount) {
    ...
    uint256 fee = leader.depositCopycatFee() * leader.factory().COPYCAT_FEE_BASE() / 1e18;
    leader.factory().copycatToken().transferFrom(msg.sender, address(this), fee + 10); // +10 to prevent rounding error;
    leader.factory().copycatToken().approve(address(leader.factory()), fee + 10);
    ...
    (shareOutputAmount, ) = leader.depositTo{value: bnbOutputAmount}(msg.sender, shareMin);

    // Reset approval
    token.approve(router, 0);
    leader.factory().copycatToken().approve(address(leader.factory()), 0);
  ...
  function withdrawToToken(ICopycatLeader leader, uint256 shareAmount, address router, uint256 tokenMin, address[] calldata path) public nonReentrant returns (uint256 tokenOutputAmount) {
    leader.transferFrom(msg.sender, address(this), shareAmount);

    // uint256 fee = leader.withdrawCopycatFee() * leader.factory().COPYCAT_FEE_BASE() / 1e18;
    // leader.factory().copycatToken().transferFrom(msg.sender, address(this), fee + 10); // +10 to prevent rounding error;
    // leader.factory().copycatToken().approve(address(leader.factory()), fee + 10);
    
    uint256 bnbOutputAmount;
    (bnbOutputAmount, ) = leader.withdrawTo(address(this), shareAmount, 0);
    ...
    tokenOutputAmount = token.balanceOf(address(this)) - tokenBefore;
    token.transfer(msg.sender, tokenOutputAmount);

    emit WithdrawToToken(address(leader), msg.sender, address(token), router, shareAmount, bnbOutputAmount, tokenOutputAmount);
  }
}
