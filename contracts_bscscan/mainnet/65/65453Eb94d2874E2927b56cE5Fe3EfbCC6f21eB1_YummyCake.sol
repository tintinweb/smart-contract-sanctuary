...
LAUNCH AT 200 MEMBERS

🍰EARN CAKE TOKEN AS A REWARD FOR HOLDING YUMMY CAKE.
...
🍰50% Burn Supply
🍰3T Max Buy and Sell
🍰Hold atleast 200B to be eligible for CAKE TOKEN Rewards

🍰Tax: 12% Rewards / 6% Liquidity / 6% Marketing ( a total of 24 tax ) (for a better rewards) 

🍰10% Marketing Token ( this is for promotion like CMS, Website, Paid Shiller, Influencer promotion) ( We will notify the group every sell + token this is to easily monitor the token for marketing.


🎂🎂🎂🎂🎂🎂🎂🎂🎂🎂🎂🎂🎂🎂🎂


WHY ITS GOOD TO BUY AND HOLD YUMMY CAKE?

🍰You will earn CAKE Token everytime there is Buy and Sell transaction.

🍰Anti Whale: We set 3T for Max Buy. 

🍰Anti Bot: We have codes that will prevent bots. 
...
contract YummyCake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    YummyCakeDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 public maxBuyTranscationAmount = 300000000000000 * (10**18);
    uint256 public maxSellTransactionAmount = 3000000000000 * (10**18);
    uint256 public swapTokensAtAmount = 3000000000001 * (10**18);
    uint256 public maxWalletToken = 500000000000000 * (10**18); // 100% of total supply

    uint256 public dividendRewardsFee;
    uint256 public marketingFee;
    uint256 public immutable totalFees;

    // sells have fees of 12 and 6 (12 * 1 and 6 * 1)
    ...
    constructor() ERC20("YummyCake", "YummyCake") {
    	...
    	dividendTracker = new YummyCakeDividendTracker();

    	buyBackWallet = 0x615c4602854AAF15C517bD559034DB19B01e9463;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 300, "YummyCake: Sell transaction multipler must be between 100 (1x) and 300 (3x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "YummyCake: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "YummyCake: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "YummyCake: The dividend tracker already has that address");

        YummyCakeDividendTracker newDividendTracker = YummyCakeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "YummyCake: The new dividend tracker must be owned by the YummyCake token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "YummyCake: Dividend reward tax must be between 0 and 10");
        dividendRewardsFee = newFee;
    }
    
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 1000000000, "YummyCake: Dividend reward tax must be between 0 and 1000000000");
        marketingFee = newFee;
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "YummyCake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "YummyCake: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "YummyCake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "YummyCake: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "YummyCake: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "YummyCake: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "YummyCake: Cannot update gasForProcessing to same value");
...
contract YummyCakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("YummyCake_Dividend_Tracker", "YummyCake_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 200000000000 * (10**18); //must hold 200000000000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "YummyCake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "YummyCake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main YummyCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 36000, "YummyCake_Dividend_Tracker: claimWait must be updated to between 30 and 300 minutes");
        require(newClaimWait != claimWait, "YummyCake_Dividend_Tracker: Cannot update claimWait to same value");
