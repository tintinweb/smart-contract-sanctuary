 ...
 */ 
 ...
 // @metagirlsbscgem, Join us on telegram now!
...
contract METAGIRLS is IERC20Metadata, Ownable {
    ...
    uint256 public marketingPortionOfSwap = 30;
    uint256 public jackpotPortionOfSwap = 30;
    ...
    uint256 private buyersTotalFees = buyingMarketingFee.add(buyingRfiFee).add(buyingLpFee).add(buyingJackpotFee);
    uint256 private sellersTotalFees = sellingMarketingFee.add(sellingRfiFee).add(sellingLpFee).add(sellingJackpotFee);
    ...
    function calcBuyersTotalFees() private {
        buyersTotalFees = buyingMarketingFee.add(buyingRfiFee).add(buyingLpFee).add(buyingJackpotFee);
    }
    
    function calcSellersTotalFees() private {
        sellersTotalFees = sellingMarketingFee.add(sellingRfiFee).add(sellingLpFee).add(sellingJackpotFee);
    }
    ...
    function _beforeTokenTransfer(address recipient) private {
        ...
        if ( !isInPresale ){
            ...
            if (!swapping && canSwap && swapEnabled && recipient == pair) {
                ...
                uint256 jackPAndMarkP = marketingPercent.add(jackpotPercent);
                ...
                sendToJackpotAddress(jackpotPercent);
    ...
    function sendToJackpotAddress(uint256 tokenAmount) private {
        uint256 initialBalance = address(this).balance;
        swapTokensForBNB(tokenAmount);
        
        uint256 transferredBalance = address(this).balance.sub(initialBalance);
        
         //Send to Jackpot address
       transferToAddressBNB(payable(jackpotWallet), transferredBalance);
    }
