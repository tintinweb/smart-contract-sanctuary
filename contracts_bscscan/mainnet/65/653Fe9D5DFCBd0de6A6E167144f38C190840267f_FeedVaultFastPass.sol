...
contract FeedVaultFastPass is FeedVault {
     ...
     */
    uint256 public referralEntryFeeBP = 7000;
    uint256 public referralExitFeeBP = 0;
     ...
     * @dev Deposit Fund to FeedVault
     */
    function depositReferrer(uint256 _amount, address _referrer) public virtual isEnabled hasVaults nonReentrant {
    ...
    function _collectEntryFees(uint256 _amount) internal override {
        ...
        if (_entryFees > 0) {
            if (autoSwapEntryFees) {
                IERC20 _feesToken = IERC20(entrySwapToken);
                uint256 buyBackBefore = _feesToken.balanceOf(address(this));
                ...
                uint256 buyBackAfter = _feesToken.balanceOf(address(this));
                uint256 buyBackAmount = buyBackAfter.sub(buyBackBefore);

                uint256 _comissionPaid =
                    _payReferralCommission(address(msg.sender), address(_feesToken), buyBackAmount, referralEntryFeeBP);
                uint256 _remainingFees = buyBackAmount.sub(_comissionPaid);
                _feesToken.safeTransfer(entryFeesCollector, _remainingFees);

                emit FeesCollected(true, address(entryFeesCollector), address(entrySwapToken), _remainingFees);
            } else {
    ...
    function _collectExitFees(uint256 _amount) internal override returns (uint256) {
        ...
        if (_exitFees > 0) {
            if (autoSwapExitFees) {
                IERC20 _feesToken = IERC20(exitSwapToken);
                uint256 buyBackBefore = _feesToken.balanceOf(address(this));
                ...
                uint256 _comissionPaid =
                    _payReferralCommission(address(msg.sender), address(_feesToken), buyBackAmount, referralExitFeeBP);
                uint256 _remainingFees = buyBackAmount.sub(_comissionPaid);
                _feesToken.safeTransfer(exitFeesCollector, _remainingFees);

                emit FeesCollected(false, address(exitFeesCollector), address(exitSwapToken), _remainingFees);
            } else {
