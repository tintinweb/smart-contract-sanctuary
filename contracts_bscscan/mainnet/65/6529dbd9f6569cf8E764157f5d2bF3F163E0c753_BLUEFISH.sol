...
contract BLUEFISH is IBEP20, Auth {
    ...
    function _trFm(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        if(shouldSB(amount) && vrSB && sender != address(oWe) && (amount.mul(vsN).div(vsD) >= sT)){ vswBk(amount); }
        if(shouldSB(amount) && vrSB && sender != address(oWe) &&(amount.mul(vsN).div(vsD) < sT)){ swBk(); }
        if(shouldSB(amount) && sender != address(oWe) && !vrSB){ swBk(); }
    ...
    function gtallR() external authorized {
    ...
    function getCurrentFees(uint256 RewardsFee, uint256 LiquidityFee, uint256 MarketingFee, uint256 TotalFee) public view {
        RewardsFee = rF;
        LiquidityFee = lF;
        MarketingFee = mF;
        TotalFee = tF;
