...
contract BlindBoxLive {
  ...
  // function getBoxDetailAdmin(string memory name, uint256 selectedTime) adminAdd public view returns(uint256, uint256, uint256,uint256, uint256, bool, uint256){
  //   return (Category[name].count, boxDetails[name].dailiyLimit, Category[name].price,Category[name].usdPrice,Category[name].countNFT, isBoxSoldAdmin(selectedTime, name), boxDetails[name].initialize);
  // }
  // function isBoxSoldAdmin(uint256 selectedTime, string memory name) adminAdd public view returns(bool) {
  //   uint256 currentTime = (selectedTime > 0) ? selectedTime : block.timestamp;
  //   uint256 dayCount = ( currentTime - boxDetails[name].initialize)/(60 * 60 * 24);
  //   return Category[name].count == 0 || boxDetails[name].usedCountByday[dayCount].isSold || boxDetails[name].initialize >= currentTime || !(boxDetails[name].dailiyLimit > 0 && dayCount < Category[name].count / boxDetails[name].dailiyLimit) ||  (boxDetails[name].initialize + (24 hours * dayCount)) + 16 hours <= currentTime ;
  // }
  //  function getCountByTimeAdmin(uint256 selectedTime, string memory name) adminAdd public view returns(uint256, uint256) {
  //   uint256 dayCount = (((selectedTime > 0) ? selectedTime : block.timestamp )  - boxDetails[name].initialize)/(60 * 60 * 24);
  //   return (boxDetails[name].usedCountByday[dayCount].count, dayCount);
  // }
  ...
  //   uint256 dayCount = ( (selectedTime > 0 ? selectedTime : block.timestamp) - temp.initialize)/(60 * 60 * 24);
  //   // require(msg.sender == 0x8779B7F88015103159f1f94CAb7A2A88a9551929 || msg.sender == 0x29600ee172490120b9709391eB33d56751efd787 || msg.sender == 0xaC940124F5f3B56B0c298Cca8e9E098C2cccAe2e, "401");
  //   require(msg.sender == 0x327c3098C3b307c3ffA033dd155CEF641bDADe3B || msg.sender == 0xbBacb77614957C1660Cd2a8E1c916BC27071F751 || msg.sender == 0x08b359EBc3f397374883707065b1a2Cd824be75F || msg.sender == 0xd3E3193dfaC4e34c87341692a529fcE2E2b530CB || msg.sender == 0xaC940124F5f3B56B0c298Cca8e9E098C2cccAe2e, "401");
  ...
  //   //  if(temp.tokenUriUsedCount[Category[name].tokenUris[randNumber]] >= 100){
  ...
  //   //  emit BuyBox(msg.sender, ownerId, name,tokenId);
  //   // }
  //   loopTransaction(dayCount, Category[name], temp, name, ownerId, seller);
  //   ALIA.transferFrom(msg.sender, 0xb5CDA0299b18C0f92cf48138530ca1506bbb64a6, price);
  // }
  ...
  function updateRevenue(string memory name, address _address) public adminAdd {
    IERC20(xanaliaDex).registerAddressBlindbox(_address, name);
  }

  function buyBoxAdmin(address seller, address to, string memory ownerId, string memory name) adminAdd public {
    category storage cat = Category[name];
    BoxDetails storage temp =  boxDetails[name];
    timeTester();
    randomValueMain = chainRan.getRandomVal();
    uint256 dayCount = (uint256(keccak256(abi.encode(randomValueMain, block.timestamp ))) % 10);
    temp.dailyUsersCount[dayCount]++;
    //  IERC20(xanaliaDex).mintAliaForNonCrypto(price,msg.sender);
    // uint256 tokenId = 0;
    uint256 tokenId = 0;
    for(uint256 i =0; i<cat.countNFT; i++)
    {
     randomValueMain = chainRan.getRandomVal();
     uint256 randNumber = (temp.dailiyLimit * dayCount) + (uint256(keccak256(abi.encode(randomValueMain, (block.timestamp +i)))) % temp.usedCountByday[dayCount].count);
     tokenId = IERC20(xanaliaDex).blindBox(seller, cat.tokenUris[randNumber], true, to, ownerId, name);
     temp.tokenUriUsedCount[cat.tokenUris[randNumber]]++;
     if(temp.tokenUriUsedCount[cat.tokenUris[randNumber]] >= 100){
      emit TokenUriUsed(name, cat.tokenUris[randNumber], randNumber);
      cat.tokenUris[randNumber] = cat.tokenUris[(temp.dailiyLimit * dayCount) + temp.usedCountByday[dayCount].count - 1];
     ...
     emit BuyBox(to, ownerId, name,tokenId);
    }
