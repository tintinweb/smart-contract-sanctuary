...
contract rechargeToken is Ownable{
    ...
    event USDCDeposti(address indexed user, uint256 depositAmount);
    ...
    function exchange(uint256 _amount)external {
        ...
        USDC.transferFrom(msg.sender, BusinessWallet, _amount.sub(tokens));
        USDC.transferFrom(msg.sender, SupportWallet, tokens);
        emit USDCDeposti(msg.sender, _amount);
    }
