...
contract DogeSon is Context, IBEP20, Ownable {
    ...
    string private _name = "DogeSon";
    string private _symbol = "DOGESON";
    uint8 private _decimals = 18;
    
    uint256 public _taxFee = 100; // 1%
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 1400; // Percentages for liquidity, buyback, marketing and dev (NOTE: liquidity is 3.75%, buyback is 3.75%, marketing is 3.75% and dev is 2.75%)
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _buyBackFee = 375; // 3.75%
    uint256 public _marketingFee = 375; // 3.75%
    uint256 public _devFee = 275; // 2.75%

    address payable public buyBackAddress = 0xD217A0fD97654f846441619D35160F0FB7927Ee0; // BuyBack Address
    address payable public marketingAddress = 0x9d865216898f98a5D2c30fF918fd1efA36F3Be79; // Marketing Address
    address payable public devAddress = 0x43cEc03Df495c8FceE11368c10F448c7507d6c7b; // Dev Address
    ...
    // Calculates the token allocation for buyback, marketing and dev
    function calculateTokenAllocationForBuyBackMarketingAndDev(uint256 tokenAmount) private view returns (uint256) {
        uint256 buyBack_Marketing_Dev_Fees = _buyBackFee.add(_marketingFee).add(_devFee);
        return tokenAmount.div(_liquidityFee).mul(buyBack_Marketing_Dev_Fees);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        // Get the token allocation for buyback, marketing and dev
        uint256 allocationForBuyBackMarketingAndDev = calculateTokenAllocationForBuyBackMarketingAndDev(contractTokenBalance);
        ...
        uint256 half = contractTokenBalance.sub(allocationForBuyBackMarketingAndDev).div(2);
        uint256 otherHalf = contractTokenBalance.sub(allocationForBuyBackMarketingAndDev).sub(half);

        // Add half and allocation for marketing to be swapped into bnb
        uint256 tokensToBeSwappedIntoBNB = half.add(allocationForBuyBackMarketingAndDev);
        ...
        uint256 totalBNBForBuyBack_Marketing_Dev = swappedBNBForBuyBack.add(swappedBNBForMarketing).add(swappedBNBForDev);
        uint256 swappedBNBForLiquidity = swappedBNB.sub(totalBNBForBuyBack_Marketing_Dev);
        
        // Add liquidity
        addLiquidity(otherHalf, swappedBNBForLiquidity);

        // Transfet BNB to buyback wallet
