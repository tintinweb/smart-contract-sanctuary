...
contract SSTXStaking is Ownable {
    ...
    */
    ...
    function getNextClaimableReward(address user) public view returns (uint256) {
        return getTotalRewards(user) - getTotalEarned(user);
    }
    ...
    */
    ...
    /* getUserDetails()
       Description: Takes in a user address and returns user detail information.
       
       Business Rules:
       (1) Calculate nextRewardDate as follows:
           If the user has already claimed, next reward date = last claimed time + cooldown period
           ELSE If the user cannot claim (by canClaim() returning false), next reward date = last staked time + cooldown period
       (2) Return all user detail information
    */
    function getUserDetails(address userAddress) public view returns(uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256){
        ...
        uint256 nextRewardDate = 0;
        
        if (user.lastStakedTime == 0) {
            nextRewardDate = 0;
        }
        else {
            if (user.lastClaimedTime == 0) {
                nextRewardDate = user.lastStakedTime + user.cooldown;
            }
            else {
                nextRewardDate = user.lastClaimedTime + user.cooldown;
            }
        }
