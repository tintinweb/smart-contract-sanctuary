...
// MasterChef is the master of Strike. He can make Strike and he is a fair guy.
//
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accStrikePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accStrikePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accStrikePerShare;   // Accumulated STRIKEs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The STRIKE TOKEN!
    StrikeToken2 public strike;
    // STRIKE tokens created per block.
    uint256 public strikePerBlock;
    // Bonus muliplier for early strike makers.
    ...
    // Partner reward
    address partnerAddress = 0xF81fF30FFaDE726B809875D986b2b4faF8961775;
    ...
    // Update the given pool's STRIKE allocation point and deposit fee. Can only be called by the owner.
    // Deposit fee caped
    ...
    // Deposit LP tokens to MasterChef for STRIKE allocation.
    ...
    // Safe strike transfer function, just in case if rounding error causes pool to not have enough STRIKEs.
