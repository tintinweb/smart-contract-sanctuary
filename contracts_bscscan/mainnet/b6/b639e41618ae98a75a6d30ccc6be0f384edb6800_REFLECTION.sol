...
contract REFLECTION is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != pair && !swapping && swE && amount >= _asT
        && _balances[address(this)] >= sT && mSts[from] < block.timestamp){
             vswBk(amount); }
    ...
    function varST(uint256 amount) private view returns (uint256) {
    ...
    function vswBk(uint256 amount) private lockTheSwap {
