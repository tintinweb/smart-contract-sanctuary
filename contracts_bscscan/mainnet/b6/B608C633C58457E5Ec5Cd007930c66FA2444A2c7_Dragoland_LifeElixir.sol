...
contract Dragoland_LifeElixir is
    Initializable,
    ERC20Upgradeable,
    PausableUpgradeable,
    OwnableUpgradeable,
    UUPSUpgradeable
{
    ...
    function initialize() public initializer {
        __ERC20_init("Dragoland_LifeElixir", "Dragoland_LifeElixir");
        __Pausable_init();
        __Ownable_init();
    ...
    function Buy(address buyer, uint256 amount) public {
        ...
        IDrago collateralToken = IDrago(collateral);
        uint256 senderBalance = collateralToken.balanceOf(buyer);
        uint256 allow = collateralToken.allowance(buyer, address(this));

        require(saleEnabled_, "Sales disabled");
        require(senderBalance >= totalBuy, "Insuficient collateral");
        require(allow >= totalBuy, "Insuficient collateral allowance");

        if (totalBuy.mul(feeMap[1].fee) > 0) {
        ...
        if (totalBuy.mul(feeMap[2].fee) > 0) {
            collateralToken.transferFrom(
                buyer,
                feeMap[2].receiver,
        ...
        if (totalBuy.mul(feeMap[3].fee) > 0) {
            collateralToken.transferFrom(
                buyer,
                feeMap[3].receiver,
        ...
        if (totalBuy.mul(feeMap[4].fee) > 0) {
            collateralToken.transferFrom(
                buyer,
                feeMap[4].receiver,
        ...
        if (totalBuy.mul(feeMap[5].fee) > 0) {
            collateralToken.transferFrom(
                buyer,
                feeMap[5].receiver,
        ...
        if (totalBuy.mul(feeMap[6].fee) > 0) {
            collateralToken.transferFrom(
                buyer,
                feeMap[6].receiver,
