...
contract Ownable is Context {
  address private _owner = 0x2D6192e3C8966DdBF9D460829665dc6E7b544A5b;
...
contract Test is Context, IBEP20, Ownable { // contract name
  ...
  address payable public _marketingAddress = payable(0x2619577aAB51D94445FBB0ba8b044c47A7bf67cB); // Marketing Address
  address payable public _burnAddress = payable(0x000000000000000000000000000000000000dEaD); // Burn Address

  string private _name = "Testv2"; // token name
  string private _symbol = "Testv2"; // token symbol
  ...
  uint256 public _tokenFee = 1; // marketing fee 1% to be sent as tokens
  uint256 private _previousTokenFee = _tokenFee; // marketing tokens fee

  uint256 public _buyBackFee = 2; // buyback fee 2%
  uint256 private _previousBuyBackFee = _buyBackFee; // buyback fee

  uint256 public _marketingBNBFee = 3; // marketing BNB fee
  uint256 private _previousMarketinBNBFee = _marketingBNBFee; // marketing BNB fee

  uint256 public _liquidityFee = 4; // liquidity fee 4%
  ...
  bool public swapAndLiquifyEnabled = false; // set auto swap to BNB and liquify collected liquidity fee

  uint256 public _maxTxAmount = _tTotal.div(99); // max allowed tokens tranfer per transaction
  uint256 public _minTokensSwapToAndTransferTo = 10000000 * 10**_decimals; // min token liquidity fee collected before swapandLiquify
  ...
  */
  ...
  function calculateMarketingTokenFee(uint256 _amount) private view returns (uint256) {
    return _amount.mul(_tokenFee).div(100);
  }
  ...
  function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
    return _amount.mul(_deductableFee).div(100);
  }
