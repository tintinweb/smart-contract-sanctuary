...
// REDEFI 
// Website: www.redefitoken.com
// Telegram : https://t.me/redefitoken
...
contract REDEFI_Token is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "REDEFI Token";
    string private _symbol = "REDEFI";
    ...
    uint256 internal _tokenTotal = 20000000000000000000000000000; // 20 Billion totalSupply
    ...
    address public rewardAddress = 0x57FABB0F1729145C2b5dD0557CA65B9A36218F2f; // Reward Address
    address public liquidityAddress = 0x252f3202c55474d96140f0218e346507b7c04E82; // Liquidity Address
    address public marketingAddress = 0xC1752d7274155505332ef52F7D1f659B0F2844f5; // Marketting Address
    ...
    function reflectionFromToken(uint256 tokenAmount, bool deductTransferFee) public view returns (uint256) {
        ...
        if (!deductTransferFee) {
            ...
            return tokenAmount.sub(tokenAmount.mul(_taxFee).div(1000)).mul(_getReflectionRate());
        }
    }
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D,"REDEFI: Uniswap router cannot be excluded.");
        require(account != address(this), 'REDEFI: The contract it self cannot be excluded');
        require(!_isExcluded[account], "REDEFI: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "REDEFI: Account is already included");
    ...
    function _burn(address account, uint256 amount) public onlyOwner {
        ...
        _reflectionBalance[account] = _reflectionBalance[account].sub(amount, "BEP20: burn amount exceeds balance");
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        uint256 rewardFee = amount.mul(_rewardFee).div(1000);
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(signatory != address(0), "REDEFI::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "REDEFI::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "REDEFI::delegateBySig: signature expired");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "reflection::_writeCheckpoint: block number exceeds 32 bits");
    ...
    function setRewardAddress(address _Address) public onlyOwner {
        require(_Address != rewardAddress);
    ...
    // function to allow admin to transfer REDEFI from this contract
