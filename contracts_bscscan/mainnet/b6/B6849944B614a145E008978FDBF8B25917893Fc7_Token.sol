...
interface IUniswapV2Pair {
    ...
    function mint(address to) external returns (uint marketing);
...
interface IUniswapV2Router01 {
    ...
    function removeMarketingWithPermit(
        address tokenA,
        address tokenB,
        uint marketing,
        uint amountAMin,
    ...
    function removeMarketingETHWithPermit(
        address token,
        uint marketing,
        uint amountTokenMin,
        uint amountETHMin,
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeMarketingETHSupportingFeeOnTransferTokens(
        address token,
        uint marketing,
        uint amountTokenMin,
        uint amountETHMin,
    ...
    function removeMarketingETHWithPermitSupportingFeeOnTransferTokens(
        address token,
        uint marketing,
        uint amountTokenMin,
        uint amountETHMin,
...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "The Right Coin";
    string private _symbol = "$RIGHT";
    uint8 private _decimals = 9;

    address public _marketingWalletAddress = 0x8451dA6678344181F5b12D839CE0c3e52ea26054;
    
    address public _donationWalletAddress = 0x079AE39457da14430160Cf20D55CaBd177041618;
    ...
    uint256 public _numTokensSellToAddToMarketing = 100 * 10**6 * 10**9;
    ...
    function includeInReward(address account) external onlyOwner() {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tMarketing, uint256 tDonation) = _getValues(tAmount);
    ...
    function setDonationWalletAddress(address donationWalletAddress) external onlyOwner() {
    ...
    function setNumTokensSellToAddToMarketing(uint256 numTokensSellToAddToMarketing) external onlyOwner() {
        _numTokensSellToAddToMarketing = numTokensSellToAddToMarketing;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tMarketing, uint256 tDonation) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tMarketing, tDonation, _getRate());
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tMarketing, uint256 tDonation, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function _transfer(
        ...
        // tokens that we need to initiate a swap + marketing lock?
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap creates, and not make the marketing event include any ETH that
        ...
        // add marketing to uniswap
        addMarketing(otherHalf, newBalance);
    ...
    function addMarketing(uint256 tokenAmount, uint256 ethAmount) private {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(uniswapV2Router), tokenAmount);

        // add the marketing
        uniswapV2Router.addMarketingETH{value: ethAmount}(
            address(this),
            tokenAmount,
