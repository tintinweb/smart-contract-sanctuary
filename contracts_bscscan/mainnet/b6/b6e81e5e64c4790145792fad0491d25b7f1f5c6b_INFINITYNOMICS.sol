...
/**
First hyper-deflationary $AXS reward of a community based token based on the binance smart chain network

Website : https://infinitynomics.org
Twitter : https://www.twitter.com/infinitynNomics
Medium : https://medium.com/@infinitynomics
Telegram : https://t.me/infinitynomics
*/
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0;
...
contract INFINITYNOMICS is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public _dividendToken = 0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0;
    ...
    INFINITYNOMICSDividendTracker public dividendTracker;
    ...
    constructor() ERC20("InfinityNomics", "INFINITYNOMICS") {
    	...
    	dividendTracker = new INFINITYNOMICSDividendTracker();

    	buyBackWallet = 0xF511de0ABfA7Ecd6cBAD038FeF2A372369266165;
    	
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //0x3E473D5D4055736c9407A5d92D72AF761e58ef1A
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "INFINITYNOMICS: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "INFINITYNOMICS: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "INFINITYNOMICS: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "INFINITYNOMICS: The dividend tracker already has that address");

        INFINITYNOMICSDividendTracker newDividendTracker = INFINITYNOMICSDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "INFINITYNOMICS: The new dividend tracker must be owned by the INFINITYNOMICS token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "INFINITYNOMICS: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "INFINITYNOMICS: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "INFINITYNOMICS: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "INFINITYNOMICS: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "INFINITYNOMICS: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "INFINITYNOMICS: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "INFINITYNOMICS: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "INFINITYNOMICS: Cannot update gasForProcessing to same value");
...
contract INFINITYNOMICSDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("INFINITYNOMICS_Dividend_Tracker", "INFINITYNOMICS_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "INFINITYNOMICS_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "INFINITYNOMICS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main INFINITYNOMICS contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "INFINITYNOMICS_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "INFINITYNOMICS_Dividend_Tracker: Cannot update claimWait to same value");
