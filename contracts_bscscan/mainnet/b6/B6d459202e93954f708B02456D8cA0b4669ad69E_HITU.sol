...
contract HITU is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 110 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "HITU";
    string private _symbol = "HITU";
    ...
    constructor () public {
        ...
        emit Transfer(address(0x0F9399FC81DaC77908A2Dde54Bb87Ee2D17a3373), _msgSender(), _tTotal);
    }
    ...
    function _takeBurn(uint256 tBurn) private {
        uint256 currentRate =  _getRate();
        uint256 rBurn = tBurn.mul(currentRate);
        _rOwned[address(BURN_ADDRESS)] = _rOwned[address(BURN_ADDRESS)].add(rBurn);
        if(_isExcluded[address(BURN_ADDRESS)])
            _tOwned[address(BURN_ADDRESS)] = _tOwned[address(BURN_ADDRESS)].add(tBurn);
    }
