...
contract kingslimitedx is Context, IBEP20, Ownable {
    ...
    uint256 public _numTokensSellToAddToLiquidity = 0 * 10**6 * 10**9;
    uint256 public _maxWalletToken = 200 * 10**6 * 10**9;

    IPancakeRouter02 public immutable pcsV2Router;
    address public immutable pcsV2Pair;

    string private _name = "kingslimitedx";
    string private _symbol = "KINGS";
    ...
    function _getAntiDumpMultiplier() private view returns (uint256) {
        ...
        uint256 hour = 60 * 60;
        if (time_since_start < 504 * hour) {
            return (5);
        } else if (time_since_start < 1440 * hour) {
            return (4);
        } else if (time_since_start < 2880 * hour) {
            return (3);
        } else if (time_since_start < 4320 * hour) {
            return (2);
        } else {
            return (1);
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // If you copy this line of BATUHAN you're a shit dev who doesn't check over code
