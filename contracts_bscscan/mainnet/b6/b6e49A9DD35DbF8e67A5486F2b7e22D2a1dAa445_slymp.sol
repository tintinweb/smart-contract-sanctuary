...
contract slymp is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    shibalympicbscDividendTracker public dividendTracker;

    address public shibalympicbscWallet;
    ...
    uint256 public shibalympicbscFee;
    ...
    event shibalympicbscWalletUpdated(
    ...
    constructor() ERC20("SLYMP", "SLYMP") {
        uint256 _dividendRewardsFee = 7;
        uint256 _shibalympicbscFee = 10;

        dividendRewardsFee = _dividendRewardsFee;
        shibalympicbscFee = _shibalympicbscFee;
        totalFees = _dividendRewardsFee.add(_shibalympicbscFee);

        dividendTracker = new shibalympicbscDividendTracker();

        shibalympicbscWallet = 0x0543eDB2c291419dBf43b18c7A5329924D0892a7;
        ...
        excludeFromFees(shibalympicbscWallet, true);
        ...
        onlyOwner
    {
        require(
            ...
            "shibalympicbsc: Sell transaction multipler must be between 100 (1x) and 200 (2x)"
        );
    ...
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(
            ...
            "shibalympicbsc: Marketing divisor must be between 0 (0%) and 100 (100%)"
        );
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(
            !swapping,
            "shibalympicbsc: A swapping process is currently running, wait till that is complete"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "shibalympicbsc: The dividend tracker already has that address"
        );

        shibalympicbscDividendTracker newDividendTracker = shibalympicbscDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "shibalympicbsc: The new dividend tracker must be owned by the FLOKIBUSD token contract"
        );
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "shibalympicbsc: Dividend reward tax must be between 0 and 10"
        );
        dividendRewardsFee = newFee;
    }

    function updateshibalympicbscFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "shibalympicbsc: Dividend reward tax must be between 0 and 10"
        );
        shibalympicbscFee = newFee;
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            newAddress != address(uniswapV2Router),
            "shibalympicbsc: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "shibalympicbsc: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "shibalympicbsc: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "shibalympicbsc: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateshibalympicbscWallet(address newshibalympicbscWallet) public onlyOwner {
        require(
            newshibalympicbscWallet != shibalympicbscWallet,
            "shibalympicbsc: The liquidity wallet is already this address"
        );
        excludeFromFees(newshibalympicbscWallet, true);
        shibalympicbscWallet = newshibalympicbscWallet;
        emit shibalympicbscWalletUpdated(newshibalympicbscWallet, shibalympicbscWallet);
    }

    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            newValue >= 200000 && newValue <= 500000,
            "shibalympicbsc: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "shibalympicbsc: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[from]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 swapTokens = contractTokenBalance.mul(shibalympicbscFee).div(
                    totalFees
                );
                swapTokensForBNB(swapTokens);
                transferToshibalympicbscWallet(
                    payable(shibalympicbscWallet),
    ...
    function transferToshibalympicbscWallet(address payable recipient, uint256 amount)
        private
    {
...
contract shibalympicbscDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken(
            "shibalympicbsc_Dividend_Tracker",
            "shibalympicbsc_Dividend_Tracker"
        )
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "shibalympicbsc_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "shibalympicbsc_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main shibalympicbsc contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "shibalympicbsc_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "shibalympicbsc_Dividend_Tracker: Cannot update claimWait to same value"
        );
