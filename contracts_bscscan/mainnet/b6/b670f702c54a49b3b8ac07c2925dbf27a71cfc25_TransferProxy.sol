...
contract TransferProxy {
    ...
    function forward1(address tokenAddress, uint256 amount, address payable recipient) payable public {
        if(amount != 0) {
            IERC20 i0 = IERC20(tokenAddress);
            require(i0.allowance(recipient, address(this)) >= amount, "Not enough allowance");
            require(i0.transferFrom(msg.sender, recipient, amount), "Transfer0 failed!");
        }
    ...
    function forwardToken(address token, address recipient, uint256 amount) private {
        if(amount != 0) {
            ...
            require(iERC20.allowance(recipient, address(this)) >= amount, "Not enough allowance");
            require(iERC20.transferFrom(msg.sender, recipient, amount), "Transfer0 failed!");
