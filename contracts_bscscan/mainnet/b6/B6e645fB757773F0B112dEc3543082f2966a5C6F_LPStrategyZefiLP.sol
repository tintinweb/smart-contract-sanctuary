...
abstract contract FeeManager is StratManager {
    ...
    uint constant public TREASURY_FEE   = 1000; // OTHER
    ...
    uint public withdrawalFee = 10;  // 0,1% OTHER
    uint public porcFee = 30; // 3%    

    uint public callFee = 0;
    uint public beefyFee = MAX_FEE - TREASURY_FEE - STRATEGIST_FEE - callFee;

    function setCallFee(uint256 _fee) external onlyManager {
        require(_fee <= MAX_CALL_FEE, "!cap");
        
        callFee = _fee;
        beefyFee = MAX_FEE - TREASURY_FEE - STRATEGIST_FEE - callFee;
    }
...
contract LPStrategyZefiLP is StratManager, FeeManager, GasThrottler {
    ...
    address constant public bifi = address(0x0288D3E353fE2299F11eA2c2e1696b4A648eCC07); // buyback ZEFI
    address public want;
    address public lpToken0;
    address public lpToken1;

    // Third party contracts
    address constant public output = address(0xE5b57E6e1b945B91FEE368aC108d2ebCcA78Aa8F); // TOKEN DA FARM    
    address constant public masterchef = address(0x23A88eDd92559d18A36B3B1B3DD957AF6459465e); // MASTERCHEF
    ...
    function chargeFeesZeFi() internal {
        
        uint256 toWbnb = IERC20(output).balanceOf(address(this)).mul(porcFee).div(1000);
        // IUniswapRouter(unirouter).swapExactTokensForTokens(toWbnb, 0, wantToWbnbRoute, address(this), now.add(600));
        // uint256 wbnbBal = IERC20(wbnb).balanceOf(address(this));
        
        // FEES
        // uint256 callFeeAmount = toWbnb.mul(callFee).div(MAX_FEE); //
        // IERC20(want).safeTransfer(msg.sender, callFeeAmount);
        
        uint256 treasuryHalf = toWbnb.mul(TREASURY_FEE).div(MAX_FEE);
        IERC20(output).safeTransfer(dead, treasuryHalf);  //        
    }
    ...
    function chargeFees() internal {
        ...
        uint256 toWbnb = IERC20(output).balanceOf(address(this)).mul(porcFee).div(1000);
        IUniswapRouter(unirouter).swapExactTokensForTokens(toWbnb, 0, outputToWbnbRoute, address(this), now);

        uint256 wbnbBal = IERC20(wbnb).balanceOf(address(this));

        uint256 treasuryHalf = wbnbBal.mul(TREASURY_FEE).div(MAX_FEE);
        IUniswapRouter(unirouter).swapExactTokensForTokens(treasuryHalf, 0, wbnbToBifiRoute, dead, now);  // 11,2        
    ...
    // performance fees
    function chargeFeesDefender() internal {
        uint256 toWbnb = IERC20(output).balanceOf(address(this)).mul(porcFee).div(1000);
        IUniswapRouter(unirouter).swapExactTokensForTokens(toWbnb, 0, outputToWbnbRoute, address(this), now);
