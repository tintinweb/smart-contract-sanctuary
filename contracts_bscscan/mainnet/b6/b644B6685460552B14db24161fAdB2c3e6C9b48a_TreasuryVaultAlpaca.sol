...
contract TreasuryVaultAlpaca is Ownable, Initializable, ReentrancyGuard {
    ...
    address public vaultProfitFund;
    ...
    function initialize(
        ...
        setVaultProfitFund(_vaultProfitFund);
    ...
    function withdraw() external onlyTreasury {
        ...
        //step 3. transfer BUSD to treasury, vaultProfitFund
        _asset.safeTransfer(treasury, vaultBalance);
        _asset.safeTransfer(vaultProfitFund, profit);
        vaultBalance = _asset.balanceOf(address(this));
    ...
    function claimIncetiveReward() external onlyOwner {
        uint256 rewardBalance = getIncentiveRewardBalance();
        IERC20(reward).safeTransfer(vaultProfitFund, rewardBalance);

        emit IncentiveClaimed(rewardBalance);
    }
    ...
    function setVaultProfitFund(address _vaultProfitFund) public onlyOwner {
        require(_vaultProfitFund != address(0), "Invalid address");
        vaultProfitFund = _vaultProfitFund;
        emit VaultProfitFundUpdated(_vaultProfitFund);
    }
    ...
    event VaultProfitFundUpdated(address indexed newVaultProfitFund);
