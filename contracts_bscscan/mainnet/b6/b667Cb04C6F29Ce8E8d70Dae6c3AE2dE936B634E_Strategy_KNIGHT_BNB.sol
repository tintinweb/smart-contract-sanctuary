...
contract Strategy_KNIGHT_BNB is Ownable {
    // This strategy is for BSC network. KNIGHT - BNB pair. RewardToken = "Knight"
    ...
    address public token0; // wrapped token
    address public token1; // Knight
    address public vault;

    // ------------> Important constants <--------------
    uint256 public constant pid = 1;
    bool public constant hasPid = true;
    // uses for LP swapping
    address public routerLP = 0x05E61E0cDcD2170a76F9568a110CEe3AFdD6c46f;
    // uses for swapping WBNB to USDT
    ...
    address public constant lpToken = 0xd71710c21FBa293eB12E0CA079Fe37f50AD17d42;
    address public constant farm = 0xE50cb76A71b0c52Ab091860cD61b9BA2FA407414;
    // ------------>        END          <--------------
    ...
    constructor(address _vault) {
        ...
        price0CumulativeLast = IPair(lpToken).price0CumulativeLast(); // fetch the current accumulated price value (0 / 1)
    ...
    function autoCompound() external {
        ...
        if (getAmountLPFromFarm() > 0) {
            ...
            if(_balance > 0){
                path[0] = rewardToken;
                path[1] = WBNB; // wrapped token 
                uint256 amount = _getAmountsOut(_balance+pendingRewardsFee, path);
                if(amount > 100) {
    ...
    function depositAsMigrate() public onlyVault {
        ...
        _approveToken(token0, routerLP);
        _approveToken(token1, routerLP);
        _approveToken(token0, routerMain);
    ...
    function _getAmountsOut(uint256 _amount, address[] memory path) internal view returns (uint256){
        uint256[] memory amounts = IRouter(routerLP).getAmountsOut(_amount, path);
        return amounts[1];
    }
    ...
    function _takeFee() internal {
        ...
        if(rewardsFeeInWBNB > 0) {
            uint256[] memory amounts = IRouter(routerMain).getAmountsOut(rewardsFeeInWBNB + pendingFee, path);
            if(amounts[1] > 100) {
                _swapExactTokensForTokens(routerMain, rewardsFeeInWBNB + pendingFee, amounts[1], path);
                pendingFee = 0;
                emit TakeFee(rewardsFeeInWBNB, amounts[1]);
            } else {
    ...
    function _swapExactETHForTokens(address _token, uint256 _amountETH) internal {
        ...
        address[] memory path = new address[](2);
        path[0] = WBNB;
        // swap a half of msg.value to token
        if(_token != WBNB) {
            path[1] = _token;
            amounts = IRouter(routerLP).getAmountsOut(_amountETH/2, path);
            uint256 desiredAmountToken = amounts[1];
            if(desiredAmountToken > 100) {
                IRouter(routerLP).swapExactETHForTokens{value:_amountETH/2}(
    ...
    function __addLiquidityETH(address _token, uint256 desiredAmountETH) internal {
        ...
        if(amount0 != 0 && amount1 != 0 && _canAddLiquidity(amount0, amount1)) {
            _transfer(_token, lpToken, amount1);
            IWBNB(WBNB).deposit{value: amount0}();
            IWBNB(WBNB).transfer(lpToken, amount0);
            IPair(lpToken).mint(address(this));
        } else {
            revert("Not enough funds to add liquidity");
        }
    ...
    function _swapTokenToWBNB(address _token) internal {
        ...
        if(_token != WBNB) {
            ...
            if(_balance > 0) {
                amounts = IRouter(routerLP).getAmountsOut(_balance, path);
                if(amounts[1] > 100) {
                    _swapExactTokensForTokens(routerLP, _balance, amounts[1], path);
                }
            }
        }
    }
    ...
    function _getTokenPrice() internal view returns (uint256) {
        ...
        path[0] = token1;
        path[1] = token0;
        return 1e36 / _getAmountsOut(1e18, path);
    }
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        ...
        if(_amount > 0) {
            ...
            // calculates how many wrapped tokens for token1
            tCI = _getTokenPrice1() * t1V / d + t0V;
        }

        // calculates total Capital from tokens that exist in the contract
        uint256 _balance = getBalanceOfToken(token1);
        if (_balance > 0) {
            tCI += _getTokenPrice1() * _balance / d;
        }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            // calculates how many wrappedToken for tokens
            if(t1V != 0) {
                path[0] = token1;
                TC = _getAmountsOut(t1V, path) + t0V;
            }
        }
