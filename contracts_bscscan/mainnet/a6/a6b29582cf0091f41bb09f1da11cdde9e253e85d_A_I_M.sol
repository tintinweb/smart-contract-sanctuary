 ...
 *
 * Web: https://A_I_M.com/
 * Telegram: https://t.me/A_I_M
 * Twitter: https://twitter.com/A_I_M
 *
 */
...
///  to toksen holders as dividends and allows token holders to withdraw their dividends.
    ...
    Ownable
{
    ...
    mapping(address => uint256) internal BUSDWithdrawnDividends;
        ...
        onlyOwner
    {
        require(
            ...
            "AIM: The router already has that address"
        );
    ...
    // Customized function to send BUSD to dividend recipients
    function swapETHForBUSD(address recipient, uint256 ethAmount)
        private
        returns (bool)
    {
        ...
        returns (uint256)
    {
        ...
        if (!swapSuccess) {
            BUSDWithdrawnDividends[recipient] = BUSDWithdrawnDividends[
                recipient
            ].add(ethAmount);
    ...
    function getBUSDDividends(address holder) external view returns (uint256) {
        return BUSDWithdrawnDividends[holder];
    }
    ...
    // call this to go back to receiving BUSD after setting another token. (call from token contract only)
    function unsetRewardToken(address holder) external onlyOwner {
        userHasCustomRewardToken[holder] = false;
        if (rewardTokenSelectionCount[userCurrentRewardToken[holder]] > 0) {
            rewardTokenSelectionCount[userCurrentRewardToken[holder]] -= 1; // remove count from old token
        }
        userCurrentRewardToken[holder] = address(BUSD);
        userCurrentRewardAMM[holder] = address(uniswapV2Router);
        userHasCustomRewardAMM[holder] = false;
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            if (
                ...
                BUSDWithdrawnDividends[user] = BUSDWithdrawnDividends[user].add(
                    _withdrawableDividend
                );
                emit DividendWithdrawn(user, _withdrawableDividend);
                bool success = swapETHForBUSD(user, _withdrawableDividend);

                if (!success) {
                    withdrawnDividends[user] = withdrawnDividends[user].sub(
                        _withdrawableDividend
                    );
                    BUSDWithdrawnDividends[user] = BUSDWithdrawnDividends[user]
                        .sub(_withdrawableDividend);
                    return 0;
                }
                return _withdrawableDividend;

                // the reward is a token, not BUSD, use an IERC20 buyback instead!
            } else {
                withdrawnDividends[user] = withdrawnDividends[user].add(
                    _withdrawableDividend
                );
...
contract DividendTracker is DividendPayingToken {
    ...
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100000000 * (10**__decimal); //must hold 100,000,000+ tokens to get divs
    }
    ...
    function _transfer(
        ...
        require(false, "___Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "___Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "___Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "___Dividend_Tracker: Cannot update claimWait to same value"
        );
...
contract A_I_M is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public valkyrieFeeOnBuy = 1;
    uint256 public marketingFeeOnBuy = 5;

    uint256 public liquidityFeeOnSell = 4;
    uint256 public rewardsFeeOnSell = 10;
    uint256 public valkyrieFeeOnSell = 1;
    uint256 public marketingFeeOnSell = 5;

    uint256 public totalFeesOnBuy =
        rewardsFeeOnBuy.add(liquidityFeeOnBuy).add(valkyrieFeeOnBuy).add(
            marketingFeeOnBuy
        );
    uint256 public totalFeesOnSell =
        rewardsFeeOnSell.add(liquidityFeeOnSell).add(valkyrieFeeOnSell).add(
            marketingFeeOnSell
        );
    ...
    event FeeOnSellChange(
        uint256 indexed newValue,
        uint256 indexed oldValue,
        string indexed taxType
    );
    event FeeOnBuyChange(
        uint256 indexed newValue,
        uint256 indexed oldValue,
        string indexed taxType
    );
    ...
    event FeesUpdated(
        uint256 indexed newRewardsFee,
        uint256 indexed newLiquidityFee,
        uint256 newMarketingFee,
        uint256 newValkyrieFee
    );
    ...
    event SendMarketing(uint256 tokensSwapped, uint256 amount);
    event SendValkyrie(uint256 tokensSwapped, uint256 amount);
    ...
    constructor() ERC20("AIM", "AIM", 18) {
        dividendTracker = new DividendTracker(18);

        liquidityWallet = owner();
        marketingWallet = owner();
        valkyrieWallet = owner();
        ...
        // dividendTracker.excludeFromDividends(valkyrieWallet);
        dividendTracker.excludeFromDividends(
            address(0x000000000000000000000000000000000000dEaD)
        ); // don't want dead address to take BUSD!!!
        dividendTracker.excludeFromDividends(address(_uniswapV2Router));

        // exclude from paying fees or having max transaction amount
        excludeFromFees(liquidityWallet, true);
        excludeFromFees(marketingWallet, true);
        excludeFromFees(valkyrieWallet, true);
        excludeFromFees(address(this), true);
        excludeFromFees(address(dividendTracker), true);

        unlimitAddress(_uniswapV2Pair, true);
        unlimitAddress(address(_uniswapV2Router), true);
        ...
        onlyOwner
    {
        require(
            ...
            "A_I_M: setWhiteListAMM:: AMM is a wallet, not a contract"
        );
    ...
    function unlimitAddress(address account, bool value) public onlyOwner {
        _isNotLimited[account] = value;
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "A_I_M: The dividend tracker already has that address"
        );
        ...
        require(
            ...
            "A_I_M: The new dividend tracker must be owned by the A_I_M token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(
            newAddress != address(uniswapV2Router),
            "A_I_M: The router already has that address"
        );
    ...
    function setLiquidityFeeOnBuy(uint256 newvalue) public onlyOwner {
        require(
            ...
            "The liquidityFeeOnBuy is already that value"
        );
        emit FeeOnBuyChange(newvalue, liquidityFeeOnBuy, "liquidityFeeOnBuy");
        totalFeesOnBuy = totalFeesOnBuy.sub(liquidityFeeOnBuy).add(newvalue);
        liquidityFeeOnBuy = newvalue;
    }

    function setRewardsFeeOnBuy(uint256 newvalue) public onlyOwner {
        require(
            rewardsFeeOnBuy != newvalue,
            "The rewardsFeeOnBuy is already that value"
        );
        emit FeeOnBuyChange(newvalue, rewardsFeeOnBuy, "rewardsFeeOnBuy");
        totalFeesOnBuy = totalFeesOnBuy.sub(rewardsFeeOnBuy).add(newvalue);
        rewardsFeeOnBuy = newvalue;
    }
    ...
    function setMarketingFeeOnBuy(uint256 newvalue) public onlyOwner {
        require(
            ...
            "The marketingFeeOnBuy is already that value"
        );
        emit FeeOnBuyChange(newvalue, marketingFeeOnBuy, "marketingFeeOnBuy");
        totalFeesOnBuy = totalFeesOnBuy.sub(marketingFeeOnBuy).add(newvalue);
        marketingFeeOnBuy = newvalue;
    }

    function setValkyrieFeeOnBuy(uint256 newvalue) public onlyOwner {
        require(
            valkyrieFeeOnBuy != newvalue,
            "The valkyrieFeeOnBuy is already that value"
        );
        emit FeeOnBuyChange(newvalue, valkyrieFeeOnBuy, "valkyrieFeeOnBuy");
        totalFeesOnBuy = totalFeesOnBuy.sub(valkyrieFeeOnBuy).add(newvalue);
        valkyrieFeeOnBuy = newvalue;
    }
    ...
    function setLiquidityFeeOnSell(uint256 newvalue) public onlyOwner {
        require(
            ...
            "The liquidityFeeOnSell is already that value"
        );
        emit FeeOnSellChange(
            newvalue,
            liquidityFeeOnSell,
            "liquidityFeeOnSell"
        );
        totalFeesOnSell = totalFeesOnSell.sub(liquidityFeeOnSell).add(newvalue);
        liquidityFeeOnSell = newvalue;
    }

    function setRewardsFeeOnSell(uint256 newvalue) public onlyOwner {
        require(
            rewardsFeeOnSell != newvalue,
            "The rewardsFeeOnSell is already that value"
        );
        emit FeeOnSellChange(newvalue, rewardsFeeOnSell, "rewardsFeeOnSell");
        totalFeesOnSell = totalFeesOnSell.sub(rewardsFeeOnSell).add(newvalue);
        rewardsFeeOnSell = newvalue;
    }
    ...
    function setMarketingFeeOnSell(uint256 newvalue) public onlyOwner {
        require(
            ...
            "The marketingFeeOnSell is already that value"
        );
        emit FeeOnSellChange(
            newvalue,
            marketingFeeOnSell,
        ...
        totalFeesOnSell = totalFeesOnSell.sub(marketingFeeOnSell).add(newvalue);
        marketingFeeOnSell = newvalue;
    }

    function setValkyrieFeeOnSell(uint256 newvalue) public onlyOwner {
        require(
            valkyrieFeeOnSell != newvalue,
            "The valkyrieFeeOnSell is already that value"
        );
        emit FeeOnSellChange(newvalue, valkyrieFeeOnSell, "valkyrieFeeOnSell");
        totalFeesOnSell = totalFeesOnSell.sub(valkyrieFeeOnSell).add(newvalue);
        valkyrieFeeOnSell = newvalue;
    }
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "A_I_M: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _getCurrentTotalFee(bool isBuyFromLp, uint256 amount)
        internal
        view
        returns (uint256 returnAmount, uint256 fee)
    {
        fee = isBuyFromLp
            ? amount.mul(totalFeesOnBuy).div(100)
            : amount.mul(totalFeesOnSell).div(100);

        returnAmount = amount.sub(fee);
    }

    function updateFeesOnBuy(
        uint256 rewardPerc,
        uint256 liquidityPerc,
        uint256 marketingPerc,
        uint256 valkyriePerc
    ) external onlyOwner {
        rewardsFeeOnBuy = rewardPerc;
        liquidityFeeOnBuy = liquidityPerc;
        marketingFeeOnBuy = marketingPerc;
        valkyrieFeeOnBuy = valkyriePerc;
        ...
        totalFeesOnBuy = rewardsFeeOnBuy
            ...
            .add(valkyrieFeeOnBuy);
    }

    function updateFeesOnSell(
        uint256 rewardPerc,
        uint256 liquidityPerc,
        uint256 marketingPerc,
        uint256 valkyriePerc
    ) external onlyOwner {
        rewardsFeeOnSell = rewardPerc;
        liquidityFeeOnSell = liquidityPerc;
        marketingFeeOnSell = marketingPerc;
        valkyrieFeeOnSell = valkyriePerc;
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            ...
            "AIM: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "AIM: Cannot update gasForProcessing to same value"
        );
    ...
    function getBUSDDividends(address holder) public view returns (uint256) {
        return dividendTracker.getBUSDDividends(holder);
    }
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        require(
            ...
            "AIM: setRewardToken:: Address is a wallet, not a contract."
        );
        require(
            rewardTokenAddress != address(this),
            "AIM: setRewardToken:: Cannot set reward token as this token due to Router limitations."
        );
        require(
            !isIgnoredToken(rewardTokenAddress),
            "AIM: setRewardToken:: Reward Token is ignored from being used as rewards."
        );
    ...
    function setRewardTokenWithCustomAMM(
        ...
        require(
            ammContractAddress != address(uniswapV2Router),
            "AIM: setRewardToken:: Use setRewardToken to use default Router"
        );
        require(
            rewardTokenAddress != address(this),
            "AIM: setRewardToken:: Cannot set reward token as this token due to Router limitations."
        ...
        require(
            ...
            "AIM: setRewardToken:: AMM is not whitelisted!"
        );
        dividendTracker.setRewardToken(
            msg.sender,
            rewardTokenAddress,
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "AIM: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function _transfer(
        ...
        require(
            ...
            "AIM: To/from address is ignored"
        );
        ...
        (uint256 returnAmount, uint256 fees) = _getCurrentTotalFee(
            isBuyFromLp,
            amount
        );
        ...
        if (!_isNotLimited[to]) {
            require(
                balanceOf(to).add(returnAmount) <= _holderLimit,
                "Exceeds bag size"
            );
        }
        ...
        if (
            ...
            swapping = true;
            uint256 _liquidity = liquidityFeeOnBuy.add(liquidityFeeOnSell);
            uint256 _marketing = marketingFeeOnBuy.add(marketingFeeOnSell);
            uint256 _valkyrie = valkyrieFeeOnBuy.add(valkyrieFeeOnSell);
            uint256 _totalFee = totalFeesOnBuy.add(totalFeesOnSell);

            uint256 swapTokens = contractTokenBalance.mul(_liquidity).div(
                _totalFee
            );
            swapAndLiquify(swapTokens);

            uint256 marketingTokens = contractTokenBalance.mul(_marketing).div(
                _totalFee
            );

            uint256 valkyrieTokens = contractTokenBalance.mul(_valkyrie).div(
                _totalFee
            );

            uint256 _totalTokens = marketingTokens.add(valkyrieTokens);

            // Wrap all BUSD distro in a single transaction
            swapAndDistro(
                _marketing,
                _valkyrie,
                _totalTokens,
                _marketing.add(_valkyrie)
            );
    ...
    function swapAndDistro(
        uint256 _marketingFee,
        uint256 _valkyrieFee,
        uint256 _tokens,
        uint256 _totalFees
    ) private {
        uint256 busdInit = BUSD.balanceOf(address(this));
        swapTokensForBUSD(_tokens);
        uint256 newBalance = address(this).balance.sub(busdInit);

        sendMarketing(newBalance.mul(_marketingFee).div(_totalFees));
        sendValkyrie(newBalance.mul(_valkyrieFee).div(_totalFees));
    }

    function sendMarketing(uint256 tokens) private {
        BUSD.transfer(marketingWallet, tokens);
        emit SendMarketing(tokens, tokens);
    }

    function sendValkyrie(uint256 tokens) private {
        BUSD.transfer(valkyrieWallet, tokens);
        emit SendValkyrie(tokens, tokens);
    }

    // Get a % of stuck BNB in the smart contract
    function recoverContractBNB(uint256 recoverRate) public onlyOwner {
        uint256 bnbAmount = address(this).balance;
        if (bnbAmount > 0) {
            payable(owner()).transfer(bnbAmount.mul(recoverRate).div(100));
        }
    }

    // Get a % of token stuck in the smart contract
    function recoverContractToken(address _token, uint256 recoverRate)
        public
        onlyOwner
    {
        require(_token != address(this), "Can't take Valhalla Token");
        IERC20 token_ = IERC20(_token);
        uint256 tokenAmount = token_.balanceOf(address(this));
        if (tokenAmount > 0) {
            token_.transfer(owner(), tokenAmount.mul(recoverRate).div(100));
        }
    }

    function holderLimit() public view returns (uint256) {
        return _holderLimit;
    }

    function setHolderLimit(uint256 limit) public onlyOwner {
        _holderLimit = limit;
    }
}