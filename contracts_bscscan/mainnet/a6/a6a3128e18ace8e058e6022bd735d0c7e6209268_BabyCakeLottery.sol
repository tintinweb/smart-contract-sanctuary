...
// File: contracts/BabyCakeLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title BabyCake Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract BabyCakeLottery is ReentrancyGuard, IBabyCakeLottery, Ownable {
    ...
    uint256 public constant MAX_BUYBACK_FEE = 8000; // 80%
    ...
    struct Lottery {
        ...
        uint256 buyBackFee; // 500: 5% // 200: 2% // 50: 0.5%
    ...
    event NewOperatorAndBuyBackAndTreasuryAndInjectorAddresses(
        address operator,
        address buyBack,
        address treasury,
        address injector
        ...
        nonReentrant
    {
        ...
        // Calculate the amount to share post-(treasury fee + buyback fee)
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInCake) *
                (10000 - _lotteries[_lotteryId].treasuryFee - _lotteries[_lotteryId].buyBackFee))
        ) / 10000;

        uint256 totalRewardPercentage = 0;
        for (uint32 k = 0; k < 6; k++) {
            totalRewardPercentage += _lotteries[_lotteryId].rewardsBreakdown[k];
        }

        // Initializes the amount to withdraw to treasury and buyback wallet
        uint256 amountToWithdrawToTreasuryAndBuyBack;
        ...
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                // A. No CAKE to distribute, they are added to the amount to withdraw to treasury and buyback address
            } else {
                _lotteries[_lotteryId].cakePerBracket[j] = 0;

                amountToWithdrawToTreasuryAndBuyBack +=
                    (_lotteries[_lotteryId].rewardsBreakdown[j] * amountToShareToWinners) /
                    totalRewardPercentage;
            }
        }
        ...
        if (_autoInjection) {
            pendingInjectionNextLottery = amountToWithdrawToTreasuryAndBuyBack;
            amountToWithdrawToTreasuryAndBuyBack = 0;
        }

        amountToWithdrawToTreasuryAndBuyBack += (_lotteries[_lotteryId].amountCollectedInCake - amountToShareToWinners);

        // Send tokens to the treasury and buyback address
        if (
            amountToWithdrawToTreasuryAndBuyBack > 0 &&
            _lotteries[_lotteryId].treasuryFee + _lotteries[_lotteryId].buyBackFee > 0
        ) {
            uint256 amountToWithdrawToTreasury = (amountToWithdrawToTreasuryAndBuyBack *
                _lotteries[_lotteryId].treasuryFee) /
                (_lotteries[_lotteryId].treasuryFee + _lotteries[_lotteryId].buyBackFee);
            uint256 amountToWithdrawToBuyBack = amountToWithdrawToTreasuryAndBuyBack - amountToWithdrawToTreasury;
            ...
            // Transfer CAKE to buyback address
            if (amountToWithdrawToBuyBack > 0) {
                cakeToken.safeTransfer(buyBackAddress, amountToWithdrawToBuyBack);
            }
        }
    ...
    function startLottery(
        ...
        require(_buyBackFee <= MAX_BUYBACK_FEE, 'Buyback fee too high');
     ...
     * @param _buyBackAddress: address of the buyback
     * @param _treasuryAddress: address of the treasury
     * @param _injectorAddress: address of the injector
     */
    function setOperatorAndBuyBackAndTreasuryAndInjectorAddresses(
        ...
        require(_buyBackAddress != address(0), 'Cannot be zero address');
        ...
        emit NewOperatorAndBuyBackAndTreasuryAndInjectorAddresses(
            _operatorAddress,
            _buyBackAddress,
            _treasuryAddress,
            _injectorAddress
