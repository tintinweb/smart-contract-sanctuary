...
/**
  *Join us on Telegram at DogeDeflate (https://t.me/DogeDeflate)
 *
* 2% Redistribution
* 6% Auto LP locked
* 6% Burn Every Transaction
*/
...
contract DogeDeflate is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Doge Deflate";
...
function _approve(address owner, address spender, uint256 amount) private {
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(signatory != address(0), "$DD::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "$DD::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "$DD::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "$DD::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "$DD::_writeCheckpoint: block number exceeds 32 bits");
