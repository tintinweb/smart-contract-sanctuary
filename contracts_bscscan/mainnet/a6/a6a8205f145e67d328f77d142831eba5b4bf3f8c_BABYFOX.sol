...
contract BABYFOX is ERC20, Ownable {
    ...
    uint256 private totalFees = CAKERewardsFee.add(liquidityFee).add(marketingFee).add(recoveryFee);
    ...
    address private _marketingWalletAddress = address(0x66b9B5A4440a74d380B70Cb86a02f0E719101331); // marketing wallet
    address private _devWalletAddress = address(0x66b9B5A4440a74d380B70Cb86a02f0E719101331); // fee for contract development
    ...
    uint256 private maxBuy = 1_000_000_000_000 * (1e18);
    uint256 private maxSell = 1_000_000_000_000 * (1e18);
    uint256 private maxWallet = 1_000_000_000_000 * (1e18);
    // must be here to receive BNB
    receive() external payable {
  	}

    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BABYFOX: The dividend tracker already has that address");

        BABYFOXDividendTracker newDividendTracker = BABYFOXDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BABYFOX: The new dividend tracker must be owned by the BABYFOX token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BABYFOX: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BABYFOX: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BABYFOX: Automated market maker pair is already set to that value");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "BABYFOX: airdropToWallets:: Arrays must be the same length");
        require(airdropWallets.length < 40, "BABYFOX: airdropToWallets:: Cannot airdrop more than 40 wallets due to gas limit restrictions");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BABYFOX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BABYFOX: Cannot update gasForProcessing to same value");
...
contract BABYFOXDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BABYFOX_Dividend_Tracker", "BABYFOX_Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BABYFOX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BABYFOX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BABYFOX contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "BABYFOX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BABYFOX_Dividend_Tracker: Cannot update claimWait to same value");
