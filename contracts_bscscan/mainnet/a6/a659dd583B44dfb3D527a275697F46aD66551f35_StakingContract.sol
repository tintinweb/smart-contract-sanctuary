...
contract StakingContract is IReflectionPool, Ownable {
    ...
    SafeEarn safeEarn = SafeEarn(payable(0xb227e7fff9883F99E51613bc121c36361A697Ae9));
    ...
    function enterStaking(uint256 amount) external {
        if (amount == 0)
            amount = safeEarn.balanceOf(msg.sender);
...
//            ReflectionLocker locker = !lockerExists ? new ReflectionLocker(msg.sender) : lockers[msg.sender];
        }
    ...
    //TODO ERROR: Transfer amount must be greater than zero: https://bscscan.com/tx/0x040c0d1f701b4009463fca01b83f17adba0cdc88eeab10543e0b3973b9ae7868
    ...
    function _removeShares() internal {
        tokenPool.totalShares = tokenPool.totalShares.sub(shares[msg.sender].amount);
...
//    IBEP20 safeEarn = IBEP20(0x099f551eA3cb85707cAc6ac507cBc36C96eC64Ff);
    ...
    function claimAll() public override onlyLockOwner returns (uint, uint) {
        ...
        safeEarn.transfer(tx.origin, amtEarn);
        if (amtMoon > 0) {
            safeMoon.transfer(tx.origin, amtMoon);
        }
