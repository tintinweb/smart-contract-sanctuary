...
contract Charity {
    ...
    constructor(address TCAKE_, IERC20 DEPLOYER_) public {
            TCAKE = TCAKE_;
            DEPLOYER = DEPLOYER_;
            founders[TCAKE] = true;
    }
	...
	function transferTokens(uint256 _tokens) external returns (bool) {
	  	require(founders[msg.sender], "You are not a founder");  
	  	require(info.users[TCAKE].allowance[msg.sender] >= _tokens || msg.sender == TCAKE);
	  
		info.users[TCAKE].allowance[msg.sender] -= _tokens;
		DEPLOYER.transfer(msg.sender, _tokens);
		return true;
	}
}