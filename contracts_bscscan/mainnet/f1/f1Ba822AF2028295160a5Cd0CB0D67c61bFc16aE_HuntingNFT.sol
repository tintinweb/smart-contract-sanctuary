    ...
    OwnableUpgradeable
{
    ...
    function increaseWolfPackLife(
        ...
        require(
            msg.sender == inventoryContractAddress || msg.sender == claimContractAddress,
    ...
    OwnableUpgradeable
{
    ...
    function buyLifeWithClaim(uint256 _wolfPackId, uint256 _amountCWOLF)
        external
        payable
        nonReentrant
    {
        ...
        uint256 lifeCommission = WolfPacksNFT(wolfPacksContractAddress)
            .energyCommission();
        address commissionWalletAddress = WolfPacksNFT(wolfPacksContractAddress)
            .commissionWalletAddress();

        uint256 lifeCommissionInBNB = Variables(variablesContractAddress)
            .getDollarsInBNB(lifeCommission);
        require(msg.value >= lifeCommissionInBNB, "Not enough value");
        payable(commissionWalletAddress).transfer(lifeCommissionInBNB);
        payable(msg.sender).transfer(msg.value - lifeCommissionInBNB);

        uint256 _amountToIncrease = (Variables(variablesContractAddress)
            .priceCWOLF() * _amountCWOLF) / 1e18;

        WolfPacksNFT(wolfPacksContractAddress).increaseWolfPackLife(_wolfPackId, _amountToIncrease);

        usersAmount[msg.sender] = usersAmount[msg.sender] - _amountCWOLF;

        emit BuyLifeWithClaim(msg.sender, _amountCWOLF);
    }
