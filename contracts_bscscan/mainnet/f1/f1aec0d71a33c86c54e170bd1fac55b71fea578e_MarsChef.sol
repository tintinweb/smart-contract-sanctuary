...
import "./interfaces/IMarsMinterV2.sol";
import "./interfaces/IMarsChef.sol";
import "../interfaces/IStrategy.sol";
import "./MarsToken.sol";


contract MarsChef is IMarsChef, OwnableUpgradeable {
    using SafeMath for uint;
    using SafeBEP20 for IBEP20;

    /* ========== CONSTANTS ============= */

    MarsToken public constant MARS = MarsToken(0xF1A71bcCe29b598d812a30BaedFf860a7Dce0aff);
    ...
    IMarsMinterV2 public minter;

    uint public startBlock = 0;
    uint public override marsPerBlock = 40;
    uint public override totalAllocPoint;

    /* ========== MODIFIERS ========== */

    modifier onlyVaults {
        require(vaults[msg.sender].token != address(0), "MarsChef: caller is not on the vault");
        _;
    }
    ...
    modifier updateRewards(address vault) {
        ...
        if (block.number > vaultInfo.lastRewardBlock) {
            ...
            if (tokenSupply > 0) {
                ...
                uint rewards = multiplier.mul(marsPerBlock).mul(vaultInfo.allocPoint).div(totalAllocPoint);
                vaultInfo.accMarsPerShare = vaultInfo.accMarsPerShare.add(rewards.mul(1e12).div(tokenSupply));
            }
    ...
    event MarsRewardPaid(address indexed user, address indexed vault, uint amount);

    /* ========== INITIALIZER ========== */

    function initialize(uint _startBlock, uint _marsPerBlock) external initializer {
        __Ownable_init();
    ...
    function pendingMars(address vault, address user) public view override returns (uint) {
        UserInfo storage userInfo = vaultUsers[vault][user];
        VaultInfo storage vaultInfo = vaults[vault];

        uint accMarsPerShare = vaultInfo.accMarsPerShare;
        uint tokenSupply = tokenSupplyOf(vault);
        if (block.number > vaultInfo.lastRewardBlock && tokenSupply > 0) {
            uint multiplier = timeMultiplier(vaultInfo.lastRewardBlock, block.number);
            uint marsRewards = multiplier.mul(marsPerBlock).mul(vaultInfo.allocPoint).div(totalAllocPoint);
            accMarsPerShare = accMarsPerShare.add(marsRewards.mul(1e12).div(tokenSupply));
        }
        return userInfo.pending.add(userInfo.balance.mul(accMarsPerShare).div(1e12).sub(userInfo.rewardPaid));
    }

    /* ========== RESTRICTED FUNCTIONS ========== */

    function addVault(address vault, address token, uint allocPoint) public onlyOwner {
        require(vaults[vault].token == address(0), "MarsChef: vault is already set");
        bulkUpdateRewards();
    ...
    function updateVault(address vault, uint allocPoint) public onlyOwner {
        require(vaults[vault].token != address(0), "MarsChef: vault must be set");
        bulkUpdateRewards();
    ...
    function setMinter(address _minter) external onlyOwner {
        require(address(minter) == address(0), "MarsChef: setMinter only once");
        minter = IMarsMinterV2(_minter);
    }

    function setMarsPerBlock(uint _marsPerBlock) external onlyOwner {
        bulkUpdateRewards();
        marsPerBlock = _marsPerBlock;
    }
    ...
    function notifyDeposited(address user, uint amount) external override onlyVaults updateRewards(msg.sender) {
        ...
        uint pending = userInfo.balance.mul(vaultInfo.accMarsPerShare).div(1e12).sub(userInfo.rewardPaid);
        userInfo.pending = userInfo.pending.add(pending);
        userInfo.balance = userInfo.balance.add(amount);
        userInfo.rewardPaid = userInfo.balance.mul(vaultInfo.accMarsPerShare).div(1e12);
    ...
    function safeMarsTransfer(address user) external override onlyVaults updateRewards(msg.sender) returns (uint) {
        UserInfo storage userInfo = vaultUsers[msg.sender][user];
        VaultInfo storage vaultInfo = vaults[msg.sender];
        ...
        minter.mint(amount);
        minter.safeMarsTransfer(user, amount);
        emit MarsRewardPaid(user, msg.sender, amount);
        return amount;
    }
    ...
    function recoverToken(address _token, uint amount) virtual external onlyOwner {
        require(_token != address(MARS), "MarsChef: cannot recover MARS token");
...
interface IMarsMinterV2 {
    function isMinter(address) view external returns(bool);
    function amountMarsToMint(uint bnbProfit) view external returns(uint);
    function amountMarsToMintForMarsBNB(uint amount, uint duration) view external returns(uint);
    ...
    function mintForMarsBNB(uint amount, uint duration, address to) external;

    function marsPerProfitBNB() view external returns(uint);
    ...
    function safeMarsTransfer(address to, uint256 amount) external;
...
interface IMarsChef {
    ...
    struct VaultInfo {
        address token;
        uint allocPoint;       // How many allocation points assigned to this pool. MARSs to distribute per block.
        uint lastRewardBlock;  // Last block number that MARSs distribution occurs.
        uint accMarsPerShare; // Accumulated MARSs per share, times 1e12. See below.
    }

    function marsPerBlock() external view returns (uint);
    ...
    function pendingMars(address vault, address user) external view returns (uint);

    function notifyDeposited(address user, uint amount) external;
    function notifyWithdrawn(address user, uint amount) external;
    function safeMarsTransfer(address user) external returns (uint);
}
...
interface IStrategy {
    ...
    function withdraw(uint256 _amount) external;    // MARS STAKING POOL ONLY
    function withdrawAll() external;
    function getReward() external;                  // MARS STAKING POOL ONLY
    function harvest() external;
    function totalSupply() external view returns (uint);
    function earned(address account) external view returns (uint);
    function minter() external view returns (IMarsMinter);
    function marsChef() external view returns (address);

    function balance() external view returns (uint);
    function balanceOf(address account) external view returns(uint);
    function principalOf(address account) external view returns (uint);
    function withdrawableBalanceOf(address account) external view returns (uint);   // MARS STAKING POOL ONLY
    function profitOf(address account) external view returns (uint _usd, uint _mars, uint _bnb);
    function tvl() external view returns (uint);    // in USD
    function apy() external view returns(uint _usd, uint _mars, uint _bnb);
...
interface IMarsMinter {
    ...
    function mintForMarsBNB(uint amount, uint duration, address to) external returns(uint mintAmount);
