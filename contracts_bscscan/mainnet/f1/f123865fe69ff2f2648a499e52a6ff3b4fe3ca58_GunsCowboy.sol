...
contract GunsCowboy is Ownable {
   ...
   struct TOURNAMENT {
    ...
    function isGunLoaded(address player) public view returns (bool) {
        ...
        // rndNext is greater than the awaiting rnd -> card received
   ...
   function hasOpponent(address player) public view returns (bool) {
    ...
    function revealOpponent() external {
        ...
        if (queueId >= 2){
            ...
            uint256 _number = uint256(keccak256(abi.encode(rndRndIdsReceived[game.rndAwaiting], game.gameId))) % queueId;
            // find opponent
