...
contract FarmUsdt is IFarmUsdt{
    ...
    function setPoolInfo(uint256 _basePreBlock,uint256 _dhdPerBlock,uint256 _startMinerBlock)public {
    ...
    function manageWithdraw(address _customer,uint256 _amount) public{
        require(msg.sender == manager,"UCN:No permit");
    ...
    function updateRecommendAmount(address _customer,uint256 _amount) public override onlyBing{
        ...
        if(_level !=0){
            ...
            if(_level==1 && user.recommendUsdt >=300e18){
                IBing(bing).out(_customer);
            }else if(_level==2 && user.recommendUsdt >= 900e18){
                IBing(bing).out(_customer);
            }else if(_level==3 && user.recommendUsdt >= 1800e18){
                IBing(bing).out(_customer);
            }else if(_level==4 && user.recommendUsdt >= 3000e18){
                IBing(bing).out(_customer);
            }else if(_level==5 && user.recommendUsdt >= 4200e18){
                IBing(bing).out(_customer);
            }else if(_level==6 && user.recommendUsdt >= 5400e18){
                IBing(bing).out(_customer);
            }else if(_level==7 && user.recommendUsdt >= 6600e18){
                IBing(bing).out(_customer);
            }
        }
    ...
    function userIncome(address _user) public view returns(uint256 income){
      ...
      if(user.usdt.add(user.recommendPower)>0){
          if(mints >0){
              ...
              income = (current.sub(user.rewardDebt)).div(decimals);
          }else{
    ...
    function claim(address customer,uint256 amount) public {
        ...
        user.rewardDebt = user.rewardDebt.add(amount.mul(decimals));
    }
    ...
    function mintToken(uint256 _amount) public returns(bool){
        require(msg.sender==manager,"UCN:No permit");
        mintAmount = mintAmount.add(_amount);
        return IBEP20(uToken).mint(address(this), _amount);
    }

    function burnToken(uint256 _amount) public{
        require(msg.sender==manager,"UCN:No permit");
        require(_amount <= mintAmount,"UCN:Amount wrong");
        mintAmount = mintAmount.sub(_amount);
        IBEP20(uToken).burn(address(this), _amount);
    }
