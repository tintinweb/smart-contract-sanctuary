...
contract KORS is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x49dCd3339e3A86200F0A9365b8E70530865dDF1f);
    address payable public devAddress = payable(0x23c0F4767dE06E1995202222Ad38C91229D8B6B3);
    ...
    string private _name = "KORS";
    string private _symbol = "KORS";
    ...
    uint256 public _wasteBurnRate = 10;
    uint256 public _wasteBurnInterval = 200;
    uint256 public _wasteSellCnt = 0;

    uint256 public _buyBackInterval = 100;
    ...
    uint256 public wasteShare = 29;
    uint256 public wasteBalance;
    ...
    constructor () {
        ...
        wasteBalance        = _tTotal.mul(wasteShare).div(100);
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > devBalance.add(wasteBalance)){
            contractTokenBalance = balanceOf(address(this)).sub(devBalance.add(wasteBalance));
        }
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                ...
                if (buyBackEnabled) {
                    ...
                    //auto burn vested
                    if(wasteBalance > 0){
                        if(_wasteSellCnt >= _wasteBurnInterval){
                            uint256 wasteBurnAmount = wasteBalance.mul(_wasteBurnRate).div(10000);
                            if(wasteBurnAmount < balanceOf(address(this))){
                                wasteBalance = wasteBalance.sub(wasteBurnAmount);
                                _wasteSellCnt = 0;
                                _tokenTransfer(address(this),deadAddress,wasteBurnAmount,false);
                            }
                        }
                    }
                    ...
                    if(_buyBackSellCnt >= _buyBackInterval){

                        uint256 marketCap = (bnbAmount.mul(totalSupply())).div(amount);
