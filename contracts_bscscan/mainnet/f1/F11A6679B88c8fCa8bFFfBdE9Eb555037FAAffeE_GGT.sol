...
contract GGT is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
   
    address payable public marketingAddress = payable(0xe88aaF0d489e865720DC2f68C8984Cd146A2858F); // Marketing Address
    ...
    uint256 private minimumTokensBeforeSwap = 2100 * 10**6 * 10**9;
    ...
    function _transfer(
        ...
        bool overMinimumTokenBalance = contractTokenBalance >= amount.mul(marketingDivisor).div(100);   
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                if (overMinimumTokenBalance && _startTimeForSwap + _intervalMinutesForSwap <= block.timestamp) {
                    _startTimeForSwap = block.timestamp;
                    contractTokenBalance = amount.mul(marketingDivisor).div(100);
   ...
   _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _transferBurn(uint256 tBurn) private {
        ...
        uint256 burnFee = _liquidityFee.sub(marketingDivisor);
        
        _rOwned[deadAddress] = _rOwned[deadAddress].add(rBurn.mul(burnFee).div(_liquidityFee));
        if(_isExcluded[deadAddress])
            _tOwned[deadAddress] = _tOwned[deadAddress].add(rBurn.mul(burnFee).div(_liquidityFee));
            
        _rOwned[address(this)] = _rOwned[address(this)].add(tBurn.mul(marketingDivisor).div(_liquidityFee));
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tBurn.mul(marketingDivisor).div(_liquidityFee));
    }
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner {
         require(_maxTxAmount >= _tTotal.mul(1).div(100), "Transfer amount exceeds the maxTxAmount.");
