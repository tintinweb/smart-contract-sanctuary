...
contract SafemoonSwapper is Context, Ownable {
  ...
  // Tells the blockchain how much Safemoon was sent and how much BNB it cost
  event SentTokenBackToHolder(
    address tokenAddress,
    uint256 safemoonAmountPurchased,
    uint256 bnbAmountSpent
  );
  ...
  function swapForSafemoon(address sender, uint256 value) internal {
    ...
    uint256 initalTokenBalance = IERC20(_safemoonAddr).balanceOf(sender);
    ...
    uint256 tokensPurchased = IERC20(_safemoonAddr).balanceOf(sender).sub(initalTokenBalance);
    
    emit SentTokenBackToHolder(_safemoonAddr, tokensPurchased, value);
