...
contract FlavorsAccess is Context {
    ...
    /**
        @title FlavorsTimedPresale
        @author flavorsDev
        @notice inheritable extension contract for the main flavors presale
    ...
    */
contract FlavorsTimedPresale is FlavorsAccess {
    ...
    function checkFluxCapacitor(uint256 start, uint256 end) internal pure {
        ...
        require(
            start < end,
            "PRESALE FLV: Hey, McFly! END must be after START!"
        );
    }
    ...
    function checkEndOrDurationAreZero(uint256 duration, uint256 end) internal pure {
        require(
            ...
            "PRESALE FLV: Can only set END or DURATION, not both"
        );
    }
    ...
    function checkTooLate() internal view {
        ...
        require(
            ...
            "PRESALE FLV: checkTooLate() - Too Late! Presale has ended."
        );
    }
    ...
    function checkTooEarly() internal view {
        ...
        require(
            ...
            "PRESALE FLV: checkToEarly() - Too Early! Presale has not started."
        );
    }
    ...
    function checkOverridesAllowedTimedPresale() internal view {
        require(
            ...
            "PRESALE FLV: Overrides After Contract Launch Are Disabled."
        );
    }
    ...
    function checkTimedPresaleIsNotInitialized() internal view {
        require(
            ...
            "PRESALE FLV: Presale has already been initialized."
        );
    }
...
contract PublicPresaleFLV is FlavorsTimedPresale {
    using SafeMath for uint256;
    ...
    // 100,000/.95 = 105,263
    uint256 internal flvPerNativeCoin = 100_000 * (10**DECIMALS_FLV);
    ...
    */
    ...
    /**
        @notice 'setRefundsEnabled_OICM' In the event the soft cap is not
            ...
            sending the contributor minted flavors tokens, the contributor is
...
//         on all failures
    ...
    function getMaxClaimableFLV() internal view returns (uint256) {
        ...
        return
            (maxHolderContribution.mul(flvPerNativeCoin))
                .div(10**DECIMALS_BNB);
    }
        ...
        returns (uint256)
    {
        ...
        return (
            (contributions[holder].mul(flvPerNativeCoin)
                .div(10**DECIMALS_BNB))
    ...
    */
    ...
    function claim_OWL() external {
        ...
        if(softCapHit()){
            uint256 amount = checkHoldersClaimableFLV(holder);
    ...
    /**
        @notice used to set the addresses of FLV and Creamery
    ...
    */
    ...
    function checkHoldersClaimedFLV(address holder) internal view {
        require(
    ...
    function checkTransferSuccess(bool success) internal pure {
        require(
            success,
            "PRESALE FLV: checkTransferSuccess() - transferFailed"
        );
     }
    ...
    function checkRefundsEnabled() internal view {
        require(
            softCapMissedRefundEnabled,
            "PRESALE FLV: The missed soft cap refunds have not been enabled"
        );
     }
    ...
    function checkHoldersClaimableRefund(
        ...
        returns (uint256)
     {
        ...
        require(
            value > 0,
            "PRESALE FLV: checkHoldersClaimableRefund() - holder has no refund to claim"
        );
        return value;
     }
    ...
    function checkHoldersClaimableFLV(
        ...
        returns (uint256)
     {
        ...
        require(
            amount > 0,
            "PRESALE FLV: checkHoldersClaimableFLV() - holder has no tokens to claim"
        );
        return amount;
     }
    ...
    function checkHolderCompletedClaims(address holder) internal view {
        require(
            !completedClaims[holder],
            "PRESALE FLV: checkHolderCompletedClaims() - holder already hit max claims"
        );
     }
    ...
    function checkBlacklist(address holder) internal view {
        require(
            !blacklist[holder],
            "PRESALE FLV: checkBlacklist() - WALLET BLACKLISTED! What did you do?"
        );
     }
    ...
    function checkWhitelist(address holder) internal view {        
        if(enforceWhitelist){
            require(
                whitelist[holder],
                "PRESALE FLV: checkWhitelist() - NOT WHITELISTED! https://flavorsbsc.com/"
            );
        }
     }
    ...
    function checkHolderMaxContribution(
        ...
        require(
            ...
            "PRESALE FLV: value exceeds holder's remaining allowed contribution"
        );
     }
    ...
    function checkHolderMinContribution(uint256 value) internal view {
        require(
            ...
            "PRESALE FLV: value is less than the minimum contribution"
        );
     }
    ...
    function checkNotInitialized() internal view {
        require(
            !initialized,
            "PRESALE FLV: checkNotInitialized() - Already Initialized!"
        );
     }
    ...
    function checkHardCapHit() internal view {        
        require(
            !hardCapHit(),
            "PRESALE FLV: Cap Reached! ...ask a dev to increase it"
        );
     }
    ...
    function checkNewHardCap(uint256 newHardCap) internal view {
        require(
            ...
            "PRESALE FLV: hardCap must be more than total contributions"
        );
     }
    ...
    function checkNewSoftCap(
        ...
        pure
     {
        require(
            ...
            "PRESALE FLV: checkNewSoftCap() - softCap must be less than hardCap"
        );
     }
    ...
    function checkSoftCapHit() internal view {
        require(
            ...
            "PRESALE FLV: checkSoftCapHit() - Soft Cap Not Hit."
        );
     }
