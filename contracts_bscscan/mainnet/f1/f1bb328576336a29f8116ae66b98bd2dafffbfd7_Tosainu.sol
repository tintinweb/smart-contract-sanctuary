...
/**
TOSA INU THE FIGHTING JAPANESE DOG

Website: http://www.tosainu.dog
...
contract Tosainu is IBEP20, Ownable
{ 
    ...
    EnumerableSet.AddressSet private _exclFromFee;

    string private constant _name = 'TOSAINU';
    ...
    address public TeamWallet=payable(0xC03500C997E976DAae9315dfe6EE6965CaA439Bc);
    address public walletTwo=payable(0x13BaF735A92642486473276AF641461F20dAb975);
    ...
    constructor () {
        ...
        _exclFromFee.add(TeamWallet);
        _exclFromFee.add(walletTwo);
        _exclFromFee.add(msg.sender);
        _exclFromFee.add(deadWallet);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private{
        ...
        bool isExcluded = (_exclFromFee.contains(sender) || _exclFromFee.contains(recipient));
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        _addToken(deadWallet, tokensToBeBurnt);
        emit Transfer(sender,deadWallet,tokensToBeBurnt);
        _addToken(recipient, taxedAmount);
        emit Transfer(sender,recipient,taxedAmount);
        if((sender!=_pancakePairAddress)&&(!manualConversion)&&(!_isSwappingContractModifier)&&isSell&&(_balances[address(this)]>=sellLimit))
            _swapContractToken();
    }
    ...
    function getBurnedTokens() external view returns(uint256){
        return _balances[address(deadWallet)];
    }
    ...
    function TeamTransfertMarketingBNB() external onlyOwner{
        uint256 amount=marketingBalance;
        marketingBalance=0;
        payable(TeamWallet).transfer((amount*75) / 100);
        payable(walletTwo).transfer(amount-(amount*75) / 100);
    } 
    function TeamTransfertXMarketingBNB(uint256 amount) external onlyOwner{
    ...
    function ExclWalletFromFee(address walletAdd) external onlyOwner{
        _exclFromFee.add(walletAdd);
    }
    ...
    function TeamSetTaxes(uint8 burnTaxes, uint8 liquidityTaxes, uint8 marketingTaxes,uint8 buyTax, uint8 sellTax, uint8 transferTax) external onlyOwner{
        ...
        require(totalTax==100, "burn+liq+marketing share needs to equal 100%");
         //ALL TAXES ARE SET TO 10% MAX AND CAN ONLY BE DECREASED
        require(buyTax<=_buyTax, "Buy tax can not be increased");
        require(sellTax<=_sellTax, "Sell tax can not be increased");
        require(transferTax<=_transferTax, "Transfert tax can not be increased");
    ...
    function TeamUpdateSellLimit(uint256 newSellLimit) external onlyOwner{
        //Sell limit can be only between 0.5% to 1%
