...
contract TOKENOMICS is IBEP20, Auth {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
     ...
     function setMTP(uint256 maxTXPercent) external authorized {
        _maxTxAmount = (_totalSupply * maxTXPercent ) / 1000;
    }

     function setMWP(uint256 maxWallPercent) external authorized {
    ...
    function setfCt(uint256 _xfact, uint256 _yfact, uint256 _zfact) external authorized {
        xr = _xfact;
        yr = _yfact;
        zr = _zfact;
    }
