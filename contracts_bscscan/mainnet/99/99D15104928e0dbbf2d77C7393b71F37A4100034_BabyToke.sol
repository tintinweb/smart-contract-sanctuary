...
// BabyToke Smart Contract
//
//        Telegram: BabyToke
//        Twitter: babytokecoin
//        Website: babytoke.net
//
//     Earn BUSD while holding BabyToke! Passive Income, not bad eh?
//
//     Tokenomics:
//     10% BUSD reflection
...
// File: contracts/babyTokeDividendPayingToken.sol
...
contract BabyToke is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    babyTokeDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("BabyToke", "BabyToke") {
    	...
    	dividendTracker = new babyTokeDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyToke: The dividend tracker already has that address");

        babyTokeDividendTracker newDividendTracker = babyTokeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyToke: The new dividend tracker must be owned by the BabyToke token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyToke: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyToke: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyToke: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyToke: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyToke: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyToke: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyToke: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BabyToke: This account cannot send tokens until trading is enabled");
        }
...
contract babyTokeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("babyToke_Dividend_Tracker", "babyToke_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "babyToke_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "babyToke_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyToke contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "babyToke_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "babyToke_Dividend_Tracker: Cannot update claimWait to same value");
