...
contract TukStrategy is Ownable, Pausable {
     ...
     */
    constructor(uint _pid, address _vault, address _fundAddress, address _feeAddress) {
        poolId = _pid;
        vault = _vault;
        fundAddress = _fundAddress;
        feeAddress = _feeAddress;

        IERC20(tuk).safeApprove(masterchef, MAX_UINT256);
    }

    /**
     * @dev Function that puts the funds to work.
     * It gets called whenever someone deposits in the strategy's vault contract.
     * It deposits in the MasterChef to farm {tuk}
     */
    function deposit() public whenNotPaused {
        uint256 pairBal = IERC20(tuk).balanceOf(address(this));
    ...
    function withdraw(uint256 _amount, bool _isNoFee) external onlyVault {
        ...
        if (pairBal < _amount) {   
            ...
            pairBal = IERC20(tuk).balanceOf(address(this));
        }
        ...
        if (!_isNoFee) {
            ...
            IERC20(tuk).safeTransfer(feeAddress, withdrawalFee);
        }

        IERC20(tuk).safeTransfer(vault, pairBal.sub(withdrawalFee));
    }
    ...
    function balanceOfLpPair() public view returns (uint256) {
        return IERC20(tuk).balanceOf(address(this));
    }
    ...
    function retireStrat() external onlyOwner {
        ...
        uint256 pairBal = IERC20(tuk).balanceOf(address(this));
        IERC20(tuk).transfer(vault, pairBal);
    }
    ...
    function pause() public onlyOwner {
        _pause();
        IERC20(tuk).safeApprove(masterchef, 0);
    }
