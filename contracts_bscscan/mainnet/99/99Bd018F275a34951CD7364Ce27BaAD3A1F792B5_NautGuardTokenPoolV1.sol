...
contract NautGuardTokenPoolV1 is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accRewardTokenPerShare; // Accumulated Rewards per share, times 1e20. See below.
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && totalStaked != 0) {
            ...
            accRewardTokenPerShare = accRewardTokenPerShare.add(tokenReward.mul(1e20).div(totalStaked));
        }
        return user.amount.mul(accRewardTokenPerShare).div(1e20).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accRewardTokenPerShare = pool.accRewardTokenPerShare.add(tokenReward.mul(1e20).div(totalStaked));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        updatePool(0);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accRewardTokenPerShare).div(1e20).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accRewardTokenPerShare).div(1e20);
