...
contract LMAO is Context, IERC20, Ownable {
    ...
    address private _charityWalletAddress = 0x534Ed6b07238bb63E6b9B00c2A2A1F2f56DD4AE9;
    address private _devWalletAddress = 0x29650BB5E08F47b3AD4585dbe3B787dA98473d66;
    ...
    uint256 public _maxTxPercent = 10000;
    uint256 public _maxWalletPercent = 10000;
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, Fees memory) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rBurn) = _getRValues(tAmount, fees.tFee, fees.tDev, fees.tLiquidity, fees.tCharity, fees.tBurn, _getRate());
        return (rAmount, rTransferAmount, rFee, rBurn, fees);
    }
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tDev, uint256 tLiquidity, uint256 tCharity, uint256 tBurn, uint256 currentRate) private pure returns (uint256, uint256, uint256, uint256) {
        Rfees memory rFees;
        
        uint256 rAmount = tAmount.mul(currentRate);
        rFees.rFee = tFee.mul(currentRate);
        rFees.rDev = tDev.mul(currentRate);
        rFees.rLiquidity = tLiquidity.mul(currentRate);
        rFees.rCharity = tCharity.mul(currentRate);
        rFees.rBurn = tBurn.mul(currentRate);
        rFees.rTransferAmount = rAmount.sub(rFees.rFee).sub(rFees.rDev).sub(rFees.rLiquidity).sub(rFees.rCharity).sub(rFees.rBurn);
        return (rAmount, rFees.rTransferAmount, rFees.rFee, rFees.rBurn);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rBurn, Fees memory fees) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(fees.tLiquidity + fees.tDev + fees.tCharity);
        _reflectFee(rFee, rBurn, fees.tFee, fees.tBurn);
        emit Transfer(sender, recipient, fees.tTransferAmount);
    }

    function _transferToExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rBurn, Fees memory fees) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(fees.tTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(fees.tLiquidity + fees.tDev + fees.tCharity);
        _reflectFee(rFee, rBurn, fees.tFee, fees.tBurn);
        emit Transfer(sender, recipient, fees.tTransferAmount);
