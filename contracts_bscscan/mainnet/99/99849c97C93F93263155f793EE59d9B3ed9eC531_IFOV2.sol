...
contract FanaticsProfile is AccessControl, ERC721Holder {
    ...
    uint256 public numberGoalToReactivate;
    uint256 public numberGoalToRegister;
    uint256 public numberGoalToUpdate;
    ...
    constructor(
        IBEP20 _goalToken,
        uint256 _numberGoalToReactivate,
        uint256 _numberGoalToRegister,
        uint256 _numberGoalToUpdate
    ) public {
        goalToken = _goalToken;
        numberGoalToReactivate = _numberGoalToReactivate;
        numberGoalToRegister = _numberGoalToRegister;
        numberGoalToUpdate = _numberGoalToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends GOAL to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        goalToken.safeTransferFrom(_msgSender(), address(this), numberGoalToRegister);
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer GOAL token to this address
        goalToken.safeTransferFrom(_msgSender(), address(this), numberGoalToUpdate);
    ...
    function reactivateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        goalToken.safeTransferFrom(_msgSender(), address(this), numberGoalToReactivate);
    ...
    /**
     * @dev Update the number of GOAL to register
     * Callable only by owner admins.
     */
    function updateNumberGoal(
        uint256 _newNumberGoalToReactivate,
        uint256 _newNumberGoalToRegister,
        uint256 _newNumberGoalToUpdate
    ) external onlyOwner {
        numberGoalToReactivate = _newNumberGoalToReactivate;
        numberGoalToRegister = _newNumberGoalToRegister;
        numberGoalToUpdate = _newNumberGoalToUpdate;
    }
     ...
     */
    function grantPointRole(address _pointer) external onlyOwner {
        grantRole(POINT_ROLE, _pointer);
    }
}
 ...
 * other FanaticsProfile requirements.
 */
contract IFOV2 is IIFOV2, ReentrancyGuard, Ownable {
    ...
    // FanaticsProfile
    FanaticsProfile public fanaticsProfile;
     ...
     * @param _fanaticsProfileAddress: the address of the FanaticsProfile
    ...
    constructor(
        ...
        address _fanaticsProfileAddress,
        uint256 _startBlock,
        uint256 _endBlock
    ) public {
        require(_lpToken.totalSupply() >= 0, "_lpToken.totalSupply >= 0");
        require(_offeringToken.totalSupply() >= 0, "_offeringToken.totalSupply >= 0");
        require(_lpToken != _offeringToken, "Tokens must be be different");

        lpToken = _lpToken;
        offeringToken = _offeringToken;
        fanaticsProfile = FanaticsProfile(_fanaticsProfileAddress);
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        require(fanaticsProfile.getUserStatus(msg.sender), "Must have an active profile");
    ...
    function _claimPoints(address _user) internal {
        if (!_hasClaimedPoints[_user]) {
            ...
            if (sumPools > thresholdPoints) {
                ...
                fanaticsProfile.increaseUserPoints(msg.sender, numberPoints, campaignId);
            }
        }
    }
