...
contract StageETH is Ownable, IBEP20, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    string constant _name = "StageETH";
    string constant _symbol = "StageETH";
    ...
    uint256 public earlySellingFee = 26; 
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal open(sender, recipient) returns (bool) {
        ...
        if(canSwap()) {
            if(shouldSwapBack()){ swapBack(ignoreLimit); }
    ...
    function setExcludeMaxTxnMultiple(address[] calldata _users, bool exempt) external onlyOwner {
        for(uint8 i = 0; i < _users.length; i++) {
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amount, uint256 intervalSecondsForSwap, bool _ignoreLimit) external onlyOwner {
