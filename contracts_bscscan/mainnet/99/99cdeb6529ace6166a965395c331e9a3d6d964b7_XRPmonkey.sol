...
/**
https://t.me/XRPmonkeybsc

This Monkey Is Giving Away XRP To The Believers !
Join Us, Together We Can Make This GEM Moon !
...
contract XRPmonkey is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    XRPmonkeyDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("XRPmonkey", "XRPmonkey") {
        _ETHRewardsFee = 5;
        _liquidityFee = 5;
		_marketingFee = 5;
    	...
    	dividendTracker = new XRPmonkeyDividendTracker();
    	antiBotEnabled = true;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "XRPmonkey: The dividend tracker already has that address");

        XRPmonkeyDividendTracker newDividendTracker = XRPmonkeyDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "XRPmonkey: The new dividend tracker must be owned by the XRPmonkey token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "XRPmonkey: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "XRPmonkey: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "XRPmonkey: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "XRPmonkey: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "XRPmonkey: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "XRPmonkey: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "XRPmonkey: Cannot update gasForProcessing to same value");
...
contract XRPmonkeyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("XRPmonkey_Dividend_Tracker", "XRPmonkey_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "XRPmonkey_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "XRPmonkeyDividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main XRPmonkey contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "XRPmonkey_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "XRPmonkey_Dividend_Tracker: Cannot update claimWait to same value");
