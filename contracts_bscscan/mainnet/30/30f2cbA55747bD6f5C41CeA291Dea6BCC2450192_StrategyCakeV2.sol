...
contract StratManagerCake is Ownable, Pausable {
     ...
     */
    function setvgatorFeeRecipient(address _vgatorFeeRecipient)
        external
        onlyOwner
    {
        vgatorFeeRecipient = _vgatorFeeRecipient;
...
abstract contract FeeManagerCake is StratManagerCake {
    ...
    uint256 public vgatorFee = MAX_FEE - callFee;

    function setCallFee(uint256 _fee) external onlyManager {
        require(_fee <= MAX_CALL_FEE, "!cap");

        callFee = _fee;
        vgatorFee = MAX_FEE - callFee;
    }
...
contract StrategyCakeV2 is StratManagerCake, FeeManagerCake {
    ...
    constructor(
        ...
        public
        StratManagerCake(_keeper, _unirouter, _vault, _vgatorFeeRecipient)
    {
        _giveAllowances();
    }
    ...
    function chargeFees() internal {
        uint256 toWrapped = IERC20(want).balanceOf(address(this)).mul(10).div(
            1000
        );
        ...
        uint256 vgatorFeeAmount = wrappedBal.mul(vgatorFee).div(MAX_FEE);
        IERC20(wrapped).safeTransfer(vgatorFeeRecipient, vgatorFeeAmount);
    }
