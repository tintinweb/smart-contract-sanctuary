...
contract TestContract is Context, IERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;

    address payable public devAddress = 0xF07253e1dE5B6f782a89Aa6D8C635709B0773d5C;
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        //     !inSwapAndLiquify &&
        //     shouldSell && 
        //     !(from == pancakePair && to == address(this)) &&
        //     !(from == pancakePair && to == DEAD) &&
        //     !(from == address(pancakeRouter) && to == DEAD) &&
        //     !(from == address(pancakeRouter) && to == address(this)) &&
        //     !(from == address(pancakeRouter) && to == address(pancakePair)) &&
        //     !(from == address(pancakePair) && to == address(pancakeRouter)) &&
        //     swapAndLiquifyEnabled &&
        //     !(from == address(this) && to == address(pancakePair)) // swap 1 time
        // ) {
        //     // only sell for minTokenNumberToSell, decouple from _maxTxAmount
