...
contract PirateBankBSC is WhitelistUpgradeable {
    ...
    IPantherRouter02 private constant AS_ROUTER = IPantherRouter02(0xb3Bb34973f9285903ddc4cCDd41e9B3c339E207a); // PCS Router
    ...
    function withdrawAll(address token) external payable onlyTokenOwner(token) {
        ...
        if (token == address(0)) {
            ...
            if (remaining > 0) require(ERC20(token).transfer(ownerOrKeeper(), remaining));
    ...
    function processDividendsManually(uint amount) internal returns (uint dividendPayout) {
        ...
        uint wbnbAmount = 0;
        uint pirateAmount = 0;
        _approveTokenIfNeeded(WBNB, address(AS_ROUTER));
        _approveTokenIfNeeded(PIRATE);

        wbnbAmount = dividendPayout.div(2);
        pirateAmount = _swapFromBNB(address(AS_ROUTER), WBNB, dividendPayout.sub(wbnbAmount), PIRATE, address(this));
        ROUTER.addLiquidityETH{value: wbnbAmount}(PIRATE, pirateAmount, 0, 0, address(this), block.timestamp);
