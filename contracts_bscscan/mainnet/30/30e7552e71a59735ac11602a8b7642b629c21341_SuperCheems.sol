...
contract SuperCheemsDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("SuperCheems_Dividend_Tracker", "SuperCheems_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SuperCheems_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SuperCheems_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Test contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SuperCheems_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");   
        require(newClaimWait != claimWait, "SuperCheems_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SuperCheems is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    SuperCheemsDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Super Cheems", "SUPERCHEEMS") {
        BNBRewardsFee = 1;
        liquidityFee = 1;
        extraFeeOnSell = 16; // extra fee on sell
        marketingFee = 1;
        buyBackFee =1; 
        marketingWallet = payable(0x9273FcFa74f99dAfF3A0f9c1352FCB7aCEcba0BD);
        buyBackWallet = payable(0x88dC6AC50dacEAA497a80868e2d3c53d258C64b8); 

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(buyBackFee); // total fee transfer and buy

        dividendTracker = new SuperCheemsDividendTracker();
