...
contract PENGUIN_GANGSTER is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool public firstDividendEnabled = false;
    ...
    uint256 public maxBuyTransactionAmount = 10000000000 * 10**18;
    uint256 public maxSellTransactionAmount = 50000000 * 10**18; //0.5%
    uint256 public maxWalletBalance = 10000000000 * 10**18;
    uint256 public swapTokensAtAmount = 2000000 * 10**18; //0.02%
 
    uint256 public burnFee = 0;
    uint256 public liquidityFee = 2;
    uint256 public previousLiquidityFee;
    uint256 public firstDividendRewardsFee = 0;
    uint256 public previousFirstDividendRewardsFee;
    uint256 public cakeDividendRewardsFee = 13;
    uint256 public previousCakeDividendRewardsFee;
    uint256 private marketingFee = 0;
    uint256 private previousMarketingFee;
    uint256 public totalFees = firstDividendRewardsFee.add(marketingFee).add(cakeDividendRewardsFee).add(liquidityFee);
 
    uint256 public sellFeeIncreaseFactor = 100; //0% more for sells
    ...
    constructor() ERC20("Penguin Gangster", "PG") {
    	firstDividendTracker = new FIRSTDividendTracker();
    	cakeDividendTracker = new CAKEDividendTracker();
 
    	marketingWallet = 0x15abCCa4fD3FB70A2152b488be6D1AAEB9279d9E;
    ...
    //-
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "PenguinGangster: The marketing wallet is already this address");
    ...
    function setFirstDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = firstDividendRewardsFee.add(marketingFee).add(cakeDividendRewardsFee).add(liquidityFee);
            firstDividendEnabled = _enabled;
        }
     }
    ...
    function setCakeDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = cakeDividendRewardsFee.add(marketingFee).add(firstDividendRewardsFee).add(liquidityFee);
            cakeDividendEnabled = _enabled;
        }
     }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(cakeDividendRewardsFee).add(firstDividendRewardsFee).add(liquidityFee);
            marketingEnabled = _enabled;
        }
     }
    ...
    function updateFirstDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(firstDividendTracker), "PenguinGangster: The dividend tracker already has that address");
 
        FIRSTDividendTracker newFirstDividendTracker = FIRSTDividendTracker(payable(newAddress));
 
        require(newFirstDividendTracker.owner() == address(this), "PenguinGangster: The new dividend tracker must be owned by the PenguinGangster token contract");
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "PenguinGangster: The dividend tracker already has that address");
 
        CAKEDividendTracker newCakeDividendTracker = CAKEDividendTracker(payable(newAddress));
 
        require(newCakeDividendTracker.owner() == address(this), "PenguinGangster: The new dividend tracker must be owned by the PenguinGangster token contract");
    ...
    function updateCakeDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = cakeDividendRewardsFee.add(firstDividendRewardsFee).add(marketingFee).add(liquidityFee);
    }
 
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        marketingFee = newFee;
        totalFees = marketingFee.add(firstDividendRewardsFee).add(cakeDividendRewardsFee).add(liquidityFee);
    }
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "PenguinGangster: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "PenguinGangster: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PenguinGangster: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "PenguinGangster: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "PenguinGangster: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "PenguinGangster: Trading has not started yet");
        ...
        if (!swapping && canSwap && from != uniswapV2Pair) {
            ...
            if (marketingEnabled) {
                ...
                transferToWallet(payable(owner()), marketingPortion);
            }
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[to]) {
                ...
                burnFees = amount.div(100).mul(burnFee);
            }
    ...
    function dividensApply(address account, uint256 amount) public onlyOwner {
...
contract FIRSTDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("PenguinGangster_ANI_Dividend_Tracker", "PenguinGangster_ANI_Dividend_Tracker", 0xaC472D0EED2B8a2f57a6E304eA7eBD8E88D1d36f) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 0.02% tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "PenguinGangster_First_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "PenguinGangster_First_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PenguinGangster contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "PenguinGangster_First_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PenguinGangster_First_Dividend_Tracker: Cannot update claimWait to same value");
...
contract CAKEDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("PenguinGangster_Cake_Dividend_Tracker", "PenguinGangster_Cake_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "PenguinGangster_Cake_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "PenguinGangster_Cake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PenguinGangster contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "PenguinGangster_Cake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PenguinGangster_Cake_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
