...
//  ////             ////  '-----`
//  " "             ////
// BAD SANTA - ❄️ BADSANTA brings BNB to all naughty degens in the DeFi space. Ultimate BNB rewards and not only.
// 20% Fees: 15% BNB Reward, 4% Marketing Wallet, 1% LP
...
contract NAUGHTYLISTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("NAUGHTYLIST_Dividend_Tracker", "NAUGHTYLIST_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 50000000 * (10**18); //must hold 50M+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "NAUGHTYLIST_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "NAUGHTYLIST_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NAUGHTYLIST contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "NAUGHTYLIST_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "NAUGHTYLIST_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BADSANTA is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    NAUGHTYLISTDividendTracker public dividendTracker;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _extraFeeOnSell) public onlyOwner {
        BNBRewardsFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        extraFeeOnSell = _extraFeeOnSell; // extra fee on sell

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(extraFeeOnSell); // total fee transfer and buy
    }
    ...
    constructor() ERC20("Bad Santa", "BADSANTA") {
        BNBRewardsFee = 15;
        liquidityFee = 1;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 4;
        marketingWallet = payable(0x7faf65054C09C2fb9682bd22FF215466803C9102);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(extraFeeOnSell); // total fee transfer and buy

    	dividendTracker = new NAUGHTYLISTDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BADSANTA: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BADSANTA: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BADSANTA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BADSANTA: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BADSANTA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BADSANTA: Cannot update gasForProcessing to same value");
