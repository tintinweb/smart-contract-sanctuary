...
/*
METAFACE TOKEN (MFT) is a Deflationary Token, NFT, NFT game with
many game use case. Metaface token concept mimics the Facebook
Metaverse 3D Gaming experience as MFT will be the first NFT 3D 
Virtual Reality Game in the NFT Blockchain system. METAFACE 3D
Game is fun and Entertaining. The next generation NFT Game that
provides innovative features in storing, earning cryptocurrencies
and NFT with token distribution to holders and great auto Liquidity
functions.

   https://metafacetoken.org/home.html
   https://twitter.com/metafacetoken
   https://t.me/metafacetoken
...
contract MetafaceToken is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    address CHARITYWallet = 0x74ea71a695a3155Ed3095FdbaDF42Fb11C936916;
    ...
    string private _symbol = "METAFACE";
    ...
    constructor() {
        ...
        _isExcludedFromReward[CHARITYWallet] = true;
    ...
    function totalCHARITYWallet() public view returns (uint256) {
        return balanceOf(CHARITYWallet);
    }
    ...
    function _transferBurn(uint256 tBurn) private {
        uint256 currentRate = _getRate();
        uint256 rBurn = tBurn.mul(currentRate);
        _rOwned[CHARITYWallet] = _rOwned[CHARITYWallet].add(rBurn);
        if (_isExcludedFromReward[CHARITYWallet]) _tOwned[CHARITYWallet] = _tOwned[CHARITYWallet].add(tBurn);
    }
    ...
    function _transferFromExcluded(
        ...
        emit TransferBurn(sender, CHARITYWallet, tBurn);
