...
contract InfoBundlerV1 {
    Reinvest public reinvest;
    IMasterChef public masterchef;

    constructor(Reinvest _reinvest) {
        reinvest = _reinvest;
        masterchef = reinvest.masterchef();
    }

    function infoBundleUser(address _user)
        public
        view
        returns (
            uint256[] memory all,
    ...
    {
        uint256 _pl = masterchef.poolLength();
        all = new uint256[](_pl);
        bal = new uint256[](_pl);
        pending = new uint256[](_pl);
        added = new uint256[](_pl);
        for (uint256 i = 0; i < _pl; i++) {
            try this.LP_INFO(i, _user) returns (uint256 _all, uint256 _bal, uint256 _pending, uint256 _added) {
                all[i] = _all;
                bal[i] = _bal;
                pending[i] = _pending;
                added[i] = _added;
    ...
    function LP_INFO(uint256 _pid, address _user)
        public
        view
        returns (
            uint256 all,
        ...
        )
    {
        ...
        all = pool.lpToken.allowance(_user, address(reinvest));
        bal = pool.lpToken.balanceOf(_user);
        pending = reinvest.seePending(_pid, _user);
        added = reinvest.addedToPoolByUser(_pid, _user);
    }

    function infoBundlePools()
        public
        view
        returns (
            IMasterChef.PoolInfo[] memory pools,
            uint256[] memory totalSupplies,
            uint8[] memory decimals,
            uint256[] memory TAP_MCPB
        )
    {
        TAP_MCPB = new uint256[](2);
        TAP_MCPB[0] = masterchef.totalAllocPoint();
        TAP_MCPB[1] = reinvest.MIN_CPB_USER();
        uint256 _pl = masterchef.poolLength();
        pools = new IMasterChef.PoolInfo[](_pl);
        totalSupplies = new uint256[](_pl);
        decimals = new uint8[](_pl);
        for (uint256 i = 0; i < _pl; i++) {
            try this.LP_INFO_POOL(i) returns (IMasterChef.PoolInfo memory pool, uint256 totalSupplyPool, uint8 _decimals) {
                pools[i] = pool;
                totalSupplies[i] = totalSupplyPool;
                decimals[i] = _decimals;
            } catch {}
        }
    }

    function LP_INFO_POOL(uint256 _pid)
        public
        view
        returns (
    ...
    {
        pool = masterchef.poolInfo(_pid);
        totalSupplyPool = pool.lpToken.balanceOf(address(masterchef));
        decimals = pool.lpToken.decimals();
    }

    function infoBundleTotal()
        public
        view
        returns (
            uint256[] memory pending,
            uint256[] memory added,
            IMasterChef.UserInfo[] memory addedMC
        )
    {
        uint256 _pl = masterchef.poolLength();
        pending = new uint256[](_pl);
        added = new uint256[](_pl);
        addedMC = new IMasterChef.UserInfo[](_pl);
        for (uint256 i = 0; i < _pl; i++) {
            try this.LP_INFO_TOTAL(i) returns (uint256 _pending, uint256 _added, IMasterChef.UserInfo memory _addedMC) {
                pending[i] = _pending;
                added[i] = _added;
                addedMC[i] = _addedMC;
            } catch {}
    ...
    function LP_INFO_TOTAL(uint256 _pid)
        public
        view
        returns (
            uint256 pending,
    ...
    {
        pending = masterchef.pendingCake(_pid, address(reinvest));
        added = reinvest.addedToPool(_pid);
        addedMC = masterchef.userInfo(_pid, address(reinvest));
    }
}

contract InfoBundlerV2 is InfoBundlerV1 {
    function LP_PAIR_INFOS(uint256 _pid)
        public
        view
        returns (
            IBEP20 token0,
    ...
    {
        IMasterChef.PoolInfo memory pool = masterchef.poolInfo(_pid);
        token0 = pool.lpToken.token0();
        token1 = pool.lpToken.token1();
        symbol0 = token0.symbol();
        symbol1 = token1.symbol();
    }

    function infoBundleSymbols()
        public
        view
        returns (
            address[] memory tokens0,
            address[] memory tokens1,
            string[] memory symbols0,
            string[] memory symbols1,
            string[] memory symbols
        )
    {
        uint256 _pl = masterchef.poolLength();
        tokens0 = new address[](_pl);
        tokens1 = new address[](_pl);
        symbols0 = new string[](_pl);
        symbols1 = new string[](_pl);
        symbols = new string[](_pl);
        for (uint256 i = 0; i < _pl; i++) {
            try this.LP_PAIR_INFOS(i) returns (IBEP20 token0, IBEP20 token1, string memory symbol0, string memory symbol1) {
                tokens0[i] = address(token0);
                tokens1[i] = address(token1);
                symbols0[i] = symbol0;
                symbols1[i] = symbol1;
            } catch {
                try masterchef.poolInfo(i).lpToken.symbol() returns (string memory s) {
                    symbols[i] = s;
                } catch {}
            }
        }
    ...
    constructor(Reinvest _reinvest) InfoBundlerV1(_reinvest) {}
}