...
contract ERC20 is Context, IERC20 {
    ...
    uint256 public _maxTxAmount = 2*10**10 * 10**8;
    uint256 public _maxWalletToken = 6*10**10 * 10**8;
    uint256 private _start_timestamp = block.timestamp;
    bool private _enableTaxes = false;
    bool private _enableMaxAmounts = false;
    ...
    function setMaxIgnores(bool enable) public virtual {
        _enableMaxAmounts = enable;
    }
    
    function getMaxIgnores() public view returns (bool) {
        return _enableMaxAmounts;
    }
    ...
    function _calculateChtAmount(uint256 totalAmount) internal view returns (uint256) {
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if (_enableTaxes) {
            ...
            chtAmount = _calculateChtAmount(amount);
            burnAmount = _calculateBurnAmount(amount);
            
            taxAmount = burnAmount + chtAmount;
...
contract QueerDoge is ERC20, AccessControl {

    constructor() ERC20("QueerDoge", "QDG") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        setCharityAddress(0x80BA4ee0243a9d4A7d549387E990D302A49a2fA1);
        _mint(msg.sender, 10**12 * 10**8);
    }
    ...
    //Max amounts will be enabled after pre-sale
    function setMaxIgnores(bool enabled) public override {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Only Owner can enable taxes");
        super.setMaxIgnores(enabled);
    }
