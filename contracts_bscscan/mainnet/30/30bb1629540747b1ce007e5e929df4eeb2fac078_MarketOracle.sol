...
/**
 * @title EARTHIE price Oracle
 *      This Oracle calculates the average USD price of EARTHIE based on the BNB-EARTHIE and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private earthieBnbPrice0CumulativeLast;
    uint private earthieBnbPrice1CumulativeLast;
    uint32 private earthieBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _earthie_bnb;
    ...
    constructor(
        address __earthie_bnb,   // Address of the EARTHIE-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx 
        ) public {

        controller = msg.sender;

        _earthie_bnb = IUniswapV2Pair(__earthie_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        earthieBnbPrice0CumulativeLast = _earthie_bnb.price0CumulativeLast();
        earthieBnbPrice1CumulativeLast = _earthie_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, earthieBnbBlockTimestampLast) = _earthie_bnb.getReserves();
    ...
    // Get the average price of 1 EARTHIE in the smallest BNB unit (18 decimals)
    function getEarthieBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_earthie_bnb));

        FixedPoint.uq112x112 memory earthieBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - earthieBnbPrice0CumulativeLast) / (_blockTimestamp - earthieBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, earthieBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        uint earthieBnbAverage;
        uint busdBnbAverage;

        (earthieBnbPrice0CumulativeLast, earthieBnbPrice1CumulativeLast, earthieBnbBlockTimestampLast, earthieBnbAverage) = getEarthieBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, earthieBnbAverage) = getEarthieBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = (earthieBnbAverage)*1e6 / busdBnbAverage;
