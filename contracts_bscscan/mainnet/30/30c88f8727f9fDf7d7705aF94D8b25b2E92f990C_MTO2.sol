...
contract MTO2 is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function setOfferingAmount(uint256 _offerAmount) public onlyAdmin {
        require(block.number < startBlock, "mto is live");
    ...
    function setStartBlock(uint256 _startBlock) public onlyAdmin {
        require(block.number < startBlock, "mto is live");
        //require(endBlock - _startBlock > 7200, "mto should last at least 6 hours");
        startBlock = _startBlock;
    }
    
    function setEndBlock(uint256 _endBlock) public onlyAdmin {
        require(block.number < startBlock, "mto is live");
        //require(_endBlock - startBlock > 7200, "mto should last at least 6 hours");
        endBlock = _endBlock;
    }
    
    function resetMTO(IBEP20 _stakeToken, uint256 _startBlock, uint256 _endBlock, uint256 _offeringAmount, uint256 _raisingAmount) public onlyAdmin {
        require(block.number > endBlock || endBlock == 0, "mto is live"); // Can't reset if MTO is live
        require(_startBlock < _endBlock, "end block is lower than start block");
        //require(_endBlock - _startBlock > 7200, "mto should last at least 6 hours");
        ...
        // offeringToken.mint(_offeringAmount - offeringToken.balanceOf(address(this)));
    ...
    function fillUserInfo(address[] memory _addresses, uint256[] memory _amount) public onlyAdmin {
        ...
        for (uint i = 0; i < _addresses.length; i++) {
            addressList.push(address(_addresses[i]));
            userInfo[_addresses[i]].amount = userInfo[_addresses[i]].amount.add(_amount[i]);
            userInfo[_addresses[i]].claimed = false;

            totalAmount = totalAmount.add(_amount[i]);
            totalDebt = totalDebt.add(_amount[i]);
        }
    }
    ...
    function finalWithdraw(uint256 _stakeTokenAmount) external onlyAdmin {
