...
contract SCP is Context, IERC20, Ownable {
    ...
    string private _name = "Swiss Crypto Payment";
    ...
    uint256 public _maxTxAmount = 5000000000 * 10**3;
    ...
    constructor (address payable burnWalletAddress, address payable mktWalletAddress) {
    ...
    (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tMkt) = _getValues(tAmount);
    ...
    function setMktFeePercent(uint256 mktFee) external onlyOwner() {
        require(mktFee <= 50, 'Marketing fees can not be higher than 50 [5%]');
        _mktFee = mktFee;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tBurn, data.tMkt);
    }
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tBurn).sub(tMkt);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tBurn, tMkt, 0));
    }
    function _getRValues( TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rBurn).sub(rMkt);
