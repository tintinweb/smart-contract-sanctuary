...
// MoonCookie BEP20 Token Available on PancakeSwap
// $MoonCookie - Is The Highest BNB Reflection Token!
// Earn 18% BNB reflection By Holding!
// Our project is 100% safe by a based dev Contract is verified.
// https://mooncookie.online/  <--- Website
// https://t.me/MoonCookie  <--- Telegram
// https://mooncookie1.gitbook.io/mooncookie/ <--- DOCS
...
contract MoonCookieDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("MoonCookie_Dividend_Tracker", "MoonCookie_Dividend_Tracker", 9) {
    	claimWait = 14400;
        minimumTokenBalanceForDividends = 1000000 * (10**9); //must hold 10000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "MoonCookie_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "MoonCookie_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MoonCookie contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MoonCookie_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MoonCookie_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MoonCookieToken is BEP20, Ownable {
    using SafeMath for uint256;

    IPancakeswapV2Router02 public pancakeswapV2Router;
    address public immutable pancakeswapV2Pair;
    address public marketingWallet = 0x305745DCb45B89C4F01861C7CD6CCff5cF1a4B07;

    bool private swapping;

    MoonCookieDividendTracker public dividendTracker;
    ...
    constructor() public BEP20("MoonCookie", "MoonCookie", 9) {
    	...
    	dividendTracker = new MoonCookieDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MoonCookie: The dividend tracker already has that address");

        MoonCookieDividendTracker newDividendTracker = MoonCookieDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MoonCookie: The new dividend tracker must be owned by the MoonCookie token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "MoonCookie: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MoonCookie: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "MoonCookie: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MoonCookie: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "MoonCookie: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MoonCookie: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MoonCookie: Cannot update gasForProcessing to same value");
