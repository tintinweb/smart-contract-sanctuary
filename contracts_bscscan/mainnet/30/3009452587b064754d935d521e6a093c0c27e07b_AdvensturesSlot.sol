...
contract AdvensturesSlot is Context, PartnerOwnable  { 
    ...
    uint256 UserMaxWinTokens = 320000;
    uint256 UserMaxBetTokens = 10000;
    ...
    function withdrawETH(uint256 _amount) external onlyPartner {   
    ...
    function setUserMaxBetTokens(uint256 _UserMaxBetTokens) external onlyAdmin { 
        UserMaxBetTokens = _UserMaxBetTokens;
    } 
    function setUserMaxWinTokens(uint256 _UserMaxWinTokens) external onlyAdmin { 
        UserMaxWinTokens = _UserMaxWinTokens;
    }
    function setUserTotalBetTokens(uint256 _UserTotalBetTokens) external onlyAdmin { 
	...
	function _bet(uint[] memory bets, uint tot_bet, uint domainId, uint tokensId) internal returns(uint win_tot, WINS memory _wins){
        ...
        user.average_bet = uint256(user.tot_bet / user.count);  
	    require(user.tot_win<=UserMaxWinTokens,'AdvensturesSlot:Total_win_over_UserMaxWinTokens');
	   

        if(tot_bet > wins.wins_result){
            TransferHelper.safeTransferFrom(FRUIT, msg.sender, address(this), tot_bet - wins.wins_result); 
        } else if(tot_bet < wins.wins_result){
            TransferHelper.safeTransfer(FRUIT, msg.sender, wins.wins_result - tot_bet);   
        }
        ...
        if(tokensInfo[tokensId].tokens_cake_all >= wins.wins_result) tokensInfo[tokensId].tokens_cake_all -= wins.wins_result;
        ...
        if(tokensInfo[tokensId].tokens_cake_all > tokensInfo[tokensId].tokens_cake_fee2){
            ...
            tokensInfo[tokensId].tokens_cake_balance = 0;
        }
    ...
    function randomize_game_rate_rand() internal  returns (uint) { 
        randNonce ++;
        randNonce = randNonce % 32767;
        seed = uint(keccak256(abi.encode(UserTotalBetTokens, seed, block.number, block.coinbase, randNonce, block.timestamp)));  
