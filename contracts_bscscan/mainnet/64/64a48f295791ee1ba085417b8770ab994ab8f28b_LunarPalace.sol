...
Hyper-deflationary with static reflection that rewards holders, 
so more Lunar Palace Toekens are being automatically added to your wallet each transaction. 
Simply Love, pet, and hold as 7% from each transaction is automatically redistributed to Lunar Palace Toekens holders.
...
2% Liquidity share
 ...
 7% tokens Redistributed to Share Holders.
...
// Decimal 9! Name Lunar Palace Tokens (LunarPalace), 

contract LunarPalace is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Lunar Palace Tokens";
    string private _symbol = "LunarPalace";
    ...
    uint256 internal _tokenTotal = 1000000000000000 * 10**9; //1000_000_000_000_000e9;
 ...
 * 
 * 
 * 2 % tax to marketing wallet (Required wallet address)
2% Liquidity
1% Burn
7% tokens Redistributed to Share Holders.
 * 
 */
    ...
    uint256 public _taxFee = 7; // means 7% which distribute to all holders
    uint256 public _liquidityFee = 2; // means 2% add liquidity on each buy and sell
    uint256 public _burnFee = 1; // means 1% it menas on burn

    uint256 public MarketingFee = 200; //200 means 2% will send to Marketing

    address payable MarketingAddress =
        0xc16CBB19423DA924C19922e91B144925658e39eB; // please
    ...
    uint256 public minTokensBeforeSwap = 1000000000000000 * 10**9; // 1000_000_000_000_000e9;
    ...
    constructor(address preSaler) public {
        ...
        _reflectionBalance[preSaler] = _reflectionTotal;

        emit Transfer(address(0),preSaler, _tokenTotal);
    }
    ...
    function excludeAccount(address account) external onlyOwner {
        ...
        require(_excluded.length < 50, "Can not add more than 50 address");
    ...
    function _transfer(
        ...
        if (
            ...
            transferAmount = transferAmount.sub(tokensToBurn);
                    //Owner fee
        _reflectionBalance[address(this)] = _reflectionBalance[address(this)]
            .add(tokensToBurn.mul(rate));
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalSwapFee = MarketingFee.add(_liquidityFee);
        uint256 marketingAmount = contractTokenBalance.mul(MarketingFee).div(
            totalSwapFee
        );
        uint256 liquiditySwap = contractTokenBalance.sub(marketingAmount);
        // split the contract balance into halves
        uint256 half = liquiditySwap.div(2);
        uint256 otherHalf = liquiditySwap.sub(half);
        ...
        totalSwapFee = marketingAmount.add(half);
        // swap tokens for ETH
        swapTokensForEth(totalSwapFee); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);

        marketingAmount = newBalance.mul(marketingAmount).div(totalSwapFee);
        liquiditySwap = newBalance.sub(marketingAmount);

        // send marketing postion
        MarketingAddress.transfer(marketingAmount);
        // add liquidity to uniswap
        addLiquidity(otherHalf, liquiditySwap);
