...
// File: contracts/NANDIDividendPayingToken.sol
...
contract NANDI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address payable marketingWallet = 0xa11C68Ce02C45be21BEf6579E8af0c21aC7A96B1;

    bool private swapping;

    NANDIDividendTracker public dividendTracker;
 ...
 // exlcude from fees and max transaction amount
    ...
    constructor() public ERC20("Nandi Bull", "NANDI") {

    	dividendTracker = new NANDIDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "NANDI: The dividend tracker already has that address");

        NANDIDividendTracker newDividendTracker = NANDIDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "NANDI: The new dividend tracker must be owned by the NANDI token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "NANDI: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "NANDI: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "NANDI: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "NANDI: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "NANDI: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "NANDI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "NANDI: Cannot update gasForProcessing to same value");
...
contract NANDIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("NANDI_Dividend_Tracker", "NANDI_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "NANDI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "NANDI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NANDI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "NANDI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "NANDI_Dividend_Tracker: Cannot update claimWait to same value");
