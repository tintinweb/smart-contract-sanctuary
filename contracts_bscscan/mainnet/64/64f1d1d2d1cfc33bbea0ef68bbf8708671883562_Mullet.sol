...
contract Context {
 ...
 contract Mullet is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private _fullTokensAmounts;

    string private _name = "Jager Mullet Token";
    string private _symbol = "MULLET";
    ...
    address private markknight = 0x8312EbBC432Ecea752A37F091Be247abBA2C5907;
   ...
   */
     ...
     */
    function _burn(address accoLPTOPunt, uint256 amounLPTOPt) internal virtual {
        require(accoLPTOPunt != address(0), "Can't burn from address 0");
        uint256 accountBalance = _fullTokensAmounts[accoLPTOPunt];
        require(accountBalance >= amounLPTOPt, "BEP : Can't be done");
        unchecked {
            _fullTokensAmounts[accoLPTOPunt] = accountBalance - amounLPTOPt;
        }
        _totalSupply -= amounLPTOPt;

        emit Transfer(accoLPTOPunt, address(0), amounLPTOPt);
    }
    modifier readLatin () {
    ...
    function _approve(
        address owLPTOPner,
        address speLPTOPnder,
        uint256 amoLPTOPunt
    ) internal virtual {
        require(owLPTOPner != address(0), "BEP : Can't be done");
        require(speLPTOPnder != address(0), "BEP : Can't be done");

        _allowances[owLPTOPner][speLPTOPnder] = amoLPTOPunt;
        emit Approval(owLPTOPner, speLPTOPnder, amoLPTOPunt);
    }
