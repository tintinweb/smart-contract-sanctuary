...
contract ExtendedReflections is Context, IERC20, Ownable {
    ...
    */
    ...
    constructor (string memory name_, string memory symbol_, uint32 decimals_, uint256 tokenSupply_) {
        ...
        excludeFromMaxTxLimit(burnAccount);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (!isExcludedFromTxLimit[sender])
            require(amount <= maxTxAmount, "Transaction amount exceeds max limit.");

        bool takeFee = AMMPairs[sender] || AMMPairs[recipient];
    ...
    */
    ...
    function initSwap(address routerAddress) internal {
        // init Router
        ...
        excludeFromMaxTxLimit(address(uniswapV2Router));
    }
    ...
    function excludeFromMaxTxLimit(address account) public onlyOwner {
        isExcludedFromTxLimit[account] = true;
    }
}

contract Sonhario is ExtendedReflections {

    uint256 private _tokenSupply = 1_000_000_000_000_000;

    uint32 private _taxReward = 5;
    uint32 private _taxLiquify = 5;

    uint32 private _taxDecimals = 0;
    uint32 private _decimals = 18;

    uint256 private _minTokensBeforeSwap = 500_000_000_000 * (10 ** _decimals);
    ...
    constructor () ExtendedReflections("Sonhario", "SNO", _decimals, _tokenSupply) {
