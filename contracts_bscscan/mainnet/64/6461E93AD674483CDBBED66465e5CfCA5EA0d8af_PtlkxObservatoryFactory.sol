...
// File: contracts/PtlkxObservatory.sol
...
contract PtlkxObservatoryInitializable is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // The address of the Ptlkx Observatory factory
    address public PTLKX_OBSERVATORY_FACTORY;
    ...
    // The block number when PTLKX mining ends.
    uint256 public bonusEndBlock;

    // The block number when PTLKX mining starts.
    ...
    // PTLKX tokens created per block.
    ...
    constructor() public {
        PTLKX_OBSERVATORY_FACTORY = msg.sender;
    }
    ...
    function initialize(
        ...
        require(msg.sender == PTLKX_OBSERVATORY_FACTORY, "Not factory");
...
// File: contracts/PtlkxObservatoryFactory.sol
...
contract PtlkxObservatoryFactory is Ownable {
    event NewPtlkxObservatoryContract(address indexed ptlkxObservatory);
     ...
     * @param _withdrawalInterval: the withdrawal interval for stakedToken (if any, else 0)
     * @param _admin: admin address with ownership
     * @return address of new Ptlkx Observatory contract
     */
    function deployPool(
        IBEP20 _stakedToken,
        IBEP20 _rewardToken,
        ...
        bytes memory bytecode = type(PtlkxObservatoryInitializable).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address ptlkxObservatoryAddress;

        assembly {
            ptlkxObservatoryAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        PtlkxObservatoryInitializable(ptlkxObservatoryAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewPtlkxObservatoryContract(ptlkxObservatoryAddress);
    }
}