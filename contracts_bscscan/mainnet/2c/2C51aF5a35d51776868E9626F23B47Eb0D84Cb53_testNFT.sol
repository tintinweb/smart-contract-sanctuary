...
contract testBase is testAccessControl,ERC1155{
    ...
    event NFTCreate(address account, uint256 amount, uint16 itemType,uint16 itemName,uint16 gameId);
    ...
    function newGame(string memory gameName,uint16 version) public onlyCEO{
    ...
    uint16 itemName,uint16 gameId,bytes memory data)public virtual onlyPartner(){
        ...
        ItemAttr memory attr = 
            ItemAttr(itemName,gameId,1,false,false,1,genes,strength,intelligence,agility,health, originalCombat);
        tokenIdToItemAttr[newItemId] = attr;
        creators[newItemId] = _msgSender();
        _tokenIds.increment();
        NFTCreate(account,1,1,itemName,gameId);
    ...
    uint16 itemName,uint16 gameId,bytes memory data)public virtual onlyPartner(){
        ...
        uint256 originalCombat = strength.add(intelligence).add(agility);
        ItemAttr memory attr = 
            ItemAttr(itemName,gameId,2,true,false,1,genes,strength,intelligence,agility,health,originalCombat);
        tokenIdToItemAttr[newItemId] = attr;
        creators[newItemId] = _msgSender();
        _tokenIds.increment();
        NFTCreate(account,1,1,itemName,gameId);
    ...
    function getRarity(uint256 _tokenId) public view returns(uint8) {
    ...
    function levelUpItem(uint256 _tokenId1, uint256 _tokenId2) public returns(uint256 _rand,uint256 _genes){
        ...
        require(token1.isStaked == false && token2.isStaked == false, "one of token is staked");
        ...
        tokenIdToItemAttr[newItemId] = 
            ItemAttr(attr.name,attr.gameId,attr.itemType,false,false,1,newGenes,strength,intelligence,agility,health,originalCombat);
        creators[newItemId] = _msgSender();
        _tokenIds.increment();
    ...
    function upgradeStar(uint256 _tokenId1, uint256 _tokenId2, uint16 choice) public returns(bool){
        ...
        uint8 tokenRarity = getRarity(_tokenId2);
        uint factor = 2;
        if (tokenRarity == 1) {
            factor = 20;
    ...
    function giveBirth(uint256 _tokenId, bytes memory data) public {
        ...
        require(token.isStaked == false, "one of token is staked");
        require(token.itemType == 1, "only hero can give birth");
        require(token.isGaveBirth == false, "the hero have gaven birth");
        require(PAY_TOKEN.transferFrom(_msgSender(), address(this), giveBirthAmount), "you need pay fee to upgrade");
        
        Game memory game = games[token.gameId];
        if (allowNFTFlag == false) {
            require(game.manager == _msgSender(),"Base: msgsender not manager!");
        }
        ...
        uint256 originalCombat = strength.add(intelligence).add(agility);
        ItemAttr memory attr = 
            ItemAttr(token.name,token.gameId,1,false,false,1,genes,strength,intelligence,agility,health, originalCombat);
        tokenIdToItemAttr[newItemId] = attr;
        creators[newItemId] = _msgSender();
        _tokenIds.increment();
        NFTCreate(msg.sender,1,1,token.name,token.gameId);
        token.isGaveBirth = true;
    }
