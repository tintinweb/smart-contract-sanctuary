...
contract FarmProxy {
    ...
    BiswapPool constant biswapPool = BiswapPool(0x994f7b32eEa35ADe9B8bFd3cef19E9B6EFE0Cb24);
    ...
    constructor(address creator) public {
        insuredFarm = creator;
        wbnb.approve(address(biswapPool), 2 ** 255);
    }
...
contract InsuredBiswapBNBFarm {
    ...
    uint256 public nutsRate = 75; // 0.75 nuts
    ...
    function deposit() payable external {
        ...
        require(wbnb.transfer(address(proxy), amount));
    ...
    function cashout(uint256 amount) external {
        ...
        uint256 beforeBiswapBalance = biswap.balanceOf(address(this));
        FarmProxy proxy = FarmProxy(farms[farmer]);
        farms[farmer].call(abi.encodePacked(proxy.withdraw.selector, abi.encode(amount)));

        WBNB wrappedBNB = WBNB(address(wbnb));
        uint256 gained = wbnb.balanceOf(address(this)) - beforeBalance;
        if (gained > 0) {
            wrappedBNB.withdraw(gained);
            address(farmer).transfer(gained);
        }
    ...
    function compensate(address farmer, uint256 amountShort, uint256 farmersCashout, uint256 systemAmount) internal {
        if (!compensationUsed) {
            ...
            uint256 totalBnbShort = (amountShort * systemAmount) / farmersCashout;
            uint256 bnbNutsValue = totalBnbShort / nutsTwap.consult(address(nuts), (10 ** 18)); // (bnb short divided by nuts price)
            uint256 beforeBalance = nuts.balanceOf(address(this));
            address(governance).call(abi.encodePacked(governance.pullCollateral.selector, abi.encode(bnbNutsValue)));
    ...
    // For beta this function just avoids blackholing wbnb IF an issue causing compensation is later resolved
