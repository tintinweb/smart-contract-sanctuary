...
contract PreKratosPresale is ReentrancyGuard {
	using SafeMath for uint256;
    ...
    uint256 public minimumDepositAmount = 1 * 10 ** 18;
    uint256 public maximumDepositAmount = 1000 * 10 ** 18;
	...
	function buyPKratos(uint256 _amount) public nonReentrant {
        ...
        require(_amount + (tokensOwned[msg.sender] * DAIPerToken) <= maximumDepositAmount, "high amount than max");
        ...
        pKratos.safeTransfer( msg.sender, tokens );
	...
	function setDAIPerToken(uint256 _newValue) external onlyOwner {
		DAIPerToken = _newValue;
	}
	...
	function claimTokens(uint256 _amount) external nonReentrant {
		...
		tokensUnclaimed[msg.sender] = callerTokensUnclaimed.sub(_amount);

        pKratos.safeTransferFrom( msg.sender, address(this), _amount );
        Kratos.safeTransfer( msg.sender, _amount.div(1e9) );
