...
contract MiniWar is Context, IERC20, Ownable {
    ...
    address public BTCBAddr=0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 _BTCB=contractTokenBalance.mul(80).div(10**2);
            
            uint256 _liquiditys=contractTokenBalance.mul(20).div(10**2);
            //add liquidity
            swapAndLiquify(_liquiditys);
            swapTokensForDividendToken(_BTCB);
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if (UsdtTimestamp[sender]==0||(UsdtTimestamp[sender]+1800)<=block.timestamp&&IERC20(BTCBAddr).balanceOf(address(this))>0&&sender!=UniswapV2RouterAddr){
            uint256 Wiamount=balanceOf(sender)*IERC20(BTCBAddr).balanceOf(address(this))/_tTotal;
            IERC20(BTCBAddr).transfer(sender,Wiamount);
