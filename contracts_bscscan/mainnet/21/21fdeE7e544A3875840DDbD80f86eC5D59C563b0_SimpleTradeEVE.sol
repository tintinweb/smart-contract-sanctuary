...
contract SimpleTradeEVE is Ownable {
    ...
    function trade(address _from, address _to) internal {
        uint256 amount = IERC20(_from).balanceOf(address(this));
        require(amount > 0);

        IERC20(_from).approve(naverSwapRouter, amount);
    ...
    function buyEVE() external onlyOwner {        
        require(previousBusdAmount == 0);
        previousBusdAmount = IERC20(busd).balanceOf(address(this));
        
        require(previousBusdAmount > 0);
        trade(busd, eve);
    }

    function sellEVE() external onlyOwner {
        require(IERC20(eve).balanceOf(address(this)) > 0);
        trade(eve, busd);

        uint256 currentAmount = IERC20(busd).balanceOf(address(this));
        require(currentAmount > previousBusdAmount);

        uint256 profit = currentAmount.sub(previousBusdAmount);
        require(profit >= 1 ether);

        IERC20(busd).transfer(profitAddress, profit);

        previousBusdAmount = 0;
    }
