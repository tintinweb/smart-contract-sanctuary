...
abstract contract StratX2 is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public buyBackRate = 500; // 100;  1%(buy-back and burn fee)
    ...
    function earnGammaProfitsOnWithdraw(address caller) internal  {
        ...
        if(pendingProfit > 0){
            ...
            pendingProfit = distributeFees(pendingProfit);
            pendingProfit = buyBack(pendingProfit); 
    ...
    function userPendingGammaProfit(address userAddress) public view virtual returns (uint256 pendingProfitInCake,uint256 pendingProfitInGamma){
        ...
        if(stratRewardPerShare > 0 && user.shares > 0){
            ...
            if(pendingProfitInCake > 0){
                pendingProfitInCake = pendingProfitInCake.sub(pendingProfitInCake.mul(controllerFee).div(controllerFeeMax));
                pendingProfitInCake = pendingProfitInCake.sub(pendingProfitInCake.mul(buyBackRate).div(buyBackRateMax));
            }
            else{
                pendingProfitInCake = 0;
            }
