...
contract SBBX is Context, IERC20, Ownable {
    ...
    uint256 private _tsbbxTotal;

    string private _name = "SBBX";
    string private _symbol = "SBBX";
    ...
    uint256 public _sbbxFee = 12;
    uint256 private _previoussbbxFee = _sbbxFee;
    
    uint256 public _maxTxAmount = 1000 * 10**9 * 10**9;
    uint256 private minimumTokensBeforeSwap = 30 * 10**6 * 10**9; 
    
    address payable public sbbxWallet = 0x0543eDB2c291419dBf43b18c7A5329924D0892a7; 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _tsbbxTotal = _tsbbxTotal.add(address(this).balance);
        TransferCharityETH(sbbxWallet, address(this).balance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_sbbxFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _sbbxFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previoussbbxFee = _sbbxFee;
    ...
    function setsbbxFeePercent(uint256 sbbxFee) external onlyOwner() {
        _sbbxFee = sbbxFee;
    }
