...
contract Gicks is Context, IERC20, Ownable {
    using Address for address;
    address private constant _marketingAddress = 0x558BfF550B1272CAD61202CFF8974CF8D526e49c;
    //MAPINGS
    ...
    string private constant _name = 'Gicks'; // to be determined
    string private constant _symbol = 'GICK'; // to be determined
    uint8 private constant _decimals = 10;

    //Supply and Reflected Supply and other options
    uint256 private constant _decimalFactor = 10**uint256(_decimals);
    uint256 private _tokenTotal = 100000000 * _decimalFactor ; 
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        _reflectBalances[_marketingAddress]+=burnToken*currentRate;
        ...
        if(_isExcludedFromReward[_marketingAddress]) _balances[_marketingAddress] += burnToken;
        _reflectedTotal -= reflectToken*currentRate;
        emit Transfer(sender, recipient, amount-burnToken-reflectToken);
        if (burnToken!=uint256(0)) emit Transfer(sender, _marketingAddress, burnToken);
    }
    ...
    function getTransferFee(address sender, address recipient) private view returns (uint256, uint256){
        ...
        uint256 burnt = balanceOf(_marketingAddress);
    ...
    function _getSupply() public view returns(uint256){
        return _tokenTotal-balanceOf(_marketingAddress);
    }
    ...
    function getFeeInfo() public view returns(uint256,uint256){
        return (getTransferFee(_marketingAddress, _marketingAddress));
    }
