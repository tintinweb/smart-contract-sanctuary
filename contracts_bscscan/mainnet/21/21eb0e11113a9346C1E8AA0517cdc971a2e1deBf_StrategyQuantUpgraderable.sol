...
// File: contracts/StrategyQuantUpgraderable.sol
...
contract StrategyQuantUpgraderable is ReentrancyGuardUpgradeSafe, OwnableUpgradeable , Pausable {
    ...
    function _farm() internal {
        ...
        IFairLaunch(farmContractAddress).deposit(pid, wantAmt);
        ...
        returns (uint256)
    {
        ...
        if (isGrandComp) {
            IFairLaunch(farmContractAddress).withdraw(pid, _wantAmt);
        }
    ...
    function unlock() public {
      ...
      IQuantToken(earnedAddress).unlock();
    }
    ...
    function pathChange() public {
        require(msg.sender == govAddress, "!gov");
        require(!isCAKEStaking, "isCAKEStaking");
        earnedToGrandPath = [earnedAddress,busdAddress, wbnbAddress, GrandAddress];
    
        earnedToToken0Path = [earnedAddress, busdAddress, token0Address];
        if (busdAddress == token0Address) {
            earnedToToken0Path = [earnedAddress, busdAddress];
        }

        earnedToToken1Path = [earnedAddress, busdAddress, token1Address];
        if (busdAddress == token1Address) {
            earnedToToken1Path = [earnedAddress, busdAddress];
        }

        token0ToEarnedPath = [token0Address, busdAddress, earnedAddress];
        if (busdAddress == token0Address) {
            token0ToEarnedPath = [busdAddress, earnedAddress];
        }

        token1ToEarnedPath = [token1Address, busdAddress, earnedAddress];
        if (busdAddress == token1Address) {
            token1ToEarnedPath = [busdAddress, earnedAddress];
        }
    }

    function pathChangeSingle() public {
        require(msg.sender == govAddress, "!gov");
        earnedToGrandPath = [earnedAddress, busdAddress ,wbnbAddress, GrandAddress];
    }
