...
contract BlissfulPlanetV2 is Context, IERC20, Ownable {
    ...
    uint256 public maxBuyAmount = 100000 * 10**3 * 10**9;
    uint256 public maxSellAmount = 100000 * 10**3 * 10**9;
    uint256 public swapTokensAtAmount = 50 * 10**3 * 10**9;
    uint256 public buyBackUpperLimit = 1 * 10**18;


    address public rewardAddress = 0x03c744141B04d384fC4788BdEA157ae4016A6f1a;
    address public teamAddress = 0x7f2221Ae1D227FCf65EDd4CBE03598110d0E01c2;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "BlissfulPlanetV2";
    string private constant _symbol = "BPcoinV2";
    ...
    function setFeeRates(uint256 _rfi, uint256 _team, uint256 _reward, uint256 _liquidity, uint256 _buyback) external onlyOwner {
        feeRates.rfi = _rfi;
        feeRates.team = _team;
        feeRates.reward = _reward;
        feeRates.liquidity = _liquidity;
        feeRates.buyback = _buyback;
        emit FeesChanged();
    }

    function setSellFeeRates(uint256 _rfi, uint256 _team, uint256 _reward, uint256 _liquidity, uint256 _buyback) external onlyOwner{
        sellFeeRates.rfi = _rfi;
        sellFeeRates.team = _team;
        sellFeeRates.reward = _reward;
    ...
    function _takeReward(uint256 rReward, uint256 tReward) private {
        ...
        {
            _tOwned[rewardAddress]+=tReward;
        }
        _rOwned[rewardAddress] +=rReward;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rTeam, to_return.rReward, to_return.rLiquidity, to_return.rBuyback) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tTeam = tAmount*sellFeeRates.team/1000;
            s.tReward = tAmount*sellFeeRates.reward/1000;
            s.tLiquidity = tAmount*sellFeeRates.liquidity/1000;
            s.tBuyback = tAmount*sellFeeRates.buyback/1000;
            s.tTransferAmount = tAmount-s.tRfi-s.tTeam-s.tReward-s.tLiquidity-s.tBuyback;
        }
        else{
            s.tRfi = tAmount*feeRates.rfi/1000;
            s.tTeam = tAmount*feeRates.team/1000;
            s.tReward = tAmount*feeRates.reward/1000;
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rTeam,uint256 rReward, uint256 rLiquidity, uint256 rBuyback) {
        ...
        rTransferAmount =  rAmount-rRfi-rTeam-rReward-rLiquidity-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rTeam,rReward,rLiquidity, rBuyback);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        emit Transfer(sender, address(this), s.tLiquidity + s.tTeam + s.tBuyback);
        emit Transfer(sender, rewardAddress, s.tReward);
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 denominator= (feeRates.liquidity + feeRates.buyback + feeRates.team) * 2;
    ...
    function updateRewardWallet(address newWallet) external onlyOwner{
        require(rewardAddress != newWallet ,'Wallet already set');
        rewardAddress = newWallet;
        _isExcludedFromFee[rewardAddress];
    }

    function updateTeamWallet(address newWallet) external onlyOwner{
        require(teamAddress != newWallet ,'Wallet already set');
        teamAddress = newWallet;
        _isExcludedFromFee[teamAddress];
    }
