...
contract MarketPlace is IMarketPlace, ReentrancyGuard, AccessControlEnumerable, IERC721Receiver {
    ...
    function listPrice(uint256 _tokenId, uint256 _price) external override nonReentrant {
        ...
        emit OnUpdateItemPrice(_tokenId, sale.owner, _oldPrice, _price);
    }
    ...
    function unlistItem(uint256 _tokenId) external override nonReentrant {
        ...
        SaleInfo storage sale = idToMarketItem[_toItemId[_tokenId]];
        require(sale.seller == msg.sender, " !owner");
        require(sale.status != 3, "PAUSED"); // PAUSED

        sale.status = 4; // UNLISTED
        _onSaleItems -= 1;
        emit offSaleItem(_tokenId, sale.owner);
    }
    ...
    function listItem(uint256 _tokenId, uint256 _price) external override nonReentrant {
        ...
        require(sale.owner == msg.sender, "Not buyer of the token");
        require(sale.status != 3, "PAUSED"); // PAUSED

        nERC721 _nft = nERC721(sale.nftContract);
    ...
    function offer(
        ...
        uint8 payingTokenId;
        require(buyer != sale.seller, "!owner");
    ...
    function _purchase(
        ...
        if (offerPrice != 0) {
            ...
            require(buyer != saleInfo.seller, "Cannot buy your own Niimfer");
        ...
        if (!_restrictedToNIMF) {
            ...
            (bool transferToSeller, ) = saleInfo.seller.call{ value: _amount }("");
            require(transferToSeller);
        } else {
            payingTokenId = 2;
            if (offerPrice != 0) {
                IERC20(payingToken).safeTransfer(saleInfo.seller, _amount);
            } else {
                IERC20(payingToken).safeTransferFrom(msg.sender, saleInfo.seller, _amount);
            }
        }
        nERC721(nftContract).safeTransferFrom(saleInfo.seller, buyer, tokenId);
        _onSaleItems -= 1;
