...
contract Locker is ILocker, IERC721ReceiverUpgradeable, AdminableUpgradeable {
    ...
    IERC20Upgradeable public gmpdToken;
    IGMPD public override collection;
    ...
    mapping(address => uint256) public override userNFT; // Stores information about the deposited NFT
    ...
    event SetCollection(IGMPD collection);

    function __Locker_init(IERC20Upgradeable _gmpdToken, ITierCalculator _tierCalculator, IGMPD _collection, address _feeTo) public initializer {
    ...
    function getLockedGMPD(address _user) external view override returns (uint256) {
    ...
    function deposit(uint256 _amount) external override {
        gmpdToken.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function depositNFT(uint256 _tokenId) external {
        require(_tokenId > 0, "depositNFT: tokenId cannot be zero");
        require(userNFT[msg.sender] == 0, "depositNFT: you had already deposited");

        collection.safeTransferFrom(msg.sender, address(this), _tokenId);
        userNFT[msg.sender] = _tokenId;
    ...
    function withdraw(uint256 _amount) external override {
        ...
        gmpdToken.safeTransfer(msg.sender, _amount.sub(feeAmount));

        if (feeAmount > 0) {
            gmpdToken.safeTransfer(feeTo, feeAmount);
        }
    ...
    function withdrawNFT() external {
        ...
        require(tokenId > 0, "withdrawNFT: tokenId cannot be zero");
    ...
    function setCollection(IGMPD _collection) external onlyOwnerOrAdmin {
        collection = _collection;
