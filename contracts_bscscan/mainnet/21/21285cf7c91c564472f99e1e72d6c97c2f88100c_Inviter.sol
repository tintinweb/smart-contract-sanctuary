...
interface IVip {
    ...
    // 获取用户vip等级
    function getVip(address _account) external view returns (uint);
...
contract Inviter is Ownable {
    ...
    function setVip(address _value) external onlyOwner {
        vip = _value;
    }
    ...
    function invite(address _account) external {
        ...
        require(IVip(vip).getVip(msg.sender) > 0, "invite: IVip(vip).getVip(msg.sender) > 0");
