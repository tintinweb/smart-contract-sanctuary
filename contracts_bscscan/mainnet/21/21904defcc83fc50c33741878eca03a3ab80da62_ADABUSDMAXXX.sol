...
contract ADABUSDMAXXX is ERC20, Ownable {
    ...
    uint256 rewardbDividendRewardsFee = 3;
    uint256 marketingFee = 4;
    uint256 buyBackAndLiquidityFee = 4;
    uint256 toBurnNativeTokenFee = 1;
    ...
    bool private tradingIsEnabled = true;
    ...
    bool private sendrewardAInTx = true;
    bool private sendrewardBInTx = true;

    rewardADividendTracker public rewardaDividendTracker;
    rewardBDividendTracker public rewardbDividendTracker;
    
    address private toBurnNativeTokenAddress = marketingFeeReceiverA;

    uint256 private buyBackUpperLimit = 1 * (10**9);

    uint256 private minimumBalanceRequired = 1 * (10**9);

    uint256 private minimumSellOrderAmount = ( _totalSupply * 1 ) / 1000000;
    
    uint256 private swapTokensAtAmount = ( _totalSupply * 3 ) / 10000;

    uint256 private previousrewardADividendRewardsFee;
    uint256 private previousrewardBDividendRewardsFee;
    uint256 private previousMarketingFee;
    uint256 private previousBuyBackAndLiquidityFee;

    uint256 private rewardaDividendPriority = 80;

    bool public sellFeeIncreaseFactorEnabled = true;
    uint256 public sellFeeIncreaseFactor = 150;

    uint256 private gasForProcessing = 1300000;

    mapping (address => bool) private isExcludedFromWallet;
    ...
    event ExcludeFromWallet(address indexed account, bool isExcluded);
    ...
    constructor() ERC20("ADABUSDMAXXX", "ADABUSDMAX") {
        ...
        excludeFromDividend(address(rewardaDividendTracker));
        excludeFromDividend(address(rewardbDividendTracker));
        excludeFromDividend(address(this));
        excludeFromDividend(address(_uniswapV2Router));
        excludeFromDividend(deadAddress);
        excludeFromFees(MAAA, true);
        excludeFromFees(MAAB, true);
        excludeFromFees(address(this), true);
        excludeFromFees(owner(), true);
        excludeFromWallet(MAAA, true);
        excludeFromWallet(MAAB, true);
        excludeFromWallet(address(this), true);
        excludeFromWallet(owner(), true);
    ...
    function updateBurnNativeTokenAddress(address _newAddress) external onlyOwner {
        toBurnNativeTokenAddress = _newAddress;
    }

    function updatemarketingFeeReceiverB(address _newWallet) external onlyOwner {
        excludeFromFees(_newWallet, true);
        excludeFromWallet(_newWallet, true);
    ...
    function _updateTotalFee() internal {
        totalFees = buyBackAndLiquidityFee.add(marketingFee).add(rewardaDividendRewardsFee).add(rewardbDividendRewardsFee).add(toBurnNativeTokenFee);
    }

    function setFees(uint256 _rewardaDividendRewardsFee, uint256 _rewardbDividendRewardsFee, uint256 _marketingFee, uint256 _buyBackAndLiquidityFee, uint256 _toBurnNativeTokenFee, uint256 _feeDenominator) external onlyOwner {
        ...
        toBurnNativeTokenFee = _toBurnNativeTokenFee;
        totalFees = buyBackAndLiquidityFee.add(marketingFee).add(rewardaDividendRewardsFee).add(rewardbDividendRewardsFee).add(toBurnNativeTokenFee);
    ...
    function excludeFromWallet(address account, bool excluded) public onlyOwner {
        require(isExcludedFromWallet[account] != excluded, "Already excluded");
        isExcludedFromWallet[account] = excluded;

        emit ExcludeFromWallet(account, excluded);
    }
   ...
   function _transfer(
        ...
        if (
            ...
            require(amount <= _maxTxAmount, "Error amount");

            uint256 contractBalanceRecipient = balanceOf(to);
            require(contractBalanceRecipient + amount <= _maxWalletToken, "Error amount");
        } else if (
            tradingIsEnabled &&
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (marketingEnabled) {
                    ...
                    if(increaseAmount > 0){
                        uint256 marketingAPortion = increaseAmount.mul(50).div(100);
                ...
                if(toBurnNativeTokenFee > 0){
                    uint256 swapTokensToBurnNativeToken = contractTokenBalance.mul(toBurnNativeTokenFee).div(totalFees);
                    buyBackNativeTokenAndBurn(swapTokensToBurnNativeToken);
                }
    ...
    function buyBackNativeTokenAndBurn(uint256 amount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = toBurnNativeTokenAddress;
