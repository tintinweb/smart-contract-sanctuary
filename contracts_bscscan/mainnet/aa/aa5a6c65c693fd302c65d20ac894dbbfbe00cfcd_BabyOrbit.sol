...
// BabyOrbit!.
//
// DIVIDEND YIELD PAID IN Andromeda V2 (M31)! With the auto-claim feature,
// simply hold$BabyOrbit and you'll receive Andromeda V2 (M31) automatically in your wallet.
// Join the Orbit today!!!
// Hold BabyOrbit and get rewarded in Andromeda V2 (M31) on every transaction!
//
//
// 📱 Telegram: https://t.me/bbyorbit
// 🌎 Website: https://www.Babyorbitbsc.com
// 🌐 Twitter: https://twitter.com/BabyOrbitbsc
*/
...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public userCurrentRewardToken = 0xB46aCB1f8D0fF6369C2f00146897aeA1dFCf2414;
  bool public userHasCustomRewardToken = true;
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyOrbit: The router already has that address");
  ...
  function swapETHForTokens(
        ...
        bool swapSuccess;
        IERC20 token = IERC20(0xB46aCB1f8D0fF6369C2f00146897aeA1dFCf2414);
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
        ...
        if(!userHasCustomRewardToken && !isIgnoredToken(userCurrentRewardToken)){
...
contract BabyOrbit is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BabyOrbitDividendTracker public dividendTracker;
    ...
    // fees
    uint256 public BNBRewardsFee = 9;
    uint256 public liquidityFee = 10; // This fee must be a TOTAL of LP, Operations Fee AND Buyback fee (Note, Buyback has been disabled)
	...
	uint256 public _maxSellPercent = 100; // Set the maximum percent allowed on sale per a single transaction
    ...
    constructor() ERC20("BabyOrbit", "BM31", 9) {

        dividendTracker = new BabyOrbitDividendTracker();

        liquidityWallet = owner();
        operationsWallet = 0x29579CE1d524cd1573e07CC02a6cC33295ecE366;
        buyBackWallet = owner();
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
      require(isContract(ammAddress), "BabyOrbit: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyOrbit: The dividend tracker already has that address");

        BabyOrbitDividendTracker newDividendTracker = BabyOrbitDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyOrbit: The new dividend tracker must be owned by the BabyOrbit token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "BabyOrbit: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyOrbit: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "BabyOrbit: The operations wallet is already this address");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "BabyOrbit: The buyback wallet is already this address");
    ...
    function updateFees(uint256 bnbRewardPerc, uint256 liquidityPerc, uint256 operationsPerc, uint256 buyBackPerc) external onlyOwner {
        require (operationsPerc.add(buyBackPerc) <= liquidityPerc, "BabyOrbit: updateFees:: Liquidity Perc must be equal to or higher than operations and buyback combined.");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyOrbit: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyOrbit: Cannot update gasForProcessing to same value");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyOrbit: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require(!_isIgnoredAddress[to] || !_isIgnoredAddress[from], "BabyOrbit: To/from address is ignored");
	...
	if(automatedMarketMakerPairs[to]){
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[to]) {
                ...
                uint256 otherSellFee = amount.mul(liquidityFee).div(100);
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if(liquidityFee > 0){ // dividing by 0 is not fun.
            ...
            if(operationsFee > 0){
                ...
                require(success, "BabyOrbit: SwapAndLiquify:: Unable to send BNB to Operations Wallet");
            }
            ...
            if(buyBackFee > 0){
                ...
                require(success, "BabyOrbit: SwapAndLiquify:: Unable to send BNB to BuyBack Wallet");
            }
...
contract BabyOrbitDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BabyOrbit_Dividend_Tracker", "BabyOrbit_Dividend_Tracker", 9) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000 * (10**9); //must hold 100000+ tokens to get divs
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BabyOrbit_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BabyOrbit_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main  contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyOrbit_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Orbit_Dividend_Tracker: Cannot update claimWait to same value");
