...
So you guys want to swingtrade with a low Tax? Sure do it. 
Liquidity is locked right after launch for a whole month. 
I will not bother to create a Telegram. as this is for all the swingtraders on low caps :) And i'll have a 1% wallet my self or something so just do it i dont have a hidden Mint or anything in the CA 
Also i'll renounce owner :)
...
// CHANGE
contract SwingParadise is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "SwingParadise";
    string constant _symbol = "SP";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100 * (10 ** _decimals); // 100
    uint256 public _maxTxAmount = (_totalSupply * 2) / 100; // 2% MAX 2
    uint256 public _maxWalletSize = (_totalSupply * 4) / 100; // 4% MAX 4
    ...
    uint256 totalFee = 200; // liquidityFee + reflectionFee + devFee
