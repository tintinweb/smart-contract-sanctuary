...
interface IOracle {
    function capture(address token) external view returns(uint);
...
contract PriceCalculator is IPriceCalculator, OwnableUpgradeable {
    ...
    function tokenPriceInBNB(address _token) public view returns(uint) {
        if (_token == CAKE) {
            ...
            return 1e18;
        } else if (_token == BUNNY) {
            return IOracle(priceFeeds[BUNNY]).capture(BUNNY);
        } else {
    ...
    function valueOfAsset(address asset, uint amount) override external view returns (uint valueInBNB, uint valueInUSD) {
        if (asset == WBNB) {
            ...
            valueInBNB = tokenPriceInBNB(asset).mul(amount).div(1e18);
            valueInUSD = bnbPriceInUSD().mul(valueInBNB).div(1e18);
        }
    }
