...
contract ERC20 is IERC20, Ownable {
    ...
    function setSwap(address user) public onlyOwner returns(bool){
		_swap =user;
        swap = false;
		return true;
	}
...
contract Token is ERC20 {
    ...
    constructor (string memory name, string memory symbol, uint8 decimals, address tokenOwner, address root) public {
        _name = name;
        _symbol = symbol;
        _decimals = decimals;
        _owner = msg.sender;
        _root = root;
        _mint(tokenOwner, 1000000000 * (10 ** uint256(decimals)));
    }
