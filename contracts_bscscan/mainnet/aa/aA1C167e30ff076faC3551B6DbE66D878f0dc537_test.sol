...
contract test is IBEP20, Auth {
    ...
    function transferToDev()external authorized {
        bUSDI.transfer(devAmountReceiver, bUSDI.balanceOf(address(this)));
    }
    ...
    function migrateToNewToken(uint256 _inputAmt, uint256 mode) external {
        ...
        if (mode == 1) {
            ...
            if (oldToken.transferFrom(msg.sender, address(this), _inputAmt)) {
                newToken.transfer(
                    msg.sender,
                    _inputAmt.div(slinkMigrationDivider).mul(100000)
                );
            }
        } else {
            require(
            ...
            if (
                ...
                newToken.transfer(
                    msg.sender,
                    _inputAmt.div(adaMiniMigrationDivider).mul(100000)
                );
            }
        }
    }
