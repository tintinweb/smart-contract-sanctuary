...
contract ArtexLocking {
    ...
    constructor(address payable _governance, ERC20 _artexToken) public {
        ...
        reserveAllocation = SafeMath.mul(25000000, 10**(18)); // 25 million  - 4 year lock
        
        reserveTimeLock = SafeMath.add(now, 1460 days);
    ...
    function releaseReserveTokens() public returns (bool success) {
        ...
        require(now >= reserveTimeLock, "!tokens cannot be withdrawn now");

        require(
            artexToken.transfer(governance, reserveAllocation),
