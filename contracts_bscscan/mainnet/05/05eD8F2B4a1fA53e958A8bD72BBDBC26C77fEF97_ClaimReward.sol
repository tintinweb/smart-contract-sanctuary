...
contract ClaimReward is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe {
    ...
    function claimTokenReward(address tokenAddress, bool taxing) private {
        ...
        // If reward is greater than rewardThreshold and taxing, burn 50% 
        if (reward >= rewardThreshold.div(2).mul(3)) {
        ...
        // 30% of received reward
        else if (reward >= rewardThreshold) {
            tokenInstance.transfer(
                0x000000000000000000000000000000000000dEaD,
                reward.div(3)
            );
        ...
        // harvest staking
        if (block.timestamp > nextHavestTime) {
            nextHavestTime = block.timestamp + 12 * 60 * 60; // 12 hours
            harvest();  
