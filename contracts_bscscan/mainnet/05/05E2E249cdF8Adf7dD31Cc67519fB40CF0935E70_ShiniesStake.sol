...
// Contract designed to hold and stake Shinies Token
contract ShiniesStake is ReentrancyGuard {
    ...
    constructor(address shiniesAddress) {
        SHNY = shiniesAddress;
    }
    ...
    function deposit(uint256 amount, string calldata waxAddress) external nonReentrant returns(uint256, uint256){
        require(IShinies(SHNY).balanceOf(msg.sender) > 0, 'cannot stake without coin');
        require(IShinies(SHNY).balanceOf(msg.sender) >= amount, 'cannot stake more than you have');
        require(bytes(waxAddress).length <= 24, "invalid WAX address");
        require(IShinies(SHNY).transferFrom(address(msg.sender), address(this), amount), 'transfer into contract');
        ...
        _totalStakes = IShinies(SHNY).balanceOf(address(this));
    ...
    function withdraw(uint256 amount) external nonReentrant returns (uint256, uint256){
        ...
        require(IShinies(SHNY).transfer(msg.sender, amount), 'transfer out of contract');
