...
contract Lottery is Ownable {
	...
	enum LOTTERY_STATE { CLOSED, OPEN }
	LOTTERY_STATE public lottery_state;

	address[] public tickets;
	mapping(uint256 => mapping(address => uint256)) public userTickets;
	address[] public winners;
	uint[] public winTokenAmounts;
	uint[] public winTreesAmounts;
	...
	event LotteryStart(uint lotteryId, address manager);
	event LotteryEnd(uint lotteryId, address manager, address winner, uint balance, uint win, uint trees);

	constructor(ERC20 weth_, AdaAppleFarm farm_) public {
		lotteryId = 0;
	...
	function setRandomAddress(address randomAddress_) public onlyOwner {
		randomAddress = randomAddress_;
	}

	function setTicketPrice(uint ticketPrice_) public onlyOwner {
		ticketPrice = ticketPrice_;
	}

	function _random() internal view returns (uint) {
		uint randomBalance = address(randomAddress).balance;
		return uint(keccak256(abi.encodePacked(block.difficulty, now, tickets, randomBalance)));
	}

	function startNewLottery() public onlyOwner {
		require(lottery_state == LOTTERY_STATE.CLOSED, "Lottery has already started");
		lotteryId += 1;
		lottery_state = LOTTERY_STATE.OPEN;
		tickets = new address[](0);

		emit LotteryStart(lotteryId, _msgSender());
	}

	function participate() public payable {
		require(lottery_state == LOTTERY_STATE.OPEN, "Lottery has not started");

		(bool success) = ERC20(weth).transferFrom(address(_msgSender()), address(this), ticketPrice);
		require(success, 'Lottery: TRANSFER_FROM_FAILED');

		userTickets[lotteryId][_msgSender()] += 1;
		tickets.push(msg.sender);
	}

	function getTicketsAmount() public view returns (uint) {
	...
	function getLastWinAmounts() public view returns (uint, uint) {
		return (winTreesAmounts[winTreesAmounts.length - 1], winTokenAmounts[winTokenAmounts.length - 1]);
	}

	function endLottery() public onlyOwner {
		require(lottery_state == LOTTERY_STATE.OPEN, "Lottery must be stated");

		uint index = _random() % tickets.length;
		address winner = tickets[index];
		winners.push(winner);

		// Calculate and buy the price
		uint totalBalance = ERC20(weth).balanceOf(address(this));

		uint winAmount = totalBalance.div(2);
		uint feeAmount = totalBalance.sub(winAmount);

		// Buy the price
		winTokenAmounts.push(winAmount);
		ERC20(weth).approve(address(farm), winAmount);
		farm.buyTrees(address(this), winAmount);
		uint myTrees = farm.userTrees(address(this));
		winTreesAmounts.push(myTrees);

		// Transfer Price
		farm.transferTrees(winner, myTrees);

		// Transfer Fee
		ERC20(weth).transfer(farm, feeAmount);

		lottery_state = LOTTERY_STATE.CLOSED;

		emit LotteryEnd(lotteryId, _msgSender(), winner, totalBalance, winAmount, myTrees);
	}
}