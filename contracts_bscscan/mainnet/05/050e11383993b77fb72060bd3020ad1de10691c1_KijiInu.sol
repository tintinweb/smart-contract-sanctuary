...
contract KijiInu is Context, IERC20, Ownable {
    ...
    uint256 private _tRBAndMarketingTotal;

    string private _name = "KijiInu";
    string private _symbol = "KijiInu";
    ...
    uint256 public _rbBoostAndMarketingFee = 10;
    uint256 private _previousRbBoostAndMarketingFee = _rbBoostAndMarketingFee;

    uint256 public _buyTaxFee = 2;
    uint256 public _buyBurnFee = 1;
    uint256 public _buyRbBoostAndMarketingFee = 5;

    uint256 public _sellTaxFee = 3;
    uint256 public _sellBurnFee = 1;
    uint256 public _sellRbBoostAndMarketingFee = 8;

    uint256 public _maxTxAmount = 1 * 10**12 * 10**9;
    uint256 private minimumTokensBeforeSwap = 50000 * 10**9;

    address payable public rbBoostAndMarketing = 0x67106cC715433B6D7E830d509584aad846e54762;
    ...
    address public routerAddressForPancake = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    ...
    constructor () public {
        _rOwned[_msgSender()] = _rTotal;
         ...
         IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(routerAddressForPancake);
    ...
    function totalMarketingAndRoyalBNBBoost() public view returns (uint256) {
        // BNB has  18 decimals!
        return _tRBAndMarketingTotal;
    }
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            // TAX WHEN Buying KijiInu TOKEN
            if(from == uniswapV2Pair){
                removeAllFee();
                _taxFee = _buyTaxFee;
                _burnFee = _buyBurnFee;
                _rbBoostAndMarketingFee = _buyRbBoostAndMarketingFee;


            }
            // TAX WHEN Selling KijiInu TOKEN
            if(to == uniswapV2Pair){
                removeAllFee();
                _taxFee = _sellTaxFee;
                _burnFee = _sellBurnFee;
                _rbBoostAndMarketingFee = _sellRbBoostAndMarketingFee;
            }
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _tRBAndMarketingTotal = _tRBAndMarketingTotal.add(address(this).balance);
        TransferBNB(rbBoostAndMarketing, address(this).balance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_rbBoostAndMarketingFee).div(
            10**2
        );
    }

    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _rbBoostAndMarketingFee == 0) return;

        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousRbBoostAndMarketingFee = _rbBoostAndMarketingFee;

        _taxFee = 0;
        _burnFee = 0;
        _rbBoostAndMarketingFee = 0;
    }

    function restoreAllFee() private {
        _taxFee = _previousTaxFee;
        _burnFee = _previousBurnFee;
        _rbBoostAndMarketingFee = _previousRbBoostAndMarketingFee;
    }
    ...
    function setRbBoostAndMarketingFeePercent(uint256 rbboostmarket) external onlyOwner() {
        _rbBoostAndMarketingFee = rbboostmarket;
    }
    ...
    function setBuyRbBoostAndMarketingFeePercent(uint256 buyrbboostmarket) external onlyOwner() {
        _buyRbBoostAndMarketingFee = buyrbboostmarket;
    }
    ...
    function setSellRbBoostAndMarketingFeePercent(uint256 sellrbboostmarket) external onlyOwner() {
        _sellRbBoostAndMarketingFee = sellrbboostmarket;
    }
    ...
    function Sweeper() external onlyOwner {
        ...
        //used in case of refund when someone mistakenly sends bnb to contract address
    }
