...
contract AIFarmer is Ownable, ReentrancyGuard {
    ...
    uint rewardPerBlock = 18000000000000000;    // 0.018
    ...
    function query_stake(address _addr) external view returns(bool, address, uint, uint, uint, uint, uint) {
    ...
    function settleAndEvenReward(User storage user, address userAddr, uint changeAmount, uint changeSharesRate, bool isAdd, bool isWithdrawUser) private {
        ...
        if (user.amount > 0) {
            uint pending = user.shares.mul(accruedTokenPerShare).sub(user.rewardDebt);    
            if (pending > 0) {
        ...
        user.rewardDebt = user.shares.mul(accruedTokenPerShare);
    }
    ...
    function pendingReward(User storage user) private view returns (uint) {
        ...
        return user.shares.mul(adjustedTokenPerShare).sub(user.rewardDebt);
    }
