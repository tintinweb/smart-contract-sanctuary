...
/**
 ...
 // ETHER auto rewards for holders 
 // Telegram = https://t.me/EtherDogeRise
...
contract EtherDogeRise is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    EtherDogeRiseDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("EtherDogeRise", "Doge") {
        _ETHRewardsFee = 6;
        _liquidityFee = 3;
		_marketingFee = 6;
    	...
    	dividendTracker = new EtherDogeRiseDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
		bulkSender = payable(0x979E44C6253C3B861705621DD0ac37f555025f82);
		marketingWallet = payable(0xB439d0e753E2DDcDd5598775B1d5915CF8999C7E);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "EtherDogeRise: The dividend tracker already has that address");

        EtherDogeRiseDividendTracker newDividendTracker = EtherDogeRiseDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "EtherDogeRise: The new dividend tracker must be owned by the EtherDogeRise token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "EtherDogeRise: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "EtherDogeRise: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "EtherDogeRise: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "EtherDogeRise: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "EtherDogeRise: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "EtherDogeRise: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "EtherDogeRise: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForBUSD(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of EtherDogeRise
            path,
            recipient,
            block.timestamp
        );
...
contract EtherDogeRiseDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("EtherDogeRise_Dividend_Tracker", "EtherDogeRise_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "EtherDogeRise_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "EtherDogeRise_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherDogeRise contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "EtherDogeRise_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EtherDogeRise_Dividend_Tracker: Cannot update claimWait to same value");
