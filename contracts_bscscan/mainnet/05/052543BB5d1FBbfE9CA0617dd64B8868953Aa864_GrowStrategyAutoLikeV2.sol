...
// File contracts/strategies/BaseGrowStrategyV2.sol
pragma solidity 0.8.6;
...
abstract contract BaseGrowStrategyV2 is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @dev SWAP UTILS: the price provider
    ...
    function setSwapUtils(address _SWAP_UTILS) external onlyRole(CONFIGURATOR_ROLE) {
        SWAP_UTILS = _SWAP_UTILS;
    }
    ...
    function _getPairPrice(address pair, uint amount) internal view returns (uint) {
        ...
        uint px0 = ISwapUtils(SWAP_UTILS).tokenPriceInBNB(token0, 1e18);
        uint px1 = ISwapUtils(SWAP_UTILS).tokenPriceInBNB(token1, 1e18);
        uint fairPriceInBNB = sqrtK.mul(2).mul(HomoraMath.sqrt(px0)).div(2**56).mul(HomoraMath.sqrt(px1)).div(2**56);

        return fairPriceInBNB.mul(amount).div(1e18);
    }
...
// File contracts/strategies/GrowStrategyAutoLikeV2.sol
pragma solidity 0.8.6;
...
contract GrowStrategyAutoLikeV2 is BaseGrowStrategyV2 {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function _wantTokenPriceInBNB(uint256 amount) public view override returns (uint256) {
        return _getPairPrice(STAKING_TOKEN, amount);
    }
