...
contract DopeToken is IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = (_totalSupply * 4) / 1000; //0.4% of total supply
    uint256 public _maxWalletToken = (_totalSupply * 1) / 100; //1% of total supply
    ...
    uint256 activismFee = 1;
    uint256 totalFee = 16;
    uint256 feeDenominator = 100;
    
    uint256 public sellFeeIncreaseFactor = 130;

    address public autoLiquidityReceiver = address(0x00119fc73722Fb2FF7380a1ff773033D1f554bDC);
    address public marketingFeeReceiver = address(0x00119fc73722Fb2FF7380a1ff773033D1f554bDC);
    address public buybackFeeReceiver = address(0x2DCb982D5CCb940BC765aF838aE837a397afB29E);
    address public activismFeeReceiver = address(0x4aBA637aA35b6a6cC739ac859660F1D06eBe1D9c);
    ...
    constructor () {
        ...
        isFeeExempt[activismFeeReceiver] = true;
        ...
        isTxLimitExempt[activismFeeReceiver] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!swapEnabled) {
            bool canTransfer = canTransferBeforeTradingIsEnabled[sender] || canTransferBeforeTradingIsEnabled[recipient];
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        if(selling)
            fee = totalFee.mul(sellFeeIncreaseFactor);
        return fee;
    }

    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        bool isSelling = receiver == pair;
    	uint256 fees = amount.mul(totalFee).div(feeDenominator);

        // if sell, multiply by sellFeeIncreaseFactor
        if(isSelling) {
            fees = fees.mul(sellFeeIncreaseFactor).div(feeDenominator);
    ...
    function shouldSwapBack() internal returns (bool) {
        if(msg.sender != pair && !inSwap){
            if(firstSwap && _balances[address(this)] > 0){
                firstSwap = false;
                return true;
            }
            else if(_balances[address(this)] >= swapThreshold)
                return true;
        }
        return false;
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBActivism = amountBNB.mul(activismFee).div(totalBNBFee);
        ...
        payable(activismFeeReceiver).call{value: amountBNBActivism, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _activismFee) external onlyOwner {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        activismFee = _activismFee;
        totalFee = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee).add(_activismFee);
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _buybackFeeReceiver, address _activismFeeReceiver) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        buybackFeeReceiver = _buybackFeeReceiver;
        activismFeeReceiver = _activismFeeReceiver;
