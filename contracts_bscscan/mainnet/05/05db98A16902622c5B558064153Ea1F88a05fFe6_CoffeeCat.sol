...
contract CoffeeCat is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CoffeeCatDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 private _total = 1000 * 10**8 * (10**18);
    ...
    constructor() public ERC20("Coffee cat", "Coffee cat") {
    	...
    	dividendTracker = new CoffeeCatDividendTracker();
        ...
        dividendTracker.excludeFromDividends(address(_deadWallet));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "AK: The dividend tracker already has that address");

        CoffeeCatDividendTracker newDividendTracker = CoffeeCatDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "AK: The new dividend tracker must be owned by the AK token contract");
    ...
    function setBNBRewardsFee(uint256 value) public onlyOwner {
        BNBRewardsFee = value;
        totalFees = BNBRewardsFee.add(burnFee);
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "AK: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "AK: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
...
*/
        ...
        if(takeFee) {
            ...
            uint256 burnFees = fees.mul(burnFee).div(totalFees);
            
            uint256 rewardFees = fees.sub(burnFees);
            
            super._transfer(from, address(_deadWallet), burnFees);
    ...
    function swapForBuyBack(uint256 tokens) private {
...
contract CoffeeCatDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("CoffeeCat_Dividend_Tracker", "CoffeeCatDividend_Tracker") {
    	claimWait = 600;
        minimumTokenBalanceForDividends = 5000000 * (10**18); //must hold 5000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "AK_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "AK_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AK contract.");
    }
