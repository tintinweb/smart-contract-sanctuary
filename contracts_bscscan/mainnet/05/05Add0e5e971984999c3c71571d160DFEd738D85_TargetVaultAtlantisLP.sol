...
import "../../interfaces/targetVaults/protocols/IAtlantisStaking.sol";
import "../TargetVault.sol";

contract TargetVaultAtlantisLP is TargetVault {
     ...
     */
    IAtlantisStaking public atlantisStaking;
    ...
    constructor(
        ...
        atlantisStaking = IAtlantisStaking(_atlantisStaking);
    ...
    function _deposit() internal virtual {
        ...
        if (_balance > 0) {
            ...
            token.safeIncreaseAllowance(address(atlantisStaking), _balance);
            atlantisStaking.deposit(_balance);
    ...
    function withdraw(uint256 _amount) external virtual onlyVault {
        if (_amount > 0) {
            ...
            atlantisStaking.withdraw(_amount);
    ...
    function emergencyWithdrawAll() external virtual onlyOwner {
        ...
        atlantisStaking.withdraw(_balance);
    ...
    function vaultBalance() public view virtual returns (uint256) {
        (uint256 _amount, ) = atlantisStaking.userInfo(address(this));
        return _amount;
    }
    ...
    function vaultToken() public view virtual returns (address) {
        return atlantisStaking.stakedToken();
    }
...
interface IAtlantisStaking {
    function pendingAtlantis(address _user) external view returns (uint256);
