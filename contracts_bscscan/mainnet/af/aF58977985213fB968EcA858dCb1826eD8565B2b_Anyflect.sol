...
contract Anyflect is Auth, IGatewayHook, IAnyflect {
    ...
    function setShares(address from, address to, uint fromBalance, uint toBalance) external override (IAnyflect) authorized {
        //TODO This method should re-scale the shares for the pool weight over all of the tokenPools


            ShareHolder memory shFrom = shareholders[from];
            ShareHolder memory shTo = shareholders[to];
            if (!excludedFrom[from])
                _setSharesToShareholder(shFrom, fromBalance, true, from);
            if (!excludedTo[to])
                _setSharesToShareholder(shTo, toBalance, false, to);
    }

    function _setSharesToShareholder(ShareHolder memory shareholder, uint balance, bool isFrom, address addr) internal {
...
contract EarnHub is IBEP20, Auth {
    ...
    uint256 _totalSupply = 7e13 ether;
    uint256 public _maxTxAmount = _totalSupply; // 0.25%
    uint256 public _swapThreshold = 1000 ether; // TODO Juan math How many tokens to store in the contract before triggering a liquidation event.
