...
contract SajinoMoto is Ownable {
    address public fast;
    address public immutable WBNB;

    constructor(address _fast, address _WBNB) {
        fast = _fast;
        WBNB = _WBNB;
    }
    ...
    function deposit() external payable {
        ...
        IERC20(WBNB).approve(fast, msg.value);
    }
    ...
    function buy(address tokenBase, uint amountBuy, address tokenBuy,  uint amountOutMin) external onlyOwner returns(bool success) {   
        ...
        IFast(fast).fastExactTokensForTokens(
              amountBuy,
              amountOutMin,
              path, 
              address(this),
    ...
    function sell(address token, uint amountSell, uint amountOutMin) external onlyOwner returns(bool success) {   
        ...
        IERC20(token).approve(fast, amountSell);
        address[] memory path;
        path = new address[](2);
        path[0] = token;
        path[1] = WBNB;
    ...
    function setfast(address _newFast) external onlyOwner returns(bool success){
        fast = _newFast;
        return true;
    }
