...
// File contracts/BIFI/interfaces/biswap/IBiswapPair.sol
...
// File contracts/BIFI/interfaces/biswap/IBiswapRouter.sol
...
// File contracts/BIFI/zaps/BeefyZapBiswap.sol
...
contract BeefyZapBiswap {
    ...
    function beefOut (address beefyVault, uint256 withdrawAmount) external {
        (IBeefyVault vault, IBiswapPair pair) = _getVaultPair(beefyVault);
    ...
    function _getVaultPair (address beefyVault) private view returns (IBeefyVault vault, IBiswapPair pair) {
        vault = IBeefyVault(beefyVault);

        try vault.want() returns (address pairAddress) {
            pair = IBiswapPair(pairAddress); // Vault V6
        } catch {
            pair = IBiswapPair(vault.token()); // Vault V5
        }
    ...
    function _getSwapAmount(uint256 investmentA, uint256 reserveA, uint256 reserveB, uint256 swapFee) private view returns (uint256 swapAmount) {
    ...
    function estimateSwap(address beefyVault, address tokenIn, uint256 fullInvestmentIn) public view returns(uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut) {
        (, IBiswapPair pair) = _getVaultPair(beefyVault);
