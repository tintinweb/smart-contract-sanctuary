...
//  All Website: 
//  www.shibjungle.com - 6% Daily Earning for Lifetime
//  www.bnbjungle.com - 3% Daily Earning for Lifetime
//  www.busdjungle.com - 3% Daily Earning for Lifetime
//  www.maticjugle.com - 6% Daily Earning for Lifetime
...
contract SHIBJungle {
    ...
    uint256 public BANNA_TO_HARVEST=1440000;
    uint256 PSN=10000;
    uint256 PSNH=5000;
		...
		if (user.referrer == address(0)) {
			if (users[referrer].deposits.length > 0 && referrer != msg.sender) {
				user.referrer = referrer;
	 	        uint256 reffer_bonus= amount.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
			 	ERC20(shib).transfer(msg.sender, reffer_bonus);
			}
        }
	...
	function planWithdraw() public payable{
		...
		ERC20(shib).transfer(ceoAddress, fee2);
		ERC20(shib).transfer(ceoAddress2, fee-fee2);
		ERC20(shib).transfer(msg.sender,totalAmount);
		emit FeePayed(msg.sender, fee);
    ...
    function sellBananas() public payable{
        ...
        ERC20(shib).transfer(ceoAddress, fee2);
        ERC20(shib).transfer(ceoAddress2, fee-fee2);
        ERC20(shib).transfer(address(msg.sender),SafeMath.sub(bannasvalue,fee));
    }
    ...
    function calculateBananaSell(uint256 bananas) public view returns(uint256){
        return calculateTrade(bananas,marketBananas,ERC20(shib).balanceOf(address(this)));
    }
    ...
    function calcuateMonkeysBuySimple(uint256 eth) public view returns(uint256){
        return calculateMonkeysBuy(eth,ERC20(shib).balanceOf(address(this)));
    }
