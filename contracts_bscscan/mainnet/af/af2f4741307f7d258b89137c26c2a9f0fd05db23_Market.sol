...
contract Market is Initializable, ReentrancyGuard {
    ...
    uint256 internal _piFeeNumerator;
    uint256 internal _piFeeDenominator;

    event RegularFeeUpdated(uint256 numerator, uint256 denominator);
    event PIFeeUpdated(uint256 numerator, uint256 denominator);
    ...
    function initialize(
        address provider,
        address piToken,
        uint256 piFeeNumerator,
        uint256 piFeeDenominator,
        uint256 regularFeeNumerator,
        uint256 regularFeeDenominator
    ) external initializer {
        require(
            piFeeDenominator >= piFeeNumerator && regularFeeDenominator >= regularFeeNumerator,
        ...
        pi = piToken;
        _piFeeNumerator = piFeeNumerator;
        _piFeeDenominator = piFeeDenominator;
     ...
     * @param denominator The fee denominator
     **/
    function updatePiFee(uint256 numerator, uint256 denominator) external onlyMarketAdmin {
        require(denominator >= numerator, MarketErrors.DEMONINATOR_NOT_GREATER_THAN_NUMERATOR);
        _piFeeNumerator = numerator;
        _piFeeDenominator = denominator;
        emit PIFeeUpdated(numerator, denominator);
    }
     ...
     * @dev Get pi fee
     * - external view function
     * @return Pi fee numerator and denominator
     **/
    function getPiFee() external view returns (uint256, uint256) {
        return (_piFeeNumerator, _piFeeDenominator);
    }
    ...
    function _calculateFee(address token, uint256 price) internal view returns (uint256 fee) {
        if (token == pi) {
            fee = ((price * SAFE_NUMBER * _piFeeNumerator) / _piFeeDenominator) / SAFE_NUMBER;
        } else {
            fee =
