...
https://t.me/LITTLEFEGCOINBSC

https://www.littlefegcoin.net/

Since 2021, Hold LITTLE FEG Earn FEG
...
contract DividendDistributor is IDividendDistributor {
    ...
    /* MAINNET ADDRESSES */
    address FEG_TOKEN_ADDR = 0x389999216860AB8E0175387A0c90E5c52522C945;
    ...
    IBEP20 FEG = IBEP20(FEG_TOKEN_ADDR);
...
contract LittleFEG is IBEP20, RSunAuth {
    using SafeMath for uint256;
    ...
    string constant _name = "LITTLE FEG";
    string constant _symbol = "$LITTLEFEG";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100 * 10**9 * (10**_decimals); // 100 billion
    uint256 public _maxSellTxAmount = _totalSupply / 500; // 0.2%
    ...
    uint256 charityFee = 99;
    uint256 totalBuyFee = 1500;

    uint256 additionalSellFee = 1000;
    ...
    uint256 buybackPercentOfBalance = 7; // Percent of BNB holding in wallet to buyback (each)
    uint256 buybackNumOfBuybacks = 3; // Number of consecutive buybacks to trigger
    uint256 buybackThreshold = 100000000000000; // 0,0001
    uint256 buybackTokenSellThreshold = 10000 * (10**_decimals); // 10K Tokens
    ...
    function swapBack(bool shouldBuyback) internal swapping {
            ...
            )
        {
            ...
            uint256 amountBNBCharity = amountBNB.mul(charityFee).div(
                totalBNBFee
            );
            ...
            if (sendMarketingSendFeeSuccess) {
                ...
                emit SendMarketingFeeFailure(
                    ...
                    marketingSendFeeData
                        ...
                        (
                bool sendCharitySendFeeSuccess,
                bytes memory sendCharitySendFeeData
            ) = payable(charityFeeReceiver).call{
                    value: amountBNBCharity,
                    gas: 30000
                }("");

                
            if (sendCharitySendFeeSuccess) {
                emit SendCharityFeeSuccess(
                    amountBNBCharity,
                    sendCharitySendFeeData
                );
            } else {
                emit SendCharityFeeFailure(
                    amountBNBCharity,
                    sendCharitySendFeeData
                );
            }
    ...
    function setFees(
        ...
        totalBuyFee = _charityFee.add(_buybackFee).add(_reflectionFee).add(
            _marketingFee
        );
        feeDenominator = _feeDenominator;
        totalSellFee = _charityFee
            .add(_buybackFee)
            .add(_reflectionFee)
            .add(_marketingFee)
            .add(_extraSellFee);
        require(totalBuyFee <= feeDenominator / 4, "Buy fee too high");
    ...
    event SendCharityFeeSuccess(uint256 amount, bytes data);
    event SendCharityFeeFailure(uint256 amount, bytes data);
}