...
interface IFarm {
    ...
    function pendingPACOCA(uint256 _pid, address user) external view returns(uint256);

    function stakedWantTokens(uint256 _pid, address _user) external view returns(uint256);
}

contract PacocaInvestmentPlan is IInvestmentPlan, PancakeSwapHelper, EmergencyWithdrawable {
    IFarm public farm;
    IBEP20 public token;
    ...
    uint256 public minBalanceForProfitWithdrawal;
    uint256 public withdrawPeriod = 2 hours;

    event Withdrawn(uint256 tokenAmount, uint256 bnbAmount);
    event Staked(uint256 tokenAmount);

    //For pacoca, farm functions are implemented in the token contract, hence token and farm will be the same: 0x55671114d774ee99d653d6c12460c780a67f1d18
    constructor(IBEP20 _token, IFarm _farm, address routerAddress, address profitDestination, uint256 previousTotalProfits) PancakeSwapHelper(routerAddress) {
        setOptions(_token, _farm, 10000 wei);
    ...
    function stakePacocaBalance() external onlyAdmins {
    ...
    function pendingProfits() external view returns(uint256) {
        return farm.pendingPACOCA(0, address(this));
    }

    function setOptions(IBEP20 _token, IFarm _farm, uint256 _minBalanceForProfitWithdrawal) public onlyOwner {
        require(address(_farm) != address(0), "PacocaInvestmentPlan: Invalid address");
        require(address(_token) != address(0), "PacocaInvestmentPlan: Invalid address");

        token = _token;
        farm = _farm;
        minBalanceForProfitWithdrawal = _minBalanceForProfitWithdrawal;
        token.approve(address(_farm), ~uint256(0));
    }

    function setProfitsDestination(address destination) public onlyOwner {
        require(destination != address(0), "PacocaInvestmentPlan: Invalid address");
    ...
    function doWithdrawProfits() private {
        require(profitsDestination != address(0), "PacocaInvestmentPlan: Not enabled");
