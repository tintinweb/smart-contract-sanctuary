...
// File interfaces/IFarmMaster.sol
...
abstract contract StrategyV3 is Ownable, ReentrancyGuard, Pausable {
    ...
    function _farm() internal virtual {
        ...
        IFarmMaster(farmContractAddress).deposit(pid, wantAmt);
    }

    function _unfarm(uint256 _wantAmt) internal virtual {
        IFarmMaster(farmContractAddress).withdraw(pid, _wantAmt);
    }
    ...
    function buyBack(uint256 _earnedAmt) internal virtual returns (uint256) {
        ...
        if (uniRouterAddress != buybackRouterAddress) {
            ...
            if (wbnbAmt > 0) {
                IERC20(wbnbAddress).safeIncreaseAllowance(buybackRouterAddress, wbnbAmt);
    ...
    function _safeSwap(
        ...
        uint256[] memory amounts = IXRouter02(_uniRouterAddress).getAmountsOut(_amountIn, _path);
        uint256 amountOut = amounts[amounts.length.sub(1)];

        IXRouter02(_uniRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            _amountIn,
...
contract StrategyV3_Thoreum is StrategyV3 {
    ...
    function _farm() internal override virtual {
        ...
        wantLockedTotal = wantLockedTotal.add(wantAmt);
        IERC20(wantAddress).safeIncreaseAllowance(farmContractAddress, wantAmt);
        IThoreumFarm(farmContractAddress).deposit(pid, wantAmt, 0x34Eb1B0ebf95c751298d8DbAD90f40dcB5C52827);
    }
