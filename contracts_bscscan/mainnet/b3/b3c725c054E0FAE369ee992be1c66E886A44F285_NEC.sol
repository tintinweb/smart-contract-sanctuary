...
contract NEC is Context, IERC20, Ownable {
    ...
    string private _name = "Nash Equilibrium Contract"; 
    ...
    uint256 public numTokensSellToAddToLiquidity = 1000 * 10**4 * 10**9; 
    address public constant blackHole = 0x300000000000000000000000000000000000deaD;
    address private devReceiver=0x5fe95A1E62e59f5746397E42528277f9d882D3D7;
    address public fomoReceiver;
    uint256 private enterCount = 0;
    uint256 public fomoMin = 5 * 10**6 * 10**9; 
    ...
    uint256[5] public feeFee = [2,4, 6, 8, 10];
    ...
    constructor () public {
        ...
        bnbswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
    ...
    function distributeToken(address[] memory addresses, uint256 _value) public onlyOwner {
      uint256 a = _value * 10**9;
      for (uint i = 0; i < addresses.length; i++) {
         _transferStandard(owner(), addresses[i], a);
      }
    }
    ...
    function _transfer(
        ...
        if((from == uniswapV2Pair || from == bnbswapV2Pair) && from != owner() && to != owner() && to != fomoReceiver)
            require(amount <= _maxTxAmount, "Buy amount exceeds the maxTxAmount.");

        if((to == uniswapV2Pair || to == bnbswapV2Pair) && from != owner() &&  from != address(this) && from != fomoReceiver)
    ...
    function manualSwapAddLiquify(uint amount) external onlyOwner{
