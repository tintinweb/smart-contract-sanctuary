...
contract DM2 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x49b7cD83De6F03bc69257E3d3a9A8e7f282Ef514); // TODO: Set marketing
    ...
    string private _name = 'DogeM2';
    string private _symbol = 'DM2';
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(to == uniswapV2Pair){
                ...
                require(amount <= _addressFees[from].maxBal / 2, "can only sell half of max balance");
        ...
        if(takeFee){
            if(from == uniswapV2Pair){
                ...
                uint256 newMax = balanceOf(to) + amount;
    ...
    function _reflectFee(uint256 rFee, uint256 tFee) private {
        ...
        if(goodBoyAddr != deadAddress && _lastTxn != deadAddress){
            // Both undead
            goodBoyFee = tFee.div(2);
            lastTxnFee = tFee.sub(goodBoyFee);
            // Add half fee each
            remainingFee = 0;
        }else{
            if(goodBoyAddr != deadAddress){
                // Good boy undead but lastTx dead
                // Add half fee to holders
                 goodBoyFee = tFee.div(2);
                 remainingFee = tFee.sub(goodBoyFee);
            }
            if(_lastTxn != deadAddress){
                 // Last tx undead but Good boy dead
                  // Add half fee to holders
                 lastTxnFee = tFee.div(2);
                 remainingFee = tFee.sub(lastTxnFee);
             }
        }
        if(goodBoyFee > 0){
            _tOwned[goodBoyAddr] = _tOwned[goodBoyAddr].add(goodBoyFee);
        }
        if(lastTxnFee > 0){
            _tOwned[_lastTxn] = _tOwned[_lastTxn].add(lastTxnFee);
        }
        // Add remaining to all holders
        if(remainingFee > 0){
            _tFeeTotal = _tFeeTotal.add(remainingFee);
        }
    }
