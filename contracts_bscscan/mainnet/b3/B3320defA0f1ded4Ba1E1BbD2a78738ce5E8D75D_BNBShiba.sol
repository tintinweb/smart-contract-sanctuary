...
contract BNBShiba is Context, IERC20, Ownable, Janitable {
    ...
    function swap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (tokensForLiquidity < contractTokenBalance) {
            ...
            marketingAddress.call{value:bnbForMarketing}("");
    ...
    function rewards(address recipient) public view returns (uint256) {
        ...
        if (brut > _claimed[recipient])
            return brut.sub(_claimed[recipient]).mul(uint256(10**3).sub(_marketingFee)).div(10**3);
        return 0;
    }
    ...
    function claim(address payable recipient) public {
        ...
        if (_boughtTotal > 0) {
            ...
            uint256 brtt = rewards(recipient);
            _claimed[recipient] = _claimed[recipient].add(toclaim);
            (bool success, ) = recipient.call{value:brtt}("");
            (bool success2, ) = marketingAddress.call{value:brut.sub(brtt)}("");
            require(success && success2, "Claim failed.");
