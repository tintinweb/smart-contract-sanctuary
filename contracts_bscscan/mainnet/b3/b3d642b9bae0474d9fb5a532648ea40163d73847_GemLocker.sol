...
contract GemLocker is Initializable, AccessControlUpgradeable, IGemLocker, ReentrancyGuardUpgradeable {
    ...
    /// @dev Residual Gem cut off from stream because of withdrawn stake that will be burned, tracked per day
    mapping(uint256 => uint256) private gemToBurn;

    mapping(address => uint256[]) private clamIdsToRemove;
    mapping(address => uint256[]) private pearlIdsToRemove;
    ...
    event SetFarmingRewardsLockDuration(uint256 duration);
    event SetMaxRewardsLock(uint256 max);
    event UpdateIndividualClamRewards(address indexed account, uint256 amountCorrected);
    event UpdateIndividualPearlRewards(address indexed account, uint256 amountCorrected);

    function initialize(IGemToken _gem) external initializer {
    ...
    function pendingClamRewards(address _user) public view override returns (uint256 totalBonusPending) {
        for (uint256 i = 0; i < clamIdsStakedPerUser[_user].length(); i++) {
            ...
            totalBonusPending = totalBonusPending.add(
                calculateRewards(bonusInfo.lastRewardDay, bonusInfo.endDay, bonusInfo.bonusRemaining)
            );
        }
    }
    ...
    function pendingPearlRewards(address _user) public view override returns (uint256 totalBonusPending) {
        for (uint256 i = 0; i < pearlIdsBurnedPerUser[_user].length(); i++) {
            uint256 pearlId = pearlIdsBurnedPerUser[_user].at(i);
            LockedNftBonus memory bonusInfo = lockedPearlRewards[_user][pearlId];
            totalBonusPending = totalBonusPending.add(
                calculateRewards(bonusInfo.lastRewardDay, bonusInfo.endDay, bonusInfo.bonusRemainingCorrected)
            );
        }
    }
    ...
    function calculateRewards(
        ...
        return _bonusRemaining.mul(passedDays).div(_endDay.sub(_startDay));
    }
            ...
            for (uint256 i = 0; i < clamIdsStakedPerUser[_account].length(); i++) {
                ...
                if (bonusInfo.lastRewardDay < getDay()) {
                    ...
                    if (bonusInfo.bonusRemaining <= rewards) {
                        ...
                        clamIdsToRemove[_account].push(clamId);
                    } else {
    ...
    function unlockPearlRewards(address _account) private returns (uint256 unlockAmount) {
        if (pearlIdsBurnedPerUser[_account].length() == 0 || totalPearlRewardsLocked[_account] == 0) {
            ...
            for (uint256 i = 0; i < pearlIdsBurnedPerUser[_account].length(); i++) {
                ...
                if (bonusInfo.lastRewardDay < getDay()) {
                    ...
                    if (bonusInfo.bonusRemainingCorrected <= rewards) {
                        ...
                        pearlIdsToRemove[_account].push(pearlId);
                    } else {
                        bonusInfo.bonusRemaining = bonusInfo.bonusRemaining.sub(rewards);
            ...
            removePearlIds(_account);
            totalPearlRewardsLocked[_account] = totalPearlRewardsLocked[_account].sub(unlockAmount);
        }
    }
    ...
    /// @notice update pearl rewards based on deposit/withdraw data
    /// @param _account address
    function updatePearlRewards(address _account) private {
        uint256 currentDay = getDay();

        // if day has passed, gemToBurn will not be mutated anymore and can be burned
        if (currentDay != 0 && gemToBurn[currentDay.sub(1)] > 0) gem.burn(gemToBurn[currentDay.sub(1)]);

        StakeData memory userStakeInfo = stakeData[_account][currentDay];
        if (userStakeInfo.withdrawn > 0) {
            uint256 newTotal;
            for (uint256 i = 0; i < pearlIdsBurnedPerUser[_account].length(); i++) {
                uint256 pearlId = pearlIdsBurnedPerUser[_account].at(i);
                LockedNftBonus storage bonusInfo = lockedPearlRewards[_account][pearlId];
                uint256 stakeDiff = userStakeInfo.deposited < userStakeInfo.withdrawn
                    ? userStakeInfo.withdrawn.sub(userStakeInfo.deposited)
                    : 0;
                if (stakeDiff > 0) {
                    uint256 percentageCutBP = stakeDiff.mul(1e12).div(userStakeInfo.totalBalance);
                    bonusInfo.bonusRemainingCorrected = bonusInfo.bonusRemaining.sub(
                        bonusInfo.bonusRemaining.mul(percentageCutBP).div(1e12)
                    );
                } else {
            ...
            if (newTotal <= totalPearlRewardsLocked[_account]) {
                gemToBurn[currentDay] = gemToBurn[currentDay].add(totalPearlRewardsLocked[_account].sub(newTotal));
                totalLocked = totalLocked.sub(totalPearlRewardsLocked[_account].sub(newTotal));
            } else if (gemToBurn[currentDay] > newTotal.sub(totalPearlRewardsLocked[_account])) {
                gemToBurn[currentDay] = gemToBurn[currentDay].sub(newTotal.sub(totalPearlRewardsLocked[_account]));
                totalLocked = totalLocked.add(newTotal.sub(totalPearlRewardsLocked[_account]));
            }
    ...
    function forceUnlock(address _account, uint256 _amount) external override {
        ...
        if (_amount >= totalLockedRewards(_account)) {
            ...
            delete clamIdsStakedPerUser[_account];
            delete totalClamRewardsLocked[_account];

            delete pearlIdsBurnedPerUser[_account];
        ...
        emit ForceUnlocked(_account, transferAmount);
    }

    function removeClamIds(address _account) private {
        if (clamIdsToRemove[_account].length > 0) {
            for (uint256 i = 0; i < clamIdsToRemove[_account].length; i++) {
                clamIdsStakedPerUser[_account].remove(clamIdsToRemove[_account][i]);
            }
            delete clamIdsToRemove[_account];
        }
    }

    function removePearlIds(address _account) private {
        if (pearlIdsToRemove[_account].length > 0) {
            for (uint256 i = 0; i < pearlIdsToRemove[_account].length; i++) {
                pearlIdsBurnedPerUser[_account].remove(pearlIdsToRemove[_account][i]);
            }
            delete pearlIdsToRemove[_account];
        }
    }

    /* Updater functions */

    function changeUserLockedClamRewards(
        address[] memory _accounts,
        uint256[] memory _clamIds,
        uint256[] memory values
    ) external {
        require(hasRole(UPDATER_ROLE, msg.sender), "GemLocker: must have updater role");
        require(_accounts.length == _clamIds.length && _clamIds.length == values.length, "incorrect array length");

        for (uint256 index = 0; index < _accounts.length; index++) {
            lockedClamRewards[_accounts[index]][_clamIds[index]].bonusRemaining = values[index];
            lockedClamRewards[_accounts[index]][_clamIds[index]].bonusRemainingCorrected = values[index];
        }
    }

    function changeUserLockedPearlRewards(
        address[] memory _accounts,
        uint256[] memory _pearlIds,
        uint256[] memory values
    ) external {
        require(hasRole(UPDATER_ROLE, msg.sender), "GemLocker: must have updater role");
        require(_accounts.length == _pearlIds.length && _pearlIds.length == values.length, "incorrect array length");

        for (uint256 index = 0; index < _accounts.length; index++) {
            lockedPearlRewards[_accounts[index]][_pearlIds[index]].bonusRemaining = values[index];
            lockedPearlRewards[_accounts[index]][_pearlIds[index]].bonusRemainingCorrected = values[index];
        }
    }

    function setFarmingRewardsLockDuration(uint256 duration) external {
        require(hasRole(UPDATER_ROLE, msg.sender), "GemLocker: must have updater role");
        farmingRewardsLockDuration = duration;
        emit SetFarmingRewardsLockDuration(duration);
    }

    function setMaxRewardsLock(uint256 max) external {
        require(hasRole(UPDATER_ROLE, msg.sender), "GemLocker: must have updater role");
        maxRewardsLock = max;
        emit SetMaxRewardsLock(max);
    }

    /// @notice Correct individual clam balances to match totalClamRewardsLocked
    /// @dev only use this function if totalClamRewardsLocked is less than individual rewards combined
    function correctIndividualClamBalancesToTotal(address _account) external {
        require(hasRole(UPDATER_ROLE, msg.sender), "GemLocker: must have updater role");
        uint256 amountCorrected = 0;
        uint256 amountRemaining = totalClamRewardsLocked[_account];

        for (uint256 i = clamIdsStakedPerUser[_account].length(); i > 0; i--) {
            uint256 clamId = clamIdsStakedPerUser[_account].at(i.sub(1));
            LockedNftBonus storage bonusInfo = lockedClamRewards[_account][clamId];
            if (bonusInfo.bonusRemaining <= amountRemaining) {
                amountRemaining = amountRemaining.sub(bonusInfo.bonusRemaining);
            } else {
                if (amountRemaining != 0) {
                    amountCorrected = amountCorrected.add(bonusInfo.bonusRemaining).sub(amountRemaining);
                    bonusInfo.bonusRemaining = amountRemaining;
                    amountRemaining = 0;
                } else {
                    clamIdsToRemove[_account].push(clamId);
                    amountCorrected = amountCorrected.add(bonusInfo.bonusRemaining);
                    delete lockedClamRewards[_account][clamId];
                }
            }
        }
        removeClamIds(_account);
        emit UpdateIndividualClamRewards(_account, amountCorrected);
    }

    /// @notice Correct individual pearl balances to match totalPearlRewardsLocked
    /// @dev only use this function if totalPearlRewardsLocked is less than individual rewards combined
    function correctIndividualPearlBalancesToTotal(address _account) external {
        require(hasRole(UPDATER_ROLE, msg.sender), "GemLocker: must have updater role");

        uint256 amountCorrected = 0;
        uint256 amountRemaining = totalPearlRewardsLocked[_account];

        for (uint256 i = pearlIdsBurnedPerUser[_account].length(); i > 0; i--) {
            uint256 pearlId = pearlIdsBurnedPerUser[_account].at(i.sub(1));
            LockedNftBonus storage bonusInfo = lockedPearlRewards[_account][pearlId];
            if (bonusInfo.bonusRemaining <= amountRemaining) {
                amountRemaining = amountRemaining.sub(bonusInfo.bonusRemainingCorrected);
            } else {
        ...
        removePearlIds(_account);
        emit UpdateIndividualPearlRewards(_account, amountCorrected);
    }
}
