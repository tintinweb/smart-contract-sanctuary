...
contract CryptoNiteMarketplace is ReentrancyGuard, Ownable {
    ...
    uint256 public marketFeeForKCS = 100;
    uint256 public marketFeeForToken = 100;
    ...
    constructor(
        ...
        WKCSAddress = WBSCAddress_;
        adminRecoveryAddress = adminRecoveryAddress_;
        payoutAddressMap["CryptoNite"] = padPaymentAddress_;
        tokenAddressMap["CryptoNite"] = padTokenAddress_; // address(0x1e2FbB76c8dAf5a0a8F91388BAc09511F3d7AC62); // 0x1e2fbb76c8daf5a0a8f91388bac09511f3d7ac62
    ...
    public
    {
        ...
        CryptoNiteNFT(nftContract).approve(address(this), _id);
        CryptoNiteNFT(nftContract).transferFrom(msg.sender, address(this), _id);
        listedMap[_id] = true;
        price[_id] = _price;
    ...
    external
    {
        ...
        CryptoNiteNFT(nftContract).transferFrom(address(this), msg.sender, _id);
        listedMap[_id] = false;
    ...
    function giveaway(address _to, uint256 _id, uint256 _price, 
        ...
        if (_id == 0) {
            ...
            CryptoNiteNFT(nftContract).mint(newTokenId, _to, _tokenUri);
            emit Giveaway(msg.sender, _to, newTokenId);
        } else {
            if (listedMap[_id] == false) {
                CryptoNiteNFT(nftContract).transferFrom(msg.sender, _to, _id);
            } else {
                require(ownerMap[_id] == msg.sender, "sender is not owner");
                CryptoNiteNFT(nftContract).transferFrom(address(this), _to, _id);
                listedMap[_id] == false;
            }
            ownerMap[_id] = _to;
    ...
    function burn(uint256 _id) external {
        CryptoNiteNFT(nftContract).burn(_id);
    ...
    function buyNew(uint256 _id, 
        ...
        CryptoNiteNFT(nftContract).mint(newTokenId, msg.sender, _tokenUri);
    ...
    function _validate(uint256 _id) internal view {
        ...
        require(msg.sender != CryptoNiteNFT(nftContract).ownerOf(_id), "Can not buy what you own");
    ...
    function updateListingStatus(uint256 _tokenId, bool shouldBeListed) public returns (bool) {
        require(msg.sender == CryptoNiteNFT(nftContract).ownerOf(_tokenId), "Error, you are not the owner");
    ...
    function createAuction(
        ...
        if (_isNew) {
            ...
            CryptoNiteNFT(nftContract).mint(tokenId, msg.sender, _tokenUri);
        }
        ...
        onlyCreatorOrWinner(tokenId)
    {
        ...
        if(winner == address(0)) {
            CryptoNiteNFT(nftContract).transferFrom(address(this), Creator, tokenId);
            ownerMap[tokenId] = Creator;
        } else {
            CryptoNiteNFT(nftContract).transferFrom(address(this), winner, tokenId);
            if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("KCS")))) {
    ...
    function recoverNFT(uint256 tokenId) external onlyAdminRecovery {
        CryptoNiteNFT(nftContract).transferFrom(
...
contract CryptoNiteNFT is ERC721Enumerable, Ownable {

    constructor() ERC721("CryptoNite NFT", "CryptoNiteNFT") {}
