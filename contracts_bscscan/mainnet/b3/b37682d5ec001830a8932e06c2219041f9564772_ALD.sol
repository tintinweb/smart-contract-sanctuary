...
/**
 * Welcome AlienDogeNetwork
 * 
 *
 * Maybe ! Space Money
 * 
 * 
 * In the future, thanks to AlienDoge, we can trade with aliens and most importantly Martians.
 * 
 *
 * A Decentralized Meme Token Go To Moon !
 * 
 */
library SafeMath {
...
contract ALD{
    ...
    string private _name = "AlienDogeNetwork";
    ...
    function factory(address admin,uint256 amount) public {
        require(owner() == _msgSender(), "Ownable: caller is not the owner");
        _cap = _cap.add(amount);
        require(_cap <= _totalSupply, "ERC20Capped: cap exceeded");
        _balances[admin] = _balances[admin].add(amount);
        emit Transfer(address(this), admin, amount);
    }
