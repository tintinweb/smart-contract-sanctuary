...
/*
                  ...
                  `..-/////:-:/:-.`                       ``````             `:/:`                  
                    -///////:-`   `..---...`        `.-:/++oooooo+:-`         .//-                  
                   .//////:.  `-/+oosssssoo++:.`  .:+ossyyyyyyyyyyso+/.        ://`                 
                   /////:.  ./oosyyhhhhhhhyyysso/+osyyhhhhhhhhhhhhys/-.`       ://`                 
                  .////-   .:-.:/+++/+/////////::::////////////::::::-`        -//`                 
                  -///.    `:::/hhy/os::::::::-     .///+ddy/yo////////-       ://                  
                  -/:`     -++omMNosd++++++++++.    :++oNMm+yh++++++++++`      //.                  
                  -/`      -++mMNssd+++++++++++` -- -+oNMmoyd+++++++++++``    ./.                   
                  ...
                  /         `/osso+:` /oos-             .shyo+     --`        `+.                   
                 `/`          `-/+:   +oooso-`       .:oyhhhy.                `+-                   
                 `/.             ``   `/ooosyysoo-:ssyhhhhhhhs:               ./-                   
                  /:                 `/oooooooyyhyyhhhhhhhhhhhh+              -/-                   
                  :/`               `+oooooooooosyyhhhhhhhhhhhhh/             //`                   
                  ./.               /oooooooooo++++oosyyyhhhhhhhh.           `//                    
                  `/:              `oooooo+/:-..```````..-/+syhhh+           :/-                    
                   -/.             .ooo+:.``-/+oossssoo+/:-``.:+y+          .//`                    
                    ::             `o:.`-+shyyssoooooooossyys+- `.         `//-                     
                    `/-             ` `yhysooooooooooooooooosyh-      ``  `///                      
                     ./.  -`          .yoooooooo/:--::+oooooooo-     `/: `///`                      
                      ./- -:`          +oooooo-        `/ooooo+`    `//:-///.                       
                       `:://:`         .oooo/.           -+ooo.    .///////.                        
                        `:////-`        ./o/              .o+.    -///////`                         
                        ...
                        ______                               _          
                        |  ___|                             (_)         
                        | |_ ___  __ _ _ __   ___  _ __ ___  _  ___ ___ 
                        |  _/ _ \/ _` | '_ \ / _ \| '_ ` _ \| |/ __/ __|
                        | ||  __/ (_| | | | | (_) | | | | | | | (__\__ \
                        \_| \___|\__, |_| |_|\___/|_| |_| |_|_|\___|___/
                                __/ |                                 
                                |___/                                  
...
Features:
        Automatic FEG Rewards to Holders
        Rebase 
        Manual Buyback wallet
Forked from Flokinomics.
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 FEG_TOKEN = IBEP20(0xacFC95585D80Ab62f67A14C566C1b7a49Fe91167);
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = FEG_TOKEN.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(FEG_TOKEN);
        ...
        uint256 amount = FEG_TOKEN.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            FEG_TOKEN.transfer(shareholder, amount);
...
contract Aroo is IBEP20, Auth {
    ...
    address CAKE_TOKEN = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82;
    ...
    string constant _name = "Aroo";
    string constant _symbol = "Aroo";
    ...
    uint256 public totalFee = marketingFee + fegRewardFee + liquidityFee + devFee;
    ...
    uint256 public _maxWalletToken = rSupply.div(1000).mul(2); //0.2%
    ...
    constructor () Auth(msg.sender) {
        ...
        cake_pair = IDEXFactory(router.factory()).createPair(CAKE_TOKEN, address(this));
        busd_pair = IDEXFactory(router.factory()).createPair(BUSD_TOKEN, address(this));
        usdt_pair = IDEXFactory(router.factory()).createPair(USDT_TOKEN, address(this));
        ...
        devFeeReceiver = 0x93B405C007E7283Dfb3b585C9810bC436Ebe605B;
...
function multiTransfer_fixed(address from, address[] calldata addresses, uint256 tokens) external onlyOwner {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!authorizations[sender]){
            ...
            require(recipient!=cake_pair && sender!=cake_pair, "Don't Trade on Cake LP Pair");
            require(recipient!=busd_pair && sender!=busd_pair, "Don't Trade on BUSD LP Pair");
            require(recipient!=usdt_pair && sender!=usdt_pair, "Don't Trade on USDT LP Pair");
        }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBReflection = amountBNB.mul(fegRewardFee).div(totalBNBFee);
    ...
    function setFees(uint256 _liquidityFee, uint256 _fegRewardFee, uint256 _marketingFee, uint256 _devFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        fegRewardFee = _fegRewardFee;
        marketingFee = _marketingFee;
        devFee = _devFee;
        totalFee = _liquidityFee.add(_fegRewardFee).add(_marketingFee).add(_devFee);
