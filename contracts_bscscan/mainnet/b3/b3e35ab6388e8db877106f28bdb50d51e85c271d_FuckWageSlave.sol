...
contract FuckWageSlave is IBEP20, Auth {
    ...
    uint256 private _tTotal = 69 * 10 ** 12 * 10 ** _decimals;
    ...
    string private _name = "FuckWageSlave";
    string private _symbol = "FuckWageSlave";
    ...
    uint256 public totalSwapFee = 10;
    uint256 private _previousSwapFee = totalSwapFee;
    ...
    uint256 public numTokensSellToAddToLiquidity = _tTotal * 1 / 100;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingWallet = 0x2DbF5533140Bb71B8895540F83CE3836d3f17d9D;
    ...
    function disableFees() external authorized {
        marketingFee = 0;
        _liquidityFee = 0;
        _taxFee = 0;
        totalSwapFee = 0;
    ...
    function enableFees() external authorized {
        marketingFee = 7;
        _liquidityFee = 3;
        totalSwapFee = 10;
    ...
    function setSwapThresholdAmount(uint256 SwapThresholdAmount) external authorized {
    ...
    function claimTokens () public authorized {
        // make sure we capture all BNB that may or may not be sent to this contract
        payable(marketingWallet).transfer(address(this).balance);
    }
    
    function claimOtherTokens(IBEP20 tokenAddress, address walletaddress) external authorized {
    ...
    function isBotWallet(address botwallet) external view returns (bool) {
    ...
    // Note that Swap Fee includes liquidity and Marketing
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tFee = calculateTaxFee(tAmount);
        uint256 tLiquidity = calculateSwapFee(tAmount);
    ...
    function calculateSwapFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(totalSwapFee).div(
            10**2
        );
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 amountToLiquify = contractTokenBalance.mul(_liquidityFee).div(totalSwapFee);
        ...
        uint256 totalBNBFee = totalSwapFee.sub(_liquidityFee.div(2));
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if(!canTrade){
            require(sender == autoLiquidityReceiver); // only owner allowed to trade or add liquidity
        }
