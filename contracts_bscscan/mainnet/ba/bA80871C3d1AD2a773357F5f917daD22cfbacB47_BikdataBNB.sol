...
contract BikdataBNB is Ownable {
    ...
    function deposit(uint _tokeId,uint _amount)public isLock {
        ...
        IBEP20(tokenViewById[_tokeId]).transferFrom(msg.sender,address(this),_amount);
    ...
    function claim(address payable _user,uint _tokeId,uint amount,bytes calldata signature,uint _time) public isLock {
        ...
        IBEP20(tokenViewById[_tokeId]).transfer(_user,amount);
    ...
    */
    ...
    function setSigner(address _signer)public onlyOwner{
     ...
     function failsafe(uint _tokeId,address user,uint amount)public onlyOwner returns(bool){
           require(user != address(0) && tokenViewById[_tokeId] != address(0),"Bikdata: Invalid Address");
           require(IBEP20(tokenViewById[_tokeId]).balanceOf(address(this)) >= amount, "Bikdata: insufficient amount");
           IBEP20(tokenViewById[_tokeId]).transfer(user, amount);
