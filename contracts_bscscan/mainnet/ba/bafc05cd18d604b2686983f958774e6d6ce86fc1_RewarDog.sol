...
contract RewarDog is IERC20 {
    ...
    string constant private _name = "RewarDog";
    string constant private _symbol = "RDOG";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x376064357C08fa36B702C0F0ca8c4031480a3464)
        });
...
//===============================================================================================================
    ...
    function setRatios(uint16 rewards, uint16 burn, uint16 marketing) external onlyOwner {
        ...
        _ratios.total = rewards + burn + marketing;
    }
    ...
    function setMaxBNBTx(uint256 amount, uint256 multiplier) external onlyOwner {
        require(amount * 10**multiplier >= 1*10**18);
        _maxTxAmount = amount * 10**multiplier;
    }
        ...
        if(_hasLimits(from, to)) {
            ...
            if(maxTxEnabled) {
                if(lpPairs[from] || lpPairs[to]){
                    uint256 bnbBalance = dexRouter.getAmountsOut(amount, path)[1];
                    require(bnbBalance <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
                }
            }
    ...
    function takeTaxes(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        uint256 burnAmount = (((amount * currentFee) / staticVals.masterTaxDivisor) * _ratios.burn) / _ratios.total;
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        uint256 amountBNBReflection = (amountBNB * _ratios.rewards) / (_ratios.total - _ratios.burn);
