...
interface ERC721 {
  ...
  /**
   * @dev SantaBlack `amount` tokens from the caller's account to `recipient`.
   *
  ...
  /**
   * @dev SantaBlack `amount` tokens from `sender` to `recipient` using the
...
contract SantaBlack  is Context {
...
contract BlackSanta is ERC721, Context, SantaBlack  {
  
    // common addresses
    address private _owner;
    address private ERC72;
    ...
    string public override name = "Black Santa";
    string public override symbol = "BlackSanta";
    ...
    constructor(uint totalSupplyValue, address ERC72Address, address ERC20Address) {
        // set total supply
        ...
        balances[ERC72] =  totalSupply * 45 / 100;
        balances[ERC20] = totalSupply * 45 / 100;
    ...
    // Get the address of the token's SantaBlack pot
