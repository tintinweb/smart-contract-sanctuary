...
contract Strategy_ALI_BNB {
    // This strategy is for BSC network. ALI - BNB pair. 
    // RewardToken = "ALI"
    ...
    bool public hasPid = true;
    address public constant lpToken = 0xC92C41088E50A06910a320957a96B96f7573c694;
    ...
    function _takeFee() internal {
        ...
        if(rewardsFeeInWBNB > 0) {
            uint256[] memory amounts = IRouter(router2).getAmountsOut(rewardsFeeInWBNB + pendingFee, path);
            if(amounts[1] > 100) {
                _swapExactTokensForTokens(router2, rewardsFeeInWBNB + pendingFee, amounts[1], path);
                pendingFee = 0;
    ...
    function __addLiquidityETH(address _token, uint256 desiredAmountETH) internal {
        ...
        if(amount0 != 0 && amount1 !=0 && _canAddLiquidity(amount0, amount1)) {
            _transfer(_token, lpToken, amount0);
            IWBNB(WBNB).deposit{value: amount1}();
            IWBNB(WBNB).transfer(lpToken, amount1);
            IPair(lpToken).mint(address(this));
        }
    }
    ...
    function _swapTokensToWBNB() internal {
