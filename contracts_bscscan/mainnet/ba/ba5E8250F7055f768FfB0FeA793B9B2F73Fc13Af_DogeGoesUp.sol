...
contract DogeGoesUp is Context, IERC20, Ownable, Janitable {
    ...
    string private _name = "DogeGoesUp";
    ...
    event AddeDGUReward(uint256 bnb);
    ...
    function swap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (tokensForLiquidity < contractTokenBalance) {
            ...
            uint256 acquireDGU = address(this).balance.sub(initialBalance);
            // calculate share for liquidity or rewards 
            uint256 bnbForLiquidity = acquireDGU.mul(tokensForLiquidity).div(tokensToSell);
            uint256 bnbForRewards = acquireDGU.sub(bnbForLiquidity);
            // update rewards
