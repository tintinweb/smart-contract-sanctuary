...
contract FundWorker is AccessControl, Lock, Pausable {
    ...
    uint256 public devRewardAmount = 120_000 * 10 ** 18;
    ...
    function collectProfits() external {
        ...
        if(block.timestamp - lastMint > 1 days) {
            if(tokenRewardAmount > 0 ){
                token.mint(msg.sender, tokenRewardAmount);
            }

            if (splitterAddr != address(0) && devRewardAmount > 0) {
                token.mint(address(this), devRewardAmount); 
                token.increaseAllowance(splitterAddr, token.balanceOf(address(this))); 
                IFundProcessor(splitterAddr).processFunds(token, token.balanceOf(address(this)));
            }
    ...
    function setRewards(uint256 reward, uint256 devReward) external onlyRole(MANAGER_ROLE) {
        tokenRewardAmount = reward;
        devRewardAmount = devReward;
    }
...
contract Fund is AbstractFund {
    ...
    mapping(address => uint8) public allowedToClaim;
    ...
    function setClaimAllowance(address _address, uint8 amountPercent) external onlyRole(DEFAULT_ADMIN_ROLE) { 
        require(address(0) != _address, "Address cannot be address(0)");
        allowedToClaim[_address] = amountPercent;
    }
...
library FundWorkerUtils {
    ...
    function pendingCollectableProfits(FundWorkerState memory worker, Fund fund) public view returns (uint256) {
        ...
        while (steps < worker.batchSize) {
            ...
            try strategy.pendingProfit() returns (uint256) {
...
library SwapUtils {
    function swapExactETHForTokens(IUniswapV2Router02 router, ERC20 token, uint256 ethAmount, uint256 slippage) external returns(uint256) {
    ...
    function swapETHForTokens(IUniswapV2Router02 router, ERC20 token, uint256 ethAmount, uint256 slippage) external {
        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(token);
    ...
    function swapTokensForEth(IUniswapV2Router02 router, ERC20 token, uint256 tokenAmount, uint256 slippage) external {
        ...
        token.increaseAllowance(address(router), tokenAmount);
    ...
    function swapTokensForTokens(IUniswapV2Router02 router, ERC20 token0, ERC20 token1, uint256 amount, uint256 slippage) external {
        ...
        token0.increaseAllowance(address(router), amount);

        // make the swap
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            amount,
            minAmount, // accept any amount of Tokens
            path,
            address(this),
            block.timestamp
        );
...
abstract contract AbstractFund is AccessControl, IFund, Lock {
    ...
    uint16 public collectProfitReward = 100;
    ...
    //Should return the vault capital address
    ...
    function addStrategy(address _strategyAddress) external override onlyRole(MANAGER_ROLE) {
        ...
        strategies[_strategyAddress] = FundUtils.StrategyConfiguration({
            capitalAllocation: 0,
            exists: true,
            index: strategyIndex.length()
        });

        strategyIndex.set(strategyIndex.length(), _strategyAddress);
    }

    // Remove a strategy 
    function removeStrategy(address _strategyId, bool keepCapitalAsAssets, bool force) external override onlyRole(MANAGER_ROLE) lock {
        if (!strategies[_strategyId].exists) {
            return;
        }
        uint256 index = strategies[_strategyId].index;
        if (!force) {
    ...
    function updateAllocation(address _strategyId, uint8 allocation) external override onlyRole(MANAGER_ROLE) {
        require(allocation <= 100, "Capital allocation overflow");
        ...
        if (strategyConfig.capitalAllocation >= allocation) {
            ...
            require(strategyConfig.capitalAllocation + unallocatedCapital >= allocation, "Not enough free capital");
    ...
    function getStrategy(uint256 index) external view override returns (address id, bool exists, uint8 allocation) {
        require(index < strategyIndex.length(), "Index out of bounds");
    ...
    function getStrategyByAddress(address _address) external view override returns (uint256 index, bool exists, uint8 allocation) {
        FundUtils.StrategyConfiguration memory strategyConfig = strategies[_address];
        return (
            strategyConfig.index,
            strategyConfig.exists,
            strategyConfig.capitalAllocation
        );
    }
