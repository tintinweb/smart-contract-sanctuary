...
contract PoolAllocator is Auth, ReentrancyGuard {
    ...
    uint marketingFee = 100; // This contract now handles the marketing fee, which is 1% of the allocated to the RBLOs. (So now MKT fee is lowered)
    uint allocatorIncentive = 100;
    uint allocatorAcc = 10000;
    ...
    function distribute() public nonReentrant {
        ...
        require(activePoolCount > 0, "No pools yet to distribute");


        uint bal = address(this).balance;
        uint incentive = address(this).balance * allocatorIncentive / allocatorAcc ;
        uint mkt = marketingFee * address(this).balance / allocatorAcc;

        balanceBeforeDistribution = address(this).balance - incentive - mkt;

        for (uint i = 0; i < exPools.length; i++) {
    ...
    function _liquifyPool(Pool memory _pool) internal {
        ...
        uint acc = 1 ether * 1 ether;
        uint liqToAssign = _pool.alloc * acc / totalAllocs;
        uint bnbToGive = liqToAssign * balanceBeforeDistribution / acc;
        address (_pool.sc).call{value:bnbToGive}("");
        emit LiquifiedPool(bnbToGive);
    }
    ...
    function setAllocatorIncentive(uint incentive, uint mkt, uint denom) external authorized {
        allocatorAcc = denom;
...
contract StakingContract is IReflectionPool, Ownable {
    ...
    mapping (address => ReflectionLocker) public lockers;

    ReflectionLocker[] public lockersArr;
    ...
    SHARKCAKE sCake = SHARKCAKE(payable(0x594D3ebfB1e0c1B48bC1Ab447EAd9fDC82c02db4));
    ...
    function enterStaking(uint256 amount) external {
        ...
        if (address(tokenPool.stakingToken) == address(sCake)) {
            ...
            ReflectionLocker locker;
            if (!lockerExists) {
                locker = lockers[msg.sender];
            } else {
                locker = new ReflectionLocker(msg.sender);
                lockersArr.push(locker); //Stores locker in array
                lockers[msg.sender] = locker; //Stores it in a mapping
                sCake.setIsFeeExempt(address(lockers[msg.sender]), true);
                emit ReflectionLockerCreated(address(locker));
            }
            sCake.transfer(address(lockers[msg.sender]), amount);
        }
...
//        }
    ...
    function harvest() external {
        require(getUnpaidEarnings(msg.sender) > 0, "No earnings ser, village need earnings first");
...
//    IBEP20 safeEarn = IBEP20(0x099f551eA3cb85707cAc6ac507cBc36C96eC64Ff);
    IBEP20 sCake = IBEP20(0x594D3ebfB1e0c1B48bC1Ab447EAd9fDC82c02db4);
    ...
    function unstakeAmount(uint amt) public onlyLockOwner {
        if (amt == 0)
            amt = sCake.balanceOf(address(this));
    ...
    function claimTokens(uint amt) public override onlyLockOwner returns (uint) {
        require(sCake.balanceOf(address (this)) >= amt, "Not enough tokens");
        if (amt == 0) {
            amt = sCake.balanceOf(address(this));
            sCake.transfer(lockOwner, amt);
        }
        else {
            sCake.transfer(lockOwner, amt);
        }
    ...
    function claimAll() public override onlyLockOwner returns (uint, uint) {
        ...
        _transferCake(lockOwner);
        uint amtEarn = sCake.balanceOf(address(this));
        sCake.transfer(lockOwner, amtEarn);
    ...
    function emergencyWithdraw() external onlyLockOwner {
        uint amtEarn = sCake.balanceOf(address(this));
        sCake.transfer(msg.sender, amtEarn);
    }
