...
contract LuckyGoddess is Context, IBEP20, Ownable {
    ...
    //_marketingAddress
    address payable private _marketingWalletAddress = payable(0x853abbd0f2f845a6A0f613656437980F1C11A5f6);
    ...
    string private _name = "Lucky Goddess Inu";
    string private _symbol = "LUCKYGODDESS";
    ...
    uint256 public lotteryThreshold = 1000000000 * 10 ** 9;
    ...
    address private _lottoPotAddress = 0x270DC5391F09258B2A43f281C1bDC876F5aED756;
    ...
    function setMarketinFeePercent(uint256 marketingFee) external onlyOwner() {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tMarketing, data.tLottoFee);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256,TData memory) {
        ...
        return (tTransferAmount, TData(0, tFee, tLiquidity, tMarketing, tLottoFee, 0));
    }
    ...
    function _getRValues(TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rLottoFee = _data.tLottoFee.mul(_data.currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rMarketing).sub(rLottoFee);
    ...
    function addAddress(address tAddress) private {
        if (_addressExists[tAddress])
            return;
        _addressExists[tAddress] = true;
        _addressList.push(tAddress);
    }
    ...
    function _takeLotto(uint256 tLottoFee) private {
        uint256 currentRate = _getRate();
        uint256 rLottoFee = tLottoFee.mul(currentRate);

        _rOwned[_lottoPotAddress] = _rOwned[_lottoPotAddress].add(rLottoFee);
        if (_isExcluded[_lottoPotAddress])
            _tOwned[_lottoPotAddress] = _tOwned[_lottoPotAddress].add(rLottoFee);
    }
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax, lotteries, liquidity fee
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tLottoFee) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
