...
/**
 * @dev NutPieFarm functions that do not require less than the min timelock
 */
interface INutPieFarm {
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
 ...
 * to position this {NutPieTimelock} as the owner of a smart contract, with
 * a multisig or a DAO as the sole proposer.
 *
 * _Available since v3.3._
 */
contract NutPieTimelock is AccessControl, ReentrancyGuard {
    ...
    modifier onlyRole(bytes32 role) {
        require(
            ...
            "NutPieTimelock: sender requires permission"
        );
        _;
    }
    ...
    function scheduleBatch(
        ...
        require(
            ...
            "NutPieTimelock: length mismatch"
        );
        require(
    ...
    function _schedule(bytes32 id, uint256 delay) private {
        require(
            _timestamps[id] == 0,
            "NutPieTimelock: operation already scheduled"
        );
        require(delay >= minDelay, "NutPieTimelock: insufficient delay");
        // solhint-disable-next-line not-rely-on-time
    ...
    function cancel(bytes32 id) public virtual onlyRole(PROPOSER_ROLE) {
        require(
            ...
            "NutPieTimelock: operation cannot be cancelled"
        );
    ...
    function _beforeCall(bytes32 predecessor) private view {
        require(
            ...
            "NutPieTimelock: missing dependency"
        );
    }
    ...
    function _afterCall(bytes32 id) private {
        require(
            isOperationReady(id),
            "NutPieTimelock: operation is not ready"
        );
        _timestamps[id] = _DONE_TIMESTAMP;
    }
    ...
    function _call(
        ...
        require(success, "NutPieTimelock: underlying transaction reverted");
    ...
    function updateMinDelay(uint256 newDelay) external virtual {
        require(
            ...
            "NutPieTimelock: caller must be timelock"
        );
    ...
    function executeSet(
        ...
        _beforeCall(predecessor);
        INutPieFarm(_nutpieFarmAddress).set(_pid, _allocPoint, _depositFeeBP, _withUpdate);
        _afterCall(id);
    }
    ...
    function add(
        ...
        INutPieFarm(_nutpieFarmAddress).add(0, _lpToken, _depositFeeBP, _withUpdate, _strat); // allocPoint = 0. Schedule set (timelocked) to increase allocPoint.
    }
