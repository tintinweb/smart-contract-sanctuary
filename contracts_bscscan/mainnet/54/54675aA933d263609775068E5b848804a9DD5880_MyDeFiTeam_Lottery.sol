...
contract MyDeFiTeam_Lottery {
    ...
    function invest(
        ...
        if( amountCAKE > 0){
            token_CAKE.transferFrom(msg.sender, lotteryWalletAddress, amountCAKE); // send to lottery wallet
            totalInvested_CAKE = totalInvested_CAKE.add(amountCAKE);
            
            deposits[address(msg.sender)] += amountCAKE;
        }
    }
    
    function getMyDeposits() public view returns(uint256) {
