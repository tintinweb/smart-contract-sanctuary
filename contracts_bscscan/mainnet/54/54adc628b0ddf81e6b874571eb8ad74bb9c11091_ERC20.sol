...
contract TesterWILLRUGDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("TesterWILLRUG_Dividend_Tracker", "TesterWILLRUG_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "TesterWILLRUG_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "TesterWILLRUG_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TesterWILLRUG contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TesterWILLRUG_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "TesterWILLRUG_Dividend_Tracker: Cannot update claimWait to same value");
...
contract FreeGas4Life is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
   ...
   TesterWILLRUGDividendTracker public dividendTracker;
    ...
    constructor() ERC20("TesterWILLRUG", "TesterWILLRUG") {
        BNBRewardsFee = 5;
        liquidityFee = 5;
        extraFeeOnSell = 5; // extra fee on sell
        marketingFee = 5;
        marketingWallet = payable(0xa8e55f296cffFD81208Ef3740F9a760C7322095B);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new TesterWILLRUGDividendTracker();
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TesterWILLRUG: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TesterWILLRUG: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TesterWILLRUG: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TesterWILLRUG: Cannot update gasForProcessing to same value");
