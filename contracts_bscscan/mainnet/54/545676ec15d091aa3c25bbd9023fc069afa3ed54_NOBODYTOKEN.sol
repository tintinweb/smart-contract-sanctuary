...
contract NOBODYTOKEN is IERC20, IERC20Metadata, Ownable {
    ...
    uint256 private MAX_SALE_AMOUNT = 500000 * 10 ** _decimals;

    uint256 private MAX_START_BUY = 29900000 * 10 ** _decimals;

    uint256[4] public _saleFees = [10,15,20,25]; // min - max tax

    uint256 public _userFee = 99; // max holding anti-whale
    ...
    constructor(address cOwner) Ownable(cOwner) {
        _name = "NOBODY TOKEN";
        _symbol = "NOBO";
        _totalSupply = 99000000 * 10 ** _decimals;
    ...
    function _transfer(
        ...
        if (isUser(sender) && isUser(recipient)) {
            ...
            if (_dailySales[sender].length == 4) {
                ...
                require(_dailySales[sender][_dailySales[sender].length-1] < block.timestamp-120, "daily sales limit");
