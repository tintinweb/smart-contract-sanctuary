...
contract ADAptor is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ADAptorDividendTracker public dividendTracker;
    ...
    address public _marketingWalletAddress = address(0x26adE4ccCa97A8957582481B1EfbA0d4bcB9d3f4); // marketing wallet
    ...
    constructor() ERC20("ADAptor", "ADAR") {

    	dividendTracker = new ADAptorDividendTracker();
        ...
        airDropLimitLiftDate = block.timestamp + 1 hours; // 1 hour restrictions on Air Drop recipients
        airDropDailySellPerc = 50;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ADAptor: The dividend tracker already has that address");

        ADAptorDividendTracker newDividendTracker = ADAptorDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ADAptor: The new dividend tracker must be owned by the ADAptor token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ADAptor: The router already has that address");
    ...
    function setADARewardsFee(uint256 value) external onlyOwner{
        ...
        ADARewardsFee = value;
        require(ADARewardsFee.add(liquidityFee).add(marketingFee)<= 15, "May not set total fees above 15%");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ADA: The PanADASwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "ADA: airdropToWallets:: Arrays must be the same length");
        require(airdropWallets.length < 40, "ADA: airdropToWallets:: Cannot airdrop more than 40 wallets due to gas limit restrictions");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        ...
        require(newValue != gasForProcessing, "ADAptor: Cannot update gasForProcessing to same value");
...
contract ADAptorDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ADAptor_Dividend_Tracker", "ADAptor_Dividend_Tracker") {
    	claimWait = 1200;
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "ADA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
