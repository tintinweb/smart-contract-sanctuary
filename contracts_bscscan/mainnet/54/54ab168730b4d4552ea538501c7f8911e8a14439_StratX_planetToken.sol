...
contract StratX_planetToken is Ownable, ReentrancyGuard, Pausable {
    ...
    address public constant unirouterplanet =
        address(0x9F088377BcdC220CB0E1Ad15aE6Bc75074beE9F6); //PlanetFinance SWAP
    address public constant masterchef =
        address(0x0ac58Fd25f334975b1B61732CF79564b6200A933);
    ...
    address[] public wbnbToTokenPath;
    ...
    constructor() public {
        ...
        pid = 0xc;
        wbnbToTokenPath = [wbnb, wantAddress];

        if (wantAddress == busd) {
            earnedToTokenPath = [earnedAddress, wbnb, busd];
        } else if (wantAddress != earnedAddress) {
            earnedToTokenPath = [earnedAddress, wbnb, busd, wantAddress];
        }

        IERC20(wantAddress).safeApprove(masterchef, uint256(-1));
        IERC20(earnedAddress).safeApprove(unirouterplanet, uint256(-1));
        IERC20(wbnb).safeApprove(unirouterplanet, uint256(-1));
        IERC20(wantAddress).safeApprove(unirouterplanet, uint256(-1));
    ...
    function _farm() internal {
        ...
        if (pairBal > 0) {
            IPlanetFinance(masterchef).deposit(pid, pairBal);
        } else {
            IPlanetFinance(masterchef).deposit(pid, 0);
        }
    }
        ...
        returns (uint256)
    {
        ...
        if (isAfiComp) {
            if (wantAmt < _wantAmt) {
                IPlanetFinance(masterchef).withdraw(pid, _wantAmt.sub(wantAmt));
            } else {
                wantAmt = _wantAmt;
            }
        }
        ...
        uint256 withdrawalFee = _wantAmt.mul(WITHDRAWAL_FEE).div(
            WITHDRAWAL_MAX
        );
    ...
    function earn() public whenNotPaused {
        ...
        IPlanetRouter01(unirouterplanet)
    ...
    function addLiquidity() internal {
        uint256 earnedAddressHalf = IERC20(wbnb).balanceOf(address(this));
    ...
    function convertDustToEarned() public whenNotPaused {
        ...
        uint256 token0Amt = IERC20(wantAddress).balanceOf(address(this));
        if (wantAddress != earnedAddress && token0Amt > 0) {
    ...
    function pause() public {
        ...
        IERC20(earnedAddress).safeApprove(unirouterplanet, 0);
        IERC20(wbnb).safeApprove(unirouterplanet, 0);
        IERC20(wantAddress).safeApprove(unirouterplanet, 0);
    }
