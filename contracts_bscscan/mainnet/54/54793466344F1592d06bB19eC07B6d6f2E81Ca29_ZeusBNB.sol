...
contract ZeusBNB is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ZeusBNBDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable public operations1Address =
        payable(0x62063f33420F1b5bEcCd35a4C633EBB00a28a7e0);
    address payable public operations2Address =
        payable(0x4fac831F26a9C61af86bF884886eF96EF7cFa7be);

    uint256 public maxSellTransactionAmount = 5**7 * (10**9);
    ...
    uint256 public BNBRewardsSellFee = 25; //1hr 25% 2ndhr 20% 3rdhr 18% 4hr 15%
    ...
    constructor() ERC20("ZeusBNB", "ZBNB") {
        dividendTracker = new ZeusBNBDividendTracker(payable(this));
    ...
    function startBlockSniper() public onlyOwner() {
        maxBuyTransactionAmount = 1**4 * (10**9); // Protect from Sniper
    }
    
    function stopBlockSniper() public onlyOwner() {
        maxBuyTransactionAmount = 1**4 * (10**9);
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        ZeusBNBDividendTracker newDividendTracker = ZeusBNBDividendTracker(
            payable(newAddress)
        );
...
contract ZeusBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "ZeusBNB_Dividend_Tracker",
            "ZeusBNB_Dividend_Tracker"
        )
    {
        zEusBNB = ZeusBNB(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ZeusBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ZeusBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ZeusBNB contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(zEusBNB);

            uint256 prevBalance = zEusBNB.balanceOf(address(this));
            ...
            {
                uint256 received = zEusBNB.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    zEusBNB.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
