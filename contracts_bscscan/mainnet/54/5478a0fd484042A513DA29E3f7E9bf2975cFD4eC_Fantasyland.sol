...
contract DividendPayingToken is ERC20, Ownable, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function distributeDividendsbusd(uint256 amount) public onlyOwner {
...
contract RewardDividendTracker is DividendPayingToken {
  ...
  constructor(address token) DividendPayingToken("bnbbusdReward_Dividend_Tracker", "bnbbusdReward_Dividend_Tracker") {
    claimWait = 1800;
  ...
  function _transfer(address, address, uint256) pure internal override {
    require(false, "bnbbusdReward_Dividend_Tracker: No transfers allowed");
  }

  function withdrawDividend() pure public override {
    require(false, "bnbbusdReward_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main bnbbusdReward contract.");
  }
  ...
  function updateClaimWait(uint256 newClaimWait) external onlyOperator {
    require(newClaimWait >= 1800 && newClaimWait <= 86400, "bnbbusdReward_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
    require(newClaimWait != claimWait, "bnbbusdReward_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Fantasyland is ERC20, Ownable {
  using SafeMath for uint256;

  IUniswapV2Router02 public uniswapV2Router;
  address public uniswapV2Pair;

  address busd = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82;
  address Wbnb = 0x2B3F34e9D4b127797CE6244Ea341a83733ddd6E4;
  ...
  uint256 public busdFee = 800;
  uint256 public accumulatedForbusd;
  uint256 public ethMultiplier = 100;
  uint256 public marketingFee = 300;
  uint256 public liquidityFee = 100;
  uint256 public totalFees = wbnbFee + busdFee + marketingFee + liquidityFee;
  ...
  constructor() ERC20("Fantasyland", "Fantasyland") {
    ...
    */
    _mint(owner(), 100000000000 * (10**uint256(_decimals)));
  }
  ...
  function updateDividendRewardFee(uint256 busd, uint256 wbnb) public onlyOwner {
    require(wbnb + busd <= 3000, "Dividend reward tax must be <= 3000");
    require(busd >= 4 * ethMultiplier, "busd fee must be >= 400");
  ...
  function _transfer(
    ...
    if (from != uniswapV2Pair && enableSwapAndLiquify && contractTokenBalance >= swapTokensAtAmount && !swapping && !_isExcludedFromFees[from]) {
      swapping = true;
      uint256 feeWithoutbusd = contractTokenBalance - accumulatedForbusd;
      uint256 onePart = feeWithoutbusd / (totalFees - busdFee);
      ...
      uint256 bnbForbusd = contractbnbBalance * accumulatedForbusd / swapTobnb;
      uint256 bnbWithoutbusd = contractbnbBalance - bnbForbusd;
      onePart = bnbWithoutbusd / (totalFees - busdFee - (liquidityFee / 2));
      uint256 bnbForLiquidity = onePart * liquidityFee / 2;
      uint256 bnbForMarketing = onePart * marketingFee;
      uint256 bnbForDividend = bnbWithoutbusd - bnbForMarketing - bnbForLiquidity;
    ...
    if(takeFee) {
      ...
      if (from == uniswapV2Pair) {
        ...
        minusbusdFee = ethAmount * ethMultiplier;
      }

      uint256 fees = amount * (totalFees - minusbusdFee) / denom;

      accumulatedForbusd += fees * (busdFee - minusbusdFee) / (totalFees - minusbusdFee);
  ...
  function updateEthMultiplier(uint256 _ethMultiplier) external onlyOwner {
    require(busdFee >= 4 * _ethMultiplier, "Discount cant be greater than busd fee");
