...
contract HARBOUR is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = 0x4870394A50fa49fbbec078a2e84eD127c4096982;
    address payable public expensesAddress = 0xc147f3639bfb2E202FefBbB9bDd98734caA23C6c;
    address payable public devAddress = 0x350E09143707730D483894da4912CA1180A037D3; // Dev Address, used only in phase 1 sells
    ...
    string private _symbol = "HARBOUR";
    ...
    // Innovation for protocol by Harbour Team
    ...
    uint256 public contractInitialization = now;
    ...
    uint256 public _liquidityFee = 10; // 6% will be added pool, 8% will be converted to BNB for a reward
    ...
    uint256 maxTokenNumberToSell = _tTotal.div(10000); // 0.01% max tx amount will trigger swap and add liquidity
    ...
    event MyLog(string log);
    event MyLogInt(uint256 log);
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        bool isBuy = recipient != pancakePair;
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount, isBuy);
    ...
    function setMinTokenNumberToSell(uint256 amount) external onlyOwner() {
        minTokenNumberToSell = amount;
    }

    function setMaxTokenNumberToSell(uint256 amount) external onlyOwner() {
        maxTokenNumberToSell = amount;
    }
    ...
    function _getValues(uint256 tAmount, bool isBuy) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getTValues(tAmount, isBuy);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateLiquidityFee(tAmount, 1);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity);
        return (tTransferAmount, tFee, tLiquidity);
    }

    function _getTValues(uint256 tAmount, bool isBuy) private view returns (uint256, uint256, uint256) {
        uint256 tFee = calculateTaxFee(tAmount);

        uint256 multiplier = getMultiplier(isBuy);

        uint256 tLiquidity = calculateLiquidityFee(tAmount, multiplier);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity);
        return (tTransferAmount, tFee, tLiquidity);
    }
    ...
    function calculateLiquidityFee(uint256 _amount, uint256 multiplier) private view returns (uint256) {
        return _amount.mul(_liquidityFee).div(100).mul(multiplier);
    }

    function getMultiplier(bool isBuy) public view returns (uint256) {
        uint256 multiplier = 1;

        uint256 daysSinceInit = getDaysSinceInit();
    ...
    function getDaysSinceInit() public view returns (uint256) {
        uint256 timeSinceInitialization = uint256(now) - uint256(contractInitialization);
        uint256 dayLength = 86400;
        uint256 daysSinceInitialization = timeSinceInitialization.div(dayLength);
        return daysSinceInitialization;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        uint256 multiplier = getMultiplier(isBuy);
        emit MyLog("multiplier");
        emit MyLogInt(multiplier);
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        require(balanceOf(msg.sender) >= 0, 'Error: must own SHIELD to claim reward');
        ...
        if (reward >= rewardThreshold) {
            ...
            (bool success,) = address(devAddress).call{value : charityamount}("");
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        */
        ...
        !(from == address(this) && to == address(pancakePair)) // swap 1 time
            ...
            // only sell for maxTokenNumberToSell, decouple from _maxTxAmount
            if (contractTokenBalance > maxTokenNumberToSell) {
                contractTokenBalance = maxTokenNumberToSell;
            }
            ...
            distributeTaxes(deltaBalance);
        }
    }

    function distributeTaxes(uint256 deltaBalance) private {
        uint256 bnbToRewards = deltaBalance.mul(4).div(10);
        uint256 bnbToMarketing = deltaBalance.mul(4).div(10);
        uint256 bnbToExpenses = deltaBalance.sub(bnbToRewards).sub(bnbToMarketing);
        uint256 bnbToDev = uint256(0);

        uint256 daysSinceInit = getDaysSinceInit();

        if (daysSinceInit < 2) {
            bnbToRewards = uint256(0);
            bnbToMarketing = deltaBalance.mul(2).div(3);
            bnbToExpenses = uint256(0);
            bnbToDev = deltaBalance.sub(bnbToMarketing);
        } else if (daysSinceInit < 3) {
            bnbToRewards = deltaBalance.div(2);
            bnbToMarketing = deltaBalance.sub(bnbToRewards);
            bnbToExpenses = uint256(0);
            bnbToDev = uint256(0);
        }

        if (bnbToMarketing > 0) {
            transferToAddressETH(marketingAddress, bnbToMarketing);
        }

        if (bnbToExpenses > 0) {
            transferToAddressETH(expensesAddress, bnbToExpenses);
        }

        if (bnbToDev > 0) {
            transferToAddressETH(devAddress, bnbToDev);
        }
    }
    ...
    function changeExpensesAddress(address payable _newaddress) public onlyOwner {
        expensesAddress = _newaddress;
    }
