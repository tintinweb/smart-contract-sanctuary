...
contract MeglaDoge is IBEP20, Ownable
{
    ...
    function _taxedTransfer(address sender, address recipient, uint amount) private{
        ...
        if(isSell){  
            ...
            if(block.timestamp<LaunchTimestamp+SellTaxDuration){
                tax=_getStartTax(SellTaxDuration,sellTax,500);
                }else tax=sellTax;
            }
        else if(isBuy){
            uint BuyTaxDuration=5 minutes;
            if(block.timestamp<LaunchTimestamp+BuyTaxDuration){
                tax=_getStartTax(BuyTaxDuration,buyTax,500);
            }else tax=buyTax;
    ...
    function _getStartTax(uint duration,uint TargetTax, uint maxTax) private view returns (uint){
        uint timeSinceLaunch=block.timestamp-LaunchTimestamp;
        return maxTax-((maxTax-TargetTax)*timeSinceLaunch/duration);
    }
