...
interface IERC20 {
    ...
    function additional(address to, uint256 amount) external ;
}
...
contract UpgradeUnionProperty is Ownable {
    ...
    event UpgradeProperty(address indexed tokenAddress,uint256 indexed unionId,uint256 indexed accountId,uint256 index,uint256 tokenAmount,uint256 contributeAmount);
    
    constructor(address _accountContract,address _unionMemberContract,address _contributeContract){
        accountContract = _accountContract;
        unionMemberContract = _unionMemberContract;
        contributeContract = _contributeContract;
    }
    
    function upgrade(address tokenAddress,uint256 unionId,uint256 accountId,uint256 amount) public {
        require(isOpen,"can not use now");
        require(tokenList.contains(tokenAddress),"can not use this token");
        require(amount>0);
        uint256 ownerAccountId = IERC721(accountContract).tokenOfOwnerByIndex(msg.sender,0);
        uint256 ownerUnionId = IERC721(unionMemberContract).getBelongUnion(ownerAccountId);
        require(ownerAccountId == accountId,"accountId is not owner");
        require(ownerUnionId == unionId,"not in this union");
        
        index ++;
        IERC20(tokenAddress).transferFrom(msg.sender,receiveAddress,amount);
        if(amount/1000 >0){
            IERC20(contributeContract).additional(msg.sender,amount/1000);
        }
        
        emit UpgradeProperty(tokenAddress,unionId,accountId,index,amount,amount/1000);
    }
    ...
    function getBelongUnion(address owner) public view returns (uint256) {
        uint256 accountId = IERC721(accountContract).tokenOfOwnerByIndex(owner,0);
        uint256 unionId = IERC721(unionMemberContract).getBelongUnion(accountId);
        return unionId;
    }
    
    function addToken(address token) public onlyController {
        tokenList.add(token);
    }
    
    function removeToken(address token) public onlyController {
