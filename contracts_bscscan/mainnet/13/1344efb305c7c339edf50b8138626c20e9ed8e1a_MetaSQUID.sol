...
contract MetaSQUID is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal =  10**14 * 10**9;
	...
	uint256 private _minBurnBalance = 1000000;

    string private _name = "MetaSQUID";
    string private _symbol = "MetaSQUID";
    ...
    function enabledTransfer(bool enable) public returns (bool){
        enableTransfer=enable;
        return true;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tBurn = calculateBurnFee(tAmount)/2;
		uint256 tDev = calculateDevFee(tAmount)/2;

        uint256 tTransferAmount = tAmount.sub(tFee).sub(tBurn).sub(tDev);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tBurn, tDev, 0));
    }
    ...
    function _getRValues( TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rFee = _data.tFee.mul(_data.currentRate)/2;
        uint256 rLiquidity = 0;
        uint256 rBurn = _data.tBurn.mul(_data.currentRate)/2;
		uint256 rDev = _data.tDev.mul(_data.currentRate)/2;
    ...
    function _takeBurn(uint256 tBurn) private {
		...
		_burnWalletAddress = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;
    ...
    function _transfer(
        ...
        require(enableTransfer == true, "Transfer should be enabled");
