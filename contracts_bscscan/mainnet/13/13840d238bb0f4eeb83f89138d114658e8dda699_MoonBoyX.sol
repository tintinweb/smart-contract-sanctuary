...
contract MoonBoyX is Context, IERC20, Ownable {
    ...
    address payable marketingAddress = payable(0x4aB948048beac16FDBc2A5AD0286B53f6a7A224D); 

    uint256 private constant MAX = ~uint256(0);
    string private _name = "MoonBoyX";
    string private _symbol = "MoonBoyX";
    ...
    uint256 private _marketingDivisor = 30;
    ...
    function includeInReward(address account) external onlyOwner() {
     ...
     function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
        _maxWalletToken = (_tTotal * maxWallPercent ) / 1000;
    }
    
    function setTokentoLP(uint256 TokentoLP) external onlyOwner() {
        numTokensSellToAddToLiquidity = TokentoLP;
    }
    ...
    function _transfer(
        ...
        // max wallet code
        if (to != address(this)  && to != address(deadAddress) && to != uniswapV2Pair && to != marketingAddress){
        ...
        if (
            ...
            //add liquidity
            if (_snlTotal.div(_swapTotal.add(1)) < (_liquidityFee.sub(_marketingDivisor)).div(_marketingDivisor))
            {
