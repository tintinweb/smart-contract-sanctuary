...
contract BabyGhozali is Context, IERC20, Ownable {
    ...
    string private _name = "Baby Ghozali";
    string private _symbol = "BGH";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000000 * 10**9 * 10**_decimals;
    
    address payable public marketingAddress = payable(0xb7222834c26041657B726C2b41e111a42b0e2D75);
    address public marketingWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFeesBGH;
    mapping (address => bool) private _isExcludedFromMaxBalanceBGH;

    uint256 private constant _maxFeesBGH = 12;
    uint256 private _totalFeesBGH;
    uint256 private _totalFeesBGHContract;
    uint256 private _burnFeesBGH;
    uint256 private _liquidityFeesBGH;
    uint256 private _marketingFeesBGH;
    
    uint256 private _maxBalanceWalletBGH;
    ...
    constructor () {
        ...
        _isExcludedFromFeesBGH[owner()] = true;
        _isExcludedFromFeesBGH[address(this)] = true;
        
        _isExcludedFromMaxBalanceBGH[owner()] = true;
        _isExcludedFromMaxBalanceBGH[address(this)] = true;
        _isExcludedFromMaxBalanceBGH[uniswapV2Pair] = true;

        _liquidityFeesBGH = 1;
        _marketingFeesBGH = 10;
        _burnFeesBGH = 1;
        _totalFeesBGH = _liquidityFeesBGH.add(_marketingFeesBGH).add(_burnFeesBGH);
        _totalFeesBGHContract = _liquidityFeesBGH.add(_marketingFeesBGH);

        _liquidityThreshhold = 3000 * 10**9 * 10**_decimals;
        _maxBalanceWalletBGH = 1000000 * 10**9 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFeesBGH).add(_marketingFeesBGH) <= _maxFeesBGH, "Fees are too high.");
        _liquidityFeesBGH = newLiquidityFee;
        _totalFeesBGH = _liquidityFeesBGH.add(_marketingFeesBGH).add(_burnFeesBGH);
        _totalFeesBGHContract = _liquidityFeesBGH.add(_marketingFeesBGH);
        emit SetLiquidityFee(_liquidityFeesBGH);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFeesBGH.add(_burnFeesBGH).add(newMarketingFee) <= _maxFeesBGH, "Fees are too high.");
        _marketingFeesBGH = newMarketingFee;
        _totalFeesBGH = _liquidityFeesBGH.add(_marketingFeesBGH).add(_burnFeesBGH);
        _totalFeesBGHContract = _liquidityFeesBGH.add(_marketingFeesBGH);
        emit SetMarketingFee(_marketingFeesBGH);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFeesBGH.add(newBurnFee).add(_marketingFeesBGH) <= _maxFeesBGH, "Fees are too high.");
        _burnFeesBGH = newBurnFee;
        _totalFeesBGH = _liquidityFeesBGH.add(_marketingFeesBGH).add(_burnFeesBGH);
        emit SetBurnFee(_burnFeesBGH);
    }
    ...
    function _transfer(
        ...
        if(
            ...
            !_isExcludedFromMaxBalanceBGH[to]  // is excludedFromMaxBalance
        ){
            require(
                balanceOf(to).add(amount) <= _maxBalanceWalletBGH,
        ...
        if(
            ...
            _totalFeesBGHContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromFeesBGH[from] || _isExcludedFromFeesBGH[to])
            && _totalFeesBGH > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_totalFeesBGHContract).div(100);
            uint256 toBurnAmount = amount.mul(_burnFeesBGH).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFeesBGH).div(_totalFeesBGHContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeesBGH).div(_totalFeesBGHContract);
