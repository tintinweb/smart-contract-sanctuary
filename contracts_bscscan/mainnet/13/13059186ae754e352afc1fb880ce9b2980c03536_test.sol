...
contract test is Context, IERC20, Ownable {
    ...
    function setMarketingDevFeePercent(uint256 Devfee) external onlyOwner {
        _Devfee = 0;
        if(Devfee < 6) {
	        _Devfee = Devfee;
	    }  
    ...
    function calculateLiquidityPlusmarketingFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _marketingFee + _Devfee).div(
            10**2
        );
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _Devfee + _liquidityFee + _marketingFee;
