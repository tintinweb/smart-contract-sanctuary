...
import * as context from "@openzeppelin/contracts/utils/Context.sol";
...
import * as addressSol from "@openzeppelin/contracts/utils/Address.sol";
...
contract Autismcoin is context.Context, IERC20Sol.IERC20, ownable.Ownable {
    ...
    string private _name = "Autismcoin";
    string private _symbol = "AUTISMCOIN";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 2; // redistributed among hodlers
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 7; // added to liquidity 
    uint256 private _previousLiquidityFee = _liquidityFee;
    
    uint256 public _burnFee = 0; // burn fee
    uint256 private _previousBurnFee = _burnFee;
    
    uint256 public charity = 1; // charity fee
    uint256 private _previouscharity = charity;
    
    
    uniswapRouter.IUniswapV2Router02 public immutable uniswapV2Router;
    ...
    constructor (address Charity, address routerAddress) {
        _rOwned[_msgSender()] = _rTotal;
        _charity = Charity;
        uniswapRouter.IUniswapV2Router02 _uniswapV2Router = uniswapRouter.IUniswapV2Router02(routerAddress);
         // Create a uniswap pair for this new token
        uniswapV2Pair = uniswapFactory.IUniswapV2Factory(_uniswapV2Router.factory())
    ...
    function toggleIncludeInFee(address account) public onlyOwner {
        _isIncludedInFee[account] = !_isIncludedInFee[account];
    }

    function toggleExcludeFromFee(address account) public onlyOwner {
    ...
    receive() external payable {
        ...
        msg.sender == address(uniswapRouter.IUniswapV2Router02(uniswapV2Router).WETH()), "No need for bnb here :)");
    }
...
//Messy code to avoid stack to deep :)
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        rTransferAmount = rTransferAmount.sub(calculateBurnFee(tAmount).mul(currentRate)).sub(calculateCharityFee(tAmount).mul(currentRate));
    ...
    function calculateCharityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(charity).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 &&  charity == 0) return;
    ...
    function _transfer(
        ...
        } else if(from == uniswapV2Pair || from == address(uniswapV2Router)){
            require(amount <= _maxTxAmount, "Buy amount exceeds the maxTxAmount."); 
        }
        ...
        if (
           ...
           if (balanceOf(uniswapV2Pair) >= 2 && IERC20Sol.IERC20(uniswapV2Router.WETH()).balanceOf(uniswapV2Pair) > 0)
            {        
                //add liquidity
        ...
        if((_isIncludedInFee[from] && !_isExcludedFromFee[to]) || (_isIncludedInFee[to] && !_isExcludedFromFee[from])){
            takeFee = true;
        }
