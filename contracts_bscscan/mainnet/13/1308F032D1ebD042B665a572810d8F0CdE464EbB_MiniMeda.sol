...
contract MiniMeda is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "MiniMeda";
    string private _symbol = "MiniMeda";
    ...
    // fees
    uint256 public _taxFee = 600; // 100 = 1% [PRESALE]
    uint256 public _charityFee = 100; // 100 = 1% [PRESALE]
    uint256 public _burnFee = 0; // 100 = 1% [PRESALE] 
    uint256 public _liquidityFee = 100; // 100 = 1% [PRESALE]
    uint256 public _marketingFee = 400; // 100 = 1% [PRESALE]
    uint256 public _earlySellFee = 300;
    //var
    ...
    function excludeAccount(address account) public onlyOwner {
        require(account != routerAddress,"MiniMeda: Uniswap router cannot be excluded.");
        require(account != address(this), 'MiniMeda: The contract it self cannot be excluded');
        require(!_isExcluded[account], "MiniMeda: Account is already excluded");
    ...
    function includeAccount(address account) public onlyOwner {
        require(_isExcluded[account], "MiniMeda: Account is already included");
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(signatory != address(0), "MiniMeda::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "MiniMeda::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "MiniMeda::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "MiniMeda::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "MiniMeda::_writeCheckpoint: block number exceeds 32 bits");
