...
/**
 ...
 @MicroFootball It will be a stealth project to avoid sniper and bot buying.

 DIVIDEND YIELD PAID IN ADA! Without claim it.
 Simply hold MFT$ and you'll receive ADA token automatically in your wallet.
 
 Hold MFT$ and get rewarded in ADA token on every transaction!

 Telegram: https://t.me/MicroFootball
 
 Website: https://MicroFootball.xyz
...
contract MicroFootbal is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("MicroFootball", "MFT$") {
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "MicroFootball: The marketing wallet is already this address");
    ...
    function updatefirstDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(firstDividendTracker), "MicroFootball: The dividend tracker already has that address");
 
        FIRSTDividendTracker newfirstDividendTracker = FIRSTDividendTracker(payable(newAddress));
 
        require(newfirstDividendTracker.owner() == address(this), "MicroFootball: The new dividend tracker must be owned by the MicroFootball token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "MicroFootball: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "MicroFootball: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MicroFootball: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "MicroFootball: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "MicroFootball: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "MicroFootball: Trading has not started yet");
...
contract FIRSTDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("MicroFootball_Dividend_Tracker", "MicroFootball_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 100000 * (10**9); //must hold 100000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "MicroFootball_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "MicroFootball_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MicroFootball contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MicroFootball_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MicroFootball_Dividend_Tracker: Cannot update claimWait to same value");
