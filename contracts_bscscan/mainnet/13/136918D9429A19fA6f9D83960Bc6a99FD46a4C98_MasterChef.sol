...
// File: contracts\flatten\RocketToken.sol
...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    uint256 constant MAXSUPPLY = 200 ether;
...
contract RocketToken is BEP20("Rocket Token", "Rocket") {
    ...
    /// @dev overrides transfer function to meet tokenomics of Rocket
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (recipient == BURN_ADDRESS) {
            ...
            require(amount == sendAmount + burnAmount, "Rocket::transfer: Burn value invalid");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Initial emission rate: 1 Rocket per block.
    uint256 public constant INITIAL_EMISSION_RATE = 1 ether;
    // Minimum emission rate: 0.001 Rocket per block.
    ...
    // Rocket referral contract address.
    IRocketReferral public RocketReferral;
    ...
    constructor(RocketToken _Rocket, uint256 _startBlock) public {
        Rocket = _Rocket;
        startBlock = _startBlock;

        devAddress = msg.sender;
        feeAddress = msg.sender;
        RocketPerBlock = INITIAL_EMISSION_RATE;
    }
    ...
    // Update the given pool's Rocket allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    function pendingRocket(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 RocketReward =
                multiplier.mul(RocketPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accRocketPerShare = accRocketPerShare.add(
                RocketReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        Rocket.mint(devAddress, RocketReward.div(10));
        Rocket.mint(address(this), RocketReward);
        pool.accRocketPerShare = pool.accRocketPerShare.add(
            RocketReward.mul(1e12).div(lpSupply)
        );
        uint256 devReward = RocketReward.div(10);
        uint256 totalRewards = Rocket.totalSupply().add(devReward).add(
            RocketReward
        );

        if (totalRewards <= Rocket.maxSupply()) {
            // mint as normal as not at maxSupply
            Rocket.mint(devAddress, RocketReward.div(10));
            Rocket.mint(address(this), RocketReward);
        } else {
            // mint the difference only to MC, update RocketReward
            RocketReward = Rocket.maxSupply().sub(Rocket.totalSupply());
            Rocket.mint(address(this), RocketReward);
        }
    ...
    function deposit(
        ...
        if (
            _amount > 0 &&
            address(RocketReferral) != address(0) &&
            _referrer != address(0) &&
            _referrer != msg.sender
        ) {
            RocketReferral.recordReferral(msg.sender, _referrer);
        }
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accRocketPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(Rocket)) {
    ...
    function safeRocketTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > RocketBal) {
            transferSuccess = Rocket.transfer(_to, RocketBal);
        } else {
            transferSuccess = Rocket.transfer(_to, _amount);
        }
        require(transferSuccess, "safeRocketTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate() public {
        ...
        require(
            RocketPerBlock > MINIMUM_EMISSION_RATE,
        ...
        uint256 newEmissionRate = RocketPerBlock;
        for (
        ...
        newEmissionRate = newEmissionRate < MINIMUM_EMISSION_RATE
            ...
            : newEmissionRate;
        if (newEmissionRate >= RocketPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = RocketPerBlock;
        RocketPerBlock = newEmissionRate;
    ...
    // Update the Rocket referral contract address by the owner
    function setRocketReferral(IRocketReferral _RocketReferral) public onlyOwner {
        RocketReferral = _RocketReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (
            address(RocketReferral) != address(0) && referralCommissionRate > 0
        ) {
            address referrer = RocketReferral.getReferrer(_user);
            uint256 commissionAmount =
                _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                Rocket.mint(referrer, commissionAmount);
