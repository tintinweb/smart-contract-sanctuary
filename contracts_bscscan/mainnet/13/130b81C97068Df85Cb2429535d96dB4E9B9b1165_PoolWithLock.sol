...
contract PoolWithLock is ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    ...
    uint256 public minimumLockingPeriod = 604800; // 1 week
    ...
    function unlockedAt(address account) public view returns (uint256) {
        return _locks[account].add(minimumLockingPeriod);
    }
        ...
        updateReward(msg.sender)
    {
        ...
        _locks[msg.sender] = block.timestamp;
        ...
        updateReward(msg.sender)
    {
        ...
        require(unlockedAt(msg.sender) <= block.timestamp, "Too early!");
    ...
    function setMinimumLockingPeriod(uint256 _minimumLockingPeriod) external restricted {
        minimumLockingPeriod = _minimumLockingPeriod;
        emit MinimumLockingPeriodUpdated(minimumLockingPeriod);
    }
    ...
    event MinimumLockingPeriodUpdated(uint256 lockingPeriod);
