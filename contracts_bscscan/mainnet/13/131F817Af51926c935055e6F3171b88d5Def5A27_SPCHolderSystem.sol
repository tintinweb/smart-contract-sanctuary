...
contract SPCHolderSystem is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accTokenPerShare; // Accumulated SPECs per share, times 1e18. See below.
    }
    ...
    uint256 public rewardPerTokenDecimal = 5;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 tokenReward = multiplier.mul(rewardPerToken).mul(1e18).div(10 ** rewardPerTokenDecimal).mul(pool.allocPoint).div(totalAllocPoint).div(864000);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerToken).mul(1e18).div(10 ** rewardPerTokenDecimal).mul(pool.allocPoint).div(totalAllocPoint).div(864000);
        pool.accTokenPerShare = pool.accTokenPerShare.add(tokenReward);
        pool.lastRewardBlock = block.number;
    }
    ...
    function updateEmissionRate(uint256 _rewardPerToken) public onlyOwner {
        massUpdatePools();
        rewardPerToken = _rewardPerToken;
    }
    ...
    function rewardPerBlock() public view returns(uint256) {
        PoolInfo storage pool = poolInfo[0];
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        return lpSupply.mul(rewardPerToken).div(10 ** rewardPerTokenDecimal).div(864000);
    }
}