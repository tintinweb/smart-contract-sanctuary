...
contract SafeBNB is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SafeBNBDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable public operations1Address =
        payable(0x18a4FD284E24232479646f138dac92cF42E3d370); 
    address payable public operations2Address =
        payable(0x529a57a9Ea1bB603e50de8d141e530E3e501662A);

    uint256 public maxSellTransactionAmount = 10**15 * (10**9);
    uint256 public maxSellTransactionAmount2 = 10**15 * (10**9);
    uint256 public maxBuyTransactionAmount = 10**15 * (10**9);
    uint256 public swapTokensAtAmount = 10000000000 * (10**9);

    uint256 public devFees = 5; // Dev 2% from here, Marketing is 3% 
    uint256 public liquidityFee = 0;
    uint256 public BNBRewardsBuyFee = 5;
    uint256 public BNBRewardsSellFee = 15;
    
    
    uint256 public snipeBNBRewardsBuyFee = 0;
    uint256 public snipeBNBRewardsSellFee = 0;
    ...
    constructor() ERC20("SafeBNB", "SafeBNB") {
        dividendTracker = new SafeBNBDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        SafeBNBDividendTracker newDividendTracker = SafeBNBDividendTracker(
            payable(newAddress)
        );
    ...
    function updateFees(
        ...
        // require(
        //     BNBRewardsBuy >= 0 && BNBRewardsBuy <= 40,
        //     "BNB reward fees must be between 0 and 40"
        // );
        // require(
        //     BNBRewardsSell >= 5 && BNBRewardsSell <= 40,
        //     "BNB reward fees must be between 5 and 40"
        // );
...
contract SafeBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "SafeBNB_Dividend_Tracker",
            "SafeBNB_Dividend_Tracker"
        )
    {
        safeBNB = SafeBNB(mainContract);
    ...
    function _transfer(
        ...
        require(false, "SafeBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SafeBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SafeBNB contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(safeBNB);

            uint256 prevBalance = safeBNB.balanceOf(address(this));
            ...
            {
                uint256 received = safeBNB.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    safeBNB.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
