...
/**
FIRST AUTOMATIC BANCOR REWARDS TOKEN IS HERE

$AUTOBANCOR💰 is a frictionless yield farming and liquidity generation protocol. 
Just hold your $AUTOBANCOR token in your wallet and you will earn more. In addition,
you will automatically get $BANCOR Rewards Every 8 hours in the $AUTOBANCOR storage wallet.


Website: https://autobancor.co
Twitter: https://www.twitter.com/AutoBancor
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0xA069008A669e2Af00a86673D9D584cfb524A42Cc;
...
contract AutoBancor is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public _dividendToken = 0xA069008A669e2Af00a86673D9D584cfb524A42Cc;
    ...
    AutoBancorDividendTracker public dividendTracker;
    ...
    constructor() ERC20("AutoBancor", "AutoBancor") {
    	...
    	dividendTracker = new AutoBancorDividendTracker();

    	buyBackWallet = 0x8B1d31b5B24D7bf8B81dF7184EeA0E2A6B7Dde65;
    	
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //0xB5FBcCBea72E077CD61C95887Acb4c7Bd22Be0B2
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "AutoBancor: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "AutoBancor: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "AutoBancor: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "AutoBancor: The dividend tracker already has that address");

        AutoBancorDividendTracker newDividendTracker = AutoBancorDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "AutoBancor: The new dividend tracker must be owned by the AutoBancor token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "AutoBancor: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "AutoBancor: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "AutoBancor: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "AutoBancor: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "AutoBancor: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "AutoBancor: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "AutoBancor: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "AutoBancor: Cannot update gasForProcessing to same value");
...
contract AutoBancorDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("AutoBancor_Dividend_Tracker", "AutoBancor_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "AutoBancor_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "AutoBancor_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AutoBancor contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "AutoBancor_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "AutoBancor_Dividend_Tracker: Cannot update claimWait to same value");
