...
Stealth launching with low market cap.
...
contract BlueChampion is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    address public marketingAddress = 0x2cA558b333B4976582011882e969F97266151c23;
    ...
    uint256 private _tTotal = 1_000_000_000 * 10**18; //total supply. 
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "BlueChampion";
    ...
    uint256 public _maxTxAmount = 5_000_000_0 * 10**18; //5%
    uint256 private minimumTokensBeforeSwap = 1000_0 * 10**18;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap 
    {
        ...
        uint256 swapableTokens =contractTokenBalance.sub(half);
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(swapableTokens);
        uint256 newBalance = address(this).balance.sub(initialBalance);
        addLiquidity(half, newBalance);
        emit SwapAndLiquify(half, newBalance, swapableTokens);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private 
    {
        ...
        uint256 marketingToken = amount.div(100).mul(_marketingFee);
        amount = amount.sub(marketingToken);
        ...
        {
            removeAllFee();
            _transferStandard(sender, marketingAddress, marketingToken);
        }
        restoreAllFee();
    }
    ...
    function setFeePercentages(uint256 liquidityFee, uint256 marketingFee)
    external onlyOwner() 
    { 
...
//////--------  ANTI WHALE -------/////--------
    ...
    uint256 public maxLimit =  _tTotal.div(100).mul(5); //5%
