...
contract DaughterDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DaughterDogeDividendTracker public dividendTracker;
    ...
    uint256 public maxWalletToken = 1000000000000 * (10**18);
    ...
    constructor() ERC20("Daughter Doge", "DDOGE") {
        ...
        dividendTracker = new DaughterDogeDividendTracker();
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "DaughterDoge: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DaughterDoge: The dividend tracker already has that address");

        DaughterDogeDividendTracker newDividendTracker = DaughterDogeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DaughterDoge: The new dividend tracker must be owned by the token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "DaughterDoge: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DaughterDoge: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DaughterDoge: Account is already the value of 'excluded'");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "DaughterDoge: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DaughterDoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DaughterDoge: Cannot update gasForProcessing to same value");
...
contract DaughterDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DaughterDoge_Dividend_Tracker", "DaughterDoge_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DaughterDoge_Dividend_Tracker: No transfers allowed");
    }


    function withdrawDividend() pure public override {
        require(false, "DaughterDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DaughterDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DaughterDoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DaughterDoge_Dividend_Tracker: Cannot update claimWait to same value");
