...
contract WATERFALL is Context, BEP20, Ownable {
    ...
    function _getsValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 stFee, uint256 stLiquidity, uint256 stMarketing) = _getsTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 srFee) = _getsRValues(tAmount, stFee, stLiquidity, stMarketing, _getRate());
        return (rAmount, rTransferAmount, srFee, tTransferAmount, stFee, stLiquidity, stMarketing);
    }
    ...
    function _getsTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(stFee).sub(stLiquidity).sub(stMarketing);
        return (tTransferAmount, stFee, stLiquidity, stMarketing);
    }
    ...
    function _getsRValues(uint256 tAmount, uint256 stFee, uint256 stLiquidity, uint256 stMarketing, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(srFee).sub(srLiquidity).sub(srMarketing);
        return (rAmount, rTransferAmount, srFee);
    }
    ...
    function _takesMarketing(uint256 stMarketing) private {
        uint256 currentRate =  _getRate();
        uint256 srMarketing = stMarketing.mul(currentRate);
        _rOwned[_mWA] = _rOwned[_mWA].add(srMarketing);
        if(_isExcluded[_mWA])
    ...
    function _transfersStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 srFee, uint256 tTransferAmount, uint256 stFee, uint256 stLiquidity, uint256 stMarketing) = _getsValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takesLiquidity(stLiquidity);
        _takesMarketing(stMarketing);
