...
contract TriskelionTokenSale is owned {
	...
	uint public start = 1630454400;
	uint public last = 1661990400;
	...
	function BuyTriskelion() SaleOnGoing payable public {
		...
		if(now < start)  {
			...
			BonusTokens = tokens.div(100).mul(bonus_1);
		} else if(now >= start && now < start + 28 days) { 	// 3rd-4th week
			BonusTokens = tokens.div(100).mul(bonus_2);
		} else if(now >= start && now < start + 42 days) { 	// 5th-6th week
			BonusTokens = tokens.div(100).mul(bonus_3);
		} else if(now >= start && now < start + 56 days) { 	// 7th-8th week
			BonusTokens = tokens.div(100).mul(bonus_4);
		} else if(now >= start && now < start + 77 days) { 	// 9th-11th week
			BonusTokens = tokens.div(100).mul(bonus_5);
		} 
		
		uint amountTobuy = msg.value;
		uint TotalWithBonus = tokens.add(BonusTokens);
