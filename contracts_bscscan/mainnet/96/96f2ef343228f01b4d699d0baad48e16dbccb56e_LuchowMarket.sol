...
/** 
 *  SourceUnit: d:\GitWork\LuchowNFT\Contracts\contracts\LuchowMarket.sol
*/
...
////import "./LuchowNFT.sol";
////import "./IBEP20.sol";

interface ILuchowNFT {
...
contract LuchowMarket is Ownable, ERC721Holder {
    using SafeMath for uint256;
	using EnumerableSet for EnumerableSet.AddressSet;
	...
	uint256 public totalEarning; /* Total Luchow Token */
	...
	function initialize(
        ...
        createCollection("LunaChow", "https://ipfs.io/ipfs/QmRxreUQ3uxJAxdK8DyabzFxjSZgx8eFCtEBA3dem3227J", true);		
	}    
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
		bytes memory bytecode = type(LuchowNFT).creationCode;
        ...
        ILuchowNFT(collection).initialize(_name, _uri, msg.sender, adminAddress, bPublic);
    ...
    function list(address _collection, uint256 _token_id, uint256 _price) OnlyItemOwner(_collection,_token_id) public {
		...
		ILuchowNFT nft = ILuchowNFT(_collection);        
    ...
    function delist(uint256 _id) external {        
        ...
        ILuchowNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pairs[_id].token_id);        
    ...
    function buy(uint256 _id) external ItemExists(_id) {
		...
		// transfer Luchow token to adminAddress
		...
		// transfer Luchow token to owner
		...
		ILuchowNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pair.token_id);
	...
	modifier OnlyItemOwner(address tokenAddress, uint256 tokenId){
        ILuchowNFT tokenContract = ILuchowNFT(tokenAddress);
