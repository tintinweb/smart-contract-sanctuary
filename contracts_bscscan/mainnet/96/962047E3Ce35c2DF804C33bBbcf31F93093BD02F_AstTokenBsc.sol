...
// File contracts/bsc/pancake/IPancakeFactory.sol
...
// File contracts/bsc/pancake/IPancakePair.sol
...
// File contracts/bsc/pancake/IPancakeRouter.sol
...
// File contracts/bsc/AstTokenBsc.sol
...
contract AstTokenBsc is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    uint256 public _repurchaseFee = 5;
    ...
    constructor (address pancakeRouter, address _repurchaseManager, address _saleMgr) public {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256/*tSale*/) {
        (uint256 tTransferAmount, uint256 tRepurchase, uint256 tLiquidity, uint256 tSale) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rRepurchase) = _getRValues(tAmount, tRepurchase, tLiquidity, tSale, _getRate());
        return (rAmount, rTransferAmount, rRepurchase, tTransferAmount, tRepurchase, tLiquidity, tSale);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tRepurchase).sub(tLiquidity).sub(tSale);
        return (tTransferAmount, tRepurchase, tLiquidity, tSale);
    }

    function _getRValues(uint256 tAmount, uint256 tRepurchase, uint256 tLiquidity, uint256 tSale, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);

        uint256 rRepurchase = tRepurchase.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rSale = tSale.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rRepurchase).sub(rLiquidity).sub(rSale);
    ...
    function _takeSaleFee(uint256 tSale) private {
        uint256 currentRate = _getRate();
        uint256 rSale = tSale.mul(currentRate);
        //repurchase + liquidity + sale
        _rOwned[address(this)] = _rOwned[address(this)].add(rSale);
        if (_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tSale);
    }
    ...
    function calculateSaleFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_saleFee).div(
            10 ** 2
        );
    }

    function removeAllFee() private {
        if (_repurchaseFee == 0 && _liquidityFee == 0 && _saleFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        .mul(
            _repurchaseFee.mul(MULTIPLY_FACTOR)
            .add(_liquidityFee.mul(MULTIPLY_FACTOR).div(2))
            .add(_saleFee.mul(MULTIPLY_FACTOR))
        )
        .div(
            _repurchaseFee.mul(MULTIPLY_FACTOR)
            .add(_liquidityFee.mul(MULTIPLY_FACTOR))
            .add(_saleFee.mul(MULTIPLY_FACTOR))
        );
        ...
        uint256 saleEth = soldEth
        .mul(_saleFee.mul(MULTIPLY_FACTOR))
        .div(
            _repurchaseFee.mul(MULTIPLY_FACTOR)
            .add(_liquidityFee.mul(MULTIPLY_FACTOR).div(2))
            .add(_saleFee.mul(MULTIPLY_FACTOR))
        );
        safeTransferETH(saleMgr, saleEth);

        uint256 liquidifyEth = soldEth.sub(repurchasedEth).sub(saleEth);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rRepurchase, uint256 tTransferAmount, uint256 tRepurchase, uint256 tLiquidity,uint256 tSale) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function updateMaxTxAmountAndNumTokensSellToAddToLiquidity(uint256 maxTxAmount_, uint256 numTokensSellToAddToLiquidity_) external onlyOwner {
