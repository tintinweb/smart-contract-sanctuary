...
contract Ownable is Context {
    ...
    address private _previousdevWalletAddress;

    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    event devWalletTransferred(address indexed previousdevWalletAddress, address indexed newdevWalletAddress);

    /**
     * @dev Initializes the contract setting the deployer as the initial owner.
     * @dev Initializes the contract setting the deployer as the dev tax wallet
     */
    constructor () internal {
        address msgSender = _msgSender();
        
        _owner = msgSender;
        _devWalletAddress = msgSender;
        _renouncedOwnership = false;
        
        emit OwnershipTransferred(address(0), msgSender);
        emit devWalletTransferred(address(0), msgSender);
    }
    ...
    /**
     * @dev Set dev wallet address to a new address (`newdevWalletAddress`).
     * Can only be called by the current owner.
     */
    function settaxwAddress(address newdevWalletAddress) public virtual onlyOwner() {
        require(newdevWalletAddress != _devWalletAddress, "Ownable: new dev wallet address is the current dev wallet address");
        emit devWalletTransferred(_devWalletAddress, newdevWalletAddress);
        _devWalletAddress = newdevWalletAddress;
    }
...
contract MainStar is Context, BEP20, Ownable {
    ...
    string private constant _name = "Wormhole";
    string private constant _symbol = "WOL";
    ...
    event devTaxUpdate(uint256 newdevTaxValue);
    ...
    // Exclude an address from reflection fee, liquidity fee, and dev fee.
    ...
    // Set the dev wallet fee percentage
    function setdevFeePercent(uint256 devFee) external onlyOwner() {
        _devFee = devFee;
        emit devTaxUpdate(devFee);
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tdevFee) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tdevFee, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tdevFee);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tdevFee = calculateFee(_devFee, tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tdevFee);
        return (tTransferAmount, tFee, tLiquidity, tdevFee);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tdevFee, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    // Takes dev fee from transfer
    function _takedevFee(uint256 tdevFee) private {
        uint256 currentRate =  _getRate();
        uint256 rdevFee = tdevFee.mul(currentRate);
        
        _rOwned[dev()] = _rOwned[dev()].add(rdevFee);
        
        if (_isExcluded[dev()])
            _tOwned[dev()] = _tOwned[dev()].add(rdevFee);
    }
    
    /*
    * Calculate Reflection Fee, Liquidity Fee, and dev Fee 
    */
    ...
    function removeAllFee() private {
        if (_reflectionTaxFee == 0 && _liquidityFee == 0 && _devFee == 0) return;
    ...
    */
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // transfer amount, it will take tax, burn, liquidity fee and dev fee
    ...
    */
    function withdrawLeftoverBNBTodev(address payable recipient, uint256 amount) public onlyOwner() {
        if (amount <= 0)
    ...
    */
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tdevFee) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        
        _takeFee(tLiquidity, tdevFee, rFee, tFee);
    ...
    // Handles liquidity fee, dev fee and reflection fee. Code was reused, so merged into one function. 
    function _takeFee(uint256 tLiquidity, uint256 tdevFee, uint256 rFee, uint256 tFee) private {
