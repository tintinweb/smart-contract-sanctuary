...
contract UniswapFlashSwapper {
    ...
    function simpleFlashSwapExecute(
        ...
        IERC20(_tokenPay).approve(address(this), amountToRepay);
...
contract ExampleContract is UniswapFlashSwapper, Ownable {
    ...
    function setR1(bool flag) external{
        r1=flag;
    }
    ...
    function execute(address _tokenBorrow, uint _amount, address _tokenPay, uint _amountToRepay, bytes memory _userData) internal {
        ...
        uint allo = token.allowance(address(this), address(apeRouter));

        require(allo==_amount, append("allow problem","_",uint2str(allo),"",""));

        if(r1)
        {
            require(false, append(uint2str(allo),"_",uint2str(_amount),"_",uint2str(_amountToRepay)));
        }
