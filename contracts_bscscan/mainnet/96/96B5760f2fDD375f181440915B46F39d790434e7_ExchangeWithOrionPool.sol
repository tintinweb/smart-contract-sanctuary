...
contract ExchangeWithOrionPool is Exchange, IPoolSwapCallback {
    ...
    function fillThroughOrionPool(
        ...
        bool isInContractTrade = uint(getBalance(path[0], order.senderAddress)) >= tmp.amount_spend;

        try IPoolFunctionality(_orionpoolRouter).doSwapThroughOrionPool(
            isInContractTrade ? address(this) : order.senderAddress,
            ...
            if (isInContractTrade) {
                ...
                require(checkPosition(order.senderAddress), isSeller ? "E1PS" : "E1PB");
    ...
    function _payMatcherFee(
        ...
        require(checkPosition(user), "E1PF");
    ...
    function swapThroughOrionPool(
        ...
        bool isInContractTrade = getBalance(path[0], msg.sender) >= amount_spend;
...
contract ReentrancyGuard {
    ...
    string private constant ERROR_REENTRANT = "E16R";
...
contract OwnableUpgradeSafe is Initializable, ContextUpgradeSafe {
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender(), "E14I");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "E14I");
...
contract Initializable {
  ...
  modifier initializer() {
    require(initializing || isConstructor() || !initialized, "E16I");
...
contract Exchange is OrionVault, ReentrancyGuard {
    ...
    /**
     * @dev Identify if the user is retail or not under the current assumptions (Might change in the future)
     */
    // function _isRetailUser(address user, address asset)
    //     internal
    //     returns (bool isRetail)
    // {
    //     isRetail = assetBalances[user][asset] >= 0;
    // }
    ...
    function _tryDeposit(
        ...
        /*try IERC20(asset).transferFrom(user, address(this), amountInBase) returns (bool r){
            if (r) {
                assetBalances[user][asset] += int192(amount);
                emit CompleteDeficit(asset, user, int112(amountInBase));
            }
        } catch {
            emit CompleteDeficitError(asset, user, int112(amountInBase), 0);
        }*/
    ...
    function updateOrderBalanceDebit(
        ...
        if (amountBase > 0) {
            ...
            if (order.matcherFeeAsset == assetOut) {
                ...
                tradeType = _updateBalance(user, order.matcherFeeAsset, -1*int256(order.matcherFee));
            }

            tradeType &= _updateBalance(user, assetOut, amountOut);
            actualIn = amountIn;
    ...
    function creditUserAssets(uint tradeType, address user, int amount, address asset) internal {
        if (tradeType == 1 && amount > 0 && assetBalances[user][asset] == 0) {
            ...
            if (contractBalance >= amountInBase) {
                ...
                _updateBalance(user, asset, amount);
            }
        } else  if (amount != 0) {
            _updateBalance(user, asset, int(amount));
        }
    }

    function _updateBalance(address user, address asset, int amount) internal
            returns (uint tradeType) { // 0 - in contract, 1 - from wallet
    ...
    function checkPosition(address user) public view returns (bool) {
        ...
        return
            calcPosition(user).state ==
            MarginalFunctionality.PositionState.POSITIVE;
    }
    ...
    function calcPosition(address user)
        public
        view
        returns (MarginalFunctionality.Position memory)
    {
        ...
        return
            MarginalFunctionality.calcPosition(
                collateralAssets,
                liabilities,
                assetBalances,
                assetRisks,
