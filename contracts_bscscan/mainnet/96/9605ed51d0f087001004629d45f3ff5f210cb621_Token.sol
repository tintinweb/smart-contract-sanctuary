...
contract Token is Context, IERC20, Ownable {
    ...
    address private _charityWallet = 0x65240b02835602e24e8277d3650221Ba50B32138 ;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeLiquidity(tLiquidity, tCharity);
    ...
    function setCharityAddress(address _account) external onlyOwner() {
    ...
    function _takeLiquidity(uint256 tLiquidity, uint256 tCharity) private {
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rLiquidity).add(rCharity);
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tLiquidity).add(tCharity);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 liquidityBalance = contractTokenBalance * _liquidityFee / (_charityFee + _liquidityFee);
        uint256 charityBalance = contractTokenBalance * _charityFee / (_charityFee + _liquidityFee);
        ...
        swapTokensForEth(half + charityBalance); 

        uint256 newBalance = address(this).balance.sub(initialBalance);

        addLiquidity(otherHalf, newBalance * _liquidityFee / (_charityFee + _liquidityFee));
        payable(_charityWallet).transfer(newBalance * _charityFee / (_charityFee + _liquidityFee));
