...
contract Pausable is Ownable {
    ...
    function pause() public onlyOwner whenNotPaused {
        require(block.timestamp < 1661004382, "Function is not callable anymore");
        paused = true;
        emit Pause(paused);
    }
...
contract ToolApe is IBEP20, HasForeignAsset, Pausable {
     ...
     */
    constructor() {
        _name = "DuelNetworkPrivate";
        _symbol = "DuelNetworkPrivate";
        _decimals = 18;
        uint256 _maxSupply = 400000;
...
contract PrivateSale is Ownable {
    ...
    event MaxAllocationChanged(uint256 newAllocation);
    ...
    modifier hasClosed() {
        require(block.timestamp < endTime, "Presale has already finished");
        _;
    }
    ...
    function buyTokens(address _beneficiary) public payable {
        ...
        uint256 tokens = _getTokenAmount(weiAmount*10000000);
        ...
        hasTokens
    {
        ...
        require(_weiAmount <= maxAllocation.sub(investments[_beneficiary]), "Sale: exceeds Max Allocation");
    }
    ...
    /**
     * @notice Withdraw not sold tokens after sale ends
     */
     ...
     * @notice Withdraw not sold tokens after sale ends
     */
    function burnUnsoldTokens() public onlyOwner hasClosed {
        token.burn(token.balanceOf(address(this)));
    }

    function setMaxAllocation(uint256 _newAllocation) public onlyOwner hasClosed {
        require(_newAllocation > 10, "Sale: allocation should be greateer than minimum");
        maxAllocation = _newAllocation;
    }

    function remainingAllocation(address _address) public view returns (uint256) {
        return maxAllocation.sub(investments[_address]);
    }
}