...
contract Stopelon_NFTFarm_v2 is ownable.Ownable, pausable.Pausable, whitelist.WhitelistAdminRole,
	...
	function cardRedeemLevelLimit(address erc1155Address, uint256 id, uint256 level) external view returns (uint256) {
		return rounds[currentRound].cards[erc1155Address][id].cardRedeemLevels[level].limit;
	}
	
	function cardRedeemLevelRedeemed(address erc1155Address, uint256 id, uint256 level) external view returns (uint256) {
		return rounds[currentRound].cards[erc1155Address][id].cardRedeemLevels[level].redeemed;
	}
