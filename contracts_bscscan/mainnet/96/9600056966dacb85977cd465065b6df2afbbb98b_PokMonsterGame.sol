    ...
    ReentrancyGuard
{
    ...
    function initialize(
        ...
        nft = _nft;
        stats = _stats;
        rewardPerSecond = 7534246575342467; // 0.007534246575342467 pok/seconds
        ...
        nonReentrant
    {
        ...
        require(user.tokenId == 0, "deposited!  ");
    ...
    function withdraw(uint256 _pid) public notContract nonReentrant {
        _withdraw(msg.sender, _pid, true);
    }
    ...
    function harvestAllRewards() public notContract nonReentrant {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            UserInfo storage user = userInfo[pid][msg.sender];
            if (user.amount > 0) {
                _withdraw( msg.sender,pid, false);
            }
        }
    }
