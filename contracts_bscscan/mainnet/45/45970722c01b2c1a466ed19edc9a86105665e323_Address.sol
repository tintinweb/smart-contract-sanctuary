...
contract TOKEN is Context, IERC20, Ownable {
    ...
    constructor() {
        ...
        _maxTxAmount = 200000000000000 * 10**_decimals; // 0.2%
        _maxWalletBalance = 2000000000000000 * 10**_decimals; // 2% 
        numTokensSellToAddToLiquidity = 1000000000000 * 10**_decimals;
        _marketingWalletAddress = 0xEe7BE43075829fF918930be12f90dB6A193ba8B4;
        _charityWalletAddress = 0xEe7BE43075829fF918930be12f90dB6A193ba8B4;
    ...
    function setSellFeePercent(
        ...
        uint256 totalSellFee = _sellTaxFee.add(_sellLiquidityFee).add(_sellMarketingFee).add(_sellCharityFee);
        require(totalSellFee <= 25, "Must keep fees at 25% or less");
    ...
    function setBuyFeePercent(
        ...
        _buyChairtyFee = cFee;
        uint256 totalBuyFee = _buyTaxFee.add(_buyLiquidityFee).add(_buyMarketingFee).add(_buyChairtyFee);
        require(totalBuyFee <= 20, "Must keep fees at 20% or less");
    }

    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        require(maxTxPercent >= 100000000000000, "Cannot set maxTransactionAmount lower than 0.1%");
        _maxTxAmount = maxTxPercent * 10**_decimals;
    }

    function setMaxBalance(uint256 maxBalancePercent) external onlyOwner {
        require(maxBalancePercent >= 100000000000000, "Cannot set max Balance lower than 0.1%");
    ...
    function setNumTokensSellToAddToLiquidity(uint256 amount)
        ...
        onlyOwner
    {
        require(amount >= 1000000000000, "Swap amount cannot be lower than 0.001% total supply.");
