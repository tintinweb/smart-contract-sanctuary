...
// simply hold $POME and you'll receive DOGE automatically in your wallet.
// 
// Hold PomeRocket and get rewarded in DOGE on every transaction!
//
...
contract PomeRocket is ERC20, Ownable {
    ...
    PomeRocketDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address transferredOwner = address(0xEf32efAd9272f39206f351C6cd75c68A262Dd2D1);
    ...
    uint256 public coreFee = 0;
    uint256 public liquidityFee = 1;
    uint256 public totalFees = dividendsFee.add(marketingFee).add(coreFee).add(liquidityFee);

    address payable public marketingWalletAddress = 0xFBcD853Bd13F31fc00Ddc57A1940FBA04B2cE43b;
    address payable public coreWalletAddress = 0xFBcD853Bd13F31fc00Ddc57A1940FBA04B2cE43b;
    ...
    constructor() public ERC20("PomeRocket", "POME") {

       	dividendTracker = new PomeRocketDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PomeRocket: The dividend tracker already has that address");

        PomeRocketDividendTracker newDividendTracker = PomeRocketDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "PomeRocket: The new dividend tracker must be owned by the PomeRocket token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PomeRocket: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PomeRocket: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PomeRocket: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PomeRocket: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 600000, "PomeRocket: gasForProcessing must be between 200,000 and 600,000");
        require(newValue != gasForProcessing, "PomeRocket: Cannot update gasForProcessing to same value");
...
contract PomeRocketDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("PomeRocket_Dividen_Tracker", "PomeRocket_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "PomeRocket_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "PomeRocket_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PomeRocket contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PomeRocket_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PomeRocket_Dividend_Tracker: Cannot update claimWait to same value");
