...
contract Auction is AccessControlEnumerable, Initializable, LinkedNodes {
    ...
    function endAuction(uint _id) external noContract {
        ...
        if (nodes[activeAuctionNodeId].next != 0) {
        ...
        if (runningAuction[_id].highestBidder == address(0)) {
          ...
          nftContract.transferFrom(address(this), runningAuction[_id].seller, _id);
        } else {
          // Successful bid won for this auction, send NFT to winner and payment to seller
          nftContract.transferFrom(address(this), runningAuction[_id].highestBidder, _id); // transfer NFT to buyer
