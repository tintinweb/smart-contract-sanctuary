...
contract SantasMoon is Context, IERC20, Ownable {
    ...
    string private _name = "SantasMoon";
    ...
    uint256 internal _maxWalletSize = 30 *10**14; // 3 Million max wallet
    ...
    // fees
    uint256 public _taxFee = 200; // 200 = 2% HOLDERS REDISTRIBUTION
    uint256 public _charityFee = 200; // 200 = 2% CHARITY FEE
    uint256 public _MarketingFee = 500; // 500 = 5% MARKETING FEE
    uint256 public _liquidityFee = 500; // 500 = 5% LIQUIDITY FEE
    //variables
    ...
    //addresses
    address public charityAddress = 0x021735ef9c4FEeF584daEC7d27Bd512121799F76; // Charity Address
    address public liquidityAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // PancakeSwapRouterV2
    address public MarketingAddress = 0x24082A18fABb8bccdBd1FA063E0A076B52EaF2Ba; // Marketing address
    address public devAddress = 0x24082A18fABb8bccdBd1FA063E0A076B52EaF2Ba; // Dev address
 ...
 /**
    ...
    */
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x10ED43C718714eb63d5aA57B78B54704E256024E,"SAM: Uniswap router cannot be excluded.");
        require(account != address(this), 'SAM: The contract it self cannot be excluded');
        require(!_isExcluded[account], "SAM: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "SAM: Account is already included");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(sender != owner() && recipient != owner() && recipient != address(0xdead) && sender != address(liquidityAddress) && recipient != address(liquidityAddress) && recipient != address(charityAddress) && sender != address(charityAddress) && recipient != address(MarketingAddress) && sender != address(MarketingAddress) && sender != address(devAddress) && recipient != address(devAddress)) {
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(signatory != address(0), "SAM::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "SAM::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "SAM::delegateBySig: signature expired");
    ...
    function setMaxWalletValue(uint256 value) public onlyOwner {
