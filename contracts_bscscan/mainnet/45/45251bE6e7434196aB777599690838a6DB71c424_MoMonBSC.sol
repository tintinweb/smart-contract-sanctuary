    ...
    IDividendPayingTokenOptional
{
    ...
    address public dividendToken = 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d; // Binance-Peg USDC
...
contract MoMonBSC is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public _dividendToken = 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d; // Binance-Peg USDC 
    ...
    monkeymoneybscDividendTracker public dividendTracker;

    address public monkeymoneybscWallet;
    ...
    uint256 public monkeymoneybscFee;
    ...
    event monkeymoneybscWalletUpdated(
    ...
    constructor() ERC20("MoMonBSC", "MOMON") {
        uint256 _dividendRewardsFee = 7;
        uint256 _monkeymoneybscFee = 10;

        dividendRewardsFee = _dividendRewardsFee;
        monkeymoneybscFee = _monkeymoneybscFee;
        totalFees = _dividendRewardsFee.add(_monkeymoneybscFee);

        dividendTracker = new monkeymoneybscDividendTracker();

        monkeymoneybscWallet = 0x0543eDB2c291419dBf43b18c7A5329924D0892a7;
        ...
        excludeFromFees(monkeymoneybscWallet, true);
        ...
        onlyOwner
    {
        require(
            ...
            "monkeymoneybsc: Sell transaction multipler must be between 100 (1x) and 200 (2x)"
        );
    ...
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(
            ...
            "monkeymoneybsc: Marketing divisor must be between 0 (0%) and 100 (100%)"
        );
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(
            !swapping,
            "monkeymoneybsc: A swapping process is currently running, wait till that is complete"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "monkeymoneybsc: The dividend tracker already has that address"
        );

        monkeymoneybscDividendTracker newDividendTracker = monkeymoneybscDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "monkeymoneybsc: The new dividend tracker must be owned by the FLOKIBUSD token contract"
        );
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "monkeymoneybsc: Dividend reward tax must be between 0 and 10"
        );
        dividendRewardsFee = newFee;
    }

    function updatemonkeymoneybscFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "monkeymoneybsc: Dividend reward tax must be between 0 and 10"
        );
        monkeymoneybscFee = newFee;
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            newAddress != address(uniswapV2Router),
            "monkeymoneybsc: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "monkeymoneybsc: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "monkeymoneybsc: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "monkeymoneybsc: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updatemonkeymoneybscWallet(address newmonkeymoneybscWallet) public onlyOwner {
        require(
            newmonkeymoneybscWallet != monkeymoneybscWallet,
            "monkeymoneybsc: The liquidity wallet is already this address"
        );
        excludeFromFees(newmonkeymoneybscWallet, true);
        monkeymoneybscWallet = newmonkeymoneybscWallet;
        emit monkeymoneybscWalletUpdated(newmonkeymoneybscWallet, monkeymoneybscWallet);
    }

    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            newValue >= 200000 && newValue <= 500000,
            "monkeymoneybsc: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "monkeymoneybsc: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[from]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 swapTokens = contractTokenBalance.mul(monkeymoneybscFee).div(
                    totalFees
                );
                swapTokensForBNB(swapTokens);
                transferTomonkeymoneybscWallet(
                    payable(monkeymoneybscWallet),
    ...
    function transferTomonkeymoneybscWallet(address payable recipient, uint256 amount)
        private
    {
...
contract monkeymoneybscDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken(
            "monkeymoneybsc_Dividend_Tracker",
            "monkeymoneybsc_Dividend_Tracker"
        )
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "monkeymoneybsc_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "monkeymoneybsc_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main monkeymoneybsc contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "monkeymoneybsc_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "monkeymoneybsc_Dividend_Tracker: Cannot update claimWait to same value"
        );
