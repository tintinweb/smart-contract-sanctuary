...
abstract contract Ownable {
    ...
    modifier onlyOwner() {
        require(isOwner(msg.sender), "NOT THE KRYPT OWNER");
        _;
    }
...
contract Kryptonect is IBEP20, Ownable {
    ...
    function _transferFrom(
        ...
        if (!isTxLimitExempt[sender] && !isTxLimitExempt[recipient]) {
            ...
            if (!tradingOpen) {
                require(
                    ...
                    "Trading is not enabled yet - Kam"
                );
            }
            // antibot
            if (
        ...
        if (isFeeExempt[sender] || isFeeExempt[recipient]) {
            ...
            // buying handler
            if (sender == pair) {
                require(amount <= maxBuyAmount, "TXN Limit Exceeded");
                setBuyFee();
            }
            // selling handler
            else if (recipient == pair) {
                require(amount <= maxSellAmount, "TXN Limit Exceeded");
                setSellFee();
            }
    ...
    function launch() public onlyOwner {
        require(launchedAt == 0, "Krypt already launched");
    ...
    function setBuyFeePercent(
        ...
        require(
            ...
            "BEP20: Cannot be greater than max fee"
        );
    }
    ...
    function addSniperInList(address _account) external onlyOwner {
        require(_account != address(router), "We can not blacklist the router");
        require(!isSniper[_account], "Sniper already exists");
