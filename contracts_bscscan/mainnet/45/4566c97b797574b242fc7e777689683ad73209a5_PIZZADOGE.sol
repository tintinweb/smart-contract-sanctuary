...
🍕 PizzaDoge 🍕
https://pizzadoge.fun/
https://twitter.com/PizzaDogeToken
https://t.me/PizzaDogeOfficial
*/
//
	...
	contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
	  ...
	  address public immutable BNBTokens = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
	  ...
	  function distributeBNBTokensDividends(uint256 amount) public {
		require(totalSupply() > 0);
	  ...
	  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
		...
		if (_withdrawableDividend > 0) {
		  ...
		  bool success = IERC20(BNBTokens).transfer(user, _withdrawableDividend);
	...
	contract PIZZADOGE is ERC20, Ownable {
		...
		address public immutable BNBTokens = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
		address payable public marketingAddress = payable(0x6F3e3f7F35a43a4386B2D16Bf78380e274B8A0Bd); 
		address payable public BuybackWallet = payable(0x8C1F41AB7d9538E2Cd271fA674FD0F10B9DB3B28); 
		...
		uint256 public _maxSellTransactionAmount = 1000000000 * (10**18);
		uint256 private _swapTokensAtAmount = 100000 * (10**18);
		uint256 public _maxTxAmount = 1000000 * (10**16);
		uint256 public _maxWalletAmount = 20000000 * (10**16);

		uint256 public immutable BNBTokensRewards;	
		uint256 public immutable LiquidityFee;
		uint256 public immutable totalFees;
		
		uint256 public _MarketingFee = 5;
		uint256 public _BuyBackMultiplier = 5;
		...
		constructor() ERC20("PIZZADOGE", "$PDG") {
			uint256 _BNBTokensRewards = 3;
			uint256 _LiquidityFee = 5;

			BNBTokensRewards = _BNBTokensRewards;
			LiquidityFee = _LiquidityFee;
			totalFees = _BNBTokensRewards.add(_LiquidityFee);
		...
		function getTotalDividendsDistributed() external view returns (uint256) {
		 ...
		 function SetBNBTokensRewards(uint256 BNBTokensRewards) external onlyOwner() {
			BNBTokensRewards = BNBTokensRewards;
		}
		...
		function swapTokensForBNBTokens(uint256 tokenAmount, address recipient) private {
		   
			// generate the uniswap pair path of weth -> BNBTokens
			...
			uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
				tokenAmount,
				0, // accept any amount of BNBTokens
				path,
				recipient,
				block.timestamp
			);
    ...
    function unblacklistMultipleWallets(address[] calldata addresses) public onlyOwner(){
		...
		function swapAndSendDividends(uint256 tokens) private {
			swapTokensForBNBTokens(tokens, address(this));
			uint256 dividends = IERC20(BNBTokens).balanceOf(address(this));
			bool success = IERC20(BNBTokens).transfer(address(dividendTracker), dividends);
			
			if (success) {
				dividendTracker.distributeBNBTokensDividends(dividends);
