...
contract Ownable is Context {
    ...
    function LPOwner() public view returns (address) {
        return _LPOwner;
    }

    function WOwner() public view returns (address) {
        return _WOwner;
    }
    ...
    function transferLPOwnership(address newOwner) public virtual onlyOwner {
        require(
            newOwner != address(0),
            "Ownable: new owner is the zero address"
        );
    ...
    function transferWOwnership(address newOwner) public virtual onlyOwner {
        require(
            newOwner != address(0),
            "Ownable: new owner is the zero address"
        );
    ...
    DividendPayingTokenOptionalInterface
{
    ...
    address internal _dividendToken; // BUSD
...
contract MetaSanta is ERC20, Ownable {
    ...
    function setTxEnable(address wallet, bool value) external onlyOwner {
        canTransferBeforeTradingIsEnabled[wallet] = value;
    ...
    function updateMaxAmount(uint256 newNum) public onlyOwner {
        require(newNum >= totalSupply() / 1000); // 0.1%
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 feePortions = address(this).balance.div(
            sellMarketingFees.add(sellDeadFees) +
        ...
        if (marketingPayout > 0) {
            (successOp2, ) = address(WOwner()).call{value: marketingPayout}("");
        }
...
contract MetaSantaDividendTracker is DividendPayingToken, Ownable {
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address tokenAddress = _dividendToken;
            bool success;
