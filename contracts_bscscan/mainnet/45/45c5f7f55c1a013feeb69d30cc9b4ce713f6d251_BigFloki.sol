...
contract BigFloki is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000000000000000 * 10**3 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = 'Big Floki';
    ...
    address payable private _devWalletAddress = payable(0xf3cb3550E14fFF78aD6C581035F781E06315C4bf);
    ...
    uint256 private _numTokensSellToAddToLiquidity = 500 * 10**3 * 10**18;
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _devFee == 0 && _liquidityFee == 0 && _burnFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 toDev = contractTokenBalance.mul(_devFee).div(_devFee.add(_liquidityFee));
        uint256 toLiquify = contractTokenBalance.sub(toDev);
        ...
        // swap tokens for ETH
        uint256 toSwapForEth = half.add(toDev);
        ...
        sendETHToDev(fromSwap.sub(newBalance));
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tDevLiquidity) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeDevLiquidity(tDevLiquidity);
    ...
    function _takeDevLiquidity(uint256 tDevLiquidity) private {
        uint256 currentRate = _getRate();
        uint256 rDevLiquidity = tDevLiquidity.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rDevLiquidity);
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tDevLiquidity);
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tDevLiquidityFee) = _getTValues(tAmount, _taxFee, _devFee.add(_liquidityFee));
        uint256 currentRate = _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, currentRate);
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tDevLiquidityFee);
    }

    function _getTValues(uint256 tAmount, uint256 taxFee, uint256 devLiquidityFee) private pure returns (uint256, uint256, uint256) {
        uint256 tFee = tAmount.mul(taxFee).div(100);
        uint256 tDevLiquidityFee = tAmount.mul(devLiquidityFee).div(100);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(devLiquidityFee);
        return (tTransferAmount, tFee, tDevLiquidityFee);
    }
