...
contract PresaleExchange {
    ...
    uint256 public startDate = 1640995200;                  // 1/1/2022
        
    uint256 public totalTokensToSell = 1000000 * 10**18;      // 1 mil tokens for sell
    uint256 public tokenPerPRE = 20 * 10**18;                // 20 Token = 1 Fre
    uint256 public minPerTransaction = 1;                   // 1 min amount per transaction
    uint256 public maxPerUser = 1000000 * 10**18;       // 1M max amount per user
    ...
    function calculatePREAmount(uint256 tokenAmount) public view returns (uint256) {
        ...
        uint256 preAmount = tokenAmount.mul(10**18).div(tokenPerPRE);
        return preAmount;
    }
}