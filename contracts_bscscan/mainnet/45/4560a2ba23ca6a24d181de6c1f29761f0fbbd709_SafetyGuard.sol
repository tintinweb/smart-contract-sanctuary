...
contract SafetyGuard is Context, IBEP20, Ownable {
    ...
    uint256 public durationBetweenEachBuyback = 60 seconds; 
    
    uint256 public _buyBackFee = 0; 
    uint256 public _previousBuyBackFee = _buyBackFee; 

    uint256 public _marketingBNBFee = 0;
    uint256 public _previousMarketinBNBFee = _marketingBNBFee; 
    ...
    uint256 private _previousTokenFee = antiBot;
    ...
    uint256 public minTokensSwapToAndTransferTo = 90000000 * 10**1 * 10**_decimals; 
    uint256 public _maxTokensPerAddress = _totalSupply.div(16); 
    ...
    function calculateMarketingTokenFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(antiBot).div(
            10**3
        );
    }
    ...
    function removeAllFee() private {
        if(_deductableFee == 0 && antiBot == 0 && _buyBackFee == 0
           && _marketingBNBFee == 0 && _liquidityFee == 0) return;
        
        _previousTokenFee = antiBot;
