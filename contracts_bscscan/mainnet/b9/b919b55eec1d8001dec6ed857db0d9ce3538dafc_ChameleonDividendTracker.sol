...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d;
...
contract ChameleonRewards is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   ChameleonDividendTracker public dividendTracker;
    ...
    uint256 public constant MAX_SELL_TRANSACTION_AMOUNT = 100000000 * (10**18);

    uint256 public constant Chameleon_DIVIDENDS_FEE = 4;
    uint256 public constant LIQUIDITY_FEE = 6;
    uint256 public constant TOTAL_FEES = Chameleon_DIVIDENDS_FEE + LIQUIDITY_FEE;
    ...
    address public _dividendToken = 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d;
    bool _maxBuyEnabled = true;

    // use by default 150,000 gas to process auto-claiming dividends
    uint256 public gasForProcessing = 999999;
    ...
    function activate() public onlyOwner {
        require(!tradingEnabled, "Chameleon: Trading is already enabled");
        _swapEnabled = true;
        tradingEnabled = true;
    }
    ...
    constructor() ERC20("ChameleonRewards", "Chameleon") {
        dividendTracker = new ChameleonDividendTracker();
        ...
        dividendTracker.excludeFromDividends(address(_dividendToken));
    ...
    receive() external payable {
      ...
      function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Chameleon: The Uniswap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Chameleon: Automated market maker pair is already set to that value");
    ...
    function excludeFromFees(address account) public onlyOwner {
        require(!_isExcludedFromFees[account], "Chameleon: Account is already excluded from fees");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        ...
        require(newValue != gasForProcessing, "Chameleon: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            ...
            if(!openForPresale){
                require(canTransferBeforeTradingIsEnabled[from], "Chameleon: This account cannot send tokens until trading is enabled");
            }
        }
    ...
    function swapAndSendToDev(uint256 tokens) private {
        ...
        address payable _devAndMarketingAddress = payable(0x8b690529bA8b0c50b1E9dB08e63deBa795a8B0d1);
...
contract ChameleonDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Chameleon_Dividend_Tracker", "Chameleon_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 20000 * (10**18); //must hold 20000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "Chameleon_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "Chameleon_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Chameleon contract.");
    }
    ...
    function excludeFromDividends(address account) external onlyOwner {
      ...
      function updateGasForTransfer(uint256 newGasForTransfer) external onlyOwner {
        require(newGasForTransfer != gasForTransfer, "Chameleon_Dividend_Tracker: Cannot update gasForTransfer to same value");
        emit GasForTransferUpdated(newGasForTransfer, gasForTransfer);
        gasForTransfer = newGasForTransfer;
    }

    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "Chameleon_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Chameleon_Dividend_Tracker: Cannot update claimWait to same value");
