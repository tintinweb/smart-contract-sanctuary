...
contract DecentralizedFeeManager is Ownable{
    ...
    // add LP tokens to list to allow easy liquidity removal. We should only add the common tokens, to reduce chances of gas error on removeallLiquidity
    ...
    // Main function to swap one token to wbnb
    ...
    // Used by owner to swap specific amounts of tokens
    ...
    function updateTeamShares(uint256 _moneyPotShare) external onlyOwner{
    ...
    function setTier1 (uint256 newTier1) external onlyOwner{
     ...
     // function used to modify the uppertier minimum SNOVA
