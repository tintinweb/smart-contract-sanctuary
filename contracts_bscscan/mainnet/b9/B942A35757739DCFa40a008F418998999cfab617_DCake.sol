...
contract DCake is Context, IERC20, Ownable {
    ...
    constructor (address payable charityWalletAddress, address payable mktWalletAddress) {
    ...
    function calculateMktFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_mktFee).div(10**3);
    }
    ...
    (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tCharity, uint256 tMkt) = _getValues(tAmount);
    ...
    function _setMktWalletAddress(address payable mktWalletAddress) external onlyOwner() {
        _mktWalletAddress = mktWalletAddress;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tCharity, data.tMkt);
    }
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tCharity).sub(tMkt);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tCharity, tMkt, 0));
    }
    function _getRValues( TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rCharity).sub(rMkt);
    ...
    function _takeMkt(uint256 tMkt, address sender) private {
        ...
        emit Transfer(sender, _mktWalletAddress, tMkt);
    }
