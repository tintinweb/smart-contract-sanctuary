...
//
// $BabyPyongyang proposes an innovative feature in its contract.
//
// DIVIDEND YIELD PAID IN SUSHI! With the auto-claim feature,
// simply hold $BabyPyongyang and you'll receive SUSHI automatically in your wallet.
// 
// Hold BabyPyongyang and get rewarded in Sushi on every transaction!
//
// 📱 Telegram: https://t.me/BabyPyongyangCoin
// 🌎 Website: https://BabyPyongyang.io
// 🌐 Twitter: https://twitter.com/BabyPyongyangcoin
...
import "./BabyPyongyangTransferHelper.sol";

contract BabyPyongyang is ERC20, Ownable {
	...
	BabyPyongyangDividendTracker public dividendTracker;
	BabyPyongyangTransferHelper private transferHelper;

	address public deadWallet = 0x000000000000000000000000000000000000dEaD;

	address public SUSHI = 0x947950BcC74888a40Ffa2593C5798F11Fc9124C4;
	...
	constructor(address routerAddress, address payable marketingWalletAddress) public ERC20("BabyPyongyang", "BabyPyongyang") {
		dividendTracker = new BabyPyongyangDividendTracker();
		transferHelper = new BabyPyongyangTransferHelper(routerAddress);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "BabyPyongyang: The dividend tracker already has that address");

		BabyPyongyangDividendTracker newDividendTracker = BabyPyongyangDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "BabyPyongyang: The new dividend tracker must be owned by the BabyPyongyang token contract");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "BabyPyongyang: The router already has that address");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "BabyPyongyang: Account is already the value of 'excluded'");
	...
	function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
		require(pair != uniswapV2Pair, "BabyPyongyang: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
	...
	function setMaxWalletAmount(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "BabyPyongyang: Amount cannot be over the total supply.");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "BabyPyongyang: Automated market maker pair is already set to that value");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "BabyPyongyang: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "BabyPyongyang: Cannot update gasForProcessing to same value");
...
contract BabyPyongyangDividendTracker is Ownable, DividendPayingToken {
	using SafeMath for uint256;
	...
	constructor() public DividendPayingToken("BabyPyongyang_Dividend_Tracker", "BabyPyongyang_Dividend_Tracker") {

	}

	function _transfer(address, address, uint256) internal override {
		require(false, "BabyPyongyang_Dividend_Tracker: No transfers allowed");
	}

	function withdrawDividend() public override {
		require(false, "BabyPyongyang_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyPyongyang contract.");
	}
	...
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyPyongyang_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "BabyPyongyang_Dividend_Tracker: Cannot update claimWait to same value");
