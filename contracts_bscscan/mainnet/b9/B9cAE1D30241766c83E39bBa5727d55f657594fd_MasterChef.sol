...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accFastPerShare;  // Accumulated FASTs per share, times 1e9. See below.
    }
    ...
    function pendingFast(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && endTime > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 fastReward = pool.amountFastTokens.mul(1e9).div(perDays).mul(time);
            if (fastReward > 0) {
                accFastPerShare = accFastPerShare.add(fastReward.div(lpSupply));
            }
        }

        return user.amount.mul(accFastPerShare).div(1e9).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (fastReward > 0) {
            pool.accFastPerShare = pool.accFastPerShare.add(fastReward.mul(1e9).div(lpSupply));
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accFastPerShare).div(1e9).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accFastPerShare).div(1e9);
