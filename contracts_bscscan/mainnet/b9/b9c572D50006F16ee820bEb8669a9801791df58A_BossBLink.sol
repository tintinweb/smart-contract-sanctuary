...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable DividendsToken = address(0xF8A0BF9cF54Bb92F17374d9e9A321E6a111a51bD);
...
contract BossBLink is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   BossBLinkDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("BossBabyLink", "BossBLink") {
        //Reflection Rewards %
        _ETHRewardsFee = 7;
        //Liqudity Fee %
        _liquidityFee = 4;
        ...
        maxBuyTranscationAmount = 1000000000 * (10**18); //Unlimted
        maxSellTransactionAmount = 800000000 * (10**18); //Max Sell 1%
        swapTokensAtAmount = 400000000 * (10**18); // 0.4% of the supply in order to get rewards
        _maxWalletToken = 2000000000 * (10**18); // 3% of total supply


    	dividendTracker = new BossBLinkDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
        //Marketing Wallet Address
		marketingWallet = payable(0xa97Cf9Eb83C431103F985BBFCAF2EC47D3873192);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BossBLink: The dividend tracker already has that address");

        BossBLinkDividendTracker newDividendTracker = BossBLinkDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BossBLink: The new dividend tracker must be owned by the BossBLink token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BossBLink: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BossBLink: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BossBLink: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BossBLink: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BossBLink: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BossBLink: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BossBLink: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForCAKE(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of BossBLink
            path,
            recipient,
            block.timestamp
        );
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "BossBLink: Marketing tax must be between 0 and 10");
        marketingFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "BossBLink: Liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
    }
    
    function updateETHRewardsFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 15, "BossBLink: ETH Rewards tax must be between 0 and 15");
        liquidityFee = newFee;
    }
...
contract BossBLinkDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BossBLink_Dividend_Tracker", "BossBLink_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "BossBLink_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BossBLink_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BossBLink contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "BossBLink_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BossBLink_Dividend_Tracker: Cannot update claimWait to same value");
