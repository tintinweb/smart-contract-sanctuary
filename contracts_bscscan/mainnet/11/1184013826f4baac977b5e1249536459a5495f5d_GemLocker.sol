...
contract GemLocker is Initializable, AccessControlUpgradeable, IGemLocker, ReentrancyGuardUpgradeable {
    ...
    function changeUserTotalLockedClamRewards(address[] memory _accounts, uint256[] memory values) external {
        require(hasRole(UPDATER_ROLE, msg.sender), "GemLocker: must have updater role");
        require(_accounts.length == values.length, "incorrect array length");

        for (uint256 index = 0; index < _accounts.length; index++) {
            totalClamRewardsLocked[_accounts[index]] = values[index];
        }
    }

    function changeUserTotalLockedPearlRewards(address[] memory _accounts, uint256[] memory values) external {
        require(hasRole(UPDATER_ROLE, msg.sender), "GemLocker: must have updater role");
        require(_accounts.length == values.length, "incorrect array length");

        for (uint256 index = 0; index < _accounts.length; index++) {
            totalPearlRewardsLocked[_accounts[index]] = values[index];
        }
    }
