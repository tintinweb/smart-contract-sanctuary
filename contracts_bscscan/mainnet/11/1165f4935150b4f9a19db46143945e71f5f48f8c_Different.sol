...
Something Different is coming to BSC...

Telegram: https://t.me/DifferentBSC
Website: https://differentbsc.com/
Twitter: https://twitter.com/DifferentBSC
...
contract BNBDistributor is IDividendDistributor {
    ...
    constructor (address _wbnb, address _router, address _charityReceiver, address _marketingReceiver, address _liquidityReceiver) {
        ...
        _stakingMagnitude = 10 * 10 ** (7 + 9); // 100 Million
    }
...
contract Different is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "Different";
    string constant _symbol = "DIFF";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 10 * 10 ** (7 + _decimals); // 100 Million
    ...
    uint256 public _maxTxAmount = _totalSupply * 0 / 1000; // 0% of the total supply, to be increased to 1% afer LP is added.
    uint256 public _walletMax = _totalSupply * 0 / 1000; // 0% of total supply, to be increased to 1% after LP is added.
    ...
    constructor() {
        ...
        isTxLimitExempt[address(bnbDistributor)] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        /*if(sender != owner()){
            require(canTrade, "Trading is not enabled yet.");
        }*/
        if(sender != owner()){
            require(restrictWhales,"Trading not enabled yet.");
        }
    ...
    function maxWalletStatus(bool status) external onlyOwner{
        restrictWhales = status;
    }
    ...
    function buyDiffWithReward() external {
    ...
    function recoverExcess(uint256 amount) external onlyOwner {
        require(amount < address(this).balance, "DIFF: Can not send more than contract balance");
        payable(marketingFeeReceiver).transfer(amount);
    }

    // only for recovering tokens that are NOT DIFF tokens sent in error by wallets
