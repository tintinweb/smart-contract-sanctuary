...
library Address {
     ...
     * Requirements:
     *
     * - the calling contract must have an RUSD balance of at least `value`.
...
interface IUniswapV2Router01 {
    ...
    function WRUSD() external pure returns (address);
    ...
    function addLiquidityRUSD(
        ...
        address to,
        uint deadline
    ) external payable returns (uint amountToken, uint amountRUSD, uint liquidity);
    ...
    function removeLiquidityRUSD(
        ...
        address to,
        uint deadline
    ) external returns (uint amountToken, uint amountRUSD);
    ...
    function removeLiquidityRUSDWithPermit(
        address token,
        uint liquidity,
        uint amountTokenMin,
        uint amountRUSDMin,
    ...
    function swapExactRUSDForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        external
        payable
        returns (uint[] memory amounts);
    function swapTokensForExactRUSD(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        returns (uint[] memory amounts);
    function swapExactTokensForRUSD(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)
        external
        returns (uint[] memory amounts);
    function swapRUSDForExactTokens(uint amountOut, address[] calldata path, address to, uint deadline)
        external
        payable
        returns (uint[] memory amounts);
...
interface IRUSDLiquidityPool is ILiquidityPool {
    function lock(uint id, uint256 amount, uint256 token) external;
}
...
// File: contracts/Pool/OctiRUSDPool.sol
...
/**
 * @author 0mllwntrmt3
 * @title Octi RUSD Liquidity Pool
 * @notice Accumulates liquidity in RUSD from LPs and distributes P&L in RUSD
 */
contract OctiRUSDPool is
    IRUSDLiquidityPool,
    Ownable,
    BEP20("Octi RUSD LP Token", "ORUSD")
{
    using SafeMath for uint256;
    ...
    IBEP20 busdToken = IBEP20(0x07663837218A003e66310a01596af4bf4e44623D);
    ...
    /*
     * @nonce A provider supplies RUSD to the pool and receives writeRUSD tokens
    ...
    /*
     * @nonce Provider burns writeRUSD and receives RUSD from the pool
     * @param amount Amount of RUSD to receive
    ...
    /*
     * @nonce Returns provider's share in RUSD
     * @param account Provider's address
     * @return Provider's share in RUSD
     */
    ...
    /*
     * @nonce Returns the amount of RUSD available for withdrawals
    ...
    /*
     * @nonce Returns the total balance of RUSD provided to the pool
...
// File: contracts/Options/OctiRUSDOptions.sol
...
/**
 * @author 0mllwntrmt3
 * @title Octi RUSD (Ether) Bidirectional (Call and Put) Options
 * @notice Octi RUSD Options Contract
 */
contract OctiRUSDOptions is Ownable, IOctiOptions {
    using SafeMath for uint256;
    ...
    OctiRUSDOptions private oldOctiRUSDOptions;
    // AggregatorV3Interface public priceProvider;
    OctiRUSDPool public pool;

    IBEP20 busdToken1 = IBEP20(0x07663837218A003e66310a01596af4bf4e44623D);

    struct CoinList { string coinName; AggregatorV3Interface priceProvider; bool isActive; }
    mapping(string=>CoinList) public coinlist;
    
    

    constructor(address owner, OctiRUSDPool _pool) public {
        pool = _pool;
        // priceProvider = pp;
        ...
        coinlist["RUSD"] = CoinList("RUSD",AggregatorV3Interface(0xB97Ad0E74fa7d920791E90258A6E2085088b4320),true);
        coinlist["ETH"] = CoinList("ETH",AggregatorV3Interface(0x9ef1B8c0E4F7dc8bF5719Ea496883DC6401d5b2e),true);
        coinlist["BTC"] = CoinList("BTC",AggregatorV3Interface(0x264990fbd0A4796A3E3d8E37C4d5F87a3aCa5Ebf),true);
        coinlist["ADA"] = CoinList("ADA",AggregatorV3Interface(0xa767f745331D267c7751297D982b050c93985627),true);
        coinlist["CAKE"] = CoinList("CAKE",AggregatorV3Interface(0xB6064eD41d4f67e353768aA239cA86f4F73665a1),true);
        coinlist["DOGE"] = CoinList("DOGE",AggregatorV3Interface(0x3AB0A0d137D4F946fBB19eecc6e92E64660231C8),true);
       
    }

    function addNewCoin(string memory coinName, AggregatorV3Interface pp) public {
            coinlist[coinName] = CoinList(coinName,pp,true);
    }
    /**
        ...
        returns (uint256 optionID)
    {
        ...
        uint256 totalInRUSD = convertCoinValue(total,coinName,"RUSD");

        require(period >= 1 days, "Period is too short");
        require(period <= 4 weeks, "Period is too long");
        require(amount > strikeFee, "Price difference is too large");
        require(token >= totalInRUSD, "Wrong value");
        busdToken1.transferFrom(msg.sender,address(this),token);
        if (token > totalInRUSD) busdToken1.transfer(msg.sender,token - totalInRUSD );
        ...
        uint256 settlementFeeInRUSD = convertCoinValue(settlementFee,coinName,"RUSD");
        uint256 premiumInRUSD = convertCoinValue(option.premium,coinName,"RUSD");
        uint256 lockedAmountInRUSD = convertCoinValue(option.lockedAmount,coinName,"RUSD");

        busdToken1.transfer(settlementFeeRecipient,settlementFeeInRUSD);
        busdToken1.transfer(address(pool),premiumInRUSD);
        
        pool.lock(optionID, lockedAmountInRUSD,premiumInRUSD);
        emit Create(optionID, msg.sender, settlementFeeInRUSD, totalInRUSD);
    }
     ...
     * @param optionID ID of your option
     */
    function exercise(uint256 optionID, string memory coinName) external {
        Option storage option = options[optionID];
        ...
        uint256 profit = payProfit(optionID,coinName);
    ...
    function migrate(uint count) external onlyOwner {
        ...
        require(address(oldOctiRUSDOptions) != address(0));
        for (uint i = 0; i < count; i++){
            uint optionID = options.length;
            OctiRUSDOptions.Option memory option;
            (
                option.state,
                option.holder,
                option.strike,
                ...
                option.expiration,
                option.optionType
            ) = oldOctiRUSDOptions.options(optionID);
    ...
    function setOldOctiRUSDOptions(address oldAddr) external onlyOwner {
        require(address(oldOctiRUSDOptions) == address(0));
        oldOctiRUSDOptions = OctiRUSDOptions(oldAddr);
    }
    ...
    {
        (,int latestPrice,,,) = coinlist[coinName].priceProvider.latestRoundData();
     ...
     * @param period Option period in seconds (1 days <= period <= 4 weeks)
     * @param strike Strike price of the option
     * @param currentPrice Current price of RUSD
    ...
    /**
     * @notice Sends profits in RUSD from the RUSD pool to an option holder's address
     * @param optionID A specific option contract id
     */
     function payProfit(uint optionID,string memory coinName)
         internal
         returns (uint profit)
     {
         ...
         uint256 profitInRUSD = convertCoinValue(profit,coinName,"RUSD");
         pool.send(optionID, option.holder, profitInRUSD);
     }
