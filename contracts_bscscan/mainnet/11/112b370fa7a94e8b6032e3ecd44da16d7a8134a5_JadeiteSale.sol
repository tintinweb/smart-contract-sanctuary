...
Jadeite Gemstone Sale Contract
...
contract JadeiteSale is ReentrancyGuard, Context, Ownable {
    ...
    uint256 public availableTokensSale;
    uint256 private previousProcent;
    ...
    function buyTokens() public SaleActive payable {
        ...
        if(block.timestamp <= endBonus && procent > 0) {
            ...
            uint256 bonus = procent.mul(weiAmount).div(100);
    ...
    function SetRound(uint256 roundProcent, uint256 endRound) external onlyOwner {
        previousProcent = roundProcent;
        procent = previousProcent;
        endBonus = endRound;
    }
