...
contract Launcher is Ownable {
    ...
    function createPresale(address token, uint presale_rate, uint softcap, uint hardcap, uint min, uint max, uint pcs_liquidity, uint pcs_rate, uint32 start_time, uint32 end_time, uint32 unlock_time, string memory metadata) payable external {
