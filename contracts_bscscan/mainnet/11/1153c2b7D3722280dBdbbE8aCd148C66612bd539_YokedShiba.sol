...
Telegram: https://t.me/YokedShibaBSC
Twitter: https://twitter.com/YokedShiba

Check out our Website and Telegram for more info.
...
contract Ownable is Context {
  ...
  function unlock() public virtual {
    ...
    require(block.timestamp > _lockTime , "Contract is locked until time runs out");
...
// YokedShiba is the strongest doggo of them all
contract YokedShiba is Context, IBEP20, Ownable {
  ...
  address payable public marketingAddress = payable(0x0e6e8989E254043Ab19eB7A86b6F9c0BA7D081d2);

  string private _name = "YokedShiba";
  string private _symbol = "YOKED";
  ...
  uint256 public _reflectionFee = 70;
  uint256 private _previousReflectionFee = _reflectionFee;

  uint256 private _taxFee = _burnFee.add(_reflectionFee);
  uint256 private _previousTaxFee = _taxFee;

  uint256 private _crctLiquidityFee = 40;
  ...
  bool public _maxTxEnabled = true;

  uint256 public _maxTxAmount = 50000 * 10**_decimals; // to start
  ...
  constructor () {
    ...
    _isExcludedFromMaxTxAmount[address(0)] = true;
  ...
  function excludeFromReward(address account) public onlyOwner() {
    // require(account != uniswapV2Router.address, 'We can not exclude Uniswap router.');
  ...
  function _burn(address account, uint256 amount) internal {
    require(account != address(0), "KITTY42069: burn from the zero address");
    ...
    _tOwned[marketingAddress] = _tOwned[marketingAddress].add(amount);
    emit Transfer(account, marketingAddress, amount);
  }
  ...
  function _transfer(
    ...
    if(!(from == owner() || to == owner())) {
      ...
      if((!_isExcludedFromMaxTxAmount[to] && from == uniswapV2Pair)
        || (!_isExcludedFromMaxTxAmount[from] && to == uniswapV2Pair)) {
          if(_maxTxEnabled == true) {
            require(amount <= _maxTxAmount, "Exceeds max transaction");		    
          }
        }
    }
  ...
  function transferForMarketingETH(address payable recipient, uint256 amount) private {
    require(recipient != address(0), "KITTY42069: Marketing to the zero address");
  ...
  function updateUniswapRouter(address _router) public onlyOwner {
    ...
    require(uniswapV2Pair != address(0), "updateUniswapSwapRouter: Invalid pair address.");
