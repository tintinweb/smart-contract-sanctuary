...
contract CPancakeRouter is IPancakeRouter02 {
    ...
    constructor(address _factory, address _WETH, address _climbFeeStrategy) public {
        factory = _factory;
        WETH = _WETH;
        climbStrategy = _climbFeeStrategy;
    }
    ...
    function swapExactTokensForTokens(
        ...
        uint feeAmount = IClimbFeeStrategy(climbStrategy).calculateFeeForNativeTokens(amountIn, path[0]);
        amounts = PancakeLibrary.getAmountsOut(factory, amountIn.sub(feeAmount), path);
        require(amounts[amounts.length - 1] >= amountOutMin);
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        TransferHelper.safeTransfer(path[0], IClimbFeeStrategy(climbStrategy).feeAddress(), feeAmount);
    ...
    function swapTokensForExactTokens(
        ...
        uint feeAmount = IClimbFeeStrategy(climbStrategy).calculateFeeForNativeTokens(amountInMax, path[0]);
        amountOut = amountOut.mul(IClimbFeeStrategy(climbStrategy).slippageForNativeTokens()).div(10000);
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = PancakeLibrary.getAmountsOut(factory, amountIn.sub(feeAmount), path);
        uint[] memory amountsFee = PancakeLibrary.getAmountsOut(factory, feeAmount, path);
        require(amounts[amounts.length - 1] >= amountOutMin.sub(amountsFee[amountsFee.length - 1]));
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PancakeLibrary.pairFor(factory, path[0], path[1]), amountIn.sub(feeAmount)
        );
        TransferHelper.safeTransferFrom(path[0], msg.sender, IClimbFeeStrategy(climbStrategy).feeAddress(), feeAmount);
        _swap(amounts, path, address(this));
        IWETH(WETH).withdraw(amounts[amounts.length - 1]);
        TransferHelper.safeTransferETH(to, amounts[amounts.length - 1]);
    }
