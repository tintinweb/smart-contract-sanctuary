...
contract CovidDistributor is IDividendDistributor {
    ...
    IERC20 RWRD = IERC20(0x98fdF9a90358f677Bb2b4b4a175990254b2585Fe);        
...
contract Depopulation is ERC20, Ownable {
    ...
    CovidDistributor public dividendDistributor;
    ...
    uint256 public maxTx = TOTAL_SUPPLY / 100 * (DECIMALS);                    // 1%  of total supply, 5   B tokens
    uint256 public swapTokensAtAmount = 5 * TOTAL_SUPPLY / 1000 * (DECIMALS);  // 0.5% of total supply, 500 M tokens
    ...
    event SetFees(uint256 indexed CovidRewardsFee, uint256 indexed marketingFee, uint256 indexed liquidityFee);
    ...
    constructor() ERC20("Depopulation", "B11529") {
        ...
        dividendDistributor = new CovidDistributor(address(_uniswapV2Router), owner());
    ...
    function updateDividendDistributor(address newAddress) public onlyOwner {
        require(newAddress != address(dividendDistributor), "Covid: The dividend distributor already has that address");

        CovidDistributor newDividendDistributor = CovidDistributor(payable(newAddress));

        require(newDividendDistributor._owner() == address(this), "Covid: The new dividend distributor must be owned by the Test token contract");
    ...
    function setSwapTokensAtAmount(uint256 _swapTokensAtAmount) public onlyOwner{
        require(2 * TOTAL_SUPPLY / 1000 <= _swapTokensAtAmount && _swapTokensAtAmount <= 2 * TOTAL_SUPPLY / 100,
