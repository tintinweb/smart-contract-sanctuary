...
NudeToken has the most insane $CAKE rewards that are generated from the 11% tax on each transaction. 
This is the highest $CAKE rewards of any reward coin released so far! You don’t need to do anything to claim your rewards, simply buy and hold NudeToken in your wallet. 
Your $CAKE rewards will automatically be deposited into your wallet every 60 mins. Make sure you use the $CAKE token address to track your rewards.

💋We all love watching porn, statistics say 97% of us do. 
So Our team here based in the USA thought why not make a coin backed by a porn site were we can all show our support for Porn. 
So we introduce to you $Nudes👅

🍑$Nudes Is a new coin which is being released with a Porn Streaming website/service🎥. 
On this service customers will be able to pay to using $Nudes tokens to watch porn videos and live streams 📸 . 
People will only be able to access the website If they are holders of the $Nudes token. 
Advertisers who wish to advertise on the porn site also have to purchase this token. 
This way people holding the coin and who use the streaming service will be rewarded for the ads being presented to them. 
Customers will also be able to donate to live pornstar streamers with the $Nudes token

How it will work? We have created a mechanism where users will be able to connect their trust wallet or metamask wallets to the website through the integrated API. 
Like you do when you connect on pancakeswap or uniswap. 
By doing this the website will be able to verify your a owner of the $Nudes token which will allow you to have unlimited access to the website.

📃Website: NudeToken.com 
🐦Twitter: https://twitter.com/nudestoken
📱Telegram: https://t.me/nudestoken
*/
...
contract LTC_DISTRIBUTOR is IDividendDistributor { // TO-DO change contract name
    ...
    IBEP20 LTC = IBEP20(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); // Reward address
...
contract Nudestoken is IBEP20, Auth { // TO-DO change contract name
    using SafeMath for uint256;

    address public LTC = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82; //Reward address
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address zeroAddress = 0x0000000000000000000000000000000000000000;
    address public burnAddress = 0x000000000000000000000000000000000000dEaD;
    address public externalAddress = 0x7df8eB228CE8E17B5892f7A16d8b87bBe6a21aEa; //TO-DO change external address
    address public autoLiquidityReceiver = 0x7df8eB228CE8E17B5892f7A16d8b87bBe6a21aEa; //TO-DO change liquidty owner address

    string constant _name = "Nudes Token";
    string constant _symbol = "NUDES";
    ...
    mapping (address => bool) public isLTCDividendExempt;
    ...
    constructor () Auth(msg.sender) {
        ...
        distributor = new LTC_DISTRIBUTOR(address(router));

        isFeeExempt[msg.sender] = true;
        isTxLimitExempt[msg.sender] = true;
        isLTCDividendExempt[pair] = true;
        isLTCDividendExempt[address(this)] = true;
        isLTCDividendExempt[burnAddress] = true;
        isLTCDividendExempt[zeroAddress] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isLTCDividendExempt[sender]){ try distributor.setShare(sender, _balances[sender]) {} catch {} }
        if(!isLTCDividendExempt[recipient]){ try distributor.setShare(recipient, _balances[recipient]) {} catch {} }
	...
	*/
    ...
    /**  
     * @dev set LTC distribution criteria
     * _minPeriod--> minimum time to wait before send
     * _minDistribution--> minimum amount of LTC reward before send to address
     */
    function setLTCDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external authorized {
        distributor.setDistributionCriteria(_minPeriod, _minDistribution);
    }

    /**  
     * @dev set LTC distributor's setting
     * gas--> gas fee to be used from sender's gas fee to send LTC reward
     */
    function setLTCDistributorSettings(uint256 gas) external authorized {
        require(gas < 750000);
        distributorGas = gas;
    }
	...
	*/
    ...
    function blacklistSingleWallet(address account) external authorized {
        ...
        setIsDividendExempt(account, true); // also remove LTC reward for address
    }
    ...
    function blacklistMultipleWallets(address[] calldata accounts) external authorized {
        ...
        for (uint256 i; i < accounts.length; ++i) {
            ...
            setIsDividendExempt(accounts[i], true); // also remove LTC reward for address
        }
    }
    ...
    function unBlacklistSingleWallet(address account) external authorized {
        ...
        setIsDividendExempt(account, false); // also includes in LTC reward for address
    }
    ...
    function unBlacklistMultipleWallets(address[] calldata accounts) external authorized {
        ...
        for (uint256 i; i < accounts.length; ++i) {
            ...
            setIsDividendExempt(accounts[i], false); // also includes in LTC reward for address
        }
    }
