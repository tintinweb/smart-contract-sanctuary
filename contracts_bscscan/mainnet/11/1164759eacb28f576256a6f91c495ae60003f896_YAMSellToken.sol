 ...
 * @author MUFC
 **/
contract YAMSellToken is Initializable {
  ...
  uint256 public tokenRateBnb;
  uint256 public tokenRateUsd;
   ...
   * @dev Update rate
   */
  function updateRateBnb(uint256 rate) public onlyIDOAdmin {
    tokenRateBnb = rate;
  } 
   ...
   * @dev Update rate
   */
  function updateRateUsd(uint256 rate) public onlyIDOAdmin {
    tokenRateUsd = rate;
  }
   ...
   * @dev Update rate for refferal
   */
  function updateRefBnb(uint256 _bnb_rate) public onlyIDOAdmin {
    bnb_rate = _bnb_rate;
  }
  ...
  function buyTokenBNB(address recipient, address _referrer) public payable returns (bool) {
    if (referrers[msg.sender] == address(0) && _referrer != address(0) && msg.sender != _referrer && msg.sender != referrers[_referrer]) {
        referrers[msg.sender] = _referrer;
        emit NewReferral(_referrer, msg.sender, 1);
    }
    
    //uint256 price = getBNBPrice(); <--bnb price, we will not use this because we need to set a fixed rate
    uint256 bnbAmount = msg.value;
    uint256 sold_amount = (tokenRateBnb * bnbAmount) / 1e18; //value tkns to trsf
	
	// send ref reward BnB
    if (_referrer != address(0) && _referrer != 0x000000000000000000000000000000000000dEaD){ //cannot waste bnb :p
        uint256 fb_reward = (msg.value).div(bnb_rate); //perc ref //2 = 50%, 4 = 25%, 5 = 20%, 10 = 10%, 50 = 2%
       //code to pay ref bnb hereeeee
	   _safeTransferBNB(_referrer, fb_reward);
	}
	
    // send ref reward Token
    if (referrers[msg.sender] != address(0)){
        uint256 f1_reward = sold_amount.mul(f1_rate).div(100); //rate perc to send to referer, accord to real perc this time
    ...
    emit SellIDO(msg.sender, sold_amount, bnbAmount);
    return (true);
  }
   ...
   * @dev execute buy Token
   **/
  function buyIDOUSD(address recipient, uint256 buy_amount, address token, address _referrer) public returns (uint256) {
    ...
    uint256 sold_amount = (buy_amount * 1e18) / tokenRateUsd;
    IERC20(IDO_TOKEN).transfer(recipient, sold_amount);
    // send ref reward
    if (referrers[msg.sender] != address(0)){
