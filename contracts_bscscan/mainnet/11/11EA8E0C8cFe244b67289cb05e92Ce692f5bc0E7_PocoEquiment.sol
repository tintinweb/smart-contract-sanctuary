...
contract PocoEquiment is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, OwnableUpgradeable, ERC721BurnableUpgradeable {
    ...
    function safeMintEvent(address to, uint256 _itemTypeID, uint256 _itemID, uint256 _additionValue) public onlyWhitelistMinter {
        _safeMint(to, _tokenIdCounter.current());
        emit NewEquiment(_tokenIdCounter.current(), to);
        properties[_tokenIdCounter.current()].itemTypeID = _itemTypeID;
        properties[_tokenIdCounter.current()].itemID = _itemID;
        properties[_tokenIdCounter.current()].additionValue = _additionValue;
        _tokenIdCounter.increment();    
    }
