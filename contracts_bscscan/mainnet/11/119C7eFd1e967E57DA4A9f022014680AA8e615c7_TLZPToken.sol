...
// TLZPToken with Governance.
contract TLZPToken is BEP20('TLZP', 'TLZP'), Pausable, ITLZP, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    modifier onlyAuthorizedMintCaller() {
        require(_msgSender() == owner() || _authorizedMintCaller[_msgSender()],"TLZP: MINT_CALLER_NOT_AUTHORIZED");
        _;
    }
    ...
    function stake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(_unlockFactor[token] > 0, "TLZP: FACTOR_NOT_SET");
        require(_unlockBlockGap[token] > 0, "TLZP: BLOCK_GAP_NOT_SET");
    ...
    function unstake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(amount > 0, "TLZP: ZERO_UNSTAKE_AMOUNT");
        LpStakeInfo storage info = _stakingRecords[_msgSender()][token];
        require(amount <= info.amountStaked, "TLZP: UNSTAKE_AMOUNT_EXCEEDED");
        uint256 unlockedAmount = _settleUnlockAmount(_msgSender(), token, info.amountStaked, info.blockNumber);
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "TLZP::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "TLZP::delegateBySig: invalid nonce");
        require(now <= expiry, "TLZP::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "TLZP::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying TLZPs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "TLZP::_writeCheckpoint: block number exceeds 32 bits");
