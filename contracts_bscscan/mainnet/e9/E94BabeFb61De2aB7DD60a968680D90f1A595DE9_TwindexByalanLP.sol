...
import "../../interfaces/dopplex/ITwindexSwapRouter.sol";
import "../../interfaces/dopplex/ITwindexPair.sol";
import "../../interfaces/dopplex/IMasterChef.sol";
import "../../interfaces/IByalan.sol";

import "../ByalanIsland.sol";
import "../Sailor.sol";

contract TwindexByalanLP is ByalanIsland, Sailor, IByalan, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using Address for address payable;

    // Tokens used
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant DOPX = 0x802A183ac9F6b082716DFeE55432ed0C04ACB49a;
    address public immutable override want;
    address public immutable lpToken0;
    address public immutable lpToken1;

    // Third party contracts
    address public constant MASTERCHEF = 0x45069E9632d1b29d16C5E06dc9Ed8F33B6aBe6DB;
    uint256 public immutable pid;

    // Routes
    address[] public dopxToWbnbRoute;
    address[] public dopxToLp0Route;
    address[] public dopxToLp1Route;
    ...
    constructor(
        ...
        address _harvester,
        uint256 _pid,
        address[] memory _dopxToLp0Route,
        address[] memory _dopxToLp1Route
    )
        ByalanIsland(
            _hydra,
        ...
        )
    {
        ...
        lpToken0 = ITwindexPair(want).token0();
        lpToken1 = ITwindexPair(want).token1();

        dopxToWbnbRoute = [DOPX, WBNB];
        if (lpToken0 != DOPX) {
            require(_dopxToLp0Route[0] == DOPX, "invalid lp 0 route");
            require(_dopxToLp0Route[_dopxToLp0Route.length - 1] == lpToken0, "invalid lp 0 route");
            require(
                ITwindexSwapRouter(unirouter).getAmountsOut(1 ether, _dopxToLp0Route)[_dopxToLp0Route.length - 1] > 0,
                "invalid lp 0 route"
            );
            dopxToLp0Route = _dopxToLp0Route;
        }

        if (lpToken1 != DOPX) {
            require(_dopxToLp1Route[0] == DOPX, "invalid lp 1 route");
            require(_dopxToLp1Route[_dopxToLp1Route.length - 1] == lpToken1, "invalid lp 1 route");
            require(
                ITwindexSwapRouter(unirouter).getAmountsOut(1 ether, _dopxToLp1Route)[_dopxToLp1Route.length - 1] > 0,
                "invalid lp 1 route"
            );
            dopxToLp1Route = _dopxToLp1Route;
        }
    ...
    function chargeFees() internal nonReentrant {
        uint256 toBnb = (IERC20(DOPX).balanceOf(address(this)) * totalFee) / MAX_FEE;
        ITwindexSwapRouter(unirouter).swapExactTokensForETH(toBnb, 0, dopxToWbnbRoute, address(this), block.timestamp);
    ...
    function addLiquidity() internal {
        uint256 dopxHalf = IERC20(DOPX).balanceOf(address(this)) / 2;
        ...
        ITwindexSwapRouter(unirouter).addLiquidity(
            lpToken0,
            lpToken1,
        ...
        returns (IERC20[] memory rewardTokens, uint256[] memory rewardAmounts)
    {
        ...
        rewardTokens[0] = IERC20(DOPX);
        rewardAmounts[0] =
            IMasterChef(MASTERCHEF).pendingDoppleX(pid, address(this)) +
            IERC20(DOPX).balanceOf(address(this));
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(DOPX).safeApprove(unirouter, type(uint256).max);

        // lp token 0 and 1 maybe dopx so approve 0 is needed here
    ...
    function _removeAllowances() internal {
        ...
        IERC20(DOPX).safeApprove(unirouter, 0);
        IERC20(lpToken0).safeApprove(unirouter, 0);
        IERC20(lpToken1).safeApprove(unirouter, 0);
    }
...
interface IMasterChef {
    ...
    function pendingDoppleX(uint256 _pid, address _user) external view returns (uint256);
