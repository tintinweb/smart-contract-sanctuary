...
contract BABYCAKEDOGE is ERC20, Ownable {
    ...
    BABYCAKEDOGEDividendTracker public dividendTracker;

    address public buyBackWallet = 0xF7dF3C968Dc16C5f77b462723C865E4cD8c2b2E5;
    
    uint256 public maxBuyTranscationAmount = 50000000000000 * (10**9);
    uint256 public maxSellTransactionAmount = 25000000000000 * (10**9);
    uint256 public swapTokensAtAmount = 2000000000 * (10**9);
    uint256 public maxWalletToken = 75000000000000 * (10**9); // 1.5% of total supply
    ...
    constructor() ERC20("BabyCakeDoge", "BABYCAKEDOGE") {
    	...
    	dividendTracker = new BABYCAKEDOGEDividendTracker();
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "BABYCAKEDOGE: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "BABYCAKEDOGE: Marketing divisor must be between 0 (0%) and 100 (100%)");
  	...
  	function prepareForPreSale() external onlyOwner {
        ...
        marketingFee = 0;
        maxBuyTranscationAmount = 50000000000000 * (10**9);
        maxWalletToken = 75000000000000 * (10**9);
    }
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "BABYCAKEDOGE: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BABYCAKEDOGE: The dividend tracker already has that address");

        BABYCAKEDOGEDividendTracker newDividendTracker = BABYCAKEDOGEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BABYCAKEDOGE: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "BABYCAKEDOGE: Dividend reward tax must be between 0 and 10");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BABYCAKEDOGE: Account is already the value of 'excluded'");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "BABYCAKEDOGE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BABYCAKEDOGE: gasForProcessing must be between 200,000 and 500,000");
...
contract BABYCAKEDOGEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BABYCAKEDOGE_Dividend_Tracker", "BABYCAKEDOGE_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BABYCAKEDOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BABYCAKEDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BABYCAKEDOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BABYCAKEDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BABYCAKEDOGE_Dividend_Tracker: Cannot update claimWait to same value");
