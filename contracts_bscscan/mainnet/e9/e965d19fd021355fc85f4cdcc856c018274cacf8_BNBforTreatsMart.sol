...
interface TreatOfTheWeekMart {
    ...
    function isGiveAwayNFT(uint256 _id) external view returns (bool);
}
...
contract BNBforTreatsMart is Ownable {
    ...
    constructor(TreatNFTMinterV2 _TreatNFTMinterAddress, address payable _TreatTreasuryAddress, address _treatDaoAddress, address _totwMartAddress, address _sweetShopAddress) public {
        treatNFTMinter = _TreatNFTMinterAddress;
        totwMart = TreatOfTheWeekMart(_totwMartAddress);
        sweetShop = SweetShop(_sweetShopAddress);
        treatTreasuryAddress = _TreatTreasuryAddress;
        melonNumber = 73400000000000000000000;
    ...
    function migrateListings(uint256 startNftId, uint256 endNftId) public onlyOwner {
        ...
        for(uint256 i = 0; i < returnArrayLen; i++) {
            nftCosts[startNftId+i] = sweetShop.nftCosts(startNftId+i);
            isGiveAwayCard[startNftId+i] = sweetShop.isGiveAwayCard(startNftId+i);
            theNftIds[i] = startNftId+i;
            theNftCosts[i] = sweetShop.nftCosts(startNftId+i);
            theNftGiveAwayFlags[i] = sweetShop.isGiveAwayCard(startNftId+i);
        }
    }
    ...
    function addNFT(uint256[] memory nftIds, uint256[] memory amounts) public {
        for (uint256 i = 0; i < nftIds.length; ++i) {
            ...
            require(totwMart.isGiveAwayNFT(nftIds[i]) == false, "cannot list nfts in totw sale contract");
        }
    ...
    function addGiveAwayTreat(uint256[] memory nftIds) public {
        for (uint256 i = 0; i < nftIds.length; ++i) {
            ...
            require(totwMart.isGiveAwayNFT(nftIds[i]) == false, "cannot giveaway nfts in totw sale contract");
        }
        require(pausedTreats == false, "Contract Paused");
        for (uint256 i = 0; i < nftIds.length; ++i) {
            require(treatNFTMinter.maxSupply(nftIds[i]) > 0, "NFT doesn't exist");
    ...
    function redeem(uint256 _nft) payable public {
        ...
        (bool success, ) = address(treatTreasuryAddress).call.value(treasuryTake)("");
    ...
    function updateTotwMart(address payable _newTotwMartAddress) public onlyOwner {
        require(_newTotwMartAddress != address(0), "cannot switch totwMart to the zero address");
        totwMart = TreatOfTheWeekMart(_newTotwMartAddress);
    }

    function updateYeOlSweetShop(address _newSweetShopAddress) public onlyOwner {
        sweetShop = SweetShop(_newSweetShopAddress);
    }
    
    function updateTreatDao(address _newTreatAddress) public onlyOwner {
        treatDaoToken = TREATcontract(_newTreatAddress);
    }
