...
// File: contracts/base/mixins/NFTMarketAuction.sol
...
// File: contracts/base/mixins/NFTMarketCreators.sol
  ...
  ReentrancyGuardUpgradeable // Adding this unused mixin to help with linearization
{
  ...
  function _getOwner(address nftContract, uint256 tokenId) internal view returns (address payable) {
    try INFT721(nftContract).ownerOf(tokenId) returns (address creator) {
...
// File: contracts/base/mixins/SendValueWithFallbackWithdraw.sol
  ...
  SendValueWithFallbackWithdraw
{
    ...
    )
  {
    ...
    // ) {
    //   creatorFeeTo = creator;
    // } catch // solhint-disable-next-line no-empty-blocks
    // {
        
    // }
    // foundationFee = price.mul(_primaryMarketFeeBasisPoints).div(BASIS_POINTS);
    // creatorFee = price.mul(_secondaryCreatorFeeBasisPoints).div(BASIS_POINTS);
    // ownerRev = price.sub(foundationFee).sub(creatorFee);
...
function _getFeesNew(
    ...
    )
  {
    ...
    creatorSecondaryFee = price.mul(_secondaryCreatorFeeBasisPoints) / BASIS_POINTS;
    foundationFee = price.mul(_primaryMarketFeeBasisPoints) / BASIS_POINTS;
    ownerRev = price.sub(foundationFee).sub(creatorSecondaryFee);
  }
  
  function _distributeFundsNew(
    address payable seller,
    uint256 price,
    address payable creator
  )
    ...
    )
  {
    // address payable creatorFeeTo;
    address payable ownerRevTo;
    (foundationFee, creatorFee, ownerRevTo, ownerRev) = _getFeesNew(seller, price);
    ...
    _sendValueWithFallbackWithdrawWithLowGasLimit(getTreasury(), foundationFee);
    _sendValueWithFallbackWithdrawWithMediumGasLimit(creator, creatorFee);
    _sendValueWithFallbackWithdrawWithMediumGasLimit(ownerRevTo, ownerRev);
  }
  /**
...
// File: contracts/base/mixins/NFTMarketReserveAuction.sol
  ...
  NFTMarketAuction
{
  ...
  function cancelReserveAuction(uint256 auctionId) public nonReentrant {
    ...
    if(auction.contractType==721){
      IERC721Upgradeable(auction.nftContract).transferFrom(address(this), auction.seller, auction.tokenId); 
    } else {
      IERC1155Upgradeable(auction.nftContract).safeTransferFrom(address(this), auction.seller, auction.tokenId, 1, "");
    }
  ...
  function finalizeReserveAuction(uint256 auctionId) public nonReentrant {
    ...
    if(auction.contractType==721){
      ...
      IERC1155Upgradeable(auction.nftContract).safeTransferFrom(address(this), auction.bidder, auction.tokenId, 1, "");
    }
   ...
   */
  function adminAccountMigrationAuction(
...
//   ERC1155HolderUpgradeable
{
  ...
  function _getSellerFor(address nftContract, uint256 tokenId)
    ...
    returns (address payable)
  {
    address payable seller = saleIdToSale[nftContractToTokenIdToSaleId[nftContract][tokenId]].seller;
    if (seller == address(0)) {
      return super._getSellerFor(nftContract, tokenId);
    }
    return seller;
  ...
  function createSale(
    ...
    require(contractType==721 || contractType==1155,"Wrong contract type");
    // If an sale is already in progress then the NFT would be in escrow and the modifier would have failed
    uint256 saleId = _getNextAndIncrementSaleId();
    nftContractToTokenIdToSaleId[nftContract][tokenId] = saleId;
  ...
  function cancelSale(uint256 saleId) public nonReentrant {
    ...
    if(sale.contractType==721){
      IERC721Upgradeable(sale.nftContract).transferFrom(address(this), sale.seller, sale.tokenId); 
    } else {
      IERC1155Upgradeable(sale.nftContract).safeTransferFrom(address(this), sale.seller, sale.tokenId, 1, "");
    }
    
    delete nftContractToTokenIdToSaleId[sale.nftContract][sale.tokenId];
    delete saleIdToSale[saleId];
    emit SaleCanceled(saleId);
  }
  
  function buyToken(uint256 saleId) public payable nonReentrant {
    Sale storage sale = saleIdToSale[saleId];
    require(sale.price != 0, "NFTMarketSale: Sale not found");
    require(sale.price <= msg.value, "NFTMarketSale: Value must be greater than or equal to price");

    sale.buyer = msg.sender;

    if(sale.contractType==721){
      IERC721Upgradeable(sale.nftContract).transferFrom(address(this), msg.sender, sale.tokenId); 
    } else {
      IERC1155Upgradeable(sale.nftContract).safeTransferFrom(address(this), msg.sender, sale.tokenId, 1, "");
    }

    // try INFT721(sale.nftContract).getTokenCreatorPaymentAddress(sale.tokenId) returns (
    //   address payable tokenCreatorPaymentAddress
    // ) {
    //   creator = tokenCreatorPaymentAddress;
    // } catch // solhint-disable-next-line no-empty-blocks
    // {
        
    // }
    // uint256 marketFee =  sale.price.mul(500).div(BASIS_POINTS);
    // uint256 creatorFee = sale.price.mul(200).div(BASIS_POINTS);
    // uint256 ownerRev = sale.price.sub(creatorFee).sub(marketFee);
    // uint256 marketFee =  (msg.value*1000)/BASIS_POINTS;
    // uint256 creatorFee = (msg.value*300)/BASIS_POINTS;
    // uint256 ownerRev = msg.value-marketFee-creatorFee;
    
    // _sendValueWithFallbackWithdrawWithLowGasLimit(getTreasury(),marketFee);
    // _sendValueWithFallbackWithdrawWithMediumGasLimit(sale.seller, creatorFee);
    // _sendValueWithFallbackWithdrawWithMediumGasLimit(sale.seller, ownerRev);
    // address payable tokenSeller= _getOwner(sale.nftContract, sale.tokenId);
    // address payable creator = _getCreator(sale.nftContract, sale.tokenId);
    
    // (uint256 marketFee, uint256 creatorFee, uint256 ownerRev) =
    //   _distributeFundsNew(tokenSeller, msg.value, creator);
      
    (uint256 marketFee, uint256 creatorFee, uint256 ownerRev) =
      _distributeFunds(sale.nftContract, sale.tokenId, sale.seller, sale.price);
      
    delete nftContractToTokenIdToSaleId[sale.nftContract][sale.tokenId];
    delete saleIdToSale[saleId];
    
    emit SaleFinalized(saleId, sale.seller, sale.buyer, marketFee, creatorFee, ownerRev);
  }
  ...
  ERC1155HolderUpgradeable
{
  ...
  function _getSellerFor(address nftContract, uint256 tokenId)
    ...
    view
    virtual
    override(NFTMarketCore, NFTMarketReserveAuction, NFTMarketSale)
    returns (address payable)
  {
    return super._getSellerFor(nftContract, tokenId);
  }
...
contract PocmonMarket is NFTMarket {
}