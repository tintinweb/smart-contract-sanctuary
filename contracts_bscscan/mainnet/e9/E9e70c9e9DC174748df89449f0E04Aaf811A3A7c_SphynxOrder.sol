...
contract SphynxOrder is Ownable, ReentrancyGuard, KeeperCompatible {
    ...
    event SphynxTokenUpdated (
        address newSphynx
    );
    event SphynxFeeUpdated (
        uint256 newFee
    );
    event MaxTxPoolUpdated (
        uint256 newLength
    );
    event KeepersRegistryUpdated (
        address newKeepersRegistry
    );
    ...
    function updateKeepersRegistry(address _keepersRegistry) external onlyOwner {
        keepersRegistry = _keepersRegistry;
        emit KeepersRegistryUpdated(_keepersRegistry);
    }

    function createOrder(uint256 _amountA, uint256 _amountB, address _tokenA, address _tokenB, uint256 _expiredAt, address _router) external payable nonReentrant {
        require(routers[_router], "not-router");
        address pair = ISphynxFactory(ISphynxRouter(_router).factory()).getPair(_tokenA, _tokenB);
        require(pair != address(0), "pair-does-not-exist");
        require(msg.value >= staticFee, "insuffient-fee");
        ...
        if(sphynxToken != address(0)) {
            IERC20(sphynxToken).safeTransferFrom(msg.sender, feeWallet, sphynxFee);
        }
        emit OrderCreated(currentOrderId, _amountA, _amountB, _tokenA, _tokenB, msg.sender, _router);
        currentOrderId++;
    }
    ...
    function _removeOrderIds(uint256 _orderId) internal {
        ...
        for(;i < userOrderIds[msg.sender].length;i++) {
            if(userOrderIds[msg.sender][i] == _orderId) {
                userOrderIds[msg.sender][i] = orders[userOrderIds[msg.sender].length - 1].id;
    ...
    function _performOrders(uint256 _orderId) internal {
        ...
        tokenAContract.safeTransferFrom(creator, address(this), currentOrder.amountA);
        ISphynxRouter router = ISphynxRouter(currentOrder.router);
        tokenAContract.approve(currentOrder.router, tokenAContract.balanceOf(address(this)));
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(tokenAContract.balanceOf(address(this)), 0, path, creator, block.timestamp + 120);
        emit OrderMatched(currentOrder.id, currentOrder.tokenA, currentOrder.tokenB, currentOrder.amountA, currentOrder.amountB);
        _removeActiveOrderId(_orderId);
    }

    function executeOrders(uint256[] memory _orderIds, uint256 _index, uint256[] memory matchOrderIds, uint256 matchedIndex) public {
    ...
    function checkUpkeep(bytes calldata) external view override returns (bool upkeepNeeded, bytes memory performData) {
        ...
        OrderMatchHelper[] memory matchHelpers = new OrderMatchHelper[](activeOrderIds.length);
        uint256 matchedIndex;
        uint256 tokenIndex;
        uint256 expiredIndex;
        for (uint256 i = 0; i < activeOrderIds.length; i++) {
            if( expiredIndex + matchedIndex > maxTxPoolLength ) { // check if it's reach max Tx length
                break;
            }
            ...
            for(uint256 tokenKey = 0; tokenKey < tokenIndex; tokenKey++) {
                if(matchHelpers[tokenKey].tokenA == path[0] && matchHelpers[tokenKey].tokenB == path[1]) {
                    tokenIndexFlag = true;
            ...
            if(tokenIndexFlag) {
                uint256[] memory amountsOut = router.getAmountsOut(curTokenIn + currentOrder.amountA, path);
                uint256 amountOut = amountsOut[1];
                if(amountOut > currentOrder.amountB + curTokenOut) {
                    matchOrderIds[matchedIndex] = activeOrderIds[i];
                    matchedIndex++;
                    matchHelpers[curTokenIndex].tokenIn = curTokenIn + currentOrder.amountA;
                    matchHelpers[curTokenIndex].tokenOut = curTokenOut + currentOrder.amountB;
                }
            } else {
                uint256[] memory amountsOut = router.getAmountsOut(currentOrder.amountA, path);
                uint256 amountOut = amountsOut[1];
                if(amountOut > currentOrder.amountB) {
                    matchOrderIds[matchedIndex] = activeOrderIds[i];
                    matchedIndex++;
                    matchHelpers[tokenIndex].tokenIn = currentOrder.amountA;
                    matchHelpers[tokenIndex].tokenOut = currentOrder.amountB;
        ...
        if(expiredIndex > 0 || matchedIndex > 0) {
            upkeepNeeded = true;
            performData = abi.encodeWithSignature("executeOrders(uint256[],uint256,uint256[],uint256)", expiredOrderIds, expiredIndex, matchOrderIds, matchedIndex);
        }
    }
    ...
    function getUserOrders() external view returns(Order[] memory) {
        Order[] memory userOrders = new Order[](userOrderIds[msg.sender].length);
        uint256 orderIndex;
        for(uint i = 0; i < userOrderIds[msg.sender].length; i++) {
