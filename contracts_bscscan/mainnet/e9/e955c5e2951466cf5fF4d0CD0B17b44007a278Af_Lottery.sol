...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("Perseus Lottery Ticket", "PLT") {}

    function newLotteryItem(address player, uint256 _lotteryNumbers, uint256 _amount, uint256 _issueIndex)
        external onlyOwner
        returns (uint256)
    {
        tokenIds.increment();
    ...
    function getLotteryNumbers(uint256 tokenId) external view returns (uint256) {
...
contract Lottery is Ownable {
    ...
    address public marketingWallet = 0xa84ec4a5D5035bbAea09F93cbBb54C53d59b296C;
    address public devWallet = 0xa84ec4a5D5035bbAea09F93cbBb54C53d59b296C;
    address private burnWalletAddress = 0x000000000000000000000000000000000000dEaD;
    address public ANM = 0x2cc970e45B359D24eE76B4902FB6E9F50375a8a6;
    ...
    Ownable()
    {
        ...
        ANMP.approve(address(uniswapV2Router), uint256(-1));
    }

    uint256 private nullTicket = 0;
    ...
    function drawing(uint256 _externalRandomNumber) public onlyOwner {
        ...
        winningNumbers=uint256(_randomNumber);

        historyNumbers[issueIndex] = winningNumbers;
        historyAmount[issueIndex] = totalAmount;
        drawingPhase = false;

        ANMP.transfer( burnWalletAddress, totalAmount.div(10) );
        ANMP.transfer( marketingWallet, totalAmount.div(25) );
        ANMP.transfer( devWallet, totalAmount.div(100) );
        uint256 buyBurnAmount = totalAmount.div(20);
    ...
    function internalBuy(uint256 _price, uint256 _numbers) internal {
        require (!drawed(), 'drawed, can not buy now');
    ...
    function buy(uint256 _price) external inDrawingPhase {
        require (_price >= minPrice, 'price must above minPrice');
        maxNumber += 1;
        uint256 tokenId = lotteryNFT.newLotteryItem(msg.sender, maxNumber, _price, issueIndex);
        lotteryInfo[issueIndex].push(tokenId);
        ...
        totalAmount = totalAmount.add(_price);
        lastTimestamp = block.timestamp;
        ANMP.safeTransferFrom(address(msg.sender), address(this), _price);
        emit Buy(msg.sender, tokenId);
    }

    function multiBuy(uint256 _price, uint256 _numbers) external inDrawingPhase {
        require (_price >= minPrice, 'price must above minPrice');
        ...
        ANMP.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            ANMP.safeTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            ANMP.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function getTotalRewards(uint256 _issueIndex) public view returns(uint256) {
        ...
        return historyAmount[_issueIndex];
    }
    ...
    function getRewardView(uint256 _tokenId) public view returns(uint256) {
        ...
        uint256 lotteryNumbers = lotteryNFT.getLotteryNumbers(_tokenId);
        uint256 _winningNumbers = historyNumbers[_issueIndex];
        require(_winningNumbers != 0, "not drawed");

        uint256 reward = 0;
        if (_winningNumbers == lotteryNumbers) {
            uint256 amount = lotteryNFT.getLotteryAmount(_tokenId);
            uint256 poolAmount = getTotalRewards(_issueIndex);
            reward = amount.mul(poolAmount).mul(80).div(100);
        }
        return reward;
    }
    ...
    function seedLottery(uint256 _amount) external onlyOwner {
        ...
        ANMP.safeTransferFrom(address(msg.sender), address(this), _amount);
    }

    function adminWithdraw(uint256 _amount) external onlyOwner {
        ANMP.safeTransfer(address(msg.sender), _amount);
