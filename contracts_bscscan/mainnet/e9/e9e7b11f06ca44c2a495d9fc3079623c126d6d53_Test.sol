...
contract Test is ReentrancyGuard, OwnableUpgradeable {
    ...
    event Lock(address indexed user, address indexed referral,  uint256 amount);
    event Claim(
    ...
    function setTotalRaisingInUSDC(uint256 _totalRaisingInUSDC) public onlyAdmin {
        totalRaisingInUSDC = _totalRaisingInUSDC;
    }
    ...
    /// @dev Deposit ERC20 tokens with support for reflect tokens
    function lockWithRef(uint256 _amount, address _referral) public onlyActiveIAO nonReentrant {
        ...
        totalRaisingInBUSD += finalDepositAmount;
        emit Lock(msg.sender, _referral, finalDepositAmount);
    }
    ...
    function claim() external nonReentrant onlyFinishedIAO {
        ...
        uint256 refundingTokenAmount = getBUSDRefundAmount(msg.sender);
        ...
        emit Claim(msg.sender, refundingTokenAmount);
    }
    ...
    function getUserAllocation(address user) public view returns (uint256) {
        ...
        // 1e4 = 1%
        // 1 = 0.0001%
        return userInfo[user].amount * 1e6 / (totalRaisingInUSDC * 1e12 + totalRaisingInBUSD);
    }
    ...
    function getOfferingAmount(address user) public view returns (uint256) {
        if (totalWeiCap <= totalRaisingInBUSD + totalRaisingInUSDC * 1e12) {
            ...
            return userInfo[user].amount * rate; // (/ 1e6 * rate * 1e18)
        }
    }
    ...
    function getBUSDRefundAmount(address _user) public view returns (uint256) {
    ...
    function userTokenStatus(address _user) public view returns (uint256 stakeTokenHarvest, uint256 offeringTokenHarvest) {
        stakeTokenHarvest = getBUSDRefundAmount(_user);
    ...
    function finalWithdraw(uint256 _stakeTokenAmount) external onlyOwner {
