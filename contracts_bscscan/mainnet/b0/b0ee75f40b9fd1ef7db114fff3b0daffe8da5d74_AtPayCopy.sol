...
Token Name: AFTERPAY
Token Symbol: AFTERPAY
Total Supply: 100,000,000,000,000  100 trillion
Token Decimal: 18
Initial max transaction = 15 mil token 15000000  (everybody has a fair chance)


dev fee = 5% set for wallet 0xba95d5B9019682638AABE09fA9f13276000397F3

tax fee = 5% for token reflection for redistribution

note: above mentioned taxes can be changed by owner at any time by external functions




Smart Chain Address: 0xba95d5B9019682638AABE09fA9f13276000397F3
...
contract AtPayCopy is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "AFTERPAY";
    string private _symbol = "AFTERPAY";
    ...
    uint256 internal _tokenTotal = 100000000000000 *10**18; // 100 Trillion totalSupply
...
function _approve(address owner, address spender, uint256 amount) private {
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        uint256 devWalletFee = amount.mul(_devWalletFee).div(10000);
        ...
        if (burningFee > 0){
         ...
         //@dev devWallet fee
        if (devWalletFee > 0){
            transferAmount = transferAmount.sub(devWalletFee);
            _reflectionBalance[devWalletAddress] = _reflectionBalance[devWalletAddress].add(devWalletFee.mul(rate));
            _devWalletTotal = _devWalletTotal.add(devWalletFee);
            emit Transfer(account,devWalletAddress,devWalletFee);
        }
    ...
    function setTaxFee(uint256 fee) public onlyOwner {
     ...
     function setDevWalletFee(uint256 fee) public onlyOwner {
        _devWalletFee = fee;
    }
    ...
    function setBurningAddress(address _Address) public onlyOwner {
     ...
     function setDevWalletAddress(address _Address) public onlyOwner {
        require(_Address != devWalletAddress);
        
       devWalletAddress = _Address;
    }
