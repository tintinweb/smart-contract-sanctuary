...
contract NFT is ERC721URIStorage, Ownable, AccessControl {
    ...
    constructor() ERC721("EverestNFT", "EV") {
    ...
    function mint(string memory _tokenURI, address _toAddress, uint _price) public _minter returns (uint) {
        uint _tokenId = latestTokenId + 1;
    ...
    function redeem(string memory _tokenURI, address account, uint _price, bytes calldata signature) public payable returns (uint) {
        require(msg.value >= _price, "Error, the amount is lower");
        // require(_verify(_hash(account, _tokenURI), signature), "Invalid signature");
        require(hasRole(MINTER_ROLE, verify(account, _tokenURI, signature)), "Invalid signature");
        uint _tokenId = latestTokenId + 1;
        latestTokenId = _tokenId;
        price[_tokenId] = _price;
    ...
    function _hash(address account, string memory tokenURI)
    internal pure returns (bytes32)
    {
        return ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(tokenURI, account)));
    }
    ...
    function buy(uint _id) external payable {
        require(sellingEnabled == true, "Trading assets is disabled.");
    ...
    function haltAssetTrading() external {
        require(msg.sender == _contractOwner, "Error, you are not the owner");
        sellingEnabled = false;
    }

    function resumeAssetTrading() external {
        require(msg.sender == _contractOwner, "Error, you are not the owner");
        sellingEnabled = true;
    }
    ...
    function changeOwner(address newOwner) external {
        require(msg.sender == _contractOwner, "Error, you are not the owner");
        _contractOwner = payable(newOwner);
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }
