...
contract Buy {
    ...
    IERC20 public RageToken =
        IERC20(0xD38c1B7b95D359978996e01B8a85286F65B3C011);
        
    address payable companyAddress = payable(0x1e9E8f80E4808c331DF509685d10d7708A08E8fa);
    ...
    function buyNFTRage(address _erc721) public returns (bool) {
        require(card[_erc721] != 0, "No Card Available");
        require(MyIERC721(_erc721).available() > 0, "Card Minted Out");
        require(
            IERC20(RageToken).allowance(msg.sender, address(this)) >= card[_erc721],
            "Not Enough Token Allowance"
        );
        require(
            IERC20(RageToken).balanceOf(msg.sender) >= card[_erc721],
            "Not Enough Token"
        );
        IERC20(RageToken).safeTransferFrom(msg.sender, companyAddress, card[_erc721]);

        MyIERC721(_erc721).mint(msg.sender);
        emit BoughtNFT(msg.sender, card[_erc721]);
        return true;
    }
    
    
    function addNFT (address[] memory _erc721, uint256[] memory price) public onlyAdmin {
        require(_erc721.length == price.length,"Data Mismatch");
        for(uint i = 0; i < _erc721.length; i++) {
            require(price[i] > 0, "Not a valid price");
            card[_erc721[i]] = price[i];
            emit AddedNFT(_erc721[i], price[i]);   
        }
    }
