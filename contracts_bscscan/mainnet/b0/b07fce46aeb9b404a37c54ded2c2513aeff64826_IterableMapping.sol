...
A community driven, DeFi token. In each trade, three simple functions occur:
Burn, LP Acquisition, and Development growth. 
Through this tokenomics model, the project is benefitted as a whole with every single transaction. 
Shiba Payment transactions benefit ALL holders..

Website: http://shibapayment.org
Twitter: https://www.twitter.com/shiba_payment
Medium: https://medium.com/@paymentshiba
Telegram: https://t.me/shibapayment
*/
...
contract ShibaPayment is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ShibaPaymentDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 private maxBuyTranscationAmount = 100000000000 * (10**18);
    uint256 private maxSellTransactionAmount = 100000000000 * (10**18);
    uint256 public swapTokensAtAmount = 200000 * (10**18);
    uint256 private maxWalletToken = 100000000000 * (10**18); 
    ...
    constructor() ERC20("ShibaPay", "ShibaPayment") {
    	...
    	dividendTracker = new ShibaPaymentDividendTracker();

    	buyBackWallet = 0x023cd0157a03a5Ab73Cd9183859f10F6B1dcC546;
    	
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //0xea7595317710Cd3699760b0C37D313B50f63B66f
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "ShibaPayment: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "ShibaPayment: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "ShibaPayment: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ShibaPayment: The dividend tracker already has that address");

        ShibaPaymentDividendTracker newDividendTracker = ShibaPaymentDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ShibaPayment: The new dividend tracker must be owned by the ShibaPayment token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "ShibaPayment: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ShibaPayment: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ShibaPayment: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ShibaPayment: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ShibaPayment: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "ShibaPayment: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ShibaPayment: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ShibaPayment: Cannot update gasForProcessing to same value");
...
contract ShibaPaymentDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ShibaPayment_Dividend_Tracker", "ShibaPayment_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ShibaPayment_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ShibaPayment_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ShibaPayment contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "ShibaPayment_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ShibaPayment_Dividend_Tracker: Cannot update claimWait to same value");
