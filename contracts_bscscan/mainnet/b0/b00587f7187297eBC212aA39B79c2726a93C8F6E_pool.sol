...
contract pool {
    ...
    function getBUST_BUSDLP(uint _amount, address addr,uint opt) internal {
        ...
        uint256 bust_rec = router.getAmountsOut(BUSD.balanceOf(address(this)),p20)[p20.length - 1];
    ...
    function getBUST_BNBLP(uint _amount, address addr, uint opt) internal{
        ...
        uint256 bust = router.getAmountsOut(address(this).balance,pETH)[pETH.length - 1];
