...
contract Volta is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 100000 *10**10  * 10**12;
    ...
    function _takefunding(uint256 tfunding) private {
        uint256 currentRate =  _getRate();
        uint256 rfunding = tfunding.mul(currentRate);
        _rOwned[communityAddress] = _rOwned[communityAddress].add(rfunding.div(2));
        _rOwned[DevWallet] = _rOwned[DevWallet].add(rfunding.div(2));
        if(_isExcludedFromReward[communityAddress])
            _tOwned[communityAddress] = _tOwned[communityAddress].add(tfunding.div(2));
            _tOwned[DevWallet] = _tOwned[DevWallet].add(tfunding.div(2));
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if (tfunding > 0) emit Transfer(sender, communityAddress, tfunding.div(2)); 
        if (tfunding > 0) emit Transfer(sender, DevWallet, tfunding.div(2)); 
    }
