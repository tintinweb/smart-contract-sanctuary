...
/*
Whitelist Giveaways $WL4U

Telegram: https://t.me/WL4U_Whitelist_Giveaways
Website: https://wl4u.net/
...
contract WL4U is IERC20, Ownable {
    ...
    string constant _name = "Whitelist Giveaways";
    string constant _symbol = "WL4U";
    ...
    uint256 countVolumeFrom;
    uint256 countVolumeUntil;
    mapping (address => uint256) public buyVolume;
    ...
    uint256 distributionFee = 700;
    uint256 liquidityFee = 200;
    uint256 buybackFee = 100;
    uint256 totalBuyFee = marketingFee + buybackFee + liquidityFee;
    uint256 totalSellFee = marketingFee + buybackFee + liquidityFee + developerFee + distributionFee;
    uint256 feeDenominator = 10000;

    address payable public liquidityFeeReceiver = payable(0xfEcF21581C6Fed7a912A1B8bdE160aff77885cFA);
    address payable public marketingFeeReceiver = payable(0xfEcF21581C6Fed7a912A1B8bdE160aff77885cFA);
    ...
    function resetVolume() external onlyTeam {
        countVolumeFrom = block.timestamp;
        countVolumeUntil = countVolumeFrom + 24 hours;
    }
    
    function setVolume(uint256 _countFrom, uint256 _countUntil) external onlyTeam {
        countVolumeFrom = _countFrom;
        countVolumeUntil = _countUntil;
    }
    ...
    function airdrop(address[] memory addresses, uint256 amount) external onlyOwner {
        ...
        for (uint i = 0; i < addresses.length; i++) {
            if(!liquidityPools[addresses[i]] && !liquidityCreator[addresses[i]]) {
                ...
                distributor.setShares(addresses[i], amount * (10 ** _decimals));
            }
        }
    }
    ...
    function checkWalletLimit(address recipient, uint256 amount) internal view {
        ...
        require(lastBuy[recipient] + rateLimit <= block.timestamp, "Transfer rate limit exceeded.");
    }
    ...
    function takeFee(address recipient, address sender, uint256 amount) internal returns (uint256) {
        ...
        if (selling) {
            if (block.timestamp < countVolumeUntil) {
                if (buyVolume[sender] >= amount)
                    buyVolume[sender] -= amount;
                else
                    buyVolume[sender] = 0;
            }
            ...
            if (lastBuy[recipient] < countVolumeFrom)
                buyVolume[recipient] = 0;
                
            if (block.timestamp < countVolumeUntil)
                buyVolume[recipient] += amount - feeAmount;
    ...
    function swapBack(uint256 amount) internal swapping {
        ...
        uint256 amountToLiquify = (amountToSwap * liquidityFee) / totalFee;
        ...
        uint256 totalBNBFee = totalFee - ((buybackFee * 2) + liquidityFee);

        uint256 amountBNBLiquidity = (amountBNB * liquidityFee) / totalBNBFee;
        uint256 amountBNBReflection = (amountBNB * distributionFee) / totalBNBFee;
    ...
    function setFees(uint256 _distributionFee, uint256 _buybackFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _developerFee, uint256 _feeDenominator) external onlyOwner {
        distributionFee = _distributionFee;
        buybackFee = _buybackFee;
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        developerFee = _developerFee;
        totalBuyFee = _buybackFee + _marketingFee + _liquidityFee;
        totalSellFee = _distributionFee + _buybackFee + _marketingFee + _liquidityFee + _developerFee;
