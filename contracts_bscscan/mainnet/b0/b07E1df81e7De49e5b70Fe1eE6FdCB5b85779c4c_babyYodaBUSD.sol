...
// File: contracts/babyYodaBUSDDividendPayingToken.sol
...
// File: contracts/babyYodaBUSD.sol
...
contract babyYodaBUSD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    address private _marketingWallet = address(0x839100f319D22A2e1f63cc5749a4609e3DB2BFC5);
    address private _devWallet = address(0x85Dc4933c8238F0369F5dF3D8a34F904ba232E45);

    bool private swapping;
    bool public airdrop;

    babyYodaBUSDDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("babyYodaBUSD", "babyYodaBUSD") {
    	...
    	dividendTracker = new babyYodaBUSDDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "babyYodaBUSD: The dividend tracker already has that address");

        babyYodaBUSDDividendTracker newDividendTracker = babyYodaBUSDDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "babyYodaBUSD: The new dividend tracker must be owned by the babyYodaBUSD token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "babyYodaBUSD: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "babyYodaBUSD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "babyYodaBUSD: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "babyYodaBUSD: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "babyYodaBUSD: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "babyYodaBUSD: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "babyYodaBUSD: This account cannot send tokens until trading is enabled");
        }
...
contract babyYodaBUSDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("babyYodaBUSD_Dividend_Tracker", "babyYodaBUSD_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "babyYodaBUSD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "babyYodaBUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main babyYodaBUSD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "babyYodaBUSD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "babyYodaBUSD_Dividend_Tracker: Cannot update claimWait to same value");
