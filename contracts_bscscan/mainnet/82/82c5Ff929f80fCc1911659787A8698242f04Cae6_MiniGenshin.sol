 ...
 * that a supply mechanism has to be added in a derived contract using {_rebase}.
 * For a generic mechanism see {ERC20PresetRebasePauser}.
 *
...
contract ERC20 is Context, IERC20, Ownable {
    ...
    function _rebase(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: rebase to the zero address");
     ...
     * rebasing and burning.
     *
     * Calling conditions:
     *
     * - when `from` and `to` are both non-zero, `amount` of ``from``'s tokens
     * will be to transferred to `to`.
     * - when `from` is zero, `amount` tokens will be rebased for `to`.
...
contract  MiniGenshin is ERC20 {

    uint tokenTotalSupply = 1000000000;

    constructor() public ERC20("MiniGenshin", " MGN") {
        _rebase(msg.sender, tokenTotalSupply * (10 ** uint256(decimals())));
    }
}