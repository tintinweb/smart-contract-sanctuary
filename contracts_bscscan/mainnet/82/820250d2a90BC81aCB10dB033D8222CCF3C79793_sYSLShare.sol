...
contract sYSLShare is YSLOpt {
    ...
    function transferOut(address _user, uint256 _amount) public override onlyRole(STRAT_ROLE) returns (uint256) {
        ...
        address sYSL = IStrategySwap(stratSwap).sYSL();

        path[0] = sYSL;
        path[1] = BUSD;

        // Decrease current amounts
        userDeposited[_user] -= _amount;
        totalDeposited -= _amount;
        ...
        // Swap sYSL to BUSD
        IERC20(sYSL).approve(apeSwap, _amount);
        amounts = IPancakeRouter02(apeSwap).swapExactTokensForTokens(
            _amount,
            1,
            path,
            _msgSender(),
        ...
        // Get exit fee from BUSD
        uint256 fee = (amountUSDAfter - amountUSDBefore) / 10;
        IERC20(BUSD).safeTransfer(feeAddress, fee);

        // Return BUSD to the router
        IERC20(BUSD).safeTransfer(stratSwap, amountUSDAfter - amountUSDBefore - fee);
        return amountUSDAfter - amountUSDBefore - fee;
    }
    ...
    function transferIn(address _user, uint256 _amount) public override onlyRole(STRAT_ROLE) returns (uint256) {
        ...
        // Get entrance fee from incoming BUSD
        uint256 fee = _amount / 10;
        _amount -= fee;

        IERC20(BUSD).safeTransfer(feeAddress, fee);

        // Calculate YSL emissions and add it together with BUSD to locked liquidity
        uint256 emission = getPrice(BUSD, _YSL, IERC20(BUSD).balanceOf(address(this)), apeSwap);
        IsYSL(_YSL).mintFor(address(this), emission);
        addLiquidity(BUSD, _YSL, IERC20(BUSD).balanceOf(address(this)), emission, apeSwap, adapter);

        // Calculate sYSL emission equivalent for BUSD
        uint256 sYslEmission = getPrice(BUSD, sYSL, _amount, apeSwap);
        IsYSL(sYSL).mintFor(address(this), sYslEmission);

        // Add emission
        userDeposited[_user] += sYslEmission;
        totalDeposited += sYslEmission;
        return sYslEmission;
    }

    function earn(address _user, bool _isAmplified) external override {
        return;
    }
    ...
    function currentReward(address _user) external view override returns(uint256) {
        return 0;
    }
}