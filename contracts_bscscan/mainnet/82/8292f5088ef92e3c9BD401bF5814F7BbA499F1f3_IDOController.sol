...
contract IDOController is Ownable, IIDOController {
    ...
    /**
     * @notice get max allocation amount of user based on tier
     */
    function getMaxAllocationAmount(address user, uint256 baseAmount) external view override returns (uint256) {
        uint256 multiplier = tier.getMultiplier(user);

        return multiplier * baseAmount;
    }

    function _setTier(IIDOTier _tier) internal {
        require(address(_tier) != address(0), "Invalid address");

        tier = _tier;
    }

    /**
     * @notice set tier address
     */
    function setTier(IIDOTier _tier) external onlyOwner {
        _setTier(_tier);
    }
    ...
    function _setFeeInfo(address _feeReceipient, uint256 _feePercent) internal {
        require(_feeReceipient != address(0), "Invalid _feeReceipient");
        require(_feePercent < 1000, "Invalid feePercent");
    ...
    function createIDO(
        ...
        IDO ido = new IDO(this, saleTarget, fundToken, fundTarget);

        ido.setBaseData(startTime, endTime, claimTime, minFundAmount, baseAmount, meta);
