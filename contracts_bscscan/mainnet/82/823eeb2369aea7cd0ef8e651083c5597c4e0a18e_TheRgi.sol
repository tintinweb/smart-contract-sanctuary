...
/*
Twitter: https://twitter.com/real_genshin

Rebasing feature is mathematically designed to increase the price
which will cause the chart to constantly rise . There will be winners with lottery also.
*/
...
contract TheRgi is IERC20, Ownable {
    ...
    string private constant NAME = "The Real Genshin Impact"; 
    string private constant SYMBOL = "THERGI"; 
    ...
    address public marketingWallet = 0x34bDc33dBf42aA28b6423aF0fC67992D24aC72Aa;
    address public lotteryWallet = 0x118147c7DEB6b247a84e809FD1F3Ced7B23D842a;
    ...
    // TOTAL_RGI is a multiple of INITIAL_FRAGMENTS_SUPPLY so that _RGIPerFragment is an integer.
    // Use the highest value that fits in a uint256 for max granularity.
    uint256 private constant TOTAL_RGI = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

    // MAX_SUPPLY = maximum integer < (sqrt(4*TOTAL_RGI + 1) - 1) / 2
    uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

    uint256 private _totalSupply;
    uint256 public _RGIPerFragment;
    mapping(address => uint256) public _RGIBalances;

    // This is denominated in Fragments, because the RGI-fragments conversion might change before it's fully paid.
    ...
    modifier onlyOperator {
        require (msg.sender == operator || msg.sender == owner(), "RGI: Not authorised");
        _;
    }
    ...
    constructor (uint256 marketingWalletPercentage) {
        ...
        _RGIBalances[owner()] = TOTAL_RGI;
        _RGIPerFragment = TOTAL_RGI / _totalSupply;
    ...
    function setOperator (address newOperator) external onlyOwner {
        require (newOperator != address(0), "RGI: Can't set operator to the zero address");
    ...
    function decreaseAllowance (address spender, uint256 subtractedValue) external returns (bool) {
        require (subtractedValue <= _allowedFragments[msg.sender][spender], "RGI: Allowance not high enough");
    ...
    function balanceOf (address account) public view override returns (uint256) {
        return _RGIBalances[account] / _RGIPerFragment;
    }
    
    
    function _approve (address owner, address spender, uint256 amount) private {
        require (owner != address(0), "RGI: Cannot approve from the zero address");
        require (spender != address(0), "RGI: Cannot approve the zero address");
    ...
    function _transfer (address sender, address recipient, uint256 amount) private {
        require (recipient != address(0), "RGI: Cannot transfer to the zero address");
        require (amount > 0, "RGI: Cannot transfer zero tokens");
        require (!isBlacklisted[sender] && !isBlacklisted[recipient], "RGI: Blacklisted");
        require (launchTime != 0 || _isExcluded[sender] || _isExcluded[recipient], "RGI: Not launched");
        ...
        if (!_isExcluded[sender] && !_isExcluded[recipient])
            require (amount <= _maxTxAmount, "RGI: Transfer amount exceeds the maxTxAmount.");
    ...
    function _tokenTransfer (address sender, address recipient, uint256 amount) private {
        if (_RGIBalances[sender] - (amount * _RGIPerFragment) == 0 && !_isExcluded[sender])
            amount -= 1;
            
        uint256 transferAmount = amount * _RGIPerFragment;
        uint256 receiveAmount = transferAmount;
        uint256 fee;
        uint256 maxWalletAmount = (_totalSupply * maxWallet / BP_DIVISOR) * _RGIPerFragment;
        
        if (!_isExcluded[recipient] && sender != owner() && recipient != swapPair)
            require (_RGIBalances[recipient] + receiveAmount <= maxWalletAmount, "RGI: Can't hold that much");
        ...
        if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            ...
            receiveAmount -= (fee * _RGIPerFragment);
        }
        
        _RGIBalances[sender] -= transferAmount;
        _RGIBalances[recipient] += receiveAmount;
        
        if (fee > 0) {
            _RGIBalances[address(this)] += (fee * _RGIPerFragment);
    ...
    function airDrop(address[] calldata recipients, uint256[] calldata amounts) external onlyOwner {
        require (recipients.length == amounts.length, "RGI: Recipients and amounts lengths must match");
    ...
    function setFees (uint256 _marketingFee, uint256 _liquidityFee, uint256 _lotteryFee) external onlyOwner {
        ...
        require (totalFee < 3000, "RGI: Total fee must be < 30%");
    ...
    function setMaxTx (uint256 _maxTx) external onlyOwner {
        require (_maxTx > 10, "RGI: Max tx must be > 0.1%");
        maxTx = _maxTx;
    }

    function setMaxWallet (uint256 _maxWallet) external onlyOwner {
        require (_maxWallet > 10, "RGI: Max wallet must be > 0.1%");
    ...
    function withdrawWETH (address _account) external onlyOwner  {
        require (_account != address(0), "RGI: Can't withdraw to the zero address");
    ...
    function withdrawToken (address _token, address _account) external onlyOwner {
        require (_token != address(0), "RGI: Can't withdraw a token of zero address");
        require (_token != address(this), "RGI: Can't withdraw RGI");
        require (_account != address(0), "RGI: Can't withdraw to the zero address");
