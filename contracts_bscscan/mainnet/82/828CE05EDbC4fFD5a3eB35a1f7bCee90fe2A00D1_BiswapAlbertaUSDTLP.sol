...
contract BiswapAlbertaUSDTLP is IAlberta, Ownable, ReentrancyGuard {
    ...
    constructor() {
        IERC20(USDT).safeApprove(address(ROUTER), type(uint256).max); // 100% trust in the router
    }

    function resetApprove() external {
        IERC20(USDT).safeApprove(address(ROUTER), 0);
        IERC20(USDT).safeApprove(address(ROUTER), type(uint256).max);
    }
    ...
    function add(
        ...
        address otherToken = lpToken.token0() == USDT ? lpToken.token1() : lpToken.token0();

        // Preparing USDT or farming tokens
        if (_paidToken == USDT) {
            ...
            (pathBnbToUsdt[0], pathBnbToUsdt[1]) = (WBNB, USDT);

            uint256 bnbBal = address(this).balance;
            _safePriceImpactSwap(bnbBal, pathBnbToUsdt, _maxPriceImpact);
            ROUTER.swapExactETHForTokens{value: bnbBal}(0, pathBnbToUsdt, address(this), block.timestamp);
        } else {
            // paid == others
            IERC20(_paidToken).safeTransferFrom(msg.sender, address(this), _amountIn);

            address[] memory pathToPaid = new address[](3);
            (pathToPaid[0], pathToPaid[1], pathToPaid[2]) = (_paidToken, WBNB, USDT);
        ...
        // Compute the optimal amount of USDT and otherToken to be converted.
        uint256 swapAmt;
        bool isReversed;
        {
            (uint256 r0, uint256 r1, ) = lpToken.getReserves();
            (uint256 usdtReserve, uint256 otherReserve) = lpToken.token0() == USDT ? (r0, r1) : (r1, r0);
    ...
    function remove(
        ...
        // Remove all liquidity back to USDT and farming tokens.
        ...
        // Convert farming tokens to USDT.
        address[] memory pathToUsdt = new address[](2);
        (pathToUsdt[0], pathToUsdt[1]) = (otherToken, USDT);

        IERC20(otherToken).safeApprove(address(ROUTER), 0);
        IERC20(otherToken).safeApprove(address(ROUTER), type(uint256).max);

        uint256 otherBal = IERC20(otherToken).balanceOf(address(this));
        _safePriceImpactSwap(otherBal, pathToUsdt, _maxPriceImpact);
        ROUTER.swapExactTokensForTokens(otherBal, 0, pathToUsdt, address(this), block.timestamp);
        ...
        if (_paidToken == USDT) {
            ...
            _safePriceImpactSwap(usdtBal, pathToBnb, _maxPriceImpact);
            ROUTER.swapExactTokensForETH(usdtBal, 0, pathToBnb, _to, block.timestamp);
        } else {
            // USDT to paid token
            address[] memory pathToPaid = new address[](3);
            (pathToPaid[0], pathToPaid[1], pathToPaid[2]) = (USDT, WBNB, _paidToken);

            uint256 usdtBal = IERC20(USDT).balanceOf(address(this));
            _safePriceImpactSwap(usdtBal, pathToPaid, _maxPriceImpact);
            ROUTER.swapExactTokensForTokens(usdtBal, 0, pathToPaid, _to, block.timestamp);
        }
    }
