...
contract Token is IBEP20, Auth {
    ...
    bool public CooldownBuyEnabled = true;
    bool public CooldownSellEnabled = true;
    uint8 public cooldownBuyInterval = 20;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xAC506C075bA0099be7e7FD95F24930964677C3fb;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(recipient == pair && CooldownSellEnabled && !authorizations[sender] && !isTimelockExempt[sender]){
                ...
                if(firstsell[sender] + (20 minutes) < block.timestamp){
                    sellnumber[sender] = 0;
                }
                if (sellnumber[sender] == 0) {
                    sellnumber[sender]++;
                    firstsell[sender] = block.timestamp;
                    sellcooldown[sender] = block.timestamp + (1 minutes);
                }
                else if (sellnumber[sender] == 1) {
                    sellnumber[sender]++;
                    sellcooldown[sender] = block.timestamp + (2 minutes);
                }
                else if (sellnumber[sender] == 2) {
                    sellnumber[sender]++;
                    sellcooldown[sender] = block.timestamp + (3 minutes);
                }
                else if (sellnumber[sender] == 3) {
                    sellnumber[sender]++;
                    sellcooldown[sender] = firstsell[sender] + (20 minutes);
                }
        }
