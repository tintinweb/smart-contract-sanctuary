...
contract Transaction is Ownable {
    ...
    address public signer = 0xFD06020e5A86528fB570F49E7b3fD881c7C0A34b;
    IBEP20 public bet97 = IBEP20(0xf03a69f6B0D0f1137a597704DA200a6705Ba8E45);
    IBEP20 public busd = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    address public ceo = 0xb6e6cF1756097731b53801dB90D9959076D1eB5E;
    struct transaction {
        address user;
        uint amount;
        uint _type; // 1 => 97bet; 2 => BUSD
   ...
   function getMessageHash(uint id, uint amount) public pure returns (bytes32) {
    ...
    function deposit97Bet(uint _id, uint _amount) public {
        require(bet97.transferFrom(_msgSender(), ceo, _amount), 'Not enough Balacne');
        transactions[_id] = transaction(_msgSender(), _amount, 1, 1, block.timestamp);
        emit Deposit(_id, _amount, _msgSender(), 1);
    }
    function depositBusd(uint _id, uint _amount) public {
        require(busd.transferFrom(_msgSender(), address(this), _amount), 'Not enough Balacne');
    ...
    function withdraw(uint _id, uint _amount, uint8 v, bytes32 r, bytes32 s) public {
        ...
        busd.transfer(_msgSender(), _amount);
        emit Withdraw(_id, _amount, _msgSender(), 2);
    }
    function withdrawByCeo(IBEP20 _bep20, uint _amount, address _to) public onlyCeo {
