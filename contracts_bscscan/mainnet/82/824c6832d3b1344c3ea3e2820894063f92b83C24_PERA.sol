 ...
 * 
  ...
  Saint features:
   ...
   5% fee added to dev wallet for charitable giving and future development
 ...
 were made to make this project inline with the mission of the token.
*/
...
*/
contract PERA is Context, IERC20, Ownable {
  ...
  address payable charityWallet = 0xe3C601b1FC6564ebf5603fCaD7956697761E39Db;
  ...
  */
  ...
  string private constant _name = "PERA";
  ...
  // addresses to exclude from 2% max holding bnb value 
  ...
  */
  ...
  function excludeAddressFromMaxTwoPercent(address _address) public onlyOwner(){
      isExcludedFromMaxWalletSize[_address] = true;
  }
  
function removeAddressFromMaxTwoPercent(address _address) public onlyOwner(){
      isExcludedFromMaxWalletSize[_address] = false;
  }
  ...
  function _transfer(
      ...
      // indicates if we check if wallet has more than 
      ...
      if(isExcludedFromMaxWalletSize[to] == true){
          checkAddress = false; 
      }
      
      if(checkAddress == true){
          uint256 balanceReceiver = balanceOf(to);
          uint256 twoPerCent = totalSupply().mul(2).div(100);
          require(balanceReceiver.add(amount) <= twoPerCent, "wallet max size 2% of total supply");
      }
