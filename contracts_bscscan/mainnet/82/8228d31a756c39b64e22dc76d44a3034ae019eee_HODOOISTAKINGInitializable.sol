...
// File: contracts/HODOOISTAKING.sol
...
contract HODOOISTAKINGInitializable is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    
    // The address of the HOD Observatory factory
    address public HODOOI_STAKING_FACTORY;
    ...
    // The block number when HOD mining ends.
    uint256 public bonusEndBlock;

    // The block number when HOD mining starts.
    ...
    event WithdrawDevFees(uint256 amount);

    constructor() public {
        HODOOI_STAKING_FACTORY = msg.sender;
    }
    ...
    function initialize(
        ...
        require(msg.sender == HODOOI_STAKING_FACTORY, "Not factory");
        ...
        if (_amount > 0) {
            ...
            if (stakedTokenTransferFee > 0) {
             ...
             if (depositFee > 0) {
                ...
                if(stakedToken == rewardToken && stakedToken == IBEP20(0x19A4866a85c652EB4a2ED44c42e4CB2863a62D51))
                {
                    uint256 devFeeLocal = fee.mul(4000).div(10000); // 40% of deposit fee 
                    devFees = devFees.add(devFeeLocal);
                    fee = fee.sub(devFeeLocal);
                }
                fees = fees.add(fee);
            } 
    ...
    function withdrawFees() external onlyOwner {
        require(fees > 0 || devFees > 0, "No fees:(");
        uint256 feeAmount = fees;
        uint256 devLocalFees = devFees;
        address devAddress = 0x9a39Cf1239d95074AdA821B2e6feAaee0F7A7366;
        address ownerAddress = 0x02bC59b1143Ea1b313C383C7DBE175714Aa2Ef02;
        stakedToken.safeTransfer(ownerAddress, feeAmount);
        fees = 0;  
        if(devLocalFees > 0){
             stakedToken.safeTransfer(devAddress, devLocalFees);
             devFees = 0;
        }
        emit WithdrawFees(feeAmount.add(devLocalFees));
    }
...
// File: contracts/HODOOISTAKINGFactory.sol
...
contract HODOOISTAKINGFactory is Ownable {
    event NewHODOOISTAKINGContract(address indexed HODOOISTAKING);
     ...
     * @param _withdrawalInterval: the withdrawal interval for stakedToken (if any, else 0)
     * @param _admin: admin address with ownership
     * @return address of new HOD Observatory contract
     */
    function deployPool(
        IBEP20 _stakedToken,
        IBEP20 _rewardToken,
        ...
        bytes memory bytecode = type(HODOOISTAKINGInitializable).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address HODOOISTAKINGAddress;

        assembly {
            HODOOISTAKINGAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        HODOOISTAKINGInitializable(HODOOISTAKINGAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewHODOOISTAKINGContract(HODOOISTAKINGAddress);
    }
}