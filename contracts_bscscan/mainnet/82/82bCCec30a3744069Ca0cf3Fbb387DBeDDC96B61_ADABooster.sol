...
// File: contracts/ADABoosterDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable ADAToken = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
...
contract ADABooster is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ADABoosterDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("ADA B00ster", "ADAB00ster") {
    	...
    	dividendTracker = new ADABoosterDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ADABooster: The dividend tracker already has that address");

        ADABoosterDividendTracker newDividendTracker = ADABoosterDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ADABooster: The new dividend tracker must be owned by the ADABooster token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ADABooster: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ADABooster: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ADABooster: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ADABooster: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ADABooster: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ADABooster: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ADABooster: Cannot update gasForProcessing to same value");
    ...
    function updateMaxBuy(uint256 newValue) public onlyOwner {
        maxBuyTranscationAmount = newValue;
    }
    
    function updateMaxSell(uint256 newValue) public onlyOwner {
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "ADABooster: This account cannot send tokens until trading is enabled");
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
       ...
       uniswapV2Router.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            address(0x6cF0a54BE82771510Fa814305ef5aF5E37fe30B0),
            block.timestamp
        );
...
contract ADABoosterDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("ADABooster_Dividend_Tracker", "ADABooster_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "ADABooster_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "ADABooster_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ADABooster contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ADABooster_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ADABooster_Dividend_Tracker: Cannot update claimWait to same value");
