...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  constructor(){
      ...
      rewardTokens.push(address(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D)); // SHIB- 0
      rewardTokens.push(address(0x2B3F34e9D4b127797CE6244Ea341a83733ddd6E4)); // FLOKI - 1
  ...
  function _withdrawDividendOfUser(address payable user, address _rewardToken) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      withdrawnDividends[_rewardToken][user] = withdrawnDividends[user][_rewardToken].add(_withdrawableDividend);
  ...
  function withdrawableDividendOf(address _owner, address _rewardToken) public view override returns(uint256) {
    return accumulativeDividendOf(_owner,_rewardToken).sub(withdrawnDividends[_rewardToken][_owner]);
  }
...
contract ShibaFlokiX is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    // use by default 500,000 gas to process auto-claiming dividends (set higher due to the increased cost of interacting with FLOKI)
    ...
    constructor() ERC20("ShibaFlokiX", "SFX"){
    	...
    	marketingWallet = address(owner()); // update to Marketing Wallet if set to Owner... can always be changed later too.
        liquidityWallet = address(0xeF3D6E5Cd89C768a3b10EafE0Eac80BA0dd9604a); // liquidity will go to dead address at first, can update later if needed by owner.
    ...
    // turn off early buyer in the event someone gets erroneously flagged.
    function removeFromBoughtEarly(address account) external onlyOwner {
    ...
    function airdropToWalletsWithAmounts(address[] memory airdropWallets, uint256[] memory amounts) external onlyOwner() {
        require(airdropWallets.length == amounts.length, "Array lengths must match");
    ...
    // never ever do this without professional assistance. Useful if desiring to change reward structure.
    ...
    function updateFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _feeDivisor) external onlyOwner {
        require(_feeDivisor == 100 || _feeDivisor == 1000, "Fee divisor must be 100 or 1000");
        if(_feeDivisor == 100){
            require(_marketingFee <= 8, "Marketing may not be set higher than 8%");
            require(_rewardsFee <= 12, "Rewards fee may not be set higher than 12%");
            require(_liquidityFee <= 4, "Liquidity may not be set higher than 4%");
            require(_buybackFee <= 4, "Buyback may not be set higher than 4%");
        }
        if(_feeDivisor == 1000){
            require(_marketingFee <= 80, "Marketing may not be set higher than 84%");
            require(_rewardsFee <= 120, "Rewards fee may not be set higher than 12%");
            require(_liquidityFee <= 40, "Liquidity may not be set higher than 4%");
            require(_buybackFee <= 40, "Buyback may not be set higher than 4%");
        }
        marketingFee = _marketingFee;
        rewardsFee = _rewardsFee;
        liquidityFee = _liquidityFee;
        ...
        } else if(_feeDivisor == 1000) {
            require(totalFees <= 200, "Must keep fees at 20% or less");
        }
    }
    ...
    function _transfer(
        ...
        if(from != owner() && from != presaleRouterAddress && from != presaleAddress && !automatedMarketMakerPairs[to] && block.number == tradingActiveBlock){
            boughtEarly[to] = true;
        }
        ...
        if(takeFee) {
            ...
            if(boughtEarly[from] && earlyBuyPenaltyEnd > block.timestamp && !automatedMarketMakerPairs[from]) {
        	    super._transfer(from, address(this), fees * 4);
                tokensForMarketing = fees*3;
        	    fees = fees * 4;
        	} else {
        ...
        if(tokensForMarketing > 0){
            swapTokensForEthMarketing(tokensForMarketing);
        }
    ...
    function swapTokensForEthMarketing(uint256 tokenAmount) private {
    ...
    function swapBack(uint256 contractTokenBalance) internal {
        ...
        // send twice to initiate buys of both reward tokens
        (bool success,) = address(dividendTracker).call{value: amountBNBReflection/2}("");
        (success,) = address(dividendTracker).call{value: amountBNBReflection/2}("");
    ...
    function rescueTokens(address _recipient, address _ERC20address, uint256 _amount) public onlyOwner returns(bool) {
        require(!automatedMarketMakerPairs[_ERC20address], "Can't transfer out LP tokens!");
