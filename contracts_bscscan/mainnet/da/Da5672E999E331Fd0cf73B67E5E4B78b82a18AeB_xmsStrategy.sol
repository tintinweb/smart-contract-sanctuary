...
contract xmsStrategy{
    ...
    address public constant liquidityminingmaster = 0x22D8d50454203bd5a41B49ef515891f1aD9f3e53; //pool
    address public constant pancakerouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public constant geg = 0x99dAB6065951BecaC1dECBaC0C1A16b9BbF12913;
    address public constant busd = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;  
    address public constant usdt = 0x55d398326f99059fF775485246999027B3197955;
    address public constant uniRouter = pancakerouter;
    address public constant pancake = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82;
    address public constant apeswap = 0xcF0feBd3f17CEf5b47b0cD257aCf6025c5BFf3b7;
    address public gegfarm; 
    ...
    function doApprove () internal{
        ...
        IERC20(out).safeApprove(apeswap, 0);
        IERC20(out).safeApprove(apeswap, uint(-1));
    }
    ...
    function setGEGFarm(address _gegFarm) external {
         require(msg.sender == governance, "!governance");
         gegfarm = _gegFarm;
    }
    ...
    function harvest(uint amount1, uint amount2) public {
        ...
        //25% WBNB
        convertCaketoWBNB(totalReward.mul(wbnbReward).div(100));
        lastHarvestTime = FarmWBNB(wbnbfarm).set_initReward(IERC20(wbnb).balanceOf(address(this)));
        IERC20(wbnb).safeTransfer(wbnbfarm, IERC20(wbnb).balanceOf(address(this)));

        //25% GEG   
        convertCaketoGEG(totalReward.mul(gegReward).div(100));
        lastHarvestTime = FarmGEG(gegfarm).set_initReward(IERC20(geg).balanceOf(address(this)));
        IERC20(geg).safeTransfer(gegfarm, IERC20(geg).balanceOf(address(this)));
    ...
    function convertCaketoGEG(uint amount) public {
        require(!Address.isContract(msg.sender),"!contract");
        address[] memory path = new address[](3);
                    path[0] = out; //cake
                    path[1] = wbnb;
                    path[2] = geg;
                    Uni(apeswap).swapExactTokensForTokens(
                            amount,
                            uint256(0),
                            path,
                            address(this),
    ...
    function convertCaketoWBNB(uint amount) public {
        require(!Address.isContract(msg.sender),"!contract");
    ...
    function convertCaketoXms() public {
        require(!Address.isContract(msg.sender),"!contract");
        address[] memory path = new address[](2);
                    path[0] = out; //cake
                    path[1] = xms;
                    Uni(marsRouter).swapExactTokensForTokens(
                            (IERC20(out).balanceOf(address(this))),
                            uint256(0),
                            path,
                            address(this),
                            block.timestamp.add(1800)
