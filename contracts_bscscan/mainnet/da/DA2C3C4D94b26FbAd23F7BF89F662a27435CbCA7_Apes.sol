    ...
    UUPSUpgradeable
{
    ...
    function listToSell(uint256 _idx, uint256 _price) public {
        IFarmLand farmLandInstance = IFarmLand(farmLandAddress);
        Ape memory current = apes[_idx];
        require(farmLandInstance.apeOwnerAddress(current.id) == address(0x000), "ape is stacked");
    ...
    function isApeOnSale(uint256 nftID) public view returns (bool) {
        return apes[nftID - 1].onSale;
    }
...
pragma solidity ^0.8.2;
interface IFarmLand {
  function apeOwnerAddress(uint256) external view returns(address);
}
