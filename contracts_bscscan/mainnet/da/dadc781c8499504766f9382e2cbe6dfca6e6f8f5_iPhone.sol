...
contract iPhone is ERC20, Ownable {
    ...
    constructor() ERC20("iPhone Inu", "iPhone") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0x07f3405D35C26df9936e05C6b9627d234CBc6ceb;

    	cakeDividendToken = 0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0; //
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "iPhone: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "iPhone: The new dividend tracker must be owned by the iPhone token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("iPhone_Dividend_Tracker", "iPhone_Dividend_Tracker", 0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "iPhone_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "iPhone_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "iPhone_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "iPhone_Dividend_Tracker: Cannot update claimWait to same value");
