...
contract Gilgagon is Context, BEP20, Ownable {
    using SafeMath for uint;
    using Address for address;
    
    string private _namez = "Gilgagon";
    string private _symbolz = "Gilgagon";
    uint8 private _decimals = 9;

    mapping (address => uint) private _rOwnedz;
    mapping (address => uint) private _tOwnedz;
    ...
    uint private initialsupply = 100_000_000_000;
    uint private _tTotalz = initialsupply * 10 ** _decimals; 
    uint private _rTotalz = (MAX - (MAX % _tTotalz));
    uint private _tFeeTotalz;
    ...
    function balanceOf(address account) public view override returns (uint) {
        if (_isExcluded[account]) return _tOwnedz[account];
        return tokenFromReflection(_rOwnedz[account]);
    }
    ...
    function deliver(uint tAmount) public {
        ...
        _rOwnedz[sender] = _rOwnedz[sender].sub(rAmount);
        _rTotalz = _rTotalz.sub(rAmount);
        _tFeeTotalz = _tFeeTotalz.add(tAmount);
    }
    ...
    function tokenFromReflection(uint rAmount) public view returns(uint) {
        require(rAmount <= _rTotalz, "Amount must be less than total reflections");
    ...
    function excludeFromReward(address account) public onlyOwner() {
        ...
        if(_rOwnedz[account] > 0) {
            _tOwnedz[account] = tokenFromReflection(_rOwnedz[account]);
        }
    ...
    function _transferBothExcluded(address sender, address recipient, uint tAmount) private {
        ...
        _tOwnedz[sender] = _tOwnedz[sender].sub(tAmount);
        _rOwnedz[sender] = _rOwnedz[sender].sub(rAmount);
        _tOwnedz[recipient] = _tOwnedz[recipient].add(tTransferAmount);
        _rOwnedz[recipient] = _rOwnedz[recipient].add(rTransferAmount);
    ...
    function _reflectFee(uint rFee, uint tFee) private {
        _rTotalz = _rTotalz.sub(rFee);
        _tFeeTotalz = _tFeeTotalz.add(tFee);
    }
    ...
    function _getCurrentSupply() private view returns(uint, uint) {
        uint rSupply = _rTotalz;
        uint tSupply = _tTotalz;
        for (uint i = 0; i < _excluded.length; i++) {
            if (_rOwnedz[_excluded[i]] > rSupply || _tOwnedz[_excluded[i]] > tSupply) return (_rTotalz, _tTotalz);
            rSupply = rSupply.sub(_rOwnedz[_excluded[i]]);
            tSupply = tSupply.sub(_tOwnedz[_excluded[i]]);
        }
        if (rSupply < _rTotalz.div(_tTotalz)) return (_rTotalz, _tTotalz);
    ...
    function _takeLiquidity(uint tLiquidity) private {
        uint currentRate =  _getRate();
        uint rLiquidity = tLiquidity.mul(currentRate);
        _rOwnedz[address(this)] = _rOwnedz[address(this)].add(rLiquidity);
        if(_isExcluded[address(this)])
            _tOwnedz[address(this)] = _tOwnedz[address(this)].add(tLiquidity);
    }
