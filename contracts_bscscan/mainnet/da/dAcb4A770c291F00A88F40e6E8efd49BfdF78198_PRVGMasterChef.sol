...
// File: contracts/StakingV2/PRVGMasterChef.sol
...
interface IPRVNFTInterface {
 ...
 contract PRVGMasterChef is Ownable, IERC721Receiver,ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    ...
    uint256 public constant MAX_EMISSION_RATE = 800000000000000000; //0.8 tokens perblock
    ...
    function pendingPrvg(uint256 _pid, address _user) external view returns (uint256,uint256,uint256) {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (prvg.totalSupply().add(prvReward)  <= prvg.MAX_SUPPLY()) {
            ...
            prvg.mint(devAddress,prvReward.div(10));
        } else {
    ...
    function payPendingReward(uint256 _pid) internal {
       ...
       if (pending > 0 ) {
            ...
            if(pool.isNFTStakingEnabled){
                ...
                prvg.mint(address(this),extraReward);
                totalRewards = totalRewards.add(extraReward);       
  
            }
            // send rewards
            safePrvTransfer(msg.sender, totalRewards);
            emit onRewardPaid(msg.sender,totalRewards);
