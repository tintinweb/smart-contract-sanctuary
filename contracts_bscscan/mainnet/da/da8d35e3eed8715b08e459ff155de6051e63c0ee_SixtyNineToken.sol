...
The 69 hours TOKEN
For the first time on BSC:
A self-destructing contract that has a 69 hours living only!
...
contract SixtyNineToken is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "SixtyNineToken";
    string constant _symbol = "$SIXNINE";
    ...
    uint256 public specialPrize     = 5;
    uint256 public totalFee        = marketingFee + reflectionFee + liquidityFee + specialPrize;
    ...
    address public specialPrizeHolder;
    ...
    bool public antiJeet = true;
    bool public lowJeet = false;
    ...
    uint256 public _maxTxAmount = rSupply.div(100).mul(3); 
    ...
    constructor () Auth(msg.sender) {
        ...
        specialPrizeHolder = 0xa440dA7f6Fadf22E7FdE2706c306EFb567d992A0;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != specialPrizeHolder  && recipient != autoLiquidityReceiver){
        ...
        if(tradingOpen
        && (launchedAt + 82800 < block.timestamp)){
            doomsday(amount);
        }
    ...
    function buyBack(uint256 amount) internal swapping {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(this);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBSPrice = amountBNB.mul(specialPrize).div(totalBNBFee);
        
        if(reflectionFee>0) {try distributor.deposit{value: amountBNBReflection}() {} catch {}}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(specialPrizeHolder).call{value: amountBNBSPrice, gas: 30000}("");
    ...
    function forceSwapBack(uint256 percentageToSwap) public onlyOwner {
        ...
        uint256 totalBNBFee = totalFee.sub(liquidityFee);
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _specialPrize, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        specialPrize = _specialPrize;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_specialPrize);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/3, "Fees cannot be more than 33%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver,  address _specialPrizeHolder) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        specialPrizeHolder = _specialPrizeHolder;
    }
    ...
    function setAntiJetMeasure(bool _antiJeet, uint256 factor, bool _lowJeet) external onlyOwner {
         antiJeet =_antiJeet;
    ...
    function redButton() external payable onlyOwner{
        require(launchedAt + 82800 < block.timestamp);
        selfdestruct(msg.sender);
    }
    
    function doomsday(uint256 amount) internal {
        uint256 rand = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, amount))) % 10000;
        if (rand < 100){
            tradingOpen = false; 
        }
    }
