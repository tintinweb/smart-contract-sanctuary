...
abstract contract StratX2 is Ownable, ReentrancyGuard, Pausable {
    ...
    address public moonRouter=0x95B09c446aF69f385455670B78B64BcBFb24eB8E;
    address public moonCollector =0x7a6eEbC10Dd283DEDe17F28b9BCbCB1EBFb4D68c;
    ...
    function earn() public virtual nonReentrant whenNotPaused {
        ...
        IERC20(earnedAddress).safeApprove(moonRouter, 0);
        ...
        if (token0Amt > 0 && token1Amt > 0) {
           ...
           (uint amountA, uint amountB, uint liquidity) = IPancakeRouter02(moonRouter).addLiquidity(
                token0Address,
                token1Address,
                token0Amt,
                token1Amt,
    ...
    function buyBack(uint256 _earnedAmt) internal virtual returns (uint256) {
        ...
        IERC20(earnedAddress).safeTransfer(moonCollector, buyBackAmt);
...
contract StratXMoon is StratX2 {
   constructor(
