...
contract HXLTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SHXL }
    ...
    address public sHXL;
    uint public sHXLQueue; // Delays change to sHXL address
    ...
    /**
        @notice allow approved address to deposit an asset for HXL
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint HXL needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( HXL ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn HXL for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IHXLERC20( HXL ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sHXL ).balanceOf( msg.sender ); // Can only borrow against sHXL held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with HXL
        @param _amount uint
     */
    function repayDebtWithHXL( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IHXLERC20( HXL ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, HXL, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( HXL ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( HXL ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns HXL valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match HXL decimals
            value_ = _amount.mul( 10 ** IERC20( HXL ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SHXL ) { // 9
            sHXLQueue = block.number.add( blocksNeededForQueue );
        } else return false;
