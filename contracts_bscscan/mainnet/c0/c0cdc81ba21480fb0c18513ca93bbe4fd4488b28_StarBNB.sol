...
contract StarBNB is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    StarBNBDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable public operations1Address =
        payable(0x81392fA760e9b06D4057E695aa6a401fc1444D79); 
    address payable public operations2Address =
        payable(0x734F27E7c655e603219Fe1eFD98B6e495F4C2878);
    ...
    uint256 public devFees = 5; // Dev 2% from here, Marketing and Buyback is 3% 
    uint256 public liquidityFee = 1;
    uint256 public BNBRewardsBuyFee = 10;
    uint256 public BNBRewardsSellFee = 25;
    ...
    constructor() ERC20("StarBNB", "StarBNB") {
        dividendTracker = new StarBNBDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        StarBNBDividendTracker newDividendTracker = StarBNBDividendTracker(
            payable(newAddress)
        );
    ...
    function updateFees(
        ...
        // require(
        //     BNBRewardsBuy >= 0 && BNBRewardsBuy <= 25,
        //     "BNB reward fees must be between 0 and 25"
        // );
        // require(
...
contract StarBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "StarBNB_Dividend_Tracker",
            "StarBNB_Dividend_Tracker"
        )
    {
        starBNB = StarBNB(mainContract);
    ...
    function _transfer(
        ...
        require(false, "StarBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "StarBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main StarBNB contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(starBNB);

            uint256 prevBalance = starBNB.balanceOf(address(this));
            ...
            {
                uint256 received = starBNB.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    starBNB.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
