...
contract Ashera is ERC20Detailed, Ownable {
  ...
  // TOTAL_Asr is a multiple of INITIAL_FRAGMENTS_SUPPLY so that _AsrPerFragment is an integer.
  // Use the highest value that fits in a uint256 for max granularity.
  uint256 private constant TOTAL_Asr = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

  // MAX_SUPPLY = maximum integer < (sqrt(4*TOTAL_Asr + 1) - 1) / 2
  uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

  uint256 private _totalSupply;
  uint256 private _AsrPerFragment;
  mapping(address => uint256) private _AsrBalances;
  ...
  ERC20Detailed("ASHERA", "ASR", uint8(DECIMALS))
  payable
  public
  {
    ...
    _AsrBalances[msg.sender] = TOTAL_Asr;
    _AsrPerFragment = TOTAL_Asr.div(_totalSupply);
...
*/
  ...
  returns (uint256)
  {
    return _AsrBalances[who].div(_AsrPerFragment);
  }
  ...
  function _transferStandard(address sender, address recipient, uint256 amount) private {
    ...
    uint256 AsrDeduct = amount.mul(_AsrPerFragment);
    uint256 AsrValue = tTransferAmount.mul(_AsrPerFragment);
    _AsrBalances[sender] = _AsrBalances[sender].sub(AsrDeduct);
    _AsrBalances[recipient] = _AsrBalances[recipient].add(AsrValue);
    _takeFee(sender, tFee);
    emit Transfer(sender, recipient, amount);
  }

  function _transferExcluded(address sender, address recipient, uint256 amount) private {
    uint256 AsrValue = amount.mul(_AsrPerFragment);
    _AsrBalances[sender] = _AsrBalances[sender].sub(AsrValue);
    _AsrBalances[recipient] = _AsrBalances[recipient].add(AsrValue);
    emit Transfer(sender, recipient, amount);
  }
  ...
  function _takeFee(address sender, uint256 tFee) private {
    uint256 rFee = tFee.mul(_AsrPerFragment);
    _AsrBalances[address(this)] = _AsrBalances[address(this)].add(rFee);
