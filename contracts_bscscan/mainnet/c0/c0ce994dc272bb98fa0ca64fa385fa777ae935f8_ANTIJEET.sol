...
The sell tax will be 49% for the first 15 mins, then the dev will steadily decrease it
The Jeets have been ruining investors 100x moonshots for too long.
Apes and Chads alike are banning together to put an end to them.
AntiJeet Token features unique tokenomics that will give diamond hands the 100x they’ve been waiting for at the expense of the Jeets.

Telegram : https://t.me/Antijeet
*/
...
contract ANTIJEET is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
    payable(0x41938E5c22dC24ef7Bb8401215d301C1174B2132);
    ...
    string private constant _name = "Anti JEET";
    string private constant _symbol = "ANTIJEET";
...
uint256 private minimumTokensBeforeSwap = _tTotal * 100 / 10000; // 0.05%
...
event AddedSniper(address indexed snipersupposedly);
...
maxTransactionAmount = _tTotal * 10 / 1000; // maxTransactionAmountTxn
maxWalletAmount = _tTotal * 50 / 1000; // maxWalletToken
...
maxTransactionAmount = _tTotal * maxtx / 100;
}
...
function addBoughtEarly(address account) external onlyOwner {
boughtEarly[account] = true;
emit AddedSniper(account);
}
