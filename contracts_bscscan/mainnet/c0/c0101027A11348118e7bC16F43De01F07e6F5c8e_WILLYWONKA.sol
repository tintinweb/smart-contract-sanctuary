...
contract WILLYWONKA is IBEP20, Auth {
    ...
    uint8 public bFreezeT = 30 seconds;
    ...
    function _transferfrom(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != FJUK && recipient != LiqRe){
            uint256 heldTokens = balanceOf(recipient);
            require((heldTokens + amount) <= _maxWalletToken,"Total Holding is currently limited, you can not buy that much.");}
        checkbTxLimit(sender, amount); 
        if(sender != pair){checksTxLimit(sender, amount);}
        if(SecSell(amount) && sender != address(_contract) && VariableSB && (amount.mul(VariableN).div(VariableD) < sThreshold)) { DefinedSwap(); }
        _balances[sender] = _balances[sender].sub(amount, "+");
        uint256 amountReceived = TotalFeeE(sender) ? ProcessTF(sender, recipient, amount) : amount;
        _balances[recipient] = _balances[recipient].add(amountReceived);
        if(SecSell(amount) && VariableSB && sender != address(_contract) && (amount.mul(VariableN).div(VariableD) < sThreshold)) { VariableSBFunction(amount); }
        if(!isDivE[sender]) {
            try distributor.setShare(sender, _balances[sender]) {} catch {} }
        if(!isDivE[recipient]) {
            try distributor.setShare(recipient, _balances[recipient]) {} catch {} }
        if(SecSell(amount) && sender != address(_contract) && !VariableSB){ DefinedSwap(); }
        if(sender != pair && sFreeze && !isTimeloE[sender]) 
            {require(scoolin[sender] < block.timestamp,"Please wait for cooldown between sells"); 
            scoolin[sender] = block.timestamp + sFreezeT;} 
        if(sender == pair && bFreeze && !isTimeloE[sender]) 
