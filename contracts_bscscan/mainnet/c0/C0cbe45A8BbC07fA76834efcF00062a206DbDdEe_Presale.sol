...
contract Presale {
    ...
    uint256 public claimTime; // time when claim allowed

    constructor() public {
        bscsDevAddress = payable(0x733d56742bb3495808f0369830f296D262B25992);
        token = IERC20(0xC8fF271B9B267139E1517eCC70c364b8a980FEB6);
        unsoldTokensDumpAddress = 0x733d56742bb3495808f0369830f296D262B25992;

        totalTokens = 14 * 10 **4 * 10**18;
        tokensLeft = 14 * 10 **4 * 10**18;
        tokenPriceInWei = 0.0045 * 10**18;
        hardCapInWei = 600 * 10**18;
        softCapInWei = 100 * 10**18;
        maxInvestInWei = 10 * 10**18;
        minInvestInWei = 0.1 * 10**18;
        openTime  = 1627138800;
        closeTime = 1627225200;
        claimTime = 1628348400;
        whitelistedAddresses[0x733d56742bb3495808f0369830f296D262B25992] = true;
    }
        ...
        investorOnly
    {
        ...
        require(block.timestamp > closeTime, "Not yet closed");
        require(block.timestamp > claimTime, "claim not available yet");
