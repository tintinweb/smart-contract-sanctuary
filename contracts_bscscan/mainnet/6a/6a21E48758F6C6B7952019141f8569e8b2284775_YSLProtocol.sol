...
contract YSLProtocol is OwnableUpgradeable {
    ...
    function setDoCompound(bool _doCompound) external onlyOwner {
        doCompound = _doCompound;
    }

    function updateEmissionRate(uint256 _sYSLPerBlock) external onlyOwner {
        massUpdatePools(0, poolInfo.length);
    ...
    function massUpdatePools(uint256 _start, uint256 _end) public {
        ...
        require(_end <= length, "Incorrect value");
    ...
    function deposit(
        ...
        // Harvest rewards (if need) with current user.amount
        ...
        user.amount += _amount;
        // Record debt with new user.amount
    ...
    function depositFrom(
        ...
        IERC20(BUSD).approve(address(pool1.strat), output);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        // Harvest first with current user.amount
        _harvest(_pid, _msgSender());

        // Withdraw lp
        user.amount -= _amount;
        // Update debt with new user.amount
        user.rewardDebt = (user.amount * pool.accsYSLPerShare) / DECIMALS;
    ...
    function safeSYSLtransfer(address _user, uint256 _amount) internal {
        ...
        ILock(lockContract).lock(_amount, 180 days, _user);
    }
}