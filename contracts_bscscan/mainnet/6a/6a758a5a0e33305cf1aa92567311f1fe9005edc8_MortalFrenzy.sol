...
contract MORTALFRNZYDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MORTALFRNZY_Dividend_Tracker", "MORTALFRNZY_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2500000 * (10**9); 
    }

    function _transfer(address, address, uint256) internal override pure {
        require(false, "MORTALFRNZY_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "MORTALFRNZY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MORTALFRNZY contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "MORTALFRNZY_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MORTALFRNZY_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MortalFrenzy is ERC20, Ownable {
    
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    MORTALFRNZYDividendTracker public dividendTracker;

    address public _frenzypoolWallet = 0x2BC9edd5D4c5777EcA43405274Dc9869e0A0d2be;

    bool public restrictWhales = false;

    uint256 public maxTxAmount = 200000000 * (10**9);
    uint256 public walletMax = 200000000 * (10**9);
    uint256 public swapTokensAtAmount = 2500000 * (10**9);

    uint256 public  BNBRewardsFee = 3;
    uint256 public  liquidityFee = 2;
    uint256 public  frenzyPool = 9;
    ...
    constructor() ERC20("Mortal Frenzy", "FRNZY") {

        dividendTracker = new MORTALFRNZYDividendTracker();
        totalFees = BNBRewardsFee.add(liquidityFee).add(frenzyPool);
    ...
    function updateFees(uint256 newRewardFee, uint256 newLiqFee, uint256 newFrenzyPool) public onlyOwner {
        BNBRewardsFee = newRewardFee;
        liquidityFee = newLiqFee;
        frenzyPool = newFrenzyPool;
        totalFees = BNBRewardsFee.add(liquidityFee).add(frenzyPool);
    }
    ...
    function setMarketingWallet(address payable wallet) public onlyOwner {
        _frenzypoolWallet = wallet;
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MORTALFRNZY: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MORTALFRNZY: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MORTALFRNZY: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MORTALFRNZY: Cannot update gasForProcessing to same value");
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        payable(_frenzypoolWallet).transfer(bnbForMarketing);
