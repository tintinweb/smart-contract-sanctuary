...
// File: contracts/StrategyBswUpgraderableBusd.sol
...
contract StrategyBswUpgraderableBusd is ReentrancyGuardUpgradeSafe, OwnableUpgradeable , Pausable {
    ...
    function initialize(
        ...
        if (isGrandComp) {
            ...
            earnedToToken0Path = [earnedAddress, wbnbAddress, busdAddress , token0Address];
            if (busdAddress == token0Address) {
                earnedToToken0Path = [earnedAddress, wbnbAddress , busdAddress];
            }

            earnedToToken1Path = [earnedAddress, wbnbAddress, busdAddress , token1Address];
            if (busdAddress == token1Address) {
                earnedToToken1Path = [earnedAddress, wbnbAddress , busdAddress];
            }

            token0ToEarnedPath = [token0Address, busdAddress , wbnbAddress, earnedAddress];
            if (busdAddress == token0Address) {
                token0ToEarnedPath = [busdAddress , wbnbAddress, earnedAddress];
            }

            token1ToEarnedPath = [token1Address, busdAddress ,wbnbAddress, earnedAddress];
            if (busdAddress == token1Address) {
                token1ToEarnedPath = [busdAddress , wbnbAddress, earnedAddress];
            }
        }
