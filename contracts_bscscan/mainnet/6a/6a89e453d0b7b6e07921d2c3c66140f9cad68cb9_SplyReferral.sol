...
contract SplyReferral is Owned , Referral{
  ...
  function buyTokens(uint _usdtAmount, address payable _referrer) public whenActive() {
    ...
    require(_usdtAmount >= 1000000000000000000, "Must have 100 usdt or more to buy");
    require(_usdtAmount <= 3000000000000000000, "Must buy less than 10000 usdt");
    //makes sure smart contract transfers SPLY from user
    ...
    require(buys[msg.sender].add(_usdtAmount) <= 300000000000000000000, "Must buy less than 10000 usdt in total");
    if(!hasReferrer(msg.sender)) {
      addReferrer(_referrer);
    }
