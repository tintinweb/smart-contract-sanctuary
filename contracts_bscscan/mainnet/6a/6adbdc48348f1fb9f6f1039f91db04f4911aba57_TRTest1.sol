...
contract TRTest1 is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "TRTest1";
    string constant _symbol = "TRTest1";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 500000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = (_totalSupply * 3) / 200; //3% of total supply
    uint256 public _maxWalletSize = (_totalSupply * 3) / 400; // 1.5% of total supply
    ...
    function setFees(uint256 _liquidityFee, uint256 _vaultfee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        vaultfee = _vaultfee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        totalFee = _liquidityFee.add(_vaultfee).add(_reflectionFee).add(_marketingFee);
