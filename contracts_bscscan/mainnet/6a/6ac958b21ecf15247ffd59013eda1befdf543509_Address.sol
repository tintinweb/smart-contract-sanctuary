...
contract yearnfinancecashcrowdfund is ReentrancyGuard, Context, Ownable {
    using SafeMath for uint256;
    ...
    //Start Pre-Sale
    function startICO(uint endDate, uint _minPurchase, uint _maxPurchase, uint availableTokens) external onlyOwner icoNotActive() {
        require(endDate > now, 'duration should be > 0');
        require(availableTokens > 0 && availableTokens <= _token.totalSupply(), 'availableTokens should be > 0 and <= totalSupply');
        require(_minPurchase > 0, '_minPurchase should > 0');
        endICO = endDate; 
        minPurchase = _minPurchase;
        maxPurchase = _maxPurchase;
        availableTokensICO = availableTokens;
    }
