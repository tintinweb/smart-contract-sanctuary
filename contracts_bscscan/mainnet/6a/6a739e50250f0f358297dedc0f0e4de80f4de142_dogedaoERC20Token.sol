...
/**
t.me/dogedaobsc (https://t.me/dogedaobsc)
 */
...
contract TWAPOracleUpdater is ERC20Permit, VaultOwned {
  ...
  function addTWAPSource( address newTWAPSourceDexPool_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.add( newTWAPSourceDexPool_ ), "dogedaoERC20TOken: TWAP Source already stored." );
    emit TWAPSourceAdded( newTWAPSourceDexPool_ );
  }

  function removeTWAPSource( address twapSourceToRemove_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.remove( twapSourceToRemove_ ), "dogedaoERC20TOken: TWAP source not present." );
...
contract dogedaoERC20Token is Divine {

  using SafeMath for uint256;

    constructor() Divine("dogedao", "dogedaoDAO", 9) {
    }
