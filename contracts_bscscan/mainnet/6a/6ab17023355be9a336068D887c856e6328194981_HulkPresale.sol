...
contract HulkPresale is Ownable {
    using SafeMath for uint256;
    ...
    function _buyTokens(address beneficiary, uint256 weiAmount) internal {
        ...
        uint256 tokens = weiAmount.mul(tokensPerBnb);
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "HulkPresale: sale is not open");
        require(!hasEnded(), "HulkPresale: sale is over");
        require(
            weiAmount >= minContribution,
            "HulkPresale: min contribution criteria not met"
        );
        require(
            contributions[beneficiary].add(weiAmount) <= maxContribution,
            "HulkPresale: max contribution criteria not met"
        );
        this;
    }

    function claimTokens() external {
        require(hasEnded(), "HulkPresale: presale is not over");
        require(softCapReached(), "HulkPresale: soft cap not reached, refund is available");
        require(
            contributions[msg.sender] > 0,
            "HulkPresale: nothing to claim"
        );
    ...
    function claimRefund() external {
        require(hasEnded(), "HulkPresale: presale is not over");
        require(!softCapReached(), "HulkPresale: soft cap not reached");
        require(
            contributions[msg.sender] > 0,
            "HulkPresale: nothing to claim"
        );
    ...
    function burnTokens() public onlyOwner {
        require(finalized, "HulkPresale: presale is not over");
