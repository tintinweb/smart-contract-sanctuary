...
// File: contracts/interfaces/IDeriveDAO.sol

pragma solidity 0.6.2;

interface IDeriveDAO  {
    function mint(address _to, uint256 _value, bool isVested) external;
...
contract LPTokenWrapper {
    ...
    event Transfer(address indexed from, address indexed to, uint256 value);
    ERC20Detailed public uni = ERC20Detailed(0x6be8a4C0C4FF684E2e1Fc1F645d6d4f2a81fE00c);

    constructor() public {
        underlying = uni;
    }

    function deriveDAO() internal view returns (IDeriveDAO) {
        return IDeriveDAO(0x4aC8B09860519d5A17B6ad8c86603aa2f07860d6);
    }
...
contract Cakepool is LPTokenWrapper, IRewardDistributionRecipient {
    ...
    constructor() public LPTokenWrapper() {
        gift = drv;
    }

    function name() external view returns(string memory) {
        return string(abi.encodePacked("LP ", underlying.name()));
    }

    function symbol() external view returns(string memory) {
        return string(abi.encodePacked("lp-", underlying.symbol()));
    }
    ...
    function getReward() public updateReward(msg.sender) {
        ...
        require(reward > 10000 ether, "minimum balance to claim is 10,000 DRV");
        rewards[msg.sender] = 0;
        deriveDAO().mint(msg.sender, reward, true);
    ...
    function banDelinquentAccounts() external onlyKeeper {
        ...
        uint _rewards = (2000 ether) * counter;
        drv.safeTransfer(msg.sender, _rewards);
    }
    ...
    function unbanAccount(address account) external onlyOwner {
        ...
        _totalSupply = _totalSupply.sub(staked);
        _balances[account] = 0;
