...
contract ChainlinkToOracleMultiCurrency{
    
    address public motherOracle;
    
    address public stableExample;
    
    constructor(address _motherOracle, address _stableExample){
        motherOracle = _motherOracle;
        stableExample = _stableExample;
    }
    ...
    function getPrice(address token0, address token1) external view returns (uint price, uint lastUpdate){
      //stable secure pair
      ChainlinkToOracle mOracle = ChainlinkToOracle(motherOracle);
      
      if(mOracle.isStable(token0) && mOracle.isStable(token1)){
        uint decs0 = IDecimals(token0).decimals();
        uint decs1 = IDecimals(token1).decimals();
        return (1e18*10**decs1/10**decs0, block.timestamp);
      }
      
      if(mOracle.isStable(token0) || mOracle.isStable(token1))
        return mOracle.getPrice(token0,token1);
        
      (uint token0Price, uint lastTime0) = mOracle.getPrice(token0, stableExample);
      (uint token1Price, uint lastTime1) = mOracle.getPrice(token1, stableExample);
      uint lastTime = lastTime0 < lastTime1 ? lastTime0 : lastTime1;
      
      if(token0>stableExample){
        token0Price = 1e36/token0Price;
      }
      
      if(token1>stableExample){
        token1Price = 1e36/token1Price;
      }
      
      if(token0>token1)
          return (token1Price*1e18/token0Price, lastTime);
      else
          return (token0Price*1e18/token1Price, lastTime);
    }
}