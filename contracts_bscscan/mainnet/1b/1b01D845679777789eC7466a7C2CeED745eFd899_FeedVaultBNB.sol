...
contract FeedVaultBNB is FeedVault {
     ...
     */
    constructor(
        address _entryFeesCollector,
        address _exitFeesCollector,
        ...
        entryFeesToTokenPath = [wBNB, entrySwapToken];
        exitFeesToTokenPath = [wBNB, exitSwapToken];
    }
     ...
     */
    function depositBNB() public payable isEnabled hasVaults nonReentrant {
        // Collect Target Vaults Profit Shares
        uint256 length = vaultInfo.length;
        for (uint256 i = 0; i < length; i++) {
            if (ITargetVault(vaultInfo[i].targetVault).balanceOfToken() > 0) ITargetVault(vaultInfo[i].targetVault).collectFees();
        }

        uint256 _beforeDepositBalance = depositedTokenBalance(false);

        // Transfer BNB from Depositor to Vault
        ...
        if (_entryFees > 0) {
            if (autoSwapEntryFees) {
                ...
                IPancakeRouter02(swapRouterAddress).swapExactETHForTokensSupportingFeeOnTransferTokens{value: _entryFees}(
                ...
                payable(address(entryFeesCollector)).transfer(_entryFees);
            }
        }
     ...
     */
    function _depositToVault(uint256 _vid, uint256 _amount) internal override {
        uint256 _balance = address(this).balance;
        if (_balance < _amount) {
            _amount = _balance;
        }
        if (_amount > 0) {
            payable(address(vaultInfo[_vid].targetVault)).transfer(_amount);
     ...
     */
    function withdraw(uint256 _shares) public override isEnabled hasVaults nonReentrant {
        UserInfo storage user = userInfo[address(msg.sender)];
        ...
        uint256 _tokenInVault = address(this).balance;
        ...
        // Collect Exit Fees
        uint256 _exitFees = _tokenAmount.mul(entryFeesBP).div(10000);
        if (_exitFees > 0) {
            if (autoSwapEntryFees) {
                ...
                uint256 amountOut = amounts[amounts.length.sub(1)];
                IPancakeRouter02(swapRouterAddress).swapExactETHForTokensSupportingFeeOnTransferTokens{value: _exitFees}(
                    amountOut.mul(slippageFactor).div(1000),
                    exitFeesToTokenPath,
                    address(this),
                    block.timestamp + 120
                ...
                emit FeesCollected(true, address(exitFeesCollector), address(exitSwapToken), _exitFees);
            } else {
                payable(address(exitFeesCollector)).transfer(_exitFees);
            }
    ...
    function setExitSwapToken(address _exitSwapToken) public override onlyRole(ADMIN_ROLE) nonReentrant {
        require(_exitSwapToken != address(0), "FeedVault(setExitSwapToken): exit swap token cannot be zero address");
     ...
     */
    function availableTokenBalance() public view override returns (uint256) {
