...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public constant ETH = address(0x2170Ed0880ac9A755fd29B2688956BD959F933F8); //ETH
...
contract SPC is ERC20, Ownable {
    ...
    address payable public marketingWallet = payable(address(0xa7A97aecFeEBF0eAB07BcE755F9620E885d3ad17)); //marketing wallet
    ...
    constructor() ERC20("SpiderCoin", "SPC") {
        ...
        sellFee.burn = 10;
        totalSellFee = sellFee.reward + sellFee.marketing;
        ...
        */
        _mint(owner(), 1 * 10**12 * (10**13));

        maxTxAmount = totalSupply().mul(100).div(100);
    }
    ...
    function setWallets(address payable _marketing) external onlyOwner {
    ...
    function _transfer(
        ...
        if (takeFee) {
            uint256 fees;
            uint256 burnAmount;
            require(amount <= maxTxAmount,"Amount exceeds transfer limit");
            
            if (automatedMarketMakerPairs[from]) {
                fees = amount.mul(totalBuyFee).div(1000);
                burnAmount = amount.mul(buyFee.burn).div(1000);
            } else if (automatedMarketMakerPairs[to]) {
                fees = amount.mul(totalSellFee).div(1000);
                burnAmount = amount.mul(sellFee.burn).div(1000);
            }

            if (fees > 0 || burnAmount > 0) {
...
contract TOKENDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()
        DividendPayingToken("SPC_Dividen_Tracker", "SPC_Dividend_Tracker")
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200 * (10**1); //must hold 200+ tokens
    }
