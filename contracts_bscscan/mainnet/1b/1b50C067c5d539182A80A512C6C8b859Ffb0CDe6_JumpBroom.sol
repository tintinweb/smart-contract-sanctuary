...
// Forked from Broom.sol
// SPDX-License-Identifier: MIT

contract JumpBroom is Ownable {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    address public constant JUMP = 0x78DE9326792ce1d6eCA0c978753c6953Cdeedd73; // jump token on fantom
    address public constant buyBackAddress = 0x84cC3900708cA8EBb561A61b90117b457F47E6B2; // hyperjump treasury
    ...
    function sweep(
        ...
        uint balance = IERC20(JUMP).balanceOf(address(this));
        uint buyBackAmount = balance.mul(buyBackRate).div(buyBackRateMax);

        _safeJUMPTransfer(buyBackAddress, buyBackAmount);
        _safeJUMPTransfer(msg.sender, balance.sub(buyBackAmount));
    }
    ...
    // Safe JUMP transfer function, just in case if rounding error causes pool to not have enough
    function _safeJUMPTransfer(address _to, uint256 _amount) private {
        uint256 balance = IERC20(JUMP).balanceOf(address(this));

        if (_amount > balance) {
            IERC20(JUMP).transfer(_to, balance);
        } else {
            IERC20(JUMP).transfer(_to, _amount);
        }
    }
}
