...
contract StrategyMasterChefClaim is BaseStrategyLP {
    ...
    address[] public earnedToClaimPath;

    uint256 public sharesClaimToken;

    /**
     * _farmAddresses: [_vaultChefAddress, _masterchefAddress, _wantAddress, _earnedAddress, _claimTokenAddress]
    */
    constructor(
        uint256 _pid,
        ...
        address[] memory _earnedToClaimPath
    ) {
        govAddress = msg.sender;
        vaultChefAddress = _farmAddresses[0];
        masterchefAddress = _farmAddresses[1];
        ...
        claimTokenAddress = _farmAddresses[4];
        ...
        earnedToClaimPath = _earnedToClaimPath;
    ...
    function earn() external override nonReentrant whenNotPaused onlyGov {
        ...
        if (earnedAmt > 0) {
            ...
            if (earnedAddress != claimTokenAddress && earnedAmt > 0) {
            ...
            uint256 claimTokenAmt = IERC20(claimTokenAddress).balanceOf(address(this));
            sharesClaimToken = sharesClaimToken.add(claimTokenAmt.div(vaultSharesTotal()));    
        }
    }

    function claim(address _to, uint256 _amount) external {
        require(msg.sender == vaultChefAddress, "only vaultChef");
        IERC20(claimTokenAddress).transfer(_to, _amount);
    }
