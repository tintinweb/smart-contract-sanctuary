...
contract GLIESENFT is ERC721, Ownable {
    ...
    mapping(uint256 => uint256) private _tokenPowers; // token
    mapping(address => uint256) private _addRessPowers; // address
    ...
    function recharge(itmap storage self, address key, uint256 shipTokenId, uint256 tokenAmount) internal returns (bool)
    {
        if (self.data[key][shipTokenId].isVaild) {
            ...
            if (shipTokenId > 0 && shipTokenId <= amountShip1) {
                if(self.data[key][shipTokenId].isPledge){
                    self.data[key][shipTokenId].energy = self.data[key][shipTokenId].overTime - block.timestamp + tokenAmount * 28800;
                    self.data[key][shipTokenId].overTime = self.data[key][shipTokenId].energy + block.timestamp;
                }else{
                ...
                if(self.data[key][shipTokenId].isPledge){
                    self.data[key][shipTokenId].energy = self.data[key][shipTokenId].overTime - block.timestamp + tokenAmount * 8640;
                    self.data[key][shipTokenId].overTime = self.data[key][shipTokenId].energy + block.timestamp;
                }else{
                ...
                if(self.data[key][shipTokenId].isPledge){
                    self.data[key][shipTokenId].energy = self.data[key][shipTokenId].overTime - block.timestamp + tokenAmount * 3928;
                    self.data[key][shipTokenId].overTime = self.data[key][shipTokenId].energy + block.timestamp;
                }else{
