    ...
    ERC20("Dragon Castle", "gDRAGON")
{
        ...
        returns (bool)
    {
        ...
        uint256 maxLastBlock = Math.max(_sender.lastStakingBlock, _receiver.lastStakingBlock);
        _receiver.lastStakingBlock = maxLastBlock;

        for (uint256 i = 0; i < rewardTokens.length; i++) {
            _sender.rewardDebt[rewardTokens[i]] = _sender
            .amount
            .mul(accTokenPerShare[rewardTokens[i]])
            .div(PRECISION_FACTOR[rewardTokens[i]]);

            _receiver.rewardDebt[rewardTokens[i]] = _receiver
            .amount
            .mul(accTokenPerShare[rewardTokens[i]])
            .div(PRECISION_FACTOR[rewardTokens[i]]);
        }
