...
contract GuitarNftFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    event Staked(address indexed user, uint256 tokenAmount, uint256 nftToBuyAmount);
    event Claimed(address indexed user, uint256 claimedTokenAmount, uint256 claimedNftAmount);
    ...
    constructor(
        ...
        require(_startAt >= block.timestamp, "Start time should be after current time");
        startAt = _startAt;
        endAt = _endAt;
    ...
    function stake(uint256 tokenAmountToStake, uint256 nftAmountToBuy) external nonReentrant {
        ...
        require(tokenAmountToStake >= nftAmountToBuy.mul(nftPrice), "Insufficient token amount");

        UserInfo storage user = userInfo[msg.sender];
        user.nftsPurchased = user.nftsPurchased.add(nftAmountToBuy);
        user.nftsLocked = user.nftsLocked.add(nftAmountToBuy);
        require(user.nftsPurchased <= maxPerUser, "Exceeds user purchasable limit");
        ...
        require(nftToken.balanceOf(address(this)) >= totalNftsSold, "Insufficient NFTs to be farmed");

        uint256 balanceBefore = stakingToken.balanceOf(address(this));
        stakingToken.safeTransferFrom(msg.sender, address(this), tokenAmountToStake);
        tokenAmountToStake = stakingToken.balanceOf(address(this)).sub(balanceBefore);
        if (depositFeeBP > 0) {
            uint256 depositFee = tokenAmountToStake.mul(depositFeeBP).div(10000);
            if (depositFee > 0) {
    ...
    function claim() external nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        require(user.tokensLocked > 0 || user.nftsLocked > 0, "Nothing to claim");
        require(stakingToken.balanceOf(address(this)) >= user.tokensLocked, "Less amount of staking token in the farm");
        require(nftToken.balanceOf(address(this)) >= user.nftsLocked, "Less amount of nfts in the farm");
        require(user.lastStakedAt.add(lockPeriod) <= block.timestamp, "Still in lock status");
    ...
    function setStartDate(uint256 _startAt) external onlyOwner {
        ...
        require(_startAt >= block.timestamp, "Start date should be after current time");
    ...
    function setEndDate(uint256 _endAt) external onlyOwner {
        require(_endAt >= block.timestamp, "End date should be after current time");
    ...
    function setTotalNftsToSell(uint256 _totalNftsToSell) external onlyOwner {
        require(_totalNftsToSell >= totalNftsSold, "Alreday sold more than this amount");
    ...
    //only owner can call this function
    function withdrawRemainedNfts(uint256 nftAmount) external onlyOwner nonReentrant {
        uint256 nftsInFarm = nftToken.balanceOf(address(this));
        require(nftsInFarm >= nftAmount.add(totalNftsLocked), "Insufficient NFT amount");
