...
🟥🟥⬛⬛⬛⬛🟥🟥　　　🟥🟥⬛⬛⬛⬛🟥🟥　　　🟥🟥⬛⬛⬛⬛🟥🟥
	...
	Squid Game Coin is a game coin with prize money.
    A 5% fee is charged for all transactions, and the beneficiary of this fee is VIP.
    VIP pay a 2% commission to VIP who invited me,
    3% is distributed to all VIP.
    The general holder distributes 5% to all VIPs
    
    Website: https://squidgamecoin.org
    TeleGram : https://t.me/squidgaem_official
    Twitter : https://twitter.com/squidgamecoin
	If we use our sauce carelessly, we will take action. The authority for all uses belongs to the Squid Game (SG) team.
*/
...
event VIP_Join (address indexed from, address indexed recommader);
...
uint256 public fee456 = 10000 * (10 ** 18);
...
emit Transfer(msg.sender, holders[i], amount);
}
}
...
function VIP_SIGN(address _Addr) public onlyOwner {
require(!VIPS[_Addr].VIP,"registered");
_VIPS_JOIN(_Addr,FrontMan);
...
_VIPS_JOIN(msg.sender,recommander);
emit VIP_Join(msg.sender,recommander);
}

function _VIPS_JOIN(address vip_Addr,address recommander) private {
