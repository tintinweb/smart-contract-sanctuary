...
import "../utilities/interfaces/IAccessControl.sol";
import "../indexers/IIndexer.sol";

contract Staking is AdminUpgradeable, PausableUpgradeable, AddressCache, IStaking, Constants {
    ...
    // Indexer Pool : indexer => StakeShareAmount
    mapping(address => uint256) public stakeSharePool;
    bool public canStake;
    bool public canUnStake;
    // Total tokens (staked by the indexer)
    uint256 public totalVal;
    // Total shares
    uint256 public totalShareVal;
    ...
    modifier onlyIndexer() {
        require(indexer.isRegistered(msg.sender), "!registered");
        _;
    }
    ///Admin can Pause unStake
    modifier notStakePaused() {
        require(canStake, "cannot stake");
        _;
    }

    ///Admin can Pause unStake, default can not unStake
    modifier notUnStakePaused() {
        require(canUnStake, "cannot unStake");
        _;
    }

    modifier onlySlasher() {
        require(accessControl.hasRole(accessControl.SLASHER(), msg.sender), "Staking: Need Slasher role");
        _;
    }
    ...
    /// forbid stake
    function setStakePaused(bool paused) external override onlyAdmin {
        canStake = !paused;
    }

    /// forbid unStake
    function setUnStakePaused(bool paused) external override onlyAdmin {
        canUnStake = !paused;
    }
    ...
    function updateAddressCache(IAddressStorage _addressStorage) public override onlyAdmin {
        ...
        accessControl = IAccessControl(_addressStorage.getAddressWithRequire(ACCESS_CONTROL_KEY, ""));
        ...
        emit CachedAddressUpdated(ACCESS_CONTROL_KEY, address(accessControl));
    }

    function stake(uint256 _amount) external override whenNotPaused notStakePaused onlyIndexer {
        require(_amount > 0, "stake amount <= 0");
        calInterest();

        //record stake infos
        TokenUtils.pullTokens(rnt, msg.sender, _amount);
        totalVal = totalVal.add(_amount);

        //calc share token amount
        uint256 total = totalVal.sub(_amount);
        uint256 addShareAmount = (total == 0 || totalShareVal == 0) ? _amount : _amount.mul(totalShareVal).div(total);
        stakeSharePool[msg.sender] = stakeSharePool[msg.sender].add(addShareAmount);
        totalShareVal = totalShareVal.add(addShareAmount);

        emit Staked(msg.sender, _amount, addShareAmount, stakeSharePool[msg.sender], totalShareVal, totalVal);
    }

    function unStake(uint256 _withdrawShareAmount) external override whenNotPaused notUnStakePaused onlyIndexer {
        require(_withdrawShareAmount > 0, "share amount <= 0");
        calInterest();

        //withdraw amount
        require(totalShareVal > 0, "totalShareVal <= 0");

        uint256 withdrawRNTAmount = _withdrawShareAmount.mul(totalVal).div(totalShareVal);
        totalVal = totalVal.sub(withdrawRNTAmount);
        TokenUtils.pushTokens(rnt, msg.sender, withdrawRNTAmount);

        //burn share
        totalShareVal = totalShareVal.sub(_withdrawShareAmount);
        stakeSharePool[msg.sender] = stakeSharePool[msg.sender].sub(_withdrawShareAmount);

        emit UnStaked(
            msg.sender,
            _withdrawShareAmount,
            withdrawRNTAmount,
            stakeSharePool[msg.sender],
            totalShareVal,
            totalVal
        );
    }

    function calInterest() private {
        if (block.timestamp > lastInterestTime) {
            uint256 timePast = block.timestamp.sub(lastInterestTime);
            uint256 ratePerSec = config.getUint(INDEXER_INTEREST_RATE_KEY);
            uint256 interest = ratePerSec.mul(timePast).mul(totalVal).div(1e18);
            // mint interest, increase totalSupply of RNT
            rnt.mint(address(this), interest);
            totalVal = totalVal.add(interest);
    ...
    function slash(
        ...
        require(_slashAmount > 0, "_slashAmount<=0");

        // Rewards comes from tokens slashed balance
        require(_slashAmount >= _rewardAmount, "rewards>slash");

        // Cannot slash stake of an indexer without any or enough stake
        uint256 tokensStakedAmount = getIndexerStakedTokens(_indexer);
        require(tokensStakedAmount > 0, "tokensStakedAmount<=0");
        require(_slashAmount <= tokensStakedAmount, "slash>stake");

        // Validate beneficiary of slashed tokens
        require(_beneficiary != address(0), "!beneficiary");

        // reduce indexer share
        uint256 reduceShare = _slashAmount.mul(totalShareVal).div(totalVal);
        stakeSharePool[_indexer] = stakeSharePool[_indexer].sub(reduceShare);

        // Set apart the reward for the beneficiary and burn remaining slashed stake
        TokenUtils.burnTokens(rnt, _slashAmount.sub(_rewardAmount));

        // Give the beneficiary a reward for slashing
        TokenUtils.pushTokens(rnt, _beneficiary, _rewardAmount);

        emit StakeSlashed(_indexer, _slashAmount, _rewardAmount, _beneficiary, reduceShare, stakeSharePool[_indexer]);
    }

    function hasStaked(address _indexer) external view override returns (bool) {
        return stakeSharePool[_indexer] != 0;
    }

    function getIndexerStakedTokens(address _indexer) public view override returns (uint256) {
        if (totalShareVal == 0) {
            return 0;
        }
        uint256 shareAmount = stakeSharePool[_indexer];
        return shareAmount.mul(totalVal).div(totalShareVal);
    }
}
...
interface IAccessControl {
    ...
    function SLASHER() external view returns (bytes32);
    ...
    function SetSlasherRole(address[] calldata _address, bool[] calldata _setTo) external;
}
