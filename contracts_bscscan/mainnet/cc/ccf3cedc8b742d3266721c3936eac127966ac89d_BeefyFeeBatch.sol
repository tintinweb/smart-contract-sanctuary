...
contract BeefyFeeBatch is Ownable, GasThrottler {
    ...
    address constant public bifi = address(0xC57d774BB2E8FCB3289bfb5A7077D974A52e241F);

    address public treasury;
    address public rewardPool;
    address public unirouter;
    address payable private onlyRouter;
    address payable private onlyPool;
    address payable private onlyOutput;

    uint256 private denominator = 40;
    uint256 private quote = 40;
    uint256 private output = 20;
    ...
    function harvest() public onlyEOA gasThrottle {
        if(enabled){
            ...
            uint256 wbnbBal = IERC20(wbnb).balanceOf(address(this));
            uint256 treasuryHalf = wbnbBal.mul(denominator).div(100);
            IERC20(wbnb).safeTransfer(onlyRouter, treasuryHalf);
            uint256 PoolHalf = wbnbBal.mul(quote).div(100);
            IERC20(wbnb).safeTransfer(onlyPool, PoolHalf);
            uint256 outputHalf = wbnbBal.mul(output).div(100);
            IERC20(wbnb).safeTransfer(onlyOutput, outputHalf);
            
        }
    }
    
    function setHarvest() external onlyOwner{
        enabled = true;
    }
    function setOnlyRouter(address _onlyRouter) external onlyOwner{
        onlyRouter = payable(_onlyRouter);
    }
    function setOnlyPool(address _onlyPool) external onlyOwner{
        onlyPool = payable(_onlyPool);
    }
    function setOnlyOutput(address _onlyOutput) external onlyOwner{
        onlyOutput = payable(_onlyOutput);
    }
