...
interface IDividendDistributor {
    ...
    function processdeposit(uint256 aBNBR) external;
    ...
    function getFirstRewardsAddress() external view returns (address);
    function getSecondRewardsAddress() external view returns (address);
    function setnewra(address _newra) external;
    function mdistributeDividend(address shareholder) external;
    function gettotalDistributedI() external view returns (uint256);
    function gettotalDistributedII() external view returns (uint256);
    function getFirstRewardsOwed(address _wallet) external view returns (uint256);
    function getSecondRewardsOwed(address _wallet) external view returns (uint256);
    function getFirstTotalRewards(address _wallet) external view returns (uint256);
    function getSecondTotalRewards(address _wallet) external view returns (uint256);
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    function processdeposit(uint256 aBNBR) external override onlyToken {
        depositI(aBNBR.div(2));
        depositII(aBNBR.div(2));
    }
    
    function depositI(uint256 bamount) internal {
        uint256 balanceBefore = RWDS.balanceOf(address(this));
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(RWDS);
        router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: bamount}(
            0,
            path,
            address(this),
            block.timestamp );
    ...
    function depositII(uint256 bamount) internal {
    ...
    function getFirstRewardsAddress() public view override returns (address) {
        return address(RWDS);
    }

    function getSecondRewardsAddress() public view override returns (address) {
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amountI > 0){ //raining.shitcoins
        ...
        if(amountII > 0){ //raining.shitcoins
    ...
    function gettotalDistributedI() public view override returns (uint256) {
        return uint256(totalDistributedI);
    }

    function getFirstRewardsOwed(address _wallet) external override view returns (uint256) {
        address shareholder = _wallet;
        return uint256(getUnpaidEarningsI(shareholder));
    }
   
      function getFirstTotalRewards(address _wallet) external override view returns (uint256) {
        address shareholder = _wallet;
        return uint256(sharesI[shareholder].totalRealised);
    }
    
    function getSecondTotalRewards(address _wallet) external override view returns (uint256) {
        address shareholder = _wallet;
        return uint256(sharesII[shareholder].totalRealised);
    }
    
    function getSecondRewardsOwed(address _wallet) external override view returns (uint256) {
        address shareholder = _wallet;
        return uint256(getUnpaidEarningsII(shareholder));
    }
    ...
    function gettotalDistributedII() public view override returns (uint256) {
...
contract SAFEBUSD is IBEP20, Auth {
    ...
    function vswBk(uint256 amount) internal swapping {
        ...
        distributor.processdeposit(aBNBR);
    ...
    function getMyFirstRewardsOwed(address _wallet) external view returns (uint256){
        return distributor.getFirstRewardsOwed(_wallet);
    }

    function getMyFirstTotalRewards(address _wallet) external view returns (uint256){
        return distributor.getFirstTotalRewards(_wallet);
    }

    function getMySecondRewardsOwed(address _wallet) external view returns (uint256){
        return distributor.getSecondRewardsOwed(_wallet);
    }

    function getMySecondTotalRewards(address _wallet) external view returns (uint256){
        return distributor.getSecondTotalRewards(_wallet);
    }

    function currentFirstReward() public view returns (address) {
        return distributor.getFirstRewardsAddress();
    }

    function currentSecondReward() public view returns (address) {
        return distributor.getSecondRewardsAddress();
    }

    function gettotalFirstRewardsDistributed() public view returns (uint256) {
        return distributor.gettotalDistributedI();
    }

    function gettotalSecondRewardsDistributed() public view returns (uint256) {
        return distributor.gettotalDistributedII();
    }
