...
contract RugScanToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public _buyBackToken = 0x994517e000aa3F117E7Ad61B0e2336C76B4Fd94a;
    ...
    RugScanTokenDividendTracker public dividendTracker;

    address public marketingWallet;
    address public devWallet;

    uint256 public maxTranscationAmount = 1000000000 * (10**9);
    ...
    constructor() ERC20("RugScan", "RGSN") {
        uint256 _dividendRewardsFee = 2;
        uint256 _marketingFee = 1;
        uint256 _buyBackFee = 2;
        uint256 _liquidityFee = 5;
        uint256 _swapFee = _buyBackFee.add(_marketingFee);
        ...
        swapAmount = _swapFee;
        totalFees = _dividendRewardsFee.add(_swapFee).add(_liquidityFee);

        dividendTracker = new RugScanTokenDividendTracker();

        marketingWallet = 0xBCc77E3B972D3E5BC519a2350D00dc7D74C5cEd3;
        devWallet = 0xb6658AE3BD78a03403c0b6989E9c1083c1cf5907;
    ...
    function setMaxTransaction(uint256 maxTxn) external onlyOwner {
        maxTranscationAmount = maxTxn * (10**9);
    }
        ...
        onlyOwner
    {
        require(
            ...
            "RugScan: Sell transaction multipler must be between 100 (1x) and 200 (2x)"
        );
    ...
    function prepareForPreSale() external onlyOwner {
        ...
        dividendRewardsFee = 2;
        buyBackEnabled = true;
        marketingFee = 1;
        buyBackFee = 2;
        liquidityFee = 5;
        swapAmount = 3;
        maxTranscationAmount = 1000000000 * (10**9);
    }
    ...
    function afterPreSale() external onlyOwner {
        ...
        swapAmount = 3;
        buyBackEnabled = true;
        maxTranscationAmount = 1000000000000 * (10**9);
    }
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(
            !swapping,
            "RugScan: A swapping process is currently running, wait till that is complete"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "RugScan: The dividend tracker already has that address"
        );

        RugScanTokenDividendTracker newDividendTracker = RugScanTokenDividendTracker(
                payable(newAddress)
            );

        require(
            newDividendTracker.owner() == address(this),
            "RugScan: The new dividend tracker must be owned by the FLOKIBUSD token contract"
        );
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "RugScan: Dividend reward tax must be between 0 and 10"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "RugScan: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "RugScan: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "RugScan: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "RugScan: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "RugScan: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "RugScan: Cannot update gasForProcessing to same value"
        );
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[from]) {
            ...
            if (!swapping && canSwap) {
                ...
                transferToWallet(
                    ...
                    address(this).balance.div(10**2).mul(marketingFee).div(2)
                );
                ...
                if (buyBackEnabled) {
                    swapBNBForTokens(buyBackBalance.div(10**2).mul(buyBackFee));
                }
    ...
    function swapBNBForTokens(uint256 amount) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            amount,
            0, // accept any amount of MTDR token
            path,
            deadAddress,
            block.timestamp
        );
...
contract RugScanTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "RugScan_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RugScan_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RugScan contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "RugScan_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "RugScan_Dividend_Tracker: Cannot update claimWait to same value"
        );
