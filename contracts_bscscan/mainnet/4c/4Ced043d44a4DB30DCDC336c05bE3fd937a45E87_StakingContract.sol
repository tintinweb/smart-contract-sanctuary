...
contract StakingContract is IReflectionPool, Auth, Pausable {
  ...
  //Event declarations
  event EnterStaking(address, uint256);
  event LeaveStaking(address, uint256);
  event Harvest(address, uint256);
  event PoolLiquified(uint256, uint256);

  constructor(
    address _router,
    IBEP20 _rewardsToken,
    SafeEarn _stakingToken
  ) Auth(msg.sender) {
  ...
  function enterStaking(uint256 amount) external whenNotPaused {
    if (amount == 0) amount = tokenPool.stakingToken.balanceOf(msg.sender);
  ...
  function leaveStaking(uint256 amt) external {
    ...
    emit LeaveStaking(msg.sender, amt);
  }
  ...
  function addShareHolder(address shareholder, uint256 amount) internal {
  ...
  function _removeShares(uint256 amt) internal {
  ...
  function isLiquid(uint256 amount) internal view returns (bool) {
  ...
  function getRewardsToken(uint256 amt) internal returns (uint256) {
    uint256 balanceBefore = rewardsToken.balanceOf(address(this));
    router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amt}(0, getRewardsTokenPath(), address(this), block.timestamp);
  ...
  function fuckShitcoins(IBEP20 _shitcoin) external authorized {
    ...
    //
    uint256 balanceBefore = rewardsToken.balanceOf(address(this));
    router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amt}(0, path, address(this), block.timestamp);
    uint256 amount = rewardsToken.balanceOf(address(this)).sub(balanceBefore);
    //
    tokenPool.totalDividends = tokenPool.totalDividends.add(amount);
    tokenPool.dividendsPerShare = tokenPool.dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(amount).div(tokenPool.totalShares));
