...
contract GOMAMasterChef is Ownable {
    ...
    function setRewardTokensPerSecond(uint256 _tokenPerSecond) external onlyOwner {
    ...
    // 
    function supplyStakeTokens(IBEP20 _stakeToken, uint256 _amount) public {
		_stakeToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        emit Supply(msg.sender, _amount);
    }
    
    // Update the given pool's allocation point. 
    function withdrawRewardTokens(address _to, uint256 _amount) external onlyOwner {
