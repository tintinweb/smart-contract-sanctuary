...
contract MasterChef is Ownable {
    ...
    function pendingAvocado(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accAvocadoPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 avocadoReward = multiplier.mul(avocadoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        avocado.mint(devaddr, avocadoReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accAvocadoPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeAvocadoTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accAvocadoPerShare).div(1e12);
