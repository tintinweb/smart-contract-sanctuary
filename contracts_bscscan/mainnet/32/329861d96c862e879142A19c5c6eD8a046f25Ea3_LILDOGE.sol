...
contract LILDOGE is ERC20, Ownable {
    ...
    uint256 public dogeDividendRewardsFee = 10;
    ...
    constructor() ERC20("Lil Doge Coin", "LILDOGE") {
    	dogeDividendTracker = new DogeDividendTracker();
 
    	marketingWallet = 0xe2A4c688EA049F5Ef2D4306C9C2DA1ebb2FEc84a;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "LILDOGE: The marketing wallet is already this address");
    ...
    function updateDogeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dogeDividendTracker), "LILDOGE: The dividend tracker already has that address");
 
        DogeDividendTracker newDogeDividendTracker = DogeDividendTracker(payable(newAddress));
 
        require(newDogeDividendTracker.owner() == address(this), "LILDOGE: The new dividend tracker must be owned by the LILDOGE token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "LILDOGE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "LILDOGE: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "LILDOGE: The PanadaSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "LILDOGE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "LILDOGE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "LILDOGE: Trading has not started yet");
...
contract DogeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("LILDOGE_Doge_Dividend_Tracker", "LILDOGE_Doge_Dividend_Tracker", 0xbA2aE424d960c26247Dd6c32edC70B295c744C43) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "LILDOGE_Doge_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "LILDOGE_Doge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LILDOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "LILDOGE_Doge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "LILDOGE_Doge_Dividend_Tracker: Cannot update claimWait to same value");
...

{
  ...
  "libraries": {
    "contracts/lildogecoin.sol": {
      "IterableMapping": "0x865ADF5Ad659c87a0801160D9A87d65e7442F70C"
    }
  }
}