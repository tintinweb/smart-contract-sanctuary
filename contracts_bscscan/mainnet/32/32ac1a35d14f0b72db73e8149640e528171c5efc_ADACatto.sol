...
/**
  ...
  */
  ...
  contract ADACatto is ERC20, Ownable {
      ...
      uint256 public constant LiquidityFeeBots = 25;
      ...
      uint256 public constant firstDividendRewardsFeeBots = 50;
      ...
      uint256 public totalFeesBots = firstDividendRewardsFeeBots.add(marketingFee).add(LiquidityFeeBots);
      ...
      event BlacklistBot(address indexed account, bool isBot);
      ...
      function setMarketingEnabled(bool _enabled) external onlyOwner {
          ...
          if (_enabled == false) {
              ...
              totalFeesBots = marketingFee.add(firstDividendRewardsFeeBots).add(LiquidityFeeBots);
              marketingEnabled = _enabled;
          }
      ...
      function blacklistBot(address account, bool listed) external onlyOwner {
          require(isBot[account] != listed, "ADACatto: Account is already blacklisted");
          isBot[account] = listed;

          emit BlacklistBot(account, listed);
      ...
      function getIsBot(address account) public view returns(bool) {
      ...
      function _transfer(
          ...
          if (!swapping && canSwap && from != uniswapV2Pair) {
              ...
              if(swapAndLiquifyEnabled) {
                  uint256 liqTokens;
                  if(botAccount) {
                    liqTokens = contractTokenBalance.mul(LiquidityFeeBots).div(totalFeesBots);
                  }
                  else {
                    liqTokens = contractTokenBalance.mul(liquidityFee).div(totalFees);
                  }
              ...
              if (firstDividendEnabled) {
                  uint256 firstTokens;
                  if(botAccount) {
                    firstTokens = contractTokenBalance.mul(firstDividendRewardsFeeBots).div(totalFeesBots);
                  }
                  else {
                    firstTokens = contractTokenBalance.mul(firstDividendRewardsFee).div(totalFees);
                  }
          ...
          if(takeFee) {
            ...
            if(botAccount) {
              fees = amount.mul(totalFeesBots).div(100);
            }
