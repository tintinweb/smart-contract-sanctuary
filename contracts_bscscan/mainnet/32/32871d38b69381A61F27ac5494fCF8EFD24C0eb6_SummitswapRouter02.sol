...
import 'summitswaplib/contracts/utils/TransferHelper.sol';
...
contract SummitswapRouter02 is ISummitswapRouter02 {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = SummitswapLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = SummitswapLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'SummitswapRouter02: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = SummitswapLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = ISummitswapPair(pair).burn(to);
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, SummitswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, SummitswapLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
...
import 'summitswapcore/contracts/interfaces/ISummitswapPair.sol';
import "./SafeMath.sol";
...
library SummitswapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'7e6c98602946f154bf9bc4d0128a9ec68f3bd5d0d593676d04e43a1cf9237ac4' // init code hash
            ))));
    }
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = ISummitswapPair(pairFor(factory, tokenA, tokenB)).getReserves();
