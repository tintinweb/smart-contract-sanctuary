...
contract BlindBoxLive {
  ...
  function initializeBox(string memory name,uint256 _time) adminAdd public {
    if(boxDetails[name].initialize > 0){
      for(uint256 i = 0; i < (Category[name].count / 10); i++){
       for (uint256 j = 0; j < 101; j++) {
    ...
    uint256 counter;
      for(uint256 i = 0; i < (Category[name].count / 10); i++){
       for (uint256 j = 0; j < 100 || (i == 99 && j < 101); j++) {
          randomValueMain = chainRan.getRandomVal();
          uint256 randNumber =  (uint256(keccak256(abi.encode(randomValueMain, (block.timestamp + counter++ )))) % (i ==99 ? 1100 : 1000));
          boxDetails[name].luckyBox[i][randNumber] = true;
        }
      }
    for (uint256 index = 0; index < (Category[name].count / 10) ; index++) {
        boxDetails[name].usedCountByday[index].isSold = false;
      boxDetails[name].usedCountByday[index].count = (index == ((Category[name].count / 10) -1) ? (10 + Category[name].count % 10) : 10);
