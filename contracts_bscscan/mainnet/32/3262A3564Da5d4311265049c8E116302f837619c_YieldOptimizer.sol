...
contract YieldOptimizer {
    ...
    function FinsCall(
        ...
        uint amountOut = pancakeRouter.getAmountsOut(_amount0, path)[1];
        uint requiredAmountOut = autosharkRouter.getAmountsOut(_amount0, path)[1];

        require(amountOut > requiredAmountOut, "FinsCall: pancakeprice is not bigger");
        ...
        require(amountReceived > requiredAmountOut, "FinsCall: didnt receive enuff other token");

        IERC20 otherToken = IERC20(token1);
        otherToken.transfer(msg.sender, requiredAmountOut);
        otherToken.transfer(tx.origin, amountReceived - requiredAmountOut);
    }
    ...
    function pancakeCall(
        ...
        uint amountOut = autosharkRouter.getAmountsOut(_amount0, path)[1];
        uint requiredAmountOut = pancakeRouter.getAmountsOut(_amount0, path)[1];

        require(amountOut > requiredAmountOut, "pancakeCall: sharkprice is not bigger");
        ...
        require(amountReceived > requiredAmountOut, "pancakeCall: didnt receive enuff other token");
