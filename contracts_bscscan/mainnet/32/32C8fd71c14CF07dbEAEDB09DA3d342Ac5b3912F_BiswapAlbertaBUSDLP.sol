...
import "../interfaces/biswap/IBiswapFactory.sol";
import "../interfaces/biswap/IBiswapRouter02.sol";
import "../interfaces/biswap/IBiswapPair.sol";

contract BiswapAlbertaBUSDLP is IAlberta, Ownable, ReentrancyGuard {
    ...
    IBiswapFactory public constant FACTORY = IBiswapFactory(0x858E3312ed3A876947EA49d572A7C42DE08af7EE);
    IBiswapRouter02 public constant ROUTER = IBiswapRouter02(0x3a6d8cA21D1CF76F653A67577FA0D27453350dD8);

    constructor() {
        IERC20(BUSD).safeApprove(address(ROUTER), type(uint256).max); // 100% trust in the router
    }

    function resetApprove() external {
        IERC20(BUSD).safeApprove(address(ROUTER), 0);
        IERC20(BUSD).safeApprove(address(ROUTER), type(uint256).max);
    }
    ...
    function optimalDeposit(
        ...
        if (amtA * resB >= amtB * resA) {
            swapAmt = _optimalDepositA(amtA, amtB, resA, resB, swapFee);
            isReversed = false;
        } else {
            swapAmt = _optimalDepositA(amtB, amtA, resB, resA, swapFee);
            isReversed = true;
        }
    }
    ...
    function _optimalDepositA(
        ...
        uint256 a = 1000 - swapFee;
        uint256 b = (2000 - swapFee) * resA;
    ...
    function add(
        ...
        IBiswapPair lpToken = IBiswapPair(_wantToken);
        address otherToken = lpToken.token0() == BUSD ? lpToken.token1() : lpToken.token0();

        // Preparing BUSD or farming tokens
        if (_paidToken == BUSD) {
            IERC20(BUSD).safeTransferFrom(msg.sender, address(this), _amountIn);
            ...
            (pathBnbToBusd[0], pathBnbToBusd[1]) = (WBNB, BUSD);

            uint256 bnbBal = address(this).balance;
            _safePriceImpactSwap(bnbBal, pathBnbToBusd, _maxPriceImpact);
            ROUTER.swapExactETHForTokens{value: bnbBal}(0, pathBnbToBusd, address(this), block.timestamp);
        } else {
            // paid == others
            IERC20(_paidToken).safeTransferFrom(msg.sender, address(this), _amountIn);

            address[] memory pathToPaid = new address[](3);
            (pathToPaid[0], pathToPaid[1], pathToPaid[2]) = (_paidToken, WBNB, BUSD);
        ...
        // Compute the optimal amount of BUSD and otherToken to be converted.
        uint256 swapAmt;
        bool isReversed;
        {
            (uint256 r0, uint256 r1, ) = lpToken.getReserves();
            (uint256 busdReserve, uint256 otherReserve) = lpToken.token0() == BUSD ? (r0, r1) : (r1, r0);
            uint32 swapFee = lpToken.swapFee();
    ...
    function remove(
        ...
        // Remove all liquidity back to BUSD and farming tokens.
        ...
        // Convert farming tokens to BUSD.
        address[] memory pathToBusd = new address[](2);
        (pathToBusd[0], pathToBusd[1]) = (otherToken, BUSD);

        IERC20(otherToken).safeApprove(address(ROUTER), 0);
        IERC20(otherToken).safeApprove(address(ROUTER), type(uint256).max);

        uint256 otherBal = IERC20(otherToken).balanceOf(address(this));
        _safePriceImpactSwap(otherBal, pathToBusd, _maxPriceImpact);
        ROUTER.swapExactTokensForTokens(otherBal, 0, pathToBusd, address(this), block.timestamp);

        // Return all back to the original caller.
        if (_paidToken == BUSD) {
            IERC20(BUSD).safeTransfer(_to, IERC20(BUSD).balanceOf(address(this)));
        } else if (_paidToken == address(0)) {
            // paid == BNB
            ...
            _safePriceImpactSwap(busdBal, pathToBnb, _maxPriceImpact);
            ROUTER.swapExactTokensForETH(busdBal, 0, pathToBnb, _to, block.timestamp);
        } else {
            // BUSD to paid token
            address[] memory pathToPaid = new address[](3);
            (pathToPaid[0], pathToPaid[1], pathToPaid[2]) = (BUSD, WBNB, _paidToken);

            uint256 busdBal = IERC20(BUSD).balanceOf(address(this));
            _safePriceImpactSwap(busdBal, pathToPaid, _maxPriceImpact);
            ROUTER.swapExactTokensForTokens(busdBal, 0, pathToPaid, _to, block.timestamp);
        }
    }
    ...
    function _safePriceImpactSwap(
        ...
        for (uint256 i; i < _path.length - 1; i++) {
            (uint256 rIn, uint256 rOut) = BiswapLibrary.getReserves(address(FACTORY), _path[i], _path[i + 1]);
            uint256 q = ROUTER.quote(_amountIn, rIn, rOut);
            uint256 out = BiswapLibrary.getAmountOut(_amountIn, rIn, rOut);
...
library BiswapLibrary {
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IBiswapPair(pairFor(factory, tokenA, tokenB)).getReserves();
