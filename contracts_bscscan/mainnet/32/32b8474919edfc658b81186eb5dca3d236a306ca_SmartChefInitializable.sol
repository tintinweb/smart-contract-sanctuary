...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
     ...
     * @param _opAccount: account to send fee
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (feeAmount > 0) {
                stakedToken.safeTransfer(address(opAccount), feeAmount);
            }            
        }
    ...
    function updateEndBlock(uint256 _bonusEndBlock) external onlyOwner {
        require(startBlock < _bonusEndBlock, "_bonusEndBlock is invalid");
        bonusEndBlock = _bonusEndBlock;
    }
     ...
     */
    function updateFee(uint256 _depositFee) external onlyOwner {
        require(_depositFee < 10000, "_depositFee is too big");
        depositFee = _depositFee;
    }

    /*
     * @notice Update op account
     */
    function updateOpAccount(address _opAccount) external onlyOwner {
        require(_opAccount != address(0), "_opAccount cant be zero address");
        opAccount = _opAccount;
    }
}