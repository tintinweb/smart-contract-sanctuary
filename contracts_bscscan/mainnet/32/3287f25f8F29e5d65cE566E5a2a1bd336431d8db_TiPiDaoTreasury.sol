...
contract TiPiDaoTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, STPD }
    ...
    address public sTPD;
    uint public sTPDQueue; // Delays change to sTPD address
    ...
    /**
        @notice allow approved address to deposit an asset for TPD
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint TPD needed and store amount of rewards for distribution
        // s = 100000000000 - 99909426854 = 0.090573146
        send_ = value.sub( _profit );
        IERC20Mintable( TPD ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn TPD for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ITPDERC20( TPD ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sTPD ).balanceOf( msg.sender ); // Can only borrow against sTPD held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with TPD
        @param _amount uint
     */
    function repayDebtWithTPD( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ITPDERC20( TPD ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, TPD, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( TPD ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( TPD ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns TPD valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match TPD decimals
            value_ = _amount.mul( 10 ** IERC20( TPD ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.STPD ) { // 9
            sTPDQueue = block.number.add( blocksNeededForQueue );
        } else return false;
