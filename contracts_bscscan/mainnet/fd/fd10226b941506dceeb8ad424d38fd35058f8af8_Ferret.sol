...
contract Ferret is ERC20, Ownable {
   ...
   FerretDividendTracker public dividendTracker;
    ...
    uint256 public gasForProcessing = 2000000;
    ...
    constructor() ERC20 ("Ferret", "FRT") {
		...
		marketingWallet = payable(0x32A66f8cD505884c34519f137Dd58DeC8e4e21Af);
    ...
    function excludeFromMaxSell(address account, bool excluded) public onlyOwner {
        require(_isExcludedMaxSellTransactionAmount[account] != excluded, "Ferret: Account is already the value of 'excluded'");
    ...
    function _transfer(
        ...
        if(!swapping && automatedMarketMakerPairs[to] && from != address(uniswapV2Router) && !_isExcludedFromFees[to] && !_isExcludedMaxSellTransactionAmount[from])
        {
		...
		if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != liquidityWallet && to != liquidityWallet && from != marketingWallet && to != marketingWallet) {
			...
			uint256 liquidityAmount = contractTokenBalance.mul(_liquidityFee).div(100);
			...
			uint256 sellTokens = contractTokenBalanceAmount.mul(_RewardsFee).div(100);
...
contract FerretDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "Ferret_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
