...
/**
   [̲̅S][̲̅T][̲̅E][̲̅A][̲̅L][̲̅T][̲̅H]      [̲̅L][̲̅A][̲̅U][̲̅N][̲̅C][̲̅H 
 
   Welcome to Rocket Doge Stealth Launch!
   
   
   
   This is a community token. Our telegram is @RocketDogeStealthLaunch
   Important: The early you join the group, the more gain you got.
   
   It's a community token, every holder should promote it, 
   ...
   I will renounce the ownership to burn addresses to transfer #RocketDogeStealthLaunch to the community, make sure it's 100% safe.

   I will add .2 BNB and all the left 10% total supply to the pool
   Can you make #RocketDogeStealthLaunch 10000X? 

   1,000,000,000,000,000 total supply
   5,000,000,000,000 tokens limitation for trade, which is 0.5% of the total supply
   
   9% fee for liquidity will go to an address that the contract creates, 
   and the contract will sell it and add to liquidity automatically, 
   it's the best part of the #RocketDogeStealthLaunch idea, increasing the liquidity pool automatically, 
...
contract RocketDogeStealthLaunch is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x266080e996098c113e68Bf076C2819Df4a90Ae1f); // Marketing Address
    ...
    string private _name = "RocketDogeStealthLaunch";
    string private _symbol = "RocketDogeStealthLaunch";
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()) {
            require(amount <= _maxTxAmount, "Transfer amount exxceeds the maxTxAmount.");
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        // make the swap
        router.swapExactTokensForKCSSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function swapETHForTokens(uint256 amount) private {
      ...
      // make the swap
        router.swapExactKCSForTokensSupportingFeeOnTransferTokens{value: amount}(
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        router.addLiquidityKCS{value: ethAmount}(
            address(this),
            tokenAmount,
