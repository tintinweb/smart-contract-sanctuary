...
contract NihaoBNB {
    ...
    uint256[7] public defaultPackages = [0.5 ether,1 ether, 2 ether, 3 ether, 5 ether];
    uint256 public PoolQualifyAmount = 5 ether;
  ...
  function _refPayout(address _addr, uint256 _amount) internal {
    ...
    function invest(address referrer) public payable {
		...
		require(msg.value >= INVEST_MIN_AMOUNT,'Min invesment 0.5 BNB');
  ...
  function getEligibleWithdrawal(address _user) public view returns(uint8 reivest, uint8 withdrwal){
      ...
      uint256 TotalDeposit = users[_user].amount;
      if(users[_user].refs[0] >=4 && (TotalDeposit >=2 ether && TotalDeposit < 3 ether)){
          reivest = 50;
          withdrwal = 50;
      }else if(users[_user].refs[0] >=2 && (TotalDeposit >=3 ether && TotalDeposit < 5 ether)){
          reivest = 40;
          withdrwal = 60;
      }else if(TotalDeposit >=5 ether){
         reivest = 30;
         withdrwal = 70;
      }else{
          reivest = 60;
