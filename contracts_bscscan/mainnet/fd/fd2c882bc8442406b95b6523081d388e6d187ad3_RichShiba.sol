...
RichShiba has built an empire and would like to give back to the people in the BSC Space.
RichShiba will be giving BUSD rewards to all who suck up to him by buying his tokens!
Come now and suck up to RichShiba and get BUSD rewards!

TG: RichShiba_Official
...
// File: contracts/RichShibaDividendPayingToken.sol
...
contract RichShiba is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RichShibaDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("RichShiba", "RichShiba") {
    	...
    	dividendTracker = new RichShibaDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "RichShiba: The dividend tracker already has that address");

        RichShibaDividendTracker newDividendTracker = RichShibaDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "RichShiba: The new dividend tracker must be owned by the RichShiba token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "RichShiba: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "RichShiba: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "RichShiba: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "RichShiba: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "RichShiba: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "RichShiba: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "RichShiba: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "RichShiba: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of RichShiba
            path,
            recipient,
            block.timestamp
        );
...
contract RichShibaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("RichShiba_Dividend_Tracker", "RichShiba_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "RichShiba_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "RichShiba_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RichShiba contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "RichShiba_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "RichShiba_Dividend_Tracker: Cannot update claimWait to same value");
