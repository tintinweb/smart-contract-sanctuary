...
contract WorldWideWealth is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingWalletAddress = payable(0x6846B5fa5040f9B8C9e7ac64Ab166424D62C8Cb9); // Marketing Address
    address payable public charityWalletAddress = payable(0x2dD8B0801439DC57d0cF2d0EE3865830029fb445); // Charity Address
    ...
    string private _name = "World Wide Wealth";
    string private _symbol = "WWW";
    ...
    constructor () {
        ...
        _totalLiqFee = _liquidityFee.add(_marketingFee).add(_charityFee);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _marketingFee.add(_charityFee) > 0)
        {
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(forWallets);
            uint256 newBalance = address(this).balance.sub(initialBalance);
    
            uint256 marketingShare = newBalance.div(_marketingFee.add(_charityFee)).mul(_marketingFee);
            ...
            if(techShare > 0)
                transferToAddressETH(charityWalletAddress, techShare);
        }
    }
