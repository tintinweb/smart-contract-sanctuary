...
ZoonUP was created because of our love for ZOON and we saw how it flew to the moon.
By Holding ZoonUP, you will recieve $ZOON as rewards. 
Buys and sells will contribute to your rewards.
...
// File: contracts/ZoonUPDividendPayingToken.sol
...
contract ZoonUP is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ZoonUPDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("ZoonUP", "ZoonUP") {
    	...
    	dividendTracker = new ZoonUPDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ZoonUP: The dividend tracker already has that address");

        ZoonUPDividendTracker newDividendTracker = ZoonUPDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ZoonUP: The new dividend tracker must be owned by the ZoonUP token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ZoonUP: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ZoonUP: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ZoonUP: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ZoonUP: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ZoonUP: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ZoonUP: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ZoonUP: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "ZoonUP: This account cannot send tokens until trading is enabled");
        }
...
contract ZoonUPDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("ZoonUP_Dividend_Tracker", "ZoonUP_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "ZoonUP_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "ZoonUP_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ZoonUP contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ZoonUP_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ZoonUP_Dividend_Tracker: Cannot update claimWait to same value");
