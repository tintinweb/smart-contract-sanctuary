 ...
 * DAPPERCATSCRYPTO.COM
 *      Notes:
 *		- The World's Top Deflationary Crypto-Purrency!
 *		- True deflation with burn on every transaction, less tokens = more value 
 *      - The longer you hold the more you earn and save cats!
 *      - Become a liquidity provider and a part of the community!
 *      - This contract is based on DapperCats v1: 0xDBA9B1d91E710b4ac9A1Df3a7b3f632f4297f0a3 (degraded)
 *      - Old contract was flawed so needed to update to this version and add functionality
 * 
 * @DAPPERCATSCRYPTO
 * @DAPPERCATSCRYPTO
 * @DAPPERCATSCRYPTO
...
contract _DapperCats is Context, IERC20, Ownable 
{
    ...
    // Owner cannot transfer out $DAPCAT from this smart contract
    function transferAnyBEP20Tokens(address _tokenAddr, address _to, uint _amount) public onlyOwner {
        require(_tokenAddr != address(this), "Cannot transfer out $DAPCAT!");
    ...
    function enableAllFeesIncludingSwapAndLiquify() external onlyOwner() 
    {
        _taxFee = 3;
	...
	function updateFee(uint256 _tax_Fee,uint256 _burn_Fee, uint256 _liquidity_Fee, uint256 _charity_Fee) public onlyOwner()
	{
    _taxFee = _tax_Fee; 
	_liquidityFee = _liquidity_Fee;
    _burnFee = _burn_Fee;
	_charityFee = _charity_Fee;
	}
