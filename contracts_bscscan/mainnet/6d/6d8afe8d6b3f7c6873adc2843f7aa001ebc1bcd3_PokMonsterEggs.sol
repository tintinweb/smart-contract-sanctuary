    ...
    ReentrancyGuard
{
    ...
    uint256 public _lastindexBody;
    uint256 public _lastNumber;
    /* ========== EVENTS ========== */
    ...
    function eggslength() external view returns (uint256) {
        return eggs.length;
    }
    ...
    function openEggs(uint256 _rand) external nonReentrant {
        require(eggs.length > 0, "!");
        uint256 length = eggs.length;
        nonce = nonce + 1;
        currentSeed = _genNextSeed((_rand + currentSeed) ^ nonce);
        ...
        uint256 _indexBody = _evolutionId % length;
        _lastNumber = _evolutionId;
        _lastindexBody = _indexBody;
        if (_indexBody > length) _indexBody = eggs.length - 1;
        IPokMonsterStats.PokBody memory _body = staticBody_[eggs[_indexBody]];

        uint256 _randNumber = _evolutionId % 10000;

        bool exits_ = BRC721(nft_).exists(_evolutionId);
        if (exits_) {
            nonce = nonce + 1;
            currentSeed = _genNextSeed(currentSeed ^ (nonce + _rand));
            _evolutionId = getRandomNumber(currentSeed, 99999, 999999999999999);
        }
        IERC20(pok_).safeTransferFrom(msg.sender, address(this), fee_);
        bool result = _randNumber < _rate;
        if (result) {
            _body.Productivity = uint16(getRandomNumber(currentSeed, 2, 7));
            _body.Luck = uint16(_evolutionId % 40);
