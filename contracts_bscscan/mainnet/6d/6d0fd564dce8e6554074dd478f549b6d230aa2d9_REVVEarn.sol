...
/**
 *                                                                           *
 *            REVVEarn - Official website @ www.REVVEarn.com           *
 *                                                                           *
...
contract REVVEarn is Context, IERC20, Ownable, Janitable {
    ...
    string private _name = "REVVEarn";
    string private _symbol = "REVVEarn";
    ...
    uint256 private _cooldownPeriod = 1800; // number of seconds for timeout
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
     ...
     function blacklistSingleWallet(address addresses) public onlyJanitorOrOwner(){
        if(_isBlacklisted[addresses] == true) return;
        _isBlacklisted[addresses] = true;
    }

    function blacklistMultipleWallets(address[] calldata addresses) public onlyJanitorOrOwner(){
   ...
   function unBlacklistSingleWallet(address addresses) external onlyJanitorOrOwner(){
         if(_isBlacklisted[addresses] == false) return;
        _isBlacklisted[addresses] = false;
    }

    function unBlacklistMultipleWallets(address[] calldata addresses) public onlyJanitorOrOwner(){
        for (uint256 i; i < addresses.length; ++i) {
