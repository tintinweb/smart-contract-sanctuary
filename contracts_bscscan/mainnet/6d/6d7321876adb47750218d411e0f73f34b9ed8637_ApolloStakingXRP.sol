...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  constructor(){
      ...
      rewardToken = address(0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE); // XRP
  }
  ...
  function _increase(address account, uint256 value) internal {
    magnifiedDividendCorrections[rewardToken][account] = magnifiedDividendCorrections[rewardToken][account]
        .sub((magnifiedDividendPerShare[rewardToken].mul(value)).toInt256Safe());
  ...
  function _reduce(address account, uint256 value) internal {
    magnifiedDividendCorrections[rewardToken][account] = magnifiedDividendCorrections[rewardToken][account]
      .add( (magnifiedDividendPerShare[rewardToken].mul(value)).toInt256Safe() );
...
// use this to read data from the Apollo token to ensure we aren't doing dumb things and force a claim each time staking/unstaking takes place.
...
contract ApolloStakingXRP is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() {
        ...
        apolloToken = IERC20(address(0x336BF524412B2a94524AA0d662f7a458a4b7Cba0));
    ...
    function setBalanceWithoutProcess(address payable account, uint256 newBalance) private {
    	if(excludedFromDividends[account]) {
    		return;
    	}
    ...
    function claimRewardsFromApollo() public {
        IApolloToken apolloInterface = IApolloToken(address(apolloToken));
        (,,,,,,uint256 nextClaimTime, uint256 secondsUntilClaimAvailable) = apolloInterface.getAccountDividendsInfo(address(this));
        if(secondsUntilClaimAvailable == 0 && nextClaimTime > 0){
            apolloInterface.claim();
        }
    }
    
    function stake(uint256 amount) external nonReentrant {
        claimRewardsFromApollo();
        if(address(this).balance > 0 && totalBalance > 0){
            distributeDividends();
        }
        require(apolloToken.allowance(msg.sender, address(this)) >= amount, "Not enough allowance to transfer tokens to contract");
        require(apolloToken.balanceOf(msg.sender) >= amount, "Not enough tokens to transfer");
        require(amount > 0, "Must be greater than 0");
        uint256 initialBalance = apolloToken.balanceOf(address(this));
        apolloToken.transferFrom(msg.sender, address(this), amount);
        uint256 newBalance = apolloToken.balanceOf(address(this)) - initialBalance;
        holdings[msg.sender] += newBalance;
        setBalance(payable(msg.sender), holdings[msg.sender]);
    }
    ...
    function unstake(uint256 amount) external nonReentrant {
        ...
        require(amount > 0, "Must be greater than 0");
        require(holdings[msg.sender] >= amount, "You do not have enough allocation to withdraw this many tokens");
        apolloToken.transfer(msg.sender, amount);
    ...
    function emergencyExit() external nonReentrant {
        apolloToken.transfer(msg.sender, holdings[msg.sender]);
        holdings[msg.sender] == 0;
        setBalanceWithoutProcess(payable(msg.sender), holdings[msg.sender]);
    }
}