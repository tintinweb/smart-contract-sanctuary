...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public DividendsToken = address(0x609D183Fb91a0fce59550b62ab7d2c931b0Bb1BE);
...
contract BabyPolkaMonster is ERC20, Ownable {
   ...
   BabyPolkaMonsterDividendTracker public dividendTracker;
	...
	uint256 private contractTokenBalanceAmount;
    ...
    constructor() ERC20 ("BabyPolkaMonster", "BabyPkmon") {
        _RewardsFee = 10;
        _liquidityFee = 3;
		_marketingFee = 5;
		_devFee = 1;
        ...
        maxBuyTranscationAmount = 2000000001 * (10**18);
        maxSellTransactionAmount = 500000000 * (10**18);
        swapTokensAtAmount = 250000000 * (10**18);
        _maxWalletToken = 2000000001 * (10**18); // 2% of total supply

        totalFees = _RewardsFee.add(_liquidityFee).add(_marketingFee).add(_devFee);
		...
		marketingWallet = payable(0xD0A5b842Ca9078d71E6D89BD6627101f90FEB963);
		devWallet = payable(0x4a8198C107935693b499c4d11b6E5feaB511f145);
	...
	function updateContractTokenAmount(uint256 amount) public onlyOwner {
	    contractTokenBalanceAmount = amount;
	}
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "BabyPolkaMonster: The router already has that address");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "BabyPolkaMonster: Account is already the value of 'blacklisted'");
    ...
    function excludeFromMaxSell(address account, bool excluded) public onlyOwner {
        require(_isExcludedMaxSellTransactionAmount[account] != excluded, "BabyPolkaMonster: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyPolkaMonster: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyPolkaMonster: The liquidity wallet is already this address");
    ...
    function _transfer(
		...
		if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != liquidityWallet && to != liquidityWallet && from != marketingWallet && to != marketingWallet && from != devWallet && to != devWallet) {
		    
		    contractTokenBalance = contractTokenBalanceAmount;
            
            swapping = true;

            uint256 swapTokens = contractTokenBalance.mul(_marketingFee).div(100);
            ...
            swapTokens = contractTokenBalance.mul(_devFee).div(100);
            swapTokensForBNB(swapTokens);
            transferToWallet(payable(devWallet), address(this).balance);
            
            contractTokenBalance = contractTokenBalance.sub(swapTokens);


            swapTokens = contractTokenBalance.mul(_liquidityFee).div(100);
    ...
    function swapTokensForRewards(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of BabyPolkaMonster
            path,
            recipient,
            block.timestamp
        );
    ...
    function restoreAllFee() external onlyOwner() {
        ...
        _maxWalletToken = 2000000001 * 10**18;
        maxBuyTranscationAmount = 2000000001 * 10**18;
        maxSellTransactionAmount = 500000000 * 10**18;
...
contract BabyPolkaMonsterDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BabyPolkaMonster_Dividend_Tracker", "BabyPolkaMonster_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "BabyPolkaMonster_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
