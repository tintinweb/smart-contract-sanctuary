...
contract PancakeHunter is Ownable {
    ...
    function swapTwice(uint256 wbnbamount, uint256 amountMin, address[] calldata path, uint256 deadline, IBEP20 token) public onlyOwner {
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(wbnbamount, amountMin, path, address(this), deadline);
        uint256 afterBalance = token.balanceOf(address(this));
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(afterBalance, amountMin, path, address(this), deadline);
    }
    ...
    function getTokenBuyAmount(IBEP20 token)  public view onlyOwner returns(uint256) {
