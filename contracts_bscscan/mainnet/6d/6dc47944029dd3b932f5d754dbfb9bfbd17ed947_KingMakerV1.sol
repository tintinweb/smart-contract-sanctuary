...
contract KingMakerV1 is Ownable {
    ...
    uint256 public farmerCost = 10;
    uint256 public knightCost = 1000;
    uint256 public nobleCost = 100000;
    uint256 public kingCost = 10000000;
    
    uint256 public minimumScoreForLeaderboard;
    ...
    function getScore(address holder) public view returns (uint256){
        return (getPeasants(holder) + (getFarmers(holder) * 1e2) + (getKnights(holder) * 1e4) + (getNobles(holder) * 1e9) + (getKings(holder) * 1e16));
    }
    ...
    function getPeasants(address holder) public view returns (uint256){
        ...
        uint256 amountFromTime = block.timestamp - getLastUpdateTime(holder);
        uint256 amountFromFarmers = (block.timestamp - getLastUpdateTime(holder)) * getFarmers(holder);
        return peasants[holder] + (((amountFromTime + amountFromFarmers)*getMultiplier(holder))/1e18);
    }
    
    function getFarmers(address holder) public view returns (uint256){
        uint256 amountFromKnights = (block.timestamp - getLastUpdateTime(holder)) * getKnights(holder);
        return farmers[holder] + amountFromKnights;
    }
    
    function getKnights(address holder) public view returns (uint256){
        uint256 amountFromNobles = (block.timestamp - getLastUpdateTime(holder)) * getNobles(holder);
        return knights[holder] + amountFromNobles;
    }
    
    function getNobles(address holder) public view returns (uint256){
        uint256 amountFromKings = (block.timestamp - getLastUpdateTime(holder)) * getKings(holder);
        return nobles[holder] + amountFromKings;
    }
    ...
    function buyMaxFarmers() external {
    ...
    function buyMaxKnights() external {
    ...
    function buyMaxNobles() external {
    ...
    function buyMaxKings() external {
        buyKings(getNobles(msg.sender) / kingCost);
    }
    
    function leaderboardLeaders(uint256 position) public view returns (address, uint256){
        require(position <=3, "Leaderboard only tracks first 3 positions in this version");
        address wallet = leaderboard[position];
        return (wallet, highScore[wallet]);
    }
    
    function leaderboardUpdate(address holder) internal {
        uint256 firstAmount;
        address firstHolder;
        (firstHolder, firstAmount) = leaderboardLeaders(1);
        uint256 secondAmount;
        address secondHolder;
        (secondHolder, secondAmount) = leaderboardLeaders(2);
        uint256 thirdAmount;
        address thirdHolder;
        (thirdHolder, thirdAmount) = leaderboardLeaders(3);
        
        if(highScore[holder] > firstAmount){
            if(firstHolder != holder){
                leaderboard[3] = leaderboard[2];
                leaderboard[2] = leaderboard[1];
                leaderboard[1] = holder;
            }
        } else if(highScore[holder] > secondAmount) {
            (firstHolder, firstAmount) = leaderboardLeaders(1);
            (secondHolder, secondAmount) = leaderboardLeaders(2);
            if(secondHolder != holder && firstHolder != holder){
                leaderboard[3] = leaderboard[2];
                leaderboard[2] = holder;
            }
        } else if(highScore[holder] > thirdAmount) {
            (firstHolder, firstAmount) = leaderboardLeaders(1);
            (secondHolder, secondAmount) = leaderboardLeaders(2);
            (thirdHolder, thirdAmount) = leaderboardLeaders(3);
            if(thirdHolder != holder && firstHolder != holder && secondHolder != holder){
                leaderboard[3] = holder;
            }
        }
        
        // update 3rd place to prevent updating when there is no update to top 3 (May replace this function entirely with an off-chain solution in future iterations)
        (, thirdAmount) = leaderboardLeaders(3);
        minimumScoreForLeaderboard = thirdAmount;
    }
    
    // clean up leaderboard if there is an issue but only for emergencies.
    function cleanLeaderboard() external onlyOwner {
        leaderboard[1] = address(0);
        leaderboard[2] = address(0);
        leaderboard[3] = address(0);
    }
    ...
    function updateScore(address holder) private {
        if(getScore(holder) > highScore[holder]){ // should be based on high score not current score
            ...
            // only update leaderboard if score qualifies
            if(minimumScoreForLeaderboard < highScore[holder]){
                leaderboardUpdate(holder);
            }
        }
    }
