...
contract RandomNumberGenerator is VRFConsumerBase, IRandomNumberGenerator, Ownable {
    ...
    /**
     * @notice Constructor
     * @dev RandomNumberGenerator must be deployed before the Tsuki Reward.
     * Once the Tsuki Reward contract is deployed, set Tsuki Reward must be called.
    ...
    function getRandomNumber(uint256 _seed) external override {
        require(msg.sender == TsukiReward || msg.sender == owner(), "Only TsukiReward");
    ...
    /**
     * @notice Set the address for the TsukiReward
     * @param _TsukiReward: address of the PancakeSwap lottery
     */
    function setTsukiRewardAddress(address _TsukiReward) external onlyOwner {
        TsukiReward = _TsukiReward;
    }
