...
interface Fee {
    function feeDistribution(uint256 amount, uint256 marketingFee, uint256 devFee, uint256 liquidityFee, uint256 buyBackFee, bool inBNB, address marketing_, address dev_) external;
}
...
contract MetaDog is Context, IERC20, Ownable {
    ...
    // Init vault
    constructor (address lpAddress, address router, address stablecoin) public  {
    ...
    function rescueLockContractBNB() external {
        require(firstOwner() == _msgSender(), "Caller does not have power");
    ...
    function calculateLiquidityPlusFees(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _devFee + _marketingFee + _buyBackFee).div(
            10**4
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee == 0 && _devFee == 0 && _buyBackFee == 0) return;
    ...
    function _transfer(
        ...
        if (
            ...
            Fee(feeManager).feeDistribution(contractTokenBalance, _marketingFee, _devFee, _liquidityFee, _buyBackFee, swapAndLiquifyInBNB, _marketingAddress, _devAddress);
        }
