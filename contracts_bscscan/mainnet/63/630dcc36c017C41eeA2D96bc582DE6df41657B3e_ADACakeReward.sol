...
contract ADACakeReward is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxSellTransactionAmount = 250000000 * (10**18); //0.25%
    uint256 public maxWalletBalance = 1000000000 * (10**18); //1%
    uint256 public swapTokensAtAmount = 2500000 * (10**18);

    uint256 public liquidityFee = 5;
    uint256 public previousLiquidityFee;
    uint256 public cakeDividendRewardsFee = 6;
    uint256 public previousCakeDividendRewardsFee;
    uint256 public adaDividendRewardsFee = 6;
    uint256 public previousAdaDividendRewardsFee;
    uint256 public marketingFee = 3;
    uint256 public previousMarketingFee;
    uint256 public totalFees = cakeDividendRewardsFee.add(marketingFee).add(adaDividendRewardsFee).add(liquidityFee);
    ...
    constructor() ERC20("RocketBomb", "RocketBomb") {
    	cakeDividendTracker = new CAKEDividendTracker();
    	adaDividendTracker = new ADADividendTracker();
 
    	marketingWallet = 0x0D22929bFeF8EDEe14C368BB4F75E56bbEdd077b;
    ...
    function setCakeDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = cakeDividendRewardsFee.add(marketingFee).add(adaDividendRewardsFee).add(liquidityFee);
            cakeDividendEnabled = _enabled;
        }
    ...
    function setAdaDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = adaDividendRewardsFee.add(marketingFee).add(cakeDividendRewardsFee).add(liquidityFee);
            adaDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(adaDividendRewardsFee).add(cakeDividendRewardsFee).add(liquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updatecakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "RocketBomb: The dividend tracker already has that address");
 
        CAKEDividendTracker newcakeDividendTracker = CAKEDividendTracker(payable(newAddress));
 
        require(newcakeDividendTracker.owner() == address(this), "RocketBomb: The new dividend tracker must be owned by the RocketBomb token contract");
    ...
    function updateadaDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(adaDividendTracker), "RocketBomb: The dividend tracker already has that address");
 
        ADADividendTracker newadaDividendTracker = ADADividendTracker(payable(newAddress));
 
        require(newadaDividendTracker.owner() == address(this), "RocketBomb: The new dividend tracker must be owned by the RocketBomb token contract");
    ...
    function updateAdaDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = adaDividendRewardsFee.add(cakeDividendRewardsFee).add(marketingFee).add(liquidityFee);
    }
 
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        marketingFee = newFee;
        totalFees = marketingFee.add(cakeDividendRewardsFee).add(adaDividendRewardsFee).add(liquidityFee);
    }
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "RocketBomb: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "RocketBomb: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "RocketBomb: The PanadaSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "RocketBomb: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "RocketBomb: Cannot update gasForProcessing to same value");
	...
	function processDividendTracker(uint256 gas) external onlyOwner {
		(uint256 uniIterations, uint256 uniClaims, uint256 uniLastProcessedIndex) = cakeDividendTracker.process(gas);
		emit ProcessedcakeDividendTracker(uniIterations, uniClaims, uniLastProcessedIndex, false, gas, tx.origin);
 
		(uint256 maticIterations, uint256 maticClaims, uint256 maticLastProcessedIndex) = adaDividendTracker.process(gas);
		emit ProcessedadaDividendTracker(maticIterations, maticClaims, maticLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "RocketBomb: Trading has not started yet");
        ...
        if (!swapping && canSwap && from != uniswapV2Pair) {
            ...
            if (cakeDividendEnabled) {
                uint256 uniTokens = contractTokenBalance.div(totalFees).mul(cakeDividendRewardsFee);
                swapAndSendCakeDividends(uniTokens);
            }
 
            if (adaDividendEnabled) {
                uint256 maticTokens = contractTokenBalance.div(totalFees).mul(adaDividendRewardsFee);
                swapAndSendAdaDividends(maticTokens);
            }
...
contract CAKEDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("RocketBomb_Cake_Dividend_Tracker", "RocketBomb_Cake_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "RocketBomb_Cake_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "RocketBomb_Cake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RocketBomb contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "RocketBomb_Cake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "RocketBomb_Cake_Dividend_Tracker: Cannot update claimWait to same value");
...
contract ADADividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("RocketBomb_Ada_Dividend_Tracker", "DualRewards_Ada_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "RocketBomb_Ada_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "RocketBomb_Ada_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RocketBomb contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "RocketBomb_Cake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "RocketBomb_Ada_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
