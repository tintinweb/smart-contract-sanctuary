...
// File contracts/MainSquidGame.sol
...
//
    ...
    function getGameInfo(uint _index) public view returns(RewardToken[] memory tokenRewards, uint minSeAmount, uint minStakeAmount, uint chanceToWin, string memory name, bool enable){
        require(_index < games.length, "Index out of bound");
        Game memory game = games[_index];
        RewardToken[] memory _tokensRewards = new RewardToken[](games[_index].rewardTokens.length);
        tokenRewards = _tokensRewards;
        minSeAmount = game.minSeAmount;
        minStakeAmount = game.minStakeAmount;
        chanceToWin = game.chanceToWin;
        name = game.name;
        enable = game.enable;
    }
