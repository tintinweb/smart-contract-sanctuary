...
contract Mongas2 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = payable(0x06b57999655752343D2fb9c56EAe672dFACc8F79); // marketing Address
    address payable public charityAddress = payable(0xf1D0D160c047ff1eA5052D232e78D806e5031d8c); // Charity Address
    address payable public prizeAddress = payable(0x56f6AeAF6405f644b7b380eFD5267B060eFd1FED); // prize Address
    address payable public buyBackAddress = payable(0x56f6AeAF6405f644b7b380eFD5267B060eFd1FED); // prize Address

    address payable public liquidityAddress = payable(0x6Bb796B0Ff92F91eF9E5ff37bEEEe43625F3d197); // Liquidity Address
    ...
    string private constant _name = "Mongas2";
    string private constant _symbol = "Mongas2";
    ...
    uint256 public _sellPrizeFee = 5;
    ...
    function setMaxTransactionAmount(uint256 amount) external onlyOwner {
        require(amount >= _tTotal * 5 / 100000);
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyPrizeFee + _buyCharityFee;
                if(_liquidityFee > 0){
                    buyOrSellSwitch = BUY;
                }
            } 
            // Sell
                ...
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee  + _sellPrizeFee + _sellCharityFee;
                if(_liquidityFee > 0){
    ...
    function swapBack() private lockTheSwap {
        ...
        bool success;
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _marketingTokensToSwap + _charityTokensToSwap + _prizeTokensToSwap;
        if(totalTokensToSwap == 0 || contractBalance == 0) {return;}
        
        uint256 totalTaxFee = _sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellCharityFee + _sellPrizeFee;

        
        // Halve the amount of liquidity tokens
        uint256 tokensForLiquidity = (minimumTokensBeforeSwap * _sellLiquidityFee / totalTaxFee) / 2;
        uint256 tokensForMarketing = (minimumTokensBeforeSwap * _sellMarketingFee / totalTaxFee);
        uint256 tokensForCharity = (minimumTokensBeforeSwap * _sellCharityFee / totalTaxFee);
        uint256 tokensForPrize = (minimumTokensBeforeSwap * _sellPrizeFee / totalTaxFee);

        uint256 amountToSwapForBNB = minimumTokensBeforeSwap.sub(tokensForLiquidity);
        ...
        uint256 bnbForMarketing = bnbBalance.mul(tokensForMarketing).div(minimumTokensBeforeSwap);
        
        uint256 bnbForCharity = bnbBalance.mul(tokensForCharity).div(minimumTokensBeforeSwap);
        uint256 bnbForPrize = bnbBalance.mul(tokensForPrize).div(minimumTokensBeforeSwap);
        
        uint256 bnbForLiquidity = bnbBalance - bnbForMarketing - bnbForCharity - bnbForPrize;
        
        _liquidityTokensToSwap -= tokensForLiquidity*2;
        _marketingTokensToSwap -= tokensForMarketing;
        _charityTokensToSwap -= tokensForCharity;
        _prizeTokensToSwap -= tokensForPrize;
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyCharityFee, uint256 buyPrizeFee)
        external
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyCharityFee + _buyPrizeFee <= 30, "Must keep taxes below 20%");
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellCharityFee, uint256 sellPrizeFee)
        external
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellCharityFee + _sellPrizeFee <= 40, "Must keep taxes below 30%");
    }
