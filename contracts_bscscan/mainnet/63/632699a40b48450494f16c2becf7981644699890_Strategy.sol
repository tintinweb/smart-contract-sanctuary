...
contract Strategy{
    ...
    function depositWithBnb(uint _amount)public{
        ...
        LiquidityMiningMaster(pool).deposit(0, IERC20(xms).balanceOf(address(this)));
    }
    
    function depositWithXms(uint _amount)external{
        LiquidityMiningMaster(pool).deposit(0, _amount);
    }
    ...
    function harvest() external{
        ...
        uint TotalReward = IERC20(wbnb).balanceOf(address(this));
    ...
    function harvestDirect() public{
        LiquidityMiningMaster(pool).deposit(0, 0);
    }
    
    function convertToWBNB() public {
        WBNBContract(wbnb).deposit();
    }
    
    function rewardStrategist() public{
        uint TotalReward = IERC20(wbnb).balanceOf(address(this));
        IERC20(wbnb).safeTransfer(strategist, TotalReward.mul(strategistReward).div(100));
    }
    
    function ConvertToNewB() public{
        uint TotalReward = IERC20(wbnb).balanceOf(address(this));
        address[] memory path = new address[](2); 
                path[0] = out;
                path[1] = busd;
