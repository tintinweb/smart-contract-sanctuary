...
contract KAFATOPU is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1 * 10**9 * 10**9; //1 Billion Supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "KAFATOPU";
    string private _symbol = "$KAFA";
    ...
    address payable public _marketingWallet = payable(address(0xB37eC6E84039D34322a80360c67d7Fd2df415079)); //Replace your Marketing Wallet Address Here

    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;

    uint256 public _maxTxAmount = 1 * 10**7 * 10**9; // 1% Of the supply
    uint256 private numTokensSellToGetBnb = 2 * 10**4 * 10**9; //20000 Tokens
...
require (marketingFee <= 15, "max marketing fee limit is 15 percent");
  ...
  function setMarketingWallet(address payable marketingWallet) external onlyOwner{
    ...
    function _transfer(
        ...
        require(amount > 0, "ERC: Transfer amount must be greater than zero");
        ...
        bool overMinTokenBalance = contractTokenBalance >= numTokensSellToGetBnb;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            from != uniswapV2Pair &&
