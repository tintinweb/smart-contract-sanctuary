...
contract Strategy_LATTEv2_BUSD is Ownable {
    ...
    function _swapLPtoWBNB(uint256 _percent) internal {
        ...
        if (_totalLP > 0) {
            ...
            // swap token0 + rewards and token1 to WBNB
            _swapTokenToWBNB(routerLP, token0);
    ...
    function _getToken0Price() internal view returns (uint256 result) {
        ...
        path[0] = token1;
        path[1] = token0;
        result = 1e36 / (_getAmountsOut(routerLP, 100 * 1e18, path) / 100);
    }
    ...
    function _getPriceWBNBToToken1() internal view returns (uint256) {
        ...
        path[0] = WBNB;
        path[1] = token1;
        return 1e36 / (_getAmountsOut(routerMain, 100 * 1e18, path) / 100);
    }
