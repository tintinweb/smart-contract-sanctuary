...
contract BnbTradebot {
    ...
    mapping(address => Withdrawn) public withdrawn;
    ...
    function takeProfit() payable external {
        ...
        uint256 _max_payout = this.maxPayoutOf(users[msg.sender].total_deposits);
        
        if((users[msg.sender].total_payouts + payout + users[msg.sender].withdrawable) > _max_payout) {
            payout = _max_payout - users[msg.sender].total_payouts;
            ...
            for(uint8 i = 0; i < bots[msg.sender].length; i++) {
                ...
                bot_stats[bot_id].collected_rewards = max_payout;
                ...
                emit BotMaxPayoutReached(msg.sender, bot_id, max_payout);
            }
        }
    ...
    function withdraw() external {
        ...
        // check top earners
        if(!withdrawn[msg.sender].withdrawn){
            top_earners.push(msg.sender);
            withdrawn[msg.sender].withdrawn = true;
        }
    ...
    function getTotalEarnersCount() public view returns (uint256) { return top_earners.length; }
