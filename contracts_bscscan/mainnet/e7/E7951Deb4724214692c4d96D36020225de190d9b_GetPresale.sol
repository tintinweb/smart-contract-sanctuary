...
contract GetPresale {
    ...
    constructor(address _owner, address tokenAddress) public {
        yourToken = GET(tokenAddress);
        owner = _owner;   
    }
    ...
    function buyToken() public payable returns(uint256) {
        ...
        _pendingWithdrawals[owner] += bnbAmount;
        
        uint256 amountToBuy = bnbAmount * tokensPerBnb;
        uint256 tokensCount =  amountToBuy.mul(1000000000).div(1000000000000000000);

        (bool sent) = yourToken.transfer(msg.sender, tokensCount);
