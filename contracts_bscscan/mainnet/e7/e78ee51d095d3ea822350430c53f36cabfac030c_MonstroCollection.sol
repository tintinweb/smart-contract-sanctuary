...
// Dependency file: contracts/PresaleWhitelist.sol


// pragma solidity >=0.8.0;

abstract contract PresaleWhitelist is Ownable {

    address public accessToken;

    constructor(address _token) {
        require(_token != address(0), "Zero token set");

        accessToken = _token;
    }

    function isInWhitelist(address _who) public view returns (bool) {
        uint balance;
        for (uint i; i < 5; i++) {
            balance = IERC1155(accessToken).balanceOf(
                _who,
                i + 1
            );
...
// Root file: contracts/MonstroCollection.sol


pragma solidity >=0.8.0;

/**
 * @title MonstroCollection
 */
contract MonstroCollection is ERC721Enumerable, ReentrancyGuard, Ownable, PresaleWhitelist {
    ...
    event RolledOver(Status status);

    constructor(address _bank, address _accessToken)
        ERC721("Monstro Fun Collection", "MONSTRO")
        PresaleWhitelist(_accessToken)
    {
        require(_bank != address(0), "Zero token set");
    ...
    receive() external payable nonReentrant {
        ...
        require(
            saleIsActive != Status.DISABLED,
            "Collection: sale is not active"
        );

        if (saleIsActive == Status.PRESALE) {
            require(isInWhitelist(_msgSender()), "Whitelist: it's not");
        }
    ...
    function mintTokens(uint numberOfTokens) external payable nonReentrant {
        require(
            saleIsActive != Status.DISABLED,
            "Collection: sale is not active"
        );
    ...
    function premint(uint256 _amount) external onlyOwner {
        require(preMinted + _amount <= 26, "Too much pre minted tokens");
