...
/**
 ...
 Earn BTC (bep20) while holding Only1BTC
 Total Supply : 1 BTC
 
 Telegram : https://t.me/only1btc
...
contract Only1BTC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    Only1BTCDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 public maxBuyTranscationAmount = 0.03 * (10**18);
    uint256 public maxSellTransactionAmount = 0.01 * (10**18);
    uint256 public swapTokensAtAmount = 0.002 * (10**18);
    uint256 public maxWalletToken = 0.05 * (10**18); // 5% of total supply
    ...
    constructor() ERC20("Only1BTC", "O1B") {
    	...
    	dividendTracker = new Only1BTCDividendTracker();

    	buyBackWallet = 0x86C57cea299d1b3C7dF318D3b1B6E21820B1dFEB;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "Only1BTC: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "Only1BTC: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function afterPreSale() external onlyOwner {
        ...
        marketingFee = 5;
        maxBuyTranscationAmount = 0.03 * (10**18);
        maxWalletToken = 0.05 * (10**18);
    }
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "Only1BTC: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Only1BTC: The dividend tracker already has that address");

        Only1BTCDividendTracker newDividendTracker = Only1BTCDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Only1BTC: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "Only1BTC: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Only1BTC: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Only1BTC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Only1BTC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Only1BTC: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "Only1BTC: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Only1BTC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Only1BTC: Cannot update gasForProcessing to same value");
...
contract Only1BTCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Only1BTC_Dividend_Tracker", "Only1BTC_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 0.0001 * (10**18); //must hold 0.0001+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "Only1BTC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "Only1BTC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Only1BTC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Only1BTC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Only1BTC_Dividend_Tracker: Cannot update claimWait to same value");
