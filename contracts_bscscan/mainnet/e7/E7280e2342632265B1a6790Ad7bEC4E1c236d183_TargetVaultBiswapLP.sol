...
import "../../interfaces/targetVaults/protocols/IPancakeMainStaking.sol";
import "../TargetVault.sol";

contract TargetVaultBiswapLP is TargetVault {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    /**
     * @dev Pancake Staking address
     */
    IPancakeMainStaking public pancakeStaking;
    ...
    /**
     * @dev Reward to Token 0 Path
     */
    address[] public rewardToToken0Path;

    /**
     * @dev Reward to Token 1 Path
     */
    address[] public rewardToToken1Path;

    /**
     * @dev Emitted when RewardToken is harvested and compounded
     */
    ...
    constructor(
        ...
        pancakeStaking = IPancakeMainStaking(_pancakeStaking);
        pid = _pid;
    ...
    function _deposit() internal virtual {
        ...
        if (_balance > 0) {
            ...
            token.safeIncreaseAllowance(address(pancakeStaking), _balance);
            pancakeStaking.deposit(pid, _balance);
    ...
    function withdraw(uint256 _amount) external virtual onlyVault {
        if (_amount > 0) {
            ...
            pancakeStaking.withdraw(pid, _amount);
            uint256 _after = token.balanceOf(address(this));
            uint256 _balance = _after.sub(_before);
            token.safeTransfer(feedVault, _balance);
    ...
    function _collectFees() internal virtual {
        ...
        if (_fees > 0) {
            if (autoBuyBack) {
                IERC20 _buyBackToken = IERC20(tokenToBuyBackPath[tokenToBuyBackPath.length - 1]);
                uint256 buyBackBefore = _buyBackToken.balanceOf(address(this));
                _safeSwap(swapRouterAddress, _fees, tokenToBuyBackPath, address(this), block.timestamp.add(600));
                uint256 buyBackAfter = _buyBackToken.balanceOf(address(this));
                uint256 buyBackAmount = buyBackAfter.sub(buyBackBefore);
                _buyBackToken.safeTransfer(feesCollector, buyBackAmount);

                emit FeesCollected(address(feesCollector), address(_buyBackToken), _fees);
            } else {
    ...
    function harvest() external virtual {
        require(address(msg.sender) == harvester, "TargetVaultBiswapLP(harvest): sender is not harvester");
        _harvest();
    }
    ...
    function _harvest() internal virtual {
        pancakeStaking.deposit(pid, 0);
        uint256 swapAmt = IERC20(rewardToken).balanceOf(address(this));

        if (swapAmt > 0) {
            _collectFees();

            swapAmt = IERC20(rewardToken).balanceOf(address(this));
            IERC20 token0 = IERC20(rewardToToken0Path[rewardToToken0Path.length.sub(1)]);
            IERC20 token1 = IERC20(rewardToToken1Path[rewardToToken1Path.length.sub(1)]);

            // Swap Token0 and Token1
            if (rewardToToken0Path.length > 1)
                _safeSwap(swapRouterAddress, swapAmt.div(2), rewardToToken0Path, address(this), block.timestamp.add(600));
            if (rewardToToken1Path.length > 1)
                _safeSwap(swapRouterAddress, swapAmt.div(2), rewardToToken1Path, address(this), block.timestamp.add(600));

            // Add Liquidity
            uint256 token0Amount = token0.balanceOf(address(this));
            uint256 token1Amount = token1.balanceOf(address(this));
            if (token0Amount > 0 && token1Amount > 0) {
                token0.safeIncreaseAllowance(swapRouterAddress, token0Amount);
                token1.safeIncreaseAllowance(swapRouterAddress, token1Amount);

                IPancakeRouter02(swapRouterAddress).addLiquidity(
                    address(token0),
                    address(token1),
                    token0Amount,
                    token1Amount,
    ...
    /**
     * @dev Convert dust of token0 or token1
     */
    function convertDust(address[] memory _dustToTokenPath) external virtual onlyOwner {
        IERC20 _dustToken = IERC20(_dustToTokenPath[0]);
        IERC20 _rewardToken = IERC20(_dustToTokenPath[_dustToTokenPath.length.sub(1)]);
        IERC20 token0 = IERC20(rewardToToken0Path[rewardToToken0Path.length.sub(1)]);
        IERC20 token1 = IERC20(rewardToToken1Path[rewardToToken1Path.length.sub(1)]);
        require(
            address(_dustToken) == address(token0) || address(_dustToken) == address(token1),
            "TargetVaultBiswapLP(convertDust): token not allowed"
        );
        require(address(_rewardToken) == address(rewardToken), "TargetVaultBiswapLP(convertDust): reward token not allowed");
        uint256 _dustAmt = _dustToken.balanceOf(address(this));
        if (_dustAmt > 0) _safeSwap(swapRouterAddress, _dustAmt, _dustToTokenPath, address(this), block.timestamp.add(600));
    }
    ...
    function emergencyWithdrawAll() external virtual onlyOwner {
        uint256 _balance = vaultBalance();
        pancakeStaking.withdraw(pid, _balance);
    ...
    function vaultBalance() public view virtual returns (uint256) {
        (uint256 _amount, ) = pancakeStaking.userInfo(pid, address(this));
        return _amount;
    }
    ...
    function vaultToken() public view virtual returns (address) {
        (IERC20 _want, , , ) = pancakeStaking.poolInfo(pid);
    ...
    /**
     * @dev Set Reward to Token 0 Path
     */
    function setRewardToToken0Path(address[] memory _rewardToToken0Path) public virtual onlyOwner {
        rewardToToken0Path = _rewardToToken0Path;
    }

    /**
     * @dev Set Reward to Token 1 Path
     */
    function setRewardToToken1Path(address[] memory _rewardToToken1Path) public virtual onlyOwner {
        rewardToToken1Path = _rewardToToken1Path;
    }
     ...
     */
    function setBuyBackToken(address _buyBackToken) public override onlyOwner {
