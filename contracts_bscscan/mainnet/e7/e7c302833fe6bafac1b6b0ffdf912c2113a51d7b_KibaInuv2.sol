...
contract KIBADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("KIBA_Dividend_Tracker", "KIBA_Dividend_Tracker", 9) {
    	claimWait = 3600; // in seconds
        minimumTokenBalanceForDividends = 1000000 * (10**9); //must hold 1 000 000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "KIBA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "KIBA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main KIBA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "KIBA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "KIBA_Dividend_Tracker: Cannot update claimWait to same value");
...
contract KibaInuv2 is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    KIBADividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 public maxSellTransactionAmount = 1000000000000000000 * (10**9); //  20000000000 max 0.2% of total supply per transaction
    uint256 public swapTokensAtAmount = 30000000 * (10**9);// 30000000 treshold to swap tokens for rewards
    ...
    // sells have fees of 25% first 24h
    uint256 public sellFeeIncreaseFactor = 1000; 
    ...
    constructor() public BEP20("Kiba Inu v2", "KIBA", 9) {
        uint256 _BNBRewardsFee = 0;
    	...
    	dividendTracker = new KIBADividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "KIBA: The dividend tracker already has that address");

        KIBADividendTracker newDividendTracker = KIBADividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "KIBA: The new dividend tracker must be owned by the KIBA token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "KIBA: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "KIBA: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "KIBA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "KIBA: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "KIBA: The liquidity wallet is already this address");
        excludeFromFees(newLiquidityWallet, true);
        emit LiquidityWalletUpdated(newLiquidityWallet, liquidityWallet);
        liquidityWallet = newLiquidityWallet;
    }

    function excludeFromDividends(address wallet)  public onlyOwner{
         dividendTracker.excludeFromDividends(wallet);
    }

    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "KIBA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "KIBA: Cannot update gasForProcessing to same value");
