...
*MEXICAN 
*
* Web in English https://mexicantkn.com
* Web in Spanish https://mexicantoken.org
* Instagram; @MexicanTKN
* Twitter; @MexicanTkn
* Discord; discord.com/invite/pFcNvwegDF
* Linkedin; linkedin.com/in/mexican-token-a25b24228
* Telegram; t.me/mexicantoken
*
* Project to Help people with economic problems, people who is dieying trying to find another kind of life, and people with basic needs (food, house, etc ..)
* By Urtzi Sanchez
*
**/
...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x7CbBb1A45561bEF9039772Ae874D1BB2AB0EFE6C;
...
contract MEXICANTKN is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0x8A0FDFEFab8D02F0DB2a0bC0a48B508273059591;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 100000000000000000000000000000000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "MexIcan";
    string private _symbol = "MEXICAN";
    uint8 private _decimals = 9;
    uint256 public _worldHungerFee  = 4;
    uint256 private _previousTaxFee = _worldHungerFee ;
    uint256 public _mexicanWallFee  = 2;
    uint256 private _previousDevelopmentFee = _mexicanWallFee ;
    uint256 public _homelessFee  = 5;
    uint256 private _previousLiquidityFee = _homelessFee ;
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_worldHungerFee ).div(
            10**3
        );
    }
    function calculateDevelopmentFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_mexicanWallFee ).div(
            10**3
        );
    }
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_homelessFee ).div(
            10**3
        );
    }
    function removeAllFee() private {
        if(_worldHungerFee  == 0 && _homelessFee  == 0) return;
        _previousTaxFee = _worldHungerFee ;
        _previousDevelopmentFee = _mexicanWallFee ;
        _previousLiquidityFee = _homelessFee ;
        _worldHungerFee  = 0;
        _mexicanWallFee  = 0;
        _homelessFee  = 0;
    }
    function restoreAllFee() private {
        _worldHungerFee  = _previousTaxFee;
        _mexicanWallFee  = _previousDevelopmentFee;
        _homelessFee  = _previousLiquidityFee;
    }
