...
contract PlayerMarketplace is Ownable {
    ...
    function buyOut(uint256 auctionID) external {
        ...
        buyOutCount++;
        removeFromActiveAuctions(auction.auctionID);
        massUpdateAuctions();
    ...
    function isActiveAuction(uint256 auctionID) internal view returns (bool) {
    ...
    function removeFromActiveAuctions(uint256 index) internal {
        if(isActiveAuction(activeAuctions[index]))
        {
    ...
    function massUpdateAuctions() public {
        for(uint256 i = 0; i < activeAuctions.length; i++)
        {
            if(auctions[activeAuctions[i]].endBlock <= block.timestamp)
            {
                ...
                removeFromActiveAuctions(activeAuctions[i]);
            }
        }
