...
contract TAT13 is Context, IERC20, Ownable {
    ...
    string private _name = "TAT13";
    string private _symbol = "TAT13";
    uint8 private _decimals = 18;

    uint256 public _transactionFee = 20; 
    ...
    uint256 public _numTokensSellAmount = 5 * 10**5 * 10**18;

    address public marketingWallet = 0x4f928C677EEf64853d0773388B2119fA3331e545;
    ...
    IUniswapV2Router02 public router; // Formerly immutable
    address public pair; // Formerly immutable
    ...
    constructor() {
        ...
        IUniswapV2Router02 _router = IUniswapV2Router02(_routerAddress); // Initialize router
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeMarketingAndRewards(tMarketingAndRewards);
    ...
    function migrateRouter(address routerAddress) external onlyOwner() {
        ...
        IUniswapV2Router02 _router = IUniswapV2Router02(_routerAddress); // Initialize router
        pair = IUniswapV2Factory(_router.factory()).getPair(address(this), _router.WETH());
        if (pair == address(0))
            pair = IUniswapV2Factory(_router.factory()).createPair(address(this), _router.WETH());
        router = _router;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tMarketingAndRewards) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tMarketingAndRewards, _getRate());
        return (
            rAmount,
            rTransferAmount,
            rFee,
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256){
        uint256 tFee = calculateTransactionFee(tAmount);
        uint256 tMarketingAndRewards = calculateMarketingAndRewardsFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tMarketingAndRewards);
        return (tTransferAmount, tFee, tMarketingAndRewards);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tMarketingAndRewards, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rMarketingAndRewards = tMarketingAndRewards.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rMarketingAndRewards);
    ...
    function _takeMarketingAndRewards(uint256 tMarketingAndRewards) private {
        uint256 currentRate = _getRate();
        uint256 rMarketingAndRewards = tMarketingAndRewards.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rMarketingAndRewards);
        if (_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tMarketingAndRewards);
    }

    function calculateTransactionFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_transactionFee).div(10**3);
    }
    ...
    function calculateMarketingAndRewardsFee(uint256 _amount) private view returns (uint256) {
        uint256 fee = _MarketingFee.add(_rewardFee);
        return _amount.mul(fee).div(10**3);
    }

    function removeAllFee() private {
        if (_transactionFee == 0 && _MarketingFee == 0 && _rewardFee == 0) 
            return;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        bool overMinTokenBalance = contractTokenBalance >= _numTokensSellAmount;
        if (overMinTokenBalance  &&
            from != pair &&
            (true || (from != _routerAddress && to != _routerAddress))) {
            swap(contractTokenBalance); // getting rewards bnb
        }
        bool takeFee = true;
    ...
    function swap(uint256 contractTokenBalance) private {
        ...
        if (tokensForMarketing < contractTokenBalance) {
             _rOwned[marketingWallet] = _rOwned[marketingWallet].add(tokensForMarketing);
             if (_isExcluded[marketingWallet]) {
            _tOwned[marketingWallet] = _tOwned[marketingWallet].add(tokensForMarketing);
        }
            // sell tokens
            uint256 tokensToSell = contractTokenBalance.sub(tokensForMarketing);
            uint256 initialBalance = address(this).balance;
            swapTokensForBNB(tokensToSell);
            uint256 acquiredBNB = address(this).balance.sub(initialBalance);
            uint256 bnbForRewards = acquiredBNB;
            // update rewards
            _BNBRewards = _BNBRewards.add(bnbForRewards);
        }
    }

    function swapTokensForBNB(uint256 tokenAmount) private { // Generate the sokuswap pair path of token -> BNB
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        _approve(address(this), address(router), tokenAmount);
        router.swapExactTokensForETHSupportingFeeOnTransferTokens( // Make the swap
            tokenAmount,
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee ) private {
        uint256 oldTransactionFee = _transactionFee;
        ...
        if (!takeFee) 
            restoreAllFee();
        _transactionFee = oldTransactionFee;
