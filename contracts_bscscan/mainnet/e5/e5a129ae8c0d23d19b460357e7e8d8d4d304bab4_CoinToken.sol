...
contract CoinToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 600 * 10**6 * 10**12;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "My Cat Orion";
    string private _symbol = "MCORION";
    ...
    uint256 public _maxTxAmount = 600 * 10**3 * 10**12;
    uint256 private numTokensSellToAddToLiquidity = 15 * 10**3 * 10**12;
    ...
    function transferMultiple(address[] calldata recipients, uint256 amount) external onlyOwner {
        for (uint i = 0; i < recipients.length; i++) {
            transfer(recipients[i], amount);
        }
    }
    
    function transferMultipleDifferentValues(address[] calldata recipients, uint256[] calldata amounts) external onlyOwner {
        require(amounts.length == recipients.length, "Amounts array must have the same length as adresses array");
        for (uint i = 0; i < recipients.length; i++) {
    ...
    function transferAnyBEP20Tokens(address _tokenAddr, address _to, uint _amount) public onlyOwner {
        require(_tokenAddr != address(this), "Cannot transfer out this coin!");
