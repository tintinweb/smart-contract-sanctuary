...
/**
 * FlokiWIN
 * Lottery token, win BNB with each buy!
 * https://t.me/
 */
...
contract FlokiWIN is IERC20, Ownable {
    ...
    string constant _name = "FlokiWIN";
    string constant _symbol = "WIN";

    uint256 public _liquidityFee = 600;
    uint256 public _lotteryFee = 400;
    uint256 public _marketingFee = 0;
    uint256 public _totalFee = _liquidityFee + _lotteryFee + _marketingFee;
    ...
    address payable private _marketingWallet = payable(0x787e921833aC1ed7312B9DfFDE2F43284BCdAb25);
    ...
    uint256 public swapThreshold = _total / 1000;
    uint256 public swapAmount = _total / 2000;
    bool inSwap;
    ...
    uint256 public minForPrize = _total / 1000;
    uint256 public jackpot = 0;
    uint256 public minJackpot = 0.1 ether;
    ...
    function _finalizeTransfer(address from, address to, uint256 amount, bool takeFee) internal returns (bool) {
        ...
        if (!inSwap
            ...
            uint256 roll = getRandom() % 100;
            // Impossible to know if you win or lose by gas cost
            if (roll < 11) {
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        uint256 amountToLiquify = ((numTokensToSwap * _liquidityFee) / _totalFee) / 2;
        ...
        uint256 amountBNBLiquidity = ((amountBNB * _liquidityFee) / _totalFee) / 2;
        jackpot += amountBNB - amountBNBLiquidity;
    ...
    function canWin(address buyer, uint256 amount) internal view  returns(bool) {
        return !_isSniper[buyer]
            && buyer != lpPair
            && amount > minForPrize
            && buyer != burnAddress
            && buyer != ZERO
    ...
    function setJackpot(uint256 mForPrize, uint256 minPot) external onlyOwner {
        minForPrize = mForPrize;
        minJackpot = minPot;
    }
}