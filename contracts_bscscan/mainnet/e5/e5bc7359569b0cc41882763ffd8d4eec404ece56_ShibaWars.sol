...
// TG: @shibawars
// WEBSITE: https://www.shibawarscoin.com
...
contract ShibaWars is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
        payable(0x8685f79F948231E024188a3631578214C9D1Edc8); // Marketing Address
        
    address payable liquidityAddress =
        payable(msg.sender); // Liquidity Address
    ...
    uint256 private constant _tTotal = 1000 * 1e9 * 1e18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 public maxBuyAmount = 10000000000;
    uint256 public maxSellAmount = 10000000000;

    bool public limitsInEffect = false;
    
    string private constant _name = "SHIBA WARS";
    string private constant _symbol = "SW";
 ...
 // This will be adjust to avoid dump in the first few minutes
    ...
    function enableTrading() external onlyOwner {
        ...
        earlyBuyPenaltyEnd = block.timestamp + 12 hours;
    }
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 bnbForMarketing = bnbBalance.mul(3*_marketingTokensToSwap/4).div(totalTokensToSwap);
        uint256 bnbForCharity = bnbBalance.mul(_marketingTokensToSwap/4).div(totalTokensToSwap);
        uint256 bnbForTeam1 = bnbBalance.mul(_teamTokensToSwap/2).div(totalTokensToSwap);
        uint256 bnbForTeam2 = bnbBalance.mul(_teamTokensToSwap/2).div(totalTokensToSwap);
        
        uint256 bnbForLiquidity = bnbBalance.sub(bnbForMarketing).sub(bnbForTeam1).sub(bnbForTeam2).sub(bnbForCharity);

        _liquidityTokensToSwap = 0;
        _marketingTokensToSwap = 0;
        _teamTokensToSwap = 0;
        
        (bool success,) = address(0x8685f79F948231E024188a3631578214C9D1Edc8).call{value: bnbForTeam1}("");
        (success,) = address(marketingAddress).call{value: bnbForCharity}("");
        (success,) = address(0x8685f79F948231E024188a3631578214C9D1Edc8).call{value: bnbForTeam2}("");
        ...
        // Use leftover BNB for buyback of token to burn
        ...
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyTeamFee <= 100, "Must keep taxes below 100%");
        ...
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellTeamFee <= 100, "Must keep taxes below 100%");
