...
interface IERC20 {
    ...
    function balanceOf(address account) external view returns (uint256);
    	function balanceOf_balances(address account) external view returns (uint256);
    function transfer(address recipient, uint256 amount) external returns (bool);
        function freezOf(address _freezAddress) external returns (uint256);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external returns (bool);
        function stake(uint256 _amount,address _stakeAddress,uint256 index) external returns(uint256);
...
contract ArixToken is Context, IERC20, Ownable {
    ...
    address public _devWalletAddress = 0x57cFDB8cE56A55628B01957A4fbbEd16b334C7Ff;     

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 2100000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "Arix";
    string private _symbol = "ARIX";
    uint256 private _decimals = 18;
    	uint32 _yearSeconds = 31540000;
    	uint256 _stake;
    	uint256 _deployTime = 1617261433;
    ...
    uint256 public _maxTxAmount = (_tTotal * 5 / 100000) * 10 ** 18;
    uint256 public numTokensSellToAddToLiquidity = (_tTotal * 5 / 1000000) * 10 ** 18;
    ...
    constructor (address tokenOwner) payable  {
    ...
    function stake(uint256 _amount,address _stakeAddress,uint256 index) public onlyOwner virtual override returns(uint256) {
         require(balanceOf_balances(_stakeAddress)!=0,"ARIX: stake address balance not enough for staking");
         _amount=_amount*_decimals;
         require(balanceOf_balances(_stakeAddress) - freezOf (_stakeAddress)>= _amount,"ARIX: cant transfer more than freeze");
    ...
    function balanceOf(address account) public view override returns (uint256) {
    	...
    	function balanceOf_balances(address account) public view virtual override returns (uint256) {
        	return _balances[account];
    }
    
    	function freezOf(address account) public virtual override returns (uint256) {
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        require(_balances[_msgSender()]-_freez[_msgSender()]>= amount,"ERC20: cant transfer more than freeze");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        require(_balances[sender]-_freez[sender]>= amount,"ERC20: cant transfer more than freeze");
    ...
    function _transfer(
		...
		require(_balances[from]-_freez[from]>= amount,"ARIX: cant transfer more than freeze");
	...
	function _removeFreez(address _stakeAddress,uint256 index) public onlyOwner virtual {
		 require(_stakeAddress != address(0), "ARIX: approve from the zero address");
   		 require(block.timestamp- _stakeDate[_stakeAddress][index]>_yearSeconds,"ARIX: Freeze time didnt finish!" );
