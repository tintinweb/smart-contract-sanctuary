...
/*
 * PAPERCUT (COMMUNITY TOKEN) // THIS IS TEST CONTRACT! DONT BUY!
 * =========================================
 * Community Chat: https://t.me/papercuttokens
 * =========================================
 * 
 * TOKENOMICS :
 * Total Supply: 10 Billion
 * Max Buy: 100 Million (1% of Total Supply)
 * Max Hold: 200 Million (2% of Total Supply)
 *
 * LP LOCKED FOR 7 DAYS, AND WILL BE RELOCKED ON DXLOCKER OR BURNED BASED BY POLL.
 * CONTRACT RENOUNCED AFTER LAUNCH.
 * 
 * WE CUT THE EARLY SELLERS AND THE BOT.
 * NO BUY TAX FEES
 * Sell within 3 hours     : 60% burn (40% Back to the Liquidity Pool, 20% Redistributed to Holders)
 * Sell within 12  hours   : 24% burn (16% Back to the Liquidity Pool, 8% Redistributed to Holders)
 * Sell within 24  hours   : 18% burn (12% Back to the Liquidity Pool, 6%  Redistributed to Holders)
 * Sell within 30  hours   : 12% burn (8% Back to the Liquidity Pool, 4%  Redistributed to Holders)
 * Standard burn           : 6% burn  (4% Back to the Liquidity Pool, 2%  Redistributed to Holders)
 * 
 * MARKETING PLAN:
 * WE HAVE ZERO TOKEN ON DEV OR TEAM WALLET, WE WILL USE OUR PERSONAL FUNDS
 * TO PROMOTE ON TIKTOK AFTER 10k MEMBER
 */
...
contract PAPERCUT is Context, IBEP20, Ownable {
    ...
    uint256 public _maxWalletToken = 10000 * 10**6 * 10**9; // Max Wallet: 200 Million (2%)

    IPancakeRouter02 public immutable pcsV2Router;
    address public immutable pcsV2Pair;

    string private _name = "PAPERCUT";
    string private _symbol = "CUT";
    ...
    function _getAntiDumpMultiplier() private view returns (uint256) {
        ...
        if (time_since_start < 3 * hour) {
            ...
            return (3);
        } else if (time_since_start < 30 * hour) {
            return (2);
        } else {
            return (1);
        }
    ...
    function setMaxWallePercent(uint256 maxWallPerc) external onlyOwner() {
        _maxWalletToken = _tTotal.mul(maxWallPerc).div(
            10**2
    );
    }
