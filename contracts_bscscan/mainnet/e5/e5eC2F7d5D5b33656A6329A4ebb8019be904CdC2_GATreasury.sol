...
contract GATreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SGA }

    address public immutable GA;
    ...
    address public sGA;
    uint public sGAQueue; // Delays change to sGA address
    ...
    /**
        @notice allow approved address to deposit an asset for GA
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint GA needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( GA ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn GA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IGAERC20( GA ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sGA ).balanceOf( msg.sender ); // Can only borrow against sGA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with GA
        @param _amount uint
     */
    function repayDebtWithGA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IGAERC20( GA ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, GA, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( GA ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( GA ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns GA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match GA decimals
            value_ = _amount.mul( 10 ** IERC20( GA ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SGA ) { // 9
            sGAQueue = block.number.add( blocksNeededForQueue );
        } else return false;
