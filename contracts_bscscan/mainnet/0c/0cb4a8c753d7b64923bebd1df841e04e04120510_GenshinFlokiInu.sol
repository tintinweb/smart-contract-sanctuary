...
contract DividendDistributor is IDividendDistributor {
  ...
  IBEP20 RWRD = IBEP20(0xbA2aE424d960c26247Dd6c32edC70B295c744C43);		//DOGE币
  address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;			//WBNB币
  IDEXRouter router;
  ...
  constructor(address _router) {
    router = _router != address(0)
      ? IDEXRouter(_router)
      : IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);		//流动池
    _token = msg.sender;
  }
...
contract GenshinFlokiInu is IBEP20, Auth {
  ...
  address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;				//WBNB币
  address DEAD = 0x000000000000000000000000000000000000dEaD;				//普通黑洞
  address ZERO = 0x0000000000000000000000000000000000000000;				//创世黑洞

  string constant _name = "MotoA";											//代币名称
  string constant _symbol = "MA";											//代币符号，简称
  uint8 constant _decimals = 4;												//代币小数点位数
  ...
  constructor() Auth(msg.sender) {
    router = IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);				//流动池
    pair = IDEXFactory(router.factory()).createPair(WBNB, address(this));			//钱包地址，流动池中转
    _allowances[address(this)][address(router)] = uint256(-1);

    pairContract = InterfaceLP(pair);												//钱包地址，流动池中转									
    _totalSupply = INITIAL_FRAGMENTS_SUPPLY;
    rate = rSupply.div(_totalSupply);

    distributor = new DividendDistributor(address(router));					//也是个钱包地址
    ...
    autoLiquidityReceiver = msg.sender;										//钱包地址，作用不明
    marketingFeeReceiver = 0x7E16C157f0Ed22D9E0A0f53a6A7FcBf2b9Bc2BCe;		//项目方钱包1会流入BNB
    devFeeReceiver = 0xA8feb526D13CfBE7d3Aee18159F3c37570cD3682;			//项目方钱包2会流入BNB
