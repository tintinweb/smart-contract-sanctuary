...
contract YSLOptSingleApe is YSLOpt {
    ...
    function currentRewardUSD(address _user) public view returns (uint256) {
        ...
        uint256 usersUSD = (userPendingBanana + userCompounds) * getPrice(address(want), BUSD, 1 ether, apeSwap) / 1 ether;
        // Optimisation
    ...
    function currentReward(address _user) public view returns (uint256) {
        address _sysl = IStrategySwap(stratSwap).sYSL();
        uint256 rewardUSD = currentRewardUSD(_user);
        if (rewardUSD == 0) {
            return 0;
        }
        return rewardUSD * getPrice(BUSD, _sysl, 1 ether, apeSwap) / 1 ether;
    }
}