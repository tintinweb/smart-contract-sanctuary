...
/* FC Fantos Binance launch Pool - BNB Reward Token

website: fcsantosfantoken.com
Listing 1 SANTOS = 1.1 USD
...
contract Santos is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0xe59806936e9D2F0e0267bCB9672A4E911304eB25); // Marketing Address
    address payable public charityAddress = payable(0xe59806936e9D2F0e0267bCB9672A4E911304eB25); // Charity/Dev Address
    ...
    uint256 private _tTotal = 30000000 * 10**8; 
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "FC Santos Fan Token";
    string private _symbol = "SANTOS";
    ...
    uint256 public _maxTxAmount = 7500 * 10**8;  //Maximum amount can be used for transaction (antiwhale setup))
    uint256 private minimumTokensBeforeSwap = 7500 * 10**8;  //allow maximum sell amount(anti-dump setup)
  ...
  function setALLFeePercent(uint256 charity,uint256 marketting,uint256 burnTaxFee,uint256 liquidityFee,uint256 taxFee,uint256 sellLiquidityFee,uint256 sellTaxFee,uint256 buyLiquidityFee,uint256 buyTaxFee) external onlyOwner() {
    ...
    uint256 private _airdropToken = 5000000000; // 50; token will be given on airdrop
    ...
    uint256 private salePrice = 75000;// 0.01*750 token for 0.01 bnb (set as per requirements)
