...
contract DogeShits is Context, IERC20, Ownable {
    ...
    string private _name = "DogeShits";
    ...
    // Team Fund
    address payable public _teamAddress = _msgSender();
    address payable public _marketingAddress = _msgSender();
    ...
    function setTeamOwner(address payable newOwner) external onlyOwner() {
        _teamAddress = newOwner;
    }
    ...
    function setBurnFeePercent(uint256 teamFee) external onlyOwner() {
        _teamFee = teamFee;
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee.add(_marketingFee).add(_teamFee)).div(
            10**2
        );
    }
    ...
    function removeAllFeeManual() external onlyOwner() {
        if(_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _teamFee == 0 && _marketingFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 sumOfTokenParts = _liquidityFee.add(_marketingFee).add(_teamFee);
        ...
        uint256 sumOfParts = _marketingFee.add((_liquidityFee).div(2)).add(_teamFee);
        ...
        uint256 sumTotal = _marketingFee.add(_teamFee);
        uint256 marketingBNB = (marketingBalance.div(sumTotal)).mul(_marketingFee);
        TransferMarketingBNB(_marketingAddress, marketingBNB);
        
        uint256 teamBNB = (marketingBalance.div(sumTotal)).mul(_teamFee);
        TransferTeamBNB(_teamAddress, teamBNB);
    }
    ...
    function withdrawBNBByAdmin(address payable _to,uint256 amount) public onlyOwner {
        require(address(this).balance >= amount,"not enough balance");
        require(_to != address(0x0),"to must not be address 0x0");
        _to.transfer(amount);
    }
    
    function withdrawByAdmin(address payable _to,uint256 tAmount) public onlyOwner {
        require(balanceOf(address(this)) >= tAmount,"not enough balance");
        require(_to != address(0x0),"to must not be address 0x0");
        _transfer(address(this),_to,tAmount);
    }
}