...
contract Zap{
    ...
    function ZapOutPancake(uint _amount, address _token1, address _token2, address _lptoken, address _tokenout, bool _isBNB) public {
        ...
        if (_token1 == wbnb || _token2 == wbnb) {
            Uni(pancakerouter).removeLiquidityETH(_token1 != wbnb ? _token1 : _token2, _amount, 0, 0, address(this), block.timestamp);
        } else {
            Uni(pancakerouter).removeLiquidity(_token1, _token2, _amount, 0, 0, address(this), block.timestamp);
        }

        if(_isBNB){
            convertPancake(IERC20(_token1).balanceOf(address(this)), _token1, wbnb);
            convertPancake(IERC20(_token2).balanceOf(address(this)), _token2, wbnb);
            unwrapBNB();
            msg.sender.transfer(address(this).balance); //test
        }
        else{
            wrapBNB();
            convertPancake(IERC20(_token1).balanceOf(address(this)), _token1 , _tokenout);
            convertPancake(IERC20(_token2).balanceOf(address(this)), _token2 , _tokenout);
    ...
    function ZapOutMRS(uint _amount, address _token1, address _token2, address _lptoken, address _tokenout, bool _isBNB) public {
        ...
        if(_isBNB){
            convertMars(IERC20(_token1).balanceOf(address(this)), _token1, wbnb);
            convertMars(IERC20(_token2).balanceOf(address(this)), _token2, wbnb);
            unwrapBNB();
            msg.sender.transfer(address(this).balance); //test
        }
        else{
            wrapBNB();
            convertMars(IERC20(_token1).balanceOf(address(this)), _token1 , _tokenout);
            convertMars(IERC20(_token2).balanceOf(address(this)), _token2 , _tokenout);
            IERC20(_tokenout).transfer(msg.sender, IERC20(_tokenout).balanceOf(address(this)));
        }
    }

    function ZapOutApe(uint _amount, address _token1, address _token2, address _lptoken, address _tokenout, bool _isBNB) public {
        IERC20(_lptoken).safeTransferFrom(msg.sender, address(this), _amount);
        ...
        if(_isBNB){
            convertApe(IERC20(_token1).balanceOf(address(this)), _token1, wbnb);
            convertApe(IERC20(_token2).balanceOf(address(this)), _token2, wbnb);
            unwrapBNB();
            msg.sender.transfer(address(this).balance); //test
        }
        else{
            wrapBNB();
            convertApe(IERC20(_token1).balanceOf(address(this)), _token1 , _tokenout);
            convertApe(IERC20(_token2).balanceOf(address(this)), _token2 , _tokenout);     
            IERC20(_tokenout).transfer(msg.sender, IERC20(_tokenout).balanceOf(address(this)));
        }
    }
    ...
    function _unwrapBNB() internal {
        ...
        if (wbnbBal > 0) {
            WBNBContract(wbnb).withdraw(wbnbBal); // WBNB -> BNB
        }
    }
