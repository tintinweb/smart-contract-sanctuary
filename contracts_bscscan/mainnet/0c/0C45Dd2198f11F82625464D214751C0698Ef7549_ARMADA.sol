...
/**                 >  Armada  < Introducing liquid Fees
 *                                                                 
 *  A HYPER-DEFLATIONARY, BUYBACK POWERED CRYPTOCURRENCY WITH LIQUID FEES
 *  
...
abstract contract DepreciatingFees {
    ...
    uint256 internal reductionPerSec = 11; //0.000011 per second
    uint256 internal reductionDivisor = 10**6; // reduction multiplier
    uint256 internal updateFeeTime = 1; // every second
    uint256 internal FEES_DIVISOR = 10**8; // gives you the true percentage
    ...
    (uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 _estimatedRfiFee, uint256 _estimatedBuyBackFee, uint256 _estimatedMarketingFee, 
            uint256 _estimatedAntiDumpFee, uint256 _estimatedAccruedFees) = getEstimatedFee(_userAddress);
    ...
    uint256 reduceMarketingFee, uint256 reduceAntiDumpFee, uint256 estimatedAccruedFees) {
        ...
        return (
            fees[_userAddress].rfiFee <= accruedFees ? 0 : fees[_userAddress].rfiFee.sub(accruedFees),
            fees[_userAddress].buyBackFee <= accruedFees ? 0 : fees[_userAddress].buyBackFee.sub(accruedFees),
            fees[_userAddress].marketingFee <= accruedFees ? 0 : fees[_userAddress].marketingFee.sub(accruedFees),
            fees[_userAddress].antiDumpFee <= accruedFees ? 0 : fees[_userAddress].antiDumpFee.sub(accruedFees),
            accruedFees
            );
        }
    ...
    */
    function calculateBuyingReadjustment(address _userAddress, uint256 _amount, uint256 accruedFees, uint256 _rfiFee, uint256 _buyBackFee,
    uint256 _marketingFee, uint256 _antiDumpFee) internal view returns (uint256, uint256, uint256, uint256) {
        
        uint256 estimatedTotal = _rfiFee.add(_marketingFee).add(_buyBackFee).add(_antiDumpFee);
    ...
    function reAdjustFees(uint256 _amount, uint256 accruedFees, uint256 _rfiFee, uint256 _buyBackFee,  uint256 _marketingFee, 
        ...
        (uint256 _newRfiFee, uint256 _newMarketingFee, uint256 _newBuyBackFee, uint256 _newAntiDumpFee) = 
            calculateBuyingReadjustment (msg.sender, _amount, accruedFees, _rfiFee, _marketingFee, _buyBackFee, _antiDumpFee);
        ...
        updateFeeHolder( 
            msg.sender, 
            _isBuying ? _newRfiFee : __rfiFee.add(_accruedFees), 
            _isBuying ? _newBuyBackFee : __buyBackFee.add(_accruedFees), 
            _isBuying ? _newMarketingFee : __marketingFee.add(_accruedFees),  
            _isBuying ? _newAntiDumpFee : __antiDumpFee.add(_accruedFees),
            block.timestamp, 
...
contract ARMADA is IERC20Metadata, DepreciatingFees, Ownable {
    ...
    string constant _name = "TEST.Armada";
    string constant _symbol = "TEST.ARMD";
    ...
    function _transferTokens(address sender, address recipient, uint256 amount, bool takeFee, bool feeReduction) private {
        ...
        // grabthe estimated reduced fees
       (uint256 reduceRfiFee, uint256 reduceBuyBackFee, uint256 reduceMarketingFee,  
        uint256 reduceAntiDumpFee, uint256 estimatedAccruedFees) = _getHolderFees(msg.sender, feeReduction);
        ...
        if(feeReduction) {
            ...
            reAdjustFees(amount, estimatedAccruedFees, reduceRfiFee, reduceBuyBackFee, reduceMarketingFee, reduceAntiDumpFee, isBuying);
        }
