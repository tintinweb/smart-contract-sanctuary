...
/*
   ...
   Tokenomics
   5% auto redistributed to HODLers
   5% auto redistributed to BUYBACK
   2% auto redistributed to LP
   
   https://t.me/AnimeDogeBSC
   https://www.animedoge.xyz/
*/
...
contract AnimeDoge is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000 * 10**9 * 10**9; // Total Supply: 1T
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "AnimeDoge";
    string private _symbol = "ANIMEDOGE";
    ...
    uint256 private _marketingPercentageOfLiquidity = 80;
    ...
    uint256 public _maxTxAmount = 2000 * 10**6 * 10**9; // Max Transaction: 2B (0.20%)
    uint256 public _maxWalletToken = 12500 * 10**6 * 10**9; // Max Wallet: 12.5B (1.25%)
    uint256 private numTokensSellToAddToLiquidity = 2000 * 10**6 * 10**9; // Liquidity sells 2B (0.20%)
    ...
    event MarketingCollected(uint256 ethCollected);
    ...
    function _transfer(
        ...
        require(to != marketing(), "The marketing address cannot receive tokens");
        require(from != marketing(), "The marketing address cannot send tokens");
    ...
    {
        _totalMarketingCollected = _totalMarketingCollected.add(address(this).balance);
        emit MarketingCollected(address(this).balance);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // this also ignores any ETH that was reserved for marketing last time
        // this was called
        uint256 initialBalance = address(this).balance;

        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> SHARK swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
        
        
        //Now reserve some of that eth for marketing to collect
        uint256 balanceToMarketing = (newBalance.mul(marketingPercentageOfLiquidity())).div(10**2);
        uint256 tokensExtra = (otherHalf.mul(marketingPercentageOfLiquidity())).div(10**2);
         
        //How much eth is left for liquidity?
        newBalance = newBalance.sub(balanceToMarketing);
