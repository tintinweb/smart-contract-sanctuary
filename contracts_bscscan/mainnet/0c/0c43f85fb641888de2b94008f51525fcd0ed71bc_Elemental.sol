...
// File: oken radom_flat_flat_flat.sol
...
contract Elemental is Context, IERC20, Ownable {
    ...
    */
    ...
    address private constant marketingWallet = 0xAc9402A98dcCDc707659D3264eEC6533B67d15FE;
    address private constant gameWallet = 0x86e7EC4BE8b12bEbA8146101d9146b473218721B;
    ...
    string private _name = 'Elemental';
    string private _symbol = 'ELMT';

    uint256 reflectionFeePercentage = 2;
    uint256 marketingFeePercentage = 3;
    uint256 gameFeePercentage = 2;
    uint256 burnFeePercentage = 3;
    ...
    */
    ...
    function _transfer(address sender, address recipient, uint256 tAmount, TransferType transferType) private {
        ...
        _rOwned[gameWallet] = _rOwned[gameWallet].add(_rValues.rGameFee);
        ...
        if (_tValues.marketingFee != 0) {
         ...
         if (_tValues.gameFee != 0) {
            emit Transfer(sender, gameWallet, _tValues.gameFee);
        }
    ...
    function _getRInputs(
        ...
        return RInputs(tAmount, reflectionFee, marketingFee, gameFee, burnFee, currentRate);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (TValues memory) {
        ...
        if (!isNoFeeAddress) {
            ...
            gameFee = tAmount.mul(gameFeePercentage).div(100);
            burnFee = tAmount.mul(burnFeePercentage).div(100);
        }

        uint256 tTransferAmount = tAmount.sub(reflectionFee).sub(marketingFee).sub(gameFee).sub(burnFee); 
        return TValues(tTransferAmount, reflectionFee, marketingFee, gameFee, burnFee); 
    }
    ...
    function _getRValues(RInputs memory rInputs) private pure returns (RValues memory) {
        ...
        uint256 rGameFee = rInputs.gameFee.mul(rInputs.currentRate);
        uint256 rBurnFee = rInputs.burnFee.mul(rInputs.currentRate);
        uint256 rTransferAmount = rAmount.sub(rReflectionFee).sub(rMarketingFee).sub(rGameFee).sub(rBurnFee);
        return RValues(rAmount, rTransferAmount, rReflectionFee, rMarketingFee, rGameFee, rBurnFee);
    }
