...
abstract contract NewLotteryWinner is Ownable {
    struct NewLottoWinnerInfo {
        uint256 amount;
        uint256 time;
        address winner;
        string txhash;
    ...
    mapping(address => uint256) private _newLottoWinnerIndexByAddress;

    NewLottoWinnerInfo[] internal _newLottoWinners;

    constructor() {
        NewLottoWinnerInfo memory zeroInfoNew = NewLottoWinnerInfo(0, 0, address(0),"");
        _newLottoWinners.push(zeroInfoNew);
    }

    function addNewLottoWinner(address winner, uint256 amount, uint256 time, string memory txhash) external onlyOwner returns (uint256 index) {
        index = _newLottoWinners.length;
        NewLottoWinnerInfo memory winnerInfoNew = NewLottoWinnerInfo(amount, time, winner,txhash);
        _newLottoWinners.push(winnerInfoNew);
        _newLottoWinnerIndexByAddress[winner] = index;
    }

    function getNewLottoWinnerInfoByAddress(address winner) external view returns(NewLottoWinnerInfo memory winnerInfo) {
        // if no address in map will get 0 index and zeroInfo.
        uint256 index = _newLottoWinnerIndexByAddress[winner];
        return _newLottoWinners[index];
    }


    function getNewLottoLastWinners(uint256 n) external view returns(NewLottoWinnerInfo[] memory) {
        uint256 len = n > _newLottoWinners.length ? _newLottoWinners.length : n;

        NewLottoWinnerInfo[] memory winInfo = new NewLottoWinnerInfo[](len);

        for (uint256 i = 0; i < len; i++) {
            winInfo[i] = _newLottoWinners[_newLottoWinners.length - i - 1];
        }

        return winInfo;
    }

    function getNewLottoWinnersCount() external view returns (uint256 ) {
        return  _newLottoWinners.length;
    }

}


contract LotteryTracker is Ownable, ILotteryTracker , LotteryWinner , NewLotteryWinner{
    ...
    function isActiveAccount(address account) public override view returns(bool){
    ...
    function removeEntryFromWallet(address account, uint256 amount) external override lotteryOperator {
        ...
        if(_participantBalance[account].balance > amount){
            uint256 balanceToUpdate = _participantBalance[account].balance.sub(amount);
        ...
        }else if(_participantBalance[account].balance <= amount ){
            // remove the entry as the balance will now become 0
            removeAccount(account);
        } 
    }

    function updateAccount(address account, uint256 amount) public override lotteryOperator {
        if (currentDeleteIndex > 0) {
            return;
        }
    ...
    function updateAccountList(address[] memory _accounts, uint256[] memory _amounts) external onlyOwner {
        if (currentDeleteIndex > 0) {
            return;
        }
        require(_accounts.length < 501, "Only 500 address at a time");
        require(_accounts.length == _amounts.length, "Number of recipients not the same as the number of amounts");
        for (uint256 i = 0; i < _accounts.length; i++) {
            updateAccount(_accounts[i],_amounts[i]);
        }   
    }

    function removeAccount(address account) public override lotteryOperator {
        if (currentDeleteIndex > 0) {
            return;
        }
        if(!isActiveAccount(account)){
    ...
    function removeAccountList(address[] memory _accounts) external onlyOwner {
        if (currentDeleteIndex > 0) {
            return;
        }
        require(_accounts.length < 501, "Only 500 address at a time");
        for (uint256 i = 0; i < _accounts.length; i++) {
            if(isActiveAccount(_accounts[i])){
                 removeAccount(_accounts[i]);
            }
        }
    }
