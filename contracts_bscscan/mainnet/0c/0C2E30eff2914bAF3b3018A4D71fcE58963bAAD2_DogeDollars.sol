...
contract DogeDollars is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
        payable(0x7414f424723dC4E9b2284AE6F3ad437916955F84); // Marketing Address
    ...
    string private _name = "DogeDollars";
    string private _symbol = "DogeDollars";
    ...
    uint256 public _maxTxSellAmount = 30000000 * 10**6 * 10**9;
    uint256 public _maxTxBuyAmount = 30000000 * 10**6 * 10**9;
        ...
        if (from != owner() && to != owner()) {
            if (to == uniswapV2Pair) {
                require(
                    ...
                    "Transfer amount exceeds the maxTxSellAmount."
                );
            } else if (from == uniswapV2Pair) {
                require(
                    amount <= _maxTxBuyAmount,
                    "Transfer amount exceeds the maxTxBuyAmount."
                );
            }
        }
            ...
            if (to == uniswapV2Pair) {
                ...
                if (buyBackEnabled) {
                    ...
                    if (_isAutoBuyBack) {
                        ...
                        uint256 startTime =
                            block.timestamp - _buyBackTimeInterval;
                        uint256 cnt = 0;
                    ...
                    uint256 _bBSLimitMin =
                        _bBSLimitMax.mul(_buyBackRangeRate).div(100);
    ...
    function _removeOldSellHistories() private {
        uint256 i = 0;
        uint256 maxStartTimeForHistories =
    ...
    function setMaxTxBuyAmount(uint256 maxTxBuyAmount) external onlyOwner {
        _maxTxBuyAmount = maxTxBuyAmount;
    }

    function setMaxTxSellAmount(uint256 maxTxSellAmount) external onlyOwner {
        _maxTxSellAmount = maxTxSellAmount;
    }
