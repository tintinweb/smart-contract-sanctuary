...
contract Fomo {
    ...
    constructor (IERC20 _cow) public {
        cow = _cow;
    ...
    function transferNotify(address user) external {
        require(msg.sender == address(cow), "permission denied");
    ...
    function swap() public {
        ...
        inSwap = true;
        uint256 tokenAmount = cow.balanceOf(address(this));
        if (tokenAmount == 0) {
            inSwap = false;
            return;
        }
        ...
        cow.approve(address(uniswapV2Router), tokenAmount);
    ...
    function buyAndBurn() public {
        ...
        uint256 tokenBalance = cow.balanceOf(address(this));
        if (tokenBalance > 0) {
            cow.transfer(blackHole, tokenBalance);
        }
      }
    }
