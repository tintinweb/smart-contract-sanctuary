 ...
 * This allows applications to reconstruct the allowance for all acBunnyArt just
...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also BunnyArted.
...
// File: contracts/BunnyArtDividendPayingToken.sol
...
// File: contracts/BunnyArt.sol

// 

pragma solidity ^0.7.6;

contract BunnyArt is ERC20, Ownable {
    using SafeMath for uint256;
	...
	address payable public _FundAddress = payable(0x439c1c05eB5e2a49FdB1222d7Cc42DbE7Dd38278); 
		
    bool private swapping;

    BunnyArtDividendTracker public dividendTracker;
    ...
    event ExcludeMultipleAcBunnyArtFromFees(address[] acBunnyArt, bool isExcluded);
    ...
    constructor() ERC20("BunnyArt", "BART"){
        ...
        dividendTracker = new BunnyArtDividendTracker();
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BunnyArt: Account is already the value of 'excluded'");
    ...
    function excludeMultipleAcBunnyArtFromFees(address[] calldata acBunnyArt, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < acBunnyArt.length; i++) {
            _isExcludedFromFees[acBunnyArt[i]] = excluded;
        }

        emit ExcludeMultipleAcBunnyArtFromFees(acBunnyArt, excluded);
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BunnyArt: The uniswap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BunnyArt: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BunnyArt: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) private onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BunnyArt: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BunnyArt: Cannot update gasForProcessing to same value");
	...
	function setFeeReceivers(address payable MarketingAddress, address payable FundAddress) external onlyOwner() {
        _MarketingAddress = MarketingAddress;
        _FundAddress = FundAddress;

    }
		function setBNBAddress(address payable BNB) external onlyOwner() {
        BNB = BNB;
	}
	function setFees(uint256 _LiquidityFee, uint256 _BNBRewardsFee, uint256 _MarketingFee, uint256 _FundFee) external onlyOwner() {
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!hasLaunched) {
            require(canTransferBeforehasLaunched[from], "BunnyArt: This account cannot send tokens until trading is enabled");
        }
...
contract BunnyArtDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BunnyArt_Dividend_Tracker", "BunnyArt_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**2); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "BunnyArt_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "BunnyArt_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BunnyArt_Dividend_Tracker: Cannot update claimWait to same value");
