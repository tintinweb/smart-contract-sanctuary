...
contract Pugsy is Context, IERC20, Ownable {
    ...
    string private constant _name = "Pugsy Malone";
    string private constant _symbol = "PUGSY";
    ...
    bool public swapAndLiquifyEnabled = true;
        ...
        //Anti-Whale System
    uint256 public _maxWalletAmount = 30000000 * 10**6 * 10**9;
    ...
    constructor () public {
        ...
        _isExcludedFromFee[_marketingDevAddress] = true;
        _isExcludedFromFee[_charityAddress] = true;

        _marketingDevAddress = 0x4205d5AE579e9708b49347580e9e780f78001285;
        
        _charityAddress = 0x5Cf769F252d4BcD973e025b9e074f5E45A7ba123;
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= 6, "Tax fee cannot be set higher than 6%");
        _taxFee = taxFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 6, "Liquidity Fee cannot be set higher than 6%");
        _liquidityFee = liquidityFee;
    }
   
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        require(maxTxPercent >= 1, "Cannot be lower");
    ...
    function setMaxWalletPercent(uint256 maxWalletPercent) external onlyOwner() {
        require(maxWalletPercent >= 1, "Cannot be lower");
    ...
    function setMarketingDevFeePercent(uint256 marketingAndDevBudget) external onlyOwner {
        _marketingFee = marketingAndDevBudget;
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && !presaleAddresses[from] && to != address(0xdead) && to != uniswapV2Pair){
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 half = contractTokenBalance.div(3).mul(2);
        uint256 otherHalf = contractTokenBalance.sub(half);
        uint256 totalLiqFee = _marketingFee + _liquidityFee + _charityFee;
        ...
        newBalance = newBalance.sub(halfBalance);
        ...
        uint256 finalBalance = newBalance.sub(marketingBalance).sub(charityBalance);
        uint256 finalHalf = otherHalf.sub(marketingPortion).sub(charityPortion);


        (bool sentMarketing, ) = _marketingDevAddress.call{value: marketingBalance.add(halfBalance.div(2))}("");
        if(sentMarketing){
        ...
        (bool sentCharity, ) = _charityAddress.call{value: charityBalance.add(halfBalance.div(2))}("");
        if(sentCharity){
        ...
        addLiquidity(finalHalf, finalBalance, _marketingDevAddress);
        
        emit SwapAndLiquify(half, newBalance, otherHalf);
    }
    
    function rescueBNB() public returns(bool) {
        _marketingDevAddress.transfer(address(this).balance);
        return true;
    }
    
    
    function rescueTokens(address token) public returns(bool) {
        IERC20(token).transfer(_marketingDevAddress, IERC20(token).balanceOf(address(this)));
        return true;
    }
