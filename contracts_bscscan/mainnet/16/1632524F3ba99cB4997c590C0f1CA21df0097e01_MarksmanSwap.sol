...
██║░╚═╝░██║ ██║░╚═╝░██║ ███████░║
╚═╝░░░░░╚═╝ ╚═╝░░░░░╚═╝ ╚═░░░░══╝
Web: https://www.marksmanswap.com/
Telegram: https://t.me/marksmanswap
*/
...
contract MarksmanSwap is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 33 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    address private _devAddress = 0xB4E6eDE1d213eAB8660d3a24bDA66A9d3CFbE43D;

    string private _name = "MarksmanSwap";
    ...
    uint256 public _maxTxAmount = 16 * 10**5 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 16 * 10**4 * 10**9;
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        (,uint256 tFee, uint256 tLiquidity, uint256 tDev) = _getTValues(tAmount, false);
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tDev) = _getTValues(tAmount, recipient == uniswapV2Pair);
    ...
    function _getTValues(uint256 tAmount, bool selling) private view returns (uint256, uint256, uint256, uint256) {
    ...
    function calculateLiquidityFee(uint256 _amount, bool selling) private view returns (uint256) {
        ...
        return _amount.mul(lFee).div(
            10**2
        );
    }

    function calculateDevFee(uint256 _amount, bool selling) private view returns (uint256) {
        uint256 dFee = selling ? _sellDevFee : _devFee;
        return _amount.mul(dFee).div(
            10**2
        );
    }
