...
// CherryToken with Governance.
contract CherryToken is BEP20('CherrySwap Token', 'CHE') {
    ...
    */
        ...
        external
    {
        ...
        require(now <= expiry, "CHERRY::delegateBySig: signature expired");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying CHERRYs (not scaled);
...
contract SyrupBar is BEP20('SyrupBar Token', 'SYRUP') {
    ...
    // Safe cherry transfer function, just in case if rounding error causes pool to not have enough CHERRYs.
    function safeCherryTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 cherryBal = cherry.balanceOf(address(this));
        if (_amount > cherryBal) {
            cherry.transfer(_to, cherryBal);
        } else {
            cherry.transfer(_to, _amount);
        }
    }
...
interface IMigratorChef {
    // Perform LP token migration from legacy CherrySwap to CherrySwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to CherrySwap LP tokens.
    // CherrySwap must mint EXACTLY the same amount of CherrySwap LP tokens or
    // else something bad will happen. Traditional CherrySwap does not
    // do that so be careful!
    function migrate(IBEP20 token) external returns (IBEP20);
}

// MasterChef is the master of Cherry. He can make Cherry and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once CHERRY is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        // We do some fancy math here. Basically, any point in time, the amount of CHERRYs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accCherryPerShare) - user.rewardDebt
        //
    ...
    struct PoolInfo {
        ...
        uint256 accCherryPerShare; // Accumulated CACHERRYper share, times 1e12. See below.
    }
    ...
    // CHERRY tokens created per block.
    ...
    // The block number when CHERRY mining starts.
    ...
    // View function to see pending CHERRYs on frontend.
