...
contract AlterationXBurningMoonNFTs is ERC165, IERC721, IERC721Metadata ,IERC721Enumerable,Ownable {
    using Address for address;
    // Token name
    string private _name="Alteration x BurningMoon";
    // Token symbol
    string private _symbol="AlterationXBM";
    ...
    uint256 public MaxNFTCount=8000;
    ...
    constructor() {
        ...
        NFTPrototypes.push(NFTData("Prismatic Sacrifice Blade", "QmXLwBPvhmj8Y3MTYaJNB95doM8EGpsvuRAffGsuURuRni"));
        NFTPrototypes.push(NFTData("Prismatic BurningMoon Blade", "QmcGg1muKnKbuawxxFAXDK8YNWf8XUu9LbUYzwdcTsbkLV"));
        NFTPrototypes.push(NFTData("Prismatic Gabriel's Song", "QmdHRSjdc9GgCn7tKbRRWFucXstAcx3HSn8MMbHRHod8VU"));
        NFTPrototypes.push(NFTData("Prismatic Rainhard's Might", "QmP98Pc1kYrv4KXs8a6YBK3QGb6D7HGVkdkjmNMaGgPFKf"));
        NFTPrototypes.push(NFTData("Sacrifice Blade", "QmeQSNbAiCeZs6dbpHaUku8XUbX7Hw4u24GA3MA6GZneTb"));
        NFTPrototypes.push(NFTData("BurningMoon Blade", "QmR7bijvF1BBmQtZVXy8KW1U7ssdkwJ6x8FdV2CcQDYtu2"));
        NFTPrototypes.push(NFTData("Gabriel's Song", "QmbVgiN517vUc4yfg3bo5LFpJbhv4DrMdPGVE9rL8RUdLc"));
        NFTPrototypes.push(NFTData("Rainhard's Might", "QmahJvMPfUzrJVA4Ekkt7s8raH2Jvkmvp6B3j2hv4yKU6r"));
        NFTPrototypes.push(NFTData("Burning Moon Blade", "QmbyAowQNG5GSyitVQwY17Hy3vhqRtvqvuEKvaE9ZBb3Rh"));
        NFTPrototypes.push(NFTData("Silver Card Emerald", "QmYKXFg4ECUyQTHiT2TVe9UpRAEXGkAybfDV8LJx7f9cnm"));
        NFTPrototypes.push(NFTData("Rainhard's Bot", "QmTK9ZxLZtdJX2fKjvQYzsvxs74oPJngRe81fFKoCjANX9"));
        NFTPrototypes.push(NFTData("Gabriel's Bot", "QmUPLADDbZE8jRZEb4J2L34bT7Que6NbXfxxkoXujcL53p"));
        NFTPrototypes.push(NFTData("Fomo Lotto Bot", "Qmer26kYgrph19eWQtawr9Xg7DYRRLJdzqRrAmZMFxw4W8"));
        NFTPrototypes.push(NFTData("0x97 Bot", "QmT5uYjQQaEvm8C1CerDKy6z3ZsJ6e2W5ot1GqFLRefkKd"));
        
        NFTPrototypes.push(NFTData("White Flag", "QmTbcT7y2rutoMZBhaVueNd3LKfFuKJR5zYTik2kZt7jQi"));
        NFTPrototypes.push(NFTData("Starstruck", "QmYQvgprjXyF8mkjUg4FoXPdXnaU7JXbqeZ1xpjWZmUuMD"));
        NFTPrototypes.push(NFTData("Lucky Racer", "QmSbCckEC3ZirUy3o3g31sbhyEpgbhBMEWTmwAEe25mZy4"));
        NFTPrototypes.push(NFTData("777 Bot", "QmQxB91hYfX2ePRuYJyYD2FhM6DrMZSd8Z4VyyAZzGHsyj"));
        Minted=new uint[](18);
    ...
    function _distributeToken() private{
        ...
        uint256 InitialBM=BM.balanceOf(address(this));
        BM.Compound();
        uint256 newBM=BM.balanceOf(address(this))-InitialBM;

        profitPerShare += ((newBM * dividentMagnifier) / totalShares);
    ...
    function NFTAirdrop( uint count, address payable oldContract) public onlyOwner{
        AlterationXBurningMoonNFTs nft=AlterationXBurningMoonNFTs(oldContract);
    ...
    function _mint(address to, uint seed) private returns (uint){
        uint ID;
        uint count=NFTHolders.length+1;//+1 As Current NFT is unminted 
        if(count%(MaxNFTCount/4)==0)
        {
            //Milestone NFTs
            ID=count/(MaxNFTCount/4)-1;
        }
        else if(count>=MaxNFTCount-4)
        {
            //Last 4 NFTs are Legendary
            ID=count-(MaxNFTCount-8);
        }
        else{
        uint value=_prng(400, seed);
        if(value==0) ID=4;      //Sacrifice Blade
        else if(value==1)ID=5;  //Burning Moon Blade
        else if(value==2)ID=6;  //Gabriels Song
        else if(value==3)ID=7;  //Rainhards Might
        else if(value<24)ID=8;  //Metalic Sword 5%
        else if(value<64)ID=9;  //Magic Sword 10%
        else if(value<94)ID=10; //Minion 01BM
        else if(value<124)ID=11;//Minion 02BM
        else if(value<154)ID=12;//Minion 03BM
        else if(value<184)ID=13;//Minion 04BM
        else if(value<238)ID=14;//Minion 01
        else if(value<292)ID=15;//Minion 02
        else if(value<346)ID=16;//Minion 03
        else ID=17;             //Minion 04
        }
        Minted[ID]++;
