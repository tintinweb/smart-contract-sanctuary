...
contract MAXXXEDOUT is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "MAXXXEDOUT";
    string constant _symbol = "MAXXXED";
    ...
    uint256 public sMN = 125;
    uint256 public sMD = 100;
    uint256 sMm = sMN.div(sMD);
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        if(selling && sME){ return gMF(); }

        return tF;
    }

    function gMF() public view returns (uint256) {
        uint256 fI = tF.mul(sMm).sub(tF);
        return tF.add(fI);
    }
