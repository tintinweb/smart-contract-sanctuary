...
contract BEP20Token is Context, IBEP20, Ownable {
    ...
    function _transfer(
        ...
        require(
            ...
            "METAPRO: trading is not open yet"
        );
        ...
        require(
            ...
            "METAPRO: resulting balance more than the maximum allowed"
        );
    ...
    function deliver(uint256 amount) external {
        ...
        require(
            outgoing <= _balances[sender],
            "METAPRO: cannot deliver more than the owned balance"
        );
    ...
    function setDexAddr(address dex) external onlyOwner {
        require(dex != address(0), "METAPRO: cannot set DEX addr to 0x0");
        _dexAddr = dex;
    }
    ...
    function setDexRouterAddr(address router) external onlyOwner {
        require(router != address(0), "METAPRO: cannot set router addr to 0x0");
    ...
    function setTreasuryAddr(address treasury) external onlyOwner {
        require(treasury != address(0), "METAPRO: cannot set treasury to 0x0");
    ...
    function setPresaleContractAddr(address presale) external onlyOwner {
        require(
            presale != address(0),
            "METAPRO: cannot set presale addr to 0x0"
        );
