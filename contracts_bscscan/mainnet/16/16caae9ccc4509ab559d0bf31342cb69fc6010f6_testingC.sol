...
contract testingC is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x73Ab97FB64c0a78f794de6478Fb68cDFA77E0152);
    address payable public charityFeeAddress = payable(0x23f0F58bf3984891e69B69c990a3786353Ff3D6f);
    ...
    string private _name = "testingC";
    string private _symbol = "testingC";
    ...
    uint256 private _liquidityFee = _marketingFee.add(_crctLiquidityFee).add(_charityFee);
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _maxWallet = 3000000 * 10**9; //3% Max Wallet
    ...
    uint256 public _maxTxAmount = 1000000 * 10**9; //1% Tokens
    ...
    constructor() {
        ...
        _isExcludedFromFee[charityFeeAddress] = true;

        //exclude owner and this contract from max wallet
    ...
    function setmarketingFeePercent(uint256 Fee) external onlyOwner {
        _marketingFee = Fee;
        _liquidityFee = _crctLiquidityFee.add(_marketingFee).add(_charityFee);
    }
    ...
    function setcharityAddress(address payable _charityAddress)
        ...
        onlyOwner
    {
        charityFeeAddress = _charityAddress;
    }
    ...
    function _transfer(
        ...
        if(!_isExcludedFromMaxWallet[to]){
            require(balanceOf(to).add(amount) <= _maxWallet, "Exceeds Max Wallet Limit");
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        // first split contract into marketing, charity and liquidity fee
        uint256 liquidityAmount = contractTokenBalance;
        uint256 initialBalance = address(this).balance;

        if (_marketingFee > 0) {
            uint256 marketingAmount = contractTokenBalance.mul(_marketingFee).div(_liquidityFee.sub(_charityFee));
        ...
        if (_charityFee > 0) {
            ...
            uint256 charityAmount = contractTokenBalance.mul(_charityFee).div(_liquidityFee.sub(_marketingFee));
            
            liquidityAmount = contractTokenBalance.sub(charityAmount);
            swapTokensForEth(charityAmount);

            transferForCharityETH(charityFeeAddress,address(this).balance.sub(initialBalance));
    ...
    function transferFormarketingETH(address payable recipient, uint256 amount)
        private
    {
    ...
    function transferForCharityETH(address payable recipient, uint256 amount)
        private
    {
