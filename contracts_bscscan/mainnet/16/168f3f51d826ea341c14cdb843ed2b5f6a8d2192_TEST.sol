...
//
// $TEST proposes an innovative feature in its contract.
//
// DIVIDEND YIELD PAID IN ADA! With the auto-claim feature,
// simply hold $TEST and you'll receive ADA automatically in your wallet.
// 
// Hold TEST and get rewarded in ADA on every transaction!
//
//
// 📱 Telegram: 
// 🌎 Website: 
...
contract TEST is ERC20, Ownable {
    ...
    uint256 public totalFees = ADARewardsFee.add(liquidityFee).add(buybackFee).add(marketingFee);

    address payable public  buybackWallet = payable(0x492fC9479395BD3642878f218E79081f6083251a);
    address payable public marketingWallet = payable(0x4BAB7B5101f185F72DdCa250867C4e09Ec137676);
    ...
    function setADARewardsFee(uint256 value) external onlyOwner{
        ADARewardsFee = value;
        totalFees = ADARewardsFee.add(liquidityFee).add(buybackFee).add(marketingFee);
    }
    ...
    function blacklistMultipleAddresses(address[] memory accounts, bool value) external onlyOwner{
        for(uint256 i = 0; i < accounts.length; i++){
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && automatedMarketMakerPairs[to] && coolDownEnabled){
            uint256 timePassed = block.timestamp - _lastTrade[from];
            require(timePassed >= coolDownTime, "You must wait 30 seconds");
            _lastTrade[from] = block.timestamp;
        }

        if(!_isExcludedFromFees[to] && automatedMarketMakerPairs[from] && coolDownEnabled){
            uint256 timePassed = block.timestamp - _lastTrade[to];
            require(timePassed >= coolDownTime, "You must wait 30 seconds");
            _lastTrade[to] = block.timestamp;
        }

        if(!_isExcludedFromFees[to] && !_isExcludedFromFees[from] && !automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && coolDownEnabled){
        ...
        if( canSwap &&
            ...
            if(buybackFee > 0 || marketingFee > 0){
                uint256 feeTokens = contractTokenBalance.mul(buybackFee.add(marketingFee)).div(totalFees);
            ...
            if(ADARewardsFee > 0){
                uint256 sellTokens = contractTokenBalance.mul(ADARewardsFee).div(totalFees);
    ...
    function swapAndSendToFee(uint256 tokens) private{
        ...
        if(buybackFee > 0){
            buybackWallet.transfer(newBalance.div(totalFees).mul(buybackFee));
        }
        if(marketingFee > 0){
...
contract TESTDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()  DividendPayingToken("TEST_Dividen_Tracker", "TEST_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function excludeFromDividendsBool(address account, bool excluded) external onlyOwner {
        require(excludedFromDividends[account] != excluded, "TEST: Account is already the value of 'excluded'");
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TEST_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
