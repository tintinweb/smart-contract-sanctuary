...
contract CakeBoss is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   CakeBossDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("CakeBoss", "CakeBoss") {
        _ETHRewardsFee = 10;
        _liquidityFee = 4;
		_marketingFee = 2;
        ...
        maxSellTransactionAmount = 1500000000 * (10**18); //Max Sell 1.5%
        swapTokensAtAmount = 200000000 * (10**18);
        _maxWalletToken = 2000000000 * (10**18); // 2% of total supply


    	dividendTracker = new CakeBossDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x650fE2C3A3f6c97F1674d65fe4673Cf33C552C5c);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CakeBoss: The dividend tracker already has that address");

        CakeBossDividendTracker newDividendTracker = CakeBossDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CakeBoss: The new dividend tracker must be owned by the CakeBoss token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CakeBoss: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CakeBoss: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CakeBoss: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CakeBoss: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "CakeBoss: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CakeBoss: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CakeBoss: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForCAKE(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of CakeBoss
            path,
            recipient,
            block.timestamp
        );
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "CakeBoss: Marketing tax must be between 0 and 10");
        marketingFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "CakeBoss: Liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
    }
    
    function updateETHRewardsFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 15, "CakeBoss: ETH Rewards tax must be between 0 and 15");
        liquidityFee = newFee;
    }
...
contract CakeBossDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("CakeBoss_Dividend_Tracker", "CakeBoss_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "CakeBoss_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "CakeBoss_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CakeBoss contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "CakeBoss_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CakeBoss_Dividend_Tracker: Cannot update claimWait to same value");
