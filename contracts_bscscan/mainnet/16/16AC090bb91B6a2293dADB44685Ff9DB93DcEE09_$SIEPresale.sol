...
contract $SIEPresale is Ownable {
     ...
     */
    modifier isActive() {
        require(block.timestamp > startDate, "Presale: You are too early!");
        require(block.timestamp < endDate, "Presale: You are too late!");
        _;
    }
    ...
    modifier tokensUnlocked() {
        require(block.timestamp > unlockDate, "Presale: tokens not unlocked yet");
        _;
    }
    ...
    onlyOwner
    {
        require(isInitialized == false, "Presale: Presale has already been initialized.");
        require(_softCap < _hardCap, "Presale: softCap cannot be higher then hardCap");
        ...
        require(_minCommitment < _maxCommitment, "Presale: minCommitment cannot be higher then maxCommitment");
        require(_pricePancakeswap < _priceFairLaunch, "Presale: Pancakeswap price cannot be lower then Fair-Launch");
        require(_unlockDate > _endDate || _unlockDate == 0, "Presale: invalid unlockDate");
    ...
    */
    ...
    /**
    * @dev Tokens can be claimed for SIEs or not
    *
    */
    ...
    function purchaseTokens() external payable isActive  {
        ...
        require(!isClosed, "Presale: sale closed");
        require(amount >= minCommitment, "Presale: amount to low");
    ...
    function closeSale() external tokensUnlocked {
        ...
        require(block.timestamp > endDate || tokensSold == hardCap, "Presale: endDate not passed or hardcap not reached");
    ...
    function claimTokens() external tokensUnlocked {
        require(isClosed, "Presale: sale not closed");
        require(canClaimTokens, "Presale: Claiming is not allowed yet!");
    ...
    function releaseTokens() external tokensUnlocked {
        require(!isClosed, "Presale: cannot release tokens for closed sale");
