...
abstract contract StratX2 is Ownable, ReentrancyGuard, Pausable {
        ...
        returns (uint256)
    {
        ...
        if (wantLockedTotal > 0 && sharesTotal > 0) {
            ...
            sharesTotal = sharesTotal.add(sharesAdded)
                .mul(entranceFeeFactor)
                .div(entranceFeeFactorMax);
            user.shares = user.shares.add(sharesTotal);
        }
        ...
        if (isAquaComp) {
            _farm();
            // This is modified to take care of only required address
    ...
    function _farm() internal virtual {
        ...
        if (isCAKEStaking) {
            ...
            stratRewardPerShare = cakeBal.mul(1e12).div(sharesTotal);
        }
    }
        ...
        returns (uint256)
    {
        ...
        if (isAquaComp) {
            _unfarm(_wantAmt);
            uint256 pendingProfit = ((user.shares).mul(stratRewardPerShare).sub(user.earnRewardDebt)).div(1e12);
            
            if(pendingProfit > 0)
            earnAquaProfitsOnWithdraw();
    ...
    function earnAquaProfitsOnWithdraw() internal  {
    ...
    function userPendingAquaProfit(address userAddress) public view virtual returns (uint256 pendingProfitInCake,uint256 pendingProfitInAqua){
        ...
        if(stratRewardPerShare > 0 && user.shares > 0){
            uint cakeBal = (IERC20(earnedAddress).balanceOf(address(this)).add(IPancakeswapFarm(farmContractAddress).pendingCake(pid,address(this))));
            
            uint tempShare = 0;
            if(cakeBal > 0 && sharesTotal > 0)
            tempShare = (cakeBal.mul(1e12).div(sharesTotal));
            else
    ...
    function earnAquaProfits() public whenNotPaused  {
        address caller = tx.origin;
        UserInfo storage user = userInfo[caller];
        require(user.shares != 0,"!!User has zero shares");
