...
// Root file: contracts/TestLotoLoto.sol
...
contract TestLotoLoto is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 internal constant MIN_BET = 0.1 ether;
    ...
    function placeBet(bytes22 _params) external payable nonReentrant whenNotPaused {
        ...
        require(msg.value >= MIN_BET && msg.value <= MAX_BET, "TestLoto Loto: Bet amount should be greater or equal than minimal amount");
    ...
    function withdrawToken(address _token, address _recipient) external onlyOwner {
        require(_recipient != address(0x0), "BNB Storage: address is zero");
    ...
    function withdraw() external onlyOwner whenPaused {
        _deliverFunds(msg.sender, address(this).balance, "TestLoto Loto: Can't send BNB");
    }
    ...
    function _processBet(address _gamer, bytes16 _id) internal {
