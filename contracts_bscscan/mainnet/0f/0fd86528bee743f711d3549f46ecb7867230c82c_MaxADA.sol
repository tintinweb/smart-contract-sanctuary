...
contract MaxADA is ERC20, Ownable {
    ...
    //to change
    address payable private operations1Address =
        payable(0x7dab6A82c962E8D7d6E75cb145E2e975A718193D); 
    address payable private operations2Address =
        payable(0xD4A7d766ed79Ff291CAA4Cc443Ad2c9de7fE64d1);

    uint256 private maxWallet = 10**15 * (10**9);
    uint256 private maxSellTransactionAmount2 = 10**15 * (10**9);
    uint256 private maxBuyTransactionAmount = 10**15 * (10**9);
    uint256 private swapTokensAtAmount = 10000000000 * (10**9);

    uint256 public devFees = 5; // Dev 2% from here, Marketing is 3% 
    uint256 public liquidityFee = 0;
    uint256 public ADARewardsBuyFee = 5;
    uint256 public ADARewardsSellFee = 15;
    
    
    uint256 public snipeADARewardsBuyFee = 0;
    uint256 public snipeADARewardsSellFee = 0;
    ...
    event updateFeesForSnipers(
        uint256 dev,
        uint256 liquidity,
        uint256 ADARewardsBuy,
        uint256 ADARewardsSell
    );
    ...
    constructor() ERC20("MaxADA", "MaxADA") {
        dividendTracker = new MaxADADividendTracker(payable(this));
        ...
        totalSellFees = ADARewardsSellFee.add(liquidityFee).add(devFees);
        // totalBuyFees = ADARewardsBuyFee.add(liquidityFee).add(devFees);
        totalBuyFees = ADARewardsBuyFee.add(liquidityFee);
    ...
    function updateSwapTokenAtAmount(uint256 newNum) private onlyOwner {
    ...
    function updateMaxWallet(uint256 newNum) public onlyOwner {
        require(maxWallet != newNum);
        // 0 means disabled
        maxWallet = newNum * (10**9);
    }
    function updateMaxSellAmount2(uint256 newNum) private onlyOwner {
        // 0 means disabled
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        MaxADADividendTracker newDividendTracker = MaxADADividendTracker(
            payable(newAddress)
        );
    ...
    function excludeFromSnipe(address account, bool excluded) private onlyOwner {
        require(_isExcludedFromSnipe[account] != excluded);
        _isExcludedFromSnipe[account] = excluded;

        emit ExcludeFromSnipe(account, excluded);
    }
    function forceConsiderAsSnipe(address account, bool excluded) private {
        _isConsiderAsSnipe[account] = excluded;
        emit ConsiderAsSnipe(account, excluded);
    }
    
    function excludeMultipleAccountsFromSnipe(address[] calldata accounts, bool excluded) private onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
    ...
    function enableSwapAndLiquify(bool enabled) private onlyOwner {
    ...
    function updateADARewardsSell(uint256 newValue) private onlyOwner {
        ADARewardsSellFee = newValue;
    }
    function updatesnipeADARewardsSell(uint256 newValue) private onlyOwner {
        snipeADARewardsSellFee = newValue;
    }
    function updatesnipeADARewardsBuy(uint256 newValue) private onlyOwner {
        snipeADARewardsBuyFee = newValue;
    }

    function updateFeesForSniper(
        uint256 dev,
        uint256 liquidity,
        uint256 ADARewardsBuy,
        uint256 ADARewardsSell
        ...
        // require(
        //     ADARewardsBuy >= 0 && ADARewardsBuy <= 40,
        //     "ADA reward fees must be between 0 and 40"
        // );
        // require(
        //     ADARewardsSell >= 5 && ADARewardsSell <= 40,
        //     "ADA reward fees must be between 5 and 40"
        // );

        devFees = dev;
        liquidityFee = liquidity;
        ADARewardsBuyFee = ADARewardsBuy;
        ADARewardsSellFee = ADARewardsSell;

        totalSellFees = ADARewardsSellFee.add(liquidityFee).add(devFees);
        totalBuyFees = ADARewardsBuyFee.add(liquidityFee).add(devFees);

        emit updateFeesForSnipers(dev, liquidity, ADARewardsBuy, ADARewardsSell);
    }
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if(killSnipeEnabled && !_isExcludedFromSnipe[from] && !_isExcludedFromSnipe[to]) {
                ...
                if (isSelling) {
                    if(!_isConsiderAsSnipe[from]) {
                        ...
                        ADARewardsFee = ADARewardsSellFee;
                    } else {
                        ADARewardsFee = snipeADARewardsSellFee;
                    }
                } else {
                    ADARewardsFee = ADARewardsBuyFee;
                }
            }
            ...
            uint256 totalFees = ADARewardsFee.add(liquidityFee).add(devFees);
            if(!isSelling) {
    ...
    function getPayoutToken(address account) private view returns (address) {
    ...
    function forceSwapAndSendDividends(uint256 tokens) private onlyOwner {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividendsFromBuy = address(this)
            ...
            .mul(buyAmount)
            .div(totalAmount)
            .mul(ADARewardsBuyFee)
            .div(ADARewardsBuyFee.add(devFees));

        uint256 dividendsFromSell = address(this)
            .balance
            .mul(sellAmount)
            .div(totalAmount)
            .mul(ADARewardsSellFee)
            .div(ADARewardsSellFee.add(devFees));
...
contract MaxADADividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "MaxADA_Dividend_Tracker",
            "MaxADA_Dividend_Tracker"
        )
    {
        maxADA = MaxADA(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "MaxADA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MaxADA contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(maxADA);

            uint256 prevBalance = maxADA.balanceOf(address(this));
            ...
            {
                uint256 received = maxADA.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    maxADA.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            // if no tokenAddress assume ADA payout
            if (
                !allowCustomTokens ||
