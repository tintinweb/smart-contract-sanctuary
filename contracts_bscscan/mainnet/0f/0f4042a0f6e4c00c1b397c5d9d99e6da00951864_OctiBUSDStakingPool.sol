 ...
 */
contract OctiBUSDStakingPool is
    Ownable
{
    ...
    ERC20 BUSD  = ERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // OCTI Token mainnet
    ERC20 OCTI  = ERC20(0x6c1dE9907263F0c12261d88b65cA18F31163F29D); // OCTI Token mainnet
    
    function getOctiDecimals() public view returns(uint256){
        return ERC20(OCTI).decimals();
    }
    ...
    function stake(uint amount) public {
        ...
        ERC20(OCTI).transferFrom(msg.sender, address(this),amount);
        uint256 userStake = stakeOfUser[msg.sender];
        stakeOfUser[msg.sender] = userStake + amount;
        
        if(BUSD.balanceOf(address(this))>0){
            octiPriceForPool = (OCTI.balanceOf(address(this))/BUSD.balanceOf(address(this)));
        }
    ...
    function unstake(uint amount) public {
        ...
        uint price;
        if(BUSD.balanceOf(address(this))>0){
            price = (OCTI.balanceOf(address(this))/BUSD.balanceOf(address(this)));
        }
        ERC20(OCTI).transfer(msg.sender,amount);
        uint256 userStake = stakeOfUser[msg.sender];
        if(userStake == amount){
        ...
        if(BUSD.balanceOf(address(this))>0 && price>0){
    ...
    function getOctiBalance() public view returns(uint) {
        return OCTI.balanceOf(address(this));
    }
