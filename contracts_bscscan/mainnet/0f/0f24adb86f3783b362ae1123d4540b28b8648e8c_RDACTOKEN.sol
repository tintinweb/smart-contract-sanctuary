...
contract RDACDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("RDAC_Dividend_Tracker", "RDAC_Dividend_Tracker") {
        claimWait = 43200;
        minimumTokenBalanceForDividends = 100000 * (10**18); //must hold 100,000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "RDAC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "RDAC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RDAC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 60 && newClaimWait <= 86400, "RDAC_Dividend_Tracker: claimWait must be updated to between 1 min and 24 hours");	
        require(newClaimWait != claimWait, "RDAC_Dividend_Tracker: Cannot update claimWait to same value");
...
contract RDACTOKEN is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    RDACDividendTracker public dividendTracker;
    ...
    function setMarktingFee(uint256 newFee) external onlyOwner{
        marketingFee = newFee;
    }
    ...
    constructor() ERC20("RDAC TOKEN", "RDAC") {
        BNBRewardsFee = 2;
        liquidityFee = 3;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 5;
        marketingWallet = payable(0x187edbc0f41063747639Bd0bc2d6076E2d11D3Bf);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new RDACDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "RDAC: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "RDAC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "RDAC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "RDAC: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "RDAC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "RDAC: Cannot update gasForProcessing to same value");
