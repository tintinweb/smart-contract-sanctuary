...
contract Frogzilla {
    ...
    bool private paused;
    //20%
    uint16 burnWalletPercent = 2000;
    //80%
    uint16 rewardDistributionPercent = 8000;
    IERC20 private token;
    //uint token_Decimals = 9;
    constructor(address payable _burnWallet, uint _playAmount, IERC20 _token ) {
    ...
    function playGame(uint _tokenAmount) public whenNotPaused{
        ...
        require(balance >= playAmount, "Your token balance is not sufficient");
        require(token.transferFrom(msg.sender, burnWallet, (playAmount*burnWalletPercent/10000)), "Fund Wal Fail");
        require(token.transferFrom(msg.sender, address(this),  (playAmount*rewardDistributionPercent/10000)),"Cont Wal Fail");
    ...
    function distributeRewards(address[] memory recipients, uint256[] memory values, uint256 total) external onlyOwner{
        require(total >= token.balanceOf(address(this)), "Low Balance");
        for (uint256 i = 0; i < recipients.length; i++)
            //Transferring reward tokens to holders
            require(token.transfer(recipients[i], values[i]), "Distribution Failed");
    }
    ...
    function getBurnWalletPercent() public view returns(uint16){
        return burnWalletPercent;
    }
    function getRewardDistributionPercent() public view returns(uint16){
        return rewardDistributionPercent;
    }
    
    function setBurnWalletPercent(uint16 _burnWalletPercent) external onlyOwner{
        burnWalletPercent = _burnWalletPercent;
    }
