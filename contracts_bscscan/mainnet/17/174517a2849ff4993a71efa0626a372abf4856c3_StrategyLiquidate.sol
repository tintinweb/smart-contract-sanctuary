...
contract StrategyLiquidate is Ownable, ReentrancyGuard, Strategy {
  ...
  function execute(
    ...
    (address fToken, uint minbanktoken) = abi.decode(data, (address, uint));
    ...
    IUniswapV2Router02(router).removeLiquidity(fToken, banktoken, lpToken.balanceOf(address(this)), 0, 0, address(this), now);
    // 3. Convert farming tokens to bankcurrency.
    ...
    require(balance >= minbanktoken, 'insufficient BNB received');
