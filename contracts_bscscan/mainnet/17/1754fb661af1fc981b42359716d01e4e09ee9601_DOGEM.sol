...
contract DOGEM is Context, IBEP20, Ownable {
    ...
    uint256 public _reflect = 2; // Consists of Reflection Tax (2%)
    uint256 private _previousreflect = _reflect;
    
    uint256 public _BLD = 13; // Consists of Buyback (7%) + Liquidity (2%) + Dividends (4%) Tax - (BLD)
    uint256 private _previousBLD = _BLD;
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tFee = calculatereflect(tAmount);
        uint256 tLiquidity = calculateBLD(tAmount);
    ...
    function calculatereflect(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_reflect).div(
            10**2
        );
    }
    
    function calculateBLD(uint256 _amount) private view returns (uint256) {
        if(launchedAt.add(3) >= block.number){
            return _amount.mul(_BLD.mul(6)).div(
                10**2
            );
        } else {
            return _amount.mul(_BLD).div(
                10**2
            );
        }
    ...
    function setreflectPercent(uint256 reflect) external onlyOwner() {
        _reflect = reflect;
    }
        
    function setBLDPercent(uint256 BLD) external onlyOwner {
        _BLD = BLD;
    }
