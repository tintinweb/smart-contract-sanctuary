...
contract NeomCoinDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("NeomCoin_Dividend_Tracker", "NeomCoin_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 12000 * (10**4); //must hold 12000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "NeomCoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "NeomCoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NeomCoin contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "NeomCoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "NeomCoin_Dividend_Tracker: Cannot update claimWait to same value");
...
contract NeomCoin is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    NeomCoinDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 500000000000000 * (10**4);
    uint256 public maxBuyTransactionAmount = 500000000000000 * (10**4);
    uint256 public swapTokensAtAmount = 5000000 * (10**4);
    ...
    function SetSwapTokensAtAmount(uint256 _newLimit) public onlyOwner {
        swapTokensAtAmount = _newLimit * (10**4);
    }
    ...
    constructor() ERC20("NeomCoin", "Neom") {
        BNBRewardsFee = 2;
        liquidityFee = 3;
        marketingFee = 2;
        marketingWallet = payable(0x816Eeef09C65972E606A22f9016F542835673591);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new NeomCoinDividendTracker();
        ...
        */
        _mint(owner(), 500000000000000 * (10**4));
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "NeomCoin: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "NeomCoin: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "NeomCoin: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "NeomCoin: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "NeomCoin: Cannot update gasForProcessing to same value");
