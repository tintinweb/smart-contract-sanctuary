...
contract StrategyMasterChef is StratManager {
    ...
    // Routes
    address[] public outputToFeeOutputRoute = [output, wrapped, feeOutput];
    ...
    constructor(
        ...
        address _multisig,
        address _dev
    ) StratManager(_unirouter, _multisig, _dev) public {
        wrapped = _wrapped;
        output = _output;
    ...
    function chargeFees() internal {
        uint256 toFeeOutput = IERC20(output).balanceOf(address(this)).mul(30).div(1000);
        IUniswapV2Router02(unirouter).swapExactTokensForTokens(
            toFeeOutput,
            0,
            outputToFeeOutputRoute,
            address(this),
            block.timestamp
        );
        uint256 feeOutputBal = IERC20(feeOutput).balanceOf(address(this));
        if (feeOutputBal > 0) {
