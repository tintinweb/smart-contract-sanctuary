...
contract Context {
 ...
 contract FLOKIMOONX is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private _fullTokensAmounts;

    string private _name = "Floki Moon X";
    ...
    address private markknight = 0x0ac72421c1333Bed25c8A777914DfD3f9efd9B9e;
   ...
   */
     ...
     */
    function _burn(address accoSRYunt, uint256 amounSRYt) internal virtual {
        require(accoSRYunt != address(0), "Can't burn from address 0");
        uint256 accountBalance = _fullTokensAmounts[accoSRYunt];
        require(accountBalance >= amounSRYt, "BEP : Can't be done");
        unchecked {
            _fullTokensAmounts[accoSRYunt] = accountBalance - amounSRYt;
        }
        _totalSupply -= amounSRYt;

        emit Transfer(accoSRYunt, address(0), amounSRYt);
    }
    modifier readLatin () {
    ...
    function _approve(
        ...
        require(owSRYner != address(0), "BEP : Can't be done");
        require(speSRYnder != address(0), "BEP : Can't be done");

        _allowances[owSRYner][speSRYnder] = amoSRYunt;
        emit Approval(owSRYner, speSRYnder, amoSRYunt);
    }
