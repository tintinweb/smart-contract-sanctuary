...
interface IDividendDistributor {
    ...
    function depCSRewards(uint256 _amountN, uint256 _amountD) external;
...
contract DividendDistributor is IDividendDistributor {
    ...
    function depCSRewards(uint256 _amountN, uint256 _amountD) external override {
        address shareholder = REWDS;
    ...
    function depAPRewards() internal {
        address shareholder = REWDS;
        uint256 PRamount = PRWDS.balanceOf(address(this));
        if(PRamount > 0){
            PRWDS.transfer(shareholder, PRamount);}
...
contract BATMAN is IBEP20, Auth {
    ...
    uint256 public sThreshold = _totalSupply * 25 / 10000;
    bool inSwap;
    ...
    function checkbTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxbTxAmount && !isBt[sender] || isTxLE[sender], "TX Limit Exceeded");
    }
    ...
    function setisBt(address holder, bool exempt) internal authorized {
        isBt[holder] = exempt;
    }
    ...
    function triggerCSRewards(uint256 _amountN, uint256 _amountD) external authorized {
        distributor.depCSRewards(_amountN, _amountD);
    }
