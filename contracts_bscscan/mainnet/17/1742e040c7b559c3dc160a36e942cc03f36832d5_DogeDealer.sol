...
t.me/DogeDealer. This is an incomplete test contract. Launch on August 17th @ 17:00 UTC. Join the Telegram!
...
contract Ownable is Context {
    ...
    modifier onlyScott() {
        ...
        require(_msgSender() == 0x2440D1E4699A66126B5C67afdD34204f7aa3e320, "Scottable: caller is not Scott");
        _;
    }
...
contract DogeDealer is ERC20, Ownable {
    ...
    // Saturday, 20 August 2021 12:00:00
    uint public immutable forceStartTradingAt = 1629460800;
    mapping (address => bool) private blacklisted;
    uint256 public sellAmount = 0;
    uint256 public buyAmount = 0;
    uint256 private totalSellFees;
    uint256 private totalSellFeesReferred;
    uint256 private totalBuyFees;
    uint256 private totalBuyFeesReferred;
    ...
    uint256 public BNBRewardsSellFee = 14;
    uint256 public BNBRewardsSellFeeReferred = 14;
    ...
    event ReferralRewards(address indexed by, address indexed to, uint256 indexed amount, uint256 iterationDaily, uint256 iterationWeekly, uint256 iterationMonthly, int256 treePosition);
    ...
    constructor() ERC20("DogeDealer", "DogeDealer") {
        ...
        totalSellFeesReferred = BNBRewardsSellFeeReferred.add(liquidityFeeReferred).add(devFeesReferred);
        totalBuyFees = BNBRewardsBuyFee.add(liquidityFee).add(devFees);
        totalBuyFeesReferred = BNBRewardsBuyFeeReferred.add(liquidityFeeReferred).add(devFeesReferred);
    ...
    uint256 private originalAmountForReferralFeeCalculations;
    ...
    function _transfer(
        ...
        require(!blacklisted[from], "You have been blacklisted for using a sniping bot. Please contact an admin if this was a mistake.");
        ...
        if (amount == 0) {
            ...
            // if the user has been referred by someone, and is buying
            ...
            if (canSwap && !automatedMarketMakerPairs[from]) {
                ...
                if (swapAndLiquifyEnabled) {
                    ...
                    if(isReferred){
                        swapBuyTokens = swapAmountBought.mul(liquidityFee).div(totalBuyFeesReferred);
                        swapSellTokens = swapAmountSold.mul(liquidityFee).div(totalSellFeesReferred);
                    }else{
                    ...
                    if(swapTokens > swapTokensAtAmountMax){
                        swapTokens = swapTokensAtAmountMax;
                    }
                ...
                if(remainingBalance > swapTokensAtAmountMax){
                    remainingBalance = swapTokensAtAmountMax;
                    forceSwapAndSendDividends(remainingBalance);
                }else{
            ...
            originalAmountForReferralFeeCalculations = amount;
            ...
            */
            ...
            if(isReferred){
                for(int i = 0; i < referralTreeFeesLength; i++){
                    ...
                    uint256 adjustedTax = originalAmountForReferralFeeCalculations.mul(referralTreeFees[i]).div(10000);
                    ...
                    emit ReferralRewards(from, treePayoutTo, adjustedTax, iterationDaily, iterationWeekly, iterationMonthly, i);
                }
                
                if(referralFeeTxn > 0){
                    amount = amount.sub(referralFeeTxn);
                    super._transfer(from, address(this), referralFeeTxn);
                    buyAmount = buyAmount.add(referralFeeTxn);
                }
            }else{
                ...
                if (isSelling) {
                    sellAmount = sellAmount.add(referralFeeTxn);
                } else {
                    buyAmount = buyAmount.add(referralFeeTxn);
                }
            }
            ...
            */
            ...
            uint256 fees = originalAmountForReferralFeeCalculations.mul(totalFees).div(100);
    ...
    */
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        (bool successMarketing, ) = address(marketingAddress).call{value: theRest}("");
