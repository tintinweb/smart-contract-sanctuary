...
contract EthPiptSwap is ProgressiveFee {
  ...
  function calcNeedEthToPoolOut(uint256 _poolAmountOut, uint256 _slippage) public view returns (uint256) {
    ...
    for (uint256 i = 0; i < len; i++) {
      ...
      if (wethPairAddress == address(0)) {
        ...
        totalEthSwap = getAmountInForUniswapValue(
          _uniswapPairFor(wethAddress, usdcAddress),
          weth,
          usdAmount,
          true
  ...
  function calcEthFee(uint256 _ethAmount, uint256 _wrapperFee) public view returns (uint256 ethFee, uint256 ethAfterFee) {
  ...
  function _setUniswapSettingAndPrepareToken(address _targetToken, address _token, address _pair) internal {
  ...
  function _swapForTokenIn(IERC20 _tokenIn, address _erc20, uint256 _erc20Out) internal returns (uint256 amountIn) {
  ...
  function _swapToken(address _token, IUniswapV2Pair _tokenPair, uint256 _amountIn, uint256 _amountOut) internal {
...
contract Erc20PiptSwap is EthPiptSwap {
  ...
  {
    uint256 ethAmount = getAmountOutForUniswapValue(
    ...
    )
  {
    ...
    (
      ...
      totalEthOut,
      poolAmountFee
    ) = calcSwapPiptToEthInputs(_poolAmountIn, _tokens);
