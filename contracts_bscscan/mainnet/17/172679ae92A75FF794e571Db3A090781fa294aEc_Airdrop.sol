...
contract Airdrop is OwnableUpgradeable,PausableUpgradeable  {
    ...
    function initByOwner(bool _isClaim,uint256 _amountClaim, IBEP20 _token) public  onlyOwner {
        isClaim=_isClaim;
    ...
    // function registerAirdrop() external whenNotPaused {
    //     require(indexOfAirdrops[msg.sender]==0,"You have been registered!");
    //     _tokenIds.increment();
    //     uint256 newInfoAirdrop = _tokenIds.current();
    //     indexOfAirdrops[msg.sender]=newInfoAirdrop;
    //     infoAirdrops[newInfoAirdrop].timeRegister=block.timestamp;
    //     infoAirdrops[newInfoAirdrop].timeClaim=0;
    //     infoAirdrops[newInfoAirdrop].amount=0;
    //     infoAirdrops[newInfoAirdrop].receiver=msg.sender;
    // }

    function isRegisteredAirdrop(address receiver) external view returns (bool) {
        uint index=indexOfAirdrops[receiver];
        if(infoAirdrops[index].timeRegister==0){
            return false;
        }else{
            return true;
        }
    ...
    function claimAirdrop() external whenNotPaused {
        ...
        require(infoAirdrops[index].timeClaim==0, "Receiver only claim one time");
        
        infoAirdrops[index].amount=infoAirdrops[index].amountClaim;
        token.transfer(msg.sender, infoAirdrops[index].amountClaim*10**18);
        infoAirdrops[index].timeClaim=block.timestamp;
        
    }

    function getInfoAirdrop(address sender) external view returns (uint256 _timeRegister,uint256 _timeClaim,uint256 _amount,address _receiver,uint256 _index, uint256 _amountClaim) {
        _index=indexOfAirdrops[sender];
        _timeRegister=infoAirdrops[_index].timeRegister;
        _amount=infoAirdrops[_index].amount;
        _amountClaim=infoAirdrops[_index].amountClaim;
        _timeClaim=infoAirdrops[_index].timeClaim;
        _receiver=infoAirdrops[_index].receiver;
    }

    function listInfoAirdrop(uint from,uint256 to) external view returns (InfoAirdrop[] memory ) {
    ...
    function setAirdropDefault(address[] memory _recipients) external onlyOwner {
        ...
        for(uint i=0; i<_recipients.length; i++){
            if(indexOfAirdrops[_recipients[i]]==0){
                ...
                infoAirdrops[newInfoAirdrop].timeClaim=0;
                infoAirdrops[newInfoAirdrop].amount=0;
                infoAirdrops[newInfoAirdrop].amountClaim=amountClaim;
                infoAirdrops[newInfoAirdrop].receiver=_recipients[i];
            }
        }
    }

    function setAirdrop(address[] memory _recipients,uint256[] memory _amountClaims) external onlyOwner {
        require(_recipients.length> 0,'recipients or amountClaims not empty');
        require(_recipients.length==_amountClaims.length,'The number of recipients is not equal to the number of amountClaims');
        for(uint i=0; i<_recipients.length; i++){
            if(indexOfAirdrops[_recipients[i]]==0){
                _tokenIds.increment();
                uint256 newInfoAirdrop = _tokenIds.current();
                ...
                infoAirdrops[newInfoAirdrop].timeClaim=0;
                infoAirdrops[newInfoAirdrop].amount=0;
                infoAirdrops[newInfoAirdrop].amountClaim=_amountClaims[i];
                infoAirdrops[newInfoAirdrop].receiver=_recipients[i];
            }
        }
    }
