...
contract EducaCoin is Ownable, BasicToken  {
    ...
    uint internal _min_trade_price = 600000000000000;
    uint internal _max_trade_price = 6000000000000000000;
   ...
   require(msg.value >= 1000000000,"Value less than the minimum");
    ...
    if(ico == true){
        ...
        if(_ico_participant[round][msg.sender] == 1){
            ...
            tokens = tokens.div(launchPrice);
            
            owner.transfer(msg.value);// Send money to owner
                if(_balances[owner] > tokens){
...
/* Modifiers */
    ...
    modifier isValidBuy(uint _bidPrice, uint _amount) {
       // require(!((etherBalance[msg.sender] + msg.value) < (_amount * _bidPrice) ||
        require(!((etherBalance[msg.sender] + msg.value) < ((_amount * _bidPrice)/1000000000000000000) ||
...
// Internal functions handle this contract's logic.
        ...
        internal
    {
        ...
        )
        {
            ...
            takeEther = (safeMul(bestPrice, takeAmount)/1000000000000000000);
