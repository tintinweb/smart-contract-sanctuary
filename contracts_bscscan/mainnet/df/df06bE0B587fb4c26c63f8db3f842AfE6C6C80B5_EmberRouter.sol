...
// File: contracts\interfaces\IEmberRouter01.sol
...
// File: contracts\interfaces\IEmberRouter02.sol
...
// File: contracts\interfaces\IEmberFactory.sol
...
// File: contracts\interfaces\IEmberPair.sol
...
// File: contracts\libraries\EmberLibrary.sol
...
library EmberLibrary {
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IEmberPair(pairFor(factory, tokenA, tokenB)).getReserves();
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        ...
        require(reserveA > 0 && reserveB > 0, 'EmberLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        ...
        require(reserveIn > 0 && reserveOut > 0, 'EmberLibrary: INSUFFICIENT_LIQUIDITY');
...
// File: contracts\EmberRouter.sol
...
contract EmberRouter is IEmberRouter02 {
    using SafeMath for uint;
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = EmberLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = EmberLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'EmberRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = EmberLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'EmberRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = IEmberPair(pair).burn(to);
    ...
    function removeLiquidityWithPermit(
        ...
        IEmberPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? EmberLibrary.pairFor(factory, output, path[i + 2]) : _to;
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'EmberRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, EmberLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'EmberRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, EmberLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
        ...
        returns (uint[] memory amounts)
    {
        ...
        assert(IWETH(WETH).transfer(EmberLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = EmberLibrary.sortTokens(input, output);
            IEmberPair pair = IEmberPair(EmberLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = EmberLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, EmberLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(EmberLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'EmberRouter: INSUFFICIENT_OUTPUT_AMOUNT');
