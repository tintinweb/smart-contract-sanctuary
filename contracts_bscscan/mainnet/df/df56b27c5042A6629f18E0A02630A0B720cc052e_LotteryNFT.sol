...
contract LotteryNFT is ERC721, Ownable {
        ...
        returns (uint256)
    {
        ...
        lotteryId[newItemId] = _lotteryId;
...
contract Lottery is LotteryOwnable, Initializable {
    ...
    // issueId => [totalAmount, firstMatchAmount, secondMatchingAmount, thirdMatchingAmount, fourthMatchingAmount, fifthMatchingAmount, sixMatchingAmount]
    ...
    function reset() external onlyAdmin {
        ...
        if(getMatchingRewardAmount(lotteryId-1, 6) == 0) {
            uint256 amount = getTotalRewards(lotteryId-1).mul(allocation[0]).div(100);
    ...
    function drawing(uint256 _externalRandomNumber) external onlyAdmin {
        ...
        historyAmount[lotteryId] = calculateMatchingRewardAmount();
        drawingPhase = false;
        emit Drawing(lotteryId, winningNumbers);
    }

    function internalBuy(uint256 _price, uint8[6] memory _numbers) internal {
        require (!drawed(), 'drawed, can not buy now');
        for (uint i = 0; i < 6; i++) {
            require (_numbers[i] <= maxNumber, 'exceed the maximum');
        }
        uint256 tokenId = lotteryNFT.newLotteryItem(address(this), _numbers, _price, lotteryId);
    ...
    function buy(uint256 _price, uint8[6] memory _numbers) external {
        ...
        for (uint i = 0; i < keyLengthForEachBuy; i++) {
            userBuyAmountSum[lotteryId][userNumberIndex[i]] = userBuyAmountSum[lotteryId][userNumberIndex[i]].add(_price);
        }
    ...
    function  multiBuy(uint256 _price, uint8[6][] memory _numbers) external {
        ...
        for (uint i = 0; i < _numbers.length; i++) {
            ...
            for (uint k = 0; k < keyLengthForEachBuy; k++) {
                userBuyAmountSum[lotteryId][numberIndexKey[k]]=userBuyAmountSum[lotteryId][numberIndexKey[k]].add(_price);
            }
        }
    ...
    function generateNumberIndexKey(uint8[6] memory number) public pure returns (uint64[keyLengthForEachBuy] memory) {
        uint64[6] memory tempNumber;
        tempNumber[0]=uint64(number[0]);
        tempNumber[1]=uint64(number[1]);
        tempNumber[2]=uint64(number[2]);
        tempNumber[3]=uint64(number[3]);
        tempNumber[4]=uint64(number[4]); 
        tempNumber[5]=uint64(number[5]);
    ...
    function calculateMatchingRewardAmount() internal view returns (uint256[7] memory) {
        uint64[keyLengthForEachBuy] memory numberIndexKey = generateNumberIndexKey(winningNumbers);

        uint256 totalAmount1 = userBuyAmountSum[lotteryId][numberIndexKey[0]];

        uint256 sumForTotalAmount2 = userBuyAmountSum[lotteryId][numberIndexKey[1]];
        sumForTotalAmount2 = sumForTotalAmount2.add(userBuyAmountSum[lotteryId][numberIndexKey[2]]);
        sumForTotalAmount2 = sumForTotalAmount2.add(userBuyAmountSum[lotteryId][numberIndexKey[3]]);
        sumForTotalAmount2 = sumForTotalAmount2.add(userBuyAmountSum[lotteryId][numberIndexKey[4]]);
        sumForTotalAmount2 = sumForTotalAmount2.add(userBuyAmountSum[lotteryId][numberIndexKey[5]]);
        sumForTotalAmount2 = sumForTotalAmount2.add(userBuyAmountSum[lotteryId][numberIndexKey[6]]);

        uint256 totalAmount2 = sumForTotalAmount2.sub(totalAmount1.mul(6));

        uint256 sumForTotalAmount3 = userBuyAmountSum[lotteryId][numberIndexKey[5]];
        sumForTotalAmount3 = sumForTotalAmount3.add(userBuyAmountSum[lotteryId][numberIndexKey[6]]);
        sumForTotalAmount3 = sumForTotalAmount3.add(userBuyAmountSum[lotteryId][numberIndexKey[7]]);
        sumForTotalAmount3 = sumForTotalAmount3.add(userBuyAmountSum[lotteryId][numberIndexKey[8]]);
        sumForTotalAmount3 = sumForTotalAmount3.add(userBuyAmountSum[lotteryId][numberIndexKey[9]]);
        sumForTotalAmount3 = sumForTotalAmount3.add(userBuyAmountSum[lotteryId][numberIndexKey[10]]);

        uint256 totalAmount3 = sumForTotalAmount3.add(totalAmount1.mul(6)).sub(sumForTotalAmount2.mul(3));
        uint256 totalAmount4 = sumForTotalAmount3.add(totalAmount1.mul(6)).sub(sumForTotalAmount2.mul(3));
        uint256 totalAmount5 = sumForTotalAmount3.add(totalAmount1.mul(6)).sub(sumForTotalAmount2.mul(3));
        uint256 totalAmount6 = sumForTotalAmount3.add(totalAmount1.mul(6)).sub(sumForTotalAmount2.mul(3));

        return [totalAmount, totalAmount1, totalAmount2, totalAmount3, totalAmount4, totalAmount5, totalAmount6];
    }

    function getMatchingRewardAmount(uint256 _lotteryId, uint256 _matchingNumber) public view returns (uint256) {
        return historyAmount[_lotteryId][7 - _matchingNumber];
    }
    ...
    function getTotalRewards(uint256 _lotteryId) public view returns(uint256) {
        ...
        return historyAmount[_lotteryId][0];
    }
    ...
    function getRewardView(uint256 _tokenId) public view returns (uint256) {
        ...
        if (matchingNumber > 1) {
            ...
            uint256 poolAmount = getTotalRewards(_lotteryId).mul(allocation[6 - matchingNumber]).div(100);
            reward = amount.mul(1e12).div(getMatchingRewardAmount(_lotteryId, matchingNumber)).mul(poolAmount);
        }
