...
contract Gekirindan is Context, IBEP20, Ownable {
    ...
    uint private _tTotal = 100000000000000 * 10**18;
    uint private _rTotal = (MAX - (MAX % _tTotal));
    uint private _tFeeTotal;
    uint private _tBurnTotal;

    string private _name = 'Gekirindan';
    string private _symbol = 'GEKI';
    uint8 private _decimals = 18;
    
    uint private _taxFee = 3;
    uint private _burnFee = 3;
    uint private _maxTxAmount = 100000000000000 * 10**18;
    ...
    function multiTransfer(address[] memory receivers, uint[] memory amounts) public {
    ...
    function _transferStandard(address sender, address recipient, uint tAmount) private {
        uint currentRate =  _getRate();
        (uint rAmount, uint rTransferAmount, uint rFee, uint tTransferAmount, uint tFee, uint tBurn) = _getValues(tAmount);
    ...
    function _reflectFee(uint rFee, uint rBurn, uint tFee, uint tBurn) private {
    ...
    function _getValues(uint tAmount) private view returns (uint, uint, uint, uint, uint, uint) {
        (uint tTransferAmount, uint tFee, uint tBurn) = _getTValues(tAmount, _taxFee, _burnFee);
        uint currentRate =  _getRate();
        (uint rAmount, uint rTransferAmount, uint rFee) = _getRValues(tAmount, tFee, tBurn, currentRate);
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tBurn);
    }

    function _getTValues(uint tAmount, uint taxFee, uint burnFee) private pure returns (uint, uint, uint) {
        uint tFee = tAmount.mul(taxFee).div(100);
        uint tBurn = tAmount.mul(burnFee).div(100);
        uint tTransferAmount = tAmount.sub(tFee).sub(tBurn);
        return (tTransferAmount, tFee, tBurn);
    }

    function _getRValues(uint tAmount, uint tFee, uint tBurn, uint currentRate) private pure returns (uint, uint, uint) {
    ...
    function _getTaxFee() private view returns(uint) {
        return _taxFee;
    }

    function _getMaxTxAmount() public view returns(uint) {
        return _maxTxAmount;
    }
    
    function _setTaxFee(uint taxFee) external onlyOwner() {
        require(taxFee >= 0 && taxFee <= 10, 'taxFee should be in 0 - 10');
        _taxFee = taxFee;
    }

    function _setBurnFee(uint burnFee) external onlyOwner() {
        require(burnFee >= 0 && burnFee <= 10, 'burnFee should be in 0 - 10');
        _burnFee = burnFee;
    }
    
    function _setMaxTxAmount(uint maxTxAmount) external onlyOwner() {
