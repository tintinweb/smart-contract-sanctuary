...
import "./libs/IPresale.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/ReentrancyGuard.sol";

import "./AthToken.sol";

// MasterChef is the manager of WorldSwap Farms.
//
// Note that it's ownable but ownership was renounced.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract AthMasterChef is Ownable, ReentrancyGuard {
    ...
    uint256[] public wsPerBlockPhases;
    // Block number breakpoints for each emission rate
    uint256[] public wsPerBlockBreakpoints;
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256[] memory _wsPerBlockPhases,
        uint256[] memory _wsPerBlockBreakpoints,
        uint256 _startBlock
    ) public {
        ws = _ws;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        wsPerBlockPhases = _wsPerBlockPhases;
        wsPerBlockBreakpoints = _wsPerBlockBreakpoints;
    ...
    function add(
        ...
        require(_depositFeeBP <= 199, "add: invalid deposit fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        poolInfo.push(
            PoolInfo({
                ...
                tokensPerBlock: wsPerBlock()
            })
        );
    }

    // View function to see pending WSs on frontend.
    function pendingWs(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 wsReward = tokensBetweenBlocks(pool.lastRewardBlock, block.number).mul(pool.allocPoint).div(
                totalAllocPoint
            );
    ...
    function updatePool(uint256 _pid) public {
        ...
        ws.mint(0x000000000000000000000000000000000000dEaD, wsReward.div(10));
        ws.mint(address(this), wsReward);
        pool.accWsPerShare = pool.accWsPerShare.add(
            wsReward.mul(1e12).div(lpSupply)
        );
        pool.tokensPerBlock = wsPerBlock();
        pool.lastRewardBlock = block.number;
    }

    // Number of tokens emitted between 2 blocks
    function tokensBetweenBlocks(uint256 from, uint256 to) public view returns (uint256) {
        uint256 tokens = 0;
        uint256 fromLast = from;
        for (uint256 i = 0; i < wsPerBlockBreakpoints.length; i++) {
            if (wsPerBlockBreakpoints[i] > fromLast) {
                uint256 toLast = to > wsPerBlockBreakpoints[i] ? wsPerBlockBreakpoints[i] : to;
                tokens = tokens.add(toLast.sub(fromLast).mul(wsPerBlockPhases[i]));
                if (wsPerBlockBreakpoints[i] > to) {
                    return tokens;
                }
                fromLast = toLast;
            }
        }
        return tokens.add(to.sub(fromLast).mul(wsPerBlockPhases[wsPerBlockPhases.length - 1]));
    }
    ...
    function deposit(
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accWsPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending =
            user.amount.mul(pool.accWsPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            safeWsTransfer(msg.sender, pending);
            payReferralCommission(msg.sender, pending);
        }
    ...
    // Safe ws transfer function, just in case if rounding error causes pool to not have enough tokens.
    ...
    function wsPerBlock() public view returns (uint256) {
        for (uint256 i = 0; i < wsPerBlockBreakpoints.length; i++) {
            if (wsPerBlockBreakpoints[i] > block.number) {
                return wsPerBlockPhases[i];
            }
        }
        return wsPerBlockPhases[wsPerBlockPhases.length - 1];
    }
    ...
    // Allows the owner to update the referral contract.
...
contract AthToken is BEP20 {
     ...
     */
    constructor() public BEP20("Athens Token", "ATH") {
...
interface IPresale {
    /**
     * @dev Amount of tokens bought by each wallet on the presale
     */
    function tokensBought(address wallet) external view returns (uint256);
}
