...
interface IUniswapV2Router01 {
    ...
    function agingCoinsForExactTokens(
        uint amountOut,
        uint amountInMax,
        address[] calldata path,
        address to,
    ...
    function agingCoinsForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        returns (uint[] memory amounts);
...
contract GrumpyPutin is Context, IERC20, Ownable {
    ...
    string private _name = "GrumpyPutin";
    string private _symbol = "GPUTIN";
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        ...
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_reflectOwned[_excluded[i]] > rSupply || _totalOwned[_excluded[i]] > tSupply) return (_rTotal, _tTotal);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        agingCoinsForEth(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
    ...
    function agingCoinsForEth(uint256 tokenAmount) private {
