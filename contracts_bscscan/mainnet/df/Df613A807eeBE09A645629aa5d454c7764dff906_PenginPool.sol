...
// Root file: contracts/pools/PenginPool.sol
...
contract PenginPool is Ownable, Pausable, ReentrancyGuard {
    ...
    constructor(IERC20 _stakingToken, address _penginChef) {
    ...
    function earned(address account) public view returns (uint256) {
        ...
        uint256 performance = balanceOf(account).mul(duration).div(365 days);
        uint256 pending = penginChef.amountToMintByFee(address(stakingToken), performance);
        return pending.add(_rewards[account]);
    }

    function deposit(uint256 amount) public nonReentrant whenNotPaused {
        _rewards[msg.sender] = earned(msg.sender);
    ...
    function depositFor(address account, uint256 amount) external {
        require(msg.sender == address(penginChef), "only chef");
    ...
    function withdraw(uint256 amount) external nonReentrant {
        ...
        amount = Math.min(amount, balanceOf(msg.sender));
    ...
    function getReward() external nonReentrant {
        ...
        if (reward > 0) {
            penginChef.mintFor(msg.sender, reward);
            emit RewardPaid(msg.sender, reward);
        }
    }

    // recover token which directly sent to this contract
    // cannot withdraw staking token
    function recoverToken(address token) public onlyOwner {
        if (token == address(0)) {
            payable(owner()).transfer(address(this).balance);
        } else {
            require(token != address(stakingToken), "wut: token?");
