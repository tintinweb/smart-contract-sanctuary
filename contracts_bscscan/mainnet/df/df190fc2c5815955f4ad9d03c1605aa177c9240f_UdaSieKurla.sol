...
contract UdaSieKurla is Context, IERC20, Ownable {
    ...
    string private _name = "INF";
    string private _symbol = "MOON";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 100000 * 10**9 * 10**_decimals;
    
    address payable public marketingAddress = payable(0x6c1689f7a5dc34D94Bf2223704a15eb18F69E0a7);
    address payable public infinityAddress = payable(0x000000000000000000000000000000000000dEaD);
    address public marketingWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public infinityWalletToken = 0x93848210b98b446FA8fF3e2cDE4368A740457E23;
    ...
    constructor () {
        ...
        _infinityFee = 3;
        _burnFee = 7;
        _totalFees = _liquidityFee.add(_marketingFee).add(_burnFee).add(_infinityFee);
        _totalFeesToContract = _liquidityFee.add(_marketingFee).add(_infinityFee);
    ...
    function setLiquidityFeePerc(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFee).add(_marketingFee).add(_infinityFee) <= _maxFees, "Fees are too high.");
    ...
    function setMarketingFeePerc(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFee.add(_burnFee).add(newMarketingFee).add(_infinityFee) <= _maxFees, "Fees are too high.");
    ...
    function setInfinityFeePerc(uint256 newInfinityFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFee.add(_burnFee).add(newInfinityFee).add(_marketingFee) <= _maxFees, "Fees are too high.");
    ...
    function setBurnFeePerc(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFee.add(newBurnFee).add(_marketingFee).add(_infinityFee) <= _maxFees, "Fees are too high.");
        _burnFee = newBurnFee;
        _totalFees = _liquidityFee.add(_marketingFee).add(_burnFee).add(_infinityFee);
    ...
    function setMaxBal(uint256 newMaxBalance) external onlyOwner(){
        ...
        require(newMaxBalance >= _totalSupply.mul(20).div(1000));
    ...
    function _transfer(
        ...
        if(
            ...
            _totalFeesToContract > 0 &&                         // LiquidityFee + MarketingFee + InfinityFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
    ...
    function collectFees() private lockTheSwap {
        ...
        swapAndSendToBurn(infinityTokensToSell);
    }
