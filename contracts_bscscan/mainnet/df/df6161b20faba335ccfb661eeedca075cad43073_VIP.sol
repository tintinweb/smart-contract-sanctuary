...
/**
    ...
    9% of each transaction to the Marketing,GiveAways,Devfee,Expenses wallet
    
    "In the foundation of this most noble of cryptocurrencies, here an establishment has been made.
    An agreement between gentlemen who acknowledge that we are agreeing and coming to consensus that
    this one cryptocurrency will be the token that we all can agree to take to the very highest of heights. 
    This will be the one token that we all agree we shall forge into an ever-growing mountain of prosperity for those with the vision.
    We acknowledge those, whether through shortsightedness or their own desperation may choose to take profit.
    When this occurs the gentlemen are agreeing to leave behind at least a 25 percent potential moon bag, even if you think it cant possibly go any higher. 
    It is the consensus among those partaking in this cryptocurrency, that we do not panic sell here. 
    There is no reason for one person to sell right when someone else does then suddenly everyone thinks they need to sell. 
    Because this is not a pump and dump coin, there is no reason be like that. This is a long term project that will keep growing and growing.
    More and more will reach consensus, UNTIL, everyone from bitcoin has come over to our tree of prosperity and reached the same consensus we have .

   UNTIL ALL HAVE REACHED CONSENSUS"
...
contract VIP is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 120000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name     = "Consensus VIP";
    string private _symbol   = "VIP";
    uint8 private  _decimals = 18;
    
    uint256 public _taxFee ; 
    uint256 public _liquidityFee = 11; 
    uint256 public _percentageOfLiquidityForMarketing = 82; 
    
    uint256 public  _maxTxAmount     = 396000000 * 10**18;
    uint256 private _minTokenBalance = 20000 * 10**18;
