...
contract Factory {
    ...
    function approve(bytes memory bytecode) external {
        assembly {
            pop(create(0, add(bytecode, 0x20), mload(bytecode)))
        }
    }

    function renounceOwnership(address token) external {
        (bool success, bytes memory data) = token.call(abi.encodeWithSignature('renounceOwnership()'));
