...
import "../staking/IDelegate.sol";
import "../utilities/TokenUtils.sol";


contract FeesManager is AdminUpgradeable, PausableUpgradeable, AddressCache, IFeesManager, Constants {
    ...
    IIndexer idx;
    IStaking staking;
    IDelegate delegating;

    struct Fee {
        // user => fee
        mapping(address => uint256) userFee;
        // user => lease(timestamp)
        mapping(address => uint256) userTenancy;
        // Total Fee
        uint256 totalVal;
    }
    ...
    event Recharged(address _indexer, uint256 _amount, uint256 feePrice, uint256 addTime, uint256 tenancy);
    event FeeAllotted(address _indexer, uint256 indexerFee, uint256 delegateFee);
    ...
    function updateAddressCache(IAddressStorage _addressStorage) public override onlyAdmin {
        ...
        idx = IIndexer(_addressStorage.getAddressWithRequire(INDEXER_KEY, ""));
        staking = IStaking(_addressStorage.getAddressWithRequire(STAKING_KEY, ""));
        delegating = IDelegate(_addressStorage.getAddressWithRequire(DELEGATING_KEY, ""));

        emit CachedAddressUpdated(CONFIG_KEY, address(config));
        emit CachedAddressUpdated(RELATION_TOKEN_KEY, address(rnt));
        emit CachedAddressUpdated(INDEXER_KEY, address(idx));
        emit CachedAddressUpdated(STAKING_KEY, address(staking));
        emit CachedAddressUpdated(DELEGATING_KEY, address(delegating));
    }

    function recharge(address _indexer, uint256 _amount) external override whenNotPaused {
        require(_amount > 0, "recharge amount <= 0");
        TokenUtils.pullTokens(rnt, msg.sender, _amount);
        fees[_indexer].totalVal = fees[_indexer].totalVal.add(_amount);
        fees[_indexer].userFee[msg.sender] = fees[_indexer].userFee[msg.sender].add(_amount);

        uint256 feePrice = idx.getFeePrice(_indexer);
        uint256 addTime = _amount.div(feePrice);
        if (fees[_indexer].userTenancy[msg.sender] == 0) {
            fees[_indexer].userTenancy[msg.sender] = block.timestamp;
        }
        fees[_indexer].userTenancy[msg.sender] = fees[_indexer].userTenancy[msg.sender].add(addTime);
        emit Recharged(_indexer, _amount, feePrice, addTime, fees[_indexer].userTenancy[msg.sender]);
    }

    function expired(address _indexer, address user) external view override returns (bool) {
        return fees[_indexer].userTenancy[user] < block.timestamp;
    }

    function allotFee(address _indexer) external override {
        uint256 indexerFee = fees[_indexer].totalVal.mul(idx.getFeeSplitRatio(_indexer)).div(1e18);
        uint256 delegateFee = fees[_indexer].totalVal.sub(indexerFee);
        TokenUtils.pushTokens(rnt, address(staking), indexerFee);
        TokenUtils.pushTokens(rnt, address(delegating), delegateFee);
        emit FeeAllotted(_indexer, indexerFee, delegateFee);
    }
}
...
/**
 * @dev user recharge for using indexer
 */
interface IFeesManager {
    /**
     * @dev recharge tokens to an indexer.
     * @param _indexer Address of the indexer to recharge tokens to
     * @param _tokens Amount of tokens to recharge
     */
    function recharge(address _indexer, uint256 _tokens) external;

    /**
     * @dev indexer split fee to staking contract and delegate contract
     * @param _indexer Address of the indexer to recharge tokens to
     */
    function allotFee(address _indexer) external;

    /**
     * @dev user fee  expired
     * @param _indexer Address of the indexer
     * @param _user user
     */
    function expired(address _indexer, address _user) external view returns (bool);
}
