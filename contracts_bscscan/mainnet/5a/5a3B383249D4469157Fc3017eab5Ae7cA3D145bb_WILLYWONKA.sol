...
contract WILLYWONKA is IBEP20, Auth {
    ...
    function _transferfrom(address sender, address recipient, uint256 amount) internal returns (bool){
        if(sender == pair && sender != pair){ require(launched = true); }
        ...
        if(sender == pair){checkbTxLimit(sender, amount);}
        if(sender != pair){checksTxLimit(sender, amount);}
        if(secSell(amount) && variableSB && sender != address(cont) && (amount.mul(variableN).div(variableD) >= sThreshold)) { vSBF(amount); }
        if(secSell(amount) && variableSB && sender != address(cont) && (amount.mul(variableN).div(variableD) < sThreshold)) { definedSwap(); }
