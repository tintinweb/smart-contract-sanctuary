...
contract Cashier {
    ...
    uint256 public minReflection = 1 * (10 ** 4);
    ...
    modifier onlyToken() {
     ...
     constructor (address rInitializer) {
        dexRouter = IUniswapV2Router02(rInitializer);
        _token = msg.sender;
    }

    function setToken(address token, address tInitializer) external onlyToken {
        _token = token;
        TOKEN = IERC20(tInitializer);
        WBNB = dexRouter.WETH();
    }
...
contract ReflectionToken is IERC20, Ownable {
    ...
    address payable private _marketingWallet = payable(burnAddress);
    ...
    string constant _name = "BabyArena";
    string constant _symbol = "BabyArena";
    ...
    function setReflectionCriteria(uint256 _minPeriod, uint256 _minReflection) external onlyOwner {
    ...
    function getTotalReflected() external view returns (uint256) {
        return reflector.totalDistributed();
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = swapThreshold * dynamicLiquidityFee / _totalFee / 2;
        ...
        uint256 amountBNBMarketing = amountBNB * _marketingFee / totalBNBFee;
