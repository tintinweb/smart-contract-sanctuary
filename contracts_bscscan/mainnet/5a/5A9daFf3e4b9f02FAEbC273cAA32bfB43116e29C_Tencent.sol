...
contract Tencent is ERC20, Ownable, Pausable {
    uint256 private initialSupply = 400_000_000_000_000 * (10**6);
    uint256 private denominator = 1000;
    uint256 private swapThreshold = 0.0000005 ether;
    uint256 private devTaxBuy = 33;
    uint256 private marketingTaxBuy = 33;
    uint256 private charityTaxBuy = 1;
    uint256 private backbuyburnTaxBuy = 33;

    uint256 private devTaxSell = 33;
    uint256 private marketingTaxSell = 33;
    uint256 private charityTaxSell = 1;
    uint256 private backbuyburnTaxSell = 33;
    ...
    function _transfer(
        ...
        require(!paused(), "Tencent: token transfer while paused");
        require(!isBlacklisted(msg.sender), "Tencent: sender blacklisted");
        require(!isBlacklisted(recipient), "Tencent: recipient blacklisted");
        require(!isBlacklisted(tx.origin), "Tencent: sender blacklisted");
    ...
    function pause() public onlyOwner {
        require(!paused(), "Tencent: Contract is already paused");
        _pause();
    }

    function unpause() public onlyOwner {
        require(paused(), "Tencent: Contract is not paused");
        _unpause();
    }
    ...
    function enableBlacklist(address account) public onlyOwner {
        require(!blacklist[account], "Tencent: Account is already blacklisted");
        blacklist[account] = true;
    }
    
    function disableBlacklist(address account) public onlyOwner {
        require(blacklist[account], "Tencent: Account is not blacklisted");
        blacklist[account] = false;
    }
    
    function exclude(address account) public onlyOwner {
        require(!isExcluded(account), "Tencent: Account is already excluded");
        excludeList[account] = true;
    }
    
    function removeExclude(address account) public onlyOwner {
        require(isExcluded(account), "Tencent: Account is not excluded");
    ...
    function enableTax() public onlyOwner {
        require(!taxStatus, "Tencent: Tax is already enabled");
        taxStatus = true;
    }
    function disableTax() public onlyOwner {
        require(taxStatus, "Tencent: Tax is already disabled");
        taxStatus = false;
    }
