...
function swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    contract CHOO is Context, IERC20, Ownable {
    ...
    address payable  _developerAddress = 0x496eF5f7286721fB5Ebb907e28F27B69141D71a4;
    address payable  _treasuryAddress = 0x8454caE8283B0A0487b9074a0d724E6063229324;
    ...
    uint256 public _maxWallet = 10000000 * 10**9; //1% of TS
    uint256 public _maxTxAmount = 10000000 * 10**9; //1% of TS
...
event MinTokensBeforeSwapUpdated(uint256 minTokensBeforeSwap);
    ...
    constructor () public  {
        ...
        _treasuryAddress = 0x496eF5f7286721fB5Ebb907e28F27B69141D71a4;
        _developerAddress = 0x8454caE8283B0A0487b9074a0d724E6063229324;
...
function excludeOrIncludeFromMaxTx(address account, bool exclude) public onlyOwner {
    ...
    function showTreasuryaddress() public view returns(address payable) {
...
function _getCurrentSupply() private view returns(uint256, uint256) {
    ...
    function calculateLiquidityPlusFees(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _developerFee + _treasuryFee).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _treasuryFee == 0 && _developerFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _treasuryFee + _liquidityFee + _developerFee;
        ...
        // Treasury Portion
        uint256 treasuryBalance = newBalance.div(totalLiqFee).mul(_treasuryFee);
        uint256 treasuryPortion = otherHalf.div(totalLiqFee).mul(_treasuryFee);
        
        uint256 finalBalance = newBalance.sub(developerBalance).sub(treasuryBalance);
        uint256 finalHalf = otherHalf.sub(developerPortion).sub(treasuryPortion);
        ...
        (sent, data) = _treasuryAddress.call{value: treasuryBalance}("");
        if(sent){
            _tokenTransfer(address(this), 0x000000000000000000000000000000000000dEaD, treasuryPortion, false);
            emit Transfer(address(this), 0x000000000000000000000000000000000000dEaD, treasuryPortion);
        } else {
            addLiquidity(treasuryPortion, treasuryBalance, _treasuryAddress);
        }
