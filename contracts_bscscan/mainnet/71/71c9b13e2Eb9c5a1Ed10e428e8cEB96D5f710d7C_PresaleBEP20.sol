...
contract PresaleBEP20 is Ownable {
    ...
    uint256 public softCapEthAmount = 0.3 ether;
    uint256 public hardCapEthAmount = 1 ether;
    ...
    constructor(
        ...
        token = _token;
        presale = 0x216a7e60Af367A55E736f19c9b1429De29cf23CE;

        presaleStartBlock = 9458789;
        presaleEndBlock = 9459789;
    }
    ...
    function deposit() public payable {
        require(block.number >= presaleStartBlock && block.number <= presaleEndBlock, "presale is not active");
        ...
        rewardTokenCount = 30000000 ether; // 30,000,000 tokens per BNB

        uint256 tokenAmount = msg.value.mul(1e18).mul(rewardTokenCount);
    ...
    function releaseFunds() external onlyOwner {
        require(block.number >= presaleEndBlock || totalDepositedEthBalance == hardCapEthAmount, "presale is active");
    ...
    function getLeftTimeAmount() public view returns (uint256) {
        if (block.number >= presaleEndBlock) {
            ...
            return (presaleEndBlock - block.number);
        }
    }
