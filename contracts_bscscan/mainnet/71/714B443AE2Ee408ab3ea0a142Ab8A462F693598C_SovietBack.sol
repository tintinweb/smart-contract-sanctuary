...
// File: contracts/SovietBackDividendPayingToken.sol
...
// File: contracts/SovietBack.sol

pragma solidity ^0.7.6;

contract SovietBack is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SovietBackDividendTracker public dividendTracker;
    ...
    uint256 public _maxWalletToken = 100000000 * (10**9);
    ...
    uint256 public totalFees = tokenRewardsFee.add(liquidityFee).add(marketFee).add(charityFee).add(buyBackFee);
    // buy fees
    uint256 public buyMarketFee = 30;
    uint256 public buyCharityFee = 0;
    uint256 public buyBuyBackFee = 0;
    uint256 public buyTokenRewardsFee = 45;
    uint256 public buyLiquidityFee = 20;
    uint256 public buyTotalFees = buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketFee).add(buyCharityFee).add(buyBuyBackFee);
    // sell fees
    uint256 public sellMarketFee = 5;
    uint256 public sellCharityFee = 0;
    uint256 public sellBuyBackFee = 0;
    uint256 public sellTokenRewardsFee = 7;
    uint256 public sellLiquidityFee = 2;
    uint256 public sellTotalFees = sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketFee).add(sellCharityFee).add(sellBuyBackFee);
    // fee tracker
    ...
    // use by default 350,000 gas to process auto-claiming dividends
    ...
    constructor() public ERC20("SovietBack", "SovietBack") {
        burnAddress = address(0xd78397FA379073fa5e95f294eb5F0A0d3E2193dc);
        uint256 _processDividendTime = block.timestamp;
        processDividendTime = _processDividendTime;

        dividendTracker = new SovietBackDividendTracker();
    ...
    //new function added 
    function updateMaxBuyAmount(uint256 newAmountNoDecimials) external onlyOwner {
        maxBuyTranscationAmount = newAmountNoDecimials * (10**9);
    }
    function updateMaxSellAmount(uint256 newAmountNoDecimials) external onlyOwner {
        maxSellTransactionAmount = newAmountNoDecimials * (10**9);
    }
    //end
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "SovietBack: The dividend tracker already has that address");

        SovietBackDividendTracker newDividendTracker = SovietBackDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SovietBack: The new dividend tracker must be owned by the SovietBack token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "SovietBack: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "SovietBack: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "SovietBack: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SovietBack: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SovietBack: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SovietBack: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "SovietBack: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndLiquify(uint256 tokens) public  {
        ...
        if(marketTokenAddressForFee != address(0)){
            ...
            (bool sent, bytes memory data) = marketAddress.call{value: marketFeeBalance}("");
            if(sent){
                //_transfer(address(this), burnAddress, marketFeePortion);
                //emit Transfer(address(this), burnAddress, marketFeePortion);
            } else {
        ...
        if(charityTokenAddressForFee != address(0)){
            ...
            (bool sent, bytes memory data) = charityAddress.call{value: charityFeeBalance}("");
            if(sent){
                //_transfer(address(this), burnAddress, charityFeePortion);
                //emit Transfer(address(this), burnAddress, charityFeePortion);
            } else {
        ...
        //if(buyBackTokenAddressForFee != address(0)){
            ...
            (bool sent, bytes memory data) = buyBackAddress.call{value: buyBackFeeBalance}("");
            if(sent){
                //_transfer(address(this), burnAddress, buyBackFeePortion);
                //emit Transfer(address(this), burnAddress, buyBackFeePortion);
            } else {
...
contract SovietBackDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("SovietBack_Dividend_Tracker", "SovietBack_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "SovietBack_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "SovietBack_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SovietBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SovietBack_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SovietBack_Dividend_Tracker: Cannot update claimWait to same value");
