...
contract TokenplayGameV2 is Ownable, Pausable {
    ...
    event PrizePoolIncreased(uint256 round, uint256 amountInBNB, uint256 amountInTOP);
    ...
    function boostPrizePool(uint256 amountInTOP) external payable onlyWhitelister {
        IBEP20(token).transferFrom(msg.sender, address(this), amountInTOP);

        rounds[currentRound].bnbPrizePool += msg.value;
        rounds[currentRound].topPrizePool += amountInTOP;

        emit PrizePoolIncreased(currentRound, msg.value, amountInTOP);
    }
    ...
    function payRewards(
        ...
        require(
            totalBNBAmount == rounds[round].bnbPrizePool * rewardRate / 10000,
            "total BNB amount is not equal to remaining pool balance"
        );

        require(
            totalTOPAmount == rounds[round].topPrizePool * rewardRate / 10000,
