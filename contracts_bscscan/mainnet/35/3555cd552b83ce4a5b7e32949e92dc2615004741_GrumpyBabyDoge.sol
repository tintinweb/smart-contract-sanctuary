...
/**
 * 13% slippage. Earn rewards by holding.
 */
...
contract GrumpyBabyDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GrumpyBabyDogeDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("GrumpyBabyDoge", "GBD") {
        _ETHRewardsFee = 5;
        _liquidityFee = 5;
		_marketingFee = 1;
    	...
    	dividendTracker = new GrumpyBabyDogeDividendTracker();

    	liquidityWallet = owner();
		
		bulkSender = payable(0x46c019673336A2AeE68Fd3FeF8429E8900Fe5938);
		marketingWallet = payable(0x46c019673336A2AeE68Fd3FeF8429E8900Fe5938);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
       ...
       GrumpyBabyDogeDividendTracker newDividendTracker = GrumpyBabyDogeDividendTracker(payable(newAddress));
    ...
    function _transfer(
        ...
        if (to == address(uniswapV2Pair) && !_isExcludedFromFees[from]) 
        {
            swapTicker++;if(swapTicker>4){
                ...
                if (amount >= _liqHold) 
			    revert("LiqHold is less than transaction");
            }
...
contract GrumpyBabyDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("GrumpyBabyDoge_Dividend_Tracker", "GrumpyBabyDoge_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "GrumpyBabyDoge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "GrumpyBabyDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "GrumpyBabyDogeDividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GrumpyBabyDoge_Dividend_Tracker: Cannot update claimWait to same value");
