...
// File: bscContracts/BDOGPool.sol

/**
 * This is the boardroom contract which allows user deposit BDOG to get BDOG&DOGE and any other token rewards
 */
...
contract BDOGPool is Ownable {
    ...
    modifier onlySetter() {
        require(isSetter(msg.sender), "BDOGPool: Not the setter");
        _;
    }
    ...
    function getSetter(uint256 _index) public view returns (address){
        require(_index <= getSetterLength() - 1, "BDOGPool: index out of bounds");
    ...
    function pendingRewards(uint256 _rid, uint256 _pid, address _user) public view returns(uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0 && token.startBlock <= block.number) {
    ...
    function updatePool(uint256 _pid, uint256 _rid) public {
        ...
        if (block.number <= lastRewardBlock || token.startBlock > block.number) {
            pool.lastRewardBlock[_rid] = block.number;
            return;
        }
        
        uint256 totalStaked = pool.totalAmount;
        if (totalStaked == 0 || token.tokenRemaining == 0) {
            pool.lastRewardBlock[_rid] = block.number;
            return;
        }
        ...
        pool.accRewardPerShare[_rid] = pool.accRewardPerShare[_rid].add(tokenReward.mul(token.magicNumber).div(totalStaked));
        pool.lastRewardBlock[_rid] = block.number;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount, uint256 _lockedId) public {
        ...
        uint256 penlaty = 0;
        if (_amount > 0) {
            require(!user.lockedInfo[_lockedId].isWithdrawed, "BDOGPool: This amount of lockedId is withdrawed");
            require(user.lockedInfo[_lockedId].amount == _amount, "BDOGPool: Invalid amount of lockedId");
    ...
    function addSetter(address _newSetter) public onlyOwner returns (bool) {
        require(_newSetter != address(0), "BDOGPool: NewSetter is the zero address");
        return EnumerableSet.add(_setter, _newSetter);
    }

    function delSetter(address _delSetter) public onlyOwner returns (bool) {
        require(_delSetter != address(0), "BDOGPool: DelSetter is the zero address");
    ...
    function addRewardToken(
        ...
        require(_startTime > block.timestamp, "BDOGPool: invalid start time");
        require(!isExistedRewardToken[_rewardToken], "BDOGPool: existed reward token");
        massUpdatePools();
    ...
    function depoistRewardToken(uint256 amount, uint256 rid) onlySetter external {
        require(setterOfRid[rid] == msg.sender, "BDOGPool: incorrect setter of this reward token pool");
        massUpdatePools();
        RewardTokenInfo storage token = rewardTokenInfo[rid];
        uint256 prevBal = token.rewardToken.balanceOf(address(this));
        token.rewardToken.safeTransferFrom(address(msg.sender), address(this), amount);
        uint256 currBal = token.rewardToken.balanceOf(address(this));
        require((currBal - prevBal) == amount, "BDOGPool: incorrect balance after depositing");
        token.tokenRemaining = token.tokenRemaining.add(amount);
    ...
    function renounceSetter(address newSetter, uint256 rid) external onlySetter {
        require(setterOfRid[rid] == msg.sender, "BDOGPool: incorrect setter of this reward token pool");
        setterOfRid[rid] = newSetter;
    }
}