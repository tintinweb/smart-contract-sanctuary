...
contract MemeKing is Context, IERC20, Ownable {
    ...
    uint256 private _totalSupply = 1*10**(8+18);
    string private _name = "Test King";
    string private _symbol = "TKING1";
    ...
    uint256 public whaleFee = 150;

    MaxWalletMode public maxWalletMode = MaxWalletMode.TAX;

    event DevFundAddressChanged(address _new);
    event FNSAddressChanged(address _new);
    event InvestFundAddressChanged(address _new);

    constructor(address _devAddress, address _rewardAddress, address _marketingAddress) {
        ...
        excludeFromMaxWallet(feeWallets.burnAddress, true);
        excludeFromMaxWallet(address(0), true);

        setBuyFees(10, 10, 10, 10, 10);
        setSellFees(20, 50, 50, 20, 10);
   ...
   /**
    ...
    function _transfer(address _from, address _to, uint256 _amount) internal {
        ...
        if (maxWalletMode == MaxWalletMode.HARD && !_isExcludedFromMaxWallet[_to]) {
            require(_balances[_to].add(_amount) <= _maxWalletAmount, "You are transferring too many tokens, try less");
        }
        
        bool takeFee = !_isExcludedFromFees[_to] && !_isExcludedFromFees[_from];
        ...
        if (takeFee) {
            ...
            if (maxWalletMode == MaxWalletMode.TAX && !automatedMarketMakerPairs[_to] && !_isExcludedFromMaxWallet[_to]) {
                if (_balances[_to].add(_amount) > _maxWalletAmount) {
                    feePercent += whaleFee;
                    whaleFeeTaken == true;
                }
            }
            ...
            calculateFee(fees, feePercent, whaleFeeTaken);
    ...
    function calculateFee(uint256 _totalFees, uint256 feePercent, bool whale) internal {
        if (whale) {
            feePercent -= whaleFee;
        }
        ...
        if (feePercent == getSumOfBuyFees()) {
            ...
            uint256 _rewardShare = buyFees.rewardFee.mul(_totalFees).div(feePercent);
            uint256 _marketingShare = buyFees.marketingFee.mul(_totalFees).div(feePercent);
            uint256 _distributionShare = buyFees.distributionFee.mul(_totalFees).div(feePercent);
            uint256 _burnShare = buyFees.burnFee.mul(_totalFees).div(feePercent);
            sendFees(_devShare, _rewardShare, _marketingShare, _distributionShare, _burnShare);
        } else {
            // sell
            uint256 _devShare = sellFees.devFee.mul(_totalFees).div(feePercent);
            uint256 _rewardShare = sellFees.rewardFee.mul(_totalFees).div(feePercent);
            uint256 _marketingShare = sellFees.marketingFee.mul(_totalFees).div(feePercent);
            uint256 _distributionShare = sellFees.distributionFee.mul(_totalFees).div(feePercent);
    ...
    function sendFees(uint256 _toDev, uint256 _toReward, uint256 _toMarketing, uint256 _toDistribute,uint256 _toBurn) internal {
        _balances[feeWallets.devAddress] += _toDev;
        _balances[feeWallets.rewardAddress] += _toReward;
        _balances[feeWallets.marketingAddress] += _toMarketing;
        // distribute
        _balances[feeWallets.burnAddress] += _toDistribute;
        _balances[feeWallets.burnAddress] += _toBurn;
        emit Transfer(_msgSender(), feeWallets.devAddress, _toDev);
        emit Transfer(_msgSender(), feeWallets.rewardAddress, _toReward);
        emit Transfer(_msgSender(), feeWallets.marketingAddress, _toMarketing);
        // distribution
        emit Transfer(_msgSender(), feeWallets.burnAddress, _toDistribute);
        emit Transfer(_msgSender(), feeWallets.burnAddress, _toBurn);
    }
    ...
    function setDevAddress(address _devAddress) public onlyOwner {
        ...
        excludeFromMaxWallet(feeWallets.devAddress, false);
        feeWallets.devAddress = _devAddress;
        excludeFromFees(_devAddress, true);
        excludeFromMaxWallet(_devAddress, true);
    }

    function setRewardAddress(address _rewardAddress) public onlyOwner {
        excludeFromFees(feeWallets.rewardAddress, false);
        excludeFromMaxWallet(feeWallets.rewardAddress, false);
        feeWallets.rewardAddress = _rewardAddress;
        excludeFromFees(_rewardAddress, true);
        excludeFromMaxWallet(_rewardAddress, true);
    }

    function setMarketingAddress(address _marketingAddress) public onlyOwner {
        excludeFromFees(feeWallets.marketingAddress, false);
        excludeFromMaxWallet(feeWallets.marketingAddress, false);
        feeWallets.marketingAddress = _marketingAddress;
        excludeFromFees(_marketingAddress, true);
        excludeFromMaxWallet(_marketingAddress, true);
    }

    function setBuyFees(uint256 _devFee, uint256 _rewardFee, uint256 _marketingFee, uint256 _distributionFee,uint256 _burnFee) public onlyOwner {
        buyFees = FeeSet({
            devFee: _devFee,
    ...
    function setSellFees(uint256 _devFee, uint256 _rewardFee, uint256 _marketingFee, uint256 _distributionFee,uint256 _burnFee) public onlyOwner {
        sellFees = FeeSet({
            devFee: _devFee,
            rewardFee: _rewardFee,
            marketingFee: _marketingFee,
    ...
    function getSumOfFeeSet(FeeSet memory set) private pure returns (uint256) {
        return set.devFee.add(set.rewardFee).add(set.marketingFee).add(set.distributionFee).add(set.burnFee);
    }
    ...
    function setMaxWalletAmount(uint256 amount) public onlyOwner {
        require(amount <= _totalSupply, "MSN: Amount cannot be bigger than supply");
