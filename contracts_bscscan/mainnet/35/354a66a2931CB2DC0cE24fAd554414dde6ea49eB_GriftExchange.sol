...
contract GriftExchange is Ownable {
    ...
    function buyGrift(
        ...
        uint256 grift_at_current_rate = calculateGriftBought(
            _bnb_minus_overhead
        );
        uint256 grift_diff = 0;
        if (_grift_target > grift_at_current_rate) {
            grift_diff = _grift_target - grift_at_current_rate;
        } else {
            grift_diff = grift_at_current_rate - _grift_target;
        }
        uint256 max_slippage = (_grift_target / 100) * _slippage_percent;
        require(grift_diff < max_slippage, "Slippage too high.");

        // Transfer the GRIFT
        griftCoin.transfer(msg.sender, _grift_target);

        // Bookkeeping
        grift_reserves -= _grift_target;
