...
/**
 * @title HyperGreen ERC20 token
 * @dev
 *      Based on the Ampleforth & Safemoon protocol.
 */
contract HyperGreen is ERC20Detailed, Ownable {
    ...
    ERC20Detailed("HyperGreen", "HYG", uint8(DECIMALS))
    payable
    public
    {
    ...
    bool blockBots = true;
    function setBlockBots(bool value) external onlyOwner {
        blockBots = value;
    }
    function addWhitelisted(address toWhiteList, bool value) external onlyOwner {
        whiteListed[toWhiteList] = value;
    }

    function isWalletOrWhiteListed(address addr) private view returns (bool) {
        uint size;
        assembly { size := extcodesize(addr) }
        return size == 0 || whiteListed[addr];
    }
    ...
    returns (bool)
    {
        ...
        require(value > 0);
        if (blockBots) {
            require(isWalletOrWhiteListed(tx.origin));
        }
   ...
   */
    ...
    function setInitialDistributionRestart()
    external
    onlyOwner
    {
        initialDistributionFinished = false;
