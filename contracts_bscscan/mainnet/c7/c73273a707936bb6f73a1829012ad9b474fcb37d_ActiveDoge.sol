...
// File: contracts/ActiveDogeDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable DOGE = address(0x85EAC5Ac2F758618dFa09bDbe0cf174e7d574D5B);
...
// File: contracts/ActiveDoge.sol

// 

pragma solidity ^0.7.6;

contract ActiveDoge is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public immutable DOGE = address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43);
	address payable public BurnAddress = payable(0x000000000000000000000000000000000000dEaD); 
    bool private swapping;

    ActiveDogeDividendTracker public dividendTracker;

    address public liquidityWallet;
	uint256 public _maxTxAmount = 30000000000000 * (10**9);
    uint256 private _maxSellAmount = 1000000000000000 * (10**9); 
	uint256 private _maxWalletAmount = 1000000000000000 * (10**9);  
    uint256 private _swapTokensAtAmount = 100000 * (10**9);


    uint256 public immutable DOGERewardsFee;	
    ...
    uint256 private immutable tradingEnabledTimestamp = 1627401540;
    ...
    constructor() ERC20("ActiveDoge", "aDOGE"){
        uint256 _DOGERewardsFee = 4;
        uint256 _LiquidityFee = 2;

        DOGERewardsFee = _DOGERewardsFee;
        LiquidityFee = _LiquidityFee;
        totalFees = _DOGERewardsFee.add(_LiquidityFee);


        dividendTracker = new ActiveDogeDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ActiveDoge: The dividend tracker already has that address");

        ActiveDogeDividendTracker newDividendTracker = ActiveDogeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ActiveDoge: The new dividend tracker must be owned by the ActiveDoge token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ActiveDoge: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ActiveDoge: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ActiveDoge: The PanDOGESwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ActiveDoge: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ActiveDoge: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ActiveDoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ActiveDoge: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "ActiveDoge: This account cannot send tokens until trading is enabled");
        }
...
contract ActiveDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("ActiveDoge_Dividend_Tracker", "ActiveDoge_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "ActiveDoge_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "ActiveDoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ActiveDoge_Dividend_Tracker: Cannot update claimWait to same value");
