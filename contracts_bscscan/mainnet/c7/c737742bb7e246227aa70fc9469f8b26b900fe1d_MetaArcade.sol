...
contract MetaArcade is Context, IERC20, Ownable {
    ...
    string private _name = "Meta Arcade";
    string private _symbol = "METARC";
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        //uint256 tGameRewards = calculateGameRewardsFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tLiquidity).sub(tMarketing).sub(tDev);//.sub(tGameRewards);
        return (tTransferAmount, tLiquidity, tMarketing, tDev);
    }
    ...
    function _takeGameRewards(uint256 tGameRewards) private {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tLiquidity, uint256 tMarketing, uint256 tDev) = _getTValues(tAmount);
        uint256 tGameRewards = calculateGameRewardsFee(tAmount);
        uint256 transferAmount = tTransferAmount - tGameRewards;
