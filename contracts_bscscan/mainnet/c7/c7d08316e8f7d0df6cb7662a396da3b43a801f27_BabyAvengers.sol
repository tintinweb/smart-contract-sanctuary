...
contract BabyAvengers is IBEP20, Auth {
    ...
    uint256 public _maxTx1 = (_totalSupply * 1) / 200; //Max Tx for the first 20 seconds is 0.5%
    uint256 public _maxTx2 = (_totalSupply * 2) / 100; //Max Tx for after 20 seconds is 2%
    ...
    uint256 timeBetweenSwap = 1 minutes;
    ...
    uint256 public swapThreshold = _totalSupply / 1000; // 0.01% of supply
    bool inSwap;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!authorizations[sender] && !authorizations[recipient]){
            ...
            if(!Done && launchAt + 20 seconds <= block.timestamp){
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        if(sender == pair && launchAt + 3 > block.number)
    ...
    function shouldSwapBack() internal view returns (bool) {
        ...
        && lastTimeSwap + timeBetweenSwap < block.timestamp;
    }
    ...
    function setTxPercentage(uint256 percentage) external authorized {
        _maxTxAmount = (_totalSupply * percentage) / 100;
    }
