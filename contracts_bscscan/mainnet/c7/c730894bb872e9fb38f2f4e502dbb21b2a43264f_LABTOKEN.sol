...
FAIR LAUNCH on saturday 16th 7PM UTC 
...
contract LABTOKEN is Context, IERC20, Ownable {
    ...
    uint256 public _marketingFeeBNB = 5;
    ...
    function rebase (uint256 epoch, int256 supplyDelta) external onlyOwner returns (uint256) {
        ...
        if (supplyDelta < 0)
          _tTotal -= (uint256(-supplyDelta) * 10**DECIMALS);
        
        else
        _tTotal = _tTotal + (uint256(supplyDelta) * 10**DECIMALS) > MAX_SUPPLY ? MAX_SUPPLY : _tTotal + (uint256(supplyDelta) * 10**DECIMALS);

        _fomosPerFragment = TOTAL_FOMOS / _tTotal;
