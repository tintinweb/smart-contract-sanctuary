...
/*
    https://t.me/BabyFOMOin
    
    Rebasing every 5-10 minute - the only way is up!
*/
...
contract BabyFOMO is ERC20Detailed, Ownable {
    ...
    bool antiDamp = true;
    uint256 public antiDampFactor = 30;
    uint256 public cooldownPeriod = 600;
    ...
    constructor() ERC20Detailed("BabyFOMO", "BOMO", uint8(DECIMALS)) {
    ...
    function _transferFrom(
        ...
        if (sender != owner() && !_isMaxWalletExempt[recipient]) {
            ...
            if (launchedAt + 10 >= block.number){
                gg=gonMaxWallet.div(10);}
            else {
                gg = gonMaxWallet;}
            require(
                (heldGonBalance + gonAmount) <= gg,
        ...
        returns (uint256)
    {
        ...
        feeAmount = gonAmount.mul(feeTmp).div(feeDenominator);
    ...
    function getAntiDamp() public view returns (uint256) {
        uint256 current = cooldownPeriod.add(launchedAt);
        if (current > block.number) {
            uint256 timeLeft = current.sub(block.number);
            uint256 feeIncrease=antiDampFactor.mul(timeLeft).div(cooldownPeriod);
            return feeIncrease;
        }
        return totalFee;
    }
    
    function setAntiDamp(uint256 _amount, bool _antiDamp) external onlyOwner{
        antiDamp = _antiDamp;
        antiDampFactor = _amount;
    }
