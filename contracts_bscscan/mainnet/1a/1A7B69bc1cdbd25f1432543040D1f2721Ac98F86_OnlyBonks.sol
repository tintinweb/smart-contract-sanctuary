...
contract OnlyBonks is Context, IBEP20, Ownable {
    ...
    string private _name = "OnlyBonks";
    ...
    uint256 public _maxWalletToken = _tTotal.div(1000).mul(15);
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        _maxTxAmount = _tTotal.div(1000).mul(maxTxPercent);
    }
    ...
    function _transfer(
        ...
        if (to != owner() && to != _marketingWalletAddress && to != address(this) && to != pancakeSwapV2Pair && to != _burnWalletAddress && from != owner()) {
            uint256 heldTokens = balanceOf(to);
            require((heldTokens.add(amount)) <= _maxWalletToken,"You are trying to buy too many tokens. You have reached the limit for one wallet.");
        }
