...
contract CareBears is Context, IERC20, Ownable {
    ...
    //======== FEE Description ===========
    uint256 public constant MARKETING_FEE = 130; //13 * 10%
    uint256 public constant LP_FEE = 15; //1.5% 
    ...
    uint256 public maxTransferAmount =  MAX_SUPPLY.mul(20).div(1000);
    uint256 public maxHoldPerWallet =  MAX_SUPPLY.mul(20).div(1000);
   ...
   event onEnableDisableFee(bool isFeeEnable);
    ...
    constructor(address _marketingWallet)  {
        ...
        maxHoldWhitelist[marketingWallet] = true;
    ...
    */
    ...
    function chargeFee(address from, uint256 _amount) internal returns(uint256){
        ...
        uint256 _marketingFee = _amount.mul(MARKETING_FEE).div(1000);
        uint256 lpFee = _amount.mul(LP_FEE).div(1000);
        _balances[address(this)] = _balances[address(this)].add(lpFee);
        _balances[marketingWallet] = _balances[marketingWallet].add(_marketingFee);
        emit Transfer(from, address(this), lpFee);
        emit Transfer(from, marketingWallet, _marketingFee);
        uint256 totalFee= _marketingFee.add(lpFee);
        return totalFee;
