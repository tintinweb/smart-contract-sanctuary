...
contract DUMMY1 is ERC20, Ownable {
    ...
    DMMYDividendTracker public dividendTracker;

    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD

    address public marketingWallet = 0x3C0cdaed29D4c8958f2475F8Be9f0Ee98Cb88C44;
    address public devWallet = 0x5BDD3685816E558Ab4Df75Aeb4e044c65F86a260;

    uint256 public swapTokensAtAmount = 2_000_000 * 10**9;
    uint256 public maxWalletAmount = 3e13 * 10**9;
    uint256 public maxTxAmount = 1e13 * 10**9;
    ...
    Taxes public buyTaxes = Taxes(5,1,1,1);
    Taxes public sellTaxes = Taxes(5,5,3,2);
    ...
    constructor() ERC20("Dummy1", "DMMY") {

        dividendTracker = new DMMYDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        DMMYDividendTracker newDividendTracker = DMMYDividendTracker(payable(newAddress));
   ...
   /////////////////////////////////
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DMMY: Account is already the value of 'excluded'");
   ...
   ///////////////////////
    ...
    function setBuyTaxes(uint256 _rewards, uint256 _marketing, uint256 _liquidity, uint256 _dev) external onlyOwner{
        ...
        totalBuyTax = _rewards + _marketing + _liquidity + _dev;
    }
    ...
    function setSellTaxes(uint256 _rewards, uint256 _marketing, uint256 _liquidity,uint256 _dev) external onlyOwner{
        ...
        totalSellTax = _rewards + _marketing + _liquidity + _dev;
    }
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(automatedMarketMakerPairs[newPair] != value, "DMMY: Automated market maker pair is already set to that value");
    ...
    function setGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DMMY: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DMMY: Cannot update gasForProcessing to same value");
   ...
   ////////////////////////
    ...
    // Airdrop tokens to users. This won't update the dividend balance in order to avoig a gas issue.
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if( canSwap && !swapping && swapEnabled && !automatedMarketMakerPairs[from] && !_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if(totalSellTax> 0){
                uint256 swapTokens = swapTokensAtAmount * (totalSellTax - sellTaxes.rewards) / (totalSellTax);
                swapAndLiquify(swapTokens);
            }
            if(sellTaxes.rewards > 0){
                uint256 sellTokens = swapTokensAtAmount * sellTaxes.rewards / totalSellTax;
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 denominator= (sellTaxes.liquidity + sellTaxes.dev + sellTaxes.marketing) * 2;
...
contract DMMYDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()  DividendPayingToken("DMMY_Dividen_Tracker", "DMMY_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10_000_000_000 * (10**decimals());
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "DMMY_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DMMY_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DMMY_Dividend_Tracker: Cannot update claimWait to same value");
