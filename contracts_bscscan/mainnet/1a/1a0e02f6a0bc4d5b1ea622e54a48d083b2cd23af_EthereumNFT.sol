...
contract EthereumNFT is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "EthereumNFTs";
    string constant _symbol = "ETHNFT";
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // cooldown timer, so a bot doesnt do quick trades! 0.5 min gap between 2 trades.
        if (sender == pair &&
            buyCooldownEnabled &&
            !isTimelockExempt[recipient]) {
            require(cooldownTimer[recipient] < block.timestamp,"Please wait for 0.5 min between two buys");
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        if(haslaunched == true && launchedAt + 5 >= block.number){
