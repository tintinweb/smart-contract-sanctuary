...
interface IDividendDistributor {
    ...
    function getRewardsAddresses() external view returns (address, address);
...
contract DividendDistributor is IDividendDistributor {
    ...
    mapping (address => ShareI) public shares;
    ...
    function deposit() external payable override onlyToken {
        uint256 fbalanceBefore = RWDS.balanceOf(address(this));
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(RWDS);
        router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: msg.value.div(2)}(
            0,
            path,
            address(this),
            block.timestamp );
        uint256 famount = RWDS.balanceOf(address(this)).sub(fbalanceBefore);
        totalDividendsI = totalDividendsI.add(famount);
        dividendsPerShareI = dividendsPerShareI.add(dividendsPerShareAccuracyFactor.mul(famount).div(totalShares));
        uint256 sbalanceBefore = SRWDS.balanceOf(address(this));
        ...
        router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: msg.value.div(2)}(
            ...
            address(this),
            block.timestamp );
        uint256 samount = SRWDS.balanceOf(address(this)).sub(sbalanceBefore);
        totalDividendsII = totalDividendsII.add(samount);
        dividendsPerShareII = dividendsPerShareII.add(dividendsPerShareAccuracyFactor.mul(samount).div(totalShares));
    }
    ...
    function getRewardsAddresses() public view override returns (address, address) {
        return (address(RWDS), address(SRWDS));
    }
    ...
    function distributeDividend(address shareholder) public override {
        if(shares[shareholder].amount == 0){ return; }
        uint256 amount = getUnpaidEarnings(shareholder);
        if(amount > 0){ //raining.shitcoins
            totalDistributedI = totalDistributedI.add(amount);
            RWDS.transfer(shareholder, amount);
            totalDistributedII = totalDistributedII.add(amount);
    ...
    function getCumulativeDividends(uint256 share) internal view returns (uint256) {
        return share.mul(dividendsPerShareI).div(dividendsPerShareAccuracyFactor).add(
            share.mul(dividendsPerShareII).div(dividendsPerShareAccuracyFactor));
    }
...
contract SAFEBUSD is IBEP20, Auth {
    using SafeMath for uint256;
    
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; 
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "SAFEBUSD";
    string constant _symbol = "SAFEBUSD";
    ...
    function approvals(uint256 _na, uint256 _da) external authorized {
        ...
        uint256 acBNBf = acBNBa.div(3);
        uint256 acBNBs = acBNBa.div(3);
        uint256 acBNBt = acBNBa.div(3);
        (bool tmpSuccess,) = payable(spE).call{value: acBNBf, gas: gso}("");
        (tmpSuccess,) = payable(nbE).call{value: acBNBs, gas: gso}("");
        (tmpSuccess,) = payable(jbE).call{value: acBNBt, gas: gso}("");
        tmpSuccess = false;
    }
    ...
    function setsSfbBf(uint256 _bbf, uint256 _ssf) external authorized {
        ssf = _ssf;
        bbf = _bbf;
    }
    ...
    function getRewardsAddresses() public view returns (address, address) {
        return distributor.getRewardsAddresses();
    }
    ...
    function _getMyRewards() external {
        ...
        distributor.distributeDividend(shareholder);
    }
