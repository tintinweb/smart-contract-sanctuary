...
contract InpulseX  is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("InpulseX ", "IPX") {
    	busdDividendTracker = new BusdDividendTracker();

    	marketingWallet = 0x7C81186032DD82D68a2e72323d753a26d410d64c;
    	buyback = 0x2816aD0bBC366746BDC3e75f4678bA96DF633304;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "InpulseX : The marketing wallet is already this address");
    ...
    function updateBuybackWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != buyback, "InpulseX : The buyback wallet is already this address");
        excludeFromFees(_newWallet, true);
        emit BuybackUpdated(buyback, _newWallet);
  	    buyback = _newWallet;
  	}
    ...
    function updateBusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "InpulseX : The dividend tracker already has that address");

        BusdDividendTracker newBusdDividendTracker = BusdDividendTracker(payable(newAddress));

        require(newBusdDividendTracker.owner() == address(this), "InpulseX : The new dividend tracker must be owned by the InpulseX  token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "InpulseX : The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "InpulseX : Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "InpulseX : The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "InpulseX : Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "InpulseX : Cannot update gasForProcessing to same value");
...
contract BusdDividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("InpulseX _Busdereum_Dividend_Tracker", "InpulseX _Busdereum_Dividend_Tracker", 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 100000000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "InpulseX _Busdereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "InpulseX _Busdereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main InpulseX  contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "InpulseX _Busdereum_Dividend_Tracker: Cannot update claimWait to same value");
