...
contract DoggyCake is Context, IERC20, Ownable {
	...
	//max wallet holding 10000 * 10**6 * 10**9 is 1,2% of 100000 * 10**6 * 10**9 (total supply)
    uint256 public _maxWalletToken = 1200 * 10**6 * 10**9;

    string private _name = "DoggyCake";
    ...
    function _transfer(
        ...
        if (to != owner() && to != address(this)  && to != address(0x000000000000000000000000000000000000dEaD) && to != uniswapV2Pair){
            ...
            require((heldTokens + amount) <= _maxWalletToken,"Total Holding is currently limited to a 1.2%, you can not buy that much.");}
