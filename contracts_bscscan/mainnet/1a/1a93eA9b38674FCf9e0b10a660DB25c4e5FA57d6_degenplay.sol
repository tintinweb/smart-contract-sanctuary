...
/**
 DEGENPLAY - SAFU DEV
 Degen Play | LOW MC | Ownership Renounced | LP Locked
 
 Telegram : https://t.me/degenplaybsc
...
contract degenplay is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = 1_000_000_000 * 10**9; // 
    uint256 private constant SWAP_TOKENS_AT_AMOUNT = 4_000_000 * 10**9; //
    string private constant _name = "Extremely Grape Rise"; // 
    string private constant _symbol = "EGR"; //    
    uint8 private constant _decimals = 18; // 
    
    uint256 public _degenplayFee = 7;
    uint256 public _liquidityFee = 3;
    address private _degenplayWallet = 0x6Bd14013059E8c55b67a004Ce528AAE93c77032d;
    
    uint256 public _buyCooldown = 1 minutes;
    ...
    constructor () {
        ...
        _isExcludedFromFee[_degenplayWallet] = true;
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 degenplayFee = amount * _degenplayFee / 100; 
        uint256 liquidityFee = amount * _liquidityFee / 100; 
        _tOwned[address(this)] += degenplayFee + liquidityFee;
        emit Transfer (from, address(this), degenplayFee + liquidityFee);
        return (amount - degenplayFee - liquidityFee);
    }
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(_degenplayWallet).transfer(ethToSend);
    }

    function swapAndLiquify() private {
        // split the contract balance into halves
        uint256 liquidityTokens = balanceOf (address(this)) * _liquidityFee / (_degenplayFee + _liquidityFee);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0)
            payable(_degenplayWallet).sendValue (ethAmount - ethFromLiquidity);
    }
}