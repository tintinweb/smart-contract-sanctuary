...
contract Dev_Wallet_Lock {
    ...
    uint8 public factor = 1;  // amount linearly vested = initial_deposit / factor; rest at expiration
    ...
    constructor (uint256 _release_time, address token_address) public {
        ...
        ape_contract = IERC20(token_address);
    }
    ...
    function start_vesting () public onlyOwner {
        initial_deposit = ape_contract.balanceOf(address(this));
        claimed = 0;
    }
    ...
    function pending_token() public view returns (uint256){
        if(release_time >= block.timestamp) {
            ...
            return ape_contract.balanceOf(address(this));
        }
    }

    function withdraw () public onlyOwner {

        uint256 balance = ape_contract.balanceOf(address(this));
        ...
        if(release_time >= block.timestamp) { //before end of vesting
          ...
          ape_contract.transfer(wallet, claimable);
          emit withdrawal(wallet, claimable);
        }

        else { //end of vesting, cashout everything
          ape_contract.transfer(wallet, balance);
