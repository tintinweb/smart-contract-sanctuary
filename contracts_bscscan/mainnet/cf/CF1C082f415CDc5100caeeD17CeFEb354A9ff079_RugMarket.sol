...
contract RugMarket is Ownable {
    ...
    function buy(uint _id) public {
        require(listings[_id].state == SaleState.OPEN, 'RugMarket: This listing is not open');
        require(listings[_id].owner != msg.sender, 'RugMarket: You cannot buy your own listing');
    ...
    // Function for emergency listing cancel by owner
    function emergencyCancel(uint _id) public onlyOwner() {
        require(listings[_id].state == SaleState.OPEN, 'RugMarket: This listing is not open');

        IBEP20 token = IBEP20(listings[_id].token);
        require(token.transfer(listings[_id].owner, listings[_id].quantity));
...

{
  ...
  "evmVersion": "istanbul",
  "libraries": {
    "/Users/Shared/rug-zombie-secondary-contracts/contracts/libraries/Percentages.sol": {
      "Percentages": "0xAbb741072f003a908DFdc13C40B2Fc95423ba050"
    }
  },
  "outputSelection": {
    "*": {
