...
import "../treasury/MFCLOANT.sol";
import "../token/MFCToken.sol";
import "../MFCMembership.sol";

contract MFCGovernance is AdminAgent {
  ...
  struct MigrationProposal {
    ...
    address mfcloantDestination;
    address membershipDestination;
    uint256 endsAt;
  }

  MFCToken private _mfc;
  MigrationProposal private _migration;
  ...
  mapping(VoteOptions => uint256) public voteCount;
  ...
  modifier hasProposal() {
    ...
    require(_mfcLoanTreasuryAddress != address(0), "MFC Treasury address not set");
    require(_mfcMembershipAddress != address(0), "Membership address not set");
    require(_migration.busdtDestination != address(0), "No proposal");
    _;
  }

  function getProposal() external view returns (MigrationProposal memory) {
    return _migration;
  }
  ...
  function setMfcLoanTreasuryAddress(address _address) external onlyDeployer {
    require(_mfcLoanTreasuryAddress == address(0), "Already set");
    _mfcLoanTreasuryAddress = _address;
  }

  function setMfcMembershipAddress(address _address) external onlyDeployer {
    require(_mfcMembershipAddress == address(0), "Already set");
    _mfcMembershipAddress = _address;
  }

  function startMigrationProposal(address busdtDestination, address mfcloantDestination, address membershipDestination) external onlyAdminAgents {
    require(_migration.busdtDestination == address(0), "Proposal already active");
    _migration.endsAt = block.timestamp + votingPeriod;
    _migration.busdtDestination = busdtDestination;
    _migration.mfcloantDestination = mfcloantDestination;
    _migration.membershipDestination = membershipDestination;
  }

  function vote(VoteOptions voteOption, uint256 quantity) external hasProposal {
    require(block.timestamp < _migration.endsAt, "Proposal already ended");
    require(_deposits[_msgSender()] == 0, "Already voted");
    ...
    _deposits[_msgSender()] += quantity;
    voteCount[voteOption] += quantity;

    emit Vote(_msgSender(), voteOption, quantity);
  }

  function executeMigrationProposal() external hasProposal onlyAdminAgents {
    require(block.timestamp > _migration.endsAt, "Proposal still ongoing");
    require(voteCount[VoteOptions.YES] + voteCount[VoteOptions.ABSTAIN] > voteCount[VoteOptions.NO], "Proposal not passed");

    // execute BUSDT migration
    BUSDT _busdt = BUSDT(_busdTreasuryAddress);
    _busdt.setMigration(_migration.busdtDestination);

    // execute MFCLOANT migration
    MFCLOANT _mfcloant = MFCLOANT(_mfcLoanTreasuryAddress);
    _mfcloant.setMigration(_migration.mfcloantDestination);

    // execute MFCMembership migration
    MFCMembership _mfcMembership = MFCMembership(_mfcMembershipAddress);
    _mfcMembership.setMigration(_migration.membershipDestination);
  }

  function withdraw() external hasProposal {
    require(block.timestamp > _migration.endsAt, "Proposal still ongoing");
    require(_deposits[_msgSender()] > 0, "Nothing to withdraw");
    uint256 quantity = _deposits[_msgSender()];
...
contract MFCLOANT is Treasury, AdminAgent, AdminGovernanceAgent, Governable {
