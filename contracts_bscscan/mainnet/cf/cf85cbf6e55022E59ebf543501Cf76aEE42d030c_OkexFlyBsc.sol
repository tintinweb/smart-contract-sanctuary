...
contract OkexFlyBsc is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 450000000 * 10**6 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "OK Fly";
    string private _symbol = "OKFly";
    ...
    uint256 public _maxTxAmount = 10000000 * 10**6 * 10**18;

    // Liquify when 250 billion tokens are stored
    uint256 private numTokensSellToAddToLiquidity = 25000 * 10**6 * 10**18;
    ...
    constructor (address _minterAddr, address _router) {
        _rOwned[ONE] = _rTotal;
        ...
        _isExcludedFromFee[ONE] = true;
        _isExcludedFromFee[_minterAddr] = true;

        minter = _minterAddr;

        emit Transfer(address(0), ONE, _tTotal);
    }
    ...
    function setTokensSellToAddToLiquidity(uint256 _numTokensSellToAddToLiquidity) public onlyOwner {
    ...
    function setMarketingAddress(address payable _addr) external onlyOwner {
    ...
    function burn(uint256 amount, bytes calldata data) external {
        _transfer(_msgSender(), ONE, amount );
    }
