...
contract YoMeta is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("YoMeta", "YoMeta") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0x3eaBA258653f50108322ddDa9374cC14012cEE93;
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "YoMeta: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "YoMeta: The new dividend tracker must be owned by the YoMeta token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("YoMeta_Dividend_Tracker", "YoMeta_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 150000 * (10**18); //must hold 150,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "YoMeta_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "YoMeta_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "YoMeta_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "YoMeta_Dividend_Tracker: Cannot update claimWait to same value");
