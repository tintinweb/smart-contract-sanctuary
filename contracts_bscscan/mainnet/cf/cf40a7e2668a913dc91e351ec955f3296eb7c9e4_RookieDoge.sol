...
contract RookieDoge is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tFee, uint256 tBurn, uint256 tLiquidity) = _getTBasics(tAmount);
    ...
    function _getTBasics(uint256 tAmount) private view returns (uint256, uint256, uint256) {
    ...
    // Innovation for protocol by OnlyUP Team
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        require(balanceOf(msg.sender) >= 0, 'Error: must own OUP to claim reward');
