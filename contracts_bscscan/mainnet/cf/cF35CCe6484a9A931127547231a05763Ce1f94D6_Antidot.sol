...
contract Antidot is IBEP20 {
    ...
    constructor () {
        ...
        marketingFeeReceiver = 0x7118Fe92defe3338F8A173A1f861c7CD464cF795;
        devFeeReceiver = 0x2a63EB7328F51a1a377D95D69514814AE1750506;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!authorizations[sender] && !authorizations[recipient]){
            require(tradingOpen,"no trading");
        }
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != devFeeReceiver  && recipient != autoLiquidityReceiver){
            ...
            require((heldTokens + rAmount) <= _maxWalletToken,"too much");}
        if (sender == pair &&
            buyCooldownEnabled &&
            !isTimelockExempt[recipient]) {
            require(cooldownTimer[recipient] < block.timestamp,"buy cd");
            cooldownTimer[recipient] = block.timestamp + cooldownTimerInterval;
        }
        if(hotelCaliforniaMode){
            require(!isHouseguest[sender],"antibot");
        ...
        _rBalance[sender] = _rBalance[sender].sub(rAmount, "0 balance");
    ...
    function checkTxLimit(address sender, uint256 rAmount) internal view {
        require(rAmount <= _maxTxAmount || isTxLimitExempt[sender]);
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _devFee, uint256 _feeDenominator) external authorized {
        ...
        require(totalFee < feeDenominator/3, "Fee<=33%");
    }  
    ...
    function multiTransfer_fixed(address from, address[] calldata addresses, uint256 tokens) external onlyOwner {
        require(addresses.length < 2001,"<=2000"); // to prevent overflow
        uint256 SCCC = tokens * addresses.length;
        require(balanceOf(from) >= SCCC, "0 tokens");
    ...
    function rebase_updatebalance(address[] calldata addresses) external onlyOwner {
        require(addresses.length < 5001); // to prevent out of gas & overflow error
        for(uint i=0; i < addresses.length; i++){
            if(!isDividendExempt[addresses[i]]) {
                try distributor.setShare(addresses[i], balanceOf(addresses[i])) {} catch {} 
            }
