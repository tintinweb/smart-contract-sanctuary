...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    bool public hasRewardPerBlockUpdated;

    uint public numberOfClaim;
    ...
    function deposit(uint256 _amount) external nonReentrant {
        require(!hasRewardPerBlockUpdated, 'Reward per block has just updated, please wait until admin distributes all the claim rewards.');
        ...
        //         if (block.number >= localAddressEndLockTime || block.number >= bonusEndBlock) {
        //             rewardToken.safeTransfer(address(msg.sender), pending);
        //         }
        //         // require(block.number >= endLockTime || block.number >= bonusEndBlock, "Lock still in affect, or pool hasn't ended yet.");
        //     }
        // }
    ...
    function updateRewardPerBlock(uint256 _rewardPerBlock) external onlyOwner {
        // v Allow rewardPerBlock to change while pool has started v
    ...
    function confirmUpdateRewardPerBlock() external onlyOwner {
        hasRewardPerBlockUpdated = true;
        numberOfClaim = 0;
    }

    function claimRewardsByAdmin(uint _no) external onlyOwner {
        require(_no > 0, 'The no should be greater than 0');
        require(hasRewardPerBlockUpdated, 'Unable to claim reward, reward per block has not yet updated by admin.');

        uint _numberOfClaim = numberOfClaim;

        for (;
            _numberOfClaim < (numberOfClaim.add(_no)) && _numberOfClaim < userArr.length;
            _numberOfClaim++ // Not using add of safemath to reduce gas
        ) {
            _claimRewards(userArr[_numberOfClaim]);
        }

        if(_numberOfClaim == userArr.length){
            hasRewardPerBlockUpdated = false;
        }

        numberOfClaim = _numberOfClaim;
    }
    ...
    function _claimRewards(address _user) internal nonReentrant {
        UserInfo storage user = userInfo[_user];
        ...
        if (user.amount > 0) {
            ...
            user.rewardDebt = user.amount.mul(accTokenPerShare).div(PRECISION_FACTOR);
            emit ClaimRewards(_user, pending);
        }
    }
}