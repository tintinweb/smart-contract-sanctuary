...
contract OrbitSwap is Context, IERC20, Ownable {
    ...
    uint private startingSupply = 900000000000000;  // total supply
    ...
    string private _name = "Orbit Swap"; 
    string private _symbol = "OTSWAP";
    ...
    uint256 private _charityFee = 0; // Set dynamically
    uint256 public _buycharityFee = 300;
    uint256 public _sellcharityFee = 300; 
    uint256 private maxcharityFee = 800;
    ...
    address payable public _marketingWallet = payable(0xACBA95953664095994140E5fA2deEEB4C7e28Ec6); //select your marketing address
    address payable public  _charityWallet = payable(0x45881a9F858f1ACA69BC29e55c7ef88581EF160e);//select charity address
    address payable public _developerWallet = payable(0x408fFe5A377aA9Bd999E2d38523C673C61f517Cc); // development team address
    ...
    function setBuyTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee, uint256 burnFee, uint256 charityFee) external onlyOwner() {
        ...
        require(charityFee <= maxcharityFee); // Prevents owner from abusing fees.
    ...
    function setSellTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee, uint256 burnFee, uint256 charityFee) external onlyOwner() {
        require(reflectFee <= maxReflectFee); // Prevents owner from abusing fees.
        require(liquidityFee <= maxLiquidityFee); // Prevents owner from abusing fees.
        require(marketingFee <= maxMarketingFee); // Prevents owner from abusing fees.
        require(burnFee <= maxBurnFee); // Prevents owner from abusing fees.
    ...
    function setCharityWallet(address payable newChWallet) external onlyOwner {
        require(_charityWallet != newChWallet, "Wallet already set!");
        _charityWallet = payable(newChWallet); 
    }
     ...
     function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(_charityWallet, transferredBalance.div(_liquidityFee).mul(_buycharityFee));  
