...
// TODO : CRITICAL : Support native
// TODO : CRITICAL : Limit to EOA
contract BananaMaker is BananaBridgedSwapper {
    ...
    ) public BananaBridgedSwapper(_factory, _banana, _wBNB){
        apeBoss = _apeBoss;
        olaBank = _olaBank;
    ...
    function doDistributionInternal(address token) internal {
        // Send 10% of existing balance to apeWallet
        (uint bossPart, uint reminder) = sendToBoss(token);

        // Convert remaining balance to banana
        uint bananaGained = _convert(token);

        // Distribute banana by percentages
        (uint burnPart, uint supplyAndBurnPart, uint olaPart) = partitionBananas();

        // - Burn (make sure sending to address 0 is possible)
        burn(banana, burnPart);

        // - Mint oBanana and burn
        uint oBananasToBurn = mintOBananas(supplyAndBurnPart);
        burn(oBananaMarket, oBananasToBurn);

        // - Send to ola bank
        IERC20(banana).safeTransfer(olaBank, olaPart);
    }

    function sendToBoss(address token) internal returns (uint bossPart, uint reminder) {
        uint selfBalance = IERC20(token).balanceOf(address(this));
        bossPart = partFrom(selfBalance, tenPercent);
        // TODO : CRITICAL : Safe math
        reminder = selfBalance - bossPart;
    }
    ...
    function mintOBananas(uint amount) internal returns (uint oBananasMinted) {
    ...
    function partitionBananas() internal returns (uint burnPart, uint supplyAndBurnPart, uint olaPart) {
        ...
        burnPart = partFrom(bananaBalance, thirtyPercent);
        supplyAndBurnPart = partFrom(bananaBalance, fiftyFivePercent);
        // TODO : CRITICAL : SafeMath
        olaPart = bananaBalance - burnPart - supplyAndBurnPart;
    }

    function partFrom(uint amount, uint part) internal view returns (uint) {
        // TODO : CRITICAL : SafeMath
        return amount.mul(fullUnit).div(part);
    }
...
contract BananaBridgedSwapper {
    ...
    address internal immutable banana;
    ...
    function _swap(
        ...
        require(address(pair) != address(0), "BananaBridgedSwapper: Cannot convert");
    ...
    function _toBANANA(address token, uint256 amountIn)
    internal
