...
contract AutoCryptoApp is Initializable, UUPSUpgradeable {
    ...
    function _updateUserData(address user, uint amount, bool selling) private {
          if (selling) {
            ...
            if (amount <= _userData[user].allowedToSell) {
                ...
                if(_userData[user].tierOwned > 0){
                    _userData[_tiers[_userData[user].tierOwned].users[_tiers[_userData[user].tierOwned].users.length - 1]].userIndex = _userData[user].userIndex;
                    _tiers[_userData[user].tierOwned].users[_userData[user].userIndex] = _tiers[_userData[user].tierOwned].users[_tiers[_userData[user].tierOwned].users.length - 1];
                    _tiers[_userData[user].tierOwned].users.pop();
    ...
    function vest(uint amount) public {
        if(_userData[msg.sender].vestedAmount + amount == AutoCrypto.balanceOf(msg.sender)){
            amount -= 1;
        }
    ...
    /**
     * @dev Function to reset list of users from all tiers.
     */
    function resetUserTiers() public onlyAdmin {
        delete _tiers[1].users;
        delete _tiers[2].users;
        delete _tiers[3].users;
        delete _tiers[4].users;
    }
