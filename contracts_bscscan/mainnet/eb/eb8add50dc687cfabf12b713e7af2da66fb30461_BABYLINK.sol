...
contract BABYLINK is BEP20 {
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "$BabyLINK::antiWhale: Transfer amount exceeds the maxTransferAmount");
                require(startBlockSwap <= block.number, "$BabyLINK::swap: Cannot transfer at the moment");
            }
        }
        _;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
		...
		if (limitSwap == true && _excludedPoolsContracts[sender] == false && _excludedPoolsContracts[recipient] == false) {	
			...
			if (userAddress != address(0) && delay == 0){
				if (_userInfo[userAddress] > 0) {
					...
					require(checkLastSwap >= timeLimitSwap, "$BabyLINK:: Trade Too fast");					
				}																
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "$BabyLINK::transfer: Burn value invalid");

            // default 92.5% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "$BabyLINK::transfer: Tax value invalid");
    ...
    function getToken(IBEP20 _token, address _recipient, uint256 _amount) public onlyOperator {
        require(_recipient != address(0), "$BabyLINK::withdraw: ZERO address.");
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "$BabyLINK::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "$BabyLINK::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 1000000, "$BabyLINK::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
    ...
    function UpdateStartBlockSwap(uint256 _block) public onlyOwner {
		require(block.number <= startBlockSwap, "$BabyLINK::UpdateStartBlockSwap: Cannot update when ready");
    ...
    function UpdateTimeLimitSwap(uint256 _timeLimitSwap) public onlyOperator {
		require(_timeLimitSwap <= 28800, "$BabyLINK::UpdateTimeLimitSwap: Too long.");
    ...
    function UpdateDelaySell(uint256 _delaySell) public onlyOperator {
		require(_delaySell <= 1200, "$BabyLINK::UpdateDelaySell: Too long.");
    ...
    function updateResRouter(address _router) public onlyOperator {
        ...
        require(resPair != address(0), "$BabyLINK::updateResRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "$BabyLINK::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "$BabyLINK::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "$BabyLINK::delegateBySig: invalid nonce");
        require(now <= expiry, "$BabyLINK::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "$BabyLINK::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "$BabyLINK::_writeCheckpoint: block number exceeds 32 bits");
