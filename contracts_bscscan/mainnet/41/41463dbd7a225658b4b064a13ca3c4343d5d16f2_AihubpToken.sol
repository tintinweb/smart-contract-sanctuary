...
contract AihubpTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("AihubpToken_Dividend_Tracker", "AihubpToken_Dividend_Tracker", 9) {
    	claimWait = 14400;
        minimumTokenBalanceForDividends = 4000 * (10**9); //must hold 4000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "AihubpToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "AihubpToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AihubpToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "AihubpToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "AihubpToken_Dividend_Tracker: Cannot update claimWait to same value");
...
contract AihubpToken is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    AihubpTokenDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 public maxSellTransactionAmount = 12000000 * (10**9);
    uint256 public swapTokensAtAmount = 400000 * (10**9);
    ...
    constructor() public BEP20("AIHUBPTOKEN", "AIH", 9) {
    	...
    	dividendTracker = new AihubpTokenDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "AihubpToken: The dividend tracker already has that address");

        AihubpTokenDividendTracker newDividendTracker = AihubpTokenDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "AihubpToken: The new dividend tracker must be owned by the AihubpToken token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "AihubpToken: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "AihubpToken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "AihubpToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "AihubpToken: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "AihubpToken: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "AihubpToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "AihubpToken: Cannot update gasForProcessing to same value");
