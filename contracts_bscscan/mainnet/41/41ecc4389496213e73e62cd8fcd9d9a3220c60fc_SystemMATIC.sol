...
Stealth Launch $SystemMATIC
1 Year LP Lock, 5 BNB Initial LP

Just Hold and get your 10% MATIC reward

Tokenomic
- 100.000.000.000.000.000 Supply
- 5% Marketing Wallet
- 3% Dev Wallet
- 10% Reward Distribution in MATIC
- Max Buy 1% token supply in every transaction
- Max Sell 1% token supply in every transaction

  
Telegram Group : https://t.me/SystemMaticBSC
Website : https://systematic.cx/
...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minimumTokenBalanceForDividends = 1000000 * (10**9); // user must hold 1,000,000 token to receive MATIC
...
contract SystemMATIC is Ownable, IBEP20, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    string constant _name = "SystemMATIC";
    ...
    uint256 public _maxWallMATICold = 2000000000;
    ...
    function setMaxWallMATICold(uint256 amount) external onlyOwner {
        _maxWallMATICold = _totalSupply.mul(amount).div(100);
    }
    ...
    function checkTxLimit(address sender, address recipient, uint256 amount) internal view {
        if (!excludeMaxWallet[recipient])
            require(balanceOf(recipient) + amount <= _maxWallMATICold * (10**9) );
    ...
    function swapBack() internal swapping {
        ...
        try router.swapExactTokensForMATICSupportingFeeOnTransferTokens(
            swapAmount,
            0,
            path,
            address(this),
    ...
    function swapBnbForTokens(uint256 amount) private {
        // generate the uniswap pair path of token -> wMATIC
        address[] memory path = new address[](2);
        path[0] = router.WMATIC();
        path[1] = address(this);

        // make the swap
        router.swapExactMATICForTokensSupportingFeeOnTransferTokens{
            value: amount
        }(
