...
contract SQUIRTGAME is ERC20, Ownable {
    ...
    SQUIRTGAMEDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    uint256 public maxWalletBalance = 1500000000 * (10**9);
    ...
    address payable public _marketingWalletAddress = 0x2E3881BE6706cc379A74f71f54f0899597E4c1C5;
    ...
    constructor() public ERC20("SQUIRTGAME", "SQUIRT") {

    	dividendTracker = new SQUIRTGAMEDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SQUIRTGAME: The dividend tracker already has that address");

        SQUIRTGAMEDividendTracker newDividendTracker = SQUIRTGAMEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SQUIRTGAME: The new dividend tracker must be owned by the SQUIRTGAME token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SQUIRTGAME: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SQUIRTGAME: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SQUIRTGAME: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SQUIRTGAME: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SQUIRTGAME: Cannot update gasForProcessing to same value");
...
contract SQUIRTGAMEDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("SQUIRTGAME_Dividen_Tracker", "SQUIRTGAME_Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 400000 * (10**9);
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "SQUIRTGAME_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "SQUIRTGAME_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SQUIRTGAME contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "SQUIRTGAME_Dividend_Tracker: claimWait must be updated to between 20 mins and 24 hours");
        require(newClaimWait != claimWait, "SQUIRTGAME_Dividend_Tracker: Cannot update claimWait to same value");
