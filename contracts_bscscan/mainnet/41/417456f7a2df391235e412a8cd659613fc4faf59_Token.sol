...
contract Token is IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    function whitelistContracts(address[] calldata addresses, bool isWhitelist) external onlyOwner {
        for (uint i=0; i<addresses.length; i++) {
            require(addresses[i].isContract(), "Token: address is not contract");
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) private {
        require(!isBlacklisted[from], "Token: transfer from blacklisted address");
        require(!isBlacklisted[to], "Token: transfer to blacklisted address");
        ...
        if (hasBotControl && !isWhitelistFromBotControl[tx.origin]) {
            require(amount <= MAX_TX_LIMIT, "Token: transfer amount exceeds limit");

            if (!inSwapAndLiquify)
                require(block.timestamp - lastTransferedAt[tx.origin] >= 10 seconds, "Token: transfer within cooldown");
