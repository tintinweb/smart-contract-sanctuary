...
import "../../interfaces/dopple/IFairLaunch.sol";
import "../../interfaces/dopple/ILPToken.sol";
import "../../interfaces/dopple/ISwap.sol";
import "../../interfaces/dopple/ITwindexSwapRouter.sol";
...
contract DoppleByalanStableLP is ByalanIsland, Sailor, GasThrottler, IByalan, ReentrancyGuard {
    ...
    address public constant DOP = 0x844FA82f1E54824655470970F7004Dd90546bB28;
    address public override want;
    address public swapToken;

    // Third party contracts
    address public constant MASTERCHEF = 0xDa0a175960007b0919DBF11a38e6EC52896bddbE;
    uint256 public immutable pid;
    uint8 public immutable swapSize;
    uint8 public immutable busdIndex;
    ...
    {
        pid = _pid;
        want = IFairLaunch(MASTERCHEF).poolInfo(_pid).stakeToken;
        swapSize = _swapSize;
        swapToken = ILPToken(want).swap();
        busdIndex = ISwap(swapToken).getTokenIndex(BUSD);
    ...
    function deposit() public override whenNotPaused {
        ...
        if (wantBal > 0) {
            IFairLaunch(MASTERCHEF).deposit(address(this), pid, wantBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external override onlyIzlude {
        ...
        if (wantBal < _amount) {
            IFairLaunch(MASTERCHEF).withdraw(address(this), pid, _amount - wantBal);
    ...
    function harvest() external override whenNotPaused onlyEOA gasThrottle {
        IFairLaunch(MASTERCHEF).harvest(pid);
        chargeFees();
        addLiquidity();
        deposit();
    ...
    function chargeFees() internal nonReentrant {
        uint256 toBnb = (IERC20(DOP).balanceOf(address(this)) * totalFee) / MAX_FEE;
        ITwindexSwapRouter(unirouter).swapExactTokensForETH(toBnb, 0, dopToWbnbRoute, address(this), block.timestamp);
    ...
    function addLiquidity() internal {
        ...
        ITwindexSwapRouter(unirouter).swapExactTokensForTokens(
            dopBal,
            0,
            dopToBusdRoute,
            address(this),
        ...
        ISwap(swapToken).addLiquidity(amounts, 0, block.timestamp);
    }
    ...
    function balanceOfPool() public view override returns (uint256) {
        return IFairLaunch(MASTERCHEF).userInfo(pid, address(this)).amount;
    }
    ...
    function pendingReward() external view override returns (uint256) {
        return IFairLaunch(MASTERCHEF).pendingDopple(pid, address(this)) + IERC20(DOP).balanceOf(address(this));
    }
    ...
    function retireStrategy() external override onlyIzlude {
        IFairLaunch(MASTERCHEF).emergencyWithdraw(pid);
    ...
    function _giveAllowances() internal {
        ...
        IERC20(DOP).safeApprove(unirouter, type(uint256).max);
        IERC20(BUSD).safeApprove(swapToken, type(uint256).max);
    }
