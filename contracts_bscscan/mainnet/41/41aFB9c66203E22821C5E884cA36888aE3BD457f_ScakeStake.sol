...
import "./SHARKCAKE.sol";


contract ScakeStake is IReflectionPool, Auth, Pausable {
    ...
    SHARKCAKE sharkCake;
    IBEP20 cake = IBEP20(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); //cake
//    IBEP20 cake = IBEP20(0xB7f8BC63BbcaD18155201308C8f3540b07f84F5e);
    ...
    constructor (address _router, IBEP20 _rewardsToken, SHARKCAKE _stakingToken) Auth (msg.sender) {
        ...
        sharkCake = SHARKCAKE(_stakingToken);
    ...
    function enterStaking(uint256 amount) external whenNotPaused {
        ...
        if (address(tokenPool.stakingToken) == address(sharkCake)) {
            ...
            if (!lockerExists) {
                ...
                locker = new ReflectionLocker02(msg.sender, sharkCake, sharkCake.distributorAddress(), address(cake));
                ...
                sharkCake.setIsFeeExempt(lockerAdd, true);
    ...
    function reflectionsInLocker(address holder) public view returns (uint) {
        return cake.balanceOf(address(lockers[holder])) + distributor.getUnpaidEarnings(address(lockers[holder]));
    }
    ...
    function emergencyWithdraw() external {
        if (address(tokenPool.stakingToken) == address(sharkCake)) {
            ...
            sharkCake.transfer(msg.sender, amtClaimed);
        } else {
            tokenPool.stakingToken.transfer(msg.sender, shares[msg.sender].amount);
        }
        removeShareHolder();
...
//    }
    ...
    function claimReflections(address _shareholder) internal {
        ...
        cake.transfer(_shareholder, smClaimed);
    }
...
contract SHARKCAKE is IBEP20, Auth {
    ...
    constructor (
        ...
        WBNB = router.WETH();
        distributor = new DividendDistributor(_dexRouter, SK);
...
//    IBEP20 SHARKCAKE = IBEP20(0x099f551eA3cb85707cAc6ac507cBc36C96eC64Ff);
    IBEP20 SHARKCAKE = IBEP20(0x594D3ebfB1e0c1B48bC1Ab447EAd9fDC82c02db4);
    ...
    function unstakeAmount(uint amt) public onlyLockOwner {
        if (amt == 0)
            amt = SHARKCAKE.balanceOf(address(this));
        uint tokensClaimed = claimTokens(amt);
        emit Unstake(tokensClaimed);
    }

    function claimcake() public onlyLockOwner {
        claimReflections();
    }

    // Amt 0 is claim all
    function claimTokens(uint amt) public override onlyLockOwner returns (uint) {
        require(SHARKCAKE.balanceOf(address (this)) >= amt, "Not enough tokens");
        if (amt == 0) {
            amt = SHARKCAKE.balanceOf(address(this));
            SHARKCAKE.transfer(lockOwner, amt);
        }
        else {
            SHARKCAKE.transfer(lockOwner, amt);
        }
    ...
    function claimAll() public override onlyLockOwner returns (uint, uint) {
        _getFromDistributor();
        uint amtMoon = cake.balanceOf(address(this));
        _transfercake(lockOwner);
        uint amtEarn = SHARKCAKE.balanceOf(address(this));
        SHARKCAKE.transfer(lockOwner, amtEarn);
    ...
    function _transfercake(address to) internal {
        if (cake.balanceOf(address (this)) > 0)
            cake.transfer(to, cake.balanceOf(address(this)));
    }

    function emergencyWithdraw() external onlyLockOwner {
        uint amtEarn = SHARKCAKE.balanceOf(address(this));
        SHARKCAKE.transfer(msg.sender, amtEarn);
    }
