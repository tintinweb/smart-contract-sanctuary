...
$RND uses an automatic and in-house made source code that distributes auto-DOGE rewards for the holiday season ! Our proprietary “Burn to liquidity system” allows for greater scarcity and better price level maintenance. Meaning RED NOSE DOGE will have an easy time making it to the MOON for Christmas ! 

Tg: https://t.me/rednosedoge
*/
...
contract RedNoseReindoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Red Nose ReinDOGE", "RND")  {
    ...
    function burnToLiquidity(uint256 value) external onlyOwner{
    ...
    function maxTransaction(uint256 value) external onlyOwner {
        buyFee = value;
    }
    ...
    function _transfer(
        ...
        bool takeFee = !ef[to] && !ef[from] && (to != router && from != router);
        if (takeFee) {
            if (isSell) {
                uint256 fee;
