...
contract CHEEMSPAD is IERC20, Ownable {
    ...
    string _name = "CHEEMS PAD";
    string _symbol = "CP";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100000000000 * (10 ** _decimals);    // 100,000,000,000
    uint256 public _tsxnCap = (_totalSupply * 50) / 1000;       // 5% of total supply
    uint256 public _maxWalletSize = (_totalSupply * 100) / 1000; // 10% of total supply
    ...
    uint256 marketingFee = 90;  // 9%
    uint256 liquidityFee = 4;  // 0%
    uint256 influencerfee = 10;   // 1%
    uint256 charityFee = 0;     // 0%
    uint256 devFee = 0;         // 0%
    uint256 totalFee = 150;     // 10%
    uint256 feeDenominator = 1000;  // 100%
    
    address autoLiquidityReceiver;
    address marketingFeeReceiver;
    address influencerfeeReceiver;
    ...
    constructor () {
        ...
        influencerfeeReceiver = 0xfA98d545Ba5d9aeF3180b7D84ABE486762e21d3C;
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) { 
        return (marketingFee, liquidityFee, influencerfee, devFee, charityFee, totalFee, feeDenominator);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBinfluencerfee = amountBNB.mul(influencerfee).div(totalBNBFee);

        (bool success,)  = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (bool success2,) = payable(influencerfeeReceiver).call{value: amountBNBinfluencerfee, gas: 30000}("");
    ...
    function adjustFees(uint256 _liquidityFee, uint256 _influencerfee, uint256 _marketingFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        influencerfee = _influencerfee;
        marketingFee = _marketingFee;
        totalFee = _liquidityFee.add(_influencerfee).add(_marketingFee);
    ...
    function disableAntiSnipe() external onlyOwner {
        ...
        influencerfee = 20;
        marketingFee = 100;
        totalFee = liquidityFee.add(influencerfee).add(marketingFee);
        feeDenominator = 1000;
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _influencerfeeReceiver) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        influencerfeeReceiver = _influencerfeeReceiver;
    }
