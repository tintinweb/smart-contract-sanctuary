    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        require(_token != address(0), "Should not be the zero address");
    ...
    function autoCompoundInterest(
        ...
        // Sending amountClaimed from distributor to DAO.
        IDistributor(distributor).sendTokenAmount(DAO, amountClaimed);
    ...
    function withdraw(uint256[] memory depositIds) public nonReentrant {
        ...
        IDistributor(distributor).sendTokenAmount(
            address(this),
            amountInterest
        );

        IERC20Upgradeable(token).safeTransfer(depositor, totalAmount);

        userCurrentDeposits[depositor] -= depositIds.length;

        updateAdjustedTokenDepositedAmount(adjAmountPrincipal, false);

        emit Withdrawal(depositor, depositIds, totalAmount);
    }
    ...
    /*
     * The following function fully withdraws t-1 deposits.
     * The last deposit Id is partially withdrawn.
     */
