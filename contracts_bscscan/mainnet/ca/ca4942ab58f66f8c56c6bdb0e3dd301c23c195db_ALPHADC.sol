...
contract ALPHADCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("ALPHADC_Dividend_Tracker", "ALPHADC_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "ALPHADC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ALPHADC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ALPHADC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ALPHADC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "ALPHADC_Dividend_Tracker: Cannot update claimWait to same value");
...
contract ALPHADC is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    ALPHADCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Alpha DogeCoin", "ALPHADC") {
        BNBRewardsFee = 0;
        ...
        marketingFee = 0;
        BuyBackFee = 0;
        marketingWallet = payable(0xb3EA93E70f0D7C48BA983E123e7F4d9FCfB4C42f);
        BuyBackWallet = payable(0xf68a7c6266652d32F5020Fd01501e667bD92483E);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(BuyBackFee); // total fee transfer and buy

    	dividendTracker = new ALPHADCDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ALPHADC: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ALPHADC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ALPHADC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ALPHADC: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ALPHADC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ALPHADC: Cannot update gasForProcessing to same value");
