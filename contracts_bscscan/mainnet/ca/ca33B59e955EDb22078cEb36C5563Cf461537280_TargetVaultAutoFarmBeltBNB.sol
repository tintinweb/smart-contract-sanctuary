...
contract TargetVaultAutoFarmBeltBNB is TargetVaultAutoFarm {
    ...
    function _collectFees() internal override {
        if (depositedBalance > 0) {
            uint256 _newPricePerShare = vaultBalance().mul(1e18).div(depositedBalance);
    ...
    function _harvest() internal override {
        ...
        if (swapAmt > 0) {
            ...
            IWBNB(wBNB).withdraw(IERC20(wBNB).balanceOf(address(this)));
            _deposit();
        }
    }
    ...
    function vaultToken() public view override returns (address) {
        return address(beltToken.token());
    }
}
