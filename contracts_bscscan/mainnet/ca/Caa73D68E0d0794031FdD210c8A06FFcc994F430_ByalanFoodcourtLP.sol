...
contract ByalanFoodcourtLP is ByalanIsland, Sailor, GasThrottler, IByalan, ReentrancyGuard {
    ...
    constructor(
        ...
        if (lpToken0 == WBNB) {
            couponToLp0Route = [COUPON, WBNB];
        } else if (lpToken0 != COUPON) {
            couponToLp0Route = [COUPON, WBNB, lpToken0];
        }

        if (lpToken1 == WBNB) {
            couponToLp1Route = [COUPON, WBNB];
        } else if (lpToken1 != COUPON) {
            couponToLp1Route = [COUPON, WBNB, lpToken1];
        }
    ...
    function chargeFees() internal nonReentrant {
        uint256 toWbnb = (IERC20(COUPON).balanceOf(address(this)) * totalFee) / MAX_FEE;
    ...
    function addLiquidity() internal {
        uint256 couponHalf = IERC20(COUPON).balanceOf(address(this)) / 2;
