...
contract BTCMania is ERC20, Ownable {
    ...
    constructor() ERC20("BTC Mania", "BTCMania") {
    	...
    	buyBackWallet = 0xA6Cc74beb036989B5727702116F42A8cE7BdAecE;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "BTCMania: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "BTCMania: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "BTCMania: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "BTCMania: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "BTCMania: Dividend reward tax must be between 0 and 10");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "BTCMania: The liquidity wallet is already this address");
...
contract BTCManiaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BTCMania_Dividend_Tracker", "BTCMania_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function withdrawDividend() pure public override {
        require(false, "BTCMania_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BTCMania contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BTCMania_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
