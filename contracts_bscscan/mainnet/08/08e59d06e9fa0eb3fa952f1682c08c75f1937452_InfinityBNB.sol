...
contract InfinityBNB is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    InfinityBNBDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable public operations1Address =
        payable(0x1E0638cC44Bc7AE8602ccf93623d60CA540d0679); //Marketing Wallet 3%
    address payable public operations2Address =
        payable(0xb99440363272246029783aa3871abc24e7C484fb); //Buyback wallet 5%
    ...
    uint256 public devFees = 8; // Dev 0.5% from here, Marketing 2.5% and Buyback is 5% 
    ...
    constructor() ERC20("InfinityBNB", "InfinityBNB") {
        dividendTracker = new InfinityBNBDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        InfinityBNBDividendTracker newDividendTracker = InfinityBNBDividendTracker(
            payable(newAddress)
        );
...
contract InfinityBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    InfinityBNB public infinityBNB;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "InfinityBNB_Dividend_Tracker",
            "InfinityBNB_Dividend_Tracker"
        )
    {
        infinityBNB = InfinityBNB(mainContract);
    ...
    function _transfer(
        ...
        require(false, "InfinityBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "InfinityBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main InfinityBNB contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(infinityBNB);

            uint256 prevBalance = infinityBNB.balanceOf(address(this));
            ...
            {
                uint256 received = infinityBNB.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    infinityBNB.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
