...
contract Swap is Ownable, ReentrancyGuard {
  ...
  function swap(uint256 amount) external payable nonReentrant {
    address baseToken = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    address fToken = address(0x14016E85a25aeb13065688cAFB43044C2ef86784);
    require(amount > baseToken.myBalance(), "insufficient balance");
    (uint256 routeIndex,) = query.oneRoute(baseToken, fToken, amount, routerIndex);
    baseToken.safeApprove(address(bestrate), 0);
    baseToken.safeApprove(address(bestrate), uint(-1));
    uint256 amountOut = bestrate.trade.value(0)(routeIndex, baseToken, amount, fToken, 0, partnerIndex);
    require(amountOut > 0, "cannot trade.");
    fToken.safeTransfer(msg.sender, fToken.balanceOf(address(this)));
  }
