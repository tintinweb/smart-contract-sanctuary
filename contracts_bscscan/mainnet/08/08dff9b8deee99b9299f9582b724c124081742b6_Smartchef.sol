...
interface ICakeJackPotReferral {
    /**
     * @dev Record referral.
     */
...
contract Smartchef is Ownable {
    ...
    ICakeJackPotReferral public CakeJackPotReferral;
    ...
    constructor(address _tokenAddress) public {
        DOGE = IBEP20(_tokenAddress);
        DOGE.approve(address(this), MAX_UINT256);
        totalStakedAmount = 0;
    }
    ...
    function deposit(uint256 _amount, bool _isCompounding, address _referrer) public {
        UserInfo storage user = userInfo[msg.sender];

        if (_amount > 0 && address(CakeJackPotReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            CakeJackPotReferral.recordReferral(msg.sender, _referrer);
        }

        if (user.amount > 0) {
            // give reward
            uint256 pendingRewardToken = getRewardAmount(user);
            if(pendingRewardToken > 0 && DOGE.balanceOf(address(this)) > pendingRewardToken) {
                DOGE.safeTransferFrom(address(this), address(msg.sender), pendingRewardToken);
                payReferralCommission(msg.sender, pendingRewardToken);
            }
        }

        if(_amount > 0) {
            _amount = takeDepositFee(_amount);
            DOGE.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function withdraw(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            DOGE.safeTransferFrom(address(this), address(msg.sender), _amount);
        }
    ...
    function emergencyWithdraw() public {
        ...
        DOGE.safeTransferFrom(address(this), address(msg.sender), amount);
    ...
    function emergencyRewardWithdraw(uint256 _amount) public onlyOwner {
        require(_amount <= DOGE.balanceOf(address(this)), 'not enough token');
        DOGE.safeTransferFrom(address(this), address(msg.sender), _amount);
    }

    function takeDepositFee(uint256 _amount) internal returns (uint256) {
        uint256 feeAmount = _amount.mul(depositFee).div(10000);
        DOGE.safeTransferFrom(address(msg.sender), feeAddress1, feeAmount.div(2));
        DOGE.safeTransferFrom(address(msg.sender), feeAddress2, feeAmount.div(2));
    ...
    function setCakeJackPotReferral(ICakeJackPotReferral _CakeJackPotReferral) public onlyOwner {
        CakeJackPotReferral = _CakeJackPotReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _amount) internal {
        if (address(CakeJackPotReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = CakeJackPotReferral.getReferrer(_user);
            uint256 commissionAmount = _amount.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                DOGE.safeTransferFrom(address(this), referrer, commissionAmount);
                CakeJackPotReferral.recordReferralCommission(referrer, commissionAmount);
