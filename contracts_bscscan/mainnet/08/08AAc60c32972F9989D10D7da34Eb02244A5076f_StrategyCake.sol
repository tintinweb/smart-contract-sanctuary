...
interface IZefiBurningVault {
    function depositAll() external;
    function zvdepositAll() external;
}
...
contract GasThrottler {
    ...
    address public gasprice = address(0x2D3fa7EC56A25aC37a6328F1896ad359B4E1f638);
...
// File: contracts/BIFI/strategies/degens/StrategyCake.sol
 ...
 */
contract StrategyCake is Ownable, Pausable, GasThrottler {
    ...
    address constant public zvzefi     = address(0xB869e8b466d16f499039F377C0642e169a7bA698);
    ...
    uint8 constant public poolId = 0;
     ...
     */
    address constant public rewards  = address(0x8D0868bD02b2682fBD7d4BdD55c8fC23c0894606);
     ...
     */
    uint constant public TREASURY_FEE   = 800;
    ...
    address[] public wantToBifiRoute = [want, wbnb, bifi];
    ...
    constructor(address _vault, address _strategist) public {
        ...
        IERC20(bifi).safeApprove(unirouter, uint(-1));
        IERC20(bifi).safeApprove(masterchef, uint(-1));   

        IERC20(want).safeApprove(zvzefi, uint(-1));
        IERC20(wbnb).safeApprove(zvzefi, uint(-1));
        IERC20(bifi).safeApprove(zvzefi, uint(-1));
    }
    ...
    function chargeFees() internal {
        ...
        // uint256 toWbnb = IERC20(want).balanceOf(address(this)).mul(50).div(1000);
        uint256 toWbnb = IERC20(want).balanceOf(address(this)); // ALL
        IUniswapRouter(unirouter).swapExactTokensForTokens(toWbnb, 0, wantToBifiRoute, address(this), now.add(600));
        // uint256 wbnbBal = IERC20(wbnb).balanceOf(address(this));


        uint256 zefiBal = IERC20(bifi).balanceOf(address(this));
        IERC20(bifi).safeTransfer(vault, zefiBal);
        
        // IZefiBurningVault(zvzefi).depositAll();
        IZefiBurningVault(vault).zvdepositAll();
