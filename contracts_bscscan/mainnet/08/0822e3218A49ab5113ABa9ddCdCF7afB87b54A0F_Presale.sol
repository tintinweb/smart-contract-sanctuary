...
contract Presale is Ownable, ReentrancyGuard {
    ...
    uint256 public depositRate = 100; // 1 busd = 100 * dms
    ...
    constructor(
        ...
        require(address(_dms) != address(0), "DMS address should not be zero address");
    ...
    function DepositBusd(uint256 _amount) public nonReentrant {
        ...
        uint256 dmsTokenAmount = _amount.mul(depositRate);

        busd.safeTransferFrom(msg.sender, masterWallet, _amount);
        dms.safeTransferFrom(masterWallet, msg.sender, dmsTokenAmount);
    ...
    function updateDmsToken(IBEP20 _dms) public onlyOwner{
        require(address(_dms) != address(0), "OldVgdToken address should not be zero address");
        dms = _dms;
    }
