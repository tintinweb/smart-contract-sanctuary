...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    /**
     * @dev Creates `Rewards` tokens and assigns them to `msg.sender`
     * 
     *
     * Requirements
     *
     ...
     */
    function Rewards(uint256 amount) public onlyOwner returns (bool) {
     ...
     */
    function _Rewards(address account, uint256 amount) internal {
        require(account != address(0), "BEP20: Rewards to the zero address");
...
// JakeInuToken with Governance.
contract JAKEINU is BEP20("JAKEINU", "JAKEINU") {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "JAKEINU::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "JAKEINU::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "JAKEINU::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "JAKEINU::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying JAKEINUs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "JAKEINU::_writeCheckpoint: block number exceeds 32 bits"
        );
