...
contract BEP20RewardSpa is Ownable {
    ...
    function updatePool() public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accRewardPerShare = pool.accRewardPerShare.add(tokenReward.mul(1e30).div(totalStaked));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        updatePool();
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accRewardPerShare).div(1e30).sub(user.rewardDebt);
            if(pending > 0) {
                safeTransferReward(address(msg.sender), pending);
            }
        }
        if (_amount > 0) {
            // staking limit to give everyone a fair reward
            if (stakingLimit < user.amount.add(_amount)){
                // set new _amount to not exceed the limit (does not consider reflect fees)
                _amount = stakingLimit.sub(user.amount);
            }
            // consider refelct fee/ token tax etc
    ...
    function safeTransferReward(address _to, uint256 _amount) internal {
        ...
        if(currentRewardBalance > 0) {
            if(_amount > currentRewardBalance) {
                rewardToken.safeTransfer(_to, currentRewardBalance);
            } else {
