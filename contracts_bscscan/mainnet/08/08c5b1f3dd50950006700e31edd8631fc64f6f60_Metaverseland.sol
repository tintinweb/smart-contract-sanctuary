...
contract Metaverseland is Context, IBEP20, IBEP20Metadata,Ownable {
    ...
    string private _name = "Metaverseland";
    string private _symbol = "MTLAND";
    ...
    /**
     * @dev Throws if called by any account other than the farmingContract.
     */
    modifier onlyFarmingContract() {
        require(farmingContract == _msgSender(), "MTLAND: caller is not the FarmingContract");
        _;
    }
    ...
    function sweep(address _tokenAddress) public onlyOwner{
        uint256 amount = IBEP20(_tokenAddress).balanceOf(address(this));
        if(amount > 0){
            IBEP20(_tokenAddress).transfer(msg.sender,amount);
        }
    }
    ...
    /**
     * @dev Send `amount` of tokens to `Farmer with NFTs`.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
     *
     * - `farmer` cannot be the zero address.
     * - `sender` must be FarmingContract address.
     */
    function farming(address farmer, uint256 tokenAmount) public onlyFarmingContract {
         require(farmer != address(0), "MTLAND: to the zero address");

        _totalSupply = _totalSupply.add(tokenAmount);
        _balances[farmer] = _balances[farmer].add(tokenAmount);
        emit Transfer(address(0), farmer, tokenAmount);
