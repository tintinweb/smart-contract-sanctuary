...
contract ObamaPresale is Ownable, ReentrancyGuard {
  ...
  constructor (address _obamaToken, uint256 _minInvestFund, uint256 _maxInvestFund) {
    obamaToken = IERC20(_obamaToken);
  ...
  function getAvailableToken(address investor) public view returns(uint256) {
    require(stages.length > 0, "Obama Sales: Invalid Stage Count");
  ...
  function setStage(
    ...
    require(stageIndex < stages.length, "Obama Sales: Invalid Stage Index");
  ...
  function setInvestRange(
    ...
    require(_min < _max, "Obama Sales: Invalid Range");
    minInvestFund = _min;
    maxInvestFund = _max;
  }
  ...
  function setVestingStrategy(
    ...
    require(_tge <= 100, "Obama Sales: invalid tge percentage");
    require(_duration > 0, "Obama Sales: invalid duration");

    tge = _tge;
    duration = _duration;
  }

  function setIFansToken(address _obamaToken) external onlyOwner {
    require(_obamaToken != address(0), "Obama Sales: zero token address");
    obamaToken = IERC20(_obamaToken);    
  }
  ...
  function invest() external payable {
    ...
    require(_bnbAmount != 0, "Obama Sales: Zero Fund");
    require(_bnbAmount >= minInvestFund && _bnbAmount <= maxInvestFund, "Obama Sales: Invalid Investment Amount");

    (uint256 stageIndex, bool isLive) = getCurrentStage();

    require(isLive, "Obama: Stage is not live now");
  
    uint256 newAmountRaised = stages[stageIndex].amountRaised.add(_bnbAmount);
    require(newAmountRaised <= stages[stageIndex].fundingGoal, "Obama Sales: Too much investment");
  ...
  function claim() external  _afterDeadline nonReentrant {
    require(balanceOf[msg.sender] > 0, "Obama Sales: No tokens to claim");

    uint256 tokensAvailable = getAvailableToken(msg.sender);
    require(tokensAvailable > 0, "Obama Sales: No tokens to claim");

    obamaToken.transfer(msg.sender, tokensAvailable);
  ...
  function depositToken(uint256 _amount) external onlyOwner {
    obamaToken.transferFrom(msg.sender, address(this), _amount);
  }

  function forwardStageFunds(uint256 stageIndex, uint256 percent, address beneficiary) external onlyOwner {
    require(stageIndex < stages.length, "Obama Sales: invalid stage index");
    require(block.timestamp > stages[stageIndex].closeTime, "Obama Sales: stage not ended yet");
    require(stages[stageIndex].forwarded == false, "Obama Sales: stage funds already forwarded");

    require(percent <= 19, "Obama Sales: too much percent");

    uint256 totalAmount = stages[stageIndex].amountRaised;
    uint256 forwardBalance = totalAmount.mul(percent).div(100);

    require(forwardBalance <= address(this).balance, "Obama Sales: insufficient balance");
  ...
  function forwardBNB(address beneficiary) external onlyOwner {
    require(beneficiary != address(0), "Obama Sales: zero beneficiary address");
  ...
  function withdrawToken() external onlyOwner {
    ...
    uint256 currentBalance = obamaToken.balanceOf(address(this));

    uint256 withdrawBalance = currentBalance.add(totalClaimed).sub(totalSold);

    require(withdrawBalance > 0, "Obama Sales: Zero Token");

    obamaToken.transfer(msg.sender, withdrawBalance);
  }
