...
/**
 Telegram: https://t.me/miniSAFEEARNbof
  * miniSAFEEARN is a deflationary BSC token with a SAFEEARN reward. Are you a holder of SAFEEARN? If yes, this token is to support and increase the value of your bag 
    If no, SAFEEARN is the first double reflection token with SAFEMOON reward. With the success of SAFEEARN, it maybe too expensive for you to buy in current market cap.
    miniSAFEEARN is here for you.
 * 1. BNB Reward Pool - 10% of every transaction is sent to the  Reward Pool
 * 2. Automatic Buyback - 7% of every transaction is sent back to the contract for LP.
*/
...
// File: contracts/miniSAFEEARNDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable DividendsToken = address(0x099f551eA3cb85707cAc6ac507cBc36C96eC64Ff);
...
// File: contracts/miniSAFEEARN.sol

//

pragma solidity ^0.7.6;

contract miniSAFEEARN is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    miniSAFEEARNDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("miniSAFEEARN", "miniSAFEEARN") {
    	...
    	dividendTracker = new miniSAFEEARNDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "miniSAFEEARN: The dividend tracker already has that address");

        miniSAFEEARNDividendTracker newDividendTracker = miniSAFEEARNDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "miniSAFEEARN: The new dividend tracker must be owned by the miniSAFEEARN token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "miniSAFEEARN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "miniSAFEEARN: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "miniSAFEEARN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "miniSAFEEARN: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "miniSAFEEARN: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "miniSAFEEARN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "miniSAFEEARN: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "miniSAFEEARN: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of miniSAFEEARN
            path,
            recipient,
            block.timestamp
        );
...
contract miniSAFEEARNDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("miniSAFEEARN_Dividend_Tracker", "miniSAFEEARN_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "miniSAFEEARN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "miniSAFEEARN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main miniSAFEEARN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "miniSAFEEARN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "miniSAFEEARN_Dividend_Tracker: Cannot update claimWait to same value");
