...
import "../interfaces/IDOv3/IIDOv3Controller.sol";

/** @title IDO contract does IDO
 * @notice
 */

contract IDOv3 is Ownable, ReentrancyGuard {
    ...
    IIDOv3Controller public controller;
    ...
    mapping(uint256 => uint256) public roundsTotalSold;
    ...
    modifier canRaise(address addr, uint256 amount) {
        ...
        if (currentRoundId <= 4) {
            ...
            require(
                roundsFundRaised[currentRoundId] + amount <= getRoundTotalAllocation(currentRoundId),
                "Target hit!"
            );
        } else {
     ...
     */
    constructor(
        IIDOv3Controller _controller,
        uint256 _saleTarget,
        address _fundToken,
        uint256 _fundTarget
    ) {
        require(address(_controller) != address(0), "Invalid controller");
        require(_saleTarget > 0 && _fundTarget > 0, "Zero sale target or fundTarget!");
    ...
    function setBaseData(
        ...
        require(_minFundAmount > 0, "Zero _minFundAmount");
        require(_baseAmount > 0, "Zero _baseAmount");
    ...
    function getCurrentRoundId() public view returns (uint256) {
        ...
        // 1: gold, 2: silver, 3: bronze, 4: iron, 5: white, 6: fcfs
    ...
    function getMaxAllocation(address addr) public view returns (uint256) {
        ...
        if (currentRoundId <= 4) {
            // tier period
            if (5 - controller.getTierId(addr) == currentRoundId) {
        ...
        isOperatorOrOwner
    {
        ...
        require(total <= allocationMultiplier, "Total should be less than 100%");
    }

    function setStartTime(uint256 _startTime) external isOperatorOrOwner isNotStarted {
        require(_startTime > block.timestamp, "Past startTime");
        startTime = _startTime;
    ...
    function setEndTime(uint256 _endTime) external isOperatorOrOwner isNotEnded {
        require(_endTime > block.timestamp && _endTime > startTime, "Invalid EndTime");
    ...
    function setClaimTime(uint256 _claimTime) external isOperatorOrOwner {
        require(_claimTime > block.timestamp && _claimTime > endTime, "Invalid ClaimTime");
    ...
    function _processBuy(address buyer, uint256 amount) private {
        ...
        roundsFundRaised[currentRoundId] = roundsFundRaised[currentRoundId] + amount;
        roundsTotalSold[currentRoundId] = roundsTotalSold[currentRoundId] + saleTokenAmount;
