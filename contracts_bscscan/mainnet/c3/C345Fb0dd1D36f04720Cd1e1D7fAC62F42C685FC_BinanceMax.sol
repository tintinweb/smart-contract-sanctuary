...
contract BinanceMax is Context, IBEP20, Ownable{
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 100000 * 10**6 * 1e9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Binance Max";
    ...
    uint256 _taxFee;
    uint256 public _taxFeeForBuying = 0; // 0% will be distributed among holder as BMAX divideneds
    uint256 public _taxFeeForSelling = 0; // 0% will be distributed among holder as BMAX divideneds
    ...
    uint256 _buyBackFee; 
    uint256 public _buyBackFeeForBuying = 40;  // 2.5% will go to the buyBack address on each buy
    uint256 public _buyBackFeeForSelling = 40;  // 3.5% will go to the buyBack address on each sell
    
    uint256 _marketFee;
    uint256 public _marketFeeForBuying = 30;  // 2.5% will go to the market address on each buy
    uint256 public _marketFeeForSelling = 30;  // 3.5% will go to the market address on each sell
    ...
    uint256 public minTokenNumberToSell = 2000 * 10**6 * 10**9;
    uint256 public maxHoldingAmount = _tTotal.mul(4).div(100); // 8% of total supply by default
    ...
    uint256 public buyCoolDownDuration = 30 seconds;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "BMAX: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "BMAX: decreased allowance below zero"));
        return true;
    }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "BMAX: approve from the zero address");
        require(spender != address(0), "BMAX: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "BMAX: transfer from the zero address");
        require(to != address(0), "BMAX: transfer to the zero address");
        require(amount > 0, "BMAX: Transfer amount must be greater than zero");

        if(!_isExcludedFromMaxTx[from] && 
            !_isExcludedFromMaxTx[to]   // by default false
        ){
            require(amount <= maxTxAmount,"BMAX: Amount exceed max limit");

            if(to != uniSwapPair){
                require(balanceOf(to).add(amount) <= maxHoldingAmount,"BMAX: max holding limit exceeds");
            }

            if(from == uniSwapPair){
                require(buyCoolDownTriggeredAt.add(buyCoolDownDuration) < block.timestamp,
                "BMAX: wait for buying cool down"
                );
            ...
            if(to == uniSwapPair){
                ...
                "BMAX: wait for selling cool down"
                );
