...
contract BigBeansGaming is Context, IERC20 {
    ...
    string constant private _name = "Big Beans Gaming";
...
//===============================================================================================================
    ...
    function setTaxesBuy(uint16 reflect, uint16 bigBeans, uint16 littleBeans, uint16 marketing) external onlyOwner {
        require(reflect <= staticVals.maxReflectFee
                && bigBeans <= staticVals.maxBigBeansFee
                && littleBeans <= staticVals.maxLittleBeansFee
                && marketing <= staticVals.maxMarketingFee);
        require(bigBeans + littleBeans + reflect + marketing <= 3450);
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if (lpPairs[to]) {
            ...
            if (buybackEnabled && from != address(this)) {
                bnbBalance = dexRouter.getAmountsOut(amount, pairPath)[1];
                tokenBalance = dexRouter.getAmountsOut(amount, pairPath)[0];
    ...
    function _getValues(address from, address to, uint256 tAmount, bool takeFee) private returns (ExtraValues memory) {
        ...
        if(takeFee) {
            ...
            values.tLiquidity = (tAmount * (currentTaxes.bigBeans + currentTaxes.marketing)) / staticVals.masterTaxDivisor;
