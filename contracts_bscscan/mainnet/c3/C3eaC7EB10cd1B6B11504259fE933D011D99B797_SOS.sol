...
contract TokenPool is Ownable {
    ...
    function rescueToken(address tokenToRescue, address to, uint256 amount) external onlyOwner {
        TransferHelper.safeTransfer(tokenToRescue, to, amount);
    }
}

contract SOS is Context, IERC20, Ownable, LGEWhitelisted {
    ...
    string private _name = 'SOSFOUNDATION.io';
    ...
    function setCommunity(address a, uint256 buyFeePct, uint256 sellFeePct) external onlyOwner {
        
        _communityAddress = a;
        _communityBuyFeePct = buyFeePct;
    ...
    function setLP(uint256 sellFeePct) external onlyOwner {
        
        _lpSellFeePct = sellFeePct;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
            ...
            if(!_isExcluded[sender]) {
    			...
    			if((communitySellFeeAmount + lpSellFeeAmount) > 0) {
                    ...
                    r.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                        ...
                        0,
                        path,
                        address(_quoteTokenPool),
                        block.timestamp
                    );
                    
                    uint256 quoteAmount = _quoteTokenPool.balance().sub(quoteBalance);
        ...
        } else if (_pair[sender] && !_pair[recipient]) {
            //buying
        ...
        if(balanceOf(sender) >= _holderMinBalance) _holders.add(sender); else _holders.remove(sender);
        if(balanceOf(recipient) >= _holderMinBalance) _holders.add(recipient); else _holders.remove(recipient);

    }

    function _transferStandard(address sender, address recipient, uint256 tAmount, uint256 tFee) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount) = _getValues(tAmount, tFee);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _getValues(uint256 tAmount, uint256 tFee) private view returns (uint256, uint256, uint256, uint256) {
    ...
    function rescuePoolToken(address tokenToRescue, address to, uint256 amount) external onlyOwner {
        _quoteTokenPool.rescueToken(tokenToRescue, to, amount);
    }
}