 ...
 * For a generic mechanism see {ERC20PresetCreatorPauser}.
 *
...
contract ERC20 is Context, IERC20 {
     ...
     * BUSD and Wei. This is the value {ERC20} uses, unless {_setupDecimals} is
     * called.
     *
...
@author Roger Wu (https://github.com/roger-wu)
@dev A creatable ERC20 token that allows anyone to pay and distribute ether
...
contract FlokiEstate is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Floki Estate", "$FLKE") {
    	busdDividendTracker = new BusdDividendTracker();

    	marketingWallet =0xCce2418D1E898A2AD1E1EA1a7bcf5932259a71dF; 
    	devWallet = 0xbb3F938892076e89cAE87b61392746B97B6cc3f5;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "FlokiEstate: The marketing wallet is already this address");
  	...
  	function updateDevWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != devWallet, "FlokiEstate: The dev wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function afterPreSale() external onlyOwner {
        ...
        maxWalletToken = 300000000 * (10**18);
    }
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(busdDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setBusdDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = busdDividendRewardsFee.add(marketingFee).add(buyBackAndLiquidityFee);
            busdDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(busdDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateBusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "FlokiEstate: The dividend tracker already has that address");

        BusdDividendTracker newBusdDividendTracker = BusdDividendTracker(payable(newAddress));

        require(newBusdDividendTracker.owner() == address(this), "FlokiEstate: The new dividend tracker must be owned by the FlokiEstate token contract");
    ...
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = buyBackAndLiquidityFee.add(busdDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "FlokiEstate: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "FlokiEstate: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FlokiEstate: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "FlokiEstate: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "FlokiEstate: Cannot update gasForProcessing to same value");
    ...
    function getAccountBusdDividendsInfo(address account)
 	...
 	function getAccountBusdDividendsInfoAtIndex(uint256 index)
  	...
  	function processDividendTracker(uint256 gas) external onlyOwner {
		(uint256 ethIterations, uint256 ethClaims, uint256 ethLastProcessedIndex) = busdDividendTracker.process(gas);
		emit ProcessedBusdDividendTracker(ethIterations, ethClaims, ethLastProcessedIndex, false, gas, tx.origin);
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (marketingEnabled) {
                    ...
                    uint256 devPortion = address(this).balance.div(10**2).mul(40);
                ...
                if (busdDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(busdDividendRewardsFee).mul(busdDividendRewardsFee);
...
contract BusdDividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("FlokiEstate_Busdereum_Dividend_Tracker", "FlokiEstate_Busdereum_Dividend_Tracker", 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 100000 * (10**18); //must hold 100000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "FlokiEstate_Busdereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "FlokiEstate_Busdereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FlokiEstate contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "FlokiEstate_Busdereum_Dividend_Tracker: Cannot update claimWait to same value");
