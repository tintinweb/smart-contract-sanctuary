...
/**
 *Submitted for verification at BscScan.com on 2021-11-05 by CRYPTEX TEAM
*/
...
contract XAEA12 is IBEP20, Auth {
    using SafeMath for uint256;

    address WBNB = 0x144383c2b732DB480E1658cE035a2585DE10EBa0;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "XAEA12";
    string constant _symbol = "XAEA12";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 100000000000 * (10 ** _decimals);
    uint256 _burntSupply = (_totalSupply * 40) /100;
    ...
    address private marketingFeeReceiver = 0x6e60D38fF071d06FbC4b5bB3daEd1efAaeCB5402;
    address private ethBridgeAndExchange = 0x10ED43C718714eb63d5aA57B78B54704E256024E; 
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        if(selling) {   return totalFee.add(burnFee).mul(multiplier); }
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
         uint256 feeAmount = amount.mul(getTotalFee(receiver == pair)).div(feeDenominator); //24k
        if(receiver == pair) {
            uint256 burnAmount = amount.mul(burnFee).div(feeDenominator); //2k
            uint256 newFeeAmount = feeAmount.sub(burnAmount); //22k
              emit Transfer(sender, DEAD, burnAmount); //transfer 2k
              
            _balances[address(this)] = _balances[address(this)].add(newFeeAmount); //add 22k
            
            emit Transfer(sender, address(this), newFeeAmount); //send 22k
            return amount.sub(feeAmount); //return 26k
        }
        else {
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function setFees(uint256 _liquidityFee,  uint256 _marketingFee, uint256 _burnFee, uint256 _feeDenominator) external authorized {
