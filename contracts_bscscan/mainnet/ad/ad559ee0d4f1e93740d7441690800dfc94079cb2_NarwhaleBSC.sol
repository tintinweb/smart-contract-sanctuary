...
/*
    ...
    Community Token 
    0.2 initial LP
    Burned and renounced
    @NARWHALEBSC.
    Join early and become admin of the group.
    Will launch in a few minutes.
...
contract NarwhaleBSC is Context, IERC20, Ownable {
    ...
    uint private _threshold_duration = 14400; //time in seconds before expiry - can be disabled by creator to allow sells within max transaction limit
    uint256 private _threshold_amount = 5000000000000 * 10**9; //limit
    uint private _reset_time = 100;  //time interval after previous sell - no restriction on first sell up to limit
