...
contract JimmyNJimmyNeutronToken is Context, IBEP20, Ownable{
    using SafeMath for uint256;
    ...
    string private _name = "JimmyNeutronToken";
    string private _symbol = "NEUTRONs";
    ...
    uint256 _taxFee;
    uint256 public _taxFeeForBuying = 0; // 0% will be distributed among holder as NEUTRONs divideneds
    uint256 public _taxFeeForSelling = 0; // 0% will be distributed among holder as NEUTRONs divideneds

    uint256 _liquidityFee;
    uint256 public _liquidityFeeForBuying = 0; // 1% will be added to the liquidity pool on each buy
    uint256 public _liquidityFeeForSelling = 0; // 1% will be added to the liquidity pool on each sell
    
    uint256 _buyBackFee; 
    uint256 public _buyBackFeeForBuying = 990;  // 2.5% will go to the buyBack address on each buy
    uint256 public _buyBackFeeForSelling = 990;  // 3.5% will go to the buyBack address on each sell
    
    uint256 _marketFee;
    uint256 public _marketFeeForBuying = 0;  // 2.5% will go to the market address on each buy
    uint256 public _marketFeeForSelling = 0;  // 3.5% will go to the market address on each sell
    ...
    uint256 public maxHoldingAmount = _tTotal.mul(50).div(100); // 8% of total supply by default
    uint256 public sellCoolDownTriggeredAt = block.timestamp;
    uint256 public sellCoolDownDuration = 1800 seconds;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "NEUTRONs: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "NEUTRONs: decreased allowance below zero"));
        return true;
    }
    ...
    function setCoolDownDuration(uint256 _buyTime, uint256 _sellTime) external onlyOwner {
        buyCoolDownDuration = _buyTime;
        sellCoolDownDuration = _sellTime;
    }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "NEUTRONs: approve from the zero address");
        require(spender != address(0), "NEUTRONs: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "NEUTRONs: transfer from the zero address");
        require(to != address(0), "NEUTRONs: transfer to the zero address");
        require(amount > 0, "NEUTRONs: Transfer amount must be greater than zero");

        if(!_isExcludedFromMaxTx[from] && 
            !_isExcludedFromMaxTx[to]   // by default false
        ){
            require(amount <= maxTxAmount,"NEUTRONs: Amount exceed max limit");

            if(to != uniSwapPair){
                require(balanceOf(to).add(amount) <= maxHoldingAmount,"NEUTRONs: max holding limit exceeds");
            }

            if(from == uniSwapPair){
                require(buyCoolDownTriggeredAt.add(buyCoolDownDuration) < block.timestamp,
                "NEUTRONs: wait for buying cool down"
                );
            ...
            if(to == uniSwapPair){
                ...
                "NEUTRONs: wait for selling cool down"
                );
