...
SphynxS are 1,000 block creatures randomly genarated from a huge number of traits. Holding a Sphynx makes you a member of the Sphynx Collectibles DAO.

As a member of the DAO, you will be able to use your SphynxS to interact with other players, fighting with them or simply showing them your Sphynx.

Each Sphynx has its own set of moves, items and characteristics.

Tournaments and events will be held!
...
function mintNFT(address _to) payable public safe {
    require(mintEnabled, "Minting enabled");
    if(!freeMint) {
        require(msg.value == price, "Wrong paid");
    }
    require(_lastTokenId < 6805, "Sold out");
    uint256 _tokenId = _lastTokenId + 1;
    super._mint(_to, _tokenId); 
    string memory _uri = string(abi.encodePacked(baseuri, _tokenId.toString()));
