...
//GemDetector
//Telegram:https://t.me/Gem_Detector_Community
//Twitter:https://twitter.com/GemDetectorNews
//Website:http://Gemdetector.co
...
contract GEMDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("GEM_Dividend_Tracker", "GEM_Dividend_Tracker") {
        claimWait = 14400;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 1000000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "GEM_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "GEM_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GEM contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 14400, "GEM_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "GEM: Cannot update claimWait to same value");
...
contract GEM is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    GEMDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Gemdetector", "GEM") {
        BNBRewardsFee = 6;
        liquidityFee = 3;
        extraFeeOnSell = 0;
        marketingFee = 3;
        marketingWallet = payable(0x8d495f1CE86ddD92d66932ECDD590544a0c41125);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new GEMDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GEM: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GEM: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GEM: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GEM: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GEM: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GEM: Cannot update gasForProcessing to same value");
