...
contract YSLOptSingleApe is YSLOpt {
    ...
    function earn(address _user, bool _isAmplified) external override onlyRole(STRAT_ROLE) {
        ...
        if (curBalance < userCompounds) {
            IPancakeMaster(apeMaster).withdraw(id, userCompounds - curBalance);
        }
    ...
    function mintBonus(
        ...
        uint256 syslemission = getPrice(_sysl, BUSD, usersCompUSD, apeSwap);
        ...
        IsYSL(_sysl).approve(stratSwap, usersIncome);
