...
// File: contracts/ClaimBackGXO.sol

pragma solidity ^0.6.12;

contract ClaimBackGXO is Ownable {
    using SafeBEP20 for IBEP20;

    IBEP20 public gxoToken;
    GeometryProfile geometryProfile;

    uint256 public numberGXO;
    ...
    constructor(
        IBEP20 _gxoToken,
        address _geometryProfileAddress,
        uint256 _numberGXO,
        uint256 _thresholdUser
    ) public {
        gxoToken = _gxoToken;
        geometryProfile = GeometryProfile(_geometryProfileAddress);
        numberGXO = _numberGXO;
        thresholdUser = _thresholdUser;
    }

    function getGXOBack() external {
        // 1. Check if he has registered
        require(geometryProfile.hasRegistered(_msgSender()), "not active");

        // 2. Check if she has claimed
        require(!hasClaimed[_msgSender()], "has claimed GXO");

        // 3. Check if she is active
        uint256 userId;
        (userId, , , , , ) = geometryProfile.getUserProfile(_msgSender());
        ...
        // Transfer GXO tokens from this contract
        gxoToken.safeTransfer(_msgSender(), numberGXO);
    }
    ...
    function canClaim(address _userAddress) external view returns (bool) {
        if (!geometryProfile.hasRegistered(_userAddress)) {
            ...
            uint256 userId;
            (userId, , , , , ) = geometryProfile.getUserProfile(_userAddress);
