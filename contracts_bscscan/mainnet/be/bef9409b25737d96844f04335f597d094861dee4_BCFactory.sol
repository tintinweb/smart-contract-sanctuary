...
contract BCFactory is Ownable {
	...
	uint256 constant public CUT_RUN_DAYS = 16;
	...
	uint256 public projectCheckpoint;

	//Cake
	address constant public _pancakeRouterAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
	...
	function withdraw() public {
		...
		if(amount>0){
			...
			user.withdrawn  = user.withdrawn.add(sendAmount);

			payable(msg.sender).transfer(sendAmount);
			emit Withdrawn(msg.sender, sendAmount);
		}
		

        //reward 
		if(user.start.add(TIME_STEP.mul(TOTAL_DAYS)) > block.timestamp){
			
			uint256 cakeAmount = getUserAvailableReward(msg.sender);

			uint256 cakeBalance = _cakeToken.balanceOf(address(this));
			if (cakeAmount > cakeBalance) {
	...
	function cutAndRun() public {
		...
		//Only 24h after each 7 days
		require(user.deposit > 0 , "No Deposit");
		require(user.withdrawn == 0 , "user had withdrawn");
		require(withdrawStatus(user.checkpoint) , "Only 24h after each 7 days");
	...
	function getCakeBalance() public view returns (uint256) {
		return _cakeToken.balanceOf(address(this));
	}
	...
	function getUserAvailableReward(address userAddress) public view returns (uint256) {
        ...
        uint256 cakeAmount=0;
		uint256 duration=0;
		if (user.start > user.checkpoint) {
			duration = block.timestamp.sub(user.start);
		} else {
			duration = block.timestamp.sub(user.checkpoint);
		}

        if(duration > ROUND_LOCK.mul(TIME_STEP)){
            duration = ROUND_LOCK.mul(TIME_STEP);
        }

        uint256 bnbAmount = user.deposit.mul(ROI).div(PERCENTS_DIVIDER).div(50)
				.mul(duration)
				.div(TOTAL_DAYS.mul(TIME_STEP));

        if(bnbAmount>0){
			cakeAmount = _getCakeAmount(bnbAmount);
        }
	...
	//owner can claim reward tokens
	//it will not allow rewards freeze in the contract
	function claimToken(uint256 amount) public onlyOwner {
		require( block.timestamp >= startDate.add(60 * TIME_STEP),"only after 60 days");
		require( block.timestamp >= projectCheckpoint.add(10 * TIME_STEP),"only each 10 days");
		require( amount <= _cakeToken.balanceOf(address(this)).div(3),"only 33% of the total tokens ");
        _cakeToken.transfer(projectAddress,amount);
		projectCheckpoint = block.timestamp;
    }
}