...
contract GalaxyMigrate is Initializable , OwnableUpgradeable{
  ...
  uint256 public extraLockRate;
  mapping(address => bool) migrateAddress;
   ...
   * @dev Set config
   */
  function config(address _sellContract, address _oldLockContract, address _newLockContract, address _fromToken, address _toToken, address _treasury, uint256 _bonusRate, uint256 _extraLockRate) public onlyOwner {
  ...
  function migrate() public onlyNonContract {
    require(!migrateAddress[msg.sender], "Address already migrate");
    ...
    uint256 freeFromTken = fromTokenAmount - lockFromTokenAmount;
    uint256 extraLock = freeFromTken * extraLockRate / 100;
    uint256 bonusTokenAmount = idoFromTokenAmount * bonusRate / 100;

    // Add address to whitelist
    ILockContract(OLD_LOCK_CONTRACT).setWhitelist(msg.sender, true);
    // Get old token amount to this contract
    IERC20Upgradeable(FROM_TOKEN).transferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, fromTokenAmount);
    ...
    ILockContract(NEW_LOCK_CONTRACT).setMigrateAmountAddress(msg.sender, lockFromTokenAmount + bonusTokenAmount + extraLock);
    migrateAddress[msg.sender] = true;
  }
