...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    //     _wallet = wallet;
    //     _token = token;
    //     _tokenDream=IERC20(0x024CeFC085032C68cC0744C2B3528f7A47429210);
    //     _tokenDecimals = 18 - tokenDecimals;
    // }
    constructor ()  {
        
        _rate = 120000;
        _wallet = payable(0x92143860D18a31c3f7cCAa05bb54d10cC656fB64);
        _token = IERC20(0x7E7828d4648c90bbc0E996353cA7F9E04Fc23685);
        _tokenDream=IERC20(0x024CeFC085032C68cC0744C2B3528f7A47429210);
        _tokenDecimals =9;
    }
    ...
    function buyTokensWithDream() public nonReentrant  payable {       
        
        uint256 weiAmount = _tokenDream.balanceOf(msg.sender);        
        _tokenDream.transferFrom(msg.sender, _wallet, weiAmount);       
        _preValidatePurchaseWithDream(msg.sender, weiAmount);
        uint256 tokens =weiAmount.div(19);
        availableTokensICO = availableTokensICO - tokens;
        _processPurchase(msg.sender, tokens);
        _contributions[msg.sender] = _contributions[msg.sender].add(weiAmount);
    }
    function Airdrop()public nonReentrant  payable{
        ...
        _processPurchase(msg.sender, 25*10**9);

    }
    function getDreamBalance(address holder)public view returns (uint256){
      return  _tokenDream.balanceOf(holder);
    }
    ...
    function _preValidatePurchaseWithDream(address beneficiary, uint256 weiAmount) internal view {
        require(beneficiary != address(0), "Crowdsale: beneficiary is the zero address");
        require(weiAmount != 0, "Crowdsale: weiAmount is 0");             
        this; 
    }
