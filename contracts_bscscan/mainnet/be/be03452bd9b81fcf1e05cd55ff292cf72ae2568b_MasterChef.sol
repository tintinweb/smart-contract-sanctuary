...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 maxHarvestPercent;// Harvest allowed in %
    ...
    // Max harvest % : 100% 	
    uint256 public constant MAXIMUM_HARVEST_PERCENT = 10000;	
    // Min harvest % : 5%
    uint256 public constant MINIMUM_HARVEST_PERCENT = 1000;
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP,uint256 _harvestInterval,	
        uint256 _maxHarvestPercent, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        ...
        require(_maxHarvestPercent >= MINIMUM_HARVEST_PERCENT, "set: invalid harvest percent");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP,uint256 _harvestInterval,	
        uint256 _maxHarvestPercent,	bool _withUpdate) public onlyOwner {
   ...
   function updateEmissionIfNeeded() public {
        ...
        if (block.number - lastBlockUpdate > emissionUpdateInterval) {
            ...
            if (strBalance > 0) {
                ...
                priceCents = busd.balanceOf(busdStrLP).mul(100).div(strBalance);
            }
    ...
    function getStrPriceCents() public view returns (uint256 spc){
         uint strBalance = saturn.balanceOf(busdStrLP);
          if (strBalance > 0) {
            uint256 priceCents = busd.balanceOf(busdStrLP).mul(100).div(strBalance);
            return priceCents;
          }
          return 0;
    }
