...
contract GateManagerMultiRewardsLP is ReentrancyGuardUpgradeable, Ownable {
    ...
    // checks that caller is either owner or notifier.
    modifier onlyZap() {
        require(msg.sender == zap, "!Only Zap can use this");
        _;
    }
    ...
    function withdrawAllInstantly() public {
        ...
        if (yieldBal > 0) {
            _withdrawMoonPotYieldShares(yieldBal);
        }
    }
    ...
    function withdrawMoonPotYield(uint256 amount) public {
        ...
        _withdrawMoonPotYieldShares(sharesAmount);
    }
    ...
    /// @param shares The amount of shares to withdraw.
    /// if isMooToken == false, shares == underlyingAmount
    function _withdrawMoonPotYieldShares(uint256 shares)
        internal
        nonReentrant
        updateReward(msg.sender)
    {
        if (isMooToken) {
            ...
            _totalSupply = _totalSupply.sub(shares);
            IERC20Upgradeable(underlying).safeTransfer(msg.sender, shares);
