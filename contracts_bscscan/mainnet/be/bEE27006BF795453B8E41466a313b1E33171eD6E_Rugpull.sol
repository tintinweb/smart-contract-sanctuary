...
contract Rugpull is Context, IBEP20, Ownable {
    ...
    */
    ...
    function executeRandomBurn() public {
        require(
            block.timestamp.sub(_lastBurn) >= 1 seconds,
            "Must wait at least 1 seconds between burns"
        );
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        // if sender is staked, initiate unstake and return
        if(_isStaked[sender]){
            _unstake(sender);
            return;
        }
