...
interface IReward {
    function clickBuyBond(address _user, uint256 _bondVal) external returns (bool);
}
...
contract MockOlympusBondDepository is Ownable {
    ...
    function setReward(address _Reward) external onlyPolicy {
        Reward = _Reward;
    }
    ...
    function deposit(
        ...
        IReward(Reward).clickBuyBond(_depositor, payout);
        if (fee != 0) {
