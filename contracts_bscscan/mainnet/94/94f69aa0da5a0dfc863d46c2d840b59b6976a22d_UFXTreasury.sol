...
contract UFXTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SUFX }
    ...
    address public sUFX;
    uint public sUFXQueue; // Delays change to sUFX address
    ...
    /**
        @notice allow approved address to deposit an asset for UFX
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint UFX needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20( UFX ).safeTransfer( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn UFX for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IERC20( UFX ).safeTransferFrom(msg.sender, BURN_ADDRESS, value);
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sUFX ).balanceOf( msg.sender ); // Can only borrow against sUFX held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with UFX
        @param _amount uint
     */
    function repayDebtWithUFX( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IERC20( UFX ).safeTransferFrom(msg.sender, BURN_ADDRESS, _amount);

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, UFX, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20( UFX ).safeTransfer( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( UFX ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns UFX valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match UFX decimals
            value_ = _amount.mul( 10 ** IERC20( UFX ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SUFX ) { // 9
            sUFXQueue = block.number.add( blocksNeededForQueue );
        } else return false;
