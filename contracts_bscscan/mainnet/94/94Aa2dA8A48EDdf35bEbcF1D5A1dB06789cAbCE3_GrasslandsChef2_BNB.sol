...
contract GrasslandsChef2_BNB is Ownable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;


    //default txrate of the buff+red token
    ...
    // Maximum amount of BUFF that can be deposited
    ...
    function adjustBlockEnd() public onlyOwner {
        require(block.number > startBlock, "Pool has NOT started");
    ...
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if (transferTaxRate > 0) {
                uint256 transferTax = _amount.mul(transferTaxRate).div(10000);
    ...
    // if the syrup token has a transfer tax fee (5% max !! )
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOwner {
        require(_transferTaxRate <= 500, "GRASSLANDS::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(syrup), "Cannot be syrup token");
