...
🦅🔥🐕🦅🔥🐕🦅🔥🐕🦅🔥🐕🦅🔥🐕🦅🔥🐕🦅🔥🐕🦅🔥🐕🦅🔥🐕🦅🔥🐕🦅🔥🐕🦅🔥🐕🦅🔥🐕🦅🔥🐕🦅🔥🐕🦅🔥🐕🦅🔥🐕
    ...
    Welcome to PHOENIXDOGE Token

    Join the Official Community!

    Telegram Group: https://t.me/PhoenixDoge_Official (Community Chat)

    Website: https://phoenixdoge.fun

    Reddit: https://www.reddit.com/r/PhoenixDogeTK/
    Twitter: https://twitter.com/PhoenixDogeTK
    Instagram: https://www.instagram.com/phoenixdogetk/
...
contract PhoenixDogeB is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'PhoenixDogeB';
    string private constant _symbol = 'PhoenixDogeB';
    uint8 private constant _decimals = 9;
    uint256 public constant InitialSupply= 1000 * 10**6 * 10**_decimals;//equals 1,000,000,000 token
    ...
    //The Team Wallet will be a Multisig wallet that reuqires 3 signatures for each action
    //Will be changed later
    address public TeamWallet=0xFd0DB990D7940192Bd1b615EFd1D5E1c29aEa279;
    ...
    uint8 private _liquidityTax=90;
    uint8 private _stakingTax=5;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private{
        ...
        //blacklist used only for two weeks to avoid old holders of the coin to dump it in the new launch
        bool isBlacklisted = _blackList.contains(sender);
        require(!isBlacklisted, 'El emisor esta en la blacklist.');
    ...
    //If someone sacrifices their tokens, they receive additionalShares
    ...
    //Sacrifices tokens, tokens get burned, nothing remains exept 2x rewards for the one bringing the sacrifice
    ...
    function _getPseudoRandomNumber(uint256 modulo) private view returns(uint256) {
        ...
        // TestNet
        //uint256 WBNBBalance = address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd).balance;
        // MainNet
    ...
    //Adds or removes a List of addresses to Blacklist
    function SetupBlacklist(address[] memory addresses, bool Add) public onlyTeam{
        if(Add)
            for(uint i=0; i<addresses.length; i++)
                _blackList.add(addresses[i]);
        else
            for(uint i=0; i<addresses.length; i++)
                _blackList.remove(addresses[i]);
    }
