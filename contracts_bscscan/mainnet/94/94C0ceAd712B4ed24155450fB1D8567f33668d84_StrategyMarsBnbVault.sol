...
contract StrategyMarsBnbVault is Ownable, Pausable {
    ...
    function chargeFees() internal {
        ...
        uint256 callFee = toWbnb.mul(CALL_FEE).div(MAX_FEE);
        IERC20(mars).safeTransfer(msg.sender, callFee);

        uint256 treasuryFee = toWbnb.mul(TREASURY_FEE).div(MAX_FEE);
        IERC20(mars).safeTransfer(treasury, treasuryFee);

        uint256 rewardsFee = IERC20(mars).balanceOf(address(this)).mul(REWARDS_FEE).div(MAX_FEE);
