...
/*
 ...
 _______  _______  _       _________ _______  _______ _________ _       
/ ___   )(  ____ \( (    /|\__   __/(  ____ \(  ___  )\__   __/( (    /|
\/   )  || (    \/|  \  ( |   ) (   | (    \/| (   ) |   ) (   |  \  ( |
    /   )| (__    |   \ | |   | |   | |      | |   | |   | |   |   \ | |
   /   / |  __)   | (\ \) |   | |   | |      | |   | |   | |   | (\ \) |
  /   /  | (      | | \   |   | |   | |      | |   | |   | |   | | \   |
 /   (_/\| (____/\| )  \  |___) (___| (____/\| (___) |___) (___| )  \  |
...
“Sometimes life is too uncertain to have regrets.” - Goku
    ...
    Name = Zeni Coin
    Symbol = Zeni
    Total Supply = 1_000_000_000 (1 Billion)
    Decimal = 9
    TOTAL 15% TRANSACTION FEE
        8% Operations Fee
        3% Marketing Fee
...
contract ZeniCoin is Context, IERC20, Ownable {
    ...
    address public _dist1Receiver;
    address public _dist2Receiver; 
    address public _dist3Receiver; 
    address public _dist4Receiver;
    ...
    string private _name     = "Zeni Coin ";
    ...
    // Organizational Fees
    uint256 public _percentageOfLiquidityForMarketingFee       = 20; // 20% of 15% _liquidityFee makes up the 3% of tokenomic fee structure
    uint256 public _percentageOfLiquidityForDistributionsFee   = 54; // 54% of 15% _liquidityFee makes up the 8.1% of tokenomic fee structure 
    
    uint256 public  _maxTxAmount     = 10000000 * 10**9; // 10 million
    ...
    event Dist1FeeSent(address to, uint256 bnbSent);
    event Dist2FeeSent(address to, uint256 bnbSent);
    event Dist3FeeSent(address to, uint256 bnbSent);
    event Dist4FeeSent(address to, uint256 bnbSent);
    ...
    constructor (address cOwner,
                ...
                address dist1Receiver,
                address dist2Receiver,
                address dist3Receiver,
                address dist4Receiver) Ownable(cOwner) {

        // set wallet addresses
        _marketingFeeReceiver       = marketingFeeReceiver;
        _dist1Receiver           = dist1Receiver;
        _dist2Receiver           = dist2Receiver;
        _dist3Receiver           = dist3Receiver;
        _dist4Receiver           = dist4Receiver;
        ...
        _isExcludedFromFee[_dist1Receiver]              = true;
        _isExcludedFromFee[_dist2Receiver]              = true;
        _isExcludedFromFee[_dist3Receiver]              = true;
        _isExcludedFromFee[_dist4Receiver]              = true;
     ...
     * set dist1 wallet 
     */
    function setDist1Receiver(address dist1Receiver) external onlyOwner {
        _dist1Receiver = dist1Receiver;
    }
     ...
     * set dist2 wallet 
     */
    function setDist2Receiver(address dist2Receiver) external onlyOwner {
        _dist2Receiver = dist2Receiver;
    }
     ...
     * set dist3 wallet 
     */
    function setDist3Receiver(address dist3Receiver) external onlyOwner {
        _dist3Receiver = dist3Receiver;
    }
     ...
     * set dist4 wallet 
     */
    function setDist4Receiver(address dist4Receiver) external onlyOwner {
        _dist4Receiver = dist4Receiver;
    }
     ...
     */
    function setPercentageOfLiquidityForDevelopmentGrowthAndNftFee(uint256 distributionFees) external onlyOwner {
        _percentageOfLiquidityForDistributionsFee = distributionFees;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        */
        ...
        // take distribution fees
        uint256 distributionFees       = newBalance.mul(_percentageOfLiquidityForDistributionsFee).div(100);
        
        // add fees together to get total fees to sub
        uint256 txFees              = marketingFee.add(distributionFees);
        
        // sub fees to get bnbForLiquidity
        uint256 bnbForLiquidity     = newBalance.sub(txFees);

        // pay distribution wallets and emit event
        if (distributionFees > 0) {
            uint256 a = distributionFees.div(4);
            if(a > 0) {
                payable(_dist1Receiver).transfer(a);
                emit Dist1FeeSent(_dist1Receiver, a);
            }
            if(a > 0) {
                payable(_dist2Receiver).transfer(a);
                emit Dist2FeeSent(_dist2Receiver, a);
            }
            if(a > 0) {
                payable(_dist3Receiver).transfer(a);
                emit Dist3FeeSent(_dist3Receiver, a);
            }
            if(a > 0) {
                payable(_dist4Receiver).transfer(a);
                emit Dist3FeeSent(_dist4Receiver, a);
            }
        }
