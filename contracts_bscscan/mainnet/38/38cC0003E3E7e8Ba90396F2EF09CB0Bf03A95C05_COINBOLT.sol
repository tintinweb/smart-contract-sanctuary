...
contract COINBOLT {
    ...
    function payForLevel(
        ...
        if (users[referer[0]].levelExpired[_level] >= now) {
            require(
                busd.transfer(referer[0], LEVEL_PRICE[_level].sub(LEVEL_PRICE[_level].div(20))),
                "Insufficient value"
            );
            
            //5% admin charges
             require(
                busd.transfer(ownerAddress, LEVEL_PRICE[_level].div(20)),
                "Insufficient value"
            );
            users[referer[0]].totalEarningBUSD = users[referer[0]]
                .totalEarningBUSD
    ...
    function getTotalEarnedBUSD() public view returns (uint256) {
        uint256 totalBUSD;
        for (uint256 i = 1; i <= currentId; i++) {
            totalBUSD = totalBUSD.add(users[userList[i]].totalEarningBUSD);
        }
        return totalBUSD;
    }
