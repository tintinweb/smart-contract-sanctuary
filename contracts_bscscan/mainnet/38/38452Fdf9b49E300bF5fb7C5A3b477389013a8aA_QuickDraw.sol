...
contract QuickDraw is IBEP20, Ownable {
    ...
    event NewBounty(address bountyAddress);
    ...
    uint256 public _maxHoldAmount = 20000000 * 10**6 * 10**9;
    ...
    IBounty private bountyContract;
    ...
    /**
     * @notice Update the bounty contract
     *
     * @param _bountyContract The address of the new bounty contract
     */
    function updateBountyContract(address _bountyContract) external onlyOwner() {
        bountyContract = IBounty(_bountyContract);
        emit NewBounty(_bountyContract);
    }

    /**
     * @notice Update the bridge contract
     *
    ...
    /**
     * @notice Enables trading after the migration is complete
     */
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(tradingEnabled || from == bridgeContract || from == owner(), "_transfer(): Trading is not enabled!");
    ...
    function _swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 liquidityCut = (_liquidityFee * contractTokenBalance) / (_liquidityFee + _marketingFee + _bountyFee);
        uint256 bountyCut = (_bountyFee * contractTokenBalance) / (_liquidityFee + _marketingFee + _bountyFee);
        uint256 marketingCut = contractTokenBalance - liquidityCut - bountyCut;
        ...
        // Send bounty cut to the bounty contract
        _swapTokensForBNB(bountyCut, address(bountyContract));
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (takeFee) {
            ...
            bountyFee = _calculateFee(amount, _bountyFee);

            // Check if this is a sell and if so, transfer 5% to the previous highest buyer since the last sell
            if (recipient == uniswapV2Pair) {

                bountyContract.logSell(sender, amount);

                if (_highestBidder != address(0) && quickDrawEnabled) {
            ...
            _takeContractTaxes(liquidityFee + marketingFee + bountyFee);
        ...
        if (sender == uniswapV2Pair) {
            ...
            bountyContract.logBuy(recipient, amount);

            //  If the buy is larger than or equal to the previous buy since the last sell
            if (amount >= _highestBid && !_isExcluded[recipient]) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 tTransferAmount) = _getValues(
            amount,
            liquidityFee + reflectionFee + quickDrawFee + marketingFee + bountyFee
        );
     ...
     */
    function _takeContractTaxes(uint256 tFee) private {
    ...
    function _quickDraw(uint256 quickDrawFee) private lockTheSwap {
        ...
        uint256 qdrawWinnings = _highestBidder.balance - balanceBefore;

        quickDrawWinningsPerAccount[_highestBidder] += qdrawWinnings;

        emit QuickDrawPayout(_highestBidder, qdrawWinnings, _highestBid);

        bountyContract.logQuickDrawWin(_highestBidder, qdrawWinnings, _highestBid);
