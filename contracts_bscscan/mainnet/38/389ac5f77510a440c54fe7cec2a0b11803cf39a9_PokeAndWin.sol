...
contract PokeAndWin is TransferOwnable {   
    ...
    function setCURRENCY(address _tokenAddress, address _pirAddress) public onlyPartner { 
        ...
        if(c.token0==busd){
            c.busd_rate = uint256(c._reserve0*1e8/c._reserve1);
        } else if(c.token1==busd){
            c.busd_rate = uint256(c._reserve1*1e8/c._reserve0);
        } else if(c.token0==wbnb){
            c.busd_rate = uint256(wbnb_busd.busd_rate * c._reserve0 / c._reserve1);
        } else if(c.token1==wbnb){
            c.busd_rate = uint256(wbnb_busd.busd_rate * c._reserve1 / c._reserve0);
        }  
        
        c.wbnb_rate = uint256(c.busd_rate / wbnb_busd.busd_rate);
