...
/*
   ...
   The boss is in town and he doesnt like how other Doge coins have been acting..
 
   The most unique coin on BSC! Earn random Binance-Pegged Cryptos by holding MAFIADOGE
   
   Earn rewards every 24 hours by holding!
   
   TG: Mafia_Doge
   Website: MafiaDoge.org
   Twitter: @Mafia_Doge
...
contract mafiadoge is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0x94Fb26855033FdB8Fdac3396022A016e36C1475C); // Marketing Address
    address payable public RewardWallets = payable(0xBd20493fc400fD34799e844ed6EbB2153c23eCd2); // REWARD Address
    ...
    string private _symbol = "MAFIADOGE";
    uint8 private _decimals = 9;

    uint256 public _burnFee = 1;
    uint256 public _liquidityFee = 0;
    uint256 public _marketingFee = 2;
    uint256 public _cryptoCharity = 2;
    ...
    constructor () {
        ...
        _totalTaxPercent = _burnFee.add(_liquidityFee).add(_marketingFee).add(_cryptoCharity);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _marketingFee.add(_cryptoCharity) > 0)
        {
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(forWallets);
            uint256 newBalance = address(this).balance.sub(initialBalance);
    
            uint256 marketingShare = newBalance.div(_marketingFee.add(_cryptoCharity
     )).mul(_marketingFee);
            uint256 rewardWalletShare = newBalance.sub(marketingShare);
            
            if(marketingShare > 0)
                transferToAddressETH(marketingWalletAddress, marketingShare);
            
            if(rewardWalletShare > 0)
                transferToAddressETH(RewardWallets, rewardWalletShare);
        }
    ...
    function setTaxes(uint256 newBurnFee, uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newLinkCharityTax) external onlyOwner() {
        ...
        _cryptoCharity = newLinkCharityTax;
        _totalTaxPercent = _burnFee.add(_liquidityFee).add(_marketingFee).add(_cryptoCharity
 );
        _prevTotalTaxPercent = _totalTaxPercent;
    }
    ...
    function setRewardWallets(address newAddress) external onlyOwner() {
        RewardWallets = payable(newAddress);
    }
