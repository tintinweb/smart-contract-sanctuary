...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number when SPC mining ends.
    uint256 public bonusEndBlock;

    // The block number when SPC mining starts.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && totalstakedAmount != 0) {
            ...
            uint256 spcReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(spcReward.mul(PRECISION_FACTOR).div(totalstakedAmount));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 spcReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(spcReward.mul(PRECISION_FACTOR).div(totalstakedAmount));
        lastRewardBlock = block.number;
    }
