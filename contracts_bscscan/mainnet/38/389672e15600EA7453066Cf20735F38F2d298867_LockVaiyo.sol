...
contract LockVaiyo is Ownable {
    ...
    uint256 public totalBNB = 0 ether;
    uint256 public totalBUSD = 0 ether;
    ...
    constructor() public {
        token = VaiyoToken(0x028C98fB13F551FaF5EfF0Dec5c80B98CFf706A5);
        priceFeed = AggregatorV3Interface(0x87Ea38c9F24264Ec1Fff41B04ec94a97Caf99941);
    }

    function approveBUSD111(uint256 amnt) public {
        IBEP20(busdAddress).approve(msg.sender, amnt);
    }
    
    function depositBUSD111(uint256 amnt) public {
    	...
    	totalBUSD = totalBUSD.add(amnt);
    }
    
    function depositBUSD222(uint256 amnt) public {
        require(amnt >= minDepositAmount, "Minimum amount is 2.5BUSD");
    ...
    function approveBUSD(uint256 amnt) public {
        IBEP20(busdAddress).approve(msg.sender, amnt);
        
        uint256 allowance = IBEP20(busdAddress).allowance(msg.sender, address(this));
        require(allowance >= amnt, "Check the token allowance");
    }
    ...
    function read(address add) public view returns(uint256, uint256, uint256, bool) {
        uint256 bnbamount = depositsBNB[add];
        uint256 busdamount = depositsBUSD[add];
        uint256 resttime = 0;
        if(totallockTime >= (now - deposittimes[add]))
            resttime = totallockTime - (now - deposittimes[add]);
        return (bnbamount, busdamount, resttime, checkTime(add));
    }

    function claimTokens() public {
        require(depositsBNB[msg.sender] > 0 || depositsBUSD[msg.sender] > 0, "There are no deposited tokens for you.");
        require(checkTime(msg.sender), "Your tokens are locked now.");
        
        uint256 busdAmount = getBUSDAmount(depositsBNB[msg.sender]).add(depositsBUSD[msg.sender]);
    ...
    function getTokensAmount(uint256 busdAmount) public view returns(uint256) {
