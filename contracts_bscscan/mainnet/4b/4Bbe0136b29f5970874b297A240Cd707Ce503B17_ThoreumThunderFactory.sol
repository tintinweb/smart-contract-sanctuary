...
// File ThoreumThunderFactory.sol
...
contract ThoreumThunderPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // The address of the thoreum thunder factory
    address public THOREUM_THUNDER_FACTORY;
    ...
    // The block number when THOREUM mining ends.
    ...
    constructor() public {
        THOREUM_THUNDER_FACTORY = msg.sender;
    }
    ...
    function initialize(
        ...
        require(msg.sender == THOREUM_THUNDER_FACTORY, "Not factory");
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        require(block.timestamp >= user.nextWithdrawalUntil || block.number >= bonusEndBlock, "Withdrawal locked");
    ...
    function canWithdraw(address _user) external view returns (bool) {
        UserInfo storage user = userInfo[_user];
        return (block.timestamp >= user.nextWithdrawalUntil || block.number >= bonusEndBlock);
    }
...
contract ThoreumThunderFactory is Ownable {
    event NewThoreumThunderContract(address indexed thoreumThunder);
     ...
     * @param _withdrawalInterval: the withdrawal interval for stakedToken (if any, else 0)
     * @param _admin: admin address with ownership
     * @return address of new thoreum thunder contract
     */
    function deployPool(
        IBEP20 _stakedToken,
        IBEP20 _rewardToken,
        ...
        bytes memory bytecode = type(ThoreumThunderPool).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address thoreumThunderAddress;

        assembly {
            thoreumThunderAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        ThoreumThunderPool(thoreumThunderAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewThoreumThunderContract(thoreumThunderAddress);
    }
}