...
contract CANDY is ERC20Detailed, Ownable {
  ...
  uint256 public transactionTax = 110;
  uint256 public buybackLimit = 10 ** 2;
  ...
  // TOTAL_CANDY is a multiple of INITIAL_FRAGMENTS_SUPPLY so that _CANDYPerFragment is an integer.
  // Use the highest value that fits in a uint256 for max granularity.
  uint256 private constant TOTAL_CANDY = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

  // MAX_SUPPLY = maximum integer < (sqrt(4*TOTAL_CANDY + 1) - 1) / 2
  uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

  uint256 private _totalSupply;
  uint256 private _CANDYPerFragment;
  mapping(address => uint256) private _CANDYBalances;
  ...
  ERC20Detailed("CANDY", "CANDY", uint8(DECIMALS))
  payable
  public
  {
    ...
    _CANDYBalances[msg.sender] = TOTAL_CANDY;
    _CANDYPerFragment = TOTAL_CANDY.div(_totalSupply);
...
*/
  ...
  returns (uint256)
  {
    return _CANDYBalances[who].div(_CANDYPerFragment);
  }
  ...
  function _transferStandard(address sender, address recipient, uint256 amount) private {
    ...
    uint256 CANDYDeduct = amount.mul(_CANDYPerFragment);
    uint256 CANDYValue = tTransferAmount.mul(_CANDYPerFragment);
    _CANDYBalances[sender] = _CANDYBalances[sender].sub(CANDYDeduct);
    _CANDYBalances[recipient] = _CANDYBalances[recipient].add(CANDYValue);
    _takeFee(sender, tFee);
    emit Transfer(sender, recipient, amount);
  }

  function _transferExcluded(address sender, address recipient, uint256 amount) private {
    uint256 CANDYValue = amount.mul(_CANDYPerFragment);
    _CANDYBalances[sender] = _CANDYBalances[sender].sub(CANDYValue);
    _CANDYBalances[recipient] = _CANDYBalances[recipient].add(CANDYValue);
    emit Transfer(sender, recipient, amount);
  }
  ...
  function _takeFee(address sender, uint256 tFee) private {
    uint256 rFee = tFee.mul(_CANDYPerFragment);
    _CANDYBalances[address(this)] = _CANDYBalances[address(this)].add(rFee);
