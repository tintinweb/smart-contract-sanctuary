...
contract YSLProtocol is OwnableUpgradeable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 lpSupply = pool.strat.totalDeposited();
        ...
        if (block.timestamp - lastTeamUpdate >= 1 days && teamRate > 0) {
            address teamWallet = 0xa7b9fFa1BB64856AA62C762bBB0a2A6792DF9613;
            lastTeamUpdate = block.timestamp;
            IsYSL(sYSL).mintFor(teamWallet, teamRate);
        }
    }
    ...
    function deposit(
        ...
        uint256 totRewards = (user.amount * pool.accsYSLPerShare) / DECIMALS;
        if (totRewards > user.rewardDebt) {
            uint256 pending = totRewards - user.rewardDebt;
            if (pending > 0) {
                safeSYSLtransfer(_msgSender(), pending);
            }
        }
        user.rewardDebt = totRewards;
    ...
    function _harvest(uint256 _pid, address _user) internal {
        ...
        uint256 totRewards = (user.amount * pool.accsYSLPerShare) / DECIMALS;
        if (totRewards < user.rewardDebt) return;
    ...
    function pendingsYSL(uint256 _pid, address _user) public view returns (uint256) {
        ...
        uint256 lpSupply = pool.strat.totalDeposited();
        if (lpSupply == 0 || user.amount == 0) {
            return 0;
        }

        uint256 pendingSYSL = 0;
        if (block.number > pool.lastRewardBlock && totalAllocPoint > 0 && pool.allocPoint > 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 sYSLReward = (multiplier * sYSLPerBlock * pool.allocPoint) / totalAllocPoint;
            accsYSLPerShare += ((sYSLReward * DECIMALS) / lpSupply);

            uint256 totRewards = (user.amount * accsYSLPerShare) / DECIMALS;
            if (totRewards > user.rewardDebt) {
                pendingSYSL = totRewards - user.rewardDebt;
            }
        }
        pendingSYSL += pool.strat.currentReward(_user);
        return pendingSYSL;
    }
