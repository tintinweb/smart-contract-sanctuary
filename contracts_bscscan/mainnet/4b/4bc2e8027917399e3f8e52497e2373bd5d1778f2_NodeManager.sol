...
contract NodeManager is Ownable {
    ...
    function _isClaimable(NodeEntity memory node) private view returns (bool) {
        return node.lastClaimTime + claimTime <= block.timestamp;
    }

    function _calculateNodeRewards(uint _lastClaimTime) private view returns (uint rewards) {
        return ((block.timestamp - _lastClaimTime) / claimTime) * rewardPerNode;
    }

    // External methods

    function createNode(address account, string memory nodeName) external onlyGuard {
        require(
    ...
    function changeNodePrice(uint256 newNodePrice) external onlyGuard {
        nodePrice = newNodePrice;
    }

    function getNodePrice() external view returns (uint256 currentNodePrice) {
        return nodePrice;
    }

    function changeRewardPerNode(uint256 newReward) external onlyGuard {
    ...
    function changeClaimTime(uint256 newTime) external onlyGuard {
        claimTime = newTime;
    }

    function getClaimTime() external view returns (uint256 currentClaimTime) {
        return claimTime;
    }
    ...
    function getTotalNodesCreated() external view returns (uint256) {
        return totalNodesCreated;
    }

    function getTotalRewardsStaked()
        external
        view
