...
contract ScratchCard is Context, Ownable{
    ...
    function BuyTicket() public{
		...
		require(cardsSold<=maxCards && GameOn==true,"Conditions for game start not met: Game hasnt started or it has finished");//Checks if machine has sold out	
	     ...
	     if(currentRandomNumber<175 && currentRandomNumber!=79 && currentRandomNumber !=69){
	         isPlaying[_msgSender()]=false;
	          winTier[_msgSender()]=0;
	         emit noPrize(_msgSender(),false);
	        
	     }
	     else if(currentRandomNumber>=175 && currentRandomNumber<=218){
	         isPrize[_msgSender()]=1250*10**18;
	         winTier[_msgSender()]=1;
	     }
		 else if(currentRandomNumber == 496 || currentRandomNumber==79){
		   ...
		   else if(currentRandomNumber > 390 && currentRandomNumber<=460){
			isPrize[_msgSender()]=2500*10**18;
			 winTier[_msgSender()]=2;

		 }
		   else if(currentRandomNumber > 460 && currentRandomNumber<=490){
			isPrize[_msgSender()]=5000*10**18;
			 winTier[_msgSender()]=3;

		 }
		   else if(currentRandomNumber > 490 && currentRandomNumber<=495){
			isPrize[_msgSender()]=12500*10**18;
			 winTier[_msgSender()]=4;

		 }
		   else if(currentRandomNumber > 495 && currentRandomNumber<=500 || currentRandomNumber == 69){
			isPrize[_msgSender()]=42500*10**18;
