...
contract s1GoblinMoon is Ownable{
    ...
    event claimCreated(address buyer, uint[] claims);
    ...
    function buy(uint amount_) external {
        uint value = price*amount_;

        uint amountAfter = qty[msg.sender] + amount_;

        require(whitelist[msg.sender], "Sorry, you are not in the whitelist");
        require(amountAfter <= maximum, "Can't buy that amount");
        require(iBusd.balanceOf(msg.sender) > price, "Not enough token!");

        qty[msg.sender] += amount_;
        iBusd.transferFrom(msg.sender, address(this), price);

        for(uint i=0;i < amount_; i++){
            claims[msg.sender].push(block.number + 3 + i);
        }

        emit claimCreated(msg.sender, claims[msg.sender]);

    }

    function claim() external{

        require(block.number > claims[msg.sender][claims[msg.sender].length-1], "Last claim block not mined yet.");

        for(uint i=0;i < claims[msg.sender].length; i++){
            uint blockNumber = claims[msg.sender][i];
