...
contract GameContract is Ownable, ReentrancyGuard {
    ...
    // Token to Pay
    address public payToken = 0xAc9178feD286e0e4F21D50d40f6C77503C96C19c;

    // Token to receive
    address public receiveToken = 0xAc9178feD286e0e4F21D50d40f6C77503C96C19c;
    ...
    // minimal Bet Amount
    uint256 public betSize = 20 *10**18;
    
    // Number for mod posibilites 
    uint256 public chance = 2;
    
    // winPercent
    uint256 public winPercent = 49;
    ...
    event Game(address indexed user, uint256 amount, uint256 winNumber, uint8 _yourNumber);
    ...
    function game(uint256 _inputAmt, uint8 _yourNumber) public {
        require (_inputAmt >= betSize, 'Game: Bet to Small');
        require (_yourNumber >= 1 && _yourNumber <= chance, 'Game: Invalid Number');
        ...
        if (winNumber == _yourNumber) {

            IERC20(receiveToken).safeTransfer(msg.sender, sendAmount);
             
        }
        
        emit Game(msg.sender, sendAmount, winNumber, _yourNumber);
    }
}