...
contract TWOHAND is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 33000000e8;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
	

	address payable private _marketingWallet = payable(0x24F2C0B74D61c41fa64424cB670da735De5492aE);
    
    string private _name = "Two Hand";
    string private _symbol = "TWH";
    ...
    uint256 public _maxTxAmount = 33000000 * 10**18;
    ...
    constructor () {
       ...
       //Test Net for Pancakeswap
        //IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
        
        //Main Net for Pancakeswap
       IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
       
       //Test Net for uniswap
	...
	function _getmarketingWallet() public view virtual returns (address) {
    ...
    function excludeFromReward(address account) public onlyOwner() {
        require(account != 0x24F2C0B74D61c41fa64424cB670da735De5492aE, 'We can not exclude Uniswap router.');
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        require(sender != address(0), "Address is not valid");
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee > 0, "Zero value is not allowed.");
        _taxFee = taxFee;
    }

    function setmarketingFeePercent(uint256 marketingFee) external onlyOwner() {
            require(marketingFee > 0, "Zero value is not allowed.");
        _marketingFee = marketingFee;
    }

    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
            require(liquidityFee > 0, "Zero value is not allowed.");
        _liquidityFee = liquidityFee;
    }

    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
            require(maxTxPercent > 0, "Zero value is not allowed.");
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tmarketing);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        return (tTransferAmount, TData(0, tFee, tLiquidity, tmarketing, 0));
    }
	...
	function _takemarketing(uint256 tmarketing) private returns(bool) {
        uint256 currentRate =  _getRate();
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        ...
        require(balanceOf(owner) >= amount, "More allowance is expected.");
