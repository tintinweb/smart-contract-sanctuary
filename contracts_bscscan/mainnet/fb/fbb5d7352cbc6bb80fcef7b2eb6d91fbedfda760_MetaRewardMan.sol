...
contract DividendDistributor is Ownable {
    ...
    // TOKENS
    IERC20 tokenToDiv = IERC20(0xD40bEDb44C081D2935eebA6eF5a3c8A31A1bBE13);
    ...
    // distribution period
    uint256 private _minPeriod = 6 * 60 * 60;
    ...
    function updateShare(address account, uint256 amount) external onlyOwner {
    ...
    function distribute(uint256 gas) external onlyOwner {
    ...
    function getDistributedToken() external view onlyOwner returns(address) {
        return address(tokenToDiv);
    }

    function isDistributeEnabled() external view onlyOwner returns(bool) {
        return _distributeEnabled;
    }

    function isDepositEnabled() external view onlyOwner returns(bool) {
    ...
    function getIndex() external view onlyOwner returns (uint256) {
...
contract MetaRewardMan is IERC20, IERC20Metadata, Context, Ownable {

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    // Name, symbol and decimals
    string private _name = "MetaRewardMan";
    string private _symbol = "MRM";
    ...
    // Fee variables
    uint256 private _buyDevFee = 9; // 9%
    uint256 private _buyDividendFee = 1; // 1%
    uint256 private _buyTotalFee = 10; // 10%
    uint256 private _sellDevFee =35; // 35%
    uint256 private _sellDividendFee = 5; // 5%
    uint256 private _sellTotalFee = 40; // 40%
    uint256 private _feeDenominator = 100;

    // Fee receivers
    address _devFeeReceiver = 0xa9982643071d853bd6732B8e1D8CC176aa9d3467;
    ...
    uint256 private _swapThreshold = _totalSupply * 10 / 100; // 10% of supply
    ...
    function getDistributedToken() external view returns(address) {
        return distributor.getDistributedToken();
    }
    ...
    function isDistributeEnabled() external view returns(bool) {
        return distributor.isDistributeEnabled();
    }

    function isDepositEnabled() external view returns(bool) {
        return distributor.isDepositEnabled();
    }

    function getDistributorOwner() external view returns (address) {
        return distributor.getOwner();
    }

    function getDistributorIndex() external view onlyOwner returns (uint256) {
    ...
    function setDistributorGasLimit(uint256 distributorGas) external onlyOwner  {
