...
contract SKFIPresale {
  ...
  event SKFIPurchase(address indexed buyer, address indexed referral, uint256 value, uint256 amount);
  ...
  function buy_token(address _beneficiary) public payable {
    require(msg.value >= 10000000000000000, "BEP20: Minimum buying is 0.01 BNB");
    ...
    emit SKFIPurchase(msg.sender, _beneficiary, weiAmount, tokens);
  ...
  function _preValidatePurchase(address _beneficiary, uint256 _weiAmount) public view returns (uint256) {
    ...
    if (tokenAmount > curBalance) {
      return curBalance.mul(1e8).div(rate);
    }
    return _weiAmount;
  }
  ...
  function _getTokenAmount(uint256 _weiAmount) public view returns (uint256) {
    return _weiAmount.mul(1e8).div(rate);
  }
