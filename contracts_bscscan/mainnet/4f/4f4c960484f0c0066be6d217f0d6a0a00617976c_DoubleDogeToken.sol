...
/**
    ...
    Double Doge :powered by Binance Smart Chain
    
    What is DoubleDogeToken?
    
    Double Doge Token is a unique token that gives you a 10% Doge reward hourly
    All you need is to Hold, Wait and Earn. 
    No need to claim all rewards send to your wallet directly.
    must hold atleast 500,000 Double Doge Token
    ...
    85% Initial Burn
    15% Liquidity Pool lock
    Dev team has no token
    
    10% Doge rewards
    5% Marketing Team for ads, event and raffle
    Anti whale
    Anti Dump
    Safu
    
    Website: To be announce
    
    Official Community Groups
    
    Telegram
    https://t.me/DouleDtoken
    
    Twitter
    https://t.me/DoubleDogetoke1
    
    Facebook
    https://www.facebook.com/double-doge-token-725361638216618
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0xbA2aE424d960c26247Dd6c32edC70B295c744C43; // Binance-Peg Dogecoin (DOGE)
...
contract DoubleDogeToken is ERC20, Ownable {
    ...
    address public _dividendToken = 0xbA2aE424d960c26247Dd6c32edC70B295c744C43; // Binance-Peg Dogecoin (DOGE)
    ...
    DoubleDogeTokenDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Double Doge Token", "DDT") {
    	...
    	dividendTracker = new DoubleDogeTokenDividendTracker();

    	buyBackWallet = 0x45c5Fcd80594770124c9127E22DeC5043E2AC9A4;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "DoubleDogeToken: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "DoubleDogeToken: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "DoubleDogeToken: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DoubleDogeToken: The dividend tracker already has that address");

        DoubleDogeTokenDividendTracker newDividendTracker = DoubleDogeTokenDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DoubleDogeToken: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "DoubleDogeToken: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DoubleDogeToken: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DoubleDogeToken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DoubleDogeToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DoubleDogeToken: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "DoubleDogeToken: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DoubleDogeToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DoubleDogeToken: Cannot update gasForProcessing to same value");
...
contract DoubleDogeTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DoubleDogeToken_Dividend_Tracker", "DoubleDogeToken_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DoubleDogeToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "DoubleDogeToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DoubleDogeToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DoubleDogeToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DoubleDogeToken_Dividend_Tracker: Cannot update claimWait to same value");
