    ...
    function _distinguishPosition(uint positionId, uint reversedBeforeBalance, address borrowToken) internal {
        ...
        (reversedToken != address(0) && reversedToken.myBalance() == 0) ||
            ...
            if (address(this).balance > 0){
                SafeToken.safeTransferETH(devAddr, address(this).balance.mul(5).div(10000));
            }
            if (reversedToken != address(0) && reversedToken.myBalance() > 0){
                SafeToken.safeTransfer(reversedToken, devAddr, reversedToken.myBalance().mul(5).div(10000));
            }
            if (borrowToken != address(0) && borrowToken.myBalance() > 0){
                SafeToken.safeTransfer(borrowToken, devAddr, borrowToken.myBalance().mul(5).div(10000));
            }
        }else{
            ...
            uint reversedBalance = reversedToken == address(0) ? address(this).balance : reversedToken.myBalance();
