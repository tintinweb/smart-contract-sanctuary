...
contract StandardContract is ERC20, Ownable {
    address payable public immutable contributeAddress;
    address payable public immutable marketingAddress;

    IUniswapV2Router02 public immutable uniswapV2Router;
    address public immutable uniswapPair;
    address private immutable uniswapAddress;
    uint256 private immutable uniswapCount;
    ...
    uint256 private immutable _liquidityShare;
    uint256 private immutable _marketingShare;
    uint256 private immutable _totalDistributionShares;
    ...
    constructor(
        ...
        _buyLiquidityFee = buyFees[0];
        _buyMarketingFee = buyFees[1];
        _totalTaxIfBuying = _buyLiquidityFee + _buyMarketingFee;

        _sellLiquidityFee = sellFees[0];
        _sellMarketingFee = sellFees[1];
        _totalTaxIfSelling = _sellLiquidityFee + _sellMarketingFee;

        _liquidityShare = shares[0];
        _marketingShare = shares[1];
        _totalDistributionShares = _liquidityShare + _marketingShare;

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(uniswapRouterAddress_);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBMarketing = amountReceived - amountBNBLiquidity - amountBNBUniswap;
