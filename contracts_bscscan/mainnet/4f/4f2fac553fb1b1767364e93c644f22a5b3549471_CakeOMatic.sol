...
// 🔥 CakeOMatic!!!!🔥
//
// ✅ Earn BOTH!! CAKE and MATIC just by holding CakeOMatic!!
//
// 🔥 Brand new coin!
// 🔥 Presale soon!
//
// ✅ TG: https://t.me/CakeOMatic
...
contract CakeOMatic is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("CakeOMatic", "COM") {
    	...
    	cakeDividendToken = 0x36f7e4479143aFe33602eFD83376397e8455Bd4b;
        maticDividendToken = 0x117813E73e0e04617f2564d965978fb1651E0228;
    ...
    receive() external payable {
  	...
  	function whitelistDxSale(address _presaleAddress, address _routerAddress) external onlyOwner {
        ...
        maticDividendTracker.excludeFromDividends(_presaleAddress);
        excludeFromFees(_presaleAddress, true);

        cakeDividendTracker.excludeFromDividends(_routerAddress);
        maticDividendTracker.excludeFromDividends(_routerAddress);
  	...
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "CakeOMatic: The team wallet is already this address");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "CakeOMatic: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(maticDividendRewardsFee).add(cakeDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setCakeDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = cakeDividendRewardsFee.add(marketingFee).add(maticDividendRewardsFee).add(buyBackAndLiquidityFee);
            cakeDividendEnabled = _enabled;
        }
    ...
    function setMaticDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = maticDividendRewardsFee.add(marketingFee).add(cakeDividendRewardsFee).add(buyBackAndLiquidityFee);
            maticDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(maticDividendRewardsFee).add(cakeDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "CakeOMatic: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "CakeOMatic: The new dividend tracker must be owned by the CakeOMatic token contract");
    ...
    function updateMaticDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(maticDividendTracker), "CakeOMatic: The dividend tracker already has that address");

        MaticDividendTracker newMaticDividendTracker = MaticDividendTracker(payable(newAddress));

        require(newMaticDividendTracker.owner() == address(this), "CakeOMatic: The new dividend tracker must be owned by the CakeOMatic token contract");
    ...
    function updateCakeDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 10, "CakeOMatic: Fee must be less than 10%");
    ...
    function updateMaticDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = maticDividendRewardsFee.add(cakeDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }
    
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= 5, "CakeOMatic: Fee must be less than 5%");
        marketingFee = newFee;
        totalFees = marketingFee.add(cakeDividendRewardsFee).add(maticDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= 5, "CakeOMatic: Fee must be less than 5%");
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(cakeDividendRewardsFee).add(maticDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "CakeOMatic: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "CakeOMatic: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CakeOMatic: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "CakeOMatic: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "CakeOMatic: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "CakeOMatic: Trading has not started yet");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (cakeDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(cakeDividendRewardsFee.add(maticDividendRewardsFee)).mul(cakeDividendRewardsFee);
                    swapAndSendCakeDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (maticDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(cakeDividendRewardsFee.add(maticDividendRewardsFee)).mul(maticDividendRewardsFee);
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("CakeOMatic_Cake_Dividend_Tracker", "CakeOMatic_Cake_Dividend_Tracker", 0x36f7e4479143aFe33602eFD83376397e8455Bd4b) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1 * (10**18); //must hold 1+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "CakeOMatic_Cake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "CakeOMatic_Cake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CakeOMatic contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CakeOMatic_Cake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CakeOMatic_Cake_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MaticDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("CakeOMatic_Matic_Dividend_Tracker", "CakeOMatic_Matic_Dividend_Tracker", 0x117813E73e0e04617f2564d965978fb1651E0228) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1 * (10**18); //must hold 1+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "CakeOMatic_Matic_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "CakeOMatic_Matic_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CakeOMatic contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CakeOMatic_Matic_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CakeOMatic_Matic_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
