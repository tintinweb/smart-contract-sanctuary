...
contract CryptoMarketingCompany is ERC20, Ownable {
    ...
    modifier feeValidation(uint8 buyFee, uint8 sellFee) {
        require(buyFee >= 0 && buyFee <= 100, "CMC: Invalid value");
        require(sellFee >= 0 && sellFee <= 100, "CMC: Invalid value");
        _;
    }

    constructor(CMCDividendTracker _weeklyTracker, CMCDividendTracker _monthlyTracker)  ERC20("Crypto Marketing Company", "CMCC") {
    ...
    function updateDividendTracker(address newAddress, uint256 _trackerType) external onlyOwner {
        require(
            ...
            "CMC: The dividend tracker already exists"
        );
        ...
        require(
            ...
            "CMC: Must be owned by the CMC token contract"
        );
    ...
    function setMarketingWallet(address payable wallet) external onlyOwner {
        require(wallet != address(0), "CMC: Zero address");
        _marketingWalletAddress = wallet;
    }
    
    function switchRewardTokenAddress(address rewardTokenAddress) external onlyOwner {
        require (rewardTokenAddress != address(0), "CMC: Zero address");
        _rewardTokenAddress = rewardTokenAddress;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "CMC: The PanusdtSwap pair cannot be removed"
        );
    ...
    function blacklistAddress(address account, bool value) external onlyOwner {
        require(_isBlacklisted[account] != value, "CMC: Previous value");
    ...
    function excludeFromDividends(address account) external onlyOwner {
        require(account != address(0), "CMC: Non zero address");
