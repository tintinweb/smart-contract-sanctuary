...
contract LockToken is Ownable {
    ...
    modifier open(address from, address to, bool doubleCheck) {
        require(isOpen || _whiteList[from] || _whiteList[to] || doubleCheck, "Not Open");
        _;
    }
    ...
    //Once it's open, can not lock the trade 
...
contract FlokiCash is Ownable, IBEP20, SafeToken, LockToken {
    ...
    string constant _name = "FlokiCash";
    string constant _symbol = "FLOCASH";
    uint8 constant _decimals = 9;
    uint256 public _totalSupply = 100000000000 * (10 ** _decimals);
    uint256 public _maxWallet = _totalSupply.mul(2).div(100); //Max wallet - Max Buy is 2% total Supply
    uint256 public _maxTxAmount = _totalSupply.mul(1).div(200); //Max sell one time is 0.5% total Supply
    ...
    bool autoLiquidity = false;

    uint256 targetLiquidity = 16;
    ...
    FlokiCashDividendTracker dividendTracker;
    ...
    constructor () {
        ...
        dividendTracker = new FlokiCashDividendTracker();
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal open(sender, recipient, excludeFee[recipient]) returns (bool) {
    ...
    function checkTxLimit(address sender, address recipient, uint256 amount) internal view {
        ...
        require(excludeMaxTxn[sender] || amount <= _maxTxAmount);
    }
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amount, bool _ignoreLimit, bool _autoLiquidity) external onlyOwner {
    ...
    function setTxLimit(uint256 amount) external onlyOwner {
        require(amount >= _totalSupply / 2000); // Can not set to 0
        _maxTxAmount = amount;
    }
...
contract FlokiCashDividendTracker is IDividendDistributor {
    using SafeMath for uint256;
    ...
    IBEP20 Bitcoin = IBEP20(0x2B3F34e9D4b127797CE6244Ea341a83733ddd6E4);
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    IDEXRouter router;
    ...
    uint256 public minDistribution = 50000 * (10 ** 9); // 0.00005 BTC minimum auto send  
    uint256 public minimumTokenBalanceForDividends = 1000000 * (10**9); // Must hold 1000,000 FlokiCash to receive Floki
