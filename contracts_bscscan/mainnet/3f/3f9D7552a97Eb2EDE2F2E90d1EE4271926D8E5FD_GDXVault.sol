...
contract GDXVault is Ownable {
    ...
    // gdx refferee
    IGDXReferral public gdxReferral;
    ...
    */
    ...
    /** 
    * @dev get total investments agains lockId
    * @param _userAddress address of user
    * @param _lockId lock id
    */
    function getStakeLength(address _userAddress, uint256 _lockId) external view returns (uint256) {
        return accounts[_userAddress][_lockId].stakes.length;
    }

    /** 
    * @dev set referral address
    * @param _gdxReferral address of referral contract
    */
    function setGDXReferral(address _gdxReferral) public onlyOwner {
        gdxReferral = IGDXReferral(_gdxReferral);
    }
    ...
    */
    function deposit(uint256 _amount, uint256 _lockId) external {
        LockInfo memory lock = lockInfo[_lockId];
     ...
     */
    function withdraw(uint256 _amount, uint256 _lockId, uint256 _index) public {
        StakeTracker storage account = accounts[msg.sender][_lockId];
        LockInfo memory lock = lockInfo[_lockId];
    ...
    */
    ...
    /**
    * @dev claim all funds
    * @param _from from index
    * @param _to to to
    */
    function harvestFromTo(uint256 _from, uint256 _to) external {
    ...
    */
    function harvest(uint256 _lockId, uint256 _index) public {
        ...
        uint256 percentage = gdxReferral.getPercentage();
        uint256 nominator = gdxReferral.getRateNominator();

        uint256 referralAmount = (reward.mul(percentage)).div(nominator);
        _safeRewardTransfer(address(gdxReferral), referralAmount);
        gdxReferral.addReferralBonus(msg.sender, referralAmount);
        ...
        if(compound[msg.sender] == true) {
            ...
            totalStakingTokens = totalStakingTokens.add(reward.sub(referralAmount));
        }else{
            _safeRewardTransfer(msg.sender, reward.sub(referralAmount));
        }
