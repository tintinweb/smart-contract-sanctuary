...
contract BananaMaker is BananaBridgedSwapper, IFeesDistributor {
    ...
    function distribute(address token) external override {
        doDistributionInternal(token);
    }

    function distributeMany(address[] calldata tokens) external override {
        uint length = tokens.length;
        for (uint i; i < length; i++) {
            doDistributionInternal(tokens[i]);
        }
    }
    ...
    function mintOBananas(uint bananasToSupply) internal returns (uint oBananasMinted) {
        ...
        // TODO : CRITICAL : SAFE MATH
        oBananasMinted = balanceAfter - balanceBefore;
    }
