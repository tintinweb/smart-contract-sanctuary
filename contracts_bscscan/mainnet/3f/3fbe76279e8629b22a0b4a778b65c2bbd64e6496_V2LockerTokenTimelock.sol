 ...
 */
contract V2LockerTokenTimelock is Ownable {
    using SafeERC20 for IERC20;

    // 11/11/2030 timestamp when token release is enabled
    uint256 public releaseTime = 1628151379;
    
    address public HGDToken = 0x39c1324Ee21b2727D3B850F02c44cd4D4Ee4761F;
    address public lpToken = 0x846778366eBa1Ac3B0E19E5796825f8d2d6C8AbA;

    /**
     * @notice Transfers tokens held by timelock to owner.
     */
    function releaseHGDToken() external onlyOwner {
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp >= releaseTime, "TokenTimelock: current time is before release time");

        uint256 amount = IERC20(HGDToken).balanceOf(address(this));
        require(amount > 0, "TokenTimelock: no tokens to release");

        IERC20(HGDToken).safeTransfer(owner(), amount);
    }
    ...
    function getHGDTokenBalance() external view returns(uint256) {
        return IERC20(HGDToken).balanceOf(address(this));
    }
    ...
    function setHGDToken(address _HGDToken) external onlyOwner {
        HGDToken = _HGDToken;
    }
