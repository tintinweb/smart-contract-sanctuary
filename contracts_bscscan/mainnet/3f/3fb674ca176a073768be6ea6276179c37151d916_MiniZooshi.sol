...
import "./MiniZooshi-imports.sol";

/**
 * @dev If I did a good job you should not need to change anything apart from the values in the `Tokenomics`,
 * the actual name of the contract `MiniZooshi` at the very bottom **and** the `environment` into which
 * you are deploying the contract `MiniZooshi(Env.Testnet)` or `MiniZooshi(Env.MainnetV2)` etc.
 * 
...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "MiniZooshi";
    string internal constant SYMBOL = "MZOOSHI";
...
contract MiniZooshi is SafeToken{
