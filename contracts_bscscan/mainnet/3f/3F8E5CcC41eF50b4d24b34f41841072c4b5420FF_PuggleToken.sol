...
╚═╝      ╚═════╝  ╚═════╝  ╚═════╝ ╚══════╝╚══════╝
 SELL:
    AUTO LIQUIDITY POOL: 7%
    REWARD HOLDERS: 2%
    MARKETING AND DEV EXPENSES: 7%
    BURN: 1%     
 BUY:                                              
...
contract PuggleToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 610000000000 * 10**_decimals;
    ...
    address public operationsAddress = 0xF0f0e4984bf46559767B5af2151824c28Fb1c773;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "Puggle";
    ...
    function setFeeRates(uint256 _rfi, uint256 _operations, uint256 _liquidity, uint256 _burn) external onlyOwner {
        feeRates.rfi = _rfi;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _operations, uint256 _liquidity, uint256 _burn) external onlyOwner{
    ...
    function _takeBurn(uint256 rBurn, uint256 tBurn) private {
        ...
        {
            _tOwned[address(deadAddress)]+=tBurn;
        }
        _rOwned[address(deadAddress)] +=rBurn;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rOperations, to_return.rLiquidity, to_return.rBurn) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tTransferAmount = tAmount-s.tRfi-s.tOperations-s.tLiquidity-s.tBurn;
        }
        else{
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rOperations, uint256 rLiquidity, uint256 rBurn) {
        ...
        rTransferAmount =  rAmount-rRfi-rOperations-rLiquidity-rBurn;
        return (rAmount, rTransferAmount, rRfi,rOperations,rLiquidity, rBurn);
    }
