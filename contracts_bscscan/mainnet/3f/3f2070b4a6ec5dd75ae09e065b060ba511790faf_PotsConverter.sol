...
contract PotsConverter is OwnableUpgradeable {
    ...
    function singleHarvest(uint256 pid, uint256 index) public onlyHarvester {
            ...
            if (convert.treasuryAlloc > 0) {
                ...
                uint256[] memory amounts = IUniswapRouterETH(convert.router).swapExactTokensForTokens(treasuryAlloc, 0, convert.treasuryRoute, address(this), now);
                uint256 treasuryIn = amounts[amounts.length - 1];
                address treasuryToken = convert.treasuryRoute[convert.treasuryRoute.length - 1];
    ...
    function setSingleRoute(uint256 prizeId, address[] memory _route, address[] memory _treasuryRoute) external onlyOwner {
        require(_route[0] == singlePrize[prizeId].prizeToken);
        require(_route[_route.length - 1] == pots);

        singlePrize[prizeId].route = _route;
        singlePrize[prizeId].treasuryRoute = _treasuryRoute;
    }
