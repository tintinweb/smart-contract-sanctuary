...
contract SingleStaking is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock; // Last block timestamp that Rewards distribution occurs.
    ...
    event SetBlockReward(uint256 reward);
    event SetFeeAddress(address indexed user, address indexed newAddress);
    event SetDepositFee(address indexed user, uint16 amount);
    event NewStartAndEndBlocks(uint256 startBlock, uint256 bonusEndBlock);
    ...
    function setTokenPerBlock(uint256 _tokenPerBlock) external onlyOwner {
        ...
        tokenPerBlock = _tokenPerBlock;
        emit SetBlockReward(_tokenPerBlock);
    }
    ...
    function startReward() external onlyOwner {
        ...
        bonusEndBlock = startBlock.add(365 * 28800); // 365 days
    ...
    function updateEndBlock(uint256 _endblock) external onlyOwner {
        require(_endblock > startBlock && _endblock > block.number, "Invalid block");
    ...
    function adjustBlockEnd() external onlyOwner {
        ...
        bonusEndBlock = block.number + totalLeft.div(tokenPerBlock);
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > poolInfo.lastRewardBlock && supply != 0 && poolInfo.lastRewardBlock > 0) {
    ...
    function updatePool() public {
        if (block.number <= poolInfo.lastRewardBlock || poolInfo.lastRewardBlock == 0) {
            return;
        }
        uint256 supply = poolInfo.token.balanceOf(address(this));
        if (supply == 0) {
    ...
    function deposit(uint256 _amount) external {
        ...
        if (_amount > 0) {
            uint256 beforeAmount = poolInfo.token.balanceOf(address(this));
            poolInfo.token.safeTransferFrom(
                address(msg.sender),
                address(this),
                _amount
            );
            uint256 afterAmount = poolInfo.token.balanceOf(address(this));

            realAmount = afterAmount.sub(beforeAmount);
            if(poolInfo.depositFee > 0) {
                 uint256 fee = realAmount.mul(poolInfo.depositFee).div(10000);
                if (fee > 0) {
                    poolInfo.token.safeTransfer(feeAddr, fee);
    ...
    function emergencyRewardWithdraw() external onlyOwner {
        ...
        uint256 _amount = availableRewardTokens();
    ...
    function updateStartTime(uint256 _startBlock) external onlyOwner {
        ...
        onlyOwner
    {
        ...
        if(_tokenAddress == address(depositToken)) {
