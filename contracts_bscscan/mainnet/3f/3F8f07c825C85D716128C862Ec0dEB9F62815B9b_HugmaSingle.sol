    ...
    Ownable
{
    ...
    constructor(uint256 id, uint256 _serviceValue)
        ...
        BEP721("HUGMA New Year 2022", "HGNEWYEAR")
    {
        _tid = id;
    ...
    {
        uint256 fee = (amount / 100) * _serviceValue;
        uint256 roy = (amount / 100) * royal;
        uint256 netamount = amount - (fee + roy);
        return (fee, roy, netamount);
    }
    ...
    function saleTokenwithToken(
        ...
        newtokenasbid(token, from, owner(), amount, tokenId);
    ...
    function transferFrom_ts(address token, uint256 amount) public {
        BEP20(token).approve(address(this), amount);
        BEP20(token).transferFrom(address(msg.sender), address(this), amount);
    }
    ...
    function newtokenasbid(
        ...
        require(balances[tokenId][from] > 0, "Is Not a Owner");
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc_custom(
            amount,
            _royal[tokenId],
            serviceValue
        );
        t.transferFrom(address(msg.sender), address(this), amount);
    ...
    function acceptBId_Tok(
        ...
        BEP20 t = BEP20(token);
        uint256 adminfee = (amount / 100) * serviceValue;
        if (adminfee > 0) {
            t.transferFrom(to, owner(), adminfee);
        }
        uint256 roy = (amount / 100) * _royal[tokenId];
        if (roy > 0) {
            t.transferFrom(to, _creator[tokenId], roy);
        }
        uint256 netamount = amount - (roy + adminfee);
        t.transferFrom(to, msg.sender, netamount);
    ...
    function saleToken_Tok(
        address token,
        address payable from,
        uint256 tokenId,
        uint256 amount
    ) public payable {
        _saleToken_Tok(token, from, tokenId, amount);
        saleTokenTransfer(from, tokenId);
    }

    function _saleToken_Tok(
        address token,
        address payable from,
        uint256 tokenId,
        uint256 amount
        ...
        uint256 adminfee = (amount / 100) * serviceValue;
        if (adminfee > 0) {
            t.transferFrom(msg.sender, owner(), adminfee);
        }
        uint256 roy = (amount / 100) * _royal[tokenId];
        if (roy > 0) {
            t.transferFrom(msg.sender, _creator[tokenId], roy);
