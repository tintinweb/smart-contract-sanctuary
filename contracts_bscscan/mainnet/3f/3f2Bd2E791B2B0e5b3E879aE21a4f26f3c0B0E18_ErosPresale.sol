...
contract ErosPresale is OwnerRole {
    using SafeMath for uint256;
    ...
    constructor(BEP20 _token, uint256 _rate, IReferral _referral) public {
        token = _token;
        rate = _rate;
        minPurchase = _rate;
    ...
    function purchase(address _referrer) public payable {
        ...
        if (msg.value > 0 && referral.getReferrer(msg.sender) == address(0) && _referrer != address(0) && _referrer != msg.sender) {
    ...
    function setReferralRate(uint256 _rate) external onlyOwner {
        require(_rate >= 0 && _rate <= 1000, "PRESALE: rate error");
        referralRate = _rate;
    }
    ...
    function deliverTokens(address _to, uint256 _amount) internal {
        ...
        if (referrer != address(0)) {
            ...
            token.mint(referrer, totalRewards);
            referral.addRewards(referrer, "presale", totalRewards);
        }
    }
