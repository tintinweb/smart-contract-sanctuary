...
// File contracts/bsc/interfaces/belt/IMasterBelt.sol
...
// File contracts/bsc/interfaces/belt/IMultiStrategyToken.sol
...
// File contracts/bsc/byalan/belt/BeltByalanMultiToken.sol
...
contract BeltByalanMultiToken is ByalanIsland, Sailor, IByalan {
    ...
    address public constant MASTERCHEF = 0xD4BbC80b9B102b77B21A06cb77E954049605E6c1;
    uint256 public immutable pid;

    // Routes
    address[] public beltToWbnbRoute;

    event Harvest(address indexed harvester);

    address[] public beltToTokenRoute;
    ...
    constructor(
        ...
        address _harvester,
        uint256 _pid,
        address[] memory _beltToTokenRoute
    )
        ByalanIsland(
            _hydra,
        ...
        )
    {
        ...
        want = IMasterBelt(MASTERCHEF).poolInfo(_pid).want;
        strat = IMasterBelt(MASTERCHEF).poolInfo(_pid).strat;
        token = IMultiStrategyToken(want).token();
        beltToWbnbRoute = [BELT, WBNB];

        require(_beltToTokenRoute[0] == BELT, "invalid token first route");
        require(_beltToTokenRoute[_beltToTokenRoute.length - 1] == token, "invalid token last route");
        require(
            IUniswapV2Router02(unirouter).getAmountsOut(1 ether, _beltToTokenRoute)[_beltToTokenRoute.length - 1] > 0,
            "invalid token route"
        );
        beltToTokenRoute = _beltToTokenRoute;
    ...
    function deposit() public override whenNotPaused {
        ...
        if (wantBal > 0) {
            IMasterBelt(MASTERCHEF).deposit(pid, wantBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external override onlyIzlude {
        ...
        if (wantBal < _amount) {
            IMasterBelt(MASTERCHEF).withdraw(pid, _amount - wantBal);
    ...
    function _harvest() private {
        IMasterBelt(MASTERCHEF).withdraw(pid, 0);

        uint256 rewardBal = IERC20(BELT).balanceOf(address(this));
        if (rewardBal > 0) {
            chargeFees();
            depositBelt();
            deposit();
    ...
    function chargeFees() internal {
        uint256 toBnb = (IERC20(BELT).balanceOf(address(this)) * totalFee) / MAX_FEE;
        IUniswapV2Router02(unirouter).swapExactTokensForETH(toBnb, 0, beltToWbnbRoute, address(this), block.timestamp);
    ...
    function depositBelt() internal {
        ...
        IMultiStrategyToken(want).deposit(tokenBal, 0);
    }
    ...
    function balanceOfPool() public view override returns (uint256) {
        return IMasterBelt(MASTERCHEF).userInfo(pid, address(this)).shares;
    }

    function balanceOfMasterChef() external view override returns (uint256) {
        return IERC20(want).balanceOf(strat);
    }
        ...
        returns (IERC20[] memory rewardTokens, uint256[] memory rewardAmounts)
    {
        ...
        rewardTokens[0] = IERC20(BELT);
        rewardAmounts[0] =
            IMasterBelt(MASTERCHEF).pendingBELT(pid, address(this)) +
            IERC20(BELT).balanceOf(address(this));
    }
    ...
    function retireStrategy() external override onlyIzlude {
        IMasterBelt(MASTERCHEF).emergencyWithdraw(pid);
    ...
    function _giveAllowances() internal {
        ...
        IERC20(token).safeApprove(want, type(uint256).max);
    }

    function _removeAllowances() internal {
        IERC20(want).safeApprove(MASTERCHEF, 0);
        IERC20(BELT).safeApprove(unirouter, 0);
        IERC20(token).safeApprove(want, 0);
    }
