...
contract BondingCalculator is IBondCalculator {
    ...
    using CowSafeMath for uint;
    using CowSafeMath for uint112;

    address public immutable COW;

    constructor( address _COW ) {
    ...
    function markdown( address _pair ) external view override returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == COW ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( COW ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}