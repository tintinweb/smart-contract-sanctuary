...
contract TickALock is Ownable, IBEP20 {
    ...
    uint256 public _totalSupply5 = _totalSupply * 500 / 1000;
    ...
    constructor() {
        ...
        lpTaxer = new LPTaxer(address(_pancakeRouter), address(this), address(BUSDToken), _burnWallet, _devWallet, _marketingWalletAddress);
    ...
    function swapContractTokens() private lockTheSwap {
        uint256 prizeTokens = _balances[address(this)] - _totalAllocatedLiquidityContract - _tokensToBurn - _totalAllocatedMarketingContract;
        uint256 remainingBalance = _balances[address(this)] - prizeTokens - _tokensToBurn - _totalAllocatedMarketingContract;
        require(
        ...
        _transferExcluded(address(this), address(lpTaxer), tokensForLP + tokensForMarketing);
        
        (uint256 half, uint256 newBalance, uint256 otherHalf) = lpTaxer.swapAndLiquify(tokensForLP, tokensForMarketing); 
...
contract LPTaxer {
    ...
    constructor (address router, address tokenAddress, address busdAddress, address burnWallet, address devWallet, address marketingWallet) {
    ...
    function swapAndLiquify(uint256 tokensForLP, uint256 tokensForMarketing) external onlyToken returns (uint256, uint256, uint256) {
        if(tokensForMarketing > 0) {
            //swap tokens for Marketing to BUSD and send to marketing wallet
            // swap the tokens for BUSD
            _swapTokensForBUSD(tokensForMarketing);
            uint256 contractBUSDBalance = BUSDToken.balanceOf(address(this));
            // send the tokens to the marketing wallet
            BUSDToken.transfer(_marketingWallet, contractBUSDBalance);
        }
