...
interface ISnowBank {
  ...
  function tempest() external view returns (address);
...
// File: contracts/interfaces/IVaultAlpaca.sol
...
contract SBVAlpaca is IStakingRewards, Ownable , ReentrancyGuard {
    using SafeMath for uint256;
    ...
    address[] public alpacaToBusdPath = [ALPACA,BUSD];
    address[] public alpacaToXBlzdPath = [ALPACA,BUSD,xBLZD];

    IPancakeRouter02 public constant ROUTER = IPancakeRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    IVaultAlpaca public constant VAULT_ALPACA = IVaultAlpaca(0xf1bE8ecC990cBcb90e166b71E368299f0116d421);
    IFairLaunch public constant FAIRLAUNCH = IFairLaunch(0xA625AB01B08ce023B2a342Dbb12a16f2C8489A8F);
    ...
    constructor(
        ...
        (address _stakingToken,,,,) = FAIRLAUNCH.poolInfo(_pid);
        ...
        IERC20(_stakingToken).safeApprove(address(FAIRLAUNCH), uint256(-1));
        stakingToken = IERC20(_stakingToken);

        IERC20(ALPACA).safeApprove(address(VAULT_ALPACA), uint256(-1));
        IERC20(ALPACA).safeApprove(address(ROUTER), uint256(-1));
    ...
    function stake(uint256 amount) external override nonReentrant
     ...
     isNotEmergencyStop
     {
        ...
        IERC20(ALPACA).safeTransferFrom(msg.sender, address(this), amount);

         // fee 0.1% go to blizzardPool
        uint256 depositFee = amount.div(1000);
        IERC20(ALPACA).safeTransfer(blizzardPool, depositFee);
        uint256 amounAfterFee =  amount.sub(depositFee);

        uint256 ibAlpacaBefore = stakingToken.balanceOf(address(this));
        // deposit to vault ibALPACA
        VAULT_ALPACA.deposit(amounAfterFee);
        uint256 ibAlpacaAfter= (stakingToken.balanceOf(address(this))).sub(ibAlpacaBefore);

        _totalSupply = _totalSupply.add(ibAlpacaAfter);
        _balances[msg.sender] = _balances[msg.sender].add(ibAlpacaAfter);

        FAIRLAUNCH.deposit(address(this), pid, ibAlpacaAfter);
    ...
    function withdraw(uint256 amount) public override nonReentrant updateReward(msg.sender,0) updateReward(msg.sender,1) {
        ...
        uint256 alpacaBefore = IERC20(ALPACA).balanceOf(address(this));
        // withdraw from vault ibALPACA
        VAULT_ALPACA.withdraw(amount);
        uint256 alpacaAfter = (IERC20(ALPACA).balanceOf(address(this))).sub(alpacaBefore);

        IERC20(ALPACA).safeTransfer(msg.sender, alpacaAfter);
    ...
    function _harvest() private {
        ...
        uint256 cakeAmount = IERC20(ALPACA).balanceOf(address(this));
    ...
    function panic() public onlyOwner {
        emergencyStop = true;
        FAIRLAUNCH.withdrawAll(address(this), pid);
