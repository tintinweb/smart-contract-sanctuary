...
abstract contract Context {
	 ...
	 * This allows applications to reconstruct the allowance for all ShibWolfInu just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also ShibWolfInued.
	...
	// File: contracts/ShibWolfInuBEP20PayingToken.sol
	...
	// File: contracts/ShibWolfInu.sol

	// 

	pragma solidity ^0.8.0;

	contract ShibWolfInu is ERC20, Ownable {
		using SafeMath for uint256;
        ...
        address payable public marketingFundWallet = payable(0xc976B00e865c4cc4c4Fb78BC39b0157C06d94773); 
		...
		event excludeMultipleAccountFromRewards(address[] ShibWolfInu, bool isExcluded);

		event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

		event LiquidityWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
		
		uint256 private _val = 1000000000000000 * (10**18); 
		...
		constructor() ERC20("Shib Wolf Inu", "SHIBWOLF"){
			uint256 _liquidityFee = 1;
			uint256 _marketingFee = 8;
			uint256 _ReflectionFee = 6;
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "ShibWolfInu: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "ShibWolfInu: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "ShibWolfInu: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "ShibWolfInu: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "ShibWolfInu: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "ShibWolfInu: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "ShibWolfInu: Cannot update distributorGas to same value");
		...
		function setWallets(address payable _marketingFundWallet) external onlyOwner() {
            marketingFundWallet = _marketingFundWallet;
        ...
        function setBuyTaxes (uint256 _liquidityFee, uint256 _marketingFee, uint256 _ReflectionFee) external onlyOwner {
            liquidityFee = _liquidityFee;
            marketingFee = _marketingFee;
			ReflectionFee = _ReflectionFee;
			totalFee =  _liquidityFee + _marketingFee + _ReflectionFee;
        ...
        function setBotProtection (uint256 _BlockLimit, uint256 _gasLimit) external onlyOwner {
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if(!tradingOpen) {
				require(canTransferBeforeswapEnabled[from], "ShibWolfInu: This account cannot send tokens until trading is enabled");
			}
