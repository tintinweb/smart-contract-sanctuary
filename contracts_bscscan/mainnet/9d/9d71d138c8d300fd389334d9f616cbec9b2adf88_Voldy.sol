...
contract Voldy is Ownable, VoldyMetaData {
   ...
   */
	...
	function _transfer(
        ...
        if( 
        	startTrading &&
            recipient == uniswapV2WETHPair && // sells only by detecting transfer to AMM
        	sender != address(uniswapV2Router) && //router -> pair is removing liquidity which shouldn't have max
            !_isExcludedFromFees[sender] && //no max for those excluded from fees
            sender != presaleRouter &&
            sender != presaleAddress
        ) {
