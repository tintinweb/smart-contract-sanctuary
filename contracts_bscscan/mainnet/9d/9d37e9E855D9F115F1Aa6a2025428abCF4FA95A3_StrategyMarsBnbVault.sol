...
/**
 * @dev Implementation of a strategy to get yields from farming LP Pools in Climb.
 * Climb is an automated market maker (“AMM”) that allows two tokens to be exchanged on the Binance Smart Chain.
 * It is fast, cheap, and allows anyone to participate. Climb is aiming to be the #1 liquidity provider on BSC.
 *
 ...
 */
contract StrategyMarsBnbVault is Ownable, Pausable {
     ...
     * {mars} - Climb Finance token, used to send funds to the MARS contract.
     * {lpPair} - Token that the strategy maximizes. The same token that users deposit in the vault.
     * {lpToken0, lpToken1} - Tokens that the strategy maximizes. Pancake tokens
     */
    ...
    address public constant masterchef =
        address(0x2556fDd7c00f17C211ec56D16055FaF5E8656590);
    uint256 public poolId;

    /**
     * @dev Climb Contracts:
     * {rewards} - Reward pool where the strategy fee earnings will go.
     * {treasury} - Address of the Climb Finance treasury
     ...
     *
     * {REWARDS_FEE} - 4% goes to Climb Tokens holders through the Climb contract.
    ...
    /**
     * @dev Routes we take to swap tokens using Climb.
     * {marssToWbnbRoute} - Route we take to get from {mars} into {wbnb}.
     * {wbnbToMarsRoute} - Route we take to get from {wbnb} into {mars}.
     * {marsToLp0Route} - Route we take to get from {mars} into {lpToken0}.
     * {marsToLp1Route} - Route we take to get from {mars} into {lpToken1}.
     */
    address[] public marssToWbnbRoute = [mars, wbnb];
    ...
    constructor(
        ...
        if (lpToken0 == wbnb) {
            marsToLp0Route = [mars, wbnb];
        } else if (lpToken0 != mars) {
            marsToLp0Route = [mars, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            marsToLp1Route = [mars, wbnb];
        } else {
            marsToLp1Route = [mars, wbnb, lpToken1];
        }

        IERC20(mars).safeApprove(unirouter, uint256(-1));
     ...
     * It deposits {lpPair} in the MasterChef to farm {mars}
     */
     ...
     * 0.5% -> Call Fee
     * 1.5% -> Treasury fee
     * 4% -> Climb Tokens Holders
     */
    //  function chargeFees() internal {
    //     uint256 toWbnb = IERC20(mars).balanceOf(address(this)).mul(6).div(100);
    ...
    /**
     * @dev Swaps {mars} for {lpToken0}, {lpToken1} & {wbnb} using Climb.
     */
    function addLiquidity() internal {
        uint256 toLpToken1 = IERC20(mars).balanceOf(address(this)).div(2);
        IPancakeRouter(unirouter).swapExactTokensForTokens(
            toLpToken1,
            0,
            marsToLp1Route,
        ...
        if (lpToken0 != mars) {
            uint256 toLpToken0 = IERC20(mars).balanceOf(address(this));
            IPancakeRouter(unirouter).swapExactTokensForTokens(
                toLpToken0,
                0,
                marsToLp0Route,
