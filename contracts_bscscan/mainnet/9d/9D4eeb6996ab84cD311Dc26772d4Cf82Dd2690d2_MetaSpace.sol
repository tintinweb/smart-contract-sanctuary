...
// Brining Utility to the DEFI ecosytem 
// Join our Telegram: https://t.me/metaspaceofficial
// Website: http://metaspacetoken.net/
// Twitter: http://www.twitter.com/metaspacenet
...
contract MetaSpace is IERC20Metadata, Ownable {
    ...
    address public marketingWallet = 0x8d7de188Fe29D9718e9FfA2B43d5C72A133d10E2;
    address public devAndPrizePoolWallet = 0x22d83d6F2e19D0E8f2F2EBC1100A996A4Ae165EE;
    
    string constant _name = "MetaSpace";
    ...
    uint256 public marketingFee = 35; // 3.5%
    uint256 public devAndPrizePoolFee = 15; // 1.5%
    uint256 public lpFee = 10; // 1%
    uint256 public totalFee = marketingFee.add(devAndPrizePoolFee).add(lpFee);
    
    // Total = 100% (1000)
    uint256 public marketingPortionOfSwap = 600;
    uint256 public lpPortionOfSwap = 150;
    uint256 public devAndPrizePoolPortionOfSwap = 250;
    ...
    event DevAndPrizePoolWalletUpdated(address indexed newDevWallet, address indexed oldDevWallet);
    ...
    function calcTotalFees() private {
        totalFee = marketingFee.add(devAndPrizePoolFee).add(lpFee);
    }
    ...
    function updateDevAndPrizePoolWallet(address newWallet) external onlyOwner {
        require(newWallet != devAndPrizePoolWallet, "The Dev&PrizePool wallet is already this address");
        emit DevAndPrizePoolWalletUpdated(newWallet, devAndPrizePoolWallet);

        devAndPrizePoolWallet = newWallet;
    }
    
    function updatePortionsOfSwap(uint256 marketingPortion, uint256  devAndPrizePoolPortion, uint256 lpPortion) 
    external onlyOwner {
        
        uint256 totalPortion = marketingPortion.add(devAndPrizePoolPortion).add(lpPortion);
        require(totalPortion == 1000, "Total must be equal to 1000 (100%)");
        
        marketingPortionOfSwap = marketingPortion;
        devAndPrizePoolPortionOfSwap = devAndPrizePoolPortion;
    ...
    function toggleAntiBot(bool toggleStatus) external onlyOwner() {
        ...
        if(antiBotEnabled){
            _startTimeForSwap = block.timestamp + 20;    
        }    
    }
    ...
    function swapBack() internal {
        ...
        uint256 amountBNBDev = amountBNB.mul(devAndPrizePoolPortionOfSwap).div(FEES_DIVISOR);
        ...
        transferToAddress(payable(devAndPrizePoolWallet), amountBNBDev);
