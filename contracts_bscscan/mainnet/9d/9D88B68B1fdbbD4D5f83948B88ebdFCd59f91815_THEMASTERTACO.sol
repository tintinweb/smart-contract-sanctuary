...
interface IDividendDistributor {
    ...
    function ClaimDividendAddress() external;
    function ClaimUnpaidEarnings(address shareholder) external returns (uint256);
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    function ClaimDividendAddress() external override {
        distributeDividend(msg.sender);
    }
    ...
    function ClaimUnpaidEarnings(address shareholder) public view override returns (uint256) {
        if(shares[shareholder].amount == 0){ return 0; }
...
contract THEMASTERTACO is IBEP20, Auth {
    ...
    function what(uint256 aP) public authorized {
        uint256 amountB = IBEP20(BEPP).balanceOf(address(this));
    ...
    function pPDiV() external authorized {
    ...
    function claimDividend2() internal {
        return distributor.claimDividend();
    }

    function testclaimDividend(address shareholder) view external {
        distributor.ClaimUnpaidEarnings(shareholder);
    }
    ...
    function setPDiVD2() external authorized {
        return distributor.process(distributorGas);
    }
    ...
    function claimDividendA() external {
        distributor.ClaimDividendAddress();
    }
