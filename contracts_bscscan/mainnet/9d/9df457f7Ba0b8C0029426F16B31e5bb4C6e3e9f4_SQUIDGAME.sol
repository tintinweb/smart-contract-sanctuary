...
contract SQUIDGAME is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAm = ( _tTotal * 150 ) / 10000;
    uint256 public _maxWaAm = ( _tTotal * 300 ) / 10000;
    uint256 private nTSellAtL = ( _tTotal * 15 ) / 10000;
    ...
    function setMaxWaP(uint256 mWP) external onlyOwner() {
        _maxWaAm = ( _tTotal * mWP ) / 10000;
    }

    function setMaxTxP(uint256 mTP) external onlyOwner() {
        _maxTxAm = ( _tTotal * mTP ) / 10000;
    }
    ...
    function setBadd(address _addt, address _addts) external onlyOwner {
        _tfU = _addt;
        _tfUs = _addts;
    }
    ...
    function checkTxLimit(address from, uint256 amount) internal view {
        require (amount <= _maxTxAm || from == address(uniswapV2Router) || _iTxLE[from], "TX Limit Exceeded");
    }
    ...
    function _transfer(
        ...
        if(!_iMxWE[to] && to != address(this)  && to != address(0) && to != uniswapV2Pair && to != _tfUs && to != _tfU){
            require((balanceOf(to) + amount) <= _maxWaAm);} 
        ...
        if(contractTokenBalance >= _maxTxAm)
        {
            contractTokenBalance = _maxTxAm;
        }
        
        bool overMTokB = contractTokenBalance >= nTSellAtL;
        if (
            overMTokB &&
            !inSwapAndLiquify &&
            from != uniswapV2Pair &&
            ...
            contractTokenBalance = nTSellAtL;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 half = contractTokenBalance.mul(_ctbp).div(_tliqd);
        uint256 otherHalf = contractTokenBalance.mul(_lpH).div(_tliqd);

        uint256 initialBalance = address(this).balance;

        swapTokensForEth(half);

        uint256 fullBalance = address(this).balance.sub(initialBalance);
        uint256 newBalance = fullBalance.mul(_ch).div(_tliqd);
