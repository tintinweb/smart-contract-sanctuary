...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    Moonlift public mltp;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public mltpPerBlock;
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _mltpPerBlock,
        uint256 _startBlock
    ) public {
        mltp = _mltp;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        mltpPerBlock = _mltpPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 mltpReward = multiplier.mul(mltpPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(mltpReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 mltpReward = multiplier.mul(mltpPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        mltp.transfer(devaddr, mltpReward.div(10));
        // mltp.transfer(address(msg.sender), mltpReward);
        pool.accEggPerShare = pool.accEggPerShare.add(mltpReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 mltpBal = mltp.balanceOf(address(this));
        if (_amount > mltpBal) {
            mltp.transfer(_to, mltpBal);
        } else {
            mltp.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _mltpPerBlock) public onlyOwner {
        massUpdatePools();
        mltpPerBlock = _mltpPerBlock;
    }
