...
contract Master is Ownable {
    ...
    function get_rebase_percentage(uint256 _percent_base1000, bool reduce)  public view returns (int) {
        
        if(reduce){
            return int(FSTM.totalSupply().div(1000).mul(_percent_base1000)).mul(-1);
        } 
        
        return int(FSTM.totalSupply().div(1000).mul(_percent_base1000));
        
    }
    
    
    function rebase_percentage(uint256 _percent_base1000, bool reduce)  public onlyOwner{
        ...
        if (supplyDelta > 0 && !posRebaseEnabled) {
         ...
         if(reduce){
            supplyDelta = int(FSTM.totalSupply().div(1000).mul(_percent_base1000)).mul(-1);
        } else{
            supplyDelta = int(FSTM.totalSupply().div(1000).mul(_percent_base1000));
        }
