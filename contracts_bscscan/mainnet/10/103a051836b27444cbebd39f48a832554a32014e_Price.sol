...
contract Price{
    ...
    function priceFirst(uint256 _pid) view public returns (uint256){
        (, , , , , , , ,,,address first,,,) = kingDeFiFarm.poolInfo(_pid);
        address token = factory.getPair(first, address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56));
        (uint112 reserveLP0, uint112 reserveLP1, ) = IPancakeLPToken(token).getReserves();
        uint256 prezzoToken = router.quote(1000000000000000000, reserveLP0, reserveLP1);
        return prezzoToken;
    }
    
    function priceSecond(uint256 _pid) view public returns (uint256){
        (, , , , , , , ,,,, address second,,) = kingDeFiFarm.poolInfo(_pid);
        address token = factory.getPair(second, address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56));
        (uint112 reserveLP0, uint112 reserveLP1, ) = IPancakeLPToken(token).getReserves();
        uint256 prezzoToken = router.quote(1000000000000000000, reserveLP0, reserveLP1);
        return prezzoToken;
    }
