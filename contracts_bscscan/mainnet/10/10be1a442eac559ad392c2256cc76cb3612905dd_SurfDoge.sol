...
contract SurfDoge is BEP20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 2000000 * (10**18); //Later shuld be changed to 40k
	...
	event BNBForLotteryWinningTransferError (address indexed error);
    ...
    constructor() BEP20("SurfTest11", "Test11") {
   ...
   function getUnlockTimeHours() public view returns (uint256) {
        if (_owner == address(0)) {
            if (_lockTime > block.timestamp) {
                return (_lockTime - (block.timestamp)) / (3600);
            }
    ...
    function getUnlockTimeDays() public view returns (uint256) {
        if (_owner == address(0)) {
            if (_lockTime > block.timestamp) {
                return (_lockTime - (block.timestamp)) / (86400);
            }
   ...
   function receiveRandomness(bytes32 hash, uint256 randomNumberProvided) external {
    ...
    function LotteryDistributeWinnings () external {
        ...
        if (lotteryBalance > currentContractBalanceLottery) {
            lotteryBalance = currentContractBalanceLottery;
        ...
        for (uint i = 10; i > 0; i--) {
            ...
            if (!successfulLotteryTransfer) { emit BNBForLotteryWinningTransferError (winner); }
    ...
    function _transfer(
        ...
        if(
            ...
            uint256 hypotheticalFullBNBBalance = (BNBBalance * (totalFees)) / (totalFees - (liquidityFee / (2))); //BNB if we swapped all tokens
            ...
            uint256 BNBForLottery = hypotheticalFullBNBBalance * (lotteryFee) / (totalFees); 
			...
			lotteryBalance += (BNBForLottery); //Stays inside contract, no trasfer
