...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "MXR: The router already has that address");
  ...
  constructor() {
    ...
    whiteListRewardTokens[address(0xA36D5e816A6e72c99A5B53dcAF964aF500436D3e)] = true; // $SOL - Helios Charts
...
contract MXRFinance is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("MaxxRewards", "MXR", 18) {
        ...
        totalFees = rewardsFee + operationsFee + liquidityFee + buybackFee + investmentFee + developmentFee;

    	dividendTracker = new DividendTracker();

    	liquidityWallet = address(0xdead);
    	operationsWallet = address(0x693B7cf6b5Be895B0284B75320b15c4dDA6dA6FF);
    	investmentWallet = address(0x419f86E8c2401eE8CcA1fa3368e0fAbE367A9503);
    	developmentWallet = address(0xEb673fD4B11A768ca81De61C2d22ddaAEd992Cca);
    ...
    receive() external payable {
  	...
  	function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
  	  require(isContract(ammAddress), "MaxxRewards: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	...
  	function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "MXR: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MXR: The new dividend tracker must be owned by the MXR token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "MXR: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "MaxxRewards: airdropToWallets:: Arrays must be the same length");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "MXR: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "MXR: The operations wallet is already this address");
    ...
    function updateFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _investmentFee, uint256 _developmentFee) external onlyOwner {
        operationsFee = _operationsFee;
        rewardsFee = _rewardsFee;
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        investmentFee = _investmentFee;
        developmentFee = _developmentFee;
        totalFees = operationsFee + rewardsFee + liquidityFee + buybackFee + investmentFee + developmentFee;
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MXR: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MXR: Cannot update gasForProcessing to same value");
    ...
    // @dev User Callable Functions start here! ---------------------------------------------
  	...
  	function setRewardToken(address rewardTokenAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "MaxxRewards: setRewardToken:: Address is a wallet, not a contract.");
  	    require(rewardTokenAddress != address(this), "MaxxRewards: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "MaxxRewards: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	...
  	function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "MaxxRewards: setRewardToken:: Address is a wallet, not a contract.");
  	    require(ammContractAddress != address(uniswapV2Router), "MaxxRewards: setRewardToken:: Use setRewardToken to use default Router");
  	    require(rewardTokenAddress != address(this), "MaxxRewards: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "MaxxRewards: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	    require(isAMMWhitelisted(ammContractAddress) == true, "MaxxRewards: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "MaxxRewards: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MXR: Automated market maker pair is already set to that value");
    ...
    function swapBack(uint256 contractTokenBalance) internal {
        ...
        uint256 amountBNBInvestment = amountBNB.mul(investmentFee).div(totalBNBFee);
        uint256 amountBNBDevelopment = amountBNB.mul(developmentFee).div(totalBNBFee);
        uint256 amountBNBDev = amountBNBOperations/10;
        amountBNBOperations -= amountBNBDev;
        ...
        (success,) = address(investmentWallet).call{value: amountBNBInvestment}("");
...
contract DividendTracker is DividendPayingToken {
    ...
    function withdrawDividend() pure external override {
        require(false, "MXR_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MXR contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "MXR_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MXR_Dividend_Tracker: Cannot update claimWait to same value");
