...
contract SwapV2 is Ownable, ReentrancyGuard {
  ...
  function swapQuery(uint256 amount, address src, address dest) external payable nonReentrant onlyStrat {
    ...
    if (etherERC20 == src) {
      bestrate.trade.value(msg.value)(routeIndex, src, amount, dest, 0, partnerIndex);
    } else {
      bestrate.trade(routeIndex, src, src.myBalance(), dest, 0, partnerIndex);
    }
    require(amountOut > 0, "cannot trade.");

    if (etherERC20 == dest) {
      uint256 afterETH = address(this).balance.sub(beforeETH);
      SafeToken.safeTransferETH(msg.sender, afterETH);
    } else {
  ...
  function swap(uint256 amount, address src, address dest, uint256 routeIndex) external payable nonReentrant onlyStrat {
    ...
    if (etherERC20 == src) {
      amountOut = bestrate.trade.value(msg.value)(routeIndex, src, amount, dest, 0, partnerIndex);
    } else {
      amountOut = bestrate.trade(routeIndex, src, amount, dest, 0, partnerIndex);
    }
    require(amountOut > 0, "cannot trade.");
  ...
  function swapDev(uint256 route) external payable nonReentrant onlyStrat {
    address BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    SafeToken.safeTransferFrom(BUSD, msg.sender, address(this), 1000000000000000000);
    SafeToken.safeApprove(BUSD, address(bestrate), uint(-1));
    bestrate.trade(route, BUSD, 1000000000000000000, etherERC20, 0, 0);
    SafeToken.safeApprove(BUSD, address(bestrate), 0);
