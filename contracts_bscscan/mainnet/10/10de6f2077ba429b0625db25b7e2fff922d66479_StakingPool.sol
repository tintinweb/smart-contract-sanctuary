...
contract P3D is ERC20, Ownable {
    ...
    constructor() ERC20("PoWH3D", "P3D") {
        _mint(msg.sender, 1000000000 * 10 ** decimals());
    }

    function setBurnFee(uint256 fee)
    public
    onlyOwner()
    {
        require(fee <= 20, "Way too high burn fee");
        burnFee = fee;
    }
    ...
    function excludeToFee(address account, bool excluded)
    public
    onlyOwner() {
        _isExcludedToFee[account] = excluded;
    }
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if (_isExcludedFromFee[msg.sender] == false && _isExcludedToFee[recipient] == false) {
        ...
        bool result = super.transfer(recipient, transferAmount);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        if (_isExcludedFromFee[sender] == false && _isExcludedToFee[recipient] == false) {
        ...
        bool result = super.transferFrom(sender, recipient, transferAmount);
...
contract StakingPool is Ownable {
    ...
    function stakeTokens(uint amount) public {
        ...
        p3dToken.transferFrom(msg.sender, address(this), amount);

        uint256 currentDivs = getMyDividends();
        ...
        payoutsTo_[msg.sender] += (getMyDividends() - currentDivs);
    }

    function canExit() public view returns(bool) {
        uint256 startTime = stakeStartTime[msg.sender];
    ...
    function exitFromStakingPool() public {
        require(canExit(), "Staking time is not over.");

        withdrawDividends();

        p3dToken.transfer(msg.sender, tokenBalanceLedger_[msg.sender]);
    ...
    function getMyDividends() public view returns(uint256) {
        uint256 allDivs = (tokenBalanceLedger_[msg.sender] * profitPerShare_) / magnitude;

        uint256 profit = allDivs - payoutsTo_[msg.sender];
