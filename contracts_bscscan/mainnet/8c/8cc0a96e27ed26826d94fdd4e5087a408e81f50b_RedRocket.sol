...
contract RedRocket is Context, IERC20, Ownable {
    ...
    address  public operationsAddress = 0x26121630b15691cA37c2789bc8c4B8B3a8F1e1a6;
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tBurn,uint256 toperations) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tBurn, toperations, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tBurn, toperations);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tBurn).sub(toperations);
        return (tTransferAmount, tFee, tBurn,toperations);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tBurn, uint256 toperations, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 roperations = toperations.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rBurn).sub(roperations);
    ...
    function _takeoperations(uint256 toperations) private {
        uint256 currentRate =  _getRate();
        uint256 roperations = toperations.mul(currentRate);
        _rOwned[operationsAddress] = _rOwned[operationsAddress].add(roperations);
        if(_isExcludedFromReward[operationsAddress])
            _tOwned[operationsAddress] = _tOwned[operationsAddress].add(toperations);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tBurn, uint256 toperations) = _getValues(tAmount);
        uint256 currentRate =  _getRate();
        uint256 rBurn =  tBurn.mul(currentRate);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        ...
        if (toperations > 0) emit Transfer(sender, operationsAddress, toperations);
    }
