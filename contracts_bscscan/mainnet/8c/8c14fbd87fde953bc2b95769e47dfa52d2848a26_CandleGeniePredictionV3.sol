...
contract CandleGeniePredictionV3 is Ownable, Pausable, ReentrancyGuard 
{
    ...
    event NewPriceFeedSource(string priceFeedSource);
    ...
    function _safeStartRound(uint256 epoch) internal 
    {
        ...
        require(block.timestamp >= Rounds[epoch - 2].closeTimestamp, "Can only start new round after round n-2 endBlock");
    ...
    function owner_ChangePriceFeedSource(string memory _priceFeedSource) external onlyOwner 
    {
        require(bytes(_priceFeedSource).length > 0, "Price feed source can not be empty");
        
        priceFeedSource = _priceFeedSource;
        emit NewPriceFeedSource(_priceFeedSource);
    }
