...
contract FUN is Context, IERC20, Ownable {
    ...
    function convertMarketingFee() public lockTheSwap {
        uint256 bal = balanceOf(marketingWallet);
        if(numTokensSellToAddToLiquidity< bal){
               _allowances[marketingWallet][address(uniswapV2Router)] = bal;
    ...
    function setMarketingFeePercent(uint256 marketingFee) external onlyOwner() {
     ...
     function setAutoBurnFeePercent(uint256 _brn) external onlyOwner() {
        _burnFee = _brn;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        uint256[5] memory tdata = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount,  tdata[1],  tdata[2],  tdata[3], tdata[4], _getRate());
        return (rAmount, rTransferAmount, rFee, tdata[0], tdata[1], tdata[2],tdata[3],tdata[4]);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256[5] memory) {
        ...
        uint256 tBurn = calculatBurnFee(tAmount);
        uint256 totalFee = tFee.add(tLiquidity).add(tMarketing).add(tBurn);
