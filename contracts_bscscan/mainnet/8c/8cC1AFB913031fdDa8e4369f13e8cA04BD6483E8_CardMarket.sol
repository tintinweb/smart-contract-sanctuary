...
contract CardMarket is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    address public immutable poker;

    address private _receiveToken;
    ...
    function receiveToken() public view returns (address) {
    ...
    function buy(uint256 tokenId) external nonReentrant {
        ...
        IERC20(_receiveToken).transferFrom(account, address(this), payment);

        address seller = card.user;
        if (_feeRate > 0 && _fundReceiver != address(0)) {
            uint256 fee = payment * _feeRate / 100;
            payment -= fee;

            IERC20(_receiveToken).safeTransfer(_fundReceiver, fee);
        }
        IERC20(_receiveToken).safeTransfer(seller, payment);
