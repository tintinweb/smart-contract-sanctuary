...
contract Jasmine is BEP20{
    ...
    // Transfer Burn Rate: 1.50%.
    uint16 public constant burnRate = 150;
    
    // Transfer FEE Rate: 1.50%.
    uint16 public constant feeRate = 150;
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "JASMINE::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    // Constructs the Jasmine contract.
    
    constructor() public BEP20('Jasmine', 'JASMINE') {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        if (recipient == burnAddr || sender == devAddr || recipient == feeAddr ) {
            ...
            // 1.5% of every transfer ist sent to feeAddr
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= MAXIMUM_TRANSFER_RATE, "JASMINE::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxTransferAmountRate >= MINIMUM_TRANSFER_RATE, "JASMINE::updateMaxTransferAmountRate: Max transfer amount rate can not be bleow minimum rate.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "JASMINE::transferOperator: new operator is the zero address");
