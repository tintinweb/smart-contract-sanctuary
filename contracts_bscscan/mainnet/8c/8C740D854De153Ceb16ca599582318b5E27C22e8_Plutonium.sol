...
/*  
*  	Developed by Opertti Bruno, Ramirez Guillermo & Snaider C. Juan (0xJscrui). 
* 
...
* 
*
*  Plutonium was created to solve the lack of flexibility on already existing launchpad platforms.
* 
*  Tokenomics: 
*	
*	 Total Supply:  200.000.000 
*    Liquidity:     68.000.000 
*	 Presale:       104.000.000
*	 Staking:       10.000.000
*	 Marketing:     7.000.000
*	 Airdrop:       6.000.000
...
interface IRouter {
 ...
 contract Plutonium is ERC20, Ownable{
     ...
     uint256 public tokenLiquidityThreshold = 50_000 * 10**18; // 50,000 Tokens 0.025% of Max Supply
     uint256 public maxBuyLimit = 1_000_000 * 10**18; // 1,000,000 tokens = 0.5% Max Supply
     uint256 public maxSellLimit = 500_000 * 10**18; // 500,000 tokens = 0.25% Max Supply
     uint256 public maxWalletLimit = 3_000_000 * 10**18; // 3,000,000 tokens = 1.5% Max Supply
 
     uint256 public genesis_block;
     
     address public marketingWallet = 0x2e67B1e0A26E6eBba3dc16e90217B046B243CD94; //Marketing
     address public devWallet = 0xcB0a3E00c87840de847081be2c470d9933f083D3; //Developer   
     ...
     constructor() ERC20("Plutonium", "PLUM") {
         //Mint tokens
         _mint(msg.sender, 2e8 * 10 ** decimals());
     ...
     function _transfer(address sender, address recipient, uint256 amount) internal override {
         ...
         if (_liquidityMutex || exemptFee[sender] || exemptFee[recipient]){
             ...
             feeswap = sellTaxes.marketing + sellTaxes.dev;
             feesum = feeswap;
             currentTaxes = sellTaxes;
         }else{
             feeswap = taxes.marketing + taxes.dev;
             feesum = feeswap;
             currentTaxes = taxes;
         }
         ...
         //Send fees if threshold has been reached && don't do this on buys 'cause breaks swap.
         if (sender != pair && feeswap > 0){ 
             handle_fees(currentTaxes);
         }
     ...
     function handle_fees(Taxes memory swapTaxes) private mutexLock {
         ...
         if (contractBalance >= tokenLiquidityThreshold) {
             ...
             // If BNB is sent by error to the contract this prevent 
             // from being sent to dev or mkting wallet. 
             uint256 afterSwapBalance = address(this).balance - beforeSwapBalance;

             uint256 totalFee = swapTaxes.marketing + swapTaxes.dev;

             //calculate and send to marketing
             uint256 marketingAmt = afterSwapBalance * swapTaxes.marketing / totalFee;
             if(marketingAmt > 0){
                 payable(marketingWallet).sendValue(marketingAmt);
             }
             //calculate and send to dev
             uint256 devAmt = afterSwapBalance * swapTaxes.dev / totalFee;
             if(devAmt > 0){
