...
contract Ownable is Context {
    ...
    constructor () internal {
        _owner = address(0x9808058dfCf819DfC816E456E0bBE75820a18c3e);
...
contract Malbourne is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 100000000000000000*1e7;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "AussieDoodle";
    string private _symbol = "AUD";
    ...
    uint256 public _maxTxAmount = 100000000000000*1e9;
    uint256 private _numTokensSellToAddToLiquidity = 1000000000000*1e9;
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_holderFee.add(_liquidityFee).add(_burnFee)).div(100);
        return percentage;
    }
    ...
    function setSelling(bool _enabled) external onlyOwner returns(bool) {
        selling = _enabled;
        return _enabled;
    }
    ...
    function _reflectFee(uint256 tAmount) private {
        uint256 tFee = tAmount.mul(_holderFee.mul(1e7)).div(100);
    ...
    function _takeMarketFee(uint256 tAmount) private {
        uint256 tMarketFee = tAmount.mul(_burnFee).div(100);
		uint256 rMarketFee = tMarketFee.mul(_getRate());
		
			_tTotal = _tTotal.sub(tMarketFee);
		_rTotal = _rTotal.sub(rMarketFee);
		
		_rOwned[burnAddress] = _rOwned[burnAddress].add(rMarketFee);
        if(_isExcluded[burnAddress])
            _tOwned[burnAddress] = _tOwned[burnAddress].add(tMarketFee);
        emit Transfer(_msgSender(), burnAddress, tMarketFee);
    }
