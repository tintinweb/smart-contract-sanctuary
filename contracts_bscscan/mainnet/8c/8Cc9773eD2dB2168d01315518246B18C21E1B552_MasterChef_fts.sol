...
contract MasterChef_fts is Ownable, Operator {
    using SafeMath for uint256;
    ...
    function pendingEgg() external view returns (uint256) {
        ...
        //     uint256 ftsReward = multiplier.mul(cakeMaster.cakePerBlock()).mul(pool.allocPoint).div(totalAllocPoint);
        //     accEggPerShare = accEggPerShare.add(ftsReward.mul(1e12).div(lpSupply));
        // }
        // return user.amount.mul(accEggPerShare).div(1e12).sub(user.rewardDebt);
        return cakeMaster.pendingCake(0, address(msg.sender));
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accEggPerShare = pool.accEggPerShare.add(ftsReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                if (pending > IBEP20(fts).balanceOf(address(this))) {
