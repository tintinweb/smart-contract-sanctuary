...
contract LaunchpadNFT is ReentrancyGuard, Ownable, Pausable {
    ...
    mapping(address => bool) whitelistDealToken; //deal token white list
    mapping(address => mapping(uint => uint)) boughtCount; //Bought NFT`s by user: address => launches => tickets count
    mapping(address => uint) totalTickets;// Total bought NFT`s count by user
    ...
    */
    function addNewLaunch(uint32 _totalCount, uint _priceInUSD, uint8 _level, uint _robiBoost, uint32 _maxToUser) public onlyOwner {
        ...
        _dealTokenInWhitelist(_dealToken)
    {
        ...
        uint price = _dealToken == usdt ?
                    _launch.priceInUSD :
                    oracle.consult(_dealToken, _launch.priceInUSD, usdt);
