...
SG_address = IBEP20(0x19FB2137cD9221E5Ff26D4F7837c050B06466a89);

BUSD_address = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
...
function Change_member(address _Square) public onlyOwner(){
...
bool public GameStart_GT = true; 

uint256 public RoundGoldenTicket = 1; 

uint256[] public numberGT = [0,1];

uint256[] public _feeGTfee = [0,100 * (10 ** 18)]; 

uint256[] public _keepFee = [0,((100 * (10 ** 18)) * 95) / 100];

uint256[] bonusFee = [0,0,0]; 

uint256[] bonusPlayer = [0,0,0];

uint256 public feeGoldenTicket = 10 * (10 ** 18);

uint256 public feeGoldenTicket_liq = (feeGoldenTicket * 50) / 100;

uint256 public Timer = block.timestamp + 1000 days;

address public last_Ticket = msg.sender;

uint256 public Prize_GT;

uint256 public Prize_SG;

uint256 public _SGfee = 100 * (10 ** 18);
...
_keepFee[RoundGoldenTicket] = (feeGTfee * 95) / 100;
...
function Squid_GameGoldenTicket() public { 
...
bonusFee[RoundGoldenTicket] += (bonusPlayer[RoundGoldenTicket] * ((_SGfee * 95) / 100));
...
SG_address.transfer(msg.sender,((numberGT[round] - 1) - joinGT[msg.sender][round].number) * _keepFee[round]);

joinGT[msg.sender][round].access = true;

joinGT[msg.sender][round].save_amount = ((numberGT[round] - 1) - joinGT[msg.sender][round].number) * _keepFee[round];
...
SG_address.transfer(msg.sender,Prize_SG);

BUSD_address.transfer(msg.sender,Prize_GT);
...
SG_address.transfer(last_Ticket,Prize_SG);

}

BUSD_address.transfer(last_Ticket,Prize_GT);
...
constructor(uint256 initialSupply) public BEP20("Squid Game 24Hour", "SG"){
