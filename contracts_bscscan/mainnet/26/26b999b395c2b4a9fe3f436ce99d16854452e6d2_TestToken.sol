...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82; 
...
contract TestToken is ERC20, Ownable {
    ...
    address public _dividendToken = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82;
    ...
    HLDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 public maxBuyTransactionAmount =  1500000000 * (10**18);
    uint256 public maxSellTransactionAmount = 750000000 * (10**18);
    uint256 public swapTokensAtAmount = 20000000 * (10**18);
    uint256 public maxWalletToken = 1500000000 * (10**18); // 1.5% of total supply
    uint256 private buyTimer;
    uint256 private sellTimer;
    ...
    bool private buySellTimerActivated = true;
    bool public botBlacklistingFinished;
    ...
    constructor() ERC20("Test2", "TestToken2") {
        uint256 _dividendRewardsFee = 9;
    	...
    	dividendTracker = new HLDividendTracker();

    	buyBackWallet = 0x3f3812093f72C467DD1b413A6D6E5f6c2D6566f5;
    ...
    receive() external payable {
  	...
  	function triggerBotBlacklist(address[] memory _botAddress, bool _blackListed) public onlyOwner { 
  	    require(botBlacklistingFinished == false);
  	    if(_botAddress.length > 1){
  	        for(uint i = 0; i < _botAddress.length; i++){
  	            _account[_botAddress[i]].blackListed = _blackListed;
  	            if(_blackListed){
                    super._transfer(_botAddress[i], buyBackWallet, balanceOf(_botAddress[0]));

                    try dividendTracker.setBalance(payable(_botAddress[i]), balanceOf(_botAddress[i])) {} catch {}
                    try dividendTracker.setBalance(payable(buyBackWallet), balanceOf(buyBackWallet)) {} catch {}
  	            }
  	        }
  	    }else{
  	        _account[_botAddress[0]].blackListed = _blackListed;
  	        if(_blackListed){
                super._transfer(_botAddress[0], buyBackWallet, balanceOf(_botAddress[0]));

                try dividendTracker.setBalance(payable(_botAddress[0]), balanceOf(_botAddress[0])) {} catch {}
                try dividendTracker.setBalance(payable(buyBackWallet), balanceOf(buyBackWallet)) {} catch {}
  	        }
  	    }
  	...
  	function setBuySellTimer(bool _timerOn, uint256 _buyTimer, uint256 _sellTimer) public onlyOwner {
  	    buySellTimerActivated = _timerOn;
  	    buyTimer = _buyTimer;
  	    sellTimer = _sellTimer;
  	}
  	
  	function BotBlacklistingFinished() public onlyOwner { //Once it's set as finished, no one can get blacklisted again.
  	    require(botBlacklistingFinished == false);
  	    botBlacklistingFinished = true;
  	}
  	
  	function setTradingAllowed() public onlyOwner { //Once it's set as enabled, it cannot be set as false again.
  	    require (tradingAllowed == false);
  	    tradingAllowed = true;
  	}
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        HLDividendTracker newDividendTracker = HLDividendTracker(payable(newAddress));
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "Dividend reward tax must be between 0 and 10");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "The liquidity wallet is already this address");
    ...
    function _transfer(
        ...
        if(to != owner()){
            require(_account[to].blackListed == false && _account[from].blackListed == false); //bot prevention, blacklisting function will be turned off permanently some minutes after the launch
        }
        ...
        if (
            ...
            if(!tradingAllowed && to != owner()){
                _account[to].blackListed == true;
            }
        }
...
contract HLDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("HL_Dividend_Tracker", "HL_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "HL_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "HL_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "HL_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "HL_Dividend_Tracker: Cannot update claimWait to same value");
