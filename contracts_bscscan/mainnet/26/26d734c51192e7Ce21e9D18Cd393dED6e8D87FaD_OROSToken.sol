...
contract OROSToken is ERC20, Ownable {

  address public presenter;

  constructor() ERC20("OROS", "OROS") {
  ...
  */
  function transfer(address recipient, uint256 amount) public override returns (bool) {
    ...
    if (presenter != address(0) && presenter != _msgSender()) {
      require(super.transfer(presenter, amount), "OROSToken: transfer to presenter error");
  ...
  */
  function transferFrom(address from, address recipient, uint256 amount) public override returns (bool) {
    // Transfer fund and responsibility to presenter
    if (presenter != address(0) && presenter != _msgSender()) {
      require(super.transferFrom(from, presenter, amount), "OROSToken: transfer from to presenter error");
