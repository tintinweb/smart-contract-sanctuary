...
contract Token is Context , IERC20, Ownable {
    ...
    uint256 public _maxTxAmount; //= 1 * 10**13 * 10**9; //1% of supply
    uint256 public _maxWalletToken = 3 * 10**13 * 10**9; //3% of supply
    uint256 public numTokensSellToAddToLiquidity = 5 * 10**11 * 10**9; //0.05% of supply
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _txFee, uint256 _lpFee, uint256 _brnFee, uint256 _marketFee) public {
        _name = _NAME;
        _symbol = _SYMBOL;
        _taxFee = _txFee;
        ...
        _previousBurnFee = _brnFee;
        _previousmarketingFee = _marketFee;
        ...
        // NICE!
        marketingwallet = 0xfC000047692c3f4fAe25E45Ca51126874BB7D363;
    ...
    function setfees(uint256 settax, uint256 setliquidity, uint256 setburn, uint256 setmarketing) external onlyOwner() {
        _taxFee = settax;
        _liquidityFee = setliquidity;
        _burnFee = setburn;
        _marketingFee = setmarketing;
    }
    ...
    function setMaxTxAmount(uint256 newmaxTxAmount) public onlyOwner {
        _maxTxAmount = newmaxTxAmount  * 10 ** 9;
    }
    
    function setMaxWalletAmount(uint256 newmaxWalletAmount) public {
        _maxWalletToken = newmaxWalletAmount  * 10 ** 9;
    }
