...
contract Puppydoge is ERC20Detailed, Ownable {
  ...
  uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 51*10 ** 7 * 10 ** DECIMALS;
  ...
  // TOTAL_Puppy is a multiple of INITIAL_FRAGMENTS_SUPPLY so that _PuppyPerFragment is an integer.
  // Use the highest value that fits in a uint256 for max granularity.
  uint256 private constant TOTAL_Puppy = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

  // MAX_SUPPLY = maximum integer < (sqrt(4*TOTAL_Puppy + 1) - 1) / 2
  uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

  uint256 private _totalSupply;
  uint256 private _PuppyPerFragment;
  mapping(address => uint256) private _PuppyBalances;
  ...
  ERC20Detailed("PUPPY DOGE", "PDG", uint8(DECIMALS))
  payable
  public
  {
    ...
    _PuppyBalances[msg.sender] = TOTAL_Puppy;
    _PuppyPerFragment = TOTAL_Puppy.div(_totalSupply);
...
*/
  ...
  returns (uint256)
  {
    return _PuppyBalances[who].div(_PuppyPerFragment);
  }
  ...
  function _transferStandard(address sender, address recipient, uint256 amount) private {
    ...
    uint256 PuppyDeduct = amount.mul(_PuppyPerFragment);
    uint256 PuppyValue = tTransferAmount.mul(_PuppyPerFragment);
    _PuppyBalances[sender] = _PuppyBalances[sender].sub(PuppyDeduct);
    _PuppyBalances[recipient] = _PuppyBalances[recipient].add(PuppyValue);
    _takeFee(sender, tFee);
    emit Transfer(sender, recipient, amount);
  }

  function _transferExcluded(address sender, address recipient, uint256 amount) private {
    uint256 PuppyValue = amount.mul(_PuppyPerFragment);
    _PuppyBalances[sender] = _PuppyBalances[sender].sub(PuppyValue);
    _PuppyBalances[recipient] = _PuppyBalances[recipient].add(PuppyValue);
    emit Transfer(sender, recipient, amount);
  }
  ...
  function _takeFee(address sender, uint256 tFee) private {
    uint256 rFee = tFee.mul(_PuppyPerFragment);
    _PuppyBalances[address(this)] = _PuppyBalances[address(this)].add(rFee);
