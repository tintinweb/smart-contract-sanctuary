...
contract SafePumpEther is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SafePumpEtherDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("SafePumpEther", "SPE") {
        _ETHRewardsFee = 6;
        _liquidityFee = 3;
		_marketingFee = 7;
        ...
        maxBuyTranscationAmount = 3500000000 * (10**18);
        maxSellTransactionAmount = 100000000000 * (10**18);
        swapTokensAtAmount = 200000000 * (10**18);
        _maxWalletToken = 3500000000 * (10**18); // 


    	dividendTracker = new SafePumpEtherDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
		bulkSender = payable(0x979E44C6253C3B861705621DD0ac37f555025f82);
		marketingWallet = payable(0xf1864fE184BFD753FAcd96f180d35453D05F1D5a);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SafePumpEther: The dividend tracker already has that address");

        SafePumpEtherDividendTracker newDividendTracker = SafePumpEtherDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SafePumpEther: The new dividend tracker must be owned by the SafePumpEther token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SafePumpEther: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SafePumpEther: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SafePumpEther: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SafePumpEther: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SafePumpEther: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SafePumpEther: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SafePumpEther: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForBUSD(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of SafePumpEther
            path,
            recipient,
            block.timestamp
        );
...
contract SafePumpEtherDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SafePumpEther_Dividend_Tracker", "SafePumpEther_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "SafePumpEther_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SafePumpEtherd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SafePumpEther contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "SafePumpEther_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SafePumpEther_Dividend_Tracker: Cannot update claimWait to same value");
