...
contract Emotchi is  Context, IERC20, Ownable {
    ...
    uint256 private _tEmotchiTotal;

    string private _name = "😎 Emotchi 😎 ";
    string private _symbol = "Emotchi";
    ...
    uint256 public _EmotchiFee = 9;
    uint256 private _previousEmotchiFee = _EmotchiFee;
    
    uint256 public _maxTxAmount = 1000 * 10**9 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 1 * 10**9 * 10**9; 
    
    address payable public EmotchiAddress = 0x6E75908c95cdca79425c15483241dE79D4CD43Dd;
   ...
   function deliver(uint256 tAmount) public {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _tEmotchiTotal = _tEmotchiTotal.add(address(this).balance);
        TransferCharityETH(EmotchiAddress, address(this).balance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_EmotchiFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _EmotchiFee == 0 && _liquidityFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousEmotchiFee = _EmotchiFee;
    ...
    function restoreAllFee() private {
        ...
        _EmotchiFee = _previousEmotchiFee;
    ...
    function setEmotchiFeePercent(uint256 EmotchiFee) external onlyOwner() {
        _EmotchiFee = EmotchiFee;
    }
