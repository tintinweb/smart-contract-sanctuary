...
interface IPenginChef {
    ...
    function amountPerformanceFee(uint256 amount) external view returns (uint256);

    function amountWithdrawalFee(uint256 amount, uint256 depositedAt) external view returns (uint256);

    function amountToMintByFee(address asset, uint256 amount) external view returns (uint256);
...
// Root file: contracts/pools/AlpacaRabbitPoolBNB.sol
...
contract AlpacaRabbitPoolBNB is PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // alpaca
    uint256 constant ALPACA_STAKE_PID = 1;
    ...
    IAlpacaBearToken constant ALPACA_IB = IAlpacaBearToken(0xd7D069493685A581d27824Fc46EdA46B7EfC0063);

    // rabbit
    uint256 constant RABBIT_STAKE_PID = 0;
    ...
    IERC20 constant RABBIT_IB = IERC20(0x45b887D3569cACa67E10662075241F972D337850);
    ...
    IERC20 public constant stakingToken = IERC20(address(0));

    STRATEGY public currentStrategy;
    IPenginChef public penginChef;
    ...
    event Withdrawn(address indexed account, uint256 indexed withdrawAmount, uint256 indexed feeAmount);
    event RewardPaid(address indexed account, uint256 indexed profitAmount, uint256 indexed feeAmount);
    event Harvested(uint256 indexed amount);
    ...
    function initialize(address _penginChef) external initializer {
        __ReentrancyGuard_init();
        __PausableUpgradeable_init();

        currentStrategy = STRATEGY.RABBIT;
        penginChef = IPenginChef(_penginChef);
    }
    ...
    function balance() public view returns (uint256 amount) {
        if (currentStrategy == STRATEGY.ALPACA) {
            ...
            amount = ibAmount.mul(RABBIT_BANK.totalToken(address(0))).div(RABBIT_IB.totalSupply());
        }
    }
    ...
    function rewardOf(address account) public view returns (uint256 staking, uint256 pengin) {
        uint256 amount = earned(account);
        if (penginChef.canMint(address(this))) {
            uint256 performance = penginChef.amountPerformanceFee(amount);

            staking = amount.sub(performance);
            pengin = penginChef.amountToMintByFee(address(stakingToken), performance);
        } else {
            staking = amount;
            pengin = 0;
        }
    ...
    function deposit(uint256 amount) external payable nonReentrant whenNotPaused {
        ...
        require(msg.value == amount, "not amount");
    ...
    function withdraw(uint256 amount) external nonReentrant safeTime(msg.sender) {
        ...
        uint256 withdrawalFee = penginChef.canMint(address(this))
            ? penginChef.amountWithdrawalFee(withdrawAmount, _depositedAt[msg.sender])
            : 0;
        if (withdrawalFee > DUST) {
            // wrap WBNB
            IWETH(PANCAKE_ROUTER.WETH()).deposit{value: withdrawalFee}();

            IERC20(PANCAKE_ROUTER.WETH()).approve(address(penginChef), 0);
            IERC20(PANCAKE_ROUTER.WETH()).approve(address(penginChef), withdrawalFee);
            penginChef.mintByFee(PANCAKE_ROUTER.WETH(), 0, withdrawalFee, msg.sender);
    ...
    function getReward() external nonReentrant safeTime(msg.sender) {
        ...
        uint256 performanceFee = penginChef.canMint(address(this))
            ? penginChef.amountPerformanceFee(withdrawAmount)
            : 0;
        if (performanceFee > DUST) {
            // wrap WBNB
            IWETH(PANCAKE_ROUTER.WETH()).deposit{value: performanceFee}();

            IERC20(PANCAKE_ROUTER.WETH()).approve(address(penginChef), 0);
            IERC20(PANCAKE_ROUTER.WETH()).approve(address(penginChef), performanceFee);
            penginChef.mintByFee(PANCAKE_ROUTER.WETH(), performanceFee, 0, msg.sender);
        ...
        emit RewardPaid(msg.sender, withdrawAmount, performanceFee);
    ...
    function _depositStakingToken(uint256 amount) private returns (uint256 rewardAmount) {
        ...
        if (amount > 0) {
            ...
            if (currentStrategy == STRATEGY.ALPACA) {
                ALPACA_IB.deposit{value: amount}(amount);
            } else {
                RABBIT_BANK.deposit{value: amount}(address(0), amount);
            }
    ...
    function _withdrawStakingToken(uint256 amount) private returns (uint256 withdrawAmount, uint256 rewardAmount) {
        ...
        if (currentStrategy == STRATEGY.ALPACA) {
            ...
            ibAmount = amount.mul(RABBIT_IB.totalSupply()).div(RABBIT_BANK.totalToken(address(0)));
        }
        ...
        if (currentStrategy == STRATEGY.ALPACA) {
            ...
            RABBIT_BANK.withdraw(address(0), ibAmount);
        }
    ...
    function _harvest(uint256 rewardAmount) private {
        ...
        if (rewardAmount > DUST) {
            ...
            // convert reward (ALPACA or RABBIT) to BNB
            uint256 bnbBefore = address(this).balance;
            address[] memory path = new address[](2);
            (path[0], path[1]) = (address(rewardToken), PANCAKE_ROUTER.WETH());

            rewardToken.safeApprove(address(PANCAKE_ROUTER), 0);
            rewardToken.safeApprove(address(PANCAKE_ROUTER), rewardAmount);

            PANCAKE_ROUTER.swapExactTokensForETH(rewardAmount, 0, path, address(this), block.timestamp);
