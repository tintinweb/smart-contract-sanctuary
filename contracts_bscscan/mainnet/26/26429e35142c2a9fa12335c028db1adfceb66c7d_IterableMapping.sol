...
contract Nudes is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    NudesDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 public maxBuyTranscationAmount = 2900000000 * (10**18);
    ...
    constructor() ERC20("NUDES", "Nudes") {
        uint256 _dividendRewardsFee = 12;
        uint256 _LPFee = 1;

        dividendRewardsFee = _dividendRewardsFee;
        LPFee = _LPFee;
        totalFees = _dividendRewardsFee.add(_LPFee);

    	dividendTracker = new NudesDividendTracker();

    	buyBackWallet = 0x3A85446163F24a93BC276d7B6732779726Bd259F;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "Nudes: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setLPDivisor(uint256 divisor) external onlyOwner {
  	    require(LPDivisor >= 0 && LPDivisor <= 100, "Nudes: LP divisor must be between 0 (0%) and 100 (100%)");
    ...
    function afterPreSale() external onlyOwner {
        dividendRewardsFee = 12;
        LPFee = 1;
        maxBuyTranscationAmount = 2900000000 * (10**18);
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "Nudes: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Nudes: The dividend tracker already has that address");

        NudesDividendTracker newDividendTracker = NudesDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Nudes: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "Nudes: Dividend reward tax must be between 0 and 10");
        dividendRewardsFee = newFee;
    }
    
    function updateLPFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "Nudes: Dividend reward tax must be between 0 and 10");
        LPFee = newFee;
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Nudes: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Nudes: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Nudes: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Nudes: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "Nudes: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Nudes: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Nudes: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                uint256 swapTokens = contractTokenBalance.mul(LPFee).div(totalFees);
                swapTokensForBNB(swapTokens);
                transferToBuyBackWallet(payable(buyBackWallet), address(this).balance.div(10**2).mul(LPDivisor));
...
contract NudesDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Nudes_Dividend_Tracker", "Nudes_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "Nudes_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "Nudes_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Nudes contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Nudes_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Nudes_Dividend_Tracker: Cannot update claimWait to same value");
