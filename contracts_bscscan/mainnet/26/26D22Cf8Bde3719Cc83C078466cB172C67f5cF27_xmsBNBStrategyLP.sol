...
contract xmsBNBStrategyLP{
    ...
    function harvest(uint amount1, uint amount2) public {
        ...
        //50% Restake
        convertCaketoXms(totalReward.mul(25).div(100));
        convertCaketoWBNB(totalReward.mul(25).div(100));
        // Liquidity pool
        uint256 token0Amt = IERC20(xms).balanceOf(address(this));
        uint256 token1Amt = IERC20(wbnb).balanceOf(address(this));
        if (token0Amt > 0 && token1Amt > 0) {
                IERC20(xms).safeApprove(marsRouter, 0);
                IERC20(xms).safeApprove(marsRouter, uint256(-1));
                IERC20(wbnb).safeApprove(marsRouter, 0);
                IERC20(wbnb).safeApprove(marsRouter, uint256(-1));
        ...
        dRewards(pool).deposit(uint256(1), IERC20(want).balanceOf(address(this)));
    ...
    function convertCaketoXms(uint amount) public {
        require(!Address.isContract(msg.sender),"!contract");
        address[] memory path = new address[](2);
                    path[0] = out; //cake
                    path[1] = xms;
