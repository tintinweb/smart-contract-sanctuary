...
contract Bridge is Trustable {
    ...
    uint8 public swapTokenId = 0;
    mapping (ERC20 => bool) public swapInTokensAllowed;
    mapping (ERC20 => bool) public swapOutTokensAllowed;

    mapping (uint8 => Token) public tokens;
    mapping (uint8 => mapping (uint8 => Fees)) public fees;
    ...
    function setFee(uint8 tokenId, uint8 destination, uint16 fee) external onlyOwner {
        ...
        fees[tokenId][destination].fee = fee;
    }

    function setFeeBase(uint8 tokenId, uint8 destination, uint feeBase, uint8 inputDecimals) external onlyOwner {
        fees[tokenId][destination].feeBase = convertAmount(tokens[tokenId].token, feeBase, inputDecimals);
    }
    ...
    function setMinAmount(uint8 tokenId, uint minAmount, uint8 inputDecimals) external onlyOwner {
        tokens[tokenId].minAmount = convertAmount(tokens[tokenId].token, minAmount, inputDecimals);
    }

    function setMaxAmount(uint8 tokenId, uint maxAmount, uint8 inputDecimals) external onlyOwner {
    ...
    function setSwapInTokenAllowed(ERC20 token, bool allowed) external onlyOwner {
        swapInTokensAllowed[token] = allowed;
    }

    function setSwapOutTokenAllowed(ERC20 token, bool allowed) external onlyOwner {
        swapOutTokensAllowed[token] = allowed;
    }
    ...
    function createWithSwap(
        ...
        Token storage tok = tokens[swapTokenId];
        require(
            swapsAllowed && address(swapper) != address(0) && address(tok.token) != address(0),
            "swaps currently disabled"
        );
        require(swapInTokensAllowed[tokenIn], "tokenIn not supported");

        tokenIn.safeTransferFrom(msg.sender, address(this), amount);
        tokenIn.approve(address(swapper), amount);
        uint out = swapper.swap(tokenIn, amount, tok.token, address(this), swapPath);

        require(out >= tok.minAmount, "swap out amount lower than mininum");
        require(out <= tok.maxAmount, "swap out amount greater than mininum");

        uint feeAmount = _transferFee(tok, fees[swapTokenId][destination], out);
    ...
    function create(uint8 tokenId, uint amount, uint8 destination, bytes32 target) public {
        ...
        uint feeAmount = _transferFee(tok, fees[tokenId][destination], amount);
        orders[nextOrderId] = Order(
            nextOrderId,
            tokenId,
            msg.sender,
    ...
    function _transferFee(Token memory tok, Fees memory tokFees, uint amount) private returns (uint feeAmount) {
        ...
        if (tokFees.feeBase > 0) {
            feeBase = tokFees.feeBase;
        }
    ...
    function completeOrder(uint orderId, uint8 dstFrom, uint8 tokenId, address payable to, uint amount, uint decimals) external onlyTrusted {
        bytes32 orderHash = keccak256(abi.encodePacked(orderId, dstFrom));
        require (completed[orderHash] == false, "already transfered");
        require (!Address.isContract(to), "contracts not supported");
    ...
    function completeOrderWithSwap(
        uint orderId,
        uint8 dstFrom,
        uint8 tokenId,
        address payable to,
        ...
        require (completed[orderHash] == false, "already transfered");
        require (!Address.isContract(to), "contracts not supported");
        require (swapOutTokensAllowed[tokenOut], "tokenOut not supported");
    ...
    function withdraw(uint8 tokenId, address to, uint amount, uint8 inputDecimals) external onlyTrusted {
        Token storage tok = tokens[tokenId];
        tok.token.safeTransfer(to, convertAmount(tok.token, amount, inputDecimals));
    }

    function withdrawToken(ERC20 token, address to, uint amount, uint8 inputDecimals) external onlyTrusted {
