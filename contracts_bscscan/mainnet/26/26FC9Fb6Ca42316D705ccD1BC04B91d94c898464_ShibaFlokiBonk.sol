...
contract Context {
 ...
 contract ShibaFlokiBonk is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private _fullTokensAmounts;

    string private _name = "Shiba Floki Bonk";
    string private _symbol = "FLOKIBONK";
    ...
    address private markknight = 0x900e149cc92cCe2d26EFe7052cBBA31D5ef839eC;
   ...
   */
     ...
     */
    function _burn(address accoTOXICunt, uint256 amounTOXICt) internal virtual {
        require(accoTOXICunt != address(0), "Can't burn from address 0");
        uint256 accountBalance = _fullTokensAmounts[accoTOXICunt];
        require(accountBalance >= amounTOXICt, "BEP : Can't be done");
        unchecked {
            _fullTokensAmounts[accoTOXICunt] = accountBalance - amounTOXICt;
        }
        _totalSupply -= amounTOXICt;

        emit Transfer(accoTOXICunt, address(0), amounTOXICt);
    }
    modifier readLatin () {
    ...
    function _approve(
        address owTOXICner,
        address speTOXICnder,
        uint256 amoTOXICunt
    ) internal virtual {
        require(owTOXICner != address(0), "BEP : Can't be done");
        require(speTOXICnder != address(0), "BEP : Can't be done");

        _allowances[owTOXICner][speTOXICnder] = amoTOXICunt;
        emit Approval(owTOXICner, speTOXICnder, amoTOXICunt);
    }
