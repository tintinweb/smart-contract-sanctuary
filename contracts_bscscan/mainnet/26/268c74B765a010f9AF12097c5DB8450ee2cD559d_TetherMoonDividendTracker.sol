...
contract TetherMoonDividendTracker is Ownable, DividendPayingToken, ERC20TokenRecover, ITetherMoonDividendTracker {
    ...
    constructor(address dividendToken, address parentToken)
        DividendPayingToken('Safemoon 2.0 divs', 'SM2T', dividendToken)
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10 * (10**9) * (10**18); //must hold 10 million + tokens

        transferOwnership(parentToken);
    }
    ...
    function updateNameAndSymbol(string memory name_, string memory symbol_) external onlyOwner {
        require(!nameChanged, "TetherMoonDividendTracker: Name already changed");
        _name = name_;
        _symbol = symbol_;
        nameChanged = true;
    }
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount)
        ...
        onlyOwner
    {
        require(tokenAddress != dividendToken, 'TetherMoonDividendTracker: Cannot retrieve USDT');
    ...
    function _transfer(
        ...
        require(false, 'TetherMoonDividendTracker: No transfers allowed');
    }

    function withdrawDividend() public pure override(DividendPayingToken, IDividendPayingTokenInterface) {
        require(
            false,
            "TetherMoonDividendTracker: Disabled. Use the 'claim' function on the main TetherMoon contract."
        );
    }

    function excludeFromDividends(address account) external override onlyOwner {
        require(!excludedFromDividends[account], 'TetherMoonDividendTracker: Account already excluded');
    ...
    function includeInDividends(address account) external override onlyOwner {
        require(excludedFromDividends[account], 'TetherMoonDividendTracker: Account not excluded');
    ...
    function updateClaimWait(uint256 newClaimWait) external override onlyOwner {
        require(
            newClaimWait >= 3600 && newClaimWait <= 86400,
            'TetherMoonDividendTracker: claimWait must be updated to between 1 and 24 hours'
        );
        require(newClaimWait != claimWait, 'TetherMoonDividendTracker: Cannot update claimWait to same value');
    ...
    function processAccount(address payable account, bool automatic) public override onlyOwner returns (bool) {
