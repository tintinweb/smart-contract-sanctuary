...
contract CloudEco is Ownable {
    ...
    function harvest(uint256 _pid) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                ...
                buyback.saveQueueByContract(address(msg.sender),pending.mul(8).div(10));
    ...
    function depo(uint256 _pid,uint256 _amount, address _referrer, address _sender) internal {
        ...
        if(_amount > 0) {
            ...
            //check promo
            if(block.timestamp>=promo.start && block.timestamp<=promo.end && _amount>promo.minAmount && user.amount==0){
                user.isPromo = true;
