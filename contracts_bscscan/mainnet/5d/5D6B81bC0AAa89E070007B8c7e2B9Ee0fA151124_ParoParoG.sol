...
Paro Paro G is the next moonshot. A new community token will take part on BSC network. A safe and unruggable project.
Paro Paro G - is a hyperdeflationary token, which rewards investors for holding tokens, This token with x100 potential. Paro Paro G is one of the trend on facebook. 
...
Every transaction in Paro Paro G 8% will be added for Marketing.

Also, another 2% of every transaction is going liquidity

Also, another 3% of every transaction is going to holders
...
contract ParoParoG is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "ParoParoG";
