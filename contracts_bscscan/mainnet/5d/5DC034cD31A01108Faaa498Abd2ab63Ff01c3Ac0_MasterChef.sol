...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Base allocation points. SPY is 0 decmial, we need this value to alter emission rate properly
    ...
    function updateEmissionRate(uint256 _spyPerBlock) public onlyOwner {
        require(_spyPerBlock <= 1000, "set: invalid spyPerBlock");
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, spyPerBlock, _spyPerBlock);
        spyPerBlock = _spyPerBlock;
    }
    ...
    function setHarvestFee(uint256 _harvestFee) public onlyOwner {
        require(_harvestFee <= 100000, "setHarvetFee: must be less than 100000 (100%) ");
    ...
    function takeHarvestFee(uint256 _pending) internal returns (uint256) {
        ...
        uint256 feeAmount = _pending.mul(harvestFee).div(10000);
        ...
        if (feeAmount > 0) {
            miningPool.safeTransfer(address(marketingPool), feeAmount);
        }
