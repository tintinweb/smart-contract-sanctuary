...
contract JAGOKU_TOKEN is Context, IERC20, IERC20Extented, Ownable {
    ...
    uint256 private numTokenSell = 500 * 10**21 * 10**18;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
                ...
                if (autoSwap) {
                    bool cek = contractTokenBalance >= numTokenSell;
                    if (cek) {
                        contractTokenBalance = numTokenSell;
    ...
    function setMintokenSell(uint256 tok) external onlyOwner() {
        numTokenSell = tok * 10**21 * 10**18;
        emit tokenSellUpdate(tok);
    }
