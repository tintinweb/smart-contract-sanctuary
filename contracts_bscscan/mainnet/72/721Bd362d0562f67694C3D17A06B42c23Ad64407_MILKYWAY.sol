...
contract MILKYWAY is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public _dumpProtectionTimer = 60 seconds;
    ...
    function getTotalFee(bool selling, address sender) public view returns (uint256) {
        ...
        if(selling && sellMultiplierEnabled){
            return ((totalFee * _sellMultiplierNumerator) / _sellMultiplierDenominator);
        }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(getTotalFee(recipient == pair, sender)).div(feeDenominator);
