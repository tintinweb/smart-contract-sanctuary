...
contract Vault is IVault, Ownable ,ReentrancyGuard {
  ...
  address[] public earnToBusdPath = [earnAddress,BUSD];
  ...
  function _harvest() internal {
    ...
    uint256[] memory amounts =
        ROUTER.getAmountsOut(reward.div(2), earnToBusdPath);
    uint256 amountOut = amounts[amounts.length.sub(1)];
        ROUTER.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            reward.div(2),
            amountOut.mul(950).div(1000), //slippage 5%
            earnToBusdPath,
            address(this),
    ...
    if (token0Amt > 0 && token1Amt > 0) {
        IERC20(earnAddress).safeIncreaseAllowance(
              address(ROUTER),
              token0Amt
        );
