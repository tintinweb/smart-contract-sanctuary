...
import "./DecubateTiers.sol";

contract BracketAllocation is Ownable {
  using SafeMath for uint256;

  /**
   *
   * @dev Bracket struct store all the included brackets
   *
   * @param {addrList} List of addresses on a bracket
   * @param {maxCount} Maximum number of allowed users in a bracket
   * @param {currCount} Current number of users in a bracket
   * @param {allowedAmount} Maximum allowed deposit amount in a bracket
   * @param {minLimit} Minimum amount of dcb to be staked to join bracket
   * @param {maxLimit} Maximum amount of dcb to be staked to join bracket
   *
   */
  struct Bracket {
    address[] addrList;
  ...
  DecubateTiers private tiersContract; //Tiers contract instance
  mapping(uint256 => Bracket) public bracketInfo; //Bracket storage
  address private crowdFunding; //Crowdfunding contract address

  bool public isEnabled; //Enable/Disable use of bracket

  mapping(address => UserAllocation) public userAllocation;

  constructor(DecubateTiers _tiersContract, address _crowdFunding) public {
    tiersContract = DecubateTiers(_tiersContract);
    crowdFunding = _crowdFunding;
  }

  /**
   *
   * @dev add new bracket, only available for owner
   *
   */
  function addBracket(
    uint256 _tierId,
    ...
    bracketInfo[_tierId].addrList = new address[](0);
    bracketInfo[_tierId].maxCount = _maxCount;
    bracketInfo[_tierId].allowedAmount = _allowedAmount;
    bracketInfo[_tierId].currCount = 0;
  ...
  /**
   *
   * @dev Get total users in a specific bracket
   *
   * @param tier tier id
   *
   * @return alist length of the pools
   *
   */
  function getBracketUsers(uint256 tier)
    external
    view
    returns (address[] memory alist)
  {
    return bracketInfo[tier].addrList;
  }

  /**
   *
   * @dev get info of all brackets
   *
   * @return brackets {Bracket[]} bracket info struct
   *
   */
  function getBrackets() external view returns (Bracket[] memory) {
    uint256 len = tiersContract.getTiersLength();
    Bracket[] memory brackets = new Bracket[](len);
    for (uint256 i = 0; i < len; i++) {
      brackets[i] = bracketInfo[i];
    }
  ...
  /**
   * @dev set address of crowdfunding
   */
  function setCrowdfundingAddress(address _crowdFunding) external onlyOwner {
    crowdFunding = _crowdFunding;
  }

  /**
   * @dev set address of tiers contract
   */
  function setTiersContract(address _tiersContract) external onlyOwner {
    tiersContract = DecubateTiers(_tiersContract);
  }

  /**
   *
   * @dev Check amount allowed and adds user to a bracket
   * Can only be called by crowdfunding contract
   *
   * @param addr Address of the user
   *
   * @return bool Status of addition
   *
   */
  function setAddress(address addr) public returns (bool) {
    require(msg.sender == crowdFunding, "Only crowdfunding contract");
    require(!userAllocation[addr].active, "User already has an allocation");

    (bool flag, uint256 pos, uint256 multiplier) = tiersContract.getTierOfUser(
      addr
    );

    Bracket storage bracket = bracketInfo[pos];

    require(flag, "User does not belong to any bracket");
    require(bracket.currCount < bracket.maxCount, "Bracket already filled");
    bracket.addrList.push(addr);
    bracket.currCount = bracket.currCount.add(multiplier);

    userAllocation[addr].active = true;
    userAllocation[addr].allowedAmount = bracket.allowedAmount.mul(multiplier);
...
contract DecubateTiers is Ownable {
  ...
  DecubateStaking private stakingContract; //Staking contract instance
  Tier[] public tierInfo; //Tier storage

  constructor(DecubateStaking _stakingContract) public {
    stakingContract = DecubateStaking(_stakingContract);
  }
  ...
  /**
   *
   * @dev set address of staking contract
   *
   */
  function setStakingContract(DecubateStaking _stakingContract)
    external
    onlyOwner
  {
    stakingContract = DecubateStaking(_stakingContract);
  ...
  function getTotalDeposit(address addr) public view returns (uint256 amount) {
    ...
    for (uint256 i = 0; i < len; i++) {
      (tempAmt, , , , ) = stakingContract.users(i, addr);
    ...
    )
  {
    ...
    for (uint256 i = 0; i < len; i++) {
      if (
        totalDeposit >= tierInfo[i].minLimit &&
        totalDeposit <= tierInfo[i].maxLimit
      ) {
        pos = i;
        flag = true;
        break;
