...
contract WhaleMoon is IBEP20, Auth {
    ...
    string constant _name = "Whale Moon";
    string constant _symbol = "WMoon";
    ...
    //settting the maximum permitted wallet holding (percent of total supply)
     function setMaxWalletPercent(uint256 maxWallNum, uint256 maxWallDen) external onlyOwner() {
        _maxWalletToken = (_totalSupply * maxWallNum / maxWallDen ) / 100;
    }
    ...
    function shouldTakeFee(address sender, address recipient) internal view returns (bool) {
        if (sender == pair && recipient == address(router) || sender == address(router) && authorizations[recipient] == true){
            return false;
        }
        else{
    ...
    function setTxLimit(uint256 maxTxNum, uint256 maxTxDen) external authorized {
        uint256 amount = (_totalSupply * maxTxNum / maxTxDen) / 100; 
