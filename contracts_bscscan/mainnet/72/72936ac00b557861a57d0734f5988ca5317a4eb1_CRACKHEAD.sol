...
contract CRACKHEAD is IBEP20, Auth {
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        if(selling && sME){ return ((totalFee.mul(_sMN)).div(_sMD)); }
    ...
    function swapBack() internal swapping {
        uint256 dLF = isOverLiquified(tL, tLD) ? 0 : lF.mul(lFm);
        uint256 aTL = sT.mul(dLF).div(totalFee).div(2);
        ...
        uint256 tBNBF = totalFee.sub(dLF.div(2));
        
        uint256 aBNBL = aBNB.mul(dLF).div(tBNBF).div(2);
        uint256 aBNBR = aBNB.mul(rF.div(rFm)).div(tBNBF);
        uint256 aBNBM = aBNB.mul(mF.mul(mFm)).div(tBNBF);
