...
// File: contracts/XRPerienceDividendPayingToken.sol
...
contract XRPerienceContract is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   XRPerienceDividendTracker public dividendTracker;

    address public liquidityWallet;
    address public marketingWallet1 = 0x4BC9DcdDEdDb3a0B657a1d041F93D4323f4b040F;
    ...
    constructor() public ERC20("XRPerience", "XPR") {
    	...
    	dividendTracker = new XRPerienceDividendTracker();

    	liquidityWallet = 0x8B825DfcF7789b622821eCCA7605A4E21CC192d4;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "XRPerience: The dividend tracker already has that address");

        XRPerienceDividendTracker newDividendTracker = XRPerienceDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "XRPerience: The new dividend tracker must be owned by the XRPerience token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "XRPerience: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "XRPerience: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "XRPerience: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "XRPerience: Automated market maker pair is already set to that value");
    ...
    function setMarketingWallet(address _marketingWallet1) public onlyOwner {
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "XRPerience: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "XRPerience: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "XRPerience: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "XRPerience: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of XRPerience
            path,
            recipient,
            block.timestamp
        );
...
contract XRPerienceDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("XRPerienceOFFICIAL", "T.me/XRPerienceOFFICIAL") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "XRPerience_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "XRPerience_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main XRPerience contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "XRPerience_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "XRPerience_Dividend_Tracker: Cannot update claimWait to same value");
