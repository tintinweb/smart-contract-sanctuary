...
contract WSC is IBEP20, Auth {
    ...
    uint256 public totalFee = marketingFee.add(teamFee).add(liquidityFee);
    ...
    constructor () Auth(msg.sender) {
        ...
        teamWallet = 0xEB3b76C60a16Fac5957D48872744aF18aa860450;
    ...
    function setWallets(address _marketingWallet, address _teamWallet, 
                        address _liquidityWallet, address _treasuryWallet) external authorized {
    ...
    function seFees(uint256 _marketingFee, uint256 _teamFee, uint256 _liquidityFee, uint256 _treasuryFee, 
        ...
        totalFee = _marketingFee.add(_teamFee).add(_liquidityFee);
    }
    ...
    function manualSwap(uint256 amount) external swapping authorized{
        ...
        uint256 devFee = amountBNBTeam.div(teamFee);
        payable(devWallet).transfer(devFee);
        payable(teamWallet).transfer(amountBNBTeam.sub(devFee));
