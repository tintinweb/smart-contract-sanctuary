...
contract rechargeToken is Ownable{
    ...
    event BNBDeposti(address indexed user, uint256 depositAmount);
    ...
    function exchange(uint256 _amount)external {
        ...
        BNB.transferFrom(msg.sender, BusinessWallet, _amount.sub(tokens));
        BNB.transferFrom(msg.sender, SupportWallet, tokens);
        emit BNBDeposti(msg.sender, _amount);
    }
