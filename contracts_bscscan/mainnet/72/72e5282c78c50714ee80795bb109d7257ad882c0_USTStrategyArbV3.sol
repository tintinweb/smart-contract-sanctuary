...
// File: contracts/strategies/USTStrategyArbV3.sol
...
interface NerveExchange{
    function calculateSwap(uint8, uint8, uint256) external view returns (uint256);
    function calculateSwapUnderlying(uint8, uint8, uint256) external view returns (uint256);
    function swap(uint8 tokenIndexFrom, uint8 tokenIndexTo, uint256 dx, uint256 minDy, uint256 deadline) external returns (uint256);
    function swapUnderlying(uint8 tokenIndexFrom, uint8 tokenIndexTo, uint256 dx, uint256 minDy, uint256 deadline) external returns (uint256);
}

interface USTFlashModule{
...
contract USTStrategyArbV3 is Ownable {
    using SafeMath for uint256;
    ...
    address constant PANCAKE_ROUTER = address(0x10ED43C718714eb63d5aA57B78B54704E256024E); // Updated router to v2
    address constant NERVE_UST_SWAP = address(0x2dcCe1586b1664f41C72206900e404Ec3cA130e0);
    ...
    function setupWithdrawTokens() internal {
        // Start with UST
        IERC20 _token = IERC20(address(0x23396cF899Ca06c4472205fC903bDB4de249D6fC));
        tokenList.push(
            TokenInfo({
                token: _token,
    ...
    function simulateExchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal view returns (uint256) {
        ...
        if(_idOut == WBNB_TOKEN_ID){
            ...
            NerveExchange router = NerveExchange(NERVE_UST_SWAP);
            _amount = router.calculateSwapUnderlying(tokIn, tokOut, _amount);
            return _amount;
        }
    }
    ...
    function exchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal {
        ...
        if(_idOut == WBNB_TOKEN_ID){
            ...
            NerveExchange router = NerveExchange(NERVE_UST_SWAP);
            tokenList[_idIn].token.safeApprove(NERVE_UST_SWAP, 0);
            tokenList[_idIn].token.safeApprove(NERVE_UST_SWAP, _amount);
            router.swapUnderlying(tokIn, tokOut, _amount, 1, now.add(60));
            return;
        }      
    }
    ...
    function checkAndSwapToken(address _executor, uint256 targetID, bool doFlash) internal {
        ...
        bool _expectIncrease = false;
        {
            if(doFlash == false){
                ...
                _expectIncrease = true;
                USTFlashModule(flashModuleAddress).doFlashTrade();
    ...
    function expectedProfit(bool inWBNBForExecutor, bool checkFlash) external view returns (uint256,uint256,uint256) {
        ...
        uint256 targetID;
        {
            ...
            if(checkFlash == false){
                ...
                _normalizedGain = USTFlashModule(flashModuleAddress).checkFlashTrade(); // Gain in BUSD
                _allocatedStipend = _allocatedStipend.mul(3);
                targetID = 1;
            }
        }
