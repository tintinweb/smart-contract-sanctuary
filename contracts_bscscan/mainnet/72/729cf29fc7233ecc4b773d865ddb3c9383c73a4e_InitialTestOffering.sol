...
// File: contracts/InitialTestOffering.sol
 ...
 */
contract InitialTestOffering is ReentrancyGuard, Initializable {
  using SafeMath for uint256;
  ...
  // The block number when ISO starts
  uint256 public startBlock;
  // The block number when ISO ends
  ...
  function deposit(uint256 _amount) public {
    require (block.number > startBlock && block.number < endBlock, 'not ISO time');
    require (_amount > 0, 'need _amount > 0');
    uint256 wantBalBefore = IBEP20(lpToken).balanceOf(address(this));
    lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
    uint256 wantBalAfter = IBEP20(lpToken).balanceOf(address(this));
...
//    require (!userInfo[msg.sender].claimedAmount, 'nothing to harvest');
  ...
  // get the amount of ISO token user is able to get so far
  ...
  // get the amount of ISO token locked for user
  function getClaimLockedOfferingAmount(address _user) public view returns(uint256) {
    ...
    return totalClaimLockedOfferingAmount;
  }
