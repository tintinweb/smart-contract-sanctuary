...
contract DividendDistributor is IDividendDistributor {
    ...
    // EARN
    IBEP20 ChemsInu = IBEP20(0xdFE6891ce8E5a5c7Cf54fFdE406A6C2C54145F71);
    ...
    constructor (address _router2) {
        _router = _router2 != address(0)
            ? IDEXRouter(_router2)
    ...
    function deposit() external payable override _onlyToken {
        uint256 balanceBefore = ChemsInu.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(ChemsInu);
        ...
        uint256 amount = ChemsInu.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if (amount > 0) {
            ...
            ChemsInu.transfer(shareholder, amount);
...
contract Cinutopia is IBEP20, Auth {
    ...
    // Addresses
    address ChemsInu = 0xdFE6891ce8E5a5c7Cf54fFdE406A6C2C54145F71; 
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address CHARITY = 0xBAA83814E5405DCCd05dDF800ec74258981B7Af7;
    ...
    // Max wallet and TX
    uint256 public _maxBuyTxAmount = _totalSupply * 20 / 10000; // 0.20% or 250k tokens
    ...
    // Buy Fees
    uint256 _liquidityFeeBuy = 300; 
    uint256 _buybackFeeBuy = 0;
    uint256 _reflectionFeeBuy = 500;
    uint256 _marketingFeeBuy = 200;
    uint256 _charityBuy = 100;
    uint256 _totalFeeBuy = 1100;
    
    // Sell fees
    uint256 _liquidityFeeSell = 300;
    uint256 _buybackFeeSell = 0;
    uint256 _reflectionFeeSell = 500;
    uint256 _marketingFeeSell = 200;
    uint256 _charitySell = 100;
    uint256 _totalFeeSell = 1100;
    ...
    uint8 public _cooldownTimerInterval = 60;
    ...
    constructor () Auth(msg.sender) {
        ...
        _isTimelockExempt[CHARITY] = true;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBCHARITY = amountBNB.mul(_charity).div(totalBNBFee); 
    ...
    // Set our buy fees
    function setBuyFees(uint256 liquidityFeeBuy, uint256 buybackFeeBuy, uint256 reflectionFeeBuy, uint256 marketingFeeBuy, uint256 CHARITYFeeBuy, uint256 feeDenominator) external authorized {
        ...
        _charityBuy = CHARITYFeeBuy;
        _totalFeeBuy = liquidityFeeBuy.add(buybackFeeBuy).add(reflectionFeeBuy).add(marketingFeeBuy).add(CHARITYFeeBuy);
        _feeDenominator = feeDenominator;
    }

    // Set our sell fees
    function setSellFees(uint256 liquidityFeeSell, uint256 buybackFeeSell, uint256 reflectionFeeSell, uint256 marketingFeeSell, uint256 CHARITYFeeSell, uint256 feeDenominator) external authorized {
        ...
        _charitySell = CHARITYFeeSell;
        _totalFeeSell = liquidityFeeSell.add(buybackFeeSell).add(reflectionFeeSell).add(marketingFeeSell).add(CHARITYFeeSell);
        _feeDenominator = feeDenominator;
    }
