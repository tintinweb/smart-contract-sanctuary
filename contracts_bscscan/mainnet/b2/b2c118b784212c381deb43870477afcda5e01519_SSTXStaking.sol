...
contract SSTXStaking is Ownable {
    ...
    function changeMembershipAPY(uint256 index, uint256 newAPY) external onlyOwner {
        ...
        if (index > 0) require(MembershipLevels[index - 1].APY < newAPY, "Cannot be lower than previous level");
        if (index < levelsCount - 1) require(MembershipLevels[index + 1].APY > newAPY, "Cannot be higher than next level");
    ...
    function changeMembershipThreshold(uint256 index, uint256 newThreshold) external onlyOwner {
        require(index <= levelsCount - 1, "Wrong membership id");
        if (index > 0) require(MembershipLevels[index - 1].threshold < newThreshold, "Cannot be lower than previous level");
        if (index < levelsCount - 1) require(MembershipLevels[index + 1].threshold > newThreshold, "Cannot be higher than next level");
    ...
    function stake(uint256 tokens) external returns (bool) {
        ...
        if (Stakes[msg.sender].staked == 0) {
            ...
            //In case a user has unclaimed tokens
            uint256 reward = getReward(msg.sender);
            if (reward > 0) {
                Stakes[msg.sender].staked += reward;
                // totalTokenstaked += reward;
        ...
        // totalTokenStaked += tokensReceived;
    ...
    function emergency_withdraw() external onlyOwner() returns (bool) {
        ...
        // totalTokensStaked = totalTokenStaked - reward - unstakeAmount;
    ...
    function unstake(uint256 unstakeAmount) external returns (bool) {
        ...
        require(Stakes[msg.sender].staked > 0, "Nothing to unstake");
        require(0 < unstakeAmount  && unstakeAmount <= Stakes[msg.sender].staked, "The number of tokens you are tyring to unstake exceeds total staked amount.");
        uint256 reward = getReward(msg.sender);

        if(unstakeAmount == Stakes[msg.sender].staked){
            _token.transferFrom(locker, msg.sender, reward + unstakeAmount);
            // totalTokenStaked = totalTokenStaked - reward - unstakeAmount;
            delete Stakes[msg.sender];
            // Decreases number of total active rewardMembers
            rewardMembers--;
            emit Claimed(msg.sender, reward);
