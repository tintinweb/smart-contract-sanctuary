...
contract aspenNFT  is ERC1155 { 
   ...
   constructor() payable ERC1155
        ('https://aspenlabs.io/collectables/tokens/{id}'){
            ...
            MetadataURI = 'https://aspenlabs.io/collectables/tokens/';
    ...
    function getMetadataURI(string calldata TokenID) public view returns(string memory) {
        string memory URI =  string(abi.encodePacked(MetadataURI,TokenID));
        return URI;
    }
    ...
    function momentsNoLongerForSale(uint momentID) public payable {
        ...
        emit momentNoLongerForSale(momentID);
    }
    ...
    function offerMomentForSale(uint momentID, uint minSalePriceInWei) public payable {
        ...
        emit momentOffered(momentID, minSalePriceInWei, zeroaddress);
    }
    ...
    function offerMomentForSaleToAddress(uint momentID, uint minSalePriceInWei, address toAddress)  payable public {
        ...
        emit momentOffered(momentID, minSalePriceInWei, toAddress);
    }
    ...
    function buyMoment(uint256 momentID) payable public {
        require(msg.value >= Packprice);
        owner.transfer(msg.value);
        emit TransferFunds(msg.sender,owner,msg.value);
        uint256 amount = PackSize;
        _safeTransferFrom(owner, msg.sender, momentID, amount , msg.data );
        momentIDToAddress[momentID] = msg.sender;
        emit Transfer(owner, msg.sender, momentID);
        emit momentBought(momentID, msg.value, owner, msg.sender);
    }
    ...
    function enterBidForMoment(uint momentID) payable public {
        ...
        emit momentBidEntered(momentID, msg.value, msg.sender);
    }
    ...
    function acceptBidForMoment(uint momentID, uint minPrice) public payable {
        ...
        emit TransferFunds(bid.bidder,msg.sender,bid.value);
        _safeTransferFrom(seller,bid.bidder,momentID , PackSize, "0x01" );
        ...
        emit momentBought(momentID, bid.value, seller, bid.bidder);
    }
    ...
    function withdrawBidForMoment(uint momentID) public payable {
        ...
        emit momentBidWithdrawn(momentID, bid.value, msg.sender);
    ...
    function transferMoment(uint momentID, address _to) public {
        require(momentIDToAddress[momentID] == msg.sender);
        uint amount = 1;
        _safeTransferFrom(msg.sender, _to, momentID, amount, msg.data);
        momentIDToAddress[momentID] = _to;
