...
contract Mdate is NFToken {
    ...
    uint256 _tokenId,
    uint256 idToType,
    string memory idToUr
    ) 
    internal  validNFToken(_tokenId)  {
        all[_tokenId] =  starall(idToUr,idToType);
    }
    ...
    function  b(address _owner) internal view returns (uint256){
...
contract zNFTCONTRACT is Mdate,Ownable{
    ...
    {
        super._mint(_to, _tokenId);
        super._setTokenstarall(_tokenId,_type,_uri);
    }
