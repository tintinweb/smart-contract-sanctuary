...
/*
    ...
    ░█░ █▄█ ░█░ █▀█ █▄▄   ░█░ █▄█ █░█ ██▄ █░▀█
   █▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀█
   █ Total Token - The inovative token on BSC █
   █▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█
   █ 5% of each transaction to all holders    █
   █ 5% of each transaction to liquidity pool █
   █     and locked forever !                 █
   █ 1% of each transaction to marketing BNB  █
   █▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█
*/
...
contract TotalToken is Context, IBEP20, Ownable {
    ...
    string private constant _name     = "Total Token";
    string private constant _symbol   = "TOKEN";
    uint8  private constant _decimals = 9;

    uint256 public _taxFee       = 500; // 5% of every transaction is redistributed to holders
    uint256 public _liquidityFee = 500; // 5% of every transaction will be converted into BNB and sent to liquidity
    uint256 public _burnFee      = 0; // 0% of every transaction is burned
    uint256 public _charityPercent = 100; // 1% of every transaction will be converted into BNB and sent to charity

    uint256 public _maxTxAmount                   = 10**4 * 10**9;
    uint256 public _numTokensSellToAddToLiquidity = 10**1 * 10**9;

    uint256 public _initialBurnPercent = 0; //0%
    ...
    constructor () Ownable() {
        _charityWallet = address(0xe343d869709cbCa032E4BcC0A3046964B93D3B98);
        _burnAddress = address(0x000000000000000000000000000000000000dEaD);

        uint256 amountToBurn = _rTotal.div(10000).mul(_initialBurnPercent);
        _rOwned[_msgSender()] = _rTotal.sub(amountToBurn);
        ...
        emit Transfer(_msgSender(), _burnAddress, amountToBurn);
    }
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockTheSwap {
        uint256 onePercent = tokenAmount.div(1000);
        
        uint256 liquidityAmount = onePercent.mul(_liquidityFee);
        uint256 liquidityHalf = liquidityAmount.div(2);
        uint256 otherHalf = liquidityAmount.sub(liquidityHalf);
        swapTokensForBnb(liquidityHalf);

        uint256 liquidityAmountBNB = address(this).balance; //Get BNB balance
        if (liquidityAmountBNB > 0) {
            addLiquidity(otherHalf, liquidityAmountBNB);
        }
        
        
        uint256 charityAmount = onePercent.mul(_charityPercent);

        swapTokensForBnb(charityAmount);

        uint256 charityAmountBNB = address(this).balance; //Get BNB balance
        if (charityAmountBNB > 0) {
            payable(_charityWallet).transfer(charityAmountBNB);
            emit CharitySent(_charityWallet, charityAmountBNB);
        }
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if(_burnFee > 0 && _liquidityFee > 0) _burn(sender, tBurn);
