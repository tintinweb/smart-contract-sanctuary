...
contract ShibaBTCKing is ERC20, Ownable {
    ...
    address public shibabtckingWallet;
    ...
    uint256 public shibabtckingFee;
    ...
    event shibabtckingWalletUpdated(
    ...
    constructor() ERC20("ShibaBTCKing", "SHIBK") {
        uint256 _dividendRewardsFee = 7;
        uint256 _shibabtckingFee = 10;

        dividendRewardsFee = _dividendRewardsFee;
        shibabtckingFee = _shibabtckingFee;
        totalFees = _dividendRewardsFee.add(_shibabtckingFee);

        dividendTracker = new shibabtckingDividendTracker();

        shibabtckingWallet = 0x9eE0C677E0d55CBDbc3322F5Ba3Cc9E9f92719f3;
        ...
        excludeFromFees(shibabtckingWallet, true);
        ...
        onlyOwner
    {
        require(
            ...
            "shibabtcking: Sell transaction multipler must be between 100 (1x) and 200 (2x)"
        );
    ...
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(
            ...
            "shibabtcking: Marketing divisor must be between 0 (0%) and 100 (100%)"
        );
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(
            !swapping,
            "shibabtcking: A swapping process is currently running, wait till that is complete"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "shibabtcking: The dividend tracker already has that address"
        );

        shibabtckingDividendTracker newDividendTracker = shibabtckingDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "shibabtcking: The new dividend tracker must be owned by the FLOKIBUSD token contract"
        );
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "shibabtcking: Dividend reward tax must be between 0 and 10"
        );
        dividendRewardsFee = newFee;
    }

    function updateshibabtckingFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "shibabtcking: Dividend reward tax must be between 0 and 10"
        );
        shibabtckingFee = newFee;
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            newAddress != address(uniswapV2Router),
            "shibabtcking: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "shibabtcking: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "shibabtcking: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "shibabtcking: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateshibabtckingWallet(address newshibabtckingWallet) public onlyOwner {
        require(
            newshibabtckingWallet != shibabtckingWallet,
            "shibabtcking: The liquidity wallet is already this address"
        );
        excludeFromFees(newshibabtckingWallet, true);
        shibabtckingWallet = newshibabtckingWallet;
        emit shibabtckingWalletUpdated(newshibabtckingWallet, shibabtckingWallet);
    }

    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            newValue >= 200000 && newValue <= 500000,
            "shibabtcking: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "shibabtcking: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[from]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 swapTokens = contractTokenBalance.mul(shibabtckingFee).div(
                    totalFees
                );
                swapTokensForBNB(swapTokens);
                transferToshibabtckingWallet(
                    payable(shibabtckingWallet),
    ...
    function transferToshibabtckingWallet(address payable recipient, uint256 amount)
        private
    {
...
contract shibabtckingDividendTracker is DividendPayingToken, Ownable {
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "shibabtcking_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main shibabtcking contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "shibabtcking_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "shibabtcking_Dividend_Tracker: Cannot update claimWait to same value"
        );
