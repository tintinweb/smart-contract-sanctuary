...
/**
 * @dev Implementation of a strategy to get yields from farming LP Pools in Climbswap.
 * Climbswap is an automated market maker (“AMM”) that allows two tokens to be exchanged on the Binance Smart Chain.
 * It is fast, cheap, and allows anyone to participate. Climbswap is aiming to be the #1 liquidity provider on BSC.
 *
 ...
 */
contract StrategyMarsBusdLP is Ownable, Pausable {
     ...
     * {mars} - Climb Token Finance token, used to send funds to the Climb contract.
     ...
     *
     * {REWARDS_FEE} - 4% goes to Climb Tokens holders through the Climbs contract.
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(mars).balanceOf(address(this)).mul(6).div(1000);
    ...
    /**
     * @dev Swaps {mars} for {lpToken0}, {lpToken1} & {wbnb} using Pancakeswap.
     */
