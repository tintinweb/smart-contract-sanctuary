...
// Root file: contracts/pools/AlpacaRabbitPool.sol
...
contract AlpacaRabbitPool is PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // alpaca
    uint256 constant ALPACA_STAKE_PID = 16;
    ...
    IAlpacaBearToken constant ALPACA_IB = IAlpacaBearToken(0x158Da805682BdC8ee32d52833aD41E74bb951E59);

    // rabbit
    uint256 constant RABBIT_STAKE_PID = 2;
    ...
    IERC20 constant RABBIT_IB = IERC20(0xFE1622F9F594A113cd3C1A93F7F6B0d3C0588781);
    ...
    IERC20 public constant stakingToken = IERC20(0x55d398326f99059fF775485246999027B3197955);
    ...
    function withdraw(uint256 amount) external nonReentrant safeTime(msg.sender) {
        ...
        if (withdrawalFee > DUST) {
            stakingToken.approve(address(penginChef), 0);
            stakingToken.approve(address(penginChef), withdrawalFee);
            penginChef.mintByFee(address(stakingToken), 0, withdrawalFee, msg.sender);
    ...
    function getReward() external nonReentrant safeTime(msg.sender) {
        ...
        if (performanceFee > DUST) {
            stakingToken.approve(address(penginChef), 0);
            stakingToken.approve(address(penginChef), performanceFee);
            penginChef.mintByFee(address(stakingToken), performanceFee, 0, msg.sender);
    ...
    function setPenginChef(address _chef) external onlyOwner {
        penginChef = IPenginChef(_chef);
    }
}