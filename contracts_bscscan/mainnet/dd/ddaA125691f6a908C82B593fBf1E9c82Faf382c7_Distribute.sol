...
contract Distribute is Ownable {
    ...
    uint constant SECONDS_PER_HOURS = 60 * 60;
    uint gameStartTime;
    ...
    function addClaimInfo(uint _tokenId, uint _canClaimTotal, uint _claimPercent) internal {
        ...
        lastClaimTime: 0,firstClaimTime: 0,claimNum: 1,abandonedTime: 0,activeTime: block.timestamp });
    ...
    function claim() hasClaimRight() public {
        ...
        //todo minutes to days, 2 to 60
        uint _days = SafeMath.div((block.timestamp - gameStartTime), SECONDS_PER_HOURS) + 2;
        //todo 1 to 30
        uint _canClaimeNumTotal = SafeMath.div(_days, 1);
        ...
        //todo minutes to days,1 to 30 
        nextClaimTime = gameStartTime + (_canClaimeNumTotal - 1) * (1 hours);
        if(_needSendTotal > 0){
