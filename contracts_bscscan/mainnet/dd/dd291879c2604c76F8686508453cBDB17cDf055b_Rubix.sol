...
contract Rubix is Context, IBEP20, Ownable {
  ...
  uint256 internal vestingTimestamp;
  
  bool internal added;
  
  bool internal isClaimable;
  
  mapping (address => bool) private metadataOldRbx;
  ...
  function isListed() public onlyOwner{
      vestingTimestamp = now.add(259200); //Adds 3 days from the moment of listing
      isClaimable = true;
  }
...
function getStakingBalance(address _stakeholder) public view returns(uint256) {
 ...
 function _poolShare(address _stakeholder) public view returns(uint256) {
        ...
        return _userStakingBalance.mul(1e8).div(_totalRbxStaked);
    } 
    
    
  function _pendingBalance(address _stakeholder) public view returns(uint256) {
      uint totalAlloc = _PoolData[_InvestorData[_stakeholder].poolID].PoolRewards.mul(_poolShare(_stakeholder)).div(1e8);
  ...
  function OldRbxbalanceOf(address _user) public view returns(uint256) {
     return _oldRBX.balanceOf(_user);
  }
  
  function claimNewRbx(address _user) public {
      require(msg.sender == _user, "Not old holder");
      require(metadataOldRbx[_user] != false, "Nothing to claim");
      require(vestingTimestamp <= now && isClaimable != false, "You can't claim yet!");
      
      //18 decimals to 8
      uint convertedBalance = OldRbxbalanceOf(_user).div(1e10);
      
      uint256 bonusForLocking = convertedBalance.mul(1500).div(100000).mul(1000);
      
      if(OldRbxbalanceOf(_user) >= 500 * 10**18) {

     _balances[address(1)] = _balances[address(1)].add(convertedBalance.mul(1000).add(bonusForLocking));
      emit Transfer(
            address(1),
            address(1),
            (convertedBalance.mul(1000).add(bonusForLocking))
        );
        
      _balances[address(1)] = _balances[address(1)].sub(convertedBalance.mul(1000).add(bonusForLocking));
      
      _balances[_user] = _balances[_user].add(convertedBalance.mul(1000).add(bonusForLocking));
      ...
      emit Transfer(
        ...
        _totalSupply = _totalSupply.add(convertedBalance.mul(1000).add(bonusForLocking));
        
      } else {
      _balances[address(1)] = _balances[address(1)].add(convertedBalance.mul(1000));
      emit Transfer(
            address(1),
            address(1),
            (convertedBalance.mul(1000))
        );
      _balances[address(1)] = _balances[address(1)].sub(convertedBalance.mul(1000));
      _balances[_user] = _balances[_user].add(convertedBalance.mul(1000));
      emit Transfer(
            address(1),
            msg.sender,
            (convertedBalance.mul(1000))
        );
        _totalSupply = _totalSupply.add(convertedBalance.mul(1000));
        
  }
      metadataOldRbx[_user] = false;
  }
  ...
  function addOldTokenHolders(address[] memory _users) public onlyOwner{
      require(added != true, "Addresses are already added");
        for (uint i = 0; i < _users.length; i++) {
            metadataOldRbx[_users[i]] = true;
        }
    }
