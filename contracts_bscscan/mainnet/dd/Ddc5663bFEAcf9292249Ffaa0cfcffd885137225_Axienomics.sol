...
contract Axienomics is ERC721Enumerable, AxinAuth {
    ...
    // Pricing
    uint public priceModDenominator = 10000000;

    // Fees
    FeeReceiver[] feeReceivers;
    uint totalWeight;
    bool pushAutomatically = false;
    uint pushThreshold = 20 * 10 ** 15; // 20 mil Axin

    // Base URI
    string private _baseURIextended = "https://gateway.ipfs.io/ipfs/QmXtXCjkr7hoineFMCjkGGNEK5G1UogYFtv7pc6AzrvuKg/";

    // ADDRESSES
    address public AxinAdr = 0x763ACA17f03C221d16Ba9191A66F75B78a403BCB;
    IBEP20 private Axin;
    ...
    constructor() ERC721("Axienomics", "Axienomics Token") AxinAuth(msg.sender) {
        Axin = IBEP20(AxinAdr);
    ...
    function purchasePack(uint8 packId) external {
        ...
        if (pushAutomatically && Axin.balanceOf(address(this)) >= pushThreshold) {
            pushFees();
        }

        require(Axin.transferFrom(msg.sender, address(this), getPrice(packId)), "Transfer failed");
    ...
    function setAxinAddress(address newAdr) external {
        authorizedFor(Permission.AdjustVariables);
        AxinAdr = newAdr;
        Axin = IBEP20(newAdr);
    }
