...
// File: contracts/interfaces/IMashLottery.sol

interface IMashLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in MASH per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in MASH token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInMash: price of a ticket in MASH
...
// File: contracts/ClimbLottery.sol


/** @title ClimbLottery Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract MashLottery is ReentrancyGuard, IMashLottery, Ownable {
    ...
    uint256 public maxPriceTicketInMash = 50 ether;
    uint256 public minPriceTicketInMash = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInMash;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] mashPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInMash;
        uint32 finalNumber;
    }
     ...
     * @param _mashTokenAddress: address of the MASH token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _mashTokenAddress, address _randomGeneratorAddress) {
        mashToken = IERC20(_mashTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of MASH to this contract
        uint256 amountMashToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInMash,
            _ticketNumbers.length
        );

        // Transfer mash tokens to this contract
        mashToken.safeTransferFrom(address(msg.sender), address(this), amountMashToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInMash += amountMashToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInMashToTransfer
        uint256 rewardInMashToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInMashToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        mashToken.safeTransfer(msg.sender, rewardInMashToTransfer);

        emit TicketsClaim(msg.sender, rewardInMashToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInMash) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in MASH for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].mashPerBracket[j] =
                ...
                // A. No MASH to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].mashPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInMash - amountToShareToWinners);

        // Transfer MASH to treasury address
        mashToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        mashToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInMash += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInMash >= minPriceTicketInMash) && (_priceTicketInMash <= maxPriceTicketInMash),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInMash: _priceTicketInMash,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            mashPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInMash: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(mashToken), "Cannot be MASH token");
    ...
    /**
     * @notice Set MASH price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInMash: minimum price of a ticket in MASH
     * @param _maxPriceTicketInMash: maximum price of a ticket in MASH
     */
    function setMinAndMaxTicketPriceInMash(uint256 _minPriceTicketInMash, uint256 _maxPriceTicketInMash)
        external
        onlyOwner
    {
        require(_minPriceTicketInMash <= _maxPriceTicketInMash, "minPrice must be < maxPrice");

        minPriceTicketInMash = _minPriceTicketInMash;
        maxPriceTicketInMash = _maxPriceTicketInMash;
    }
     ...
     * @param _priceTicket price of a ticket (in MASH)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].mashPerBracket[_bracket];
        } else {
            return 0;
        }
    }
