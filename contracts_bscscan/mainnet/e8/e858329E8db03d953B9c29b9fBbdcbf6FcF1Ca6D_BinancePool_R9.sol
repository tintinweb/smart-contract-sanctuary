...
contract BinancePool_R9 is PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, IGlobalPool {
    ...
    IERC20Manageable private _fEthContract;
    ...
    function countPendingStakesUnderThreshold(uint256 threshold) public view returns (uint256) {
        uint256 total = 0;
        uint256 i = 0;
        for (i = _pendingGap; i < _pendingStakers.length; i++) {
            ...
            total = total.add(_pendingUserStakes[_pendingStakers[i]]);
        }
        return i;
    }

    function getRawPendingStakes() public view returns (address[] memory, uint256[] memory) {
    ...
    function claimToIntermediary(address payable intermediary, uint256 networkFee, uint256 threshold) public onlyOperator payable {
        ...
        if (networkFee > _collectedFee) {
            networkFee = _collectedFee;
        }
    ...
    function _stake(uint256 amount) internal {
        ...
        require(amount >= REQUESTER_MINIMUM_POOL_STAKING, "Value must be greater than min amount");
    ...
    function stakeAndClaimAethC(uint256 amount) public payable {
        _stake(amount);
        _claimAETH();
    }
    ...
    function _claimFETH() internal {
        revert("Not implemented");
    }

    function stakeAndClaimAethB(uint256 amount) public payable {
        _stake(amount);
        _claimFETH();
    }
    ...
    function forcePushStakersIntoQueue(address[] memory stakers) public onlyOwner {
        for (uint256 i = 0; i < stakers.length; i++) {
            _pendingStakers.push(stakers[i]);
        }
    }

    function initState(address[] memory stakers, uint256[] memory amounts, uint256 pendingGap, uint256 collectedFee) public onlyOwner {
        for (uint256 i = 0; i < stakers.length; i++) {
            _pendingUserStakes[stakers[i]] = amounts[i];
            _pendingStakers.push(stakers[i]);
        }
        _pendingGap = pendingGap;
        _collectedFee = collectedFee;
    }
    ...
    function changeFethContract(IAETH fEthContract) public onlyOwner {
        _fEthContract = fEthContract;
    }
}
