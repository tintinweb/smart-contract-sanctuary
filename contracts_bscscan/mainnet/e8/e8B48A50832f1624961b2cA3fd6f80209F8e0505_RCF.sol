...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        validateTokenTransfer(sender, recipient, amount);
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        validateTokenTransfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        validateTokenTransfer(account, address(0), amount);
...
contract RCFRewardprovider is RewardPayingToken, Ownable {
    ...
    event claimTimeoutUpdated(uint256 indexed newValue, uint256 indexed oldValue);
    event Claim(address indexed account, uint256 amount, bool indexed automatic);

    constructor() RewardPayingToken("RCF_Reward_Tracker", "RCF_Reward_Tracker") {
    	claimTimeout = 3600;
        minimumTokenBalanceForRewards = 10000 * (10**9); 
    }

    function _transfer(address, address, uint256) internal override pure {
        require(false, "RCF_Reward_Tracker: No transfers allowed");
    }

    function withdrawReward() public override  pure {
        require(false, "RCF_Reward_Tracker: withdrawReward disabled. Use the 'claim' function on the main contract.");
    }
    ...
    function updateclaimTimeout(uint256 newclaimTimeout) external onlyOwner {
        require(newclaimTimeout >= 600 && newclaimTimeout <= 86400, "RCF_Reward_Tracker: claimTimeout must be updated to between 1 and 24 hours");
        require(newclaimTimeout != claimTimeout, "RCF_Reward_Tracker: Cannot update claimTimeout to same value");
        emit claimTimeoutUpdated(newclaimTimeout, claimTimeout);
    ...
    function getAccount(address _account)
        ...
        nextClaimTime = lastClaimTime > 0 ?
                                    lastClaimTime.add(claimTimeout) :
                                    0;
    ...
    function canAutoClaim(uint256 lastClaimTime) private view returns (bool) {
    	...
    	return block.timestamp.sub(lastClaimTime) >= claimTimeout;
    }
...
contract RCF is ERC20, Ownable {
    
    using SafeMath for uint256;
    uint256 public liquidityAddPercentage = 4;

    uint256 public rewardpercent = 10;
    uint256 public marketingComissionPercentage = 4;
  
uint256 private constant Supply_token = 8*10 ** 7 * 10 ** DECIMALS;

 uint256 private _totalSupply=Supply_token;
    uint256 public maxTxAmount = 1000000 * (10**9);
    uint256 public walletMax = _totalSupply;
    uint256 public swapTokensAmount = 25000 * (10**9);
  ...
  uint256 private constant Final_SUPPLY = ~uint128(0); 
    ...
    mapping (address => bool) public isAddressLimitexcuse;
    mapping (address => bool) public txLimitexcuse;
    mapping (address => bool) public _isNotAllowed;
      address public pancakeV2Pair;
    IpancakeV2Router02 public pancakeV2Router;
    RCFRewardprovider public Rewardprovider;

    address public walletMarket = 0x642a570B86D4EB8aF014A5f7f08613458632D33b;
    ...
    bool public swapliquifylimited = false;
    ...
    constructor() ERC20("Rich Floki", "RCF") {
        ...
        // exclude from receiving Rewards
        	Rewardprovider = new RCFRewardprovider();
        	 Rewardprovider.excludeFromRewards(owner());
        Rewardprovider.excludeFromRewards(address(this));
        Rewardprovider.excludeFromRewards(address(pancakeV2Pair));
        Rewardprovider.excludeFromRewards(address(pancakeV2Router));
        Rewardprovider.excludeFromRewards(address(Rewardprovider));
        Rewardprovider.excludeFromRewards(0x000000000000000000000000000000000000dEaD);
        Rewardprovider.excludeFromRewards(address(0));
        	
        totalFees = rewardpercent.add(liquidityAddPercentage).add(marketingComissionPercentage);

    

        isAddressLimitexcuse[owner()] = true;
        isAddressLimitexcuse[pancakeV2Pair] = true;
        isAddressLimitexcuse[address(this)] = true;
        
        txLimitexcuse[owner()] = true;
        txLimitexcuse[address(this)] = true;
    ...
    function updateFees(uint256 newRewardFee, uint256 newLiqFee, uint256 newmarketingComissionPercentage) public onlyOwner {
        rewardpercent = newRewardFee;
        liquidityAddPercentage = newLiqFee;
        marketingComissionPercentage = newmarketingComissionPercentage;
    ...
    function changeIsRewardexcuse(address holder, bool excuse) external onlyOwner {
        if(excuse){
            Rewardprovider.excludeFromRewards(holder);
        }else{
             try Rewardprovider.includeInRewards(payable(holder), balanceOf(holder)) {} catch {}
        }
    }

    function changetxLimitexcuse(address holder, bool excuse) external onlyOwner {
        txLimitexcuse[holder] = excuse;
    }
    ...
    function updatepancakeV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeV2Router), "RCF: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "RCF: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 750000, "RCF: gasForProcessing must be between 200,000 and 750,000");
        require(newValue != gasForProcessing, "RCF: Cannot update gasForProcessing to same value");
        emit GasForProcessingUpdated(newValue, gasForProcessing);
        gasForProcessing = newValue;
    }

    function updateclaimTimeout(uint256 claimTimeout) external onlyOwner {
        Rewardprovider.updateclaimTimeout(claimTimeout);
    }

    function updateMinimumTokenBalanceForRewards(uint256 claimTimeout) external onlyOwner {
        Rewardprovider.updateMinimumTokenBalanceForRewards(claimTimeout);
    }

    function getDistributionSettings() external view returns(uint256, uint256) {
        return (Rewardprovider.claimTimeout(), Rewardprovider.minimumTokenBalanceForRewards());
    }

    function getTotalRewardsDistributed() external view returns (uint256) {
        return Rewardprovider.totalRewardsDistributed();
    }

    function withdrawableRewardOf(address account) public view returns(uint256) {
    	return Rewardprovider.withdrawableRewardOf(account);
  	}

	function RewardTokenBalanceOf(address account) public view returns (uint256) {
		return Rewardprovider.balanceOf(account);
	}

	function processRewardprovider(uint256 gas) external {
		(uint256 iterations, uint256 claims, uint256 lastProcessedIndex) = Rewardprovider.process(gas);
		emit ProcessedRewardprovider(iterations, claims, lastProcessedIndex, false, gas, tx.origin);
    }

    function claim() external {
		Rewardprovider.processAccount(payable(msg.sender), false);
    }
    ...
    function changeisAddressLimitexcuse(address holder, bool excuse) external onlyOwner {
        isAddressLimitexcuse[holder] = excuse;
    }

    function changeSwapSettings(bool enableSwapBack, bool swapByLimitOnly, uint256 newSwapBackLimit) external onlyOwner {
        swapliquifylimited = swapByLimitOnly;
        swapTokensAmount = newSwapBackLimit;
        
        if(swapliquifyEnabled != enableSwapBack)
        {
            swapliquifyEnabled = enableSwapBack;
            emit swapliquifyEnabledUpdated(enableSwapBack);
        }
    }
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
    ...
    function _transfer(address from, address to, uint256 amount ) internal override {
        ...
        if(!txLimitexcuse[from] && !txLimitexcuse[to] && !swapping) {
            require(amount <= maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        }

        if(enableDisableWallet && !isAddressLimitexcuse[to]){
            require(balanceOf(to).add(amount) <= walletMax, "Wallet limit reached");
        }

        uint256 contractTokenBalance = balanceOf(address(this));

        if(swapliquifyEnabled &&  totalFees > 0 && contractTokenBalance >= swapTokensAmount && !swapping && from != pancakeV2Pair) { 
            if(swapliquifylimited)
                contractTokenBalance = swapTokensAmount;
        ...
        try Rewardprovider.setBalance(payable(from), balanceOf(from)) {} catch {}
        try Rewardprovider.setBalance(payable(to), balanceOf(to)) {} catch {}

        if(!swapping) {
	    	uint256 gas = gasForProcessing;

	    	try Rewardprovider.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
	    		emit ProcessedRewardprovider(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
	    	} 
	    	catch {}
        }
    }
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 tRewards = tokensToLiquify.mul(rewardpercent).div(totalFees);
        uint256 tokensToLP = tokensToLiquify.mul(liquidityAddPercentage).div(totalFees).div(2);
        ...
        _approve(address(this), address(pancakeV2Router), tokensToLiquify);
        ...
        uint256 bnbFeeFactor = totalFees.sub(liquidityAddPercentage.div(2));
        
        uint256 bnbForLiquidity = bnbBalance.mul(liquidityAddPercentage).div(bnbFeeFactor).div(2);
        ...
        {
            (bool success,) = address(Rewardprovider).call{value: bnbForReflection}("");
