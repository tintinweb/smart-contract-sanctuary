...
/** 
   ...
   -1% fee auto moved to team wallet
...
contract ARTN is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
 ...
 */
    ...
    address payable public _presaleWallet;
    ...
    string private constant _name = "GoldenArts";
    string private constant _symbol = "ARTN";
    ...
    constructor (address payable _marketing, address payable _team, address dexAddress, address payable _dev, address payable _presale) {
        _rOwned[_msgSender()] = _rTotal;
		_presaleWallet = _presale;
		_marketingWallet = _marketing;
		_teamWallet = _team;
        _devWallet = _dev;


        IPancakeSwapV2Router02 _pancakeSwapV2Router = IPancakeSwapV2Router02(dexAddress); 
    ...
    function getBalancePresale(address recipient) public view returns (uint256 amount) {
        amount = preSaleWhitelist[recipient];
       return  amount;
    
    }
    
    function updateDex(address _newDexAddress) external onlyOwner() {
        IPancakeSwapV2Router02 _pancakeSwapV2Router = IPancakeSwapV2Router02(_newDexAddress);
   ...
   function name() public pure returns (string memory) {
    ...
    /* modifier isLoked(address sender){
        bool isOkay = true;
        
        if(preSaleWhitelist[sender]>0 && timeLock > block.timestamp){
            isOkay = false;
        }
        require(isOkay,"TOKEN LOCKATI");
        _;
    ...
    modifier onlyPresale() {
        require(_presaleWallet == _msgSender(), "Ownable: caller is not the presale address");
        _;
    }
    
    function transferForPresale(address recipient,uint256 amount) public onlyPresale returns (bool){
        preSaleWhitelist[recipient] = amount;
   ...
   /*  function setTimeLock(uint256 _timeLock) external onlyOwner() {
    ...
    function setPresaleWallet(address payable presaleWallet) external onlyOwner() {
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _teamFee == 0 && _marketingFee == 0 &&  _devFee == 0) return;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount.sub(tMarketing).sub(tTeam));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount.sub(tMarketing).sub(tTeam));
        }  else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount.sub(tMarketing).sub(tTeam));
        } else {
            _transferStandard(sender, recipient, amount.sub(tMarketing).sub(tTeam));
        }
