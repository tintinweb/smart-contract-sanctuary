...
contract MarkhorMetaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("MarkhorMeta_Dividend_Tracker", "MarkhorMeta_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MarkhorMeta_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MarkhorMeta_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MarkhorMeta contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MarkhorMeta_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "MarkhorMeta_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MarkhorMeta is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    MarkhorMetaDividendTracker public dividendTracker;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _devFee) public onlyOwner {
    ...
    function setdevWallet(address payable _newBuybakWallet) public onlyOwner {
        devWallet = _newBuybakWallet;
    }

    constructor() ERC20("Markhor Meta", "MM") {
        BNBRewardsFee = 200;
        liquidityFee = 100;
        extraFeeOnSell = 100; // extra fee on sell
        marketingFee = 200;
        devFee = 200;
        marketingWallet = payable(0x6dA6409852B37D96d762bfd5fa5047ea1f53Dde6);
        devWallet = payable(0x0e059d58C4E63F881542617ffA04B6331004f850);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(devFee); // total fee transfer and buy

    	dividendTracker = new MarkhorMetaDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MarkhorMeta: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MarkhorMeta: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MarkhorMeta: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MarkhorMeta: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MarkhorMeta: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MarkhorMeta: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if(automatedMarketMakerPairs[to]) {
                extraFee =(amount*extraFeeOnSell)/10000;
                fees=fees+extraFee;
            }
        	amount = amount-fees;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        marketingWallet.transfer(marketingAmount);
         ...
         uint256 buybackAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee).sub(marketingFee)).mul(devFee);
        devWallet.transfer(buybackAmount);
