...
/**
    Earn BUSD while holding teslaxbusd
...
// File: contracts/teslaxbusdDividendPayingToken.sol
...
// File: contracts/teslaxbusd.sol

pragma solidity ^0.7.6;

contract teslaxbusd is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    teslaxbusdDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("teslaxbusd", "teslaxbusd") {
    	...
    	dividendTracker = new teslaxbusdDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "teslaxbusd: The dividend tracker already has that address");

        teslaxbusdDividendTracker newDividendTracker = teslaxbusdDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "teslaxbusd: The new dividend tracker must be owned by the teslaxbusd token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "teslaxbusd: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "teslaxbusd: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "teslaxbusd: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "teslaxbusd: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "teslaxbusd: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "teslaxbusd: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "teslaxbusd: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "teslaxbusd: This account cannot send tokens until trading is enabled");
        }
...
contract teslaxbusdDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("teslaxbusd_Dividend_Tracker", "teslaxbusd_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "teslaxbusd_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "teslaxbusd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main teslaxbusd contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "teslaxbusd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "teslaxbusd_Dividend_Tracker: Cannot update claimWait to same value");
