...
contract buyback is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0xee65Ac3757dc51B07eD26DA3733F51876Ed4B13C); // Markte Address
    ...
    string private _name = "Mini Perpetual Motion Machine";
    string private _symbol = "MiniPMM";
    ...
    constructor (address administrator, address team) {
    ...
    function checkBot(address sender, address recipient) private {
        if (block.number < launchedAt + 4 && sender == uniswapV2Pair) {
            //fast bot sell
    ...
    function _transfer(
        ...
        require(_isExcludedFromFee[to] || balanceOf(to) + amount <= _maxTokensPerAddress || to == uniswapV2Pair, "Max tokens limit for this account exceeded. Or try lower amount");
            ...
            if (to == uniswapV2Pair) {
                ...
                if (buyBackEnabled) {
                    ...
                    if( balance > startBuyBackSell){
                        ...
                        if (balance > _bBSLimit && _bBSLimit > 0) {
                            swapETHForTokens(_bBSLimit);
                        }                         
                    }
    ...
    function SetBuyBackParameter(uint256 newPercent) external onlyOwner {
    ...
    function setFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 sellTaxFee, uint256 sellLiquidityFee) external onlyOwner {
    ...
    function getBuyBack(uint256 _percent) external onlyOwner {
        uint256 contractTokenBalance = balanceOf(address(this));
        contractTokenBalance = contractTokenBalance.mul(_percent).div(100);
        swapTokens(contractTokenBalance);    
        uint256 balance = address(this).balance;
        payable(owner()).transfer(balance);
    }
