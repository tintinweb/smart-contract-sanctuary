...
contract BlocksSpace4 is Ownable {
    ...
    uint256 public takeoverPriceMultiplier;
    ...
    event TakeoverPriceMultiplierUpdated(uint256 newMultiplier);

    constructor(address rewardsPoolContract_, bool hasLogo, uint256 blockClaimPrice_, uint256 takeoverPriceMultiplier_, uint256 maxBlockWidth_, uint256 maxBlockHeight_, uint256 maxBlocks_) {
        rewardsPool = BlocksRewardsManager3(rewardsPoolContract_);
        blockClaimPrice = blockClaimPrice_;
        takeoverPriceMultiplier = takeoverPriceMultiplier_;
    ...
    function calculateBlocksOwnershipChanges(BlockAreaLocation memory blocksArea_, uint256 numberOfBlocks_) internal returns (address[] memory, uint256[] memory) {
        ...
        for (uint256 i = blocksArea_.startBlockX; i <= blocksArea_.endBlockX; ++i) {
            for (uint256 j = blocksArea_.startBlockY; j <= blocksArea_.endBlockY; ++j) {
                ...
                Block storage currentBlock = blocks[i * 100 + j];
                currentBlock.price = blockClaimPrice * takeoverPriceMultiplier; // Set constant price * multiplier
    ...
    function updateTakeoverPriceMultiplier(uint256 newMultiplier) external onlyOwner {
        takeoverPriceMultiplier = newMultiplier;
        emit TakeoverPriceMultiplierUpdated(takeoverPriceMultiplier);
    }
}
