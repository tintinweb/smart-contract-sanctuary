...
contract FinalOLEVEL is Context, IERC20, Ownable {
    ...
    string private _name = "FinalOlevel";
    string private _symbol = "FINOL";
    ...
    mapping(address => uint256) private LoyaltyPointsTEMP;
    ...
    function _transfer(
        ...
        uint256 firstaugust = 1627855200;
    ...
    function get_MyLoyaltyPoints(address account) public view returns(string memory, uint256) {
        if (balanceOf(account) == 0) {
            string memory Message = "Note: You need to own tokens to view loyalty points";
            uint256 Loyalty = 0;
            return (Message, Loyalty);
        }
        else {
            uint256 HodlingInSecs = block.timestamp - HodlingStartTime[account];
            uint256 HodlingDays = HodlingInSecs / 1 days;
            uint256 Loyalty_Points = LoyaltyPointsTEMP[account];

            if (HodlingDays > Loyalty_Points) {
                string memory Message = "Nice, you increased your loyalty points. Keep hodling!";
                uint256 Loyalty = HodlingDays * 100;
                return (Message, Loyalty);
            }
            else {
                string memory Message = "To increase your loyalty points: You need to hodl longer then before!";
                uint256 Loyalty = LoyaltyPointsTEMP[account] * 100;
                return (Message, Loyalty);
            }
        }
    }
