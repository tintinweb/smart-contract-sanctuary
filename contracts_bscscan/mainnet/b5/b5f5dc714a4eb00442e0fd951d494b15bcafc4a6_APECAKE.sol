...
Telegaram : https://t.me/ape_cake
Website: : https://apecakes.com
...
contract APECAKE is ERC20, Ownable {
    ...
    constructor() ERC20("APECAKE", "APECAKE") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0x22a16575A40aF1843661a8621aeEa2bfAc8a727c;
    ...
    function tradeStart() external onlyOwner {
        ...
        maxBuyTranscationAmount = 50000000000000000000000000000 * (10**18);
        maxSellTransactionAmount = 50000000000000000000000000000 * (10**18);
        maxWalletToken = 50000000000000000000000000000 * (10**18);
    }
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "APECAKE: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "APECAKE: The new dividend tracker must be owned by the APECAKE token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("APECAKE_Dividend_Tracker", "APECAKE_Dividend_Tracker", 0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "APECAKE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "APECAKE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "APECAKE_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "APECAKE_Dividend_Tracker: Cannot update claimWait to same value");
