...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0x154A9F9cbd3449AD22FDaE23044319D6eF2a1Fab; // CryptoBlades Skill
...
contract LittleSkill is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public _dividendToken = 0x154A9F9cbd3449AD22FDaE23044319D6eF2a1Fab; // CryptoBlades Skill
    ...
    LittleSkillDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 public maxBuyTranscationAmount = 5000000000000;
    uint256 public maxSellTransactionAmount = 3000000000000;
    uint256 public swapTokensAtAmount = 1500000000000;
    uint256 public maxWalletToken = 15000000000000; // 15% of total supply

    uint256 public dividendRewardsFee;
    uint256 public marketingFee;
    uint256 public immutable totalFees;

    // sells have fees of 20 and 10 (10 * 3 and 5 * 3)
    uint256 public sellFeeIncreaseFactor = 200;
    
    uint256 public marketingDivisor = 95;
    ...
    constructor() ERC20("LittleSkill", "LittleSkill") {
    	...
    	dividendTracker = new LittleSkillDividendTracker();

    	buyBackWallet = 0x6DA23aBabFf2EC5A24EaaEa42128Dc981d204A16;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 300, "LittleSkill: Sell transaction multipler must be between 100 (1x) and 300 (3x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "LittleSkill: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "LittleSkill: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "LittleSkill: The dividend tracker already has that address");

        LittleSkillDividendTracker newDividendTracker = LittleSkillDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "LittleSkill: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "LittleSkill: Dividend reward tax must be between 0 and 10");
        dividendRewardsFee = newFee;
    }
    
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 1000000000, "LittleSkill: Dividend reward tax must be between 0 and 1000000000");
        marketingFee = newFee;
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "LittleSkill: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "LittleSkill: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "LittleSkill: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LittleSkill: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "LittleSkill: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LittleSkill: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "LittleSkill: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                uint256 buyBackBalance = address(this).balance;
                if (buyBackEnabled && buyBackBalance > uint256(1)) {
...
contract LittleSkillDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("LittleSkill_Dividend_Tracker", "LittleSkill_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 1500000000000; //must hold 1500000000000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "LittleSkill_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "LittleSkill_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LittleSkill contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 36000, "LittleSkill_Dividend_Tracker: claimWait must be updated to between 30 and 300 minutes");
        require(newClaimWait != claimWait, "LittleSkill_Dividend_Tracker: Cannot update claimWait to same value");
