...
contract HSWToken is BEP20("HiSwap Token", "HSW") {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "HSW::delegateBySig: invalid signature"
        );
        require(
...
// HSWBar with Governance.
contract HSWBar is BEP20("HSWBar Token", "HSWBar") {
    //Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    constructor(HSWToken _hsw) public {
        hsw = _hsw;
    }

    // Safe hsw transfer function, just in case if rounding error causes pool to not have enough HSW tokens.
    function safeHswTransfer(address _to, uint256 _amount) public onlyOwner {
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of HSW tokens
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. HSW tokens to distribute per block.
        uint256 lastRewardBlock; // Last block number that HSW tokens distribution occurs.
        uint256 accHswPerShare; // Accumulated HSW tokens per share, times 1e12. See below.
    }

    // The HSW TOKEN!
    HSWToken public hsw;
    // The HSWBAR TOKEN!
    HSWBar public hswbar;
    // Dev address.
    ...
    // View function to see pending HSW tokens on frontend.
    function pendingHsw(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 hswReward = multiplier
                .mul(hswPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accHswPerShare = accHswPerShare.add(
                hswReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 hswReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        hsw.mint(devaddr, hswReward.div(10));
        hsw.mint(address(hswbar), hswReward);
        pool.accHswPerShare = pool.accHswPerShare.add(
            hswReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accHswPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeHswTransfer(msg.sender, pending);
    ...
    function enterStaking(uint256 _amount) public {
        ...
        hswbar.mint(msg.sender, _amount);
    ...
    function leaveStaking(uint256 _amount) public {
        ...
        hswbar.burn(msg.sender, _amount);
    ...
    function safeHswTransfer(address _to, uint256 _amount) internal {
        hswbar.safeHswTransfer(_to, _amount);
    }
