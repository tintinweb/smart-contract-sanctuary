...
contract BorrowerOperations is LiquityBase, OwnableUpgradeable, CheckContract, IBorrowerOperations {
    ...
    event TroveCreatedFor(address indexed _caller, address indexed _borrower);
    ...
    function _openTrove(address _borrower, uint _maxFeePercentage, uint _LUSDAmount, address _upperHint, address _lowerHint) internal {
        ...
        contractsCache.troveManager.setTroveStatus(_borrower, 1);
        contractsCache.troveManager.increaseTroveColl(_borrower, msg.value);
        contractsCache.troveManager.increaseTroveDebt(_borrower, vars.compositeDebt);
        ...
        vars.arrayIndex = contractsCache.troveManager.addTroveOwnerToArray(_borrower);
        ...
        _withdrawLUSD(contractsCache.activePool, contractsCache.lusdToken, _borrower, _LUSDAmount, vars.netDebt);
        ...
        emit TroveUpdated(_borrower, vars.compositeDebt, msg.value, vars.stake, BorrowerOperation.openTrove);
    ...
    function openTrove(uint _maxFeePercentage, uint _LUSDAmount, address _upperHint, address _lowerHint) external payable override {
        _openTrove(msg.sender, _maxFeePercentage, _LUSDAmount, _upperHint, _lowerHint);
    }

    function openTroveOnBehalfOf(address _borrower, uint _maxFeePercentage, uint _LUSDAmount, address _upperHint, address _lowerHint) external payable override {
        _openTrove(_borrower, _maxFeePercentage, _LUSDAmount, _upperHint, _lowerHint);

        emit TroveCreatedFor(msg.sender, _borrower);
    }
