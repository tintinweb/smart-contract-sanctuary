...
Thank you for choosing HAZY, the worlds 
first tokenized crypto BEER coming to 
stores near you soon!

Manufactured and developed by LCB, Local
Craft Beer.

Instagram:
https://www.instagram.com/localcraftbeer/
...
contract HAZY is IERC20, Ownable {
    uint256 private constant MAX_UINT256 = ~uint256(0);
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 1 * 10**12 * 10**DECIMALS;
    uint256 private constant BP_DIVISOR = 10000;
    
    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    
    string private constant NAME = "HAZY"; 
    string private constant SYMBOL = "HZY"; 
    uint8 private constant DECIMALS = 9;

    uint256 public liquidityTax = 200;
    uint256 public marketingTax = 300;
    uint256 public rewardsTax = 500;
    uint256 public transactionTax = liquidityTax + marketingTax + rewardsTax;
    uint256 public numTokensSellDivisor = 1000;

    ISwapRouter02 public swapRouter;
    address public swapPair;
    address public marketingWallet = 0x4DE5759c6ad1382034819F5756DC4A4F9D0A85b0;
    address public rewardsWallet = 0x6d3e35d0Bc7A9817D2C6F66cBCA34A92D93BCBe8;
    ...
    modifier onlyOperator {
        require (msg.sender == operator || msg.sender == owner(), "HAZY: Not authorised");
        _;
    }
    ...
    function setOperator (address newOperator) external onlyOwner {
        require (newOperator != address(0), "HAZY: Can't set operator to the zero address");
    ...
    function decreaseAllowance (address spender, uint256 subtractedValue) external returns (bool) {
        require (subtractedValue <= _allowedFragments[msg.sender][spender], "HAZY: Allowance not high enough");
    ...
    function _approve (address owner, address spender, uint256 amount) private {
        require (owner != address(0), "HAZY: Cannot approve from the zero address");
        require (spender != address(0), "HAZY: Cannot approve the zero address");
    ...
    function _transfer (address sender, address recipient, uint256 amount) private {
        require (recipient != address(0), "HAZY: Cannot transfer to the zero address");
        require (amount > 0, "HAZY: Cannot transfer zero tokens");
        require (!isBlacklisted[sender] && !isBlacklisted[recipient], "HAZY: Blacklisted");
        require (launchTime != 0 || _isExcluded[sender] || _isExcluded[recipient], "HAZY: Not launched");
        ...
        if (!_isExcluded[sender] && !_isExcluded[recipient])
            require (amount <= _maxTxAmount, "HAZY: Transfer amount exceeds the maxTxAmount.");
    ...
    function _tokenTransfer (address sender, address recipient, uint256 amount) private {
        ...
        if (!_isExcluded[recipient] && sender != owner() && recipient != swapPair)
            require (_creditsBalances[recipient] + receiveAmount <= maxWalletAmount, "HAZY: Can't hold that much");
    ...
    function swapAndLiquify (uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (marketingETH > 0) {
            ...
            payable(rewardsWallet).transfer (address(this).balance);
        }
    }
    ...
    function airDrop(address[] calldata recipients, uint256[] calldata amounts) external onlyOwner {
        require (recipients.length == amounts.length, "HAZY: Recipients and amounts lengths must match");
    ...
    function setFees (uint256 _marketingFee, uint256 _liquidityFee, uint256 _rewardsFee) external onlyOwner {
        uint256 totalFee = _marketingFee + _liquidityFee + _rewardsFee;
        require (totalFee < 3000, "HAZY: Total fee must be < 30%");
        liquidityTax = _liquidityFee;
        marketingTax = _marketingFee;
        rewardsTax = _rewardsFee;
    ...
    function setRewardsWallet(address payable wallet) external onlyOwner{
        require(rewardsWallet != address(0), "Ronin: Can't set marketing wallet to the zero address");
        rewardsWallet = wallet;
    }

    function setMaxTx (uint256 _maxTx) external onlyOwner {
        require (_maxTx > 10, "HAZY: Max tx must be > 0.1%");
        maxTx = _maxTx;
    }

    function setMaxWallet (uint256 _maxWallet) external onlyOwner {
        require (_maxWallet > 10, "HAZY: Max wallet must be > 0.1%");
    ...
    function withdrawWETH (address _account) external onlyOwner  {
        require (_account != address(0), "HAZY: Can't withdraw to the zero address");
    ...
    function withdrawToken (address _token, address _account) external onlyOwner {
        require (_token != address(0), "HAZY: Can't withdraw a token of zero address");
        require (_token != address(this), "HAZY: Can't withdraw SafeBET");
        require (_account != address(0), "HAZY: Can't withdraw to the zero address");
