...
/**
 * 
 * AdMonkey is a deflationary BSC token backed by a cutting edge AdTech platform.
 * Automatic BNB Rewards
 * Receive rewards in ANY BSC token
 * 
 * AdMonkey v3.0 - Released Q3 2021
 * 
 ...
 * Shout to AdMonkey, which the contract is based on.
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  IUniswapV2Router02 public uniswapV2Router = IUniswapV2Router02(0xcF0feBd3f17CEf5b47b0cD257aCf6025c5BFf3b7);
  
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "AdMonkey: The router already has that address");
...
contract AdMonkey is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("AdMonkeyNetwork", "ADM", 12) {
        
        uint256 _rewardsFee = 4;
        ...
        totalSellFees = (rewardsFee * 120 / 100) + operationsFee + liquidityFee + buybackFee + teamFee;
        ...
        uint256 totalSupply = 100 * 10 ** 6 * 10 ** 12; // Total supply of 100,000,000 with 12 decimal places
        swapTokensAtAmount = totalSupply * 2 / 10000; // 0.02% swap tokens amount
        maxWalletSize = totalSupply * 25 / 10000; // 0.25% max wallet size
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
      require(isContract(ammAddress), "AdMonkey: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
        ...
        require(newAmount >= totalSupply() * 5 / 10000, "Swap amount cannot be lower than 0.05% total supply.");
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "AdMonkey: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "AdMonkey: The new dividend tracker must be owned by the AdMonkey token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "AdMonkey: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "AdMonkey: airdropToWallets:: Arrays must be the same length");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "AdMonkey: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "AdMonkey: The operations wallet is already this address");
    ...
    function updateTeamWallet(address newWallet) external onlyOwner {
        require(newWallet != teamWallet, "AdMonkey: The team wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "AdMonkey: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "AdMonkey: Cannot update gasForProcessing to same value");
    ...
    // update the maxWalletSize 
    function updateMaxWalletSize(uint256 _maxWalletSize) external onlyOwner {
        require(_maxWalletSize <= totalSupply() * 5 / 1000, "MaxWalletSize can't be more than 0.5%");
    ...
    function setRewardToken(address rewardTokenAddress) external returns (bool) {
        require(isContract(rewardTokenAddress), "AdMonkey: setRewardToken:: Address is a wallet, not a contract.");
        require(rewardTokenAddress != address(this), "AdMonkey: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isBlacklistedToken(rewardTokenAddress), "AdMonkey: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
        require(isContract(rewardTokenAddress), "AdMonkey: setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(uniswapV2Router), "AdMonkey: setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "AdMonkey: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isBlacklistedToken(rewardTokenAddress), "AdMonkey: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
        require(isAMMWhitelisted(ammContractAddress) == true, "AdMonkey: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "AdMonkey: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "AdMonkey: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require(!_isBlacklisted[to] && !_isBlacklisted[from], "AdMonkey: To/from address is blacklisted");
        
        if(!tradingActive){
            require(to == owner() || from == owner() || (_canTradeWhenInactive[from] || _canTradeWhenInactive[to]), "AdMonkey: Trading is disabled");
        }
        ...
        // lets make sure that the current transaction doesn't push over the wallet limit, or fail with error.
        if(automatedMarketMakerPairs[from]){
            require(amount.add(balanceOf(to)) < maxWalletSize, "AdMonkey: Exceeds max wallet size");
        }
        ...
        if(totalFees > 0){
            ...
            if(_isExcludedFromFees[from] || _isExcludedFromFees[to] || from == address(this) || (isBuyTaxDisabled && !automatedMarketMakerPairs[to])) {
                takeFee = false;
            }
    ...
    function recoverExcess(uint256 amount) external onlyOwner {
        require(amount < address(this).balance, "AdMonkey: Can not send more than contract balance");
...
contract DividendTracker is DividendPayingToken {
    ...
    function withdrawDividend() pure external override {
        require(false, "AdMonkey_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AdMonkey contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "AdMonkey_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "AdMonkey_Dividend_Tracker: Cannot update claimWait to same value");
