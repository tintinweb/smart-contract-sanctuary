...
contract Staking is Initializable, OwnableUpgradeable {
     ...
     * @param _rewardAPY The amount of reward to be distributed per second.
     */
    function setRewardAPY(uint256 _rewardAPY) public onlyOwner {
        updatePool();
        ...
        returns (uint256 pending)
    {
        ...
        if (block.timestamp > lastRewardTime && lpSupply != 0) {
            ...
            uint256 timeReward = lpSupply.mul(time).mul(ACC_REWARD_PRECISION).mul(rewardAPY).div(100).div(365 days);
            accRewardPerShare_ = accRewardPerShare_.add(timeReward / lpSupply);
        }
        pending = int256(
    ...
    function updatePool() public {
        if (block.timestamp > lastRewardTime) {
            ...
            if (lpSupply > 0) {
                uint256 time = block.timestamp.sub(lastRewardTime);
                uint256 timeReward = lpSupply.mul(time).mul(ACC_REWARD_PRECISION).mul(rewardAPY).div(100).div(365 days);
                accRewardPerShare = accRewardPerShare.add(timeReward / lpSupply);
            }
            lastRewardTime = block.timestamp;
