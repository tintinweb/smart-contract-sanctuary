...
contract SpaceDog is IERC20, IERC20Metadata, Ownable {
    ...
    uint256 private MAX_SALE_AMOUNT = 800000000000000 * 10 ** _decimals;

    uint256 private MAX_START_BUY = 10000000000000000 * 10 ** _decimals;

    uint256[4] public _saleFees = [10,20,30,40]; //

    uint256 public _userFee = 99; // wallet to wallet
    ...
    function _transfer(
        ...
        if (isUser(sender) && isUser(recipient)) {
            ...
            if (_dailySales[sender].length == 4) {
                require(_dailySales[sender][0] < block.timestamp-86400, "max timeblock");
                delete _dailySales[sender];
                _dailySales[sender][0] = block.timestamp;
            } else if (_dailySales[sender].length > 0) {
                require(_dailySales[sender][_dailySales[sender].length-1] < block.timestamp-300, "delay between sales required");
