...
contract IDOv3Controller is Ownable, IIDOv3Controller {
    ...
    function getTierId(address user) external view override returns (uint256) {
        return tier.getTierId(user);
    }

    function getMaxAllocationAmountAtTierId(uint256 tierId, uint256 baseAmount)
        external
        view
        override
        returns (uint256)
    {
        return tier.getMultiplierAtTierId(tierId) * baseAmount;
    }
    ...
    function createIDO(
        ...
        IDOv3 ido = new IDOv3(this, saleTarget, fundToken, fundTarget);
