...
interface IERC721 {
    ...
    function mint(address to, string memory _owner_id, uint256 keyword_sid) external;
}
...
contract NFT_Market is Ownable {
    ...
    function mint(string memory _owner_id, uint256 keyword_sid) public {
        ...
        IERC721(trustedNftAddress).mint(msg.sender, _owner_id, keyword_sid);
    }

    function mintNative(string memory _owner_id, uint256 keyword_sid) payable public {
        // owner can mint without fee
        // other users need to pay a fixed fee in token
