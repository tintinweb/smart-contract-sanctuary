...
// File: contracts/WhenLamboDividendPayingToken.sol
...
contract WhenLambo is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("WhenLambo", "WHENLAMBO")public {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "WhenLambo: The new dividend tracker must be owned by the WhenLambo token contract");
    ...
    function updatePancakeSwapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(PancakeSwapV2Router), "WhenLambo: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "WhenLambo: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PancakeSwapV2Pair, "WhenLambo: The PanCAKESwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "WhenLambo: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "WhenLambo: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "WhenLambo: gasForProcessing must be between 200,000 and 500,000");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "WhenLambo: This account cannot send tokens until trading is enabled");
        }
        ...
        if( 
            ...
            require(amount <= _MaxSupply, "Sell transfer amount exceeds the MaxSupply.");
        }
...
contract WhenLamboDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("WhenLambo_Dividend_Tracker", "WhenLambo_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "WhenLambo_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "WhenLambo_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
