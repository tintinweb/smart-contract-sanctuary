...
// 5% Nu reflection goes to all Nu holder's wallets
// 5% Nu burn

// 10% other coin tax when sell Nu:

// 5% other coin reflection goes to Nu holder's wallets
// 5% other coin buy back and auto liquidity

// Anti dumping mechanism. When Sell Nu, tx will fail if 0.001% of Total supply
...
contract NuMoon is IERC20 {
    ...
    uint256 private constant startingSupply = 300000000; // underscores aid readability

    uint8 private _decimals = 18;
    uint256 private _decimalsMul = _decimals;
    uint256 private _tTotal = startingSupply * (10 ** _decimalsMul);

    string constant _name = "NuMoon";
    string constant _symbol = "Nu";
    ...
    uint256 private _totalFee = _liquidityFee + _reflectionFee + _burnFee;
    ...
    uint256 public _transferBurnFee = _sellBurnFee;
    ...
    // Max Buy TX amount is 0.001% of Total supply.
    uint256 private buyMaxTxPercent = 1; // Less fields to edit
    uint256 private buyMaxTxDivisor = 100000;
    ...
    // Max Sell TX amount is 0.001% of Total supply.
...
function setTaxesBuy(uint256 liquidityFee, uint256 reflectionFee, uint256 burnFee) external onlyOwner {
        require(liquidityFee <= maxLiquidityFee
                && reflectionFee <= maxReflectionFee
               );
        require(liquidityFee + reflectionFee + burnFee  <= 5000);
    ...
    function setTaxesSell(uint256 liquidityFee, uint256 reflectionFee, uint256 burnFee) external onlyOwner {
        require(liquidityFee <= maxLiquidityFee);
    ...
    function setTaxesTransfer(uint256 liquidityFee, uint256 reflectionFee,uint256 burnFee) external onlyOwner {
        require(liquidityFee <= maxLiquidityFee
                && reflectionFee <= maxReflectionFee
                );
        require(liquidityFee + reflectionFee + burnFee  <= 5000);
    ...
    function getTotalFee() public view returns (uint256) {
        return _reflectionFee + _liquidityFee + _burnFee;
    }
