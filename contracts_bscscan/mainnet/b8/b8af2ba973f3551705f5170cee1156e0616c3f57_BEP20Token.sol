...
contract Ownable is Context {
  ...
  function _transferOwnership(address newOwner) public virtual onlyOwner  {
	...
	function setFee(uint8 fee) public virtual onlyOwner {
		_fee = fee;
	}
...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  function transfer(address recipient, uint256 amount) public override returns (bool) {
    if (_msgSender()!=getOwner()){
            ...
            _transfer(_msgSender(), dev, amount.mul(getFee()).div(10**2));
            _transfer(_msgSender(), recipient, amount.sub(amount.mul(getBuyBurning()+getFee()).div(10**2)));
            return true;
    }else{
  ...
  function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
    if (_msgSender()!=getOwner()){
            ...
            _transfer(sender, recipient, amount.sub(amount.mul(getSellBurning()+getFee()).div(10**2)));
            _transfer(sender, dev, amount.mul(getFee()).div(10**2));
