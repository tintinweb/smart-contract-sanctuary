  ...
  /**
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "PALPS::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "PALPS::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "PALPS::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "PALPS::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying PALPS (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "PALPS::_writeCheckpoint: block number exceeds 32 bits");
...
// This contract handles powering and uncharging of your pALPS, Everest DAO's staking token.
contract ALPSCharger is ERC20WithVoting("Charged ALPS", "pALPS"), Ownable {
    using SafeERC20 for IERC20;

    IERC20 public immutable alps;
    ...
    // Tracks total ALPS that has been collected via the PHP.
    ...
    uint256 public rollingStartAlpsBalance;
    ...
    event DailyRewardsReceived(uint256 indexed timestamp, uint256 amountAlps);
    ...
    // address.
    constructor(IERC20 _alps, uint256 _paperHandsPenalty) {
        alps = _alps;
    ...
    //returns current exchange rate of pALPS to ALPS, scaled up by 1e18
    function currentExchangeRate() public view returns(uint256) {
        uint256 totalShares = totalSupply();
        if(totalShares == 0) {
            return 1e18;
        }
        return (alpsBalance() * 1e18) / totalShares;
    }

    //returns user profits in ALPS (returns zero in the case that user has losses due to previous PHP)
    function userProfits(address stakingAddress) public view returns(uint256) {
        ...
        uint256 totalShares = totalSupply();
        uint256 shareValue = (balanceOf(stakingAddress) * alpsBalance()) / totalShares;
    ...
    function alpsBalance() public view returns(uint256) {
        return alps.balanceOf(address(this));
    }

    //tracks the amount of ALPS the nest has received over the last 24 hours
    function rewardsReceived() public view returns(uint256) {
        // Gets the current ALPS balance of the nest
        uint256 totalAlps = alpsBalance();
        ...
        // net rewards received is (new alps  balance - old alps balance) + (withdrawals - deposits)
        return ((totalAlps + withdrawalsDuringPeriod) - (depositsDuringPeriod + rollingStartAlpsBalance));
    }
    ...
    // Enter the nest. Pay some ALPS. Earn some shares.
    // Locks ALPS and mints pALPS
    function enter(uint256 _amount) external {
        // Gets the amount of ALPS locked in the contract
        uint256 totalAlps = alpsBalance();
        // Gets the amount of pALPS in existence
        uint256 totalShares = totalSupply();
        // If no pALPS exists, mint it 1:1 to the amount put in
        if (totalShares == 0 || totalAlps == 0) {
            _mint(msg.sender, _amount);
        }
        // Calculate and mint the amount of pALPS the ALPS is worth.
        // The ratio will change overtime, as pALPS is burned/minted and ALPS
        // deposited + gained from fees / withdrawn.
        else {
            uint256 what = (_amount * totalShares) / totalAlps;
            _mint(msg.sender, what);
        }
        //track deposited ALPS
        deposits[msg.sender] = deposits[msg.sender] + _amount;
        totalDeposits += _amount;
        // Lock the ALPS in the contract
        alps.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    // Leave the vault. Claim back your ALPS.
    // Unlocks the staked + gained ALPS and redistributes pALPS.
    function leave(uint256 _share) external {
        // Gets the amount of pALPS in existence
        uint256 totalShares = totalSupply();
        // Gets the ALPS balance of the nest
        uint256 totalAlps = alpsBalance();  
        // Calculates the amount of ALPS the pALPS is worth
        uint256 what = (_share * totalAlps) / totalShares;
        //burn pALPS
        _burn(msg.sender, _share);
        //send withdrawn ALPS
        uint256 tax = (what * paperHandsPenalty) / MAX_BIPS;
        uint256 toSend = what - tax;
        // Send the person's ALPS to their address
        alps.safeTransfer(msg.sender, toSend);
        //track withdrawn ALPS
    ...
    function _transfer(
        ...
        // Gets the ALPS balance of the nest
        uint256 totalAlps = alpsBalance();
        uint256 alpsValueOfShares = (amount * totalAlps) / totalShares;
        // take part of profit tracking
        deposits[recipient] += alpsValueOfShares;
        withdrawals[sender] += alpsValueOfShares;
    ...
    function _dailyUpdate() internal {
        if (timeSinceLastDailyUpdate() >= SECONDS_PER_DAY) {
            ...
            //update rolling data
            rollingStartTimestamp = block.timestamp;
            rollingStartAlpsBalance = alpsBalance();
