...
- ✔️ Supply Name: BLACKDOG.help
- ✔️ Symbol: BLACKDOG
- ✔️ Buy Tax per transaction: 3% (redistributes to all holders)
- ✔️ Sell Tax per transaction: 3% (redistributes to all holders)
...
- ✔️ Send to Dev Wallet per transaction: 1% (Sent on ETH)
- ✔️ Send to Charity Wallet per transaction: 6% (Sent on ETH)
- ✔️ Implemented Dynamic Sell Logic

    // ROUTER ADDRESSES 
    //For BSC_MAINNET: 0xc98AF38e5d5b31Ee93Dc984c45BEaa9Bbf511717
    //For BSC_TESTNET: 0xc98AF38e5d5b31Ee93Dc984c45BEaa9Bbf511717
*/
...
contract BLACKDOG is Context, IERC20, Ownable { // Nominal name
    using SafeMath for uint256;

    string private constant _name = unicode"🐕 BLACKDOG.help 🐕"; 
    ...
    uint256 public _taxFee = 7; // 7% redistribution to all holders
    ...
    uint256 public _appdevCutPct = 3; // 3% cut to Project 
    ...
    constructor(address payable teamFunds, address payable marketingFunds, address payable appdevFunds) {
    ...
    function sendETHToFee(uint256 beforeSplit) private {
        ...
        uint256 appdevCut = beforeSplit.div(_totalTeamFeesExcludingLiquidity).mul(_appdevCutPct);
    ...
    function addInitialLiquidity() external onlyOwner() {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xc98AF38e5d5b31Ee93Dc984c45BEaa9Bbf511717); //Nominal router.
    ...
    function setTeamCutFee (uint256 _teamCut) external onlyOwner() {
        ...
        _teamFee = _teamCut.add(_marketingCutPct).add(_appdevCutPct).add(_liquidityCutPct);
    ...
    function setMarketingCutFee (uint256 _marketingCut) external onlyOwner() {
        ...
        _teamFee = _teamCutPct.add(_marketingCut).add(_appdevCutPct).add(_liquidityCutPct);
        _storedTeamFee = _teamFee;
    }
    function setappdevCutFee (uint256 _appdevCut) external onlyOwner() {
        _appdevCutPct = _appdevCut;
        _teamFee = _teamCutPct.add(_marketingCutPct).add(_appdevCut).add(_liquidityCutPct);
        _storedTeamFee = _teamFee;
    }
    function setLiquidityCutFee (uint256 _liquidityCut) external onlyOwner() {
        _liquidityCutPct = _liquidityCut;
        _teamFee = _teamCutPct.add(_marketingCutPct).add(_appdevCutPct).add(_liquidityCut);
        _storedTeamFee = _teamFee;
    }
