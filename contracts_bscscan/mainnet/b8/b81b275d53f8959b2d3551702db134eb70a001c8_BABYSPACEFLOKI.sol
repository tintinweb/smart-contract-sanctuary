...
library SafeMath { // Old SafeMath is no longer needed after Solidity 0.8.0. See https://github.com/OpenZeppelin/openzeppelin-contracts/issues/2465
...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "BABYSPACEFLOKI";
    string internal constant SYMBOL = "$BSF";
    ...
    uint256 internal constant TOTAL_SUPPLY = 1000000000 * 10**6 * 10**9;
     ...
     */
    uint256 internal constant maxTransactionAmount = TOTAL_SUPPLY / 50 ; // 2% Max Transaction Fee
     ...
     * See: `Liquifier`
     */
    uint256 internal constant numberOfTokensToSwapToLiquidity = TOTAL_SUPPLY / 100 ; //Liquify is set to 1%
    ...
    address internal charityAddress = 0x000000000000000000000000000000000000dEaD;
    ...
    function _addFees() private {
        ...
        _addFee(FeeType.Liquidity, 100, address(this) );
...
abstract contract Liquifier is Ownable, Manageable {
    ...
    address public marketingAddress = 0x2663529e955972A22009DA9Ed13906b187e53042;
    ...
    function _swapAndLiquify(uint256 amount) private lockTheSwap {
        ...
        // split the contract balance into 1/4 tokens and 3/4 for bnb
        uint256 oneQuarter = amount.div(4);
        uint256 threeQuarters = amount.sub(oneQuarter);
        ...
        // swap tokens for ETH
        _swapTokensForEth(threeQuarters); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);

        uint256 oneQuarterAsETH = newBalance.div(3);

        // add liquidity to uniswap
        _addLiquidity(oneQuarter, oneQuarterAsETH);

        // send remaining ETH (5%) to marketing wallet
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        (uint256 tokenAmountSent, uint256 ethAmountSent, uint256 liquidity) = _router.addLiquidityETH{value: ethAmount}(
            ...
            // 0 = accept any amount (slippage is inevitable)
            0,
            // this is a centralized risk if the manager's account is ever compromised (see Certik SSL-04)
            manager(),
            block.timestamp
        );
    ...
    /**
    * @dev Sets the uniswapV2 pair (router & factory) for swapping and liquifying tokens (in case of an upgrade)
    */
...
contract BABYSPACEFLOKI is SafeToken{
