...
contract EatMyArse is Context, IBEP20, Ownable {
    ...
    string private constant _name     = "EatMyArse";
    string private constant _symbol   = "EMA";
    ...
    uint256 public _devFee = 200; // 2% of every transaction is sent to Dev wallet
    ...
    function deliver(uint256 tAmount) public {
        ...
        (uint256 rAmount,,) = _getRValues(tAmount, tFee, tLiquidity, tDev, currentRate);
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            ...
            (, uint256 tFee, uint256 tLiquidity, uint256 tDev) = _getTValues(tAmount);
            uint256 currentRate = _getRate();
            (, uint256 rTransferAmount,) = _getRValues(tAmount, tFee, tLiquidity, tDev, currentRate);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        uint256 currentRate = _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tDev, currentRate);

        _rOwned[sender]    = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);

        takeTransactionFee(address(this), tLiquidity, currentRate);
        takeTransactionFee(address(_DevWallet), tDev, currentRate);
