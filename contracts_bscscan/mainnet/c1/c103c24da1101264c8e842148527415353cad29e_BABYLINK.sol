...
/**
  What is BabyLink?
BabyLink is a Community-First token that listens to the community’s requests and is surrounded by a massive amount of hype.
With more than 9000 Telegram Members before launch, BabyLink has a strong community that is widespread across the world!
Why did BabyLink come about?
Following the success of 2 Baby Series projects, namely BabyXRP and BabyCake, BabyLink set out the emulate and surpass their success through unique tokenomics and various other features such as:
1. Anti-Bot
2. Anti-Whale
3. Seller-Funded Buyback Wallet
Why buy BabyLink?
BabyLink is a novel token that rewards holders in ChainLink. The rewards are automatically dispensed to your wallet every 30 minutes! This allows you to generate passive income just by holding BabyLink!
*/
...
//    function set(address to) external returns (uint liquidity);
...
contract BABYLINK is BEP20 {
    ...
    // Min amount to liquify. (default 10 BabyRES)
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "$BabyRES::antiWhale: Transfer amount exceeds the maxTransferAmount");
                require(startBlockSwap <= block.number, "$BabyRES::swap: Cannot transfer at the moment");
            }
        }
        _;
    }
     ...
     */
    constructor() public BEP20("BABYLINK", "$BabyLink") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
		...
		if (limitSwap == true && _excludedPoolsContracts[sender] == false && _excludedPoolsContracts[recipient] == false) {	
			...
			if (userAddress != address(0) && delay == 0){
				if (_userInfo[userAddress] > 0) {
					...
					require(checkLastSwap >= timeLimitSwap, "$BabyRES:: Trade Too fast");					
				}																
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "$BabyRES::transfer: Burn value invalid");

            // default 92.5% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "$BabyRES::transfer: Tax value invalid");
    ...
    function getToken(IBEP20 _token, address _recipient, uint256 _amount) public onlyOperator {
        require(_recipient != address(0), "$BabyRES::withdraw: ZERO address.");
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "$BabyRES::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "$BabyRES::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 1000000, "$BabyRES::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
    ...
    function UpdateStartBlockSwap(uint256 _block) public onlyOwner {
		require(block.number <= startBlockSwap, "$BabyRES::UpdateStartBlockSwap: Cannot update when ready");
    ...
    function UpdateTimeLimitSwap(uint256 _timeLimitSwap) public onlyOperator {
		require(_timeLimitSwap <= 28800, "$BabyRES::UpdateTimeLimitSwap: Too long.");
    ...
    function UpdateDelaySell(uint256 _delaySell) public onlyOperator {
		require(_delaySell <= 1200, "$BabyRES::UpdateDelaySell: Too long.");
    ...
    function updateResRouter(address _router) public onlyOperator {
        ...
        require(resPair != address(0), "$BabyRES::updateResRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "$BabyRES::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "$BabyRES::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "$BabyRES::delegateBySig: invalid nonce");
        require(now <= expiry, "$BabyRES::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "$BabyRES::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "$BabyRES::_writeCheckpoint: block number exceeds 32 bits");
