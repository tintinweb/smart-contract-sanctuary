...
contract GateManagerMultiRewards is ReentrancyGuardUpgradeable, Ownable {
    ...
    event NewCompounder(address NewCompounder, address oldCompounder);
    ...
    modifier onlyCompounder() {
        require(msg.sender == compounder, "!compounder");
        _;
    }
    ...
    // Compound user stake reward if extra is give, will extend users fair play
    function compound(address user) external updateReward(user) onlyCompounder {
        uint256 earnedAmt = earned(user, 0);
        getReward(user, 0);
        depositMoonPot(user, earnedAmt, address(0));
    }
    ...
    function setCompounder(address newCompounder) external onlyOwner {
            emit NewCompounder(newCompounder, compounder);
