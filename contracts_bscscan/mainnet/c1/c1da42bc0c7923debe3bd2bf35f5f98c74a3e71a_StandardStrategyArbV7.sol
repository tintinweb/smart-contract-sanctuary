...
// File: contracts/strategies/StandardStrategyArbV7.sol
...
contract StandardStrategyArbV7 is Ownable {
    using SafeMath for uint256;
    ...
    address constant STABLE_QUANT_EXCHANGE = address(0x3ED4b2070E3DB5eF5092F504145FB8150CfFE5Ea);
    ...
    uint256 constant EXCHANGE_COUNT = 9;
    ...
    function setupWithdrawTokens() internal {
        ...
        tokenList.push(
            TokenInfo({
                token: _token,
                decimals: _token.decimals(),
                nerveID: 0, // DAI not on nerve
                quantID: 0, // DAI not on quant
                acryptoID: 2,
    ...
    function simulateExchange(uint256 _idIn, uint256 _idOut, uint256 _amount, uint256 _exchangeNum) internal view returns (uint256) {
        if(_idOut == WBNB_TOKEN_ID){
            ...
            }else if(_exchangeNum == 8){
                // We use stablequant which uses a form of Curve
                // Quant doesn't support DAI
                if(_idIn == 1 || _idOut == 1){return 0;}
                ValueLikeExchange router = ValueLikeExchange(STABLE_QUANT_EXCHANGE);
                return router.calculateSwap(tokenList[_idIn].quantID, tokenList[_idOut].quantID, _amount);
            }
        }            
    }
    ...
    function exchange(uint256 _idIn, uint256 _idOut, uint256 _amount, uint256 _exchangeNum) internal {
        if(_idOut == WBNB_TOKEN_ID){
            ...
            if(_exchangeNum == 0){
                ...
                if(_idIn == 1 || _idOut == 1){return;}
                ValueLikeExchange router = ValueLikeExchange(STABLE_QUANT_EXCHANGE);
                tokenList[_idIn].token.safeApprove(STABLE_QUANT_EXCHANGE, 0);
                tokenList[_idIn].token.safeApprove(STABLE_QUANT_EXCHANGE, _amount);
                router.swap(tokenList[_idIn].quantID, tokenList[_idOut].quantID, _amount, 1, now.add(60));
                return;
            }
        }
    }
