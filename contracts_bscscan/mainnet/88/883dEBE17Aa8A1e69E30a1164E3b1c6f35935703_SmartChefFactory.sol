...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 savedPendingReward;
        uint256 totalPendingReward;
        uint256 currentPendingReward;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        require(isContractNotPaused()
        , 'Reward per block has just updated, please wait until admin distributes all the reward.');
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                // }
                // require(block.number >= localAddressEndLockTime || block.number >= bonusEndBlock, "Lock still in affect, or pool hasn't ended yet.");
            }
        }
    ...
    function updateRewardPerBlock(uint256 _rewardPerBlock) external onlyOwner {
        require(
            isContractNotPaused(),
            'Unable to execute this function, please execute updateSavedPendingReward and updateCurrentAndTotalPendingReward functions first.'
        );

        for (uint i = 0; i < userArr.length; i++) {
            temporaryPendingReward[userArr[i]] = pendingReward(userArr[i]);
        }

        rewardPerBlock = _rewardPerBlock;

        hasAllRewardDistributedByAdmin = false;
        hasSavedPendingRewardUpdatedByAdmin = false;

        numberOfClaimSavedPendingReward = 0;
        numberOfClaimCurrentAndTotalPendingReward = 0;

        emit NewRewardPerBlock(_rewardPerBlock);
    }

    function updateSavedPendingReward(uint256 _num) external onlyOwner {
        require(_num > 0, 'The no should be greater than 0');
        require(
            !hasAllRewardDistributedByAdmin && !hasSavedPendingRewardUpdatedByAdmin,
            'Unable to execute this function, please execute updateRewardPerBlock or updateCurrentAndTotalPendingReward function'
        );

        uint _numberOfClaim = numberOfClaimSavedPendingReward;

        for (;
            _numberOfClaim < (numberOfClaimSavedPendingReward.add(_num)) && _numberOfClaim < userArr.length;
            _numberOfClaim++ // Not using add of safemath to reduce gas
        ) {
            UserInfo storage user = userInfo[userArr[_numberOfClaim]];
            user.savedPendingReward = user.totalPendingReward;
        }

        if (_numberOfClaim == userArr.length) {
            hasSavedPendingRewardUpdatedByAdmin = true;
            hasAllRewardDistributedByAdmin = false;
        }

        numberOfClaimSavedPendingReward = _numberOfClaim;
    }

    function updateCurrentAndTotalPendingReward(uint256 _num) external onlyOwner {
        require(_num > 0, 'The no should be greater than 0');
        require(
            !hasAllRewardDistributedByAdmin && hasSavedPendingRewardUpdatedByAdmin,
            'Unable to execute this function, please execute updateSavedPendingReward function first'
        );

        uint _numberOfClaim = numberOfClaimCurrentAndTotalPendingReward;

        for (;
            _numberOfClaim < (numberOfClaimCurrentAndTotalPendingReward.add(_num)) && _numberOfClaim < userArr.length;
            _numberOfClaim++ // Not using add of safemath to reduce gas
        ) {
            UserInfo storage user = userInfo[userArr[_numberOfClaim]];
            user.currentPendingReward = temporaryPendingReward[userArr[_numberOfClaim]];
            user.totalPendingReward = user.currentPendingReward.add(user.savedPendingReward);
        }

        if (_numberOfClaim == userArr.length) {
            hasSavedPendingRewardUpdatedByAdmin = true;
            hasAllRewardDistributedByAdmin = true;
        }

        numberOfClaimCurrentAndTotalPendingReward = _numberOfClaim;
    }
    ...
    function pendingReward(address _user) public view returns (uint256) {
        if(isContractNotPaused()){
            ...
            return temporaryPendingReward[_user];
        }
    }
    ...
    function isContractNotPaused() public view returns (bool) {
        return hasSavedPendingRewardUpdatedByAdmin && hasAllRewardDistributedByAdmin;
    }
    ...
    function _claimReward(address _user) internal {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                if (user.totalPendingReward > 0) {
                    rewardToken.safeTransfer(address(_user), pending.add(user.totalPendingReward));
                } else {
