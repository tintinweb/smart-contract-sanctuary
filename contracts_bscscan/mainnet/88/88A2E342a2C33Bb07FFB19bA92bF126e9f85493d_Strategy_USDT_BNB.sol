...
contract Strategy_USDT_BNB {
    ...
    function _getLiquidityAmounts(
        ...
        // pay attention!
        // amountADesired - for the token0
        // amountBDesired - for the token1
    ...
    function sendFundsOwner() public onlyVault {
        payable(vault).transfer(address(this).balance);
        _transfer(token0, vault, IERC20(token0).balanceOf(address(this)));
        _transfer(token1, vault, IERC20(token1).balanceOf(address(this)));
    }
}