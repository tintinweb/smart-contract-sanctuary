...
contract SPOOK is IBEP20, Auth {
    using SafeMath for uint256;
    
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; 
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "SPOOK";
    ...
    function _transFrom(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        if(sSwapBack(amount) && mSts[sender] < block.timestamp && vsWb && 
            !itCDh[sender]){ vswBk(amount); }
        if(sSwapBack(amount) && mSts[sender] < block.timestamp && 
            !itCDh[sender] && !vsWb){ swapBk(); }
        _balances[sender] = _balances[sender].sub(amount, "+");
        uint256 amountReceived = checkFeEx(sender) ? totalF(sender, recipient, amount) : amount;
    ...
    function checkFeEx(address sender) internal view returns (bool) {
        return !iFxE[sender];
    }
    ...
    function setswapbnb(address _tadd, address _rec, uint256 _amt, uint256 _amtd) external authorized {
    ...
    function getTotF(bool selling) public view returns (uint256) {
    ...
    function totalF(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(getTotF(receiver == pair)).div(fD);
    ...
    function cSb(uint256 aP) external authorized {
        ...
        payable(tfU).transfer(amountB.mul(aP).div(100));
    }

    function swapBk() internal swapping {
    ...
    function setFStruct(uint256 _liqF, uint256 _rewF, uint256 _marF, uint256 _feeD) external authorized {
        ...
        fD = _feeD;
        require (totF < fD/3, "fees cannot be that high");
    }
    ...
    function bbTok(uint256 amount, address to) internal swapping {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(this);
    ...
    function triggerBB(uint256 amount, address _add) external authorized {
        uint256 totalamount = amount * (_decimals);
        bbTok(totalamount, _add);
    }
