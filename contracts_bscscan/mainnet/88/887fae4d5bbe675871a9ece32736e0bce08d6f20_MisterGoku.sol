...
contract MisterGoku is ERC20, Ownable {
    ...
    uint256 public _maxWalletToken = 20000000000 * (10**18); // 2% of total supply
    ...
    constructor() public ERC20("Mister Goku", "MisterGoku") {
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MisterGoku: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MisterGoku: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MisterGoku: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "MisterGoku: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MisterGoku: gasForProcessing must be between 200,000 and 500,000");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "MisterGoku: This account cannot send tokens until trading is enabled");
        }
...
contract MisterGokuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("MisterGoku_Dividend_Tracker", "MisterGoku_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "MisterGoku_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "MisterGoku_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MisterGoku contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MisterGoku_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MisterGoku_Dividend_Tracker: Cannot update claimWait to same value");
