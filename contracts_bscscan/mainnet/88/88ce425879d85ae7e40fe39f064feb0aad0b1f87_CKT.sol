...
// File: contracts/CKTDividendPayingToken.sol
pragma solidity ^0.7.6;
...
// File: contracts/CKT.sol
pragma solidity ^0.7.6;
contract CKT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CKTDividendTracker public dividendTracker;
    ...
    uint256 public swapTokensAtAmount = 650000000 * (10**18);
    ...
    address public insuranceTokenAddressForFee;
    // transfer fees
    ...
    uint256 public totalFees = tokenRewardsFee.add(liquidityFee).add(marketFee).add(charityFee).add(insuranceFee).add(burnFee);
    // buy fees
    uint256 public buyMarketFee = 1;
    uint256 public buyCharityFee = 1;
    uint256 public buyInsuranceFee = 1;
    uint256 public buyTokenRewardsFee = 1;
    uint256 public buyLiquidityFee = 1;
    uint256 public buyBurnFee = 1;
    uint256 public buyTotalFees = buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketFee).add(buyCharityFee).add(buyInsuranceFee).add(buyBurnFee);
    // sell fees
    uint256 public sellMarketFee = 1;
    uint256 public sellCharityFee = 1;
    uint256 public sellInsuranceFee = 2;
    uint256 public sellTokenRewardsFee = 4;
    uint256 public sellLiquidityFee = 1;
    uint256 public sellBurnFee = 1;
    uint256 public sellTotalFees = sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketFee).add(sellCharityFee).add(sellInsuranceFee).add(sellBurnFee);
    // fee tracker
    uint256 private previousMarketFee = marketFee;
    uint256 private previousCharityFee = charityFee;
    uint256 private previousInsuranceFee = insuranceFee;
    ...
    bool public isInsuranceActive = false;
    ...
    uint256 public tradingEnabledTimestamp = 1634188993;
    ...
    uint256 public _transferAmountLimit = 5000000000 * (10**18);
    uint256 public _transferOverLimitTax = 20;    
    ...
    constructor() public ERC20("CovidKillerToken", "CKT") {
        // LP address
        LPaddress = address(0x4748647956A7eF531e366AB7D0EA7658Ad1FCA8a);
        uint256 _processDividendTime = block.timestamp;
        processDividendTime = _processDividendTime;

        dividendTracker = new CKTDividendTracker();
    ...
    function restoreFees() internal {
            ...
            insuranceFee = previousInsuranceFee;
    ...
    function updateInsuranceMode(bool _isInsuranceActive) external onlyOwner {
        isInsuranceActive = _isInsuranceActive;
    }    
    ...
    function updateInsuranceAddress(address payable newAddress) external onlyOwner {
        insuranceAddress = newAddress;
        _isExcludedFromFees[newAddress] = true;
        //dividendTracker.excludeFromDividends(insuranceAddress);
    }    
    ...
    function updateInsuranceTokenFeeAddress(address newAddress) external onlyOwner {
        insuranceTokenAddressForFee = newAddress;
    }
    
    function burnTokens(uint256 amountNoDecimals) external onlyOwner {
        uint256 sendAmount = amountNoDecimals * (10**decimals());
        super._transfer(address(this), LPaddress, sendAmount);
    }
    function updateFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _charityFee, uint256 _insuranceFee, uint256 _burnFee) external onlyOwner {
        ...
        burnFee = _burnFee;
        totalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_charityFee).add(_insuranceFee).add(_burnFee);
        previousMarketFee = _marketFee;
        previousCharityFee = _charityFee;
        previousInsuranceFee = _insuranceFee;
        previousTokenRewardsFee = _tokenRewardsFee;
        previousLiquidityFee = _liquidityFee;
        previousBurnFee = _burnFee;
        previousTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_charityFee).add(_insuranceFee).add(_burnFee);
    }    
    
    function updateBuyFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _charityFee, uint256 _insuranceFee, uint256 _burnFee) external onlyOwner {
        ...
        buyBurnFee = _burnFee;
        buyTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_charityFee).add(_insuranceFee).add(_burnFee);
    }    
    
    function updateSellFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _charityFee, uint256 _insuranceFee, uint256 _burnFee) external onlyOwner {
        ...
        sellTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_charityFee).add(_insuranceFee).add(_burnFee);
    }
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "CKT: The dividend tracker already has that address");

        CKTDividendTracker newDividendTracker = CKTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CKT: The new dividend tracker must be owned by the CKT token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "CKT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "CKT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "CKT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CKT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CKT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CKT: Cannot update gasForProcessing to same value");
    ...
    function swapAndLiquify(uint256 tokens) private  {
        ...
        if(insuranceFee != 0){
            insuranceFeeFull = tokens.mul(insuranceFee).div(100);
        }

        uint256 tokensAfterFee = tokens - marketFeeFull - charityFeeFull - insuranceFeeFull;
        ...
        if(insuranceFee != 0){
            insuranceFeeBalance = newBalance.mul(insuranceFee).div(100);
        }
        
        uint256 finalBalance = newBalance - marketFeeBalance - charityFeeBalance - insuranceFeeBalance;
        ...
        if(isInsuranceActive){
            swapEthForTokens(insuranceFeeBalance, address(this), insuranceAddress);
        }else{
            (bool sent,) = insuranceAddress.call{value: insuranceFeeBalance}("");
        }}
    ...
    function _transfer(
        ...
        if (
            ...
            previousInsuranceFee = insuranceFee;
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "CKT: This account cannot send tokens until trading is enabled");
        }
        ...
        if(takeFee || buyOrSell ) {
            ...
            super._transfer(from, LPaddress, burnAmount);
            }
        }else{
            if(_isExcludedFromFees[from] || _isExcludedFromFees[to]){
...
contract CKTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("CKT_Dividend_Tracker", "CKT_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "CKT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "CKT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CKT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CKT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CKT_Dividend_Tracker: Cannot update claimWait to same value");
