...
contract TestCake is IBEP20, Auth {
    ...
    string constant _name = "TestCake";
    string constant _symbol = "TestCake";
    ...
    uint256 reflectionFee = 12000000000;
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        if(block.number > launchedAt + 100){require(amount <= _maxTxAmount || isTxLimitExempt[sender], "TX Limit Exceeded");}
    }
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        if(launchedAt + 100 >= block.number){ return feeDenominator.sub(1); }
        if(selling && buybackMultiplierTriggeredAt.add(buybackMultiplierLength) > block.timestamp){ return getMultipliedFee(); }
        if(selling) { return totalFee.add(totalFee); }
        return totalFee;
    }
