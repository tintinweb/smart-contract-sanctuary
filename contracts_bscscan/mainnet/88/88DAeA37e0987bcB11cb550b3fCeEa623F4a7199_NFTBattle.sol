...
contract NFTBattle is Context, Ownable, ReentrancyGuard {
    ...
    function startBattle(
		...
		if (IERC165(attackerNFT).supportsInterface(type(IERC1155).interfaceId)) {
        	require(
        	    IERC1155(attackerNFT).balanceOf(_msgSender(), attackerID) > 0,
        	    "Attacker does not own NFT"
        	);
		} else if (IERC165(attackerNFT).supportsInterface(type(IERC721).interfaceId)) {
        	require(
        	    IERC721(attackerNFT).ownerOf(attackerID) == _msgSender(),
        	    "Attacker does not own NFT"
        	);
		} else {
			revert("Unrecognized token standard: attacker");
		}
		if (IERC165(defenderNFT).supportsInterface(type(IERC1155).interfaceId)) {
        	require(
        	    IERC1155(defenderNFT).balanceOf(defender, defenderID) > 0,
        	    "Defender does not own NFT"
        	);
		} else if (IERC165(defenderNFT).supportsInterface(type(IERC721).interfaceId)) {
        	require(
        	    IERC721(defenderNFT).ownerOf(defenderID) == defender,
        	    "Defender does not own NFT"
        	);
		} else {
        ...
        battle.nft1 = attackerID;
        battle.addr2 = defender;
        battle.nft2 = defenderID;

        emit StartBattle(totalBattles, _msgSender(), attackerID, defender, defenderID);
    }
	...
	function setNextBattleId(uint256 id) public onlyOwner {
		totalBattles = id;
	}
