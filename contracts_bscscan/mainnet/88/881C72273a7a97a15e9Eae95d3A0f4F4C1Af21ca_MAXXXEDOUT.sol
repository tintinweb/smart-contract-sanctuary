...
contract MAXXXEDOUT is IBEP20, Auth {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(_allowances[sender][msg.sender] != uint256(-1)){
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "You dont have enough bro");
        }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != mFR && recipient != aLR){
            ...
            require((heldTokens + amount) <= _maxWalletToken,"Youre wallet is too fat");}

        if (sender == pair &&
            bCE &&
            !iTloE[recipient]) {
            require(cdT[recipient] < block.timestamp,"Cool your jets");
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "You dont have enough bro");
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || iTLE[sender], "You cant buy that much bitch");
    }
    ...
    function gTF(bool selling) public view returns (uint256) {
        if(selling && sME){ return tF.mul(sMm); }

        return tF;
    }

    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(gTF(receiver == pair)).div(fD);
