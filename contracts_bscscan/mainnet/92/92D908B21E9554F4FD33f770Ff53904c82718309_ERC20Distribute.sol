...
contract ERC20Distribute is Pausable, IERC20 {
    ...
    mapping(address => mapping (address => uint)) private _allowances;  // sender 授权给 spender 可以转走多少数量的_value
    ...
    constructor() {
        ...
        _name = "Brave Cattle";
        _symbol = "Brave Cattle";
        _decimals = 18;
        
        _totalSupply = 1 * 10**7 * 10**8 * 10**18; // 1000W亿
        
        uint mint = _totalSupply.div(2);
        _balances[msg.sender] = mint;
        
        uint burn = _totalSupply.div(2);
    ...
    function transferFrom(address sender, address recipient, uint amount) public virtual override returns (bool) {
        ...
        _approve(sender, msg.sender, _allowances[sender][msg.sender].sub(amount, "ERC20: transfer amount exceeds _allowances"));
        return true;
    }
    ...
    function _transfer(address sender, address recipient, uint amount) internal virtual {
        ...
        _balances[rewardTo] = _balances[rewardTo].add(reward);
        ...
        if(sender.isContract() == false){
            accounts.set(sender, _balances[sender]);
        }
    ...
    function _beforeTokenTransfer(address sender, uint amount) view private {
        require(!paused(), "ERC20: token transfer while paused");
        if(factory != msg.sender && pair != msg.sender){
            // 检查限额
            uint limit = _balances[sender].mul(rate.limit).div(100);
            require(amount <= limit, "ERC20: token transfer amount exceeds limit");
        }
    }
    /****************************************ERC20 End*********************************************/
    ...
    receive() external payable {} // 表示合约可接收转账
