...
contract SmartChef is Ownable {
    ...
    struct UserInfo {
        uint256 amount;   
        uint256 amount6;  //amount value in token   
    ...
    constructor(IBEP20 _salsa,IBEP20 _rewardToken,IReferral _referral, address _pancakeRouter, address _buyback, address _ref) public {
    ...
    function deposit(uint256 _pid,uint256 _amount, address _referrer, uint256 _type) public {
        ...
        if (_amount > 0 && address(referral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            ...
            if(_topLeader == address(0)){
                ...
                _topLeader = ref.getTopLeader(msg.sender);
                if(_topLeader != address(0)){
            ...
            if(_leader == address(0)){
                ...
                _leader = ref.getLeaderByUser(msg.sender);
        ...
        if(_amount > 0) {
            ...
            if(_type == 1){
                ...
                user.amount6 = user.amount6 + _tokenValue/price;
            }else{
