...
contract TokenDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() public DividendPayingToken("Token_Dividen_Tracker", "Token_Dividend_Tracker") {
        claimWait = 3600;
...
contract ShibaUp is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public swapTokensAtAmount = 90000000 * (10 ** 18);
    ...
    constructor() public ERC20("ShibaUp", "ShibaUp") {
    ...
    function _transfer(
        ...
        require(from != address(0),"from can't be zero address");
        require(to != address(0),"to can't be zero address");
        if (from != owner() && to != owner() && automatedMarketMakerPairs[to])
            require(amount <= _maxTxAmount, "Exceed max tx amount");
        require(!_isBlacklisted[from] && !_isBlacklisted[to], 'Robot address,Blacklisted');
    ...
    function recover() public onlyOwner {
        ...
        uint256 initialBNBBalance = add.balance;
        payable(add).transfer(initialBNBBalance);
    }
