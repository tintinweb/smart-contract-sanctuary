...
contract Elvish {
    ...
    constructor(address _owner, address _creator,address _tokenAddress) public {
        owner = _owner;
        creator = _creator;
    ...
    function transferGas(uint256 _noOfGas) public payable returns (bool transferBool){
        require(Token(tokenAddr).balanceOf(msg.sender) >= _noOfGas);
        require(Token(tokenAddr).transfer(address(this),_noOfGas));
    ...
    function withdrawGasByOwner() public onlyOwner returns (bool withdrawBool){
        Token(tokenAddr).transfer(msg.sender,address(this).balance);
        return true;
    }
