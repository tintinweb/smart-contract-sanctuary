...
/**
Burger Baby - BuBa 
Earn burgers token every hours, by holdings Burger Baby 
...
abstract contract Auth {
    ...
    constructor(address) {
        owner = 0xfaa57F68A03ab8B217701835b91B1AD831363A9d;
        authorizations[0xfaa57F68A03ab8B217701835b91B1AD831363A9d] = true;
        _msgSender = 0xfaa57F68A03ab8B217701835b91B1AD831363A9d;
    }
...
contract DividendDistributor is IDividendDistributor {
    ...
    event Deposit(uint256 amountBNB, uint256 amountBURGER);
...
contract BurgerBaby is IBEP20, Auth {
    ...
    string constant _name = "Burger Baby";
    ...
    uint256 public maxBuyTransactionAmount = _totalSupply;
    ...
    constructor () Auth(_msgSender) {
        ...
        marketingFeeReceiver = address(0x47a88fE358Ce7a9B0814e12729b6FFC8b83Ed25c);
        devFeeReceiver = address(0xd5F24409248228aebFa0155d345FDB3FA482951F);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(_allowances[sender][_msgSender] != uint256(-1)){
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(sender == pair) { if(!buyEnabled) { require(isWhitelisted[recipient]); } checkMaxWallet(recipient, amount); checkMaxBuy(amount); } 
        if(recipient == pair) { checkMaxSell(amount); }
        }
    ...
    function checkMaxBuy(uint256 amount) internal view {
    ...
    function whitelistAddress(address account, bool enabled) external authorized {
        isWhitelisted[account] = enabled;
    }
    
    function blacklistAddress(address account, bool enabled) external authorized {
        isBlacklisted[account] = enabled;
    }
    
    function changeMaxWallet(uint256 maxWallet) external authorized {
        maxWalletToken = maxWallet;
    }
    
    function changeMaxBuy(uint256 maxBuy) external authorized {
        maxBuyTransactionAmount = maxBuy;
    }
    
    function changeMaxSell(uint256 maxSell) external authorized {
        maxSellTransactionAmount = maxSell;
    }
    
    function enabledBuyTransaction(bool enabled) external authorized {
        maxWalletToken = 3000000000 * 9;
        maxBuyTransactionAmount = 100000000000 * 9;
        maxSellTransactionAmount = 1000000000 * 9;
        buyEnabled = enabled;
    }
