...
contract VenusPool is BasePool {
    ...
    constructor( IBEP20 assetToken,
        ...
        {
        _venus = venus;
        require(assetToken.approve(address(_venus), MAX_INT), 'constructor: fail approve');
        uint256 decimals =10**(18-assetToken.decimals());
        setDecimalAdjustment(decimals * DEFAULT_BASE);
    }
    ...
    function _celebrityClaimAsset(address celebrity, CelebrityInfo memory celebrityInfo)  internal virtual override {
        ...
        if (venusClaim.claimAmount > 0)
        {
            ...
            if(balance > systemReward)
                require(_assetToken.transfer(celebrity, balance - systemReward), '_celebrityClaimAsset: fail transfer platform fee');
            if(systemReward > 0)
                require(_assetToken.transfer(_platformFeeAddress, systemReward), '_celebrityClaimAsset: fail transfer celebrity reward');
        }
    }
    ...
    function _unstakeAsset(address celebrity, StakerInfo memory stakerInfo, CelebrityInfo memory celebrityInfo, uint256 amount) internal virtual override {
        ...
        if(balance > 0)
            require(_assetToken.transfer(msg.sender, balance), '_unstakeAsset: fail transfer asset');
        //-------------venus unstake--------------
