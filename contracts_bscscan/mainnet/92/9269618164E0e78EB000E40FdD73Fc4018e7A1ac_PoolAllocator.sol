...
contract PoolAllocator is Auth, ReentrancyGuard {
    ...
    uint marketingFee = 1000; // This contract now handles the marketing fee, which is 1% of the allocated to the RBLOs. (So now MKT fee is lowered)
    uint allocatorIncentive = 1000;
    ...
    event Distribute(uint indexed amt, uint indexed incentive);
