...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BLNB
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accNativeTokenPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accNativeTokenPerShare` (and `lastRewardTimestamp`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. BLNB to distribute per second.
        uint256 lastRewardTimestamp;  // Last timestamp that BLNB distribution occurs.
        uint256 accNativeTokenPerShare;   // Accumulated BLNB per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    ERC20PresetMinterPauser public nativeToken;
    address public devaddr;

    uint256 public nativeTokenPerSec; // BLNB tokens created per second.
    uint256 public constant BONUS_MULTIPLIER = 1; // Bonus multiplier for early BLNB makers.
    ...
    // The timestamp when NativeToken mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 nativeTokenPerSec);
    ...
    constructor(
        ERC20PresetMinterPauser _nativeToken,
        address _devaddr,
        address _feeAddress,
        uint256 _nativeTokenPerSec,
        uint256 _startTimestamp
    ) {
        nativeToken = _nativeToken;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        nativeTokenPerSec = _nativeTokenPerSec;
    ...
    // Update the given pool's BLNB allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending BLNB on frontend.
    function pendingNativeToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp  > pool.lastRewardTimestamp  && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 nativeTokenReward = multiplier.mul(nativeTokenPerSec).mul(pool.allocPoint).div(totalAllocPoint);
            accNativeTokenPerShare = accNativeTokenPerShare.add(nativeTokenReward.mul(1e12).div(pool.lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accNativeTokenPerShare = pool.accNativeTokenPerShare.add(nativeTokenReward.mul(1e12).div(pool.lpSupply));
        pool.lastRewardTimestamp = block.timestamp;
    }

    // Deposit LP tokens to MasterChef for BLNB allocation.
    ...
    // Safe nativeToken transfer function, just in case if rounding error causes pool to not have enough BLNB.
    function safeNativeTokenTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > nativeTokenBal) {
            transferSuccess = nativeToken.transfer(_to, nativeTokenBal);
        } else {
            transferSuccess = nativeToken.transfer(_to, _amount);
        }
        require(transferSuccess, "safeNativeTokenTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _nativeTokenPerSec) external onlyOwner {
        massUpdatePools();
        nativeTokenPerSec = _nativeTokenPerSec;
        emit UpdateEmissionRate(msg.sender, _nativeTokenPerSec);
    }
