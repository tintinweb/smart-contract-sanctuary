...
contract TombTokenTest is Context, IERC20, Ownable {
    ...
    string private                          _name = "TombFIN";
    string private                        _symbol = "TOMBFIN";
    uint public                      burialFactor = 10;

    uint256 public        totalEligibleToBeBuried = 0;
    uint256 public                    totalBuried = 0;
    ...
    mapping(address => bool) activeBurials;
    ...
    event WalletExcludedFromFees(address wallet);
    event WalletIncludedInFees(address wallet);
    event walletExcludedFromTXNLimit(address wallet);
    event walletIncludedInTXNLimit(address wallet);
    event walletExcludedFromWalletLimit(address wallet);
    event walletIncludedInWalletLimit(address wallet);
    event walletExcludedFromAllLimitations(address wallet);
    event walletIncludedInAllLimitations(address wallet);
    ...
    event CollectionsLiquificationEnabled();
    event CollectionsLiquificationDisabled();
    ...
    event TOMBReserveAdded(uint256 amount);

    event Burial(address token, uint256 buriedAmount, uint256 tombAmount);
    ...
    event AllowToTransfer(address wallet);
    ...
    constructor () {
        ...
        emit TOMBReserveAdded(reservedTokenBalance);
    ...
    function registerEconoMEMEToken(address _token) external onlyOwner {
        isEconoMEMEToken[_token] = true;
        IERC20 token = IERC20(_token);

        totalEligibleToBeBuried += token.totalSupply();
        emit EconoMEMETokenRegistered(_token);
    }

    function deregisterEconoMEMEToken(address token) external onlyOwner {
        require(!activeBurials[token], "token has already had burials");
        isEconoMEMEToken[token] = false;
        emit EconoMEMETokenDeRegistered(token);
    }

    function tokenInEconoMEME(address token) external view returns (bool) {
    ...
    function buryTokens(address _token, uint256 _amount) external {
        ...
        uint256 tombAmount = _amount.div(burialFactor);
        _transferExactTokens(address(this), msg.sender, tombAmount);
        reservedTokenBalance = reservedTokenBalance.sub(_amount);
        totalBuried += _amount;
        activeBurials[_token] = true;
        emit Burial(_token, _amount, tombAmount);
    }

    function enableCollectionsLiquification () external onlyOwner {
        liquifyCollections = true;
        emit CollectionsLiquificationEnabled();
    }

    function disableCollectionsLiquification () external onlyOwner {
        liquifyCollections = false;
        emit CollectionsLiquificationDisabled();
    }
