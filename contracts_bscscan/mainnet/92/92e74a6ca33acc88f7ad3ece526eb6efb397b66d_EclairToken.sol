...
// File: contracts\EclairToken.sol
...
// EclairToken with Governance.
contract EclairToken is BEP20 {
    ...
    // Min amount to liquify. (default 500 ECLAIRs)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable.
    IUniswapV2Router02 public eclairSwapRouter;
    // The trading pair
    address public eclairSwapPair;
    // In swap and liquify
    ...
    /**
     * @notice Constructs the EclairToken contract.
     */
    constructor() public BEP20("ECLAIR", "ECLAIR") {
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the eclairSwap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = eclairSwapRouter.WETH();

        _approve(address(this), address(eclairSwapRouter), tokenAmount);

        // make the swap
        eclairSwapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        eclairSwapRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from eclairSwapRouter when swapping
