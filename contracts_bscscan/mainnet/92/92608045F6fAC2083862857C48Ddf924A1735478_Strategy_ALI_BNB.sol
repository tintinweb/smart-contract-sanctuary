...
contract Strategy_ALI_BNB is Ownable {
    ...
    uint256 private amountPerToken = 1;
    ...
    address public routerLP = 0x730aCC3bBf2443f2EaEaCFc7ac7b4d8DC9E32dB8;
    ...
    function _swapWBNBToToken(address _router, address _token) internal {
        ...
        uint256 amount = _getAmountsOut(_router, _balanceWBNB, path);
        if(amount > 100) {
            _swapExactTokensForTokens(_router, _balanceWBNB, amount, path);  
        }
    }
    ...
    function _getTokenPrice() internal view returns (uint256) {
        ...
        path[0] = token1; // wbnb
        path[1] = token0; // ali
        return 1e36 / (_getAmountsOut(routerLP, amountPerToken * 1e18, path));
    }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            if (t0V != 0) {
                path[0] = token0;
                TC = _getAmountsOut(routerLP, t0V, path) + t1V;
            }
        }
