...
contract ZapUniswapV2 {
    ...
    uint256 public constant minimumAmount = 1;
    ...
    function zapIn(
        ...
        require(value >= minimumAmount, 'BiShares: Insignificant input amount');
    ...
    function _getVaultPair(
        ...
        vault_ = IVault(vault);
        try vault_.want() returns (IERC20 pairAddress) {
    ...
    function _checkPairReserves(
        ...
        address tokenA,
        address tokenB
    ) private view returns (bool success, uint112 reserve0, uint112 reserve1) {
        (address token0, ) = UniswapV2Library.sortTokens(tokenA, tokenB);
        address pair = UniswapV2Library.pairFor(factory, tokenA, tokenB);
        if (pair != address(0)) {
            (reserve0, reserve1, ) = IUniswapV2Pair(pair).getReserves();
            success = reserve0 > minimumAmount && reserve1 > minimumAmount;
            (reserve0, reserve1) = tokenA == token0
                ? (reserve0, reserve1)
                : (reserve1, reserve0);
        }
    }

    function _calculateNonLPTokenSwapAmountOut(
        IUniswapV2Router02 router,
        address tokenIn,
        address token0,
        address token1,
        ...
        tokenOut = token0;
        (bool success, uint112 reserve0, uint112 reserve1) = _checkPairReserves(factory, tokenIn, tokenOut);
        if (!success) {
            tokenOut = token1;
            (success, reserve0, reserve1) = _checkPairReserves(factory, tokenIn, tokenOut); 
        }
        require(success, "BiShares: Invalid tokenIn");
        amountOut = UniswapV2Library.getAmountOut(tokenInAmount, reserve0, reserve1);
    }
    ...
    function _swapNonLPToken(
        ...
        (address tokenOut, uint256 amountOut) = _calculateNonLPTokenSwapAmountOut(
            router,
            tokenIn,
            token0,
            token1,
        ...
        for (uint256 i; i < tokens.length; i++) {
            ...
            if (balance > 0) {
                if (tokens[i] == WETH) {
                    ...
                    (bool success,) = caller.call{value: balance}(new bytes(0));
                    require(success, 'BiShares: ETH transfer failed');
                } else {
    ...
    function zapInEstimate(
        ...
        (tokenIn, fullInvestment) = _calculateNonLPTokenSwapAmountOut(
            router_,
            tokenIn,
            pair.token0(),
            pair.token1(),
        ...
        uint256 tokenAmountOutMin = UniswapV2Library.getAmountOut(
            _getSwapAmount(
                router_,
                fullInvestment, 
                reserve0,
        ...
        uint256 amountLiquidity = Math.min(
            fullInvestment.sub(tokenAmountOutMin).mul(pairTotalSupply) / reserve0,
            tokenAmountOutMin.mul(pairTotalSupply) / reserve1
        );
        vaultAmount = amountLiquidity.mul(1e18).div(vault_.getPricePerFullShare());
    }
    ...
    function zapOutEstimate(
        ...
        address[] memory path = new address[](2);
        path[0] = success ? pair.token1() : pair.token0();
        path[1] = success ? pair.token0() : pair.token1();
        (uint256 reserve0, uint256 reserve1,) = pair.getReserves();
        uint256 pairTotalSupply = pair.totalSupply();
        uint256 liquidityAmount = withdrawAmount.mul(vault_.getPricePerFullShare()).div(1e18);
        uint256 amount0 = liquidityAmount.mul(
            IERC20(pair.token0()).balanceOf(address(pair))
        ).div(pairTotalSupply);
        uint256 amount1 = liquidityAmount.mul(
            IERC20(pair.token1()).balanceOf(address(pair))
        ).div(pairTotalSupply);

        uint256 minAmountOut = UniswapV2Library.getAmountOut(
            success ? amount1 : amount0,
            success ? reserve1 : reserve0,
            success ? reserve0 : reserve1
        );
        path[0] = path[1];
        path[1] = desiredToken;
        desiredTokenAmount = UniswapV2Library.getAmountsOut(
            router_.factory(),
            minAmountOut.add(success ? amount0 : amount1),
            path
        )[1];
    }
