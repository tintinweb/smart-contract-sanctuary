...
contract FTCake is ERC20, Ownable {
    ...
    // TOKEN
    string tokenName = "FairToken Cake";
    string tokenSymbol = "FRTC";
    uint256 tokenTotalSupply = 1_000_000_000 * (10**18);
    
    // ADRESSES
    address marketingWallet = 0x477fe4B365c481B507d37BA4877126F6526831B4;
    address devWallet = 0x477fe4B365c481B507d37BA4877126F6526831B4;
    ...
    bool public isTaxDisabled = false;
    ...
    function handleFees(address sender, address recipient, uint256 amount) internal returns (uint256 fee) {
        ...
        bool isTaxable = isExcluded[sender] || isExcluded[recipient];
        ...
        if(isTaxable) {
            ...
            if(isBuy) {
                ...
                devAmount = feeTokens * 10**18 / taxSum * devTaxBuy / 10**18;
                marketingAmount = feeTokens * 10**18 / taxSum * marketingTaxBuy / 10**18;
                redistributionAmount = feeTokens * 10**18 / taxSum * marketingTaxBuy / 10**18;
                amountSum = devAmount + marketingAmount + redistributionAmount;
            } else if(isSell) {
                fees = amount * 10**18 / denominator * (devTaxSell + marketingTaxSell + redistributeTaxSell) / 10**18;

                taxSum = devTaxBuy + marketingTaxBuy + redistributeTaxBuy;
                
                devAmount = feeTokens * 10**18 / taxSum * devTaxSell / 10**18;
                marketingAmount = feeTokens * 10**18 / taxSum * marketingTaxSell / 10**18;
                redistributionAmount = feeTokens * 10**18 / taxSum * redistributeTaxSell / 10**18;
                amountSum = devAmount + marketingAmount + redistributionAmount;
            }
            ...
            if(sender != address(_UniswapV2Pair)) {
                if(feeTokens > 0) {
                    ...
                    _approve(address(this), address(_UniswapV2Router02), devAmount + marketingAmount);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override virtual {
        ...
        if(isExcluded[msg.sender] || isExcluded[tx.origin]) {
