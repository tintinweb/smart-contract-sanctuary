...
contract nftContract is ERC721Full, Whitelist {

    IERC20 tokenContract = IERC20(address(0xAf7BfA6240745Fd41D1ED4b5fADE9DCAF369bA6c));
    uint256 mint1PackPrice = 500;
    uint256 mint3PackPrice = 1250;
    uint256 mint5PackPrice = 2000;
    ...
    address public mintTo = 0x554C38F7E742B289bF7D2015e963D533071Af388;
...
;
    ...
    function mint1(
        ...
        if(isWhitelisted(msg.sender)){
            require(tokenContract.transferFrom(msg.sender,mintTo,mint1PackPrice/2*10**18), 'Could not transfer tokens from your address to this contract');
            remove(msg.sender);
        }
        else{require(tokenContract.transferFrom(msg.sender,mintTo,mint1PackPrice*10**18), 'Could not transfer tokens from your address to this contract');}
    ...
    if( _rarity_perc>990){_rarity = 4;}
        ...
        return(nfts[_id-1].name, nfts[_id-1].rarity);
    ...
    function mint3(
        uint random,
        uint random2,
        uint random3) public returns (uint256,uint256,uint256,uint256,uint256,uint256) {
        ...
        require(this.totalSupply() < maxSupply, "You cannot mint any more item since you already reached the maximum supply.");
        if(isWhitelisted(msg.sender)){
            require(tokenContract.transferFrom(msg.sender,mintTo,mint3PackPrice/2*10**18), 'Could not transfer tokens from your address to this contract');
            remove(msg.sender);
        }
        else{require(tokenContract.transferFrom(msg.sender,mintTo,mint3PackPrice*10**18), 'Could not transfer tokens from your address to this contract');}
        ...
        uint256 seed = uint256(keccak256(abi.encodePacked(
            block.timestamp + block.difficulty + random / (now))));
            
            
        uint256 seed2 = uint256(keccak256(abi.encodePacked(
            block.timestamp + block.difficulty + random2 / (now))));
   
    
        uint256 seed3 = uint256(keccak256(abi.encodePacked(
        block.timestamp + block.difficulty + random3 +
        ((uint256(keccak256(abi.encodePacked(block.coinbase)))) / (now)))));

        
        
    uint256 _name =  seed - (seed / 18) * 18;
    uint256 _rarity_perc = seed2 - (seed2 / 1000) * 1000;
    ...
    _name =  seed2 - (seed2 / 18) * 18;
    _rarity_perc = seed3 - (seed3 / 1000) * 1000;
    ...
    uint256 _name3 =  seed3 - (seed3 / 18) * 18;
    uint256 _rarity_perc3 = seed - (seed / 1000) * 1000;
    uint256 _rarity3;
    
    if(_rarity_perc3 <= 600){_rarity = 0;}
    if(_rarity_perc3 <= 840 && _rarity_perc3>600){_rarity3 = 1;}
    if(_rarity_perc3 <= 940 && _rarity_perc3>840){_rarity3 = 2;}
    if(_rarity_perc3 <= 990 && _rarity_perc3>940){_rarity3 = 3;}
    if( _rarity_perc3>990){_rarity = 4;}
        ...
        emit nftTransaction(_id, "claimed", address(0x0), msg.sender, 0);
        emit nftTransaction(_id-1, "claimed", address(0x0), msg.sender, 0);
        emit nftTransaction(_id-2, "claimed", address(0x0), msg.sender, 0);
        
            return(nfts[_id-1].name, nfts[_id-1].rarity,
                nfts[_id-2].name, nfts[_id-2].rarity,
                nfts[_id-3].name, nfts[_id-3].rarity
                );
    ...
    function mint5(
        ...
        uint random3,
        uint random4,
        uint random5) public returns (uint256,uint256,uint256,uint256,uint256,uint256) {
        ...
        require(this.totalSupply() < maxSupply, "You cannot mint any more item since you already reached the maximum supply.");
        if(isWhitelisted(msg.sender)){
            require(tokenContract.transferFrom(msg.sender,mintTo,mint5PackPrice/2*10**18), 'Could not transfer tokens from your address to this contract');
            remove(msg.sender);
        }
        else{require(tokenContract.transferFrom(msg.sender,mintTo,mint5PackPrice*10**18), 'Could not transfer tokens from your address to this contract');}
        ...
        uint256 seed3 = uint256(keccak256(abi.encodePacked(
        block.timestamp + block.difficulty + random3  + block.gaslimit + block.number
    )));
    
        uint256 seed4 = uint256(keccak256(abi.encodePacked(
        random4 +
        ((uint256(keccak256(abi.encodePacked(block.coinbase)))) / (now)) +
        block.gaslimit)));    
        
        uint256 seed5 = uint256(keccak256(abi.encodePacked(
        block.timestamp + random5 +
        block.gaslimit)));
    ...
    _name =  seed2 - (seed2 / 18) * 18;
    _rarity_perc = seed4 - (seed4 / 1000) * 1000;
    ...
    if( _rarity_perc>990){_rarity = 4;}
        ...
        _name =  seed4 - (seed4 / 18) * 18;
        _rarity_perc = seed5 - (seed5 / 1000) * 1000;
        
        if(_rarity_perc <= 600){_rarity = 0;}
        if(_rarity_perc <= 840 && _rarity_perc>600){_rarity = 1;}
        if(_rarity_perc <= 940 && _rarity_perc>840){_rarity = 2;}
        if(_rarity_perc <= 940 && _rarity_perc>990){_rarity = 3;}
        if( _rarity_perc>990){_rarity = 4;}
        ...
        _name =  seed5 - (seed5 / 18) * 18;
        _rarity_perc = seed - (seed / 1000) * 1000;
        ...
        emit nftTransaction(_id-1, "claimed", address(0x0), msg.sender, 0);
        emit nftTransaction(_id-2, "claimed", address(0x0), msg.sender, 0);
        emit nftTransaction(_id-3, "claimed", address(0x0), msg.sender, 0);
        emit nftTransaction(_id-4, "claimed", address(0x0), msg.sender, 0);
