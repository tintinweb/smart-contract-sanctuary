...
import "./CopycatUniswapV2Adapter.sol";
import "../../lib/CopycatAdapterFactoryBase.sol";
import "hardhat/console.sol";
import "../../interfaces/IUniswapV2Router.sol";

contract CopycatUniswapV2AdapterFactory is CopycatAdapterFactoryBase {
  address[] public router;
  address[] public tradingToken;
  mapping(uint256 => address[]) public tradingRouteBuy;
  mapping(uint256 => address[]) public tradingRouteSell;
  bool[] public enabled;
  ...
  constructor() {
    ...
    tradingToken.push(address(0));
    enabled.push(false);
  }

  event AddPair(uint256 indexed pairId, address indexed _router, address indexed _tradingToken);
  function addPair(
    address _router,
    ...
    tradingRouteBuy[tradingToken.length] = _tradingRouteBuy;
    tradingRouteSell[tradingToken.length] = _tradingRouteSell;
    router.push(_router);
    tradingToken.push(_tradingToken);
    enabled.push(true);

    emit AddPair(tradingToken.length - 1, _router, _tradingToken);
  }

  event SetPairEnabled(uint256 indexed pairId, bool _enabled);
  function setPairEnabled(uint256 pairId, bool _enabled) public onlyOwner {
    enabled[pairId] = _enabled;
    emit SetPairEnabled(pairId, _enabled);
  }

  event SetFeeRate(address indexed setter, uint256 oldFeeRate, uint256 newFeeRate);
  function setFeeRate(uint256 _feeRate) public onlyOwner {
    require(_feeRate <= 1000, "L"); // Fee limit 1%
    emit SetFeeRate(msg.sender, feeRate, _feeRate);
    feeRate = _feeRate;
  }

  function getTradingRouteBuy(uint256 pairId) public view returns (address[] memory) {
    return tradingRouteBuy[pairId];
  }

  function getTradingRouteSell(uint256 pairId) public view returns (address[] memory) {
    return tradingRouteSell[pairId];
  }

  function deployAdapter(
    address leaderContract,
    uint256 pairId
  ) external onlyLeader(leaderContract) nonReentrant returns(address payable adapterAddress) {
    require(pairId > 0 && pairId < tradingToken.length, "I");
    require(enabled[pairId], "D");

    // TOEDIT: Change adapter type
    bytes memory bytecode = type(CopycatUniswapV2Adapter).creationCode;
    bytes32 salt = keccak256(abi.encodePacked(msg.sender, nextAdapterId));

    assembly {
      adapterAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
    }

    // TOEDIT: Add parameters
    CopycatUniswapV2Adapter(adapterAddress).initializeFromFactory(
      ICopycatLeader(leaderContract),
      pairId
    );

    finalize(leaderContract, adapterAddress);
  }
}

pragma solidity 0.8.7;

import "./CopycatUniswapV2AdapterFactory.sol";
import "../../lib/CopycatAdapterBase.sol";
import "../../interfaces/IUniswapV2Router.sol";

contract CopycatUniswapV2Adapter is CopycatAdapterBase {
  string public override adapterType = "uniswapv2";

  IUniswapV2Router02 public uniswapV2Router;
  IERC20 public tradingToken;
  address[] private tradingRouteBuy; // end at Token
  address[] private tradingRouteSell; // end at BNB
  uint256 public pairId; // Pair ID for syncing with factory

  bool private ZERO_MODE = false;

  function getTradingRouteBuy() external view returns(address[] memory) {
    return tradingRouteBuy;
  }

  function getTradingRouteSell() external view returns(address[] memory) {
    return tradingRouteSell;
  }

  function initialize(
    ICopycatLeader _leaderContract,
    ...
    initializeLeader(_leaderContract);
  }
  ...
  function initializeFromFactory(
    ...
    pairId = _pairId;
    CopycatUniswapV2AdapterFactory _factory = CopycatUniswapV2AdapterFactory(factory);

    initialize(
      _leaderContract,
      IUniswapV2Router02(_factory.router(pairId)),
      IERC20(_factory.tradingToken(pairId)),
      _factory.getTradingRouteBuy(pairId),
      _factory.getTradingRouteSell(pairId)
    );
  }
  ...
  function getTokenAmountFromBnb(uint256 amountIn) public view returns(uint256) {
    if (amountIn == 0) return 0; // Uniswap edge case
    uint256[] memory amounts = uniswapV2Router.getAmountsOut(amountIn, tradingRouteBuy);
    return amounts[amounts.length - 1];
  }

  function getTokenAmountRequiredFromBnb(uint256 amountOut) public view returns(uint256) {
    if (amountOut == 0) return 0; // Uniswap edge case
    uint256[] memory amounts = uniswapV2Router.getAmountsIn(amountOut, tradingRouteSell);
    return amounts[0];
  }

  function getBnbRequiredFromTokenAmount(uint256 amountOut) public view returns(uint256) {
    if (amountOut == 0) return 0; // Uniswap edge case
    uint256[] memory amounts = uniswapV2Router.getAmountsIn(amountOut, tradingRouteBuy);
    return amounts[0];
  }

  function getBnbValueAmountIn(uint256 amountIn) public view returns(uint256) {
    if (amountIn == 0) return 0; // Uniswap edge case
    uint256[] memory amounts = uniswapV2Router.getAmountsOut(amountIn, tradingRouteSell);
    return amounts[amounts.length - 1];
  }
  
  function getBnbValuePercentage(uint256 percentage) public view returns(uint256) {
    return getBnbValueAmountIn(tradingToken.balanceOf(address(this)) * percentage / 1e18);
  }

  // Sum of BNB value in all mode
  function getBnbValue() override public view returns(uint256) {
    if (ZERO_MODE) return 0;
    return getBnbValueAmountIn(tradingToken.balanceOf(address(this)));
  }

  function getTokenAmount() public view returns(uint256) {
    return tradingToken.balanceOf(address(this));
  }

  // Transfer BNB from leader to adapter and then convert BNB = msg.value to xxx
  function toAdapter(uint256 tokenMin) override external payable onlyLeaderContract nonReentrant returns(uint256) {
    if (msg.value < 1e2) { // Hope there are no coin such that decimal is too low
      return 0;
    }

    uint256 tokenBefore = tradingToken.balanceOf(address(this));

    uint256 fee = msg.value * CopycatUniswapV2AdapterFactory(factory).feeRate() / 100000;
    payable(CopycatUniswapV2AdapterFactory(factory).feeAddress()).transfer(fee);

    uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: msg.value - fee}(
      tokenMin,
      tradingRouteBuy,
      address(this),
      block.timestamp
    ...
    uint256 receivedToken = tradingToken.balanceOf(address(this)) - tokenBefore;

    // require(receivedToken >= minToken, "Insufficient Token");

    return receivedToken;
  }

  // Adapter sell xxx to BNB and transfer to leader (sell only ... percent (100% = 1e18)), returns BNB value
  function toLeader(uint256 bnbAmount, uint256 bnbMin) override external onlyLeaderContract nonReentrant returns(uint256) {
    uint256 tokenAmount = getTokenAmountRequiredFromBnb(bnbAmount);

    if (tokenAmount < 1e2) { // Hope there are no coin such that decimal is too low
      return 0;
    }

    uint256 bnbBefore = address(this).balance;

    if (tradingToken.balanceOf(address(this)) < tokenAmount) {
      tokenAmount = tradingToken.balanceOf(address(this));
    }

    uint256 fee = tokenAmount * CopycatUniswapV2AdapterFactory(factory).feeRate() / 100000;
    tradingToken.transfer(CopycatUniswapV2AdapterFactory(factory).feeAddress(), fee);

    tokenAmount -= fee;

    tradingToken.approve(address(uniswapV2Router), tokenAmount);
    ...
    // require(receivedBnb >= minBnb, "Insufficient BNB");

    address(msg.sender).call{value: receivedBnb}("");

    return receivedBnb;
  }

  function contractSignature() override public view returns(bytes32) {
  ...
  function _approveSubAdapter(address adapter, uint256 amount) internal override {
    tradingToken.approve(adapter, amount);
  }

  // This is used to build custom deposit / withdraw strategy in the future for example depositing BUSD directly
  function toggleZeroMode(uint256 adapterId, bool mode) external nonReentrant {
    require(msg.sender == address(leaderContract.adapters(adapterId)), "NA");
    ZERO_MODE = mode;
  }
}
...
import "@openzeppelin/contracts/access/Ownable.sol";
import "../interfaces/ICopycatAdapter.sol";
import "./CopycatEmergency.sol";
import "./CopycatAdapterFactoryBase.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

abstract contract CopycatAdapterBase is ICopycatAdapter, CopycatEmergency, ReentrancyGuard {
  address public override factory;
  ICopycatLeader internal leaderContract;
  ...
  function contractSignature() override public virtual view returns(bytes32);

  // Initialize the adapter and bond it to leader
  function initializeLeader(ICopycatLeader _leaderContract) override public notInitialized {
    leaderContract = _leaderContract;
    adapterId = _leaderContract.getAdaptersLength();
    initialized = true;
  }

  function getLeaderContract() override public view returns(ICopycatLeader) {
    return leaderContract;
  }

  function getLeaderAddress() override public view returns(address) {
    return Ownable(address(leaderContract)).owner();
  }

  modifier onlyLeader {
    require(msg.sender == getLeaderAddress(), "OL");
    _;
  }

  modifier onlyLeaderContract {
    require(msg.sender == address(getLeaderContract()) || allowedAdapters[msg.sender], "OLC");
    _;
  }

  function allowEmergencyCall(ICopycatEmergencyAllower allower, bytes32 txHash) public override view returns(bool) {
    return (
      msg.sender == address(getLeaderContract()) && 
      CopycatAdapterFactoryBase(factory).isAllowEmergency(allower) && 
      allower.isAllowed(txHash)
    );
  }

  function _approveSubAdapter(address adapter, uint256 amount) virtual internal;

  function approveSubAdapter(uint256 adapterId, uint256 amount) external nonReentrant override {
    address adapter = address(leaderContract.adapters(adapterId));
    require(msg.sender == adapter && adapter != address(0), "NA");
    allowedAdapters[adapter] = true;
    _approveSubAdapter(adapter, amount);
  }
}
