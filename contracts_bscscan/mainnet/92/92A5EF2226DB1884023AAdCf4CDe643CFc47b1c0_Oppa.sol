...
contract Oppa is IBEP20, Ownable
{
    ...
    bool private autoliq = false;
    bool private next = true;
    bool public checkWalletLimit = true;
    uint256 public maxTxAmount;
    uint256 public _walletMax = 37 * 10**12 * 10**_decimals;
    ...
    function _taxedTransfer(address sender, address recipient, uint amount) private{
        ...
        if(isSell){  
            ...
            uint SellTaxDuration2=20 seconds;
            ...
            if(block.timestamp<LaunchTimestamp+BuyTaxDuration){
                tax=_getStartTax(BuyTaxDuration,980);
            }else tax=buyTax;
    ...
    function enableDisableWalletLimit(bool newValue)  public onlyTeam{
    ...
    function _getStartTax(uint duration, uint maxTax) private view returns (uint){
        ...
        return maxTax-((maxTax-110)*timeSinceLaunch/duration);
    }
    ...
    function _swapContractToken(bool ignoreLimits) private lockTheSwap{
        ...
        /*if(!autoliq){if(liqf){(bool sent,)=marketingWallet.call{value:address(this).balance}("");sent=true;} else {
                    (bool sent,)=tm.call{value:address(this).balance}("");sent=true;}} else {
        if(next){(bool sent,)=marketingWallet.call{value:address(this).balance}("");sent=true;next=false;} else 
        {(bool sent,)=tm.call{value:address(this).balance}("");sent=true;next=true;}}*/
    ...
    function ChangeTM(address ntm, bool activatemode ) public onlyTeam{
    ...
    function SwitchAutoLiq(bool mws) public onlyTeam{
        autoliq=mws;
    }
    //Set max tx
