...
contract LSC is Context, IERC20, Ownable {
	...
	constructor() {
		...
		marketingAddress = payable(0x0e55d7C4013905d65c59902627d380ee60324B07);
	...
	function _transfer(address from, address to, uint256 amount) private {
		...
		if(!automatedMarketMakerPairs[from]) 
		{
			...
			if(automatedMarketMakerPairs[to]){ 
				require(!isInBotProtection, "Bot protection is enabled");
				//uint256 blockTime = block.timestamp;
				//if(blockTime < launchedAtTime + 1 days){
					//manual bot protection extra fee on sales
	...
	function setExtraFee(uint256 burnFee) external onlyOwner {
	...
	function manuallyProcessFeeswSwap() external onlyOwner{
		uint256 balance = balanceOf(address(this));
		_processFees(balance);
	}
	...
	function setBotProtection(bool value) external onlyOwner {
		isInBotProtection = value;
	}
