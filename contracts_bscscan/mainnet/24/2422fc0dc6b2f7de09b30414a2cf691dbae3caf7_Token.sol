    ...
    Pauseable
{
    ...
    // Preventions */
    bool private timeLimited;
    mapping(address => uint256) private _timeOfLastTransfer;
    uint256 private transferTime; // 180 seconds to start
    ...
    function initialize(
        ...
        __Ownable_init();
        __Pausable_init();
        // Totals name decimals
        _tTotal = 5000000000 * 10**9; // 5bln + 9 decimals
        ...
        _maxTxAmount = 40000000 * 10**9; // 20 million
        numTokensSellToAddToLiquidity = 750000 * 10**9; // 750g
        ...
        //exclude owner charity marketing and this contract from fee
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 portionForFees =
            contractTokenBalance.sub(halfOfLiquify).sub(otherHalfOfLiquify);
        ...
        // Split remainder into half which should be the other 1/3's
        sendBNBToMarketing(portionForFees.div(4).mul(2));
        sendBNBToCharity(portionForFees.div(4).mul(2));
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) private isPausedOverride {
        isTimeLimited(from, to);
        require(from != address(0), 'ERC20: transfer from the zero address');
        require(to != address(0), 'ERC20: transfer to the zero address');
        require(amount > 0, 'Transfer amount must be greater than zero');
        require(!_isBlackListedBot[to], 'To is blacklisted');
    ...
    /** Remove Fees
        Description: Remove fees when account is excluded
     */
    ...
    /** Restore Fees
        Description: Restore fees when account is excluded after transfer
     */
    ...
    /** Send BNB Charity
        Description: On Liquify event send X amount of BNB to charity wallet
    ...
    /** Send BNB Charity
        Description: On Liquify event send X amount of BNB to marketing wallet
        @param amount {uint256}
     */
    ...
    /** Include 
        Description: When an account is included fees are included
        @param account {address}
     */
    ...
    function setMaxTxAmt(uint256 _amt) external onlyOwner {
        _maxTxAmount = _amt;
    }
