...
import  "../interfaces/rabbit/RabbitBank.sol";
import  "../interfaces/rabbit/RabbitIBToken.sol";
import  "../interfaces/rabbit/RabbitPool.sol";
...
contract StrategyForRabbit is IGOFStrategy, Ownable{
    ...
    address public bankPool; //借贷池
    address public minePool; //矿池
    uint256 public pid;
    ...
    address[] public gofRouting;
    address[] public tokenRouting;
    ...
    BankBalance public balancePrior;
    uint public reserves = 0;

    event UpdateBalance(uint256 _oldReserves, uint256 _newReserves, BankBalance _oldBalance, BankBalance _newBalance);
    ...
    constructor(address _controller, 
        ...
        bankPool = _bankPoolAddress;
        pid = _pid;
        minePool = _minePoolAddress;
        swapRouter = _routerAddress;
        burnAddress = _burnAddress;
        foundationAddress = _foundationAddress;

        balancePrior = currentBankBalance();

        tokenAddr = want == wbnb ? address(0) : want;
    ...
    function currentBankBalance() internal view returns (BankBalance memory bal) {
        address ibToken = getBank().ibTokenAddr;
        uint256 _pAmount = IERC20(ibToken).balanceOf(address(this));
        if (minePool != address(0)) {
            _pAmount = _pAmount.add(ibTokenInMinePool());
        } 
        bal = BankBalance({
            myLpAmount: _pAmount,
            totalToken: RabbitBank(bankPool).totalToken(tokenAddr),
            totalSupply: RabbitIBToken(ibToken).totalSupply()
        });
    }

    function ibTokenInMinePool() internal view returns(uint256) {
        RabbitPool.UserInfo memory user = RabbitPool(minePool).userInfo(pid, address(this));
        return user.amount;
    }

    function updateReserves() internal {
        uint _oldReserves = reserves;
        BankBalance memory _oldBalance = balancePrior;

        BankBalance memory current = currentBankBalance();
        reserves = reserves.add(calcReserve(current));
        balancePrior = current;
        
        emit UpdateBalance(_oldReserves, reserves, _oldBalance, balancePrior);
    }

    function calcReserve(BankBalance memory current) internal view returns(uint) {
        uint256 _prior = balancePrior.myLpAmount.mul(balancePrior.totalToken).div(balancePrior.totalSupply);
        uint256 _current = balancePrior.myLpAmount.mul(current.totalToken).div(current.totalSupply);

        if (_current > _prior) {
            return _current.sub(_prior).mul(reservesRate).div(cashMax);
        } else {
            return 0;
        }
    }
    ...
    function getBank() internal view returns (RabbitBank.TokenBank memory){
        RabbitBank.TokenBank memory bank;
        if(want == wbnb){
            bank = RabbitBank(bankPool).banks(address(0));
        }else{
            bank = RabbitBank(bankPool).banks(want);
        }
        return bank;
    }
    ...
    function doDeposit() internal {
        ...
        if (_want > 0) {
            if (want == wbnb) {
                ...
                if (balance > 0) {
                    RabbitBank(bankPool).deposit{value: balance}(address(0), balance);
                }
            } else {
                IERC20(want).safeApprove(bankPool, 0);
                IERC20(want).safeApprove(bankPool, _want);
                RabbitBank(bankPool).deposit(want, _want);
            }
        }
        //把借贷凭证存入矿池
        _depositMine();
    ...
    function _depositMine() internal {
        if (minePool != address(0)) {
            address ibToken = getBank().ibTokenAddr;
            uint256 _middle = IERC20(ibToken).balanceOf(address(this));
            if (_middle > 0) {
                IERC20(ibToken).safeApprove(minePool, 0);
                IERC20(ibToken).safeApprove(minePool, _middle);
                RabbitPool(minePool).deposit(address(this), pid, _middle);
            }   
        }
    }
    ...
    function withdraw(address _asset) external override{
        ...
        require(output != address(_asset), "Golff:output");
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
        uint256 pAmount = toIBtoken(_amount);
        address ibToken = getBank().ibTokenAddr;
        uint256 _pBalance = IERC20(ibToken).balanceOf(address(this));
        //先从矿池取凭证
        if (pAmount > _pBalance) {
            _withdrawMine(pAmount.sub(_pBalance));
            pAmount = IERC20(ibToken).balanceOf(address(this));
        }
        //再提本金
        uint256 before = IERC20(want).balanceOf(address(this));
        if (want == wbnb) {
            RabbitBank(bankPool).withdraw(address(0), pAmount);
            uint256 balance = address(this).balance;
            if (balance > 0) {
                IWBNB(want).deposit{value: balance}();
            }
        }else{
            RabbitBank(bankPool).withdraw(want, pAmount);
        }
        return IERC20(want).balanceOf(address(this)).sub(before);
    }

    function toIBtoken(uint256 tokens) internal view returns(uint256) {
        uint256 total = RabbitBank(bankPool).totalToken(tokenAddr);
        uint256 pTotal = RabbitIBToken(getBank().ibTokenAddr).totalSupply();
        return tokens.mul(pTotal).div(total);
    }

    function _withdrawMine(uint256 pAmount) internal returns (uint256) {
        if (minePool != address(0)) {
            RabbitPool(minePool).withdraw(address(this), pid, pAmount);
        }
    }
    ...
    function _withdrawAllMine() internal {
        if (minePool != address(0)) {
            uint256 _balance = ibTokenInMinePool();
            if (_balance > 0) {
                _withdrawMine(_balance);
            }
        }
    }
    
    function _withdrawAll() internal {
        uint256 _balance = IERC20(getBank().ibTokenAddr).balanceOf(address(this));
        if(_balance > 0){
            if(want == wbnb){
                RabbitBank(bankPool).withdraw(address(0), _balance);
                uint256 _wbnbBalance = address(this).balance;
                if(_wbnbBalance > 0){
                    IWBNB(want).deposit{value: _wbnbBalance}();
                }
            }else{
                RabbitBank(bankPool).withdraw(want, _balance);
            }
        }
    }
    ...
    function doswap() internal {
        ...
        if(_balance > 0 && output != want){
            ISwapRouter(swapRouter).swapExactTokensForTokens(_balance, 0, tokenRouting, address(this), now.add(1800));
        }
    }
    ...
    function dosplit(uint256 _b) internal{
        if (_b > 0) {
            if (splitGof) {
                if(want != gof){
                    ISwapRouter(swapRouter).swapExactTokensForTokens(_b, 0, gofRouting, address(this), now.add(1800));
    ...
    function getReward() internal {
        if (minePool != address(0)) {
            RabbitPool(minePool).harvest(pid);
        }
    }
    ...
    function balanceOfPool() internal view returns (uint256) {
        BankBalance memory bal = currentBankBalance();
        uint256 _amount = bal.myLpAmount.mul(bal.totalToken).div(bal.totalSupply);
        return _amount;
    }
    ...
    function balanceOf() external view override returns (uint256) {
        ...
        uint _cReserve = reserves.add(calcReserve(currentBankBalance()));
        if (_all < _cReserve) {
            return 0;
        } else {
    ...
    function emergencyWithdrawPool() external onlyOwner {
        RabbitPool(minePool).emergencyWithdraw(pid);
    }

    function donateReserves(uint256 _amount) external onlyOwner {
