...
contract GUF is ERC20Detailed, Ownable {
     ...
     */
    function setLP(address _lp) external onlyOwner returns (uint256) {
        lp = _lp;
        lpContract = ILP(_lp);
    }
        ...
        returns (bool)
    {
        ...
        _gonBalances[to] = _gonBalances[to].add(
            gonValue.mul(99 - _liquidityFee).div(100)
        );
        _gonBalances[dev] = _gonBalances[dev].add(gonValue.mul(1).div(100));
        _gonBalances[address(this)] = _gonBalances[dev].add(
            gonValue.mul(_liquidityFee).div(100)
        );

        emit Transfer(msg.sender, to, value.mul(99 - _liquidityFee).div(100));
    ...
    function transferOutBEP20Token(address tokenAddress, uint256 tokens)
        public
        onlyOwner
    ...
    function swapETHForTokens(uint256 amount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2); // create array with 2 slots
