...
interface IExchange {
    ...
    function swapExtractOut(address tokenIn, address tokenOut, address recipient, uint256 amountIn, uint256 slippage, uint256 deadline) external returns (uint256);
...
// File: localhost/mint/implement/BorrowProxy.sol
...
contract BorrowProxy is IBorrowProxy, ConfigBase {
    
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    event BorrowAccess(address indexed spender, bool state);
    
    mapping (address => bool) public borrowAccess;
    
    constructor (IConfig _config) ConfigBase(_config) {
    }
    
    function setBorrowAccess(address spender, bool state) override external onlyOwner {
        borrowAccess[spender] = state;
        emit BorrowAccess(spender, state);
    }
    
    function borrow(address owner, uint256 id, address tokenA, uint256 amountA, address tokenB, uint256 amountB, address borrowToken, uint256 leverage, uint256 deadLine) override external returns (uint256, uint256, uint256) {
        // leeqiang
        // require(borrowAccess[msg.sender], "publics:not_borrow_access");
        config.isBlacklist(owner);
        config.isOpen(msg.sender);
        config.isLeverage(msg.sender, leverage);
        if (address(0) != borrowToken) {
            config.isLoan(borrowToken);
        }
        require(0 < amountA || 0 < amountB, "publics:amounta_and_amountb_is_zero");
        config.getApproveProxy().claim(tokenA, owner, address(this), amountA);
        config.getApproveProxy().claim(tokenB, owner, address(this), amountB);
        leverage = this.getBorrowAmount(tokenA, amountA, tokenB, amountB, borrowToken, leverage);
        // leverage->borrowAmount
        if (0 < leverage) {
            uint256 code = config.getLoanPublics(borrowToken).doCreditLoanBorrow(owner, leverage, id, ILoanTypeBase.LoanType.MINNING_SWAP_PROTOCOL);
            require(0 == code, "publics:loan_publics_borrow_error");
            if (tokenA == borrowToken) {
                amountA = amountA.add(leverage);
            }else if (tokenB == borrowToken) {
                amountB = amountB.add(leverage);
            }else {
                IERC20(borrowToken).approve(address(config.getExchange()), leverage);
                amountA = amountA.add(config.getExchange().swapExtractOut(borrowToken, tokenA, address(this), leverage, config.getSlippage(), deadLine));
            }
        }
        (amountA, amountB) = this.trim(tokenA, amountA, tokenB, amountB, deadLine);
        require(0 < amountA && 0 < amountB, "publics:amounta_or_amountb_is_zero");
        IERC20(tokenA).safeTransfer(msg.sender, amountA);
        IERC20(tokenB).safeTransfer(msg.sender, amountB);
        return (amountA, amountB, leverage);
    }
    
    // leeqiang
    function trim(address tokenA, uint256 amountA, address tokenB, uint256 amountB, uint256 deadLine) external returns (uint256, uint256) {
        uint256 oneA = 10 ** IERC20(tokenA).decimals();
        uint256 oneB = 10 ** IERC20(tokenB).decimals();
        uint256 price = config.getExchange().swapEstimateOut(tokenA, tokenB, oneA);
        uint256 valueA = amountA.mul(price);
        uint256 valueB = amountB.mul(oneA);
        if (valueA < valueB) {
            uint256 swapB = valueB.sub(valueA).div(oneA).div(2);
            if (0 < swapB) {
                amountB = amountB.sub(swapB);
                IERC20(tokenB).approve(address(config.getExchange()), swapB);
                price = config.getExchange().swapExtractOut(tokenB, tokenA, address(this), swapB, config.getSlippage(), deadLine);
                amountA = amountA.add(price);
            }
        }else {
            uint256 swapA = valueA.sub(valueB).div(oneB).div(2);
            if (0 < swapA) {
                amountA = amountA.sub(swapA);
                IERC20(tokenA).approve(address(config.getExchange()), swapA);
                price = config.getExchange().swapExtractOut(tokenA, tokenB, address(this), swapA, config.getSlippage(), deadLine);
                amountB = amountB.add(price);
            }
        }
    ...
    // leeqiang
    function trimV2(address tokenA, uint256 amountA, address tokenB, uint256 amountB) external view returns (uint256, uint256) {
        uint256 oneA = 10 ** IERC20(tokenA).decimals();
        uint256 oneB = 10 ** IERC20(tokenB).decimals();
        uint256 price = config.getExchange().swapEstimateOut(tokenA, tokenB, oneA);
        uint256 valueA = amountA.mul(price);
        ...
        if (valueA < valueB) {
            ...
            if (0 < swapB) {
                amountB = amountB.sub(swapB);
                price = config.getExchange().swapEstimateOut(tokenB, tokenA, swapB);
                amountA = amountA.add(price);
            }
        }else {
            uint256 swapA = valueA.sub(valueB).div(oneB).div(2);
            if (0 < swapA) {
                amountA = amountA.sub(swapA);
                price = config.getExchange().swapEstimateOut(tokenA, tokenB, swapA);
                amountB = amountB.add(price);
            }
        }
        return (amountA, amountB);
    ...
    // leeqiang
    function trimV3(address tokenA, uint256 amountA, address tokenB, uint256 amountB) external view returns (uint256, uint256) {
        uint256 oneA = 10 ** IERC20(tokenA).decimals();
        uint256 oneB = 10 ** IERC20(tokenB).decimals();
        uint256 price = config.getExchange().swapEstimateOut(tokenA, tokenB, oneA);
        uint256 valueA = amountA.mul(price);
    ...
    // leeqiang
    function getBorrowAmount(address tokenA, uint256 amountA, address tokenB, uint256 amountB, address borrowToken, uint256 leverage) external view returns (uint256) {
        leverage = leverage.sub(10);
        uint256 borrowAmount;
        if (0 < leverage) {
            uint256 decimal = IERC20(tokenA).decimals();
            uint256 price = config.getExchange().swapEstimateOut(tokenA, borrowToken, 10 ** decimal);
            uint256 value = leverage.mul(amountA).mul(price);
            borrowAmount = value.div(10 ** decimal).div(10);
            decimal = IERC20(tokenB).decimals();
            price = config.getExchange().swapEstimateOut(tokenB, borrowToken, 10 ** decimal);
            value = leverage.mul(amountB).mul(price);
            borrowAmount = borrowAmount.add(value.div(10 ** decimal).div(10));
        }
        return borrowAmount;
    }

    // leeqiang
    function doCreditLoanBorrow(address owner, uint256 id, address borrowToken, uint256 borrowAmount) external {
        config.getLoanPublics(borrowToken).doCreditLoanBorrow(owner, borrowAmount, id, ILoanTypeBase.LoanType.MINNING_SWAP_PROTOCOL);
    }
    
    function swap(address borrowToken, uint256 borrowAmount, address tokenA, uint256 deadLine) external {
        IERC20(borrowToken).approve(address(config.getExchange()), borrowAmount);
        config.getExchange().swapExtractOut(borrowToken, tokenA, address(this), borrowAmount, config.getSlippage(), deadLine);
    }
