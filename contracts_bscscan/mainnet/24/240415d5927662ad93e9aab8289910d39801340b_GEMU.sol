...
**    Total tax - 10%
**
**    5% - Reflections: to be distributed equally amongst all holders of TEMP only on selling of TEMP tokens and not on wallet to wallet transfers. 
**
...
**
**    2% - Development wallet: this will be sent straight to the tax generated development wallet as TEMP. 
**
**    3% - Liquidity pool: this will be sent directly to the liquidity pool as BNB 
**    
**    1 trillion token burned every 7 days, untlil total supply would be set 1 trillion 
**
**    Burning will be stopped once totalSupply reach to 1 trillion    
**
**/
...
contract GEMU  is Context, IERC20, Ownable {
    ...
    string private _name = "GEMU";
    string private _symbol = "GEMU";
    ...
    uint256 private burningTokenCriteria = 10**12 * 10**18;
    uint256 private burnTokenEveryWeek = 500000000000  * 10**18;
    ...
    constructor (address payable developmentWallet) public {
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        //(uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tDevelopment, uint256 tCharity) = _getValues(tAmount);
        uint256[8] memory values = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256[8] memory values) {
        // (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tDevelopment, uint256 tCharity) = _getTValues(tAmount);
        ...
        // @return rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tBurn, tDevelopment, tCharity
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256[5] memory tValues) {
        ...
        uint256 tTransferAmount = tempTransferAmount.sub(tBurn).sub(tDevelopment);
        return [tTransferAmount, tFee, tLiquidity, tBurn, tDevelopment];
    }
    
    function _getRValues(uint256[5] memory tValues, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        //function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, tBurn, tDevelopment, tCharity uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = tempTransferAmount.sub(rBurn).sub(rDevelopment);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _developmentFee == 0) return;
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax, burn, liquidity, development, charity fee
