...
contract DividendDistributor is IDividendDistributor {
    ...
    event Deposit(uint256 amountBNB, uint256 amountBTCB);
...
contract BitLife is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "BitLife";
    string constant _symbol = "BTL";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 21000000 * (10 ** _decimals); // 21%
    uint256 public _maxTxAmount = _totalSupply / 50; // 4%

    //max wallet holding of 4%
    uint256 public _maxWalletToken = ( _totalSupply * 1 ) / 25;
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        return selling ? totalFee.add(sellFee) : totalFee;
    }
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        if(receiver == pair && autoLiquifyEnabled){
            sellFeeAccumulator = sellFeeAccumulator.add(feeAmount.mul(sellFee).div(totalFee.add(sellFee)));
        }
    ...
    function swapBack() internal swapping {
        if(sellFeeAccumulator >= swapThreshold && autoLiquifyEnabled){
            sellFeeAccumulator = sellFeeAccumulator.sub(swapThreshold);
    ...
    function setFees(
        ...
        require(totalFee.add(sellFee) < feeDenominator/5);

        emit FeesUpdated(_enabled, _sellFee, _liquidityFee, _reflectionFee, _marketingFee, _devFee, _feeDenominator);
    }
    ...
    function autoClaimStatus() external view returns (bool) {
        return(
            autoClaimEnabled
        );
    }
    ...
    event FeesUpdated(bool enabled, uint256 sellFee, uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee, uint256 devFee, uint256 feeDenominator);
