...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 Deviden = IBEP20(0xdFE6891ce8E5a5c7Cf54fFdE406A6C2C54145F71);
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = Deviden.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(Deviden);
        ...
        uint256 amount = Deviden.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            Deviden.transfer(shareholder, amount);
...
contract CheemsDoge is Ownable, IBEP20, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    string constant _name = "CheemsDoge";
    string constant _symbol = "CheemsDoge";
    ...
    uint256 public _maxWalletHold = 40000000000000;// Maxwallet
    ...
    uint256 marketingFee = 1200;
    uint256 devFee = 100;
    uint256 buybackFee = 100;
    uint256 totalFee = burnFee.add(reflectionFee).add(marketingFee).add(devFee).add(buybackFee);
    ...
    function swapBack() internal swapping {
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
			...
			payable(buybackwallet).call{value: amountBNBBuyback, gas: 30000}("");
    ...
    function setFees(uint256 _burnFee, uint256 _devFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _buybackFee, uint256 _feeDenominator) external onlyOwner {
        burnFee = _burnFee;
