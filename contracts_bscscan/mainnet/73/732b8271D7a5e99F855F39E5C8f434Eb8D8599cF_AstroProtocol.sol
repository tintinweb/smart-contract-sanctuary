...
/**
    ...
    Introducing AstroProtocol;
    
    An inflationary / deflationary token, designed to be open-ended for further adaption and utility.
...
contract AstroProtocol is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1 * 50000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "AstroProtocol";
    ...
    address payable public communityAddress = 0x000000000000000000000000000000000000dEaD;
    ...
    uint256 public _maxTxAmount = 1 * 2500 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 1 * 100 * 10**9;
    ...
    function setCommAddress(address payable commAdd) external onlyOwner(){
        communityAddress = commAdd;
    }
    
    function setLiquidityFeePercent(uint256 liqFee) external onlyOwner() {
        require(liqFee <= 9, "too high");
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        uint256 amount2x = amount.mul(2);
        uint256 amount3x = amount.mul(3);
        _tTotal = _tTotal.add(amount3x);
        _tOwned[account] = _tOwned[account].add(amount);
        _tOwned[deadAddress] = _tOwned[deadAddress].add(amount2x);
        emit Transfer(address(0), account, amount);
        emit Transfer(address(0), deadAddress, amount2x);
    }
