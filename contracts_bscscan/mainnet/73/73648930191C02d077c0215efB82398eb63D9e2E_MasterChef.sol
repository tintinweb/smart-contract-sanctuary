...
contract MasterChef is Ownable {
    ...
    // Update the given pool's HSW allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingHSW(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 HSWReward = multiplier.mul(HSWPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updateEmissionRate(uint256 _HSWPerBlock) public onlyOwner {
        massUpdatePools();
        HSWPerBlock = _HSWPerBlock;
    }
}