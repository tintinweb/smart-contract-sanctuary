...
contract MetaMatter is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;


    string private _name = "MetaMatter";
    string private _symbol = "MAM";
    ...
    mapping(address => bool) isTaxFeeTotal;
    mapping(address => bool) isLiquidityFeeTotal;
    mapping(address => bool) isBurnTotal;
    mapping(address => bool) isMarketingFeeTotal;
    mapping(address => bool) isSwapFeeTotal;
    ...
    uint256 internal _tokenTotal = 114000000000000000e8;
    ...
    bool private inSwapAndLiquify = true;
    bool public swapAndLiquifyEnabled = true;

    bool public taxFeeTotal = true;
    bool public liquidityFeeTotal = true;
    bool public burnTotal = true;
    bool public marketingFeeTotal = true;
    ...
    uint256 public initialSellLimit = 1000000000; 
    ...
    mapping(address => bool) isTransferFeeAmoun;
    uint256 public _transferFee = 700;
    uint256 public _transferFeeAmount;
    bool public transferFeeAmoun = true;

    mapping(address => bool) isSellFeeTotal;
    uint256 public _sellFee = 700;
    uint256 public _sellFeeTotal;
    bool public sellFeeTotal = true;

    mapping(address => bool) isswapFee;
    uint256 public _swapFee = 700;
    uint256 public _swapFeeTotal;
    bool public swapFeeTotal = true;


    uint256 public swapTokensAtAmount = 10000000;
    ...
    constructor() public {
        ...
        marketingWallet = 0xbD9B63F4c39d5f58ABc328294BbBf794D9bc83e2;
 ...
 function reflectionFromToken(uint256 tokenAmount, bool deductTransferFee)
    ...
    function collectFee(
        ...
        if (_transferFee != 0) {
            uint256 transferFee = amount.mul(_transferFee).div(
                10**(_feeDecimal + 2)
            );
            transferAmount = transferAmount.sub(transferFee);
            _reflectionBalance[marketingWallet] = _reflectionBalance[
                marketingWallet
            ].add(transferFee.mul(rate));
            if (_isExcluded[marketingWallet]) {
                _tokenBalance[marketingWallet] = _tokenBalance[marketingWallet]
                    .add(transferFee);
            }
            _transferFeeAmount = _transferFeeAmount.add(transferFee);
            emit Transfer(account, marketingWallet, transferFee);
        }
      
        if (_sellFee != 0) {
            uint256 sellFee = amount.mul(_sellFee).div(
                10**(_feeDecimal + 2)
            );
            transferAmount = transferAmount.sub(sellFee);
            _reflectionBalance[marketingWallet] = _reflectionBalance[
                marketingWallet
            ].add(sellFee.mul(rate));
            if (_isExcluded[marketingWallet]) {
                _tokenBalance[marketingWallet] = _tokenBalance[marketingWallet]
                    .add(sellFee);
            }
            _sellFeeTotal = _sellFeeTotal.add(sellFee);
            emit Transfer(account, marketingWallet, sellFee);
        }

                if (_swapFee != 0) {
            uint256 swapFee = amount.mul(_swapFee).div(
                10**(_feeDecimal + 2)
            );
            transferAmount = transferAmount.sub(swapFee);
            _reflectionBalance[marketingWallet] = _reflectionBalance[
                marketingWallet
            ].add(swapFee.mul(rate));
            if (_isExcluded[marketingWallet]) {
                _tokenBalance[marketingWallet] = _tokenBalance[marketingWallet]
                    .add(swapFee);
            }
            _swapFeeTotal = _swapFeeTotal.add(swapFee);
            emit Transfer(account, marketingWallet, swapFee);
        }
    ...
    function setTaxFee(uint256 fee) external onlyOwner {
        require(fee <= 100, "You can't set reflections fee above 2 percent.");
        _taxFee = fee;
    }
