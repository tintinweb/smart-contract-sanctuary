...
contract HeroMiningV2 is Initializable, Ownable, Constants {
    ...
    function initialize(
        ...
        endBlock = _endBlock;
        unfrizedTime = block.timestamp + (_frizedDays * 1 days);
    ...
    function harvest(uint256 heroId) public {
        ...
        require(block.timestamp >= unfrizedTime, "funds are blocked");
        uint256 pending = user.heroesPower.mul(pool.accPactPerShare).div(1e12).sub(user.rewardDebt);
        uint256 clame = user.clamedDebt + pending;
        if(clame > 0) {
            user.clamedDebt = 0;
            safePactTransfer(msg.sender, clame);
        }
    ...
    function deposit(uint256 heroId) onlyHeroTeamCurrency(heroId) public {
        ...
        if (user.heroesPower > 0) {
            uint256 pending = user.heroesPower.mul(pool.accPactPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                user.clamedDebt = user.clamedDebt + pending;
            }
