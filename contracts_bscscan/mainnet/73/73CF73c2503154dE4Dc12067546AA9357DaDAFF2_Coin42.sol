...
contract Coin42 {
    ...
    modifier isNotCoin (address _address) {
        require(_address != address(this), "ERC20: 42-coin Token address");
        _;
    }
    ...
    function mint(address _to, uint256 _amount) public issuerOnly isNotZeroAddress(_to) isNotCoin(_to) returns (bool success) {
        totalSupply += _amount;
    ...
    function transferFrom( address _from, address _to, uint256 _amount) public isNotZeroAddress(_to) isNotCoin(_to) returns (bool success) {
        _allowance[_from][msg.sender] -= _amount;
        _balanceOf[_from] -= _amount;
        _balanceOf[_to] += _amount;
    ...
    constructor() {
        _owner = msg.sender;
        setIssuerRights(_owner, true);
        mint(_owner, MAX_SUPPLY);
