...
contract BnbPricePrediction is Ownable, Pausable {
    ...
    constructor(
        ...
        neverPair = _neverPair;
        neverToken = _neverToken;
        everToken = _everToken;
    ...
    function betBear(uint256 _amount) external whenNotPaused notContract {
        ...
        everToken.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function claim(uint256 epoch) external notContract {
        ...
        _safeTransferEver(address(msg.sender), reward);
    ...
    function claimTreasury() external onlyAdmin {
        ...
        _safeTransferEver(feeAddress, currentTreasuryAmount);
        treasuryAmount = 0;
     ...
     */
    function _getPriceFromOracle() internal returns (uint256) {
        (uint112 never_eve_reserves_0, uint112 never_eve_reserves_1,) = neverPair.getReserves();
        uint256 reserve_0 = uint256(never_eve_reserves_0);
        uint256 reserve_1 = uint256(never_eve_reserves_1);
        uint256 never_price_eve = reserve_1.mul(1e18).div(reserve_0);
        return never_price_eve;
    }

    function _safeTransferEver(address to, uint256 value) internal {
        everToken.safeTransfer(to, value);
    }
