...
contract SaleBase is Ownable {
        ...
        onlyWhileOpen
    {
        ...
        require(
            _paymentToken.balanceOf(_msgSender()) >= paymentTokenAmount,
    ...
    function _processPurchase(
        ...
        _paymentToken.safeTransferFrom(_msgSender(), _tokenWallet, paymentTokenAmount);
    ...
    function buyTokens(uint256 weiAmount) public {
        uint256 paymentTokenAmount = weiAmount;
        address beneficiary = _msgSender();
        _preValidatePurchase(beneficiary, weiAmount, paymentTokenAmount);
        uint256 tokenAmount = _getTokenAmount(weiAmount);
        _weiRaised = _weiRaised.add(weiAmount);
        _processPurchase(beneficiary, paymentTokenAmount, tokenAmount);
