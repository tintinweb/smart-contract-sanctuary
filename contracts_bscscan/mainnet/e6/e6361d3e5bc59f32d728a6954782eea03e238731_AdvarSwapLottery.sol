...
interface IAdvarSwapLottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in ADVAR per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in ADVAR token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInAdvar: price of a ticket in ADVAR
...
/** @title AdvarSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract AdvarSwapLottery is ReentrancyGuard, IAdvarSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInAdvar = 500 ether;
    uint256 public minPriceTicketInAdvar = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInAdvar;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] advarPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInAdvar;
        uint32 finalNumber;
    }
     ...
     * @param _advarTokenAddress: address of the ADVAR token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _advarTokenAddress, address _randomGeneratorAddress) {
        advarToken = IERC20(_advarTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of ADVAR to this contract
        uint256 amountAdvarToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInAdvar,
            _ticketNumbers.length
        );

        // Transfer advar tokens to this contract
        advarToken.safeTransferFrom(address(msg.sender), address(this), amountAdvarToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInAdvar += amountAdvarToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInAdvarToTransfer
        uint256 rewardInAdvarToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInAdvarToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        advarToken.safeTransfer(msg.sender, rewardInAdvarToTransfer);

        emit TicketsClaim(msg.sender, rewardInAdvarToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInAdvar) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in ADVAR for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].advarPerBracket[j] =
                ...
                // A. No ADVAR to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].advarPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInAdvar - amountToShareToWinners);

        // Transfer ADVAR to treasury address
        advarToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        advarToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInAdvar += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInAdvar >= minPriceTicketInAdvar) && (_priceTicketInAdvar <= maxPriceTicketInAdvar),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInAdvar: _priceTicketInAdvar,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            advarPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInAdvar: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(advarToken), "Cannot be ADVAR token");
    ...
    /**
     * @notice Set ADVAR price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInAdvar: minimum price of a ticket in ADVAR
     * @param _maxPriceTicketInAdvar: maximum price of a ticket in ADVAR
     */
    function setMinAndMaxTicketPriceInAdvar(uint256 _minPriceTicketInAdvar, uint256 _maxPriceTicketInAdvar)
        external
        onlyOwner
    {
        require(_minPriceTicketInAdvar <= _maxPriceTicketInAdvar, "minPrice must be < maxPrice");

        minPriceTicketInAdvar = _minPriceTicketInAdvar;
        maxPriceTicketInAdvar = _maxPriceTicketInAdvar;
    }
     ...
     * @param _priceTicket price of a ticket (in ADVAR)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].advarPerBracket[_bracket];
        } else {
            return 0;
        }
    }
