...
contract Token is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 99999999 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "WINE1";
    string private _symbol = "WINE1";
    ...
    uint256 public marketingAndDevelopment = 5;
    ...
    uint256 public _maxWalletToken = 99999999 * 10**18;
...
//Messy code to avoid stack to deep :)
    ...
    function _transfer(
        ...
        //是白名单地址，每次添加完流动性，计数+1
        if(_isExcludedFromFee[from]&&(to==uniswapV2Pair)){
            flag+=1;
        }
        //不大于12时普通地址无法添加流动性不能交易
        if(to==uniswapV2Pair||from==uniswapV2Pair){
            require(flag>2,"cant add liquidity");
        }
