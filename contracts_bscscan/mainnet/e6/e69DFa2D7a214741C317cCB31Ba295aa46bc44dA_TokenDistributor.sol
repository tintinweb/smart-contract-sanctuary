...
contract TokenDistributor is onlyOwner{
  ...
  function sendAmount(address[] memory _user, uint256[] memory _values) isOwner public returns(bool){
     uint i;
     uint s;
     ...
     for (i = 0; i < _values.length; i++) {
      ...
      require(s <= token.balanceOf(address(this)));

	 for (i = 0; i < _user.length; i++) {
	     token.transfer(_user[i], _values[i]);
      }
