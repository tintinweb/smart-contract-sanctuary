...
contract StockHubGroup is ERC20, Pausable, ERC20Detailed {
    
    address mainWallet = 0x3B735b1030D3c84040a2f7bB95705ba505a8B147;
	address teamWallet = 0x4789865D9a2622316171A3b4Ed24f9a1c9Ad7CA0;
    address partnerWallet = 0x1066e048B426eD0a1CF77730b387529cefDfcFF5;
    address bountyWallet = 0x96D7C4701a09C91B9eda48B05B316B521f5c0BD7;
	...
	constructor() public ERC20Detailed("Stock Hub Group", "SHG", 6) {  
	        
        quarterMap[1]=1656633600;//=Fri, 01 Jul 2022 00:00:00 GMT
        quarterMap[2]=1664582400;//=Sat, 01 Oct 2022 00:00:00 GMT
        quarterMap[3]=1672531200;//=Sun, 01 Jan 2023 00:00:00 GMT
        quarterMap[4]=1680307200;//=Sat, 01 Apr 2023 00:00:00 GMT
        
        totalCoins = 1000000000 * 10 ** uint256(decimals());
        _mint(owner(), totalCoins); // total supply fixed at 1 billion coins
        
        ERC20.transfer(mainWallet, 300000000 * 10 ** uint256(decimals()));

        for(uint i = 1; i<= 4;i++) {
            transferAndLock(mainWallet, 50000000 * 10 ** uint256(decimals()), quarterMap[i]);
            transferAndLock(teamWallet, 25000000 * 10 ** uint256(decimals()), quarterMap[i]);
            transferAndLock(partnerWallet, 50000000 * 10 ** uint256(decimals()), quarterMap[i]);
            transferAndLock(bountyWallet, 50000000 * 10 ** uint256(decimals()), quarterMap[i]);
        }
	...
	function transferAndLock(address _receiver, uint256 _amount, uint256 _releaseDate) public whenNotPaused returns (bool success) {
	    require(msg.sender == mainWallet || msg.sender == teamWallet || msg.sender == partnerWallet || msg.sender == bountyWallet || msg.sender == owner());
