...
contract MetaInu is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 77000000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "Meta Inu";
    string private _symbol = "MetaInu";
    uint8 private _decimals = 18;

    uint256 public _taxFee;
    uint256 private _previousTaxFee = _taxFee;
    uint256 public _prizeFee;
    ...
    uint256 public _maxTxAmount = 770000000000 * 10**18;
    ...
    constructor (address payable marketingWalletAddress, address payable prizePoolAddress) {
    ...
    function setMarketingFeePercent(uint256 marketingFee) external onlyOwner() {
     ...
     function setPrizePoolFeePercent(uint256 prizeFee) external onlyOwner() {
        _prizeFee = prizeFee;
    }
    ...
    //     (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tPrizePool) = _getTValues(tAmount);
    //     (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tMarketing, tPrizePool, _getRate());
    //     return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tMarketing, tPrizePool);
    // }
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tempTransferAmount.sub(tMarketing).sub(tPrizePool);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tPrizePool, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rMarketing).sub(rPrizePool);
    ...
    function _takePrizePool(uint256 tPrizePool, address sender) private {
        uint256 currentRate =  _getRate();
        uint256 rPrizePool = tPrizePool.mul(currentRate);
        _rOwned[_prizePoolAddress] = _rOwned[_prizePoolAddress].add(rPrizePool);
        if(_isExcluded[_prizePoolAddress])
            _tOwned[_prizePoolAddress] = _tOwned[_prizePoolAddress].add(tPrizePool);
    ...
    function _setMarketingWalletAddress(address payable marketingWalletAddress) external onlyOwner() {
            _marketingWalletAddress = marketingWalletAddress;
    }
    function _setPrizePoolWalletAddress(address payable prizeWalletAddress) external onlyOwner() {
            _prizePoolAddress = prizeWalletAddress;
    }
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _prizeFee == 0 && _marketingFee == 0) return;
    ...
    // function charityPayout() payable public {
    //     uint256 balance = address(this).balance;
    //     _charityWalletAddress.transfer(balance);
    // }
}