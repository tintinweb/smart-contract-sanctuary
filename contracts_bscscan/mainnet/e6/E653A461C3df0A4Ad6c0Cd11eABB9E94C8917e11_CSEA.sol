...
contract ExtendedReflections is Context, IERC20, Ownable {
    ...
    */
    ...
    uint32 private _taxCharityDecimals;
    ...
    // This percent of a transaction will be transferred to charity wallet.
    uint32 private _taxCharity; 
    ...
    bool private _charityRewardEnabled;
    ...
    */
    ...
    event TaxCharityUpdate(uint32 previousTax, uint32 previousDecimals, uint32 currentTax, uint32 currentDecimal);
    event MinTokensBeforeSwapUpdated(uint256 previous, uint256 current);
    event CharityAddressUpdated(address previous, address current);
    ...
    event EnabledCharityReward();
    event DisabledReward();
    event DisabledAutoSwapAndLiquify();
    event DisabledCharityReward();
    ...
    function taxCharity() public view virtual returns (uint32) {
        return _taxCharity;
    }
    ...
    function taxCharityDecimals() public view virtual returns (uint32) {
    ...
    function charityRewardEnabled() public view virtual returns (bool) {
        return _charityRewardEnabled;
    }
    ...
    function _getTValues(ValuesFromAmount memory values, bool deductTransferFee) view private {
        ...
        if (deductTransferFee) {
            ...
            // amount after fee
            values.tTransferAmount = values.amount - values.tRewardFee - values.tLiquifyFee - values.tCharityFee;
        }
    ...
    function _getRValues(ValuesFromAmount memory values, bool deductTransferFee) view private {
        ...
        if (deductTransferFee) {
            ...
            values.rTransferAmount = values.rAmount - values.rRewardFee - values.rLiquifyFee - values.rCharityFee;
        }
    ...
    */
    ...
    function enableCharityTax(uint32 taxCharity_, uint32 taxCharityDecimals_, address charityAddress_) public onlyOwner {
        require(!_charityRewardEnabled, "Charity tax feature is already enabled.");
        require(taxCharity_ > 0, "Tax must be greater than 0.");
        require(taxCharityDecimals_ + 2  <= decimals(), "Tax decimals must be less than token decimals - 2");

        _charityRewardEnabled = true;
        setCharityTax(taxCharity_, taxCharityDecimals_);
        setCharityAddress(charityAddress_);

        emit EnabledCharityReward();
    }
    ...
    function disableCharityTax() public onlyOwner {
        require(_charityRewardEnabled, "Charity reward feature is already disabled.");

        setCharityTax(0, 0);
        setCharityAddress(address(0x0));
        _charityRewardEnabled = false;
        
        emit DisabledCharityReward();
    }
    ...
    function setCharityTax(uint32 taxCharity_, uint32 taxCharityDecimals_) public onlyOwner {
        require(_charityRewardEnabled, "Charity reward feature must be enabled. Try the enableCharityTax function.");

        uint32 previousTax = _taxCharity;
        uint32 previousDecimals = _taxCharityDecimals;
    ...
    function setCharityAddress(address charityAddress_) public onlyOwner {
        require(_charityRewardEnabled, "Charity reward feature must be enabled. Try the enableCharityTax function.");
        require(charityAddress != charityAddress_, "New charity address must be different than old one.");

        if (!_isExcludedFromReward[charityAddress_])
            excludeAccountFromReward(charityAddress_);

        excludeAccountFromFee(charityAddress_);

        address previous = charityAddress;
        charityAddress = charityAddress_;

        emit CharityAddressUpdated(previous, charityAddress_);
    }
...
contract CSEA is ExtendedReflections {

    uint256 private _tokenSupply = 1_000_000_000_000;

    uint32 private _taxReward = 5;
    uint32 private _taxCharity = 3;
    uint32 private _taxLiquify = 2;

    uint32 private _taxDecimals = 0;
    uint32 private _decimals = 18;

    uint256 private _minTokensBeforeSwap = 1_000_000 * (10 ** _decimals);
    ...
    constructor (address _charityAddress) ExtendedReflections("CleanSea", "CSEA", _decimals, _tokenSupply) {
        enableReward(_taxReward, _taxDecimals);
        enableCharityTax(_taxCharity, _taxDecimals, _charityAddress);
