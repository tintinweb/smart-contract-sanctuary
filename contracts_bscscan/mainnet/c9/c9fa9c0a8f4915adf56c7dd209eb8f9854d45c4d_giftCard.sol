...
contract giftCard is ERC721, Ownable {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _cost, string memory _uri) ERC721(_name, _symbol) {
    ...
    function mintWithBnb(string memory _message) external payable returns(uint256) {
        
        require(msg.value == cost, "WRONG AMOUNT");
        
        uint256 indexID = totalSupply();
        _safeMint(msg.sender, indexID);
        _setTokenURI(indexID, uri);
        cards.push(giftInfo(_message));
        
        return(indexID);

    }
    
    function viewInfo(uint256 nftIndex) external view returns(string memory) {
        return(cards[nftIndex].message);
    }
