    ...
    OwnableUpgradeable
{
    ...
    mapping(address => uint256) public usersFirstDateOnTaxZero;
    mapping(address => uint256) public usersDaysOfHold;
    mapping(address => uint256) public usersBonusHolderLevel;
    uint8 public holdDays;
    address public bonusHolderContractAddress;
    mapping(address => uint256) public referalDepositDateBeforeClaim;
    uint256 public referalRewardDaysLocked;
    mapping(address => bool) public usersExcluded;
    ...
    event BuyLifeWithClaim(address indexed wallet, uint256 indexed amount);
    ...
    function initialize(
        ...
        address bonusHolderContractAddress_
    ) public initializer {
        __Ownable_init();
        isInitialized = true;
        ...
        bonusHolderContractAddress = bonusHolderContractAddress_;
    ...
    function addReward(address _userAddress, uint256 _rewardAmount) external {
        ...
        if (usersFirstDateOnTaxZero[_userAddress] > 0) {
            usersDaysOfHold[_userAddress] = DateTimeLibrary.diffDays(
                usersFirstDateOnTaxZero[_userAddress],
                block.timestamp
            );
        }
    ...
    function claimReward() external nonReentrant {
        ...
        require(!usersExcluded[msg.sender], "User banned");
        require(
        ...
        usersBonusHolderLevel[msg.sender] = 0;
        usersFirstDateOnTaxZero[msg.sender] = 0;
    ...
    function addReferalReward(
        ...
        if (referalDepositDateBeforeClaim[_userAddress] == 0) {
            referalDepositDateBeforeClaim[_userAddress] = block.timestamp;
        }
    ...
    function claimReferalReward(uint256 _amountToClaim) external nonReentrant {
        require(usersReferalAmount[msg.sender] > 0, "Not amount to claim");
        require(
            _amountToClaim <= usersReferalAmount[msg.sender],
            "Insufficient amount"
        );
        uint256 _days = DateTimeLibrary.diffDays(
            referalDepositDateBeforeClaim[msg.sender],
            block.timestamp
        );
        require(_days >= referalRewardDaysLocked, "You can't claim now");
        ...
        referalDepositDateBeforeClaim[msg.sender] = 0;
        emit ClaimReferalReward(msg.sender, _amountToClaim);
    }

    function increaseBonusHolder(address _userAddress) external {
        require(msg.sender == bonusHolderContractAddress, "Caller unauthorized");
        usersBonusHolderLevel[_userAddress] += 1;
    }

    function decreaseDaysOfHold(address _userAddress, uint256 _amountToDecrease) external {
        require(msg.sender == bonusHolderContractAddress, "Caller unauthorized");
        usersDaysOfHold[_userAddress] -= _amountToDecrease;
    }
    ...
    function decreasePenalizationAmount(
        ...
        if (usersTaxAmount[_userAddress] == 0) {
            usersFirstDateOnTaxZero[_userAddress] = block.timestamp;
            usersBonusHolderLevel[_userAddress] = 0;
        }
        ...
        nonReentrant
    {
        ...
        /*
        if(BonusHolder(bonusHolderContractAddress).haveFenrirReliquary(msg.sender)){
            _amountCWOLF = _amountCWOLF - (_amountCWOLF * 300 / 10000);
        }*/
    ...
    function changeHoldDays(uint8 _newHoldDays) external onlyOwner {
        holdDays = _newHoldDays;
    }

    function changeBonusHolderContractAddress(address _newAddress)
        external
        onlyOwner
        returns (bool)
    {
        bonusHolderContractAddress = _newAddress;
        return true;
    }

    function changeReferalRewardDaysLocked(uint256 _newReferalRewardDaysLocked)
        external
        onlyOwner
        returns (bool)
    {
        referalRewardDaysLocked = _newReferalRewardDaysLocked;
        return true;
    }

    function changeLevelsPercentages(uint8 _level, uint256 _newPercentage) external onlyOwner {
        levelsPercentages[_level] = _newPercentage;
    }

    function setExcludeUser(address _userToExclude) external onlyOwner {
        usersExcluded[_userToExclude] = true;
    }
}
    ...
    OwnableUpgradeable
{
    ...
    address public bonusHolderContractAddress;
    mapping(uint256 => uint256) public wolfPackInitialLife;
        ...
        returns (bool)
    {
        ...
        wolfPackInitialLife[_wolfPackId] = wolfPackLife[_wolfPackId];
    ...
    function increaseWolfPackLife(
        ...
        require(
            msg.sender == inventoryContractAddress ||
                msg.sender == claimContractAddress ||
                msg.sender == bonusHolderContractAddress,
...
/*
    ...
    */
    ...
    function setInitialWolfPackLife(uint256[] memory _wolfPackIds, uint256[] memory _amountOfLife) external {
        require(msg.sender == minterWalletAddress, "Invalid account");
        for(uint256 i=0; i < _wolfPackIds.length; i++){
            wolfPackInitialLife[_wolfPackIds[i]] = _amountOfLife[i];
        }
    } 
    ...
    function changeBonusHolderContractAddress(
        address _newBonusHolderContractAddress
    ) external onlyOwner {
        bonusHolderContractAddress = _newBonusHolderContractAddress;
    }
    ...
    OwnableUpgradeable
{
    ...
    address public claimContractAddress;
    address public wolfPacksNFTContractAddress;
    address public minterWalletAddress;
    address public variablesContractAddress;
    mapping(address => uint256) public usersBonusReceived;
    mapping(address => bool) public usersHaveBonus;
    uint256 public gasToMinter;
    mapping(address => bool) public haveFenrirReliquary;

    constructor() initializer {}

    function initialize(
        address claimContractAddress_,
        address wolfPacksNFTContractAddress_
    ) public initializer {
        __Ownable_init();
        isInitialized = true;

        claimContractAddress = claimContractAddress_;
        wolfPacksNFTContractAddress = wolfPacksNFTContractAddress_;
        // Uncomment if we want deploy paused
        // _pause();
    }

    function claimBonusHolder() external payable {
        /*
        require(
            Claim(claimContractAddress).usersTaxAmount(msg.sender) == 0,
            "Illegal tax"
        );
        require(
            Claim(claimContractAddress).usersDaysOfHold(msg.sender) >=
                Claim(claimContractAddress).holdDays(),
            "Insufficient hold days"
        );
        require(
            Claim(claimContractAddress).usersBonusHolderLevel(msg.sender) ==
                0,
            "No more bonus"
        );*/
        if(Claim(claimContractAddress).usersBonusHolderLevel(msg.sender) == 0) {
            useOakAmulet(msg.sender);
        }
        else if(Claim(claimContractAddress).usersBonusHolderLevel(msg.sender) == 1) {
            useFenrirReliquary(msg.sender);
        }
    }

    function setDataBonusHolder(address _userAddress) internal {
        Claim(claimContractAddress).increaseBonusHolder(_userAddress);
        uint256 holdDays = Claim(claimContractAddress).holdDays();
        if (
            holdDays > Claim(claimContractAddress).usersDaysOfHold(_userAddress)
        ) {
            Claim(claimContractAddress).decreaseDaysOfHold(
                _userAddress,
                Claim(claimContractAddress).usersDaysOfHold(_userAddress)
            );
        } else {
            Claim(claimContractAddress).decreaseDaysOfHold(
                _userAddress,
    ...
    function useOakAmulet(address _userAddress) internal {
        require(msg.sender == claimContractAddress, "Caller not permission");
        uint256[] memory wolfPackIds = WolfPacksNFT(wolfPacksNFTContractAddress).walletOfOwner(_userAddress);
        for(uint256 i = 0; i < wolfPackIds.length; i++){
            uint256 _initialLife = WolfPacksNFT(wolfPacksNFTContractAddress).wolfPackInitialLife(wolfPackIds[i]);
            uint256 _life = WolfPacksNFT(wolfPacksNFTContractAddress).wolfPackLife(wolfPackIds[i]);
            uint256 _amountToIncrease = _life * 100 / 10000;
            increaseLife(_initialLife, _life, _amountToIncrease, wolfPackIds[i]);
        }
    }

    function increaseLife(uint256 _initialLife, uint256 _life, uint256 _amountToIncrease, uint256 _wolfPackId) internal {
        if((_life + _amountToIncrease) >= _initialLife) {
            WolfPacksNFT(wolfPacksNFTContractAddress).increaseWolfPackLife(_wolfPackId, _initialLife - _life);    
        }else{
            WolfPacksNFT(wolfPacksNFTContractAddress).increaseWolfPackLife(_wolfPackId, _amountToIncrease);
        }
    }

    function useFenrirReliquary(address _userAddress) public {
        require(msg.sender == minterWalletAddress, "Caller unauthorized");
        haveFenrirReliquary[_userAddress] = true;
        setDataBonusHolder(_userAddress);
    }

    function setFenrirReliquiary(address _userAddress) external onlyOwner {
        haveFenrirReliquary[_userAddress] = true;
    }

    function changeClaimContractAddress(address _newClaimContractAddress)
        external
        onlyOwner
    {
    ...
    function changeWolfPacksNFTContractAddressAddress(
        address _newWolfPacksNFTContractAddress
    ) external onlyOwner {
        wolfPacksNFTContractAddress = _newWolfPacksNFTContractAddress;
    }

    function changeMinterWalletAddress(
        address _newMinterWalletAddress
    ) external onlyOwner {
        minterWalletAddress = _newMinterWalletAddress;
    }

    function changeVariablesContractAddress(
        address _newVariablesContractAddress
    ) external onlyOwner {
        variablesContractAddress = _newVariablesContractAddress;
    }

    function changeGasToMinter(
        uint256 _newGasToMinter
    ) external onlyOwner {
        gasToMinter = _newGasToMinter;
    }
}
    ...
    OwnableUpgradeable
{
        ...
        whenNotPaused
    {
        ...
        emit ChangePrice(_id, tokenAddress[_id], tokenId[_id], _amountInDollars);
    }
