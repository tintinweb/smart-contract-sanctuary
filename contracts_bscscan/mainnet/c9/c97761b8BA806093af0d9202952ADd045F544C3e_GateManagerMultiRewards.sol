...
contract GateManagerMultiRewards is ReentrancyGuardUpgradeable, Ownable {
    ...
    // Returns the latest reward tokens reward rate
    function rewardRate() external view returns (uint256) {
        uint256 id = rewardInfo.length - 1;
        return rewardInfo[id].rewardRate;
    }
    ...
    // returns earned amount of latest reward token added
    function earned(address account) external view returns (uint256) {
        return(earned(account, rewardInfo.length - 1));    
    }
        ...
        returns (uint256)
    {
        ...
        return _withdrawMoonPotYield(sharesAmount);
    }
    ...
    function getReward(uint256 id) public updateReward(msg.sender) {
    ...
    // User claims all avaialable rewards
    ...
    // Sets new reward duration for exisiting reward token 
    function setRewardDuration(uint256 id, uint256 rewardDuration) external onlyOwner {
            ...
            rewardInfo[id].stakingRewardsDuration = rewardDuration;
    }
        ...
        updateReward(address(0))
    {
        ...
        if (block.timestamp >= info.periodFinish) {
            info.rewardRate = reward.div(info.stakingRewardsDuration);
        } else {
            uint256 remaining = info.periodFinish.sub(block.timestamp);
            uint256 leftover = remaining.mul(info.rewardRate);
            info.rewardRate = reward.add(leftover).div(
                info.stakingRewardsDuration
            );
        }
        info.lastUpdateTime = block.timestamp;
        info.periodFinish = block.timestamp.add(info.stakingRewardsDuration);
