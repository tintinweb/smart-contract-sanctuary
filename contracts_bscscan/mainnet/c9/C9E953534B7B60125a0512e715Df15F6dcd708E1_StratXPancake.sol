...
abstract contract StratX2 is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public controllerFee = 20; // 70;
    uint256 public constant controllerFeeMax = 10000; // 100 = 1%
    uint256 public constant controllerFeeUL = 300;

    uint256 public buyBackRate = 200; // 250;
    ...
    uint256 public slippageFactor = 5; // 5% default slippage tolerance
    ...
    function earn() public virtual nonReentrant whenNotPaused {
        ...
        if (token0Amt > 0 && token1Amt > 0) {
           ...
           (uint amountA, uint amountB, uint liquidity) = IPancakeRouter02(uniRouterAddress).addLiquidity(
                token0Address,
                token1Address,
                token0Amt,
                token1Amt,
...
contract StratXPancake is StratX2 {
   function setData (
        ...
        bool _isAutoComp,
        address[] memory _earnedToCRUDEPath,
        ...
        crudeFarmAddress = _addresses[2];
        CRUDEAddress = _addresses[3];
        ...
        earnedToCRUDEPath = _earnedToCRUDEPath;
