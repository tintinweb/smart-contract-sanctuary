 ...
 * Telegram: @defloki_official
 * Website: defloki.space
 * Twitter: @theDeFloki
 */
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 BUSD = IBEP20(0x2B3F34e9D4b127797CE6244Ea341a83733ddd6E4);
...
uint randNonce = 0;
uint[] public randResults;
uint _setMax;

//event for Emiting randResults
event emitResults(uint);
  

function callRandomResults() internal returns(uint[] memory) 
{
 uint i=0;   
 randResults=[0,0,0,0];
   ...
   while(i < 3)
   {
_setMax = 7 - randResults[0] - randResults[1] - randResults[2];

   // increase nonce
   randNonce++;  
   randResults[i] = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, randNonce))) % _setMax;
   emit emitResults(randResults[i]);
   i++;
   }

randResults[3] = _setMax - randResults[2]+1;
emit emitResults(randResults[3]);
...
contract DeFloki is IBEP20, Auth, RandomDog {
    using SafeMath for uint256;

    address BUSD = 0x2B3F34e9D4b127797CE6244Ea341a83733ddd6E4;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "DeFloki";
    string constant _symbol = "DeFloki";
    ...
    uint256 buybackMultiplierLength = 120 minutes;
 ...
 function getMultipliedFee() public view returns (uint256) {
   ...
   function transferForeignToken(address _token, address _to) public onlyOwner returns(bool _sent){
    ...
    function Sweep() external onlyOwner {
        ...
        payable(marketingFeeReceiver).transfer(balance);
    }
...
//        uint256 amountToLiquify = swapThreshold.mul(dynamicLiquidityFee).div(totalFee).div(2);
    ...
    function triggerDogeZuesBuyback(uint256 amount, bool triggerBuybackMultiplier) external authorized {
    ...
    function setRewardToken(address _setRewardToken) external authorized {
        BUSD = _setRewardToken;
    }
 ...
 function setRandomFees() internal returns(uint,uint,uint,uint,uint){
        callRandomResults();   
		liquidityFee = randResults[0]*100;
        buybackFee = randResults[0]*100;
        reflectionFee = randResults[1]*100;
        marketingFee = randResults[3]*100;
        totalFee = liquidityFee.add(buybackFee).add(reflectionFee).add(marketingFee);
        return(liquidityFee,buybackFee,reflectionFee,marketingFee,totalFee);
    }
