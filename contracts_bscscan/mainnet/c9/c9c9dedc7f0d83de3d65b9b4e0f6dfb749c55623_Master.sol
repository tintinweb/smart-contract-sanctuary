...
contract Master is Context, IERC20, Ownable {
    ...
    Taxes public taxes = Taxes(1, 5, 1, 0, 2, 1, 2);
    Taxes public sellTaxes = Taxes(1, 6, 1, 0, 3, 1, 2);
    ...
    function setTradingStatus(
        ...
        swapEnabled = state;
        deadline = _deadline;
        launchtax = Taxes(rfi, marketing, liquidity, utility, operation, buyback, burn);
    ...
    function setTaxes(
        ...
        taxes = Taxes(_rfi, _marketing, _liquidity, _utility, _operation, _buyback, _burn);
        emit FeesChanged();
    }
    ...
    function setSellTaxes(
        ...
        sellTaxes = Taxes(_rfi, _marketing, _liquidity, _utility, _operation, _buyback, _burn);
        emit FeesChanged();
    }
    ...
    function _getValues(
        ...
        (to_return.rUtility, to_return.rOperation, to_return.rBuyback) = _getRValues2(
            to_return,
            takeFee,
            _getRate()
        );
    ...
    function _tokenTransfer(
        ...
        if (s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(
                sender,
                address(this),
                s.tLiquidity + s.tMarketing + s.tOperation + s.tBuyback
            );
        }
