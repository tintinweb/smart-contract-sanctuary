...
contract Token is BEP20 {
    ...
    // Min amount to liquify. (default 500 Tokens)
    ...
    // Masterchef addresses to whitelist in transfer fees to help stakers to compound
    address public masterchefFire = address(0);
    address public masterchefIce = address(0);
    ...
    event MasterchefUpdated(address indexed operator, address indexed masterchefFire, address indexed masterchefIce);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (recipient == BURN_ADDRESS || recipient == masterchefFire || sender == masterchefFire || recipient == masterchefIce || sender == masterchefIce || transferTaxRate == 0) {
    ...
    function updatePancakeSwapRouter(address _router) public onlyOperator {
        require(address(pancakeSwapRouter) == address(0), "TOKEN::updatePancakeSwapRouter: Router address can only be initialized 1 time.");
    ...
    /**
     * @dev Update the masterchef addresses.
     * Can only be called by the current operator.
     */
    function initMasterchef(address _masterchefFire, address _masterchefIce) public onlyOperator {
        require(masterchefFire == address(0), "TOKEN::initMasterchef: Masterchef addresses can only be initialized 1 time.");
        masterchefFire = _masterchefFire;
        masterchefIce = _masterchefIce;
        emit MasterchefUpdated(msg.sender, _masterchefFire, _masterchefIce);
    }
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, uint256 _harvestInterval, address _nftRequiredAddress, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
    ...
    function canUsePool(uint256 _pid, address _user) public view poolExists(_pid) returns (bool) {
        PoolInfo storage pool = poolInfo[_pid];
        return pool.nftRequiredAddress == address(0) || IBEP721(pool.nftRequiredAddress).balanceOf(_user) >= 1;
    }
    ...
    function updatePool(uint256 _pid) poolExists(_pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(tokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        token.mint(devAddress, tokenReward.div(50));
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant poolExists(_pid) {
        ...
        require(pool.nftRequiredAddress == address(0) || IBEP721(pool.nftRequiredAddress).balanceOf(msg.sender) >= 1, "deposit: This pool requires a specific NFT in your wallet");
        updatePool(_pid);
        ...
        if (_amount > 0) {
            ...
            _amount = _after.sub(_before); // correct calculation for Token with Transfertax
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant poolExists(_pid) {
        ...
        updatePool(_pid);
        if (pool.nftRequiredAddress == address(0) || IBEP721(pool.nftRequiredAddress).balanceOf(msg.sender) >= 1) {
            payOrLockupPendingToken(_pid);
        }
        if (_amount > 0) {
    ...
    function setTokenReferral(ITokenReferral _tokenReferral) public onlyOwner {
    ...
    // Update start block (USE ONLY IF BSC CREATES A TOO BIG BLOCK SHIFT)
    function updateStartBlock(uint256 _startBlock) public onlyOwner {
        require(startBlock > block.number, "updateStartBlock: farming already started");
        require(_startBlock > block.number, "updateStartBlock: must be future");
        startBlock = _startBlock;
