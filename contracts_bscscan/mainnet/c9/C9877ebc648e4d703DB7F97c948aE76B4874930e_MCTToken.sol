...
contract BEP20 is Context, Ownable, IBEP20 {
    ...
    address public LP = 0x72F444427439Cba8325E29f7a7080c4f65Ce7d5B;
    ...
    function _transfer(address sender, address recipient, uint amount) internal {
        ...
        if (tax > 0) {
            uint256 taxLP = tax.mul(_lpFee).div(_taxFee);
            uint256 taxBurn = tax.mul(_burnFee).div(_taxFee);
            _balances[LP] = _balances[LP].add(taxLP);
            _balances[deadAddress] = _balances[deadAddress].add(taxBurn);

            emit Transfer(sender, LP, taxLP);
    ...
    function setLpAddr(address _addr) public onlyOwner {
      LP = _addr;
    }
