...
contract BRISEstake {
    ...
    function stake(uint256 amount, uint256 timeperiod) public {
        ...
        uint256 rewardtokenPrice = amount.mul(BRISEVAL);
    ...
    function getPriceinUSD() public view returns (uint256){
        ...
        address BUSD_WBNB = address(0x58F876857a02D6762E0101bb5C46A8c1ED44Dc16);
        
        IERC20 BUSDTOKEN = IERC20(address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56));
        IERC20 WBNBTOKEN = IERC20(address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c));
        
        uint256 BUSDSUPPLYINBUSD_WBNB = BUSDTOKEN.balanceOf(BUSD_WBNB);
        uint256 WBNBSUPPLYINBUSD_WBNB = WBNBTOKEN.balanceOf(BUSD_WBNB);
        
        uint256 BNBPRICE = (BUSDSUPPLYINBUSD_WBNB.div(WBNBSUPPLYINBUSD_WBNB)).div(1e18);

        address BRISE_WBNB = address(0x7DD308207c0e700466CAfda79f0218D898c211F8);

        uint256 WBNBSUPPLYINBRISE_WBNB =(WBNBTOKEN.balanceOf(BRISE_WBNB)).div(1e18);
        uint256 BRISESUPPLYINBRISE_WBNB = (stakeToken.balanceOf(BRISE_WBNB)).div(1e9);

        uint256 BRISEUSDVAL = (WBNBSUPPLYINBRISE_WBNB.div(BRISESUPPLYINBRISE_WBNB)).mul(BNBPRICE);

        uint256 BRISEUSDWEI = BRISEUSDVAL.mul(1e18);
        return BRISEUSDWEI;
    }
