...
/**
 * Only coin in the world with trustworthy and automated charity function built into the contract so no one can manipulate
 *
 * Total 10% transaction fee:
 *	5% goes to Liquidity Pool (LP)
 *	4% gets redistributed among holders
 *	1% is converted to BNB and sent to the Binance chartiy wallet which can be verified using the Binance link given below 
 *	https://www.binance.charity/binance-charity-wallet/donate-anonymously
 * 
 * Website - https://spacegod.org/
 * Join us at Telegram - https://t.me/SpacegodOrg
 * Join us at Discord - https://discord.gg/QXCw2u3j4t
 * Who is behind Spacegod: Hyperactive team of computer nerds of two decades as of 2021
 * 
 */
...
contract Spacegod is Context, IERC20, Ownable {
    ...
    address public _charityWalletAddress = 0x8B99F3660622e21f2910ECCA7fBe51d654a1517D;
	// See https://www.binance.charity/binance-charity-wallet/donate-anonymously to validate this is the correct Address
    ...
    bool private limitWhaling = false;
    ...
    constructor (
		 ...
		 // Pancakeswap Router in BSC Testnet
         //   Factory: 0x6725F303b657a9451d8BA641348b6761A6CC7a17
         //   Router: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1     
         // Pancakeswap Router in BSC Mainet           
		...
		_charityFee = _CHARITYFEERATE;
		_previousCharityFee = _CHARITYFEERATE;
		
        _maxTxAmount = _MAXAMOUNT * 10 ** _decimals;
		_previousMaxTxAmount = _maxTxAmount;
		
		        
		// Number of tokens in the contract that will activate swap and liquifiy. It will sell half of that
		// token amount in the contract to the Dex Router that is assigned (pancakeswap in this case) for BNB. It will then re-add it to the LP.
    ...
    function beginLimitWhaling() public onlyOwner() {
        limitWhaling = true;
    }
    
    function stopLimitWhaling() public onlyOwner() {
        limitWhaling = false;
    }
    ...
    function _transfer(
        ...
        if (limitWhaling) {
            if(from != owner() && to != owner()) {
                require(amount <= (totalSupply()) * (5) / (10**3), "Transfer amount exceeds 0.5% of the supply!");
            }
            if(to == address(uniswapV2Pair) || to == address(uniswapV2Router)) {
                ...
                if (fromBalance > threshold) {
                    ...
                    require(amount < fromBalance / (5), "For security, max sell is 20% if you hold 0.5% or more of supply!");
                    require( _now - (previousSale[from]) > 1 days, "Wait 24 hours before you may sell again.");
                }
            }
        }
	...
	function manualBurn(uint256 amount, bool shouldTakeFee) public onlyOwner returns (bool) {
        _tokenTransfer(_msgSender(), address(0), amount, shouldTakeFee);
        return true;
    }
    ...
    function updateRouterAddress(address newRouter) public onlyOwner {
