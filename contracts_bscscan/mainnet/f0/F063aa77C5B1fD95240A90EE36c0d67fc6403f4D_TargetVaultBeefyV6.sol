...
import "./TargetVaultBeefy.sol";

contract TargetVaultBeefyV6 is TargetVaultBeefy {
    ...
    constructor(
        ...
        address[] memory _tokenToBuyBackPath
    ) public {
        token = IERC20(_token);
        ...
        tokenToBuyBackPath = _tokenToBuyBackPath;
    }
    ...
    function vaultToken() public view override returns (address) {
        return IBeefyVault(beefyVault).want();
    }

    /**
     * @dev Set Token to Buyback Path
     */
    function setTokenToBuyBackPath(address[] memory _tokenToBuyBackPath) public virtual onlyOwner {
        tokenToBuyBackPath = _tokenToBuyBackPath;
    }
}
...
import "@openzeppelin/contracts/math/SafeMath.sol";
import "@openzeppelin/contracts/token/ERC20/SafeERC20.sol";
import "../../../interfaces/targetVaults/protocols/IBeefyVault.sol";
import "../TargetVault.sol";

contract TargetVaultBeefy is TargetVault {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    /**
     * @dev beefyVault address
     */
    ...
    function deposit() external virtual onlyVault {
        ...
        if (_balance > 0) {
            IERC20(token).safeIncreaseAllowance(address(beefyVault), _balance);
            beefyVault.deposit(_balance);
    ...
    function _collectFees() internal virtual {
        uint256 _newPricePerShare = beefyVault.getPricePerFullShare();
        if (cachedPricePerShare > 0 && cachedPricePerShare < _newPricePerShare) {
            uint256 _balance = beefyVault.balanceOf(address(this));
            ...
            if (_amount > 10**minimumFeesDecimal) beefyVault.withdraw(_amount);
        ...
        cachedPricePerShare = beefyVault.getPricePerFullShare();
    }
    ...
    function vaultToken() public view virtual returns (address) {
        return address(beefyVault.token());
    }
}
