...
/**
PUMPKIN SHIBA it’s a best token with a solid and great rewards, & it’s a unique new project on the Binance Smart Chain.
The longer holding your PUMPKIN SHIBA Tokens, the more SHIBA you get as reward, Huge Marketing,100% SAFU project.

/http://pumpkinshiba.org
/TG:https://t.me/pumpkinshiba
*/
...
contract PUMPKINSHIBA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PUMPKINSHIBADividendTracker public dividendTracker;
    ...
    constructor() ERC20("PumpkinShiba", "PUMPKINSHIBA") {
    	...
    	dividendTracker = new PUMPKINSHIBADividendTracker();

    	buyBackWallet = 0x929F70481a612B7B87FA6Fc0B21ca9193312E37F;
    	
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //0x1B0Be4927D9e75968DEE5d198Efd1c7Ed1062295
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "PUMPKINSHIBA: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "PUMPKINSHIBA: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "PUMPKINSHIBA: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PUMPKINSHIBA: The dividend tracker already has that address");

        PUMPKINSHIBADividendTracker newDividendTracker = PUMPKINSHIBADividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "PUMPKINSHIBA: The new dividend tracker must be owned by the PUMPKINSHIBA token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "PUMPKINSHIBA: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PUMPKINSHIBA: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PUMPKINSHIBA: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PUMPKINSHIBA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PUMPKINSHIBA: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "PUMPKINSHIBA: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PUMPKINSHIBA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PUMPKINSHIBA: Cannot update gasForProcessing to same value");
...
contract PUMPKINSHIBADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PUMPKINSHIBA_Dividend_Tracker", "PUMPKINSHIBA_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "PUMPKINSHIBA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "PUMPKINSHIBA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PUMPKINSHIBA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "PUMPKINSHIBA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PUMPKINSHIBA_Dividend_Tracker: Cannot update claimWait to same value");
