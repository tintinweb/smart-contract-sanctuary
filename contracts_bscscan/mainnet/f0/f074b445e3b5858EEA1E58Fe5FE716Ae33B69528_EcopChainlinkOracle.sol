...
/**
 * @title ECOP's CErc20 Contract
 * @notice ETokens which wrap an EIP-20 underlying
 * @author ECOP
 */
contract CErc20 is EToken, CErc20Interface {
    /**
    ...
    function initialize(address underlying_,
        ...
        // EToken initialize does the bulk of the work
        super.initialize(ecoptroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_);
    ...
    function liquidateBorrow(address borrower, uint repayAmount, ETokenInterface eTokenCollateral) external returns (uint) {
        (uint err,) = liquidateBorrowInternal(borrower, repayAmount, eTokenCollateral);
        return err;
    }
    ...
    * @param ecopLikeDelegatee The address to delegate votes to
    * @dev ETokens whose underlying are not EcopLike should revert here
    */
    function _delegateEcopLikeTo(address ecopLikeDelegatee) external {
        require(msg.sender == admin, "only the admin may set the ecop-like delegate");
        EcopLike(underlying).delegate(ecopLikeDelegatee);
    }
}
...
contract EToken is ETokenInterface, Exponential, TokenErrorReporter {
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setEcoptroller(EcoptrollerInterface newEcoptroller) public returns (uint) {
...
contract ETokenStorage {
     ...
     */
    EcoptrollerInterface public ecoptroller;
...
contract ETokenInterface is ETokenStorage {
    ...
    /**
     * @notice Event emitted when ecoptroller is changed
     */
    event NewEcoptroller(EcoptrollerInterface oldEcoptroller, EcoptrollerInterface newEcoptroller);
...
contract EcopChainlinkOracle is PriceOracle {
    ...
    function getUnderlyingPrice(EToken eToken) public view returns (uint) {
        ...
        } else if (compareStrings(symbol, "eECOP")) {
