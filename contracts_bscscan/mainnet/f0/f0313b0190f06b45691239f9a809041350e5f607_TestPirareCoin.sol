...
contract TestPirareCoin is Context, IBEP20, Ownable {	
    ...
    string private _name = "Test Pirate Coin";	
    string private _symbol = "TestPirateCoin \xe2\x98\xa0";
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {	
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tLotto, uint256 tDev, uint256 tMar) = _getValues(tAmount);	
    ...
    function setMarAddress(address payable mar) public onlyOwner() {
        _marWallet = mar;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {	
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tLotto, data.tDev, data.tMar);	
    }	
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {	
		...
		uint256 tDev = calculateDevFee(tAmount);
				uint256 tMar = calculateMarFee(tAmount);
				
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity);
                tTransferAmount = tTransferAmount.sub(tLotto);
                tTransferAmount = tTransferAmount.sub(tDev);
                tTransferAmount = tTransferAmount.sub(tMar);	
        return (tTransferAmount, TData(0, tFee, tLiquidity, tLotto, tDev, tMar, 0));	
    }	
    ...
    function _takeMar(uint256 tMar) private {
        uint256 currentRate =  _getRate();
        uint256 rMar = tMar.mul(currentRate);

        if (_shouldSwapToBNB) {
            swapTokensForEth(rMar);
            _marWallet.transfer(address(this).balance);
        } else {
            _rOwned[_marWallet] = _rOwned[_marWallet].add(rMar);
            if(_isExcluded[_marWallet])
                _tOwned[_marWallet] = _tOwned[_marWallet].add(tMar);
        }
    }
    ...
    function removeAllFee() private {	
        if(_taxFee == 0 && _liquidityFee == 0 && _devFee == 0 && _marFee == 0 && _lottoFee == 0) return;	
