...
contract FarmLP is FarmHashrate, IFarmLP, Initializable, Ownable, Recoverable {
    ...
    function stakeLP(uint256 amount) external override needInit {
        ...
        userHashrateRecords[_msgSender()].push(Model.HashrateRecord({
            category: Model.CATEGORY_LP,
            blockNumber: block.number,
            timestamp: block.timestamp,
            totalHashrate: lpHashrateMap[_msgSender()]
        }));
        ...
        if (hc.rebate == 1)
        {
            ...
            for (uint8 gen = 1; gen <= 20; gen++) {
                ...
                lpHashrateMap[genUserAddr] = SafeMath.add(lpHashrateMap[genUserAddr], hashrateCommission);
                userHashrateRecords[genUserAddr].push(Model.HashrateRecord({
                    category: Model.CATEGORY_LP,
                    blockNumber: block.number,
                    timestamp: block.timestamp,
                    totalHashrate: lpHashrateMap[genUserAddr]
                }));    
    ...
    function unstakeLP() external override needInit {
        ...
        for (uint8 gen = 1; gen <= 20; gen++) {
            ...
            lpHashrateMap[genUserAddr] = SafeMath.sub(lpHashrateMap[genUserAddr], hashrateCommission);
            userHashrateRecords[genUserAddr].push(Model.HashrateRecord({
                category: Model.CATEGORY_LP,
                blockNumber: block.number,
                timestamp: block.timestamp,
