...
// Sun is the master of Rainbow. Sun creates light for life and is very sincere.
contract Sun is Ownable , ReentrancyGuard{
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // Info of each user.
    struct UserInfo {
        uint256 amount;   // How many RBW tokens the user has provided.
        uint256 rewardDebt;  // Reward debt. See explanation below.
        uint256 rewardPending;
        //
        // We do some fancy math here. Basically, any point in time, the amount of RBWs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accRewardPerShare) - user.rewardDebt + user.rewardPending
        //
        // Whenever a user deposits or withdraws RBW tokens to a pool. Here's what happens:
    ...
    // The RBW TOKEN!
    IBEP20 public rainbow;
    // rewards created per block.
    uint256 public rewardPerBlock;

    // Info.
    PoolInfo public poolInfo;
    // Info of each user that stakes Rainbow tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = rainbow.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 rainbowSupply = rainbow.balanceOf(address(this));
        if (rainbowSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(rainbowSupply));
        poolInfo.lastRewardBlock = block.number;
    }


    // Deposit Rainbow tokens to Sun for Reward allocation.
    function deposit(uint256 _amount) public nonReentrant {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        rainbow.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Rainbow tokens from Sun.
    function withdraw(uint256 _amount) public nonReentrant {
        ...
        updatePool();
        rainbow.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        rainbow.safeTransfer(address(msg.sender), user.amount);
