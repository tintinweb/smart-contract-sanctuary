...
// File: .deps/github/Loesil/VaultChef/contracts/interfaces/IBank.sol
...
interface IBank
{
    ...
    function getPoolPayoutRate(uint256 _pid) external view returns(uint256);
    
    function depositFromVault(uint256 _pid, uint256 _amount) external;
}

// File: .deps/github/Loesil/VaultChef/contracts/VaultChef.sol

contract VaultChef is IVaultChef, ReentrancyGuard, AccessControl
{
	...
	address public constant DEFAULT_TREASURY = 0x4348Bbb326C67742D90F87cEd103badAB867ffbE;
	
	uint256 public constant override PERCENT_FACTOR = 1000000; //100%

    uint256 public constant MAX_REWARD_FEE = 50000; // 5%
    
    uint256 public constant MAX_CLAIM_REWARD = 10000; //1%	
    ...
    uint256 public rewardFee = 20000; // 2%
    uint256 public claimReward = 5000; // 0.5%
	...
	event EmergencyWithdraw(uint256 indexed _vid, address indexed _user, uint256 _amount);
    ...
    constructor()
    {
        ...
        _setRoleAdmin(ROLE_DEPLOYER, ROLE_ADMIN);
    ...
    {
        requireRole_Admin();
        if (_fee < MAX_REWARD_FEE)
        {
            rewardFee = _fee;
        }
    }
    
    function setClaimReward(uint256 _reward) external
    {
        requireRole_Admin();
        if (_reward < MAX_CLAIM_REWARD)
        {
            claimReward = _reward;
        }
    }
    ...
    function pause(bool _disableDeposit, bool _disableWithdraw, bool _disableCompound) external
    {
        ...
        emit Pause(msg.sender, _disableDeposit, _disableWithdraw, _disableCompound);
    }
    ...
    function unpause(bool _enableDeposit, bool _enableWithdraw, bool _enableCompound) external
    {
        ...
        emit Unpause(msg.sender, _enableDeposit, _enableWithdraw, _enableCompound);
    }
    ...
    function getVaultConfig() external override view returns(address, address, uint256, uint256)
    {
        return (treasury, bank, rewardFee, claimReward);
    }

    function addVault(address _vault) external
    {
        requireRole_Deployer();
        
        vaultMap[_vault] = vaultInfo.length;
        vaultInfo.push
        (
            VaultInfo(
			{
                depositToken: IVault(_vault).depositToken(),
                vault: _vault,
                enabled: true
            })
        );
    ...
    function setVault(uint256 _vid, address _vault) external
	{
	    requireRole_Deployer();
		require(_vid < vaultInfo.length, "invalid vault ID");
		
		VaultInfo storage vault = vaultInfo[_vid];
		vaultMap[vault.vault] = type(uint256).max; //destroy mapping
        vault.vault = _vault;
		vault.depositToken = IVault(_vault).depositToken();
		vaultMap[_vault] = _vid;
	}
	...
	function findMappedVaultID(address _vault) internal view returns(bool, uint256)
    {
        uint256 pid = vaultMap[_vault];
        VaultInfo storage vault = vaultInfo[pid];
        if (vault.vault == _vault)
        {
            return (true, pid);   
        }
	...
	{	
		VaultInfo storage vault = vaultInfo[_vid];	
		return (IERC20(vault.depositToken).allowance(_user, vault.vault) != 0);	
	}

	function getVaultStrategy(uint256 _vid) external view returns(address)
	{
		VaultInfo storage vault = vaultInfo[_vid];
        return vault.vault;
	}
	
	function isVaultMinable(uint256 _vid) public view returns(bool)
    {
        VaultInfo storage vault = vaultInfo[_vid];
        return (vault.enabled
            && IVault(vault.vault).isMinable());
    }

    //-------------------------------------------------------------------------
    // VAULT COMPOUND FUNCTIONS
    //-------------------------------------------------------------------------

    function compoundVault(uint256 _vid) public
    {
        require(!pauseCompound, "Compounds paused!");
        VaultInfo storage vault = vaultInfo[_vid];
        IVault(vault.vault).compound(msg.sender);
        emit Compound(_vid, msg.sender);
    }
	
	function tryCompoundVault(uint _vid) internal
	{
	    require(!pauseCompound, "Compounds paused!");
		VaultInfo storage vault = vaultInfo[_vid];
		if (IVault(vault.vault).tryCompound(msg.sender))
		{
			emit Compound(_vid, msg.sender);
		}
	}
    ...
    function deposit(uint256 _vid, uint256 _amount) external nonReentrant notContract
    {
        require(!pauseDeposit, "Deposits paused!");
        VaultInfo storage vault = vaultInfo[_vid];
		uint256 balance = IERC20(vault.depositToken).balanceOf(msg.sender);
		require(balance >= _amount, "insufficient balance");
        if (_amount > 0)
        {
            tryCompoundVault(_vid);
            IVault(vault.vault).deposit(msg.sender, _amount);
        }
        emit Deposit(_vid, msg.sender, _amount);
    }

    function withdraw(uint256 _vid, uint256 _amount) external nonReentrant notContract
    {
        ...
        {
            tryCompoundVault(_vid);
            _amount = IVault(vault.vault).withdraw(msg.sender, _amount);
        }
        emit Withdraw(_vid, msg.sender, _amount);
    }
	
	function emergencyWithdraw(uint256 _vid) external nonReentrant notContract
    {
        require(!pauseWithdraw, "Withdraws paused!");
        VaultInfo storage vault = vaultInfo[_vid];        
        uint256 amount = IVault(vault.vault).withdraw(msg.sender, type(uint256).max);
        emit EmergencyWithdraw(_vid, msg.sender, amount);
    }
    ...
    function getBankPoolPayoutRate() external override view returns(uint256)
    {
        (bool found, uint256 pid) = findMappedVaultID(msg.sender);
        if (bank != address(0)
            && found)
        {
            return IBank(bank).getPoolPayoutRate(pid);
        }
        
        return 0;
    }
    
    function depositToBankVault(uint256 _amount) external override
    {
        (bool found, uint256 pid) = findMappedVaultID(msg.sender);
        if (bank != address(0)
            && found)
        {
            IBank(bank).depositFromVault(pid, _amount);
        }
    }
	...
	function withdrawToTreasury(address _token) external
	{
		...
		//transfer (this contract should never have a balance)
    ...
    function requireRole_Deployer() internal view
    {
        require(hasRole(ROLE_ADMIN, msg.sender) || hasRole(ROLE_DEPLOYER, msg.sender), "Caller is not Admin/Deployer");
    }
