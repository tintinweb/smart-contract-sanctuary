...
contract HTRAXToken is ERC20, ERC20Snapshot, Pausable, AccessControl, HTRAXTokenSale {
    ...
    ERC20("HTRAX Token", "HTRAX")
    {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());

        // token release for phase-1
        _mint(_wallet, _release);
    }
    ...
    function addBlackList(address[] memory _address) public onlyRole(RISK_MANAGER_ROLE) {
        for (uint256 i = 0; i < _address.length; i++) {
            ...
            require(!hasRole(DEFAULT_ADMIN_ROLE,_address[i]), 'The address is from admin');
            require(!hasRole(RISK_MANAGER_ROLE,_address[i]), 'The address is from risk manager');            
...
abstract contract HTRAXTokenSale is AccessControl {
    ...
    uint128 private saleStartDate;
    uint128 private saleEndDate;
    uint256 private level1;
    uint256 private level2;
    uint256 private level3;
     ...
     */
    function setSaleDate(uint128 _startDate, uint128 _endDate) public onlyRole(PRESALES_MANAGER_ROLE) {
     ...
     */
    function getSaleDate() public view virtual returns (uint128, uint128) {
        return (saleStartDate, saleEndDate);
    }

    /**
     * @dev Caller with pre sales manager role can set token amount for level1, Leve2 and Level3.
     * @param _level1: token need to specify in format: _level1 *10^18
     * @param _level2: token need to specify in format: _level2 *10^18
     * @param _level3: token need to specify in format: _level3 *10^18
     */
    function setSaleLevel(uint256 _level1, uint256 _level2, uint256 _level3) public onlyRole(PRESALES_MANAGER_ROLE) {
        level1 = _level1;
        level2 = _level2;
        level3 = _level3;
    }
    ...
    /**
     * @dev returns Level1, Level2 and Level3 token amount in format _tokenAmount *10^18
     */
    function getSaleLevel() public view virtual returns (uint256, uint256, uint256) {
