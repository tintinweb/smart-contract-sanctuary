...
contract VestStream is Ownable, ReentrancyGuard {
  ...
  */
  ...
  /**
    Sweep all of a particular BEP-20 token from the contract beside LPI Token. 

    @param _token The token to sweep the balance from except LPI
  */
  function sweep(IBEP20 _token) external onlyOwner {
    require(_token != token, "Can't sweep LPI Token");
