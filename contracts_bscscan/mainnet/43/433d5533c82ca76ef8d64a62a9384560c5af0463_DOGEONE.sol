...
**
**
**    1% - Burned: sent directly to the burn address as SKYLANA. This burn function will stop when total supply of SKYLANA = 100 trillion.  Tax will now total only 8% as of that point. 
**
**    1% - charity wallet: this will be sent straight to the tax generated charity wallet as SKYLANA. 
**
**    3% - Liquidity pool: this will be sent directly to the liquidity pool as BNB 
**    
**    800,000,000,000,000 token burned every 7 days, untlil total supply would be set 1 trillion 
**
...
contract DOGEONE  is Context, IERC20, Ownable {
    ...
    uint256 private burnTokenEveryWeek =  400 * 10**12 * 10**18;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        //(uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tCharity, uint256 tCharity) = _getValues(tAmount);
        uint256[8] memory values = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256[8] memory values) {
        // (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tCharity, uint256 tCharity) = _getTValues(tAmount);
        ...
        // @return rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tBurn, tCharity, tCharity
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256[5] memory tValues) {
        ...
        uint256 tCharity = calculateCharityFee(tAmount);            // charity token to collect         
        uint256 tempTransferAmount = tAmount.sub(tFee).sub(tLiquidity);
        uint256 tTransferAmount = tempTransferAmount.sub(tBurn).sub(tCharity);
    ...
    function _getRValues(uint256[5] memory tValues, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        //function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, tBurn, tCharity, tCharity uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = tempTransferAmount.sub(rBurn).sub(rCharity);
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax, burn, liquidity, charity, charity fee
