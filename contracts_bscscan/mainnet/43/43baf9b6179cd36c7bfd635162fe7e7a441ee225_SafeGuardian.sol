...
// File: contracts/SafeGuardian.sol
pragma solidity ^0.7.6;

contract SafeGuardian is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SafeGuardianDividendTracker public dividendTracker;
    ...
    uint256 public totalFees = tokenRewardsFee.add(liquidityFee).add(marketFee).add(charityFee);
    // buy fees
    ...
    uint256 public buyTotalFees = buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketFee).add(buyCharityFee);
    // sell fees
    ...
    uint256 public sellTotalFees = sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketFee).add(sellCharityFee);
    // fee tracker
    ...
    uint256 public tradingEnabledTimestamp = 1632591751;
    ...
    constructor() public ERC20("SafeGuardian", "SFG") {
        // LP address receiver 
        burnAddress = address(0x1D4e1D6aE6Cd00E899565047c321E38d086DdfB8);
        uint256 _processDividendTime = block.timestamp;
        processDividendTime = _processDividendTime;

        dividendTracker = new SafeGuardianDividendTracker();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "SafeGuardian: The dividend tracker already has that address");

        SafeGuardianDividendTracker newDividendTracker = SafeGuardianDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SafeGuardian: The new dividend tracker must be owned by the SafeGuardian token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "SafeGuardian: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "SafeGuardian: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "SafeGuardian: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SafeGuardian: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SafeGuardian: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SafeGuardian: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "SafeGuardian: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndLiquify(uint256 tokens) private  {
        ...
        uint256 walletTotalBalance = marketFeeBalance + charityFeeBalance ;
        uint256 walletTotalPortion = marketFeePortion + charityFeePortion ;
...
contract SafeGuardianDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("SafeGuardian_Dividend_Tracker", "SafeGuardian_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "SafeGuardian_Dividend_Tracker: No transfers allowed");
    }
    function withdrawDividend() public override {
        require(false, "SafeGuardian_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SafeGuardian contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SafeGuardian_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SafeGuardian_Dividend_Tracker: Cannot update claimWait to same value");
