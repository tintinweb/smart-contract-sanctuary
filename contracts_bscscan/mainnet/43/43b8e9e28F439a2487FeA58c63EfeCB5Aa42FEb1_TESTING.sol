...
contract TESTING is IERC20 {
    ...
    //uint256 private constant startingSupply = 1_000_000_000_000_000;
    uint256 private constant startingSupply = 100_000;

    uint8 private _decimals = 9;
    uint256 private _decimalsMul = _decimals;
    uint256 private _tTotal = startingSupply * (10 ** _decimalsMul);

    string constant _name = "TBATESTDONOTBUY";
    ...
    uint256 private _giveawayFee = 0; // Adjusted by buys and sells.
    ...
    uint256 public _transferGiveawayFee = _sellGiveawayFee;

    uint256 public maxReflectionFee = 1000;
    uint256 public maxLiquidityFee = 1000;
    uint256 public maxMarketingFee = 2500;
    ...
    address payable private _marketingWallet = payable(0x2dA3C25496E0800CB6e5ABC540DdEE9821c97C2c);
    address payable private _giveawayWallet = payable(0x7F66615Ae624c13aB208C79C9e055682065F7648);
    ...
    uint256 public maxSellAmtLimit = 10;
    uint256 public maxSellAmtLimitDivisor = 100;
    mapping (address => uint256) userSellTime;
    mapping (address => uint256) userSoldAmt;
    mapping (address => uint256) userSoldAmtLimit;
    ...
    function setSellLimits(uint256 timeAmt, uint256 percent, uint256 divisor) external onlyOwner {
        require (timeAmt <= 24 hours, "Cannot set time limit to higher than a day.");
        require (percent >= 1 && divisor >= 1000, "Cannot set lower than 0.1%.");
    ...
    function _getTime(address account) public view onlyOwner returns (uint256){
        return (block.timestamp - userSellTime[account]);
    }

    function _setTimeInPast(uint256 time, address account) external onlyOwner {
        userSellTime[account] = time;
    }

    function _getAmt(address account) external view onlyOwner returns (uint256, uint256) {
        return((userSoldAmt[account]) / (10**_decimalsMul), userSoldAmtLimit[account] / (10**_decimalsMul));
    }
            ...
            if(lpPairs[to]) {
                ...
                if(sellLimitsEnabled) {
                    if (block.timestamp - userSellTime[from] >= maxSellTimeLimit) {
                        ...
                        userSoldAmtLimit[from] = (balanceOf(from) * maxSellAmtLimit) / maxSellAmtLimitDivisor;
                        userSellTime[from] = block.timestamp;
                    } else {
                        require(userSoldAmt[from] + amount <= userSoldAmtLimit[from], "Selling too much within the time limit!");
                        userSoldAmt[from] += amount;
                    }
                }
            } else {
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        ...
        transferBNBToWallets(amountBNBMarketing);
    ...
    function transferBNBToWallets(uint256 amtBNB) internal {
        uint256 amtBNBToMarketing = (amtBNB * _marketingFee) / (_marketingFee + _giveawayFee);
        _marketingWallet.transfer(amtBNBToMarketing);
        _giveawayWallet.transfer(amtBNB - amtBNBToMarketing);
    }
