...
contract ICO {
    ...
    uint timeLimit1 = 1639317600; // unix timestamp December 12 UTC 2pm (UTC)
    uint timeLimit2 = 1640181600; // unix timestamp December 22 UTC 2pm (UTC)
    uint timeLimit3 = 1641996000; // unix timestamp January 12 UTC 2pm (UTC)

    mapping (address => uint256) public buyerList;
    mapping (address => uint256) public leftAmountList;

    uint tokenSupplyAmount = 25 * 10 ** 4;

    AggregatorV3Interface internal pricefeed;
    
    constructor (Pegasus _token) public{
        token=_token;
        owner=msg.sender;
    ...
    function getList(address _address) view external returns (uint256){
      return leftAmountList[_address];
    }

    function getPegasusBalance() view external returns (uint256) {
        return tokenSupplyAmount;
    }

    function getPegasusBalanceReal() view external returns (uint256) {
        return token.balanceOf(address(this));
    }

    function buy(uint256 _tokenNum) external payable {
        uint256 tokenBalance = token.balanceOf(address(this));
        require(tokenBalance > 0, 'Contract has no Pegasus token now.');
        require(tokenSupplyAmount > _tokenNum); 
        ...
        tokenSupplyAmount = tokenSupplyAmount - _tokenNum;
    }
    ...
    function recoverPegasus(uint256 _tokenNum) public {
        require(msg.sender == owner);
        uint256 leftToken = token.balanceOf(address(this));
        _tokenNum = _tokenNum.mul(10**18);
        require(leftToken > _tokenNum);
