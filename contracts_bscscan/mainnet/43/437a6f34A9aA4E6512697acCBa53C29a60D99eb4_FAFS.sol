...
contract FAFS is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "FAFS";
    string constant _symbol = "FAFS";
    ...
    address public marketingFeeReceiver = address(0x7FF2F5197350328aB7725E608137a0C507cD72F6);

    IDEXRouter public router;
    address public pair;

    DividendDistributor public fegDistributor;
    DividendDistributor public safemoonDistributor;
    ...
    constructor () {
        ...
        fegDistributor = new DividendDistributor(address(router), Feg);
        safemoonDistributor = new DividendDistributor(address(router), Safemoon);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[sender]){ 
            try fegDistributor.setShare(sender, _balances[sender]) {} catch {} 
            try safemoonDistributor.setShare(sender, _balances[sender]) {} catch {} 
        }
        if(!isDividendExempt[recipient]){ 
            try fegDistributor.setShare(recipient, _balances[recipient]) {} catch {} 
            try safemoonDistributor.setShare(recipient, _balances[recipient]) {} catch {} 
        }

        try fegDistributor.process(distributorGas) {} catch {}
        try safemoonDistributor.process(distributorGas) {} catch {}
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountFegBNBReflection = amountBNB.mul(fegReflectionFee).div(totalBNBFee);
        uint256 amountSafeMoonBNBReflection = amountBNB.mul(babyCakeReflectionFee).div(totalBNBFee);
        uint256 amountBNBMarketing = amountBNB.mul(marketingFee).div(totalBNBFee);

        try fegDistributor.deposit{value: amountFegBNBReflection}() {} catch {}
        try safemoonDistributor.deposit{value: amountSafeMoonBNBReflection}() {} catch {}
    ...
    function setIsDividendExempt(address holder, bool exempt) external onlyOwner {
        ...
        if(exempt){
            fegDistributor.setShare(holder, 0);
            safemoonDistributor.setShare(holder, 0);
        }else{
            fegDistributor.setShare(holder, _balances[holder]);
            safemoonDistributor.setShare(holder, _balances[holder]);
        }
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _fegReflectionFee, uint256 _babyCakeReflectionFee, uint256 _marketingFee) external onlyOwner {
        liquidityFee = _liquidityFee;
        fegReflectionFee = _fegReflectionFee;
        babyCakeReflectionFee = _babyCakeReflectionFee;
        marketingFee = _marketingFee;
        totalFee = _liquidityFee.add(_fegReflectionFee).add(_babyCakeReflectionFee).add(_marketingFee);
    }
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external onlyOwner {
        fegDistributor.setDistributionCriteria(_minPeriod, _minDistribution);
        safemoonDistributor.setDistributionCriteria(_minPeriod, _minDistribution);
    }
    ...
    function claim() public {
        fegDistributor.distributeDividend(msg.sender);
        safemoonDistributor.distributeDividend(msg.sender);
    }
