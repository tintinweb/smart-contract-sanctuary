...
import "../../interfaces/apeswap/IApeRouter02.sol";
import "../../interfaces/apeswap/IApePair.sol";
import "../../interfaces/apeswap/IMasterApe.sol";
import "../../interfaces/IByalan.sol";

import "../ByalanIsland.sol";
import "../Sailor.sol";

contract ApeswapByalanLP is ByalanIsland, Sailor, IByalan {
    ...
    address public constant MASTERCHEF = 0x5c8D727b265DBAfaba67E050f2f739cAeEB4A6F9;
    uint256 public immutable pid;

    // Routes
    address[] public bananaToWbnbRoute;
    address[] public bananaToLp0Route;
    address[] public bananaToLp1Route;
    ...
    constructor(
        ...
        address _harvester,
        uint256 _pid,
        address[] memory _bananaToLp0Route,
        address[] memory _bananaToLp1Route
    )
        ByalanIsland(
            _hydra,
        ...
        )
    {
        ...
        want = IMasterApe(MASTERCHEF).poolInfo(_pid).lpToken;
        lpToken0 = IApePair(want).token0();
        lpToken1 = IApePair(want).token1();

        bananaToWbnbRoute = [BANANA, WBNB];
        if (lpToken0 != BANANA) {
            require(_bananaToLp0Route[0] == BANANA, "invalid lp 0 route");
            require(_bananaToLp0Route[_bananaToLp0Route.length - 1] == lpToken0, "invalid lp 0 route");
            require(
                IApeRouter02(unirouter).getAmountsOut(1 ether, _bananaToLp0Route)[_bananaToLp0Route.length - 1] > 0,
                "invalid lp 0 route"
            );
            bananaToLp0Route = _bananaToLp0Route;
        }

        if (lpToken1 != BANANA) {
            require(_bananaToLp1Route[0] == BANANA, "invalid lp 1 route");
            require(_bananaToLp1Route[_bananaToLp1Route.length - 1] == lpToken1, "invalid lp 1 route");
            require(
                IApeRouter02(unirouter).getAmountsOut(1 ether, _bananaToLp1Route)[_bananaToLp1Route.length - 1] > 0,
                "invalid lp 1 route"
            );
            bananaToLp1Route = _bananaToLp1Route;
        }
    ...
    function deposit() public override whenNotPaused {
        ...
        if (wantBal > 0) {
            IMasterApe(MASTERCHEF).deposit(pid, wantBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external override onlyIzlude {
        ...
        if (wantBal < _amount) {
            IMasterApe(MASTERCHEF).withdraw(pid, _amount - wantBal);
    ...
    function harvest() external override whenNotPaused onlyEOA onlyHarvester gasThrottle {
        IMasterApe(MASTERCHEF).deposit(pid, 0);
        chargeFees();
        addLiquidity();
        deposit();
    ...
    function chargeFees() internal {
        uint256 toBnb = (IERC20(BANANA).balanceOf(address(this)) * totalFee) / MAX_FEE;
        IApeRouter02(unirouter).swapExactTokensForETH(toBnb, 0, bananaToWbnbRoute, address(this), block.timestamp);
    ...
    function addLiquidity() internal {
        uint256 bananaHalf = IERC20(BANANA).balanceOf(address(this)) / 2;

        if (lpToken0 != BANANA) {
            IApeRouter02(unirouter).swapExactTokensForTokens(
                bananaHalf,
                0,
                bananaToLp0Route,
                address(this),
        ...
        IApeRouter02(unirouter).addLiquidity(
            lpToken0,
            lpToken1,
    ...
    function balanceOfPool() public view override returns (uint256) {
        return IMasterApe(MASTERCHEF).userInfo(pid, address(this)).amount;
    }
        ...
        returns (IERC20[] memory rewardTokens, uint256[] memory rewardAmounts)
    {
        ...
        rewardTokens[0] = IERC20(BANANA);
        rewardAmounts[0] =
            IMasterApe(MASTERCHEF).pendingCake(pid, address(this)) +
    ...
    function retireStrategy() external override onlyIzlude {
        IMasterApe(MASTERCHEF).emergencyWithdraw(pid);
    ...
    function _giveAllowances() internal {
        ...
        IERC20(BANANA).safeApprove(unirouter, type(uint256).max);

        // lp token 0 and 1 maybe BANANA so approve 0 is needed here
    ...
    function _removeAllowances() internal {
        ...
        IERC20(BANANA).safeApprove(unirouter, 0);
        IERC20(lpToken0).safeApprove(unirouter, 0);
        IERC20(lpToken1).safeApprove(unirouter, 0);
    }
