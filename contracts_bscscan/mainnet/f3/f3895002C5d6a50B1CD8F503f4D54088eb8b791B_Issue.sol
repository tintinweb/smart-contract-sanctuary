...
interface IERC721 is IERC165 {
    ...
    function getDuck(uint256 _tokenId) external view returns (uint256 ducknumber, uint256 level, uint256 exp,  bool stakeFreeze, uint remainToNextLevel, uint createTimestamp, address tokenOwner, string memory uri);    
...
contract Issue is AccessControl, ReentrancyGuard {
    ...
    IERC721 public duckNFT;
    IBEP20 public ppollToken;
    ...
    constructor( address minter, address _duckNft, address _ppollToken ) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(CREATOR_ADMIN_SERVER, minter);
        _setupRole(TOKEN_MINTER_ROLE, minter);
        _initialExpBoost = 0;
		duckNFT = IERC721(_duckNft); // DuckNFT PPOLL
        ppollToken = IBEP20(_ppollToken); // PPOLL TOKEN
        ...
        duckChance.push(ChanceTableDuck({level: 1, maxValue: 6, minValue: 4, chance: 300}));
        duckChance.push(ChanceTableDuck({level: 1, maxValue: 9, minValue: 5, chance: 110}));
        duckChance.push(ChanceTableDuck({level: 2, maxValue: 20, minValue: 11, chance: 60}));
        duckChance.push(ChanceTableDuck({level: 2, maxValue: 60, minValue: 30, chance: 25}));
    ...
    function setPriceOracle (address account) public {    
    ...
    function changeFeeUSD(uint256 _fee) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, address(msg.sender)), "Caller is not a owner");
    ...
    function summonegg(address to) public nonReentrant {
        ...
        duckNFT.addDuckNumber(tokenId, 1, heroes[1].name, heroes[1].category, heroes[1].exp, heroes[1].level, block.timestamp, false );
        duckNFT.safeMint(to, tokenId);
    }
    ...
    function hatcheggnormal(uint256 tokenId) public notContract nonReentrant {
        require(duckNFT.ownerOf(tokenId) == msg.sender, "Not owner of token");
        require(duckNFT.getHeroesNumber(tokenId).heroesNumber == 1, "Only Egg category can be hatch");
        duckNFT.burn(msg.sender, tokenId);
        duckNFT.deleteHeroesNumber(tokenId);  
        _lastTokenId += 1;
        uint newtokenId = _lastTokenId;
        duckNFT.addDuckNumber(newtokenId, 2, heroes[2].name, heroes[2].category, heroes[2].exp, heroes[2].level, block.timestamp, false );
        duckNFT.safeMint(msg.sender, newtokenId);        
    }

    function hatcheggpremium(uint256 tokenId) public notContract nonReentrant {
        require(duckNFT.ownerOf(tokenId) == msg.sender, "Not owner of token");
        require(duckNFT.getHeroesNumber(tokenId).heroesNumber == 1, "Only Egg category can be hatch");
        _distributePayment(feePremium);
        duckNFT.burn(msg.sender, tokenId);
        duckNFT.deleteHeroesNumber(tokenId);  
        _lastTokenId += 1;
        uint newtokenId = _lastTokenId;
        ...
        duckNFT.addDuckNumber(newtokenId, level + 1, heroes[level + 1].name, heroes[level + 1].category, exp * 1e18, level, block.timestamp, false );
        duckNFT.safeMint(msg.sender, newtokenId);        
    }
    ...
    function levelUp(uint[] calldata tokenId) public nonReentrant {
        ...
        uint currentLevel = duckNFT.getHeroesNumber(tokenId[0]).level;
        ...
        for (uint i = 0; i < numbersOfToken; i++) {
            ( ,uint256 level, uint256 exp, bool stakeFreeze, , , ,) = duckNFT.getDuck(tokenId[i]); 
            require(stakeFreeze == false, "ERC721: Token frozen/stake");
    ...
    function _distributePayment(uint Offer) internal {
        //ORACLE
        uint256 price = priceOracle.getDUCKbyUSD(Offer);
        uint256 amountBurn = price.mul(amountBurnFee).div(100);
        uint256 amountTreasury = price.mul(amountTreasuryFee).div(100);
        uint256 amountMarketing = price.mul(amountMarketingFee).div(100);
        uint256 amountJackpot = price.mul(amountJackpotFee).div(100);
        
        ppollToken.safeTransferFrom(address(msg.sender), address(BurnFee), amountBurn);
        ppollToken.safeTransferFrom(address(msg.sender), address(TreasuryFee), amountTreasury);
        ppollToken.safeTransferFrom(address(msg.sender), address(MarketingFee), amountMarketing);
        ppollToken.safeTransferFrom(address(msg.sender), address(JackpotFee), amountJackpot); 
    ...
    function _mintLevelUp(uint level, uint[] memory tokenId) private {
        ...
        for (uint i = 0; i < tokenId.length; i++) {
            require(duckNFT.ownerOf(tokenId[i]) == msg.sender, "Not owner of token");
            newEXP += duckNFT.getHeroesNumber(tokenId[i]).exp;
            duckNFT.burn(msg.sender, tokenId[i]);
            duckNFT.deleteHeroesNumber(tokenId[i]);  
        }
        newEXP = newEXP + (newEXP * _levelUpPercent) / 100;
        _lastTokenId += 1;
        uint newTokenId = _lastTokenId;
        duckNFT.addDuckNumber(newTokenId, level + 1, heroes[level + 1].name, heroes[level + 1].category, newEXP, level, block.timestamp, false );
        duckNFT.safeMint(msg.sender, newTokenId);   
    }
