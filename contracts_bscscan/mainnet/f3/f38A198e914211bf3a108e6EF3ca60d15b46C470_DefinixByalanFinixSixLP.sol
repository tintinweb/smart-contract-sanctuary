...
contract DefinixByalanFinixSixLP is ByalanIsland, Sailor, GasThrottler, IByalan, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using Address for address payable;

    // Tokens used
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant override want = 0xC4B4aC94a73aF2a4788acA0204ce0bE779999dC9; // six-finix lp token
    address public constant SIX = 0x070a9867Ea49CE7AFc4505817204860e823489fE; // lptoken0
    address public constant FINIX = 0x0F02b1F5AF54E04Fb6dd6550f009aC2429C4e30D; // lptoken1
    ...
    address[] public finixToSixRoute;
    ...
    function chargeFees() internal nonReentrant {
        uint256 toWbnb = (IERC20(FINIX).balanceOf(address(this)) * totalFee) / MAX_FEE;
        IDefinixRouter02(unirouter).swapExactTokensForETH(toWbnb, 0, finixToWbnbRoute, address(this), block.timestamp);

        uint256 wbnbBal = address(this).balance;

        uint256 callFeeAmount = (wbnbBal * callFee) / MAX_FEE;
        payable(msg.sender).sendValue(callFeeAmount);

        uint256 treasuryFeeAmount = (wbnbBal * treasuryFee) / MAX_FEE;
        payable(treasuryFeeRecipient).sendValue(treasuryFeeAmount);

        uint256 kswFeeAmount = (wbnbBal * kswFee) / MAX_FEE;
        payable(kswFeeRecipient).sendValue(kswFeeAmount);
    }

    // Adds liquidity to AMM and gets more LP tokens.
    function addLiquidity() internal {
        uint256 finixHalf = IERC20(FINIX).balanceOf(address(this)) / 2;
        ...
        IDefinixRouter02(unirouter).addLiquidity(
            SIX,
            FINIX,
            IERC20(SIX).balanceOf(address(this)),
    ...
    function pendingReward() external view override returns (uint256) {
        return IHerodotus(MASTERCHEF).pendingFinix(pid, address(this));
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(SIX).safeApprove(unirouter, type(uint256).max);
    }

    function _removeAllowances() internal {
        IERC20(want).safeApprove(MASTERCHEF, 0);
        IERC20(FINIX).safeApprove(unirouter, 0);
        IERC20(SIX).safeApprove(unirouter, 0);
    }
