...
abstract contract SmarterTokenInterface is ERC20, Ownable {
    ...
    function mint(address _to, uint256 _amount) public onlyOwner {
        require(
            ...
            "SMART::mint: cannot exceed max supply"
        );
        _mint(_to, _amount);
...
contract MasterChefSmarter is Ownable {
    ...
    constructor(
        ...
        require(
            0 <= _treasuryPercent && _treasuryPercent <= 980,
            "constructor: invalid treasury percent value"
        );
        require(
            0 <= _investorPercent && _investorPercent <= 980,
            "constructor: invalid investor percent value"
        );
        require(
            _devPercent + _treasuryPercent + _investorPercent <= 980,
    ...
    function updatePool(uint256 _pid) public  {
        ...
        joe.mint(address(this), joeReward.mul(devPercent+20).div(1000));
        
        joe.approve(PancakeRouter, joeReward.mul(devPercent+20).div(1000));
        ...
        _pancakeRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            joeReward.mul(devPercent).div(1000),
            0,
            path,
            devAddr,
            block.timestamp+5
    ...
    function setTvlThresholdsd(uint256[] memory _tvlThresholdArray) external onlyOwner {
    ...
    function getjoePerSec() public view returns(uint256 amount ) {
        ...
        path[1] = _pancakeRouter.WETH();
        path[2] = address(USD);
        uint256 joeUsdValue = (_pancakeRouter.getAmountsOut(1e18, path))[2];
        
        uint256 tvl = (joeUsdValue.mul(((joe.balanceOf(address(poolInfo[0].lpToken))).mul(2)).add(stakedJoe))).div(1e18);

        for (uint256 i; i<TvlThresholds.length; i++) {
            if (tvl>TvlThresholds[i]) {
                return amount = ((((joe.balanceOf(address(poolInfo[0].lpToken))).mul(2)).add(stakedJoe)).mul(TvlAPR[i])).div(31536000);
            }
        }
