...
contract SquirtDoge is ERC20, Ownable {
    ...
    SquirtDogeDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    uint256 public maxWalletBalance = 50000000000000 * (10**9);
    ...
    constructor() public ERC20("SQUIRTDOGE", "SQD") {

    	dividendTracker = new SquirtDogeDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SQUIRTDOGE: The dividend tracker already has that address");

        SquirtDogeDividendTracker newDividendTracker = SquirtDogeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SQUIRTDOGE: The new dividend tracker must be owned by the SQUIRTDOGE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SQUIRTDOGE: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SQUIRTDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SQUIRTDOGE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SQUIRTDOGE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SQUIRTDOGE: Cannot update gasForProcessing to same value");
...
contract SquirtDogeDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("SQUIRTDOGE_Dividen_Tracker", "SQUIRTDOGE_Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 400000 * (10**9);
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "SQUIRTDOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "SQUIRTDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SQUIRTDOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "SQUIRTDOGE_Dividend_Tracker: claimWait must be updated to between 20 mins and 24 hours");
        require(newClaimWait != claimWait, "SQUIRTDOGE_Dividend_Tracker: Cannot update claimWait to same value");
