...
Receiving FEG every hours while we are working on usecases and collab with FEG devs
Stealth Launch
Based and transparent Dev
FEG is the future and we will grow together with it 

Buy FEGDADDY and enjoy FEG rewards with auto reflection feature🏖
...
⚠️ But we will not activated immediately and will listen to all of you about this! ⚠️

TG : https://t.me/FEGDADDYBSC
*/
...
contract FEGDADDY is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FEGDADDYDividendTracker public dividendTracker;
    ...
    uint256 public maxWalletToken = 30000000 * (10**18); // 3% of total supply
    ...
    constructor() ERC20("FEGDADDY", "FEGDADDY") {
    	...
    	dividendTracker = new FEGDADDYDividendTracker();

    	buyBackWallet = 0x474A6648b3EE87E96EEC98192BDb779e1DA1A8FF;
    	
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //0xE7AB1F133cfa7e26ef5921D2Ace5F3D2393a268e
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "FEGDADDY: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "FEGDADDY: Marketing divisor must be between 0 (0%) and 100 (100%)");
  	...
  	function prepareForPreSale() external onlyOwner {
        ...
        marketingFee = 5;
        maxBuyTranscationAmount = 2000000 * (10**18);
        maxWalletToken = 3000000 * (10**18);
    }
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "FEGDADDY: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FEGDADDY: The dividend tracker already has that address");

        FEGDADDYDividendTracker newDividendTracker = FEGDADDYDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FEGDADDY: The new dividend tracker must be owned by the FEGDADDY token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "FEGDADDY: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FEGDADDY: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FEGDADDY: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FEGDADDY: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FEGDADDY: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "FEGDADDY: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FEGDADDY: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FEGDADDY: Cannot update gasForProcessing to same value");
...
contract FEGDADDYDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("FEGDADDY_Dividend_Tracker", "FEGDADDY_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 100000 * (10**18); //must hold 100000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "FEGDADDY_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "FEGDADDY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FEGDADDY contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "FEGDADDY_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FEGDADDY_Dividend_Tracker: Cannot update claimWait to same value");
