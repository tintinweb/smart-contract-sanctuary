...
import {IUniversalVault} from "../toddlerVault/ToddlerVault.sol";
import {ToddlerVaultFactory} from "../toddlerVault/ToddlerVaultFactory.sol";
...
interface IStakerInu is IRageQuit {
    /* admin events */

    event StakerInuERC20Created(address rewardPool, address powerSwitch);
    event StakerInuERC20Funded(address token, uint256 amount);
...
/// Access Control
/// - Power controller:
///     Can power off / shutdown the StakerInu
...
/// - StakerInu admin:
///     Can add funds to the StakerInu, register bonus tokens, and whitelist new vault factories
///     Is a subset of owner permissions
/// - User:
///     Can stake / unstake / ragequit / claim vested rewards
/// StakerInu State Machine
/// - Online:
///     StakerInu is operating normally, all functions are enabled
/// - Offline:
///     StakerInu is temporarely disabled for maintenance
...
/// - Shutdown:
///     StakerInu is permanently disabled
...
contract StakerInuERC20 is IStakerInu, Powered {
    ...
    /// @param rewardTokenAddress address The address of the reward token for this StakerInu
    constructor(
        ...
        // emit event
        emit StakerInuERC20Created(rewardPool, powerSwitch);
    }
    ...
    function fund(address token, uint256 amount) external {
        ...
        // emit event
        emit StakerInuERC20Funded(token, amount);
    }
    ...
    function registerVaultFactory(address factory) external {
        _admin();
        // add factory to set
        require(_vaultFactorySet.add(factory), "StakerInu: vault factory already registered");
    ...
    function removeVaultFactory(address factory) external {
        ...
        require(_vaultFactorySet.remove(factory), "StakerInu: vault factory not registered");
    ...
    function registerBonusToken(address bonusToken) external {
        ...
        require(_bonusTokenSet.length() < MAX_BONUS_TOKENS, "StakerInu: max bonus tokens reached ");
    ...
    function removeBonusToken(address bonusToken) external {
        _admin();
        require(_bonusTokenSet.remove(bonusToken), "StakerInu: bonus token not present ");
    ...
    {
        uint256 serialNumber = ToddlerVaultFactory(nftFactory).tokenIdToSerialNumber(nftId);
    ...
    {
        uint256 balance = ToddlerVaultFactory(nftFactory).balanceOf(owner);
        nftIds = new uint256[](balance);
        for (uint256 index = 0; index < balance; index++) {
            uint256 nftId = ToddlerVaultFactory(nftFactory).tokenOfOwnerByIndex(owner, index);
            nftIds[index] = nftId;
        }
    }
    ...
    /// @notice Exit StakerInu without claiming reward
    ...
    /// state machine:
    ///   - when vault exists on this StakerInu
    ...
    function rageQuit() external override {
        require(_vaultSet.contains(msg.sender), "StakerInuERC20: no vault");
        ...
        require(vaultTokens.length() > 0, "StakerInuERC20: no stake");
    ...
    function stakeERC20(
        ...
        require(isValidVault(vault, vaultFactory), "StakerInuERC20: vault is not valid");
        // verify non-zero amount
        require(amount != 0, "StakerInuERC20: no amount staked");
        ...
        require(
            ...
            "StakerInuERC20: MAX_TOKENS_STAKED_PER_VAULT reached"
        );
    ...
    /// state machine:
    ///   - when vault exists on this StakerInuERC20
    ...
    function unstakeERC20AndClaim(
        ...
        require(_vaultSet.contains(vault), "StakerInuERC20: no vault");
        // fetch vault storage reference
        VaultData storage vaultData = _vaults[vault];
        // verify non-zero amount
        require(amount != 0, "StakerInuERC20: no amount unstaked");
        // validate recipient
        require(isValidAddress(recipient), "StakerInuERC20: invalid recipient");
        // check for sufficient vault stake amount
        require(vaultData.tokens.contains(token), "StakerInuERC20: no token in vault");
        // check for sufficient vault stake amount
        require(
            vaultData.tokenStake[token] >= amount,
            "StakerInuERC20: insufficient vault token stake"
        );
...
import {IRageQuit} from "../stakerInu/StakerInuERC721.sol";
...
import {IUniversalVault} from "./ToddlerVault.sol";

/// @title ToddlerVaultFactory
contract ToddlerVaultFactory is Ownable, IFactory, IInstanceRegistry, ERC721 {
    using SafeMath for uint256;
    ...
    constructor() ERC721("ToddlerVault", "TODDLERVAULT") {
        ERC721._setBaseURI("https://backend.toddlerinu.wtf/nft/");
    }
...
interface IStakerInuERC721 is IRageQuit {
    /* admin events */

    event StakerInuERC721Created(address rewardPool, address powerSwitch);
    event StakerInuERC721Funded(address token, uint256 amount);
    event BonusTokenRegistered(address token);
    event BonusTokenRemoved(address token);
    event VaultFactoryRegistered(address factory);
    event VaultFactoryRemoved(address factory);
...
/// @title StakerInuERC721
/// @notice Reward distribution contract that handles ERC721 Token with Tiers
/// Access Control
/// - Power controller:
///     Can power off / shutdown the StakerInuERC721
///     Can withdraw rewards from reward pool once shutdown
/// - Owner:
///     Is unable to operate on user funds due to UniversalVault
///     Is unable to operate on reward pool funds when reward pool is offline / shutdown
/// - StakerInuERC721 admin:
///     Can add funds to the StakerInuERC721, register bonus tokens, and whitelist new vault factories
///     Is a subset of owner permissions
/// - User:
///     Can stake / unstake / ragequit / claim vested rewards
/// StakerInuERC721 State Machine
/// - Online:
///     StakerInuERC721 is operating normally, all functions are enabled
/// - Offline:
///     StakerInuERC721 is temporarely disabled for maintenance
///     User staking and unstaking is disabled, ragequit remains enabled
///     Users can delete their stake through rageQuit() but forego their pending reward
///     Should only be used when downtime required for an upgrade
/// - Shutdown:
///     StakerInuERC721 is permanently disabled
///     All functions are disabled with the exception of ragequit
///     Users can delete their stake through rageQuit()
///     Power controller can withdraw from the reward pool
///     Should only be used if Owner role is compromised
contract StakerInuERC721 is IStakerInuERC721, Powered {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    /// @notice Initizalize StakerInuERC721
    /// access control: only proxy constructor
    /// state machine: can only be called once
    /// state scope: set initialization variables
    /// token transfer: none
    ...
    /// @param rewardPoolFactory address The factory to use for deploying the RewardPool
    /// @param powerSwitchFactory address The factory to use for deploying the PowerSwitch
    /// @param rewardTokenAddress address The address of the reward token for this StakerInuERC721
    constructor(
        address adminAddress,
        address rewardPoolFactory,
        address powerSwitchFactory,
        ...
        // emit event
        emit StakerInuERC721Created(rewardPool, powerSwitch);
    }
    ...
    /// @notice Add funds to StakerInuERC721
    /// access control: only admin
    /// state machine:
    ///   - can be called multiple times
    ///   - only online
    ...
    function fund(address token, uint256 amount) external {
        ...
        // emit event
        emit StakerInuERC721Funded(token, amount);
    }
    ...
    function registerVaultFactory(address factory) external {
        _admin();
        // add factory to set
        require(_vaultFactorySet.add(factory), "StakerInuERC721: vault factory already registered");
    ...
    function removeVaultFactory(address factory) external {
        _admin();
        // remove factory from set
        require(_vaultFactorySet.remove(factory), "StakerInuERC721: vault factory not registered");
    ...
    function registerBonusToken(address bonusToken) external {
        ...
        require(
            ...
            "StakerInuERC721: max bonus tokens reached "
        );
    ...
    function removeBonusToken(address bonusToken) external {
        _admin();
        require(_bonusTokenSet.remove(bonusToken), "StakerInuERC721: bonus token not present ");
    ...
    /// @notice Exit StakerInuERC721 without claiming reward
    /// @dev This function should never revert when correctly called by the vault.
    ///      A max number of tokens staked per vault is set with MAX_TOKENS_STAKED_PER_VAULT to
    ///      place an upper bound on the for loop.
    /// access control: callable by anyone but fails if caller is not an approved vault
    /// state machine:
    ///   - when vault exists on this StakerInuERC721
    ///   - when active stake from this vault
    ///   - any power state
    /// state scope:
    ///   - decrease stakedTokenTotal[token], delete if 0
    ...
    /// token transfer: none
    function rageQuit() external override {
        require(_vaultSet.contains(msg.sender), "StakerInuERC721: no vault");
        //fetch vault storage reference
        VaultData storage vaultData = _vaults[msg.sender];
        // revert if no active tokens staked
        EnumerableSet.AddressSet storage vaultTokens = vaultData.tokens;
        require(vaultTokens.length() > 0, "StakerInuERC721: no stake");
    ...
    function stakeERC721(
        ...
        // verify vault is valid
        require(isValidVault(vault, vaultFactory), "StakerInuERC721: vault is not valid");
        ...
        require(
            vaultData.tokens.length() < MAX_TOKENS_STAKED_PER_VAULT,
            "StakerInuERC721: MAX_TOKENS_STAKED_PER_VAULT reached"
        );
    ...
    function unstakeERC721AndClaimReward(
        ...
        require(_vaultSet.contains(vault), "StakerInuERC721: no vault");
        // fetch vault storage reference
        VaultData storage vaultData = _vaults[vault];
        // validate recipient
        require(isValidAddress(recipient), "StakerInuERC721: invalid recipient");
        // check for sufficient vault stake amount
        require(vaultData.tokens.contains(token), "StakerInuERC721: no token in vault");
        // check for sufficient vault stake amount
        require(
            vaultData.tokenStake[token] >= 1,
            "StakerInuERC721: insufficient vault token stake"
        );
        // check for sufficient total token stake amount
        // if the above check succeeds and this check fails, there is a bug in stake accounting
