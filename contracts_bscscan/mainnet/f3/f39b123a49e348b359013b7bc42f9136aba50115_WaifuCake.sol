...
/*
Welcome to $WAIFUCAKE | Earn 6.9% CAKE & 6.9% ANIME Token Rewards!
...
contract WaifuCake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxBuyTransactionAmount = 69696969 * 10**18;
    uint256 public maxSellTransactionAmount = 348484 * 10**18; //0.5%
    uint256 public maxWalletBalance = 69696969 * 10**18;
    uint256 public swapTokensAtAmount = 14000 * 10**18; //0.02%
 
    uint256 public burnFee = 0;
    uint256 public liquidityFee = 0;
    uint256 public previousLiquidityFee;
    uint256 public firstDividendRewardsFee = 69;
    uint256 public previousFirstDividendRewardsFee;
    uint256 public cakeDividendRewardsFee = 69;
    ...
    constructor() ERC20("WaifuCake - 6.9% CAKE AND ANIME", "WAIFUCAKE") {
    	firstDividendTracker = new FIRSTDividendTracker();
    	cakeDividendTracker = new CAKEDividendTracker();
 
    	marketingWallet = 0x6De6c9eb851A6d1942A3845c7C6D018218619992;
    	firstDividendToken = 0xaC472D0EED2B8a2f57a6E304eA7eBD8E88D1d36f;
    ...
    //-
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "WaifuCake: The marketing wallet is already this address");
    ...
    function updateFirstDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(firstDividendTracker), "WaifuCake: The dividend tracker already has that address");
 
        FIRSTDividendTracker newFirstDividendTracker = FIRSTDividendTracker(payable(newAddress));
 
        require(newFirstDividendTracker.owner() == address(this), "WaifuCake: The new dividend tracker must be owned by the WaifuCake token contract");
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "WaifuCake: The dividend tracker already has that address");
 
        CAKEDividendTracker newCakeDividendTracker = CAKEDividendTracker(payable(newAddress));
 
        require(newCakeDividendTracker.owner() == address(this), "WaifuCake: The new dividend tracker must be owned by the WaifuCake token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "WaifuCake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "WaifuCake: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "WaifuCake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "WaifuCake: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "WaifuCake: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "WaifuCake: Trading has not started yet");
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[to]) {
                fees = fees.div(1000).mul(sellFeeIncreaseFactor);
...
contract FIRSTDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("WaifuCake_ANI_Dividend_Tracker", "WaifuCake_ANI_Dividend_Tracker", 0xaC472D0EED2B8a2f57a6E304eA7eBD8E88D1d36f) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000 * (10**18); //must hold 2000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "WaifuCake_First_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "WaifuCake_First_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WaifuCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "WaifuCake_First_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "WaifuCake_First_Dividend_Tracker: Cannot update claimWait to same value");
...
contract CAKEDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("WaifuCake_Cake_Dividend_Tracker", "WaifuCake_Cake_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "WaifuCake_Cake_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "WaifuCake_Cake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WaifuCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "WaifuCake_Cake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "WaifuCake_Cake_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
