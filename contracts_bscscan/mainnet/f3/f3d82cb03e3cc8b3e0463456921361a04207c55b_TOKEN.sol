...
// File: contracts/PUSSYDOGEDividendPayingToken.sol
...
contract TOKEN is ERC20, Ownable {
    ...
    constructor() public ERC20("PUSSYDOGE", "PUSSYDOGE") {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PUSSYDOGE: The dividend tracker already has that address");
 
        XDOGEDividendTracker newDividendTracker = XDOGEDividendTracker(payable(newAddress));
 
        require(newDividendTracker.owner() == address(this), "PUSSYDOGE: The new dividend tracker must be owned by the PUSSYDOGE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PUSSYDOGE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PUSSYDOGE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PUSSYDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PUSSYDOGE: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "PUSSYDOGE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PUSSYDOGE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PUSSYDOGE: Cannot update gasForProcessing to same value");
...
contract XDOGEDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("PUSSYDOGE_Dividend_Tracker", "PUSSYDOGE_Dividend_Tracker") {
    	claimWait = 86400; //24h
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10K tokens
    }
 
    function _transfer(address, address, uint256) internal override {
        require(false, "PUSSYDOGE_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() public override {
        require(false, "PUSSYDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PUSSYDOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PUSSYDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PUSSYDOGE_Dividend_Tracker: Cannot update claimWait to same value");
