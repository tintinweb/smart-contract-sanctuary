...
contract Ownable is Context {
    ...
    constructor()  {
        ...
        _owner =0x7a76b48b8fD0D868067D07Ce58bA985d7Bc0df50; //0x30Fd3F75FE330d06A7Ab8E91836BD7727ead35DC;
...
contract faketest is Context, IERC20, Ownable {
    ...
    uint256 public _chartiyFee = 0; // means 3% which distribute to charity wallet

    uint256 public _foundersFee  = 0; 
    ...
    uint256 public _SellchartiyFee = 0;
    ...
    address marketingAddress=0x7AC7480bfcB1086a3BC1eE8f441e396E62c0A58A;

    address foundersAddress=0xf829cD32b31edD86151cA8A46CDe32bDaC29E76e;

    address charityAddress=0xFf8d0Dec72121a903D9d4a9763599010958e8cCF;
    ...
    uint256 public buyMaxTxAmount = _tokenTotal; // 

    uint256 public sellMaxTxAmount = _tokenTotal; // 
    ...
    constructor()  {
        ...
        excludeFromMaxBuyLimit[address(this)] =true;

        excludeFromMaxSellLimit[address(this)] =true;
    ...
    function _transfer(
        ...
        require(!isBlackListed[_msgSender()] && !isBlackListed[recipient],"Account is blacklisted");
        ...
        if (!inSwapAndLiquify && overMinTokenBalance && sender != uniswapV2Pair && swapAndLiquifyEnabled) {
            ...
            distributeTeam(contractTokenBalance);
    ...
    function collectBuyFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(_marketingFee != 0){
            ...
            uint256 sellMarketingFee = amount.mul(_marketingFee).div(10**(_feeDecimal + 2));

            transferAmount = transferAmount.sub(sellMarketingFee);

            _reflectionBalance[address(this)] = _reflectionBalance[address(this)].add(sellMarketingFee.mul(rate));

            if(_isExcluded[marketingAddress]){

                _tokenBalance[address(this)] = _tokenBalance[address(this)].add(sellMarketingFee);

            }

            emit Transfer(account,marketingAddress,sellMarketingFee);
        ...
        if(_chartiyFee != 0){
            ...
            uint256 sellCharityFee = amount.mul(_chartiyFee).div(10**(_feeDecimal + 2));

            transferAmount = transferAmount.sub(sellCharityFee);

            _reflectionBalance[charityAddress] = _reflectionBalance[charityAddress].add(sellCharityFee.mul(rate));

            if(_isExcluded[charityAddress]){

                _tokenBalance[charityAddress] = _tokenBalance[charityAddress].add(sellCharityFee);

            }

            emit Transfer(account,charityAddress,sellCharityFee);
        ...
        if(_foundersFee != 0){
            ...
            uint256 foundersFee = amount.mul(_foundersFee).div(10**(_feeDecimal + 2));

            transferAmount = transferAmount.sub(foundersFee);

            _reflectionBalance[address(this)] = _reflectionBalance[address(this)].add(foundersFee.mul(rate));

            if(_isExcluded[foundersAddress]){

                _tokenBalance[address(this)] = _tokenBalance[address(this)].add(foundersFee);

            }

            emit Transfer(account,foundersAddress,foundersFee);
    ...
    function collectSellFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(_SellmarketingFee != 0){
            ...
            uint256 sellMarketingFee = amount.mul(_SellmarketingFee).div(10**(_feeDecimal + 2));
        ...
        if(_SellchartiyFee != 0){

            uint256 sellCharityFee = amount.mul(_SellchartiyFee).div(10**(_feeDecimal + 2));
    ...
    function _getReflectionRate() private view returns (uint256) {
      ...
      function distributeTeam(uint256 amount) private lockTheSwap {
        ...
        uint256 FounderFee = amountToTranfer.mul(300).div(10**(_feeDecimal + 2));

        uint256 MarkeingFee = amountToTranfer.sub(FounderFee);

        

        payable(foundersAddress).transfer(FounderFee);

        payable(marketingAddress).transfer(MarkeingFee);
    ...
    function setSellCharityFee(uint256 fee) external onlyOwner {
    ...
    function setFoundersFee(uint256 fee) external onlyOwner {
