...
library BscsswapOracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = IBscsswapPair(pair).price0CumulativeLast();
        price1Cumulative = IBscsswapPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IBscsswapPair(pair).getReserves();
...
contract Oracle {
    ...
    function sortTokens(address tokenA, address tokenB) public pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'BscsswapFactory: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'BscsswapFactory: ZERO_ADDRESS');
    }
    ...
    function update(address tokenA, address tokenB) external {
        require(msg.sender == priceUpdater, 'BscsswapOracle: Price can update only price updater address');
        address pair = pairFor(tokenA, tokenB);

        Observation storage observation = pairObservations[pair];
        uint timeElapsed = block.timestamp - observation.timestamp;
        require(timeElapsed >= CYCLE, 'BscsswapOracle: PERIOD_NOT_ELAPSED');
        (uint price0Cumulative, uint price1Cumulative,) = BscsswapOracleLibrary.currentCumulativePrices(pair);
...
contract SwapFeeReward is Ownable{
    ...
    function getSwapFee(address tokenA, address tokenB) internal view returns (uint swapFee) {
        swapFee = uint(1000).sub(IBscsswapPair(pairFor(tokenA, tokenB)).swapFee());
    }
    ...
    event EventRewarded(address input, address output, uint256 contractBalance,
    address pair, bool poolStatus, uint256 fee, uint256 quantity,
    uint256 totalRewarded);
    
      event EventRewardedFirst(address input, address output, uint256 contractBalance);
    
    event EventRewardedSecond(address input, address output, uint256 contractBalance, address pair,
    bool poolStatus);
    ...
    { 
        ...
        uint contractBalance = IERC20(bscsToken).balanceOf(address(this));
        
        emit EventRewardedFirst(input, output, contractBalance);
        
        require(contractBalance > 0, "Out of BSCS token");
        
        address pair = pairFor(input, output);
        PairsList storage pool = pairsList[pairOfPid[pair]];
        
        emit EventRewardedSecond(input, output, contractBalance, pair, pool.enabled);
        ...
        totalRewarded = totalRewarded.add(quantity);
        _balances[account] = _balances[account].add(quantity);
        
        emit EventRewarded(input, output, contractBalance, pair,  pool.enabled, fee, 
         quantity, totalRewarded);
    ...
    function withdraw() public returns(bool){
        ...
        if (balance > 0){
            ...
            //sending bscs token reward to user
            IERC20(bscsToken).transfer(msg.sender, balance);
    ...
    //get contract balances of bscs token
    function getBscsTokenBalance() public view onlyOwner returns(uint){
        return  IERC20(bscsToken).balanceOf(address(this));
    }
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        if (outputToken == anchorToken) {
        ...
        } else if (IBscsswapFactory(factory).getPair(outputToken, anchorToken) != address(0) && checkPairExist(outputToken, anchorToken)) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (IBscsswapFactory(factory).getPair(outputToken, intermediate) != address(0) 
                    && IBscsswapFactory(factory).getPair(intermediate, anchorToken) != address(0) 
