...
Welcome to BabyChildCake

Launch at 50 Members
Earn cake 
Big Plans ahead
Telegram Group:https://t.me/BabyChildCake
*/
...
contract BabyChildCake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BabyChildCakeDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 public maxBuyTranscationAmount = 100000000000000 * (10**18);
    uint256 public maxSellTransactionAmount = 3000000000000 * (10**18);
    uint256 public swapTokensAtAmount = 3000000000001 * (10**18);
    ...
    // sells have fees of 10 and 2 (10 * 1 and 2 * 1)
    ...
    constructor() ERC20("BabyChildCake", "BabyChildCake") {
    	...
    	dividendTracker = new BabyChildCakeDividendTracker();

    	buyBackWallet = 0xB5e73168b8885794C46c0C6022A062c769E6b451;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 300, "BabyChildCake: Sell transaction multipler must be between 100 (1x) and 300 (3x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "BabyChildCake: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "BabyChildCake: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyChildCake: The dividend tracker already has that address");

        BabyChildCakeDividendTracker newDividendTracker = BabyChildCakeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyChildCake: The new dividend tracker must be owned by the BabyChildCake token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "BabyChildCake: Dividend reward tax must be between 0 and 10");
        dividendRewardsFee = newFee;
    }
    
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 1000000000, "BabyChildCake: Dividend reward tax must be between 0 and 1000000000");
        marketingFee = newFee;
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyChildCake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyChildCake: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyChildCake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyChildCake: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "BabyChildCake: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyChildCake: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyChildCake: Cannot update gasForProcessing to same value");
...
contract BabyChildCakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BabyChildCake_Dividend_Tracker", "BabyChildCake_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 1000000000000 * (10**18); //must hold 1000000000000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BabyChildCake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BabyChildCake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyChildCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 36000, "BabyChildCake_Dividend_Tracker: claimWait must be updated to between 30 and 300 minutes");
        require(newClaimWait != claimWait, "BabyChildCake_Dividend_Tracker: Cannot update claimWait to same value");
