...
contract THEBANGER is IBEP20, Auth {
    ...
    function getTF(bool selling, address recipient) public view returns (uint256) {
        if(bottF && (launchedAt + 4 > block.number)){ return btotf.mul(1); }
        if(!selling && bbt && !itCDh[recipient] && 
            (_balances[recipient] > tAl) &&
        ...
        if(!selling && bbt && !itCDh[recipient] &&
            (_balances[recipient] > tAl) &&
            (tCDtb[recipient] + tCDt3 > block.timestamp) && 
            (tCDtb[recipient] + tCDt4 < block.timestamp)){ 
                return bbf3.mul(1); }
        if(!selling && bbt && !itCDh[recipient] &&
            (_balances[recipient] > tAl) &&
            (tCDtb[recipient] + tCDt4 > block.timestamp)){ 
    ...
    function _MyBuyFee(address wallet) external view returns (uint256) {
        return getTF(false, wallet);
    }
