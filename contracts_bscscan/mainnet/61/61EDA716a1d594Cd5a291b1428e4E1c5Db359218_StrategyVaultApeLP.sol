 ...
 */
contract StrategyVaultApeLP is Ownable, Pausable {
     ...
     * {mars} - JetfuelFinance token, used to send funds to the treasury.
    ...
    address public constant output =
    ...
    address public constant aperouter =
     ...
     * {wbnbToFuelRoute} - Route we take to get from {wbnb} into {mars}.
    ...
    address[] public wbnbToFuelRoute = [wbnb, mars];
    ...
    constructor(
        ...
        IERC20(output).safeApprove(aperouter, uint256(-1));
        IERC20(wbnb).safeApprove(unirouter, uint256(-1));

        IERC20(lpToken0).safeApprove(aperouter, 0);
        IERC20(lpToken0).safeApprove(aperouter, uint256(-1));

        IERC20(lpToken1).safeApprove(aperouter, 0);
        IERC20(lpToken1).safeApprove(aperouter, uint256(-1));
    }
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(output).balanceOf(address(this)).mul(6).div(
            100
        );
        IUniswapRouterETH(aperouter).swapExactTokensForTokens(
            toWbnb,
            0,
            outputToWbnbRoute,
            address(this),
    ...
    function addLiquidity() internal {
        ...
        IUniswapRouterETH(aperouter).addLiquidity(
            lpToken0,
            lpToken1,
            lp0Bal,
            lp1Bal,
