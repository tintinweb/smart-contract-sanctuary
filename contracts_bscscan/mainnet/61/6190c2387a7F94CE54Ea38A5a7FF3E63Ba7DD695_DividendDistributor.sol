...
contract DividendDistributor is IDividendDistributor {
    ...
    function setShare(address shareholder, uint256 amount) external override onlyToken {
        ...
        tokenPools[token].shares = pool.shares.sub(shares[shareholder][token].amount).add(amount);
        ...
        tokenPools[token].realisedDividends = pool.realisedDividends.add(excluded);
    }
    ...
    function deposit() external payable override onlyToken {
        ...
        TokenPool storage pool = tokenPools[currentToken];

        while (!isTokenLiquid(currentToken) || resetIndex) {
            resetIndex = false;
            currentToken = nextToken();
        } //TODO Check not infinite loop
