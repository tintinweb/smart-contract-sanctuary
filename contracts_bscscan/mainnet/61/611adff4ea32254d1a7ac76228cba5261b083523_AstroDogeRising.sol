...
contract AstroDogeRising is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("AstroDogeRising", "ADR") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0xA4257Aac7b589E321DDbB16f93f772e021A562b5;
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "AstroDogeRising: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "AstroDogeRising: The new dividend tracker must be owned by the AstroDogeRising token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("AstroDogeRising_Dividend_Tracker", "AstroDogeRising_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 150000 * (10**18); //must hold 150,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "AstroDogeRising_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "AstroDogeRising_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "AstroDogeRising_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "AstroDogeRising_Dividend_Tracker: Cannot update claimWait to same value");
