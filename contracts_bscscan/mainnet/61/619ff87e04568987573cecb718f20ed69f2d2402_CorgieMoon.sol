...
contract created by @dayacoder && @coDmitry

📱 Telegram: https://t.me/CorgieMoonCommunity
🌎 Website: https://corgiemoon.com
🌐 Twitter: https://twitter.com/CorgieMoon
...
contract CorgieMoon is Context, IERC20, Ownable, ReentrancyGuard {
	...
	address BURN_ADDRESS = 0xC606805d2E5aeecfeBf835ebFa1365cc47584C6c;
	...
	string private _name = "CorgieMoon";
	string private _symbol = "CRG";
	uint8 private _decimals = 8;
	
	bool public _isFee = true;
	uint256 public _rewardFee = 5;
	uint256 private _previousRewardFee = _rewardFee;
	uint256 public _burnFee = 5;
	uint256 private _previousBurnFee = _burnFee;
	
	bool public _isDump = true;
	uint256 private _maxPercentDump = 1;
	
	bool public _isCooldown = true;
	uint256 public _sellCooldown = 240;
    uint256 public _buyCooldown = 180;

	IUniswapV2Router02 public immutable uniswapV2Router;
	address public immutable uniswapV2Pair;
	address public addressPoolPancake;
	...
	function excludeFromFeeAndReward(address account) public onlyOwner {
	...
	function isBlacklisted(address account) public view returns (bool) {
        if (account == owner() || account == addressPoolPancake || account == address(this) || account == BURN_ADDRESS) {
            return false;
        }
	...
	function setMaxPercentDump(uint256 maxPercentDump) external onlyOwner {
		_maxPercentDump = maxPercentDump;
	}
	
	function setAddressPoolToken(address _addressPool) external onlyOwner(){
        addressPoolPancake = _addressPool;
    }
	
	function setIsDump(bool isDump) external onlyOwner {
		_isDump = isDump;
	}
	
	function setIsCooldown(bool isCooldown) external onlyOwner {
		_isCooldown = isCooldown;
	}
	
	function setIsFee(bool isFee) external onlyOwner {
		_isFee = isFee;
	}
	
	function setOnAll() external onlyOwner {
		_isDump = true;
        _isFee = true;
        _isCooldown = true;
	}
	
	function setOffAll() external onlyOwner {
		_isDump = false;
        _isFee = false;
        _isCooldown = false;
	}

    function _handleSellCooldown(address from) internal {
        if (_sellCooldown > 0 && from != owner() && from != addressPoolPancake && from != address(this) && _isExcludedFromFee[from] == false) {
            require(block.timestamp - _sells[from] > _sellCooldown);
            _sells[from] = block.timestamp;
        }
    }
    
    function _handleBuyCooldown(address to) internal {
        if (_buyCooldown > 0 && to != owner() && to != addressPoolPancake && _isExcludedFromFee[to] == false) {
            require(block.timestamp - _buys[to] > _buyCooldown);
            _buys[to] = block.timestamp;
        }
    }
	...
	function calculateAmountFee(uint256 _amount) private view returns (uint256) {
		return _amount.mul(_maxPercentDump).div(
			10**3
		);
	}
	...
	function _transfer(
		...
		require(isBlacklisted(from) == false, "You are in blacklist");
		
		/* Сooldown */
        if (_isCooldown == true && from == addressPoolPancake){ _handleBuyCooldown(to); }
        if (_isCooldown == true && to == addressPoolPancake){ _handleSellCooldown(from); }
        if (_isCooldown == true && to != addressPoolPancake && from != addressPoolPancake){ _handleSellCooldown(from); }
		
		setFee();
		bool takeFee = true;
		
		/* Antidump system */
        if(amount >= calculateAmountFee(balanceOf(addressPoolPancake)) && from != addressPoolPancake && true == _isDump){
		    setDumpFee();
		}
		
		if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || false == _isFee){
			takeFee = false;
		}
