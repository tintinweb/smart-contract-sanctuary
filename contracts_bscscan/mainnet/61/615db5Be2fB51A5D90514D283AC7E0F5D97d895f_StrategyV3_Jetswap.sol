...
interface IJetSwapFarm {
    function enterStaking(uint256 _amount) external;
    function leaveStaking(uint256 _amount) external;
}

contract StrategyV3_Jetswap is StrategyV3 {
    ...
    constructor(
        ...
        earnedToWMATICPath = [earnedAddress, wbnbAddress];
        WMATICToNATIVEPath = [wbnbAddress, NATIVEAddress];
    ...
    function _farm() internal override virtual {
        ...
        if (isSingleVault) {
            IJetSwapFarm(farmContractAddress).enterStaking(wantAmt);
        } else {
    ...
    function _unfarm(uint256 _wantAmt) internal override virtual {
        if (isSingleVault) {
            IJetSwapFarm(farmContractAddress).leaveStaking(_wantAmt);
        } else {
...
abstract contract StrategyV3 is Ownable, ReentrancyGuard, Pausable {
    ...
    function buyBack(uint256 _earnedAmt) internal virtual returns (uint256) {
        ...
        if (uniRouterAddress != buybackRouterAddress) {
            ...
            uint256 wmaticAmt = IERC20(wbnbAddress).balanceOf(address(this));
            if (wmaticAmt > 0) {
    ...
    function _wrapMATIC() internal virtual onlyAllowGov {
        ...
        if (maticBal > 0) {
            IWBNB(wbnbAddress).deposit{value: maticBal}(); // MATIC -> WMATIC
        }
    }
