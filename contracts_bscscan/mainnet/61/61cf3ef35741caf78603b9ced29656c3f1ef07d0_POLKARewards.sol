...
contract ERC20 is Context, IERC20 {
     ...
     * POLKA and Wei. This is the value {ERC20} uses, unless {_setupDecimals} is
     * called.
     *
...
contract POLKARewards is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public polkaDividendRewardsFee;
    uint256 public previousPolkaDividendRewardsFee;
    ...
    constructor() ERC20("POLKA Rewards", "$PRWD") {
    	polkaDividendTracker = new PolkaDividendTracker();

    	marketingWallet = 0x834F58aC077B456850e656BB4c9970b4196f36e1;
    	devWallet = 0xF91b5AB668E679Fb15a71AD3c7252E461B7f2FAD;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "POLKARewards: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
        emit MarketingWalletUpdated(marketingWallet, _newWallet);
  	    marketingWallet = _newWallet;
  	}
  	function updateDevWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != devWallet, "POLKARewards: The dev wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(polkaDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setPolkaDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = polkaDividendRewardsFee.add(marketingFee).add(buyBackAndLiquidityFee);
            polkaDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(polkaDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updatePolkaDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(polkaDividendTracker), "POLKARewards: The dividend tracker already has that address");

        PolkaDividendTracker newPolkaDividendTracker = PolkaDividendTracker(payable(newAddress));

        require(newPolkaDividendTracker.owner() == address(this), "POLKARewards: The new dividend tracker must be owned by the POLKARewards token contract");
    ...
    function updateLiquidityFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = buyBackAndLiquidityFee.add(polkaDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "POLKARewards: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "POLKARewards: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "POLKARewards: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "POLKARewards: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "POLKARewards: Cannot update gasForProcessing to same value");
    ...
    function getAccountPolkaDividendsInfo(address account)
 	...
 	function getAccountPolkaDividendsInfoAtIndex(uint256 index)
  	...
  	function processDividendTracker(uint256 gas) external onlyOwner {
		(uint256 ethIterations, uint256 ethClaims, uint256 ethLastProcessedIndex) = polkaDividendTracker.process(gas);
		emit ProcessedPolkaDividendTracker(ethIterations, ethClaims, ethLastProcessedIndex, false, gas, tx.origin);
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (marketingEnabled) {
                    ...
                    uint256 devPortion = address(this).balance.div(10**2).mul(50);
                ...
                if (polkaDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(polkaDividendRewardsFee).mul(polkaDividendRewardsFee);
    ...
    function swapTokensForDividendToken(uint256 _tokenAmount, address _recipient, address _dividendAddress) private {
        // generate the uniswap pair path of weth -> polka
...
contract PolkaDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("POLKARewards_Polkaereum_Dividend_Tracker", "POLKARewards_Polkaereum_Dividend_Tracker", 0x7083609fCE4d1d8Dc0C979AAb8c869Ea2C873402) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "POLKARewards_Polkaereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "POLKARewards_Polkaereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main POLKARewards contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "POLKARewards_Polkaereum_Dividend_Tracker: Cannot update claimWait to same value");
