...
contract SCTFarming is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  ...
  function deposit(uint256 _amount) external nonReentrant {
    ...
    //pool request is not full the first time.
    ...
    // Pool condition is full, can't continue Staking
    if(totalStakingTokens.add(_amount)==maxStakingTokens){
        isFull=true;
    }
