    ...
    IDividendPayingTokenOptional
{
    ...
    address public dividendToken = 0x8076C74C5e3F5852037F31Ff0093Eeb8c8ADd8D3;
...
contract SMNToken is ERC20, Ownable {
    ...
    address public _dividendToken = 0x8076C74C5e3F5852037F31Ff0093Eeb8c8ADd8D3;
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;

    bool private swapping;
    bool public tradingIsEnabled = false;

    SMNTokenDividendTracker public dividendTracker;

    address public marketingWallet;

    uint256 public maxTranscationAmount = 1000000000000000 * (10**9);
    ...
    // swap precentage
    uint256 private devidendSwap = 6;
    uint256 private marketingSwap = 6;
    uint256 private liquiditySwap = 2;
    uint256 private totalSwap = 14;
    ...
    constructor(address newOwner) ERC20("SafeMoonomics", "SAFEMOONOMICS") {
        dividendTracker = new SMNTokenDividendTracker();

        marketingWallet = 0x9d79BC333e770d48F220658B72A7a1D9F0C3A4e2;
        ...
        */
        _mint(newOwner, 1000000000000000 * (10**9));
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "SMN: The dividend tracker already has that address"
        );

        SMNTokenDividendTracker newDividendTracker = SMNTokenDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "SMN: The new dividend tracker must be owned by the FLOKIBUSD token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "SMN: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "SMN: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "SMN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "SMN: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "SMN: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "SMN: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[to]) {
            ...
            if (!swapping && canSwap) {
                ...
                transferToWallet(
                    payable(marketingWallet),
                    address(this).balance.div(10**2).mul(marketingSwap).div(2)
                );

                // swap and add liquidity
                swapAndLiquify(
                    contractTokenBalance.mul(liquiditySwap).div(totalSwap)
                );
...
contract SMNTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("SMN_Dividend_Tracker", "SMN_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "SMN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SMN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SMN contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "SMN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "SMN_Dividend_Tracker: Cannot update claimWait to same value"
        );
