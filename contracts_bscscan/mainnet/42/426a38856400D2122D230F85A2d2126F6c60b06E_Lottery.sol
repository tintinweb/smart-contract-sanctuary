...
contract Lottery is Ownable {
    ...
    function getLotteryStatus() public view returns(
        uint256 _lotteryId,
    ...
    function enterLottery(uint256 _amount) external inState(STATE.OPEN){
        require(_amount >= 250000000000000000000, 'Minimum entry is 250 CHPZ');
    ...
    function getLastLotteryInfo() public view returns( 
    ...
    function withdrawMarketing(uint256 amount) external onlyOwner(){
        require(amount <= (chipz.balanceOf(address(this))).sub(balanceOfPrizePool).sub(balanceOfNextPool), 'The amount is bigger than the balance of marketing pool.');
        safeChipzTransfer(msg.sender, amount);
    }

    function getMarketingBalance() external view returns(uint256 _balance){
        _balance =  (chipz.balanceOf(address(this))).sub(balanceOfPrizePool).sub(balanceOfNextPool);
    }

    function sendToThisMarketing(address marketingAddress) external onlyOwner(){
        safeChipzTransfer(marketingAddress, (chipz.balanceOf(address(this))).sub(balanceOfPrizePool).sub(balanceOfNextPool));
    }
    ...
    function sendBNBToThisMarketing(address marketingAddress) external onlyOwner(){
