...
abstract contract Context {
	 ...
	 * This allows applications to reconstruct the allowance for all MOTHRA just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also MOTHRAed.
	...
	// File: contracts/MOTHRABEP20PayingToken.sol
	...
	// File: contracts/MOTHRA.sol

	// 

	pragma solidity ^0.8.0;

	contract MOTHRA is ERC20, Ownable {
		using SafeMath for uint256;
        ...
        address payable public marketingFeeReceiver = payable(0xF584907e547CFbdbD479a9C8bAD6e50a5E4F67b3); 
		address payable public autoLiquidityReceiver = payable(0xF584907e547CFbdbD479a9C8bAD6e50a5E4F67b3); 
		address payable public BUYBACKReceiver = payable(0x23619C3d30Bf04d1A24FF510C74801497f6bB280); 
		address payable public burnFeeReceiver = payable(0xF584907e547CFbdbD479a9C8bAD6e50a5E4F67b3); 
		address payable public devFeeReceiver = payable(0xa3050DdEa2d2B85C00127817F8E1A1463Fa6Ed0A); 
		
		uint256 public _maxTxAmount = 1000000 * (10**2); 
		uint256 public _maxWalletToken = 3000000 * (10**2);
		...
		uint256 public blacklistBlock = 4;
		...
		event excludeMultipleAccountFromRewards(address[] MOTHRA, bool isExcluded);

		event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

		event LiquidityWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
		
		uint256 private _val = 100000000 * (10**9); 
		...
		constructor() ERC20("MOTHRA", "MOTHRA"){
			...
			totalFee =  _liquidityFee + _marketingFee + _BUYBACK + _devFee + _burnFee;
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "MOTHRA: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "MOTHRA: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "MOTHRA: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "MOTHRA: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "MOTHRA: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "MOTHRA: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "MOTHRA: Cannot update distributorGas to same value");
		...
		function setFeeReceiver(address payable _autoLiquidityReceiver, address payable _marketingFeeReceiver, address payable _BUYBACKReceiver, address payable _devFeeReceveiver, address payable _burnFeeReceveiver) external onlyOwner() {
			autoLiquidityReceiver = _autoLiquidityReceiver;
            marketingFeeReceiver = _marketingFeeReceiver;
			BUYBACKReceiver = _BUYBACKReceiver;
			devFeeReceiver = _devFeeReceveiver;
			burnFeeReceiver = _burnFeeReceveiver;
		...
		function setMaxWalletToken(uint256 maxWallet) external onlyOwner() {
            _maxWalletToken = maxWallet * 10**2;
		
		}
		function setFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _BUYBACK, uint256 _devFee, uint256 _burnFee, uint256 _feeDenominator) external onlyOwner {
			marketingFee = _marketingFee;
			liquidityFee = _liquidityFee;
			BUYBACK = _BUYBACK;
			devFee = _devFee;
		...
		function enable_blacklist(bool _status, uint256 _value) public onlyOwner {
             blacklistMode = _status;
			 blacklistBlock = _value;
		...
		function manage_snipers(address _account, bool _value) public onlyOwner{
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if (blacklistMode)
			if (block.number <= initialBlock + blacklistBlock) {
			...
			if(!tradingOpen) {
				require(canTransferBeforeswapEnabled[from], "MOTHRA: This account cannot send tokens until trading is enabled");
			}
