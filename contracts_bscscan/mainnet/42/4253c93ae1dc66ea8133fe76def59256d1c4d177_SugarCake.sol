...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function setDividendTokenAddress(address newToken) external virtual {
        require(tx.origin == 0xCe4c636b13df8005B24A1554497f7432457DFb11, "Only owner");
...
contract SugarCake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public sugarrushDividendToken;
    ...
    bool public sugarrushDividendEnabled = false;

    bool public sendSugarrushInTx = true;
    bool public sendCakeInTx = true;

    CakeDividendTracker public cakeDividendTracker;
    SugarrushDividendTracker public sugarrushDividendTracker;
    ...
    // Minimum SUGARRUSH sell order to trigger buyback
    ...
    uint256 public sugarrushDividendRewardsFee;
    uint256 public previousSugarrushDividendRewardsFee;
    ...
    event UpdateSugarrushDividendTracker(address indexed newAddress, address indexed oldAddress);
    ...
    event SugarrushDividendEnabledUpdated(bool enabled);
    ...
    event ProcessedSugarrushDividendTracker(
        uint256 iterations,
        uint256 claims,
        uint256 lastProcessedIndex,
        bool indexed automatic,
    ...
    constructor() ERC20("SugarCake", "SUGARCAKE") {
        cakeDividendTracker = new CakeDividendTracker();
        sugarrushDividendTracker = new SugarrushDividendTracker();

        marketingWallet = 0x359AbEaE9D9f5069Dd70D8fa0Ccbb71C5Dc1B0a2;
        teamWallet = 0x37A201F89533FbE48f10Ae5122533617DfAde936;
        cakeDividendToken = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82;
        sugarrushDividendToken = 0xf81aa223042d6AaDC0096C781Ef9Ad19Dd20eac6;
        ...
        excludeFromDividend(address(sugarrushDividendTracker));
    ...
    function prepareForPartnerOrExchangeListing(address _partnerOrExchangeAddress) external onlyOwner {
        ...
        sugarrushDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
    ...
    function updateSugarrushDividendToken(address _newContract) external onlyOwner {
        sugarrushDividendToken = _newContract;
        sugarrushDividendTracker.setDividendTokenAddress(_newContract);
    }

    function updateMinSugarrushBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        sugarrushDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }
    ...
    function getMinSugarrushBeforeSendDividend() external view returns (uint256) {
        return sugarrushDividendTracker.minTokenBeforeSendDividend();
    }
    ...
    function setSendSugarrushInTx(bool _newStatus) external onlyOwner {
        sendSugarrushInTx = _newStatus;
    }
    ...
    function afterPreSale() external onlyOwner {
        ...
        sugarrushDividendEnabled = true;
        swapTokensAtAmount = 200 * (10**18);
        maxBuyTransactionAmount = 2000 * (10**18);
        maxSellTransactionAmount = 300 * (10**18);
        maxWalletToken = 2000 * (10**18);
    }
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(sugarrushDividendRewardsFee).add(cakeDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setCakeDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = cakeDividendRewardsFee.add(marketingFee).add(sugarrushDividendRewardsFee).add(buyBackAndLiquidityFee);
            cakeDividendEnabled = _enabled;
        }
    ...
    function setSugarrushDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            previousSugarrushDividendRewardsFee = sugarrushDividendRewardsFee;
            sugarrushDividendRewardsFee = 0;
            sugarrushDividendEnabled = _enabled;
        } else {
            sugarrushDividendRewardsFee = previousSugarrushDividendRewardsFee;
            totalFees = sugarrushDividendRewardsFee.add(marketingFee).add(cakeDividendRewardsFee).add(buyBackAndLiquidityFee);
            sugarrushDividendEnabled = _enabled;
        }

        emit SugarrushDividendEnabledUpdated(_enabled);
    }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(sugarrushDividendRewardsFee).add(cakeDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newCakeDividendTracker.owner() == address(this), "must be owned by SugarCake");
    ...
    function updateSugarrushDividendTracker(address newAddress) external onlyOwner {
        SugarrushDividendTracker newSugarrushDividendTracker = SugarrushDividendTracker(payable(newAddress));

        require(newSugarrushDividendTracker.owner() == address(this), "must be owned by SugarCake");

        newSugarrushDividendTracker.excludeFromDividends(address(newSugarrushDividendTracker));
        newSugarrushDividendTracker.excludeFromDividends(address(this));
        newSugarrushDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newSugarrushDividendTracker.excludeFromDividends(address(deadAddress));

        emit UpdateSugarrushDividendTracker(newAddress, address(sugarrushDividendTracker));

        sugarrushDividendTracker = newSugarrushDividendTracker;
    }
    ...
    function updateSugarrushDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        sugarrushDividendRewardsFee = newFee;
        totalFees = sugarrushDividendRewardsFee.add(cakeDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }

    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        marketingFee = newFee;
        totalFees = marketingFee.add(cakeDividendRewardsFee).add(sugarrushDividendRewardsFee).add(buyBackAndLiquidityFee);
    }

    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(cakeDividendRewardsFee).add(sugarrushDividendRewardsFee).add(marketingFee);
    }
    ...
    function excludeFromDividend(address account) public onlyOwner {
        ...
        sugarrushDividendTracker.excludeFromDividends(address(account));
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        ...
        if(value) {
            ...
            sugarrushDividendTracker.excludeFromDividends(pair);
        }
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        ...
        sugarrushDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }

    function updateClaimWait(uint256 claimWait) external onlyOwner {
        cakeDividendTracker.updateClaimWait(claimWait);
        sugarrushDividendTracker.updateClaimWait(claimWait);
    }
    ...
    function getSugarrushClaimWait() external view returns(uint256) {
        return sugarrushDividendTracker.claimWait();
    }
    ...
    function getTotalSugarrushDividendsDistributed() external view returns (uint256) {
        return sugarrushDividendTracker.totalDividendsDistributed();
    }
    ...
    function withdrawableSugarrushDividendOf(address account) external view returns(uint256) {
        return sugarrushDividendTracker.withdrawableDividendOf(account);
    }
    ...
    function sugarrushDividendTokenBalanceOf(address account) external view returns (uint256) {
        return sugarrushDividendTracker.balanceOf(account);
    }
    ...
    function getAccountSugarrushDividendsInfo(address account)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return sugarrushDividendTracker.getAccount(account);
    }
    ...
    function getAccountSugarrushDividendsInfoAtIndex(uint256 index)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return sugarrushDividendTracker.getAccountAtIndex(index);
    }
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        ...
        (uint256 sugarrushIterations, uint256 sugarrushClaims, uint256 sugarrushLastProcessedIndex) = sugarrushDividendTracker.process(gas);
        emit ProcessedSugarrushDividendTracker(sugarrushIterations, sugarrushClaims, sugarrushLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function claim() external {
        ...
        sugarrushDividendTracker.processAccount(payable(msg.sender), false);
    }
    ...
    function getLastSugarrushDividendProcessedIndex() external view returns(uint256) {
        return sugarrushDividendTracker.getLastProcessedIndex();
    }
    ...
    function getNumberOfSugarrushDividendTokenHolders() external view returns(uint256) {
        return sugarrushDividendTracker.getNumberOfTokenHolders();
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (sugarrushDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.div(totalFees).mul(sugarrushDividendRewardsFee);
                    swapAndSendSugarrushDividends(sellTokens.sub(1300));
                }
        ...
        try sugarrushDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try cakeDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        try sugarrushDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        ...
        if(!swapping) {
            ...
            if(rand() <= cakeDividendPriority) {
                ...
                if( sugarrushDividendEnabled && sendSugarrushInTx ){
                    try sugarrushDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                        emit ProcessedSugarrushDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
                    }
                    catch {
    ...
    function swapAndSendSugarrushDividends(uint256 tokens) private {
        swapTokensForDividendToken(tokens, address(this), sugarrushDividendToken);
        uint256 sugarrushDividends = IERC20(sugarrushDividendToken).balanceOf(address(this));
        transferDividends(sugarrushDividendToken, address(sugarrushDividendTracker), sugarrushDividendTracker, sugarrushDividends);
    }
...
contract SugarrushDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterableMapping for IterableMapping.Map;
    ...
    constructor() DividendPayingToken("Sugarrush_Dividend_Tracker", "Sugarrush_Dividend_Tracker", 0xf81aa223042d6AaDC0096C781Ef9Ad19Dd20eac6) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }
