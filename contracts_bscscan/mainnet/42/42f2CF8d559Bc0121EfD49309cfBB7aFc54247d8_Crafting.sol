...
contract Crafting is Ownable {
    ...
    address public tokenX1;
    address public tokenX2;
    address public tokenX3;
    address public tokenY;

    IRouter router = IRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);

    uint public tokensDecimals = 8;
    uint public craftingTime = 30 days;
    uint public depositRatio = 50;
    
    uint public rewardMultiplierX1 = 14;
    uint public rewardMultiplierY = 95;

    uint public rateX1OverX2 = 1000;
    ...
    function swapTokens(address _token0, address _token1, uint _amountToken0) public returns(uint) {
        address[] memory path = new address[](2);
        path[0] = _token0;
        path[1] = _token1;
        uint[] memory amountsOut = router.getAmountsOut(_amountToken0, path);
        uint amountOut = amountsOut[0];
        uint amountOutMin = amountOut * 95 / 100;
        uint deadline = block.timestamp + 10 minutes;

        uint[] memory amountsResult = router.swapExactTokensForTokens(_amountToken0, amountOutMin, path, msg.sender, deadline);
        return amountsResult[0];
    }
    ...
    function deposit(uint _amountX1, uint _amountX2, uint _amountX3) external {
        ...
        uint convertedAmountX2InX3 = swapTokens(tokenX2, tokenX3, _amountX2);
        uint convertedAmountX3InY = swapTokens(tokenX3, tokenY, convertedAmountX2InX3 + _amountX3);
