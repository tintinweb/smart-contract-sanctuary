...
contract TreasuryReward is Ownable {
    ...
    uint256 public DAY2SEC = 60;
    uint256 public MAX_LUCKY_COUNTS = 10;
    uint256 public LaunchTime;
    uint256 public LaunchPeriod;
    ...
    function getWhiteAccounts() public onlyOwner {
       ...
       participants = passedAccounts.length;
    }

    function voteLuckyAccount(address account) public {
        require(participants > 0, "White lists should be over zero");
        require(isWhiteListed(account), "msg.sender is not white listed");
        ...
        if(participants > MAX_LUCKY_COUNTS) {
            ...
            uint256 tempNum = oneRand % participants;
            for(uint k = 0; k < luckyNumbers.length; k++) {
                if(tempNum == luckyNumbers[k])
                    return;
            }
            luckyNumbers.push(tempNum);
            luckyAccounts.push(passedAccounts[tempNum]);
        } else {
            for(uint256 k = 0; k< participants; k++) {
                luckyAccounts.push(passedAccounts[k]);
            }
        }
    }
    ...
    function isWhiteListed(address account) public view returns(bool) {
        ...
        bool isWhite = false;
        if(stakedTime > LaunchTime && stakedTime < LaunchTime + LaunchPeriod * DAY2SEC) {
    ...
    function isLive() public view returns(bool) {
        bool bLive;
        if(block.timestamp > LaunchTime + LaunchPeriod * DAY2SEC && luckyAccounts.length < 10)
            bLive = true;
        return bLive;
    }

    function resetPassedAccounts() internal {
        require(passedAccounts.length > 0, "Passed Accounts lengh is zero");
    ...
    function updateStakingHelper(address _stakingAddr) public onlyOwner {
        require(_stakingAddr != address(0), "Staking Address is zero");

        stakingHelper = IStakingHelper(_stakingAddr);
    }

    function updateLaunchTime(uint256 _newLaunchTime) public onlyOwner {
        require(_newLaunchTime > 0, "new time should be over zero");
        
        LaunchTime = _newLaunchTime;
    }

    function updateLaunchPeriod(uint256 _newLaunchPeriod) public onlyOwner {
        require(_newLaunchPeriod > 0, "new time should be over zero");

        LaunchPeriod = _newLaunchPeriod;
    }
    
    function updateDay(uint256 _newDay2SEC) public onlyOwner {
        require(_newDay2SEC > 0, "new day should be over zero");
        
        DAY2SEC = _newDay2SEC;
    }

    function updateMaxLuckyCounts(uint256 _newMax) public onlyOwner {
        require(_newMax > 0, "new Max should be over zero");
        
        MAX_LUCKY_COUNTS = _newMax;
    }

    function updateTsukiMinimum(uint256 _newMini) public onlyOwner {
        require(_newMini > 0, "New Mini should be over zero");
