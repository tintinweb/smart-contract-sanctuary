...
// LZPToken with Governance.
contract LZPToken is BEP20('LZP', 'LZP'), Pausable, ILZP, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function stake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(_unlockFactor[token] > 0, "LZP: FACTOR_NOT_SET");
        require(_unlockBlockGap[token] > 0, "LZP: BLOCK_GAP_NOT_SET");
    ...
    function unstake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(amount > 0, "LZP: ZERO_UNSTAKE_AMOUNT");
        LpStakeInfo storage info = _stakingRecords[_msgSender()][token];
        require(amount <= info.amountStaked, "LZP: UNSTAKE_AMOUNT_EXCEEDED");
        uint256 unlockedAmount = _settleUnlockAmount(_msgSender(), token, info.amountStaked, info.blockNumber);
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "LZP::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "LZP::delegateBySig: invalid nonce");
        require(now <= expiry, "LZP::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "LZP::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying LZPs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "LZP::_writeCheckpoint: block number exceeds 32 bits");
