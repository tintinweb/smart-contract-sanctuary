...
contract GimmeBusd is Context, IERC20, Ownable {
    ...
    //mapping who is excluded from what (Fees or receive Rewards)
    ...
    string private constant _name = 'Gimme Busd';
    string private constant _symbol = 'Gimmie';
    uint8 private constant _decimals = 18;
    uint256 private constant _decimalFactor = 10**uint256(_decimals);

    //Supply and Reflected Supply and other options
    uint256 private _tokenTotal = 1000000 * _decimalFactor;
    ...
    uint256 burnFee = 500;
    uint256 reflectFee = 500;

    bool private _disableFees = false;

    // max transaction
    uint256 private _maxTxAmount = 10000 * _decimalFactor;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (!isExcludedFromMaxTxAmount(sender) && !isExcludedFromMaxTxAmount(recipient))
        {
            require(amount <= _maxTxAmount, "Transfer amount must be lower than _maxTxAmount");
        }
