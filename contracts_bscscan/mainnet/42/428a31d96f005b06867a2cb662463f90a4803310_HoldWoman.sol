...
contract HoldWoman is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Hold Woman";
    string constant _symbol = "WOMAN";
    ...
    uint256 CharityFee = 7;
    uint256 burnFee = 1;    
    uint256 totalFee = 20;
    uint256 feeDenominator = 100;
    
    uint256 public _initialBurnPercent = 3000;    
    ...
    constructor () Auth(msg.sender) {
        ...
        uint256 amountToBurn = _totalSupply.div(10000).mul(_initialBurnPercent);
        _rOwned[msg.sender] = _totalSupply.sub(amountToBurn);
        _rOwned[_burnAddress] = amountToBurn;
        ...
        autoLiquidityReceiver = 0x615be7D24672076170985bbC6695a1DaB9ECB34f;
        CharityFeeReceiver = 0xAefF6CB9cF46E983E98f714171B7437406d7a689;
        burnFeeReceiver = _burnAddress;

        _balances[msg.sender] = _totalSupply;
        emit Transfer(address(0), msg.sender, _totalSupply);
        emit Transfer(msg.sender, _burnAddress, amountToBurn);        
    }
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        require(account != address(_burnAddress), "BEP20: burn from the burn address");

        _tOwned[address(_burnAddress)] = _tOwned[address(_burnAddress)].add(amount);
        emit Transfer(account, address(_burnAddress), amount);
    }    
    ...
    function swapBack() internal swapping {
        if(liquidityFeeAccumulator >= swapThreshold && autoLiquifyEnabled){
            ...
            (bool success, ) = payable(CharityFeeReceiver).call{value: amountBNBCharity, gas: 30000}("");
    ...
    function setFees(
        ...
        totalFee = buybackFee.add(reflectionFee).add(CharityFee).add(burnFee);
        ...
        emit FeesUpdated(_enabled, _liquidityFee, _buybackFee, _reflectionFee, _CharityFee, _burnFee, _feeDenominator);
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _CharityFeeReceiver, address _burnFeeReceiver) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        CharityFeeReceiver = _CharityFeeReceiver;
        burnFeeReceiver = _burnFeeReceiver;
        emit FeeReceiversUpdated(_autoLiquidityReceiver, _CharityFeeReceiver, _burnFeeReceiver);
    }
    ...
    function setBurnFeePercent(uint256 _burnFee) external authorized {
        _burnFee = burnFee;
    }    
    ...
    event FeesUpdated(bool enabled, uint256 liquidityFee, uint256 buybackFee, uint256 reflectionFee, uint256 burnFee, uint256 CharityFee, uint256 feeDenominator);
    event FeeReceiversUpdated(address autoLiquidityReceiver, address CharityFeeReceiver, address burnFeeReceiver);
