...
abstract contract YSLOpt is AccessControlUpgradeable, IStrategy {
    ...
    function compound() external virtual override;
    ...
    function collectControllerFee(uint256 _amount, address _router, address _feeAddress) internal {
        uint256 fee = (_amount * controllerFee) / FEE_ACCURACY;

        address[] memory _path = new address[](2);
        _path[0] = BUSD;
        _path[1] = WBNB;
        IERC20(BUSD).approve(_router, fee);
        IPancakeRouter02(_router).swapExactTokensForETH(fee, 1, _path, _feeAddress, block.timestamp + 10000);
    }
...
contract YSLOptPancake is YSLOpt {
    ...
    function compound() external override {
        ...
        if (wantBalance > DUST) {
            ...
            collectControllerFee(usdBefore, pancakeSwap, 0xa03C61784D51565411DE238de0bfF5901E4d1477);
    ...
    function mintBonus(
        ...
        ILock(lockContract).lock(usersIncome, 180 days, _user);
    }
    ...
    function sendReferralBonus(uint256 syslemission, address _user) internal {
        ...
        ILock(lockContract).lock(bonus, 180 days, creator);
    }
    ...
    function currentRewardUSD(address _user) public view returns (uint256) {
        ...
        totalCompounds -= totalDeposited;

        if (totalCompounds <= DUST) {
            return 0;
        }

        uint256 totalCompoundsUsd = IStrategySwap(stratSwap).getBusdAmount(totalCompounds, lpToken, pancakeSwap, path0, path1);

        // Pending rewards
        uint256 pendingCakes = IPancakeMaster(pancakeMaster).pendingCake(id, address(this));
        if (pendingCakes > DUST) {
            address[] memory cakePath = new address[](2);
            cakePath[0] = address(want);
            cakePath[1] = BUSD;
            totalCompoundsUsd += IStrategySwap(stratSwap).getBusdAmount(pendingCakes, cakePath, pancakeSwap);
        }

        // Get users share of compounds: rewards * user's share
        uint256 userCompoundsUsd = (totalCompoundsUsd * userDeposited[_user]) / totalDeposited;

        if (maturityEarn > 0) {
            uint256 period = block.timestamp - lastEarn[_user];
            if (period < maturityEarn) {
                userCompoundsUsd = (userCompoundsUsd * period) / maturityEarn;
            }
        }
        if (userCompoundsUsd <= DUST) {
            return 0;
        }

        // Optimisation
        uint256 usersIncome = (userCompoundsUsd * coef_opt) / FEE_ACCURACY;
