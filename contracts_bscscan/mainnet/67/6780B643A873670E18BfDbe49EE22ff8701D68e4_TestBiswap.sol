...
contract TestBiswap {
    ...
    function addLiquidity(address _tokenA, address _tokenB, uint _amountA, uint _amountB) public returns (uint){
        ...
        (uint amountA, uint amountB, uint liquidity) = IBiswapRouter02(BISWAP_ROUTER).addLiquidity(_tokenA, _tokenB, _amountA, _amountB, 1, 1, address(this), block.timestamp);
    ...
    function removeLiquidity(address _tokenLP, uint _amountLP) public {
        ...
        IBiswapRouter02(BISWAP_ROUTER).removeLiquidity(IBiswapPair(_tokenLP).token0(), IBiswapPair(_tokenLP).token1(), _amountLP, 1, 1, address(this), block.timestamp);
    }
    ...
    function swapExactTokensForTokens(address _tokenA, address _tokenB, uint _amountIn, uint typeSwap) public {
        ...
        if (typeSwap == 1) {
            ...
            IBiswapRouter02(BISWAP_ROUTER).swapExactTokensForTokens(_amountIn, 1, path, address(this), block.timestamp);
        } else if (typeSwap == 2) {
