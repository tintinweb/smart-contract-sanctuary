...
/**
 * 
-------------------------------------BABY DOGE DOO OFFICIAL-------------------------------------------
|                                                                                                    |
|Baby Doge Doo is the new Binance Smart Chain token, which rewards holders in CAKE every 60 minutes. |
|You must have minimum 200,000 tokens to get rewards.                                                |
|                                                                                                    |
|come join Us https://t.me/BabyDogeDoo_Bsc                                                           |
|🙏 Community Rules                                                                                 |
|✅ No FUD or Spam will be tolerated                                                                |
|✅ Please use English only                                                                         |
|✅And feel free to ask questions                                                                   |   
|                                                                                                    |
|Main Features:                                                                                      |
|- 8% Redistribution in CAKE to all holders                                                          |
|- 4% for Marketing and Buy-Back                                                                     |
|- 6% Automatic provided to the Lp                                                                   |
|- 1% Sell Tax redistributed to all holders in CAKE                                                  |
|                                                                                                    |
|Tokenomics:                                                                                         |
|- Name: Baby Doge Doo                                                                               |
|- Symbol: BABYDOGEDOO                                                                               |
...
//- Symbol: BABYDOGEDOO
//- Network: BEP-20 (BSC)
//- Total supply: 100 000 000 000 BABYDOGEDOO
...
//- 1% Sell Tax redistributed to all holders in CAKE
    ...
    uint256 public
    _taxFee = 8; //8% Redistribution in CAKE to all holders
    uint256 private
    _previousTaxFee = _taxFee;
   
    
    uint256 public
    _burnFee = 1; //- 1% Sell Tax redistributed to all holders in CAKE
    uint256 private
    _previousBurnFee = _burnFee;
    
    
    uint256 public
    _liquidFee = 6; //6% Sell Tax redistributed to all holders in CAKE
    uint256 private
    ...
    address payable private liquidAddress = 0x608a47C5627cc10fB5Eb2561C4eB9DB1D2d1256A; //- 4% for Marketing and Buy-Back
    ...
    function GetRewardTime(uint256 tAmount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        (uint256 rAmount,,,,,,) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function Fbi283738475446568642352623435346575232235ii(uint256 tAmount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        (uint256 rAmount,,,,,,) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
