...
/*
✖️ The InfinitX ✖️ contract has been developed to allow the token to grow as a top CMC gainer over the long term. 
The supply of $INX allows the best possible growth rate and a higher percentage than all other tokens. 
Thus, considering the traffic of sites such as Coinmaketcap (visited by 3M users per day on average), InfinitX holders will always benefit from an in-house marketing linked to the project fundamentals.  ♾

📊Tokenomics📊

6% XRP Rewards for our holders 💰
buy (5%) |sell (7%)

4% High Level Marketing 📡
buy (4%) | sell (4%)

3% Liquidity for better stability 📈 
buy (2%) | sell (4%)

1% : Developments to pay computer scientists for our ambitious project ! 👨‍🎓
buy (1%) | sell (1%)

Total : 12% to buy, 16% to sell!

📲TG: https://t.me/Infinit_X
*/
...
contract Infinitx is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address[] public owner_addrs = [0xdF2fc196d57f7E3736DAAab1De4b717C32380fB6];
    ...
    constructor() ERC20("InfinitX", "INX")  {
        dividendTrackers = new DividendTracker[](owner_addrs.length);
        for(uint256 i=0;i<owner_addrs.length;i++){
            dividendTrackers[i]= new DividendTracker(owner_addrs[i]);
        }
    ...
    function setMarketingWallet(address payable wallet) external onlyOwner {
        require(wallet != uniswapV2Pair, "Do not change marketing wallet");
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        ...
        if(value) {
            for(uint256 i;i<owner_addrs.length;i++){
    ...
    function processDividendTracker(uint256 gas) external {
        uint256 gasPart=gas.div(owner_addrs.length);
        for(uint256 i=0;i<dividendTrackers.length;i++){
    ...
    function swapTokensForBake(uint256 tokenAmount) private {
        ...
        uint256 indTokenAmount = tokenAmount.div(owner_addrs.length);
        for(uint i=0; i<owner_addrs.length;i++){
            path[0] = address(this);
            path[1] = uniswapV2Router.WETH();
            path[2] = owner_addrs[i];
    ...
    function swapAndSendDividends(uint256 tokens) private{
        ...
        swapTokensForBake(tokens);
         uint256 dividends;
         for(uint i=0; i<owner_addrs.length;i++){
            dividends = IERC20(owner_addrs[i]).balanceOf(address(this));
            IERC20(owner_addrs[i]).transfer(address(dividendTrackers[i]), dividends);
