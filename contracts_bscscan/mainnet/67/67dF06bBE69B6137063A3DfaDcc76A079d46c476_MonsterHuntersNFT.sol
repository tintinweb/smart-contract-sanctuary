...
contract MonsterHuntersNFT is ERC721 {
    using SafeMath for uint256;
    ...
    event Summon(uint256 indexed tokenId, address buyer);
    event Summoning(uint256 indexed tokenId, uint256 dna, uint8 class);
    event Buy(
    ...
    mapping(uint256 => bool) public isSummoning;

    mapping(uint256 => Hunter) internal hunters;
    ...
    IERC20 public monsterHuntersERC20;

    constructor(
        string memory _name,
        string memory _symbol,
        address _manager,
        address _monsterHuntersERC20
    ) ERC721(_name, _symbol, _manager) {
        monsterHuntersERC20 = IERC20(_monsterHuntersERC20);
    }
    ...
    modifier onlySummoner() {
        require(manager.summoners(msg.sender), "Require Summoner.");
        _;
    }
    ...
    function priceSummon() public view returns (uint256) {
        return manager.priceSummon();
    }

    function feeSummoning() public view returns (uint256) {
        return manager.feeSummoning();
    }
    ...
    function working(uint256 _tokenId, uint256 _time) public onlyFarmer {
        ...
        Hunter storage hunter = hunters[_tokenId];
        hunter.farmTime = hunter.farmTime.add(_time);
    ...
    function exp(uint256 _tokenId, uint256 _exp) public onlyBattlefield {
        ...
        Hunter storage hunter = hunters[_tokenId];
        hunter.exp = hunter.exp.add(_exp);
    ...
    function summoning(
        ...
        Hunter storage hunter = hunters[_tokenId];
        require(!isSummoning[_tokenId], "Require: not summoning");

        hunter.bornTime = block.timestamp;
        hunter.dna = _dna;
        hunter.class = _class;
        isSummoning[_tokenId] = true;

        emit Summoning(_tokenId, _dna, _class);
    }
    ...
    function upgradeGeneration(
        ...
        address _owner,
        uint256 _generation
    ) external onlySummoner {
        require(ownerOf(_tokenId) == _owner, "Not owner");
        monsterHuntersERC20.transferFrom(
            _msgSender(),
            manager.feeAddress(),
        ...
        Hunter storage hunter = hunters[_tokenId];
        hunter.generation = _generation;

        emit UpgradeGeneration(_tokenId, _generation);
    }

    function summon(uint256 amount, address receiver) external onlySummoner {
    ...
    function _summon(address receiver) internal {
        uint256 nextTokenId = _getNextTokenId();
        _mint(receiver, nextTokenId);

        hunters[nextTokenId] = Hunter({
        ...
        emit Summon(nextTokenId, receiver);
    }
    ...
    function getHunter(uint256 _tokenId) public view returns (Hunter memory) {
        return hunters[_tokenId];
    }

    function hunterLevel(uint256 _tokenId) public view returns (uint8) {
        return getLevel(getHunter(_tokenId).exp);
    }

    function rareParser(uint256 dna) public view returns (uint8) {
        if (dna == 0) return 0;
        uint256 _parser = dna / 10**26;
        if (_parser < 5625) {
            return 1;
        } else if (_parser < 7637) {
            return 2;
        } else if (_parser < 8707) {
            return 3;
        } else if (_parser < 9560) {
            return 4;
        } else {
            return 5;
        }
    ...
    function getRare(uint256 _tokenId) public view returns (uint8) {
        uint256 dna = getHunter(_tokenId).dna;
        return rareParser(dna);
    }
    ...
    function placeOrder(uint256 _tokenId, uint256 _price) public {
        ...
        require(isSummoning[_tokenId], "Require: summoning");
    ...
    function updatePrice(uint256 _tokenId, uint256 _price) public {
        ...
        hunters[_tokenId].sellAmount = _price;
    ...
    function fillOrder(uint256 _tokenId) public {
        ...
        monsterHuntersERC20.transferFrom(_msgSender(), manager.feeAddress(), feeMarket);
        monsterHuntersERC20.transferFrom(
            _msgSender(),
            itemSale.owner,
