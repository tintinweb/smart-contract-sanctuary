...
// << Join Telegram: https://t.me/opensand >>
// << Website: https://www.opensand.net >>

/* OpenSand strives to become the principal pre-audited cryptocurrency token launchpad, and the first P2P security protocol backed blockchain that uses own language to provide required security.
Our central purpose is to expose scam projects guarantee safety to investors, and instill confidence in the cryptocurrency market in its entirety. Our approach will be to implement methodical
...
contract SANDO is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("SANDO", "SANDO") {
    	...
    	marketingWallet = 0x4d71B36eCb99B157c5caA44993a2C8ddEA6B3430;

    	cakeDividendToken = 0x5Ad3003826BFFEeB0CF14d9068f377A014724653; //
    ...
    receive() external payable {
  	...
  	/* Below function is needed in order to deploy contract, it will be terminated after launch when verifying on bscscan, safety first! */
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "SANDO: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "SANDO: The new dividend tracker must be owned by the SANDO token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("SANDO_Dividend_Tracker", "SANDO_Dividend_Tracker", 0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "SANDO_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "SANDO_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "SANDO_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "SANDO_Dividend_Tracker: Cannot update claimWait to same value");
