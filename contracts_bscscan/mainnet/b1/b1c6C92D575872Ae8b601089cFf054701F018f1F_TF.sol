...
contract TF is ERC20, Ownable {
    ...
    TFDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public REWARD = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    ...
    constructor() ERC20("TF", "TF") {
        ...
        dividendTracker = new TFDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "TF: The dividend tracker already has that address");

        TFDividendTracker newDividendTracker = TFDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TF: The new dividend tracker must be owned by the TF token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TF: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TF: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TF: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TF: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TF: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TF: Cannot update gasForProcessing to same value");
    ...
    function switchToken(address rewardToken) public onlyOwner {
        ...
        dividendTracker.switchToken(rewardToken);
    }
...
contract TFDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor()  DividendPayingToken("TF_Dividen_Tracker", "TF_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**9);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "TF_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "TF_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TF contract.");
    }
    ...
    function excludeFromDividendsBool(address account, bool excluded) external onlyOwner {
        require(excludedFromDividends[account] != excluded, "TF: Account is already the value of 'excluded'");
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TF_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TF_Dividend_Tracker: Cannot update claimWait to same value");
