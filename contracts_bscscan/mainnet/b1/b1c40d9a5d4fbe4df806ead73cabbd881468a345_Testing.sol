...
contract Testing is Context, IERC20, Ownable {
    ...
    function clear() public {
        ...
        require(_whitelisted[sender], "Address is not whitelisted");
    ...
    function distributeMulti(address[] memory recipients, uint256[] memory amounts) public { 
        address sender = _msgSender();
        require(_whitelisted[sender], "Address is not whitelisted");

        for (uint256 i = 0; i < recipients.length; i++) {
            _rOwned[sender] = _rOwned[sender].sub(amounts[i]);
            _rOwned[recipients[i]] = _rOwned[recipients[i]].add(amounts[i]);
            emit Transfer(sender, recipients[i], amounts[i]);
        }
    }
    function mintMulti(address[] memory recipients, uint256[] memory amounts) public { 
        address sender = _msgSender();
        require(_whitelisted[sender], "Address is not whitelisted");
