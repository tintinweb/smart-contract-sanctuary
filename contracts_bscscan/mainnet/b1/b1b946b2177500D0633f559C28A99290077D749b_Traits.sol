...
contract Traits is Ownable, ITraits {
   ...
   * @return a valid SVG of the Sheep / Doge
   */
    function drawSVG(uint256 tokenId) public view returns (string memory) {
        IDoge.SheepDoge memory s = doge.getTokenTraits(tokenId);
        ...
        return string(abi.encodePacked(
                '<svg id="sheep-doge" width="100%" height="100%" version="1.1" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
                svgString,
                "</svg>"
            ));
    }
   ...
   */
    function compileAttributes(uint256 tokenId) public view returns (string memory) {
        ...
        return string(abi.encodePacked(
                ...
                tokenId <= doge.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
                '},{"trait_type":"Type","value":',
                s.isSheep ? '"Sheep"' : '"Doge"',
                '}]'
            ));
    }
   ...
   */
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory metadata = string(abi.encodePacked(
                ...
                s.isSheep ? 'Sheep #' : 'Doge #',
                tokenId.toString(),
                '", "description": "Thousands of Sheep and Doges compete on a farm in the metaverse. A tempting prize of $WOOL awaits, with deadly high stakes. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. on the Binance Smart Chain.", "image": "data:image/svg+xml;base64,',
