...
contract BNBSuper is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BNBSuperDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable public operations1Address =
        payable(0x19F8780ACd50b6d41d135FF9a393e286F4ea457C); //Marketing Wallet 3%
    address payable public operations2Address =
        payable(0xeEE176eD71d2a725A0B8C18877BAd557F81bb937); //Buyback wallet 5%
    ...
    uint256 public BNBRewardsSellFee = 18;
    ...
    constructor() ERC20("BNBSuper", "BNBS") {
        dividendTracker = new BNBSuperDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        BNBSuperDividendTracker newDividendTracker = BNBSuperDividendTracker(
            payable(newAddress)
        );
...
contract BNBSuperDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "BNBSuper_Dividend_Tracker",
            "BNBSuper_Dividend_Tracker"
        )
    {
        bNBSuper = BNBSuper(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BNBSuper_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BNBSuper_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BNBSuper contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(bNBSuper);

            uint256 prevBalance = bNBSuper.balanceOf(address(this));
            ...
            {
                uint256 received = bNBSuper.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    bNBSuper.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
