...
contract DividendPayingToken is ERC20, Ownable, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address Wfloki = 0x2B3F34e9D4b127797CE6244Ea341a83733ddd6E4;
  ...
  function distributeDividendscake(uint256 amount) public onlyOwner {
    require(totalSupply() > 0);

    if (amount > 0) {
      magnifiedDividendPerShare.cake = magnifiedDividendPerShare.cake.add(
        (amount).mul(magnitude) / totalSupply()
      );
      emit DividendsDistributed(msg.sender, amount, "cake");

      _totalDividendsDistributed.cake = _totalDividendsDistributed.cake.add(amount);
    }
  }

  function distributeDividendsfloki(uint256 amount) public onlyOwner {
    require(totalSupply() > 0);

    if (amount > 0) {
      magnifiedDividendPerShare.floki = magnifiedDividendPerShare.floki.add(
        (amount).mul(magnitude) / totalSupply()
      );
      emit DividendsDistributed(msg.sender, amount, "floki");

      _totalDividendsDistributed.floki = _totalDividendsDistributed.floki.add(amount);
    }
  }
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (Tokens memory withdrawable) {
    ...
    if (withdrawable.floki > 0 ) {
      withdrawnDividends[user].floki = withdrawnDividends[user].floki.add(withdrawable.floki);
      emit DividendWithdrawn(user, withdrawable.floki, "floki");

      bool success = IERC20(Wfloki).transfer(user, withdrawable.floki);
      if(!success) {
        withdrawnDividends[user].floki = withdrawnDividends[user].floki.sub(withdrawable.floki);
        withdrawable.floki = 0;
      }
    }


    if (withdrawable.cake > 0 ) {
      withdrawnDividends[user].cake = withdrawnDividends[user].cake.add(withdrawable.cake);
      emit DividendWithdrawn(user, withdrawable.cake, "cake");

      bool success = IERC20(cake).transfer(user, withdrawable.cake);
      if(!success) {
        withdrawnDividends[user].cake = withdrawnDividends[user].cake.sub(withdrawable.cake);
        withdrawable.cake = 0;
      }
    }
  }
  ...
  function withdrawableDividendOf(address _owner) public view override returns(Tokens memory acc) {
    ...
    acc.floki -= withdrawnDividends[_owner].floki;
    acc.cake -= withdrawnDividends[_owner].cake;
  }
  ...
  function accumulativeDividendOf(address _owner) public view override returns(Tokens memory) {
    Tokens memory acc;
    acc.floki = magnifiedDividendPerShare.floki.mul(balanceOf(_owner)).toInt256Safe()
    .add(magnifiedDividendCorrections[_owner].floki).toUint256Safe() / magnitude;
    acc.cake = magnifiedDividendPerShare.cake.mul(balanceOf(_owner)).toInt256Safe()
    .add(magnifiedDividendCorrections[_owner].cake).toUint256Safe() / magnitude;
    return acc;
  }
  ...
  function _mint(address account, uint256 value) internal override {
    ...
    magnifiedDividendCorrections[account].floki = magnifiedDividendCorrections[account].floki
    .sub((magnifiedDividendPerShare.floki.mul(value)).toInt256Safe() );
    magnifiedDividendCorrections[account].cake = magnifiedDividendCorrections[account].cake
    .sub((magnifiedDividendPerShare.cake.mul(value)).toInt256Safe() );
  }
  ...
  function _burn(address account, uint256 value) internal override {
    ...
    magnifiedDividendCorrections[account].floki = magnifiedDividendCorrections[account].floki
    .add((magnifiedDividendPerShare.floki.mul(value)).toInt256Safe() );
    magnifiedDividendCorrections[account].cake = magnifiedDividendCorrections[account].cake
    .add((magnifiedDividendPerShare.cake.mul(value)).toInt256Safe() );
  }
...
contract RewardDividendTracker is DividendPayingToken {
  ...
  constructor(address token) DividendPayingToken("FlokiCakeReward_Dividend_Tracker", "FlokiCakeReward_Dividend_Tracker") {
    claimWait = 1800;
  ...
  function _transfer(address, address, uint256) pure internal override {
    require(false, "FlokiCakeReward_Dividend_Tracker: No transfers allowed");
  }

  function withdrawDividend() pure public override {
    require(false, "FlokiCakeReward_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FlokiCakeReward contract.");
  }
  ...
  function updateClaimWait(uint256 newClaimWait) external onlyOperator {
    require(newClaimWait >= 1800 && newClaimWait <= 86400, "FlokiCakeReward_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
    require(newClaimWait != claimWait, "FlokiCakeReward_Dividend_Tracker: Cannot update claimWait to same value");
  ...
  function processAccount(address payable account, bool automatic) public onlyOwner returns (bool) {
    ...
    if(w.floki > 0) {
      emit Claim(account, w.floki, automatic);
    } else {
      res = false;
    }
    if(w.cake > 0) {
      emit Claim(account, w.cake, automatic);
    } else {
      res = false;
    }
...
contract FlokiCake is ERC20, Ownable {
  using SafeMath for uint256;
  ...
  uint8 _decimals = 9;
  uint256 public maxBuyTranscationAmount = 10000000000 * (10**uint256(_decimals));
  uint256 public maxWalletToken = 10000000000 * (10**uint256(_decimals)); // 100% of total supply
  uint256 public swapTokensAtAmount = 20000 * (10**uint256(_decimals));
  bool public enableSwapAndLiquify = true;

  uint256 public dividendRewardsFee = 1600;
  uint256 public wflokiFee = 800;
  uint256 public cakeFee = 800;
  uint256 public accumulatedForcake;
  uint256 public ethMultiplier = 100;
  uint256 public marketingFee = 300;
  uint256 public liquidityFee = 100;
  uint256 public totalFees = wflokiFee + cakeFee + marketingFee + liquidityFee;
  uint256 public denom = 10000;
  address public marketingWallet = 0x883b0E9aA5656dd013FC5a88D87089a0fd5d6879;
  ...
  constructor() ERC20("Floki Cake Token", "FlokiCake") {
    ...
    */
    _mint(owner(), 10000000000 * (10**uint256(_decimals)));
  }
  ...
  function updateDividendRewardFee(uint256 cake, uint256 wfloki) public onlyOwner {
    require(wfloki + cake <= 3000, "Dividend reward tax must be <= 3000");
    require(cake >= 4 * ethMultiplier, "cake fee must be >= 400");
  ...
  function _transfer(
    ...
    if (from != uniswapV2Pair && enableSwapAndLiquify && contractTokenBalance >= swapTokensAtAmount && !swapping && !_isExcludedFromFees[from]) {
      swapping = true;
      uint256 feeWithoutcake = contractTokenBalance - accumulatedForcake;
      uint256 onePart = feeWithoutcake / (totalFees - cakeFee);
      uint256 halfOfLiquidity = onePart.mul(liquidityFee).div(2);

      uint256 swapTofloki = contractTokenBalance - halfOfLiquidity; // all without half of liquidity part

      swapTokensForfloki(swapTofloki);
      uint256 contractflokiBalance = address(this).balance;
      uint256 flokiForcake = contractflokiBalance * accumulatedForcake / swapTofloki;
      uint256 flokiWithoutcake = contractflokiBalance - flokiForcake;
      onePart = flokiWithoutcake / (totalFees - cakeFee - (liquidityFee / 2));
      uint256 flokiForLiquidity = onePart * liquidityFee / 2;
      uint256 flokiForMarketing = onePart * marketingFee;
      uint256 flokiForDividend = flokiWithoutcake - flokiForMarketing - flokiForLiquidity;

      if (flokiForMarketing > 0) {
        transferToMarketing(flokiForMarketing);
      }

      if (flokiForLiquidity > 0) {
        addLiquidity(halfOfLiquidity, flokiForLiquidity);
        emit SwapAndLiquify(halfOfLiquidity, flokiForLiquidity, halfOfLiquidity);
      }

      if (flokiForDividend > 0) {
        sendDividendsInfloki(flokiForDividend);
      }

      if (flokiForcake > 0 ) {
        swapAndSendDividends(flokiForcake, cake);
        accumulatedForcake = 0;
      }
    ...
    if(takeFee) {
      ...
      if (from == uniswapV2Pair) {
        ...
        minuscakeFee = ethAmount * ethMultiplier;
      }

      uint256 fees = amount * (totalFees - minuscakeFee) / denom;

      accumulatedForcake += fees * (cakeFee - minuscakeFee) / (totalFees - minuscakeFee);
  ...
  function swapflokiForDividendTokens(uint256 amount, address recipient, address dividendToken) private {
    address[] memory path = new address[](2);
  ...
  function swapAndSendDividends(uint256 flokiAmount, address dividendToken) private {
    swapflokiForDividendTokens(flokiAmount, address(this), dividendToken);
    ...
    if (success) {
      dividendTracker.distributeDividendscake(dividends);
      emit SendDividends(dividends, "cake");
    }
  }

  function sendDividendsInfloki(uint256 flokiAmount) private {
    (bool success,) = address(dividendTracker).call{value: flokiAmount}("");

    if (success) {
      dividendTracker.distributeDividendsfloki(flokiAmount);
      emit SendDividends(flokiAmount, "floki");
    }
  }
  ...
  function updateEthMultiplier(uint256 _ethMultiplier) external onlyOwner {
    require(cakeFee >= 4 * _ethMultiplier, "Discount cant be greater than cake fee");
