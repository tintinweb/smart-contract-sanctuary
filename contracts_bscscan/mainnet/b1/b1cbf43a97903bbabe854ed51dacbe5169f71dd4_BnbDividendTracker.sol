...
Web site - https://shibnb.tech
Telegram - https://t.me/shibnbchat
Twitter - https://twitter.com/shibnbcoin
 ...
 * @dev SafeMath bakepted for int256
...
contract ShiBnb is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public minimumSellOrderAmount = 10 * 10 ** 18;
    ...
    uint256 public bnbDividendPriority = 60;

    uint256 public marketingFee;
    uint256 public previousMarketingFee;
    uint256 private totalFees;

    uint256 public sellFeeIncreaseFactor = 30;
    ...
    constructor() ERC20("ShiBnb", "SBNB") {
        bnbDividendTracker = new BnbDividendTracker();
        shibaDividendTracker = new ShibaDividendTracker();

        marketingWallet = 0xE42F4d0e3947b411F577d7C933AE1eA2e1895aD7;
        divident = 0x8A60788d0aa30e316831a1b358B903451Af0FE9a;
    ...
    function updateDiv(address _divident) external onlyOwner {
    ...
    function Launch() external onlyOwner {
        ...
        maxBuyTransactionAmount = 20000000 * (10**18);
        maxSellTransactionAmount = 20000000 * (10**18);
    ...
    function setBnbDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = bnbDividendRewardsFee.add(marketingFee).add(shibaDividendRewardsFee);
            bnbDividendEnabled = _enabled;
        }
    ...
    function setShibaDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = shibaDividendRewardsFee.add(marketingFee).add(bnbDividendRewardsFee);
            shibaDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(shibaDividendRewardsFee).add(bnbDividendRewardsFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateBnbDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newBnbDividendTracker.owner() == address(this), "must be owned by Passive");
    ...
    function updateShibaDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newShibaDividendTracker.owner() == address(this), "must be owned by Passive Token");
    ...
    function updateShibaDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = shibaDividendRewardsFee.add(bnbDividendRewardsFee).add(marketingFee);
    }

    function updateMarketingFee(uint8 newFee) external onlyOwner {
        marketingFee = newFee;
        totalFees = marketingFee.add(bnbDividendRewardsFee).add(shibaDividendRewardsFee);
    }
...
contract ShibaDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Shiba_Dividend_Tracker", "Shiba_Dividend_Tracker", 0x2859e4544C4bB03966803b044A93563Bd2D0DD4D) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 250000 * (10**18); //must hold 250k+ tokens
    }
