...
contract GamyFiNFT is ERC721URIStorage, Pausable, AccessControl, Ownable {
    bytes32 public constant UPDATE_TOKEN_URI_ROLE =
        keccak256("UPDATE_TOKEN_URI_ROLE");
    bytes32 public constant UPDATE_CONTRACT_URI_ROLE =
        keccak256("UPDATE_CONTRACT_URI_ROLE");
    bytes32 public constant PAUSED_ROLE = keccak256("PAUSED_ROLE");

    // address public platformAddr;
    // uint256 public platformFee = 1e17;
    ...
    // minters
    mapping(address => bool) private minters_;

    event Created(string name, string symbol, string contractUri);
    ...
    constructor(
        ...
        _setupRole(UPDATE_CONTRACT_URI_ROLE, _msgSender());
        _setupRole(PAUSED_ROLE, _msgSender());
        contractUri = _contractUri;

        emit Created(_name, _symbol, _contractUri);
    }

    modifier isMinter() {
        require(minters_[msg.sender], "Can't mint");
        _;
    }
        ...
        returns (uint256)
    {
        ...
        //     platformAddr.call{value: platformFee}("");
        ...
        emit Mint(_beneficiary, tokenId, _tokenUri);
        return tokenId;
    }

    function addMinter(address _minter) public onlyOwner {
        minters_[_minter] = true;
    }
    ...
    function setContractUri(string memory _contractUri) public {
        require(
            hasRole(UPDATE_CONTRACT_URI_ROLE, _msgSender()),
            "Must have update contract uri role"
        );
        contractUri = _contractUri;
    ...
    function getApprovedBulk(uint256[] memory _tokenIds)
        public
        view
        returns (address[] memory)
    {
        address[] memory tokenApprovals = new address[](_tokenIds.length);
        for (uint256 i = 0; i < _tokenIds.length; ++i) {
            tokenApprovals[i] = getApproved(_tokenIds[i]);
        }
