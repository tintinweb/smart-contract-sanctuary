...
import "./MiniDAIDividendPayingToken.sol";

/**
MiniDAI

Tokenomics : 
-7% redistribution in DAI
-2% liquidity 
-2% marketing 
...
Tg: https://t.me/MiniDAI
*/
contract MiniDAI is ERC20, Ownable {
    using SafeMath for uint256;

    IPancakeSwapV2Router02 public PancakeSwapV2Router;
    address public immutable PancakeSwapV2Pair;

    address public immutable BANANA = address(0x1AF3F329e8BE154074D8769D1FFa4eE058B1DBc3); // DAI
	address payable public MarketingWallet = payable(0x2a44948F3580a7353c0E6ADe44d31dE2B8160716); // Marketing Address

    bool private swapping;

    MiniDAIDividendTracker public dividendTracker;
    ...
    constructor() ERC20("MiniDAI", "MDAI") {
        ...
        dividendTracker = new MiniDAIDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MiniDAI: The dividend tracker already has that address");

        MiniDAIDividendTracker newDividendTracker = MiniDAIDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MiniDAI: The new dividend tracker must be owned by the MiniDAI token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MiniDAI: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PancakeSwapV2Pair, "MiniDAI: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MiniDAI: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "MiniDAI: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MiniDAI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MiniDAI: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!tradingOpen) {
            require(canTransferBeforetradingOpen[from], "MiniDAI: This account cannot send tokens until trading is enabled");
        }
...
contract MiniDAIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() DividendPayingToken("MiniDAI_Dividend_Tracker", "MiniDAI_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(true, "MiniDAI_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "MiniDAI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MiniDAI_Dividend_Tracker: Cannot update claimWait to same value");
