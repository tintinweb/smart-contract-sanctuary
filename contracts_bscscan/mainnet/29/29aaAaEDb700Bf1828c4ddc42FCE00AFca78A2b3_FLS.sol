...
contract FLSPaybackTrack is PaybackReward, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() PaybackReward("FLS_Reward_Tracker", "FLS_Reward_Tracker") {
        claimWait = 7200;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "FLS_Reward_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "FLS_Reward_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FLS contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "FLS_Reward_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FLS_Reward_Tracker: Cannot update claimWait to same value");
...
contract FLS is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    FLSPaybackTrack public PaybackTrack;

    address public walletMarketingTeam =0x5D8ec5735f0FcEAdefDF34DCD8B8B1A3461aDb2c;
...
uint256 public  payBackPercent = 6;
  ...
  ILP public adressLiquidity;
    ...
    constructor() ERC20("FLOKI SPACE", "FLS") {

        PaybackTrack = new FLSPaybackTrack();
 ...
 function rebaseToken(uint256 epoch, int256 supplyDelta)
  ...
  returns (uint256)
  {
    ...
    function updateDEXV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(DEXV2Router), "FLS: The router already has that address");
        emit UpdateDEXV2Router(newAddress, address(DEXV2Router));
        DEXV2Router = IDEXV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FLS: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FLS: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FLS: Cannot update gasForProcessing to same value");
