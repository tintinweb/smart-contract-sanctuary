...
contract THUGnyanDOGE is Context, IERC20, Ownable {
    ...
    string constant private _name = "THUG nyanDOGE";
    string constant private _symbol = "TNDOGE";
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256[4] memory) {
        uint256[4] memory tValues = [0, calculateTaxFee(tAmount), calculateLiquidityFee(tAmount), calculateBurnFee(tAmount)];
        tValues[0] = _getTTransferAmount(tAmount, tValues);

        return tValues;
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity,  uint256 tBurn,  uint256 currentRate) private pure returns (uint256[3] memory) {
        ...
        uint256[4] memory tempRValues = [rAmount, rLiquidity, rFee, rBurn];
