...
contract FlokiX is ERC20, Ownable {
    ...
    uint256 public BUSDRewardsSplitValue = 8;
    ...
    uint256 public buyingFeesWithDecimals = (BUSDRewardsSplitValue.add(liquiditySplitValue).add(marketingSplitValue).add(bananaRewardSplitValue)) * feeDecimalFactor;
    
    uint256 public sellingFeesWithDecimals = (BUSDRewardsSplitValue.add(liquiditySplitValue).add(marketingSplitValue).add(bananaRewardSplitValue)) * feeDecimalFactor;
    
    address public _marketingWalletAddress = 0x1174357f7320ec4046042Be895F2Ea30f42e4e1C;
    bool public hasLaunched;
    address public liquidityFeeReceiver = 0xA94172ea9ADFFba0Ea239A21089b6C77c18a2604;
    ...
    constructor() public ERC20("FlokiX", "FLKX") {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FlokiX: The dividend tracker already has that address");

        FlokiXDividendTracker newDividendTracker = FlokiXDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FlokiX: The new dividend tracker must be owned by the FlokiX token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FlokiX: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FlokiX: Account is already the value of 'excluded'");
    ...
    function setBUSDRewardsSplitValue(uint256 value) external onlyOwner{
        require(value >= 1 && value <= 1000);
        BUSDRewardsSplitValue = value;
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FlokiX: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FlokiX: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FlokiX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FlokiX: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 marketingTokens = contractTokenBalance.mul(marketingSplitValue).div(BUSDRewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);
            swapAndSendToFee(marketingTokens);

            uint256 swapTokens = contractTokenBalance.mul(liquiditySplitValue).div(BUSDRewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);
            swapAndLiquify(swapTokens);

            totalBananas += contractTokenBalance.mul(bananaRewardSplitValue).div(BUSDRewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);

            uint256 sellTokens = contractTokenBalance.mul(BUSDRewardsSplitValue).div(BUSDRewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);
    ...
    function swapAndSendDividends(uint256 tokens) private{
        uint256 initialCardanoBalance = IERC20(BUSD).balanceOf(address(this));
        swapTokensForBUSD(tokens);
        uint256 dividends = (IERC20(BUSD).balanceOf(address(this))).sub(initialCardanoBalance);
...
contract FlokiXDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("FlokiX_Dividend_Tracker", "FlokiX_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function withdrawDividend() public override {
        require(false, "FlokiX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FlokiX contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FlokiX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FlokiX_Dividend_Tracker: Cannot update claimWait to same value");
