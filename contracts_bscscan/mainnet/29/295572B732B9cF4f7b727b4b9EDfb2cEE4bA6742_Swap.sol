...
contract Swap is Ownable, ReentrancyGuard {
  ...
  /// @dev Throws if called by any account other than the strat.
  modifier onlyStrat() {
    require(isStrat(), "caller is not the strat");
    _;
  }

  /// @dev Returns true if the caller is the current strat.
  function isStrat() public view returns (bool) {
    return okStrats[msg.sender];
  }

  function swapQuery(uint256 amount, address src, address dest) external nonReentrant onlyStrat {
  ...
  function swap(uint256 amount, address src, address dest, uint256 routeIndex) external nonReentrant onlyStrat {
    require(amount <= SafeToken.balanceOf(src, msg.sender), "insufficient balance");
    SafeToken.safeTransferFrom(src, msg.sender, address(this), amount);
    SafeToken.safeApprove(src, address(bestrate), uint(-1));
    uint256 amountOut = bestrate.trade(routeIndex, src, amount, dest, 0, partnerIndex);
  ...
  function setQuery(INarwhalSwapBestRateQuery _query, uint256[] calldata _routerIndex) external onlyOwner nonReentrant {
    query = _query;
    bestrate = INarwhalSwapRouter(_query.narwhal());
    routerIndex = _routerIndex;
  }
