...
contract satoshicakeUSDC is Context, IERC20, Ownable {
    ...
    uint256 private _tbnbrewardTokenTotal;

    string private _name = "SATOSHICAKE";
    string private _symbol = "SATOSHICAKE";
    ...
    uint256 public _bnbrewardTokenFee = 14;
    uint256 private _previousbnbrewardTokenFee = _bnbrewardTokenFee;
    
    uint256 public _maxTxAmount = 1000 * 10**9 * 10**9;
    uint256 private minimumTokensBeforeSwap = 1 * 10**9 * 10**9; 
    
    address payable public BNBREWARDS_DISTRIBUTION = 0xE1378ACb40192A631E396Ea5a53795576c47dE2f;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _tbnbrewardTokenTotal = _tbnbrewardTokenTotal.add(address(this).balance);
        TransferCharityETH(BNBREWARDS_DISTRIBUTION, address(this).balance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_bnbrewardTokenFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _bnbrewardTokenFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousbnbrewardTokenFee = _bnbrewardTokenFee;
        
        _taxFee = 0;
        _burnFee = 0;
        _bnbrewardTokenFee = 0;
    }
    
    function restoreAllFee() private {
        _taxFee = _previousTaxFee;
        _burnFee = _previousBurnFee;
        _bnbrewardTokenFee = _previousbnbrewardTokenFee;
    }
    ...
    function setbnbrewardTokenFeePercent(uint256 bnbrewardTokenFee) external onlyOwner() {
        _bnbrewardTokenFee = bnbrewardTokenFee;
    }
