...
// File: contracts/StakingV2/PRV2Referral.sol
...
// File: contracts/LockedStaking/LockedMasterChef.sol
...
abstract contract ReentrancyGuard {
 ...
 contract PRVGLockedMasterChef is Ownable,ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    ...
    mapping(address => uint256) public pendingRemaining;
    ...
    event onPoolAdded(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP,  bool _withUpdate);
    event onPoolSet(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP,  bool _withUpdate);
    ...
    event onSetPrvReferral(address previousAddress,address newAddress);
    event onChangeOperator(address previousAddress,address newAddress);
    ...
    constructor(
        ...
        panaltyWallet = _panaltyWallet;
        operator = msg.sender;
        add(1800,_prvg,0,false);
    }
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken , uint16 _depositFeeBP,  bool _withUpdate) internal  {
    ...
    function pendingPrvg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return pending.add(pendingRemaining[_user]);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 prvReward =multiplier.mul(prvPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        if (prvg.totalSupply().add(prvReward)  <= prvg.MAX_SUPPLY()) {
            //  mint as normal as not at maxSupply
            prvg.mint(address(this), prvReward);
        } else {
            // mint the difference only to MC, update prvReward
            prvReward = prvg.MAX_SUPPLY().sub(prvg.totalSupply());
            prvg.mint(address(this), prvReward );
        }
        if (prvReward  != 0) {
                pool.accPrvPerShare =
        pool.accPrvPerShare.add(prvReward.mul(1e18).div(lpSupply)); 
        }
        pool.lastRewardBlock = block.number;
    } 
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant  {
        require(block.timestamp< expireTime,"Staking CLOSED");
        PoolInfo storage pool = poolInfo[_pid];
        ...
        uint256 pendingReward  = user.amount.mul(pool.accPrvPerShare).div(1e18).sub(user.rewardDebt);
        pendingRemaining[msg.sender] =  pendingRemaining[msg.sender].add(pendingReward);
    ...
    function withdraw(uint256 _pid) public  nonReentrant{
        ...
        updatePool(_pid);
        uint256 currentPendingReward = user.amount.mul(pool.accPrvPerShare).div(1e18).sub(user.rewardDebt);
        pendingRemaining[msg.sender] =  pendingRemaining[msg.sender].add(currentPendingReward);

        bool isPanaltyApplied =  block.timestamp<expireTime;

        uint256 totalPendingReward =  pendingRemaining[msg.sender];
        uint256 rewardPenalty = isPanaltyApplied?totalPendingReward.mul(300).div(1000):0;
        uint256 finalReward = totalPendingReward.sub(rewardPenalty);

        uint256 totalPrincipalAmount = user.amount;
        uint256 principalPanalty = isPanaltyApplied?totalPrincipalAmount.mul(300).div(1000):0;
        uint256 finalPrincipal  = totalPrincipalAmount.sub(principalPanalty);


        user.rewardDebt = 0;
        user.amount = 0;
        pendingRemaining[msg.sender] = 0;

        
        uint256 commissions =  prvReferral.payReferralComission(msg.sender, finalReward,false);
        safePrvTransfer(address(prvReferral), commissions);

        uint256 finalAmount = finalPrincipal.add(finalReward).sub(commissions);
        uint256 totalPanalty = rewardPenalty.add(principalPanalty);
        if(totalPanalty>0){
            safePrvTransfer(panaltyWallet,totalPanalty);
        }
        if(finalAmount>0){
            safePrvTransfer(msg.sender,finalAmount);
        }
    ...
    function emergencyWithdraw(uint256 _pid) public  nonReentrant{
        ...
        user.amount = 0;
        user.rewardDebt = 0;
        uint256 panalty = block.timestamp> expireTime?0:amount.mul(300).div(1000);
        if(panalty>0){
            safePrvTransfer(panaltyWallet,panalty);
        }

        pool.lpToken.safeTransfer(address(msg.sender), amount.sub(panalty));
        emit EmergencyWithdraw(msg.sender, _pid, amount.sub(panalty));
    }
    ...
    function safePrvTransfer(address _to, uint256 _amount) internal {
        uint256 prvBal = prvg.balanceOf(address(this));
        if (_amount > prvBal) {
            prvg.transfer(_to, prvBal);
        } else {
            prvg.transfer(_to, _amount);
        }
    }
   ...
   // Update the prv referral contract address by the owner
    function setPrvReferral(PRV2Referral _prvReferral) public onlyOwner {
        emit onSetPrvReferral(address(prvReferral),address(_prvReferral));
    ...
    function changeOperator(address addr) public {
        ...
        emit onChangeOperator(operator,addr);
    ...
    function payComissionOnDeposit(IBEP20 token,uint256 amount,address user) internal returns(uint256){
        ...
        if(customDepositReferral[refererer] == true && customDepositReferralComission[refererer] >0){
