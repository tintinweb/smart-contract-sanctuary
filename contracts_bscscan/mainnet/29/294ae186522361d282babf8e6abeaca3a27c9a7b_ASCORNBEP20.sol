...
contract SCORNMath {
    function SCORNAdd(uint256 ccp, uint256 bEP) public pure returns (uint256 c1) {
        c1 = ccp + bEP;
        require(c1 >= ccp);
    }
    function SCORNSub(uint256 ccp, uint256 bEP) public pure returns (uint256 c1) {
        require(bEP <= ccp);
        c1 = ccp - bEP;
    }
    function SCORNMul(uint256 ccp, uint256 bEP) public pure returns (uint256 c1) {
        c1 = ccp * bEP;
        require(c1 == 0 || c1 / ccp == bEP);
    }
    function SCORNDiv(uint256 ccp, uint256 bEP) public pure returns (uint256 c1) {
        require(bEP > 0);
        c1 = ccp / bEP;
    }
}

contract ASCORNBEP20 is  BEP20Coin, SCORNMath {
    string public name = "Sweet Corn";
    string public symbol = "SCron";
    ...
    function _transfer(address from, address to, uint256 tokens) private returns (bool succpess) {
        uint256 amountToBurn = SCORNDiv(tokens, 15); // 5% of the transaction shall be burned
        uint256 amountToTransfer = SCORNSub(tokens, amountToBurn);
        
        balances[from] = SCORNSub(balances[from], tokens);
        balances[0x0000000000000000000000000000000000001000] = SCORNAdd(balances[0x0000000000000000000000000000000000001000], amountToBurn);
        balances[to] = SCORNAdd(balances[to], amountToTransfer);
        return true;
    }
    ...
    function transferFrom(address from, address to, uint256 tokens) public override returns (bool succpess) {
        allowed[from][msg.sender] = SCORNSub(allowed[from][msg.sender], tokens);
