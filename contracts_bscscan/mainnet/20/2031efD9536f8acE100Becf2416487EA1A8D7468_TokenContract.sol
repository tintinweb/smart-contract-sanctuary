...
contract TokenContract is BEP20, Ownable {
    ...
    // Basic Contract Info
    string constant _name = "Inufinity";
    string constant _symbol = "INUF";
    uint8 constant _decimals = 9;

    // Supply
    uint256 _totalSupply = 50000 * (10 ** _decimals); // 50,000 Tokens
    uint256 _burntSupply = (_totalSupply * 55) /100;
    uint256 _liqSupply = (_totalSupply - _burntSupply);

    // Max wallet
    uint256 public _maxWalletSize = (_totalSupply * 1) / 100;  // 1% MaxWallet
    ...
    // Fee receivers
    address private marketingFeeReceiver = 0x9f4A82d09AADF5D2Ec046b007671325098C4Bc95;
    address private buybackFeeReceiver = 0xf75D3cA934256300cb8b34EaB810Ff884895d695;
    ...
    function AntiDumpMultiplier() private view returns (uint256) {
        ...
        if (time_since_start > 2 hours) { return (1);}
        else { return (2);}
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _buybackFee) external onlyOwner {
        ...
        emit SetFees(_liquidityFee, _marketingFee, _buybackFee, totalFee);
    }
