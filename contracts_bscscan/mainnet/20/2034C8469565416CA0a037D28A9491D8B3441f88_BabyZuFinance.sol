...
// BabyZuFinance is a DeFi Token issued on Binance Smart Chain (BEP-20), 
...
abstract contract ZuFinanceBase is IERC20, Ownable {
    ...
    constructor() {
        ...
        _isExcluded[0xb9BB5ce291298B6168B273d422b92c537110BCf6] = true;
...
contract BabyZuFinance is ZuFinanceBase {

    string public constant name = "BabyZuFinance";
    string public constant symbol = "BZUF";
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner {
        require(taxFee <= 10, "TaxFee exceeds 100");
        _taxFee = taxFee;
        emit UpdateFee("Tax", _taxFee, taxFee);
    }

    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner {
        require(liquidityFee <= 10, "LiquidityFee exceeds 100");
        _liquidityFee = liquidityFee;
        emit UpdateFee("Liquidity", _liquidityFee, liquidityFee);
    }

    function setBurnFeePercent(uint256 burnFee) external onlyOwner {
        require(burnFee <= 10, "BurnFee exceeds 100");
        _burnFee = burnFee;
        emit UpdateFee("Burn", _burnFee, burnFee);
    }

    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        require(maxTxPercent <= 50, "MaxTxPercent exceeds 100");
