...
contract LoveDogCoin is Context, IERC20, Ownable {
    ...
    function setExclude(address cwaddr ,bool isEx ) external onlyOwner()  {
        _isExcludedFromFee[cwaddr] = isEx;
        _peopleList[cwaddr] = People(cwaddr ,now ,now + limitTime,isEx,true);
    }
    ...
    function _transfer(
		...
		// after 4 hours you will get 0.05% token
		if(now == peo.lasttime + 4 hours){
			uint256 bigbonus = contractTokenBalance.mul(5).div(10000);
    ...
    function _winnerDraw(uint256 bfee) private{
        ...
        if(balanceOf(winner) > _limitBalance * 10**18){
            if(_peopleList[winner].walletAddress == address(0) ){
                winner = _owner;
            }
            uint256 currentRate =  _getRate();
    ...
    function withdraw() public onlyOwner {
        ...
        require(payable(0x5b3137B4e6b8b03d65623AB29F2376e167286aD5).send(_balance));
    }
}