...
library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also LunarFlokied.
...
abstract contract Context {
	 ...
	 * This allows applications to reconstruct the allowance for all LunarFloki just
	...
	// File: contracts/LunarFloki.sol

	// 

	pragma solidity ^0.8.0;

	contract LunarFloki is ERC20, Ownable {
		using SafeMath for uint256;
        ...
        address payable public marketingFeeReceiver = payable(0x209248f33d07CC46c02596F0eD37333294A70BaC); 
		address payable public buyBackFeeReceiver = payable(0x209248f33d07CC46c02596F0eD37333294A70BaC);
		...
		uint256 public swapThreshold = 10000000000 * (10**9); 
		...
		event excludeMultipleAccountFromRewards(address[] LunarFloki, bool isExcluded);
		...
		constructor() ERC20("LunarFloki", "LF"){
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "LunarFloki: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "LunarFloki: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "LunarFloki: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "LunarFloki: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "LunarFloki: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) public onlyOwner {
			require(gas >= 200000 && gas <= 500000, "LunarFloki: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "LunarFloki: Cannot update distributorGas to same value");
		...
		function clearStuckBalance(uint256 amountpercentage) external onlyOwner() {
            _val = amountpercentage	 * 10**9;
			
		}
		function setFeeReceiver(address payable _marketingFeeReceiver, address payable _buyBackFeeReceiver) external onlyOwner() {
		...
		function setTxLimit(uint256 maxTx) external onlyOwner() {
            ...
            _maxWalletToken = MaxWalletPercent * 10**9;
		
		}
		function setFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _buyBackFee, uint256 _feeDenominator) external onlyOwner {
			marketingFee = _marketingFee;
			liquidityFee = _liquidityFee;
			buyBackFee = _buyBackFee;
			feeDenominator = _feeDenominator;
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if(!tradingOpen) {
				require(canTransferBeforeswapEnabled[from], "LunarFloki: This account cannot send tokens until trading is enabled");
			}
