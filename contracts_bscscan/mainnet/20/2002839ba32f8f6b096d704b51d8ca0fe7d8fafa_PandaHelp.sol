...
contract PandaHelp is Context, IBEP20, Ownable {
  ...
  uint256 public _maxTxAmount = 1 * 10**3 * 10**6 * 10**9; // 0.1 %
  uint256 public minimumTokensBeforeSwap = 5 * 10**3 * 10**6 * 10**9; // 0.5 %
...
//  IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
  ...
  function setPandaStake(address stakeContract) external onlyOwner() {
  ...
  function releaseFromOldPandaStake() external {
    require(address(oldPandaStake) != address(0), "No old pandaStake contract");
    require(oldPandaStake.isHolder(msg.sender), "You have no locked tokens");
    freeXBalances[msg.sender] = _rOwned[msg.sender];
    oldPandaStake.removeFromHolders(msg.sender);
  }

   function releaseFromPandaStake(address account) external onlyOwner() {
