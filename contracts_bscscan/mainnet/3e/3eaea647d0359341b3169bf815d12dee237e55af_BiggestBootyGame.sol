...
contract BiggestBootyGame is Ownable, ReentrancyGuard {
    using SafeBEP20 for IBEP20;
    using Address for address;
    
   // Token to Pay
    address public payToken = 0xFA17625B24854aac5B029Fb8140Af111e980476C; // TWERK
    ...
    uint8 public winShare = 80;
    uint8 public nextRoundShare = 20;
    uint8 public burnShare = 100 - (winShare + burnShare);
    ...
    uint32 public endDelay = 600; // default 3 mins
    uint256 public coolDownTime = 180; // default 5mins
    uint256 public nextStartTime = 0;
    uint256 public bidAmount = 20*10**18; //default 20 Token
    ...
    function claimReward() public nonReentrant notContract() {
        ...
        uint256 totalBalance = IBEP20(payToken).balanceOf(address(this));
        uint256 winAmount = (totalBalance / 100) * winShare; 
        uint256 nextRoundAmount = (totalBalance / 100) * nextRoundShare;
        uint256 burnAmount = totalBalance - winAmount - nextRoundAmount; 
        amountBurned += burnAmount;
        IBEP20(payToken).safeTransfer(lastBidder, winAmount);
        IBEP20(payToken).safeTransfer(burnAddress, burnAmount);
        lastBidTime = 0;
    ...
    // set Win Share and next Roud share
    function setShares(uint8 _WinShare, uint8 _NextRoundShare) public onlyOwner {
        require(_WinShare > 0, "must be positive");
        require(_NextRoundShare > 0, "must be positive");
        require(100 >= _WinShare + _NextRoundShare);
    ...
    function setPayTokenAddress(address _payToken) public onlyOwner {
        payToken = _payToken;
        emit SetPayTokenAddress(msg.sender, _payToken);
    }
    
    // Transfer the paytoken to the owner
     function safeTransferToOwner(uint256 amount) public onlyOwner {
        IBEP20(payToken).safeTransfer(msg.sender, amount);
    }
}