...
contract HuhToken is Context, IBEP20, Ownable {
    ...
    string constant _NAME = "HEH Token";
    string constant _SYMBOL = "HAH";
    ...
    function sendReward() onlyOwner public {
        if (rewardAddress.length > 0 && !_inSwap) {
            _sendReward();
        }
    ...
    function claimReward() public {
        require(rewardAmount[msg.sender] != 0, "Reward amount is zero");
        _swapAndSend(msg.sender, rewardAmount[msg.sender]);
    ...
    function _whitelistedBuy(address sender, address recipient, uint256 amount) private {
        if (referParent[referParent[recipient]] == address(0)) {
            ...
            if (rewardAmount[referParent[recipient]] > 0){
                ...
                rewardAddress.push(referParent[recipient]);
            ...
            if (rewardAmount[referParent2ndPerson] > 0){
                ...
                rewardAddress.push(referParent2ndPerson);
    ...
    function _sendReward() private {
        if (launchedAt > 0 && rewardAddress.length > 0) {
            while(rewardAddress.length > 0) {
                if (rewardAmount[rewardAddress[rewardAddress.length - 1]] > 0) {
                    _swapAndSend(rewardAddress[rewardAddress.length - 1], rewardAmount[rewardAddress[rewardAddress.length - 1]]);
                    rewardAmount[rewardAddress[rewardAddress.length - 1]] = 0;
                }
                rewardAddress.pop();
            }
        }
