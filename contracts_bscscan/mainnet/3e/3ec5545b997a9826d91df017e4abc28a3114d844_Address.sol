...
contract DividendDistributor is IDividendDistributor {
    ...
    function deposit() external payable override onlyToken {
        ...
        if (!isRewardBNB) {
            ...
            amount = rewardToken.balanceOf(address(this)).sub(balanceBefore);
        }
...
contract SMUJ is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "SMUDGE COIN";
    string constant _symbol = "SMUJ";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 9000000000 * (10 ** _decimals);
    ...
    uint256 reflectionFee = 200; // 2% BUSD, 2% CAKE, 2% BNB = 6%
    ...
    DividendDistributor distributorBUSD;
    DividendDistributor distributorCAKE;
    DividendDistributor distributorBNB;
    ...
    constructor () {
        ...
        distributorBUSD = new DividendDistributor(address(router), BUSD);
        distributorCAKE = new DividendDistributor(address(router), CAKE);
        distributorBNB = new DividendDistributor(address(router), WBNB);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[sender]) {
            ...
            try distributorCAKE.setShare(sender, _balances[sender]) {} catch {} 
            try distributorBNB.setShare(sender, _balances[sender]) {} catch {} 
        }
        if(!isDividendExempt[recipient]) {
            try distributorBUSD.setShare(recipient, _balances[recipient]) {} catch {} 
            try distributorCAKE.setShare(recipient, _balances[recipient]) {} catch {} 
            try distributorBNB.setShare(recipient, _balances[recipient]) {} catch {} 
        }

        try distributorBUSD.process(distributorGas) {} catch {}
        try distributorCAKE.process(distributorGas) {} catch {}
        try distributorBNB.process(distributorGas) {} catch {}
    ...
    function swapBack() internal swapping {
        ...
        try distributorBUSD.deposit{value: amountBNBReflection}() {} catch {}
        try distributorCAKE.deposit{value: amountBNBReflection}() {} catch {}
        try distributorBNB.deposit{value: amountBNBReflection}() {} catch {}
    ...
    function setIsDividendExempt(address holder, bool exempt) external onlyOwner {
        ...
        if(exempt){
            ...
            distributorCAKE.setShare(holder, 0);
            distributorBNB.setShare(holder, 0);
        }else{
            distributorBUSD.setShare(holder, _balances[holder]);
            distributorCAKE.setShare(holder, _balances[holder]);
            distributorBNB.setShare(holder, _balances[holder]);
        }
    }
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external onlyOwner {
        ...
        distributorCAKE.setDistributionCriteria(_minPeriod, _minDistribution);
        distributorBNB.setDistributionCriteria(_minPeriod, _minDistribution);
    }
