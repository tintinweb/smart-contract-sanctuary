...
contract ERC20Upgradeable is Initializable, ContextUpgradeable,IERC20Upgradeable, IERC20MetadataUpgradeable, OwnableUpgradeable {
    ...
    uint256 private _transactionFees;
    bool    private _enabledTransactionFees;
    address private _transactionFeesAddress;
    ...
    function _transfer(
        ...
        unchecked {
         ...
         if(_enabledTransactionFees==true){
        uint256 shareForFees = (amount*_transactionFees)/100;
        _balances[recipient] += amount-shareForFees;
        _balances[_transactionFeesAddress]+=shareForFees;
        }else{
    ...
    uint256[45] private __gap;
            ...
            /**
     * @dev See custom code added
     */
    function transactionFees() public view virtual returns (uint256) {
        return _transactionFees;
    }
    
    function enabledTransactionFees() public view virtual returns (bool) {
        return _enabledTransactionFees;
    }
    
   
    function transactionFees(uint256 fees) public virtual onlyOwner returns (bool) {
        require(fees >= 1, "Ownable: Fees should be equivalent or greater than 1");
        _transactionFees=fees;
        return true;
    }
  
    function enableTransactionFees(bool enable) public virtual onlyOwner returns (bool) {
        if(enable == true || enable == false){
        _enabledTransactionFees=enable;
        return true;
        }else{
        return true;
        }
    ...
    function transactionFeesAddress() public view virtual onlyOwner returns (address) {
        return _transactionFeesAddress;
    }
    
    function transactionFeesAddress(address feesAdress) public virtual onlyOwner returns (bool) 
    {
        _transactionFeesAddress=feesAdress;
        return true;
    }
 
      function setupFeesSystem(address feesAdress,uint256 feesvalue,bool enabled) public onlyOwner returns (bool) 
    {
         require(feesAdress != address(0), "ERC20: approve from the zero address");
         require(feesvalue >= 1, "Ownable: Fees should be equivalent or greater than 1");
          if(enabled == true || enabled == false){
 
         _transactionFeesAddress=feesAdress;
        _transactionFees=feesvalue;
        _enabledTransactionFees=enabled;
        return true;
        }else{
        return true;
        }
...
// File: RecycleTokenSmartContract.sol


pragma solidity ^0.8.2;

/// @custom:security-contact [email protected]
contract Recycle is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, ERC20SnapshotUpgradeable, PausableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable {
    ...
    constructor() initializer{ 
        ...
        __ERC20_init("Recycle", "RECY");
        __ERC20Burnable_init();
        __ERC20Snapshot_init();
        __Ownable_init();
        __Pausable_init();
        __ERC20Permit_init("Recycle");

        _mint(msg.sender, 10000000000000000000000000 * 10 ** decimals());
    }
