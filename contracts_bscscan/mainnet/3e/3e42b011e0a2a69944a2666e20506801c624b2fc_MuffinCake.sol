...
contract MuffinCake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("MuffinCake", "MuffinCake") {
    ...
    //-
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "MuffinCake: The marketing wallet is already this address");
    ...
    function updateFirstDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(firstDividendTracker), "MuffinCake: The dividend tracker already has that address");
 
        FIRSTDividendTracker newFirstDividendTracker = FIRSTDividendTracker(payable(newAddress));
 
        require(newFirstDividendTracker.owner() == address(this), "MuffinCake: The new dividend tracker must be owned by the MuffinCake token contract");
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "MuffinCake: The dividend tracker already has that address");
 
        CAKEDividendTracker newCakeDividendTracker = CAKEDividendTracker(payable(newAddress));
 
        require(newCakeDividendTracker.owner() == address(this), "MuffinCake: The new dividend tracker must be owned by the MuffinCake token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "MuffinCake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "MuffinCake: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MuffinCake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "MuffinCake: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "MuffinCake: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "MuffinCake: Trading has not started yet");
...
contract FIRSTDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("MuffinCake_ANI_Dividend_Tracker", "MuffinCake_ANI_Dividend_Tracker", 0xaC472D0EED2B8a2f57a6E304eA7eBD8E88D1d36f) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 0.02% tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "MuffinCake_First_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "MuffinCake_First_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MuffinCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "MuffinCake_First_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MuffinCake_First_Dividend_Tracker: Cannot update claimWait to same value");
...
contract CAKEDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("MuffinCake_Cake_Dividend_Tracker", "MuffinCake_Cake_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "MuffinCake_Cake_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "MuffinCake_Cake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MuffinCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "MuffinCake_Cake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MuffinCake_Cake_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
