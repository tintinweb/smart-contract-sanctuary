...
/**
 * @title BASED price Oracle
 *      This Oracle calculates the average USD price of BASED based on the BNB-BASED and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private BASEDBnbPrice0CumulativeLast;
    uint private BASEDBnbPrice1CumulativeLast;
    uint32 private BASEDBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _BASED_bnb;
    ...
    constructor(
        address __BASED_bnb,   // Address of the BASED-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _BASED_bnb = IUniswapV2Pair(__BASED_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        BASEDBnbPrice0CumulativeLast = _BASED_bnb.price0CumulativeLast();
        BASEDBnbPrice1CumulativeLast = _BASED_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, BASEDBnbBlockTimestampLast) = _BASED_bnb.getReserves();
    ...
    // Get the average price of 1 BASED in the smallest BNB unit (18 decimals)
    function getBASEDBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_BASED_bnb));

        require(_blockTimestamp != BASEDBnbBlockTimestampLast, "BASED Last and current are equal");

        FixedPoint.uq112x112 memory BASEDBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - BASEDBnbPrice0CumulativeLast) / (_blockTimestamp - BASEDBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, BASEDBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        uint BASEDBnbAverage;
        uint busdBnbAverage;

        (BASEDBnbPrice0CumulativeLast, BASEDBnbPrice1CumulativeLast, BASEDBnbBlockTimestampLast, BASEDBnbAverage) = getBASEDBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, BASEDBnbAverage) = getBASEDBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = BASEDBnbAverage*(1e6) / busdBnbAverage;
