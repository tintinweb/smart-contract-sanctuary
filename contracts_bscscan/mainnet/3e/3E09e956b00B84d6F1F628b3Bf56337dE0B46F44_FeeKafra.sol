...
contract FeeKafra is Ownable, IFeeKafra {
    ...
    uint256 public override depositFee = 0; // 2 %
    uint256 public override treasuryFeeDeposit = 5000; // 50% of total fee
    uint256 public override kswFeeDeposit = 5000; // 50% of total fee
    ...
    function calculateDepositFee(uint256 _wantAmount, address) public view override returns (uint256) {
        return (_wantAmount * depositFee) / MAX_FEE;
    }

    function calculateWithdrawFee(uint256 _wantAmount, address) public view override returns (uint256) {
        return (_wantAmount * withdrawFee) / MAX_FEE;
    }

    function setDepositFee(uint256 _fee) external onlyOwner {
        require(_fee <= MAX_DEPOSIT_FEE, "!cap");
    ...
    function setTreasuryFeeDeposit(uint256 _fee) external onlyOwner {
        treasuryFeeDeposit = _fee;
    }

    function setKSWFeeDeposit(uint256 _fee) external onlyOwner {
        kswFeeDeposit = _fee;
    }
    ...
    function distributeDepositFee(IERC20 _token, address) external override {
        uint256 feeAmount = _token.balanceOf(address(this));

        uint256 treasuryFeeAmount = (feeAmount * treasuryFeeDeposit) / MAX_FEE;
        if (treasuryFeeAmount > 0) {
            _token.safeTransfer(treasuryFeeRecipient, treasuryFeeAmount);
        }

        uint256 kswFeeAmount = (feeAmount * kswFeeDeposit) / MAX_FEE;
        if (kswFeeAmount > 0) {
    ...
    function distributeWithdrawFee(IERC20 _token, address) external override {
        ...
        uint256 treasuryFeeAmount = (feeAmount * treasuryFeeWithdraw) / MAX_FEE;
        if (treasuryFeeAmount > 0) {
            _token.safeTransfer(treasuryFeeRecipient, treasuryFeeAmount);
        }

        uint256 kswFeeAmount = (feeAmount * kswFeeWithdraw) / MAX_FEE;
        if (kswFeeAmount > 0) {
            _token.safeTransfer(kswFeeRecipient, kswFeeAmount);
        }
    }
