...
contract testiny is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    testinyDividendTracker public dividendTracker;
    ...
    address payable public marketingWallet = 0xCa23C602BeED1f1B790611B691d652Bf0Abec816;
    ...
    constructor() ERC20("testiny", "TEZTY") {
        
        totalFees = BNBRewardsFee.add(liquidityFee);


    	dividendTracker = new testinyDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "testiny: The router already has that address");
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "testiny: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "testiny: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "testiny: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 1000000, "testiny: gasForProcessing must be between 200,000 and 1,000,000");
        require(newValue != gasForProcessing, "testiny: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "testiny: This account cannot send tokens until trading is enabled");
        }
...
contract testinyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("testiny_Dividend_Tracker", "testiny_Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "testiny_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "testiny_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main babyMusk contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 900 && newClaimWait <= 86400, "testiny_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "testiny_Dividend_Tracker: Cannot update claimWait to same value");
