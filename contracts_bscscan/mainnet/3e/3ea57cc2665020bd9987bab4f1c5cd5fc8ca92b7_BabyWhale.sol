...
TG: https://t.me/BabyWhale_Official
website: https://www.babywhale.live/

BabyWhale is a token that rewards holders with BUSD rewards and it gives holders a GUARANTEED chance to receive BabyWhale NFT every 48-72hours depending on remaining supply and number of holders. 
These NFTs will be listed and will be tradeable on Lootex [and opensea]

In order to be eligible for the randomised NFT airdrop, holders have to hold for 24-72hrs upon purchase till time of airdrop. Snapshot of holdings will be taken 1hr before airdrop. Hence, if you sell/jeet/dump, you will not be eligible for the airdrop. 

Each NFTs will have their own unique attributes which gives them a rarity ranking. 
*/
...
// File: contracts/BabyWhaleDividendPayingToken.sol
...
contract BabyWhale is ERC20, Ownable {
    ...
    BabyWhaleDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("BabyWhale", "BabyWhale") {
    	...
    	dividendTracker = new BabyWhaleDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyWhale: The dividend tracker already has that address");

        BabyWhaleDividendTracker newDividendTracker = BabyWhaleDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyWhale: The new dividend tracker must be owned by the BabyWhale token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyWhale: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyWhale: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyWhale: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyWhale: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyWhale: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyWhale: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyWhale: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BabyWhale: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of BabyWhale
            path,
            recipient,
            block.timestamp
        );
...
contract BabyWhaleDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BabyWhale_Dividend_Tracker", "BabyWhale_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "BabyWhale_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BabyWhale_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyWhale contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyWhale_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyWhale_Dividend_Tracker: Cannot update claimWait to same value");
