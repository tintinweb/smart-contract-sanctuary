...
/* ==========  Events  ========== */
  ...
  event TokenAdded(address token, address factory, address router, uint256 categoryID);
...
/* ==========  Storage  ========== */
  ...
  // Array of factories for each category.
  mapping(uint256 => address[]) internal _categoryFactories;
...
/* ==========  Category Management  ========== */
  ...
  function updateCategoryPrices(
    ...
    address[] memory factories = _categoryFactories[categoryID];
    IBisharesUniswapV2Oracle oracle = _categoryOracles[categoryID];
    pricesUpdated = oracle.updatePrices(factories, tokens);
  }
   ...
   */
  function createCategory(bytes32 metadataHash, IBisharesUniswapV2Oracle oracle) external onlyOwner {
  ...
  ) external onlyOwner validCategory(categoryID) {
    require(
      _categoryTokens[categoryID].length < MAX_CATEGORY_TOKENS,
      "ERR_MAX_CATEGORY_TOKENS"
    );
    _addToken(categoryID, factory, router, token, weight);
    IBisharesUniswapV2Oracle oracle = _categoryOracles[categoryID]; 
    oracle.updatePrice(factory, token);
  ...
  function addTokens(
    ...
    require(factories.length == len, "ERR_FACTORIES_LEN");
    require(routers.length == len, "ERR_ROUTERS_LEN");
    require(weights.length == len, "ERR_WEIGHTS_LEN");
    for (uint256 i = 0; i < len; i++) {
      _addToken(categoryID, factories[i], routers[i], tokens[i], weights[i]);
    }
   
    _categoryOracles[categoryID].updatePrices(factories, tokens);
    // Decrement the timestamp for the last category update to ensure
    ...
    for (; i < len; i++) {
      if (_categoryTokens[categoryID][i] == token) {
        ...
        if (i != last) {
          ...
          address lastFactory = _categoryFactories[categoryID][last];
          address lastRouter = _categoryRouters[categoryID][last];
          _categoryTokens[categoryID][i] = lastToken;
          _categoryFactories[categoryID][i] = lastFactory;
          _categoryRouters[categoryID][i] = lastRouter;
        }
        _lastCategoryUpdate[categoryID] -= MAX_SORT_DELAY;
        _categoryTokens[categoryID].pop();
        _categoryFactories[categoryID].pop();
        _categoryRouters[categoryID].pop();
  ...
  function orderCategoryTokensByMarketCap(uint256 categoryID) external validCategory(categoryID) {
    ...
    address[] storage categoryFactories = _categoryFactories[categoryID];
    address[] storage categoryRouters = _categoryRouters[categoryID];
    IBisharesUniswapV2Oracle oracle = _categoryOracles[categoryID];
    uint256 len = categoryTokens.length;
    uint144[] memory marketCaps = computeAverageMarketCaps(categoryFactories, categoryTokens, oracle);
    for (uint256 i = 1; i < len; i++) {
      uint144 cap = marketCaps[i];
      address token = categoryTokens[i];
      address factory = categoryFactories[i];
      address router = categoryRouters[i];
      uint256 j = i - 1;
      while (int(j) >= 0 && marketCaps[j] < cap) {
        marketCaps[j + 1] = marketCaps[j];
        categoryTokens[j + 1] = categoryTokens[j];
        categoryFactories[j + 1] = categoryFactories[j];
        categoryRouters[j + 1] = categoryRouters[j];
        j--;
      }
      marketCaps[j + 1] = cap;
      categoryTokens[j + 1] = token;
      categoryFactories[j + 1] = factory;
      categoryRouters[j + 1] = router;
...
/* ==========  Market Cap Queries  ========== */
  ...
  function computeAverageMarketCap(
    ...
    IERC20 lpToken = vault.token();
    uint256 vaultPrice = vault.getPricePerFullShare();
    uint256 vaultSupply = lpToken.totalSupply() * 1e18 / vaultPrice;
  ...
  function computeAverageMarketCaps(
    ...
    for (uint256 i = 0; i < len; i++) {
      IVault vault = IVault(tokens[i]);
      IERC20 lpToken = vault.token();
      uint256 vaultPrice = vault.getPricePerFullShare();
      totalSupplies[i] = lpToken.totalSupply() * 1e18 / vaultPrice;
    }
...
/* ==========  Category Queries  ========== */
   ...
   */
  function getLastCategoryUpdate(
    uint256 categoryID
  ) external view validCategory(categoryID) returns (uint256) {
  ...
  function getCategoryTokens(
    uint256 categoryID
  ) external view validCategory(categoryID) returns (
    address[] memory tokens,
    address[] memory factories,
    address[] memory routers
  ) {
    ...
    for (uint256 i = 0; i < len; i++) {
      tokens[i] = categoryTokens[i];
      factories[i] = categoryFactories[i];
      routers[i] = categoryRouters[i];
    }
  }
  ...
  function getCategoryMarketCaps(
    uint256 categoryID
  ) external view validCategory(categoryID) returns (uint144[] memory marketCaps) {
    return computeAverageMarketCaps(
      _categoryFactories[categoryID],
      _categoryTokens[categoryID],
      _categoryOracles[categoryID]
    );
  }
  ...
  function getTopCategoryTokens(
    uint256 categoryID,
    uint256 num
  ) public view validCategory(categoryID) returns (
    address[] memory tokens,
    address[] memory factories,
    address[] memory routers
  ) {
    ...
    factories = new address[](num);
    routers = new address[](num);
...
/* ==========  Category Utility Functions  ========== */
  ...
  function _addToken(
    ...
    _categoryFactories[categoryID].push(factory);
    _categoryRouters[categoryID].push(router);
    _categoryTokens[categoryID].push(token);
    _categoryTokensInitialWeights[categoryID][token] = weight;
    emit TokenAdded(token, factory, router, categoryID);
  }
}
...
/* ==========  Pool Deployment  ========== */
    ...
    returns (address poolAddress, address initializerAddress)
  {
    ...
    initializer.initialize(poolAddress, factories, tokens, balances, oracle);
...
/* ==========  Pool Rebalance Actions  ========== */
  ...
  function reindexPool(address poolAddress) external {
    ...
    (address[] memory tokens, address[] memory factories,) = getTopCategoryTokens(categoryID, size);
...
/* ==========  Pool Queries  ========== */
  ...
  {
    (address[] memory _tokens, address[] memory _factories,) = getTopCategoryTokens(categoryID, indexSize);
    ...
    factories = new address[](indexSize);
    tokens = new address[](indexSize);
...
/* ==========  Internal Pool Utility Functions  ========== */
  ...
  function _estimatePoolValue(IIndexPool pool) internal view returns (uint144) {
    ...
    (address factory, address token, uint256 value) = pool.extrapolatePoolValueFromToken();
...
/* ========== Mutative ========== */
  ...
  function claimTokens(address[] memory accounts) external;
...
/* ========== Views ========== */
  ...
  function getDesiredFactories() external view returns (address[] memory);
  ...
  function getCreditForTokens(address factory, address token, uint256 amountIn) external view returns (uint144);
}
...
/* ========= View Functions ========= */
  ...
  function _calculatePriceCumulativeLast(
    ...
    uint256 rate = rateNominator.mul(1e18).div(rateDenominator); // x 1e18
