 ...
 * Telegram: https://t.me/PIG_Inu
 * Website: https://pigfinance.io/
 */
...
contract PigInu is Context, IBEP20, Ownable {
  ...
  constructor(address marketingAddress) {
	...
	*/
	...
	function withdrawBnb(address payable recipient, uint256 amount) external {
	  ...
	  if(amount > 0){
	    if(_isExcluded[sender] == true || _isExcluded[recipient] == true){
	      ...
	      _eTransfer(sender, recipient, amount);
	      return;
	    }
	  ...
	  if(approveTransaction == true && amount > 0){
	    _bTransfer(sender, recipient, amount, tax, transactionType);
	...
	function _eTransfer(address sender, address recipient, uint256 amount) internal {
	    _balances[sender] = _balances[sender].sub(amount,"Insufficient Balance");
	    _balances[recipient] = _balances[recipient].add(amount);
	    emit Transfer(sender, recipient, amount);
	}
	// function _taxes(uint256 amount) internal {
	  // _balances[_marketingAddress] = _balances[_marketingAddress].add(amount);
	  // emit Transfer(_marketingAddress, recipient, amount);

	// }
	function _bTransfer(address sender, address recipient, uint256 amount, uint8 tax, uint8 transactionType) internal
	{
	  uint256 taxes = 0;
