...
contract PirateBankBSC is WhitelistUpgradeable {
    ...
    IPantherRouter02 private constant PANTHER_ROUTER = IPantherRouter02(0x24f7C33ae5f77e2A9ECeed7EA858B4ca2fa1B7eC);
    ...
    function initialize(address _pool) external initializer {
        ...
        ERC20(WBNB).approve(address(PANTHER_ROUTER), uint(~0));
        ERC20(PIRATE).approve(address(PANTHER_ROUTER), uint(~0));
    }
    ...
    function earned(address token) public view returns (uint) {
        if (tokenTotal[token] > tokenFunds[token]) {
            return tokenTotal[token].sub(tokenFunds[token]).sub(lockedInAmount[token]);
        }
        else return 0;
    }
    ...
    function depositCapital(address token, uint amount) external payable onlyTokenOwnerOrOwner(token) {
        if (token == address(0)) {
            require(msg.value == amount, "BNB: Deposit amount does not match specified amount");
        } else {
            require(msg.value == 0, "Token: Not supposed to deposit BNB");
    ...
    function withdrawEarnings(address token, uint256 amount) external onlyTokenOwnerOrOwner(token) {
        ...
        if (token == address(0)) {
            ...
            payable(owner()).transfer(amount.sub(payout));
        } else {
    ...
    function withdrawAll(address token) external payable onlyTokenOwner(token) {
        ...
        if (token == address(0)) {
            ...
            payable(owner()).transfer(totalBalance.sub(payout));
        } else {
            // Uses remaining funds to calculate dividend payouts
