...
contract Staking is Ownable {
    ...
    constructor(uint256 _startBlock, uint256 _totalTokens, IERC20 _depositToken, IERC20 _rewardToken) {
        ...
        endBlock = startBlock + 20*60*24*7; // 1 year is approx. 10512000 blocks
        totalTokens = _totalTokens;
        tokensPerBlock = totalTokens*precisionLevel / (20*60*24*365); // precision level??
        depositToken = _depositToken;
        rewardToken = _rewardToken;
        lastDepositBlock = endBlock - 20*60; // 1 months and 50 days
    }    
    ...
    function deposit(uint256 _amount) public {
        ...
        require(block.number <= lastDepositBlock, "Error: Staking has finished");
        ...
        stakingWallets[msg.sender].unlockBlock = block.number + 20*10; // 10 mins
    ...
    function getClaimCycle(address _address) internal view returns(uint256) {
        if (block.number > stakingWallets[_address].unlockBlock) {
            uint256 claimCycle = (block.number - stakingWallets[_address].unlockBlock) / 100;
            if (claimCycle == 0) {
                return 1;
            } else {
                return claimCycle;
