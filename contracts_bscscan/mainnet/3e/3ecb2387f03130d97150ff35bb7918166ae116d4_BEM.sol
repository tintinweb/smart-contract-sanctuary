...
contract BEM is BEP20 {
    ...
    address public constant BURN_ADDRESS =
        0x8BCC19A571137B718AF51C1De005901E6397816C;
    ...
    // Min amount to liquify. (default 500 BEM)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to BEM's router when our own AMM release
    IUniswapV2Router02 public BEMSwapRouter;
    
    // The trading pair
    address public BEMSwapPair;
    //busdAddress
        ...
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(
                    ...
                    "BEM::antiWhale: Transfer amount exceeds the maxTransferAmount"
                );
            }
        }
        _;
    ...
    /**
     * @notice Constructs the BEM contract.
     */
    constructor() public BEP20("The BEMIL COIN", "BEM") {
        ...
        mint(_msgSender(),1e9*1e18);
    }

    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (BEMMaster).
    function mint(address _to, uint256 _amount) public onlyMinter {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true &&
            _inSwapAndLiquify == false &&
            address(BEMSwapRouter) != address(0) &&
            BEMSwapPair != address(0)
        ) {
            swapAndLiquify();
        }
        _mint(_to, _amount);
    ...
    /// @dev overrides transfer function to meet tokenomics of BEM
    function _transfer(
        address sender,
        address recipient,
        uint256 amount
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(
                ...
                "BEM::transfer: Burn value invalid"
            );
            ...
            require(
                ...
                "BEM::transfer: Tax value invalid"
            );
    ...
    function swapAndLiquify() private lockTheSwap transferTaxFree {
        ...
        if(currentPrice.mul(10000) > expectedPrice.mul(alowanceDiff.add(10000))) {
           uint256 amountToSwap =  ((currentPrice.mul(1e8).div(expectedPrice))).sqrt().sub(1e4).mul(balanceOf(BEMSwapPair)).div(1e4); // 160 000 000  => 12 649 .sub  2649 * price
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the BEMSWAP pair path of token -> WBNB
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = busdAddress;

        _approve(address(this), address(BEMSwapRouter), tokenAmount);
        // make the swap
        BEMSwapRouter.swapExactTokensForTokens(
            tokenAmount,
    ...
    function getWexPrice() public view returns (uint256) {
        // wexLPAddress
        return IBEP20(busdAddress).balanceOf(BEMSwapPair).mul(1e2).div(balanceOf(BEMSwapPair));
    }
    // /// @dev Add liquidity
    ...
    //     // add the liquidity
    //     BEMSwapRouter.addLiquidityETH{value: ethAmount}(
    //         address(this),
    //         tokenAmount,
    ...
    // To receive BNB from BEMSwapRouter when swapping
        ...
        onlyOperator
    {
        require(
            ...
            "BEM::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate."
        );
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(
            _burnRate <= 100,
            "BEM::updateBurnRate: Burn rate must not exceed the maximum rate."
        );
        ...
        onlyOperator
    {
        require(
            ...
            "BEM::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate."
        );
     ...
     * Can only be called by the current operator.
     */
    function updateBEMSwapRouter(address _router) public onlyOperator {
        BEMSwapRouter = IUniswapV2Router02(_router);
        BEMSwapPair = IUniswapV2Factory(BEMSwapRouter.factory()).getPair(
            address(this),
            BEMSwapRouter.WETH()
        );
        require(
            BEMSwapPair != address(0),
            "BEM::updateBEMSwapRouter: Invalid pair address."
        );
        whiteList[BEMSwapPair] = true;
        emit BEMSwapRouterUpdated(
            msg.sender,
            address(BEMSwapRouter),
            BEMSwapPair
        );
    }
    ...
    function recoverLostTokensExceptOurTokens(address _token, uint256 amount)
        ...
        onlyOperator
    {
        require(_token != address(this), "Cannot recover BEM tokens");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(
            ...
            "BEM::transferOperator: new operator is the zero address"
        );
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "BEM::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "BEM::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "BEM::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "BEM::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying BEM (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "BEM::_writeCheckpoint: block number exceeds 32 bits"
        );
