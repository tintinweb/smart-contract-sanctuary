...
/**
BnbReward
$BREWARD aims to be the greatest reflection coin in existence!

Website: https://bnbreward.net/

Telegram: https://t.me/BNBRewardofficial

18% BNB Automatic Reward
*/
...
contract BnbRewardDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BnbReward_Dividend_Tracker", "BnbReward_Dividend_Tracker", 9) {
    	claimWait = 14400;
        minimumTokenBalanceForDividends = 1000000 * (10**9); //must hold 10000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "BnbReward_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BnbReward_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BnbReward contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BnbReward_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BnbReward_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BnbRewardToken is BEP20, Ownable {
    using SafeMath for uint256;

    IPancakeswapV2Router02 public pancakeswapV2Router;
    address public immutable pancakeswapV2Pair;
    address public marketingWallet = 0xB344DEd68d29367a37eb344C871b8d8B58f5e343;

    bool private swapping;

    BnbRewardDividendTracker public dividendTracker;
    ...
    constructor() public BEP20("BnbReward", "$BREWARD", 9) {
    	...
    	dividendTracker = new BnbRewardDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BnbReward: The dividend tracker already has that address");

        BnbRewardDividendTracker newDividendTracker = BnbRewardDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BnbReward: The new dividend tracker must be owned by the BnbReward token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "BnbReward: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BnbReward: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "BnbReward: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BnbReward: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BnbReward: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BnbReward: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BnbReward: Cannot update gasForProcessing to same value");
