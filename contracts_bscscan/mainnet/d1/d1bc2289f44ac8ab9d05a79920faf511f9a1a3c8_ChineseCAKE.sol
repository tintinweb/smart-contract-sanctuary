...
这是中国蛋糕，我们根据中国鲸鱼的订单制作的硬币。
他们打算在新高地获得这枚硬币。
与我们一起踏上旅程。

为了方便全球投资者，该网站有英文和中文两种语言。

🥞🥞🥞🥞🥞

This is ChineseCAKE a Coin that is the demand of international
Whales.
They take us on this moonride along with them.

For the convenience of global investors, the website is available in English and Chinese.

Community / 社区 : 

Telegram : https://t.me/ChineseCAKEbsc
Twitter : https://twitter.com/ChineseCAKEbnb 
Website : www.ChineseCAKE.one
...
// File: contracts/ChineseCAKEDividendPayingToken.sol
...
// File: contracts/ChineseCAKE.sol
...
contract ChineseCAKE is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   ChineseCAKEDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("ChineseCAKE", "CHAK") {
    	...
    	dividendTracker = new ChineseCAKEDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ChineseCAKE: The dividend tracker already has that address");

        ChineseCAKEDividendTracker newDividendTracker = ChineseCAKEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ChineseCAKE: The new dividend tracker must be owned by the ChineseCAKE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ChineseCAKE: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ChineseCAKE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ChineseCAKE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ChineseCAKE: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ChineseCAKE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ChineseCAKE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ChineseCAKE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "ChineseCAKE: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of ChineseCAKE
            path,
            recipient,
            block.timestamp
        );
...
contract ChineseCAKEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("ChineseCAKE_Dividend_Tracker", "ChineseCAKE_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "ChineseCAKE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "ChineseCAKE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ChineseCAKE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ChineseCAKE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ChineseCAKE_Dividend_Tracker: Cannot update claimWait to same value");
