...
// babyflokiADA @ https://t.me/babyflokiADA
...
// File: contracts/babyflokiADADividendPayingToken.sol
...
// File: contracts/babyflokiADA.sol
...
contract babyflokiADA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable BUSD = address(0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47);
    address private _marketingWallet = address(0xAA8c4A430201322de714FF4AFF71f801445380C5);
    // address private _devWallet = address(0x028b567F883f4a54A763b8426cEFdd32545862D2);

    bool private swapping;
    bool public airdrop;

    babyflokiADADividendTracker public dividendTracker;
    ...
    constructor() public ERC20("babyflokiADA", "babyflokiADA") {
    	...
    	dividendTracker = new babyflokiADADividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "babyflokiADA: The dividend tracker already has that address");

        babyflokiADADividendTracker newDividendTracker = babyflokiADADividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "babyflokiADA: The new dividend tracker must be owned by the babyflokiADA token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "babyflokiADA: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "babyflokiADA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "babyflokiADA: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "babyflokiADA: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "babyflokiADA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "babyflokiADA: Cannot update gasForProcessing to same value");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "babyflokiADABSC: Account is already the value of 'excluded'");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "babyflokiADA: This account cannot send tokens until trading is enabled");
        }
...
contract babyflokiADADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("babyflokiADA_Dividend_Tracker", "babyflokiADA_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "babyflokiADA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "babyflokiADA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main babyflokiADA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "babyflokiADA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "babyflokiADA_Dividend_Tracker: Cannot update claimWait to same value");
