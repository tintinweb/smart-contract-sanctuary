...
Telegaram : https://t.me/blackadatoken
...
contract BlackADA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("BlackADA", "BlackADA") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0x4f823EE231C5CC2F8ca802612d46547B0BAFfe60;
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "BlackADA: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "BlackADA: The new dividend tracker must be owned by the BlackADA token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BlackADA_Dividend_Tracker", "BlackADA_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BlackADA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BlackADA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "BlackADA_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "BlackADA_Dividend_Tracker: Cannot update claimWait to same value");
