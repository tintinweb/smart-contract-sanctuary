...
contract GateManagerMultiRewards is ReentrancyGuardUpgradeable, Ownable {
    ...
    // checks that caller is either owner or potsConverter.
    ...
    function TVL() external view returns (uint256) {
        ...
        if (isMooToken){
            uint256 totalGateManagerBal = IERC20Upgradeable(mooToken).balanceOf( address(this));
            ...
            underlyingAmountGM = totalSupply();
            underlyingAmountYS = totalYieldSourceBal;
        ...
        updateReward(user)
    {
        ...
        if(isMooToken) {
            ...
            balances[msg.sender] = balances[msg.sender].add(balDiff);
            emit Staked(msg.sender, balDiff);
        }
    }
    ...
    function compound() external updateReward(msg.sender)  {
        ...
        depositMoonPot(earnedAmt, referrer);
    }
        ...
        updateReward(address(0))
    {
        ...
        if (isMooToken == false && id == 0) {
        uint256 balance = IERC20Upgradeable(underlying).balanceOf(address(this));
        require(reward <= balance.sub(_totalSupply), "!too many underlying");
        }
