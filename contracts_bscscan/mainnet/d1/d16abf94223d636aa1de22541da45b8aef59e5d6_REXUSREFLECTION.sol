...
contract REXUSREFLECTION is Context, IERC20, Ownable {
    ...
    uint256 private _tfnftTotal;

    string private _name = "REXUSRFTN";
    string private _symbol = "REXUSRFTN";
    ...
    uint256 public _fnftFee = 12;
    uint256 private _previousfnftFee = _fnftFee;
    
    uint256 public _maxTxAmount = 1000 * 10**9 * 10**9;
    uint256 private minimumTokensBeforeSwap = 3 * 10**6 * 10**9; 
    
    address payable public fnftWallet = 0x12D4B29b270010C4F3C73974900c2D96beE8c520; 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _tfnftTotal = _tfnftTotal.add(address(this).balance);
        TransferCharityETH(fnftWallet, address(this).balance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_fnftFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _fnftFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousfnftFee = _fnftFee;
    ...
    function setfnftFeePercent(uint256 fnftFee) external onlyOwner() {
        _fnftFee = fnftFee;
    }
