...
import "../prize-pool/pancakeswap/PancakePrizePoolProxyFactory.sol";
import "../prize-pool/amcstaking/AMCStakingPrizePoolProxyFactory.sol";
...
contract PoolWithMultipleWinnersBuilder {
  ...
  event PancakePrizePoolWithMultipleWinnersCreated(
    PancakePrizePool indexed prizePool,
    MultipleWinners indexed prizeStrategy
  );

  event AMCStakingPrizePoolWithMultipleWinnersCreated(
    AMCStakingPrizePool indexed prizePool,
    MultipleWinners indexed prizeStrategy
  );
  ...
  struct PancakePrizePoolConfig {
    MasterChiefInterface chief;
    uint256 maxExitFeeMantissa;
    uint256 maxTimelockDuration;
  }

  struct AMCStakingPrizePoolConfig {
    AMCStakingInterface chief;
    uint256 maxExitFeeMantissa;
    uint256 maxTimelockDuration;
  }
  ...
  PancakePrizePoolProxyFactory public pancakePrizePoolProxyFactory;
  AMCStakingPrizePoolProxyFactory public amcStakingPrizePoolProxyFactory;
  ...
  constructor (
    ...
    PancakePrizePoolProxyFactory _pancakePrizePoolProxyFactory,
    AMCStakingPrizePoolProxyFactory _amcStakingPrizePoolProxyFactory,
    ...
    require(address(_pancakePrizePoolProxyFactory) != address(0), "GlobalBuilder/pancakePrizePoolProxyFactory-not-zero");
    require(address(_amcStakingPrizePoolProxyFactory) != address(0), "GlobalBuilder/amcStakingPrizePoolProxyFactory-not-zero");
  ...
  function createPancakeMultipleWinners(
    PancakePrizePoolConfig memory prizePoolConfig,
    MultipleWinnersBuilder.MultipleWinnersConfig memory prizeStrategyConfig,
    uint8 decimals
  ) external returns (PancakePrizePool){
    PancakePrizePool prizePool = pancakePrizePoolProxyFactory.create();
    MultipleWinners prizeStrategy = multipleWinnersBuilder.createMultipleWinners(
      prizePool,
      prizeStrategyConfig,
      decimals,
    ...
    prizePool.initialize(
      ...
      prizePoolConfig.maxExitFeeMantissa,
      prizePoolConfig.maxTimelockDuration,
      MasterChiefInterface(prizePoolConfig.chief)
    );
    prizePool.setPrizeStrategy(prizeStrategy);
    prizePool.setCreditPlanOf(
      address(prizeStrategy.ticket()),
    ...
    prizePool.transferOwnership(msg.sender);
    emit PancakePrizePoolWithMultipleWinnersCreated(prizePool, prizeStrategy);
    return prizePool;
  }

  function createAMCStakingMultipleWinners(
    AMCStakingPrizePoolConfig memory prizePoolConfig,
    MultipleWinnersBuilder.MultipleWinnersConfig memory prizeStrategyConfig,
    uint8 decimals
  ) external returns (AMCStakingPrizePool){
    AMCStakingPrizePool prizePool = amcStakingPrizePoolProxyFactory.create();
    MultipleWinners prizeStrategy = multipleWinnersBuilder.createMultipleWinners(
      prizePool,
      prizeStrategyConfig,
      decimals,
    ...
    prizePool.initialize(
      ...
      prizePoolConfig.maxExitFeeMantissa,
      prizePoolConfig.maxTimelockDuration,
      AMCStakingInterface(prizePoolConfig.chief)
    );
    prizePool.setPrizeStrategy(prizeStrategy);
    prizePool.setCreditPlanOf(
      address(prizeStrategy.ticket()),
    ...
    prizePool.transferOwnership(msg.sender);
    emit AMCStakingPrizePoolWithMultipleWinnersCreated(prizePool, prizeStrategy);
    return prizePool;
  }
...
interface AMCStakingInterface {
    function amc() external view returns (address);
...
interface VBNBInterface is IERC20Upgradeable {
...
abstract contract PrizePool is PrizePoolInterface, OwnableUpgradeable, ReentrancyGuardUpgradeable, TokenControllerInterface {
    ...
    onlyControlledToken(controlledToken)
  {
    ...
    if (amount > _currentAwardBalance) {
      _currentAwardBalance = 0;
    }
    else {
...
import "../../external/amcstaking/AMCStakingInterface.sol";
import "../PrizePool.sol";

/// @title Prize Pool with AMS staking
/// @notice Manages depositing and withdrawing assets from the Prize Pool
contract AMCStakingPrizePool is PrizePool {
  using SafeMathUpgradeable for uint256;
  using SafeERC20Upgradeable for IERC20Upgradeable;

  event AMCStakingPrizePoolInitialized(address indexed chief);

  /// @notice Interface for the AMCStaking contract
  AMCStakingInterface public chief;

  /// @notice Initializes the Prize Pool and Yield Service with the required contract connections
  /// @param _controlledTokens Array of addresses for the Ticket and Sponsorship Tokens controlled by the Prize Pool
  /// @param _maxExitFeeMantissa The maximum exit fee size, relative to the withdrawal amount
  /// @param _maxTimelockDuration The maximum length of time the withdraw timelock could be
  /// @param _chief Address of the Masterchief contract
  function initialize (
    RegistryInterface _reserveRegistry,
    ControlledTokenInterface[] memory _controlledTokens,
    uint256 _maxExitFeeMantissa,
    uint256 _maxTimelockDuration,
    AMCStakingInterface _chief
  )
    public
    initializer
  {
    ...
    emit AMCStakingPrizePoolInitialized(address(chief));
  }
  ...
  function _balance() internal override returns (uint256) {
    uint256 amount;
    (amount, ) = chief.userInfo(address(this));
    return chief.pendingAmc(address(this)).add(amount);
  }

  /// @dev Gets the underlying asset token used by the Yield Service
  /// @return A reference to the interface of the underling asset token: should be pancake cake token address
  function _token() internal override view returns (IERC20Upgradeable) {
    // return IERC20Upgradeable(chief.cake());
    return IERC20Upgradeable(chief.amc());
  }
  ...
  function _supply(uint256 amount) internal override {
    ...
    // move pending tokens to deposit
    chief.enterStaking(0);
    uint256 redeemedAmount = assetToken.balanceOf(address(this));
    assetToken.safeApprove(address(chief), redeemedAmount);
    chief.enterStaking(redeemedAmount);
  }
  ...
  function _canAwardExternal(address _externalToken) internal override view returns (bool) {
    return _externalToken != address(chief);
  }
  ...
  function _redeem(uint256 amount) internal override returns (uint256) {
    ...
    // move all pending token to deposit
    chief.leaveStaking(0);
    uint256 redeemedAmount = assetToken.balanceOf(address(this)); 
    if (redeemedAmount > 0) {
      assetToken.safeApprove(address(chief), redeemedAmount);
      chief.enterStaking(redeemedAmount);
...
import "./AMCStakingPrizePool.sol";
import "../../external/openzeppelin/ProxyFactory.sol";

/// @title AMCStaking Prize Pool Proxy Factory
/// @notice Minimal proxy pattern for creating new AMCStaking Prize Pools
contract AMCStakingPrizePoolProxyFactory is ProxyFactory {

  /// @notice Contract template for deploying proxied Prize Pools
  AMCStakingPrizePool public instance;

  /// @notice Initializes the Factory with an instance of the AMCStaking Prize Pool
  constructor () public {
    instance = new AMCStakingPrizePool();
  }

  /// @notice Creates a new AMCStaking Prize Pool as a proxy of the template instance
  /// @return A reference to the new proxied AMCStaking Prize Pool
  function create() external returns (AMCStakingPrizePool) {
    return AMCStakingPrizePool(deployMinimal(address(instance), ""));
  }
}
...
import "../../external/pancakeswap/MasterChiefInterface.sol";
import "../PrizePool.sol";

/// @title Prize Pool with Pancakeswap staking
/// @notice Manages depositing and withdrawing assets from the Prize Pool
contract PancakePrizePool is PrizePool {
  using SafeMathUpgradeable for uint256;
  using SafeERC20Upgradeable for IERC20Upgradeable;

  event PancakePrizePoolInitialized(address indexed chief);

  /// @notice Interface for the Pancakeswap masterchief
  MasterChiefInterface public chief;
    ...
    initializer
  {
    ...
    emit PancakePrizePoolInitialized(address(chief));
  }
  ...
  function _balance() internal override returns (uint256) {
    uint256 amount;
    (amount, ) = chief.userInfo(0, address(this));
    return chief.pendingCake(0, address(this)).add(amount);
  }
...
import "./PancakePrizePool.sol";
import "../../external/openzeppelin/ProxyFactory.sol";

/// @title Pancakeswap Prize Pool Proxy Factory
/// @notice Minimal proxy pattern for creating new Pancakeswap Prize Pools
contract PancakePrizePoolProxyFactory is ProxyFactory {

  /// @notice Contract template for deploying proxied Prize Pools
  PancakePrizePool public instance;

  /// @notice Initializes the Factory with an instance of the Pancakeswap Prize Pool
  constructor () public {
    instance = new PancakePrizePool();
  }

  /// @notice Creates a new Pancakeswap Prize Pool as a proxy of the template instance
  /// @return A reference to the new proxied Pancakeswap Prize Pool
  function create() external returns (PancakePrizePool) {
    return PancakePrizePool(deployMinimal(address(instance), ""));
  }
}
...
import "../../external/venus/VBNBInterface.sol";
import "../PrizePool.sol";
...
contract VenusPrizePool is PrizePool {
  ...
  /// @param _bnbPool Flag for if the current pool is a BNB venus pool
  function initialize (
    RegistryInterface _reserveRegistry,
    ControlledTokenInterface[] memory _controlledTokens,
    uint256 _maxExitFeeMantissa,
  ...
  function _supply(uint256 amount) internal override {
    if (nativePool) {
      VBNBInterface vBNB = VBNBInterface(address(vToken));
  ...
  function _redeem(uint256 amount) internal override returns (uint256) {
    if (nativePool) {
      ...
      return (address(this)).balance.sub(before);
    } else {
      IERC20Upgradeable assetToken = _token();
      uint256 before = assetToken.balanceOf(address(this));
      require(vToken.redeemUnderlying(amount) == 0, "VenusPrizePool/redeem-failed");
      return assetToken.balanceOf(address(this)).sub(before);
    }
  }
...
contract VenusPrizePoolProxyFactory is ProxyFactory {
  ...
  function create() external returns (VenusPrizePool) {
    return VenusPrizePool(payable(deployMinimal(address(instance), "")));
  }
}
