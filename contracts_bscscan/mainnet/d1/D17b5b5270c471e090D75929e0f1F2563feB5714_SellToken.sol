 ...
 * @author Bitcoinnami
 **/
contract SellToken is Initializable {
  ...
  function addPancakepair(address _pair) public onlyIDOAdmin {
    pancakePair = _pair;
  }
  ...
  function receivedAmount(address recipient) external view returns (uint256) {
    ...
    for (uint256 i = 0; i < buyerAmount[recipient].length; i++) {
      if (block.timestamp < buyerAmount[recipient][i].buyDate + lockPeriod){
        lockAmount += buyerAmount[recipient][i].buyAmount;
      }
    }
    return lockAmount;
  }
   ...
   */
  function updateRateRef(uint8 _f1_rate) public onlyIDOAdmin {
    f1_rate = _f1_rate;
  }
   ...
   * @dev Update is enable
   */
  function updateLockPeriod(uint256 _lockPeriod) public onlyIDOAdmin {
  ...
  function IFGRate() public view returns (uint256) {
   ...
   * @param _referrer the ref recipient of the IDO tokens
  ...
  function buyTokenBNB(address recipient, address _referrer) public payable returns (bool) {
    ...
    uint256 sold_amount = usdtAmount * 1e18 / IFGRate();
    buyerAmount[recipient].push(BuyInfo(sold_amount, block.timestamp));
  ...
  function buyIDOUSD(
    ...
    uint256 sold_amount = (buy_amount * 1e18) / IFGRate();
    buyerAmount[recipient].push(BuyInfo(sold_amount, block.timestamp));
    IERC20(IDO_TOKEN).transfer(recipient, sold_amount);
