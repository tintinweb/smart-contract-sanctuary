...
contract Executor is ReentrancyGuard, Ownable {
    ...
    function setDao(address _distributor)
        public
        onlyOwner
    {
    ...
    function execDistribution(uint256 _amount, address _daoAddress)
        external
        nonReentrant
        onlyOwner
    {
        IDaoModule dao = IDaoModule(_daoAddress);

        dao.executePermitted(
                distributor,
                abi.encodeWithSignature("zapIn(uint256)", _amount),
                0
            );
    }
