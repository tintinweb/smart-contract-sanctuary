...
contract VegasCrypto is Context, IERC20, Ownable {
    ...
    string private _name = "Vegas Crypto";
    string private _symbol = "VCR";
    ...
    uint256 internal _tokenTotal = 10000 *10**16; // 100B total supply
    ...
    // fees
    uint256 public _taxFee = 0; // 100 = 1%  SET TO 0 FOR PRESALE PURPOSES
    uint256 public _BurnFee = 0; // 100 = 1% SET TO 0 FOR PRESALE PURPOSES
    uint256 public _liquidityFee = 0; // 100 = 1% SET TO 0 FOR PRESALE PURPOSES
    //variables
    uint256 public _BurnFeeTotal;
    uint256 public _taxFeeTotal;
    uint256 public _liquidityFeeTotal;
    //addresses
    address public liquidityAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // PancakeSwap Router V2 (Needs to be changed later with function, I'll guide you)
 ...
 /**
    ...
    */
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x10ED43C718714eb63d5aA57B78B54704E256024E,"VCR: Uniswap router cannot be excluded.");
        require(account != address(this), 'FBL: The contract it self cannot be excluded');
        require(!_isExcluded[account], "VCR: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "VCR: Account is already included");
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(signatory != address(0), "VCR::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "VCR::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "VCR::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "VCR::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "VCR::_writeCheckpoint: block number exceeds 32 bits");
