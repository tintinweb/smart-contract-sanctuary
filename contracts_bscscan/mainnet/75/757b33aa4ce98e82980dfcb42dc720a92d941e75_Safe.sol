...
contract Safe is Context, IERC20, Ownable {
	...
	address payable public DevelopmentAddress = payable(msg.sender);
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(DevelopmentAddress, transferredBalance.div(LiquidityFee).mul(marketingFee));
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateDevelopmentFee(tAmount);
    ...
    function setBotFee(uint256 _botFee) external onlyOwner() {
        BotFee = _botFee;
    ...
    function setDevelopmentFee (uint256 _devFee) external onlyOwner {
     numTokensSellToAddToLiquidity = _devFee;

     }

     function setLiquidityFee (uint256 _liqudityFee) external onlyOwner {
     LiquidityFee = _liqudityFee;
