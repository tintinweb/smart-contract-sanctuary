...
contract PancakePredictionV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    event BetBear(address indexed sender, uint256 indexed epoch, uint256 amount, uint256 timestamp);
    event BetBull(address indexed sender, uint256 indexed epoch, uint256 amount, uint256 timestamp);
    event Claim(address indexed sender, uint256 indexed epoch, uint256 amount, uint256 timestamp);
    event EndRound(uint256 indexed epoch, uint256 indexed roundId, int256 price, uint256 blockNumber, uint256 timestamp);
    event LockRound(uint256 indexed epoch, uint256 indexed roundId, int256 price, uint256 blockNumber, uint256 timestamp);
    ...
    function betBear(uint256 epoch) external payable whenNotPaused nonReentrant notContract {
        ...
        emit BetBear(msg.sender, epoch, amount, block.timestamp);
    }
    ...
    function betBull(uint256 epoch) external payable whenNotPaused nonReentrant notContract {
        ...
        emit BetBull(msg.sender, epoch, amount, block.timestamp);
    }
    ...
    function claim(uint256[] calldata epochs) external nonReentrant notContract {
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            ...
            emit Claim(msg.sender, epochs[i], addedReward, block.timestamp);
        }
    ...
    function _safeEndRound(
        ...
        emit EndRound(epoch, roundId, round.closePrice, block.number, block.timestamp);
    }
    ...
    function _safeLockRound(
        ...
        emit LockRound(epoch, roundId, round.lockPrice, block.number, block.timestamp);
    }
