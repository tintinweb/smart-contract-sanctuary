...
interface IStrategy {
    ...
    function withdrawWantToken(address _userAddress, uint256 _wantAmt) external;
}
...
contract SugarFarm is Ownable, ReentrancyGuard {
    ...
    // Token address
    address public SUGAR = 0x7A3306b0aA282829E8cEc27f98D99Dc743229E81;
    ...
    function withdrawWantedToken(uint256 pid, uint256 _wantAmt) public nonReentrant returns (uint256) {
        updatePool(pid);
        ...
        uint256 sharesTotal = IStrategy(pool.strat).sharesTotal();
        uint256 wantLockedTotal =
            IStrategy(poolInfo[pid].strat).wantLockedTotal();
        if (sharesTotal == 0) {
            return 0;
        }
        uint256 pending = user.shares.mul(wantLockedTotal).div(sharesTotal);

        if (pending < _wantAmt) {
            _wantAmt = pending;
        }

        IStrategy(pool.strat).withdrawWantToken(address(msg.sender), _wantAmt);
