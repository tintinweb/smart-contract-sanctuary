...
contract MasterChef is Ownable  {
    ...
    struct UserInfo {
      ...
      //
        // We do some fancy math here. Basically, any point in time, the amount of Mmonx's
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       //  How many allocation points assigned to this pool. Mmonx's to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Mmonx distribution occurs.
        uint256 accMmonxPerShare;   //Accumulated Mmonx's per share, times 1e12. See below.
    ...
    //Mmonx's tokens created per block.
    uint256 public mmonxPerBlock;
    // Bonus muliplier for early Mmonx's makers.
    ...
    event SetLiqAddress(address indexed user, address indexed newAddress);
 ...
 constructor(
  ...
  // Safe Mmonx transfer function, just in case if rounding error causes pool to not have enough Mmonx's.
    function safeMmonxTransfer(address _to, uint256 _amount) internal {
        uint256 mmonxBal =  mmonx.balanceOf(address(this));
        if (_amount > mmonxBal) {
           mmonx.transfer(_to, mmonxBal);
        } else {
           mmonx.transfer(_to, _amount);
        }
    }
