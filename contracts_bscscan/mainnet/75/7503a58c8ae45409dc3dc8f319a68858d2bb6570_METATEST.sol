...
contract METATEST is ERC20, Ownable {
    ...
    METATESTDividendTracker public dividendTracker;
    ...
    address public _marketingWalletAddress = 0x8dC943582670B3314Ce2DEf64535d0EFb7a9D399;
    ...
    constructor() public ERC20("METATEST", "METATEST") {

        dividendTracker = new METATESTDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "METATEST: The dividend tracker already has that address");

        METATESTDividendTracker newDividendTracker = METATESTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "METATEST: The new dividend tracker must be owned by the METATEST token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "METATEST: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "METATEST: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "METATEST: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "METATEST: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "METATEST: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "METATEST: Cannot update gasForProcessing to same value");
...
contract METATESTDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("METATEST_Dividen_Tracker", "METATEST_Dividend_Tracker") {
        claimWait = 1800;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 1000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "METATEST_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "METATEST_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main METATEST contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "METATEST_Dividend_Tracker: claimWait must be updated to between 0.5 and 24 hours");
        require(newClaimWait != claimWait, "METATEST_Dividend_Tracker: Cannot update claimWait to same value");
