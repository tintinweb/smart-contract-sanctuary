...
contract GetUSDC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    getUSDCDividendTracker public dividendTracker;

    address public getUSDCWallet;
    ...
    constructor() ERC20("GetUSDC", "GUSDC") {
        uint256 _dividendRewardsFee = 7;
        uint256 _getUSDCFee = 10;

        dividendRewardsFee = _dividendRewardsFee;
        getUSDCFee = _getUSDCFee;
        totalFees = _dividendRewardsFee.add(_getUSDCFee);

        dividendTracker = new getUSDCDividendTracker();

        getUSDCWallet = 0x4e275fBBD60637467fC578ca40643417277E2F4D;
        ...
        excludeFromFees(getUSDCWallet, true);
        ...
        onlyOwner
    {
        require(
            ...
            "getUSDC: Sell transaction multipler must be between 100 (1x) and 200 (2x)"
        );
    ...
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(
            ...
            "getUSDC: Marketing divisor must be between 0 (0%) and 100 (100%)"
        );
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(
            !swapping,
            "getUSDC: A swapping process is currently running, wait till that is complete"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "getUSDC: The dividend tracker already has that address"
        );

        getUSDCDividendTracker newDividendTracker = getUSDCDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "getUSDC: The new dividend tracker must be owned by the FLOKIBUSD token contract"
        );
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "getUSDC: Dividend reward tax must be between 0 and 10"
        );
        dividendRewardsFee = newFee;
    }

    function updategetUSDCFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "getUSDC: Dividend reward tax must be between 0 and 10"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "getUSDC: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "getUSDC: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "getUSDC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "getUSDC: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updategetUSDCWallet(address newgetUSDCWallet) public onlyOwner {
        require(
            newgetUSDCWallet != getUSDCWallet,
            "getUSDC: The liquidity wallet is already this address"
        );
        excludeFromFees(newgetUSDCWallet, true);
        getUSDCWallet = newgetUSDCWallet;
        emit getUSDCWalletUpdated(newgetUSDCWallet, getUSDCWallet);
    }

    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            newValue >= 200000 && newValue <= 500000,
            "getUSDC: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "getUSDC: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[from]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 swapTokens = contractTokenBalance.mul(getUSDCFee).div(
                    totalFees
                );
                swapTokensForBNB(swapTokens);
                transferTogetUSDCWallet(
                    payable(getUSDCWallet),
    ...
    function transferTogetUSDCWallet(address payable recipient, uint256 amount)
        private
    {
...
contract getUSDCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "getUSDC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "getUSDC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main getUSDC contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "getUSDC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "getUSDC_Dividend_Tracker: Cannot update claimWait to same value"
        );
