...
/**
 *    TerminatorFloki Tokenomics
 *    totalSupply = 1 Q
 *     35% burn at launch
 *    Note:
 *      - 1% tx fee redistributed to token holders on each transaction.  
 *      - 4% liquidity fee is taken on each transaction and locked forever in the liquidity pool.
 *      - 4% cmarketing fee is taken on each transaction.
 *      - Functionality for max wallet size has been added (2% of total supply)
 *      - Cleaned up solc warnings
 *      - Updated to compile under solc >=0.8.0
 *      t.me/terminatorfloki
 */
...
contract TerminatorFloki is Context,IBEP20,Ownable {
    ...
    string private constant _name = "TerminatorFloki";
    ...
    uint256 public _maxWalletToken = 30000000000000 * 10**9;
