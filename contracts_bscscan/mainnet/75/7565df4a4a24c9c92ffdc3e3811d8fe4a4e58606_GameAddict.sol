...
contract ERC20 is Context, IERC20 {
     ...
     * Adaer and Wei. This is the value {ERC20} uses, unless {_setupDecimals} is
     * called.
     *
...
contract GameAddict is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool public SwapNliquifyIsEnabled = false;
    ...
    constructor() ERC20("Game Addict", "GA") {
    	adaDividendTracker = new AdaDividendTracker();
    	ethDividendTracker = new EthDividendTracker();

    	marketingWallet = 0xd19b25Cf5B1004BEf2FCeFDB4e389cb268578B5f;
    	teamWallet = 0x1eA53c4144f31EB430b6bb58825Fed3e00c83e3d;
    ...
    receive() external payable {
  	...
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "GameAddict: The team wallet is already this address");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "GameAddict: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function setSwapNliquifyIsEnabled(bool _enabled) external onlyOwner {
        SwapNliquifyIsEnabled = _enabled;
    }
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(ethDividendRewardsFee).add(adaDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setAdaDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = adaDividendRewardsFee.add(marketingFee).add(ethDividendRewardsFee).add(buyBackAndLiquidityFee);
            adaDividendEnabled = _enabled;
        }
    ...
    function setEthDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = ethDividendRewardsFee.add(marketingFee).add(adaDividendRewardsFee).add(buyBackAndLiquidityFee);
            ethDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(ethDividendRewardsFee).add(adaDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateAdaDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(adaDividendTracker), "GameAddict: The dividend tracker already has that address");

        AdaDividendTracker newAdaDividendTracker = AdaDividendTracker(payable(newAddress));

        require(newAdaDividendTracker.owner() == address(this), "GameAddict: The new dividend tracker must be owned by the GameAddict token contract");
    ...
    function updateEthDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "GameAddict: The dividend tracker already has that address");

        EthDividendTracker newEthDividendTracker = EthDividendTracker(payable(newAddress));

        require(newEthDividendTracker.owner() == address(this), "GameAddict: The new dividend tracker must be owned by the GameAddict token contract");
    ...
    function updateEthDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = ethDividendRewardsFee.add(adaDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }
    
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        marketingFee = newFee;
        totalFees = marketingFee.add(adaDividendRewardsFee).add(ethDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(adaDividendRewardsFee).add(ethDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "GameAddict: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "GameAddict: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GameAddict: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "GameAddict: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "GameAddict: Cannot update gasForProcessing to same value");
	...
	function processDividendTracker(uint256 gas) external onlyOwner {
		(uint256 ethIterations, uint256 ethClaims, uint256 ethLastProcessedIndex) = adaDividendTracker.process(gas);
		emit ProcessedAdaDividendTracker(ethIterations, ethClaims, ethLastProcessedIndex, false, gas, tx.origin);
		
		(uint256 dogeBackIterations, uint256 dogeBackClaims, uint256 dogeBackLastProcessedIndex) = ethDividendTracker.process(gas);
		emit ProcessedEthDividendTracker(dogeBackIterations, dogeBackClaims, dogeBackLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function _transfer(
        ...
        require(SwapNliquifyIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "GameAddict: SwapNliquify has not started yet");
        
        bool excludedAccount = isExcludedFromFees[from] || isExcludedFromFees[to];
        
        if (
            SwapNliquifyIsEnabled &&
            ...
            if (!swapping && canSwap) {
                ...
                if (marketingEnabled) {
                    ...
                    uint256 teamPortion = address(this).balance.div(10**2).mul(33);
                ...
                if (adaDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(adaDividendRewardsFee.add(ethDividendRewardsFee)).mul(adaDividendRewardsFee);
                    swapAndSendAdaDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (ethDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(adaDividendRewardsFee.add(ethDividendRewardsFee)).mul(ethDividendRewardsFee);
        ...
        bool takeFee = SwapNliquifyIsEnabled && !swapping && !excludedAccount;
...
contract AdaDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("GameAddict_Adaereum_Dividend_Tracker", "GameAddict_Adaereum_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 20000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "GameAddict_Adaereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "GameAddict_Adaereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GameAddict contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "GameAddict_Adaereum_Dividend_Tracker: Cannot update claimWait to same value");
...
contract EthDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("GameAddict_Eth_Dividend_Tracker", "GameAddict_Eth_Dividend_Tracker", 0x2170Ed0880ac9A755fd29B2688956BD959F933F8) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 20000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "GameAddict_Eth_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "GameAddict_Eth_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GameAddict contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "GameAddict_Eth_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
