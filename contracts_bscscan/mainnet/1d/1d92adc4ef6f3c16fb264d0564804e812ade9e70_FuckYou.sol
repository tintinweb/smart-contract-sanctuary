...
// FuckYouToken with Governance.
contract FuckYou is BEP20 {
    ...
    // Min amount to liquify. (default 250 FUCKYOUs)
    ...
    // The trading pair
    address public fuckyouSwapPair;
    // In swap and liquify
    ...
    uint16 public maxHoldingRate = 1;
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxHolding() > 0 && sender != owner() && recipient != owner()) {
            if ( _excludedFromAntiWhale[recipient] == false ) {
                require(amount <= maxHolding() - balanceOf(recipient) && balanceOf(recipient) <= maxHolding(), "FUCKYOU::antiWhale: Transfer amount would result in a balance bigger than the maxHoldingRate");
            }
        }
        _;
    }
    ...
    /**z
     * @notice Constructs the FuckYouToken contract.
     */
    constructor() public BEP20("FuckYou Token", "FUCKYOU") {
    ...
    /// @dev overrides transfer function to meet tokenomics of FUCKYOU
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (
            ...
            && fuckyouSwapPair != address(0)
            && sender != fuckyouSwapPair
            && sender != owner()
        ) {
            swapAndLiquify();
        }
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "FUCKYOU::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "FUCKYOU::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the FuckYouSwap pair path of token -> weth
    ...
    function maxHolding() public view returns (uint256) {
        return totalSupply().mul(maxHoldingRate);
    }
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "FUCKYOU::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "FUCKYOU::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxHoldingRate(uint16 _maxHoldingRate) public onlyOperator {
        require(_maxHoldingRate >= 100, "FUCKYOU::updateMaxHoldingRate: Max holding rate must not be below the minimum rate.");
    ...
    function updateSwapPair() public onlyOperator {
        fuckyouSwapPair = IUniswapV2Factory(SwapRouter.factory()).getPair(address(this), SwapRouter.WETH());
        require(fuckyouSwapPair != address(0), "FUCKYOU::updateSwapPair: Invalid pair address.");
        emit SwapRouterUpdated(msg.sender, address(SwapRouter), fuckyouSwapPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "FUCKYOU::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "FUCKYOU::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "FUCKYOU::delegateBySig: invalid nonce");
        require(now <= expiry, "FUCKYOU::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "FUCKYOU::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying FUCKYOUs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "FUCKYOU::_writeCheckpoint: block number exceeds 32 bits");
