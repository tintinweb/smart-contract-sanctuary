...
contract RamaReferral is IRamaReferral, Ownable {
...
contract RamaStaking is Ownable{
    ...
    RamaReferral public ramaReferral;
    uint256 public ANNUAL_PLAN_APY = 2400;
    uint256 public HALF_YEAR_PLAN_APY = 1000;
    uint256 public MULTIPLIER = 10000;
    uint256 public firstLevelComission = 1200;
    uint256 public secondLevelComission = 600;
    uint256 public thirdLevelComission = 300;
    ...
    event RefComissionPaid(address user,address paidTo,uint256 amount,uint256  level);
    ...
    constructor(address _companyWallet,IBEP20 _ramaToken) public {
        companyWallet = _companyWallet;
        ramaToken = _ramaToken;
        ramaReferral = new RamaReferral();
        ramaReferral.updateOperator(address(this),true);
        ramaReferral.transferOwnership(msg.sender);
        
    }
    
    
    
    function setRamaReferral(address addr) public onlyOwner{
        ramaReferral = RamaReferral(addr);
    }
  ...
  function getDeposit(address user,uint256 index) public view returns( 
    ...
    function getReferral(address user) public view returns (address){
        address addr = ramaReferral.referrers(user);
        if(addr == address(0)){
    ...
    function payReferralCommission(address user,uint256 amount) internal{
        ...
        uint256 thirdLComissionAmount = amount.mul(thirdLevelComission).div(MULTIPLIER);

        address firstLevelReferrer = getReferral(user);
        address secondLevelReferrer = getReferral(firstLevelReferrer);
        address thirdlevelReferrer = getReferral(secondLevelReferrer);
        ...
        transferTokens(thirdlevelReferrer,thirdLComissionAmount);
        refEarnings[thirdlevelReferrer] =refEarnings[thirdlevelReferrer].add(thirdLComissionAmount);

        emit RefComissionPaid(user,firstLevelReferrer,firstLComissionAmount,1);
        emit RefComissionPaid(user,secondLevelReferrer,secondLComissionAmount,2);
        emit RefComissionPaid(user,thirdlevelReferrer,thirdLComissionAmount,3);


    }

    function setReferral(address user,address refferal) internal{
        ramaReferral.recordReferral(user,refferal);
    }
    
    
    
    function transferTokens(address _user,uint256 amount) internal{
        uint256 balance = ramaToken.balanceOf(address(this));
        if(balance < amount){
            ramaToken.transfer(_user,balance);

        }else{
            ramaToken.transfer(_user,amount);
        }
    }
