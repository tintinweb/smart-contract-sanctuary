...
//PANCAKE ROCKS
//Missed ether rocks? This is your chance on the BSC

//WEB: http://pancakerocksnft.com
//TW:    https://twitter.com/pancakerocksnft
//TG:    https://t.me/pancakerocksnft


//Contract is ready to request listing after minting 100%
//-**Metadata secured on IFPS
//-**ERC721 Standard
...
contract PCSRocks is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0xa37A5519f52DAba5BA23127640988cB342E767Ef);
    address payable private _DevAddress;

    uint256 public PCSRocks_MAX = 100;
    uint256 public PURCHASE_LIMIT = 4;

    bool private _isActive = false;
    string private _contractURI = "";
    string private _tokenBaseURI1 = "";

    Counters.Counter private _publicPCSRocks;

    constructor() ERC721("Pancake Rocks", "PCSRocks") {
        _DevAddress = payable(msg.sender);
    }

    function PRICE() public view returns(uint256){
        if (_publicPCSRocks.current() < 20) return 200_000_000_000_000_000;//0.2BNB
        return 1_000_000_000_000_000_000;//1BNB
    }
    ...
    function gift(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicPCSRocks.current();

            if (_publicPCSRocks.current() < PCSRocks_MAX) {
                _publicPCSRocks.increment();
                _safeMint(to, tokenId);
            }
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(
            _publicPCSRocks.current() < PCSRocks_MAX,
            "Purchase would exceed PCSRocks_MAX"
        );
        require(
