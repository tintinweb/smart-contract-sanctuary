...
contract Eat is Context, IBEP20, Ownable {
    ...
    mapping(address => bool) public initialPurchaser;
    ...
    address public addressBurn = 0x000000000000000000000000000000000000dEaD;
    address public addressDev = 0xbB81e9682f196633736ec9201f4071Fe908f3Be9;
    address public addressCharity = 0xC9c0a1FbFe692F5957D3d7e2d455d6af5718D73D;
    ...
    uint256 public _totalFees = _devFee + _charityFee + _liquidityFee + _burnFee;
    ...
    function airDrop(address recipient, uint256 value, bool presaler) public onlyOwner returns(bool){
        removeAllFee();
        _transfer(_msgSender(), recipient, value * 10 ** 9);
        if(presaler){
            initialPurchaser[recipient] = true;
        }
        restoreAllFee();
        return true;
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _charityFee == 0 && _devFee == 0 && _burnFee == 0) return;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 stakingShare = contractTokenBalance.div(2);

            swapTokensForEth(stakingShare);

            //redistribute BnB
            uint256 balance = address(this).balance;
            totalSwappedToBnb += balance;

            uint256 stakingBnbShare = (5625 * balance).div(10000);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee)  private {
        ...
        else if(recipient == panswapV2Pair){ require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount."); }
        else{
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(burnAmt).sub(devAmt).sub(charityAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(burnAmt).sub(devAmt).sub(charityAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(devAmt).sub(charityAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(burnAmt).sub(devAmt).sub(charityAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(devAmt).sub(charityAmt)));
        }
        ...
        _transferStandard(sender, addressDev, devAmt);
        _transferStandard(sender, addressCharity,charityAmt);
        _transferStandard(sender, addressBurn, burnAmt);
