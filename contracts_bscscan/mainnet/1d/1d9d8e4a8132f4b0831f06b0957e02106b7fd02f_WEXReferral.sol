...
contract WEXReferral is IWedexReferral, Ownable {
    ...
    mapping(address => uint256) public totalFund; // referrer address => referrals count
    ...
    function addTotalFund(address _referrer, uint256 _amount, uint256 _loop) public override onlyOperator {
        if (_referrer != address(0) && _amount > 0 && _loop<maximumLoop) {
            uint256 currentLoop = _loop+1;
            totalFund[_referrer].add(_amount.mul(lpPrice));
    ...
    function reduceTotalFund(address _referrer, uint256 _amount, uint256 _loop) public override onlyOperator {
        if (_referrer != address(0) && _amount > 0 && _loop<maximumLoop && totalFund[_referrer]>_amount) {
            uint256 currentLoop = _loop+1;
            totalFund[_referrer].sub(_amount.mul(lpPrice));
            if(referrers[_referrer] != address(0)){
    ...
    function getReferrerPercentage(address _referrer) public view override returns(uint256) {
        if(address(wexProfile) != address(0) && wexProfile.getUserStatus(_referrer)){
        if(referalsTier[_referrer]>5e6)
            return 50;
        if(referalsTier[_referrer]>2e6)
            return 45;
        if(referalsTier[_referrer]>1e6)
            return 40;
        if(referalsTier[_referrer]>2e5)
            return 35;
        if(referalsTier[_referrer]>1e5)
            return 30;
        if(referalsTier[_referrer]>5e4)
            return 25;
        return 20;
        }
        return 0;
    ...
    function getTeam(address _user) public view returns (address [] memory){
        return team[_user];
    }
    ...
    function getTotalFund(address _referrer) public view returns (uint256){
        
        return totalFund[_referrer];
    }
