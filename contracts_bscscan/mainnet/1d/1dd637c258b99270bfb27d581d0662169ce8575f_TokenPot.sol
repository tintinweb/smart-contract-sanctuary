...
contract TPotDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("TPot_Dividend_Tracker", "TPot_Dividend_Tracker") {
        claimWait = 14400;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "TPot_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "TPot_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TPot contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TPot_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "TPot_Dividend_Tracker: Cannot update claimWait to same value");
...
contract TokenPot is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    TPotDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 1000000 * (10**9);
    uint256 public swapTokensAtAmount = 2 * 10**4 * (10**9);
    ...
    constructor() ERC20("TokenPot", "TPot") {
        BNBRewardsFee = 2;
        liquidityFee = 2;
        extraFeeOnSell = 2; // extra fee on sell
        MarketingFee = 6;
        MarketingWallet = payable(0x2176e90BC82dfBafaF6D121750E821Ad4454A486);  

        totalFees = BNBRewardsFee.add(liquidityFee).add(MarketingFee); // total fee transfer and buy

    	dividendTracker = new TPotDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TPot: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TPot: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TPot: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TPot: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TPot: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TPot: Cannot update gasForProcessing to same value");
