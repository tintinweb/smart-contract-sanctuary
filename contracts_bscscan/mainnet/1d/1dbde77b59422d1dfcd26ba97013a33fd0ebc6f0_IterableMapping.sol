...
/*
    ...
    Just buy and hold and get BNB!

    15% BNB automated redistribution, no need to claim, 
    
    5% added to liquidity
    
    Telegram: https://t.me/DogeUSDToken
...
contract DogeUSD is ERC20, Ownable {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "DogeUSD: The new dividend tracker must be owned by the DogeUSD token contract");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "DogeUSD: The liquidity wallet is already this address");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "DogeUSD: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "DogeUSD: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "DogeUSD: The fixed-sale is only available to certain participants at the start");
            }
...
contract DogeUSDDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("DUSD_Dividend_Tracker", "DUSD_Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DUSD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "DUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DUSD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "DUSD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DUSD_Dividend_Tracker: Cannot update claimWait to same value");
