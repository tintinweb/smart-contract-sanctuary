...
// Feisty Doge NFT NFD BY Rogue Mage Andy
// Fractional.art
...
contract InitializedProxy is ERC20, Ownable {
    ...
    constructor() ERC20("Feisty Doge NFT", "NFD") {
    ...
    function updateDogeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dogeDividendTracker), "NFD: The dividend tracker already has that address");

        DogeDividendTracker newDogeDividendTracker = DogeDividendTracker(payable(newAddress));

        require(newDogeDividendTracker.owner() == address(this), "NFD: The new dividend tracker must be owned by the NFD token contract");
...
contract DogeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("NFD_Dividend_Tracker", "NFD_Dividend_Tracker", 0xbA2aE424d960c26247Dd6c32edC70B295c744C43) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 500000 * (10**18); //must hold 500,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "NFD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "NFD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "NFD_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "NFD_Dividend_Tracker: Cannot update claimWait to same value");
