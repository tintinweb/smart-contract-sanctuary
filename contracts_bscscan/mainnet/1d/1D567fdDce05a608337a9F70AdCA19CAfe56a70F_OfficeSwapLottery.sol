...
// File: contracts/interfaces/IOfficeSwapLottery.sol

pragma solidity ^0.8.4;

interface IOfficeSwapLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in OFT per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in OFT token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInOft: price of a ticket in OFT
...
// File: contracts/OfficeSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title OfficeSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract OfficeSwapLottery is ReentrancyGuard, IOfficeSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInOft = 50 ether;
    uint256 public minPriceTicketInOft = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInOft;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] oftPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInOft;
        uint32 finalNumber;
    }
     ...
     * @param _oftTokenAddress: address of the OFT token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _oftTokenAddress, address _randomGeneratorAddress) {
        oftToken = IERC20(_oftTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of OFT to this contract
        uint256 amountOftToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInOft,
            _ticketNumbers.length
        );

        // Transfer oft tokens to this contract
        oftToken.safeTransferFrom(address(msg.sender), address(this), amountOftToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInOft += amountOftToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInOftToTransfer
        uint256 rewardInOftToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInOftToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        oftToken.safeTransfer(msg.sender, rewardInOftToTransfer);

        emit TicketsClaim(msg.sender, rewardInOftToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInOft) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in OFT for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].oftPerBracket[j] =
                ...
                // A. No OFT to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].oftPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInOft - amountToShareToWinners);

        // Transfer OFT to treasury address
        oftToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        oftToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInOft += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInOft >= minPriceTicketInOft) && (_priceTicketInOft <= maxPriceTicketInOft),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInOft: _priceTicketInOft,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            oftPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInOft: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(oftToken), "Cannot be OFT token");
    ...
    /**
     * @notice Set OFT price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInOft: minimum price of a ticket in OFT
     * @param _maxPriceTicketInOft: maximum price of a ticket in OFT
     */
    function setMinAndMaxTicketPriceInOft(uint256 _minPriceTicketInOft, uint256 _maxPriceTicketInOft)
        external
        onlyOwner
    {
        require(_minPriceTicketInOft <= _maxPriceTicketInOft, "minPrice must be < maxPrice");

        minPriceTicketInOft = _minPriceTicketInOft;
        maxPriceTicketInOft = _maxPriceTicketInOft;
    }
     ...
     * @param _priceTicket price of a ticket (in OFT)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].oftPerBracket[_bracket];
        } else {
            return 0;
        }
    }
