...
contract Hydra is IERC20, Ownable {
    ...
    modifier launchRestrict(address sender, address recipient, uint256 amount) {
        ...
        if (state == State.Restricted) {
            require(amount <= maxRestrictionAmount, "Hydra: amount greater than max limit in restricted mode");
            if (!isWhitelisted[sender] && !isWhitelisted[recipient]) {
                require(lastTx[sender].add(60) <= block.timestamp && lastTx[recipient].add(60) <= block.timestamp, "Hydra: only one tx/min in restricted mode");
                lastTx[sender] = block.timestamp;
                lastTx[recipient] = block.timestamp;
            } else if (!isWhitelisted[recipient]) {
                require(lastTx[recipient].add(60) <= block.timestamp, "Hydra: only one tx/min in restricted mode");
                lastTx[recipient] = block.timestamp;
            } else if (!isWhitelisted[sender]) {
                require(lastTx[sender].add(60) <= block.timestamp, "Hydra: only one tx/min in restricted mode");
                lastTx[sender] = block.timestamp;
            }
        }
        _;
...
contract HydraSeedAndPreSale is Ownable {
