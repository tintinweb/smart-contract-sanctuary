...
contract Luckydoge is IERC20, Ownable, Pausable {
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    string constant public name = "Luckydoge";
    string constant public symbol = "Luckydoge";
    ...
    function setFeePercentage(uint8 feePercentage_) external onlyOwner {
        require(feePercentage_ <= 30, "Luckydoge: transaction fee percentage exceeds 30");
        require(feePercentage_ >= 0, "Luckydoge: transaction fee percentage equals 0");
    ...
    function blacklist(address[] calldata addresses, bool isBlacklist) external {
        require(hasBlacklistPermission[msg.sender], "Luckydoge:insufficient permission");
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "Luckydoge: transfer from blacklisted address");
        require(!isBlacklisted[recipient], "Luckydoge: transfer to blacklisted address");
        require(!isBlacklisted[tx.origin], "Luckydoge: transfer called from blacklisted address");
