...
import "./BTestDividendTracker.sol";
import "./BTestGame.sol";
...
contract BTest is ERC20, Ownable {
    ...
    BTestGame public bTestGameInstance;
    bool public isGameEnabled;

    BTestDividendTracker public dividendTracker;
    ...
    uint256 public bTestAmountLiquidityFee = 0;
    ...
    constructor() ERC20("BTest", "BTest") {
        dividendTracker = new BTestDividendTracker();
    ...
    function setBTestGameAddress(address _bTestGameAddress) public onlyOwner {
        bTestGameInstance = BTestGame(_bTestGameAddress);
    }

    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(
            newAddress != address(pancakeRouter),
            "BTest: The router already has that address"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "BTest: The dividend tracker already has that address"
        );

        BTestDividendTracker newDividendTracker = BTestDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "BTest: The new dividend tracker must be owned by the BTest token contract"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "BTest: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "BTest: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "BTest: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (!tradingEnabled) {
            require(
                ...
                "BTest: This account cannot send tokens until trading is enabled"
            );
        }
        ...
        if (isGameEnabled && msg.sender != pancakePair) {
            bTestGameInstance.endUserGames(msg.sender);
        }
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        bTestAmountLiquidityFee = amounts[0];
...
contract BTestDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor()
        DividendPayingToken("BTestDividendTracker", "BTest_DividendTracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "BTestDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BTestDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main BTest contract."
        );
    }
