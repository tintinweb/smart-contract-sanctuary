...
// File: contracts\libs\IWedexReferral.sol
...
contract Wedex is BEP20 {
    ...
    event WedexSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "WEDEX::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "WEDEX::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "WEDEX::transfer: Tax value invalid");
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "WEDEX::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "WEDEX::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "WEDEX::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
    ...
    function updateWedexSwapRouter(address _router) public onlyOperator {
        ...
        wedexSwapPair = IUniswapV2Factory(wedexSwapRouter.factory()).getPair(address(this), wedexSwapRouter.WETH());
        require(wedexSwapPair != address(0), "WEDEX::updateWedexSwapRouter: Invalid pair address.");
        emit WedexSwapRouterUpdated(msg.sender, address(wedexSwapRouter), wedexSwapPair);
    }
   ...
   function recoverLostTokensExceptOurTokens(address _token, uint256 amount) public onlyOperator {
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "WEDEX::transferOperator: new operator is the zero address");
        emit OperatorTransferred(_operator, newOperator);
        _operator = newOperator;
    }
    
    function transferMinter(address newMinter) public onlyOperator {
        require(newMinter != address(0), "WEDEX::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "WEDEX::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "WEDEX::delegateBySig: invalid nonce");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "WEDEX::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "WEDEX::_writeCheckpoint: block number exceeds 32 bits");
...
contract WedexChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. WEDEXES to distribute per block.
    ...
    // WEDEX tokens created per block.
    ...
    function pendingWedex(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 WedexReward = multiplier.mul(wedexPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accWedexPerShare = accWedexPerShare.add(WedexReward.mul(1e12).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accWedexPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 wedexReward = multiplier.mul(wedexPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        wedex.mint(devAddress, wedexReward.div(10));
        wedex.mint(address(this), wedexReward);
        pool.accWedexPerShare = pool.accWedexPerShare.add(wedexReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(wedexReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            wedexReferral.recordReferral(msg.sender, _referrer);
            wedexReferral.addTotalFund(_referrer,_amount,0);
        }
        payOrLockupPendingWedex(_pid);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(wedex)) {
                uint256 transferTax = _amount.mul(wedex.transferTaxRate()).div(10000);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            wedexReferral.reduceTotalFund(wedexReferral.getReferrer(msg.sender),_amount,0);
        }
        user.rewardDebt = user.amount.mul(pool.accWedexPerShare).div(1e12);
    ...
    function payOrLockupPendingWedex(uint256 _pid) internal {
        ...
        uint256 pending = user.amount.mul(pool.accWedexPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updateEmissionRate(uint256 _wedexPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, wedexPerBlock, _wedexPerBlock);
        wedexPerBlock = _wedexPerBlock;
    }

    // Update the Wedex referral contract address by the owner
    function setWedexReferral(IWedexReferral _wedexReferral) public onlyOwner {
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        uint256 referralCommissionRate = wedexReferral.getReferrerPercentage(_user);
        if (address(wedexReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = wedexReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                wedex.mint(referrer, commissionAmount);
                wedexReferral.recordReferralCommission(referrer, commissionAmount);
                emit ReferralCommissionPaid(_user, referrer, commissionAmount);
            }
        }
    }
}
contract WEXRedeem is Ownable  {
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;
    ...
    constructor() public
    {
        wex = IBEP20(0x968a9F654404A86ABde86472f2C2514aCaB2b875); //Token Contract
