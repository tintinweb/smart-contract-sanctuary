...
// TG https://t.me/EarnDogeXRP  // 10% XRP rewards // Fair launch
...
contract EarnDogeXRP is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    EarnDogeXRPDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("EarnDogeXRP", "EDX") {
        _ETHRewardsFee = 10;
        _liquidityFee = 2;
		_marketingFee = 6;
        ...
        _maxWalletToken = 10000000000 * (10**18); // 


    	dividendTracker = new EarnDogeXRPDividendTracker();
    	antiBotEnabled = true;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "EarnDogeXRP: The dividend tracker already has that address");

        EarnDogeXRPDividendTracker newDividendTracker = EarnDogeXRPDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "EarnDogeXRP: The new dividend tracker must be owned by the EarnDogeXRP token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "EarnDogeXRP: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "EarnDogeXRP: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "EarnDogeXRP: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "EarnDogeXRP: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "EarnDogeXRP: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "EarnDogeXRP: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "EarnDogeXRP: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForBUSD(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of EarnDogeXRP
            path,
            recipient,
            block.timestamp
        );
...
contract EarnDogeXRPDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("EarnDogeXRP_Dividend_Tracker", "EarnDogeXRP_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "EarnDogeXRP_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "EarnDogeXRP_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EarnDogeXRP contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "EarnDogeXRP_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EarnDogeXRP_Dividend_Tracker: Cannot update claimWait to same value");
