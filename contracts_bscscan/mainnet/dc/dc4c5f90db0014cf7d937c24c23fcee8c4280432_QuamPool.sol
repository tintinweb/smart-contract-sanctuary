...
// File: contracts/IQuamFactory.sol
...
// File: contracts/QuamFactory.sol


pragma solidity 0.6.12;


contract QuamMarginFactory is Ownable {
    using SafeMath for uint256;
    ...
    function createPool(address _token) public returns (address) {
        ...
        address pool;
        bytes memory bytecode = type(QuamPool).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_token));
        assembly {
            pool := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        QuamPool(pool).initialize(_token, WETH);
