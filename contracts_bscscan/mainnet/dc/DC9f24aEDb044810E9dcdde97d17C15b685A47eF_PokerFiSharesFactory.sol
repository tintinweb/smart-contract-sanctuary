...
contract PokerFiSharesFactory is Ownable, ReentrancyGuard {
    ...
    event RewardAdded(uint256 amount,uint256 start,uint256 finish);
    ...
    function earned(address account) public view returns (uint256) {
        ...
        return rewards[account] + (shares[account] * (rewardPerShare() - rewardPerSharePaid[account]) / 1e18);
    }
    ...
    function stake(uint256 amount) external nonReentrant updateReward(_msgSender()) {
        ...
        stakingToken.safeTransfer(fundReceiver, amount * 30 / 100);

        IERC20Burnable(address(stakingToken)).burn(amount * 70 / 100);

        emit Staked(account, amount);
    }

    function addRewardAmount(uint256 amount, uint256 rewardDuration) external onlyOwner updateReward(address(0)) {
        require(amount % 1e18 == 0 && rewardDuration % 86400 == 0, "Invalid amount or reward duration");

        IERC20Mintable(address(rewardsToken)).mint(address(this), amount);

        rewardRate = rewardsToken.balanceOf(address(this)) / rewardDuration;
        periodFinish = block.timestamp + rewardDuration;

        lastUpdateTime = block.timestamp;

        emit RewardAdded(amount, block.timestamp, periodFinish);
    }
}
