...
contract BUTTHURT is IBEP20, Auth {
    ...
    function _transferfrom(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        uint256 amountReceived = tlFE(sender) ? processTF(sender, recipient, amount) : amount;
    ...
    function chTxL(address sender, uint256 amount) internal view {
        if(sender != pair){ require (amount <= _maxsellTxAmount && !isBt[sender] || isTxLE[sender], "TX Limit Exceeded"); }
    ...
    function definedSwap() internal swapping {
        ...
        payable(SPUK).transfer(aBNBFM);
        payable(SUKE).transfer(aBNBTM.sub(aBNBFM));
        if(amountL > 0){
    ...
    function currentFees(uint256 RewardsFee, uint256 LiquidityFee, uint256 MarketingFee, uint256 TotalFee) public view {
        RewardsFee = uint256(rwF);
        LiquidityFee = uint256(lqF);
        MarketingFee = uint256(mkF);
        TotalFee = uint256(ttF);
    }
