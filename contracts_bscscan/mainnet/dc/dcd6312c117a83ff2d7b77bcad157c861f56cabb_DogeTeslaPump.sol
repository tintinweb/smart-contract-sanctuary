...
contract DogeTeslaPump is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "DogeTeslaPump";
    string constant _symbol = "DogeTeslaPump";
    ...
    uint256 public totalFee        = marketingFee + reflectionFee + liquidityFee + devFee + buyback;
    ...
    uint256 public  sellLimit = rSupply.div(100).mul(10);
    ...
    constructor () Auth(msg.sender) {
        ...
        buybackReceiver = 0x16120D1deD0B73398eEFC46a5AC238BE92E3A73A;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBDev2 = amountBNB.mul(buyback).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(devFeeReceiver).call{value: amountBNBDev, gas: 30000}("");
        (tmpSuccess,) = payable(buybackReceiver).call{value: amountBNBDev2, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _devFee,  uint256 _buyback, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        devFee = _devFee;
        buyback = _buyback;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_devFee).add(_buyback);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/3, "Fees cannot be more than 33%");
    }

    
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _devFeeReceiver, address _buybackReceiver ) external authorized {
