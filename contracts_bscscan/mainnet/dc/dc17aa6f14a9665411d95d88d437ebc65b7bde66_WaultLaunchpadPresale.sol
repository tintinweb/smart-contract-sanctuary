...
contract WaultLaunchpadPresale is Ownable {
    ...
    function withdrawWei() public onlyOwner {
        ...
        weiSub.safeTransfer(msg.sender, weiSub.balanceOf(address(this)));
    }
    
    function withdrawWeiOriginal(uint256 amount) public onlyOwner {
        require(endUnlocked, 'presale has not yet ended');
    ...
    function buy(uint256 amount) public {
        ...
        require(weiRaised.add(amount) <= weiTarget, "target already hit");
        require(whitelistedAddresses[msg.sender] > 0, "you are not whitelisted");
        require(whitelistedAddresses[msg.sender] >= amount, "amount too high");
        
        uint256 equals = amount.mul(multiplier);
        
        require(totalOwed.add(equals) <= token.balanceOf(address(this)), "sold out");
        require(claimable[msg.sender].add(amount) <= whitelistedAddresses[msg.sender], "maximum purchase cap hit");
        
        weiSub.safeTransferFrom(msg.sender, address(this), amount);
        
        if(claimable[msg.sender] == 0)
            participants = participants.add(1);

        claimable[msg.sender] = claimable[msg.sender].add(amount);
        totalOwed = totalOwed.add(equals);
