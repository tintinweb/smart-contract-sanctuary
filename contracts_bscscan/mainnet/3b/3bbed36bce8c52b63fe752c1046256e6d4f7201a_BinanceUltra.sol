...
contract BinanceUltra is Context, IBEP20, Ownable{
    using SafeMath for uint256;
    ...
    string private _name = "Binance Ultra";
    ...
    uint256 _buyBackFee; 
    uint256 public _buyBackFeeForBuying = 30;  
    uint256 public _buyBackFeeForSelling = 40;  
    
    uint256 _marketFee;
    uint256 public _marketFeeForBuying = 30; 
    uint256 public _marketFeeForSelling = 40;  
    
    uint256 _devFee;
    uint256 public _devFeeForBuying = 20;  
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "BULTRA: approve from the zero address");
        require(spender != address(0), "BULTRA: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "BULTRA: transfer from the zero address");
        require(to != address(0), "BULTRA: transfer to the zero address");
        require(amount > 0, "BULTRA: Transfer amount must be greater than zero");

        if(!_isExcludedFromMaxTx[from] && 
            !_isExcludedFromMaxTx[to]   // by default false
        ){
            require(amount <= maxTxAmount,"BULTRA: Amount exceed max limit");

            if(to != uniSwapPair){
                require(balanceOf(to).add(amount) <= maxHoldingAmount,"BULTRA: max holding limit exceeds");
            }

            if(from == uniSwapPair){
                require(buyCoolDownTriggeredAt.add(buyCoolDownDuration) < block.timestamp,
                "BULTRA: wait for buying cool down"
                );
            ...
            if(to == uniSwapPair){
                ...
                "BULTRA: wait for selling cool down"
                );
...
library SafeMath {
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        // Gas optimization: this is cheaper than requiring 'a' not being zero, BULTRA the
