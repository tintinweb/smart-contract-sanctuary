    ...
    IDividendPayingTokenOptional
{
    ...
    address public dividendToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // Binance-Peg USDC
...
contract SBBN is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public _dividendToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // Binance-Peg USDC 
    ...
    shibabinancebscDividendTracker public dividendTracker;

    address public shibabinancebscWallet;
    ...
    uint256 public shibabinancebscFee;
    ...
    event shibabinancebscWalletUpdated(
    ...
    constructor() ERC20("SBBN", "SBBN") {
        uint256 _dividendRewardsFee = 7;
        uint256 _shibabinancebscFee = 10;

        dividendRewardsFee = _dividendRewardsFee;
        shibabinancebscFee = _shibabinancebscFee;
        totalFees = _dividendRewardsFee.add(_shibabinancebscFee);

        dividendTracker = new shibabinancebscDividendTracker();

        shibabinancebscWallet = 0x0543eDB2c291419dBf43b18c7A5329924D0892a7;
        ...
        excludeFromFees(shibabinancebscWallet, true);
        ...
        onlyOwner
    {
        require(
            ...
            "shibabinancebsc: Sell transaction multipler must be between 100 (1x) and 200 (2x)"
        );
    ...
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(
            ...
            "shibabinancebsc: Marketing divisor must be between 0 (0%) and 100 (100%)"
        );
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(
            !swapping,
            "shibabinancebsc: A swapping process is currently running, wait till that is complete"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "shibabinancebsc: The dividend tracker already has that address"
        );

        shibabinancebscDividendTracker newDividendTracker = shibabinancebscDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "shibabinancebsc: The new dividend tracker must be owned by the FLOKIBUSD token contract"
        );
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "shibabinancebsc: Dividend reward tax must be between 0 and 10"
        );
        dividendRewardsFee = newFee;
    }

    function updateshibabinancebscFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "shibabinancebsc: Dividend reward tax must be between 0 and 10"
        );
        shibabinancebscFee = newFee;
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            newAddress != address(uniswapV2Router),
            "shibabinancebsc: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "shibabinancebsc: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "shibabinancebsc: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "shibabinancebsc: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateshibabinancebscWallet(address newshibabinancebscWallet) public onlyOwner {
        require(
            newshibabinancebscWallet != shibabinancebscWallet,
            "shibabinancebsc: The liquidity wallet is already this address"
        );
        excludeFromFees(newshibabinancebscWallet, true);
        shibabinancebscWallet = newshibabinancebscWallet;
        emit shibabinancebscWalletUpdated(newshibabinancebscWallet, shibabinancebscWallet);
    }

    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            newValue >= 200000 && newValue <= 500000,
            "shibabinancebsc: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "shibabinancebsc: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[from]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 swapTokens = contractTokenBalance.mul(shibabinancebscFee).div(
                    totalFees
                );
                swapTokensForBNB(swapTokens);
                transferToshibabinancebscWallet(
                    payable(shibabinancebscWallet),
    ...
    function transferToshibabinancebscWallet(address payable recipient, uint256 amount)
        private
    {
...
contract shibabinancebscDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken(
            "shibabinancebsc_Dividend_Tracker",
            "shibabinancebsc_Dividend_Tracker"
        )
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "shibabinancebsc_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "shibabinancebsc_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main shibabinancebsc contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "shibabinancebsc_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "shibabinancebsc_Dividend_Tracker: Cannot update claimWait to same value"
        );
