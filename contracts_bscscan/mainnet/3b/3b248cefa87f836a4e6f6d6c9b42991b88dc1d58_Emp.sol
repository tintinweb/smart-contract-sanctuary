...
// File contracts/Emp.sol


pragma solidity 0.6.12;
contract Emp is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 48000 ether;
    // Initial distribution for the day 2-5 EMP-WETH LP -> EMP pool
    uint256 public constant INITIAL_EMP_POOL_DISTRIBUTION = 672000 ether;
    ...
    /**
     * @notice Constructs the EMP ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("emp.money", "EMP") {
        // Mints 1 EMP to contract creator for initial pool setup
    ...
    function _getEmpPrice() internal view returns (uint256 _empPrice) {
        try IOracle(empOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Emp: failed to fetch EMP price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _empPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_empPrice >= taxTiersTwaps[tierId]) {
    ...
    function setEmpOracle(address _empOracle) public onlyOperatorOrTaxOffice {
        require(_empOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints EMP to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of EMP to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentEmpPrice = _getEmpPrice();
            currentTaxRate = _updateTaxRate(currentEmpPrice);
            if (currentEmpPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_empPool != address(0), "!_empPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_empPool, INITIAL_EMP_POOL_DISTRIBUTION);
