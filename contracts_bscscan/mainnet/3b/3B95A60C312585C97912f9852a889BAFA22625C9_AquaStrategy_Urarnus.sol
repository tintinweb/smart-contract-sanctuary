...
abstract contract StratX2 is Ownable, ReentrancyGuard, Pausable {
        ...
        returns (uint256)
    {
        ...
        if (isAquaComp) {
            ...
            if(pendingProfit > 0)
            earnAquaProfitsOnWithdraw(_userAddress);
        }
    ...
    function earnAquaProfitsOnWithdraw(address caller) internal  {
        ...
        if(pendingProfit > 0){
            ...
            // uint256[] memory amounts = IPancakeRouter02(planetRouterAddress).getAmountsOut(pendingProfit,earnedToAQUAPath);
            /*Buy AQUA using remaining profit*/
    ...
    function userPendingAquaProfit(address userAddress) public view virtual returns (uint256 pendingProfitInCake,uint256 pendingProfitInAqua){
        ...
        if(pendingProfitInCake > 0){
            uint256[] memory amounts = IPancakeRouter02(planetRouterAddress).getAmountsOut(pendingProfitInCake,earnedToAQUAPath);
            pendingProfitInAqua = amounts[amounts.length - 1];
        }
    ...
    function earnAquaProfits() public nonReentrant whenNotPaused  {
...
contract AquaStrategy_Urarnus is StratX2 {
