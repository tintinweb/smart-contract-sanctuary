...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  mapping(address=>bool) public dogOwnerContracts;
  ...
  function setDogOwnerContract(address _dogOwnerContract, bool allow) external onlyOwner{
      dogOwnerContracts[_dogOwnerContract] = allow;
  }
  /// @notice Distributes ether to token holders as dividends.
  ...
  function distributeDividends() public override payable onlyDogOwner{
  ...
  function distributeDividends(uint256 amount) public override onlyDogOwner {
    require(totalSupply() > 0);
  ...
  function setDividendTokenAddress(address newToken) public onlyOwner {
      require(newToken==address(0),"Already dividend token address set");
  ...
  modifier onlyDogOwner() {
    require(dogOwnerContracts[_msgSender()], "DividendPayingToken: caller is not the dogOwnerContract");
    _;
  }
...
contract DogOwner is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DOGOWNDividendTracker public dividendTracker;

    address public buyBackWallet = 0x0DD56e3d80733db6cb02a7D649dc3EF3656fA089;
    ...
    modifier onlyManager() {
        require(_msgSender() == managerAddress, "Not Manager address");
        _;
    }

    constructor() ERC20("DogOwner", "DOGOWN") {
        
    	dividendTracker = new DOGOWNDividendTracker();
    ...
    function transferManager(address _managerAddress) external onlyManager {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(multiplier >= 100 && multiplier <= 500, "DOGOWN: Sell transaction multipler must be between 100 (1x) and 500 (5x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setTreasuryDivisor(uint256 divisor) external onlyOwner {
  	    require(treasuryDivisor >= 0 && treasuryDivisor <= 100, "DOGOWN: Treasury divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) external onlyOwner {
        require(!swapping, "DOGOWN: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyManager {
        require(newAddress != address(dividendTracker), "DOGOWN: The dividend tracker already has that address");

        DOGOWNDividendTracker newDividendTracker = DOGOWNDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DOGOWN: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyManager {
        require(newFee >= 0 && newFee <= 10, "DOGOWN: Dividend reward tax must be between 0 and 10");
        dividendRewardsFee = newFee;
        totalFees = dividendRewardsFee.add(treasuryFee).add(liquidityFee);
    }
    
    function updateTreasuryFee(uint8 newFee) public onlyManager {
        require(newFee >= 0 && newFee <= 10, "DOGOWN: Dividend reward tax must be between 0 and 10");
        treasuryFee = newFee;
        totalFees = dividendRewardsFee.add(treasuryFee).add(liquidityFee);
    }
    
    function updateLPFee(uint8 newFee) public onlyManager {
        require(newFee >= 0 && newFee <= 10, "DOGOWN: Dividend LP must be between 0 and 10");
        liquidityFee = newFee;
        totalFees = dividendRewardsFee.add(treasuryFee).add(liquidityFee);
    }
    
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DOGOWN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DOGOWN: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DOGOWN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DOGOWN: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "DOGOWN: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DOGOWN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DOGOWN: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (!hasExcluded && 
            ...
            if (!swapping && canSwap) {
                ...
                transferToBuyBackWallet(payable(buyBackWallet), address(this).balance.mul(treasuryDivisor).div(10**2));
                
                uint256 buyBackBalance = address(this).balance;
                if (buyBackEnabled && buyBackBalance > uint256(1 * 10**18)) {
                    swapBNBForTokens(buyBackBalance.mul(rand()).div(10**2));
                }
...
contract DOGOWNDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DOGOWN_Dividend_Tracker", "DOGOWN_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DOGOWN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "DOGOWN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DOGOWN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DOGOWN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DOGOWN_Dividend_Tracker: Cannot update claimWait to same value");
