...
// TODO: remove string from options or add string to initiatives
// TODO: createInitiative should take a list of options, can think if we want to leave createOption public
// TODO: provide API to get GROY staked per initiative
// TODO: provide API to get Vote count per initiative
// TODO: provide API to get Vote count per initiative
...
contract GroyGovernor is Ownable, ReentrancyGuard {
    ...
    function decreaseVote(uint optionId, uint amount) external optionExists(optionId, optionCount) nonReentrant {
        ...
        _updatePool(initiative.endBlock, pool, ActionType.WITHDRAW, amount);
    ...
    function increaseVote(uint optionId, uint amount) external optionExists(optionId, optionCount) nonReentrant {
        ...
        PoolInfo storage pool = poolInfo[initiativeId];
        _updatePool(initiative.endBlock, pool, ActionType.DEPOSIT, amount);
    ...
    function claimRewards(uint initiativeId) external initiativeExists(initiativeId, initiativeCount) nonReentrant {
        ...
        PoolInfo storage pool = poolInfo[initiativeId];
        _updatePool(initiative.endBlock, pool, ActionType.CLAIM_REWARD, user.amount);
     ...
     */
    function _updatePool(uint endBlock, PoolInfo storage pool, ActionType actionType, uint amount) private {
        uint oldTotal = pool.totalVotes;
        ...
        if (block.number <= pool.lastRewardBlock || upperBlock == pool.lastRewardBlock) {
            return;
        }

        uint256 multiplier = upperBlock - pool.lastRewardBlock;
        uint256 reward = multiplier * pool.rewardPerBlock;

        pool.accRewardPerShare += reward * 1e12 / oldTotal;
    ...
    function allowEarlyWithdrawal(uint initiativeId, bool allow) external
        onlyOwner
        initiativeExists(initiativeId, initiativeCount)
    {
        Initiative storage initiative = initiatives[initiativeId];
    ...
    function createInitiative(uint startBlock, uint lockBlock, uint endBlock, address rewardToken, uint rewardAmount) external onlyOwnerOrAnyone nonReentrant {
    ...
    function state(uint initiativeId) external view returns (InitiativeState) {
        Initiative memory initiative = initiatives[initiativeId];
    ...
    function createOption(string memory description, uint initiativeId) external
        onlyOwnerOrAnyone
        initiativeExists(initiativeId, initiativeCount)
    {
        require(creationEnabled, "GroyGovernor: Creation is disabled");
