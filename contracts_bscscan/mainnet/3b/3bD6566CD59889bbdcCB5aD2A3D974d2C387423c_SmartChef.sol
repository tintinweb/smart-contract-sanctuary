...
contract SmartChef is Ownable {
    ...
    function startReward() public onlyOwner {
        ...
        rewardPerBlock = 12000000000000000;
    ...
    function deposit(uint256 _amount, address _referrer) public {
        ...
        require(amountStaked[msg.sender].add(_amount) <= 176144000000000000000000);
        // require (_amount.add(user.amount) <= maxStaking, 'exceed max stake');
        require(pool.lpToken.balanceOf(address(this)).add(_amount) <= 176144000000000000000000, "pool balance too large"); // only allow up to 2 million tokens staked (2 million + 2 million token rewards)
        updatePool(0);
        if (user.amount > 0) {
