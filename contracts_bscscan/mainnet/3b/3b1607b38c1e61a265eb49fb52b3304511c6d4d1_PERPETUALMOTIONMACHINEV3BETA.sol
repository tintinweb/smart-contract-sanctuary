...
contract Ownable is Context {
    address private _owner = 0xF17Ab14285d0E7C8bB29CA71707D646801E10C0e;
...
contract PERPETUALMOTIONMACHINEV3BETA is Context, IBEP20, Ownable { // contract name
    ...
    address payable public _marketingAddress = payable(0xdAAd7fAdcbb24EEe46F28E5a16A83134466e239f); // Marketing Address
    address payable public _burnAddress = payable(0x000000000000000000000000000000000000dEaD); // Burn Address

    string private _name = "Perpetual Motion Machine V3 BETA"; // token name
    string private _symbol = "PMM V3 BETA"; // token symbol
    ...
    uint256 public _buyBackFee = 100; // buyback fee 10%
    ...
    uint256 public burnFee = 10; // burn fee 1%
    uint256 private _previousBurnFee = burnFee; // restore burn fee
	...
	uint256 private _transactionLockTime = 15 minutes; //Cool down time between each transaction per address
    ...
    uint256 public _maxSellTxnPercentage = 100; // max sell per transaction 10%
     ...
     */
    function setMaxSellPercentage(uint256 percentage) external onlyOwner {
        _maxSellTxnPercentage = percentage;
    }
	...
	*/
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 mTFee, uint256 tBurn, uint256 tLiquidity) = _getTValues(tAmount);
        return (tTransferAmount, mTFee, tBurn, tLiquidity);
    }
     ...
     */
    function _sendToMarketing(address sender, uint256 amount) private {
        if(amount > 0) {// No need to send if collected marketing token fee is zero
            ...
            emit Transfer(sender, _marketingAddress, amount);
        }
    }

    /**  
     * @dev take's burn fee tokens from tansaction and send to burn address
     */
    ...
    function removeAllFee() private {
        if(_deductableFee == 0 && _tokenFee == 0 && _buyBackFee == 0
           && _marketingBNBFee == 0 && _liquidityFee == 0 && burnFee == 0) return;
    ...
    function _transfer(
        ...
        if(!_isExcludedFromTransactionlock[from] && to == pancakePair)
            require(block.timestamp >= _transactionCheckpoint[from] + _transactionLockTime,
            "Wait for transaction cooldown time to end before making a tansaction");
        if(!_isExcludedFromMaxTxAmount[from] && to == pancakePair)
            require(amount <= (balanceOf(from).mul(_maxSellTxnPercentage).div(1000)), "Transfer amount exceeds the max Sell Amount.");
        ...
        {
            uint256 buyBackAmount = address(this).balance.div(33);
        ...
        if(contractTokenBalance >= _tTotal.div(1000))
        {
            contractTokenBalance = _tTotal.div(1000);
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount,bool takeFee) private {
        ...
        (uint256 tTransferAmount, uint256 mTFee, uint256 tBurn, uint256 tLiquidity) = _getValues(tAmount);
