...
contract IslamicGoldDinar is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 2200000 * 10**3 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public maxSellAmount = 2200000 * 10**3 * 10**18;
    uint256 public swapTokensAtAmount = 10 * 10**3 * 10**18;
    uint256 public buyBackUpperLimit = 1 * 10**18;

    address public marketingsAddress = 0xd8492F6969f4d71424A084e5CCa43234e92505B1;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "Islamic Gold Dinar";
    string private constant _symbol = "IGD";
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[marketingsAddress] = true;
    ...
    function setFeeRates(uint256 _rfi, uint256 _marketings, uint256 _liquidity) external onlyOwner {
        feeRates.rfi = _rfi;
        feeRates.marketings = _marketings;
        feeRates.liquidity = _liquidity;
        emit FeesChanged();
    }

    function setSellFeeRates(uint256 _rfi, uint256 _marketings, uint256 _liquidity) external onlyOwner{
        sellFeeRates.rfi = _rfi;
        sellFeeRates.marketings = _marketings;
    ...
    function _takeMarketing(uint256 rMarketing, uint256 tMarketing) private {
        totFeesPaid.marketings +=tMarketing;
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tMarketing = tAmount*sellFeeRates.marketings/1000;
            ...
            s.tMarketing = tAmount*feeRates.marketings/1000;
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 denominator= (feeRates.liquidity + feeRates.buyback + feeRates.marketings) * 2;
        ...
        // Send BNB to marketingsWallet
        uint256 marketingsAmt = unitBalance * 2 * feeRates.marketings;
        if(marketingsAmt > 0){
          payable(marketingsAddress).transfer(marketingsAmt);
        }
    }
    ...
    function updateMarketingWallet(address newWallet) external onlyOwner{
        require(marketingsAddress != newWallet ,'Wallet already set');
        marketingsAddress = newWallet;
        _isExcludedFromFee[marketingsAddress];
    }
