...
BabyPyram Tokenomics
7% Rewards
7% LP

Telegram: BabyPyram_BSC
*/
...
// File: contracts/BabyPyramDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable DividendsToken = address(0xedeCfB4801C04F3EB394b89397c6Aafa4ADDa15B);
...
contract BabyPyram is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BabyPyramDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("BabyPyram", "BabyPyram") {
    	...
    	dividendTracker = new BabyPyramDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyPyram: The dividend tracker already has that address");

        BabyPyramDividendTracker newDividendTracker = BabyPyramDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyPyram: The new dividend tracker must be owned by the BabyPyram token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyPyram: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyPyram: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyPyram: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyPyram: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyPyram: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyPyram: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyPyram: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BabyPyram: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of BabyPyram
            path,
            recipient,
            block.timestamp
        );
...
contract BabyPyramDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BabyPyram_Dividend_Tracker", "BabyPyram_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "BabyPyram_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BabyPyram_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyPyram contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyPyram_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyPyram_Dividend_Tracker: Cannot update claimWait to same value");
