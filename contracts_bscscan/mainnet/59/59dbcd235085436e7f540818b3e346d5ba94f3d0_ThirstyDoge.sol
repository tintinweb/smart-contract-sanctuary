 ...
 * that a supply mechanism has to be added in a derived contract using {_create}.
...
contract ERC20 is Context, IERC20 {
     ...
     * XRP and Wei. This is the value {ERC20} uses, unless {_setupDecimals} is
     * called.
     *
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  /// @dev Internal function that creates tokens to an account.
  /// Update magnifiedDividendCorrections to keep dividends unchanged.
...
contract ThirstyDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool public swapandliquifyIsEnabled = false;
    ...
    constructor() ERC20("Thirsty Doge", "$TDOGE") {
    	bnbDividendTracker = new BnbDividendTracker();
    	dogeDividendTracker = new DogeDividendTracker();

    	marketingWallet = 0x3C5F4f58986c9000D17F70E60763b6CC5BEC4524;
    	charityWallet = 0x1Ff3323007A3a4038232027EdE4356E0519DB74D;
        ...
        */
        _create(owner(), 1000000000000000 * (10**9));
    }
    ...
    receive() external payable {
  	...
  	function updateCharityWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != charityWallet, "ThirstyDoge: The charity wallet is already this address");
        excludeFromFees(_newWallet, true);
        emit MarketingWalletUpdated(charityWallet, _newWallet);
  	    charityWallet = _newWallet;
  	}
  	
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "ThirstyDoge: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function afterPreSale() external onlyOwner {
        ...
        maxBuyTranscationAmount = 1000000000000000 * (10**9);
        maxSellTransactionAmount = 1000000000000000 * (10**9);
        maxWalletToken = 1000000000000000 * (10**9);
    }
    
    function setSwapandliquifyIsEnabled(bool _enabled) external onlyOwner {
        swapandliquifyIsEnabled = _enabled;
    }
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(dogeDividendRewardsFee).add(bnbDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setBnbDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = bnbDividendRewardsFee.add(marketingFee).add(dogeDividendRewardsFee).add(buyBackAndLiquidityFee);
            bnbDividendEnabled = _enabled;
        }
    ...
    function setDogeDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = dogeDividendRewardsFee.add(marketingFee).add(bnbDividendRewardsFee).add(buyBackAndLiquidityFee);
            dogeDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(dogeDividendRewardsFee).add(bnbDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateBnbDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(bnbDividendTracker), "ThirstyDoge: The dividend tracker already has that address");

        BnbDividendTracker newBnbDividendTracker = BnbDividendTracker(payable(newAddress));

        require(newBnbDividendTracker.owner() == address(this), "ThirstyDoge: The new dividend tracker must be owned by the ThirstyDoge token contract");
    ...
    function updateDogeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dogeDividendTracker), "ThirstyDoge: The dividend tracker already has that address");

        DogeDividendTracker newDogeDividendTracker = DogeDividendTracker(payable(newAddress));

        require(newDogeDividendTracker.owner() == address(this), "ThirstyDoge: The new dividend tracker must be owned by the ThirstyDoge token contract");
    ...
    function updateDogeDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = dogeDividendRewardsFee.add(bnbDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }
    
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        marketingFee = newFee;
        totalFees = marketingFee.add(bnbDividendRewardsFee).add(dogeDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(bnbDividendRewardsFee).add(dogeDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "ThirstyDoge: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "ThirstyDoge: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ThirstyDoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "ThirstyDoge: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "ThirstyDoge: Cannot update gasForProcessing to same value");
	...
	function processDividendTracker(uint256 gas) external onlyOwner {
		(uint256 ethIterations, uint256 ethClaims, uint256 ethLastProcessedIndex) = bnbDividendTracker.process(gas);
		emit ProcessedBnbDividendTracker(ethIterations, ethClaims, ethLastProcessedIndex, false, gas, tx.origin);
		
		(uint256 dogeBackIterations, uint256 dogeBackClaims, uint256 dogeBackLastProcessedIndex) = dogeDividendTracker.process(gas);
		emit ProcessedDogeDividendTracker(dogeBackIterations, dogeBackClaims, dogeBackLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function _transfer(
        ...
        require(swapandliquifyIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "ThirstyDoge: Swapandliquify has not started yet");
        
        bool excludedAccount = isExcludedFromFees[from] || isExcludedFromFees[to];
        
        if (
            swapandliquifyIsEnabled &&
            ...
            if (!swapping && canSwap) {
                ...
                if (marketingEnabled) {
                    ...
                    uint256 charityPortion = address(this).balance.div(10**2).mul(50);
                    uint256 marketingPortion = address(this).balance.sub(charityPortion);
                ...
                if (bnbDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(bnbDividendRewardsFee.add(dogeDividendRewardsFee)).mul(bnbDividendRewardsFee);
                    swapAndSendBnbDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (dogeDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(bnbDividendRewardsFee.add(dogeDividendRewardsFee)).mul(dogeDividendRewardsFee);
        ...
        bool takeFee = swapandliquifyIsEnabled && !swapping && !excludedAccount;
...
contract BnbDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("ThirstyDoge_Bnbereum_Dividend_Tracker", "ThirstyDoge_Bnbereum_Dividend_Tracker", 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 200000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ThirstyDoge_Bnbereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ThirstyDoge_Bnbereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ThirstyDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "ThirstyDoge_Bnbereum_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DogeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("ThirstyDoge_Doge_Dividend_Tracker", "ThirstyDoge_Doge_Dividend_Tracker", 0xbA2aE424d960c26247Dd6c32edC70B295c744C43) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 200000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ThirstyDoge_Doge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ThirstyDoge_Doge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ThirstyDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "ThirstyDoge_Doge_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
