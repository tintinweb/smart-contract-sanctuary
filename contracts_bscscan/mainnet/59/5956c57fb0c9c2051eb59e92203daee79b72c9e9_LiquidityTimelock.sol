...
contract LiquidityTimelock is Ownable {
    ...
    // timestamp 11/11/2022 token release is enabled
    uint256 public releaseTime = 1632164955;
    
    address public FlokiToken = 0x9F5f757512aE04a8b6CBF60cDa4104Ec6Bc539E6;
    address public lpToken = 0x50F00fa5F0e0B62c10C5Ca9eBBe7A936E5FC99c4;

    /**
     * @notice Transfers tokens held by timelock to owner.
     */
    function releaseFlokiToken() external onlyOwner {
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp >= releaseTime, "TokenTimelock: current time is before release time");

        uint256 amount = IERC20(FlokiToken).balanceOf(address(this));
        require(amount > 0, "TokenTimelock: no tokens to release");

        IERC20(FlokiToken).safeTransfer(owner(), amount);
    }
    ...
    function getFlokiTokenBalance() external view returns(uint256) {
        return IERC20(FlokiToken).balanceOf(address(this));
    }
    ...
    function setFlokiToken(address _FlokiToken) external onlyOwner {
        FlokiToken = _FlokiToken;
    }
