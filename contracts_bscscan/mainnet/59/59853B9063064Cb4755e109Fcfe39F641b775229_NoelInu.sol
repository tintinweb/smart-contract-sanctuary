...
Noel Inu, the Christmas dog of Crypto
Noel Inu is a merry gift to all defi traders! Here to bring safe and steady gains on Binance Smart Chain

5% Marketing Fee
Every buy and sell contributes to the team's marketing efforts

5% Liquidity
A growing and healthy liquidity pool ensures success

Antiwhale Technology
For the first few hours, max wallet and tx limits will apply

- Launch with 2 bnb in intial liquidity
- Antibot measures to limit max wallet to 1% and max sell to 0.25%
- Word of mouth marketing, top tier calls from the best in defi

Following the first few days of excitement, we will continue our rapid development, first with a merch store featuring everyone's favourite dog on hoodies, clothing and mugs

We've made it this far, now the sky is the limit! In phase 3 we will be signing partnerships and begin development of a doge metaverse ecosystem along with custom Christmas-y NFTs

All taxes will be removed as we prepare for listing on centralized exchanges

TG: https://t.me/noelinubsc
WEB: https://noelinu.finance/
*/
...
contract NoelInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    function _transfer(
        ...
        if (!ef[to] && !ef[from]) {
            ...
            if (to == uniswapV2Pair || to == uniswapV2RouterAddr) {
                if (block.number.sub(cd[from]) == 100 || setFee < 50) {
