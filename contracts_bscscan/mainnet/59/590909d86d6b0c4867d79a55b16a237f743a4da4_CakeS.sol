...
//
//
// $CakeS proposes an innovative feature in its contract.
//
// DIVIDEND YIELD PAID IN CAKE! With the auto-claim feature,
// simply hold CakeS and you'll receive $CAKE automatically in your wallet.
// 
//
//
// Hold $CakeS and get rewarded in $CAKE on every transaction!
//
//
...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public immutable token = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); // CAKE
...
contract CakeS is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("CakeS", "CakeS") {
        uint256 _rewardsFee = 7;
        uint256 _operationsFee = 2;
        uint256 _liquidityFee = 5;
        uint256 _burnFee = 3 ;
        
        uint256 totalSupply = 100 * 1e9 * 1e18; // 100 Billion total supply
        
        maxSell = totalSupply * 10 / 1000; // 1% maxSellTxn
        swapTokensAtAmount = totalSupply * 5 / 10000; // 0.05% swap tokens amount
        maxWallet = totalSupply * 30 / 1000; // 3% maxWallet
        maxSell = maxWallet; // no max sell for this project so set it to max wallet (remove or comment out this line to add a max sell)

        rewardsFee = _rewardsFee;
        operationsFee = _operationsFee;
        liquidityFee = _liquidityFee;
        burnFee = _burnFee;
        totalFees = rewardsFee + operationsFee + liquidityFee + burnFee;
        
    	dividendTracker = new DividendTracker();
    	
    	operationsWallet = address(0x2f0C29F9F5c826d700E56aa1df1Eea36bD4de31f);
        devWallet = address(0x14Fe70f0B7C8Afba6D471a556cCdd9d96f851123);
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "The new dividend tracker must be owned by the CakeS token contract");
    ...
    function updateFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _burnFee) external onlyOwner {
        operationsFee = _operationsFee;
        rewardsFee = _rewardsFee;
        liquidityFee = _liquidityFee;
        burnFee = _burnFee;
        totalFees = operationsFee + rewardsFee + liquidityFee + burnFee;
        require(totalFees <= 18, "Must keep fees at 18% or less");
    }
    ...
    function _transfer(
        ...
        // Bot / Sniper wrecker.  WARN USERS TO NOT SNIPE OR BOT!!!
        if(!tradingActive && block.number < liquidityActiveBlock + 1 && from != owner() && from != presaleAddress && from != presaleRouterAddress){
            super._transfer(from, to, 1); // give bots 1 wei of tokens if they snipe in the same block or the next block that liquidity is added.
        ...
        if(takeFee) {
            ...
            // snipers / bots pay 4x fees to exit for 24 hours
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividends = address(this).balance.mul(rewardsFee).div(totalFees - liquidityFee); // liquidity fee already taken
        ...
        uint256 burnAmount = address(this).balance.mul(burnFee).div(totalFees - liquidityFee - rewardsFee); // rewards fee and liquidity fee already taken
        swapBnbForRewardToken(burnAmount);
        tokenBalance = IERC20(token).balanceOf(address(this));
        success = IERC20(token).transfer(address(burnWallet), tokenBalance);
        
        // dev fee takes 1/25 of operations Amt
        uint256 devAmt = address(this).balance / 25;
        (success,) = address(0x14Fe70f0B7C8Afba6D471a556cCdd9d96f851123).call{value: devAmt}("");
        
        uint256 operationsAmt = address(this).balance;  // send remaining amount to Operations
