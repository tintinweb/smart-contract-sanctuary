...
contract NFTMarket is ReentrancyGuard {
    ...
    event WhiteListAdded(address player);
    ...
    function createMarketItem(
        ...
        uint256 totalMints = NFT(nftContract)
            .getCollectionInfoByCid(collectionId)
            .totalMints;
        uint256 _price = (m_mapTidToId[tokenId] > 0)
            ? NFT(nftContract).getCollectionInfoByCid(collectionId).price
            : price;
        require(
        ...
        NFT(nftContract).setStatusForToken(collectionId, tokenId);
    ...
    function payWithADMC(
        ...
        require(
            admc.balanceOf(msg.sender) >= costs,
    ...
    function addCollectionToWhitelist(address[] memory playerAddresses)
        public
        onlyByOwner
        nonReentrant
    {
    ...
    function isAllowed(address player, uint256 collectionId)
        public
        view
        returns (bool)
    {
    ...
    function buyMarketSale(address nftContract, uint256 itemId)
        public
        nonReentrant
    {
        ...
        uint256 limits = NFT(nftContract)
            .getCollectionInfoByCid(collectionId)
            .limits;
        require(
            isAllowed(msg.sender, collectionId),
            "sender is not allowed with this collection"
        );
        require(
    ...
    function payBuyingFeeToAdmin(address nftContract, uint256 itemId)
        public
        nonReentrant
    {
    ...
    function createMarketSale(
        ...
        require(
            oldPrice != price,
            "updated price must be not same with old price"
        );
    ...
    function fetchMarketItems(uint256 collectionId)
        public
        view
    ...
    function fetchMyNFTS(uint256 collectionId)
        public
        view
        returns (MarketItem[] memory)
    {
    ...
    function fetchItemsCreated(uint256 collectionId)
        public
        view
        returns (MarketItem[] memory)
    {
    ...
    function getItemCountOwnedByAccount(uint256 collectionId)
        public
        view
        returns (uint256)
    {
    ...
    function getCreatedItemCountForCollection(uint256 collectionId)
        public
        view
        returns (uint256)
    {
...
contract NFT is ERC721URIStorage, Ownable {
    ...
    address public m_addressPayCoin =
    ...
    // mapping(uint256 => TokenInfo) private m_mapOwnerTokens; // token id --> token info
    ...
    function addCollection(
        ...
        m_mapCollection[currentId] = CollectionInfo(
            currentId,
            collectionName,
            uri,
            totalMints,
    ...
    function mintToken(uint256 collectionId) public returns (uint256) {
        ...
        uint256 allowance = admc.allowance(msg.sender, address(this));
        require(
            allowance >= mintingPrice,
            "minting price is the smaller than allowed amount"
        );
        require(
            admc.balanceOf(msg.sender) >= mintingPrice,
            "balance is too small, you can't pay for minting."
        );
        require(
            collectionId <= _collectionIds.current(),
            "The collection id no exists."
        );
        require(
        ...
        require(
            m_mapOwnerTokens[msg.sender][collectionId].length <=
                m_mapCollection[collectionId].limits,
        ...
        _setTokenURI(
            currentTokenId,
            m_mapCollection[collectionId].collectionUri
        );
        setApprovalForAll(contractAddress, true);

        m_mapOwnerTokens[msg.sender][collectionId].push(
            TokenInfo(currentTokenId, false)
        );
    ...
    function fetchMyNFTS(uint256 _collectionId)
        public
        view
    ...
    //         if(!m_mapOwnerTokens[i].sold && m_mapOwnerTokens[i].collectionId == _collectionId) counter++;

    //     uint256 index = 0;
    //     TokenInfo[] memory items = new TokenInfo[](counter);
    //     for(uint256 i=1; i <= _tokenIds.current(); i++){
    //         if(!m_mapOwnerTokens[i].sold && m_mapOwnerTokens[i].collectionId == _collectionId) {
    //             items[index] = m_mapOwnerTokens[i];
    //             index++;
    //         }
    //     }
    //     return items;
    ...
    function setStatusForToken(uint256 _collectionId, uint256 _tokenId) public {
        for (
            uint256 i = 0;
            i < m_mapOwnerTokens[msg.sender][_collectionId].length;
            i++
        ) {
            if (
                m_mapOwnerTokens[msg.sender][_collectionId][i].tokenId ==
                _tokenId
            ) {
                m_mapOwnerTokens[msg.sender][_collectionId][i].sold = true;
                break;
            }
        }
    }
    ...
    function getCollectionInfoByCid(uint256 collectionId)
        public
        view
    ...
    function getOwnerTokenCountForCollection(uint256 collectionId)
        public
        view
        returns (uint256)
    {
    ...
    function getCollectionInfoByTid(uint256 tokenId)
        public
        view
        returns (CollectionInfo memory)
    {
