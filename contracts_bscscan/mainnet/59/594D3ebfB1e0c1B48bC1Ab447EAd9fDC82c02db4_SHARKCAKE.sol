...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 minPeriod = 1 hours;
    uint256 minDistribution = 1 * (10 ** 18);

    uint256 depositPeriod = 20 seconds;
    uint lastDeposit;
    ...
    function deposit() external payable override onlyToken {
        if (block.timestamp < lastDeposit.add(depositPeriod)) {
            amountToDeposit.add(msg.value);
        ...
        router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amountToDeposit == 0 ? msg.value : amountToDeposit.add(msg.value)}(
            0,
            path,
            address(this),
            block.timestamp
