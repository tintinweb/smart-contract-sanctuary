...
contract Card is IERC721Receiver {
    ...
    function restIn(uint256 tokenId, uint256 amount) public onlyActivate {
        require(nft.ownerOf(tokenId) == msg.sender, "Token not belong to you");
        require(amount >= 30 * 10 ** 8 * 10 ** nftf.decimals(), "Amount must great than 3");
        ...
        _tokenInfo[tokenId].amount = uint160(amount.mul(92).div(100));
    }
    ...
    // 查询锁定的金额
    function restAmount(uint256 tokenId) public view returns (uint160, uint16){
        TokenInfo memory tf = _tokenInfo[tokenId];
        if (tf.amount == 0) {
            return (0, 0);
        }
        uint160 total = tf.amount;
        uint16 totalTimes;
    ...
    function restTake(uint256 tokenId) public onlyActivate {
        require(nft.ownerOf(tokenId) == msg.sender, "Token not belong to you");
        (uint160 amount,) = restAmount(tokenId);
    ...
    function _dayRate(uint256 tokenId, uint16 _day, uint160 _lastAmount) private view returns (uint160){
        ...
        return _lastAmount * (100 + rate) / 100;
    }
