...
import "../interfaces/IBlockMonsters.sol";
import "../libraries/StarterChoice.sol";

contract MonsterSale is Whitelisted {
    ...
    bool public bnbIsLocked = false;

    uint256 public thresholdForSpecial = 75e16;

    uint256 public totalContributedAmount;
    uint256 public hardcap = 500e18;

    mapping(address => Order) private orders;

    struct Order {
        uint256 contributedAmount;
        StarterChoice starterBlockMonster;
        bool hasAlreadyChosen;
    }

    event MonsterSaleInitialized(uint256 startDate, uint256 endDate);
    event JourneyStarted(address buyer, StarterChoice starterChoice);
    event MysteryMint(address buyer);

    modifier isActive() {
        require(block.timestamp > startDate, "MonsterSale: Sale has not yet started");
        require(block.timestamp < endDate, "MonsterSale: Sale is over");
        _;
    }

    modifier isValidPurchase(uint256 _amount) {
        require(_amount >= minCommitment && _amount <= maxCommitment, "MonsterSale: Input amount is either too high or too low");
        require(orders[_msgSender()].contributedAmount.add(_amount) <= maxCommitment, "MonsterSale: maximum contribution reached");
        require(totalContributedAmount.add(_amount) <= hardcap, "MonsterSale: hardcap reached");
        _;
    }
    ...
    modifier hasValidParameters(
        ...
        require(_startDate > block.timestamp && _endDate > block.timestamp, "MonsterSale: Start and ending have to be in the future.");
        require(_startDate < _endDate, "MonsterSale: Starting date cannot be past end.");
        require(_minCommitment < _maxCommitment, "MonsterSale: Invalid amounts.");
        _;
    }
    ...
    function initializeMonsterSale(
        ...
        require(isInitialized == false, "MonsterSale: Pre-sale has already been initialized.");
    ...
    function buy(StarterChoice _starterChoice) external payable isActive isValidPurchase(msg.value) onlyWhitelist {
        ...
        if(!orders[buyer].hasAlreadyChosen) {
            orders[buyer].hasAlreadyChosen = true;
            orders[buyer].starterBlockMonster = _starterChoice;
        }

        emit JourneyStarted(buyer, _starterChoice);
    }

    function claim() external onlyWhitelist {
        address blockMonTrainer = _msgSender();
        require(address(nft) != address(0), "MonsterSale: No BlockMonster NFT set");
        require(address(token) != address(0), "MonsterSale: No BlockMonster Token set");
        require(canClaim == true, "MonsterSale: Claiming not yet activated");
        require(orders[blockMonTrainer].contributedAmount > 0, "MonsterSale: Nothing to claim");
        
        nft.mintStarterBlockMon(blockMonTrainer, orders[blockMonTrainer].starterBlockMonster);
        uint256 amountTokensToTransfer = orders[blockMonTrainer].contributedAmount.mul(privateSaleRate);
        token.transfer(blockMonTrainer, amountTokensToTransfer);
    }

    function claimSpecial() external onlyWhitelist {
        address blockMonTrainer = _msgSender();
        require(orders[blockMonTrainer].contributedAmount >= thresholdForSpecial, "MonsterSale: not possible");
        nft.mintPrivateSaleSpecial(blockMonTrainer);
        emit MysteryMint(blockMonTrainer);
    }

    function activateClaim() external onlyOwner {
        require(canClaim == false, "MonsterSale: Redeem already activated");
        canClaim = true;
    }
...
enum StarterChoice { Petaliz, Orcalf, Flamisire }
