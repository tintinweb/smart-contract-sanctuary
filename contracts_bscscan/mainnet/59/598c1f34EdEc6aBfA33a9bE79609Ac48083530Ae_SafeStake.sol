...
contract SafeStake is IReflectionPool, Ownable, Pausable {
    ...
    constructor (address _router, IBEP20 _rewardsToken, SafeEarn _stakingToken) {
    ...
    function enterStaking(uint256 amount) external whenNotPaused {
        ...
        if (address(tokenPool.stakingToken) == address(safeEarn)) {
            ...
            if (!lockerExists) {
                ...
                locker = new ReflectionLocker02(msg.sender, safeEarn, safeEarn.distributorAddress());
            ...
            //            tokenPool.stakingToken.transferFrom(address(this), address(lockers[msg.sender]), amount);
        }
...
//        }
    ...
    function setSwapperExcluded(address _add, bool _excluded) external onlyOwner {
    ...
    event EnterStaking(uint);
    event LeaveStaking(uint, uint);
    event Harvest(uint);
}
...
contract ReflectionLocker02 is IReflectionLocker {
    ...
    IBEP20 safeMoon = IBEP20(0x8076C74C5e3F5852037F31Ff0093Eeb8c8ADd8D3);
//    IBEP20 safeEarn = IBEP20(0x099f551eA3cb85707cAc6ac507cBc36C96eC64Ff);
    IBEP20 safeEarn = IBEP20(0xb227e7fff9883F99E51613bc121c36361A697Ae9);
    ...
    function _getFromDistributor() internal {
        try distributor.claimDividend() {
            ...
            revert("Error getting from distributor");
        }
    }
