...
contract VodkaIDO is Ownable {
    ...
    // REF PROGRAM
    bool public refRewardsActive = false;
    ...
    constructor(VodkaIDO IDO_OLD) {
        STK = IDO_OLD.STK();
        STK_OLD = IDO_OLD.STK_OLD();
        BUSD = IDO_OLD.BUSD();
        TKN = IDO_OLD.TKN();

        uint256 plength = IDO_OLD.getParticipantsLength();
        for (uint256 i = 0; i < plength; i++) {
            address _u = IDO_OLD.participants(i);
            User storage user = users[_u];

            participants.push(_u);
            uint256 tickets = 1; // Only this time it was true by coincidence
            user.hasRegistered = true;
            user.ticketCount = tickets;
        }
        list = IDO_OLD.getTicketsList();
    }
    ...
    function claim() public {
        ...
        require(user.claimed < TKN_AMOUNT && user.claimed != 1, 'you have claimed already');
        if (user.isWinner) {
            ...
            require(amountToClaim > 0, "nothing to claim yet");
            TKN.transfer(msg.sender, amountToClaim);
            user.claimed += amountToClaim;
            if (user.claimed == amountToClaim) _distributeRefRewards(BUSD_AMOUNT, msg.sender);
        } else {
    ...
    function take(IERC20 token, uint256 amount) public restricted {
        require(
            (block.timestamp >= (DRAW_DATE + VESTING_FREEZE + VESTING_PERIOD)) || EMERGENCY_ALLOWED,
            'Vesting period must finish first'
        );
        token.transfer(msg.sender, amount > 0 ? amount : token.balanceOf(address(this)));
    }

    function takeBUSD(uint256 amount) public restricted {
        BUSD.transfer(msg.sender, amount > 0 ? amount : BUSD.balanceOf(address(this)));
    }
