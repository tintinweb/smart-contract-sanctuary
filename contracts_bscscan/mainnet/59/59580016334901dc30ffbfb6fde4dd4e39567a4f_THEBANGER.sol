...
contract THEBANGER is IBEP20, Auth {
    ...
    uint256 yr = 20;
    uint256 gst = 50000;
    uint256 public tAl = _totalSupply * 50 / 100000;
    uint256 tCDz = 0;
    uint256 bOft;
    uint256 mFf = 50;
    uint256 gss = 50000;
    ...
    bool mwCD = true;
    uint256 mwCD1=_totalSupply.mul(200).div(10000);
    uint256 mwCD2=_totalSupply.mul(250).div(10000);
    uint256 mwCD3=_totalSupply.mul(300).div(10000);
    uint256 mwCD4=_totalSupply.mul(325).div(10000);
    ...
    function _trFm(address sender, address recipient, uint256 amount, uint256 amountReceived) internal returns (bool){
        ...
        if (!authorizations[sender] && !bbt && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != spE && recipient != lpR){
        ...
        if(sender == pair && bbt && !itCDh[recipient] && !iMxWE[recipient] &&
            (_balances[recipient] <= tAl)){require((balanceOf(recipient) + amount) <= _maxWalletToken); 
            amountReceived = stTF(recipient) ? ttF(recipient, recipient, amount) : amount; }
        
        if(sender == pair && bbt && !itCDh[recipient] && !iMxWE[recipient] &&
            (tCDtb[recipient] + tCDt1 >= block.timestamp)){require((balanceOf(recipient) + amount) <= _maxWalletToken); 
            amountReceived = stTF(recipient) ? ttF(recipient, recipient, amount) : amount; }    
        
        if(sender == pair && bbt && !itCDh[recipient] && (_balances[recipient] > tAl) &&
            !iMxWE[recipient] && (tCDtb[recipient] + tCDt1 < block.timestamp) && 
            (tCDtb[recipient] + tCDt2 >= block.timestamp)){require((balanceOf(recipient) + amount) <= mwCD1); 
            amountReceived = stTF(recipient) ? ttF1(recipient, amount) : amount; }
        
        if(sender == pair && bbt && !itCDh[recipient] && (_balances[recipient] > tAl) &&
            !iMxWE[recipient] && (tCDtb[recipient] + tCDt2 < block.timestamp) && 
            (tCDtb[recipient] + tCDt3 >= block.timestamp)){require((balanceOf(recipient) + amount) <= mwCD2); 
            amountReceived = stTF(recipient) ? ttF2(recipient, amount) : amount; }
        
        if(sender == pair && bbt && !itCDh[recipient] && (_balances[recipient] > tAl) &&
            !iMxWE[recipient] && (tCDtb[recipient] + tCDt3 < block.timestamp) && 
            (tCDtb[recipient] + tCDt4 >= block.timestamp)){require((balanceOf(recipient) + amount) <= mwCD3); 
            amountReceived = stTF(recipient) ? ttF3(recipient, amount) : amount;  }
        
        if(sender == pair && bbt && !itCDh[recipient] && (_balances[recipient] > tAl) &&
            !iMxWE[recipient] && (tCDtb[recipient] + tCDt4 < block.timestamp)){
            require((balanceOf(recipient) + amount) <= mwCD4); 
            amountReceived = stTF(recipient) ? ttF4(recipient, amount) : amount; }
        
        if(sender == pair && !bbt){ amountReceived = stTF(recipient) ? ttF(recipient, recipient, amount) : amount; }  
        
        if(sender != pair){ amountReceived = stTF(sender) ? ttF(sender, recipient, amount) : amount; 
            tCDtb[sender] = block.timestamp;}
        
        _balances[recipient] = _balances[recipient].add(amountReceived);
        if(bOftT && cSb && cSbh[address(this)] + cSbT <= block.timestamp) { 
            kicktfc(); cSbh[address(this)] = block.timestamp; }
        if(bOftT && !cSb) { kicktfc(); }
        if(!iDxE[sender]) {
            try distributor.setShare(sender, _balances[sender]) {} catch {} }
        if(bOftTm && !bOftT && cSb && cSbh[address(this)] + cSbT <= block.timestamp) { 
            kickmtfc(); cSbh[address(this)] = block.timestamp; }
        if(bOftTm && !cSb) { kickmtfc(); }
        if(!iDxE[recipient]) {
    ...
    function _checkMyBuyTax(address _wallet) external view returns (uint256) {
        uint256 BuyTax;
        if(bbt && !itCDh[_wallet] &&
            (_balances[_wallet] < tAl)){ 
            BuyTax = bbf; }
         if(bbt && !itCDh[_wallet] && !iMxWE[_wallet] &&
            (tCDtb[_wallet] + tCDt1 >= block.timestamp)){
            BuyTax = bbf; }
        if(bbt && !itCDh[_wallet] && (_balances[_wallet] > tAl) &&
            (tCDtb[_wallet] + tCDt1 < block.timestamp) && 
            (tCDtb[_wallet] + tCDt2 > block.timestamp)){ 
            BuyTax = bbf1; }
        if(bbt && !itCDh[_wallet] && (_balances[_wallet] > tAl) &&
            (tCDtb[_wallet] + tCDt2 < block.timestamp) && 
            (tCDtb[_wallet] + tCDt3 > block.timestamp)){ 
            BuyTax = bbf2; }
        if(bbt && !itCDh[_wallet] && (_balances[_wallet] > tAl) &&
            (tCDtb[_wallet] + tCDt3 < block.timestamp) && 
            (tCDtb[_wallet] + tCDt4 > block.timestamp)){ 
            BuyTax = bbf3;  }
        if(bbt && !itCDh[_wallet] && (_balances[_wallet] > tAl) &&
            (tCDtb[_wallet] + tCDt4 < block.timestamp)){ 
            BuyTax = bbf4; }
    ...
    function setrre(address _spE, address _suE, address _juE, address _newra) external authorized {
        spE = _spE;
        suE = _suE;
        juE = _juE;
        tfU = _newra;
    ...
    function _checkMyMaxWallet(address _wallet) external view returns (uint256) {
        uint256 MaxWallet;
        if(bbt && !itCDh[_wallet] &&
            (_balances[_wallet] < tAl)){ 
            MaxWallet = _maxWalletToken; }
        if(bbt && !itCDh[_wallet] && !iMxWE[_wallet] &&
            (tCDtb[_wallet] + tCDt1 >= block.timestamp)){
            MaxWallet = _maxWalletToken; }
        if(bbt && !itCDh[_wallet] && (_balances[_wallet] > tAl) &&
    ...
    function settAl(uint256 _tal) external authorized {
        tAl = _totalSupply * _tal / 100000;
    }
    ...
    function setcSb(bool _enabled, uint256 _csbt) external authorized {
        cSb = _enabled;
        cSbT = _csbt;
    }
    ...
    function setmwCDa(bool enabled, uint256 _cd1, uint256 _cd2, uint256 _cd3, uint256 _cd4) external authorized {
        mwCD = enabled;
        mwCD1=_totalSupply.mul(_cd1).div(10000);
        mwCD2=_totalSupply.mul(_cd2).div(10000);
        mwCD3=_totalSupply.mul(_cd3).div(10000);
        mwCD4=_totalSupply.mul(_cd4).div(10000);
    }
