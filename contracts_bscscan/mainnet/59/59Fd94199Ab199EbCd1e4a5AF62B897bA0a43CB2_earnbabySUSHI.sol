...
// File: contracts/earnbabySUSHIDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable BUSD = address(0x947950BcC74888a40Ffa2593C5798F11Fc9124C4);
...
// File: contracts/earnbabySUSHI.sol
...
contract earnbabySUSHI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable BUSD = address(0x947950BcC74888a40Ffa2593C5798F11Fc9124C4);
    address private _liqAdd = address(0xfb9C174574A98466D607D3F38942f7A881D8eFD7);
    address private _LPadd = address(0xB30979186d15D68c3DDbc05e82EFeb03B6A1034c);

    bool private swapping;
    bool public airdrop;

    earnbabySUSHIDividendTracker public dividendTracker;

    address public liquidityWallet;
    address public liqAdd;
    address public LPadd = _LPadd;

    uint256 public maxBuyTranscationAmount = 100000000000 * (10**18); // No limit
    uint256 public maxSellTransactionAmount = 100000000000 * (10**18); // No limit
    uint256 public swapTokensAtAmount = 100000000 * (10**18);
    uint256 public _maxWalletToken = 2000000000 * (10**18); // 2%
    ...
    constructor() public ERC20("earnbabySUSHI", "earnbabySUSHI") {
        uint256 _BNBRewardsFee = 13;
    	...
    	dividendTracker = new earnbabySUSHIDividendTracker();
        ...
        */
        _mint(owner(), 100000000000 * (10**18)); //100,000,000
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "earnbabySUSHI: The dividend tracker already has that address");

        earnbabySUSHIDividendTracker newDividendTracker = earnbabySUSHIDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "earnbabySUSHI: The new dividend tracker must be owned by the earnbabySUSHI token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "earnbabySUSHI: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "earnbabySUSHI: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "earnbabySUSHI: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "earnbabySUSHI: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "earnbabySUSHI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "earnbabySUSHI: Cannot update gasForProcessing to same value");
    ...
    function getliqAdd() public view returns (address) {
        return _liqAdd;
    }
    
    function getLPadd() public view returns (address) {
        return _LPadd;
    }
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "earnbabySUSHI: This account cannot send tokens until trading is enabled");
        }
        ...
        if(
            ...
            sellFeeIncreaseFactor = sellFeeIncreaseFactor;
        ...
        if(
            ...
            uint256 bnbrewardstoken = contractTokenBalance.mul(12).div(totalFees);
            swapTokensForBNB(bnbrewardstoken);
            
            uint256 liqtokens = contractTokenBalance.mul(3).div(totalFees);
            swapTokensForBNB2(liqtokens);
            
            uint256 swapTokens = contractTokenBalance.mul(2).div(totalFees);
...
contract earnbabySUSHIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("earnbabySUSHI_Dividend_Tracker", "earnbabySUSHI_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 100000000 * (10**18); //must hold 1+ tokens
    }



    function _transfer(address, address, uint256) internal override {
        require(false, "earnbabySUSHI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "earnbabySUSHI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main earnbabySUSHI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "earnbabySUSHI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "earnbabySUSHI_Dividend_Tracker: Cannot update claimWait to same value");
