...
contract DogePoo is Context, IERC20, Ownable {
    ...
    string private _name = "DogePoo";
    string private _symbol = "DogePoo";
    ...
    function setrewardAddress(address payable reward) public onlyOwner {
    ...
    function showrewardaddress() public view returns(address payable) {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateLiquidityPlusrewardFee(tAmount);
    ...
    function calculateLiquidityPlusrewardFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _rewardFee + _marketingAndDevBudget).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingAndDevBudget == 0 && _rewardFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _marketingAndDevBudget + _liquidityFee + _rewardFee;
        ...
        // calculate the portions of the liquidity to add to reward
        uint256 rewardBalance = newBalance.div(totalLiqFee).mul(_rewardFee);
        uint256 rewardPortion = otherHalf.div(totalLiqFee).mul(_rewardFee);
        ...
        uint256 finalBalance = newBalance.sub(rewardBalance).sub(marketingBalance);
        uint256 finalHalf = otherHalf.sub(rewardPortion).sub(marketingPortion);
        
        
        
        (bool sent, bytes memory data) = _rewardAddress.call{value: rewardBalance}("");
        if(sent){
            _tokenTransfer(address(this), 0x0000000000000000000000000000000000000001, rewardPortion, false);
            emit Transfer(address(this), 0x0000000000000000000000000000000000000001, rewardPortion);
        } else {
            addLiquidity(rewardPortion, rewardBalance, _rewardAddress);
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount, address reward) private {
