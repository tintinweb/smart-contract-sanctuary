...
/*  BabyChikuInu
    ...
    Telegram:t.me/Baby_Chiku_Inu
    Twitter:twitter.com/Baby_Chiku_Inu
*/
...
contract BabyChikuInu is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   BabyChikuInuDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Baby Chiku Inu", "Baby Chiku Inu") {
        ...
        _maxWallet = 2500000000 * (10**18);
        _maxBuy = 2500000000 * (10**18);
        _maxSell = 2500000000 * (10**18);
    	...
    	dividendTracker = new BabyChikuInuDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x2Af77e71D00D902B00BAcCeCd18A9f97ADb73F19);
        _devWallet = payable(0xaE20d921158ad7E18117FFCDE6e6010Ed94FFfdF);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyChikuInu: The dividend tracker already has that address");

        BabyChikuInuDividendTracker newDividendTracker = BabyChikuInuDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyChikuInu: The new dividend tracker must be owned by the BabyChikuInu token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "BabyChikuInu: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyChikuInu: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyChikuInu: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyChikuInu: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyChikuInu: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyChikuInu: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "BabyChikuInu: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyChikuInu: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyChikuInu: Cannot update gasForProcessing to same value");
	...
	function letsGoLive() external onlyOwner() {
        updateBuyFees(2,4,3,6);
        updateSellFees(2,5,3,6);

        updateMaxWallet(2500000000);
        updateMaxBuySell(2500000000, 2500000000);
...
contract BabyChikuInuDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BabyChikuInuInu_Dividend_Tracker", "BabyChikuInuInu_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "BabyChikuInuInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyChikuInuInu contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "BabyChikuInuInu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyChikuInuInu_Dividend_Tracker: Cannot update claimWait to same value");
