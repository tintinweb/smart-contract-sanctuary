...
contract FlappyFloki is Context, IERC20, Ownable {
    ...
    string private _name = "Flappy Floki";
    string private _symbol = "FLAP";
    ...
    uint256 public _jackPotFee = 400; // means 4%
    uint256 public _developmentFee = 500; // means 5%
    ...
    uint256 public _SellJackPotFee = 600; // means 6%
    uint256 public _SellDevelopmentFee = 700; // means 7%
    
    
    address private marketingAddress=0xcA2a3A58819AfF26CB81F3819ba99d49D84B3971;
    address private jackPotAddress=0xE37788D2D9982b35DE351B0a4C61644BCa620889;
    address private developmentAddress=0x41B0E1635f4A99AeE8126A8eD523fb8637858114;
    ...
    function collectBuyFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(_jackPotFee != 0){
            uint256 jackPotFee = amount.mul(_jackPotFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(jackPotFee);
            _reflectionBalance[jackPotAddress] = _reflectionBalance[jackPotAddress].add(jackPotFee.mul(rate));
            if(_isExcluded[jackPotAddress]){
                _tokenBalance[jackPotAddress] = _tokenBalance[jackPotAddress].add(jackPotFee);
            }
          
            emit Transfer(account,jackPotAddress,jackPotFee);
        }
    ...
    function collectSellFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
       ...
       //@dev tax fee
        ...
        if(_SellJackPotFee != 0){
            uint256 jackPotFee = amount.mul(_SellJackPotFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(jackPotFee);
            _reflectionBalance[jackPotAddress] = _reflectionBalance[jackPotAddress].add(jackPotFee.mul(rate));
            if(_isExcluded[jackPotAddress]){
                _tokenBalance[jackPotAddress] = _tokenBalance[jackPotAddress].add(jackPotFee);
    ...
    function setBuyFee(uint256 developmentFee,uint256 marketingFee,uint256 jackPotFee,uint256 liquidityFee,uint256 taxFee) external onlyOwner {
    ...
    function setSellFee(uint256 developmentFee,uint256 marketingFee,uint256 jackPotFee,uint256 liquidityFee,uint256 taxFee) external onlyOwner {
    ...
    function setWalletAddress(address _marketingAddress,address _developmentAddress,address _jackPotAddress) external onlyOwner{
