...
We all love Floki and its ride ! lets get this pumping ! With trending name and trending rewards (ADA) , we strive to get back the trust in micro launches!

Official Telegram group : https://t.me/MysteryFlokiOfficial
...
import "./DeFlokiDividendTracker.sol";

contract DeFloki is ERC20, Ownable {
    ...
    DeFlokiDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("Mystery Floki", "DeFloki") {

    	dividendTracker = new DeFlokiDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DeFloki: The dividend tracker already has that address");

        DeFlokiDividendTracker newDividendTracker = DeFlokiDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DeFloki: The new dividend tracker must be owned by the DeFloki token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DeFloki: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DeFloki: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DeFloki: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DeFloki: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DeFloki: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DeFloki: Cannot update gasForProcessing to same value");
