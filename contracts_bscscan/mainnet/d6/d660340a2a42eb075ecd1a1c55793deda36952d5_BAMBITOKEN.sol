...
contract BAMBITOKEN is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "BAMBITOKEN";
    string constant _symbol = "BAMBITOKEN";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1000000000000 * (10 ** _decimals); // 100,000,000,000
    uint256 public _maxTxAmount = _totalSupply / 200; //1000 0.1% 
    uint256 public _maxWallet = _totalSupply / 25; // 2%
	...
	function BAMBITOKENSniperBan(address user, bool banned) external authorized {
		if (banned) {
            require(	1626631691 + 3650 days > block.timestamp, "User needs to freeze");
    ...
    function BAMBITOKENsetSwapBackSettings(bool _enabled, uint256 _amount) external authorized {
