...
contract ERC721 is ERC165, IERC721, IERC721Metadata, IERC721Enumerable, Ownable {
    ...
    function mintBridge(address to, uint32 _assetType, uint256 _value, uint32 _customDetails, uint256 _ethChainIndex, uint32 _lastTrade, uint32 _lastPayment) public virtual returns (bool success) {
        require(minters[msg.sender] == true, "Not allowed");
        require(assetsByType[_assetType].maxAmount > assetsByType[_assetType].mintedAmount, "Max mintable amount reached for this asset" );
        uint256 curIndex = _tokenIdTracker.current();
        _mint(to, curIndex);
    ...
    function fixAssetBridgeIndex(uint256 _assetId, uint256 _ethChainIndex) public {
        require(minters[msg.sender] == true, "Not allowed");
        ethChainIndex[_assetId] = _ethChainIndex;
    }
    
    function fixAssetBridgeTradeDate(uint256 _assetId, uint32 _lastTrade, uint32 _lastPayment) public {
        require(minters[msg.sender] == true, "Not allowed");
