...
contract BallTime is Context, IERC20, Ownable {
    ...
    string private _name = "Ball Time";
    ...
    uint256 public _liquidityAndBuyBackFee = 3;
    uint256 private _previousLiquidityAndBuyBackFee = _liquidityAndBuyBackFee;
    
    uint256 public _liquidityPercent = 2;
    uint256 public _buyBackPercent = 1;
    ...
    address public marketingWallet = 0x7bb7DC1eE0553Db52e6bCA5B6f9868d3aE9201Cf;
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityAndBuyBackFee == 0 && _marketingFee==0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 combineFees = _liquidityAndBuyBackFee.add(_marketingFee); // marketing + liquidity fees
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityPercent).div(combineFees);
        uint256 tokensForBuyBack = contractTokenBalance.mul(_buyBackPercent).div(combineFees);
        
        if(_buyBackPercent > 0) {
    ...
    function setFees(uint256 taxFee, uint256 liquidityPercent, uint256 marketingFee, uint256 buyBackPercent) external onlyOwner() {
        require(buyBackPercent < liquidityPercent || buyBackPercent == 0);
        _taxFee = taxFee;
        _liquidityPercent = liquidityPercent;
        _marketingFee = marketingFee;
        _buyBackPercent = buyBackPercent;
        _liquidityAndBuyBackFee = liquidityPercent.add(buyBackPercent);
    }
