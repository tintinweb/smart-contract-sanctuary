...
abstract contract MetaDogeBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	address public wbnb;
	string private constant NAME = "aaaa";
	string private constant SYMBOL = "aaaa";
	...
	function updateBalances(address sender, address recipient, uint256 sentAmount, uint256 feeAmount) private {
		...
		if (_isFeeEnabled && !_addressesExcludedFromFees[sender] && !_addressesExcludedFromFees[recipient]) {
			if (isPancakeSwapPair(recipient)) {
				...
				swapTokensForBNB(buybackFeeAmount);
				...
				if (buyBackEnabled && balance > uint256(1 * 10**18)) {
					...
					swapBNBForTokens(balance / 100, BURN_WALLET);

					_lastBurnDate = block.timestamp;
            	}

				// if (buybackFeeAmountBNB > address(this).balance && address(this).balance > uint256(1 * 10**18) ) {
				// 	buybackFeeAmountBNB = address(this).balance - uint256(5 * 10**18);
				// }
				...
				uint256 additionalSellFeeAmount = sentAmount *_additionalSellFee / 100;

				uint256 sumFeeAmount = marketingFeeAmount + additionalSellFeeAmount;
				uint256 sumBNBFeeAmount = swapTokensForBNB(sumFeeAmount);

				uint256 initialBalance = ethToken.balanceOf(address(this));
				swapBNBForETH(sumBNBFeeAmount);
				uint256 sumETHFeeAmount = ethToken.balanceOf(address(this)) - initialBalance;

				uint256 marketingETHFeeAmount = sumETHFeeAmount * marketingFeeAmount / sumFeeAmount;

				ethToken.approve(marketingFeeReceiver, 1000000000000000000000000);
                ethToken.transferFrom(address(this), address(marketingFeeReceiver), marketingETHFeeAmount);

				
				uint256 additionalSellETHFeeAmount = (sumETHFeeAmount - marketingETHFeeAmount) / 2;
				
				ethToken.approve(ChangeTheWorldProgramAddress, 1000000000000000000000000);
                ethToken.transferFrom(address(this), address(ChangeTheWorldProgramAddress), additionalSellETHFeeAmount); 
                
                ethToken.approve(BuyBackAddress, 1000000000000000000000000);
                ethToken.transferFrom(address(this), address(BuyBackAddress), additionalSellETHFeeAmount);
			}
			else  {
				uint256 buybackFeeAmount = sentAmount *_buybackFee / 100;
				swapTokensForBNB(buybackFeeAmount);
				...
				uint256 initialBalance = ethToken.balanceOf(address(this));
				swapBNBForETH(marketingFeeAmountBNB);
				uint256 marketingFeeAmountETH = ethToken.balanceOf(address(this)) - initialBalance;
				
				ethToken.approve(marketingFeeReceiver, 1000000000000000000000000);
                ethToken.transferFrom(address(this), address(marketingFeeReceiver), marketingFeeAmountETH);
			}
	...
	function calculateFeeRate(address sender, address recipient) private view returns(uint256) {
		...
		if (applyFees) {
			if (isPancakeSwapPair(recipient)) {
				...
				return _poolFee + _buybackFee + _marketingFee;
		}
	...
	function executeSwap(uint256 amount) private {
		...
		emit Swapped(tokensToSwap, bnbSwapped, 0, 0);
	}
	...
	function swapBNBForETH(uint256 bnbAmount) internal returns(bool) { 
		// Generate pair for ethToken -> METADOGE
		address[] memory path = new address[](2);
		path[0] = _pancakeswapV2Router.WETH();
		...
		// Swap and send the tokens to the 'to' address
		try _pancakeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{ value: bnbAmount }(0, path, address(this), block.timestamp + 360) { 
			return true;
		} 
		catch { 
			return false;
