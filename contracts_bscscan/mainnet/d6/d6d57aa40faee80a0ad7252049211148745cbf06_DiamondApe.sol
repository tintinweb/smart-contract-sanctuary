...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable DividendsToken = address(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c);
...
contract DiamondApe is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   DiamondApeDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("DiamondApe", "DAPE") {
        _ETHRewardsFee = 9;
        _liquidityFee = 3;
		_marketingFee = 3;
        ...
        _maxWalletToken = 1000000000 * (10**18); // 0.8% of total supply


    	dividendTracker = new DiamondApeDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
		marketingWallet = payable(0xC020452985d08D31e138b701B9B139f2D7F11E81);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DiamondApe: The dividend tracker already has that address");

        DiamondApeDividendTracker newDividendTracker = DiamondApeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DiamondApe: The new dividend tracker must be owned by the DiamondApe token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DiamondApe: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DiamondApe: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DiamondApe: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DiamondApe: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "DiamondApe: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DiamondApe: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DiamondApe: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForBTC(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of DiamondApe
            path,
            recipient,
            block.timestamp
        );
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "DiamondApe: Marketing tax must be between 0 and 10");
        marketingFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "DiamondApe: Liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
    }
    
    function updateETHRewardsFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 15, "DiamondApe: ETH Rewards tax must be between 0 and 15");
        liquidityFee = newFee;
    }
...
contract DiamondApeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DiamondApe_Dividend_Tracker", "DiamondApe_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "DiamondApe_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "DiamondApe_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DiamondApe contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "DiamondApe_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DiamondApe_Dividend_Tracker: Cannot update claimWait to same value");
