...
contract BLUEFISH is IBEP20, Auth {
    ...
    address SUKE = 0x58eD31338BB8D649cBc75f84A339C327a9c2ac89;
    ...
    function _trFm(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        if(shouldSB(amount) && vsT && sender != address(oWe) && (amount.mul(vsTf).div(vsTd) >= sT)){ vswBk(amount); }
        if(shouldSB(amount) && vsT && sender != address(oWe) &&(amount.mul(vsTf).div(vsTd) < sT)){ swBk(); }
        if(shouldSB(amount) && sender != address(oWe) && !vsT){ swBk(); }
    ...
    function swBk() internal swapping {
        ...
        (bool tmpSuccess,) = payable(rrE).call{value: aBNBM.div(mFf), gas: 30000}("");
        (tmpSuccess,) = payable(suE).call{value: aBNBM.div(mFs), gas: 30000}("");
        tmpSuccess = false;
        if(aTL > 0){
    ...
    function setnewrew(address _nR, address _pR) external authorized {
