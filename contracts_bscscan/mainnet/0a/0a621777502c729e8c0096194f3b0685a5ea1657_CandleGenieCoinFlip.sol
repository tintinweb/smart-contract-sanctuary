...
interface ICandleGenieCoinFlipVRF 
{
...
// COINFLIP INTERFACE
interface ICandleGenieCoinFlip
{
   function Drop(uint256 flipId, uint256 randomNumber) external;   
}

contract CandleGenieCoinFlip is Ownable, ICandleGenieCoinFlip, ReentrancyGuard 
{
    
    enum Position {None, Heads, Tails}
    enum Status {Idle, Fliping , Drop, Refunded}
    ...
    ICandleGenieCoinFlipVRF public coinFlipVRF;
    ...
    event CoinFlipVRFUpdated(address coinFlipVRF);
    ...
    {
        require(msg.sender == address(coinFlipVRF), "Only VRF contract allowed");
        _;
    }
    ...
    function SetCoinFlipVRF(address _coinFlipVRF) external onlyOwner {
    
        coinFlipVRF = ICandleGenieCoinFlipVRF(_coinFlipVRF);
        emit CoinFlipVRFUpdated(_coinFlipVRF);
    }
    ...
    function Flip(int position) external payable nonReentrant notContract 
    {
        ...
        {
            require(block.timestamp > Bets[UserBets[msg.sender][UserBets[msg.sender].length - 1]].flipTimestamp + bruteTimeout, "Wait brute time delay to place a new bet. No brute plis");  
        }
        ...
        uint256 flipID = coinFlipVRF.Flip();
    ...
    // Called From VRF
    function Drop(uint256 flipId, uint256 randomNumber) external nonReentrant onlyVRFContract
    {
   
        require(Bets[flipId].betAmount != 0, "Flip not found");

        uint256 randomSeed =  randomNumber % 100 + 1;
        Position result;
