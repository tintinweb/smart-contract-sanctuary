...
contract Godzilla is IERC20, Ownable {
    ...
    constructor(address tokenOwner) {
        ...
        _tTotal = 10 * 10**4 * 10**8 * 10**_decimals;
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair && !_isExcludedFromFee[from]) {
            uint256 p1 = _tTotal.div(100);
            
            require(balanceOf(from) <= p1.mul(3), "hold amount exceeds 3%");
            
            require(amount <= p1, "sold amount exceeds 1%");
           
            require(lastSellTime[from] + 60 < block.timestamp);
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 8; i++) {
            ...
            uint256 curTAmount = tAmount.div(1000).mul(5);
