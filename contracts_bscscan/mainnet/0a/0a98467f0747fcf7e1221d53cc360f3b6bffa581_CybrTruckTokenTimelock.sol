 ...
 */
contract CybrTruckTokenTimelock is Ownable {
    using SafeERC20 for IERC20;

    // (20/07/2030) timestamp when token release is enabled
    uint256 public releaseTime = 1626768946;
    
    address public cybrtruckToken = 0x92d0A04815FB4f7Ee143208e332356167dc81407;
    address public lpToken = 0x546FF9b52fC3Bbe4A457D41a6a7a7B858AA0B493;

    /**
     * @notice Transfers tokens held by timelock to owner.
     */
    function releaseCybrTruckToken() external onlyOwner {
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp >= releaseTime, "TokenTimelock: current time is before release time");

        uint256 amount = IERC20(cybrtruckToken).balanceOf(address(this));
        require(amount > 0, "TokenTimelock: no tokens to release");

        IERC20(cybrtruckToken).safeTransfer(owner(), amount);
    }
    ...
    function getCybrTruckTokenBalance() external view returns(uint256) {
        return IERC20(cybrtruckToken).balanceOf(address(this));
    }
    ...
    function setCybrTruckToken(address _cybrtruckToken) external onlyOwner {
        cybrtruckToken = _cybrtruckToken;
    }
