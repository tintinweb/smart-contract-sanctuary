...
contract SimpleRipple is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SimpleRippleDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("SimpleRipple", "SRP") {
        _ETHRewardsFee = 9;
        _liquidityFee = 3;
		_marketingFee = 4;
    	...
    	dividendTracker = new SimpleRippleDividendTracker();
    	antiBotEnabled = true;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SimpleRipple: The dividend tracker already has that address");

        SimpleRippleDividendTracker newDividendTracker = SimpleRippleDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SimpleRipple: The new dividend tracker must be owned by the SimpleRipple token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SimpleRipple: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SimpleRipple: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SimpleRipple: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SimpleRipple: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SimpleRipple: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SimpleRipple: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SimpleRipple: Cannot update gasForProcessing to same value");
...
contract SimpleRippleDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SimpleRippleDividend_Tracker", "SimpleRipple_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "SimpleRipple_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SimpleRipple_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SimpleRipple contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "SimpleRipple_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SimpleRipple_Dividend_Tracker: Cannot update claimWait to same value");
