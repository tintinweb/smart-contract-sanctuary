...
Join Us At Telegam: https://t.me/ProADABSC
...
contract ProADA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ProADADividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("ProADA", "ProADA") {
        _ETHRewardsFee = 7;
        _liquidityFee = 3;
		_marketingFee = 6;
    	...
    	dividendTracker = new ProADADividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
		bulkSender = payable(0x979E44C6253C3B861705621DD0ac37f555025f82);
		marketingWallet = payable(0x5CA072D081701d2E056990FD1F53E2008313d850);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ProADA: The dividend tracker already has that address");

        ProADADividendTracker newDividendTracker = ProADADividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ProADA: The new dividend tracker must be owned by the ProADA token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ProADA: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ProADA: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ProADA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ProADA: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ProADA: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ProADA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ProADA: Cannot update gasForProcessing to same value");
...
contract ProADADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ProADADividend_Tracker", "ProADA_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "ProADA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ProADA_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ProADA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "ProADA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ProADA_Dividend_Tracker: Cannot update claimWait to same value");
