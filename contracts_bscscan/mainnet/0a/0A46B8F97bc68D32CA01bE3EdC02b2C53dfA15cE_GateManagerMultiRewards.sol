...
contract GateManagerMultiRewards is ReentrancyGuardUpgradeable, Ownable {
    ...
    modifier updateReward(address account) {
        uint256 length = rewardId.length;
        uint256 i;
        for (i = 0; i < length; ++i) {
            uint256 rewardPerTokenStored = rewardPerToken(i);
            uint256 lastUpdateTime = lastTimeRewardApplicable(i);
            if (account != address(0)) {
                rewards[rewardId[i].rewardToken][account] = earned(account, i);
                userRewardPerTokenPaid[rewardId[i].rewardToken][account] = rewardPerTokenStored;
                }
             _;
        }
    }
    ...
    function totalSupply() public view returns (uint256) {
     ...
     function rewardTokenLength() external view returns (uint256) {
        return rewardId.length;
    }

    function rewardInfo(uint256 id) external view returns (uint256 stakingRewardsDuration, uint256 periodFinish, uint256 rewardRate, uint256 lastUpdateTime, uint256 rewardPerTokenStored) {
        RewardInfo storage info = rewardId[id];
        return(
    ...
    function lastTimeRewardApplicable(uint256 id) public view returns (uint256) {
        return MathUpgradeable.min(block.timestamp, rewardId[id].periodFinish);
    }

    function rewardPerToken(uint256 id) public view returns (uint256) {
        RewardInfo storage rewards = rewardId[id];
        if (totalSupply() == 0) {
            return rewards.rewardPerTokenStored;
        }
        return
            rewards.rewardPerTokenStored.add(
                lastTimeRewardApplicable(id)
                    .sub(rewards.lastUpdateTime)
                    .mul(rewards.rewardRate)
                    .mul(1e18)
                    .div(totalSupply())
            );
    }
    ...
    function getReward() public updateReward(msg.sender) {
        uint256 i;
        for (i = 0; i < rewardId.length; i++) {
        uint256 reward = earned(msg.sender, i);
        if (reward > 0) {
            address token = rewardId[i].rewardToken;
    ...
    function addRewardToken(address _rewardToken, uint256 _stakingRewardsDuration) external onlyOwner {
        rewardId.push(RewardInfo({
            rewardToken: _rewardToken,
            stakingRewardsDuration: _stakingRewardsDuration,
            periodFinish: 0,
        ...
        updateReward(address(0))
    {
        ...
        if (block.timestamp >= info.periodFinish) {
            ...
            info.rewardRate = reward.add(leftover).div(info.stakingRewardsDuration);
        }
    ...
    function inCaseTokensGetStuck(address _token) external onlyOwner {
      ...
      //  require(_token != address[](rewards), "!rewards token");
