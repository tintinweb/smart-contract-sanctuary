...
contract testwc is ERC20, Ownable {
	using SafeMath for uint256;
	...
	testwcDividendTracker public dividendTracker;
	testwcTransferHelper private transferHelper;
	...
	uint256 public _maxWalletAmount = 15000000 * (10 ** 18);
	...
	MaxWalletMode public maxWalletMode = MaxWalletMode.ENABLED;
	...
	constructor(address routerAddress, address payable marketingWalletAddress, address payable devWalletAddress) public ERC20("testwc", "testwc1") {
		dividendTracker = new testwcDividendTracker();
		transferHelper = new testwcTransferHelper(routerAddress);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "testwc: The dividend tracker already has that address");

		testwcDividendTracker newDividendTracker = testwcDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "testwc: The new dividend tracker must be owned by the testwc token contract");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "testwc: The router already has that address");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "testwc: Account is already the value of 'excluded'");
	...
	function setBuyFees(uint256 _dividendsFee, uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee) public onlyOwner {
		buyFees = FeeSet({
	...
	function setSellFees(uint256 _dividendsFee, uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee) public onlyOwner {
		sellFees = FeeSet({
			dividendsFee: _dividendsFee,
			marketingFee: _marketingFee,
			liquidityFee: _liquidityFee,
	...
	function getSumOfFeeSet(FeeSet memory set) private pure returns (uint256) {
		return set.dividendsFee.add(set.marketingFee).add(set.liquidityFee).add(set.devFee);
	}
	...
	function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
		require(pair != uniswapV2Pair, "testwc: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
	...
	function setMaxWalletAmount(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "testwc: Amount cannot be over the total supply.");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "testwc: Automated market maker pair is already set to that value");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "testwc: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "testwc: Cannot update gasForProcessing to same value");
	...
	function _transfer(address _from, address _to, uint256 _amount) internal override {
		...
		if (takeFee) {
			...
			if (feePercent > 0) {
				if (block.number <= (launchedAt + 3) && automatedMarketMakerPairs[_from] && _to != address(uniswapV2Router) && _to != address(this) && _to != owner()) {
					feePercent = 90;
				}
	...
	function processFees(uint256 amountIn) private {
		...
		uint256 ethForDev = amountOut.mul(buyFees.devFee).div(totalFees);
		uint256 ethForLiquidity = amountOut.sub(ethForDividends).sub(ethForMarketing).sub(ethForDev);
        ...
        if (ethForDev > 0) {
            payable(_devWalletAddress).transfer(ethForDev);
        }
...
contract testwcDividendTracker is Ownable, DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() public DividendPayingToken("testwc_Dividend_Tracker", "testwc_Dividend_Tracker") {

	}

	function _transfer(address, address, uint256) internal override {
		require(false, "testwc_Dividend_Tracker: No transfers allowed");
	}

	function withdrawDividend() public override {
		require(false, "testwc_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main testwc contract.");
	}
	...
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "testwc_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "testwc_Dividend_Tracker: Cannot update claimWait to same value");
...
contract testwcTransferHelper is Ownable {
