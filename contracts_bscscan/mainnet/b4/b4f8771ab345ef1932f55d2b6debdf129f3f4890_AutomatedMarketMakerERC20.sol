...
// File: deployment-process\contracts-origin\contracts\amm\AutomatedMarketMakerERC20.sol
...
/**
 * @title The mystical automated market maker contract for ERC20 tokens.
 * @author int(200/0), slidingpanda
 * @dev Provides a fix point to a fixed exchange rate for the pegged and ten-Token.
 */
contract AutomatedMarketMakerERC20 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    event Bought(address account, uint256 amount);
    event Sold(address account, uint256 amount);

    using SafeERC20Upgradeable for IERC20Upgradeable;

    IERC20Upgradeable private _inToken;
    TenToken private _tenToken;

    bool private _canBuy;
    uint8 private _calcDecimals;

    /**
     * @dev Sets the contract owner and the ten-Token:Token relation.
     * @param inToken address of the ERC20 token
     * @param tenToken address of the ten-Token
     */
    function initialize(address inToken, address tenToken) public initializer {
        __Ownable_init();
        _inToken = IERC20Upgradeable(inToken);
        _tenToken = TenToken(tenToken);
        _calcDecimals = _tenToken.calcDecimals();
        _canBuy = false;
    }
    ...
    /**
     * @dev Returns the ten-Token address.
     * @return ten-Token address
     */
    ...
    /**
     * @dev Mints new ten-Tokens for the sent ERC20 tokens.
    ...
    /**
     * @dev Burns ten-Tokens and sends the ERC20 token amount.
