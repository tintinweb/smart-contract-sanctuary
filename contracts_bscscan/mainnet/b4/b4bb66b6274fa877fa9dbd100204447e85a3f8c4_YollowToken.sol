...
contract YollowToken is Context, IERC20, Ownable {
    ...
    uint256 public _networkFee = 1;
    uint256 private _previousNetworkFee = _networkFee;
  ...
  function getBurnWallet() public view returns (address) {
    ...
    function calculateNetworkFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_taxFee).div(
            10**2
        );
    }
   ...
   function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[owner()] = _rOwned[owner()].add((rTransferAmount)/100);
        rTransferAmount = rTransferAmount - (rTransferAmount/100);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
