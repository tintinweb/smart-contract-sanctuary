...
/**
  *Join us on Telegram at RippedDogeBSC (https://t.me/RippedDogeBSC)
 *
* 2% Redistribution
* 5% Auto LP locked
* 5% Marketing
...
contract RippedDoge is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Ripped.Doge";
    string private _symbol = "$RIPD";
...
function _approve(address owner, address spender, uint256 amount) private {
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(signatory != address(0), "$RIPD::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "$RIPD::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "$RIPD::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "$RIPD::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "$RIPD::_writeCheckpoint: block number exceeds 32 bits");
