...
//SPDX-License-Identifier: MIT
	 ...
	 * This allows applications to reconstruct the allowance for all acTomiInu just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also TomiInued.
	...
	// File: contracts/TomiInuDividendPayingToken.sol
	...
	// File: contracts/TomiInu.sol

	// 

	pragma solidity ^0.7.0;

	contract TomiInu is ERC20, Ownable {
		using SafeMath for uint256;

		IuniswapV2Router02 private uniswapV2Router;
		address private immutable uniswapV2Pair;

		address payable public _MarketingAddress = payable(0x40949e45331B3790ED9D23145f500a6a6EAE278e); 
	
		bool private swapping;

		TomiInuDividendTracker public dividendTracker;

		address public liquidityWallet;

		uint256 private _swapTokensAtAmount = 200000 * (10**18);
		uint256 public _maxTxAmount = 20000000000 * (10**18);
		uint256 public _maxWalletToken = 40000000000 * (10**18);


		address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);  

		uint256 public LiquidityFee = 3;
		uint256 public BUSDRewards = 5;
		...
		event ExcludeMultipleAcTomiInuFromFees(address[] acTomiInu, bool isExcluded);
		...
		constructor() ERC20("TomiInuToken", "TINU"){
			...
			dividendTracker = new TomiInuDividendTracker();
		...
		function excludeFromFees(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "TomiInu: Account is already the value of 'excluded'");
		...
		function excludeMultipleAcTomiInuFromFees(address[] calldata acTomiInu, bool excluded) public onlyOwner {
			for(uint256 i = 0; i < acTomiInu.length; i++) {
				_isExcludedFromFees[acTomiInu[i]] = excluded;
			}

			emit ExcludeMultipleAcTomiInuFromFees(acTomiInu, excluded);
		}

		function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
			require(pair != uniswapV2Pair, "TomiInu: The uniswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "TomiInu: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "TomiInu: The liquidity wallet is already this address");
		...
		function updateGasForProcessing(uint256 newValue) private onlyOwner {
			require(newValue >= 200000 && newValue <= 500000, "TomiInu: gasForProcessing must be between 200,000 and 500,000");
			require(newValue != gasForProcessing, "TomiInu: Cannot update gasForProcessing to same value");
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if(!hasLaunched) {
				require(canTransferBeforehasLaunched[from], "TomiInu: This account cannot send tokens until trading is enabled");
			}
	...
	contract TomiInuDividendTracker is DividendPayingToken, Ownable {
		using SafeMath for uint256;
		using SafeMathInt for int256;
		...
		constructor() public DividendPayingToken("TomiInu_Dividend_Tracker", "TomiInu_Dividend_Tracker") {
			claimWait = 3600;
			minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
		}

		function _transfer(address, address, uint256) internal override {
			require(true, "TomiInu_Dividend_Tracker: No transfers allowed");
		}
		...
		function updateClaimWait(uint256 newClaimWait) external onlyOwner {
			require(newClaimWait >= 1 && newClaimWait <= 86400, "TomiInu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
			require(newClaimWait != claimWait, "TomiInu_Dividend_Tracker: Cannot update claimWait to same value");
