...
contract GOMAsterChef is Ownable {
    ...
    // 
    function pendingRewardsOfUser(address _user) public view returns (uint256 _real, uint256 _reflection) {
        UserInfo storage user = userInfo[_user];
                
		if (blockTimestamp() > lastRewardTimestamp && stakedTokenDeposied != 0 && user.amount != 0) {
            uint256 _accRewardTokensPerShare = accRewardTokensPerShare + (getMultiplierNow() * tokensPerSecond * reflectionMultiplier);
            _reflection = user.amount * _accRewardTokensPerShare / stakedTokenDeposied;
            if (_reflection >= user.rewardDebt) {
                _reflection = _reflection - user.rewardDebt;
            } else {
                _reflection = 0;
            }
            _real = _reflection / reflectionMultiplier;            
		}
        
        return (_real, _reflection);            
    }
