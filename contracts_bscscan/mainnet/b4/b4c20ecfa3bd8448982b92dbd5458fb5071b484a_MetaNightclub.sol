...
contract MetaNightclub is Context, IERC20, IERC20Metadata,Ownable {
    ...
    constructor() {
        ...
        taxAddress = 0xa4Df7363ED13b91071fcdffD67c86A2dE09A70b0;
    ...
    function _transfer(
        ...
        if((recipient==pairAddress&&takeFee)|| (sender == pairAddress &&takeFee))
        {
            uint256 taxFee = amount.mul(dexTaxFee).div(10000);
            uint256 dfee = amount.mul(fee).div(10000);
            _balances[taxAddress] =_balances[taxAddress].add(dfee);
            _balances[address(this)] =_balances[address(this)].add(taxFee);
            emit Transfer(sender, address(this), taxFee);
            swapTokensForEth(taxFee);
            amount = amount.sub(taxFee.add(dfee));
        }
