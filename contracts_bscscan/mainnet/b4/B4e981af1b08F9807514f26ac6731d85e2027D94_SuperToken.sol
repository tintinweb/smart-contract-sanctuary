...
contract SuperToken is ERC20, Ownable {
    ...
    uint256 private buyBackUpperLimitBnb = 1;       //1 BNB

    // Minimum BNB balance before buyback IF lower than this number no buyback
    uint256 public minimumBalanceRequiredBnb = 1;   //1 BNB
    ...
    constructor() ERC20("TEST3TOKEN", "TEST3T") {
    ...
    function setTradingIsEnabled(bool _enabled) external onlyOwner {
        require(tradingIsEnabled != _enabled, "Can't set flag to same status");
        tradingIsEnabled = _enabled;
    }

    function setBuyBackMode(bool _enabled) external onlyOwner {
        require(buyBackMode != _enabled, "Can't set flag to same status");
        buyBackMode = _enabled;
    }

    function setMinimumBalanceRequiredBnb(uint256 _newAmount) public onlyOwner {
        require(_newAmount >= 0, "newAmount error");
        minimumBalanceRequiredBnb = _newAmount;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (buyBackAndLiquifyEnabled && buyBackMode) {
                uint256 buyBackBalanceBnb = address(this).balance;
                if (buyBackBalanceBnb >= minimumBalanceRequiredBnb.mul(10**18) && amount >= minimumSellOrderAmount) {
                    if (buyBackBalanceBnb > buyBackUpperLimitBnb.mul(10**18)) {
                        buyBackBalanceBnb = buyBackUpperLimitBnb.mul(10**18);
                    }
    ...
    function buyBackAndBurnBnb(uint256 _amountBnb) public onlyOwner {
        uint256 balance = address(this).balance;
        require(buyBackAndLiquifyEnabled, "buyBackAndLiquifyEnabled is not enabled");
        require(balance >= minimumBalanceRequiredBnb.mul(10**18).add(_amountBnb.mul(10**18)), "buyback amount is too big");

        if (
            !swapping
        ) {
            buyBackAndBurn(_amountBnb.mul(10**18));
        }
    }
