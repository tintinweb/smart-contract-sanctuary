...
contract RewardToken is ERC20, Ownable {
    ...
    uint256 public trxDividendRewardsFee = 5;
    uint256 public previousTrxDividendRewardsFee;
    uint256 public marketingFee = 4;
    uint256 public previousMarketingFee;
    uint256 public totalFees = adaDividendRewardsFee.add(marketingFee).add(trxDividendRewardsFee).add(liquidityFee);
    ...
    constructor() ERC20("AdaTron", "ADATRX") {
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "ADATron: The marketing wallet is already this address");
    ...
    function setAdaDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = adaDividendRewardsFee.add(marketingFee).add(trxDividendRewardsFee).add(liquidityFee);
            adaDividendEnabled = _enabled;
        }
    ...
    function setTrxDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = trxDividendRewardsFee.add(marketingFee).add(adaDividendRewardsFee).add(liquidityFee);
            trxDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(trxDividendRewardsFee).add(adaDividendRewardsFee).add(liquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateadaDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(adaDividendTracker), "ADATron: The dividend tracker already has that address");
 
        ADADividendTracker newadaDividendTracker = ADADividendTracker(payable(newAddress));
 
        require(newadaDividendTracker.owner() == address(this), "ADATron: The new dividend tracker must be owned by the ADATron token contract");
    ...
    function updatetrxDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(trxDividendTracker), "ADATron: The dividend tracker already has that address");
 
        TRXDividendTracker newtrxDividendTracker = TRXDividendTracker(payable(newAddress));
 
        require(newtrxDividendTracker.owner() == address(this), "ADATron: The new dividend tracker must be owned by the ADATron token contract");
    ...
    function updateTrxDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = trxDividendRewardsFee.add(adaDividendRewardsFee).add(marketingFee).add(liquidityFee);
    }
 
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        marketingFee = newFee;
        totalFees = marketingFee.add(adaDividendRewardsFee).add(trxDividendRewardsFee).add(liquidityFee);
    }
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "ADATron: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "ADATron: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ADATron: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "ADATron: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "ADATron: Cannot update gasForProcessing to same value");
...
contract ADADividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("ADATron_Ada_Dividend_Tracker", "ADATron_Ada_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "ADATron_Ada_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "ADATron_Ada_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ADATron contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "ADATron_Ada_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ADATron_Ada_Dividend_Tracker: Cannot update claimWait to same value");
...
contract TRXDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("ADATron_Trx_Dividend_Tracker", "ADATron_Trx_Dividend_Tracker", 0x85EAC5Ac2F758618dFa09bDbe0cf174e7d574D5B) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "ADATron_Trx_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "ADATron_Trx_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ADATron contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "ADATron_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ADATron_Trx_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
