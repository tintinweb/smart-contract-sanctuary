...
/**
 * @title defiswapChef is the master of defiswap
 * @notice defiswapChef contract:
 * - Users can:
 *   # Deposit
 *   # Harvest
 *   # Withdraw
...
contract DeFiChef is Ownable, ReentrancyGuard {
    ...
    * @dev Any point in time, the amount of defiswaps entitled to a user but is pending to be distributed is:
    ...
    */
     ...
     * @param allocPoint: How many allocation points assigned to this pool. defiswaps to distribute per block
     * @param lastRewardBlock: Last block number that defiswaps distribution occurs
     * @param accRewardPerShare: Accumulated defiswaps per share, times 1e12. See below
     */
    struct PoolInfo {
        IERC20 lpToken; // Address of LP token contract.
        uint256 allocPoint; // How many allocation points assigned to this pool. defiswaps to distribute per block.
        uint256 lastRewardBlock; // Last block number that defiswaps distribution occurs.
        uint256 accRewardPerShare; // Accumulated defiswaps per share, times 1e12. See below.
    }
    /// The defiswap TOKEN!
    IERC20 public defiswap;
    /// Dev address.
    address public devaddr;
    /// The Liquidity Provider
    ILiquidityProvider public provider;
    ///  Block number when bonus defiswap period ends.
    uint256 public endBlock;
    ///  defiswap tokens created in first block.
    ...
    /// The block number when defiswap mining starts.
    ...
    modifier onlyMigrator() {
        require(
            ...
            "defiswapChef: Only migrator can call"
        );
        _;
    }
    ...
    constructor(
        ...
        require(
            address(_defiswap) != address(0x0),
            "defiswapChef::set zero address"
        );
        defiswap = _defiswap;
        devaddr = msg.sender;
    ...
    function add(
        ...
        require(
            ...
            "defiswapChef:: LP token already added"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    /**
     * @notice Update the given pool's defiswap allocation point. Can only be called by the owner
     */
    function set(
        uint256 _pid,
        uint256 _allocPoint,
    ...
    function transferdefiswap(uint256 _amount) external onlyOwner {
        safedefiswapTransfer(msg.sender, _amount);
    }

    function setPoolLPToken(uint256 _pid, address _lpToken) external onlyOwner {
        require(
            !isPoolExist[_lpToken],
            "defiswapChef:: LP token already added"
        );
    ...
    /**
     * @notice View function to see pending defiswaps on frontend
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 defiswapReward = multiplier.mul(pool.allocPoint).div(
                totalAllocPoint
            );
            accRewardPerShare = accRewardPerShare.add(
                defiswapReward.mul(1e12).div(lpSupply)
            );
        }
        return
    ...
    function updatePool(uint256 _pid) public poolExists(_pid) {
        ...
        safedefiswapTransfer(devaddr, defiswapReward.div(10));
    ...
    /**
     * @notice Deposit LP tokens to defiswapChef for defiswap allocation
    ...
    /**
     * @notice Function which send accumulated defiswap tokens to messege sender
     * @param _pid: pool ID from which the accumulated defiswap tokens should be received
     */
    function harvest(uint256 _pid) public poolExists(_pid) {
        ...
        if (user.amount > 0) {
            ...
            safedefiswapTransfer(msg.sender, pending);
    ...
    /**
     * @notice Function which send accumulated defiswap tokens to messege sender from all pools
     */
    ...
    /**
     * @notice Function which transfer defiswap tokens to _to with the given amount
     * @param _to: transfer reciver address
     * @param _amount: amount of defiswap token which should be transfer
     */
    function safedefiswapTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 defiswapBal = defiswap.balanceOf(address(this));
            if (_amount > defiswapBal) {
                defiswap.transfer(_to, defiswapBal);
            } else {
                defiswap.transfer(_to, _amount);
            }
        }
    }
    ...
    function speedStake(
        ...
        (address routerAddr, , ) = provider.apis(_apiID);
         IDeFiSwapRouter router = IDeFiSwapRouter(routerAddr);
        delete routerAddr;
        require(
        ...
        IDeFiSwapPair lpToken = IDeFiSwapPair(address(pool.lpToken));
        if (
    ...
    /**
     * @notice Function which migrate user to defiswapChef
     */
    function setUser(
        uint256 _pid,
        address _user,
        ...
        require(poolInfo.length != 0, "defiswapChef: Pools must be migrated");
        updatePool(_pid);
...
import "../dex/interfaces/IDeFiSwapPair.sol";
import "../dex/interfaces/IDeFiSwapRouter.sol";

interface ILiquidityProvider {
    function apis(uint256) external view returns(address, address, address);
    function addExchange(IDeFiSwapRouter) external;
