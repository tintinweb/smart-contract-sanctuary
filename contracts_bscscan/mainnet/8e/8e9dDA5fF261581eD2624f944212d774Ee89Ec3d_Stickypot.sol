...
/*
STICKY POT
!!!Selling open for 5 minutes every 60 minutes!!!

Telegram: t.me/stickypotv2
Website: https://cryptoweb.world/stickypotv2
...
contract Stickypot is IERC20, Ownable {
    ...
    string constant _name = "Sticky Pot v2";
    string constant _symbol = "STICKY2";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = (_totalSupply * 1) / 2000;
    ...
    uint256 public tradingWindow = 5 minutes;
    uint256 public tradingHalt = 60 minutes;
    uint256 public tradingWindowLimit = 15;
    uint256 public tradingAnytimeLimit = 5;
    uint256 public tradingLimitReset = 24 hours;
    bool public sticky = false;
    mapping (address => uint256) public lastBuy;
    mapping (address => uint256) public lastSell;
    mapping (address => uint256) public tradeTimer;
    mapping (address => bool) public lastSellInWindow;
    mapping (address => uint256) public tradeLimit;

    uint256 liquidityFee = 200;
    uint256 gemLiquidityFee = 0;
    IERC20 gemToken;
    ...
    constructor () {
        ...
        devFeeReceiver = marketingFeeReceiver;
    ...
    function checkSale(address recipient, uint256 amount) internal returns(bool) {
        if (tradeTimer[recipient] == 0 || block.timestamp - tradeTimer[recipient] >= tradingLimitReset) {
            tradeTimer[recipient] = block.timestamp;
            lastSellInWindow[recipient] = true;
            tradeLimit[recipient] = 0;
        }
        
        uint256 limit;
        if (!lastSellInWindow[recipient] || isSticky()) {
            limit = (balanceOf(recipient) * tradingAnytimeLimit) / 100;
            lastSellInWindow[recipient] = false;
        } else {
            limit = (balanceOf(recipient) * tradingWindowLimit) / 100;
        }
        
        if (tradeLimit[recipient] + amount > limit)
            return false;
        tradeLimit[recipient] += amount;
        return true;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender != pair && sticky) require(checkSale(sender, amount), "Sticky pot!");

        if(!launched()){ require(sender == owner() || recipient == owner(), "Contract not launched yet."); }
    ...
    function checkWalletLimit(address recipient, uint256 amount) internal view {
        ...
        require(block.timestamp > lastBuy[recipient] + rateLimit, "Rate limit exceeded");
        require(!recipient.isContract(), "Direct buys only");
    }
    ...
    function swapBack(uint256 amount) internal swapping {
        ...
        uint256 amountDevCollection = amountBNB - (amountBNBLiquidity + amountBNBPrizePool + amountBNBMarketing + (amountBNB * reflectionFee) / totalBNBFee);
        
        if (rewardsActive && hasDistributor){
            try distributor.deposit{value: amountBNBPrizePool}() {} catch {}
        }
        
        marketingFeeReceiver.transfer(amountBNBMarketing);
        devFeeReceiver.transfer(amountDevCollection);
    ...
    function launch(uint256 _dead, uint256 _rate) external onlyOwner {
        require(_dead < 7);
    ...
    function setTradeLimits(uint256 halt, uint256 window, uint256 windowLimit, uint256 anytimeLimit, uint256 limitReset, bool _sticky) external onlyOwner {
        require(halt <= 7200 && window >= 60 && windowLimit > 1 && anytimeLimit >= 1 && limitReset <= 168 hours);
        tradingHalt = halt;
        tradingWindow = window;
        tradingWindowLimit = windowLimit;
        tradingAnytimeLimit = anytimeLimit;
        tradingLimitReset = limitReset;
        sticky = _sticky;
    }
