...
import "../../../interfaces/targetVaults/protocols/IRabbitBank.sol";
import "../../../interfaces/targetVaults/protocols/IRabbitFairLaunch.sol";
import "../TargetVault.sol";

contract TargetVaultRabbitNative is TargetVault {
    ...
    /**
     * @dev Rabbit Bank Address
     */
    IRabbitBank public rabbitBank;

    /**
     * @dev Rabbit Farm Address
     */
    IRabbitFairLaunch public rabbitFarm;
    ...
    constructor(
        ...
        rabbitFarm = IRabbitFairLaunch(_rabbitFarm);
        pid = _pid;
        ...
        rewardTokenToTokenPath = [rewardToken, wBNB, _token];

        rabbitBank = IRabbitBank(0xc18907269640D11E2A91D7204f33C5115Ce3419e);
    }
    ...
    function _deposit() internal virtual {
        ...
        if (_balance > 0) {
            ...
            uint256 _ibTokenBefore = ibToken.balanceOf(address(this));
            token.safeIncreaseAllowance(address(rabbitBank), _balance);
            rabbitBank.deposit(address(token), _balance);
            uint256 _ibTokenAmount = ibToken.balanceOf(address(this)).sub(_ibTokenBefore);

            ibToken.safeIncreaseAllowance(address(rabbitFarm), _ibTokenAmount);
            rabbitFarm.deposit(address(this), pid, _ibTokenAmount);
    ...
    function withdraw(uint256 _amount) external virtual onlyVault {
        if (_amount > 0) {
            uint256 _ibTokenBefore = ibToken.balanceOf(address(this));
            rabbitFarm.withdraw(address(this), pid, _amount);
            uint256 _ibTokenAmount = ibToken.balanceOf(address(this)).sub(_ibTokenBefore);

            uint256 _tokenBefore = token.balanceOf(address(this));
            rabbitBank.withdraw(address(token), _ibTokenAmount);
            uint256 _tokenAmount = token.balanceOf(address(this)).sub(_tokenBefore);
    ...
    function harvest() external virtual {
        require(address(msg.sender) == harvester, "TargetVaultRabbit(harvest): sender is not harvester");
        _harvest();
    }
    ...
    function _harvest() internal virtual {
        rabbitFarm.deposit(address(this), pid, 0);
        uint256 rewardAmt = IERC20(rewardToken).balanceOf(address(this));
        if (rewardAmt > 0) {
            _collectFees();
            _deposit();
            emit Harvested(rewardAmt);
        }
    }
    ...
    function emergencyWithdrawAll() external virtual onlyOwner {
        ...
        rabbitFarm.withdraw(address(msg.sender), pid, _balance);
        rabbitBank.withdraw(address(token), ibToken.balanceOf(address(this)));
    ...
    function vaultBalance() public view virtual returns (uint256) {
        (uint256 _amount, , , ) = rabbitFarm.userInfo(pid, address(this));
        return _amount;
    }
    ...
    function balanceOfToken() public view virtual returns (uint256) {
        uint256 _tokenBalance = vaultBalance().mul(rabbitBank.totalToken(address(token))).div(ibToken.totalSupply());
        return availableBalance().add(_tokenBalance);
    }
    ...
    function setRewardToken(address _rewardToken) public onlyOwner {
        require(_rewardToken != address(0), "TargetVault(setRewardToken): reward token cannot be zero address");

        rewardToken = _rewardToken;

        rewardTokenToTokenPath = [rewardToken, wBNB, address(token)];

        emit RewardTokenChanged(buyBackToken);
    }
}
...
interface IRabbitBank {
    function banks(address _token) external view returns (address _tokenAddr, address _ibTokenAddr);

    function deposit(address _token, uint256 _amount) external payable;

    function withdraw(address _token, uint256 _pAmount) external;

    function totalToken(address _token) external view returns (uint256);
}

// SPDX-License-Identifier: MIT
pragma solidity >=0.7.0 <0.8.0;

interface IRabbitFairLaunch {
