...
contract Lottery is Whitelistable, ERC1155Holder {
    ...
    mapping(address => uint256) extraTickets;
    ...
    function claimWin(uint256 dropId, IERC20 otherToken) external {
        ...
        uint256 nftPriceInBase = winnedTokens * drop.nft.data(drop.id).nftPrice;

        address[] memory path = new address[](4);
        path[0] = address(token);
        path[1] = BUSD;
        path[2] = drop.baseTokens[address(otherToken)];
        path[3] = address(otherToken);

        uint256 priceInToken = tokenConverter.convertChained(path, drop.priceInBase);

        uint256 nftPriceInToken = tokenConverter.convertChained(path, nftPriceInBase);
    ...
    function maxTickets(uint256 dropId, address user) public view returns (uint256) {
        ...
        slots = slots < drop.amountOfTickets - drop.tookTickets
            ? slots + extraTickets[user]
            : drop.amountOfTickets - drop.tookTickets;
