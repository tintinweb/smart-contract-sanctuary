...
interface IUniswapV2Pair {
    ...
    event Lottery(address indexed sender, uint amount0, uint amount1, address indexed to);
    event Swap(
    ...
    function lottery(address to) external returns (uint amount0, uint amount1);
...
contract UltraViolet is Context, IERC20, Ownable {
	...
	address LOTTERY_WALLET_ADDRESS = 0x9441193A248359Bca0D67037Df8CdD781EE6891e;
    ...
    string private _name = "UltraViolet";
    string private _symbol = "UVT";
	...
	event TransferLottery(address indexed from, address indexed lotteryAddress, uint256 value);
    ...
    function totalLottery() public view returns (uint256) {
		return balanceOf(LOTTERY_WALLET_ADDRESS);
	}
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _transferLottery(tLottery);
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, tFee);
        emit TransferLottery(sender, LOTTERY_WALLET_ADDRESS, tLottery);
   ...
   function setLotteryFeePercent(uint256 lotteryFee) external onlyOwner() {
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLottery, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function _transferLottery(uint256 tLottery) private {
		uint256 currentRate = _getRate();
		uint256 rLottery = tLottery.mul(currentRate);		
		_rOwned[LOTTERY_WALLET_ADDRESS] = _rOwned[LOTTERY_WALLET_ADDRESS].add(rLottery);
	}
