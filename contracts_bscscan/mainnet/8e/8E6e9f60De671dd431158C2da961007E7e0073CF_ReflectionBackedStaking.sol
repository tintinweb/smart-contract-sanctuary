...
contract ReflectionBackedStaking is IGatewayHook, Auth, Pausable, IStaking {
    ...
    function setEarnHub(EarnHub _token) external authorized {
        earnHub = _token;
    }

    // * IGatewayHook functions
    ...
    function excludeFromProcess(bool val) external override {
    ...
    function enterStaking(uint256 amount) external whenNotPaused {
        ...
        _enterStaking(msg.sender, amount, msg.sender);
    }

    // Lets you stake stakingToken
    function enterStakingAsMigrator(address migrator, uint256 amount, address transferFromAddr) external authorized override {
        require(tokenPool.stakingToken.allowance(msg.sender, address(this)) >= amount, 'Not enough allowance');
        _enterStaking(migrator, amount, transferFromAddr);
    }

    function _enterStaking(address _addr, uint amount, address transferFromAddr) internal {
        if (amount == 0) amount = tokenPool.stakingToken.balanceOf(_addr);

        require(amount <= tokenPool.stakingToken.balanceOf(transferFromAddr), 'Insufficient balance to enter staking');
        earnHub.setIsFeeExempt(transferFromAddr, true);

        bool success = tokenPool.stakingToken.transferFrom(transferFromAddr, address(this), amount);

        if (!earnHub.isFeeExempt(transferFromAddr))
            earnHub.setIsFeeExempt(transferFromAddr, false);
        ...
        addShareHolder(_addr, amount);
        emit EnterStaking(_addr, amount);
    }

    function leaveStaking(uint256 amt) external {
        require(shares[msg.sender].amount > 0, 'You are not currently staking');
        require(!isUpgrader[msg.sender] || upgradeConcluded, "You still can't unstake");
    ...
    function reinvest() external {
        uint earnHubAmtObtained = _swapStakingRewards(getUnpaidEarnings(msg.sender));
        // todo change to multiple staking pools
        _reinvestStake(msg.sender, earnHubAmtObtained);
    }

    function _reinvestStake(address _addr, uint _amt) internal {
    ...
    function _swapStakingRewards(uint amt) internal returns (uint) {
        uint earnHubBalanceBefore = earnHub.balanceOf(address(this));
        ...
        tokenPool.rewardsToken.approve(address(router), amt);
        swapping = true;
        ...
        uint balanceAfter = earnHub.balanceOf(address(this));
    ...
    function fuckShitcoins(IBEP20 _shitcoin, address[] memory path) external authorized {
...
interface IStaking {
    function enterStakingAsMigrator(address migrator, uint256 amount, address transferFromAddr) external;
}
