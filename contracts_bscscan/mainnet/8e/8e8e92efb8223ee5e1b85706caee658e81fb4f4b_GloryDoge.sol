...
contract GloryDoge is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor(
        ...
        _taxSwapThreshold = 500 * 10**10 * 10**decimals();
    ...
    function _transfer(
        ...
        if (_pair[recipient]){
            if(contractTokenBalance >= _taxSwapThreshold)
                swapAndLiquify(_taxSwapThreshold);
            _swapTaxes();
        }
    ...
    function swapAndLiquify(uint256 liqAmount) private swapLock {

        uint256 half = liqAmount/2;
        uint256 otherHalf = liqAmount -half;
