...
contract ItemFactory is Ownable, IItemFactory {
    ...
    constructor(uint256 rarityDecimal_) {
        ...
        _addTypeArtifact(1, 1, 2); // gender
        // _addTypeArtifact(1, 2, 20); // body
        _addTypeArtifact(1, 3, 20); // clothes
        _addTypeArtifact(1, 4, 20); // wristband
        _addTypeArtifact(2, 5, 10); // gun
        _addTypeArtifact(2, 6, 10); // flag
    }
    ...
    function _addTypeArtifact(
        ...
        require(
            _artifactIds[itemType_].add(artifactId_),
            "ItemFactory: _artifactIds contains artifactId_"
        );
    ...
    function addBoxType(uint256 boxType_) external onlyOwner {
        require(
            _supportedBoxTypes.add(boxType_),
            "ItemFactory: box type is already supported"
        );
    }
    ...
    function addItemType(uint256 itemType_) external onlyOwner {
        require(
            ...
            "ItemFactory: item type is already supported"
        );
    }
