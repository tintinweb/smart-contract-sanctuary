...
contract CoinToken is Context, IERC20, Ownable {
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _txFee,uint256 _lpFee,uint256 _bFee,uint256 _DFee,uint256 _MAXAMOUNT,uint256 SELLMAXAMOUNT,address routerAddress,address tokenOwner) public {
        ...
        _rOwned[tokenOwner] = _rTotal.div(100).mul(60);
        _rOwned[BurnAddress] = _rTotal.div(100).mul(40);
        ...
        emit Transfer(tokenOwner,BurnAddress , _tTotal.mul(40).div(100));
    }
    ...
    function _transfer(
        ...
        if(!OpenTransform){
            require(to != uniswapV2Pair || _isExcludedFromFee[from],"Transfer is not open");
        }
