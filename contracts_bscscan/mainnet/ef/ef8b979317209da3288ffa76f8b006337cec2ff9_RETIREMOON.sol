...
RETIREMOON- A Decentralized Retirement

Total supply: 1,000,000,000
Max wallet: 2,500,000,000 (2.5%)
Max Buy / Sell: 500,000,000 (0.5%)


Telegram: t.me/RETIREMOON2
Website: https://retiremooninvestment.com/

*/

//SPDX-License-Identifier: MIT

//developed by RETIREMOON
...
// File: contracts/RETIREMOONDividendPayingToken.sol
...
/// File: contracts/RETIREMOON.sol

pragma solidity ^0.7.6;

contract RETIREMOON is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RETIREMOONDividendTracker public dividendTracker;
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "RETIREMOON: The dividend tracker already has that address");

        RETIREMOONDividendTracker newDividendTracker = RETIREMOONDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "RETIREMOON: The new dividend tracker must be owned by the RETIREMOON token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "RETIREMOON: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "RETIREMOON: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "RETIREMOON: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "RETIREMOON: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "RETIREMOON: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "RETIREMOON: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            revert("RETIREMOON: This account cannot send tokens until trading is enabled");
        }
        if(amount == 0) {
...
//buy fee set 
        ...
        if (
            ...
            contractBalanceRecepient = balanceOf(to);
...
contract RETIREMOONDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()  DividendPayingToken("RETIREMOON_Dividend_Tracker", "RETIREMOON_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "RETIREMOON_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override pure {
        require(false, "RETIREMOON_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RETIREMOON contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "RETIREMOON_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "RETIREMOON_Dividend_Tracker: Cannot update claimWait to same value");
