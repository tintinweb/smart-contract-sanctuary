...
/**
 *
$CAKEPUPPY is a Multi-Dividend Auto-Reflection BSC Token
With 10% Rewards Paid out in $CAKE and $DOGE EVERY HOUR! (Depending on Volume).

Total Tax: 19%
10% Auto-Reflections (5% $CAKE & 5% $DOGE)
4% Auto-Liquidity
5% Marketing

******* The CakePuppies Game **********
The CakePuppies Game is a simple runner game! You play as our adorable Shiba Inu Puppy named Doh! 
Control Doh through multiple challenging environments and collect as many Coins as you can without 
getting hit by any enemies or traps on each level and World during game play. CakePuppies Runner 
Game is also a Play & Earn Crypto Game. The more levels you complete and points you earn, the 
more you are rewarded with $CAKEPUPPY Tokens. The more $CAKEPUPPY Tokens you hold, 
the more $CAKE and $DOGE you earn passively.

Read More at: https://CakePuppies.com

Join Our Community: t.me/CakePuppiesCommunity
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0x5be7bE74108D9A4A996dAdFA6a46aAAF6AAB87Fd, "Only owner can change dividend contract address");
...
contract CakePuppies is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Cake Puppies", "CAKEPUPPY") {
    	cakeDividendTracker = new CakeDividendTracker();
    	dogeDividendTracker = new DogeDividendTracker();

    	marketingWallet = 0x5a823F8cdFbB33570bd882121d47d6E1ed7965eA;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "CakePuppies: The marketing wallet is already this address");
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "CakePuppies: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "CakePuppies: The new dividend tracker must be owned by the CakePuppies token contract");
    ...
    function updateDogeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dogeDividendTracker), "CakePuppies: The dividend tracker already has that address");

        DogeDividendTracker newDogeDividendTracker = DogeDividendTracker(payable(newAddress));

        require(newDogeDividendTracker.owner() == address(this), "CakePuppies: The new dividend tracker must be owned by the CakePuppies token contract");
    ...
    function updateCakeDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "CakePuppies: Fee must be less than 6%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "CakePuppies: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "CakePuppies: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CakePuppies: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "CakePuppies: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "CakePuppies: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "CakePuppies: Trading has not started yet");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("CakePuppies_CAKE_Dividend_Tracker", "CakePuppies_CAKE_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "CakePuppies_CAKE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "CakePuppies_CAKE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CakePuppies contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CakePuppies_CAKE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CakePuppies_CAKE_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DogeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("CakePuppies_Doge_Dividend_Tracker", "CakePuppies_Doge_Dividend_Tracker", 0xbA2aE424d960c26247Dd6c32edC70B295c744C43) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "CakePuppies_Doge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "CakePuppies_Doge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CakePuppies contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CakePuppies_Doge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CakePuppies_Doge_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
