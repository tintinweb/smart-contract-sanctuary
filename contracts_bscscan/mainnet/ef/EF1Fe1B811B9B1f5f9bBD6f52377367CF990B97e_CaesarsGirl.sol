...
Telegram: t.me/caesarsgirl
Twitter: twitter.com/CaesarsGirlBsc
Website: CaesarsGirlBsc.com
...
library Address {
     ...
     * The Developer of this contract is a 3rd party and not directly involved with GemzAddicted.
     * _Available since v3.4._
     */
     ...
     * The outcome and/or results of this project is in no shape or form the responsibility of the contract developer.
...
// CHANGE NEEDED
contract CaesarsGirl is IBEP20, Ownable {
    ...
    string private constant _name = "CaesarsGirl | t.me/caesarsgirl";
    string private constant _symbol = "CaesarsGirl";
    uint8 private constant _decimals = 9;
    uint256 private constant _tTotalSupply = 100000000 * 10 ** _decimals; // 100M Total Supply
    uint256 private _buyMaxWalletSize = 2000000 * 10 ** _decimals; // 2M max wallet per person - 2% of total supply
	uint256 private _buyMaxTxAmount = 2000000 * 10 ** _decimals; // 2M max buy per transaction - 2% of total supply
    uint256 private _sellMaxTxAmount = 1000000 * 10 ** _decimals; // 1M max sell per transaction - 01% of total supply
    ...
    uint256 private _selldevFee = 6;
    
    
    uint256 private constant numTokensSellToAddToLiquidity = 50000 * 10 ** _decimals; // 50k to split
    ...
    function setTotalBuyFees(uint256 taxFee, uint256 liquidityFee, uint256 devFee) external onlyOwner() {
        _buyTaxFee = taxFee;
        _buyLiquidityFee = liquidityFee;
        _buydevFee = devFee;
    }
    
    function setTotalSellFees(uint256 taxFee, uint256 liquidityFee, uint256 devFee) external onlyOwner() {
        _sellTaxFee = taxFee;
        _sellLiquidityFee = liquidityFee;
        _selldevFee = devFee;
        require(_sellTaxFee + _sellLiquidityFee + _selldevFee <= 26);
    }
    
    function setMaxBuyTxLimit(uint256 amount) external onlyOwner {
        _buyMaxTxAmount = amount * 10 ** _decimals;
    }
    
    function setMaxSellTxLimit(uint256 amount) external onlyOwner {
        _sellMaxTxAmount = amount * 10 ** _decimals;
    }
   ...
   function setdevAddress(address payable devAddress) public onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            // set _buyMaxWalletSize to not exceed 2% max wallet
            uint256 heldTokens = balanceOf(to);
            require((heldTokens + amount) <= _buyMaxWalletSize || to == pancakePair || to == 0x000000000000000000000000000000000000dEaD,"Transfer amount exceeds the _buyMaxWalletSize.");
        }
