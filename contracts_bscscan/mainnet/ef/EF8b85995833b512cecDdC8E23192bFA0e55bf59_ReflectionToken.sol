...
contract ReflectionToken is IERC20 {
    ...
    uint256 private constant startingSupply = 1_000_000_000_000_000; // 1 Quadrillion

    uint8 private _decimals = 9;
    uint256 private _decimalsMul = _decimals;
    uint256 private _tTotal = startingSupply * (10 ** _decimalsMul);

    string constant _name = "CakeKingdom";
    string constant _symbol = "CAKEKINGDOM";
    ...
    uint256 public _totalFee = _liquidityFee + _reflectionFee + _marketingFee + _devFee + _buybackFee;
    uint256 public masterTaxDivisor = 10000;

    uint256 constant public maxReflectionFee = 1500;
    uint256 constant public maxLiquidityFee = 400;
    uint256 constant public maxMarketingFee = 400;
    uint256 constant public maxDevFee = 400;
    uint256 constant public maxBuybackFee = 400;
    ...
    address payable private _marketingWallet = payable(0xf490aC72B69Eaf0C1b59EFa4D38bA3B36a1B513D);
    address payable private _devWallet = payable(0x30b880B24b6204D078fACaaA943E83ACb317a657);
    address payable private _buybackWallet = payable(0x506b33dc29ae1853C89ce9731c3aaEe296420A51);
    ...
    function setTaxes(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee, uint256 devFee, uint256 buybackFee) external onlyOwner {
        require(liquidityFee <= maxLiquidityFee
                && reflectionFee <= maxReflectionFee
                && marketingFee <= maxMarketingFee
                && devFee <= maxDevFee
                && buybackFee <= maxBuybackFee);
        require(liquidityFee + reflectionFee + marketingFee + devFee + buybackFee <= 5000);
        _liquidityFee = liquidityFee;
        _reflectionFee = reflectionFee;
        _marketingFee = marketingFee;
        _devFee = devFee;
        _buybackFee = buybackFee;
        _totalFee = liquidityFee + reflectionFee + marketingFee + devFee + buybackFee;
    }
    ...
    function setBlacklistEnabled(address wallet, bool enabled) external onlyOwner {
        _isBlacklisted[wallet] = enabled;
    }
    ...
    function _finalizeTransfer(address from, address to, uint256 amount, bool takeFee) internal returns (bool) {
        ...
        if (sniperProtection){
            ...
            if (isSniperOrBlacklisted(from) || isSniperOrBlacklisted(to)) {
                revert("Sniper or Blacklisted address rejected.");
            }
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        ...
        uint256 amountBNBOut = amountBNB - (amountBNBLiquidity + amountBNBReflection);
        transferBNBOut(amountBNBOut);
    ...
    function transferBNBOut(uint256 amountBNB) internal {
        uint256 totalFees = _marketingFee + _buybackFee + _devFee;
        uint256 amountBNBMarketing = (amountBNB * _marketingFee) / totalFees;
        uint256 amountBNBBuyback = (amountBNB * _buybackFee) / totalFees;
        uint256 amountBNBDev = amountBNB - (amountBNBMarketing + amountBNBBuyback);
