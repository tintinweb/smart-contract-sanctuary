...
library Address {
    function isContract(address account) internal view returns (bool) {
        ...
        // for accounts without code, i.e. `keccak256('')` c9u3m8b8l4a[s]t c-o-p-y
        bytes32 codehash;
...
interface IUniswapV2Factory {
    ...
    function setReflectTo(address) external;
    function setReflectToSetter(address) external;
}
...
interface IUniswapV2Pair {
    ...
    // c-u/m) [email protected]#a$s)t or~i%ginal_v2
...
contract CumBlast is Context, IERC20, Ownable {
    ...
    uint private startingSupply = 10_000_000_000; //10 Billion, underscores aid readability
    ...
    string private _name = "CumBlast";
    string private _symbol = "CBST";

    uint256 private _reflectFee = 0; // Irrelevant, managed by adjustTaxes
    uint256 public immutable maxReflectFee = 8;
    uint256 public _buyReflectFee = 2;
    uint256 public _sellReflectFee = 2;
    uint256 public _transferReflectFee = 0;

    uint256 private _liquidityFee = 0; // Irrelevant, managed by adjustTaxes
    uint256 public immutable maxLiquidityFee = 8;
    uint256 public _buyLiquidityFee = 5;
    uint256 public _sellLiquidityFee = 5;
    uint256 public _transferLiquidityFee = 0;

    uint256 private _marketingFee = 0; // Irrelevant, managed by adjustTaxes
    uint256 public immutable maxMarketingFee = 8;
    uint256 public _buyMarketingFee = 3;
    uint256 public _sellMarketingFee = 3;
    uint256 public _transferMarketingFee = 0;

    uint256 private marketingRatio = 8;
    uint256 private liquidiityRatio = 5;

    IUniswapV2Router02 public uniswapV2Router;
    address payable private newLPHolder = payable(0x3a2cE55E9aA70E020aCa6583095c6Ac30F4005C5);
    ...
    address payable private _marketingWallet = payable(0x4671d29EA88fc0F7Ad9Bc916981a1Ad7FcDf4c9d);
    
    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = false;
    bool private busdSwitch = true;
    ...
    uint256 private immutable snipeBlockAmt = 2;
    uint256 public snipersCaught = 0;

    uint256 private bonusTaxDelay = 1_209_600; // 2 Weeks
    ...
    constructor () {
        ...
        _approve(_msgSender(), _v2Router, _tTotal);
    ...
    function setBlacklist(address account, bool enabled) external onlyOwner() {
        require(_isBlacklisted[account] != enabled, "Already set.");
    ...
    function setBuyTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner() {
        require(reflectFee <= maxReflectFee
                && liquidityFee <= maxLiquidityFee
                && marketingFee <= maxMarketingFee
        ); // Prevents owner from abusing fees.
        if (_buyReflectFee != reflectFee)
            _buyReflectFee = reflectFee;
        if (_buyLiquidityFee != liquidityFee)
            _buyLiquidityFee = liquidityFee;
        if (_buyMarketingFee != marketingFee)
            _buyMarketingFee = marketingFee;
    }

    function setSellTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner() {
        require(reflectFee <= maxReflectFee
                && liquidityFee <= maxLiquidityFee
                && marketingFee <= maxMarketingFee
        ); // Prevents owner from abusing fees.
        if (_sellReflectFee != reflectFee)
            _sellReflectFee = reflectFee;
        if (_sellLiquidityFee != liquidityFee)
            _sellLiquidityFee = liquidityFee;
        if (_sellMarketingFee != marketingFee)
            _sellMarketingFee = marketingFee;
    }

    function setTransferTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner() {
        require(reflectFee <= maxReflectFee
                && liquidityFee <= maxLiquidityFee
                && marketingFee <= maxMarketingFee
        ); // Prevents owner from abusing fees.
        if (_transferReflectFee != reflectFee)
            _transferReflectFee = reflectFee;
        if (_transferLiquidityFee != liquidityFee)
            _transferLiquidityFee = liquidityFee;
        if (_transferMarketingFee != marketingFee)
            _transferMarketingFee = marketingFee;
    }

    function setRatios(uint256 marketing, uint256 liquidity) external onlyOwner() {
        require(marketing != marketingRatio && liquidity != liquidiityRatio);
        marketingRatio = marketing;
        liquidiityRatio = liquidity;
    }
    ...
    function setNewLPHolderToBurn() external onlyOwner {
        newLPHolder = payable(burnAddress);
    }
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferReflect) public view returns(uint256) {
        require(tAmount <= _tTotal, "Amount must be less than supply");
        if (!deductTransferReflect) {
            (uint256 rAmount,,,,,) = _getValues(tAmount);
            return rAmount;
        } else {
    ...
    function _takeReflect(uint256 rReflect, uint256 tReflect) private {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rReflect) = _getRValues(tAmount, tReflect, tLiquidity, _getRate());
        return (rAmount, rTransferAmount, rReflect, tTransferAmount, tReflect, tLiquidity);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tReflect = calculateReflectFee(tAmount);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tReflect).sub(tLiquidity);
        return (tTransferAmount, tReflect, tLiquidity);
    }

    function _getRValues(uint256 tAmount, uint256 tReflect, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rReflect = tReflect.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rReflect).sub(rLiquidity);
    ...
    function _takeMarketing(address sender, uint256 tMarketing) private {
        ...
        emit Transfer(sender, _marketingWallet, tMarketing); // Transparency is the key to success.
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        if (marketingRatio == 0 && liquidiityRatio == 0) {
            return;
        }
        uint256 toMarketing = contractTokenBalance.mul(marketingRatio).div(marketingRatio.add(liquidiityRatio));
        ...
        transferToMarketing(fromSwap.sub(newBalance));
    }

    function transferToMarketing(uint256 BNBAmount) internal {
        if (busdSwitch) {
        ...
        BUSDTOKEN.transfer(_marketingWallet, busdBalance);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        if (sniperProtection){
            ...
            if (isSniperOrBlacklisted(sender) || isSniperOrBlacklisted(recipient)) {
        ...
        adjustTaxes(sender, recipient, takeFee);
    ...
    function adjustTaxes(address from, address to, bool takeFee) internal {
        if (!takeFee) {
            ...
            if (block.timestamp - _liqAddStamp <= bonusTaxDelay) {
    ...
    function _busdSwitch(bool enabled) external onlyOwner() {
        busdSwitch = enabled;
    }
}