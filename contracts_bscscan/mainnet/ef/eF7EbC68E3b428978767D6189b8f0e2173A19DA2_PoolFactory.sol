...
contract PoolFactory is Ownable {
    ...
    address public immutable coinflect;
    uint public lifeTime = 60 days;
    uint public claimCycle = 24 hours;
    address public feeRecipient;

    PoolInfo[] public pools;

    constructor(address _coinflect) public {
        coinflect = _coinflect;
        feeRecipient = msg.sender;
    }

    function setLifeTime(uint _lifeTime) external onlyOwner {
        lifeTime = _lifeTime;
    }

    function setClaimCycle(uint _cycle) external onlyOwner {
        claimCycle = _cycle;
    }
    ...
    function deploy(string memory _name, address _asset, bool _invert) external onlyOwner {
        CoinflectStaking pool = _invert==false ? (new CoinflectStaking(coinflect, _asset)) : (new CoinflectStaking(_asset, coinflect));

        pool.setEndTime(block.timestamp + lifeTime);
        pool.setRewardCycle(claimCycle);
        pool.setFeeRecipient(feeRecipient);

        pools.push(PoolInfo({name: _name, pool: address(pool)}));
