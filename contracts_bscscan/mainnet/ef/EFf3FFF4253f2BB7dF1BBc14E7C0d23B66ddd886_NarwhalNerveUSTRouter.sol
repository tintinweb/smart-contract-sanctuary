...
// File: contracts/interfaces/INerveSwap.sol
...
// File: contracts/routes/NarwhalNerveUSTRouter.sol
...
contract NarwhalNerveUSTRouter is INarwhalTradingRoute, WhitelistedRole, ReentrancyGuard {
    using SafeERC20 for IERC20;

    INerveSwap public constant swapPool = INerveSwap(0x2dcCe1586b1664f41C72206900e404Ec3cA130e0);
    ...
    IERC20 public constant ust = IERC20(0x55d398326f99059fF775485246999027B3197955); // 0
        ...
        returns(uint256 _destAmount)
    {
        ...
        j = _dest == usdt ? 1 : j;
        j = _dest == ust ? 0 : j;
        require(i != uint8(-1) && j != uint8(-1), "tokens're not supported!");

        uint256 balanceBefore = _dest.balanceOf(address(this));
        _src.safeTransferFrom(msg.sender, address(this), _srcAmount);
        _src.safeApprove(address(swapPool), _srcAmount);

        swapPool.swapUnderlying(i, j, _srcAmount, amountOutMin, deadline);
        ...
        returns(uint256 _destAmount)
    {
        ...
        return swapPool.calculateSwapUnderlying(i, j, _srcAmount);
    }
}