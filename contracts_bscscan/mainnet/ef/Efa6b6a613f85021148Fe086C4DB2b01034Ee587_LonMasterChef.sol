...
contract LonMasterChef is Ownable, ReentrancyGuard {
    ...
    // Used to get the amount of tokens a wallet can deposit on the exclusive pool.
    ...
    mapping(IBEP20 => mapping(bool => bool)) public poolExistence;
    modifier nonDuplicated(IBEP20 _lpToken, bool _exclusive) {
        require(poolExistence[_lpToken][_exclusive] == false, "nonDuplicated: duplicated");
        _;
    }
    ...
    function add(
        ...
        bool _exclusive,
        bool _withUpdate
    ) public onlyOwner nonDuplicated(_lpToken, _exclusive) {
        ...
        poolExistence[_lpToken][_exclusive] = true;
        poolInfo.push(
            PoolInfo({
                lpToken: _lpToken,
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 wsReward =
                multiplier.mul(wsPerBlock()).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 tokensPerBlock = wsPerBlock();
    ...
    function deposit(
        ...
        // Only users who participated on the presale can access to the exclusive native pool up to the amount of tokens bought
        if (pool.exclusive && address(presale) != address(0)) {
            require(presale.tokensBought(msg.sender) >= user.amount + _amount, "deposit: exceeds presale allowance");
        }
    ...
    // Allows the owner to update the presale contract.
    function setPresale(IPresale _presale) external onlyOwner {
        presale = _presale;
    }
