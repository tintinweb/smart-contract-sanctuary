...
contract PinADA is ERC20, Ownable {
    ...
    uint256 tokenTotalSupply = 1_000_000_000_000_000 * (10**18);
    ...
    // REDISTRIBUTION
    uint256 redistributeMin = 300_000_000_000 * (10**18);
    address[] tokens = [
    ...
    function startRoundAll() external onlyOwner {
        for(uint256 i; i < holdersEFR.length; i++) {
            for(uint256 i2; i2 < tokens.length; i2++) {
                if(IERC20(tokens[i2]).balanceOf(address(this)) == 0 || holdersEFR[i] == address(0)) continue;
