...
library SphynxLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'da06c87ef146ec4e060e3c41fc6c8daf970250e7c88464135b1ed611482c3d50'   // Change to INIT_CODE_PAIR_HASH of Sphynx Factory
            ))));
    }
...
contract SphynxRouter01 is ISphynxRouter01 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            ISphynxPair(SphynxLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        uint amountIn,
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, SphynxLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = SphynxLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'SphynxRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapETHForExactTokens(uint amountOut, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'Sphynx: INVALID_PATH');
        amounts = SphynxLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'SphynxRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(SphynxLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint swapFee) public pure override returns (uint amountOut) {
        return SphynxLibrary.getAmountOut(amountIn, reserveIn, reserveOut, swapFee);
    }
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint swapFee) public pure override returns (uint amountIn) {
        return SphynxLibrary.getAmountOut(amountOut, reserveIn, reserveOut, swapFee);
    }
