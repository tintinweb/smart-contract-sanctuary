 ...
 * in bugs, because programmers k̸a̶r̷m̵a̵ ̵i̷n̶u̵ ̵o̶r̷i̶g̵i̶n̷a̴l̴ that an overflow raises an
...
library Address {
    function isContract(address account) internal view returns (bool) {
        ...
        // and 0xc5d2460186f7233c927e7db2dcc70K̸a̴r̵m̵a̶ ̵I̷n̵u̷ ̸C̴o̵p̵y̶2273b7bfad8045d85a470 is returned
...
contract KarmaInu is Context, IERC20, Ownable {
    ...
    uint private startingSupply = 2_000_000; //2 Million, underscores aid readability
    ...
    string private _name = "Karma Inu";
    ...
    uint256 public _marketingFee = 550; // All taxes are divided by 100 for more accuracy
    uint256 private _previousMarketingFee = _marketingFee;
    uint256 private maxMarketingFee = 800;

    uint256 public _lotteryFee = 50; // All taxes are divided by 100 for more accuracy
    uint256 private _previousLotteryFee = _lotteryFee;
    uint256 private maxLotteryFee = 300;

    uint256 public _karmaFee = 400; // All taxes are divided by 100 for more accuracy
    uint256 private _previousKarmaFee = _karmaFee;
    uint256 private maxKarmaFee = 800;

    uint256 private masterTaxDivisor = 10000; // All tax calculations are divided by this number.

    uint256 private buyBackUpperLimit = 1 * 10**18;
    bool public buyBackEnabled = false;
    uint256 private buyBackBalanceDivisor = 100;
    uint256 private buyBackBalanceLimiter = 1 * 10**18;
    ...
    address payable private _marketingWallet = payable(0x8D9FC6845329c97da9c2A2fd1F39Ef053Dd00c68);
    address payable private _lotteryWallet = payable(0xeC9618A7FF7f90c2F34C199E42eF89c35a26A109);
    ...
    function setNewRouter(address newRouter) public onlyOwner() {
        ...
        lpPair = IUniswapV2Factory(_newRouter.factory())
    ...
    function setTaxes(uint256 reflectFee, 
                      uint256 liquidityFee, 
                      uint256 marketingFee, 
                      uint256 karmaFee, 
                      uint256 lotteryFee) 
    external onlyOwner() {
        // Fees are set to a master divisor of 10,000. This means 400 fee is 4%, and 40 is 0.4%, etc.
        require(reflectFee <= maxReflectFee
                && liquidityFee <= maxLiquidityFee
                && marketingFee <= maxMarketingFee
                && karmaFee <= maxKarmaFee
                && lotteryFee <= maxLotteryFee
                ); // Prevents owner from abusing fees.
        if (_reflectFee != reflectFee)
            _reflectFee = reflectFee;
        if (_liquidityFee != liquidityFee)
            _liquidityFee = liquidityFee;
        if (_marketingFee != marketingFee)
            _marketingFee = marketingFee;
        if (_karmaFee != karmaFee)
            _karmaFee = karmaFee;
        if (_lotteryFee != lotteryFee)
    ...
    function setLotteryWallet(address payable newWallet) external onlyOwner {
        require(_lotteryWallet != newWallet, "Wallet already set!");
        _lotteryWallet = payable(newWallet);
    }
    ...
    function setBuyBackBalanceDivisor(uint256 divisor) external onlyOwner() {
        buyBackBalanceDivisor = divisor;
    }

    function setBuyBackBalanceLimiter(uint256 value, uint256 multiplier) external onlyOwner() {
        buyBackBalanceLimiter = value * (10**multiplier);
    }
    ...
    function excludePresaleAddresses(address router, address presale) external onlyOwner {
        require(!presaleCheck, "Function can only be used once.");
...
,        address to,
        ...
        if (!inSwapAndLiquify
            ...
            if (buyBackEnabled && balance > buyBackBalanceLimiter) {
                if (balance > buyBackUpperLimit)
                    balance = buyBackUpperLimit;
                
                buyBackTokens(balance.div(buyBackBalanceDivisor));
            }
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        if (_marketingFee + _liquidityFee + _karmaFee + _lotteryFee == 0)
            return;
        uint256 toKarma = contractTokenBalance.mul(_karmaFee).div(_marketingFee.add(_liquidityFee).add(_karmaFee).add(_lotteryFee));
        uint256 toLottery = contractTokenBalance.mul(_lotteryFee).div(_marketingFee.add(_liquidityFee).add(_karmaFee).add(_lotteryFee));
        uint256 toMarketing = contractTokenBalance.mul(_marketingFee).div(_marketingFee.add(_liquidityFee).add(_karmaFee).add(_lotteryFee));
        uint256 toLiquify = contractTokenBalance.sub(toMarketing.add(toKarma).add(toLottery));
        ...
        // swap tokens for ETH
        uint256 toSwapForEth = half.add(toMarketing).add(toLottery).add(toKarma);
        ...
        uint256 karmaBalance = fromSwap.mul(toKarma).div(toSwapForEth);
        ...
        uint256 afterLiquify = fromSwap.sub(newBalance).sub(karmaBalance);
        transferEthOut(afterLiquify);
    }
    ...
    function transferEthOut(uint256 ethAmount) internal {
        uint256 toLottery = ethAmount.mul(_lotteryFee).div(_marketingFee.add(_liquidityFee).add(_karmaFee).add(_lotteryFee));
        uint256 toMarketing = ethAmount.sub(toLottery);

        _lotteryWallet.transfer(toLottery);
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee.add(_marketingFee).add(_lotteryFee).add(_karmaFee)).div(masterTaxDivisor);
    }
