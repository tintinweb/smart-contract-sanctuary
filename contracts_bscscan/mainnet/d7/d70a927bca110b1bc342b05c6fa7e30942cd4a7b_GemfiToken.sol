...
Decimals: 9
Token Name: Gemfi Token
Token Symbol: GUM

Marketing Wallet: 0xd4C9D7B161F33490Ce833226557a63F7B5baB09E
...
This contract has a hard-coded fee limit. The total fees can never be set above 12% 
On launch a whitelist feature will be used, but this feature will later be switched off and locked. 
It is not possible for anybody to set the contract back to 'whitelist only' state once the setting has been locked.

The Cake LP tokens for the auto-liquidity that is made by the contract is sent to the burn address. 

There are anti-whale limits in place to prevent a wallet from holding too much of the supply.

About the way the fees are set on the contract...
...
So, this means that 500 is 5%  (250 would be 2.5%)
...
abstract contract Ownable is Context {
    ...
    constructor() {
        _owner = 0xd4C9D7B161F33490Ce833226557a63F7B5baB09E;
    ...
    // This modifier is only used once, it has the ability to change the wallet address for the 0.5% dev fee. It can not change any other settings.
    modifier onlyGEN() {
        require(
            0x60a4EA71566405Fd2717331465F184b5349a28EA == _msgSender(),
            "Only GEN can change this setting"
        );
        _;
    }
...
contract GemfiToken is Context, IERC20, Ownable {
    ...
    */
    ...
    address payable private Wallet_Marketing =
        payable(0xd4C9D7B161F33490Ce833226557a63F7B5baB09E);

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "Gemfi Token";
    ...
    uint256 public _BuyFeeRate_X100 = 50; //50%

    uint256 public _FeeReflection = 2; //5%
    ...
    uint256 public _FeeLiquidity_X100 = 200; // 2%
    uint256 public _FeeMarketing_X100 = 200; // 2%

    //Max fee limit - When changing fee the total must always be below this number - This is hard coded to protect buyers and increase confidence
    uint256 maxFee = 12; //Total fees on deployment is 9% max possible is 12%
    ...
    uint256 private _promoFee_X100 = _FeeMarketing_X100;
    uint256 public _FeesTotal =
        (_FeeMarketing_X100 + _FeeLiquidity_X100) / 100 + _FeeReflection;

    //fee for the auto LP and the all bnb wallets - used to process fees
    uint256 private _liquidityAndPromoFee =
        (_FeeMarketing_X100 + _FeeLiquidity_X100) / 100;
    ...
    */
    ...
    The total fees can not go above 12% 

    The reflection fee can't be lowered below 5% This is hard-coded into the contract and can't be changed. 
    ...
    */
    ...
    function fees_setFeeReflectionPercent(uint256 FeeReflection)
        external
        onlyOwner
    {
        //buyer protection - total fees can not be increased above 12% reflection can not be set below 5%
        uint256 limitCheck = (_FeeMarketing_X100 + _FeeLiquidity_X100) /
            100 +
            FeeReflection;
        if (limitCheck <= maxFee && FeeReflection >= 5) {
            _FeeReflection = FeeReflection;
            _FeesTotal =
                (_FeeMarketing_X100 + _FeeLiquidity_X100) /
                100 +
                _FeeReflection;
        }
    }
    ...
    function fees_setFeeLiquidityPercent_X100(uint256 FeeLiquidity_X100)
        external
        onlyOwner
    {
        //buyer protection - total fees can not be increased above 12%
        uint256 limitCheck = (_FeeMarketing_X100 + FeeLiquidity_X100) /
            100 +
            _FeeReflection;
    ...
    function fees_setFeeMarketing_X100(uint256 FeeMarketing_X100)
        external
        onlyOwner
    {
        //buyer protection - total fees can not be increased above 12%
        uint256 limitCheck = (FeeMarketing_X100 + _FeeLiquidity_X100) /
            100 +
            _FeeReflection;
        if (limitCheck <= maxFee) {
            ...
            _FeesTotal =
                ...
                100 +
                _FeeReflection;
            _promoFee_X100 = _FeeMarketing_X100;
            _liquidityAndPromoFee = (_FeeLiquidity_X100 + _promoFee_X100) / 100;
        }
    }
    ...
    */
    ...
    function Wallet_Update_BuyFeeRate(uint256 buyFeeRate_X100)
        public
        onlyOwner
    {
        _BuyFeeRate_X100 = buyFeeRate_X100;
    }
    ...
    */
    ...
    function process_set_NumberOfTransBeforeLiquifyTrigger(uint8 numTrans)
        public
        onlyOwner
    {
    ...
    */
    ...
    function safeLaunch_Whitelist_ADD(address[] calldata addresses)
        external
        onlyOwner
    {
        uint256 startGas;
    ...
    function safeLaunch_Whitelist_REMOVE(address[] calldata addresses)
        external
        onlyOwner
    {
        uint256 startGas;
    ...
    function safeLaunch_Blacklist_ADD(address[] calldata addresses)
        external
        onlyOwner
    {
        uint256 startGas;
    ...
    function safeLaunch_Blacklist_REMOVE(address[] calldata addresses)
        external
        onlyOwner
    {
        uint256 startGas;
    ...
    */
    ...
    function safeLaunch_LockSettings() public onlyOwner {
        require(
            !onlyWhitelist,
            "Cannot lock settings while restricted to whitelist only."
        );
        LockSettings = true;
    }
    ...
    function safeLaunch_OnlyWhitelist(bool _enabled) public onlyOwner {
        require(
            !LockSettings,
            "To protect buyers, this setting has been locked."
        );
    ...
    */
    ...
    function safeLaunch_setMaxTxPercent(uint256 maxTxPercent)
        external
        onlyOwner
    {
    ...
    function safeLaunch_setMaxWalletPercent(uint256 maxWallPercent)
        external
        onlyOwner
    {
        _maxWalletToken = _tTotal.mul(maxWallPercent).div(10**2);
    }

    //settting the maximum permitted wallet holding (in tokens)
    function safeLaunch_setMaxWalletTokens(uint256 maxWallTokens)
        external
        onlyOwner
    {
    ...
    {
        uint256 tFee = calculateFeeReflection(tAmount, isBuy);
        uint256 tLiquidity = calculateLiquidityAndPromoFee(tAmount, isBuy);
    ...
    function calculateFeeReflection(uint256 _amount, bool isBuy)
        private
        view
        returns (uint256)
    {
        if (isBuy) {
            _amount.mul(_FeeReflection).div(10**2).mul(_BuyFeeRate_X100).div(
                10**2
            );
        }
        return _amount.mul(_FeeReflection).div(10**2);
    }

    function calculateLiquidityAndPromoFee(uint256 _amount, bool isBuy)
        private
        view
        returns (uint256)
    {
        ...
        return _amount.mul(_liquidityAndPromoFee).div(10**2);
    }
    ...
    */
    ...
    function restoreAllFee() private {
        ...
        _FeesTotal =
            ...
            _FeeReflection;
        _promoFee_X100 = _FeeMarketing_X100;
        _liquidityAndPromoFee = (_FeeMarketing_X100 + _FeeLiquidity_X100) / 100;
    }
    ...
    function _transfer(
        ...
        */
        ...
        if (from != owner() && to != owner())
            require(
                amount <= _maxTxAmount,
                "You are trying to buy more than the max transaction."
            );
        ...
        */
        ...
        if (onlyWhitelist && from != owner()) {
            require(
                _isWhitelisted[to] && _isWhitelisted[from],
                "Contract is currently restricted to whitelisted wallets only."
            );
        }
    ...
    CakeLP tokens are created when liquidity is added. The following function makes these
    tokens and sends them directly to the burn address! So the developers can never 'pull the
    liquidity' - For the buyer, this is the safest option :) 
    ...
    */
    ...
    function process_Purge_BNBFromContract() public onlyOwner {
        ...
        sendToWallet(Wallet_Marketing, bnbAmount);
    }

    // Manual 'swapAndLiquify' Trigger (Helps to reduce the red on the chart)
    function process_Purge_SwapAndLiquify_Now(uint256 tokensToLiquify)
        public
        onlyOwner
    {
        //Do not trigger if already in swap
        ...
        if (tokensToLiquify == 0) {
            tokensToLiquify = tokensOnContract;
        } else if (tokensToLiquify > tokensOnContract) {
            tokensToLiquify = tokensOnContract;
        }
    ...
    */
    ...
    function _transferStandard(
        ...
        (
            ...
            uint256 tFee,
            uint256 tLiquidity
        ) = _getValues(tAmount, sender == uniswapV2Pair);
