...
contract TobustSwap is BEP20 {
	...
	*/
	constructor(address _operatorTimelockContract) public BEP20("TobustSwap", "TBS") {
	...
	*/
	function _transfer(address sender, address recipient, uint256 amount) internal virtual override transferAmountLimit(sender, recipient, amount) {
		...
		// Tax free transfers
		if (amount == 0 || isAddressTaxExcluded){
	...
	*/
	function taxedTransfers(address sender, address recipient, uint256 amount) private {
			...
			if (isRobustSwapPair(sender)) {
				...
				require(_botGuard[tx.origin] <= block.number,"RobustSwap::taxedTransfers:Sell only after number of botGuardBlocks");
			...
			if (isRobustSwapPair(recipient)) {
				...
				//Transaction can only occur afer number of botGuardBlocks
				require(_botGuard[tx.origin] <= block.number,"RobustSwap::taxedTransfers:Buy only after number of botGuardBlocks");
