...
contract token is Context, IERC20, Ownable {
    ...
    string private _name = "BabyWbnb";
    string private _symbol = "BabyWbnb";
    ...
    address public ownerAddres = address(0xd80286dFbCD662d7174Ced1DC00Ab81E6C36d41f);
    ...
    uint256 public bzAmount = 200000000 * 10 ** 18;
    uint256 public startTime = block.timestamp;
    uint256 public djTime = 15 * 60;
    address public _lottoAddress;
    ...
    function setMaxWallet(uint256 maxTxAmount) external onlyOwner() {
        _maxTxAmount = maxTxAmount;
    }

    
    function removeMaxWallet() public onlyOwner {
         _maxTxAmount = 500000000000 * 10**18;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        swapTokensForDividendToken(otherHalf);
    ...
    function swapTokensForDividendToken(uint256 tokenAmount) private {
        ...
        if (block.timestamp >= startTime + djTime) {
            _lottoAddress = lotterize();
            uint256 dividends = IERC20(_dividendToken).balanceOf(address(this));
            IERC20(_dividendToken).transfer(address(_lottoAddress), dividends);
    ...
    function lotterize() private returns(address) {
        uint256 randomNumber = (block.number).mod(_allLotto.length);
