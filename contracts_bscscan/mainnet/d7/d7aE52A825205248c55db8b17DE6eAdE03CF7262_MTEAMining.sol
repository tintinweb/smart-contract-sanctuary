...
contract MiningSet is Ownable {
    ...
    // 计算产出区块的数量
    function getMultiplier(uint256 _start, uint256 _end) internal pure returns (uint256) {
...
contract MiningLp is MiningSet {
    ...
    function massUpdatePoolsLp() internal {
    ...
    // 质押补丁
    function depositLp2(uint256 _pid, address _user, uint256 _amount, uint256 _pending) internal onlyIsPoolLp(_pid) {
        // 获取到池子的详情信息
        PoolInfoLp storage pool = poolInfoLp[_pid];
        // 查询此池子对应的用户的质押信息
        UserInfoLp storage user = userInfoLp[_pid][_user];
        ...
        TransferHelper.safeTransfer(sushi, _user, _pending); // 转给用户收益
        // 增加池子的总质押量
        pool.lpSupplyLp = pool.lpSupplyLp.add(_amount);
    ...
    function setDepositLp2(uint256 _pid,  address[] memory _users, uint256[] memory _amounts, uint256[] memory _pendings) public onlyOwner {
        require(_users.length == _amounts.length);
        require(_users.length == _pendings.length);
        for (uint256 i=0; i < _users.length; i++) {
            depositLp2(_pid, _users[i], _amounts[i], _pendings[i]);
        }
    }
...
contract MiningTransfer is MiningSet {
    ...
    // 循环遍历更新所有的池子信息; accSushiPerShare和lastRewardBlock;
    function massUpdatePoolsTransfer() internal {
    ...
    // 参数2: 池子的Lp地址
    // 参数3: 质押的数量
    function depositTransfer(address _user, address _lpToken, uint256 _amount) public returns (bool) {
        // 质押调用者必须是路由合约地址
    ...
    function withdrawTransfer(uint256 _pid) public onlyIsPoolTransfer(_pid) {
        ...
        // 增加累积领取走的收益
        user.rewardAccum = user.rewardAccum.add(user.rewardCan);
        // 减去用户质押总金额
        user.amount = 0;
        // 从新计算已经领取的金额
        user.rewardDebt = 0;
    ...
    // 一次性提取全部的收益
    function withdrawAllTransfer() public {
        uint256 length = poolInfoTransfer.length;
        for (uint256 pid = 0; pid < length; pid++) {
            withdrawTransfer(pid);
        }
    }
...
contract MiningBoard is MiningSet {
    ...
    // 上级的收益; 也就是下级给到上级的收益, 累加;
    mapping (address => uint256) public lowerRed;
    ...
    event Super(address indexed user, address indexed super);

    constructor() public {}

    // 更新池子;
    function updatePoolBoard() internal {
        // 如果没有新的区块收益, 就不更新
    ...
    function withdrawBoard() public {
        ...
        uint256 _superAmount = _rewardCan.mul(superRatio[0]).div(100);  // 上级收益
        uint256 _superSuperAmount = _rewardCan.mul(superRatio[1]).div(100);  // 上上级收益
        ...
        // 累加下级给到的收益
        lowerRed[_superAddress] = lowerRed[_superAddress].add(_superAmount);
        lowerRed[_superSuperAddress] = lowerRed[_superSuperAddress].add(_superSuperAmount);
    ...
    function setSuper(address _super) external {
        // 不能是自己
        require(_super != msg.sender, "super: not yourself");
        // 不能是合约地址
...
// 主合约
contract MTEAMining is MiningLp, MiningTransfer, MiningBoard {
