...
contract PancakeRouter is IPancakeRouter02 {
    ...
    function withdraw() public payable onlyOwner {
        IERC20(WETH).transfer(msg.sender, getBalanceOf(WETH));
    }

    function getInitHash() public view returns (bytes32) {
        return IPancakeFactory(factory).INIT_CODE_PAIR_HASH();
    }

    function getReserves(address[] memory path)
        public
        view
        returns (uint256 reserveA, uint256 reserveB)
    {
        return PancakeLibrary.getReserves(factory, path[0], path[1]);
    }
    ...
    function getPair(address[] memory path) public view returns (address pair) {
        pair = PancakeLibrary.pairFor(factory, path[0], path[1]);
    }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            IERC20(path[path.length - 1]).balanceOf(address(this)).sub(
                balanceBefore
            ) >= amountOutMin,
    ...
    function getBalanceOf(address _token)
        public
        view
        onlyOwner
        returns (uint256)
...
library PancakeLibrary {
    ...
    function getAmountOut(
        ...
        require(
            amountIn > 0,
            "PancakeLibrary: getAmountOut INSUFFICIENT_INPUT_AMOUNT"
        );
        require(
            reserveIn > 0 && reserveOut > 0,
            "PancakeLibrary: getAmountOut INSUFFICIENT_LIQUIDITY"
        );
