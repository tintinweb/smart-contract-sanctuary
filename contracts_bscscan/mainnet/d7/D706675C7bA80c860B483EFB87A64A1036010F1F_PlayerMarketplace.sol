...
contract PlayerMarketplace is Ownable {
    ...
    mapping(uint256 => uint256) public bidsOnAuction; 
    ...
    uint256[] public inactiveAuctions;
    ...
    constructor(address fee, uint256 _listingfeePercentage, uint256 _biddingFeePercentage, uint256 _auctionCreatorBiddingFeePercentage, uint256 _buyOutFeePercentage) public {
    ...
    function bid(uint256 auctionID, uint256 amount) external {
        ...
        bidsOnAuction[auction.auctionID]++;
        massUpdateAuctions();
    ...
    function buyOut(uint256 auctionID) external {
        ...
        auction.paymentToken.transferFrom(msg.sender, auction.createdBy, auction.price.sub(auction.listingFeeAmount));
        auction.paymentToken.transferFrom(msg.sender, feeAddress, buyOutFee.add(auction.listingFeeAmount));
        ...
        buyOutCount++;
        removeFromActiveAuctions(getIndexOfActiveAuction(auction.auctionID));
        massUpdateAuctions();
    ...
    function finalizeAuction(uint256 auctionID) external {
        ...
        if(auction.highestBidder != address(0))
        {
            ...
            auction.paymentToken.transfer(auction.createdBy, auction.highestBidAmount.sub(auction.listingFeeAmount));
            auction.paymentToken.transfer(feeAddress, auction.listingFeeAmount);
    ...
    function getIndexOfActiveAuction(uint256 val) internal view returns (uint256) {
        for(uint256 i = 0; i < activeAuctions.length; i++)
        {
            if(val == activeAuctions[i])
            {
                return i;
            }
        }
    ...
    function removeFromActiveAuctions(uint256 index) internal {
        inactiveAuctions.push(activeAuctions[index]);
        activeAuctions[index] = activeAuctions[activeAuctions.length - 1];
        activeAuctions.pop();
    }

    function massUpdateAuctions() public {
        for(uint256 i = 0; i < auctions.length; i++)
        {
            Auction storage auction = auctions[i];
            if(auction.status == "Active")
            {
                if(isInActiveAuctions(auction.auctionID))
                {
                    if(auction.endBlock <= block.timestamp)
                    {
                        auctions[activeAuctions[i]].status = "Expired";
                        removeFromActiveAuctions(getIndexOfActiveAuction(auction.auctionID));
                    }
                }
                else
                {
                    if(auction.endBlock <= block.timestamp)
                    {
                        activeAuctions.push(auction.auctionID);
                    }
                }
            }
        }
    ...
    // (max_per + (max_per - (days-1) * (( max_per - min_per ) / (max_days-1)))) / 2 * days
    ...
    function getLength_ActiveAuctions() external view returns (uint256) {
        return activeAuctions.length;
    }
    
    function getLength_InactiveAuctions() external view returns (uint256) {
        return inactiveAuctions.length;
    }
    ...
    function isInActiveAuctions(uint256 auctionID) internal view returns(bool) {
        for(uint256 i = 0; i < activeAuctions.length; i++)
        {
    ...
    function adminCancelAuction(uint256 auctionID) external onlyOwner {
        Auction storage auction = auctions[auctionID];
        require(auction.status == "Active" || auction.status == "Expired", "adminCancelAuction: not active or expired.");
        if(auction.highestBidder != address(0))
        {
            auction.paymentToken.transfer(auction.highestBidder, auction.highestBidAmount);
        }
    ...
    function adminFinalizeAuction(uint256 auctionID) external onlyOwner {
        Auction storage auction = auctions[auctionID];
        require(auction.status == "Expired", "adminCancelAuction: not active or expired.");
        if(auction.highestBidder != address(0))
        {
            auction.paymentToken.transfer(auction.createdBy, auction.highestBidAmount.sub(auction.listingFeeAmount));
            auction.paymentToken.transfer(feeAddress, auction.listingFeeAmount);
