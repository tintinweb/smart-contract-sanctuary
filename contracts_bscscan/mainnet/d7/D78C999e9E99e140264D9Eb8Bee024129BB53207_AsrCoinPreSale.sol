     ...
     if(currentRound==1){
       ...
       if(token>0){
          ...
          IERC20(tokenContract).transferFromPresale(msg.sender,token);
           uint256 bonus=0;
      if(msg.value>=0.05 ether && msg.value<0.15 ether){
        
         bonus=0;
          
      }
      else if(msg.value>=0.15 ether&&msg.value<0.3 ether){
         ...
         bonus=token.mul(40).div(100);
      }
      else if(msg.value>=10 ether){
            bonus=token.mul(60).div(100);
      }
