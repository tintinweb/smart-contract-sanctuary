...
contract FcnFarm is Ownable {
    ...
    event ToCashDividends(address indexed _account,uint256 _getAmount,uint256 userTotalFcn,uint256 userEarningsAmount);
    event ReceiveEarningsByCashDividends(address indexed _account,uint256 _userEarningsCashDividends);
    ...
    function receiveEarningsByCashDividends() public returns (bool) {
        // Data validation
        require(farmSwitchState,"-> farmSwitchState: Farm has not started yet.");
        uint256 userIndexInFarm = isJoinFarmSuccessIndex[msg.sender];
        uint256 userEarningsCashDividends = joinFarmOrders[userIndexInFarm].earningsCashDividends;
        require(userIndexInFarm>0,"-> isJoinFarmSuccessIndex: This address is not already participating in FARM.");
        require(userEarningsCashDividends>0,"-> userEarningsCashDividends: The recoverable income must be greater than 0.");

        fcnErc20Contract.safeTransfer(msg.sender,userEarningsCashDividends);// Transfer fnc to user address
        joinFarmOrders[userIndexInFarm].earningsCashDividends = 0;
        joinFarmOrders[userIndexInFarm].receiveCashDividendsTotal += userEarningsCashDividends;
        joinFarmOrders[userIndexInFarm].lastCashDividendsTime = block.timestamp;

        emit ReceiveEarningsByCashDividends(msg.sender,userEarningsCashDividends);// set log
        return true;// return result
    }

    function toCashDividends() public returns (bool) {
        // Data validation
        require(farmSwitchState,"-> farmSwitchState: Farm has not started yet.");
        require(block.timestamp>=nextCashDividendsTime,"-> nextCashDividendsTime: The start time has not been reached.");

        // Calculation of 1
        uint256 userTotalFcn = fcnErc20Contract.totalSupply().sub(fcnErc20Contract.balanceOf(address(0)));
        uint256 userEarningsAmount = 0;
        if(nowCashDividends>0){
              for(uint256 i=1;i<=joinFarmTotalCount;i++){
                  if(fcnErc20Contract.balanceOf(joinFarmOrders[i].account)>=joinFarmAmountLimit){
                      userEarningsAmount = fcnErc20Contract.balanceOf(joinFarmOrders[i].account).mul(1000000000000000).div(userTotalFcn).mul(nowCashDividends).div(1000000000000000);
                      joinFarmOrders[i].earningsCashDividends += userEarningsAmount;
                      nowCashDividends -= userEarningsAmount;
                  }
              }
              fcnErc20Contract.safeTransfer(address(0),nowCashDividends);// Transfer fnc to blackHole address
              nowCashDividends = 0;
        }
        nextCashDividendsTime += 1800;

        emit ToCashDividends(msg.sender,nowCashDividends,userTotalFcn,userEarningsAmount);// set log
        return true;// return result
    }
    ...
    function joinFarm() public returns (bool) {
        ...
        joinFarmOrders[joinFarmTotalCount] = JoinFarmOrder(joinFarmTotalCount,msg.sender,true,block.timestamp,0,0,0,0,0);// add JoinFarmOrder
    ...
    function farmDistribute() public returns (bool) {
        // Data validation
        require(farmSwitchState,"-> farmSwitchState: Farm has not started yet.");
        require(block.timestamp.sub(farmStartTime)>1800,"-> farmStartTime: farm time not reached.");
