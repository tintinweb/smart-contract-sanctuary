...
contract WastedWhitelist is AccessControl, ReentrancyGuard {
    ...
    struct MemberInfo {
        uint256 maxPacksSupply;
        uint256 bought;
    }
    struct MembersInfoControl {
    ...
    function buyWastedWarrior(uint256 amount, uint256 rarityPackage)
        ...
        payable
    {
        MemberInfo storage member = membersInfo[msg.sender];
        require(amount != 0, "WW: invalid amount");
        require(
            member.bought + amount <= member.maxPacksSupply,
            "WW: not eligible"
        );

        if (rarityPackage == uint256(IWastedWarrior.PackageRarity.PLASTIC)) {
            require(
                msg.value == wastedWarrior.getPlasticPackageFee() * amount,
                "WW: Not enough fee"
            );
        } else if (
            ...
            require(
                msg.value == wastedWarrior.getSteelPackageFee() * amount,
                "WW: Not enough fee"
            );
        } else if (
            ...
            require(
                msg.value == wastedWarrior.getGoldPackageFee() * amount,
                "WW: Not enough fee"
            );
        } else if (
            ...
            require(
                msg.value == wastedWarrior.getPlatinumPackageFee() * amount,
                "WW: Not enough fee"
            );
        }

        balance += msg.value;
        member.bought += amount;
        wastedWarrior.mintFor(msg.sender, amount, rarityPackage);
    }

    function addMembersToWhitelist(MembersInfoControl[] memory newMembers)
        external
        onlyRole(CONTROLLER_ROLE)
    {
        require(newMembers.length != 0, "WW: invalid");
        for (uint256 i = 0; i < newMembers.length; i++) {
            membersInfo[newMembers[i].addressMember] = newMembers[i].memberInfo;
        }
    }

    function updateMembersWhitelist(MembersInfoControl[] memory updateMembers)
        external
        onlyRole(CONTROLLER_ROLE)
    {
        require(updateMembers.length != 0, "WW: invalid");
        for (uint256 i = 0; i < updateMembers.length; i++) {
            require(
                membersInfo[updateMembers[i].addressMember].maxPacksSupply > 0,
                "WW: not found"
            );
            require(
                updateMembers[i].memberInfo.maxPacksSupply >=
                    membersInfo[updateMembers[i].addressMember].bought,
                "WW: max supply must be greater than bought"
            );
            membersInfo[updateMembers[i].addressMember] = updateMembers[i]
                .memberInfo;
        }
    }
