...
contract PSTARS is Context, IBEP20, Ownable { // change contract name
    ...
    string private _name = "PancakeStars"; // token name
    string private _symbol = "PSTARS"; // token symbol
    ...
    uint256 public _burnFee = 5; // burn fee 0.5%
    uint256 private _previousBurnFee = _burnFee; // burn fee

    uint256 public _reflectionFee = 100; //reflection fee 10%
    ...
    uint256 public _maxTxAmount = 10000 * 10**6 * 10**_decimals; // max allowed tokens tranfer per transaction
    uint256 public _minTokensSwapToAndTransferTo = 5 * 10**6 * 10**_decimals; // min token liquidity fee collected before swapandLiquify
    uint256 public _maxTokensPerAddress            = 10000 * 10**6 * 10**_decimals; // Max number of tokens that an address can hold
	...
	*/
    ...
    function removeAllFee() private {
        if(_burnFee == 0 && _reflectionFee == 0 && _liquidityFee == 0) return;
