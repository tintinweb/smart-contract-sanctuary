...
import "./TokenHolderManager.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";

contract TokenDelegator is Ownable {
    TokenHolderManager private _tokenHolderManager;
    IERC20 private _v1Token;

    constructor(TokenHolderManager tokenHolderManager, IERC20 v1Token) {
        _tokenHolderManager = tokenHolderManager;
        _v1Token = v1Token;
    }

    function delegateV1Tokens(address holder, uint256 amount) public {
        require(amount > 0, 'Should transfer some tokens');
        require(amount <= _v1Token.balanceOf(holder), 'Holder does not have enough tokens');

        if (_v1Token.transferFrom(holder, address(this), amount)) {
            _tokenHolderManager.addTokens(holder, amount);
        }
    }
    ...
    function getTotalDelegated() public view returns(uint256) {
        return _tokenHolderManager.getTotalTokens();
    }

    function getPercentageDelegated() public view returns(uint256) {
        return _tokenHolderManager.getTotalTokens() * 100 / _v1Token.totalSupply();
    }
}
...
abstract contract Allowable is Ownable {
    ...
    function allow(address allowable, bool enable) public onlyOwner {
        _allowables[allowable] = enable;
    }
}
