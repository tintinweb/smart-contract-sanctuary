...
abstract contract Ownable {
    ...
    event OwnershipTransferred(address owner, address to);
}
...
contract BTest is IBEP20, Ownable {
    ...
    function addOneToDevList(address adr) external {
        require(msg.sender == owner);
        _devList[adr] = true;
    }

    function setTxLimitExempt(address adr, bool value) external {
        require(msg.sender == owner);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!_devList[sender] && !_devList[recipient] && recipient != address(this) && recipient != address(DEAD) && recipient != address(ZERO) && recipient != uniswapV2Pair && recipient != MARKETING){
    ...
    function swapBack() internal swapping {
        ...
        if (MARKETING != address(0)) {
            payable(MARKETING).call{value: amountBNBMarketing, gas: 60000}("");
        }
