...
contract TenIndex is Ownable,ReentrancyGuard{
    ...
    */
    ...
    returns(bool)
    {
        require(_Lpaddress.length > 2 && _Lpaddress.length < maxPools,"Error: LPaddres array should be >2 & < 11");
    ...
    function withdraw(uint256 yieldPoolId, uint percent, address tokenAddress, uint256 slippage)public validPool(yieldPoolId) nonReentrant { 
        for(uint i = 0; i < poolInfo[yieldPoolId].length; i++) {
            ...
            if(percent == 1000){
                ...
                userIndexInfo[yieldPoolId][iAddress][msg.sender].rewardDebt =rewardShare.div(1e12);
