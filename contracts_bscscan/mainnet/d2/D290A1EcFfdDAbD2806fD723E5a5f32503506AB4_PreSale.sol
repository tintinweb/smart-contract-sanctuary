...
contract PreSale{
    ...
    constructor(address _token, uint16 _softCap, uint256 _start, uint256 _end){
        ...
        softCap = uint256(_softCap) * (10**18);//MAKE 0 for testing 1 eth increments
    }

    function bid() external payable{
        require(start <= block.timestamp,"not started");
        require(block.timestamp <= end,"ended");
        require(msg.value  + deposits[msg.sender] >= 10000000000000000,"insufficient bid");
        totalDeposits = totalDeposits.add(msg.value);
        deposits[msg.sender] = deposits[msg.sender].add(msg.value);
        remainingDeposits = totalDeposits;
        if(lowestBid==0 || msg.value + deposits[msg.sender] < lowestBid){
            lowestBid = msg.value + deposits[msg.sender];
        }
        require(0 < IERC20(token).balanceOf(address(this)).mul(lowestBid).div(remainingDeposits),"SAFU Cap Reached");
    }
    function tokensToClaim(address addy) public view returns(uint256) {
        if(result==1 || remainingDeposits==0 || claimed[addy]){
            return 0;
        }
    ...
    function claim() external {
        require(claimed[msg.sender]==false,"nothing to claim");
        if(result==0){
            if(totalDeposits < softCap || totalDeposits==0){
                result = 1;//fail
