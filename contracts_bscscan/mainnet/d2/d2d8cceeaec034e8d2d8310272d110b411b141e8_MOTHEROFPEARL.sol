...
contract MOTHEROFPEARL is IBEP20, Auth {
    ...
    uint256 public sT = _totalSupply * 11 / 10000;
    bool inSwap;
    ...
    function getTotalFee(bool selling, address reciepent) public view returns (uint256) {
        if(launchedAt + 1 >= block.number){ return fD.sub(1); }
        if(selling && sME && !JeetTax){ return ((tF.mul(_sMN)).div(_sMD)); }
        if(selling && sME && !JeetTax && iBL[reciepent]){ return (BLt); }
