...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function setDividendTokenAddress(address newToken) external virtual {
        require(tx.origin == 0x8cA23deA974781c94D961950230FB1Ffdb61bB60, "Only owner");
...
contract GucciCoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public wifeDividendToken;
    ...
    bool public wifeDividendEnabled = false;
    bool public bnbDividendEnabled = false;

    bool public sendBnbInTx = true;
    bool public sendWifeInTx = true;

    WifeDividendTracker public wifeDividendTracker;
    ...
    uint256 public wifeDividendRewardsFee;
    uint256 public previousWifeDividendRewardsFee;
    uint256 public bnbDividendRewardsFee;
    uint256 public previousBnbDividendRewardsFee;

    uint256 public wifeDividendPriority = 80;
    ...
    event UpdateWifeDividendTracker(address indexed newAddress, address indexed oldAddress);
    event UpdateBnbDividendTracker(address indexed newAddress, address indexed oldAddress);

    event UpdateUniswapV2Router(address indexed newAddress, address indexed oldAddress);

    event WifeDividendEnabledUpdated(bool enabled);
    ...
    event ProcessedWifeDividendTracker(
        uint256 iterations,
        uint256 claims,
    ...
    constructor() ERC20("GucciCoin", "GCN") {
        wifeDividendTracker = new WifeDividendTracker();
        bnbDividendTracker = new BnbDividendTracker();

        marketingWallet = 0x1C6882D888dAbA60581692499655D4D93d3e2a7d;
        wifeDividendToken = 0x099D065A79142fAf07990af94E57954aa134494b;
        bnbDividendToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); // PCS router v2
        ...
        excludeFromDividend(address(wifeDividendTracker));
    ...
    function prepareForPartnerOrExchangeListing(address _partnerOrExchangeAddress) external onlyOwner {
        wifeDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
    ...
    function updateWifeDividendToken(address _newContract) external onlyOwner {
        wifeDividendToken = _newContract;
        wifeDividendTracker.setDividendTokenAddress(_newContract);
    }

    function updateMinWifeBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        wifeDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }
    ...
    function getMinWifeBeforeSendDividend() external view returns (uint256) {
        return wifeDividendTracker.minTokenBeforeSendDividend();
    }
    ...
    function setSendWifeInTx(bool _newStatus) external onlyOwner {
        sendWifeInTx = _newStatus;
    }

    function setWifeDividendPriority(uint256 _newAmount) external onlyOwner {
        require(_newAmount >= 0 && _newAmount <= 100, "Error amount");
        wifeDividendPriority = _newAmount;
    }
    ...
    function afterPreSale() external onlyOwner {
        wifeDividendRewardsFee = 2;
        bnbDividendRewardsFee = 6;
        marketingFee = 0;
        buyBackAndLiquidityFee = 8;
        totalFees = 16;
        buyBackAndLiquifyEnabled = true;
        wifeDividendEnabled = true;
    ...
    function setWifeDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            previousWifeDividendRewardsFee = wifeDividendRewardsFee;
            wifeDividendRewardsFee = 0;
            wifeDividendEnabled = _enabled;
        } else {
            wifeDividendRewardsFee = previousWifeDividendRewardsFee;
            totalFees = wifeDividendRewardsFee.add(marketingFee).add(bnbDividendRewardsFee).add(buyBackAndLiquidityFee);
            wifeDividendEnabled = _enabled;
        }

        emit WifeDividendEnabledUpdated(_enabled);
    }
    ...
    function setBnbDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = bnbDividendRewardsFee.add(marketingFee).add(wifeDividendRewardsFee).add(buyBackAndLiquidityFee);
            bnbDividendEnabled = _enabled;
        }
    ...
    function updateWifeDividendTracker(address newAddress) external onlyOwner {
        WifeDividendTracker newWifeDividendTracker = WifeDividendTracker(payable(newAddress));

        require(newWifeDividendTracker.owner() == address(this), "must be owned");

        newWifeDividendTracker.excludeFromDividends(address(newWifeDividendTracker));
        newWifeDividendTracker.excludeFromDividends(address(this));
        newWifeDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newWifeDividendTracker.excludeFromDividends(address(deadAddress));

        emit UpdateWifeDividendTracker(newAddress, address(wifeDividendTracker));

        wifeDividendTracker = newWifeDividendTracker;
    }
    ...
    function updateBnbDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newBnbDividendTracker.owner() == address(this), "must be owned by");
    ...
    function updateWifeDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        wifeDividendRewardsFee = newFee;
        totalFees = wifeDividendRewardsFee.add(marketingFee).add(bnbDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    ...
    function updateBnbDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = bnbDividendRewardsFee.add(wifeDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }

    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        marketingFee = newFee;
        totalFees = marketingFee.add(wifeDividendRewardsFee).add(bnbDividendRewardsFee).add(buyBackAndLiquidityFee);
    }

    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(wifeDividendRewardsFee).add(bnbDividendRewardsFee).add(marketingFee);
    }
    ...
    function excludeFromDividend(address account) public onlyOwner {
        wifeDividendTracker.excludeFromDividends(address(account));
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        ...
        if(value) {
            wifeDividendTracker.excludeFromDividends(pair);
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        wifeDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
        bnbDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }

    function updateClaimWait(uint256 claimWait) external onlyOwner {
        wifeDividendTracker.updateClaimWait(claimWait);
        bnbDividendTracker.updateClaimWait(claimWait);
    }

    function getWifeClaimWait() external view returns(uint256) {
        return wifeDividendTracker.claimWait();
    }
    ...
    function getTotalWifeDividendsDistributed() external view returns (uint256) {
        return wifeDividendTracker.totalDividendsDistributed();
    }
    ...
    function withdrawableWifeDividendOf(address account) external view returns(uint256) {
        return wifeDividendTracker.withdrawableDividendOf(account);
    }
    ...
    function wifeDividendTokenBalanceOf(address account) external view returns (uint256) {
        return wifeDividendTracker.balanceOf(account);
    }
    ...
    function getAccountWifeDividendsInfo(address account)
    external view returns (
        ...
        uint256,
        uint256) {
        return wifeDividendTracker.getAccount(account);
    }
    ...
    function getAccountWifeDividendsInfoAtIndex(uint256 index)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return wifeDividendTracker.getAccountAtIndex(index);
    }
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        (uint256 wifeIterations, uint256 wifeClaims, uint256 wifeLastProcessedIndex) = wifeDividendTracker.process(gas);
        emit ProcessedWifeDividendTracker(wifeIterations, wifeClaims, wifeLastProcessedIndex, false, gas, tx.origin);
    ...
    function claim() external {
        wifeDividendTracker.processAccount(payable(msg.sender), false);
        bnbDividendTracker.processAccount(payable(msg.sender), false);
    }

    function getLastWifeDividendProcessedIndex() external view returns(uint256) {
        return wifeDividendTracker.getLastProcessedIndex();
    }
    ...
    function getNumberOfWifeDividendTokenHolders() external view returns(uint256) {
        return wifeDividendTracker.getNumberOfTokenHolders();
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (wifeDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.div(totalFees).mul(wifeDividendRewardsFee);
                    swapAndSendWifeDividends(sellTokens.sub(1300));
                }

                if (bnbDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.div(totalFees).mul(bnbDividendRewardsFee);
                    swapAndSendBnbDividends(sellTokens.sub(1300));
                }
        ...
        try wifeDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try bnbDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try wifeDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        try bnbDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}

        if(!swapping) {
            uint256 gas = gasForProcessing;

            if(rand() <= wifeDividendPriority) {

                if( wifeDividendEnabled && sendWifeInTx ){
                    try wifeDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                        emit ProcessedWifeDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
                    }
                    catch {
    ...
    function swapAndSendWifeDividends(uint256 tokens) private {
        swapTokensForDividendToken(tokens, address(this), wifeDividendToken);
        uint256 wifeDividends = IERC20(wifeDividendToken).balanceOf(address(this));
        transferDividends(wifeDividendToken, address(wifeDividendTracker), wifeDividendTracker, wifeDividends);
    }
...
contract WifeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Wife_Dividend_Tracker", "Wife_Dividend_Tracker", 0x099D065A79142fAf07990af94E57954aa134494b) {
        claimWait = 3600;
...
contract BnbDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BNB_Dividend_Tracker", "BNB_Dividend_Tracker", 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }
