...
*
* No buy tax
* 9% Sell Tax - 2% Auto LP, 2% Operation, 5% BNB Rewards
*
* Launch tax for 120 minutes after launch, 25%. To deter bots. Auto-disables after 120 minutes.
*
*/
...
contract LuckyToken is ERC20, Ownable {
    ...
    bool public isBuyTaxDisabled = true;
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
      require(isContract(ammAddress), "LuckyToken: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "LuckyToken: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "LuckyToken: The new dividend tracker must be owned by the LuckyToken token contract");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "LuckyToken: airdropToWallets:: Arrays must be the same length");
...
contract DividendTracker is DividendPayingToken {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "LuckyToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
