...
contract Reader {
    ...
    function getPrices(IVaultPriceFeed _priceFeed, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            address token = _tokens[i];
            amounts[i * propsLength] = _priceFeed.getPrice(token, true, true);
            amounts[i * propsLength + 1] = _priceFeed.getPrice(token, false, true);
            amounts[i * propsLength + 2] = _priceFeed.isAdjustmentAdditive(token) ? 1 : 0;
            amounts[i * propsLength + 3] = _priceFeed.adjustmentBasisPoints(token);
        }
