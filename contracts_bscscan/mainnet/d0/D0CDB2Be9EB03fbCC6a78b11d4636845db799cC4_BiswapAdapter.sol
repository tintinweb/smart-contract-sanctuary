...
// File: original_contracts/lib/biswap/IBiswapPair.sol
...
// File: original_contracts/lib/biswap/BiswapAdapter.sol
...
contract BiswapAdapter is IExchange, AdapterStorage {
    using SafeMath for uint256;
    ...
    constructor(address _factory, bytes32 _initCode, uint256 _feeFactor) public {
        initCode = _initCode;
    ...
    override
    {
        ...
        BiswapV2Data memory data = abi.decode(payload, (BiswapV2Data));
        _swap(
            fromAmount,
            data.path
        );
    ...
    returns (uint256 tokensSold)
    {
        ...
        for (uint8 i = length - 1; i > 0; i--) {
            address tokenFrom = path[i-1] == Utils.ethAddress() ? Utils.wethAddress() : path[i-1];
            address tokenTo = path[i] == Utils.ethAddress() ? Utils.wethAddress() : path[i];
            address pair = UniswapV3LibFork.pairFor(factory, tokenFrom, tokenTo, initCode);
            (amounts[i - 1], pairs[i - 1]) = UniswapV3LibFork.getAmountInAndPair(
                ...
                tokenTo,
                initCode,
                feeFactor - IBiswapPair(pair).swapFee(),
                feeFactor
            );
        }
    ...
    returns(uint256 tokensBought)
    {
        ...
        for(uint8 i = 0; i < pairs; i++) {
            ...
            tokensBought = UniswapV3LibFork.getAmountOutByPair(
                tokensBought,
                currentPair,
                tokenSold,
                tokenBought,
                feeFactor - IBiswapPair(currentPair).swapFee(),
                feeFactor
            );
            ...
            if ((i + 1) == pairs) {
                ...
                receiver = UniswapV3LibFork.pairFor(factory, tokenBought, path[i+2] == Utils.ethAddress() ? Utils.wethAddress() : path[i+2], initCode);
            }
