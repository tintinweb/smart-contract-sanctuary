...
contract BuyBack is Ownable {
    ...
    uint256 public markdown = 90;

    address public masterChef = 0xE399d7504310B84D38eFD56Cf8A29b38801D6449;

    event Log(address indexed users,address indexed seller,address indexed contractAdd);
    ...
    function setMarkdown(uint256 _markdown) public{
        markdown = _markdown;
    }
    ...
    function intSaveQueue(address sellerAdd,uint256 totalXOSParam)  internal{
        P2PQueue memory p2p;
        p2p.seller = sellerAdd;
        p2p.amountXOS = totalXOSParam;
        p2p.amountSwapToken = getAmountOuts(totalXOSParam)*markdown/100;
    ...
    function saveQueueByContract(address sellerAdd,uint256 totalXOSParam) public onlyMasterChef{
        intSaveQueue(sellerAdd,totalXOSParam);
    }
    ...
    function saveQueue(uint256 totalXOSParam) public{
        ...
        xosToken.safeTransferFrom(address(msg.sender),address(this), totalXOSParam);

        intSaveQueue(address(msg.sender),totalXOSParam);
    }
    ...
    function savetrans(P2PQueue storage p2p) internal{
        emit Log(address(msg.sender),address(p2p.seller),address(this));
        ...
        swapToken.safeTransferFrom(address(msg.sender),address(this), p2p.amountSwapToken);
        //transfer busd to seller
        swapToken.safeTransfer(address(p2p.seller), p2p.amountSwapToken);
        //transfer xos to buyer
        xosToken.safeTransfer(address(msg.sender), p2p.amountXOS);
        //add to history
        history.push(p2p);
    }
}