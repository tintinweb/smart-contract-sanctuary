...
contract Stickman is IBEP20, Ownable
{
	...
	uint256 public constant INITIAL_SUPPLY= 1000000000* 10**_DECIMALS;//equals 2000000 token
	//Divider for the MaxBalance based on circulating Supply (5%)
	uint8 public constant BALANCE_LIMIT_DIVIDER=33;
	//Divider for sellLimit based on circulating Supply (5%)
	uint16 public constant SELL_LIMIT_DIVIDER=100;
	...
	uint16 public constant MAX_SELL_LOCK_TIME= 0;
	...
	uint16 public constant MAX_BUY_LOCK_TIME= 0;
	...
	function _isTeam(address addr) private view returns (bool){
		return addr==owner()||addr==marketing||addr==constantProvider||addr==manualClaim;
	}
	//Constructor///////////
	constructor () {
		...
		// Pancake Router
		_pancakeRouter = IPancakeRouter02(PANCAKE_ROUTER);
		//Creates a Pancake Pair
		_pancakePairAddress = IPancakeFactory(_pancakeRouter.factory()).createPair(address(this), _pancakeRouter.weth());
		
		//Sets Buy/Sell limits
		balanceLimit=INITIAL_SUPPLY/BALANCE_LIMIT_DIVIDER;
		sellLimit=INITIAL_SUPPLY/SELL_LIMIT_DIVIDER;
	    //Sets sellLockTime
		sellLockTime=0;
		//Sets buyLockTime
		buyLockTime=0;
	...
	function _newDividendsOf(address staker) private view returns (uint256) {
		...
		return (fullPayout - alreadyPaidShares[staker]) / DISTRIBUTION_MULTIPLIER;
	}
	...
	function getBurnedTokens() public view returns(uint256){
		return (INITIAL_SUPPLY-_circulatingSupply)/10**_DECIMALS;
	}
