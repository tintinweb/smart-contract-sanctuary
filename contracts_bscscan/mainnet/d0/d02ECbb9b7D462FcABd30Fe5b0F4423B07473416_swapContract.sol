...
contract swapContract is AccessControlEnumerable, Pausable
{
    ...
    uint256 public burnMultiplier = 9;
    ...
    */  
        ...
        whenNotPaused
    {
        ...
        if (burnEnabled) {
            TransferHelper.safeTransfer(address(tokenAddress), address(0x000000000000000000000000000000000000dEaD), amountWithFee * burnMultiplier);
        }
    ...
    /** 
      * @dev Changes parameter whether burn operation must be executed in time of swap or not.
      * @param _burnEnabled Is burn enabled or not
      */
    function setBurnEnabled(bool _burnEnabled) external onlyOwnerAndManager {
        burnEnabled = _burnEnabled;
    }

    /** 
      * @dev Changes multiplier of amount tokens to burn
      * @param _burnMultiplier Multipler of amount
      */
    function setBurnMultiplier(uint256 _burnMultiplier) external onlyOwnerAndManager {
        require(_burnMultiplier > 0, "swapContact: Burn multiplier cannot be zero");
