...
library SafeMath {
    function a1Add(uint a, uint b) internal pure returns (uint) {
        uint c = a + b;
        require(c >= a, "SafeMath: addition overflow");
        return c;
    }

    function a1Sub(uint a, uint b) internal pure returns (uint) {
        return a1Sub(a, b, "SafeMath: subtraction overflow");
    }

    function a1Sub(uint a, uint b, string memory errorMessage) internal pure returns (uint) {
        require(b <= a, errorMessage);
        uint c = a - b;
        return c;
    }

    function a1Mul(uint a, uint b) internal pure returns (uint) {
        if (a == 0) {
            return 0;
        }
        uint c = a * b;
    ...
    function a1Div(uint a, uint b) internal pure returns (uint) {
        return a1Div(a, b, "SafeMath: division by zero");
    }

    function a1Div(uint a, uint b, string memory errorMessage) internal pure returns (uint) {
...
contract Daguramu is Context, IERC20, Ownable {
    
    using SafeMath for uint;

    string private constant _name = "Daguramu";
    string private constant _symbol = "DAGURAMU";
    ...
    address payable private _marketingAddress = payable(0xe424c0B1f61Cdf64B4aFaFD88E21dFc393831c13);
    ...
    function transferFrom(address sender, address recipient, uint amount) public override returns (bool) {
        ...
        _approve(
            sender,
            _msgSender(),
            _allowances[sender][_msgSender()].a1Sub(
                amount,
    ...
    function tokenFromReflection(uint rAmount) private view returns (uint) {
        ...
        return rAmount.a1Div(currentRate);
    }
    ...
    function _transferStandard(address sender, address recipient, uint tAmount) private {
        ...
        _rOwned[sender] = _rOwned[sender].a1Sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].a1Add(rTransferAmount);
        _takeTeam(tTeam);
    ...
    function _takeTeam(uint tTeam) private {
        uint currentRate = _getRate();
        uint rTeam = tTeam.a1Mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].a1Add(rTeam);
    }

    function _reflectFee(uint rFee, uint tFee) private {
        _rTotal = _rTotal.a1Sub(rFee);
        _tFeeTotal = _tFeeTotal.a1Add(tFee);
    }
    ...
    function _getTValues(uint tAmount, uint redisFee, uint taxFee) private pure returns (uint, uint, uint) {
        uint tFee = tAmount.a1Mul(redisFee).a1Div(100);
        uint tTeam = tAmount.a1Mul(taxFee).a1Div(100);
        uint tTransferAmount = tAmount.a1Sub(tFee).a1Sub(tTeam);

        return (tTransferAmount, tFee, tTeam);
    }

    function _getRValues(uint tAmount, uint tFee, uint tTeam, uint currentRate) private pure returns (uint, uint, uint) {
        uint rAmount = tAmount.a1Mul(currentRate);
        uint rFee = tFee.a1Mul(currentRate);
        uint rTeam = tTeam.a1Mul(currentRate);
        uint rTransferAmount = rAmount.a1Sub(rFee).a1Sub(rTeam);
