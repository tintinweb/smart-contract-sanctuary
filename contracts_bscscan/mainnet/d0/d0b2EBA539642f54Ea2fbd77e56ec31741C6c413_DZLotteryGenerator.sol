...
contract DZLotteryGenerator is VRFConsumerBase, Ownable {
    using Address for address payable;
    using SafeERC20 for IERC20;
    
    DZLottery[] public lotteries;
    ...
    DZLottery public currentLottery;
    ...
    function createLottery (uint256 endTime) external payable onlyOwner {
        require (currentLotteryState == LotteryState.NOT_STARTED, "DZLotteryGenerator: Can't start a lottery whilst one is still in progress");
        //require (endTime > block.timestamp + 15 minutes, "DZLotteryGenerator: Lottery must last longer than 15 minutes"); //CHANGEME removed for testing
        require (endTime < block.timestamp + 1 weeks, "DZLotteryGenerator: Lottery must last less than one week");
        
        currentLotteryID = lotteries.length;
        currentLottery = new DZLottery (currentLotteryID, entryFee, minEntriesToBeWinnable, maxEntries, endTime);
    ...
    function endLottery (uint256 newEndTimeForFailedLottery) external onlyOwner returns (bool){
        require (currentLottery.endTime() < block.timestamp, "DZLotteryGenerator: Cannot end a lottery before its end time");
        require (currentLotteryState != LotteryState.ENDED, "DZLotteryGenerator: Lottery already ended and waiting for result");
        currentLotteryState = LotteryState.ENDED;
        
        if (currentLottery.totalTicketsHeld() < currentLottery.minEntriesToBeWinnable()) {
            //require (newEndTimeForFailedLottery > block.timestamp + 15 minutes, "DZLotteryGenerator: Lottery must last longer than 15 minutes"); //CHANGEME removed for testing
            require (newEndTimeForFailedLottery < block.timestamp + 1 weeks, "DZLotteryGenerator: Lottery must last less than one week");
    ...
    function declareLottery() external onlyOwner {
        require (lastLotteryResultReturned == currentLotteryID, "DZLotteryGenerator: no result returned for current lottery yet");
        require (currentLotteryState == LotteryState.ENDED, "DZLotteryGenerator: cannot declare winner until lottery has ended");
    ...
    function setEntryFee (uint256 newEntryFee) external onlyOwner {
        require (newEntryFee > 0, "DZLotteryGenerator: fee must be > 0");
    ...
    function setMaxEntries (uint256 newMaxEntries) external onlyOwner {
        require (newMaxEntries > 0, "DZLotteryGenerator: max entries per user must be > 0");
    ...
    function setTreasuryWallet (address newTreasuryWallet) external onlyOwner {
        require (newTreasuryWallet != address(0), "DZLotteryGenerator: treasury wallet can't be the zero address");
    ...
    function getLotteryStats (uint256 lotteryID) public view returns (address lotteryAddress, address[] memory entrants, address winner, uint256 winnings, uint256 ticketsHeld, uint256 endTime, bool isLive) {
        require (lotteryID < lotteries.length, "DZLotteryGenerator: No lottery with that ID exists");
    ...
    function enterLottery() public payable returns (uint256 numberOfTicketsBought) {
        require (currentLottery.endTime() >= block.timestamp, "DZLotteryGenerator: Lottery has expired");
        require (currentLotteryState == LotteryState.STARTED, "DZLotteryGenerator: Lottery can't be entered");
        ...
        require (currentLottery.ticketsHeld(msg.sender) + numberOfTicketsBought <= maxTickets, "DZLotteryGenerator: user has bought too many tickets");
    ...
    function getRandomNumber (uint256 seed) internal {
        require (keyHash != bytes32(0), "DZLotteryGenerator: Must have valid key hash");
        require (LINK.balanceOf (address(this)) >= linkFee, "DZLotteryGenerator: Not enough LINK tokens");

        latestRequestId = requestRandomness (keyHash, linkFee, seed);
    }
    
    function fulfillRandomness (bytes32 requestId, uint256 randomness) internal override {
        require (latestRequestId == requestId, "DZLotteryGenerator: Wrong requestID");
        lotteryResult = randomness;
        lastLotteryResultReturned = currentLotteryID;
    }
    
    function withdrawTokens (address token) external onlyOwner {
        require (token != address (0), "DZLotteryGenerator: can't withdraw token of zero address");
    ...
    function withdrawEth() external onlyOwner {
        require (address(this).balance > 0, "DZLotteryGenerator: No Eth to withdraw");
        payable(owner()).sendValue (address(this).balance);
    }
    
    function withdrawTokens (address token, uint256 lotteryID) external onlyOwner {
        require (lotteryID < lotteries.length, "DZLotteryGenerator: Lottery ID does not exist");
