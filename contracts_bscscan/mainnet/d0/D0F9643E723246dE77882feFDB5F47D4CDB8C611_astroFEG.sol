...
// astroFEG @ https://t.me/astroFEGbsc
...
// File: contracts/astroFEGDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable BUSD = address(0xacFC95585D80Ab62f67A14C566C1b7a49Fe91167);
  
  //fegtoken 0xacFC95585D80Ab62f67A14C566C1b7a49Fe91167
...
contract astroFEG is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    astroFEGDividendTracker public dividendTracker;
    ...
    uint256 public maxSellTransactionAmount = 1000000000000000 * (10**18); // No limit
    ...
    constructor() public ERC20("astroFEG", "astroFEG") {
    	...
    	dividendTracker = new astroFEGDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "astroFEG: The dividend tracker already has that address");

        astroFEGDividendTracker newDividendTracker = astroFEGDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "astroFEG: The new dividend tracker must be owned by the astroFEG token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "astroFEG: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "astroFEG: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "astroFEG: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "astroFEG: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "astroFEG: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "astroFEG: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "astroFEG: This account cannot send tokens until trading is enabled");
        }
...
contract astroFEGDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("astroFEG_Dividend_Tracker", "astroFEG_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "astroFEG_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "astroFEG_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main astroFEG contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "astroFEG_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "astroFEG_Dividend_Tracker: Cannot update claimWait to same value");
