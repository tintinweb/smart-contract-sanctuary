...
contract HBCICO 
{
    ...
    constructor(address ownerAddress, IBEP20 _hbcToken, IBEP20 _busdToken) public 
    {
        ...
        User memory user = User({
            ...
            autoreferrals:new address[](0),
            withdrawalType:1,
            totalWithdrawal: uint(0)
        });
        lastWithdrawalDate[ownerAddress]=0;
    ...
    function registration(address userAddress, address referrerAddress,uint256 withType) private{
        ...
        (uint256 dedPer,uint256 nextTime)=get_withdraw_percent(withType);
        lastWithdrawalDate[userAddress]=block.timestamp+nextTime;
        ...
        users[referrerAddress].autoreferrals.push(userAddress);
    ...
    function withdraw() public payable{
        ...
        require(lastWithdrawalDate[msg.sender]<block.timestamp,"Time Issue.");
        ...
        (uint256 dedPer,uint256 nextTime)=get_withdraw_percent(users[msg.sender].withdrawalType);
        uint256 ded=(exactAmount.mul(dedPer)).div(1000);
        uint256 sendAmt;
        if(users[msg.sender].withdrawalType<4)
        sendAmt=exactAmount-ded;
        else
        sendAmt=exactAmount+ded;
        
        hbcToken.transfer(msg.sender,sendAmt);
        
        users[msg.sender].totalWithdrawal=users[msg.sender].totalWithdrawal+exactAmount;
        lastWithdrawalDate[msg.sender]=block.timestamp+nextTime;
        if(isDone)
        {
    ...
    function _calculatePoolReward(address _referrer,address  _from) private 
    {
        ...
        uint256 income=(autop.div(tokenPrice)).mul(1e18);
         while(i<5)
         {
             if(is_qualify_autopool(_referrer))
             {
    ...
    function token_setting(uint min_buy,  uint min_sale) public payable{
     ...
     function token_price_setting(uint token_price_set) public payable{
           require(msg.sender==owner,"Only Owner Can Change Token Price");
              tokenPrice = token_price_set;
   ...
   function get_capping(uint256 _id) public pure returns(uint256){
    ...
    function get_withdraw_percent(uint256 _id) public pure returns(uint256,uint256){
      if(_id==1)
      return (150,1 days);
      if(_id==2)
      return (8,7 days);
    ...
    function get_ref_percent(uint256 _id) public pure returns(uint256){
      ...
      function is_qualify_autopool(address _user) public view returns(bool)
      {
       	
	   uint256 i;
	   uint8 count;
	   bool is_qualify;
	   if(users[_user].autoreferrals.length>=2)
	   {
	   for(i=0; i<users[_user].autoreferrals.length;i++)
	   {
	     if(users[users[_user].autoreferrals[i]].currentPlanId>=users[_user].currentPlanId)
	     {
	         count++;
	         if(count>=2)
	         break;
