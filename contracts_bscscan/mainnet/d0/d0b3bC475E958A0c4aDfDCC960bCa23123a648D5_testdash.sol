...
contract testdash is ERC20, Ownable {
    ...
    testdashDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("testdash", "testdash") {
        ...
        dividendTracker = new testdashDividendTracker();
    ...
    /// @notice Withdraw tokens sent by mistake. Impossible to withdraw testdash tokens
    /// @param tokenAddress The address of the token to withdraw
    function rescueBEP20Tokens(address tokenAddress) external onlyOwner {
        require(
            tokenAddress != address(this),
            "Impossible to withdraw testdash tokens"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "testdash: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(
            ...
            "testdash: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[newPair] = value;
    ...
    function setGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            ...
            "testdash: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "testdash: Cannot update gasForProcessing to same value"
        );
...
contract testdashDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        public
        DividendPayingToken(
            "testdash_Dividen_Tracker",
            "testdash_Dividend_Tracker"
        )
    {
        claimWait = 3600;
    }
    ...
    function _transfer(
        ...
        require(false, "testdash_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(
            false,
            "testdash_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main testdash contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "testdash_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "testdash_Dividend_Tracker: Cannot update claimWait to same value"
        );
