...
// File: lp-farming/IReservoir.sol
...
// File: lp-farming/FarmingPool.sol



pragma solidity ^0.7.0;

//import "../BWT/SafeMath.sol";
 ...
 *  XXX: Removed devaddr logic;
 *  XXX: Updated to per second distribution;
 *  XXX: Added check if LP token has already been added;
 *  XXX: Owner can update reservoir address and tokensPerSecond value;
 *  XXX: Removed bonus multiplier;
 *  XXX: Updated getMultiplier logic (using LP balances coef);
 *  XXX: Uses 1e18 instead of 1e12 for accumulated tokens per share;
 *  XXX: Uses safeTransfer instead of transfer;
 *  XXX: Only massUpdatePools logic (deposit/withdraw);
 *  XXX: Max pools limit to prevents deadlock and unpredictable gas spending.
 */
contract FarmingPool is Ownable {
    ...
    // Max pools limit to prevents deadlock and unpredictable gas spending.
    ...
    event SetTokenReservoir(address tokenReservoir);
    event SetTokensPerSecond(uint256 tokensPerSecond);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) external onlyOwner {
        require(poolInfo.length < MAX_POOLS, "add: max pools limit reached");

        // Trying to add the same LP token more than once
        require(!lpTokens[address(_lpToken)], "add: LP token has already been added");
    ...
    // Set a new tokenReservoir. Can only be called by the owner.
    function setTokenReservoir(IReservoir _tokenReservoir) external onlyOwner {
        tokenReservoir = _tokenReservoir;
        emit SetTokenReservoir(address(_tokenReservoir));
    }

    // Set a new tokensPerSecond. Can only be called by the owner.
    function setTokensPerSecond(uint256 _tokensPerSecond) external onlyOwner {
        tokensPerSecond = _tokensPerSecond;
        emit SetTokensPerSecond(_tokensPerSecond);
    }

    // Return reward multiplier over the given _from to _to timestamp.
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        // Calculation of the LP balances coef. Safe gas spending: no more than MAX_POOLS pools.
        ...
        return _to.sub(_from).mul(MULTIPLIER).mul(totalPoolsBalance).div(MAX_POOLS_SUPPLY);
    }
    ...
    // Update reward variables for all pools. Safe gas spending: no more than MAX_POOLS pools.
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        updatePools(); // safe gas spending: no more than MAX_POOLS pools
        if (user.amount > 0) {
