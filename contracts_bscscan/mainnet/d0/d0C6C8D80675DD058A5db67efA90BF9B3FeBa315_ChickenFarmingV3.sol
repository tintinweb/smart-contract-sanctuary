...
contract ChickenFarmingV3 is Ownable {
    ...
    // Store more chicken in farm
     function depositMore(uint256 _chicken , uint256 _farmId) public {
        ...
        uint256 _diffrenceTime = getUnlockTime(msg.sender).sub(block.timestamp) ;
        user.depositTime = block.timestamp.sub(eggInterval).sub(_diffrenceTime) ;
