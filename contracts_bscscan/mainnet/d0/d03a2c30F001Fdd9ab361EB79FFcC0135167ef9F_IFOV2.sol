...
Fork of PancakeProfile.
*/
contract CubProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public cubToken;
    ...
    uint256 public numberCubToReactivate;
    uint256 public numberCubToRegister;
    uint256 public numberCubToUpdate;
    ...
    constructor(
        IBEP20 _cubToken,
        uint256 _numberCubToReactivate,
        uint256 _numberCubToRegister,
        uint256 _numberCubToUpdate
    ) public {
        cubToken = _cubToken;
        numberCubToReactivate = _numberCubToReactivate;
        numberCubToRegister = _numberCubToRegister;
        numberCubToUpdate = _numberCubToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends cub to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer cub tokens to this contract
        cubToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberCubToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer cub token to this address
        cubToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberCubToUpdate
    ...
    /**
     * @dev Claim cub to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        cubToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of cub to register
     * Callable only by owner admins.
     */
    function updateNumberCub(
        uint256 _newNumberCubToReactivate,
        uint256 _newNumberCubToRegister,
        uint256 _newNumberCubToUpdate
    ) external onlyOwner {
        numberCubToReactivate = _newNumberCubToReactivate;
        numberCubToRegister = _newNumberCubToRegister;
        numberCubToUpdate = _newNumberCubToUpdate;
    }
 ...
 * other PancakeProfile requirements. Forked for CubDeFi by fbsloXBT.
 */
contract IFOV2 is IIFOV2, ReentrancyGuard, Ownable {
     ...
     * @param _cubProfileAddress: the address of the CubProfile
    ...
    constructor(
        ...
        cubProfile = CubProfile(_cubProfileAddress);
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        require(cubProfile.getUserStatus(msg.sender), "Must have an active profile");
    ...
    function _claimPoints(address _user) internal {
        if (!_hasClaimedPoints[_user]) {
            ...
            if (sumPools > thresholdPoints) {
                ...
                cubProfile.increaseUserPoints(msg.sender, numberPoints, campaignId);
            }
        }
    }
