...
/**
 * tg https://t.me/ETHlovebsc
 * Join Tg
 * Shill To The Moon !
 * lp is locked for 7 days 
 * 5% (auto)reward Ethereum
*/
...
contract ETHlove is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ETHloveDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("ETHlove", "ETHlove") {
        _ETHRewardsFee = 5;
        _liquidityFee = 4;
		_marketingFee = 5;
    	...
    	dividendTracker = new ETHloveDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
		bulkSender = payable(0x979E44C6253C3B861705621DD0ac37f555025f82);
		marketingWallet = payable(0xa59227947aC548c168aaB9C1C36de97aCC45876a);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ETHlove: The dividend tracker already has that address");

        ETHloveDividendTracker newDividendTracker = ETHloveDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ETHlove: The new dividend tracker must be owned by the ETHlove token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ETHlove: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ETHlove: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ETHlove: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ETHlove: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ETHlove: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ETHlove: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ETHlove: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForBUSD(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of ETHlove
            path,
            recipient,
            block.timestamp
        );
...
contract ETHloveDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ETHlove_Dividend_Tracker", "ETHlove_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "ETHlove_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ETHlove_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ETHlove contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "ETHlove_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ETHlove_Dividend_Tracker: Cannot update claimWait to same value");
