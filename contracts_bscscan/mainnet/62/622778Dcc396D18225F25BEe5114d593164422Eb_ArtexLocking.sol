...
contract ArtexLocking {
    ...
    uint256 public researchAllocation;

    uint256 public researchTimeLock;
    ...
    constructor(address payable _governance, ERC20 _artexToken) public {
        ...
        researchAllocation = SafeMath.mul(15000000, 10**(18)); // 15 million  - 3 year lock
        
        researchTimeLock = SafeMath.add(now, 1095 days);
    ...
    function releaseTeamTokens() public returns (bool success) {
        ...
        require(now >= researchTimeLock, "!tokens cannot be withdrawn now");

        require(
            artexToken.transfer(governance, researchAllocation),
