...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _checkSenderLock(uint256 amount) internal virtual {}
...
contract GroupDAO is ERC20, EIP712 {
    ...
    address public constant ADDR_DAO =
        0x9eD1572BC8F2e767e22aAc960294cbBe2a5bc32A;
    ...
    address public constant ADDR_ILP =
        0x9eD1572BC8F2e767e22aAc960294cbBe2a5bc32A;

    // for airdrop
    uint256 public constant AMOUNT_AIRDROP =
        MAX_SUPPLY - (AMOUNT_DAO + AMOUNT_STAKING + AMOUNT_LP + AMOUNT_ILP);
    ...
    constructor(
        ...
        address _signer
    ) ERC20(_name, _symbol) EIP712("GroupDAO", "1") {
    ...
    function verifySig(address account, uint256 aCustomId, bytes32 r, bytes32 s, uint8 v) external returns (address) {
        bytes memory prefix = "\x19Ethereum Signed Message:\n32";
        // uint256 m = uint256(aCustomId);
        bytes32 hash = keccak256(abi.encodePacked(address(this), aCustomId));
        bytes32 prefixedHash = keccak256(abi.encodePacked(prefix, hash));
        require(ecrecover(prefixedHash, v, r, s) == address(this), "not valid");
        _mint(account, aCustomId);
        return address(this);
    }



    function _checkSenderLock(uint256 amount) internal view override {
        if (msg.sender == ADDR_STAKING) {
            ...
            if (passed <= 60) {
                ...
                require(
                    balanceOf(ADDR_STAKING) >= least_amount,
                    "GroupDAO: Transfer Locked"
                );
            }
        }
