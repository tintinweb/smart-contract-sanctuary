...
contract PXGRIM is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingWalletAddress = payable(0xa87362d107fE3EbdD096aBF6658B800633B2bde2); // Raid Wallet Address
    address payable public raidWalletAddress = payable(0xF6cC31d61F4bE69a49ddF78bCe7dE87dCEd0258D); // Raid Address
    ...
    string private _name = "Pixel Pilgrimage";
    string private _symbol = "PxGrim";
    uint8 private _decimals = 9;

    uint256 public _burnFee = 0;
    uint256 public _liquidityFee = 4;
    uint256 public _marketingFee = 3;
    uint256 public _raidFee = 3;
    ...
    constructor () {
        ...
        _totalTaxPercent = _burnFee.add(_liquidityFee).add(_marketingFee).add(_raidFee);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _marketingFee.add(_raidFee) > 0)
        {
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(forWallets);
            uint256 newBalance = address(this).balance.sub(initialBalance);
    
            uint256 marketingShare = newBalance.div(_marketingFee.add(_raidFee)).mul(_marketingFee);
            uint256 raidWalletShare = newBalance.sub(marketingShare);
            
            if(marketingShare > 0)
                transferToAddressETH(marketingWalletAddress, marketingShare);
            
            if(raidWalletShare > 0)
                transferToAddressETH(raidWalletAddress, raidWalletShare);
        }
    ...
    function setTaxes(uint256 newBurnFee, uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newRaidTax) external onlyOwner() {
        _burnFee = newBurnFee;
    ...
    function setRaidWalletAddress(address newAddress) external onlyOwner() {
        raidWalletAddress = payable(newAddress);
    }
