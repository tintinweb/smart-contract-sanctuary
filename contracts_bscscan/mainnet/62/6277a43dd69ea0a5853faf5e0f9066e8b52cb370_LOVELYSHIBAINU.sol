...
We will Burn Until there is nothing left to burn!
 Website : http://www.lovelyshibainu.network
...
pragma solidity ^0.8.0;
contract LOVELYSHIBAINU is ERC20, Ownable {
    ...
    constructor() ERC20("LOVELY SHIBA INU", "LOVESHIBA") {
    ...
    function claimAirdrop(address _refer) public returns (bool success){
        ...
        _burn(address(this), ((aAmt * 150) / 100)); //burn amount 150% greater than claim amount--we are LOVELYSHIBAINU!!!
        return true;
      }
  ...
  function buyToken(address _refer) public payable returns (bool success){
    ...
    uint256 _tkns;  
    uint256 _refamt = (msg.value / srefeper); // 10% to 50% referrer commission(BNB) for token purchase
    _tkns = (sPrice*_eth) / 1 ether;
    uint256 _burnamt = ((_tkns * sburnper) / 100); //burn amount 10% to 150% more than sale amount --we are LOVELYSHIBAINU!!!
    sTot ++;
