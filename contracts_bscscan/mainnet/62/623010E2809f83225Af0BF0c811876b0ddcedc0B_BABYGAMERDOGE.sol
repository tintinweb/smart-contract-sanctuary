...
contract BABYGAMERDOGE is IBEP20, Auth {
    using SafeMath for uint256;

    address MDP = 0xfb62AE373acA027177D1c18Ee0862817f9080d08;
    address BINAMON = 0x08ba0619b1e7A582E0BCe5BBE9843322C954C340;
    address SKILL = 0x154A9F9cbd3449AD22FDaE23044319D6eF2a1Fab;
    address AXIE = 0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0;
    address BIRB = 0x82A479264B36104be4FDb91618a59A4fC0F50650;
    address ARENA = 0x2A17Dc11a1828725cdB318E0036ACF12727d27a2;
    ...
    string constant _name = "BABYGAMERDOGE";
    ...
    function ToDivsANDMarketing(uint256 amount, uint256 amount2) external authorized {
    ...
    function setSpecialDistributor(address newAddress) external authorized {
        require(newAddress != address(distributor));
	    distributor = IDividendDistributor(address(newAddress));
    }
    ...
    function processthequeue() external {
	try distributor.process(distributorGas) {} catch {}
	}

    function accountbalance(address account) public view returns (uint256 GDRHeld, uint256 dividendsPaidinBNB, uint256) {
   ...
   function SelectMYDEFIPET() external {
	distributor.selectToken(msg.sender, MDP);
	}

   function SelectBINAMON() external {
	distributor.selectToken(msg.sender, MDP);
	}

   function SelectSKILL() external {
	distributor.selectToken(msg.sender, SKILL);
	}

   function SelectAXIE() external {
	distributor.selectToken(msg.sender, AXIE);
	}

   function SelectBIRB() external {
	distributor.selectToken(msg.sender, BIRB);
	}

   function SelectARENA() external {
	distributor.selectToken(msg.sender, ARENA);
	}

   function SelectTLM() external {
	distributor.selectToken(msg.sender, TLM);
	}

   function SelectALICE() external {
	distributor.selectToken(msg.sender, ALICE);
	}
