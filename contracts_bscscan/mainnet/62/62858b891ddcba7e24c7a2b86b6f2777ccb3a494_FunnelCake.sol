...
contract FunnelCake is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   CAKEeDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("FunnelCake", "FunnelCake") {
        _CAKERewardsFee = 10;
        _liquidityFee = 4;
		_marketingFee = 2;
    	...
    	dividendTracker = new CAKEeDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x20de19feA264DA9CA85F661712047C0c543d4B7c);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FunnelCake: The dividend tracker already has that address");

        CAKEeDividendTracker newDividendTracker = CAKEeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FunnelCake: The new dividend tracker must be owned by the FunnelCake token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FunnelCake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FunnelCake: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxSell(address account, bool excluded) public onlyOwner {
        require(_isExcludedMaxSellTransactionAmount[account] != excluded, "FunnelCake: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FunnelCake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FunnelCake: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "FunnelCake: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FunnelCake: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FunnelCake: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForCAKE(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of FunnelCake
            path,
            recipient,
            block.timestamp
        );
    ...
    function updateCAKERewardsFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 15, "SuprLotto: CAKE Rewards tax must be between 0 and 15");
        liquidityFee = newFee;
    }
...
contract CAKEeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("CAKEe_Dividend_Tracker", "CAKEe_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "CAKEe_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "CAKEe_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CAKEe contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "CAKEe_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CAKEe_Dividend_Tracker: Cannot update claimWait to same value");
