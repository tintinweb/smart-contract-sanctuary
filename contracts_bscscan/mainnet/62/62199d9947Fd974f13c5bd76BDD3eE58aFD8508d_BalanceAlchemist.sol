...
contract BalanceAlchemist is Ownable, IAlchemist {
  ...
  address public bgldToken;
  mapping(address => Allocation) public minters;

  constructor(address _bgldToken) public {
    ...
    if (_amount>0) {
      if (!allocation.revoked) {
        ...
        if (minted <= allocation.total) {
          ...
          IERC20(bgldToken).transfer(_to, _amount);
          return true;
        }
      }
    }
  ...
  function withdrawAll(address _to) public onlyOwner {
    uint256 balance = IERC20(bgldToken).balanceOf(address(this));
    require (balance > 0, "withdrawAll: no balance");

    IERC20(bgldToken).transfer(_to, balance);
  }
}
