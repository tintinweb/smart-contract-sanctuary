...
contract FegZilla is Context, IERC20, IERC20Extented, Ownable {
    using SafeMath for uint256;
    string private constant _name = "Feg Zilla";
    string private constant _symbol = "FegZilla";
        ...
        if (from != owner() && to != owner() && !presale && !_isContractWallet[from] && !_isContractWallet[to] && from != address(this) && to != address(this)) {
            ...
            if (!inSwap && from != uniswapV2Pair && !_isExchange[from]) { //sells, transfers
                ...
                if (!_isBridge[from] && !_isBridge[to]) {
                    ...
                    if(_dailyMaxEnabled) {
                        if(block.timestamp.sub(_firstSell[from]) > (0 days)) {
