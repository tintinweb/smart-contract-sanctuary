...
contract insvestWallet is ReentrancyGuard {
    ...
    mapping(string => agreement) internal _agreements;
    mapping(address => mapping(uint256 => investing)) internal _wallet;
    mapping(address => uint256) public _userSlot; //User slots
    mapping(address => uint256) internal _balances; //User balance
    mapping(uint256 => uint256) internal _statistics; //Mapp structure for statitstics
    mapping(uint256 => string) public _agreementrelation;
    uint256 public _totalSupply; //Pool of  token
    uint256 internal _agreementCount;
    ...
    constructor(address _stakingToken) ReentrancyGuard() {
        ...
        addAgreement(
            "init5", //code
            820, //0.82 daily in format of 100000  = 100%+3 decimals
            5, //no locked
            0,
    ...
    function readAgreements(bool active)
        public
        view
        returns (string memory agreements)
    {
        string memory time;
        string memory id;
        ...
        for (uint256 i = 1; i <= _agreementCount; i++) {
            if (_agreements[_agreementrelation[i]].active == active) {
                id = Strings.toString(i);
                reward = Strings.toString(
                    _agreements[_agreementrelation[i]].reward
                );
                time = Strings.toString(
                    _agreements[_agreementrelation[i]].time
                );
                minAmount = Strings.toString(
                    _agreements[_agreementrelation[i]].minAmount
                );
    ...
    function stake(uint256 _amount, string memory _code) external {
        ...
        emit Stake(msg.sender, _amount, _balances[msg.sender]); // set balance
    }
    ...
    function withdraw(uint256 slot) external nonReentrant {
        ...
        if (_agreements[_wallet[msg.sender][slot].code].time > 0) {
            rewardDate +=
                _agreements[_wallet[msg.sender][slot].code].time *
                1 days;
        } else {
        ...
        uint256 rewardDays = block.timestamp -
            _wallet[msg.sender][slot].signedDate;
        //convert secons in days
        rewardDays = rewardDays / 1 days;
        //get reward %
        uint256 percent = _agreements[_wallet[msg.sender][slot].code].reward *
            rewardDays;
    ...
    function getWallet(address account)
        public
        view
        returns (string memory wallet, uint256 time)
    {
        ...
        for (uint256 i = 0; i < _userSlot[account]; i++) {
            if (_wallet[account][i].claimed == false) {
                rewardDate = _wallet[account][i].signedDate;
                if (_agreements[_wallet[account][i].code].time > 0) {
                    rewardDate +=
                        _agreements[_wallet[account][i].code].time *
                        1 days;
                }
                if (comma > 0) {
                ...
                slot = Strings.toString(i);
                wallet = string(
                    abi.encodePacked(
                        wallet,
                        '{"code":"',
                        _wallet[account][i].code,
                        '",',
                        '"cantidad":"',
                        amount,
                        '",',
    ...
    function viewStcs(string memory _code)
        public
        view
        returns (string memory wallet)
    {
        if (msg.sender == _ownerAddr) {
            string memory quantt = Strings.toString(
                _statistics[_agreements[_code].id]
            );
            wallet = string(
                abi.encodePacked(
                    wallet,
    ...
    function ownerWithdraw(uint256 _amount) external nonReentrant {
        ...
        emit Withdraw(msg.sender, _amount, _totalSupply); //commit tranction
    }
