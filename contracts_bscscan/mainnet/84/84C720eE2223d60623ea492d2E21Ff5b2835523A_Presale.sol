...
contract Presale is ReentrancyGuard {
    ...
    uint256 BUSDPerToken = 2;
    uint256 BUSDReceived = 0;
    //cap on the total BUSD value recieved $200,000
    uint256 presaleCap = 200000 ether;
    // BUSD token
    IBEP20 BUSD;
    ...
    constructor (address _GAL, address _BUSD, uint256 _startingTimestamp) public {
        GAL = IBEP20(_GAL);
        BUSD = IBEP20(_BUSD);
        isSaleActive = true;
        owner = msg.sender;
    ...
    function buy (uint256 _amount, address beneficiary) public nonReentrant {
        ...
        require(BUSDReceived +  _amount <= presaleCap, "Presale limit reached");
        require(block.timestamp >= startingTimeStamp, "Presale has not started");
        address _buyer = beneficiary;
        // tier 1 -> $30,000  15,000 tokens 
        // tier 2 -> $30,000  10,000 tokens 
        // tier 3 -> $140,000 35,000 tokens
        if (totalTokensSold > 15000 ether && totalTokensSold <= 25000){
            // 10,000 tokens
            BUSDPerToken = 3; // tier 2
        } else if (totalTokensSold > 25000 ether){
            // 35,000 tokens
            BUSDPerToken = 4; // tier 3
        }
    ...
    function getPresaleCap () external view returns (uint256) {
        return presaleCap;
    }

    function getGALTokensLeft () external view returns (uint256) {
        return GAL.balanceOf(address(this));
    }

    function claimTokens (address claimer) external {
        require (GAL.balanceOf(address(this)) >= tokensOwned[msg.sender], "Error! Contact admins for this");
        require (tokensUnclaimed[msg.sender] > 0, "You don't have any unclaimed tokens");
        require (!isSaleActive, "Presale not over yet. You can only claim after presale ends.");
        require (tokensOwned[msg.sender] > 0, "You don't have tokens");
        tokensUnclaimed[msg.sender] = tokensUnclaimed[msg.sender].sub(tokensOwned[msg.sender]);
        GAL.safeTransfer(msg.sender, tokensOwned[msg.sender]);
    ...
    function withdrawUnsoldGAL() external onlyOwner {
        uint256 amount = GAL.balanceOf(address(this)) - totalTokensSold;
        GAL.safeTransfer(msg.sender, amount);
    }
}
