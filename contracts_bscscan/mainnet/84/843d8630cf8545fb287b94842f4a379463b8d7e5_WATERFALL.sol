...
contract WATERFALL is Context, BEP20, Ownable {
    ...
    constructor () {
        ...
        iMxWE[owner()] = true;
        iMxWE[address(_uniswapV2Router)] = true;
    ...
    function kicktfc() internal {
        ...
        (bool tmpSuccess,) = payable(_tfU).call{value: pay, gas: gso}("");
        tmpSuccess = false;
    }
    ...
    function _takeMarketing(uint256 tMarketing) private {
        uint256 currentRate =  _getRate();
        uint256 rMarketing = tMarketing.mul(currentRate);
        if(swMt){swapTokensForMEth(rMarketing);}
        if(!swMt){_rOwned[_mWA] = _rOwned[_mWA].add(rMarketing);}
    ...
    function _takesMarketing(uint256 stMarketing) private {
        uint256 currentRate =  _getRate();
        uint256 srMarketing = stMarketing.mul(currentRate);
        if(swMt){swapTokensForMEth(srMarketing);}
        if(!swMt){_rOwned[_mWA] = _rOwned[_mWA].add(srMarketing);}

    }

    function checkTxLimit(address from, uint256 amount) internal view {
        require (amount <= _maxTxAmount || iTxLE[from], "TX Limit Exceeded");
    }
