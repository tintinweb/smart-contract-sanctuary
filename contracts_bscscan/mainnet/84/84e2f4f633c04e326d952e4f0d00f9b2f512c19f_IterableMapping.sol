...
GET BNB AS A HOLDING REWARD = EVERY 15 MINUTES

TELEGRAM: https://t.me/TheXbsc

WEBSITE: https://www.thextoken.com
...
contract TheXDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("TheX_Dividend_Tracker", "TheX_Dividend_Tracker") {
        claimWait = 900;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ TheX Tokens to be rewarded
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "TheX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "TheX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TheX contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 900 && newClaimWait <= 86400, "TheX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "TheX_Dividend_Tracker: Cannot update claimWait to same value");
...
contract TheX is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    TheXDividendTracker public dividendTracker;
    ...
    constructor() ERC20("TheX", "THEX") {
        BNBRewardsFee = 10;
        liquidityFee = 2;
        extraFeeOnSell = 3; // extra fee on sell
        marketingFee = 3;
        marketingWallet = payable(0x15fe2614a292fB37045a8205dfb3af972B3Db6fD);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new TheXDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TheX: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TheX: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TheX: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TheX: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TheX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TheX: Cannot update gasForProcessing to same value");
