...
contract SmartChef is Ownable {
    ...
    function startReward() public onlyOwner {
        ...
        bonusEndBlock = block.number.add(10512000);
        rewardPerBlock = 4756468797564688;
    ...
    function deposit(uint256 _amount, address _referrer) public {
        ...
        require(pool.lpToken.balanceOf(address(this)).add(_amount) <= 100000000000000000000000, "pool balance too large"); // only allow up to 2 million tokens staked (2 million + 2 million token rewards)
        updatePool(0);
        if (user.amount > 0) {
        ...
        if(_amount > 0) {
            ...
            pool.lpToken.safeTransferFrom(0x9C86C36c1Ec4A5f9a0066D503c0791C4CdFF6119,_referrer, _depositFee);
