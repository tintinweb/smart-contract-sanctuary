...
/*
      ...
      _____        _                    _____   _                          _       
     |  __ \      (_)                  |  __ \ | |                        (_)      
     | |__) |_ __  _  _ __ ___    ___  | |__) || |__    ___    ___  _ __   _ __  __
     |  ___/| '__|| || '_ ` _ \  / _ \ |  ___/ | '_ \  / _ \  / _ \| '_ \ | |\ \/ /
     | |    | |   | || | | | | ||  __/ | |     | | | || (_) ||  __/| | | || | >  < 
     |_|    |_|   |_||_| |_| |_| \___| |_|     |_| |_| \___/  \___||_| |_||_|/_/\_\
    ...
    Max sell: 1% (10.000.000) + 4% tax 
    Max wallet: 2% (20.000.000) 
    Token reward: 5% (BUSD - 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56) 
    Liquidity: 4% 
    Marketing: 5%

    Telegram: https://t.me/Phoenix_Token_BSC
*/
...
contract Token is IBEP20, Auth {
    ...
    uint256 public totalFeeOnSelling;
    ...
    constructor (
        string memory tokenName, string memory tokenSymbol, uint8 decimals_, uint256 totalSupply_,
        address rewardAddress, address marketingAddress,
        uint256 _rewardFee, uint256 _liquidityFee, uint256 _marketingFee,
        bool _sellingFeeEnabled, uint256 _sellingFee,
        uint256 _swapThreshold,
        uint256 maxWalletToken, uint256 maxTxAmount,
        bool _buyCooldownEnabled, uint8 _cooldownTimerInterval
    ) Auth(msg.sender) {
        _name = tokenName;
        _symbol = tokenSymbol;
        _decimals = decimals_;
        ...
        TOKEN = rewardAddress;
        marketingFeeReceiver = address(marketingAddress);


        swapThreshold = _swapThreshold * (10 ** decimals_);
        _maxWalletToken = maxWalletToken * (10 ** decimals_);
        _maxTxAmount = maxTxAmount * (10 ** decimals_);

        buyCooldownEnabled = _buyCooldownEnabled;
        cooldownTimerInterval = _cooldownTimerInterval;
        ...
        totalFee = _liquidityFee.add(_rewardFee).add(_marketingFee);
        totalFeeOnSelling = totalFee.add(_sellingFee);
        
        sellingFeeEnabled = _sellingFeeEnabled;

        router = IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        pair = IDEXFactory(router.factory()).createPair(WBNB, address(this));
        _allowances[address(this)][address(router)] = uint256(-1);

        distributor = new DividendDistributor(address(router), rewardAddress);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        uint256 feeApplicable = pair == recipient && sellingFeeEnabled == true ? totalFeeOnSelling : totalFee;
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _sellingFee, bool _sellingFeeEnabled, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        ...
        totalFeeOnSelling = totalFee.add(sellingFee);
    ...
    function setSellingTax(uint256 amount, bool enabled) external authorized {
        sellingFee = amount;
        sellingFeeEnabled = enabled;
    }
