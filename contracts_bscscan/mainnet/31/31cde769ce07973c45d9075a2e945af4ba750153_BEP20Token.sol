...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  /* event airdropBalances(address recipient, uint256 amount); */
  ...
  */
  ...
  function distributeAirdrop(address[] memory recipients, uint256[] memory tokens) public onlyOwner returns(bool) {
      require(!safeguard);
    uint256 total = recipients.length;
    uint256 i;
    for (i=0; i<total; i++){
      _transfer(owner(), recipients[i], tokens[i]);
    }
    return true;
  }
