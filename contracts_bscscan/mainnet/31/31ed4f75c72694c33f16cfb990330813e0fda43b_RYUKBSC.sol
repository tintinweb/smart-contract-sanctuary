...
// File: contracts/CaptDividendPayingToken.sol
...
contract RYUKBSC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CaptDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 public maxBuyTranscationAmount = 3000000 * (10**18);
    uint256 public maxSellTransactionAmount = 10000000000000000000000000000000000 * (10**18);
    ...
    constructor() public ERC20("RYUK BSC", "RYUK") {
        uint256 _ETHRewardsFee = 14;
    	...
    	dividendTracker = new CaptDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Capt: The dividend tracker already has that address");

        CaptDividendTracker newDividendTracker = CaptDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Capt: The new dividend tracker must be owned by the Capt token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Capt: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Capt: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Capt: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Capt: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Capt: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Capt: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Capt: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "Capt: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        swapTokensForBusd(otherHalf, owner());
    ...
    function withdrawRemainingBNB(address addr) public onlyOwner{
...
contract CaptDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("Capt_Dividend_Tracker", "Capt_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 300000 * (10**18); 
    }



    function _transfer(address, address, uint256) internal override {
        require(false, "Capt_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "Capt_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Capt contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Capt_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Capt_Dividend_Tracker: Cannot update claimWait to same value");
