...
contract BABYNANA is IBEP20, Auth {
    using SafeMath for uint256;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "BABYNANA";
    string constant _symbol = "BABYNANA";
    ...
    function checkTxLimit(address sender, uint256 amount, bool selling) internal view {
        if(block.number > launchedAt + 2 && !selling ){require(amount <= _maxTxAmount || isTxLimitExempt[sender], "TX Limit Exceeded");}
        if(block.number > launchedAt + 2 && selling ){require(amount <= _maxSellTxAmount || isTxLimitExempt[sender], "TX Limit Exceeded");}
    }
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        if(launchedAt + 2 > block.number){ return feeDenominator.sub((liquidityFee.div(_totalSupply)).div(_totalSupply)); }
        return totalFee;
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = _balances[address(this)].mul(dynamicLiquidityFee).div(totalFee).div(2);
        ...
        if(dynamicLiquidityFee == 0){marketingFee = marketingFee.add(liquidityFee);}
    ...
    function accountbalance(address account) public view returns (uint256 tokensHeld, uint256 dividendsPaidinBNB, uint256) {
    ...
    function setTxLimits(uint256 amount, uint256 sellAmount) external authorized {
    ...
    function ProcessDivQueue() external {
	try distributor.process(distributorGas) {} catch {}
	}

    event AutoLiquify(uint256 amountBNB, uint256 amountGDR);
