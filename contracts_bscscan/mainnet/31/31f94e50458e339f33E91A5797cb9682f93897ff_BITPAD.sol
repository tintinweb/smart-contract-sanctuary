...
contract BTPRewardTracker is RewardPayingToken, Ownable {
    ...
    constructor() RewardPayingToken("BTP_Reward_Tracker", "BTP_Reward_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForRewards = 10000 * (10**9); 
    }

    function _transfer(address, address, uint256) internal override pure {
        require(false, "BTP_Reward_Tracker: No transfers allowed");
    }

    function withdrawReward() public override  pure {
        require(false, "BTP_Reward_Tracker: withdrawReward disabled. Use the 'claim' function on the main contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "BTP_Reward_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BTP_Reward_Tracker: Cannot update claimWait to same value");
...
uint256 private constant INITIAL_SUPPLY =  10000000   * 10 ** DECIMALS;
 ...
 uint256 private _totalSupply=INITIAL_SUPPLY;
  ...
  uint256 private constant Final_SUPPLY = ~uint128(0); 
    ...
    BTPRewardTracker public RewardTracker;

    address public marketingAdress = 0xbb57097Cf47a1C91cA5f67A61d0EF4E46dFEEFa9;
    address public walletAdress = 0x2D02361CD35FEBF29e3f4116f91d9f168066A17D;
...
address public lp;
  ILP public liquidutyReciever;
    ...
    constructor() ERC20("BIT PAD", "BTP") {

    	RewardTracker = new BTPRewardTracker();
    ...
    function setMarketingWallet(address wallet) public onlyOwner {
        marketingAdress = payable(wallet);
    }
    
    function setwalletBack(address wallet) public onlyOwner {
       walletAdress = payable(wallet);
    }
    
   
    function updateIDEXV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(IDEXV2Router), "BTP: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BTP: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 750000, "BTP: gasForProcessing must be between 200,000 and 750,000");
        require(newValue != gasForProcessing, "BTP: Cannot update gasForProcessing to same value");
 ...
 function rebaseToken(uint256 epoch, int256 supplyDelta)
  ...
  returns (uint256)
  {
    ...
    function updateFees(uint256 newRewardFee, uint256 newLiqFee, uint256 newMarketingFee, uint256 newbuyBackFee) public onlyOwner {
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 tRewards = tokensToLiquify.mul(rewardsFee).div(totalFees);
        ...
        uint256 bnbForReflection = bnbBalance.mul(rewardsFee).div(bnbFeeFactor);
        uint256 bnbForBTPBuyBack = bnbBalance.mul(buyBackFee).div(bnbFeeFactor);
      
        uint256 bnbForMarketing = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection).sub(bnbForBTPBuyBack);

        if(tokensToLP > 0 && bnbForLiquidity > 0)    
            addLiquidity(tokensToLP, bnbForLiquidity);
    
        if(bnbForMarketing > 0)    
            payable(marketingAdress).transfer(bnbForMarketing);
        
        if(bnbForBTPBuyBack > 0)    
            payable(walletAdress).transfer(bnbForBTPBuyBack);
