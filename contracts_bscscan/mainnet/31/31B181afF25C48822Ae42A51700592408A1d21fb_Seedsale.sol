...
contract Seedsale is Ownable{
    ...
    function setToken(address token) external onlyOwner{
        require(token!=address(0), "token is 0");
        _token = IERC20(token);
    }

    function setWhitelist(address[] memory seeders) external onlyOwner{
        for(uint i=0;i<seeders.length; i++){
            whitelist[seeders[i]] = true;
        }
    }
    ...
    function setMinHold(uint minHold) external onlyOwner {
        minHoldAmount = minHold;
    }
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount) virtual internal onlyWhileOpen view {
        ...
        if(!whitelist[beneficiary]){
            require(_token.balanceOf(beneficiary) >= minHoldAmount, "Minimum token hold required");
        }
