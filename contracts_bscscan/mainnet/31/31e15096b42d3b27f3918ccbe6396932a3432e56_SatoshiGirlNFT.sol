...
contract SatoshiGirlNFT is ERC721Enumerable, Ownable{
    ...
    constructor() ERC721("SatoshiGirl", "SatoshiGirl") {
        _baseTokenURI = "https://satoshigirl.findmynft.net/api/";
        beneficiary = 0xDA77E70EeFE69398B16691f50f9a0FA21097ec21;
    }
       ...
       
    {
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "SatoshiGirl recepient is the null address"
            );
            _tokenIds.increment();
            _newItemId = _tokenIds.current();
            _mint(_addresses[ind], _newItemId);
            emit minting(_newItemId);
           
        }
    }
    function mint_SatoshiGirl()  external
        onlyOwner payable returns (uint256) {
        require(totalSupply() <= 100, "enough minting");

        _tokenIds.increment();
        uint256 newSatoshiGirl = _tokenIds.current();
        uint256 price = getPrice(newSatoshiGirl);
        require(msg.value >= price, "not enough");

        _mint(msg.sender, newSatoshiGirl);
        emit minting(newSatoshiGirl);

        return newSatoshiGirl;
    }
