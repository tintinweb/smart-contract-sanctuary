...
contract WOWSummitNFTAuction {
    ...
    function claim(uint256 _auctionId) public {
        ...
        if (
            ...
            uint recentValue = auction.auctionToken.balanceOf(msg.sender);
            auction.auctionToken.transfer(msg.sender, recentBidAmount);
            uint _after = auction.auctionToken.balanceOf(msg.sender);
            require(recentBidAmount == _after.sub(recentValue), "insufficient balance");
            auction.hasClaimed[msg.sender] = true;
        }
    }
