...
/**
 * @title Highstack Farm Certificates (HS-CERT)
 * @author highstack.co
...
contract HighstackFarmCerts is ERC20, Ownable, ReentrancyGuard {
...
// RewardTokens will eventually be emitted via smart contract.
  ...
  function setFees(uint256 _stakeFee, uint256 _unStakeFee, uint256 _earlyUnstakeSeconds) public onlyOwner {
    stakeFee = _stakeFee;
  ...
  // help out our users
  function harvestForUser(address _userAddy) public onlyOwner nonReentrant returns (uint256) {
    _updatePool();
  ...
  function stakeTokens(uint256 _amount) external nonReentrant {
    ...
    // divided by 1e36 (probably to be multiplied by 1e36 later)
  ...
  function unstakeTokens(uint256 _amount, bool shouldHarvest) external nonReentrant {
    ...
    if (block.timestamp <= _staker.timeOriginallyStaked.add(earlyUnstakeSeconds)) {
      unstakeTax = earlyUnstakeFee.mul(_amountToRemoveFromStaked).div(100);
      _stakedERC20.transfer(address(rewardVault), unstakeTax);
      _amountToRemoveFromStaked = _amountToRemoveFromStaked.sub(unstakeTax);
    }
    require(
  ...
  function _harvestTokens(address _userAddy, uint256 newBalance) private returns (uint256) {
    ...
    _staker.rewardDebt = newBalance.mul(pool.accERC20PerShare).div(
      1e36
    );
    _staker.blockLastHarvested = block.number;
    return _num2Trans;
...
contract RewardVault is Ownable {
  ...
  function withdrawResiduals(IERC20 token) public {
    require(authorizedAddresses[msg.sender] == true, 'unauthorized');
