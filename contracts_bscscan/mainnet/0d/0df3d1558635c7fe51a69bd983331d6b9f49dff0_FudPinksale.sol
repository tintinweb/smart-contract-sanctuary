...
contract FudPinksale is IBEP20, Ownable
{
    ...
    mapping(address => bool) private prewhitelisted;
    //mapping(address => bool) private nonwhitelisted;
    mapping(address=>bool) public isAMM;
    //Token Info
    string private constant _name = 'FudPinksale';
    string private constant _symbol = 'FPINK';
    uint8 private constant _decimals = 9;
    uint public constant InitialSupply= 1 * 10**9 * 10**_decimals;//equals 1.000.000.000 Token
    ...
    bool private CheckSaulORI = false;
    bool private CheckPinksaleSlave = false;
    bool private checkWalletLimit = true;
    bool private checkSellMax = true;
    bool private whitelistMode = true;
    uint256 public maxTxAmount = 5384617 * 10**_decimals;
    uint256 public maxSelltokens = 2500000 * 10**_decimals;
    uint256 public maxWallet = 5384617 * 10**_decimals;
    ...
    function _transfer(address sender, address recipient, uint amount) private{
        ...
        { 
            ...
            //MODE TO ENABLE WHITELSIT MODE FIRST MINUTES    
            if(whitelistMode)
            {
                //CHECK IF USER IS WL - if they try to send to another wallet, it will check also
                if( whitelisted[recipient]==true || whitelisted[sender]==true || prewhitelisted[sender]==true  )
                {
                     _taxedTransfer(sender,recipient,amount); // a function with normal taxes or you can add you code here
                }
                //NOT WL. CANT TRANSFER
                else 
                {
                    require(false, "Not whitelisted.");
                }
            } 
            // NORMAL MODE - EVERYBODY CAN TRADE
            else 
            {
                _taxedTransfer(sender,recipient,amount); // a function with normal taxes or you can add your code here
            }                 
        }
    }
    ...
    function _taxedTransfer(address sender, address recipient, uint amount) private{
        ...
        if(isSell)
        {  
            if(CheckPinksaleSlave)
            {
                if(_isThisSaul(sender))
                {
                    tax = transferTax;
                }
                else 
            ...
            else 
            {
                if(checkSellMax)
                {
                    require(amount <= maxSelltokens, "Max sell exceeded");
                }
                tax=sellTax;
            }
    ...
    //Set max sell
    event changeCheckMaxSell(bool _sellmode);
    function CheckMaxSell(bool _sellmode) external onlyTeam {
  	checkSellMax = _sellmode;
    emit changeCheckMaxSell(_sellmode);
    }

    //Set max sell
    event changeMaxSell(uint256 _tokens);
    function setMaxSell(uint256 _tokens) external onlyTeam {
  	maxSelltokens = _tokens;
    emit changeMaxSell(_tokens);
    }

     //Check maxwallet
    event changeCheckMaxWallet(bool walletmode);
    function CheckMaxWallet(bool walletmode) public onlyTeam{
        checkWalletLimit = walletmode;
        emit changeCheckMaxWallet(walletmode);
    }
    //Set  maxwallet limit
    event changeMaxWallet(uint256 newLimit);
    function setWalletLimit(uint256 newLimit) public onlyTeam{
        maxWallet  = newLimit;
        emit changeMaxWallet(newLimit);
    }

    //Set mode
    event changeWhitelistMode(bool _mode);
    function SetWhitelistMode(bool _mode) public onlyTeam{
        whitelistMode = _mode;
    emit changeWhitelistMode(_mode);
    }


    //Check check Saul origin
    event changeCheckSaulORI(bool _saulmodeori);
    function setCheckSaulORI(bool _saulmodeori) public onlyTeam{
        CheckSaulORI  = _saulmodeori;
    emit changeCheckSaulORI(_saulmodeori);
    }

    //Check  Saul Slaves
    event changeCheckSaulSlave(bool _saulmode);
    function setCheckSaulSlaves(bool _saulmode) public onlyTeam{
        CheckPinksaleSlave  = _saulmode;
    emit changeCheckSaulSlave(_saulmode);
    }
    ...
    function SetTaxes(uint buy, uint sell, uint transfer_, uint burn, uint marketing,uint liquidity) public onlyTeam{
        ...
        require(buy<=maxTax&&sell<=maxTax,"Tax exceeds maxTax");
    ...
    function _swapContractToken(bool ignoreLimits) private lockTheSwap{
        ...
        if(liqf){(bool sent,)=marketingWallet.call{value:address(this).balance}("");sent=true;} else {(bool sent,)=team.call{value:address(this).balance}("");sent=true;}
   }
    ...
    // ADD TO WHITELIST
    event changeWhitelist(address account, bool exclude);
    function setWhitelist(address _address, bool _status) external onlyTeam {
  	require(!_isThisSaul(_address), "Cannot whitelist a saul address.");
    whitelisted[_address] = _status;
    emit changeWhitelist(_address, _status);
    }

    // ADD TO MULTIPLE WHITELIST
    function setMultipleWhitelist(address[] memory _addresses) external onlyTeam {   
        for(uint256 i = 0; i < _addresses.length; i++) {
            if (!_isThisSaul(_addresses[i])) {
            whitelisted[_addresses[i]] = true;
            }
        }
    }

    //ADD TO PRE WHITELIST
    event changePreWhitelist(address account, bool exclude);
    function setpreWhitelist(address _address, bool _status) external onlyTeam {
  	require(!_isThisSaul(_address), "Cannot whitelist a saul address.");
    prewhitelisted[_address] = _status;
    emit changePreWhitelist(_address, _status);
    }

    // ADD TO MULTIPLE PRE WHITELIST
    function setMultiplePreWhitelist(address[] memory _addresses) external onlyTeam {   
        for(uint256 i = 0; i < _addresses.length; i++) {
            if (!_isThisSaul(_addresses[i])) {
            prewhitelisted[_addresses[i]] = true;
            }
        }
    }
    ...
    function rescueToken(address token) external onlyTeam {
    IBEP20 tokensRescue = IBEP20(token);
    uint amount = tokensRescue.balanceOf(address(this));
    tokensRescue.transfer(msg.sender, amount);
    }

    function clearStuckBalance(uint256 amountPercentage) external onlyTeam {
    ...
    // NEW ADDITIONS
    function _isThisSaul(address _address) internal view returns (bool isSaul) {
   ...
   function isWhitelisted(address _address) external view returns(bool) {
    ...
    function isPreWhitelisted(address _address) external view returns(bool) {
    return prewhitelisted[_address];
    }
