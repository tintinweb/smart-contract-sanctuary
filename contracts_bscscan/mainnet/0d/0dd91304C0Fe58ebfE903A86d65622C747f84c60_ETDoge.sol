...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minPeriod = 1 hours; //distribution time
    uint256 public minDistribution = 1 * (10 ** 18);    //minimum distribution set at 0.01bnb
    ...
    function getDistributionCriteria() external view returns (uint256, uint256) {
        return (
            minPeriod,
            minDistribution
        );
...
contract ETDoge is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "ETDoge";
    string constant _symbol = "ETDOGE";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1000000000000000 * (10 ** _decimals); // 1,000,000,000,000,000
    uint256 public _maxTxAmount = _totalSupply; //initial 100%
    ...
    uint256 buybackFee = 900;
    uint256 reflectionFee = 500;
    uint256 marketingFee = 200;
    uint256 totalFee = 1600;
    uint256 extraSellFee = 200;
    uint256 feeDenominator = 10000;

    address public marketingWalletAddress  = 0xB88CC5B248ab253B04e54B1651a0FcDa814032b0;
    ...
    uint256 autoBuybackCap = 100 * (10**18); //100BNB
    uint256 autoBuybackAccumulator;
    uint256 autoBuybackAmount = 0.1 * (10**18); //0.1
    ...
    uint256 swapThreshold = _totalSupply / 100; // 1%
    ...
    function shouldTakeFee(address sender, address recipient) internal view returns (bool) {
        return feeEnabled && !isFeeExempt[sender] && !isFeeExempt[recipient];
    }

    function getTotalFee(bool selling) public view returns (uint256) {
        return selling ? totalFee.add(extraSellFee) : totalFee;
    }
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(marketingWalletAddress).call{value: amountBNBMarketing, gas: 30000}("");
    ...
    function setFees(
        ...
        totalFee = buybackFee.add(reflectionFee).add(marketingFee);

        feeDenominator = _feeDenominator;
        
        emit FeesUpdated(_enabled, _buybackFee, _reflectionFee, _marketingFee, extraSellFee, _feeDenominator);
    }

    function setMarketingWalletAddress(address payable wallet)  external authorized  {
    ...
    function getDistributorSettings() external view returns (uint256, bool) {
        return (
            distributorGas,
            autoClaimEnabled
        );
    ...
    event FeesUpdated(bool enabled, uint256 buybackFee, uint256 reflectionFee, uint256 marketingFee, uint256 extraSellFee, uint256 feeDenominator);
