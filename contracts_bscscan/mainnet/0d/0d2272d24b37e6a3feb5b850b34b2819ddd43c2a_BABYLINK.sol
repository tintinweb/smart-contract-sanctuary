...
contract BABYLINK is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0x50eD5460B21Bf048e3994F8C95c920bf85a76E5b); // Marketing Address
    address payable public linkCharityWalletAddress = payable(0xae7D2f0a626E99929B8bA1Dd7951FDc578374FC2); // link Address
    ...
    string private _symbol = "BBYLINK";
    uint8 private _decimals = 9;

    uint256 public _burnFee = 2;
    uint256 public _liquidityFee = 5;
    uint256 public _marketingFee = 5;
    uint256 public _linkCharityFee = 5;
    ...
    constructor () {
        ...
        _totalTaxPercent = _burnFee.add(_liquidityFee).add(_marketingFee).add(_linkCharityFee);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _marketingFee.add(_linkCharityFee) > 0)
        {
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(forWallets);
            uint256 newBalance = address(this).balance.sub(initialBalance);
    
            uint256 marketingShare = newBalance.div(_marketingFee.add(_linkCharityFee)).mul(_marketingFee);
            ...
            if(linkWalletShare > 0)
                transferToAddressETH(linkCharityWalletAddress, linkWalletShare);
        }
    ...
    function setTaxes(uint256 newBurnFee, uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newLinkCharityTax) external onlyOwner() {
        ...
        _linkCharityFee = newLinkCharityTax;
        _totalTaxPercent = _burnFee.add(_liquidityFee).add(_marketingFee).add(_linkCharityFee);
        _prevTotalTaxPercent = _totalTaxPercent;
    }
    ...
    function setLinkCharityWalletAddress(address newAddress) external onlyOwner() {
        linkCharityWalletAddress = payable(newAddress);
    }
