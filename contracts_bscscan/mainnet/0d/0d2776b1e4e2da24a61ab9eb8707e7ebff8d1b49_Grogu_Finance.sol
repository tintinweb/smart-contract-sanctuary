 ...
 * Cloned with permission and attribution, Grogu.Finance, https://grogu.finance/
 *
 ...
 * like SafeMoon and their clones can.
 *
 ...
 *
 * GROGU's core team is welcoming, positive and inclusive: If you clone our token or
...
contract MultiSig is Context {
    ...
    modifier whenChangeByOperatorOff() {
        require(
            _change_operator_stopped,
            "GROGU::Multisig::Error: Changing Operator To Permission Revoked"
        );
        _;
    }
    ...
    modifier whenRateChangeOff() {
        require(
            ...
            "GROGU::Multisig::Error: RateChange Permission Revoked"
        );
        _;
    }
    ...
    modifier whenOwnerPrivilegesOff() {
        require(
            ...
            "GROGU::MultiSig::Error: OwnerPrivileges Revoked"
        );
        _;
    }
    ...
    modifier whenMultiSigOff() {
        require(
            _multisig_stopped,
            "GROGU::MultiSig::Error: MultiSig Denied: Permission Revoked"
        );
        _;
    }
...
abstract contract Ownable is Context, MultiSig {
    ...
    function transferMultiSig(address newMultiSig) public virtual onlyOperator whenMultiSigOn {
        require(
...
contract Grogu_Finance is BEP20("Grogu.Finance", "BabyYoda") {
    using Address for address;
    using SafeMath for uint256;
        ...
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(
                    ...
                    "GROGU::antiWhale: Transfer amount exceeds the maxTransferAmount"
                );
            }
        }
        _;
    ...
    modifier blacklisted(address sender, address recipient) {
        if (_blacklist[sender] == true || _blacklist[recipient] == true) {
            ...
            revert("GROGU::blacklisted: Transfer declined");
        }
        _;
    }
 ...
 modifier NoContractsOn(address sender, address recipient) {
    ...
    modifier MintWrappersOn(address recipient) {
        if (_mint_wrappers_stopped == true) {
            if (
                ...
                revert(
                    "GROGU::MultiSig::Minting to non-whitelisted contracts declined"
                );
            }
        }
        _;
        ...
        whenRateChangeOn
    {
        require(
            ...
            "GROGU::transferCharityAddress: contract cannot be the zero address"
        );
        ...
        whenRateChangeOn
    {
        require(
            ...
            "GROGU::E4_transferTaxAddress: contract cannot be the zero address"
        );
        ...
        whenRateChangeOn
    {
        require(
            _charityRate <= 10000,
            "GROGU::E5_updateCharityRate: Charity rate must not exceed the maximum rate."
        );
        ...
        whenRateChangeOn
    {
        require(
            ...
            "GROGU::E6_updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate."
        );
        ...
        whenRateChangeOn
    {
        require(
            ...
            "GROGU::E4_updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate."
        );
    ...
    // blacklisted(sender, recipient) antiWhale(sender, recipient, amount)
    {
        ...
        if (charityRate > 0 && transferTaxRate == 0) {
            ...
            require(
                ...
                "GROGU::transfer: Charity value invalid"
            );
            ...
            require(
                ...
                "GROGU::transfer: Tax value invalid"
            );
        ...
        whenRateChangeOn
    {
        ...
        require(
            groguSwapPair != address(0),
            "GROGU::updateGroguRouter: Invalid pair address."
        );
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "GROGU::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "GROGU::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "GROGU::delegateBySig: signature expired"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "GROGU::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "GROGU::_writeCheckpoint: block number exceeds 32 bits"
        );
