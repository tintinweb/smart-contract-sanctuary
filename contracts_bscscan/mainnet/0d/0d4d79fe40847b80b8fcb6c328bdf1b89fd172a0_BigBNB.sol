...
contract BigBNB is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address payable public operations1Address =
        payable(0x2EA21487AF6106f4BdFB92C14555A72f91787816); 
    address payable public operations2Address =
        payable(0x2A8ee86F5E44Caa14Dd0D3B8b78D927e66cfcDEA);
    ...
    constructor() ERC20("BigBNB", "BigBNB") {
        dividendTracker = new BigBNBDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        BigBNBDividendTracker newDividendTracker = BigBNBDividendTracker(
            payable(newAddress)
        );
...
contract BigBNBDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "BigBNB_Dividend_Tracker",
            "BigBNB_Dividend_Tracker"
        )
    {
        bigBNB = BigBNB(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "BigBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BigBNB contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(bigBNB);

            uint256 prevBalance = bigBNB.balanceOf(address(this));
            ...
            {
                uint256 received = bigBNB.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    bigBNB.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
