...
// DragoniteToken with Governance.
contract DragoniteToken is BEP20 {
    ...
    // Min amount to liquify. (default 250 DRAGONITEs)
    ...
    // The trading pair
    address public dragoniteSwapPair;
    // In swap and liquify
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxHolding() > 0 && sender != owner() && recipient != owner()) {
            if ( _excludedFromAntiWhale[recipient] == false ) {
                require(amount <= maxHolding() - balanceOf(recipient) && balanceOf(recipient) <= maxHolding(), "DRAGONITE::antiWhale: Transfer amount would result in a balance bigger than the maxHoldingRate");
            }
        }
        _;
    }
    ...
    /**z
     * @notice Constructs the DragoniteToken contract.
     */
    constructor() public BEP20("Dragonite Token", "DRAGONITE") {
    ...
    /// @dev overrides transfer function to meet tokenomics of DRAGONITE
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (
            ...
            && dragoniteSwapPair != address(0)
            && sender != dragoniteSwapPair
            && sender != owner()
        ) {
            swapAndLiquify();
        }
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "DRAGONITE::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "DRAGONITE::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the DragoniteSwap pair path of token -> weth
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "DRAGONITE::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "DRAGONITE::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxHoldingRate(uint16 _maxHoldingRate) public onlyOperator {
        require(_maxHoldingRate >= 100, "DRAGONITE::updateMaxHoldingRate: Max holding rate must not be below the minimum rate.");
    ...
    function updateSwapPair() public onlyOperator {
        dragoniteSwapPair = IUniswapV2Factory(SwapRouter.factory()).getPair(address(this), SwapRouter.WETH());
        require(dragoniteSwapPair != address(0), "DRAGONITE::updateSwapPair: Invalid pair address.");
        emit SwapRouterUpdated(msg.sender, address(SwapRouter), dragoniteSwapPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "DRAGONITE::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "DRAGONITE::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "DRAGONITE::delegateBySig: invalid nonce");
        require(now <= expiry, "DRAGONITE::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "DRAGONITE::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying DRAGONITEs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "DRAGONITE::_writeCheckpoint: block number exceeds 32 bits");
