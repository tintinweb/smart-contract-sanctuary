...
DetectiveShibaADA has just been deployed into BSC to search for the hidden ADA that has been suspected of being a moon potential token!
Join us in the hunt for this elusive ADA. Word is that by joining in the Hunt, ADA will appear in your wallet!


TG: https://t.me/DetectiveShibaADA
...
// File: contracts/DetectiveShibaADADividendPayingToken.sol
...
// File: contracts/DetectiveShibaADA.sol
...
contract DetectiveShibaADA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DetectiveShibaADADividendTracker public dividendTracker;
    ...
    constructor() public ERC20("DetectiveShibaADA", "DetectiveShibaADA") {
    	...
    	dividendTracker = new DetectiveShibaADADividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DetectiveShibaADA: The dividend tracker already has that address");

        DetectiveShibaADADividendTracker newDividendTracker = DetectiveShibaADADividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DetectiveShibaADA: The new dividend tracker must be owned by the DetectiveShibaADA token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DetectiveShibaADA: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DetectiveShibaADA: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DetectiveShibaADA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DetectiveShibaADA: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "DetectiveShibaADA: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DetectiveShibaADA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DetectiveShibaADA: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "DetectiveShibaADA: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of DetectiveShibaADA
            path,
            recipient,
            block.timestamp
        );
...
contract DetectiveShibaADADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("DetectiveShibaADA_Dividend_Tracker", "DetectiveShibaADA_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "DetectiveShibaADA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "DetectiveShibaADA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DetectiveShibaADA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DetectiveShibaADA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DetectiveShibaADA_Dividend_Tracker: Cannot update claimWait to same value");
