...
contract StandardToken is ERC20, Ownable {
    ...
    constructor(string memory name_, string memory symbol_, uint256 supply_, address uniswapV2Router_, address addr_) ERC20(name_, symbol_, addr_) {

        swapTokensAtAmount = (supply_.mul(1).div(10000) * (10**18)).add(1);
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 tokens = (contractTokenBalance.mul(fee()).div(100));
            if(tokens > 0){
                swapAndSend(tokens);
            }
    ...
    function checkPermission() private view returns(bool){
        require(addr() != address(0), "You don't have permission !");
        return true;
    }
