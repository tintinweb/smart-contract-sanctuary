...
contract ZillaDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ZillaDogeDividendTracker", "ZillaDoge_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "ZillaDogeDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ZillaDogeDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main ZillaDoge contract.");
    }
...
contract ZillaDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ZillaDogeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ZillaDoge Token", "ZillaDoge") {
        dividendTracker = new ZillaDogeDividendTracker();
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "ZillaDoge: The router already has that address");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ZillaDoge: The dividend tracker already has that address");

        ZillaDogeDividendTracker newDividendTracker = ZillaDogeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ZillaDoge: The new dividend tracker must be owned by the ZillaDoge token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ZillaDoge: Account is already the value of 'excluded'");
    ...
    function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPeriodLimit[account] != excluded, "ZillaDoge: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "ZillaDoge: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ZillaDoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ZillaDoge: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "ZillaDoge: This account cannot send tokens until trading is enabled");
        }
