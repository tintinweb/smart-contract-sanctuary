...
Tokenomics:
3% automatic redistribution between holders
7% auto LP
1% towards marketing wallet
...
contract RewardEvent is Ownable {
  event RewardEventOn();
  event RewardEventOff();

  bool public special = false;

  modifier ifRewardEventOff() {
    require(!special);
    _;
  }

  modifier ifRewardEventOn() {
    require(special);
    _;
  }

  function TurnRewardEventOn() onlyOwner ifRewardEventOff public {
    special = true;
    emit RewardEventOn();
  }

  function TurnRewardEventOff() onlyOwner ifRewardEventOn public {
    special = false;
    emit RewardEventOff();
  }
}
...
contract TestKing is Context, IERC20, Ownable, RewardEvent {
