...
contract SafeEarnPresale is ReentrancyGuard, Ownable {
    ...
    uint256 public whitelisterHardcap = 1 ether;
    ...
    uint256 public tokensPerBNB = 168_000_000_000;
    uint256 bnbReceived = 0;

    uint256 whiteListDuration = 15 minutes;
    ...
    function buy (address beneficiary) public payable nonReentrant {
        ...
        if (whiteListDuration + timestampStarted > block.timestamp) {
            ...
            require(address(this).balance + msg.value <= whitelisterHardcap, "You are trying to buy more than the whitelist hardcap, unlucky.");
        }
        ...
        require (bnbReceived <= 3 ether, "Hardcap reached");
    ...
    function setWhitelisterOnly(bool only) external onlyOwner {
        onlyWhitelister = only;
    }
    ...
    function claimTokens() external nonReentrant {
        ...
        require (tokensOwned[msg.sender] > 0, "User should own some SAFEARN tokens");
        require (tokensUnclaimed[msg.sender] > 0, "User should have unclaimed SAFEARN tokens");
        require (bountyToken.balanceOf(address(this)) >= tokensOwned[msg.sender], "There are not enough SAFEARN tokens to transfer, wtf?");
