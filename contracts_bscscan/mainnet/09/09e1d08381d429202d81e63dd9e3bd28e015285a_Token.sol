 ...
 */
contract Token is Context, AccessControlEnumerable, ERC20Burnable {
    ...
    event Minting(address indexed from, address indexed to, uint256 value);
    
    constructor(string memory name, string memory symbol, bool _redeemOption, address _contract,uint256 _share) 
    ERC20(name, symbol) {
    ...
    function addMinter(address minter) public virtual{
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ERC20PresetMinterPauser: must have admin role to add minter");
    ...
    function provideLiquidity() public payable{
        ...
        if(poolShare[_msgSender()] == 0){
            require(msg.value >= 1000000000000000000, "CrowdFundingException: Value must be greater than one BNB");
            subscribers.push(_msgSender());
            poolShare[_msgSender()] = msg.value;
            timestamps[_msgSender()] = block.timestamp;
            poolTotal = poolTotal.add(msg.value);
            _mint(_msgSender(),(msg.value).mul(rate));
        }else{
            uint256 value = poolShare[_msgSender()];
            require(msg.value.add(value) >= 1000000000000000000, "CrowdFundingException: Total value must be greater than one BNB");
    ...
    */
    function refund() public virtual{
       require(isRedeemOpen,"CrowdFundingException: Redeem is not opened yet!");
        ...
        poolTotal = poolTotal.sub(poolShare[_msgSender()]);
        // burnFrom(msg.sender,poolShare[_msgSender()].mul(rate));
        // burnFrom(msg.sender, poolShare[msg.sender]);
        _burn(_msgSender(), poolShare[_msgSender()]);
        subscriber.transfer(poolShare[_msgSender()].add(earnings[_msgSender()]));
        poolShare[_msgSender()] = 0;
    ...
    function redeem() public virtual{
        ...
        address payable subscriber = payable(_msgSender());
        poolTotal = poolTotal.sub(poolShare[_msgSender()]);
        _burn(msg.sender,poolShare[_msgSender()].mul(rate));
        subscriber.transfer(poolShare[_msgSender()].add(earnings[_msgSender()]));
        poolShare[_msgSender()] = 0;
    }
