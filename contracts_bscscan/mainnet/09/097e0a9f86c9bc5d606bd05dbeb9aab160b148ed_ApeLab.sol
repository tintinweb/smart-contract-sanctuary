...
contract ApeLab is Context, Ownable, ERC20 {
    using SafeMath for uint256;
    ...
    uint256 public maxTransfer = 100 *10**18;

    uint256 public rewardDivider = 10;
    ...
    constructor() ERC20("ApeLab","AL") public {
        
        
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        //Testnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1 BSC
        //Mainnet: 0x10ED43C718714eb63d5aA57B78B54704E256024E BSC
    ...
    function setRewardDivider(uint256 _rewardDivider) external onlyOwner {
        require(_rewardDivider >= 0 && _rewardDivider <= 50, "RewardDivider out of range!");
	...
	maxTransfer = _amount * 10**18;
	}

    function _transfer(address sender, address recipient, uint256 amount) internal override {
		
		if(!tradingenabled && !isTaxless[sender]){
        ...
        if(isSwapEnabled && !inSwap && sender != uniswapV2Pair && !isTaxless[sender]){
            swapAndDistribute();
        }
        uint256 transferAmount = amount;
        
        
        if(isTaxless[sender] || isTaxless[recipient] || (recipient == stakingPool)){
            ...
            transferAmount = amount.sub(fee).sub(_burnFee);
            taxFeeTotal = taxFeeTotal.add(fee);

        
        //If it's a sell we increase the fee and burn the Tokens
        uint256 transferAmountAfterFee = transferAmount;
        ...
        if(recipient == uniswapV2Pair && !isTaxless[sender]){
            ...
            super._transfer(sender,address(this),feeForSelling);
            
            transferAmount = transferAmountAfterFee.sub(feeForSelling);
        }
    ...
    function swapAndDistribute() private lockTheSwap {
        ...
        uint256 amountToMarketing = (address(this).balance).mul(rewardDivider).div(100);
        uint256 amountToPool = (address(this).balance).sub(amountToMarketing);
        
        transferToAddressETH(marketingAddress,amountToMarketing);
