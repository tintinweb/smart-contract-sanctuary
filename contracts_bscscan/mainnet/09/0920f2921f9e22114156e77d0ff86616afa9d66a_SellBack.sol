...
contract SellBack is Ownable {
    ...
    function saveQueueByContract(address sellerAdd,uint256 _amount,address _referral) public onlyMasterChef{
        //convert 50% to xos
        uint256 amountUSDT = _amount*10000000000/2; 
    ...
    function savetrans(P2PQueue storage p2p) internal{
        ...
        //transfer
        xosusdtToken.safeTransferFrom(address(this), address(cloudEco), liquidity);
        //add cloud eco
        cloudEco.depositFromSellback(0,liquidity,address(p2p.referral),address(p2p.seller));
        //add to history
        history.push(p2p);
    }
}