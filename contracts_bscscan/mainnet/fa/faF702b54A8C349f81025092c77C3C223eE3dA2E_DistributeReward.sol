...
function swapBNBForCake(uint256 bnbAmount) private {
    ...
    function distribute() public notContract nonReentrant {
        ...
        Love(LoveToken).transferFrom(msg.sender, deadAddress, 1e18); //Burns 1 LOVE token. Oh no!!
        uint256 balance = address(this).balance;

        uint256 devReward = balance * 20 / 100;
    ...
    function vaultDeposit() internal {
        uint256 tokenAmount = ERC20(CAKE).balanceOf(address(this));
        if (tokenAmount != 0){
        MRCVault(MRCVaultAddr).deposit(tokenAmount);
        }
    }

    function getCallRewardBalance() public view returns(uint256) {
        uint256 balance = address(this).balance;
        uint256 callReward =  balance * 3 / 100;
...
contract MRCVault is Ownable, Pausable {
    using SafeERC20 for IERC20;
    ...
    function harvest() external notContract whenNotPaused {
        require(MyRevengeNFT(NFTContract).balanceOf(msg.sender) > 0, "Must own a MyRevengeCat NFT to claim!");
        Love(LoveToken).transferFrom(msg.sender, deadAddress, 1e18);
        uint256 before = available();
        IMasterChef(masterchef).leaveStaking(0);
        uint256 bal = available() - before;
        token.safeTransfer(msg.sender, bal); //We give all the yield to the caller of the function
        _earn();
