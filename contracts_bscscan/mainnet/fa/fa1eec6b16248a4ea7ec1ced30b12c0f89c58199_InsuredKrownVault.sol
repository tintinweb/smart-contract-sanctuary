...
contract InsuredKrownVault {
    ...
    BeefyVault constant krwPool = BeefyVault(0xEb4804389A843676686715785C8Df27EA6E43F9b);
    ...
    int256 public lastTotalCake;
    uint256 public lastMooValue;
    ...
    function deposit(uint256 amount) external {
        ...
        uint256 beforeBalance = krwPool.balanceOf(address(this));
        krwPool.deposit(amount);
        uint256 newBalance = krwPool.balanceOf(address(this));
        uint256 mooGained = newBalance - beforeBalance;
        
        uint256 increase = mooGained;
        if (beforeBalance > 0) {
        ...
        totalCake += int256((mooGained * krwPool.getPricePerFullShare()) / (10 ** 18));
    ...
    function cashout(uint128 amount) external {
        ...
        uint256 totalMoo = krwPool.balanceOf(address(this));
        ...
        address(krwPool).call(abi.encodePacked(krwPool.withdraw.selector, abi.encode(mooCashout)));

        uint256 gained = krw.balanceOf(address(this)) - beforeBalance;
        totalCake -= int256(gained);
    ...
    function pullOutstandingDivs() external {
        ...
        uint256 mooPrice = krwPool.getPricePerFullShare();
        uint256 moo = krwPool.balanceOf(address(this));
        
        uint256 mooValue = (moo * mooPrice) / (10 ** 18);
        int256 deposits = totalCake - lastTotalCake;
        int256 divsGained = int256(mooValue - lastMooValue) - deposits;
        require(divsGained > 0); // Sanity
        uint256 nutsCut = uint256(divsGained) * 20 / 100;
        uint256 mooAmount = (nutsCut * (10 ** 18)) / mooPrice; 
        
        lastTotalCake = totalCake;
        lastMooValue = mooValue;
        
        uint256 beforeBalance = krw.balanceOf(address(this));
        address(krwPool).call(abi.encodePacked(krwPool.withdraw.selector, abi.encode(mooAmount)));
        nutsCut = krw.balanceOf(address(this)) - beforeBalance;
        if (nutsCut > 0) {
            pendingNutsAlloc += nutsCut;
            totalCake -= int256(nutsCut);
        }
    }
    ...
    function mooBalance(address farmer) view public returns (uint256) {
        return (players[farmer].balance * krwPool.balanceOf(address(this))) / totalDeposits;
    }
    
    function cakeBalance(address farmer) view public returns (uint256) {
        return (mooBalance(farmer) * krwPool.getPricePerFullShare()) / (10 ** 18);
    }
    
    function totalCakeBalance() view public returns (uint256) {
        return (krwPool.balanceOf(address(this)) * krwPool.getPricePerFullShare()) / (10 ** 18);
    }
