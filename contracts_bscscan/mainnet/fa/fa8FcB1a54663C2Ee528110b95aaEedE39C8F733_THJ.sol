...
contract THJ is ERC20,Ownable {
    ...
    function publicSale() external payable {
        ...
        uint256 tokensBought = (bnb * 10**18).div(tokenPrice);
        require(
            publicSalePaused == false,
            "Public sale: public sale is paused or it has stopped"
        );
        require(
            totalSupply() + tokensBought < MAX_SUPPLY,"max supply reached");    
        
        payable(owner()).transfer(msg.value);
        _mint(_msgSender(), tokensBought);
    }
