...
contract IGOPool is Ownable, ReentrancyGuard {
    ...
    mapping(address=>uint256) public userIgoList;
    ...
    function getUserInfo(address _address) public view returns (Info memory igoInfo,uint256 quote) {
        igoInfo = Info(igoToken,USDT,minAmount,maxAmount,price);
        quote = maxAmount.mul(userIgoList[_address]);
    }
    ...
    function ido(uint256 _amount) public nonReentrant {
        ...
        require(_amount.add(userIgoList[msg.sender])<=maxAmount,"e02");
        uint256 usdtAmount = _amount.mul(price).div(1e18);
        USDT.transferFrom(msg.sender,feeAddress,usdtAmount);
        igoToken.safeTransfer(msg.sender,_amount);
        userIgoList[msg.sender] = userIgoList[msg.sender].add(_amount);
    }
