...
// Dependency file: contracts/interfaces/IPancakeSwapV2Router02.sol
...
// import "contracts/interfaces/IPancakeSwapV2Router02.sol";
...
contract DePayRouterV1PancakeSwap01 {
  ...
  address public immutable PancakeSwapV2Router02;

  // Indicates that this plugin requires delegate call
  bool public immutable delegate = true;

  // Pass WBNB and the PancakeSwapRouter when deploying this contract.
  constructor (
    address _WBNB,
    address _PancakeSwapV2Router02
  ) public {
    WBNB = _WBNB;
    PancakeSwapV2Router02 = _PancakeSwapV2Router02;
  }

  // Swap tokenA<>tokenB, ETH<>tokenA or tokenA<>ETH on PancakeSwap as part of the payment.
  ...
  function execute(
    ...
    // Make sure swapping the token within the payment protocol contract is approved on the PancakeSwap router.
    if( 
      path[0] != ETH &&
      IERC20(path[0]).allowance(address(this), PancakeSwapV2Router02) < amounts[0]
    ) {
      Helper.safeApprove(path[0], PancakeSwapV2Router02, MAXINT);
    }

    // PancakeSwap uses WBNB within their path to indicate bridge swapping (instead of ETH).
    // This prepares the path as applicable to the PancakeSwap router.
    ...
    if(path[0] == ETH) {
      IPancakeSwapV2Router01(PancakeSwapV2Router02).swapExactETHForTokens{value: amounts[0]}(
        amounts[1],
        uniPath,
        address(this),
        amounts[2]
      ...
      IPancakeSwapV2Router01(PancakeSwapV2Router02).swapExactTokensForETH(
        amounts[0],
        amounts[1],
        uniPath,
        address(this),
      ...
      IPancakeSwapV2Router02(PancakeSwapV2Router02).swapExactTokensForTokens(
        amounts[0],
        amounts[1],
        uniPath,
        address(this),
