...
contract MINIBASKETBALL is Context, IERC20, Ownable {
    ...
    address FUND_ADDRESS = 0x53778e0ea2c0d738D58efFA3C1bf34d1413A94eB;
    ...
    bool swapOpened;
    string private _name = "Mini Basketball";
    string private _symbol = "MBC";
    ...
    event TransferLP(uint256 value);
    event TransferFund(
    ...
    constructor() {
        ...
        ); // 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 for bsc test mainnet
        ...
        _isExcludedFromFee[FUND_ADDRESS] = true;
        _isExcludedFromReward[FUND_ADDRESS] = true;
    ...
    function setSwapStatus(bool val) external onlyOwner {
        swapOpened = val;
    }
    ...
    function totalSentToFund() public view returns (uint256) {
        return balanceOf(FUND_ADDRESS);
    }
        ...
        )
    {
        ...
        (
            ...
            uint256 rHODLrFee
        ) = _getRValues(tAmount, tHODLrFee, tLP, tFund, _getRate());
        return (
            rAmount,
            rTransferAmount,
            rHODLrFee,
        ...
        )
    {
        ...
        uint256 tTransferAmount = tAmount.sub(tHODLrFee).sub(tLP).sub(tFund);
        return (tTransferAmount, tHODLrFee, tLP, tFund);
    }
        ...
        )
    {
        ...
        uint256 rTransferAmount = rAmount.sub(rHODLrFee).sub(rLP).sub(rFund);
    ...
    function removeAllFee() private {
        if (_rewardFee == 0 && _lpFee == 0) return;
    ...
    function _setFundWallet(address _FUND_ADDRESS) external onlyOwner {
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            if (to == uniswapV2Pair) {
                require(swapOpened, "swap is not open.");
            }
        }
        bool takeFee = true;
    ...
    function _transferLP(uint256 tLP) private {
        uint256 currentRate = _getRate();
        uint256 rLP = tLP.mul(currentRate);
        _rOwned[BURN_ADDRESS] = _rOwned[BURN_ADDRESS].add(rLP);
        if (_isExcludedFromReward[BURN_ADDRESS])
            _tOwned[BURN_ADDRESS] = _tOwned[BURN_ADDRESS].add(tLP);
    }

    function _transferFund(uint256 tFund) private {
        uint256 currentRate = _getRate();
        uint256 rFund = tFund.mul(currentRate);
        _rOwned[FUND_ADDRESS] = _rOwned[FUND_ADDRESS].add(rFund);
        if (_isExcludedFromReward[FUND_ADDRESS])
            _tOwned[FUND_ADDRESS] = _tOwned[FUND_ADDRESS].add(tFund);
    }
    ...
    function _transferFromExcluded(
        ...
        emit TransferLP(tLP);
        emit TransferFund(sender, FUND_ADDRESS, tFund);
