...
contract discovery is owned {
    ...
    function payForLevel(address _user) internal returns (bool){
        payOut(userAddressByID[userInfos[_user].origRefID], 25 * ( 10 ** 17), 1, 1);
    ...
    function findFreeReferrer(address _user) public view returns(address) {
        ...
        address[] memory parents = new address[](1022);
    ...
    event payOutEv(uint timeNow,address caller,uint totalAmount, uint _type,  uint _level);
    function payOut(address _user, uint _amount, uint _type,  uint _level) internal returns(bool)
    {
        ...
        emit payOutEv(now, _user, _amount, _type, _level);
    ...
    function updateNPayAutoPool(uint _level,address _user) internal returns (bool)
    {
        ...
        if(_level < 4)
        {
            ...
            payOut(ref, 5 ** _level * ( 10 ** 18), 3, 1);
        }
        else
        {
            ref = userAddressByID[autoPoolLevel[_level-1][temp.autoPoolParent].userID];
            payOut(ref, 10 ** _level * ( 10 ** 18), 3, 1);
        }
