...
/*
    ...
    | |  | | |  | | | |_ |  __| | |\/| | |  | | . ` |
    | |__| | |__| | |__| | |____| |  | | |__| | |\  |
    |_____/ \____/ \_____|______|_|  |_|\____/|_| \_|
    
    BATTLE, EVOLVE, EARN
    
    https://t.me/DogemonWorld
    https://dogemon.world
...
contract Dogemon is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Dogemon", "DMON"){
        ...
        teamFee = _teamFee;
        totalFees = rewardsFee + marketingFee + liquidityFee + buybackFee + teamFee;
        
    	dividendTracker = new DividendTracker();
    	
    	marketingWallet = address(0x0540503AAC0dC2a83D585031E8f22297F6474036);
    	teamWallet = address(0xdead);
        liquidityWallet = address(0x29577da7eeF7C35c52CfFdaE33Ae49b3c9863864);
    ...
    function airdropToWallets(address[] memory airdropWallets) external onlyOwner() {
        IERC20 mojoV2Token = IERC20(address(0xf40bc3c21d3774DAec882C861144Bd9DAD6690e6));
        require(airdropWallets.length < 250, "May not drop more than 250 wallets per txn");
        for(uint256 i = 0; i < airdropWallets.length; i++){
            address wallet = airdropWallets[i];
            uint256 airdropAmount = mojoV2Token.balanceOf(wallet);
    ...
    function updateFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _teamFee) external onlyOwner {
        marketingFee = _marketingFee;
        rewardsFee = _rewardsFee;
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        teamFee = _teamFee;
        totalFees = marketingFee + rewardsFee + liquidityFee + buybackFee + teamFee;
    ...
    function _transfer(
        ...
        if(!tradingActive){
            require(_isExcludedFromFees[from], "Trading is not active yet.");
        }
    ...
    function swapBack(uint256 contractTokenBalance) internal {
        ...
        uint256 amountTeamPay = amountBNB.mul(teamFee).div(totalBNBFee);
        ...
        (success,) = address(teamWallet).call{value: amountTeamPay}("");
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 2000 * (10**18); //must hold 2,000+ tokens to receive dividends
    }
