...
contract YSLOptSingleApe is YSLOpt {
    ...
    function setCompoundingAddress(address _compoundingAddress) external onlyRole(STRAT_ROLE) {
    ...
    function compound() external override {
        ...
        if (wantBalance > DUST) {
            ...
            collectControllerFee(usdBefore, apeSwap, compoundingAddress);
    ...
    function currentRewardUSD(address _user) public view returns (uint256) {
        ...
        uint256 totalCompoundsUsd = IStrategySwap(stratSwap).getBusdAmount(totalCompounds, path0, apeSwap);

        // Pending rewards
        uint256 pendingBanana = IPancakeMaster(apeMaster).pendingCake(id, address(this));
        totalCompoundsUsd += IStrategySwap(stratSwap).getBusdAmount(pendingBanana, path0, apeSwap);
