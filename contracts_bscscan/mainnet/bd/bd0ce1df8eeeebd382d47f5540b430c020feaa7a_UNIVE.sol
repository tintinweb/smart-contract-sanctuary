...
contract UNVDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("UNV_Dividend_Tracker", "UNV_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 500 * (10**8); //must hold 500+ tokens
    }

    function _transfer(address, address, uint256) internal override pure {
        require(false, "UNV_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "UNV_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main UNV contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "UNV_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "UNV_Dividend_Tracker: Cannot update claimWait to same value");
...
contract UNIVE is ERC20, Ownable {
    
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    UNVDividendTracker public dividendTracker;

    address public _marketingWallet = 0xfc912A0B3E8dC7c640d5b81c03aAC606f8FFD2b1;

    bool public restrictWhales = false;

    uint256 public maxTxAmount = 50000 * (10**8);
    uint256 public walletMax = 100000000 * (10**8);
    uint256 public swapTokensAtAmount = 25000000 * (10**8);
    ...
    constructor() ERC20("UNIVE", "UNV") {

        dividendTracker = new UNVDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "UNV: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "UNV: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "UNV: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "UNV: Cannot update gasForProcessing to same value");
