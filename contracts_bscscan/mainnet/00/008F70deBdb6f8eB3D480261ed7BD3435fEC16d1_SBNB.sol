...
//     Small BNB Smart Contract
//
//     Tokenomics:
//     13% Buy Back
//     2% Marketing Fee
//
//     Max wallet size of 5%
//
...
// File: contracts/SBNBDividendPayingToken.sol
...
contract SBNB is ERC20, Ownable {
    ...
    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    address private _marketingWallet = address(0x095487E2885Ea3A44F1153aFDB6e556Efd833969);
    ...
    uint256 public BuyBackTime = block.timestamp + 300;
    uint256 public maxBuyTranscationAmount = 50 * (10**18);
    uint256 public maxSellTransactionAmount = 10 * (10**18);
    uint256 public swapTokensAtAmount = 10 * (10**18);
    uint256 public _maxWalletToken = 50 * (10**18); // 1.5% of total supply
    ...
    constructor() public ERC20("Small BNB", "SBNB") {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SBNB: The dividend tracker already has that address");

        SBNBDividendTracker newDividendTracker = SBNBDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SBNB: The new dividend tracker must be owned by the SBNB token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SBNB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SBNB: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SBNB: The liquidity wallet is already this address");
    ...
    function _transfer(
        ...
        require(to != address(0), "ERC20: transfer to the zero address");
            ...
            if (BuyBackTime <= block.timestamp) {
                swapETHForTokens(BNBBalance);
                BuyBackTime = BuyBackTime + 300;
            }
        }
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "SBNB: This account cannot send tokens until trading is enabled");
        }
        ...
        if(
            ...
            if (sellFeeResetTime <= block.timestamp) {
                sellFeeResetTime = sellFeeResetTime + 20;
...
contract SBNBDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("SBNB_Dividend_Tracker", "SBNB_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
