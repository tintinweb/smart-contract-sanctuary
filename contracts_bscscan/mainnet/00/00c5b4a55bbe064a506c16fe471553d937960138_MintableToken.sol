 ...
 * @dev see https://eips.ethereum.org/EIPS/eip-721
 */
contract ERC721Base is Ownable, HasSecondarySaleFees, ERC721, HasContractURI, HasTokenURI, ERC721Enumerable {
    // Token name
    string public name;
    ...
    // Royality limit to capped royities while minting.
    uint16 public royalityLimit;

    // mintingCharges in wei, Will be controlled by owner
    uint public mintingCharge;
    ...
    constructor (
        ...
        uint16 _royalityLimit,
        uint _mintingCharge
    ) HasContractURI(contractURI) HasTokenURI(_tokenURIPrefix) public {
        name = _name;
        symbol = _symbol;
        royalityLimit = _royalityLimit;
    ...
    function setRoyalityLimit(uint16 _royalityLimit) public onlyOwner {
        require(_royalityLimit <= 10000, "Royality limit must be below 100% ");
        royalityLimit = _royalityLimit;
    }

    function setMintingCharge(uint _mintingCharge) public onlyOwner {
        mintingCharge = _mintingCharge;
    }
    ...
    function _mint(address to, uint256 tokenId,  uint _royality) internal {
        require(_royality <= royalityLimit, "Royality must be below royality limit");
        require(msg.value >= mintingCharge, "Minting charges required");
        _mint(to, tokenId);
...
contract MintableToken is Ownable, IERC721, IERC721Metadata, ERC721Burnable, ERC721Base {
    ...
    function mint(string memory tokenURI, uint _royality) public payable{
