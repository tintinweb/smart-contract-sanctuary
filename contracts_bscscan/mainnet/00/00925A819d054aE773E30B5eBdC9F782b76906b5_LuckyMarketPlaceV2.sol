...
contract LuckyMarketPlaceV2 is IBidNFT ,ISwapNFT, ERC721Holder, Ownable, Pausable {
   ...
   function updateBidPrice(uint256 _tokenId, uint256 _price) public override whenNotPaused {
    ...
    function getStepByTokenId(uint256 _tokenId)public view returns(uint256) {
     ...
     function offerNFT(uint256 _tokenId,uint256 _tokenTarget, uint256 _price) public override whenNotPaused  {
        ...
        _offerOwnerMap[address(_msgSender())].push(OfferOwner({userTarget:_listMap[_tokenTarget],tokenId:_tokenId, price: _price,tokenIdTarget: _tokenTarget}));
            ...
            returns (OfferOwner memory,uint256)
        {
            ...
            for (uint256 i = 0; i < len; i++) {
                if (_tokenId == offerEntries[i].tokenId) {
                    _index = i;
                    offerEntry = OfferOwner({ price: offerEntries[i].price, tokenId:offerEntries[i].tokenId, tokenIdTarget:offerEntries[i].tokenIdTarget,userTarget:offerEntries[i].userTarget});
                    break;
                }
            }
            return (offerEntry,_index);
   ...
   function getOffersLength(uint256 _tokenId) public view returns (uint256) {
    ...
    function getOffersByOwner(address _owner) public view returns (OfferOwner[] memory) {
        ...
        for (uint256 i = 0; i < len; ++i) {
            offers[i] = OfferOwner({tokenId: _offerOwnerMap[address(_owner)][i].tokenId, price:  _offerOwnerMap[address(_owner)][i].price, userTarget:  _offerOwnerMap[address(_owner)][i].userTarget, tokenIdTarget:  _offerOwnerMap[address(_owner)][i].tokenIdTarget});
        }
        return offers;            
    }  
    ...
    function autoConfirmBid(uint256 _tokenId, address _to) public {
        require(address(_msgSender()) == address(systemAdr), 'address system not exist');
