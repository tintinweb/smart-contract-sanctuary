...
contract TWAPOracleUpdater is ERC20Permit, VaultOwned {
    ...
    function addTWAPSource(address newTWAPSourceDexPool_) external onlyOwner {
        require(_dexPoolsTWAPSources.add(newTWAPSourceDexPool_), "POWER: TWAP Source already stored.");
        emit TWAPSourceAdded(newTWAPSourceDexPool_);
    }

    function removeTWAPSource(address twapSourceToRemove_) external onlyOwner {
        require(_dexPoolsTWAPSources.remove(twapSourceToRemove_), "POWER: TWAP source not present.");
...
contract PowerToken is Divine {
    using SafeMath for uint256;

    constructor() Divine("Power Token", "POWER", 9) {}
