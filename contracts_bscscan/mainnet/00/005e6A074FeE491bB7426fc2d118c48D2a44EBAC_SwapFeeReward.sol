...
// File: contracts\SwapFeeReward.sol
...
interface ISwpyToken is IERC20 {
...
contract SwapFeeReward is Ownable{
    ...
    event Rewarded(address account, address input, address output, uint256 amount, uint256 quantity, uint256 pairFee, uint256 fee, uint256 quantity1);
    ...
    function getSwapFee(address tokenA, address tokenB) internal view returns (uint swapFee) {
        swapFee = uint(1000).sub(IPancakePair(pairFor(tokenA, tokenB)).swapFee());
    }
    ...
    function swap(address account, address input, address output, uint256 amount) public onlyRouter returns (bool) {
        ...
        uint256 quantity1 = getQuantity(output, fee, targetToken);
        uint256 quantity = quantity1.mul(pool.percentReward).div(100);
        if (totalMined.add(quantity) > currentPhase.mul(maxMiningInPhase)){
            return false;
        }
        _balances[account] = _balances[account].add(quantity);
        emit Rewarded(account, input, output, amount, quantity, pairFee, fee, quantity1);
        return true;
    }
    ...
    function withdraw(uint8 v, bytes32 r, bytes32 s) public returns(bool){
        ...
        if (balance > 0){
            swpyToken.mint(msg.sender, balance);
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        if (outputToken == anchorToken) {
        ...
        } else if (IPancakeFactory(factory).getPair(outputToken, anchorToken) != address(0) && checkPairExist(outputToken, anchorToken)) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (IPancakeFactory(factory).getPair(outputToken, intermediate) != address(0) && IPancakeFactory(factory).getPair(intermediate, anchorToken) != address(0) && checkPairExist(intermediate, anchorToken)) {
