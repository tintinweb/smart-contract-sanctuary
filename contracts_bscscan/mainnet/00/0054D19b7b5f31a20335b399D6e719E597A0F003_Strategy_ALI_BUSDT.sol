...
interface IFarm{
    function ali() external view returns (address);
...
contract Strategy_ALI_BUSDT {
    // This strategy is for BSC network. ALI - BUSDT pair. RewardToken = "ali"
    ...
    address public token0; // ALI
    address public token1; // BUSDT
    address public vault;

    // ------------> Important constants <--------------
    uint256 public constant pid = 1;
    bool public constant hasPid = true;
    address public router = 0x730aCC3bBf2443f2EaEaCFc7ac7b4d8DC9E32dB8;
    address public constant lpToken = 0x3B63b57e309b0Ba9821556361616Ee955aa1EbB3;
    address public constant farm = 0x4f7b2Be2bc3C61009e9aE520CCfc830612A10694;
    address public router2 = 0xcF0feBd3f17CEf5b47b0cD257aCf6025c5BFf3b7;
    // ------------>        END          <--------------
    ...
    function autoCompound() external {
        ...
        if (getAmountLPFromFarm() > 0) {
            ...
            if(_balance > 0){
                path[0] = rewardToken;
                path[1] = WBNB; // WBNB 
                uint256 amount = _getAmountsOut(router, _balance+pendingRewardsFee, path);
                if(amount > 100) {
    ...
    function _getAmountsOut(address _router, uint256 _amount, address[] memory path) internal view returns (uint256){
    ...
    function _takeFee() internal {
        ...
        if(rewardsFeeInWBNB > 0) {
            uint256 amount = _getAmountsOut(router2, rewardsFeeInWBNB+pendingFee, path);
            if(amount > 100) {
                uint256 _balanceUSDT = getBalanceOfToken(USDT);
                _swapExactTokensForTokens(router2, rewardsFeeInWBNB + pendingFee, amount, path);
    ...
    function _addLiquidityFromAllTokens() internal {
        _swapWBNBToToken(router2, token1);
        _swapToken1ToToken0();
    ...
    function _swapExactETHForBUSDT(uint256 _amountETH) internal {
        ...
        if(_amountETH > 0){
            ...
            path[0] = WBNB;
            path[1] = token1;
            uint256 amount = _getAmountsOut(router2, _amountETH, path);
            if(amount > 100){
                IRouter(router2).swapExactETHForTokens{value:_amountETH}(
    ...
    function _swapToken1ToToken0() internal {
        ...
        if(_balance > 0){
            ...
            path[0] = token1;
            path[1] = token0;
            uint256 amount = _getAmountsOut(router, _balance, path);
            if(amount > 100){
    ...
    function _addLiquidityFromETH(uint256 _amountETH) internal {
        // swap all wrapped tokens to BUSDT
        _swapExactETHForBUSDT(_amountETH);
        _swapToken1ToToken0();
    ...
    function _swapLPtoWBNB(uint256 _percent) internal {
        ...
        if (_totalLP > 0) {
            ...
            // swap token0 and token1 to WBNB
            _swapTokenToWBNB(token0);
            _swapTokenToWBNB(token1);
        }
    }
    ...
    function _swapToYELs(uint256 _percent) internal returns (uint256 newYelBalance){ 
        ...
        _approveToken(YELtoken, router2);
        _approveToken(WBNB, router2);
        _swapWBNBToToken(router2, YELtoken);
    ...
    function _swapTokenToWBNB(address _token) internal {
        ...
        if(_balance > 0) {
            address _router = _token == token1 ? router2 : router;
    ...
    function _getToken0Price() public view returns (uint256 result) {
        // ALI to BUSDT
        address[] memory path = new address[](2);
        path[0] = token1;
        path[1] = token0;
        result = 1e36 / (_getAmountsOut(router, 100 * 1e18, path) / 100);
    }

    function _getPriceWBNBToBUSDT() public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = token1;
        return 1e36 / (_getAmountsOut(router2, 100 * 1e18, path) / 100);
    }

    function _getPriceWBNBToALI() public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = token0;
        return 1e36 / (_getAmountsOut(router, 100 * 1e18, path) / 100);
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        ...
        if(_amount > 0) {
            ...
            tCI = _getToken1Price() * t1V / d + _getToken0Price() * t0V / d;
        }
        ...
        if(_balance != 0) {
            tCI += _getToken0Price() * _balance / d;
        }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            if(t1V != 0) {
                path[0] = token1;
                path[1] = WBNB;
                TC = _getAmountsOut(router2, t1V, path);
            }
            if(t0V != 0) {
                path[0] = token0;
                path[1] = WBNB;
                TC += _getAmountsOut(router, t0V, path);
            }
        }
        ...
        if(_balance != 0) {
            path[0] = token1;
            path[1] = WBNB;
            TC += _getAmountsOut(router2, _balance, path);
        }
