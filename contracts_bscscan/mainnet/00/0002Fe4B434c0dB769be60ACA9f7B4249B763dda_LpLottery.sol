...
contract Lottery {
    ...
    function participate(uint256 _amount, address _particpant) external {
        ...
        require(!allParticipants[_particpant], "Already participated");
        ...
        participantList.push(_particpant);
...
contract LpLottery is PancakeClass {
    ...
    function getCashPrice(uint256 _lotteryId)
        public
        view
        returns (uint256)
    {
        return lotteryStructs[_lotteryId].getPrizeValue();
    }

    function getMinimumInvest(uint256 _lotteryId)
        public
        view
        returns (uint256)
    {
        return lotteryStructs[_lotteryId].getMinimumAmountInBusd();
    }
        ...
        returns (uint256)
    {
      ...
      function getAllParticiapntsByLottery(uint256 _lotteryId)
        external
        view
        returns (address[] memory)
    {
        return lotteryStructs[_lotteryId].getAll();
    }
