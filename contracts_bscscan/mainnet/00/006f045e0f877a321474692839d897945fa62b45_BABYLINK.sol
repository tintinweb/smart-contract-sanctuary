...
The only project which takes taxes from transactions and gives them back to community.
   ...
   Earn LINK rewards every 24 hours by holding!
   
   This is a manual operation done through the linkCharityWallet. 
...
contract BABYLINK is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0x1d9cd2CEF9F3a99b478F942d2d0c49Fe25f26477); // Marketing Address
    address payable public linkCharityWallet = payable(0xB1D9bDdBd9c6851d89b4cf8c130a67893587E878); // LINK Address
    ...
    constructor () {
        ...
        _totalTaxPercent = _burnFee.add(_liquidityFee).add(_marketingFee).add(_linkCharity);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _marketingFee.add(_linkCharity) > 0)
        {
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(forWallets);
            uint256 newBalance = address(this).balance.sub(initialBalance);
    
            uint256 marketingShare = newBalance.div(_marketingFee.add(_linkCharity
     )).mul(_marketingFee);
            uint256 linkWalletShare = newBalance.sub(marketingShare);
            
            if(marketingShare > 0)
                transferToAddressETH(marketingWalletAddress, marketingShare);
            
            if(linkWalletShare > 0)
                transferToAddressETH(linkCharityWallet, linkWalletShare);
        }
    ...
    function setTaxes(uint256 newBurnFee, uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newLinkCharityTax) external onlyOwner() {
        _burnFee = newBurnFee;
        _liquidityFee = newLiquidityTax;
        _marketingFee = newMarketingTax;
        _linkCharity = newLinkCharityTax;
        _totalTaxPercent = _burnFee.add(_liquidityFee).add(_marketingFee).add(_linkCharity
 );
        _prevTotalTaxPercent = _totalTaxPercent;
    }
    ...
    function setlinkCharityWallet(address newAddress) external onlyOwner() {
        linkCharityWallet = payable(newAddress);
    }
