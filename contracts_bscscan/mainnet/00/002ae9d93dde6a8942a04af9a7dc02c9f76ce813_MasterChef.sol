...
// sphynxToken with Governance.
contract sphynxToken is BEP20('Sphynx Token', 'SPHYNX') {
...
// MasterChef is the master of sphynx. He can make sphynx and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once sphynx is sufficiently
...
contract MasterChef is Ownable {
    ...
    function massUpdatePools() public {
     ...
     function updateEmissionRate(uint256 _valuePerBlock) public onlyOwner {
        massUpdatePools();
        sphynxPerBlock = _valuePerBlock;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 reward = multiplier.mul(sphynxPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        sphynx.mint(devaddr, reward.div(100));
        sphynx.mint(address(this), reward);
        pool.accsphynxPerShare = pool.accsphynxPerShare.add(reward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
