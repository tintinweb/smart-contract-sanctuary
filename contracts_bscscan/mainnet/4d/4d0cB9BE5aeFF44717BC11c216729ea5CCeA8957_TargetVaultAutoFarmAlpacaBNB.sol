...
import "../../../interfaces/targetVaults/protocols/IAlpacaIBToken.sol";
import "./TargetVaultAutoFarm.sol";

contract TargetVaultAutoFarmAlpacaBNB is TargetVaultAutoFarm {
    ...
    constructor(
        ...
        ibToken = IAlpacaIBToken(_ibToken);
        ...
        tokenToBuyBackPath = [wBNB, buyBackToken];
        autoTokenToTokenPath = [AUTOv2, wBNB];
    }
    ...
    function _deposit() internal override {
        ...
        if (_balance > 0) {
            uint256 _ibBefore = ibToken.balanceOf(address(this));
            // Deposit to Belt to get ibToken
            ibToken.deposit{value: _balance}(_balance);
            _balance = ibToken.balanceOf(address(this)).sub(_ibBefore);

            // Deposit ibToken to AutoFarm
            depositedBalance += _balance;
            ibToken.approve(address(autoFarm), _balance);
    ...
    function withdraw(uint256 _amount) external override onlyVault {
        if (_amount > 0) {
            ...
            uint256 _balance = ibToken.balanceOf(address(this));

            // Redeem ibToken back to Token
            uint256 _tokenBefore = address(this).balance;
            ibToken.withdraw(_balance);
            uint256 _tokenAmount = address(this).balance.sub(_tokenBefore);

            // Send token back to FeedVault
            payable(address(feedVault)).transfer(_tokenAmount);
    ...
    function _collectFees() internal override {
        ...
        if (_fees > 0) {
            if (autoBuyBack) {
                ...
                IPancakeRouter02(swapRouterAddress).swapExactETHForTokensSupportingFeeOnTransferTokens{value: _fees}(
    ...
    function _harvest() internal override {
        ...
        if (swapAmt > 0) {
            ...
            uint256 amountOut = amounts[amounts.length.sub(1)];
            IPancakeRouter02(swapRouterAddress).swapExactTokensForETHSupportingFeeOnTransferTokens(
                swapAmt,
                amountOut.mul(slippageFactor).div(1000),
    ...
    function balanceOf() public view override returns (uint256) {
        return ibToken.balanceOf(address(this)).add(vaultBalance());
    }

    /**
     * @dev Get target vault price per share
     */
    function targetPricePerShare() public view override returns (uint256) {
        return ibToken.totalToken().mul(1e18).div(ibToken.totalSupply());
    }
...
interface IAlpacaIBToken is IERC20 {
