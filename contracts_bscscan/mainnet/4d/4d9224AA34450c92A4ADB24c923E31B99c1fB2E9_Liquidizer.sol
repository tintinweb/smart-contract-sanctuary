...
contract Liquidizer  is Ownable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    
    
    IBLockmineRouter02 public router = IBLockmineRouter02(0xcF0feBd3f17CEf5b47b0cD257aCf6025c5BFf3b7); 
    IBEP20 public NUGGET = IBEP20(0xD86aA2066783AE96817065A984924f5C6536e227);
    ...
    function extractNugget() public onlyOwner {
        NUGGET.transfer(msg.sender, NUGGET.balanceOf(address(this)));
    }
    
    
    function liquidize(uint256 _balance) public onlyOwner {
        if (_balance > NUGGET.balanceOf(address(this)))
            _balance = NUGGET.balanceOf(address(this));
        
        // swap to busd/WBNB
        uint256 amountSwap = _balance.div(2);
        
        address[] memory path = new address[](2);
        path[0] = address(NUGGET);
        path[1] = address(WBNB);
        _swap(address(NUGGET), path, amountSwap);
        path[1] = address(BUSD);
        _swap(address(NUGGET), path, amountSwap);
