...
contract BananaFarm is BEP20('Banana Farm', 'Banana') {
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying Banana (not scaled);
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    event UpdateEmissionRate(address indexed user, uint256 BananaPerBlock);
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) external onlyOwner nonDuplicated(_lpToken) {
        ...
        require(_depositFeeBP <= 500, "add: invalid deposit, limit 5%");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function pendingBanana(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accBananaPerShare = accBananaPerShare.add(BananaReward.mul(1e12).div(pool.lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accBananaPerShare = pool.accBananaPerShare.add(BananaReward.mul(1e12).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function safeBananaTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > BananaBal) {
            transferSuccess = Banana.transfer(_to, BananaBal);
        } else {
            transferSuccess = Banana.transfer(_to, _amount);
        }
        require(transferSuccess, "safeBananaTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _BananaPerBlock) external onlyOwner {
        massUpdatePools();
        BananaPerBlock = _BananaPerBlock;
        emit UpdateEmissionRate(msg.sender, _BananaPerBlock);
    }
}