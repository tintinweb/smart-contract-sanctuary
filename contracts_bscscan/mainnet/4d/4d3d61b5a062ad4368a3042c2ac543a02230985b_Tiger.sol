...
contract Tiger is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0x619752F1edAF89E77ffaD14aa625f037A0581400); // Marketing Address
    address payable public teamWalletAddress = payable(0x1D716f1FbF877EaFC8B91601C5031844eea759bd); // Team Address
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountTigeream = amountReceived.mul(_teamShare).div(totalBNBFee);
        uint256 amountBNBMarketing = amountReceived.sub(amountBNBLiquidity).sub(amountTigeream);

        if(amountBNBMarketing > 0)
            transferToAddressETH(marketingWalletAddress, amountBNBMarketing);

        if(amountTigeream > 0)
            transferToAddressETH(teamWalletAddress, amountTigeream);
