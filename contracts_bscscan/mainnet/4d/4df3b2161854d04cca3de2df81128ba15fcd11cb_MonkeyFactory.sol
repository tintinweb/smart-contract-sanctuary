...
contract ChimpyMonkeys is ERC721, Ownable {
    using Counters for Counters.Counter;

    // Map the number of tokens per chimpyId
    mapping(uint8 => uint256) public bunnyCount;

    // Map the number of tokens burnt per chimpyId
    mapping(uint8 => uint256) public bunnyBurnCount;

    // Used for generating the tokenId of new NFT minted
    Counters.Counter private _tokenIds;

    // Map the chimpyId for each tokenId
    mapping(uint256 => uint8) private bunnyIds;

    // Map the chimpyName for a tokenId
    mapping(uint8 => string) private bunnyNames;

    constructor(string memory _baseURI) public ERC721("Chimpy Monkeys", "CM") {
        _setBaseURI(_baseURI);
    }

    /**
     * @dev Get chimpyId for a specific tokenId.
     */
    ...
    /**
     * @dev Get the associated chimpyName for a specific chimpyId.
     */
    ...
    /**
     * @dev Get the associated chimpyName for a unique tokenId.
     */
    ...
    /**
     * @dev Set a unique name for each chimpyid. It is supposed to be called once.
     */
...
/** @title MonkeyMintingStation.
@dev It is a contract that allow different factories to mint
Chimpy Collectibles/Monkeys.
*/

contract MonkeyMintingStation is AccessControl {
    ChimpyMonkeys public pancakeBunnies;
    ...
    constructor(ChimpyMonkeys _pancakeBunnies) public {
        pancakeBunnies = _pancakeBunnies;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev Mint NFTs from the ChimpyMonkeys contract.
     */
...
contract MonkeyFactory is Ownable {
    ...
    MonkeyMintingStation public bunnyMintingStation;
    ...
    // Number of CHIMPYs a user needs to pay to acquire a token
    ...
    constructor(
        MonkeyMintingStation _bunnyMintingStation,
        IBEP20 _cakeToken,
        uint256 _tokenPrice,
    ...
    /**
     * @dev Mint NFTs from the MonkeyMintingStation contract.
     * Users can specify what monkeyId they want to mint. Users can claim once.
     */
    function mintNFT(uint8 _bunnyId) external {
        ...
        // Check that the _monkeyid is within boundary:
        ...
        // Send CHIMPY tokens to this contract
    ...
    /**
     * @dev It transfers the CHIMPY tokens back to the chef address.
    ...
    /**
     * @dev Set up json extensions for monkeys 5-9
