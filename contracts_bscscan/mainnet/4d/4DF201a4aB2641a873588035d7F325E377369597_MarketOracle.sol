...
/**
 * @title GFC price Oracle
 *      This Oracle calculates the average USD price of GFC based on the BNB-GFC and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private gfcBnbPrice0CumulativeLast;
    uint private gfcBnbPrice1CumulativeLast;
    uint32 private gfcBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _gfc_bnb;
    ...
    constructor(
        address __gfc_bnb,   // Address of the gfc-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _gfc_bnb = IUniswapV2Pair(__gfc_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        gfcBnbPrice0CumulativeLast = _gfc_bnb.price0CumulativeLast();
        gfcBnbPrice1CumulativeLast = _gfc_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, gfcBnbBlockTimestampLast) = _gfc_bnb.getReserves();
    ...
    // Get the average price of 1 gfc in the smallest BNB unit (18 decimals)
    function getgfcBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_gfc_bnb));

        require(_blockTimestamp != gfcBnbBlockTimestampLast, "GFC Last and current are equal");

        FixedPoint.uq112x112 memory gfcBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price1Cumulative - gfcBnbPrice1CumulativeLast) / (_blockTimestamp - gfcBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, gfcBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        (gfcBnbPrice0CumulativeLast, gfcBnbPrice1CumulativeLast, gfcBnbBlockTimestampLast, gfcBnbAverage) = getgfcBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, gfcBnbAverage) = getgfcBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = gfcBnbAverage*(1e6) / busdBnbAverage;
