...
contract LightLemonUnicorn is IBEP20 {
    ...
    int256[] public buyTaxDTIs = [int256(-1000000), 4000000, 8000000, 12000000, 16000000, 24000000, 28000000, 32000000, 36000000, 40000000, 44000000, 48000000, 52000000, 56000000, 60000000];
    uint256[] public buyTaxValues = [0, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500];
 
    int256[] public sellTaxDTIs = [int256(-3000000), -2500000, -2000000, -1500000, -1000000, -500000, 1, 1000000, 3000000, 5000000, 7000000, 9000000, 12000000, 24000000, 36000000];
    uint256[] public sellTaxValues = [9000, 8000, 7000, 6000, 5500, 5000, 4800, 4000, 3500, 3000, 2500, 2000, 1200, 900, 400, 0];
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        DTITx memory dtiTx = DTITx(int256(amount), txType);
    ...
    function takeFee(address sender, DTITx memory dtiTx) private returns (uint256) {
        ...
        uint256 feeAmount = uint256(dtiTx.amount) * discountedFee / FEE_DENOMINATOR;

        _balances[address(this)] += feeAmount;
        emit Transfer(sender, address(this), feeAmount);

        return uint256(dtiTx.amount) - feeAmount;
    }
    ...
    function getBuyDTIValue() public view returns (int256) {
        ...
        for (uint256 i; i < dtiTransactions.length; i++) {
            ...
            if (dtiTx.txType == TxType.BUY) {
                dtiValue += dtiTx.amount / int256(10**DECIMALS);
    ...
    function secretDiscount(address account) private view returns (uint256) {
        ...
        try secret.balanceOf(account) returns (uint256 balance) {
            if (balance >= 5 * 10**6 * 10**8) { return 300; }
        } catch {
            return 0;
        }
    ...
    function buyback(uint256 amount) private swapping {
        address[] memory path = new address[](3);
        path[0] = ROUTER.WETH();
        path[1] = BUSD;
        path[2] = address(this);
    ...
    function setBuyTaxes(int256[] calldata thresholds, uint256[] calldata values) external onlyOwner {
        ...
        require(values.length == thresholds.length - 1);
