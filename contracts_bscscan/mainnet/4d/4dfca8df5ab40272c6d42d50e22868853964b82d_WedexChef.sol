...
contract WedexChef is Ownable, ReentrancyGuard {
    ...
    function payDirectCommission(uint256 _pid,uint256 _amount, bool vipBranch, uint256 leaderCommission, address referrer) internal {
        ...
        if(vipBranch){
            wedex.mint(
                address(referrer),
                _amount.mul(lpPrice).mul(uint256(10).sub(leaderCommission)).div(1e4)
            );
            wedexReferral.recordReferralCommission(referrer,_amount.mul(lpPrice).mul(uint256(10).sub(leaderCommission)).div(1e4));
            if(getLeader(msg.sender) != address(0)){
                wedex.mint(
                    getLeader(msg.sender),
                    _amount.mul(lpPrice).mul(leaderCommission).div(1e2).div(1e2)
                );
            ...
            wedex.mint(
                address(referrer),
                _amount.mul(lpPrice).mul(7).div(1e2).div(1e2)
            );
            wedexReferral.recordReferralCommission(referrer,_amount.mul(lpPrice).mul(7).div(1e2).div(1e2));
            address upperVip = getUpperVip(msg.sender);
            if(upperVip!=address(0)){
                wedex.mint(
                    upperVip,
                    _amount.mul(lpPrice).mul(3).div(1e4)
                );
                wedexReferral.recordReferralCommission(upperVip,_amount.mul(lpPrice).mul(3).div(1e4));
            }
        }
    }
