...
contract Olevel20 is Context, IERC20, Ownable {
    ...
    string private _name = "Olevel20";
    string private _symbol = "20LEVEL";
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (from != uniswapV2Pair && cooldownEnabled) { // sell & transfers
                ...
                else if (sellnumber[from] <= 3 && amount <= balanceOf(uniswapV2Pair).div(10000).mul(_josh)) { 
                    require (amount <= balanceOf(uniswapV2Pair).div(10000).mul(_josh));
                    sellnumber[from]++;
                    sellcooldown[from] = block.timestamp + _initialdelay;
                    _liquidityFee = 11;
                ...
                else if (sellnumber[from] == 4 && amount <= balanceOf(uniswapV2Pair).div(10000).mul(_josh)) {
                    require (amount <= balanceOf(uniswapV2Pair).div(10000).mul(_josh));
                    sellnumber[from]++;
