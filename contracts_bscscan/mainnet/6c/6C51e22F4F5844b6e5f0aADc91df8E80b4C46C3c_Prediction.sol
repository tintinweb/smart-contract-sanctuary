...
contract Prediction is Ownable, Pausable, ReentrancyGuard {
    ...
    address public usdtAddress; // address of usdt token
    ...
    function claim(uint256[] calldata epochs) external nonReentrant notContract {
        ...
        if (reward > 0) {
            IERC20(usdtAddress).transfer(address(msg.sender), reward);
        }
    }
    ...
    function claimTreasury() external nonReentrant onlyAdmin {
        ...
        treasuryAmount = 0;
        IERC20(usdtAddress).transfer(adminAddress, currentTreasuryAmount);
