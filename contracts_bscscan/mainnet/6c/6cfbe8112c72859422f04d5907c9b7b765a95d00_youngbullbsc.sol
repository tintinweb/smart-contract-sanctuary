...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWRD = IBEP20(0x3fd5B5746315E3F8d43A46b09c826a001EBb977d);
...
contract youngbullbsc is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Young Bull NFT";
    string constant _symbol = "$YBULL";
    ...
    address public marketingFeeReceiver = 0x02C077A6238Cd503e61D959FA2cC29b26fD4B817;
    ...
    uint256 public cooldownTimerIntervalBuy = 5;
    uint256 public cooldownTimerIntervalSell = 60 * 60 * 12;

    mapping (address => uint) private cooldownTimer;
    mapping (address => uint) private cooldownTimerSell;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender == pair &&
            ...
            require(cooldownTimer[recipient] < block.timestamp,"Buy cooldown error");
            cooldownTimer[recipient] = block.timestamp + cooldownTimerIntervalBuy;
        }

        if (recipient == pair &&
            sellCooldownEnabled &&
            !isTimelockExempt[sender]) {
            require(cooldownTimerSell[sender] < block.timestamp,"Sell cooldown error");
            cooldownTimerSell[sender] = block.timestamp + cooldownTimerIntervalSell;
        }
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        ...
        require(amount <= (sellRatio * _balances[sender] / 100 )  || isTxLimitExempt[sender], "Max sell amoutn reached");
    }
    ...
    function set_sell_ratio(uint256 _Ratio) external onlyOwner{
        sellRatio = _Ratio;
    }
    ...
    function cooldownEnabled(bool _statusBuy, uint256 _intervalBuy,bool _statusSell, uint256 _intervalSell) public onlyOwner {
        buyCooldownEnabled = _statusBuy;
        cooldownTimerIntervalBuy = _intervalBuy;
        
        sellCooldownEnabled = _statusSell;
        cooldownTimerIntervalSell = _intervalSell;
    }
    ...
    function migrate() external onlyOwner{
        if(balanceOf(pair) > (_totalSupply * 50 / 100 )){
