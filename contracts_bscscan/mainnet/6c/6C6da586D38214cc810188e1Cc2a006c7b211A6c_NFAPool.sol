...
contract NFAPool is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accRewardPerShare;  // Accumulated NUGGETs per share, times 1e12. See below.
    ...
    constructor(
        ...
        rewardPerBlock = 5 * 10 ** 16;
        startBlock = _startBlock;
        bonusEndBlock = _startBlock.add(14*28800); 
        ...
        // vip manager + vault
        vipManager = _manager;
        stakeVault = new Vault(_stakeToken);
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accRewardPerShare = accRewardPerShare.add(cakeReward.mul(1e24).div(lpSupply));
        }
    ...
    function blocksToUnlock(uint256 _pid, address _user) public view returns (uint256) {
        // lock feature not implemented, but needed on frontend
        return 0;
    }
    ...
    function updatePool() public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cakeReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accRewardPerShare = pool.accRewardPerShare.add(cakeReward.mul(1e24).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        require(vipManager.hasVIPStatus(msg.sender), "deposit: you need vip status");
        ...
        if(_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(stakeVault), _amount);
    ...
    function withdraw(uint256 _amount) public {
        ...
        require(vipManager.hasVIPStatus(msg.sender), "withdraw: you need vip status");
        require(user.read_lock == false, "withraw: Dammit, there was a read lock on your account due to a previous process.");
        user.read_lock = true;
        updatePool();
        uint256 pending = user.amount.mul(pool.accRewardPerShare).div(1e24).sub(user.rewardDebt);
        ...
        if(_amount > 0) {
            ...
            stakeVault.safeTokenTransfer(address(msg.sender), _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accRewardPerShare).div(1e24);
        user.read_lock = false;
    ...
    function emergencyWithdraw() public {
        // lost your NFA? :(
        // Sold your NFA? :O
        // Well, fortunately, you can still emergencyWithdraw your staked funds, but won't get the rewards
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[msg.sender];
...
// to not have stakes and rewards in the same contract
