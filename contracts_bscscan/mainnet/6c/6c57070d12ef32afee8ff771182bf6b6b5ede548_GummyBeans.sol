...
library SafeMath {
    ...
    /**
     * @dev Returns the integer division of two unsigned integers. Reverts with custom messa-ge on
     * division by zero. The result is rounded towards zero.
     *
     * Counterpart to Solidity's `/` operator. Note: this function uses a
     * `revert` opcode (which leaves remaining gas untouched) while Solidity
    ...
    /**
     * @dev Returns the remainder of dividing two unsigned integers. (unsigned integer modu-lo),
...
library Address {
    ...
    function functionCallWithValue(address target, bytes memory data, uint256 value) internal returns (bytes memory) {
        return functionCallWithValue(target, data, value, "Address: low-level call with value fai-led");
    }
...
contract GummyBeans is Context, IERC20, Ownable, Pausable {
  ...
  string private _symbol = "GUMMIE";
  string private _name = "Gummy Beans";
