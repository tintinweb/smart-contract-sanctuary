...
contract PeanutERC20 is IPeanutERC20 {
    using SafeMath for uint256;

    string public constant name = "PEANUT LP Token";
    string public constant symbol = "PEANUT LP";
    ...
    function permit(
        ...
        require(deadline >= block.timestamp, "PeanutSwap: EXPIRED");
        ...
        require(
            ...
            "PeanutSwap: INVALID_SIGNATURE"
        );
        _approve(owner, spender, value);
    }
}

contract PeanutPair is IPeanutPair, PeanutERC20 {
    using SafeMath for uint256;
    ...
    modifier lock() {
        require(unlocked == 1, "PeanutSwap: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, "PeanutSwap: FORBIDDEN");
        // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1)
        ...
        returns (bool feeOn)
    {
        address feeTo = IPeanutFactory(factory).feeTo();
        feeOn =
            feeTo != address(0) &&
            IPeanutFactory(factory).getPairRate(address(this)) != 9;
            ...
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    uint256 denominator = rootK
                        .mul(IPeanutFactory(factory).getPairRate(address(this)))
                        .add(rootKLast); //分母
                    // 流动性=分子/分母；
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        require(liquidity > 0, "PeanutSwap: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "PeanutSwap: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function swap(
        ...
        require(
            ...
            "PeanutSwap: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves();
        // gas savings
        require(
            amount0Out < _reserve0 && amount1Out < _reserve1,
            "PeanutSwap: INSUFFICIENT_LIQUIDITY"
        );
        ...
        uint256 balance1;
        {
            ...
            address _token0 = token0;
            address _token1 = token1;
            require(to != _token0 && to != _token1, "PeanutSwap: INVALID_TO");
        ...
        require(
            ...
            "PeanutSwap: INSUFFICIENT_INPUT_AMOUNT"
        );
        {
            // scope for reserve{0,1}Adjusted, avoids stack too deep errors
            uint256 balance0Adjusted = balance0.mul(1e4).sub(
                amount0In.mul(
                    IPeanutFactory(factory).getPairFees(address(this))
                )
            );
            uint256 balance1Adjusted = balance1.mul(1e4).sub(
                amount1In.mul(
                    IPeanutFactory(factory).getPairFees(address(this))
                )
            );
            require(
                balance0Adjusted.mul(balance1Adjusted) >=
                    uint256(_reserve0).mul(_reserve1).mul(1e8),
                "PeanutSwap: K"
            );
        }
...
contract PeanutFactory is IPeanutFactory {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet private _supportList;

    uint256 public constant FEE_RATE_DENOMINATOR = 1e4;
    uint256 public feeRateNumerator = 300; //分子 ,150-basis-point fees earned by，基点费,1个基点等于1%的百分之一 0.3%
    address public feeTo;
    address public feeToSetter;
    uint256 public feeToRate = 30; //3个基点，万分之3
    ...
    constructor(address _feeToSetter) public {
        ...
        initCodeHash = keccak256(
            abi.encodePacked(type(PeanutPair).creationCode)
        );
    }
    ...
    function createPair(address tokenA, address tokenB)
        ...
        returns (address pair)
    {
        require(tokenA != tokenB, "PeanutSwapFactory: IDENTICAL_ADDRESSES");
        (address token0, address token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        require(token0 != address(0), "PeanutSwapFactory: ZERO_ADDRESS");
        require(
            getPair[token0][token1] == address(0),
            "PeanutSwapFactory: PAIR_EXISTS"
        );
        // single check is sufficient
        bytes memory bytecode = type(PeanutPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IPeanutPair(pair).initialize(token0, token1);
    ...
    function setFeeToSetter(address _feeToSetter) external {
        ...
        require(
            ...
            "PeanutSwapFactory: FeeToSetter is zero address"
        );
        feeToSetter = _feeToSetter;
    }
    ...
    function addPair(address pair) external returns (bool) {
        ...
        require(
            pair != address(0),
            "PeanutSwapFactory: pair is the zero address"
        );
    ...
    // Set pair fee , max is 5%
    function setPairFees(address pair, uint256 fee) external {
        require(msg.sender == feeToSetter, "PeanutSwapFactory: FORBIDDEN");
        require(fee <= 500, "PeanutSwapFactory: EXCEEDS_FEE_RATE_DENOMINATOR");
        pairFees[pair] = fee;
    }
    ...
    function setDefaultFeeToRate(uint256 rate) external {
        ...
        require(
            rate > 0 && rate <= 10,
            "PeanutSwapFactory: FEE_TO_RATE_OVERFLOW"
        );
        ...
        returns (uint256 reserveA, uint256 reserveB)
    {
        ...
        (uint256 reserve0, uint256 reserve1, ) = IPeanutPair(
    ...
    function quote(
        ...
        require(amountA > 0, "PeanutSwapFactory: INSUFFICIENT_AMOUNT");
        require(
            reserveA > 0 && reserveB > 0,
            "PeanutSwapFactory: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "PeanutSwapFactory: INSUFFICIENT_INPUT_AMOUNT");
        require(
            reserveIn > 0 && reserveOut > 0,
            "PeanutSwapFactory: INSUFFICIENT_LIQUIDITY"
        );
        uint256 fee = getPairFees(pairFor(token0, token1));
        uint256 amountInWithFee = amountIn.mul(FEE_RATE_DENOMINATOR.sub(fee));
        uint256 numerator = amountInWithFee.mul(reserveOut);
        uint256 denominator = reserveIn.mul(FEE_RATE_DENOMINATOR).add(
            amountInWithFee
        );
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, "PeanutSwapFactory: INSUFFICIENT_OUTPUT_AMOUNT");
        require(
            reserveIn > 0 && reserveOut > 0,
            "PeanutSwapFactory: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountsOut(uint256 amountIn, address[] memory path)
        ...
        returns (uint256[] memory amounts)
    {
        require(path.length >= 2, "PeanutSwapFactory: INVALID_PATH");
