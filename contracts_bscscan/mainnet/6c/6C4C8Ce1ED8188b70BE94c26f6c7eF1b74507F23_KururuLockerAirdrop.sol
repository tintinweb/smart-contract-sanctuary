...
import "./KururuLocker.sol";

contract KururuLockerAirdrop is Ownable {
	event Locked(uint256 indexed id, address indexed sender);
	event Unlocked(uint256 indexed id, address indexed sender);
	event SendAirdrop(address indexed to, uint256 airdropAmount);

	using SafeERC20 for IERC20;

	IERC20 public immutable kuru;
	IERC20 public immutable stakeToken;
	IERC20 public immutable airdrop;
	...
	mapping (address => uint256) lastLocks;
	...
	constructor(
		...
		limitDuration = limitDuration_;
		maxPerTx = maxPerTx_;
	}

	function setLockDuration(uint256 duration) external onlyOwner {
		require(duration <= 400 days, "too long");
		lockDuration = duration;
	}

	function setLimitDuration(uint256 duration) external onlyOwner {
	...
	function receiveAirdrop(uint256 amount) public view returns (uint256) {
		return (amount * rate) / 1 ether;
	}

	function sendAirdrop(uint256 amount) internal {
		uint256 r = receiveAirdrop(amount);
		require(r > 0, "empty airdrop");
		require(airdrop.balanceOf(address(this)) >= r, "airdrop not available");

		airdrop.transfer(msg.sender, r);
		emit SendAirdrop(msg.sender, r);
	}
	...
	function lock(uint256 amount) external returns (uint256) {
		...
		require(amount <= maxPerTx, "exceed limit per tx");
		require(lastLocks[msg.sender] + lockDuration < block.timestamp, "exceed limit duration");

		lastLocks[msg.sender] = block.timestamp;

		_transferFee();

		uint256 x = stakeToken.balanceOf(address(this));
		stakeToken.safeTransferFrom(msg.sender, address(this), amount);
		uint256 y = stakeToken.balanceOf(address(this));
	...
	function unlock(uint256 id) external {
		...
		it.unlocked = true;
		stakeToken.safeTransfer(it.owner, it.amount);
		emit Unlocked(id, msg.sender);
	}

	function withdrawAirdrop() external onlyOwner {
		airdrop.safeTransfer(msg.sender, airdrop.balanceOf(address(this)));
	}
}
