...
contract Diplomat {
    ...
    uint256[] public GI_PERCENT = [200,100,50,50, 10,10,10,10,10,10,10,10,10,10,10];
    uint256[] public level_condition = [2,2,2,5,5,5,5,5,5,5,8,8,8,8,8];
    ...
    uint256 public dmc_rate = 70; // 0.07$  X->1000
    ...
    constructor(IBEP20 tokenAdd) public {
        ...
        //days , total return percentage//min invest/max inve
        tarifs.push(Tarif(400, 200,100,500));  //0.5% for 400 days  = 200%
        tarifs.push(Tarif(333, 200,501,2500)); //0.6% for 333 days  = 200%
        tarifs.push(Tarif(285, 200,2501,10000)); //0.7% for 285 days  = 200%
        tarifs.push(Tarif(250, 200, 10001,50000)); //0.8% for 250 days  = 200%
        tarifs.push(Tarif(200, 200, 50001,1000000000)); //1% for 200 days  = 200%
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        ...
        for(uint8 i = 0; i < ref_bonuses.length; i++) {
         ...
         if(players[up].structure[0] <  level_condition[i] ) continue;
    ...
    function deposit(uint8 _tarif, address _upline, uint256 token_quantity) external payable {
       ...
       uint256 token_value_in_USD = token_quantity.mul(dmc_rate).div(1000);
       uint256 value_to_check = token_value_in_USD.div(1 * 10 ** 12);
        ...
        player.available = player.available.add(token_value_in_USD.mul(5));
    ...
    function withdraw() payable external {
        ...
        uint256 amount = player.dividends + player.match_bonus + player.gi_bonus;
          require(amount > 10 * 10 ** 12,'Less then the min withdrawl');
        ...
        // uint256 acl_rate = aclicx_rate.div(10);
           amount = amount.mul(1000).div(dmc_rate);
