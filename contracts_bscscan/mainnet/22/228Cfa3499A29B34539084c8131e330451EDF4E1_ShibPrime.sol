...
*/
abstract contract ERC20Deflationary is Context, IERC20, Ownable {

    uint8 private _taxTeam = 6;
    address private _teamAddress = 0xAd3F1eE218eaDB490C5EA544353206C3a1F0F5aF;
    uint8 private _taxTeamDecimals = 0;
    bool private _teamTaxEnabled = true;

    function setTeamAddress(address teamAddress_) public onlyOwner {
        require(_teamAddress != teamAddress_, "New team address must be different than previous.");

        address previous = _teamAddress;
        _teamAddress = teamAddress_;

        emit teamAddressUpdated(previous, teamAddress_);
    }
    ...
    */
    ...
    event TaxTeamUpdate(uint8 previousTax, uint8 previousDecimals, uint8 currentTax, uint8 currentDecimal);
    event MinTokensBeforeSwapUpdated(uint256 previous, uint256 current);
    event teamAddressUpdated(address previous, address current);
    ...
    constructor (string memory name_, string memory symbol_, uint8 decimals_, uint256 tokenSupply_) {
        ...
        // Mint
        _reflectionBalances[_teamAddress] = _reflectionTotal;

        // exclude owner and this contract from fee.
        excludeAccountFromFee(owner());
        excludeAccountFromFee(_teamAddress);
        ...
        emit Transfer(address(0), _teamAddress, _totalSupply);
    }
    ...
    function _afterTokenTransfer(ValuesFromAmount memory values) internal virtual {
        ...
        if (_teamTaxEnabled) {
            _reflectionBalances[_teamAddress] += values.rTeamFee;
        }
    ...
    function _getTValues(ValuesFromAmount memory values, bool deductTransferFee) view private {
        ...
        if (deductTransferFee) {
            ...
            values.tTeamFee = _calculateTax(values.amount, _taxTeam, _taxTeamDecimals);
            
            // amount after fee
            values.tTransferAmount = values.amount - values.tBurnFee - values.tRewardFee - values.tLiquifyFee - values.tTeamFee;
        }
    ...
    function _getRValues(ValuesFromAmount memory values, bool deductTransferFee) view private {
        ...
        if (deductTransferFee) {
            ...
            values.rTeamFee = values.tTeamFee * currentRate;
            values.rTransferAmount = values.rAmount - values.rBurnFee - values.rRewardFee - values.rLiquifyFee - values.rTeamFee;
        }
    ...
    */
    ...
    function setTaxBurn(uint8 taxBurn_, uint8 taxBurnDecimals_) public onlyOwner {
        ...
        require(taxBurn_ + _taxReward + _taxLiquify + _taxTeam < 100, "Tax fee too high.");
    ...
    function setTaxReward(uint8 taxReward_, uint8 taxRewardDecimals_) public onlyOwner {
        ...
        require(_taxBurn + taxReward_ + _taxLiquify + _taxTeam < 100, "Tax fee too high.");
    ...
    function setTaxLiquify(uint8 taxLiquify_, uint8 taxLiquifyDecimals_) public onlyOwner {
        ...
        require(_taxBurn + _taxReward + taxLiquify_ + _taxTeam < 100, "Tax fee too high.");
    ...
    function setTaxTeam(uint8 taxTeam_, uint8 taxTeamDecimals_) public onlyOwner {
        require(_teamTaxEnabled, "Team tax must be enabled.");
        require(_taxBurn + _taxReward + _taxLiquify + taxTeam_< 100, "Tax fee too high.");

        uint8 previousTax = _taxTeam;
        uint8 previousDecimals = _taxTeamDecimals;
        _taxTeam = taxTeam_;
        _taxTeamDecimals = taxTeamDecimals_;

        emit TaxTeamUpdate(previousTax, previousDecimals, taxTeam_, taxTeamDecimals_);
    }

}

contract ShibPrime is ERC20Deflationary {

    uint256 private _tokenSupply = 10000000000000; // 10 trillion

    uint8 private _taxBurn = 2;
    uint8 private _taxReward = 2;
    uint8 private _taxDecimals = 0;
    uint8 private _decimals = 18;
    uint256 private _minTokensBeforeSwap = 1000 * (10 ** _decimals);

    address private _routerAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

    constructor () ERC20Deflationary("SHIB PRIME", "PSHIB", _decimals, _tokenSupply) {
