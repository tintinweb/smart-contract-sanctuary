...
contract MausoleumV2 is Ownable {
    ...
    constructor(IBEP20 _zombie, address payable _treasury, address _lpStorage, IUniswapV2Router02 _apeswapRouter, IPriceConsumerV3 _priceConsumer) {
        zombie = _zombie;
        treasury = _treasury;
    ...
    function addAuction(uint256 _startingBid, uint256 _endDate, IPancakePair _bidToken, IRevivedRugNft _prize, uint256 _unlockFee) public onlyOwner {
        ...
        require(_prize.owner() == address(this), 'Prize: masoleum must be owner of auction prize.');
        require(bytes(_prize.baseURI()).length > 0, 'Prize: TokenURI is not set.');
    ...
    function finalizeAuction(uint _aid) external auctionExists(_aid) auctionEnded(_aid) {
        ...
        if(bidsLength(_aid) > 1) {
            ...
            // allow apeswap router
            auction.bidToken.approve(address(apeswapRouter), remaining);
        ...
        auction.prize.reviveRug(finalBid.bidder);
        auction.prize.renounceOwnership();
    ...
    function swapZombieForBnb(uint256 bnbAmount) private {
        ...
        path[0] = apeswapRouter.WETH();
        path[1] = address(zombie);

        // make the swap
        apeswapRouter.swapExactETHForTokens{value: bnbAmount} (
            0,
            path,
            address(this),
            block.timestamp
...

{
  ...
  "libraries": {
    "/Users/Shared/rug-zombie-contracts/contracts/libraries/Percentages.sol": {
      "Percentages": "0xcb22e1A2C00303EB855c5A5Aa0D6F3Ac62405370"
    }
  },
  "outputSelection": {
    "*": {
