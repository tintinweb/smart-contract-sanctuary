...
contract MultiPadLaunchApp is IBEP20 {
    ...
    */
    ...
    function claimPurchasedTokens(uint256 _lockNumber) external validateClaim(msg.sender,_lockNumber) returns (bool){
        uint256 calculation;
        if(_lockNumber == 1){
             calculation = (_finalSoldAmount[msg.sender]).div(4);
        }
        else{
             calculation = (_finalSoldAmount[msg.sender].mul(1875)).div(10000);
        }
    ...
    modifier validateClaim(address _userAddress, uint256 _lockNumber)
    {
        ...
        if(_lockNumber == 2){    // 1 month
            require(block.timestamp > saleEndTime + 400  && reEntrance[_userAddress][_lockNumber] != true , "Insufficient Unlocked Tokens");
        }
        if(_lockNumber == 3){    // 1 month
            require(block.timestamp > saleEndTime  + 500 && reEntrance[_userAddress][_lockNumber] != true ,  "Insufficient Unlocked Tokens");
        }
        ...
        if(_lockNumber == 5){    // 1 month
            require(block.timestamp > saleEndTime + 700 &&  reEntrance[_userAddress][_lockNumber] != true,  "Insufficient Unlocked Tokens");
        }
        _;
    }
    ...
    function getUserTokensByAdd(address _userAddress) external view returns(uint256 _div1, uint256 _div2, uint256 _div3, uint256 _div4, uint256 _div5){
        ...
        uint256 otherDivision = (_finalSoldAmount[_userAddress]).sub(division).div(4);
        _div1 = division;
        _div2 = otherDivision;
        _div3 = otherDivision;
        _div4 = otherDivision;
    ...
    function getClaimDates() view external returns(uint256 _d1, uint256 _d2, uint256 _d3, uint256 _d4, uint256 _d5){
        _d1 = saleEndTime + 300;   // After 1.5 hours of end sale
        _d2 = saleEndTime + 400;
        _d3 = saleEndTime + 500;
        _d4 = saleEndTime + 600;
        _d5 = saleEndTime + 700;
    ...
    function getClaimedTokensHistory(address _userAddress) view external returns(uint256 r1,uint256 r2, uint256 r3, uint256 r4, uint256 r5){
        ...
        r5 = _claimedByUser[_userAddress][5];
