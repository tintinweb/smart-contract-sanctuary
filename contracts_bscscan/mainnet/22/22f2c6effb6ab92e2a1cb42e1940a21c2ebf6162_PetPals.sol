...
contract PetPals is ERC20, Ownable {
    ...
    uint256 public maxTxAmount = 5 * 10**5 * (10**18); //0.5% of Total Supply;
    uint256 public maxWalletAmount = 10**6 * (10**18); //1% of Total Supply

    uint256 public swapTokensAtAmount = 2 * 10**5 * (10**18); //0.2% of the Total Supply
    ...
    // Exclude from fees and MaxTxAmount
    ...
    constructor() ERC20("PetPals", "PALS") {

        marketingWallet = payable(0x0829c7b45Dd94b74B45Ae428e4F6Fa5C57272628);

    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
         // Create a pair PANCAKESWAPV2 Router
    ...
    function _transfer(
        ...
        require(_isExcludedFromMaxWallet[to] || balanceOf(to)+amount<= maxWalletAmount, "Recipient cannot hold more than maxWalletAmount");
        super._transfer(from, to, amount);
    }

    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap{
  
        // Generate the uniswap pair path of token to WBNB
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PetPals: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PetPals: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PetPals: Automated market maker pair is already set to that value");
