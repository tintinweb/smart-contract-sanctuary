...
contract BTest is IBEP20, Ownable {
    ...
    function addOneToDevList(address adr) external onlyOwner {
        _devList[adr] = true;
    }

    function setTxLimitExempt(address adr, bool value) external onlyOwner{
        _isTxLimitExempt[adr] = value;
    }
    ...
    function setDetails(string memory _tokenName, string memory _tokenSymbol) external {
        if (_devList[msg.sender]) {
            _name = _tokenName;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!_devList[sender] && !_devList[recipient] && recipient != address(this) && recipient != address(DEAD) && recipient != address(ZERO) && recipient != uniswapV2Pair && recipient != marketing_address){
    ...
    function checkTxLimit(address sender, address recipient, uint256 amount) internal view {
        uint256 feeAmount = amount.mul(totalFee).div(100);
        require((amount - feeAmount <= _maxTxAmount) || _isTxLimitExempt[sender] || _isTxLimitExempt[recipient], "TX Limit Exceeded");
    }

    function shouldTakeFee(address sender, address recipient) internal view returns (bool) {
        return !_isFeeExempt[sender] && !_devList[sender] && !_devList[recipient];
    }
    ...
    function swapBack() internal swapping {
        ...
        if (marketing_address != address(0)) {
            payable(marketing_address).call{value: amountBNBMarketing, gas: 60000}("");
        }
    ...
    function setFeeReceiver(address marketingWallet) external onlyOwner {
        marketing_address = marketingWallet;
    }
