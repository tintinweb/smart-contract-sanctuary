...
// File: original_contracts/lib/biswap/BiswapRouter.sol
...
contract BiswapRouter {
    ...
    constructor(address _factory, address _WETH, bytes32 _initCode, uint256 _feeFactor) public {
        factory = _factory;
        WETH = _WETH;
        initCode = _initCode;
    ...
    returns (uint256 tokensBought)
    {
        ...
        for(uint8 i = 0; i < pairs; i++) {
            ...
            tokensBought = UniswapV3LibFork.getAmountOutByPair(
                ...
                tokenSold,
                tokenBought,
                feeFactor - IBiswapPair(currentPair).swapFee(), // should we also deduct devFee? @TODO - check contract
                feeFactor
            );
    ...
    returns (uint256 tokensSold)
    {
        ...
        for (uint8 i = length - 1; i > 0; i--) {
            address tokenFrom = path[i-1];
            address tokenTo = path[1];
            ...
            (amounts[i - 1], pairs[i - 1]) = UniswapV3LibFork.getAmountInAndPair(
                ...
                tokenTo,
                initCode,
                feeFactor - IBiswapPair(pair).swapFee(), // should we also deduct devFee? @TODO - check contract
                feeFactor
            );
        }
