...
// File contracts/BIFI/interfaces/common/IMasterChefWault.sol



pragma solidity ^0.6.0;

interface IMasterChefWault {
...
// File contracts/BIFI/strategies/Common/StrategyCommonWexLP.sol



pragma solidity ^0.6.0;
contract StrategyCommonWexLP is StratManager, FeeManager, GasThrottler {
    ...
    constructor(
        ...
        if (lpToken0 == wbnb) {
            wexToLp0Route = [wex, wbnb];
        } else if (lpToken0 != wex) {
            wexToLp0Route = [wex, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            wexToLp1Route = [wex, wbnb];
        } else if (lpToken1 != wex) {
            wexToLp1Route = [wex, wbnb, lpToken1];
        }
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IMasterChefWault(masterchef).deposit(poolId, wantBal, true);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IMasterChefWault(masterchef).withdraw(poolId, _amount.sub(wantBal), true);
    ...
    function _harvest() internal {
        ...
        IMasterChefWault(masterchef).deposit(poolId, 0, true);
        chargeFees();
        addLiquidity();
        deposit();
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(wex).balanceOf(address(this)).mul(45).div(1000);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toWbnb, 0, wexToWbnbRoute, address(this), now);
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != wex) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(wexHalf, 0, wexToLp0Route, address(this), now);
        }

        if (lpToken1 != wex) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(wexHalf, 0, wexToLp1Route, address(this), now);
        }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IMasterChefWault(masterchef).userInfo(poolId, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        ...
        IMasterChefWault(masterchef).emergencyWithdraw(poolId);
    ...
    function _giveAllowances() internal {
        ...
        IERC20(wex).safeApprove(unirouter, uint256(-1));
