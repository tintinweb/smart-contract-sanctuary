...
contract swapContract is AccessControl, Pausable, ECDSAOffsetRecovery{
    ...
    )
    {
        ...
        require(IERC20(RubicAddresses[_numOfThisBlockchain]).approve(address(blockchainRouter), type(uint256).max),
    ...
    function _processTransaction(address user, uint256 amountWithFee, bytes32 originalTxHash, bytes memory concatSignatures) private{
        ...
        (uint8 statusCode, bytes32 savedHash) = isProcessedTransaction(originalTxHash);
        require(statusCode == 0 && savedHash != hashedParams, "swapContract: Transaction already processed");
    ...
    /**
      ...
      * equal to the amount of tokens initially spent by user (we are refunding them), though AmountWithFee should
      * be the same as if you would have called another 'swapToUser' function because this value is stored in
        ...
        )
    {
        ...
        uint256 amountIn = blockchainRouter.getAmountsIn(params.amountOutMin, params.path)[0];
        ...
        require(RBCToken.transferFrom(blockchainPool, address(this), amountIn),
                                                        "swapContract: transfer from pool failed");
        ...
        require(RBCToken.transfer(blockchainPool, amountIn - RBCSpent), 'swapContract: remaining RBC transfer to pool failed');

        emit TransferFromOtherBlockchain(params.user, amountIn, amountIn, params.originalTxHash);
    }
    ...
    function setRefundSlippage(uint256 _refundSlippage) external onlyOwnerAndManager{
        refundSlippage = _refundSlippage;
    }
    ...
    /**
      ...
      * @return statusCode Transaction status code: 0-Not processed, 1-Processed, 2-Reverted
      * @return hashedParams Hashed params with which the initial transaction was executed
      */
    function isProcessedTransaction(bytes32 originalTxHash) public view returns (uint8 statusCode, bytes32 hashedParams) {
        hashedParams = processedTransactions[originalTxHash].hashedParams;
        statusCode = processedTransactions[originalTxHash].statusCode;
    }
    ...
    /**
      ...
      */
    function changeTxStatus(bytes32 originalTxHash, uint8 statusCode, bytes32 hashedParams) external onlyRelayer {
        require(statusCode != 0, 'swapContract: you cannot set the statusCode to 0');
        require(processedTransactions[originalTxHash].statusCode != 2, 'swapContract: transaction with this originalTxHash has already been set as reverted');
        require(processedTransactions[originalTxHash].statusCode != 1, 'swapContract: transaction with this originalTxHash has already been set as succeed');
