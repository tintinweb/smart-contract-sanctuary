...
contract DeGeThalPresale1 is Ownable {
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "DeGeThalPresale: sale is not open");
        require(!hasEnded(), "DeGeThalPresale: sale is over");
        require(
            weiAmount >= minContribution,
            "DeGeThalPresale: min contribution criteria not met"
        );
        require(
            contributions[beneficiary].add(weiAmount) <= maxContribution,
            "DeGeThalPresale: max contribution criteria not met"
        );
        this;
    }

    function claimTokens() external {
        require(hasEnded(), "DeGeThalPresale: private sale is not over");
        require(
            softCapReached(),
            "DeGeThalPresale: soft cap not reached, refund is available"
        );
        require(
            contributions[msg.sender] > 0,
            "DeGeThalPresale: nothing to claim"
        );
    ...
    function claimRefund() external {
        require(hasEnded(), "DeGeThalPresale: private sale is not over");
        require(!softCapReached(), "DeGeThalPresale: soft cap not reached");
        require(
            contributions[msg.sender] > 0,
            "DeGeThalPresale: nothing to claim"
        );
    ...
    function endPresale() external onlyOwner {
        require(!finalized, "DeGeThalPresale: private sale is already over");
        finalized = true;
    ...
    function withdrawTokens() public onlyOwner {
        require(finalized, "DeGeThalPresale: private sale is not over");
