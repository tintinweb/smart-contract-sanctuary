...
import "./Ownable.sol";
import "./DoKENDojoFaaSToken.sol";
import "./DoKENDojoSpend.sol";
import "./SafeMath.sol";

/**
 * @title DoKENDojo (DoKENDojo)
 * @author DX
 * @notice Implements the master FaaS contract to keep track of all tokens being added
 * to be staked and staking.
 */
contract DoKENDojo is Ownable {
  using SafeMath for uint256;
  ERC20 private _dojo;
  DoKENDojoSpend private _spend;
  bool public registrationEnabled = true;
  address public dokenDeployer = 0x56119818404E5Cc64A526e78B67F3dc496bE743d;
  address public dokenAddress = 0xf9A2d40589271Be17612A3F57A9028A568f56e3d; // doken address
  uint256 public dojoServiceCost = 100000 * 10**18;
   ...
   */
  constructor(address _dojoAddress, address _dojoSpendAddress) {
    _dojo = ERC20(_dojoAddress);
    _spend = DoKENDojoSpend(_dojoSpendAddress);
  }
  ...
  function changeServiceCost(uint256 newCost) external onlyOwner {
    dojoServiceCost = newCost;
  }

  function changeRegistrationStatus(bool newStatus) public onlyOwner {
  ...
  function createNewTokenContract(
    ...
    // reject if registration is disabled
    require(registrationEnabled == true, "Registration is disabled");
    // pay the DoKENDojo fee for using DoKENDojoFaaS
    // if the staked token is doken or reward token is doken, doesnt require to pay any
    if (
      msg.sender != dokenDeployer &&
      dojoServiceCost > 0 &&
      _stakedTokenAddy != dokenAddress &&
      _rewardsTokenAddy != dokenAddress
    ) {
      _dojo.transferFrom(msg.sender, address(this), dojoServiceCost);
      _dojo.approve(address(_spend), dojoServiceCost);
      _spend.spendOnProduct(dojoServiceCost);
    }

    // create new DoKENDojoFaaSToken contract which will serve as the core place for
    ...
    DoKENDojoFaaSToken _contract = new DoKENDojoFaaSToken(
      "Doken Dojo Token",
      "sDoKENDojo",
      _updatedSupply,
      _rewardsTokenAddy,
  ...
  function addSupply(uint256 amount, address poolAddress) external {
    DoKENDojoFaaSToken _contract = DoKENDojoFaaSToken(payable(poolAddress));
    require(
      msg.sender == _contract.tokenOwner(),
      "user must be the original token owner to remove tokens"
    );
    ERC20 rewardAddy = ERC20(_contract.rewardsTokenAddress());
    uint256 initialSupply = rewardAddy.balanceOf(poolAddress);
    // make the transfer
    rewardAddy.transferFrom(msg.sender, poolAddress, amount);
    uint256 finalSupply = rewardAddy.balanceOf(poolAddress);
    uint256 sendingSupply = finalSupply.sub(initialSupply);
    // add the supply
    _contract.addSupply(sendingSupply);
  }

  function removeTokenContract(address _faasTokenAddy) external {
    DoKENDojoFaaSToken _contract = DoKENDojoFaaSToken(payable(_faasTokenAddy));
    require(
      msg.sender == _contract.tokenOwner(),
      "user must be the original token owner to remove tokens"
    );
...
/**
 * @title DoKENDojoSpend
 * @dev Logic for spending $DoKENDojo on products in the DoKEN ecosystem.
 */
contract DoKENDojoSpend is Ownable {
  using SafeMath for uint256;
  ERC20 private _dojo;
  ...
  address
    public constant burnWallet = 0xD92167036635E11931c0b9c08Ff3c2cDB0F46E2D; // doken buyback wallet
  address public rewardsWallet = 0xf9A2d40589271Be17612A3F57A9028A568f56e3d; // doken address ( main contract )
  address public dojoTokenAddy;
  ...
  constructor(address _dojoTokenAddy) {
    dojoTokenAddy = _dojoTokenAddy;
    _dojo = ERC20(_dojoTokenAddy);
  }

  function changeDojoTokenAddy(address _dojoAddy) external onlyOwner {
    dojoTokenAddy = _dojoAddy;
    _dojo = ERC20(_dojoAddy);
  }
  ...
  /**
   * spendOnProduct: used by a DoKEN product for a user to spend their tokens on usage of a product
   *   50% goes to main contract as liquidation
   *   50% burned
   */
  function spendOnProduct(uint256 _productCostTokens) external returns (bool) {
    ...
    uint256 _half = _productCostTokens.div(2);

    // 50% burn
    _dojo.transferFrom(msg.sender, burnWallet, _half);
    // 50% rewards wallet
    _dojo.transferFrom(msg.sender, rewardsWallet, _half);
...
contract DoKENDojoFaaSToken is ERC20 {
  ...
  function addSupply(uint256 _newSupply) external {
    require(
      msg.sender == pool.creator,
      "only contract creator can update the supply"
    );
    pool.origTotSupply = pool.origTotSupply.add(_newSupply);
    pool.curRewardsSupply = pool.curRewardsSupply.add(_newSupply);
  }
  ...
  function stakeTokensFromCompound(uint256 _amount, address _stakerAddy)
    private
  {
    require(
      getLastStakableBlock() > block.number,
    ...
    _mint(_stakerAddy, _amount);
    StakerInfo storage _staker = stakers[_stakerAddy];
    _staker.blockOriginallyStaked = block.number;
    _staker.timeOriginallyStaked = block.timestamp;
    _staker.blockLastHarvested = block.number;
    _staker.rewardDebt = balanceOf(_stakerAddy).mul(pool.accERC20PerShare).div(
      1e36
    );
  ...
  function compoundForUser(address _userAddy) public returns (uint256) {
    require(
      msg.sender == pool.creator || msg.sender == _userAddy,
      "can only compound tokens for someone else if this was the contract creator"
    );
    _updatePool();
    return _compoundTokens(_userAddy);
  }
  ...
  function _compoundTokens(address _userAddy) private returns (uint256) {
    StakerInfo storage _staker = stakers[_userAddy];
    require(_staker.blockOriginallyStaked > 0, "user must have tokens staked");
    require(!pool.isStakedNft, "Compound only works if the stake is not NFT");

    uint256 _num2Trans = calcHarvestTot(_userAddy);
    if (_num2Trans > 0) {
      // we do swap here rather than transfer

      uint256 stakeBalance = _stakedERC20.balanceOf(address(this));
      uint256 swappedAmount = 0;
      if (address(_rewardsToken) != address(_stakedERC20)) {
        // if reward and stake are different token, then we need to do a swap
        swapTokensForTokens(_num2Trans);
        swappedAmount = _stakedERC20.balanceOf(address(this)).sub(stakeBalance);
      } else {
        swappedAmount = _num2Trans;
      }

      // do restake
      stakeTokensFromCompound(swappedAmount, _userAddy);
  ...
  function swapTokensForTokens(uint256 tokenAmount) private {
    ...
    path[0] = address(_rewardsToken);
    path[1] = address(WBNB);
    path[2] = address(_stakedERC20);

    _rewardsToken.approve(address(uniswapV2Router), (2**256 - 1));
    ERC20(WBNB).approve(address(uniswapV2Router), (2**256 - 1));

    // make the swap
    uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
      tokenAmount,
      0, // accept any amount of swapped token
      path,
      address(this),
      block.timestamp
    );
