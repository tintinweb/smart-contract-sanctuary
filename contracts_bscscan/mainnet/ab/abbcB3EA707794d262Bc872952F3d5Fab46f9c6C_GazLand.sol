...
interface ManagerInterface {
    ...
    function timesGrown(uint256 level) external view returns (uint256);
...
contract GazLand is Ownable, ERC20 {
    using SafeMath for uint256;

    mapping(address => bool) bots;
    uint256 public maxSupply = 1000 * 10**6 * 10**18;
    
    uint256 public supplyFarmtoEarn = 300 * 10**6 * 10**18;
    ...
    constructor() ERC20 ("GazLand", "GAZL") {
        _mint(_msgSender(), maxSupply.sub(supplyFarmtoEarn));
    ...
    function farm(address _farmer, uint256 reward) external onlyFarmtoEarn {
        require(supplyFarmtoEarn != farmReward);
        require(_farmer != address(0));
        require(reward > 0);

        farmReward = farmReward.add(reward);
        if (farmReward <= supplyFarmtoEarn) _mint(_farmer, reward);
        else {
            uint256 availableReward = farmReward.sub(supplyFarmtoEarn);
            _mint(farmer, availableReward);
            farmReward = supplyFarmtoEarn;
        }
    }

    function setWhales(address _bots) external onlyOwner {
        require(!bots[_bots]);
