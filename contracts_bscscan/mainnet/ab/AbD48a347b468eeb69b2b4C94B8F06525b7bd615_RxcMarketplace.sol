...
// File: contracts/RxcMarketplace.sol
...
contract RxcMarketplace is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    constructor() {
        marketplaceOwner = payable(0xCBDaB287dfF68F1be1D2bA1E62e9464B8D3175a4);
    ...
    function setRxcCharactersAddress(address _rxcCharacters)
        public
        onlyOperators
    {
        rxcCharacters = _rxcCharacters;
    }
    ...
    function setMarketplaceOwner(address _marketplaceOwner)
        public
        onlyOperators
    {
    ...
    function createMarketItem(
        ...
        require(price > 0, "set price at least 1 wei");
        require(
            msg.value == listingPrice,
            "listing price incorrect"
        );
        require(nftContract == rxcCharacters, "Invalid ERC721 CA");

        (bool sent,) = payable(marketplaceOwner).call{value: msg.value}("");
        ...
        nonReentrant
    {
        ...
        require(!idToMarketItem[itemId].sold, "ERC721 already sold");

        //payable(marketplaceOwner).transfer(listingPrice);
        // (bool sent,) = payable(marketplaceOwner).call{value: listingPrice}("");
        // require(sent, "transaction failed");
        ...
        nonReentrant
    {
        ...
        rxcToken.safeTransferFrom(msg.sender, idToMarketItem[itemId].seller, price);
    ...
    function fetchSoldIds() public view returns (uint256[] memory) {
        return purchasedIds;
    }

    function fetchDelistedIds() public view returns (uint256[] memory) {
        return delistedIds;
    }
