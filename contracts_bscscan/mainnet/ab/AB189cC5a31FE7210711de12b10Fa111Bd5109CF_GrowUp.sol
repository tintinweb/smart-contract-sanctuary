...
contract GrowUp is ERC20Detailed, Ownable {
  ...
  // TOTAL_GWP is a multiple of INITIAL_FRAGMENTS_SUPPLY so that _GWPPerFragment is an integer.
  // Use the highest value that fits in a uint256 for max granularity.
  uint256 private constant TOTAL_GWP = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

  // MAX_SUPPLY = maximum integer < (sqrt(4*TOTAL_GWP + 1) - 1) / 2
  uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

  uint256 private _totalSupply;
  uint256 private _GWPPerFragment;
  mapping(address => uint256) private _GWPBalances;
  ...
  ERC20Detailed("GROW UP", "GWP", uint8(DECIMALS))
  payable
  public
  {
    ...
    _GWPBalances[msg.sender] = TOTAL_GWP;
    _GWPPerFragment = TOTAL_GWP.div(_totalSupply);
...
*/
  ...
  returns (uint256)
  {
    return _GWPBalances[who].div(_GWPPerFragment);
  }
  ...
  function _transferStandard(address sender, address recipient, uint256 amount) private {
    ...
    uint256 GWPDeduct = amount.mul(_GWPPerFragment);
    uint256 GWPValue = tTransferAmount.mul(_GWPPerFragment);
    _GWPBalances[sender] = _GWPBalances[sender].sub(GWPDeduct);
    _GWPBalances[recipient] = _GWPBalances[recipient].add(GWPValue);
    _takeFee(sender, tFee);
    emit Transfer(sender, recipient, amount);
  }

  function _transferExcluded(address sender, address recipient, uint256 amount) private {
    uint256 GWPValue = amount.mul(_GWPPerFragment);
    _GWPBalances[sender] = _GWPBalances[sender].sub(GWPValue);
    _GWPBalances[recipient] = _GWPBalances[recipient].add(GWPValue);
    emit Transfer(sender, recipient, amount);
  }
  ...
  function _takeFee(address sender, uint256 tFee) private {
    uint256 rFee = tFee.mul(_GWPPerFragment);
    _GWPBalances[address(this)] = _GWPBalances[address(this)].add(rFee);
