...
abstract contract CandleGenieDiceRNG
{
...
contract CandleGenieDice is Ownable, ReentrancyGuard 
{
    ...
    CandleGenieDiceRNG public candleGenieDiceRNG;
    ...
    modifier onlyCGContracts() 
    {
        require(msg.sender == candleGenieDiceRNG.rollControllerAddress() || msg.sender == address(candleGenieDiceRNG), "Only CG contracts allowed");
        _;
    }
    ...
    function SetRNG(address _rngAddress) external onlyOwner {
    
        candleGenieDiceRNG = CandleGenieDiceRNG(_rngAddress);
        emit DiceRandomSourceUpdated(_rngAddress);
    }
    ...
    function RefundBet(address user, uint256 rollId) external onlyOwner
    {
        ...
        bool payment = _safeTransferBNB(bet.user, bet.betAmount);
        if (payment)
        {
            bet.paid = true;
    ...
    function _safeRoll(address user, uint256 betAmount, int position, bool useVRF) internal
    {
        ...
        if (useVRF)
        {
            ...
            rollID = candleGenieDiceRNG.RollWithVRF();
        }
        else
        {
            // Rolling Dice via PSEUDO
            rollID = candleGenieDiceRNG.RollWithPSEUDO();
        }
        ...
        if (!useVRF)
        {
           candleGenieDiceRNG.fulfillPSEUDO(bet.rollId); 
        }

    }
    
    function Drop(uint256 rollId, int rollResult) external onlyCGContracts
    {
