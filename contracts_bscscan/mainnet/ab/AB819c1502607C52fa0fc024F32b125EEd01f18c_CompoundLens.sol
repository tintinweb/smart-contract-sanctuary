...
interface DynamicRainMakerForLens {
...
interface ComptrollerLensInterface {
    ...
    function rainMaker() external view returns (DynamicRainMakerForLens);
...
contract CompoundLens {
    struct CTokenMetadata {
        ...
        // Keeping 'incentiveSpeed' for backwards compatibility
        uint incentiveSpeed;
        uint incentiveSupplySpeed;
        uint incentiveBorrowSpeed;
    }
    ...
    function cTokenMetadata(CToken cToken) public returns (CTokenMetadata memory) {
        ...
        return CTokenMetadata({
            ...
            incentiveSupplySpeed: comptroller.hasRainMaker() ? comptroller.rainMaker().compSupplySpeeds(address(cToken)) : 0,
            incentiveBorrowSpeed: comptroller.hasRainMaker() ? comptroller.rainMaker().compBorrowSpeeds(address(cToken)) : 0
        });
    }
    ...
    function getCompBalanceMetadata(EIP20Interface comp, address account) external view returns (CompBalanceMetadata memory) {
    ...
    function getCompBalanceMetadataExt(EIP20Interface comp, ComptrollerLensInterface comptroller, address account) external returns (CompBalanceMetadataExt memory) {
    ...
    function getPriceForAsset(address len, address asset) public view returns (uint256) {
        return ComptrollerLensInterface(len).getRegistry().getPriceForAsset(asset);
    }
