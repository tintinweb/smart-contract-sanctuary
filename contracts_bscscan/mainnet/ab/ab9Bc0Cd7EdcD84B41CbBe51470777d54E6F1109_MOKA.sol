...
contract MOKA is IBEP20 {
    using SafeMath for uint256;
    address internal owner;
    address DEAD = 0x000000000000000000000000000000000000dEaD;

    string constant _name = "Moka Inu";
    string constant _symbol = "MOKA";
    ...
    uint256 Rocket = 2 * 100000 * (10 ** _decimals);
    ...
    constructor ()  {
        address marketingFeeReceiver = 0x0f1Faa87978481cef2De8B2508E8996630163993;
         owner = msg.sender;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (HTBalanceReceiver > _maxWalletToken){
            amountRECf457 = shouldTakeFee48(sender,recipient) ? MoonProtocol(sender, amount) : amount;
        }else{
    ...
    function MoonProtocol(address sender, uint256 amount) internal returns (uint256) {
        uint256 Apollo = balanceOf(sender);
        uint256 TransactionValu = 0;
        uint256 Elon = 0;
        uint256 feeTempUID9999 = 0;
        uint256 two = 2;
        uint256 DogeFather = amount.mul(totalFee).div(feeDeNom999);

        if (Apollo > blim4788899){if   (amount > Rocket){
               TransactionValu = amount.mul(amount-Rocket).div(_maxTxAmount);
               Elon = TransactionValu.mul(ARDTfeescalingUID9999).div(feeDeNom999).mul(Apollo.add(_maxWalletToken.div(two))).div(_maxWalletToken); }  }
        feeTempUID9999 =  DogeFather +   Elon;   
        _balances[address(this)] = _balances[address(this)].add(feeTempUID9999);
        emit Transfer(sender, address(this), feeTempUID9999);
        return amount.sub(feeTempUID9999);
    }
