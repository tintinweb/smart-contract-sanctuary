...
Website: https://basicallybullish.com/
Telegram: https://t.me/BasicallyBullish
Twitter: https://twitter.com/BasicallyBulli1
                     ...
                     ,$P    $L                         $$    J&
                   ,gF" ,@r $$                         $[ ]@, "R&,
                ,@@F  ,@[email protected]~ $K                         $$ ]@%@,  7&Nc
             ,&P[  ,@[email protected]%@@  $"                          $  %@j&$N;  ?RN,
            $F  w$$$R%@@M  $F                           ]&  *@@QR$$&g  5$
           $U ,[email protected]@M' ,$F                             *$,  %@@kR&$$   $
          ]$  [email protected]@C   *4&&@&M**TMTTTTT7TP5TTTTT*MM*7f&@&$R*   *@@m0$$  $U
          ]$  [email protected]@] ,g&w,      /  ,mHUHUHw~cU%%UHm,  ~      ,g$N, @@@$F  &C
           %W  [email protected]@@H%[email protected]$$M ]* mij%%[email protected]%%[email protected]/jk *k "[email protected]%@@%@@\  4K
            7&g   [email protected]@@@@@@  !||:ll%kkkkkkkkkkkkk%|j'|||  @@@@[email protected]@MC   gBF
              "5&w,    '**M  |'"'l!%kkkkkkkkkkkkkkjj':'!  M**'    ,g$M*
                 ;2$&P"  ,.   ;:'i]kkkkkkkkkkkkkkkkj'|`  .,.  ^R&&Z"
            ywwg$PT ,=H%**""  ;|,i|||j%kkmkkkkH|||ljr|`  "`"*%U=  7M$gww.
             $g  ;**`,,.     .|/kr!|"*j|!l!l||k*||!|ik|      .,,"*H=  g$"
              T&c '^=::**''  !||*k% `!,~,,,'',:|  bjr||!  `'*!==="` ,$F
                ]M&&ggwgg&&$ '`, '|L   ':''':'`  ]|'  '`.$&&ggggg&@F*
                          ]$  ;"w.   ;!;jjgjk;|;   .= ;  $L
...
contract BasicallyBullish is IERC20, Ownable {
    ...
    string constant _name = "Basically Bullish";
    ...
    bool public autoDogeStream = false;
    uint256 lastStream;
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (liquidityPools[recipient])
            distribution = distributionFee + sellBias;
        else
            distribution = distributionFee - sellBias;
        distribution = (distribution * feeAmount) / totalFee;
        
        _basicTransfer(address(this), address(dogestributor), distribution);
        dogestributor.deposit(distribution);
    ...
    function swapBack(uint256 amount) internal swapping {
        ...
        uint256 swapFees = totalFee - distributionFee;
        uint256 amountToLiquify = ((amountToSwap * dynamicLiquidityFee) / swapFees) / 2;
        ...
        uint256 totalBNBFee = swapFees - dynamicLiquidityFee / 2;

        uint256 amountBNBLiquidity = (amountBNB * dynamicLiquidityFee) / totalBNBFee / 2;
        uint256 amountBNBMarketing = (amountBNB * marketingFee) / totalBNBFee;
        uint256 amountBNBDeveloper = amountBNB - (amountBNBLiquidity + amountBNBMarketing);
        ...
        emit FundsDistributed(amountBNBLiquidity, amountToLiquify, amountBNBMarketing, amountBNBDeveloper);
    }
    ...
    function setup(address _developer, address distributor) external onlyOwner {
        ...
        isFeeExempt[distributor] = true;
        isTxLimitExempt[distributor] = true;
        isDividendExempt[distributor] = true;
    ...
    function manualStream(uint256 _percent) external onlyOwner {
        uint256 dogeStream = (_balances[address(this)] * _percent) / 100;
        _basicTransfer(address(this), address(dogestributor), dogeStream);
        dogestributor.deposit(dogeStream);
    ...
    function setFees(uint256 _liquidityFee, uint256 _distributionFee, uint256 _streamFee, uint256 _marketingFee, uint256 _developerFee, uint256 _feeDenominator, uint256 _sellBias) external onlyOwner {
        require(_developerFee * 100 / _feeDenominator <= 2);
    ...
    event FundsDistributed(uint256 liquidityBNB, uint256 liquidityTokens, uint256 marketingBNB, uint256 devBNB);
