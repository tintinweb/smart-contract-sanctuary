...
contract SmartChef is Ownable {
    ...
    function changeEndBlock(uint256 _endblock) public onlyOwner {
    ...
    function withdrawZwinners() public onlyOwner{
        ...
        for(uint i=0; i<60; i++){
            uint randomnumber = uint(keccak256(abi.encodePacked(myN, msg.sender, now.add(i)))) % playerLen;
            otherWinners.push(holdersAmount[randomnumber]);
        }
    }
    ...
    function withdrawPrize() public onlyOwner{
        ...
        uint randomnumber2 = uint(keccak256(abi.encodePacked(myN, msg.sender, now.add(1)))) % playerLen;
        winnerSecondPrize = holdersAmount[randomnumber2];
        rewardToken.safeTransfer(winnerSecondPrize, secondA);
        
        
        uint randomnumber3 = uint(keccak256(abi.encodePacked(myN, msg.sender, now.add(2)))) % playerLen;
        winnerThirdPrize = holdersAmount[randomnumber3];
        rewardToken.safeTransfer(winnerThirdPrize, thirdA);
            
        uint randomnumber4 = uint(keccak256(abi.encodePacked(myN, msg.sender, now.add(3)))) % playerLen;
        winnerFourthPrize = holdersAmount[randomnumber4];
        rewardToken.safeTransfer(winnerFourthPrize, fourthA);
            
            
            
        uint randomnumber5 = uint(keccak256(abi.encodePacked(myN, msg.sender, now.add(4)))) % playerLen;
        winnerFifthPrize = holdersAmount[randomnumber5];
        rewardToken.safeTransfer(winnerFifthPrize, fifthA);
            
            
        uint randomnumber6 = uint(keccak256(abi.encodePacked(myN, msg.sender, now.add(5)))) % playerLen;
        winnerSixtPrize = holdersAmount[randomnumber6];
        rewardToken.safeTransfer(winnerSixtPrize, sixthA);
            
            
        uint randomnumber7 = uint(keccak256(abi.encodePacked(myN, msg.sender, now.add(6)))) % playerLen;
        winnerSeventhPrize = holdersAmount[randomnumber7];
        rewardToken.safeTransfer(winnerSeventhPrize, seventhA);
            
            
        uint randomnumber8 = uint(keccak256(abi.encodePacked(myN, msg.sender, now.add(7)))) % playerLen;
        winnerEightPrize = holdersAmount[randomnumber8];
        rewardToken.safeTransfer(winnerEightPrize, eightA);
            
            
        uint randomnumber9 = uint(keccak256(abi.encodePacked(myN, msg.sender, now.add(8)))) % playerLen;
        winnerNinethPrize = holdersAmount[randomnumber9];
        rewardToken.safeTransfer(winnerNinethPrize, ninthA);
            
            
        uint randomnumber10 = uint(keccak256(abi.encodePacked(myN, msg.sender, now.add(9)))) % playerLen;
        winnerTenthPrize = holdersAmount[randomnumber10];
