...
contract busdSwap is Ownable {
    ...
    address public depthSwapAddress = 0x40EdED89bb6c94BDb2eDEDbfb194b2e29c1FCc9A;  //depth swap address .convert usdt to busd
    address public dexSwapAddress  = 0x10ED43C718714eb63d5aA57B78B54704E256024E;  //other token swap.default is pancke swap PancakeRouter

    mapping(address => address[]) paths;

    constructor () public {
        paths[xvs] = [cowAddress, wbnb, busd];          // cow
        paths[cowAddress] = [cowAddress, usdt, busd];   // xvs
        paths[wbnb] = [wbnb, busd];                     // bnb
    }
    ...
    function swapTokensToEarnToken(address _token,uint256 _amount) external{
        ...
        uint256 _beforeHusdBalance = IERC20(busd).balanceOf(address(this));
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);

        if (_token==usdt){
            IStableRouter(depthSwapAddress).exchange_underlying(1,0,_amount,0);
        } else if (_token==usdc){
            IStableRouter(depthSwapAddress).exchange_underlying(2,0,_amount,0);
        }else {
            ...
            ISwapRouter(dexSwapAddress).swapExactTokensForTokens(_amount, 0, path, address(this), block.timestamp.add(1800));
        }

        uint256 _afterHusdBalance = IERC20(busd).balanceOf(address(this)).sub(_beforeHusdBalance);
        if(_afterHusdBalance>0){
            IERC20(busd).safeTransfer(msg.sender, _afterHusdBalance);
        }
    }
