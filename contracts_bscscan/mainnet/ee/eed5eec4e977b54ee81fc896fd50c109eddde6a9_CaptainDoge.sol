...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public DividendsToken = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82);
  ...
  function updateDividendToken(address _DividendsToken) public {
...
contract CaptainDoge is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   CaptainDogeDividendTracker public dividendTracker;
	...
	uint256 private marketingDivisor = 50;
    ...
    constructor() ERC20 ("CaptainDoge", "CDOG") {
        _RewardsFee = 10;
        _liquidityFee = 5;
		_marketingFee = 2;
		_CharityFee = 1;
        ...
        _maxWalletToken = 2000000000 * (10**18); // 2% of total supply

        totalFees = _RewardsFee.add(_liquidityFee).add(_marketingFee).add(CharityFee);

    	dividendTracker = new CaptainDogeDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();

		marketingWallet = payable(0x6a6817486aefD3F6528726F28A3D280d5Fbc7177);
		CharityWallet = payable(0x334AaBB542141c9a2b53124c3931234217302572);
    ...
    function updatesellFeeIncreaseFactor(uint256 _sellFeeIncreaseFactor) public onlyOwner {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CaptainDoge: The dividend tracker already has that address");

        CaptainDogeDividendTracker newDividendTracker = CaptainDogeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CaptainDoge: The new dividend tracker must be owned by the CaptainDoge token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "CaptainDoge: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CaptainDoge: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CaptainDoge: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
    
    function excludeFromMaxSell(address account, bool excluded) public onlyOwner {
        require(_isExcludedMaxSellTransactionAmount[account] != excluded, "CaptainDoge: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CaptainDoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CaptainDoge: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "CaptainDoge: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CaptainDoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CaptainDoge: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
		...
		if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != liquidityWallet && to != liquidityWallet && from != marketingWallet && to != marketingWallet && from != CharityWallet && to != CharityWallet) {
            
            swapping = true;

            uint256 swapTokens = marketingAmount;
            swapTokensForBNB(swapTokens);
            transferToWallet(payable(marketingWallet), address(this).balance);
            
            swapTokens = CharityAmount;
            swapTokensForBNB(swapTokens);
            transferToWallet(payable(CharityWallet), address(this).balance);
        ...
        if(takeFee) {
        	...
        	CharityAmount = fees.mul(CharityFee);
        	marketingAmount = fees.mul(marketingFee);
    ...
    function swapTokensForRewards(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of CaptainDoge
            path,
            recipient,
            block.timestamp
        );
    ...
    function removeAllFee() external onlyOwner() {
        ...
        maxBuyTranscationAmount = 0 * 10**18;
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "CaptainDoge: Marketing tax must be between 0 and 10");
        marketingFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "CaptainDoge: Liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
    }
    
    function updateRewardsFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 20, "CaptainDoge: Rewards tax must be between 0 and 20");
        RewardsFee = newFee;
    }
    
    function updateCharityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 20, "CaptainDoge: Charity Fee must be between 0 and 20");
        CharityFee = newFee;
    }
...
contract CaptainDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("CaptainDoge_Dividend_Tracker", "CaptainDoge_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "CaptainDoge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "CaptainDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CaptainDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "CaptainDoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CaptainDoge_Dividend_Tracker: Cannot update claimWait to same value");
