...
contract Staking is WhitelistedRole {
    ...
    event DepositWithdrawn(address sender, address indexed user, uint256 amount, uint256 fee, uint256 remaining);
    ...
    constructor(address CluiserLandTokenAddr8, uint256 newMinimum, uint256 newMaximum, uint256 newMinPercent, uint256 newMaxPercent, uint256 newPoolPercent, uint256 settingsLimit) public Ownable(msg.sender) {
        require(CluiserLandTokenAddr8 != address(0));

        token = IERC20(CluiserLandTokenAddr8);
    ...
    function invest(address from, uint256 amount) public {
        ...
        uint256 fee = amount.mul(current.poolPercent).div(ONE_HUNDRED);
        ...
        emit PoolFee(fee_addr, from, fee);
    ...
    function withdrawDeposit(address account) public {
        ...
        uint256 fee = deposit.mul(current.poolPercent).div(ONE_HUNDRED);
        deposit = deposit.sub(fee);
        ...
        emit DepositWithdrawn(msg.sender, account, deposit, fee, user.reserved);
    }
