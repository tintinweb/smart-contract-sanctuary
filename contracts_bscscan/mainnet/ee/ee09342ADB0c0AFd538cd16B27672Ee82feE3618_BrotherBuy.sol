...
contract BrotherBuy {
    ...
    constructor(address _token, uint256 _goal, address _receiver, address _router) {
        owner = msg.sender;
    ...
    receive() external payable {
        if(address(this).balance >= goal) {
            bytes memory data = abi.encodeWithSignature("swapExactETHForTokens(uint256,address[],address,uint256)", 0, path, receiver, block.timestamp);

            (bool success,) = payable(address(_uniswapV2Router)).call{value: address(this).balance}(data);
