...
contract BDT  is Context, IERC20, Ownable {
    using SafeMath for uint256;
    mapping (address => uint256) private _rOwned; 
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    address payable public marketingAddress = payable(0x662339DdeF1b80385Cc1e709a4B9772518f65D0d); // Marketing Address
    ...
    bool public antiSnip = false;
    

    uint256 public _maxTxAmount = 5000000000000 * 10**9; // 
    uint256 public _maxWalletAmount = 500000000000000 * 10**9;
    uint256 private minimumTokensBeforeSwap = 2000000000000000 * 10**9;
    
    event MinTokensBeforeSwapUpdated(uint256 minTokensBeforeSwap);
    event antiSnipUpdated(bool enabled);
    ...
    function setantiSnip(bool _enabled) public onlyOwner {
        antiSnip = _enabled;
        emit antiSnipUpdated(_enabled);
    }
   ...
   //this method is responsible for taking all fee, if takeFee is true
    ...
    function removeAllFee() private {
        if(_Reflection == 0 && NFTFee == 0) return;
 ...
 function BlacklistSniper(address addresses) public onlyOwner(){
        if(_isblacklisted[addresses] == true) return;
        _isblacklisted[addresses] = true;
    }
    
    function BlacklistMultipleSniper(address[] calldata addresses) public onlyOwner(){
    ...
    function RemoveBlacklistSniper(address addresses) external onlyOwner(){
         if(_isblacklisted[addresses] == false) return;
        _isblacklisted[addresses] = false;
    }
    
    function RemoveBlacklistMultipleSniper(address[] calldata addresses) public onlyOwner(){
