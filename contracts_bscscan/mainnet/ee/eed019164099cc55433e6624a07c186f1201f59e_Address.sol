...
contract Santa_Cats is Context, IERC20, Ownable {
    ...
    string private _name = "Santa_Cats";
    ...
    address public MarketingWallet = 0x56d4e87318b3697eF1D6097f633B93F048c78880;
    address public BurnWallet = 0x000000000000000000000000000000000000dEaD;
    address public AdminWallet = 0x7FCCFbCFc37e08A5C2823d9E117F311d2750866A;
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee == 0 && _burnFee == 0 && _adminFee == 0) return;
            ...
            if(_addressFees[from].enable){
                ...
                if(_addressFees[to].enable){
                    ...
                    if(from == uniswapV2Pair){
                        ...
                        _burnFee = _addressFees[to]._buyBurnFee;
                        _adminFee = _previousAdminFee;
                    }
                }
            }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 AdminAmt = amount.mul(_adminFee).div(100);

        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(MarketingAmt).sub(BurnAmt).sub(AdminAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(MarketingAmt).sub(BurnAmt).sub(AdminAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(MarketingAmt).sub(BurnAmt).sub(AdminAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(MarketingAmt).sub(BurnAmt).sub(AdminAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(MarketingAmt).sub(BurnAmt).sub(AdminAmt)));
        }
        ...
        _transferStandard(sender, AdminWallet, AdminAmt);
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBurnFee) external onlyOwner {
    ...
    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellBurnFee) external onlyOwner {
    ...
    function setAddressFee(address _address, bool _enable, uint256 _addressTaxFee, uint256 _addressLiquidityFee, uint256 _addressMarketingFee, uint256 _addressBurnFee) external onlyOwner {
        ...
        _addressFees[_address]._burnFee = _addressBurnFee;
    }
    
    function setBuyAddressFee(address _address, bool _enable, uint256 _addressTaxFee, uint256 _addressLiquidityFee, uint256 _addressMarketingFee, uint256 _addressBurnFee) external onlyOwner {
        _addressFees[_address].enable = _enable;
        ...
        _addressFees[_address]._buyBurnFee = _addressBurnFee;
    }
    
    function setSellAddressFee(address _address, bool _enable, uint256 _addressTaxFee, uint256 _addressLiquidityFee, uint256 _addressMarketingFee, uint256 _addressBurnFee) external onlyOwner {
        _addressFees[_address].enable = _enable;
        ...
        _addressFees[_address]._sellBurnFee = _addressBurnFee;
    }
