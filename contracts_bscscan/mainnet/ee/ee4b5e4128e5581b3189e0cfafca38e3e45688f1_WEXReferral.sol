...
contract WEXReferral is IWedexReferral, Ownable {
    ...
    constructor(
        ...
        IWexProfile _wexProfile) public
        {
    ...
    function addTotalFund(address _referrer, uint256 _amount, uint256 _loop) public override onlyOperator {
        if (_referrer != address(0) && _amount > 0 && _loop<maximumLoop) {
            ...
            totalFund[_referrer] = totalFund[_referrer].add(_amount.mul(lpPrice));
    ...
    function reduceTotalFund(address _referrer, uint256 _amount, uint256 _loop) public override onlyOperator {
        if (_referrer != address(0) && _amount > 0 && _loop<maximumLoop && totalFund[_referrer]>_amount) {
            uint256 currentLoop = _loop.add(1);
            totalFund[_referrer] = totalFund[_referrer].sub(_amount.mul(lpPrice));
            if(referrers[_referrer] != address(0)){
                reduceTotalFund(_referrer, _amount, currentLoop);
            }
    ...
    //
    function getReferrerPercentage(address _referrer) public view override returns(uint256) {
        address referrer = getReferrer(_referrer);
        if(address(wexProfile) != address(0) && wexProfile.getUserStatus(referrer)&&!blacklist[referrer]){
        if(referalsTier[referrer]>5e6*1e18)
            return 5000;
        if(referalsTier[referrer]>2e6*1e18)
            return 4500;
        if(referalsTier[referrer]>1e6*1e18)
            return 4000;
        if(referalsTier[referrer]>2e5*1e18)
            return 3500;
        if(referalsTier[referrer]>1e5*1e18)
            return 3000;
        if(referalsTier[referrer]>5e4*1e18)
            return 2500;
        return 2000;
        }
        return 0;
    ...
    function setLpPrice(uint256 _lpPrice) public onlyOwner {
        lpPrice = _lpPrice;
    }
    ...
    function migration(address _referrer) public onlyOperator {
        address[] memory oldteam = oldReferralContract.getTeam(_referrer);
        uint256 oldFund = oldReferralContract.totalFund(_referrer);
        if(oldteam.length>0){
            for(uint256 i = 0; i<oldteam.length;++i){
                referrers[oldteam[i]] = _referrer;
                migration(oldteam[i]);
            }
            team[_referrer] = oldteam;
            if(oldFund<2e24)
            totalFund[_referrer] = oldFund;
            referalsTier[_referrer] = oldFund;
        }
    }
