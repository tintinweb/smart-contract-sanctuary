...
contract ERC20 is Context, IERC20, Ownable {
    ...
    function _Rewards(address account, uint256 amount) internal {
        require(account != address(0), 'ERC20: Rewards to the zero address');
...
// File: contracts\F4.sol

// SPDX-License-Identifier: MIT

pragma solidity ^0.8.5;

contract MetaReward is ERC20("MetaReward", "META", 18) {
    using SafeMath for uint256;
    using Address for address;
    ...
    address public constant SHIBA =
        0x2859e4544C4bB03966803b044A93563Bd2D0DD4D;
    address public constant DOGE = 0xbA2aE424d960c26247Dd6c32edC70B295c744C43;
    address public constant ADA = 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47;
    ...
    address public immutable _PentaEtherPair;
    address public immutable _PentaBusdPair;
    address public immutable _PentaEgcPair;
    address public immutable _PentaShibaPair;
    address public immutable _PentaDogePair;
    address public immutable _PentaAdaPair;
    ...
    constructor() {
        _marketingAddress = payable(0x7A720ad9d3bb6e7543D8607B0ec026ea823cfD3e);
        _Rewards(_marketingAddress, 10**15 * 10**decimals());
        ...
        _PentaEtherPair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(
            address(this),
            uniswapV2Router.WETH()
        );
        _PentaBusdPair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(
            address(this),
            BUSD
        );
        _PentaEgcPair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(
            address(this),
            EGC
        );
        _PentaShibaPair = IUniswapV2Factory(uniswapV2Router.factory())
            .createPair(address(this), SHIBA);
        _PentaDogePair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(
            address(this),
            DOGE
        );
        _PentaAdaPair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(
            address(this),
            ADA
        );
    ...
    function setReflectionFee(uint256 reflectionFee) external onlyOwner {
        ...
        require(taxFee <= TAX_UPPER_LIMIT, "Penta: transfer tax exceeds limit");
    ...
    function swapTokens(uint256 tokenBalance) private lockTheSwap {
        ...
        if (reflectionBalance.div(5) > 0) {
            ...
            swapTokensForTokens(reflectionBalance.div(5), SHIBA);
            swapTokensForTokens(reflectionBalance.div(5), DOGE);
            swapTokensForTokens(reflectionBalance.div(5), ADA);
        }
    }
