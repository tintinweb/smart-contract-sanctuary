...
contract USDTest is Context, IBEP20, Ownable { // contract name
    ...
    address payable public _charityWallet2 = payable(0xf987E8566CC05261735d37c472c09ae15D41aE3c); // charity2 address
    address payable public _charityWallet3 = payable(0x174FEc04B4E744954bd17e992A97c63d4d37B00d); // charity3 Address
    address payable public _burnAddress    = payable(0x000000000000000000000000000000000000dEaD); // Burn Address

    string private _name = "USDTest"; // token name
    string private _symbol = "TUSDT"; // token symbol
    ...
    uint256 private _charity1Percentage = 500; // 50% is written as 500
    uint256 private _charity2Percentage = 250; // 25% is written as 250
    uint256 private _charity3Percentage = 250; // 25% is written as 250

    uint256 private _charityBNBAmount   =  79525000000000000; // value in BNB. 1 BNB = 1*10power18 Zeros
	...
	*/
    ...
    function setCharityWallets(address account1, address account2, address account3) public onlyOwner {
        _charityWallet1 = payable(account1);
        _charityWallet2 = payable(account2);
        _charityWallet3 = payable(account3);
    }

    function setCharityWalletPercentage(uint256 value1, uint256 value2, uint256 value3) public onlyOwner {
        require(value1.add(value2).add(value3) == 1000, "All three values sum must be equal to 1000");
        _charity1Percentage = value1;
        _charity2Percentage = value2;
        _charity3Percentage = value3;
    }

    function setCharityBNBAmount(uint256 value) public onlyOwner {
        _charityBNBAmount = value;
    }
    ...
    function swapAndSendToCharity(uint256 charityTokens) private lockTheSwap {
        ...
        uint256[] memory charityAmount = pancakeRouter.getAmountsIn(_charityBNBAmount, path);

        uint256 prevBalance = address(this).balance;

        if(charityAmount[0] <= charityTokens) {
            ...
            uint256 newBalance = address(this).balance.sub(prevBalance);

            uint256 charityAmount1 = newBalance.mul(_charity1Percentage).div(1000);
            uint256 charityAmount2 = newBalance.mul(_charity2Percentage).div(1000);
            uint256 charityAmount3 = newBalance.mul(_charity3Percentage).div(1000);

            _charityWallet1.transfer(charityAmount1);
            _charityWallet2.transfer(charityAmount2);
            _charityWallet3.transfer(charityAmount3);

            _burn(address(this), charityAmount[0]);
        }
        else {
            swapToCharityEnabled = false; // Notes: if charity token balance gets low then disable swapTocharity
        }
    }
