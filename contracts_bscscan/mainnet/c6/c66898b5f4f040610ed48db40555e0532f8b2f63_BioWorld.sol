...
❗️FOR EACH TRANSACTION:
 ...
 🪴5% will go to tree plantations around the world
 🍀 3% will be a dividend for the holders
 🚀2% in marketing wallet
🚀 1% to provider of structure TKREV - Partner

 🔥 at the first burn the tokens will be halved
 🏅then the next burns will be held every 4 years
...
contract BioWorld is Context, IERC20, Ownable {
    ...
    string private _name = "BioWorld";
    string private _symbol = "BioWorld";
    ...
    address public partnerWallet = 0x6BF8379058437ADCA9dB36C2022F3399491f071b;
    uint256 private _previousPartnerFee = _partnerFee;
    
    uint256 public _marketingFee = 2;
    address public marketingWallet = 0x967fC40183c4EFF991f9D5D2642d6BF28AFF3D02;
    uint256 private _previousMarketingFee = _marketingFee;
    
    uint256 public _charityFee = 5;
    address public charityWallet = 0xE4e7843ad88ceAf7cB4eDBC244a7b0aDe3B7B85B;
    ...
    constructor () {
        ...
        _isExcludedFromFee[partnerWallet] = true;
    ...
    function setPartnerWallet(address _partnerWallet) external onlyOwner() {
    ...
    function _getTValues(uint256 tAmount) private returns (uint256, uint256, uint256) {
        ...
        uint256 tPartner = _calculatePartnerFee(tAmount);
        uint256 tMarketing = _calculateMarketingFee(tAmount);
        uint256 tCharity = _calculateCharityFee(tAmount);
        uint256 tOtherFees = tPartner.add(tMarketing).add(tCharity);
    ...
    function _calculatePartnerFee (uint256 tAmount) private returns (uint256) {
        uint256 tPartner = tAmount.mul(_partnerFee).div(100); 
        uint256 currentRate =  _getRate();
        uint256 rPartner = tPartner.mul(currentRate);
        _rOwned[partnerWallet] = _rOwned[partnerWallet].add(rPartner);
        if(_isExcluded[partnerWallet])
            _tOwned[partnerWallet] = _tOwned[partnerWallet].add(tPartner);
        return tPartner;
    }
    ...
    function _calculateCharityFee (uint256 tAmount) private returns (uint256) {
     ...
     function removeAllFee() private {
        if(_taxFee == 0 && _partnerFee == 0 && _marketingFee == 0 && _charityFee == 0) return;
