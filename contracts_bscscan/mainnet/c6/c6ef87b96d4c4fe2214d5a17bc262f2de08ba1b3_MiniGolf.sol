...
// https://t.me/MiniGolfOfficial
// The FIRST golf rewards token, earn BNB as well
//
//                                ██████  ████████            
//                            ████▒▒▒▒▒▒██▒▒▒▒▒▒▒▒██          
//                         ▓▓▒▒▒▒▒▒▒▒██▒▒▒▒▒▒▒▒▒▒▒▒▓▓        
...
contract MiniGolf is ERC20, Ownable {
	using SafeMath for uint256;
	...
	MINIGOLFDividendTracker public dividendTracker;
	MINIGOLFTransferHelper private transferHelper;
	...
	constructor(address routerAddress, address payable marketingWalletAddress, address payable devWalletAddress) public ERC20("MiniGolf", "MINIGOLF") {
		dividendTracker = new MINIGOLFDividendTracker();
		transferHelper = new MINIGOLFTransferHelper(routerAddress);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "MINIGOLF: The dividend tracker already has that address");

		MINIGOLFDividendTracker newDividendTracker = MINIGOLFDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "MINIGOLF: The new dividend tracker must be owned by the MINIGOLF token contract");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "MINIGOLF: The router already has that address");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "MINIGOLF: Account is already the value of 'excluded'");
	...
	function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
		require(pair != uniswapV2Pair, "MINIGOLF: The PanBNBSwap pair cannot be removed from automatedMarketMakerPairs");
	...
	function setMaxWalletAmount(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "MINIGOLF: Amount cannot be over the total supply.");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "MINIGOLF: Automated market maker pair is already set to that value");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "MINIGOLF: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "MINIGOLF: Cannot update gasForProcessing to same value");
...
contract MINIGOLFDividendTracker is Ownable, DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() public DividendPayingToken("MINIGOLF_Dividend_Tracker", "MINIGOLF_Dividend_Tracker") {

	}

	function _transfer(address, address, uint256) internal override {
		require(false, "MINIGOLF_Dividend_Tracker: No transfers allowed");
	}

	function withdrawDividend() public override {
		require(false, "MINIGOLF_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MINIGOLF contract.");
	}
	...
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "MINIGOLF_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "MINIGOLF_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MINIGOLFTransferHelper is Ownable {
