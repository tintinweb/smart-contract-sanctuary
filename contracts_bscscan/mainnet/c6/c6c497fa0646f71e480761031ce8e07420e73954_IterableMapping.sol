...
contract BNBMASTIFFDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BNBMASTIFF_Dividend_Tracker", "BNBMASTIFF_Dividend_Tracker", 18) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 1 * (10**18);
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "BNBMASTIFF_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BNBMASTIFF_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BNBMASTIFF contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BNBMASTIFF_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BNBMASTIFF_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BNBMASTIFFtest is BEP20, Ownable {
    using SafeMath for uint256;

    IPancakeswapV2Router02 public pancakeswapV2Router;
    address public immutable pancakeswapV2Pair;
    address public marketingWallet = 0x81afba23D493f31bFeFb3e4354D2797d0C6F0038;

    bool private swapping;

    BNBMASTIFFDividendTracker public dividendTracker;
    ...
    constructor() public BEP20("BNBMASTIFFtest", "BNBMASTIFFtest", 18) {
    	...
    	dividendTracker = new BNBMASTIFFDividendTracker();
        ...
        */
        _mint(address(0x8797f1B711AFc5385e3967Ebf080E173472cA5B6), 100000000000000 * (10**18));
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BNBMASTIFF: The dividend tracker already has that address");

        BNBMASTIFFDividendTracker newDividendTracker = BNBMASTIFFDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BNBMASTIFF: The new dividend tracker must be owned by the BNBMASTIFF token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "BNBMASTIFF: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BNBMASTIFF: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "BNBMASTIFF: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BNBMASTIFF: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BNBMASTIFF: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BNBMASTIFF: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BNBMASTIFF: Cannot update gasForProcessing to same value");
