...
contract BabyVault is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0xee11B8c256734EB1636C16E321eBeC837f511f08); // Marketing Address
    ...
    string private _name = "TestBV";
    string private _symbol = "TESTBV";
    uint8 private _decimals = 9;

    uint256 public _taxFee = 25;
    uint256 public TAXFEE_DIV_NUM = 1000;
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 25;
    uint256 public LIQUIDITYFEE_DIV_NUM = 1000;
    uint256 private _previousLiquidityFee = _liquidityFee;
   
    uint256 public sellBuyBackFee = 15;
    uint256 public _previousSellBuyBackFee = 15;
    ...
    event UpdateAccountLastPeriodSellVolume(uint256 oldValue, uint256 newValue);
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BabyVault: This account cannot send tokens until trading is enabled");
        }
        ...
        if (tradingEnabled)
        {
            if (!inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
                ...
                _transferStandard(from, address(this), forBuyBack);
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_taxFee).div(TAXFEE_DIV_NUM);
    }
    
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee).div(LIQUIDITYFEE_DIV_NUM);
    }
    ...
    function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPeriodLimit[account] != excluded, "BabyVault: Account is already the value of 'excluded'");
