...
contract BNBDash is ERC20, Ownable {
    ...
    bool public protectEnabled = true;
    ...
    modifier antiBot(address account) {
        require(
            tradingEnabled || _allowedTransfer[account],
    ...
    BNBDashDividendTracker public dividendTracker;
    ...
    uint256 public totalFees =
        BNBRewardsFee.add(liquidityFee).add(marketingFee).add(devFee);
    ...
    uint256 private constant TOTAL_ATOMS =
        MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);
    uint256 private constant MAX_SUPPLY = ~uint128(0);

    uint256 private _totalSupply;
    uint256 private _atomsPerFragment;
    mapping(address => uint256) public _atomBalances;
    ...
    address marketingWallet = 0x1C1e14FB03f1ed1813db35d5D21336fF4E449e02;
    address devWallet = 0xc637813356b673fE21866B76E9FAfb1cbc88753E;
    ...
    event SendDevWalletWallet(uint256 amount);
    ...
    constructor() public ERC20("BNB Dash", "$BDASH") {
    ...
    /// @notice Withdraw tokens sent by mistake. Impossible to withdraw BNBDash tokens
    /// @param tokenAddress The address of the token to withdraw
    function rescueBEP20Tokens(address tokenAddress) external onlyOwner {
        require(
            tokenAddress != address(this),
            "Impossible to withdraw BNBDash tokens"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "BNBDash: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setAllowedTransfer(address account, bool value)
        external
        onlyOwner
    {
    ...
    /// @dev Update devWallet address. It must be different
    ///   from the current one
    function setDevWallet(address newAddress) external onlyOwner {
        require(devWallet != newAddress, "devWallet already set");
    ...
    function bulkSetBot(address[] calldata bots, bool value)
        external
        onlyOwner
    {
    ...
    function setMaxBuyAndMaxSellDivisor(uint256 newMaxBuy, uint256 newMaxSell)
        external
        onlyOwner
    {
    ...
    function setDividend(bool _enabled) external onlyOwner {
        dividendEnabled = _enabled;
    }

    function setProtect(bool _enabled) external onlyOwner {
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(
            ...
            "BNBDash: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[newPair] = value;
    ...
    function setGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            ...
            "BNBDash: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "BNBDash: Cannot update gasForProcessing to same value"
        );
    ...
    function bulkSend(address[] calldata accounts, uint256[] calldata amounts)
        external
        onlyOwner
    {
        if (amounts.length > 1) {
    ...
    function _transfer(
        ...
        if (automatedMarketMakerPairs[from]) {
            require(
                tradingEnabled || _allowedTransfer[to],
                "Trading not enabled yet"
            );
        } else {
            require(
                tradingEnabled || _allowedTransfer[from],
                "Trading not enabled yet"
            );
        }
        ...
        if (
            ...
            if (coolDownEnabled) {
                require(
                    _lastTrade[from].add(coolDownInterval) <= block.timestamp,
        ...
        if (
            ...
            if (coolDownEnabled) {
                require(
                    _lastTrade[to].add(coolDownInterval) <= block.timestamp,
                    "Cooldown not passed yet"
                );
            }
        ...
        if (
            ...
            revert("to address connot be contract");
        }
        ...
        if (
            ...
            if (liquidityFee > 0 || marketingFee > 0 || devFee > 0) {
                uint256 swapTokens = contractTokenBalance
                    .mul(liquidityFee.add(marketingFee).add(devFee))
                    .div(totalFees);
        ...
        if (takeFee) {
            uint256 fees = startAt + 2 > block.number
                ? amount.mul(99).div(100)
                : amount.mul(totalFees).div(100);
        ...
        if (dividendEnabled) {
            try
                dividendTracker.setBalance(
                    payable(from),
                    INITIAL_FRAGMENTS_SUPPLY.mul(balanceOf(from)).div(
                        _totalSupply
                    )
                )
            {} catch {}
            ...
            try
                dividendTracker.setBalance(
                    payable(to),
                    INITIAL_FRAGMENTS_SUPPLY.mul(balanceOf(to)).div(
                        _totalSupply
                    )
                )
            {} catch {}
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if (marketingWalletAmt > 0) {
            (bool success, ) = payable(marketingWallet).call{
                value: marketingWalletAmt
            }("");
            if (success) emit SendMarketingWallet(marketingWalletAmt);
        }

        // Send BNB to devWallet
        uint256 devWalletAmt = unitBalance * 2 * devFee;
        if (devWalletAmt > 0) {
            (bool success, ) = payable(devWallet).call{value: devWalletAmt}("");
            if (success) emit SendDevWalletWallet(devWalletAmt);
        }
    }
...
contract BNBDashDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        public
        DividendPayingToken(
            "BNBDash_Dividen_Tracker",
            "BNBDash_Dividend_Tracker"
        )
    {
        claimWait = 3600;
    }
    ...
    function _transfer(
        ...
        require(false, "BNBDash_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(
            false,
            "BNBDash_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BNBDash contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "BNBDash_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "BNBDash_Dividend_Tracker: Cannot update claimWait to same value"
        );
