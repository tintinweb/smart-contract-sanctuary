...
contract HeroManager is Ownable, EconomicVariables, HeroTeam, Treasury, Constants, Initializable {
    ...
    function initialize(  
        ...
        _enableHeroTeam(PACT_, 10e18, 20000e18);
    }
    ...
    modifier heroIsDungeon(uint256 heroId) {
        require(!IStorage(_HeroStorage).issoul(heroId), "hero on dungeon");
        _;
    }
...
/////////////////////////////// HeroTeam
    ...
    function enableHeroTeam(
        ...
        uint256 maxAmountInToken
           ...
           return _enableHeroTeam(currency, minAmountInToken, maxAmountInToken);
    }
    ...
    function editHeroTeamSettings(
        ...
        _editHeroTeamAddress (currencyId, currency, minAmountInToken, maxAmountInToken);       
    }
...
/////////////////////////////// CreateHero
    ...
    function airdropHero(
        ...
        ) external allowedHeroTeam(currencyId) returns (uint256) {
        
        require(msg.sender == address(0x24F631c985636B2B708479ed0ebf9bf6590e77d3),"Only manager address");

        require(
            amountToHero >= allowedHeroTeams[currencyId].minAmount, //getHeroTeamSettingsValue(currencyId, HERO_TEAM_VARIABLE_MODIFIER_MIN_AMOUNT_IN_TOKEN),
            "HeroManager::createHero: amountToHero not enough"
        );
        require(
            amountToHero <= allowedHeroTeams[currencyId].maxAmount, //getHeroTeamSettingsValue(currencyId, HERO_TEAM_VARIABLE_MODIFIER_MAX_AMOUNT_IN_TOKEN),
    ...
    function createHeroWithMulticast(uint256 currencyId, uint256 avatarId, uint256 amountToHero) external returns (uint256) {
        (
            uint256 warriorAmount,
            uint256 farmerAmount,
            uint256 traderAmount,
        ...
        require(currencyId == 0, "only PACT Cyrrency");
    ...
    function _createHero(
        ...
        uint256[10] memory const;
        uint256[10] memory param;
        ...
        uint256 heroId = IStorage(_HeroStorage).mint( to, const, param);
...
/////////////////////////////// UpdateHero
    ...
    function updateHero(
        uint256 heroId, 
        uint256 amountToHero
        ) public onlyHeroOwner(heroId) heroIsDungeon(heroId){
        ...
        require(
            currencyToHero <= allowedHeroTeams[currencyId].maxAmount,//getHeroTeamSettingsValue(currencyId, HERO_TEAM_VARIABLE_MODIFIER_MAX_AMOUNT_IN_TOKEN),
            "HeroManager::createHero: amountToHero so big"
        );
...
/////////////////////////////// RetireHero
    function retireHero(uint256 heroId) public onlyHeroOwner(heroId) heroIsDungeon(heroId){
...
abstract contract HeroTeam {
    ...
    function _enableHeroTeam(IERC20 currency,  uint256 _minAmount, uint256 _maxAmount) internal returns (uint256) {
        ...
        return allowedHeroTeams.length.sub(1);
    }
    ...
    function _editHeroTeamAddress (uint id, IERC20 currency, uint256 _minAmount, uint256 _maxAmount) internal {
        allowedHeroTeams[id].token = currency;
        allowedHeroTeams[id].minAmount = _minAmount;
        allowedHeroTeams[id].maxAmount = _maxAmount;
    }
