...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also SHABICAKEed.
...
// File: contracts/SHABICAKEDividendPayingToken.sol
...
contract SHABICAKE is ERC20, Ownable {
    using SafeMath for uint256;
	...
	address payable public MarketingWallet = payable(0x1D294B6E48E00Da2370E9329c2c75cBDc2B57E63); // Marketing Address

    bool private swapping;

    SHABICAKEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SHABICAKE", "SHABICAKE") {
        ...
        dividendTracker = new SHABICAKEDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SHABICAKE: The dividend tracker already has that address");

        SHABICAKEDividendTracker newDividendTracker = SHABICAKEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SHABICAKE: The new dividend tracker must be owned by the SHABICAKE token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SHABICAKE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PancakeSwapV2Pair, "SHABICAKE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SHABICAKE: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SHABICAKE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SHABICAKE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SHABICAKE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!tradingOpen) {
            require(canTransferBeforetradingOpen[from], "SHABICAKE: This account cannot send tokens until trading is enabled");
        }
...
contract SHABICAKEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("SHABICAKE_Dividend_Tracker", "SHABICAKE_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "SHABICAKE_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "SHABICAKE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SHABICAKE_Dividend_Tracker: Cannot update claimWait to same value");
