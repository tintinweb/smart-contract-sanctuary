...
// Zap WETH or ERC20 tokens for UniV2 LP tokens.
contract Zap {
    ...
    function zapInETH(address wantLP, uint256 tokenAmountOutMin)
        external
        payable
    {
        ...
        _swapAndSafeTransfer(wantLP, tokenAmountOutMin, WETH);
    }
    ...
    function zapIn(
        ...
        require(
            ...
            "Zap: Insignificant input amount"
        );
        require(
            IBEP20(tokenIn).allowance(msg.sender, address(this)) >=
                tokenInAmount,
            "Zap: Input token is not approved"
        );

        IBEP20(tokenIn).safeTransferFrom(
            msg.sender,
            address(this),
            tokenInAmount
        );

        _swapAndSafeTransfer(wantLP, tokenAmountOutMin, tokenIn);
    }

    function zapOut(address wantLP, uint256 withdrawAmount) external {
        IUniswapV2Pair pair = _getVaultPair(wantLP);

        IBEP20(wantLP).safeTransferFrom(
            msg.sender,
            address(this),
            withdrawAmount
        );
    ...
    function zapOutAndSwap(
        ...
        require(
            ...
            "Zap: desired token not present in liqudity pair"
        );
        ...
        router.swapExactTokensForTokens(
            IBEP20(swapToken).balanceOf(address(this)),
            desiredTokenOutMin,
            path,
            address(this),
            block.timestamp
    ...
    function _removeLiqudity(address pair, address to) private {
        IBEP20(pair).safeTransfer(pair, IBEP20(pair).balanceOf(address(this)));
    ...
    function _getVaultPair(address wantLP)
        private
        view
        returns (IUniswapV2Pair pair)
    {
        pair = IUniswapV2Pair(wantLP);
        require(
            pair.factory() == router.factory(),
            "Zap: Incompatible liquidity pair factory"
        );
    }

    function _swapAndSafeTransfer(
        address wantLP,
        uint256 tokenAmountOutMin,
        address tokenIn
    ) private {
        ...
        require(
            ...
            "Zap: Liquidity pair reserves too low"
        );

        bool isInputA = pair.token0() == tokenIn;
        require(
            isInputA || pair.token1() == tokenIn,
            "Zap: Input token not present in liqudity pair"
        );
        ...
        uint256 fullInvestment = IBEP20(tokenIn).balanceOf(address(this));
        ...
        IBEP20(wantLP).safeTransfer(
            msg.sender,
            IBEP20(wantLP).balanceOf(address(this))
        );
        _returnAssets(path);
    }
    ...
    function _returnAssets(address[] memory tokens) private {
        ...
        for (uint256 i; i < tokens.length; i++) {
            balance = IBEP20(tokens[i]).balanceOf(address(this));
            if (balance > 0) {
                if (tokens[i] == WETH) {
                    ...
                    require(success, "Zap: WETH like token transfer failed");
                } else {
                    IBEP20(tokens[i]).safeTransfer(msg.sender, balance);
                }
            }
        }
    }
