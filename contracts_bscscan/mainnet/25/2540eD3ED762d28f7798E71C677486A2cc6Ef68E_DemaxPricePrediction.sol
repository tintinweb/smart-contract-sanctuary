...
contract DemaxPricePrediction is Ownable, Pausable, Initializable {
    ...
    function configure(address _oracle, address _operatorAddress) public onlyOwner {
     ...
     * @dev Claim reward
     */
    function _claim(uint256 epoch) internal {
    ...
    function _claimList(uint256 start, uint256 end) internal returns (uint256) {
        if(start > end) {
            start = end;
        }
        uint256 length = userRounds[msg.sender].length;
        require(end <= length, 'invalid paramters');
        uint256 updateIndex;
        for (uint256 i = start; i < end; i++) {
            uint256 epoch = userRounds[msg.sender][i];
            if(
                rounds[epoch].startBlock != 0 
                && block.number > rounds[epoch].endBlock 
                && !ledger[epoch][msg.sender].claimed
            ) {
                if (rounds[epoch].oracleCalled && claimable(epoch, msg.sender)) {
                    _claim(epoch);
    ...
    function claimList(uint256 start, uint256 end) external notContract {
        _claimList(start, end);
    }

    function claimAll() external notContract {
        uint256 updateIndex = _claimList(userClaimIndex[msg.sender], userRounds[msg.sender].length);
        if(updateIndex > 0) {
            userClaimIndex[msg.sender] = updateIndex + 1;
        }
    }
    ...
    function getUserRounds(
        ...
        return (values, cursor + length, userRounds[user].length);
    }

    function getRevestUserRounds(
        address user,
        uint256 cursor,
        uint256 size
    ) external view returns (uint256[] memory, uint256, uint256) {
        ...
        if (userRounds[user].length == 0) {
            length = 0;
            next = 0;
        } else if (cursor >= userRounds[user].length) {
            length = 0;
            next = userRounds[user].length - 1;
        } else if (cursor == 0) {
            length = 1;
            next = 0;
        } else {
        ...
        uint256 index;
        for (uint256 i = 0; i < length; i++) {
            values[index] = userRounds[user][cursor - i];
            index++;
        }

        return (values, next, userRounds[user].length);
    }

    function countUserRound(address user) public view returns (uint256) {
     ...
     * Reset genesis state. Once paused, the rounds would need to be kickstarted by genesis
     */
    function restart() public onlyAdminOrOperator {
        genesisStartOnce = false;
        genesisLockOnce = false;
        emit Restart(currentEpoch);
    }
    /**
    ...
    function _safeTransfer(address to, uint256 value) internal {
        ...
        bytes memory data;
        if(token == address(0)) {
            (success, ) = to.call{gas: 23000, value: value}("");
        } else  {
    ...
    function _safeTransferFrom(address from, address to, uint value) internal {
