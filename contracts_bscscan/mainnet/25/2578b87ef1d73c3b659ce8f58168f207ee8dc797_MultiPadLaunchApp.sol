...
contract MultiPadLaunchApp is IBEP20{
    ...
    */
    ...
    function claimPurchasedTokens(uint256 _lockNumber) external validateClaim(msg.sender,_lockNumber) returns (bool){
        uint256 calculation;
        if(_lockNumber == 1)
        calculation  = (_finalSoldAmount[msg.sender]).mul(34).div(100);
        else
        calculation  = (_finalSoldAmount[msg.sender]).mul(33).div(100);
    ...
    modifier validateClaim(address _userAddress, uint256 _lockNumber)
    {
        ...
        require(_lockNumber == 1 || _lockNumber == 2 || _lockNumber == 3, "Invalid Lock Number");
        ...
        if(_lockNumber == 3){    // 2 month
            require(block.timestamp > saleEndTime + _claimTime1 +  5259486  && reEntrance[_userAddress][_lockNumber] != true , "Insufficient Unlocked Tokens");
        }
        _;
    }
    ...
    function getUserTokensByAdd(address _userAddress) external view returns(uint256 _div1, uint256 _div2, uint256 _div3, uint256 _div4, uint256 _div5){
        _div1 = _finalSoldAmount[_userAddress].mul(34).div(100);
        _div2 = _finalSoldAmount[_userAddress].mul(33).div(100);
        _div3 = _finalSoldAmount[_userAddress].mul(33).div(100);
        _div4 = 0;
        _div5 = 0;
    ...
    function getClaimDates() view external returns(uint256 _d1, uint256 _d2, uint256 _d3, uint256 _d4, uint256 _d5){
        ...
        _d3 = saleEndTime + _claimTime1 +  5259486;
        _d4 = 0;
        _d5 = 0;
