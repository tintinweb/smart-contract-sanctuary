...
contract MuskSwapIco is Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => uint256) public unlockInfos;
     ...
     * callable by admin
     */
    function setIcoContract(address _contractAddress) external onlyOperator {
    ...
    function setPriceFeedAddress(address _priceFeedAddress) external onlyOperator {
    ...
    function receivedAmount(address recipient) external view returns (uint256) {
        if (lockInfos[recipient].amountLocked == 0) {
            ...
            lockedAmount = lockedAmount - unlockInfos[recipient];
            return lockedAmount;
        } else {
            return lockInfos[recipient].amountLocked - unlockInfos[recipient];
        }
    }
    ...
    function unlockAddress(address _address, uint256 _amount) external onlyOperator {
        require(_address != address(0), "Cannot be zero address");
        unlockInfos[_address] = _amount;
    }

    function lockAccount(uint256 _amountLock, address account) external onlyOperator{
        _lockAmountToken(_amountLock, account);
    }
}
