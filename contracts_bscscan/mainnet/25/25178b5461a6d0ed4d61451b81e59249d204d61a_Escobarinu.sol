...
contract EscobarinuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Escobarinu_Dividend_Tracker", "Escobarinu_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Escobarinu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Escobarinu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Test contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Escobarinu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");   
        require(newClaimWait != claimWait, "Escobarinu_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Escobarinu is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    EscobarinuDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 150000000 * (10**18);
    uint256 public maxWalletToken = 450000000 * (10**18);
    ...
    constructor() ERC20("Escobar Inu", "PABLO") {
        BNBRewardsFee = 2;
        liquidityFee = 2;
        extraFeeOnSell = 8; // extra fee on sell
        marketingFee = 4;
        buyBackFee =4; 
        marketingWallet = payable(0x0DC8b329f6DD927B9FC5cA124E0D64988a95269a);
        buyBackWallet = payable(0x4f693e342e5312E4F483a855F67F84ec354929a7); 

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(buyBackFee); // total fee transfer and buy

        dividendTracker = new EscobarinuDividendTracker();
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokensToAddLiquidityWith = contractTokenBalance.mul(liquidityFee).div(totalFees.mul(2));
        ...
        uint256 buyBackAmount = deltaBalance.sub(bnbToAddLiquidityWith).sub(marketingAmount).div(totalFees.sub(liquidityFee).sub(marketingFee)).mul(buyBackFee);
