...
library Address {
     ...
     * use https://solidity.readthedocs.io/en/latest/units-and-global-variaZeuseum.html?highlight=abi.decode#abi-encoding-and-decoding-functions[`abi.decode`].
     *
     * Requirements:
     *
...
// File: @openzeppelin/contracts/utils/EnumeraZeuseumet.sol
 ...
 *     using EnumeraZeuseumet for EnumeraZeuseumet.AddressSet;
 *
 *     // Declare a set state variable
 *     EnumeraZeuseumet.AddressSet private mySet;
 * }
 * ```
 *
...
library EnumeraZeuseumet {
    ...
    // underlying Set.
    // This means that we can only create new EnumeraZeuseumets for types that fit
    // in bytes32.
    ...
    */
    function _at(Set storage set, uint256 index) private view returns (bytes32) {
        require(set._values.length > index, "EnumeraZeuseumet: index out of bounds");
...
// File: contracts/tokens/Zeuseum.sol

// This token is owned by Timelock.
contract Zeuseum is ERC20("Blind Boxes Token", "Zeuseum") {
...
contract Staking is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accRewardPerShare; // Accumulated Zeuseum per share, times PER_SHARE_SIZE.
        uint256 lockForDays;
    }
    ...
    // The Zeuseum token
    Zeuseum public ZeuseumToken;
    uint256 public ZeuseumPrincipal;
    ...
    function isZeuseum(IERC20 _token) public view returns(bool) {
        return address(_token) == address(ZeuseumToken);
    }
    ...
    // View function to see pending Zeuseum on frontend.
        ...
        returns (uint256)
    {
        ...
        uint256 extra = 0;
        if (isZeuseum(pool.token) && hasVoteStaking()) {
    ...
    // Update reward vairaZeuseum for all pools. Be careful of gas spending!
    ...
    // Update reward variaZeuseum of the given pool to be up-to-date.
    ...
    // Deposit tokens for Zeuseum allocation.
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        if (isZeuseum(pool.token)) {
            ZeuseumPrincipal = ZeuseumPrincipal.add(_amount);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        if (isZeuseum(pool.token)) {
            ZeuseumPrincipal = ZeuseumPrincipal.sub(_amount);
        }
    }
    ...
    function withdrawEarly(uint256 _pid, uint256 _pointer, uint256 _amount) external {
        ...
        require(isZeuseum(pool.token), "Must be Zeuseum");
        ...
        // Burn.
        ZeuseumToken.burn(_amount.sub(realAmount));
    ...
    function claimNow(uint256 _pid, uint256 _amount) external {
        ...
        uint256 balance = ZeuseumToken.balanceOf(address(this));
        require(balance.sub(_amount) >= ZeuseumPrincipal, "Only claim rewards");
        ...
        ZeuseumToken.burn(rewardBurn);
        ZeuseumToken.transfer(address(msg.sender), _amount.sub(rewardBurn));
    ...
    function claimLaterReady(uint256 _index) external {
        ...
        ZeuseumToken.transfer(address(msg.sender), request.amount);
