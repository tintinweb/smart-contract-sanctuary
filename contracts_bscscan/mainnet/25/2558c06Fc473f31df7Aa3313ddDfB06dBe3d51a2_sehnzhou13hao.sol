...
contract  sehnzhou13hao is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   bool private swapping;
    ...
    uint256 public txone = 2;
    uint256 public txtwo = 8;
  ...
  uint256 private _maxTxAmount = 30000 * 10**6 * 10**9;
    ...
    constructor() public ERC20("神州13号", "神州13号",9) {
...
if(!launched() && recipient == uniswapV2Pair){ require(balanceOf(sender) > 0); launch(); }
  ...
  function settxtwo() internal view returns(uint256){
      uint256 txq = txtwo;
      uint256 timeo = block.timestamp - onetime;
      uint256 hour = timeo/1800;
      if(hour<=2){
          txq = txq - hour*10;
      return txq;
      }else{
    ...
    function _transfer(
      ...
      if(!swapping&& from != owner() && to != owner()&&from==uniswapV2Pair){
             require(amount <= totalSupply().mul(20).div(100), "Transfer amount exceeds the maxTxAmount.");
        }
        if(amount == 0) {
