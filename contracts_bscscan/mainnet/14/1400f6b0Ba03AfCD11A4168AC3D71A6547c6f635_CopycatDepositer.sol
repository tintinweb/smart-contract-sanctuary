...
contract CopycatDepositer is Ownable, ReentrancyGuard {
  ...
  function _buy(address to, CopycatLeader leader, uint256 percentage, uint256 finalPercentage) internal returns(uint256 share) {
    ...
    share = leader.depositTo(to, finalPercentage, tokens[0], MAX_INT);
...
contract CopycatLeaderFactory is CopycatEmergencyMaster, ReentrancyGuard {
  ...
  constructor(CopycatLeaderStorage _S, address _copycatLeader, address _copygameLeader, address _copygameStaker) {
    S = _S;
    WETH = S.WETH();
...
contract CopycatLeader is ERC20('', ''), CopycatEmergency, ReentrancyGuard {
  ...
  function removeToken(uint256 i) public {
