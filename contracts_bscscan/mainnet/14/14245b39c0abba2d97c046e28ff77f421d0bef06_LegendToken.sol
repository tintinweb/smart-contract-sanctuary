...
contract LGEDERC20 is Ownable, ERC20 {
    using SafeMath for uint256;

    uint256 public tokenForLegend = 500000 * 10**18;

    address public addressForLegend;
    uint256 public sellFeeRate = 5;
    uint256 public buyFeeRate = 0;

    constructor(string memory name, string memory symbol) ERC20(name, symbol) {
        addressForLegend = _msgSender();
    }
    ...
    function setMinTokensBeforeSwap(uint256 _tokenForLegend)
        public
        onlyOwner
    {
        require(_tokenForLegend < 20 * 10**6 * 10**18);
        tokenForLegend = _tokenForLegend;
    }
}
...
contract LegendToken is LGEDERC20, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function sweepTokenForLegend() public nonReentrant {
        uint256 contractTokenBalance = balanceOf(address(this));
        if (contractTokenBalance >= tokenForLegend) {
            swapTokensForEth(tokenForLegend);
        }
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            path,
            addressForLegend, // The contract
            block.timestamp
        );
    }

    function setAddressForLegend(address _addressForLegend) external onlyOwner {
        require(_addressForLegend != address(0), "0x is not accepted here");

        addressForLegend = _addressForLegend;
    }
