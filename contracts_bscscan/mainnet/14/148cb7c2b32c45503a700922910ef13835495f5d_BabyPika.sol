...
| $$  \ $$| $$  | $$| $$  \ $$    | $$    | $$        | $$  | $$\  $$ | $$  | $$
| $$$$$$$/| $$  | $$| $$$$$$$/    | $$    | $$       /$$$$$$| $$ \  $$| $$  | $$
|_______/ |__/  |__/|_______/     |__/    |__/      |______/|__/  \__/|__/  |__/

Welcome To the BabyPika Community!
 
One fine day, while scrolling BSC tokens Pikachu found that none of the Projects have the potential or the legitimacy that Pika desired. Tired of all the exorbitant high tax and rug tokens, our sweet Pikachu decided to create one that is just as cute as him, and named it BABYPIKA.
 
No more absurd Tokenomics with high robbery taxation, we are bringing BSC back 
to tokenomics that benefits the community and investors, while rewarding them with huge amounts of BNB at the same time. Wait, there's more!
 
Along with massive rewards distributed to holders every 60 minutes, a NFT platform is also in the works, the beta for which releases after the presale.

1T TOTAL SUPPLY

Buy Fee 15%:
 
💪  8% BNB Rewards sent every hour automatically to holders
...
💪  9.6%% BNB Rewards sent every hour automatically to holders

💧   1.2% Auto Liquidity

✅  2.4%  Pika Thunderbolt
 
🚀  3.6% Marketing

🃏  1.2% NFTs holders

Website: https://www.babypika.org/
TG: https://t.me/babypikabsc
Twitter:https://twitter.com/BabyPikaBsc
...
abstract contract BabyPikaBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
    ...
    uint8 private _NFTsHoldersFee; //% of each transaction that will be used for NFTsHolders BNB pool
    ...
    // marketing and NFTsHolders address
    address private _marketingWallet = 0xdc3E52ff9f209d3f64c38Fb8c0BB63c7fAEb43e7;
    address private _NFTsHoldersAddress = 0x652fB063C30a1bB6D6a50eBb75ae13bB6a6C7b82;
	...
	constructor (address routerAddress) {
		...
		// 1% liquidity fee, 12% reward fee, 3% buyback fee, 2% marketing fee, 1% NFTsHolders fee, 6% additional sell fee
		setFees(1, 8, 2, 3, 1, 3);
	...
	function executeSwap(uint256 amount) private {
		...
		uint256 tokensReservedForNFTsHolders = amount * _NFTsHoldersFee / _poolFee;
		uint256 tokensReservedForReward = amount - tokensReservedForLiquidity - tokensReservedForBuyback - tokensReservedForMarketing - tokensReservedForNFTsHolders;
		...
		uint256 tokensToSwap = tokensReservedForReward + tokensToSwapForLiquidity + tokensReservedForBuyback + tokensReservedForMarketing + tokensReservedForNFTsHolders;
		...
		//send bnb to NFTsHolders BNB pool
		uint256 bnbToBeSendToNFTsHolders = bnbSwapped * tokensReservedForNFTsHolders / tokensToSwap;
		(sent, ) = _NFTsHoldersAddress.call{value: bnbToBeSendToNFTsHolders}("");
		require(sent, "Failed to send BNB to NFTsHolders BNB pool");
	...
	// This function swaps a {tokenAmount} of BABYPIKA tokens for BNB and returns the total amount of BNB received
	...
	// Returns how many more $BABYPIKA tokens are needed in the contract before triggering a swap
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 buybackFee, uint8 marketingFee, uint8 NFTsHoldersFee, uint8 additionalSellFee) public onlyOwner {
        ...
        require(NFTsHoldersFee >= 1 && NFTsHoldersFee <= 5, "NFTsHolders fee must be between 1% and 5%");
        ...
        _NFTsHoldersFee = NFTsHoldersFee;
		_additionalSellFee = additionalSellFee;
		
		// Enforce invariant
		_poolFee = _rewardFee + _liquidityFee + _buybackFee + _marketingFee + _NFTsHoldersFee;
	}
...
contract BabyPika is BabyPikaBase {
	...
	uint256 private _totalBNBAsBABYPIKAClaimed; // The total number of BNB that was converted to BABYPIKA and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsBABYPIKAClaimed; // The amount of BNB converted to BABYPIKA and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy BABYPIKA automatically
	...
	bool private _reimburseAfterBABYPIKAClaimFailure; // If true, and BABYPIKA reward claim portion fails, the portion will be given as BNB instead
	...
	function onBeforeTransfer(address sender, address recipient, uint256 amount) internal override {
		...
		bool isSelling = isPancakeSwapPair(recipient);
		if (!isSelling) {
			// Wait for a dip, BabyPika diamond hands
			return;
		}
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough BABYPIKA balance");
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $BABYPIKA on a zero balance address:
		...
		// Depending on the % of $BABYPIKA tokens transferred, relative to the balance, we might need to extend the period
