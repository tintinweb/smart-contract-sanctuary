...
contract UFFInvestmentPlan is IInvestmentPlan, PancakeSwapHelper, EmergencyWithdrawable {
    ...
    event Withdrawn(uint256 amount, uint256 bnbAmount);
    event Staked(uint256 amount);

    //Token: 0x3993a8f82f5e1a5381e678fc237a3da668c1f4eb, master chef: 0xB3596634F39f39A803f20D40E0a4E9BE1e5E2073
    ...
    function stakeUFFBalance() external onlyAdmins {
    ...
    function withdrawAll() external onlyAdmins {
        masterChef.withdraw(0, totalStaked);
        delete totalStaked;
    }

    function pendingProfits() public view returns(uint256) {
        return masterChef.pendingUff(0, address(this));
    }

    function setOptions(IBEP20 _token, IMasterChef _masterChef) public onlyOwner {
        require(address(_masterChef) != address(0), "UFFInvestmentPlan: Invalid address");
        require(address(_token) != address(0), "UFFInvestmentPlan: Invalid address");
    ...
    function setProfitsDestination(address destination) public onlyOwner {
        require(destination != address(0), "UFFInvestmentPlan: Invalid address");
        profitsDestination = destination;
    }


    function doStake(uint256 amount) internal {
        masterChef.deposit(0, amount, referrer);
        totalStaked += amount;
        emit Staked(amount);
    }

    function doWithdrawProfits() private {
        require(profitsDestination != address(0), "UFFInvestmentPlan: Not enabled");
