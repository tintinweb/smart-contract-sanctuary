...
contract Luckybaby is Context, IERC20, Ownable, Pausable 
{
    ...
    uint256 public _founderFee = marketingDivisor.add(charityDivisor);
    uint256 private _previousfounderFee = _founderFee;

    address payable public marketingWallet =  payable(0x1FFA244830ac826e0e99a76300321acB65816600);
    address payable public charityWallet =    payable(0x46fBC2B7EB7695eEe4dB9019200a7E12b2Ca2030);
    ...
    uint256 public _maxTxAmount = _tTotal.div(200); //0.5%
    uint256 private numTokensSellToAddToLiquidity = _tTotal.div(1000000);
    uint256 public maxLimit = _maxTxAmount = _tTotal.div(100); //1%;
    ...
    constructor () 
    {
        ...
        //excluded from free here. 
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[marketingWallet] = true;
        _isExcludedFromFee[charityWallet] = true;
        
        // excluded from whales(having max tokens limit)
    ...
    function setAllFees(uint256 taxFee, uint256 liquidityFee, uint256 burnFee) external onlyOwner() 
    {
        _taxFee = taxFee;
    ...
    {  
        ...
        return totalAmount.mul(_liquidityFee.add(_founderFee)).div(100);
    }
    ...
    {
        uint256 allFee = _liquidityFee.add(_founderFee);
        ...
        marketingWallet.transfer(newBalance.div(_founderFee).mul(marketingDivisor));
        charityWallet.transfer(newBalance.div(_founderFee).mul(charityDivisor));
    }
    ...
    function setAllWallets(address marketing, address charity) external onlyOwner() 
    {
    ...
    function  setWalletsDivisors(uint256 marketing, uint256 charity) public onlyOwner 
    {
        marketingDivisor = marketing;
        charityDivisor = charity;
        _founderFee = marketingDivisor.add(charityDivisor);
