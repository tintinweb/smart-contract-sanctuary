...
contract SupremeCheems is IBEP20, Auth {

	address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

	string constant _name = "SupremeCheems";
    string constant _symbol = "SUP";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 6900000 * (10 ** _decimals);
    uint256 private _maxTxAmountBuy = _totalSupply / 100;
    uint256 private _maxTxAmountSell = _totalSupply / 200;
	uint256 private _maxWalletAmount = _totalSupply / 100;
    uint256 public _maxBuyToken = 69000;
    uint256 public _maxSellToken = 34500;
    uint256 public _maxWalletToken = 69000;
    ...
    address public marketingWallet = payable(0xc8A4333F4CcA1b2dA984F0E6E7d98234BAec1359);
    address private teamWallet = payable(0x541ec104b4811A8636804468F2ddf677D41d5355);
    address private devWallet = payable(0xe6497e1F2C5418978D5fC2cD32AA23315E7a41Fb);
	...
	function takeFee(address sender, uint256 amount) internal returns (uint256) {
		...
		if (block.number - launchedAt <= antiSniperBlocks || gasLimitActive && tx.gasprice >= antiSniperGasLimit) {
			...
			if (totalFees > 0) {
				marketingtokens = amount * totalFees / feeDenominator;
                if(isSell){
                    marketingtokens = amount * sellTax / feeDenominator;
                }
                if(isJeet){
                    marketingtokens = amount * jeetTax / feeDenominator;
                }
				_balances[address(this)] += marketingtokens;
				emit Transfer(sender, address(this), marketingtokens);
			}
	...
	function liquify() internal swapping {
        ...
        uint256 onePercentOfTax = amountBNB * 2 / 17;
        uint256 twoPercentOfTax = onePercentOfTax * 2;
        
        payable(devWallet).transfer(onePercentOfTax);
        payable(teamWallet).transfer(twoPercentOfTax);
	...
	function setLimits(uint256 maxwall, uint256 maxsell, uint256 maxbuy, uint256 sellCoolDown) external authorized {
		_maxWalletAmount = maxwall * _totalSupply / 1000;
        _maxTxAmountSell = maxsell * _totalSupply / 1000;
        _maxTxAmountBuy = maxbuy * _totalSupply / 1000;
        _maxBuyToken = 6900000 * maxbuy / 1000;
        _maxSellToken = 6900000 * maxsell / 1000;
        _maxWalletToken = 6900000 * maxwall / 1000;
        _coolDown = sellCoolDown;
	}
    ...
    function setFees(uint256 _totalFees, uint256 _feeDenominator, uint256 _sellTax, uint256 _jeetTax, uint256 _jeetBlocks, uint256 _percentOfTaxToMarketing) external authorized {
        ...
        require(totalFees <= feeDenominator / 5, "Maximum allowed buyTax is 20%.");
        require(sellTax <= feeDenominator / 3, "Maximum allowed sellTax is 33%.");
        require(jeetTax <= feeDenominator / 2, "Maximum allowed jeetTax is 50%.");
    }
