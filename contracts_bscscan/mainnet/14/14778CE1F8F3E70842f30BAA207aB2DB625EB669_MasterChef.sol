...
contract CryptoLifeToken is BEP20('CryptoLife Token', 'LIFE') {
...
// MasterChef is the master of CryptoLife. He can make CryptoLife and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once CryptoLife is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of CryptoLifes
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. CryptoLifes to distribute per block.
        uint256 lastRewardBlock;  // Last block number that CryptoLifes distribution occurs.
        uint256 accLifePerShare;   // Accumulated Lifes per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The LIFE TOKEN!
    CryptoLifeToken public life;
    // Dev address.
    address public devaddr;
    // CryptoLifeToken tokens created per block.
    uint256 public lifePerBlock;
    // Bonus muliplier for early CryptoLifeToken makers.
    ...
    // The block number when CryptoLifeToken mining starts.
    ...
    constructor(
        CryptoLifeToken _life,
        address _devaddr,
        address _feeAddress,
        uint256 _lifePerBlock,
        uint256 _startBlock
    ) public {
        life = _life;
        devaddr = _devaddr;
    ...
    // Update the given pool's cCryptoLifeToken allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending CryptoLifeToken on frontend.
    function pendingLife(uint256 _pid, address _user) external view returns (uint256) {
    ...
    // Deposit LP tokens to MasterChef for cryptoLife allocation.
