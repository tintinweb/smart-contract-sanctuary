...
contract SatoToken is ERC721, ReentrancyGuard, Ownable {
    ...
    ) external nonReentrant returns (uint256)
    {
        ...
        emit MintNewToken(newItemId, _msgSender(), _tokenURI, count, price);
        return newItemId;
    }
    ...
    function buyNFT(
        ...
        uint256 _tokenPrice = tokenOwners[tokenId][owner].tokenPrice;
        uint256 totalPrice = _tokenPrice * count;
        ...
        if (buyerInfo.tokenCount == 0) {
            buyerInfo.tokenPrice = _tokenPrice;
        }
        ...
        (bool sent, ) = payable(owner).call{value: soldValue}("");
        require(sent, "Failed to send payment");
        // payable(owner).send(soldValue);

        console.log("buyNFT() -- 1 : ", address(this).balance);

        emit BuyToken(tokenId, owner, _msgSender(), _tokenPrice);
    }
