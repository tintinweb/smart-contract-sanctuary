...
contract ArtOnlinePlatformStorage is AccessControl {
  ...
  mapping(uint256 => mapping(uint256 => mapping(address => uint256))) internal _bonuses;
  ...
  event ActivateItem(address indexed account, uint256 id, uint256 tokenId);
  event DeactivateItem(address indexed account, uint256 id, uint256 tokenId);
...
contract ArtOnlinePlatform is Context, ERC165, EIP712Base, IERC1155, IERC1155MetadataURI, Pausable, ArtOnlinePlatformStorage {
  ...
  function _beforeTokenTransfer(
    ...
    function activateItem(uint256 id, uint256 tokenId) public whenNotPaused isWhiteListed(msg.sender) {
      address account = msg.sender;
      _beforeAction(account, id, tokenId);
      if (_miners[id][account] > 0) {
        getReward(id);
      }
      _activateItem(account, id, tokenId);
      emit ActivateItem(account, id, tokenId);
    }

    function deactivateItem(uint256 id, uint256 tokenId) public whenNotPaused isWhiteListed(msg.sender) {
      address account = msg.sender;
      _beforeAction(account, id, tokenId);
      if (_miners[id][account] > 0) {
        getReward(id);
      }
      _deactivateItem(account, id, tokenId);
      emit DeactivateItem(account, id, tokenId);
    }
    ...
    function _itemBonuses(address sender, uint256 id) internal view returns (uint256) {
      ...
      for (uint256 i = 0; i < _items.length; i++) {
        ...
        if (_bonuses[id][itemId][sender] > 0) {
          percentage = percentage + (_maxReward[itemId] * _bonuses[id][itemId][sender]);
        }
    ...
    function _activateItem(address account, uint256 id, uint256 itemId) internal {
      unchecked {
        _bonuses[id][itemId][account] += 1;
      }
      _checkHalving(id);
      _rewardPerGPU(id);
    }

    function _deactivateItem(address account, uint256 id, uint256 itemId) internal {
      unchecked {
        _bonuses[id][itemId][account] -= 1;
      }
      _checkHalving(id);
