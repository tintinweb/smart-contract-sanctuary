...
contract ADAMoonsHORT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ADAMoonsHORTDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("ADAMoonsHORT", "ADAMP") {
        _ETHRewardsFee = 8;
        _liquidityFee = 4;
		_marketingFee = 3;
    	...
    	dividendTracker = new ADAMoonsHORTDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
		bulkSender = payable(0x362E7fEc1623817E5165079501B4A3532d1Fc013);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ADAMoonsHORT: The dividend tracker already has that address");

        ADAMoonsHORTDividendTracker newDividendTracker = ADAMoonsHORTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ADAMoonsHORT: The new dividend tracker must be owned by the ADAMoonsHORT token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ADAMoonsHORT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ADAMoonsHORT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ADAMoonsHORT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ADAMoonsHORT: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ADAMoonsHORT: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ADAMoonsHORT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ADAMoonsHORT: Cannot update gasForProcessing to same value");
...
contract ADAMoonsHORTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ADAMoonsHORTDividend_Tracker", "ADAMoonsHORT_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "ADAMoonsHORT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ADAMoonsHORTDividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ADAMoonsHORT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "ADAMoonsHORT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ADAMoonsHORT_Dividend_Tracker: Cannot update claimWait to same value");
