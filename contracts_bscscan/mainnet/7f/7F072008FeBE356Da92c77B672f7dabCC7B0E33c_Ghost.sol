...
contract Ghost is IBEP20, Auth {
    ...
    function isDiamondHand(address holder) external view returns(bool){
        return neverSold[holder];
    }
    ...
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        ...
        if (msg.sender != pair && recipient != pair && !_isWL[msg.sender]){
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (block.timestamp > launchTime + 5 seconds && block.timestamp < launchTime + 10 seconds){
            require(tx.gasprice <= 500000000000, "Gas price above limit");
        }
