...
import "./oracle/ChainlinkBNBUSDPriceConsumer.sol";
import "../../governance/AccessControl.sol";

// gu => usd
// anchor : Anchor the object
// synth : Synthetic on-chain assets
contract GuStablecoin is ERC20Custom, AccessControl, Owned {
    ...
    ChainlinkBNBUSDPriceConsumer private bnb_anchor_pricer;
    uint8 private bnb_anchor_pricer_decimals;

    UniswapPairOracle private guBnbOracle;
    UniswapPairOracle private grBnbOracle;
    ...
    uint256 public constant genesis_supply = 2000000e18; // 2M Gu (only for testing, genesis supply will be 5k on Mainnet). This is to help with establishing the Uniswap pools, as they need liquidity
    ...
    function globalCollateralValue() public view returns (uint256) {
        ...
        for (uint i = 0; i < synth_pools_array.length; i++){
            ...
            if (synth_pools_array[i] != address(0)){
                total_collateral_value_d18 = total_collateral_value_d18.add(Pool(synth_pools_array[i]).collatAnchorBalance());
            }
    ...
    function refreshCollateralRatio() public {
        ...
        if (gu_price_cur > price_target.add(price_band)) { //decrease collateral ratio
            if(global_collateral_ratio <= 500000){ //if within a step of 50%, go to 50%
                global_collateral_ratio = 500000;
            } else {
    ...
    function setBNBAnchorOracle(address _bnb_anchor_consumer_address) public onlyByOwnerGovernanceOrController {
        ...
        bnb_anchor_pricer = ChainlinkBNBUSDPriceConsumer(bnb_anchor_consumer_address);
    ...
    function setSynthBnbOracle(address gu_oracle_addr, address _wbnb_address) public onlyByOwnerGovernanceOrController {
        ...
        guBnbOracle = UniswapPairOracle(gu_oracle_addr);
    ...
    function setGrBnbOracle(address _gr_oracle_addr, address _wbnb_address) public onlyByOwnerGovernanceOrController {
        ...
        grBnbOracle = UniswapPairOracle(_gr_oracle_addr);
        wbnb_address = _wbnb_address;
