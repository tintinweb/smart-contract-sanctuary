...
contract preSaleLove{
    ...
    constructor() public {
        ...
        token = IBEP20(0x5114B60d0ec980DCC429f85665614C256b5f9C52);
        priceFeedBnb = AggregatorV3Interface(0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE);
        tokenPerUsd = 100;
        minAmount = 0.05 ether;
        maxAmount = 5 ether;
        maxSell = 50000000 ether;
        preSaleTime = block.timestamp + 50 days;
        reffralpercent = 10;
    ...
    function buyTokenLove(address _referrer) payable public {
        ...
        require(numberOfTokens.mul(1e18).add(token.balanceOf(msg.sender)) <= maxToken.mul(1e18),"BEP20: Amount exceeded max limit");
        require(block.timestamp < preSaleTime,"BEP20: PreSale over");
        require(soldToken.add(numberOfTokens.mul(1e18)) <= maxSell,"BEP20: Amount exceed preSale limit");
        
        
        token.transferFrom(owner, msg.sender, numberOfTokens.mul(1e18));
        token.transferFrom(owner,_referrer,numberOfTokens.mul(reffralpercent).mul(1e18).div(100));
