...
contract DividendTracker is DividendPayingToken {
    ...
    constructor()
        DividendPayingToken("SRT_Dividend_Tracker", "SRT_Dividend_Tracker", 18)
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100000000 * 1e18; //must hold 100,000,000+ tokens to get divs
    }
    ...
    function _transfer(
        ...
        require(false, "SRT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SRT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BNB Shinobi contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "SRT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "SRT_Dividend_Tracker: Cannot update claimWait to same value"
        );
...
contract SmartRewardsToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public burningFee = 2;
    uint256 public rewardsFee = 3;
    uint256 public marketingFee = 2;
    uint256 public totalFees =
        liquidityFee.add(burningFee).add(rewardsFee).add(marketingFee);
    ...
    constructor() ERC20("Smart Rewards Token", "SRT", 18) {
        address newOwner = 0x30EBA87e38B249055FCDd4aA76D034113f550F6f;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "SRT: The dividend tracker already has that address"
        );
        ...
        require(
            ...
            "SRT: The new dividend tracker must be owned by the SmartRewardsToken token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(
            ...
            "SRT: The router already has that address"
        );
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "SRT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function updateFees(
        ...
        burningFee = burningPerc;
        marketingFee = marketingPerc;
        
        totalFees = liquidityFee.add(burningFee).add(rewardsFee).add(marketingFee);
        emit FeesUpdated(rewardPerc, liquidityPerc, burningPerc, marketingPerc);
    }
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            ...
            "SRT: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "SRT: Cannot update gasForProcessing to same value"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "SRT: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 burnTokens = contractTokenBalance.mul(burningFee).div(
                totalFees
            );
            super._transfer(address(this), DEAD, burnTokens);

            uint256 marketTokens = contractTokenBalance.mul(marketingFee).div(
                totalFees
            );
            super._transfer(address(this), owner(), marketTokens);
