...
contract BINANCE8 is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "BINANCE8";
    string constant _symbol = "B8";
    ...
    function triggerManualBuyback(uint256 amount) external onlyOwner {
        buyBINANCE8Token(amount, DEAD);
    }

    function triggerAutoBuyback() internal {
        buyBINANCE8Token(autoBuybackAmount, DEAD);
        autoBuybackBlockLast = block.number;
        autoBuybackAccumulator = autoBuybackAccumulator.add(autoBuybackAmount);
        if(autoBuybackAccumulator > autoBuybackCap){ autoBuybackEnabled = false; }
    }

    function buyBINANCE8Token(uint256 amount, address to) internal swapping {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(this);
