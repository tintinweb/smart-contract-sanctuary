...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock;  // Last block number that Onyx distribution occurs.
    ...
    // View function to see pending Onyx on frontend.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 OnyxReward = multiplier.mul(OnyxPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        oxc.mint(devaddr, OnyxReward.div(10));
        oxc.mint(address(this), OnyxReward);
    ...
    function safeOnyxTransfer(address _to, uint256 _amount) internal {
        uint256 OnyxBal = oxc.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > OnyxBal) {
            transferSuccess = oxc.transfer(_to, OnyxBal);
        } else {
