...
contract ILiquidationInterface {
    ...
    bool isDirectlyLiquidationAllowed = false;
    
    //直接清算
    function liquidateBorrowedDirectly(address _account, uint256 _id) public returns(uint256);
...
contract Liquidation is ILiquidationInterface, ErrorReporter, CarefulMath {
    ...
    //直接清算
    function liquidateBorrowedDirectly(address _borrower, uint256 _id) public returns(uint256) {
        ...
        liquidateBorrowDrectlyFresh(msg.sender, _borrower, _id);
        emit LiquidateBorrowedDirectlyEvent(_borrower, _id);
    }

    function liquidateBorrowDrectlyFresh(address _liquidator, address _borrower, uint256 _id) internal returns(uint256) {
        /*
        //1. 检测开关，关闭时无法直接清算
        //TODO
