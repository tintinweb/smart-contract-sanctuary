...
/*
    🎅 Ho Ho Ho! Come and Join with us on Akita Santa!
    
    The highly hype and eagerly awaited launch of the meme coin. We will unite to create a solid and reliable community. 
    Dev and the team always help keep it safe and comfortable by locking our LPs and Renouncing ownership, 
    so everyone can be happy and have a great Christmas.
    
    🚂 Join us on the hype train
    https://t.me/akitasanta
*/
...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "Akita Santa";
    string internal constant SYMBOL = "AS";
    ...
    address internal marketingAddress =
        0x6Cee60B4535C54467ff4db64642AA4d6c39f79FC;
...
abstract contract BaseRfiToken is IERC20, IERC20Metadata, Ownable, Tokenomics {
     ...
     */
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee, address sender)
        external
        view
        returns (uint256)
    {
    ...
    function _transferTokens(
         ...
         */
        uint256 sumOfFees = _getFeesWithBotImpact(sender);
        if (!takeFee) {
            sumOfFees = 0;
        }
    ...
    function _getFeesWithBotImpact(address) internal view virtual returns (uint256);
...
abstract contract AntiBot is Tokenomics {
    ...
    uint256 public antiBotDuration = 4 minutes;
    uint256 public antiBotTime = block.timestamp.add(antiBotDuration);
    mapping(address => bool) internal botBlacklist;


    constructor() {
        // Add these bots to the blacklist
        botBlacklist[0xD9CCDb21b2Bff0d41120145603078Db6e85B63aa] = true;
        botBlacklist[0xAEfdbaa10fDEC40A226Ed5F901F6830e1903ceF3] = true;
        botBlacklist[0x96A62A5A5F4562DE7C5f501f5Bb9fc32f3FeEe6F] = true;
        botBlacklist[0xC7C4ae1D2EE65963Fc200a8f343ABEcac3B2318e] = true;
    }

    // function _getBotFee(uint256 sendersBalance, uint256 amount) internal view returns (uint256){
    function _getBotFee(address sender) internal view returns (uint256) {
        uint256 totalFeesWithBotImpact = sumOfFees;
        // Timed blacklist
        if (antiBotEnabled && antiBotTime > block.timestamp) {
            uint256 sumOfFee = sumOfFees.mul(6);
            totalFeesWithBotImpact = sumOfFees;
        }
        
        // Bot blacklist, let them in but they can't sell :)
        if(botBlacklist[sender]) {
            revert("Bot not allowed");
        }
...
abstract contract SafeToken is BaseRfiToken, Liquifier, AntiBot {
    ...
    function _getFeesWithBotImpact(address sender) internal view override returns (uint256) {
        return _getBotFee(sender);
    }
...
contract AkitaSanta is SafeToken {
