...
// File: contracts/BSCEmpireDividendPayingToken.sol
...
// File: contracts/BSCEmpire.sol
pragma solidity ^0.7.0;

contract BSCEmpire is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BSCEmpireDividendTracker public dividendTracker;
    ...
    uint256 public dividendTime = 1800;
    //
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "BSCEmpire: The dividend tracker already has that address");

        BSCEmpireDividendTracker newDividendTracker = BSCEmpireDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BSCEmpire: The new dividend tracker must be owned by the BSCEmpire token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "BSCEmpire: The router already has that address");
        uniswapV2Router = IUniswapV2Router02(newAddress);
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "BSCEmpire: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "BSCEmpire: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BSCEmpire: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BSCEmpire: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BSCEmpire: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BSCEmpire: This account cannot send tokens until trading is enabled");
        }
...
contract BSCEmpireDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()  DividendPayingToken("BSCEmpire_Dividend_Tracker", "BSCEmpire_Dividend_Tracker") {
        claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "BSCEmpire_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BSCEmpire_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BSCEmpire contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "BSCEmpire_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BSCEmpire_Dividend_Tracker: Cannot update claimWait to same value");
