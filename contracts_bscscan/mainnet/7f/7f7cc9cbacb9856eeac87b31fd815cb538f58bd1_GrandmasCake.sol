...
// https://t.me/GMACake
// The FIRST cake NFT reward ecosystem. Grandma's feeling generous today!
// Earn $CAKE rewards for holding. 
//
//                                                                        ██████                
//                                                          ▒▒▓▓        ▓▓      ▓▓▒▒            
//                                                      ████████████  ██            ██          
...
contract GrandmasCake is ERC20, Ownable {
	using SafeMath for uint256;
	...
	GMACakeDividendTracker public dividendTracker;
	GMACakeTransferHelper private transferHelper;
	...
	constructor(address routerAddress, address payable marketingWalletAddress, address payable devWalletAddress) public ERC20("GrandmasCake", "GMACAKE") {
		dividendTracker = new GMACakeDividendTracker();
		transferHelper = new GMACakeTransferHelper(routerAddress);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "GMACake: The dividend tracker already has that address");

		GMACakeDividendTracker newDividendTracker = GMACakeDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "GMACake: The new dividend tracker must be owned by the GMACake token contract");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "GMACake: The router already has that address");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "GMACake: Account is already the value of 'excluded'");
	...
	function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
		require(pair != uniswapV2Pair, "GMACake: The PanCAKESwap pair cannot be removed from automatedMarketMakerPairs");
	...
	function setMaxWalletAmount(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "GMACake: Amount cannot be over the total supply.");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "GMACake: Automated market maker pair is already set to that value");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "GMACake: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "GMACake: Cannot update gasForProcessing to same value");
...
contract GMACakeDividendTracker is Ownable, DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() public DividendPayingToken("GMACake_Dividend_Tracker", "GMACake_Dividend_Tracker") {

	}

	function _transfer(address, address, uint256) internal override {
		require(false, "GMACake_Dividend_Tracker: No transfers allowed");
	}

	function withdrawDividend() public override {
		require(false, "GMACake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GMACake contract.");
	}
	...
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "GMACake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "GMACake_Dividend_Tracker: Cannot update claimWait to same value");
...
contract GMACakeTransferHelper is Ownable {
