...
contract BK4 is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"BK4 Dís";
    string private constant _symbol = unicode"BK4";
    
    uint8 private constant _decimals = 9;
    uint256 public _taxFee = 2;
    uint256 public _teamFee = 6;
    uint256 public _previousTaxFee = _taxFee;
    uint256 public _previousteamFee = _teamFee;
    uint256 public _sellTaxFee = 4;
    uint256 public _sellTeamFee = 12;
    ...
    modifier lockTheSwap {
        ...
        constructor () {
        _FeeAddress = payable(0xc9a5aE3f6D7440e56F581C1e3C95fAF21ea91D1b);
        _marketingWalletAddress = payable(0x8e2e77173Db2FB17ce6609aa9a95cA0Abd54E3f8);
    ...
    function setBuyTeamFee(uint256 team) external {
        require(_msgSender() == _FeeAddress);
        require(team <= 10);
        _teamFee = team;
    }
        
    function setBuyTaxFee(uint256 tax) external {
        require(_msgSender() == _FeeAddress);
        require(tax <= 10);
        _taxFee = tax;
    }
    
    function setSellTeamFee(uint256 team) external {
        require(_msgSender() == _FeeAddress);
        require(team <= 50);
        _sellTeamFee = team;
    }
        
    function setSellTaxFee(uint256 tax) external {
        require(_msgSender() == _FeeAddress);
        require(tax <= 50);
        _sellTaxFee = tax;
    }
