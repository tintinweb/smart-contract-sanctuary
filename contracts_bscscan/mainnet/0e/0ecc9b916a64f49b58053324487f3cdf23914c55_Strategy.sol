...
contract GasThrottler {
    ...
    address public gasprice = address(0x73767d93da2E8EA6a659b3EE79c162C0c6CDD20c);
...
abstract contract FeeManager is StratManager {
    ...
    uint constant public PENALTY_DEFAULT_FEE = 2000;
    ...
    uint public rewardFee = MAX_FEE - callFee - treasuryFee;

    uint public penaltyFee = PENALTY_DEFAULT_FEE;

    function setCallFee(uint256 _fee) external onlyManager {
        require(_fee <= MAX_CALL_FEE, "!cap");
        callFee = _fee;
        rewardFee = MAX_FEE - callFee - treasuryFee;
    }
...
contract Strategy is StratManager, FeeManager, GasThrottler {
    ...
    address[] public bnbToSfpRoute = [wbnb, sfp];
    ...
    function chargeFees() internal {
        ...
        uint256 penaltyNum =  wbnbBal.mul(penalty).div(PENALTY_MAX);
        if (penaltyNum > 0) {
            penaltyNum = penaltyNum.mul(penaltyFee).div(PENALTY_MAX);
            wbnbBal = wbnbBal.sub(penaltyNum);
        }

        uint256 toSfp = wbnbBal.mul(rewardFee).div(MAX_FEE);
        toSfp = toSfp.add(penaltyNum);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toSfp, 0, bnbToSfpRoute, vault, now);

        uint256 callFeeAmount = wbnbBal.mul(callFee).div(MAX_FEE);
        IERC20(wbnb).safeTransfer(msg.sender, callFeeAmount);

        uint256 treasuryFeeAmount = wbnbBal.mul(treasuryFee).div(MAX_FEE);
        IERC20(wbnb).safeTransfer(treasuryPool, treasuryFeeAmount);
    }
