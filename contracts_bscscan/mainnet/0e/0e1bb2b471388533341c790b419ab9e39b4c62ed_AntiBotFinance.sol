...
Telegram: https://t.me/AntiBotBSC
Website: Coming soon!  Check Telegram!
*/
...
contract AntiBotFinance is IBEP20, Auth {

    using SafeMath for uint256;

    string constant _name = "AntiBotFinance";
    string constant _symbol = "$ABF";
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingWallet = 0x5927Ceb692FdCe6717A77504c6A0bC3877d99b9b;  // marketingwallet
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        ...
        if(_allowances[sender][msg.sender] != uint256(-1)){
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "AntiBotFinance: Insufficient Allowance");
        }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        require(_isBlacklisted[sender] == false, "AntiBotFinance: You are banned");
        require(_isBlacklisted[recipient] == false, "AntiBotFinance: The recipient is banned");

        if(!authorizations[sender] && !authorizations[recipient]){
            require(tradingOpen, "AntiBotFinance: Trading is not open yet");
        }

        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "AntiBotFinance: Transaction Limit Exceeded");
        ...
        //Exchange tokens
        _balances[sender] = _balances[sender].sub(amount, "AntiBotFinance: Insufficient Balance");
