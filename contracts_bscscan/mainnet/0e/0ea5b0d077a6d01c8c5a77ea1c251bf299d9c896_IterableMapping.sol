...
contract EtherReward is ERC20, Ownable {
    ...
    constructor() ERC20("EtherReward", "EtherRewardEarn") {
    ...
    receive() external payable {
  	...
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "EtherReward: The team wallet is already this address");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "EtherReward: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function updateEthDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "EtherReward: The dividend tracker already has that address");

        EthDividendTracker newEthDividendTracker = EthDividendTracker(payable(newAddress));

        require(newEthDividendTracker.owner() == address(this), "EtherReward: The new dividend tracker must be owned by the EtherReward token contract");
    ...
    function updateDogeBackDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dogeBackDividendTracker), "EtherReward: The dividend tracker already has that address");

        DogeBackDividendTracker newDogeBackDividendTracker = DogeBackDividendTracker(payable(newAddress));

        require(newDogeBackDividendTracker.owner() == address(this), "EtherReward: The new dividend tracker must be owned by the EtherReward token contract");
    ...
    function updateEthDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "EtherReward: Fee must be less than 6%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "EtherReward: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "EtherReward: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "EtherReward: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "EtherReward: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "EtherReward: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "EtherReward: Trading has not started yet");
...
contract EthDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("EtherReward_Ethereum_Dividend_Tracker", "EtherReward_Ethereum_Dividend_Tracker", 0x2170Ed0880ac9A755fd29B2688956BD959F933F8) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "EtherReward_Ethereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "EtherReward_Ethereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherReward contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "EtherReward_Ethereum_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EtherReward_Ethereum_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DogeBackDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("EtherReward_DogeBack_Dividend_Tracker", "EtherReward_DogeBack_Dividend_Tracker", 0x08C975868e547BFE5F76Db7d1e075680e9736034) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "EtherReward_DogeBack_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "EtherReward_DogeBack_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherReward contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "EtherReward_DogeBack_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EtherReward_DogeBack_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
