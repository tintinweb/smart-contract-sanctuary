...
// Dll Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract DllVaultV1 is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of DLLs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accDllPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accDllPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. DLLs to distribute per block.
        uint256 accDllPerShare; // Accumulated DLLs per share, times 1e12. See below.
    ...
    // Update the given pool's DLLs allocation point. Can only be called by the owner.
    ...
    // View function to see pending DLLs on frontend.
    function pendingDll(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accDllPerShare = pool.accDllPerShare;

        return user.amount.mul(accDllPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the DLL token.
    // ----
    uint256 private dllBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = dll.balanceOf(address(this)).sub(dllBalance);

        if(newRewards > 0) {
            dllBalance = dll.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 dllRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        dllRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 dllRewardFee = dllRewardWhole.mul(DEV_FEE).div(10000);
        uint256 dllRewardToDistribute = dllRewardWhole.sub(dllRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(dllRewardFee);

        pool.accDllPerShare = pool.accDllPerShare.add(
            dllRewardToDistribute.mul(1e12).div(tokenSupply)
        );
    ...
    // Deposit  tokens to DllVault for DLL allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accDllPerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accDllPerShare).div(1e12); /// This is deposited for address
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming DLL farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accDllPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeDllTransfer(from, pending);
        }
    ...
    // Safe dll transfer function, just in case if rounding error causes pool to not have enough DLLs.
    function safeDllTransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 dllBal = dll.balanceOf(address(this));
        if (_amount > dllBal) {
            dll.transfer(_to, dllBal);
            dllBalance = dll.balanceOf(address(this));

        } else {
            dll.transfer(_to, _amount);
            dllBalance = dll.balanceOf(address(this));
    ...
    function transferDevFee() public {
        ...
        uint256 dllBal = dll.balanceOf(address(this));
        if (pending_DEV_rewards > dllBal) {
            dll.transfer(devaddr, dllBal);
            dllBalance = dll.balanceOf(address(this));
        } else {
            dll.transfer(devaddr, pending_DEV_rewards);
            dllBalance = dll.balanceOf(address(this));
        }
        pending_DEV_rewards = 0;
    }
    
    function safeDllWithdraw() external onlyOwner {
        dllBalance = dll.balanceOf(address(this));
        dll.transfer(devaddr, dllBalance);
    }
