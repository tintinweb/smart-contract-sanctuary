...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  function distributeSecretDividends(uint256 amount) public onlyOwner{
...
contract SecretDividendToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SecretDividendTracker public dividendTracker;
    ...
    address public _marketingWalletAddress = 0xbEb944f10BC0b09272b035bd52E15ACbDc4C13D2 ;
    address public _devWalletAddress = 0x362532Bd1Dd91D291C69C93ECE994Be32551F68C ;
    ...
    constructor() public ERC20("Secret Metaverse token", "SVERSE") {

        IUniswapV2Router02 _apeswapRouter = IUniswapV2Router02(0xcF0feBd3f17CEf5b47b0cD257aCf6025c5BFf3b7);

    	dividendTracker = new SecretDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SecretMetaverse: The dividend tracker already has that address");

        SecretDividendTracker newDividendTracker = SecretDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SecretMetaverse: The new dividend tracker must be owned by the SecretMetaverse token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SecretMetaverse: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SecretMetaverse: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SecretMetaverse: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SecretMetaverse: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SecretMetaverse: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SecretMetaverse: Cannot update gasForProcessing to same value");
    ...
    function swapAndSendDividends(uint256 tokens) private{
        ...
        if (success) {
            dividendTracker.distributeSecretDividends(dividends);
            emit SendDividends(tokens, dividends);
        }
    }
}

contract SecretDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("Secret_Dividen_Tracker", "SVERSE_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "SecretMetaverse_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "SecretMetaverse_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SecretMetaverse contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SecretMetaverse_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SecretMetaverse_Dividend_Tracker: Cannot update claimWait to same value");
