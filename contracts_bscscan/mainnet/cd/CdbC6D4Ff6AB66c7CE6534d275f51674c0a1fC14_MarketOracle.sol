...
/**
 * @title RMOON price Oracle
 *      This Oracle calculates the average USD price of RMOON based on the BNB-RMOON and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private RMOONBnbPrice0CumulativeLast;
    uint private RMOONBnbPrice1CumulativeLast;
    uint32 private RMOONBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _RMOON_bnb;
    ...
    constructor(
        address __RMOON_bnb,   // Address of the RMOON-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _RMOON_bnb = IUniswapV2Pair(__RMOON_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        RMOONBnbPrice0CumulativeLast = _RMOON_bnb.price0CumulativeLast();
        RMOONBnbPrice1CumulativeLast = _RMOON_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, RMOONBnbBlockTimestampLast) = _RMOON_bnb.getReserves();
    ...
    // Get the average price of 1 RMOON in the smallest BNB unit (18 decimals)
    function getRMOONBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_RMOON_bnb));

        require(_blockTimestamp != RMOONBnbBlockTimestampLast, "RMOON Last and current are equal");

        FixedPoint.uq112x112 memory RMOONBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - RMOONBnbPrice0CumulativeLast) / (_blockTimestamp - RMOONBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, RMOONBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        uint RMOONBnbAverage;
        uint busdBnbAverage;

        (RMOONBnbPrice0CumulativeLast, RMOONBnbPrice1CumulativeLast, RMOONBnbBlockTimestampLast, RMOONBnbAverage) = getRMOONBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, RMOONBnbAverage) = getRMOONBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = RMOONBnbAverage*(1e6) / busdBnbAverage;
