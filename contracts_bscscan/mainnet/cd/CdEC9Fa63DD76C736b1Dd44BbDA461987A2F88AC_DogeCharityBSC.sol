...
contract DogeCharityBSC is Context, IERC20, Ownable {
    ...
    uint256 private _tshibacharitybscTotal;

    string private _name = "DogeCharityBSC";
    string private _symbol = "DOCHA";
    ...
    uint256 public _shibacharitybscFee = 15;
    uint256 private _previousshibacharitybscFee = _shibacharitybscFee;
    
    uint256 public _maxTxAmount = 1000000 * 10**6 * 10**9;
    uint256 private minimumTokensBeforeSwap = 1 * 10**6 * 10**9; 
    
    address payable public shibacharitybscWallet = 0xa97382409f8579eDE0BD0AEa1B5573B91a86c185; 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _tshibacharitybscTotal = _tshibacharitybscTotal.add(address(this).balance);
        TransferCharityETH(shibacharitybscWallet, address(this).balance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_shibacharitybscFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _shibacharitybscFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousshibacharitybscFee = _shibacharitybscFee;
        
        _taxFee = 0;
        _burnFee = 0;
        _shibacharitybscFee = 0;
    }
    
    function restoreAllFee() private {
        _taxFee = _previousTaxFee;
        _burnFee = _previousBurnFee;
        _shibacharitybscFee = _previousshibacharitybscFee;
    }
    ...
    function setshibacharitybscFeePercent(uint256 shibacharitybscFee) external onlyOwner() {
        _shibacharitybscFee = shibacharitybscFee;
    }
