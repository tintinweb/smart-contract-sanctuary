...
contract CUPCAKE is ERC20, Ownable {
    ...
    uint256 public totalFees = CAKERewardsFee.add(liquidityFee);
    uint256 public maxTx = 100_000_000_000 * (1e18);
    ...
    address public _devWalletAddress = address(0xba3C81D7987fe2312393aedb2B5c2e053D1a4fb5);
    ...
    function setCAKERewardsFee(uint256 value) external onlyOwner{
        ...
        require(CAKERewardsFee.add(liquidityFee) <= 15, "May not set total fees above 15%");
        ...
        if (antiDump) {
            if (
                ...
                if (automatedMarketMakerPairs[from]) {
                    require(amount <= maxTx, "can't exceed maxBuy");
                    require(amount + balanceOf(to) <= maxWallet, "can't exceed maxWallet");
                } 
                //when sell
                else if (automatedMarketMakerPairs[to]) {
                    if (!_isExcludedMaxSellTransactionAmount[from]) {
                        require(amount <= maxperblock, "Sell transfer amount exceeds the maxSellTransactionAmount.");
                    }
                }
                //normal transfer
                else {
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        IERC20(CAKE).transfer(_devWalletAddress, newBalance);
    }
