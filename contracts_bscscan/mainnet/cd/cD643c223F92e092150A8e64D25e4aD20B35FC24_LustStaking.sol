...
contract LustStaking is AccessControl {
    ...
    mapping(address => uint256) public userPackageIndex;

    uint256[5] public defaultPacages = [0, 30 days, 90 days, 180 days, 270 days];
    uint256[5] public packagePercent = [0, 45, 60, 90, 100];
    uint256[5] public lustRewardPercents = [0, 30, 40, 45, 50];
    uint256[5] public lwapRewardPercents = [0, 70, 60, 55, 50];
    ...
    function stake(uint256 amount, uint256 period) public{
        ...
        userPackageIndex[msg.sender] = packageIndex;
    }

    function unStake() public{
        uint256 packageIndex = userPackageIndex[msg.sender];
        uint256 period = block.timestamp - stakeTimes[msg.sender];
    ...
    function getReward(address user) public view returns(uint256 lustReward, uint256 lwapReward) {
        uint256 packageIndex = userPackageIndex[user];

        uint256 totalLustReward = staked[user] * packagePercent[packageIndex] / 12 / 100;
    ...
    function setDefaultPacages(uint256[5] memory newDefaultPacages) public onlyAdmin {
        defaultPacages = newDefaultPacages;
    }

    function setPackagePercent(uint256[5] memory newPackagePercent) public onlyAdmin {
        packagePercent = newPackagePercent;
    }

    function setLustRewardPercents(uint256[5] memory newLustRewardPercents) public onlyAdmin {
        lustRewardPercents = newLustRewardPercents;
    }

    function setLwapRewardPercents(uint256[5] memory newLwapRewardPercents) public onlyAdmin {
