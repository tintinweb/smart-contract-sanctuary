...
// Millionaires Of Digital Island ( $MODI ) is a metaverse for the meme millionaires who will become millionaires of digital island.

// https://modi.io/
// https://t.me/modi_io
// https://twitter.com/modi_io
...
contract modi is Context, IERC20, Ownable {
    ...
    // whale restriction exempted addresses
    mapping (address => bool) private whaleExempted;
    
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tokTotalSupply = 100000000000 * 10**9;
    uint256 private _refTotalSupply = (MAX - (MAX % _tokTotalSupply));
    uint256 private _tokFeeTotal;
    uint256 private _tokBurned;
    uint256 private _whaleRestrictionUpto;

    address private _devFundAddress;
    string private _name = 'Millionaires of Digital Island';
    string private _symbol = 'MODI';
    uint8 private _decimals = 9;
    uint8 private _whalePercent = 10;

    constructor () public {
        _refOwned[_msgSender()] = _refTotalSupply;
        _devFundAddress = _msgSender();
        _whaleRestrictionUpto = block.number.add(10000);
    ...
    function distribute(uint256 tokAmount, bool burn) public {
        ...
        require(!_isExempted[sender], "MODI: Exempted addresses not allowed");
        if (!burn) {
            ...
            (uint256 refAmount,,,,) = _getValues(tokAmount);
            _refOwned[sender] = _refOwned[sender].sub(refAmount);    
            uint256 tokAmountThird = tokAmount.div(3); 
            (uint256 refAmountThird,,,,) = _getValues(tokAmountThird);
            _tokOwned[_devFundAddress] = _tokOwned[_devFundAddress].add(tokAmountThird);
            _refOwned[_devFundAddress] = _refOwned[_devFundAddress].add(refAmountThird);
            _refTotalSupply = _refTotalSupply.sub(refAmountThird);
            _tokFeeTotal = _tokFeeTotal.add(tokAmountThird);
            _tokBurned = _tokBurned.add(tokAmountThird);
        }
    }

    function burn(uint256 tokAmount) public {
        address sender = _msgSender();
        require(!_isExempted[sender], "MODI: Exempted addresses cannot burn");
        (uint256 refAmount,,,,) = _getValues(tokAmount);
        _refOwned[sender] = _refOwned[sender].sub(refAmount);
        _refTotalSupply = _refTotalSupply.sub(refAmount);
    ...
    function SplitFromToken(uint256 tokAmount, bool deductTransferFeeAndBurn) public view returns(uint256) {
        require(tokAmount <= (_tokTotalSupply + _tokFeeTotal), "MODI: Amount must be less than supply");
    ...
    function tokenFromSplit(uint256 refAmount) public view returns(uint256) {
        require(refAmount <= _refTotalSupply, "MODI: Amount must be less than total split");
    ...
    function exemptAccount(address account) external onlyOwner() {
        require(!_isExempted[account], "MODI: Account is already exempted");
    ...
    function obligateAccount(address account) external onlyOwner() {
        require(_isExempted[account], "MODI: Account is not Exempted");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        require(sender != address(0), "MODI: transfer from the zero address");
        require(recipient != address(0), "MODI: transfer to the zero address");
        require(amount > 0, "MODI: Transfer amount must be greater than zero");
        require(amount <= balanceOf(sender), "MODI: insufficient balance");
        
        // check for whale restriction upto the set blockNum
        if( block.number < _whaleRestrictionUpto ){
            if(!whaleExempted[sender]){
                uint256 allowedAmount = _tokTotalSupply.div(1000).mul(_whalePercent);
                if(amount > allowedAmount){
                    revert("MODI: Whale movement restricted please try a lower amount");
                }
            }
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tokAmount) private {
        (uint256 refAmount, uint256 refTransferAmount, uint256 refFeeThird, uint256 tokTransferAmount, uint256 tokFeeThird) = _getValues(tokAmount);
        _refOwned[sender] = _refOwned[sender].sub(refAmount);
        _refOwned[recipient] = _refOwned[recipient].add(refTransferAmount);
        _distributeFee(refFeeThird, tokFeeThird);
    ...
    function _distributeFee(uint256 refFeeThird, uint256 tokFeeThird) private {
        // Add one third of the transfer fee to _devFundAddress
        _tokOwned[_devFundAddress] = _tokOwned[_devFundAddress].add(tokFeeThird);
        _refOwned[_devFundAddress] = _refOwned[_devFundAddress].add(refFeeThird);
        _refTotalSupply = _refTotalSupply.sub(refFeeThird);
        _tokFeeTotal = _tokFeeTotal.add(tokFeeThird);
        _tokBurned = _tokBurned.add(tokFeeThird);
        _tokTotalSupply = _tokTotalSupply.sub(tokFeeThird);
    }

    function _getValues(uint256 tokAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        (uint256 tokTransferAmount, uint256 tFee, uint256 tokFeeThird) = _getTValues(tokAmount);
        uint256 currentRate =  _getRate();
        (uint256 refAmount, uint256 refTransferAmount, uint256 refFeeThird) = _getRValues(tokAmount, tFee, tokFeeThird, currentRate);
        return (refAmount, refTransferAmount, refFeeThird, tokTransferAmount, tokFeeThird);
    }

    function _getTValues(uint256 tokAmount) private pure returns (uint256, uint256, uint256) {
        uint256 tokFeeThird = tokAmount.div(100);
        uint256 tFee = tokFeeThird.mul(3);
        uint256 tokTransferAmount = tokAmount.sub(tFee);       
        return (tokTransferAmount, tFee, tokFeeThird);
    }

    function _getRValues(uint256 tokAmount, uint256 tFee, uint256 tokFeeThird, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 refAmount = tokAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 refFeeThird = tokFeeThird.mul(currentRate);
        uint256 refTransferAmount = refAmount.sub(rFee);
        return (refAmount, refTransferAmount, refFeeThird);
    }
    ...
    function setWhaleRestrictionBlock(uint256 blockNum) public onlyOwner {
        require(blockNum > block.number, "MODI: Block number to be greater than current block");
        _whaleRestrictionUpto = blockNum;
    }
    
    function getWhaleRestrictionBlock() public view returns (uint256) {
        return _whaleRestrictionUpto;
    }
    
    function setWhaleExempted(address user, bool value) public onlyOwner {
        whaleExempted[user] = value;
    }
    
    function setWhaleAmountInPercentage(uint8 value) public onlyOwner {
        require(value > 0, "MODI: Value should be greater than zero");
        _whalePercent = value;
    }
    
    function setDevFundAddress(address devFundAddress) public onlyOwner {
