...
contract MasterChef is Ownable, ReentrancyGuard {
  ...
  struct UserInfo {
    ...
    //   1. The pool's `accDynaPerShare` (and `lastRewardBlock`) gets updated.
  ...
  struct PoolInfo {
    ...
    uint256 accDynaPerShare;   // Accumulated PANTHERs per share, times 1e12. See below.
  ...
  constructor(
    DYNAToken _dyna,
    uint256 _startBlock,
    uint256 _dynaPerBlock
  ) public {
    dyna = _dyna;
    startBlock = _startBlock;
    dynaPerBlock = _dynaPerBlock;
  ...
  function pendingDyna(uint256 _pid, address _user) external view returns (uint256) {
    ...
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      ...
      uint256 dynaReward = multiplier.mul(dynaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
  ...
  // Safe dyna transfer function, just in case if rounding error causes pool to not have enough PANTHERs.
  ...
  function updateEmissionRate(uint256 _dynaPerBlock) public onlyOwner {
    massUpdatePools();
    emit EmissionRateUpdated(msg.sender, dynaPerBlock, _dynaPerBlock);
    dynaPerBlock = _dynaPerBlock;
  }

  // Update the dyna referral contract address by the owner
  function setDynaReferral(IDYNAReferral _dynaReferral) public onlyOwner {
...
contract DYNAToken is BEP20('Dynamis Token', 'DYNA'), MinterRole, Pausable {
    using SafeMath for uint256;
    uint256 public constant INITIAL_SUPPLY = 15552000 * 1e18;
    uint256 public constant PRESALE_SUPPLY = 12960000 * 1e18;
    uint256 public constant MAX_SUPPLY = 51840000 * 1e18;
    ...
    constructor() public {
        _mint(address(this), PRESALE_SUPPLY);
        _mint(msg.sender, INITIAL_SUPPLY.sub(PRESALE_SUPPLY));
    ...
    function _transfer(address _sender, address _recepient, uint256 _amount) internal override antiWhale(_sender, _recepient, _amount) {
        ...
        if (_recepient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(_amount == sendAmount + taxAmount, "PANTHER::transfer: Tax value invalid");
    ...
    function updatePantherSwapRouter(address _router) public onlyOperator {
        ...
        require(pancakeSwapPair != address(0), "DYNA::updatePantherSwapRouter: Invalid pair address.");
    }
