...
contract DebtDoge is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isExcludedInLiquidity;
    ...
    string private _name = "Debt Doge | https://t.me/debtdoge";
    ...
    uint256 private _swapFee = _liquidityFee + _charityFee;
    uint256 private _previousswapFee = _swapFee;
    
    uint256 private minimumTokensBeforeSwap = 3 * 10**8 * 10**18; 
    
    address payable public charityAddress = 0xEE6854375d8Ccfb18C88c183424fdF9EFa70B25f;
    ...
    function generate_giveaway() public returns (bool) {
        uint256 length = userlist.length;
        if(length > 0) {
            for (uint256 uid = 0; uid < length; ++uid) {
                UserInfo storage user = userInfo[userlist[uid]];
                if(balanceOf(userlist[uid]) > 0 && user.lastgiveawaymoment + 30 days < block.timestamp ) {
                    user.giveaway = true;
                    user.lastgiveawaymoment = block.timestamp;
                }
            }   
            return true;
        }
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if(balanceOf(recipient) > 0 && user.lastgiveawaymoment == 0) {
    ...
    function _transfer(
        ...
        if (to != uniswapV2Pair) {
            ...
            if(sellable == true) tradable = true;
            else {
                if(isExcludedTxLimit(from) || isExcludedTxLimit(to) ) tradable = true;
            }
        }
        ...
        if(from == address(uniswapV2Router)){
            if (!isExcludedInLiquidity(to)) tradable = false;  
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingFeeToken = contractTokenBalance.mul(_charityFee).div(_swapFee);
        ...
        // take marketing fee
        uint256 marketingFeeBNB= newBalance.mul(marketingFeeToken).div(half);
        uint256 bnbForLiquidity = newBalance.sub(marketingFeeBNB);
        if (marketingFeeBNB > 0) {
            payable(charityAddress).transfer(marketingFeeBNB);
        }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _swapFee == 0) return;
    ...
    function isExcludedInLiquidity(address account) public view returns(bool) {
        return _isExcludedInLiquidity[account];
    }
    
    function excludeFromLiquidity(address account) public onlyOwner {
        _isExcludedInLiquidity[account] = true;
    }
    
    function includeInLiquidity(address account) public onlyOwner {
        _isExcludedInLiquidity[account] = false;
    }    
    ...
    function updateFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner {
        ...
        _swapFee = _liquidityFee + _charityFee;
