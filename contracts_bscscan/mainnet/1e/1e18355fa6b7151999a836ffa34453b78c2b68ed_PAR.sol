...
// File: contracts/governance/PAR.sol
...
contract PAR is DelegateERC20, Ownable {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;

    EnumerableSet.AddressSet private _minters;

    uint256 private constant maxSupply = 1_000_000_000e18;     // the max supply of PAR token

    constructor(uint256 airdropSupply) public ERC20("PAR Token", "PAR"){
    ...
    function addMinter(address _addMinter) public onlyOwner returns (bool) {
        require(_addMinter != address(0), "PAR Token: _addMinter is the zero address");
        return EnumerableSet.add(_minters, _addMinter);
    }

    function delMinter(address _delMinter) public onlyOwner returns (bool) {
        require(_delMinter != address(0), "PAR Token: _delMinter is the zero address");
    ...
    function getMinter(uint256 _index) public view onlyOwner returns (address){
        require(_index <= getMinterLength() - 1, "PAR Token: index out of bounds");
        return EnumerableSet.at(_minters, _index);
    }

    // modifier for mint function
    modifier onlyMinter() {
        require(isMinter(msg.sender), "PAR Token: caller is not the minter");
        _;
    }
}