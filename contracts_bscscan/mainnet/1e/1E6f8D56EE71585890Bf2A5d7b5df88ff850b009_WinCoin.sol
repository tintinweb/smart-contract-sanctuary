...
interface IUniswapV2Pair {
    ...
    function burn(address to) external returns (uint liquidity);
...
contract WinCoin is Context, IBEP20, Ownable {
    ...
    address public giveawayWallet = 0x98156e6d3E19CB7b17AaAF8Ef1c58F1F8fAf5c55;
    ...
    address public BuyBackWallet = 0x98156e6d3E19CB7b17AaAF8Ef1c58F1F8fAf5c55;
    ...
    function setMarketingFeePercent (uint256 marketingFee) external onlyOwner() {
     ...
     function setGiveawayFeePercent (uint256 devFee) external onlyOwner() {
        _giveawayFee = devFee;
    }
     function setBuyBackFeePercent (uint256 BBFee) external onlyOwner() {
        _BuyBackFee  = BBFee;
    }
    function setmarketingWallet(address mWallet) external onlyOwner() {
        MarketingWallet = mWallet;
    }
    function setGiveawayWallet(address gWallet) external onlyOwner() {
        giveawayWallet = gWallet; 
    }
    function setBuyBackWallet(address BBWallet) external onlyOwner() {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tCharity = calculategiveawayFee(tAmount);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _BuyBackFee ==0 && _MarketingFee ==0 && _giveawayFee == 0) return;
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax, burn, liquidity fee giveaway, marketing, buyback fee
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        //Calculate burn amount , giveaway, marketing, buy back fee amount
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(burnAmt).sub(marketingAmt).sub(giveawayAmt).sub(BuyBackAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(burnAmt).sub(marketingAmt).sub(giveawayAmt).sub(BuyBackAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(marketingAmt).sub(giveawayAmt).sub(BuyBackAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(burnAmt).sub(marketingAmt).sub(giveawayAmt).sub(BuyBackAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(marketingAmt).sub(giveawayAmt).sub(BuyBackAmt)));
        }
