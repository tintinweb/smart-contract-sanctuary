...
contract Wolfgirl is ERC20SnapshotUpgradeableCustom, OwnableUpgradeable {
  ...
  function setCost(uint256 aiCost) public onlyOwner {
    require(aiCost <= 500000,"TOO_HIGH_AI_NFT_COST");

    AINFTCost = aiCost;
  }

  function setPriceFloor(uint256 floor) public onlyOwner {
    priceFloor = floor;
  }
  ...
  function manageReserve() public {
    ...
    require(IERC721(NFTCONTRACT).balanceOf(_msgSender()) > 0, 'DO_NOT_OWN_NFT');
    ...
    if (totalSupply() < nextInterimClaimThreshold) {
      ...
      interimAssetAmountSnapshot = currentAssetSupply;
  ...
  function _transferHelper(
    ...
    if (_isLP(recipient) && priceFloorEnabled) {
      ...
      if (priceFloor > projectedPrice) {
      burnAmount = _pct(800, amount);
      reserveAmount = _pct(1600, amount);
      }
    }
