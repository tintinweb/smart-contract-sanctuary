...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also ADAREFLECTIONed.
...
// File: contracts/ADAREFLECTIONDividendPayingToken.sol
...
// File: contracts/ADAREFLECTION.sol
...
contract ADAREFLECTION is ERC20, Ownable {
    using SafeMath for uint256;

    IPancakeSwapV2Router02 public PancakeSwapV2Router;
    address public immutable PancakeSwapV2Pair;

    address public immutable Adatoken = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); // Adatoken
	address payable public MarketingWallet = payable(0x3406bC28A0322ec42230A0120c7753Cd6a4E8c86); // Marketing Address

    bool private swapping;

    ADAREFLECTIONDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ADAREFLECTION", "ADAREFLECTION") {
        ...
        dividendTracker = new ADAREFLECTIONDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ADAREFLECTION: The dividend tracker already has that address");

        ADAREFLECTIONDividendTracker newDividendTracker = ADAREFLECTIONDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ADAREFLECTION: The new dividend tracker must be owned by the ADAREFLECTION token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ADAREFLECTION: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PancakeSwapV2Pair, "ADAREFLECTION: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ADAREFLECTION: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ADAREFLECTION: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ADAREFLECTION: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ADAREFLECTION: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!tradingOpen) {
            require(canTransferBeforetradingOpen[from], "ADAREFLECTION: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForAdatoken(uint256 tokenAmount, address recipient) private {
        ...
        // generate the PancakeSwap pair path of weth -> Adatoken
...
contract ADAREFLECTIONDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("ADAREFLECTION_Dividend_Tracker", "ADAREFLECTION_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "ADAREFLECTION_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "ADAREFLECTION_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ADAREFLECTION_Dividend_Tracker: Cannot update claimWait to same value");
