...
contract KapexPresale {
    ...
    mapping(address => string) public allocatedBand; // If the address doesnt belong to any band, the value will be ""
    ...
    constructor(
        ...
        require(
            _startDateBuy >= block.timestamp,
            "_startDateBuy has to be greater than block.timestamp"
        );
        require(
            _startDateClaim >= block.timestamp,
            "_startDateClaim has to be greater than block.timestamp"
        );
    ...
    //////////
    // Getters
    function calculateBNBToPresaleToken(uint256 _amount)
        public
        view
        returns (uint256)
    {
        ...
        uint256 tokens = ((_amount * price) /100) /
            (10**(18 - uint256(IERC20(presaleToken).decimals())));

        uint256 tokensWithBand = tokens +
            (tokens * bandsPercentages[allocatedBand[msg.sender]]) /
            10**9;
    ...
    function buy() public payable {
        ...
        require(
            bytes(allocatedBand[msg.sender]).length > 0,
            "msg.sender does not belong to any band (not whitelisted)"
        );
        require(
            IERC20(houseToken).balanceOf(msg.sender) >= minHouseTokenHoldAmount,
            "msg.sender doesnt hold enough Koda"
        );
        require(
            bought[msg.sender] + msg.value <= maxPurchase,
            "Cannot buy more than max purchase amount"
        );
    ...
    function claim() public {
        ...
        require(
            bytes(allocatedBand[msg.sender]).length > 0,
            "msg.sender does not belong to any band"
        );
    ...
    function setBandPercentage(string calldata _band, uint256 _percentage)
        external
        onlyOwner
    {
    ...
    function setBands(address[] calldata _wallets, string calldata _band)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _wallets.length; i++)
            allocatedBand[_wallets[i]] = _band;
    }

    function setBand(address _wallet, string calldata _band)
        external
        onlyOwner
    {
    ...
    function setPresaleToken(address _presaleToken, address _receiver)
        external
        onlyOwner
    {
        if (presaleToken != address(0)) {
            ...
            if (contractBal > 0)
                IERC20(presaleToken).transfer(_receiver, contractBal);
        }
    ...
    function setMinHouseTokenHoldAmount(uint256 _minHouseTokenHoldAmount)
        external
        onlyOwner
    {
        minHouseTokenHoldAmount = _minHouseTokenHoldAmount;
