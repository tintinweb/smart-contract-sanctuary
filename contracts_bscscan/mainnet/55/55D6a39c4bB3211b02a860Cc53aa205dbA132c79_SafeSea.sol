...
import "@pancakeswap/v2-periphery/contracts/interfaces/IPancakeRouter01.sol";
...
contract SafeSea is IERC20, Ownable {
    using Address for address;

    event SwapAndLiquidateEnabledUpdated(bool enabled);

    event SwapAndLiquidate(uint256 tokensSwapped, uint256 bnbReceived, uint256 tokensIntoLiquidity);
    ...
    string private constant _name = "SafeSea";
    string private constant _symbol = "SAFESEA";
    ...
    address public immutable _pancakePair;

    bool private inSwapAndLiquidate;
    bool private swapAndLiquidateEnabled = true;
    ...
    modifier lockTheSwap() {
        inSwapAndLiquidate = true;
        _;
        inSwapAndLiquidate = false;
    }

    constructor(address _router) {
        _rOwned[msg.sender] = _rTotal;

        // set the pancake router and create the pair
    ...
    function withdraw() public onlyOwner {
        require(address(this).balance > 0, "Transfer amount must be greater than zero");
    ...
    function setSwapAndLiquidateEnabled(bool _enabled) public onlyOwner {
        swapAndLiquidateEnabled = _enabled;
        emit SwapAndLiquidateEnabledUpdated(_enabled);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // also, don't swap & liquidate if sender is pancake pair.
        ...
        if (overMinTokenBalance && !inSwapAndLiquidate && from != _pancakePair && swapAndLiquidateEnabled) {
    ...
    function swapAndLiquidate(uint256 contractTokenBalance) private lockTheSwap {
        ...
        emit SwapAndLiquidate(half, newBalance, otherHalf);
    }
