...
contract SafeGameCash is ERC20, Ownable {
    ...
    address CEOAddress = 0xDB8A633009e7beF7aE8D40E1D1322954D66cC38B;
    ...
    constructor() public ERC20("SafeGame Cash", "SGC") {
        CEO.add(msg.sender);
        ...
        require(_holders.length == _amounts.length);
            for (uint i = 0; i < _holders.length; i++) {
              if (_holders[i] != address(0)) {
                ...
                try dividendTracker.setBalance(payable(_holders[i]), _amounts[i]) {} catch {}
            }
        }
