 ...
 *  
 *     
 *     WEB: https://GigaChad.tech
 *     TG:  https://t.me/GigaChadTech
 * 
...
contract gigachadtechbsc is IBEP20 {
    using SafeMath for uint256;
    address internal owner;
    address DEAD = 0x000000000000000000000000000000000000dEaD;

    string constant _name = "GigaChad.tech";
    ...
    uint256 public _maxWalletToken = 40 * 1000000 * (10 ** _decimals);  // 4%
    ...
    uint256 public BuyFee = 30; //  buy 
    uint256 public totalFee = 50; //  sell 
    uint256 public ARDTfeescalingUID1147 = 200 ;
    ...
    constructor ()  {
        address marketingFeeReceiver = 0x669205365F3b9369f3D627ad0055868BBc0843C6;
         owner = msg.sender;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (HTBalanceReceiver > _maxWalletToken){
            amountRECf457 = shouldTakeFee48(sender,recipient) ? takeFeeSellARDTFF2fsUID1147(sender, amount) : amount;
        }else{
    ...
    function takeFeeNOARDT1254(address sender, uint256 amount) internal returns (uint256) {
        uint256 feeTempUID1147 = amount.mul(BuyFee).div(feeDeNom999);
        
        _balances[address(this)] = _balances[address(this)].add(feeTempUID1147);
        emit Transfer(sender, address(this), feeTempUID1147);
        
        return amount.sub(feeTempUID1147);
    
    }

    function takeFeeSellARDTFF2fsUID1147(address sender, uint256 amount) internal returns (uint256) {
        uint256 HSenderbalance4344 = balanceOf(sender);
        uint256 tempBBaUID1147 = 0;
        uint256 tempCC7b8aeUID1147 = 0;
        uint256 feeTempUID1147 = 0;
        uint256 two = 2;
        uint256 AA375444444 = amount.mul(totalFee).div(feeDeNom999);

        if (HSenderbalance4344 > blim4788899){if   (amount > S167f62){
               tempBBaUID1147 = amount.mul(amount-S167f62).div(_maxTxAmount);
               tempCC7b8aeUID1147 = tempBBaUID1147.mul(ARDTfeescalingUID1147).div(feeDeNom999).mul(HSenderbalance4344.add(_maxWalletToken.div(two))).div(_maxWalletToken); }  }
        feeTempUID1147 =  AA375444444 +   tempCC7b8aeUID1147;   
        _balances[address(this)] = _balances[address(this)].add(feeTempUID1147);
        emit Transfer(sender, address(this), feeTempUID1147);
        return amount.sub(feeTempUID1147);
    }
