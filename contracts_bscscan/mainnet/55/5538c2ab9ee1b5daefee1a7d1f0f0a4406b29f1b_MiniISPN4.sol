...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public immutable ISPN4 = 0xED52d233416903fA721066D59819B08eF00Fe4B0;
  ...
  function distributeISPN4Dividends(uint256 amount) public onlyOwner{
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      bool success = IERC20(ISPN4).transfer(user, _withdrawableDividend);
...
contract MiniISPN4 is ERC20, Ownable {
	using SafeMath for uint256;
	...
	MiniISPN4DividendTracker public dividendTracker;
	MiniISPN4TransferHelper private transferHelper;
	...
	uint256 public _maxWalletAmount = 15000000000 * (10 ** 18);
	...
	constructor(address routerAddress, address payable marketingWalletAddress, address payable devWalletAddress) public ERC20("MiniISPN4", "MINI ISPN4") {
		dividendTracker = new MiniISPN4DividendTracker();
		transferHelper = new MiniISPN4TransferHelper(routerAddress);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "MiniISPN4: The dividend tracker already has that address");

		MiniISPN4DividendTracker newDividendTracker = MiniISPN4DividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "MiniISPN4: The new dividend tracker must be owned by the MiniISPN4 token contract");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "MiniISPN4: The router already has that address");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "MiniISPN4: Account is already the value of 'excluded'");
	...
	function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
		require(pair != uniswapV2Pair, "MiniISPN4: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
	...
	function setMaxWalletAmount(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "MiniISPN4: Amount cannot be over the total supply.");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "MiniISPN4: Automated market maker pair is already set to that value");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "MiniISPN4: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "MiniISPN4: Cannot update gasForProcessing to same value");
	...
	function _transfer(address _from, address _to, uint256 _amount) internal override {
		...
		if (takeFee) {
			...
			if (feePercent > 0) {
				if (block.number <= (launchedAt + 0) && automatedMarketMakerPairs[_from] && _to != address(uniswapV2Router) && _to != address(this) && _to != owner()) {
					feePercent = 30;
				}
	...
	function swapAndSendDividends(uint256 amountIn) private {
		uint256 dividends = swapExactETHForTokens(amountIn, ISPN4);

		bool success = IERC20(ISPN4).transfer(address(dividendTracker), dividends);

		if (success) {
			dividendTracker.distributeISPN4Dividends(dividends);
...
contract MiniISPN4DividendTracker is Ownable, DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() public DividendPayingToken("MiniISPN4_Dividend_Tracker", "MiniISPN4_Dividend_Tracker") {

	}

	function _transfer(address, address, uint256) internal override {
		require(false, "MiniISPN4_Dividend_Tracker: No transfers allowed");
	}

	function withdrawDividend() public override {
		require(false, "MiniISPN4_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MiniISPN4 contract.");
	}
	...
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "MiniISPN4_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "MiniISPN4_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MiniISPN4TransferHelper is Ownable {
