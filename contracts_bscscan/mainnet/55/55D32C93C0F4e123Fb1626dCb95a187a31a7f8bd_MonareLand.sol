...
contract MonareLand is ERC721, Ownable {
    string public collectionName;
    string public collectionNameSymbol;
    uint256 public cryptoTokenCounter;

    address payable MonAreAddress;
    ...
    constructor(address payable _mainAddress) ERC721("Monare Land", "MonareLa") {
        collectionName = name();
        collectionNameSymbol = symbol();
        setMainAddress(_mainAddress);
    }

    function customMint(string memory _tokenURI) public payable {
        cryptoTokenCounter++;
        _mint(msg.sender, cryptoTokenCounter);
        super._setTokenURI(cryptoTokenCounter, _tokenURI);
        address payable sendTo = MonAreAddress;
        ...
        allCryptoNFT[cryptoTokenCounter] = newCryptoNFT;
    }
    ...
    function setMainAddress (address payable _mainAddress) public onlyOwner{
        MonAreAddress = _mainAddress;
    }
    ...
    function setTokenPriceAndSale(uint256 _tokenId, uint256 _newPrice) public {
        require(msg.sender != address(0));
        require(_exists(_tokenId));
        address tokenOwner = ownerOf(_tokenId);
        require(tokenOwner == msg.sender);
    ...
    function cancelForSale(uint256 _tokenId) public {
        require(msg.sender != address(0));
        require(_exists(_tokenId));
        address tokenOwner = ownerOf(_tokenId);
        require(tokenOwner == msg.sender);
