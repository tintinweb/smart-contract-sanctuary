...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0xD3E4DddEEC510727d22A154f9E48C21Ea6f76fCa;
...
contract AFScake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    AFScakeDividendTracker public dividendTracker;

    address public buyBackWallet;
    address public marketingWallet;

    uint256 public maxBuyTranscationAmount = 30000000000 * (10**18);
    ...
    uint256 public _buyBackMultiplier = 200;
    ...
    constructor() ERC20("AFScake", "AFScake") {
        uint256 _dividendRewardsFee = 8;
        uint256 _marketingFee = 15;

        dividendRewardsFee = _dividendRewardsFee;
        marketingFee = _marketingFee;
        totalFees = _dividendRewardsFee.add(_marketingFee);

    	dividendTracker = new AFScakeDividendTracker();
		marketingWallet = payable(0x571aeDb5B74a7Dc524e92160755Bf68d81c06907);

    	buyBackWallet = 0x4300Ec6957bbb3a4A2C45bE9dfEB47167fE16C4D;
    	
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //0x4300Ec6957bbb3a4A2C45bE9dfEB47167fE16C4D
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "AFScake: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "AFScake: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "AFScake: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "AFScake: The dividend tracker already has that address");

        AFScakeDividendTracker newDividendTracker = AFScakeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "AFScake: The new dividend tracker must be owned by the AFScake token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "AFScake: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "AFScake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "AFScake: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "AFScake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "AFScake: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "AFScake: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "AFScake: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "AFScake: Cannot update gasForProcessing to same value");
    ...
    function rand() public view returns(uint256) {
        ...
        uint256 randNumber = (seed - ((seed / 1) * 1));
        if (randNumber == 0) {
            randNumber += 1;
            return randNumber;
        } else {
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && canSwap && from != marketingWallet && to != marketingWallet) {
                swapping = true;
...
contract AFScakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("AFScake_Dividend_Tracker", "AFScake_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 75000 * (10**18); //must hold 75,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "AFScake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "AFScake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AFScake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 3600, "AFScake_Dividend_Tracker: claimWait must be updated to between 30 Minutes and 1 Hour");
        require(newClaimWait != claimWait, "AFScake_Dividend_Tracker: Cannot update claimWait to same value");
