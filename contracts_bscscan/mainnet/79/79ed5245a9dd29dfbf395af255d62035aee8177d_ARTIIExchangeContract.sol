...
contract Sale is Ownable{
    ...
    event SaleShare(address indexed from, address indexed to);
    using SafeMath for uint256;
    struct Order{
        uint256 tokenId;
        uint256 price;
    }
    struct creatorListStruct{
        address[] listOfOwner;
    ...
    mapping (uint256 => creatorListStruct) private CreatorMapping;
    constructor(uint256 _serviceValue, uint256 _transvalue, uint256 id) public{
        serviceValue = _serviceValue;
        transvalue = _transvalue;
        _tid = id;
    }
    function deleteCreators(uint256 tokenId) internal{
        if(CreatorMapping[tokenId].listOfOwner.length > 0){
            delete CreatorMapping[tokenId];
        }
    }
    function getCreatorsList(uint256 tokenId) public view returns(address[] memory, uint256[] memory){
        return (CreatorMapping[tokenId].listOfOwner,CreatorMapping[tokenId].OwnerPercentage);
    }
    function setCreatorList(address[] memory _ownerList, uint256[] memory _percentage, uint256 tokenId) internal{
        require(_ownerList.length == _percentage.length, "Array Length not Matched");
        uint256 totalfee = 0;
        for(uint i = 0; i < _percentage.length; i++) {
            totalfee = _percentage[i].add(totalfee);
        }
        require(totalfee == uint256(100).mul(1e18),"Percentage split not Correct");
        creatorListStruct memory creatorlisting;
        creatorlisting = creatorListStruct({ 
            listOfOwner : new address[](0),  
            OwnerPercentage :new uint256[](0)    
        }); 
        CreatorMapping[tokenId] = creatorlisting;
        for(uint i = 0; i < _ownerList.length; i++) {
            CreatorMapping[tokenId].OwnerPercentage.push(_percentage[i]);   
            CreatorMapping[tokenId].listOfOwner.push(_ownerList[i]);
        }
    ...
    function _acceptBId(string memory tokenAss,address from, uint256 amount, uint256 tokenId, address[] memory owners, uint256[] memory percentvalue) internal{
        ...
        if(CreatorMapping[tokenId].listOfOwner.length > 0){
            for(uint i = 0; i < CreatorMapping[tokenId].listOfOwner.length; i++) {
                uint256 creatorsfee = percent(roy, CreatorMapping[tokenId].OwnerPercentage[i]);
                t.transferFrom(from,CreatorMapping[tokenId].listOfOwner[i],creatorsfee);
            }
        }
        else{
        ...
        if(owners[0] != address(0)){
            for(uint i = 0; i < owners.length; i++) {
                uint256 ownersfee = percent(netamount, percentvalue[i]);
                t.transferFrom(from,owners[i],ownersfee);
            }
        }
        else{
    ...
    function _saleToken(address payable from, uint256 tokenId, uint256 amount, address[] memory owners, uint256[] memory percentvalue) internal{
        ...
        if(CreatorMapping[tokenId].listOfOwner.length > 0){
            for(uint i = 0; i < CreatorMapping[tokenId].listOfOwner.length; i++) {
                address payable create = address(uint160(CreatorMapping[tokenId].listOfOwner[i]));
                uint256 creatorsfee = percent(roy, CreatorMapping[tokenId].OwnerPercentage[i]);
                create.transfer(creatorsfee);
            }
        }
        else{
        ...
        if(owners[0] != address(0)){
            for(uint i = 0; i < owners.length; i++) {
                address payable owner = address(uint160(owners[i]));
                uint256 ownersfee = percent(netamount, percentvalue[i]);
    ...
    function _saleShare(address payable from, uint256 amount, uint256 tokenId) internal{
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], serviceValue);
        for(uint i = 0; i < CreatorMapping[tokenId].listOfOwner.length; i++) {
                address payable create = address(uint160(CreatorMapping[tokenId].listOfOwner[i]));
                uint256 creatorsfee = percent(roy, CreatorMapping[tokenId].OwnerPercentage[i]);
        ...
        from.transfer(netamount);
        emit SaleShare(from, msg.sender);
    }

}
contract ARTIIExchangeContract is Sale{
    ...
    function saleShare(address payable from, uint256 amount, uint256 tokenId) public payable{
        _saleShare(from, amount, tokenId);
        
    }
    function tokenTrans(uint256 tokenId,address from, address to, uint256 NOFToken, uint256 _type, address token) internal{
        if(_type == 721){
           ARTII721 tok= ARTII721(token);
    ...
    function acceptBId(string memory bidtoken,address bidaddr, uint256 amount, uint256 tokenId, address token, uint256 _type, uint256 NOFToken,
    address[] memory owners, uint256[] memory percentvalue) public{
        _acceptBId(bidtoken, bidaddr, amount, tokenId, owners, percentvalue);
        if(_type == 721){
    ...
    function saleToken(address payable from, uint256 tokenId, uint256 amount, address token, uint256 _type, uint256 NOFToken, 
    address[] memory owners, uint256[] memory percentvalue) public payable{
       _saleToken(from, tokenId, amount, owners, percentvalue);
    ...
    function transferOwnershipForColle(address newOwner, address token721, address token1155) public onlyOwner {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        ARTII721 tok= ARTII721(token721);
        ARTII1155 tok1155= ARTII1155(token1155);
        tok._transferOwnership(newOwner);
        tok1155._transferOwnership(newOwner);
    }
    function mint(address token ,string memory tokenuri, uint256 value, uint256 tokenId, uint256 royal, uint256 _type, uint256 supply, address[] memory ownersList,
       uint256[] memory Percentage) public{
       require(_creator[tokenId] == address(0), "Token Already Minted");
       if(_type == 721){
           if(ownersList[0] != address(0)){
               require(ownersList[0] == msg.sender, "Creator Not found in Creator List");
               setCreatorList(ownersList, Percentage, tokenId);
           }
           ARTII721 tok= ARTII721(token);
