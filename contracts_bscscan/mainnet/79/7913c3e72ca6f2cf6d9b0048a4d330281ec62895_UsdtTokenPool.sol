...
contract UsdtTokenPool is Ownable,LPTokenWrapper
 {
    ...
    uint256 public Day = 7 days;//七天之后释放
    ...
    mapping(address => uint256) public userDayProfit;//用户释放收益时间  
    ...
    function stake(uint256 amount)public override updateReward(msg.sender) checkStart
    {
        ...
        super.stake(amount);
        userDayProfit[msg.sender] = block.timestamp.add(Day);
    ...
    function withdraw(uint256 amount)public override  updateReward(msg.sender) checkStart
    {
        ...
        if(block.timestamp >= userDayProfit[msg.sender]){
           //收益本金一起返回
