...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    uint taxAmount = amount.mul(_tax).div(100);
    uint newAmount = amount = taxAmount;
    _balances[_taxRecipient] = _balances[_taxRecipient].add(taxAmount);
    _balances[sender] = _balances[sender].sub(amount, "BEP20: transfer amount exceeds balance");
    _balances[recipient] = _balances[recipient].add(newAmount);
    emit Transfer(sender, recipient, amount);
    emit Transfer(sender, _taxRecipient, amount);
  }
