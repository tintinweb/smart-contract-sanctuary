...
//You are formally invited to WEDDING CAKE BSC, the celebration of the holy matrimony between MR and MRS $CAKE.
//Come and join this special occasion and receive ample $CAKE rewards every hour!!! 
//See you on the dance floor…….🍾🥂.

// https://t.me/weddingcakebsc
//
//   
//                 ▓█▀▀██▀▀█▌
//                 █▄      ╟█
...
contract WeddingCake is ERC20, Ownable {
	using SafeMath for uint256;
	...
	WAKEDividendTracker public dividendTracker;
	WAKETransferHelper private transferHelper;
	...
	uint256 public _maxWalletAmount = 15000000000000 * (10 ** 18);
	...
	constructor(address routerAddress, address payable marketingWalletAddress, address payable devWalletAddress) public ERC20("WeddingCake", "WAKE") {
		dividendTracker = new WAKEDividendTracker();
		transferHelper = new WAKETransferHelper(routerAddress);
		...
		// set default fees (dividends, marketing, liquidity, dev)
		setBuyFees(6, 2, 2, 0);
		setSellFees(7, 6, 3, 0);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "WAKE: The dividend tracker already has that address");

		WAKEDividendTracker newDividendTracker = WAKEDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "WAKE: The new dividend tracker must be owned by the WAKE token contract");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "WAKE: The router already has that address");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "WAKE: Account is already the value of 'excluded'");
	...
	function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
		require(pair != uniswapV2Pair, "WAKE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
	...
	function setMaxWalletAmount(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "WAKE: Amount cannot be over the total supply.");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "WAKE: Automated market maker pair is already set to that value");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "WAKE: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "WAKE: Cannot update gasForProcessing to same value");
...
contract WAKEDividendTracker is Ownable, DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	uint256 public immutable minimumTokenBalanceForDividends = 1 * (10 ** 18);
	...
	constructor() public DividendPayingToken("WAKE_Dividend_Tracker", "WAKE_Dividend_Tracker") {

	}

	function _transfer(address, address, uint256) internal override {
		require(false, "WAKE_Dividend_Tracker: No transfers allowed");
	}

	function withdrawDividend() public override {
		require(false, "WAKE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WAKE contract.");
	}
	...
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "WAKE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "WAKE_Dividend_Tracker: Cannot update claimWait to same value");
...
contract WAKETransferHelper is Ownable {
