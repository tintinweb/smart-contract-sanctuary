...
Telegaram : https://t.me/maxietoken
...
contract MAXIE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("MAXIE", "MAXIE") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0xE7F6462fEC3d280F17B47B1Dd388480ec80c94A1;
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "MAXIE: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "MAXIE: The new dividend tracker must be owned by the MAXIE token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("MAXIE_Dividend_Tracker", "MAXIE_Dividend_Tracker", 0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "MAXIE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "MAXIE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "MAXIE_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "MAXIE_Dividend_Tracker: Cannot update claimWait to same value");
