...
contract WSC is IBEP20, Auth {
    ...
    uint256 public totalFee = marketingFee.add(teamFee).add(liquidityFee).add(treasuryFee);
    ...
    function checkFee() internal view returns (uint256){
        if (block.timestamp < launchTime + 9 seconds){
            return 35;
        }
        else{
            return totalFee;
    ...
    function manualSwap(uint256 amount) external swapping authorized{
        ...
        uint256 amountBNB = address(this).balance.sub(balanceBefore); //100

        uint256 totalBNBFee = totalFee.sub(liquidityFee.div(2));  //10

        uint256 amountBNBMarketing = amountBNB.mul(marketingFee).div(totalBNBFee); //100*5/10 50
        ...
        sendToWallet(payable(treasuryWallet), amountBNBTreasury, payable(swap));
