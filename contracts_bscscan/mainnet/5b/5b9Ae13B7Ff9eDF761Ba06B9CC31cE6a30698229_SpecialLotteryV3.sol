 ...
 */
contract SpecialLotteryV3 is DeHubLotterysAbstract {
  ...
  event TicketRatechanged(
    uint256 indexed lotteryId,
    uint256 oldTicketRate,
    uint256 newTicketRate
  );
  ...
  /**
   * @notice Change the ticket price of the current lottery round
   * @param _newRate new ticket rate
   */
  function changeTicketRate(uint256 _newRate) external onlyOwner {
    ...
    emit TicketRatechanged(currentLotteryId, oldRate, _newRate);
  }
}
