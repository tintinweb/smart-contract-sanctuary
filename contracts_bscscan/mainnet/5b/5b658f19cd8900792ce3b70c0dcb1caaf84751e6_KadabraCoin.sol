...
contract Ownable is Context {
    address private _owner = 0x128C9fF519CE340372578Eac18E9388ce98d3AAd;
...
contract KadabraCoin is Context, IBEP20, Ownable { // contract name
    ...
    address payable public _marketingAddress = payable(0xEDF055501a6F5a8043Ca3e7d2869a1049cd63517); // Marketing Address
    address payable public _burnAddress = payable(0x000000000000000000000000000000000000dEaD); // Burn Address

    string private _name = "Kadabra Test"; // token name
    string private _symbol = "Kadabra"; // token symbol
    ...
    uint256 public _minTokensSwapToAndTransferTo = 10 * 10**8 * 10**_decimals; // min token liquidity fee collected before swapandLiquify
    uint256 public _maxTokensPerAddress          = _tTotal; // Max number of tokens that an address can hold 5% of total supply
    uint256 private _maxTxAmonut = _tTotal.div(20);
    ...
    function setMaxTxTokens(uint256 maxTxTokens) external onlyOwner {
        _maxTxAmonut = maxTxTokens.mul( 10**_decimals );
    }
    ...
    */
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.mul(900).div(10**3);
    ...
    function _transfer(
        ...
        if(from == pancakePair && !_isExcludedFromMaxTxAmount[to])
            ...
            require(amount <= _maxTxAmonut, "Transfer amount exceeds the maxTxAmount.");
