...
contract Pluteus is Context, IERC20, Ownable, ITimelockable {
    ...
    constructor() {
        ...
        PLUTEUS_ADDRESS = address(0x23A590FE704F9987430dE05b6e7eC92530e6ad3a);
        ...
        _isExcludedFromFee[PLUTEUS_ADDRESS] = true;

        _isExcludedFromCap[owner()] = true;
        _isExcludedFromCap[address(this)] = true;
        _isExcludedFromCap[PLUTEUS_ADDRESS] = true;
    ...
    function transferFrom(
        ...
        if (!_isExcludedFromCap[recipient]) {
            require(
                (balanceOf(recipient) + amount) <= (_tTotal * uint256(1).div(100)),
                "User can only hold max 1% of total supply"
            );
        }

        if (sender == PLUTEUS_ADDRESS) {
            // Lock tokens outgoing of PLUTEUS FUND for 2 days
            require(!recipient.isContract(), "Resctriced to EOA"); // Prevent Timelock bypass
            uint256 releaseTime = block.timestamp + 172800;
            TokenTimelock ttl = new TokenTimelock(
                address(this),
                recipient,
                releaseTime
            );
...
//          swapAndLiquify(contractTokenBalance);
    ...
    event AddressExcludedFromCap(address account);

    function excludeFromCap(address account) public onlyOwner {
        _isExcludedFromCap[account] = true;
        emit AddressExcludedFromCap(account);
    }

    event AddressIncludedInCap(address account);

    function includeInCap(address account) public onlyOwner {
        _isExcludedFromCap[account] = false;
        emit AddressIncludedInCap(account);
    }
}
