...
contract WellbeCoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    WellbeCoinDividendTracker public dividendTracker;

    address public liquidityWallet;

    address payable public operations1Address =
        payable(0x122611B0AffBaFAB0fa9c68c790194517633d2eB);
    address payable public operations2Address =
        payable(0x80D45fC9fA18213b9d4f6A70641012Bc1c7CFaB1);
    ...
    constructor() ERC20("WellbeCoin", "WELB") {
        dividendTracker = new WellbeCoinDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        WellbeCoinDividendTracker newDividendTracker = WellbeCoinDividendTracker(
            payable(newAddress)
        );
...
contract WellbeCoinDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "WellbeCoin_Dividend_Tracker",
            "WellbeCoin_Dividend_Tracker"
        )
    {
        wellbeCoin = WellbeCoin(mainContract);
        minimumTokenBalanceForAutoDividends = 250000000 * (10**9);
        minimumTokenBalanceForDividends = 200 * (10**9);
    ...
    function _transfer(
        ...
        require(false, "WellbeCoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "WellbeCoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WellbeCoin contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(wellbeCoin);

            uint256 prevBalance = wellbeCoin.balanceOf(address(this));
            ...
            {
                uint256 received = wellbeCoin.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    wellbeCoin.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
