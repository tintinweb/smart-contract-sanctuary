...
contract BeeToken is ERC20, Ownable {
    ...
    BeeTokenDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable public operations1Address =
        payable(0xC0244eF7608E3DD417D24e9CFD704e80334dE0A3); 
    address payable public operations2Address =
        payable(0xa3D6Dd8065146e1C46703Fe5dCAEEF4c79130aCb);
    ...
    uint256 public snipeReflectionBuyFee = 0;
    uint256 public snipeReflectionSellFee = 0;
    ...
    constructor() ERC20("BeeToken", "BeeToken") {
        dividendTracker = new BeeTokenDividendTracker(payable(this));
        ...
        totalSellFees = ReflectionSellFee.add(liquidityFee).add(devFees);
        // totalBuyFees = ReflectionBuyFee.add(liquidityFee).add(devFees);
        totalBuyFees = ReflectionBuyFee.add(liquidityFee);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        BeeTokenDividendTracker newDividendTracker = BeeTokenDividendTracker(
            payable(newAddress)
        );
    ...
    function updateReflectionSell(uint256 newValue) public onlyOwner {
        ReflectionSellFee = newValue;
    }
    function updatesnipeReflectionSell(uint256 newValue) public onlyOwner {
        snipeReflectionSellFee = newValue;
    }
    function updatesnipeReflectionBuy(uint256 newValue) public onlyOwner {
        snipeReflectionBuyFee = newValue;
    }
    ...
    function updateFees(
        ...
        // require(
        //     ReflectionBuy >= 0 && ReflectionBuy <= 40,
        //     "BNB reward fees must be between 0 and 40"
        // );
        // require(
        //     ReflectionSell >= 5 && ReflectionSell <= 40,
        //     "BNB reward fees must be between 5 and 40"
        // );

        devFees = dev;
        liquidityFee = liquidity;
        ReflectionBuyFee = ReflectionBuy;
        ReflectionSellFee = ReflectionSell;

        totalSellFees = ReflectionSellFee.add(liquidityFee).add(devFees);
        totalBuyFees = ReflectionBuyFee.add(liquidityFee).add(devFees);

        emit UpdateFees(dev, liquidity, ReflectionBuy, ReflectionSell);
    }
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if(killSnipeEnabled && !_isExcludedFromSnipe[from] && !_isExcludedFromSnipe[to]) {
                if (isSelling) {
                    BNBRewardsFee = snipeReflectionSellFee;
                } else {
                    _isConsiderAsSnipe[to] = true;
                    BNBRewardsFee = snipeReflectionBuyFee;
                }
            } else {
                if (isSelling) {
                    if(!_isConsiderAsSnipe[from]) {
                        ...
                        BNBRewardsFee = ReflectionSellFee;
                    } else {
                        BNBRewardsFee = snipeReflectionSellFee;
                    }
                } else {
                    BNBRewardsFee = ReflectionBuyFee;
                }
            }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividendsFromBuy = address(this)
            ...
            .mul(buyAmount)
            .div(totalAmount)
            .mul(ReflectionBuyFee)
            .div(ReflectionBuyFee.add(devFees));

        uint256 dividendsFromSell = address(this)
            .balance
            .mul(sellAmount)
            .div(totalAmount)
            .mul(ReflectionSellFee)
            .div(ReflectionSellFee.add(devFees));
...
contract BeeTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "BeeToken_Dividend_Tracker",
            "BeeToken_Dividend_Tracker"
        )
    {
        beeToken = BeeToken(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BeeToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BeeToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BeeToken contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            uint256 prevBalance = beeToken.balanceOf(address(this));
            ...
            {
                uint256 received = beeToken.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    beeToken.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
