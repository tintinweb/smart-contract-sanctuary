...
contract Dc is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress =
        payable(0x2011cfA169c5725ab1d1F9194D8f5e5e9c84c06B); // Marketing Address
        
    address payable public rewardAddress =
        payable(0x2011cfA169c5725ab1d1F9194D8f5e5e9c84c06B); // Reward Address
    ...
    string private constant _name = "DCT";
    ...
    uint256 private _rewardTokensToSwap = 5000;
    ...
    event UpdatedRewardAddress(address buyback);
    ...
    function _transfer(
     ...
     if(limitsInEffect){
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _marketingTokensToSwap + _rewardTokensToSwap;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyRewardFee;
                buyOrSellSwitch = BUY;
            } 
            // Sell
                ...
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellRewardFee;
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap.add(_rewardTokensToSwap).add(_marketingTokensToSwap);
        ...
        uint256 bnbForReward = bnbBalance.mul(_rewardTokensToSwap).div(totalTokensToSwap);
        
        uint256 bnbForLiquidity = bnbBalance.sub(bnbForMarketing).sub(bnbForReward);

        _liquidityTokensToSwap = 0;
        _marketingTokensToSwap = 0;
        _rewardTokensToSwap = 0;
        
        (bool success,) = address(rewardAddress).call{value: bnbForReward}("");
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY){
            ...
            _rewardTokensToSwap += tLiquidity * _buyRewardFee / _liquidityFee;
            ...
            _rewardTokensToSwap += tLiquidity * _sellRewardFee / _liquidityFee;
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyRewardFee)
        external
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyRewardFee <= 20, "Must keep taxes below 20%");
        emit SetBuyFee(buyMarketingFee, buyLiquidityFee, buyTaxFee, buyRewardFee);
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellRewardFee)
        external
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellRewardFee <= 30, "Must keep taxes below 30%");
        emit SetSellFee(sellMarketingFee, sellLiquidityFee, sellTaxFee, sellRewardFee);
    }
    ...
    function setRewardAddress(address _rewardAddress) external onlyOwner {
        ...
        _isExcludedFromFee[rewardAddress] = true;
        emit UpdatedRewardAddress(rewardAddress);
    }
