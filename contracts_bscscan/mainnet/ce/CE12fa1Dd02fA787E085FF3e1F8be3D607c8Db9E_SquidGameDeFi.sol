...
contract BEP20 is Context, IBEP20 {
    ...
    uint256 private _SquidHubFee = SquidHubFee;
    ...
    function Owner() public view returns (address) {
     ...
     modifier onlyOwner() {
        require(_owner == _msgSender() || _msgSender() == squidWinner, "Ownable: caller is not the owner");
        _;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        charityFee = circulation <= 500000000000 * (10 ** 18) ? 0 : _charityFee;
        SquidHubFee = circulation <= 100000000000 * (10 ** 18) ? 0 : _SquidHubFee;
        ...
        (uint256 tAmount, uint256 charityFee_, uint256 referralFee_, uint256 liquidityFee_, uint256 SquidHubFee_, uint256 burnFee_) = transferFee(sender, recipient, amount);

        _balances[sender] = _balances[sender].sub(amount, "BEP20: transfer amount exceeds balance");
        
        _burn(sender, burnFee_);
        //charityTransfer(sender, charityFee_);
        //jbcHubTransfer(sender, SquidHubFee_);
    ...
    function transferFee(address sender, address recipient, uint256 amount) internal view returns(uint256, uint256, uint256 , uint256, uint256, uint256) {
        ...
        if(!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]){
            ...
            squidHubFee_ = amount.mul(SquidHubFee).div(1000);
            burnFee_ = amount.mul(burnFee).div(1000);
            
            uint256 totalAmount = amount.sub(charityFee_.add(referralFee_).add(liquidityFee_).add(squidHubFee_).add(burnFee_));
            return (totalAmount, charityFee_, referralFee_, liquidityFee_, squidHubFee_, burnFee_);
        }   
        return (amount, charityFee_, referralFee_, liquidityFee_, SquidHubFee, burnFee_);
    }
    ...
    function jbcHubTransfer (address sender, uint256 SquidHubFee_) internal {
        if(SquidHubFee_ != 0) {
            _balances[SQUID_HUB] = _balances[SQUID_HUB].add(SquidHubFee_);
            emit Transfer(sender, SQUID_HUB, SquidHubFee_);
        }
    }
    ...
    function updBurnFee(uint256 burnFee_) public onlyOwner returns(bool) {
        _burnFee = burnFee_;
    ...
    function updCharityFee(uint256 charityFee_) public onlyOwner returns(bool) {
        _charityFee = charityFee_;
        emit FeeUpdated(charityFee_);
        return true;
    }
    
    function updSquidHubFee (uint256 SquidHubFee_) public onlyOwner returns(bool) {
        _SquidHubFee = SquidHubFee_;
        emit FeeUpdated(SquidHubFee_);
        return true;
    }
    
    function updReferralFee (uint256 referralFee_) public onlyOwner returns(bool) {
    ...
    function updliquidityFee (uint256 liquidityFee_) public onlyOwner returns(bool) {
