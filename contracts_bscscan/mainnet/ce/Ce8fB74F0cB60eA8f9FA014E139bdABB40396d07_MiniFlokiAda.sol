...
contract MiniFlokiAda is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    event RewardClaimedSuccessfully(address indexed recipient, uint256 cakeReceived, uint256 nextAvailableClaimDate, uint256 timestamp);
    ...
    constructor(address _rewardToken, ITopHolderRewardDistributor _topHolderRewarDistributor, address _router,uint interval,address _owner,address _marketingWallet, address _charityWallet) public {
        ...
        rewardToken = _rewardToken;//0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82;
        marketingWallet = _marketingWallet;//0x4d50DA60fE164904074A78C82F6024548342b1dC;
        charityWallet = _charityWallet;//0xc499a2E63d38dE517789037e25D69e4Fbbc55eF3;
    ...
    function _transfer(
        ...
        if (!inSwap && isFeeActive && !isTaxless[_msgSender()] && !isTaxless[recipient] && !inSwap) {
    ...
    function topUpClaimCycleAfterTransfer(
        ...
        if (recipient == pair && currentSenderBalance == amount) {
            ...
            nextAvailableClaimDate[recipient] = rewardCycleInterval;//calculateTopUpClaim(currentRecipientBalance, amount);
        }
    }
