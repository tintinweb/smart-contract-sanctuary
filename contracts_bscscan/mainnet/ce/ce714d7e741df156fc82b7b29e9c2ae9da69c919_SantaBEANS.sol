...
contract SBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SBNB_Dividend_Tracker", "SBNB_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "SBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "SBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SBNB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "SBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SBNB_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SantaBEANS is ERC20, Ownable {
    IPinkAntiBot public pinkAntiBot;
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    SBNBDividendTracker public dividendTracker;

    address public _marketingWallet = 0x57461A7Ce9dE8F430c3E5519D9A60fCe8d438906;
    ...
    constructor() ERC20("SantaBEANS", "SantaBEANS") {

        dividendTracker = new SBNBDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SBNB: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SBNB: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SBNB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SBNB: Cannot update gasForProcessing to same value");
