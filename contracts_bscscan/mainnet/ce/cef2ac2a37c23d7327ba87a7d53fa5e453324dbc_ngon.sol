...
/*
 *  N-Gon: Ecosystem TEST
 *  Website: https://ngon.tech/
*/
...
contract NGonDividendDistributor is IDividendDistributor {
    ...
    function setRewardToken(IBEP20 newrewardToken) external onlyToken{
    ...
    function deposit() external payable override onlyToken {
        ...
        totaldividendsOfToken[address(rewardtoken)] = totaldividendsOfToken[address(rewardtoken)].add(amount);
        
        dividendsPerShareRewardToken[address(rewardtoken)] = dividendsPerShareRewardToken[address(rewardtoken)].add(dividendsPerShareAccuracyFactor.mul(amount).div(totalShares));
    }
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            rewardshares[address(rewardtoken)][shareholder].totalRealised = rewardshares[address(rewardtoken)][shareholder].totalRealised.add(amount);
            rewardshares[address(rewardtoken)][shareholder].totalExcluded = getCumulativeDividends(rewardshares[address(rewardtoken)][shareholder].amount);
        }
    }
...
contract ngon is IBEP20, Auth {
    ...
    string constant _name = "N-GonTEST";
    string constant _symbol = "testngon";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 75000000000000000;
...
*/
    ...
    uint256 public devmarketingFee = 50;
    uint256 public developmentfee = 25;
    uint256 public lotteryPot = 25;
    uint256 public rewardtokenFee = 150;
    uint256 public sellpremium = 1;
    uint256 public totalFee = 350;
    uint256 public feeDenominator = 10000;

    address public autoLiquidityReceiver;
    address public devmarketingFeeReceiver;
    ...
    NGonDividendDistributor public distributor;
    ...
    constructor (
        ...
        distributor = new NGonDividendDistributor(address(router));
        ...
        devmarketingFeeReceiver = msg.sender;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(teamwallets[sender] || teamwallets[recipient]){
            require(teamCanTrade, "team isnt allowed to trade");
        }
    ...
    // returns any miss-sent tokens to the marketing wallet
    function claimtokensback(IBEP20 tokenAddress) external authorized {
        payable(devmarketingFeeReceiver).transfer(address(this).balance);
        tokenAddress.transfer(devmarketingFeeReceiver, tokenAddress.balanceOf(address(this)));
    }
    ...
    function setFees(uint256 _liquidityFee,  uint256 _reflectionFee, uint256 _devmarketingFee, uint256 _devFee, uint256 _charityFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        devmarketingFee = _devmarketingFee;
        developmentfee = _devFee;
        lotteryPot = _charityFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_devmarketingFee).add(developmentfee).add(lotteryPot);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBMarketing = amountBNB.mul(devmarketingFee).div(totalBNBFee);

       
       // payable(devmarketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (bool dmfrsuccess, /*bytes memory data*/) = payable(devmarketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
            require(dmfrsuccess, "receiver rejected ETH transfer");
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _devmarketingFeeReceiver, address devWallet) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        devmarketingFeeReceiver = _devmarketingFeeReceiver;
