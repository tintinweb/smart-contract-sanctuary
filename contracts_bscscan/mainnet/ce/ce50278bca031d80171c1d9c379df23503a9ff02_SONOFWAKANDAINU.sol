...
contract SONOFWAKANDAINU is Context, IERC20, Ownable {
    ...
    string private  _name               = "Son Of Wakanda Inu";
    string private  _symbol             = "SOWI";
    ...
    constructor () {
        ...
        _wallet_marketing = payable(0x33109A8BbF503ed78EDe17C3c6B919bdB9abf339);
        _wallet_team = payable(0x277BdadF7A82Ab1a9C5Cac664abfdF748aFF3486);
        _wallet_charity = payable(0x61Ff3eD77D679b0767B49F3fA2F5E209F5a87DE0);
        _wallet_buyback = payable(0x4D8993210445BD5B3E76AF4FEAFd9e2D78Fa3841);
        _wallet_utility = 0x2A47a07e91d36d8DD4Be4AfEC1D4D3b8708dB534;
...
/*  Wallet Management  */
    ...
    function buyback_mode (bool _enabled) external onlyOwner() {
...
// Reflect Finance core code
    ...
    function _getValues(uint256 tAmount, bool isSell) private view returns (
        ...
        uint256 tTransferAmount,uint256 tUtility, uint256 tMarketing, uint256 tLiquidity, uint256 tTeam, uint256 tReflection) {
        ...
        tTeam           = ( tAmount * _fee_team * 2 ) * multiplier / (_fee_denominator * 100); // taken 2x, one for charity, one for team, to avoid stack too deep error with too many taxes/variables
        tReflection     = ( tAmount * _fee_reflection ) * multiplier  / (_fee_denominator * 100);

        tTransferAmount = tAmount - (tUtility + tMarketing + tLiquidity + tTeam + tReflection);
...
// Made all parameters in alphabetical order
    function _setAllFees(uint256 utilityFees, uint256 marketingFee, uint256 liquidityFees, uint256 teamFee, uint256 reflectionFees) private {
        _fee_utility           = utilityFees;
    ...
    function set_All_Fees(uint256 Team_Fee, uint256 Utility_Fees, uint256 Liquidity_Fees, uint256 Reflection_Fees, uint256 MarketingFee) external onlyOwner {
        uint256 total_fees = Utility_Fees + MarketingFee + Liquidity_Fees +  Team_Fee + Reflection_Fees;
        require(total_fees < 4000, "Cannot set fees this high, pancake swap will hate us!");
        _setAllFees( Utility_Fees, MarketingFee, Liquidity_Fees, Team_Fee, Reflection_Fees);
    }
    ...
    function restoreAllFee() private {
        _setAllFees(_fee_utility_old, _fee_marketing_old, _fee_liquidity_old, _fee_team_old, _fee_reflection_old);
    }
...
// Liquidity functions
    ...
    function swapAndLiquify(uint256 tokensToSwap) private lockTheSwap {
        ...
        if(buybackEnabled){
            swapTokensForEthAndSend(tokensToSwap,_wallet_buyback);
        } else{
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // extra bracket to supress stack too deep error
        {
            ...
            if(!inSwapAndLiquify && from != uniswapV2Pair && swapAndLiquifyEnabled){
                ...
                _fees_to_bnb(_fee_team_convert_limit,_wallet_charity, _fee_team_min_bal);
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rReflection, uint256 tTransferAmount, uint256 tUtility, uint256 tMarketing, uint256 tLiquidity, uint256 tTeam,  uint256 tReflection) = _getValues(amount, (to == uniswapV2Pair));
