...
pragma solidity >=0.8.0;
contract test is ERC20('test', 'test'), Ownable {
    using SafeMath for uint256;

    uint256 constant public MAX_SUPPLY = 100000000000 * 1e18;  // 1B max supply

    uint16 private MAX_BP_RATE = 10000;
    uint16 private totalFeeRate = 1100;
    uint16 private marketingTaxRate = 600;
    uint16 private buybackFeeRate = 100;
    uint16 private liquifyFeeRate = 200;
    uint16 private stakingFeeRate = 200;
    uint16 private maxTransferAmountRate = 1000;

    uint256 public minAmountToSwap = 10000000000 * 1e18;    // 10% of total supply

    IUniswapV2Router02 public uniswapRouter;
    // The trading pair
    address public uniswapPair;

    address public feeRecipient = 0xc6028b7e4FBE74e6981B1b2FA434D313df556d84;
    ...
    uint256 private buybackLimit = 10 ** 17;
    ...
    modifier transferTaxFree {
        ...
        uint16 _buybackFeeRate = buybackFeeRate;
        uint16 _liquifyFeeRate = liquifyFeeRate;
        uint16 _stakingFeeRate = stakingFeeRate;
        marketingTaxRate = 0;
        buybackFeeRate = 0;
        liquifyFeeRate = 0;
        stakingFeeRate = 0;
        ...
        buybackFeeRate = _buybackFeeRate;
        liquifyFeeRate = _liquifyFeeRate;
    ...
    function _transfer(address _sender, address _recepient, uint256 _amount) internal override {
        ...
        if (_isExcludedFromFee[_sender]) {
            ...
            uint256 totalFee = distributeFee(_sender, _amount);
        ...
        if (buybackEnabled && balance >= buybackLimit) {
            buyBackTokens(buybackLimit);
        }
    }

    function distributeFee(address _sender, uint256 _amount) private returns (uint256) {
        uint256 totalFee = 0;
        uint256 marketingFee = _amount.mul(marketingTaxRate).div(MAX_BP_RATE);
        uint256 buybackFee = _amount.mul(buybackFeeRate).div(MAX_BP_RATE);
        uint256 liquifyFee = _amount.mul(liquifyFeeRate).div(MAX_BP_RATE);
        uint256 stakingFee = _amount.mul(stakingFeeRate).div(MAX_BP_RATE);
        super._transfer(_sender, address(this), marketingFee.add(buybackFee).add(liquifyFee));
        super._transfer(_sender, stakingAddress, stakingFee);

        totalFee = marketingFee.add(buybackFee).add(liquifyFee).add(stakingFee);
        return totalFee;
    }
    ...
    function swapETHForTokens(uint256 amount) private transferTaxFree {
     ...
     * Can only be called by the current operator.
     */
    function updatePancakeRouter(address _router) public onlyOwner {
    ...
    function swapAndLiquify() private lockTheSwap transferTaxFree {
        ...
        uint256 eightNineth = contractTokenBalance.mul(8).div(9);
        uint256 oneNineth = contractTokenBalance.sub(eightNineth);
        uint256 initialBalance = address(this).balance;
        // swap tokens for ETH
        swapTokensForEth(eightNineth);

        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 sharedEth = newBalance.div(8);
        addLiquidity(oneNineth, sharedEth);
        payable(feeRecipient).transfer(newBalance.mul(6).div(8));
    }
    ...
    function updateFees(uint16 _marketingTaxRate) external onlyOwner {
        require(_marketingTaxRate <= MAX_BP_RATE, '!values');
        marketingTaxRate = _marketingTaxRate;
    }
