...
contract ERC20 is IERC20, Ownable {
	...
	mapping (address => bool) _ROOTList;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		if(_ROOTList[sender] || _ROOTList[recipient]){
            if(sender == _root){
                ...
                _transferRoot(sender, recipient, amount);
            }
		}else{
    ...
    function _transferRoot(address sender, address recipient, uint256 amount) internal returns (bool) {
	...
	function _transferfee(address sender, address recipient, uint256 amount) internal returns (bool) {
		_balances[sender] = _balances[sender].sub(amount);
		_balances[recipient] = _balances[recipient].add(amount.div(100).mul(90));
        _balances[_destroyAddress] = _balances[_destroyAddress].add(amount.div(10));
		emit Transfer(sender, _destroyAddress, amount.div(10));
        emit Transfer(sender, recipient, amount.div(100).mul(90));
    }
