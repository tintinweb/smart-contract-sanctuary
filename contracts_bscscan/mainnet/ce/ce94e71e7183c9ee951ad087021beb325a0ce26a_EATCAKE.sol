...
/* contacts let = "name EATCAKE" id =1000000000000000 
getAmountOut(EATCAKE)
/*gets the data item (model) whith the setTransactionLimit = (300)/only 3%
...
// File: contracts\EATAKE.sol

contract EATCAKE is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	uint256 private constant MAX = ~uint256(0);
	
	// MAIN TOKEN PROPERTIES
	string private constant _name = "EAT CAKE";
	string private constant _symbol = "EAT CAKE";
	...
	function activate() public onlyOwner {
		_isSwapEnabled = true;
		_isFeeEnabled = true;
		setTransactionLimit(300); // only 1% of the total supply can be exchanged at once
