...
contract CaptainDoge is ERC20, Ownable {
   ...
   CaptainDogeDividendTracker public dividendTracker;
    ...
    uint256 public totalFees = _RewardsFee.add(_liquidityFee).add(_marketingFee).add(charityFee);
    ...
    constructor() ERC20 ("CaptainDoge", "CDOG") {
        ...
        maxSellTransactionAmount = 75000000000 * (10**18);
        swapTokensAtAmount = 150000000000 * (10**18);
        _maxWalletToken = 300000000000 * (10**18); // 2% of total supply
    ...
    function _transfer(
        ...
        if(takeFee) {
        	...
        	charityAmount = fees.mul(charityFee).div(100);
        	marketingAmount = fees.mul(marketingFee).div(100);
    ...
    function restoreAllFee() external onlyOwner() {
        ...
        _maxWalletToken = 300000000000 * 10**18;
        maxBuyTranscationAmount = 300000000000 * 10**18;
        maxSellTransactionAmount = 75000000000 * 10**18;
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "Captain Doge: Marketing tax must be between 0 and 10");
        marketingFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "Captain Doge: Liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
    }
    
    function updateRewardsFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 20, "Captain Doge: Rewards tax must be between 0 and 20");
        liquidityFee = newFee;
    }
    
    function updateCharityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 20, "Captain Doge: Charity Fee must be between 0 and 20");
        charityFee = newFee;
    }
