...
contract Pledge is Ownable {
    ...
    event UnStake(address indexed sender_, uint indexed amount_, uint indexed slot_, uint mode_);
    ...
    function unStake(uint slot_, uint mode_) isStart checkTime checkHolder public {
        ...
        if (mode_ == 0) {
             tempAmount= userSlotSPE[msg.sender][slot_].stakeAmount;
             tempPower= userSlotSPE[msg.sender][slot_].power;
            require(userSlotSPE[msg.sender][slot_].stakeAmount > 0, '2');
            ...
            if (temp != 0) {
             ...
             tempAmount = userSlotNBTC[msg.sender][slot_].stakeAmount;
             tempPower = userSlotNBTC[msg.sender][slot_].power;
            require(userSlotNBTC[msg.sender][slot_].stakeAmount > 0, '2');
            require(userSlotNBTC[msg.sender][slot_].endTime < block.timestamp, 'not time');
            uint temp = claimStatic(slot_, mode_);
            SpeToken.transfer(msg.sender, tempAmount + (temp * 99 / 100));
            SpeToken.transfer(address(0), temp / 100);
            burnTotal += temp / 100;
        ...
        emit UnStake(msg.sender, tempAmount, slot_,mode_);
    }
    ...
    function setAddress(address addr_, address token_, address claim_) public onlyOwner {
        air = AirDrop(addr_);
    ...
    function check(address addr_) public view returns (uint spe, uint nbtc, uint stakeSpe, uint stakeNBTC, uint vouchers, uint comm, uint node, uint quota){
        ...
        (comm,node,quota)= claim.userClaim(addr_);
