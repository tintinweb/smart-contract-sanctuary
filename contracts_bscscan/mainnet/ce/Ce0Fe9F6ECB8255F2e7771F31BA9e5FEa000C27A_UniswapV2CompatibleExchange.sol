...
contract UniswapV2CompatibleExchange is IExchange, Ownable, ReentrancyGuard
{
	...
	{
		address[] memory _path = _buildPath(_from, _to);
		if (_path.length == 1) return _inputAmount;
		return IUniswapV2CompatibleRouter(router).getAmountsOut(_inputAmount, _path)[_path.length - 1];
	}
	...
	function calcConversionFromOutput(address _from, address _to, uint256 _outputAmount) external view override returns (uint256 _inputAmount)
	{
		...
		return IUniswapV2CompatibleRouter(router).getAmountsIn(_outputAmount, _path)[0];
	}
	...
	function convertFundsFromInput(address _from, address _to, uint256 _inputAmount, uint256 _minOutputAmount) external payable override nonReentrant returns (uint256 _outputAmount)
	{
		...
		if (_path.length == 1) {
			...
			IERC20(_token).safeApprove(router, _inputAmount);
			_outputAmount = IUniswapV2CompatibleRouter(router).swapExactTokensForTokens(_inputAmount, _minOutputAmount, _path, address(this), uint256(-1))[_path.length - 1];
	...
	function convertFundsFromOutput(address _from, address _to, uint256 _outputAmount, uint256 _maxInputAmount) external payable override nonReentrant returns (uint256 _inputAmount)
	{
		...
		if (_path.length == 1) {
			...
			address _token = _from == address(0) ? WTOKEN : _from;
			IERC20(_token).safeApprove(router, _maxInputAmount);
			_inputAmount = IUniswapV2CompatibleRouter(router).swapTokensForExactTokens(_outputAmount, _maxInputAmount, _path, address(this), uint256(-1))[0];
			IERC20(_token).safeApprove(router, 0);
		}
	...
	function _buildPath(address _from, address _to) internal view returns (address[] memory _path)
	{
		...
		address _factory = IUniswapV2CompatibleRouter(router).factory();
	...
	event ChangeRouter(address indexed _oldRouter, address indexed _newRouter);
