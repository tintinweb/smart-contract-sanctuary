...
Twitter :https://www.twitter.com/VenomFloki
...
//SPDX-License-Identifier: MIT
	 ...
	 * This allows applications to reconstruct the allowance for all acVenomFloki just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also VenomFlokied.
	...
	// File: contracts/VenomFlokiDividendPayingToken.sol
	...
	// File: contracts/VenomFloki.sol

	// 

	pragma solidity ^0.7.0;

	contract VenomFloki is ERC20, Ownable {
		using SafeMath for uint256;

		IuniswapV2Router02 private uniswapV2Router;
		address private immutable uniswapV2Pair;

		address payable public _MarketingAddress = payable(0x43a3F5e57792D4c186A749e8af8be64079875b70); 
	
		bool private swapping;

		VenomFlokiDividendTracker public dividendTracker;
		...
		event ExcludeMultipleAcVenomFlokiFromFees(address[] acVenomFloki, bool isExcluded);
		...
		constructor() ERC20("VenomFloki", "VENOMFLOKI"){
			...
			dividendTracker = new VenomFlokiDividendTracker();
		...
		function excludeFromFees(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "VenomFloki: Account is already the value of 'excluded'");
		...
		function excludeMultipleAcVenomFlokiFromFees(address[] calldata acVenomFloki, bool excluded) public onlyOwner {
			for(uint256 i = 0; i < acVenomFloki.length; i++) {
				_isExcludedFromFees[acVenomFloki[i]] = excluded;
			}

			emit ExcludeMultipleAcVenomFlokiFromFees(acVenomFloki, excluded);
		}

		function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
			require(pair != uniswapV2Pair, "VenomFloki: The uniswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "VenomFloki: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "VenomFloki: The liquidity wallet is already this address");
		...
		function updateGasForProcessing(uint256 newValue) private onlyOwner {
			require(newValue >= 200000 && newValue <= 500000, "VenomFloki: gasForProcessing must be between 200,000 and 500,000");
			require(newValue != gasForProcessing, "VenomFloki: Cannot update gasForProcessing to same value");
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if(!hasLaunched) {
				require(canTransferBeforehasLaunched[from], "VenomFloki: This account cannot send tokens until trading is enabled");
			}
	...
	contract VenomFlokiDividendTracker is DividendPayingToken, Ownable {
		using SafeMath for uint256;
		using SafeMathInt for int256;
		...
		constructor() public DividendPayingToken("VenomFloki_Dividend_Tracker", "VenomFloki_Dividend_Tracker") {
			claimWait = 3600;
			minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
		}

		function _transfer(address, address, uint256) internal override {
			require(true, "VenomFloki_Dividend_Tracker: No transfers allowed");
		}
		...
		function updateClaimWait(uint256 newClaimWait) external onlyOwner {
			require(newClaimWait >= 1 && newClaimWait <= 86400, "VenomFloki_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
			require(newClaimWait != claimWait, "VenomFloki_Dividend_Tracker: Cannot update claimWait to same value");
