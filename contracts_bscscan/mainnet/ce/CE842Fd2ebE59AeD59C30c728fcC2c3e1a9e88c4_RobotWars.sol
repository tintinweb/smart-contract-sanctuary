...
contract RobotWars is BEP20 {
    using SafeMath for uint256;
    uint256 public maxSupply = 1 * 10**8 * 10**18;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;

    uint256 public farmerFee = 350;
    uint256 public devRewardFee = 25;
    uint256 public LPFee = 125;
    
    bool public antiWhale = false;
    uint256 public maxTranfer = 100000_000000000000000000;
     ...
     */
    constructor() public BEP20("RobotWars", "WAR") {
    ...
    function _transfer( address sender, address recipient, uint256 amount ) internal virtual override {
        ...
        if(takeFee){
            if(antiWhale && amount > maxTranfer){
                revert("Anti Whale");
            }
            
            uint256 _farmerFee = amount.mul(farmerFee).div(10000);
            uint256 _devRewardFee = amount.mul(devRewardFee).div(10000);
            uint256 _LPFee = amount.mul(LPFee).div(10000);
    ...
    function disableAntiWhale() public onlyOwner {
        antiWhale = false;
    }
    ...
    function updateMaxTranferInAntiWhaleMode(uint256 _maxTranfer) public onlyOwner {
        maxTranfer = _maxTranfer;
    }
