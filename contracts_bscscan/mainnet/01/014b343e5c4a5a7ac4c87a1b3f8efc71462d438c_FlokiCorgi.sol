...
+  Marketing, airdop:  5.000.000.000.000 (5%)
 ...
 16% Transaction Fees
 10% is auto redistributed to all holders In BNB Hold $FlokiCorgi tokens, earn BNB. 
...
Website: https://flokicorgi-inu.com
Telegram: https://t.me/flokicorgi
Twitter: https://twitter.com/CorgiFloki
*/
...
contract FlokiCorgi is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "FlokiCorgi Inu";
    string constant _symbol = "FlokiCorgi";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 520 * 10**5 * (10 ** _decimals); 
    ...
    function triggerManualBuyback(uint256 amount) external onlyOwner {
        buyFlokiCorgiToken(amount, DEAD);
    }

    function triggerAutoBuyback() internal {
        buyFlokiCorgiToken(autoBuybackAmount, DEAD);
        autoBuybackBlockLast = block.number;
        autoBuybackAccumulator = autoBuybackAccumulator.add(autoBuybackAmount);
        if(autoBuybackAccumulator > autoBuybackCap){ autoBuybackEnabled = false; }
    }

    function buyFlokiCorgiToken(uint256 amount, address to) internal swapping {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(this);
