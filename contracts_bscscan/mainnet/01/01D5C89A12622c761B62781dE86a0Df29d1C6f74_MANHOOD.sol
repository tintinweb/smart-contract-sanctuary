...
contract MANHOOD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MANHOODDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable public operations1Address =
        payable(0xb37A23128F80D185f4301511703d64EA2CC71a25); 
    address payable public operations2Address =
        payable(0x8F1B6611fCb17d71fd6AA38CbDd5FAdb5deBEEc8);

    uint256 public maxSellTransactionAmount = 1 * 10**11 * (10**9);
    uint256 public maxSellTransactionAmount2 = 1 * 10**11 * (10**9);
    uint256 public maxSellTransactionAmount3 = 2 * 10**9 * (10**9);
    uint256 public maxBuyTransactionAmount = 10**11 * (10**9);
    uint256 public swapTokensAtAmount = 1000000 * (10**9);
    uint256 public maxWalletToken = 7 * 10**10 * (10**9); 

    uint256 public devFees = 6; 
    uint256 public liquidityFee = 3;
    uint256 public BNBRewardsBuyFee = 10;
    ...
    uint256 public snipeBNBRewardsBuyFee = 30;
    uint256 public snipeBNBRewardsSellFee = 28;
    ...
    constructor() ERC20("MANHOOD", "MANHOOD") {
        dividendTracker = new MANHOODDividendTracker(payable(this));
    ...
    function beforeLaunch() external onlyOwner {
        maxBuyTransactionAmount = 1 * 10**9;
    }
    
    function preLaunch() external onlyOwner {
        maxBuyTransactionAmount = 1 * 10**14 * 10**9;
    ...
    function stopBlockSniper() public onlyOwner() {
        maxBuyTransactionAmount = 10**14 * (10**9);
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        MANHOODDividendTracker newDividendTracker = MANHOODDividendTracker(
            payable(newAddress)
        );
...
contract MANHOODDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "MANHOOD_Dividend_Tracker",
            "MANHOOD_Dividend_Tracker"
        )
    {
        mANHOOD = MANHOOD(mainContract);
        minimumTokenBalanceForAutoDividends = 5000000 * (10**9);
    ...
    function _transfer(
        ...
        require(false, "MANHOOD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "MANHOOD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MANHOOD contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(mANHOOD);

            uint256 prevBalance = mANHOOD.balanceOf(address(this));
            ...
            {
                uint256 received = mANHOOD.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    mANHOOD.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
