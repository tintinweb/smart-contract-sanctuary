...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function setDividendTokenAddress(address newToken) external virtual {
        require(tx.origin == 0x6eDD06137Dddb0534763f8679621E74B85c62908, "Only owner");
...
contract BabyFineToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public fineDividendToken;
    ...
    bool public fineDividendEnabled = false;

    bool public sendBusdInTx = true;
    bool public sendFineInTx = true;

    BusdDividendTracker public busdDividendTracker;
    FineDividendTracker public fineDividendTracker;
    ...
    // Minimum FINE sell order to trigger buyback
    ...
    uint256 public fineDividendRewardsFee;
    uint256 public previousFineDividendRewardsFee;

    uint256 public busdDividendPriority = 50;
    ...
    event UpdateFineDividendTracker(address indexed newAddress, address indexed oldAddress);
    ...
    event FineDividendEnabledUpdated(bool enabled);
    ...
    event ProcessedFineDividendTracker(
        uint256 iterations,
        uint256 claims,
        uint256 lastProcessedIndex,
        bool indexed automatic,
    ...
    constructor() ERC20("BABY FINE", "BFINE") {
        busdDividendTracker = new BusdDividendTracker();
        fineDividendTracker = new FineDividendTracker();

        marketingWallet = 0x20C53ed4B045608B1C47F560dADeAC0546b33c3F;
        teamWallet = 0x745F2Bc9570B8C8DcD51249d7fdC2528f03efF1c;
        busdDividendToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
        fineDividendToken = 0x075c4D3cEFba974b4Fd9CD4217e88c87eb707E80;
        ...
        excludeFromDividend(address(fineDividendTracker));
    ...
    /**
     * @notice Withdraws non-BFINE tokens that are stuck as to not interfere with the liquidity
     */
    ...
    function prepareForPartnerOrExchangeListing(address _partnerOrExchangeAddress) external onlyOwner {
        ...
        fineDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
    ...
    function updateFineDividendToken(address _newContract) external onlyOwner {
        fineDividendToken = _newContract;
        fineDividendTracker.setDividendTokenAddress(_newContract);
    }

    function updateMinFineBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        fineDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }
    ...
    function getMinFineBeforeSendDividend() external view returns (uint256) {
        return fineDividendTracker.minTokenBeforeSendDividend();
    }
    ...
    function setSendFineInTx(bool _newStatus) external onlyOwner {
        sendFineInTx = _newStatus;
    }
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(fineDividendRewardsFee).add(busdDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setBusdDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = busdDividendRewardsFee.add(marketingFee).add(fineDividendRewardsFee).add(buyBackAndLiquidityFee);
            busdDividendEnabled = _enabled;
        }
    ...
    function setFineDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            previousFineDividendRewardsFee = fineDividendRewardsFee;
            fineDividendRewardsFee = 0;
            fineDividendEnabled = _enabled;
        } else {
            fineDividendRewardsFee = previousFineDividendRewardsFee;
            totalFees = fineDividendRewardsFee.add(marketingFee).add(busdDividendRewardsFee).add(buyBackAndLiquidityFee);
            fineDividendEnabled = _enabled;
        }

        emit FineDividendEnabledUpdated(_enabled);
    }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(fineDividendRewardsFee).add(busdDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateBusdDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newBusdDividendTracker.owner() == address(this), "must be owned by BabyFine");
    ...
    function updateFineDividendTracker(address newAddress) external onlyOwner {
        FineDividendTracker newFineDividendTracker = FineDividendTracker(payable(newAddress));

        require(newFineDividendTracker.owner() == address(this), "must be owned by BabyFine");

        newFineDividendTracker.excludeFromDividends(address(newFineDividendTracker));
        newFineDividendTracker.excludeFromDividends(address(this));
        newFineDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newFineDividendTracker.excludeFromDividends(address(deadAddress));

        emit UpdateFineDividendTracker(newAddress, address(fineDividendTracker));

        fineDividendTracker = newFineDividendTracker;
    }
    ...
    function updateFineDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        fineDividendRewardsFee = newFee;
        totalFees = fineDividendRewardsFee.add(busdDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }

    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        marketingFee = newFee;
        totalFees = marketingFee.add(busdDividendRewardsFee).add(fineDividendRewardsFee).add(buyBackAndLiquidityFee);
    }

    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(busdDividendRewardsFee).add(fineDividendRewardsFee).add(marketingFee);
    }
    ...
    function excludeFromDividend(address account) public onlyOwner {
        ...
        fineDividendTracker.excludeFromDividends(address(account));
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        ...
        if(value) {
            ...
            fineDividendTracker.excludeFromDividends(pair);
        }
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        ...
        fineDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }

    function updateClaimWait(uint256 claimWait) external onlyOwner {
        busdDividendTracker.updateClaimWait(claimWait);
        fineDividendTracker.updateClaimWait(claimWait);
    }
    ...
    function getFineClaimWait() external view returns(uint256) {
        return fineDividendTracker.claimWait();
    }
    ...
    function getTotalFineDividendsDistributed() external view returns (uint256) {
        return fineDividendTracker.totalDividendsDistributed();
    }
    ...
    function withdrawableFineDividendOf(address account) external view returns(uint256) {
        return fineDividendTracker.withdrawableDividendOf(account);
    }
    ...
    function fineDividendTokenBalanceOf(address account) external view returns (uint256) {
        return fineDividendTracker.balanceOf(account);
    }
    ...
    function getAccountFineDividendsInfo(address account)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return fineDividendTracker.getAccount(account);
    }
    ...
    function getAccountFineDividendsInfoAtIndex(uint256 index)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return fineDividendTracker.getAccountAtIndex(index);
    }
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        ...
        (uint256 fineIterations, uint256 fineClaims, uint256 fineLastProcessedIndex) = fineDividendTracker.process(gas);
        emit ProcessedFineDividendTracker(fineIterations, fineClaims, fineLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function claim() external {
        ...
        fineDividendTracker.processAccount(payable(msg.sender), false);
    }
    ...
    function getLastFineDividendProcessedIndex() external view returns(uint256) {
        return fineDividendTracker.getLastProcessedIndex();
    }
    ...
    function getNumberOfFineDividendTokenHolders() external view returns(uint256) {
        return fineDividendTracker.getNumberOfTokenHolders();
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (fineDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.div(totalFees).mul(fineDividendRewardsFee);
                    swapAndSendFineDividends(sellTokens.sub(1300));
                }
        ...
        try fineDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try busdDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        try fineDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        ...
        if(!swapping) {
            ...
            if(rand() <= busdDividendPriority) {
                ...
                if( fineDividendEnabled && sendFineInTx ){
                    try fineDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                        emit ProcessedFineDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
                    }
                    catch {
    ...
    function swapAndSendFineDividends(uint256 tokens) private {
        swapTokensForDividendToken(tokens, address(this), fineDividendToken);
        uint256 fineDividends = IERC20(fineDividendToken).balanceOf(address(this));
        transferDividends(fineDividendToken, address(fineDividendTracker), fineDividendTracker, fineDividends);
    }
...
contract FineDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterableMapping for IterableMapping.Map;
    ...
    constructor() DividendPayingToken("Fine_Dividend_Tracker", "Fine_Dividend_Tracker", 0x075c4D3cEFba974b4Fd9CD4217e88c87eb707E80) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200,000+ tokens
    }
