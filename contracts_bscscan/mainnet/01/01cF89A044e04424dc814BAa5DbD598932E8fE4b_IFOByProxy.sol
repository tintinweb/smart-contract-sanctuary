...
// max amount of raising tokens  to be raised
  uint256 public maxCap = 600000000000000000000;    // 5000 KCS
  ...
  function deposit(uint256 _amount) public {
    ...
    require (_amount >= 100000000000000000, 'need _amount > 0');
    require(userInfo[msg.sender].amount.add(_amount) <= 2000000000000000000, 'maximum contribution reached');
...
//   // to remove end
  ...
  function harvest() public nonReentrant {
    ...
    // if(totalAmount >= raisingAmount ){
        offeringToken.safeTransfer(address(msg.sender), offeringTokenAmount.mul(50000).div(1000000000));
    // }
    // else {
  ...
  function getOfferingAmount(address _user) public view returns(uint256) {
    ...
    //   uint256 allocation = getUserAllocation(_user);
      return userInfo[msg.sender].amount.mul(50000).div(1000000000);
    }
    else {
  ...
  function checkleft() public view returns (uint256){
    uint256 totalToBurn = maxCap - totalAmount; 
    return totalToBurn.mul(50000).div(1000000000);
  }
  
  function adminWithdrawTokens(uint256 _tokens) public onlyAdmin{
    require (block.number > endBlock, 'not withdrawal time');
    offeringToken.safeTransfer(msg.sender, _tokens);
