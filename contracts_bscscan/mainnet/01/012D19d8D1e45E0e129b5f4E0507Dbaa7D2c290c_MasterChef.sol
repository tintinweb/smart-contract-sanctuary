...
contract MasterChef is SafeOwnable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accBTPerShare; 
    }

    BTToken public rewardToken;
    ...
    function updateBTPerBlock(uint256 _rewardPerBlock) external onlyOwner {
    ...
    function pendingBT(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBTPerShare = pool.accBTPerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accBTPerShare = accBTPerShare.add(rewardReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accBTPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accBTPerShare = pool.accBTPerShare.add(rewardReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accBTPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeBTTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accBTPerShare).div(1e12);
