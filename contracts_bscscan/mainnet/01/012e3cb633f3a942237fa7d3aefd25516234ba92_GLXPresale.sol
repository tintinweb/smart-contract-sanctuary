...
contract GLXPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	function getGLXTokensLeft() external view returns (uint256) {
		return GLX.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some GLX tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed GLX tokens");
		require(GLX.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough GLX tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		GLX.safeTransfer(msg.sender, callerTokensUnclaimed);
	...
	function withdrawUnsoldGLX() external onlyOwner {
		uint256 amount = GLX.balanceOf(address(this)) - totalTokensSold;
		GLX.safeTransfer(msg.sender, amount);
	}

	function withdrawAllGLX() external onlyOwner {
		GLX.safeTransfer(msg.sender, GLX.balanceOf(address(this)));
	}
}