...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 depositTime;
        //
        // We do some fancy math here. Basically, any point in time, the amount of JAVA
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, uint16 _withdrawFeeBP, uint256 _lockTime, bool _withUpdate) public onlyOwner {
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, uint16 __withdrawFeeBP, uint256 _lockTime, bool _withUpdate) public onlyOwner {
    ...
    function updatePool(uint256 _pid) public {
        ...
        // rewardToken.mint(address(devaddr), javaReward.div(10));
        // rewardToken.mint(address(this), javaReward);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require(user.depositTime + pool.lockTime < block.timestamp, "time lock");
    ...
    function safeJAVATransfer(address _to, uint256 _amount) internal {
        uint256 javaBal = IERC20(rewardToken).balanceOf(address(this));
        if (_amount > javaBal) {
            IERC20(rewardToken).transfer(_to, javaBal);
        } else {
    ...
    function getRewardAmount() public view returns(uint256) {
        return IERC20(rewardToken).balanceOf(address(this)) - totalStakedAmount;
    }
}