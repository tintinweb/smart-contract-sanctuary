...
contract LianaJungle is BEP20('LianaJungle Token', 'LIANA') {
    ...
    // Safe chimpy transfer function, just in case if rounding error causes pool to not have enough CHIMPYs.
    function safeCakeTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 chimpyBal = chimpy.balanceOf(address(this));
        if (_amount > chimpyBal) {
            chimpy.transfer(_to, chimpyBal);
        } else {
            chimpy.transfer(_to, _amount);
        }
    }
