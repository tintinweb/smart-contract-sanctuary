...
contract GateManagerRewards is ReentrancyGuardUpgradeable, Ownable {
    ...
    function awardBalance() external view returns (uint256) {
        ...
        .totalSupply();
        uint256 totalYieldBal = IERC20Upgradeable(mooToken).balanceOf(prizePool.yieldSource());
        ...
        returns (uint256)
    {
        ...
        emit Withdrawn(msg.sender, sharesAmount);
    }

    /// @notice withdraw underlying from yield earning vault
    /// @param shares The amount of shares to withdraw.
    /// @return The actual amount of tokens that were withdrawn.
    function _withdrawMoonPotYield(uint256 shares) internal returns (uint256) {
        uint256 mooTokenBalanceBefore = mooToken.balanceOf(address(this));
    ...
    function inCaseTokensGetStuck(address _token) external onlyOwner {
        ...
        require(_token != address(underlying), "!reward");
        require(_token != address(prizePoolControlledToken), "!reward");
