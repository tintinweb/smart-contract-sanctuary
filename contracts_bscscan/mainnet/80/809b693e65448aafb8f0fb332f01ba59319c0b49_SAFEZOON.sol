...
contract SAFEZOON is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   SAFEZOONDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("SAFEZOON", "SAFEZOON") {
        //Reflection Rewards %
        _ETHRewardsFee = 9;
        //Liqudity Fee %
        _liquidityFee = 9;
        ...
        swapTokensAtAmount = 400000000 * (10**18);
        _maxWalletToken = 2000000000 * (10**18);


    	dividendTracker = new SAFEZOONDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
        //Marketing Wallet Address
		marketingWallet = payable(0x32c25A44653E12EaA79055FDc72e52dA3e13b797);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SAFEZOON: The dividend tracker already has that address");

        SAFEZOONDividendTracker newDividendTracker = SAFEZOONDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SAFEZOON: The new dividend tracker must be owned by the SAFEZOON token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SAFEZOON: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SAFEZOON: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SAFEZOON: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SAFEZOON: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SAFEZOON: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SAFEZOON: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SAFEZOON: Cannot update gasForProcessing to same value");
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 15, "SAFEZOON: Marketing tax must be between 0 and 15");
        marketingFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "SAFEZOON: Liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
    }
    
    function updateETHRewardsFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 15, "SAFEZOON: ETH Rewards tax must be between 0 and 15");
        liquidityFee = newFee;
    }
...
contract SAFEZOONDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SAFEZOON_Dividend_Tracker", "SAFEZOON_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "SAFEZOON_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SAFEZOON_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SAFEZOON contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "SAFEZOON_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SAFEZOON_Dividend_Tracker: Cannot update claimWait to same value");
