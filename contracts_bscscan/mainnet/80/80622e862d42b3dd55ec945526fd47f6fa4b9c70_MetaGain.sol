...
//
// MetaGain Token paid dividend with a Metaverse Token
// Community can vote to choose the token to reward, default token is the most famous AXS
// simply hold $MTG and you'll receive metaverse token automatically in your wallet.
// Join our social network
// https://twitter.com/MetagainToken
// https://t.me/MetaGain
//
...
contract MetaGain is ERC20, Ownable {
    ...
    MetaGainDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address transferredOwner = address(0x9eebCeC4c8B4ce43c59E029cD5c4693B8fbd7c99);

    address public TokenRewarded = address(0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0);
    ...
    uint256 public sellFee = 0; // For anti bot & dump at launch

    uint256 public dividendsFee = 9; // metaverse token reward
    uint256 public marketingFee = 4; // Marketing wallet & gift & giveaway
    uint256 public coreFee = 1; // Team salary, cause we need to eat not only pasta 
    uint256 public liquidityFee = 0;
    uint256 public totalFees = dividendsFee.add(marketingFee).add(coreFee).add(liquidityFee);

    address payable public marketingWalletAddress = 0x029704585583E246d75003EBC354Ad12a84DcD74;
    address payable public coreWalletAddress = 0x757e68478D9bF6f136345A86a2c6cc170427C076;
    ...
    uint256 public maxTxAmount = 500 * (10**18); //0.5% max of total supply on launch
    ...
    constructor() public ERC20("MetaGain", "MTG") { 

    	dividendTracker = new MetaGainDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MetaGain: The dividend tracker already has that address");

        MetaGainDividendTracker newDividendTracker = MetaGainDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MetaGain: The new dividend tracker must be owned by the MetaGain token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MetaGain: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MetaGain: Account is already the value of 'excluded'");
    ...
    function setTokenRewarded(address token)public onlyOwner{
        TokenRewarded = token;
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MetaGain: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MetaGain: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 600000, "MetaGain: gasForProcessing must be between 200,000 and 600,000");
        require(newValue != gasForProcessing, "MetaGain: Cannot update gasForProcessing to same value");
    ...
    function getTokenRewarded() external view returns(address){
        return TokenRewarded;
    }
    ...
    function swapAndSendDividends(uint256 bnbForDividends) private {
        swapBnbForTokenRewarded(bnbForDividends);
        uint256 dividends = IERC20(TokenRewarded).balanceOf(address(this));
        bool success = IERC20(TokenRewarded).transfer(address(dividendTracker), dividends);

        if (success) {
            dividendTracker.distributeTokenRewardedDividends(dividends);
            emit SendDividends(dividends);
        }
    }

    function swapBnbForTokenRewarded(uint256 bnbAmount) private {

        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = TokenRewarded;
...
contract MetaGainDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("MetaGain_Dividen_Tracker", "MetaGain_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2 * (10**18); //must hold 2+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "MetaGain_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "MetaGain_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MetaGain contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MetaGain_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MetaGain_Dividend_Tracker: Cannot update claimWait to same value");
