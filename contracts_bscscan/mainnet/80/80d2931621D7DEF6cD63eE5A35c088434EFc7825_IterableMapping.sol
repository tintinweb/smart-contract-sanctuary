...
// File: contracts/MetaShibaDogeDividendPayingToken.sol
...
// File: contracts/MetaSquidSanta.sol
...
contract MetaSquidSanta is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    address private _marketingWallet = address(0xF81621aF0044539E3aD271062c344Ea557F343c6);
    // address private _devWallet = address(0x426618d3Ba386Af297CE427812C01E1C8228b717);

    bool private swapping;
    bool public airdrop;

    MetaSquidSantaDividendTracker public dividendTracker;

    address public liquidityWallet;
    address public marketingWallet;
    // address public devWallet = _devWallet;

    uint256 public maxBuyTranscationAmount = 20000 * (10**18); // 100% of total supply
    uint256 public maxSellTransactionAmount = 1000000 * (10**18); // 100% of total supply
    uint256 public swapTokensAtAmount = 10000 * (10**18);
    uint256 public _maxWalletToken = 1000000 * (10**18); // 100%
    ...
    uint256 public immutable tradingEnabledTimestamp = 1637676000;
    ...
    constructor() public ERC20("MetaSquidSanta", "MSS") {
       ...
       BUSDRewardsFee = _BUSDRewardsFee;
        ...
        totalFees = _BUSDRewardsFee.add(_liquidityFee).add(_marketingFee);


    	dividendTracker = new MetaSquidSantaDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MetaSquidSanta: The dividend tracker already has that address");

        MetaSquidSantaDividendTracker newDividendTracker = MetaSquidSantaDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MetaSquidSanta: The new dividend tracker must be owned by the MetaSquidSanta token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MetaSquidSanta: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MetaSquidSanta: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MetaSquidSanta: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "MetaSquidSanta: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MetaSquidSanta: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MetaSquidSanta: Cannot update gasForProcessing to same value");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MetaSquidSanta: Account is already the value of 'excluded'");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "MetaSquidSanta: This account cannot send tokens until trading is enabled");
        }
        ...
        if(
            ...
            uint256 marketingTokens = contractTokenBalance.mul(BUSDRewardsFee).div(totalFees);
...
contract MetaSquidSantaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("MetaSquidSanta_Dividend_Tracker", "MetaSquidSanta_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "MetaSquidSanta_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "MetaSquidSanta_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MetaSquidSanta contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MetaSquidSanta_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MetaSquidSanta_Dividend_Tracker: Cannot update claimWait to same value");
