...
contract MetaClaus is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
        payable(0xd723aA965a8bF83D2DB9B4b287151aaFDBb1a9db); // Marketing Address
    address payable public miniGameAddress =
        payable(0xC05001559c64Fe855b89d204D2b61B5C609E16d2); // Mini Game Address
    ...
    string private _name = "MetaClaus";
    string private _symbol = "METACLAUS";
    uint8 private _decimals = 9;

    uint256 public _reflectionFee = 2;
    uint256 private _previousReflectionFee = _reflectionFee;

    uint256 public _marketingMiniGameFee = 10;
    uint256 private _previousMarketingMiniGameFee = _marketingMiniGameFee;

    uint256 marketingDivisor = 6;
    uint256 miniGameDivisor = 4;
    ...
    constructor() {
        ...
        _isExcludedFromFee[miniGameAddress] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[marketingAddress] = true;
        _isExcludedFromFee[deadAddress] = true;

        maxTxExempt[owner()] = true;
        maxTxExempt[miniGameAddress] = true;
        ...
        canTransferBeforeTrade[miniGameAddress] = true;
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        miniGameAddress.transfer(
            transferredBalance.div(_marketingMiniGameFee).mul(miniGameDivisor)
        );
        marketingAddress.transfer(
            transferredBalance.div(_marketingMiniGameFee).mul(marketingDivisor)
        );
    }
    ...
    function calculateLiqMarketing(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(_marketingMiniGameFee).div(10**2);
    }

    function removeAllFee() private {
        if (_reflectionFee == 0 && _marketingMiniGameFee == 0) return;

        _previousReflectionFee = _reflectionFee;
        _previousMarketingMiniGameFee = _marketingMiniGameFee;

        _reflectionFee = 0;
        _marketingMiniGameFee = 0;
    }

    function restoreAllFee() private {
        _reflectionFee = _previousReflectionFee;
        _marketingMiniGameFee = _previousMarketingMiniGameFee;
    }
    ...
    function setMarketingAndGameAddress(
        ...
        address _miniGameAddress
    ) external onlyOwner {
        marketingAddress = payable(_marketingAddress);
        miniGameAddress = payable(_miniGameAddress);
    }
    ...
    // only run when ready and everyone's claimed.
    function enabledTradingOnly() external onlyOwner {
        require(
