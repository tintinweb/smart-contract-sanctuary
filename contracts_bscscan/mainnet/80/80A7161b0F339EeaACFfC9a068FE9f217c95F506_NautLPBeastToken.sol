...
contract NautLPBeastToken is Ownable {
    using SafeMath for uint256;
    ...
    // Fees
    uint256 public stakeFee = 15;
    uint256 public unstakeFee = 25;
    address public feeTaker = 0x6233fcB52E6D9A401C1fb57A6E46805c6eBDBAd7;
    ...
    function deposit(uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            uint _fee = finalDepositAmount.mul(stakeFee).div(1e3) ;
            if(_fee > 0){
            pool.lpToken.safeTransfer(feeTaker, _fee);
            }
            finalDepositAmount = finalDepositAmount.sub(_fee);
    ...
    function withdraw(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            uint _fee = _amount.mul(unstakeFee).div(1e3) ;
    ...
    /// @param _fee value of stakeFee
    function setStakingFee(uint256 _fee)  external onlyOwner  {
       stakeFee = _fee ;
    }

    /// @param _fee value of unstakeFee
    ...
    /// @dev Obtain the stake token balance (if any) earned by reflect token
    function getStakeTokenBalance() public view returns (uint256) {
        return totalStakeTokenBalance().sub(totalStaked);
    }

      /// @dev _days Number of days to find block
    function getBlockNumber(uint256 _days) public view returns (uint256) {
        uint256 _perMinuteBlock = 20 ;
        uint256 _perDayMinutes = 24*60 ;
        uint256 _newEndBlock = block.number.add(_days.mul(_perDayMinutes).mul(_perMinuteBlock)) ;
        return _newEndBlock;
    }
    ...
    function skimStakeTokenFees() external onlyOwner {
        uint256 stakeTokenFeeBalance = getStakeTokenBalance();
    ...
    function emergencyWithdraw() external {
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[msg.sender];
        uint _fee = user.amount.mul(unstakeFee).div(1e3) ;
        if(_fee > 0){
            pool.lpToken.safeTransfer(feeTaker, _fee);
        }
   
        uint _amount = user.amount.sub(_fee);
        pool.lpToken.safeTransfer(address(msg.sender), _amount);
