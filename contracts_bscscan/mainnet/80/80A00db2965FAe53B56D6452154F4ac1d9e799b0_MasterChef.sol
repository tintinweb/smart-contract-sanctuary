...
interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to FishSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // FishSwap must mint EXACTLY the same amount of FishSwap LP tokens or
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accFishPerShare; // Accumulated FISHs per share, times 1e12. See below.
    }
    // The FISH TOKEN!
    FishToken public fish;
    // Dev address.
    address public devaddr;
    // Block number when bonus FISH period ends.
    uint256 public bonusEndBlock;
    // FISH tokens created per block.
    uint256 public fishPerBlock;
    // Bonus muliplier for early fish makers.
    ...
    returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accFishPerShare = accFishPerShare.add(
                fishReward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(accFishPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        fish.mint(devaddr, fishReward.div(10));
    ...
    // Safe fish transfer function, just in case if rounding error causes pool to not have enough FISHs.
    function safeFishTransfer(address _to, uint256 _amount) internal {
        uint256 fishBal = fish.balanceOf(address(this));
        if (_amount > fishBal) {
            fish.transfer(_to, fishBal);
        } else {
            fish.transfer(_to, _amount);
        }
    }
...
// FishToken with Governance.
contract FishToken is ERC20("FishToken", "FISH"), Ownable {
    ...
    */
    ...
    external
    {
        ...
        require(signatory != address(0), "FISH::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "FISH::delegateBySig: invalid nonce");
        require(now <= expiry, "FISH::delegateBySig: signature expired");
    ...
    returns (uint256)
    {
        require(blockNumber < block.number, "FISH::getPriorVotes: not yet determined");
    ...
    internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying FISHs (not scaled);
    ...
    internal
    {
        uint32 blockNumber = safe32(block.number, "FISH::_writeCheckpoint: block number exceeds 32 bits");
