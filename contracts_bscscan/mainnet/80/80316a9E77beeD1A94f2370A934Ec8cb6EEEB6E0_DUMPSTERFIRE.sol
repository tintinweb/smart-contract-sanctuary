...
contract DUMPSTERFIRE is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "DUMPSTERFIRE";
    string constant _symbol = "DUMPSTER";
    ...
    bool public dumpProtectionTimerEnabled = true;
    uint256 public _dumpProtectionTimer = 1 minutes;
    ...
    function setSellMultiplierEnabledSettings(bool _enabled) external onlyOwner {
        sellMultiplierEnabled = _enabled;
    }

    function setDumpProtectionTimerEnabledSettings(bool _enabled) external onlyOwner {
        dumpProtectionTimerEnabled = _enabled;
    }

    function setDumpProtectionTimer(uint256 amount) external onlyOwner() {
        _dumpProtectionTimer = amount;
    }

    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        if(inSwap){ return _basicTransfer(sender, recipient, amount); }
 
        if (recipient != address(owner()) && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver){
    ...
    function getTotalFee(bool selling, address sender) public view returns (uint256) {
        if(launchedAt + 1 >= block.number){ return feeDenominator.sub(1); }
        if(selling && dumpProtectionTimerEnabled && sellMultiplierEnabled){
    ...
    function takeFee(address sender, address reciever, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(getTotalFee(reciever == pair, sender)).div(feeDenominator);
        if (reciever == pair){ return lastSell[sender] = block.timestamp;}
    ...
    function swapBack() internal swapping {
        ...
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBUSDMarketing, gas: 30000}("");

        tmpSuccess = false;
    

        if(amountToLiquify > 0){
            router.addLiquidityETH{value: amountBUSDLiquidity}(
                address(this),
                amountToLiquify,
                0,
                0,
    ...
    function setsetAutoBuybackSettingsEnabledSettings(bool _enabled) external onlyOwner {
        autoBuybackEnabled = _enabled;
    }

    function setAutoBuybackSettings(uint256 _cap, uint256 _amount, uint256 _period) external onlyOwner {
        autoBuybackCap = _cap;
    ...
    function setIsWalletMaxExempt(address holder, bool exempt) external onlyOwner {
    ...
    //CONTRACT BROUGHT TO YOU BY @RAINING_SHITCOINS - BNB BEP20 DONATIONS FOR FURTHER WORK CAN BE SENT TO 0x3C8eEc63D0eB8EcD0451B29cEb1a715e2bda573F
}