...
contract Admin is Initializable {
    ...
    address public wjax_usd_pair; // = 0xb00eef3E8112410316A5ea2194c9DdCAAA38927d;
    ...
    function set_system_status(uint flag) public onlyAdmin {
        ...
        wjax.set_active_status(flag);

        emit Set_System_Status(flag);    
    }
    ...
    event Set_Use_Wjax_Usd_Dex_Pair(
        uint flag
    );

    function set_use_wjax_usd_dex_pair(uint flag) public onlyAdmin {
        use_wjax_usd_dex_pair = flag;
        emit Set_Use_Wjax_Usd_Dex_Pair(flag);
    }

    event Set_Wjax_Usd_Ratio(
        uint wjax_usd_ratio
    );

    function set_wjax_usd_ratio(uint rate) public onlyAdmin {
        require(rate > 0, "Rate should not be zero");
        wjax_usd_ratio = rate;
        emit Set_Wjax_Usd_Ratio(rate);
    }
    
    event Set_Use_Wjxn_Usd_Dex_Pair(
        uint flag
    );

    function set_use_wjxn_usd_dex_pair(uint flag) public onlyAdmin {
        use_wjxn_usd_dex_pair = flag;
        emit Set_Use_Wjxn_Usd_Dex_Pair(flag);
    }

    event Set_Wjxn_Usd_Ratio(
        uint wjxn_usd_ratio
    );

    function set_wjxn_usd_ratio(uint rate) public onlyAdmin {
        require(rate > 0, "Rate should not be zero");
        wjxn_usd_ratio = rate;
        emit Set_Wjxn_Usd_Ratio(rate);
    }
    ...
    function register_colony(uint128 tx_tax, string memory colony_policy_link, bytes32 colony_policy_hash, address mother_colony_public_key) public {
        ...
        wjax.registerColony(msg.sender, tx_tax, colony_policy_link, colony_policy_hash, mother_colony_public_key);
    }
    ...
    function set_colony_address(address colony_address) public {
        ...
        wjax.setColonyAddress(msg.sender, colony_address);
    }

    event Transfer_Ajax_Prime_Ownership(
        address new_ajax_prime
    );

    function transferAjaxPrimeOwnership(address _new_ajax_prime) public onlyAjaxPrime {
        ajax_prime = _new_ajax_prime;
        emit Transfer_Ajax_Prime_Ownership(_new_ajax_prime);
    }

    function set_jaxcorp_dao(address jax_corp_dao, uint128 tx_tax, string memory policy_link, bytes32 policy_hash) public onlyAjaxPrime {
        jusd.setJaxCorpDAO(jax_corp_dao, tx_tax, policy_link, policy_hash);
        jinr.setJaxCorpDAO(jax_corp_dao, tx_tax, policy_link, policy_hash);
        wjax.setJaxCorpDAO(jax_corp_dao, tx_tax, policy_link, policy_hash);
    }

    event Set_Wjax_Usd_Pair_Address(
        address pair_address
    );

    function set_wjax_usd_pair_address(address _pair) public onlyAdmin {
        wjax_usd_pair = _pair;
        emit Set_Wjax_Usd_Pair_Address(_pair);
    }

    event Set_Wjxn_Usd_Pair_Address(
        address pair_address
    );
    ...
    function setTokenAddresses(address _wjxn, address _wjax, address _vrp, address _jusd, address _jinr) public onlyAdmin {
        ...
        jusd = IXBEP20(_jusd);
        jinr = IXBEP20(_jinr);
        emit Set_Token_Addresses(_wjxn, _wjax, _vrp, _jusd, _jinr);
    }


    event Transfer_Ownership_Of_Tokens(
        address newOwner
    );
    ...
    function transferOwnershipOfTokens(address newOwner) public onlyAdmin {
        ...
        emit Transfer_Ownership_Of_Tokens(newOwner);
    }
    ...
    function transferOwnership(address _newAdmin) public onlyAdmin {
        admin = _newAdmin;
        emit Transfer_Ownership(_newAdmin);
    }
    ...
    event Set_Governor_Policy(
        string governor_policy_hash,
        string governor_policy_link
    );
    ...
    function remove_from_blacklist(address _addr) public onlyGovernor {
        ...
        wjax.remove_from_blacklist(_addr);
    }
    ...
    function set_wjax_transaction_fee(uint _tx_fee, uint _tx_fee_cap, address _wallet) public onlyGovernor {
        wjax.setTransactionFee(_tx_fee, _tx_fee_cap, _wallet);
    }

    function set_wjax_ubi_tax(uint _ubi_tax, address _wallet) public onlyAjaxPrime {
        wjax.setUbiTax(_ubi_tax, _wallet);
    }
    ...
    function set_cashback_percent(uint _cashback_percent) public onlyGovernor {
        ...
        wjax.setCashback(_cashback_percent);
    }
    ...
    function set_wjax_referral_fee(uint _referral_fee, uint _threshold) public onlyGovernor {
        wjax.setReferralFee(_referral_fee, _threshold);        
    }
    ...
    function get_wjxn_jusd_ratio() public view returns (uint, uint)
    {
        ...
        if( use_wjxn_usd_dex_pair == 0 ) {
            return (wjxn_usd_ratio * (10 ** (usd_decimal - ratio_decimal)), usd_decimal);
        }
    ...
    event Freeze_Vrp_Wjxn_Exchange(
        uint flag
    );

    function freeze_vrp_wjxn_exchange(uint flag) public onlyGovernor {
        _freeze_vrp_wjxn_exchange = flag;
        emit Freeze_Vrp_Wjxn_Exchange(flag);
    }

    event Set_Wjxn_Wjax_Collateralization_Ratio(
        uint wjxn_wjax_collateralization_ratio
    );
    ...
    function set_wjxn_wjax_collateralization_ratio(uint wjxn_wjax_collateralization_ratio) public onlyGovernor {
        ...
        emit Set_Wjxn_Wjax_Collateralization_Ratio(wjxn_wjax_collateralization_ratio);
    }
   ...
   function exchange_wjxn_vrp(uint wjxn_amount) public isActive notContract returns (uint) {
    ...
    function exchange_vrp_wjxn(uint vrp_amount) public isActive notContract returns (uint) {
    ...
    function withdraw_wjax(uint _amount) public onlyGovernor {
        ...
        emit Withdraw_Wjax(_amount);
    }

    function deposit_wjax(uint _amount) public onlyGovernor {
        wjax.transferFromByOwner(governor, address(this), _amount);
    }

    event Set_Wjax_Jusd_Markup_Fee(
        uint wjax_jusd_markup_fee, 
        address wallet
    );
    ...
    function get_wjax_jusd_ratio() public view returns (uint, uint)
    {
        ...
        if( use_wjax_usd_dex_pair == 0 ) {
            return (wjax_usd_ratio * (10 ** (usd_decimal - ratio_decimal)), usd_decimal);
        }
	...
	*/
	function exchange_wjax_jusd(uint wjax_amount) public isActive returns (uint) {
        ...
        // pay fee
        wjax.transferFromByOwner(msg.sender, wjax_jusd_markup_fee_wallet, fee_amount);
        wjax.transferFromByOwner(msg.sender, address(this), wjax_amount - fee_amount);
	...
	*/
    ...
    function set_jusd_jinr_ratio(uint _ratio) public onlyGovernor {
        ...
        emit Set_Jusd_Jinr_Ratio(_ratio);
    }

    event Set_Jusd_Jinr_Markup_Fee(
        uint markup_fee, 
        address wallet
    );
    ...
    function set_jusd_jinr_markup_fee(uint _markup_fee, address _wallet) public onlyGovernor {
        ...
        emit Set_Jusd_Jinr_Markup_Fee(_markup_fee, _wallet);
    }
    ...
    function convert_jusd_jinr(uint _amount) public isActive notContract returns (uint) {
    ...
    function convert_jinr_jusd(uint _amount) public isActive notContract returns (uint) {
    ...
    function initialize() public initializer {
        ...
        use_wjax_usd_dex_pair = 0;
        use_wjxn_usd_dex_pair = 0;
        // System state
...
interface IXBEP20 {
   ...
   /**
     ...
     */
    function transferFromByOwner(address sender, address recipient, uint256 amount) external returns (bool);
