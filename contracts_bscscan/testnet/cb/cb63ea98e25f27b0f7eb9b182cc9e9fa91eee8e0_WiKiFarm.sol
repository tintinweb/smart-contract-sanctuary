...
contract WiKiFarm is Ownable {
    ...
    function exitFarm(uint256 _farmId, uint256 _orderIndex) public returns (bool) {
        ...
        if(_farmId==1){
            ...
            // Transfer
            wnftTokenContract.safeTransfer(receiveRedeemPayWnftAddress, oneFarmRedeemPayWnftAmount);// Transfer wiki to receiveRedeemPayWnft Address
            wnftTokenContract.safeTransfer(address(msg.sender), oneFarmOrder.wnftFarmAmount.sub(oneFarmRedeemPayWnftAmount));// Transfer wiki to farm address
            ...
            // Transfer
            wnftTokenContract.safeTransfer(receiveRedeemPayWnftAddress, twoFarmRedeemPayWnftAmount);// Transfer wiki to receiveRedeemPayWnft Address
            wnftTokenContract.safeTransfer(address(msg.sender), twoFarmOrder.wnftFarmAmount.sub(twoFarmRedeemPayWnftAmount));// Transfer wiki to farm address
    ...
    function joinFarm(uint256 _farmId, address _inviterAddress) public returns (bool) {
        ...
        if(_farmId==1){
            ...
            oneFarmAccountOrders[msg.sender][oneFarmAccountOrderCount[msg.sender]] = FarmAccountOrder(oneFarmAccountOrderCount[msg.sender],address(msg.sender),true,block.timestamp,0,0,oneFarmNeedWnftAmount,oneFarmNeedBzzoneAmount,0,0);// add FarmAccountOrder
            ...
            twoFarmAccountOrders[msg.sender][twoFarmAccountOrderCount[msg.sender]] = FarmAccountOrder(twoFarmAccountOrderCount[msg.sender],msg.sender,true,block.timestamp,0,0,twoFarmNeedWnftAmount,twoFarmNeedBzzoneAmount,0,0);// add FarmAccountOrder
    ...
    function getSedimentToken1(address _erc20TokenContract, address _to, uint256 _amount) public onlyOwner returns (bool) {
