...
contract LitedexPrivatesale is TimeCrowdsale{
    ...
    constructor(address _wbnb, address _factory, address _tokenAddress, address _pairAddress) public {
    ...
    function resetCrowdsaleData() internal onlyOwner {
        stakedBNB = 0;
        stakedUSD = 0;
        token_sold = 0;
        total_received = 0;
    ...
    function purchaseInBNB() payable public onlyWhileOpen onlyWhitelist returns(bool) {
        ...
        // require(amountInBnb <= msg.value, 'calculating exceed msg.value!');
    ...
    function updateDataCrowdsale(uint256 receives, uint256 reached) private {
        ...
        crowdsaleInfo[_id].totalSold = token_sold.add(reached);
    ...
    function getBnbPrice() public view returns(uint256){
        ...
        if(Lp.token0() == address(wrappedbnb)){
            bnbPrice = ((_reserve1).div(_reserve0)).mul(1e18);
        }else{
            bnbPrice = ((_reserve0).div(_reserve1)).mul(1e18);
        }
        return bnbPrice;
    }
    ...
    function withdrawBNB() public onlyOwner{
        ...
        wrappedbnb.transfer(msg.sender, stakedBNB);
        stakedBNB = 0;
    ...
    function recoverStuckBNB() external onlyOwner {
        return safeTransferBNB(msg.sender, address(this).balance);
    }
