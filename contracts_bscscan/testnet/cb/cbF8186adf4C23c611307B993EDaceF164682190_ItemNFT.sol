...
contract ItemNFT is BaseNFT {
  ...
  function mintItem(address _owner, uint8 _gene, uint16 _class, uint _price, uint _index) onlyMintAdmin public returns (uint) {
    ...
    _mint(_owner, newId);
    Item memory item = Item(_gene, _class, block.timestamp, _price, 0, _owner);
    items[newId] = item;
    return newId;
  }
  ...
  function getItem(uint _tokenId) public view returns (uint8, uint16, uint, uint, uint, address) {
    return (
      items[_tokenId].gene,
  ...
  function addItem() external onlyMainAdmin {
    // TODO
  }
