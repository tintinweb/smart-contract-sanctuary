...
contract FeeKafraV2Proxy is Initializable, OwnableUpgradeable, IFeeKafraV2 {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    uint256 public constant override MAX_FEE = 10000;

    uint256 public constant MAX_WITHDRAW_FEE = 1000; // 10%
    uint256 public override treasuryFeeWithdraw; // % of total fee
    uint256 public override kswFeeWithdraw; // % of total fee
    uint256 public withdrawFeeSum;

    uint256 public override withdrawFee;
    uint256 public holdingKSWWithdrawFee;
    uint256 public holdingKSWGodWithdrawFee;

    uint256 public holdingKSW;
    uint256 public holdingKSWGod;
    ...
    function initialize(
        ...
        treasuryFeeWithdraw = 5000; // 50% of total fee
        kswFeeWithdraw = 5000; // 50% of total fee
        withdrawFeeSum = treasuryFeeWithdraw + kswFeeWithdraw;

        withdrawFee = 20; // 0.2 %
        holdingKSWWithdrawFee = 10; // 0.1 %
        holdingKSWGodWithdrawFee = 5; // 0.05 %

        holdingKSW = 1500 ether;
        holdingKSWGod = 10000 ether;
    }

    function userBalance(address _user) public view returns (uint256) {
        uint256 bal = IERC20Upgradeable(ksw).balanceOf(_user);
        return bal;
    }
    ...
    function distributeWithdrawFee(address _token, address) external override {
        uint256 feeAmount = IERC20Upgradeable(_token).balanceOf(address(this));

        uint256 treasuryFeeAmount = (feeAmount * treasuryFeeWithdraw) / withdrawFeeSum;
        if (treasuryFeeAmount > 0) {
            IERC20Upgradeable(_token).safeTransfer(treasuryFeeRecipient, treasuryFeeAmount);
        }

        uint256 kswFeeAmount = (feeAmount * kswFeeWithdraw) / withdrawFeeSum;
        if (kswFeeAmount > 0) {
            IERC20Upgradeable(_token).safeTransfer(kswFeeRecipient, kswFeeAmount);
        }
    }
}
...
interface IFeeKafraV2 is IFeeKafra {
    ...
    function distributeWithdrawFee(address _token, address _fromUser) external;
