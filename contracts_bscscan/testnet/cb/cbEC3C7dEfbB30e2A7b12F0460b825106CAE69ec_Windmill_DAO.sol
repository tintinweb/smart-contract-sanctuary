...
interface I_Windmill_Trade_Deployer is IAccessControlEnumerable, I_With_DAORole, I_With_BUSDToken{
	function name() external returns (string memory);
	
	function setDeployerCode(bytes memory bytecode, string memory name) external;
	
	function deployNewTrade(address owner, address manager, address fund) external returns (I_Windmill_Trade_Abstract);
}

interface I_Windmill_Trade_Manager is I_Math, IAccessControlEnumerable, I_With_DAORole, I_With_FundContract{
    struct DeployerData{
        I_Windmill_Trade_Deployer deployer;
        bool enabled;
    }
    ...
    function addTradeDeployer(I_Windmill_Trade_Deployer trade) external;
 ...
 */
interface I_Windmill_Fund is I_Math, I_Payable, IAccessControlEnumerable, I_With_DAORole, I_With_DAOContract,
                             I_With_PWRToken, I_With_BUSDToken, I_With_WBNBToken, I_With_PancakeRouter{
...
contract Windmill_DAO is Math, Adressable, Payable, Gas_Refundable, AccessControlEnumerable, With_DAORole, With_FundContract,
                         With_TradeManagerContract, With_PWRToken, With_BUSDToken, With_WBNBToken, With_PancakeRouter, I_Windmill_DAO{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    uint256 public constant nbProposals = 29;
    ...
    function init(uint256[11] calldata data,
        ...
        currentDAOCycle = 0;
        DAOCycleDurationNbBlock = data[9];
        ...
        currentRoyaltyCycle = 0;
        //royaltyCycleDurationNbBlock = _royaltyCycleDurationNbBlock;
        //currentRoyaltyCycleEndBlock = block.number + royaltyCycleDurationNbBlock;

        currentStackingCycle = 0;
        //stackingCycleDurationNbBlock = _stackingCycleDurationNbBlock;
    ...
    ////
    //////////////// Private functions ////////////////
    function updateRoyaltydAddress(I_Windmill_Royalty _royalty) internal{
        I_Windmill_Royalty oldRoyalty = royalty;
        royalty = _royalty;

        royalty.setPWRToken(PWRToken);
        (uint256 n, uint256 d) = oldRoyalty.percentPWRRoyaltyMint();
        royalty.setRoyaltyRatio(n, d);

        address oldRoyaltyAddr = address(oldRoyalty);
        address royaltyAddr = address(royalty);

        royalty.grantRole(royalty.DAO_ROLE(), oldRoyaltyAddr);
        oldRoyalty.migrateTo(royalty);
        royalty.revokeRole(royalty.DAO_ROLE(), oldRoyaltyAddr);

        PWRToken.revokeRole(PWRToken.MINTER_ROLE(), oldRoyaltyAddr);
        PWRToken.grantRole(PWRToken.MINTER_ROLE(), royaltyAddr);
     ...
     */
    function updateFundAddress(I_Windmill_Fund _fund) internal{
        //Change the storage variable
        I_Windmill_Fund oldFund = fund;
        fund = _fund;

        //Initialize the new Windmill_Fund
        fund.setBUSDToken(BUSDToken);
        fund.setPWRToken(PWRToken);
        fund.setDAO(I_Windmill_DAO(thisAddr));
    ...
    function applyProposal(Proposal storage proposal) internal{
        if (proposal.id == 0){
            ...
            refundBNBBonusRatio.numerator = proposal.paramsUint256;
            ...
            refundNbBlockDelay = proposal.paramsUint256;
        }else if (proposal.id == 21){
            royalty.setRoyaltyRatio(proposal.paramsUint256, 10000);
        }else if (proposal.id == 22){
            updateRoyaltydAddress(I_Windmill_Royalty(payable(proposal.paramsAddress)));
        }else if (proposal.id == 23){
            royaltyCycleDurationNbBlock = proposal.paramsUint256;
        }else if (proposal.id == 24){
            stackingCycleDurationNbBlock = proposal.paramsUint256;
    ...
    function checkProposalParameters(Proposal memory proposal) internal view returns (bool){
        if (proposal.id == 0){
            ...
            I_Windmill_Fund newFund = I_Windmill_Fund(payable(proposal.paramsAddress));
            ...
            I_Windmill_Royalty newRoyalty = I_Windmill_Royalty(payable(proposal.paramsAddress));
            if (   proposal.paramsUint256 != 0
                || !newRoyalty.hasRole(newRoyalty.DAO_ROLE(), thisAddr)){
                return false;
            }
        }else if (proposal.id == 23){
            if (   proposal.paramsAddress != address(0x0)
