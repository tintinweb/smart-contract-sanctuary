...
contract MonoStaking is ReentrancyGuard, DataStorage, Events, Ownable, Utils {
    ...
    function withdraw(uint256 poolId) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 currentDividends = getUserDividends(_msgSender(), poolId).add(user.rewardDebt);
            user.rewardDebt = 0;
...
contract Utils is DataStorage {
    using SafeMath for uint256;
    IUltisProxy utils = IUltisProxy(0xC79f069c81FB034C285D68db439c6D23128BbbFc);
    ...
    function getUserDividends(address userAddress, uint256 poolId)
        ...
        returns (uint256)
    {
        (uint256 checkpoint, uint256 start, uint256 amount, bool isUnStake, uint256 rewardDebt) = getUserInfo(userAddress, poolId);
        Pool memory pool = pools[poolId];

        uint256 totalAmount = 0;
        uint256 fromTime = start > checkpoint
            ? start
            : checkpoint;
        ...
        totalAmount = pool.rewardPerDay.mul(period).mul(amount).div(
            pool.totalAmount
        );
        ...
        )
    {
        ...
        (
            ...
            uint256 checkpoint,
            uint256 rewardDebt
        ) = utils.getUserInfo(currentUser, currentPoolId);
        ...
        return (checkpoint, start, amount, isUnStake, rewardDebt);
    }
...
interface IUltisProxy {
    function getUserInfo(address userAddress, uint256 poolId) external view
        returns (
            uint256 totalPayout,
            uint256 registerTime,
            uint256 lastStake,
