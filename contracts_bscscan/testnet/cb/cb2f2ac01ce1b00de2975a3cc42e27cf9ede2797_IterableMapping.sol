...
contract X is ERC20, Ownable {
    ...
    uint256 public constant MAX_Sell_FEE_IncreaseFactor = 150;
    ...
    address public marketingWallet  = address(0x725252Fd175AB01078B86e8D14bBf1E40B56D078);
    ...
    uint256 public  sellFeeIncreaseFactor = 139; 
    ...
    constructor() public ERC20("Uhhh BOOBA", "BOOBA2") {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "XMAS: The dividend tracker already has that address");

        XMASBNBDividendTracker newDividendTracker = XMASBNBDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "XMAS: The new dividend tracker must be owned by the XMAS token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "XMAS: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "XMAS: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "XMAS: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "XMAS: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "XMAS: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "XMAS: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "XMAS: Cannot update gasForProcessing to same value");
    ...
    function updateSellFeeIncreaseFactor(uint256 newIncreaseFactor) external onlyOwner {
        require(newIncreaseFactor <= MAX_Sell_FEE_IncreaseFactor, "wrong");
        sellFeeIncreaseFactor = newIncreaseFactor;
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "XMAS: This account cannot send tokens until trading is enabled");
        }
        ...
        if( 
            ...
            require(amount <= maxSellTransactionAmount, "XMAS: Sell transfer amount exceeds the maxSellTransactionAmount.");
        }
    ...
    function updatemarketingWallet(address newmarketingWallet) public  onlyOwner {
        require(newmarketingWallet != marketingWallet, "XMAS: The marketing wallet is already this address");   
...
contract XMASBNBDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("XMASBNB_Dividend_Tracker", "XMASBNB_Dividend_Tracker") {
    	claimWait = 30;
    ...
    function withdrawDividend() public override {
        require(false, "XMASBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main XMAS contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 30 && newClaimWait <= 86400, "XMASBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
