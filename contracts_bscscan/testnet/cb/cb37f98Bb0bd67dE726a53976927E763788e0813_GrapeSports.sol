...
contract GrapeSports {
    ...
    uint256 public percentOfBetForFairPlayFee;
    
    // Fixture List
    uint[] private fixturelist;

    // Fixture => position => count
    mapping(uint => mapping(uint => uint)) private positionCount;

    // Fixture => Verified 0|1
    mapping(uint => uint) private emptyFixtureVerified;
    ...
    constructor() 
    {
        ...
        percentOfBetForFairPlayFee = 1100000000000000000; // 1.1% by default
    ...
    *  *****************************/
    ...
    function setPercentOfBetForFairPlayFee(uint256 newValue) external
    {
        require(msg.sender == owner, 'FN'); //Forbidden

        percentOfBetForFairPlayFee = newValue;
    }
    ...
    *  **********************************************************/
    ...
    {
        uint256[] memory result = new uint256[](23);
        ...
        if(player != address(0))
        {
            ...
            // If has bet, get reward values
            if(fixturePlayerBetPosition[fixture][player] > 0)
            {
                uint256 winnerRewardAmount = getRewardValueWithTreasuryFee(fixture, fixturePlayerBetPosition[fixture][player], fixturePlayerBetAmount[fixture][player]);
                if(winnerRewardAmount > 0)
                {
                    uint256 winnerRewardAmountInUSD = getAmountOutMin(internalToken, usdToken, winnerRewardAmount);
                    result[20] = winnerRewardAmount; // Winner Reward in Internal Token
                    result[21] = winnerRewardAmountInUSD; // Winner Reward in USD Token
                }
            }
        }
        ...
        if(result[14] > 0)
        {
            uint256 participationAwardInUSD = getAmountOutMin(internalToken, usdToken, result[14]);
            result[22] = participationAwardInUSD; // Loser Reward in USD Token
        }
    ...
    *  **********************************************************/
    ...
    function getRewardValue(uint fixture, uint position, uint256 betAmount) public view returns (uint256)
    {
        ...
        uint256 result = SafeMath.safeMulFloat(betAmount, multiplyValue, IERC20(internalToken).decimals());
        return result;
    }
    ...
    *  **********************************************************/
    ...
    function bet(uint fixture, uint betPosition, address tokenBet, uint256 amountInWei) external
    {
        ...
        positionCount[fixture][betPosition]++;
        ...
        uint256 fairPlayAmountForEachPool;
        uint256 enteredBetAmount = betAmount;
        if(percentOfBetForFairPlayFee > 0)
        {
            uint256 fairPlayTotalAmount = SafeMath.safeDiv(SafeMath.safeMul(betAmount, percentOfBetForFairPlayFee), ONE_HUNDRED);
            fairPlayAmountForEachPool = SafeMath.safeDiv(fairPlayTotalAmount, 2);
            enteredBetAmount = SafeMath.safeSub(betAmount, fairPlayTotalAmount);
        }

        if(betPosition == 1) // HOME
        {
            fixturePoolTeamHome[fixture] = SafeMath.safeAdd(fixturePoolTeamHome[fixture], enteredBetAmount);

            // Fair play add
            fixturePoolTeamAway[fixture] = SafeMath.safeAdd(fixturePoolTeamAway[fixture], fairPlayAmountForEachPool);
            fixturePoolDraw[fixture] = SafeMath.safeAdd(fixturePoolDraw[fixture], fairPlayAmountForEachPool);
        }
        else if(betPosition == 2) // AWAY
        {
            fixturePoolTeamAway[fixture] = SafeMath.safeAdd(fixturePoolTeamAway[fixture], enteredBetAmount);

            // Fair play add
            fixturePoolTeamHome[fixture] = SafeMath.safeAdd(fixturePoolTeamHome[fixture], fairPlayAmountForEachPool);
            fixturePoolDraw[fixture] = SafeMath.safeAdd(fixturePoolDraw[fixture], fairPlayAmountForEachPool);
        }
        else if(betPosition == 3) // DRAW
        {
            fixturePoolDraw[fixture] = SafeMath.safeAdd(fixturePoolDraw[fixture], enteredBetAmount);
    ...
    *  **********************************************************/
    ...
    function winnerClaim(uint fixture, uint directInternalToken) external
    {
        ...
        // Check empty participation pools with some value to transfer to participation award treasury
        if(emptyFixtureVerified[fixture] == 0)
        {
            if(fixtureMatchResultPosition[fixture] == 1) // HOME
            {
                if(positionCount[fixture][2] == 0 && fixturePoolTeamAway[fixture] > 0)
                {
                    participationAwardAmount = SafeMath.safeAdd(participationAwardAmount, fixturePoolTeamAway[fixture]);
                }

                if(positionCount[fixture][3] == 0 && fixturePoolDraw[fixture] > 0)
                {
                    participationAwardAmount = SafeMath.safeAdd(participationAwardAmount, fixturePoolDraw[fixture]);
                }
            }
            else if(fixtureMatchResultPosition[fixture] == 2) // AWAY
            {
                if(positionCount[fixture][1] == 0 && fixturePoolTeamHome[fixture] > 0)
                {
                    participationAwardAmount = SafeMath.safeAdd(participationAwardAmount, fixturePoolTeamHome[fixture]);
                }
            ...
            else if(fixtureMatchResultPosition[fixture] == 3) // DRAW
            {
                if(positionCount[fixture][1] == 1 && fixturePoolTeamHome[fixture] > 0)
                {
                    participationAwardAmount = SafeMath.safeAdd(participationAwardAmount, fixturePoolTeamHome[fixture]);
                }
            ...
            emptyFixtureVerified[fixture] = 1;
        }
