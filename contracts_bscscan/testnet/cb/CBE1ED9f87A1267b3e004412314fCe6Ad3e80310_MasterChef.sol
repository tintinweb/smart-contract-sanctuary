...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    uint256 public rewardCycle = 1 days;
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardBlock && TotalShares != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.timestamp);
            uint256 rnoReward = multiplier.mul(TotalShares).mul(pool.allocPoint).div(rewardCycle).div(Max_Fee);
            accsugarPerShare = accsugarPerShare.add(rnoReward.mul(1e12).div(TotalShares));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (TotalShares == 0 || pool.allocPoint == 0) {
            pool.lastRewardBlock = block.timestamp;
            return;
        }
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.timestamp);
        uint256 rnoReward = multiplier.div(rewardCycle).mul(TotalShares).mul(pool.allocPoint).div(Max_Fee);
        ...
        pool.accsugarPerShare = pool.accsugarPerShare.add(rnoReward.mul(1e12).div(TotalShares));
        pool.lastRewardBlock = block.timestamp;
    }
