...
library NFTS {
    ...
    struct FactoryNFTExpanded {
        ...
        uint256 factoryDeploy;
        uint256 factoryPlayTime;
        uint256 playRewards;
        uint256 factoryGrowTime;
        bool canFactoryPlay;
        bool canFactoryGrow;
    }
    ...
    struct EmployeeNFTExpanded {
        ...
        uint256 employeePlayTime;
        bool canEmployeePlay;
    }
}

contract Factories is Context, AccessControlEnumerable, ERC721Enumerable {
    string private _baseUri = "";
    uint256 private _count = 1;

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    mapping(uint256 => NFTS.FactoryNFT) private factories;
    ...
    function getFactory(uint256 id)
        public
        view
        returns (NFTS.FactoryNFTData memory)
    {
        return NFTS.FactoryNFTData(factories[id], uris[id]);
    }
    ...
    function mint(
        ...
        factories[_count] = NFTS.FactoryNFT(multiplier, buildType, modelType);
        uris[_count] = buildTokenURI(buildType, modelType, multiplier, _count);
    ...
    function getType(uint256 id) public view virtual returns (string memory) {
        return factories[id].buildType;
    }
