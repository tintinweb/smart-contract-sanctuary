...
contract FoxCommToken is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    using Address for address;
    using Strings for uint256;

    string private _name = "Fox Community Token";

    string private _symbol = "FOX-COMM";
    ...
    function safeMint(address to, uint256 tokenId) external {
      require(msg.sender == _minter, "Must be minter");
      _safeMint(to, tokenId);
    }

    function burn(uint256 tokenId) external {
      require(msg.sender == FoxCommToken.ownerOf(tokenId));
      _burn(tokenId);
    }

    function setMinter(address minter) external onlyOwner {
      require(minter != _minter, "Is already minter");
      _minter = minter;
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = FoxCommToken.ownerOf(tokenId);
    ...
    function _transfer(
        ...
        require(FoxCommToken.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(FoxCommToken.ownerOf(tokenId), to, tokenId);
    }
...
contract FoxToken is Context, IERC20, IERC20Metadata, Ownable {
    ...
    // pancake on bsc-testnet 0x9ac64cc6e4415144c455bd8e4837fea55603e5c3
...
contract FoxPE is Ownable {
  ...
  address private _foxComm;
  address private _buyToken;

  mapping(address => address) public referrer;

  constructor(address fox_, address foxComm_, address buyToken_) {
    _fox = fox_;
    _foxComm = foxComm_;
    _buyToken = buyToken_;
  }

  function leaderBuy(address _referrer) external {
