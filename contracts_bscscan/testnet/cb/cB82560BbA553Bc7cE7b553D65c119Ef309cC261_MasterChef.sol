...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function pendingYerbamate(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accYerbamatePerShare = accYerbamatePerShare.add(yerbamateReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accYerbamatePerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accYerbamatePerShare = pool.accYerbamatePerShare.add(yerbamateReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                safeYerbamateTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function safeYerbamateTransfer(address _to, uint256 _amount) internal {
    ...
    function updateEmissionRate(uint256 _yerbamatePerBlock) public onlyOwner {
        massUpdatePools();
        yerbamatePerBlock = _yerbamatePerBlock;
        emit UpdateEmissionRate(msg.sender, _yerbamatePerBlock);
    }
