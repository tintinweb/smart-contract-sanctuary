...
contract StakingPool is Ownable, Whitelist, ReentrancyGuard {
    ...
    function innerStakeTokens(uint256 _amountToStake, bool reinvest) private {
        ...
        if (_amountToStake > 0) {
            ...
            uint256 received = stakingToken.balanceOf(address(this)) -
                balanceBefore;
    ...
    function transferPendingReward(UserInfo memory user, bool reinvest)
        private
        returns (uint256)
    {
    ...
    // TODO Make staking time and reward amount configurable by owner
    // extend duration and amount
    ...
    function extendDuration(uint256 time, uint256 _addTokenAmount)
        external
        onlyOwner
        nonReentrant
    {
        require(now < finishTime, "Pool was finished");
        require(time > finishTime,"Should be greater than existing finishTime");
        finishTime = time;
        rewardToken.safeTransferFrom(msg.sender,address(this),_addTokenAmount);
        poolTokenAmount = poolTokenAmount.add(_addTokenAmount);
        rewardPerSec = poolTokenAmount.div(finishTime.sub(now));
...
contract Whitelist is Ownable {
    ...
    modifier onlyWhitelisted() {
        if (hasWhitelisting) {
            require(isWhitelisted(msg.sender),"Account should be whitelisted");
        }
        _;
    }
