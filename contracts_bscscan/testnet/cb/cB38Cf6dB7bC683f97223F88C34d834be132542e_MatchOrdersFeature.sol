    ...
    NativeOrdersInfo
{
    ...
    function matchNativeOrders(
        LibNativeOrder.NativeOrder calldata sellOrder,
        LibNativeOrder.NativeOrder calldata buyOrder,
        LibSignature.Signature calldata sellSignature,
        LibSignature.Signature calldata buySignature,
        uint256 price,
        uint8 sellType,
    ...
    function _matchNativeOrders(
        LibNativeOrder.NativeOrder memory sellOrder,
        LibNativeOrder.NativeOrder memory buyOrder,
        LibSignature.Signature memory sellSignature,
        LibSignature.Signature memory buySignature,
        uint256 price,
        uint8 sellType,
        ...
        returns (LibNativeOrder.MatchedFillResults memory matchedFillResults)
    {
        ...
        uint128 amountSellByType = sellType == LibNativeOrder.MATCH_AMOUNT ? sellOrder.makerAmount : sellOrder.takerAmount;
        uint128 amountBuyByType = buyType == LibNativeOrder.MATCH_AMOUNT ? buyOrder.takerAmount : buyOrder.makerAmount;
        LibNativeOrder.OrderInfo memory sellOrderInfo = getNativeOrderInfo(sellOrder, amountSellByType);
        LibNativeOrder.OrderInfo memory buyOrderInfo = getNativeOrderInfo(buyOrder, amountBuyByType);
        ...
        matchInfo.buyType = buyType;
        // Compute proportional fill amounts
        matchedFillResults = calculateMatchedFillResultsNative(
            sellOrder,
            buyOrder,
            matchInfo
        );
    ...
    function getNativeOrderInfo(LibNativeOrder.NativeOrder memory order, uint128 amount)
    public
    view
    returns (LibNativeOrder.OrderInfo memory orderInfo)
    {
        // Compute the order hash and fetch the amount of takerAsset that has already been filled
        LibNativeOrder.OrderInfo memory orderInfo = getNativeOrderInfoV2(order, amount);
    ...
    function calculateMatchedFillResultsNative(
        LibNativeOrder.NativeOrder memory sellOrder,
        LibNativeOrder.NativeOrder memory buyOrder,
        LibNativeOrder.MatchOrderInfoPlus memory matchInfo
    )
    ...
    returns (LibNativeOrder.MatchedFillResults memory matchedFillResults)
    {
        ...
        if (sellAmountRemaining == buyAmountRemaining) {
            ...
            matchedFillResults.realSellFee =
                sellAmountRemaining
                .safeMul(sellOrder.makerTokenFeeAmount)
                .safeDiv(sellOrder.makerAmount);
            matchedFillResults.realBuyFee = sellAmountRemaining
                .safeMul(buyOrder.makerTokenFeeAmount)
                .safeMul(matchInfo.price)
                .safeDiv(buyOrder.makerAmount)
                .safeDiv(decimal);
        } else if (sellAmountRemaining > buyAmountRemaining) {
            ...
            matchedFillResults.takerAmountFinal = buyAmountRemaining.safeMul(matchInfo.price).safeDiv(decimal);
            // fee
            matchedFillResults.sellFeePaid = getFeeMatchNative(
                sellOrder,
                LibNativeOrder.SELL_ORDER + matchInfo.sellType,
                matchedFillResults
            );
            ...
            //fee
            matchedFillResults.sellFeePaid = LibNativeOrdersStorage.getStorage().orderHashToFeeAmountRemaining[matchInfo.sellOrderHash];
            matchedFillResults.buyFeePaid = getFeeMatchNative(
                buyOrder,
                LibNativeOrder.BUY_ORDER + matchInfo.buyType,
                matchedFillResults
            );
    ...
    function _settleMatchedNativeOrders(
        bytes32 sellOrderHash,
        bytes32 buyOrderHash,
        LibNativeOrder.NativeOrder memory sellOrder,
        LibNativeOrder.NativeOrder memory buyOrder,
    ...
    function sendBalanceWithNative(address token, address payable to, uint256 amount) public {
        if (token == LibNativeOrder.NULL_ADDRESS) {
            to.transfer(amount);
        } else {
            sendBalanceTo(
