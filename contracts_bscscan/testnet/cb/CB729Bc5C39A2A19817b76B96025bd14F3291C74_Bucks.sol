...
contract Bucks is Context, IBEP20, Ownable {
	...
	address payable public devWallet=0x1dce55E0382A075dd8ADc7A726984122145964AC;
	address payable public charitywallet=0x1dce55E0382A075dd8ADc7A726984122145964AC;
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_holderFee.add(_liquidityFee).add(_devFee).add(_charityFee).add(_rewardFee)).div(1000);
        return percentage;
    }
    ...
    function _reflectFee(uint256 tAmount) private {
        uint256 tFee = tAmount.mul(_holderFee).div(1000);
    ...
    function _takeLiquidity(uint256 tAmount) private {
        uint256 tLiquidity = tAmount.mul(_liquidityFee).div(1000);
    ...
    function _takeDevFee(uint256 tAmount) private {
        uint256 tDevFee = tAmount.mul(_devFee).div(1000);
		uint256 rDevFee = tDevFee.mul(_getRate());
		_rOwned[devWallet] = _rOwned[burnAddress].add(rDevFee);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // charitywallet.transfer(newBalance.mul(_charityFee).div(100));
