...
contract  KiLLuMiNaTi is Context, IERC20, Ownable 
{
    ...
    string private _name = "KiLLuMiNaTi";
    string private _symbol = "KiLu";
    ...
    address public marketingWallet = 0x00A267A6451d6E5E6fBBdcEb7122852B130d6EC3;
    
    address public charityWallet = 0xdb995a5e8564Af80C411374fAf951cE267558DAA;
    ...
    uint256 public _maxTxAmount = 90000000000000 * 10**18;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap 
    {
        ...
        uint256 swapableToken = contractTokenBalance.sub(half);
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(swapableToken); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 ethForLiquidity = newBalance.mul(_liquidityFee).div(totalFee.mul(2));
        addLiquidity(half, ethForLiquidity);
        emit SwapAndLiquify(half, ethForLiquidity, half);
    }
     ...
     function doManualBuyBack(uint amount) public onlyOwner
    {
        ...
        swapETHForBuyback(amount);
    }  

    function swapETHForBuyback(uint256 amount) private 
    {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
