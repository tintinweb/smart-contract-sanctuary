...
contract LockUp is  Ownable, LockInterface, Pausable{
    ...
    /* start */
    function strat(uint _startBlock) public onlyOwner whenNotPaused returns(bool success) {
    ...
    function getUserAvailable(address _userAddress) public view returns(uint256 available) {
        ...
        uint256 reward = 0;
        if(nowLockOf[_userAddress] > 0 && startBlock <= blockNumber && startBlock > 0) {
            uint last = lastBlock[_userAddress];
            if(last == 0) {
                reward +=  lockOf[_userAddress].safeMul(firstRes).safeDiv(PERCENTS_DIVIDER);
                last = startBlock;
            }
            reward += (lockOf[msg.sender].safeMul(PERCENTS_DIVIDER.safeSub(firstRes)).safeDiv(PERCENTS_DIVIDER)).safeMul(blockNumber.safeSub(last)).safeDiv(DAY_BLOCKNUMBER.safeMul(releaseDays));
        }
    ...
    function withdraw() public whenNotPaused returns(bool success) {
        uint blockNumber = block.number;
        require(startBlock > 0 && startBlock < blockNumber, 'not open !');
