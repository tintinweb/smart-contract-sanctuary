...
contract KnightDoge is ERC20, Ownable {
    using SafeMath for uint256;
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (dogeDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(dogeDividendRewardsFee).mul(dogeDividendRewardsFee);
...
contract DogeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("KnightDoge_Doge_Dividend_Tracker", "KnightDoge_Doge_Dividend_Tracker", 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "KnightDoge_Doge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "KnightDoge_Doge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main KnightDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "KnightDoge_Doge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
