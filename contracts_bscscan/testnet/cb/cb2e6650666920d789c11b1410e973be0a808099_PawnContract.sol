...
contract PawnContract is Ownable, Pausable, ReentrancyGuard {
    ...
    */
    ...
    returns (uint256 _idx)
    {
        ...
        uint256 allowed = IERC20(collateral.loanAsset).allowance(msg.sender, address(this));
        emit DebugInfo(allowed);
        require(allowed >= _loanAmount, 'offer-owner-not-yet-approve-for-withdraw');
