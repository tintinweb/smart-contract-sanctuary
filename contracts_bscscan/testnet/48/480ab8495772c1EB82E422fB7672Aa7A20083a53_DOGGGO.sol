...
contract DOGGGO is Ownable, IBEP20 {
	...
	function _transfer(
		...
		if (!excluded[sender] && !excluded[recipient] && !circuitBreaker) {
			...
			charityTax = amount.mul(CHARITY_FEE).div(10**3);
			_balances[CHARITY_WALLET] += charityTax;

			balancerTax = amount.mul(FIXED_FEE).div(10**3).add(sellTax);
	...
	function sellingTax(
		...
		uint256 unwghtCircSupply = totalSupply().sub(_balances[CHARITY_WALLET]);

		if (newCumSum > unwghtCircSupply.mul(MAX_DAILY_SELL).div(10**3) || newCumSum > poolBalance.mul(FIXED_FEE).div(10**3)) {
			revert("DOGGGO: selling amount is above max allowed");
		}

		uint256 taxAmount = newCumSum.mul(newCumSum).mul(100).ceilDiv(unwghtCircSupply);
	...
	function balancer(
		...
		sendToTreasuryAddress(half);

		//send half tokens to contract wallet
		_balances[address(this)] += half_2;

		//dynamically set the swapLimitThreshold looking at liquidity in pool
		uint256 swapLimitThreshold = tokenPoolBalance.div(bnbPoolBalance).div(10);

		//swap if limit is reached
		uint256 _liquidityPool = _balances[address(this)];
		if (_liquidityPool >= swapLimitThreshold && !_liqSwapReentrancyGuard) {
			_liqSwapReentrancyGuard = true;
			uint256 tokenOut = addLiquidity(_liquidityPool);
	...
	function addLiquidity(uint256 tokenAmount) private returns (uint256) {
		...
		try router.swapExactTokensForETHSupportingFeeOnTransferTokens(half, 0, route, address(this), block.timestamp) {
			...
			router.addLiquidityETH{ value: BNBfromSwap }(address(this), half_2, 0, 0, treasuryWallet, block.timestamp); //will not be catched
			emit AddToLiquidity("Liquidity increased");
			return tokenAmount;
		} catch {
			emit AddToLiquidity("Liquidity failed");
			return 0;
		}
	}

	function sendToTreasuryAddress(uint256 tokenAmount) private {
	...
	function endOfPenaltyPeriod() external view returns (uint256) {
		return lastTx[msg.sender].lastTimestamp + RESET_RATE;
	}
