 ...
 */
 ...
 contract MarketTradingNFT is ERC721("Market Trading NFT", "SummitNFT") {
     ...
     */
    constructor(MarketTradingAccessControls _accessControls,  address _devAddr) public {
  ...
  function createOffer(
    ...
    require(_exists(_tokenId), "SummitMarket: createOffer: TokenID does not exist");
    require(
      ownerOf(_tokenId) == _msgSender() && isApproved(_tokenId, address(this)), "SummitMarket: createOffer: Not owner or contract is not approved"
    );
    require(offers[_tokenId].salePrice == 0, "SummitMarket: createOffer: This token is already on Market");
    // create offer
    _createOffer(
      _tokenId,
      _salePrice
   ...
   */
  function confirmOffer(uint256 _index) external payable{
    // uint256 _tokenId = categories[_category].products[_index];
    uint256 _tokenId = _index;
    // require(_msgSender().isContract() == false, "SummitMarket: confirmOffer: No contracts permitted");

    Offer storage offer = offers[_tokenId];
    // Ensure this contract is still approved to move the token
    require(isApproved(_tokenId, address(this)), "SummitMarket.confirmOffer: offer not approved");
    
    // price in token
    uint256 _salePrice = offer.salePrice;
    
    require(msg.value >= _salePrice, "SummitMarket: confirmOffer: deposited ether is less than NFT price." );
    (bool success,) = payable(ownerOf(_tokenId)).call{value: _salePrice}("");
  ...
  */
  function cancelOffer(uint256 _index) external {
    ...
    Offer storage offer = offers[_tokenId];
    require(
      ownerOf(_tokenId) == _msgSender(), "SummitMarket: cancelOffer: Not owner or contract is not approved"
    );
    require(offer.salePrice != 0, "SummitMarket: cancelOffer: Offer does not exist");
