    ...
    UUPSUpgradeable
{
    ...
    function __Monsterra_init_unchained(
        string memory baseTokenURI
    ) internal initializer {
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        require(_exists(_tokenId), "Monsterra: URI query for nonexistent token");
    ...
    function mintBatch(address _to, uint256[] memory _tokenIds, string[] memory _dataArr, Type[] memory _typeArr) public virtual minterGuard {
        ...
        for (uint256 i = 0; i < _dataArr.length; i++) {
            mint(_to, _tokenIds[i], _dataArr[i], _typeArr[i]);
        }
    }

    function stake(uint256 _tokenId) public {
        require(block.timestamp >= _datas[_tokenId].mintTime + _blockingPeriod, string(abi.encodePacked("Monsterra: required ", _blockingPeriod,"s after minted time to stake again")));
        super.burn(_tokenId);
        emit Stake(_tokenId, _datas[_tokenId].nftType);
        delete _datas[_tokenId];
    }

    function stakeBatch(uint256[] memory _tokenIdArr) public virtual {
