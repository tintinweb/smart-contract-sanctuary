...
/*
☠️ $JOLLY is a deflationary, automated yield and liquidity generation charity protocol.☠️

When the Services of Good Pirates are Required

https://www.jollyroger.blue
https://t.me/jollyrogerbsc

   2% fee auto add to the liquidity pool with LPs being burnt forever
   3% fee auto distribute to all holders
   2% fee auto add to charity pool
   1% fee auto add to marketing/dev pool
   
   Dynamic Fees (Dumping Mitigation):

    Sell within 30 mins:      31% tax (8% Liquidity, 12% Donation, 8% Marketing, 3% Redistributed to Holders)

    Sell within 2 hours:      24% tax (6% Liquidity, 9% Donation, 6% Marketing, 3%  Redistributed to Holders)

    Sell within 4 hours:      17% tax (4% Liquidity, 6% Donation, 4% Marketing, 3%  Redistributed to Holders)

    Sell after 4 hours:       10% tax  (2% Liquidity, 3% Donation, 2% Marketing, 3%  Redistributed to Holders)

   Max Transaction Amount:

    1000000000 (1%) during the first day of trading
    2000000000 (2%) during the second day of trading
    3000000000 (3%) during the third day of trading
    4000000000 (4%) during the fourth day of trading
    8000000000 (8%) during the fifth day of trading
    10000000000 (10%) after the fifth day of trading

   Max Wallet Amount:

    2000000000 (1%) during the first day of trading
    4000000000 (4%) during the second day of trading
    6000000000 (6%) during the third day of trading
    8000000000 (8%) during the fourth day of trading
    16000000000 (16%) during the fifth day of trading
    20000000000 (20%) after the fifth day of trading
...
contract FOFF is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "FOFF";
  ...
  /*  function setSwapAndLiquifyEnabled(bool _swapAndLiquifyEnabled) external onlyOwner() {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pancakePair && to != address(pancakeRouter) && !_isExcludedFromFee[to]) {
            ...
            if(!inSwapAndLiquify && from != pancakePair && !_isExcludedFromFee[from]) {
                require(tradingEnabled, "Trading is not enabled");
                require(amount <= _maxTX(), "Transfer amount cannot exceeds MaxTXAmount");
                uint256 contractBalanceRecipient = balanceOf(to);
                require(contractBalanceRecipient + amount <= _maxWalletAmount(), "Exceeds maximum wallet token amount");
