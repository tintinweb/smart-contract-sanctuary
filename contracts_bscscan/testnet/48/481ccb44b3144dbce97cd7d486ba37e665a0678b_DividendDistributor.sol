...
to test:
sell tax doesnt increase during initial buys
...
contract rafflecastle is IBEP20, Auth {
    ...
    string constant _name = "bit13";
    string constant _symbol = "bit13";
...
*/
    ...
    function getTotalFee(bool selling) public view returns (uint256) { 
        ...
        if (selling && greenwallTriggeredAt.add(redwallDuration) > block.timestamp) {return getredwallsellFee();} // changed from redwallTriggeredAt
...
/**added for lottery busd
   ...
   function setMultiplierSettings(uint256 PaperHandNum, uint256 DiamondHandNum, uint256 denominator, uint256 length) external authorized{
    ...
    function launch() external authorized {
        ...
        greenwallTriggeredAt = block.number;
        redwallTriggeredAt = block.number;
