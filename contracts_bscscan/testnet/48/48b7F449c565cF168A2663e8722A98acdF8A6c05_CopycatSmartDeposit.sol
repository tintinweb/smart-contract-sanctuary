...
contract CopycatSmartDeposit {
  ...
  function withdrawToToken(ICopycatLeader leader, uint256 shareAmount, address router, uint256 tokenMin, address[] memory path) public returns (uint256 tokenOutputAmount) {
...
contract CopycatLeaderFactory is CopycatEmergencyMaster {
  ...
  uint256[5] public FEE_LIST = [100 ether, 1000 ether, 10000 ether, 1 ether, 0.0002 ether];
  ...
  ) external payable returns(address payable copycatLeaderAddress) {
    require(_level < 3, "L");
    require(msg.value == FEE_LIST[4] + 0.0001 ether, "IL");

    if (_level >= excludeFromFee[msg.sender]) {
      collectDeployFee(msg.sender, FEE_LIST[_level] / (_migrateFrom == address(0) ? 1 : 2));
    }
...
// Copycat Leader with NFT renaming support
contract CopycatLeader is ERC20('', ''), Ownable, ICopycatLeader, CopycatEmergency {
  ...
  uint256 public override depositCopycatFee = 1e18; // Base fee
  uint256 public override withdrawCopycatFee = 1e18; // Base fee
  ...
  function initialize(
    ...
    require(depositCopycatFee >= 1e18, "NF");
    require(withdrawCopycatFee >= 1e18, "NF");

    // depositFeeRate = _depositFeeRate;
    // withdrawFeeRate = _withdrawFeeRate;
    depositCopycatFee = _depositCopycatFee;
    withdrawCopycatFee = _withdrawCopycatFee;
    level = _level;
    ...
    emit Initialize(msg.sender, _leaderAddr, _depositCopycatFee, _withdrawCopycatFee, _level);
  }
  ...
  // event SetDepositFeeRate(address indexed setter, uint256 oldRate, uint256 newRate);
  // function setDepositFeeRate(uint256 _depositFeeRate) override public onlyOwner {
  //   require(_depositFeeRate <= 2000, "Too greedy");
  //   emit SetDepositFeeRate(msg.sender, depositFeeRate, _depositFeeRate);
  //   depositFeeRate = _depositFeeRate;
  // }
  ...
  function getAdaptersLength() override public view returns(uint256) {
  ...
  function addAdapter(ICopycatAdapter adapter) override public {
    require(!disabled, "D");
    require(factory.registeredAdapterFactory(msg.sender) || !initialized, "F");
  ...
  function removeAdapter(uint256 adapterId) override public onlyOwner {
    require(
  ...
  event Deposit(address indexed depositer, uint256 amount, uint256 totalShare, uint256 shareRatio);
  function depositTo(address to, uint256 shareMin) override public payable returns (uint256 totalShare) {
    require(!disabled, "D");
    ...
    uint256 shareRatio = getShareRatioSaveGas(totalBnb);
    ...
    _mint(to, totalShare);
    emit Deposit(to, value, totalShare, shareRatio);

    // Emit event to factory to make it globally trackable
    factory.emitDeposit(to, level, value);
  }

  function deposit(uint256 shareMin) override public payable returns (uint256 totalShare) {
    totalShare = depositTo(msg.sender, shareMin);
  }
  ...
  event Withdraw(address indexed withdrawer, uint256 amount, uint256 outputAmount, uint256 shareRatio);
  ...
  function withdrawTo(address to, uint256 amount, uint256 bnbMin) override public returns (uint256 outputAmount) {
    if (address(migratedTo) == address(0)) {
      ...
      // shareRatio in 1e9 unit, percentage in 1e18 unit
      ...
      for (uint256 i = 0; i < adapters.length; i++) {
        if (address(adapters[i]) != address(0)) {
          adapters[i].toLeader(percentage, 0);
        }
      }
      ...
      emit Withdraw(to, amount, outputAmount, shareRatio);
  ...
  function withdraw(uint256 amount, uint256 bnbMin) override public returns (uint256 outputAmount) {
    outputAmount = withdrawTo(msg.sender, amount, bnbMin);
  }
  ...
  function toAdapter(uint256 adapterId, uint256 amountBnb, uint256 tokenMin) public override onlyOwner returns(uint256) {
    require(!disabled, "D");
  ...
  function toLeader(uint256 adapterId, uint256 percentage, uint256 bnbMin) public override onlyOwner returns(uint256) {
    require(!disabled, "D");
    emit ToLeader(adapterId, percentage, bnbMin);
    return adapters[adapterId].toLeader(percentage, bnbMin);
  }
  ...
  function allowEmergencyCall(bytes32 txHash) public override view returns(bool) {
    return msg.sender == owner() && factory.isAllowEmergency(txHash);
  }
...
contract CopycatEmergencyMaster is Ownable, ICopycatEmergencyMaster {
  mapping(bytes32 => bool) public override isAllowEmergency;

  event AllowEmergency(bytes32 indexed txHash, address indexed target, uint value, string signature, bytes data, uint eta);
  function allowEmergency(address target, uint value, string memory signature, bytes memory data, uint eta) public override onlyOwner returns(bytes32) {
    require(eta >= block.timestamp + 1 days, "D");
    bytes32 txHash = keccak256(abi.encode(target, value, signature, data, eta));
    isAllowEmergency[txHash] = true;
    emit AllowEmergency(txHash, target, value, signature, data, eta);
    return txHash;
  }
}
...
abstract contract CopycatAdapterFactoryBase is CopycatEmergencyMaster {
...
abstract contract CopycatEmergency {
    ...
    function allowEmergencyCall(bytes32 txHash) public virtual view returns(bool);
    ...
    function executeTransaction(address target, uint value, string memory signature, bytes memory data, uint eta) public payable returns (bytes memory) {
        ...
        require(allowEmergencyCall(txHash), "A");
...
interface ICopycatEmergencyMaster {
  function isAllowEmergency(bytes32 txHash) external view returns(bool);
  function allowEmergency(address target, uint value, string memory signature, bytes memory data, uint eta) external returns(bytes32);
}