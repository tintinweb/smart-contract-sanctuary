...
contract StakeEarn is Ownable, ReentrancyGuard {
    ...
    uint256 public minStakePerUser = 5000000000000000;
    ...
    function deposit(uint256 amount, uint256 package) external nonReentrant {
		...
		require(amount >= minStakePerUser, "Minimum staking amount required");
		require(package == 1 || package == 2 || package == 3, "Incorrect package");
		...
		if(package == 1)
		{
		    endTime = block.timestamp.add(14 days);
			reward = amount.mul(25).div(100);
			withdrawn = amount.add(reward);
		}
		else if(package == 2)
		{
		    endTime = block.timestamp.add(30 days);
			reward = amount.mul(60).div(100);
			withdrawn = amount.add(reward);
		}
		else if(package == 3)
		{
		    endTime = block.timestamp.add(60 days);
			reward = amount.mul(130).div(100);
			withdrawn = amount.add(reward);
		}

        user.deposits.push(Deposit(amount, withdrawn, block.timestamp, endTime, 1, 0));
	...
	function withdraw(uint256 depositNumber) public {
	   ...
	   require(user.deposits[depositNumber].withdrawnTime >= block.timestamp);
