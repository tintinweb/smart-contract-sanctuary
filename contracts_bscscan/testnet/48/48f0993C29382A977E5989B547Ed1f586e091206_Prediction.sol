...
contract Prediction is Ownable, Pausable {
    ...
    event NewTicket(address indexed buyer, bool label, uint256 indexed pid, uint256 indexed epoch, uint256 tid, uint256 index_by_label, uint256 price);
    ...
    event UpdatedFeeRatio(uint256 indexed _old, uint256 indexed _new);
    event UpdatedLotteryRatio(uint256 indexed _old, uint256 indexed _new);
    event UpdatedImprovementDelta(uint256 indexed _old, uint256 indexed _new);
    event UpdatedBufferTimestamp(uint256 indexed _old, uint256 indexed _new);
    event UpdatedOperator(address indexed _old, address indexed _new);
    event UpdatedReferralRewardPerTicket(uint256 indexed _old, uint256 indexed _new);
     ...
     * @param _feeRatio Fee ratio (real value * 1e5)
    ...
    constructor(
        ...
        require(_lotteryRatio + _feeRatio < oneRatio, "PREDICTION: lotteryRatio + feeRatio need to be less than 100%");
     ...
     */
    function setFeeAndLotteryRatio(uint256 _feeRatio, uint256 _lotteryRatio) public onlyOwner {
        require(_lotteryRatio + _feeRatio < oneRatio, "PREDICTION: lotteryRatio + feeRatio need to be less than 100%");
        emit UpdatedFeeRatio(feeRatio, _feeRatio);
     ...
     */
    function genesisStartRound(uint256 _pid) external poolExisted(_pid) onlyOperator whenNotPaused(_pid) {
     ...
     * @param _pid id of pool
     */
    function genesisLockRound(uint256 _pid) external poolExisted(_pid) onlyOperator whenNotPaused(_pid) {
        (int price, , , ,) = priceService.getLastPrice(pools[_pid].oraclePriceCaller);
    ...
    function numbersDrawn(uint256 _pid, bytes32 _requestId, uint256 _randomNumber) external onlyRandomService() {
        ...
        if (block.timestamp > rounds[_pid][_epoch].endTime + bufferTimestamp) {
            return;
        }
        require(
    ...
    function refundable(uint256 _pid, uint256 _epoch, uint256 _tid,address _user) public view returns (bool) {
        ...
        return 
            ...
            block.timestamp > round.endTime + bufferTimestamp && 
            ticket.price != 0;
    }
    ...
    function getNextTicketPrice(uint256 _pid, bool _label) public view poolExisted(_pid) returns(uint256) {
        ...
        if (_label) {
            newBluePool = (specification.bluePool / specification.blueTicketNumber + improvementDelta) * specification.blueTicketNumber + pool.ticketPriceStart;
            price = newBluePool - specification.bluePool;
        } else {
            newRedPool = (specification.redPool / specification.redTicketNumber + improvementDelta) * specification.redTicketNumber + pool.ticketPriceStart;
    ...
    function _safeLockRound(uint256 _pid, uint256 _epoch, uint256 _price) internal {
        ...
        require(block.timestamp <= rounds[_pid][_epoch].endTime + bufferTimestamp, "PREDICTION: Can only lock round within bufferTimes");
    ...
    function _safeEndRound(uint256 _pid, uint256 _epoch) internal {
        require(block.timestamp <= rounds[_pid][_epoch].endTime + bufferTimestamp, "PREDICTION: Can only end round within bufferTimes");
    ...
    function _calculateRewards(uint256 _pid, uint256 _epoch) internal {
        ...
        if (round.closePrice > round.startPrice) {
            result = 1;
            rewardAmount = 
                (specification.bluePool + specification.redPool) *
                (oneRatio - lotteryRatio - feeRatio) / oneRatio /
                specification.blueTicketNumber;
            lotteryReward = (specification.bluePool + specification.redPool) * lotteryRatio / oneRatio;
        }
        // Red wins
        ...
        if (rewardAmount == 0) {
            ...
            Pool memory pool = pools[_pid];
            treasuryAmount = treasuryAmount + (
                rewardAmount * pool.ticketNumberInit
            ); 
        }
    ...
    function _buyTicket(uint256 _pid, bool _label, address _buyer, bool _initial, address _referral) internal returns(uint256 _price) {
        ...
        uint256 tid = tickets[msg.sender][_pid][_currentEpoch].length;
        tickets[msg.sender][_pid][_currentEpoch].push(ticket);
        emit NewTicket(_buyer, _label, _pid, _currentEpoch, tid, index, price);
        return (price);
    }
    ...
    function _split(uint256 _randomNumber, uint256 _pid, uint256 _epoch) internal view returns(uint16) {
        ...
        // Calculation range
        uint256 range = result ? specification.blueTicketNumber - pool.ticketNumberInit : specification.redTicketNumber - pool.ticketNumberInit;
        uint256 position = numberRepresentation % range;
        return uint16(position + pool.ticketNumberInit);
    }
    
    function _burnFee(uint256 _pid, uint256 _epoch) internal {
        Specification memory specification = specifications[_pid][_epoch];
        uint256 fee = (specification.bluePool + specification.redPool) * feeRatio / oneRatio;
