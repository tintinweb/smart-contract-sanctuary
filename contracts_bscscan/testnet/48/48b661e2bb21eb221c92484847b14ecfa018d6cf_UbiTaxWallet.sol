...
contract UbiTaxWallet is Initializable {
    ...
    modifier onlyAdmin() {
        require(jaxAdmin.userIsAjaxPrime(msg.sender), "Only Admin can perform this operation.");
        _;
    }
    ...
    function harvest() external {
        ...
        updateTotalShare();
        uint share = block.number - info.lastCountBlock;
        uint balance = IERC20(rewardToken).balanceOf(address(this));
        uint reward = balance * share / totalShare;
        IERC20(rewardToken).transfer(msg.sender, reward);
        totalShare -= share;
        info.lastCountBlock = block.number;
        emit Harvest(msg.sender, reward);
    }

    function updateTotalShare() internal {
        totalShare += userCount * (block.number - lastCountBlock);
        lastCountBlock = block.number;
    }

    function setUserInfo(address user, uint32 idHash, Status newStatus) external onlyAdmin {
