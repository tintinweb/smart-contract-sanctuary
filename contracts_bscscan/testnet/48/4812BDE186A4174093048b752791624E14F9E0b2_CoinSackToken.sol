...
contract CoinSackToken is IBEP20, Manageable {
    ...
    uint256 _minReserveETHForAutoReinject = 2 * 10**9;
    ...
    function _reinjectTokensFromEscrow(uint256 reinjectFactor) private {
        ...
        uint256 reserveTokensToUse = _pancakeRouter.getAmountOut(_contextAddress().balance, reserve1, reserve0).mul(reinjectFactor).div(100);

        if(reserveTokensToUse > balanceOf(_deadAddress)){
            reserveTokensToUse = balanceOf(_deadAddress);
        }

        _transfer(_deadAddress, _contextAddress(), reserveTokensToUse);
        _approve(_contextAddress(), address(_pancakeRouter), reserveTokensToUse);
        _pancakeRouter.addLiquidityETH{value: _contextAddress().balance}(_contextAddress(), reserveTokensToUse, 0, 0, _contextAddress(), _msgTimestamp());
    }
    ...
    function endTokenPresaleAndInjectInitialLiquidity(uint256 reserveFactor) public payable onlyManagement() returns (bool) {
        ...
        emit AddLiquidity(amountETHToInject, amountTokensToInject);
    ...
    /* @dev : */
    event AddLiquidity(uint256 amountETH, uint256 amountTokens);
