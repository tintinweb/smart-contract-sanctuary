...
contract SummitReferral is Ownable {
    ...
    mapping(address => bool) public firstBuy;        
    ...
    function addLeadInfluencer(address _inf) public onlyOwner {
    ...
    function removeLeadInfluencer(address _inf) public onlyOwner {
        leadInfluencers[_inf] = false;
    }

    function addSubInfluencer(address _inf, uint256 _myFee, uint256 _infFee) public {
        ...
        influencers[_inf].leadFee = _myFee;
    }

    function changeSubInfluencer(address _inf, uint _myFee, uint _infFee) public {
        require(influencers[_inf].leadAddress == msg.sender, "No permission to change this influencer");
        influencers[_inf].refFee = _infFee;
        influencers[_inf].leadFee = _myFee;
    }

    function rewardBalance(address user, address token) public view returns (uint256) {
    ...
    function swap(address user, address _tokenA, address _tokenB, uint256 _amountA, uint256 _amountB) public onlyRouter {
        ...
        if (influencers[referrer].leadAddress != address(0)) {
            uint256 amountL = amountReward.mul(influencers[referrer].leadFee).div(feeDenominator);
            ...
            amountR = amountReward.mul(influencers[referrer].refFee).div(feeDenominator);
        } else {
        ...
        if (firstBuy[user] == false) {
            _balances[user][rewardToken] = _balances[user][rewardToken] + amountReward.div(100);
            firstBuy[user] = true;
        }
    ...
    function getRewardTokens() public view returns(address[] memory result) {
        result = rewardTokens;
    }
}
