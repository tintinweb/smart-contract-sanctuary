...
contract TokenLockerCliff is ITokenLocker, Ownable, ReentrancyGuard {
    ...
    function lockOf(address _account) external view override returns (uint256) {
        uint256 tgeAmount = _locks[_account].mul(tge).div(100-tge);
    ...
    {
        uint256 tgeAmount = _locks[_account].mul(tge).div(100-tge);
        return tgeAmount.add(_released[_account]);
    }
        ...
        returns (uint256)
    {
        ...
        if (timestamp < startReleaseTimestamp + cliffDuration)
            return 0;
        ...
        returns (uint256)
    {
        ...
        if (timestamp < startReleaseTimestamp)
            return 0;
        if (timestamp < startReleaseTimestamp.add(cliffDuration))
            return _lockAmount.mul(tge).div(100);
        if (timestamp >= endReleaseTimestamp)
            return _lockAmount;
        uint256 tgeAmount = _lockAmount.mul(tge).div(100);
        uint256 _releasedTime = timestamp.sub(
            startReleaseTimestamp.add(cliffDuration)
        );
        uint256 _totalVestingTime = endReleaseTimestamp.sub(
        ...
        return tgeAmount.add(leftover.mul(_releasedTime).div(_totalVestingTime));
    }
    ...
    function firstUnlock(
        ...
        uint256 tgeAmount = _amount.mul(tge).div(100);
