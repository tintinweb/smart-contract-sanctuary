...
contract MotherBox is ERC721 {
  ...
  event BlindBoxCreated(address owner, address nft, uint256 tokenId);
  event NFTCreated(address owner, address nft);

  function setPriceNft(uint256 _price_nft) public onlyGovernance {
    price_nft = _price_nft;
  }

  function setPriceBlindBox(uint256 _price_blind_box) public onlyGovernance {
  ...
  function createNewBlindBox(
    ...
    uint256 _level
  ) public onlyGovernance nonReentrant returns (address) {
    BaseERC721BlindBox box = new BaseERC721BlindBox(name, symbol, _level);
    box.mySafeMint(msg.sender, ++id);
    box.mySetTokenURI(id, jsonUrl);
    box.setGovernance(msg.sender);
    boxes.push(address(box));
    emit BlindBoxCreated(msg.sender, address(box), _level);
    return address(box);
  }
  ...
  function createNewNFT(
    ...
    nft.mySafeMint(msg.sender, ++id);
    nft.mySetTokenURI(id, jsonUrl);
