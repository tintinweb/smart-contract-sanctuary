...
// To Do:
// set approve parameter as from pantherswap tokens
...
contract TokenBuyback is BEP20 {
    ...
    IWETH private constant WBNBContract =
    ...
    ///in emergency case of stuck balance or incorrect funds
    function fixStuckBalance(address[] calldata tokens) external onlyOwner {
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (tokens[i] == ETHAddress) {
                ...
                qty =
                    IBEP20(tokens[i]).balanceOf(address(this));
                IBEP20(tokens[i]).safeTransfer(owner(), qty);
            }
        }
    }
    ...
    function TriggerLiquidityUnwrapEth(address LPTokenAddress, address tokenPairedWithEth, uint256 amountLPTokensETH) external onlyOwner {
      unwrapLiquidityTokensEth(LPTokenAddress, tokenPairedWithEth, amountLPTokensETH, address(this));
    }

    function unwrapLiquidityTokensEth(address LPTokenAddress, address tokenPairedWithEth, uint256 amountLPTokensETH, address to) internal {
      _approveToken(address(LPTokenAddress), address(pancakeswapRouter), amountLPTokensETH);
    ...
    function TriggerLiquidityUnwrapToken(address LPTokenAddress, address tokenA, address tokenB, uint256 amountLPTokens) external onlyOwner {
      unwrapLiquidityTokensToken(LPTokenAddress, tokenA, tokenB, amountLPTokens, address(this));
    }

    function unwrapLiquidityTokensToken(address LPTokenAddress, address tokenA, address tokenB, uint256 amountLPTokens, address to) internal {
      _approveToken(address(LPTokenAddress), address(pancakeswapRouter), amountLPTokens);
    ...
    function TriggerTokenTradeForWBNB(address tokenAddressToSwap, uint256 amountTokenToSwap) external onlyOwner {
    ...
    function swapTokensForWBNB(address tokenAddressToSwap, uint256 amountTokenToSwap, address to) internal {
      ...
      _approveToken(address(tokenAddressToSwap), address(pancakeswapRouter), amountTokenToSwap);
    ...
    // function wrap BNB to WBNB
    function wrapWBNB() external payable onlyOwner {
      WBNBContract.deposit();
    }

    // function to unwrap WBNB to BNB
    function unwrapWBNB(uint256 amount) external onlyOwner {
      WBNBContract.withdraw(amount);
    }
