...
import "../token/interface/ISHIP.sol";

/**
 * @title Ship Box Contract
 * @author ETHST-TEAM
 * @notice This Contract Draw Ship
 */
contract ShipBox is AccessControlEnumerable, VRFConsumerBase {
    ISHIP public shipNFT;

    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");

    address public receivingAddress;
    uint256 public boxBNBPrice = 2e17;
    uint256[] public boxTokenPrices = [10000e18];
    address[] public tokenAddrs = [0x18b2978D7E4EAc4A00c66Ab00d68bA7E77141A79];
    ...
    mapping(bytes32 => uint256[]) public requestIdToShipIds;

    bytes32 public keyHash;
    uint256 public fee;

    event BuyBoxes(address indexed user, uint256 boxesLength);
    event ProduceShips(address indexed user, uint256[] shipIds);
     ...
     * @param shipAddr Initialize SHIP NFT Address
    ...
    function setTokensInfo(
        ...
        require(
            ...
            "Tokens Info Length does Not Match"
        );
    ...
    function buyBoxes(uint256 boxesLength, uint256 tokenId) external {
        ...
        emit BuyBoxes(msg.sender, boxesLength);
    }
    ...
    /**
     * @dev Get Total Token Buy Amount
     */
    function getTotalTokenBuyAmount(uint256 tokenId)
        external
        view
        returns (uint256)
    {
        return totalTokenBuyAmount[tokenId];
    }

    /**
     * @dev Get User Token Buy Amount
     */
    function getUserTokenBuyAmount(address user, uint256 tokenId)
        external
        view
        returns (uint256)
    {
        return userTokenBuyAmount[user][tokenId];
    }
    ...
    /**
     * @dev Produce Ships to User when get Randomness Response
     */
        ...
        override
    {
        for (uint256 i = 0; i < requestIdToBoxesLength[requestId]; i++) {
            uint256 level = (randomness % 100) + 1;

            uint256 shipId = shipNFT.produceShip(
            ...
            requestIdToShipIds[requestId].push(shipId);
            randomness /= 100;
        }

        emit ProduceShips(
            requestIdToUser[requestId],
            requestIdToShipIds[requestId]
        );
    }
}
 ...
 * @author ETHST-TEAM
 * @notice Interface of the SHIP
 */
interface ISHIP is IERC721Enumerable {
    function produceShip(
        address to,
    ...
    function upgrade(uint256 shipId, uint256 upgradeLevels) external;

    function getShipLevel(uint256 shipId) external view returns (uint256);

    function getShipRemainLevels(uint256 shipId)
        external
        view
        returns (uint256);
}
