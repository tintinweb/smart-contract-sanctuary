...
contract StakingContract{
	...
	if (user.deposits.length == 0) {
      ...
      function winthdrawBullBnbTokens(uint256 i)public returns(bool){
        ...
        uint256 time= getUserTimeAfterDeposit(userAddress,i).div(1 minutes);
		...
		uint256 dividends;
				...
				if (user.deposits[i].withdrawn.add(dividends) > getMaxPercentage(user.deposits[i].amount) ){
					dividends = getMaxPercentage(user.deposits[i].amount).sub(user.deposits[i].withdrawn);
				}
