...
contract StarInfinity  is Initializable {
    ...
    function BuyClub() external {
        ...
        require(users[msg.sender].history.buy_club<block.timestamp,"already Buy Club");
    ...
    function UserWithdraw(address recipeint, uint256 amount) external onlyOwner {
        ...
        if(users[recipeint].croppingWithdraw<=package[users[recipeint].currentPackage].mul(10)){
            users[recipeint].croppingWithdraw=users[recipeint].croppingWithdraw.add(amount);
        }
        // amount=amount.add(users[recipeint].holdAmount);
        // users[recipeint].holdAmount=0;
        reinvest(recipeint,amount.mul(50).div(100));
        users[recipeint].totalWithdraw=users[recipeint].totalWithdraw.add(amount);
        users[recipeint].history.last_withdrawal=block.timestamp;
