...
interface I_Windmill_Fund is I_Math, I_Payable, IAccessControlEnumerable, I_With_DAORole, I_With_UpdaterRole, I_With_TradeManagerRole,
    ...
    function getWithdrawFees() external view returns(Fraction memory);
...
contract Windmill_Fund is Adressable, Math, Payable, AccessControlEnumerable, With_DAORole, With_UpdaterRole, With_TradeManagerRole,
    ...
    function buyPWR(uint256 amountBUSD) external{
        ...
        (uint256 nbFreePwr, uint256 nbLockedPwr, uint256 flatLockPercentNumerator, uint256 lockPercentNumerator, uint256[2][] memory lockDatas) = getDataPWRLock(msg.sender);
    ...
    function beforePWRTransfer(address from, address to, uint256 amount) external onlyRole(PWR_TOKEN_ROLE){
        (uint256 nbFreePwrFrom, uint256 nbLockedPwrFrom, uint256 flatLockPercentNumeratorFrom, uint256 lockPercentNumeratorFrom, uint256[2][] memory lockDatasFrom) = getDataPWRLock(from);
        
        require(nbFreePwrFrom >= amount, "Windmill_Fund: Cannot transfer locked PWR tokens.");
        
        (uint256 nbFreePwrTo, uint256 nbLockedPwrTo, uint256 flatLockPercentNumeratorTo, uint256 lockPercentNumeratorTo, uint256[2][] memory lockDatasTo) = getDataPWRLock(to);
    ...
    function getFreePWR(address addr) external view returns(uint256){
        (uint256 nbFreePWR, uint256 nbLockedPWR, uint256 flatLockPercentNumerator, uint256 lockPercentNumerator, uint256[2][] memory lockDatas) = getDataPWRLock(addr);
    ...
    function sellPWR(uint256 amountPWR) external{
        ...
        require(nbBUSDTowithdraw >= BUSDToken.balanceOf(thisAddr), "Windmill_Fund: All BUSD are locked in trade.");
