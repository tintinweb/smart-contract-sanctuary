...
contract BoobaToken is ERC20("BoobaToken", "BOOBA") {
...
contract BoobaStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accBoobaPerShare;   // Accumulated Yieldes per share, times 1e18. See below.
        uint16 withdrawFeeBP;      // withdraw fee in basis points
        uint256 lpSupply;
        
    }

    BoobaToken public immutable Booba;
    address public devAddress;
    address public feeAddress;
    

    uint256 public BoobaPerBlock = 8 ether;
    
    uint256 constant MAX_BOOBA_PERBLOCK= 30 ether;

    // Info of each pool.
    PoolInfo[] public poolInfo; // contract.poolInfo(i)
    ...
    event UpdateEmissionRate(address indexed user, uint256 BoobaPerBlock);
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
		if (Booba.totalSupply() >= 10_000_000 ether) {
			return 0;
		}
    ...
    function pendingBooba(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBoobaPerShare = pool.accBoobaPerShare;
        uint256 lpSupply = pool.lpSupply;
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && totalAllocPoint>0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 BoobaReward = multiplier.mul(BoobaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accBoobaPerShare = accBoobaPerShare.add(BoobaReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accBoobaPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 BoobaReward = multiplier.mul(BoobaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        Booba.mint(devAddress, BoobaReward.div(10));
        Booba.mint(address(this), BoobaReward.mul(88).div(100));

        pool.accBoobaPerShare = pool.accBoobaPerShare.add(BoobaReward.mul(88).div(100).mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accBoobaPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeBoobaTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accBoobaPerShare).div(1e18);
    ...
    function safeBoobaTransfer(address _to, uint256 _amount) internal {
        uint256 BoobaBal = Booba.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > BoobaBal) {
            transferSuccess = Booba.transfer(_to, BoobaBal);
        } else {
            transferSuccess = Booba.transfer(_to, _amount);
        }
        require(transferSuccess, "safeBoobaTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _BoobaPerBlock) external onlyOwner {
        require(_BoobaPerBlock<=MAX_BOOBA_PERBLOCK,"Too high");
        massUpdatePools();
        BoobaPerBlock = _BoobaPerBlock;
        emit UpdateEmissionRate(msg.sender, _BoobaPerBlock);
    }
