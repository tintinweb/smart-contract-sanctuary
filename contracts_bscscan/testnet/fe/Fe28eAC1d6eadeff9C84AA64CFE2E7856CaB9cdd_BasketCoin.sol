...
contract BasketCoin is BEP20, Pausable, Ownable {
    
    address public devAddress;
    address public minterAddress;
    
    constructor(address _devAddress,address _taxAddress) BEP20("BasketCoin", "BSKT",_taxAddress) {
        devAddress =_devAddress;
    }
    
    modifier onlyAuth {
        address user = _msgSender();
        require(user == devAddress || user == owner() || user == minterAddress, "Auth people only accessible");
        _;
    }
...
contract BEP20 is Context, IBEP20, IBEP20Metadata {
    ...
    function _transfer(
        ...
        if(!excludeFee[_msgSender()] && feeDeductStatus){
            uint256 TFee;
            uint256 bFee;
