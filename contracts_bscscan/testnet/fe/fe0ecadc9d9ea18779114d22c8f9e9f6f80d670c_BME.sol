...
contract BME is ERC20, Ownable {
    ...
    function _transfer(
        ...
        if(takeFee) {
          ...
          if(automatedMarketMakerPairs[to] && from != liquidityWallet && to != liquidityWallet ) {
        ...
        uint256 balanceOfFrom =  balanceOf(from);
          ...
          if( !automatedMarketMakerPairs[from] || !_isExcludedFromFees[from]  ||  from != liquidityWallet ){
    
          tokenHoldersMap.set(from, balanceOfFrom);
    
          }
    
          if( !automatedMarketMakerPairs[to] || !_isExcludedFromFees[to] || to != liquidityWallet ){
    
          tokenHoldersMap.set(to, balanceOfTo);
    
          }
    
          if( balanceOfFrom <= 10 * (10**18) ||  from != liquidityWallet ){
          HoldingDays[from] = 0;
