...
// File: contracts/helpers/AccessControl.sol
...
// File: contracts/CrabFarmTimelock.sol
...
/**
 * @dev CrabFarm functions that do not require less than the min timelock
 */
interface ICrabFarm {
    function add(
        uint256 _allocPoint,
...
interface ICrabToken {
     function swapTokensForEth(address tokenAddress) external;

    function switchUniRouter(bool isPancakeSwap) external;
    ...
    function setRouterAddress(address _pcsRouterAddress, address _otherRouterAddress) external;
    
    function setWBNBAddress(address _wbnbAddress) external;
}
...
contract TimelockController is AccessControl {
    ...
    address public crabTokenAddress;
     ...
     */
    constructor(address payable _devWalletAddress, address _wbnbAddress)
        public
    ...
    function executeSet(
        ...
        _beforeCall(predecessor);
        ICrabFarm(_crabfarmAddress).set(_pid, _allocPoint, _withUpdate);
        _afterCall(id);
    }
    ...
    function add(
        ...
        ICrabFarm(_crabfarmAddress).add(0, _want, _withUpdate, _strat); // allocPoint = 0. Schedule set (timelocked) to increase allocPoint.
    }
    ...
    function switchUniRouter(bool isPancakeSwap) public onlyRole(EXECUTOR_ROLE) {
        ICrabToken(crabTokenAddress).switchUniRouter(isPancakeSwap);
    }

    function setSwapAndLiquifyEnabled(bool _enabled) public onlyRole(EXECUTOR_ROLE) {
        ICrabToken(crabTokenAddress).setSwapAndLiquifyEnabled(_enabled);
    }
    
    function setBuyBackEnabled(bool _enabled) public onlyRole(EXECUTOR_ROLE) {
        ICrabToken(crabTokenAddress).setBuyBackEnabled(_enabled);
    }
    
    function setBuybackUpperLimit(uint256 buyBackLimit) public onlyRole(EXECUTOR_ROLE) {
        ICrabToken(crabTokenAddress).setBuybackUpperLimit(buyBackLimit);
    }

    function setRouterAddress(address _pcsRouterAddress, address _otherRouterAddress) public onlyRole(EXECUTOR_ROLE) {
        ICrabToken(crabTokenAddress).setRouterAddress(_pcsRouterAddress, _otherRouterAddress);
    }
    
    function setWBNBAddress(address _wbnbAddress) public onlyRole(EXECUTOR_ROLE) {
        ICrabToken(crabTokenAddress).setWBNBAddress(_wbnbAddress);
    }
    
    function setCrabTokenAddress(address _crabTokenAddress) public onlyRole(EXECUTOR_ROLE) {
        crabTokenAddress = _crabTokenAddress;
    }
    ...
    function swapTokensForEth(
        ...
        IUniswapV2Router02 uniswapV2Router = IUniswapV2Router02(uniRouterAddress);
        ...
        if (tokenBalance > 0) {
            IERC20(tokenAddress).approve(uniRouterAddress, tokenBalance);
            ...
            emit SwapTokensForETH(tokenBalance, path);
        }
    }
    ...
    function distributeRewards()
        ...
        returns (uint256)
    {
        require(crabTokenAddress != address(0), "crabTokenAddress Invalid!");
        
        _wrapToWBNB();

        uint256 rewardsAmt = IERC20(wbnbAddress).balanceOf(address(this));
        uint256 devFee = rewardsAmt.mul(200).div(10000);
        uint256 buybackFee = rewardsAmt.mul(800).div(10000);
        IERC20(wbnbAddress).safeTransfer(devWalletAddress, devFee);
        IERC20(wbnbAddress).safeTransfer(crabTokenAddress, buybackFee);
    }

    function _wrapToWBNB() private {
        // BNB -> WBNB
