...
contract BullBNB is Context, IBEP20, Ownable {
  ...
  constructor() {
    ...
    _symbol = "BullBNB";
    _decimals = 12;
    _totalSupply = 1000000000 * (10 ** 12);  // total Supply: 1B

    _liquidityFee = 700;

    _minTokensToAddToLiquidity = 10000 * (10 ** 12);    // 0.1M
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if (!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient] && sender == uniswapV2Pair) {
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
    ...
    payable(providerAddress).transfer(address(this).balance);
