...
contract ChainLinkWrapper is IPriceFeedsExt, Ownable {
    ...
    constructor(
        ...
        require(
            ...
            "Only support decimals 8 and 18"
        );
    ...
    function setPairName(string memory _pairName) public onlyOwner {
        pairName = _pairName;
    }
        ...
        returns (uint256 ethAmount)
    {
        if (tokenAddress == address(wbaseToken)) {
            ...
            (uint256 toEthRate, uint256 toEthPrecision) =
                queryRate(tokenAddress, address(wbaseToken));
...
contract Constants {
    ...
    function _setWbaseToken(address _wbaseTokenAddress) internal {
        require(
            Address.isContract(_wbaseTokenAddress),
            "_wbaseTokenAddress not a contract"
        );
