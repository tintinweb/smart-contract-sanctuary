    ...
    OwnableUpgradeable
{
        ...
        returns (bool)
    {
        ...
        uint256 commissionInDollars = _getCommissionForAttack(_positionToAttack);
        uint256 commissionInBNB = Variables(VariablesContractAddress)
            .getDollarsInBNB(commissionInDollars);
        ...
        payable(minterWalletAddress).transfer(gasToMinter);
        payable(commissionWalletAddress).transfer(commissionInBNB);
        payable(msg.sender).transfer(msg.value - (gasToMinter + commissionInBNB));
    ...
    function _getCommissionForAttack(uint256 _positionToAttack) internal pure returns (uint256 commission) {
