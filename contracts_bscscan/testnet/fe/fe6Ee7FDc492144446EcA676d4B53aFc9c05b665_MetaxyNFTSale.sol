...
contract MetaxyNFTSale is IChest, Ownable, VRFConsumerBase {
    ...
    mapping(address => mapping(bytes32 => uint256)) private s_results;
    ...
    uint256 public constant TOTAL_SUPPLY_COMMON_CHEST = 1359;
    uint256 public constant TOTAL_SUPPLY_DELUXE_CHEST = 876;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        ...
        override
    {
        s_results[s_rollers[requestId]][requestId] = randomness;
        ...
        returns (bytes32 requestId)
    {
        ...
        s_results[msg.sender][requestId] = ROLL_IN_PROGRESS;
    ...
    function mintAirdrop(address to, uint256 amount, uint16 rankId) external onlyOwner {
        require(rankLengh[rankId] >= amount, "MetaxyNFTSale: Run out of id");
        IMetaxyNFT(mxy721).mintGenesis(amount, to, 0);
        uint256 startId = currentId;
        currentId += amount;
        for (uint256 i = 1; i <= amount; i++) {
            rankOf[startId + i] = rankId;
        }
        rankLengh[rankId] = uint16(rankLengh[rankId] - amount);
    }

    function isReadyClaimChest(address user, bytes32 requestId) external view returns(bool) {
        if (s_rollers[requestId] == user && s_results[user][requestId] != ROLL_IN_PROGRESS) {
            return true;
        }
        return false;
    }

    function claimChest(uint16 chestType, bytes32 requestId) external override {
        ...
        uint256[] memory rands = expand(s_results[msg.sender][requestId], AMOUNT_NFT_PER_CHEST);
...
interface IChest {
    ...
    function claimChest(uint16 chestType, bytes32 requestId) external;
...
interface IMetaxyNFT is IERC721EnumerableUpgradeable {
