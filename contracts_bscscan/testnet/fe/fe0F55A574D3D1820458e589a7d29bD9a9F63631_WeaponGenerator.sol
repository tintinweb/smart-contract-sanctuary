...
contract WeaponGenerator  {
	...
	function fakeMint(address safeAddress) public view returns(uint256 [] memory) {

		uint256 sum = 0;
		uint256[] memory result;
		
		 for (uint i = 0; i < 4; i++) {
				uint256 seed =  uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), safeAddress, i)));
