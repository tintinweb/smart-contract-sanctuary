...
contract EmpowerCalcFormula is Ownable, ICalcFormula {
    using SafeMath for uint256;
    FormulaParams private params;
    
    constructor(uint256 _constantValue, uint256 _linearCoefficient, uint256 _quadraticCoefficient, uint256 _percentage){
        params.constantValue = _constantValue;
        params.linearCoefficient = _linearCoefficient;
        params.quadraticCoefficient = _quadraticCoefficient;
        params.percentage = _percentage;
    }
    
    function getFormulaParams() public view returns (FormulaParams memory) {
        return params;
    }
    
    function calcCapacity(uint256 tokenMineralLevel, bool isMatchPlace) public virtual override view returns (uint256) {
        uint256 capacity = 0;
        if(tokenMineralLevel == 1){
            capacity = 1e18;
        } else {
            
            capacity = params.quadraticCoefficient.mul(tokenMineralLevel**2)
                .add(params.linearCoefficient.mul(tokenMineralLevel))
                .add(params.constantValue);
        }

        if(!isMatchPlace){
            capacity = capacity.mul(params.percentage).div(1e18);
        }

        return capacity;
    }
    
    function setFormulaParams(uint256 _constantValue, uint256 _linearCoefficient, uint256 _quadraticCoefficient, uint256 _percentage) public onlyController{
        params.constantValue = _constantValue;
        params.linearCoefficient = _linearCoefficient;
        params.quadraticCoefficient = _quadraticCoefficient;
        params.percentage = _percentage;
