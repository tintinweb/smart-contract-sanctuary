...
// File: src/interfaces/ITreasurer.sol
...
contract Capital is ICapital, Operator, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    modifier checkCondition {
        require(!migrated, "Capital: migrated");
        require(block.timestamp >= startTime, "Capital: not started yet");
        _;
    }
    ...
    modifier checkEpoch {
        ...
        require(block.timestamp >= _nextEpochPoint, "Capital: not opened yet");
        _;
    ...
    // With Capital v1, we will only utilize collateral from a single pool to do rebalancing
    function buyback(uint256 _collateral_value, uint256 _min_share_amount) external onlyStrategist notMigrated hasRebalancePool checkRebalanceCooldown {
        ...
        IPool(rebalancing_pool).transferCollateralToCapital(_collateral_amount_sell); // Transfer collateral from pool to Capital
    ...
    function recollateralize(uint256 _share_amount, uint256 _min_collateral_amount) external onlyStrategist notMigrated hasRebalancePool checkRebalanceCooldown {
        ...
        if (_collateral_balance > 0) {
            IERC20(rebalancing_pool_collateral).safeTransfer(rebalancing_pool, _collateral_balance); // Transfer collateral from Capital to Pool
        }
    ...
    function allocateSeigniorage() external nonReentrant checkCondition checkEpoch {
        ...
        if (_exceeded) {
            ...
            IPool(rebalancing_pool).transferCollateralToCapital(_allocation_amount); // Transfer collateral from pool to capital
            IERC20(rebalancing_pool_collateral).safeApprove(treasurer, 0);
            IERC20(rebalancing_pool_collateral).safeApprove(treasurer, _allocation_amount);
            ITreasurer(treasurer).allocateSeigniorage(_allocation_amount);
        }
    }

    function migrate(address _new_capital) external onlyOperator notMigrated {
        migrated = true;
        uint256 _share_balance = IERC20(share).balanceOf(address(this));
        if (_share_balance > 0) {
            IERC20(share).safeTransfer(_new_capital, _share_balance);
        }
        if (rebalancing_pool_collateral != address(0)) {
            uint256 _collateral_balance = IERC20(rebalancing_pool_collateral).balanceOf(address(this));
            if (_collateral_balance > 0) {
                IERC20(rebalancing_pool_collateral).safeTransfer(_new_capital, _collateral_balance);
            }
        }
    }
    ...
    function setTreasurer(address _treasurer) public onlyOperator {
    ...
    function executeTransaction(
        ...
        require(success, string("Capital::executeTransaction: Transaction execution reverted."));
