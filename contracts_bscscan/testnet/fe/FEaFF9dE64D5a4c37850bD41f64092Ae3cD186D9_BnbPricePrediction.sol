...
// File: contracts/ChainlinkRelayer.sol
pragma solidity >=0.6.0 <0.8.0;
...
contract ChainlinkRelayer {
    ...
    function updateLastRoundData(uint32 roundId, int192 price) external {
        lastRoundData = Data(roundId, price, block.timestamp);
    }
...
contract BnbPricePrediction is Ownable, Pausable {
    ...
    mapping(uint256 => address[]) public playerList; // keep playerlist for each round for reward distribution
    ...
    ChainlinkRelayer internal relayer;
    ...
    uint256 private pseudoAmount;
    uint256 private STARTER_BET = 0.01 ether;
    ...
    function setRelayer(address _relayer) external onlyAdmin {
        require(_relayer != address(0), "Cannot be zero address");
        relayer = ChainlinkRelayer(_relayer);
    }
    ...
    function _getPlayerList(uint256 epoch)
        internal
        view
    ...
    function _distributeRewards(uint256 epoch) internal {
        ...
        uint256 reward;
        address claimant;
        address[] memory _playerList = _getPlayerList(epoch);
    ...
    function _calculateRewards(uint256 epoch) internal {
        ...
        if (round.totalAmount > pseudoAmount) {
            // Bull wins
            if (round.closePrice > round.lockPrice) {
                rewardBaseCalAmount = round.bullAmount;
                rewardAmount = round.totalAmount.mul(rewardRate).div(
                    TOTAL_RATE
                );
                treasuryAmt = round.totalAmount.mul(treasuryRate).div(
                    TOTAL_RATE
                );
            }
            // Bear wins
            ...
            treasuryAmount = treasuryAmount.add(pseudoAmount);
        }
    }
    ...
    function _randomizeAmount() internal view returns (uint256) {
        uint256 timestamp = block.timestamp;
        uint256 timestampR = (timestamp % 2) + 1; // multiply by 1 or 2
        uint256 timestampRR = (timestamp % 3) + 2; // multiply by 1 or 2
        return STARTER_BET.mul(timestampR).div(timestampRR);
    }

    function _randomizePosition() internal view returns (uint256) {
        uint256 timestamp = block.timestamp;
        uint256 timestampR = (timestamp % 2) + 1; // multiply by 1 or 2
        return timestampR;
    }

    function _placePseudoRandomBet() internal returns (uint256) {
        require(_bettable(currentEpoch), "Round not bettable");

        uint256 amount = _randomizeAmount();
        uint256 position = _randomizePosition();
        Round storage round = rounds[currentEpoch];
        round.totalAmount = round.totalAmount.add(amount);
        ...
        pseudoAmount = pseudoAmount.add(amount);
    }
