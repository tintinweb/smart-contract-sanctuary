...
contract TheHarderTheyFall is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingWalletAddress = payable(0xDe7796cc7F65F98a053Dba0215a75a7449b34e08); // Marketing Address
    address payable public buybackWalletAddress = payable(0xd9Cb687EccC712A045eE3Af1542A4b057d8611C1); // BuyBack Address
    ...
    string private _name = "TheHarderTheyFall";
    string private _symbol = "THTF";
    ...
    uint256 public _numTokensSellToAddToLiquidity = 25 * 10**3 * 10**9;
    ...
    constructor () {
        ...
        _totalLiqFee = _liquidityFee.add(_marketingFee).add(_buyBackFee);
    ...
    function setTaxes(uint256 newRewardFee, uint256 newLiquidityFee, uint256 newMarketingfee, uint256 newBuybackFee) external onlyOwner() {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (overMinimumTokenBalance && !inSwapAndLiquify && !_isMarketPair[from] && swapAndLiquifyEnabled)
        {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 tokensForLP = tAmount.mul(_liquidityFee).div(_totalLiqFee).div(2);
        uint256 tokensForSwap = tAmount.sub(tokensForLP);

        swapTokensForEth(tokensForSwap);

        uint256 amountReceived = address(this).balance;
        uint256 totalBNBFee = _totalLiqFee.sub(_liquidityFee.div(2));
        
        uint256 bnbForLiquidity = amountReceived.mul(_liquidityFee).div(totalBNBFee).div(2);
        uint256 bnbForBuyback = amountReceived.mul(_buyBackFee).div(totalBNBFee);
        uint256 bnbForMarketing = amountReceived.sub(bnbForLiquidity).sub(bnbForBuyback);
        ...
        if(bnbForBuyback > 0) {
            transferToAddressETH(buybackWalletAddress, bnbForBuyback);
        }
