...
contract TTHStake is Owned {
    ...
    mapping (address => stakerini) Staker;
    address[] public StakerAddresses;
    // mapping(address => mapping(address => uint)) public Staker;
    ...
    function stake(uint _amount) external onlyRegistered() whenActive() {
        ...
        Staker[msg.sender].lastClocks = block.timestamp.sub(remainder);
        Staker[msg.sender].stakeDayss = 20;
        Staker[msg.sender].totalStakeds = afterTax;
        Staker[msg.sender].idRandomer = Staker[msg.sender].idRandomer + 1;
        StakerAddresses.push(msg.sender);
