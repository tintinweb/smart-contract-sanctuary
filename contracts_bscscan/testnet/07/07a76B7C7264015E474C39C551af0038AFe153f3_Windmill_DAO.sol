 ...
 */
interface I_Windmill_Stacking is I_With_PWRToken, IAccessControlEnumerable {
    ...
    function percentPWRStackingMintNumerator() external view returns (uint256);
    function percentPWRStackingMintDenominator() external view returns (uint256);

    function stackingBonusFactorNumerator() external view returns (uint256);
    function stackingBonusFactorDenominator() external view returns (uint256);

    function earlyUnstackFeesPercyclePercentNumerator() external view returns (uint256);
    function earlyUnstackFeesPercyclePercentDenominator() external view returns (uint256);
 ...
 */
interface I_Windmill_DAO is I_With_PWRToken{
    /**
     ...
     */
    function setBUSDToken(IERC20 _BUSDToken) external;
     ...
     */
    function setRoyaltydAddress(I_Windmill_Royalty _royalty) external;
    ...
    function updateCycleNeeded() external view returns (bool);

    function updateOneCycle() external;
...
contract Windmill_DAO is With_PWRToken, I_Windmill_DAO, Gas_Refundable, Ownable{
    ...
    function init(uint256 _nbProposalPerUserPerCycle,
                  ...
                  uint256 _superVoteMajorityPercent,
                  uint256 _cycleDurationNbBlock,
                  bool[] calldata _isProposalSuper) external onlyOwner{
    ...
    function setFundAddress(I_Windmill_Fund _fund) external onlyOwner{
        fund = _fund;
    }

    function setRoyaltydAddress(I_Windmill_Royalty _royalty) external onlyOwner{
        royalty = _royalty;
    }

    function setStackingAddress(I_Windmill_Stacking _stacking) external onlyOwner{
        stacking = _stacking;
    }
