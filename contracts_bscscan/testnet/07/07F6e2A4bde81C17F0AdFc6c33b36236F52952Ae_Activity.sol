...
contract Activity {
    ...
    function removeEmoji(uint activity_id) public {
        address sender = msg.sender;
        uint _user_id = users.getUserId(sender);
        for (uint i = 0; i < emojis.length; i++) {
            if ((emojis[i].activity_id == activity_id) && (emojis[i].user_id == _user_id)) {
                delete emojis[i];
            }
        }
        revert('Emoji does not exist');
    }

    function getEmojiDetail(uint activity_id, uint user_id) view public returns(uint, uint, string memory) {
        uint k = 0;
        bool validUser = false;
        for (uint i = 0; i < emojis.length; i++) {
            for (uint j = 0; j < emojis.length; j++) {
                if ((emojis[i].activity_id == activity_id) && (emojis[j].user_id == user_id)) {
                    k = j;
                    validUser = true;
                }
            }
        ...
        if (validUser == true) {
            return (emojis[k].activity_id, emojis[k].user_id, emojis[k].emoji);
        } else revert('Cannot find Emoji');
    }
}