...
contract PN is Context, IBEP20, Ownable {
	...
	address payable public _unoWallet;
    ...
    string private constant _name = "Pene Coin";
	...
	uint256 public _unoFee = 4;
    uint256 private _previousUnoFee = _unoFee;
    ...
    constructor (address payable marketing, address payable _uno) {
   ...
   function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
	...
	function setUnoFeePercent(uint256 unoFee) external onlyOwner() {
        _unoFee = unoFee;
    }
	...
	function setUnoWallet(address payable unoWallet) external onlyOwner() {
         _unoWallet = unoWallet;
    }
	...
	function _takeUno(uint256 tUno) private {
        uint256 currentRate =  _getRate();
        uint256 rUno = tUno.mul(currentRate);
        _rOwned[_unoWallet] = _rOwned[_unoWallet].add(rUno);
        if(_isExcluded[_unoWallet])
            _tOwned[_unoWallet] = _tOwned[_unoWallet].add(tUno);
    }
	...
	function calculateUnoFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_unoFee).div(
            10**2
        );
    }

    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _unoFee == 0 && _marketingFee == 0) return;

        _previousTaxFee = _taxFee;
        _previousLiquidityFee = _liquidityFee;
		_previousUnoFee = _unoFee;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
		...
		uint256 tUno = calculateUnoFee(amount);
		if(tMarketing > 0){
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount.sub(tMarketing).sub(tUno));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount.sub(tMarketing).sub(tUno));
        }  else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount.sub(tMarketing).sub(tUno));
        } else {
            _transferStandard(sender, recipient, amount.sub(tMarketing).sub(tUno));
        }
