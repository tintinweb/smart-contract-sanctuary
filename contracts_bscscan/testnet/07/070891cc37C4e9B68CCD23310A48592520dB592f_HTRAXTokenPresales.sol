...
contract HTRAXTokenPresales is Crowdsale, Ownable {
    uint256  public limitBuy = 3000000000000000000;
    TokenInterface TokenContract;

    constructor(uint256 rate, address payable wallet, ERC20 token, address _addressToken) Crowdsale(rate, wallet, token) {
  ...
  function buyTokens(address _beneficiary) public override payable {      
    //require(TokenContract.balanceOf(_beneficiary) <= limitBuy.mul(rate), "This account has already purchased tokens");
    uint256 weiAmount = msg.value;   
    require(weiAmount <= limitBuy, "Maximum purchase is 3 BNBs.");
...
contract Crowdsale {
   ...
   */
  function _preValidatePurchase(address _beneficiary, uint256 _weiAmount) internal view virtual {
