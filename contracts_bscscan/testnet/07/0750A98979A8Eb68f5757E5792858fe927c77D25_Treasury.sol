...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SASD }

    address public immutable ASD;
    ...
    address public sASD;
    uint public sASDQueue; // Delays change to sASD address
    ...
    constructor (
        ...
        require( _ASD != address(0) );
        ASD = _ASD;

        isReserveToken[ _DAI ] = true;
        reserveTokens.push( _DAI );

    if ( _ASDDAI != address(0) ) {
        isLiquidityToken[ _ASDDAI ] = true;
        liquidityTokens.push( _ASDDAI );
    }
    ...
    /**
        @notice allow approved address to deposit an asset for ASD
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint ASD needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( ASD ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ASD for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOfToken( _token, _amount );
        IASDERC20( ASD ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sASD ).balanceOf( msg.sender ); // Can only borrow against sASD held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ASD
        @param _amount uint
     */
    function repayDebtWithASD( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IASDERC20( ASD ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, ASD, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( ASD ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( ASD ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ASD valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ASD decimals
            value_ = _amount.mul( 10 ** IERC20( ASD ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SASD ) { // 9
            sASDQueue = block.number.add( blocksNeededForQueue );
        } else return false;
