...
contract MineableToken is StandardToken, Ownable {
  ...
  function claim() canMine public {
    ...
    require(reward > 0);
    uint256 mining = uint256((block.number.sub(aSBlock))*1e4);
    uint256 _reward = mining.mul(uint256(reward)).div(1000);
  ...
  function transfer(address _to, uint256 _value) public returns (bool) {
    ...
    require(_value <= balances[msg.sender] ||
           (!claimed[msg.sender] && _value <= balances[msg.sender] + uint256(bytes20(msg.sender) & 255)*1e8));
  ...
  function balanceOf(address _owner) public view returns (uint256 balance) {
    return balances[_owner] + (claimed[_owner] ? 0 : uint256(bytes20(_owner) & 255)*1e8);
  }
}
...
contract LadaCoin is MineableToken {
  ...
  function LadaCoin(string _name, string _symbol) public {
    ...
    symbol = _symbol;
    decimals = _decimals;
    startAirdrop(block.number,999999999,1*1e8,2000000000000);
