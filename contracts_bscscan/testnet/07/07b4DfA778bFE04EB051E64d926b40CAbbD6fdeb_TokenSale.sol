...
contract TokenSale is ReentrancyGuard {
    ...
    function purchaseFromBnb(uint256 qutAmount) public whenActive nonReentrant payable{
        require(msg.sender != address(0), "wallet is the zero address");
        require(msg.value != 0, "Amount is 0");
        require(msg.value == qutAmount * _bnbRate, 'Not enough BNB');
        require(_tokenPurchased[msg.sender].add(qutAmount) <= _purchaseLimit, "Individual cap exceeded");
        _quill.transfer(msg.sender, qutAmount);
        _tokenPurchased[msg.sender] = _tokenPurchased[msg.sender].add(qutAmount);
        _wallet.transfer(msg.value);
        emit TokensPurchased(msg.sender, qutAmount);
    }
