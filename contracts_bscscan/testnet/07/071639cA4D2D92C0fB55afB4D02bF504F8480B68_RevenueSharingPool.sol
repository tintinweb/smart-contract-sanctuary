...
contract RevenueSharingPool is Ownable {
    ...
    function getDepositDate() internal view returns (uint256) {
        for (uint256 i = 1; i <= MAX_DATE; i++) { 
            if (block.timestamp >= START_ROUND_DATE && block.timestamp < START_ROUND_DATE + (i * 1 minutes)) { // must be change from minutes to days 
                return i;
            }
        }
    }
    ...
    ) external payable { // must be onlyOwner function
