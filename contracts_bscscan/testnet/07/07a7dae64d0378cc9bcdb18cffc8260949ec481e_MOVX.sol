...
contract MOVX is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    string private _name = "MOV-X";
    string private _symbol = "MOV-X";
    ...
    uint256 private _sellResearchFee = 8;
    ...
    uint256 private _previousTotalBuyFee = totalBuyFee;
    ...
    uint256 public _sellMaxTxAmount = 500000 * 10**9; // 0.05% percent of total supply per sell transaction
    ...
    constructor (address payable buybackWallet, address payable marketingWallet, address payable researchWallet, address pancakeSwapRouter) {
        ...
        _isExcludedFromFee[address(pancakeSwapV2Pair)] = true;
        _isExcludedFromFee[address(pancakeSwapV2Router)] = true;
        ...
        _isExcludeAddressFromMaxTx[address(pancakeSwapV2Pair)] = true;
        ...
        _isExcludedFromTransactionlock[address(_burnAddress)] = true;
        _isExcludedFromTransactionlock[address(_buybackWallet)] = true;
        _isExcludedFromTransactionlock[address(_marketingWallet)] = true;
        _isExcludedFromTransactionlock[address(_researchWallet)] = true;
        _isExcludedFromTransactionlock[address(pancakeSwapV2Pair)] = true;
    ...
    function setFeePercent(uint256 Taxfee, uint256 BuybackFee, uint256 LiquidityFee, uint256 MarketingFee, uint256 ResearchFee) external onlyOwner() {
        taxFee = Taxfee;
        buybackFee = BuybackFee;
        liquidityFee = LiquidityFee;
        marketingFee = MarketingFee;
        researchFee = ResearchFee;
        totalLiqFee = liquidityFee.add(buybackFee).add(marketingFee).add(researchFee);
    }
    ...
    function setBuyFeePercent(uint256 Taxfee, uint256 BuybackFee, uint256 LiquidityFee, uint256 MarketingFee, uint256 ResearchFee) external onlyOwner() {
        ...
        totalBuyFee = _buyLiquidityFee.add(_buyBuybackFee).add(_buyMarketingFee).add(_buyResearchFee);
    }
    ...
    function setSellFeePercent(uint256 SellTaxfee, uint256 SellBuybackFee, uint256 SellLiquidityFee, uint256 SellMarketingFee, uint256 SellResearchFee) external onlyOwner() {
        ...
        totalSellFee = _sellLiquidityFee.add(_sellBuybackFee).add(_sellMarketingFee).add(_sellResearchFee);
    }
    
    function setMaxTxAmount(uint256 buyTxAmount, uint256 sellTxAmount, uint256 maxWalletAmount) external onlyOwner() {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!(_isExcludeAddressFromMaxTx[from] || _isExcludeAddressFromMaxTx[to]) && (from == pancakeSwapV2Pair)) {
            require(amount <= _buyMaxTxAmount, "Transfer amount exceeds the Buy maxTxAmount.");
        }

        if(to != address(0) && to != pancakeSwapV2Pair) {
            uint256 contractBalanceRecepient = balanceOf(to);
            require(contractBalanceRecepient + amount <= _maxWalletAmount, "Exceeds maximum wallet amount");
        }

        if (!_isExcludedFromTransactionlock[from] && limitByDay && (to == pancakeSwapV2Pair) && (from != pancakeSwapV2Pair)) {
            ...
            require(_isExcludedFromTransactionlock[from] || (_transactionCheckpointAmt[from] + amount <= _sellMaxTxAmount)
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(pancakeSwapV2Pair) > 0) {
            if (to == pancakeSwapV2Pair) {
                if (overMinTokenBalance && _startTimeForSwap + _intervalMinutesForSwap <= block.timestamp) {
                    _startTimeForSwap = block.timestamp;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 ethForLiquidity = balanceDiff.sub(ethForBuyback).sub(ethForMarketing).sub(ethForResearch);
