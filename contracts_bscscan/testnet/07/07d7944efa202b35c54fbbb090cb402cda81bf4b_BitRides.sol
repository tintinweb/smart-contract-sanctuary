...
contract RewardsDistributor is IRewardsDistributor {
    ...
    // initialilze with the mainnet and get from the router in the contstructor in case we are on testnet
    // 3% reward paid in WBNB
    IBEP20 addressRewardWBNB = IBEP20(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c); 
    ...
    uint256 public totalBitRidesDistributed;
    ...
    constructor (address _router) {
        ...
        _token = msg.sender;
        addressRewardWBNB = IBEP20(_pancakeswapV2Router2.WETH()); 
    }
    ...
    function depositBitrides() external payable override onlyToken {
        // RIDES are deposited to the contract automatically on buys/sells, no need to convert
    ...
    function depositWBNB() external payable override onlyToken {
        // already in WBNB, so no pancakeswapping is needed
        uint256 amount = msg.value;
    ...
    function distributeDividendBitRides(address shareholder) internal {
        ...
        if(amount > 0){
            totalBitRidesDistributed = totalBitRidesDistributed.add(amount);
    ...
    function distributeDividendWBNB(address shareholder) internal {
        ...
        if(amount > 0){
            totalWBNBDistributed = totalWBNBDistributed.add(amount);
...
contract BitRides is IBEP20, Ownable {
    ...
    constructor () {
        ...
        _pancakeswapV2Pair = IUniswapV2Factory(_pancakeswapV2Router.factory())
            .createPair(_pancakeswapV2Router.WETH(), address(this));
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender != owner() 
			...
			&& sender != _pancakeswapV2Pair 
			&& recipient != address(addressDead) 
			&& recipient != _pancakeswapV2Pair 
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountReflectionBitRides = amountToSwap.mul(reflectionFeeBitRides).div(totalFee);
        uint256 amountBNBReflectionWBNB = amountBNB.mul(reflectionWBNBFee).div(totalBNBFee);
        uint256 amountBNBMarketing = amountBNB.mul(marketingFee).div(totalBNBFee);

        // deposit the WBNB reflection reward dividend to the contract ( not necessary )
        // update the totals
