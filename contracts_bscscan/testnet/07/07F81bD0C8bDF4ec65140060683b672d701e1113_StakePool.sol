...
/** 
    ...
    @notice Version v1.0.7  date: dec 16, 2021
*/
...
contract StakePool is ReentrancyGuard, Ownable, Pausable {
   ...
   modifier withdrawEnabled {
    ...
    function stake(uint _amount) external stakeEnabled {
        ...
        /// Storing reward before manuplating user balance and timestamp so reward doesn't get lost
        ...
        /// lock time when  user start staking
        uint _unlock = block.timestamp;   // Staking time

        /// store the times in a map
    ...
    function withdraw(uint _amount) public virtual withdrawEnabled {
        ...
        /// make the actual withdraw transfer
    ...
    function calculateWithdrawFees(uint _amount) internal view returns (uint _fee){
        ...
        /// Storing time when user staked to local variable
        uint _stakingTime = timeLocks[msg.sender]; 

        /// Comparing current time and staking time plus 8 weeks
        /// There is no fee after 8 weeks    
        if(block.timestamp > _stakingTime + uint(8 weeks)){
             _fee = 0;
        }

        /// If shorter then 8 weeks the withdraw fee is 2%. this overwrites previews setting
        if(block.timestamp <= _stakingTime + uint(8 weeks)){
           _fee = (_amount * 2) / 100;
        }
        
        /// If shorter then 6 weeks the withdraw fee is 5%. this overwrites previews setting
        if(block.timestamp <= _stakingTime + uint(6 weeks)){
           _fee = (_amount * 5) / 100;
        }

        /// If shorter then 4 weeks the withdraw fee is 10%. this overwrites previews setting
        if(block.timestamp <= _stakingTime + uint(4 weeks)){
           _fee = (_amount * 10) / 100;
        }

        /// If shorter then 2 weeks the withdraw fee is 20%. this overwrites previews setting
    ...
    function claimReward() public  {
        ...
        /// Updating rewards mapping
        rewards[msg.sender] += pendingRewards(msg.sender); 
        uint _reward = rewards[msg.sender];

        /// check if sender has reward balance
...
/// @dev  subroutines
    ...
    function viewCollectedFee() external view onlyOwner returns(uint) {
        return collectedFee;
    }
    ...
    function pendingRewards(address _account) public view returns(uint){
        ...
        /// how much tokens user will get in a year based on its balance
        uint _tokens = _balance.mul(apy).div(100);  
        
        /// how much tokens user will get every second
        uint _tokensPerSecond = _tokens.div(52 * 1 weeks);
        
        /// time when user staked or claimed rewards
        uint _userTime = userTime[_account]; 
        
        /// current time on latest block
        uint _currentTime = block.timestamp; 
        
        /// subtracting current time with user staking time to get no of seconds user have staked
        uint _time = _currentTime.sub(_userTime); 
        
        /// multiply the tokens per second with not of seconds
    ...
    /// @dev Returns no of total tokens all users earned based on staking balance
    /// @dev Only for the owner of thist contract
    ...
    /// @dev Function to see rewards
    /// @param _account Address of user
    /// @return No of reward available to claim
