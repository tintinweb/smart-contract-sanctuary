...
contract ag4tst is IERC20, Ownable {
    ...
    string private constant NAME = "ag4test"; 
    string private constant SYMBOL = "AG4T"; 
	...
	address public prouter = 0xBBe737384C2A26B15E23a181BDfBd9Ec49E00248;
    ...
    modifier onlyOperator {
        require (msg.sender == operator || msg.sender == owner(), "FL2: Not authorised");
        _;
    }
	
	function setRouter (address newRouter) external onlyOwner {
        require (newRouter != address(0), "FL2: Can't set router to the zero address");
        prouter = newRouter;
    }
    ...
    function setOperator (address newOperator) external onlyOwner {
        require (newOperator != address(0), "FL2: Can't set operator to the zero address");
    ...
    function decreaseAllowance (address spender, uint256 subtractedValue) external returns (bool) {
        require (subtractedValue <= _allowedFragments[msg.sender][spender], "FL2: Allowance not high enough");
    ...
    function _approve (address owner, address spender, uint256 amount) private {
        require (owner != address(0), "FL2: Cannot approve from the zero address");
        require (spender != address(0), "FL2: Cannot approve the zero address");
    ...
    function _transfer (address sender, address recipient, uint256 amount) private {
        require (recipient != address(0), "FL2: Cannot transfer to the zero address");
        require (amount > 0, "FL2: Cannot transfer zero tokens");
        require (!isBlacklisted[sender] && !isBlacklisted[recipient], "FL2: Blacklisted");
        require (launchTime != 0 || _isExcluded[sender] || _isExcluded[recipient], "FL2: Not launched");
        ...
        if (!_isExcluded[sender] && !_isExcluded[recipient])
            require (amount <= _maxTxAmount, "FL2: Transfer amount exceeds the maxTxAmount.");
    ...
    function _tokenTransfer (address sender, address recipient, uint256 amount) private {
        ...
        if (!_isExcluded[recipient] && sender != owner() && recipient != swapPair)
            require (_fomoBalances[recipient] + receiveAmount <= maxWalletAmount, "FL2: Can't hold that much");
    ...
    function airDrop(address[] calldata recipients, uint256[] calldata amounts) external onlyOwner {
        require (recipients.length == amounts.length, "FL2: Recipients and amounts lengths must match");
    ...
    function setFees (uint256 _marketingFee, uint256 _liquidityFee, uint256 _lotteryFee) external onlyOwner {
        ...
        require (totalFee < 3000, "FL2: Total fee must be < 30%");
    ...
    function setMaxTx (uint256 _maxTx) external onlyOwner {
        require (_maxTx > 10, "FL2: Max tx must be > 0.1%");
        maxTx = _maxTx;
    }

    function setMaxWallet (uint256 _maxWallet) external onlyOwner {
        require (_maxWallet > 10, "FL2: Max wallet must be > 0.1%");
    ...
    function withdrawWETH (address _account) external onlyOwner  {
        require (_account != address(0), "FL2: Can't withdraw to the zero address");
    ...
    function withdrawToken (address _token, address _account) external onlyOwner {
        require (_token != address(0), "FL2: Can't withdraw a token of zero address");
        require (_token != address(this), "FL2: Can't withdraw token");
        require (_account != address(0), "FL2: Can't withdraw to the zero address");
