...
contract ElpisIGO is Pausable, Ownable {
    ...
    constructor(
        ...
        require(_blockStartPreSale >= block.number, "constructor: invalid pre-sale time");
        require(_blockStartPublicSale > _blockStartPreSale, "constructor: invalid public sale time");
        require(_blockFinishSale > _blockStartPublicSale, "constructor: invalid sale end time");
    ...
    modifier validTransaction(uint256 tokensNumber) {
        require(tokensNumber > 0, "ElpisIGO: wrong amount");
        require(
            tokensNumber <= maxTokensPerTransaction,
            "ElpisIGO: max tokens per transaction number exceeded"
        );
        require(
            nftsNumberSold.add(tokensNumber) <= nftsPublicSale,
            "ElpisIGO: tokens number to mint exceeds number of public tokens"
        );
        require(
            nftPrice.mul(tokensNumber) <= msg.value,
    ...
    function setWhiteList(
        ...
        require(
            ...
            "setWhiteList: addressWhiteList and allowances length mismatch"
        );
        for (uint256 i = 0; i < _addressWhiteList.length; ++i) {
            whiteList[_addressWhiteList[i]] = _allowances[i];
        }
    }
    ...
    function buyHeroesPreSale(uint256 tokensNumber)
        external
        payable
        whenNotPaused
        onlyWhiteList
        validTransaction(tokensNumber)
    {
        whiteList[address(msg.sender)] = whiteList[address(msg.sender)].sub(
            tokensNumber,
            "buyHeroesPreSale: the amount of purchase exceeds allowance"
        );
        nftsNumberSold = nftsNumberSold.add(tokensNumber);
        for (uint256 i = 0; i < tokensNumber; i++) {
            elpisHero.mint(address(msg.sender));
        }
    }

    function buyHeroes(uint256 tokensNumber)
        external
        payable
        whenNotPaused
        onlyOpeningTimes
