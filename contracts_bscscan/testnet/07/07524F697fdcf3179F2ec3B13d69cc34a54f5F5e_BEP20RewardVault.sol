...
contract BEP20RewardVault is Ownable {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 accRewardTokenPerShareIndicator = pool.accRewardTokenPerShare;
        ...
        return
            ...
            indicator.amount.mul(accRewardTokenPerShareIndicator).div(1e30).sub(
                indicator.rewardDebt
            );
    }
    ...
    function deposit(uint256 _amount, address _indicator) public {
        ...
        if (_amount > 0) {
            if (user.indicator == address(0)) {
                ...
                indicatorInfo[_indicator].amount.add(_amount); //create indicator client  list with client amount
            }
        ...
        if (_indicator != address(0)) {
            indicatorInfo[_indicator].rewardDebt = user
                .amount
                .mul(pool.accRewardTokenPerShare)
                .div(1e30);
        }
        ...
        returns (uint256)
    {
        ...
        if (indicator.amount > 0) {
            ...
            uint256 accRewardTokenPerShareIndicator = tokenRewardIndicator
                .mul(1e30)
    ...
    function withdraw(uint256 _amount) public {
        ...
        if (_amount > 0) {
            if (indicatorInfo[user.indicator].amount >= _amount) {
                indicatorInfo[user.indicator].amount.sub(_amount);
            }
        ...
        if (user.indicator != address(0)) {
            indicatorInfo[user.indicator].rewardDebt = user
                .amount
                .mul(pool.accRewardTokenPerShare)
                .div(1e30);
        }
