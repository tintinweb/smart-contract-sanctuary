...
contract TKODA is Context, IBEP20, Ownable { // change contract name
    ...
    function setRouterAddress(address newRouter) public onlyOwner {
        require(newRouter != address(summitSwapRouter), "New router address is same as current router");
        
        ISummitSwapRouter02 _newSummitSwapRouter = ISummitSwapRouter02(newRouter);
        address doesPairExist = ISummitSwapFactory(_newSummitSwapRouter.factory()).getPair(address(this), _newSummitSwapRouter.WETH());
        if(doesPairExist == address(0))
            summitSwapPair = ISummitSwapFactory(_newSummitSwapRouter.factory()).createPair(address(this), _newSummitSwapRouter.WETH());
        else
            summitSwapPair = doesPairExist;
