...
contract HuanCasino is ERC1155Holder {
 ...
 fallback() external payable {}
  ...
  function setBetBase(uint _base) public onlyOwner{
    BET_BASE = _base;
  }
...
//get tones token
  ...
  function distributePrizes_5(uint256 numberWin) private {
    ...
    numberWinner_5 = (numberWin + salt) % BET_BASE + 1;
    ...
    for (uint256 i = 0; i < players_5.length; i++) {
      ...
      if (playerInfo_5[playerAddress].numberSelected == numberWinner_5) {
        ...
        sendNftTo(playerAddress, numberMapNftId_5[numberWinner_5], 1);
        countWin++;
      } else {
  ...
  function distributePrizes_10(uint256 numberWin) private {
    ...
    numberWinner_10 = (numberWin + salt) % BET_BASE + 1;
    ...
    for (uint256 i = 0; i < players_10.length; i++) {
      ...
      if (playerInfo_10[playerAddress].numberSelected == numberWinner_10) {
        ...
        sendNftTo(playerAddress, numberMapNftId_10[numberWinner_10], 1);
        countWin++;
      } else {
        losers[countLose] = playerAddress;
        countLose++;
  ...
  function sendNftTo(address player, uint id, uint nftAmout) private {
    require(nftsOfBalance[id] >= nftAmout, "not enough nft");
