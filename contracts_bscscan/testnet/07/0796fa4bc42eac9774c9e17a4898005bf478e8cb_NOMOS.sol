...
contract NOMOS is Context, IBEP20, Ownable {
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _liqFee,uint256 _marketingFee,address _FeeAddress,address tokenOwner) payable   {
		_NAME = _name;
		_SYMBOL = _symbol;
		_DECIMALS = _decimals;
	...
	function updateFee(uint256 _txFee,uint256 _burnFee,uint256 _marketingFee,uint256 _liqFee) onlyOwner() public{
		require(_txFee < 100 && _burnFee < 100 && _marketingFee < 100 && _liqFee < 100);
        _TAX_FEE = _txFee* 100; 
        _BURN_FEE = _burnFee * 100;
		_MARKETING_FEE = _marketingFee* 100;
		_LIQ_FEE = _liqFee* 100;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(_LIQ_FEE > 0 && _MARKETING_FEE > 0 && _BURN_FEE > 0 && _TAX_FEE > 0){
                 require(
