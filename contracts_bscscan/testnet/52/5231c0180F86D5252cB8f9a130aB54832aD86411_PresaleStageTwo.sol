...
contract PresaleStageTwo is Ownable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    uint256 private immutable _devFee = 8;
    uint256 private immutable _marketingFee = 8;
    uint256 private immutable _referrerFee = 5;
    ...
    bool public claimingTokensPermitted;
    bool public finalized;

    mapping(address => bool) public isStageOneContributor;
    ...
    function contribute(address referrer) public payable {
        require(
            referrer != msg.sender,
            "DeGeThalPresaleStageTwo: self referral not allowed"
        );
    ...
    function _buyTokens(
        ...
        tokenContributions[beneficiary] = tokenContributions[beneficiary].add(
            weiAmount
        );
        totalContributions[beneficiary] = totalContributions[beneficiary].add(
            weiAmount
        );
        ...
        if (referrer != address(0)) {
            ...
            totalContributions[referrer] = totalContributions[referrer].add(
                referrerFeeAmount
            );
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "DeGeThalPresaleStageTwo: sale is not open");
        require(!hasEnded(), "DeGeThalPresaleStageTwo: sale is over");
        require(
            weiAmount >= minContribution,
            "DeGeThalPresaleStageTwo: min contribution criteria not met"
        );
        require(
            tokenContributions[beneficiary].add(weiAmount) <= maxContribution,
            "DeGeThalPresaleStageTwo: max contribution criteria not met"
        );
        this;
    }

    function claimTokens() external {
        require(hasEnded(), "DeGeThalPresaleStageTwo: sale is not over");
        require(
            softCapReached(),
            "DeGeThalPresaleStageTwo: soft cap not reached, refund is available"
        );
        require(
            claimingTokensPermitted,
            "DeGeThalPresaleStageTwo: tokens could only be clamed after claiming tokens is permitted"
        );
        require(
            hasClaimableContributions(msg.sender),
            "DeGeThalPresaleStageTwo: nothing to claim"
        );
        uint256 tokens = _getTokenAmount(totalContributions[msg.sender]);
        referredTokens[msg.sender] = _getTokenAmount(
            referrerContributions[msg.sender]
        );
    ...
    function claimRefund() external {
        ...
        require(
            !softCapReached(),
            "DeGeThalPresaleStageTwo: soft cap not reached"
        );
        require(
            tokenContributions[msg.sender] > 0,
            "DeGeThalPresaleStageTwo: nothing to claim"
    ...
    function endPresale() external onlyOwner {
        require(!finalized, "DeGeThalPresaleStageTwo: sale is already over");
        finalized = true;
        if (weiRaised > softCap) {
            _transferTotalWeiRaised();
        }
        emit PresaleFinalized(weiRaised);
    }

    function withdrawTokens() public onlyOwner {
        require(finalized, "DeGeThalPresaleStageTwo: sale is not over");
        uint256 tokens = token.balanceOf(address(this));
        token.transfer(owner(), tokens);
    }

    function addStageOneContributorsList(
        address[] memory stageOneContributorsList
    ) external onlyOwner {
        for (uint256 i = 0; i < stageOneContributorsList.length; i++) {
            isStageOneContributor[stageOneContributorsList[i]] = true;
        }
    }

    function permitClaimingTokens() external onlyOwner {
        claimingTokensPermitted = true;
    }

    function addStageOneContributor(address contributorAddress)
        external
        onlyOwner
    {
        isStageOneContributor[contributorAddress] = true;
    }

    function _transferTotalWeiRaised() private {
        uint256 totalWeiRaised = address(this).balance;
        uint256 devFee = totalWeiRaised.mul(_devFee).div(10**2);
        uint256 marketingFee = totalWeiRaised.mul(_marketingFee).div(10**2);
        uint256 ownerWeiAmount = totalWeiRaised - devFee - marketingFee;

        payable(devWallet).transfer(devFee);
        payable(marketingWallet).transfer(marketingFee);
        payable(owner()).transfer(ownerWeiAmount);
    }
    ...
    function hasClaimableContributions(address addr)
        public
        view
        returns (bool)
    {
        if (tokenContributions[addr] > 0) {
            return true;
        }

        return referrerContributions[addr] > 0 && isStageOneContributor[addr];
    }
}