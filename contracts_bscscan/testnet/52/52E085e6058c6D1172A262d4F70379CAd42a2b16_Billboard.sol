...
contract Billboard is Ownable {
    ...
    function getEntryName(uint adID) public view returns(string memory) {
      ...
      string memory val = "0";
      string memory startKey = string(abi.encodePacked(separator, val));
      string memory returnStr = stringUnpacker.findInString(startKey, separator, searchString);
      return returnStr;
    }
    ...
    function getEntrySymbol(uint adID) public view returns(string memory) {
      ...
      string memory val = "1";
      string memory startKey = string(abi.encodePacked(separator, val));
      return stringUnpacker.findInString(startKey, separator, searchString);
    }
...
library stringUnpacker {
  ...
  function findInString(string memory startKey, string memory endKey, string memory searchString) internal view returns (string memory) {
    uint posStart;
    uint posEnd;
    string memory slice;
