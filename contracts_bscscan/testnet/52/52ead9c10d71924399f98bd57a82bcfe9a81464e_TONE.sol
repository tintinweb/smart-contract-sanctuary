...
2) 2,5% burn on every transaction
3) 5% tax is collected and it is added to the Liquidity
4) 1,5% tax is collected and is sent to a specific wallet created for marketing purposes
...
contract TONE is Context, IERC20, Ownable {
    ...
    string private _name = "TONE";
    string private _symbol = "TONE";
	...
	uint256 public coolDown = 30 * 1 seconds;
    ...
    uint256 private numTokensSellToAddToLiquidity = 200000000 * 10**9;
    uint256 public maxTxAmount = _tTotal.mul(3).div(1000);
    ...
    function manualSend() external onlyOwner{
        swapAndSendToMarketing(address(this).balance);
    }
    ...
    function enableCooldown(bool _enabled) external onlyOwner{
        require(coolDownEnabled != _enabled, 'Status already set');
    ...
    function blacklistBot(address account, bool _bool) external onlyOwner{
        _isBot[account] = _bool;
    }
    ...
    function transferAnyBEP20Tokens(address _tokenAddr, address _to, uint _amount) public onlyOwner {
        require(_tokenAddr != address(this), "Cannot transfer out TONE!");
    ...
    function _transfer(
        ...
        require(!_isBot[from] && !_isBot[to], "Bots can't trade");
        
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            require(amount <= maxTxAmount, 'you are exceeding maxTxAmount');
            if(coolDownEnabled && from == uniswapV2Pair) {
                require(_lastTrade[to].add(coolDown) <= block.timestamp, 'You must wait coolDown');
            }
        }
        ...
        if (
            ...
            uint256 totFee = _liquidityFee.add(_marketingFee);
            uint256 liqTokens = contractTokenBalance.div(totFee).mul(_liquidityFee);
            swapAndLiquify(liqTokens);
            uint256 marketingTokens = contractTokenBalance.sub(liqTokens);
    ...
    function swapAndSendToMarketing(uint256 tokens) private lockTheSwap{
        ...
        _marketingWallet.transfer(transferBalance);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(_burnFee > 0)  {
            ...
            _taxFee = 0; _liquidityFee = 0; _marketingFee = 0;
            _transferStandard(sender, deadAddress, burnAmt);
            _taxFee = _previousTaxFee; _liquidityFee = _previousLiquidityFee; _marketingFee = _previousMarketingFee;
        }
        else{
            if (_isExcluded[sender] && !_isExcluded[recipient]) {
