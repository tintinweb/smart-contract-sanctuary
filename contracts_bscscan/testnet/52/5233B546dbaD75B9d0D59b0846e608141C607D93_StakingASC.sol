...
contract CRS is ERC20MintBurn, AccessControl {
    ...
    uint8 public constant override decimals = 18;
    uint256 public constant genesisSupply = 750000000e18;
...
contract CeresAnchor is AccessControl {
    ...
    function setUsdcAddress(address newAddress) public onlyGovernance {
...
abstract contract Staking is IStaking, ReentrancyGuard, Pausable {
    ...
    address public owner;  // TODO GOVERNANCE
    ...
    function rewardPerShare() public view returns (uint256) {
        ...
        rewardPerShareStored.add(
            lastTimeRewardApplicable().sub(lastUpdateTime).mul(rewardRate).mul(1e18).div(_totalStaking)
        );
    }
    ...
    // ------------------------------------------------------------------------
    function reinvestReward() public override nonReentrant updateReward(msg.sender) {
    ...
    // ------------------------------------------------------------------------
    function getRewardWithPercent(uint256 percent) public nonReentrant updateReward(msg.sender) {
    ...
    function notifyRewardAmount(uint256 reward) external onlyOwner updateReward(address(0)) {
        ...
        // very high values of rewardRate in the earned and rewardsPerShare functions;
        // Reward + leftover must be less than 2^256 / 10^18 to avoid overflow.
        uint balance = coinCRS.balanceOf(address(this)) - _totalStaking;
...
contract StakingASC is Staking, IRedeemer {
    ...
    function redeemEarnedCRS(address account) public view returns (uint256) {
        ...
        / SHARE_PRECISION + redeemsCRS[account];
    }
    

    // ------------------------------------------------------------------------
    // User redeem amount - USDC
    // ------------------------------------------------------------------------
    ...
    // Notify auto redeem
    // ------------------------------------------------------------------------
    function notifyRedeem(uint256 ascAmount, uint256 crsAmount, uint256 usdcAmount) public override onlyPools {
        ...
        if (crsAmount > 0) {
            ...
            redeemPerShareStoredCRS += crsAmount * SHARE_PRECISION / _totalShare;
        }
        if (usdcAmount > 0) {
    ...
    function withdrawAll(uint256 shareAmount) external override {
        require(_shareBalances[msg.sender] > 0, "no share balance");
        uint256 percent = shareAmount * 100 / _shareBalances[msg.sender];
    ...
    function withdraw(uint256 shareAmount) public override nonReentrant updateReward(msg.sender) updateRedeem(msg.sender) {
        require(shareAmount > 0 && shareAmount <= _shareBalances[msg.sender], "check your share amount!");
    ...
    // ------------------------------------------------------------------------
    // get redeem reward by percent 1-100
    // ------------------------------------------------------------------------
    function getRedeemWithPercent(uint256 percent) public override nonReentrant updateRedeem(msg.sender) {
        ...
        if (redeemCRS > 0) {
            ...
            SafeERC20.safeTransfer(coinASC, msg.sender, redeemCRS);
        }
        if (redeemUSDC > 0) {
            redeemsCRS[msg.sender] -= redeemCRS;
            SafeERC20.safeTransfer(coinASC, msg.sender, redeemUSDC);
        }
