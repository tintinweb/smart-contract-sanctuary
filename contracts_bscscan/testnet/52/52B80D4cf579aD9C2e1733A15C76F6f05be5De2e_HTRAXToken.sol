...
contract HTRAXToken is ERC20, ERC20Snapshot, Ownable, Pausable, AccessControl   {
    ...
    function transferDiscountedTokens(address recipient, uint totalAmount) public {
        ...
        if (totalAmount >= 69444 && totalAmount < 2777778) {
            discountedAmount = totalAmount + (totalAmount*10)/100;
        } else if (totalAmount >= 277778 && totalAmount < 2777778) {
            discountedAmount = totalAmount + (totalAmount*20)/100;
        } else if (totalAmount >= 2777778) {
        ...
        uint256 lockedAmount6 = (discountedAmount*10)/100;
        uint64 timeInterval6 = uint64(block.timestamp) + 15778463;
        timeLocks[recipient].push(TimeLock(discountedAmount, lockedAmount6, 
                                  uint128(startDate), timeInterval6, lockedAmount6));

        // Release 30% after 12 months from transfer date
        uint256 lockedAmount12 = (discountedAmount*30)/100;
        uint64 timeInterval12 = uint64(block.timestamp) + 31556926;
        timeLocks[recipient].push(TimeLock(discountedAmount, lockedAmount12, 
                                  uint128(startDate), timeInterval12, lockedAmount12));

        // Relase 30% after 18 months from transfer date
        uint256 lockedAmount18 = (discountedAmount*30)/100;
        uint64 timeInterval18 = uint64(block.timestamp) + 47335389;
        timeLocks[recipient].push(TimeLock(discountedAmount, lockedAmount18, 
                                  uint128(startDate), timeInterval18, lockedAmount18));

        // Relase 30% after 24 months from transfer date
        uint256 lockedAmount24 = (discountedAmount*30)/100;
        uint64 timeInterval24 = uint64(block.timestamp) + 63113852;
        timeLocks[recipient].push(TimeLock(discountedAmount, lockedAmount24, 
                                  uint128(startDate), timeInterval24, lockedAmount24));
