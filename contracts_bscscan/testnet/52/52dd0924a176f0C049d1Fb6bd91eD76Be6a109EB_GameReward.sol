...
//    address public TaxAddress;
    ...
    function transfer(address recipient, uint256 amount) public returns (bool)
    {
        ...
        if (bpEnabled) {
            BP.protect(_msgSender(), recipient, amount);
        }
        ...
        _transfer(_msgSender(), recipient, amount - liquidityFee - rewardFee - marketFee - giveawayFee);

        if(liquidityFee > 0){
            _transfer(_msgSender(), LiquidityAddress, liquidityFee);
        }
        if(rewardFee > 0){
            _transfer(_msgSender(), RewardAddress, rewardFee);
        }
        if(marketFee > 0){
            _transfer(_msgSender(), MarketingAddress, marketFee);
        }
        if(giveawayFee > 0){
            _transfer(_msgSender(), GiveawayAddress, giveawayFee);
        }
    ...
    {
        require(amount <= _allowances[_msgSender()][recipient], "there's no allowed ballance");
        _approve(_msgSender(), recipient, _allowances[_msgSender()][recipient].sub(amount));
...
contract GameReward is Ownable, Events {
    ...
    function lock(address _from, uint256 amount) public returns(bool){
        ...
        IERC20(qdropToken).approve(lockAddress, allowance + amount);
