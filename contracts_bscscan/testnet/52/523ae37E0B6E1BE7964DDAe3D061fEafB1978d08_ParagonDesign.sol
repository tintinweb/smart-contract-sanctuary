...
contract ParagonDesign is Pausable, AccessControl {
    ...
    mapping(address => SupportToken) public supportedToken;
    ...
    mapping(address => Price) public supportedFee;
    ...
    event FeeAdded(address currency, uint256 amount);
    ...
    function editSupportedToken(
        ...
        require(feePercent <= 100000, "Must be valid percent");
        supportedToken[token] = SupportToken(enable, feePercent);
    ...
    function editSupportedFee(
        ...
        supportedFee[currency] = Price(enable, amount);
        emit FeeChange(currency, amount, enable);
    }

    function createParaArt(
        address owner,
        bytes32 hashed,
        address currency
    ) public onlyRole(MINTER_ROLE) {
        Price memory fee = supportedFee[currency];
        require(fee.enable, "Must be supported fee");
        IERC20(currency).transferFrom(owner, address(this), fee.amount);
        paraArt.safeMint(owner, hashed);
        emit FeeAdded(currency, fee.amount);
    }

    function rentByBNB(bytes32 hashed) public payable {
        SupportToken memory token = supportedToken[address(0x0)];
    ...
    function rentByToken(bytes32 hashed, address tokenAddress) public {
        SupportToken memory token = supportedToken[tokenAddress];
        require(token.enable, "Currency must be enable");
        require(paraArt.isValidHashed(hashed), "Hashed is invalid");
...
interface IParaArt {
    function safeMint(address to, bytes32 hashed) external returns (uint256);
