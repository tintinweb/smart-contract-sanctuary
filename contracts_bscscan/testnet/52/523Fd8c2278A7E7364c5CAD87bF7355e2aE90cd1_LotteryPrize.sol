...
import "./gateways/IYieldSource.sol";
import "./interfaces/IGateway.sol";

contract LotteryPrize is PausableUpgradeable, OwnableUpgradeable {
    ...
    mapping(IERC20 => bool) public isSetPrize;
    modifier onlyANLottery() {
        require(_msgSender() == ANLottery, "LotteryPrize: No permission");
        _;
    }

    modifier onlyManager() {
        require(_msgSender() == owner() || _msgSender() == ANLottery, "LotteryPrize: No permission");
        _;
    }

    function initialize(address _ANLottery, IERC20 _erc20, IERC20 _ALI, IYieldSource _yieldSource) public initializer {
    ...
    function setTokenPrize(IERC20 _erc20) public onlyANLottery {
        _erc20.approve(address(ANLottery), UINT256_MAX);
        isSetPrize[_erc20] = true;
    }
    function deposit(uint _poolId, uint _amount) public onlyOwner {
        require(erc20.transferFrom(_msgSender(), address(this), _amount));
        yieldSource.deposit(_poolId, _amount);
        stakeOnBalance();
    }
    function _withdraw(uint _amount) public onlyOwner {
    ...
    function getTotalPrize(address[] memory _erc20s) public view returns(uint[] memory balances) {
        uint pending = yieldSource.pendingALI(0, address(this));
        uint ALIBalance = ALI.balanceOf(address(this));
        balances[balances.length] = pending + ALIBalance;
        for(uint i = 0; i < _erc20s.length; i++) {
            uint erc20Balance = IERC20(_erc20s[i]).balanceOf(address(this));
            balances[balances.length] = pending + erc20Balance;
        }
    }
    ...
    function _giveAllowances() internal {
        ...
        ALI.approve(ANLottery, UINT256_MAX);
    }

    function _removeAllowances() internal {
        erc20.approve(address(yieldSource), 0);
        ALI.approve(ANLottery, 0);
    }
}
