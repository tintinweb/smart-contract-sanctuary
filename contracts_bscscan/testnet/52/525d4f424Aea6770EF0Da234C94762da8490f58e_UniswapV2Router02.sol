...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'db7f6091235b55efc17de1f25c6d5caf2f05cd40a648824df630a8c047769dfa' // init code hash
            ))));
    }
...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    function _transferOut(address pair, address token, uint amount, address to) internal returns (bool success){
        ...
        if(to != address(this)){
            ...
            if(balance < amount){
                IInvestment(vault).withdrawWithReBalance(token, amount.sub(balance));
            }
    ...
    function _transferETH(address to, uint amount) internal {
        ...
        if(balance < amount){
            IInvestment(vault).withdrawWithReBalance(WETH, amount.sub(balance));
        }
