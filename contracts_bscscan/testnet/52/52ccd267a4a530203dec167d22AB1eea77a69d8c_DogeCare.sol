...
contract DogeCare is Context, IBEP20, Ownable {
	...
	uint256 public _charityFee = 400; 
...
// 	uint256 private _previousrewardFee = _rewardFee;	
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_holderFee.add(_liquidityFee).add(_devFee).add(_charityFee)).div(10000);
        return percentage;
    }
    ...
    function _takeCharityFee(uint256 tAmount) private {
        uint256 tCharityFee = tAmount.mul(_charityFee).div(10000);
