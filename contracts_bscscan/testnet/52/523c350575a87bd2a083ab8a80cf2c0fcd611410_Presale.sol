...
contract Presale is  Ownable, Pausable, Initializable, ReentrancyGuard {
    using SafeBEP20 for IBEP20;
    
    event BuyOffer( address indexed user, uint indexed pSale, uint amount, uint qty, uint purchaseTime);
    event Claim( address indexed user, uint releasedAmount, uint claimTime);
    
    mapping(address => mapping(uint => UserInfo)) public userInfo; // user info
    ...
    uint256[2] public offeringAmount;
    uint256[2] public totalSoldTokens;
    // total released tokens
    uint256[2] public totalOfferedTokens;
    // price of offering token
    uint256[2] public offeringPrice = [0.00003e18, 0.00005e18];
    // total amount of raising tokens that have already raised
    uint256[2] public totalAmount;
    // The block number of 6 month for unlock period
    uint256 unlockBlock = 5184000;  
    // The block of launch
     ...
     * @dev Initializes the contract setting the deployer as the initial owner.
     */
    function initialize(IBEP20 _offeringToken, uint256[2] memory _startBlock, uint256[2] memory _endBlock, uint[2] memory _offeringAmount, uint[2] memory _launchBlock) external onlyOwner initializer  {
        require((block.number < _startBlock[0]) && (block.number < _startBlock[1]), "Presale :: initialize : start block must be greater than current block.");
        require((_startBlock[0] < _endBlock[0]) && (_startBlock[1] < _endBlock[1]), "Presale :: initialize : end block must be greater than start block.");
        require((_offeringAmount[0] > 0) && (_offeringAmount[0] > 0), "Presale :: initialize : _offeringAmount must not be zero.");
    ...
    function setOfferingAmountSaleOne(uint256 _offerAmount) external onlyOwner {
        require (block.number < startBlock[0], "Presale :: setOfferingAmountSaleOne : cant change offering amount after start block.");
        offeringAmount[0] = _offerAmount;
    }
    ...
    function setOfferingAmountSaleTwo(uint256 _offerAmount) external onlyOwner {
        require (block.number < startBlock[1], "Presale :: setOfferingAmountSaleTwo : cant change offering amount after start block.");
        offeringAmount[1] = _offerAmount;
    }
     ...
     * Can only be called by the current owner.
     */
    function unsoldTokens( uint _pSale) external onlyOwner onlyPresales( _pSale) {
        require(!isPresaleActive[_pSale], "Presale :: getUnsoldTokens : cannot get tokens until the presale is closed.");
        require(offeringAmount[_pSale] > totalSoldTokens[_pSale], "Presale :: unsoldTokens : no unsold tokens left");
        
        uint _unsold = offeringAmount[_pSale] - (totalSoldTokens[_pSale] - totalOfferedTokens[_pSale]);
        offeringAmount[_pSale] -= _unsold;
        offeringToken.safeTransfer(owner(), _unsold);
    }
    
    /**
     * @dev purchase offering tokens.
     * @param _qty quantity of token user wish to purchase 1 = 1e18
     */
    function buyOffer( uint _pSale, uint _qty) external payable whenNotPaused onlyPresales( _pSale) {
        ...
        require((block.number >= startBlock[_pSale]) && (startBlock[_pSale] != 0), "Presale :: buyOffer : cannot buy token until start block");
        require(block.number <= endBlock[_pSale], "Presale :: buyOffer : presale is closed, cannot buy token after end block");
        require(msg.value == calcualteOfferInBNB( _pSale, _qty), "Presale :: buyOffer : invalid amount");
        require((offeringToken.balanceOf(address(this)) - totalSoldTokens[_pSale]) >= _qty, "Presale :: buyOffer : insufficient offering token"); 
        
        UserInfo storage _usrInfo = userInfo[_msgSender()][_pSale];
        totalAmount[_pSale] += msg.value;
        _usrInfo.balances += _qty;
        totalSoldTokens[_pSale] += _qty;
        
        emit BuyOffer( _msgSender(), _pSale, msg.value, _qty, block.number);
    }
     ...
     */    
    function claim( uint _pSale) external onlyPresales( _pSale) whenNotPaused nonReentrant {
        require(isClaimActive[_pSale], "Presale :: claim : not active");
        require(block.number >= launchBlock[_pSale], "Presale :: claim : wait until claim period starts");
        ...
        _var._currentBlock = block.number;
        _var._oneMonth = unlockBlock/6;
        _var._launchEndMonth = launchBlock[_pSale] + unlockBlock;
        ...
        if(_usrInfo.lastClaimTime == 0) {
            ...
            _usrInfo.lastClaimTime = _var._launchEndMonth;
            _usrInfo.balances -= _var._releaseAmount;  
            _usrInfo.totalClaimAmount += _var._releaseAmount;
        }
             
        if((_var._currentBlock >= (_usrInfo.lastClaimTime+_var._oneMonth)) && (_usrInfo.balances > 0)) {
            
            if(_var._currentBlock > (_var._launchEndMonth + unlockBlock)) _var._currentBlock = _var._launchEndMonth + unlockBlock;
            
            _var._month = (_var._currentBlock - _usrInfo.lastClaimTime)/_var._oneMonth;
            ...
            _usrInfo.lastClaimTime += _var._oneMonth*_var._month;
        ...
        if((_var._releaseAmount > 0) && (offeringAmount[_pSale] >= _var._releaseAmount)) {
            offeringAmount[_pSale] -= _var._releaseAmount;
            totalOfferedTokens[_pSale] += _var._releaseAmount;
            offeringToken.safeTransfer(_msgSender(), _var._releaseAmount);
            emit Claim( _msgSender(), _var._releaseAmount, block.number);
        }
    ...
    function calcualteOfferInBNB( uint _pSale, uint _qty) public onlyPresales( _pSale) view  returns (uint _offer) {
      _offer = offeringPrice[_pSale]*_qty/1e18;
    }
