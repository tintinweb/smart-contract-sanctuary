...
contract MysteryBoxS3 is RandomGenerator, Context {
    ...
    function openBox(uint tokenId_, uint expireAt_, bytes32 r, bytes32 s, uint8 v) external startStage returns (uint) {
        ...
        uint tm = (block.timestamp).sub(startTime);
        require(tm > 7 days, "Has ended");
        ...
        uint rdm = randomCeil(100);
        if (rdm <= 1) {          // 10倍的稀有卡牌
            cardId = 1111;
        } else if (rdm <= 4) {   // 3倍的稀有卡牌
            cardId = 2222;
        } else if (rdm <= 9) {   // 2倍的稀有卡牌
            cardId = 3333;
        } else if (rdm <= 17) {  // 1.5倍的稀有卡牌
            cardId = 4444;
        } else if (rdm <= 72) {
            uint amount = price.mul(60).div(100);
            emit Debug(amount);
            U.transfer(_msgSender(), amount);
            backU += amount;
