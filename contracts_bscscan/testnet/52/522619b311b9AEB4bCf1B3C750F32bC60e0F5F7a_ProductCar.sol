...
contract ProductCar is Ownable,Initializable,Permission{
    ...
    event evtComposeCar(address indexed,uint256 tokenId,uint256[] tokenIds);
    ...
    function mintCar(address to, uint256 tokenId,string calldata _carType) public onlyRole(MINTER_ROLE) needInit virtual {
            nftCar.mint(to, tokenId);
            appConf.mintCar(tokenId,_carType);
            evtMintCar(to,_carType,tokenId);
    } 

    function mintParts(address to, uint256 tokenId,string calldata _carType,string calldata _partsType) public  onlyRole(MINTER_ROLE) virtual {
        //require(hasRole(MINTER_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have minter role to mint");
        //bytes32  a = keccak256(abi.encodePacked(_carType));
        nftCar.mint(to, tokenId);

        appConf.mintParts(tokenId,_carType,_partsType);

        evtMintParts(to,_partsType,tokenId);
    }

    function mintEquitycard(address to, uint256 tokenId,string calldata _cardType) public onlyRole(MINTER_ROLE) virtual {
        //require(hasRole(MINTER_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have minter role to mint");
    ...
    function compose(uint256[] memory tokenIds) public returns(bool){
        ...
        evtComposeCar(ower,tokenId,tokenIds);
    }

    function queryTokenDetail(uint256 tokenId) view public  returns(Model.NftStatus memory) {
         return appConf.queryTokenDetail(tokenId);
    }

    function queryCarProps(uint256 tokenId) view public  returns(Model.carProperty[] memory){
        return appConf.queryCarProps(tokenId);
    }
