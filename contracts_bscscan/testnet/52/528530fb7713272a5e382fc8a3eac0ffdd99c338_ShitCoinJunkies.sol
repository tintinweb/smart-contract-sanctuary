...
contract ShitCoinJunkies is Context, IBEP20, Ownable {
    ...
    mapping (address => bool) public _isExcludeFromShitCoinGuardianCheck;
    ...
    address payable private _externalAddress  = payable(0x31A5C92f142954d5d92483c7aD91FE20B46Ad942); //Binance Charity Wallet
    address payable private _burnAddress      = payable(0x000000000000000000000000000000000000dEaD);

    string private _name    = "ShitCoinJunkies001";
    string private _symbol  = "STJ1";
    ...
    bool private _isShitCoinGuardianWLActivated = true;
    bool private _isShitCoinGuardianBLActivated = true;
    IShitCoinGuardian private _guardianRouter;
    ...
    constructor () {
        ...
        _guardianRouter =  IShitCoinGuardian(0xd61f783cfd65728062BCa6A260FcA19f985155e8);
        
        address _pancakeSwapRouterAddress = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
        ...
        _isExcludeFromShitCoinGuardianCheck[owner()]                 = true;
        _isExcludeFromShitCoinGuardianCheck[address(this)]           = true;
        _isExcludeFromShitCoinGuardianCheck[_pancakePair]            = true;
        _isExcludeFromShitCoinGuardianCheck[address(_pancakeRouter)] = true;
        _isExcludeFromShitCoinGuardianCheck[_burnAddress]            = true;
    ...
    function excludedFromShitCoinGuardianCheck(address account) public onlyOwner {
        _isExcludeFromShitCoinGuardianCheck[account] = true;
    }
    
    function includeInShitCoinGuardianCheck(address account) public onlyOwner {
        _isExcludeFromShitCoinGuardianCheck[account] = false;
    }
    
    function setisShitCoinGuardianWhiteListActivated(bool enable) external onlyOwner(){
        _isShitCoinGuardianWLActivated = enable;
    }
    
    function setisShitCoinGuardianBlackListActivated(bool enable) external onlyOwner(){
        _isShitCoinGuardianBLActivated = enable;
    }
    ...
    //Set Addresses
    function setShitCoinGuardianRouterAddress(address _address) external onlyOwner() {
        _guardianRouter = IShitCoinGuardian(_address);
    }
...
//  }
    ...
    function _transfer(
        ...
        if(from == _pancakePair)
        {
            ...
            if(_isShitCoinGuardianWLActivated)
                require(_isExcludeFromShitCoinGuardianCheck[to] || _guardianRouter.isWhiteListed(to),"Denied");
            else if(_isShitCoinGuardianBLActivated)
                require(_isExcludeFromShitCoinGuardianCheck[to] || !_guardianRouter.checkIsBlackListed(to),"Denied");
    ...
    function __Info_Addresses() external view returns(
        ...
        address External,
        address Guardian
        ) {return (owner(), _pancakePair, address(pancakeRouter), _externalAddress, address(_guardianRouter));
    }
        
    function _Tokenomics_Fee_Percentages() external view returns(
        uint256 burnFee,
    ...
    function _Tokenomics_Limits_NoDecimals() external view returns(
      ...
      function _Tokenomics_Totals_NoDecimals() external view returns(
        uint256 TotalSupply,
        uint256 TotalFees,
    ...
    function __Check_ShitCoinGuardian(address account) external view returns(
        address Router,
        ...
        bool Blacklist
            ...
            return (
            address(_guardianRouter),
            _isShitCoinGuardianWLActivated, 
            _isShitCoinGuardianBLActivated,
            _guardianRouter.isWhiteListed(account),
            _guardianRouter.checkIsBlackListed(account)
            );
    }
    
     function __Check_Exclusions(address account) public view returns (
        bool ExcludedShitCoinGuardian,
        ...
        return (
            _isExcludeFromShitCoinGuardianCheck[account],
            _isExcluded[account],
    ...
    //Launch functions
     function __Launch1_PreLP () public onlyOwner {
        ...
        _isShitCoinGuardianWLActivated    = true;
        _isShitCoinGuardianBLActivated    = true;
    } 
   
   //Fair Launch settings for Whitelist
    function __Launch2_ShitCoinGuardianWhiteList () public onlyOwner {
        _burnFee                    = 0;
        _reflectionFee              = 0;
        _externalFee                = 0;
        _liquidityFee               = 0;
        ...
        _isShitCoinGuardianWLActivated    = true;
        _isShitCoinGuardianBLActivated    = false;
    } 
   ...
   //Fair Launch settings for Public
    function __Launch3_Public () public onlyOwner {
        ...
        _swapAndLiquifyEnabled      = true;
        _isShitCoinGuardianWLActivated    = false;
        _isShitCoinGuardianBLActivated    = true;
    } 
...
/**
 ...
 Purchase of this contract should be considered gambling.
 Tokenomics is by far the most risky financial decision you can make.
 We will do our best to provide safe contracts free of bugs.
 If the event of a failed contract we will make every effort to correct it.
 If the contract is denounced we can do nothing.
 If you buy a contract after it has been denounced you take 100% ownership of the risk.
 LP will either be send to a dead wallet or locked. We will not rug pull.
