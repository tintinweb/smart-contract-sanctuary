...
contract TokenVesting is Ownable {
  ...
  *@param _firstRelease in percent for percent of total tokens that user will receive at first claim
  ...
  */
  function fundVesting(uint256 _totalTokens) public onlyOwner {
    // require(totalTokens < 1000000, "Vesting already funded");
    // address _owner = owner();
    require(token.allowance(msg.sender, address(this)) == _totalTokens, 'Not allow spend MALL token');
  ...
  */
  function claimTokens() public {
    ...
    if (timePassed < cliff) {
      ...
      uint256 firstClaim = user.amount.mul(20).div(100);
      ...
      uint256 _firstClaimed = user.amount.mul(20).div(100);
      uint256 _amount = user.amount.sub(_firstClaimed);
      uint256 tokensToClaim = _amount
        .div(totalPeriods)
        .mul(time)
        .sub(user.tokensClaimed.sub(_firstClaimed));
      if (user.tokensClaimed == 0) {
        tokensToClaim = tokensToClaim.add(_firstClaimed);
      }
