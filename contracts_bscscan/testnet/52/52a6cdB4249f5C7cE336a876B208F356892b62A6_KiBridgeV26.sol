...
contract KiBridgeV26 is KiBridgeV1Factory, Ownable, ReentrancyGuard {
  ...
  bytes32 constant private REQUEST_SPECID = 0x6266643464393434313764303439366261373335373564346462343139616136;
  bytes32 constant private SYN_SPECID = 0x3836313137383965303233343461316339643239653637343839616433393266;
  bytes32 constant private REMOVE_SPECID = 0x3031303763376236666434353439386462363465356636613831326133393164;
  ...
  {
    require(
      _executeFunctionId == this.tokenOutTransferToUser.selector ||
      _executeFunctionId == this.tokenOutMintToUser.selector,
    ...
    onlyOwner
  {
    ...
    require(
      _tokenInFunctionId == this.tokenInReqTransferToChainAddLiq.selector ||
      _tokenInFunctionId == this.tokenInBridgeToChain.selector ||
      _tokenInFunctionId == this.tokenInBurnToChain.selector ||
      _tokenInFunctionId == this.tokenInBurnFromToChain.selector ||
      _tokenInFunctionId == this.tokenInBurnToBurnAddrToChain.selector,
       "Must use whitelisted functions"
    );
  ...
  function liquidityFromChain(
    ...
    private 
  {
    if (liquidityInfo[_token].pendingLiquidityToChain[_chainId] >= _amount) {
      liquidityInfo[_token].pendingLiquidityToChain[_chainId] -= _amount;
    } else {
      liquidityInfo[_token].fulfilledLiquidityFromChain[_chainId] =
      liquidityInfo[_token].fulfilledLiquidityFromChain[_chainId] +
      _amount - liquidityInfo[_token].pendingLiquidityToChain[_chainId];
      liquidityInfo[_token].pendingLiquidityToChain[_chainId] = 0;
    }
  }
  function liquidityToChain(
    ...
    private
  {
    if (liquidityInfo[_token].fulfilledLiquidityFromChain[_chainId] >= _amount) {
      liquidityInfo[_token].fulfilledLiquidityFromChain[_chainId] -= _amount;
    } else {
      liquidityInfo[_token].pendingLiquidityToChain[_chainId] = 
      liquidityInfo[_token].pendingLiquidityToChain[_chainId] +
      _amount - liquidityInfo[_token].fulfilledLiquidityFromChain[_chainId];
      liquidityInfo[_token].fulfilledLiquidityFromChain[_chainId] = 0;
    }
  }
    ...
    checkMaintenance
  {
    ...
    require(userInfo[msg.sender].commitments[requestId] == 0, "Must use a unique ID");
    userInfo[msg.sender].commitments[requestId] = keccak256(abi.encode(_callbackFunctionId,_fromChainId, _toChainId, _txIndex));
    ...
    checkMaintenance
  {
    ...
    userInfo[msg.sender].requestData[requestId] = LiquidityRequest({ 
        requestId: requestId,
    ...
    nonReentrant
  {
    ...
    LiquidityRequest memory liquidityRequest = userInfo[msg.sender].requestData[_requestId];
    ...
    onlyAuthorizedNode
  {
    ...
    if (toChainId == block.chainid) {
      ...
      (,,Transaction memory transaction) = abi.decode(_data, (uint256, uint256, Transaction));
  ...
  function tokenInReqTransferToChainAddLiq(
    ...
    external 
    payable
    permittedTokenInCheck(_token, this.tokenInReqTransferToChainAddLiq.selector)
    checkAmount(_amount)
    checkAddress(address(tokenInfo[_token].toChainToken[_toChainId]))
    nonReentrant
  {
  ...
  function tokenInBridgeToChain(
    ...
    external 
    payable
    permittedTokenInCheck(_token, this.tokenInBridgeToChain.selector)
    nonReentrant
  {
    _amount = realTonkenTransferAmount(_token,address(this),_amount);
  ...
  function tokenInBurnToChain(
    ...
    external 
    payable
    permittedTokenInCheck(_token, this.tokenInBurnToChain.selector)
    nonReentrant
  {
    _amount = realTonkenTransferAmount(_token,address(this),_amount);
  ...
  function tokenInBurnFromToChain(
    ...
    external 
    payable
    permittedTokenInCheck(_token, this.tokenInBurnFromToChain.selector)
    nonReentrant
  {
    IERC20mb(address(_token)).burnFrom(msg.sender,_amount);
    tokenInTransferToChain(_toChainId, _token, _amount);
  }
  function tokenInBurnToBurnAddrToChain(
    ...
    external 
    payable
    permittedTokenInCheck(_token, this.tokenInBurnToBurnAddrToChain.selector)
    nonReentrant
  {
    _amount = realTonkenTransferAmount(_token,BURN_ADDRESS,_amount);
    tokenInTransferToChain(_toChainId, _token, _amount);
  }
    ...
    returns (uint256 realAmount)
  {
    ...
    realAmount = _token.balanceOf(_receiver) - balanceBefore;
  }
  function tokenOutTransferToUser(
    ...
    nonReentrant
  {
    require(msg.sender == authorizedNode || msg.sender == _user, "Only node or user");
  ...
  modifier permittedTokenInCheck(IERC20 _token, bytes4 _functionId) {
    require(
      tokenInfo[_token].tokenInFunctionId == _functionId,
  ...
  modifier checkMaintenance() {
    require(!maintenance, "KiBridge: Under maintenance");
    _;
  }
