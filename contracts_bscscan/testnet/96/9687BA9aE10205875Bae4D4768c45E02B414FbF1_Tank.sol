...
contract Tank is Initializable, ERC20Upgradeable, OwnableUpgradeable {
  ...
  uint256 private _tTotal = 10000000 * 1000000 * 10**9;
  ...
  uint256 public tankDivisor = 5;

  uint256 public maxTxAmount;// = 3000000 * 10**6 * 10**9;
  ...
  function _transfer(
    ...
    if (!inSwapAndLiquify && recipient == uniswapV2Pair) {
      ...
      uint256 tankAmount = busdBalance.mul(tankDivisor).div(liquidityFee);
      IERC20Upgradeable(BUSD).transfer(tankPool, tankAmount);
      IERC20Upgradeable(BUSD).transfer(lotteryPool, busdBalance.sub(tankAmount));
    }
