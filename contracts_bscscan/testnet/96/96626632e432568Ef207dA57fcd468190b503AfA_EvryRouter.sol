...
contract EvryRouter is IEvryRouter2 {
    ...
    function _swapV2(uint[] memory amounts, address[] memory path, address _to) internal virtual {
          for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = EvryLibrary.sortTokens(input, output);
            uint amountOut = amounts[i + 1];
    ...
    function swapExactTokensForTokens(
        ...
        // - amount for transfer to pool need to decrese
        // - send fee amount to _swap() function
