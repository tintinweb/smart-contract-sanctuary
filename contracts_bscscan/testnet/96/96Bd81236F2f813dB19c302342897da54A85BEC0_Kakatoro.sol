...
contract Kakatoro is ERC20, Ownable {
    ...
    KakataroDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable REWARD = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
    ...
    uint256 public totalFees = dividendsFee.add(marketingFee).add(charityFee).add(liquidityFee);

    address payable public marketingWalletAddress = 0x0FCeADCE3e776FBA31B7B67685FFA84c5313aDB4;
    address payable public charityWalletAddress = 0x49b07da7527829d25bAbBc68C18D576A6956A1D8;
    ...
    constructor() public ERC20("Kakatoro", "KAKA") {

        dividendTracker = new KakataroDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Kakataro: The dividend tracker already has that address");

        KakataroDividendTracker newDividendTracker = KakataroDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Kakataro: The new dividend tracker must be owned by the Kakataro token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Kakataro: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Kakataro: Account is already the value of 'excluded'");
    ...
    function calculateTotalFees() private {
        totalFees = dividendsFee.add(marketingFee).add(charityFee).add(liquidityFee);
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Kakataro: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Kakataro: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 600000, "Kakataro: gasForProcessing must be between 200,000 and 600,000");
        require(newValue != gasForProcessing, "Kakataro: Cannot update gasForProcessing to same value");
    ...
    function swapTokens(uint256 tokens) private {
        ...
        // Fees
        uint256 totalFeesWithoutLiq = dividendsFee.add(marketingFee).add(charityFee);
        uint256 bnbBalance = address(this).balance;
        uint256 bnbForFees = bnbBalance.mul(marketingFee).div(totalFeesWithoutLiq);
        marketingWalletAddress.transfer(bnbForFees);
        bnbForFees = bnbBalance.mul(charityFee).div(totalFeesWithoutLiq);
        if (bnbForFees > 0) {
            charityWalletAddress.transfer(bnbForFees);
        }
    ...
    function swapTokensOnDemand(bool liquify, bool sendToFee, bool sendDividends) external onlyOwner {
        ...
        if (sendToFee) {
            ...
            charityTokens = contractTokenBalance.mul(charityFee).div(totalFees);
            tokensToSwap += feeTokens + charityTokens;
        }
        ...
        if (tokensToSwap > 0) {
            ...
            if (sendToFee) {
                ...
                bnbAmount = bnbReceived.mul(charityTokens).div(tokensToSwap);
                if (bnbAmount > 0) {
                    charityWalletAddress.transfer(bnbAmount);
                }
            }
...
contract KakataroDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("Kakataro_Dividen_Tracker", "Kakataro_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "Kakataro_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "Kakataro_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Kakataro contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Kakataro_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Kakataro_Dividend_Tracker: Cannot update claimWait to same value");
