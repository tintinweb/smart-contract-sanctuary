...
contract HumanPool is IPool, Ownable, IERC721Receiver, Pausable, ReentrancyGuard{
	...
	// human earn 10000 $ASH per day
	uint256 public constant DAILY_ASH_RATE = 10000 ether;
	// human must have 5 days worth of $ASH to unstake
	uint256 public constant MINIMUM_TO_EXIT = 5 days;
	...
	*/
	function sendToPool(address account, uint16[] calldata tokenIds) whenNotPaused external nonReentrant override{
		...
		for (uint i = 0; i < tokenIds.length; i++) {
			...
			require(character.getTokenTraits(tokenIds[i]).species == 1, "NOT HUMAN");
	...
	* to unstake a Human it will require it has 5 days worth of $ASH unclaimed
	...
	*/
	function _claimHumanFromPool(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
		...
		require(!(unstake && block.timestamp - stake.lastClaimTime < MINIMUM_TO_EXIT), "No enough ash to leave.");
