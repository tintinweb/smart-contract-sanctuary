...
contract PuzzleNetwork is Context{
	...
	function getClientStatus(address client) public view returns (bool) {
		return profile[client].clientCheck;
	}
	function amountReferrals(address referrer) public view returns(uint){
		return profile[referrer].referrals_list.length;
	}
	function getReferral(address referrer, uint number) public view returns(address){
		return profile[referrer].referrals_list[number];
	}
	...
	function registration(address client, address referrer) external onlyCtrlContract returns (bool) {
		...
		profile[referrer].referrals_list.push(client);
