...
contract TeaBondDepository is Ownable {
    ...
    address public immutable treasury; // mints TEA when receives principle
    ...
    struct Bond {
        uint payout; // TEA remaining to be paid
    ...
    function deposit(
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) TEA
         */
        ...
        if (fee != 0) {// fee is transferred to dao
            IERC20(TEA).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(address _recipient, bool _stake, uint _amount) internal returns (uint) {
        if (!_stake) {// if user does not want to stake
            IERC20(TEA).transfer(_recipient, _amount);
            // send payout
        } else {// if user wants to stake
            if (useHelper) {// use if staking warmup is 0
                IERC20(TEA).approve(stakingHelper, _amount);
    ...
    function maxPayout() public view returns (uint) {
        return IERC20(TEA).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function bondPriceDev() public view returns (uint price_) {
        price_ = terms.controlVariable.mul(debtRatio()).add(1000000000).div(1e7);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to TEA supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint debtRatio_) {
        uint supply = IERC20(TEA).totalSupply();
    ...
    /**
     *  @notice calculate amount of TEA available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or TEA) to the DAO
     *  @return bool
     */
