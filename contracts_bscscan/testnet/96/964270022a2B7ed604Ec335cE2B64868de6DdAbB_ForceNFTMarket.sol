...
contract ForceNFTMarket is Ownable, IERC721Receiver, Initializable, Pausable {
    ...
    function setStartTime(uint startTime_) onlyOwner whenPaused public {
    ...
    function initialize(IERC20 currToken_, uint nftPrice_, address tokenReceiveAddress_,
        uint[] memory supportedBoxSizes_, uint[] memory nftsGrades_) onlyOwner public initializer {
    ...
    function pullNFTs(address tokenAddress, address receivedAddress) onlyOwner public {
    ...
    function shuffle() onlyOwner external {
        for (uint i = 0; i < nftsGrades.length; i++) {
            ...
            for (uint j = 0; j < nftIds.length; j++) {
                uint n = j + uint(keccak256(abi.encodePacked(now))) % (nftIds.length - j);
                uint temp = nftIds[n];
                nftIds[n] = nftIds[j];
                nftIds[j] = temp;
            }
