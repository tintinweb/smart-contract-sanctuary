...
/** 
	...
	4% of every trade is burned
	1% of every trade is auto-locked in liquidity
	2% of every trade goes to Devmar(developer & marketing)
	Burning on every trade makes NEWROZ TRULY HYPER-Deflationary - to a final total supply of 50,000,000,000 NEWR0Z
*/
...
contract NEWROZ is Context, IBEP20, Ownable {
    ...
    address private _devmarWalletAddress = 0x9cCe923fa09f23B850ea98FbCf35d77d9396fFfB;
    ...
    string private _name = "Newroz Coin";
    string private _symbol = "NEWROZ";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 3;
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _devmarFee = 2;
    uint256 private _previousDevmarFee = _devmarFee;
    ...
    uint256 public _maxTxAmount = 100000000000000000000000000 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 1000000 * 10**9;
	uint256 public numTokensBurn = 999999000000000 * 10**9;
    ...
    function setDevmarFeePercent(uint256 devmarFee) external onlyOwner() {
        _devmarFee = devmarFee;
    }
	...
	function setDevmarWalletAddress(address devmarWalletAddress) external onlyOwner() {
         _devmarWalletAddress = devmarWalletAddress;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tDevmar = calculateDevmarFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tDevmar);
        return (tTransferAmount, tFee, tLiquidity, tDevmar);
    }
    ...
    function _takeDevmar(uint256 tDevmar) private {
        uint256 currentRate =  _getRate();
        uint256 rDevmar = tDevmar.mul(currentRate);
        _rOwned[_devmarWalletAddress] = _rOwned[_devmarWalletAddress].add(rDevmar);
        if(_isExcluded[_devmarWalletAddress])
            _tOwned[_devmarWalletAddress] = _tOwned[_devmarWalletAddress].add(tDevmar);
    }
    ...
    function calculateDevmarFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_devmarFee).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _devmarFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousDevmarFee = _devmarFee;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tDevmar) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
