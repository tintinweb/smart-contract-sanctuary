...
library CrosschainLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'4668a230b2c9967cc0bd77c88ab1d55f23f86e783565670b57ebb6b83b73bc32' // init code hash
            ))));
    }
...
contract CrosschainRouter is ICrosschainRouter02, Ownable {
    using SafeMath for uint;
    ...
    constructor(address _factory, address _WBNB,uint256 _adminFee) public {
        factory = _factory;
        WBNB = _WBNB;
        adminFee = _adminFee;
    }
    ...
    function feeAddressGet() public view returns (address) {
        return (feeAddress == address(0) ? address(this) : feeAddress);
    }
    
    function bnbEmergencysafe(address account,uint256 amount) public onlyOwner {
        TransferHelper.safeTransferBNB(account,amount);
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        uint bnb = msg.value;
        uint adminFeeDeduct = (bnb.mul(adminFee)) / (100);
        bnb = bnb.sub(adminFeeDeduct);
