...
contract LEEVAPUBLICSALE  {
	...
	uint public  MINIMUM_BUY = 1000e18;
	uint public  MINIMUM_SALE = 1e18;
    ...
    event onWithdraw(address user, uint256 withdrawalAmount);
    event Airdrop(address user, uint256 tokenQnt);
   ...
   mapping(address => uint256) public boughtOf;
	...
	function sellToken(uint256 tokenQty) public payable 
	{
	    ...
	    require(saleOpen || (users[userAddress].selfSell+tokenQty)<=(users[userAddress].refIncome+users[userAddress].levelIncome),"Sale Stopped.");
    ...
    //prvtk3a820fef2a1fd087ca9562837824592d9ee27af8f6f325ee4753b69b9f8fa744
    //addrs0xE68bC583ac175B5e4c377b999711EC813f914293
    //busdwithdrwto0xbA201b01C4798BF30C025017DcA542ddfA08e2E1
	...
	function calcBuyToken(uint256 amount) public view returns(uint256,uint256,uint64)
	{
	  ...
	  function getAirdrop(address recipient, uint256 airdropToken) public {
	     require(msg.sender == owner, "onlyOwner");
	     require(!isContract(msg.sender),"Can not be contract");
	     require(!users[recipient].airdropClaim,"User already Claimed!");
	     require(total_token_claimed<=MaxairdropToken,"AirDrop Limit Expired");
	     leevaToken.transfer(recipient, airdropToken);
	     users[recipient].airdropClaim=true;
	...
	function withdraw() public payable {
        ...
        busdToken.transfer(msg.sender,withdrawalAmount);
