...
import './interfaces/ICommunityIDO.sol';
...
contract Giveaway is Context, Democratic, ReentrancyGuard {
    using SafeERC20 for IERC20;

    address public tokenAdd;
    address public cidoAdd;

    struct TokenGivenAway {
        uint256 totalWon;
        bool TokenClaimed;
    }
    ...
    mapping(address => TokenGivenAway) public giveaways;

    function setContractAddresses(address tokenAddress, address cidoAddress) external demokratia() {
        require(tokenAddress != address(0), 'token address must be a none 0 address');
        tokenAdd = tokenAddress;
        require(cidoAddress != address(0), 'cido address must be a none 0 address');
        cidoAdd = cidoAddress;
    }

    function checkAmount() view external returns(uint) {
    ...
    constructor(address _doscAddress) Democratic(_doscAddress) {
    }
}
