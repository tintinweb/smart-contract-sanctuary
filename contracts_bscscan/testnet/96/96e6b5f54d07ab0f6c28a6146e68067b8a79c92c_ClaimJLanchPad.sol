...
contract ClaimJLanchPad is DataStorage, Access, Events, Manageable {
    ...
    function setContractEnabled(bool _contractEnabled) external onlyAdmins {
    ...
    function addTokenClaimByAdmin(
        ...
        BEP20Token storage bep20Token = bep20Tokens[_token];
        bool isStopped = true;
    ...
    function stopCampaignByAdmin(uint256 campaignId, address _token) external onlyAdmins {
        Campaign storage campaign = campaigns[campaignId];       
        Campaign[] storage currentCampaigns = bep20Tokens[_token].campaigns;
        for (uint256 index = 0; index < currentCampaigns.length; index++) {
            if(currentCampaigns[index].id == campaignId) {
