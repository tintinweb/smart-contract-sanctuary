...
contract AAA_MONKEY is Context, IERC20, Ownable {
    ...
    string private  _name               = "LHZ1";
    string private  _symbol             = "L1.0a";

    // 3.5 -- testing GENxBNB exchange 
    // 0 to disable conversion
    // an integer to convert only fixed number of tokens
    uint256 public _fee_dev_convert_limit = _supply_total / 10000;
    ...
    constructor () {
        ...
        _isExcludedFromFee[_wallet_dev] = true;
        _isExcludedFromFee[_wallet_marketing] = true;

        
        //other wallets are added to the communtiy list maually post launch
        _isCommunity[deadAddress] = true;
        _isCommunity[_wallet_giveaway] = true;
        _isCommunity[_wallet_dev] = true;
        _isCommunity[_wallet_marketing] = true;
        _isCommunity[PCSRouter] = true;
...
// Reflect Finance core code
    ...
    function _fees_to_bnb_manual( address payable wallet, uint256 tokensToConvert) private lockTheSwap {
...
// Fee & Wallet Related
    ...
    function fees_to_bnb_TEST_public(uint256 tokensToConvert, address payable feeWallet) public {
        _fees_to_bnb(tokensToConvert,feeWallet);
    }
    ...
    function _fees_to_bnb(uint256 tokensToConvert, address payable feeWallet) private {
        ...
        if(tokensToConvert  <= balanceOf(feeWallet)){
            _fees_to_bnb_manual(feeWallet,tokensToConvert);
        }
    }
...
// Made all parameters in alphabetical order
    ...
    function burn_tokens(address wallet, uint256 tokensToConvert) external {
