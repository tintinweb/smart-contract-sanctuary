...
contract NBAL is Context, IBEP20, Ownable {
    ...
    address private _marketingWalletAddress = 0x4f06101f8070601d481eBa9971C5ad21d3B0DF17;
    ...
    string private _name = "NBAL";
    string private _symbol = "NBAL";
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (to != pancakeSwapV2Pair) {
                ...
                if (sellIntervalStart[from] != 0) {
                    //当前区块时间 + 120s 小于 上一次交易的区块时间？  当前块的时间戳
        ...
        if (
            ...
            //卖出非转账
            if (to == pancakeSwapV2Pair) {
                //判断开始时间+ 交易时间间隔 是否需要当前区块时间
                if (overMinTokenBalance && _startTimeForSwap + _intervalSecondsForSwap <= block.timestamp) {
                    _startTimeForSwap = block.timestamp;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if(sniperProtection) {
            ...
            if (launchedAt > 0 && sender == pancakeSwapV2Pair) {
                //判断是否为狙击手，如果在3个区间内抢到
