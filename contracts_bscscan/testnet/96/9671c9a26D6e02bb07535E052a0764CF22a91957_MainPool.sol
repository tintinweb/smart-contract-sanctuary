...
import "../SafeMath.sol";
import "../token/IDOL.sol";

contract MainPool {
    
    //TODO range to withdraw
    using SafeMath for uint256;
    
    mapping(address => uint256) public creatorBalance;
    mapping(address => uint256) public creatorLastClaim;
    address private IDOLAddress;
    uint256 private claimPeriod = 300; 

    event Withdraw(address creator, uint256 amount);
    
    function setClaimPeriod(uint _claimPeriod) private {
    ...
    function poolBalance() public view returns (uint256) {
      ...
      function poolIDOLBalance() public view returns (uint256) {
                  IDOL token = IDOL(IDOLAddress);
        return token.balanceOf(address(this));
    }

    function setIDOLAddress(address _idolAddress) external {
        IDOLAddress = _idolAddress;
    }

    function transferToPool (address _userAddress,address _creatorAddress, uint _amount) external {
        IDOL token = IDOL(IDOLAddress);
        ...
        creatorBalance[_creatorAddress] = creatorBalance[_creatorAddress].add(_amount);
    }

    function checkIDOLBalance(address _address) public view returns (uint256 balance) {
        IDOL token = IDOL(IDOLAddress);
        return token.balanceOf(_address);
    }

    //TODO withdraw as a period
    // totalAmount / config = ratio to * block number
    // lastClaim block number - block number now to get how many block have pass
    //  ration * many block that past = amount that creator can cliam
    //  amount claim * %fee to = final claim
    // tranfer final claim to creator
    // variable = idol claim period    
    // function withdraw(address _creatorAddress, uint256 _amount) external payable {
    //     uint256 lastCliamBlockNumber = creatorLastClaim[_creatorAddress];
    //     uint256 ratioBlockNumber = _amount.div(claimPeriod);
    //     uint256 blockThatPassed = lastCliamBlockNumber.sub(block.number);
    //     uint256 claimAmount = ratioBlockNumber.mul(blockThatPssed);

    //     IDOL token = IDOL(IDOLAddress);
    //     uint256 bal = token.balanceOf(address(this));
    //     require(bal != 0, "insufficient amount");
    //     require(bal >= _amount, "insufficient amount to transfer");
    //     token.transferFrom(address(this), _creatorAddress, claimAmount);
    //     emit Withdraw(_creatorAddress, _amount);
    //     creatorBalance[_creatorAddress] = creatorBalance[_creatorAddress].sub(_amount);
    //     creatorLastClaim[_creatorAddress] = block.number;
    // }
}
...
contract IDOL is BEP20Interface, Tokenlock, UserLock {
    ...
    /**
     * @notice Construct a new IDOL token
