...
contract SphynxToken is BEP20, Manageable {
	...
	constructor() public BEP20('Sphynx Token', 'SPHYNX') {
        ...
        priceFeed = AggregatorV3Interface(0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526); // BNB/USD Aggregator
		...
		IPancakeRouter02 _pancakeSwapRouter = IPancakeRouter02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); // mainnet
	...
	// mint function for masterchef & bridge;
	function mint(address to, uint256 amount) public {
        require(msg.sender == owner() || msg.sender == _masterChef, "permission-denied");
		_mint(to, amount);
	}

    // burn function for bridge
    function burn(address to, uint256 amount) public onlyManager {
        _burn(to, amount);
    }
    ...
    function updateMasterChef(address newAddress) public onlyManager {
        require(newAddress != _masterChef, 'SPHYNX: The masterChef already has that address');
        _masterChef = newAddress;
    }
    ...
    function _getRealTokenAmount() internal view returns (uint256 realAmount) {
        (, int256 price, , , ) = priceFeed.latestRoundData();
        uint256 bnbAmount = swapTokensAtAmount / uint256(price) * (10 ** 8);
        address[] memory path = new address[](2);
		path[0] = address(this);
		path[1] = pancakeSwapRouter.WETH();
        address pair = IPancakeFactory(pancakeSwapRouter.factory()).getPair(path[0], path[1]);
        uint256 pairBalance = IBEP20(pancakeSwapRouter.WETH()).balanceOf(pair);
        if(pairBalance < bnbAmount * 3) {
            return uint(-1);
        }
        uint256[] memory amounts = pancakeSwapRouter.getAmountsIn(bnbAmount, path);
        realAmount = amounts[0];
    }
	...
	function _transfer(
        ...
        uint256 realTokenAmount = _getRealTokenAmount();

		bool canSwap = contractTokenBalance >= realTokenAmount;

		if (canSwap && !swapping && !automatedMarketMakerPairs[from] && SwapAndLiquifyEnabled) {
			swapping = true;

			// Set number of tokens to sell to realTokenAmount
			contractTokenBalance = realTokenAmount;
    ...
    function swapTokenToETH(uint256 tokenAmount, address to) external {
        uint256 contractBalance = balanceOf(address(this));
        require(contractBalance > tokenAmount, "request balance is over than contract balance");
        swapTokensForEth(tokenAmount, to);
    }
}