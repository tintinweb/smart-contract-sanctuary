...
contract AdvisorVesting is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
	...
	/* the cliff time of the advisor vesting */
    uint256 private cliffTime;

	/* the duration of the advisor vesting */
    uint256 private duration;

    /* the address of the token contract */
    IERC20Upgradeable private tokenReward;

	/* the balances (in INERY) of all advisors */
	mapping(address => uint256) public balanceOfINERY;

	/* the whitelist of the advisors */
    ...
    function initialize(
        ...
        tokenReward = _token;
        tgeTime = _tge;
		cliffTime = tgeTime.add(60 * 60 * 24 * 30 * 9);	// 9 months cliff
	...
	function _vestedAmount(address addr) private view returns (uint256) {
		...
		if (block.timestamp < cliffTime) {
			return 0;
		// after 1 year cliff + duration
		} else if (block.timestamp > cliffTime.add(duration)) {
			return balanceOfINERY[addr];
		// linear part during 9 months cliff + 12 months
		} else {
