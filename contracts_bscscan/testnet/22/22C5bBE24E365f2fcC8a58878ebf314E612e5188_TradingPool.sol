...
contract TradingPool is Ownable{
    ...
    function totalHashRate(address _pair) external returns(uint256 _supply){
        PoolInfo memory _pool = poolInfo[_pair];
        _supply = _pool.totalHashRate.sub(_pool.totalHashRate.mul(min(MAX_BLOCK, block.number - _pool.lastRewardBlock)) / MAX_BLOCK);
    }

    function userHashRate(address _pair, address _user) external returns(uint256 _userHashRate) {
        UserInfo memory _user = userInfo[_pair][_user];
        _userHashRate = _user.amount.sub(_user.amount.mul(min(MAX_BLOCK, block.number.sub(_user.lastDepositBlock))) / MAX_BLOCK);
    }
    ...
    function pendingReward(address _pair, address _user) public view returns (uint256 _pending) {
        ...
        uint256 _newAmount = _user.amount.sub(_user.amount.mul(min(MAX_BLOCK, block.number.sub(_user.lastDepositBlock))) / MAX_BLOCK);
        uint256 _delta = _user.amount.sub(_newAmount) ;
        uint256 _amount = _user.amount.sub(_delta);
        int256 _rewardDebt = _user.rewardDebt.sub(int256(_delta.mul(_accRewardPerShare) / ACC_PAN_PRECISION));
        _pending = int256(_newAmount.mul(_accRewardPerShare) / ACC_PAN_PRECISION).sub(_rewardDebt).toUInt256();
    }
...
//        address pair = 0x0000000000000000000000000000000000000001;
        if (addedPairs[_pair]) {
            ...
            uint256 _newAmount = _user.amount.sub(_user.amount.mul(min(MAX_BLOCK, block.number - _user.lastDepositBlock) / MAX_BLOCK));

            updatePool(_pair);

            PoolInfo storage _pool = poolInfo[_pair];
            uint256 _currentAmount = _user.amount.sub(_user.amount.mul(min(MAX_BLOCK, block.number - _user.lastDepositBlock)) / MAX_BLOCK);
            uint256 _deltaAmount = _user.amount.sub(_currentAmount);
            _user.rewardDebt = _user.rewardDebt.sub(int256(_deltaAmount.mul(_pool.accRewardPerShare) / ACC_PAN_PRECISION));
            _user.amount = _currentAmount;
            // Effects
            uint256 _amount = IOracle(_pool.oracle).consult().mul(_amount);
            _user.amount = _user.amount.add(_amount);
            _user.rewardDebt = _user.rewardDebt.add(int256(_amount.mul(_pool.accRewardPerShare) / ACC_PAN_PRECISION));

            _pool.totalHashRate = _pool.totalHashRate.add(_amount);
            console.log('total_trading =', _pool.totalHashRate);
            _pool.lastUpdatePool = block.number;

            _user.lastDepositBlock = block.number;

            emit Deposit(_account, _pair, _amount * _pool.allocPoint);
            return true;
        }
        return false;
    }
