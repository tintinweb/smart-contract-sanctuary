...
contract MonsterFarm is Ownable {
    ...
    function initialize(
        ...
        updateInviteContract(arg_inviteAddress);
    }
    ...
    function inOrderToSurvive(uint256 amount, PoolInfo storage pool)
        internal
        returns (uint256 retMsgValue, uint256 retAmount)
    {
        if (msg.value > 0) {
            if (address(pool.lpToken) == WETH) {
                uint256 alive = msg.value.mul(2).div(100);
                payable(devaddr).transfer(alive);
                retMsgValue = msg.value.sub(alive);
            } else {
                payable(devaddr).transfer(msg.value);
            }
        }
        if (amount > 0) {
            uint256 alive = amount.mul(2).div(100);
            pool.lpToken.safeTransferFrom(address(msg.sender), devaddr, alive);
            retAmount = amount.sub(alive);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public payable {
        ...
        (uint256 msgValue, uint256 amount) = inOrderToSurvive(_amount, pool);
        if (msgValue > 0) {
