...
contract SellTokenBNB is ReentrancyGuard {
  ...
  uint256 public minimumBuyAmount = 100000000000000000; //0.01 BNB
   ...
   * @param recipients recipients of the transfer
   */
  function sendFTLNFT(address[] calldata recipients, uint256[] calldata idTokens) public onlyIDOAdmin {
    for (uint256 i = 0; i < recipients.length; i++) {
  ...
  function buyTest(address _referrer) public ifNotPaused  payable {
        uint256 weiValue = msg.value;
        require((weiValue >0), "Minimum amount is 0.05 Eth");
        ...
        uint256 amount = weiValue;
        IERC20 token = IERC20(IDO_TOKEN);
