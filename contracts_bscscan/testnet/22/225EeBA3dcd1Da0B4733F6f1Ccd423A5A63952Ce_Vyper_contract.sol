...
def exchange_underlying(i: int128, j: int128, dx: uint256, min_dy: uint256):
    ...
    x:uint256 = dx
    if i==0:
        x = as_unitless_number(msg.value)
    ...
    dy_buyback_fee: uint256 = dy_fee * self.buyback_fee / FEE_DENOMINATOR
    dy_company_fee : uint256 = dy_fee * self.company_fee / FEE_DENOMINATOR
    dy_lp :uint256 = dy_fee-(dy_buyback_fee+dy_company_fee)
    
    if j==0:
        IWETH(self.underlying_coins[0]).withdraw(dy)
        log.AmountTransfer(dy-dy_fee,dy_buyback_fee,dy_company_fee,dy_fee)
        send(msg.sender,self.balance-as_unitless_number(dy_fee))
        send(self.buyback_addr,dy_buyback_fee)
        send(self.company_addr,dy_company_fee)

        # lp wallet
        send(self.owner,dy_lp)  
