...
contract UNWSwapAgentImpl is Context {
    ...
    function registerSwapPairToBSC(address erc20Addr) external returns (bool) {
        ...
        if (erc20Addr == address(0)) {
            ...
            name = IERC20Query(erc20Addr).name();
            symbol = IERC20Query(erc20Addr).symbol();
            decimals = IERC20Query(erc20Addr).decimals();
    ...
    function fillBSC2ETHSwap(bytes32 bscTxHash, address erc20Addr, address payable toAddress, uint256 amount) onlyOwner external returns (bool) {
    ...
    function swapETH2BSC(address erc20Addr, uint256 amount) payable external notContract returns (bool) {
        ...
        emit SwapStarted(erc20Addr, msg.sender, amount, swapFee);
        return true;
    }
