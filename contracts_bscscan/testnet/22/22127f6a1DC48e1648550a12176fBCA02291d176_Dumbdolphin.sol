...
contract Dumbdolphin is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 100000 *1e9 * 1e9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "DumbDolphinCoin";
    ...
    uint256 public _taxFee = 40; // 4% will be distributed among holder as token divideneds
    ...
    uint256 public _burnFee = 30;  // 3% will go to the Dead address to deflate token
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_taxFee.add(_liquidityFee).add(_burnFee)).div(1e3);
        return percentage;
    }
    ...
    function _transfer(
        ...
        require(from != address(0), "DDC: transfer from the zero address");
        require(to != address(0), "DDC: transfer to the zero address");
        require(amount > 0, "DDC: Transfer amount must be greater than zero");
        require(balanceOf(from) >= amount, "DDC: Insufficient user balance");
        require(!_isSniper[to], "DDC: Can not send to bot");
        require(!_isSniper[from], "DDC: Bots can not send");

        if (
            _isExcludedFromMaxTx[from] == false &&
            _isExcludedFromMaxTx[to] == false // by default false
        ) {
            require(amount <= _maxTxAmount, "DDC: amount exceeded max limit");

            if (!_tradingOpen) {
                require(
                    from != pancakePair && to != pancakePair,
                    "DDC: Trading is not enabled"
                );
            }
