...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "VokenDeploy3";
    string internal constant SYMBOL = "VKND3";
...
abstract contract BaseRfiToken is IERC20, IERC20Metadata, Ownable, Presaleable, Tokenomics {
     ...
     */
    function _afterTokenTransfer(address from, address to, uint256 amount, uint256 tTransferAmount) internal virtual {}
...
abstract contract ERC20Permit is BaseRfiToken, IERC20Permit, EIP712 {
...
abstract contract ERC20Votes is ERC20Permit {
    ...
    function _afterTokenTransfer(address from, address to, uint256 amount, uint256 tTransferAmount) internal virtual override {
        super._afterTokenTransfer(from, to, amount, tTransferAmount);

        _moveVotingPower(delegates(from), delegates(to), amount, tTransferAmount);
    }
...
// File: VokenTokenV3.sol

contract VokenTokenV3 is VokenToken, ERC20Permit, ERC20Votes {
    constructor() VokenToken(Env.Testnet) ERC20Permit() { //TODO: change this to mainnetv2
        _approve(owner(), address(_router), ~uint256(0));
    }

    function _afterTokenTransfer(address from, address to, uint256 amount, uint256 tTransferAmount) internal override(BaseRfiToken, ERC20Votes) {
        super._afterTokenTransfer(from, to, amount, tTransferAmount);
    }
}