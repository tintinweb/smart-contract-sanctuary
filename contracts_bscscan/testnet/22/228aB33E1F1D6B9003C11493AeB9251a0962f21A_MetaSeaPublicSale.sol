...
contract MetaSeaPublicSale is AccessControlEnumerable {
    ...
    modifier batchExisted(uint256 batchNumber_) {
        require(
            ...
            "MetaSeaPublicSale: batchNumber_ does not exist"
        );
        _;
    }
    ...
    function _updateBatchSaleInfo(
        ...
        _info.tokenForSale = tokenForSale_;
    ...
    function addBatchSale(
        ...
        require(batchNumber_ > 0, "MetaSeaPublicSale: batchNumber_ is 0");
        require(
            _batches.add(batchNumber_),
            "MetaSeaPublicSale: batchNumber_ already existed"
        );
        require(
            recipient_ != address(0),
            "MetaSeaPublicSale: recipient_ is zero address"
        );
        require(
            tokenForSale_ != address(0),
            "MetaSeaPublicSale: tokenForSale_ is zero address"
        );
        require(
            paymentToken_ != address(0),
            "MetaSeaPublicSale: paymentToken_ is zero address"
        );
        require(price_ > 0, "MetaSeaPublicSale: price_ is 0");
        ...
        batchExisted(batchNumber_)
    {
        require(
            ...
            "MetaSeaPublicSale: status is same as before"
        );
        batchStatus[batchNumber_] = BatchStatus(status_);
    ...
    function addWhitelistAddressToBatch(
        ...
        require(
            ...
            "MetaSeaPublicSale: address is already in whitelist"
        );
    }
    ...
    function addWhitelistAddressesToBatch(
        ...
        require(
            ...
            "MetaSeaPublicSale: whitelistAddresses_ is empty"
        );
        for (
            uint256 _index = 0;
    ...
    function removeWhitelistAddressOutBatch(
        ...
        require(
            ...
            "MetaSeaPublicSale: address is not in whitelist"
        );
    }
    ...
    function removeWhitelistAddressesOutBatch(
        ...
        for (
            ...
            require(
                ...
                "MetaSeaPublicSale: address is not in whitelist or already removed"
            );
        ...
        batchExisted(batchNumber_)
    {
        if (_whitelistAddresses[batchNumber_].length() > 0) {
            require(
                ...
                "MetaSeaPublicSale: sender is not in whitelist"
            );
        }
        require(
            batchStatus[batchNumber_] == BatchStatus.ACTIVE,
            "MetaSeaPublicSale: the sale is inactive"
        );
        require(
            block.timestamp >= batchSaleInfos[batchNumber_].start,
            "MetaSeaPublicSale: the sale does not start"
        );
        require(
            block.timestamp < batchSaleInfos[batchNumber_].end ||
                batchSaleInfos[batchNumber_].end == 0,
            "MetaSeaPublicSale: the sale is ended"
        );

        IERC20WithMetadata _tokenForSale = IERC20WithMetadata(
            batchSaleInfos[batchNumber_].tokenForSale
        );
        uint256 _receivedAmount = (paymentAmount_ *
            10**_tokenForSale.decimals()) / batchSaleInfos[batchNumber_].price;
        require(
            _receivedAmount >= batchSaleInfos[batchNumber_].softCap,
            "MetaSeaPublicSale: amount is too low"
        );
        require(
            _receivedAmount <= _tokenForSale.balanceOf(address(this)) &&
                (batchSaleInfos[batchNumber_].hardCap == 0 ||
                    _receivedAmount <=
                    batchSaleInfos[batchNumber_].hardCap -
                        totalSoldAmount[batchNumber_]),
            "MetaSeaPublicSale: amount is too high"
        );
        ...
        soldAmount[batchNumber_][_msgSender()] += _receivedAmount;
        totalSoldAmount[batchNumber_] += _receivedAmount;
        ...
        _tokenForSale.safeTransfer(
            _msgSender(),
            _receivedAmount
        );

        emit TokensPurchased(_msgSender(), paymentAmount_, _receivedAmount);
    }
}