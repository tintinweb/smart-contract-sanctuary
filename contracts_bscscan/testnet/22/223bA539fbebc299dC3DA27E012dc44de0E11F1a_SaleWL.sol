...
contract SaleWL {
    ...
    IERC20 constant public token = IERC20(0xdf23957d7510E03f7d6547DD4926968fAfF51CDE);// Token MemeKiller
    ...
    mapping(address => bool) private _userAddr;
    mapping(address => uint256) balances;
    mapping(address => uint256) invested;

    function invest(uint256 _amount) public {
        require(block.timestamp >= _START_TIME,"Expect the start");
        require(block.timestamp <= _START_TIME.add(_SALE_DAYS),"End Sale");
        require(_status == true,"End Sale");
        require(_userAddr[msg.sender] == true,"The address is not in the whitelist");
        require(getLeftToken() >= _minBUSD.mul(_RATE),"End Sale");
    ...
    function getToken() public {
        require(block.timestamp >= _START_TIME.add(_SALE_DAYS) || getLeftToken() < _minBUSD.mul(_RATE) || _status == false,"Expect the end of the sale");
    ...
    function get_totalSold() public view returns (uint256) {
        return _totalSold;
    }
    ...
    function insertListAddresses(address[] memory _listAddresses) public returns (bool) {
        require(msg.sender == _OWNER);
        for(uint i = 0; i < _listAddresses.length; i++) {
            address addr = _listAddresses[i];
            if(_userAddr[addr] == false){
    ...
    function insertAddress(address _address) public returns (bool) {
        require(msg.sender == _OWNER);
        if(_userAddr[_address] == false){
