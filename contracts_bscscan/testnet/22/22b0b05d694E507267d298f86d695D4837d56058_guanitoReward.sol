...
contract DividendDistributor is IDividendDistributor {
    ...
    function changeToken(address newToken, bool forceChange) external override onlyToken {
        ...
        tokenConversionDivisor = rewardToken.balanceOf(address(this));
        ...
        if (tokenConversionDivisor > 0) {
            rewardToken.approve(address(router), tokenConversionDivisor);
        ...
        if (totalDividends > 0) {
            tokenConversionNumerator = rewardToken.balanceOf(address(this));
...
contract guanitoReward is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "guanitoReward";
    string constant _symbol = "guanitoReward";
    ...
    function getSharesAmount(address _shareholder) public view returns (uint){
        return distributor.getSharesAmount(_shareholder);
    }
    
    function getSharesHolders() public view returns (address [] memory){
        return distributor.getSharesHolders();
    }
    
    function getCumulativeDividends(address _shareholder) public view returns (uint256) {
        return distributor.getCumulativeDividends(distributor.getSharesAmount(_shareholder));
    }
    
    function getSharesExcluded(address _shareholder) public view returns (uint) {
        return distributor.getSharesExcluded(_shareholder);
    }
    
    function getSharesRealised(address _shareholder) public view returns (uint) {
        return distributor.getSharesRealised(_shareholder);
    }
    
    function setRewardToken(address newToken, bool forceChange) external onlyOwner {
        require(newToken.isContract(), "Enter valid contract address");
        distributor.changeToken(newToken, forceChange);
    }
    
    function getRewardToken() external view returns (address) {
        return distributor.getRewardToken();
    }
