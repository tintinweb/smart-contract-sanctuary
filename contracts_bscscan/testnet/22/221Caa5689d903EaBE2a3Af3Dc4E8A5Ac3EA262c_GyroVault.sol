...
contract GyroVault is Ownable {
    ...
    function stake(uint256 amount_, address recipient_) external returns (bool) {
        ...
        IERC20(sGyro).safeTransfer(warmupContract, amount_);
        return true;
    }

    /**
        @notice retrieve sGyro from warmup
        @param recipient_ address
     */
    function claim(address recipient_) public {
        Claim memory info = warmupInfo[recipient_];
        if (epoch.number >= info.expiry && info.expiry != 0) {
            delete warmupInfo[recipient_];
            IWarmup(warmupContract).retrieve(recipient_, ISGyro(sGyro).balanceForGons(info.gons));
        }
    }

    /**
        @notice forfeit sGyro in warmup and retrieve gyro
     */
    function forfeit() external {
        Claim memory info = warmupInfo[msg.sender];
        delete warmupInfo[msg.sender];

        IWarmup(warmupContract).retrieve(address(this), ISGyro(sGyro).balanceForGons(info.gons));
    ...
    function setContract(CONTRACTS contract_, address address_) external onlyOwner() {
        ...
        } else if (contract_ == CONTRACTS.WARMUP) {
            // 1
            require(warmupContract == address(0), "Warmup cannot be set more than once");
            warmupContract = address_;
     ...
     * @param warmupPeriod_ uint
     */
    function setWarmup(uint256 warmupPeriod_) external onlyOwner() {
