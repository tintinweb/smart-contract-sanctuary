...
contract DDA is IERC20,Ownable{
    ...
    event TransferPledge(address indexed from, uint256 usdtAmount, uint256 value,uint256 rate);
    ...
    event Destory(address indexed account,uint256 ddaAmount,uint256 amount);
    ...
    IERC20 USDT = IERC20(0x47D2ee530d2466fc5D4f6914ac73cF0496C61B0F);
    ...
    function pledge(uint256 amount,uint256 rate) public{
        ...
        uint256 ddaRate=uint256(100).sub(rate);
        //dda usdt amount
        uint256 ddaUsdAmount = amount.mul(ddaRate).div(rate);
        ...
        require(USDT.transferFrom(msg.sender,_master,amount),"USDT transfer error");
        _balances[msg.sender] = _balances[msg.sender].sub(ddaAmount);
        emit TransferPledge(msg.sender,amount,ddaAmount,rate);
    }
    ...
    function destory(uint256 amount) public {
        uint256 ddaAmount = amount
            ...
            .div(_price);
        _balances[msg.sender] =  _balances[msg.sender].sub(ddaAmount);
        _balances[_master]=_balances[_master].add(ddaAmount);
        emit Destory(msg.sender,ddaAmount,amount);
    }
