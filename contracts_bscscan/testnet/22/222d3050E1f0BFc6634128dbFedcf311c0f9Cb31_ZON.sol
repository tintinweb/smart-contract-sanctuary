...
contract ZON is Context, IERC20, Ownable {
    ...
    string private _name = "ZONGIAVERSE";
    string private _symbol = "ZON";
    ...
    address public buybackWallet = 0xca9023B5A7f3b18B57e4A9C15Ab2C2a59BB2C9c3;

    address public treasuryWallet = 0xca9023B5A7f3b18B57e4A9C15Ab2C2a59BB2C9c3;
    ...
    uint256 private numTokensSellToAddToLiquidity = 10000000 * 10**18; // 0.1%
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _treasuryFee == 0 && _burnFee == 0) return;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(sender != owner() && recipient != owner() && sender!= address(this))
        {
            ...
            buybackAmount = calculateBuybackFee(amount);
        }


        
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount.sub(burnAmount).sub(buybackAmount).sub(marketingAmount));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount.sub(burnAmount).sub(buybackAmount).sub(marketingAmount));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, amount.sub(burnAmount).sub(buybackAmount).sub(marketingAmount));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount.sub(burnAmount).sub(buybackAmount).sub(marketingAmount));
        } else {
            _transferStandard(sender, recipient, amount.sub(burnAmount).sub(buybackAmount).sub(marketingAmount));
        }
        ...
        if(marketingAmount > 0) {
            _transferStandard(sender, treasuryWallet, marketingAmount);
        }

        if(buybackAmount > 0){
            _transferStandard(sender, buybackWallet, buybackAmount);
        }
