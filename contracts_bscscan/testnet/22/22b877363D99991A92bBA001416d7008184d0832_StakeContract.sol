...
contract StakeContract {
    ...
    struct UserInfo {
        ...
        uint256 pastReward;       // Rewards claimed so far...
    ...
    function deposit(uint256 _amount) public {
        ...
        if (user.amount == 0) {
            ...
            // still lock time...
            if (curBlock <= (startBlock + poolPeriod)) {
                if (curBlock <= (user.firstStakedBlock + lockTokenPeriod)) {
                    ...
                    user.pastReward = user.rewardDebt;
                    user.rewardDebt = 0;
                    user.lastRewardBlock = curBlock;
                }
            }
    ...
    function withdraw(uint256 _amount) public {
        ...
        if (curBlock > (user.firstStakedBlock + lockTokenPeriod)) {
            ...
            uint256 interval;
            // if pool end, stop APY.
            if (curBlock > (startBlock + poolPeriod)) {
                interval = (startBlock + poolPeriod) - user.lastRewardBlock;
            } else {
                interval = curBlock - user.lastRewardBlock;
            }
    ...
    function calcCurrentReward(address _addr) public view returns(uint256) {
        ...
        uint256 interval = curBlock - user.lastRewardBlock;
        uint256 reward = user.amount.mul(APY).mul(interval).div(100).div(yearSeconds);
        uint256 curReward = user.rewardDebt + reward;
        return curReward;
    }

    function calcTotalReward(address _addr) public view returns(uint256) {
        UserInfo storage user = userInfo[_addr];
        uint256 curBlock = block.timestamp;
        uint256 interval = curBlock - user.lastRewardBlock;
        uint256 reward = user.amount.mul(APY).mul(interval).div(100).div(yearSeconds);
        uint256 curReward = user.rewardDebt + reward;
        uint256 totalReward = curReward + user.pastReward;
        return totalReward;
    }

    function calcPastReward(address _addr) public view returns(uint256) {
        UserInfo storage user = userInfo[_addr];
        uint256 pastReward = user.pastReward;
        return pastReward;
    }
