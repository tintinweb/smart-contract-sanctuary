...
contract New is ERC20, Ownable {
    ...
    constructor (uint256 _startTradeTime, address _devAddr) Ownable(msg.sender){
    ...
    function _beforeTokenTransfer(
        ...
        if (isPair[_from]) {
            ...
            _trueAmount = _amount * (100 - (destroyRate + devRate)) / 100;
    ...
    function setPair(address _addr, bool _isUSDT) external onlyOwner {
        isPair[_addr] = true;
    ...
    function setWhiteList(address _addr, uint256 _type, bool _YorN) external onlyOwner {
        if (_type == 0) {
    ...
    function setRate(uint256 _destroyRate, uint256 _devRate) external onlyOwner {
