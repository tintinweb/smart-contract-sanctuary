...
contract HulkToken is BEP20 {
    ...
    // Min amount to liquify. (default 500 HULKs)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to HulkSwap's router when our own AMM release
    IUniswapV2Router02 public hulkSwapRouter;
    // The trading pair
    address public hulkSwapPair;
    // In swap and liquify
    ...
    event HulkSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "HULK::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the HulkToken contract.
     */
    constructor() public BEP20("HulkSwap Token", "HULK") {
    ...
    /// @dev overrides transfer function to meet tokenomics of HULK
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(hulkSwapRouter) != address(0)
            && hulkSwapPair != address(0)
            && sender != hulkSwapPair
            && sender != owner()
        ) {
            swapAndLiquify();
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "HULK::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "HULK::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the hulkSwap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = hulkSwapRouter.WETH();

        _approve(address(this), address(hulkSwapRouter), tokenAmount);

        // make the swap
        hulkSwapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        hulkSwapRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from HulkSwapRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "HULK::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "HULK::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "HULK::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateHulkSwapRouter(address _router) public onlyOperator {
        hulkSwapRouter = IUniswapV2Router02(_router);
        hulkSwapPair = IUniswapV2Factory(hulkSwapRouter.factory()).getPair(address(this), hulkSwapRouter.WETH());
        require(hulkSwapPair != address(0), "HULK::updateHulkSwapRouter: Invalid pair address.");
        emit HulkSwapRouterUpdated(msg.sender, address(hulkSwapRouter), hulkSwapPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "HULK::transferOperator: new operator is the zero address");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // View function to see if user can harvest HULKs.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 hulkReward = multiplier.mul(hulkPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        hulk.mint(devAddress, hulkReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(hulk)) {
                uint256 transferTax = _amount.mul(hulk.transferTaxRate()).div(10000);
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(hulkReferral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                hulk.mint(referrer, commissionAmount);
