...
contract LiqsAirDropFactoryV1 {
    uint private _counter = 0;
    mapping(uint => address) private _airDrops;

    constructor() {

    }

    function createAirDrop(address token_, uint quantity_, uint tranche_) external returns(address) {
        LiqsAirdropV1 airDrop = new LiqsAirdropV1(token_, quantity_, tranche_);
        _airDrops[_counter] = address(airDrop);
        _counter += 1;
        return address(airDrop);
    }

    function getAirDrops() external view returns(uint) {
        return _counter;
    }

    function getAirDrop(uint index_) external view returns(address) {
        return _airDrops[index_];
    }
}