...
contract EtherDogePresale is Ownable {
   ...
   function addWhitelist(address[] calldata whitelistAddress) public onlyOwner {
        for(uint i=0;i<whitelistAddress.length;i++) {
            isWhitelisted[whitelistAddress[i]] = true;
        }
    }
    function removeWhitelist(address[] calldata whitelistAddress) public onlyOwner {
        for(uint i=0;i<whitelistAddress.length;i++) {
            isWhitelisted[whitelistAddress[i]] = false;
        }
    }
}