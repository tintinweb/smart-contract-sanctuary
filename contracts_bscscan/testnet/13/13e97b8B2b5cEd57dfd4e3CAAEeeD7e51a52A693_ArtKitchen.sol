...
contract ArtKitchen is ERC721Holder, Ownable, AccessControl, FeeCalculator, IArtKitchen {
    ...
    function signed(bytes32 _digest, bytes memory _signature, uint256 _nonce, uint256 _expiry) internal {
        address signer = _digest.toEthSignedMessageHash().recover(_signature);
        require(signer == _msgSender(), "Art Kitchen: Invalid signature");
        require(signer != address(0), "ECDSA: Invalid signature");
        require(block.timestamp <= _expiry, "Art Kitchen: Signed transaction expired");
        require(_nonce == nonces[signer], "Art Kitchen: Invalid nonce");
    ...
    /**
     * @dev Get owner for a specific tokenId.
     */
    ...
    function create(
    ...
    ) external override payable returns (uint256) {
        {
            ...
            signed(_digest, _signature, _nonce, _expiry);
        }
...
interface IArtKitchen {
     ...
     */
    function create(string calldata _itemName, string calldata _tokenURI, uint256 _price, uint256 _nonce, uint256 _expiry, bytes memory _signature) external payable returns (uint256);
     ...
     * - `_price` cannot be less than minimum price.
     */
    function sell(uint256 _tokenId, uint256 _price, uint256 _nonce, uint256 _expiry, bytes memory _signature) external;
     ...
     */
    function buy(uint256 _tokenId, uint256 _nonce, uint256 _expiry, bytes memory signature) external payable;
     ...
     * - `_tokenId` must not exist.
     */
    function withdrawNFT(uint256 _tokenId, uint256 _nonce, uint256 _expiry, bytes memory _signature) external;
     ...
     * - `_price` cannot be less than minimum price.
     */
    function updatePrice(uint256 _tokenId, uint256 _price, uint256 _nonce, uint256 _expiry, bytes memory _signature) external;
