...
contract HelloWorld is Ownable, ReentrancyGuard {
    ...
    function withdrawfromBridge(uint256 _amount, address _withdrawERC20, address _depositedERC20, uint256[2] memory chainIds, uint256 _nonce, bytes32 _txHash, Sig[] memory signatures, address[] memory _validatedBy) external noContractsAllowed nonReentrant {
        ...
        require(approvalCountForTrade(_amount, _withdrawERC20, _depositedERC20, _nonce, _txHash, signatures, _validatedBy) >= validatorApprovalrequired, "WithdrawfromBridge:: Not validated by required validators ");
    ...
    function approvalCountForTrade(uint256 _amount, address _withdrawERC20, address _depositedERC20, uint256 _nonce, bytes32 _txHash, Sig[] memory signatures, address[] memory _validatedBy) internal view returns (uint256) {
        uint256 noOfApprovals = 0;
        for(uint256 i=0; i< signatures.length; i++) {
            noOfApprovals = isValidTrade(_amount, _withdrawERC20, _depositedERC20, _nonce, _txHash, signatures[i], _validatedBy[i]);
    ...
    function isValidTrade(uint256 _amount, address _withdraERC20, address _depositedERC20, uint256 _nonce, bytes32 _txHash, Sig memory signature, address _validator) internal view returns (uint8) {
	    address validatedBy = StringLibrary.getAddress(abi.encodePacked(msg.sender, _amount, _withdraERC20, _depositedERC20, homeChainId, foreignChainId, _nonce, _txHash, address(this)), signature.v, signature.r, signature.s);
        if(isValidator[validatedBy] == true && validatedBy == _validator ) return 1;
        else return 0;
	}
