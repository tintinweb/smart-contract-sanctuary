...
import "./interfaces/IVirtualPhant.sol";
import "./Proxyable.sol";

// Note: use OZ's IERC20 here as using ours will complain about conflicting names
// during the build (VirtualPhant has IERC20 from the OZ ERC20 implementation)
import "openzeppelin-solidity-2.3.0/contracts/token/ERC20/IERC20.sol";

// Used to have strongly-typed access to internal mutative functions in Phantom

interface IPhantomInternal {
...
contract Exchanger is Owned, MixinSystemSettings, IExchanger {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[3] = CONTRACT_PHANTOM;
    ...
    // note: this is only used for access control and events
    ...
    function calculateAmountAfterSettlement(
        ...
        // balance of a phant will show an amount after settlement
        uint balanceOfSourceAfterSettlement = IERC20(address(issuer().phants(currencyKey))).balanceOf(from);
    ...
    function isPhantRateInvalid(bytes32 currencyKey) external view returns (bool) {
        return _isPhantRateInvalid(currencyKey, exchangeRates().rateForCurrency(currencyKey));
    }
    ...
    function exchange(
        ...
        bytes32 trackingCode
    ) external onlyPhantomOrPhant returns (uint amountReceived, IVirtualPhant vPhant) {
        uint fee;
        if (from != exchangeForAddress) {
            require(delegateApprovals().canExchangeFor(exchangeForAddress, from), "Not approved to act on behalf");
        }

        (amountReceived, fee, vPhant) = _exchange(
            exchangeForAddress,
            sourceCurrencyKey,
            sourceAmount,
        ...
        if (trackingCode != bytes32(0)) {
            IPhantomInternal(address(phantom())).emitExchangeTracking(
                trackingCode,
                destinationCurrencyKey,
                amountReceived,
                fee
    ...
    function _suspendIfRateInvalid(bytes32 currencyKey, uint rate) internal returns (bool circuitBroken) {
        if (_isPhantRateInvalid(currencyKey, rate)) {
            systemStatus().suspendPhant(currencyKey, CIRCUIT_BREAKER_SUSPENSION_REASON);
            circuitBroken = true;
        } else {
    ...
    function _updatePHMIssuedDebtOnExchange(bytes32[2] memory currencyKeys, uint[2] memory currencyRates) internal {
        ...
        debtCache().updateCachedPhantDebtsWithRates(keys, rates);
    }
        ...
        )
    {
        ...
        // When using a virtual phant, it becomes the destinationAddress for event and settlement tracking
        if (vPhant != IVirtualPhant(0)) {
            destinationAddress = address(vPhant);
        }
        ...
        if (fee > 0) {
            ...
            issuer().phants(pUSD).issue(feePool().FEE_ADDRESS(), fee);
        ...
        // in these currencies
        _updatePHMIssuedDebtOnExchange([sourceCurrencyKey, destinationCurrencyKey], [sourceRate, destinationRate]);

        // Let the DApps know there was a Phant exchange
        IPhantomInternal(address(phantom())).emitPhantExchange(
            from,
            sourceCurrencyKey,
            sourceAmountAfterSettlement,
            destinationCurrencyKey,
    ...
    function _convert(
        ...
        address recipient,
        bool virtualPhant
    ) internal returns (IVirtualPhant vPhant) {
        // Burn the source amount
        issuer().phants(sourceCurrencyKey).burn(from, sourceAmountAfterSettlement);

        // Issue their new phants
        IPhant dest = issuer().phants(destinationCurrencyKey);

        if (virtualPhant) {
            Proxyable phant = Proxyable(address(dest));
            vPhant = _createVirtualPhant(IERC20(address(phant.proxy())), recipient, amountReceived, destinationCurrencyKey);
            dest.issue(address(vPhant), amountReceived);
        } else {
            dest.issue(recipient, amountReceived);
        }
    }

    function _createVirtualPhant(
        IERC20,
        address,
        uint,
        bytes32
    ) internal returns (IVirtualPhant) {
    ...
    function suspendPhantWithInvalidRate(bytes32 currencyKey) external {
        ...
        require(issuer().phants(currencyKey) != IPhant(0), "No such phant");
        require(_isPhantRateInvalid(currencyKey, exchangeRates().rateForCurrency(currencyKey)), "Phant price is valid");
        systemStatus().suspendPhant(currencyKey, CIRCUIT_BREAKER_SUSPENSION_REASON);
    }

    // SIP-78
    function setLastExchangeRateForPhant(bytes32 currencyKey, uint rate) external onlyExchangeRates {
    ...
    function resetLastExchangeRate(bytes32[] calldata currencyKeys) external onlyOwner {
        ...
        require(!anyRateInvalid, "Rates for given phants not valid");
    ...
    function _ensureCanExchange(
        ...
        require(sourceCurrencyKey != destinationCurrencyKey, "Can't be same phant");
        require(sourceAmount > 0, "Zero amount");

        bytes32[] memory phantKeys = new bytes32[](2);
        phantKeys[0] = sourceCurrencyKey;
        phantKeys[1] = destinationCurrencyKey;
        require(!exchangeRates().anyRateIsInvalid(phantKeys), "Src/dest rate invalid or not found");
    }

    function _isPhantRateInvalid(bytes32 currencyKey, uint currentRate) internal view returns (bool) {
        if (currentRate == 0) {
            return true;
        }
        ...
        // if no last exchange for this phant, then we need to look up last 3 rates (+1 for current rate)
        ...
        )
    {
        ...
        if (updateCache) {
            ...
            key[0] = currencyKey;
            debtCache().updateCachedPhantDebts(key);
        }
    ...
    function reclaim(
        ...
        issuer().phants(currencyKey).burn(from, amount);
        IPhantomInternal(address(phantom())).emitExchangeReclaim(from, currencyKey, amount);
    }
    ...
    function refund(
        ...
        issuer().phants(currencyKey).issue(from, amount);
        IPhantomInternal(address(phantom())).emitExchangeRebate(from, currencyKey, amount);
    }
    ...
    modifier onlyPhantomOrPhant() {
        IPhantom _phantom = phantom();
        require(
            msg.sender == address(_phantom) || _phantom.phantsByAddress(msg.sender) != bytes32(0),
            "Exchanger: Only phantom or a phant contract can perform this action"
        );
        _;
    }
