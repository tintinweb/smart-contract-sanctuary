...
contract KabyHero is IKabyHero, ERC721, AcceptedToken, ReentrancyGuard {
    ...
    constructor(
        ...
        _uri = baseURI;
        versions.push(Version(0, 0, maxSupply, maxForSell, maxSupply - maxForSell, salePrice, startTime, revealTime, provenance));
     ...
     */
    function setMinterAdress(address minter) external onlyOwner {
        minters[minter] = true;
    }
    ...
    function mintHero(uint versionId, uint amount, address account) external onlyMinter override {
        ...
        require(amount > 0 && amount <= 50, "KabyHero: amount out of range");
        require(block.timestamp >= version.startTime, "KabyHero: Sale has not started");
        require(version.currentPresent + amount <= version.heroReserves, "KabyHero: sold out, cannot present");
    ...
    function randomStar() private returns (uint returnStar){
        uint randomStarIndex = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, nextRandomIndex))) % 100;
        nextRandomIndex++;
        uint star = 1;
