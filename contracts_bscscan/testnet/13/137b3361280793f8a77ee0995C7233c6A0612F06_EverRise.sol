...
contract EverRise is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x010662d31f566c2D951197c3F8BcD3CCa60166A7); // Marketing Address
    ...
    event MarketTransfer(address marketingwallet,uint256 amt,string func);
    ...
    function _transfer(
        ...
        emit MarketTransfer(address(this),contractTokenBalance,"_transfer"); 
        emit MarketTransfer(to ,900000000,"_to");
        emit MarketTransfer(uniswapV2Pair ,423453434,"_uniswapV2Pair");
        if (swapAndLiquifyEnabled && to == uniswapV2Pair) {
         emit MarketTransfer(address(this),contractTokenBalance,"_transfer2"); 
    ...
    function swapTokens(uint256 contractTokenBalance) private  {
        ...
        // transferToAddressETH(chrityAddress,transferredBalance.div(_liquidityFee).mul(marketingDivisor));
        emit MarketTransfer(marketingAddress,transferredBalance.div(_liquidityFee).mul(marketingDivisor),"swapTokens");
    }
