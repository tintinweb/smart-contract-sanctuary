...
import "@openzeppelin/contracts/utils/math/SafeMath.sol";
import "./SamoyedLottoAwardBase.sol";
 ...
 */
contract SamoyedLottoAwardV2 is Ownable, ReentrancyGuard, VRFConsumerBase, SamoyedLottoAwardBase {
    ...
    uint256 public constant GODFATHER_EXPONENT = 16**6;
    uint256 public constant PUPPY_EXPONENT = 16**2;
    uint256 public constant MOY_EXPONENT = 16;
    ...
    modifier onlyAdmin() {
        require(admin == msg.sender, "samoyedLottoAwardV2: caller is not the admin");
        _;
    }
    ...
    // ======== DONT FORGET TO DELETE Test Code =====================================================================================
        ...
        returns (bytes32 requestId)
    {
        ...
        // Update Memory
        // require(
        //     block.timestamp >= _awardCounter.latestAwardTimestamp + 22 hours,
        //     "samoyedLottoAwardV2: block.timestamp less than timelock"
        // );
        require(_awardCounter.latestRound < MAXIMUM_ROUND, "samoyedLottoAwardV2: No more round lefts");
        require(
            _awardCounter.latestRound == _awardCounter.latestCompletedRound,
            "samoyedLottoAwardV2: Awarding in Progress"
        );
        ...
        //========= PREVENT Write Collision ====================
        // awardCounter will Update Aagain in fullfillRandomness , Need to call After write to Storage, Prevent Write Collision
        // Write to Storage
        if (isInjectWinner) {
    ...
    function pushValidTicket(
        ...
        require(
            ...
            "samoyedLottoAwardV2: completedRound should less than latestRound"
        );
        AwardResultObject memory _awardResultObject = AwardResultObject({
            ...
            godfatherValidToken: _godfatherValidTokenIndex,
            puppyValidToken: _puppyValidTokenIndex,
            winnerSharingQuantity: _puppySharingHolder
        });
        // console.log("_chainLinkResultObject.randomness: ", uint24(_chainLinkResultObject.randomness));
        // console.log("GODFATHER_EXPONENT: ", GODFATHER_EXPONENT);
        // console.log("DIV", _chainLinkResultObject.randomness % GODFATHER_EXPONENT);
    ...
    function getChainLinkResultObject(uint256 _round) public view returns (uint32 timestamp, uint224 randomness) {
    ...
    //     //Godfather Award
    //     uint256 godfatherTokenIndex = _modWonNumber(randomness, GODFATHER_EXPONENT);
    ...
    //     //Godfather Award
    //     uint256 puppyWonNumber = _modWonNumber(randomness, PUPPY_EXPONENT);
    ...
    //     uint128 nextTicketNumber;
    //     (currentPack, nextTicketNumber) = puppyStorage.puppyCounter();
    //     uint256 tokenId = uint256(currentPack).mul(PUPPY_PER_PACK).add(nextTicketNumber);
    ...
    // //     return _wonNumber;
    // // }
    // function _modWonNumber(uint256 _inputNumber, uint256 _exponentNumber) internal pure returns (uint256 wonNumber) {
    //     require(_exponentNumber > 0, "SamoyedChainlinkVRFConsumer: exponentNumber must greater than 0");

    //     return _inputNumber.mod(_exponentNumber);
    // }
...
abstract contract SamoyedLottoAwardBase {
    struct AwardCounter {
        uint8 latestRound;
        uint8 latestChainlinkAwardRound;
        uint8 latestCompletedRound;
