...
contract sorting is AccessControl {
    ...
    uint256[] public data;
    function addlistArray(uint256 _min, uint256 _max) public {
        for(uint256 i = _min; i < _max; i++){
            data.push(i);
        }
    }
    // function getListArray(uint256 _min, uint256 _max) internal returns(uint256[] memory){
    //     uint256[] calldata result;
    //     for(uint256 i = _min; i < _max; i++){
    //         result.push(i);
    //     }
    //     return result;
