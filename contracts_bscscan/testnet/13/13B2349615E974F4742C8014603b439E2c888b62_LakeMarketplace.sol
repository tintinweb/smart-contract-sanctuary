...
interface ILakePriceFeed {
    function wBNB() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract LakeMarketplace is Ownable, ReentrancyGuard {
    ...
    /// @notice Constructor
    constructor(address payable _treasury, uint16 _platformFee) public {
    ...
    function _buyItem(
        ...
        if (minter != address(0) && royalty != 0) {
            ...
            minter = collectionRoyalties[_nftAddress].treasury;
    ...
    function registerRoyalty(
        ...
        require(_isLakeNFT(_nftAddress), "invalid nft address");
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC721)) {
            IERC721 nft = IERC721(_nftAddress);
            require(nft.ownerOf(_tokenId) == _msgSender(), "not owning item");
        } else if (
    ...
    function registerCollectionRoyalty(
        ...
        require(_royalty <= 10000, "invalid royalty");
        require(
            _royalty == 0 || _treasury != address(0),
    ...
    function _isLakeNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.lake() == _nftAddress ||
            ILakeNFTFactory(addressRegistry.erc721Factory()).exists(
                _nftAddress
            ) ||
            ILakeNFTFactory(addressRegistry.privateERC721Factory()).exists(
                _nftAddress
            ) ||
            ILakeNFTFactory(addressRegistry.erc1155Factory()).exists(
                _nftAddress
            ) ||
            ILakeNFTFactory(addressRegistry.privateERC1155Factory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        ...
        uint8 decimals;
        if (_payToken == address(0)) {
            ILakePriceFeed priceFeed = ILakePriceFeed(
                addressRegistry.priceFeed()
            );
            (unitPrice, decimals) = priceFeed.getPrice(priceFeed.wBNB());
        } else {
            (unitPrice, decimals) = ILakePriceFeed(addressRegistry.priceFeed())
                .getPrice(_payToken);
        }
        if (decimals < 18) {
    ...
    /**
     @notice Method for updating platform treasury
     @dev Only admin
