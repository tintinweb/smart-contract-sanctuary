...
library Math {
  ...
  function genRandomNumber(string calldata _seed, uint _oracleRandomSeed) internal view returns (uint8) {
    return genRandomNumberInRange(_seed, _oracleRandomSeed, 0, 99);
  }

  function genRandomNumberInRange(string calldata _seed, uint _oracleRandomSeed, uint _from, uint _to) internal view returns (uint8) {
...
abstract contract BaseUpgrading is Auth, EIP712Upgradeable, VRFConsumerBaseUpgradable {
  ...
  //  uint256 constant internal oracleFee = 0.2 * 10 ** 18; // TODO
//  bytes32 private constant keyHash = 0xc251acd21ec4fb7f31bb8868288bfdbaeb4fbfec2df3735ddbd4f7dc8d60103c;
  bytes32 private constant keyHash = 0xcaf3c3727e033261d383b315559476f48034c13b18f8cafed4d871abe5049186;
  uint256 private constant oracleFee = 0.1 * 10 ** 18;
  ...
  function initialize(
    ...
    //    VRFConsumerBaseUpgradable.initialize(0x747973a5A2a4Ae1D3a8fDF5479f1514F65Db9C31, 0x404460C6A5EdE2D891e8297795264fDe62ADBB75); TODO
    // TODO remove test
  ...
  function takeFund(IMarketPlace.PaymentCurrency _paymentCurrency, uint _upgradingFee) internal {
...
//  function _getRandomNumberFromChainLink() private returns (bytes32 requestId) {
  function _getRandomNumberFromChainLink() public returns (bytes32 requestId) {
...
import "../interfaces/IMarketPlace.sol";
import "./BaseUpgrading.sol";

contract ItemUpgrading is BaseUpgrading {
  ...
  function initialize(
    ...
    super.initialize(_name, _version, _mainAdmin, _fundAdmin, _fotaPricer);
  ...
  function _takeFund(IMarketPlace.PaymentCurrency _paymentCurrency, uint16 _itemClass, uint8 _acceptedRatio) private returns (uint _upgradingFee){
    ...
    takeFund(_paymentCurrency, _upgradingFee);
  }
  ...
  function _upgradeItem(
    ...
    success = isSuccess(_seed, _acceptedRatio);
    uint8 gene = uint8(itemClass / 100);
    if (success) {
      uint newTokenId = itemNft.mintItem(msg.sender, gene, itemClass, _ownPrice, 0);
      emit Upgraded(msg.sender, gene, _upgradingInfo, newTokenId, fee, _acceptedRatio, success);
    } else {
      emit Upgraded(msg.sender, gene, _upgradingInfo, _willKeepMaterialIdIfFailed, fee, _acceptedRatio, success);
    }
  }
