...
contract FlokinomicsNFTMarketplace is ReentrancyGuard, Ownable {
    ...
    public
    {
        ...
        FlokinomicsNFT(nftContract).approve(address(this), _id);
        FlokinomicsNFT(nftContract).transferFrom(msg.sender, address(this), _id);
        listedMap[_id] = true;
        price[_id] = _price;
    ...
    external
    {
        ...
        FlokinomicsNFT(nftContract).transferFrom(address(this), msg.sender, _id);
        listedMap[_id] = false;
    ...
    //         FlokinomicsNFT(nftContract).mint(newTokenId, _to, _tokenUri);
    //         emit Giveaway(msg.sender, _to, newTokenId);
    //     } else {
    //         if (listedMap[_id] == false) {
    //             FlokinomicsNFT(nftContract).transferFrom(msg.sender, _to, _id);
    //         } else {
    //             require(ownerMap[_id] == msg.sender, "sender is not owner");
    //             FlokinomicsNFT(nftContract).transferFrom(address(this), _to, _id);
    //             listedMap[_id] == false;
    //         }
    //         ownerMap[_id] = _to;
    ...
    function burn(uint256 _id) external {
        FlokinomicsNFT(nftContract).burn(_id);
    ...
    function buyNew(uint256 _id, 
        ...
        //    FlokinomicsNFT(nftContract).transferFrom(msg.sender, receiver, _sellerValue);
        //    FlokinomicsNFT(nftContract).transferFrom(msg.sender, adminRecoveryAddress, _commissionValue.div(2));
        //    FlokinomicsNFT(nftContract).transferFrom(msg.sender, payoutAddressMap[paymentType], _commissionValue.div(2));
        ...
        FlokinomicsNFT(nftContract).mint(newTokenId, msg.sender, _tokenUri);
    ...
    function _validate(uint256 _id) internal view {
        ...
        require(msg.sender != FlokinomicsNFT(nftContract).ownerOf(_id), "Can not buy what you own");
    ...
    function updateListingStatus(uint256 _tokenId, bool shouldBeListed) public returns (bool) {
        require(msg.sender == FlokinomicsNFT(nftContract).ownerOf(_tokenId), "Error, you are not the owner");
    ...
    function createAuction(
        ...
        if (_isNew) {
            ...
            FlokinomicsNFT(nftContract).mint(tokenId, msg.sender, _tokenUri);
        }
        ...
        onlyCreatorOrWinner(tokenId)
    {
        ...
        if(winner == address(0)) {
            FlokinomicsNFT(nftContract).transferFrom(address(this), Creator, tokenId);
            ownerMap[tokenId] = Creator;
        } else {
            FlokinomicsNFT(nftContract).transferFrom(address(this), winner, tokenId);
            if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("BNB")))) {
    ...
    function recoverNFT(uint256 tokenId) external onlyAdminRecovery {
        FlokinomicsNFT(nftContract).transferFrom(
 ...
 * @author Remco Bloemen <[email protected]π.com>, Eenae <[email protected].io>
...
contract FlokinomicsNFT is ERC721Enumerable, Ownable {
