...
contract DebtCache is BaseDebtCache {
    ...
    // This function exists in case a phant is ever somehow removed without its snapshot being updated.
    function purgeCachedPhantDebt(bytes32 currencyKey) external onlyOwner {
        require(issuer().phants(currencyKey) == IPhant(0), "Phant exists");
        delete _cachedPhantDebt[currencyKey];
    }

    function takeDebtSnapshot() external requireSystemActiveIfNotOwner {
        bytes32[] memory currencyKeys = issuer().availableCurrencyKeys();
        (uint[] memory values, bool isInvalid) = _currentPhantDebts(currencyKeys);

        uint numValues = values.length;
        uint phmCollateralDebt;
        for (uint i; i < numValues; i++) {
            uint value = values[i];
            phmCollateralDebt = phmCollateralDebt.add(value);
            _cachedPhantDebt[currencyKeys[i]] = value;
        }
        uint newDebt = phmCollateralDebt;
        _cachedDebt = newDebt;
    ...
    function updateCachedPhantDebts(bytes32[] calldata currencyKeys) external requireSystemActiveIfNotOwner {
        (uint[] memory rates, bool anyRateInvalid) = exchangeRates().ratesAndInvalidForCurrencies(currencyKeys);
        _updateCachedPhantDebtsWithRates(currencyKeys, rates, anyRateInvalid);
    }

    function updateCachedPhantDebtWithRate(bytes32 currencyKey, uint currencyRate) external onlyIssuer {
        bytes32[] memory phantKeyArray = new bytes32[](1);
        phantKeyArray[0] = currencyKey;
        uint[] memory phantRateArray = new uint[](1);
        phantRateArray[0] = currencyRate;
        _updateCachedPhantDebtsWithRates(phantKeyArray, phantRateArray, false);
    }

    function updateCachedPhantDebtsWithRates(bytes32[] calldata currencyKeys, uint[] calldata currencyRates)
        external
        onlyIssuerOrExchanger
    {
        _updateCachedPhantDebtsWithRates(currencyKeys, currencyRates, false);
    }
    ...
    function _updateCachedPhantDebtsWithRates(
        ...
        // Update the cached values for each phant, saving the sums as we go.
        uint cachedSum;
        uint currentSum;
        uint[] memory currentValues = _issuedPhantValues(currencyKeys, currentRates);

        for (uint i = 0; i < numKeys; i++) {
            bytes32 key = currencyKeys[i];
            uint currentPhantDebt = currentValues[i];
            cachedSum = cachedSum.add(_cachedPhantDebt[key]);
            currentSum = currentSum.add(currentPhantDebt);
            _cachedPhantDebt[key] = currentPhantDebt;
        }

        // Compute the difference and apply it to the snapshot
        if (cachedSum != currentSum) {
            uint debt = _cachedDebt;
            // This requirement should never fail, as the total debt snapshot is the sum of the individual phant
            // debt snapshots.
            require(cachedSum <= debt, "Cached phant sum exceeds total debt");
...
interface IDebtCache {
    ...
    function cachedPhantDebt(bytes32 currencyKey) external view returns (uint);
    ...
    function currentPhantDebts(bytes32[] calldata currencyKeys)
        external
        view
        returns (
    ...
    function cachedPhantDebts(bytes32[] calldata currencyKeys) external view returns (uint[] memory debtValues);
    ...
    function updateCachedPhantDebts(bytes32[] calldata currencyKeys) external;
    ...
    function purgeCachedPhantDebt(bytes32 currencyKey) external;
...
contract BaseDebtCache is Owned, MixinSystemSettings, IDebtCache {
    ...
    mapping(bytes32 => uint) internal _cachedPhantDebt;
    ...
    function cachedPhantDebt(bytes32 currencyKey) external view returns (uint) {
        return _cachedPhantDebt[currencyKey];
    }
    ...
    function _issuedPhantValues(bytes32[] memory currencyKeys, uint[] memory rates)
        internal
        view
        returns (uint[] memory values)
    {
        ...
        IPhant[] memory phants = issuer().getPhants(currencyKeys);

        for (uint i = 0; i < numValues; i++) {
            address phantAddress = address(phants[i]);
            require(phantAddress != address(0), "Phant does not exist");
            uint supply = IERC20(phantAddress).totalSupply();
    ...
    function _currentPhantDebts(bytes32[] memory currencyKeys)
        internal
        view
        returns (
            uint[] memory phmIssuedDebts,
            bool anyRateIsInvalid
        )
    {
        (uint[] memory rates, bool isInvalid) = exchangeRates().ratesAndInvalidForCurrencies(currencyKeys);
        uint[] memory values = _issuedPhantValues(currencyKeys, rates);
    ...
    {
        return _currentPhantDebts(currencyKeys);
    }

    function _cachedPhantDebts(bytes32[] memory currencyKeys) internal view returns (uint[] memory) {
        uint numKeys = currencyKeys.length;
        uint[] memory debts = new uint[](numKeys);
        for (uint i = 0; i < numKeys; i++) {
            debts[i] = _cachedPhantDebt[currencyKeys[i]];
        }
        return debts;
    }

    function cachedPhantDebts(bytes32[] calldata currencyKeys) external view returns (uint[] memory phmIssuedDebts) {
        return _cachedPhantDebts(currencyKeys);
    }
    ...
    function _currentDebt() internal view returns (uint debt, bool anyRateIsInvalid) {
        ...
        // Sum all issued phant values based on their supply.
        uint[] memory values = _issuedPhantValues(currencyKeys, rates);
