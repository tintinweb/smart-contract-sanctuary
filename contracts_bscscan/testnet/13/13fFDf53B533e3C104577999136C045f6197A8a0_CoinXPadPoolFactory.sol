...
contract CoinXPadPoolFactory is Ownable {
    ...
    function createPoolPublic(IDOInfo calldata poolInfo) external onlyOwner {
        ...
        setCanBuyAfterTierMinutes(_idoPool, poolInfo._canBuyAfterMinutes);
    ...
    function setIDOTierInfo(
        ...
        uint256[] calldata _tiersDuration,
    ...
    function setIDOFCFSInfo(
        ...
        uint256 _allTiersDuration = 0;
        for (uint256 i = 0; i < _tiersDuration.length; i++) {
            _allTiersDuration = _allTiersDuration.add(_tiersDuration[i]);
        }
        _pool.setFCFSAllocInfo(
            _allTiersDuration,
            _fcfsStartTime,
            _maxAmountThatCanBeInvestedInFCFS,
            _minAmountThatCanBeInvestedInFCFS,
            _balanceRequiredForInvestmentInFCFS,
            _fcfsAllocation
        );
    }

    /**
     * @dev To set tier canBuyAfterMinuter in IDO contract
     * @param _pool The IDOPool contract object
     * @param canBuyAfterTierMinutes An array of valid minutes after which buy can be executed for tier
     */
    function setCanBuyAfterTierMinutes(IDOPool _pool, uint256[] calldata canBuyAfterTierMinutes) internal {
        _pool.setCanBuyAfterMinutesTierInfo(canBuyAfterTierMinutes);
    }
...
contract IDOPool is Ownable, ReentrancyGuard {
    ...
    modifier publicSaleActive() {
        require((block.timestamp >= startTime && block.timestamp <= endTime), "Public sale is not yet activated");
        _;
    }
    ...
    function buy(uint256 amount) external publicSaleActive nonReentrant {
        ...
        */
        ...
        if ((tier == 0) && (now >= fcfsStartTime)) {
            _fcfsInfo = fcfsInfo[0];
            if (!tierAllocationMovedToFCFS) {
                // Set totalAllocation for FCFS
                for (uint8 i = 0; i < totalTiers; i++) {
                    _tierInfo = tierInfo[i + 1];
                    _fcfsInfo.totalAllocation = _fcfsInfo.totalAllocation.add(_tierInfo.availableTokens);
                    tierInfo[i + 1].availableTokens = 0;
                }
            ...
            require(token.balanceOf(msg.sender) >= balanceRequiredForInvestment, "FCFS: insufficient balance");
            ...
            uint256 canBuyAfterMinutes = _tierInfo.canBuyAfterMinutes;
            ...
            require(now >= startTime.add(canBuyAfterMinutes * 60), "Wait for tier duration to start");
    ...
    function setTierInfo(
        ...
        for (uint8 i = 0; i < _tiersAllocation.length; i++) {
            ...
            tierInfo[i + 1] = TierInformation({
                ...
                duration: _tiersDuration[i],
                canBuyAfterMinutes: 1
            });
        }
     ...
     * @param _allTiersDuration Duration of all tiers
    ...
    function setFCFSAllocInfo(
        ...
        require(_fcfsStartTime > (startTime.add(_allTiersDuration)), "FCFS must be > startTime + allTiersDuration");
    ...
    /**
     * @dev To tiers canBuyAfterMinutes to the contract storage
     * @param canBuyAfterTierMinutes An array of minutes w.r.t. tiers
     */
    function setCanBuyAfterMinutesTierInfo(uint256[] memory canBuyAfterTierMinutes) public onlyOwner returns (bool) {
        TierInformation memory _tierInfo;
        for (uint8 i = 0; i < canBuyAfterTierMinutes.length; i++) {
            _tierInfo = tierInfo[i + 1];
            _tierInfo.canBuyAfterMinutes = canBuyAfterTierMinutes[i];
            // Write to storage
            tierInfo[i + 1] = _tierInfo;
        }
        return true;
    }
