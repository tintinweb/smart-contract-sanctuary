...
contract Olympia is ERC20, Ownable {
    ...
    function _swap() private swapping {
        ...
        uint256 ethAmount = address(this).balance.sub(ethBalance);
        ...
        _sendDividends(reflectionEthBalance);
        
        uint256 remainingEthBalance = ethAmount.sub(teamEthAmount).sub(providerEthAmount).sub(marketingEthAmount).sub(reflectionEthBalance);
        _liquify(liquidityTokenAmount, remainingEthBalance);
    }
    ...
    function abc_swapAndLiquify() external onlyOwner {
        ...
        uint256 reflectionEthBalance = ethAmount.mul(liquidityFeesPerTenThousand).div(totalFeesPerTenThousand);
        _liquify(liquidityTokenAmount, reflectionEthBalance);
    }
