...
interface IBSCdefiSwapLottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in test per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in test token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInTest: price of a ticket in test
...
/** @title BSCdefi Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract BSCdefiLottery is ReentrancyGuard, IBSCdefiSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInTest = 50 ether;
    uint256 public minPriceTicketInTest = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInTest;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] testPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInTest;
        uint32 finalNumber;
    }
     ...
     * @param _testTokenAddress: address of the test token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _testTokenAddress, address _randomGeneratorAddress) {
        ...
        nonReentrant
    {
        ...
        // Calculate number of test to this contract
        uint256 amountTestToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInTest,
            _ticketNumbers.length
        );

        // Transfer test tokens to this contract
        testToken.safeTransferFrom(address(msg.sender), address(this), amountTestToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInTest += amountTestToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardIntestToTransfer
        uint256 rewardInTestToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInTestToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        testToken.safeTransfer(msg.sender, rewardInTestToTransfer);

        emit TicketsClaim(msg.sender, rewardInTestToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInTest) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in test for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].testPerBracket[j] =
                ...
                // A. No test to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].testPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInTest - amountToShareToWinners);

        // Transfer test to treasury address
        testToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        testToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInTest += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInTest >= minPriceTicketInTest) && (_priceTicketInTest <= maxPriceTicketInTest),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInTest: _priceTicketInTest,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            testPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInTest: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(testToken), "Cannot be Test token");
    ...
    /**
     * @notice Set test price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInTest: minimum price of a ticket in Test
     * @param _maxPriceTicketInTest: maximum price of a ticket in Test
     */
    function setMinAndMaxTicketPriceInTest(uint256 _minPriceTicketInTest, uint256 _maxPriceTicketInTest)
        external
        onlyOwner
    {
        require(_minPriceTicketInTest <= _maxPriceTicketInTest, "minPrice must be < maxPrice");

        minPriceTicketInTest = _minPriceTicketInTest;
        maxPriceTicketInTest = _maxPriceTicketInTest;
    }
     ...
     * @param _priceTicket price of a ticket (in test)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].testPerBracket[_bracket];
        } else {
            return 0;
        }
    }
