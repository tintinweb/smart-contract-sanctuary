...
contract PokerLoot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 public payFee = 5000000000000000000;
    uint256 public payDecreaseStep = 1000000000000000000;
    string _desc =
        "PokerLoot is randomized adventurer gear generated and stored on chain. Stats, images, and other functionality are intentionally omitted for others to interpret. Feel free to use Loot in any way you want.";
    uint256 minted = 0;
    mapping(uint256 => uint8[5]) public minted_pokers;
    mapping(uint256 => string) public minted_poker_images;
    bool useTimestampRandom = false;
    

    event MintNum(uint256 tokenId, uint8[5] cards);
    ...
    function gen(uint256 tokenId) private returns (uint8[5] memory) {
        uint8[52] memory arr = [
            0,
            1,
            2,
        ...
        for (uint8 i = 0; i < 5; i++) {
            uint8 len = 51 - i;
            uint256 rand;
            if(useTimestampRandom) {
                rand = random(
                    string(
                        abi.encodePacked(
                            toString(tokenId),
                            toString(block.timestamp),
                            toString(uint256(last))
                        )
                    )
                );
            } else {
            ...
            // swap
            uint8 value = arr[index];
            arr[index] = arr[i];
            arr[i] = value;
            last = value;
        }
        
        uint8[5] memory result = [arr[0], arr[1], arr[2], arr[3], arr[4]];
    ...
    function mint(uint16 tokenId) public payable {
        require(msg.value >= payFee, "you have to pay for mint");
        require(minted < _totalSupply, "mint over totalSupply");
        ...
        minted_pokers[tokenId] = arr;
        // minted_poker_images[minted] = svg;
        _safeMint(owner(), tokenId);
        if(payFee > 0) {
            payFee -= payDecreaseStep;
        }
    ...
    function genSVG(uint256 tokenId)
        private
        returns (uint8[5] memory arr, string memory svg)
    {
        arr = gen(tokenId);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        Base64.encode(bytes(minted_poker_images[tokenId])),
                        '"}'
                    )
                )
            )
    ...
    function setRandomUseTimestamp(bool _useTimestampRandom) public onlyOwner {
        useTimestampRandom = _useTimestampRandom;
    }
