...
// File: contracts/level1/GachaMachine.sol
...
contract GachaponMachine {
    ...
    function _randomWithSalt() internal returns (uint256) {
        salt = (salt + 77) % 33;
        return uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), msg.sender, salt))) % 100;
    }
}