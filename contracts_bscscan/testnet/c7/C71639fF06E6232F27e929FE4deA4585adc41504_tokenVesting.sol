...
contract tokenVesting is Ownable, Pausable {
    ...
    function claim (uint stage) external {
        require(stage >= 0 && stage <= 8, "Stage not valid");
        require(startTime + (stage * 10 minutes) <= block.timestamp, "Stage not ready to claim yet");
        ...
        uint256 amountToTranfer = userAllocation[msg.sender] / 10;    
        if(stage == 0){
            amountToTranfer = amountToTranfer * 3;
        }    
