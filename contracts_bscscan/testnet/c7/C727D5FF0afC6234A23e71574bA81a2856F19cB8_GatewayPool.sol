...
contract GatewayPool is OwnableUpgradeable, ReentrancyGuard {
    ...
    function outNetwork(
        ...
        require(_amount >= outFee, "_amount less than fee");
        ...
        // move fee
        IERC20(token).safeTransfer(feeAddress, outFee);
        _amount = _amount - outFee;
    ...
    function inNetwork(
        ...
        // Verify signature
        bytes32 signMessage = hashForSignature(_networkId, _txHash, _amount, _receiver, _nHash);
        ...
        adminCaps[msg.sender] = adminCaps[msg.sender].sub(_amount);
        uint256 _before = IERC20(token).balanceOf(address(this));
        uint256 transferAmount = _amount;
        uint256 fee = (_amount * inFee) / 100000;
        require(_amount >= fee, "not enough fee");
        IERC20(token).safeTransfer(feeAddress, fee);
        _amount = _amount - fee;
        IERC20(token).safeTransfer(_receiver, _amount);
        uint256 _after = IERC20(token).balanceOf(address(this));
        require(transferAmount == _before - _after, "invalid transfer");
