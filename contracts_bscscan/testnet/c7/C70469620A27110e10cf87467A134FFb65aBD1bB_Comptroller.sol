...
// File: contracts/FAIControllerInterface.sol
...
// File: contracts/Governance/FTS.sol
...
contract FTS is Tokenlock {
    /// @notice BEP-20 token name for this token
    string public constant name = "Fortress";

    /// @notice BEP-20 token symbol for this token
    string public constant symbol = "FTS";

    /// @notice BEP-20 token decimals for this token
    uint8 public constant decimals = 18;

    /// @notice Total number of tokens in circulation
    uint public constant totalSupply = 10000000e18; // 10 million FTS
    ...
    /**
     * @notice Construct a new FTS token
    ...
    function approve(address spender, uint rawAmount) external validLock returns (bool) {
        ...
        if (rawAmount == uint(-1)) {
            ...
            amount = safe96(rawAmount, "FTS::approve: amount exceeds 96 bits");
        }
    ...
    function transfer(address dst, uint rawAmount) external validLock returns (bool) {
        uint96 amount = safe96(rawAmount, "FTS::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external validLock returns (bool) {
        ...
        uint96 amount = safe96(rawAmount, "FTS::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != uint96(-1)) {
            uint96 newAllowance = sub96(spenderAllowance, amount, "FTS::transferFrom: transfer amount exceeds spender allowance");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public validLock {
        ...
        require(signatory != address(0), "FTS::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "FTS::delegateBySig: invalid nonce");
        require(now <= expiry, "FTS::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "FTS::getPriorVotes: not yet determined");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(src != address(0), "FTS::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "FTS::_transferTokens: cannot transfer to the zero address");

        balances[src] = sub96(balances[src], amount, "FTS::_transferTokens: transfer amount exceeds balance");
        balances[dst] = add96(balances[dst], amount, "FTS::_transferTokens: transfer amount overflows");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "FTS::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "FTS::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "FTS::_writeCheckpoint: block number exceeds 32 bits");
...
contract FAI is LibNote {
    // --- Auth ---
    mapping (address => uint) public wards;
    function rely(address guy) external note auth { wards[guy] = 1; }
    function deny(address guy) external note auth { wards[guy] = 0; }
    modifier auth {
        require(wards[msg.sender] == 1, "FAI/not-authorized");
        _;
    }

    // --- BEP20 Data ---
    string  public constant name     = "FAI Stablecoin";
    ...
    function add(uint x, uint y) internal pure returns (uint z) {
        require((z = x + y) >= x, "FAI math error");
    }
    ...
    function transferFrom(address src, address dst, uint wad)
        public returns (bool)
    {
        require(balanceOf[src] >= wad, "FAI/insufficient-balance");
        if (src != msg.sender && allowance[src][msg.sender] != uint(-1)) {
            require(allowance[src][msg.sender] >= wad, "FAI/insufficient-allowance");
    ...
    function burn(address usr, uint wad) external {
        require(balanceOf[usr] >= wad, "FAI/insufficient-balance");
        if (usr != msg.sender && allowance[usr][msg.sender] != uint(-1)) {
            require(allowance[usr][msg.sender] >= wad, "FAI/insufficient-allowance");
                    ...
                    bool allowed, uint8 v, bytes32 r, bytes32 s) external
    {
        ...
        require(holder != address(0), "FAI/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "FAI/invalid-permit");
        require(expiry == 0 || now <= expiry, "FAI/permit-expired");
        require(nonce == nonces[holder]++, "FAI/invalid-nonce");
...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    /// @notice Emitted when disable borrow fortress is changed
    event NewDisableBorrowFortress(FToken indexed fToken, bool disable);
    ...
    /**
      * @notice Disable the FTS reward for the given fToken market.
      * @param fToken The address of the market (token) to change the FTS reward setting for.
      * @param disable The boolean value to disable the FTS reward or not.
      */
    function _disableBorrowFortress(FToken fToken, bool disable) external {
    	require(msg.sender == admin || msg.sender == borrowCapGuardian, "only admin or borrow cap guardian can set disable borrow fortress"); 
        disableBorrowFortress[address(fToken)] = disable;
        emit NewDisableBorrowFortress(fToken, disable);
    }
    ...
    function updateFortressBorrowIndex(address fToken, Exp memory marketBorrowIndex) internal {
        ...
        if (deltaBlocks > 0 && borrowSpeed > 0 && !disableBorrowFortress[fToken]) {
    ...
    function getFTSAddress() public view returns (address) {
        return 0x00d1d87E5A64DA9F87464aCB9f15aEE980bf7eA5;
    }
