...
contract ReflectionToken is Context, IERC20, Ownable, AccessControl {
  ...
  constructor(
    ...
    _maxTxAmount = MAX;
    numTokensSellToAddToLiquidity = MAX;
  ...
  function setNumTokensSellToAddToLiquidity(uint256 swapNumber) external onlyOwner {
  ...
  function _transfer(
    ...
    if (from != _owner && to != _owner) require((enabled && amount <= _maxTxAmount) || hasRole(DEFAULT_ADMIN_ROLE, msg.sender), 'Transfer amount exceeds the maxTxAmount.');
