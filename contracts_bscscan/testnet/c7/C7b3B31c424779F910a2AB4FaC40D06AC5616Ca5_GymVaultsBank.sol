...
contract GymVaultsBank is ReentrancyGuard, Ownable {
    ...
    address constant buyBack = 0x3Aa5ebB10DC797CAC828524e59A333d0A371443c;
    address public farming;
    // contracts[7] - RelationShip address 
    address constant relationship = 0xc6e7DF5E7b4f2A278906862b61205850344D4e7d;
    ...
    constructor(
        ...
        alpacaToWBNB = [0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0, 0xa513E6E4b8f2a923D98304ec87F64353C4D5C853];
    ...
    function deposit(
        ...
        PoolInfo storage pool = poolInfo[_pid];
        if (address(pool.want) == 0xa513E6E4b8f2a923D98304ec87F64353C4D5C853) { // If `want` is WBNB
            IWETH(0xa513E6E4b8f2a923D98304ec87F64353C4D5C853).deposit{value: msg.value}();
            _wantAmt = msg.value;
        }
    ...
    function getBNBAmount(address _user) public view returns (uint256) {
        ...
        uint256 pendingAlpacaContract = IFairLaunchV1(0xa513E6E4b8f2a923D98304ec87F64353C4D5C853)
            .pendingAlpaca(1, pool.strategy);
        uint256 totalToken = IVault(0x959922bE3CAee4b8Cd9a407cc3ac1C251C2007B1).totalToken();
        uint256 totalSupply = IERC20(0x959922bE3CAee4b8Cd9a407cc3ac1C251C2007B1).totalSupply();
        uint256[] memory alpacaToBNBAmount = IPancakeRouter02(0x610178dA211FEF7D417bC0e6FeD39F05609AD788)
            .getAmountsOut(pendingAlpacaContract, alpacaToWBNB);
        uint256 pendingBNBUser = (user.shares * alpacaToBNBAmount[1]) /
            wantLockedTotal;
        uint256 pendingIbBNB = (pendingBNBUser * totalSupply) / totalToken;

        uint256 userAmount = (user.shares *
            IFairLaunchV1(0xa513E6E4b8f2a923D98304ec87F64353C4D5C853).userInfo(1, pool.strategy).amount) /
            wantLockedTotal;
    ...
    function _deposit(
        ...
        if (_wantAmt > 0) {
            if (address(pool.want) != 0xa513E6E4b8f2a923D98304ec87F64353C4D5C853) { // If `want` not WBNB
    ...
    function _transfer(address _token, address _receiver, uint256 _amount) private {
        if (_token == 0xa513E6E4b8f2a923D98304ec87F64353C4D5C853) { // If _token is WBNB
