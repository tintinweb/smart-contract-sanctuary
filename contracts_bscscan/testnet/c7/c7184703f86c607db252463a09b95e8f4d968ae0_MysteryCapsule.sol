...
contract MysteryCapsule is AccessControlUpgradeable {
    ...
    int128 public mintSpaceshipFee;
    ...
    function initialize() public initializer {
        ...
        spaceshipRate[0] = 97_559;
        spaceshipRate[1] = 1_951;
        spaceshipRate[2] = 415;
        spaceshipRate[3] = 73;
        spaceshipRate[4] = 2;
        ...
        mintSpaceshipFee = ABDKMath64x64.divu(10, 1); // $10
    }
    ...
    function setSpaceship(address spaceshipAddr) public restricted {
        spaceship = Spaceship(spaceshipAddr);
    }

    function setRandom(address randomAddr) public restricted {
        randoms = IRandoms(randomAddr);
    }

    function setPriceOracle(address priceOracleAddr) public restricted {
        priceOracle = IPriceOracle(priceOracleAddr);
    }

    function setDateTime(address dateTimeAddr) public restricted {
        dateTime = DateTime(dateTimeAddr);
        currentMonth = dateTime.getMonth(block.timestamp);
    }

    function setMonthLimitRate(uint256 _rate, uint256 _limit) public restricted {
    ...
    function resetLimitByMonth() public restricted {
        _resetLimitByMonth();
    }
    ...
    function setSpaceshipMintFee(int128 fee) public restricted {
        mintSpaceshipFee = fee;
    }

    function getSpaceshipMintFee() public view returns (uint256) {
        return usdToSip(mintSpaceshipFee);
    }

    function setMaxClass(uint8 max) public restricted {
        maxClass = max;
    }

    function setMaxElement(uint8 max) public restricted {
        maxElement = max;
    }

    function setMaxRarity(uint8 max) public restricted {
        maxRarity = max;
    }

    function usdToSip(int128 usdAmount) public view returns (uint256) {
        return usdAmount.mulu(priceOracle.currentPrice());
    }

    function _getRarity(uint256 seed) private view returns (uint8) {
        uint256 r = RandomUtil.randomSeededMinMax(0, 99998, seed);
        uint256 current = 0;
        for (uint8 rarity = maxRarity; rarity >= 0; rarity--) {
            current += spaceshipRate[rarity];
            if (r <= current) {
    ...
    function increaseCountAtRate(uint8 rarity) public restricted {
        spaceshipTotalByRate[rarity] += 1;
    }

    function issueSpaceship() public onlyNonContract oncePerBlock(msg.sender) returns (uint256) {
        uint256 shipFee = usdToSip(mintSpaceshipFee);

        require(token.balanceOf(msg.sender) >= shipFee, "TA");
        _payContractTokenOnly(msg.sender, shipFee);

        return _createSpaceship(msg.sender);
    }

    function createSpaceship(address player) public restricted returns (uint256) {
    ...
    function _payContractTokenOnly(address playerAddress, uint256 amount) internal {
        token.transferFrom(playerAddress, address(this), amount);
    }
    ...
    function _restricted() internal view {
        require(hasRole(GAME_ADMIN, msg.sender), "NGA"); // NGA: Not Game Admin
    }

    function withdraw(address addr) external restricted {
        require(token.balanceOf(address(this)) > 0, "BZ");
        token.transfer(addr, token.balanceOf(address(this)));
    }

    function addAdmin(address _admin) public restricted {
        _setupRole(GAME_ADMIN, _admin);
    }
}
...
contract Spaceship is ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, AccessControlUpgradeable {
    ...
    function initialize() public initializer {
        __ERC721_init("SIP.SPACE (space SIP) Ship", "SIPS");
    ...
    function setSpaceshipAttr(address spaceshipAttrAddr) public restricted {
    ...
    function mint(
        ...
        tokens.push(Ship(true, tokenId, rarity, class, element));
        spaceshipAttr.setSpaceshipAttr(tokenId, attrSeed, rarity);
    ...
    function transferCooldownEnd(uint256 tokenId) public view returns (uint256) {
        return lastTransferTimestamp[tokenId].add(transferCooldown);
    }
    ...
    function setTransferCooldown(uint256 _transferCooldown) public restricted {
        transferCooldown = _transferCooldown;
    }

    function setIsTransferCooldown(bool _isTransferCooldown) public restricted {
    ...
    function setTokenURI(uint256 tokenId, string memory _tokenURI) public restricted {
    ...
    // function mySpaceships() public view returns (ShipDetail[] memory) {
    //     uint256[] memory ids = spaceshipsOfOwner(msg.sender);
    //     uint256 length = ids.length;

    //     ShipDetail[] memory _ships = new ShipDetail[](length);

    //     for (uint256 i = 0; i < length; i++) {
    //         uint256 id = ids[i];
    //         if (_isNotFreshLookup(id)) {
    //             _ships[i] = get(id);
    //         }
    //     }
    ...
    function spaceshipsOfOwner(address _owner) public view returns (uint256[] memory ownerTokens) {
        ...
        if (tokenCount == 0) {
            ...
            for (uint256 shipId = 0; shipId <= tokenCount - 1; shipId++) {
    ...
    function get(uint256 tokenId)
        public
        view
        shipRequired(tokenId)
        noFreshLookup(tokenId)
        returns (
            uint256,
            uint256,
            uint256
    ...
    {
        Ship memory ship = tokens[tokenId];
        return (ship.rarity, ship.class, ship.element);
    }

    modifier shipRequired(uint256 tokenId) {
        Ship memory ship = tokens[tokenId];
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) {
            ...
            if (isTransferCooldown) {
                require(lastTransferTimestamp[tokenId] < block.timestamp.sub(transferCooldown), "CD");
            }
...
contract SpaceshipAttr is AccessControlUpgradeable {
    ...
    mapping(uint256 => mapping(uint256 => uint256[2])) public baseAttr; // rarity => attr => min, max
    // 0: Fual
    // 1: HP
    // 2: Attack
    // 3: Level

    mapping(uint256 => ShipAttr) public attrs;
    ...
    function setSpaceshipAttr(
        ...
        uint256 baseFual = RandomUtil.getRandomAttr(attrSeed, 0, baseAttr[rarity][0][0], baseAttr[rarity][0][1]);
    ...
    function getSpaceshipAttr(uint256 tokenId)
        public
        view
        shipRequired(tokenId)
        returns (
    ...
    {
        ShipAttr memory ship = attrs[tokenId];
        // TODO: calc hp and attack
        return (ship.currentFual, ship.currentLevel, ship.baseHP, ship.baseAttack);
    }

    function getCurrentLevel(uint256 tokenId) public view shipRequired(tokenId) returns (uint256) {
        ShipAttr memory ship = attrs[tokenId];
        return (ship.currentLevel);
    }

    function getCurrentFual(uint256 tokenId) public view shipRequired(tokenId) returns (uint256) {
        ShipAttr memory ship = attrs[tokenId];
        return (ship.currentFual);
    }

    function getCurrentEXP(uint256 tokenId) public view shipRequired(tokenId) returns (uint256) {
        ShipAttr memory ship = attrs[tokenId];
        return (ship.currentEXP);
    }

    function setCurrentEXP(uint256 tokenId, uint256 _exp) public restricted shipRequired(tokenId) {
        attrs[tokenId].currentEXP = _exp;
    }

    function increaseLevel(uint256 tokenId) public restricted shipRequired(tokenId) {
        ShipAttr memory ship = attrs[tokenId];
        if (ship.currentLevel >= ship.maxLevel) {
            return;
        }
        attrs[tokenId].currentLevel += 1;
    }

    function changeFual(uint256 tokenId, uint256 _fual) public restricted shipRequired(tokenId) {
        ShipAttr memory ship = attrs[tokenId];
        if (_fual >= ship.baseFual) {
            attrs[tokenId].currentFual = ship.baseFual;
            return;
        }
        attrs[tokenId].currentFual = _fual;
    }
