...
contract BouiSingleNft is
...

 {
    ...
    function _acceptBId(string memory tokenAss,address from, address admin, uint256 amount, uint256 tokenId,string memory _type) internal{
        uint256 val = pBEPent(amount, serviceValue).add(amount);
        BEP20 t = BEP20(tokentype[tokenAss]);
        ...
        require( approveValue >= val, "Insufficient Balance1");
        require(balances[tokenId][msg.sender] > 0, "Is Not a Owner");
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], serviceValue);
        emit Checking(approveValue, _adminfee, netamount, roy);
        require( approveValue >= _adminfee.add(roy.add(netamount)), "Insufficient Balance2");
    ...
    function _saleToken(
        ...
        uint256 userbalance =  t.balanceOf(msg.sender);
        test1=userbalance;
        test2=userbalance.div(1e18);
        
         uint256 discountamt=0;
        if(userbalance.div(1e18)>=discountMinAmount){
         discountamt = (amount*discountMinPercent).div(1000);
            amount=amount.sub(discountamt);
            test3=amount;
        }
        ...
        require( msg.value == _adminfee.add(roy.add(netamount)).add(discountamt), "Insufficient Balance3");
    ...
    function tokenTrans(
     ...
     function acceptBId(string memory bidtoken,address bidaddr, uint256 amount, uint256 tokenId, string memory _type) public{
        _acceptBId(bidtoken, bidaddr, owner(), amount, tokenId,_type);
