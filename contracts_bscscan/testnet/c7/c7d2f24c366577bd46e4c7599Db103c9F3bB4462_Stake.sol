...
contract Stake {
    ...
    function userWithdraw(uint _type,uint256 _pid,uint256 _amount,address _user)internal {
         ...
         uint percent = userInfo.depositAmount*100/totalDeposit[_pid];
         uint reward = percent*totalSupply[_pid]/100;
    ...
    function pendingReward(uint _type,uint256 _pid, address _user) public isLock returns(uint256) {
         require(_type == 1 || _type == 2,"Invalid type");
         user storage userInfo = users[_user][_pid];
         address stake = stakeAddress[_type];
         uint256 amt = Masterchef(stake).pendingCake(_pid,address(this));
         uint256 percent = userInfo.depositAmount*100/totalDeposit[_pid];
         uint256 reward = percent*amt/100;
         return reward;
    }
