    ...
    ERC721PausableUpgradeable
{
    ...
    function initialize() public initializer {
        ...
        _setBaseURI("localhost/");
    }

    function getTokenInfo(uint256 tokenId) public view returns (uint256, uint256, address) {
        BuniBounty memory tokenInfo = tokens[tokenId];
        return (tokenInfo.hashRate, tokenInfo.createdAt, tokenInfo.stakedAt);
    }
    ...
    function getTokenInfoOfOwnerByIndex(address owner, uint256 index) public view returns (uint256, uint256, address, uint256) {
        uint256 tokenId = tokenOfOwnerByIndex(owner, index);
        (uint256 hashRate, uint256 createdAt, address stakedAt) = getTokenInfo(tokenId);
        return (hashRate, createdAt, stakedAt, tokenId);
    }
}
