...
contract MultiRewards is ReentrancyGuard, Pausable {
    ...
    mapping(address => uint256[]) public rewardsForStake;
    ...
    function earned_for_stake(address account, address _rewardsToken, uint256 stakeI) public view returns (uint256) {
        return userStakes[account][stakeI].remainingBalance.mul(rewardPerToken(_rewardsToken).sub(userRewardPerTokenPaid[account][_rewardsToken])).div(1e18).add(rewardsForStake[account][stakeI]);
    }
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        for (uint i; i < rewardTokens.length; i++) {
            ...
            for (uint j; j < userStakes[msg.sender].length; j++) {
                ...
                if (outAmount > 0) {
    	   	        rewardsForStake[msg.sender][j] = 0;
    		        actualAmount = actualAmount.add(outAmount);
                }
            }
    ...
    function collectRemainingReward(address _rewardsToken) external updateReward(address(0)) {
        require(rewardData[_rewardsToken].rewardsDistributor == msg.sender);
    ...
    modifier updateReward(address account) {
        for (uint i; i < rewardTokens.length; i++) {
            ...
            if (_totalSupply == 0)
                rewards[rewardData[token].rewardsDistributor][token].add(lastTimeRewardApplicable(token).sub(rewardData[token].lastUpdateTime).mul(rewardData[token].rewardRate));
            ...
            if (account != address(0)) {
                ...
                for (uint j; j < userStakes[account].length; j++) {
                    rewardsForStake[account][j] = earned_for_stake(account, token, j);
