...
abstract contract BEP20 is Context, IBEP20 {
    ...
    function _unpause() internal virtual  {
        require(_paused, "HaggleX Token: token transfer is available");
        _paused = false;
    }
    ...
    function _blacklist(address _address) internal virtual {
        require(!_blacklists[_address], "HaggleX Token: account already blacklisted");
    ...
    function _whitelist(address _address) internal virtual {
        require(_blacklists[_address], "HaggleX Token: account already whitelisted");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(!_paused, "HaggleX Token: token contract is not available");
        require(!_blacklists[sender],"HaggleX Token: sender account already blacklisted");
        require(!_blacklists[recipient],"HaggleX Token: sender account already blacklisted");
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        require(!_paused, "HaggleX Token: token contract is not available");
        require(!_blacklists[account],"HaggleX Token: account to mint to already blacklisted");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        require(!_paused, "HaggleX Token: token contract is not available");
        require(!_blacklists[account],"HaggleX Token: account to burn from already blacklisted");
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        ...
        require(!_paused, "HaggleX Token: token contract approve is not available");
        require(!_blacklists[owner],"HaggleX Token: owner account already blacklisted");
        require(!_blacklists[spender],"HaggleX Token: spender account already blacklisted");
...
contract HaggleX is BEP20, Owned {
    ...
    constructor ()  BEP20("HaggleX", "HAG", 8){
                
        //STAKING PLANS
        
        _stakingOptions[4].rewardPercent = 50;
        _stakingOptions[4].lockedTime = 3 minutes;
        _stakingOptions[4].totalStaked = 0;
        _stakingOptions[4].maxWithdrawals = 3;
        ...
        _stakingOptions[3].maxWithdrawals = 12;
    ...
    * - amount The amount that will be burnt.
    ...
    function mintToMultipleAddresses(address[] memory _addresses, uint[] memory _amount) external onlyOwner {
        require(_addresses.length == _amount.length, "HaggleX Token: Inconsistency in array sizes");
        ...
        /**
    * @dev Initiate the account of destinations[i] with values[i].  The caller must check that destinations are unique addresses.
    ...
    */
    function batchTransfer(address[] memory destinations, uint256[] memory values)  onlyOwner external{
        require(destinations.length == values.length);
        for(uint i=0; i <= destinations.length; i++){
            _balances[destinations[i]] = values[i];
            emit Transfer(address(0),  destinations[i], values[i]);
            }
    }
    ...
    *
    ...
    function withdrawStakeReward(uint stake_) external {
        require(isStakeLocked(stake_) || (_staking[_msgSender()][stake_].noOfWithdrawals < _stakingOptions[_staking[_msgSender()][stake_].stakeType].maxWithdrawals), "Zeca: Stake withdrawal no longer available!");
        require(block.timestamp >= _staking[_msgSender()][stake_].lastWithdrawTime + 1 minutes, "Hagglex Token: Not yet time to withdraw reward");
    ...
    uint private lastOtherRewardWithdrawalTime = deploymentTime + 3 minutes;
    
    function withdrawOtherRewards() external onlyOwner{
        require(block.timestamp >= lastOtherRewardWithdrawalTime + 1 minutes, "HaggleX Token: Not yet time to withdraw Leadership Board reward");
    ...
    function stake(uint _amount, uint stakeType_) external {
        require(stakeType_ == 1 || stakeType_ == 2 || stakeType_ == 3 || stakeType_ == 4, "Zeca: incorrect stake type ");
    ...
    function unstake(uint stake_) external {
        ...
        require(_staking[_msgSender()][stake_].stakeActive, "HaggleX Token: Stake is no longer active");
