    ...
    AccessControlUpgradeable
{
    ...
    {
        require(listedTokenTypes.contains(address(_tokenAddress)), "Invalid token type");
        uint256 totalTax;

        for (uint256 i = 0; i < _ids.length; i++) {
            uint256 currentId = _ids[i];
            if (!listedTokenIDs[address(_tokenAddress)].contains(currentId)) {
                continue;
            }

            uint256 sellerPrice = listings[address(_tokenAddress)][currentId].price;
