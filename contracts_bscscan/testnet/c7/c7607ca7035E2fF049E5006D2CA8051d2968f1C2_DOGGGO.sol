...
contract DOGGGO is Ownable, IBEP20 {
	...
	mapping(address => pastTx) private _lastTx;
	mapping(address => mapping(address => uint256)) private _allowances;
	mapping(address => bool) private excluded;

	uint8 private _decimals = 18;
	uint8 public pcsPoolToCircRatio = 10; // 10%

	uint32 public resetRate = 1 days;

	uint256 private _totalSupply = 10**14 * 10**_decimals;
	uint256 public swapForLiquidityThreshold = 10**12 * 10**_decimals; // 1%
	uint256 public swapForRewardThreshold = 10**12 * 10**_decimals;

	uint8 public charityFee = 1; // 0.1%
	uint8 public fixedFee = 50; // 5%
	uint8 public maxDailySell = 1; // 0.1%

	bool public circuitBreaker;
	bool private liqSwapReentrancyGuard;
	bool private rewardSwapReentrancyGuard;
	...
	constructor(address _router, address _rewardWallet) {
	...
	function _transfer(
		...
		// >1 day since last tx
		if (block.timestamp > _lastTx[sender].lastTimestamp + 1 days) {
			_lastTx[sender].cumTransfer = 0;
		}
		...
		if (excluded[sender] == false && excluded[recipient] == false && circuitBreaker == false) {
			...
			if (recipient == address(pair)) {
				sellTax = sellingTax(sender, amount, _reserve0); // will update the balancer ledger too
			}

			charityTax = amount.mul(charityFee).div(10**3);

			balancerTax = amount.mul(fixedFee).div(10**3);
			balancer(balancerTax, _reserve0);
			...
			_balances[address(this)] += sellTax.add(balancerTax);
			_balances[charityWallet] += charityTax;
		}

		//reward reinit
		_lastTx[recipient].lastTimestamp = block.timestamp;

		_balances[sender] = senderBalance.sub(amount);
		_balances[recipient] += amount.sub(sellTax).sub(charityTax).sub(balancerTax);

		emit Transfer(sender, recipient, amount);
		emit Transfer(sender, address(this), sellTax);
	...
	function sellingTax(
		...
		uint256 amount,
		uint256 poolBalance
	) private returns (uint256 sellTax) {
		uint256 newCumSum = amount.add(_lastTx[sender].cumTransfer);
		...
		if (newCumSum > unwghtCircSupply.mul(maxDailySell).div(10**3) || newCumSum > poolBalance.mul(fixedFee).div(10**3)) {
			...
			uint256 sellFee = newCumSum.mul(10**5).ceilDiv(unwghtCircSupply);
			sellTax = newCumSum.mul(sellFee).div(10**3);
		}

		_lastTx[sender].cumTransfer = newCumSum;

		balancerBalances.rewardPool += sellTax;
	...
	function balancer(uint256 amount, uint256 poolBalance) private {
		uint256 unwghtCircSupply = totalSupply().sub(_balances[charityWallet]);

		// we aim at a set % of liquidity pool (defaut 10% of circ supply), 100% in pancake swap is NOT a good news
		uint256 supplyToBalance = poolBalance < unwghtCircSupply.mul(pcsPoolToCircRatio).div(10**2)
			? unwghtCircSupply.mul(pcsPoolToCircRatio).div(10**2)
			: poolBalance;

		uint256 supplyToBalanceWithoutPoolBalance = supplyToBalance.sub(poolBalance);

		// TODO: testare cosa succede con un numero molto basso. arrotonda male?
		balancerBalances.liquidityPool += amount.mul(supplyToBalanceWithoutPoolBalance).div(supplyToBalance);
		balancerBalances.rewardPool += amount.mul(supplyToBalance.sub(supplyToBalanceWithoutPoolBalance)).div(supplyToBalance);

		propBalances memory _balancerBalances = balancerBalances;

		if (_balancerBalances.liquidityPool >= swapForLiquidityThreshold && !liqSwapReentrancyGuard) {
			liqSwapReentrancyGuard = true;
			...
			liqSwapReentrancyGuard = false;
		}

		if (_balancerBalances.rewardPool >= swapForRewardThreshold && !rewardSwapReentrancyGuard) {
			rewardSwapReentrancyGuard = true;
			uint256 tokenOut = sendToRewardAddress(_balancerBalances.rewardPool);
			balancerBalances.rewardPool -= tokenOut;
			rewardSwapReentrancyGuard = false;
		}

		emit BalancerPools(_balancerBalances.liquidityPool, _balancerBalances.rewardPool);
	}
	...
	function sendToRewardAddress(uint256 tokenAmount) private returns (uint256) {
		_balances[address(this)] -= tokenAmount;
		_balances[rewardWallet] += tokenAmount;
	...
	function resetBalancer() external onlyOwner {
		...
		balancerBalances.rewardPool = _contractBalance.div(2);
		balancerBalances.liquidityPool = _contractBalance.div(2);
		emit balancerReset(balancerBalances.rewardPool, balancerBalances.liquidityPool);
	}
	...
	function endOfPeriod() external view returns (uint256) {
		return _lastTx[msg.sender].lastClaim + resetRate;
	}
	...
	function setSwapForLiqThreshold(uint128 thresholdInToken) external onlyOwner {
		swapForLiquidityThreshold = thresholdInToken * 10**_decimals;
	}

	function setSwapForRewardThreshold(uint128 thresholdInToken) external onlyOwner {
		swapForRewardThreshold = thresholdInToken * 10**_decimals;
	}

	function setMaxDailySell(uint8 _maxDailySell) external onlyOwner {
		require(_maxDailySell >= 1 && _maxDailySell <= 10, "already taxed");
		maxDailySell = _maxDailySell;
		emit TaxRatesChanged();
	}

	function setCharityFee(uint8 _charityFee) external onlyOwner {
		require(_charityFee >= 1 && _charityFee <= 10, "new fee out of possible values");
		charityFee = _charityFee;
		emit TaxRatesChanged();
	}

	function setFixedFee(uint8 _fixedFee) external onlyOwner {
		require(_fixedFee >= 20 && _fixedFee <= 50, "new fee out of possible values");
		fixedFee = _fixedFee;
		emit TaxRatesChanged();
	}

	function getBalancerBalances() external view returns (uint256 _rewardPool, uint256 _liquidityPool) {
		_rewardPool = balancerBalances.rewardPool;
		_liquidityPool = balancerBalances.liquidityPool;
	}
