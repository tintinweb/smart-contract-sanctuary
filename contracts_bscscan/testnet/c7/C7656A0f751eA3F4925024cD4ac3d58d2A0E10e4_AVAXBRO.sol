...
contract AVAXBRO is ERC721URIStorage, Ownable {
    ...
    Counters.Counter private _tokenIds;
     ...
     event Minted(address Holder,uint256 id);
    /// Boom... Let's go!
    constructor() ERC721("AVAX BRO", "BRO") {
     ...
     */
    function mintTokens(string memory URI) public payable {
        require(saleIsActive, "Sale must be active");
        require(totalSupply() < maxSupply, "Not enough tokens left");
        require(tokenPrice == msg.value, "Ether amount not correct");
        require(balanceOf(msg.sender) <= 10,"Max buy limit Reached" );
        if(totalSupply() >= 100){    
            uint256 Times = totalSupply().div(100);
            tokenPrice = Times.mul(100000000000000000);
            tokenPrice = tokenPrice.add(100000000000000000);
        }
     ...
     */
    function _mintTokens(string memory URI) private {
            _tokenIds.increment();
