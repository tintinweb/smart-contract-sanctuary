 ...
 */
contract Windmill_Royalty is ERC20, AccessControlEnumerable {
    /**
     * DAO_ROLE is able to grant and revoke roles. It can be used when the DAO
     * vote to change some contracts of Windmill.
     */
    ...
    /**
     * @notice Update the address of the PWR token.
     */
    ...
    function migrateTo(Windmill_Royalty newRoyalty) external onlyRole(DAO_ROLE){
        for (uint256 i=1; i<holders.length; i++){
            address addr = holders[i];
            uint256 ROYBalance = balanceOf(addr);
