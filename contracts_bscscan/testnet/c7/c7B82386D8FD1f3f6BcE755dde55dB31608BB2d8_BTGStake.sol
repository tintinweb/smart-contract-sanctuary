...
contract BTGStake is Initializable {
    ...
    function Claim(uint8 index) external {
        User storage user = users[msg.sender];
        uint256 totalAmount=0;
        uint256 finish = user.deposits[index].start.add(plans[user.deposits[index].plan].time);
        bool alreadyClaimed = user.deposits[index].isClaimed;
        bool alreadyWithdraw = user.deposits[index].isWithdraw;
		if (block.timestamp >= finish && !alreadyClaimed &&!alreadyWithdraw) {
			uint256  share= user.deposits[index].amount.mul(plans[user.deposits[index].plan].percent).div(100);
			totalAmount=totalAmount.add(share).add(user.deposits[index].amount);
            user.deposits[index].isClaimed=true;
		}
        require(totalAmount > 0, "User has no dividends");
		user.checkpoint = block.timestamp;
		user.withdrawn = user.withdrawn.add(totalAmount);
