...
import "@geometrydev/lib/contracts/math/SafeMath.sol";
import "@geometrydev/lib/contracts/token/BEP20/IBEP20.sol";
import "@geometrydev/lib/contracts/token/BEP20/SafeBEP20.sol";
import "@geometrydev/lib/contracts/access/Ownable.sol";
...
// MasterCat is the master of GXO AND CATNIP.
// He can make GXO and he is a fair guy.
//
...
contract MasterCat is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of GXO
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. GXO to distribute per block.
        uint256 lastRewardBlock; // Last block number that GXO distribution occurs.
        uint256 accGXOPerShare; // Accumulated GXO per share, times 1e12. See below.
    }
    ...
    // View function to see pending GXO on frontend.
    ...
    function updatePool(uint256 _pid) public validatePool(_pid) {
        ...
        uint256 gxoReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        gxo.mint(devAddress, gxoReward.div(10));
        gxo.mint(address(catnip), gxoReward);
    ...
    // Deposit LP tokens to MasterCat for GXO allocation.
    ...
    // Withdraw LP tokens from MasterCat.
    ...
    function enterStaking(uint256 _amount) public {
        ...
        catnip.mint(msg.sender, _amount);
    ...
    function leaveStaking(uint256 _amount) public {
        ...
        require(
            user.amount >= _amount,
            "not enough to withdraw: I'm feline sad"
        );
        updatePool(0);
        uint256 pending = user.amount.mul(pool.accGXOPerShare).div(1e12).sub(
            user.rewardDebt
        ...
        catnip.burn(msg.sender, _amount);
        ...
        )
    {
        return (
            ...
            poolInfo[_pid].accGXOPerShare
        );
    }

    // Safe gxo transfer function, just in case if rounding error causes pool to not have enough GXO.
    function safeGXOTransfer(address _to, uint256 _amount) internal {
        catnip.safeGXOTransfer(_to, _amount);
    }
    ...
    function dev(address _devaddr) public {
        require(
            ...
            "onlyAdmin: remember to stay paw-sitive!"
        );
        devAddress = _devaddr;
    }
}
...
import "@geometrydev/lib/contracts/token/BEP20/BEP20.sol";

// GXOToken with Governance.
contract GXOToken is BEP20("Geometry", "GXO") {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterCat).
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying GXO (not scaled);
...
// Catnip with Governance.
contract CatnipToken is BEP20('Catnip', 'CATNIP') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterCat).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    // Safe gxo transfer function, just in case if rounding error causes pool to not have enough GXO.
    function safeGXOTransfer(address _to, uint256 _amount) public onlyOwner {
