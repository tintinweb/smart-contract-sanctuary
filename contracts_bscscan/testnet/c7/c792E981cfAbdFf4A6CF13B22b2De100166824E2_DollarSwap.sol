 ...
 **/
contract DollarSwap is IDollarSwap, AccessControlEnumerable, ReentrancyGuard {
	...
	// The path to get price
	address[] public dollarToRewardPath;
	...
	constructor(
		...
		address[] memory _dollarToRewardPath
	) {
		...
		rewardToken = IERC20Metadata(
			address(_dollarToRewardPath[_dollarToRewardPath.length - 1])
		);
		dollarToRewardPath = _dollarToRewardPath;
	...
	function swap(uint256 _dollarAmount) external override nonReentrant {
		...
		IBasisAsset(dollarToRewardPath[0]).burnFrom(
			_msgSender(),
			_dollarAmount
		);
		_updateRewardFund();
