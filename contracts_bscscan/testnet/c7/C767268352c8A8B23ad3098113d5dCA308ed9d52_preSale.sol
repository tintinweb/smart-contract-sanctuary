...
contract preSale {
    ...
    function buyToken() public payable allowed{
        ...
        require(amountRaised.add(msg.value) <= hardCap,"PRESALE: Hardcap reached");
        ...
        if(saleType == 1){
            if(amountRaised >= softCap && voteUp > voteDown){
                if(!profitClaim){
                    ...
                    uint256 refundToken = getContractTokenBalance().sub(soldTokens).sub(totalTokenForVesting);
                    if(refundToken > 0)
                        token.transfer(tokenOwner, refundToken); 
                    uint256 remainingCoin = getContractcoinBalance().sub(totalCoinForVesting).sub(totalCoinForVesting);
                    if(remainingCoin > 0)
