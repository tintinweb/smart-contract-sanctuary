...
/**
  TubbyBoi.com  Telgram : @tubbyboitoken
  ...
  prizes. Anyone can donate TubbyBoi to 0xdead to get 2 entries for themselves
...
contract TubbyBoi is Context, IERC20, Ownable, VRFConsumerBase {
    ...
    uint256 private _reflectionFee = 10;
    ...
    function _getFees(uint256 amount) private view returns (uint256, uint256) {
        uint256 reflectionFee = amount.mul(_reflectionFee).div(10**2);
        uint256 amountMinusFees = amount.sub(reflectionFee);
        return (amountMinusFees, reflectionFee);
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        for(uint256 idx = randomIndex; idx < numAddresses; idx++){
            address randomRecipient = _allReflectionAddresses[idx];
            if (
    ...
    function _grantReflectionEntry(address recipient) private {
        ...
        // give ~1% win rate to marketing address
    ...
    function getTimeUntilNextJackpot() public view returns (uint256) {
        (lotteryLastRun + 1 days) - block.timestamp;
    }
    ...
    function _tokenTransfer(address from, address to, uint256 amount) private {
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            (uint256 amountMinusFees, uint256 reflectionFee) = _getFees(amount);
            ...
            */
            ...
            if (block.timestamp >= lotteryLastRun + 30 seconds) {
                lockThePayout = false;
            }
