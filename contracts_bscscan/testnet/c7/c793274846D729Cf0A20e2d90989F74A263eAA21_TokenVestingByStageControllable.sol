...
contract TokenVestingByStageControllable is Ownable, ReentrancyGuard {
    ...
    /**
     * Early withdrawal when trouble happened can only raise by current owner
     */
    event EarlyWithdrawal(uint256 amount, uint256 timestamp);
    ...
    /**
     * early withdraw tokens when trouble happened.
     */
    function earlyWithdrawal() public onlyOwner {
        // get all remaining token
        ...
        if (tokensToSend > 0) {
            ...
            // Raising event
            emit EarlyWithdrawal(tokensToSend, block.timestamp);
        }
    }
