...
contract BlocksSpace2 is Ownable {
    ...
    BlocksLottery lotteryContract;
    uint256 rollFrequencyInSeconds = 24 hours;
    uint256 lastRollBlock = block.number;
...
contract BlocksLottery is VRFConsumerBase, Ownable {
    ...
    event NoWinnerSelected(uint256 stage, uint256 randomness);
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        uint256 stage;
        address winner = space.getOwnerOfBlock(randomness % 902);
        // Select winner that is not blacklisted (3 tries)
        if(blacklist[winner]){
            winner = space.getOwnerOfBlock((randomness + 240) % 902);
            stage = 1;
            if(blacklist[winner]){
                winner = space.getOwnerOfBlock((randomness + 420) % 902);
                stage = 2;
            }
        }
        ...
        if (blacklist[winner] == false) {
            uint256 tokensWon = safeBlsTransfer(winner, amountOfTokenWin);
            emit WinnerChosen(winner, tokensWon, stage, randomness);
        }else{
