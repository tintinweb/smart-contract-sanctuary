...
contract MowaOpenBox is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => MowaBox) mowaBoxs;
    ...
    constructor(
        ...
        mowaBoxs[24] = MowaBox(24,1,0,0,1,0,0,0);
    }

    function getWhitelist(address _user) public view returns (bool wl) {
    ...
    function checkBeforeClaim(address _user) public view returns (bool wl) {
        if (whiteList[_user] > 0 && opened[_user] == false) {
            return true;
        }
        return false;
    }
    ...
    function claimNFT(uint256 character) public payable nonReentrant {
        uint256 tokenId = mowaItemsCore.getTokenWithCharacter(character);
        if (tokenId == 0) {
            tokenId = mowaItemsCore.getNextNFTId();
        }
        mowaItemsCore.safeMintNFT(_msgSender(), tokenId, 20);
    ...
    function claimMuiltiNFT() public payable nonReentrant {

        uint256[] memory tokenIds = new uint256[](3);
        uint256[] memory amount = new uint256[](3);

        uint256 tokenId = mowaItemsCore.getNextNFTId();

        for (uint i = 0; i < 3; i++) {
            uint256 tokenIdn = mowaItemsCore.getTokenWithCharacter(i+1);
            if (tokenIdn == 0) {
        ...
        mowaItemsCore.safeBatchMintNFT(_msgSender(), tokenIds, amount);
    }

    function burn(uint256 tokenId, uint256 amount) public {
        mowaItemsCore.burnNFT(_msgSender(), tokenId, amount);
    }

    function burnMulti(uint256[] memory tokenId, uint256[] memory amount) public {
        mowaItemsCore.burnBatchNFT(_msgSender(), tokenId, amount);
    }

    function tranfer(address to, uint256 id, uint256 amount) public {
        mowaItemsNFT.safeTransferFrom(_msgSender(), to, id, amount,"0x0");
    }

    function tranferct(uint256 id, uint256 amount) public {
        mowaItemsNFT.safeTransferFrom(_msgSender(), address(this), id, amount, "0x0");
    }

    function tranfernt(uint256 id, uint256 amount) public {
        mowaItemsNFT.safeTransferFrom(address(this), _msgSender(), id, amount, "0x0");
    }

    function tranferMuilti(address from, address to, uint256[] memory tokenId, uint256[] memory amount) public {
        mowaItemsNFT.safeBatchTransferFrom(from, to, tokenId, amount, "0x0");
    }
    ...
    function addWhiteList(address[] memory newWhiteList, uint256[] memory number) public onlyDev {
        for (uint256 index = 0; index < newWhiteList.length; index++) {
            whiteList[newWhiteList[index]] = number[index];
        }
    }

    function removeWhiteList(address[] memory removedWhiteList) public {
        for (uint256 index; index < removedWhiteList.length; index++) {
            whiteList[removedWhiteList[index]] = 0;
        }
    }

//    function removeClaim(address[] memory removeClaimList) public {
//        for (uint256 index; index < removeClaimList.length; index++) {
//            opened[removeClaimList[index]] = false;
//        }
//    }
}
