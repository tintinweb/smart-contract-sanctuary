...
abstract contract DepreciatingFees {
    ...
    */
    ...
    uint256 _MarketingChestFee, uint256 _MutinyFee) internal view returns (uint256, uint256, uint256, uint256) {
        ...
        (uint256 getFleetRewardsFee) = formula(_FleetRewardsFee, currentBalance, _amount );
        (uint256 getWarChestFee) = formula( _WarChestFee, currentBalance, _amount );
        (uint256 getMarketingChestFee) = formula(_MarketingChestFee, currentBalance, __amount );
        (uint256 getMutinyFee) = formula(_MutinyFee, currentBalance, __amount );
    ...
    function formula(uint256 currentFee, uint256 currentBalance, uint256 tokensPurchased) private pure returns(uint256) {
        return  ((currentFee * currentBalance) + (30000000000 * tokensPurchased) )/ (currentBalance + tokensPurchased);
    }
    ...
    function reAdjustFees(address userAddress, uint256 _amount, uint256 accruedTime, uint256 _FleetRewardsFee, uint256 _WarChestFee,  uint256 _MarketingChestFee, 
        ...
        (uint256 _newFleetRewardsFee, uint256 _newMarketingChestFee, uint256 _newWarChestFee, uint256 _newMutinyFee) = 
            calculateBuyingReadjustment (userAddress, _amount, _FleetRewardsFee, _MarketingChestFee, _WarChestFee, _MutinyFee);
...
contract ARMADA is IERC20Metadata, DepreciatingFees, Ownable {
    ...
    string constant _symbol = "CHECKARMD";
