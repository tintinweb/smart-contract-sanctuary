...
contract Fund is Crawler, ICrawler {
    ...
    function executeExecutor() onlyOwner public {
        recentReqId = iterateRequestId();
        crawlerQuery(recentReqId, BridgeExecutor, __encodedIPFS);
    }
    ...
    function getExecutorInfo() external view returns(string memory) {
        return string(__encodedExecutorMessage);
    }
