...
contract Smartchef is Ownable {
    ...
    uint256 public constant LOCK_PERIOD = 30 minutes; // 1 year
    ...
    function withdraw(uint256 _amount) public {
        ...
        require(block.timestamp > user.depositTime + LOCK_PERIOD, "withdraw: Can not withdraw in the Lock Period");
    ...
    function compound(uint256 _amount) private view returns (uint256) {
        UserInfo storage user = userInfo[msg.sender];

        uint256 compoundAmount = _amount.mul(compoundPercentage).div(10000);
        user.amount.add(compoundAmount);
