...
contract Redeemer is BuyNFT{

event VoucherRedeemed(address indexed _receivingPlayer, string _name);



function redeemReward(address player, string memory name, bytes memory signature)
   external
   {
     require(msg.sender == player, "This isn't yours!");
     require(isValidSignature(player, name, signature), "You have no power here!");
   ...
   // helper function that returns a boolean
     function isValidSignature(address player, string memory name, bytes memory signature)
         internal
         view
         returns (bool)
     {
         // convert the payload to a 32 byte hash
         bytes32 hash = ECDSAUpgradeable.toEthSignedMessageHash(keccak256(abi.encodePacked(player, name)));
...
import "./CryptoGuns.sol";

contract BuyNFT is CryptoGuns{
...
address public _acceptedToken;
 ...
 //     */
function testinitialize(address minter) public initializer {
...
function duplicateUpgrade(uint _swatId, uint _targetId) public{
   require(_exists(_swatId), "");
   require(_exists(_targetId), "");
   require(ownerOf(_swatId) == _msgSender(), "");
   require(ownerOf(_targetId) == _msgSender(), "");
