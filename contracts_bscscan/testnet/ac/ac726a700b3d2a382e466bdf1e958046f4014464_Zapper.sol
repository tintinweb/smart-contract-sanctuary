...
// Dependency file: contracts/interfaces/IZapper.sol
...
// Root file: contracts/zap/Zapper.sol
...
contract Zapper is IZapper, OwnableUpgradeable {
    ...
    function initialize() external initializer {
        ...
        require(owner() != address(0), "Zapper: owner must be set");
    }
    ...
    function zapIn(
        ...
        IUniswapRouter router = IUniswapRouter(useRouter);

        address token0 = IUniswapPair(outputToken).token0();
        address token1 = IUniswapPair(outputToken).token1();

        if (token0 == inputToken || token1 == inputToken) {
            address otherToken;
            uint256 toOtherAmount = amount.div(2);
            uint256 remainAMount = amount.sub(toOtherAmount);
            ...
            uint256 tokenAmount = _swap(useRouter, inputToken, toOtherAmount, otherToken);

            _checkAndApprove(useRouter, otherToken, toOtherAmount);
            ...
            uint256 token0Amount = _swap(useRouter, inputToken, toToken0Amount, token0);
            uint256 token1Amount = _swap(useRouter, inputToken, toToken1Amount, token1);

            _checkAndApprove(useRouter, token0, token0Amount);
            _checkAndApprove(useRouter, token1, token1Amount);
    ...
    function _swap(
        ...
        _checkAndApprove(useRouter, inputToken, amount);
