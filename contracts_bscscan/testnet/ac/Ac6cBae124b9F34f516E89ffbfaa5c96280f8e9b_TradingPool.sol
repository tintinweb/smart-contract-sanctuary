...
contract TradingPool is Ownable{
    ...
    uint256 private constant ACC_PAN_PRECISION = 1e15;
    uint256 private constant ORACLE_PRECISION = 1e6;
    uint256 public MAX_BLOCK = 2400;
    ...
    function pendingReward(address _pair, address _account) public view returns (uint256 _pending) {
        PoolInfo memory _pool = poolInfo[_pair];
        ...
        if (_deltaBlock < MAX_BLOCK) {
            uint256 _cur = _user.amount.mul(MAX_BLOCK - _deltaBlock) / MAX_BLOCK;
            _pending = (_cur.add(_user.amount) / 2).mul(_accRewardPerShare.sub(_user.debt)) / ACC_PAN_PRECISION;
        } else {
            _pending = (_user.amount / 2).mul(_accRewardPerShare.sub(_user.debt)) / ACC_PAN_PRECISION;
...
//        }
        if (addedPairs[_pair]) {
            ...
            _user.debt = _pool.accRewardPerShare;
            ...
            _user.amount = _curAmount.add(_amount);
    ...
    function setMaxBlock(uint256 _newMaxBlock) external onlyOwner {
        MAX_BLOCK = _newMaxBlock;
    }
}
