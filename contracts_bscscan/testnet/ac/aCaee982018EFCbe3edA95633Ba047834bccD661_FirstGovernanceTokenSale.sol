 ...
 */
contract FirstGovernanceTokenSale {
    ...
    function _getPrice(uint256 amount) private view returns(uint256) {
        return amount
        .div(1e18)
        .mul(tokenSaleCost.div(1e18))
        .mul(1e18);
    }
    ...
    function getAmountInWei(uint256 amount) external view returns(uint256) {
        uint256 amountInWei = amount.mul(1e18);
        return amountInWei;
    }

    function getTokenBalance() external view returns(uint256) {
        uint256 availableToSaleAmount = token.balanceOf(address(this));
        return availableToSaleAmount;
    }

    /*
        @params {amount} in wei
    */
