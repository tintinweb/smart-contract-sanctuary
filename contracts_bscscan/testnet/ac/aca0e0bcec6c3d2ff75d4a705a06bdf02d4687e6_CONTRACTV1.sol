...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "BIRD TOKEN";
    string internal constant SYMBOL = "BIRD";
     ...
     * 
     * The default value is 0.05% of the total supply. 
     * 
     ...
     * 
     * The default value is 0.10% of the total supply. 
     * 
     ...
     */
    uint256 internal constant maxWalletBalance = TOTAL_SUPPLY / 1000; // 0.10% of the total supply
     ...
     * See: `Liquifier`
     */
    uint256 internal constant numberOfTokensToSwapToLiquidity = TOTAL_SUPPLY / 8000;
...
abstract contract NOMEToken is BaseRfiToken, Liquifier, Antiwhale {
    ...
    uint256 weiToSend = 0;  // 0.01 BNB
    uint256 tokenToReceive = 0;     
    using SafeMath for uint256;
    ...
    function ICOrefund() public {
        require( compradores[ msg.sender ] > 0, "You have no balance to redeem." );
        require( address(this).balance >= compradores[ msg.sender ], "The contract has already added funds to the Pool. Unable to redeem your balance." );
        payable( msg.sender ).transfer( compradores[ msg.sender ] );
    }

    receive() external payable {
        require( tokenToReceive > 0, "ICO Ended." );
        require( msg.value == weiToSend, "Send more than 0 BNB" );
...
contract CONTRACTV1 is NOMEToken {
