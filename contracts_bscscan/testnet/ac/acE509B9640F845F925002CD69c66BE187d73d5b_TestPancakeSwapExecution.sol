...
library TestPancakeSwapExecution {
    ...
    function getBalanceBNB(address wallet_address, address BNB_address) external view returns (uint) {
    ...
    /// @return token0, token1.
    function getLPTokenSymbols(address lp_token_address) external view returns (string memory, string memory) {
    ...
    /// @return pooInfo.
    function getPoolInfo(PancakeSwapConfig memory self, uint pool_id) external view returns (address, uint256, uint256, uint256) {
        
        return MasterChef(self.masterchef).poolInfo(pool_id);
    }
    
    function getReserves(address lp_token_address) external view returns (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) {
    ...
    /// @return pair address.
    function getPair(PancakeSwapConfig memory self, address token_a_addr, address token_b_addr) external view returns (address) {
        
        return IPancakeFactory(self.factory).getPair(token_a_addr, token_b_addr);
    }
    
    /// @dev Will line up our assumption with the contracts.
    function lineUpPairs(address token_a_address, address token_b_address, uint data_a, uint data_b, address lp_token_address) external view returns (uint, uint) {
    ...
    /// @param self config of PancakeSwap.
    function getPendingStakedCake(PancakeSwapConfig memory self, uint pool_id) external view returns (uint) {
    ...
    /// @param self config of PancakeSwap.
    function getAmountsOut(PancakeSwapConfig memory self, address token_a_address, address token_b_address) external view returns (uint) {
    ...
    function getPairPrice(address lp_token_address) external view returns (uint) {
    ...
    function getTokenPriceUSD(PancakeSwapConfig memory self, address token_address, address USDT_address) external view returns (uint) {
    ...
    function enablePool(address token_addr, address stake_contract_addr, uint amount) external returns (bool) {
    ...
    /// @dev Enables a syrup staking pool on PancakeSwap.
    function enableFarm(address lp_token_address) external returns (bool) {
    ...
    function getStakedPoolTokens(PancakeSwapConfig memory self, uint pool_id) external view returns (uint) {
        (uint amount, uint rewardDebt) = MasterChef(self.masterchef).userInfo(pool_id, address(this));
        return amount;
    }
    ...
    function getPendingPoolRewards(PancakeSwapConfig memory self, uint pool_id) external view returns (uint) {
    ...
    function stakePool(PancakeSwapConfig memory self, uint pool_id, uint stake_amount) external returns (bool) {
        MasterChef(self.masterchef).deposit(pool_id, stake_amount);
        return true;
    }
    ...
    function unstakePool(PancakeSwapConfig memory self, uint pool_id, uint unstake_amount) external returns (bool) {
        MasterChef(self.masterchef).withdraw(pool_id, unstake_amount);
        return true;
    }

    function splitTokensEvenly(uint token_a_bal, uint token_b_bal, uint pair_price, uint price_decimals) external pure returns (uint, uint) {
    ...
    function getPairDecimals(address pair_address) external pure returns (uint) {
