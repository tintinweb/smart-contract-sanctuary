...
interface IADefiSwapLottery {
     ...
     */
    function buyTickets(uint256 _lotteryId, uint32 _ticketNumber)  external;
...
contract ADefiSwapLottery is ReentrancyGuard, IADefiSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInADefi = 50 ether;
    uint256 public minPriceTicketInADefi = 0.005 ether;

    uint256 public openingTime = 10 minutes;
     ...
     * @dev Callable by users
     */
    function buyTickets(uint256 _lotteryId, uint32 _ticketNumber) // todo
        external
        override
        notContract
        nonReentrant
    {
        ...
        require((_ticketNumber >= 1000000) && (_ticketNumber <= 1999999), "Outside range");

        uint8[] memory arrTicketNumber;
        uint32 tempNumber = _ticketNumber;
        uint8 numberOfDedit = 0;

        // parse ticketNumber to array and check duplicate digit
        for (uint i = 0 ;i < 6; i++){ 
            uint8 number = uint8(tempNumber % 10);
            if (number !=0){
                for(uint8 j =0; j< arrTicketNumber.length ;j++){
                require(arrTicketNumber[j] != number, "Duplicate digit in ticket number");
                   
               }
                numberOfDedit++;
            }
            arrTicketNumber[i] = number;
            tempNumber /=10;
        }
        

        // Calculate number of ADEFI to this contract
        uint256 amountADefiToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInADefi[numberOfDedit],
            1
        );
        ...
        _numberTicketsPerLotteryId[_lotteryId][1111 + (_ticketNumber % 10000)]++;
        _numberTicketsPerLotteryId[_lotteryId][11111 + (_ticketNumber % 100000)]++;
        _numberTicketsPerLotteryId[_lotteryId][111111 + (_ticketNumber % 1000000)]++;
     ...
     * @param _priceTicket price of a ticket (in ADEFI)
