...
contract CollectRoomManager is SafeOwnable, Random {
    ...
    function tokenWithdraw(IERC20 _token, uint _amount) external onlyOwner {
        ...
        _token.safeTransfer(tokenReceiver, _amount);
    ...
    function feeWithdraw(IERC20 _token, uint _amount) external onlyOwner {
        ...
        _token.safeTransfer(feeReceiver, _amount);
    ...
    constructor(IPandoraERC1155 _nftToken, address _receiver, address _feeReceiver, address _rewardReceiver, address _linkAccessor) Random(_linkAccessor) {
    ...
    function add(
        ...
        for (uint i = 0; i < _nftTypes.length; i ++) {
            ...
            emit RoomRange(rid, _nftTypes[i], nftIDs[i], _nftTypes[i], lastEndIndex);
