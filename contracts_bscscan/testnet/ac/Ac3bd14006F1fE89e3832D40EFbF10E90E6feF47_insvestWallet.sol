...
contract insvestWallet is ReentrancyGuard {
   ...
   //agreement  
    function addAgreement(  
     ...
     uint256 minAmount
         ...
         if(_agreements[code].id == 0 && minAmount >= 100 ){
             _agreementCount += 1;
             _agreements[code].id         = _agreementCount;
             _agreements[code].reward     = reward;
             _agreements[code].active     = true; 
  ...
  //function add to wallet 
    function addwallet( 
      ...
      for (uint i = 0; i >= _userSlot[msg.sender]; i++ ){
        if(_wallet[account][i].signedDate == 0 || _wallet[account][i].claimed == true ){ 
           _wallet[account][i].amount = amount;
           _wallet[account][i].code = code;
           _wallet[account][i].signedDate = block.timestamp;
           _wallet[account][i].claimed = false;
           i = _userSlot[msg.sender] + 1;
              confirmation = 1;
          }
       }if(confirmation != 1){
         _userSlot[msg.sender] += 1;
         _wallet[account][_userSlot[msg.sender]].amount = amount;
         _wallet[account][_userSlot[msg.sender]].code = code;
         _wallet[account][_userSlot[msg.sender]].signedDate = block.timestamp;
         _wallet[account][_userSlot[msg.sender]].claimed = false;
      }
  ...
  //stake
   function stake(uint256 _amount,string memory _code) external {
        ...
        addwallet(msg.sender, _amount,_code ); 
    ...
    function withdraw(uint256 slot) external nonReentrant {
             ...
             uint rewardDate = _wallet[msg.sender][slot].signedDate;
            ...
            uint amount =_wallet[msg.sender][slot].amount;
     ...
     function getWallet(address account)public view returns( string memory wallet){
            if(msg.sender == _ownerAddr || msg.sender == account  ){
               wallet = string (abi.encodePacked(wallet,'{', '"', "wallet", '"',':', '['));
               uint rewardDate;
               for (uint i = 0; i >= _userSlot[msg.sender]; i++ ){
                   if(_wallet[account][i].claimed == false ){
                     ...
                     if(i ==  _userSlot[msg.sender] ){
                            wallet = string (abi.encodePacked(
                            wallet,'{','"', 
                            "code",'"',':', '"',_wallet[account][i].code,'"',',','"',
                            "cantidad",'"',':', '"', _wallet[account][i].amount,'"',',','"',
                            "signed",'"',':', '"', _wallet[account][i].signedDate,'"',',','"',
                            "rewardD",'"',':', '"' ,rewardDate,'"' ,                        
                            '}' )); 
                    } else{ 
                            wallet = string (abi.encodePacked(
  ...
  function viewStcs(string memory _code)public view returns( string memory wallet){
            if(msg.sender == _ownerAddr ){
               wallet = string (abi.encodePacked(wallet,'{', '"', "statistics", '"',':', '['));
                    wallet = string (abi.encodePacked(
                            wallet,'{','"', 
                            "code",'"',':', '"',_code,'"',',','"',
                            "cantidad",'"',':', '"',_statistics[_agreements[_code].id] ,'"',',','"',                          
                            '}' ));                                    
              wallet = string (abi.encodePacked(wallet,']','}'));               
             }
     }
