...
/*
  ...
  // Manual 'swapAndLiquify' Trigger (Enter the percent of the tokens that you'd like to send to swap and liquify)
    function _A_Swap_to_BUSD_2 (uint256 percent_Of_Tokens_To_Swap) public onlyOwner {
        ...
        SWAP_TO_BUSD_2(tokenAmount);

    }

    function _A_Swap_to_BUSD_3 (uint256 percent_Of_Tokens_To_Swap) public onlyOwner {
        // Do not trigger if already in swap
        if (percent_Of_Tokens_To_Swap > 100){percent_Of_Tokens_To_Swap == 100;}
        uint256 tokensOnContract = balanceOf(address(this));
        uint256 tokenAmount = tokensOnContract*percent_Of_Tokens_To_Swap/100;
        SWAP_TO_BUSD_3(tokenAmount);
    ...
    function _A_MAKE_LP_BUSD (uint256 percent_Of_Tokens_To_Swap) public onlyOwner {
        ...
        uint256 tokenAmount = tokensOnContract*percent_Of_Tokens_To_Swap/100;
            ...
            addLiquidity_BUSD(tokenAmount, BUSD_BAL);
    ...
    function addLiquidity_BUSD(uint256 tokenAmount, uint256 tokenBUSD) private {
        ...
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        _approve(BUSD_T, address(uniswapV2Router), tokenBUSD);
        _approve(BUSD_T, address(this), tokenBUSD);
        IERC20(BUSD_T).approve(address(this),999999999999999);
        IERC20(BUSD_T).approve(address(uniswapV2Router),999999999999999);
    ...
    function SWAP_TO_BUSD_2(uint256 tokenAmount) private {
    ...
    function SWAP_TO_BUSD_3(uint256 tokenAmount) private {
