...
contract H4GToken is ERC20, Ownable {
    ...
    address public gamePool = 0x2550bcA4990892B694f4A9B5432110F0af433CCb;
    ...
    // swap precentage
    uint256 private devidendSwap = 13;
    uint256 private liquiditySwap = 2;
    uint256 private devSwap = 2;
    uint256 private lotterySwap = 1;
    uint256 private totalSwap = 18;
    ...
    constructor() ERC20("Hodl4Gold", "H4G") {
        ...
        devWallet = 0xBCc77E3B972D3E5BC519a2350D00dc7D74C5cEd3;
        ...
        dividendTracker.excludeFromDividends(gamePool);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(
            ...
            "H4G: The new dividend tracker must be owned by the FLOKIBUSD token contract"
        );
    ...
    function updateBuylFees(
        ...
        buyBurnFee = burn;
        buyGameFee = game;
        buyTotalFees = reward.add(dev).add(liquidity).add(burn).add(game).add(
            lottery
        );
    }
    ...
    function updateSellFees(
        ...
        sellBurnFee = burn;
        sellGameFee = game;
        sellTotalFees = reward.add(dev).add(liquidity).add(burn).add(game).add(
            lottery
        );
    }

    // update swap precentage
    function updateSwapPrecentage(
        ...
        totalSwap = reward.add(lottry).add(liquidity).add(dev);
    }
    ...
    function updateGamePoolAddress(address newAddress) public onlyOwner {
        gamePool = newAddress;
    }
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[to]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 bnbSwap = devSwap.add(lotterySwap);
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to]) {
                ...
                if (sellGameFee > 0) {
                    gameAmount = amount.div(100).mul(sellGameFee);
                }
            } else {
                ...
                if (buyGameFee > 0) {
                    gameAmount = amount.div(100).mul(buyGameFee);
                }
            }
            ...
            // send game pool
            super._transfer(address(this), gamePool, gameAmount);
        }
    ...
    function swapAndSendBnb(uint256 tokenAmount, uint256 totalFee) private {
        ...
        transferToWallet(
            payable(devWallet),
            address(this).balance.div(totalFee).mul(devSwap).div(2)
        );
        transferToWallet(
            payable(lotteryWallet),
            address(this).balance.div(totalFee).mul(lotterySwap)
        );
    }
