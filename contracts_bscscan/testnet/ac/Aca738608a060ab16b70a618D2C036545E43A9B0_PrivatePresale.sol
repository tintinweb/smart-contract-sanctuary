...
contract PrivatePresale {
    ...
    function privateRound() public payable {
        ...
        require(_inPresale[msg.sender].amount + _txCollected <= _walletLimit, "PrivateRound: Wallet limit has been reached");
        _totalCollected += _txCollected;
        _txCount ++;

        _inPresale[msg.sender].isWhiteListed = true;
        _inPresale[msg.sender].amount += _txCollected;
        _inPresale[msg.sender].txCount ++;
        _inPresale[msg.sender].tokenAmount += (_txCollected*10**2)/_privatePrice;

        emit DepositSuccesful(msg.sender, _txCollected, _inPresale[msg.sender].amount);
    }
    ...
    function isWhitelisted(address _address) public view returns (bool){
        if (_inPresale[_address].isWhiteListed == true) {
            return true;
        } else {    
            return false;
        }
    ...
    function claimTokens() public {
        ...
        if (_inPresale[msg.sender].tgeCollect == false) {
            uint256 _amountToCollect = (_inPresale[msg.sender].tokenAmount*10**3)/_tgeToCollect;
            _inPresale[msg.sender].tgeCollect == true;
            _inPresale[msg.sender].newAmount -= _amountToCollect;
            _inPresale[msg.sender].nextCollect += 2 weeks;
            _inPresale[msg.sender].numOfCollectTX ++;

            require(_inPresale[msg.sender].newAmount >= _amountToCollect, "You have not enought tokens left");
            // send tokens

            msn.safeTransfer(msg.sender, _amountToCollect);

        } else {
            require(_inPresale[msg.sender].numOfCollectTX < 8, "Claim Error: You have already claimed all tokens");
            require(_inPresale[msg.sender].tokenAmount > 0,"Claim Error: You have already claimed all tokens");

            if (_inPresale[msg.sender].numOfCollectTX == 1) {
                require(block.timestamp >= _1Collect,"Claim Error: Claim Time has not been reached");
            } else if (_inPresale[msg.sender].numOfCollectTX == 2) {
                require(block.timestamp >= _2Collect,"Claim Error: Claim Time has not been reached");
            } else if (_inPresale[msg.sender].numOfCollectTX == 3) {
                require(block.timestamp >= _3Collect,"Claim Error: Claim Time has not been reached");
            } else if (_inPresale[msg.sender].numOfCollectTX == 4) {
                require(block.timestamp >= _4Collect,"Claim Error: Claim Time has not been reached");
            } else if (_inPresale[msg.sender].numOfCollectTX == 5) {
                require(block.timestamp >= _5Collect,"Claim Error: Claim Time has not been reached");
            } else if (_inPresale[msg.sender].numOfCollectTX == 6) {
                require(block.timestamp >= _6Collect,"Claim Error: Claim Time has not been reached");
            } else if (_inPresale[msg.sender].numOfCollectTX == 7) {
                require(block.timestamp >= _7Collect,"Claim Error: Claim Time has not been reached");
            } else if (_inPresale[msg.sender].numOfCollectTX == 8) {
                require(block.timestamp >= _8Collect,"Claim Error: Claim Time has not been reached");
            }
            uint256 _amountToCollect = (_inPresale[msg.sender].tokenAmount*10**3)/_normalCollect;
            require(_inPresale[msg.sender].newAmount >= _amountToCollect, "You have not enought tokens left");
            _inPresale[msg.sender].newAmount -= _amountToCollect;
             _inPresale[msg.sender].numOfCollectTX ++;
    ...
    function depositAmount(address _address) public view returns (uint256) {
        require(_inPresale[_address].isWhiteListed == true, "PresaleError: Address has not participated");
        return(_inPresale[_address].amount);
    }
