...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  constructor(string memory _name, string memory _symbol, uint8 _decimals) ERC20(_name, _symbol, _decimals) {
    ...
    ammIsWhiteListed[address(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F)] = false; // PCS V1 router
    ammIsWhiteListed[address(0xcF0feBd3f17CEf5b47b0cD257aCf6025c5BFf3b7)] = false; // ApeSwap router
  }
...
contract AURUMFinance is ERC20, Ownable {
    ...
    AURUMDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Aurum", "AUR", 9) {

    	dividendTracker = new AURUMDividendTracker();
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	...
  	function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "AURUM: The dividend tracker already has that address");

        AURUMDividendTracker newDividendTracker = AURUMDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "AURUM: The new dividend tracker must be owned by the AURUM token contract");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "AURUM: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "AURUM: The operations wallet is already this address");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "AURUM: The buyback wallet is already this address");
    ...
    function updateFees(uint256 bnbRewardPerc, uint256 liquidityPerc, uint256 operationsPerc, uint256 buyBackPerc) external onlyOwner {
        require (operationsPerc.add(buyBackPerc) <= liquidityPerc, "AURUM: updateFees:: Liquidity Perc must be equal to or higher than operations and buyback combined.");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "AURUM: gasForProcessing must be between 200,000 and 500,000");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userRawBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "Aurum: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if(liquidityFee > 0){ // dividing by 0 is not fun.
            ...
            if(operationsFee > 0){
                ...
                require(success, "AURUM: SwapAndLiquify:: Unable to send BNB to Operations Wallet");
            }
            ...
            if(buyBackFee > 0){
                ...
                require(success, "AURUM: SwapAndLiquify:: Unable to send BNB to BuyBack Wallet");
            }
...
contract AURUMDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("AURUM_Dividend_Tracker", "AURUM_Dividend_Tracker", 9) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 30 * 10 ** 9 * (10**9); //must hold 30,000,000,000+ tokens to get divs
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "AURUM_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "AURUM_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
