...
/**
 * @title ISO NFTs implemented using the ERC-721 standard.
 * @dev This top level file holds no data directly to ease future upgrades.
 */
contract ISONFT721 is
  ISOTreasuryNode,
  ISOAdminRole,
  ...
  NFT721Mint
{
  ...
  /**
   * @notice Allows a Reconcile admin to update NFT config variables.
   * @dev This must be called right after the initial call to `initialize`.
   */
  function adminUpdateConfig(address _nftMarket, string memory baseURI) public onlyISOAdmin {
...
import "../interfaces/IISONFTMarket.sol";
...
/**
 * @notice Holds a reference to the ISO Market and communicates fees to 3rd party marketplaces.
 */
abstract contract NFT721Market is ISOTreasuryNode, HasSecondarySaleFees, NFT721Creator {
  using AddressUpgradeable for address;

  event NFTMarketUpdated(address indexed nftMarket);

  IISONFTMarket private nftMarket;

  /**
   * @notice Returns the address of the ISO NFTMarket contract.
   */
  ...
  function _updateNFTMarket(address _nftMarket) internal {
    ...
    nftMarket = IISONFTMarket(_nftMarket);
...
abstract contract NFT721Mint is Initializable, ERC721Upgradeable, NFT721Creator, NFT721Market, NFT721Metadata {
  ...
  /**
   * @notice Allows a creator to mint an NFT and set approval for the Reconcile marketplace.
   ...
   */
  function mintAndApproveUser(string memory tokenIPFSPath, address to  ) public returns (uint256 tokenId) {
    tokenId = _mintToken(tokenIPFSPath);
