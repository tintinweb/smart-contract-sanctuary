...
contract BattleHeroChestShop is Context, AccessControlEnumerable {
    ...
    mapping(bytes32 => uint256) _presaleCap;
    mapping(bytes32 => mapping(uint256 => uint256)) _presaleSold;

    mapping(address => mapping(bytes32 => uint256)) _amounts;
    mapping(bytes32 => Chest) _chests;
    ...
    constructor(
        ...
        _presaleStart = block.timestamp; // now for testnet
    }
    ...
    function _createChest(
        ...
        require(msg.sender == _admin, "Only admin can create chests");
        bytes32 chestId = keccak256(
            abi.encodePacked(chestType, chestRarity, chestToken)
        );
        ...
        _chestsIds.push(chestId);
    }

    function mint(bytes32 chestId, uint256 quantity) internal {
        require(msg.sender == _admin, "Only admin can mint chests");
        _mint(_admin, chestId, quantity);
    }
    ...
    function _mint(
        ...
        require(msg.sender == _admin, "Only admin can mint chests");
        require(_chests[chestId].when != 0, "This chest not exists");
        _amounts[to][chestId] = _amounts[to][chestId].add(quantity);
    }

    function burn(bytes32 chestId, uint256 amount) public {
        require(msg.sender == _admin);
        _amounts[_seller][chestId] = _amounts[_seller][chestId].sub(amount);
    }

    function chestsOf(address from, uint256 page)
        public
        view
        returns (uint256[] memory, Chest[] memory)
    {
        uint256 results_per_page = 10;
        uint256 greeter_than = results_per_page * page;
        uint256 start_pointer = (results_per_page * page) - results_per_page;
        uint256 counter = 0;
        Chest[] memory c = new Chest[](results_per_page);
        uint256[] memory a = new uint256[](results_per_page);

        for (uint256 i = start_pointer; i < greeter_than; i++) {
            bytes32 _chestId = _chestsIds[i];
            uint256 _amount = _amounts[from][_chestId];
            if (_amount > 0) {
                c[counter] = _chests[_chestId];
                a[counter] = _amount;
                counter = counter + 1;
            }
        }
    ...
    function buyChest(bytes32 chestId, uint256 amount) public payable {
        require(
            block.timestamp >= (_presaleStart + 86400 * 3),
            "Is already on presale"
        );
        Chest memory chest = _chests[chestId];
        require(chest.when != 0, "Chest not exists");
        _processPurchase(chest);
        _mint(msg.sender, chestId, amount);
    }

    function chestsId() public view returns(bytes32[] memory) { 
        return _chestsIds;
    }

    function buyFirstRound(bytes32 chestId) public payable {
        uint amount = 1;
        require(
            block.timestamp >= _presaleStart &&
            block.timestamp <= _presaleStart + 1 days,
            "Time end"
        );
        require(
            _presaleSold[chestId][0] <= _presaleCap[chestId],
            "All chests sold"
        );
        require(amount <= _presaleCap[chestId], "Max cap reached");
        Chest memory chest = _chests[chestId];
        require(chest.when != 0, "Chest not exists");
        _processPurchase(chest);
        _mint(msg.sender, chestId, amount);
        _presaleSold[chestId][0] = _presaleSold[chestId][0].add(amount);
    }

    function buySecondRound(bytes32 chestId) public payable {
        uint amount = 1;
        require(
            block.timestamp >= (_presaleStart + 1 days) &&
            block.timestamp <= (_presaleStart + 2 days),
            "Time end"
        );
        require(
            _presaleSold[chestId][1] <= _presaleCap[chestId],
            "All chests sold"
        );
        require(amount <= _presaleCap[chestId], "Max cap reached");
        Chest memory chest = _chests[chestId];
        ...
        _processPurchase(chest);
        _mint(msg.sender, chestId, amount);
        _presaleSold[chestId][1] = _presaleSold[chestId][1].add(amount);
    }

    function buyThirdRound(bytes32 chestId) public payable {
        uint amount = 1;
        require(
            block.timestamp >= (_presaleStart + 2 days) &&
            block.timestamp <= (_presaleStart + 3 days),
            "Time end"
        );
        require(
            _presaleSold[chestId][2] <= _presaleCap[chestId],
            "All chests sold"
        );
        require(amount <= _presaleCap[chestId], "Max cap reached");
        Chest memory chest = _chests[chestId];
        ...
        _processPurchase(chest);
        _mint(msg.sender, chestId, amount);
        _presaleSold[chestId][2] = _presaleSold[chestId][2].add(amount);
    }

    function _processPurchase(Chest memory chest) internal {
        if (chest.acceptedToken == ChestToken.BATH) {
            require(
                _erc20.balanceOf(msg.sender) >= chest.chestPrice,
                "Insufficient BATH Balance"
            );
            require(
                _erc20.allowance(msg.sender, address(this)) >= chest.chestPrice,
                "Insufficient BATH Allowance"
            );
            _erc20.burnFrom(msg.sender, chest.chestPrice);
        }
        if (chest.acceptedToken == ChestToken.BNB) {
            require(msg.value >= chest.chestPrice, "Insufficient BATH Balance");
            payable(_seller).transfer(msg.value);
        }
    }

    function openChest(bytes32 chestId) public {
        require(_amounts[msg.sender][chestId] > 0, "No chests left");
        _amounts[msg.sender][chestId] = _amounts[msg.sender][chestId].sub(1);
        Chest memory chest = _chests[chestId];
        if (chest.chestRarity != ChestRarity.RANDOM) {
            if (chest.chestType == ChestType.WEAPON) {
                string memory weapon = _genScience.generateWeapon(
                    chestRaritytoRarity(chest.chestRarity)
                );
                _breeder.breed(msg.sender, weapon);
            ...
            if (chest.chestType == ChestType.CHARACTER) {
                ...
                _breeder.breed(msg.sender, character);
                //emit ChestOpened(index, msg.sender,block.timestamp, tokenIds);
            }
        }
        if (chest.chestRarity == ChestRarity.RANDOM) {
    ...
    function chestRaritytoRarity(ChestRarity chestRarity)
        public
        pure
        returns (IBattleHeroGenScience.Rarity)
    {
    ...
    function getPresaleCaps() public view returns(PresaleCap[] memory){
        require(
            block.timestamp >= _presaleStart &&
            block.timestamp <= _presaleStart + 3 days,
            "Time end"
        );
        bytes32[] memory cId = _chestsIds;
        uint256 len = cId.length;
        PresaleCap[] memory p = new PresaleCap[](len);
        for(uint i = 0; i < len; i++){
            bytes32 chestId = _chestsIds[i];

            uint256 soldDay1 = _presaleSold[chestId][0];
            uint256 soldDay2 = _presaleSold[chestId][1];
            uint256 soldDay3 = _presaleSold[chestId][2];

            uint256 maxCap = _presaleCap[chestId];
            PresaleCap memory cap = PresaleCap(chestId, soldDay1, soldDay2, soldDay3, maxCap);
            p[i] = cap;
        }   
       return p;
    }
