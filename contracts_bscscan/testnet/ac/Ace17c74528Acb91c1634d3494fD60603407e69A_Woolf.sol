...
contract Woolf is IWoolf, ERC721Enumerable, Ownable, Pausable {
  ...
  uint256 public wool_price = 2000000 ether;
  uint256 public weed_price = 4000000 ether;
   ...
   */
   /**
    ...
    2 WEED
    */
  function mint(uint256 amount, bool stake, uint8 tokenType) external payable whenNotPaused {
    ...
    if (totalWeedCost > 0) weed.transferFrom(_msgSender(), address(this), totalWeedCost);
  ...
  function setMintPrice(uint256 _price, uint256 tokenType) external onlyOwner {
      if(tokenType == 0){
        MINT_PRICE = _price;
      }else if(tokenType == 1){
        wool_price = _price;
