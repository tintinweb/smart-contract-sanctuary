...
contract XyroldGame {  
    ...
    event CreateGame(address creator, uint gameId, uint amount);
    event PlayGame(address oponent, address creator, uint gameId);
    event GameWinner(address player, uint gameId, uint amount);
    ...
    function createnewgame(uint8 _round1, uint8 _round2, uint8 _round3) external payable {
        ...
        emit CreateGame(_creator, _game.ID, _game.bet);
    }
    ...
    function playgame(uint256 _gameId, uint8 _round1, uint8 _round2, uint8 _round3) public payable {
        ...
        emit PlayGame(_player, _game.creator, _game.ID);
    ...
    function processgamewinner(uint256 _gameId) internal returns (address) {
        ...
        emit GameWinner(_game.winner,  _game.ID, (_game.bet * 2));
