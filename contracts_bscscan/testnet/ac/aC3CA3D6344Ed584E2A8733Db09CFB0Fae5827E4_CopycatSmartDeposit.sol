...
interface ICopycatLeader is IERC20 {
  ...
  function adapterMintShare(uint256 amount, uint256 adapterId) external;
...
contract CopycatLeaderFactory is CopycatEmergencyMaster {
  ...
  uint256[5] public FEE_LIST = [100 ether, 1000 ether, 10000 ether, 1 ether, 0.3 ether];
...
contract CopycatLeader is ERC20('', ''), Ownable, ICopycatLeader, CopycatEmergency, ReentrancyGuard {
  ...
  event SetDepositCopycatFee(address indexed setter, uint256 oldRate, uint256 newRate);
  function setDepositCopycatFee(uint256 _depositCopycatFee) external onlyOwner {
    require(_depositCopycatFee <= 2000, "L");
    emit SetDepositCopycatFee(msg.sender, depositCopycatFee, _depositCopycatFee);
    depositCopycatFee = _depositCopycatFee;
  }

  event SetDepositPercentageFee(address indexed setter, uint256 oldRate, uint256 newRate);
  function setDepositPercentageFee(uint256 _depositPercentageFee) external onlyOwner {
    require(_depositPercentageFee <= 2000, "L");
    emit SetDepositPercentageFee(msg.sender, depositPercentageFee, _depositPercentageFee);
    depositPercentageFee = _depositPercentageFee;
  }
  ...
  function depositTo(address to, uint256 shareMin) override public payable nonReentrant returns (uint256 totalShare) {
    require(!disabled, "D");
  ...
  function withdrawTo(address to, uint256 amount, uint256 bnbMin) override nonReentrant public returns (uint256 outputAmount) {
  ...
  function toAdapter(uint256 adapterId, uint256 amountBnb, uint256 tokenMin) public override nonReentrant onlyOwner returns(uint256) {
    require(!disabled, "D");
  ...
  function toLeader(uint256 adapterId, uint256 percentage, uint256 bnbMin) public override nonReentrant onlyOwner returns(uint256) {
    require(!disabled, "D");
  ...
  event AdapterMintShare(address indexed adapter, uint256 amount);
  function adapterMintShare(uint256 amount, uint256 adapterId) external nonReentrant override {
    require(address(adapters[adapterId]) == msg.sender, "I");
    _mint(msg.sender, amount);
    emit AdapterMintShare(msg.sender, amount);
  }
