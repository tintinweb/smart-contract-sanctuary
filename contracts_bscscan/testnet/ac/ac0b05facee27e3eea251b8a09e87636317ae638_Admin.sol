...
contract Admin is Initializable, Governance, WJXN_VRP, WJAX_JUSD, JUSD_JINR {
    ...
    function initialize() public initializer {
        ...
        wjax_busd_pair = 0xb00eef3E8112410316A5ea2194c9DdCAAA38927d;
        // JUSD_JINR exchange
...
contract WJXN_VRP is Governance{
    ...
    function exchange_wjxn_vrp(uint wjxn_amount) public isActive returns (uint) {
        ...
        if( vrp.totalSupply() == 0 || wjxn.balanceOf(address(this)) == 0){
            ...
            vrp.totalSupply() / wjxn.balanceOf(address(this));
        }
    ...
    function exchange_vrp_wjxn(uint vrp_amount) public isActive returns (uint) {
        ...
        uint vrp_wjxn_ratio = wjxn.balanceOf(address(this)) * (10 ** vrp.decimals() ) / vrp.totalSupply() * (10 ** vrp.decimals());
        uint wjxn_to_be_withdrawn = (vrp_amount / (10 **vrp.decimals())) * (vrp_wjxn_ratio / (10 ** vrp.decimals()));
...
contract WJAX_JUSD is Governance {
    ...
    address wjax_busd_pair; // = 0xb00eef3E8112410316A5ea2194c9DdCAAA38927d;
	...
	*/
	function exchange_wjax_jusd(uint wjax_amount) public isActive returns (uint) {
        (uint wjax_price, ) = get_wjax_busd_ratio();
        // Calculate fee
        ...
        uint jusd_amount = (wjax_amount - fee_amount) * wjax_price;
	...
	*/
	function exchange_jusd_wjax(uint jusd_amount) public isActive returns (uint) {
        ...
        uint wjax_amount = jusd_amount / wjax_price;
        uint fee_amount = calculate_wjax_jusd_markup_fee(wjax_amount);
   ...
   function get_wjax_busd_ratio() public view returns(uint, uint)
    {
        IPancakeswapV2Pair pair = IPancakeswapV2Pair(wjax_busd_pair);
        (uint Res0, uint Res1,) = pair.getReserves();

        // decimals
        if( pair.token1() == address(wjax) ){
            return (Res0 * (10**wjax.decimals()) /Res1, wjax.decimals());
        }
        return (Res1 * (10**wjax.decimals()) / Res0, wjax.decimals()); // return amount of token0 needed to buy token1
    }
   
    function get_busd_wjax_ratio() public view returns(uint, uint)
    {
        (uint wjax_busd_ratio, uint decimal) = get_wjax_busd_ratio();
        uint busd_wjax_ratio = (10 ** decimal) / wjax_busd_ratio * (10 ** decimal);
        return (busd_wjax_ratio, decimal);
    }
}
...
contract JUSD_JINR is Governance {
    ...
    function convert_jinr_jusd(uint _amount) public isActive returns (uint) {
        uint256 jusd_amount = _amount / (usd_inr_ratio / (10 ** ratio_decimal));
        // Calculate Fee on receiver side
    ...
    function get_usd_inr_ratio() public view returns (uint, uint) {
        return (usd_inr_ratio, ratio_decimal);
    }

    function get_inr_usd_ratio() public view returns (uint, uint) {
        return ((10 ** ratio_decimal / usd_inr_ratio) * (10 ** ratio_decimal), ratio_decimal);
    }

    function set_usd_inr_ratio(uint _ratio) public onlyGovernor {
        require(validate_conversion_ratios(_ratio) == true, "USD-INR Conversion ratio is not valid.");
...
contract Governance {
    ...
    modifier isActive() {
        require(current_system_state == 0, "System has been paused by Admin.");
        _;
    }
    ...
    function validate_conversion_ratios(uint _ratio) public view onlyGovernor returns (bool) {
        return true;
    }
