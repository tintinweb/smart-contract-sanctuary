...
contract CarbonToken is ERC20, Pausable, ERC20Detailed {
    ...
    mapping (uint => uint) private quarterMapProjectOwner;
    address [] private lockedAddressList;
    
	constructor() public ERC20Detailed("Carbon-5", "CO2", 9) {  
        ...
        quarterMapProjectOwner[1]=1654041600;
        ...
        quarterMapProjectOwner[10]=1677628800;
        for(uint i = 1; i<= 10;i++) {
            transferAndLock(projectOwner, 65000000 * 10 ** uint256(decimals()), quarterMapProjectOwner[i]);
        }

        transferAndLock(privateSaleWallet, 25000000 * 10 ** uint256(decimals()), 1666396800);
        transferAndLock(workingTeamWallet, 25000000 * 10 ** uint256(decimals()), 1666396800);
        transferAndLock(boardOfAdvisory, 50000000 * 10 ** uint256(decimals()), 1666396800);
    }
	...
	function transfer(address receiver, uint256 amount) public whenNotPaused returns (bool success) {
     ...
     * @param amount The amount to be transferred.
     */
    function transferFrom(address from, address receiver, uint256 amount) public whenNotPaused returns (bool) {
	...
	function transferAndLock(address receiver, uint256 amount, uint256 releaseDate) public whenNotPaused returns (bool success) {
        ERC20._transfer(msg.sender, receiver, amount);
    	if (lockList[receiver].length==0) lockedAddressList.push(receiver);
    	LockItem memory item = LockItem({amount: amount, releaseDate: releaseDate});
