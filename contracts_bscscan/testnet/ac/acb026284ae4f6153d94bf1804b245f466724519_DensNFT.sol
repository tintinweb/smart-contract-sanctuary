  ...
  Ownable
{
  ...
  uint public constant MAX_UNU = 814;
  ...
  mapping(address => NFTCollection) public nftReveled;

  /**
   * @dev Contract constructor. Sets metadata extension `name` and `symbol`.
   */
  constructor(string memory uri) public {
    nftName = "DENS NFT TEST";
    nftSymbol = "DFT";
  ...
  function changeName(string calldata name, string calldata symbol) external onlyOwner {
    ...
    function addWhiteListUser(address _goodUser) public onlyOwner {
  ...
  function mint( address _to, uint256 _tokenId, string calldata _uri, string calldata _payload) external  {
    // require(isWhiteListed[msg.sender], "User isn't white listed by admin");
   require(idToOwner[_tokenId] == address(0), NFT_ALREADY_EXISTS);
  ...
  function revealNft () public {
        NFTCollection storage nft = nftReveled[msg.sender];
        require(nft.buyer == msg.sender, "invalid buyer");
        
        for (uint i = 0; i < nft.tokensCount; i++) {
               
            super._mint(msg.sender, tokens.length);
            super._setTokenUri(tokens.length, contractURI() );
  ...
  function multiMint(uint256 iteration) external payable {
        NFTCollection storage nft = nftReveled[msg.sender];
        require(nft.buyer != msg.sender, "already bought");
        
        require(tokens.length < MAX_UNU, "Sale end");
        require(iteration <= 20, "Exceeds 20");
        require(tokens.length + iteration <= MAX_UNU, "Max limit");
        require(msg.value >= price(iteration), "Value below price");

        nft.buyer = msg.sender;
        nft.tokensCount = iteration;
  ...
  function randMod(uint _modulus, uint _randNonce) internal returns(uint) {
