...
contract StakingPlanet is IERC721Receiver, Ownable {
        ...
        returns (uint256[] memory)
    {
        ...
        uint256 claimableDays = _getClaimableDays(lastClaimedTimestamp, block.timestamp, period);
        if(period > 0){
        ...
        returns(uint256) 
    {
        if(lastClaimedTimestamp < nowTimestamp){
            uint256 estimate = (nowTimestamp.sub(lastClaimedTimestamp)).div(uint256(86400));
            if(estimate < period){
                return estimate;
            }
            else{
    ...
    function _sendRewardTo(address to) private {
        ...
        for(uint i=0; i<stakedTokens[to].length(); i++){
            ...
            tokenInfos[tokenId].lastClaimedTimestamp = lastClaimed + (_getClaimableDays(lastClaimed, block.timestamp, rewardPools[tokenId].period)).mul(86400);
            computed = _computeUnclaimedRewards(tokenId);
            for(uint j=0; j<rewardsTokenAddresses.length; j++){
                rewards[j] = rewards[j] + computed[j];
                if(computed[j]>0){
