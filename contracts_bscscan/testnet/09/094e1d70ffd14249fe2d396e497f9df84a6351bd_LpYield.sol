...
contract LpYield is Initializable{
    ...
    struct UserInfo {
        ...
        uint256 busdStaked; // how many busd the user has provided
        uint256 average;
        uint256 lastCountBlock;
    }
    uint public totalAverage;
    uint public startBlock;
    uint public lastCountBlock;
    ...
    modifier onlyAdmin() {
        require(jaxAdmin.userIsAdmin(msg.sender), "Only Admin can perform this operation.");
        _;
    }
    ...
    function depositBUSD(uint amount) public {
        ...
        uint[] memory amounts = JaxLibrary.swapWithPriceImpactLimit(address(router), amount_to_buy_wjax, priceImpactLimit, path, address(this));
        ...
        if(wjax_amount - wjax_liquidity > 0)
            amounts = JaxLibrary.swapWithPriceImpactLimit(address(router), wjax_amount - wjax_liquidity, priceImpactLimit, path, msg.sender);

        userInfo[msg.sender].amount += liquidity;
        userInfo[msg.sender].busdStaked += amount - amounts[1];
        totalBusdStaked += amount - amounts[1];
        totalAmount += liquidity;

        emit Deposit(msg.sender, amount - amounts[1]);
    }
    ...
    function harvest() public {
        ...
        uint reward = balance.mul(user.average).div(totalAverage);
    ...
    function withdraw() public {
        ...
        uint[] memory amounts = JaxLibrary.swapWithPriceImpactLimit(address(router), amountWJAX, priceImpactLimit, path, address(this));
    ...
    function update_average_supply() internal {
        ...
        if(totalAverage > totalAmount) {
            totalAverage -= (totalAverage - totalAmount) * (block.number - lastCountBlock) / totalBlocks;
        }
        if(totalAverage < totalAmount) {
            totalAverage += (totalAmount - totalAverage) * (block.number - lastCountBlock) / totalBlocks;
        }
        lastCountBlock = block.number;
    }
    ...
    function average_supply() public view returns(uint) {
        uint totalBlocks = block.number - startBlock;
        if(totalBlocks == 0) return totalAverage;
        if(totalAverage > totalAmount) {
            return totalAverage - (totalAverage - totalAmount) * (block.number - lastCountBlock) / totalBlocks;
        }
        if(totalAverage < totalAmount) {
            return totalAverage + (totalAmount - totalAverage) * (block.number - lastCountBlock) / totalBlocks;
        }
        return totalAverage;
    }
    ...
    function average_lp(address account) public view returns(uint){
        ...
        uint totalBlocks = block.number - startBlock;
        if(totalBlocks == 0) return user.average;
