...
contract YUMMYToken is Context, IERC20, Ownable {
  ...
  */
    ...
    function fund() public payable returns (bool){
      ...
      //retrieve values for the minting
      (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tokenCount);
      
      //mint 
      _tOwned[msg.sender] = _tOwned[msg.sender].add(tTransferAmount);
    ...
    function withdraw(uint tokenCount) public returns (bool) {
     ...
     if (balanceOf(msg.sender) >= tokenCount) {
        ...
        _tOwned[msg.sender] = _tOwned[msg.sender].sub(tokenCount);
