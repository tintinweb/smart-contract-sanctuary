...
*/
//License owned and provided by snipe.finance
...
contract Migration is Context, Owned {
  ...
  function exchangeToken(uint256 tokens)external   
        {
         ...
         if(enableRatio)
         {
                 uint256 extra = calPercentofTokens(tokens,1100000);
                 ERC20(newToken).transfer(_msgSender(), tokens +extra);
         }
          else  
        ...
        {
        uint256 custPercentofTokens = (_tokens*(cust))/(100 * 10**uint(7));
   ...
   function RatioToggle(bool toggle) external onlyOwner
   {
       enableRatio = toggle;
   }
