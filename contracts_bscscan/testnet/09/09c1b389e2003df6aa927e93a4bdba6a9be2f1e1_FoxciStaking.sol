...
contract FoxciStaking is IFoxciStaking, Ownable ,ReentrancyGuard{
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Foxcis
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accFoxciPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws Staking tokens to a pool. Here's what happens:
        //   1. The pool's `accFoxciPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accRewardPerShare; // Accumulated Foxcis per share, times 1e12. See below.
        uint256 accRewardPerShareTilBonusEnd; // Accumated Foxcis per share until Bonus End.
    }
    ...
    function addPool(
        ...
        poolInfo.push(
            PoolInfo({
                        ...
                        accRewardPerShare: 0,
                        accRewardPerShareTilBonusEnd: 0
                    })
        );
    }
    ...
    function pendingReward(uint256 _pid, address _user) external override view returns (uint256, address) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accFoxciPerShare = accFoxciPerShare.add(foxciReward.mul(GLO_VAL).div(lpSupply));
        }
        return (user.amount.mul(accFoxciPerShare).div(GLO_VAL).sub(user.rewardDebt), address (0));
    }
    ...
    function updatePool(uint256 _pid) public override {
        ...
        pool.accRewardPerShare = pool.accRewardPerShare.add(reward.mul(GLO_VAL).div(lpSupply));
        // update accRewardPerShareTilBonusEnd
        if (block.number <= bonusEndBlock) {
            pool.accRewardPerShareTilBonusEnd = pool.accRewardPerShare;
        }
        if(block.number > bonusEndBlock && pool.lastRewardBlock < bonusEndBlock) {
            uint256 rewardBonusPortion = bonusEndBlock.sub(pool.lastRewardBlock).mul(bonusMultiplier).mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            pool.accRewardPerShareTilBonusEnd = pool.accRewardPerShareTilBonusEnd.add(rewardBonusPortion.mul(GLO_VAL).div(lpSupply));
        }
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(address _for, uint256 _pid, uint256 _amount) nonReentrant public override {
        ...
        user.rewardDebt = user.amount.mul(pool.accRewardPerShare).div(GLO_VAL);
        user.bonusDebt = user.amount.mul(pool.accRewardPerShareTilBonusEnd).div(GLO_VAL);
    ...
    // Harvest Foxcis earn from the pool.
    ...
    function _harvest(address _to, uint256 _pid) internal {
        ...
        uint256 pending = user.amount.mul(pool.accRewardPerShare).div(GLO_VAL).sub(user.rewardDebt);
        require(pending <= IERC20(rewardToken).balanceOf(address(this)), "wtf not enough Foxci");
        safeFoxciTransfer(_to, pending);
    }
