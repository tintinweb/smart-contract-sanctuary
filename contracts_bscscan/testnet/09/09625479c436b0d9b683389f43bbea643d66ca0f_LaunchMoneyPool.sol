...
contract LaunchMoneyPool is Ownable {
    ...
    constructor(address _token, uint256 miniStake) public {
        TOKEN = _token;
        _owner = _msgSender();
        minimumStakeAmount = miniStake;
    }

    function stake(uint256 _quantity) public {
        
        require(_quantity >= minimumStakeAmount, "Too small amount");
    ...
    function unStake(uint256 _amount) public {
        ...
        UserData storage _userData = userData[_msgSender()];
        require(_userData.tokenQuantity >= _amount, "LMY: Insufficient amount");
        
        uint256 pendingReward = calculateRewards(userData[_msgSender()].user);
        
        if(_amount == 0) {
            require(_userData.tokenQuantity > 0, "LMY: NO REWARD YET.");
        ...
        if(_amount > 0) {
            ...
            require( _amount <= _userData.tokenQuantity, "LMY: AMOUNT IS GREATER THAN USER STAKED TOKEN");
    ...
    function withdrawLMYToken(address _to, uint256 _amount) onlyOwner external {
