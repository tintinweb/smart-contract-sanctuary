...
contract Animal is ERC721Pausable, AccessControl, TransferOwnable {
    ...
    //) public ERC721(name, symbol) {
        ...
        mintFeeAddr = 0x2037f7E7242abd6B71938fb83D2c65DE5D4A42B0;
        mintFeeAmount = 1e15;
    ...
    function setHeals(uint256 token_id, uint256 state) public{

        _tokenAnimals[token_id].state = state;

    }

    function getHeals(address owner) public view returns (uint256){
        ...
        for(uint i = 0; i < length; i++){
            ...
            //AnimalEntry amimal = getDetails(token_id);
    ...
    function healsAll(address owner) public payable  {

        require(msg.value >= mintFeeAmount, 'msg value too low');
        uint256 heals = getHeals(owner);
        ...
        for(uint i = 0; i < length; i++){
            ...
            if(animal.state == 3){
                ...
                _tokenAnimals[token_id].state = 1;
    ...
    function addContract(uint256 token_id) public payable{
        ...
        AnimalEntry memory animal = _tokenAnimals[token_id];
        uint256 contractDueTime = block.timestamp + 60 * 60 * 24 * 7;
        ...
        _tokenAnimals[token_id].contractDueTime = contractDueTime;
        if (msg.value > mintFeeAmount) TransferHelper.safeTransferETH(msg.sender, msg.value - mintFeeAmount);
    ...
    function mint(address to, string memory _tokenURI) public payable returns (uint256 tokenId)  {
        ...
        _tokenAnimals[tokenId] = AnimalEntry({tokenId: tokenId, level: level, state: 3,
                                              health_state: 2, contractDueTime: 0, day: 0, 
                                              hour: 0, owner: msg.sender});
        //delete _tokenAnimals[tokenId];
        //
    ...
    /*
    function mintes(address to, string memory _tokenURI, uint256 quantity) public payable returns (uint256[] memory) {
        if(!isPublic){
            require(allowInfo[msg.sender] == 1, 'mintes: require allowInfo[msg.sender]');
        }
        require(block.number>=lastBlockNumber[msg.sender],'Attack_check: Too fast');
        lastBlockNumber[msg.sender] = block.number+1; 
        require(quantity <= 100, 'quantity too much');
        require(msg.value >= mintFeeAmount, 'msg value too low');
        TransferHelper.safeTransferETH(mintFeeAddr, mintFeeAmount);
