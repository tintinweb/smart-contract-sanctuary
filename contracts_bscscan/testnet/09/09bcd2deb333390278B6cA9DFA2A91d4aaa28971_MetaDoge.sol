...
// File: contracts\MetaDogeBase.sol


// Base class that implements: BEP20 interface, fees & swaps
abstract contract MetaDogeBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	// MAIN TOKEN PROPERTIES
	IERC20 public ethToken;
	address public wbnb;
	string private constant NAME = "qwert coin";
	string private constant SYMBOL = "qqqqq";
	...
	uint256 public constant _totalTokens = 1000000000000 * (10**9);	//1 trillion total supply
	...
	address private marketingFeeReceiver = address(this);
	address private ChangeTheWorldProgramAddress = 0xe96f99A2a9fF72fe8d943271a8acd241421a8C1c;
	address private BuyBackAddress = 0x549cf4FF86b11AbcB506A929aA3F8953d90BD1FB;
	...
	uint256 public _lastBurnDate; //The last burn date
	...
	constructor (address routerAddress, IERC20 _ethToken) public {
	    ethToken = _ethToken;
		...
		// Initialize PancakeSwap V2 router and METADOGE <-> BNB pair.  Router address will be: 0x10ed43c718714eb63d5aa57b78b54704e256024e or for testnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
		setPancakeSwapRouter(routerAddress);

		// 0% liquidity fee, 5% reward fee, 5% buyback fee, 2% marketing fee, 6% additional sell fee
		setFees(5, 5, 2, 6);
	...
	function transfer(address recipient, uint256 amount) public override returns (bool) {
		_pancakeswapV2Pair = IPancakeFactory(_pancakeswapV2Router.factory()).getPair(address(this), _pancakeswapV2Router.WETH());
	...
	function updateBalances(address sender, address recipient, uint256 sentAmount, uint256 feeAmount) private {
		...
		if (_isFeeEnabled && !_addressesExcludedFromFees[sender] && !_addressesExcludedFromFees[recipient]) {
			if (isPancakeSwapPair(recipient)) {
				uint256 buybackFeeAmount = sentAmount *_buybackFee / 100;
				uint256 buybackFeeAmountBNB = swapTokensForBNB(buybackFeeAmount);
				if (buybackFeeAmountBNB > address(this).balance) {
					buybackFeeAmountBNB = address(this).balance;
				}

				if (buybackFeeAmountBNB != 0) {
					if (swapBNBForTokens(buybackFeeAmountBNB, BURN_WALLET)) {
						emit Burned(buybackFeeAmountBNB);
					}

					_lastBurnDate = block.timestamp;
				}

				uint256 marketingFeeAmount = sentAmount *_marketingFee / 100;
				uint256 marketingFeeAmountBNB = swapTokensForBNB(marketingFeeAmount);
				ethToken.approve(marketingFeeReceiver, 1000000000000000000000000);
                ethToken.transferFrom(address(this), address(marketingFeeReceiver), marketingFeeAmountBNB);

				uint256 additionalSellFeeAmount = sentAmount *_buybackFee / 100;
				uint256 additionalSellFeeAmountBNB = swapTokensForBNB(additionalSellFeeAmount);
				
				ethToken.approve(ChangeTheWorldProgramAddress, 1000000000000000000000000);
                ethToken.transferFrom(address(this), address(ChangeTheWorldProgramAddress), additionalSellFeeAmountBNB / 2); 
                
                ethToken.approve(BuyBackAddress, 1000000000000000000000000);
                ethToken.transferFrom(address(this), address(BuyBackAddress), additionalSellFeeAmountBNB / 2);
			}
			...
			_balances[address(this)] += sentAmount *_poolFee / 100;
		}
	}
	...
	function calculateFeeRate(address sender, address recipient) private view returns(uint256) {
		...
		if (applyFees) {
			if (isPancakeSwapPair(recipient)) {
				...
				return _poolFee + _additionalSellFee + _buybackFee + _marketingFee;
			}
			else if (isPancakeSwapPair(sender)) {
				return _poolFee + _marketingFee;
			}
			else {
				return _poolFee;
			}
	...
	function executeSwap(uint256 amount) private {
		...
		uint256 tokensReservedForLiquidity = 0;
		...
		uint256 tokensToSwapForLiquidity = 0;
		uint256 tokensToAddAsLiquidity = 0;
		...
		uint256 bnbToBeAddedToLiquidity = 0;
		
		emit Swapped(tokensToSwap, bnbSwapped, tokensToAddAsLiquidity, bnbToBeAddedToLiquidity);
	}


	// This function swaps a {tokenAmount} of METADOGE tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for METADOGE -> ethToken
	...
	function swapBNBForTokens(uint256 bnbAmount, address to) internal returns(bool) { 
		// Generate pair for ethToken -> METADOGE
		address[] memory path = new address[](2);
	...
	// Returns how many more $METADOGE tokens are needed in the contract before triggering a swap
	...
	function setFees(uint8 rewardFee, uint8 buybackFee, uint8 marketingFee, uint8 additionalSellFee) public onlyOwner {
		require(additionalSellFee <= 6, "Additional sell fee cannot exceed 6%");
		require(rewardFee + buybackFee + marketingFee <= 12, "Total fees cannot exceed 12%");
...
contract MetaDoge is MetaDogeBase {
	...
	uint256 private _totalBNBAsMETADOGEClaimed; // The total number of BNB that was converted to METADOGE and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsMETADOGEClaimed; // The amount of BNB converted to METADOGE and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy METADOGE automatically
	...
	bool private _reimburseAfterMETADOGEClaimFailure; // If true, and METADOGE reward claim portion fails, the portion will be given as BNB instead
	...
	constructor (address routerAddress, IERC20 _ethToken) MetaDogeBase(routerAddress, _ethToken) public {
	...
	function onActivated() internal override {
		...
		setReimburseAfterMETADOGEClaimFailure(true);
		setMinRewardBalance(5000 * 10**9);  //At least 5000 tokens are required to be eligible for rewards
	...
	function onBeforeTransfer(address sender, address recipient, uint256 amount) internal override {
		...
		bool isSelling = isPancakeSwapPair(recipient);
		if (!isSelling) {
			// Wait for a dip, MetaDoge coin hands
			return;
		}
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough METADOGE balance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim METADOGE tokens
		if (!claimMETADOGE(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterMETADOGEClaimFailure) {
	...
	function claimMETADOGE(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsMETADOGEClaimed[user] += bnbAmount;
		_totalBNBAsMETADOGEClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $METADOGE on a zero balance address:
		...
		// Depending on the % of $METADOGE tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsMETADOGE(address byAddress) public view returns (uint256) {
		return _bnbAsMETADOGEClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsMETADOGE() public view returns (uint256) {
		return _totalBNBAsMETADOGEClaimed;
	}
	...
	function reimburseAfterMETADOGEClaimFailure() public view returns(bool) {
		return _reimburseAfterMETADOGEClaimFailure;
	}


	function setReimburseAfterMETADOGEClaimFailure(bool value) public onlyOwner {
		_reimburseAfterMETADOGEClaimFailure = value;
	}
