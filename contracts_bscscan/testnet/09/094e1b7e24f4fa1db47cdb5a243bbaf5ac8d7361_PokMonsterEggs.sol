    ...
    ReentrancyGuard
{
    ...
    uint256 public fee_;
    uint256 private currentSeed;

    uint256 nonce;

    mapping(uint256 => uint256) public indexEvolution;
    mapping(uint256 => IPokMonsterStats.PokBody) private staticBody_;

    uint256 public evolutionRate;
    uint256 public bonusRate;
    uint256[] public eggs;
    mapping(address => bool) public whitelistedContract;
    uint256 public caskback_;
    /* ========== EVENTS ========== */
    ...
    event BurnFee(uint256 indexed tokenId, uint256 fee);
    event ReserveFee(uint256 indexed tokenId, uint256 fee);
    event OpenEgg(uint256 indexed tokenId, bool isSuccess);
    ...
    function setFeeRates(uint256 _fee, uint256 _caskback) external onlyOwner {
        fee_ = _fee;
        caskback_ = _caskback;
    }

    function setEvolutionRate(uint256 _openRate) external onlyOwner {
        require(_openRate > 0, "zero");
        evolutionRate = _openRate;
    }

    function setPokMonnsterInfo(address _stats, address _factory)
        external
        onlyOwner
    {
        require(_stats != address(0) && _factory != address(0), "zero");
        stats_ = _stats;
        factory_ = _factory;
    }
    ...
    function staticBody(uint256 index)
        external
        view
        returns (IPokMonsterStats.PokBody memory body)
    {
        body = staticBody_[index];
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    function addEgg(uint256 _index) public onlyOwner {
        require(_index > 0, "zero");
        eggs.push(_index);
    }

    function setBodyIndex(
        uint256 tokenId,
        IPokMonsterStats.PokBody memory newBody
    ) public onlyOwner {
        delete staticBody_[tokenId];
        staticBody_[tokenId] = newBody;
    }
    ...
    function getLuckyNumber(uint256 _randomNumber)
        private
        pure
        returns (uint256)
    {
        ...
        return
            uint256(keccak256(abi.encodePacked(_randomNumber, maxRanger))) %
            100;
    }
    ...
    function getRandomNumber(
        ...
        require(_to > _from, "invalid number");
        uint256 maxRanger = _to - _from;
        return
            (uint256(keccak256(abi.encodePacked(_randomNumber, maxRanger))) %
                maxRanger) + _from;
    }

    function openEggs() external nonReentrant {
        require(eggs.length > 0, "!");
        nonce = nonce + 1;
        currentSeed = _genNextSeed(currentSeed ^ nonce);
        uint256 _evolutionId = getRandomNumber(
            currentSeed,
            99999,
            999999999999999
        );

        uint256 _rate = evolutionRate;
        uint256 _indexBody = evolutionRate % eggs.length;
        IPokMonsterStats.PokBody memory _body = staticBody_[eggs[_indexBody]];

        uint256 _randNumber = _evolutionId % 10000;
        IERC20(pok_).safeTransferFrom(msg.sender, address(this), fee_);
        bool result = _randNumber < _rate;
        if (result) {
            IPokMonsterFactory _factory = IPokMonsterFactory(factory_);
            _factory.mintToken(msg.sender, _evolutionId, _body);
        } else {
            IERC20(pok_).safeTransfer(msg.sender, caskback_);
        }

        emit OpenEgg(_evolutionId, result);
    }
    ...
    function rescueStuckErc721(uint256 _tokenId) external onlyOwner {
