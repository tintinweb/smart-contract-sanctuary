 ...
 * a TOKEN/BUSD liquidity pair. And breaks a liquidity pair to single assets
 *
 */
...
contract Pokito is Ownable, IZapper {
    using SafeMath for uint;
    using SafeBEP20 for IBEP20;

    address public WBNB;
    address public _pcsRouter; 
    address public _ETH; 
    address public _BUSD; 
    address public _LPaddress;
    address public _beefyVaultAddress;
    ...
    constructor () public {
         ...
         _pcsRouter = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
         WBNB =      0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd   ;
         _ETH =       0x8BaBbB98678facC7342735486C851ABD7A0d17Ca;
         _BUSD =      0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
         _LPaddress = 0xe0e92035077c39594793e61802a350347c320cf2;
         _beefyVaultAddress = 0xc8D4F58890cab2Aa8C6e4ae9D5bB241F50014bbA;
        ...
        router = IPancakeRouter02(_pcsRouter);
        vault = IBeefyVault(_beefyVaultAddress);



        //bsc testnet _pcsRouter = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
        //bsc testnet cake = 0xF9f93cF501BFaDB6494589Cb4b4C15dE49E85D0e


        // approve our main input tokens
        // IBEP20(_ETH).safeApprove(address(ROUTER), 9999999);
        // IBEP20(_PR).safeApprove(address(ROUTER), uint(~0));
        // IBEP20(_BUSD).safeApprove(address(ROUTER), uint(~0));

        //approve to breakLP
        //IBEP20(_LPaddress).safeApprove(address(ROUTER), uint(~0));
        //IBEP20(_PRBUSD).safeApprove(address(ROUTER), uint(~0));

        // set route pairs for our tokens
        //routePairAddresses[_PR] = _BUSD;
        //routePairAddresses[_ETH] = _BUSD;
    }
    ...
    function setBeefyVault(address _address) public onlyOwner {
        _beefyVaultAddress = _address;
    }
    ...
    function breakLP(address _from, uint amount) external override {
        ...
        router.removeLiquidity(token0, token1, amount, 0, 0, msg.sender, block.timestamp + 600);
    }
    ...
    function zapTokenToLP(address _from, uint amount, address _to) public override {
        IBEP20(_from).safeTransferFrom(msg.sender, address(this), amount);
        ...
        // PR, PP, BUSD to create PR-BUSD or PP-BUSD will hit this if
        if (_from == token0 || _from == token1) {
            ...
            router.addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, msg.sender, block.timestamp + 600);
        } else {
    ...
    function _swapBNBToLP(address lp, uint amount, address receiver) private {
        ...
        router.addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, receiver, block.timestamp + 600);
    }
    ...
    function _swapTokenForBNB(address token, uint amount, address receiver) private returns (uint) {
        ...
        uint[] memory amounts = router.swapExactTokensForETH(amount, 0, path, receiver, block.timestamp + 600);
     ...
     * Not very useful for our current use case as bolt input currencies will only be BUSD, WBNB, PP, PR
    ...
    function _swap(address _from, uint amount, address _to, address receiver) private returns (uint) {
        ...
        if (intermediate != address(0) && (_from == WBNB || _to == WBNB)) {
            ...
            // E.g. [PR/PP, BUSD, WBNB, xTokenWithWBNBLiquidity]
            path = new address[](4);
            path[0] = _from;
            path[1] = intermediate;
            path[2] = WBNB;
            ...
            // E.g. [xTokenWithWBNBLiquidity, WBNB, BUSD, PR/PP]
            path = new address[](4);
            path[0] = _from;
            path[1] = WBNB;
            path[2] = intermediate;
        ...
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount, 0, path, receiver, block.timestamp + 600);
    }



//1.Swap BNB vers CAKE
// --> avant de swap il faut approve le contrat déployé auprès du wbnb - montant 999999999999999999999999999999999999999999999
// --> ensuite il faut envoyer des BNB (ou TBNB) sur le contrat :)

    function _swapBNBForToken(address token, uint value, address receiver) public returns (uint) {
        address[] memory path;
        ...
        uint[] memory amounts = router.swapExactETHForTokens{value : value}(0, path, receiver, block.timestamp + 600);
        return amounts[amounts.length - 1];
    }


//2. Deposit sur vault cake de beefy : 0x97e5d50fe0632a95b9cf1853e744e02f7d816677

    function _depositInBeefy(uint _amount) public onlyOwner { 
