...
contract BEP20Token is Context, IERC20, Ownable {
    ...
    string private _name = "NEWTESTHL"; 
    ...
    uint256 public fomoMin = 5 * 10**3 * 10**9; 
    IWrap public wrap;
    uint256[4] public feeFomo = [0, 10, 15, 20];
    uint256[4] public feeFee = [0, 2, 3, 4];
    ...
    function setWrap(IWrap _wrap) onlyOwner public {
        wrap = _wrap;
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tFomo, uint256 tFee) {
        if (!feeIt) {
            return (tAmount, 0, 0);
        }
        ...
        tTransferAmount = tAmount.sub(tFomo).sub(tFee);
    }
    ...
    function _takeTax(uint256 tFomo) private {
    ...
    function getBuyLimitLevel() private view returns (uint256) {
        uint256 bal = IERC20(address(this)).balanceOf(uniswapV2Pair).div(15);
        return bal;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFomo, uint256 tFee) = _getTValues(tAmount);
