...
contract Ktest is Context, IERC20, Ownable {
    ...
    string private constant _name = "Ktest Coin";
    ...
    uint256 public _tributeFee = 100;
    ...
    uint256 public _maxTxAmount = 5 * 10**5 * 10**18; // 0.005
    ...
    constructor (address payable tributeAddress, address payable maintenanceAddress, address payable liquidityWalletAddress) {
    ...
    function setTributeFeePercent(uint256 tributeFee) external onlyOwner() onlyUnlocked(){
        require(tributeFee <= 250, "Cannot set percentage over 1.00%");
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (valuesResult.rAmount, valuesResult.rTransferAmount, valuesResult.rFee, valuesResult.tTransferAmount, valuesResult.tFee, valuesResult.tLiquidity, valuesResult.tTribute, valuesResult.tMaintenance, valuesResult.tLiquidityWallet);
    }
    ...
    function _getTValues(uint256 tAmount, ValuesResult memory valuesResult) private view returns (ValuesResult memory) {
        ...
        valuesResult.tTransferAmount = tAmount.sub(valuesResult.tFee).sub(valuesResult.tTribute).sub(valuesResult.tMaintenance).sub(valuesResult.tLiquidityWallet);
        return valuesResult;
    }
    ...
    function _getRValues(uint256 tAmount, ValuesResult memory valuesResult, uint256 currentRate) private pure returns (ValuesResult memory) {
        ...
        valuesResult.rTransferAmount = valuesResult.rAmount.sub(valuesResult.rFee).sub(valuesResult.rTribute).sub(valuesResult.rMaintenance).sub(valuesResult.rLiquidityWallet);
        return (valuesResult);
    }
    ...
    function _takeTribute(uint256 tTribute) private {
        uint256 currentRate =  _getRate();
        uint256 rTribute = tTribute.mul(currentRate);
        _rOwned[tribute()] = _rOwned[tribute()].add(rTribute);
        if(_isExcluded[tribute()])
            _tOwned[tribute()] = _tOwned[tribute()].add(tTribute);
    }
    ...
    function addTokensTransferred(address wallet, uint256 amount) private {
        uint256 rate = _taxFee.add(_tributeFee).add(_maintenanceFee).add(_liquidityWalletFee);
    ...
    */
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tTribute, uint256 tMaintenance, uint256 tLiquidityWallet) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _takeTribute(tTribute);
