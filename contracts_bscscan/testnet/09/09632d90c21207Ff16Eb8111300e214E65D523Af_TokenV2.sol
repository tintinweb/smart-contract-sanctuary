 ...
 function setDev(address wallet)  public {
  ...
  function transfer(address _to, uint256 _amount) 
   public virtual override returns (bool) {
     ...
     uint256 fee = (_amount / 100) * fees; // Calculate fee
   // balanceOf[msg.sender] = balanceOf[msg.sender].sub(_amount); // subtract the full amount
   // balanceOf[devfee] = balanceOf[devfee].add(fee); // add the fee to the admin balance
   // balanceOf[_to] = balanceOf[_to].add((_amount - fee));  // add the remainder to the recipient balance
     
     super._transferTax(msg.sender,_to, _amount, fee ,devfee);
}
  ...
  //      uint256 deduceFee=amount;
   ...
   //     else 
   //     {
     //     deduceFee = deduceFee-((deduceFee/100)*fees);
   //     
    //    }

    //   require(deduceFee>0, "The value must be greater than zero");
