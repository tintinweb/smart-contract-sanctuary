...
contract GachaBox is Initializable , OwnableUpgradeable{
    ...
    function getRewardNFTCard(uint256 boxID) public view returns (uint256[] memory) {
        // Gacha fighter card
        uint256[] memory totalIdx = new uint256[](ILastSurvivorCharacter(lastSurvivorCharacter).balanceOf(msg.sender));
        ...
        for (uint256 i = 0; i < selectNumber; i++) {
            if (needRandom) { 
                ...
                rewardNft[i] = ILastSurvivorCharacter(lastSurvivorCharacter).tokensOfOwners(msg.sender, rewardIndex);
                totalIdx[rewardIndex] = totalIdx[totalIdx.length];
            } else {
                rewardNft[i] = ILastSurvivorCharacter(lastSurvivorCharacter).tokensOfOwners(msg.sender, i);
            }
    ...
    function openBox(uint256 _boxId) public onlyNonContract  returns (uint256) {
        ...
        uint256[] memory reChaIdx = getRewardNFTCard(_boxId);
