...
contract $THOTHDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("$THOTH_Dividend_Tracker", "$THOTH_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "$THOTH_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "$THOTH_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main $THOTH contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "$THOTH_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "$THOTH_Dividend_Tracker: Cannot update claimWait to same value");
...
contract THOTH is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    $THOTHDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 1000000000000 * (10**9);
    uint256 public maxBuyTransactionAmount = 1000000000000 * (10**9);
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _charityFee, uint256 _buybackFee) 
    public onlyOwner 
    {
        ...
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(charityFee).add(buybackFee); // total fee
    }
    ...
    constructor() ERC20("K+W=U", "$THOTH") 
    {
        ...
        marketingWallet = payable(0x6E4E4cA7Bb8d4ba5aC3e2B690E8bE9AF94A3a235);
        charityWallet = payable(0x8125E55B0f854dc6629075E4ea84D0822d9d2c0e);
        

    	dividendTracker = new $THOTHDividendTracker();
        ...
        */
        _mint(owner(), 1000_000_000_000 * (10**9));
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "$THOTH: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "$THOTH: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "$THOTH: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "$THOTH: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "$THOTH: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "$THOTH: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // worthy charity fee
        uint256 charityAmount = deltaBalance.mul(charityFee).div(totalFees);
        if (charityAmount > 0) {
