  ...
  ReentrancyGuardUpgradeable
{
   ...
   */
  function __UndeadPresenter_init(address _token, address _router) public initializer {
    __Ownable_init();
    __CoinDexTools_init();
  ...
  function receiveTokens(
    ...
    if (flags[8] || flags[4] || isInsideTrigger) {
      ...
      uint taxAmount_ = 0;
      // Apply tax on buy and sell only
      if (!flags[7]) {
        taxAmount_ = _taxCollector(_amount);
      }
      if (_to != address(this)) {
   ...
   */
  function solvePendingLP() external nonReentrant {
