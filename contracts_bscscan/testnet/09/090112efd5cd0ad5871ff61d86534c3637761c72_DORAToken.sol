...
contract DORAToken is owned {
    ...
    string constant private _name = "DORA Token";
    string constant private _symbol = "DORA";
    ...
    function manualWithdrawCoin()onlyOwner public{
    ...
    struct allocationType
    {
        ...
        uint monthlyReleaseLimit; // amount of tokens
        uint allocatedTotal;
        uint releasedTotal;
    }
    ...
    mapping (uint => mapping( address => uint)) _allocationBalance;
    mapping (uint => mapping( address => uint)) _claimedAmount;
    uint public tokenPrice = 10 ** 18;

    event defineAllocationEv(bytes32 Name, uint _startTime, uint _totalAllocation,uint _initialLockTime, uint _monthlyReleaseLimit,uint allocationTypeIndex); 
    function defineAllocation(bytes32 Name, uint _startTime, uint _totalAllocation,uint _initialLockTime, uint _monthlyReleaseLimit  ) public onlyOwner returns(bool)
    {
        ...
        temp.monthlyReleaseLimit = _monthlyReleaseLimit;
        allocationTypes.push(temp);
        totalAllocated += _totalAllocation;
        require(totalAllocated <= totalSupply(), "total supply crossed");
        emit defineAllocationEv(Name,_startTime,_totalAllocation,_initialLockTime,_monthlyReleaseLimit, allocationTypes.length -1);      
        return true;
    }

    function editAllocationReleaseLimit(uint allocationTypeIndex, uint _monthlyReleaseLimit  ) public onlyOwner returns(bool)
    {
        require(allocationTypeIndex < allocationTypes.length, "Invalid Index");
        allocationTypes[allocationTypeIndex].monthlyReleaseLimit = _monthlyReleaseLimit;      
        return true;
    }
    ...
    function deleteAllocation(uint allocationTypeIndex) public onlyOwner returns(bool)
    {
        ...
        for(uint i=allocationTypeIndex; i< len - 2; i++)
        {
            allocationTypes[i] = allocationTypes[i+1];
        }
        allocationTypes.length = len -1;
        return true;
    }
    ...
    function claimAllocation(uint allocationTypeIndex,uint _claimAmount) public returns(bool)
    {
        ...
        require(releaseTime <=  now,  "In lock period");
        uint currentMonthCount = ((now - releaseTime) / 30 days) + 1;
        uint amountClaimable = (allocationTypes[allocationTypeIndex].monthlyReleaseLimit * currentMonthCount) - _claimedAmount[allocationTypeIndex][msg.sender];
