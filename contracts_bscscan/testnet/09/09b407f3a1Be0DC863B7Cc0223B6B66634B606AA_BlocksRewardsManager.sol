...
contract BlocksRewardsManager is Ownable {
    ...
    uint256 public globalBlsRewardsAcc; // bls rewards accumulated
    ...
    function addSpace(address spaceContract_, uint256 blsPerBlockAreaPerBlock_) external onlyOwner {
        ...
        // console.log("allBlsPerBlockAreaPerBlock", allBlsPerBlockAreaPerBlock);
    ...
    function updateBlsPerBlockAreaPerBlock(uint256 spaceId_, uint256 newAmount_) external onlyOwner {
        ...
        // console.log("UPDATEallBlsPerBlockAreaPerBlock", allBlsPerBlockAreaPerBlock);
    ...
    function pendingBlsTokens(uint256 spaceId_, address user_) public view returns (uint256) {
        ...
        if (user.amount > 0 && space.blsRewardsAccLastUpdated < block.number) {
            ...
            // console.log("blsRewardsFinishedBlock", blsRewardsFinishedBlock);
            // console.log("multiplier", multiplier);
            // console.log("space.blsPerBlockAreaPerBlock", space.blsPerBlockAreaPerBlock);
            rewards = user.amount * blsRewards;
        }
        // console.log("user.amount",user.amount);
        // console.log("space.globalBlsRewardsAcc",space.globalBlsRewardsAcc);
        // console.log("rewards",rewards);
        // console.log("user.rewardsDebt",user.rewardsDebt);
        return user.amount * space.globalBlsRewardsAcc + rewards + user.pendingRewards - user.rewardsDebt;
    }

    function getMultiplier(uint256 spaceBlsRewardsAccLastUpdated_) internal view returns (uint256) {
        if (block.number > blsRewardsFinishedBlock && blsRewardsFinishedBlock > spaceBlsRewardsAccLastUpdated_) { // If we are already after finished distribution of rewards
            // console.log("blsRewardsFinishedBlock",blsRewardsFinishedBlock);
            // console.log("spaceBlsRewardsAccLastUpdated_",spaceBlsRewardsAccLastUpdated_);
            return blsRewardsFinishedBlock - spaceBlsRewardsAccLastUpdated_;
        } else {
            return block.number - spaceBlsRewardsAccLastUpdated_;
        }
    }
    ...
    function updateSpace(uint256 spaceId_) internal {
        ...
        if (block.number > space.blsRewardsAccLastUpdated) {
            ...
            space.globalBlsRewardsAcc = space.globalBlsRewardsAcc + multiplierSpace * space.blsPerBlockAreaPerBlock;
    ...
    function blocksAreaBoughtOnSpace(
        ...
        uint256 spaceBlsRewardsAcc = space.globalBlsRewardsAcc;
        ...
        if (numberOfBlocksAdded > 0) {
            ...
            if (blsBalance > globalBlsRewardsAcc) {
                uint256 blocksTillBlsRunOut = (blsBalance + blsRewardsClaimed - globalBlsRewardsAcc) /
    ...
    function claim(uint256 spaceId_) public {
        ...
        // console.log("WANNA CLAIM: ", toClaimAmount);
        if (toClaimAmount > 0) {
            ...
            user.rewardsDebt = spaceInfo[spaceId_].globalBlsRewardsAcc * user.amount + claimedAmount;
    ...
    function depositBlsRewardsForDistribution(uint256 amount_) external onlyOwner {
        ...
        // console.log("-----depositBlsRewardsForDistribution-----");
        // TODO: update all spaces
        updateSpace(0);
        // We are adding additiona BLS rewards after old ones ran out
        if(block.number > blsRewardsFinishedBlock){
            globalBlsRewardsAcc = globalBlsRewardsAcc + (blsRewardsFinishedBlock - blsRewardsAccLastUpdatedBlock) * allAllocationBlocks * allBlsPerBlockAreaPerBlock;        
        }else{ // We are adding additiona BLS rewards still when old ones did not run out
            globalBlsRewardsAcc = globalBlsRewardsAcc + (block.number - blsRewardsAccLastUpdatedBlock) * allAllocationBlocks * allBlsPerBlockAreaPerBlock;
        }
        blsRewardsAccLastUpdatedBlock = block.number;
        uint256 blsBalance = blsToken.balanceOf(address(this));
        // console.log("blsBalance", blsBalance);
        // console.log("allAllocationBlocks", allAllocationBlocks);
        // console.log("globalBlsRewardsAcc", globalBlsRewardsAcc);
        if (blsBalance >= globalBlsRewardsAcc && allAllocationBlocks > 0) {
            uint256 blocksTillBlsRunOut = (blsBalance + blsRewardsClaimed - globalBlsRewardsAcc) /
                (allBlsPerBlockAreaPerBlock * allAllocationBlocks);
        // console.log("allBlsPerBlockAreaPerBlock", allBlsPerBlockAreaPerBlock);
        // console.log("blsRewardsClaimed", blsRewardsClaimed);
        // console.log("blocksTillBlsRunOut", blocksTillBlsRunOut);
            blsRewardsFinishedBlock = block.number + blocksTillBlsRunOut;
        }
    }
