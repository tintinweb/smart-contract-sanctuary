...
contract Zap is OwnableUpgradeable {
    ...
    function zapIn(
        ...
        address pair,
        uint256 amount
    ) external returns (uint256 liquidityBought) {
        ...
        if (token1 == WBNB) {
            ...
            (,,liquidityBought) =  router.addLiquidityETH(token0,
                _amountTokenDesired,
                0,
                0,
                address(this),
    ...
    function zapOut(
        ...
        // calculate liquidity
        (uint256 balance0, uint256 balance1,) = IPancakePair(pair).getReserves();
        uint256 amount0 = liquidity.mul(balance0) / IPancakePair(pair).totalSupply();
        uint256 amount1 = liquidity.mul(balance1) / IPancakePair(pair).totalSupply(); 
        ...
        if (isWantPair) {
            ...
            if (token == token0) {
                swapAmounts = _swap(
                    amount1,
                    0,
                    _getRoute(token1, token0),
                    address(this)
                );
                // transfer the desire token
                IERC20Upgradeable(token0).safeTransfer(msg.sender, amount0.add(swapAmounts[swapAmounts.length - 1]));
                ...
                swapAmounts = _swap(
                    amount0,
                    0,
                    _getRoute(token0, token1),
                    address(this)
                );
                
                // transfer the desire token
                IERC20Upgradeable(token1).safeTransfer(msg.sender, amount1.add(swapAmounts[swapAmounts.length - 1]));
                token0Amount = 0;
    ...
    function _processSwap(address _token, uint256 _amount, address token0, address token1) private returns (uint256, uint256) {
        ...
        if (_token != token0 && _token != token1) {
            ...
            if (_token == token0) {
                pathToOther = _getRoute(token0, token1); 
            } else {
                pathToOther = _getRoute(token1, token0);
            }
    ...
    function _getRoute(address tokenFrom, address tokenTo) private view returns (address[] memory route)  {
