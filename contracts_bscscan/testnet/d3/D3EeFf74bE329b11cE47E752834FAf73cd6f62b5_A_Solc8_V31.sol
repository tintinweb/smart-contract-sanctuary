...
abstract contract Ownable is Context, MultiSig {
    ...
    modifier onlyOperator() {
        require(operator() == _operator, "Ownable: caller is not the operator");
        _;
    }
    /**
     * @dev Throws if called by any account other than the owner.
...
contract A_Solc8_V31 is BEP20("ArghV31", "ADMRSL6V31") {
    using Address for address;
    using SafeMath for uint256;
    ...
    function J1_MultiSig_Enable() public onlyOperator whenMultiSigOn {
        _stop_multisig();
    }
