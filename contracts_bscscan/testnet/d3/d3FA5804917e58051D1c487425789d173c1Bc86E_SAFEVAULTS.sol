...
contract SAFEVAULTS is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingWalletAddress = payable(0x719729A83FD0c7C3bc4F641C715194287162f5bA); // Marketing Address
    address payable public busdCharityWalletAddress = payable(0x220688164cE1eabf299e83F95Ca9d1cd093B93d1); // busd Address
    ...
    string private _name = "Safevaults";
    ...
    uint256 public _busdCharityFee = 0;
    ...
    constructor () {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); //PCS
        ...
        _totalTaxPercent = _burnFee.add(_liquidityFee).add(_marketingFee).add(_busdCharityFee);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _marketingFee.add(_busdCharityFee) > 0)
        {
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(forWallets);
            uint256 newBalance = address(this).balance.sub(initialBalance);
    
            uint256 marketingShare = newBalance.div(_marketingFee.add(_busdCharityFee)).mul(_marketingFee);
            uint256 busdWalletShare = newBalance.sub(marketingShare);
            
            if(marketingShare > 0)
                transferToAddressETH(marketingWalletAddress, marketingShare);
            
            if(busdWalletShare > 0)
                transferToAddressETH(busdCharityWalletAddress, busdWalletShare);
        }
 ...
 function includeInFee(address account) public onlyOwner {
    ...
    function setTaxes(uint256 newBurnFee, uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newBusdCharityTax) external onlyOwner() {
        _burnFee = newBurnFee;
        _liquidityFee = newLiquidityTax;
        _marketingFee = newMarketingTax;
        _busdCharityFee = newBusdCharityTax;
        _totalTaxPercent = _burnFee.add(_liquidityFee).add(_marketingFee).add(_busdCharityFee);
        _prevTotalTaxPercent = _totalTaxPercent;
    }
    ...
    function setBusdCharityWalletAddress(address newAddress) external onlyOwner() {
        busdCharityWalletAddress = payable(newAddress);
    }
