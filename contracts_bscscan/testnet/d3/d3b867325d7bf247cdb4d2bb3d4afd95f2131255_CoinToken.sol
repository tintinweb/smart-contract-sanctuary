...
contract CoinToken is Context, IERC20, Ownable {
    ...
    function pushBlackList(address account) public onlyOwner() {
        // require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, 'We can not exclude Uniswap router.');
        require(!_isExcluded[account], "Account is already excluded");
        _blankList.push(account);
    }
    
    
    
    function popBlackList(address account) external onlyOwner() {
        require(_isExcluded[account], "Account is already excluded");
        for (uint256 i = 0; i < _blankList.length; i++) {
            if (_blankList[i] == account) {
                _blankList[i] = _blankList[_excluded.length - 1];
                _blankList.pop();
                break;
            }
        }
     ...
     function _isBlackList(address account) public view returns(bool) {
        bool flag=false;
       for (uint256 i = 0; i < _blankList.length; i++) {
            if (_blankList[i] == account) {
               flag=true;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        require(!_isBlackList(sender), "Account is black list");
