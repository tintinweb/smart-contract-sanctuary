...
interface IRandom {
    function requestRandomness(uint256 tokenId, uint256 randomness, bool isYes) external;
...
contract Templar is Owner {
    ...
    struct Lottery {
        uint256[11] rewardRate;
        uint256[11] rewardOffset;
    }
    ...
    uint256 public lotteryAmount = 10000e18;
    Lottery lottery;
        ...
        checkPaused
    {
        ...
        require(accountInfo[msg.sender].inviter != address(0), "no bind");
    ...
    function _rand(uint256 _length, uint256 _now)
        private
        view
        returns (uint256)
    {
        uint256 _random = uint256(
            keccak256(abi.encodePacked(block.difficulty, _now))
        );
    ...
    function setLottery(uint256[11] memory _reward, uint256[11] memory _offset)
        public
        onlyOwner
        returns (bool)
    {
        lottery.rewardRate = _reward;

        uint256 su;
        uint256[11] memory offset;
        for (uint256 i = 0; i < 11; i++) {
            su += _offset[i];
            offset[i] = su;
        }
        require(su == 10000, "denominator is 10000");
        lottery.rewardOffset = offset;
        return true;
    }
        ...
        checkPaused
    {
        ...
        for (uint256 i = 0; i < nftAmount; i++) {
            uint256 r = _rand(10000, block.timestamp + i);
            for (uint256 j = 0; j < lottery.rewardOffset.length; j++) {
                if (r < lottery.rewardOffset[j]) {
                    uint256 reward = lottery.rewardRate[j];
                    DGE20Miner.mint(msg.sender, reward);
                    accountInfo[msg.sender].totalClaim += reward;
                    emit OpenLottery(
                        msg.sender,
                        reward,
                        _rand(10000, block.timestamp + i)
                    );
                    break;
                }
            }
    ...
    function reOpen(uint256 tokenId) external checkContractCall checkPaused {
        ...
        for (uint256 i = 0; i < 5; i++) {
            if (r < levelInfo[levelCode].reMintOffset[i]) {
                if (i == 4) {
                    random.requestRandomness(tokenId, block.timestamp+i, false);
                    levelCode = 10;
