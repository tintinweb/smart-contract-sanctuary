...
contract BABYnfts is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 420000000000000 * 10**18; //420,000,000,000,000
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 public startTime;

    string private _name = "BABYnfts";
    string private _symbol = "BBNF";
    uint8 private _decimals = 18;
                                //This is where you can change tax functions - before deployment
    uint256 public _taxFee = 30; // 3% reflection
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 50; // 5% liquidity
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _charityFee = 10; //1% Charity
    uint256 private _previousCharityFee = _charityFee;

    uint256 public _devFee = 20; //2% Marketing/Admin fee
    uint256 private _previousDevFee = _devFee;

    uint256 public _burnFee = 10; //1% burn
    ...
    bool private isLiquidityAdded;
                                    //Anti-Whale Measures Below
    uint256 public _maxTxAmount = 1000000000 * 10**18; //max tokens buy/sell/transfer per transaction
    uint256 public _maxWalletLimit = 10000000000 * 10**18; // 1% of the supply antiwhale
    uint256 public _maxSoldin24Hrs = 1000000000 * 10**18; // 0.1% of the supply
    uint256 private numTokensSellToAddToLiquidity = 25000000 * 10**18; //taxed token swapped for bnb to add liquidity and various tax
    ...
    constructor (address charityWallet, address devWallet) {
        _rOwned[_msgSender()] = _rTotal;

       IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); //PCS testnet router
   ...
   function balanceOf(address account) public view override returns (uint256) {
    ...
    function setMaxTxPerDay(uint256 amount) external onlyOwner() {
        require(amount >= 1000000, "No rug pull");
        _maxSoldin24Hrs = amount * 10**18;
    }

    function setMaxWalletLimit(uint256 amount) external onlyOwner() {
        _maxWalletLimit = amount * 10**18;
    }
   ...
   function setDevWallet(address payable devWallet) external onlyOwner{
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tCharity, _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        tTransferAmount = tTransferAmount.sub(calculateDevFee(tAmount)).sub(tCharity);
        tTransferAmount = tTransferAmount.sub(calculateBurnFee(tAmount));
        return (tTransferAmount, calculateTaxFee(tAmount), calculateLiquidityFee(tAmount), calculateDevFee(tAmount), tCharity);
    }

    function _getRValues(uint256 tAmount, uint256 tCharity, uint256 currentRate) private view returns (uint256, uint256, uint256) {
        ...
        rTransferAmount = rTransferAmount.sub(calculateDevFee(tAmount).mul(currentRate)).sub(rCharity);
    ...
    function _takeFees(uint256 tAmount) private {
        (,,uint256 tLiquidity, uint256 tDev, uint256 tCharity) = _getTValues(tAmount);
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rDev = tDev.mul(currentRate);
        uint256 rCharity = tCharity.mul(currentRate);
        uint256 tVal = tDev.add(tLiquidity).add(tCharity);
        uint256 rVal = rDev.add(rLiquidity).add(rCharity);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _devFee == 0 && _charityFee == 0 && _burnFee == 0) return;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 devAmount = contractTokenBalance.mul(_devFee).div(_devFee+_liquidityFee+_charityFee);
            swapTokensForEth(devAmount,_devWallet);

            //Sending to charity wallet
            uint256 charityAmount = contractTokenBalance.mul(_charityFee).div(_devFee+_liquidityFee+_charityFee);
            swapTokensForEth(charityAmount,_charityWallet);

            //add liquidity
            uint256 liquidityAmount = contractTokenBalance.mul(_liquidityFee).div(_devFee+_liquidityFee+_charityFee);
