    ...
    function claimTokens() public {
        ...
        if (timePassed < cliff) {
            ...
            if (user.tokensClaimed == 0) {
                user.tokensClaimed = firstClaim;
                flag = true;
            }
            uint256 tokensToClaim = _amount.mul(time).div(totalPeriods).sub(user.tokensClaimed.sub(firstClaim));
            if (flag && user.tokensClaimed == firstClaim) {
                tokensToClaim = tokensToClaim.add(firstClaim);
            }
