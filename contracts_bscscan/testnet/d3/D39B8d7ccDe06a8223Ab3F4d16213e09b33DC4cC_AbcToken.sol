...
contract AbcToken is Context, IERC20, Ownable {
    ...
    address payable public _charityAddress =
        payable(0x9f0B027c674e1F728e09B79E7cC89FA1561e2794);
    address payable public _marketingAddress =
        payable(0x7788f28Bdc6bF455dDAE488DA9B8e08b7c4a0194);
    ...
    function setDefaultFees() external onlyOwner {
        ...
        contractRates |= bytes32(bytes4(uint32(3600))) >> 224;
    ...
    function getRate(uint8 index, uint8 subIndex)
        ...
        returns (uint32)
    {
        require(index <= 1, "subIndex to high");

        if (index == 1) {
            require(subIndex <= 9, "subIndex too high");

            return uint32(bytes4(_contractRates << (subIndex * 32)));
        } else {
            require(subIndex <= 15, "subIndex too high");

            return uint16(bytes2(_contractFees << (subIndex * 16)));
        }
    }
    ...
    function _transfer(
        ...
        if (!_lockSwap && _transferType != TransferType.Excluded) {
            ...
            tokenFees += charityFee + marketingFee + buyBackFee + liquidityFee;
            ...
            if (
                getToggle(2) && _transferType == TransferType.Sell
                // _minimumTokensForLiquidation >= balanceMinusSupport
            ) {
                (uint256 tokenA, uint256 tokenB, ) = IPancakePair(PancakePair)
                    .getReserves();
                // if (tokenA > tokenB) (tokenA, tokenB) = (tokenB, tokenA);
                uint32 commonDenominator = getCommonDenominator(amount);
                liquidateTokens(commonDenominator, amount);

                bool timeForNextSend = uint32(bytes4(_contractRates << 224)) +
                    _lastSellTime <=
                    block.timestamp;
                ...
                sendAndAddLiquidity(commonDenominator, amount, tokenA, tokenB);

                if (
                    getToggle(3) // Buyback/burn Tokens
                ) {
                    buybackAndBurn(amount - tokenFees, tokenA, tokenB);
                }
            } else {
                setTracker(0, 0, buyBackFee, true);
                setTracker(0, 1, charityFee, true);
                setTracker(0, 2, marketingFee, true);
                setTracker(0, 3, liquidityFee, true);
            }
        }
    ...
    function buybackAndBurn(
        ...
        if (getToggle(4) && burnNow && tokensBought > 0) {
            _burn(tokensBought);
        } else if (tokensBought > 0) {
    ...
    function liquidateTokens(uint32 commonDenominator, uint256 amount)
        private
        LockSwap
    {
        (uint16 charityRate, uint16 marketingRate, , ) = getBaseRates(amount);
        (, uint32 buybackRate, uint32 liquidityRate, , , ) = getSupportRates(
            amount
        );
        uint64 charityFee = (_minimumTokensForLiquidation *
            ((charityRate * DENOMINATOR) / commonDenominator)) / DENOMINATOR;
        uint64 marketingFee = (_minimumTokensForLiquidation *
            ((marketingRate * DENOMINATOR) / commonDenominator)) / DENOMINATOR;
        uint64 tokensToSell = charityFee;
        uint64 liquidityFee = (_minimumTokensForLiquidation *
            ((liquidityRate * DENOMINATOR) / commonDenominator)) / DENOMINATOR;
        uint64 buybackFee = (_minimumTokensForLiquidation *
            ((buybackRate * DENOMINATOR) / commonDenominator)) / DENOMINATOR;
        ...
        if (tokensToSell > 0) {
            ...
            setTracker(0, 0, buybackFee, false); // Remove Tokens for Burning
            setTracker(0, 1, charityFee, false); // Remove Tokens for Charity
            setTracker(0, 2, marketingFee, false); // Remove Tokens for Marketing
            setTracker(0, 3, liquidityFee, false); // Remove Tokens for LP
        }
    }

    function sendAndAddLiquidity(
        uint32 commonDenominator,
        uint256 amount,
        uint256 tokenA,
        uint256 tokenB
    ) private {
        ...
        ) = getSupportRates(amount);
        uint128 supportBNB;
        if (supportStreamRate > 0) {
            supportBNB =
                (_minimumBnbForLiquidation *
                    ((supportStreamRate * DENOMINATOR) / commonDenominator)) /
                DENOMINATOR;

            setTracker(
                1,
                2,
                (supportBNB * ((buybackRate * DENOMINATOR) /
                    commonDenominator)) /
                    DENOMINATOR,
                true
            );
        ...
        sendBNB(amount, supportStreamRate, supportBNB);

        if (liquidityRate > 0) {
            uint128 bnbLpFee = ((supportBNB * ((liquidityRate * DENOMINATOR))) /
                commonDenominator /
                DENOMINATOR);

            addLiquidity(bnbLpFee, tokenA, tokenB);
        }
    }

    function sellTokens(uint256 tokensToSell) private {
    ...
    function sendBNB(
        ...
        (uint16 charityRate, uint16 marketingRate, , ) = getBaseRates(amount);
        uint32 commonDenominator = charityRate +
            marketingRate +
            supportStreamRate;
        ...
        if (charityRate > 0)
            charityBNB = (((_minimumBnbForLiquidation - supportBNB) *
                ((charityRate * DENOMINATOR) / commonDenominator)) /
                DENOMINATOR);
        ...
        if (marketingRate > 0)
            marketingBNB = (((_minimumBnbForLiquidation - supportBNB) *
                ((marketingRate * DENOMINATOR) / commonDenominator)) /
                DENOMINATOR);
    ...
    function addLiquidity(
        ...
        if (
            ...
            getTokenFeeTracker(3) >= tokenQuote
        ) {
            ...
            setTracker(0, 3, uint64(tokenQuote), false);
        }
    }
    ...
    function getCommonDenominator(uint256 amount)
        private
        view
        returns (uint32)
    {
        (uint16 charityRate, uint16 marketingRate, , ) = getBaseRates(amount);
        (uint32 supportStreamRate, , , , , ) = getSupportRates(amount);
