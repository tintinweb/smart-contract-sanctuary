...
contract myltcfarm {
	...
	function invest(address referrer, uint8 plan, uint256 amounterc
        ...
        require(amounterc < INVEST_MIN_AMOUNT, "There is a minimum investment of 0.005 LTC.");
        require(amounterc >= plans[plan].mininvest);
        
        if(plan == 3 && getUserTotalDeposits(msg.sender) == 1.5 ether){
            isVIP = true;
        } else revert("User not qualified for VIP Plan.");
