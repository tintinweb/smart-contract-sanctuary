    ...
    */
    ...
    function _getTValues(ValuesFromAmount memory values, bool deductTransferFee) view private {
        ...
        if (deductTransferFee) {
            ...
            // amount after fee
            values.tTransferAmount = values.amount - values.tBurnFee - values.tRewardFee - values.tAdminFee - values.tOwnerFee - values.tLiquifyFee;
        }
    ...
    function _getRValues(ValuesFromAmount memory values, bool deductTransferFee) view private {
        ...
        if (deductTransferFee) {
            ...
            values.rTransferAmount = values.rAmount - values.rBurnFee - values.rRewardFee - values.rAdminFee - values.rOwnerFee - values.rLiquifyFee;
        }
...
contract Dowg is ExtendedReflections {

    uint256 private _tokenSupply = 1; // 1B

    uint32 private _taxOwner = 192;
    uint32 private _taxReward = 633;
    uint32 private _taxAdmin = 175;
    uint32 private _taxLiquify = 225;

    uint32 private _taxDecimals = 2;
    uint32 private _decimals = 4;
    ...
    constructor (address _adminAddress) ExtendedReflections("Dowg", "DOWG", _decimals, _tokenSupply) {
