...
contract Windmill_Deployer is Base, Adressable, Ownable, With_PancakeRouter, With_BUSDToken, With_WBNBToken, I_Windmill_Deployer{
    ...
    function initWindmill(uint256[12] calldata data,
        ...
        for (uint256 i=0; i<windmill.tradeDeployers.length; i++){
            if (keccak256(bytes(windmill.tradeDeployers[i].name())) == keccak256(bytes("Windmill_Trade_Long"))){
                I_Windmill_Trade_Deployer_Long deployer = I_Windmill_Trade_Deployer_Long(address(windmill.tradeDeployers[i]));
