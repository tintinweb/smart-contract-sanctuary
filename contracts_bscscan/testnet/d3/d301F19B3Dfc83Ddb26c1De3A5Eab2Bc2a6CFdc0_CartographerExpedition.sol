...
contract CartographerExpedition is Ownable, Initializable, ReentrancyGuard, ICartographerExpedition {
    ...
    mapping(address => uint8) public override userTotem;
    ...
    constructor(address _ICartographer) public onlyOwner {
    ...
    function enableSummit(SummitToken _summit) external onlyCartographer() override {
        summit = _summit;
    }
    ...
    modifier validTotem(uint8 totem) {
        require(totem < elevationHelper.getExpeditionTotemCount(), "Invalid totem");
        _;
    }
    ...
    // Add a new lp to the pool. Can only be called by the owner.
    function add(uint8 _pid, IBEP20 _rewardToken, uint256 _rewardAmount, uint256 _rounds, bool _withUpdate) external onlyCartographer() nonDuplicated(_rewardToken) override {
        ...
        uint256 roundDuration = elevationHelper.getExpeditionRoundDuration();
        
        registerPool(_pid, _rewardToken);

        uint256 rewardEmission = _rewardAmount.div(roundDuration.mul(_rounds).div(3));
        uint8 totemCount = elevationHelper.getExpeditionTotemCount();
    ...
    function rolloverExpedition() external onlyCartographer() override {
        uint256 currRound = elevationHelper.getElevationRound(EXPEDITION);
    ...
    function endRoundAndStartNext(uint8 _pid, uint256 currRound, uint256 nextRound) internal poolExistsAndEnabled(_pid) {
        ...
        uint256 winningTotemRoundRewards = pool.totemRoundRewards[elevationHelper.getRoundWinningTotem(EXPEDITION, currRound)];
        ...
        pool.roundRewards = 0;
        pool.totemRoundRewards = new uint256[](elevationHelper.getExpeditionTotemCount());
    }
    ...
    function getPendingReward(uint8 _pid, address _userAdd) external view onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) override returns (uint256) {
        ExpeditionPoolInfo storage pool = expeditionPoolInfo[_pid];
    ...
    function getHypotheticalReward(uint8 _pid, address _userAdd) external view poolExists(_pid) validUserAdd(_userAdd) override returns (uint256, uint256) {
        ...
        return (
            rew,                                                                // CURRENT: How much the user has farmed currently
            rew.mul(liveRoundRewards).div(liveTotemRoundRewards)    // IF WIN: User rewards * (total round rewards / totem rewards)
        );
    }
    ...
    function getRoundWinnings(ExpeditionPoolInfo storage pool, UserInfo storage user, uint256 roundIndex, address userAdd) internal view returns (uint256) {
        if (userTotem[userAdd] != elevationHelper.getRoundWinningTotem(EXPEDITION, roundIndex)) { return 0; }
    ...
    function getExpeditionPendingRewards(ExpeditionPoolInfo storage pool, UserInfo storage user, address userAdd) internal view poolExists(pool.pid) returns (uint256) {
        ...
        if (user.staked == 0 || user.prevInteractedRound == currRound) { return 0; }
    ...
    function updateUserRoundInteraction(ExpeditionPoolInfo storage pool, UserInfo storage user, uint256 amount, bool isDeposit) internal {
        // Deposit Only
        // If initializing the users mid round reward
        uint256 currRound = elevationHelper.getElevationRound(EXPEDITION);
        if (isDeposit && user.staked == 0) {
            user.roundRew = 0;
            user.staked = amount;
            user.roundDebt = amount.mul(pool.accRewPerShare).div(1e12);
            return;
        }
    ...
    function switchAllTotems(uint8 _newTotem, address _userAdd) external nonReentrant onlyCartographer() validTotem(_newTotem) validUserAdd(_userAdd) expeditionNotLockedUntilRollover() override {
    ...
    function getIsTotemInUse(address _userAdd) external view override returns (bool) {
    ...
    function switchTotem(uint8 _pid, uint8 _newTotem, address _userAdd) internal {
        ...
        user.prevInteractedRound = elevationHelper.getElevationRound(EXPEDITION);
    }


    // Deposit SUMMIT tokens in Expedition pool to earn rewardToken
    function deposit(uint8 _pid, uint256 _amount, uint8 _totem, address _userAdd) external nonReentrant onlyCartographer() poolExistsAndEnabled(_pid) validTotem(_totem) validUserAdd(_userAdd) expeditionNotLockedUntilRollover() override returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
    ...
    function withdraw(uint8 _pid, uint256 _amount, address _userAdd) external nonReentrant onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) override expeditionNotLockedUntilRollover() returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
    ...
    function emergencyWithdraw(uint8 _pid, address _userAdd) external nonReentrant onlyCartographer() poolExistsAndEnabled(_pid) validUserAdd(_userAdd) override expeditionNotLockedUntilRollover() returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
