...
//  uint public constant blockInOneMonth = 864000; // 30 * 24 * 60 * 20
  ...
  function releasePrivateSaleAllocation(address _buyerAddress, uint _amount) external onlyPrivateSaleContract returns (bool) {
...
contract TokenAuth is Context {
  ...
  modifier onlyPrivateSaleContract() {
    require(_msgSender() == privateSaleContract, "TokenAuth: invalid caller");
    _;
  }
