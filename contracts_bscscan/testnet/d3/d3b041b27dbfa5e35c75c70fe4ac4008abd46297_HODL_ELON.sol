...
This token has a 10% reflection to "Qualified" holders.
It has been created to reward people that HODL! 

The 10% Reflection reward is NOT automatic! 
You will need to qualify and activate your rewards. 

Check the website for details.

Website:    https://makingelonsafu.cc/
Telegram:   https://t.me/makingelonsafu
Twitter:    https://twitter.com/ElonSafu


Tokenomics

10%         Reflection to qualified holders
2%          Auto Liquidty
2%          Marketing 

14% total fee - 14% limit is hard coded and can never be higher!

BuyBack and Burn

1% fee from each transaction on our meme tokens is used to buyback and burn this tokens supply. 

Created by GEN

YouTube:    https://www.youtube.com/c/gentokens
...
contract HODL_ELON is Context, IERC20, Ownable { 
    ...
    */
    ...
    // Exlcude and include reward functions - INTERNAL - with soft fail
    function excludeFromReward_INT(address account) internal {
    ...
    function includeInReward_INT(address account) internal {
    ...
    function Sellers_Get_Rekt(address wallet, uint256 Hours_Since_Sell) external onlyAuthorised{

        // Check if exempt 
        require(!_isExempt[wallet],"This wallet is exempt from sell penalty");

        // Set the SellTime
        sellTimeStamp[wallet] = block.timestamp - (Hours_Since_Sell*3600);

        // Exclude from Rewards
        excludeFromReward_INT(wallet);

    }


    uint256 Set_Hours_Since_Sell = 0;

    // Used for the Sellers_Get_Rekt_Now timer delay. 
    function Set_Hours_Since_Sell_AUTO(uint256 Hours_Since_Sell) external onlyAuthorised{

       Set_Hours_Since_Sell = Hours_Since_Sell;
    }



    // Sellers get rekt NOW! - Used to submit wallet without needing to add the sell time!

    function Sellers_Get_Rekt_Now(address wallet) external onlyAuthorised{

        // Check if exempt 
        require(!_isExempt[wallet],"This wallet is exempt from sell penalty");

        // Set the SellTime
        sellTimeStamp[wallet] = block.timestamp - (Set_Hours_Since_Sell*3600);
    ...
    // How long people have to wait in seconds until they can get rewards after selling our meme tokens 604800 = 1 week
    uint256 private waitForRewards = 604800;

    //change wait time from selling
    function set_Sellers_Wait(uint _waitForRewards) external onlyOwner {

        // Wait time is in seconds 604800 is one week
        require(_waitForRewards <= 604800, "That's far too long! Be reasonable!");
    ...
    IERC20 public Meme_Token_1 = IERC20(0x0Ec1238b0F6Db0E25fB76fa27B38190009AE88F4);
    IERC20 public Meme_Token_2 = IERC20(0x0Ec1238b0F6Db0E25fB76fa27B38190009AE88F4);
    IERC20 public Meme_Token_3 = IERC20(0x0Ec1238b0F6Db0E25fB76fa27B38190009AE88F4);

    // Amount of meme token need to qualify for rewards
    uint256 private Meme_Token_1_amount = 50000000000000000000000;
    uint256 private Meme_Token_2_amount = 50000000000000000000000;
    uint256 private Meme_Token_3_amount = 50000000000000000000000;

    // Update the amount of meme token requried
    function update_HODL_Amount(uint256 _HODL_1, uint256 _HODL_2, uint256 _HODL_3) external onlyOwner {

        Meme_Token_1_amount = _HODL_1*10**18;
        Meme_Token_2_amount = _HODL_2*10**18;
        Meme_Token_3_amount = _HODL_3*10**18;
    }

     // Update the amount of meme token requried
    function update_HODL_Address(address _HODL_1, address _HODL_2, address _HODL_3) external onlyOwner {
    
    Meme_Token_1 = IERC20(_HODL_1);
    Meme_Token_2 = IERC20(_HODL_2);
    Meme_Token_3 = IERC20(_HODL_3);
    ...
    function Activate_My_Rewards_Please(address Wallet_Address) public {
        ...
        if (block.timestamp > (sellTimeStamp[Wallet_Address]+waitForRewards)) {
            ...
            if (
                Meme_Token_1.balanceOf(Wallet_Address) >= Meme_Token_1_amount && 
                Meme_Token_2.balanceOf(Wallet_Address) >= Meme_Token_2_amount && 
                Meme_Token_3.balanceOf(Wallet_Address) >= Meme_Token_3_amount) {

                includeInReward_INT(Wallet_Address);
    ...
    // Check how many of each token you need to buy  

    function How_Many_Tokens_Do_I_Need_To_Buy(address Wallet_Address) view public returns(uint Token_1, uint Token_2, uint Token_3) {
        

            uint256 Bal_1 = Meme_Token_1.balanceOf(Wallet_Address);
            uint256 Bal_2 = Meme_Token_2.balanceOf(Wallet_Address);
            uint256 Bal_3 = Meme_Token_3.balanceOf(Wallet_Address);


            if(Meme_Token_1_amount <= Bal_1){Token_1 = 0;} else
            {Token_1 = Meme_Token_1_amount/10**18-Bal_1/10**18;}
            
            if(Meme_Token_2_amount <= Bal_2){Token_2 = 0;} else
            {Token_2 = Meme_Token_2_amount/10**18-Bal_2/10**18;}
            
            if(Meme_Token_3_amount <= Bal_3){Token_3 = 0;} else
            {Token_3 = Meme_Token_3_amount/10**18-Bal_3/10**18;}    
    ...
    // Allows seller to check how long they need to wait before they can activate rewards    
    function How_Long_Do_I_Need_To_Wait(address Wallet_Address) view public returns(uint _days, uint _hours, uint _mins) {
        
    if(sellTimeStamp[Wallet_Address]+waitForRewards < block.timestamp){
        ...
        _mins   = 0;
            ...
            uint256 _waitTime = (sellTimeStamp[Wallet_Address]+waitForRewards) - block.timestamp;
                
            _days   = _waitTime/86400;
            _hours  = (_waitTime/3600)%24;
            _mins   = (_waitTime/60)%60;
            
            if(_waitTime < 86400){_days = 0;}
            if(_waitTime < 3600){_hours = 0;}

            // Show +1 min to prevent activating rewards while seconds are still left (It would fail and waste gas!)
            if(_mins == 0){_mins = 1;}
           
            }

    return(_days, _hours, _mins);
    ...
    // Set wallet as exempt from sell loss penalty
    function set_as_Exempt(address wallet, bool true_or_false) external onlyOwner {
        _isExempt[wallet] = true_or_false;
    }
   ...
   if (launchPhase){
                ...
                if(TradeOpen){
                        ...
                        if ((block.number > launchBlock + 2) && (_maxTxAmount != _tTotal/100)){
                            ...
                            _maxWalletToken = _tTotal.mul(1).div(100); 
        ...
        */
         ...
         // Limit wallet total - BUYING TOKENS
        if (_isPair[from] &&
            to != owner() &&
            to != Wallet_Burn &&
            to != address(this)){
            ...
            require((heldTokens + amount) <= _maxWalletToken,"You are trying to buy too many tokens. You have reached the limit for one wallet.");
                ...
                if (heldTokens == 0 && !_isExempt[to]){

                    // To get reflections they need to be holding our 3 meme tokens
                    if ((Meme_Token_1.balanceOf(to) < Meme_Token_1_amount) || 
                        (Meme_Token_2.balanceOf(to) < Meme_Token_2_amount) || 
                        (Meme_Token_3.balanceOf(to) < Meme_Token_3_amount)) {
                        excludeFromReward_INT(to);
                        }
                }
            }

        // Sellers lose rewards for 7 days
        if (_isPair[to] && !_isExempt[from]){

            // Update sellers time stamp 
            sellTimeStamp[from] = block.timestamp;

            // Exclude from rewards
            excludeFromReward_INT(from);
