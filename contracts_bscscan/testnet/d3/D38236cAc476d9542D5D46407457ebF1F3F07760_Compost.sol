...
contract Compost is Ownable {
    ...
    address router2 = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    address public bnbAddress = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    
    uint256 public FeeFactor = 100; //    --  ((ExtraBNB - FeeFactor) / 2) sent back to user
    ...
    function _Setbnb(address _bnbAddress) external onlyOwner {
    ...
    uint256 QTTokenAmount = 0;
        ...
        // perform swap two
        if (swapAmount > 0) {
            if (_TokenB == bnbAddress || router != router2) {
                ...
                if (router != router2 && _TokenB != bnbAddress){
            ...
            if ( _TokenH != _TokenA) {
                ...
                if (_TokenB == bnbAddress) {
                    ...
                    path3[1] = RA2.WETH();
                    path3[2] = _TokenA;
                    RA2.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                    swapAmount,
                    0,
                    path3,
                    address(this),
    ...
    if (_TokenB == bnbAddress){
        ...
        function checkAllowance1(address _Token, uint256 _amount, address _router) private {
            if (IERC20(_Token).allowance(address(this),_router) < _amount){
                    IERC20(_Token).approve(_router,(_amount * 2));
                }
        }
