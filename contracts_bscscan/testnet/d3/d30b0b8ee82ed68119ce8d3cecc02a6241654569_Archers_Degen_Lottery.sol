...
function random(uint t, uint x, uint y, uint z) private view returns (uint) {
    return uint(keccak256(abi.encodePacked(t, x, y, z, players)));
    }

function pickWinner(uint chaos, uint entropy, uint disorder, uint randomness) public restricted {
    //Random numbers generated @ https://www.random.org/integers/ before drawing
    uint t = chaos;
    uint x = entropy;
    uint y = disorder;
    uint z = randomness;
