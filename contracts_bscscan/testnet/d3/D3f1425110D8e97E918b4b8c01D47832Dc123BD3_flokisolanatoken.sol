...
contract flokisolanatoken is Context, IERC20, Ownable, Pausable {
    ...
    string private _name = 'Floki Solana Token';
    string private _symbol = 'FSL';
    uint8 private _decimals = 18;

    uint256 public genesisReward;
    uint256 public constant AIRDROP_AMOUNT = 4*10**6*10**18; //4,000,000
    uint256 public constant PRESALE_AMOUNT = 2*10**7*10**18; //5,000,000
    uint256 public aidropDistributed;
    uint256 public presaleDistributed;
    uint256 public constant BASE_MINT = 2 * 10**6 * 10**18;
    ...
    constructor () public {
        ...
        uint256 amount =  BASE_MINT*100; //200m
        _tTotal = amount;
    ...
    function distributeAirdrop(address[] memory _receivers, uint _value) public onlyOwner {
        ...
        for(uint i = 0; i < _receivers.length; i++){
            _rOwned[sender] = _rOwned[sender].sub(_value.mul(_currentRate));
            _rOwned[_receivers[i]] =  _rOwned[_receivers[i]].add(_value.mul(_currentRate));
            emit Transfer(sender, _receivers[i], _value);
        }
    }
    
    function distributePresale(address _presaleAddress, uint256 amount) public onlyOwner {
        require(amount <= PRESALE_AMOUNT, "Cannot SEND this amount");
        uint256 _currentRate = _getRate();
        address sender = _msgSender();
         _rOwned[sender] = _rOwned[sender].sub(amount.mul(_currentRate));
         _rOwned[_presaleAddress] = _rOwned[_presaleAddress].add(amount.mul(_currentRate));
        
        emit Transfer(sender, _presaleAddress, amount);
    }
    ...
    function _burn(uint256 amount) public {
        require(amount < BASE_MINT * 100, "Cannot burn this amount");
        address sender = _msgSender();
        uint256 _rate = _getRate();
        _rOwned[sender] =  _rOwned[sender].sub(amount.mul(_rate));
        _rOwned[address(0x0000dead)] = _rOwned[address(0x0000dead)].add(amount.mul(_rate));
        emit Transfer(sender, address(0x0000dead), amount);
