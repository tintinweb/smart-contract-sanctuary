...
import "../interfaces/alpaca/IIBToken.sol";
import "../interfaces/alpaca/IIBTokenConfig.sol";
import "../interfaces/alpaca/IAlpacaInterestRateModel.sol";
import "../interfaces/alpaca/IFairLaunchV2.sol";
import "./BaseRhoStrategy.sol";
import "../interfaces/ITokenExchange.sol";
import "../interfaces/IPriceOracle.sol";

contract AlpacaStrategy is BaseRhoStrategy {
    using SafeERC20Upgradeable for IERC20MetadataUpgradeable;

    string public constant override NAME = "Alpaca";

    IIBToken public ibToken;
    uint256 private ibTokenPid;
    IERC20MetadataUpgradeable public alpaca;
    IAlpacaInterestRateModel public interestRateModel;
    IFairLaunchV2 public fairLaunchV2;

    IPriceOracle public alpacaUsdPriceOracle;
    ...
    function initialize(
        ...
        uint256 _ibTokenPid,
        address _interestRateModelAddr,
        address _fairLaunchV2Addr,
        address _tokenExchange,
        address _alpacaAddr,
        address _alpacaUsdPriceOracleAddr,
        uint256 _lockDurationInBlock,
        uint256 _blocksPerYear
    ) external initializer {
        BaseRhoStrategy.__initialize(_underlyingAddr, _lockDurationInBlock);
        ibToken = IIBToken(_ibTokenAddr);
        ibTokenPid = _ibTokenPid;
        interestRateModel = IAlpacaInterestRateModel(_interestRateModelAddr);
        fairLaunchV2 = IFairLaunchV2(_fairLaunchV2Addr);
        alpaca = IERC20MetadataUpgradeable(_alpacaAddr);
        alpacaUsdPriceOracle = IPriceOracle(_alpacaUsdPriceOracleAddr);
        BLOCK_PER_YEAR = _blocksPerYear;
        tokenExchange = ITokenExchange(_tokenExchange);
    }

    /**
     * @notice current supply rate per block excluding bonus token ALPACA
     ...
     */
    function _supplyRate(uint256 deposit) private view returns (uint256) {
        uint256 debt = ibToken.vaultDebtVal();
        // Alpaca charge 19% for lending performance fee
        return (interestRateModel.getInterestRate(debt, deposit - debt) * 365 days * 81e16 * debt) / deposit / 1e18;
    }

    function _supplyRate() private view returns (uint256) {
        return _supplyRate(ibToken.totalToken());
    }

    /**
     * @notice current supply rate excluding bonus token ALPACA
    ...
    /**
     * @dev calculates the ALPACA bonus (in terms of USD) per block  as follows:
     * NOTE: The steps are packed in one line to retain precision.
     * 1. alpacaPerBlockPerIbToken = (fairLaunchV2.alpacaPerBlock() * fairLaunchV2.poolInfo(ibTokenPid).allocPoint * 1e18) / fairLaunchV2.totalAllocPoint() / stake;
     * 2. alpacaPerBlockPerUnderlying = (alpacaPerBlockPerIbToken * ibToken.totalSupply()) / ibToken.totalToken();
     * 3. usdPerBlockPerUnderlying = (alpacaPerBlockPerUnderlying * alpacaUsdPriceOracle.price(address(alpaca))) / 10**alpacaUsdPriceOracle.decimals();
     */
    function _bonusRatePerBlock(uint256 stake) internal view returns (uint256) {
        uint256 alpacaPerBlockPerUnderlying = (fairLaunchV2.alpacaPerBlock() *
            fairLaunchV2.poolInfo(ibTokenPid).allocPoint *
            1e18 *
            ibToken.totalSupply()) /
            fairLaunchV2.totalAllocPoint() /
            stake /
            ibToken.totalToken();
        return
            (alpacaPerBlockPerUnderlying * alpacaUsdPriceOracle.price(address(alpaca))) /
            10**alpacaUsdPriceOracle.decimals();
    }

    function _bonusPerBlockPerUnderlying() internal view returns (uint256 usdPerBlockPerUnderlying) {
        return _bonusRatePerBlock(ibToken.balanceOf(address(fairLaunchV2)));
    }

    /**
     * @notice current bonus rate per block for bonus token ALPACA
    ...
    /**
     * @notice current bonus supply rate for bonus token ALPACA
    ...
    /**
     * @notice effective supply rate of AlpacaStrategy
     * @dev returns the effective supply rate from Alpaca protocol
     * taking into account rewards token ALPACA
    ...
    function effectiveSupplyRate() external view override returns (uint256) {
        return _supplyRate() + this.bonusSupplyRate();
    }

    /**
     * @notice effective supply rate of AlpacaStrategy
     * @dev returns the effective supply rate from Alpaca protocol
     * taking into account rewards token ALPACA AND the change in deployed amount.
    ...
    function effectiveSupplyRate(uint256 delta, bool isPositive) external view override returns (uint256) {
        uint256 newDeposit = ibToken.totalToken();
        newDeposit = isPositive ? newDeposit + delta : newDeposit - delta;

        uint256 newStake = ibToken.balanceOf(address(fairLaunchV2));
        uint256 ibDelta = (delta * ibToken.totalSupply()) / ibToken.totalToken();
        newStake = isPositive ? newStake + ibDelta : newStake - ibDelta;

        return _supplyRate(newDeposit) + _bonusRatePerBlock(newStake) * BLOCK_PER_YEAR;
    }

    function _balanceOfUnderlying() internal view returns (uint256) {
        return ((fairLaunchV2.userInfo(ibTokenPid, address(this)).amount *
            (ibToken.totalToken() +
                ibToken.pendingInterest(0) -
                ((ibToken.pendingInterest(0) * IIBTokenConfig(ibToken.config()).getReservePoolBps()) / 10000))) /
            ibToken.totalSupply());
    }

    /**
     * @dev view function to return balance in underlying,
     * @return balance (interest included) from Alpaca protocol, in terms of underlying (in wei)
     */
    ...
    /**
     * @dev deploy the underlying to Alpaca, deposit and stake ibTokens
     * @param _amount amount of underlying (in wei) to deploy
     */
    function deployInternal(uint256 _amount) internal override {
        underlying.safeIncreaseAllowance(address(ibToken), _amount);
        ibToken.deposit(_amount);
        uint256 ibTokenBalance = ibToken.balanceOf(address(this));
        ibToken.increaseAllowance(address(fairLaunchV2), ibTokenBalance);
        fairLaunchV2.deposit(address(this), ibTokenPid, ibTokenBalance);
    }

    /**
     * @notice not all of the funds deployed to a strategy might be available for withdrawal
     * @return the amount of underlying tokens available for withdrawal from Alpaca strategy
     */
    function underlyingWithdrawable() public view override returns (uint256) {
        uint256 fairLaunchCash = ibToken.balanceOf(address(fairLaunchV2));
        uint256 fairLaunchBalance = fairLaunchV2.userInfo(ibTokenPid, address(this)).amount;
    ...
    /**
     * @dev Withdraw the amount in underlying from Alpaca protocol by unstake and withdraw ibTokens and transfer to vault
     * @notice Alpaca does not have function to withdraw underlying, need to convert amount in underlying to ibTokens, there is rounding error for conversion
    ...
    function withdrawUnderlyingInternal(uint256 _amount) internal override {
        uint256 ibAmount = (_amount * ibToken.totalSupply()) /
            (ibToken.totalToken() +
                ibToken.pendingInterest(0) -
                (ibToken.pendingInterest(0) * IIBTokenConfig(ibToken.config()).getReservePoolBps()) /
                10000);
        fairLaunchV2.withdraw(address(this), ibTokenPid, ibAmount);
        ibToken.withdraw(ibAmount);
        underlying.safeTransfer(_msgSender(), underlying.balanceOf(address(this)));
    }

    /**
     * @dev Withdraw all underlying from Alpaca protocol by unstake and withdraw ibTokens and transfer to vault
     */
    function withdrawAllCashAvailableInternal() internal override {
        uint256 balance = _balanceOfUnderlying();
        uint256 withdrawable = underlyingWithdrawable();
        if (balance > withdrawable) emit StrategyOutOfCash(balance, withdrawable);
        if (withdrawable == 0) {
            return;
        }
        fairLaunchV2.withdrawAll(address(this), ibTokenPid);
        ibToken.withdraw(ibToken.balanceOf(address(this)));
        underlying.safeTransfer(_msgSender(), underlying.balanceOf(address(this)));
    }

    /**
     * @dev Collect ALPACA tokens available and sell into underlying for the strategy
     */
    function collectRewardToken() external override onlyRole(VAULT_ROLE) whenNotPaused nonReentrant {
        if (address(alpaca) != address(0) && address(tokenExchange) != address(0)) {
            if (fairLaunchV2.userInfo(ibTokenPid, address(this)).amount != 0) {
                fairLaunchV2.harvest(ibTokenPid);
            }

            alpaca.safeIncreaseAllowance(address(tokenExchange), alpaca.balanceOf(address(this)));
            tokenExchange.sellExactInput(alpaca, underlying, _msgSender(), alpaca.balanceOf(address(this)));
        }
    }

    /**
     * @return address of ALPACA token contract
     */
    function bonusToken() external view override returns (address) {
        return address(alpaca);
    }

    /**
     * @return ALPACA tokens accrued
     */
    function bonusTokensAccrued() external view override returns (uint256) {
        return fairLaunchV2.pendingAlpaca(ibTokenPid, address(this));
    }

    /**
     * @param rewardCollectThreshold minimum threshold for collecting ALPACA token
    ...
    function shouldCollectReward(uint256 rewardCollectThreshold) external view override returns (bool) {
        uint256 alpacaPrice = alpacaUsdPriceOracle.price(address(alpaca));
        uint8 alpacaPriceDec = alpacaUsdPriceOracle.decimals();
        uint256 alpacaQty = alpaca.balanceOf(address(this)) + fairLaunchV2.pendingAlpaca(ibTokenPid, address(this));
        uint256 minUnderlyingQty = (alpacaQty * alpacaPrice * 10**underlying.decimals()) /
            10**(alpaca.decimals() + alpacaPriceDec);
    ...
    function sweepERC20Token(address token, address to) external override onlyRole(SWEEPER_ROLE) {
        ...
        require(token != address(alpaca), "!safe");
        super._sweepERC20Token(token, to);
    }

    /**
     * @return amount of underlying holds by Alpaca (ibTokens)
     */
    function getCash() internal view override returns (uint256) {
        return underlying.balanceOf(address(ibToken));
    }
}
