...
interface IBEP20 {
    ...
    //_name = name_; "Scuubydoo" ;
        //_symbol = symbol_; "Scuubydoo" ;
        //_decimals = decimals_; 19;
...
contract ScuubydooDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("Scuubydoo_Dividend_Tracker", "Scuubydoo_Dividend_Tracker", 9) {
        claimWait = 14400;
        minimumTokenBalanceForDividends = 1000000 * (10**9); //must hold 10000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "Scuubydoo_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "Scuubydoo_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Scuubydoo contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Scuubydoo_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Scuubydoo_Dividend_Tracker: Cannot update claimWait to same value");
...
contract ScuubydooToken is BEP20, Ownable {
    using SafeMath for uint256;

    IPancakeswapV2Router02 public pancakeswapV2Router;
    address public immutable pancakeswapV2Pair;
    address public marketingWallet = 0xB7E57b802Dd5e190c58F53dF67D3642a6b51D24b;

    bool private swapping;

    ScuubydooDividendTracker public dividendTracker;
    ...
    constructor() public BEP20("Scuubydoo", "Scuubydoo", 9) {
...
//ScoTest123 "SCOTEST", 9) {
        dividendTracker = new ScuubydooDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Scuubydoo: The dividend tracker already has that address");

        ScuubydooDividendTracker newDividendTracker = ScuubydooDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Scuubydoo: The new dividend tracker must be owned by the Scuubydoo token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "Scuubydoo: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Scuubydoo: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "Scuubydoo: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Scuubydoo: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Scuubydoo: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Scuubydoo: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Scuubydoo: Cannot update gasForProcessing to same value");
