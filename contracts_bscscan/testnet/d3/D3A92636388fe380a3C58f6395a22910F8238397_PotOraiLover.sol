...
library SortitionSumTreeFactory {
    ...
    function draw(SortitionSumTrees storage self, bytes32 _key, uint _drawnNumber) public returns (bytes32 ID) {
        ...
        uint256 temp = tree.nodes[treeIndex];
        tree.nodes[treeIndex] = 0;
        updateParents(self, _key, treeIndex, false, temp);
    }
...
contract PotController is Whitelist {
    ...
    function getTreeNodeLength(bytes32 key) public view returns (uint256){
        return _sortitionSumTree.sortitionSumTrees[key].nodes.length;
    }

    function getAccount(bytes32 key, uint256 _index) public view returns (address){
        return address(uint(_sortitionSumTree.sortitionSumTrees[key].nodeIndexesToIDs[_index]));
    }
...
contract SignData {
    ...
    bytes32 public NEXT_TO_STEP_HASH;
    ...
    function initSignData() internal {
        ...
        DEPOSIT_HASH = keccak256("Data(address to,uint256 amount,uint256 nonce)");
        WITHDRAW_HASH = keccak256("Data(address to,uint256 nonce)");
        NEXT_TO_STEP_HASH = keccak256("Data(address to,uint256 nonce)");
    }
...
contract PotOraiLover is PotController, SignData {//Valcontroller
    ...
    uint private WEIGHT_BASE;
    uint public WINNER_COUNT;
    /* ========== STATE VARIABLES ========== */
    ...
    modifier onlyValidDeposit(uint amount, address account) {
        require(_available[account] == 0 || _depositedAt[account] >= startedAt, "BunnyPot: cannot deposit before claim");
        require(amount >= minAmount && amount.add(_available[account]) <= maxAmount, "BunnyPot: invalid input amount");
        if (_available[account] == 0) {
    ...
    function initializePotOraiLover(IBEP20 _stakingToken, IVenusStrategy _strategy, uint256 _min, uint256 _max) public {
    ...
    function weightOf(address _account) public view returns (uint) {
        return _totalWeight > 0 ? getWeight(_getTreeKey(), bytes32(uint256(_account))) : 0;
    }
    ...
    function potInfoOf(address _account) public view returns (PotConstant.PotInfo memory, PotConstant.PotInfoMe memory) {
        ...
        info.rewards = VENUS_STRATEGY.reward(address(this)).mul(100 - burnRatio).div(100);
        ...
        infoMe.weight = _totalWeight > 0 ? getWeight(_getTreeKey(), bytes32(uint256(_account))) : 0;
    ...
    /* ========== MUTATIVE FUNCTIONS ========== */
    //Đã sửa theo VENUS
    function depositPermit(address account, uint amount, uint8 v, bytes32 r, bytes32 s)
    public onlyValidState(PotConstant.PotState.Opened) onlyValidDeposit(amount, account) {
        verify(keccak256(abi.encode(DEPOSIT_HASH, account, amount, nonces[account]++)), account, v, r, s);
        ...
        uint weightCurrent = _calculateWeight(account, amount);
    ...
    //Giữ lại hết
    function stepToNext(address account, uint8 v, bytes32 r, bytes32 s) public onlyValidState(PotConstant.PotState.Opened) {
        verify(keccak256(abi.encode(NEXT_TO_STEP_HASH, account, nonces[account]++)), account, v, r, s);
        ...
        uint weightCurrent = _calculateWeight(account, available);
    ...
    //Đã sửa theo VENUS
    function withdrawAll(address account, uint8 v, bytes32 r, bytes32 s) public {
        verify(keccak256(abi.encode(WITHDRAW_HASH, account, nonces[account]++)), account, v, r, s);
        uint amount = _available[account];
    ...
    function setWinnerCount(uint256 _winnerCount) external onlyOwner onlyValidState(PotConstant.PotState.Opened) {
    ...
    function setRandomSeed(string calldata _randomSeed) external onlyOwner onlyValidState(PotConstant.PotState.Closed) {
        randomSeed = _randomSeed;
    }

    function overCook() external onlyOwner onlyValidState(PotConstant.PotState.Closed) {
        state = PotConstant.PotState.Cooked;
        _getRandomNumber(randomSeed);
    }
    ...
    function _getRandomNumber(string memory _randomSeed) private {
        bytes32 _structHash;
    ...
    function _overCook() private {
        ...
        uint256 _totalHarvested = VENUS_STRATEGY.claimAll(address(this));
    ...
    //Giữ lại hết
    function _calculateWeight(address account, uint256 _newBalance) public view returns (uint) {
        if (_depositedAt[account] < startedAt) return 0;
        uint256 weightBefore = getWeight(_getTreeKey(), bytes32(uint256(account)));
        uint wTime = _timeWeight(account);
        uint wCount = _countWeight(account);
        uint wValue = _valueWeight(account, _newBalance);
        return wTime.mul(wCount).mul(wValue).div(100).add(weightBefore);
    }

    //Giữ lại hết
    function _timeWeight(address account) public view returns (uint) {
        if (_depositedAt[account] < startedAt) return 0;
    ...
    //Giữ lại hết
    function _countWeight(address account) public view returns (uint) {
    ...
    //Giữ lại hết
    function _valueWeight(address account, uint256 _newBalance) private view returns (uint) {
        uint denom = Math.max(minAmount, 1);
        return Math.min(_newBalance.mul(10).div(denom), maxAmount.mul(10).div(denom));
    }

    //Giữ lại hết
    function _getTreeKey() public view returns (bytes32) {
