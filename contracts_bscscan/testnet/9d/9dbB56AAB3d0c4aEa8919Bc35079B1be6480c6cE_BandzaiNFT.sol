...
import "./SignerChecker.sol";
import "./Interfaces.sol";

// Main NFT monster/card contract

contract BandzaiNFT is ERC721Enumerable, Ownable {
    using SafeMath for uint256;

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    constructor() ERC721("BandZai_NFT", "ZAI") {}
    ...
    // A monster can work in a center , training or coaching. He can't fight if he isn't free
    string[4] _status = ["Free","Training","Coaching","Working"];
    mapping(uint256 => uint256) _statusID;
    mapping(uint256 => uint256) _onCenter;

    mapping(uint256 => Monster) public monster;
    mapping(uint256 => string) IpfsPath;
    mapping(string => bool) _isMinted;

    // Use for front end stat about all powers in game
    Powers public totalPowersInGame;

    uint256 public xpUpBase = 10000;
    // each level xp needed to raise another level is multiply by 1.05
    uint256 public multiplierXpUp = 105;

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        return string(abi.encodePacked("https://ipfs.io/ipfs/", IpfsPath[tokenId]));
    }
    ...
    function mintMonster(
        ...
        require(SignerChecker.recoverSigner(_ipfsPath, _signature) == gameAddresses.getAuthorizedSigner(),"Signer !Auth");
        require(!_isMinted[_ipfsPath], "Already minted");
        _isMinted[_ipfsPath] = true;

        _tokenIds.increment();
        uint256 _newItemId = _tokenIds.current();
        _safeMint(_to, _newItemId);
        IpfsPath[_newItemId] = _ipfsPath;

        // All monsters are created with 6 powers points in a random distribution
        uint256[6] memory randoms = _generateRandomDatas(_ipfsPath, _to, _newItemId);

        Monster storage m = monster[_newItemId];
        m.state = _state;
        m.name = _name;
        for(uint256 i = 0; i < randoms.length; i++){
            if(randoms[i].mod(5) == 0 ){
                m.powers.water = m.powers.water.add(1);
                totalPowersInGame.water = totalPowersInGame.water.add(1);
            }
            if(randoms[i].mod(5) == 1 ){
                m.powers.fire = m.powers.fire.add(1);
                totalPowersInGame.fire = totalPowersInGame.fire.add(1);
            }
            if(randoms[i].mod(5) == 2 ){
                m.powers.metal = m.powers.metal.add(1);
                totalPowersInGame.metal = totalPowersInGame.metal.add(1);
            }
            if(randoms[i].mod(5) == 3 ){
                m.powers.air = m.powers.air.add(1);
                totalPowersInGame.air = totalPowersInGame.air.add(1);
            }
            if(randoms[i].mod(5) == 4 ){
                m.powers.stone = m.powers.stone.add(1);
                totalPowersInGame.stone = totalPowersInGame.stone.add(1);
            }
        }
        require(ILevelStorage(gameAddresses.getLevelStorageAddress()).addFighter(0,_newItemId));
        return (_newItemId);
    }
    ...
    function updateXp(
        ...
        if (_level > m.level) {
            // update new level
            uint256 _numberOfLevelUp = _level.sub(m.level);
            // monster win 3 points by level raised
            m.creditForUpgrade = m.creditForUpgrade.add(_numberOfLevelUp.mul(3));
            // monster update from level storage
            ILevelStorage I = ILevelStorage(gameAddresses.getLevelStorageAddress());
            require(I.removeFighter(m.level,_id));
            require(I.addFighter(_level,_id));
    ...
    function updatePowers(
        ...
        uint256 _toSub =_water.add(_fire).add(_metal).add(_air).add(_stone);
        m.creditForUpgrade = m.creditForUpgrade.sub(_toSub,"Not enough pts");
       
       if(_water >0){
        m.powers.water = m.powers.water.add(_water);
        totalPowersInGame.water = totalPowersInGame.water.add(_water);
       }
       if(_fire >0){
        m.powers.fire = m.powers.fire.add(_fire);
        totalPowersInGame.fire = totalPowersInGame.fire.add(_fire);
       }
       if(_metal >0){
        m.powers.metal = m.powers.metal.add(_metal);
        totalPowersInGame.metal = totalPowersInGame.metal.add(_metal);
       }
       if(_air >0){
        m.powers.air = m.powers.air.add(_air);
        totalPowersInGame.air = totalPowersInGame.air.add(_air);
       }
       if(_stone >0){
        m.powers.stone = m.powers.stone.add(_stone);
        totalPowersInGame.stone = totalPowersInGame.stone.add(_stone);
       }
    }

    function updateAlchemyXp(uint256 _tokenId, uint256 _xpRaised) external onlyAuth{
        monster[_tokenId].alchemyXp = monster[_tokenId].alchemyXp.add(_xpRaised); 
    }

    function updateStatus(uint256 _tokenId, uint256 _newStatusID, uint256 _center) external onlyAuth {
        _statusID[_tokenId] = _newStatusID;
        _onCenter[_tokenId] = _center;
    }

    function getStatus(uint256 _tokenId) external view returns(string memory status, uint256 centerId){
        return (_status[_statusID[_tokenId]],_onCenter[_tokenId]);
    }

    function isFree(uint256 _tokenId) external view returns(bool){
        return (_statusID[_tokenId] == 0);
    }
    ...
    function _getLevel(uint256 _xp) private view returns (uint256) {
        ...
        while (_xpNeededToGoUp < _xp) {
            _level = _level.add(1);
            _xpNeededToGoUp = _xpNeededToGoUp.add(_xpNeededToGoUp.mul(multiplierXpUp).div(100));
        }
        return _level;
    }
        ...
        )
    {
        ...
        uint256[5]memory _powers =[m.powers.water,m.powers.fire,m.powers.metal,m.powers.air,m.powers.stone];
        return (
            m.level,
            m.xp,
            m.alchemyXp,
    ...
    function burnMonster(uint256 _tokenId) external onlyAuth {
        _burn(_tokenId);
    }
    ...
    function _generateRandomDatas(
        ...
        address _user,
        uint256 _nonce
    ) private returns (uint256[6] memory) {
        uint256 r = IOracle(gameAddresses.getOracleAddress()).getRandom(keccak256(abi.encodePacked(_user, _ipfsPath, _nonce)));
        uint256 [6] memory randoms;
        uint256 _mult = 1000;
