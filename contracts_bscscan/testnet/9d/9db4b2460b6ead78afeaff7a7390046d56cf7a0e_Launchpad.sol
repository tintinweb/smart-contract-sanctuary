...
contract Launchpad {
    ...
    function buyToken() public payable returns (bool) {
        ...
        if(buyPrice == 11){
            ...
            uint weeklyTokens = tokensToSend / 5;
            ...
            claim[msg.sender].tokenArr.push(tokensToSend);
            claim[msg.sender].tokenArr.push(weeklyTokens);
            claim[msg.sender].tokenArr.push(weeklyTokens);
            claim[msg.sender].tokenArr.push(weeklyTokens);
            claim[msg.sender].tokenArr.push(weeklyTokens);
            claim[msg.sender].tokenArr.push(weeklyTokens);
            
            claim[msg.sender].dateArr.push(t);
            claim[msg.sender].dateArr.push(t+3600);
            claim[msg.sender].dateArr.push(t+7200);
            claim[msg.sender].dateArr.push(t+10800);
            claim[msg.sender].dateArr.push(t+14400);
            claim[msg.sender].dateArr.push(t+18000);
            tokenR1 -= tokens;
        }
        ...
        if(buyPrice == 16){
            ...
            uint remToken = tokens - tokensToSend;
            uint t = block.timestamp;
            uint weeklyTokens = remToken / 2;
            token.transfer(msg.sender, tokensToSend);
            buyer[msg.sender].buyAt.push(t);
            buyer[msg.sender].claimed.push(0);
            buyer[msg.sender].count += 3;
            uint claimLeft = tokens - tokensToSend;
            buyer[msg.sender].claimLeft.push(claimLeft);
            buyer[msg.sender].halfOfBuy.push(claimLeft);
            buyer[msg.sender].buyPrice.push(buyPrice);
        ...
        if(buyPrice == 19){
            ...
            claim[msg.sender].tokenArr.push(tokens);
    ...
    function claimDetails(address user) public view returns (uint[] memory, uint[] memory) {
        ...
        uint[] memory tokens = new uint[](buyer[user].count);
        uint[] memory dates = new uint[](buyer[user].count);
        

        for(uint i = 0; i < buyer[user].count; i++){
            tokens[i] = claim[user].tokenArr[i];
            dates[i] = claim[user].dateArr[i];
        }
    ...
    function showBool(address user) public view returns (bool[] memory, bool[] memory, bool[] memory, bool[] memory,bool[] memory) {
        bool[] memory firstClaim = new bool[](buyer[user].bought.length);
        bool[] memory secondClaim = new bool[](buyer[user].bought.length);
        bool[] memory thirdClaim = new bool[](buyer[user].bought.length);
        bool[] memory fourthClaim = new bool[](buyer[user].bought.length);
        bool[] memory fifthClaim = new bool[](buyer[user].bought.length);
        
        for(uint j = 0; j < buyer[user].bought.length; j++){
            firstClaim[j] = claim[user].firstClaim[j];
            secondClaim[j] = claim[user].secondClaim[j];
            thirdClaim[j] = claim[user].thirdClaim[j];
            fourthClaim[j] = claim[user].fourthClaim[j];
            fifthClaim[j] = claim[user].fifthClaim[j];
        }
        return(firstClaim, secondClaim, thirdClaim, fourthClaim, fifthClaim);
    }
