...
contract MoneyMakerToken is Context, IBEP20, Ownable {
    ...
    mapping (address => uint256) public _pretime;
    mapping (address => uint256) public _presale;
    mapping (address => uint256) public _pritime;
    mapping (address => uint256) public _prisale;
    mapping (address => uint256) public _pubtime;
    mapping (address => uint256) public _pubsale;
    ...
    uint presaledays = block.timestamp.add(10);
    uint privatesaledays = block.timestamp.add(20);
    uint publicsaledays = block.timestamp.add(30);
    uint public oneDay = 1 ;
    bool public isSell;
    ...
    uint public base = 10 * (10 ** _decimals-4);
    uint public presale_price = 3 * (10 ** _decimals-4);
    uint public prisale_price = 6 * (10 ** _decimals-4);
    uint public pubsale_price = 9 * (10 ** _decimals-4);
    ...
    function setSell(bool _isSell) public  onlyOwner returns(bool)
    {
        isSell = _isSell;
        return true;
    }
    event preBuyToken(address _user,uint256 _amount);
    function preSale( uint256 tokenAmount) public returns(uint256)
    {
      require(!safeguard);
      require(!isContract(msg.sender),  'No contract address allowed');
      require(tokenAmount > 0 , "invalid amount sent");
      uint lD = presaledays;
      uint daysPassed = (block.timestamp - lD) / oneDay;
      uint per = 2;
      require(daysPassed > lD,"Presale period is over");
      uint max_burns = MAX_BURN;
      uint totalAllocated = max_burns * per / 100;
      tokenAmount = tokenAmount * presale_price;
      require(tokenAmount <  totalAllocated,"Amount exceeds");
      burn(tokenAmount);
      transfer(msg.sender,tokenAmount);
      emit preBuyToken(msg.sender, tokenAmount);
      return tokenAmount;
    }
     event priBuyToken(address _user,uint256 _amount);
    function private_Sale( uint256 tokenAmount) public returns(uint256)
    {
      require(!safeguard);
      require(!isContract(msg.sender),  'No contract address allowed');
      require(tokenAmount > 0 , "invalid amount sent");
      uint lD = privatesaledays;
      uint daysPassed = (block.timestamp - lD) / oneDay;
        uint per = 3;
      require(daysPassed  < lD,"Private sale period is over");
      uint max_burns = MAX_BURN;
      uint totalAllocated = max_burns * per / 100;
      tokenAmount = tokenAmount * prisale_price;
      require(tokenAmount <  totalAllocated,"Amount exceeds");
      burn(tokenAmount);
      transfer(msg.sender,tokenAmount);
      emit priBuyToken(msg.sender, tokenAmount);
      return tokenAmount;
    }
    event pubBuyToken(address _user,uint256 _amount);
    function public_Sale( uint256 tokenAmount) public returns(uint256)
    {
      require(!safeguard);
      require(!isContract(msg.sender),  'No contract address allowed');
      require(tokenAmount > 0 , "invalid amount sent");
       uint lD = publicsaledays;
      uint daysPassed = (block.timestamp - lD) / oneDay;
      uint per = 5;
      require(daysPassed  < lD,"Public sale period is over");
      uint max_burns = MAX_BURN;
      uint totalAllocated = max_burns * per / 100;
      tokenAmount = tokenAmount * pubsale_price;
      require(tokenAmount <  totalAllocated,"Amount exceeds");
      burn(tokenAmount);
      transfer(msg.sender,tokenAmount);
      emit pubBuyToken(msg.sender, tokenAmount);
      return tokenAmount;
    }
