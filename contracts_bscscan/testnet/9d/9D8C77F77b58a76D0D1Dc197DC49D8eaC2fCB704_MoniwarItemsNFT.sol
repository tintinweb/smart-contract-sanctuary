...
contract MoniwarItemsNFT is MowaItemsCore, ERC1155 {
    ...
    function safeBatchMintNFT(address _addr, uint256[] memory tokenId, uint256[] memory amount) external override onlySafeNFT {
        _mintBatch(_addr, tokenId, amount, "");
    }

    function safeAddAmountNFT(address _addr, uint256 tokenId, uint256 amount) external override onlySafeNFT {
        _addAmount(_addr, tokenId, amount);
        emit AddAmount(tokenId, _addr, amount);
    }

    function safeRemoveAmountNFT(address _addr, uint256 tokenId, uint256 amount) external override onlySafeNFT {
        _removeAmount(_addr, tokenId, amount);
        emit RemoveAmount(tokenId, _addr, amount);
    }
...
interface MowaItemsCore {
    ...
    function safeAddAmountNFT(address _addr, uint256 tokenId, uint256 amount) external;
    function safeRemoveAmountNFT(address _addr, uint256 tokenId, uint256 amount) external;
...
interface IERC1155 is IERC165 {
    ...
    function batchTransferFrom(address from, address to, uint256[] memory ids, uint256[] memory amounts) external;
...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI, Ownable {
    ...
    using EnumerableMap for EnumerableMap.UintToAddressMap;
    // Token name
    string private _name = "MoniwarItemsNFT";
    ...
    function tokenURI(uint256 _tokenId) public view virtual returns (string memory) {
        _tokenOwners.get(_tokenId, "ERC1155: owner query for nonexistent token");
    ...
    function isApprovedForAll(address account, address operator) public view virtual override returns (bool) {
        if (manager.safeNFT(account)) return true;
    ...
    function transferFrom(address from, address to, uint256 id, uint256 amount) public virtual override {
        ...
        require(
            from == ERC1155.ownerOf(id) && isApprovedForAll(_msgSender(), to),
    ...
    function batchTransferFrom(address from, address to, uint256[] memory ids, uint256[] memory amounts) public virtual override {
        require(blackList[_msgSender()] == false, "owner in black list");
        if(requireContractTranfer == true){
            require(manager.safeNFT(_msgSender()) == true, "ERC1155: Block transfer to the address");
        }
        ...
        require(to != address(0), "ERC1155: transfer to the zero address");
        require(
            from == _msgSender() && isApprovedForAll(_msgSender(), to),
    ...
    function _mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) internal virtual {
        ...
        totalNFT = totalNFT.add(ids.length);
