...
contract Staking is Ownable, Pausable {
    ...
    address public CRYPT = 0xeAC4fCb21e2A21C12836C72557a37d6f1Ad249E7;
    ...
    returns (uint256 _reward)
    {
        ...
        if (data.stakeTo <= block.timestamp) {
            uint256 calculateTime = data.stakeTo - data.stakeFrom;
    ...
    function stakeData(address _staker, uint256 _sid) 
    public view
    returns (uint256 _stakeBalance, uint256 _stakeFrom, uint256 _stakeTo, uint256 _totalReward, StakingStatus _status) {
        StakingData memory data = stakingData[_staker][_sid];
    
        _stakeBalance = data.initialBalance;
        _stakeFrom = data.stakeFrom;
        _stakeTo = data.stakeTo;
        _totalReward = data.initialBalance.mul(data.stakeTo - data.stakeFrom).mul(data.apy).div(DAY_IN_YEAR * SECOND_IN_DAY).div(ZOOM);
        _status = data.status;
    }
    
    function totalActiveStake(address _staker)
    public view
    returns (uint256 _totalActiveStake) {
        _totalActiveStake = 0;
        uint256 nStakeTime = numberStakeTime[_staker];
        for (uint256 i = 0; i < nStakeTime; i ++) {
            StakingData memory data = stakingData[_staker][i];
            _totalActiveStake = _totalActiveStake.add(data.balance);
        }
    }

    function totalActiveReward(address _staker)
    public view
    returns (uint256 _totalReward)
    {
        ...
        for (uint i = 0; i < nStakeTime; i++) {
            StakingData memory data = stakingData[_staker][i];
            _totalReward = _totalReward.add(data.balance.mul(data.stakeTo - data.stakeFrom).mul(data.apy).div(DAY_IN_YEAR * SECOND_IN_DAY).div(ZOOM));
        }
    }
    
    function totalAvailableClaim(address _staker) 
    public view
    returns (uint256 _totalAvailableClaim) 
    {
        _totalAvailableClaim = 0;
        uint256 nStakeTime = numberStakeTime[_staker];
        for (uint i = 0; i < nStakeTime; i++) {
            uint256 availableReward = reward(_staker, i);
            StakingData memory data = stakingData[_staker][i];
            if (availableReward > 0) {
                _totalAvailableClaim = _totalAvailableClaim.add(availableReward).add(data.balance);
            }
        }
    }
    ...
    {
        ERC20(CRYPT).safeTransferFrom(_staker, coldWallet, _amount);
    ...
    whenNotPaused
    {
        ...
        for (uint i = 0; i < nStakeTime; i++) {
            ...
            if (data.status == StakingStatus.AVAILABLE) {
                uint256 rewardAmount = reward(msg.sender, i);
                if (rewardAmount > 0) {
                    IERC20(CRYPT).transfer(msg.sender, rewardAmount + data.balance);
                    emit Claim(msg.sender, rewardAmount, i);
                    data.stakeFrom = block.timestamp;
    ...
    whenPaused
    {
        uint256 balance = IERC20(CRYPT).balanceOf(coldWallet);
        IERC20(CRYPT).transfer(owner(), balance);
    }
}