...
contract PandaStake is IPandaStake, Context, Ownable {
  ...
  constructor () public {
    ...
    function rFreeBalance(address account, uint256 n) external view returns (uint256) {
      return _rXBalances[account][n];
    }

  function getTransferXFee(address account, uint256 remain) external override onlyOwner() returns (uint256, uint256) {
    uint256 rXFee;
    uint256 freeSum;
  ...
  function addToRXBalance(address account, uint256 rAmount) external override onlyOwner() returns (uint256) {
    uint256 freeSum = updateRXBalances(account);
    _rXBalances[account][0] = _rXBalances[account][0].add(rAmount);
    return freeSum;
  }

  function updateRXBalances(address account) public override onlyOwner() returns (uint256) {
