...
contract Lottery {
    ...
    function payToken(uint _multiple) public {
    ...
    //投注
    function bet(uint256  betStrFirst, uint256  betStrSecond, uint256  betStrThird, uint256 sum) public {//投注函数
        ...
        //支付token
        tokenReward.transferFrom(msg.sender, address(this), sum*(10**18));//这个sum是投注的倍数，这里是1个代币投一次，如果是多个代币投一次，总支付代币则是（单注代币数*sum）
