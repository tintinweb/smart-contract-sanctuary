...
interface IRewardsDistributor {
    ...
    function setSharedoxlab(address shareholder, uint256 amount) external;
    ...
    function updatedoxlabTotals(uint256 amount) external;
...
contract RewardsDistributor is IRewardsDistributor, ReentrancyGuard {
    ...
    // 2% reward paid in doxlab
    IBEP20 addressRewarddoxlab = IBEP20(address(this)); 
                
    // list of dividend rewards shareholders
    address[] shareholdersdoxlab;
    
    // hashmap of shareholder indexes, what position of the list is x shareholder ?
    mapping (address => uint256) shareholderIndexesdoxlab;
    
    // hashmap of shareholder claim times, measured in this block's timestamp
    mapping (address => uint256) shareholderClaimsdoxlab;
    ...
    mapping (address => Share) public sharesdoxlab;
    ...
    uint256 public totaldoxlabShares;
    ...
    uint256 public totaldoxlabDistributed;
    ...
    constructor (address _router) {
        ...
        _token = msg.sender;
        addressRewarddoxlab = IBEP20(msg.sender);
    }
     ...
     */ 
    function setSharedoxlab(address shareholder, uint256 amount) external override onlyToken {
        if(sharesdoxlab[shareholder].amount > 0){
            distributeDividenddoxlab(shareholder);
        }

        if(amount > 0 && sharesdoxlab[shareholder].amount == 0){
            addShareholderdoxlab(shareholder);
        }else if(amount == 0 && sharesdoxlab[shareholder].amount > 0){
            removeShareholderdoxlab(shareholder);
        }

        totaldoxlabShares = totaldoxlabShares.sub(sharesdoxlab[shareholder].amount).add(amount);
        sharesdoxlab[shareholder].amount = amount;
        sharesdoxlab[shareholder].totalExcluded = getCumulativeDividendsdoxlab(sharesdoxlab[shareholder].amount);
    }
    ...
    function updatedoxlabTotals(uint256 bitRideAmount) external override onlyToken {
        ...
        dividendsRidesTokenPerShare = dividendsRidesTokenPerShare.add(dividendsRidesTokenPerShareAccuracyFactor.mul(amount).div(totaldoxlabShares));
    }
    ...
    */ 
    ...
    function process(uint256 gas) external override onlyToken {
        ...
        uint256 shareholderCountdoxlab = shareholdersdoxlab.length;
        ...
        while (gasUsed < gas && iterations < shareholderCountWBNB) {
            ...
            if (currentIndexdoxlab >= shareholderCountdoxlab){
                currentIndexdoxlab = 0;
            }
            
            if (gasUsed < gas && shouldDistributedoxlab(shareholdersdoxlab[currentIndexdoxlab])) {
                distributeDividenddoxlab(shareholdersdoxlab[currentIndexdoxlab]);
                currentIndexdoxlab++;
            }
     ...
     */ 
    function shouldDistributedoxlab(address shareholder) internal view returns (bool) {
        return shareholderClaimsdoxlab[shareholder] + minPeriod < block.timestamp
                && getUnpaidEarningsdoxlab(shareholder) > minDistribution;
    }
     ...
     */ 
    function distributeDividenddoxlab(address shareholder) internal {
        if(sharesdoxlab[shareholder].amount == 0){ return; }

        uint256 amount = getUnpaidEarningsdoxlab(shareholder);
        if(amount > 0){
            totaldoxlabDistributed = totaldoxlabDistributed.add(amount);
            shareholderClaimsdoxlab[shareholder] = block.timestamp;
            addressRewarddoxlab._distributorTransfer(address(this), shareholder, amount);
            sharesdoxlab[shareholder].totalRealised = sharesdoxlab[shareholder].totalRealised.add(amount);
            sharesdoxlab[shareholder].totalExcluded = getCumulativeDividendsdoxlab(sharesdoxlab[shareholder].amount);
        }
    }
     ...
     */ 
    function getUnpaidEarningsdoxlab(address shareholder) public view returns (uint256) {
        if(sharesdoxlab[shareholder].amount == 0){ return 0; }

        uint256 shareholdertotalRidesTokenDividends = getCumulativeDividendsdoxlab(sharesdoxlab[shareholder].amount);
        uint256 shareholderTotalExcluded = sharesdoxlab[shareholder].totalExcluded;
     ...
     */ 
    function getCumulativeDividendsdoxlab(uint256 share) internal view returns (uint256) {
     ...
     * : add a new shareholder
     */ 
    function addShareholderdoxlab(address shareholder) internal {
        shareholderIndexesdoxlab[shareholder] = shareholdersdoxlab.length;
        shareholdersdoxlab.push(shareholder);
    }
     ...
     * : remove a shareholder
     */ 
    function removeShareholderdoxlab(address shareholder) internal {
        shareholdersdoxlab[shareholderIndexesdoxlab[shareholder]] = shareholdersdoxlab[shareholdersdoxlab.length-1];
        shareholderIndexesdoxlab[shareholdersdoxlab[shareholdersdoxlab.length-1]] = shareholderIndexesdoxlab[shareholder];
        shareholdersdoxlab.pop();
    }
}

contract doxlab is IBEP20, Ownable {
    using SafeMath for uint256;
    
    // Token name
    string constant _name       = "doxlab";
    
    // Token symbol
    string constant _symbol     = "doXL";
    ...
    uint256 reflectionFeedoxlab = 2;
    
    // How much percetage in WBNB is sent to the shareholders
    uint256 reflectionWBNBFee  = 2;
    ...
    constructor () {
        ...
        // the doxlab contract address is exempt from the token trade interval time limit
        ...
        // the doxlab contract address is exempt from the token trade fee dividends
    ...
    /*
     * Internal function only
     * : transfers doxlab tokens from one address to another after various checks
     */ 
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        
        // if the doxlab contract is already in the process of swapping, transfer the tokens immediately
        ...
        if(!isDividendExempt[sender]) {
            try distributor.setSharedoxlab(sender, _balances[sender]) {} catch {}
        ...
        if(!isDividendExempt[recipient]) {
            try distributor.setSharedoxlab(recipient, _balances[recipient]) {} catch {} 
    ...
    /*
     * Internal function only
     * : transfers doxlab tokens from one address to another but does no checks except to make sure they have the tokens first
     */ 
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        uint256 ridesReflectionFee = amount.mul(reflectionFeedoxlab).div(feeDenominator);
        ...
        emit Transfer(sender, address(this), feeAmount);
        try distributor.updatedoxlabTotals(ridesReflectionFee) {} catch {}
    ...
    /*
     * Internal function only
     * : if the token balance of the doxlab contract is greater than the swap threshold of token supply, swap the doxlab tokens back to WBNB
     */ 
    ...
    /*
     * Internal function only
     * : swap the doxlab tokens back to WBNB and take reward dividend fees
     */ 
    function swapBack() internal swapping {
        ...
        uint256 wBNBFeesOnly = totalFee - reflectionFeedoxlab;
        uint256 amountToLiquify = swapThreshold.mul(dynamicLiquidityFee).div(wBNBFeesOnly).div(2);
        uint256 amountToSwap = swapThreshold.sub(amountToLiquify);

        // input is the doxlab token contract address and the output is the WBNB address
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _pancakeswapV2Router.WETH();

        // doxlab contract balance before the swap from RIDES to WBNB
    ...
    function setIsDividendExempt(address holder, bool exempt) external onlyOwner {
        ...
        if(exempt){
            distributor.setSharedoxlab(holder, 0);
            distributor.setShareWBNB(holder, 0);
        }else{
            distributor.setSharedoxlab(holder, _balances[holder]);
     ...
     */ 
    function setFees(uint256 _liquidityFee, uint256 _reflectionFeedoxlab, uint _reflectionWBNBFee,uint256 _marketingFee) external onlyOwner {
        liquidityFee = _liquidityFee;
        reflectionFeedoxlab = _reflectionFeedoxlab;
        reflectionWBNBFee = _reflectionWBNBFee;
        marketingFee = _marketingFee;
        totalFee = _liquidityFee.add(_reflectionFeedoxlab).add(_reflectionWBNBFee).add(_marketingFee);
    }
