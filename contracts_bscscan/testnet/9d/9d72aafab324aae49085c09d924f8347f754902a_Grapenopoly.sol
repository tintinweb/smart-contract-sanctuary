...
contract Grapenopoly
{
    ...
    function rollDices() public returns (DiceResult memory)
    {
        ...
        {
            //To do convert rentvalue from usd to selected token to pay rent and check houses and hotel
            /*
            uint256 rentValue = nftRent[ playerPosition[msg.sender] ][0];

            if(rentValue > 0)
            {
                if(vaultBalances[msg.sender][networkcoinaddress] > rentValue)
                {
