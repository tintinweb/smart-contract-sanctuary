...
contract SideKickNames {
    ...
    constructor(IERC20 _rooted, address payable _dev, Payment _subscription ) {
        rooted = _rooted;
        //100 Rooted / SK
        price = 1e20;
        taxedPrice = 5e20;
    ...
    function _checkSubscribed(address sender) internal view returns (bool isSubbed){
        for (uint i = 0; i < subscriptionPlans; i++){
            isSubbed = subscription.checkSubscription(planIdMapping[subscriptionPlans], sender);
            if (isSubbed == true){
                return isSubbed;
            }
        }
    ...
    function setNameRecord(string calldata _name) public returns (bool _success) {	
        ...
        if(_checkSubscribed(_addr) == false){
    ...
    function setNFT721(address _nftAddr) public returns (bool _success){
        if (getNFTbyHeroAddress(msg.sender) == address(0)){
            require(payRooted(true), '');
        } else {
            require(payRooted(false), '');
        }
        address _addr = msg.sender;
    ...
    function setEligiblePlans(uint planId) onlyOwner() public {
