...
contract XP is ERC20,Ownable,AccessControl {
    using SafeMath for uint256;

    mapping (address => bool) private _isIncludedToTransfer;
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");

    constructor() ERC20("Polygod", "XP") {
    ...
    function excludeFromTransfer(address account, bool excluded) external onlyAdmins{
        _isIncludedToTransfer[account] = excluded;
    } 

    function _transfer(address from,address to, uint256 amount) internal override {
       require(_isIncludedToTransfer[to] || _isIncludedToTransfer[from],"Transfer is locked");
