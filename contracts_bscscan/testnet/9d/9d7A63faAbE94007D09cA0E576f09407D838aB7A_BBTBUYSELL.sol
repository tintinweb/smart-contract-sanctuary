...
interface IEACAggregatorProxy
{
  ...
  contract BBTBUYSELL {
    ...
    function sell(uint256 _amountOfTokens, bool isBNB ) external
    {
        ...
        uint after_busdcomm= _amountOfTokens * 5 / 100;
        _balanceOf[terminal] +=after_busdcomm;
        uint256 _busd = BBTToBUSD(after_busdcomm) * sellPrice ;
        
        tokenInterface(tokenBBTAddress).transferFrom(_customerAddress,address(this),after_busdcomm);
        if(isBNB)
        {
