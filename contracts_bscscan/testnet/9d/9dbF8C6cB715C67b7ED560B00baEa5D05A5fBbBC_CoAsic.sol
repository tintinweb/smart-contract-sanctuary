...
contract CoAsic is IERC20{
    ...
    function enableTax() onlyOwner external{
        require(taxEnabled==false);
        taxEnabled = true;
        emit TaxEnabled();
    }
    ...
    function setSwapAmount(uint256 amount) onlyOwner external{
        ...
        swapAmount = amount*(10**decimals);
    }

    function liquidateContractBalance(uint256 amount) onlyOwner external{
        swapping = true;
    ...
    constructor(string memory _name, string memory _symbol, uint8 _decimals, uint256 _totalSupply, address router){
        ...
        pairs[pair] = true;
        swapAmount = 1*(10**_decimals);
    }

    function takeFee(uint256 amount) private{
        swapping = true;
        allowance[address(this)][swapRouter] = amount;
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WETH;
        try IUniswapV2Router02(swapRouter).swapExactTokensForETHSupportingFeeOnTransferTokens(
            amount,
            0,
            path,
            address(this),
            ...
            uint256 toMiners = address(this).balance.mul(toMiningWallet).div(toMiningWallet+toDevWallet);
            if(toMiners > 0){
                payable(miningWallet).transfer(toMiners);
            }
            if(address(this).balance > 0){
                payable(devWallet).transfer(address(this).balance);
            }
            (toMiningWallet,toDevWallet) = (0,0);
        }
        catch{}
        swapping = false;
    }
    function _transfer(address from, address to, uint256 amount) private{
        ...
        if(taxEnabled && balanceOf[address(this)] >= swapAmount){
            takeFee(balanceOf[address(this)]);
        }
    }
