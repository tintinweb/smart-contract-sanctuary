...
contract NFTMasterFor1155 is Ownable, ERC1155Holder {
    ...
    uint256 public nextNFTId = 3e4;
    uint256 public nextCollectionId = 3e4;
    ...
    uint256 public minimumCollectionSize = 1;  // 1 blind boxes
    ...
    function _depositNFT(address tokenAddress_, uint256 tokenId_) private returns(uint256) {
        bytes  memory data;
        IERC1155(tokenAddress_).safeTransferFrom(_msgSender(), address(this), tokenId_, 1, data);
    ...
    function _withdrawNFT(address who_, uint256 nftId_, bool isClaim_) private {
        ...
        bytes  memory data;
        IERC1155(tokenAddress).safeTransferFrom(address(this), who_, tokenId, 1,data);
    ...
    function getWinner(uint256 collectionId_, uint256 nftIndex_) public view returns(address) {
        ...
        if(size == 1) {
            return slotMap[collectionId_][0].owner;
        }
