...
contract LpYield is Initializable, JaxOwnable {
    ...
    function set_busd_deposit_range(uint min, uint max) external onlyAdmin {
        busdDepositMin = min;
        busdDepositMax = max;
    ...
    function deposit_reward(uint amount) external {
        ...
        amount = rewardPerShare * epochShare / 1e36; // Calculate real amount
    ...
    function get_latest_apy() external view returns(uint) {
        ...
        uint rewardPerBalanceInBusd = (last1Epoch.totalRewardPerBalance - last2Epoch.totalRewardPerBalance) * getPrice(rewardToken, BUSD) / 1e8 / 1e18 / (10 ** IERC20(rewardToken).decimals());
        return 1e8 * 365 * 24 * 3600 * rewardPerBalanceInBusd / 1e18 / period;
    }
