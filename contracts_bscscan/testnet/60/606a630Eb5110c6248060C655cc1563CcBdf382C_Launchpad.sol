...
contract DataStorage {
	...
	uint256 public priceToken = 10000*10**18;
	uint256 public totalSupply= 1200*10**18;
	uint256 public minInvest = 1*10**18;
	uint256 public maxInvest = 1200*10**18;
...
contract Launchpad is Ownable, DataStorage, Access, Events {
    ...
    function _registerBuy(address _beneficiary, uint256 amount) internal {
        ...
        require(
            buyToken.allowance(_beneficiary, address(this)) >= amount.mul(priceToken),
        ...
        require(
            weiAmount >= minInvest,
            "Requried: buy token not enough"
        );   
        require(
            weiAmount <= maxInvest,
            "Requried: buy token too much"
        );      
        _preValidatePurchase(_beneficiary, weiAmount);
        totalRegister += 1;
        require(totalRegister <= TOTAL_SLOT, "Required: Not engough slot to register");
        tokenHolders[_beneficiary] = weiAmount;
        // update state
        wasSale = wasSale.add(weiAmount);
        user.owner = _beneficiary;
        user.amountInvest = user.amountInvest.add(weiAmount);
        user.tokenBuy = user.tokenBuy.add(weiAmount);
        _forwardFunds(weiAmount, _beneficiary);
        _deliverTokens(msg.sender, user.tokenBuy);
        emit TokenPurchase(_beneficiary, weiAmount.mul(priceToken), weiAmount);
