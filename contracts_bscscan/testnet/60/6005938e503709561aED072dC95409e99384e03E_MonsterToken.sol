...
contract MonsterToken is MonsterERC20, ReentrancyGuard {
    ...
    function swapToBoss() public nonReentrant {
    ...
    function setAddressForBosses(address _BOSS_WALLET) external onlyOwner {
        require(_BOSS_WALLET != address(0), "0x is not accepted here");

        BOSS_WALLET = _BOSS_WALLET;
    }
...
contract MonsterERC20 is Ownable, ERC20 {
    ...
    function setTransferFeeRate(uint256 _sellFeeRate, uint256 _buyFeeRate)
        ...
        onlyOwner
    {
        require(_sellFeeRate<=10, "not allow over Rate");
        require(buyFeeRate<=5, "not allow over Rate");
    ...
    function setMinTokensBeforeSwap(uint256 _tokenForBosses)
        ...
        onlyOwner
    {
        require(_tokenForBosses < 20 * 10**6 * 10**18, "not allow over Rate");
