...
contract SyrupBar is BEP20('SyrupBar Token', 'SYRUP') {
    ...
    function safeCornTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 cornBal = corn.balanceOf(address(this));
        if (_amount > cornBal) {
            corn.transfer(_to, cornBal);
        } else {
...
interface IMigratorChef {
    // Perform LP token migration from legacy PancornSwap to CornSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to PancornSwap LP tokens.
    // CornSwap must mint EXACTLY the same amount of CornSwap LP tokens or
    // else something bad will happen. Traditional PancornSwap does not
...
contract MasterChef is Ownable {
    ...
    // Bonus muliplier for early corn makers.
    ...
    function updatePool(uint256 _pid) public {
        ...
        corn.mint(address(syrup), cornReward);
    ...
    function safeCornTransfer(address _to, uint256 _amount) internal {
        syrup.safeCornTransfer(_to, _amount);
    }
