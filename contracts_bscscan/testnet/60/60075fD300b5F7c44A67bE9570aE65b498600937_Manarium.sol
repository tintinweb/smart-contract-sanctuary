...
contract Manarium is ERC20, Ownable
{
    ...
    event ClaimDividends(address owner, uint256 amount);
    ...
    function updateDividendDistributor(address address_) public onlyOwner{
        require(address(_distributor) != address_);
        _distributor = IDividendDistributor(address_);
        _excludeFromFeesAndDividends(address(_distributor), true);
    }
    ...
    internal override {
        ...
        if(!_isExcludedFromDividends[sender]){ _distributor.setShare(sender, balanceOf(sender)); }
        if(!_isExcludedFromDividends[recipient]){ _distributor.setShare(recipient, balanceOf(recipient)); }
    }
    ...
    function sendToDistribution(uint256 amount) private{
        super._transfer(address(this), address(_distributor), amount);
        _distributor.deposit(amount);
        emit SendTokensForDividends(amount);
    }
    
    function sendToPrizePool(uint256 amount) private {
        super._transfer(address(this), PRIZE_POOL_WALLET, amount);
    ...
    function claimDividend() external {
        ...
        emit ClaimDividends(msg.sender, amount);
    }
...
contract DividendDistributor is IDividendDistributor {
    ...
    function getUnpaidEarnings(address shareholder) external override view returns (uint256) {
