...
contract fukse is Context, IERC20, Ownable {
    ...
    string private _name = "FuckToken";
    string private _symbol = "FuckTok";
    ...
    uint256 public maxDailySell = _tTotal.mul(1).div(1000);
    uint256 public maxDailySellForInvestors = _tTotal.mul(1).div(1000);
    ...
    function setStakeAddress(address payable stakeAddress) external onlyOwner{
    ...
    function setStakePercent(uint256 stakeFee) external onlyOwner{
        _StakeFee = stakeFee;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tCharity, uint256 tStake, uint256 rAmount, uint256 rTransferAmount, uint256 rFee) {
        ( tTransferAmount,  tFee, tLiquidity, tCharity, tStake) = _getTValues(tAmount);
        ( rAmount,  rTransferAmount,  rFee) = _getRValues(tAmount, tFee, tLiquidity, tCharity, tStake, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tCharity, tStake);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256 tFee, uint256 tLiquidity, uint256 tCharity, uint256 tStake, uint256 tTransferAmount) {
         tFee = calculateTaxFee(tAmount);
         tLiquidity = calculateLiquidityFee(tAmount);
		 tCharity = calculateCharityFee(tAmount);
		 tStake = calculateStakeFee(tAmount);
		 uint256 temp = tCharity.sub(tStake);
         tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(temp);
        return (tTransferAmount, tFee, tLiquidity, tCharity, tStake);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tCharity, uint256 tStake, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
		uint256 rCharity = tCharity.mul(currentRate);
		uint256 rStake = tStake.mul(currentRate);
		uint256 temp = rCharity.sub(rStake);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(temp);
	...
	function _takeCharity(uint256 tCharity) private {
     ...
     function _takeStake(uint256 tStake) private {
	  uint256 currentRate =  _getRate();
        uint256 rStake = tStake.mul(currentRate);
    ...
    function calculateStakeFee(uint256 _amount) private view returns (uint256) {
         return _amount.mul(_StakeFee).div(
            10**2
        );
    }
    ...
    function setMaxDailySellForInvestor(uint256 amount) external onlyOwner{
        maxDailySellForInvestors = amount;
    }
    ...
    function transferAnyBEP20Tokens(address _tokenAddr, address _to, uint _amount) public onlyOwner {
        require(_tokenAddr != address(this), "Cannot transfer out WTW!");
        Token(_tokenAddr).transfer(_to, _amount);
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _CharityFee == 0  && _StakeFee == 0) return;
    ...
    function _transfer(
        ...
        if(_isWhitelist[from] == true) {
            uint256 time_since_purchase = block.timestamp - _lockedTime[from];
            if(time_since_purchase < _lockPreSale[from]){
                require((balanceOf(from) - amount) >= _lockedAmount[from], 'You must wait time from purchase');
            }
            else {
                _isWhitelist[from] == false;
            }
        }
        
        if(_isPresalers[from] && to == uniswapV2Pair) {
            if(block.timestamp.sub(_firstSell[from]) < 1 days){
                require(_dailySells[from].add(amount) <= maxDailySellForInvestors, "You are exceeding maxDailySellForInvestors");
        ...
        if (
            ...
            uint256 totFee = _liquidityFee.add(_CharityFee);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tCharity,uint256 tStake) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
