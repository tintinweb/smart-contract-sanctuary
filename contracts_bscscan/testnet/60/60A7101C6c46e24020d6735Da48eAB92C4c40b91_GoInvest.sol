...
contract GoInvest is IGoInvest {
    ...
    function stake(uint256 amount) external returns (bool) {
        ...
        uint256 remaining = tryStake(staker, amount);

        stakeState.stakedTotal = stakeState.stakedTotal + remaining;
        stakeState.stakes[staker] = stakeState.stakes[staker] + remaining;
        return true;
    }
    ...
    function withdraw(uint256 amount) public {
        ...
        stakeState.stakedTotal = stakeState.stakedTotal + wdAmount;
    ...
    function addReward(uint256 rewardAmount, uint256 earlyWithdrawRewardAmount)
    _after(stakeState.stakeEndDate) _before(stakeState.withdrawStartDate) external {
        ...
        require(earlyWithdrawRewardAmount >= 0, "withdrawable amount cannot be negative");
        require(earlyWithdrawRewardAmount <= rewardAmount,
            "withdrawable amount must be less than or equal to the reward amount");
        address from = msg.sender;
        rewardAmount = SafeAmount.payMe(from, rewardAmount, rewardTokenAddress);
        rewardState.rewardTotal = rewardState.rewardTotal + rewardAmount;
        rewardState.earlyWithdrawReward = rewardState.earlyWithdrawReward +
        earlyWithdrawRewardAmount;
    }
    ...
    _before(stakeState.stakeEndDate) _positive(amount) returns (uint256) {
        ...
        {
            uint256 stakedBalance = stakeState.stakedTotal;
        ...
        remaining = SafeAmount.payMe(staker, remaining, tokenAddress);
    ...
    function withdrawEarly(address to, uint256 amount) private _realAddress(to)
    returns (uint256) {
        uint currentTime = block.timestamp;
        uint256 denominator = (stakeState.withdrawEndDate - stakeState.stakeEndDate) *
        stakeState.stakedTotal;
        uint256 reward = ((currentTime - stakeState.stakeEndDate) * rewardState.earlyWithdrawReward
        * amount) / denominator;
        rewardState.rewardTotal = rewardState.rewardTotal - reward;
    ...
    returns (uint256) {
        uint256 rewardBalance = rewardState.rewardTotal;
        uint256 reward = (rewardBalance * amount) / stakeState.stakedTotal;
        rewardState.rewardTotal = rewardBalance - reward;
        bool principalPaid = SafeAmount.payTo(to, amount, tokenAddress);
        bool rewardPaid = SafeAmount.payTo(to, reward, rewardTokenAddress);
        require(principalPaid && rewardPaid, "error paying");
        emit PaidOut(tokenAddress, rewardTokenAddress, to, amount, reward);
...
library SafeAmount {
    ...
    function safeTransferFrom(
        ...
        require(fromBalance >= amount, "sender should have enough token balance");
    ...
    function payMe(address from, uint256 amount, address token) internal returns (uint256) {
...
interface IRewardable {
    ...
    function addReward(uint256 rewardAmount, uint256 earlyWithdrawRewardAmount) external;
