...
contract DuelStaking is Ownable {
    ...
    event PoolUpdate(uint256 indexed pid, uint256 indexed allocation);
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (block.number <= pool.lastRewardBlock || pool.totalReward >= pool.totalAllocation) {
            return;
        }
    ...
    function setTokenPerBlock(uint256 _amount) external onlyOwner {
        ...
        require(_amount >= 1 * 1e18, "DuelStaking: Min per block 1 token");
