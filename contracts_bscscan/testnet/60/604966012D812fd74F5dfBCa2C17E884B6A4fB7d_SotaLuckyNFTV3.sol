...
contract SotaLuckyNFTV3 is VRFConsumerBase, Ownable {
    ...
    function operatorLuckyDraw(
        uint256 _start,
        uint256 _end,
        address[] calldata _addressDraw
    ) external {
        require(isOperator[msg.sender], "Only-operator");
        require(
            _start > 0 &&
                _end > _start &&
                (_addressDraw.length <= _end - _start),
            "Invalid-start-end"
        );
        Round memory newRound;
        newRound.start = _start;
        ...
        for (uint256 index = 0; index < _addressDraw.length; index++) {
            lotteryNumbers[
                keccak256(abi.encodePacked(totalRound, _addressDraw[index]))
            ].push(index + 1);
        }
        ...
        mapBytesRound[requestId] = totalRound;
        ...
        returns (uint256)
    {
        ...
        round.roundRandom = randomNumber;
