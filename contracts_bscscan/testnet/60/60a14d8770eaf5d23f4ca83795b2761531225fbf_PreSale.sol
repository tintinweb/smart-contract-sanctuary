...
contract PreSale is Ownable {
    ...
    uint256 public _PRYZ_THRESHOLD;
    ...
    constructor() {
        tokenPryz = IERC20(0x6130AAaffDd8F39dC8EC00c6af0681DF0cBb2EEF);
    }
    ...
    function ownerWithdrawToken() public onlyOwner{        
        tokenForSale.transfer(msg.sender, tokenForSale.balanceOf(address(this)));
    }

    function setTokenForPresale(address _tokenAddress) public onlyOwner{        
        tokenForSale = IERC20(_tokenAddress);
    }

    function updateWhitelistThresholdPRYZ(uint256 pryzThresold) public onlyOwner{        
        _PRYZ_THRESHOLD = pryzThresold;
    }
    ...
    function setupPresale(uint256 start, uint256 end, uint256 release, uint256 rate, uint256 softcap, uint256 hardcap, uint256 maxPerWallet, uint256 minPerWallet, uint256 pryzThresold) public onlyOwner{
        _RATE = rate;
    ...
    function isWalletWhitelisted(address walletAddr) public view returns(bool){
        if(isPreSaleOpen){
            return true;
        }else {
            if(tokenPryz.balanceOf(walletAddr) >= _PRYZ_THRESHOLD || isPresaleWhitelist[walletAddr]){
                return true;
            }else {
                return false;
            }
    ...
    function deposite() public payable {
        ...
        if(!isPreSaleOpen){
            if(tokenPryz.balanceOf(msg.sender) < _PRYZ_THRESHOLD){ // If msg.sender keep PRYZ more than threshold --> Whitelisted
    ...
    function withdraw() public {
        ...
        if(_TOTAL_DEPOSIT_BNB < _SOFTCAP){
            ...
            require(block.timestamp > _RELEASE_TOKEN_TIME, "Please check tokenForSale release time");
            tokenForSale.transfer(msg.sender, totalPurchased[msg.sender].mul(_RATE));
            withdrawn[msg.sender] = true;
        }
    }
}