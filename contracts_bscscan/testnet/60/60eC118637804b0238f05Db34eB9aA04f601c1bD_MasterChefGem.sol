...
contract TicketNFT is Context, AccessControl, ERC1155Burnable, ERC1155Pausable {
    ...
    function createTicket(
        uint256 _price,
        uint8 _rarity
    ) external onlyAdmin rarityIsValid(_rarity) {
    ...
    function mint(
        ...
        uint256 _tokenId,
        uint256 _amount
    ) external onlyMinter tokenExists(_tokenId) {
...
// MEG with Governance.
contract MegMockToken is BEP20("Elpis", "ELPIS") {
...
// MasterChefLapis is the master of LAPISs. He can make LAPISs and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once LAPISs is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract MasterChefLapis is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 lapisPerBlock; // LAPISs created per block
        uint256 lastRewardBlock; // Last block number that LAPISs distribution occurs.
        uint256 accLapisPerToken; // Accumulated LAPISs per token, times 1e30. See below.
    }
    ...
    event Bought(address indexed user, uint256 indexed tokenId, uint256 amount);

    constructor(TicketNFT _ticket, uint256 _startBlock) public {
        ticket = _ticket;
    ...
    function add(uint256 _lapisPerBlock, BEP20 _lpToken) public onlyOwner {
        require(
    ...
    function set(
        uint256 _pid,
        uint256 _lapisPerBlock,
        bool _withUpdate
    ) public onlyOwner {
        if (_withUpdate) {
            updatePool(_pid);
        }
        poolInfo[_pid].lapisPerBlock = _lapisPerBlock;
    }
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 soulReward = multiplier.mul(pool.lapisPerBlock);

            accLapisPerToken = accLapisPerToken.add(
                soulReward.mul(PRECISION).div(10**pool.decimals)
            );
        }
        return
    ...
    function buyBatchTicket(
        ...
        for (uint256 i = 0; i < length; i++) {
            buyTicket(_tokenIds[i], _amounts[i], false);
        }
    }
    ...
    function buyTicket(
        ...
        (uint256 tokenId, uint256 price, ) = ticket.getTicketInfo(_tokenId);
        require(tokenId > 0, "buyTicket: ticket does not exist");
        require(_amount > 0, "buyTicket: amount must be greater than 0");

        if (_withCalculation) {
            rewardCalculation();
        }
        require(
            _balances[msg.sender] >= price.mul(_amount),
            "buyTicket: insufficient balance"
        );

        _balances[msg.sender] = _balances[msg.sender].sub(price.mul(_amount));
...
import "./MagicElpisGem.sol";

// MasterChefGem is the master of MEGs. He can make MEGs and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once MEGs is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract MasterChefGem is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        uint256 rewardDebt; // Reward debt. See explanation below.
        //
        // We do some fancy math here. Basically, any point in time, the amount of MEGs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accGemPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accGemPerShare` (and `lastRewardBlock`) gets updated.
        //   2. User receives the pending reward sent to his/her address.
        //   3. User's `amount` gets updated.
        //   4. User's `rewardDebt` gets updated.
    }
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. MEGs to distribute per block.
        uint256 lastRewardBlock; // Last block number that MEGs distribution occurs.
        uint256 accGemPerShare; // Accumulated MEGs per share, times 1e12. See below.
    }

    // The MEGs TOKEN!
    MagicElpisGem public meg;
    // Dev address.
    address public devaddr;
    // Treasury address.
    address public treasury;
    // MEGs tokens created per block.
    uint256 public megPerBlock;
    // Bonus muliplier for early ELPIS makers.
    uint256 public BONUS_MULTIPLIER = 1;
    ...
    // The block number when ELPIS mining starts.
    uint256 public immutable startBlock;
    ...
    constructor(
        ...
        address _devaddr,
        address _treasury,
        uint256 _megPerBlock,
        uint256 _startBlock
    ) public {
        meg = _meg;
        devaddr = _devaddr;
        treasury = _treasury;
        megPerBlock = _megPerBlock;
        startBlock = _startBlock;
    }
    ...
    function updateDivisorForDev(uint256 divisorNumber) public onlyOwner {
        BONUS_DIVISOR_FOR_DEV = divisorNumber;
    }

    function setElpisPerBlock(uint256 _megPerBlock) public onlyOwner {
        massUpdatePools();
        megPerBlock = _megPerBlock;
    }
    ...
    // Update the given pool's MEGs allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier =
                getMultiplier(pool.lastRewardBlock, block.number);
            uint256 megReward =
                multiplier.mul(megPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accGemPerShare = accGemPerShare.add(
                megReward.mul(PRECISION).div(lpSupply)
            );
        }
        return
            user.amount.mul(accGemPerShare).div(PRECISION).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (BONUS_DIVISOR_FOR_DEV > 0) {
            meg.mint(devaddr, megReward.div(BONUS_DIVISOR_FOR_DEV));
        }
    ...
    // Deposit LP tokens to MasterChef for MEGs allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accGemPerShare).div(PRECISION).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
                transferMegReward(msg.sender, pending);
            }
        }
        if (_amount > 0) {
            uint256 amountAdded = _amount;
            ...
            if (depositFee > 0) {
                ...
                amountAdded = amountExcludeDespositFee;
            }

            user.amount = user.amount.add(amountAdded);
            pool.lpToken.safeTransferFrom(
                address(msg.sender),
                address(this),
                amountAdded
        ...
        user.rewardDebt = user.amount.mul(pool.accGemPerShare).div(PRECISION);
        emit Deposit(msg.sender, _pid, _amount);
    }
    ...
    // MEGs transfer function.
    function transferMegReward(address _to, uint256 _amount) internal {
        meg.mint(_to, _amount);
    }
...
// Magic Elpis Gem token.
contract MagicElpisGem is BEP20("Magic Elpis Gem", "MEG") {
    mapping(address => bool) public minters;

    modifier onlyMinter() {
        require(minters[msg.sender], "MEG::not authorized");
        _;
    }
    ...
    function mint(uint256 _amount) public override onlyMinter returns (bool) {
