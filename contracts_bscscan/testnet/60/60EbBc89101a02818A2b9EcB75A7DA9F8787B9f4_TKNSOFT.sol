...
// only admin can set liquidity to be added
    ...
    function addLiquidityToPancakeSwap(
       ...
       INightmareRouter01 router = INightmareRouter01(pancakeRouter);
       return router.addLiquidity(tokenA, tokenB, amountADesired, amountBDesired, amountAMin, amountBMin, to, deadline);
    //    (bool success, ) = pancakeRouter.call(abi.encodeWithSignature("addLiquidity(address,address,uint,uint,uint,uint,address,uint)", tokenA, tokenB, liquidityLimit, liquidityLimit, liquidityLimit, liquidityLimit, to, deadline));
    //    bytes memory encodedData = abi.encode(tokenA, tokenB, liquidityLimit, liquidityLimit, liquidityLimit, liquidityLimit, to, deadline);
    //    (bool success, ) = pancakeRouter.call(bytes4(keccak256("addLiquidity(address,address,uint,uint,uint,uint,address,uint)")),abi.encode(tokenA, tokenB, liquidityLimit, liquidityLimit, liquidityLimit, liquidityLimit, to, deadline));
       
    //    return success;

        // bytes memory payload = abi.encodeWithSignature("addLiquidity(address,address,uint,uint,uint,uint,address,uint)", tokenA, tokenB, liquidityLimit, liquidityLimit, liquidityLimit, liquidityLimit, to, deadline);

    //    address(pancakeRouter).call(payload);
