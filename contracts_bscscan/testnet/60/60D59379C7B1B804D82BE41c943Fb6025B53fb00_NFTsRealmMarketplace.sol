...
contract NFTsRealmMarketplace is ReentrancyGuard {
    ...
    function buy(uint _id, address _creator, uint256 _price, bool _isNew, string memory _tokenUri) external payable {
        if (_isNew) {
            ...
            _validate(_id);
            address _previousOwner = NFTsRealm(nftContract).ownerOf(_id);
            ...
            transferKCSOrWKCS(payable(_previousOwner), _sellerValue);
            transferKCSOrWKCS(payable(adminRecoveryAddress), _commissionValue);
            NFTsRealm(nftContract).transferFrom(_previousOwner, _newOwner, _id);
    ...
    function createAuction(
        ...
        NFTsRealm(nftContract).transferFrom(
            NFTsRealm(nftContract).ownerOf(tokenId),
            address(this),
            tokenId
        );
        ...
        auctionComplete(tokenId)
    {
        ...
        if (CreatorFeePercent > 0) {
            ...
            transferKCSOrWKCS(payable(Creator), CreatorAmount);
