...
contract StakingManagerV1 is IStakingManagerV1, Authorizable, Pausable, IDCounter, ReentrancyGuard {
  ...
  uint256 private _distributionRewardAmountModifier = 500 * 10 ** 10;
  ...
  function _getAvailableRewards() private view returns (uint256) {
    ...
    rewards += (block.timestamp - _getMostRecentDepositOrDistribution()) * _distributionRewardRate;
    rewards += Math.mulScale(
      _waitingRewards,
      (block.timestamp - _lastDistributionAt) * _distributionRewardAmountModifier,
      10 ** 18
    );
  ...
  function getAllRewardsForAddress(address account) external view override returns (uint256 pending, uint256 claimed) {
    ...
    for (uint40 i = 0; i < _count && gasleft() > 20000; i++) {
      (uint256 amount,) = _staking[i].pending(account);
      pending += amount;
      (,,,,uint256 amountClaimed) = _staking[i].getStakingDataForAccount(account);
...
contract StakingV1 is Authorizable, Pausable, IDCounter, ReentrancyGuard {
  ...
  bool private _autoClaimOnDeposit = true;

  /** @dev should autoClaim be run automatically on manual claim? */
  bool private _autoClaimOnClaim = true;
  ...
  function _pending(address account) private view returns (uint256 amount, uint64 lastDepositId) {
    amount = 0;
    ...
    if (_stakers[account].amount != 0) {
      ...
      // TEMP use `gasleft() > x` as a temporary solition
      for (lastDepositId = _lastClaimId(account); lastDepositId < _count && gasleft() > 20000; lastDepositId++) {
        amount += Math.mulScale(
          _deposits[lastDepositId].amount,
          Math.percent(
            _stakers[account].amount,
            _deposits[lastDepositId].totalStaked,
            18
          ),
          10 ** 18
        );
      ...
      lastDepositId = uint64(_count);
    }
  }

  function pending(address account) external view returns (uint256 amount, uint64 lastDepositId) {
    return _pending(account);
  }

  function _claim(address account) private returns (bool) {
    (uint256 pendingRewards, uint64 lastDepositId) = _pending(account);

    // always keep track of the last claimed id to minimize iterations in _pending
    _stakers[account].lastClaimData = uint192(lastDepositId);
    _stakers[account].lastClaimData |= uint192(uint64(block.number)) << 64;
    _stakers[account].lastClaimData |= uint192(uint64(block.timestamp)) << 128;
  ...
  function _autoClaim() private {
    ...
    while (startingGas - gasleft() < _autoClaimGasLimit && iterations++ < _totalNumStakers) {
      ...
      _claim(_autoClaimQueue[1 + (index % _totalNumStakers)]);
    }
  }
