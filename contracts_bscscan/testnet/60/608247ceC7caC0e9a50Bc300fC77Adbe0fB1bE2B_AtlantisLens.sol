...
contract AtlantisLens {
    ...
    function getAtlantisBalanceMetadata(Atlantis atl, address account) external view returns (AtlantisBalanceMetadata memory) {
        return AtlantisBalanceMetadata({
            balance: atl.balanceOf(account),
            votes: uint256(atl.getCurrentVotes(account)),
            delegate: atl.delegates(account)
        });
    }
    ...
    function getAtlantisBalanceMetadataExt(Atlantis atl, ComptrollerLensInterface comptroller, address account) external returns (AtlantisBalanceMetadataExt memory) {
        uint balance = atl.balanceOf(account);
        comptroller.claimAtlantis(account);
        uint newBalance = atl.balanceOf(account);
        uint accrued = comptroller.atlantisAccrued(account);
        uint total = add(accrued, newBalance, "sum atl total");
    ...
    function getAtlantisVotes(Atlantis atl, address account, uint32[] calldata blockNumbers) external view returns (AtlantisVotes[] memory) {
        AtlantisVotes[] memory res = new AtlantisVotes[](blockNumbers.length);
        for (uint i = 0; i < blockNumbers.length; i++) {
            res[i] = AtlantisVotes({
                blockNumber: uint256(blockNumbers[i]),
                votes: uint256(atl.getPriorVotes(account, blockNumbers[i]))
            });
        }
        return res;
    }
...
contract GovernorAlpha {
    ...
    constructor(address timelock_, address atl_, address guardian_) public {
        timelock = TimelockInterface(timelock_);
        atl = AtlantisInterface(atl_);
        guardian = guardian_;
    }

    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        require(atl.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
    ...
    function cancel(uint proposalId) public {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || atl.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
    ...
    function _castVote(address voter, uint proposalId, bool support) internal {
        ...
        uint96 votes = atl.getPriorVotes(voter, proposal.startBlock);
