...
contract EpicAuction is AccessControl, EpicWhiteList {
    ...
    function placeAuction(
        ...
        require(_getWhiteList(token), "Token is not in white list");
        ...
        require(IERC20(token).balanceOf(msg.sender) >= getFee(price), "Balance is not enough to pay fee");
        IERC20(token).transferFrom(msg.sender, _wallet, getFee(price));
        items.push(
            Item(
                item,
                itemType,
    ...
    function cancelBid(uint256 marketId, uint256 offerId) public onlyExistItem(marketId) returns(bool) {
        ...
        require(IERC20(items[marketId]._token).balanceOf(msg.sender) >=  getFee(bidData._price), "Balance is not enough to pay fee");
        IERC20(items[marketId]._token).transferFrom(msg.sender, _wallet, getFee(bidData._price));
