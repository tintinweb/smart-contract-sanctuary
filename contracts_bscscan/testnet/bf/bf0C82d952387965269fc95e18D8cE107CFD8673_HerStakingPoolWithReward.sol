...
import "./IHerStakingPool.sol";
import "./HerStakingPool.sol";
import "./HerStaking.Library.sol";
import "hardhat/console.sol";

abstract contract RewardAdder is IHerStakingPool {
    address  public rewardTokenAddress;
    HerStakingLib.HerStakingRewardState public rewardState;
    ...
    external returns (bool) {
        return HerStakingLib.addReward(rewardAmount, withdrawableAmount,
            rewardTokenAddress, rewardState);
    }
}

contract HerStakingPoolWithReward is HerStakingPool, RewardAdder {
    ...
    constructor (string memory name_,
        ...
        require(rewardTokenAddress_ != address(0), "HerStakingPool: 0 reward address");
    ...
    external {
        require(msg.sender == rewardSetter, "HerStakingPool: Not allowed");
        rewardState.earlyWithdrawReward = withdrawableAmount;
        HerStakingLib.addMarginalReward(rewardTokenAddress, tokenAddress,
...
/**
 * @dev HER Staking interface
 */
interface IHerStakingPool {
...
contract HerStakingPool is IHerStakingPool {
    ...
    HerStakingLib.HerStakingState public stakeState;
    ...
    constructor (
        ...
        require(HerStakingLib.VERSION() == LIB_VERSION, "Bad linked library version");

        _name = name_;

        require(tokenAddress_ != address(0), "HerStakingPool: Missing token address");
        tokenAddress = tokenAddress_;

        require(stakingStarts_ > 0, "HerStakingPool: zero staking start time");
        ...
        require(stakingEnds_ >= stakingStarts, "HerStakingPool: staking end must be after staking starts");
        stakingEnds = stakingEnds_;

        require(withdrawStarts_ >= stakingEnds, "HerStakingPool: withdrawStarts must be after staking ends");
        withdrawStarts = withdrawStarts_;

        require(withdrawEnds_ >= withdrawStarts, "HerStakingPool: withdrawEnds must be after withdraw starts");
        withdrawEnds = withdrawEnds_;

        require(stakingCap_ >= 0, "HerStakingPool: stakingCap cannot be negative");
    ...
    */
    ...
    function _stake(address payer, address staker, uint256 amount) internal virtual returns (bool) {
        return HerStakingLib.stake(payer, staker, amount,
...
library HerStakingLib {
    ...
    returns (uint256) {
        ...
        // require((remaining + stakedTotal) <= stakingCap, "HerStaking: this will increase staking amount pass the cap");
    ...
    function addReward(
        ...
        HerStakingRewardState storage state
    )
    external
    returns (bool) {
        require(rewardAmount > 0, "HerStaking: reward must be positive");
        require(withdrawableAmount >= 0, "HerStaking: withdrawable amount cannot be negative");
        require(withdrawableAmount <= rewardAmount, "HerStaking: withdrawable amount must be less than or equal to the reward amount");
        address from = msg.sender;
    ...
    function tryWithdraw(
        ...
        uint256 stakingEnds,
        HerStakingState storage state,
        HerStakingRewardState storage rewardState
    )
    public
    ...
    returns (uint256) {
        require(amount <= state._stakes[from], "HerStaking: not enough balance");
    ...
    returns (uint256) {
        ...
        require(principalPaid && rewardPaid, "HerStaking: error paying");
    ...
    modifier _realAddress(address addr) {
        require(addr != address(0), "HerStaking: zero address");
        _;
    }

    modifier _positive(uint256 amount) {
        require(amount != 0, "HerStaking: negative amount");
        _;
    }

    modifier _after(uint eventTime) {
        require(block.timestamp >= eventTime, "HerStaking: bad timing for the request");
        _;
    }
