...
contract LPTokenWrapper {
    ...
    function stake(uint256 amount) public {
        ...
        depositAddr.safeTransferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256 amount) public {
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        depositAddr.safeTransfer(msg.sender, amount);
    }

    function withdrawTo(address to, uint256 amount) internal {
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        depositAddr.safeTransfer(to, amount);
    }
...
contract PlayerStation {
    ...
    function bePlayer(uint256 buySize,uint256 limitPA) internal {
        playerLimit[msg.sender].totalLimit += buySize.mul(limitPA);
    }
...
contract ERC20Pool is PlayerStation, LPTokenWrapper, IRewardDistributionRecipient {
    // rewardToken
    ...
    uint256 public farmSize;
    uint256 public pricePerAcre;
    uint256 public limitPerAcre;
    uint256 public farmUsed;
    
    bool    private farmInit;
    ...
    event initFarmed(address farmowner,address stake,address earn,uint256 size ,uint256 price,uint256 limitPA,uint256 rate );
    event started(uint256 start,uint256 period);
    event bePlayered(address playeradr,uint256 buySize,uint256 amount,uint256 farmerUsed,uint256 limitPA,uint256 supply);

    constructor() public{
        transferOper(0xfBb4EB0A18920dc242761Fb3417663ba25784c22);
        //depositAddr = IERC20(_trc20);
    ...
    modifier checkStart() {
        ...
        require(farmInit == true , "not init");
        _;
    }
    ...
    function initFarm(address farmowner,address stake,address earn,uint256 size ,uint256 price,uint256 limitPA,uint256 rate) public onlyOper {
        require((farmowner!=address(0))&&(stake != address(0))&&(earn != address(0)));
        //depositAddr = stake;
        farmer       = farmowner;
        depositAddr  = IERC20(stake);
        rewardToken  = IERC20(earn);
        farmSize     = size;
        pricePerAcre = price;
        dividendRate = rate;
        limitPerAcre = limitPA;
        
        farmInit = true;
        
        emit initFarmed(farmowner,stake,earn,size,price,limitPA,rate);
    }
    
    function setStartTime( uint256 start,uint256 period ) public onlyOper {
        require(start > block.timestamp);
        starttime = start;
        DURATION  = period;
        emit started(start,period);
    }
    
    function bePlayer(uint256 buySize ) public  checkStart  {
        //string memory tokenName = IERC20(tokenAddr).symbol(); 
        
        require(farmSize >= (farmUsed.add(buySize)));
        
        super.bePlayer(buySize,limitPerAcre);
        
        farmUsed += buySize;
        
        uint256 amount = buySize.mul(pricePerAcre);
        
        rewardToken.transferFrom(msg.sender,farmer,amount);
        
        emit bePlayered(msg.sender,buySize,amount,farmUsed,limitPerAcre,super.totalSupply());
    }
    ...
    function earned(address account) public view returns (uint256) {
        ...
        return playerEarn.mul(100 - dividendRate).div(100);
    }
    
    function farmerEarned() public view returns (uint256){
        return rewardNow.mul(dividendRate).div(100);
    }
    ...
    function getReward() public updateReward(msg.sender) checkStart {
        uint256 trueReward   = _earned(msg.sender);
        uint256 farmerReward = trueReward.mul(dividendRate).div(100);
        uint256 playerReward = trueReward - farmerReward;
        if (trueReward > 0) {
            rewards[msg.sender] = 0;
            rewardToken.safeTransfer(msg.sender, playerReward);
            rewardToken.safeTransfer(farmer,farmerReward);
            //emit RewardPaid(msg.sender, trueReward);
            emit RewardPaid(farmer, farmerReward);
            emit RewardPaid(msg.sender, playerReward);
        }
    }
   ...
   updateReward(address(0))
    ...
    /**
    * @dev rescue simple transfered elfin.
    */
    ...
    onlyOwner
    {
        ...
        require(token_ != depositAddr, "must not this plToken");
