...
contract TEST is IBEP20, Ownable {
    ...
    uint256 private constant _tTotalSupply = 2000000000 * 10 ** 9; 
	uint256 private _buyMaxTxAmount = 2000000000 * 10 ** 9; 
    uint256 private _sellMaxTxAmount = 2000000000 * 10 ** 9;
	
    uint256 private _buyTaxFee = 3;
    uint256 private _buyLiquidityFee = 1;
    uint256 private _buyMarketingFee = 2;
    uint256 private _buyWhaleBuybackFee = 2;
    
    uint256 private _sellTaxFee = 3;
    uint256 private _sellLiquidityFee = 1;
    uint256 private  _sellMarketingFee = 4;
    uint256 private _sellWhaleBuybackFee = 6;
    ...
    uint256 private _whaleBuybackFee;
    uint256 private _previousWhaleBuybackFee;

    uint256 private _tFeeTotal;
    uint256 private _tMarketingTotal;
    uint256 private _tWhaleBuybackTotal;
    ...
    address public constant marketingWallet = 0xd4aAd6f7F219d218ba220068F8045bA94967F84A;
    //0xC02344726E1b541cbA47E0aAF793CB9716BE6f68;

    // wallet for whale buyback
    address public constant whaleBuybackWallet = 0x5008068b88F39Cf71C6D3DbE75c11a39C00ccd93;
    // 0x1C02598D78f50ec4711bf34968F879Da8331aA9C;
    ...
    constructor(address tokenOwner) {
        ...
        _whaleBuybackFee = _buyWhaleBuybackFee;
        _previousWhaleBuybackFee = _whaleBuybackFee;
    ...
    function getSellWhaleBuybackFee() public view returns (uint256) {
        return _sellWhaleBuybackFee;
    }
    
    function getBuyWhaleBuybackFee() public view returns (uint256) {
        return _buyWhaleBuybackFee;
    }
    ...
    function totalWhaleBuybackFees() public view returns (uint256) {
        return _tWhaleBuybackTotal;
    }
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns (uint256) {
        ...
        if (!deductTransferFee) {
            ...
            _tv.tTransferAmount = 0; _tv.tFee = 0; _tv.tLiquidity = 0; _tv.tMarketingFee = 0; _tv.tWhaleBuybackFee = 0;
            _rv.rTransferAmount = 0; _rv.rFee = 0; _rv.rLiquidity = 0; _rv.rMarketingFee = 0; _rv.rWhaleBuybackFee = 0;
            return _rv.rAmount;
        } else {
            (rValues memory _rv, tValues memory _tv) = _getValues(tAmount); 
            _tv.tTransferAmount = 0; _tv.tFee = 0; _tv.tLiquidity = 0; _tv.tMarketingFee = 0; _tv.tWhaleBuybackFee = 0;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (from == pancakePair) { // Buy
                ...
                _whaleBuybackFee = _sellWhaleBuybackFee;
            } else { // other
                _taxFee = 0;
                _liquidityFee = 0;
                _marketingFee = 0;
    ...
    function _getValues(uint256 tAmount) private view returns (rValues memory, tValues memory) {
        ...
        rValues memory _rv = _getRValues(tAmount, _tv.tFee, _tv.tLiquidity, _tv.tMarketingFee,  _tv.tWhaleBuybackFee, _getRate());
        return (_rv, _tv);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (tValues memory) {
        ...
        _tv.tWhaleBuybackFee = calculateWhaleBuybackFee(tAmount);
        _tv.tTransferAmount = tAmount.sub(_tv.tFee).sub(_tv.tLiquidity).sub(_tv.tMarketingFee).sub(_tv.tWhaleBuybackFee);
        return (_tv);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketingFee, uint256 tWhaleBuybackFee, uint256 currentRate) private pure returns (rValues memory) {
        rValues memory _rv;
        ...
        _rv.rWhaleBuybackFee = tWhaleBuybackFee.mul(currentRate);
        _rv.rTransferAmount = _rv.rAmount.sub(_rv.rFee).sub(_rv.rLiquidity).sub(_rv.rMarketingFee).sub(_rv.rWhaleBuybackFee);
        return (_rv);
    }
    ...
    function calculateWhaleBuybackFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_whaleBuybackFee).div(10**2);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 _totalLiquidityFee = _buyLiquidityFee + _sellLiquidityFee;
        uint256 liquidityPercent = _totalLiquidityFee.div(totalFees);
        uint256 liquidityQuota = liquidityPercent.mul(contractTokenBalance);
        ...
        swapBNBForBUSD(marketingWallet, address(this).balance.sub(initialBalance));
        ...
        swapBNBForBUSD(whaleBuybackWallet, address(this).balance.sub(initialBalance2));
    ...
    function swapBNBForBUSD(address recipient, uint256 tokenAmount) private{
    ...
    function restoreAllFee() private {
        ...
        _whaleBuybackFee = _previousWhaleBuybackFee;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        //_transferWhaleBuybackFee(sender, _rv.rWhaleBuybackFee, _tv.tWhaleBuybackFee);
    }
    ...
    // function _transferWhaleBuybackFee(address sender, uint256 rWhaleBuybackFee, uint256 tWhaleBuybackFee) private {
    //     if (_whaleBuybackFee > 0) {
    //         _rBalances[whaleBuybackWallet] = _rBalances[whaleBuybackWallet].add(rWhaleBuybackFee);
    //         emit Transfer(sender, whaleBuybackWallet, tWhaleBuybackFee);
    //         _tWhaleBuybackTotal = _tWhaleBuybackTotal.add(tWhaleBuybackFee); 
    //     }
    // }
