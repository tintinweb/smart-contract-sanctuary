...
contract Staking is Ownable {
    ...
    function withdrawAllTokens() external {
        ...
        if (pending > 0 || user.pendingRewards > 0) {
            user.pendingRewards = user.pendingRewards + pending;
            tempPendingReward = user.pendingRewards;
        ...
        liquidityMining.stakingToken.safeTransfer(address(msg.sender), user.amount + tempPendingReward);
        tempPendingReward = tempPendingReward - tempPendingReward;
