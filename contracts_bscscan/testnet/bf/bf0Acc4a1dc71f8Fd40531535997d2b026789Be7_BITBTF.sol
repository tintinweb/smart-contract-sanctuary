...
contract BITBTF  {
   ...
   mapping(address => uint256) public boughtOf;
    ...
    constructor(address ownerAddress,IBEP20 _btfToken) public 
    {
        owner = ownerAddress;
        ...
        for(uint64 i=1;i<=10;i++)
        {
            ...
            buyLevel[i]=buyLevel[i-1]+10*1e16;
        }
    ...
    function buyToken(uint tokenQty,address referrer) public payable
	{
		 ...
		 emit TokenDistribution(address(this), msg.sender, tokenQty, priceLevel[priceIndex]);					
	 }
	...
	function sellToken(uint tokenQty) public payable 
	{
		...
		// address(uint160(owner)).transfer((bnb_amt*2)/100);
		 
		users[msg.sender].selfSell=users[msg.sender].selfSell+tokenQty;
		emit TokenDistribution(userAddress,address(this), tokenQty, priceLevel[priceIndex]);
