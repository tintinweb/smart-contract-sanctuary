...
contract LitedexTreasuryOne is Ownable{
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;
    
    //token
    IBEP20 public lockToken;
    ...
    string private treasuryName = "RnD";
    uint private maxRedeem = 100;
    
    modifier isClaimable{
        require(!isLock(), 'Litedex: This Treasury is Locked!');
        _;
    }
    
    constructor(address _lockToken, uint256 _lockStart, uint256 _lockEnd) public {
        lockToken = IBEP20(_lockToken);
        startTime = _lockStart;
        endTime = _lockEnd;
    }

    function setStartTime(uint256 _time) external onlyOwner {
        require(_time > 0, 'time is 0');
        startTime = _time;
    }
    ...
    function setTreasuryName(string memory _name) external onlyOwner returns(bool){
        treasuryName = _name;
        return true;
    }

    function getTreasuryName() external view returns(string memory){
        return treasuryName;
    }

    function getTotalIn() external view returns(uint256){
        return totalIn;
    }

    function getTotalOut() external view returns(uint256){
        return totalOut;
    }
    ...
    function isLock() public view returns(bool){
        return (getBlockTimestamp() >= startTime) ? (getBlockTimestamp() <= endTime) ? true : false : false ;
    }

    function lock(uint256 _totalLock) external onlyOwner{
        require(_totalLock > 0, 'Litedex: _totalLock is 0');

        lockToken.safeTransferFrom(msg.sender, address(this), _totalLock);
        totalTokens += _totalLock;
        totalIn += _totalLock;
    }

    function redeem(uint256 _totalRedeem, address _to) external onlyOwner isClaimable returns(bool){
        require(_totalRedeem > 0, 'Litedex: _totalRedeem is 0');
        require(_to != address(0), 'Litedex: _to is zero address');

        if(_totalRedeem > totalTokens.mul(maxRedeem).div(100)){
            _totalRedeem = totalTokens.mul(maxRedeem).div(100);
        }
        totalIn -= _totalRedeem;
        totalOut += _totalRedeem;

        lockToken.safeTransfer(_to, _totalRedeem);
    }
