	...
	if (user.deposits.length == 0) {
      ...
      function winthdrawBullBnbTokens(uint256 i)public returns(bool){
        ...
        if(user.deposits[i].unstaked){
            require(block.timestamp>user.deposits[i].nextForHalfAmountReleasingTime.add(15 days),"you have to wait for 15 days");
            bullBNBToken.transferFrom(owner,userAddress,user.deposits[i].otherhalfAmount.mul(33).div(100));
            user.deposits[i].otherhalfAmount=user.deposits[i].otherhalfAmount.mul(67).div(100);
		...
		uint256 dividends;
					dividends = (user.deposits[i].amountInBullBNB.mul(percentage).div(PERCENTS_DIVIDER))
						.mul(getUserTimeAfterDeposit(userAddress,i)).div(1 minutes);
        ...
        user.deposits[i].otherhalfAmount=user.deposits[i].otherhalfAmount.add(totalAmount.mul(33).div(100));
		bullBNBToken.transferFrom(owner,msg.sender,totalAmount.mul(33).div(100));
