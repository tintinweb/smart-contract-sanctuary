...
contract DogeBack is BEP20, Ownable {
    ...
    uint256 private _sellMaxTxAmount  = 400000 * 10**9; // 0.04% percent of total supply per sell transaction
    uint256 public _sellMaxTxAmount2  = 200001 * 10**9;
    uint256 public _maxWalletAmount   = 1500000000 * 10**9; // 1.5% of total supply  
    uint256 public _minWalletAmount   = 25000 * 10**9; // 0.0025% of total supply for safety
    uint256 private tokenSellToLiq    = 20000000 * 10**9; // 0.05% tx amount will trigger swap and add liquidity
    ...
    constructor() BEP20("DogeBack", "DOGEBACK") {      
        ...
        _isExcludedFromFees[address(pancakeswapV2Router)] = true;
    ...
    function setMinBuy(uint256 _minBuy) external onlyOwner { minBuy = _minBuy; }  
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (!(_isExcludedFromMaxTx[from] || _isExcludedFromMaxTx[to])) {
            if (automatedMarketMakerPairs[from] && to != address(this)) {
        ...
        if (!(_isExcludedMaxWallet[from] || _isExcludedMaxWallet[to]) && (!automatedMarketMakerPairs[to])) {
