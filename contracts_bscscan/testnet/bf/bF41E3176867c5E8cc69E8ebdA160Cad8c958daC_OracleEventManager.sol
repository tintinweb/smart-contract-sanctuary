...
// contract OracleEventManager is AccessControl {
// contract OracleEventManager is PriceConsumerV3, AccessControl {

    bytes32 public constant EVENT_RUNNER_ROLE = keccak256("EVENT_RUNNER_ROLE");
    ...
    uint public _lastEventId = 0;           // TODO: check for need
    uint public _checkPeriod = 60;          // in seconds

    GameEvent public _queuedEvent;
    GameEvent public _ongoingEvent;         // TODO: check for need
    ...
    // ) PriceConsumerV3(priceFeedAddress) {
        ...
        // priceFeed = AggregatorV3Interface(0x9326BFA02ADD2366b30bacB125260Af641031331); // Kovan, pair: ETH / USD
        // priceFeed = AggregatorV3Interface(0x264990fbd0A4796A3E3d8E37C4d5F87a3aCa5Ebf); // BSC, pair: BTC / USD
        _priceFeed = AggregatorV3Interface(priceFeedAddress);
        _lastEventId = 0;           // TODO: check for need
        _checkPeriod = checkPeriod;
    }

    function prepareEvent() external {
        // ========= Check for already prepared, for impossible re-adding =========
        require(_queuedEvent.createdAt == 0, "Already prepared event");
        // ========= Check for already prepared, for impossible re-adding =========
    ...
    function startEvent() external {
        require(_queuedEvent.createdAt != 0, "Not prepared event");
        require(_queuedEvent.startedAt == 0, "Event already started");

        require(block.timestamp > _queuedEvent.eventStartTimeExpected - (_checkPeriod / 2), "Too early start");
        require(block.timestamp < _queuedEvent.eventStartTimeExpected + (_checkPeriod / 2), "It's too late to start");
        ...
        _queuedEvent.startedAt = block.timestamp;

        emit AppStarted(block.timestamp, _queuedEvent.eventStartTimeExpected, _startRoundData.roundID, _queuedEvent.startedAt);

        // ===================== TODO: Дополнительная проверка событий от ELC ================================

    }

    function endEventTooLate() public onlyOwner {
        require(_queuedEvent.createdAt != 0, "Not prepared event");
        require(_queuedEvent.startedAt == 0, "Event already started");
        if (block.timestamp < _queuedEvent.eventStartTimeExpected + (_checkPeriod / 2)) {
            // Send 0 means draw. 
            int8 gameResult = 0;
            IEventLifeCycle(_eventLifeCycleAddress).endEvent(gameResult);

            _queuedEvent.endedAt = block.timestamp;
            
            emit AppEnded(_queuedEvent.endedAt, _queuedEvent.eventEndTimeExpected, _endRoundData.roundID, gameResult);
    ...
    function checkEvent() internal view {
        require(_queuedEvent.createdAt != 0, "Not prepared event");
        require(_queuedEvent.startedAt != 0, "Event not started");
        require(block.timestamp >= _queuedEvent.eventEndTimeExpected, "Too early end");
        require(_queuedEvent.endedAt == 0, "Event already finalazed");
    }
    ...
    function finalizeEvent() public {
        ...
        uint diff = 0;
        if (_queuedEvent.eventEndTimeExpected > timeStamp) {
            diff = _queuedEvent.eventEndTimeExpected - timeStamp;
        }
        if (timeStamp > _queuedEvent.eventEndTimeExpected) {
            diff = timeStamp - _queuedEvent.eventEndTimeExpected;
        }
        
        emit Some(_queuedEvent.eventEndTimeExpected, block.timestamp, timeStamp);
        ...
        require(timeStamp == _queuedEvent.eventEndTimeExpected, "TimeStamp is not equal eventEndTimeExpected");
    ...
    // Deprecated, use `transferOwnership` from the owner account to a new one
    // function changeGovernanceAddress(address governanceAddress) public onlyGovernance {}

    // function changeOracleAddress(address eventLifeCycleAddress) public onlyOwner {
    //     require (eventLifeCycleAddress != address(0), "New event lifecycle address should be not null");
    //     require (_eventLifeCycleAddress != eventLifeCycleAddress, "New event lifecycle address should be equal old address");
    //     _eventLifeCycleAddress = eventLifeCycleAddress;
