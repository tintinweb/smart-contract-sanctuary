...
contract GEMUNIMarketplace is IGEMUNIMarketplace, PermissionGroupUpgradeable, PausableUpgradeable, ReentrancyGuard {
    ...
    function getPricePass(uint passId, uint price, IGENIPass.PassType passType) public override view returns(uint pricePass) {
        bool isExistedToken = IGENIPass(address(geniPass)).getMintedCount(passId);
    ...
    function putOnSale(address token, uint tokenId, uint price, uint startTime, uint expirationTime) external override whenNotPaused {
        ...
        if(token == address(geniPass)) {
            ...
            emit PassPutOnSale(tokenId, saleInfo.price, uint(IGENIPass.PriceType.GENI), saleInfo.seller, startTime, expirationTime);
        } else {
    ...
    function updatePriceSale(address token, uint tokenId, uint price) external override whenNotPaused {
        ...
        if(token == address(geniPass)){
            emit PassUpdateOnSale(tokenId, saleInfo.price);
        } else {
            emit ItemUpdateOnSale(tokenId, saleInfo.price);
        }
    }
    ...
    function makeOffer(address token, uint tokenId, uint offerPrice, uint expirationTime) external override whenNotPaused nonReentrant {
        require(token == address(geniPass) || token == address(gemuniItem), "GEMUNIMarketplace: invalid token");
        require( expirationTime > block.timestamp, "GEMUNIMarketplace: invalid start time");
        SaleInfo storage saleInfo = saleInfos[token][tokenId];
        address buyer = msg.sender;
        ...
        if(token == address(geniPass)) {
            emit PassOffered(tokenId, buyer, offerPrice, expirationTime);
        } else {
            emit ItemOffered(tokenId, buyer, offerPrice, expirationTime);
        }
    }
    ...
    function takeOffer(address token, uint tokenId, address buyer, uint takeOfferPrice) external override whenNotPaused nonReentrant {
        ...
        if(token == address(geniPass)) {
            ...
            if(saleInfo.seller == address(0)) {
                ...
                geni.safeTransfer(saleInfo.seller, price);
                gemuniItem.transferFrom(address(this), buyer, tokenId);
                emit ItemBought(tokenId, buyer, saleInfo.seller, offerInfo.priceOffer);
            }
        }
...
interface IGEMUNIMarketplace {
    ...
    event PassOffered(uint indexed passId, address buyer, uint price, uint expirationTime);
    event PassOfferCancelled(uint indexed passId, address buyer);
    event PassOfferUpdated(uint indexed passId, address buyer, uint newOfferPrice);

    event ItemPutOnSale(uint indexed itemId, uint price, address seller, uint startTime, uint expirationTime);
    ...
    event ItemOffered(uint indexed itemId, address buyer, uint price, uint expirationTime);
...
contract GENIPassSale is IERC721Receiver, PermissionGroup, ReentrancyGuard, Pausable {
    ...
    event PassPutOnSale(uint indexed passId, uint price, uint priceType, address seller);
    event PassUpdateSale(uint indexed passId, uint newPrice);
    event PassRemoveFromSale(uint indexed passId);
    event PassBought(
        uint indexed passId,
        address buyer,
        address seller,
    ...
    function putOnSale(uint passId, uint price, IGENIPass.PriceType priceType) public whenNotPaused onlyOperator {
        IGENIPass.GeniPass memory pass =  IGENIPass(address(geniPass)).getPass(passId);
        require(pass.passType != IGENIPass.PassType.Stone, "GENIPassSale: not sale Stone pass");
    ...
    function mintForSale(string memory serialNumber, uint price, IGENIPass.PassType passType, IGENIPass.PriceType priceType) public whenNotPaused onlyOperator {
    ...
    function updateSale(uint passId, uint price) external onlyOperator whenNotPaused {
        ...
        emit PassUpdateSale(passId, saleInfo.price);
    }
    ...
    function removeFromSale(uint passId) external onlyOperator whenNotPaused nonReentrant {
        ...
        emit PassRemoveFromSale(passId);
    ...
    function putOnSaleBatch(CreateSalePassesWithoutMint[] memory input) external whenNotPaused onlyOperator {
        for (uint i = 0; i < input.length; i++) {
            putOnSale(input[i].passId, input[i].price, input[i].priceType);
        }
    }
    
    function mintForSaleBatch(CreateSalePasses[] memory input) external whenNotPaused onlyOperator {
        for (uint i = 0; i < input.length; i++) {
             mintForSale(input[i].serialNumber, input[i].price, input[i].passType, input[i].priceType);
        }
    }
    ...
    function purchase(uint passId, uint buyPrice) external payable whenNotPaused nonReentrant {
        ...
        emit PassBought(passId, buyer, treasury, payPrice);
        delete saleInfos[passId];
    }
    ...
    function purchaseWithReferal(uint passId, uint buyPrice, address referalAddress, uint nonce, bytes memory signature) external payable whenNotPaused nonReentrant {
        ...
        emit PassBoughtWithReferal( passId, buyer, treasury, referalAddress, payPrice);
        delete saleInfos[passId];
    }
    ...
    function getPriceDiscount(uint price) external view returns (uint) {
