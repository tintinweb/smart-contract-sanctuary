...
contract GEPSaleContract is owned {
     ...
     using SafeMath for uint256;
       ...
       uint256 public totalSaleToken = 350000000 ;
       ...
       mapping (address => uint256)  waitReleaseNumber;   //用户冻结数
    ...
    function Swap(uint256 _amount) public returns(bool){
        ...
        waitReleaseNumber[msg.sender] = a.add(tokenNumber); 
    ...
    function userClaim() public {
        ...
        transferToken(msg.sender,releaseBalance.mul(gepDecimal));
        emit mglog(102);
    }
    ...
    function managerClaim(address _addr) onlyOwner public {
        ...
        require(releaseBalance>0,"cliam fail balance isn't enough");
         cliamNumber[_addr] = 0;
        transferToken(_addr,releaseBalance.mul(gepDecimal));
        emit mglog(103);
    }
    ...
    function releaseSendToken() onlyOwner public{
         ...
         for(uint i=0;i<len; i ++ ){
             ...
             if(feezoneBalance>0){
                 ...
                 if(feezoneBalance>=a){
                     thisend = a;
                     waitReleaseNumber[addr] = feezoneBalance.sub(a);
                 }else{
                     thisend = feezoneBalance;
                     waitReleaseNumber[addr] = 0;
                 }
                 transferToken(addr,thisend.mul(gepDecimal));
             }
         }
         emit mglog(100);
    }
