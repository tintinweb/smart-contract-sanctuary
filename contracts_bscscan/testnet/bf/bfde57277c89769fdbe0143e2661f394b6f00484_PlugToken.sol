...
contract PlugToken is Context,IMintableToken, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Plug Token";
    string private _symbol = "Plug";
    uint8 private _decimals = 18;

    address payable public marketingWalletAddress = payable(0x34bBb5EB7e698e2C565600a5599E64985087Ed3A); // Marketing Address
    address payable public buyBackWalletAddress = payable(0x2A1DA3F6711973524908EAbACAAFEcDcACD29388); // buyback Address
    ...
    uint256 private _initialSupply = 10 * 10**9 * 10** uint256(_decimals); // initial supply for owner 
    uint256 public _maxTxAmount = 10 * 10**9 * 10** uint256(_decimals); // max transaction amount
    uint256 public _walletMax = 6000 * 10** 6; // max a wallet can hold.
    uint256 private _maxSupply = 50 * 10**9 * 10** uint256(_decimals); // max circulating supply;
    uint256 private minimumTokensBeforeSwap = 8000 * 10**6; // minimum token for swapping
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        require(_totalSupply.add(amount)<=_maxSupply,"max limit reached!");
    ...
    function convertRewardToBnb() external returns(bool) {
        
        require(_balances[_msgSender()]>0," Insufficient balance");
    ...
    constructor () {
        ...
        //0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 test router
