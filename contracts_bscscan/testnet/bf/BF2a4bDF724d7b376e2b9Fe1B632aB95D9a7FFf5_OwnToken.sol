...
contract OwnToken is ERC20, Ownable {
    ...
    OwnTokenDividendTracker public dividendTracker;
    address public marketingWallet = 0xC9a5f6310D639fAd59e51B3E955266Dc45B833D7;
    address public buybackWallet = 0x98c72A46A1aA46b1ceC05A86C11a0797587Abb87;
    ...
    constructor() ERC20("OWNTOKEN", "OWT") {
        dividendTracker = new OwnTokenDividendTracker();
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "OwnToken: The router already has that address");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "OwnToken: The dividend tracker already has that address");

        OwnTokenDividendTracker newDividendTracker = OwnTokenDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "OwnToken: The new dividend tracker must be owned by the OwnToken token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "OwnToken: Account is already the value of 'excluded'");
    ...
    function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPeriodLimit[account] != excluded, "OwnToken: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "OwnToken: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "OwnToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "OwnToken: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "OwnToken: This account cannot send tokens until trading is enabled");
        }
...
contract OwnTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("OwnTokenDividendTracker", "Honey_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "OwnTokenDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "OwnTokenDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main Honey contract.");
    }
