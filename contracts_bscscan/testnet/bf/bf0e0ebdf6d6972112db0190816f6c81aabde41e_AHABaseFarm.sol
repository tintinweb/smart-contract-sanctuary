...
contract AHABaseFarm is LPTokenWrapper, Ownable, IRewardDistributionRecipient {
    ...
    mapping(address => uint256) public vaidDate;
    ...
    function setvaidDate(uint256 _vaidDate,address _addr) external onlyOwner{
        vaidDate[_addr] = _vaidDate;
    }
        ...
        checkStart
    {
        ...
        if(balanceOf(msg.sender) > mintokenamount){
            vaidDate[msg.sender] = block.timestamp;
        }
    ...
    function withdraw(uint256 amount)
        ...
        checkStart
    {   
        require(lastStakeDate[msg.sender] + withdrawinterval < block.timestamp);
        require(amount > 0, 'Cannot withdraw 0');
        super.withdraw(amount);
        emit Withdrawn(msg.sender, amount);
        if(balanceOf(msg.sender) < mintokenamount){
            vaidDate[msg.sender] = 0;
        }
    }
    ...
    function isWhitelist(address _addr)
        ...
        returns(bool)
    {   
        if(vaidDate[_addr] + whitelistDays < block.timestamp){
            return true;
        }
        else{
            return false;
