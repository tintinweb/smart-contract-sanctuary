...
contract ValidatorTeam is Context, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    
    // validator
    mapping(address => bool) public validator;
    address [] private validatorArray;
    
    /*modifier onlyValidator() {
        require(validator[msg.sender], "!validator");
    _;
    }*/
    ...
    function addValidator(address _address) external onlyOwner {
        require(validator[_address] == false, "existed");
        validator[_address] = true;
        validatorArray.push(_address);
    }

    function removeValidator(address _address) external onlyOwner {
        require(validator[_address] == true, "!validator");
        delete validator[_address];
        for (uint256 i = 0; i < validatorArray.length; i++) {
            if (validatorArray[i] == _address) {
                validatorArray[i] = address(0); 
                break;
            }
        }
    }
    ...
    function getAllValidator() external view returns (address[] memory) {
        uint256 len;
        for (uint i = 0; i < validatorArray.length; i++) {
            if (validatorArray[i] != address(0)) {
                len++;
            }
        }
        ...
        for (uint i = 0; i < validatorArray.length; i++) {
            address _validator = validatorArray[i];
            if (_validator != address(0)) {
                result[index] = _validator;
                index++;
            }
        }
        return result;
