...
import "./SmartPiToken.sol";





contract SmartPiFarm is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of SmartPi
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accSmartPiPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accSmartPiPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. SmartPi to distribute per block.
        uint256 lastRewardBlock; // Last block number that SmartPi distribution occurs.
        uint256 accSmartPiPerShare; // Accumulated SmartPi per share, times 1e12. See below.
        uint16 exitFeeBP; // Exit fee in basis points.
    }
    // The SMARTPI TOKEN!
    SmartPiToken public smartPi;
    // Dev address.
    ...
    // SMARTPI tokens created per block.
    uint256 public smartPiPerBlock;
    // Reduction interval
    ...
    // The block number when SMARTPI mining starts.
    ...
    constructor(
        SmartPiToken _smartPi,
        address _devaddr,
        address _insAddr,
        address _feeAddr,
        uint256 _smartPiPerBlock,
        uint256 _startBlock,
        ...
        insAddr = _insAddr;
        feeAddr = _feeAddr;
        smartPiPerBlock = _smartPiPerBlock;
    ...
    // Update the given pool's SMARTPI allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _exitFeeBP,
    ...
    // View function to see pending SMARTPI reward on frontend.
    function pendingSmartPi(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accSmartPiPerShare = pool.accSmartPiPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 smartPiReward = multiplier.mul(smartPiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accSmartPiPerShare = accSmartPiPerShare.add(smartPiReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accSmartPiPerShare).div(1e12).sub(user.rewardDebt);
    }

    // Update reward variable for all pools. Be careful of gas spending!
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 smartPiTotalCap = smartPi.cap();
        uint256 smartPiTotalSupply = smartPi.totalSupply();
        uint256 remainingFeed = smartPiTotalCap.sub(smartPiTotalSupply);
        uint256 smartPiReward = multiplier.mul(smartPiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        if (remainingFeed < smartPiReward) {
            smartPiReward = remainingFeed;
        }
        uint256 devReward = smartPiReward.div(20); // 5% Feed to Dev Fund
        uint256 insReward = smartPiReward.div(50); // 2% Feed to Insurance Fund
        uint256 userReward = smartPiReward.sub(devReward).sub(insReward);
        smartPi.mintTo(address(this), userReward);
        smartPi.mintTo(devAddr, devReward);
        smartPi.mintTo(insAddr, insReward);
        pool.accSmartPiPerShare = pool.accSmartPiPerShare.add(userReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to SmartPiFarm for SmartPi allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount,
        address _referrer
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accSmartPiPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accSmartPiPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw LP tokens from SmartPiFarm.
    ...
    // Safe smartPi transfer function, just in case if rounding error causes pool to not have enough smartPi.
    function safeSmartPiTransfer(address _to, uint256 _amount) internal {
        uint256 smartPiBalance = smartPi.balanceOf(address(this));
        if (_amount > smartPiBalance) {
            smartPi.transfer(_to, smartPiBalance);
        } else {
            smartPi.transfer(_to, _amount);
        }
    }
    ...
    // Update to emission rate (smartPiPerBlock)
    function updateEmissionRate(uint256 newEmissionRate) public onlyOwner {
        ...
        uint256 previousEmissionRate = smartPiPerBlock;
        smartPiPerBlock = newEmissionRate;
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            safeSmartPiTransfer(referrer, refBonusEarned);
            safeSmartPiTransfer(_user, _pending.sub(refBonusEarned));
            emit ReferralPaid(_user, referrer, refBonusEarned);
        } else {
            safeSmartPiTransfer(_user, _pending);
        }
    }
