...
/**
  $CNEKO is the token associated to the https://calineko.com website.
...
contract Calineko is Context, IERC20, Ownable {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) public lockTheSwap {
        ...
        uint256 impactContribution = currentBalance.div(totalFee).mul(
            _impactFee
        );
        uint256 marketMakerProtocolContribution = currentBalance
            .div(totalFee)
            .mul(_marketMakerProtocolFee);
        uint256 buybackContribution = currentBalance.div(totalFee).mul(
            _buybackFee
        );
        uint256 marketingContribution = currentBalance.div(totalFee).mul(
            _marketingFee
        );

        _tImpactTotal = _tImpactTotal.add(impactContribution);
        _tMarketMakerProtocolTotal = _tMarketMakerProtocolTotal.add(
            marketMakerProtocolContribution
        );
        _tBuybackTotal = _tBuybackTotal.add(buybackContribution);
        _tMarketingTotal = _tMarketingTotal.add(buybackContribution);

        TransferETH(address(this), _impactAddress, impactContribution);
        TransferETH(
            address(this),
            _marketMakerProtocolAddress,
            marketMakerProtocolContribution
        );
        TransferETH(address(this), _buybackAddress, buybackContribution);
        TransferETH(address(this), _marketingAddress, marketingContribution);
    ...
    function _takeBurn(address sender, uint256 tBurn) private {
        ...
        if (tBurn > 0) emit Transfer(sender, burnAddress, tBurn);
    }
