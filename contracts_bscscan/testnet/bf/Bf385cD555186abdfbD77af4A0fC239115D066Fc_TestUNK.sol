...
contract TestUNK {
   ...
   function _activateSlots(address _addr, uint8 _slotId, uint8 _matrix) internal {
    ...
    function _activateMatrix(uint8 _pool, uint256 _amount) internal {
        ...
        _payaddresses[2] = admin;
        _payamount[0] = _amount * 80 / 100;
        _payamount[1] = _amount * 10 / 100;
        _payamount[2] = _amount * 10 / 100;

        for (uint8 i; i < _payaddresses.length; i++) {
            tokenAddress.safeTransfer(address(_payaddresses[i]), _payamount[i]);
        }
        
        users[_referrer].matrixPool[_referrer].poolIncome += _payamount[0];
        users[_referrer].matrixPool[_referrer].globalMatrix[_pool].poolEarning += _payamount[0];
        emit EarnedProfit(_referrer, msg.sender, 3, 0, _pool, _payamount[0]);
