...
contract FixStaking is AccessControl, Pausable {
    ...
    modifier isPoolEmpty(uint256 _poolIndex) {
        require(
            database.getPoolInfo(_poolIndex).stakedAmount == 0,
    ...
    modifier isPoolExist(uint256 _poolIndex) {
        require(
            database.getPoolInfo(_poolIndex).startTime > 0,
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        database.setMinMaxStake(_minStake, _maxStake);
     ...
     */
    function setPenaltyDay(uint256 _duration)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        database.setPenaltyDay(_duration);
    ...
    {
        database.fundPool(msg.sender, _poolIndex, _fundingAmount);
    ...
    function withdrawERC20(address _token, uint256 _amount)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        database.withdrawERC20(_token, msg.sender, _amount);
    }
    ...
    function setExternalAddress(address _token)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        database.setExternalAddress(_token);
    }
        ...
        isValid(_startTime, _endTime, _apy)
    {
        ...
        uint256 poolIndex = database.getPoolsLength() - 1;
    ...
    function withdrawReserve(uint256 _poolIndex)
        ...
        isPoolEmpty(_poolIndex)
    {
        database.withdrawReserve(_poolIndex, msg.sender);
    ...
    function removePool(uint256 _poolIndex)
        ...
        isPoolEmpty(_poolIndex)
    {
        if (database.getPoolInfo(_poolIndex).reserve > 0) {
            database.withdrawReserve(_poolIndex, msg.sender);
        }

        database.removePool(_poolIndex);
    ...
    function deposit(uint256 _amount, uint256 _poolIndex)
        ...
        lock
    {
        uint256 index = database.getCurrentStakerIndex(msg.sender);

        IDatabase.PoolInfo memory pool = database.getPoolInfo(_poolIndex);
        (uint256 minStake, uint256 maxStake) = database.getMinMaxStake();
        ...
        require(
            database.getStakerAmount(msg.sender) + _amount <= maxStake,
    ...
    function withdraw(uint256 _stakerIndex) external whenNotPaused lock {
        IDatabase.StakerInfo memory staker = database.getStaker(
            msg.sender,
            _stakerIndex
        );
        IDatabase.PoolInfo memory pool = database.getPoolInfo(staker.poolIndex);
        ...
        database.withdraw(msg.sender, totalReward, totalWithdraw, _stakerIndex);
    ...
    function emergencyWithdraw(uint256 _stakerIndex) external whenPaused {
        ...
        database.transferAndRemove(msg.sender, withdrawAmount, _stakerIndex);
    }
    ...
    function claimPending(uint256 _stakerIndex) external whenNotPaused {
        ...
        require(
            block.timestamp >= staker.pendingStart + database.getPenaltyDays(),
            "claim: Please wait your time has not been up."
        );

        uint256 totalPending = staker.pendingAmount + staker.pendingReward;

        database.transferAndRemove(msg.sender, totalPending, _stakerIndex);
        ...
        returns (uint256)
    {
        ...
        uint256 endTime = block.timestamp > staker.closedTime
            ? staker.closedTime
            : block.timestamp;
        uint256 rate = database.getPoolInfo(staker.poolIndex).apy;
...
interface IDatabase {
    ...
    function withdrawReserve(uint256 _poolIndex, address _admin) external;
