...
// update 4 = FEE BUY/SELL OK OK, MAX WALLET OK OK, MAX BUY/SELL OK OK, TRNSFR KE BURN AND ANTAR WALET DARI OWNER NO FEE OK OK, LIMIT BY DAY OK OK, TRADING ON/OFF OK OK, CAN TRANSFER BEFORE TRADING ON OK OK
// TRNSFR WALLET TO WALLET KENA SELL FEE BUT NO MAX WALLET OK OK, MANUAL swap & SWEEP OK OK, LOCK WALLET CANT SELL & CANT TRANSFER BUT CAN BUY OK OK, EXCLUDE FROM MAX TX OK OK
// ONLY MIN WALLET NO
...
contract JOBXTOKEN is Context, IBEP20, Ownable {
    ...
    uint256 public _txLockTime = 1400 * 1 minutes; // 24h
    ...
    function startLockByDay() public onlyOwner() {
        lockByDay = true;
    }
    
    function stopLockByDay() public onlyOwner() {
        lockByDay = false;
    }

    function SetLockTimeMinutes(uint256 newMinutes) external onlyOwner {
        _txLockTime = newMinutes * 1 minutes;
    }
    ...
    function GetLockTimeMinutes() public view returns(uint256) {
        return _txLockTime.div(60);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludeAddressFromMaxTx[from] && !_isExcludeAddressFromMaxTx[to] && !_isExcludedMaxWallet[from] && !_isExcludedMaxWallet[to]) {
            ...
            if (!_isExcludeAddressFromMaxTx[to] && !_isExcludedMaxWallet[to] && (from == pancakeSwapV2Pair) && from != owner() && to != owner()) {
            ...
            if (!_isExcludedFromTxlock[from] && lockByDay && (to == pancakeSwapV2Pair) && (from != pancakeSwapV2Pair) && from != owner() && to != owner()) {
            ...
            if (!_isExcludeAddressFromMaxTx[from] && !_isExcludedMinWallet[from] && (to == pancakeSwapV2Pair) && from != owner() && to != owner()) {
		        require(amount <= _sellMaxTxAmount2, "Transfer amount exceeds the maxTxAmount");
            }

            if ((to == pancakeSwapV2Pair) && from != owner() && to != owner()) {
                require(balanceOf(from).add(amount) >= _minWalletAmount, "Exceeds min wallet amount");
            }
        }
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            // set FEES to buy or sell action
