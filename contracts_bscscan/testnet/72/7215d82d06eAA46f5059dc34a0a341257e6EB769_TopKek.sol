...
contract TopKek is Context, IERC20, Ownable {
    ...
    string private _name = "Top Kek";
    string private _symbol = "KEK";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 100000 * 10**9 * 10**_decimals;

    address payable public marketingAddress =
        payable(0x6bA5F92821FcB4D20dF526F3Cd5939f0FAeEce69);
    address public marketingWalletToken =
        ...
        onlyOwner
    {
        ...
        require(
            newLiquidityFee.add(_burnFee).add(_marketingFee) <= _maxFees,
            "Fees are too high."
        );
    ...
    function setMarketingFeePercent(uint256 newMarketingFee)
        external
        onlyOwner
    {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(
            _liquidityFee.add(_burnFee).add(newMarketingFee) <= _maxFees,
            "Fees are too high."
        );
    ...
    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner {
        require(
            _liquidityFee.add(newBurnFee).add(_marketingFee) <= _maxFees,
            "Fees are too high."
        );
        _burnFee = newBurnFee;
        _totalFees = _liquidityFee.add(_marketingFee).add(_burnFee);
    ...
    function setMarketingWalletToken(address _marketingWalletToken)
        external
        onlyOwner
    {
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 marketingTokensToSell = balanceOf(address(this))
            .mul(_marketingFee)
            .div(_totalFeesToContract);
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        IERC20(marketingWalletToken).transfer(
            marketingAddress,
            IERC20(marketingWalletToken).balanceOf(address(this))
        );
    }
