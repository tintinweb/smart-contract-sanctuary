...
contract battle is IERC721Receiver,Ownable{
    ...
    event SceneBattle(bool status,uint256 gold,address attack,uint256 equipmentId,uint256[] equipmentInfo);
    ...
    function scene(uint256[] memory token,uint256 sceneId) public onlyUser(token){
        if(sceneId==2){
            ...
            emit SceneBattle(false,0,msg.sender,0,equipmentInfo);
        }
        ...
        emit SceneBattle(true,gold,msg.sender,equipmentId,equipmentInfo);
    }
