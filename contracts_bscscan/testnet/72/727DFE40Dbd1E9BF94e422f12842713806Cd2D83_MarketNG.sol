...
contract MarketNG is IERC721Receiver, IERC1155Receiver, ReentrancyGuard, Ownable, Pausable {
    ...
    uint8 public constant OP_COMPLETE_SELL = 1; // complete sell (cancel KIND_AUCTION if any)
    uint8 public constant OP_BUY = 2; // create KIND_BUY
    uint8 public constant OP_ACCEPT_BUY = 3; // complete KIND_BUY
    uint8 public constant OP_CANCEL_BUY = 4; // cancel KIND_BUY
    uint8 public constant OP_REJECT_BUY = 5; // reject KIND_BUY
    uint8 public constant OP_BID = 6; // bid (create or update KIND_AUCTION)
    uint8 public constant OP_COMPLETE_AUCTION = 7; // complete auction (by anyone)
    uint8 public constant OP_ACCEPT_AUCTION = 8; // accept auction in an early stage (by seller)
    uint8 public constant OP_MAX = 9;

    uint8 public constant TOKEN_MINT = 0; // mint token
    ...
    struct Inventory {
        ...
        address buyer;
        IERC20 currency;
        uint256 price; // price / current bid price (auction)
        uint256 netPrice; // actual profit (auction)
        uint256 endTimestamp; // deadline
        uint8 kind;
        uint8 status;
    }
    ...
    struct SellInput {
        ...
        TokenPair[] bundle;
        IERC20 currency;
        uint256 price; // sell price or start price (auction)
        uint256 endTimestamp; // deadline for both direct sell and auction
        bytes32 salt;
        uint8 kind;
    }
    ...
    struct OfferInput {
        ...
        TokenPair[] bundle;
        uint256 endTimestamp; // deadline for buy offer
    }
    ...
    event EvInventoryUpdate(uint256 indexed id, bool isNew, Inventory inventory);
    ...
    function onERC721Received(
        ...
        uint256 tokenId,
        bytes calldata data
    ) external override _onTransferOnly returns (bytes4) {
        (operator);
        (from);
        (tokenId);
        (data);
    ...
    function run(
        SellInput calldata sell,
        OfferInput calldata offer,
        bytes calldata sigSell,
        bytes calldata sigOffer
    ) public payable nonReentrant whenNotPaused {
        require(offer.deadline > block.timestamp, 'deadline reached');
        require(marketSigners[offer.signer], 'unknown market signer');

        _validateOpCode(offer.opcode);
        require(
            verify(sigOffer, keccak256(abi.encode(offer)), offer.signer),
            'offer signature error'
        );

        if (!isBuyOp(offer.opcode)) {
            bytes memory encodedSell = abi.encode(sell);
            require(keccak256(encodedSell) == offer.sellHash, 'sell hash does not match');
            require(verify(sigSell, keccak256(encodedSell), sell.seller), 'seller signature error');
        }

        if (offer.opcode == OP_COMPLETE_SELL) {
            _assertSender(offer.caller);
            require(sell.kind == KIND_SELL, 'sell.kind should be KIND_SELL');
            _newDeal(
                offer.id,
                sell.seller,
                sell.bundle,
        ...
        } else if (offer.opcode == OP_ACCEPT_BUY) {
            _assertSender(offer.caller);
            _acceptBuy(offer.id, offer.caller, offer.settlement);
        } else if (offer.opcode == OP_CANCEL_BUY) {
            _cancelBuyAnyway(offer.id);
        } else if (offer.opcode == OP_REJECT_BUY) {
            _rejectBuy(offer.id);
        } else if (offer.opcode == OP_BID) {
            _assertSender(offer.caller);
            require(sell.kind == KIND_AUCTION, 'sell.kind should be KIND_AUCTION');
            _bid(
                offer.id,
                sell.seller,
                sell.bundle,
        ...
        } else if (offer.opcode == OP_COMPLETE_AUCTION) {
            _completeAuction(offer.id, offer.settlement);
        } else if (offer.opcode == OP_ACCEPT_AUCTION) {
            _assertSender(offer.caller);
            require(offer.caller == sell.seller, 'seller only');
            _acceptAuction(offer.id, offer.settlement);
        } else {
            revert('impossible');
        }
    }
    ...
    function emergencyCancelAuction(uint256 id, bool noTransfer) public onlyOwner {
        ...
        emit EvInventoryUpdate(id, false, inv);
    }
    ...
    function swap(Swap memory req, bytes memory signature) public nonReentrant whenNotPaused {
        ...
        require(verify(signature, keccak256(abi.encode(req)), req.creator), 'signature error');
    ...
    function _assertSender(address sender) internal {
        require(sender == msg.sender, 'wrong sender');
    }

    function _validateOpCode(uint8 opCode) internal {
    ...
    function _newBuy(
        ...
        require(
            ...
            'endTimestamp must be greater than block timestamp'
        );
        ...
        emit EvInventoryUpdate(id, true, inventories[id]);
    }
    ...
    function _cancelBuyAnyway(uint256 id) internal {
        ...
        emit EvInventoryUpdate(id, false, inventories[id]);
    }
    ...
    function _newDeal(
        ...
        require(endTimestamp > block.timestamp, 'sell has already ended');
    ...
    function _bid(
        ...
        require(endTimestamp > block.timestamp, 'auction ended');
        ...
        if (isAuction(id)) {
            ...
            if (block.timestamp + minAuctionDuration >= auc.endTimestamp) {
                auc.endTimestamp += minAuctionDuration;
            }
        } else {
            newAuction = true;
        ...
        emit EvInventoryUpdate(id, newAuction, inventories[id]);
    }

    function _completeAuction(uint256 id, Settlement calldata settlement) internal {
        require(isAuction(id), 'auction does not exist');
        require(inventories[id].endTimestamp < block.timestamp, 'auction still going');
    ...
    function _completeTransaction(uint256 id, Settlement calldata settlement) internal {
        ...
        uint256 tokenCount = inventoryTokenCounts[id];
        address[] memory tokens = new address[](tokenCount);
        for (uint256 i = 0; i < tokenCount; i++) {
            TokenPair storage t = inventoryTokens[id][i];
            tokens[i] = address(t.token);
        }
    ...
    function _transfer(
        ...
        require(to != address(0), 'cannot transfer to address(0)'); // keep this before amount check
        if (amount == 0) {
            return;
        }
        if (currency == weth) {
    ...
    function isAuctionOpen(uint256 id) public view returns (bool) {
        return
            ...
            inventories[id].endTimestamp > block.timestamp;
    }
    ...
    function isExpired(uint256 id) public view returns (bool) {
        return block.timestamp > inventories[id].endTimestamp;
    }
    ...
    function isBuyOp(uint8 op) public pure returns (bool) {
