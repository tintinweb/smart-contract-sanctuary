...
contract SQUEEZYSHIBA is Context, IERC20, IERC20Extented, Ownable {
    ...
    uint256 public _maxWalletAmount = 800000000 * 10**9; //230 000 000
    ...
    function getTokenPriceBNB(uint256 amount) external view returns(uint256) {
        IERC20Extented token0 = IERC20Extented(IUniswapV2Pair(uniswapV2Pair).token0());//SQUEEZYSHIBA
        ...
        if(pairSwapped) {
            token0 = IERC20Extented(IUniswapV2Pair(uniswapV2Pair).token1());//SQUEEZYSHIBA
