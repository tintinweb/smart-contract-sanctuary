 ...
 *    - Principle: 0x8d11ec38a3eb5e956b052f67da8bdc9bef8abf3e
 *    - Treasury: 0xcb54ea94191b280c296e6ff0e37c7e76ad42dc6a
 *    - DAO: 0x677d6ec74fa352d4ef9b1886f6155384acd70d90
 *    - BondCalculator: 0x0000000000000000000000000000000000000000
 *
...
contract BondStakeDepository is Ownable {
  ...
  struct Bond {
    uint256 gonsPayout; // sHEC gons remaining to be paid
    uint256 hecPayout; //hec amount at the moment of bond
  ...
  function deposit(
    ...
    uint256 stakeGons = ISTOKEN(sHEC).gonsForBalance(payout);
  ...
  function redeem(address _recipient, bool _stake) external returns (uint256) {
    ...
    uint256 _amount = ISTOKEN(sHEC).balanceForGons(info.gonsPayout);
    ...
    )
  {
    ...
    payout = ISTOKEN(sHEC).balanceForGons(info.gonsPayout);
  ...
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = IERC20(HEC).totalSupply();
    ...
    returns (uint256 pendingPayout_)
  {
    ...
    uint256 payout = ISTOKEN(sHEC).balanceForGons(
      _bondInfo[_depositor].gonsPayout
    );
