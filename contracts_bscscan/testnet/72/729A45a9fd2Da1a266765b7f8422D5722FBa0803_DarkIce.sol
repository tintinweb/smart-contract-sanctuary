...
contract DarkIce {
    ...
    constructor() {
        emit Transfer(address(0x0), address(this), (totalSupply * 75) / 100);
        balances[address(this)] = (totalSupply * 75) / 100;

        emit Transfer(address(0x0), founderAddress, (totalSupply * 25) / 100);
        balances[founderAddress] = (totalSupply * 25) / 100;
    }
    ...
    function work() public returns (bool) {
        ...
        // uint32 playerLevel = players[playerAddress].level;
        // uint32 playerEconomicLevel = players[playerAddress].economicLevel;

        if (canWork(playerAddress)) {
            // uint256 reward = 25 * (10**decimals) * playerEconomicLevel;
            uint256 reward = 25 * (10**decimals);
