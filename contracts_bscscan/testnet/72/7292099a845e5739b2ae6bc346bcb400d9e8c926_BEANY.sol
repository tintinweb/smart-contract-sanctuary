...
contract  BEANY is Context, IERC20, Ownable {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap 
    {
        ...
        uint256 tokensLiquidityShare = contractTokenBalance.sub(tokensCollectedForBuyBack);
        uint256 half = tokensLiquidityShare.div(2);
        uint256 otherHalf = tokensLiquidityShare.sub(half);
        uint256 initialBalance = address(this).balance;
        uint256 tokensToBeSwaped = tokensCollectedForBuyBack.add(half);
        swapTokensForEth(tokensToBeSwaped); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
        //how much eth is share for liquidity, the remaining will be for buyback. 
        uint256 ratio = half.div(tokensToBeSwaped);
        uint256 ethBalanceForLiquidity = newBalance.mul(ratio);
        // add liquidity to uniswap
        addLiquidity(otherHalf, ethBalanceForLiquidity);
    ...
    returns (uint256, uint256)
    {
        ...
        uint256 tcharity = tAmount.div(1000).mul(_charityFee);
