...
contract DAI_DISTRIBUTOR is IDividendDistributor { // Change contract name
    ...
    IBEP20 DAI = IBEP20(0x8a9424745056Eb399FD19a0EC26A14316684e274); // Reward address
    address WBNB = 0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F; // WBNB address
...
contract TESTTOKEN is IBEP20, Auth { // Change contract name
    using SafeMath for uint256;

    address public DAI = 0x8a9424745056Eb399FD19a0EC26A14316684e274; //Reward address
    address WBNB = 0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F;
    address zeroAddress = 0x0000000000000000000000000000000000000000;
    address public burnAddress = 0x000000000000000000000000000000000000dEaD;
    address public externalAddress = 0x4950348a16868CD30687Fa1bd08f65629c117557; //Change external address
    address public autoLiquidityReceiver = 0x41712b1167DB4940A04522005e29c0FE9c7F8A86; //Change liquidty owner address
	...
	*/
    ...
    /**  
     * @dev Remove/add an address from DAI reward
     */
    ...
    /**  
     * @dev set DAI distribution criteria
     * _minPeriod--> minimum time to wait before send
     * _minDistribution--> minimum amount of DAI reward before send to address
     */
    function setDAIDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external authorized {
        distributor.setDistributionCriteria(_minPeriod, _minDistribution);
    }

    /**  
     * @dev set DAI distributor's setting
     * gas--> gas fee to be used from sender's gas fee to send DAI reward
     */
    function setDAIDistributorSettings(uint256 gas) external authorized {
        require(gas < 750000);
        distributorGas = gas;
    }
	...
	*/
    ...
    function blacklistSingleWallet(address account) external authorized {
        ...
        setIsDividendExempt(account, true); // also remove DAI reward for address
    }
    ...
    function blacklistMultipleWallets(address[] calldata accounts) external authorized {
        ...
        for (uint256 i; i < accounts.length; ++i) {
            ...
            setIsDividendExempt(accounts[i], true); // also remove DAI reward for address
        }
    }
    ...
    function unBlacklistSingleWallet(address account) external authorized {
        ...
        setIsDividendExempt(account, false); // also includes in DAI reward for address
    }
    ...
    function unBlacklistMultipleWallets(address[] calldata accounts) external authorized {
        ...
        for (uint256 i; i < accounts.length; ++i) {
            ...
            setIsDividendExempt(accounts[i], false); // also includes in DAI reward for address
        }
    }
