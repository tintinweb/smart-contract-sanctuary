...
contract Calcifire is Context, IERC20, Ownable {
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        _reflectOwned[account] = ownedAmount.mul(currentRate);
   ...
   function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
    ...
    function calcReflectionTotalSupply(uint256 totalSupply) public view returns (uint256) {
        return (MAX - (MAX % totalSupply));
    }

    function calcReflectionRate() public view returns (uint256) {
        return _getRate();
    }
