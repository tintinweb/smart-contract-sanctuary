...
mapping (address => User) internal users;
    ...
    uint256 public _liquidityFee = 0;
     uint256 public _taxFeeBuy = 12;
       ...
       uint256 public _liquidityBuy = 6;
       uint256 public _liquiditySell = 2;
 ...
 uint256 public _buyMarketing=2;
 uint256 public _burnFee = 0;
uint256 public _sellBurnFee =0;
 uint256 public _buyHolderShare=7;
  uint256 public _sellMarketing=1;
  uint256 minShareHolderBal=0;
    mapping (address => AddressFee) public _addressFees;
	uint256 constant public SELL_LIMIT = 15 minutes;
     ...
     function _transfer(
        ...
        if (to == uniswapV2Pair && balanceOf(uniswapV2Pair) > 0) {
			...
			if(amount<=balanceOf(from).mul(50).div(100)){
			 ...
			 if(balanceOf(from).sub(amount)<10000){
			     delete index[from];
			     totalParticipant--;
			 }
			user.checkpoint=block.timestamp;
             ...
             if(balanceOf(to).add(amount)>=10000){
            if (index[to] == 0) {
             participant.push(to) ; 
             totalParticipant+= 1;
              index[to] += 1;
