...
contract BetContract{
    ...
    constructor()public{
        Admin =  0xa48f0D9d0cB30eb087C6589dc804CfEe7f1ed8eD;
    ...
    modifier BetOpen(){
        require(BetAvaliable,"You cannot bet Until admin Opens Again");
        _;
    }
    function BETTOKENS(bool Up , uint256 amountoftokens)public{
    ...
    function Start(address pfeed) public isAdmin {
        priceFeed = AggregatorV3Interface(pfeed);
        BEtERCount = 0;
        StartBetPrice = getLatestPrice();
        EndBetprice = getLatestPrice();
        StartBettime = block.timestamp;
        EndBettime = block.timestamp + 5 minutes;
        ToatalBetAmont = 0;
        WinersWeightCount = 0;
    }
    function Stop() public{
        require(block.timestamp > StartBettime+5 minutes,"Cant stop before Default time");
        EndBetprice = getLatestPrice();
