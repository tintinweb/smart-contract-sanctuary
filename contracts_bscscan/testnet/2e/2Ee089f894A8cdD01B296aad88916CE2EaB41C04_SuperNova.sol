...
contract SuperNova is OwnableUpgradeable {
    ...
    modifier refundGas() {
        ...
        (bool sent, ) = payable(address(msg.sender)).call{value: gasSpent}("");
    ...
    function initialize(
        ...
        minumumBNBBalance = 10**16;
        minumumNexusBalance = 1 * 10**2 * 10**18;
        maxBNBTransaction = 5 * 10**18;
        maxNexusTransaction = 100 * 10**3 * 10**18;
    ...
    function nexusAddr() external view returns (address) {
        return _nexusAddr;
    }
    ...
    function nexusLP() external view returns (address) {
        return _nexusLP;
    }
        ...
        onlyOwner
    {
        ...
        uint256 recoverAmount = _tokenAmount;
        if (_tokenAmount == 0) {
            recoverAmount = tokenBalance;
        }
        if (_tokenAddress == _nexusAddr) {
            require(tokenBalance > minumumNexusBalance, "Nexus is too low");
            if (tokenBalance - recoverAmount <= minumumNexusBalance) {
                recoverAmount = tokenBalance.sub(minumumNexusBalance);
            }
        }

        IERC20(_tokenAddress).transfer(address(msg.sender), recoverAmount);
    }

    function withdraw(uint256 amount) external onlyOwner {
        require(address(this).balance > minumumBNBBalance, "BNB is too low");
        uint256 transferAmount = amount;
        if (address(this).balance - amount <= minumumBNBBalance) {
            transferAmount = address(this).balance.sub(minumumBNBBalance);
        }
    ...
    function liquidate(uint256 amount) private {
        ...
        // Uniswap pair path for BNB -> NEXUS
        IERC20(_nexusAddr).approve(address(uniswapV2Router), amount);
        address[] memory path = new address[](2);
        path[0] = _nexusAddr;
    ...
    function triggerLiquidate(uint256 amount) external isAutomating onlyOracle refundGas{
        require(amount <= maxNexusTransaction, "Transaction too big");

        uint256 tokenBalance = IERC20(_nexusAddr).balanceOf(address(this));
        uint256 recoverAmount = amount;
        if (amount == 0) {
            recoverAmount = tokenBalance;
        }
        ...
        liquidate(recoverAmount);

    }

    function triggerBuyBack(uint256 amount) external isAutomating onlyOracle refundGas{
        ...
        buyBack(transferAmount);
    }

    function setMinimunNexus(uint256 _newMinimun) external onlyOwner {
        minumumNexusBalance = _newMinimun;
    }

    function setMaxNexusTransaction(uint256 _newMax) external onlyOwner {
        maxNexusTransaction = _newMax;
    }

    function setMinimunBNB(uint256 _newMinimun) external onlyOwner {
        minumumBNBBalance = _newMinimun;
    }

    function setMaxBNBTransaction(uint256 _newMax) external onlyOwner {
        maxBNBTransaction = _newMax;
    }
}