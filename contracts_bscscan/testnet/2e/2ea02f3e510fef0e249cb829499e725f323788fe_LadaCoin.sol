...
contract MineableToken is StandardToken, Ownable {
  ...
  uint8 public decimals;
    ...
    uint sat = 1e8;
    uint satm = 1e9; //_balance * 10 for mining
  ...
  function AddMine() canMine public {
    ...
    uint256 rewardInt = uint256(_reward) + (_reward)*satm;
...
*/
  ...
  function balanceOf(address _owner) public view returns (uint256 balance) {
    ...
    return balances[_owner] + (claimed[_owner] ? 0 : balanceCla(_owner)) + (claimined[_owner] ? 0 : balanceMine(_owner)) + (claimstaked[_owner] ? 0 : miningStake(reward)) ;
  }
