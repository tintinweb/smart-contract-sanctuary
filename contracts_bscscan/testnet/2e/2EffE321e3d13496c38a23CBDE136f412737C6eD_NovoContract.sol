...
contract NovoContract is Context, IBEP20, Ownable {
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    require(_balances[recipient].add(amount) < _totalSupply.div(200), "NONO: recipient can't have more than 0.5% of total supply");
    ...
    require(_balances[sender] >= amount.add(fee), "NOVO: fee amount exceeds balance");
    ...
    for (i = 0; i < _totalHolders; i ++) {
      ...
      bs_ts_th_lh = bs_ts.add(th_lh).div(uint(10) ** 7).div(2); // 50 < A < 100

      sum = sum.add(bs_ts.mul(uint(2) ** bs_ts_th_lh));
    }
    ...
    for (i = 0; i < _totalHolders; i ++) {
      ...
      if (i == _totalHolders - 1) {
        _balances[holderAddress] = _balances[holderAddress].add(fee.sub(sumFee));
        break;
      }
      ...
      uint256 feeAmount = bs_ts.mul(uint(2) ** bs_ts_th_lh).mul(fee).div(sum);
