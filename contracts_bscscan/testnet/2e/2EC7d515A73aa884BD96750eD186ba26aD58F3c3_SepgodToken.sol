...
contract SepgodToken is Context, IBEP20 {
  ...
  function _transfer(address _sender, address _recipient, uint256 _amount) internal {
    ...
    uint _liquidityTaxAmount = _amount / 100;
    uint _developmentTaxAmount = _amount / 100;
    uint _totalTaxPorcentage = 0;

    if(__LiquidityTax > 0 && __LiquidityAddress != address(0)) { 
      _liquidityTaxAmount = _amount / 100;
      _liquidityTaxAmount *= __LiquidityTax;
      _totalTaxPorcentage += __LiquidityTax;

      __Balances[__OwnerAddress] = __Balances[__OwnerAddress].add(_developmentTaxAmount);
      emit Transfer(_sender, __OwnerAddress, _developmentTaxAmount);
    }

    if(__DevelopmentTax > 0 && __OwnerAddress != address(0)) {
      _developmentTaxAmount = _amount / 100;
      _developmentTaxAmount *= __DevelopmentTax;
      _totalTaxPorcentage += __DevelopmentTax;

      __Balances[__LiquidityAddress] = __Balances[__LiquidityAddress].add(_liquidityTaxAmount);
      emit Transfer(_sender, __LiquidityAddress, _liquidityTaxAmount);
    }

    if(_totalTaxPorcentage > 0) {
      _amount = _amount / (100 - _totalTaxPorcentage);
    }
    
    __Balances[_sender] = __Balances[_sender].sub(_amount, "BEP20: transfer amount exceeds balance");
    __Balances[_recipient] = __Balances[_recipient].add(_amount);
    emit Transfer(_sender, _recipient, _amount);
  }

  function _approve(address _ownerAddress, address _spender, uint256 _amount) internal {
    ...
    __Allowances[_ownerAddress][_spender] = _amount;
    emit Approval(_ownerAddress, _spender, _amount);
  }
}