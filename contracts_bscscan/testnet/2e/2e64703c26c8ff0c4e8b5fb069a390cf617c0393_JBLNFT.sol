...
contract JBLNFT is ERC721{
    ...
    uint256 public PRICE = 70; //單位70個JBL-Token

    uint256 [] private w = [80,65,65,80,70,80,85,60,65,85,70,70,65,60,60,70,60,85,80,85];//新價格
    uint256 [] private q = [4,2,1,4,1,1,0,2,3,3,4,0,2,3,3,4,3,4,4,2];
    uint256 [] private _level = [0,1,2,3,5];//'外野手','內野手','捕手','投手','打擊手'
    uint256 [] private NFT_APY = [11*1*10**17,11*1*10**17,105*1*10**16,105*1*10**16,1*10**18];//'外野手','內野手','捕手','投手','打擊手'

    Card[] public cards; // First Item has Index 0
    address public owner;
    mapping (uint256 => string) private _tokenURIs;
    uint8 public maxSupply = 0;
    ...
    constructor ()  public ERC721("JBL-BLINDBOX", "JBL-BLINDBOX"){
        owner = msg.sender; 
    ...
    function mintCard(address account,uint8 _num) public returns (bool) {
        uint256 cardId = cards.length;
        uint256 JBL_B = token.balanceOf(msg.sender);
        uint256 allowance = token.allowance(msg.sender, address(this));
        require(_num > 0,"Please enter number.");
        require(allowance >= _num*PRICE*1*10**18, "Check the JBL token allowance");
        require(JBL_B >= _num*PRICE*1*10**18,"Check your JBL token balance");
        require(isActive());

        
        string memory name = "JBL-BLINDBOX";

        for(uint i=cardId;i<(cardId+_num);i++)
        {
            uint256 nft_type = _get_level(i);
            uint256 nft_apy = _get_apy(i);
            uint256 new_price = _get_new_price(i);//新價格
            string memory tokenURI = _get_img(nft_type);

            cards.push(Card(name, nft_type, tokenURI, nft_apy, new_price));// 寫入新價格
            _mint(account, i); 
            _setTokenURI(i, tokenURI);
        }
        token.transferFrom(msg.sender, address(this), _num*PRICE*1*10**18);
    ...
    function _get_img(uint256 x) public view returns (string memory){
        uint256 z = now % 2;
        ...
        if(x==0)
        {
            if(z==1)
            {
                img_url = "https://farms.jblgamefi.io/nft/blindbox/level1/of1.png";
            }
            else
            {
                img_url = "https://farms.jblgamefi.io/nft/blindbox/level1/of2.png";
            }
        }
        else if(x==1)
        {
            if(z==1)
            {
                img_url = "https://farms.jblgamefi.io/nft/blindbox/level1/if1.png";
            }
            else
            {
                img_url = "https://farms.jblgamefi.io/nft/blindbox/level1/if2.png";
            }
        }
        else if(x==2)
        {
            if(z==1)
            {
                img_url = "https://farms.jblgamefi.io/nft/blindbox/level1/c1.png";
            }
            else
            {
                img_url = "https://farms.jblgamefi.io/nft/blindbox/level1/c2.png";
            }
        }
        else if(x==3)
        {
            if(z==1)
            {
                img_url = "https://farms.jblgamefi.io/nft/blindbox/level1/p1.png";
            }
            else
            {
                img_url = "https://farms.jblgamefi.io/nft/blindbox/level1/p2.png";
            }
        }
        else if(x==5)
        {
            if(z==1)
            {
                img_url = "https://farms.jblgamefi.io/nft/blindbox/level1/h1.png";
            }
            else
            {
                img_url = "https://farms.jblgamefi.io/nft/blindbox/level1/h2.png";
            }
        }
        return img_url;
    }

    function _get_apy(uint256 x) public view returns (uint256){
        uint z = ( x*x + now ) % 20;
        uint a = q[z];
        return NFT_APY[a];
    }

    function _get_level(uint256 x) public view returns (uint256){
        uint z = ( x*x + now ) % 20;
        uint a = q[z];
        return _level[a];
    }
    ...
    // 設定APY加乘 (需乘 10^18)
    function set_APY(uint256 _k,uint256 _apy) public onlyOwner {
        emit _chg_apy(NFT_APY[_k], _apy, now);
        NFT_APY[_k] = _apy;
    }
    ...
    function _get_new_price(uint256 x) public view returns (uint256){
        uint z = ( x*x + now ) % 20;
        return w[z];
    }
}