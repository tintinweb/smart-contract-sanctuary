...
contract Lottery is ERC721Enumerable, Ownable {
    ...
    function buyTicket() external returns(uint256){
        require(BZAI.transferFrom(msg.sender, gameAddresses.getPaymentsAddress(), lotteryPrice));
        IPayments(gameAddresses.getPaymentsAddress()).distributeFees(lotteryPrice);
    ...
    function useTicket(uint256 _ticketID,string memory _name,string[] calldata paths, bytes[] calldata signatures) external returns(uint256) {
        require(ownerOf(_ticketID) == msg.sender,"Not your ticket");
        _burn(_ticketID);
        return(_lotteryMint(_name,paths,signatures));
    }

    function buyRandomMonster(string memory _name, string[] calldata paths, bytes[] calldata signatures) external returns(uint256){
        require(BZAI.transferFrom(msg.sender, gameAddresses.getPaymentsAddress(), lotteryPrice));
        IPayments(gameAddresses.getPaymentsAddress()).distributeFees(lotteryPrice);
        return(_lotteryMint(_name, paths, signatures));
    }

    function _lotteryMint (string memory _name, string[] calldata paths, bytes[] calldata signatures)
        internal
        returns (uint256)
    {
        uint256 _index = nonce.mod(4);
        bytes32 _id = keccak256(abi.encodePacked(msg.sender, paths[_index], nonce));
        nonce = nonce.add(1);
        ...
        return _generateRandomMonster(paths[_state], _name, msg.sender, _state, signatures[_state]);
    }
    ...
    function _generateRandomMonster(
        ...
        uint256 monsterId = IBandZaiNFT(gameAddresses.getMonsterAddress()).mintMonster(
            _user,
            _name,
            _state,
            _path,
