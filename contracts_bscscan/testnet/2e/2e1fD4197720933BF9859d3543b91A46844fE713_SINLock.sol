...
// SIN Lock
contract SINLock is AccessProtected, ReentrancyGuard {
    using SafeERC20 for IERC20;
    IERC20 private _sinToken;
    event TokenLocked(address _userAddress,uint256 amount);

    constructor(IERC20 sinToken) {
        _sinToken = sinToken;
    }

    function lockTokens(uint256 amount) external nonReentrant returns (bool _isSuccess) {
        require(
            _sinToken.allowance(msg.sender, address(this)) == amount,
            "Tokens to be locked should equal the allowance given by the user"
        );
        _sinToken.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function releaseTokensToBSC(address userAddress, uint256 amount) external onlyAdmin returns (bool _isSuccess) {
        ...
        _sinToken.transfer(userAddress, amount);
        return true;
    }
