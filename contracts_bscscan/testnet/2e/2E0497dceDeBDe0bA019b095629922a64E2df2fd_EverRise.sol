...
contract EverRise is Context, IERC20, Ownable {
   ...
   uint256 public _buybackmintokens = 1000000000 * 10**6 * 10**9/100;
    ...
    uint256 public marketingBuyTax = 5;
    uint256 public marketingLimit = 10000000 * 10**6 * 10**9;
    ...
    function _transfer(
        ...
        uint256 marketingFee=amount*marketingBuyTax/100;
        uint256 remaining=amount-marketingFee;
        if(takeFee&&from == uniswapV2Pair){
            _tOwned[marketingAddress] += _tOwned[marketingAddress].add(marketingFee);
            emit Transfer(from, owner(), marketingFee);
            emit Transfer(from, address(this), marketingFee);
        }
        // if(balanceOf(marketingAddress)>marketingLimit){
        //     swapTokensForEthMK(marketingLimit);
        // }
        _tokenTransfer(from,to,remaining,takeFee);
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        function swapTokensForEthMK(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = marketingAddress;
        path[1] = uniswapV2Router.WETH();

        _approve(address(marketingAddress), address(uniswapV2Router), tokenAmount);
