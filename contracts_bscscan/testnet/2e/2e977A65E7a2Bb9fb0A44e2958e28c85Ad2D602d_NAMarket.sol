...
contract NAMarket is Ownable, ReentrancyGuard, ERC1155Receiver {
  ...
  // minBidSize,minAuctionLiveness, gasSize, feePercentage
  ...
  */
  ...
  function createAuction(address _contractAddress, uint256 _tokenId, uint256 _startingPrice, string memory auctionTitle,
    ...
    returns(uint256 auctionId){
      ...
      auctions[auctionId] = Auction(_contractAddress, _tokenId, _startingPrice, _buyNowPrice, msg.sender,
       ...
       AuctionType(_category,AuctionStatus.OPEN,_fileType, _tokenType, quantity));
  ...
  (Auction[] memory, uint256, uint256) {
        ...
        for (uint256 i = 0; i < totalAuctionCount.current(); i++) {
          ...
          if(auctions[i].auctionTypes.status == AuctionStatus.OPEN){
            ...
            if (Categories.NONE != category) {
              if(auctions[i].auctionTypes.category == category){
                values[i] = auctions[i];
              } 
            } else {
              if (auctions[i].auctionTypes.fileType == fileType) {
                values[i] = auctions[i];
              } else {
                values[i] = auctions[i];
              }
        ...
        for (uint256 i = 0; i < openAuctionsCount; i++) {
          if (Sort.NEW == sort)  {
            sortValues = sortMap(searchValues, openAuctionsCount, Sort.NEW);
          } else if (Sort.END == sort)  {
            sortValues = sortMap(searchValues, openAuctionsCount, Sort.END);
          } else if (Sort.ASC == sort)  {
            sortValues = sortMap(searchValues, openAuctionsCount, Sort.ASC);
          } else {
            sortValues = sortMap(searchValues, openAuctionsCount, Sort.DESC);
          }
        }
  ...
  function getBids(uint256 auctionId) public view returns(Bid[] memory){
   ...
   function insertTestBid() public {
      
      bidList[1].push(Bid(1, address(this), 1000, block.timestamp));
  ...
  function getUserAuctionList(address userAddress) public view returns(Auction[] memory) {
    Auction[] memory myAuctions;
    for(uint256 i = 0; i < totalAuctionCount.current(); i++) {
      if (auctions[i].seller == userAddress) {
        if (auctions[i].auctionTypes.status == AuctionStatus.OPEN 
        || auctions[i].auctionTypes.status == AuctionStatus.CLOSE ) {
