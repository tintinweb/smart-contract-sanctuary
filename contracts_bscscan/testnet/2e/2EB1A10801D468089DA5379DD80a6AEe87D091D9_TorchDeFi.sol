...
contract LpVault{
    ...
    uint public init_time;
    uint public init_time_lp;
    
    bool public initiated;
    bool public initiated_lp;
    ...
    event Unstake(address indexed _user, uint _poolId, uint indexed _amount);
...
// -------------------------------------------Super methods-----------------------------------------------------
    ...
    function changeUserInfo(address user, uint poolId, uint newTimestamp) external onlyOwner{
        require(poolId>=1 && poolId <=5, "wrong pool id");
        userStake[user].timestamps[poolId-1] = newTimestamp;
    }
...
contract TorchDeFi is LpVault{
    ...
    uint[3] public actTokenCond = [10 * 1 ether, 30 * 1 ether, 60 * 1 ether]; // act tokens required to claim dynamic rewards
    ...
    uint public miningEpoch=init_time_lp + DURATION;
    ...
    mapping(address=>mapping(uint=>bool)) public userReg; // user address => (day index => true or false), if registered for node pool
    ...
    event Activate(address indexed _user, uint _amount);
    ...
    function regClaim() external notBlack{
        ...
        uint node = dailyOutput * nodePortion.div(500); // 40% divided by 5 level 
        uint temp_amount;
        
        // iterate over levels
        for(uint i=1; i<=lev; i++){
            NodePool_reg[dayIndex][i] += 1;
            userReg[_msgSender()][dayIndex] = true; // record registeration
            
            if(userReg[_msgSender()][dayIndex-1])
            {
    ...
    function claimStatic() public updateReward(msg.sender) notBlack{
        ...
        uint staticReward = _amount * staticPortion / 100; // 40% of mining rewards as static
        ...
        for(uint i=0;i<3;i++){
            ...
            if(temp_user == address(this)){break;}
                ...
                if(userActLev >= i+1){userInfo[temp_user].dynamicToClaim_d += _amount * conditionalDynamic[i] / 100;}
                temp_user = userAccount[temp_user].invitor;
            }
        }
    ...
    function claimDynamic() external updateReward(_msgSender()){
        ...
        IERC20(Torch).transfer(_msgSender(), temp_amount1+temp_amount2);
        
        emit ClaimDynamic(_msgSender(), temp_amount1+temp_amount2);
    }
    ...
    function getLevel(address _user)public view returns(uint l){
        if(userInfo[_user].GroupLPinPool >= levelLP_group[0] * 1e18  && userStake[_user].stake_total >= level_indiv[0] * 1e18)
        {return l=5;}
        else if(userInfo[_user].GroupLPinPool >= levelLP_group[1] * 1e18 &&  userStake[_user].stake_total >= level_indiv[1] * 1e18)
        {return l=4;}
        else if(userInfo[_user].GroupLPinPool >= levelLP_group[2] * 1e18 && userStake[_user].stake_total >= level_indiv[2] *1e18)
        {return l=3;}
        else if(userInfo[_user].GroupLPinPool >= levelLP_group[3]* 1e18 && userStake[_user].stake_total >= level_indiv[3] * 1e18)
        {return l=2;}
        else if(userInfo[_user].GroupLPinPool >= levelLP_group[4] * 1e18 && userStake[_user].stake_total >= level_indiv[4] * 1e18)
        {return l=1;}
        else{return l=0;}
    }
...
//------------------------------------------AirDrop--------------------------------------------------
    ...
    */
    function registerAirdrop() external{
        ...
        require(10000 * userStakeAirdrop[_msgSender()] / userStake[_msgSender()].stake_total >= dropInfo[airdropTimes].req, "airdrop stake requirement");
    ...
    */
    function claimAirdrop() external{
        ...
        userAirdrop[_msgSender()][airdropTimes].claimedAmount = shares*70/100;
        IERC20(dropInfo[airdropTimes].BEP).transfer(_msgSender(),shares*70/100);
        ...
        for(uint i=0;i<3;i++){
            ...
            if(temp_user == address(this)){break;}
                ...
                if(userActLev >= i+1){userAirdrop[temp_user][airdropTimes].rewards_d += shares * DirectAirdropDynamic[i] / 100;}
        ...
        uint node = total * nodePortion_air.div(500); // 20% divided by 5 level, each 4% 
        ...
        emit AirdropClaim(_msgSender(), airdropTimes, shares*70/100);
