...
contract Lottery is Ownable {
    ...
    struct seri {
        ...
        mapping(uint => uint) seriAssetRemain; // seri => asset index => remain
        mapping(uint => uint) winAmount;
    }
    struct ticket {
        string[] number;
        uint[] buyTicket;
   ...
   function getMessageHash(uint timestamp, string memory result) public pure returns (bytes32) {
    ...
    function getbytesDataSetWinners(uint timestamp, uint _seri, address[] memory _winners, uint[][] memory _buyTickets, string[] memory _assets) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(timestamp, abi.encode(_seri, _winners, _buyTickets, _assets)));
    }
    function permitSetWinners(uint timestamp, uint _seri, address[] memory _winners, uint[][] memory _buyTickets, string[] memory _assets, uint8 v, bytes32 r, bytes32 s) public view returns (bool) {
        bytes32 _messageHash = getbytesDataSetWinners(timestamp, _seri, _winners, _buyTickets, _assets);
        return ecrecover(getEthSignedMessageHash(_messageHash), v, r, s) == signer;
    }
    constructor () public {
        assets['BNB'] = asset('BNB', address(0), AggregatorV3Interface(0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526), false);
        assets['BUSD'] = asset('BUSD', 0x10297304eEA4223E870069325A2EEA7ca4Cd58b4, AggregatorV3Interface(0x9331b55D9830EF609A2aBCfAc0FBCE050A52fdEa), false);
        assets['BTCB'] = asset('BTCB', 0xf6f3F4f5d68Ddb61135fbbde56f404Ebd4b984Ee, AggregatorV3Interface(0x5741306c21795FdCBb9b265Ea0255F499DFe515C), false);
        assets['USDT'] = asset('USDT', 0x013345B20fe7Cf68184005464FBF204D9aB88227, AggregatorV3Interface(0xEca2605f0BCF2BA5966372C99837b1F182d3D620), false);
        assets['ETH'] = asset('ETH', 0x979Db64D8cD5Fed9f1B62558547316aFEdcf4dBA, AggregatorV3Interface(0x143db3CEEfbdfe5631aDD3E50f7614B6ba708BA7), false);
        assets['USDC'] = asset('USDC', 0xF53E2228ff7F680D4677878eeA2c7814a5233C85, AggregatorV3Interface(0x90c069C4538adAc136E051052E14c1cD799C41B7), false);
        assets['XRP'] = asset('XRP', 0xd2926D1f868Ba1E81325f0206A4449Da3fD8FB62, AggregatorV3Interface(0x4046332373C24Aed1dC8bAd489A04E187833B28d), false);
    }
    ...
    function getUserTickets(uint _seri, address _user) public view returns(string[] memory) {
        return series[_seri].userTickets[_user];
    }
    function seriAssetRemain(uint _seri, uint _asset) public view returns(uint) {
    ...
    function takeAsset2CarryOver(uint _seri) internal {
        for(uint i = 0; i < series[_seri].assetIndex.length; i++) {
            if(series[_seri].seriAssetRemain[series[_seri].assetIndex[i]] > 0) {
                uint takeAmount = series[_seri].seriAssetRemain[series[_seri].assetIndex[i]];
    ...
    function sendNFT(uint _seri, uint startTime, address[] memory _winners, uint[][] memory _buyTickets, string[] memory _assets) internal {
    ...
    function setWinners(uint _seri, uint startTime, address[] memory _winners, uint[][] memory _buyTickets, string[] memory _assets, uint timestamp, uint8 v, bytes32 r, bytes32 s) public onlyOwner {
        require(_winners.length <= MAX_LOOP, 'Over max loop');
        require(currentSignTime < timestamp, 'Invalid timestamp');
        require(permitSetWinners(timestamp, _seri, _winners, _buyTickets, _assets, v, r, s), 'Invalid signal');
    ...
    function takeAsset(uint _seri, uint _winTickets, uint _buyTickets) internal {
        for(uint i = 0; i < series[_seri].assetIndex.length; i++) {
            if(series[_seri].seriAssetRemain[series[_seri].assetIndex[i]] > 0) {
                uint takeAmount = series[_seri].winAmount[series[_seri].assetIndex[i]];
        ...
        if(series[_seri].seriType == 2) {
            uint takeAssetInitAmount = series[_seri].winInitPrize;
    ...
    function buyCarryOver(uint _seri, string memory _numberInfo, uint _assetIndex, uint totalTicket) public payable{
    ...
    function buy(uint _seri, string memory _numberInfo, uint _assetIndex, uint totalTicket) public payable{
        uint assetPerTicket = ticket2Asset(_seri, priceFeeds[_assetIndex]);
        series[_seri].userTickets[msg.sender].push(_numberInfo);
        require(series[_seri].soldTicket + totalTicket <= series[_seri].max2sale, 'over max2sale');
        uint assetAmount = assetPerTicket.mul(totalTicket);
        ...
        if(_assetIndex == 0) {
            ...
            IBEP20 _asset = IBEP20(assets[_symbol].asset);
            require(_asset.transferFrom(msg.sender, address(this), assetAmount), 'insufficient-allowance');
            if(assets[_symbol].isBuy) require(_asset.transfer(stake, shareStakeAmount), 'insufficient-allowance');
            else {
    ...
    function setAssets(AggregatorV3Interface[] memory _priceFeeds, string[] memory _symbols, address[] memory _bep20s) public onlyOwner {
        ...
        for(uint i = 0; i < _symbols.length; i++) {
            assets[_symbols[i]] = asset(_symbols[i], _bep20s[i], _priceFeeds[i], false);
        }
        priceFeeds = _symbols;
    }
