        ...
        ERC20("zz", "xx") 
{
    ...
    // LFW out
    uint256 lfwOut = 500000000000000000000;

    // BUSD out
    uint256 busdIn = 125000000000000000000;

    // Number of deposit chest
    uint256 depositChestMax = 10;

    // Deposit
    uint256 depositMax = 4;
    ...
    // Whether a limit is set for the chest
    bool public hasPoolChestLimit;
    ...
    // The pool cap for chest (0 if none)
    uint256 public poolCapChest;
    ...
    struct UserInfo {
        ...
        uint256[] lastStakingBlockLFW;
        uint256[] rewardStakingBlockLFW;
        uint256[] lastStakingBlockNFT;
        uint256[] rewardStakingBlockNFT;
        uint256 amountChest;
    ...
    function initialize(
        ...
        if (_poolCapChest > 0) {
            hasPoolChestLimit = true;
            poolCapChest = _poolCapChest;
        }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        require(_amount == busdIn, "ERROR: 125 BUSD one deposit time");
        UserInfo storage user = userInfo[msg.sender];
        user.noOfDepositBUSD = user.noOfDepositBUSD + 1;
        require(user.noOfDepositBUSD <= depositMax, "Exceed the deposit time");
        user.lastStakingBlockLFW.push(block.number);
        user.rewardStakingBlockLFW.push(block.number + (stakingEndBlock - stakingBlock));
        ...
        user.noOfcanClaimLFW = user.noOfcanClaimLFW + 1;
        numberSlot = numberSlot + 1;
    ...
    /*
     * @notice Deposit 10 chest to qualify claiming reward
     * @param _tokenID: tokenID of chest (have to be 1001100249)
     * @param amount: amount of chest to be deposited (have to be 10)
     */
    function depositChest(
        ...
        require(amount[0] == depositChestMax, "Error: 10 box per 1 deposit time");
      //  address seller = msg.sender;
        UserInfo storage user = userInfo[msg.sender];
        user.noOfDepositChest = user.noOfDepositChest + 1;
        require(user.noOfDepositChest <= depositMax, "User exceed number of deposit chest");
        user.lastStakingBlockNFT.push(block.number);
        user.rewardStakingBlockNFT.push(block.number + (stakingEndBlock - stakingBlock));
        require(stakingBlock <= block.number, "Staking has not started");
        require(stakingEndBlock >= block.number, "Staking has ended");
//        require(user.noOfDepositNFT <= 4, "User exceed number of deposit");
        ...
        if (hasPoolChestLimit) {
            uint256 chestSupply = nftContract.balanceOf(address(this), tokenId[0]);
            require(
                chestSupply <= poolCapChest,
                "Pool cap reached"
            );
        }
        ...
        user.noOfcanClaimNFT = user.noOfcanClaimNFT + 1;
    }
    ...
    function withdraw() external nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        //require(user.lastStakingBlock[0])
        require(user.noOfcanClaimLFW > 0, "User does not stake anything");
        require(user.noOfcanClaimNFT > 0, "User does not stake anything");
        require(user.noOfcanClaimLFW == user.noOfcanClaimNFT, "User need to stake chest");
        require(user.noOfClaimedLFW < user.noOfcanClaimLFW, "You do not meet the claim requirement");
        // require(user.rewardStakingBlockLFW[user.noOfClaimedLFW] <= block.number, "Unstaking for user not started");
        // require(user.rewardStakingBlockNFT[user.noOfClaimedLFW] <= block.number, "Unstaking for user not started");
        require(user.rewardStakingBlockLFW[user.noOfClaimedLFW] >= block.number, "Unstaking for user not started");
        require(user.rewardStakingBlockNFT[user.noOfClaimedLFW] >= block.number, "Unstaking for user not started");
        _updatePool();

        uint256 lfwAmount = lfwOut;
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            ERC20(rewardTokens[i]).transfer(address(msg.sender), lfwAmount);
        }
        user.noOfClaimedLFW = user.noOfClaimedLFW + 1;
  ...
  //      require(block.number < unStakingBlock, "Unstaking has started");
    ...
    function claimNFT(address nftAddress) external validNFTContractAddress(nftAddress) {
        ...
        require(user.noOfcanClaimLFW > 0, "User does not stake anything");
        require(user.noOfcanClaimNFT > 0, "User does not stake anything");
        require(user.noOfcanClaimNFT == user.noOfcanClaimLFW, "User need to stake BUSD");
        require(user.noOfClaimedNFT < user.noOfcanClaimNFT, "Run out of turn to withdraw");
        // require(user.rewardStakingBlockLFW[user.noOfClaimedNFT] <= block.number, "Unstaking for user not started");
        // require(user.rewardStakingBlockNFT[user.noOfClaimedNFT] <= block.number, "Unstaking for user not started");
        require(user.rewardStakingBlockLFW[user.noOfClaimedNFT] >= block.number, "Unstaking for user not started");
        require(user.rewardStakingBlockNFT[user.noOfClaimedNFT] >= block.number, "Unstaking for user not started");

        IERC721 nft = getNftContract(nftAddress);
        uint256 randomSlot = random(0, 5);
        ...
        nftIdRewards.pop();
        // Update number of claim NFT if satisfies
        user.noOfClaimedNFT = user.noOfClaimedNFT + 1;
    ...
    // i.e., 1, 2, 3
    function getNumberOfDepositBUSD(address _usr) 
        external
        view
        returns (uint256)
    {
        UserInfo storage user = userInfo[_usr];
        return (user.noOfDepositBUSD);   
    }

    // i.e., 1, 2, 3
    function getNoOfDepositChest(address _usr) 
        external
        view
        returns (uint256)
    {
        UserInfo storage user = userInfo[_usr];
        return (user.noOfDepositChest);   
    }

    function getHistory(address _usr, uint256 index_) 
        external
        view
        returns (uint256)
    {
        UserInfo storage user = userInfo[_usr];
        if (user.rewardStakingBlockLFW[index_] < user.rewardStakingBlockNFT[index_]) {
            return user.rewardStakingBlockNFT[index_];
        } else {
            return user.rewardStakingBlockLFW[index_];
        }
    }    
    ...
    {
        UserInfo storage user = userInfo[_usr];
        if (user.noOfcanClaimLFW <= user.noOfcanClaimNFT) {
            return (4 - user.noOfcanClaimLFW);
        } else {
            return (4 - user.noOfcanClaimLFW);
        }
    }
        ...
        returns (uint256)
    {
        UserInfo storage user = userInfo[_usr];
        if (user.noOfcanClaimLFW <= user.noOfcanClaimNFT) {
            return user.noOfcanClaimLFW;
        } else {
            return user.noOfcanClaimNFT;
        }
    }

    function getUserNextRewardBlock(address _usr) 
        external
        view
        returns (uint256)
    {
        UserInfo storage user = userInfo[_usr];
        if (user.rewardStakingBlockLFW[0] < user.rewardStakingBlockNFT[0]) {
            return user.rewardStakingBlockNFT[0];
        } else {
            return user.rewardStakingBlockLFW[0];
        }
    }
        ...
        returns (uint256)
    {
        UserInfo storage user = userInfo[_usr];
        if (user.rewardStakingBlockLFW[index_] < user.rewardStakingBlockNFT[index_]) {
            return (block.number - user.rewardStakingBlockNFT[index_]);
        } else {
            return (block.number - user.rewardStakingBlockLFW[index_]);
        }        
    }
