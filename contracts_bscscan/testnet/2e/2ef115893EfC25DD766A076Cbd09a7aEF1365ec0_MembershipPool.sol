...
contract MembershipPool is ReentrancyGuard,Authorizable{
    ...
    uint public constant POOLS = 4;
    uint public constant POOLS_INDEX = POOLS + 1;
    ...
    function getBalances(uint _index) public view returns(uint256) {
        if(address(tokens[_index]) == address(rewardToken)){
            return tokens[_index].balanceOf(address(this)).sub(storedFees[_index].add(reserveAmount[_index].add(rewardTokenAmount)));
        }
        ...
        if(terminate){
            ...
            for(uint8 i=0; i<N_COINS; i++){
                if(_tokens[i] > 0){
                    ...
                    _total += _tokens[i];
                    emit ImmediatelyWithdraw(msg.sender,_tokens[i].sub(temp),i);
                }
            }
    ...
    function notifyMedalReward(address[] memory addresses) public onlyAuthorized(){
        for(uint256 i=0;i< addresses.length;i++){
           if(members[addresses[i]].totalAmount > 0 ){
