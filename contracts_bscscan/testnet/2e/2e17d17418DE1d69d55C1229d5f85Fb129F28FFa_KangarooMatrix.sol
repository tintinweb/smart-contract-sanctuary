...
contract KangarooMatrix {
    ...
    address public oldMatrix;
    bool private isMigrated=false;
    ...
    function finishMigration() external onlyRoot {
        ...
        require(lastUserId==oldlastUserId,string(abi.encodePacked("not all ",uint2str(oldlastUserId)," users have migrated")));
        oldMatrix=address(0);
        isMigrated=true;
    }
    ...
    function buyNewLevel(uint8 matrix, uint8 level) external {
        require(isMigrated, "migration not finished");
    ...
    function migrationsMatrix6(uint32 id,uint8 level) public view returns(uint32, uint32[] memory, uint32[] memory, bool, uint32, uint32) {
    ...
    function migrationsMatrix3(uint32 id,uint8 level) public view returns(uint32, uint32[] memory, bool, uint32) {
    ...
    function migrationsUser(uint32 id) external returns(bool){
        
        require(lastUserId==id,"invalid id");
        ...
        for(uint8 level=1;level<=static_user.activeX3Levels;level++){
            ...
            (matrixX3.currentReferrerID,
            matrixX3.referralsID,
            matrixX3.blocked,
            matrixX3.reinvestCount)=migrationsMatrix3(id,level);
            user.x3Matrix[level]=matrixX3;
        }
        X6 memory matrixX6;
        for(uint8 level=1;level<=static_user.activeX6Levels;level++){
            
            (matrixX6.currentReferrerID,
            matrixX6.firstLevelReferralsID,
            matrixX6.secondLevelReferralsID,
            matrixX6.blocked,
            matrixX6.closedPart,
            matrixX6.reinvestCount)=migrationsMatrix6(id,level);
            user.x6Matrix[level]=matrixX6;
            
        }
        AddressToId[static_user.userAddress]=lastUserId;
        
        require(static_user.activeX6Levels>=1 || stakePool.createUser(static_user.userAddress),"cant create User");
