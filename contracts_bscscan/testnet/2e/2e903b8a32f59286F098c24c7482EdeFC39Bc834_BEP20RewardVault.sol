    ...
    function deposit(uint256 _amount, address _indicator) public {
        ...
        if (_amount > 0) {
            ...
            if (user.indicator == address(0) && _indicator != address(0)) {
                ...
                totalIndicationStaked = totalIndicationStaked.add(_amount);
            } else if (user.indicator != address(0)) {
    ...
    function withdraw(uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (indicatorInfo[user.indicator].amount >= _amount) {
                if (indicatorInfo[user.indicator].amount == _amount) {
                    pendingRewardAmountIndicator = indicatorInfo[user.indicator]
                        .amount
                        .mul(
                            indicatorInfo[user.indicator].accRewardTokenPerShare
                        )
                        .div(1e30)
                        .sub(indicatorInfo[user.indicator].rewardDebt);
                    safeTransferReward(
                        address(user.indicator),
                        pendingRewardAmountIndicator
                    );
                ...
                totalIndicationStaked = totalIndicationStaked.sub(_amount);
            }
        }
