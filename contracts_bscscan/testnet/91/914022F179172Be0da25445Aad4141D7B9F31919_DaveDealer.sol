...
contract DaveDealer is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DaveDealerDividendTracker public dividendTracker;

    address public liquidityWallet;

    address payable public marketingAddress = payable(0x2228d90a12bf94b7BD112a78b7A2c997985b8715);
    ...
    constructor() ERC20("DaveDealer", "DogeDealer") {
        dividendTracker = new DaveDealerDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        DaveDealerDividendTracker newDividendTracker = DaveDealerDividendTracker(
            payable(newAddress)
        );
    ...
    function updatePayoutToken(address token) public {
        require(balanceOf(msg.sender) > 0, "You must own more than zero $DaveDealer tokens to switch your payout token!");
...
contract DaveDealerDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "DaveDealer_Dividend_Tracker",
            "DaveDealer_Dividend_Tracker"
        )
    {
        daveDealer = DaveDealer(mainContract);
    ...
    function _transfer(
        ...
        require(false, "DaveDealer_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DaveDealer_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DaveDealer contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(daveDealer);

            uint256 prevBalance = daveDealer.balanceOf(address(this));
            ...
            {
                uint256 received = daveDealer.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    daveDealer.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
