...
contract HYBN is Ownable {
    ...
    uint256[] public MINIMUM = [500000000000000000000,500000000000000000000,5000000000000000000000];
    ...
    function _init() private {
        ...
        //second
        investmentPlans_.push(Objects.Plan(20,15*60*60*24,20));
        investmentPlans_.push(Objects.Plan(30,30*60*60*24,30));
        investmentPlans_.push(Objects.Plan(45,60*60*60*24,45));
        investmentPlans_.push(Objects.Plan(60,90*60*60*24,60));
        investmentPlans_.push(Objects.Plan(80,180*60*60*24,80));
        //third
    ...
    function getInvestorInfoByUID(uint256 _uid) public view returns (uint256, uint256, uint256,  uint256, uint256[] memory, uint256[] memory, uint256[] memory, uint256[] memory) {
        ...
        uint256[] memory currentDividends = new  uint256[](investor.planCount);
         uint256[] memory tokenInvestment = new uint256[](investor.planCount);
        uint256[] memory nextWithdrawalDate = new  uint256[](investor.planCount);
        for (uint256 i = 0; i < investor.planCount; i++) {
            require(investor.plans[i].investmentDate != 0, "wrong investment date");
            currentDividends[i] = investor.plans[i].currentDividends;
            tokenInvestment[i] = investor.plans[i].investmentToken;
    ...
    function _invest(address _addr, uint256 _planId, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        if(_planId==0)
        {
            ...
            nextWithdrawal=block.timestamp+7 days;
        }
        
         else if(_planId>0 && _planId<=5)
        {
            require(_amount>=MINIMUM[1] && _amount%(100*1e18)==0,"Wrong Amount2.");
            tokenAmount=(_amount.div(tPrice)).mul(1e18);
        ...
        {
            require(_amount>=MINIMUM[2] && _amount%(1000*1e18)==0,"Wrong Amount1.");
            tokenAmount=(_amount.div(tPrice)).mul(1e18);
            nextWithdrawal=block.timestamp+investmentPlans_[_planId].term;
        }
        ...
        investor.plans[planCount].investmentToken = tokenAmount;
    ...
    function withdraw() public payable {
        ...
        for (uint256 i = 0; i < uid2Investor[uid].planCount; i++) 
        {
            ...
            if (plan.term > 0) 
            {
                ...
                if (withdrawalDate >= endTime) 
                {
                    ...
                    isExpired = true;
                    withdrawalAmount=withdrawalAmount+((uid2Investor[uid].plans[i].investment.div(_tokenPrice())).mul(1e18));
                }
            }
            ...
            refRoi=refRoi+((amount.mul(1e18)).div(_tokenPrice()));
        ...
        hbcToken.transfer(msg.sender,((withdrawalAmount.mul(1e18)).div(_tokenPrice())));
