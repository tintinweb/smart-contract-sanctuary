...
import "./ElpisHeroStakingPool.sol";

contract ElpisHeroStakingFactory is IElpisHeroStakingFactory, Ownable {
    ...
    function getStakingPoolsLength() external view override returns (uint256) {
        return stakingPools.length;
    }

    constructor(IERC721 _elpisHeroes, IElpisBattle _eba) {
        require(
            address(_elpisHeroes) != address(0),
            "elpisHeroes is the zero address"
        );
        require(address(_eba) != address(0), "eba is the zero address");
    ...
    function createPool(
        ...
        string memory empty = "";
        require(
            keccak256(abi.encode(_name)) != keccak256(abi.encode(empty)),
            "name is the empty string"
        );
        require(_startBlock > block.number, "Invalid start block");
        require(_merkleRoot != bytes32(0), "Invalid merkle root");
        require(getPool[_name] == address(0), "Pool name already exist");

        pool = address(
            new ElpisHeroStakingPool{
                salt: keccak256(
                    abi.encode(_name, _ebaPerBlock, _startBlock, _merkleRoot, _owner)
                )
            }()
        );

        IElpisHeroStakingPool(pool).initialize(
            elpisHeroes,
            eba,
            _name,
            _ebaPerBlock,
...
contract ElpisHeroStakingPool is IElpisHeroStakingPool, ERC721Holder, Ownable {
    ...
    struct UserInfo {
        ...
        //   amount = holderTokens.lenght() // The number of tokens in ``owner``'s account.
    ...
    modifier onlyGenesis(uint256 tokenId) {
        require(tokenId < 20000, "The tokenId is not in the Genesis");
        _;
    }
    ...
    function updateEabPerBlock(uint256 _ebaPerBlock) external override onlyOwner {
        updateStakingPool();
        ebaPerBlock = _ebaPerBlock;
    ...
    function accumulated(address _user)
        external
        view
        override
        returns (uint256)
    ...
    function pendingEba(address _user)
        external
        view
        override
        returns (uint256)
    ...
    function updateStakingPool() public {
        ...
        uint256 multiplier = getMultiplier(lastRewardBlock, block.number);
        uint256 ebaReward = multiplier.mul(ebaPerBlock);
        accEbaPerShare = accEbaPerShare.add(ebaReward.div(lpSupply));
        lastRewardBlock = block.number;
    }
    ...
    function batchStake(
        ...
        require(
            _indexs.length == _tokenIds.length &&
                _indexs.length == _merkleProofs.length,
            "indexs, tokenIds and merkleProofs length missmatch"
        );
        updateStakingPool();
        ...
        for (uint256 i = 0; i < _indexs.length; ++i) {
            require(
                _tokenIsValid(_indexs[i], _tokenIds[i], _merkleProofs[i]),
                "Verify tokenId failed"
            );
            elpisHeroes.safeTransferFrom(
                address(msg.sender),
...
interface IElpisHeroStakingPool {
    ...
    /// @notice update the amount EBAs eba tokens created per block. Should only be called by the owner
    /// @param eabPerBlock The amount of EBAs tokens created per block.
    function updateEabPerBlock(uint256 eabPerBlock) external;

    /// @notice View function to see pending eba on frontend.
    /// @param user The address of user.
    /// @return the pending reward of user.
    function pendingEba(address user) external view returns (uint256);

    /// @notice View function to see the reward users have accumulated so far.
    /// @param user The address of user.
    /// @return the reward accumulated of user.
