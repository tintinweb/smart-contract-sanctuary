...
contract deployerCRDN is ReentrancyGuard {
    ...
    modifier onlyAdmin(){
        require(msg.sender == admin,"DEPLOYER: Not an admin");
        _;
    }
    ...
    function createPreSaleBNB(
        ...
        require(address(token) != address(0), 'DEPLOYER: ZERO_ADDRESS');
        require(isPreSaleExist[address(token)] == false, 'DEPLOYER: PRESALE_EXISTS'); // single check is sufficient
        
        uint256 _vestingPercent = hundred.sub(_initialClaimPercent).mul(100).div(_totalClaimCycle);
        // parameters.push(_tokenPrice);
        // parameters.push(_presaleTime);
        // parameters.push(_initialClaimPercent);
        // parameters.push(_vestingPercent);
        // parameters.push(_refPercent);
        // parameters.push(_minAmount);
        // parameters.push(_maxAmount);
        // parameters.push(_hardCap);
        // parameters.push(_softCap);
        // parameters.push(_listingPrice);
        // parameters.push(_liquidityPercent);
        // parameters.push(_totalClaimCycle);
        ...
        //     parameters[1], // _tokenPrice,
        //     parameters[10], // _listingPrice,
        //     parameters[8], // _hardCap,
        //     parameters[11] // _liquidityPercent
        // );
    ...
    function setPreSaleBNB(
        ...
        tokenAmount = tokenAmount.mul(10 ** (token.decimals()));
        token.transferFrom(msg.sender, getPreSale[address(token)], tokenAmount);
        IPreSale(getPreSale[address(token)]).initializeRemaining(
            _tokenPrice,
            _presaleTime,
            _hardCap,
            _softCap,
