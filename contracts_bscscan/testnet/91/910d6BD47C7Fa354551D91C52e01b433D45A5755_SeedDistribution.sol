...
contract SeedDistribution {
    ...
    function addAddresses(address[] memory _addresses, uint256[] memory _ammounts) public onlyOwner {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            inPresale[_addresses[i]].tokenAmount = _ammounts[i]*10**18;
            inPresale[_addresses[i]].newAmount = _ammounts[i]*10**18;
            inPresale[_addresses[i]].tgeCollect = (((_ammounts[i]*10**2)/seedPrice)*tgeToCollect)/(1*10**5)*10**18;
            inPresale[_addresses[i]].nrmCollect = (((_ammounts[i]*10**2)/seedPrice)*normalCollect)/(1*10**5)*10**18;
        }
    ...
    function setTGEHappened() public onlyOwner {
        ...
        Collect1 = block.timestamp + 5 minutes;
        Collect2 = block.timestamp + 10 minutes;
        Collect3 = block.timestamp + 15 minutes;
        Collect4 = block.timestamp + 15 minutes;
        Collect5 = block.timestamp + 15 minutes;
        Collect6 = block.timestamp + 15 minutes;
        Collect7 = block.timestamp + 15 minutes;
        Collect8 = block.timestamp + 15 minutes;
        Collect9 = block.timestamp + 15 minutes;
        Collect10 = block.timestamp + 15 minutes;
    }
