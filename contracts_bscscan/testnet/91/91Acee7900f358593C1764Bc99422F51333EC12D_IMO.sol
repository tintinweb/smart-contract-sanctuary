...
contract IMO is ReentrancyGuard {
    ...
    uint256 public totalWrapAmount;
    ...
    function setOfferingAmount(uint256 _offerAmount) public onlyAdmin {
        require(block.number < imoInfo[imoNumber].endBlock, "no");
    ...
    function deposit(uint256 _amount) public {
        require(
            block.number > imoInfo[imoNumber].harvestBlock ||
                block.number < imoInfo[imoNumber].endBlock,
            "not ifo time"
        );
        require(_amount > 0, "need _amount > 0");
        uint256 amountAfterFee = _amount.mul(9).div(10);
        if (
            block.number < imoInfo[imoNumber].startLateBlock ||
            block.number > imoInfo[imoNumber].harvestBlock
        ) {
    ...
    function depositWrap(uint256 _amount) public {
        ...
        if (
            ...
            totalWrapAmount = totalWrapAmount.add(_amount);
        } else if (block.number > imoInfo[imoNumber].startLateBlock) {
            lpTokenWrap.safeTransferFrom(
                address(msg.sender),
                address(this),
    ...
    function depositFund(uint256 _amount) public nonReentrant {
        ...
        require(
            userInfo[msg.sender].amountFund[imoNumber] + _amount <=
                getFundLimit(msg.sender),
            "exceed fund limits"
        );
        imoInfo[imoNumber].fundToken.safeTransferFrom(
            address(msg.sender),
            address(this),
            _amount
        );
        userInfo[msg.sender].amountFund[imoNumber] = userInfo[msg.sender]
        .amountFund[imoNumber]
        .add(_amount);
        imoInfo[imoNumber].totalFund = imoInfo[imoNumber].totalFund.add(
            _amount
        );
    }
    ...
    function harvest() public nonReentrant {
        ...
        imoInfo[imoNumber].offeringToken.safeTransfer(
            address(msg.sender),
            offeringTokenAmount
        );
        if (refundingTokenAmount > 0) {
            imoInfo[imoNumber].fundToken.safeTransfer(
                address(msg.sender),
    ...
    function withdraw(uint256 _amount) public nonReentrant {
        require(
            block.number > imoInfo[imoNumber].withdrawStartBlock ||
                block.number < imoInfo[imoNumber].endBlock,
            "not withdraw time"
        );
    ...
    function withdrawWrap(uint256 _amount) public nonReentrant {
        ...
        totalAmount = totalAmount.sub(_amount);
        totalWrapAmount = totalWrapAmount.sub(_amount);
    }
    ...
    function getUserAllocation(address _user) public view returns (uint256) {
        return
            userInfo[_user]
                .amountFund[imoNumber]
                .mul(1e12)
                .div(imoInfo[imoNumber].totalFund)
                .div(1e6);
    }
    ...
    function getOfferingAmount(address _user) public view returns (uint256) {
        if (imoInfo[imoNumber].totalFund > imoInfo[imoNumber].raisingAmount) {
            ...
            return
                userInfo[_user]
                    .amountFund[imoNumber]
                    .mul(imoInfo[imoNumber].offeringAmount)
                    .div(imoInfo[imoNumber].raisingAmount);
        }
    }
    ...
    function getRefundingAmount(address _user) public view returns (uint256) {
        ...
        uint256 allocation = getUserAllocation(_user);
        uint256 payAmount = imoInfo[imoNumber]
        .raisingAmount
        .mul(allocation)
        .div(1e6);
    ...
    function finalWithdraw(
        ...
        require(
            _fundAmount <=
                imoInfo[_imoNumber].fundToken.balanceOf(address(this)),
            "not enough fund token "
        );
        require(
            _offerAmount <=
                imoInfo[_imoNumber].offeringToken.balanceOf(address(this)),
        ...
        if (_fundAmount > 0) {
            imoInfo[_imoNumber].fundToken.safeTransfer(
                address(msg.sender),
                _fundAmount
            );
        }
        if (_offerAmount > 0) {
            imoInfo[_imoNumber].offeringToken.safeTransfer(
                address(msg.sender),
                _offerAmount
            );
        }
    ...
    function transferToVault() public onlyAdmin {
        if (imoInfo[imoNumber].fundToken.balanceOf(address(this)) > 0) {
            imoInfo[imoNumber].fundToken.safeTransfer(
                address(vaultAddress),
                imoInfo[imoNumber].fundToken.balanceOf(address(this))
            );
        }
        if (imoInfo[imoNumber].offeringToken.balanceOf(address(this)) > 0) {
            imoInfo[imoNumber].offeringToken.safeTransfer(
                address(vaultAddress),
                imoInfo[imoNumber].offeringToken.balanceOf(address(this))
            );
        }
    }

    function getAmountFund(address _user, uint256 _imoNumber)
        external
        view
        returns (uint256)
    {
    ...
    function setVaultAddress(address _vaultAddress) public onlyAdmin {
        vaultAddress = _vaultAddress;
    }

    function getFundLimit(address _user) public view returns (uint256) {
        uint256 fundLimit = 0;
        if (getTier(_user) == Tier.One) {
            fundLimit = hardCap[1].mul(imoInfo[imoNumber].raisingAmount).div(
                1e6
            );
        } else if (getTier(_user) == Tier.Two) {
            fundLimit = hardCap[2].mul(imoInfo[imoNumber].raisingAmount).div(
                1e6
            );
        } else if (getTier(_user) == Tier.Three) {
            fundLimit = hardCap[3].mul(imoInfo[imoNumber].raisingAmount).div(
                1e6
            );
        } else if (getTier(_user) == Tier.Four) {
            fundLimit = hardCap[4].mul(imoInfo[imoNumber].raisingAmount).div(
                1e6
            );
        } else if (getTier(_user) == Tier.Five) {
            fundLimit = hardCap[5].mul(imoInfo[imoNumber].raisingAmount).div(
                1e6
            );
        }
        return fundLimit;
