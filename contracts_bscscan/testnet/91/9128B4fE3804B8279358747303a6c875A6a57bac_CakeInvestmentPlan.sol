...
contract CakeInvestmentPlan is IInvestmentPlan, PancakeSwapHelper, EmergencyWithdrawable {
    ...
    event Deposited(uint256 cakeAmount, uint256 bnbAmount);

    constructor(IBEP20 _cake, IMasterChef _masterChef, address routerAddress, address profitDestination) PancakeSwapHelper(routerAddress) {
    ...
    function deposit() external override payable {
        uint256 cakeAmount = swapBNBForTokens(msg.value, cake, address(this));

        masterChef.enterStaking(cakeAmount);
        totalStaked += cakeAmount;

        emit Deposited(cakeAmount, msg.value);
    ...
    function doWithdrawProfits() private {
        ...
        uint256 initialCakeBalance = cake.balanceOf(address(this));
        masterChef.leaveStaking(0);
        uint256 profitsCake = cake.balanceOf(address(this)) - initialCakeBalance;

        uint256 bnb = swapTokensForBNB(profitsCake, cake, profitsDestination);
        totalProfitsBNB += bnb;

        emit Withdrawn(profitsCake, bnb);
    }
}
