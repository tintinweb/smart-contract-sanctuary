...
contract BeeToken is BEP20 {
	...
	modifier antiWhale(
		...
		if (maxTransferAmount() > 0) {
			if (_excludedFromAntiWhale[sender] == false && _excludedFromAntiWhale[recipient] == false) {
				require(amount <= maxTransferAmount(), "BEE::antiWhale: Transfer amount exceeds the maxTransferAmount");
			}
		}
		_;
	}
	 ...
	 */
	constructor() public BEP20("Lalat Token", "Lalat") {
	...
	function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
		...
		require(
			...
			"BEE::updateMaxTransferAmountRate: Max transfer amount rate must be grater than min rate"
		);
	...
	function setSwapPair() public onlyOperator {
		...
		require(swapPair != address(0), "BEE::updateSwapRouter: Invalid pair address. Check if liquidity was added");
	}
