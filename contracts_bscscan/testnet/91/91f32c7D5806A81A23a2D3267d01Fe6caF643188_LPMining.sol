...
contract LPMining is Ownable, ReentrancyGuard, ILPMining {
    ...
    function emergencyWithdraw(uint256 _pid, uint256 _amount) public nonReentrant validatePoolByPid(_pid) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function withdrawAnyToken(IERC20 _token, uint256 amount) external onlyOwner {
        require(!isLPPoolAdded[address(_token)], "Can't withdraw LP token");
