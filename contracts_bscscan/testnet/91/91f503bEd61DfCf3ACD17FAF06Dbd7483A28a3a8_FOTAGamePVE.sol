...
contract FOTAGamePVE is Auth, EIP712Upgradeable {
  ...
  function finishGame(uint _mission, bool _win, uint8 _currentEnergy, bytes memory _signature) external {
    _validateFinishGame(_mission, _win, _currentEnergy, _signature);
    // TODO update game data
  }

  // PRIVATE FUNCTIONS

  function _validateFinishGame(uint _mission, bool _win, uint8 _currentEnergy, bytes memory _signature) private {
    require(_mission <= totalStations, "Station invalid");
    bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
        keccak256("FinishGamePVE(address user,uint256 nonce,uint256 stage,bool win,uint8 energy)"),
        msg.sender,
        nonces[msg.sender],
        _mission,
        _win,
