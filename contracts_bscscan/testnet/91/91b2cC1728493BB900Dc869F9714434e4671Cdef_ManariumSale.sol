...
contract ManariumSale is Ownable, OwnerPausable
{
    ...
    uint256 public maxContribution = 5 * 10**18;  // 5 bnb
    
    uint256 public tokensPerBNB; 
    uint256 public receivedAmount;

    uint256[] public unlockTimeStamp; // timestamp when unlock
    uint256 constant public vestingPeriod = 15 minutes; 
    uint8 constant public vestingUnlockProcent = 85; // 8.5%
    uint8 constant public listingUnlockProcent = 150; // 150%
    uint16 constant public maxProcent = 1000; //100%
    ...
    function finilize() external onlyOwner{
        ...
        while(procent < maxProcent) 
        {
            ...
            unlockTimeStamp.push(nextUnlock); // Add all unlock timestamp
    ...
    function calculateWithdrawTokens(address sender) private view returns (uint256)
    {
        ...
        for(uint8 i = 0; i < unlockTimeStamp.length; i++){
             if(block.timestamp < unlockTimeStamp[i])
                break; 
