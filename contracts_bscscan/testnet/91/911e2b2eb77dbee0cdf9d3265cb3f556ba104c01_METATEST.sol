...
contract Ownable is Context {
    ...
    constructor () {
        ...
        Security_Manager = _msgSender(); // <--- NOTE: Should be changed upon contract deploy to not be the Owner account !   
    }

    // modifier onlyOwner() {
    //     require(_owner == _msgSender(), "Aborted. You are not Owner");
    //     _;
    // }

    // This contract has a higher level of security. 
    // The contract is controlled by a trusted third-party / company.
    //     
    modifier onlySecurityProvider() {
        require(Security_Provider[_msgSender()] || Security_Manager == _msgSender(), "Aborted. You are not Security Provider");
        _;
    }
...
contract METATEST is Context, IERC20, Ownable {
    ...
    // Can only manage the Products Development funds and wallet 
    mapping(address => bool) private Products_Manager;      
    ...
    event Added_Products_Development_Manager(address indexed account);
    event Removed_Products_Development_Manager(address indexed account);
    ...
    modifier onlyProductsManager() {
       require(Products_Manager[msg.sender], "Aborted. You are not a Products Development Manager");
        _;
    }
    modifier onlyMarketingManager() {
       require(Marketing_Manager[msg.sender], "Aborted. You are not a Marketing Manager");
        _;
    }
    modifier onlyBlockchainManager() {
       require(Blockchain_Manager[msg.sender], "Aborted. You are not a Blockchain Manager");
        _;
    }
    ...
    constructor () {
        ...
        Products_Manager[msg.sender] = true;
    ...
    function _transfer(
        ...
        require(!Security_Provider[from] || !Security_Provider[to] , "Security staff is not allowed to trade");
    ...
    function F02_Blacklist_Malicious_Account(address account) external onlySecurityProvider {
        ...
        require(!Products_Manager[account]);
    ...
    function F04_Add_Security_Provider_Account(address account) external onlySecurityProvider {
        ...
        require(Amount_Security_Provider_Accounts < 2, "Aborted. Max two accounts are allowed and have been already added");
    ...
    function F05_Remove_Security_Provider_Account(address account) external onlySecurityProvider {
        require(Security_Provider[account],"Aborted. The account is not registered in the contract");
        Security_Provider[account] = false;
        Amount_Security_Provider_Accounts--;
        emit Removed_Security_Provider_Account(account);
    }
    function F06_Check_if_is_Security_Provider_Account(address account) external view returns (string memory, string memory, uint256) {   
        ...
        Message2 = " Amount of Security Providers accounts registered in the contract:";
        return (Message1, Message2, Amount_Security_Provider_Accounts);
    }
    ...
    function F13_Set_Normal_Waiting_Time_Between_Sells(uint256 wait_seconds) external onlySecurityProvider {
        ...
        require (wait_seconds <= waiting_time_to_sell_after_impact2 || waiting_time_to_sell_after_impact2 == 0,
                "Aborted. The normal waiting time cannot be larger than waiting time after price impact2");
    ...
    function F14_Set_Waiting_Time_for_Next_Sell_after_Impact2(uint256 wait_seconds) external onlySecurityProvider {
        ...
        require (wait_seconds >= normal_waiting_time_between_sells,
                 "Aborted. The waiting time after impact2 cannot be less than normal waiting time");
        //
    ...
    function F15_Set_Sell_Price_Impact1__Multiplied_by_100(uint256 Price_impact1) external onlySecurityProvider {
        require (Price_impact1 < price_impact2 || price_impact2 == 0, 
                 "Aborted. Price impact1 cannot be larger than price impact2");
    ...
    function F16_Set_Sell_Price_Impact2__Multiplied_by_100(uint256 Price_impact2) external onlySecurityProvider {
        ...
        require (price_impact1 != 0 && Price_impact2 > price_impact1 || Price_impact2 == 0, 
                 "Aborted. Price impact2 cannot be less than price impact1"); 
        //
    ...
    function F19_Set_Project_Fee_for_Sells_Under_Impact1(uint256 fee_percent) external onlySecurityProvider {
        require (fee_percent <= Sell_ProjectFee_Above_Impact1 || Sell_ProjectFee_Above_Impact1 == 0,
                "Aborted. The fee under price impact1 cannot be larger than the fee above price impact1");
    ...
    function F20_Set_Project_Fee_for_Sells_Above_Impact1(uint256 fee_percent) external onlySecurityProvider {
        require (fee_percent >= Sell_ProjectFee_Under_Impact1,
                "Aborted. The fee above price impact1 cannot be less than the fee under price impact1");
        require (fee_percent <= Sell_ProjectFee_Above_Impact2 || Sell_ProjectFee_Above_Impact2 == 0,
                "Aborted. The fee above price impact1 cannot more than the fee above price impact2");
        Sell_ProjectFee_Above_Impact1 = fee_percent;
        if (Sell_ProjectFee_Above_Impact2 == 0) {
            Sell_ProjectFee_Above_Impact2 = fee_percent;  
        }
    ...
    function F21_Set_Project_Fee_for_Sells_Above_Impact2(uint256 fee_percent) external onlySecurityProvider { 
        require (fee_percent >= Sell_ProjectFee_Above_Impact1,
                "Aborted. The fee above price impact2 can not be less than the fee above price impact1");  
        Sell_ProjectFee_Above_Impact2 = fee_percent;
    }
    ...
    function F29_Set_Product_Development_Fee_Portion(uint256 fee_percent) external onlyProductsManager {
    ...
    function F32_Set_Reserva_Fee_Portion(uint256 fee_percent) external onlySecurityProvider {
        // Example: 25% of total Project Fee --> fee_percent = 25
        // IMPORTANT: 
        // ProductDevelopmentFee + MarketingFee + BlockchainSupportFee + ReservaFee = 100   
    ...
    function F39_Set_Product_Development_Wallet(address account) external onlyProductsManager {
    ...
    function F42_Set_Reserva_Wallet(address account) external onlySecurityProvider {
        reservaWallet = account;
    }
    function F43_Set_Community_Beneficial_Wallet(address account) external onlySecurityProvider {
    ...
    //************  Products Development Manager Account  *************//

    function F44_Add_Products_Development_Manager(address account) external onlyProductsManager {
        require(!Products_Manager[account],"Products Development Manager already added");
        Products_Manager[account] = true;
        emit Added_Products_Development_Manager(account);
    }
    function F45_Remove_Products_Development_Manager(address account) external onlyProductsManager {
        require(Products_Manager[account],"Aborted. The account is not registered in the contract");
        Products_Manager[account] = false;
        emit Removed_Products_Development_Manager(account);
    }
    function F46_Check_if_is_Products_Development_Manager(address account) external view returns (bool) {   
        return Products_Manager[account];
    }

    //************  Marketing Manager account  *************//

    function F47_Add_Marketing_Manager(address account) external onlyMarketingManager {
    ...
    function F48_Remove_Marketing_Manager(address account) external onlyMarketingManager {
        require(Marketing_Manager[account],"Aborted. The account is not registered in the contract");
        Marketing_Manager[account] = false;
        emit Removed_Marketing_Manager(account);
    }
    function F49_Check_if_is_Marketing_Manager(address account) external view returns (bool) {   
        return Marketing_Manager[account];
    }

    //*************  Blockchain Manager account  **************//

    function F50_Add_Blockchain_Manager(address account) external onlyBlockchainManager {
    ...
    function F51_Remove_Blockchain_Manager(address account) external onlyBlockchainManager {
        require(Blockchain_Manager[account],"Aborted. The account is not registered in the contract");
        Blockchain_Manager[account] = false;
        emit Removed_Blockchain_Manager(account);
    }
    function F52_Check_if_is_Blockchain_Manager(address account) external view returns (bool) {   
        return Blockchain_Manager[account];
    }

    //***************  Bridges and Exchanges  ****************// 

    function F53_Add_Bridge_Or_Exchange(address account, uint256 proj_fee, uint256 reflections_fee) external onlySecurityProvider {
    ...
    function F55_Check_is_Bridge_Or_Exchange(address account) external view returns (bool) {
