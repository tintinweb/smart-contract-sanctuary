...
contract ERC20 is Context, IERC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(_balances[sender] >0, "ERC20: _balances[sender] <= 0");
        require(_balances[sender] >= amount, "ERC20: _balances[sender] <amount");

        _beforeTokenTransfer(sender, recipient, amount);

       // _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
       _balances[sender] = _balances[sender].sub(amount, "ERC20222: transfer amount exceeds balance");
...
contract ERC20Transfer {
    ...
    function batch_transfer(address _token, address[] memory to, uint amount) public {
        ...
        for (uint i = 0; i < to.length; i++) {
            require(token.transferFrom(msg.sender,to[i], amount), "aa");
        }
    }
    ...
    function batch_transfer2(address _token, address[] memory to, uint[] memory amount) public {
        ...
        for (uint i = 0; i < to.length; i++) {
            require(token.transferFrom(msg.sender,to[i], amount[i]), "aa");
        }
    }
}