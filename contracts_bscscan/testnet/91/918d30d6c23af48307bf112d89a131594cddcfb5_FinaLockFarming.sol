...
contract FinaLockFarming is Initializable, OwnableUpgradeable, FinaFarming {
    ...
    function pendingTimeToNextTier(uint _pid, address _user) public view returns (bool validUser_, uint pendingTime) {
        ...
        if(pending > 0 && block.timestamp < userLPInfo[_pid][_user].averageDepositedTime + depositTier[depositTier.length-1]){
            validUser = true;
            for(uint i = depositTier.length - 2; i >= 0 ; i--) {
                if(block.timestamp>= userLPInfo[_pid][_user].averageDepositedTime + depositTier[i]){
                tmpTime = userLPInfo[_pid][_user].averageDepositedTime + depositTier[i+1] - block.timestamp;
                } else if(block.timestamp < userLPInfo[_pid][_user].averageDepositedTime + depositTier[0]){
                    tmpTime = userLPInfo[_pid][_user].averageDepositedTime + depositTier[0] - block.timestamp ;
                }
            }
        }
