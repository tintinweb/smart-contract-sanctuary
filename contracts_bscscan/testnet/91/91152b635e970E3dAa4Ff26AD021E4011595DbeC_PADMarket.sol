...
contract PADMarket is Initializable, OwnableUpgradeable, UUPSUpgradeable, IERC721ReceiverUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, AccessControlUpgradeable {
  ...
  /**
    ...
    */
  function initialize(address _BUSD, address _NFTContract, address _feeReceiver, uint _fees, uint _bnbfees) public initializer {
    __Ownable_init();
    __Pausable_init();
   ...
   // === WRITE FUNCTIONS === //
    ...
    function createOffer(uint256 tokenId, uint256 price, uint limit, address _buyer) public payable isOwner(tokenId) nonReentrant whenNotPaused {
      ...
      // console.log("Adding item ", itemId ," to marketplace => Card ", tokenId);
      ...
      // console.log("Transfering card ", tokenId, " to market");

      IPADNFT(PAD_ADDRESS).safeTransferFrom(msg.sender, address(this), tokenId);

      // console.log("Emiting event MarketCardCreated");
    ...
    function sellOffer(uint256 itemId, uint256 _amount) public payable nonReentrant whenNotPaused {
      ...
      // console.log("Transfering value to seller");
      SafeERC20Upgradeable.safeTransferFrom(IERC20Upgradeable(BUSD), msg.sender, token.seller, realPrice);

      // console.log("Transfering card to buyer");
      IPADNFT(PAD_ADDRESS).safeTransferFrom(address(this), msg.sender, token.tokenId);

      // console.log("Setting owner to MarketCard offer recently sold");
      token.owner = payable(msg.sender);

      // console.log("Sending fees to ", feeReceiver);
      SafeERC20Upgradeable.safeTransferFrom(IERC20Upgradeable(BUSD), msg.sender, FEE_RECEIVER, fees);

      // console.log("Increasing items sold counter");
  ...
  // ======================= //
   ...
   // === READ FUNCTIONS === //
    ...
    function marketBalanceOf() external view returns (uint){
      ...
      // console.log("Searching ", msg.sender ," cards in marketplace");
    ...
    function marketBalanceOf(address _user) external view returns (uint){
      ...
      // console.log("Searching ", _user ," cards in marketplace");
      for (uint256 i = 1; i <= totalItemCount; i++) {
    ...
    function fetchUserMarketCards(address _userAddress) public view returns (MarketCard[] memory) {
      ...
      // console.log("Searching ", _userAddress ," cards in marketplace");
      for (uint256 i = 1; i <= totalItemCount; i++) {
      ...
      // console.log("Fetching ", _userAddress ," market cards");
      for (uint256 i = 1; i <= totalItemCount; i++) {
        if (idToMarketCard[i].seller == _userAddress && idToMarketCard[i].owner == address(0)) {
          MarketCard storage currentItem = idToMarketCard[i];
          items[c] = currentItem;
  ...
  // ================ //
   ...
   // === FEES CONFIG === //
     ...
     */
    function setFees(uint256 _fee) external onlyRole(ADMIN_ROLE) whenNotPaused {
      FEES = _fee;
    }
  ...
  // ========================== //
   ...
   // === FEE RECEIVER CONFIG === //
    ...
    /**
     * @notice Sets the new fee receiver address
     * @param _feeReceiver The address to set
     */
    function setFeeReceiver(address _feeReceiver) external onlyRole(ADMIN_ROLE) whenNotPaused {
      FEE_RECEIVER = _feeReceiver;
    }

    /**
     * @notice Check the fee receiver address stored
