...
interface IKawaiiMarketplace {
    function createAuction(address _nftAddress, uint256 _tokenId, uint256 _amount, uint256 _startingPrice, uint256 _endingPrice, uint256 _duration, address sender, uint8 v, bytes32 r, bytes32 s) external;
...
interface IKawaiiSalePack {
    function buyPack(address nftRegister, address sender, string memory pack, uint256 amount, uint8 v, bytes32 r, bytes32 s) external;
}


contract KawaiiRelay {

    function createAuction(IKawaiiMarketplace _marketplace, address _nftAddress, uint256 _tokenId, uint256 _amount, uint256 _startingPrice, uint256 _endingPrice, uint256 _duration, address sender, uint8 v, bytes32 r, bytes32 s) external {
        _marketplace.createAuction(_nftAddress, _tokenId, _amount, _startingPrice, _endingPrice, _duration, sender, v, r, s);
    }
    ...
    function buyPack(IKawaiiSalePack kawaiiSalePack, address nftRegister, address sender, string memory pack, uint256 amount, uint8 v, bytes32 r, bytes32 s) external {
        kawaiiSalePack.buyPack(nftRegister, sender, pack, amount, v, r, s);
    }
