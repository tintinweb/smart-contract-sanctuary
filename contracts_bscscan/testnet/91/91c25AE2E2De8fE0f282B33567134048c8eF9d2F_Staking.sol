...
contract Staking is Ownable{
    ...
    IERC20 public stakingPeriod = IERC20 (0x0d27742fFfd004B1e6e6c05C99c6714Bc373416f);

    uint256 year = 365;
    

    address public rewardAddress = 0x7C7572a2227065321Ce01f444CB1A63A3caA8509;
    ...
    function stake(uint256 amount,uint256 rate) external {
        ...
        stakingPeriod.transferFrom(msg.sender, address(this), amount);
        ...
        require(totallock < stakingPeriod.balanceOf(address(this)),"Stake : staking is full");
    ...
    function withdraw(uint256 rate) external {
        ...
        stakingPeriod.transfer(rewardAddress, _stakers[msg.sender].amount[rate].mul(rewardFee).div(100));

        uint256 finalamount = _stakers[msg.sender].amount[rate] + amount;
        stakingPeriod.transfer(msg.sender, finalamount);
    ...
    function getTotoalAmount() external view returns(uint256){
       return stakingPeriod.balanceOf(address(this));
    }
    ...
    function withdrawOwner(uint256 amount) external onlyOwner{
         require(amount < stakingPeriod.balanceOf(address(this)).sub(lockedBalance[0]+lockedBalance[1] + lockedBalance[2]));
        stakingPeriod.transfer(_msgSender(), amount);
    }
