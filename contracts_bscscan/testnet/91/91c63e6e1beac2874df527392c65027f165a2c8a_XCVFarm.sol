...
contract XCVFarm is Ownable {
     ...
     */
    function _deposit(address _nftAddress, uint256 _tokenId, address _mortgagor, address _lender, uint256 _amount, uint256 _duration) public {
     ...
     */
    function _withdraw(address _nftAddress, uint256 _tokenId, address _mortgagor, address _lender, uint256 _amount) public {
        ...
        if (totalReward > 0) {
            uint256 mortgagorReward = totalReward.mul(3).div(10);
            uint256 lenderReward = totalReward.sub(mortgagorReward);
            // 如果没有超过约定的清算时间
            if (block.timestamp <= paybackTime) {
                ...
                _toBeClaimed[devaddr] = _toBeClaimed[devaddr].add(mortgagorReward);
                _toBeClaimed[lender] = _toBeClaimed[lender].add(lenderReward);
            }
        }
        // 清零该笔订单
