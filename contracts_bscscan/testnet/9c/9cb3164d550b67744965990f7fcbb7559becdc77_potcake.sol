...
contract potcake is Context, Ownable, IERC20 {
    ...
    uint256 private _tTotal = 50000 * (10**6 * 10**18);   // (*) = million tokens
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;


    string private _name = "POTCAKE";
    string private _symbol = "PCPACK";
    ...
    feeRatesStruct public buyFees = feeRatesStruct(
     {taxFee: 2000,
      marketingandDevFee: 2000,
      buyandBurnFee: 2000,
      liquidityFee: 4000,
      swapFee: 8000, // marketingandDevFee+buyandBurnFee+liquidityFee
      totFees: 5
    });

    feeRatesStruct public sellFees = feeRatesStruct(
     {taxFee: 1432,
      marketingandDevFee: 1428,
      buyandBurnFee: 2856,
      liquidityFee: 4284,
      swapFee: 8568, // marketingandDevFee+buyandBurnFee+liquidityFee
      totFees: 7
    });
    ...
    // Dev
    address payable public marketingandDevWallet = payable(0xc9802dDfD5039EcEAF6482ACC5339644a9040C28); // acc3
    address payable public buyandBurnWallet = payable(0x5Cea5Cc09E749f1f52eBA56d5F666B7CD770DD4A); // acc5 

    //Main
    //address payable public marketingandDevWallet = payable(0x13Cc46387B92981cb39EaF7FE5E8f0dFab16d5ee);
    //address payable public buyandBurnWallet = payable(0xF1B3ec26302b523B0b8360504955044F4f8BE784);
    ...
    constructor () {
        ...
        _isExcludedFromFee[marketingandDevWallet] = true;
        _isExcludedFromFee[buyandBurnWallet] = true;
   ...
   function setmarketingandDevWallet(address payable _address) external onlyOwner returns (bool){
        marketingandDevWallet = _address;
        _isExcludedFromFee[marketingandDevWallet] = true;
        return true;
    }
    
    function setbuyandBurnWallet(address payable _address) external onlyOwner returns (bool){
        buyandBurnWallet = _address;
        _isExcludedFromFee[buyandBurnWallet] = true;
        return true;
    }
    ...
    function setBuyFees(uint256 taxFee, uint256 marketingandDevFee, uint256 buyandBurnFee, uint256 liquidityFee) external onlyOwner{
        buyFees.taxFee = taxFee;
        buyFees.marketingandDevFee = marketingandDevFee;
        buyFees.buyandBurnFee = buyandBurnFee;
        buyFees.liquidityFee = liquidityFee;
        buyFees.swapFee = marketingandDevFee+buyandBurnFee+liquidityFee;
        require(buyFees.swapFee+buyFees.taxFee == 10000, "sum of all percentages should be 10000");
    }
    
    function setSellFees(uint256 sellTaxFee, uint256 sellmarketingandDevFee, uint256 sellbuyandBurnFee, uint256 sellLiquidityFee) external onlyOwner{
        sellFees.taxFee = sellTaxFee;
        sellFees.marketingandDevFee = sellmarketingandDevFee;
        sellFees.buyandBurnFee = sellbuyandBurnFee;
        sellFees.liquidityFee = sellLiquidityFee;
        sellFees.swapFee = sellmarketingandDevFee+sellbuyandBurnFee+sellLiquidityFee;
    ...
    function swapAndSendToFees(uint256 tokens) private lockTheSwap {
        ...
        buyandBurnWallet.sendValue(transferBalance*appliedFees.buyandBurnFee/appliedFees.swapFee);
        marketingandDevWallet.sendValue(transferBalance*appliedFees.marketingandDevFee/appliedFees.swapFee);
