...
contract caritaFirstSales {
    ...
    uint256 private realTimeLiquidityPotential;
    uint256 private realTimeBalanceToken;
    uint256 private realTimeBalance;
  ...
  function  checkContractBalances() public {
        ...
        realTimeBalance = address(this).balance;
        if(checkAmountValidity(realTimeBalance) == true){
            realTimeLiquidityPotential = getEstimatedTokenForBNB(address(this).balance)[0];
        }else{
            realTimeLiquidityPotential = maxApprovationAmount;
        }
