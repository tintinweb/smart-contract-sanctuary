...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accHappyPerShare;   // Accumulated FISHes per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The FISH TOKEN!
    HappyTokenV2 public happy;
    address public devAddress;
    address public feeAddress;
    uint256 private withdrawFee24Hr;
    uint256 private withdrawFee72Hr;
    uint256 private withdrawFeeDefault;

    // FISH tokens created per block.
    uint256 public happyPerBlock = 4;
    ...
    // The block number when Happy mining starts.
    ...
    function pendingHappy(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accHappyPerShare = pool.accHappyPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 happyReward = multiplier.mul(happyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accHappyPerShare = accHappyPerShare.add(happyReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accHappyPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 happyReward = multiplier.mul(happyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        happy.mint(devAddress, happyReward.div(20));
        happy.mint(address(this), happyReward);
        pool.accHappyPerShare = pool.accHappyPerShare.add(happyReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accHappyPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeHappyTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accHappyPerShare).div(1e18);
    ...
    function safeHappyTransfer(address _to, uint256 _amount) internal {
        uint256 happyBal = happy.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > happyBal) {
            transferSuccess = happy.transfer(_to, happyBal);
        } else {
            transferSuccess = happy.transfer(_to, _amount);
        }
        require(transferSuccess, "safeHappyTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _happyPerBlock) external onlyOwner {
        massUpdatePools();
        require(happyPerBlock > _happyPerBlock,"Error:Emmission: New Emission Rate need to be lower than existing");
        happyPerBlock = _happyPerBlock;
        emit UpdateEmissionRate(msg.sender, _happyPerBlock);
    }
