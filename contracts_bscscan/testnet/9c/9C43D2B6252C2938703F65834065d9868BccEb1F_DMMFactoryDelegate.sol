...
contract DMMFactoryDelegate is IDMMFactoryDelegate, ManageUser {
    ...
    function changePlatformFeePercent(address pool, uint32 platformFeePercent) override public {
        DMMPool(pool).changePlatformFeePercent(platformFeePercent);
    }
}
...
interface IDMMFactoryDelegate {
    ...
    function changePlatformFeePercent(address pool, uint32 platformFeePercent) external;
...
contract DMMPool is IDMMPool, ERC20Permit, ReentrancyGuard, VolumeTrendRecorder {
    ...
    uint32 public platformFeePercent = 2000;
    ...
    function changePlatformFeePercent(uint32 _platformFeePercent) public {
        require(msg.sender == address(factory), "DMM: FORBIDDEN");
        require(_platformFeePercent <= 10000 && _platformFeePercent >= 0, "PLATFORM FEE PERCENT: NOT VALID");
        platformFeePercent = _platformFeePercent;
    }

    function getPlatformFeePercent() public view returns (uint32) {
        return platformFeePercent;
    }

    function feeForFactory(uint256 _feeInPrecision, uint256 _decimalToken) private view returns (uint256) {
        uint256 precisionPool = 10 ** 18; //as fee formula
        uint256 precisionToken = 10 ** _decimalToken;
        return _feeInPrecision.mul(precisionToken).div(precisionPool).mul(platformFeePercent).div(BPS);
    }

    function sendFeeToFactory(uint256 _feeInPrecision, IERC20 token) private {
        (address feeTo,) = factory.getFeeConfiguration();
        uint256 _decimal = FeeFomula.decimal(address(token));
        require(_decimal >= 0, "DMM: PRECISIONS NOT VALID");
        token.safeTransfer(feeTo, feeForFactory(_feeInPrecision, _decimal));
        _sync();
    }
