...
// Dependency file: contracts/TestLotoV3Loto.sol
...
/// @notice TestLotoV3 Loto
contract TestLotoV3Loto is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(address _storage, address _team) {
        require(_storage != address(0x0) && _team != address(0x0), "TestLotoV3 Loto: address is zero");
    ...
    function placeBet(bytes22 _params) external payable virtual nonReentrant whenNotPaused {
        require(!msg.sender.isContract(), "TestLotoV3 Loto: sender cannot be a contract");
        require(tx.origin == msg.sender, "TestLotoV3 Loto: msg sender is not original user");
        require(
            msg.value >= betLimits.min && msg.value <= betLimits.max,
            string(
                abi.encodePacked(
                    "TestLotoV3 Loto: Bet amount should be greater or equal than ",
        ...
        require(bytes16(_params) != 0, "TestLotoV3 Loto: Id should not be 0");

        bytes16 id = bytes16(_params);

        require(!tickets[id], "TestLotoV3 Loto: this ticket already exists");
    ...
    function _beforeProcessBet(uint256 _amount) internal {
        ...
        _deliverFunds(storageAddress, storageFee, "TestLotoV3 Loto: failed transfer BNB to Staking Storage");

        _deliverFunds(teamAddress, teamFee, "TestLotoV3 Loto: failed transfer BNB to Team");
    }
    ...
    function _processBet(address _gamer, bytes16 _id) internal virtual {
        ...
        if (matchesCount >= _winMatches()) {
            ...
            //sent to gamer prize
            _deliverFunds(_gamer, prize, "TestLotoV3 Loto: failed transfer BNB to Gamer");
    ...
    function _withdraw(address _recipient, uint256 _amount) internal virtual {
        require(_recipient != address(0x0), "TestLotoV3 Loto: address is zero");
        require(_amount <= address(this).balance, "TestLotoV3 Loto: not enought BNB balance");
        ...
        _deliverFunds(_recipient, _amount, "TestLotoV3 Loto: Can't send BNB");
    ...
    function _withdrawToken(
        ...
        require(_recipient != address(0x0), "TestLotoV3 Loto: address is zero");
        require(_amount <= IERC20(_token).balanceOf(address(this)), "TestLotoV3 Loto: not enought token balance");
...
// Dependency file: contracts/v2/TestLotoV3LotoV2.sol


// pragma solidity 0.8.4;

// import "contracts/TestLotoV3Loto.sol";

/// @notice TestLotoV3 Loto with support partnership tokens

contract TestLotoV3LotoV2 is TestLotoV3Loto {
    using SafeERC20 for IERC20;
    using Address for address;
    ...
    constructor(address _storage, address _team) TestLotoV3Loto(_storage, _team) {}
    ...
    function setPartnershipJackpot(address _token, uint256 _jackpot) external onlyOwner {
        require(partnershipTokens[_token], "TestLotoV3 Loto: token is not active");
...
// Root file: contracts/v3/TestLotoV3LotoV3.sol


pragma solidity 0.8.4;

// import "contracts/v2/TestLotoV3LotoV2.sol";
// import "@chainlink/contracts/src/v0.8/VRFConsumerBase.sol";
// import "hardhat/console.sol";

contract TestLotoV3LotoV3 is TestLotoV3LotoV2, VRFConsumerBase {
    ...
    constructor(
        ...
        address _storage,
        address _team
    ) VRFConsumerBase(_vrfCoordinator, _linkToken) TestLotoV3LotoV2(_storage, _team) {
        keyHash = _keyHash;
        linkFee = _linkFee; // 0.1 * 10 ** 18;
    }

    function _processBet(address _gamer, bytes16 _id) internal override {
        require(LINK.balanceOf(address(this)) >= linkFee, "TestLotoV3s: Not enough LINK");

        bytes32 requestId = requestRandomness(keyHash, linkFee);

        RandomnessRequestData memory data = RandomnessRequestData({betId: _id, gamer: _gamer, token: address(0x0)});
    ...
    function _processBetWithToken(
        ...
        bytes32 _requestId = keccak256(abi.encode(1));
        RandomnessRequestData memory data = RandomnessRequestData({betId: _id, gamer: _gamer, token: _token});

        randomnessRequests[_requestId] = data;
    }

    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        RandomnessRequestData storage data = randomnessRequests[requestId];

        if (data.token != address(0x0)) {
            _processBetWithRandomnessWithToken(data, randomness);
        } else {
    ...
    function _processBetWithRandomness(RandomnessRequestData storage _data, uint256 _randomness) internal virtual {
        ...
        bytes32 blockHash = keccak256(abi.encode(_randomness));
        ...
        //if (matchesCount >= _winMatches()) {
            ...
            //sent to gamer prize
            _deliverFunds(gamer, prize, "TestLotoV3 Loto: failed transfer BNB to Gamer");
    ...
    function _processBetWithRandomnessWithToken(RandomnessRequestData storage _data, uint256 _randomness) internal virtual {
        bytes16 betId = _data.betId;

        address gamer = _data.gamer;

        address token = _data.token;
        ...
        //more than winMatches, default 6
        //if (matchesCount >= _winMatches()) {
            uint256 prize = (partnershipJackpots[token] * bet.amount) / PRECISION;

            if (prize > IERC20(token).balanceOf(address(this))) {
                prize = IERC20(token).balanceOf(address(this));
            }

            partnershipJackpots[token] -= prize;

            bet.reward = prize;

            //sent to gamer prize
            _deliverTokens(token, gamer, prize);

            emit PayoutBetWithToken(token, betId, prize, gamer);
        //}

        emit ProcessBetWithToken(token, betId, gamer, bet.amount, bet.reward, bet.bet, bet.hash);
    }
