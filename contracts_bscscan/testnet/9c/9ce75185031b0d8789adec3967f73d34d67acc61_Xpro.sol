...
contract Xpro is Context, IBEP20, Ownable {
    ...
    uint256 private _buyTaxFee       = 1; // Fee distributed to holders.
    ...
    function setOtherFee(uint256 fee, uint256 BbFee, uint256 LqFee, uint256 MktFee, uint256 RFee) external onlyOwner() {
        _otherTaxFee = fee;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        bool beli  = sender == pancakeSwapV2Pair && recipient != address(pancakeSwapV2Router);
        bool jual = recipient == pancakeSwapV2Pair;
        ...
        if (!takeFee) {
            ...
            taxFee       = _otherTaxFee;
            liquidityFee = _otherLiquidityFee;
            buybackFee   = _otherBuybackFee;
            marketingFee = _otherMarketingFee;
            researchFee  = _otherResearchFee;
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = tAmount.mul(totalLiqFee).div(10**2);
