...
contract TTT is Context, IERC20, Ownable {
    ...
    address payable public charityAddress =
        payable(0x99561fa59D66747EC9F363AD6D7d1065cA3Eade1);
    ...
    function __transferAndReflect(
        ...
        if (!isExcluded[0].fromReward) {
            ...
            /// Sell + LP + To any Excluded wallets remove the tokens from the reflection amount
            if (isExcluded[1].fromReward) {
                unchecked {
                    reflections.inCirculation -= ((amount + reflectionFee) * reflectionRate);
            ...
            /// Buy + To any non-excluded wallets
            if (!isExcluded[1].fromReward) {
                reflections.inCirculation += ((amountMinusFees + reflectionFee) * reflectionRate);
