...
library BiswapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'02952034b426dbda163bc5fd03e3b01931e93374f3a8cc42e5c2a59851ec8efe' // init code hash
            ))));
    }
...
contract BiswapRouter02 is IBiswapRouter02, Ownable {
    ...
    function swapExactTokensForTokens(
        ...
        amounts = BiswapLibrary.getAmountsOut(factory, amountIn, path);//getting amounts out based on the input amount and path
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);//getting the token paths
            (address token0,) = BiswapLibrary.sortTokens(input, output);//sort the tokens
            IBiswapPair pair = IBiswapPair(BiswapLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            { // scope to avoid stack too deep errors
            (uint reserve0, uint reserve1,) = pair.getReserves();//getting the amount input from reserves and balances 
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        );//first transfering money to the pair contract
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);//balance of account of to address before swaping 
