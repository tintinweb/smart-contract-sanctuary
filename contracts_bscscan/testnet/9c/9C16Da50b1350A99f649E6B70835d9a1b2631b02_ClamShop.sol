...
contract ClamShop is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @notice price in GEM, with 18 decimals for precision and consistency
    /// @notice derived from priceUsd, using gemOracle
    uint256 public price;
    /// @notice price in USD, with 18 decimals for precision and consistency
    uint256 public priceNextWeek;
    /// @notice percentage of gem burned every time a clam is bought
    uint256 public burnRate;
    /// @notice weekly available supply of clams
    uint256 public clamsPerWeek;
    /// @notice clams sold (minted) this week
    uint256 public mintedThisWeek;
    /// @notice timestamp of start of current week
    uint256 public lastWeeklyTimeStamp;
    /// @notice timestamp of when clams were sold out. If not sold out, it's 0
    ...
    uint256 public floorPriceUsd;
    IOracle public gemOracle;

    /// @notice percentage of max price drop per week
    uint256 public maxPriceDropPerWeek;
    /// @notice percentage of max price increase per week
    uint256 public maxPriceIncreasePerWeek;
    /// @notice price at end of week
    uint256 public closingPrice;
    /// @notice usd price at end of week
    uint256 public closingPriceUsd;
    /// @notice quantity sold at end of week
    uint256 public clamsSoldLastWeek;
    /// @notice target of clam sales, depends on clams sold last week
    uint256 public clamSalesTarget;
    /// @notice gem spent per user
    mapping(address => uint256) public gemSpent;
    ...
    event ClamUsdPriceSet(uint256 price);
    ...
    event SetGemOracle(address indexed gemOracle);
    event SetMaxPriceDropPerWeek(uint256 indexed maxPriceDropPerWeek);
    event SetMaxPriceIncreasePerWeek(uint256 indexed maxPriceIncreasePerWeek);
    ...
    /**
     * @dev get current Clam price in GEM
     */
    function getUpdatedPrice() public returns (uint256) {
        if (clamSalesTarget == 0) clamSalesTarget = 200;

        uint256 currentTimeStamp = block.timestamp;
        uint256 secondsSinceWeekStart = currentTimeStamp.sub(lastWeeklyTimeStamp);

        uint256 targetClamsSold = clamSalesTarget.mul(secondsSinceWeekStart).div(1 weeks);

        // if clams are soldout, return latest price
        if (soldOutTimeStamp != 0 || targetClamsSold == 0) return price;

        // adjust to prevent 0-division errors
        uint256 clamsSold = mintedThisWeek == 0 ? 1 : mintedThisWeek;
        // percentage of clams sold against the target at this moment
        uint256 percentageSold = clamsSold.mul(100).div(targetClamsSold);
        // percentage >= 100 means more clams sold than target
        bool targetAchieved = percentageSold >= 100;
        // actual percentage diff, regardless percentageSold above or under 100
        uint256 percentageDiff = targetAchieved ? percentageSold.sub(100) : uint256(100).sub(percentageSold);
        // price changes on the first day of week are less impactful
        uint256 percentageDiffCorrected = percentageDiff
            .mul(MathUpgradeable.min(secondsSinceWeekStart.mul(700).div(1 weeks), 100))
            .div(100);
        // increase/decrease price based on percentage sold
        uint256 uncappedPrice = targetAchieved
            ? priceUsd.add(priceUsd.mul(percentageDiffCorrected).div(100))
            : priceUsd.sub(priceUsd.mul(percentageDiffCorrected).div(100));

        uint256 maxPrice = priceUsd.add(priceUsd.mul(maxPriceIncreasePerWeek).div(100));
        uint256 minPrice = priceUsd.sub(priceUsd.mul(maxPriceDropPerWeek).div(100));

        // capped by min/max price increase
        uint256 cappedPrice = uncappedPrice > priceUsd
            ? MathUpgradeable.min(uncappedPrice, maxPrice)
            : MathUpgradeable.max(uncappedPrice, minPrice);

        // capped downside by floorPrice
        cappedPrice = MathUpgradeable.max(cappedPrice, floorPriceUsd);

        uint256 gemPrice = getGemPriceUsd();
        price = cappedPrice.mul(1 ether).div(gemPrice);
        ...
        if (secondsSinceWeekStart >= 1 weeks) {
            ...
            // set clamSalesTarget, cannot increase more than maxPriceIncrease, nor decrease further than maxPriceDrop
            // if it is in between, it's set to mintedThisWeek
            if (cappedPrice == floorPriceUsd || cappedPrice == minPrice) {
                clamSalesTarget = MathUpgradeable.max(
                    mintedThisWeek,
                    clamSalesTarget.sub(clamSalesTarget.mul(maxPriceDropPerWeek).div(100))
                );
            } else if (cappedPrice == maxPrice) {
                clamSalesTarget = MathUpgradeable.min(
                    mintedThisWeek,
                    clamSalesTarget.add(clamSalesTarget.mul(maxPriceIncreasePerWeek).div(100))
                );
            } else {
                clamSalesTarget = mintedThisWeek;
            }
            ...
            closingPrice = price;
            closingPriceUsd = cappedPrice;
            clamsSoldLastWeek = mintedThisWeek;
            mintedThisWeek = 0;
        }
    ...
    /// @notice get price of gem from oracle
    /// @return gem price in usd, 18 decimals
    function getGemPriceUsd() public view returns (uint256) {
        require(address(gemOracle) != address(0), "ClamShop: GemOracle not set");

        return gemOracle.consult();
    }
    ...
    function _finishTransaction(uint256 currentPrice) internal {
        gemSpent[msg.sender] += currentPrice;
    ...
    /// @notice sets price in USD, with 18 decimals. Price in GEM will be derived from this variable
    function setPriceUsd(uint256 _priceInUsd) external onlyOwner {
        priceUsd = _priceInUsd;

        emit ClamUsdPriceSet(_priceInUsd);
    }

    /// @notice sets floor price in USD, with 18 decimals.
    function setFloorPriceUsd(uint256 _floorPriceInUsd) external onlyOwner {
        floorPriceUsd = _floorPriceInUsd;

        emit ClamUsdPriceSet(_floorPriceInUsd);
    }
    ...
    function setGemOracle(address _gemOracle) external onlyOwner {
        gemOracle = IOracle(_gemOracle);

        emit SetGemOracle(_gemOracle);
    }

    /// @notice set percentage of max price drop per week
    /// @param _maxPriceDropPerWeek percentage absolute. 50% = 50
    function setMaxPriceDropPerWeek(uint256 _maxPriceDropPerWeek) external onlyOwner {
        maxPriceDropPerWeek = _maxPriceDropPerWeek;

        emit SetMaxPriceDropPerWeek(_maxPriceDropPerWeek);
    }

    /// @notice set percentage of max price increase per week
    /// @param _maxPriceIncreasePerWeek percentage absolute. 100% = 100
    function setMaxPriceIncreasePerWeek(uint256 _maxPriceIncreasePerWeek) external onlyOwner {
        maxPriceIncreasePerWeek = _maxPriceIncreasePerWeek;

        emit SetMaxPriceIncreasePerWeek(_maxPriceIncreasePerWeek);
    }
}
...
interface IClam is IERC721EnumerableUpgradeable {
    struct ClamInfo {
        ...
        uint256 gemBoost; // deprecated
        uint256 pearlBoostM; // multiplier for gem boost for every pearl that is born from this clam. In million format because of decimals, devide by 1_000_000
    }
    ...
    function getTraits(uint256) external view returns (IDNADecoder.Traits memory);

    function burn(uint256) external;

    function calculatePearlBoost(
        uint256 size,
        uint256 lifespan,
        uint256 rarityValue
    ...
    event SetBoostFactor(uint256 _boostFactor);
    event SetClamExchange(address _clamExchange);
}
