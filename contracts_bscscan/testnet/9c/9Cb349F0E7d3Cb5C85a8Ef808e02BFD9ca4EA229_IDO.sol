...
contract IDO is AccessControl, ReentrancyGuard {
    ...
    function createCampaign(
        ...
        string memory strError = checkDistributionInfo(distributionToken);
    ...
    function joinToCampaign(
        ...
        userInfo.rewardTokens += calcTotalReward(amount, campaignID);

        campaignInfos[campaignID].receivedTokens += amount;

        require(campaignInfo.receivedTokens <= campaign.maxGoal, "max Goal");

        emit JoinToCampaign(campaignID, amount, msg.sender, block.timestamp);
    }
     ...
     */
    function calcTotalReward(uint256 amount, uint256 campaignID)
        internal
        view
        returns (uint256)
    {
        CampaignInfo memory campaignInfo = campaignInfos[campaignID];
        uint256 reward = ((amount * 1e18) /
            campaignMainData[campaignID].tokenRate);
        if (
     ...
     */
    function checkDistributionInfo(DistributionInfo[] memory distributionToken)
        internal
        view
        returns (string memory)
    {
        ...
        for (uint256 i; i < distributionToken.length; i++) {
            if (i == 0) {
                if (distributionToken[i].unlockTime < block.timestamp) {
                ...
                if (
                    ...
                    return "unlockTime[i]<unlockTime[i-1]";
                }
            }
