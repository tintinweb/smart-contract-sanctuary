...
abstract contract ReentrancyGuard {
    ...
    // amount. Since refunds are capped to a perallanpoetage of the total
...
// AllanPoe Token
contract AllanPoe is BEP20{
    // Burn address

    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    // DEV address
    address public constant DEV_ADDRESS = 0x77eFf2484DAf77C5B07a38bdB7b3A81374347dE1;
    // Fee address
    address public constant FEE_ADDRESS = 0x77eFf2484DAf77C5B07a38bdB7b3A81374347dE1;
    ...
    // Constructs the AllanPoe contract.
    constructor() public BEP20('AllanPoe Centaurus.Network', 'AXE') {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        bool _isNonTallanpoedTransfer =  isExcludedFromTax(recipient) || isExcludedFromTax(sender);
        if (_isNonTallanpoedTransfer)
...
// MasterChefAllanPoeDistr is the master of AllanPoe (AllanPoe) distribution. He can make AllanPoe and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. Initially the ownership is
// transferred to TimeLock.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract MasterChefAllanPoe is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of AllanPoes
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accAllanPoePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accAllanPoePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. AllanPoes to distribute per block.
        uint256 lastRewardBlock; // Last block number that AllanPoes distribution occurs.
        uint256 accAllanPoePerShare; // Accumulated AllanPoes per share, times 1e12. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
    }

    // The AllanPoe TOKEN!
    AllanPoe public allanpoe;
    
    // Dev address.
    address public devAddr = 0x77eFf2484DAf77C5B07a38bdB7b3A81374347dE1;
    
    // Deposit Fee address
    address public feeAddress = 0x77eFf2484DAf77C5B07a38bdB7b3A81374347dE1;
    
    // AllanPoe tokens created per block.
    uint256 public allanpoePerBlock;

    // Initial emission rate: 1 AllanPoe per block.
    uint256 public constant initialEmissioRate = 1000 finney;
    
    // Minimum emission rate: 0 AllanPoe per block.
    uint256 public constant minEmissionRate = 0 finney;
    
    // Final emission rate: 0 AllanPoe per block.
    ...
    // current Bock of AllanPoe Minting Start.
    uint256 public startBlock;

    // The block number when AllanPoe mining starts.
    ...
    constructor(
        ...
        allanpoe = _allanpoe;
        allanpoePerBlock = initialEmissioRate;
    ...
    // View function to see pending AllanPoes on frontend.
    function pendingAllanPoe(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accAllanPoePerShare = pool.accAllanPoePerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 allanpoeReward = multiplier.mul(allanpoePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accAllanPoePerShare = accAllanPoePerShare.add(allanpoeReward.mul(1e12).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accAllanPoePerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 allanpoeReward = multiplier.mul(allanpoePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        uint256 feeAmount = allanpoeReward.mul(10).div(1000);
        
        allanpoe.mint(feeAddress, feeAmount);
        allanpoe.mint(address(this), allanpoeReward);
        
        pool.accAllanPoePerShare = pool.accAllanPoePerShare.add(allanpoeReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        payOrLockupPendingAllanPoe(_pid);
        ...
        user.rewardDebt = user.amount.mul(pool.accAllanPoePerShare).div(1e12);
        updateEmissionRate();
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
	    ...
	    // Pay or lockup pending AllanPoes.
    function payOrLockupPendingAllanPoe(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];

        uint256 pending = user.amount.mul(pool.accAllanPoePerShare).div(1e12).sub(user.rewardDebt);
        ...
        if (block.number >= startBlockHarvest && block.number < lastHarvestPeriod1) {
            if (pending > 0 || user.rewardLockedUp > 0) {        
                ...
                // send rewards
                safeAllanPoeTransfer(msg.sender, pending.mul(25).div(100));
            }
        }
        // 2. Harvest Period
            ...
            if (pending > 0 || user.rewardLockedUp > 0) {        
               // reset lockup
                ...
                // send rewards
                safeAllanPoeTransfer(msg.sender, pending.mul(50).div(100));
            }
        }
        ...
        // 3. Harvest Period
            ...
            if (pending > 0 || user.rewardLockedUp > 0) {        
               // reset lockup
                ...
                // send rewards
                safeAllanPoeTransfer(msg.sender, pending.mul(75).div(100));
            }
        }
        ...
        // 4. Harvest Period
            ...
            if (pending > 0 || user.rewardLockedUp > 0) {        
                ...
                // send rewards
                safeAllanPoeTransfer(msg.sender,  totalRewards);
            }
        }
 ...
 // Safe AllanPoe transfer function, just in case if rounding error causes pool to not have enough AllanPoes.
    function safeAllanPoeTransfer(address _to, uint256 _amount) internal {
        uint256 allanpoeBal = allanpoe.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > allanpoeBal) {
            transferSuccess = allanpoe.transfer(_to, allanpoeBal);
        } else {
            transferSuccess = allanpoe.transfer(_to, _amount);
        }
        require(transferSuccess, "safeAllanPoeTransfer: Transfer failed");
    }
   ...
   function updateEmissionRate() public {
        ...
        if ((allanpoePerBlock <= minEmissionRate)){return;}
        ...
        uint256 newEmissionRate = allanpoePerBlock;
        ...
        uint256 previousEmissionRate = allanpoePerBlock;
        allanpoePerBlock = newEmissionRate;
