...
contract ZRXNFT_FACTORY is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    function mintContract(uint256 numContracts) public payable {
        uint payment = 0;
        payment = calculatePrice().mul(numContracts);
        require(totalSupply() < MAX_MINT, "Sale has already ended");
        ...
        require(payment != 0, "Exceeds MAX_MINT limit");
        // require(msg.value >= calculatePrice().mul(numContracts), "BNB value sent is below the price");
        
        payable(msg.sender).transfer(payment);
        // this.safeTransferFrom(address(msg.sender), address(this), payment, "");
