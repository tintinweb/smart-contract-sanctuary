...
contract SeederVerseBridge is OwnableUpgradeable, ERC721Holder, OracleClientV2 {
    ...
    Counters.Counter public withdrawTicketCounter;
    Counters.Counter public downloadTicketCounter;

    mapping(uint256 => address) public tokenIdToDepositor;
    mapping(uint256 => address) public ticketIdToWithdrawer;
    mapping(address => uint256) public downloaderToAmount;
    mapping(uint256 => address) public ticketIdToDownloader;
    mapping(address => uint256) public downloaderToTickerId;

    event SeedmonDeposited(uint256 tokenId, address depositor);
    event WithdrawSeedmonRequested(uint256 ticketId, uint256 tokenId, address withdrawer);
    event SeedmonWithdrawn(uint256 ticketId, uint256 tokenId, address withdrawer);
    event SeedUploaded(uint256 amount, address uploader);
    event DownloadSeedRequested(uint256 ticketId, uint256 amount, address downloader);
    event SeedDownloaded(uint256 ticketId, uint256 amount, address downloader);
    event SeedDownloadCancelled(uint256 ticketId, address downloader);
    ...
    function deposit(uint256 tokenId) external {
        ...
        emit SeedmonDeposited(tokenId, msg.sender);
    }

    function uploadSeed(uint256 amount) external {
        
        seed.transferFrom(msg.sender, address(this), amount);
        seed.burn(amount);

        emit SeedUploaded(amount, msg.sender);
    }

    function createWithdrawRequest(uint256 tokenId) external payable {
        address depositor = tokenIdToDepositor[tokenId];
        require(msg.sender == depositor, "SeederVerse: Unauthorized");

        withdrawTicketCounter.increment();
        ticketIdToWithdrawer[withdrawTicketCounter.current()] = msg.sender;
        requestWithdrawSeedmon(withdrawTicketCounter.current(), tokenId, msg.value);

        emit WithdrawSeedmonRequested(withdrawTicketCounter.current(), tokenId, msg.sender);
    }

    function createDownloadRequest(uint256 amount) external payable {

        require(amount > 0, "SeederVerse: Invalid input amount");
        require(downloaderToAmount[msg.sender] == 0, "SeederVerse: Duplicated download request");

        downloadTicketCounter.increment();
        uint256 ticketId = downloadTicketCounter.current();
        downloaderToAmount[msg.sender] = amount;
        ticketIdToDownloader[ticketId] = msg.sender;
        downloaderToTickerId[msg.sender] = ticketId;
        requestDownloadSeed(ticketId, msg.sender, msg.value);

        emit DownloadSeedRequested(ticketId, amount, msg.sender);
    }

    function createCancelDownloadRequest() external {
        require(!msg.sender.isContract(), "SeederVerse: not allow call by contract address");
        uint256 ticketID = downloaderToTickerId[msg.sender];
        require(ticketID != 0, "SeederVerse: No ticket for refund");

        requestCancelDownloadSeed(ticketID);
    }

    function processWithdraw(uint256 ticketId, uint256 tokenId, uint256 exp) internal override {
        
        (, SeedmonAttribute.UniqueProfile memory uniqueProfile, SeedmonAttribute.Stat memory bonusStat) 
            = seedmon.tokenInfo(tokenId);

        uniqueProfile.exp = exp;
        seedmon.updateSeedmon(tokenId, uniqueProfile, bonusStat);
        address withdrawer = ticketIdToWithdrawer[ticketId];
        seedmon.safeTransferFrom(address(this), withdrawer, tokenId);
        
        delete tokenIdToDepositor[tokenId];
        delete ticketIdToWithdrawer[ticketId];

        emit SeedmonWithdrawn(ticketId, tokenId, withdrawer);
    }

    function processDownload(uint256 requestId, uint256 tokenBalance) internal override {

        address downloader = ticketIdToDownloader[requestId];
        require(downloader != address(0), "SeederVerse: Invalid request ID");
        uint256 requestAmount = downloaderToAmount[downloader];
        

        require(tokenBalance >= requestAmount, "SeederVerse: Insufficient Seed balance");
        
        seed.mint(downloader, requestAmount);

        delete ticketIdToDownloader[requestId];
        delete downloaderToAmount[downloader];
        delete downloaderToTickerId[downloader];

        emit SeedDownloaded(requestId, requestAmount, downloader);
    }

    function processCancel(uint256 requestId, uint256 providedGas) internal override {
        address downloader = ticketIdToDownloader[requestId];
        require(downloader != address(0), "VendingMachine: Ticket is not valid");

        if (providedGas > 0) {
            payable(downloader).transfer(providedGas);
        }
        ...
        emit SeedDownloadCancelled(requestId, downloader);
    }
}
...
import "../gacha/OracleClient.sol";

abstract contract OracleClientV2 is IClientV2 {
    ...
    function requestWithdrawSeedmon(uint256 requestId, uint256 tokenId, uint256 providedGas) internal {
        require(requestId > 0, "OracleClient: Invalid request Id for requesting withdraw seedmon");
        oracle.requestWithdrawSeedmon{value: providedGas} (requestId, tokenId);
    }

    function requestDownloadSeed(uint256 requestId, address downloader, uint256 providedGas) internal {
        require(requestId > 0, "OracleClient: Invalid request Id for requesting download seed token");
        oracle.requestDownloadSeed{value: providedGas} (requestId, downloader);
    }

    function requestCancelDownloadSeed(uint256 requestId) internal {
        require(requestId > 0, "OracleClient: Invalid request Id for requesting cancel download seed token");
        ...
        oracle.cancelDownloadSeed(requestId);
    }


    // =========== Process ====================
    function processWithdraw(uint256 requestId, uint256 tokenId, uint256 exp) internal virtual {
    }

    function processDownload(uint256 requestId, uint256 tokenBalance) internal virtual {
    }

    function processCancel(uint256 requestId, uint256 providedGas) internal virtual {
    }

    // ============ Prophet method ============
    function onWithdrawSeedmonReceived(uint256 requestId, uint256 tokenId, uint256 exp) external override onlyOracle {
        processWithdraw(requestId, tokenId, exp);
    }

    function onDownloadSeedReceived(uint256 requestId, uint256 tokenBalance) external override onlyOracle returns (bool isCompleted) {
        processDownload(requestId, tokenBalance);
        isCompleted = true;
    }

    function onCancelDownloadSeedReceived(uint256 requestId) external payable override onlyOracle {
        processCancel(requestId, msg.value);
    }
...
interface IClientV2 {
    function onWithdrawSeedmonReceived(uint256 requestId, uint256 tokenId, uint256 exp) external;
    function onDownloadSeedReceived(uint256 requestId, uint256 tokenBalance) external returns (bool isCompleted);
...
abstract contract OracleClient is IClient {
    ...
    // ============ Prophet method ============
    function onRandomNumberReceived(uint256 requestId, uint256[] memory randomNumbers) external override onlyOracle returns (bool isCompleted) {
        processRandomNumberReceived(requestId, randomNumbers);
        isCompleted = true;
    }
    function onRandomSeedmonReceived(uint256 requestId, bytes32 seedmon, uint256[] memory stats) external override onlyOracle returns (bool isCompleted) {
        processCreation(requestId, seedmon, stats);
        isCompleted = true;
    }
    function onRefund(uint256 requestId) external payable override onlyOracle { // test onlyOracle
...
interface IOracle {
    ...
    function requestWithdrawSeedmon(uint256 requestId, uint256 tokenId) external payable;
    function requestDownloadSeed(uint256 requestId, address downloader) external payable;
