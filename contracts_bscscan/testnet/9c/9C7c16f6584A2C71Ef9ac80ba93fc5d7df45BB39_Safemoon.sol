...
contract Safemoon is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade {
        require(!_upgraded, "Panda: Already upgraded");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "Panda: Invalid tier index");
        _;
    }
    ...
    function __Safemoon_v2_init_unchained(address _router) internal initializer {
        ...
        numTokensSellToAddToLiquidity = 100 * 10**6 * 10**9;
    ...
    function __Safemoon_tiers_init() internal initializer {
        _defaultFees = _addTier(0, 300, 0, 300, 0, address(0), address(0));
    ...
    preventBlacklisted(_account, "Panda: Selected account is in blacklist")
    {
        require(_account != address(0), "Panda: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner() {
        require(_account != address(0), "Panda: Invalid address");
        require(_accountsTier[_account] > 0, "Panda: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "Panda: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_ecoSystem != address(0), "Panda: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_owner != address(0), "Panda: Address Zero is not allowed");
    ...
    private
    preventBlacklisted(owner, "Panda: Owner address is blacklisted")
    preventBlacklisted(spender, "Panda: Spender address is blacklisted")
    {
    ...
    private
    preventBlacklisted(_msgSender(), "Panda: Address is blacklisted")
    preventBlacklisted(from, "Panda: From address is blacklisted")
    preventBlacklisted(to, "Panda: To address is blacklisted")
    {
    ...
    function migrate(address account, uint256 amount) external override {
        require(migration != address(0), "Panda: Migration is not started");
        require(_msgSender() == migration, "Panda: Not Allowed");
