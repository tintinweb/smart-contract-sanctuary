...
contract Windmill_Fund is Adressable, Math, Payable, AccessControlEnumerable, With_DAORole, With_UpdaterRole, With_TradeManagerRole,
    ...
    constructor() {
        ...
        withdrawFees.numerator = 100;
        withdrawFees.denominator = 10000;
    }
    ...
    function setWithdrawFees(uint256 numerator, uint256 denominator) external onlyRole(DAO_ROLE){
        require(denominator>0, "Windmill_Fund: Denominator cannot be null");
        withdrawFees.numerator = numerator;
        withdrawFees.denominator = denominator;
    }
    ...
    function getFundBUSD() public view returns (uint256){
        ...
        for(uint256 i=0; i<l; i++){
            ...
            nbBUSD += data.trade.estimateBUSDBalance();
        }
    ...
    function buyPWR(uint256 amountBUSD) external{
        ...
        (uint256 nbFreePwr, uint256 nbLockedPwr, uint256 flatLockPercentNumerator) = _getDataPWRLock(msg.sender);
    ...
    function beforePWRTransfer(address from, address to, uint256 amount) external onlyRole(PWR_TOKEN_ROLE){
        (uint256 nbFreePwrFrom, uint256 nbLockedPwrFrom, uint256 flatLockPercentNumeratorFrom) = _getDataPWRLock(from);
        (uint256 nbFreePwrTo, uint256 nbLockedPwrTo, uint256 flatLockPercentNumeratorTo) = _getDataPWRLock(to);
    ...
    function getFreePWR(address addr) public view returns(uint256){
        (uint256 nbFreePWR, uint256 nbLockedPwr, uint256 flatLockPercentNumerator) = _getDataPWRLock(addr);
    ...
    function sellPWR(uint256 amountPWR) external{
        ...
        uint256 feesPWR = (amountPWR * withdrawFees.numerator) / (withdrawFees.denominator);
    ...
    function _updateLockedPWR(address addr, uint256 prevNbLockedPWR, uint256 currNbPWR, uint256 flatLockPercentNumerator) internal{
        ...
        if (currNbPWR == 0){
            ...
            uint256 currLockPercentNumerator = (prevNbLockedPWR * 100000) / currNbPWR;
    ...
    function _getDataPWRLock(address addr) internal view returns (uint256, uint256, uint256){
        ...
        for(uint256 i=0; i<l; i++){
            ...
            if (data.blockStart >= dataLock.startBlock || dataLock.factor.denominator == 0){
                lockPercentNumerator += data.percentPWRLockedNumerator;
            }else{
                lockPercentNumerator += (data.percentPWRLockedNumerator * dataLock.factor.numerator) / dataLock.factor.denominator;
            }
        }
        
        uint256 nbLockedPWR = nbPWRUser * lockPercentNumerator / 100000;
        uint256 nbFreePWR = nbPWRUser - nbLockedPWR;
        
        return (nbFreePWR, nbLockedPWR, flatLockPercentNumerator);
    }
}