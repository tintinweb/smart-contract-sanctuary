...
contract PixelFarm is Ownable, ReentrancyGuard {
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && totalBoostedShares != 0) {
            ...
            accPIXELPerShare = accPIXELPerShare.add(
                PIXELReward.mul(1e12).div(totalBoostedShares)
            );
        }
        if ((block.timestamp.sub(user.lastActionTimeStamp)) < penaltyBase) {
            return
                (
                    user
                        .boostedShares
                        .mul(accPIXELPerShare)
                        .div(1e12)
                        .sub(user.rewardDebt)
                        .mul(block.timestamp.sub(user.lastActionTimeStamp))
                )
                    .div(penaltyBase);
        } else {
            return
                user.boostedShares.mul(accPIXELPerShare).div(1e12).sub(
                    user.rewardDebt
                );
        }
    }
        ...
        returns (uint256)
    {
        ...
        if ((block.timestamp.sub(user.lastActionTimeStamp)) < penaltyBase) {
            return
                (TEN_THOUSAND).sub(
                    (
                        TWENTY_FIVE_HUNDRED.mul(FOUR.sub(user.nftId1)).mul(
                            (block.timestamp.sub(user.lastActionTimeStamp))
                        )
                    )
                        .div(penaltyBase)
                );
    ...
    function updatePool(uint16 _pid) public {
        ...
        PIXELToken(PIXEL).mint(
            owner(),
            PIXELReward.mul(ownerPIXELReward).div(1000)
        );
        PIXELToken(PIXEL).mint(address(this), PIXELReward);

        pool.accPIXELPerShare = pool.accPIXELPerShare.add(
            PIXELReward.mul(1e12).div(pool.totalBoostedShares)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint16 _pid, uint256 _wantAmt) external nonReentrant {
        ...
        if (user.shares > 0) {
            ...
            .sub(user.rewardDebt);
            if ((block.timestamp.sub(user.lastActionTimeStamp)) < penaltyBase) {
                uint256 pendingAfterPenalty = (
                    pending.sub(
                        currentPenalty(_pid, msg.sender).mul(pending).div(
                            TEN_THOUSAND
                        )
                    )
                );
                ...
                if (pending > 0) {
                    ...
                    safePIXELTransfer(
                        burnAddress,
                        pending.sub(pendingAfterPenalty)
                    );
                }
            } else if (pending > 0) {
        ...
        user.rewardDebt = user.boostedShares.mul(pool.accPIXELPerShare).div(
            1e12
        );
    ...
    function _updateBoostedShares(uint16 _pid) private {
        ...
        if (user.nftId2 > 0) {
            ...
            user.boostedShares = user.boostedShares.add(
                user.boostedShares.mul(bonus).div(10000)
            );
        }

        pool.totalBoostedShares = pool
        .totalBoostedShares
        .add(user.boostedShares)
        .sub(oldBoostedShares);
    }
