...
contract vTokenUnwinder is BaseDepositUnwinder {
        ...
        returns (uint256 underlyingAmnt)
    {
        ...
        require(IVToken(token).balanceOf(address(this)) > 0, "no token is held by unwinder");
        ...
        uint256 tokenAmnt = vToken.balanceOf(address(this));
        ...
        // redeem all vTokens in this contract
        require(vToken.redeem(vToken.balanceOf(address(this))) == 0, "vTokenUnwind: Fail to withdraw from Venus");
        // check if all vtokens are minted
        require(vToken.balanceOf(address(this)) == 0, "vTokenUnwind: Failed to withdraw all tokens from Venus");

        underlyingAmnt = IERC20MetadataUpgradeable(underlying).balanceOf(address(this)) - underlyingAmntBefore;
        require(underlyingAmnt > 0, "vTokenUnwind: no underlying has been redeemed");
    ...
    function exchangeRate(address token) external view override returns (uint256) {
        require(underlyingToken[token] != address(0), "token is not supported");
        return IVToken(token).exchangeRateStored();
    }
}
