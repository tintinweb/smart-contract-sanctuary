...
/**
 ...
 ____   ______ __ __ __ __               _______         __
|_   | |   __ \__|  |  |__|.-----.-----.|_     _|.-----.|  |--.-----.-----.
 _|  |_|   __ <  |  |  |  ||  _  |     |  |   |  |  _  ||    <|  -__|     |
|______|______/__|__|__|__||_____|__|__|  |___|  |_____||__|__|_____|__|__|


   # 1BILLIONTOKEN features:
   - 2% fee auto add to the liquidity pool to locked forever when selling
   - 2% to liquidity pool
   - 8% fee auto distribute to all holders

   # Off to the moon
   # Founders: Noel Serrato, Man Bui, Michael Navarro
   Yes we doxxed ourselves.
...
contract BillionToken is Context, IERC20, Ownable {
    ...
    address public _devWallet = 0x305e7fD631f47D37C75E2e0eEAe754017f0A1C0c;
    ...
    string private _name = "1BillionToken";
    string private _symbol = "1BILL";
    ...
    // Limit selling per day
    uint256 public _maxSellTransactionAmount = 1000 * 10**9; // can't sell more than this per transaction
    uint256 private _cooldownPeriod = 300; // number of seconds for timeout
    uint256 public _maxCumulativeSell = 2000 * 10**9; // max to be sold per day
    ...
    constructor () public {
        _rOwned[_devWallet] = (_rTotal.div(100)).mul(43);  // 43% Supply for development
        _rOwned[_msgSender()] = _rTotal.sub(_rOwned[_devWallet]);
    ...
    function setMaxCumulativeSell(uint256 maxCumulativeSell) external onlyOwner() {
    ...
    function _reflectFee(uint256 rFee, uint256 tFee, uint256 tBurn) private {
        ...
        _rOwned[_burnWallet] = _rOwned[_burnWallet].add(tBurn.mul(_getRate()));
