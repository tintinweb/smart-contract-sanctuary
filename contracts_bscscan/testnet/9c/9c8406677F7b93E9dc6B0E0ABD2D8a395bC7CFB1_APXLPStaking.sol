...
import "./APXBUSDLPToken.sol";

contract APXLPStaking is APXBUSDLPToken, Ownable, Pausable, ReentrancyGuard {
    ...
    function staked(address _user) public view returns (uint256) {
        UserInfo storage user = userInfo[_user];
        return user.amount;
    }

    // View function to see pending APXs on frontend.
    function pendingApx(address _user) public view returns (uint256) {
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[_user];
    ...
    function deposit(uint256 _amount) public nonReentrant checkStart whenNotPaused oncePerBlock(msg.sender) {
    ...
    // Withdraw LP tokens
    function withdraw(uint256 _amount) public nonReentrant whenNotPaused oncePerBlock(msg.sender) {
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[msg.sender];

        require(_amount > 0, "Invalid withdraw amount");
        require(user.amount >= _amount, "Insufficient balance");
        require(balanceOf(msg.sender) >= _amount, "Insufficient APX LP");
    ...
    function claim() public nonReentrant whenNotPaused oncePerBlock(msg.sender) {
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[msg.sender];
    ...
    function safeApxTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > balance) {
            rewardsToken.transfer(_to, balance);
        } else {
...
contract APXBUSDLPToken is ERC20 {

    constructor() ERC20('APX-BUSD LP Token', 'APXLPToken')  {}
