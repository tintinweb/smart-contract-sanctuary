...
contract BoxMarketplace is Ownable, Pausable, ERC1155Holder, Initializable {
    ...
    event Buy(uint256 indexed _saleId, address indexed _buyer, uint256 _quantity, address _seller);
    event UpdateSaleInfo(uint256 indexed _saleId, address indexed _seller, uint256 _price, uint256 _quantity, uint256 _status);
    event UpdateSalePrice(uint256 indexed _saleId, address indexed _seller, uint256 indexed _price);
    event CancelListed(uint256 indexed _itemId, address indexed _receiver);
    ...
    function list(uint256 _mysteryBoxId, uint256 _quantity, uint256 _price, uint256 _quoteErc20Key) external whenNotPaused returns (uint256 _saleId){
        ...
        require(mysteryBoxNFT.isApprovedForAll(_msgSender(), address(this)) == true, 'Not Approve Box For Market Yet');
    ...
    function buy(uint256 _saleId, uint256 _quantity) external payable whenNotPaused {
        ...
        emit Buy(_saleId, _msgSender(), _quantity, info.seller);
    }
    ...
    function updateSaleInfo(uint256 _saleId, uint256 _price, uint256 _quantity) external whenNotPaused returns (bool) {
        SaleInfo storage info = saleInfos[_saleId];
        require(info.seller == _msgSender(), 'Not the owner of this item');
        require(info.status == 1, 'Sale Is Unavailable');
        ...
        emit UpdateSaleInfo(_saleId, _msgSender(), _price, _quantity, info.status);
        return true;
    }
