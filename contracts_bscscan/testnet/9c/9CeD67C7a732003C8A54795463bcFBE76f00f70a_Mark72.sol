...
//        emit NoLongerForSale(_nftContract, _nftId, offer.seller, offer.startPrice, bid.bidder, bid.amount);
    ...
    function handleBuy(IYoloERC721 _nftContract, uint _nftId, uint _serviceFee, uint _royaltyFee, address _from, address _to) internal {
        IYoloERC721 NftContract = IYoloERC721(_nftContract);
    ...
    returns (address[] memory contracts, uint[] memory ids) {
        ...
        OfferPreview[] memory preview = getOfferPreview(totalL, l, contractList);
    ...
    returns (address[] memory contracts, uint[] memory ids) {
        ...
        IYoloERC721[] memory contractList = new IYoloERC721[](l);
    ...
    function getOfferPreview(uint totalL, uint l, IYoloERC721[] memory contractList) internal view returns (OfferPreview[] memory){
    ...
    function getResult(uint _start, uint _end, uint _totalL, uint _returnL, OfferPreview[] memory _preview, bool _highToLow)
    internal view returns (address[] memory contracts, uint[] memory ids){
        address[] memory addresses = new address[](_returnL);
        uint[] memory ids = new uint[](_returnL);
        uint cl;
        if (_highToLow) {
            uint index;
