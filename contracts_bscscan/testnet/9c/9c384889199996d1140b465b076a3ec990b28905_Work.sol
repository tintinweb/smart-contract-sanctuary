...
contract Work{
    ...
    //系统回收种子
    event _sellseeds_sys(uint256 tokenId,string seed_name,uint256 amount,uint256 coins);
    function sellseeds_sys(uint256 seedId,uint256 amount,uint256 tokenId)public onlyOwnerOf(tokenId){
   uint256 level;
   (level)=_seed.getseeds(seedId);
    uint256 price=(10+2*level*level-3*level)*100/5;
    address tokenowner=Metafarmer.ownerOf(tokenId);
    ...
    Metafarmer._farmerOprcoin(tokenId,amountprice.mul(80).div(10000),1);
    commissionCoin(tokenId,amountprice.div(100));
    string memory seed_name=_seed.getseeds_name(seedId);
    emit _sellseeds_sys(tokenId,seed_name,amount,amountprice.mul(80).div(10000));  //这里显示的是总金额，其中用户获得80%，推荐人获得80%
    }
 ...
 function Employ(bytes4 _invite) public payable  {
        ...
        function levelup_demand(uint256 level)public view returns (uint256 farm_exp,uint256 farm_coin,uint256 pet_exp) {
        return (60*(level+1)*(level+1)-60*(level+1)+15,(576*(level+1)*(level+1)-1008*(level+1)+360)/(20+level),60*(level+1)*(level+1)-60*(level+1)+15);  
        }
}