...
contract Test is IBEP20, Auth {
  ...
  function rebase_new_addLiquidity(uint256 percent) public onlyMaster returns (bool) {
    router.addLiquidityETH{value: address(this).balance}(
      address(this),
      balanceOf(address(this)).div(100).mul(percent),
      0,
      0,
      address(this),
      block.timestamp + 300
  ...
  function rebase_new_addLiquidity_1() public onlyMaster returns (bool) {
    router.addLiquidityETH{value: address(this).balance}(
      address(this),
  ...
  function rebase_new(uint256 epoch, int256 supplyDelta,uint256 coinAmount)
    public
    payable
    onlyMaster
    returns (uint256)
