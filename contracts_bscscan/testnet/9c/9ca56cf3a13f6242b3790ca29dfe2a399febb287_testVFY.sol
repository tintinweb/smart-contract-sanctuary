...
// File: contracts/testVFYDividendPayingToken.sol
pragma solidity ^0.7.6;
...
// File: contracts/testVFY.sol
pragma solidity ^0.7.6;
contract testVFY is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    testVFYDividendTracker public dividendTracker;
    ...
    uint256 public totalFees = tokenRewardsFee.add(liquidityFee).add(RDFee).add(charityFee).add(buyBackFee).add(burnFee);
    // buy fees
    uint256 public buyRDFee = 1;
    ...
    uint256 public buyTotalFees = buyTokenRewardsFee.add(buyLiquidityFee).add(buyRDFee).add(buyCharityFee).add(buyBuyBackFee).add(buyBurnFee);
    // sell fees
    ...
    uint256 public sellTotalFees = sellTokenRewardsFee.add(sellLiquidityFee).add(sellRDFee).add(sellCharityFee).add(sellBuyBackFee).add(sellBurnFee);
    // fee tracker
    ...
    //     RDTokenAddressForFee = newAddress;
    // }    
    ...
    function updateFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _RDFee, uint256 _charityFee, uint256 _buyBackFee, uint256 _burnFee) external onlyOwner {
        ...
        burnFee = _burnFee;
        totalFees = _tokenRewardsFee.add(_liquidityFee).add(_RDFee).add(_charityFee).add(_buyBackFee).add(_burnFee);
        ...
        previousTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_RDFee).add(_charityFee).add(_buyBackFee).add(_burnFee);
    }    
    
    function updateBuyFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _RDFee, uint256 _charityFee, uint256 _buyBackFee, uint256 _burnFee) external onlyOwner {
        ...
        buyBurnFee = _burnFee;
        buyTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_RDFee).add(_charityFee).add(_buyBackFee).add(_burnFee);
    }    
    
    function updateSellFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _RDFee, uint256 _charityFee, uint256 _buyBackFee, uint256 _burnFee) external onlyOwner {
        ...
        sellTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_RDFee).add(_charityFee).add(_buyBackFee).add(_burnFee);
    }
    //
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "testVFY: The dividend tracker already has that address");

        testVFYDividendTracker newDividendTracker = testVFYDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "testVFY: The new dividend tracker must be owned by the testVFY token contract");
    ...
    function updateRouter(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "testVFY: The router already has that address");
        emit UpdateRouter(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
        _isExcludedFromLimits[newAddress];
        dividendTracker.excludeFromDividends(newAddress);
    }    
    function updatePair(address newAddress) external onlyOwner {
        require(newAddress != uniswapV2Pair , "testVFY: The pair already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "testVFY: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "testVFY: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "testVFY: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "testVFY: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "testVFY: Cannot update gasForProcessing to same value");
    ...
    function swapAndLiquify(uint256 tokens) private  {
        ...
        uint256 tokensAfterFee = tokens - RDFeeFull - charityFeeFull - buyBackFeeFull;
        ...
        uint256 finalBalance = newBalance - RDFeeBalance - charityFeeBalance - buyBackFeeBalance;
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "testVFY: This account cannot send tokens until trading is enabled");
        }
...
contract testVFYDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("testVFY_Dividend_Tracker", "testVFY_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "testVFY_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "testVFY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main testVFY contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "testVFY_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "testVFY_Dividend_Tracker: Cannot update claimWait to same value");
