...
interface IStdReference {
    ...
    function getReferenceData(string calldata _bases , string calldata _quotes )
        external
        view
...
contract JrepoOracle is IPriceFeedsExt, Ownable {
    ...
    function latestAnswer() external view returns (uint256 _price) {
        ...
        //IBANDOracle.ReferenceData memory data = tfiOracleAddress.getReferenceData("XAU","USD");
        _price = data.rate;      
    }
    ...
    function latestTimestamp() external view returns (uint256 _timestamp) {
        IStdReference.ReferenceData memory data = ref.getReferenceData("XAU","USD");
        // IBANDOracle.ReferenceData memory data = tfiOracleAddress.getReferenceData("XAU","USD");
        _timestamp = data.lastUpdatedQuote;   
    }
    function setTFIOracleAddress(IStdReference _ref) public onlyOwner {
        ref = _ref;
    }      
}
...
contract PriceFeeds is Constants, Ownable {
        ...
        public
    {
        ...
        decimals[_wbaseTokenAddress] = 18;
        _setWbaseToken(_wbaseTokenAddress);
