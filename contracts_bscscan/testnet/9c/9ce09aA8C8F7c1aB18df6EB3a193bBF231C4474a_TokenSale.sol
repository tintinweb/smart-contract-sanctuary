...
contract TokenSale is Ownable {
    ...
    uint256 public devDividend = 25; //2%
    uint256 public ownerDividend = 75; // 75%
    ...
    IERC20 public token = IERC20(0x45E9A6849023C816D32407F524B5f887D389637a);
        ...
        returns (uint256)
    {
        ...
        uint256 amount = _amount.mul(price).div(res);
        return amount;
    }
        ...
        returns (uint256 _receivingAmount)
    {
        if (_amount >= 5000 ether && _amount < 10000 ether) {
            ...
            _amount = _amount.add(_amount.mul(25).div(100));
        }
        return _amount;
    }
    ...
    function buyTokens(
        ...
        if(referrers[msg.sender]==address(0)){
            require(_referrer!=address(0) && _referrer!=msg.sender && (_referrer==owner() || referrers[_referrer]!=address(0)), "Invalid referrer");
        }
        uint256 currPrice = getCurrentPriceInUSD(_type);
        uint256 recievingAmount;
        uint256 amountToPay;
        if (_type == 2) {
            amountToPay = amountNeedsToBePaid(msg.value, _type);

            payable(owner()).transfer(msg.value.mul(ownerDividend).div(100));
            payable(dev).transfer(msg.value.mul(devDividend).div(100));
        } else {
            ...
            depositToken.safeTransferFrom(
                msg.sender,
                owner(),
                _amount.mul(ownerDividend).div(100)
            );
            depositToken.safeTransferFrom(
                msg.sender,
                dev,
                _amount.mul(devDividend).div(100)
            );
        }
        ...
        recordReferralCommission(
            ...
            recievingAmount.mul(refBonus).div(100)
        );

        token.safeTransfer(
            msg.sender,
            recievingAmount.sub(recievingAmount.mul(refBonus).div(100))
        );

        token.safeTransfer(msg.sender, recievingAmount.mul(refBonus).div(100));
    ...
    // updated dev or owner wallet, if _isDev is true then dev wallet else owner wallet
    function updateDevOrOwnerWallet(address _newAddress, bool _isDev)
        public
        onlyOwner
    {
        require(_newAddress != address(0), 'invalid address');
        if (_isDev) dev = _newAddress;
        else transferOwnership(_newAddress);
    }

    // updated dev or owner dividends, if _isDev is true then dev else owner
    function updateDevOrOwnerDividend(uint256 _newDividend, bool _isDev)
        public
        onlyOwner
    {
        if (_isDev) devDividend = _newDividend;
        else ownerDividend = _newDividend;
    }

    /* REFERRAL RELATED FUNCTIONS */
    
    // function to save referral related info
    function recordReferral(address _user, address _referrer) internal {
        if (referrers[_referrer] != address(0)) return;
        if (
            _user != address(0) &&
            _referrer != address(0) &&
            _user != _referrer &&
    ...
    // to keep track of referral income earned so far by user
