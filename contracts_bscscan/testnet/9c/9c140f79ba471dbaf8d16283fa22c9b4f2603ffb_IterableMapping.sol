...
//powered by snipe.finance for RTT Team
...
// File: contracts/RTTDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public DividendToken = address(this);
...
// File: contracts/RTT.sol
// 
pragma solidity ^0.7.6;
contract RTTv2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RTTDividendTracker public dividendTracker;

    address public burnAddress;
    
    uint256 public maxBuyTranscationAmount = 10000000000 * (10**9);
    uint256 public maxSellTransactionAmount = 50000000 * (10**9);
    uint256 public swapTokensAtAmount = 2000000 * (10**9);
    uint256 public _maxWalletToken = 10000000000 * (10**9);

//powered by snipe.finance
    ...
    constructor()  ERC20("Real Trump Token v2", "RTTv2") {
        uint256 _tokenRewardsFee = 2;
        uint256 _liquidityFee = 2;
        uint256 _wallet1Fee = 1;
        uint256 _wallet2Fee = 1;
        uint256 _wallet3Fee = 1;
        uint256 _wallet4Fee = 1;
        uint256 _wallet5Fee = 2;
        uint256 _lpLockTime = 259200;
        ...
        totalFees = _tokenRewardsFee + _liquidityFee + _wallet1Fee + _wallet2Fee + _wallet3Fee + _wallet4Fee + _wallet5Fee;
        lpLockTime = _lpLockTime;
        


        dividendTracker = new RTTDividendTracker();
        ...
        // owner can transfer even if trading is disabled
    ...
    function updateFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _wallet1Fee, uint256 _wallet2Fee, uint256 _wallet3Fee, uint256 _wallet4Fee, uint256 _wallet5Fee) external onlyOwner {
        ...
        totalFees = tokenRewardsFee.add(liquidityFee).add(wallet1Fee).add(wallet2Fee).add(wallet3Fee).add(wallet4Fee).add(wallet5Fee);
    }
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "RTT: The dividend tracker already has that address");

        RTTDividendTracker newDividendTracker = RTTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "RTT: The new dividend tracker must be owned by the RTT token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "RTT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "RTT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "RTT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "RTT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "RTT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "RTT: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "RTT: This account cannot send tokens until trading is enabled");
        }
    ...
    function portionCalculatorOne(uint256 _wallet1Fee, uint256 _otherHalf, uint256 _newBalance) internal returns (uint256,uint256){
        ...
        uint256 wallet1feeBalance = _newBalance.div(totalFees).mul(_wallet1Fee);
        uint256 wallet1feePortion = _otherHalf.div(totalFees).mul(_wallet1Fee);        
    ...
    function portionCalculatorTwo(uint256 _wallet2Fee, uint256 _otherHalf, uint256 _newBalance) internal returns (uint256,uint256){
        // calculate the portions of the liquidity to add to wallet3fee
        uint256 wallet2feeBalance = _newBalance.div(totalFees).mul(_wallet2Fee);
        uint256 wallet2feePortion = _otherHalf.div(totalFees).mul(_wallet2Fee);        
    ...
    function portionCalculatorThree(uint256 _wallet3Fee, uint256 _otherHalf, uint256 _newBalance) internal returns (uint256,uint256){
        // calculate the portions of the liquidity to add to wallet3fee
        uint256 wallet3feeBalance = _newBalance.div(totalFees).mul(_wallet3Fee);
        uint256 wallet3feePortion = _otherHalf.div(totalFees).mul(_wallet3Fee);        
    ...
    function portionCalculatorFour(uint256 _wallet4Fee, uint256 _otherHalf, uint256 _newBalance) internal returns (uint256,uint256){
        // calculate the portions of the liquidity to add to wallet3fee
        uint256 wallet4feeBalance = _newBalance.div(totalFees).mul(_wallet4Fee);
        uint256 wallet4feePortion = _otherHalf.div(totalFees).mul(_wallet4Fee);        
    ...
    function portionCalculatorFive(uint256 _wallet5Fee, uint256 _otherHalf, uint256 _newBalance) internal returns (uint256,uint256){
        // calculate the portions of the liquidity to add to wallet3fee
        uint256 wallet5feeBalance = _newBalance.div(totalFees).mul(_wallet5Fee);
        uint256 wallet5feePortion = _otherHalf.div(totalFees).mul(_wallet5Fee);        
...
contract RTTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()  DividendPayingToken("RTT_Dividend_Tracker", "RTT_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "RTT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "RTT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RTT contract.");
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "RTT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "RTT_Dividend_Tracker: Cannot update claimWait to same value");
