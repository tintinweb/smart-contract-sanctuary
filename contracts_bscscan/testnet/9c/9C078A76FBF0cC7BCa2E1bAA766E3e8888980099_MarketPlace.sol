    ...
    OwnableUpgradeable
{
    ...
    function removeItem(bytes32 _id) external returns (bytes32) {
        ...
        require(
            sellerAddress[_id] == msg.sender || nftContractsAllowed[tokenAddress[_id]] == true,
            "Not allowed"
        );
    ...
    function changeNftContractsAllowed(
        address[] memory _addresses,
        bool[] memory _allowed
    ) external onlyOwner {
        require(_addresses.length == _allowed.length, "Not same length");

        for (uint256 index = 0; index < _addresses.length; index++) {
            nftContractsAllowed[_addresses[index]] = _allowed[index];
        }
    }
