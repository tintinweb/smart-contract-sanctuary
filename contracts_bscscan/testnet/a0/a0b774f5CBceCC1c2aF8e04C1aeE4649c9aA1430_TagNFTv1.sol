    ...
    UUPSUpgradeable
{
    ...
    mapping(uint256 => TagStruct) private NFTMap;
     ...
     *
     */
    function initialize(address _forwarder) external initializer {
        __AccessControl_init();
        __ERC2771Context_init(_forwarder);
        __ERC721_init("Tag Protocol Hashtag NFT", "HTAG");
        __Pausable_init();
    ...
    function setURIBase(string memory _uri) public virtual onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
        uriBase = _uri;
        return true;
    }
    ...
    function setTokenURI(uint256 tokenId, string memory _uri) public virtual returns (bool) {
        ...
        require(ownerOf(tokenId) == _msgSender(), "ERC721URIStorage: URI query for conflicting token");
        NFTMap[tokenId].TAGURI = _uri;
        return true;
    }

    function pauseToken() public virtual onlyRole(PAUSE_ROLE) returns (bool) {
        _pause();
        return true;
    }

    function tokenURLS(uint256 tokenId) public view virtual returns (string memory) {
        require(_exists(tokenId), "ERC721URIStorage: URI query for nonexistent token");
        string memory _tokenURI = NFTMap[tokenId].TAGURI;
        string memory base = _baseURI();
        return string(abi.encodePacked(base, _tokenURI));
    }

    function unpauseToken() public virtual onlyRole(PAUSE_ROLE) returns (bool) {
        _unpause();
        return true;
    }
        ...
        returns (bool)
    {
        ...
        NFTMap[tokenID] = TagStruct(tokenDetails, hash, "");
        isMint[hash] = true;
    ...
    function fetchTokenDetails(uint256 _tokenID) public view virtual returns (string memory tokenDetail, bytes32 hash) {
        tokenDetail = NFTMap[_tokenID].TAGName;
