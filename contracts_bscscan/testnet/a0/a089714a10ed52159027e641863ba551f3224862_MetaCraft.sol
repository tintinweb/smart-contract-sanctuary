...
contract MetaCraft is ERC721Enumerable,IMetaCraft {
    ...
    function get_all_summoners_info(address _owner) view external returns(SummonerInfo[] memory) {
        uint count = ERC721.balanceOf(_owner);
        SummonerInfo[] memory res = new SummonerInfo[](count);
        for(uint i = 0; i < count; i++) {
            uint _summoner = tokenOfOwnerByIndex(_owner,i);
            res[i] = SummonerInfo(xp[_summoner], adventurers_log[_summoner], class[_summoner], level[_summoner]);
        }
        return res;
    }

    function get_summoners_info_page(address _owner, uint page, uint size) view external returns (SummonerInfo[] memory) {
        require(page > 0 && size > 0, 'invalid param');
        uint count = ERC721.balanceOf(_owner);
        uint end = page * size;
        if (end > count) {
            if (end - count >= size) {
                return new SummonerInfo[](0);
            } else {
                size = size - (end - count);
                end = count;
            }
        }
        SummonerInfo[] memory res = new SummonerInfo[](size);
        uint start = end - size;
        for (uint i = 0; i < size; i++) {
            uint _summoner = tokenOfOwnerByIndex(_owner, start + i);
            res[i] = SummonerInfo(xp[_summoner], adventurers_log[_summoner], class[_summoner], level[_summoner]);
        }
        return res;
    }
