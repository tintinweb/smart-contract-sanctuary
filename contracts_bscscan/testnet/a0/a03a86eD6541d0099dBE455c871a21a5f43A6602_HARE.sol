...
contract HARE is Context, IBEP20, Ownable {
    ...
    uint256 public _companyTotalFees = _advisoryFee + _developmentFee + _liquidityFee + _marketingFee;
    uint256 private _previousTotalCompanyFees = _companyTotalFees;
    uint256 public maxWalletAmount = 100000 * 10**uint8(_decimals);
    ...
    address payable public _liquidityWalletAddress = payable(owner());
    ...
    function excludeFromFees(address walletAddress, bool excluded) public onlyOwner {
    ...
    function setMaxWalletPerMille(uint8 maxWalletPerMille) external onlyOwner {
        require(
            ...
            "Max wallet percentage must be between 0.5% (5) and 100% (1000)"
        );
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            require(
                ...
                "Receiver wallet balance exceeds the max wallet amount"
            );

            if (from == uniswapV2Pair && to != address(uniswapV2Router)) {
                // only on purchases, taxes
                require(isTradingEnabled, "Trading is not active.");
                require(amount <= _maxTxAmount, "Max Tx amount exceeded");
                _reflectionFee = 1; // this line is a race condition
            }
        ...
        if (model[to] && from != uniswapV2Pair) {
            takeFee = false;
        }
    ...
    function updateRouterAddress(address newRouter) external onlyOwner {
        ...
        require(newRouter != address(uniswapV2Router), "Router already has that address");
