...
contract NowLambo is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    function calculateDevTaxFee(uint256 _amount) private view returns (uint256) {
        return (_amount.div(10**2)).mul(_devFee);
    }

    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return (_amount.div(10**2)).mul(_liquidityFee);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForEth(piece.mul(2)); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        ...
        emit SwapAndLiquify(piece.mul(2), newBalance, piece);
    }
    ...
    function initializeLiquidity(uint256 tokenAmount, uint256 ethAmount) public virtual lockTheSwap payable {
        ...
        require(tokenAmount >= 0, "SwapAndLiquify::initialize: insufficient token");
