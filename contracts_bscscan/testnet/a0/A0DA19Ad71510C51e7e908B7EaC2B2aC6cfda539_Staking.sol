        ...
        ReentrancyGuard
{
    ...
    address public AI;
    address public PEPE;
    address public _aiTreasuryAddress = 0xA6856A6a15e6fDf2BE1857696fc4b1d7a75eACD6;
    ...
    function deposit(address tokenAddress, uint256 contractType, uint256[] memory amountOrIDs) public nonReentrant whenNotPaused {
        ...
        if (contractType == 0) {
          ...
          for (uint256 i = 0; i < amountOrIDs.length; i++) {
            if (IERC721(tokenAddress).ownerOf(amountOrIDs[i]) == msg.sender) {
              ...
              balances[msg.sender][tokenAddress] = balances[msg.sender][tokenAddress].add(1);
              amount++;
            }
          }
        }
        ...
        pNextEpoch.size = contractType == 0 ? IERC20(tokenAddress).balanceOf(address(this)) : IERC721(tokenAddress).balanceOf(address(this));
        pNextEpoch.set = true;
        ...
        if (contractType == 0) {
          ...
          for (uint256 i = 0; i < amountOrIDs.length; i++) {
            if (IERC721(tokenAddress).ownerOf(amountOrIDs[i]) == address(this) && _isOwner(msg.sender, amountOrIDs[i])) {
              ...
              balances[msg.sender][tokenAddress] = balances[msg.sender][tokenAddress].sub(1);
              amount++;
            }
          }
        }
