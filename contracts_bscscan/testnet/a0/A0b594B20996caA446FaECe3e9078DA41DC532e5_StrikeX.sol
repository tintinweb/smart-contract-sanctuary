...
contract StrikeX is Context, IERC20, Ownable {
    ...
    mapping(address => uint256) public _rOwned; // reflected token owned amount
    mapping(address => uint256) public _tOwned; // addresses token owned amount
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private _isExcludedFromFee; // wallets excluded from fee
    

    uint256 private constant MAX = ~uint256(0);
    uint256 public constant _tTotal = 10**9 * 10**9; // total supply 1,000,000,000 (1 billion)
    uint256 public _rTotal = (MAX - (MAX % _tTotal)); // reflected total supply
