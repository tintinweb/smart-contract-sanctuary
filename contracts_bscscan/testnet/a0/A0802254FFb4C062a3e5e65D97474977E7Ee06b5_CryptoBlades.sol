...
contract CryptoBlades is Initializable, AccessControlUpgradeable {
    ...
    function getMaxTokenClaimAmountPreTax() public view returns(uint256) {
        if(tokenRewards[msg.sender] >= vars[VAR_CLAIM_DEPOSIT_AMOUNT]) { 
...
contract Blacksmith is Initializable, AccessControlUpgradeable {
    ...
    uint256 public constant ITEM_CHARACTER_REFILL_STAMINA = 9;
    ...
    function migrateTo_16884dd(
        ...
        address _charRefillStaminaChange
    ) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not admin");
        ...
        itemAddresses[ITEM_CHARACTER_REFILL_STAMINA] = _charRefillStaminaChange;
        ...
        itemFlatPrices[ITEM_CHARACTER_REFILL_STAMINA] = 0.5 ether;
    }
    ...
    function setCharacterRefillStaminaPrice(uint256 newPrice) external isAdmin {
        require(newPrice > 0, 'invalid price');
        itemFlatPrices[ITEM_CHARACTER_REFILL_STAMINA] = newPrice;
    }

    function characterRefillStaminaPrice() public view returns (uint256){
        return itemFlatPrices[ITEM_CHARACTER_REFILL_STAMINA];
    }

    function purchaseRefillStamina(uint256 paying) public {
        require(paying == itemFlatPrices[ITEM_CHARACTER_REFILL_STAMINA], 'Invalid price');
        game.payContractTokenOnly(msg.sender, itemFlatPrices[ITEM_CHARACTER_REFILL_STAMINA]);
        Consumables(itemAddresses[ITEM_CHARACTER_REFILL_STAMINA]).giveItem(msg.sender, 1);
    }
