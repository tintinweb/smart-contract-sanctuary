...
contract StarterRaceRouter is ReentrancyGuard,Ownable,ITokenRacesRouter {
    ...
    uint256 public maxLenghtRace = 2 hours;
    ...
    function setMinAndMaxParticipationFee(uint _minAndMaxParticipationFeeAmount) external override onlyOwner() {
        if(currentRaceId != 0) {
            require(race[currentRaceId].status == Status.Close,"You cannot change the time until the race is over.");
        } 
    ...
    function setMaxLenghtRace(uint256 _maxLenghtRace) external override onlyOwner() {
        if(currentRaceId != 0) {
            require(race[currentRaceId].status == Status.Close,"You cannot change the time until the race is over.");
        }        
    ...
    function openTrade() external onlyOwner() {
        ...
        require(lastPlayerIndex(currentRaceId) == totalParticipants,"Not all players joined in race");
         ...
         currentRaceTokenAddress = ITokenRacesCreateFactory(tokenRacesFactory).startRacingToken(
