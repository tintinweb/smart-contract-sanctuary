 ...
 *      BscBank manages the minting and burning of tokens which
 *      represent evrnet based assets, while EvrnetBank manages
 *      the locking and unlocking of BNB and BEP20 token assets
 *      based on Bsc.
 **/

contract BridgeBank is Initializable, EvrynetBank, BscBank {
    ...
    // }
    /*
     * @dev: Creates a new BridgeToken by operator
     *
     ...
     * @param _ethereumRecipient: The intended recipient's address.
    ...
    /*
     * @dev: Burns BridgeTokens representing native evrynet assets
     *
     * @param _recipient: representation of destination address.
     * @param _token: token address in origin chain (0x0 if ethereum)
     * @param _amount: value of burn
     */
    function burn(
        address _recipient,
        address _token,
    ...
    function getCosmosDepositStatus(bytes32 _id) public view returns (bool) {
        return isLockedBscDeposit(_id);
    }

    /*
     * @dev: Allows access to a evrnet deposit's information via its unique identifier.
     *
     * @param _id: The deposit to be viewed.
     * @return: Original sender's Ethereum address.
     * @return: Intended evrnet recipient's address.
...
/**
 * @title BridgeToken
 * @dev AccessControl, ERC20Pausable, ERC20Capped compatible BankToken for use by BridgeBank
 **/

contract BridgeToken is ERC20Capped, AccessControl, ERC20Pausable {
    constructor(string memory _symbol, uint256 cap_)
...
/*
 *  @title: EvrnetBank
 *  @dev: Bsc bank which locks Bsc/ERC20 token deposits, and unlocks
 *        Bsc/ERC20 tokens once the prophecy has been successfully processed.
 */
contract BscBank {
    ...
    function burnFunds(
        ...
        lockBurnNonce++;
        emit LogBurn(_sender, _recipient, _token, _symbol, _amount, lockBurnNonce, _chainName);
    }
...
contract EvrynetBank {
    uint256 public bridgeTokenCount;
    mapping(string => address) controlledBridgeTokens;
    mapping(bytes32 => BscDeposit) bscDeposit;

    struct BscDeposit {
        address bscSender;
        address payable evrnetRecipient;
        address bridgeTokenAddress;
        uint256 amount;
        bool locked;
    }

    uint256 private constant CAP = 1_000_000_000_000_000_000_000_000_000; // 1000M with decimal = 18
    ...
    /*
     * @dev: Creates a new BscDeposit with a unique ID
     *
     * @param _bscSender: The sender's evrnet address.
     * @param _evrnetRecipient: The intended recipient's Evrnet address.
     * @param _token: The currency type
     * @param _amount: The amount in the deposit.
     * @return: The newly created BscDeposit's unique id.
     */
    function newBscDeposit(
        address _bscSender,
        address payable _evrnetRecipient,
        address _token,
        uint256 _amount
    ) internal returns (bytes32) {
        bytes32 depositID = keccak256(
            abi.encodePacked(_bscSender, _evrnetRecipient, _token, _amount)
        );

        bscDeposit[depositID] = BscDeposit(
            _bscSender,
            _evrnetRecipient,
            _token,
            _amount,
    ...
    function deployNewBridgeToken(string memory _symbol)
        ...
        returns (address)
    {
        // Checking if symbol existed by compare it to address(0)
        // Return failed if it is existed
        require(
            controlledBridgeTokens[_symbol] == address(0),
        ...
        BridgeToken newBridgeToken = (new BridgeToken)(_symbol, CAP);
     ...
     *
     * @param _bscSender: The sender's bsc address.
     * @param _evrnetRecipient: The intended recipient's Bsc address.
    ...
    /*
     * @dev: Checks if an individual BscDeposit exists.
     *
     * @param _id: The unique BscDeposit's id.
     * @return: Boolean indicating if the BscDeposit exists in memory.
     */
    function isLockedBscDeposit(bytes32 _id) internal view returns (bool) {
        return (bscDeposit[_id].locked);
    }
     ...
     * @return: Amount of bsc/erc20 in the item.
     * @return: Unique nonce of the item.
     */
    function getBscDeposit(bytes32 _id)
        internal
        view
        returns (address, address payable, address, uint256)
    {
        BscDeposit memory deposit = bscDeposit[_id];

        return (
            deposit.bscSender,
            deposit.evrnetRecipient,
