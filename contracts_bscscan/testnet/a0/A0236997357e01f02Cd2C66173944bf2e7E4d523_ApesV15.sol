...
contract ApesV15 is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable {
  ...
  function mintWithFee(string memory _name, string memory _tokenURI) public {
    ...
    pancake.swapTokensForEth(quarterPart); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
    ...
    pancake.addLiquidity(bnbAddr, aGoldAddr, amountBNBGiven, quarterPart, 0, 0, devWallet, 1661729110);
...
abstract contract IPancakeRouter {
  ...
  function swapTokensForEth(uint256 tokenAmount) external virtual;
}
