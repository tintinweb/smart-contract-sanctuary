...
contract TestFactor {
	...
	uint256 constant public TIME_STEP = 2 minutes;
	...
	function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			uint256 finish = user.deposits[i].start.add(plans[user.deposits[i].plan].time.mul(2 minutes));
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint8 plan, uint256 percent, uint256 amount, uint256 start, uint256 finish) {
		...
		finish = user.deposits[index].start.add(plans[user.deposits[index].plan].time.mul(2 minutes));
	}
