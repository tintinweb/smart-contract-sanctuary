...
abstract contract Merchant is MerchantInt{
    ...
    function getClaimable(address reseller, address memberAddress) public view returns(uint256 unclaimed){
        if(_members[memberAddress].exist == false) return 1;
        if(_hashRates[reseller][memberAddress].exist == false) return 2;
        if(_hashRates[reseller][memberAddress].expiry < block.timestamp && _hashRates[reseller][memberAddress].hashRate ==0) return 3;
        uint span = _hashRates[reseller][memberAddress].expiry > block.timestamp? block.timestamp - _hashRates[reseller][memberAddress].startDate : _hashRates[reseller][memberAddress].expiry - _hashRates[reseller][memberAddress].startDate;
        return span * (_hashRates[msg.sender][reseller].hashRate);
    }
