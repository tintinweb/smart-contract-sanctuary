...
contract GoldenDogeLottery is ReentrancyGuard, IGoldenDogeLottery, Ownable {
    ...
    uint8 private burnPercent = 50;
    uint8 private valutPercent = 0;

    address private deadAddress = 0x000000000000000000000000000000000000dEaD;
    address private valutAddress = 0xa11Dd636B66D36c8bE6400586cce82694D7369C8;
        ...
        nonReentrant
    {
        ...
        // Burn GDOGE
        uint256 burnAmount = (amountToWithdrawToTreasury * burnPercent) / 100;
        cakeToken.safeTransfer(deadAddress, burnAmount);

        // Transfer GDOGE to golden valut
        uint256 valutAmount = (amountToWithdrawToTreasury * valutPercent) / 100;

        if (valutAmount > 0) {
            cakeToken.safeTransfer(valutAddress, valutAmount);
        }

        // Transfer GDOGE to treasury address
        cakeToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury - burnAmount - valutAmount);
    ...
    /**
     * @notice Set burn percent
     * @dev Only callable by owner
     * @param _percent: percent to burn
     */
    function setBurnPercent(uint8 _percent) external onlyOwner {
