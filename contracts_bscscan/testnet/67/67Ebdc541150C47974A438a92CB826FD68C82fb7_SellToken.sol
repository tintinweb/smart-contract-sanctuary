...
contract SellToken {
  ...
  constructor(address _idoAdmin, address _buyToken, address _idoToken) {
    IDOAdmin = _idoAdmin;
    IDO_TOKEN = _idoToken;
    BUY_TOKEN = _buyToken;
  }
   ...
   **/
  function buyIDO(address recipient, uint256 buy_amount) public returns (uint256) {
    IERC20(BUY_TOKEN).transferFrom(msg.sender, address(this), buy_amount);
    uint256 sold_amount = buy_amount * 1e18 / tokenRate;
    IERC20(IDO_TOKEN).transferFrom(address(this), recipient, sold_amount);
    return sold_amount;
  }
}
