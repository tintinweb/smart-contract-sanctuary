...
contract WhitelistDemo is MaxAccess {
  ...
  function addBatchWhitelist(address[] memory newAddresses) public onlyOwner {
  ...
  function removeBatchWhitelist(address[] memory newAddresses) public onlyOwner {
    for(uint x = 0; x < newAddresses.length; x++) {
  ...
  function WhitelistEfficiency() external view returns (uint) {
    if(whitelist.totalRemoved() == 0) {
      ...
      return whitelist.totalAdded() * 1000 / whitelist.totalRemoved();
    }
  }
}
...
library Whitelist {
  ...
  function totalRemaining(List storage list) internal view returns (uint) {
    // since you have to be on the whitelist to be removed this should never be below zero.
    // but since 0.8.x this will automatically revert, hence the requires on .add() and .remove()
    // so an if/else in here just in case, but not warranted, and yes = would be 0 anyways.
    if(list._added.current() > list._removed.current()) {
      return list._added.current() - list._removed.current();
    } else {
      return 0;
    }
  }
  ...
  function add(List storage list, address _address) internal {
    ...
    // require(!list.onList(_address), "Whitelist: Address already whitelisted.");
  ...
  function remove(List storage list, address _address) internal {
    // require(list.onList(_address), "Whitelist: Address already not whitelisted.");
