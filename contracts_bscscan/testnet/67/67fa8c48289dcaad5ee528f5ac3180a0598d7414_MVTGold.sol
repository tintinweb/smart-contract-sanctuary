...
contract MVTGold is Ownable  {
    ...
    event MetaVault_Purchased ( address _purchaser, uint _bnbamount, uint _tokenamount );
    ...
    constructor() {
        ...
        maxTokensForSale = 3000000 * 10 ** 18;
    }
    
    function purchaseMetaVault () public payable {
        require ( saleOpen , "Sale not open" );
        require ( !tokensWithdrawal, "Withdrawal open" );
        require ( WhiteList[msg.sender ], "Address not whitelisted ");
        require ( msg.value >= 500000000000000000, "Minimum 0.5 BNB");
        uint256 _total = msg.value + Sales[msg.sender].bnbSpent;
        require ( _total <= 4000000000000000000, "Maximum 4 BNB" );
        ...
        emit MetaVault_Purchased ( msg.sender, msg.value, _amount );
    }
    ...
    function toggleWithdrawal () public onlyTeam {
        ...
        require ( tokensperbnb > 0 , "Please set Tokens Per BNB" );
    ...
    function receiveTokens() public  {
         ...
         require ( block.timestamp > Sales[msg.sender].lastpaymentblocktime , "MetaVault not available yet" );
         
         Sales[msg.sender].numberofpaymentsreceived++;
         Sales[msg.sender].lastpaymentblocktime = block.timestamp + 100 seconds; // change to 30 days
