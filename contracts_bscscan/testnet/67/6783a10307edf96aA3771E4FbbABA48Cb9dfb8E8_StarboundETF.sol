...
contract StarboundETF is ERC20, Ownable, Pausable {
    ...
    function calculateFundsLayout(address _asset, uint _amount, bool _flag) external view returns (uint newPrice, uint[] memory newWeights) {
        newWeights = new uint[](assets.length());

        if (!assets.contains(_asset) || totalSupply() == 0) return (basePrice, newWeights);
        ...
        for (uint i = 0; i < assets.length(); i++) {
            newWeights[i] = assetValues[i].mul(MAX_FEE).div(totalValue);
        }
    ...
    function _updateAssetWeights() internal {
        ...
        for (uint i = 0; i < assets.length(); i++) {
            assetInfo[assets.at(i)].weight = assetValues[i].mul(MAX_FEE).div(totalValue);
        }
    }
