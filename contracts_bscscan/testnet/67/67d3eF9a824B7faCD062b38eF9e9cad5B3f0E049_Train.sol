...
contract Train is ERC20, PaymentSplitter, RailNode {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal view override {
        ...
        /*console.log("amount: %s", uniswapV2Pair);
        console.log("antiWhale: %s", to);
        console.log("hasAntiWhale: %s", from);
        console.log("enableTrading: %s", enableTrading);
        console.log("c2: %s", hasAntiWhale && amount <= antiWhale * 10**18);
        */
        // require(enableTrading || !automatedMarketMakerPairs[to], 'The trader has not started yet');
        // require(amount <= antiWhale * 10**18 || !automatedMarketMakerPairs[to], "You are not permitted to transfer more than #antiWhale tokens");
    }
    ...
    function createNode(string memory name, uint8 nbNode, RailNode.NodeType nodeType) public onlyNoder(name, nbNode) {
        ...
        for(uint8 i = 0; i < nbNode; i++) {
            RailNode.createNode(sender, name, nodeType);
        }
