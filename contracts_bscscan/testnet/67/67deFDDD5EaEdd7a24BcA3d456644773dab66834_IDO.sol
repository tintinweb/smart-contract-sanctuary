...
contract IDO is AccessControl, ReentrancyGuard {
    ...
    event DepositDLS(uint256 amount, uint256 campaignID, uint256 timestamp);
    event WithdrawDLS(uint256 amount, uint256 campaignID, uint256 timestamp);
    enum CampaignState {
        Unknown,
        Expectation,
        Vesting,
    ...
    function depositDLS(uint256 amount, uint256 campaignID) external {
        ...
        emit DepositDLS(amount, campaignID, block.timestamp);
    }
        ...
        nonReentrant
    {
        ...
        emit WithdrawDLS(amount, campaignID, block.timestamp);
    }
