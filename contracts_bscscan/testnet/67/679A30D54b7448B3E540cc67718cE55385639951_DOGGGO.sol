...
contract DOGGGO is Ownable, IBEP20 {
	...
	// address public constant TREASURY_WALLET = address(0x95eA0b449EA75EeD4577bB6E9A5Bed7B984C478d);
	...
	function _transfer(
		...
		if (!excluded[sender] && !excluded[recipient] && !circuitBreaker) {
			...
			treasuryTax = amount.mul(FIXED_FEE).ceilDiv(10**3).add(sellTax);

			_balances[TREASURY_WALLET] += treasuryTax;
			emit Transfer(_msgSender(), TREASURY_WALLET, treasuryTax);
		...
		_balances[recipient] += amount.sub(charityTax).sub(treasuryTax);
		emit Transfer(sender, recipient, amount.sub(charityTax).sub(treasuryTax));
	}
