...
Seekreward public token;
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        require(token.transferFrom(msg.sender, admin1, _amount), "Seekreward: transaction failed");
        ...
        token.transferFrom(admin1,admin1, adminFee.div(2)); // 2.5% admin1
        token.transferFrom(admin1,admin2, adminFee.div(2)); // 2.5% admin2
        adminFee = 0;
    ...
    function withdraw() external isLock {
        ...
        if (to_payout > 0) {
            ...
            dummyremainingBalance=users[msg.sender].remainingBalance;
        if (users[msg.sender].payouts < max_payout && users[msg.sender].remainingBalance > 0) {
            if  (users[msg.sender].payouts.add(users[msg.sender].remainingBalance) > max_payout) {
                users[msg.sender].remainingBalance=max_payout;
                users[msg.sender].remainingBalance = max_payout.sub(users[msg.sender].remainingBalance);
                }
        ...
        token.transferFrom(admin1,msg.sender, to_payout); // Daily roi and matching bonus
