...
contract BNBHMarket is Initializable, AccessControlUpgradeable, IERC721ReceiverUpgradeable {
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < listedTokenIDs.length() && counter < pageEnd; i++) {
            uint256 characterLevel = characters.getLevel(listedTokenIDs.at(i));
            if(rarity == 0) {
                ...
                uint256 heroRarity = characters.getRarity(listedTokenIDs.at(i));
                if (heroRarity == rarity && characterLevel >= _minLevel && characterLevel <= _maxLevel){
                    if(counter >= pageEnd - _limit) {
                        tokens[tokenIterator] = listedTokenIDs.at(i);
                        tokenIterator++;
                    }
