...
contract babydogeshiba is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 690 * 10**15 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "BabyDogeShiba";
    ...
    uint256 public bnbMinimumAmountInContractToReward = 1000000000000000000;    
    // uint256 public bnbMinimumAmountInContractToReward = 100000000000000000;

    uint256 public bnbMaxUntilCharityCut = 1000000000000000000;    
    // uint256 public bnbMaxUntilCharityCut = 100000000000000000;     
    uint256 public percentOfCharityCut = 30;

    uint256 public maxNumOfTransfersToDoForReward = 5;

    uint256 public minimumAmountToHoldForRewards = 5000000000 * 10**9; 

    uint256 public hoursToAddForReward = 30 minutes;
    ...
    address public charityWallet = 0xfB5a4f8F4b7ACF7ead3A051da9574cae3926CAe1;
    ...
    uint256 public numTokensSellToAddToLiquidity = 300000000000 * 10**9;
    ...
    constructor () {
        ...
        address ownerWallet = 0x3235887915bA48197F497a4BF0c4e8925e550e9e;
        _rOwned[ownerWallet] = _rTotal.div(1000).mul(700);
        emit Transfer(address(0), ownerWallet, _rTotal.div(1000).mul(700));
        
        _rOwned[deadAddressThree] = _rTotal.div(1000).mul(300);  // burn
        emit Transfer(address(0), deadAddressThree, _rTotal.div(1000).mul(300));

        //MAINNET: 0x10ED43C718714eb63d5aA57B78B54704E256024E
        //TESTNET: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3

        address routerDEXAddress = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    function swapTokensForEthAndSwapAndLiquify(uint256 amountToSellForLIQ, uint256 amountToSellForBNB, address fromAddress) private lockTheSwap() {
        ...
        if (amountToSellForLIQ >= numTokensSellToAddToLiquidity && fromAddress != uniswapV2Pair && isSwapAndLiquifyEnabled) {
            ...
            tliquidityAmountTrackingSell = 0;       
        ...
        if(fromAddress != uniswapV2Pair && isRewardSystemEnabled){
            ...
            rconvertBNBAmountTrackingSell = 0;      
    ...
    function distributeBNBReward() private {
        ...
        if(BNBbalanceInContract >= bnbMinimumAmountInContractToReward){ 

            uint256 numberOfTransfersToDo = maxNumOfTransfersToDoForReward;     
            if(numberOfTransfersToDo > addressesToReward.length){
                numberOfTransfersToDo = addressesToReward.length; //8      
            }
            uint256 count = 0;
            uint256 i = 0;
            ...
            while (i < addressesToReward.length && count < numberOfTransfersToDo) {
                if (block.timestamp >= timeToReward[addressesToReward[i]]) {         
                    ...
                    if (tokenBalanceOfRewarded >= minimumAmountToHoldForRewards) {     
                        ...
                        payableAddress(addressesToReward[i]).transfer(bnbToReward);     
                        ...
                        timeToReward[addressesToReward[i]] = block.timestamp.add(hoursToAddForReward);    

                        address addressToAddBack = addressesToReward[i];

                        removeIndexFromRewardArray(i);

                        addressesToReward.push(addressToAddBack);      
