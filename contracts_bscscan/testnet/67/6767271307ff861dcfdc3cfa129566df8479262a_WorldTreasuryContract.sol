...
contract WorldTreasuryContract is Ownable, IOtterTreasury {
    using SafeMath for uint256;
    ...
    address public immutable TRSRY;
    ...
    address public sTRSRY;
    uint256 public sTRSRYQueue; // Delays change to sTRSRY address
    ...
    constructor(
        ...
        require(_TRSRY != address(0));
        TRSRY = _TRSRY;

        isReserveToken[_DAI] = true;
        reserveTokens.push(_DAI);

        isLiquidityToken[_TRSRYDAI] = true;
        liquidityTokens.push(_TRSRYDAI);
        bondCalculator[_TRSRYDAI] = _bondCalculator;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for TRSRY
        @param _amount uint
    ...
    function deposit(
        ...
        // mint TRSRY needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IERC20Mintable(TRSRY).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn TRSRY for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = valueOfToken(_token, _amount);
        ITRSRYERC20(TRSRY).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sTRSRY).balanceOf(msg.sender); // Can only borrow against sTRSRY held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with TRSRY
        @param _amount uint
     */
    function repayDebtWithTRSRY(uint256 _amount) external {
        require(isDebtor[msg.sender], 'Not approved');

        ITRSRYERC20(TRSRY).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, TRSRY, _amount, _amount);
    }
        ...
        override
    {
        ...
        IERC20Mintable(TRSRY).mint(_recipient, _amount);
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(IERC20(TRSRY).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns TRSRY valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    ...
    {
        if (isReserveToken[_token]) {
            // convert amount to match TRSRY decimals
            value_ = _amount.mul(10**IERC20(TRSRY).decimals()).div(
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.STRSRY) {
            // 9
            sTRSRYQueue = block.number.add(blocksNeededForQueue);
        } else return false;
