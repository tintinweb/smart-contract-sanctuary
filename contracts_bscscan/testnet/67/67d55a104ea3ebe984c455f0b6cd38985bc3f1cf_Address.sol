...
contract AmericanDream is Context, IBEPMEME, Ownable {
    ...
    string private _name = "OmegaCringer";
    string private _symbol = "OCringe";
    ...
    function _getCurrentSupply() private view returns (uint256, uint256) {
        ...
        for (uint256 i = 0; i < _excluded.length; i++) {
            ...
            ) return (_rTotalSupply, _allTotalSupply);
            rSupply = rSupply.sub(_rOwned[_excluded[i]]);
            tSupply = tSupply.sub(_tOwned[_excluded[i]]);
        }
        if (rSupply < _rTotalSupply.div(_allTotalSupply))
            return (_rTotalSupply, _allTotalSupply);
