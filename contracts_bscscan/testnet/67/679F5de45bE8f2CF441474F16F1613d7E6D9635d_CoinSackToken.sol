...
contract CoinSackToken is IBEP20, Manageable {
    ...
    uint256 public _amountTokensInEscrow = 0;
    ...
    function _transfer(address from, address to, uint256 tAmount) private {
        ...
        if(_areFeesEnabled && !(_isExcludedFromFees[to] || _isExcludedFromFees[from])){
            ...
            _amountTokensInEscrow = balanceOf(_contextAddress()).sub(_amountTotalFeesPendingLiquidation);
    ...
    function _buybackTokens(uint256 buybackFactor) private internalSwapLock() {
        ...
        _pancakeRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: reserveETHToUse}(0, path, _contextAddress(), _msgTimestamp().add(300));
    ...
    function _reinjectTokensFromEscrow(uint256 reinjectFactor) private {
        ...
        uint256 reserveETHToUse = _contextAddress().balance.mul(reinjectFactor).div(100);
        (uint112 reserve0, uint112 reserve1,) = _pancakePair.getReserves();
        uint256 reserveTokensToUse = _pancakeRouter.getAmountOut(reserveETHToUse, reserve1, reserve0);

        if(reserveETHToUse > balanceOf(_contextAddress())){
            reserveETHToUse = balanceOf(_contextAddress());
        }

        _approve(_contextAddress(), address(_pancakeRouter), reserveTokensToUse);
        _pancakeRouter.addLiquidityETH{value: reserveETHToUse}(_contextAddress(), reserveTokensToUse, 0, 0, _contextAddress(), _msgTimestamp());

        emit AddLiquidity(reserveETHToUse, reserveTokensToUse);
    }
