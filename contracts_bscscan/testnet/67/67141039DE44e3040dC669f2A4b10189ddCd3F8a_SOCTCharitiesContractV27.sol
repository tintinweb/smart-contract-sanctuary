...
pragma solidity ^0.8.4;
    ...
    // Charities Contract test Version 27 for the SOCT37 token. Only for testing purposes on the BSC testnet
    ...
    * The _checkIfReadyToSwap method doesn't contain an IF statement to trade or not within the same function because every call to that kind of function would cost GAS even if the trading is not done.
...
contract SOCTCharitiesContractV27 is Context, Ownable {
    ...
    function _checkIfReadyToSwap() public onlyOwner() returns (bool ready, uint SOCTFundsInBNB) {
