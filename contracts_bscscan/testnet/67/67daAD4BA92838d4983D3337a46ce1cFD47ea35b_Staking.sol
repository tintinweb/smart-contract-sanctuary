...
contract Staking is Ownable {
    ...
    uint32[2] public periods = [30 seconds, 90 seconds];
    uint8[2] public APRS = [50, 60];
    ...
    function stake(uint8 _class, uint256 _amount) public {
        ...
        uint256 _finalAmount = getFinalAmount(_class, _amount);
    ...
    function prolong(uint256 _index) public {
        ...
        uint8 _class = _s.class;
        require(block.timestamp >= _s.timestamp + periods[_class]); // staking period finished
        uint256 _newFinalAmount = getFinalAmount(_class, _s.finalAmount);
        ...
        _s.cycle++;
        require(_s.cycle * periods[_class] <= 360 days, "total staking time exceeds 360 days");
        emit Prolonged(msg.sender, _class, _s.cycle, _s.finalAmount, _newFinalAmount);
    ...
    function returnAccidentallySent(IERC20 _TKN) external onlyOwner {
        ...
        _TKN.safeTransfer(msg.sender, _amount);
    }
    ...
    function changeMinAmountToStake(uint256 _minAmount) external onlyOwner {
        minAmountToStake = _minAmount;
    }

    function getFinalAmount(uint256 _class, uint256 _amount) public view returns (uint256) {
        return _amount + (_amount * APRS[_class] * periods[_class]) / (100 * 365 days);
    }
