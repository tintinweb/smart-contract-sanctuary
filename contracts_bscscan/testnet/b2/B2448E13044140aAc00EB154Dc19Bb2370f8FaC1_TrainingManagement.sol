...
contract TrainingManagement is TrainingCenter {
    ...
    function _getSpotStatus(uint256 _spotId,uint256 _trainingId) internal view returns(uint256 _state){
        if(trainingSpots[_trainingId].length > 0){
    ...
    function hireCoach(uint256 _minCoachLevel, uint256 _percentPayment, uint256 _spotId, uint256 _trainingId) external onlyCenterOwner(_trainingId){
        TrainingInstance storage t = trainingSpots[_trainingId][_spotId];
        require(_getSpotStatus(_spotId, _trainingId) != 3, "Spot!=Free");

        t.coach.minLevelReq = _minCoachLevel;
        t.coach.percentPayment = _percentPayment;
    ...
    monstOwner(_monsterId)
    {
        ...
        require(BZAI.transferFrom(msg.sender, gameAddresses.getPaymentsAddress(), t.price));
        ...
        if(t.coach.coachId != 0){
            ...
            IPayments(_monsterAddress).payOwner(ownerOfCoach, coachPayment);
        }

        IPayments(_monsterAddress).payOwner(ownerOfTC, ownerPayment);
...
interface IBandZaiNFT {
    function mintMonster(address _to,uint256 _state,string memory _ipfsPath,bytes memory _signature) external returns (uint256);
...
interface IPotions {
    function mintPotion(uint256 _fromLab,uint256 _price, uint256[5] memory _power,uint256 _numberOfUse) external returns (uint256);

    function updatePotion(uint256 _tokenId) external; 

    function burnPotion(uint256 _tokenId) external returns(bool);

    function getPotion(uint256 _itemId) external view returns(uint256[5] memory);
