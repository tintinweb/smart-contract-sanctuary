...
contract MagicStella is BEP20, Ownable {
	...
	// 10M
	uint256 private _initTotalSupply = 1 * 10**7 * (10**_decimals);

	// Max Price Impact
	uint256 public priceImpact = 2;

	// thresold for transaction is 50 000 tokens
	uint256 public maxSellTransactionAmount = (_initTotalSupply * 5) / 1000; //
	// Min amount to transform token in contract to dividends, burn, feed algo & liquify 0.01%
	uint256 public swapTokensAtAmount = (_initTotalSupply * 1) / 10000; // 1000 tokens
	...
	uint256 private _maxSellPercent = 99; // Set the maximum percent allowed on sale per a single transaction
	...
	function _removeRestrictionForEcoSystemWallets(
		...
		isAllowedDuringDisabled[address(this)] = true;
		isAllowedDuringDisabled[owner()] = true;
		isAllowedDuringDisabled[teamWallet] = true; // To give a part of wallet for core team partners
		isAllowedDuringDisabled[liquidityWallet] = true; // To Provide liquidity
	}
	...
	function updateStellaRouter(address newAddress) external authorized {
		...
		emit updateStellaRouterEvent(newAddress, address(stellaRouter));
	...
	function blacklistWallet(address wallet, bool status) external authorized {
	...
	function updateBuyBackWallet(address payable newBuyBackWallet)
		external
		authorized
	{
	...
	function updateFees(
		...
		uint256 localTotalFee = _buyBackFee
			.add(_liquidityFee)
			.add(_dividendFee)
			.add(_burnFee);
		require(localTotalFee <= 4000); // 40% of fees
	...
	function setIgnoreTokenToDividends(address tokenAddress, bool isIgnored)
		external
		authorized
		returns (bool)
	{
	...
	function setIsMaxTokenByWalletExempt(address holder, bool exempt)
		external
		authorized
	{
	...
	function setLiquidityBNB(bool _useLiquidityBNB) public authorized{
		useLiquidityBNB = _useLiquidityBNB;
	}
	...
	function _checkPrerequisites(
		...
		if (!isTradingEnabled) {
			require(
				isAllowedDuringDisabled[to] || isAllowedDuringDisabled[from]
			);
		}
	...
	function _transfer(
		...
		if (takeFee) {
			...
			uint256 fees = (amount.mul(localFeesRation)).div(100);
	...
	function _executeSwap(uint256 tokens, uint256 native) private {
		...
		bool isSwaped = swapTokensForNative(swapTokensTotal);
		if(!isSwaped) return;
		...
		if (liquidityFee > 0 && nativeLiquidity > 0) {
			if(useLiquidityBNB){
				addLiquidityBNB(addTokensLiquidity, nativeLiquidity);
			}
			else{
	...
	function swapTokensForNative(uint256 tokens) private returns (bool) {
		...
		try stellaRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
			tokens,
		...
		authorized
	{
		...
		if (stellaAmount > 0) {
			_sendToBuyBackWallet(stellaAmount.mul(recoverRate).div(100));
		}
	}

	function _sendToBuyBackWallet(uint256 amount) private {
	...
	function _setMaxWallet(uint256 _maxTokenByWallet) external authorized {
