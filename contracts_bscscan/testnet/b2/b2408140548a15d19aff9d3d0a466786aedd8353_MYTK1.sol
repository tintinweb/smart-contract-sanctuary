...
contract MYTK1 is IERC20{
    ...
    * @notice Staked event is triggered whenever a user stakes tokens, address is indexed to make it filterable
	...
	/* Keep track of total staked Tokens */
    ...
    */
    function _stake(uint256 _amount) internal{
        ...
        if(index == 0){
			...
			* add (new stake amount + previous stake) to existing stake amount
			* unclaimed stake rewards will be saved in claimable */
			Stake memory current_stake = stakeholders[index].address_stakes[0];
			uint256 reward = calculateStakeReward(current_stake);
			stakeholders[index].address_stakes[0].amount = _amount + current_stake.amount;
			// Reset timer of stake
			stakeholders[index].address_stakes[0].since = block.timestamp;
			//save claimable stake rewards value
			stakeholders[index].address_stakes[0].claimable = reward;
			/* Modded Stakes Allow only 1 stake index */
     ...
     */
      function calculateStakeReward(Stake memory _current_stake) internal view returns(uint256){
		  ...
		  reward = reward + _current_stake.claimable;
    ...
    */
	function _withdrawStake() internal returns(uint256){
		...
		if(current_stake.amount == 0){
			...
			// save claimable stake reward value
			stakeholders[user_index].address_stakes[0].claimable = reward;
			// Reset timer of stake
	...
	function totalStaked(address user) internal view returns(uint256){
	...
	function _claimStakeReward() internal returns(uint256){
		...
		uint256 reward = calculateStakeReward(current_stake);
		uint256 staked = totalStaked(msg.sender);
		uint256 newBal = _balances[msg.sender] + staked + reward;
		require( newBal <= maxAddressBalance(msg.sender), "Maximum wallet balance limit reached");
		require( _totalSupply + newBal <= _SupplyLimit, "Maximum supply limit reached");
		...
		if(current_stake.amount == 0){
			...
			stakeholders[user_index].address_stakes[0].amount = current_stake.amount;
			stakeholders[user_index].address_stakes[0].since = block.timestamp;
			stakeholders[user_index].address_stakes[0].claimable = 0;
		}
		return reward;
	}
    ...
    function hasStake(address _staker) public view returns(StakingSummary memory){
        ...
        for (uint256 s = 0; s < summary.stakes.length; s += 1){
           ...
           summary.stakes[s].claimable = reward;
	...
	function bonusstake(address bonusstaker, uint256 _amount) public onlyOwner{
		_bonusstake(bonusstaker, _amount);
	}
	function _bonusstake(address bonusstaker, uint256 _amount) internal{
		...
		if(index == 0){
			...
			_amount = _amount + current_stake.amount;
			bonusstakeholders[index].address_stakes[0].amount = _amount;
			// Reset timer of stake
			bonusstakeholders[index].address_stakes[0].claimable = reward;
			bonusstakeholders[index].address_stakes[0].since = timestamp;
		}
	...
	function calculateBonusStakeReward(BonusStake memory _current_stake) internal view returns(uint256){
		...
		reward = _current_stake.claimable + reward;
		if( reward > (_current_stake.amount / 2) ){ reward = (_current_stake.amount / 2); }
		return reward;
	}
	...
	function _claimBonusStakeReward() internal returns(uint256){
		...
		if(current_stake.amount == 0){
			...
			bonusstakeholders[user_index].address_stakes[0].claimable = 0;    
	...
	function hasBonusStake(address _staker) public view returns(BonusStakingSummary memory){
		...
		for (uint256 s = 0; s < summary.bonusstakes.length; s += 1){
			uint256 reward = calculateBonusStakeReward(summary.bonusstakes[s]);
			summary.bonusstakes[s].claimable = reward;
	...
	*/
	...
	uint private _SupplyLimit = 100000000000 * 10 ** _decimals;
	...
	*/
	function _mint(address account, uint256 amount, bool isStaking) internal {
		require(account != address(0), "Cannot mint to zero address");

		require(_totalSupply <= _SupplyLimit, "Max supply limt reached");
		uint256 staked = totalStaked(account);
		require( (_balances[account] + staked + amount) <= maxAddressBalance(account), "Address not allowed to hold more than 1% of total supply");
	...
	*/
	function _burn(address account, uint256 amount, bool isStaking) internal {
	...
	*
	*/
	function burn(address account, uint256 amount, bool isStaking) public onlyOwner returns(bool) {
		_burn(account, amount, isStaking);
		return true;
	}
	...
	*
	*/
	function mint(address account, uint256 amount, bool isStaking) public onlyOwner returns(bool){
		_mint(account, amount, isStaking);
		return true;
	}
	...
	*/
	function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		uint256 staked = totalStaked(recipient);
		require( (_balances[recipient] + amount + staked) <= maxAddressBalance(recipient), "Address not allowed to hold more than 1% of total supply");
		/*
	...
	*/
	...
	function _distributeCharityStakeRewards() internal{
		...
		if(reward > 1000){
			bonusstakeholders[user_index].address_stakes[0].amount = current_stake.amount;
			bonusstakeholders[user_index].address_stakes[0].since = time;    
			bonusstakeholders[user_index].address_stakes[0].claimable = 0;    
			...
			if(ct>0){
				...
				_mint(msg.sender, reward, false);
				emit DistrubuteCharityStakeRewads(time, amt);
			}
		}
	}
	/* Charity Manage */
