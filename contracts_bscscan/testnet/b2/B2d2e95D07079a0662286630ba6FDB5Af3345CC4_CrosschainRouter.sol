...
library CrosschainLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'47dc14a6850c8f5ab2590ae3a34188c8ddca7bb754b71d3bec6af38b8b8d9513' // init code hash
            ))));
    }
...
contract CrosschainRouter is ICrosschainRouter02 {
    using SafeMath for uint;
    ...
    function addLiquidityBNB(
        ...
        address pair = CrosschainLibrary.pairFor(factory, token, WBNB);
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, CrosschainLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function swapExactBNBForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WBNB, 'CrosschainRouter: INVALID_PATH');
        amounts = CrosschainLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'CrosschainRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWBNB(WBNB).deposit{value: amounts[0]}();
        assert(IWBNB(WBNB).transfer(CrosschainLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactBNB(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WBNB, 'CrosschainRouter: INVALID_PATH');
        amounts = CrosschainLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'CrosschainRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, CrosschainLibrary.pairFor(factory, path[0], path[1]), amounts[0]
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, CrosschainLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWBNB(WBNB).transfer(CrosschainLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IBEP20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IBEP20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'CrosschainRouter: INSUFFICIENT_OUTPUT_AMOUNT');
