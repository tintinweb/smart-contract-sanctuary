...
contract Mark72 is Ownable {
    ...
    constructor() {
        ...
        PROTOCOL_LISTING_FEE = 1e14; // 1e17
        PROTOCOL_BUYING_FEE = 200;
        MIN_PRICE = 1e14; // 1e17
        ROYALTY_FEE = 300;
        // 6h
        BID_WITHDRAW_CD = 300; // 1200 * 6
        // 2 months
        MAX_OFFER_BLOCKS = 28800; // 28800 * 7
    ...
    function getOffers(bool sorted, uint start, uint count) public view returns (address[] memory addresses, uint[] memory ids){
        uint l = whitelist.length;
        uint totalL;

        for (uint i = 0; i < l; i++) {
            totalL += contractToIds[whitelist[i]].length;
        }
    ...
    function quickSort(OfferPreview[] memory preview) internal pure {
        if (preview.length > 1) {
            quickPart(preview, 0, preview.length - 1);
        }
    }

    function quickPart(OfferPreview[] memory preview, uint low, uint high) internal pure {
        if (low < high) {
            uint pivotVal = preview[(low + high) / 2].madeBlock;
            ...
            if (low < high1) {
                quickPart(preview, low, high1);
            }
            high1++;
            if (high1 < high) {
                quickPart(preview, high1, high);
            }
        }
    }
