...
contract cherry is IERC20, Ownable, Pausable {
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    string constant public name = "Babycherry";
    string constant public symbol = "cherry";
    uint8 constant public decimals = 18;

    uint256 private _totalSupply = 1E11 * 1E18;

    address public beneficiaryAddress;
    uint8 public feePercentage = 20;
    ...
    function setFeePercentage(uint8 feePercentage_) external onlyOwner {
        require(feePercentage_ <= 30, "polyplay: transaction fee percentage exceeds 30");
        require(feePercentage_ >= 0, "Polyplay: transaction fee percentage equals 0");
    ...
    function blacklist(address[] calldata addresses, bool isBlacklist) external {
        require(hasBlacklistPermission[msg.sender], "Polyplay:insufficient permission");
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "Polyplay: transfer from blacklisted address");
        require(!isBlacklisted[recipient], "Polyplay: transfer to blacklisted address");
        require(!isBlacklisted[tx.origin], "Polyplay: transfer called from blacklisted address");
