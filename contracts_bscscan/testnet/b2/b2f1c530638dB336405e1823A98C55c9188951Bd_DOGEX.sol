...
contract DOGEX is Context, IERC20, IERC20Extented, Ownable {
    ...
    uint256 private _maxSellAmountBNB = 100000000000000000;// 0.1 eth//2000000000; // 20 BNB
    uint256 private _minBuyBNB = 1000000000000000;// 0.001 eth //1000000; // 0.01 BNB
    uint256 private _minSellBNB = 1000000000000000;// 0.001 eth //1000000; // 0.01 BNB
    uint256 private _taxFreeBlocks = 300;//3600; // 1 hour
    ...
    constructor() {
        ...
        _maxWalletAmount = _tTotal.div(10); // 10%
    ...
    function updateFee(uint256 currentPrice) private {
        if(block.timestamp >= _goldenHourStartBlock && block.timestamp <= _taxFreeWindowEnd) {
            enableGoldenHourFees();
        }
        if (block.timestamp > _taxFreeWindowEnd && block.timestamp <= _slideEndBlock) {
            restoreAllFee();
            enableSlideFees();
        }
    ...
    function _transferStandardBuy(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rReflection, uint256 tTransferAmount, uint256 tBuyback, uint256 tMarketing, uint256 tReflection, uint256 tLiquidity, uint256 tDev) = _getValuesBuy(tAmount);
    ...
    function _transferStandardSell(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rReflection, uint256 tTransferAmount, uint256 tBuyback, uint256 tMarketing, uint256 tReflection, uint256 tLiquidity, uint256 tDev) = _getValuesSell(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
    ...
    // Sell GetValues
    function _getValuesSell(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        SellBreakdown memory sellFees = _getTValuesSell(tAmount, _sellBuybackFee, _sellMarketingFee, _sellReflectionFee, _sellLiquidityFee, _sellDevFee);
        uint256 currentRate = _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rReflection) = _getRValuesSell(tAmount, sellFees.tBuyback, sellFees.tMarketing, sellFees.tReflection, sellFees.tLiquidity, sellFees.tDev, currentRate);
        return (rAmount, rTransferAmount, rReflection, sellFees.tTransferAmount, sellFees.tBuyback, sellFees.tMarketing, sellFees.tReflection, sellFees.tLiquidity, sellFees.tDev);
    }
    ...
    // Buy GetValues
    function _getValuesBuy(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        BuyBreakdown memory buyFees = _getTValuesBuy(tAmount, _buyBuybackFee, _buyMarketingFee, _buyReflectionFee, _buyLiquidityFee, _buyDevFee);
        uint256 currentRate = _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rReflection) = _getRValuesBuy(tAmount, buyFees.tBuyback, buyFees.tMarketing, buyFees.tReflection, buyFees.tLiquidity, buyFees.tDev, currentRate);
        return (rAmount, rTransferAmount, rReflection, buyFees.tTransferAmount, buyFees.tBuyback, buyFees.tMarketing, buyFees.tReflection, buyFees.tLiquidity, buyFees.tDev);
    }
    ...
    function enableGoldenHourFees() private {
        _previousBuyMarketingFee = _buyMarketingFee;
        _previousBuyBuybackFee = _buyBuybackFee;
        _previousBuyReflectionFee = _buyReflectionFee;
        _previousBuyLiquidityFee = _buyLiquidityFee;
        ...
        _sellMarketingFee = _floorMarketingFee;
        _sellBuybackFee = _floorBuybackFee;
        _sellReflectionFee = _floorReflectionFee;
        _sellLiquidityFee = _floorLiquidityFee;
        _sellDevFee = _floorDevFee;
    }
    
    function enableFloorFees() private {
        _previousSellMarketingFee = _sellMarketingFee;
        _previousSellBuybackFee = _sellBuybackFee;
        _previousSellReflectionFee = _sellReflectionFee;
        _previousSellLiquidityFee = _sellLiquidityFee;
    ...
    function enableSlideFees() private {
        _previousSellMarketingFee = _sellMarketingFee;
        _previousSellBuybackFee = _sellBuybackFee;
        _previousSellReflectionFee = _sellReflectionFee;
        _previousSellLiquidityFee = _sellLiquidityFee;
        ...
        _sellMarketingFee = _slideMarketingFee;
        _sellBuybackFee = _slideBuybackFee;
        _sellReflectionFee = _slideReflectionFee;
        _sellLiquidityFee = _slideLiquidityFee;
        _sellDevFee = _slideDevFee;
    }
