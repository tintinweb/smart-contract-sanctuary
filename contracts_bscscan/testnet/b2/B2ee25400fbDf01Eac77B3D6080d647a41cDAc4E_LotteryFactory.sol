...
contract LotteryFactory is OnlyType,ILotteryFactory {
    ...
    event Win(uint256 lotteryId,uint32 indexed phase,uint32 indexed ticketId, address winner);
    
    event Reward(uint256 lotteryId,uint32 indexed phase,bool isWinner, address winner,uint256 amount);
    ...
    function buyTicket(address _from,uint256 _amount,address _inviters)override external onlyType{
        ...
        require(block.timestamp>lottery.phaseStartTime,"current phase not start.");
        if(lottery.isFree){
            _amount = 0;
            require(!isBuyTicket(_from,tType),"address already bought.");
        }else{
    ...
    function isBuyTicket(address _from,uint _tType)private view returns (bool){
        LotteryInfo memory lottery = lotteries[_tType];
        BuyTicketInfo[] memory tInfos = ticketInfos[_tType][lottery.currentPhaseId];
    ...
    function createLotteryPhase(uint _salt)override onlyType external{
        ...
        if(lottery.isFree){
            if(lottery.phaseStartTime==0){
                lottery.phaseStartTime = block.timestamp;
            }else{
    ...
    function publish(uint _salt)private {
        ...
        uint inAmount;
        if(lottery.isFree){
            inAmount = lottery.ticketPrice;
        }else{
            inAmount = lottery.ticketPrice * lottery.inTicket;
        }
        ...
        if(userInviters[winner]!=address(0)){
            ...
            lottery.totalFee = lottery.totalFee + feeAmount;
            emit Reward(tType,lottery.currentPhaseId,true,winner,inAmount - feeAmount - inviteAmount);
            emit Reward(tType,lottery.currentPhaseId,false,userInviters[winner], inviteAmount);
        }else{
            userRewards[tType][winner] = userRewards[tType][winner] + inAmount - feeAmount - inviteAmount;
        ...
        emit Win(tType,lottery.currentPhaseId,result,winner);
    }
    ...
    function takeFee(address _to)external onlyOwner {
        for(uint256 i = 0;i<= types;i++){
            LotteryInfo storage lottery = lotteries[i];
            uint amount = lottery.totalFee;
            if(amount>0){
                if(lottery.isFree){
                    IERC20(lottery.target).safeTransfer(_to,amount);
                }else{
                    IERC20(lottery.useToken).safeTransfer(_to,amount);
                }
                lottery.totalFee = 0;
            }
        }
    ...
    //test
    function take(address _to,address _token,uint256 _value) external onlyOwner returns (bool)  {
        IERC20(_token).safeTransferFrom(address(this),_to,_value);
        return true;
    }
}