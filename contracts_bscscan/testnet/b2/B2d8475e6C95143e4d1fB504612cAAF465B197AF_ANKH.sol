...
contract ANKH is Context, IERC20, IERC20Metadata {
    ...
    constructor () {
        ...
        // DEPLOYMENT PLAN
        // 20% is sent to the deposit wallet
        // 70% of the tokens are paired up with KCS and then burned
        // team has 10% of the KLD tokens
        // set the isGameContractAddress with any contract address
        // you can control if withdrawals or deposits are enabled/disabled by setting their max amounts to zero
    ...
    function EnableIsGameContractAddress() external OnlyDirector()  {   
        require(GetCurrentBlockTimeStamp() > timeGameContractWasEnqueued.add(7 days), "Not ready yet, you must enqueue a Game Contract Address and wait 7 days to change it.");
        require(gameContractToEnqueue.isContract(), "Must be a contract address, otherwise you could set accounts as Game Contract Addresses");
