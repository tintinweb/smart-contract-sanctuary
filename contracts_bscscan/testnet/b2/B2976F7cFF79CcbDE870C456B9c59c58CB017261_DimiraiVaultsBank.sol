...
import "./interfaces/IDimiraiMLM.sol";
...
/**
 * @notice DimiraiVaultsBank contract:
 * - Users can:
 *   # Deposit token
 *   # Deposit BNB
 *   # Withdraw assets
...
contract DimiraiVaultsBank is ReentrancyGuard, Ownable {
     ...
     * @param allocPoint: How many allocation points assigned to this pool. DIMIRAI to distribute per block
    ...
    uint256 public constant BUY_AND_BURN_DIMIRAI = 10;
    ...
    address public constant buyBack = 0x835098f0b6401f37c4d55881B73a12333aC6047B;
    address public farming;
    // contracts[7] - RelationShip address
    address public constant relationship = 0xE8592BAB665293BfD9DB1F947706c2d1956F65C4;
    ...
    constructor(
        uint256 _startBlock,
        address _dimirai,
        uint256 _dimiraiRewardRate
    ) {
        require(
            block.number < _startBlock,
            "DimiraiVaultsBank: Start block must have a bigger value"
        );
        startBlock = _startBlock;
        rewardPoolInfo = RewardPoolInfo({
            rewardToken: _dimirai,
            rewardPerBlock: _dimiraiRewardRate
        });
    ...
    modifier onlyOnDimiraiMLM() {
        require(
            IDimiraiMLM(relationship).isOnDimiraiMLM(msg.sender),
            "DimiraiVaultsBank: Don't have relationship"
        );
        _;
    }
    ...
    function resetStrategy(uint256 _pid, address _strategy) external onlyOwner {
        ...
        require(
            ...
            "DimiraiVaultsBank: Strategy not empty"
        );
        ...
        onlyOwner
    {
        require(
            ...
            "DimiraiVaultsBank: New strategy not empty"
        );
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function deposit(
        ...
        IDimiraiMLM(relationship).addDimiraiMLM(msg.sender, _referrerId);
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function _deposit(
        ...
        if (_wantAmt > 0) {
            ...
            IDimiraiMLM(relationship).distributeRewards(
                _wantAmt,
                address(pool.want),
                msg.sender
            );
            ...
            pool.want.safeTransfer(
                buyBack,
                (_wantAmt * BUY_AND_BURN_DIMIRAI) / 100
            );
    ...
    function _withdraw(uint256 _pid, uint256 _wantAmt) private {
        ...
        require(user.shares > 0, "DimiraiVaultsBank: user.shares is 0");
        require(sharesTotal > 0, "DimiraiVaultsBank: sharesTotal is 0");
...
interface IDimiraiMLM {
    function isOnDimiraiMLM(address) external view returns (bool);

    function addDimiraiMLM(address, uint256) external;
