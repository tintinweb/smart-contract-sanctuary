...
contract RecyclePoolReturnImmediate is AccessControlEnumerable, ReentrancyGuard{
    ...
    function recycleMoreTimes(uint256[] calldata heroTokenIds) public{
        for(uint i = 0; i < heroTokenIds.length; i++){
            recycle(heroTokenIds[i]);
        }
    }
    ...
    function recycle(uint256 heroTokenId) nonReentrant public {
        ...
        contractManager.sgcToken().mint(caller, returnTokenAmount);
