...
contract VOYCEVoteFactory is Ownable {
    ...
    function setFactorySettings(
        address voyceToken_,
        address voceVoteImplementation_
    ) external onlyOwner {
    ...
    function createPoll(
        ...
        voyceVote.initialize(startTime, endTime, quorum, charityNames, voyceToken, VOYCEVote(voceVoteImplementation).amountToHold());
    ...
    {
        result = new address[](length);
        startIndex = startIndex > voyceVotes.length - 1
            ? voyceVotes.length - 1
            : startIndex;
    ...
    function getLastVoyceVote() external view returns (address lastVoyceVote) {
        if (voyceVotes.length > 0)
            lastVoyceVote = address(voyceVotes[voyceVotes.length - 1]);
        else lastVoyceVote = address(0);
    }
}
