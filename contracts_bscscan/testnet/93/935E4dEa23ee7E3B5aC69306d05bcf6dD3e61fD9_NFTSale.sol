    ...
    PausableUpgradeable
{
    ...
    CryptiaNFT721 private _cryptiaNFT721Contract;
    ...
    modifier onlyOwnerToken (uint256 tokenId) {
        require(address(this) == _cryptiaNFT721Contract.ownerOf(tokenId), "token-not-sell");
        require(msg.sender == _informationOf[tokenId].seller, "not-owner-token");
        _;
    }
    ...
    function checkCurrency(address currency) public view returns (bool, uint256) {
    ...
    function setPaymentCurrency(address currency, bool accepted, uint256 maxValue)
        public
        onlyOwner
    {
        if(accepted == true){
    ...
    function sellNFT(
        ...
        require(msg.sender == _cryptiaNFT721Contract.ownerOf(tokenId), "not-owner-token");
        require(_cryptiaNFT721Contract.getApproved(tokenId) == address(this), "not-approval");
        require(_currencyWhitelist[currency], "currency-invalid");
        require(_informationOf[tokenId].price == 0 && price > 0 && price <= _maxValueOfCurrency[currency], "price-invalid");
        ...
        onlyOwnerToken(tokenId)
    {
        require(_informationOf[tokenId].price > 0 
                && newPrice > 0 
                && newPrice <= _maxValueOfCurrency[_informationOf[tokenId].currency], 
                "price-invalid");
        _informationOf[tokenId] = Information(
    ...
    function purchaseNFT(uint256 tokenId) public payable whenNotPaused {
        require(msg.sender != address(this) && msg.sender != _informationOf[tokenId].seller, "caller-invalid");
        require(address(this) == _cryptiaNFT721Contract.ownerOf(tokenId), "token-not-sell");
        require(_informationOf[tokenId].price > 0);
...
contract CryptiaNFT721 is ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function setDataNFT(uint256 tokenId, string memory data) public onlyOperator {
