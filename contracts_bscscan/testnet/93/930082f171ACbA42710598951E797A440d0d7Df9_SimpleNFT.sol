...
contract SimpleNFT is ERC721, ERC721URIStorage, ERC2981Collection, ContractURI, IMAX721, ReentrancyGuard, ERC165Storage, DeveloperV2, Ownable {
  ...
  event UpdatedTokenAddress(IERC20 _old, IERC20 _new);
  event UpdatedTotalChoices(uint _old, uint _new);

  // @notice bytes4 constants for ERC165, calculated and audited in v2.0->v2.1
 ...
 */
  ...
  function publicMint(uint256 amount) public nonReentrant() {
    ...
    // @notice this approves this contract then transfers ERC20 token to 0xdEaD (IERC20.sol)
    // @notice Effects Phase
    uint tokenAmount = amount * mintFees;
    token.approve(address(this), tokenAmount);
    token.transferFrom(msg.sender, DEAD_ADDRESS, tokenAmount);
  ...
  //  as a nonce, using block.timestamp, block.difficulty, and msg.sender then
  //  modulo division by total number of NFT's
  function mintID() internal view returns (string memory) {
    uint value = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, msg.sender, address(this), _tokenIdCounter.current()))) % totalChoices;
 ...
 */
  ...
  //  of secondary sales
  // ERC165 datum setRoyaltyInfo(address,uint256) => 0xe2e784d5
 ...
 */
  ...
  // @notice will set the IERC20 value of token, and emit an event
  // @param address _token - address of the token to change
  // ERC165 datum setTokenAddress(address) = > 0x26a4e8d2
  function setTokenAddress(address _token) public onlyDev {
    IERC20 old = token;
    token = IERC20(_token);
    emit UpdatedTokenAddress(old, token);
  }

  // @notice this will set the available number of NFT Metadata's
  // @param uint256 _amount - number of available metadatas
  // ERC165 datum setTotalChoices(uint256) => 0xf727ca49
  function setTotalChoices(uint256 _number) public onlyDev {
    uint256 old = totalChoices;
    totalChoices = _number;
    emit UpdatedTotalChoices(old, totalChoices);
  }
  ...
  // @param uint256 _amount - set number to mint
  // ERC165 datum setMintSize(uint256) => 0x4a886ad2
  ...
  //  (to user address) 
  // @param address _user - user address to input
  // @param uint256 _amount - amount of token to transfer
  // @param address _token - token contract address
  // ERC165 datum sweepERCToAddress(address,uint256,address) => 0xe08b1b63
  function sweepERCToAddress(address _user, uint256 _amount, address _token) public onlyDev {
    IERC20(_token).transferFrom(address(this), _user, _amount);
  }

  ///
  /// @dev these are all the Interface Overrides 
  ///
