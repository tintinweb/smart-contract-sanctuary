...
contract Staking is Ownable, ReentrancyGuard {
    ...
    function updateManager(address _address, bool _status) external onlyOwner {
        require(_address != address(0), "UpdateManager:: Manager can't be the zero address");
        require(managers[_address]._isActive != _status, "UpdateManager:: No Change in Status");
        managers[_address]._isActive = _status;
        emit ManagerUpdated(_address, _status);
    }

    modifier onlyManager {
        require(managers[msg.sender]._isActive, "Manager:: Unauthorized Access");
        _;
    }
    
    function stakeAndLock(address _to, uint256 _amount) public nonReentrant onlyManager {
    ...
    function _setAmounts(address _to, uint256 _amount) internal {
        ...
        for(uint256 i = 1; i<= 100; i++) {
            user.amounts.push(_getAmount(_amount));
        }
    }
    ...
    function _setStatus(uint256 _size, bool _value) internal pure returns (bool[] memory isClaimed) {
    ...
    function _setMilestones(uint256 _size) internal view returns (uint256[] memory milestones) {
    ...
    function harvest() external nonReentrant {
        ...
        require(unlockedTokens > 0, "Harvest:: Withdrawing too early");
