...
library ECDSA {
     ...
     * be too long), and then calling `toBnbSignedMessageHash` on it.
     */
    function recover(bytes32 hash, bytes memory signature) internal pure returns (address) {
        ...
        // unique. Appendix F in the Bnbereum Yellow paper (https://ethereum.github.io/yellowpaper/paper.pdf), defines
    ...
    /**
     * @dev Returns an Bnbereum Signed Message, created from a `hash`. This
     ...
     * See `recover`.
     */
    function toBnbSignedMessageHash(bytes32 hash) internal pure returns (bytes32) {
        // 32 is the length in bytes of hash,
        // enforced by the type signature above
        return keccak256(abi.encodePacked("\x19Bnbereum Signed Message:\n32", hash));
    }
}
...
contract Stixex_USDT is BlackList, Pausable {
    ...
    IBEP20 private _usdt;                               // interface of usdt token
    ...
    constructor (IBEP20 usdt) public {
    ...
    function toBnbSignedMessageHash(bytes32 hash) public pure returns (bytes32) {
        return hash.toBnbSignedMessageHash();
    }
    ...
    function getSignerAddress(
        ...
        bytes32 message = ECDSA.toBnbSignedMessageHash(dataHash);
