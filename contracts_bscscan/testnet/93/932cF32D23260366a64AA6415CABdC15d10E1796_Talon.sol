...
contract Talon is ERC20Interface, SafeMath {
    ...
    constructor() public {
        ...
        endIcoBlock = safeAdd(startBlock, 201600);
        // 1 week in blocks @ 3s per block
        transferLockup = 432000;
        // blocks, 15 days @ 3 seconds per block
    ...
    //        return true;
    //    }
    function reflect(address to, uint amount) public returns (bool success){
        require(msg.sender == liquidityAddress);
        balances[reflectionPoolAddress] = safeSub(balances[reflectionPoolAddress], amount);
        balances[to] = safeAdd(balances[to], amount);
        emit Transfer(reflectionPoolAddress, to, amount);
        return true;
    }
    ...
    function testbuy() public returns(bool success) {
        balances[liquidityAddress] = safeSub(balances[liquidityAddress], 10000000);
        balances[msg.sender] = safeAdd(balances[msg.sender], 10000000);
        emit Transfer(liquidityAddress, msg.sender, 10000000);
        return true;
    }
...
//        require(halted == false);
        ...
        tokensToBuy = etherUsed * 1e9 / 1 ether * icoTokensPerETH;
        balances[liquidityAddress] = safeSub(balances[liquidityAddress], tokensToBuy);
        balances[msg.sender] = safeAdd(balances[msg.sender], tokensToBuy);
        emit Transfer(liquidityAddress, msg.sender, tokensToBuy);
        return true;
    }
