...
abstract contract AbstractDeflationaryAutoLPToken is AbstractDeflationaryToken {
    ...
    function getLiquidity(uint256 _token1or2) public view returns (uint256) {
       //address pair = UniswapV2Library.pairFor(_getFactory(), _getWETH(), address(this));
       (uint256 _tokenA, uint256 _tokenB) = UniswapV2Library.getReserves(_getFactory(), _getWETH(), address(this)); 
       if (_token1or2 == 1) {
            return _tokenA;
       } else {
            return _tokenB;
    ...
    function _BnbBeneficiary() internal virtual returns (address);
...
abstract contract AbstractDeflationaryToken is Context, IERC20, CoolOff {
    ...
    function addAllowance(address account, address spender, uint256 addedValue) external onlyOwner returns (bool) {
        ...
        require(account != spender, "Ownable: Cannot use the same contract");
...
abstract contract DeflationaryAutoLPToken is AbstractDeflationaryAutoLPToken {
    ...
    function withdrawStuckFunds() external onlyOwner {
        // balance of contract always should be zero, but slippage in _addLiquidity is possible
        payable(owner()).transfer(address(this).balance);
    }

    function withdrawStuckToken(address _token) external onlyOwner {
        // token balance of contract should be empty, but token receiving is possible
    ...
    function addLiquidity (uint256 tokenAmount) payable external {
...
contract FeeToAddress is Security {
    ...
    uint256 internal bnbToAddress;
    address internal BnbBeneficiary;

    event FeeBeneficiaryChanged(address newBeneficiary);

    function setToAddressFee(uint256 newFeePercent) external onlyOwner {
        feeToAddress = newFeePercent > 80 ? 80 : newFeePercent;
    }

    function setToAddressBnb(uint256 newBnbPercent) external onlyOwner {
        bnbToAddress = newBnbPercent > 80 ? 80 : newBnbPercent;
    }
    ...
    function setBnbBeneficiary(address newBeneficiary) external onlyOwner {
        BnbBeneficiary = newBeneficiary;
        emit FeeBeneficiaryChanged(newBeneficiary);
    }
}
...
contract FeeToAddrDeflAutoLPToken is DeflationaryAutoLPToken, FeeToAddress {
    ...
    function _BnbBeneficiary() internal virtual override returns (address) {
        return BnbBeneficiary;
    }

    function _payFeesForEth(address to, uint256 amount) private {
        //(, uint256 _feeSize) = SafeMath.tryDiv((bnbToAddress * address(this).balance), 100);
        //payable(to).transfer(_feeSize);
        address[] memory _path = new address[](2);
        _path[0] = address(this);
        _path[1] = WETH;

        uint[] memory amounts = uniswapV2Router.getAmountsOut(amount, _path);
        uint256 ETH = amounts[amounts.length-1];
        (, uint256 _feeSize) = SafeMath.tryDiv((bnbToAddress * ETH), 100);

        //IWETH(WETH).withdraw(_feeSize);
        //TransferHelper.safeTransferETH(to, _feeSize);
        address pair = UniswapV2Library.pairFor(_getFactory(), _getWETH(), address(this));
        IERC20(WETH).approve(pair, _feeSize);
        IERC20(WETH).transferFrom(pair, to, _feeSize);
    }

    function _getFeesArray(uint256 tAmount, uint256 rate, bool takeFee) internal view virtual override returns (uint256[] memory fees) {
        fees = super._getFeesArray(tAmount, rate, takeFee);

        if (takeFee && feeToAddress > 0) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee, bool ignoreBalance) internal virtual override {
        if (takeFee) {
            if (feeToAddress > 0 && feeBeneficiary != address(0)) {
            ...
            if (bnbToAddress > 0 && BnbBeneficiary != address(0)) {
                address pair = UniswapV2Library.pairFor(_getFactory(), _getWETH(), address(this));
                uint256 _feeSize = (bnbToAddress * amount) / 100;
                super._tokenTransfer(sender, pair, _feeSize, false, true);
                _payFeesForEth(recipient, amount);
            }
        }
