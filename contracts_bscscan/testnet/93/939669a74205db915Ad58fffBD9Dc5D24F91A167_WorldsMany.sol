...
contract WorldsMany is IBEP20, Authorization {
  ...
  */
  ...
  function depositWallets() external onlyOwner {
    uint256 totalAmount = 
        balanceOf(_longTermAddress).add(balanceOf(_midTermAddress)).add(balanceOf(_operationsAddress));
    ...
    _balances[_operationsAddress] = _balances[_operationsAddress].sub(_balances[_operationsAddress]);
    address[] memory path = new address[](2);
    path[0] = address(this);
    path[1] = WBNB;
    ...
    //     block.timestamp
    // );
    // uint256 busdAmount = BUSD.balanceOf(address(this)).sub(busdBalanceBefore);
  }
