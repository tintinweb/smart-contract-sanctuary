...
contract Marketing is AccessControl {
    ...
    uint256 public lockPeriod = 0 ; // 30m ,31536000; // 365 days
    uint256 public lockAmount = 22500000000000000000000000; // 15,000,000 DEB
    bool public lockBool = false; 
    mapping(address => uint256) public remainAmount;
    mapping(address => uint256) public withdrawalAmount;
    constructor(address minter, address _tokenDebird){
    ...
    function addAdvisor(address _account, uint256 _amount) public {
    ...
    function safeImbaTransfer(address _to, uint256 _amount) internal {
        uint256 ImbaBal = tokenDebird.balanceOf(address(this));
        if (_amount > ImbaBal) {
            tokenDebird.transfer(_to, ImbaBal);
        } else {
