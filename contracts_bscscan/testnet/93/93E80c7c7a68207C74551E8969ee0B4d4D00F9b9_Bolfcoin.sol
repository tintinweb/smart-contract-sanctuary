...
contract Bolfcoin is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "BOLFCOIN";
    string constant _symbol = "BOLF";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 100 * 1e9 * (10 ** _decimals);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if((sender != owner ) && recipient == pair){
            require(tradingEnabled , "canot sell trading needs to be enabled");
        }
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(totalFee.sub(DevFee)).div(feeDenominator);
        ...
        return amount.sub(feeAmount).sub(DevfeeAmount);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = swapThreshold.mul(dynamicLiquidityFee).div(totalFee.sub(DevFee)).div(2);
        ...
        uint256 amountBNBLiquidity = amountBNB.mul(dynamicLiquidityFee).div(totalBNBFee.sub(DevFee)).div(2);
        uint256 amountBNBReflection = amountBNB.mul(reflectionFee).div(totalBNBFee.sub(DevFee));
