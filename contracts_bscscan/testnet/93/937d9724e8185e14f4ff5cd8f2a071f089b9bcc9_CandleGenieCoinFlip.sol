...
contract CandleGenieCoinFlip is Ownable, ReentrancyGuard 
{
    ...
    uint256 public maximumBetAmount = 5000000000000000000;
    ...
    function Flip(int position) external payable nonReentrant notContract 
    {
        ...
        if (currentBetId > 3)
        {
            require(Bets[UserBets[msg.sender][currentBetId - 1]].betAmount == 0, "Wait at least 3 blocks to make a new bet. No brute plis");
            require(Bets[UserBets[msg.sender][currentBetId - 2]].betAmount == 0, "Wait at least 2 blocks to make a new bet. No brute plis");
            require(Bets[UserBets[msg.sender][currentBetId - 3]].betAmount == 0, "Wait at least 1 blocks to make a new bet. No brute plis");
        }
        ...
        uint256 randomNumber = seed - (seed / 10) * 10;
         
        Position result;
        if (randPivot == 0)
        {
            result = randomNumber < 4 ? Position.Heads : Position.Tails;  
            randPivot = 1;
        }
        else if (randPivot == 1)
        {
