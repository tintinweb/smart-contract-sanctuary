...
contract NFTWithPools is IERC721, ITRC165, IERC721Metadata, IERC721Enumerable {
    ...
    string internal _name = "NFTWithPools";  // TODO
    string internal _symbol = "PM";       // TODO

    uint256 internal totalMinted = 0;

    uint256 public constant MINTING_LIMIT = 50; // TODO

    uint256 public mintingPrice = 2e15;  // TODO
    ...
    uint256 public firstStagePool = 0;
    uint256 public secondStagePool = 0;

    uint256 public firstStageHolders = 0;
    uint256 public secondStageHolders = 0;
    ...
    struct UserStructure {
        ...
        uint256 lastKnownReward;
        uint256 firstStageReward;
    }
    ...
    mapping(address => bool) public tokenClaimer;
    ...
    function _airdropMintTo(address _tokenReceiver, uint256 amountToMint) external reentrancyGuard mintingEnabled onlyMinter returns(uint256[MAXIMUM_MINT_LIMIT] memory) {
    ...
    function _mint(address _to) internal notZeroAddress(_to) returns (uint256 _mintedTokenId) {
        ...
        uint256 fsh = (firstStageHolders > 0) ? firstStageHolders : 1;
        uint256 ssh = (secondStageHolders > 0) ? secondStageHolders : 1;
        firstStagePool += mintingPrice * firstStageNom / fsh / firstStageDenom;
        secondStagePool += mintingPrice * secondStageNom / ssh / secondStageDenom;
        ...
        if (users[_to].exists) {
            ...
            if (users[_to].mintAmount >= firstStage && users[_to].level < 1) {
                users[_to].level = 1;
                firstStageHolders += 1;
                users[_to].lastKnownReward = firstStagePool;
            } else if (users[_to].mintAmount >= secondStage && users[_to].level < 2) {
                users[_to].level = 2;
                secondStageHolders += 1;
                firstStageHolders -= 1;
                users[_to].firstStageReward = firstStagePool - users[_to].lastKnownReward;
                users[_to].lastKnownReward = secondStagePool;
            }
    ...
    function getCurrentUserReward(address user) external view returns(uint256) {
        if (
            ...
            if (users[user].level == 1) {
                return firstStagePool - users[user].lastKnownReward;
            } else {
                return secondStagePool - users[user].lastKnownReward;
            }
        } else {
            return 0;
        }
    ...
    function withdrawHolderPart() external {
        ...
        if (users[receiver].level == 1) {
            toPayout = firstStagePool - users[receiver].lastKnownReward;
            users[receiver].lastKnownReward = firstStagePool;
        }

        if (users[receiver].level == 2) {
            toPayout = secondStagePool - users[receiver].lastKnownReward;
            if (users[receiver].firstStageReward != 0) {
                toPayout += users[receiver].firstStageReward;
                users[receiver].firstStageReward = 0;
            }
            users[receiver].lastKnownReward = secondStagePool;
        }

        totalPayouts += toPayout;
    ...
    function _setMintingPrice(uint256 newPrice) external onlyContractOwner reentrancyGuard {
    ...
    function addTokenClaimer(address _tokenClaimer) external onlyContractOwner {
    ...
    modifier transferApproved(uint256 _tokenId) {
        ...
        require(
            ...
            ((ownerToOperators[tokenOwner][msg.sender] == true) && tokenOwner != address(0)), 
