...
contract GoldFurnace is ERC721Holder, Ownable, Divestor {
    ...
    mapping(uint8 => LevelInfo) public levelInfoes;
    ...
    function swap(uint8 mode_, uint amount_) public onlyOpen returns (uint) {
        ...
        require(amount_ % levelInfoes[mode_].amount == 0, "wrong amount");

        uint number = amount_ / levelInfoes[mode_].amount;
        uint cardId = levelInfoes[mode_].cardId;
        
        meta.KMT.safeTransferFrom(_msgSender(), address(this), amount_);
        uint tokenId = meta.KTA.mintMulti(_msgSender(), cardId, number);
        
        meta.cardAmount += 1;
        meta.tokenAmount += amount_;
        levelInfoes[mode_].orderNum += 1;

        emit Swap(_msgSender(), mode_, tokenId, number);
        return tokenId;
    }
