...
contract TestToken is Context, IBEP20, Ownable {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tFee = tAmount.div(100).mul(_taxFee).mul(_feeMultiplier);
        uint256 tLiquidity = tAmount.div(100).mul(_liquidityFee).mul(_feeMultiplier);
    ...
    function halvingDistribution() private {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && to == pcsV2Pair){
            if (block.timestamp - _deployTimeStamp < 120 days){
            _feeMultiplier = 9;
            } else if (block.timestamp - _deployTimeStamp > 120 days && block.timestamp - _deployTimeStamp < 150 days){
                _feeMultiplier = 8;
            } else if (block.timestamp - _deployTimeStamp > 150 days && block.timestamp - _deployTimeStamp < 180 days){
                _feeMultiplier = 5;
            } else if (block.timestamp - _deployTimeStamp > 180 days){
                _feeMultiplier = 1;
            }
            if (block.timestamp - _deployTimeStamp < 210 days){
                if (_sellLockTimeStamp[from] < block.timestamp){
                    _sellLockTimeStamp[from] = block.timestamp + 24 hours;
            ...
            } else if (block.timestamp - _deployTimeStamp > 210 days && block.timestamp - _deployTimeStamp < 240 days){
                if (_sellLockTimeStamp[from] < block.timestamp){
                    _sellLockTimeStamp[from] = block.timestamp + 24 hours;
                ...
                require(_sellLockAmount[from] > amount, "Sell amount exceeds locked amount!");
                _sellLockAmount[from] = _sellLockAmount[from].sub(amount);
            } else if (block.timestamp - _deployTimeStamp > 240 days && block.timestamp - _deployTimeStamp < 270 days){
                if (_sellLockTimeStamp[from] < block.timestamp){
                    _sellLockTimeStamp[from] = block.timestamp + 24 hours;
                ...
                require(_sellLockAmount[from] > amount, "Sell amount exceeds locked amount!");
                _sellLockAmount[from] = _sellLockAmount[from].sub(amount);
            } else if (block.timestamp - _deployTimeStamp > 270 days && block.timestamp - _deployTimeStamp < 300 days){
                if (_sellLockTimeStamp[from] < block.timestamp){
                    _sellLockTimeStamp[from] = block.timestamp + 24 hours;
                ...
                require(_sellLockAmount[from] > amount, "Sell amount exceeds locked amount!");
                _sellLockAmount[from] = _sellLockAmount[from].sub(amount);
            } else if (block.timestamp - _deployTimeStamp > 300 days && block.timestamp - _deployTimeStamp < 330 days){
                if (_sellLockTimeStamp[from] < block.timestamp){
                    _sellLockTimeStamp[from] = block.timestamp + 24 hours;
                ...
                require(_sellLockAmount[from] > amount, "Sell amount exceeds locked amount!");
                _sellLockAmount[from] = _sellLockAmount[from].sub(amount);
            } else if (block.timestamp - _deployTimeStamp > 330 days && block.timestamp - _deployTimeStamp < 360 days){
                if (_sellLockTimeStamp[from] < block.timestamp){
                    _sellLockTimeStamp[from] = block.timestamp + 24 hours;
                ...
                require(_sellLockAmount[from] > amount, "Sell amount exceeds locked amount!");
                _sellLockAmount[from] = _sellLockAmount[from].sub(amount);
            } else if (block.timestamp - _deployTimeStamp > 360 days && block.timestamp - _deployTimeStamp < 390 days){
                if (_sellLockTimeStamp[from] < block.timestamp){
                    _sellLockTimeStamp[from] = block.timestamp + 24 hours;
                ...
                require(_sellLockAmount[from] > amount, "Sell amount exceeds locked amount!");
                _sellLockAmount[from] = _sellLockAmount[from].sub(amount);
            } else if (block.timestamp - _deployTimeStamp > 390 days && block.timestamp - _deployTimeStamp < 420 days){
                if (_sellLockTimeStamp[from] < block.timestamp){
                    _sellLockTimeStamp[from] = block.timestamp + 24 hours;
        ...
        if (_halvingTimeStamp != 0 && _halvingTimeStamp < block.timestamp){
            ...
            _halvingTimeStamp = block.timestamp + 90 days;
            halvingDistribution();
        }
