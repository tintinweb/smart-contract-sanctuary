...
contract Exchange is OrderState, TransferSafe {
		...
		if (
			...
			if (order.orderType == 1 || order.orderType == 3) {
				if (!isStore) {
					...
					require(
						order.key.sellAsset.token == address(_VOAXStore),
						"invalid sell asset"
					);
					erc1155safeMintTransferFrom(
						IVOAXStore(order.key.sellAsset.token),
						order.key.owner,
						buyer,
						order.key.sellAsset.tokenId,
						1,
				...
				if (!isStore) {
					...
					require(
						order.key.buyAsset.token == address(_VOAXStore),
						"invalid buy asset"
					);
					erc1155safeMintTransferFrom(
						IVOAXStore(order.key.buyAsset.token),
						buyer,
						order.key.owner,
						order.key.buyAsset.tokenId,
						1,
	...
	function setMintableStore(address newMintableStore) external onlyOwner {
		...
		_VOAXStore = IVOAXStore(newMintableStore);
	...
	function VOAXStore() external view returns (address) {
		return address(_VOAXStore);
	}
}