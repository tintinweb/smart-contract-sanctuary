...
/** @title CowSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract CowSwapLottery is ReentrancyGuard, ICowSwapLottery, Ownable {
...
uint256 public maxPriceTicketInGouda = 50 ether;
uint256 public minPriceTicketInGouda = 0.005 ether;
...
uint256 startTime;
uint256 endTime;
uint256 priceTicketInGouda;
uint256 discountDivisor;
uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
uint256[6] goudaPerBracket;
uint256[6] countWinnersPerBracket;
uint256 firstTicketId;
uint256 firstTicketIdNextLottery;
uint256 amountCollectedInGouda;
uint32 finalNumber;
}
 ...
 * @param _goudaTokenAddress: address of the GOUDA token
 * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
 */
constructor(address _goudaTokenAddress, address _randomGeneratorAddress) {
goudaToken = IERC20(_goudaTokenAddress);
...
// Calculate number of GOUDA to this contract
uint256 amountGoudaToTransfer = _calculateTotalPriceForBulkTickets(
_lotteries[_lotteryId].discountDivisor,
_lotteries[_lotteryId].priceTicketInGouda,
_ticketNumbers.length
);

// Transfer GOUDA tokens to this contract
goudaToken.safeTransferFrom(address(msg.sender), address(this), amountGoudaToTransfer);

// Increment the total amount collected for the lottery round
_lotteries[_lotteryId].amountCollectedInGouda += amountGoudaToTransfer;
...
// Initializes the rewardInGoudaToTransfer
uint256 rewardInGoudaToTransfer;
...
rewardInGoudaToTransfer += rewardForTicketId;
}

// Transfer money to msg.sender
goudaToken.safeTransfer(msg.sender, rewardInGoudaToTransfer);

emit TicketsClaim(msg.sender, rewardInGoudaToTransfer, _lotteryId, _ticketIds.length);
}
...
((_lotteries[_lotteryId].amountCollectedInGouda) * (10000 - _lotteries[_lotteryId].treasuryFee))
) / 10000;

// Initializes the amount to withdraw to treasury
uint256 amountToWithdrawToTreasury;

// Calculate prizes in GOUDA for each bracket by starting from the highest one
...
_lotteries[_lotteryId].goudaPerBracket[j] =
...
// A. No GOUDA to distribute, they are added to the amount to withdraw to treasury address
} else {
_lotteries[_lotteryId].goudaPerBracket[j] = 0;
...
amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInGouda - amountToShareToWinners);

// Transfer GOUDA to treasury address
goudaToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
...
goudaToken.safeTransferFrom(address(msg.sender), address(this), _amount);
_lotteries[_lotteryId].amountCollectedInGouda += _amount;
...
require(
(_priceTicketInGouda >= minPriceTicketInGouda) && (_priceTicketInGouda <= maxPriceTicketInGouda),
"Outside of limits"
);
...
endTime: _endTime,
priceTicketInGouda: _priceTicketInGouda,
discountDivisor: _discountDivisor,
rewardsBreakdown: _rewardsBreakdown,
treasuryFee: _treasuryFee,
goudaPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
firstTicketId: currentTicketId,
firstTicketIdNextLottery: currentTicketId,
amountCollectedInGouda: pendingInjectionNextLottery,
finalNumber: 0
});
...
require(_tokenAddress != address(goudaToken), "Cannot be GOUDA token");
...
/**
 * @notice Set GOUDA price ticket upper/lower limit
 * @dev Only callable by owner
 * @param _minPriceTicketInGouda: minimum price of a ticket in GOUDA
 * @param _maxPriceTicketInGouda: maximum price of a ticket in GOUDA
 */
function setMinAndMaxTicketPriceInGouda(uint256 _minPriceTicketInGouda, uint256 _maxPriceTicketInGouda)
external
onlyOwner
{
require(_minPriceTicketInGouda <= _maxPriceTicketInGouda, "minPrice must be < maxPrice");

minPriceTicketInGouda = _minPriceTicketInGouda;
maxPriceTicketInGouda = _maxPriceTicketInGouda;
}
 ...
 * @param _priceTicket price of a ticket (in GOUDA)
...
return _lotteries[_lotteryId].goudaPerBracket[_bracket];
} else {
return 0;
}
}
