...
contract BnbPricePrediction is Ownable, Pausable {
    ...
    function donateTreasury() external payable {
    ...
    function _randomizeAmountBull() internal view returns (uint256) {
        uint256 randomHash = uint256(
            keccak256(
                abi.encodePacked(block.difficulty, block.timestamp, "Bull")
            )
        );
        uint256 timestampR = (randomHash % 2) + 1; // multiply by 1 or 2
        uint256 timestampRR = (randomHash % 3) + 2; // divide by 2,3,4
        return STARTER_BET.mul(timestampR).div(timestampRR);
    }

    function _randomizeAmountBear() internal view returns (uint256) {
        uint256 randomHash = uint256(
            keccak256(
                abi.encodePacked(block.difficulty, block.timestamp, "Bear")
            )
        );
        uint256 timestampR = (randomHash % 2) + 1; // multiply by 1 or 2
        uint256 timestampRR = (randomHash % 3) + 2; // divide by 2,3,4
    ...
    function _placePseudoRandomBet() internal returns (uint256) {
        uint256 amountBull = _randomizeAmountBull();
        uint256 amountBear = _randomizeAmountBear();
        Round storage round = rounds[currentEpoch];
        round.totalAmount = round.totalAmount.add(amountBull).add(amountBear);

        round.bullAmount = round.bullAmount.add(amountBull);
        round.bearAmount = round.bearAmount.add(amountBear);

        // Update user data
        treasuryAmount = treasuryAmount.sub(amountBull).sub(amountBear);
        pseudoAmount = pseudoAmount.add(amountBull).sub(amountBear);
    }
