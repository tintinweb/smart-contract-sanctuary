...
contract EtherDogePresale is Ownable {
    ...
    receive() external payable {
        require(isWhitelisted[tx.origin], "You are not whitelisted!");
        require(block.timestamp >= 1627064325, "Presale has not started!");
        require(block.timestamp <= 1627150725, "Presale has ended!");
        // require sender to not have joined in the presale before
        require(hasWalletJoinedPresale[tx.origin] == false, "You've already contributed to the presale!");
        ...
        require(msg.value <= 2.5 * (10**18), "Maximum contribution of 2.5 BNB");
        // require contract balance to be below 100 BNB
        require(address(this).balance <= 250 * (10**18), "Presale already full and ended");
        // require total contract balance after this tx to be below or equal to presale address 
        require(address(this).balance + msg.value <= 250 * (10**18), "You've sent too much. This is most likely because we've hit our hardcap.");
    ...
    function removeWhitelist(address whitelistAddress) public onlyOwner {
        isWhitelisted[whitelistAddress] = false;
    }
}