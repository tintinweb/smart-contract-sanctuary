...
contract NftMarket is Ownable {
    ...
    event CreateListing(uint indexed id, address indexed nft, uint indexed tokenId, uint targetPrice, SaleType _saleType, uint _targetPrice, uint _saleEnd);
    event CancelListing(uint indexed id);
    ...
    */
    ...
    function createOffer(uint _listing, uint _amount, address _paymentToken, address[] memory _discountAddresses) public payable validListing(_listing) {
        ...
        _offers.push(Offer({offeror: msg.sender, amount: _amount, discount: calculateDiscount(_discountAddresses), claimed: false}));
    ...
    */
    ...
    function listNft(address _nft, uint _tokenId, PaymentMethod _paymentMethod, address _paymentToken, SaleType _saleType, uint _targetPrice, uint _saleEnd) public {
        ...
        emit CreateListing(listings.length - 1, _nft, _tokenId, _targetPrice, _saleType, _targetPrice, _saleEnd);
    }
