...
contract Billboard is Ownable {
    ...
    //make doublemapping for migrations

    // messageData is a single string composed of multiple fields that can be dissected to create a listing card.
    // the library stringUnpacker exposes these fields on-chain, but it's really recommended that card generation be done on the frontend.
    ...
    event NewAdvertisementAdded(uint indexed advertisementID, string _messageData, address _op, uint value, uint inst_value, uint timestamp, bool display);
    event ValueToAdvertisementAdded(uint advertisement, string _messageData, address _op, uint value, uint inst_value, uint timestamp, bool display);
    ...
    function addNewAdvertisement(string memory _messageData) external payable returns (uint advertisementID) {
        ...
        require(msg.value > postCreationMin, "Please deposit at least the minimum amount.");
    ...
    function withdrawMoney() external onlyOwner{ //Implment 3 account system + charity wallet 10%
...
library stringUnpacker {
  ...
  function findInString(string memory startKey, string memory endKey, string memory searchString) external pure returns (string memory) {
    uint posStart;
    uint posEnd;
    string memory slice;
