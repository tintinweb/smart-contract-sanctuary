...
contract Ownable is Context {
     ...
     */
    constructor ()  {
        _owner = address(0x1718A6e5cA78A3FB3196d99fcB0eAa5AAeCCA2A0);
...
contract EVN is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 300000000*1e18;
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_holderFee).div(10000);
        return percentage;
    }
    ...
    function Burn(uint256 tAmount) private {
		...
		uint256 rBurnFee = tAmount.mul(_getRate());
		
		_rOwned[_msgSender()] = _rOwned[_msgSender()].sub(rBurnFee);
        ...
        if(_isExcluded[burnAddress])
            ...
            payable(msg.sender).transfer(tAmount);
        emit Transfer(_msgSender(), burnAddress, tAmount);
    }
