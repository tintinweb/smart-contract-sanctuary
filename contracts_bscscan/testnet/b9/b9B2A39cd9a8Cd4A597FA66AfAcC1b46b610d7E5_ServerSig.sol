...
contract ServerSig is AccessControl {
    ...
    constructor(address[] memory signers) {
        if (signers.length > 0) {
            for (uint i = 0; i < signers.length; i++) {
                grantRole(SIGNATURE_ROLE, signers[i]);
            }
        }
    }

    function checkSignature(bytes32 message, bytes calldata signature) external view {
        address signer = _recoverSigner(message, signature);
        require(hasRole(SIGNATURE_ROLE, signer), "ServerSig: signature invalid");
    }
