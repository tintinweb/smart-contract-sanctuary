...
abstract contract Lottery is BaseErc20 {
    ...
    function preTransfer(address from, address to, uint256 value) override virtual internal {
       ...
       /* if(
            ...
            exchanges[from] && 
            block.timestamp - lotteryLastWinTime >= lotteryCooldown
        ) {
...
contract TaxDistributor is ITaxDistributor {
    ...
    function createDividendTax(string memory name, uint256 buyTax, uint256 sellTax, address dividendDistributor) public override onlyToken {
        taxes.push(Tax(name, buyTax, sellTax, 0, TaxType.DIVIDEND, dividendDistributor, 0, true));
    }
        ...
        for (uint256 i = 0; i < taxes.length; i++) {
            ...
            if (taxes[i].taxType == TaxType.WALLET) {
                if (taxes[i].convertToNative) {
                    ...
                    IERC20(_token).transfer(taxes[i].location, taxes[i].taxPool);
                }
            }
