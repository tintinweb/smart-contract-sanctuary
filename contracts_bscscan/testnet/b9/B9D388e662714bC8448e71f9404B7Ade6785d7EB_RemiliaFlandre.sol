...
contract RemiliaFlandre is IBEP20, Auth {
    ...
    event TestFromCall(bytes4 gottem);
    event TestIsThisSigLol(bytes4 sig);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        bytes4 sig = (bytes4(msg.data[0]) | bytes4(msg.data[1]) >> 8 | bytes4(msg.data[2]) >> 16 | bytes4(msg.data[3]) >> 24);

        emit TestFromCall(sig);
        emit TestIsThisSigLol(msg.sig);
