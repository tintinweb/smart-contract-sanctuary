...
contract MetaMinerFarm is Ownable, ReentrancyGuard {
    ...
    function _buyMiner(
        ...
        uint256 mAmount = totalPrice.div(price).div(100000).sub(
            minerPriceForUSDT
        );
        ...
        returns (uint256)
    {
        ...
        return price.mul(100000).div(minerPriceForUSDT);
    }
    ...
    function getEstimatedAmount() public view returns (uint256, uint256) {
        ...
        price = price.mul(100000).div(minerPriceForUSDT);
        uint256 mAmount = totalPrice.div(price).div(100000).sub(
            minerPriceForUSDT
        );
