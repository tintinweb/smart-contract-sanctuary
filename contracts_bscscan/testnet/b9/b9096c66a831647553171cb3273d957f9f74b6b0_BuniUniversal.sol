...
contract BuniUniversal is Initializable, AccessControlUpgradeable {
    ...
    function initialize(
        ...
        staminaCostFight = 40;
        mintTrainerFee = ABDKMath64x64.divu(10, 1);//10 usd;
        mintBunicornFee = ABDKMath64x64.divu(3, 1);//3 usd;
        ...
        _burReforgeBunicornFees = [
            ABDKMath64x64.divu(5, 10),  // 1 stars:         0.5 usd;
            ABDKMath64x64.divu(10, 10), // 2 stars:         1.0 usd;
            ABDKMath64x64.divu(15, 10), // 3 stars:         1.5 usd;
            ABDKMath64x64.divu(20, 10), // 4 stars:         2.0 usd;
            ABDKMath64x64.divu(25, 10), // 5 stars:         2.5 usd;
            ABDKMath64x64.divu(30, 10), // 6 stars:         3.0 usd;
            ABDKMath64x64.divu(35, 10), // 7 stars:         3.5 usd;
            ABDKMath64x64.divu(40, 10)  // 8 stars:         4.0 usd;
        ];
    ...
    int128[6] private _burReforgeTrainerFees;
    ...
    function fight(uint256 trainer, uint256 bunicorn, uint32 target, uint8 fightMultiplier)
        ...
        (uint8 trainerTrait, uint24 basePowerLevel, uint64 timestamp) =
            unpackFightData(trainers.getFightDataAndDrainStamina(trainer, staminaCostFight * fightMultiplier));
    ...
    function reforgeTrainer(uint256 reforgeID, uint256 burnID)
        ...
        requestPayBurFromPlayer(_calculateReforgeTrainerFee(reforgeID))
    {
        _payBurContract(msg.sender, _calculateReforgeTrainerFee(reforgeID));
        trainers.reforge(reforgeID, burnID);
    }

    function _calculateReforgeTrainerFee(uint256 reforgeID) internal view returns(int128) {
        uint8 trainerLevel = trainers.getLevel(reforgeID);
        if (trainerLevel >= 250) {
            return 0;
        }
        else if (trainerLevel <= 9) {
            return _burReforgeTrainerFees[0];
        }
        else {
            uint8 index = (trainerLevel / 50);
            return _burReforgeTrainerFees[index + 1];
        }
    }
    ...
    function reforgeBunicorn(uint256 reforgeID, uint256 burnID)
        ...
        requestPayBurFromPlayer(_calculateReforgeBunicornFee(burnID))
    {
        _payBurContract(msg.sender, _calculateReforgeBunicornFee(burnID));
        bunicorns.reforge(reforgeID, burnID);
    }

    function _calculateReforgeBunicornFee(uint256 id) internal view returns(int128) {
    ...
    function getReforgeTrainerFees() external view returns(int128[6] memory fees) {
        for (uint8 i = 0; i < _burReforgeTrainerFees.length; i++) {
            fees[i] = _burReforgeTrainerFees[i];
        }
    }

    function setReforgeTrainerValues(uint256[6] memory cents) public restricted {
        for (uint8 i = 0; i < 6; i++) {
            _burReforgeTrainerFees[i] = ABDKMath64x64.divu(cents[i], 100);
        }
    }
    ...
    function setReforgeBunicornValues(uint256[8] memory cents) public restricted {
        for (uint8 i = 0; i < 8; i++) {
