...
contract PrivateSale is ReentrancyGuard, Ownable, Vestable {
    ...
    function purchaseFromBnb(uint256 _lastPrice, bytes memory signature) 
        public 
        whenActive 
        nonReentrant 
        payable
        onlyValid(signature)
    {
        ...
        require(_tokenPurchased[msg.sender].add(token) <= purchaseLimit, "Individual cap exceeded");
        addVest(token);
    ...
    function purchaseFromBusd(uint256 _busdAmount, bytes memory signature) 
        external 
        whenActive 
        onlyValid(signature)  
    {
    ...
    function purchaseFromQuill(uint256 _busdAmount) 
        external 
        whenActive 
    {
        // only if active
    ...
    modifier whenActive {
        require(block.timestamp < endDate, 'Token Sale is not active');
        _;
    }

    modifier whenInactive {
        require(block.timestamp >= endDate, 'Token Sale is active');
        _;
    }
    ...
    modifier onlyValid(bytes memory signature){
        require(signature.length == 65, "Address not whitelisted.");
...
contract Vestable is Ownable {
    ...
    function getSchedule(uint32 _date, uint256 _index) public view returns (
        ...
        if (_date > block.timestamp) {
            ...
            percentage = 10;
            amount = _tenPercentOf(vest.amount);
            if (isClaimed[_index][_date]) { status = Status.Claimed; } 
    ...
    function historiesByVest(uint _index) public view returns (uint[] memory) {
    ...
    function historyByIndex(uint _index) public view returns (
        uint256 index,
        uint32 releaseDate,
        uint32 dateClaimed, 
        uint256 claimedAmount
    ...
    function _ninePercentOf(uint256 _amount) private pure returns (uint256 tenPercent) {
        tenPercent = (_amount / 100) * 9;
    }
