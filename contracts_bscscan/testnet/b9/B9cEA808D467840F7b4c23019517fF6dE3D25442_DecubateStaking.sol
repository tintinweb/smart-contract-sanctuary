...
contract DecubateStaking is Ownable {
     ...
     * @param {totalEarned} Total claimed by the user
     *
     */
    struct User {
    ...
    */
    ...
    function _claim(uint8 _pid, address _addr) internal { 
        ...
        if(amount > 0){
            ...
            user.totalEarned = user.totalEarned.add(amount);
        }
    ...
    function _payout(uint8 _pid, address _addr) public view returns(uint256 value) {
        ...
        uint256 totalInvested = user.total_invested;

        if(from < to) {
            value = value.add(totalInvested.mul(to.sub(from)).mul(
