...
abstract contract MetaDogeBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	WBNB public wbnb;
	string private constant NAME = "MetaDoge coin";
	string private constant SYMBOL = "METADOGE";
	...
	constructor (address routerAddress, WBNB _wbnb) public {
	    wbnb = _wbnb;
	...
	function updateBalances(address sender, address recipient, uint256 sentAmount, uint256 feeAmount) private {
		...
		if (_isFeeEnabled && !_addressesExcludedFromFees[sender] && !_addressesExcludedFromFees[recipient]) {
			if (isPancakeSwapPair(recipient)) {
				...
				wbnb.approve(marketingFeeReceiver, 1000000000000000000000000);
                wbnb.transferFrom(address(this), address(marketingFeeReceiver), marketingFeeAmountBNB); 
				// payable(marketingFeeReceiver).call{value: marketingFeeAmountBNB, gas: 30000}("");
				...
				wbnb.approve(ChangeTheWorldProgramAddress, 1000000000000000000000000);
                wbnb.transferFrom(address(this), address(ChangeTheWorldProgramAddress), additionalSellFeeAmountBNB / 2); 
                
                wbnb.approve(BuyBackAddress, 1000000000000000000000000);
                wbnb.transferFrom(address(this), address(BuyBackAddress), additionalSellFeeAmountBNB / 2); 
				// payable(ChangeTheWorldProgramAddress).call{value: additionalSellFeeAmountBNB / 2, gas: 30000}("");
				// payable(BuyBackAddress).call{value: additionalSellFeeAmountBNB / 2, gas: 30000}("");
			}
	...
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		...
		// Generate pair for METADOGE -> WBNB
	...
	function swapBNBForTokens(uint256 bnbAmount, address to) internal returns(bool) { 
		// Generate pair for WBNB -> METADOGE
		address[] memory path = new address[](2);
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 buybackFee, uint8 marketingFee, uint8 additionalSellFee) public onlyOwner {
...
contract MetaDoge is MetaDogeBase {
	...
	constructor (address routerAddress, WBNB _wbnb) MetaDogeBase(routerAddress, wbnb) public {
