...
//   Mainnet Router : 0x10ED43C718714eb63d5aA57B78B54704E256024E
  ...
  address public dividendToken = _uniswapV2Router.WETH();
...
contract SUPER_CAKE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool public forceStopDiv = false;
    bool public buyBackRandomEnabled = true;

    SUPERCAKEDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 public maxBuyTranscationAmount = 10**14 * (10**18);
    uint256 public maxSellTransactionAmount = 10**14 * (10**18);
    uint256 public swapTokensAtAmount = 10**10 * (10**18);
    uint256 public maxWalletToken = 10**14 * (10**18); // 1.5% of total supply
    ...
    constructor() ERC20("SUPERCAKE", "SUPERCAKE") {
        ...
        dividendTracker = new SUPERCAKEDividendTracker();

        buyBackWallet = 0xA1ce75414653951a10f0c002AC1f4C0922807A49;
...
//   Mainnet Router : 0x10ED43C718714eb63d5aA57B78B54704E256024E
        ...
        _dividendToken = _uniswapV2Router.WETH();
    ...
    function startStopDiv() public onlyOwner() {
        forceStopDiv = true;
    }
    
    function stopStopDiv() public onlyOwner() {
        forceStopDiv = false;
    }
    ...
    function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
        require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "SUPERCAKE: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
        sellFeeIncreaseFactor = multiplier;
    }
    
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(marketingDivisor >= 0 && marketingDivisor <= 100, "SUPERCAKE: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "SUPERCAKE: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SUPERCAKE: The dividend tracker already has that address");

        SUPERCAKEDividendTracker newDividendTracker = SUPERCAKEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SUPERCAKE: The new dividend tracker must be owned by the SUPERBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "SUPERCAKE: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SUPERCAKE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SUPERCAKE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SUPERCAKE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SUPERCAKE: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "SUPERCAKE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SUPERCAKE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SUPERCAKE: Cannot update gasForProcessing to same value");
...
contract SUPERCAKEDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("SUPERCAKE_Dividend_Tracker", "SUPERCAKE_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "SUPERCAKE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "SUPERCAKE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SUPERCAKE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SUPERCAKE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SUPERCAKE_Dividend_Tracker: Cannot update claimWait to same value");
