...
ERC721HolderUpgradeable
{
    ...
    function initialize(IDefinaCard nftToken_, string memory baseTokenURI,
        ...
        for (uint i = 0; i < cardIds_.length; i++) {
            ...
            for (uint j = 0; j < cardsNum_[i]; j++) {
                allCandidateCards.push(cardIds_[i]);
            }
        }
    ...
    function fulfillRandomness(uint256 tokenId, address user, uint256 randomness) internal {
        ...
        uint index = allCandidateCards.length;
        do {
            index = _randModulus(user, randomness, i, allCandidateCards.length);
            uint cardId = allCandidateCards[index];
        ...
        uint cardId = allCandidateCards[index];
        allCandidateCards[index] = allCandidateCards[allCandidateCards.length - 1];
        allCandidateCards.pop();
