...
contract DnftTradable is ERC1155, Ownable {
    ...
    modifier creatorOnly(uint256 _id) {
        require(
            ...
            "DnftTradable#creatorOnly: ONLY_CREATOR_ALLOWED"
        );
        _;
    }
    ...
    function mint(
        ...
        uint256 _quantity,
        bytes memory _data
    ) external virtual creatorOnly(_id) {
        _mint(_to, _id, _quantity, _data);
        tokens[_id].supply = tokens[_id].supply.add(_quantity);
    }
    ...
    {
        tokens[_tokenId].customUri = _newURI;
        emit URI(_newURI, _tokenId);
    }

    function uri(uint256 _id) public view override returns (string memory) {
        require(_exists(_id), "ERC1155Tradable#uri: NONEXISTENT_TOKEN");
        bytes memory customUriBytes = bytes(tokens[_id].customUri);
        if (customUriBytes.length > 0) {
            return tokens[_id].customUri;
        } else {
