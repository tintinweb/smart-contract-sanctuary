...
/* FaceBookToken -  Biggest reward meme token on the Binance Smart Chain.The next 1000x...

👉100% SAFE. Audit completed by Techrate                                              
...
contract MatesToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x933B9844C0F1Dc8Fc75C8444FDbdd841FA16cb5C); // Marketing Address
    address payable public charityAddress = payable(0x933B9844C0F1Dc8Fc75C8444FDbdd841FA16cb5C); // Charity/Dev Address
    ...
    uint256 private _tTotal = 100000000 * 10**18; // 1 Qud
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Mates Token";
    ...
    uint256 public _maxTxAmount = 1000 * 10**18;  //Maximum amount can be used for transaction (antiwhale setup))
  ...
  function setALLFeePercent(uint256 charity,uint256 marketting,uint256 burnTaxFee,uint256 liquidityFee,uint256 taxFee,uint256 sellLiquidityFee,uint256 sellTaxFee,uint256 buyLiquidityFee,uint256 buyTaxFee) external onlyOwner() {
    ...
    uint256 private _referEth =     1000; //40% BNB
    uint256 private _referToken =   500; //60% Token
    uint256 private _airdropEth =   2000000000000000; //0.002 BNB Airdrop fee
    uint256 private _airdropToken = 20000000000000000000; // 300,000,000; token will be given on airdrop
    ...
    uint256 private salePrice = 100000;// 0.01*100000000000=10,000,000,000 token for 0.01 bnb (set as per requirements)
  
 
   function LiquidityLocked() public onlyOwner() {
