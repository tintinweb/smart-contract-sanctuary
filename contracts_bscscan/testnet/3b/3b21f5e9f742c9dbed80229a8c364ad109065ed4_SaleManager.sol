...
contract SpectraNFT is ERC721 {
    ...
    function myNFTs() external view returns (uint256[] memory items) {
...
contract SaleManager is Ownable {
    ...
    mapping(string => SpectraNFT) _allNFTInfo;
    ...
    function mintSingleNFT(string memory _tokenHash) external payable onlyAdmin {
        require(!_tokenHashExists[_tokenHash], "Existing NFT hash value....");
        mkNFT = SpectraNFT(mkNFTaddress);
        mkNFT.mintNFT();
        _allNFTInfo[_tokenHash] = mkNFT;
        _tokenHashExists[_tokenHash] = true;
    }

    function mintMultipleNFT(string[] memory _tokenHashs) external payable onlyAdmin {
        for (uint256 i = 0; i < _tokenHashs.length; i++) {
            require(!_tokenHashExists[_tokenHashs[i]], "Existing NFT hash value....");
            mkNFT = SpectraNFT(mkNFTaddress);
            mkNFT.mintNFT();
            _allNFTInfo[_tokenHashs[i]] = mkNFT;
            _tokenHashExists[_tokenHashs[i]] = true;
        }
    }

    function createSale(string memory _tokenHash, uint _interval, uint _startPrice, uint24 _royalty, uint8 _kind) external nonReentrant onlyReseller returns (bool) {
        require(_interval >= 0, "Invalid auction interval....");
        require(_royalty >= 0, "Invalid royalty value....");
        _isCancelled = false;
        _isOnSale = true;

        SaleInfo memory saleInfo = SaleInfo(_saleIdCounter, _tokenHash, msg.sender, msg.sender, _startPrice, address(0), 0, block.timestamp, _interval, _royalty, _kind);
        
        // mkNFT.transferFrom(msg.sender, address(this), _saleIdCounter);
    ...
    function destroySale(string memory _tokenHash) external onlyReseller nonReentrant returns (bool) {
        ...
        require(_allSaleInfo[_getSaleId[_tokenHash]].maxBid == 0, "Max bid is not zero...");
        _isCancelled = true;
        _isOnSale = false;
        // mkNFT.transferFrom(address(this), _allSaleInfo[_getSaleId[_tokenHash]].currentOwner, _getSaleId[_tokenHash]);
        emit SaleDestroyed();
        return true;
    }
    ...
    function performBid(string memory _tokenHash) external nonReentrant returns (bool) {
        ...
        // mkNFT.transferFrom(address(this), saleInfo.maxBidder, _getSaleId[_tokenHash]);
        _isOnSale = false;
    ...
    function getAuthentication(address _addr) external view onlyOwner returns (uint8) {
        require(_addr != address(0), "Invalid input address...");
        return _isCreator[_addr];
    }

    function getAuctionState(string memory _tokenHash) internal view returns (AuctionState) {
        if (_isCancelled) return AuctionState.CANCELLED;
    ...
    function getWithdrawBalance(uint8 _kind) public view onlyOwner returns (uint256) {
