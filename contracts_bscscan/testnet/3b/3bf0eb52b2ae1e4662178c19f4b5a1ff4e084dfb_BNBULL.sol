...
// $BNBULL proposes an innovative feature in its contract.
//
// DIVIDEND YIELD PAID IN BNB! With the auto-claim feature,
// simply hold $BNBULL and you'll receive BNB automatically in your wallet.
// 
// Hold BNBULL and get rewarded in BNB on every transaction!
//
// 📱 Telegram: https://t.me/BNBullOfficial
// 🌎 Website: https://www.bnbull.org/
// 🌐 Dashboard: https://www.dashboard.bnbull.org/
...
contract BNBULL is ERC20, Ownable, Marketable {
    ...
    BNBULLDividendTracker public dividendTracker;
    ...
    // sellTax = (totalFees * sellFeesIncreaseFactor)/100 - totalFees
    // sells have fees of 2.04% initally
    uint256 public sellFeeIncreaseFactor = 112;
    ...
    constructor() public ERC20("BNBULL", "BNBULL") {
    	...
    	dividendTracker = new BNBULLDividendTracker();
    ...
    function updateMaxSellTransactionAmount(uint newValue) public onlyOwner {
        require(newValue>=0, "BNBULL: The new maxSellTransaction amount should be greater than or equal to zero");
    ...
    function updateSellFeesIncreaseFactor(uint256 newSellingFees) public onlyOwner {
        require(newSellingFees>=0, "BNBULL: New Selling fees should be greater than 0");
        emit UpdateSellFeesIncreaseFactor(newSellingFees, sellFeeIncreaseFactor);
        sellFeeIncreaseFactor = newSellingFees;
    }

    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BNBULL: The dividend tracker already has that address");

        BNBULLDividendTracker newDividendTracker = BNBULLDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BNBULL: The new dividend tracker must be owned by the BNBULL token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BNBULL: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BNBULL: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BNBULL: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BNBULL: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BNBULL: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BNBULL: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BNBULL: Cannot update gasForProcessing to same value");
...
contract BNBULLDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("BNBULL_Dividend_Tracker", "BNBULL_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "BNBULL_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BNBULL_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BNBULL contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BNBULL_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BNBULL_Dividend_Tracker: Cannot update claimWait to same value");
