...
interface IDividendPayingToken {
  ...
  function withdrawDividendOne() external;
  function withdrawDividendTwo() external;
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function withdrawDividendOne() public virtual override {
    _withdrawDividendOfUser(msg.sender,DividendTokenOne);
  }  
  function withdrawDividendTwo() public virtual override {
    _withdrawDividendOfUser(msg.sender,DividendTokenTwo);
  }

  /// @notice Withdraws the ether distributed to the sender.
  /// @dev It emits a `DividendWithdrawn` event if the amount of withdrawn ether is greater than 0.
  /// modified to support BNB dividend
  function _withdrawDividendOfUser(address payable user, address _dividendToken) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      withdrawnDividends[user] = withdrawnDividends[user].add(_withdrawableDividend);
        ...
        if(_dividendToken == address(0)){
            ...
            success = sent;
            emit DividendWithdrawn(user, _withdrawableDividend, _dividendToken);     
        }else{
            success = IERC20(_dividendToken).transfer(user, _withdrawableDividend);
            emit DividendWithdrawn(user, _withdrawableDividend, _dividendToken);
        }        
...
contract TKDIVI is ERC20, Ownable {
    ...
    constructor() public ERC20("sep921v2", "sep9v2") {
        // LP burn address
    ...
    function claim() external {
        dividendTracker.processAccount(msg.sender, false,DividendTokenOne,DividendTokenTwo);
    }
    ...
    // modified to support BNB dividend
            function swapAndSendDividendsOne(uint256 tokens) private {
        address payable diviTracker = address(dividendTracker);
        bool success = false;
        uint256 dividendsOne;

        if(DividendTokenOne != address(0)){
            swapTokensForTokens(tokens, address(this), DividendTokenOne);
            dividendsOne = IERC20(DividendTokenOne).balanceOf(address(this));
            success = IERC20(DividendTokenOne).transfer(address(dividendTracker), dividendsOne);
        }else{
            uint256 initialBalance = address(this).balance;
        ...
        if (success) {
            ...
            emit SendDividends(tokens, dividendsOne,DividendTokenOne);
        }        

    } 
    function swapAndSendDividendsTwo(uint256 tokens) private {
        address payable diviTracker = address(dividendTracker);
        bool success = false;
        uint256 dividendsTwo;

        if(DividendTokenTwo != address(0)){
            swapTokensForTokens(tokens, address(this), DividendTokenTwo);
            dividendsTwo = IERC20(DividendTokenTwo).balanceOf(address(this));
            success = IERC20(DividendTokenTwo).transfer(address(dividendTracker), dividendsTwo);
        }else{
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(tokens);
            uint256 newBalance = address(this).balance.sub(initialBalance);
        ...
        if (success) {
            ...
            emit SendDividends(tokens, dividendsTwo,DividendTokenTwo);
        }        
    ...
    function _transfer(
        ...
        if(
            ...
            uint256 sellTokensHalf = sellTokens.div(2);
            uint256 sellTokensHalf2 = sellTokens.sub(sellTokensHalf);

            swapAndSendDividendsOne(sellTokensHalf);
            swapAndSendDividendsTwo(sellTokensHalf2);
...
contract TKDIVIDividendTracker is DividendPayingToken, Ownable {
    ...
    function setBalance(address payable account, uint256 newBalance) external onlyOwner {
        ...
        processAccount(account, true,DividendTokenOne,DividendTokenTwo);
    }
    ...
    function process(uint256 gas) public returns (uint256, uint256, uint256) {
        ...
        while(gasUsed < gas && iterations < numberOfTokenHolders) {
            ...
            if(canAutoClaim(lastClaimTimes[account])) {
                if(processAccount(payable(account), true,DividendTokenOne,DividendTokenTwo)) {
                    claims++;
                }
            }
    ...
    function processAccount(address payable account, bool automatic, address _dividendTokenOne,address _dividendTokenTwo) public onlyOwner returns (bool) {
        uint256 amountOne = _withdrawDividendOfUser(account,_dividendTokenOne);
        uint256 amountTwo = _withdrawDividendOfUser(account,_dividendTokenTwo);

        if(amountOne > 0 || amountTwo > 0) {
            lastClaimTimes[account] = block.timestamp;
            emit Claim(account, amountOne, automatic);
            emit Claim(account, amountTwo, automatic);
            return true;
        }
...

{
  ...
  "evmVersion": "istanbul",
  "libraries": {
    "/Users/fly/Desktop/dev/safeFlight/contracts/buybackTokenNoTxFeesWithBurnZeroFeeTwoDivV2.sol": {
      "IterableMapping": "0x47975b4EE874FAd22460B4A480E736A5f4Fdbdc6"
    }
  },
  "outputSelection": {
    "*": {
