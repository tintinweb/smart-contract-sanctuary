...
contract Pool is Ownable {
    ...
    modifier isWhitelisted() {
        ...
        require(
            tierOf[_msgSender()] >= 1 && tierOf[_msgSender()] <= 6,
            "Invalid tier"
        );
        _;
    }
    ...
    function purchase() public payable inProgress isWhitelisted {
        ...
        require(
            totalUserPurchase >= minAllocation,
            "Purchase less than min allocation"
        );
        require(
            totalUserPurchase <= maxAllocation,
            "Purchase more than max allocation"
        );

        uint256 totalTierPurchase = totalPurchaseOfTier[tier];
        require(
            totalTierPurchase.add(msg.value) <= hardcapOfTier[tier],
            "Hardcap for this tier reached"
        );
    ...
    function setAllocationForTier(
        ...
        require(
            minAmount <= maxAmount,
            "Min amount should be less than or equal to maxAmount"
        );
        minAllocationOfTier[tier] = minAmount;
        maxAllocationOfTier[tier] = maxAmount;
    }

    function addAllocation(address[] memory users, uint8 tier)
        external
        onlyOwner
    {
        require(tier >= 1 && tier <= 6, "Invalid tier");
    ...
    function distributeRange(uint256 start, uint256 end) external onlyOwner {
        ...
        require(
            ...
            "End index should be whitelist length - 1 at most"
        );
    ...
    function _silentDistribute(address user) internal {
    ...
    function populateCaps(uint256[6] memory caps) private {
        hardcapOfTier[1] = caps[0];
        hardcapOfTier[2] = caps[1];
        hardcapOfTier[3] = caps[2];
        hardcapOfTier[4] = caps[3];
        hardcapOfTier[5] = caps[4];
        hardcapOfTier[6] = caps[5];
    }
}