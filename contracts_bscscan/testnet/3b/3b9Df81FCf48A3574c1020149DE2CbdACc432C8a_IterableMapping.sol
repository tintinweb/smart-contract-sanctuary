...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable DividendsToken = address(0xD3E4DddEEC510727d22A154f9E48C21Ea6f76fCa);
...
contract CakeBNB is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   CakeBNBDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("CakeBNB", "CakeBNB") {
        _ETHRewardsFee = 8;
        _liquidityFee = 8;
		_marketingFee = 2;
    	...
    	dividendTracker = new CakeBNBDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
		bulkSender = payable(0x8680e2918ac720e0c22474E1c01B3014CC9D14FA);
		marketingWallet = payable(0xD3E4DddEEC510727d22A154f9E48C21Ea6f76fCa);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CakeBNB: The dividend tracker already has that address");

        CakeBNBDividendTracker newDividendTracker = CakeBNBDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CakeBNB: The new dividend tracker must be owned by the CakeBNB token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CakeBNB: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CakeBNB: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CakeBNB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CakeBNB: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "CakeBNB: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CakeBNB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CakeBNB: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForCAKE(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of CakeBNB
            path,
            recipient,
            block.timestamp
        );
...
contract CakeBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("CakeBNB_Dividend_Tracker", "CakeBNB_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "CakeBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "CakeBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CakeBNB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "CakeBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CakeBNB_Dividend_Tracker: Cannot update claimWait to same value");
