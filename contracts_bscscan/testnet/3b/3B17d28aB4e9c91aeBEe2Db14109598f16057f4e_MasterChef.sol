...
contract MasterChef is Ownable, ReentrancyGuard {
        ...
        returns (uint256)
    {
        ...
        uint256 pending = user.amount.mul(accMowaPerShare).div(1e12).sub(
            user.rewardDebt
        );
    ...
    function deposit(
        ...
        if (_amount > 0) {
            ...
            user.amount = user.amount.add(_amount).add(
                _amount.mul(hashRate).div(100)
            );
            pool.totalLP = pool.totalLP.add(_amount);
            pool.totalLP = pool.totalLP.add(_amount.sub(_amount.mul(hashRate).div(100)));
        }
