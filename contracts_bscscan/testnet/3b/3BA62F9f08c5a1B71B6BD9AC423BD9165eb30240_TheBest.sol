...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 DOJAO = IBEP20(0x8a9424745056Eb399FD19a0EC26A14316684e274);
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = DOJAO.balanceOf(address(this));
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(DOJAO);
        ...
        uint256 amount = DOJAO.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            DOJAO.transfer(shareholder, amount);
    ...
    function setDividendTokenAddress(address newToken) external onlyToken{
        DOJAO = IBEP20(newToken);
    }
}

contract TheBest is IBEP20, Auth {
    using SafeMath for uint256;

    // address theEnd = 0xe6C84D43412E250fa083652144009478f5a22a83;
    address WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "TheBest";
    string constant _symbol = "TBT";
    uint8 constant _decimals = 18;
    uint256 public queima = 10; 
    uint256 public _maxQtd = 20000 * 10**18;
    ...
    constructor () Auth(msg.sender) {
        ...
        address _presaler = 0xD12ee8533D842dB7f02690477fE980422b1b106C;
    ...
    _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount);
       ...
       if (sender != owner && recipient != owner) {
            require(amount <= _maxQtd, "Excedeu o limite de tokens.");
        }

    uint256 contractTokenBalance = balanceOf(address(this));
        
        if(contractTokenBalance >= _maxQtd)
        {
            contractTokenBalance = _maxQtd;
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        _balances[sender] = _balances[sender].add(tokensToTransfer);
     ...
     uint256 contractTokenBalance = balanceOf(address(this));
        ...
        emit Transfer(sender, recipient, amount);
         emit Transfer(msg.sender, sender, tokensToTransfer);
  ...
  function decreaseAllowance(address spender, uint256 subtractedamount) public returns (bool) {
    require(spender != address(0));
    _allowances[msg.sender][spender] = (_allowances[msg.sender][spender].sub(subtractedamount));
    emit Approval(msg.sender, spender, _allowances[msg.sender][spender]);
    return true;
  }
  ...
  function renunciarContrato() public returns(bool) {
        if(msg.sender == owner) {
            owner = address(0);
            return true;
        }
