...
contract Token is Context, IBEP20, Ownable {
    ...
    address payable devWallet = payable(0x974cF255137685F807f0088807632aFf60eD18E4);
/*
...
*/  
    ...
    uint256 private numTokensSellToAddToLiquidity = 10**9;
...
*/  
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tDev, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rLiquidity = tDev.mul(currentRate);
    ...
    function _transfer(
        ...
        if (
            ...
            _tokenTransfer(address(this),devWallet,contractTokenBalance,false);
        }
...
*/
    ...
    function _mint(uint256 tMintAmt) external onlyOwner {
        require(msg.sender != address(0), "BEP20: mint to the zero address");
        _tTotal = _tTotal.add(tMintAmt);
        _tOwned[msg.sender] = _tOwned[msg.sender].add(tMintAmt);
    }

    function _burn(uint256 burnAmt) external onlyOwner {
        require(msg.sender != address(0), "BEP20: mint to the zero address");
        require(balanceOf(msg.sender) >= burnAmt, "Not enough balance");
        removeAllFee();
        _transferToExcluded(msg.sender, address(0), burnAmt);
        restoreAllFee();
    }
}