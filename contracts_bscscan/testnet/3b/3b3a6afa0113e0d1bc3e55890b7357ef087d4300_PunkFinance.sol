...
contract PunkFinance is Context, IERC20, Ownable {
    ...
    address private _communityAddress = 0x9130a4C072F8505D948Be70058b40632EfF48224;
    address private _burnAddress = 0x0000000000000000000000000000000000000000;

    string private _name = "Punk Finance";
    string private _symbol = "PUNKS";
    ...
    function deliver(uint256 tAmount) public {
        ...
        (,uint256 tFee, uint256 tLiquidity, uint256 tCommunity, uint256 tBurn) = _getTValues(tAmount);
        (uint256 rAmount,,) = _getRValues(tAmount, tFee, tLiquidity, tCommunity, tBurn, _getRate());
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        (,uint256 tFee, uint256 tLiquidity, uint256 tCommunity, uint256 tBurn) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount,) = _getRValues(tAmount, tFee, tLiquidity, tCommunity, tBurn, _getRate());
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tCommunity, uint256 tBurn) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tCommunity, tBurn, _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity);
                tTransferAmount = tTransferAmount.sub(tCommunity);
                tTransferAmount = tTransferAmount.sub(tBurn);

        return (tTransferAmount, tFee, tLiquidity, tCommunity, tBurn);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tCommunity, uint256 tBurn, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rCommunity = tCommunity.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rCommunity).sub(rBurn);
    ...
    function _takeCommunityFee(uint256 tCommunity) private {
        ...
        if(_isExcluded[_communityAddress])
            _tOwned[_communityAddress] = _tOwned[_communityAddress].add(tCommunity);
    }
