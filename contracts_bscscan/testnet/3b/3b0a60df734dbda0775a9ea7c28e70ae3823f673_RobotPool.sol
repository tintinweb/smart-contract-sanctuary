...
contract RobotPool is IPool, Ownable, IERC721Receiver, Pausable, ReentrancyGuard{
	...
	address private immutable DEAD_ADDRESS = address(0x000000000000000000000000000000000000dEaD);
	...
	// any rewards distributed when no robots are staked
	...
	// robot take a 20% tax on all $ASH claimed
	uint256 public constant ASH_CLAIM_TAX_PERCENTAGE = 20;
	// 40% chance robot burned when unstaked
	uint256 public constant ROBOT_BURNED_PERCENTAGE = 40;
	...
	// number of Robot staked in the Pool
	...
	// amount of $ASH due for each Robot staked
	uint256 public ashPerRobot = 0; 
	...
	*/
	...
	/**
	* adds a single Robot to the pool
	* @param account the address of the staker
	* @param tokenId the ID of the Robot to add to the Pool
	*/
	function _addRobotToPool(address account, uint256 tokenId) internal whenNotPaused _updateEarnings {
		stakeStatus[tokenId] = Stake({
			owner: account,
			tokenId: uint16(tokenId),
			value: ashPerRobot,
	...
	*/
	function claimFromPool(uint16[] calldata tokenIds, bool unstake) external whenNotPaused _updateEarnings nonReentrant override{
		...
		for (uint i = 0; i < tokenIds.length; i++) {
			...
			if(specie == 2)
				owed += _claimRobotFromPool(tokenIds[i], unstake);
		}
		if (owed == 0) return;
	...
	*/
	function _claimHumanFromPool(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
		...
		_payRobotTax(owed * ASH_CLAIM_TAX_PERCENTAGE / 100); // percentage tax to staked Robots
		owed = owed * (100 - ASH_CLAIM_TAX_PERCENTAGE) / 100; // remainder goes to Human
	
		emit HumanClaimed(tokenId, owed, unstake);
	}
  
	/**
	* realize $ASH earnings for a single Robot and optionally unstake it
	* Robot earn $ASH from human protect tax
	* @param tokenId the ID of the Robot to claim earnings from
	* @param unstake whether or not to unstake the Robot
	* @return owed - the amount of $ASH earned
	*/
	function _claimRobotFromPool(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
		require(character.ownerOf(tokenId) == address(this), "NOT IN POOL");
		Stake memory stake = stakeStatus[tokenId];
		require(stake.owner == _msgSender(), "Bruh, don't touch others character.");
		owed = ashPerRobot - stake.value; // Calculate unclaimed $ASH
	
		if (unstake) {
			uint random = randomizer.random(owed);
			if(random %100 < ROBOT_BURNED_PERCENTAGE)
				character.safeTransferFrom(address(this), DEAD_ADDRESS, tokenId, ""); // robot burned
			else
				character.safeTransferFrom(address(this), _msgSender(), tokenId, ""); // send back Robot
			delete stakeStatus[tokenId];
			totalHumanStaked -= 1;
		} else {
			// reset stake
	...
	*/
	...
	/** 
	* add $ASH to claimable pot for robots
	* @param amount $ASH to add to the pot
	*/
	function _payRobotTax(uint256 amount) internal {
		if (totalRobotStaked == 0) { // if there's no staked robot
			unaccountedRewards += amount; // keep track of $ASH due to robots
		return;
		}
		// makes sure to include any unaccounted $ASH 
		ashPerRobot += (amount + unaccountedRewards) / totalRobotStaked;
