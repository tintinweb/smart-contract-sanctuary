...
contract GachaBox is Initializable , OwnableUpgradeable{
    ...
    // Update config
    function updateConfig(address _lastSurvivorCharacter, address _lastSurvivorItem, address _treasury, address _randomContract) public onlyOwner{
    ...
    // Update boxConfig
    function updateBoxConfig(uint256 _quota, uint256 _price, uint256 _boxId, uint256 _maxRewardCardNumber, address _currencyToken) public onlyOwner{
    ...
    function getRewardNFTCard(uint256 boxID, address _sender) public view returns (uint256[] memory) {
        ...
        if (boxID == 1) { 
            if (totalIdx.length < 5) { selectNumber = totalIdx.length; }
            else if (totalIdx.length == 5) { selectNumber = 4; }
            else { selectNumber = 4; needRandom = true;}
        } else if (boxID == 2) { 
 ...
 */
interface ILastSurvivorItem is IERC165Upgradeable {
    /**
