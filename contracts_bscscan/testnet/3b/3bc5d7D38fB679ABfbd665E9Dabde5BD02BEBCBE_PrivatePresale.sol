...
contract PrivatePresale {
    ...
    uint256 private previousCollected = 0;
    ...
    // investers is an array with all investor wallets
    // required for refund process
    address[] investers;
    ...
    function withdrawDepositToken() public onlyOwner {
    ...
    function privateRound() public payable {
        ...
        if (!inPresale[msg.sender].isWhiteListed){
        ...
        inPresale[msg.sender].newAmount += (txCollected*10**2)/privatePrice;
    ...
    function changePrivateRoundStatus() public onlyOwner {
        if(!privateOpen) {
            ...
            privateOpen = false;
            emit PrivateRoundStatus("Private Round is now closed!");
        }
    }

    /// @dev Changes price of tokens and pushes update to all addresses
    /// @notice Input in cents (0.2 BUSD per token = 20)
    function changePrivatePrice(uint256 _newPrice) public onlyOwner {
        require(!tgeHappened,"PrivatePresale: TGE has already happened");
        privatePrice = _newPrice;
        for (uint i=0; i< investers.length ; i++) {
            uint256 _depositAmount = inPresale[investers[i]].amount;
            inPresale[investers[i]].tokenAmount = (_depositAmount*10**2)/privatePrice;
            inPresale[investers[i]].newAmount = inPresale[investers[i]].tokenAmount;
        }
    }
    ...
    function closePresale() public onlyOwner {
        require(softCap <= totalCollected,"ClosePresale Error: Soft Cap has not been reached");
        depositToken().safeTransfer(msg.sender, totalCollected);
        totalCollected = 0;
        privateOpen = false;
    }
    ...
    /** @notice Transfers return token to the owner
    * Could be used after all claims have happened
    * Do not forget about decimals 
    */
    function withdrawReturnToken(uint256 _amount) public onlyOwner {
        require(_amount <= returnToken().balanceOf(address(this)),"Not enough tokens left");
        returnToken().safeTransfer(msg.sender, _amount);
    }
    
    /// @dev function to claim tokens after TGE has happened
    function claimTokens() public {
        require(tgeHappened == true, "Claim Error: TGE has not yet happened");
        if (inPresale[msg.sender].numOfCollectTX == 0) {
            uint256 _amountToCollect = (inPresale[msg.sender].newAmount*tgeToCollect)/(1*10**5);
            ...
            require(inPresale[msg.sender].numOfCollectTX <= 8, "Claim Error: You have already claimed all tokens");
            require(inPresale[msg.sender].newAmount >= 0,"Claim Error: You have already claimed all tokens");

            if (inPresale[msg.sender].numOfCollectTX == 1) {
                require(block.timestamp >= Collect1,"Claim Error: Claim Time has not been reached");
            } else if (inPresale[msg.sender].numOfCollectTX == 2) {
                require(block.timestamp >= Collect2,"Claim Error: Claim Time has not been reached");
            } else if (inPresale[msg.sender].numOfCollectTX == 3) {
                require(block.timestamp >= Collect3,"Claim Error: Claim Time has not been reached");
            } else if (inPresale[msg.sender].numOfCollectTX == 4) {
                require(block.timestamp >= Collect4,"Claim Error: Claim Time has not been reached");
            } else if (inPresale[msg.sender].numOfCollectTX == 5) {
                require(block.timestamp >= Collect5,"Claim Error: Claim Time has not been reached");
            } else if (inPresale[msg.sender].numOfCollectTX == 6) {
                require(block.timestamp >= Collect6,"Claim Error: Claim Time has not been reached");
            } else if (inPresale[msg.sender].numOfCollectTX == 7) {
                require(block.timestamp >= Collect7,"Claim Error: Claim Time has not been reached");
            } else if (inPresale[msg.sender].numOfCollectTX == 8) {
                require(block.timestamp >= Collect8,"Claim Error: Claim Time has not been reached");
            }
            uint256 _amountToCollect = (inPresale[msg.sender].newAmount*normalCollect)/(1*10**5);
            require(inPresale[msg.sender].newAmount >= _amountToCollect, "You have not enought tokens left");
            inPresale[msg.sender].newAmount -= _amountToCollect;
            inPresale[msg.sender].numOfCollectTX ++;
            returnToken().safeTransfer(msg.sender, _amountToCollect);
