...
contract PresaleTreasury is OlympusAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    event ExchangePresale(address indexed token, uint256 amount, address indexed exchange, uint256 value);
    ...
    constructor(
        ...
        require(_ohm != address(0), "Zero address: BONK");
        BONK = IOHM(_ohm);
    ...
    /**
     * @notice allow approved address to deposit an asset for BONK
    ...
    function deposit(
        ...
        // mint BONK needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        BONK.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to exchange reserved asset for BONK
     * @param _amount uint256
     * @param _token address
    ...
    function exchangePresale(
        ...
        IERC20(_exchange).safeTransfer(msg.sender, _amount);
        // BONK.mint(msg.sender, send_); // Just transfer what we have

        emit ExchangePresale(_token, _amount, _exchange, value);
    }

    /**
     * @notice allow approved address to burn BONK for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        BONK.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new BONK using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        BONK.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or BONK from the treasury, using sBONK as collateral. This might allow an
     * sBONK holder to provide BONK liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        ...
        sBONK.changeDebt(value, msg.sender, true);
        require(sBONK.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(BONK)) {
            BONK.mint(msg.sender, value);
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sBONK.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with BONK
     * @param _amount uint256
     */
    function repayDebtWithOHM(uint256 _amount) external {
        ...
        BONK.burnFrom(msg.sender, _amount);
        sBONK.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        ohmDebt = ohmDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(BONK), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.sBONK) {
            sBONK = IsOHM(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.sBONK) {
            // 9
            sBONK = IsOHM(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(BONK.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns BONK valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(BONK)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return BONK.totalSupply() - ohmDebt;
    }
}
...
interface ITreasury {
    ...
    function exchangePresale( 
        ...
        address _token,
        address _exchange,
        uint256 _profit) external returns (uint256);
