...
contract pixelSquidChallenger is  ERC721,Ownables {
  ...
  string public name_ = "challenger";
  struct Ability {
    uint256 strength;
    uint256 agility;
  ...
  string public symbol_ = "challenger";
  uint256 challengerMax = 20000;
  uint[] market;
  ...
  function recruitChallenger(string memory _token_name ,uint256 _quality,uint256 _lv, string memory _url, string memory _animation_url,string memory _background,uint256 _strength,uint256 _agility,uint256 _intelligence,uint256 _energy,uint256 _stamina, uint256 _val) public {
  ...
  function getAllChallengersOnMarket() public view returns (uint[] memory token_ids){
  ...
  function setChallengerTokenName(uint256 _tokenId, string calldata newName) external {
      require(msg.sender == ownerOf(_tokenId),"not owner");
      require(tokenIdToPrice[_tokenId] == 0,"this challenger list in market");
      challengers[_tokenId].token_name = newName;
  }
  
  function seeChallengerCount () public view returns (uint256) {
      return challengers.length;    
  }
  
  function setChallengerUrl(uint256 _tokenId, string calldata newUrl) external {
      require(msg.sender == ownerOf(_tokenId),"not owner");
      require(tokenIdToPrice[_tokenId] == 0,"this challenger list in market");
      challengers[_tokenId].url = newUrl;
  }
  
  function setChallengerAnimation(uint256 _tokenId, string calldata newAnimationUrl) external {
      require(msg.sender == ownerOf(_tokenId),"not owner");
      require(tokenIdToPrice[_tokenId] == 0,"this challenger list in market");
      challengers[_tokenId].animation_url = newAnimationUrl;
  }
