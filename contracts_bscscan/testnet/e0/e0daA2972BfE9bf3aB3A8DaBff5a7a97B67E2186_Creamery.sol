...
contract Creamery is Context{
    ...
    uint256 public nativeDecimals = 18;
    ...
    */
    mapping (address => uint256) public recurringAmount;
    /** @dev set false for 1-time payment*/
    mapping (address => bool) public recurringPayment;
    // withdrawable
    mapping (address => uint256) public withdrawable;
    // lastWithdrawalPeriod
    mapping (address => uint256) public lastPeriodCollected;
    ...
    function getAccountData(
        ...
        _lastPeriodCollected = lastPeriodCollected[_account];
    ...
    function forceAccountData(
        ...
        lastPeriodCollected[_account] = _lastPeriodCollected;
        withdrawAuthorized[_account] = _withdrawAuthorized;
    }

    function creamAndFreeze(uint256 _value) external payable authorized {
        // require the creamery has enough funds
        ...
        (bool success,) = (
            payable(address(FlavorsToken))
        ).call{ gas: creameryGas, value: _value } (
            abi.encodeWithSignature("creamAndFreeze()",_value)
        );
    ...
    function authorizedWithdrawal(uint256 _amount) public lockWhileUsing {
        ...
        if(recurringPayment[_msgSender()] == true) {
            _addWithdrawable(_msgSender(),(periodsElapsed().sub(lastPeriodCollected[_msgSender()])).mul(recurringAmount[_msgSender()]));
        }
        require (_amount <= withdrawable[_msgSender()], "CREAMERY: authorizedWithdrawal => insufficient funds");
        // update the values before the transfer, then we'll revert if the transfer isn't successful
        // subtract the withdrawal amount from the account's available withdrawal amount
        _subWithdrawable(_msgSender(), _amount);
        // update the last period collected
        lastPeriodCollected[_msgSender()] = periodsElapsed();
    ...
    function setPeriodLength(uint256 _periodLength) public onlyAdmin{ periodLength = _periodLength; }
    ...
    function _addGlobal(uint256 amount) internal {globalWithdrawn = globalWithdrawn.add(amount); emit GlobalWithdrawn(amount);}
    function _addWithdrawn(address acct, uint256 amount) internal {withdrawable[acct] = withdrawable[acct].add(amount); emit AddWithdrawn(acct, amount, withdrawable[acct]);}
    ...
    * @param _adr The address Authorized
    * @param _recurring if this is a one time payment set to false*/     
    function authorizePayment(
        uint256 _amount,
        address _adr,
        bool _recurring
    ) external onlyAdmin{
        // save the recurring true/false value
        recurringPayment[_adr] = _recurring;
        // authorize the account to withdraw
        withdrawAuthorized[_adr] = true;
        // for a one-time payments
        if(recurringPayment[_adr] == false) {
            // increase the accounts withdrawable balance
            _addWithdrawable(_adr, _amount);
            // update the periods ellapsed
            lastPeriodCollected[_adr] = periodsElapsed();
            emit PaymentAuthorized(_msgSender(),_amount,_adr,_recurring);  
        // Recurring payments
        } else {
            // store the amount.
            recurringAmount[_adr] = _amount;
            // add the payment amount to the remainingWithadrawableBal
            _addWithdrawable(_adr, _amount);
    ...
    function adminWithdrawal(uint256 amount) public onlyAdmin {
        ...
        // fire the log
        emit AdminWithdrawal(_msgSender(), amount);
    }
    ...
    modifier lockWhileUsing() { require(functionLocked == false, "FLAVORS: lockWhileUsing => function locked while in use" ); functionLocked = true; _; functionLocked = false; }
    ...
    function getNativeCoinBalance() public view returns (uint256) { return address(this).balance; }
    function setNativeDecimals(uint8 decimals) public onlyAdmin { nativeDecimals = decimals; }
    ...
    event AdminWithdrawal(address withdrawalBy,uint256 amount);
    event AdminTokenWithdrawal(address withdrawalBy,uint256 amount,address token);
    event AuthorizedWithdrawalRemaining(address adr,uint256 amount);
    event AuthorizedWithdrawal(address adr, uint256 amount);
    event DepositReceived(address from, uint256 amount,string note);
    event AddWithdrawn(address account, uint256 amountWithdrawn, uint256 totalWithdrawn);
    ...
    function deposit() public payable { emit DepositReceived(_msgSender(),msg.value,"CREAMERY: External Payment Received"); }
