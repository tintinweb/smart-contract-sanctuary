...
contract Company {
    ...
    function newMemberInCompany(string memory _companyId, string memory _userMappingId, string memory role, string memory permission, string memory userMappingId, string memory dataUserMapping, uint userId) external {
        uint j = iUserMapping(userMappingId);
        userMappings[j].dataUserMapping = dataUserMapping;
        userMappings[j].userId = userId;
        for (uint i = 0; i < companyUsers.length; i++) {
            if ((keccak256(abi.encodePacked(companyUsers[i].companyId)) == keccak256(abi.encodePacked(_companyId))) &&
                (keccak256(abi.encodePacked(companyUsers[i].userMappingId)) == keccak256(abi.encodePacked(_userMappingId)))) {
                companyUsers[i].role = role;
