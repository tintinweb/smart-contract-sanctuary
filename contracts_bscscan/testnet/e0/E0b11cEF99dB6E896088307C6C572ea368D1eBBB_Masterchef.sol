...
contract Masterchef is Ownable {
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate,RewardToken _rwdToken,uint256 _lockedPeriod) public onlyOwner {
        require(_depositFeeBP <= 10000, "add: invalid deposit fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
       _lockedPeriod = _lockedPeriod.mul(1 days);
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate,RewardToken _rwdToken,uint256 _lockedPeriod) public onlyOwner {
     ...
     function startStaking(uint256 _startBlock) external onlyOwner {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 rwdReward = multiplier.mul(RewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accRewardPerShare = accRewardPerShare.add(rwdReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
       ...
       uint256 depositedAmount = pool.depositedAmount;
        ...
        uint256 multiplier = block.number.sub(pool.lastRewardBlock);
        uint256 rwdReward = multiplier.mul(RewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        //rwd.mint(devaddr,rwdReward.div(10));
        //rwd.mint(address(this),rwdReward);
    ...
    function getEqualientToken(uint256 _tokenIn,uint256 _tokenOut,uint256 _amount) private pure returns (uint256){
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        if(block.timestamp > user.lastClaim + pool.lockedPeriod){
        require(block.timestamp > user.lastClaim + pool.lockedPeriod, "You cannot withdraw yet!");
    ...
    function claim(uint256 pid) public {
        ...
        if (pending > 0 || user.pendingRewards > 0) {
            user.pendingRewards = user.pendingRewards.add(pending);
            uint256 claimedAmount = safeRewardTransfer(msg.sender, user.pendingRewards, pid);
    ...
    function safeRewardTransfer(address _to, uint256 _amount, uint256 _pid) internal returns (uint256) {
        ...
        if (_amount > pool.rewardsAmount) {
           _amount = pool.rewardsAmount;
        }
