...
contract KawaiiMinting is Signed, Pausable, SignData {
    using SafeMath for uint256;
    ...
    function setKawaiiToken(IBEP20 _kawaiiToken) external onlyOwner {
        kawaiiToken = _kawaiiToken;
    }
    ...
    function convertItem(address _nft1155Address, uint256 _tokenId, uint256 _amount, uint256 _timestamp, address _sender, bytes memory _adminSignedData, uint8 v, bytes32 r, bytes32 s) external whenNotPaused {
        ...
        address signer = getSigner(
            keccak256(
                abi.encode(address(this), this.convertItem.selector, _sender, _nft1155Address, _tokenId, _amount, _timestamp, nonces[_sender]++)
            ), v, r, s);
