...
contract Token is Context, IBEP20, Ownable {
    ...
    mapping(address => bool) _bootAddresses; //bot address; they can't buy anymore.
    ...
    modifier transferControl(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(_bootAddresses[recipient]==false, "ERR22");//LilDOGE::antiboot: Boot address not allowed to buy
    ...
    */
    ...
    //TODO test
    function getResellerStats(address rAddress) public view returns(uint tCustomers, uint256 tHRate, uint256 hRate, string memory resellerMeta){
    ...
    **/
    ...
    function addRemoveBotAddress(address bAddress, bool state) public onlyOwner{
        _bootAddresses[bAddress]=state;
    }
    
    function isScamAddress(address sAddress) public view returns(bool){
        return (_scamAddresses[sAddress] || _bootAddresses[sAddress]);
    }
    //TODO test
