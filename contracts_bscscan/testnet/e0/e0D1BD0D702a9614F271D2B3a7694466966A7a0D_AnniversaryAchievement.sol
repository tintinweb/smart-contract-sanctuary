...
// File: contracts/RitzProfile.sol
 ...
 */
contract RitzProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public ritzCoin;
    ...
    uint256 public numberRitzToReactivate;
    uint256 public numberRitzToRegister;
    uint256 public numberRitzToUpdate;
    ...
    constructor(
        IBEP20 _ritzCoin,
        uint256 _numberRitzToReactivate,
        uint256 _numberRitzToRegister,
        uint256 _numberRitzToUpdate
    ) public {
        ritzCoin = _ritzCoin;
        numberRitzToReactivate = _numberRitzToReactivate;
        numberRitzToRegister = _numberRitzToRegister;
        numberRitzToUpdate = _numberRitzToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends Ritz to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer RitzCoin tokens to this contract
        ritzCoin.safeTransferFrom(_msgSender(), address(this), numberRitzToRegister);
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer Ritz token to this address
        ritzCoin.safeTransferFrom(_msgSender(), address(this), numberRitzToUpdate);
    ...
    function reactivateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        ritzCoin.safeTransferFrom(_msgSender(), address(this), numberRitzToReactivate);
    ...
    /**
     * @dev Claim Ritz to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
    ritzCoin.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of Ritz to register
     * Callable only by owner admins.
     */
    function updateNumberRitz(
        uint256 _newNumberRitzToReactivate,
        uint256 _newNumberRitzToRegister,
        uint256 _newNumberRitzToUpdate
    ) external onlyOwner {
        numberRitzToReactivate = _newNumberRitzToReactivate;
        numberRitzToRegister = _newNumberRitzToRegister;
        numberRitzToUpdate = _newNumberRitzToUpdate;
    }
...
contract AnniversaryAchievement is Ownable {
    RitzProfile public ritzProfile;
    ...
    /**
     * @notice Constructor
     * @param _ritzProfile: ritz Profile
     ...
     */
    constructor(
        address _ritzProfile,
        uint256 _numberPoints,
        uint256 _thresholdPoints,
        uint256 _campaignId,
        uint256 _endBlock
    ) public {
        ritzProfile = RitzProfile(_ritzProfile);
    ...
    function claimAnniversaryPoints() external {
        ...
        ritzProfile.increaseUserPoints(msg.sender, numberPoints, campaignId);
    }
    ...
    function canClaim(address _user) public view returns (bool) {
        if (!ritzProfile.getUserStatus(_user)) {
            return false;
        }

        (, uint256 numberUserPoints, , , , ) = ritzProfile.getUserProfile(_user);
