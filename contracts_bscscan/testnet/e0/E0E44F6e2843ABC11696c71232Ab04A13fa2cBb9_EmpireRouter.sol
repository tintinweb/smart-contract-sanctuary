...
library EmpireLibrary {
    ...
    bytes32 internal constant PAIR_INIT_HASH =
        hex"65160f89dfeb5ab7ad07dcc800c155a36049720dc092702a8d064d2b5df38369"; // See mocks/InitHashExtractor.sol
    ...
    function quote(
        ...
        require(amountA > 0, "EpmireLibrary: INSUFFICIENT_AMOUNT");
        require(
            reserveA > 0 && reserveB > 0,
            "EpmireLibrary: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "EpmireLibrary: INSUFFICIENT_INPUT_AMOUNT");
        require(
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, "EpmireLibrary: INSUFFICIENT_OUTPUT_AMOUNT");
        require(
            reserveIn > 0 && reserveOut > 0,
            "EpmireLibrary: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountsOut(
        ...
        require(path.length >= 2, "EpmireLibrary: INVALID_PATH");
...
contract EmpireRouter is IEmpireRouter {
    ...
    // **** ADD EMPIRE LIQUIDITY ****   
    function _addEmpireLiquidity(
        address tokenA,
        address tokenB,
        uint256 amountADesired,
        uint256 amountBDesired,
        ...
        // create the pair if it doesn't exist yet
        if (IEmpireFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IEmpireFactory(factory).createEmpirePair(tokenA, tokenB, pairType, _option);
        }
        (uint256 reserveA, uint256 reserveB) =
            EmpireLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
    ...
    {
        // require that the pair is brand new
        require(IEmpireFactory(factory).getPair(tokenA, tokenB) == address(0));
        (amountA, amountB) = _addEmpireLiquidity(
            tokenA,
            tokenB,
            amountADesired,
            amountBDesired,
    ...
    function addEmpireLiquidityETH(
        address token,
        uint256 amountTokenDesired,
        uint256 amountTokenMin,
        uint256 amountETHMin,
    ...
    {
        (amountToken, amountETH) = _addEmpireLiquidity(
            token,
            WETH,
            amountTokenDesired,
            msg.value,
