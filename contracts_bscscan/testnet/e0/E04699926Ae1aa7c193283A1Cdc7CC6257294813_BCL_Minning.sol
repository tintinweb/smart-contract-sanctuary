...
contract BCL_Minning is Ownable{
    ...
    function checkReward(address addr_) public view returns(uint _reward){
        ...
        _reward = (_debt - userInfo[addr_].debt) * userInfo[addr_].stakeAmount / Acc;
        _reward  = (_reward * coe) / 10 ;
    }
    ...
    function claimReward() public checkMiners{
        ...
        if (brunCoe > 0){
            BCL.transfer(bank, (_newRew * brunCoe) / 10);
        }
    ...
    function unStakeLp() external checkMiners {
        require(userInfo[msg.sender].stakeAmount > 0, 'no amount');
        UserInfo storage uu = userInfo[msg.sender];
