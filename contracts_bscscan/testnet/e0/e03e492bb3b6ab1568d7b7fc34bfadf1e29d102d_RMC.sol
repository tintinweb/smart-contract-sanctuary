...
contract RMC is Context, IERC20, Ownable {
    ...
    string private _name = "Rubymoon";
    string private _symbol = "RMC";
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tMarketing, data.tCharity);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tMarketing).sub(tCharity);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tMarketing, tCharity, 0));
    }
