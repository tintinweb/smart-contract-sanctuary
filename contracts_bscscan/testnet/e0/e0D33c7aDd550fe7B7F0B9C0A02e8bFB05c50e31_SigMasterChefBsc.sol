...
//    address public sharedVault;
    ...
    function totalRewardAtBlock(uint256 atBscBlock) public view returns (uint256) {
        if (atBscBlock < startBscBlock) return 0;
        uint256 atEthBlock = bscToEthBlockNumber(atBscBlock);
        uint256 startEthBlock = bscToEthBlockNumber(startBscBlock);
        uint256 t = (atEthBlock - startEthBlock).div(rewardTimeFactor);
...
//        sharedVault = _sharedVault;
    ...
    // Faction of total mint which is belong to BSC chain.
    // Same amount of SigToken token gets lock on Ethereum bridge contract
