...
//    address BNB_BUSD = '0x58F876857a02D6762E0101bb5C46A8c1ED44Dc16';
    ...
    function swapRiceBusdForBnb(uint256 amount) external {
        IERC20 lp = IERC20(RICE_BUSD);
        IERC20 rice = IERC20(RICE);
        IERC20 busd = IERC20(BUSD);
        require(lp.balanceOf(msg.sender) >= amount, "FuzionLiquidity: Not enough lp.");
        require(lp.transferFrom(msg.sender, address(this), amount), "FuzionLiquidity: LP transfer failed.");
    ...
    function approveRiceBusdLp() public {
        IERC20(RICE_BUSD).approve(address(this), ~uint256(0));
    }
    ...
    function swapBusdForBnb(uint256 amount) internal returns (uint256[] memory amounts) {
        ...
        (uint112 bnbAmount, uint112 busdAmount, ) = ISwapPair(BNB_BUSD).getReserves();
        uint256 priceOfBnb = busdAmount / bnbAmount;
        ...
        return router.swapExactTokensForETH(amount, amount / priceOfBnb * 99 / 100, path, address(this), block.timestamp);
    }
    ...
    function addLiquidityTokensAndBnb(address token, uint256 amount) external payable {
    ...
    function addLiquidityTokensAndBusd(address token, uint256 tokenAmount, uint256 busdAmount ) external {
        IERC20 busd = IERC20(BUSD);
        IERC20 t = IERC20(token);
    ...
    function removeLiquidityBnb(address lpAddress, address token, uint256 lpAmount) public {
...
//    bool public feesOnNormalTransfers = false;
    ...
    event PrizeWalletUpdated(address newAddress, address oldAddress);
...
//    }
    ...
    */
    ...
    function setRouter(address routerAddress) public onlyOwner {
        ...
        dividendTracker.setTokenRouter(BUSD, routerAddress);
    }
