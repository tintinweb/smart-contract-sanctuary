...
contract MoneyMakerToken is Context, IBEP20, Ownable {
    ...
    uint256 public base = 10 * (10 ** _decimals-2);
    uint256 public currentPrice_= base  ;
    uint256 public presale_price = 3 * (10 ** _decimals-2);
    uint256 public prisale_price = 6 * (10 ** _decimals-2);
    uint256 public pubsale_price = 9 * (10 ** _decimals-2);
    ...
    constructor (address _tokenBUSDAddress,address _EACAggregatorProxyAddress) {
        ...
        presaledays = block.timestamp.add(60);// after live deployedtime.add(864000);
        privatesaledays = presaledays.add(60);// after live presaledays.add(864000);
        publicsaledays = privatesaledays.add(60);// after live privatesaledays.add(864000);
    ...
    {
       return ((busdAmount / currentPrice_) * (10 ** _decimals)) ;
    }

    function TokenToBUSD(uint TokenAmount) public view returns(uint)
    {
        return TokenAmount / (10**_decimals) * currentPrice_;
    }
    ...
    function buy( uint256 tokenAmount) public payable returns(bool)
    {
      ...
      if(block.timestamp <= presaledays)
      {
        ...
        MMTToken = BUSDToToken(tokenAmount);
        //MMTToken = MMTToken * presale_price;
        require(MMTToken <  totalPreAllocated,"Amount exceeds");
        presalesoldout +=MMTToken;
        //payable(_customerAddress).transfer(MMTToken);
        userInfo[_customerAddress]._frozenbalances = userInfo[_customerAddress]._frozenbalances.add(MMTToken);
        userInfo[_customerAddress]._frozentimeout = block.timestamp.add(21040000) ;
      }
      if(block.timestamp > presaledays && block.timestamp <= privatesaledays)
      {
       ...
       // MMTToken = MMTToken * prisale_price;
        ...
        //payable(_customerAddress).transfer(MMTToken);
        userInfo[_customerAddress]._frozenbalances = userInfo[_customerAddress]._frozenbalances.add(MMTToken);
        userInfo[_customerAddress]._frozentimeout = block.timestamp.add(15780000) ;
      }
      if(block.timestamp > presaledays && block.timestamp > privatesaledays && block.timestamp <= publicsaledays)
      {
        ...
        //payable(_customerAddress).transfer(MMTToken);
        userInfo[_customerAddress]._frozenbalances = userInfo[_customerAddress]._frozenbalances.add(MMTToken);
        userInfo[_customerAddress]._frozentimeout = block.timestamp.add(7890000) ;
      }
      tokenInterface(tokenBUSDAddress).transferFrom(msg.sender, address(this), BUSDToken);
      emit BuyToken(msg.sender, MMTToken);
      return true;
    }
