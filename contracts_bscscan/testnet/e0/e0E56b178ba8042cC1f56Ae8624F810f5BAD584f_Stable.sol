...
contract Stable is Ownable , ERC721Holder,Divestor {
    ...
    address public cattleItem;
    uint public maxGrowAmount = 100 ;
    uint public refreshTime;
    uint public upLevelPrice = 100e18;
    uint[] public levelLimit = [10000 ,5000,2000,1000,500,200,100];
    struct UserInfo {
        uint stableAmount;
        uint stableExp;
        uint[] cows;
    ...
    function setAdmin(address addr_, bool com_) external onlyOwner {
        admin[addr_] = com_;
        ICOW(cattle).setApprovalForAll(addr_,com_);
    }
    ...
    function putIn(uint tokenId) external {
        ...
        require(ICOW(cattle).deadTime(tokenId) > block.timestamp,'cattle is dead');
        require(user.cows.length < user.stableAmount, 'out of stableAmount');
        require(findOnwer(tokenId) == _msgSender(), 'not owner');
        ICOW(cattle).safeTransferFrom(msg.sender,address(this),tokenId);
    ...
    function putOut(uint tokenId) external {
        ...
        if (block.timestamp < ICOW(cattle).deadTime(tokenId)){
            ICOW(cattle).safeTransferFrom(address(this),msg.sender,tokenId);
        }
    ...
    function feed(uint tokenId, uint amount_) external {
        require(isStable[tokenId], 'not in stable');
        require(CattleOwner[tokenId] == _msgSender(), 'not owner');
        ICattle1155(cattleItem).burn(msg.sender,forageId,amount_);
        uint energyLimit = ICOW(cattle).getEnergy(tokenId);
    ...
    function useItem(uint tokenId, uint itemId, uint amount_) external {
        require(isStable[tokenId], 'not in stable');
        require(CattleOwner[tokenId] == _msgSender(), 'not owner');
        uint[3]memory effect = ICattle1155(cattleItem).checkItemEffect(itemId);
        if (effect[0] > 0){
            uint energyLimit = ICOW(cattle).getEnergy(tokenId);
        ...
        if (effect[2] > 0){
            ICOW(cattle).addDeadTime(tokenId,effect[2] * amount_);
        }
        ICattle1155(cattleItem).burn(msg.sender,itemId,amount_);
    ...
    function buyStable(uint amount_) external {
        ...
        IERC20(BVT).safeTransferFrom(_msgSender(), address(this), amount_ * stablePrice);
