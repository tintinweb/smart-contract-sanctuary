...
contract UniswapV2TokenExchange is ITokenExchange, AccessControlEnumerableUpgradeable {
    ...
    function sellExactInput(
        ...
        address recipient,
        uint256 amountIn_
    ) external override onlyRole(TRADING_ROLE) returns (uint256) {
        // check
        ...
        // amountIn_ in max 18, price in max 18, outputToken in max 18, tp.slippage in 18
        // if all multiplication is done before division, overflow is plausible
        // To avoid overflow, calculate exactAmount first
        uint256 exactAmount = (amountIn_ * price * 10**outputToken.decimals()) / 10**(inputToken.decimals() + dec);
        // the error of amountOutMinimum will be +- 1 wei since (1e18 - tp.slippage)) / 1e18 < 1 wei
        // the accurarcy of is not vitally important as it is just specifying a limit
