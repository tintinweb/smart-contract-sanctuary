...
contract Shop is IShop, PancakeSwapHelper, EmergencyWithdrawable {
    ...
    uint256 public coalValue = 300000000000000;
    uint8 public globalDiscountRate;
    uint8 public slippage = 70;

    event ItemBought(uint256 id, uint256 price, uint8 currency, address from);
    ...
    function upsertItem(uint256 id, uint8 typeId, uint256 price, uint8 discountRate, uint8 bulkDiscountRate, uint256 val1, uint256 val2, address fulfilment, address fundsReceiver, bool allowCoalCurrency) external override onlyAdmins {
        ...
        item.allowCoalCurrency = allowCoalCurrency;

        xld.approve(fundsReceiver, ~uint256(0));
    }
    ...
    function buyItem(uint256 id, uint256 quantity, uint256 param) external payable notUnauthorizedContract notPaused {
        ShopItem storage item = items[id];
        ...
        IDepositable(item.fundsReceiver).deposit{value: msg.value}(address(0), 0);
        IShopItemFulfilment(item.fulfilment).fulfill(id, msg.sender, quantity, param);

        emit ItemBought(id, msg.value, 0, msg.sender);
    }

    function buyItemWithXLD(uint256 id, uint256 quantity, uint256 amount, uint256 param) external notUnauthorizedContract notPaused {
        ShopItem storage item = items[id];
        require(id > 0 && item.id == id, "Shop: Item does not exist");
        ...
        IShopItemFulfilment(item.fulfilment).fulfill(id, msg.sender, quantity, param);
        emit ItemBought(id, amount, 1, msg.sender);
    }

    function buyItemWithCoal(uint256 id, uint256 quantity, uint256 param) external notUnauthorizedContract notPaused {
        ShopItem storage item = items[id];
        require(id > 0 && item.id == id, "Shop: Item does not exist");
        require(quantity >= 1, "Shop: Invalid quantity");
        require(item.allowCoalCurrency, "Shop: Coal not allowed");

        uint256 price = priceOf(item, quantity);
        uint256 priceCoal = price / coalValue;

        lottery.decreaseCoal(msg.sender, priceCoal);
        IShopItemFulfilment(item.fulfilment).fulfill(id, msg.sender, quantity, param);

        emit ItemBought(id, priceCoal, 2, msg.sender);
    }
    ...
    function setLottery(ILottery _lottery) external onlyOwner {
        require(address(_lottery) != address(0), "Shop: Invalid address");
        lottery = _lottery;
    }
}
