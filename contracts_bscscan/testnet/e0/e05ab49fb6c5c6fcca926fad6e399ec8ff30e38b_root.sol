...
contract root {
    ...
    function estProfit (address _addrA, address _addrB,address[] calldata buyPath, address[] calldata reversedSellPath) public view returns (uint256){
        uint256 buyAmountA = Router(_addrA).getAmountsOut(buyQuantity, buyPath)[buyPath.length - 1];
        uint256 _profit = Router(_addrB).getAmountsOut(buyAmountA, reversedSellPath)[reversedSellPath.length - 1];
