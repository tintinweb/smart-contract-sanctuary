...
contract LadyLassie is Context, IERC20, Ownable {
    ...
    //anti-leech related variables
    uint public antileech_block;
    ...
    { 
    //added antileech function to set fees
    if(block.number <= antileech_block+3) {
            _setTaxes(18,36,18,9,18);
...
// Added the argument _leechShieldStatus to set up the leechshield. If true, the next 3 blocks will be taxed to 99%


    function setTradingStatus(bool state, bool _leechShieldStatus) external onlyOwner{
        tradingEnabled = state;
        swapEnabled = state;
        //adding antileech variable
        if(state == true && _leechShieldStatus == true) antileech_block = block.number;
    ...
    //added a private _setTaxes function for the antileech system, to allow calling _setfee by the contract
    function _setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _dev, uint256 _buyback) private {
       taxes = Taxes(_rfi,_marketing,_liquidity,_dev,_buyback);
        emit FeesChanged();
    }
