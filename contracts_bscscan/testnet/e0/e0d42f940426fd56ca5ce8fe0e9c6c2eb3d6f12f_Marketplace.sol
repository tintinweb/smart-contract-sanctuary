...
contract Marketplace is ERC1155, ERC2981Base, Ownable {
    ...
    modifier onlyNFTOwner(string memory _tokenHash) {
        require(getNFTInfo(_tokenHash).currentOwner == msg.sender, "Not NFT Owner...");
        _;
    }

    modifier notOnlyNFTOwner(string memory _tokenHash) {
        require(getNFTInfo(_tokenHash).currentOwner != msg.sender, "NFT-Owner cannot bid...");
        _;
    }
    ...
    function putNFT2Marketplace(string memory _tokenHash, uint256 price) public onlyNFTOwner(_tokenHash) {
        require(tokenHashExists[_tokenHash], "Non-Existing NFT hash value....");
        getNFTInfo(_tokenHash).lastBidder = payable(msg.sender);
        getNFTInfo(_tokenHash).lastPrice = price;
        safeTransferFrom(getNFTInfo(_tokenHash).currentOwner, address(this), getTokenIdFromHash[_tokenHash], 1, '');
    }

    function placeBid(string memory _tokenHash, uint8 kind) public notOnlyNFTOwner(_tokenHash) payable {
        require(tokenHashExists[_tokenHash], "Non-Existing NFT hash value....");
        SpectraNFT memory spectraNFT = getNFTInfo(_tokenHash);
        if (spectraNFT.currentOwner == spectraNFT.lastBidder)
            return;

        if (spectraNFT.lastPrice < msg.value) {
            transferToContract(spectraNFT.lastBidder, spectraNFT.lastPrice, kind);
    ...
    function transferNFT(string memory _tokenHash, uint8 kind) external {
        ...
        uint256 price = getNFTInfo(_tokenHash).lastPrice;
        address ownerOfNFT = getNFTInfo(_tokenHash).currentOwner;
        address lastBidder = getNFTInfo(_tokenHash).lastBidder;
        
        safeTransferFrom(address(this), lastBidder, indexOfToken, 1, '');
