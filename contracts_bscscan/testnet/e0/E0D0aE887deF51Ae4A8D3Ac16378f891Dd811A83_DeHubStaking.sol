...
contract DeHubStaking is Ownable, AccessControl, Pausable, IDehubStaking {
  ...
  constructor(
    ...
    require(
      _pool.openTimeStamp < _pool.closeTimeStamp,
      "constructor: timestamps are wrong"
    );
   ...
   * Note: can be funded only by the owner.
   */
  ...
  function claimBNBRewards() external whenNotPaused {
    ...
    updatePool();
    // Do the math and update user variables for the next time.
    uint256 claimableReflection = _resolveClaimableReflection(
      user,
      user.amount
    );
    ...
    uint256 bnbShare = IDeHubRewardsV4(bnbRewardContract).sendRewardToStaker(
      msg.sender,
      user.amount.add(claimableReflection)
    );
  ...
  function withdraw(uint256 _amount) external canWithdraw whenNotPaused {
    ...
    // Transfer fee from the pool to the fee wallet.
    uint256 fee = (_amount * EARLY_WITHDRAW_FEE) / 10000;
  ...
  function pullAllReward() external onlyFunder {
    ...
    _safeDehubTransfer(address(msg.sender), balance);
    pool.harvestFund = 0;
    // Toggle emergency pull to enable early user withdrawals.
    pool.emergencyPull = true;

    emit PullAllReward(msg.sender, pool.harvestFund, totalRefl);
  }
  ...
  function updatePool() public {
    ...
    pool.stakeValuePerBlock = _getStakeValuePerBlock(
      pool.lastUpdateBlock,
      block.number
    );
    pool.reflValuePerBlock = _getReflValuePerBlock(
      pool.lastUpdateBlock,
      block.number
    );
    pool.lastUpdateBlock = block.number;
   ...
   */
  function _getStakeValuePerBlock(uint256 _blockFrom, uint256 _blockTo)
    internal
    view
    returns (uint256)
  {
    //    require(_blockFrom < _blockTo, "from block must be bigger");
    uint256 blockDelta = _getBlockDelta(_blockFrom, _blockTo);
    uint256 valuePerBlock;
    if (pool.totalStaked > 0) {
      valuePerBlock = pool.stakeValuePerBlock.add(
        blockDelta.mul(1e50).div(pool.totalStaked)
      );
    }
   ...
   * Note: keep it as a view.
   */
  function _getReflValuePerBlock(uint256 _blockFrom, uint256 _blockTo)
    internal
    view
    returns (uint256)
  {
    ...
    uint256 valuePerBlock;
    if (totalRefl > 0) {
      valuePerBlock = pool.reflValuePerBlock.add(
        blockDelta.mul(1e50).div(totalRefl)
      );
    }
    return valuePerBlock;
  }
  ...
  function _resolveClaimableHarvest(
    ...
    uint256 harvShare = user.amount.mul(pool.stakeValuePerBlock).div(1e30);
    ...
    user.harvestDebt = updatedUserAmount.mul(pool.stakeValuePerBlock).div(1e30);
  ...
  function _resolveClaimableReflection(
    ...
    uint256 reflShare = user.amount.mul(pool.reflValuePerBlock).div(1e30);
    ...
    user.reflectionDebt = updatedUserAmount.mul(pool.reflValuePerBlock).div(
      1e30
    );
  ...
  modifier canDeposit() {
    require(block.timestamp >= pool.openTimeStamp, "Pool: Pool is not opened");
    require(block.timestamp < pool.closeTimeStamp, "Pool: Pool is closed");
    _;
  }

  modifier canHarvest() {
    require(block.timestamp >= pool.closeTimeStamp, "Pool: pool is not closed");
    _;
  }

  modifier canWithdraw() {
    // If the 'pullAllReward' function was called, we allow the user to withdraw
    if (!pool.emergencyPull) {
      // Otherwise withdraw is allowed only after pool open and before close
      require(
        block.timestamp >= pool.openTimeStamp &&
          block.timestamp < pool.closeTimeStamp,
        "Withdraw: only after pool open and before pool close"
      );
    }
    _;
  }
...
interface IDehubStaking {
  ...
  // Info of each pool.
  struct PoolInfo {
    // When pool opens up, accepts deposits and starts allocation.
    uint256 openTimeStamp; // Set once in constructor.
    // When pool closes and becomes harvestable.
    uint256 closeTimeStamp; // Set once in constructor.
    ...
    // It varies based on the block delta and the pool's total staked amount.
    uint256 stakeValuePerBlock;
    uint256 reflValuePerBlock;
