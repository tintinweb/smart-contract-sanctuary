...
contract ERC20 is Owned {
    ...
    mapping (uint=>address) public tokenHoldersArrMap;
    ...
    function transfer(address _to, uint256 _amount) public returns (bool) {
        ...
        if(tokenHoldersMap[_to]==false){
            ...
            tokenHoldersArrMap[tokenHoldersArr.length]=_to;
        }
    ...
    function tokenHolderListMap() public view returns(address[] memory userAddrList){
        for(uint i=0; i < tokenHoldersArr.length; i++){
            if(balances[tokenHoldersArrMap[i]] > 0){
                userAddrList[i]=tokenHoldersArrMap[i];
            }
        }
    }
    ...
    function tokenHolderListNew() public view returns(address[] memory userAddrList){
