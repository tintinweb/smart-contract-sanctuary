...
contract Arcade is Context, IBEP20, Ownable {
    ...
    function reflectionBurn(uint256 amountToReflectionBurn) private {
    ...
    function distributeBusd(uint256 contractTokenBalance) private {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        if (_reflectionFee > 0 || _burnFee > 0) {
            ...
            reflectionBurn(amountToReflectionBurn);
        }

        distributeBusd(contractTokenBalance);

        emit SwapAndLiquify(contractTokenBalance, 0);
    }
    ...
    function triggerAutoBurn() internal {
