...
contract StakingWizardMinter is Ownable, Pausable {
    ...
    function allStakesLength() private view returns (uint256) {
    ...
    function mintWizard() public whenNotPaused returns (uint256) {
        require(allStakesLength() > userClaimedAmount[_msgSender()], 'Can not claim Wizard');
        require(allStakesLength() - userClaimedAmount[_msgSender()] > myActiveStakesCount(_msgSender()), 'Unstake first to claim Wizards');
    ...
    function mintManyWizards(uint256 _maxAmount) external whenNotPaused {
        uint256 toUnstakeAmount = Math.min(_maxAmount, allStakesLength());
