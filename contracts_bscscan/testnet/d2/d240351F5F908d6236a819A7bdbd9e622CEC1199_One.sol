...
contract One is ERC721, BAYC, ContractURI, IMAX721, IMAX721Whitelist, Whitelist, ERC165Storage, PaymentSplitter, Developer, Ownable {
  using Counters for Counters.Counter;
  Counters.Counter private _tokenIdCounter;
  Counters.Counter private _teamMintCounter;
  uint private mintStartID;
  uint private constant mintFees = 0.1 ether;
  uint private constant presaleMintFees = 0.07 ether;
  uint private endOfPresale;
  uint private constant mintSize = 7000;
 ...
 */
  ...
  function publicMint(uint amount) public payable {
    ...
    if(enableWhiteList) {
      require(msg.value == presaleMintFees * amount, "Wrong amount of Native Token");
 ...
 */
  ...
  // @notice click this to start it up initally
  ...
  // @notice this will enable/disable publicMint()
  function toggleMinter() public onlyOwner {
    bool old = enableMinter;
    enableMinter = !enableMinter;
    emit UpdatedMintStatus(old, enableMinter);
  }

  // @notice this will enable/disable whitelist or "if" in publicMint()
 ...
 */
  ...
  function setProvidence(string memory _images, string memory _json) public onlyDev {
    require(!lockedProvidence, "Already Set!");
    // This is the initial setting
  ...
  function addPayee(address addy, uint shares) public onlyDev {
  ...
  function sweepEthToAddress(address _user, uint _amount) public onlyDev {
