...
// File contracts/interfaces/IFieldVault.sol

interface IFieldVault {
...
contract SweetVault is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 stake;
        // How many staked $FIELD user had at his last action
        uint256 autoFieldShares;
        // Field shares not entitled to the user
        uint256 rewardDebt;
    ...
    IERC20 constant public FIELD = IERC20(0x84b9B910527Ad5C03A9Ca831909E21e236EA7b06);
    IFieldVault immutable public AUTO_FIELD;
    IERC20 immutable public STAKED_TOKEN;

    // Runtime data
    mapping(address => UserInfo) public userInfo; // Info of users
    uint256 public accSharesPerStakedToken; // Accumulated AUTO_FIELD shares per staked token, times 1e18.
    ...
    address[] public pathToField; // Path from staked token to FIELD
    ...
    // Setting updates
    event SetPathToField(address[] oldPath, address[] newPath);
    ...
    constructor(
        ...
        require(
            _pathToField[0] == address(_farmRewardToken) && _pathToField[_pathToField.length - 1] == address(FIELD),
            "SweetVault: Incorrect path to FIELD"
        );
        ...
        AUTO_FIELD = IFieldVault(_autoField);
    ...
    // 1. Harvest rewards
    // 2. Collect fees
    // 3. Convert rewards to $FIELD
    // 4. Stake to field auto-compound vault
    function earn(
        uint256 _minPlatformOutput,
        uint256 _minKeeperOutput,
        uint256 _minBurnOutput,
        uint256 _minFieldOutput
    ) external onlyKeeper {
        if (IS_CAKE_STAKING) {
        ...
        // Convert remaining rewards to FIELD
        _swap(
            _rewardTokenBalance(),
            _minFieldOutput,
            pathToField,
        ...
        uint256 previousShares = totalAutoFieldShares();
        uint256 fieldBalance = _fieldBalance();
        ...
        AUTO_FIELD.deposit(fieldBalance);

        uint256 currentShares = totalAutoFieldShares();
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        user.autoFieldShares = user.autoFieldShares.add(
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        // Withdraw field rewards if user leaves
        if (user.stake == 0 && user.autoFieldShares > 0) {
            _claimRewards(user.autoFieldShares, false);
        }
    ...
    function _claimRewards(uint256 _shares, bool _update) private {
        ...
        require(user.autoFieldShares >= _shares, "SweetVault: claim amount exceeds balance");

        user.autoFieldShares = user.autoFieldShares.sub(_shares);

        uint256 fieldBalanceBefore = _fieldBalance();

        AUTO_FIELD.withdraw(_shares);

        uint256 withdrawAmount = _fieldBalance().sub(fieldBalanceBefore);

        _safeFIELDTransfer(msg.sender, withdrawAmount);
    ...
    function getExpectedOutputs() external view returns (
        ...
        uint256 fieldOutputWithoutFees = _getExpectedOutput(pathToField);

        platformOutput = wbnbOutput.mul(platformFee).div(10000);
        keeperOutput = wbnbOutput.mul(keeperFee).div(10000);
        burnOutput = fieldOutputWithoutFees.mul(buyBackRate).div(10000);

        fieldOutput = fieldOutputWithoutFees.sub(
            fieldOutputWithoutFees.mul(platformFee).div(10000).add(
                fieldOutputWithoutFees.mul(keeperFee).div(10000)
            ).add(
                fieldOutputWithoutFees.mul(buyBackRate).div(10000)
            )
        );
    }
    ...
    function balanceOf(
        ...
        stake = user.stake;
        autoFieldShares = user.autoFieldShares.add(pendingShares);
        field = autoFieldShares.mul(AUTO_FIELD.getPricePerFullShare()).div(1e18);
    }
    ...
    function _fieldBalance() private view returns (uint256) {
        return FIELD.balanceOf(address(this));
    }
    ...
    function totalAutoFieldShares() public view returns (uint256) {
        (uint256 shares, , ,) = AUTO_FIELD.userInfo(address(this));

        return shares;
    }

    // Safe FIELD transfer function, just in case if rounding error causes pool to not have enough
    function _safeFIELDTransfer(address _to, uint256 _amount) private {
        uint256 balance = _fieldBalance();

        if (_amount > balance) {
            FIELD.transfer(_to, balance);
        } else {
            FIELD.transfer(_to, _amount);
        }
    }
    ...
    function setPathToField(address[] memory _path) external onlyOwner {
        require(
            _path[0] == address(FARM_REWARD_TOKEN) && _path[_path.length - 1] == address(FIELD),
            "SweetVault: Incorrect path to FIELD"
        );

        address[] memory oldPath = pathToField;

        pathToField = _path;

        emit SetPathToField(oldPath, pathToField);
    }
