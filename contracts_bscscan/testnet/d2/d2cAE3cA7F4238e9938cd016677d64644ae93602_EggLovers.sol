...
contract EggsConfigs is Ownable {
    ...
    constructor() {
        ...
        configs.busdAddress = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    ...
    function setNestExpirationTime(uint newValue) onlyOwner external {
        configs.nestExpirationTime = newValue;
    }
    ...
    function setBusdAddress(address newValue) onlyOwner external {
        configs.busdAddress = newValue;
    }
...
contract EggLovers is Ownable, EggsConfigs, Pausable {
    ...
    function getBusdContract() internal view returns(IERC20){
        return IERC20(EggsConfigs.configs.busdAddress);
    }
    ...
    function sellCoins(uint256 _amount) external whenNotPaused {
        ...
        getBusdContract().transfer(msg.sender, _amount * EggsConfigs.configs.coinsPrice);
    }
    ...
    function buyFrango() whenNotPaused public {
        ...
        if (i.firstBuyTs == 0) {
            ...
            i.currentWeek = block.timestamp / weekTime;
        }
        getBusdContract().transferFrom(msg.sender, address(this), 50 ether);
        i.availableChickens++;
        i.usedCards++;
    ...
    function _getContractBalance() onlyOwner public view returns (uint256) {
        return getBusdContract().balanceOf(address(this));
    }

    function _withdrawFromContract(uint256 _amount) onlyOwner public {
        require(_amount <= _getContractBalance());
        getBusdContract().transfer(owner(), _amount * EggsConfigs.configs.coinsPrice);
