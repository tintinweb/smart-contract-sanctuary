...
// File: contracts\MindfulnessNFT.sol
...
contract MindfulnessNFT is ERC721,ERC721URIStorage, ERC721Enumerable, Ownable {
    ...
    event MintedAuctionToken(address indexed minter, uint256 minBidPrice, address currency, uint256 startTime, uint256 endTime, uint256 nftID, uint256 royalty, string uri);
    ...
    event BidCreate(address indexed buyer, uint nftID, uint price, address currency);
    event BidCancel(address indexed buyer, uint nftID);

    event UpdatedSupportCurrency(address currency , bool acceptable);


    constructor() ERC721("MindfulnessNFT", "Mindfulness") {
    ...
    function bulkMint(uint256 _numberOfToken, uint256 _price, address currency, uint256 _royalty) external onlyOwner {
        for(uint i = 0; i < _numberOfToken; i++){
            ...
            mintFixed(_tokenUri, _price, currency, _royalty);
            bulkMintIndex = bulkMintIndex.add(1);
        }
    }
    

    function mintFixed(string memory _tokenURI, uint256 _price, address _currency, uint256 _royalty) public returns (uint256) {
        ...
        mapNFTAttribute[_tokenId].currency = _currency;
    ...
    function mintAuction(string memory _tokenURI, uint256 _minBidPrice, address _currency, uint256 _startTime, uint256 _endTime, uint256 _royalty) external returns (uint256) {
        uint256 _tokenId = mintIndex;
        mintIndex = mintIndex.add(1);
        ...
        emit MintedAuctionToken(_msgSender(), _minBidPrice, _currency, _startTime, _endTime, _tokenId, _royalty, _tokenURI);
    ...
    function buy(uint256 _id) external payable _validateBuy(_id) {
        ...
        address _currency = getCurrencyOfBid(_id, _msgSender());

        if (_currency != address(0x0)){//Not BNB
            address _buyer = _newOwner;
            ...
            IERC20 paymentToken = IERC20(_currency);
    ...
    function bid(uint256 _id, uint256 _price, address _currency) external _validateBid(_id, _price){
        BidEntity memory newBidEntity = BidEntity(_id, _msgSender(), _price, _currency);
        bidArrayOfToken[_id].push(newBidEntity);
        bidStatusOfToken[_id][_msgSender()] = true;

        emit BidCreate(_msgSender(), _id, _price, _currency);
    }
    ...
    function sell(uint256 _id, address _buyer) public _validateSell(_id, _buyer){
        ...
        uint256 _price = 0;
        address _currency = address(this);

        for(uint256 i = 0; i < bidArrayOfToken[_id].length; i++)
        {
            if (bidArrayOfToken[_id][i].buyer == _buyer)
            {
                _price = bidArrayOfToken[_id][i].price;
                _currency = bidArrayOfToken[_id][i].currency;
                break;
            }
        }
        ...
        require(_price <= paymentToken.allowance(_buyer, address(this)), "Error, the allowance amount is lower");
        require(_price <= paymentToken.balanceOf(_buyer), "Error, the balance is lower");
        ...
        paymentToken.transferFrom(_buyer, mapNFTAttribute[_id].creator, _royaltyValue);
        paymentToken.transferFrom(_buyer, feeAddress, _commissionValue);
        paymentToken.transferFrom(_buyer, ownerOf(_id), _price.sub(_commissionValue).sub(_royaltyValue));
        ...
        emit Sell(ownerOf(_id), _buyer, _price, _id, tokenURI(_id));
    }

    function startAuction(uint256 _tokenId, uint256 _minBidPrice, address _currency, uint256 _startTime, uint256 _endTime) external _validateStartAuction(_tokenId){
        mapNFTAttribute[_tokenId].minBidPrice = _minBidPrice;
        mapNFTAttribute[_tokenId].currency = _currency;
        mapNFTAttribute[_tokenId].startTimestamp = _startTime;
        mapNFTAttribute[_tokenId].endTimestamp = _endTime;
    ...
    function getCurrencyOfBid(uint256 _id, address _buyer) public view returns(address){
        for(uint256 i = 0; i < bidArrayOfToken[_id].length; i++)
        {
            if (bidArrayOfToken[_id][i].buyer == _buyer)
            {
                return bidArrayOfToken[_id][i].currency;
            }
        }
        return address(this);
    }
