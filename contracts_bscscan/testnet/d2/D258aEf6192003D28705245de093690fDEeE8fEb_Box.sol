...
contract Box {
    ...
    mapping(address => uint256) private boxLengedBought;
    ...
    constructor(
        ...
        pvsPlace = PlaceveresNFT(_searPlace);
    ...
    function buyEpicBox() public whenNotPaused {
        ...
        require(_epicBoxCounter.current() < boxEpicSale, "Sold out");
        require(
            block.timestamp - lastBuyEpic[sender] > antiBotTime,
            "Wait minutes to buy new box"
        );

        require(searToken.transferFrom(sender, address(this), boxEpicPrice));
        ...
        uint256 boxDetail = BoxDetails.encode(
            BoxDetails.Details(boxId, BOX_BOUGHT, boughtAt, EPIC, 0)
        );
    ...
    function buyLegendBox() public whenNotPaused {
        address sender = msg.sender;
        uint256 boughtLegend = boxLengedBought[sender];

        require(boughtLegend >= boxLegendPerUser, "Limit buy legend");
        require(_legendBoxCounter.current() < boxLegendSale, "Sold out");

        require(searToken.transferFrom(sender, address(this), boxLegendPrice));

        boxLengedBought[sender] = boughtLegend.add(1);
        ...
        uint256 boughtAt = block.timestamp;
        uint256 boxDetail = BoxDetails.encode(
            BoxDetails.Details(boxId, BOX_BOUGHT, boughtAt, LEGEND, 0)
        );
    ...
    function openBox(
        ...
        emit OpenBox(sender, _boxId, tokenId, _boxHash, _placeId, _uri);
    }

    function setAntiBotTime(uint256 timesec) public onlyOwner {
        antiBotTime = timesec;
        emit SetAntiBoxTime(owner, timesec);
    }
    ...
    function setBoxEpicPrice(uint256 _boxEpicPrice) public onlyOwner {
        require(_boxEpicPrice > 0, "box epic price is zero");
        boxEpicPrice = _boxEpicPrice;
    }

    function setBoxLegendPrice(uint256 _boxLegendPrice) public onlyOwner {
        require(_boxLegendPrice > 0, "box epic price is zero");
        boxLegendPrice = _boxLegendPrice;
    }
    ...
    function setBoxEpicSale(uint256 _boxEpicSale) public onlyOwner {
        require(
            ...
            "Number of epic box sale must be bigger than init box sale"
        );
    ...
    function setBoxLegendSale(uint256 _boxLegendSale) public onlyOwner {
        require(
            ...
            "Number of legend box sale must be bigger than init box sale"
        );
        boxLegendSale = _boxLegendSale;
    }

    function withdrawToken() public onlyOwner {
        uint256 amount = searToken.balanceOf(address(this));
        searToken.transfer(msg.sender, amount);
    ...
    {
        uint256 boxDetail = boxDetails[_boxId];
        require(boxDetail > 0, "Not found box");
    ...
    {
        require(_user != address(0), "User is zero");
        uint256[] memory boxIds = boxByAddress[_user];
        require(boxIds.length > 0, "User not buy any box");
