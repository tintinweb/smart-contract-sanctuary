...
contract Sensei is Context, IERC20, Ownable {
    ...
    constructor () public {
        ...
        _rOwned[0xAEd10274DF208F99D8Ec54EF7A9B837e281F01ef] = _rTotal.div(100).mul(65); // supply restant 65%
        ...
        emit Transfer(address(0), 0xAEd10274DF208F99D8Ec54EF7A9B837e281F01ef, _tTotal.div(100).mul(65)); // supply restant 65%
    }
    ...
    function _takeBurn(uint256 tBurn) private {
        uint256 currentRate =  _getRate();
        uint256 rBurn = tBurn.mul(currentRate);
        if (isMember(_msgSender()) && txAmountMember[_msgSender()] < 3) {
            address influencerAddress = getInfluencerForMember(_msgSender());
            txAmountMember[_msgSender()] = txAmountMember[_msgSender()] + 1;

            _rOwned[influencerAddress] = _rOwned[influencerAddress].add(rBurn.div(2));
            _rOwned[_msgSender()] = _rOwned[_msgSender()].add(rBurn.div(2));
            ...
            if(_isExcluded[_msgSender()]) {
                _tOwned[_msgSender()] = _tOwned[_msgSender()].add(tBurn.div(2));
            }
        } else {
