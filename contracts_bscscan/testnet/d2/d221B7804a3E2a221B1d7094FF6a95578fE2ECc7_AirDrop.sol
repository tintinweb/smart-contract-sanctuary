...
contract AirDrop is Ownable, Divestor {
    using SafeERC20 for IERC20;

    constructor() {
    }

    function airdrop(address[] calldata accounts_, uint[] calldata amounts_, address token_) onlyOwner public {
        require(accounts_.length == amounts_.length, "unequal length");

        IERC20 token = IERC20(token_);
        for (uint i = 0; i < accounts_.length; i++) {
            token.safeTransfer(accounts_[i], amounts_[i]);
        }
    }

    function airdropKTA(address[] calldata accounts_, uint[] calldata cardIds_, uint[] calldata amounts_, address kta_) onlyOwner public {
        require(accounts_.length == cardIds_.length && accounts_.length == amounts_.length, "unequal length");

        IKTA KTA = IKTA(kta_);
        for (uint i = 0; i < accounts_.length; i++) {
            KTA.mintMulti(accounts_[i], cardIds_[i], amounts_[i]);
        }
    }

    function airdropKTN(address[] calldata accounts_, uint[] calldata cardIds_, uint[] calldata amounts_, address ktn_) onlyOwner public {
        require(accounts_.length == cardIds_.length && accounts_.length == amounts_.length, "unequal length");

        IKTN KTN = IKTN(ktn_);
        for (uint i = 0; i < accounts_.length; i++) {
            KTN.mint(accounts_[i], cardIds_[i], amounts_[i]);
        }
    }
}
