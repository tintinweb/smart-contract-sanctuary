...
contract Arbitrage {
    ...
    function startArbitrage(
        ...
        require(pairAddress != address(0), 'Arbitrage: Invalid Pair');

        address token0 = IUniswapV2Pair(pairAddress).token0();
        address token1 = IUniswapV2Pair(pairAddress).token1();

        require(token0 != address(0) && token1 != address(0), 'Arbitrage: Invalid token address');

        IUniswapV2Pair(pairAddress).swap(
            tokenIn == token0 ? amount : 0,
            tokenIn == token1 ? amount : 0,
            address(this),
            abi.encode(sourceRouter, targetRouter)
        );
    }

    function executeArbitrage(address sender, uint256 amount0, uint256 amount1, bytes memory data) internal {
        ...
        require(token0 != address(0) && token1 != address(0), 'Arbitrage: Invalid empty token address');
        
        // if amount0 is zero sell token1 for token0
        // else sell token0 for token1 as a result
        address[] memory path1 = new address[](2);
        address[] memory path = new address[](2);
        path[0] = path1[1] = amount0 == 0 ? token1 : token0;
        path[1] = path1[0] = amount0 == 0 ? token0 : token1;

        (address sourceRouter, address targetRouter) = abi.decode(data, (address, address));
        require(sourceRouter != address(0) && targetRouter != address(0), 'Arbitrage: Invalid router');
        ...
        require(amountReceived > amountRequired, 'Arbitrage: Invalid amount received');
    ...
    function uniswapV2Call(address sender, uint256 amount0, uint256 amount1, bytes calldata data) external {
        executeArbitrage(sender, amount0, amount1, data);
    }
