...
contract Yokitrade is owned {
    ...
    /**
     * Get selling price for 1 yoki
     */
    function getSellPrice(uint256 tokenAmount) public view returns(uint256){
        uint256 price;
        address token0 = IPancakePair(lpAddress).token0();

        (uint112 reserve0, uint112 reserve1,) = IPancakePair(lpAddress).getReserves();
        if(token0 == yokiAddress){
    ...
    function sellTokens(uint256 yokiAmount) external {
        ...
        IYokiToken(yokiAddress).busdTransferTo(msg.sender, busdWallet, busdAmount);  // send BUSD to user
