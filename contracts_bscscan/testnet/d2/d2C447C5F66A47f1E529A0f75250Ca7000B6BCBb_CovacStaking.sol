...
contract CovacStaking is Ownable {
    ...
    struct Plan {
        ...
        uint256 effectiveRewardPercentage; // Calculated reward percentage, with Covac Tax fee with 2 decimal ex. 20% = 2000
        ...
        returns (uint256 stakeIndex)
    {
        ...
        // TODO: Check the logic below about fee and should we move reward transfer to when user withdraw?
        // Reward amount is the rewardAmount + Covac Fee to transfer
        rewardAmount = (
            _stakeAmount.mul(300 + plan.rewardPercentage).div(9700)
        );

        IBEP20(tokenAddress).transferFrom(
            rewardAccount,
            address(this),
            _stakeAmount.mul(plan.effectiveRewardPercentage).div(10000)
        );
    ...
    function withdrawStaking(uint256 _stakeIndex) public {
        ...
        // Transfer stake amount + rewared amount to the stake holder if withdrawStaking after plan expired
     ...
     * @param _effective_percentage is the reward percentage with Covac Fee of 3%
     * 		_effective_percentage = 6.3 + 1.063 * _reward_percentage;
     */
    function createPlan(
        string memory _name,
        ...
        plan.effectiveRewardPercentage = _effective_percentage;
