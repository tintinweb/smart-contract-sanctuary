...
// TODO: "getAllowedMultiplier" support
...
abstract contract WithLevelsSale is Ownable, Timed, GeneralIDO, WithLimits, WithWhitelist {
    ILevelManager public levelsStaking;
    ...
    function getUserLevelState(address account) view public returns (string memory, uint, uint, bool) {
        ...
        ? levelsStaking.getUserTier(account)
        : levelsStaking.getTierById(levelBytes.length == 0 ? 'none' : userLevel[account]);
        ...
        ? false
        : tier.random && (levelsOpen || whitelisted[account] || (winnersPicked[tier.id] > 0 && userWeight[account] > 0));
    ...
    function setLevelsStaking(ILevelManager _staking) external onlyOwner {
    ...
    function register() external ongoingRegister {
        ...
        address account = _msgSender();
        ILevelManager.Tier memory tier = levelsStaking.getUserTier(account);
        ...
        require(userWeight[account] == 0 || tier.multiplier > userWeight[account], 'Sale: Already registered');
        // If user re-registers with higher level...
        if (userWeight[account] > 0 && tier.multiplier > userWeight[account]) {
            totalWeights -= userWeight[account];
        }

        // Lock the staked tokens based on the current user level.
        // The black level is locked only if the user lock time is less than 5 days
        // so if they previously were lower level, we lock for the full period of black level.
        // But if the unlock time is more than 5 days in future, we don't lock it further.
        bool unlocksIn5Days = levelsStaking.getUserUnlockTime(account) < block.timestamp + 5 * 24 * 3600;
        if (lockOnRegister && (!isBlack || unlocksIn5Days)) {
            levelsStaking.lock(account, startTime);
        }
    ...
    function setWinners(string calldata id, address[] calldata winners) external onlyOwner {
        ...
        for (uint i = 0; i < levelAddresses[id].length; i++) {
            address addr = levelAddresses[id][i];
            // Skip users who re-registered with higher level
            if (userWeight[addr] > weight) {
                continue;
            }
