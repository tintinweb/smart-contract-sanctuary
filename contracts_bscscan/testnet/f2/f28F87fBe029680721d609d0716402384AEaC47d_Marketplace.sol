  ...
  UUPSUpgradeable
{
  ...
  function purchase(uint256 tokenId) external payable {
    ...
    uint256 netRoyalty = (sellingPrice / 100000) *
      _pentasNFT.royaltyOf(tokenId);

    require(msg.value >= sellingPrice, "Marketplace: Payment not enough");

    // Royalty payment
    address payable minter = payable(_pentasNFT.minterOf(tokenId));
    (bool paidMinter, ) = minter.call{ value: netRoyalty }("");
  ...
  UUPSUpgradeable
{
   ...
   */
  function sale(uint256 tokenId, uint256 price) external {
    // Set permission
