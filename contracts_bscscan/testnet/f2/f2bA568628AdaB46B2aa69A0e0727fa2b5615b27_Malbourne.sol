...
contract Malbourne is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 10000000000*1e9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Mal";
	...
	uint256 private _previouscharityFee = _burnFee;
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_holderFee.add(_liquidityFee).add(_burnFee).add(_charityFee)).div(100);
        return percentage;
    }
