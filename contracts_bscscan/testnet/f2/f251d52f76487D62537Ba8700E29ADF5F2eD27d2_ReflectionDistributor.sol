...
contract ReflectionDistributor is Allowable, IReflectionDistributor {
    ...
    uint256 public _minPeriod = 5 minutes;
    ...
    uint256 public _dividendsPerShareAccuracyFactor = 1_000 ** 18;
    ...
    receive() external payable {
        ...
        _dividendsPerShare = _totalAmount > 0
            ? _dividendsPerShare.add(ethAmount.div(_totalAmount).div(_dividendsPerShareAccuracyFactor))
            : 0;
    }
    ...
    function totalShares() external view returns (uint256) {
        return _totalAmount.div(_dividendsPerShareAccuracyFactor);
    }

    function totalShares2() external view returns (uint256) {
    ...
    function _shouldDistribute(address shareholder) private view returns (bool) {
        return
            block.timestamp >= _shareholderClaims[shareholder].add(_minPeriod) &&
    ...
    function _setShare(address shareholder, uint256 amount) private {
        ...
        _totalAmount = _totalAmount.add(amount).sub(previousAmount);
