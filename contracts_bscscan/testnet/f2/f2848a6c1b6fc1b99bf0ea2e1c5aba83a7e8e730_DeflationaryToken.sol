...
contract DeflationaryToken is Initializable, Context, IERC20, Ownable {
  ...
  // FEES
  uint256 public _taxFee = 5; // Distributed to Holders
  uint256 public _extraFee = 5; // Shared by wallets
  uint256 public _liquidityFee = 5; // Added to Liquidity
  ...
  uint256 public _maxTxAmount; // Max Amount to Transfer
  uint256 private numTokensSellToAddToLiquidity; // Min Amount to Liquidity

  event MinTokensBeforeSwapUpdated(uint256 minTokensBeforeSwap);
  event SwapAndLiquifyEnabledUpdated(bool enabled);
  event SwapAndLiquify(uint256 tokensSwapped, uint256 weiReceived, uint256 tokensIntoLiqudity);
  ...
  function initialize(
    ...
    // Configure Meta and Reflection Total
    _name = name_;
    _symbol = symbol_;
    _decimals = decimals_;
    _taxFee = taxFee_;
    ...
    numTokensSellToAddToLiquidity = _tTotal.mul(10).div(10000);

    _rOwned[owner_] = _rTotal;

    // Configure Liquidity Provider
    ISwapRouter02 _swapRouter = ISwapRouter02(router_);
  ...
  function setPairAndRouter(address _swapRouter, address _swapPair) external  {
    swapRouter = ISwapRouter02(_swapRouter);
    swapPair = _swapPair;
  }

  function setWallets(address[] memory _addresses, uint256[] memory _shares) public onlyOwner returns(uint256) {
    uint256 totalShares;
    for (uint256 i = 0; i < _addresses.length; i++) {
      Wallet memory _wallet = Wallet({
        grantee: _addresses[i],
        shares: _shares[i],
        addedAt: block.timestamp,
        status: true
      });
      _wallets.push(_wallet);
      totalShares = totalShares.add(_wallet.shares);
    }
    require(totalShares == 10000, "Shares Must Not Exceed 100%");
    return totalShares;
  }
  ...
  function deliver(uint256 tAmount) public returns(uint256, uint256, uint256) {
    address sender = _msgSender();
    require(!_isExcluded[sender], "Excluded Address Cannot Call This Function");
    (uint256 rAmount,,,,,,) = _getValues(tAmount);
    _rOwned[sender] = _rOwned[sender].sub(rAmount);
    _rTotal = _rTotal.sub(rAmount);
    _tFeeTotal = _tFeeTotal.add(tAmount);
    return (_rOwned[sender], _rTotal, _tFeeTotal);
  }
  ...
  function _takeLiquidity(uint256 tLiquidity, uint256 rExtra) private {
    ...
    if (rExtra > 0) {
      for (uint256 i = 0; i < _wallets.length; i++) {
        Wallet memory _wallet = _wallets[i];
        _rOwned[_wallet.grantee] = _rOwned[_wallet.grantee].add(rExtra.mul(_wallet.shares).div(10000));
      }
    }
  }
  ...
  function _transfer(
    ...
    if (overMinTokenBalance && !inSwapAndLiquify && from != swapPair && swapAndLiquifyEnabled) {
    ...
    if (withTimeLock && !_isExcludedFromFee[from] && from != swapPair && from != address(swapRouter)) {
      _timeLocks[from] = block.timestamp + timeLock;
    }

  }

  function clearExcessWeis() public returns(bool) {
    // transfer excess weis
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
    ...
    // swap tokens for ETH
    swapTokensForWei(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
