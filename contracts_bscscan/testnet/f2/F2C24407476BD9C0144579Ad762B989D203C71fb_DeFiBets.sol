...
contract DeFiBets is Context, IERC20, Ownable{
    ...
    uint256 public earlySellingFeeCutOffPercent = 20;
    ...
    string private constant _name     = "DeFiBets";
    string private constant _symbol   = "BETS";
    ...
    event StepsCheck(address fron, address to, uint256 amount, uint256 step);
    ...
    function _transfer(
        ...
        emit StepsCheck(from, to,  amount,  1);
        ...
        emit StepsCheck(from, to,  amount,  4);
        ...
        emit StepsCheck(from, to,  amount,  5);
        ...
        emit StepsCheck(from, to,  amount,  7);
...
//delete me 
    function recordLastBuy(address buyer) public {
        lastBuyTime[buyer] = block.timestamp;
        emit UpdateLastBuyTime(buyer, block.timestamp);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        emit StepsCheck(sender, recipient,  amount,  8);
        if (!takeFee) {
            _taxFee       = 0;
            _ecosystemDevelopmentTax = 0;
            _totalTaxForDistribution  = 0;
        }
         emit StepsCheck(sender, recipient,  amount,  9);
        if(_loweredTaxExclusions[sender] && takeFee){
         ...
         emit StepsCheck(sender, recipient,  amount,  10);
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            emit StepsCheck(sender, recipient,  amount,  11);
            _transferFromExcluded(sender, recipient, amount,earlySellingTaxAmount);

        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            emit StepsCheck(sender, recipient,  amount,  12);
            _transferToExcluded(sender, recipient, amount,earlySellingTaxAmount);

        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            emit StepsCheck(sender, recipient,  amount,  13);
            _transferBothExcluded(sender, recipient, amount,earlySellingTaxAmount);

        } else {
            emit StepsCheck(sender, recipient,  amount,  14);
            _transferStandard(sender, recipient, amount,earlySellingTaxAmount);
        }
        emit StepsCheck(sender, recipient,  amount,  15);
        if (!takeFee) {
            emit StepsCheck(sender, recipient,  amount,  16);
        ...
        if(_loweredTaxExclusions[sender] && takeFee){
            emit StepsCheck(sender, recipient,  amount,  17);
            _totalTaxForDistribution  = previousTaxForDistributionFee;
        }
        emit StepsCheck(sender, recipient,  amount,  18);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount,uint256 earlySellingTaxAmount) private {
        ...
        emit StepsCheck(sender, recipient,  rAmount,  19);
