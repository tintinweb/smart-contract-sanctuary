...
contract MasterChef is Ownable {
        ...
        returns (uint256,uint256,uint256)
    {
        ...
        UserInfo storage user = userInfo[_pid][_user];
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        uint256 chachaReward =  IChaChaLP(address(ChaCha)).getMultiplierForLp(block.timestamp - 1 days, block.timestamp);
        if(chachaReward==0||lpSupply == 0){
            ...
            uint256 dayChaChaReward =  chachaReward.mul(10 ** 18).div(lpbalance);
            return (user.amount,lpSupply,dayChaChaReward.mul(365));
        }
    }   
