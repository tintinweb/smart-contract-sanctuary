...
contract TWAPOracleUpdater is ERC20Permit, VaultOwned {
    ...
    function addTWAPSource(address newTWAPSourceDexPool_) external onlyOwner {
        require(_dexPoolsTWAPSources.add(newTWAPSourceDexPool_), "OhmniToken: TWAP Source already stored.");
        emit TWAPSourceAdded(newTWAPSourceDexPool_);
    }

    function removeTWAPSource(address twapSourceToRemove_) external onlyOwner {
        require(_dexPoolsTWAPSources.remove(twapSourceToRemove_), "OhmniToken: TWAP source not present.");
...
contract OhmniToken is Divine {
    using SafeMath for uint256;

    constructor() Divine("Ohmnishiba", "OHMNI", 9) {}
