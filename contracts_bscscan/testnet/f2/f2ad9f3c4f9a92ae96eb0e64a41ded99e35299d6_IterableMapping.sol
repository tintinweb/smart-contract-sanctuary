...
contract TokenRugged is ERC20, Ownable {
   ...
   TKRDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("Token Rugged", "TKR") {

    	dividendTracker = new TKRDividendTracker();

    	liquidityWallet = owner();
    	marketingWallet = address (0xFba1D5ADe6a6b1075Eb48Ec17509D2F0856474Ee);
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TKR: Account is already the value of 'excluded'");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TKR: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "TKR: The liquidity wallet is already this address");
    ...
    function setMaxWalletToken(uint256 value) external onlyOwner{
        require(value >= 20000000 * (10**18), "TKR: Minimum max wallet limit is 2 percent");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TKR: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TKR: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "TKR: This account cannot send tokens until trading is enabled");
        }
   ...
   transferToAddressETH(payable(marketingWallet),newBNBBalance);
...
contract TKRDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() public DividendPayingToken("TKR_Dividend_Tracker", "TKR_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 50000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "TKR_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "TKR_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TKR contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TKR_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TKR_Dividend_Tracker: Cannot update claimWait to same value");
