...
contract PrivateSale is Context, Ownable {
    ...
    function addWhitelistGroup(address[] calldata _investors) external onlyAdmin {
    ...
    function removeWhitelistGroup(address[] calldata _investors) external onlyAdmin {
    require(_investors.length > 0);
    for(uint index = 0; index <= _investors.length; index++) {
        address currentInvestor = _investors[index];
    ...
    function buyCurrentSale(uint _purchaseAmount) external onlyInvestors() icoActive() {
        ...
        if(investorPurchased[tokenForSale][sender].isInvestor){
            ...
            investorPurchased[tokenForSale][sender] = Sale(sender,tokenForSaleDecimalCount,tokenForSale,tokenForContribution,_amountPurchased,0,0,vestTime,vestPercentage,firstVest,startTime,endTime,true);
