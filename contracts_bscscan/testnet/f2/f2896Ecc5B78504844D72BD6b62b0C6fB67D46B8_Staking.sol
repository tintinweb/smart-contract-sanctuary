...
contract Staking is Ownable{
    address public funiTok; // swap token
    ...
    function safeTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 funiBal = IBEP20(funiTok).balanceOf(address(this));
        if (_amount > funiBal) {
            IBEP20(funiTok).transfer(_to, funiBal);
        } else {
            IBEP20(funiTok).transfer(_to, _amount);
        }
    }
}