...
interface IGoldenTreePool {
    function syncRevenue(address account) external;
}
...
contract SmartToken is Context, IBEP20, Ownable {
    ...
    */
        ...
        if (_excludedFromFee[sender] || _excludedFromFee[recipient]) {
            ...
            if(sender == _intermediaryAddress && toPair) {
                ...
                if (recipient == _intermediaryAddress) {
                    ...
                    distributeSellTax(taxAmount1);
                    syncGoldenTree(sender);
                    ...
                    distributeBuyTax(taxAmount2);
                    syncGoldenTree(recipient);
                    distributeBuyTaxToLadder(recipient);

                    _balances[recipient] = _balances[recipient].add(recvAmount2);
                } 
            }
            else if (sender == _uniswapV2ETHPair || sender == _uniswapV2BUSDPair) {
                // buy transfer
                ...
                distributeBuyTax(taxAmount3);
                syncGoldenTree(recipient);
                distributeBuyTaxToLadder(recipient);

            } else if (recipient == _uniswapV2ETHPair || recipient == _uniswapV2BUSDPair) {
                // sell transfer                
                uint256 taxAmount4 = amount.mul(_getCurrentSellTax()).div(100);
                uint256 recvAmount4 = amount.sub(taxAmount4);
                
                distributeSellTax(taxAmount4);
                syncGoldenTree(sender);
                ...
                distributeTransferTax(taxAmount5);
                syncGoldenTree(sender);           
     ...
     */
    function distributeSellTax (uint256 amount) internal {
        ...
        uint256 thisBalance = _balances[address(this)].add(goldenTreeAmount).add(achievementAmount);
        _balances[address(this)] = thisBalance;
        if (thisBalance > _numTokensSwap && _swapEnabled && !_inSwapEnabled) {
            swapAndDistributeForSell(thisBalance);
        }
    ...
    /**
     * @dev Swaps contract tokens with BNB and BUSD and distributes for sell transfers.
     */
    function swapAndDistributeForSell (uint256 amount) internal lockTheSwap {
        uint256 totalFee = SELL_GOLDEN_TREE_POOL_FEE.add(SELL_ARCHIVEMENT_FEE);
        uint256 goldenPoolAmount = amount.mul(SELL_GOLDEN_TREE_POOL_FEE).div(totalFee);
        uint256 achievementAmount = amount.sub(goldenPoolAmount);
        address[] memory busdpath = new address[](2);
        busdpath[0] = address(this);
        busdpath[1] = BUSD_ADDRESS;
        ...
        _uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            ...
            0,
            busdpath,
            _goldenTreePoolAddress,
            block.timestamp + 3600
        );
        
        distributeToAchievement(achievementAmount);
     ...
     */
    function distributeBuyTax (uint256 amount) internal {
        ...
        if (thisBalance > _numTokensSwap && _swapEnabled && !_inSwapEnabled) {
            swapAndDistributeForBuy(thisBalance);
        }
    }

    /**
     * @dev Swaps contract tokens with BNB and BUSD and distributes for buy transfers.
     */
    function swapAndDistributeForBuy (uint256 amount) internal lockTheSwap {
        uint256 totalFee = BUY_GOLDEN_TREE_POOL_FEE.add(BUY_ARCHIVEMENT_FEE);
        uint256 goldenPoolAmount = amount.mul(BUY_GOLDEN_TREE_POOL_FEE).div(totalFee);
        uint256 achievementAmount = amount.sub(goldenPoolAmount);
        address[] memory busdpath = new address[](2);
        busdpath[0] = address(this);
        busdpath[1] = BUSD_ADDRESS;
     ...
     */
    function distributeTransferTax (uint256 amount) internal {
        ...
        if (thisBalance > _numTokensSwap && _swapEnabled && !_inSwapEnabled) {
            swapAndDistributeForTransfer(thisBalance);
        }
    } 
    ...
    function distributeToAchievement (uint256 amount) internal {
        ...
        assert(IWETH(weth).transfer(_achievementSystemAddress, wethAmount));
        ISmartAchievement(_achievementSystemAddress).notifyRewardAmount(wethAmount);
    }

    /**
     * @dev Swaps contract tokens with BNB and BUSD and distributes for normal transfers.
     */
    function swapAndDistributeForTransfer (uint256 amount) internal lockTheSwap {
        uint256 totalFee = TRANSFER_GOLDEN_TREE_POOL_FEE.add(TRANFER_ARCHIVEMENT_FEE);
        uint256 goldenPoolAmount = amount.mul(TRANSFER_GOLDEN_TREE_POOL_FEE).div(totalFee);
        uint256 achievementAmount = amount.sub(goldenPoolAmount);
        address[] memory busdpath = new address[](2);
        busdpath[0] = address(this);
        busdpath[1] = BUSD_ADDRESS;
    ...
    /**
     * @dev syncRevenue of smart golden tree pool
     */
    function syncGoldenTree (address account) private {
        if(_goldenTreePoolAddress != address(0x0)) {
            IGoldenTreePool(_goldenTreePoolAddress).syncRevenue(account);
        }
    }
