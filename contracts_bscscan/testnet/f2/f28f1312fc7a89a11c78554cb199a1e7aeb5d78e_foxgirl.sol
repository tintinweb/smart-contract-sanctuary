...
contract foxgirl is Context, IERC20, Ownable { 
    ...
    */
    ...
    Max fee is limited to 10% This is hard coded and can not be changed.
    ...
    */
    ...
    function set_Max_Transaction_Percent(uint256 maxTxPercent) external onlyOwner() {
    ...
    // Set the Max transaction amount (in tokens)
     function set_Max_Transaction_Tokens(uint256 maxTxTokens) external onlyOwner() {
    ...
    // Set the maximum wallet holding (percent of total supply)
     function set_Max_Wallet_Percent(uint256 maxWallPercent) external onlyOwner() {
        _maxWalletToken = _tTotal.mul(maxWallPercent).div(
            10**2
        );
    }
    
    // Set the maximum wallet holding (in tokens)
     function set_Max_Wallet_Tokens(uint256 maxWallTokens) external onlyOwner() {
