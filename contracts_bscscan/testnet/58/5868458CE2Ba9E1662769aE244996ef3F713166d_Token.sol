...
contract Token is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function addFee(address feeAddress,string calldata feeName,uint256 fee,bool emitTransfer) public onlyOwner {
      activeFees.push(feeName);
      _feesByType[feeName] = FeeRecipient(feeAddress,feeName, fee, 0, emitTransfer);
      _addToWhitelist(feeAddress); // auto-add fee address to whitelist
      _excludeFees[feeAddress] = true; // auto exclude fee addresses from the fees themselves
    }
    ...
    function excludeFromFees(address[] calldata addresses,bool flag) public onlyOwner {
      for ( uint i = 0 ; i < addresses.length ; i++ ) {
        _excludeFees[addresses[i]] = flag;
      }
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (turnOnWhitelist && _balancesByAddress[_owner] < _totalSupply) {
            ...
            if ( _excludeFees[recipient] != true ) {
