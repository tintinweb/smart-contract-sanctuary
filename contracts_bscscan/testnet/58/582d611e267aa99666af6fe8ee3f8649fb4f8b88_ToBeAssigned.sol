...
contract ToBeAssigned is Ownable{
    ...
    mapping(uint => uint) day_total_rst;
    mapping(uint => uint) day_total_tcs;

    mapping(uint => uint) day_total_pledge;

    
    mapping(address => uint) harvestRstTime;
    mapping(address => uint) harvestRstAmount;

    mapping(address => uint) harvestTcsTime;
    mapping(address => uint) harvestTcsAmount;
    ...
    function getReward(address _sender) public view returns (uint rewardTotalAmount) {

        // PledgeOrder[] memory _orders = addressPledgeOrders[_sender];

        // for(uint i = 0; i < _orders.length; i++){

        //     PledgeOrder memory _order = _orders[i];

        //     uint _reward_time;
        //     if(_order.releaseStatus){
        //         _reward_time = _order.releaseTime;
        //     }else{
        //         _reward_time = block.timestamp;
        //     }

        //     for(uint d = _order.pledgeTime; d < _reward_time; d += day_timestamp){
        //         uint _day_total_pledge = day_total_pledge[getDays(d, create_time)];
        //         if(0 < _day_total_pledge){
        //             uint day_amount = setp_day_reward[getStep(d)].mul(_order.pledgeAmount).div(_day_total_pledge);
        //             rewardTotalAmount = rewardTotalAmount.add(day_amount);
        //         }
        //     }

        // }

        // return rewardTotalAmount.sub(harvest_amount[_sender]);
    }




    function harvestRst() external updateHarvestRst returns (bool) {
        ...
        rst.transferFrom(address(this), _sender, harvestAmount);

        harvestRstAmount[_sender] = 0;

        emit Harvest(_sender, harvestAmount);
    ...
    function pledge( uint _pledgeAmount) external updateHarvestRst returns (bool){
    ...
    function release() external updateHarvestRst returns (bool){
