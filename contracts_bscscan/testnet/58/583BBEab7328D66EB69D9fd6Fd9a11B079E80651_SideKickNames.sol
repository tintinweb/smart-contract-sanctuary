...
contract SideKickNames {
    ...
    function checkAvailability(string memory name) public view returns (string memory) {
        string memory nameString = StringUtils.upper(name);

        // Validate characters in the name...
        if (_checkCharacters(bytes(name))) {
          //return (nameAddressMap[name] == address(0));
          //this is a test, above is correct code;
          return nameString;
        }
        return "";
    }
