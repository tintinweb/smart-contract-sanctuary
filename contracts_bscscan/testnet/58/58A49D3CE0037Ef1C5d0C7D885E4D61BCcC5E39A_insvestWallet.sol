...
contract insvestWallet is ReentrancyGuard {
                ...
                _agreementrelation[_agreementCount]; 
                      if(i > 0){
                          agreements = string (abi.encodePacked(agreements,','));
                            ...
                            agreements = string (abi.encodePacked(
                            agreements,'{"code":"' ,_agreementrelation[i], '",',
                              '"id":"', id,'",',
                              '"reward":"', reward ,'",',
                              '"time":"' ,time, '",' , 
                              '"minAmount":"' ,minAmount,'"}' ));                 
  ...
  //stake
   ...
   function getWallet(address account)public view returns( string memory wallet, uint256 time ){
              ...
              rewardDate = 0; 
               ...
               for (uint i = 0; i < _userSlot[account]; i++ ){
                   if(_wallet[account][i].claimed == false ){
                       ...
                       wallet = string (abi.encodePacked(
                       wallet,'{"code":"', _wallet[account][i].code,'",',
                              '"cantidad":"', amount ,'",',
                               '"slot":"', slot  ,'",',
                               '"signed":"' , signedDstr,'",',
                              '"rewardD":"' ,rewarDstr,'"}' ));  
  ...
  function viewStcs(string memory _code)public view returns( string memory wallet){
    ...
    function getBalance(address account)public view returns(uint256 balance,uint256 timestamp){
