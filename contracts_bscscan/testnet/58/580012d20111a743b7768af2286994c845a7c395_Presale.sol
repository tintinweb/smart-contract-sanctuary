...
/// @title PorToken Presale Contract
...
contract Presale is Ownable {
    ...
    * @param _closeTime End time for presale EPOC ex: 1609459200 - Friday, 1 January 2021 00:00:00
    */ 
    constructor(
    ...
    function setPresaleToken(IERC20 tokenAddress) external onlyOwner {
        require(block.timestamp < closeTime, "Closed");
        token = tokenAddress;
    }

    function updateCloseTime(uint256 _closeTime) external onlyOwner {
        closeTime = _closeTime;
    }

    function updateOpenTime(uint256 _openTime) external onlyOwner {
        openTime = _openTime;
    }

    function updateTotalTokens(uint256 _totalTokens) external onlyOwner {
