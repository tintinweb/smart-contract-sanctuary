...
contract MFCMembership is BUSDT {
  ...
  event TransferBuybackCredits(uint256 amount);
  ...
  ) BUSDT(busdContractAddress_, treasurers_) {
    _busdtContractAddress = busdtContractAddress_;
    _mfcToken = MFCToken(mfcTokenAddress_);
  }

  function createOriginMember(address account) external onlyPrivilegedAgents {
  ...
  function setMembersActive(address[] calldata _addresses) external onlyPrivilegedAgents {
  ...
  function setMembersInactive(address[] calldata _addresses) external onlyPrivilegedAgents {
    for (uint i = 0; i < _addresses.length; i++) {
      require(_memberExist(_addresses[i]), "Member doesn't exist");
  ...
  function creditBalance() external view returns (uint256) {
    return _memberships[_msgSender()].credits;
  }

  function depositMemberCredits(address[] calldata _addresses, uint256[] calldata amounts) external onlyPrivilegedAgents {
  ...
  function depositBuybackCredits(uint256 amount) external onlyPrivilegedAgents {
    buybackCreditBalance += amount;
  }

  function transferBuybackCredits(uint256 amount) external onlyPrivilegedAgents {
    require(buybackCreditBalance >= amount, "Insufficient buyback balance");
    require(getTreasuryToken().balanceOf(address(this)) >= amount, "Insufficient balance");
    buybackCreditBalance -= amount;
    getTreasuryToken().transfer(_msgSender(), amount);
    emit TransferBuybackCredits(amount);
  }
...
contract MFCToken is MFCBEP20 {
  ...
  event Permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s);
  ...
  function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
    emit Permit(owner, spender, value, deadline, v, r, s);
  }
}
