...
contract Mining {
    ...
    function countFetch(uint256 _type) public view returns (uint256 canNumber) {
        ...
        if(allBlock <= yearBlock) {
            canNumber = allBlock.mul(20).mul(propor).div(100).div(100);
        }else if(allBlock % yearBlock == 0) {
            for(uint256 i = 0; i < allBlock.div(yearBlock); i++) {
                canNumber = canNumber.add(yearBlock.mul(20).mul(propor).div(100).div(100).mul(9**i).div(10**i));
            }
        }else {
            for(uint256 i = 0; i < allBlock.div(yearBlock).add(1); i++) {
                if(i == allBlock.div(yearBlock)) {
                    // 计算剩余的区块可以领取多少
                    canNumber = canNumber.add((allBlock % yearBlock).mul(20).mul(propor).div(100).div(100).mul(9**i).div(10**i));
                }else {
                    canNumber = canNumber.add(yearBlock.mul(20).mul(propor).div(100).div(100).mul(9**i).div(10**i));
                    // canNumber = canNumber + yearBlock * 20 * propor / 100 / 100 * (9 ** i) / (10 ** i);
                }
            }
        }
