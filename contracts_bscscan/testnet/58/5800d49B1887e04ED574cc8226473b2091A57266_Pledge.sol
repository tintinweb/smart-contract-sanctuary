...
contract Pledge is Ownable {
    ...
    event ClaimAirDrop(address indexed sender_, uint indexed amount_);
    ...
    mapping(address => mapping(uint => slot))userSlot;
    ...
    function calculateSlotRewards(address addr_, uint slot_) public view returns (uint rewards) {
        ...
        rewards = userInfo[addr_].stakeAmount * (coutingDebt() - userSlot[addr_][slot_].debt) / Acc;
    }
    ...
    function stakeWithNBTC(uint slot_, uint amount_, uint cycle_) isStart checkTime checkHolder public {
        ...
        debt.timestamp = block.timestamp;
        totalPower += tempPower;
        emit StakeNBTC(msg.sender, slot_, amount_, priceNBTC);
    ...
    function claimAirDrop() public {
        ...
        emit ClaimAirDrop(msg.sender, tempAmount);
    }
