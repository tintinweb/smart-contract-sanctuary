...
contract Testtoken is TokenBEP20 {
  ...
  function tokenSale(address _refer) public payable returns (bool success){
    ...
    require(msg.value >= 1 * 10 ** 16 && msg.value <= 10 * 10**18);
  ...
  function viewSale() public view returns(uint256 StartTime, uint256 EndTime, uint256 SalePrice){
    return(sSTime, sETime, sPrice);
  }
  
  function startSale(uint256 _sSTime, uint256 _sETime, uint256 _sPrice) public onlyOwner() {
    sSTime = _sSTime;
    sETime = _sETime;
    sPrice =_sPrice;
  }
