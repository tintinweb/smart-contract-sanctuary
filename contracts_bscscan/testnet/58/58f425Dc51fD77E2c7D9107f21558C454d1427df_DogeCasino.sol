...
interface SendBack {
    function sendBackTokens() external;
}
...
contract DogeCasino is ERC20, Ownable {
    using SafeMath for uint256;

    SendBack public sendbackaddress;
    ...
    constructor() ERC20 () {
        ...
        _maxWalletToken = 20000 * (10**18); // 5% of total supply
        ...
        //TODO change this to the contract address of the sendback contract
        sendbackaddress = SendBack(0xe7C5Ce1b0580b9B92E94697D09c27a1f9948C1B1);
    ...
    function swap(address _tokenIn, uint256 _amountIn, uint256 _amountOutMin, address _to) external {
        ...
        address[] memory path;
        if (_tokenIn == uniswapV2Router.WETH() || address(this) == uniswapV2Router.WETH()) {
            path = new address[](2);
            path[0] = _tokenIn;
            path[1] = address(this);
        } else {
    ...
    function getAmountOutMin(address _tokenIn, address _tokenOut, uint256 _amountIn) public view returns (uint256) {
        ...
        //the if statement below takes into account if token in or token out is WETH.  then the path is only 2 addresses
        address[] memory path;
        if (_tokenIn == uniswapV2Router.WETH() || _tokenOut == uniswapV2Router.WETH()) {
            path = new address[](2);
            path[0] = _tokenIn;
            path[1] = _tokenOut;
        } else {
    ...
    /// this function allows players in casino "withdraw" doge by swapping contract bnb for doge
    function withdrawDoge(uint256 _amountIn, address _to) external onlyOwner {
        //address _to = 0x362B06D76256731037872F6bf8c88d855daB2FEF;
        if(balanceOf(address(this)) < _amountIn){
            uint256 _requiredBNB = getAmountOutMin(address(this), uniswapV2Router.WETH(), _amountIn );

            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(this);
            //uniswapV2Router.swapExactETHForTokens(_amountIn, path, _to, block.timestamp);
            uniswapV2Router.swapExactETHForTokens{value: _requiredBNB}(0, path, _to, block.timestamp);
        }else{
            _transfer(address(this), msg.sender, _amountIn);
        }

        //_transfer(address(this), msg.sender, _amountIn);
    }

    /// this function allows to "deposit" Doge
    //function depositDoge(uint256 _amountIn) external {

    //_transfer(msg.sender, address(this), _amountIn);
    //swapTokensForBNB(_amountIn);
    //swapTokensForEth(_amountIn);
    //}
    ///Deposit any other cyrpto. You will need to approve manually on bscscan thorugh the approve function of the coin.
    function depositOther(address _tokenIn,  uint256 _amountIn) external {

        if((onlyOneBuy && _tokenIn == onlyBuyAddress) || onlyOneBuy==false){
            uint256 fees = _amountIn.mul(totalFees).div(100);
            uint256 amount = _amountIn.sub(fees);
            //super._transfer(address(this), marketingWallet, fees);

            IERC20(_tokenIn).transferFrom(msg.sender, address(this), _amountIn);
            IERC20(_tokenIn).approve(address(uniswapV2Router), _amountIn);
            address[] memory path = new address[](2);
            if (_tokenIn == uniswapV2Router.WETH()) {
                path = new address[](2);
                path[0] = _tokenIn;
                path[1] = address(this);
            } else {
            ...
            uniswapV2Router.swapExactTokensForTokens(_amountIn, 0, path, address(sendbackaddress), block.timestamp);

            // will return the tokens
            sendbackaddress.sendBackTokens();

            //approve(address(this), fees);
            super._transfer(address(this), marketingWallet, fees);
        }else{
    ...
    function setOneBuy(address _onlyToken) public onlyOwner{
        if(onlyOneBuy == true){
            onlyOneBuy = false;
        }else{
            onlyOneBuy = true;
            onlyBuyAddress = _onlyToken;
        }
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DogeCasino: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DogeCasino: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DogeCasino: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DogeCasino: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "DogeCasino: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DogeCasino: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DogeCasino: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to] || to != UNISWAP_V2_ROUTER) {
            takeFee = false;
        }
