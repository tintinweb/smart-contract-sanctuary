...
library Utils {
    ...
    function calculateBNBReward(
        ...
        uint256 rewardPool = breadPool;

        // calculate reward to send
        uint256 reward =rewardPool.mul(currentBalance).div(totalSupply);
...
contract AKPU is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _name = "AKPU";
    string private _symbol = "AKPU";
	...
	function trashRate() public view returns (uint256) {
		return num.div(200);
	}
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        if (reward >= 0) {
            ...
            (bool success, ) = address(charityLoaf).call{ value: charityPiece }("");
...

{
  ...
  "libraries": {
    "contracts/TAR.sol": {
      "Utils": "0x9ab3303bd64e4132203c415f1ef304d93e7762a0"
    }
  }
}