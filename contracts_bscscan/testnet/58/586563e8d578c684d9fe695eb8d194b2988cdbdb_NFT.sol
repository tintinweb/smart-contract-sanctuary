...
contract NFT is ERC721, Ownable {
    ...
    constructor(uint256 _commission, address _currency)
    ...
    function mint(
        ...
        listedMap[_tokenId] = true; //set auto listing
    ...
    function _validate(uint256 _id, uint256 _amount) view internal {
        bool isItemListed = listedMap[_id];
        require(_exists(_id), "Error, wrong tokenId");
        require(isItemListed, "Item not listed currently");
        require(_amount >= price[_id], "Error, the amount is lower");
        require(msg.sender != ownerOf(_id), "Can not buy what you own");
        require(
            currency.balanceOf(msg.sender) >= _amount,
            "Error, amount insufficien"
        );
    }

    function _trade(uint256 _id, uint256 _amount) internal {
        address _buyer = msg.sender;
        address _owner = ownerOf(_id);

        _transfer(_owner, _buyer, _id);
        uint256 _commissionValue = (_amount * commission) / 10000;
        uint256 _sellerValue = _amount - _commissionValue;

        IBEP20(currency).transferFrom(_buyer, _owner, _sellerValue);
        IBEP20(currency).transferFrom(_buyer, owner(), _commissionValue);
