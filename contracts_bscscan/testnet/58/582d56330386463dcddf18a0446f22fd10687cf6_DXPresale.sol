...
contract DXPresale is Owned {
   ...
   bool public isSuccess = false;
    ...
    constructor(address _token,uint256 _min,uint256 _max,uint256 _rate, uint256 _soft , uint256 _hard,uint256 _pancakeRate,address _router,uint256 _unlockon,uint256 _percent,bool isAuto,uint256 _start,uint256 _end) payable public {
        tokenAddress = _token;
        minEthLimit = _min;
        maxEthLimit = _max;
        ...
        intervalDays = _end.mul(1 days);
        endTime = block.timestamp.add(intervalDays);
        isPresaleOpen = true;
        }else{
        startTime = block.timestamp.add(_start.mul(1 days));
        endTime = startTime.add(_end.add(1 days));
        }
        isautoAdd = isAuto;
