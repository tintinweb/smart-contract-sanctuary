...
contract MainPoolV2 is MainPool {
    using SafeMath for uint256;
    ...
    function addReserveCoupon(string calldata _code, address _tokenAddress, uint256 _amount) public onlyOwner {
        ...
        Coupon storage c = coupons[_code];
   ...
   function transferToPool(
        ...
        if (coupons[_code].discount != 0) {
            require(coupons[_code].expired > block.number, "the coupon is expired");
            ...
            require(coupons[_code].reserve >= discountAmount, "discount amount exceed coupon reserve");
            ...
            _addAmountToPool(_creatorAddress, IDOLAddress, discountAmount);
            _addAmountToPool(_creatorAddress, _tokenAddress, leftoverAmount);
            coupons[_code].reserve = coupons[_code].reserve.sub(discountAmount);
        } else {
