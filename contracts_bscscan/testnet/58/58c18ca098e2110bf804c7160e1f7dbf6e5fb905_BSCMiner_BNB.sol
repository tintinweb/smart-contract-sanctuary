...
/**
 ...
 Referral bonus from 4% -> 6% - for the first day.
...
contract BSCMiner_BNB {
    ...
    uint256 public REFERRAL_PERCENT = 40;
    ...
    function hatchEggs(address ref, bool useCompoundBonus) public {
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                uint256 amount = eggsUsed.mul(REFERRAL_PERCENT).div(PERCENTS_DIVIDER);
                ...
                uint256 amount = eggsUsed.mul(REFERRAL_PERCENT).div(PERCENTS_DIVIDER);
                users[insurance].claimedEggs = users[insurance].claimedEggs.add(amount);
                users[insurance].referralEggRewards = users[insurance].referralEggRewards.add(amount);
                totalRefBonus = totalRefBonus.add(amount);
            }
        }
    ...
    function changeProject(address value) external {
    ...
    function changeMarketing(address value) external {
        require(msg.sender == owner, "Admin use only.");
        marketing = payable(value);
    }

    function changeInsurance(address value) external {
        require(msg.sender == owner, "Admin use only.");
        insurance = payable(value);
    }

    /** setup **/
    function hatchEggs(address _addr, uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value > 0 && value <= 30000); 
