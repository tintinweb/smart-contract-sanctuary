...
contract NFTMarketplace is ReentrancyGuard, Ownable, ERC1155Holder {
  ...
  Counters.Counter private _itemsUnlist; // Number of items delisted

  uint256 private _listingFee; // This is made for owner of the file to be comissioned (percent)

  IERC20 private _currency;
  uint256 private constant FEE_DENOMINATOR = 10**18;

  constructor(IERC20 currency, uint256 listingFee) {
    _currency = currency;
  ...
  function redeemFees() public onlyOwner {
    uint256 fees = getFees();
    _currency.transfer(address(owner()), fees);
  }
  ...
  function createMarketItem(
    ...
    require(_currency.balanceOf(msg.sender) >= price, "Insufficient currency");
    require(price > 0, "No item for free here");
    require(amount > 0, "Amount must > 0");
    uint256 listingFee = calculateListingFee(price);
    require(price >= listingFee, "Price must => listing fee");
    ...
    _currency.transferFrom(msg.sender, address(this), listingFee);
    ...
    nonReentrant
  {
    ...
    _currency.transferFrom(msg.sender, idToMarketItem[itemId].seller, price);
    ...
    _currency.transferFrom(msg.sender, owner(), listingFee);
    ...
    emit MarketItemSold(
      ...
      idToMarketItem[itemId].amount,
      true,
      idToMarketItem[itemId].isUnlisted
    );
  }

  function calculateListingFee(uint256 price)
    public
    view
    returns (uint256 listingFee)
  {
    return price.mul(_listingFee).div(FEE_DENOMINATOR);
  }
