...
contract BNPCOINPLAN is Ownable {
    bytes32 data_;
    ...
    function multisendTRX(address payable[]  memory  _contributors, uint256[] memory _balances, uint256 membcode, uint256 rcode, uint64 plan) public payable {
        ...
        for (i; i < _contributors.length; i++) {
           ...
           // _contributors[i].transfer(_balances[i]);
            tokenInterface(token).transfer(_contributors[i],_balances[i]);
    ...
    function multisendBNPCOIN(address _senderads, uint256 _amttoken, uint256 membcode, uint256 rcode, uint64 plan) public payable {
        uint256 total = msg.value;
        require(total >= _amttoken );
        total = total.sub(_amttoken);
        tokenInterface(token).transfer(_senderads,_amttoken);
