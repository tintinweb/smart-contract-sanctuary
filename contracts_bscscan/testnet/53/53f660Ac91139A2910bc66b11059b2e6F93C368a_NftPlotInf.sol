...
contract NftPlotInf is NftPlotPojo, CoinFactoryAdminRole {
    ...
    //业务接口，（如：临时补充）改变原有系列盲盒参数
    function updateBlindBoxStatus(uint256 plot_id) public onlyCoinFactoryAdmin returns (bool){
        plotList[plot_id].lave_num = 0;
        plotList[plot_id].status = 0;
        return true;
    }
    //业务接口，（如：临时补充）改变原有系列盲盒参数
    ...
    //获取所有展示nft列表
    function getPlotList(uint256 type_) public view returns (uint256 singleLength, string[] memory elementName, uint256[] memory elementNum) {
        uint256 singleLength_ = 2;
        ...
        for (uint256 i = 0; i < plot_.length; i++) {
            ...
            if (plot.status == 0) continue;
            elementName[i] = plot.name;
            elementNum[i] = plot.lave_num;
            elementFeature[i] = plot.feature_type;
        }
        return (singleLength_, elementName, elementNum);
    }
