...
contract Sale is Ownable{
    ...
    function _acceptBId(address token,address from, address admin,  uint tokenprice, uint256 tokenId) internal{
        ...
        require(balances[tokenId][msg.sender] > 0, "Is Not a Owner");
        (uint256 _adminfee, uint256 netamount, uint256 royaltyamount) = calc(tokenprice, totalroyalty, serviceValue);
        BEP20 t = BEP20(token);
    ...
    function calc(uint256 tokenprice, uint256 royal, uint256 _serviceValue) internal pure returns(uint256, uint256, uint256){
