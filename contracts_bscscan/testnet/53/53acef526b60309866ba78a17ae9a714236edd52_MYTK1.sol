...
contract MYTK1 is IERC20{
	...
	*/
	...
	NGO[] internal ngos;
	mapping (address => uint256) private _ngoid;
	...
	*/
	constructor(){
		...
		/*Set start and end date for Purchase bonus*/
		startdate = block.timestamp;
		enddate = (startdate + (_icoperiod));
	...
	*/
	function _burn(address account, uint256 amount) internal {
		...
		if( (_totalSupply - amount) < _minSupply ){
			if( _totalSupply > _minSupply){
				amount = _totalSupply - _minSupply;
			}else{
				amount = 0;
			}
		}
	...
	*/
	function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		*/
		...
		// Decrease totalSupply
		_totalSupply = _totalSupply - burnAmt;
	...
	*/
	function stake(uint256 _amount) public {
		...
		require(_totalSupply < _SupplyLimit, "Max Supply level reached.");
	...
	*/
	...
	function setdevFundAaddress(address newDF) public {
		require(msg.sender == _owner, "Only for owner");
		_devFund = newDF;
	}
	
	/*Change Charity Fund Address */
	function setCharityFundAaddress(address newCF) public {
		require(msg.sender == _owner, "Only for owner");
		_charityFund = newCF;
	}
	
	/* SEt Token Maanager Contract Address */
	function setManagerAaddress(address newDEX) public {
		require(msg.sender == _owner, "Only for owner");
	...
	/* Charity Manage */
	function addngo(string memory ngoname, string memory location, string memory website, address daddr) public{
		require(msg.sender == _owner, "Only for owner");
		ngos.push();
		uint256 ngoIndex = ngos.length - 1;
		ngos[ngoIndex].name = ngoname;
		ngos[ngoIndex].location = location;
        ngos[ngoIndex].website  = website;
		ngos[ngoIndex].donationaddress = daddr;
		ngos[ngoIndex].isActive = true;
		_ngoid[daddr] = ngoIndex;
	}
	
    function getNGO(address addr) public view returns (NGO memory){
		require( _ngoid[addr] > 0, "That id doesnot exist" );
        return ngos[_ngoid[addr]];
    }
	
	function getNGOid(address addr) public view returns (uint256){
        return _ngoid[addr];
    }
	
    function getNGOlist() public view returns (NGO[] memory){
        return ngos;
    }
	
	function changeNgoStatus(address addr, bool status) public{
		require(msg.sender == _owner, "Only for owner");
		require( _ngoid[addr] > 0, "That id doesnot exist" );
		ngos[_ngoid[addr]].isActive = status;
	}
	
	function distributeCharityStakeRewards() public{
		require(msg.sender == _owner, "Only for owner");
		...
		if(reward > 1000){
			...
			uint256 ct = ngos.length - 1;
			uint256 amt = 0;
			if(ct>0){
				uint256 amountPerNgo = reward / ct;
				for(uint256 i=1;i<ct;i++){
					_balances[_charityFund] -= amountPerNgo;
					_balances[ngos[i].donationaddress] += amountPerNgo;
					ngos[i].totalDonation += amountPerNgo;
					amt += amountPerNgo;
				}
