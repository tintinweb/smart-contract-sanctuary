    ...
    PausableUpgradeable
{
    ...
    function linearSetEarlyWithdrawTier(
        uint256 _poolId,
        uint16[] calldata _percentageKey,
        uint16[] calldata _percentageValue
    ) external onlyOwner linearValidatePoolById(_poolId) {
        uint256 length = _percentageKey.length;
        require(
            length == _percentageValue.length,
        ...
        for (uint256 i = 0; i < length; i++) {
            pool.penaltyRate[_percentageKey[i]] = _percentageValue[i];
            emit LinearSetEarlyWithdrawTier(
                _poolId,
                _percentageKey[i],
                _percentageValue[i]
    ...
    function linearAddWhitelist(uint256 _poolId, address[] calldata _account)
        external
        onlyOwner
        linearValidatePoolById(_poolId)
    {
        uint256 length = _account.length;
        AdditionalPoolInfo storage pool = additionalPoolInfo[_poolId];

        for (uint256 i = 0; i < length; i++) {
            if (!pool.isWhitelisted[_account[i]]) {
                pool.isWhitelisted[_account[i]] = true;
            }
            emit LinearAddWhitelist(_poolId, _account[i]);
        }
    }
        ...
        linearValidatePoolById(_poolId)
    {
        ...
        // emit event
        emit LinearEarlyWithdraw(_poolId, account, amount, reward);
