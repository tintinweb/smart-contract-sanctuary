...
contract VelhallaStaking is Ownable, Pausable, Haltable {
    ...
    // SCAR tokens reward per block. 20000000*1e18 * 3 / (240*24*60*60), about 2.893 SCAR per block. total 20000000 SCAR token for 240 days, where 3 second/block
    uint256 private scarPerBlock = 2893518518518518518;
    ...
    function emergencyWithdraw() external whenNotPaused{
        require(!emergencyWithdrawLock, "The emergency withdraw is locked");
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[0][msg.sender];
