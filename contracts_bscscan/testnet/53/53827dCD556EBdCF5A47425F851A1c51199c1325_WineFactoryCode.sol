...
import "./vendors/access/InheritedOwner.sol";
    ...
    IWineFactory
{
    ...
    address public override winePoolCode;
        ...
        initializer
    {
        ...
        _initializeInheritedOwner(manager_);
        _initializeReentrancyGuard();
        proxyAdmin = proxyAdmin_;
        winePoolCode = winePoolCode_;
        baseUri = baseUri_;
    ...
    function getPool(uint256 poolId)
        ...
        returns (address)
    {
        require(pools[poolId] != address(0), 'getPool - pool is deleted');
        return pools[poolId];
    }
    ...
    function editWinePoolCode(address winePoolCode_)
        public
    ...
    function createWinePool(
        ...
        returns (address winePoolAddress)
    {
        require(!name_.isEmpty(), "createWinePool: name is empty!");

        uint256 poolId = pools.length;

        winePoolAddress = address(new TransparentUpgradeableProxyInitializable());
        ITransparentUpgradeableProxyInitializable(winePoolAddress).initializeProxy(winePoolCode, proxyAdmin, bytes(""));

        require (
            IWinePool(winePoolAddress).initialize(
                name_,
                string(abi.encodePacked(baseSymbol, poolId.toString())),
            ...
            "initializeWinePool: Failed"
        );
        pools.push(winePoolAddress);
    ...
    function disablePool(uint256 poolId)
        ...
        onlyManager
    {
        IWinePool pool = IWinePool(getPool(poolId));
        require(pool.disabled() == false, 'disablePool - pool is disabled');
        pool.disablePool();
        if (pool.getMaxTotalSupply() == 0) {
            pools[poolId] = address(0);
        }
    }
}
