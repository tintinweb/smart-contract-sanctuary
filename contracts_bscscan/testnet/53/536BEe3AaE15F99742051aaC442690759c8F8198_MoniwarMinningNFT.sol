...
contract MoniwarMinningNFT is Ownable, ReentrancyGuard {
    ...
    function getUserInfo(uint256 pid, uint256 numberUid, address _userAddress) public view returns (ListNFT[] memory)
    {
        ...
        for (uint256 index = 0; index <= numberUid; index++) {
            if(listNFT[pid][_userAddress][index].tokenId == 0){
                list[index].tokenId = 0;
                list[index].amountCalHashrate = 0;
                list[index].amountDeposit = 0;
                list[index].hashRate = 0;
                list[index].uid = numberUid;
            } else{
