...
contract Test is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 21000000000 * 10 ** uint256(_decimals);
    ...
    uint256[2] public limit = [100 /*buy*/, 10 /*sell*/];
    uint256 public constant DENOMINATOR = 10000;

    address public marketingWallet = 0x81C8BD2949AF61626c9777b4A7CD5C88E6DA4DBe;
    ...
    function calculateMarketingFee(uint256 _amount) private view returns (uint256) {
        return _amount * _marketFee / DENOMINATOR;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 half = contractTokenBalance / 3;
        half += half;
        ...
        emit SwapAndLiquify(contractTokenBalance / 3, newBalance, otherHalf);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        require((!blackList[sender]) && (!blackList[recipient]), "blacklisted account");
        ...
        bool isSell = (sender == uniswapV2Pair);

        if(!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]){
            if(!isSell)
                require(amount <= totalSupply() * limit[1] / DENOMINATOR, "exceed sell transfer limit");
            else
                require(amount <= totalSupply() * limit[0] / DENOMINATOR, "exceed buy transfer limit");
        }
    ...
    function setLimit(uint8 index, uint value) public onlyOwner {
        require((index == 0) || (index == 1), "invalid index");
        require((value > 0) && (value < DENOMINATOR), "value should be greater than 0 and less than DENOMINATOR");
        limit[index] = value;
    }

    function setBlackList( address acc, bool stats) public onlyOwner {
        blackList[acc] = stats;
    }
