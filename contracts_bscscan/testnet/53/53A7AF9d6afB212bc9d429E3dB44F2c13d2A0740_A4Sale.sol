...
contract A4Sale is Ownable  {
    ...
    uint256 public _currencyDecimal = 1e6;
    uint256 public _minTotal = 1000 * 1e6;
    ...
    event CurrencyAdded(address, uint256);
    event CurrencyUpdated(address, uint256, uint256);
    ...
    {
        require(_releaseTime > block.timestamp, "ICO is stopped");
    ...
    {
        require(_releaseTime <= block.timestamp, "ICO is active");
        require(_deposits[msg.sender] > 0, "Zero tokens");

        uint256 tokens = _deposits[msg.sender].mul(_rate);
