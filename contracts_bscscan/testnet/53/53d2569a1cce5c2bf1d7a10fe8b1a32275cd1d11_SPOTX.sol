...
contract SPOTXDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("SPOTX_Dividend_Tracker", "SPOTX_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SPOTX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SPOTX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SPOTX contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SPOTX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "SPOTX_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SPOTX is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    SPOTXDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 2_000_000_0 * (10**9);
    uint256 public swapTokensAtAmount = 2_000_00 * (10**9);
    uint256 public maxLimit = 4_000_000_0 * (10**9);
    ...
    constructor() ERC20("SpotX", "SPOTX") 
    {
        ...
        marketingWallet = payable(0xbd782B5c984482f3D50905D005a098fA536dfc7b);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new SPOTXDividendTracker();
        ...
        _isExcludedFromWhale[_uniswapV2Pair]=true;

        /*
            _mint is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _mint(owner(), 2_000_000_000 * (10**9));
    }
    ...
    function setAllFee(uint256 _rewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _burnFee) 
    public onlyOwner
    {
        BNBRewardsFee = _rewardFee;
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        burnFee = _burnFee;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SPOTX: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SPOTX: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SPOTX: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SPOTX: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SPOTX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SPOTX: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
       ...
       { 
           ...
           uint256 newBalance = balanceOf(to).add(amount).sub(fee);
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            super._transfer(from, deadWallet, burnAmount); //burn to dead wallet. 
            if(extraFee>0)
            {
