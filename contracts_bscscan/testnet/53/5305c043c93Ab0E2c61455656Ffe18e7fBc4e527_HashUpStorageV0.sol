...
contract HashUpStorageV0 {
    ...
    /**
     *  Token yields approved if >= 5 approvals fired
     */
    mapping(address => uint8) public tokenApproveCount;
    ...
    function getTokenApproveCount(address tokenAddress)
    public
    view
    returns (uint8)
    {
        return tokenApproveCount[tokenAddress];
    }
    ...
    returns (address)
    {
        tokenApproveCount[approvedContract_] += 1;
        if (tokenApproveCount[approvedContract_] >= 5) {
            _approveContract(approvedContract_);
        }
    }
