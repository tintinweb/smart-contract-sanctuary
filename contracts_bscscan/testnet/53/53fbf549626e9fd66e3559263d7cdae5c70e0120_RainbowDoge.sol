...
// RainbowDoge proposes an innovative feature in its contract.

// Hold $RADO and get rewarded in $RAINBOW on every transaction!
...
//      2% Liquidity
//      2% Marketing
// 📱 Telegram: https://t.me/Rainbow_Doge
// 🌎 Website: https://www.rainbowdoge.net
// 🌐 Twitter: https://twitter.com/rainb0wdoge
...
contract RAINBOWDistributor is IRAINBOWDistributor {
    
    using SafeMath for uint256;
    using Address for address;
    // $RAINBOW Contract
    address _token;
    // Share of the $RAINBOW Pie
    struct Share {
        uint256 amount;
        uint256 totalExcluded;
        uint256 totalRealised;
    }
    // $RAINBOW Contract Address
    address RAINBOW = 0x673Da443da2f6aE7c5c660A9F0D3DD24d1643D36;
    ...
    // 10 Million $RAINBOW Minimum Distribution
    uint256 public minDistribution = 10_000_000 * (10 ** 9);
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = IERC20(RAINBOW).balanceOf(address(this));
        ...
        uint256 amount = IERC20(RAINBOW).balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            IERC20(RAINBOW).transfer(shareholder, amount);
    ...
    function updatePancakeRouterAddress(address pcs) external override onlyToken {
        router = IUniswapV2Router02(pcs);
    }
}

// The official $RADO smart contract
contract RainbowDoge is IERC20, Context, Ownable {
    using SafeMath for uint256;
    using SafeMath for uint8;
    using Address for address;
    
    // Our $RAINBOW Distributor 
    RAINBOWDistributor rainbowDistributor;
    
    // General Info
    string constant _name = "RainbowDoge";
    string constant _symbol = "RADO";
    uint8 constant _decimals = 9;
    
    // Supply
    uint256 _totalSupply = 777_777_777_777_777 * (10 ** _decimals);
    
    // Token Limits
    uint256 public _maxTxAmount = _totalSupply.div(1000); // 0.1% or 777_777_777_777 tokens
    
    // Addresses
    address public RAINBOW  = 0x673Da443da2f6aE7c5c660A9F0D3DD24d1643D36; //$RAINBOW
    ...
    uint256 public swapThreshold = _totalSupply.div(1000); // 0.1% or 777_777_777_777 Tokens
    bool    inSwap;
    ...
    uint256 public totalBNB_RAINBOW_Reflections     = 0;
    ...
    constructor () {
        ...
        // Liquidity Pool Address for BNB -> $RADO
        ...
        // Our $RAINBOW Distributor
        rainbowDistributor = new RAINBOWDistributor(_dexRouter);
        ...
        // Exempt important Addresses from Token Limit
        isTxLimitExempt[msg.sender] = true;
        isTxLimitExempt[marketingFeeReceiver] = true;
        isTxLimitExempt[address(rainbowDistributor)] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(shouldSwapBack()) { 
            ...
            try rainbowDistributor.process(gasToUse) {} catch {}
        }
    ...
    function handleTransferBody(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(!isDividendExempt[sender]){
             rainbowDistributor.setShare(sender, _balances[sender]); 
        }
        if(!isDividendExempt[recipient]){
             rainbowDistributor.setShare(recipient, _balances[recipient]);
        }
    ...
    // True if we should swap from RainbowDoge => BNB
    ...
    // Swaps RainbowDoge for BNB if Threshold is reached and the swap is enabled
    //  Uses BNB retrieved to:
    //  fuel the Contract for buy/burns
    //  provide Distributor with BNB for $RAINBOW
    ...
    //  add liquidity if liquidity is low
     function swapBack() internal swapping {
        ...
        uint256 amountBNB_RAINBOW = amountBNB.mul(reflectionFee).div(totalBNBFee);
        uint256 amountBNBMarketing = amountBNB.mul(marketingFee).div(totalBNBFee);
        
        // deposit BNB for reflections and marketing
        transferToDistributorAndMarketing(amountBNB_RAINBOW, amountBNBMarketing);
    ...
    function transferToDistributorAndMarketing(uint256 RAINBOWdistributorBNB, uint256 marketingBNB) internal {
        (bool successOne,) = payable(address(rainbowDistributor)).call{value: RAINBOWdistributorBNB, gas: 30000}("");
        if (successOne) {
            try rainbowDistributor.deposit() {totalBNB_RAINBOW_Reflections = totalBNB_RAINBOW_Reflections.add(RAINBOWdistributorBNB);} catch {}
        }
    ...
    // Should RainbowDoge buy/burn right now?
    ...
    // Buys $RADO with BNB in the Contract, sending to target Address
    ...
    function manuallyProcessDividends(uint256 distributorGasFee) public {
        if (distributorGasFee == 0) {
            try rainbowDistributor.processManually() {} catch {}
        } else if (distributorGasFee == 1) {
            try rainbowDistributor.process(distributorGas) {} catch {}
        } else {
            try rainbowDistributor.process(distributorGasFee) {} catch {}
        }
    }
    // Sets Various Fees
    ...
    function setIsDividendExempt(address holder, bool exempt) external onlyOwner {
        ...
        if(exempt) {
            rainbowDistributor.setShare(holder, 0);
        } else {
            rainbowDistributor.setShare(holder, _balances[holder]);
        }
    }
    // Is Holder Exempt From Fees
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amount, bool shouldAutomateLiquidity) external onlyOwner {
    ...
    // Set Criteria for $RAINBOW Distributor
    function setRAINBOWDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external onlyOwner {
        rainbowDistributor.setDistributionCriteria(_minPeriod, _minDistribution);
    }
    ...
    // Buy and Burn $RADO with BNB stored in Contract
    function triggerRAINBOWDOGEBuyback(uint256 amount) public onlyOwner {
        buyTokens(amount, BURN);
        emit RAINBOWDOGEBuyBackAndBurn(amount);
    }
    ...
    // Returns the Circulating Supply of $RADO ( supply not owned by Burn Wallet )
    ...
    function getRAINBOWDistributorAddress() external view returns (address) {
        return address(rainbowDistributor);
    }
    event AutoLiquify(uint256 amountBNB, uint256 amountBOG);
    event RAINBOWDOGEBuyBackAndBurn(uint256 amountBNB);
}