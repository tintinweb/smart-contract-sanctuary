...
// File: contracts/IFO_VESTING/CafeSwapICO_Heron.sol
...
contract CafeSwapICO_Heron is ReentrancyGuard{
    using SafeBEP20 for IBEP20;

    uint256 public constant HARVEST_PERIODS = 13;
    ...
    event SetClaimStatus(address caller, bool newStatus);
    ...
    constructor(
        ...
        for (uint256 i = 0; i < HARVEST_PERIODS; i++) {
            ...
            if (i == 0){
               ...
               harvestReleaseBlocks[i] = endBlock + (_vestingBlockOffset * (i+1)); 
            }
    ...
    modifier isClaimEnabled() {
        require(claim == true, "claim is not enable");
        _;
    }
    ...
    function harvest(uint256 harvestPeriod) external nonReentrant isClaimEnabled {
    ...
    function getOfferingAmountPerPeriod(address _user, uint256 _harvestPeriod)
        public
        view
        returns (uint256)
    {
        uint256 _amount = (getOfferingAmount(_user) * 10) /100; // initial claim amount
        if(_harvestPeriod == 0){
          return _amount;
        }
        return (getOfferingAmount(_user) - _amount) / 12; //remaing unlock monthly(12 months)
    }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < HARVEST_PERIODS; i++) {
            uint256 userOfferingPerPeriod = getOfferingAmountPerPeriod(_user, i);
            if (
    ...
    function setClaimStatus(bool _claim) public onlyAdmin {
        require(claim != _claim, "can't update the same status of claim");
        claim = _claim;
        emit SetClaimStatus(msg.sender, _claim);
    }
}