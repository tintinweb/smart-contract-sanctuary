...
contract ZWZZWZPool is
    Ownable,
    ReentrancyGuard,
    ERC20("ZWZ ZWZ Pool", "ZWZ-ZWZ")
{
    using SafeMath for uint256;

    // The address of the smart chef factory
    address public ZWZ_CASTLE_FACTORY;
    ...
    function initialize(
        ...
        require(msg.sender == ZWZ_CASTLE_FACTORY, "Not factory");
        require(
        ...
        returns (uint256[] memory, ERC20[] memory)
    {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                zwzReward = multiplier.mul(rewardPerBlock[rewardTokens[i]]);
                adjustedTokenPerShare = accTokenPerShare[rewardTokens[i]].add(
                    zwzReward.mul(PRECISION_FACTOR[rewardTokens[i]]).div(
                        stakedTokenSupply
                    )
                );
        ...
        returns (uint256)
    {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
            uint256 zwzReward = multiplier.mul(rewardPerBlock[_token]);
            uint256 adjustedTokenPerShare = accTokenPerShare[_token].add(
                zwzReward.mul(PRECISION_FACTOR[_token]).div(
                    stakedTokenSupply
                )
            );
