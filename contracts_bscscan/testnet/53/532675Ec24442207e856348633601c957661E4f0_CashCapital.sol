...
contract CashCapital {
     ...
     struct X6 
    {
        address currentReferrer;
        address[] referrals;
        address[] secondReferrals1;
        address[] secondReferrals2;
        bool blocked;
        uint reinvestCount;
    ...
    function updateBoard(address _user) private 
    {
            ...
            else
            {
                ...
                users[freeX3Referrer].x6Matrix[1].referrals.push(_user);
                if(users[topReferrer].x6Matrix[1].referrals[0]==freeX3Referrer && users[topReferrer].x6Matrix[1].referrals[0]!=users[topReferrer].x6Matrix[1].referrals[1])
                {
                    users[topReferrer].x6Matrix[1].secondReferrals1.push(_user);
                    if(users[topReferrer].x6Matrix[1].secondReferrals1.length==2) 
                    {
                ...
                {
                    users[topReferrer].x6Matrix[1].secondReferrals2.push(_user);
                 
            
                    if(users[topReferrer].x6Matrix[1].secondReferrals2.length==2) 
                    {
                        BUSD.transfer(topReferrer,orbitIncome);  
                    }
                }
                ...
                uint8 _length=uint8(users[topReferrer].x6Matrix[1].secondReferrals1.length+users[topReferrer].x6Matrix[1].secondReferrals2.length);
                ...
                if(_length==4)
                {
                    ...
                    users[topReferrer].x6Matrix[1].secondReferrals1 = new address[](0);  
                    users[topReferrer].x6Matrix[1].secondReferrals2 = new address[](0);  
    ...
    function usersX3Matrix(address userAddress, uint8 level) public view returns(address, address[] memory, bool) {
     ...
     function usersX6Matrix(address userAddress, uint8 level) public view returns(address, address[] memory,address[] memory,address[] memory, bool,uint256) {
        return (users[userAddress].x6Matrix[level].currentReferrer,
                users[userAddress].x6Matrix[level].referrals,
                users[userAddress].x6Matrix[level].secondReferrals1,
                users[userAddress].x6Matrix[level].secondReferrals2,
                users[userAddress].x6Matrix[level].isUplineBoardBreak,
                users[userAddress].x6Matrix[level].reinvestCount);
    }
    
      function getUserBoard(address userAddress, uint8 level) public view returns(uint256, uint256[] memory,uint256[] memory,uint256[] memory, bool) {
       
        if(users[userAddress].x6Matrix[level].referrals.length<2 && userAddress!=owner)
        userAddress=users[userAddress].x6Matrix[level].currentReferrer;
       
        uint256[] memory firstLevel = new uint256[](2);
        uint256[] memory secondLevel1 = new uint256[](2);
        uint256[] memory secondLevel2 = new uint256[](2);
       
        for(uint8 i=0;i<2;i++)
        {
            if(i<users[userAddress].x6Matrix[level].referrals.length)
        ...
        for(uint8 i=0;i<2;i++)
        {
           if(i<users[userAddress].x6Matrix[level].secondReferrals1.length) 
           secondLevel1[i]=users[users[userAddress].x6Matrix[level].secondReferrals1[i]].id;  
           else
           secondLevel1[i]=0;
        }
        ...
        for(uint8 i=0;i<2;i++)
        {
            if(i<users[userAddress].x6Matrix[level].secondReferrals2.length) 
            secondLevel2[i]=users[users[userAddress].x6Matrix[level].secondReferrals2[i]].id;
            else
            secondLevel2[i]=0;
        }
