...
contract BABYNEW is ERC20, Ownable {
    ...
    address public marketingWallet = address(0x0fAb08BECA2b6fe36BCA6694A2F84E3FFE43c260);

    address public rewardTokenForMarketing = address(0x36d468D6055ac88d2D08793B4816B67AB49a0339); // Default reward token - BUSD // main net 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56 //testnet 0x36d468D6055ac88d2D08793B4816B67AB49a0339;
    ...
    function setFeeStructure(address from, address to) internal{
        ...
        emit SetFeeStructure(RewardsFee, liquidityFee, marketingFee, buybackFee, totalFees);
    }
    ...
    function swapTokensAndSendToMarketing(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = rewardTokenForMarketing;
        // make the swap
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
    ...
    function setRewardToken(address tokenAddress) external{
        ...
        dividendTracker.setRewardToken(msg.sender, tokenAddress);
    }
    ...
    function setRewardTokenForMarketing(address tokenAddress) external{
        require(address(this) != tokenAddress, "BABYNEW can't be set as a reward token.");
        require(uniswapFactory.getPair(tokenAddress, WBNB) != address(0), "Please input the address of token that has listed on PancakeSwap");
        rewardTokenForMarketing = tokenAddress;
    }

    function getRewardTokenInfo(address account) external view returns (address) {
...
contract BABYNEWDividendTracker is Ownable, DividendPayingToken {
    ...
    function processAccount(address payable account, bool automatic) public onlyOwner returns (bool) {
        ...
        if(amount > 0) {
            ...
            if (tokenHoldersMap.rewardToken[account] != address(0)) {
                rewardToken = tokenHoldersMap.rewardToken[account];
            } else {
    ...
    function setRewardToken(address account, address token) public onlyOwner{
        require(tokenHoldersMap.getIndexOfKey(account) >= 0, "You must hold BABYNEW tokens to set up reward tokens.");
        tokenHoldersMap.rewardToken[account] = token;
    }
    
    function getRewardTokenInfo(address account) public view returns (address) {
        return tokenHoldersMap.rewardToken[account];
    }
