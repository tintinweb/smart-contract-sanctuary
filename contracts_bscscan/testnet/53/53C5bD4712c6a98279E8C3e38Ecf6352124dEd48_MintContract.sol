...
contract MintContract is Ownable {
    ...
    function setLimit(uint256 _digits) public onlyOwner {
        require(_digits > 0, "MintContract: Digits should over 0");

        limit = 10**_digits;
    }

    function generateReferralId(address _owner) public view returns (uint256) {
        return
            limit +
            (uint256(
                keccak256(
    ...
    function mint() public {
        ...
        if (referralIds[msg.sender] == 0) {
            uint256 referralId = generateReferralId(msg.sender);
            while (referees[referralId] != address(0)) {
                referralId = generateReferralId(msg.sender);
            }

            referralIds[msg.sender] = referralId;
            referees[referralId] = msg.sender;
        }
    }
