...
// File: contracts/BMTDividendPayingToken.sol
...
contract BMT is ERC20, Ownable {
    ...
    BMTDividendTracker public dividendTracker;
    ...
    function updateCharityAddress(address payable newAddress)
        external
        onlyOwner
    {
    ...
    function updateCharityTokenFeeAddress(address newAddress)
        external
        onlyOwner
    {
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(
            ...
            "BMT: The dividend tracker already has that address"
        );

        BMTDividendTracker newDividendTracker = BMTDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "BMT: The new dividend tracker must be owned by the BMT token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(
            ...
            "BMT: The router already has that address"
        );
        uniswapV2Router = IUniswapV2Router02(newAddress);
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "BMT: Account is already the value of 'excluded'");
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "BMT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "BMT: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "BMT: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "BMT: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            require(
                ...
                "BMT: This account cannot send tokens until trading is enabled"
            );
        }
    ...
    function swapAndLiquify(uint256 tokens) internal {
        ...
        //     // swapEthForTokens(burnFeeBalance, charityTokenAddressForFee, charityAddress);
        //     //_transfer(address(this), burnAddress, burnFeePortion);
        //     //emit Transfer(address(this), burnAddress, burnFeePortion);
        // }else{
        //     (bool sent,) = charityAddress.call{value: burnFeeBalance}("");
        //     if(sent){
        //         //_transfer(address(this), burnAddress, burnFeePortion);
        //         //emit Transfer(address(this), burnAddress, burnFeePortion);
        //     } else {
        //         addLiquidity(burnFeePortion, burnFeeBalance);
        //     }
        // }
...
contract BMTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("BMT_Dividend_Tracker", "BMT_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "BMT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BMT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BMT contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "BMT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "BMT_Dividend_Tracker: Cannot update claimWait to same value"
        );
