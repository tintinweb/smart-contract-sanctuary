...
contract MultiSigEIP712 is EIP712Upgradeable, OwnableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
   ...
   * @param _validators validators count
   * @param _weights weights count
   */
  function setValidatorsRequired(uint256 _validators, uint256 _weights) external onlyOwner {
    validatorsRequired = _validators;
    weightsRequired = _weights;
  }

  function setValidator(address _account, uint256 _weight) external onlyOwner {
    grantRole(VALIDATOR_ROLE, _account);
    validatorWeight[_account] = _weight;
  }

  /**
   * @dev Deposit token
   * @param _sender deposit sender
   * @param _depositId deposit id request
  ...
  function depositToken(
    ...
    if (_payMode == 1) {
      require(hasRole(PAYER_ROLE, msg.sender), "Invalid payer role");
    }
    if (_payMode == 2) {
      require(_sender == msg.sender, "Invalid sender");
    }
    require(depositEntries[_depositId].amount == 0, "Invalid id");

    authenticate(_sender, _depositId, _token, _amount, _signatures);
    depositEntries[_depositId] = ProcessedEntry(_depositId, _sender, _token, _amount);

    require(IERC20Upgradeable(_token).transferFrom(_sender, address(this), _amount), "Transfer from failed");
    // Log and Event
    emit TokenDeposited(_sender, _depositId, _token, _amount);
  }

  /**
   * @dev Withdraw token
   * @param _receiver withdraw receiver
  ...
  function withdrawToken(
    ...
    if (_payMode == 2) {
      require(_receiver == msg.sender, "Invalid sender");
    }
    require(withdrawalEntries[_withdrawId].amount == 0, "Invalid id");
    // Authentication
    authenticate(_receiver, _withdrawId, _token, _amount, _signatures);

    withdrawalEntries[_withdrawId] = ProcessedEntry(_withdrawId, _receiver, _token, _amount);
    ...
    require(IERC20Upgradeable(_token).transfer(_receiver, _amount), "Transfer failed");
    // Log and Event
    emit TokenWithdrawn(_receiver, _withdrawId, _token, _amount);
  }
  ...
  function authenticate(
    ...
    uint256 validWeight_ = 0;
    address lastSigner_ = address(0);
    for (uint256 i = 0; i < _signatures.length; i++) {
      address signer_ = validateSignature(_user, _withdrawId, _token, _amount, _signatures[i]);
      require(hasRole(VALIDATOR_ROLE, signer_), "Wrong role");
      require(lastSigner_ != signer_, "Duplicate signer");
      lastSigner_ = signer_;
      validValidator_++;
      validWeight_ = validWeight_ + validatorWeight[signer_];
    }
    require(validValidator_ >= validatorsRequired, "Insufficient validators");
    require(validWeight_ >= weightsRequired, "Insufficient weights");
  }
  ...
  function validateSignature(
    ...
    bytes32 digest_ = _hash(_user, _withdrawId, _token, _amount);
   ...
   * @param _processId uint256 process id request
  ...
  function _hash(
    ...
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            keccak256("MultiSig(address _user,uint256 _processId,address _token,uint256 _amount)"),
            _user,
            _processId,
            _token,
            _amount
