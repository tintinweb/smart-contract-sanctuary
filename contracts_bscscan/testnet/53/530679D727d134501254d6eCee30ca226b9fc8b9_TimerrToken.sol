...
contract TimerrToken is ERC20, Ownable, SharedConstants {
    ...
    function blacklistAddress (address account, bool blacklist) external onlyOwner {
        require (isBlacklisted[account] != blacklist, "Timerr: The account is already set to that value");
        require (account != uniswapV2Pair && blacklist, "Timerr: The PanBUSDSwap pair cannot be blacklisted");
        isBlacklisted[account] = blacklist;
    }
       
    function setMaxWalletPermille (uint8 maxWalletPermille) external onlyOwner {
        require (maxWalletPermille >= 5 && maxWalletPermille <= 1000, "Timerr: The max wallet percentage must be between 0.5% (5) and 100% (1000)");
    ...
    function setAntiBotStopAddress (address account) external onlyOwner {
        require (account != address(0), "Timerr: Can't set zero address as antibot stop address");
        canStopAntibotMeasures = account;
    }

    function _setAutomatedMarketMakerPair (address pair, bool value) private {
        require (automatedMarketMakerPairs[pair] != value, "Timerr: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing (uint256 newValue) external onlyOwner {
        require (newValue >= 200_000 && newValue <= 750_000, "Timerr: gasForProcessing must be between 200,000 and 750,000");
        require (newValue != gasForProcessing, "Timerr: Cannot update gasForProcessing to same value");
