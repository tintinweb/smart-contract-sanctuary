    ...
    function update() external override(I_Windmill_Trade_Abstract, Windmill_Trade_Abstract) onlyRole(UPDATER_ROLE){
        ...
        while(i>0){
            ...
            if (_oneUpdateNeeded(order)){
                ...
                emit Test(0, sizeIn, BUSDToken.balanceOf(thisAddr), token.balanceOf(thisAddr), order.increaseSize);
                
                if (order.increaseSize){
                    uint256 nbToken = BUSDToken.balanceOf(thisAddr);
                    emit Test(1, 0, 0, 0, true);
                    if (nbToken > 0){
                    emit Test(2, 0, 0, 0, true);
                        if (nbToken < sizeIn){
                            sizeIn = nbToken;
                        }
                        _swap(sizeIn, route, BUSDToken);
                    ...
                    emit Test(3, 0, 0, 0, true);
                    uint256 nbToken = token.balanceOf(thisAddr);
                    if (nbToken > 0){
                    emit Test(4, 0, 0, 0, true);
                        if (nbToken < sizeIn){
                            sizeIn = nbToken;
                        }
                        _swap(sizeIn, reverseRoute, token);
