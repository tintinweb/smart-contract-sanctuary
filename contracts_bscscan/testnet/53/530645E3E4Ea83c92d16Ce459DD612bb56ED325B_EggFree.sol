...
contract EggFree is Context, Ownable, Pausable {
  using SafeERC20 for IERC20;
  using SafeMath for uint256;

  // Dont forget to set the selling parameter
  uint256 public constant MAX_SELL_PER_TX = 5;
  uint256 public constant MAX_SELL = 900_000;
  uint256 public constant MAX_SELL_PER_USER = 20;

  IERC20 public sellToken;
  IMooEggV5 public mooEgg;
  uint256 public totalSell;

  mapping(address => uint256) public userSold;

  event SetPrice(uint256 price);
  event EggSale(address to, uint256 tokenId);

  constructor(IMooEggV5 _mooEgg) {
    mooEgg = _mooEgg;
  }
  ...
  function buyEggs(uint256 n) external onlyEOA whenNotPaused {
    ...
    for (
      ...
      mooEgg.mint(msg.sender, IMooEggV5.EggCategory.NORMAL);
      emit EggSale(_msgSender(), tokenId);
    }
  }
