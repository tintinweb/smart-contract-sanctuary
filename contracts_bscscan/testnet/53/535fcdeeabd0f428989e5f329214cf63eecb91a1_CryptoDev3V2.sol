...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("CryptoDev3_Busd_Dividend_Tracker", "CryptoDev3_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 3000000000 * (10**18); //must hold 3,000,000,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "CryptoDev3_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "CryptoDev3_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CryptoDev3 contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CryptoDev3_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CryptoDev3_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract CryptoDev3V2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("CryptoDev3", "CR3") {
        	
    	marketingWallet = 0xaA4D491Fa63cc0663f1ABB42fB12fE7c610477FA; 
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "CryptoDev3: The marketing wallet is already this address");
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "CryptoDev3: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "CryptoDev3: The new dividend tracker must be owned by the CryptoDev3 token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "CryptoDev3: Account is already exluded from fees");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "CryptoDev3: Trading has not started yet");
