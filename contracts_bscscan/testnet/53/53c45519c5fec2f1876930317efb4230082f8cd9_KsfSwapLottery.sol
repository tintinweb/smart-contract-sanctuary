...
// File: contracts/KsfSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title KsfSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract KsfSwapLottery is ReentrancyGuard, IKsfSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInKsf = 50 ether;
    uint256 public minPriceTicketInKsf = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInKsf;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] ksfPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInKsf;
        uint32 finalNumber;
    }
     ...
     * @param _ksfswapFinanceAddress: address of the KSF token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _ksfswapFinanceAddress, address _randomGeneratorAddress) {
        ksfswapFinance = IERC20(_ksfswapFinanceAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of KSF to this contract
        uint256 amountKsfToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInKsf,
            _ticketNumbers.length
        );

        // Transfer ksf tokens to this contract
        ksfswapFinance.safeTransferFrom(address(msg.sender), address(this), amountKsfToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInKsf += amountKsfToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInKsfToTransfer
        uint256 rewardInKsfToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInKsfToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        ksfswapFinance.safeTransfer(msg.sender, rewardInKsfToTransfer);

        emit TicketsClaim(msg.sender, rewardInKsfToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInKsf) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in KSF for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].ksfPerBracket[j] =
                ...
                // A. No KSF to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].ksfPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInKsf - amountToShareToWinners);

        // Transfer KSF to treasury address
        ksfswapFinance.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        ksfswapFinance.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInKsf += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInKsf >= minPriceTicketInKsf) && (_priceTicketInKsf <= maxPriceTicketInKsf),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInKsf: _priceTicketInKsf,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            ksfPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInKsf: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(ksfswapFinance), "Cannot be KSF token");
    ...
    /**
     * @notice Set KSF price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInKsf: minimum price of a ticket in KSF
     * @param _maxPriceTicketInKsf: maximum price of a ticket in KSF
     */
    function setMinAndMaxTicketPriceInKsf(uint256 _minPriceTicketInKsf, uint256 _maxPriceTicketInKsf)
        external
        onlyOwner
    {
        require(_minPriceTicketInKsf <= _maxPriceTicketInKsf, "minPrice must be < maxPrice");

        minPriceTicketInKsf = _minPriceTicketInKsf;
        maxPriceTicketInKsf = _maxPriceTicketInKsf;
    }
     ...
     * @param _priceTicket price of a ticket (in KSF)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].ksfPerBracket[_bracket];
        } else {
            return 0;
        }
    }
