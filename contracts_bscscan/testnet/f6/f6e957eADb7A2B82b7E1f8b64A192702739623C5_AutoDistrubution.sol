...
contract MasterChefMoney is Ownable, ReentrancyGuard {
    ...
    returns (uint256, uint256)
    {
        ...
        moneyPendingReward = user.amount.mul(accMoneyPerShare).div(1e12).sub(user.rewardDebt);
        ...
        busdPendingReward = user.amount.mul(accBusdPerShare).div(1e12).sub(user.busdRewardDebt);
    ...
    function depositFor(address recipient, uint256 _pid, uint256 _amount) public validatePoolByPid(_pid) nonReentrant {
        ...
        if (pool.lockPeriod == 0) {
            // TODO -- staking lock should be reviewed again
            // TODO -- we apply a tax and burn but save user.amount full (wrong?)
            if (user.amount > 0) {
                // TESTER: I thing that you need to run this block on every deposit.
                uint256 pending =
                user.amount.mul(pool.accMoneyPerShare).div(1e12).sub(
                    user.rewardDebt
                );
                if (pending > 0) {
                    // AUDIT: MCM-14 | The logic issue of depositFor()
                    safeMoneyTransfer(recipient, pending);
                }
            }
        ...
        // if(pool.secondaryReward) {
        //     // TESTER: careful, here secondary reward always get computed independent of any lock.
    ...
    function withdraw(uint256 _pid, uint256 _amount) external validatePoolByPid(_pid) nonReentrant {
        ...
        if (pool.lockPeriod > 0 ) {
            if(_getNow() < user.lastDepositTime + pool.lockPeriod) {
                ...
                uint256 pending = user.amount.mul(pool.accMoneyPerShare).div(1e12).sub(user.rewardDebt);
                if (pending > 0) {
...
contract AutoDistrubution is Ownable {
    ...
    function getFeeToWallet6() external onlyOwner {
        ...
        for(uint256 i = 0; i < lpTokens.length; i++ ) {
            ...
            swapTokensForMoney(amount, lpTokens[i], address(this));
    ...
    function getFeeToWallet9() external onlyOwner {
        ...
        address[] memory moneyBnbStakers = masterChefTime.usersByPid(0);
        address[] memory moneyBusdStakers = masterChefTime.usersByPid(1);
        ProviderInfo[] memory provider = providerInfo[wallet];
        uint256 balance = moneyToken.balanceOf(provider[0].provider);
        uint256 amount = balance.mul(provider[0].percent).div(maxPercent);
    ...
    function swapTokensForMoney(uint amountIn, address tokenA, address to) internal {
