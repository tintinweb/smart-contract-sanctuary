...
contract AutoStakingV2 is IAutoStaking, OwnableUpgradeable {
    ...
    function getStatus(address _user, uint256 _bn) external view override returns (Status memory) {
        ...
        if (accMult != 0)
            rpb = mult * minter.X() / accMult;

        uint256 stakingDeposit = staking.getAccDeposit();
        if (stakingDeposit != 0)
            rpb = rpb * staking.deposited(address(this)) / stakingDeposit;
    ...
    function _withdraw(uint256 _amount) internal {
        ...
        if (user.shares > 0) {// think about this
    ...
    function setWithdrawFeePeriod(uint256 _newValue) external onlyOwner override {
        withdrawFeePeriod = _newValue;
        emit PeriodChange(_newValue);
    }

    function setPerformanceFee(uint256 _newValue) external onlyOwner override {
        performanceFee = _newValue;
    }

    function setCallFee(uint256 _newValue) external onlyOwner override {
        callFee = _newValue;
    }

    function setWithdrawFee(uint256 _newValue) external onlyOwner override {
        withdrawFee = _newValue;
    }

    function setDivisor(uint256 _newValue) external onlyOwner override {
        divisor = _newValue;
    }
}
...
//        console.log("_accStakingReward", _accStakingReward);
//        console.log("_lastAccumulatedReward", _lastAccumulatedReward);
//        console.log("farm[_addr].multiplier", farm[_addr].multiplier);
