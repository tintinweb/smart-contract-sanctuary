...
interface I_Windmill_Deployer is I_With_PancakeRouter, I_With_BUSDToken, I_With_WBNBToken{
    ...
    function deployUpdater(bytes memory bytecode) external returns (address payable);
...
contract Windmill_Deployer is Adressable, Ownable, With_PancakeRouter, With_BUSDToken, With_WBNBToken, I_Windmill_Deployer{
    ...
    function initWindmill(uint256[11] calldata data,
        ...
        windmill.PWR.grantRole(role, updaterAddress);
        ...
        windmill.fund.grantRole(role, updaterAddress);
        ...
        windmill.royalty.grantRole(role, updaterAddress);
        ...
        windmill.stacking.grantRole(role, updaterAddress);
        ...
        for (uint256 i=0; i<windmill.tradeDeployers.length; i++){
            ...
            windmill.tradeDeployers[i].grantRole(role, updaterAddress);
        ...
        windmill.tradeManager.grantRole(role, updaterAddress);
        windmill.tradeManager.revokeRole(role, thisAddr);
        
        //Setup updater
        windmill.updater.setStackingAddress(windmill.stacking);
        windmill.updater.setRoyaltyAddress(windmill.royalty);
        windmill.updater.setTradeManager(windmill.tradeManager);
        windmill.updater.setFund(windmill.fund);
        windmill.updater.setDAO(windmill.DAO);
        
        windmill.updater.grantRole(role, DAOAddress);
        windmill.updater.revokeRole(role, thisAddr);
