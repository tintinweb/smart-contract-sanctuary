...
contract Shop is Ownable{
    ...
    event startedSale (uint256 _id, uint256 _start, uint256 _end, uint256 _startPrice, uint256 _idFromNft, address _nft, address _customer);
    event NFTsold (address from, address to, address nft, uint256 NFT_ID, uint256 price);

    constructor (){

    }

    function startSale (uint256 _startPrice, uint256 _idFromNft, address _nft) public payable{

        require (salesOfCustomer[msg.sender].length < 3, "Customer has already 3 ongoing Sales");
        //require (getSaleId(saleId) == false, "SaleId already exists");
        require (transferNFTtoContract(_idFromNft, _nft, msg.sender), "something went wrong");
        saleId += 1;
        ...
        //TODO emit startedSale
    }

    function transferNFTtoContract (uint256 _idFromNft, address _nft, address _artist) internal returns (bool){

        ERC721 erc721NFT = ERC721(_nft);
        erc721NFT.approve(address(this), _idFromNft);
        erc721NFT.transferFrom(_artist, address(this), _idFromNft);

        if (erc721NFT.ownerOf(_idFromNft) == address(this)) return true;
        else return false;
    } 
    ...
    function getSaleId (uint256 _saleId)  public view returns (bool){

        if (salesMap[_saleId].created) return false;
        else return true;
    }
