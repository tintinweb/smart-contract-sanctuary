...
/**
 ...
 Launch Event Bonuses:
  ...
  ~ Compounding bonus to 3% -> 5% - for first 7 days.
  ~ Lottery 1% to 2% per deposit or compound - for the first lottery.
  ~ Cut off time 36 hours -> 72 hours for the first week.
 ...
 Lottery:
  ...
  ~ Each lottery will run every 4 hours or if it reaches 100 participants.
...
contract BSC_Defi {
    ...
    uint256 public EGGS_TO_HIRE_1MINERS_COMPOUND = 1200000;
    ...
    function hatchEggs(address ref, bool isCompound) public {
        ...
        if(isCompound) {
            ...
            //check the value if it can buy a ticket. if not, skip buyticket.
    ...
    function getEggsSinceLastHatch(address adr) public view returns(uint256){
        ...
        uint256 cutoffTime = min(secondsSinceLastHatch, CUTOFF_STEP); //get min time.
    ...
    /** setup for partners **/
    function addressSetup(address _addr) external {
        require(msg.sender == owner, "Admin use only.");
