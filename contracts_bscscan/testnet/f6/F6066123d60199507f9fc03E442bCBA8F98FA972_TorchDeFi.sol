...
contract LpVault{
    ...
    event Stake(address indexed _user, uint _poolId, uint _amount);
    event Unstake(address indexed _user, uint _poolId, uint _amount);
...
// -------------------------------------------User methods-----------------------------------------------------
    ...
    // user stakes Torch
    function stake(uint poolId, uint _amount, address _invitor) external notBlack{
        ...
        uint LP_rewards = _amount * poolInfo[poolId].yieldRate;
        ...
        emit Stake(_msgSender(), poolId, _amount);
    }  
    ...
    function getTorchPrice() internal view returns(uint price){
...
// -------------------------------------------Read-only methods-----------------------------------------------------
   ...
   function getUserStake(address _addr, uint _poolId) public view returns(uint _stake, uint _rewards, uint _timestamp){
...
contract TorchDeFi is LpVault{
    ...
    struct UserInfo{
        ...
        uint dynamicClaimed; // accumulated dynamic rewards
        
        uint actToken_consumed; // activateToken quantity consumed;
    }
    
    event StakeLp(address indexed user, uint amount);
    event ExitLp(address indexed user, uint amount);
    event ClaimStatic(address indexed user, uint amount);
    event ClaimDynamic(address indexed user, uint amount);
    event NodeRewards(address indexed user, uint dayindex, uint amount);
    event NodeReg(address indexed user, uint dayindex, uint level);
    ...
    function stakeLp(uint _amount, address _invitor) external updateReward(msg.sender) notBlack
        {   
        require(initiated && block.timestamp >= init_time_lp,"not initiated yet");
    ...
    function claimDynamic() external updateReward(_msgSender()){
        ...
        uint temp_amount = user.dynamicToClaim_g + user.dynamicToClaim_d;
        
        user.dynamicToClaim_d = 0;
        user.dynamicToClaim_g = 0;
        user.dynamicClaimed = temp_amount;
        TotalRewards_dynamic += temp_amount;
        IERC20(Torch).transfer(_msgSender(), temp_amount);
        
        emit ClaimDynamic(_msgSender(), temp_amount);
    }
    ...
    modifier updateMiningRate() {
        ...
        if(TotalRewards_static + TotalRewards_dynamic >= 24000000* 1e18){
            dailyOutput = 0;
        }
        _;
    }
...
//------------------------------------------AirDrop--------------------------------------------------
    ...
    event AirdropRegister(address indexed _user, uint _serial_n);
    event AirdropClaim(address indexed _user, uint _serial_n, uint _amount);
    ...
    */
    ...
    function stakeAirdrop(uint amount) external{
        require(amount<= userAccount[_msgSender()].LP_n, "insufficient LP");
        userAccount[_msgSender()].LP_n -= amount;
    ...
    function unstakeAirdrop()external{
        ...
        userAccount[_msgSender()].LP_n += temp;
    }
