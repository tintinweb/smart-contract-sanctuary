...
contract Equity is ReflectiveToken {
    constructor(address[] memory teamWallets_, address marketingWallet_, address uniswapV2Router02Address_, address reflectionTrackerAddress_) ReflectiveToken(
        "Test",
        "Test",
        1020000,
        teamWallets_,
...
contract ReflectiveToken is ERC20, SharedOwnable {
    ...
    constructor(string memory name_, string memory symbol_, uint256 supply_, address[] memory teamWallets_, address marketingWallet_, address uniswapV2Router02Address_, uint256 minimumTokenBalanceForSwapAndSendReflections_, bool regularTransferAllowed_, TransferFee memory defaultUniswapV2PairTransferFee_, uint256 teamAndMarketingWalletsTimeLockInSeconds_, address reflectionTrackerAddress) ERC20(name_, symbol_) {
        ...
        _regularTransferAllowed = regularTransferAllowed_;
        ...
        for (uint i = 0; i < teamWalletsLength; i++)
            _setDeveloperWalletOptions(_teamWallets[i], teamAndMarketingWalletsTimeLockInSeconds_);
        _setDeveloperWalletOptions(_marketingWallet, teamAndMarketingWalletsTimeLockInSeconds_);
        _setDeveloperWalletOptions(msg.sender, 0);

        _reflectionTracker = IReflectionTracker(reflectionTrackerAddress);
    ...
    function _setDeveloperWalletOptions(address account, uint256 timeLockInSeconds) private {
