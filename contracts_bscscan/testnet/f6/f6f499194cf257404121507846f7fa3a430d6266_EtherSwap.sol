...
require(token.balanceOf(msg.sender) >= _amount, "Error g token");
...
require(token1.balanceOf(address(this)) >= etherAmount, "Error r token");
...
emit TokensSold(address(token1),_amount,rate);
