...
contract SafePokemanToken is Context, IERC20, Ownable {
    ...
    string private _name = 'SafePokemanToken';
    string private _symbol = 'SAFEPOKEMAN';
    uint8 private _decimals = 18;

    constructor (address rewardAddress, address devAddress) public {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _transferToRewardPool(sender, rReserve, tReserve.div(2));
        _transferToDev(sender, rReserve, tReserve.div(2));
    ...
    function _transferToRewardPool(address sender, uint256 rReserve, uint256 tReserve) private {
        _rOwned[_rewardAddress] = _rOwned[_rewardAddress].add(rReserve);
        if (_isExcluded[_rewardAddress]) {
            _tOwned[_rewardAddress] = _tOwned[_rewardAddress].add(tReserve);
        }
        emit Transfer(sender, _rewardAddress, tReserve);
    }
    
    function _transferToDev(address sender, uint256 rReserve, uint256 tReserve) private {
        _rOwned[_devAddress] = _rOwned[_devAddress].add(rReserve);
        if (_isExcluded[_devAddress]) {
            _tOwned[_devAddress] = _tOwned[_devAddress].add(tReserve);
        }
        emit Transfer(sender, _devAddress , tReserve);
    }
