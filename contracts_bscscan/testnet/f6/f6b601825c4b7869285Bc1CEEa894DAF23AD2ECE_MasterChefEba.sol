...
// MasterChefEba is the master of EBAs. He can make EBAs and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once EBAs is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract MasterChefEba is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accEbaPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accEbaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. EBAs to distribute per block.
        uint256 lastRewardBlock; // Last block number that EBAs distribution occurs.
        uint256 accEbaPerShare; // Accumulated per share, times 1e12. See below.
    }

    // The EBAs TOKEN!
    ElpisBattle public eba;
    // Dev address.
    address public devaddr;
    // Treasury address.
    address public treasury;
    // EBAs tokens created per block.
    uint256 public ebaPerBlock;
    // Bonus muliplier for early EBAs makers.
    ...
    function setEbaPerBlock(uint256 _ebaPerBlock) public onlyOwner {
        massUpdatePools();
        ebaPerBlock = _ebaPerBlock;
    }
    ...
    // Update the given pool's EBAs allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
        ...
        returns (uint256)
    {
        ...
        uint256 accEbaPerShare = pool.accEbaPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier =
                getMultiplier(pool.lastRewardBlock, block.number);
            uint256 ebaReward =
                multiplier.mul(ebaPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accEbaPerShare = accEbaPerShare.add(
                ebaReward.mul(PRECISION).div(lpSupply)
            );
        }
        return
            user.amount.mul(accEbaPerShare).div(PRECISION).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (BONUS_DIVISOR_FOR_DEV > 0) {
            eba.mint(devaddr, ebaReward.div(BONUS_DIVISOR_FOR_DEV));
        }
        pool.accEbaPerShare = pool.accEbaPerShare.add(
            ebaReward.mul(PRECISION).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for EBAs allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accEbaPerShare).div(PRECISION).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accEbaPerShare).div(PRECISION);
