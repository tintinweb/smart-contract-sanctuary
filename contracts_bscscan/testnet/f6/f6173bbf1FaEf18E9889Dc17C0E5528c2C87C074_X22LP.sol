...
contract X22LP is ReentrancyGuard {
    ...
    mapping(address => uint256[N_COINS]) public requestedAmount;
    ...
    function calculateTotalToken(bool _supply)public view returns(uint256){
        ...
        if(_supply){
            return selfBalance.sub(reserveTotal).sub(storedFeesTotal).add(loanGiven);
        }
        else{
            return selfBalance.sub(reserveTotal).sub(storedFeesTotal);
        }
    ...
    function requestWithdrawWithXPT(uint256 amount,uint256 _index, bool payingCharges, uint256[N_COINS] memory maxBurn) external nonReentrant validAmount(amount){
        ...
        if(payingCharges == true){
           for(uint8 i=0;i<N_COINS;i++){
            uint256 currentPoolAmount = getBalances(i);
            if(requestedAmount[msg.sender][i]>currentPoolAmount){
                _withdraw(requestedAmount[msg.sender],maxBurn);
            }
           }
           uint temp = (tokenAmount.mul(fees)).div(100);
           reserveAmount[_index] =reserveAmount[_index].sub(tokenAmount);
           selfBalance = selfBalance.sub((tokenAmount.sub(temp)).mul(1e18).div(10**decimal));
           totalBurnt =totalBurnt.add(amountBurnt[msg.sender][_index]);
           tokens[_index].safeTransfer(msg.sender, tokenAmount.sub(temp));
           emit userRecieved(msg.sender,tokenAmount.sub(temp),_index);
           amountBurnt[msg.sender][_index]=0;
           storedFees[_index] =storedFees[_index].add(temp);
           XPTtoken.burn(msg.sender, amount);
           amountBurnt[msg.sender][_index] = amount;
           updateLockedXPT(msg.sender,totalBurnt);
        }
        else{    
        require(tokenAmount <= calculateTotalToken(false),"Not Enough Pool balance");
        requestedAmount[msg.sender][_index] = tokenAmount;
    ...
    function cancelWithdraw() external{
        for(uint8 i=0; i<N_COINS; i++){
            requestedAmount[msg.sender][i] = 0;
        }
    ...
    function claimTokens(bool payingCharges, uint256[N_COINS] memory max_burn) external  nonReentrant{
        ...
        if(payingCharges){
            for(uint8 i=0;i<N_COINS;i++){
                temp = (requestedAmount[msg.sender][i].mul(fees)).div(100);
            }
        }
        else{
        ...
        for(uint8 i=0;i<N_COINS;i++){
            uint256 currentPoolAmount = getBalances(i);
            if(requestedAmount[msg.sender][i]>currentPoolAmount){
                _withdraw(requestedAmount[msg.sender],max_burn);
            }
        }
        for(uint8 i=0; i<N_COINS; i++) {
            if(requestedAmount[msg.sender][i] > 0) {
                decimal=tokens[i].decimals();
                reserveAmount[i] =reserveAmount[i].sub(requestedAmount[msg.sender][i]);
                selfBalance = selfBalance.sub((requestedAmount[msg.sender][i].sub(temp)).mul(1e18).div(10**decimal));
                totalBurnt =totalBurnt.add(amountBurnt[msg.sender][i]);
                tokens[i].safeTransfer(msg.sender, requestedAmount[msg.sender][i].sub(temp));
                emit userRecieved(msg.sender,requestedAmount[msg.sender][i].sub(temp),i);
                requestedAmount[msg.sender][i] = 0;
                amountBurnt[msg.sender][i]=0;
                storedFees[i] =storedFees[i].add(temp);
                XPTtoken.burn(msg.sender, toBurn[msg.sender]);
                amountBurnt[msg.sender][i] = toBurn[msg.sender];
            }
        }
  ...
  //Withdraw total liquidity from yield generation pool
    ...
    function rebalance(uint[N_COINS] memory max_burn,uint[N_COINS] memory toWithdraw) onlyWallet() external {
        require(checkValidArray(toWithdraw), "queue empty");
        _withdraw(toWithdraw,max_burn);
