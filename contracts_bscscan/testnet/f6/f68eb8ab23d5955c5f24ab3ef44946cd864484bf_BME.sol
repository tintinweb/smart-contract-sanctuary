...
contract BME is ERC20, Ownable {
    ...
    address public immutable BTCB = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
    ...
    uint256 public  oneDayRestriction;
    ...
    constructor() public ERC20("test1", "test2") {
    ...
    function getClaimTime() external view returns(uint256) {
      return dividendBTCBTracker.getClaimTime(msg.sender);
    }

    function getNumberOfDividendTokenHolders() external view returns(uint256) {
        return dividendBTCBTracker.getNumberOfTokenHolders();
    }
    ...
    function StartCycel(uint256 newStartTime ,uint256 newEndTime ,uint256 newMinimumTokens ) external onlyOwner{
        dividendBMETracker.StartCycel(newStartTime ,newEndTime ,newMinimumTokens );
    }

    function resetCycel() external onlyOwner{
        dividendBMETracker.resetCycel();
    }
    ...
    function UpdateCycel(uint256 newnumberOfTokenHolders ,uint256 newMinimumHoldingDays ,uint256 newtimeToBeOut) external onlyOwner{
        dividendBMETracker.UpdateCycel(newnumberOfTokenHolders ,newMinimumHoldingDays ,newtimeToBeOut);
    }
    ...
    function _transfer(
        ...
        if(balanceOf(to) == 0){
            ...
            antiWhaleSellAmount[to] = 0;
            antiWhaleRestriction[to] = block.timestamp;


        }else
        if(antiWhaleRestriction[from] >= block.timestamp.add(oneDayRestriction)){
        ...
        if(
            ...
            require(antiWhaleSellAmount[from].add(amount) <= maxSellTransactionAmount, "Sell transfer total day amount exceeds the maxSellTransactionAmount.");
        }
        ...
        if(automatedMarketMakerPairs[from] && amount >= minimumAmountBuyTrades ){
          if(NumberOfBuyTrades[to] < minimumBuyTrades ){

            NumberOfBuyTrades[to] = NumberOfBuyTrades[to].add(1);

          }else if(NumberOfBuyTrades[to] >= minimumBuyTrades ){
            dividendBMETracker.startCalculetHoldingDays(to);
        ...
        dividendBTCBTracker.process(gas);
        
	    dividendBMETracker.process(gas); 
    ...
    function calculetBMEDividends(uint256 tokens) private {
        ...
        TotalTokensReward = tokens;
        uint256 iterations = 0;
        
        uint256 rewardPerToken = TotalTokensReward.mul(magnitude).div(totalSupply());

          while(iterations < numberOfTokenHolders) {
          iterations++;

          address account = tokenHoldersMap.keys[iterations];
          uint256 tokensForoneToken = rewardPerToken.mul(balanceOf(account)).div(magnitude);
    ...
    function SendBMEDividends() private {
...
contract BMEBTCBTracking is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("BMETrackingBTCB", "BMETB") {
      ...
      claimWait1Hour = 3600;
      claimWait6Hours = 21600;
      claimWait12Hours = 43200;
      claimWait24Hours = 86400;
...
contract BMETokenTracking is DividendPayingTokenBME, Ownable {
    ...
    mapping (address => bool) public isEnableToStartHolding;
    mapping (address => uint256) public HoldingDays;
    mapping (address => bool) public isEnableToJoinCycel;
    ...
    constructor() public DividendPayingTokenBME("BMETrackingToken", "BMETT") {
      ...
      oneDay = 86400; // time to calculet holding days (1 Day)
    ...
    function resetCycel() external onlyOwner returns (uint256) {
      uint256 numberOfTokenHolders = tokenHoldersMap.keys.length;
    ...
    function UpdateCycel(uint256 newnumberOfTokenHolders ,uint256 newMinimumHoldingDays ,uint256 newtimeToBeOut) external onlyOwner{
        ...
        maxmumCycelmembers = newnumberOfTokenHolders;
    ...
    function setStageOne(uint256 rewardAmount ,uint256 getRewardFrom ,uint256 getRewardTo ) external onlyOwner{
        ...
        stageOneReward = rewardAmount.div(100);
    ...
    function setStageTwo(uint256 rewardAmount,uint256 getRewardFrom,uint256 getRewardTo) external onlyOwner{
        require(getRewardTo >= getRewardFrom.add(rewardAmount), "BMETrackingToken: getRewardTo  must be higher than getRewardFrom with rewardAmount");
        stageTwoReward = rewardAmount.div(100);
    ...
    function setStageThree(uint256 rewardAmount ,uint256 getRewardFrom ,uint256 getRewardTo ) external onlyOwner{
        require(getRewardTo >= getRewardFrom.add(rewardAmount), "BMETrackingToken: getRewardTo  must be higher than getRewardFrom with rewardAmount");
        stageThreeReward = rewardAmount.div(100);
    ...
    function setBalance(address payable account, uint256 newBalance) external onlyOwner {
      ...
      if( tokenHoldersMap.keys.length < maxmumCycelmembers ){
      if( isEnableToJoinCycel[account] ){
    	if(newBalance >= minimumTokenBalanceForDividends) {
            _setBalance(account, newBalance);
    		tokenHoldersMap.set(account, newBalance);
        lastClaimTimes[account] = block.timestamp;
    ...
    function process(uint256 gas) public returns (uint256, uint256, uint256) {
      ...
      }else if( lastClaimTimes[account] > block.timestamp.add(timeToBeOut) ){
          excludedFromDividends[account] = true;
      }
    ...
    function exceptHoldingDays(uint256 checkHoldingDays) public returns (bool){

      if(checkHoldingDays >= MinimumHoldingDays ){
    ...
    function EnableCalculetHoldingDays(address account) public onlyOwner {

      isEnableToStartHolding[account] = true;


    }

    function checkisEnableToStartHolding(address account) public returns (bool){

       return isEnableToStartHolding[account];

    }

    function startCalculetHoldingDays(address account) public onlyOwner {
        
        bool canJoin = HoldingDays[account] < MinimumHoldingDays;

      if(isEnableToStartHolding[account] && canJoin ){

      HoldingDays[account] = HoldingDays[account].add(1);

      }
      if(isEnableToStartHolding[account] && HoldingDays[account] >= MinimumHoldingDays ){

        isEnableToJoinCycel[account] = true;
      }
    }
