...
contract MoonBunny is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public xrpDividendToken;
    ...
    bool public xrpDividendEnabled = false;
    bool public polkaDividendEnabled = false;

    XrpDividendTracker public xrpDividendTracker;
    ...
    uint256 private xrpDividendRewardsFee;
    uint256 private previousXrpDividendRewardsFee;
    ...
    event UpdateXrpDividendTracker(address indexed newAddress, address indexed oldAddress);
    ...
    event XrpDividendEnabledUpdated(bool enabled);
    ...
    event ProcessedXrpDividendTracker(
    	uint256 iterations,
    	uint256 claims,
    ...
    constructor() ERC20("MoonBunny", "$MBY") {
    	xrpDividendTracker = new XrpDividendTracker();
    	polkaDividendTracker = new PolkaDividendTracker();

    	marketingWallet = 0xa0Ed02d1eB1166ED8f95bC9202a8f19d6cED0312;
    	divident = 0x5fAc36F132E5ddb920AB7396446a4E457c85521A;
    	xrpDividendToken = 0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE;
        ...
        excludeFromDividend(address(xrpDividendTracker));
    ...
    receive() external payable {
  	...
  	function whitelistDxSale(address _presaleAddress, address _routerAddress) external onlyOwner {
        ...
        xrpDividendTracker.excludeFromDividends(_presaleAddress);
        polkaDividendTracker.excludeFromDividends(_presaleAddress);
        excludeFromFees(_presaleAddress, true);

        xrpDividendTracker.excludeFromDividends(_routerAddress);
  	...
  	function prepareForPartherOrExchangeListing(address _partnerOrExchangeAddress) external onlyOwner {
  	    xrpDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
  	...
  	function updateXrpDividendToken(address _newContract) external onlyOwner {
  	    xrpDividendToken = _newContract;
  	    xrpDividendTracker.setDividendTokenAddress(_newContract);
  	}
  	
  	
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "MoonBunny: The marketing wallet is already this address");
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(polkaDividendRewardsFee).add(xrpDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setXrpDividendEnabled(bool _enabled) external onlyOwner {
        require(xrpDividendEnabled != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
            previousXrpDividendRewardsFee = xrpDividendRewardsFee;
            xrpDividendRewardsFee = 0;
            xrpDividendEnabled = _enabled;
        } else {
            xrpDividendRewardsFee = previousXrpDividendRewardsFee;
            totalFees = xrpDividendRewardsFee.add(marketingFee).add(polkaDividendRewardsFee).add(buyBackAndLiquidityFee);
            xrpDividendEnabled = _enabled;
        }

        emit XrpDividendEnabledUpdated(_enabled);
    }
    ...
    function setPolkaDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = polkaDividendRewardsFee.add(marketingFee).add(xrpDividendRewardsFee).add(buyBackAndLiquidityFee);
            polkaDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(polkaDividendRewardsFee).add(xrpDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateXrpDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(xrpDividendTracker), "MoonBunny: The dividend tracker already has that address");

        XrpDividendTracker newXrpDividendTracker = XrpDividendTracker(payable(newAddress));

        require(newXrpDividendTracker.owner() == address(this), "MoonBunny: The new dividend tracker must be owned by the MoonBunny token contract");

        newXrpDividendTracker.excludeFromDividends(address(newXrpDividendTracker));
        newXrpDividendTracker.excludeFromDividends(address(this));
        newXrpDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newXrpDividendTracker.excludeFromDividends(address(deadAddress));

        emit UpdateXrpDividendTracker(newAddress, address(xrpDividendTracker));

        xrpDividendTracker = newXrpDividendTracker;
    }
    
    function updatePolkaDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(polkaDividendTracker), "MoonBunny: The dividend tracker already has that address");

        PolkaDividendTracker newPolkaDividendTracker = PolkaDividendTracker(payable(newAddress));

        require(newPolkaDividendTracker.owner() == address(this), "MoonBunny: The new dividend tracker must be owned by the MoonBunny token contract");
    ...
    function updateXrpDividendRewardFee(uint8 newFee) external onlyOwner {
        xrpDividendRewardsFee = newFee;
        totalFees = xrpDividendRewardsFee.add(marketingFee).add(polkaDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    
    function updatePolkaDividendRewardFee(uint8 newFee) external onlyOwner {
              polkaDividendRewardsFee = newFee;
        totalFees = polkaDividendRewardsFee.add(xrpDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }
    
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        marketingFee = newFee;
        totalFees = marketingFee.add(xrpDividendRewardsFee).add(polkaDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(xrpDividendRewardsFee).add(polkaDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "MoonBunny: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "MoonBunny: Account is already exluded from fees");
    ...
    function excludeFromDividend(address account) public onlyOwner {
        xrpDividendTracker.excludeFromDividends(address(account));
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MoonBunny: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "MoonBunny: Automated market maker pair is already set to that value");
        automatedMarketMakerPairs[pair] = value;

        if(value) {
            xrpDividendTracker.excludeFromDividends(pair);
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "MoonBunny: Cannot update gasForProcessing to same value");
        gasForProcessing = newValue;
        emit GasForProcessingUpdated(newValue, gasForProcessing);
    }
    
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        xrpDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
        polkaDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }

    function updateClaimWait(uint256 claimWait) external onlyOwner {
        xrpDividendTracker.updateClaimWait(claimWait);
        polkaDividendTracker.updateClaimWait(claimWait);
    }

    function getXrpClaimWait() external view returns(uint256) {
        return xrpDividendTracker.claimWait();
    }
    ...
    function getTotalXrpDividendsDistributed() external view returns (uint256) {
        return xrpDividendTracker.totalDividendsDistributed();
    }
    ...
    function withdrawableXrpDividendOf(address account) external view returns(uint256) {
    	return xrpDividendTracker.withdrawableDividendOf(account);
  	}
	...
	function xrpDividendTokenBalanceOf(address account) external view returns (uint256) {
		return xrpDividendTracker.balanceOf(account);
	}
    ...
    function getAccountXrpDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return xrpDividendTracker.getAccount(account);
    }
	...
	function getAccountXrpDividendsInfoAtIndex(uint256 index)
        external view returns (
            address,
            int256,
            int256,
            ...
            uint256,
            uint256) {
    	return xrpDividendTracker.getAccountAtIndex(index);
    }
	...
	function processDividendTracker(uint256 gas) external onlyOwner {
		(uint256 ethIterations, uint256 ethClaims, uint256 ethLastProcessedIndex) = xrpDividendTracker.process(gas);
		emit ProcessedXrpDividendTracker(ethIterations, ethClaims, ethLastProcessedIndex, false, gas, tx.origin);
    ...
    function claim() external {
		xrpDividendTracker.processAccount(payable(msg.sender), false);
		polkaDividendTracker.processAccount(payable(msg.sender), false);
    }
    function getLastXrpDividendProcessedIndex() external view returns(uint256) {
    	return xrpDividendTracker.getLastProcessedIndex();
    }
    ...
    function getNumberOfXrpDividendTokenHolders() external view returns(uint256) {
        return xrpDividendTracker.getNumberOfTokenHolders();
    }
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "MoonBunny: Trading has not started yet");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (xrpDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(xrpDividendRewardsFee.add(polkaDividendRewardsFee)).mul(xrpDividendRewardsFee);
                    swapAndSendXrpDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (polkaDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(xrpDividendRewardsFee.add(polkaDividendRewardsFee)).mul(polkaDividendRewardsFee);
        ...
        try xrpDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try polkaDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try xrpDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        try polkaDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}

        if(!swapping) {
	    	uint256 gas = gasForProcessing;

	    	try xrpDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
	    		emit ProcessedXrpDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
	    	}
	    	catch {
    ...
    function swapAndSendXrpDividends(uint256 tokens) private {
        swapTokensForDividendToken(tokens, address(this), xrpDividendToken);
        uint256 xrpDividends = IERC20(xrpDividendToken).balanceOf(address(this));
        transferDividends(xrpDividendToken, address(xrpDividendTracker), xrpDividendTracker, xrpDividends);
    }
...
contract XrpDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MoonBunny_Xrpereum_Dividend_Tracker", "MoonBunny_Xrpereum_Dividend_Tracker", 0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "MoonBunny_Xrpereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "MoonBunny_Xrpereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MoonBunny contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "MoonBunny_Xrpereum_Dividend_Tracker: Cannot update claimWait to same value");
...
contract PolkaDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("MoonBunny_Polka_Dividend_Tracker", "MoonBunny_Polka_Dividend_Tracker", 0x7083609fCE4d1d8Dc0C979AAb8c869Ea2C873402) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "MoonBunny_Polka_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "MoonBunny_Polka_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MoonBunny contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "MoonBunny_Polka_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
