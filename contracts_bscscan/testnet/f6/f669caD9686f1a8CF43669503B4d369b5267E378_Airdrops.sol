...
*/
	...
	constructor( iMoonCoin _MoonCoin) public { 
	  ...
	  function setJoinBonus(uint256 _JOIN_BONUS) external { 
		...
		function addAirdropsToRegisteredByUserId(uint256[] memory _receivers, uint256[] memory _amounts, uint256 _totalCost) public {
			...
			function addAirdropsByTeamLeaderByAddress(address[] memory _receivers, uint256[] memory _amounts, uint256 _totalCost) public {
			uint256 amountReceived;			
			for (uint256 i = 0; i < _receivers.length; ++i) {				
