...
contract MinoWarMarket is Ownable {
    ...
    uint nonce = 0;
    IERC721 public minoWarNFT;
    uint256 public feeMarket = 100;
    address payable public feeWallet;
    uint256 constant public PERCENTS_DIVIDER = 10000;

    constructor(address payable _feeWallet, IERC721 _minoWarNFT)  {
        feeWallet = _feeWallet; 
        minoWarNFT = _minoWarNFT;
    }
   ...
   /**
    ...
    function placeOrder(uint256 _tokenId, uint256 _price, address _token) public {
        require(minoWarNFT.ownerOf(_tokenId) == _msgSender(), "not own");
    ...
    function fillOrder(uint256 _tokenId, address _token) public payable {
        ...
        uint256 _feeMarket = itemSale.price.mul(feeMarket).div(
            PERCENTS_DIVIDER
        );
        if (_token == address(0)) {
        ...
        if(_token != address(0)) {
            IERC20(_token).transferFrom(_msgSender(), feeWallet, _feeMarket);
            IERC20(_token).transferFrom(
                _msgSender(),
                itemSale.owner,
                itemSale.price.sub(_feeMarket)
            );
        }
    ...
    function tokenOrder(
        ...
        ItemSale storage itemSale = markets[_tokenId];
        if (_sell) {
            minoWarNFT.transferFrom(_msgSender(), address(this), _tokenId);
            ...
            minoWarNFT.transferFrom(address(this), _msgSender(), _tokenId);
