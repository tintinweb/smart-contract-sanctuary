...
contract ShkoobyStake is Ownable {
    ...
    function addTokenPool(address _tokenAddress,uint256 _duration, uint256 _rewards, uint256 _withdrawalFee, uint256 _unlockPenalty ) public returns (bool){
    ...
    function getPoolDetails(uint256 _stakePoolID) public view returns(address, address, uint256[] memory){
        stakePool memory stakePoolDetails = stakePools[_stakePoolID];

        uint [] memory stakePoolDetailsArray = new uint[](9);
        ...
        stakePoolDetailsArray[7] = stakePoolDetails.status;
        
        stakePoolDetailsArray[8] = stakePoolDetails.createdTime;
        
        return (stakePoolDetails.tokenAddress, stakePoolDetails.creator,stakePoolDetailsArray);
    }

    function addUserStakes(uint256 _stakeId) public returns (bool){
        userStake storage userStakeDetails = userStakes[msg.sender];
        //uint256 arrayIndex = userStake.activeStakes.length;
        userStakeDetails.activeStakes.push(_stakeId);
        //uint256[] memory stakeIds = userStake.activeStakes;
        //uint256[] memory stakeIds = [10,20,30];
        //stakeid.push(_stakeId);
        //stakeIds = userStake.activeStakes;
        
        //uint256 arrayIndex = stakeIds.length;
        
        //stakeIds[arrayIndex] = _stakeId;
        //userStake.activeStakes=stakeIds;
        //userStake.activeStakes[0]=_stakeId;
        
        userStakes[msg.sender] = userStakeDetails;
        return true;
    }
