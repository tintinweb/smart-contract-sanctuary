...
contract RoomManager is SafeOwnable, Random {
    using SafeMath for uint256;
    ...
    modifier RoomNotBegin(uint rid) {
        require(block.timestamp < roomInfo[rid].startSeconds, "Room Already Begin");
        _;
    }
    ...
    function add(IERC20 _rewardToken, uint256 _rewardAmount, uint256 _startSeconds, uint256 _endSeconds, IERC20[] memory _tokens, uint256[] memory _amounts, uint256[] memory _nftTypes, uint256[] memory _nftPercents) external onlyOwner {
        require(address(_rewardToken) != address(0), "rewardToken is zero address");
        require(_startSeconds >= block.timestamp && _endSeconds > _startSeconds, "illegal time");
        ...
        uint rangeInfoLength = rangeInfo[rid].length;
        uint lastEndIndex = 0;
        for (uint i = 0; i < rangeInfoLength; i ++) {
            lastEndIndex = rangeInfo[rid][i].endIndex;
        }
        for (uint i = 0; i < _nftTypes.length; i ++) {
            uint nftId = nftToken.create(uint256(-1), 0, _nftTypes[i], "0x");
            nftToken.disableTokenTransfer(nftId);
    ...
    function buyBlindBox(uint256 _rid, IERC20 _token, uint256 _num, address _to) external {
        ...
        SafeERC20.safeTransferFrom(_token, msg.sender, receiver, valueAmount[_rid][_token].mul(_num));
        ...
        RandomInfo memory random;
        random.to = _to;
        random.rid = _rid;
        random.num = _num;
        randomInfo[requestId] = random;
    }
    ...
    function claim(uint256 rid, address to) external {
        ...
        require(room.rewardToken.balanceOf(address(this)) >= reward, "reward token not enough");
        for (uint i = 0; i < balances.length; i ++) {
            balances[i] = minNum;
        }
