...
contract DividendTracker is DividendPayingToken {
  ...
  constructor(
    ...
    minimumTokenBalanceForDividends = 50_000 * 10**9; 
  }
...
// File: contracts/IHeroDogeCoin.sol
...
interface IHeroDogeCoin is IERC20, IERC20Metadata {
  ...
  function changeDistribution(uint256 percentToLiquidity, uint256 percentToMarketing, uint256 percentToDividends)  external;
  function changeBuyFees(uint256 totalBuyFee)  external;
  function changeSellFees(uint256 totalSellFee)  external;

}

// File: contracts/HeroDogeCoin.sol
  ...
  ReentrancyGuard
{
  ...
  uint256 public _marketingDivisor;
  uint256 public _liquidityDivisor;
  uint256 public _dividendRewardsDivisor;
  uint256 public _totalDivisor = 100;
  ...
  constructor(
    ...
    uint256 liquidityDivisor_,
    uint256 marketingDivisor_,
    uint256 dividendRewardsDivisor_,
    ...
    changeDistribution(liquidityDivisor_, marketingDivisor_, dividendRewardsDivisor_);
    _totalBuyFee = _totalFee;
    _totalSellFee = _totalFee;
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    ...
    dividendTracker.excludeFromDividends(dxRouter);
  }
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    require(
      ...
      'HeroDoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs'
    );
  ...
  function _setAutomatedMarketMakerPair(address pair, bool value) private {
    require(
      ...
      'HeroDoge: Automated market maker pair is already set to that value'
    );
  ...
  function _transfer(address from, address to, uint256 amount) private {
    ...
    if(
      ...
      uint256 swapForMarketing = (contractTokenBalance * _liquidityDivisor) / _totalDivisor;
      uint256 swapForLiq = (contractTokenBalance * _liquidityDivisor) / _totalDivisor;
      uint256 swapForDividends = contractTokenBalance - swapForLiq - swapForMarketing;
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    require(
      ...
      'HeroDogeCoin: The dividend tracker already has that address'
    );
    ...
    require(
      ...
      'HeroDogeCoin: The new dividend tracker must be owned by the token contract'
    );
  ...
  function changeDistribution(
    uint256 percentToLiquidity,
    uint256 percentToMarketing,
    uint256 percentToDividends
  )
    public
    override
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    require(percentToLiquidity <= 25);
    require(percentToMarketing <= 50);
    require(percentToDividends <= 60);
    require(percentToLiquidity + percentToMarketing + percentToDividends == 100);

    _liquidityDivisor = percentToLiquidity;
    _marketingDivisor = percentToMarketing;
    _dividendRewardsDivisor = percentToDividends;
  }

  function changeBuyFees(
    uint256 totalBuyFee_
  )
    public
    override
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    ...
    require(totalBuyFee_ <= 20);

    _totalBuyFee =  totalBuyFee_;
  }

  function changeSellFees(
    uint256 totalSellFee_
  )
    public
    override
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    // fees are setup so they can not exceed 30% in total
    // and specific limits for each one.
    require(totalSellFee_ <= 30);

    _totalSellFee =  totalSellFee_;
  }
