...
contract ElderDogeCoin is Context, IBEP20, Ownable { // change contract name
    ...
    mapping (address => bool) private _isExcludedFromTransactionlock; // Address to be excluded from transaction cooldown


    address[] private _excluded; // storing reflection excluded address so, no reflection send to them
   
    address payable public _marketingAddress = payable(0xf7c35eC53A4e1F287104fD6C729F645D6EeF944b); // Marketing Address
    address payable immutable public _burnAddress = payable(0x000000000000000000000000000000000000dEaD); // Burn Address

    string private _name = "Elder Doge Coin"; // token name
    string private _symbol = "EDC"; // token symbol
    ...
    uint256 private _tTotal = 21000 * 10**6 * 10**_decimals;
    ...
    uint256 public _burnFee = 30; // burn fee 3%
    ...
    uint256 private _previousBuyBackFee = _marketingFee; // buyBack fee
    ...
    uint256 private _totalLiquidityFee = _marketingFee.add(_liquidityFee).add(_buyBackFee); // liquidity + buyback + marketing fee on each transaction
    ...
    uint256 public  numTokensSellToAddToLiquidity = 1000 * 10**6 * 10**_decimals; // min token liquidity fee collected before swapandLiquify
    uint256 public _maxTokensPerAddress            = 50000000 * 10**_decimals; // Max number of tokens that an address can hold
    ...
    modifier transactionIsUnlocked(){
		...
		|| getIsExcludedFromTransactionlock(_msgSender())
    ...
    function transfer(address recipient, uint256 amount) public override transactionIsUnlocked returns (bool) {
     ...
     */
    function transferFrom(address sender, address recipient, uint256 amount) public override transactionIsUnlocked returns (bool) {
    ...
    function setMarketingFeePercent(uint256 Fee) external onlyOwner() {
        _marketingFee = Fee;
        _totalLiquidityFee = _liquidityFee.add(_marketingFee).add(_buyBackFee);
    }

    /**  
     * @dev set's buyBack fee percentage
     */
    ...
    {
        numTokensSellToAddToLiquidity = minMarketingValue.mul( 10**_decimals );
    }
	...
	*/
    ...
    /**
	 * @dev Excludes an address from transactions from cooldowns.
	 * Can only be set by owner.
	 */
	function setIsExcludedFromTransactionlock(address excludeAddress) public onlyOwner(){
		_isExcludedFromTransactionlock[excludeAddress] = true;
	}

    /**
	 * @dev returns true/false address is excluded from transaction cooldown or not.
	 */
    ...
    function doAirdrop(address[] calldata recipients, uint256[] calldata amounts) public onlyOwner() {
        ...
        {
            if (_isExcluded[sender] && !_isExcluded[recipients[i]]) {
                _transferFromExcluded(sender, recipients[i], amounts[i]);
            } else if (!_isExcluded[sender] && _isExcluded[recipients[i]]) {
                _transferToExcluded(sender, recipients[i], amounts[i]);
            } else if (_isExcluded[sender] && _isExcluded[recipients[i]]) {
                _transferBothExcluded(sender, recipients[i], amounts[i]);
            } else {
                _transferStandard(sender, recipients[i], amounts[i]);
            }
        }
        restoreAllFee();
    }
    ...
    function removeAllFee() private {
        if(_totalLiquidityFee == 0 && _burnFee == 0 && _liquidityFee == 0
           && _marketingFee == 0 && _reflectionFee == 0 && _buyBackFee == 0) return;
    ...
    function swapAndLiquify(uint256 tokenBalance) private lockTheSwap {
        ...
        uint256 swapPercent = _marketingFee.add(_buyBackFee).add(_liquidityFee/2);
        uint256 swapTokens = tokenBalance.div(_totalLiquidityFee).mul(swapPercent);
        ...
        if(_marketingFee > 0)
        {
            marketingAmount = transferredBalance.mul(_marketingFee);
    ...
    /**  
     * @dev swap's exact amount of ETH for tokens if swapandliquify is enabled
     */
    ...
    function blacklistMultipleWallets(address[] calldata addresses) public onlyOwner(){
        require(addresses.length <= 800, "Can only blacklist max 800 addresses per transaction");
    ...
    function unBlacklistMultipleWallets(address[] calldata addresses) public onlyOwner(){
        require(addresses.length <= 800, "Can only unblacklist max 800 addresses per transaction");
        for (uint256 i; i < addresses.length; ++i) {
    ...
    /**  
     * @dev Buy back tokens and send those tokens to burn address
     * NOTE! if ownership is renounced then it will not work
     */
    function buyBackTokens(uint256 buyBackAmount) public onlyOwner()
    {
