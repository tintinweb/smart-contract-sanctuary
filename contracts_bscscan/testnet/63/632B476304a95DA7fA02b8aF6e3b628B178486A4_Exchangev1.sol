...
contract Exchangev1 is Ownable, Pausable {
    struct Item {
        address currency;
        uint256 price;
        bool listed;
    }
    mapping(uint256 => Item) public itmes;
    ...
    function setERC721TransferProxy(ERC721TransferProxy _transferProxy)
        public
        onlyOwner
    {
        transferProxy = _transferProxy;
    ...
    function listing(
        ...
        bool isItemListed = listedMap[_nftId];
        require(!isItemListed, "Error, item listing already");

        itmes[_nftId] = Item(_currency, _amount, true);
    }

    function buy(uint256 _id, uint256 _amount) external payable whenNotPaused {
        if (msg.value != 0) {
            _amount = msg.value;
        }
        ...
        _trade(_id, itmes[_id].currency, _amount);
    ...
    function _validate(uint256 _id, uint256 _amount) internal view {
        ...
        require(_amount >= itmes[_id].price, "Error, the amount is lower");
    ...
    function _trade(
        ...
        if (itmes[_id].currency == address(0)) {
    ...
    function updatePrice(uint256 _nftId, uint256 _price)
        ...
        returns (bool)
    {
        uint256 oldPrice = itmes[_nftId].price;
        require(
            _msgSender() == nft.ownerOf(_nftId),
            "Error, you are not the owner"
        );
        itmes[_nftId].price = _price;

        emit PriceUpdate(msg.sender, oldPrice, _price, _nftId);
        return true;
    }

    function updateListingStatus(uint256 _nftId, bool shouldBeListed)
        public
        whenNotPaused
        returns (bool)
    {
        ...
        itmes[_nftId].listed = shouldBeListed;

        emit NftListStatus(_msgSender(), _nftId, shouldBeListed);
