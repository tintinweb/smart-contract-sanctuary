...
contract BscsSealedAuction is ERC721Holder, Initializable, UUPSUpgradeable, OwnableUpgradeable{ 
    ...
    mapping(address => uint) public bidderToAmount;
    ...
    function placeBid(uint _tokenId, uint _amount) public payable {
        Auction storage auction = tokenIdToAuction[_tokenId];
        uint endAt = auction.startedAt + auction.duration;

        require(_amount >= auction.price, "placeBid: Price must bigger than minimum price");

        require(
            msg.sender != auction.seller,
            "placeBid: You are the onwer"
        );
        require(
            block.timestamp > auction.startedAt,
            "placeBid: Auction is not yet started"
        );
        require(block.timestamp < endAt, "placeBid: Auction is already ended");
        require(
        ...
        uint currentBid = bidderToAmount[msg.sender] + msg.value;
        ...
        if (currentBid > bidderToAmount[auction.highestBidder]) {
            //Set highest bidder
            ...
            auction.highestBindingBid = min(
                currentBid,
                bidderToAmount[auction.highestBidder] + bidIncrement
            );
        } else {
            //Set highest binding bid
            auction.highestBindingBid = min(
                currentBid + bidIncrement,
                bidderToAmount[auction.highestBidder]
            );
        }
        emit AuctionBided(
            auction.id,
    ...
    function finalizeAuction(uint _auctionId) public {
        ...
        require(
            auction.auctionState == State.Cancelled || block.timestamp > endAt,
            "Auction is not yet ended"
        );

        //The owner or bidder but he has bidded
        require(
            msg.sender == auction.seller || bidderToAmount[msg.sender] > 0,
        ...
        if (auction.auctionState == State.Cancelled) {
            ...
            value = bidderToAmount[msg.sender];
        } else {
            /* Auction is ended */
