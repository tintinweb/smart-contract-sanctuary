...
contract Lol is Context, IERC20, Ownable {
    ...
    string private _name = "SafeDeiMudda";
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        //take Ceo fee
        if(_ceoFee != 0){
            uint256 ceoFee = amount.mul(_ceoFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(ceoFee);
            _reflectionTotal = _reflectionTotal.sub(ceoFee.mul(rate));
            if (_isExcluded[ceoWallet]) {
                _tokenBalance[ceoWallet] = _tokenBalance[ceoWallet].add(ceoFee);
            }
            _ceoFeeTotal = _ceoFeeTotal.add(ceoFee);
            emit Transfer(account,ceoWallet,ceoFee);
        }
    ...
    function setCeoFee(uint256 fee) public onlyOwner {
        _ceoFee = fee;
    }
    ...
    function setCharityWallet(address wallet) public onlyOwner {
        charityWallet = wallet;
    }
   
    function setCeoWallet(address wallet) public onlyOwner {
        ceoWallet = wallet;
    }
