...
contract LCRtoken is ERC20 {
    ...
    constructor() ERC20("Lucrar", "LCR")
    {
		...
		//intervalBlocksYear = 10512000; // 1 year of blocks (1 each 3 seconds)
		intervalBlocksYear = 12 * 60 / 3; // testes 12 minutos
	...
	function getBnbBalance(address endereco) public view returns (uint256 value)
	{	
	    
	    return address(endereco).balance; // BNB balance
	}
	
	function getLcrBalance(address endereco) public view returns (uint256 value)
	{	
	...
	function releaseFunds() public onlyOwner
	{
		...
		for (uint i= 0; i < 80; i++)
		{
			if (block.number >= releasePeriods[i] && block.number < releasePeriods[i+1]) 
			{
				...
				releaseDone[i] = true;	
				transferFrom(address(this), lcrFundDeposit, valueToRelease);
			}
		}
	}
	...
	{	
		...
		emit CreateLCR(address(this), value);  // logs money creation
	...
	function getFakeStakeInfo(address account) public view returns (uint256 stakedBalance, uint256 interest, uint256 timeToWait)
	{		
	   
	    //require (stakeBalances[account] > 0, "zero stake balance");
