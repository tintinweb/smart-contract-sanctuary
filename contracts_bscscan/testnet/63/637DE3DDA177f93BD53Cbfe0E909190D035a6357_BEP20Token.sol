...
// 	address payable public owner;
  ...
  function _msgSender() internal view returns (address payable) {
	...
	//contructor function
	function Lotteryy(uint _winningGuess) public {
  ...
  function makeUser() public{
    ...
    users[_msgSender()].tokensBought = 0;
  ...
  // money can be released using selfdestruct(address)
    ...
    if (present == 1) {
      users[_msgSender()].tokensBought += tokensToAdd;
    }
	}

	// to add user guesses
	function makeGuess(uint _userGuess) public {
    require(_userGuess < 1000000 && users[_msgSender()].tokensBought > 0);
    users[_msgSender()].guess.push(_userGuess);
    users[_msgSender()].tokensBought--;
	}

	// doesn't allow anyone to buy anymore tokens
	function closeGame() view public returns(address payable)  {
    // can only be called my the owner of the contract
		// require(owner == 0xdA14809A1641D30187177e5Cd1946c18c9AfA142);
	...
	function getPrice() public onlyOwner returns (uint){
    address payable winner = winnerAddress();
    if (winner == owner()) {
