...
contract CandleGenieCoinFlip is Ownable, ReentrancyGuard 
{
    ...
    function Flip(int position) external payable nonReentrant notContract 
    {
        ...
        {
            require(block.timestamp > Bets[UserBets[msg.sender][UserBets[msg.sender].length - 1]].timestamp + bruteTimeout, "Wait brute time delay to place a new bet. No brute plis");  
        }
