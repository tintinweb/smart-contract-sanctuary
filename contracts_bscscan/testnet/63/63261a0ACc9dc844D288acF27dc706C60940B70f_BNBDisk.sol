...
contract BNBDisk is Ownable, ReentrancyGuard {
    ...
    modifier tokenOwned(uint256 _tokenId) {
        ...
        require(
            tokenInfo.isUnlimitedSale || tokenInfo.tokenCount > 0,
            "No token"
        );
        _;
    }
    ...
    ) external nonReentrant returns (uint256)
    {
        ...
        emit MintNewToken(newItemId, _msgSender(), _count, _price, _dbId);
        return newItemId;
    }
    ...
    // Get URI of tokenId
    function tokenDBId(uint256 _tokenId) public view  returns (uint256) {
        require(_exists(_tokenId), "Invalid token Id");
        return tokenList[_tokenId].dbId;
    }

    function tokenPrice(
        uint256 _tokenId
    ) public view tokenOwned(_tokenId) returns (uint256) {
        require(_exists(_tokenId), "Invalid token ID");
    ...
    function tokenCount(
        uint256 _tokenId
    ) public view tokenOwned(_tokenId) returns (uint256 , bool) {
        TokenInfo storage tokenInfo = tokenList[_tokenId];
    ...
    function tokenInfoOf(uint256 _tokenId) external view returns(TokenInfo memory) {
        require(_exists(_tokenId), "Invalid token ID");
        TokenInfo memory ret = tokenList[_tokenId];
        return ret;
    }
    ...
    function buyNFT(
        uint256 _tokenId
    ) public payable nonReentrant tokenOwned(_tokenId) {
        ...
        require(tokenInfo.tokenOwner != _msgSender(), "buyer should not be owner");
        ...
        (bool sent, ) = payable(tokenInfo.tokenOwner).call{value: soldValue}("");
        require(sent, "Failed to send payment");

        tokenList[_tokenId] = tokenInfo;
        isTokenBought[_tokenId][_msgSender()] = true;
        emit BuyToken(_tokenId, tokenInfo.tokenOwner, _msgSender(), tokenInfo.tokenPrice);
    }
    ...
    function setTokenPrice(
        uint256 _tokenId,
        uint256 _newPrice
    ) public tokenOwned(_tokenId) nonReentrant {
        require(_msgSender() != address(0), "null address");
        require(_exists(_tokenId), "token not exist");

        address tokenOwner = tokenList[_tokenId].tokenOwner;
        require(tokenOwner == _msgSender(), "Only owner can set price");
