...
contract FeeController is AccessControl, IFeeController {
    ...
    constructor(IBookkeeper bookkeeper_) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        updateAddresses(bookkeeper_);
        // default rate = 5%
        tiers.push(Tier({minVolume: 0, rate: defaultFeeRate}));
    }
    ...
    function updateAddresses(IBookkeeper bookkeeper_) public _onlyAdmin {
        bookkeeper = bookkeeper_;
    }

    function updateSettings(uint256 windowSize_, Tier[] calldata tiers_)
        public
        _onlyAdmin
    {
    ...
    function getUserVolume(address user) public view returns (uint256) {
        ...
        uint256 endDate = (volumeWindowSize > startDate)
            ? 0
            : startDate - volumeWindowSize;
