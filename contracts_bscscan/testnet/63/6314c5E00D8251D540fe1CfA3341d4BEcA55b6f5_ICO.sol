...
contract ICO is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint32 public UNLOCK_START_TIME;
    // whitelist end time
    uint32 public whitelistEndTime;
    ...
    modifier whenIcoCompleted {
        require(block.timestamp > icoEndTime || tokenRaised >= FUNDING_GOAL, "TOKENSALE: NOT_COMPLETE_YET");
        _;
    }

    modifier whenSaleNotPause {
        require(!isPauseSaleWithCrypto, "TOKENSALE: PAUSE");
        _;
    }
    ...
    /// @param _icoStartTime use for set sale start time
    /// @param _icoEndTime use for set sale end time
    /// @param _tokenAddress address of token and will set as token sale instance
    ...
    function initialize(
        ...
        tokenLocker = ITokenLocker(_tokenLockerAddress);

        FUNDING_GOAL = _fundingGoal;        

        UNLOCK_START_TIME = _unlockStartTime;
    ...
    /// @dev Use this for extend or change sale period but not more than UNLOCK_START_TIME for correct calculated claim token
    /// @param _icoStartTime start time of this sale
    /// @param _icoEndTime end time of this sale not more than UNLOCK_START_TIME
    function setSalePeriod(uint32 _icoStartTime, uint32 _icoEndTime) external onlyRole(CONFIG_ROLE) {  
        require(_icoStartTime != 0, "TOKENSALE: INPUT_ZERO_AMOUNT");
        require(_icoEndTime != 0, "TOKENSALE: INPUT_ZERO_AMOUNT");
        require(_icoStartTime < _icoEndTime, "TOKENSALE: CONFIG_INVALID_SALE_TIME");
        // can not buy after claimed period start
        require(_icoEndTime < UNLOCK_START_TIME, "TOKENSALE: CONFIG_INVALID_SALE_ENDTIME");
        // set sale period
        icoStartTime = _icoStartTime;
        icoEndTime = _icoEndTime;
    }
    ...
    /// @dev Start whitelist time will equal icoStartTime
    /// If not have whitelist period just set _whitelistEndTime equal icoStartTime
    /// @param _whitelistEndTime end time of this whitelist period
    function setWhitelistPeriod(uint32 _whitelistEndTime) external onlyRole(CONFIG_ROLE) {
        require(_whitelistEndTime >= icoStartTime && _whitelistEndTime <= icoEndTime, "TOKENSALE: CONFIG_INVALID_WHITELIST_ENDTIME");        
        // set whitelist period
        // if set whitelistEndTime == icoStartTime mean no whitelist
    ...
    function isWhitelistPeriod() public view returns (bool) {
        return whitelistEndTime != icoStartTime && block.timestamp >= icoStartTime && block.timestamp <= whitelistEndTime;
    }

    /// @notice Check if this sale complete
    function isIcoCompleted() external view returns (bool) {
        return block.timestamp > icoEndTime || tokenRaised >= FUNDING_GOAL;
    }
    ...
    /// @notice Pause sale only crypto channel
    /// @dev This will pause only buy and buy with token not pause buy with distributor
    /// @param _isPauseSaleWithCrypto boolean for indicate this sale with crypto have pause or not
    ...
    /// @notice Pause sale only distributor channel
    /// @dev This will pause only distributor channel but not on buy with crypto
    /// @param _isPauseSaleWithDistributor boolean for indicate this sale with distributo have pause or not
    ...
    function buy() public payable nonReentrant whenSaleNotPause {
        // check ico time
        require(block.timestamp >= icoStartTime && block.timestamp <= icoEndTime, "TOKENSALE: END_TIME");                   
        // check raise cap
        ...
        // check min max
        require(msg.value >= etherBuyData.min, "TOKENSALE: AMOUNT_LEES_THAN_MIN");
        require(msg.value <= etherBuyData.max, "TOKENSALE: AMOUNT_MORE_THAN_MAX");
        ...
        tokensToReceived = etherUsed * tokenReceivedUnit / etherUnit * etherBuyData.rate / RATE_DECIMALS;
        ...
        if (tokenRaised + tokensToReceived > FUNDING_GOAL) {
            ...
            etherExceed = tokensToReceivedExceed * etherUnit / tokenReceivedUnit * RATE_DECIMALS / etherBuyData.rate;
    ...
    function buyTokenWithDistributor(address _receiver, uint256 _amount, string memory _reference) public nonReentrant onlyRole(DISTRIBUTOR_ROLE) whenSaleWithDistributorNotPause {    
        ...
        require(tokenRaised < FUNDING_GOAL, "TOKENSALE: CAP_REACH");
         // cannot buy more than funding goal
