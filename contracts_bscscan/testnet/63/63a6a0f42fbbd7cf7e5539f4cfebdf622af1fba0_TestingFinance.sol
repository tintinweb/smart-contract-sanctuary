...
contract TestingFinance {
        ...
        function withdrawEarnings() public 
		{
			...
			player[msg.sender].checkpoint = player[msg.sender].checkpoint.add(1 days);
        ...
        function poolIncome(address _addr) view external returns(uint256, uint256, uint256) 
		{
		    ...
		    uint256 poolAIncome = pool_a_balance.mul(poolAShare.mul(100).div(pool_a_investment)).div(100).div(10).mul((block.timestamp.sub(player[_addr].checkpoint))/1 days);
			uint256 poolBIncome = pool_b_balance.mul(poolBShare.mul(100).div(pool_b_investment)).div(100).div(10).mul((block.timestamp.sub(player[_addr].checkpoint))/1 days);
			uint256 poolCIncome = pool_c_balance.mul(poolCShare.mul(100).div(pool_c_investment)).div(100).div(10).mul((block.timestamp.sub(player[_addr].checkpoint))/1 days);
