...
contract SampleNFT is ERC721, ERC721URIStorage, ContractURI, IMAX721, ReentrancyGuard, ERC165Storage, DeveloperV2, Ownable {
 ...
 */
  ...
  function publicMint(uint amount) public nonReentrant() {
    ...
    ERC20Address.transferFrom(msg.sender, DEAD_ADDRESS, tokenAmount);
  ...
  //  stated above. Threshold is set by onlyDev!
  function claimNFT() public nonReentrant() {
    // @notice using Checks-Effects-Interactions
    // @notice Checks Phase
    require(enableMinter, "Minter not active");
    require(ERC20Address.balanceOf(msg.sender) >= thresholdAmount, "Do not hold enough ERC20 tokens to claim.");
    require(_tokenIdCounter.current() < mintSize, "Can not mint that many");
    require(!hasClaimed[msg.sender], "Can not claim a second time");
    // @notice Effects Phase
 ...
 */
  ...
  function setERC20Address(address _ERC20Address) public onlyDev {
  ...
  // @param uint _amount - number of available metadatas
  // ERC165 datum setTotalChoices(uint256) => 0xf727ca49
  function setTotalChoices(uint _number) public onlyDev {
    uint old = totalChoices;
  ...
  function setMintSize(uint _amount) public onlyDev {
    uint old = mintSize;
    mintSize = _amount;
  ...
  function sweepERCToAddress(address _user, uint _amount, address _token) public onlyDev {
  ...
  function ERC20ERC20Address() public view returns (address) {
  ...
  function ERC20ThresholdAmountForClaimNFT() public view returns (uint) {
