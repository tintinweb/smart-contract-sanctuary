...
contract UnicornFarm is Ownable,IERC721Receiver {
    ...
    mapping(uint=>PawnLib.Pindex) public pindexes;
    ...
    function unstake(uint tokenId) public {
        ...
        (uint pid,uint pindex) = (pindexes[tokenId].pid, pindexes[tokenId].pindex);
        
        PawnLib.Pawn memory pawn = pawns.remove(pid,pindex,msg.sender,pindexes);
    ...
    function withdrawAll() public {
        for(uint pid; pid<poolLength(); pid++) {
            for(uint j;j<getLength(msg.sender,pid);j++) {
                PawnLib.Pawn memory pawn = pawns[pid][msg.sender][j];
    ...
    function getAllPawns(address account) public view returns(PawnLib.Pawn[] memory _pawns) {
        _pawns = new PawnLib.Pawn[](100);
         uint count;
         for(uint pid; pid<poolLength(); pid++) {
            for(uint j;j<getLength(account,pid);j++) {
                if(count>=100) break;
                _pawns[count] = pawns[pid][account][j];
                count++;
            }
        }
...
library PawnLib {
    ...
    function add(mapping(uint=>mapping(address=>Pawn[])) storage pawns,uint pid,address account,Pawn memory pawn,mapping(uint=>PawnLib.Pindex) storage pindexes) internal returns(uint pindex) {
        pindex = pawns[pid][account].length;
        pawns[pid][account].push(pawn);
        
        pindexes[pawn.tokenId] = Pindex({
            pid: pid,
            pindex: pindex,
            active: true
        });
    ...
    function remove(mapping(uint=>mapping(address=>Pawn[])) storage pawns,uint pid,uint pindex,address account,mapping(uint=>PawnLib.Pindex) storage pindexes) internal returns(Pawn memory pawn) {
        ...
        if(pindex != lastIndex) {
            ...
            Pawn memory lastPawn = pawns[pid][account][lastIndex];
            
            pawns[pid][account][pindex] = lastPawn;
            
            pindexes[lastPawn.tokenId] = Pindex({
                pid: pid,
                pindex: pindex,
                active: true
            });
