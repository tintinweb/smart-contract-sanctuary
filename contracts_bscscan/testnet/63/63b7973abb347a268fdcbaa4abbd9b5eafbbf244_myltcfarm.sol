...
VIP PLAN:
 ...
 User will need to have a total amount of 1.5 LTC in other plans to qualify.
...
contract myltcfarm {
	...
	uint256 public INVEST_MAX_AMOUNT = 5 ether; // 5 ltc
	...
	function invest(address referrer, uint8 plan, uint256 amounterc
        ...
        //check if user wants to invest in VIP Plan, but also check if he is qualified(should have total deposit of 1.5 LTC or higher)
        if(plans[plan].time == 5 && getUserTotalDeposits(msg.sender) >= 1.5 ether){
            isVIP = true;
        } else revert("User not qualified for VIP Plan. Total deposit must be equal or greater than 1.5 LTC");
        
        token_LTC.transferFrom(address(msg.sender), address(this), amounterc);
        ...
        emit FeePayed(msg.sender, SafeMath.div(fee,ltc));
	...
	function withdraw() public {
		...
		// withdraw tax. 20% to keep in contract. 5% to marketing
	    uint256 marketing = totalAmount.mul(MARKETING_FEE).div(PERCENTS_DIVIDER);
	    uint256 ltcMiner = totalAmount.mul(LTC_CONTRACT_DEPOSIT).div(PERCENTS_DIVIDER);
	    uint256 contractLongevity = totalAmount.mul(CONTRACT_LONGEVITY_FEE).div(PERCENTS_DIVIDER);
	    totalAmount = totalAmount.sub(marketing).sub(ltcMiner).sub(contractLongevity);
        ...
        if(totalAmount > MAX_WITHDRAW) {// user can only withdraw max of 10 LTC   
