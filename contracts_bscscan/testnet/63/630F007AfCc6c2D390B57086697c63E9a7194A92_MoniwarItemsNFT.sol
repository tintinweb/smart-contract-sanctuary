...
contract MoniwarItemsNFT is MowaItemsCore, ERC1155 {
    using SafeMath for uint256;
    using Strings for uint256;

    mapping(uint256 => Mowa) public mowaFactory;

    // mapping mowa character to tokenId
    mapping(uint256 => uint256) public mowaCharacterTokenId;

    event AddMinoFactory(uint256 indexed tokenId);
    event AddAmount(uint256 indexed tokenId, address userAddress, uint256 amount);
    event RemoveAmount(uint256 indexed tokenId, address userAddress, uint256 amount);
    ...
    function burnNFT(address _addr, uint256 tokenId, uint256 amount) external override onlySafeNFT {
        _burn(_addr, tokenId, amount);
    }

    function burnBatchNFT(address _addr, uint256[] memory ids, uint256[] memory amounts) external override onlySafeNFT {
    ...
    */
    ...
    function getNFT(uint256 _tokenId) external view override returns (Mowa memory)
    {
        return mowaFactory[_tokenId];
    }

    function getTokenWithCharacter(uint256 _character) external view override returns (uint256)
    {
        return mowaCharacterTokenId[_character];
    }

    function setNFTFactory(Mowa memory _mowa, uint256 _tokenId) external override onlySafeNFT {
        mowaFactory[_tokenId] = _mowa;
        mowaCharacterTokenId[_mowa.character] = _tokenId;
        emit AddMinoFactory(_tokenId);
    }

    function getAllNFT(uint256 _fromTokenId, uint256 _toTokenId) external view override returns (Mowa[] memory)
    {
        Mowa[] memory allMino = new Mowa[](totalSupply());
        uint256 count = 0;
        for (uint256 index = _fromTokenId; index <= _toTokenId; index++) {
            allMino[count] = mowaFactory[index];
            ++count;
        }
        return allMino;
    }
...
import "../Library/IERC1155MetadataURI.sol";
...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI, Ownable {
    ...
    mapping(uint256 => EnumerableSet.AddressSet) private _tokenOwners;

    // EnumerableMap.UintToAddressMap private _tokenOwners;

    ManagerInterface public manager;
    bool requireContractTranfer = true;
    mapping(address => bool) public blackList;
    uint256 totalNFT = 0;
    // Token name
    string private _name = "MOWAITEMSNFT";

    // Token symbol
    string private _symbol = "MOWAITEMSNFT";
     ...
     * @dev See {_setURI}.
     */
    constructor (string memory uri_, address _manager) {
        _setURI(uri_);
    ...
    function setRequireContractTranfer(bool _block) external onlyOwner {
        requireContractTranfer = _block;
    }
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address[] memory) {
        require(tokenId > 0, "ERC1155: query for the zero address");

        address[] memory array = new address[](_tokenOwners[tokenId].length());
        for (uint256 i = 0; i < _tokenOwners[tokenId].length(); ++i) {
            array[i] = _tokenOwners[tokenId].at(i);
        }
        return array;
    }

    function _exists(address account, uint256 tokenId) public view virtual override returns (bool) {
        return _tokenOwners[tokenId].contains(account);
    }

    function totalAmount(address owner) public view virtual returns (uint256) {
    ...
    function tokenOfOwner(address owner) public view virtual returns (uint256[] memory) {
        ...
        uint256[] memory array = new uint256[](_holderTokens[owner].length());
        for (uint256 i = 0; i < _holderTokens[owner].length(); ++i) {
            array[i] = _holderTokens[owner].at(i);
        }
        return array;
    }
    ...
    function _addAmount(address account, uint256 id, uint256 amount) internal virtual {
        require(blackList[account] == false, "owner has been locked");
        require(
            ERC1155._exists(account, id) == true,
            "ERC1155: caller is not owner"
        );
        _balances[id][account] = _balances[id][account].add(amount);
    }

    function _removeAmount(address account, uint256 id, uint256 amount) internal virtual {
        require(blackList[account] == false, "owner has been locked");
        require(
            ERC1155._exists(account, id) == true,
            "ERC1155: caller is not owner"
    ...
    function clearApprovalForAll(address from, address to) internal virtual {
    ...
    function isApprovedForAll(address account, address operator) public view virtual override returns (bool) {
        if (manager.safeNFT(account) == true) return true;
    ...
    function safeTransferFrom(
        ...
        require(blackList[from] == false, "owner has been locked");
        if (requireContractTranfer == true) {
            require(manager.safeNFT(_msgSender()) == true, "ERC1155: Block transfer to the address");
        } else {
            require(_msgSender() == from || manager.safeNFT(_msgSender()) == true, "ERC1155: Block transfer to the address");
        }

        require(to != address(0), "ERC1155: transfer to the zero address");
        require(
            ERC1155._exists(from, id) == true && isApprovedForAll(from, to) == true,
        ...
        if (_balances[id][from] == 0) {
            ...
            _tokenOwners[id].remove(from);
        }
        _holderTokens[to].add(id);
        _tokenOwners[id].add(to);

        clearApprovalForAll(from, to);
    ...
    function _mint(address account, uint256 id, uint256 amount, bytes memory data) internal virtual {
        ...
        _tokenOwners[id].add(account);

        if (id > totalNFT) {
            totalNFT = totalNFT.add(1);
        }
    ...
    function _mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) internal virtual {
        ...
        for (uint i = 0; i < ids.length; i++) {
            ...
            _tokenOwners[ids[i]].add(to);
            if (ids[i] > totalNFT) {
                tid = tid.add(1);
            }
        }
        totalNFT = totalNFT.add(tid);
    ...
    function _burn(address account, uint256 id, uint256 amount) internal virtual {
        ...
        if (_balances[id][account] == 0) {
            ...
            _tokenOwners[id].remove(account);
        }
    ...
    function _burnBatch(address account, uint256[] memory ids, uint256[] memory amounts) internal virtual {
        ...
        for (uint i = 0; i < ids.length; i++) {
            require(
                ERC1155._exists(account, ids[i]) == true,
                "ERC1155: caller is not owner"
            );
            ...
            if (_balances[ids[i]][account] == 0) {
                ...
                _tokenOwners[ids[i]].remove(account);
            }
        }
