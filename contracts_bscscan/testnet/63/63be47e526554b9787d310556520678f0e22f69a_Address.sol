...
contract PigToken is Context, IERC20, Ownable {
    ...
    address public immutable presetAddress = address(0x0caF696ea97f706290f6f7B9d3303CE3aBAe2018);
    
    address public immutable burnAddress = address(0x000000000000000000000000000000000000dEaD);
    
    uint256 public _presetFee = 5;
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        tTransferAmount = tTransferAmount.sub(tLiquidity).sub(tPreset).sub(tBurn);

        
        return (tTransferAmount, tFee, tLiquidity, tPreset, tBurn);
    }
    ...
    function _getRate() private view returns(uint256) {
        ...
        // 流动供应总量 除以 最大发行量 取整 <= tSupply
        // 默认rSupply.div(tSupply) = 0
        // 假设交易后 1000 100
        // 990 100 = 9
    ...
    function _takePreset(uint256 tPreset) private {
        uint256 currentRate =  _getRate();
        uint256 rPreset = tPreset.mul(currentRate);
        
        _rOwned[presetAddress] = _rOwned[presetAddress].add(rPreset);
        if(_isExcluded[presetAddress])
            _tOwned[presetAddress] = _tOwned[presetAddress].add(tPreset);
    }
  

    function _checkIsOverdue1000(uint256 amount) private view returns (bool) {
        uint256 remainSupply = _tTotal.sub(burned).div(1000);
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        ...
        if(_checkIsOverdue1000(_amount)) {
            ...
            return _amount.mul(_taxFee).mul(2).div(
                10**2
            );
        }
    ...
    function calculateBurn(uint256 _amount) private view returns (uint256) {
        ...
        if(_checkIsOverdue1000(_amount)) {
            ...
            return _amount.mul(_burnFee).mul(2).div(
                10**2
            );
        }
        return 0;
    ...
    function _transfer(
        ...
        if(_burned > 0) {
            ...
            _transfer(from, burnAddress, _burned);
        }
    }
