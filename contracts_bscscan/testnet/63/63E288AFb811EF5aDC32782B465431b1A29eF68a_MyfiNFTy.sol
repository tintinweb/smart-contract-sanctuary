...
contract MyfiNFTy is Context, IBEP20, Ownable {
    ...
    uint256 _walletTransferFee = 900; // 90% by default

    uint256 public maxTxAmount = _totalSupply.div(1000); // should be 0.1% percent per transaction
    uint256 public minTokenNumberToSell = _totalSupply.div(100000); // 0.001% tx amount will trigger swap and add liquidity
    uint256 public coolDownMultiplier = 200; // tax fee will be 200% during cooldown
    uint256 public coolDownTriggeredAt;
    uint256 public coolDownDuration = 30 minutes;
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "$Myfi: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "$Myfi: decreased allowance below zero"
            )
        );
        return true;
    }
    ...
    function _approve(
        ...
        require(owner != address(0), "$Myfi: approve from the zero address");
        require(spender != address(0), "$Myfi: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "$Myfi: transfer from the zero address");
        require(to != address(0), "$Myfi: transfer to the zero address");
        require(amount > 0, "$Myfi: Transfer amount must be greater than zero");
        ...
        if (
            ...
            require(
                amount <= maxTxAmount,
                "$Myfi: transfer amount exceeds the maxTxAmount."
            );

            if (!tradingOpen) {
                require(
                    to != pancakePair && from != pancakePair,
                    "$Myfi: trading is not enabled yet"
                );
            }
        }
    ...
    function _tokenTransfer(
        ...
        if (recipient == pancakePair && takeFee) {
            if (coolDownTriggeredAt.add(coolDownDuration) > block.timestamp) {
                _fee = amount.mul(getSellFee()).div(divider);
                coolDownTriggeredAt = block.timestamp;
            } else {
                _fee = amount.mul(_totalFeePerTx).div(divider);
            }
        ...
        else if (sender != pancakePair && recipient != pancakePair && takeFee) {
            _fee = amount.mul(_walletTransferFee).div(divider);
            _balances[sender] = _balances[sender].sub(amount);
            _balances[recipient] = _balances[recipient].add(amount.sub(_fee));
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            uint256 liquidtyPercent = _liquidityFee
                .mul(1e4)
                .div(_totalFeePerTx)
                .div(2);
            ...
            uint256 otherPiece = contractTokenBalance.mul(liquidtyPercent).div(
                1e4
            );
            ...
            uint256 totalPercent = liquidtyPercent.add(marketPercent).add(
                payoutPercent
            );
            uint256 balanceBnb = address(this).balance;

            uint256 bnbToBeAddedToLiquidity = balanceBnb
                .mul(liquidtyPercent)
                .div(totalPercent);
            ...
            payoutWallet.transfer(
                balanceBnb.mul(payoutPercent).div(totalPercent)
            );
