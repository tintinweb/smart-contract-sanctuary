...
contract Purchase is Ownable {
    ...
    function removeArr(address _user, uint submitLengthIndex) internal {
        userSells[_user].submits[userSells[_user].submitLength[submitLengthIndex]] = submit(IBEP20(0), 0, address(0), 0);
    ...
    function buys(address[] memory _uses, uint[][] memory submitLengthIndexs) public onlyOwner {
        ...
        for(uint i = 0; i < _uses.length; i++) {
            for(uint j = 0; j < submitLengthIndexs[i].length; j++) {
                ...
                userSells[_uses[i]].submits[userSells[_uses[i]].submitLength[submitLengthIndexs[i][j]]] = submit(IBEP20(0), 0, address(0), 0);
    ...
    function cancelSells(uint[] memory indexs) public {
        for(uint i = 0; i < indexs.length; i++) {
            ...
            userSells[msg.sender].submits[userSells[msg.sender].submitLength[indexs[i]]] = submit(IBEP20(0), 0, address(0), 0);
    ...
    function sell(IBEP20 _token, uint _amount, uint _code) public {
        ...
        if(getRemainingToken(BUSD) >= busdAmount) {
            ...
            submits[_msgSender()][_code] = submit(_token, _amount, _msgSender(), 2);
            ...
            submits[_msgSender()][_code] = submit(_token, _amount, _msgSender(), 1);
            userSells[msg.sender].submits[block.number] = submits[_msgSender()][_code];
