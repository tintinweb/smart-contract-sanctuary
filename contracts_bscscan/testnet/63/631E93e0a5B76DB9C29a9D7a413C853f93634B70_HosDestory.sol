...
contract HosDestory is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    address private constant uniswapRouterAddress = address(0x6DF7a80aef9210Fe65978100273D2AA5e9CF8a83);
    address public profitAddress = address(0xd3C6F0f03094E930b387d71698020b94a1b55EFA);
    address public usdtAddress = address(0x337610d27c682E347C9cD60BD4b3b107C9d34dDd);
    address public usdtHosLpAddress = address(0x53C9dAB10dc2E88a740df94e79b1Ad85CA0AF779);
    ...
    address public constant rootAddress = address(0x6B0D437509E3E46353c9a61ed8464b3aA043C263);
    ...
    uint256 public extraprice = 1000 * 10 ** 9; 
    ...
    constructor(
        ...
        address(0xCb2e89385c5804bf2bB0C6EF924130464Ca0cABb)
        ];
        for (uint256 index = 0; index < ads.length; index++) {
           
            setDesToken(ads[index], 0);
        }
    ...
    function earned(address account) public view returns (uint256) {
        uint256 maxHos = powerMap[account].originalPower.mul(2);
        uint256 lastHos = maxHos.sub(powerMap[account].receivedProfit);
        if(powerMap[account].ljProfit<lastHos){
    ...
    function usdtHos() public view returns (uint256,uint256){
        (uint256 reserves0,uint256 reserves1,) = IUniswapV2Pair(usdtHosLpAddress).getReserves();
        return (reserves0,reserves1);
    }

     function getUserHos() public view returns (uint256, uint256, uint256, uint256){
        require(address(msg.sender) == address(tx.origin), "no contract");
        uint256 maxHos = powerMap[msg.sender].originalPower.mul(2);
        uint256 lastHos = maxHos.sub(powerMap[msg.sender].receivedProfit);
        return (powerMap[msg.sender].originalPower,maxHos,powerMap[msg.sender].receivedProfit,lastHos);
    }


    function getTotalHos(address desAddress) public view returns (uint256){
        require(address(msg.sender) == address(tx.origin), "no contract");
        return stakeTokenMap[desAddress].total;
    }


     function hosToken(address desAddress, uint256 addPower,uint256 desamount,uint256 usdtamount) public  isBindIntro changeAverage(msg.sender) {
        require(_isDIS, "is disable");
        require(desamount > stakeTokenMap[desAddress].minAmount, "less token");
        IERC20(usdtAddress).safeTransferFrom(address(msg.sender), address(this), usdtamount);
        IERC20(desAddress).safeTransferFrom(address(msg.sender),burnAddress , desamount);
        ...
        emit Pledge(msg.sender, desAddress, addPower, block.timestamp);
    }
    ...
    function extraProfit(address userAddress, uint256 amount, uint256 orderNo) public  onlyProfit {
        ...
        powerMap[userAddress].receivedProfit = powerMap[userAddress].receivedProfit.add(amount);
    }
    ...
    function setDesToken(address tokenAddress, uint256 _minAmount) public onlyOwner() {
