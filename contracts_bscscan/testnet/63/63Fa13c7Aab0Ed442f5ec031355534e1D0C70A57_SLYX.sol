...
abstract contract SLYXBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	// MAIN TOKEN PROPERTIES
	string private constant NAME = "Slyx Coin";
	string private constant SYMBOL = "SLYX";
	...
	uint256 private constant _totalTokens = 1000000000000000 * 10**DECIMALS;	//total supply
    ...
    address private _devAddress = 0x53Ea97382D1C69fe011567432f8c5B9513b3AB82;
	...
	constructor (address routerAddress) {
		...
		// Initialize PancakeSwap V2 router and SLYX <-> BNB pair.
		setPancakeSwapRouter(routerAddress);

		_maxHoldAmount = 10000000000000 * 10**DECIMALS;

		// 3% liquidity fee, 0% reward fee, 3% marketing fee, 3% dev fee, 3% buyback fee
		setFees(3, 0, 3, 3, 3);
		_highBuyFee = 99;
	...
	function activate() public onlyOwner {
		...
		setTransactionSellLimit(100000000000000 * 10**DECIMALS);
		setTransactionBuyLimit(100000000000000 * 10**DECIMALS);
		activateBuying(true);
		onActivated();
	}
	...
	function executeSwap(uint256 amount) private {
		...
		uint256 tokensReservedForReward = amount - tokensReservedForLiquidity - tokensReservedForMarketing - tokensReservedForDev - tokensReservedForBuyback;
		...
		uint256 tokensToSwap = tokensReservedForReward + tokensToSwapForLiquidity + tokensReservedForMarketing + tokensReservedForDev + tokensReservedForBuyback;
	...
	// This function swaps a {tokenAmount} of SLYX tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for SLYX -> WBNB
	...
	// Returns how many more $SLYX tokens are needed in the contract before triggering a swap
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 marketingFee, uint8 devFee, uint8 buybackFee) public onlyOwner {
		require(liquidityFee >= 0 && liquidityFee <= 5, "Liquidity fee must be between 0% and 5%");
		require(rewardFee >= 0 && rewardFee <= 10, "Reward fee must be between 0% and 15%");
		...
		// Enforce invariant
		_poolFee = _rewardFee + _liquidityFee + _marketingFee + _devFee + _buybackFee;
	}
...
contract SLYX is SLYXBase {
	...
	uint256 private _totalBNBAsSLYXClaimed; // The total number of BNB that was converted to SLYX and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsSLYXClaimed; // The amount of BNB converted to SLYX and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy SLYX automatically
	...
	bool private _reimburseAfterSLYXClaimFailure; // If true, and SLYX reward claim portion fails, the portion will be given as BNB instead
	...
	//anti-bot
	uint256 public antiBlockNum = 3600;
	...
	constructor (address routerAddress) SLYXBase(routerAddress) {
	...
	function onActivated() internal override {
		...
		setReimburseAfterSLYXClaimFailure(true);
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough SLYX balance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim SLYX tokens
		if (!claimSLYX(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterSLYXClaimFailure) {
	...
	function claimSLYX(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsSLYXClaimed[user] += bnbAmount;
		_totalBNBAsSLYXClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $SLYX on a zero balance address:
		...
		// Depending on the % of $SLYX tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsSLYX(address byAddress) public view returns (uint256) {
		return _bnbAsSLYXClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsSLYX() public view returns (uint256) {
		return _totalBNBAsSLYXClaimed;
	}
	...
	function reimburseAfterSLYXClaimFailure() public view returns(bool) {
		return _reimburseAfterSLYXClaimFailure;
	}


	function setReimburseAfterSLYXClaimFailure(bool value) public onlyOwner {
		_reimburseAfterSLYXClaimFailure = value;
	}
