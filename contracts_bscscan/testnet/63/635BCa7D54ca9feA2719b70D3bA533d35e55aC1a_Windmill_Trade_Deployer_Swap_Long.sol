...
abstract contract Windmill_Trade_Deployer_Abstract is Adressable, AccessControlEnumerable, With_DAORole, With_BUSDToken, I_Windmill_Trade_Deployer_Abstract{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
	...
	function setTradeCode(bytes memory _bytecode, string memory _name) external onlyRole(DAO_ROLE){
        ...
        tradeTemplate = _deployNewTrade();
    }
	
	function deployNewTrade(address owner, address manager, address fund) public onlyRole(DAO_ROLE) returns (I_Windmill_Trade_Abstract){
        I_Windmill_Trade_Abstract trade =  I_Windmill_Trade_Abstract(_deployNewTrade());
        _initTrade(trade, owner, manager, fund);
    ...
    function _initTrade(I_Windmill_Trade_Abstract trade, address owner, address manager, address fund) internal virtual{
        trade.setBUSDToken(BUSDToken);
        trade.initTrade(owner, manager, fund);
    }
}// Version: 1.0.0
...
interface I_Windmill_Trade_Swap_Long is I_Windmill_Trade_Abstract{
	function setAuthorizedTokens(IERC20[] calldata tokens) external;
}// Version: 1.0.0
...
contract Windmill_Trade_Deployer_Swap_Long is Windmill_Trade_Deployer_Abstract, With_PancakeRouter, I_Windmill_Trade_Deployer_Swap_Long{
    ...
    ////
    //////////////// Public variables ////////////////
    IERC20[] public authorizedTokens;
	...
	function checkProposal(uint256 id, uint256 paramsUint256, address paramsAddress) external onlyRole(DAO_ROLE) returns (bool){
        require(id < 1, "Windmill_Trade_Deployer_Swap_Long: Proposal id not found");
        
        if (id == 0){
            address[] memory route = new address[](2);
            route[0] = address(BUSDToken);
            route[1] = paramsAddress;
            
            try pancakeRouter.getAmountsOut(1 ether, route) returns (uint256[] memory v){
                return true;
            }catch{
                return false;
            }
	...
	function applyProposal(uint256 id, uint256 paramsUint256, address paramsAddress) external onlyRole(DAO_ROLE){
        if (id == 0){
            authorizedTokens.push(IERC20(paramsAddress));
        }
    }
    ////
    ////
    ////
    //////////////// Private functions ////////////////
    function _initTrade(I_Windmill_Trade_Abstract trade, address owner, address manager, address fund) internal override{
        I_Windmill_Trade_Swap_Long tradeDerived = I_Windmill_Trade_Swap_Long(address(trade));
        
        tradeDerived.setAuthorizedTokens(authorizedTokens);
        
        super._initTrade(trade, owner, manager, fund);
    }
}