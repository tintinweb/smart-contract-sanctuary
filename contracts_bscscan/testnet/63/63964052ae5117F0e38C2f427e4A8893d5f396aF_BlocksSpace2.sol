...
contract BlocksSpace2 is Ownable {
    ...
    //    console.log("Allowing BLS in contract: ", allowedMaxBls_);
        require(
        ...
        uint256[] memory previousOwnersBlsTakeover = new uint256[](numberOfBlocks);
        {
            ...
            for(uint256 i; i < numberOfBlocks; ++i){
                if(blocks[i].priceBls == tempMinTakeover){
        ...
        for (uint256 i = blocksArea.startBlockX; i <= blocksArea.endBlockX; ++i) {
            for (uint256 j = blocksArea.startBlockY; j <= blocksArea.endBlockY; ++j) {
                ...
                if(currentBlock.blockWhenClaimed > 0){
                    // console.log("currentBlock.blockWhenClaimed", currentBlock.blockWhenClaimed);
...
contract BlocksRewardsManager2 is Ownable {
    ...
    function pendingBlsTokens(uint256 spaceId_, address user_) public view returns (uint256) {
        ...
        // console.log("user.takeoverRewards", user.takeoverRewards);
        return user.amount * space.blsRewardsAcc + rewards + user.pendingRewards + user.takeoverRewards - user.rewardsDebt;
    }
