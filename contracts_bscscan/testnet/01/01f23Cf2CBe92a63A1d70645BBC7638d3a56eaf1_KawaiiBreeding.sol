...
contract KawaiiBreeding is Signed, Pausable, SignData {
    ...
    function breeding(bytes memory data) external whenNotPaused {
        ...
        require(_milkyPrice >= minFeeMilky && _milkyPrice <= maxFeeMilky, "Invalid milky price");
        {
            ...
            verify(keccak256(abi.encode(BREEDING_HASH, keccak256(adminSignedData), nonce)), sender, v, r, s);
            (v, r, s) = abi.decode(adminSignedData, (uint8, bytes32, bytes32));
            address signer = getSigner(
                keccak256(
                    abi.encode(address(this), this.breeding.selector, _inputNFTIds, _inputAmounts, _tokenIds, _kawaiiPrice,_milkyPrice, _timestamp, nonce)
                ), v, r, s);
