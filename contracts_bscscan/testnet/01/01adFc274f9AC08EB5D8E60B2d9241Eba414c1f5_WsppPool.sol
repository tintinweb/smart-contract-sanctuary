...
// File: contracts/farms/wspp/common/IERC20Metadata.sol
...
// File: contracts/farms/wspp/WsppPool.sol
...
contract WsppPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
...
//        uint256 _poolLimitPerUser,
    ...
    /* Wspp Admin functions to setup pool */

    function wsppStartIn(uint256 _blocks) public onlyOwner {
        wsppSetStart(block.number.add(_blocks));
    }

    function wsppSetStart(uint256 _startBlock) public onlyOwner {
    ...
    function wsppSetDuration(uint256 _durationBlocks) public onlyOwner {
        bonusEndBlock = startBlock.add(_durationBlocks);
        wsppCalcRewardPerBlock();
    }

    function wsppCalcRewardPerBlock() public onlyOwner {
        uint256 rewardBal = rewardToken.balanceOf(address(this));
        rewardPerBlock = rewardBal.div(rewardDuration());
    }

    function wsppSetRewardMaxTxAmount(uint256 _maxTxAmount) public onlyOwner {
