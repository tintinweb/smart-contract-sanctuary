...
interface IKawaiiCore {
    ...
    function getKawaiiDatas(uint256 tokenId) external view returns (KawaiiData memory);

    function sizeOfCategory(string calldata category) external view returns (uint256);

    function getNFTInCategory(string calldata category, uint256 index) external view returns (uint256);

    function getTotalUserNFTInCategory(address account, string calldata category) external view returns (uint256);
...
contract KawaiiOwnableNFT {
    ...
    function isTransferable(IKawaiiCore kawaiiCore, IKawaiiWarehouse kawaiiWarehouse, address from, address to, uint256 id, uint256 amount) external view returns (bool){
        ...
        string memory category = kawaiiCore.getKawaiiDatas(id).category;
        uint256 limited = kawaiiWarehouse.getLimited(to, category);
        uint256 currentHoldingNumber = kawaiiCore.getTotalUserNFTInCategory(to, category);
    ...
    function isBatchTransferable(IKawaiiCore kawaiiCore, IKawaiiWarehouse kawaiiWarehouse, address from, address to, uint256[] calldata ids, uint256[] calldata amounts) external view returns (bool){
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            string memory category = kawaiiCore.getKawaiiDatas(ids[i]).category;
            isDuplicate = false;
        ...
        for (uint256 i = 0; i < totalDiffCategory; i++) {
            if (categories[i].amount > 0) {
                ...
                uint256 currentHoldingNumber = kawaiiCore.getTotalUserNFTInCategory(to, categories[i].category);
