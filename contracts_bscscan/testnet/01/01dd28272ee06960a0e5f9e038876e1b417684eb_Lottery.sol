...
contract Lottery is Ownable {
    ...
    uint public currentCarryOverSeri;
    ...
    uint public share2affiliateCO = 40;
    uint public share2OperatorCO = 20;
    ...
    struct seri {
        ...
        uint max2sale;
        uint totalWin;
        uint seriType; // 1 normal; 2 carryOver;
        uint initPrize;
        uint initPrizeTaken;
    }
    struct ticket {
    ...
    mapping(uint => mapping(address => mapping(uint => uint))) public userTicketsWon; // seri => user => ticket id => token id
    mapping(uint => mapping(address => mapping(uint => uint))) public userTicketsWonb; // seri => user => token id => ticket id
    ...
    function permit(uint timestamp, string memory result, uint8 v, bytes32 r, bytes32 s) public view returns (bool) {
    ...
    function getLatestPrice(string memory _symbol) public view returns (int) {
        (,int _price,,,) = assets[_symbol].priceFeed.latestRoundData();
        return _price * 10**10;
    }
    function asset2USD(string memory _symbol) public view returns (uint _amountUsd){
        return uint(getLatestPrice(_symbol));
    }
    function asset2USD(string memory _symbol, uint _amount) public view returns (uint _amountUsd){
        return _amount.mul(uint(getLatestPrice(_symbol))).div(1 ether);
    }
    function ticket2Asset(uint _seri, string memory _symbol) public view returns(uint _amountUsd){
    ...
    function openSeriCarryOver(uint _seri, uint _max2sale, uint _initPrize) public onlyOwner {
    ...
    function openResult(uint _seri, bool _isWin, uint _totalWin, uint timestamp, string memory _result, uint8 v, bytes32 r, bytes32 s) public onlyOwner {
    ...
    function setWinners(uint _seri, uint startTime, address[] memory _winners, uint[][] memory _buyTickets, string[] memory _assets) public onlyOwner {
        ...
        for(uint i = 0; i < _winners.length; i++) {
            for(uint j = 0; j < _buyTickets[i].length; j++) {
                uint tokenID = nft.mint(_winners[i], _seri, startTime, now, sr.result, 2, sr.totalWin, _winners[i], 1, _assets[i]);
    ...
    function takeAsset(uint _seri, uint _winTickets, uint _buyTickets) internal {
        for(uint i = 0; i < series[_seri].assetIndex.length; i++) {
            if(seriAssetRemain[_seri][series[_seri].assetIndex[i]] > 0) {
                uint takeAmount = seriAssetRemain[_seri][series[_seri].assetIndex[i]].mul(_buyTickets).div(_winTickets);
        ...
        if(series[_seri].seriType == 2) {
            uint takeAssetInitAmount = series[_seri].initPrize.mul(_buyTickets).div(_winTickets);
    ...
    function totalPrize(uint _seri) public view returns(uint _prize) {
        for(uint i = 0; i < series[_seri].assetIndex.length; i++) {
            if(seriAssetRemain[_seri][series[_seri].assetIndex[i]] > 0) {
                string memory symbol = priceFeeds[series[_seri].assetIndex[i]];
                _prize += asset2USD(symbol, seriAssetRemain[_seri][series[_seri].assetIndex[i]]);
    ...
    function _takePrizeExpired(uint _seri) internal {
        ...
        if(series[_seri].seriType == 2) {
            uint takeAssetInitRemain = series[_seri].initPrize.sub(series[_seri].initPrizeTaken);
    ...
    function buyCarryOver(uint _seri, string[] memory _number, uint[] memory _numTicket, uint _assetIndex) public payable{
        ...
        uint shareAffiliateAmount = assetAmount.mul(share2affiliateCO).div(100);
        uint takeTokenAmount = assetAmount.mul(share2OperatorCO).div(100);
        if(_assetIndex == 0) {
        ...
        uint assetRemain = assetAmount.sub(shareAffiliateAmount).sub(takeTokenAmount);
    ...
    function buy(uint _seri, string[] memory _number, uint[] memory _numTicket, uint _assetIndex) public payable{
        require(_number.length == _numTicket.length, 'invalid array length');
        uint assetPerTicket = ticket2Asset(_seri, priceFeeds[_assetIndex]);
    ...
    function configAddress(address payable _stake, address payable _purchase, address payable _operator, address payable _affiliateAddress, NFT _nft, 
    address payable _carryOver, IBEP20 _initCarryOverAsset) public onlyOwner {
        stake = _stake;
        purchase = _purchase;
        operator = _operator;
    ...
    function config(uint _price, uint _expiredPeriod, uint _share2Stake, uint _share2Purchase, uint _share2affiliate, uint _share2Operator,
    uint _share2affiliateCO, uint _share2OperatorCO) public onlyOwner {
