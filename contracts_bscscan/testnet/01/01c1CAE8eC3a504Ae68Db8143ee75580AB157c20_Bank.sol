...
contract Bank is Ownable, IERC721Receiver, Pausable {
  ...
  uint256 public DAILY_LOOT_RATE = 2500 ether;
  ...
  uint256 public constant MAXIMUM_GLOBAL_LOOT = 1700000000 ether;
  ...
  function _addThiefToBankWithTime(
    ...
    totalLootEarned +=
      ((time - lastClaimTimestamp) * totalThiefStaked * DAILY_LOOT_RATE) /
      1 days;
...
import "./interfaces/IPoliceAndThief.sol";
import "./interfaces/IBank.sol";
  ...
  Pausable
{
  ...
  uint256 public MINT_PRICE = 0.000000001 ether;
  // max number of tokens that can be minted - 50000 in production
  // uint256 public immutable MAX_TOKENS = 28500;
  uint256 public immutable MAX_TOKENS = 2850;
  // number of tokens that can be claimed for free - 20% of MAX_TOKENS
  // uint256 public PAID_TOKENS = 3500;
  uint256 public PAID_TOKENS = 350;
  ...
  function mintCost(uint256 tokenId) public view returns (uint256) {
    ...
    if (tokenId <= PAID_TOKENS + 10000) return 10000 ether;
    if (tokenId <= PAID_TOKENS + 15000) return 20000 ether;
    if (tokenId <= PAID_TOKENS + 20000) return 30000 ether;
    return 40000 ether;
  }
