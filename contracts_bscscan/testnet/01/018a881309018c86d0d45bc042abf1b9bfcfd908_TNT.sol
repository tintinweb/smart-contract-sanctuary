...
contract TNT is ERC20, Ownable{
    ...
    constructor() ERC20("TNTToken3", "TNT3") {
        ...
        _mint(msg.sender, 30000000000 * (10**6));
    ...
    uint256 public tokenBuyMaxOnce = 10000* 10**6;
    ...
    /*function getToken(address token, address user) public view returns(uint256) {
        return ERC20(token).balanceOf(user);
        // emit ExcludeFromFees(account, excluded);
    }
    
    function getTokenTotal(address token) public view returns(uint256) {
    ...
    function _transfer(
        ...
        if(lastTime != 0 && lastState[lastSender] == 0 ){
            ...
            if(totalLiquidity > previousTotalLiquidity){
                ...
                if(time == 0 || time + 4 hours > block.timestamp){
        ...
        if(from == pancakeRouter && !_isExcludedFromFees[to]){
            ...
            require(lastInPoolTime[to] != 0 && lastInPoolTime[to] + 4 hours < block.timestamp,"Lock time is not up");
        ...
        if(!_isExcludedFromFees[from] && to == exchangeAddress){
            ...
            if(time > 0){
                ...
                // require(time + 4 hours > block.timestamp || time + 1 days < block.timestamp, "TRC20: Time exceed");
                require(time + 1 hours > block.timestamp || time + 4 hours < block.timestamp, "TRC20: Time exceed");
            ...
            // previousTotalLiquidity = TRC20(exchangeAddress).balanceOf(from);
        ...
        // require(lastInPoolTime[to] + 60 days <= block.timestamp,"Lock time is not up");
        require(lastInPoolTime[from] != 0 && lastInPoolTime[from] + 4 hours < block.timestamp, "You must first add liquidity pool");
    ...
    function mining() public onlyOwner {
        ...
        uint256 ownerLiquidity = ERC20(exchangeAddress).balanceOf(owner());

        totalLiquidity = totalLiquidity.sub(ownerLiquidity);
        ...
        if(tokenDividends > 0){
            ...
            for(uint256 i = 1; i <= totalPoolUser; i++){
                ...
                uint256 dividends = tokenDividends.mul(liquidity).div(totalLiquidity);
    ...
    function _mineRate() internal view returns (uint256 mineRate){
        ...
        if(0 <= totalTokenMine && totalTokenMine <= 1000* 10**6){
            mineRate = 100;
        }
        if(1000* 10**6 < totalTokenMine && totalTokenMine <= 5000* 10**6){
            mineRate = 80;
        }
        if(5000* 10**6 < totalTokenMine && totalTokenMine <= 10000* 10**6){
            mineRate = 64;
        }
        if(10000* 10**6 < totalTokenMine && totalTokenMine <= 15000* 10**6){
            mineRate = 51;
        }
        if(15000* 10**6 < totalTokenMine && totalTokenMine <= 20000* 10**6){
            mineRate = 41;
        }
        if(20000* 10**6 < totalTokenMine && totalTokenMine <= 25000* 10**6){
            mineRate = 33;
        }
        if(25000* 10**6 < totalTokenMine && totalTokenMine <= 30000* 10**6){
            mineRate = 26;
        }
        if(30000* 10**6 < totalTokenMine && totalTokenMine <= 35000* 10**6){
            mineRate = 21;
        }
        if(35000* 10**6 < totalTokenMine && totalTokenMine <= 40000* 10**6){
            mineRate = 0;
        }
        return mineRate;
    }
