...
contract Starlink is IStarlink, IDepositable, StarlinkComponent, PancakeSwapHelper {
    ...
    function addPool(address token, uint16 starlinkPointsPerToken, bool isEnabled) public onlyAdmins {
        ...
        pools.push(pool);
        poolIndexByTokenAddress[token] = pools.length - 1;
    }
    ...
    function poolOf(address token) public view returns (Pool memory) {
        uint256 poolIndex = poolIndexByTokenAddress[token];

        Pool storage pool = pools[poolIndex];
        require(pool.token == token, "Starlink: Unsupported pool");
    ...
    function calculateReward(Pool storage pool, UserStaking storage userStake) private view returns (uint256) {
        uint256 totalDividends = pool.totalDividends;
        uint256 newDividends = totalDividends - userStake.lastDividends;
        return newDividends * userStake.amount / availableSupply();
    }
    ...
    function updateStakingOf(address userAddress, address token) private {
        ...
        userStake.lastDividends = pool.totalDividends;
    }
    ...
    function availableSupply() internal view returns(uint256) {
        return TOTAL_SUPPLY - xld.balanceOf(address(0)) - xld.balanceOf(BURN_ADDRESS);
    }
}
