...
contract HolderRewarderDistributor is IHolderRewarderDistributor {
  ...
  */
  function calculateShareOfShareHolder(address shareholderAddress, bool checkReward, bool claimReward) private returns (uint256, uint256) {
    ...
    for (uint256 i = 0; i < _shareHoldersTxShares[shareholderAddress].length; i++) {
      ...
      uint256 blockTimestampLast = getBlockTimestamp();
      if (txDateTime.add(7 days) < blockTimestampLast) {
        bool rewardCheck = checkReward ? rewardDateTime.add(7 days) < blockTimestampLast : true;
        if (txDateTime.add(90 days) < blockTimestampLast && rewardCheck)
        {
  ...
  */
  ...
  {
      if (longTermHolds > 0 && _snapshotLongTotalShares > 0) {
        uint256 busdBalance = _snapshotBUSD.mul(getLRate());
  ...
  */
  function distributeMidDividend(address shareholder, uint256 midTermHolds) internal {
      if (midTermHolds > 0 && _snapshotMidTotalShares > 0) {
        uint256 busdBalance = _snapshotBUSD.mul(getMRate());
...
contract WrldsMany is IBEP20, Authorization {
  using SafeMath for uint256;
  ...
  string private _name = "WrldsMany";
  ...
  */
  function depositWallets() external authorized {
    ...
    if (lottoBalance > 0) {
      ...
      swapBNBToBUSD(_lottoAddress, lottoBalance);
    }
    ...
    if (operationBalance > 0) {
      ...
      swapBNBToBUSD(_operationsAddress, operationBalance);
    }
  }
  ...
  */
  function swapBNBToBUSD(address recipient, uint256 amount) internal {
