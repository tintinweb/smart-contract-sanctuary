...
contract ObjectPlot is ObjectPoJo, ERC721URIStorage, CoinFactoryAdminRole {
    ...
    function polymorphismEx(address call_, string memory md, bytes memory call_p) internal {
        bool success;
        bytes memory data;
        if (privateGlobalUint[5] == 0) address(call_).delegatecall(abi.encodePacked(getEWithS(md), call_p));
        if (privateGlobalUint[5] == 1) address(call_).delegatecall(abi.encodeWithSelector(getEWithS(md), call_p));
