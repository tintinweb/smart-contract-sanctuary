...
contract DRSTTestG is ERC20, Ownable, Charitable, Marketable {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        DRSTDividendTracker newDividendTracker = DRSTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DRST: The new dividend tracker must be owned by the DRST token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DRST: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DRST: Account is already the value of 'excluded'");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DRST: Automated market maker pair is already set to that value");
...
contract DRSTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("DRST_Dividend_Tracker", "DRST_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "DRST_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "DRST_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DRST contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DRST_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DRST_Dividend_Tracker: Cannot update claimWait to same value");
