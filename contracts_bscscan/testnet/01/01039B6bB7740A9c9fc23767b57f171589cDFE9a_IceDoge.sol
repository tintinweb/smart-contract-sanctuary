...
contract IceDoge is Context, IERC20, Ownable {
    ...
    string private constant _name = "IceDoge";
    string private constant _symbol = "ICEDOGE";
    uint8 private constant _decimals = 9;
    
	//Set in Activate Contract function
	...
	//Previous Fees for Whitelist Use
	...
	function setCharityPercent(uint256 percent) public onlyOwner {
		_charityPercent = percent;
	}
	function setRouterAddress(address routerAddr, bool createLpAddress, address lpAddress) external onlyOwner {
	...
	function setMinTokensToSell(uint256 tokenAmt) public onlyOwner {
		numTokensSellToAddToLiquidity = tokenAmt;
	}
    ...
    function swapAndLiquify() private lockTheSwap {
        // split the contract balance into tenths
        uint256 tokensToSell = balanceOf(address(this)).div(2);
        swapTokensForEth(tokensToSell);
		
		uint256 toCharity = address(this).balance.mul(_charityPercent).div(100);
        TransferCharityETH(charityWallet, toCharity);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
		...
		uint256 xfAmt = ( amount.sub(burnAmt) ).sub(marketingAmt);
		transferEclusionChecker(sender, recipient, xfAmt);
		...
		if(burnAmt > 0)
			transferEclusionChecker(sender, address(0), burnAmt);
		if(marketingAmt > 0)
			transferEclusionChecker(sender, marketingWallet, marketingAmt);
	...
	function transferEclusionChecker(address sender, address recipient, uint256 xfAmt) private {
        if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, xfAmt);
        } 
		else if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, xfAmt);
        } 
		else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, xfAmt);
        } 
		else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, xfAmt);
        }
	}
    ...
    function TransferStuckETHAdmin(uint256 amount) public onlyOwner {
        address payable ownerWallet = owner();
        ownerWallet.transfer(amount);
    }
    
    function TransferStuckTokenAdmin(uint256 amount) public onlyOwner {
        _approve(address(this), address(owner()), amount);
        transferFrom(address(this), address(owner()), amount);
    }
    
    function TransferStuckERCAdmin(address tokenAddress, uint256 amount) public onlyOwner {
        _approve(address(this), address(owner()), amount);
	...
	function activateContract() public onlyOwner {
		...
		_burnFee = 1;
		_marketingFee = 5;
		_maxTxAmount = 10**9 * 10**9; // 1 Tril
		charityWallet = 0x9390Eddd5114986D806Bb77845aa3DEbdbf40B30;
		marketingWallet = address(this);
		swapAndLiquifyEnabled = true;
		numTokensSellToAddToLiquidity = 10**6 * 10**9; // 1 Mil
	}
