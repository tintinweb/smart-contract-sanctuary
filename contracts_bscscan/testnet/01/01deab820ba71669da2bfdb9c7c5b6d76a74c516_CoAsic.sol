...
contract CoAsic is IERC20{
    struct holder{
        address account;
        uint256 timestamp;//first tokens received
    }
    ...
    mapping(address=>uint160) public holderIndex;
    mapping(uint160=>holder) public holders;
    uint160 public numHolders;
    ...
    function _transfer(address from, address to, uint256 amount) private{
        ...
        if(holderIndex[to] == 0 && holders[0].account != to){
            holderIndex[to] = numHolders;
            holders[numHolders].account = to;
            holders[numHolders].timestamp = block.timestamp;
            numHolders++;
        }
