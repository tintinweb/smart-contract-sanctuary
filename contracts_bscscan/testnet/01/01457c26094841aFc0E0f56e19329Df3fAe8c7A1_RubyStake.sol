...
event arvestRewards(uint _poolId,address _staker, uint _amount, uint _time);
event poolCreated(uint _poolId,address _creator,uint _yield,address _stakeToken );
  ...
  function createPool(address tokenAddress,uint bps)public{
      
      uint  time=block.timestamp;
      IBEP20 tokenContract=IBEP20(tokenAddress);
      Pool memory stakePool=Pool(poolCounter,tokenContract,0,time,bps);
...
userInfo[poolId][staker]=user;
 ...
 emit unStakeCoins( poolId,staker, address(idToPool[poolId]._token),amount, user._time);
  ...
  uint rewardDecimals=rewardToken.decimals();
   ...
   rewardDecimals=1*10**rewardDecimals;
 ...
 emit arvestRewards(poolId,staker, rewardableTokens, userInfo[poolId][staker]._time-oldTime);
 ...
 function userPoolData(uint poolId) public view returns ( address _token,uint _staked,uint _pooltime,uint _apy,uint _reward, uint _stakeAmount,uint _pendingRewards, uint _stakeTime){
  Pool memory pool= idToPool[poolId];
  UserInfo  memory user= userInfo[poolId][msg.sender];
  uint pooldecimal=pool._token.decimals();
  uint rewardDecimals=rewardToken.decimals();
   uint dailyreward=rewardCoins(pool._apy,pooldecimal,rewardDecimals);
     uint pendingrewards= pendingHarvest(poolId,msg.sender);
   return ( address(pool._token),pool._staked,pool._timestamp,pool._apy,dailyreward,user._amount,pendingrewards,user._time);
}
function viewUserData( uint poolId) public view returns (   uint _amount,uint _time){
  UserInfo  memory user= userInfo[poolId][msg.sender];
   return ( user._amount,user._time);
}

/*
issue  the rewards
*/
function issueReward(address staker, uint tokens) public{
    uint  rewardbalance= rewardToken.balanceOf(address(this));
    require(rewardbalance>=tokens,"RubyStaking:insufficient Ruby balance");
     rewardToken.transfer(staker,tokens); // transfer  the rewards
}
/*
convert yield into rewardcoins per mili seconds
*/
