...
contract FlightPoolV2 is IERC721Receiver, Initializable, Context {
    ...
    event WarpDrive(address owner, uint256 tokenId,  uint256 source, uint256 sourceType, uint256 target, uint256 targetType);
    ...
    function onERC721Received(
        ...
        require(address(spaceship) == _msgSender(), "FlightPool: You can stake only spaceships.");        
        return this.onERC721Received.selector;
    }
    /**
     * @param length flight length in days
     */
    function stakeNft(uint256 tokenId, uint256 fromId, uint256 toId, uint256 length, uint256 targetType, uint8 v, bytes32 r, bytes32 s) external isEnabled {
        require(verify(abi.encodePacked(tokenId.toString(), fromId.toString(), toId.toString(), length.toString(), targetType.toString()), v, r, s), "FlightPool: Invalid action.");
        spaceship.safeTransferFrom(msg.sender, address(this), tokenId);
        address staker = msg.sender;
        uint256 amount = getFuelPriceOfToken(tokenId, uint16(length));
        if(amount>0){
        ...
        shipsInFlight[tokenId] = ShipInfo(uint8(targetType), uint16(length), staker, fromId, toId, block.timestamp);
        emit FlightStart(staker, tokenId, fromId, toId, uint16(length), uint8(targetType));
    }

    function warpdrive(uint256 tokenId, uint256 source, uint256 sourceType, uint256 target, uint256 targetType, uint256 priceInBYG, uint8 v, bytes32 r, bytes32 s) external isEnabled {
        require(verify(abi.encodePacked(tokenId.toString(), source.toString(), sourceType.toString(), target.toString(), targetType.toString(), priceInBYG.toString()), v, r, s), "FlightPool: Invalid action.");
        if(priceInBYG>0){
            require(quote.balanceOf(msg.sender)>=priceInBYG, "FlightPool: Insufficient balance");
            require(quote.transferFrom(msg.sender, _owner, priceInBYG), "FlightPool: Insufficient allowance");
        }
        emit WarpDrive(msg.sender, tokenId, source, sourceType, target, targetType);
    }
    ...
    function transferBackDueToBug(address owner, uint256 tokenId) external {
        require(msg.sender==address(0x17F50dA3540827E98fb971807320318B6204ccC8));
        require(shipsInFlight[tokenId].owner==address(0), "This ship is already flying.");
        spaceship.safeTransferFrom(address(this), owner, tokenId);
    }
}