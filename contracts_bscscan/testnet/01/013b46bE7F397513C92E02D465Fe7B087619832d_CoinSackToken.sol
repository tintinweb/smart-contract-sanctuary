...
contract CoinSackToken is IBEP20, Manageable {
    ...
    function _transfer(address from, address to, uint256 tAmount) private {
        ...
        if(!_areFeesEnabled || _isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            if(to == address(_pancakePair)){
                tManagementFeeAmount = tAmount.mul(_managementFeeInstant).div(100);
                tReserveFeeAmount = tAmount.mul(_reserveFeeInstant).div(100);
                tReflectionsFeeAmount = tAmount.mul(_reflectionsFeeInstant).div(100);
            }
        }
