...
contract Testie is Context, IERC20, Ownable {
   ...
   // address payable private marketAddress = 0xa49909B357Aedbad1900C4FD80B8327205fe1cb6;
   // address payable private charityAddress = 0x3cF0A285a7766feFb03957019FC5f623cF3d56df;
   ...
   /* function _takeMarketFees(uint256 amount) private returns (uint256) {
        if (_isExcludedFromFee[msg.sender] == false) {
            _rOwned[marketAddress] = _rOwned[marketAddress].add(amount.div(100).mul(2));
            _rOwned[charityAddress] = _rOwned[charityAddress].add(amount.div(100).mul(4));
            sendBNBToMarket(amount.div(100).mul(2));
            sendBNBToCharity(amount.div(100).mul(4));
            
            amount = amount.sub(amount.div(100).mul(6));
        }
        return amount;
    } */
   ...
   /* function sendBNBToCharity(uint256 amount) private { 
    ...
    function sendBNBToMarket(uint256 amount) private {
        swapTokensForEth(amount);
    ...
    /*function _setCharityWallet(address payable charityWalletAddress) external onlyOwner() {
        charityAddress = charityWalletAddress;
    }*/
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        //rTransferAmount = _takeMarketFees(rTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, tFee);
        emit Transfer(sender, recipient, tTransferAmount);
