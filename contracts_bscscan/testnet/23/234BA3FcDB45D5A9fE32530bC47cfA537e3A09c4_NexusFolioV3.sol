...
contract NexusFolioV3 is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair && (from != owner() || from != address(this)) && antiBot ) {
        ...
        if (from == uniswapV2Pair && (to != owner() || to != address(this)) && antiWhale) {
    ...
    function getLastSwapTime(address account)  external view onlyOwner() returns(uint256, uint256){
        return (_addressToLastSwapTime[account], block.timestamp - _addressToLastSwapTime[account]);
    }
