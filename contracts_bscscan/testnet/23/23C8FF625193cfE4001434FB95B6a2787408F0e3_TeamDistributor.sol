...
contract TeamDistributor is Ownable, ReentrancyGuard {
    ...
    function getMemberInfo(address _member) external view returns (DistributeIterableMapping.DistributeMember memory) {
    ...
    function getMemberInfoAtIndex(uint256 memberIndex)
        external
        view
        returns (DistributeIterableMapping.DistributeMember memory)
    {
    ...
    function updateBaseAccount(address _baseAccount) external onlyOwner {
        require(_baseAccount != address(0), "BaseAccount can't be empty address");

        members.get(baseAccount).addr = _baseAccount;
        baseAccount = _baseAccount;

        emit DistributeBaseAccountChanged(baseAccount);
    }
