...
contract rigelfutureExchangeFactory is rigelfutureExchangeInterface {
    ...
    function createPair(address tokenA, address tokenB, address aggregator) external returns (address pair) {
        ...
        require(tokenA != address(0), 'RIGEL: ZERO_ADDRESS');
        require(getPair[tokenA][tokenB] == address(0), 'RGPFutrueExchange: PAIR_EXISTS'); // single check is sufficient
        ...
        rigelfutureExchangePair(pair).initialize(tokenA, tokenB, aggregator, feeToSetter);
