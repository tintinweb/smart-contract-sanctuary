...
contract AnyPadStakingPool is Configurable, ReentrancyGuard {
    ...
    function stake(uint256 amount) external {
        require(
            block.timestamp >= stakingStartTime &&
                block.timestamp < purchasingStartTime,
            "ANYPAD: It is not time to stake"
        );
        require(
            purchasedCurrencyOf[msg.sender] == 0,
            "ANYPAD: Already purchased"
    ...
    function volumeOf(address user) public view returns (uint256) {
        uint256 allocation = allocationOf(user);
    ...
    function withdrawEarningsAndRemaining(address payable to)
        external
        governance
        nonReentrant
    {
        (uint256 amount, uint256 volume) = withdrawable();
        if (currencyToken == address(0)) {
            to.transfer(amount);
        } else {
            IERC20(currencyToken).safeTransfer(to, amount);
