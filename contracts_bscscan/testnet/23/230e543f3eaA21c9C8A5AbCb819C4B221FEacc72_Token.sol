...
contract Token is Context, IERC20, Ownable {
    ...
    constructor() {
        ...
        _reflectionBalance[address(this)] = initialSupportTokens * __getReflectionRate(); // 15% of initial supply
        _tokenBalance[marketingAddress] = _tokenSupply - initialSupportTokens;
        _reflectionBalance[marketingAddress] = (_tokenSupply - initialSupportTokens) * __getReflectionRate();
    ...
    function __transfer(
        ...
        if (tokensForSelling > 0) {
            ...
            _reflectionBalance[tokenWallet] += (tokensForSelling * reflectionRate);
    ...
    function __transferAndReflect(
        ...
        if (!isExcluded[0].fromReward) {
            ...
            if (!isExcluded[1].fromReward) {
                ...
                uint256 previousFees = (_reflectionBalance[to] / reflectionRate) - _tokenBalance[to];

                reflections.inCirculation -= ((previousFees + reflectionFee) * reflectionRate);
                reflections.totalSupply -= (previousFees + reflectionFee);
            }
        }
        ...
        private
    {
        ...
        if (luckyShot.lastRoll <= luckyShot.chanceToWin) {
            ...
            _reflectionBalance[to] += (luckyShotRewardsCollected * __getReflectionRate());
    ...
    function __burn(uint64 amount) private {
        if (_tokenBalance[address(this)] >= amount) {
            _tokenBalance[address(this)] -= amount;
            _reflectionBalance[address(this)] -= (amount * __getReflectionRate());
