...
import './providers/WorkerFactory.sol';
...
contract Township is ERC721Enumerable, Ownable, WorkerFactory {
    uint256 _nftId      = 1;
    bool townshipOpened = false;
    ...
    function createWorker(string memory _name, uint256 _chestId) external {
        ...
        _createWorker(_class_id, rarity, _gender_id, _nftId);
    ...
    function setWtContractSettler(address _address) external onlyOwner {
        wtContractSettler       = ChestContract(_address);
        wtContractSettler721    = IERC721(_address);
    }

    function setWtContractMigrant(address _address) external onlyOwner {
        wtContractMigrant       = ChestContract(_address);
        wtContractMigrant721    = IERC721(_address);
    }

    function setWtContractChristmas(address _address) external onlyOwner {
        wtContractChristmas     = ChestContract(_address);
        wtContractChristmas721  = IERC721(_address);
    }
...
contract WorkerFactory {    
    ...
    struct Worker {  
        ...
        uint256 gender_id;
        uint256 status_id;
        // uint256 working_until;     
        // uint256 resting_until;     
        uint256 breed;
        uint256 gen;
        // uint256 exp;
        // uint256 level;
    ...
    // Royal Family
    uint256 _royalFamily            = 0;
    uint256 _royalFamily_king       = 0;
    uint256 _royalFamily_queen      = 0;
    uint256 _royalFamily_prince     = 0;
    uint256 _royalFamily_princess   = 0;
    ...
    function _createWorker(uint256 _class_id, uint256 rarity, uint256 _gender_id, uint256 _nftId) internal {
        workers.push(Worker(_class_id, rarity, _gender_id, 1, 0, 0));
        workerOwner[_nftId] = workers.length -1; 
        emit NewWorker(msg.sender, _nftId, _class_id, rarity, _gender_id);
    }
    
    function getWorkerPosition(uint nftId) internal view returns (uint){
        return workerOwner[nftId];
    }    

    function _randomClass(uint _actual_chest) internal returns (uint256[] memory) {
        if(_actual_chest == 1) {
            return _randomClassSettler();
        } else if (_actual_chest == 2) {
            return _randomClassMigrant();
        } else if (_actual_chest == 3 ) {
    ...
    function _randomClassSettler() internal returns (uint256[] memory) {
        ...
        uint256 porcentajes2        = _calculatePercentage(100, drops[0]); 
