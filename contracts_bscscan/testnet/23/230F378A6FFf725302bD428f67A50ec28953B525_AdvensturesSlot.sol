...
contract AdvensturesSlot is Context, PartnerOwnable { 
	...
	constructor()   { 
        ...
        tokens_create(0x31508f0098A2566074EcCC94a0900721386D0C4b,bytes('TEST'),500); // CAKE
    ...
    function bet(uint[] calldata bets,uint domainId, uint tokensId) external  
        ...
        play_start();  
        require(wins.wins_type[0] < bets_length,'AdvensturesSlot: bet_play_start_error'); 
        ...
        uint256 fee1 = win_tot * domainInfo[0].domain_fee_rate / 1000;
        uint256 fee2 = win_tot * domainInfo[domainId].domain_fee_rate / 1000;
        wins.wins_bet = tot_bet;
        wins.wins_win = win_tot;
        wins.wins_fee = fee1 + fee2;
    ...
    function play_start() internal {  
        ...
        random = randomize_game_rate_rand(); // randomize one of game_layout index 0-21 < 22  
        wins.wins_layout[0] = random;
        wins.wins_type[0] = uint(game_layout[random]);
        wins.wins_rate[0] = game_rate[random];
