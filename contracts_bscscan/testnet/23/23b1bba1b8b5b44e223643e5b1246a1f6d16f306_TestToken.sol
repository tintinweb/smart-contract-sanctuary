...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 TOK = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = TOK.balanceOf(address(this));
        ...
        uint256 amount = TOK.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            TOK.transfer(shareholder, amount);
...
contract TestToken is IBEP20, Auth {
    ...
    address TOK = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    uint256 public _maxTxAmount = _totalSupply.mul(2).div(_divider);

    //max wallet holding of 2%
    uint256 public _maxWalletToken = _totalSupply.mul(2).div(_divider);
    ...
    uint256 public swapThreshold = _totalSupply.mul(10).div(_divider * 100); // 0.01% of supply
    bool inSwap;
    ...
    constructor () Auth(msg.sender) {
        ...
        _allowances[address(this)][address(router)] = uint256(type(uint256).max);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(_allowances[sender][msg.sender] != uint256(type(uint256).max)){
    ...
    function setTxLimit(uint256 txLimitPercent) external authorized {
        _maxTxAmount = _totalSupply.mul(txLimitPercent).div(_divider);
    }

    //settting the maximum permitted wallet holding (percent of total supply)
     function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
        _maxWalletToken = _totalSupply.mul(maxWallPercent).div(_divider);
    }
    
    function setDividerPercent(uint256 dividerPercent) external onlyOwner() {
        _divider = dividerPercent;
    }
