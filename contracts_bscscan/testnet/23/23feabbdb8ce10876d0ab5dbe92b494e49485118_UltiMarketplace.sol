...
import "../interfaces/IUltiMarketplace.sol";
import "../role-manager/LibRole.sol";
import "../tokens/erc-721/LibERC721Mint.sol";
import "./LibTransfer.sol";

contract UltiMarketplace is ERC721HolderUpgradeable, ReentrancyGuardUpgradeable, IUltiMarketplace {
    ...
    mapping(address => mapping(uint256 => Auction)) public override auctions;
    ...
    function mint(
        address nftAddress,
        LibERC721Mint.MintData calldata data,
        address to
    ) external override onlyRole(LibRole.MINTER_ROLE) {
        require(
            IERC165(nftAddress).supportsInterface(type(IERC721Mintable).interfaceId),
            "NFT contract does not implement IERC721Mintable interface"
        );
        IERC721Mintable(nftAddress).mint(data, to);
    }
    ...
    function bid(
        ...
        if (auction.buyNowPrice > 0 && amount.value == auction.buyNowPrice) {
            _buy(nftAddress, tokenId, amount, auction, msg.sender);
    ...
    function finalize(address nftAddress, uint256 tokenId) external override nonReentrant {
        _validateNftAddress(nftAddress);
        ...
        if (auction.highestBidder != address(0)) {
            ...
            uint256 bidValue = auction.highestBid;

            _transferWithFees(auction.seller, Amount(bidValue, auction.currency), nftAddress, tokenId);
            _transferTokenAndCloseAuction(nftAddress, tokenId, bidder);
            emit AuctionFinalized(nftAddress, tokenId, bidValue, msg.sender, bidder);
        } else {
            _transferTokenAndCloseAuction(nftAddress, tokenId, auction.seller);
            emit AuctionCanceled(nftAddress, tokenId, msg.sender);
        }
    }
    ...
    function _buy(
        ...
        _receiveFromSender(amount);
        _payOffLastBidder(auction);
        _transferWithFees(auction.seller, amount, nftAddress, tokenId);
        _transferTokenAndCloseAuction(nftAddress, tokenId, purchaser);
    }

    function _transferTokenAndCloseAuction(
        address nftAddress,
        uint256 tokenId,
        address recipient
    ) private {
    ...
    function _validateNftAddress(address nftAddress) private view {
    ...
    function _validateAuctionCreation(
        ...
        require(auctions[nftAddress][tokenId].seller == address(0), "Auction already exists");
        require(msg.sender == IERC721(nftAddress).ownerOf(tokenId), "Auction creation caller is not a token owner");
        require(_availableCurrencies[currency] != 0, "Auction creation query with non-existent currency");
    ...
    function _validateBid(Auction storage auction, Amount memory amount) private view {
    ...
    function _validateBuyNow(Auction storage auction, Amount memory amount) private view {
        require(auction.seller != address(0), "Auction does not exist");
    ...
    function _validateFinalizing(Auction storage auction) private view {
        require(auction.seller != address(0), "Auction does not exist");
        if (msg.sender == auction.seller) {
            if (auction.endTime > 0) {
                require(
                    block.timestamp >= auction.endTime || auction.highestBidder == address(0),
                    "Seller cannot finalize auction if bid already placed"
                );
            }
        } else {
            require(auction.endTime > 0, "Cannot finalize infinite auction if not seller");
            require(block.timestamp >= auction.endTime, "Caller cannot finalize timed auction before it is ended");
        }
    }
    ...
    function _receiveFromSender(Amount memory amount) private {
    ...
    function _transferRoyalties(
        ...
        if (nftAddress.supportsInterface(type(IRoyalties).interfaceId)) {
            ...
            for (uint256 i = 0; i < royalties.length; i++) {
                ...
                _transfer(payable(royalties[i].account), value, totalAmount.currency);
                transferred += value;
            }
        }
        return transferred;
    ...
    function _transferWithFees(
        ...
        _transfer(recipient, amount.value - transferred, amount.currency);
    }
...
interface IUltiMarketplace {
    ...
    event AuctionCanceled(address indexed nftAddress, uint256 indexed tokenId, address indexed caller);
    ...
    event BidPlaced(address indexed nftAddress, uint256 indexed tokenId, uint256 bidAmount, address indexed bidder);
    ...
    function auctions(address nftAddress, uint256 tokenId)
        external
        returns (
            address payable,
            uint256,
    ...
    function finalize(address nftAddress, uint256 tokenId) external;
}
