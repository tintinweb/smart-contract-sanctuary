  ...
  contract FlogeInu is Context, IERC20, Ownable {
      ...
      function sendAirdrop (address[] calldata receivers, uint256 amount) external onlyAirdropper returns (bool) {
          for (uint i = 0; i < receivers.length; i++) {
              require(!_airdropped[_msgSender()], "Cannot airdrop to addresses already airdropped");
              _transfer(_msgSender(), receivers[i], amount);
              _airdropLog[_msgSender()] = AirdropData(amount, block.timestamp + airdropLockDuration);
              _airdropped[_msgSender()] = true;
          }
          return true;
      }
