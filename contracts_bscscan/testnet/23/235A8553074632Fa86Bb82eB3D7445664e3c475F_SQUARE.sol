...
// 🌎 Website: https://SQUARE.finance
// 📱 Telegram: https://t.me/SQUAREfinance
// 🌐 Twitter: https://www.twitter.com/SQUAREfinance
// 🌎 Reddit:  https://reddit.com/r/SQUAREfinance
...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("SQUARE_Busd_Dividend_Tracker", "SQUARE_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**5); //must hold 1,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "SQUARE_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "SQUARE_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SQUARE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SQUARE_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SQUARE_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MarketingDistributor{
    ...
    IUniswapV2Router02  constant uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);     ///pancake swap

    uint256 public liquidityThreshhold = 1000*(10**5);
    IERC20 public BUSD = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);   ///what's this
    ...
    constructor(address _SQUAREToken){
        SQUAREToken = IERC20(_SQUAREToken);
    }
    ...
    function swapAndLiquify() public {
        ...
        uint256 tokenAmount  = SQUAREToken.balanceOf(address(this));
        if(tokenAmount>=liquidityThreshhold && enableLiquify){
            tokenAmount = liquidityThreshhold;
            address[] memory path = new address[](2);
            path[0] = address(SQUAREToken);
            path[1] = address(BUSD);
            SQUAREToken.approve(address(uniswapV2Router), tokenAmount);
...
contract SQUARE is ERC20, Ownable {
    ...
    constructor() ERC20("SQUARE", "SQUARE") {
    	...
    	busdDividendToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; ///////BUSD CONTRACT
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "SQUARE: The marketing wallet is already this address");
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "SQUARE: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "SQUARE: The new dividend tracker must be owned by the SQUARE token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "SQUARE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "SQUARE: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SQUARE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "SQUARE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "SQUARE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "SQUARE: Trading has not started yet");
