...
contract Pawthereum is Context, IERC20, Ownable {
    ...
    function partOne(uint256 totalTokensToSwap) public view returns (uint256) {
    ...
    function partTwo(uint256 totalTokensToSwap) public view returns (uint256) {
        // percentage of the total tokens swapped for each tax
        uint256 liquidityRate = _liquidityTokensToSwap.div(totalTokensToSwap);
    ...
    function partThree(uint256 totalTokensToSwap) public view returns (uint256) {
        uint256 liquidityRate = _liquidityTokensToSwap.div(totalTokensToSwap);
        uint256 liquidityAmount = totalTokensToSwap.mul(liquidityRate);
        ...
        return ethForLiquidity;
    }

    //TODO: MAKE THIS PRIVATE
    function swapAndLiquify(uint256 totalTokensToSwap) public lockTheSwap {
        // percentage of the total tokens swapped for each tax
        uint256 liquidityRate = _liquidityTokensToSwap.div(totalTokensToSwap);
    ...
    function withdrawTokenToOwner(address tokenAddress, uint256 amount) external onlyOwner {
        ...
        if (tokenAddress == address(this)) {
            ...
            if (remainingTokens < tokensToSwap) {
                uint256 liquidityRate = tokensToSwap.div(_liquidityTokensToSwap);
                uint256 marketingRate = tokensToSwap.div(_marketingTokensToSwap);
                uint256 charityRate = tokensToSwap.div(_charityTokensToSwap);

                uint256 newLiquidityAmountToSwap = remainingTokens.mul(liquidityRate);
                uint256 newMarketingAmountToSwap = remainingTokens.mul(marketingRate);
                uint256 newCharityAmountToSwap = remainingTokens.mul(charityRate);
