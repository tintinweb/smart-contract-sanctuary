...
contract HolderRewarderDistributor is IHolderRewarderDistributor {
  ...
  function calculateShareOfShareHolder(address shareholderAddress, bool excludeIfRewarded) private returns (uint256, uint256) {
    ...
    while (!feesTaken)
    {
        uint256 txDateTime = _shareHoldersTxShares[shareholderAddress][txInteration].txDateTime;
        uint256 rewardDateTime = _shareHoldersTxShares[shareholderAddress][txInteration].rewardDateTime;
        uint256 blockTimestampLast = getBlockTimestamp();
        if (txDateTime + 5 minutes < blockTimestampLast)
        {
          ...
          {
            longTermHolds += _shareHoldersTxShares[shareholderAddress][txInteration].amount;
            _shareHoldersTxShares[shareholderAddress][txInteration].rewardDateTime = blockTimestampLast;
          }
          else if (rewardCheck)
          {
            midTermHolds += _shareHoldersTxShares[shareholderAddress][txInteration].amount;
            _shareHoldersTxShares[shareholderAddress][txInteration].rewardDateTime = blockTimestampLast;
          }
        }
        if (txInteration == (_shareHoldersTxShares[shareholderAddress].length - 1))
        {
          feesTaken = true;
          break;
        }
