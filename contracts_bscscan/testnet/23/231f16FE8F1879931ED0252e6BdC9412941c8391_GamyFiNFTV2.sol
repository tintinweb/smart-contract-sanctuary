...
contract GamyFiNFTV2 is ERC721URIStorage, Pausable, AccessControl, Ownable {
    ...
    event HodlTokenAddress(address hodlTokenAddress);
    ...
    uint256 private tokenUrisCount;

    address public hodlTokenAddress;
    ...
    modifier initialized() {
        require(
            hodlTokenAddress != address(0),
            "HodlTokenAddress is not initialized"
        );
        require(tokenUrisCount > 0, "Empty TokenURI");
        require(mintFee > 0, "Fee is not initialized");
        _;
    }

    function setHodlTokenAddress(address _hodlTokenAddress) external onlyOwner {
        require(_hodlTokenAddress != address(0), "Address can't be null");
        require(_hodlTokenAddress == hodlTokenAddress, "Already set");

        hodlTokenAddress = _hodlTokenAddress;
        hodlToken = IERC20(_hodlTokenAddress);

        emit HodlTokenAddress(_hodlTokenAddress);
    }

    function addTokenUris(string[] memory uris) external onlyOwner {
        require(uris.length > 0, "Empty data");
     ...
     */
    function mint() public payable initialized returns (uint256) {
        require(tokenUrisCount > 0, "URI is not initialized");
        require(msg.value == mintFee, "Invalid Fee");
        require(availableMints[msg.sender] > 1, "Can't mint more");
        require(hodlToken.balanceOf(msg.sender) >= 1e18, "Not enough balance");
        ...
        availableMints[msg.sender] = availableMints[msg.sender] + 1;
