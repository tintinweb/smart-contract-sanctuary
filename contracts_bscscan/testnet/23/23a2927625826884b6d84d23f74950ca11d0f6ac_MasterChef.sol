...
interface IAPYCalcualte{
    function USDToToken0(address _token0)external view returns(uint256);
}
...
contract MasterChef is Ownable {
    ...
    IAPYCalcualte public APYCalcualte;
    // Info of each pool.
    ...
    function APY_config(address _APY)public onlyOwner{
        APYCalcualte = IAPYCalcualte(_APY);
    }
    ...
    function APY(uint8 _pid) public view returns(uint256 lpOutput, uint256 rewardPerBlock){
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if(_pid != 0){
            ...
            uint256 USDToToken0 = APYCalcualte.USDToToken0(address(token0));
