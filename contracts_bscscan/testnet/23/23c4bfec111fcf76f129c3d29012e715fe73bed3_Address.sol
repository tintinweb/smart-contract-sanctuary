...
contract LOLPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	function getLOLTokensLeft() external view returns (uint256) {
		return LOL.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some LOL tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed LOL tokens");
		require(LOL.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough LOL tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		LOL.safeTransfer(msg.sender, callerTokensUnclaimed);
	...
	function withdrawUnsoldLOL() external onlyOwner {
		uint256 amount = LOL.balanceOf(address(this)) - totalTokensSold;
		LOL.safeTransfer(msg.sender, amount);
	}

	function withdrawAllLOL() external onlyOwner {
		LOL.safeTransfer(msg.sender, LOL.balanceOf(address(this)));
	}
}