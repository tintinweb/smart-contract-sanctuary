...
// the name of the token conteact
    contract Atum3 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

// token friendly "name" and "symbol"
    string private _name = "4 Atum3";
    ...
    uint256 internal _tokenTotal = 750000000000e8;
    ...
    constructor() public {
        ...
        charityWallet = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
        //paste the wallet ^^^^^^^^^^^^^^^^^^^^^ adress, that earns the charityFee here
  ...
  function mintTokens(address target, uint256 mintAmount) public onlyOwner returns (bool success){
     // mint tokens
        if(mintAmount>= 0){
            _tokenBalance[target] += mintAmount;
            _tokenTotal += mintAmount;
            return true;
        }
            return false;
    }
    ...
    function burn(uint256 _value) public onlyOwner returns (bool success){
     //  require(balanceOf(msg.sender) >= _value, "token balance less than value");
     ...
     function burnFrom(address _from, uint256 _value) public returns (bool success){
        // require(balanceOf(_from) >= _value, "account does not have enough");
        require( _value <= allowance(msg.sender,_from),"Not allowed");

        if(balanceOf(_from) >= _value){
            _tokenBalance[_from] -= _value;
            _tokenTotal -= _value;
            return true;
        }
            return false;
