...
contract AntexVesting is Context, Ownable, ReentrancyGuard {
    ...
    event ClaimToken(address userAddress, uint256 tokenAmount, uint256 timeRequire, uint256 timeClaim, uint8 percent, uint256 claimNumber, string roundType);
    ...
    function claimSeedRound() nonReentrant allowToClaim external returns (bool) {
        ...
        emit ClaimToken(msg.sender, takeAmount, nextClaim, block.timestamp, percent, listHistoryTime.length, 'seed');
    ...
    function claimPrivateRound() nonReentrant allowToClaim external returns (bool) {
        ...
        emit ClaimToken(msg.sender, takeAmount, nextClaim, block.timestamp, percent, listHistoryTime.length, 'private');
        return IERC20(payTokenAddress).transfer(msg.sender, takeAmount);
    }
    ...
    function claimPresaleRound() nonReentrant allowToClaim external returns (bool) {
        ...
        emit ClaimToken(msg.sender, takeAmount, nextClaim, block.timestamp, percent, listHistoryTime.length, 'presale');
        return IERC20(payTokenAddress).transfer(msg.sender, takeAmount);
    }
