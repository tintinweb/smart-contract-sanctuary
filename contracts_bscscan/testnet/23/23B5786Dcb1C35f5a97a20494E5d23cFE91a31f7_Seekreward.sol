...
contract Ownable is Context {
     ...
     */
    constructor ()  {
        _owner = address(0x4cb3F201e4BC7ce4D99dEB93008940d3848386E6);
...
contract Seekreward is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 200000000*1e18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "SeekReward";
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_holderFee.add(_burnFee).add(_seekrewardFee)).div(100);
        return percentage;
    }
    ...
    function setBurnFeePercent(uint256 burnFee) external onlyOwner() {
     ...
     function setburningamountlimit(uint256 _limit)public onlyOwner returns(bool){
    ...
    function _takeSeekFee(uint256 tAmount) private {
        ...
        _rOwned[seekrewardContractAddress] = _rOwned[address(this)].add(rSeekFee);
        if(_isExcluded[seekrewardContractAddress])
            _tOwned[seekrewardContractAddress] = _tOwned[seekrewardContractAddress].add(tSeekFee);
