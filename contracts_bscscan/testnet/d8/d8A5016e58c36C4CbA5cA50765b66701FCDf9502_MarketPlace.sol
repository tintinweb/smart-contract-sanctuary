...
contract MarketPlace is MarketAuth, PausableUpgradeable {
  ...
  uint constant minClass = 1;
  uint constant maxClass = 1000;
  ...
  function makeOrder(
    ...
    if (_kind == OrderKind.hero) {
      ...
      (uint class,,) = nftTokens[_kind].getItem(_tokenId);
      require(class >= minClass && class <= maxClass, "MarketPlace: item invalid");
    }
  ...
  function takeOrder(OrderKind _kind, uint _tokenId, USDCurrency _usdCurrency) external whenNotPaused {
    ...
    if (currentPrice > 0) {
      ...
      _shareOrderValue(_kind, _tokenId, maker, sharingAmount, _usdCurrency);
    }
    if (orderType == OrderType.trading) {
      _transferNFTToken(_kind, address(this), msg.sender, _tokenId);
    }
    emit OrderTaken(_kind, orderType, _tokenId, msg.sender, paymentMethod, currentPrice);
  }
  ...
  function _takeFund(uint _amount, USDCurrency _usdCurrency) private {
    if (paymentMethod == PaymentMethod.usd) {
      if (_usdCurrency == USDCurrency.usdt) {
        require(usdtToken.allowance(msg.sender, address(this)) >= _amount, "MarketPlace: please approve usdt first");
        require(usdtToken.balanceOf(msg.sender) >= _amount, "MarketPlace: please fund your account");
        require(usdtToken.transferFrom(msg.sender, address(this), _amount), "MarketPlace: transfer usdt failed");
      } else {
        require(busdToken.allowance(msg.sender, address(this)) >= _amount, "MarketPlace: please approve busdt first");
        require(busdToken.balanceOf(msg.sender) >= _amount, "MarketPlace: please fund your account");
        require(busdToken.transferFrom(msg.sender, address(this), _amount), "MarketPlace: transfer busd failed");
      }
    } else {
  ...
  function _shareOrderValue(OrderKind _kind, uint _tokenId, address _maker, uint _totalShare, USDCurrency _usdCurrency) private {
  ...
  function listHeroes(
    ...
    for(uint i = 0; i < _quantity; i++) {
      uint _tokenId = nftTokens[OrderKind.hero].mintHero(address(this), _gene, _class, _name, _price);
  ...
  function listItems(
    ...
    for(uint i = 0; i < _quantity; i++) {
      uint _tokenId = nftTokens[OrderKind.item].mintItem(address(this), _class, _name, _price);
...
interface IGameNFT is IERC721Upgradeable {
  ...
  function mintHero(address _owner, Gene _gene, uint _class, string calldata _name, uint _price) external returns (uint);
  ...
  function mintItem(address _owner, uint _class, string calldata _name, uint _price) external returns (uint);
