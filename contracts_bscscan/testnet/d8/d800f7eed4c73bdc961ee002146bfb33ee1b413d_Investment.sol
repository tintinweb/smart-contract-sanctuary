...
contract Investment {
    ...
    function getCurrentPlan (address  userAdd) public view returns (uint256, uint256) {
     ...
     function getCurrentLeader (address  userAdd) public view returns (uint256, uint256) {
        ...
        uint256 userId=address_Id[userAdd].uniqueId;
          return (groupXmax[userId].parent,groupXmax[userId].refererId);
       
    }
    
     function getIDbalance (address  userAdd) public view returns (uint256, uint256) {
   
        uint256 userId=address_Id[userAdd].uniqueId;
          return (groupXmax[userId].balance,groupX[userId].balance);
