...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function add(
        uint256 _allocPoint,
        address _lpToken,
        uint256 _taxWithdraw,
        uint256 _taxWithdrawBeforeLock,
        uint256 _lock,
        uint256 _depositFee,
        bool _withUpdate,
        uint256 _cake_pid
        ...
        require(_taxWithdraw <= 10000, "add: invalid deposit fee basis points");
        require(_taxWithdrawBeforeLock <= 2500, "add: invalid emergency brun rate basis points");
        require(_lock <= 30 days, "add: invalid lock period");
    ...
    function set(
        ...
        require(_taxWithdraw <= 10000, "set: invalid deposit fee basis points");
        require(_taxWithdrawBeforeLock <= 2500, "set: invalid emergency brun rate basis points");
        require(_lock <= 30 days, "set: invalid lock period");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    returns (uint256, uint256)
    {
        ...
        return (tokenPendingReward, tokenPendingReward);
    }
    ...
    function updatePool(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        token.mint(devaddr, tokenReward.mul(901).div(10000));
    ...
    public validatePoolByPid(_pid) nonReentrant notContract notBlacklisted {
        ...
        if (pool.lock == 0) {
            if (user.amount > 0) {
                ...
                if (pending > 0) {
                    safeTokenTransfer(recipient, pending);
                }
            }
        ...
        if (_amount > 0) {
            if (pool.depositFee > 0) {
                ...
                reflectDeposit(recipient, _pid, _amount);
    ...
    nonReentrant notContract {
        ...
        if (pool.lock > 0) {
            require(user.amount == _amount, "withdraw: Should unstake 100% of token");
        } else {
        ...
        if (pool.lock > 0) {
            if (block.timestamp < user.lastDepositTime + pool.lock && pool.taxWithdrawBeforeLock > 0) {
                if (_amount > 0) {
                    user.amount = user.amount.sub(_amount);
                    userPool(_pid, msg.sender);
                    uint256 tax = _amount.mul(pool.taxWithdrawBeforeLock).div(10000);
                    uint256 sent = _amount.sub(tax);
                    reflectWithdraw(_pid, _amount);
                    pool.lpToken.safeTransfer(taxAddr, tax);
                    pool.lpToken.safeTransfer(address(msg.sender), sent);
                    emit WithdrawWithTax(msg.sender, _pid, sent, tax);
                }
                user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(1e12);
    ...
    function emergencyWithdraw(uint256 _pid) external validatePoolByPid(_pid) nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(pool.lock == 0, "use withdraw");
    ...
    function setTaxAddr(address _taxAddr) external {
        require(msg.sender == _taxAddr, "dev: wut?");
        emit SetTaxAddr(taxAddr, _taxAddr);
        taxAddr = _taxAddr;
    }
    ...
    function reflectDeposit(address recipient, uint256 _pid, uint256 _amount) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][recipient];
        if (pool.cake_pid == 0) return;
        uint256 cake_pid = pool.cake_pid;
        mc.deposit(cake_pid, _amount);
        reflectHarvest(_pid);
    }

    function reflectWithdraw(uint256 _pid, uint256 _amount) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        if (pool.cake_pid == 0) return;
        uint256 cake_pid = pool.cake_pid;
        mc.withdraw(cake_pid, _amount);
        reflectHarvest(_pid);
    }
    ...
    function adminEmergencyWithdraw(uint256 _pid, uint256 _amount) external onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        if (pool.cake_pid == 0) return;
        mc.emergencyWithdraw(pool.cake_pid);
    }
    ...
    function getLpOf(uint256 pid) public view returns (address) {
        (address lpToken, uint256 allocPoint, uint256 lastRewardBlock, uint256 accEggPerShare, uint16 depositFeeBP) =
        mc.poolInfo(pid);
        return lpToken;
    }

    function calculateHarvestRewards(uint256 pid) external view returns (uint256) {
        uint256 amount = mc.pendingCake(pid, address(this));
        amount = amount.add( cake.balanceOf(address(this)) );
    ...
    function setContractAllowed(bool status) external onlyOwner {
        contractAllowed[msg.sender] = status;
    }

    function setBlaclisted(address addr, bool status) external onlyOwner {
    ...
    function reflectHarvest(uint256 pid) internal {
        ...
        cake.safeTransfer(taxAddr, reserveAmount);
...
interface IFarm {
    ...
    function poolInfo(uint256 _pid) external view returns (address lpToken, uint256 allocPoint, uint256 lastRewardBlock, uint256 accEggPerShare, uint16 depositFeeBP);
