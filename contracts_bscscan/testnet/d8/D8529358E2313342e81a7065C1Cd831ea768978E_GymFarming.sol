...
contract GymFarming is Ownable, ReentrancyGuard {
    ...
    function speedStake(
        ...
        if (_tokenAmount > 0) {
            ...
            token.approve(routerAddr, _tokenAmount);
    ...
    function autoDeposit(
        ...
        uint256 _amount,
        address _from
    ) public poolExists(_pid) onlyBank {
        updatePool(_pid);
        ...
        (, , uint256 liquidity) = IPancakeRouter02(ROUTER_ADDRESS)
            .addLiquidityETH{value: balanceDifference}(
            address(rewardToken),
            _amount / 2,
            0,
            0,
