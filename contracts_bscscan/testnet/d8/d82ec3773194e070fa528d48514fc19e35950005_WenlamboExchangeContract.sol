...
contract Sale is Ownable{
    ...
    constructor(uint256 _serviceValue, uint256 _transvalue, uint256 id, uint256 _PlatformValue) public{
    ...
    function _acceptBId(string memory tokenAss,address from, uint256 amount, uint256 tokenId, address admin) internal{
        ...
        require(balances[tokenId][msg.sender] > 0, "Is Not a Owner");
        (uint256 _adminfee, uint256 roy, uint256 netamount, uint256 _platformfee) = calc(amount, _royal[tokenId], serviceValue, PlatformValue);
        require( approveValue >= _adminfee.add(roy.add(netamount).add(_platformfee)), "Insufficient Balance");
        t.transferFrom(from, admin, _adminfee);
        t.transferFrom(from, admin, _platformfee);
    ...
    function _saleToken(address payable from, uint256 tokenId, uint256 amount, string memory bidtoken, string memory nft) internal{
      uint256 val = percent(amount, serviceValue).add(amount);
      ...
      if(keccak256(abi.encodePacked((bidtoken))) == keccak256(abi.encodePacked(("BNB")))){   
        ...
        (uint256 _adminfee, uint256 roy, uint256 netamount, uint256 _platformfee) = calc(amount, _royal[tokenId], serviceValue, PlatformValue);
        require( msg.value == _adminfee.add(roy.add(netamount).add(_platformfee)), "Insufficient Balance");
        address payable admin = address(uint160(owner()));
        admin.transfer(_adminfee);
        admin.transfer(_platformfee);
        create.transfer(roy);
        if(keccak256(abi.encodePacked((nft))) == keccak256(abi.encodePacked(("micro")))){
             //address payable conaddress = address(uint160(address(this)));
             //address(this).transfer(netamount);   
        }
        else{
        ...
        t.transferFrom(msg.sender, owner(), _platformfee);
        t.transferFrom(msg.sender,_creator[tokenId],roy);
        if(keccak256(abi.encodePacked((nft))) == keccak256(abi.encodePacked(("micro")))){
            t.transferFrom(msg.sender,address(this),netamount);
        }
        else{
    ...
    function calc(uint256 amount, uint256 royal, uint256 _serviceValue, uint256 _platformValue) internal pure returns(uint256, uint256, uint256, uint256){
        uint256 fee = percent(amount, _serviceValue);
        uint256 fee1 = percent(amount, _platformValue);
        uint256 roy = percent(amount, royal);
        uint256 netamount = amount.sub(fee.add(roy).add(fee1));
        fee = fee.add(fee);
        fee1 = fee1.add(fee1);
        return (fee, roy, netamount, fee1);
    }
    ...
    function setPlatFormValue(uint256 _platformValue) internal{
        PlatformValue = _platformValue;
    }
...
contract WenlamboExchangeContract is Sale{
    ...
    constructor(uint256 _serviceValue, uint256 _transvalue, uint256 id, uint256 _platformValue) Sale(_serviceValue, _transvalue, id, _platformValue) public{
    ...
    function platFormFunction(uint256 _platformValue) public onlyOwner{
        setPlatFormValue(_platformValue);
    }
    ...
    function saleToken(address payable from, uint256 tokenId, uint256 amount, address token, uint256 _type, uint256 NOFToken, string memory nft) public payable{
       require(amount == order_place[from][tokenId].price.mul(NOFToken) , "Insufficent Balance");
       _saleToken(from, tokenId, amount, "BNB", nft);
       saleTokenTransfer(from, tokenId, token, _type, NOFToken);
    }
    function saleTokenTransfer(address payable from, uint256 tokenId, uint256 amount, address token, uint256 _type, uint256 NOFToken, string memory tokenAss, string memory nft) public{
        require(amount == order_place[from][tokenId].price.mul(NOFToken) , "Insufficent fund");
        _saleToken(from, tokenId, amount, tokenAss, nft);
        saleTokenTransfer(from, tokenId, token, _type, NOFToken);
    }
    ...
    function buyMicroOwner(address tokenAddress,uint256 slot) public payable{
        BEP20 t = BEP20(tokenAddress);
        t.transfer(msg.sender, slot);
        address payable admin = address(uint160(owner()));
        admin.transfer(msg.value);
    }
    function claim(uint256 slot, uint256 val, address tokenAddress) public{
        BEP20 t = BEP20(tokenAddress);
        if(msg.sender == owner()){
            ...
            require(t.balanceOf(msg.sender) == slot, "Not Eligible to claim");
            t.transferFrom(msg.sender, address(this), slot);
            address payable user= address(uint160(msg.sender));
            user.transfer(val);
        }
    }
