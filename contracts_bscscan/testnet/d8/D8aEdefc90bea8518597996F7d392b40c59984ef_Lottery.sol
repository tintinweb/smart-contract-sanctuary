...
contract Lottery is LotteryOwnable, Initializable {
    ...
    // default fee rate : 15%
    uint16 public feeRate = 1500;
    // default Max fee fee rate: 20%
    uint16 public constant MAXIMUM_FEE_RATE = 2000;
    ...
    // Update lottery fee rate by the owner
    function setFeeRate(uint16 _feeRate) public onlyAdmin {
        require(_feeRate <= MAXIMUM_FEE_RATE, "setFeeRate: invalid FEE rate basis points");
        feeRate = _feeRate;
    }
   
    // Update the reward token contract address by the owner
    function setRewardToken(IERC20 _epic) public onlyAdmin {
        epic = _epic;
    }

    // Update the totalAmount reward by the owner
    function setTotalAmount(uint256 _totalAmount) public onlyAdmin {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        //lottery fee
        uint256 feeAmount = _price.mul(feeRate).div(10000);
        //fee
        epic.safeTransferFrom(address(msg.sender), adminAddress, feeAmount);
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        //lottery fee
        uint256 feeAmount = totalPrice.mul(feeRate).div(10000);
        //fee
        epic.safeTransferFrom(address(msg.sender), adminAddress, feeAmount);
