...
contract ChuyentestVault is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public minDeposit = uint256(4000 ether);
    ...
    function pendingRewards(uint256 pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && depositedAmount != 0) {
            ...
            uint256 eulerReward = multiplier.mul(eulerPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEulerPerShare = accEulerPerShare.add(eulerReward.mul(1e12).div(depositedAmount));
        }
        return user.amount.mul(accEulerPerShare).div(1e12).sub(user.rewardDebt).add(user.pendingRewards);
    }
    ...
    function updatePool(uint256 pid) internal {
        ...
        pool.accEulerPerShare = pool.accEulerPerShare.add(eulerReward.mul(1e12).div(depositedAmount));
        pool.lastRewardBlock = block.number;
    }
    ...
    function claim(uint256 pid) public {
        ...
        if (pending > 0 || user.pendingRewards > 0) {
            user.pendingRewards = user.pendingRewards.add(pending);
            uint256 claimedAmount = safeEulerTransfer(msg.sender, user.pendingRewards, pid);
    ...
    function safeEulerTransfer(address to, uint256 amount, uint256 pid) internal returns (uint256) {
        PoolInfo memory pool = poolInfo[pid];
        if (amount > pool.rewardsAmount) {
            euler.safeTransfer(to, pool.rewardsAmount);
