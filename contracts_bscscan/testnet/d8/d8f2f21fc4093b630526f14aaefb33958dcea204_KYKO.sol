...
contract KYKO is Context, IBEP20, Ownable {
    ...
    string private constant _name     = "Kyko";
    string private constant _symbol   = "KYKO";
    uint8  private constant _decimals = 10;
    
    uint256 public _holderFee       = 1000; // 10% of every transaction is redistributed to holders
    uint256 public _devFee = 100; // 1% of every transaction is sent to dev wallet
    uint256 public _dev2Fee   = 100; // 1% of every transaction is sent to dev2 wallet

    uint256 private _previousTaxFee       = _holderFee;
    uint256 private _previousMarketingFee = _devFee;
    uint256 private _previousCharityFee   = _dev2Fee;
    ...
    constructor (address cOwner, address marketingWallet, address charityWallet) Ownable(cOwner) {
        _devWallet = marketingWallet;
        _dev2Wallet   = charityWallet;
        ...
        _isExcludedFromFee[_dev2Wallet]   = true;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        takeTransactionFee(address(_devWallet), tMarketing, currentRate);
        takeTransactionFee(address(_dev2Wallet), tCharity, currentRate);
    ...
    function removeAllFee() private {
        if (_holderFee == 0 && _devFee == 0 && _dev2Fee == 0) return;
        
        _previousTaxFee       = _holderFee;
        _previousMarketingFee = _devFee;
        _previousCharityFee   = _dev2Fee;
    ...
    function restoreAllFee() private {
        ...
        _devFee = _previousMarketingFee;
        _dev2Fee   = _previousCharityFee;
    }
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tFee       = tAmount.mul(_holderFee).div(10000);
        uint256 tMarketing = tAmount.mul(_devFee).div(10000);
        uint256 tCharity   = tAmount.mul(_dev2Fee).div(10000);
