...
contract LotteryToken is Context, IERC20, Ownable {
    ...
    uint256 public timeWeek = 20 * 60; //20 min
    uint256 public timeMonth = 30 * 60; //30 min
    

    uint256 public totalWeeklyBurnToken = 0;
    uint256 public lastTimeWeeklyBurn = 0;

    bool public lotteryStarted = true;
    ...
    function _transfer(
        ...
        if(!inSwapAndLiquify ){
            if (lotteryStarted && block.timestamp.div(timeMonth) > checkpointTier1 && _currentTier1LotteryAward > 0){
                loteryDraw(1);
                checkpointTier1 = block.timestamp.div(timeMonth);
            }else if (lotteryStarted && block.timestamp.div(timeWeek) > checkpointTier2  && _currentTier2LotteryAward > 0){
                loteryDraw(2);
                checkpointTier2 = block.timestamp.div(timeWeek);
            }else if (lotteryStarted && block.timestamp.div(timeDay) > checkpointTier3  && _currentTier3LotteryAward > 0){
                loteryDraw(3);
                checkpointTier3 = block.timestamp.div(timeDay);
    ...
    function sendLotteryAward(address winner, uint256 tokenAmount) private{
        uint256 initialBalance = address(this).balance;
        ...
        if(tier == 1){
            ...
            while(found == false){
                ...
                if (current.sub(tier2TimeStamp[tier2Cadidates[winnerIndex]]) >= tier2QualifiedTime){
                    ...
                    sendLotteryAward(lastTier2Winner, lastTier2Award);
                    found = true;
                }
            }
        }else if(tier == 3){
            while(found == false){
                ...
                if (current.sub(tier3TimeStamp[tier3Cadidates[winnerIndex]]) >= tier3QualifiedTime){
                    ...
                    sendLotteryAward(lastTier3Winner, lastTier3Award);
                    found = true;
                }
            }
        }
    ...
    function havestCakeAndBuyBack() private lockTheSwap{
        if(investedCake){
