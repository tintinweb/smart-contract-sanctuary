...
contract Token is ERC20 {
    uint256 private ethBurn  = 1 * 10 ** 16;
    uint256 private power0   = 1000;
    uint256 private power1   = 700;
    uint256 private power2   = 500;
    uint256 private power3   = 300;
    uint256 private power4   = 100;
    uint256 private power5   = 70;
    uint256 private power6   = 50;
    uint256 private power7   = 30;
    uint256 private power8   = 10;
    uint256 private power9   = 7;
    uint256 private power10  = 5;
    
    uint256 private sec9Rate = 10416666 * 10 ** 12;  // 1000 power 9second = 10416.666
    uint256 private timeLast = 86400;
    uint256 private backRate = 0;             // 10% coin to admin, when claim
    uint256 private maxnum   = 30 * 10 ** 33;
    ...
    mapping (address => address[])  private team8; // user -> teams8
    mapping (address => address[])  private team9; // user -> teams9
    mapping (address => address[])  private team10; // user -> teams10
    ...
    constructor() ERC20("Christmas Coin", "Chc") {
    ...
    function getTeam8(address addr) public view returns (address[] memory) {
        return team8[addr];
    }
    
    function getTeam9(address addr) public view returns (address[] memory) {
        return team9[addr];
    }
    
    function getTeam10(address addr) public view returns (address[] memory) {
        return team10[addr];
    }
    
    function getData(address addr) public view returns (uint256[19] memory, address, address,uint256[10] memory) {
        ...
        uint256[10] memory teamArr=[team1[addr].length,team2[addr].length,team3[addr].length,team4[addr].length,team5[addr].length,team6[addr].length,team7[addr].length,team8[addr].length,team9[addr].length,team10[addr].length];
    ...
    function sumInvitePower(address addr) public view returns (uint256) {
        ...
        for (uint256 i=0; i<team6[addr].length; i++) {
            address team = team6[addr][i];
            if (data[team][0] + timeLast > block.timestamp) {
                total += power6;
            }
        }
        for (uint256 i=0; i<team7[addr].length; i++) {
            address team = team7[addr][i];
            if (data[team][0] + timeLast > block.timestamp) {
                total += power7;
            }
        }
        for (uint256 i=0; i<team8[addr].length; i++) {
            address team = team8[addr][i];
            if (data[team][0] + timeLast > block.timestamp) {
                total += power8;
            }
        }
        
        for (uint256 i=0; i<team9[addr].length; i++) {
            address team = team9[addr][i];
            if (data[team][0] + timeLast > block.timestamp) {
                total += power9;
            }
        }
        
        for (uint256 i=0; i<team10[addr].length; i++) {
            address team = team10[addr][i];
            if (data[team][0] + timeLast > block.timestamp) {
                total += power10;
            }
        }
        ...
        if (boss[_msgSender()] == address(0) && _msgSender() != invite && invite != address(0)) {
            ...
            if (invite2 != address(0)) {
                ...
                if (invite2 != address(0)) {
                    ...
                    invite2 = boss[invite2];
                if (invite2 != address(0)) {
                    team8[invite2].push(_msgSender());
                    invite2 = boss[invite2];
                if (invite2 != address(0)) {
                    team9[invite2].push(_msgSender());
                    invite2 = boss[invite2];
                if (invite2 != address(0)) {
                    team10[invite2].push(_msgSender());
                    }
                    }
                    }
                 }    
