 ...
 *    | $$  | $$| $$_____/| $$      | $$  | $$| $$  | $$| $$  | $$| $$  | $$| $$_____/    *
 *    | $$  | $$|  $$$$$$$| $$      |  $$$$$$/| $$$$$$$/|  $$$$$$/|  $$$$$$$|  $$$$$$$    *
 *    |__/  |__/ \_______/|__/       \______/ |_______/  \______/  \____  $$ \_______/    *
 *                                                                 /$$  \ $$              *
 *                                                                |  $$$$$$/              *
 *                                                                 \______/               *
 ******************************************************************************************/
...
library Address {
    ...
    function isContract(address account) internal view returns (bool) {
        ...
        bytes32 codehash;
		//Need to keep this hash address the same all the time
...
contract HeroDogeCoin is Context,IERC20,Ownable {
    ...
    constructor (string memory NAME, string memory SYMBOL, uint256 DECIMALS, uint256 SUPPLY, uint256 taxFee, uint256 LPFee, uint256 charityFee, address charityWallet, uint256 maxTaxAmount, uint256 maxSellAmountAllowed, address routerAddress, address heroDogeOwner) public {
        _name = NAME;
        _symbol = SYMBOL;
        _decimals = DECIMALS;
        ...
        _taxFee = taxFee;
        _liquidityFee = LPFee;
        //This is Charity and Marketing Fee
		_charityFee = charityFee; 
		//EX: 0x4eF9A651F8656DEf8454178406eEae16FB7Ca458;
		_charityWallet = charityWallet;
		_previousCharityFee = _charityFee;
        _previousTaxFee = _taxFee;
        _previousLiquidityFee = LPFee;
        _maxTxAmount =  maxTaxAmount * 10 ** _decimals;
        numTokensSellToAddToLiquidity = maxSellAmountAllowed * (10 ** _decimals);
        _rOwned[heroDogeOwner] = _rTotal;
        ...
        _isExcludedFromFee[heroDogeOwner] = true;
        _isExcludedFromFee[address(this)] = true;
        _owner = heroDogeOwner;
        emit Transfer(address(0), heroDogeOwner, _tTotal);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        //Calculate charity/marketing amount
        uint256 charityAmt = amount.mul(_charityFee).div(100);

        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(charityAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(charityAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(charityAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(charityAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(charityAmt)));
        }
	...
	//Set dynamic router address for new version (EX: PancakeSwap V2 address: 0x10ED43C718714eb63d5aA57B78B54704E256024E)
