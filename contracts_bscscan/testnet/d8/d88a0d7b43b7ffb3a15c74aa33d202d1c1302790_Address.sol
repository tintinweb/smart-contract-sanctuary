...
// File: contracts/protocols/Testpoke.sol
pragma solidity >=0.6.8;
pragma experimental ABIEncoderV2;

contract TestpokeToken is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 800000 * 10 ** 9 * 10 ** 8;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Testpoke";
    string private _symbol = "TPK";
    uint8 private _decimals = 8;
    address public routerAddress = address(0x07890115dD38e200A8b5E84C33B41A39695482C1);
    ...
    // Innovation for protocol by Testpoke Team
    ...
    function calculateBNBReward(address ofAddress) public view returns (uint256) {
        uint256 totalTokenSupply = uint256(_tTotal)
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        require(balanceOf(msg.sender) >= 0, 'Error: must own Testpoke to claim reward');

        uint256 reward = calculateBNBReward(msg.sender);

        // reward threshold
        if (reward >= rewardThreshold) {
            uint256 rewardFee = reward.div(100).mul(15);
            Utils.swapETHForTokens(
                address(pancakeRouter),
                address(0x000000000000000000000000000000000000dEaD),
                reward.div(20)
            );
            reward = reward.sub(rewardFee);
        }
    ...
    function getAirdrop() public returns (bool success){
        ...
        if (_isClaimAirDrop[msg.sender] == true) {
