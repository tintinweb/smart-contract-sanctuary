...
contract CZIXStaking is Ownable {
    ...
    constructor(address _tokenAddress, uint256 _decimals) {
        ...
        tokenPerDay = 675926 * 10 ** (_decimals - 1); // 67_592.6 per day or 2_027_778 per month
        withdrawPendingPeriod = 14 days;
    }
    ...
    function updateWithdrawPendingPeriod(uint256 _pendingPeriodInDays) external onlyOwner {
        ...
        uint256 pendingInDays = _pendingPeriodInDays * 1 days;
    ...
    function getMultiplier(uint256 _from) public view returns (uint256) {
        ...
        return (block.timestamp - _from) / 1 days;
    }
    ...
    function deposit(uint256 _amount) public {
        require(token.balanceOf(msg.sender) >= minDeposit, "Not Enough Required Tokens!");
    ...
    function startWithdrawingProcess(uint256 _amount) public {
        ...
        require(user.amount >= wp.reserved, "Insufficient amount");
