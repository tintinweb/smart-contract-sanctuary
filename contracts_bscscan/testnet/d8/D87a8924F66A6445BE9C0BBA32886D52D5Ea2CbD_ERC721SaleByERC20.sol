...
contract ERC721SaleByERC20 is Ownable, IERC721Receiver, AbstractSale {
    using AddressLibrary for address;
    using UintLibrary for uint256;
    using StringLibrary for string;
    uint private constant maxBuyerFeePercentage = 10000;
    ...
    event Buy(address indexed token, uint256 indexed tokenId, address seller, address buyer, uint256 price, uint256 nonce, address erc20);

    TransferProxy public transferProxy;
    ERC20TransferProxy public erc20TransferProxy;
    ERC721SaleNonceHolder public nonceHolder;

    constructor(TransferProxy _transferProxy, ERC721SaleNonceHolder _nonceHolder, ERC20TransferProxy _erc20TransferProxy, address payable beneficiary) AbstractSale(beneficiary) public {
    ...
    function buy(IERC721 token, uint256 tokenId, address erc20, uint256 price, uint256 sellerFee, Sig memory signature) public payable {
        ...
        require(erc20TransferProxy.userBalance(IERC20(erc20), msg.sender) == price.add(buyerFeeValue), "Insufficient Balance" );
        string memory messageSigned = prepareMessage(address(token), tokenId, price, erc20, sellerFee, nonce);
        ...
        uint256 sellerFeeToSlash = transferERC20FeeToBeneficiary(price, sellerFee, erc20);
        transferERC20(token, tokenId, erc20, price, sellerFeeToSlash);
        nonceHolder.setNonce(token, tokenId, nonce + 1);
        emitERC20Sale(token, tokenId, owner, price, nonce + 1, erc20);
    }
    
    function emitERC20Sale(IERC721 token, uint256 tokenId, address owner, uint256 price, uint256 nonce, address erc20) internal {
        emit Buy(address(token), tokenId, owner, msg.sender, price, nonce, erc20);
    }
     
    function transferERC20(IERC721 token, uint256 tokenId, address _erc20, uint256 price, uint256 sellerFeeToSlash) internal returns (uint256) {
        address payable owner = address(uint160(token.ownerOf(tokenId)));
        
        // Transfer Createor Royalty to Creator
        uint256 creatorFee = getCreatorFee(address(token), tokenId);
        address creator = getCreator(address(token), tokenId);
        uint256 creatorSlash = price.mul(creatorFee).div(maxBuyerFeePercentage);
        erc20TransferProxy.erc20safeTransferFrom(IERC20(_erc20), msg.sender, creator, creatorSlash);
        
        // Transfer Seller
        uint256 sellerSlash = price.sub(creatorSlash).sub(sellerFeeToSlash);
        erc20TransferProxy.erc20safeTransferFrom(IERC20(_erc20), msg.sender, owner, sellerSlash);
    }
    
    function getCreatorFee(address erc721Adddress, uint256 tokenId) internal view returns (uint256) {
        uint256[] memory fees = ERC721TokenInterface(erc721Adddress).getFeeBps(tokenId);
        return fees[0];
    }
    
    function getCreator(address erc721Adddress, uint256 tokenId) internal view returns (address) {
        address[] memory creators = ERC721TokenInterface(erc721Adddress).getFeeRecipients(tokenId);
        return creators[0];
    }
    ...
    function prepareMessage(address token, uint256 tokenId, uint256 price, address _erc20, uint256 fee, uint256 nonce) internal pure returns (string memory) {
        return prepareMessage(token, tokenId, price, fee, nonce).append(". erc20: ", _erc20.toString());
    }
