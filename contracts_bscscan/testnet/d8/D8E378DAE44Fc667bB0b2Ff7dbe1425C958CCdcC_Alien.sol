...
contract Alien is ERC721Enumerable {
    ...
    function mint_monster(uint _monster) private{
        ...
        generate(next_monster, rand);
        _safeMint(msg.sender, _next_monster);
        emit monstered(msg.sender, _next_monster);
    }

    // function mint_birdbodydragonheadgod() public payable{
    //     require(msg.value == 1, "1BNB IS REQUIRED");
    //     birdbodydragonheadgod_minted++;
    //     next_monster ++;
    //     uint _next_monster = next_monster;
    //     class[_next_monster] = 1;
    //     uint rand = uint(keccak256(abi.encodePacked(_next_monster)));
    //     generate(next_monster, rand);
    //     _safeMint(msg.sender, _next_monster);
    //     emit monstered(msg.sender, _next_monster);
    ...
    function generate(uint256 tokenId, uint256 rand) internal returns (Component memory t) {
        t = selectTraits(rand);
        ...
        return generate(tokenId, uint(keccak256(abi.encodePacked(next_monster))));
    }
    
    function selectTrait(uint16 rand, uint8 traitType) internal view returns (uint8) {
        uint8 trait = uint8(rand) % uint8(birdbodydragonheadgod[traitType].length);
        return birdbodydragonheadgod[traitType][trait];
    }
    ...
    function selectTraits(uint256 rand) internal view returns (Component memory t) {    
        t.bkgd = selectTrait(uint16(rand % birdbodydragonheadgod[0].length), 0);

        t.head = selectTrait(uint16(rand % birdbodydragonheadgod[1].length), 1);

        t.neck = selectTrait(uint16(rand % birdbodydragonheadgod[2].length), 2);

        t.body = selectTrait(uint16(rand % birdbodydragonheadgod[3].length), 3);
  
        t.arm = selectTrait(uint16(rand % birdbodydragonheadgod[4].length), 4);

        t.hand = selectTrait(uint16(rand % birdbodydragonheadgod[5].length), 5);
  
        t.leg = selectTrait(uint16(rand % birdbodydragonheadgod[6].length), 6);
    
        t.foot = selectTrait(uint16(rand % birdbodydragonheadgod[7].length), 7);
    
        t.clothing = selectTrait(uint16(rand % birdbodydragonheadgod[8].length), 8);
  
        t.tail = selectTrait(uint16(rand % birdbodydragonheadgod[9].length), 9);
      
        t.attribute = selectTrait(uint16(rand % birdbodydragonheadgod[10].length), 10);
    }     
