...
contract Distribute is Ownable, IDistribute {
    ...
    uint256 private maxBonusPercent = 50000000000000000;
    ...
    function getMaxBonusPercent() public view returns(uint256) {
    ...
    function getUnionManagersBonusInfo (
        ...
        BonusInfo[] memory bonus = new BonusInfo[](managersLength);
        for(uint256 idx = 0; idx < managersLength; idx++) {
            bonus[idx].accountId = managers[idx];
            bonus[idx].bonusPercent = unoinsManagersBonusPercent[unionId][managers[idx]];
        }
        return bonus;
    }
    ...
    function batchClaimReward(
        ...
        for (uint256 idx; idx < batchAccountIds.length; idx++) {
            ...
            uint256 trueRewards = _earned(unionId, accountId, poolStartTime);
            if (trueRewards > 0) {
    ...
    function setManagerBonusPercent(
        ...
        uint256 ownerAccountId = IERC721(accountContract).tokenOfOwnerByIndex(msg.sender,0);
        uint256 iUnoinDentity = getMemberIdentity(unionId, ownerAccountId);
        require(iUnoinDentity > 0, "Set failed, you is not in this union");
        require(iUnoinDentity == 1, "Set failed, you do not have this permission");
        uint256 iAccountDentity = getMemberIdentity(unionId, accountId);
        require(iAccountDentity > 0, "Set failed, account is not in this union");
        require(iAccountDentity == 1 || iAccountDentity == 2, "Set failed, account do not have this permission");
        uint256 expectBonusPercent = getUnionBonusPercent(unionId).sub(unoinsManagersBonusPercent[unionId][accountId]).add(bonusPercent);
        require(expectBonusPercent < maxBonusPercent,"Set failed, exceed max bonus percent");
        ...
        for(uint256 idx = 0; idx < managers.length-removeCount; idx++) {
            ...
            if(identity != 1 && identity != 2) {
                for(uint256 rev = managers.length-removeCount-1; rev > idx; rev--) {
                    ...
                    if(revIdentity != 1 && revIdentity != 2) {
                        removeCount++;
                    } else {
    ...
    function earned(
        ...
        uint256 nowTime = block.timestamp;
        uint256 poolStartTime = nowTime-((nowTime-initializeTime)%duration);
        return _earned(unionId, accountId, poolStartTime);
    }
    ...
    function _earned(
        ...
        if (unoinsPools[unionId].length == 2) {
            uint256 identity = getMemberIdentity(unionId, accountId);
            if (identity == 1 || identity == 2) {
                trueRewards = unionManagersRewards[unionId][accountId];
            }
        }
        // 获取贡献值占比
        return trueRewards;
    ...
    function unionWarRewards(
        ...
        uint256 poolRewardsAmount = rewardsAmount;
        uint256[] storage managers = unoinsManagers[unionId];
        for(uint256 idx = 0; idx < managers.length; idx++) {
            uint256 accountId = managers[idx];
            uint256 identity =  getMemberIdentity(unionId, accountId);
            if(identity == 1 || identity == 2) {
                uint256 managerRewardsAmount = rewardsAmount.mul(unoinsManagersBonusPercent[unionId][accountId]).div(1e18);
                unionManagersRewards[unionId][accountId] = unionManagersRewards[unionId][accountId].add(managerRewardsAmount);
                poolRewardsAmount = poolRewardsAmount.sub(managerRewardsAmount);
            }
        }
        ...
        if (pools[0].startTime == poolStartTime) {
            ...
            pools[0].rewardsAmount = pools[0].rewardsAmount.add(pools[1].rewardsAmount);
