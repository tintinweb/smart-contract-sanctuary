...
contract BNBDaddy {
	...
	function getSiteInfo() public view returns(uint256 _totalBet, uint256 _totalBetAmount, uint256 _totalWin, uint256 _totalWinAmount) {
		return(totalBet, totalBetAmount, totalWin, totalWinAmount);
	}

	function getUserInfo(address userAddress) public view returns(uint256 _Bet, uint256 _BetAmount, uint256 _Win, uint256 _WinAmount) {
        User storage user = users[userAddress];
		return(user.Bet, user.BetAmount, user.Win, user.WinAmount);
	}
