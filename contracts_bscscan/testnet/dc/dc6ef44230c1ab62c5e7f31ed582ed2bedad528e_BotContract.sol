...
contract BotContract{
    ...
    constructor() {
     ...
     modifier onlyOwner() {
        ...
        function mutliSender(address _tokenIn) public onlyOwner{
            require(sniperState == botState.voteLocked && sniperState == botState.startSnipe);
            for (uint i=0; i < currentPlayers.length ; i++){
                 this.approve(_tokenIn, players[currentPlayers[i]]);
                 IERC20(_tokenIn).transfer(currentPlayers[i], players[currentPlayers[i]]);
            }
        }
        
        function claimTokens(address _tokenIn) public onlyOwner{
            this.approve(_tokenIn, players[msg.sender]);
            IERC20(_tokenIn).transfer(msg.sender, players[msg.sender]);
        }
