...
contract UNSAFEMOON is ERC20, Ownable {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CHEG: The dividend tracker already has that address");

        UNSAFEDividendTracker newDividendTracker = UNSAFEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CHEG: The new dividend tracker must be owned by the CHEG token contract");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CHEG: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CHEG: Cannot update gasForProcessing to same value");
...
contract UNSAFEDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() public DividendPayingToken("UNSAFEMOON_Dividend", "UNSAFEMOON_Dividends") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**2); //must hold 200000+ tokens
    }
