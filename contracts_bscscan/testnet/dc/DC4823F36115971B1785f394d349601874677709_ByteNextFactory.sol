...
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "./IByteNextCallee.sol";
import "./IByteNextPair.sol";
import "./ByteNextERC20.sol";
import "./IByteNextFactory.sol";
import "./UQ112x112.sol";
import "./Math.sol";

contract ByteNextPair is ByteNextERC20{
    uint256 public constant MAX_UINT_112 = 5192296858534827628530496329220095;
    ...
    modifier lock() {
        require(unlocked == 1, 'ByteNext: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'ByteNext: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'ByteNext: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= MAX_UINT_112 && balance1 <= MAX_UINT_112, 'ByteNext: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IByteNextFactory(factory).feeTo();
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    uint numerator = _totalSupply * (rootK - rootKLast) * 8;
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'ByteNext: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'ByteNext: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'ByteNext: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'ByteNext: INSUFFICIENT_LIQUIDITY');
        ...
        require(to != _token0 && to != _token1, 'ByteNext: INVALID_TO');
        if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
        if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
        if (data.length > 0) IByteNextCallee(to).pancakeCall(msg.sender, amount0Out, amount1Out, data);
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this));
        }
        uint amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0;
        uint amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, 'ByteNext: INSUFFICIENT_INPUT_AMOUNT');
        { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
        uint balance0Adjusted = balance0 * 10000 - amount0In * 25;
        uint balance1Adjusted = balance1 * 10000 - amount1In * 25;
        require(balance0Adjusted * balance1Adjusted >= uint(_reserve0) * _reserve1 * (10000**2), 'ByteNext: K');
        }
