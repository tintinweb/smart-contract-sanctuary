...
contract MyMasterWarCore is ERC721, Ownable {
    ...
    event Destroy(uint256 indexed nftId, uint256 time);
        ...
        returns (uint256)
    {
        ...
        myMasterWars[_nftId] = MyMasterWar(_gene, block.timestamp);

        //charge fee
        coreFee.chargeBornFee(_toAddress, _nftId, _gene);

        emit Born(msg.sender, _nftId, _gene, block.timestamp);
    ...
    function evolve(uint256 _nftId, uint256 _newGene) external {
        ...
        emit Evolve(_nftId, oldGene, _newGene, block.timestamp);
    }
    ...
    function destroy(uint256 _nftId) external {
        ...
        emit Destroy(_nftId, block.timestamp);
    }
