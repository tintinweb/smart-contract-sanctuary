...
contract AMZEToken is ERC20, ERC20Burnable, ERC20Snapshot, Ownable, Pausable, AccessControl, ERC20Permit   {
    ...
    function transferLockedTokens(address recipient, uint totalAmount, uint256 lockedAmount, uint128 startDate, uint64 timeInterval, uint256 tokenRelease) public onlyOwner{
        uint256 _totalAmount = totalAmount * (10 ** uint256(decimals()));
        uint256 _lockedAmount = lockedAmount * (10 ** uint256(decimals()));
        uint256 _tokenRelease = tokenRelease * (10 ** uint256(decimals()));
        timeLocks[recipient] = TimeLock(_totalAmount, _lockedAmount, startDate, timeInterval, _tokenRelease);
        transfer(recipient, _totalAmount);
    }    
    ...
    function releaseTokens(address account) public {
        ...
        uint256 unlockableAmount = ((uint256(timeLocks[account].tokenRelease) * 1 ether) * steps);
