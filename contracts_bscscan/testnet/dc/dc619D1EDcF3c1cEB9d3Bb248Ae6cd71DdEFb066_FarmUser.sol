...
contract FarmUser is IFarmUser, Initializable, Ownable {
    ...
    uint256 public totalLevelHashrate;
    ...
    // 初始化
    function init(IAppData _appData, IPancake _pancake, address uToken) public onlyOwner {
        appData = _appData;
        pancake = _pancake;
        ...
        coolAddr = appData.getCoolAddr();
        rootInviter = appData.getRootInviter();
    ...
    function upgradeLevel(uint8 levelNo) public override needInit {
        ...
        uint256 hashrate = SafeMath.div(deltaAmount, usdtDecimals);
        levelHashrateMap[user.addr] = SafeMath.add(levelHashrateMap[user.addr], hashrate);
        totalLevelHashrate = SafeMath.add(totalLevelHashrate, hashrate);
        ...
        for (uint8 gen = 1; gen <= 20; gen++) {
            ...
            // 算力返佣
            uint256 hashrateCommission = SafeMath.div(SafeMath.mul(hashrate, commissionRate), 100);  
            levelHashrateMap[genUser.addr] = SafeMath.add(levelHashrateMap[genUser.addr], hashrateCommission);
            totalLevelHashrate = SafeMath.add(totalLevelHashrate, hashrateCommission);
        ...
        emit UpgradeLevel(user.addr, user.levelNo, newLevel.levelNo, deltaAmount);
    }
    ...
    function genCommission(Model.User storage genUser, uint8 gen, uint256 baseAmount) private returns(uint256) {
        uint256 commissionRate = appData.getLevelCommissionRate(genUser.levelNo, gen);
    ...
    function getTotalLevelHashrate() public override view returns(uint256) {
        return totalLevelHashrate;
    }
}
...
interface IFarmUser {
    ...
    function getInviterUser(address userAddr) external returns(address, address, uint8);
    ...
    function getUserByAddr(address userAddr) external returns(address, address, uint8);
    function existUser(address userAddr) external returns(bool);
    function getTotalLevelHashrate() external returns(uint256);
}
...
interface IAppData {
    function validPair(address token0, address token1) external returns(bool);    
    function getScaleMultiple(uint8 scaleType) external returns(uint256);
    function getLevelCommissionRate(uint8 levelNo, uint8 gen) external returns(uint256);
    function getLevel(uint8 levelNo) external returns(Model.Level memory);
    function getCoolAddr() external returns(address);
    function getRootInviter() external returns(address);
    function getLPMultiple() external returns(uint256);
}
...
interface IPancake {
    function getUsdtDecimals() external returns (uint8);
    function getUsdtToken() external returns (address);
    function getPriceBaseUsdt(address token) external returns (uint256);
    function getUsdtLPToken() external returns (address);
    function getLPToken(address token0, address token1) external returns (address);
}
