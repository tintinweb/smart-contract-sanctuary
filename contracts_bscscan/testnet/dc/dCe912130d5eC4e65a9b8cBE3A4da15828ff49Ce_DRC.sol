...
contract DRC is Ownable, Rebasable {
    ...
    string public name = "ABC Protocol";
    ...
    uint256 initSupply = 1 * 10**12 * 10**9;
    ...
    uint256 public _minTokensBeforeSwap = 10 * 10**6 * 10**9;
    ...
    constructor(IPancakeRouter02 pancakeRouter)
        public
        Ownable()
        Rebasable()
    {
    ...
    function _isPancakePairAddress(address _addr)
        internal
        view
        returns (bool)
    {
    ...
    function setFYFee(uint16 fee) external onlyOwner {
        require(
            fee > 2,
            "DRC: Frictionless yield fee should be less than 50%"
        );
        FYFee = fee;
    }
    ...
    function balanceOf(address account) public view returns (uint256) {
        if (_isExcluded[account])
            return
                _tOwned[account].mul(DRCScalingFactor).div(
                    internalDecimals
                );
    ...
    function reflect(uint256 tAmount) public {
        ...
        uint256 TAmount = tAmount.mul(internalDecimals).div(
            DRCScalingFactor
        );
    ...
    function _transfer(
        ...
        require(
            amount > 0,
            "DRC: Transfer amount must be greater than zero"
        );
    ...
    function _transferStandard(
        ...
        if (inSwapAndLiquify) {
            ...
            _reflectFee(
                (fData.rFYFee + fData.rBFee + fData.rCFee),
                (fData.tFYFee + fData.tBFee + fData.tCFee)
            );
    ...
    function _transferStandardSell(
        ...
        if (fData.rRewardFee != 0) {
            _rOwned[rewardAddress] = _rOwned[rewardAddress].add(fData.rRewardFee);
        ...
        )
    {
        ...
        tfeeValues.FYFee = TAmount.div(FYFee);
        ...
        tfeeValues.TransferAmount = _getTValues2(
            TAmount,
            tfeeValues
        );
        return (tfeeValues.TransferAmount, tfeeValues.FYFee, tfeeValues.BurnFee, tfeeValues.RewardFee, tfeeValues.BFee, tfeeValues.CFee);
    }

    function _getTValues2(
        uint256 TAmount,
        feeValues memory tfeeValues
        ...
        )
    {
        ...
        rfeeValues.FYFee = rAmount.div(FYFee);
        ...
        rfeeValues.TransferAmount = _getRValues2(
            rAmount,
            rfeeValues
        );
        return (rfeeValues.TransferAmount, rfeeValues.BurnFee, rfeeValues.FYFee, rfeeValues.RewardFee, rfeeValues.BFee, rfeeValues.CFee);
    }
