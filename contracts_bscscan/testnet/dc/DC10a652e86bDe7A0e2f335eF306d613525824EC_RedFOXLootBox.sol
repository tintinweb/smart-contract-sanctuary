...
contract RedFOXLootBox is VRFConsumerBase, Ownable {
    ...
    mapping(uint256 => PackSettings) packToSettings;
    mapping(address => OpenedPack) ownedPacks;
    mapping(address => uint256) userNonce;
    mapping(bytes32 => address) userRequests;

    event OpenPack(bytes32 requestId, address opener);
     ...
     * fee = 0.1 * 10 ** 18;
     */
    constructor(uint256 basicPackPrice)
        VRFConsumerBase(
    ...
    function expand(uint256 _randomValue, uint256 _n)
        public
        view
    ...
    function openPack(Pack _pack) public payable {
        PackSettings memory pack = packToSettings[uint256(_pack)];
        require(msg.value >= pack.price, "RedFOXLootbox: Underprice");

        bytes32 requestId = getRandomNumber();
        userRequests[requestId] = msg.sender;

        emit OpenPack(requestId, msg.sender);
    }
}