...
contract MakaSwapper is Ownable, ReentrancyGuard {
    ...
    function calculateFeesAndBuyTokens(IBEP20 token, address sender, uint256 amount) external nonReentrant {
        // Calculate fee rate
        uint256 feeRate = calculateFeeRate(sender);
        (uint256 feeAmount, uint256 transferAmount) = getFees(feeRate, amount);
        buyTokensWithToken(token, sender, transferAmount, feeAmount);
    }
