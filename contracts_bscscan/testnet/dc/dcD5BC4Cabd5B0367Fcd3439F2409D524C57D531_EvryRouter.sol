...
contract EvryRouter is IEvryRouter2 {
    ...
    function swapExactTokensForTokens(
        ...
        // Transfer fee amount to platform address
    ...
    function swapTokensForExactTokens(
        ...
        // Transfer fee amount to platform address
        uint256 platformFeeAmount = EvryLibrary.calculatePlatformFee(factory, amounts[0]);
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            IEvryFactory(factory).feeToPlatform(),
        ...
        TransferHelper.safeTransferFrom(
            ...
            msg.sender,
            EvryLibrary.pairFor(factory, path[0], path[1]),
            amounts[0].sub(platformFeeAmount)
        );
        ...
        returns (uint[] memory amounts)
    {
        ...
        uint256 amountInWithoutFee = amounts[0].sub(platformFeeAmount);

        IWETH(WETH).deposit{value: amountInWithoutFee}();
        assert(IWETH(WETH).transfer(EvryLibrary.pairFor(factory, path[0], path[1]), amountInWithoutFee));
        _swap(amounts, path, to);
    }
