...
contract ERC20 is Context, IERC20, Ownable {
    ...
    function _transfer(
     ...
     function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 devPlusMarketing = newBalance.div(_liquidityFee).mul(devAndMarketingDivisor);
        uint256 halfEth = devPlusMarketing.div(2);
        uint256 otherHalfEth = devPlusMarketing.sub(half);

        //Send to Marketing address
        transferToAddressETH(marketingAddress, halfEth);

        //Send to Dev address
        transferToAddressETH(devAddress, otherHalfEth);

        // add liquidity to uniswap
        addLiquidity(otherHalf, newBalance);
        
        emit SwapToDevAndMarketing(newBalance, halfEth.add(otherHalfEth));
...
contract TestERC20 is ERC1363, TokenRecover {
    constructor ()
        ERC1363("TestBNBPay", "TBPAY")
    {
    }
}