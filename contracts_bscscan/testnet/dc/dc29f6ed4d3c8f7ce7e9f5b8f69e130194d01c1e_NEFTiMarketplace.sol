...
contract NEFTiMarketplace is NEFTiMPStorages, ReentrancyGuard , Ownable {
    ...
    // using SafeERC20 for IERC20Details;
    ...
    // IERC20Details internal NEFTi20;
    ...
    // uint256 private ratingValue = 100 * (10**NEFTi20.decimals());
    uint8 private ratingScore = 10;
    ...
    uint8 private directListingFee = 0;                  // FREE
    uint8 private directNegotiateFee = 0;                // FREE
    uint8 private directNegotiateCancellationFee = 5;    // 0.5% x Negotiate Price
    uint8 private directCancellationFee = 0;             // FREE
    uint8 private directTxFee = 8;                       // 0.8% x Item Price

    uint8 private auctionListingFee = 3;                 // 0.3% x Item Price
    uint8 private auctionListingCancellationFee = 5;     // 0.5% x Item Price
    uint8 private auctionBiddingFee = 1;                 // 0.1% x Bid Price
    uint8 private auctionBiddingCancellationFee = 5;     // 0.5% x Bid Price
    uint8 private auctionTxFee = 8;                      // 0.8% x Item Price

    uint8 private contractListingFee = 3;                // 0.3% x Item Price
    uint8 private contractListingCancellationFee = 5;    // 0.5% x Item Price
    uint8 private contractNegotiateFee = 1;              // 0.1% x Negotiate Price
    uint8 private contractTxFee = 8;                     // 0.8% x Item Price
    
    uint256 constant staticPercent = 1000;

    // event Logger(address _log);

    event UpgradeNEFTiMultiTokens(address NEFTiMultiTokens);
    event UpgradeNEFTiToken(address NEFTiToken);
    ...
    function upgradeNEFTiToken(address _NEFTiToken, uint8 _decimals) public onlyOwner {
        NEFTi20 = IERC20(_NEFTiToken);
        NEFTi20_decimals = _decimals;
        emit UpgradeNEFTiToken(_NEFTiToken);
    }

    function upgradeNEFTiMultiTokens(address _NEFTiMultiTokens) public onlyOwner {
        NEFTiMT = INEFTiMultiTokens(_NEFTiMultiTokens);
        emit UpgradeNEFTiMultiTokens(_NEFTiMultiTokens);
    }
    ...
    **/
        ...
        public nonReentrant
    {
        ...
        require (_price < _selling[_sid].price, "Negotiate value is too low!");
    ...
    **/
        ...
        public nonReentrant
    {
        ...
        emit Purchase(
            ...
            _sid,
            _selling[_sid].tokenId,
            /* price  */ _poolNegotiating[_sid][_negotiator].value.div( _poolNegotiating[_sid][_negotiator].amount ),
            /* amount */ _poolNegotiating[_sid][_negotiator].amount,
            1,
            _selling[_sid].seller,
            [ false, false, false, false ],
            uint8(PurchaseStatus.FULFILLED)
    ...
    **/
    ...
    constructor(address _NEFTi20, address _NEFTiMT) {
        ...
        // NEFTi20 = IERC20Details(_NEFTi20);
