 ...
 * Website: https://www.8-bitnostalgia.com/
 *
 */
...
contract EightBit is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, IERC2981, ERC165Storage {
 ...
 */
  ...
  constructor() ERC721("8-bit Nostalgia", "8BIT") {
    // init of all variables
    tokenCounter = 0;
    mintFee = 1 * 10**18; // live 450 * 10**18; // 450 FTM
    maxMint = 350;
    royaltyFee = 10; // as a percentage
    royaltyAddress = msg.sender; // live 0x0413e238c56005ac17F60E4eBDBFC45Fe6fdb0F0; (owner)
    devAddress = msg.sender;
    enableMinter = true; // live false;
    directory = "QmXdPmDJGFXhTFSfr1dCGAprKSVQKDGmoKPZunSrKK1ZNi/";
    mainPayment = msg.sender;
    gandmPaymentOne = msg.sender;
    gandmPaymentTwo = msg.sender;
    gandmPaymentThree = msg.sender;

    // constructor mint of ID 0-14 "aka first 15"
    while (tokenCounter < 15) {
  ...
  function paymentSplitter(uint256 _amount) internal {
    uint256 firstPayment = _amount.div(8); // 50% of 25%
    uint256 secondPayment = _amount.div(16); // 25% of 25%
    uint256 thirdPayment = _amount.div(16); // 25% of 25%
    uint256 leftOver = _amount.sub(firstPayment).sub(secondPayment).sub(thirdPayment); // remaining 75%
    ...
    payable(gandmPaymentThree).transfer(thirdPayment);
  }
  ...
  function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view override(IERC2981) returns (address receiver, uint256 royaltyAmount) {
    ...
    royaltyAmount = _salePrice.div(100).mul(royaltyFee);
  }
