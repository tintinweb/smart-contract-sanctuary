...
contract RedFOXLootBox is VRFConsumerBase, Ownable {
    ...
    // Modifier that check if class is exists
    modifier classHasCards(Class _class) {
        require(
            classToIds[_class].length > 0,
            "RedFOXLootbox: Class empty cards"
        );
        _;
    }
    ...
    function expand(
        ...
        for (uint256 i = 0; i < _n; i++) {
            ...
            _seed = _seed.add(expandedValues[i].mod(NUM_CLASSES));
        }
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomness)
        ...
        override
    {
        (address owner, bytes32 requestId, Pack pack, ) = getRequestOwner(_requestId);
        requestIdToRandomNumber[requestId] = _randomness;
        userSeed[owner] = userSeed[owner].add(_randomness.mod(NUM_CLASSES));
    ...
    function openRandomCards(
        ...
        // Array of card ids. Class to array of ids
        uint256[] memory randomNumbers;
        uint256[] memory cardIds;
        uint256 minted = 0;
        uint256 seed = userSeed[_owner];
        // Get pack settings
        ...
        for (uint256 i = 0; i < guarantees.length; i++) {
            if (guarantees[i] > 0) {
                randomNumbers = pickRandomIn(
                    _randomness,
                    guarantees[i],
                    i
                );
                seed += i;
                cardIds = getRandomCardIdsByClass(Class(i), randomNumbers);
                minted += cardIds.length;
                redFoxCollectible.mintOneBatch(_owner, cardIds, "");
            }
        }

        uint256 remainCards = numberOfDraws.sub(minted);

        if (remainCards > 0) {
            uint256[] memory remainCardIds = new uint256[](remainCards);
            randomNumbers = pickRandomIn(_randomness, remainCards, seed);

            for (uint256 i = 0; i < remainCards; i++) {
                uint256 randomNumber = randomNumbers[i];
                Class cardClass = getClassByProbabilities(randomNumber, classProbabilities);
                remainCardIds[i] = getRandomCardIdByClass(cardClass, randomNumber);
                minted += 1;
            }

            redFoxCollectible.mintOneBatch(_owner, remainCardIds, "");
        }
    }
    ...
    function pickRandomIn(
        ...
        uint256 _numbers,
        uint256 _seed
    ) public view returns (uint256[] memory randomResults) {
        randomResults = expand(
            _randomness,
            _numbers,
            _randomness.mod(_seed.add(NUM_CLASSES))
        );
    }
    ...
    function getRandomCardIdByClass(Class _class, uint256 _number)
        public
        view
        classExists(_class)
        classHasCards(_class)
        returns (uint256 ids)
    {
        uint256 index = _number.mod(classToIds[_class].length);
        return classToIds[_class][index];
    }

    function getClassByProbabilities(uint256 _random, uint16[NUM_CLASSES] memory _classProbabilities) public view returns (Class _class) {
        for (uint256 i = _classProbabilities.length - 1; i > 0; i--) {
            uint256 probability = _classProbabilities[i];
            if (_random < probability) {
                return Class(i);
            } else {
                _random = _random.sub(probability);
            }
        }
