...
contract MakaSwap is Ownable, ReentrancyGuard {
  ...
  constructor(address routerAddress, address maka) {
  ...
  function buyToken() public payable nonReentrant returns(bool) {
    ...
    if (_whitelistedAddresses[msg.sender]) {
      ...
      _pancakeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: tokensReservedForContract}(0, path, address(this), block.timestamp);
      
      address payable makaToken = payable(MAKA);
      makaToken.transfer(tokensReservedForContract);
      (bool successSentRewards,) = MAKA.call{value:tokensReservedForReward}("");
