...
interface IPresaleSetting {
    ...
    function getDexFactoryAddress() external view returns (address);
}
...
contract Presale is ReentrancyGuard {
    ...
    struct PRESALE {
        ...
        PresaleZeroRoundInfo ZERO_ROUND;
    }
    ...
    constructor(address _presaleGenerator) {
        ...
        PRESALE_LOCK_FORWARDER = IPresaleLockForwarder(0xb23cA7C9b1298Ff4e4E56a66fBFfD218870bf551);
        ...
        PRESALE_ITEM.INFO.DEX_FACTORY_ADDRESS = PRESALE_SETTING.getDexFactoryAddress();*/
        WrapToken = IWrapToken(PRESALE_SETTING.getWrapTokenAddress());
    ...
    function setMainInfo(
        ...
        // ZERO ROUND
        PRESALE_ITEM.ZERO_ROUND.TOKEN_ADDRESS = PRESALE_SETTING.getZeroRoundTokenAddress();
        PRESALE_ITEM.ZERO_ROUND.TOKEN_AMOUNT = PRESALE_SETTING.getZeroRoundTokenAmount();
        PRESALE_ITEM.ZERO_ROUND.FINISH_BEFORE_FIRST_ROUND = PRESALE_SETTING.getFinishBeforeFirstRound();
        PRESALE_ITEM.ZERO_ROUND.FINISH_AT = _startTime - PRESALE_ITEM.ZERO_ROUND.FINISH_BEFORE_FIRST_ROUND;
        PRESALE_ITEM.ZERO_ROUND.PERCENT = PRESALE_SETTING.getZeroRoundPercent();
        PRESALE_ITEM.ZERO_ROUND.MAX_BASE_TOKEN_AMOUNT = _hardCap.div(1000).mul(PRESALE_ITEM.ZERO_ROUND.PERCENT);
        PRESALE_ITEM.ZERO_ROUND.MAX_SLOT = PRESALE_ITEM.ZERO_ROUND.MAX_BASE_TOKEN_AMOUNT.div(_limitPerBuyer);
        PRESALE_ITEM.ZERO_ROUND.REGISTERED_SLOT = 0;
    }
    ...
    function getPresaleRound() public view returns (int8) {
        int8 round = - 1;
        if (block.timestamp < PRESALE_ITEM.ZERO_ROUND.FINISH_AT) {
            round = 0;
    ...
    function userDeposit(uint256 _amount) external payable nonReentrant {
        ...
        if (getPresaleRound() < 0) {
            ...
            require(block.timestamp < PRESALE_ITEM.ZERO_ROUND.FINISH_AT, "PRESALE: ZERO ROUND FINISHED");
            require(PRESALE_ITEM.ZERO_ROUND.LIST_USER.contains(msg.sender), "PRESALE: ZERO ROUND NOT REGISTERED");
    ...
    function userWithdrawToken() external nonReentrant {
    ...
    function userWithdrawBaseTokens() external nonReentrant {
        ...
        if (PRESALE_ITEM.ZERO_ROUND.LIST_USER.contains(msg.sender)) {
            TransferHelper.safeTransfer(PRESALE_ITEM.ZERO_ROUND.TOKEN_ADDRESS, msg.sender, PRESALE_ITEM.ZERO_ROUND.TOKEN_AMOUNT);
        }
        ...
        uint256 tokenBought = remainingBaseBalance.mul(buyer.baseDeposited).div(baseRemainingDenominator);
        require(tokenBought > 0, 'NOTHING TO WITHDRAW');
        ...
        TransferHelper.safeTransferBaseToken(address(PRESALE_ITEM.INFO.BASE_TOKEN), payable(msg.sender), tokenBought, !PRESALE_ITEM.INFO.PRESALE_IN_MAIN_TOKEN);
    }
    ...
    function addLiquidity() external nonReentrant {
        ...
        if (PRESALE_ITEM.ZERO_ROUND.REGISTERED_SLOT > 0) {
            uint256 zeroRoundRegisteredTokenAmount = PRESALE_ITEM.ZERO_ROUND.REGISTERED_SLOT.mul(PRESALE_ITEM.ZERO_ROUND.TOKEN_AMOUNT);
            uint256 zeroRoundTokenBalance = IERC20(PRESALE_ITEM.ZERO_ROUND.TOKEN_ADDRESS).balanceOf(address(this));
            ...
            TransferHelper.safeTransfer(PRESALE_ITEM.ZERO_ROUND.TOKEN_ADDRESS, 0x000000000000000000000000000000000000dEaD, zeroRoundTokenBurn);
        }
    ...
    function registerZeroRound() external nonReentrant {
        ...
        require(PRESALE_ITEM.ZERO_ROUND.TOKEN_ADDRESS != address(0), 'PRESALE: ZERO ROUND NOT ACTIVE');
        require(getPresaleRound() == 0, "PRESALE: ZERO ROUND FINISHED");
        require(PRESALE_ITEM.ZERO_ROUND.REGISTERED_SLOT < PRESALE_ITEM.ZERO_ROUND.MAX_SLOT, "PRESALE: ZERO ROUND ENOUGH SLOT");
        require(!PRESALE_ITEM.ZERO_ROUND.LIST_USER.contains(msg.sender), "PRESALE: ZERO ROUND ALREADY REGISTERED");

        TransferHelper.safeTransferFrom(PRESALE_ITEM.ZERO_ROUND.TOKEN_ADDRESS, address(msg.sender), address(this), PRESALE_ITEM.ZERO_ROUND.TOKEN_AMOUNT);
        PRESALE_ITEM.ZERO_ROUND.REGISTERED_SLOT = PRESALE_ITEM.ZERO_ROUND.REGISTERED_SLOT.add(1);
        PRESALE_ITEM.ZERO_ROUND.LIST_USER.add(msg.sender);
    }
    ...
    function getZeroRoundInfo() external view returns (
        ...
        return (
        PRESALE_ITEM.ZERO_ROUND.TOKEN_ADDRESS,
        PRESALE_ITEM.ZERO_ROUND.TOKEN_AMOUNT,
        PRESALE_ITEM.ZERO_ROUND.FINISH_BEFORE_FIRST_ROUND,
        PRESALE_ITEM.ZERO_ROUND.FINISH_AT,
        PRESALE_ITEM.ZERO_ROUND.PERCENT,
        PRESALE_ITEM.ZERO_ROUND.MAX_BASE_TOKEN_AMOUNT,
        PRESALE_ITEM.ZERO_ROUND.MAX_SLOT,
        PRESALE_ITEM.ZERO_ROUND.REGISTERED_SLOT
        );
    }

    // ZERO ROUND USER LENGTH
    function getZeroRoundUserLength() external view returns (uint256) {
        return PRESALE_ITEM.ZERO_ROUND.LIST_USER.length();
    }
    // ZERO ROUND USER AT INDEX
    function getZeroRoundUserAtIndex(uint256 _index) external view returns (address) {
        return PRESALE_ITEM.ZERO_ROUND.LIST_USER.at(_index);
    }
