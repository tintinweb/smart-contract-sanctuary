...
import "../../external/bunny/BunnyPoolInterface.sol";
import "../../external/pancake/IPancakeRouter02.sol";
import "../../external/bunny/IPriceCalculatorBSC.sol";

/// @title Prize Pool with Bunny
/// @notice Manages depositing and withdrawing assets from the Prize Pool
contract BunnyPrizePool is PrizePool {
  using SafeMathUpgradeable for uint256;
  using SafeERC20Upgradeable for IERC20Upgradeable;

  address public constant BUNNY = 0xC9849E6fdB743d08fAeE3E34dd2D1bc69EA11a51;
  address internal constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
  address internal constant REWARD_TOKEN = 0x7Bb89460599Dbf32ee3Aa50798BBcEae2A5F7f6a;
  IPancakeRouter02 private constant ROUTER = IPancakeRouter02(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);
  IPriceCalculatorBSC private constant PRICE_CALCULATOR = IPriceCalculatorBSC(0x542c06a5dc3f27e0fbDc9FB7BC6748f26d54dDb0);

  event BunnyPrizePoolInitialized(address indexed vToken);

  /// @notice Interface for the Yield-bearing vToken by Bunny
  //  CTokenInterface public vToken;
  BunnyPoolInterface public vToken;

  /// @notice Initializes the Prize Pool and Yield Service with the required contract connections
  /// @param _controlledTokens Array of addresses for the Ticket and Sponsorship Tokens controlled by the Prize Pool
  /// @param _maxExitFeeMantissa The maximum exit fee size, relative to the withdrawal amount
  /// @param _maxTimelockDuration The maximum length of time the withdraw timelock could be
  /// @param _vToken Address of the Bunny vToken interface
  function initialize (
    RegistryInterface _reserveRegistry,
    ControlledTokenInterface[] memory _controlledTokens,
    uint256 _maxExitFeeMantissa,
    uint256 _maxTimelockDuration,
    BunnyPoolInterface _vToken
  )
  public
  initializer
  {
    ...
    emit BunnyPrizePoolInitialized(address(vToken));
  }
...
//    require(vToken.deposit(amount) == 0, "BunnyPrizePool/deposit-failed");
    vToken.deposit(amount);
  }
...
//    require(vToken.withdraw(amount) == 0, "BunnyPrizePool/withdraw-failed");
    vToken.withdraw(amount);
  ...
  function _token() internal override view returns (IERC20Upgradeable) {
    return IERC20Upgradeable(BUNNY);
  }

  function earned() external view returns (uint){
    uint256 earned = vToken.earned(address(this));
    (, uint valueInUSD) = PRICE_CALCULATOR.valueOfAsset(address(REWARD_TOKEN), earned);
    return valueInUSD;
  }
  ...
  function getReward() external returns (uint256){
    ...
    vToken.getReward();
    /// swap WBNB to BUNNY
    uint256 balance = IERC20Upgradeable(WBNB).balanceOf(address(this));
    if (balance > 0) {
      address[] memory path = new address[](2);
      path[0] = address(WBNB);
      path[1] = address(BUNNY);
      IERC20Upgradeable(WBNB).safeApprove(address(PRICE_CALCULATOR), balance);
      ROUTER.swapExactTokensForTokens(balance, 0, path, address(this), block.timestamp);
      uint256 reward = IERC20Upgradeable(BUNNY).balanceOf(address(this));
      /// deposit again without ticket
      if (reward > 0) {
        _supply(reward);
        return reward;
      }
...
contract BunnyPrizePoolProxyFactory is ProxyFactory {
  ...
  function create() external returns (BunnyPrizePool) {
    return BunnyPrizePool(deployMinimal(address(instance), ""));
  }
}
