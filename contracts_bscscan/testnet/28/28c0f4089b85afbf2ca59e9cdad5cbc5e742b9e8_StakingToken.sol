...
contract StakingToken is Ownable{
    ...
    mapping ( address => stakingData )   stakingAddress;
    //mapping (uint256 => uint256) public totalStructs;
    
    constructor () {
        MTVToken = ERC20(0x81c0412CFF0Cba4e3d547739Ced00DcBCe1d9c84) ;
        rewardPercent = 100;
        withdrawTimestamp =  86400  ; //1 year in seconds
        limitStack = 2000 ether ; // Staking Limit Token  
        totalStakAmount = 0; 
        Pause = false;
        Owner = msg.sender;
    }
    modifier onlyUser(address _sender) {
        ...
        require(stakingAddress[_sender].time > 0, "User Not Exist");
        _;
    }
    ...
    function staking(uint256 _amount)  public   returns(bool)  {
        ...
        require(!Pause,"Stak is Pause by owner");
        require(stakingAddress[msg.sender].time == 0 , "User exist and cannot staking again");
        require(getStakIsNotFull(0) ,"Stack Is Full Now" );
        require(getStakIsNotFull(_amount) ,"After Staking this much token, Stak Will be full!" );

        stakingAddress[msg.sender]=stakingData(block.timestamp,_amount,0);
    ...
    function claimReward() public onlyUser(msg.sender){
        ...
        require(reward > 0   , "Not Enoght Reward Token" );
    ...
    function exit() public onlyUser(msg.sender){
        ...
        require( block.timestamp >= (stakingAddress[msg.sender].time + withdrawTimestamp)  , "Not in time" );
        if(calculateReward(msg.sender) > 0) {
            claimReward();
        }
        uint balanceOf = getBalance(msg.sender) ;
        MTVToken.transfer(msg.sender,balanceOf);
        stakingAddress[msg.sender]= stakingData(0,0,0) ;
    }

    function calculateReward(address _player)  public view  onlyUser(_player) returns(uint256) {

        require(stakingAddress[_player].time != 0 && stakingAddress[_player].amount != 0   , "User Does not exist");
        uint256 reward;
        ...
        if( block.timestamp >= (stakingAddress[_player].time + withdrawTimestamp )) { 
            ...
            reward = ( nowTime-userTime  ) * rewardPercent / withdrawTimestamp  * userAmount / 100;
        }
    ...
    event ExitwEvent(address _spender,uint256 _amount);  
