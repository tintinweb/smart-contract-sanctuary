...
contract Kryptonect is IBEP20, Ownable {
    ...
    function takeFee(address sender, uint256 amount)
        ...
        returns (uint256)
    {
        uint256 feeAmount = amount.mul(totalFeePerTx(sender, amount)).div(
            feeDenominator
        );
    ...
    function totalFeePerTx(address sender, uint256 amount)
        internal
        returns (uint256 totalFeeAmount)
    {
        ...
        if (_reflectionFee > 0) {
            reflectionFeeAmount = amount.mul(_reflectionFee).div(
                feeDenominator
            );
        ...
        if (_liquidityFee > 0) {
            ...
            _accumulatedLiquidity = _accumulatedReflection.add(
                liquidityFeeAmount
            );
        }
        if (_devFee > 0) {
            devFeeAmount = amount.mul(_devFee).div(feeDenominator);
            _accumulatedDev = _accumulatedReflection.add(devFeeAmount);
        }
        if (_marketFee > 0) {
            marketFeeAmount = amount.mul(_marketFee).div(feeDenominator);
            _accumulatedMarket = _accumulatedReflection.add(marketFeeAmount);
        }
        if (_burnFee > 0) {
        ...
        totalFeeAmount = reflectionFeeAmount
            .add(liquidityFeeAmount)
            .add(devFeeAmount)
            .add(marketFeeAmount)
            .add(burnFeeAmount);
    }
    ...
    function swapBack() internal swapping {
        uint256 dynamic_Liquidity = isOverLiquified(
            targetLiquidity,
            targetLiquidityDenominator
        )
            ? 0
            : _accumulatedLiquidity;
        uint256 totalAmount = dynamic_Liquidity.add(_accumulatedReflection).add(_accumulatedDev).add(_accumulatedMarket);

        uint256 halfLiquidity = dynamic_Liquidity.div(2);

        uint256 amountToSwap = totalAmount.sub(halfLiquidity);
        ...
        uint256 amountBNBBuyback = amountBNB.mul(_accumulatedDev).div(amountToSwap);
        uint256 amountBNBMarketing = amountBNB.mul(_accumulatedMarket).div(amountToSwap);
    ...
    function setNormalFeePercent(
        ...
        feeDenominator = _feeDenominator;
        require(
            reflectionFee.add(liquidityFee).add(devFee).add(
                marketFee
            ) <= feeDenominator / 4,
            "BEP20: Can not be greater than max fee"
        );
