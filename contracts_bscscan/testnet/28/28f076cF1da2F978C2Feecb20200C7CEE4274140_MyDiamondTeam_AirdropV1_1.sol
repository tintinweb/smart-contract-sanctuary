...
contract MyDiamondTeam_AirdropV1_1 {
    ...
    address payable public project; // wallet for tax

    MyDiamondTeam mainContract;

    uint256 public total_airdrops;

    event NewAirdrop(address indexed from, address indexed to, uint256 amount, uint256 timestamp);
    event ClaimAirdrop(address indexed to, uint256 amount, uint256 timestamp);

    struct Airdrop {
        uint256 airdrops_sent;
        uint256 airdrops_unpaid;
        uint256 airdrops_payout;
        uint256 last_airdrop_sent;
        uint256 last_airdrop_received;
    }

    mapping(address => Airdrop) public airdrops;
 
    function initialize(address _mainContract, address _projectWallet) external{
		require(!initialized);
		owner = msg.sender;
		
		AIRDROP = 0; // 0% Tax on airdrop
        mainContractAddress = _mainContract;
        mainContract = MyDiamondTeam(mainContractAddress);    
        project = payable(_projectWallet);
    ...
    function info(address _addr) view external returns(uint256 _contract_balance, uint256 _total_airdrops, uint256 _last_airdrop_sent, uint256 _airdrops_unpaid, uint256 _airdrops_payout) {
        return (address(this).balance, total_airdrops, airdrops[_addr].last_airdrop_sent, airdrops[_addr].airdrops_unpaid, airdrops[_addr].airdrops_payout);
    }

    function setMainContractAddress(address _address) public {
        require(msg.sender == owner, "Admin use only");
        mainContractAddress = _address;
        mainContract = MyDiamondTeam(_address);            
    }

    
    function airdrop(address payable _to) payable external {
        require(started, "contract inactive");
        ...
        // amount in now in contract
        // user has to claim
 
        //User stats
        airdrops[_addr].airdrops_sent += payout; // sender
        airdrops[_addr].last_airdrop_sent = block.timestamp; // sender
        airdrops[_to].airdrops_unpaid += payout; // recipient
    ...
    function teamAirdrop(uint256 teamId, bool excludeOwner) payable external {
        ...
        (MyDiamondTeam.Team memory exdata__team, ) = mainContract.teamInfo(teamId);

        //Make sure _to exists in the system; we increase
        require(exdata__team.owner != address(0), "team not found"); 

        uint256 memberDivider = exdata__team.members.length;
        if(excludeOwner == true){
            memberDivider--;
        }
        uint256 amountDivided = _amount.div(memberDivider);

        for(uint8 i = 0; i < exdata__team.members.length; i++){

            address _to = payable(exdata__team.members[i]);
            if(excludeOwner == true && _to == exdata__team.owner){
                continue;
            }

            //amount already received in airdrop contract. track the divided amount
    
            //User stats
            airdrops[_addr].airdrops_sent += amountDivided; // sender
            airdrops[_addr].last_airdrop_sent = block.timestamp; // sender
            airdrops[_to].airdrops_unpaid += amountDivided; // recipient
            airdrops[_to].last_airdrop_received = block.timestamp; // recipient

            emit NewAirdrop(_addr, _to, amountDivided, block.timestamp);
        }
    ...
    function claim() external {
        ...
        address _addr = msg.sender;
        uint256 payout = airdrops[_addr].airdrops_unpaid;

        require(payout > 0, "nothing to claim");

        // send it to caller wallet
        payable(_addr).transfer(payout);
        // track
        airdrops[_addr].airdrops_payout += payout;
        airdrops[_addr].airdrops_unpaid -= payout;

        emit ClaimAirdrop(_addr, payout, block.timestamp);
    }
...
contract MyDiamondTeam {
    ...
    uint256 public AIRDROP = 0; // 0% Tax on airdrop
    ...
    function airdrop(address payable _to) payable external {
        ...
        users[_to].checkpoint = block.timestamp;
   ...
   function changeTeamName(uint256 teamId, string memory name) external {
        ...
        require(checkAlphaNumericStr(name), "name not allowed");
    ...
    function userInfo2(address _addr) view external returns(uint256 last_airdrop, uint8 teams_counter, TeamInfo[] memory member_of_teams, string memory nickname) {

        return (airdrops[_addr].last_airdrop, user_teams_counter[_addr], user_teams[_addr], nicknames[_addr]);
    }
