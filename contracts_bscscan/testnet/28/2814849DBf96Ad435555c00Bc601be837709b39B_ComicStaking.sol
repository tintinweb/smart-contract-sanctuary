...
contract ComicStaking is Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
    ...
    uint128 public lockupPeriod = 360; // 10 Minutes
...
//0x3496b1bffcbf6d49b9caa0babbe07fc0bba043a6
//0x456b0DF1ceC361b6C1c997992415B606394197A6
    ...
    */
    function stake(uint256[] calldata tokenIds) external whenNotPaused nonReentrant {
        require(tokenIds.length <= 40 && tokenIds.length > 0, "Stake: amount prohibited");
         OuruNFT nftContract = OuruNFT(0x456b0DF1ceC361b6C1c997992415B606394197A6);

        for (uint256 i; i < tokenIds.length; i++) {
            (uint256 level, uint256 tier) = nftContract.getHero(tokenIds[i]);
    ...
    */
    function calculateRewards(uint256 tokenID) public view returns (uint256) {
        ...
        OuruNFT nftContract = OuruNFT(0x456b0DF1ceC361b6C1c997992415B606394197A6);
        (uint256 level, uint256 tier) = nftContract.getHero(tokenID);
        for(uint256 i=0; i < rewardChanges.length; i++) {
            ...
            result = (rewardChanges[i].rewardPerBlock + (level + tier)) * 1/200;  
            rewards += result * blocksPassed;
        }
        return rewards;
    }      
