...
contract DRC is Ownable, Rebasable {
    ...
    string public name = "Doggy Rebase Coin";
    ...
    address public BreedAddress = 0xDEAd000000000000000dEAd0000000000000dEAD;
    uint16 public BFee = 0;
    address public CharityAddress = 0xDEAd000000000000000dEAd0000000000000dEAD;
    ...
    function setCFee(uint16 fee) external onlyOwner {
        require((fee >= 50 || fee == 0), "DRC: Breeding fee should be less than 2%");
        CFee = fee;
    }
    ...
    function setTxFee(uint16 fee) external onlyOwner {
        require(fee <= 2, "DRC: Transaction fee should be less than 2%");
        TX_FEE = fee;
    }

    function setFYFee(uint16 fee) external onlyOwner {
        require((fee >= 50 || fee == 0), "DRC: Frictionless yield fee should be less than 50%");
        FYFee = fee;
    }

    function setSellFee(uint16 fee) external onlyOwner {
        require(fee <= 4, "DRC: Sell fee should be less than 50%");
        SELL_FEE = fee;
    }
