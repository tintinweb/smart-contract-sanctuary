...
contract NAMarket is Ownable, ReentrancyGuard, ERC1155Receiver {
  ...
  */
  ...
  //Approval of ownership rights for sale
  function approvedNFTToken(address _contractAddress) public nonReentrant{
      IERC721(_contractAddress).setApprovalForAll(address(this), true);
      emit NFTApproved(_contractAddress);
  }

  function createAuction(address _contractAddress, uint256 _tokenId, uint256 _startingPrice,
    uint256 _buyNowPrice, uint256 expiryDate, TokenType tokenType, uint256 _quantity
    ) public nonReentrant 
    returns(uint256 auctionId){
      ...
      auctions[auctionId] = Auction(_contractAddress, _tokenId, _startingPrice, _buyNowPrice, msg.sender,
       address(0), AuctionStatus.OPEN, expiryDate, auctionId, tokenType, quantity);
