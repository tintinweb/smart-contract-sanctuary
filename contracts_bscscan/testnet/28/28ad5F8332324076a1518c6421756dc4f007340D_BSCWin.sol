...
contract BSCWin is Context, IERC20, Ownable {
    ...
    function sendPresaleTokensFixed(address[] memory users, uint256 amount) public {
        require(msg.sender == owner());
        for(uint i; i < users.length; i++) {
            _tokenTransfer(msg.sender,users[i], amount, false);
            // presale[users[i]].isPreseller = true;
            // presale[users[i]].dayStartTime = block.timestamp;
            // presale[users[i]].presaleAmount = amount;
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (newBalance > 2) {
            ...
            // IERC20(WBNB).transfer(DAOWallet, newBalance.mul(20).div(100));
