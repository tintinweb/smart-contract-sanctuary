...
contract GHCBridge {
    /** Admin account with access to funds in the vault */
    address public admin;

    /** Token that will be withdrawn/deposited. This should be a GHC token. */
    ERC20 public token;
    ...
    function depositFrom(address from, uint256 amount) public {
        ...
        token.transferFrom(from, admin, amount);    // Transfer token from {from} to {admin}
    ...
    function withdrawTo(address to, uint256 amount, uint otherChainNonce) public {
        ...
        token.transferFrom(admin, to, amount);  // Transfer token from {admin} to {to}
