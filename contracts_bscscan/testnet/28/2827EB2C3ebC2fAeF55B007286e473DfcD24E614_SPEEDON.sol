...
contract SPEEDON is IERC20Metadata, DepreciatingFees, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address public marketingAddress = 0x010662d31f566c2D951197c3F8BcD3CCa60166A7; // Marketing Address
    address internal deadAddress = 0x000000000000000000000000000000000000dEaD;
    address public presaleAddress = address(0);
    
    string constant _name = "Speedon";
    string constant _symbol = "SPD";
    uint8 constant _decimals = 18;
    
    uint256 private constant MAX = ~uint256(0);
    uint256 internal constant _totalSupply = 100000 * 10**18;
    ...
    uint256 public autoBuybackAmount = 1 * 10**1;
    ...
    event Print(string funct,uint256 amt);
    event Print(address ads,uint256 amt ,string msg);
    ...
    function _beforeTokenTransfer(address recipient) private {
        ...
        if ( !isInPresale ){
            ...
            emit Print("_beforeTokenTransfer",swapTokensAtAmount);
            emit Print(recipient,swapTokensAtAmount,"recipient");
            if (!swapping && canSwap && swapEnabled  && recipient == pair) {
                swapping = true;
                contractTokenBalance = swapTokensAtAmount;
                swapTokens(contractTokenBalance);
                emit Print("_beforeTokenTransfer 2 swapped",contractTokenBalance);
                swapping = false;
            }
