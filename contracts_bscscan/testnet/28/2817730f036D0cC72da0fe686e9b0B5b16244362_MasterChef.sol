...
contract MasterChef is Ownable, ReentrancyGuard {
     ...
     */
    function add(uint256 _allocPoint, IBEP20 _lpToken,  bool _withUpdate, bool _isSwapPair, uint256 _startingBlock, bool _isStarting, uint16 _customDepositBp ) public onlyAdmin {
        if (_withUpdate) {
     ...
     * Must only be called by the Admin.
     */
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate, bool _isSwapPair, bool _isStarting , uint16 _customDepositBp) public onlyAdmin {
        if (_withUpdate) {
            massUpdatePools(_isStarting);
        }
        ...
        swampWolf.setNewTradingPair(address(poolInfo[_pid].lpToken), _isSwapPair);
        poolInfo[_pid].depositFeeBP = _customDepositBp != 0 ? _customDepositBp : swampWolf.isSwapPair(address(poolInfo[_pid].lpToken)) || poolInfo[_pid].lpToken == swampWolf ? 0 : depositFeeBp;
    }
