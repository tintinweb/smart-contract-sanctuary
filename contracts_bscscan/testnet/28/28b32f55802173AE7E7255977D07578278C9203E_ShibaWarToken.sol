...
contract ShibaWarToken is ERC20, DividendPayingTokenInterface, Ownable {
  ...
  function withdrawDividend(address _holder) public {
    ...
    if (_withdrawableDividend > 0) {
      withdrawnDividends[_holder] = withdrawnDividends[_holder].add(_withdrawableDividend);
  ...
  function calculateFees(address from, address to, uint256 amount) internal virtual view returns(uint256 donation, uint256 superDraw, uint256 dev, uint256 marketing, uint256 rewards, uint256 liquidity) {
    Fees memory fee;
    if(_marketMaker[to]) {
        fee = sell;
    }
  ...
  function _calculateTotalFee(address from, address to, uint256 amount) internal virtual override returns(uint256) {
     (uint256 donation, uint256 superDraw, uint256 dev, uint256 marketing, uint256 rewards, uint256 liquidity) = calculateFees(from, to, amount);
  ...
  function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
    int256 _magCorrection = magnifiedDividendPerShare.mul(amount).toInt256();
  ...
  function swapTokensForEth(uint256 tokenAmount, address to) internal virtual {
