...
contract NFTBox is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Strings for uint256;
    using EnumerableSet for EnumerableSet.UintSet;
    INFTCore public nft;
    ...
    constructor(
        ...
        nftToken = _nftToken;
        saleBoxs[0] = SaleBox(0, 4000 * 10**18, 1629813629, 1661349629);
        saleBoxs[1] = SaleBox(1, 3000 * 10**18, 1629813629, 1661349629);
        saleBoxs[2] = SaleBox(2, 2000 * 10**18, 1629813629, 1661349629);
    }
     ...
     */
    function buyDiamond(uint256 _amount) public payable nonReentrant {
        uint256 typeBox = 0;
     ...
     * @dev Sale Gold Box
     */
    function buyGold(uint256 _amount) public payable nonReentrant {
        uint256 typeBox = 1;
        _buyNFT(typeBox, _amount);
    }
     ...
     * @dev Sale Silver Box
     */
    function buySilver(uint256 _amount) public payable nonReentrant {
        uint256 typeBox = 2;
        _buyNFT(typeBox, _amount);
    }
    ...
    function _buyNFT(uint256 _typeBox, uint256 _amount) internal {
        ...
        NFTItem memory nftItem = NFTItem(tokenId, "gun", block.timestamp);
