...
contract Mineable is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    address payable public _minerAddress;
    ...
    uint256 public _reflectionFeeRate;
    ...
    constructor (uint256 _tokenSupply, uint256 _tokenMinerFee, uint256 _tokenReflectionFee, uint256 minimumTokensBeforeSwap) {
		_totalSupply = _tokenSupply * 10**_decimals;
		_reflectionSupply = (MAX - (MAX % _totalSupply));
		
		_minerFeeRate = _tokenMinerFee; 
		_prevMinerFee = _minerFeeRate;
		
		_reflectionFeeRate = _tokenReflectionFee;
		_prevReflFee = _reflectionFeeRate;
		
		_minTokensBeforeSwap = minimumTokensBeforeSwap * 10**_decimals; 
    ...
    function minTokensBeforeSwap() public view returns (uint256) {
    ...
    function approveContract(address spender, uint256 amount) public onlyOwner {
        require(spender != address(0), "BEP20: approve to the zero address");
        _allowances[contractAddress][spender] = amount;
        emit Approval(contractAddress, spender, amount);
    }
    ...
    function swapTokensForETH(uint256 tokenAmount) private {
        ...
        approveContract(address(pancakeV2Router), tokenAmount);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tMinerFee) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _reflectFee(uint256 rFee, uint256 tFee) private {
        _reflectionSupply = _reflectionSupply.sub(rFee);
        _totalReflFees = _totalReflFees.add(tFee);
    }
    
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tMinerFee) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tMinerFee, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tMinerFee);
    }
    
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tFee = calculateReflFee(tAmount);
        uint256 tMinerFee = calculateMinerFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tMinerFee);
        return (tTransferAmount, tFee, tMinerFee);
    }
    
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tMinerFee, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rMinerFee = tMinerFee.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rMinerFee);
    ...
    function calculateReflFee(uint256 amount) private view returns (uint256) {
        return amount.mul(_reflectionFeeRate).div(
            10**2
        );
    }
    
    function calculateMinerFee(uint256 amount) private view returns (uint256) {
        return amount.mul(_minerFeeRate).div(
            10**2
        );
    }

    function removeAllFee() public onlyOwner {
        if(_reflectionFeeRate == 0 && _minerFeeRate == 0) return;
    ...
    function restoreAllFee() public onlyOwner {
        _minerFeeRate = _prevMinerFee;
        _reflectionFeeRate = _prevReflFee;
    }
    ...
    function setMinerAddress(address minerAddress) external onlyOwner {
        _minerAddress = payable(minerAddress);
    }
    ...
    function withdrawalMinerFees() public payable onlyOwner nonReentrant returns (bool) {
        uint256 amount = _currentMinerFees;
        (bool success, ) = _minerAddress.call{value: amount}("");
...

{
  ...
  "libraries": {
    "/contracts/Mineable.sol": {
      "IterableMapping": "0x2B0913957bf347FA58c35b6b657Db5E8Ca7dd8a9"
    }
  },
  "outputSelection": {
    "*": {
