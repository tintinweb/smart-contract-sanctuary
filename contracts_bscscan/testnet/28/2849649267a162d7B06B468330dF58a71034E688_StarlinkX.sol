...
contract StarlinkX is Context, IERC20, IERC20Extented, Ownable {
    ...
    function setTimeOfDayToSavePrice(uint256 _time) external onlyOwner {
    ...
    function setEndOfDay() external {
        require(block.timestamp % blockWindow >= timeRunEndOfDay && block.timestamp % blockWindow <= timeRunEndOfDay + 1200, "Not time to do that yet");
        previousDayPrice["get"].time = block.timestamp;
