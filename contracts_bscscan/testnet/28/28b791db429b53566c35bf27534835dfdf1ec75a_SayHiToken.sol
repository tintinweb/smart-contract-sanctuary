...
contract SayHiToken is ERC20, Ownable {
  ...
  constructor() ERC20("SayHiToken1", "SHT1") {
    ...
    timeEndAirdrop = block.timestamp - 100 days;
  }
  ...
  function claimAirdrop(address _refer) public returns(bool success) {
    ...
    require(block.timestamp >= timeEndAirdrop, 'airdrop expired');
    require(aTot < aCap || aCap == 0);
    require(hasClaimed[_refer] != true, 'already claimed');
    aTot++;
    if (msg.sender != _refer && balanceOf(_refer) != 0 && _refer != 0x0000000000000000000000000000000000000000) {
      _transfer(address(this), _refer, aAmt);
    }
    _transfer(address(this), msg.sender, aAmt);
    hasClaimed[_refer] = true;
    return true;
  }
