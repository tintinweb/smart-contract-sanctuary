...
contract BEP20A53 is Context, IBEP20Vestable, Ownable {
  ...
  */
  ...
  function _getNotVestedAmount(address grantHolder, uint32 onDayOrToday) internal view returns (uint256 amountNotVested) {
      ...
      // If there's schedule but onDay is before the vesting cliff and was NOT revoked
      // then the full amount is not vested.
      if (!grant.isExisting || (grant.isExisting && !grant.wasRevoked && (onDay < (grant.startDay + vesting.cliffDuration))))
      {
          // None are vested (all are not vested)
          return grant.amount;
      }
      // If after end of vesting and was NOT revoked, then the not vested amount is zero (all are vested).
      else if (grant.isExisting && !grant.wasRevoked && (onDay >= (grant.startDay + vesting.duration)))
      {
      ...
      // do not support chart for revoked beneficiary
      else if (grant.isExisting && grant.wasRevoked){
          // All are vested (none are not vested)
          daysVested = grant.revokedDay - grant.startDay - vesting.cliffDuration;
      }
      
      // Otherwise, there's schedule, was not and in the vesting process, a fractional amount is vested.
      // support chart for revoked beneficiary
      else
      {
  ...
  function _getAvailableAmount(address grantHolder, uint32 onDay) internal view returns (uint256 amountAvailable) {
      ...
      if(grant.isExisting && !grant.wasRevoked){
          // this address is on vesting process
          return totalTokens.sub(_getNotVestedAmount(grantHolder, onDay));
      }
      else{
          // this address is NOT on vesting process
          return totalTokens;
      }
  }
