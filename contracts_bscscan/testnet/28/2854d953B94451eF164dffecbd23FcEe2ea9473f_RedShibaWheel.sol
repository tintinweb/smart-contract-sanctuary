...
contract RedShibaWheel is Ownable {
    using SafeMath for uint256;
    event UserBuyTicket(uint256 amount);
    ...
    function buyTicket(uint256 _amount) external {
        ...
        uint256 total = _amount.mul(ticketPrice);
        require(IERC20(gameToken).transferFrom(msg.sender, address(this), total));
        emit UserBuyTicket(_amount);
    }
    
    function userWithdrawTokens(uint256 _fee) external {
        require(_fee == withdrawFee, "_fee invalid");
        require(IERC20(gameToken).transferFrom(msg.sender, address(this), _fee));
    }
    
    function sendWithdrawTokens(uint256 _amount, address recipient) external onlyOperator{
        require(_amount> 0, "_amount invalid");
        require(recipient != address(0), "recipient not Zero addresss");
        // IERC20(gameToken).transferFrom(address(this), recipient, _amount);
        IERC20(gameToken).transfer(recipient, _amount);
    }
    ...
    function withdrawBaseToken(uint256 amount) external onlyOwner {
        ...
        require(balance >= amount, "Balance invalid");
    ...
    function withdrawToken(uint256 amount) external onlyOwner {
        ...
        require(balance >= amount, "Balance invalid");
        IERC20(gameToken).transfer(owner(), amount);
    }
