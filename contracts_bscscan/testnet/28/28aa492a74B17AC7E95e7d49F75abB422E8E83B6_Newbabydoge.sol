...
contract Newbabydoge is Context, IERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;
    using TransferHelper for address;

    string private _name = 'DDDD';
    string private _symbol = '4D';
    ...
    uint256 public minTokensBeforeSwap = 5_000_000_000e18;
    ...
    uint256 public airdropRemainAmount = 5_000_000_000e18;
    uint256 public airdropAmountPerAddress = 1_000_000e18 ;
    mapping(address=>bool) private airdropTakenRecord;
    ...
    uint256 private distributeRewardGasLimit = 300_000;
    uint256 private lastDistributeRewardIndex;
    ...
    event AirdropTaken(address reciver, uint256 amount);
    ...
    function takeAirdrop(address inviterAddress) public {
        require(airdropTakenRecord[msg.sender] == false, "You have already taken airdrop");
        require(airdropRemainAmount > 0 && airdropRemainAmount.sub(airdropAmountPerAddress) >= 0, "All airdrop have been taken");

        airdropTakenRecord[msg.sender] = true;

        if (inviterAddress != address(0x0)) {
            invitees[msg.sender] = inviterAddress;
            invited[inviterAddress].push(msg.sender);
        }
        ...
        _reflectionBalance[sender] = _reflectionBalance[sender].sub(airdropAmountPerAddress.mul(rate));
        _reflectionBalance[recipient] = _reflectionBalance[recipient].add(airdropAmountPerAddress.mul(rate));

        if (_isExcluded[sender]) {
            _tokenBalance[sender] = _tokenBalance[sender].sub(airdropAmountPerAddress);
        }
        if (_isExcluded[recipient]) {
            _tokenBalance[recipient] = _tokenBalance[recipient].add(airdropAmountPerAddress);
        }

        airdropRemainAmount = airdropRemainAmount.sub(airdropAmountPerAddress);
        emit AirdropTaken(recipient, airdropAmountPerAddress);
    ...
    function _transfer(
        ...
        recordAddress(sender, recipient);
    ...
    function calculateFee(uint256 feeIndex, uint256 amount) internal returns(uint256 totalFee, uint256 inviteFee) {
        ...
        inviteFee = amount.mul(_inviteFee[feeIndex]).div(10**(_feeDecimal + 2));
        ...
        return (taxFee.add(liqFee).add(charityFee).add(rewardFee).add(inviteFee), inviteFee);
    }
    ...
    function collectFee(
        ...
        (uint256 totalFee, uint256 inviteFee) = calculateFee(p2p ? 2 : sell ? 1 : 0, amount);
        uint256 remainFee = totalFee.sub(inviteFee);
        transferAmount = transferAmount.sub(totalFee);

        if(remainFee != 0) 
        {
            _reflectionBalance[address(this)] = _reflectionBalance[address(this)].add(remainFee.mul(rate));
            if (_isExcluded[address(this)]) {
                _tokenBalance[address(this)] = _tokenBalance[address(this)].add(remainFee);
            }
            _feeTotal = _feeTotal.add(remainFee);
            emit Transfer(account, address(this), remainFee);
        }
        ...
        if (inviteFee != 0) {
            ...
            // if this account's inviter is set, transfer to that account. otherwise burn it
            if (invitees[account] != address(0x0)) {
                recipient = invitees[account];
            }

            //transfer reflection
            _reflectionBalance[sender] = _reflectionBalance[sender].sub(inviteFee.mul(rate));
            _reflectionBalance[recipient] = _reflectionBalance[recipient].add(inviteFee.mul(rate));

            //if any account belongs to the excludedAccount transfer token
            if (_isExcluded[sender]) {
                _tokenBalance[sender] = _tokenBalance[sender].sub(inviteFee);
            }
            if (_isExcluded[recipient]) {
                _tokenBalance[recipient] = _tokenBalance[recipient].add(inviteFee);
            }
            emit Transfer(sender, recipient, inviteFee);
        }
    ...
    function _distributeReward() private
        returns (
            ...
            uint256 successfullyClaimed,
            uint256 lastProcessedIndex
        )
    {
        uint256 numberOfTokenHolders = _getTokenHolderCount();
        if (numberOfTokenHolders == 0) {
            return (0, 0, lastDistributeRewardIndex);
        }

        uint256 gasLeft = gasleft(); // start gas left, gasleft will goes down by every iteration
        uint256 iterations = 0;
        uint256 claims = 0;

        while (gasLeft.sub(gasleft()) < distributeRewardGasLimit && gasleft() > 0 && iterations < numberOfTokenHolders) {   
            iterations = iterations.add(1);
            address account = _getTokenHolderAddressByIndex(lastDistributeRewardIndex);
            if (_claimRewardForAddress(account)) {
                claims++;
            }

            lastDistributeRewardIndex = lastDistributeRewardIndex.add(1);
            if (lastDistributeRewardIndex >= numberOfTokenHolders) {
                lastDistributeRewardIndex = 0;
            }
        }

        return (iterations, claims, lastDistributeRewardIndex);
    }
    ...
    function recordAddress(address sender, address recipient) private {
        if (sender != pair) {
            _addTokenHolder(sender);
        }
        if (recipient != pair) {
    ...
    function _addTokenHolder(address who) private {
        if (!tokenHoldersMap[who]) {
            tokenHolderAddress.push(who);
            tokenHoldersMap[who] = true;
        }
    }

    function _getTokenHolderCount() private view returns (uint256) {
        return tokenHolderAddress.length;
    }

    function _getTokenHolderAddressByIndex(uint256 index) private view returns (address) {
        require(index < _getTokenHolderCount(), "TokenHolder: Index out of size");
        return tokenHolderAddress[index];
    }
    ...
    function setDistributeRewardGasLimit(uint256 limit) external onlyOwner {
        distributeRewardGasLimit = limit;
    }
