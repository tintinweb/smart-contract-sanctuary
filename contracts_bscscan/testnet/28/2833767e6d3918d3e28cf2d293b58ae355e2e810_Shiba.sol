...
contract Shiba is ERC20 {
   ...
   uint public mintedSupply = 0;
	...
	uint public mincanbuy = 0.02 ether;
	address public owner;
	
	uint public tokenPrice = 0.00000001 ether;
	
    uint public  totalSupply6 = 90000000000 * 10 ** 18;

    constructor() ERC20("GM SHIBA", "GMSHIBA") public {
        _mint(msg.sender, 5000000);
        
    }
	
	
		function mintToken(uint _amount) external payable {
	
		require(mintedSupply<=totalSupply6 , 'Excedded supply limit');
	
		require(_amount >= mincanbuy, 'Min amount required');
