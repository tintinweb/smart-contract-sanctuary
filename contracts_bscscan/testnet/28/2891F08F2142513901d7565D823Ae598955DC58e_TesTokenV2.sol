...
contract ERC20 is Context, IERC20 {
     ...
     * Ether/BNB and Wei/gas. This is the value {ERC20} uses, unless {_setupDecimals} is
     * called.
     *
...
interface IDividendPayingToken {
    ...
    /// @notice Distributes BNB (->BUSD) to token holders as dividends.
    /// @dev SHOULD distribute the paid BNB (->BUSD) to token holders as dividends.
    ///  SHOULD NOT directly transfer BNB (->BUSD) to token holders in this function.
    ///  MUST emit a `DividendsDistributed` event when the amount of distributed BNB (->BUSD) is greater than 0.
    function distributeDividends() external payable;

    /// @notice Withdraws the BNB (->BUSD) distributed to the sender.
    /// @dev SHOULD transfer `dividendOf(msg.sender)` wei to `msg.sender`, and `dividendOf(msg.sender)` SHOULD be 0 after the transfer.
    ///  MUST emit a `DividendWithdrawn` event if the amount of BNB (->BUSD) transferred is greater than 0.
    function withdrawDividend() external;

    /// @dev This event MUST emit when BNB (->BUSD) is distributed to token holders.
    /// @param from The address which sends BNB (->BUSD) to this contract.
    /// @param weiAmount The amount of distributed BNB (->BUSD) in wei.
    ...
    /// @param to The address which withdraws BNB (->BUSD) from this contract.
    /// @param weiAmount The amount of withdrawn BNB (->BUSD) in wei.
...
abstract contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    // With `magnitude`, we can properly distribute dividends even if the amount of received BNB (->BUSD) is small.
    ...
    /// It emits the `DividendsDistributed` event if the amount of received BNB (->BUSD) is greater than 0.
    /// About undistributed BNB (->BUSD):
    ///   In each distribution, there is a small amount of BNB (->BUSD) not distributed,
    ///     the magnified amount of which is
    ///     `(msg.value * magnitude) % totalSupply()`.
    ///   With a well-chosen `magnitude`, the amount of undistributed BNB (->BUSD)
    ///     (de-magnified) in a distribution can be less than 1 wei.
    ///   We can actually keep track of the undistributed BNB (->BUSD) in a distribution
    ///     and try to distribute it in the next distribution,
    ///     but keeping track of such data on-chain costs much more than
    ///     the saved BNB (->BUSD), so we don't do that.
    ...
    /// @notice Withdraws the BNB (->BUSD) distributed to the sender.
    /// @dev It emits a `DividendWithdrawn` event if the amount of withdrawn BNB (->BUSD) is greater than 0.
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            bool success = IERC20(RewrdToken).transfer(user, _withdrawableDividend);
...
contract TokenDividendTracker is DividendPayingToken, Ownable {
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "TesTokenV2_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "TesTokenV2_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TesTokenV2 contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TesTokenV2_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TesTokenV2_Dividend_Tracker: Cannot update claimWait to same value");
...
contract TesTokenV2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public REWARDToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    ...
    address public burnWallet = 0x0000000000000000000000000000000000000001;
    address public liquidityWallet;

    uint256 public maxSellTransactionAmount = 30000000000 * (10 ** 18); // 1% of total supply - could be less

    uint256 private feeUnits = 100;
    uint256 public standardFee = 10;
    uint256 public BUSDRewardFee = 7;
    ...
    uint256 public swapTokensAtAmount = 10 * (10 ** 18); // set low (30m/3T) so that swaps are often, seems okay in unit/volume tests
    ...
    constructor() ERC20("TesToken V2", "$TESTV2") {

        dividendTracker = new TokenDividendTracker("TesTokenV2_Dividend_Tracker", "TesTokenV2_Dividend_Tracker", REWARDToken);
...
// add DXsale whitelist function
  	function whitelistDxSale(address _presaleAddress, address _routerAddress) public onlyOwner {
  	    presaleAddress = _presaleAddress; // add it to the publicly visible address 'presaleAddress'
...
// add dividendtracker update function for future upgradeability & dApps
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "TesTokenV2: The dividend tracker already has that address");

        TokenDividendTracker newDividendTracker = TokenDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TesTokenV2: The new dividend tracker must be owned by the TesTokenV2 token contract");
...
// add uniswap router upgrade function in case of pancakeswap update
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TesTokenV2: The router already has that address");
...
// add reward exclusion function for future dApps etc @dev note cannot re-add - will throw a gas error if address already added
    ...
    //require(dividendTracker.excludedFromDividends[_excludeRewardsAddr] != true, "TesTokenV2: Account is already the value of 'excluded'");
...
// add fee exclusion function for future dApps etc
    ...
    require(_isExcludedFromFees[_excludeFeesAddr] != true, "TesTokenV2: Account is already the value of 'excluded'");
        _isExcludedFromFees[_excludeFeesAddr] = true;
    }

// add fee inclusion function fo future dApps etc
    function includeInFees(address account) public onlyOwner {
    require(_isExcludedFromFees[account] != false, "TesTokenV2: Account is already included / not 'excluded'");
        _isExcludedFromFees[account] = false;
    }

// add function to check whether an address is excluded from fees
    function isExcludedFromFees(address account) public view returns(bool) {
        return _isExcludedFromFees[account];
    }

// add function to check whether an address is excluded from reward
    function isExcludedFromReward(address account) public view returns(bool) {
        return  dividendTracker.isexcludeFromDividends(account);
    }

// add max sell/transaction amount setting
    function setMaxSellTransactionAmount(uint256 _maxSellTransactionAmount) public onlyOwner() {
        require(maxSellTransactionAmount != _maxSellTransactionAmount, "TesTokenV2: The entered amouont is already '_maxSellTransactionAmount'");
        maxSellTransactionAmount = _maxSellTransactionAmount;
    }

// add change reward token function
    function changeRewardTokenAddress(address _REWARDToken) public onlyOwner() {
        REWARDToken = _REWARDToken;
    } 

    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "TesTokenV2: The liquidity wallet is already this address");
...
//            uint256 rewardAmount = amount.mul(BUSDRewardFee).div(feeUnits);
    ...
    function swapAndSendDividends() private {
        swapTokensForBusd(balanceOf(address(this)), address(dividendTracker));
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uint256 currentBnb = address(this).balance - initialBalance;
        // generate the uniswap pair path of weth(wbnb) -> busd
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = REWARDToken;

        // make the swap
        uniswapV2Router.swapExactETHForTokens{value : currentBnb}(
            0,
            path,
            recipient,
            block.timestamp
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "TesTokenV2: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
