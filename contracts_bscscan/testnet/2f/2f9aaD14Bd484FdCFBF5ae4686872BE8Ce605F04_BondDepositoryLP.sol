...
contract BondDepositoryLP is Ownable {
    ...
    address public immutable LOVE; // token given as payment for bond
    ...
    function deposit(
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) LOVE
         */
        ...
        if (fee != 0) {
            ...
            IERC20(LOVE).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(LOVE).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(LOVE).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(LOVE).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(LOVE).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(LOVE).totalSupply();
