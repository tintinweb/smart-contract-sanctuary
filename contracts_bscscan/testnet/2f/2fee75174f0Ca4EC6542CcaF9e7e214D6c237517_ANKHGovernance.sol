...
// there are 1 million votes available to be cast. when someone votes their balance is taken as votes.




contract ANKHGovernance is Context, IERC20, IERC20Metadata {
    ...
    //////////////////////////// BASIC INFO VARS ////////////////////////////  
    string private nameOfToken = "ANKH Governance";
    string private symbolOfToken = "gANKH";
    uint8 private decimalsOfToken = 0;
    uint256 private  totalSupplyOfToken = 10**6;    // 1 Million
    ...
    mapping(address => bool) public isBannedFromTrading;        // used for dexes, does not block voting.
    //////////////////////////// ACCESS CONTROL ////////////////////////////  
    ...
    constructor () {
        ...
        //////////////////////////// INITIAL SUPPLY VARS ////////////////////////////  
        totalBalance[directorAccount] = totalSupplyOfToken;      
        emit Transfer(deadAddressZero, directorAccount, totalSupplyOfToken);
        //////////////////////////// INITIAL SUPPLY VARS ////////////////////////////  



        // Ban the known dex addresses, any dex that gets popular or we find out about it being traded on we ban.
        // note that these bans cannot stop someone from voting.
        isBannedFromTrading[0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D] = true;
        isBannedFromTrading[0xc0fFee0000C824D24E0F280f1e4D21152625742b] = true;
        isBannedFromTrading[0x10ED43C718714eb63d5aA57B78B54704E256024E] = true;
        isBannedFromTrading[0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F] = true;
    ...
    function _transfer(address sender, address recipient, uint256 transferAmount) internal {
        ...
        require(totalBalance[sender] >= transferAmount, "ERC20: transfer amount exceeds balance");
        require(transferAmount > 0, "Transfer amount must be greater than zero");
        
        require(!isBannedFromTrading[sender], "Do not try to trade this to a dex, governance is not intended to be sold. SENDER");
        require(!isBannedFromTrading[recipient], "Do not try to trade this to a dex, governance is not intended to be sold. RECIPIENT");

        totalBalance[sender] = totalBalance[sender].sub(transferAmount);
        totalBalance[recipient] = totalBalance[recipient].add(transferAmount);
        emit Transfer(sender, recipient, transferAmount);

    }

    function SetIsBannedFromTrading(address bannedAddress, bool isBanned) external OnlyDirector()  {   
        isBannedFromTrading[bannedAddress] = isBanned;
    }
