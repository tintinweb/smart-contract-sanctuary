...
contract YasuINU is Context, IERC20, Ownable {
    ...
    string private _name = "Yasu INU";
    string private _symbol = "YASU";
    ...
    address payable public marketingWallet = payable(0x5DA084A7b052c8F58B4e04A8aF67b11619F03244);
    
    uint256 public _devFee = 2;
    uint256 private _previousDevFee = _devFee;
    address payable public DevWallet1 = payable(0x35c00437b0b3715065032B3680C9C932DD022544);
    address payable public DevWallet2 = payable(0x09F1A7ac1A7B303c1FcAFD444035b6BB6339d0C3);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee==0 && _devFee == 0 && _burnFee==0) return;
    ...
    function _transfer(
        ...
        if (
            ...
            //add liquidity and send bnb to wallet addresses
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        // combine the Liquidity, marketing and dev fees
        uint256 combineFees = _liquidityFee.add(_marketingFee).add(_devFee); // marketing + liquidity fees + devFee
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee).div(combineFees); 
        
        uint256 tokensForWallets = contractTokenBalance.sub(tokensForLiquidity);
        swapTokensForEth(tokensForWallets);
    ...
    function swapAndSendToWallets(uint256 tokensForWallets) private  {
       
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(tokensForWallets);
        uint256 transferredBalance = address(this).balance.sub(initialBalance);
        uint256 marketingShare = transferredBalance.mul(_marketingFee).div(_marketingFee.add(_devFee));
        uint256 dev1Share = transferredBalance.sub(marketingShare).div(2);
        uint256 dev2Share = transferredBalance.sub(marketingShare.add(dev1Share));

        //Send to Marketing address
        transferToAddressETH(marketingWallet, transferredBalance);
        
        //Send to Dev Wallet1
        transferToAddressETH(DevWallet1, dev1Share);
        
        //Send to Dev Wallet2
        transferToAddressETH(DevWallet2, dev2Share);
    ...
    function setDevWallets(address payable devWallet1, address payable devWallet2) external onlyOwner() {
        DevWallet1 = devWallet1;
        DevWallet2 = devWallet2;
    }
    
    function setFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 burnFee, uint256 devFee) external onlyOwner() {
        _taxFee = taxFee;
    ...
    function SetSwapTokensAtAmount(uint256 newAmt) external onlyOwner() {
        swapTokensAtAmount = newAmt * (10**18);
    }
