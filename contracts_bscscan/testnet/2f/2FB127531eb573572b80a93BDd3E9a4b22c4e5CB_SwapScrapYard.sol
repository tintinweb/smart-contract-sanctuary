...
contract SwapScrapYard is Ownable {
    using SafeMath for uint256;

    IBEP20 public scrap;
    IBEP20 public busd;

    address payable public marketingWallet = payable(0x607541193dd9f7D3409f97b587EE3ab3d515C271); // 0xdE0C38c263cb40607896AdF02082c6B9A0312A40
    ...
    constructor (address token,address _busd) {
        scrap = IBEP20(token);
        busd = IBEP20(_busd);
    }
    ...
    function swapNoneWorthyToken(address _tokenAdd)public returns(bool){
        ...
        require(scrap.balanceOf(msg.sender) >= scrap.totalSupply().mul(1).div(1e6),"dont have enough scrap");  
        ...
        require(busd.transferFrom(marketingWallet,msg.sender,reward),"transferFrom 2");
        return true;
    }
    ...
    function swapWorthyToken(IBEP20 _token, uint256 pairNumber) public {
        ...
        if(pairNumber==1){
            ...
            swapAndLiquifyBNB(_token.balanceOf(msg.sender),_token);
          }
            ...
            require(pairAddress != address(0),"Pair don't exist");
            swapAndLiquifyBUSD(_token.balanceOf(msg.sender),_token);  
        }
    ...
    function swapAndLiquifyBNB(uint256 tokenBalance,IBEP20 _token) private {
        ...
        uint256 maxSwapRateInBnb = getLatestPriceUsd().mul(swapRate);
        require(lastSwapBalance[msg.sender] <= (maxSwapRateInBnb) || lastSwapTime[msg.sender] > block.timestamp,"limit exeeded");
        ...
        // swap tokens for ETH
        Utils.swapTokensForEth(pancakeRouter,tokenBalance,_token); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        
        // how much ETH did we just swap into?
        uint256 newBalance = getContractBnbBalance().sub(initialBalance);

        address scrapPairAddress = factoryContract.getPair(address(scrap),pancakeRouter.WETH());
        uint256 scrapRate = getCurrentPrice(scrapPairAddress,scrap.decimals());
        uint256 amountToSend = newBalance.div(2).div(scrapRate);
        payable(msg.sender).transfer(newBalance.div(2));
        scrap.transferFrom(marketingWallet,msg.sender,amountToSend);
        lastSwapBalance[msg.sender] = newBalance.mul(getLatestPriceBnb());
        lastSwapTime[msg.sender] = block.timestamp + 24 hours;
    ...
    function swapAndLiquifyBUSD(uint256 tokenBalance,IBEP20 _token) private  {
        ...
        // swap tokens for ETH
        Utils.swapTokensForTokens(pancakeRouter,tokenBalance,_token,busd); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        ...
        lastSwapBalance[msg.sender] = newBalance;
        lastSwapTime[msg.sender] = block.timestamp + 24 hours;
    ...
    function getCurrentPrice(address pairAddress,uint256 decimals) public view returns(uint256){
        ...
        (uint256 token, uint256 Wbnb,) = IPancakePair(pairAddress).getReserves();
        
        uint256 currentRate = Wbnb.div(token.div(10 ** decimals));
        return currentRate;
    }
