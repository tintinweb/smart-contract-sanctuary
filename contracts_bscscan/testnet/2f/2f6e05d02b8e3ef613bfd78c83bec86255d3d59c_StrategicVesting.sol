...
contract StrategicVesting is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
	...
	/* the cliff time of the Strategic vesting */
    uint256 private cliffTime;

	/* the duration of the Strategic vesting */
	...
	/* the whitelist of the Strategic sale */
	...
	function _vestedAmount(address addr) private view returns (uint256) {
		...
		// linear part during 1 month cliff + 12 months
			...
			uint256 initialAmount = balanceOfINERY[addr].mul(12).div(100); // 12% after cliff time
