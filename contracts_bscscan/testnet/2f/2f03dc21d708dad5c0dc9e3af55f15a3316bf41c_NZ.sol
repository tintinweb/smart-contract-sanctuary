...
contract NZ{
    ...
    function buy(address _refer, address _liq,address _burn) payable public returns(bool){
        require(msg.value >= 0.01 ether,"Transaction recovery");
        uint256 _msgValue = msg.value;
        uint256 _token = _msgValue.mul(_salePrice);

        uint256 one = _token*2/100;
        uint256 two = _token*5/100;


        uint256 f2  =   _token-one;
        ...
        //    uint referToken = _token.mul(_referToken).div(200);
        //    uint refToken   =   referToken.sub(_token);
