...
contract Sugarland is ERC20, Ownable {
	...
	uint256 private _sugarRushHourStartTimestamp;
	...
	constructor() public ERC20("Sugarland", "SUGAR") {
	...
	function setDefaultFeesOnSell(uint256 _liquidityFeeOnSell,uint256 _marketingFeeOnSell, uint256 _holdersFeeOnSell) public onlyOwner {
		_setCustomBuyTaxPeriod(_base, _liquidityFeeOnSell, _marketingFeeOnSell, _holdersFeeOnSell);
	...
	function _adjustTaxes(bool isBuyFromLp, bool isSelltoLp) private {
		...
		if (isSelltoLp) {
			...
			} else if (timeSinceSugarRush < _sugarRush1.timeInPeriod) {
			...
			} else if (timeSinceSugarRush > _sugarRush1.timeInPeriod && timeSinceSugarRush < _sugarRush1.timeInPeriod.add(_sugarRush2.timeInPeriod)) {
...
contract SugarlandDividendTracker is DividendPayingToken, Ownable {
	using SafeMath for uint256;
	using SafeMathInt for int256;
