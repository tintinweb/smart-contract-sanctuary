...
contract LPTokenBoardroom is Boardroom {
	...
	address[] public wantLP0ToStable;
	address[] public wantLP1ToStable;
	...
	constructor(
		...
		address[] memory _wantLP0ToStable,
		address[] memory _wantLP1ToStable
	)
		Boardroom(
			_cash,
			_share,
		...
		)
	{
		...
		wantLP0ToStable = _wantLP0ToStable;
		wantLP1ToStable = _wantLP1ToStable;

		share.safeApprove(address(router), type(uint128).max);
		cash.safeApprove(address(zapper), type(uint128).max);
	}
	...
	function compound() external onlyOneBlock updateReward(msg.sender) {
		...
		if (cashReward > 0 || shareReward > 0) {
			require(
				(directors[msg.sender].epochTimerStart + rewardLockupEpochs) <=
					treasury.epoch(),
			...
			if (shareReward > 0) {
				cashReward = cashReward + _swapShareToCash(shareReward);
			}

			uint256 wantToDeposit = _zapCashToWant(cashReward);
