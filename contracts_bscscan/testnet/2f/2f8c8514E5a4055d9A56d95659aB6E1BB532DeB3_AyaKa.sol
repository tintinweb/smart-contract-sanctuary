...
contract AyaKa is IBEP20, Auth {
    ...
    // Name and symbol
    string constant _name = "Aya";
    string constant _symbol = "NAK";
    uint8 constant _decimals = 18;
    
    // Total supply
    uint256 _totalSupply = 48_000_000_000 * (10 ** _decimals); // 48Md
    
    // Max wallet and TX
    uint256 public _maxBuyTxAmount = 48000000; // 48m tokens
    uint256 public _maxSellTxAmount = 48000000; // 48m tokens
    uint256 public _maxWalletToken = 480000000; // 480m tokens
    ...
    // Buy Fees
    uint256 _liquidityFeeBuy = 400;
    uint256 _buybackFeeBuy = 400;
    uint256 _marketingFeeBuy = 400;
    uint256 _totalFeeBuy = 1000;
    
    uint256 _deathNoteLiquidityFeeBuy = 100;
    uint256 _deathNoteBuybackFeeBuy = 0;
    uint256 _deathNoteMarketingFeeBuy = 400;
    uint256 _deathNoteTotalFeeBuy = 500;
    ...
    uint256 _deathNoteLiquidityFeeSell = 300;
    uint256 _deathNoteBuybackFeeSell = 1500;
    uint256 _deathNoteMarketingFeeSell = 1200;
    uint256 _deathNoteTotalFeeSell = 3000;
    ...
    // Death Note mode
    uint256 _deathNoteModeTriggeredAt;
    uint256 _deathNoteDuration = 900;
    
    // Sell amount of tokens when a sell takes place
    uint256 public _swapThreshold = 48000000; // 0.1% of supply
    ...
    constructor () Auth(msg.sender) {
        ...
        _isFeeExempt[_buyBackFeeReceiver] = true;
        _isTxLimitExempt[_buyBackFeeReceiver] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Check if we are in Death Note mode
        bool deathNoteMode = inDeathNoteMode();
        
        // Set buy or sell fees
        setCorrectFees(isSell, deathNoteMode);
    ...
    function setCorrectFees(bool isSell, bool deathNoteMode) internal {
        if (isSell) {
            if (deathNoteMode) {
                _liquidityFee = _deathNoteLiquidityFeeSell;
                _buybackFee = _deathNoteBuybackFeeSell;
                _marketingFee = _deathNoteMarketingFeeSell;
                _totalFee = _deathNoteTotalFeeSell;
            }
            else {
            ...
            if (deathNoteMode) {
                _liquidityFee = _deathNoteLiquidityFeeBuy;
                _buybackFee = _deathNoteBuybackFeeBuy;
                _marketingFee = _deathNoteMarketingFeeBuy;
                _totalFee = _deathNoteTotalFeeBuy;
            }
            else {
    ...
    // Check if we are in Death Note mode
    function inDeathNoteMode() public view returns (bool){
        if (_deathNoteModeTriggeredAt.add(_deathNoteDuration) > block.timestamp) {
            return true;
        }
        else {
            return false;
    ...
    // Enable Death Note mode
    function enableDeathNoteMode(uint256 durationInSeconds) public authorized {
        _deathNoteModeTriggeredAt = block.timestamp;
        _deathNoteDuration = durationInSeconds;
    }
    
    // Disable the Death Note mode
    function disableDeathNoteMode() external authorized {
        _deathNoteModeTriggeredAt = 0;
    }
    ...
    // Set our Death Note buy fees
    function setDeathNoteBuyFees(uint256 deathNoteLiquidityFeeBuy, uint256 deathNoteBuybackFeeBuy, uint256 deathNoteMarketingFeeBuy) external authorized {
        _deathNoteLiquidityFeeBuy = deathNoteLiquidityFeeBuy;
        _deathNoteBuybackFeeBuy = deathNoteBuybackFeeBuy;
        _deathNoteMarketingFeeBuy = deathNoteMarketingFeeBuy;
        _deathNoteTotalFeeBuy = deathNoteLiquidityFeeBuy.add(deathNoteBuybackFeeBuy).add(deathNoteMarketingFeeBuy);
    }
    
    // Set our Death Note sell fees
    function setDeathNoteSellFees(uint256 deathNoteLiquidityFeeSell, uint256 deathNoteBuybackFeeSell, uint256 deathNoteMarketingFeeSell) external authorized {
        _deathNoteLiquidityFeeSell = deathNoteLiquidityFeeSell;
        _deathNoteBuybackFeeSell = deathNoteBuybackFeeSell;
        _deathNoteMarketingFeeSell = deathNoteMarketingFeeSell;
        _deathNoteTotalFeeBuy = deathNoteLiquidityFeeSell.add(deathNoteBuybackFeeSell).add(deathNoteMarketingFeeSell);
    }
    ...
    function setFeeReceivers(address autoLiquidityReceiver, address marketingFeeReceiver, address buyBackFeeReceiver) external authorized {
        ...
        _isTxLimitExempt[buyBackFeeReceiver] = true;
    }
