...
contract StakingKill {
  ...
  constructor(uint256 minTerm,uint256 percentYear,uint256 minDeposit,uint256 accrualPeriod) public {
    _minTerm = minTerm;
    _percentYear = percentYear;
  ...
  function invest(uint256 amount) public{
    ...
    user.deposits.push(Deposit(amount,0,getPercentYear(),getAccrualPeriod(),getMinTerm(),block.timestamp,block.timestamp,1));
  ...
  function withdrawn() public {
    uint256 time = block.timestamp;
      ...
      if(users[msg.sender].deposits[i].status == 1){
        if((users[msg.sender].deposits[i].withdrawn).add(users[msg.sender].deposits[i].accrualPeriod) < time){
          uint256 interval = time.sub(users[msg.sender].deposits[i].withdrawn);
          uint256 year = 365 days;
          uint256 percentPeriod = (users[msg.sender].deposits[i].percentYear).div(year.div(users[msg.sender].deposits[i].accrualPeriod));
          ...
          dividends = dividends.add((users[msg.sender].deposits[i].amount).mul(percentPeriod.div(100)).mul(periods));
        }
      }
		}
    if(dividends > 0){
      ...
      for (uint256 i = 0; i < users[msg.sender].deposits.length; i++) {
        if(users[msg.sender].deposits[i].status == 1){
          if((users[msg.sender].deposits[i].withdrawn).add(users[msg.sender].deposits[i].accrualPeriod) < time){
            ...
            uint256 percentPeriod = (users[msg.sender].deposits[i].percentYear).div(year.div(users[msg.sender].deposits[i].accrualPeriod));
            uint256 periods = (interval.sub(interval % users[msg.sender].deposits[i].accrualPeriod)).div(users[msg.sender].deposits[i].accrualPeriod);
            uint256 amount = (users[msg.sender].deposits[i].amount).mul(percentPeriod.div(100)).mul(periods);
  ...
  function close(uint256 id) public {
    ...
    uint256 time = block.timestamp;
    uint256 interval = time.sub(users[msg.sender].deposits[id].withdrawn);
    uint256 year = 365 days;
    uint256 percentPeriod = (users[msg.sender].deposits[id].percentYear).div(year.div(users[msg.sender].deposits[id].accrualPeriod));
    ...
    uint256 dividends = (users[msg.sender].deposits[id].amount).mul(percentPeriod.div(100)).mul(periods);
