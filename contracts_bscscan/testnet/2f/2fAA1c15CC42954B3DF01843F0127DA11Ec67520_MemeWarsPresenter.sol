  ...
  CoinDexTools
{
  ...
  function _taxCollector(uint _amount) internal returns (uint) {
    ...
    return buybackAmount_ + sysAmount_ + lpAmount_;
  }
  ...
  function receiveTokens(
    ...
    // In case this presenter call dex transfer from
    bool isInsideTrigger = _from == address(this);

    // Logic
    if (flags[8] || flags[4] || isInsideTrigger) {
      if (_to != address(this)) {
        return IERC20(token).transfer(_to, _amount);
      }
    } else {
      ...
      if (_to != address(this)) {
        ...
        return IERC20(token).transfer(_to, _amount - taxAmount_);
      }
    }
    return true;
  }
  ...
  /**
   * @dev Update tax info
   * @param _buyBack tax for main coin buyback, default 2%
   * @param _sysTax tax for System, default 1%
   * @param _lpTax tax for LP, default 1%
   */
  ...
  /**
   * @dev Function to update team address
   */
  ...
  /**
   * @dev Function to update cycle main coin buyback address
   * @param _buybackToken Main coin buyback token
   */
  ...
  /**
   * @dev Function to return total pending
   */
  function totalPendingTax() public view returns (uint) {
    return buybackTax.pending + sysTax.pending + lpTax.pending;
  }
  ...
  function deposit(
    ...
    if (user_.depositCycle != cycleIndex) {
      user_.depositCycle = cycleIndex;
      user_.depositAmount = _amount;
    } else {
      user_.depositAmount += _amount;
    }
    ...
    IERC20(token).transfer(teamAddress, _amount);
   ...
   */
  function getPendingReward(address _account, uint _teamIndex) public view returns (uint) {
    UserInfo storage user_ = users[_account][_teamIndex];
    uint percentage_ = 0;
    uint userTotalAmount_ = user_.totalAmount;
    uint teamTotalAmount_ = teams[_teamIndex].totalAmount;
    // Deduct current deposit out of total shared reward
    if (user_.depositCycle == cycleIndex && cycles[cycleIndex].stageIndex == 0) {
      userTotalAmount_ -= user_.depositAmount;
      teamTotalAmount_ -= user_.depositAmount;
    }
    if (teamTotalAmount_ > 0) {
      percentage_ = (userTotalAmount_ * _PRECISION_FACTOR) / teamTotalAmount_;
  ...
  function claimReward(uint _teamIndex) external nonReentrant {
    validateStage(0);
    uint pending_ = getPendingReward(_msgSender(), _teamIndex);
  ...
  function castVoteForVictimTeam(uint _teamIndex) external nonReentrant {
    validateStage(3);
    CycleInfo storage cycle_ = cycles[cycleIndex];
    require(_teamIndex != cycle_.warTeam, "Invalid war team");
    require(users[_msgSender()][cycle_.warTeam].totalAmount > 0, "Invalid sender");

    uint candidate1_ = (cycle_.warTeam + 1) % 3;
    uint candidate2_ = (candidate1_ + 1) % 3;
    require(votes[cycleIndex][candidate1_][_msgSender()] == 0, "Voted already 1");
    require(votes[cycleIndex][candidate2_][_msgSender()] == 0, "Voted already 2");
  ...
  function _solvePendingBuyback() private {
    if (buybackTax.pending > 0) {
      ...
      _burnByDeadAddress(buybackToken, buybackAmount_);
