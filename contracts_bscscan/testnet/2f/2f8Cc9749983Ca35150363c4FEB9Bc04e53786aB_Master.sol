...
contract Master is IPlatformMaster {
    ...
    // The address of the fee-splitter (the address that NFT bying fees are sent to)
    ...
    function newCollectionContract(
        ...
        emit NewCollectionContract(msg.sender, modelContractAddress, modelName);
...
contract ModelCollection is ERC721 {
    ...
    function distributeFee(address to, uint256 amount, address purchaseTokenAddress) internal {
        ...
        if (
            ...
            referrerCut = (amount * referrerFee) / 100;
            purchaseToken.transferFrom(msg.sender, referrer, referrerCut);
            platformFee  -= referrerFee;

            master.logReferralPay(msg.sender, to, amount, purchaseTokenAddress);

        }

        uint256 platformCut = (amount * platformFee) / 100;
        ...
        if(purchaseTokenAddress == master.getMainToken()) {
            purchaseToken.transferFrom(msg.sender, master.getFarmAddress(), platformCut);
        } else {
            purchaseToken.transferFrom(msg.sender, feeSplitter, platformCut);
        }

        purchaseToken.transferFrom(msg.sender, to, amount - platformCut - referrerCut);
