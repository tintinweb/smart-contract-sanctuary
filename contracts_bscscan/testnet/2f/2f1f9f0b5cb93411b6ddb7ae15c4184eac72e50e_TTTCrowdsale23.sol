...
contract TTTCrowdsale23{
    ...
    function buyTokenBUSD(address _referrer,uint256 plan,uint256 Amount)  public {
        ...
        //seconds 1, minutes 60, hours 360 days 86400 
        users[msg.sender].claimtime = timelock[plan].mul(1);
        
        users[msg.sender].boughtamount = numberOfTokens;
        //считаем timelockBonus правильно
        users[msg.sender].usertimelockAmount = numberOfTokens.mul(timelockBonus[plan]).div(10000).mul(timelock[plan]);
        users[msg.sender].usertimelockBonus = numberOfTokens.mul(timelockBonus[plan]).div(10000);
