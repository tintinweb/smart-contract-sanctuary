...
// File: contracts/HoldexBunnies.sol

pragma solidity ^0.6.0;

/** @title HoldexBunnies.
 * @notice It is the contracts for HoldexSwap NFTs.
 */
contract HoldexBunnies is ERC721, Ownable {
    ...
    constructor(string memory _baseURI) public ERC721("Holdex Bunnies", "HEB") {
 ...
 * Holdex Collectibles/Bunnies.
 */
contract BunnyMintingStation is AccessControl {
    HoldexBunnies public holdexBunnies;
    ...
    constructor(HoldexBunnies _holdexBunnies) public {
        holdexBunnies = _holdexBunnies;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @notice Mint NFTs from the holdexBunnies contract.
     * Users can specify what bunnyId they want to mint. Users can claim once.
     * There is a limit on how many are distributed. It requires HOLDEX balance to be > 0.
     */
    function mintCollectible(
        ...
        uint256 tokenId = holdexBunnies.mint(_tokenReceiver, _tokenURI, _bunnyId);
        return tokenId;
    }
    ...
    function setBunnyName(uint8 _bunnyId, string calldata _bunnyName) external onlyOwner {
        holdexBunnies.setBunnyName(_bunnyId, _bunnyName);
    }
    ...
    function changeOwnershipNFTContract(address _newOwner) external onlyOwner {
        holdexBunnies.transferOwnership(_newOwner);
    }
}
...
contract TradingCompV1 is Ownable {
    ...
    IHoldexProfile public holdexProfile;
    ...
    struct CompetitionRewards {
        ...
        uint256[5] holdexRewards; // holdex rewards per group
    ...
    function claimReward() external {
        ...
        if (userRewardGroup > 0) {
            holdexToken.safeTransfer(senderAddress, userRewards.holdexRewards[userRewardGroup]);
     ...
     * @dev Only callable if the user has an active HoldexProfile.
     */
    function register() external {
        ...
        (, , userTeamId, , , isUserActive) = holdexProfile.getUserProfile(senderAddress);
    ...
    /**
     * @notice It allows the owner to claim the HOLDEX remainder
     * @dev Only callable by owner.
     * @param _amount: amount of HOLDEX to withdraw (decimals = 18)
     */
     ...
     * @param _holdexRewards: HOLDEX rewards for each user group for teamId
     ...
     * @return userHoldexRewards: the HOLDEX to claim/claimed
    ...
    )
    {
        ...
        if ((currentStatus != CompetitionStatus.Claiming) && (currentStatus != CompetitionStatus.Over)) {
            ...
            uint256 userHoldexRewards = _rewardCompetitions[userTeamId].holdexRewards[userRewardGroup];
            ...
            return (hasUserRegistered, hasUserClaimed, userRewardGroup, userHoldexRewards, userPointRewards, canClaimNFT);
        }
    }
