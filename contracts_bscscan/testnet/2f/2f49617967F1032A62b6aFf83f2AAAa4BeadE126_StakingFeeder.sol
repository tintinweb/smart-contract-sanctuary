...
contract StakingFeeder  {
	...
	iMoonCoinApp public MoonCoinApp;
	iMoonCoinApp public MoonCoinAppApprover;
	...
	uint256 public TIME_STEP = 60 seconds; // TBA 1 days
	uint256 public constant PERCENTS_DIVIDER = 1000;
	uint256 public FEED_PERCENT = 5; // 0%-1% max  // TBA
	uint256 public EXCHANGE_APP_TOKENS ;
	uint256 public lastTriggerByPlatform; // for staking app feeding
	uint256 public lastReleaseByPlatform; // for airdrop
	...
	constructor(iMoonCoinApp _MoonCoin, pancakeInterface _pancakeRouter) public { 
		...
		MoonCoinAppApprover = _MoonCoin;		
		MoonCoinAppApprover.approve(address(_pancakeRouter), 1000000000000000000000000000000000000);		
	}
	...
	function setFeedPercent(uint256 _FEED_PERCENT) public { 
		...
		if(_FEED_PERCENT  <= 10 ) {  // TBA
			FEED_PERCENT=_FEED_PERCENT;
		}
	} 
	
	function setApps(iMoonCoinApp _MoonCoin, iMoonCoinFinanceApp _MoonCoinFinanceApp,  address payable stakingAppAddress, pancakeInterface _pancakeRouter, address[] memory _path,address[] memory _reversePath) public {
		require(msg.sender == platformAddress, "1");
	...
	function getStakingAppBalance() public view returns (uint256) {
		uint256 stakingAppBalance;
		(,,,stakingAppBalance,,)=MoonCoinFinanceApp.getterGlobal1();
		return stakingAppBalance;
	}
	
	function getStakingAppBalanceAvailable() public view returns (uint256) {
		return getStakingAppBalance().mul(FEED_PERCENT).div(PERCENTS_DIVIDER);
	}
	function getStakingAppTokens() public view returns (uint256) {
		return getStakingAppBalanceAvailable().mul(getStakingAppRate()).div(10**18);
	}
	/////////////////////////////////////////////////////
	...
	function getterPoolBalanceAvailable() public view returns ( uint256 ) {
        return ( getterPoolBalance().mul(FEED_PERCENT).div(PERCENTS_DIVIDER));		
    }
	...
	function updateExchangeStats() public {
		EXCHANGE_APP_TOKENS=getExchangeAppRate(getterPoolBalanceAvailable());
	}
	function getExchangeAppTokens() public view returns (uint256) {
		return EXCHANGE_APP_TOKENS;
	}
	function getTotalCoinsForRElease() public view returns (uint256) {
		return getExchangeAppTokens().add(getStakingAppTokens());
	}
	/////////////////////////////////////////////////////
	
	function stakingAppFeedRelease() public {	
		require(msg.sender == platformAddress, "1");
	...
	function airdropFeedRelease() public {	
		require(msg.sender == platformAddress, "1");
		require(block.timestamp.sub(lastReleaseByPlatform) > 1*TIME_STEP, "14");  // TBA max once daily
		lastReleaseByPlatform = block.timestamp;		
		updateExchangeStats();
		uint256 tokensToRelease = getExchangeAppTokens().add(getStakingAppTokens());
		MoonCoinAppApprover.transfer(platformAddress, tokensToRelease);
	}
...
function getRemainingMoons() public { 	// TBA	
		require(msg.sender == platformAddress, "1");
		MoonCoinAppApprover.transfer(msg.sender, MoonCoinAppApprover.balanceOf(address(this)));		
	} 
