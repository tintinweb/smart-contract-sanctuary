...
contract DemoGreen is Context, IERC20, Ownable {
    ...
    event TransferParametersAre(address from, address to, uint256 amount);
    event TokenTransfer(address sender, address recipient, uint256 amount,bool takeFee);
    event transferFromExcluded(address sender, address recipient, uint256 amount);
    event transferToExcluded(address sender, address recipient, uint256 amount);
    event transferBothExcluded(address sender, address recipient, uint256 amount);
    event transferStandard(address sender, address recipient, uint256 amount);
    event reflectFee(uint256 rFee, uint256 tFee);
    event takeSwap(uint256 tSwap);
    ...
    function _transfer(
        ...
        emit TransferParametersAre(from, to, amount);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        emit TokenTransfer(sender, recipient, amount, takeFee);
        if(!takeFee)
            removeAllFee();

        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            emit transferFromExcluded(sender, recipient, amount);
            // _transferFromExcluded(sender, recipient, amount);
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            emit transferToExcluded(sender, recipient, amount);
            // _transferToExcluded(sender, recipient, amount);
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            emit transferBothExcluded(sender, recipient, amount);
            // _transferBothExcluded(sender, recipient, amount);
        } else {
            emit transferStandard(sender, recipient, amount);
