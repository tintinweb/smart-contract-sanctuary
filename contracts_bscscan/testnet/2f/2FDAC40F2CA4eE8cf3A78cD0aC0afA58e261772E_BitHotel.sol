...
contract BitHotel is Ownable,ERC1155,ERC2981PerTokenRoyalties{
    ...
    event TokenMint(uint256 tokenId,uint256 supply,address to);

    constructor(string memory _name, string memory _symbol,string memory tokenURIPrefix) ERC1155(tokenURIPrefix) {
        name = _name;
        symbol = _symbol;
    }

    function mint(uint256 id, uint256 supply, string memory uri,address royaltyRecipient,uint256 royaltyValue)  external onlyOwner{
       _mint(msg.sender, id, supply, "");
       _setTokenURI(id, uri);
       if (royaltyValue > 0) {
        _setTokenRoyalty(id, royaltyRecipient, royaltyValue);
       }
       emit TokenMint(id, supply, msg.sender);
    }
    ...
    function _getTokenURI(uint256 tokenId) external view returns(string memory) {
     ...
     function _clearTokenURI(uint256 tokenId) external onlyOwner {
