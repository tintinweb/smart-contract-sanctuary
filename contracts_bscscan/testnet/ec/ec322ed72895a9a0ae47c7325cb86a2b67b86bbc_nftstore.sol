...
contract nftstore {
    ...
    event AuctionCreated(address indexed from, address indexed currency, uint256 tokenId, uint256 orderId, uint256 price, uint256 duration,address indexed _nft);
    event AuctionSuccessful(address indexed from, uint256 orderId, uint256 duration,uint256 orderprice);
    event AuctionCancelled(uint256 orderId, uint256 duration);
    ...
    function sell(address _nft, uint256 _tokenId, address _currency, uint256 _price) public {
        ...
        emit AuctionCreated(msg.sender, _currency, _tokenId, orderId,_price,now,_nft);
    }
    ...
    function cancel(uint256 _orderId) public {
        ...
        emit AuctionCancelled(_orderId,now);
    }
    ...
    function buy(uint256 _orderId) public payable {
        ...
        IERC721(order.nft).safeTransferFrom(address(this), msg.sender, order.tokenId);
        emit AuctionSuccessful(msg.sender, _orderId,now,orderprice);
    }
