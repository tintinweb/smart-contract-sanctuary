...
contract Ottomanempirev3 is ERC721Enumerable, ReentrancyGuard, Ownable {
   ...
   string[] private LevelList = [
    ...
    function _speed(uint256 tokenId) public pure returns (string memory) {
        return randomSkill(tokenId, "SILAH");
    }
  ...
  function uint2str(uint _i) internal pure returns (string memory _uintAsString) {
    ...
    function randomSkill(uint256 tokenId, string memory keyPrefix) internal pure returns (string memory) {
        uint256 rand = random(string(abi.encodePacked(keyPrefix, toString(tokenId))));
        string memory output = uint2str(rand % 100);
        return output;
    }
