...
contract NFT_MARKETPLACE_OPERATOR is Ownable {
    ...
    */
    ...
    mapping(address => mapping(uint256 => address[])) private $quoteNFTCurrencies;
    ...
    */
        ...
        public
    {
        ...
        for (uint i; i < quotes.length; i++) {
            ...
            $quoteNFTCurrencies[erc721_address][serial_no].push(quote.currency_address);
        }
    }
        ...
        public
    {
        ...
        address[] memory currencies = $quoteNFTCurrencies[erc721_address][serial_no];
        for (uint i; i < currencies.length; i++) {
            address currency_address = currencies[i];
            delete $quoteNFT[erc721_address][serial_no][currency_address];
        }
    }
