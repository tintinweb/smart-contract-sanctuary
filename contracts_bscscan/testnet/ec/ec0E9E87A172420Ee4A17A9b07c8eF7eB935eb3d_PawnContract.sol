...
contract PawnContract is Ownable, Pausable, ReentrancyGuard {
    ...
    */
    ...
    event DebugData (
        address asset, address from, address to, uint256 amount, uint256 balanceAfter
    );
    function safeTransfer(address asset, address from, address to, uint256 amount) internal {
        if (asset == address(0)) {
            ...
            if (to == address(this)) {
                ...
                emit DebugData(asset, from, to, amount, from.balance);
                (bool success, ) = to.call{value:amount}('');
                emit DebugData(asset, from, to, amount, to.balance);
