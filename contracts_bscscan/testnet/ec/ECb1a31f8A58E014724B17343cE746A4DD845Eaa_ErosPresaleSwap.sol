...
contract ErosPresaleSwap {

    BEP20e public token;
    BEP20 public presaleToken;

    event Swap(address indexed _address, uint256 _amount);

    constructor(BEP20e _token, BEP20 _presaleToken) public {
        token = _token;
        presaleToken = _presaleToken;
    }

    function swap(uint256 _amount) external {
        presaleToken.transferFrom(msg.sender, presaleToken.deadAddress(), _amount);
