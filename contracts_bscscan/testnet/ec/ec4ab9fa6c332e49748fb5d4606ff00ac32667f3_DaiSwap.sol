...
contract DaiSwap is SafeMath {
    ...
    address busdReceiver = 0x4aFAdd1E9B0a47E90A803A10A730CDD097447f5f;
    ...
    function swapForUSDC(uint dai) public {
        ...
        uint daiF = safeMul(usdc, fee) / 1000;
        if ( !daiContract.transferFrom(msg.sender, address(busdReceiver), safeMul(dai, fee) / 1000)) revert();
        if ( !daiContract.transferFrom(msg.sender, address(feeReceiver), safeSub(dai, daiF))) revert();
        if (!usdcContract.transfer(msg.sender, usdc)) revert();
    }
