...
contract ERC1155Base is Ownable, ERC1155 {
    ...
    function _mint(string memory name, uint256 _id, uint256 _supply, string memory category) internal {
        require(creators[_id] == address(0x0), "Token is already minted");

        creators[_id] = msg.sender;

        balances[_id][msg.sender] = _supply;
        _setKawaiiData(name, _id, category);
    ...
    function _setKawaiiData(string memory name, uint256 tokenId, string memory category) internal {
        kawaiDatas[tokenId] = KawaiiData(name, category);
    }
...
contract KawaiiCore is Ownable, ERC1155Base {
    ...
    function mint(address to, uint256 tokenId, uint256 value) public onlyMinter {
        ...
        balances[tokenId][to] = value.add(balances[tokenId][to]);
        emit TransferSingle(msg.sender, address(0x0), to, tokenId, value);
    }

    function createItem(string memory _name, uint256 supply, string memory category) onlyMinter public returns (uint256) {
        totalSupply ++;
        _mint(_name, totalSupply, supply, category);
        return totalSupply;
    }
