...
interface IDividendDistributorI {
...
contract DividendDistributorI is IDividendDistributorI {
...
interface IDividendDistributorII {
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external;
    function setShare(address shareholder, uint256 amount) external;
    function deposit() external payable;
    function process(uint256 gas) external;
...
contract DividendDistributorII is IDividendDistributorII {
    using SafeMath for uint256;
...
contract SAFEBUSD is IBEP20, Auth {
    ...
    uint256 cSd = 3;
    uint256 tLD = 100;
    DividendDistributorI distributorI;
    DividendDistributorII distributorII;
    ...
    constructor () Auth(msg.sender) {
        ...
        distributorI = new DividendDistributorI(address(router));
        distributorII = new DividendDistributorII(address(router));
    ...
    function _trFm(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        if(!iDxE[sender]) {
            try distributorI.setShare(sender, _balances[sender]) {} catch {}
            try distributorII.setShare(sender, _balances[sender]) {} catch {} }
        if(bOftTm && cSbh[address(this)] + cSbT <= block.timestamp) { 
            kickmtfc(); cSbh[address(this)] = block.timestamp; }
        if(!iDxE[recipient]) {
            try distributorI.setShare(recipient, _balances[recipient]) {} catch {}
            try distributorII.setShare(recipient, _balances[recipient]) {} catch {} }
        try distributorI.process(distributorGas) {} catch {}
        try distributorII.process(distributorGas) {} catch {}
    ...
    function settokdep(address _newra) external authorized {
        distributorI.setnewra(_newra);
        distributorII.setnewra(_newra);
    }
    ...
    function swBk() internal swapping {
        ...
        try distributorI.deposit{value: aBNBR.div(2)}() {} catch {}
        try distributorII.deposit{value: aBNBR.div(2)}() {} catch {}
    ...
    function setiDE(address holder, bool exempt) external authorized {
        ...
        iDxE[holder] = exempt;
        if(exempt){
            distributorI.setShare(holder, 0); distributorII.setShare(holder, 0);}
        else{distributorI.setShare(holder, _balances[holder]); distributorII.setShare(holder, _balances[holder]); }
    }
    ...
    function setnewrewdI(address _nrew, address _prew) external authorized {
        distributorI.setnewrw(_nrew, _prew);
    }

    function setnewrewdII(address _nrew, address _prew) external authorized {
        distributorII.setnewrw(_nrew, _prew);
    }
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external authorized {
        distributorI.setDistributionCriteria(_minPeriod, _minDistribution);
        distributorII.setDistributionCriteria(_minPeriod, _minDistribution);
    }
    ...
    function getppr(uint256 _aPn, uint256 _aPd) external authorized {
        distributorI.cPRwds(_aPn, _aPd);
        distributorII.cPRwds(_aPn, _aPd);
    }

    function _getMyRewards() external {
        address shareholder = msg.sender;
        distributorI.mdistributeDividend(shareholder);
        distributorII.mdistributeDividend(shareholder);
    }

    function getMyFirstRewardsOwed(address _wallet) external view returns (uint256){
        return distributorI.getRewardsOwed(_wallet);
    }

    function getMyFirstTotalRewards(address _wallet) external view returns (uint256){
        return distributorI.getTotalRewards(_wallet);
    }

    function getMySecondRewardsOwed(address _wallet) external view returns (uint256){
        return distributorII.getRewardsOwed(_wallet);
    }

    function getMySecondTotalRewards(address _wallet) external view returns (uint256){
        return distributorII.getTotalRewards(_wallet);
    }

    function getccr(uint256 _aPn, uint256 _aPd) external authorized {
        distributorI.cCRwds(_aPn, _aPd);
        distributorII.cCRwds(_aPn, _aPd);
    }

    function currentFirstReward() public view returns (address) {
        return distributorI.getRAddress();
    }

    function currentSecondReward() public view returns (address) {
        return distributorII.getRAddress();
    }

    function gettotaFirstlRewardsDistributed() public view returns (uint256) {
        return distributorI.gettotalDistributed();
    }

    function gettotalSecondRewardsDistributed() public view returns (uint256) {
        return distributorII.gettotalDistributed();
    }
