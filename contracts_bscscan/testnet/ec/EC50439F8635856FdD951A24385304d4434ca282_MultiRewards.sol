...
contract MultiRewards is ReentrancyGuard, Pausable {
    ...
    function unlockedStake(address account) public view returns (uint256) {
        ...
        for (uint i; i < userStakes[account].length; i++) {
	        if (userStakes[account][i].remainingBalance > 0 && userStakes[account][i].stakeTime.add(lockDuration) >= thisTest) {		
		        actualAmount = actualAmount.add(userStakes[account][i].remainingBalance);
            }
	    }
        return actualAmount;
    }
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
	    ...
	    uint askedAmount = amount;
	    uint actualAmount = 0;
	    uint thisTest = lastTimeRewardApplicable(rewardTokens[0]);
        for (uint i; i < userStakes[msg.sender].length; i++) {
	        if (userStakes[msg.sender][i].remainingBalance > 0 && userStakes[msg.sender][i].stakeTime.add(lockDuration) >= thisTest) {		
	   	        uint outAmount = Math.min(askedAmount, userStakes[msg.sender][i].remainingBalance);
