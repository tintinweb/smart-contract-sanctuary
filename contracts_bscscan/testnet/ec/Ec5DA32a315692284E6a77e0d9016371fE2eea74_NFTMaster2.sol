...
contract NFTMaster2 is Ownable, IERC721Receiver {
    ...
    function slotByOwner(address owner) public view returns (SlotCollection[] memory){
        SlotCollection[] memory slots = new SlotCollection[](nextCollectionId - 1);
        uint256 count = 0;
        for (uint256 i = 0; i < nextCollectionId - 1; i++) {
            uint256 collectionId = i + 1;
            for(uint256 k = 0 ; k < slotMap[collectionId].length ; k++){
                if (slotMap[collectionId][k].owner == address(owner)) {
                    Slot[] memory s = new Slot[](1);
                    s[0] = slotMap[collectionId][k];
                    slots[count] = SlotCollection({collectionId : collectionId, slots : s});
                    count++;
                    continue;
                }
            }
