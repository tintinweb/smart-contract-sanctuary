...
contract BatchPackages is AccessControl {
    using SafeMath for uint;
    using SafeBEP20 for IBEP20;
    IERC721 public PHE;
    bytes32 public constant CREATOR_ADMIN = keccak256("CREATOR_ADMIN");
    constructor( address minter, address _PHE ) {
    ...
    function batchPackages(address _receive, uint256[] memory listPackage) public {
        require(hasRole(CREATOR_ADMIN, address(msg.sender)), "Caller is not a admin");
        require(listPackage.length <= 20, "The number of packages is more than the limit of address");
        for(uint256 i = 0; i < listPackage.length; i++){
            PHE.safeMint(address(_receive), listPackage[i]);
            emit BatchPackage(
                _receive,
                listPackage[i],
                block.timestamp
