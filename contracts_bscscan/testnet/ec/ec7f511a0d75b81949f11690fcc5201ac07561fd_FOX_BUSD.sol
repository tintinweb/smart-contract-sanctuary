...
contract FOX_BUSD is ownerShip {
    ...
    function regUserI(address msgsender, uint _parentID,bool _new) internal returns(bool){
        ...
        if(_new) 
        {
            tokenInterface(token).transfer(address(uint160(ownerWallet)), priceOffirstLevel[2]);           
        }
    ...
    function updateNPayAutoPool(uint _level,address _user, bool _new) internal returns (bool)
    {
        ...
        if(nextMemberFillBox_[a] == 0)
        {
            ...
            else payNow(priceOffirstLevel[_level]/2, usr, _level, userInfos[_user].id, 2, false, _new);            
        }   
        else
        {
            place = 2;
            ...
            else
            {
                ...
                else payNow(priceOffirstLevel[_level]/2, ownerWallet, _level, userInfos[_user].id, 2, false, _new);                 
            }
        }
