...
constructor(IERC20 bidToken) public {
    ...
    function expand() internal returns (uint256[5] memory expandedValues) {
        ...
        for (uint256 pid = 0; pid < length; ++pid){
            ...
            for (uint256 i = 0; i < 3; i++) {
                expandedValues[i] = uint256(keccak256(abi.encode(projBidders[pid].length, i))).mod(projBidders[pid].length.sub(1));
