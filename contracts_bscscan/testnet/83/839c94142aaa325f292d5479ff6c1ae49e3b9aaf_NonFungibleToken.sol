...
contract NonFungibleToken is ERC721URIStorage, Ownable, Pausable {
  ...
  function setCollectMaxItems(uint256 _collectionMaxItems) external onlyOwner {
    require(
      _collectionMaxItems >= totalSupply,
      "max items lower than total supply"
    );
  ...
  function specialMint(address[] memory recipients, uint256[] memory amounts)
    external
    onlyOwner
  {
  ...
  function mintThe666() external onlyOwner {
    require(_exists(666) == false, "Sold out is necessary to mint this NFT");
  ...
  function withdraw() external payable returns (bool success) {
    (success, ) = payable(receiveEthAddress).call{
  ...
  function _getNumToMint() private view returns (uint256 _numToMint) {
    ...
    require(
      (_numToMint + totalSupply) <= COLLECTION_MAX_ITEMS,
  ...
  function _beforeTokenTransfer(
    ...
    if (from != address(0)) {
      ...
      if (_idToPreviousOwnerIndex < (_len - 1)) {
        _ownerToIds[from][_idToPreviousOwnerIndex] = _ownerToIds[from][
          _len - 1
        ];
