...
contract CorgiShop is Ownable {
    ...
    uint256 eggPrice = 100 ether;
    uint256 itemPrice = 10 ether;

    uint256 itemBoxLength = 0;

    uint256 buyerCommisionRatio = 500;
    uint256 referCommisionRatio = 500;
    ...
    mapping(uint256 => ItemBox) itemBoxs;
    mapping(uint8 => uint256) itemComboPrices;
    mapping(uint8 => uint256) eggComboPrices;
    ...
    function buyEggBox(uint8 _comboType, address _referral) public {
        require(
            _comboType == 3 || _comboType == 5 || _comboType == 10,
            "invalid _comboType"
        );
        uint256 price;
