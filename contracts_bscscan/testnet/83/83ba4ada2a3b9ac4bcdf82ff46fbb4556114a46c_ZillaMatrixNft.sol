...
Zilla NFT #1 Edition Collection of 10000 Uniquely generated Zilla-theme NFT. 
*/ 
...
contract ZillaMatrixNft is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant MAX_NFT = 10001; //MAX SUPPLY IS 4444 - VALIDATION < 4445 = 4444 
    uint256 public PURCHASE_LIMIT = 5;
    uint private price = 20_000_000_000_000_000;
    ...
    Counters.Counter private _zillaCounter;

    constructor() ERC721("ZillaTry NFT", "zilla1") {
        _zillaCounter.increment(); //INCREMENT THE BLOCK COUNTER TO 1 BECAUSE OUR NFT BLOCK START FROM 1 / TOKEN 1 
    ...
    function gift(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _zillaCounter.current();

            if (_zillaCounter.current() < MAX_NFT) {
                _zillaCounter.increment();
                _safeMint(to, tokenId);
            }
        }
    }
