...
// File: contracts/Pool/RisqBCHPool.sol
...
/**
 * @author macemclain
 * @title Risq BCH Liquidity Pool
 * @notice Accumulates liquidity in BCH from LPs and distributes P&L in BCH
 */
contract RisqBCHPool is
    IERCLiquidityPool,
    Ownable,
    BEP20("Risq BCH LP Token", "writeBCH")
{
    using SafeMath for uint256;
    ...
    /*
     * @nonce A provider supplies BCH to the pool and receives writeBCH tokens
    ...
    /*
     * @nonce Provider burns writeBCH and receives BCH from the pool
     * @param amount Amount of BCH to receive
    ...
    /*
     * @nonce Returns provider's share in BCH
     * @param account Provider's address
     * @return Provider's share in BCH
     */
    ...
    /*
     * @nonce Returns the amount of BCH available for withdrawals
    ...
    /*
     * @nonce Returns the BCH total balance provided to the pool
...
// File: contracts/Options/RisqBCHOptions.sol
...
/**
 * @author macemclain
 * @title Risq BCH (Binance-Peg Bitcoin Cash Token) Bidirectional (Call and Put) Options
 * @notice Risq Protocol Options Contract
 */
contract RisqBCHOptions is Ownable, IRisqOptions {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    ...
    RisqBCHPool public pool;
    IPancakeRouter01 public pancakeRouter;
    address[] public ethToBchSwapPath;
    IBEP20 public bch;

    /**
     * @param _priceProvider The address of ChainLink BCH/USD price feed contract
     * @param _pancake The address of Pancakeswap router contract
     * @param token The address of BCH BEP20 token contract
     */
    constructor(
        ...
        ethToBchSwapPath = new address[](2);
        ethToBchSwapPath[0] = pancakeRouter.WETH();
        ethToBchSwapPath[1] = address(bch);
        ...
        returns (uint256 optionID)
    {
        ...
        uint amountIn = swapToBCH(totalETH, total);
    ...
    /**
     * @notice Allows the BCH pool contract to receive and send tokens
     */
    function approve() public {
        bch.safeApprove(address(pool), uint(-1));
        bch.safeApprove(address(settlementFeeRecipient), uint(-1));
    }
        ...
        )
    {
        ...
        totalETH = pancakeRouter.getAmountsIn(total, ethToBchSwapPath)[0];
    }
     ...
     * @param period Option period in seconds (1 days <= period <= 4 weeks)
     * @param strike Strike price of the option
     * @param currentPrice Current price of BCH
    ...
    /**
     * @notice Sends profits in BCH from the BCH pool to an option holder's address
    ...
    /**
     * @notice Swap ETH to BCH via Pancakeswap router
     * @param maxAmountIn The maximum amount of ETH that can be required before the transaction reverts.
     * @param amountOut The amount of BCH tokens to receive.
     */
    function swapToBCH(
        uint maxAmountIn,
        uint amountOut
    ...
    function getOptions (address myAddress) public view returns(Option[] memory result){
       uint arrayLength = options.length;
       uint resultlength = 0;
       for  (uint256 i = 0; i < arrayLength; i++) {
           if(options[i].holder==myAddress){
               resultlength++;
           }
       }
        result = new Option[](resultlength);
        uint j=0;
