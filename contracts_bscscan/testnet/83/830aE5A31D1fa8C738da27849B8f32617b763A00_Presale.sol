...
contract Presale is Ownable {
    ...
    uint256[] public rStep = [200,400,600,800];
     uint256[] public rLockinPeriod = [200,400,600,800];
    ...
    constructor(address _token,address _recipient,uint256 _MaxValue)  {
        tokenAddress = _token;
    ...
    function setRStep(uint256[] memory _rStep) external onlyOwner {
        for(uint i=0;i< _rStep.length;i++){
    ...
    function buyToken() public payable  returns(address){
        ...
        if (totalBNBAmount < rStep[0] * (10**18)){
            ...
            wallet = createTokenLockedWallets(rLockinPeriod[1],tokenAmount,_msgSender());
        
        }
        else if (totalBNBAmount < rStep[2] * (10**18)){
            tokenAmount = tokenAmount.mul(rPercent[2]).div(100);
            wallet =createTokenLockedWallets(rLockinPeriod[2],tokenAmount,_msgSender());
        }
        else if (totalBNBAmount < rStep[3] * (10**18)){
            tokenAmount = tokenAmount.mul(rPercent[3]).div(100);
            wallet =createTokenLockedWallets(rLockinPeriod[3],tokenAmount,_msgSender());
        }
         else if (totalBNBAmount < rStep[4] * (10**18)){
            tokenAmount = tokenAmount.mul(rPercent[4]).div(100);
             wallet =   createTokenLockedWallets(rLockinPeriod[4],tokenAmount,_msgSender());
        }
    ...
    function createTokenLockedWallets(uint256 duration,uint256 tokenAmount, address _userAddress) private returns(address wallet){
     address timeLockedWallet = walletFactory.newTimeLockedWallet(_userAddress,duration);
      require(IToken(tokenAddress).transfer(timeLockedWallet, tokenAmount), "Insufficient balance of presale contract!");
