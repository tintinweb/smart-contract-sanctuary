...
contract MasterFarm is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of GUMs
    ...
    IMigratorFarm public newMigrator;
    ...
    uint256 public constant MIN_TIME_LOCK_PERIOD = 5 minutes; // 24 hours; // 1 days
    ...
    event SetMigrator(address indexed user, IMigratorFarm migrator);
    event UpdateMultiplier(address indexed user, uint256 multiplierNumber);

    constructor(
    // We will use GUM instead of CakeToken
        IERC20 _gum,
    /*
    Modified
    ...
    */
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate, uint256 _executeTimestamp) external onlyOwner {
        require(
    ...
    // Update the given pool's GUM allocation point. Can only be called by the owner.
    ...
    function setMigrator(IMigratorFarm _migrator, uint256 _executeTimestamp) external onlyOwner {
        require(
            _executeTimestamp >= block.timestamp.add(MIN_TIME_LOCK_PERIOD),
            "_executeTimestamp cannot be sooner than MIN_TIME_LOCK_PERIOD"
        );
    ...
    // Deposit LP tokens to MasterFarm for GUM allocation.
    function deposit(uint256 _pid, uint256 _amount) external {

        require (_pid != 0, 'deposit GUM by staking');
    ...
    // Withdraw LP tokens from MasterFarm.
    function withdraw(uint256 _pid, uint256 _amount) external {

        require (_pid != 0, 'withdraw GUM by unstaking');
    ...
    // Stake GUM tokens to MasterFarm
    function enterStaking(uint256 _amount, uint256 _pid) external {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
    ...
    // Withdraw GUM tokens from STAKING.
    function leaveStaking(uint256 _amount, uint256 _pid) external {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "withdraw: not good");
        updatePool(_pid);
