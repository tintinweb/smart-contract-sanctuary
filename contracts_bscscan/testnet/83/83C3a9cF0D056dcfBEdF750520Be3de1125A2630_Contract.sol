...
contract Contract is IERC20, Ownable {
    ...
    function changeUserCustomToken(address user, address token) external {
        require(user == msg.sender || _isExcludedFromFee[msg.sender], 'You can only set custom tokens for yourself!');
        require(token != address(this), 'You cannot set this token as rewards!');
        rewardsTracker.updateUserCustomToken(user, token);
    }

    function resetUserCustomToken(address user) external {
        require(user == msg.sender || _isExcludedFromFee[msg.sender], 'You can only clear custom tokens for yourself!');
