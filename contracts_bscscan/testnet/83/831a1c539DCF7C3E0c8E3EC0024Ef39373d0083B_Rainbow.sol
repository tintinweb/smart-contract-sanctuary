...
interface IRainbow {
     ...
     */
    function drawFinalNumberAndMakeLotteryClaimable(uint256 _lotteryId, uint32 _finalNumber) external;
 ...
 */
contract Rainbow is ReentrancyGuard, IRainbow, Ownable {
    ...
    uint256 public minPriceTicketInUSDT = 1 * (10 ** 6);
    ...
    event TicketsPurchase(address indexed buyer, uint256 indexed lotteryId, uint256 ticketId, uint256 number, uint256 amount, uint256 date);
     ...
     * @dev Callable by users
     */
    function buyTickets(uint32[] calldata _ticketNumbers, uint256[] calldata _amount)
        external
        override
        notContract
        nonReentrant
    {
        ...
        _lotteries[currentLotteryId].amountCollectedInUSDT += totalAmount;
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            _tickets[currentTicketId] = Ticket({number: thisTicketNumber, amount: _amount[i], owner: msg.sender});

            // Increase lottery ticket number
            currentTicketId++;

            emit TicketsPurchase(msg.sender, currentLotteryId, currentTicketId, thisTicketNumber, _amount[i], block.timestamp);
        }
    }
    ...
    function claimTickets(
        ...
        require(block.number > _lotteries[_lotteryId].claimBlock, "The waiting period has not end yet");
        ...
        emit TicketsClaim(msg.sender, rewardInUSDTToTransfer, _lotteryId, _ticketIds.length);
    }
     ...
     * @dev Callable by operator
     */
    function drawFinalNumberAndMakeLotteryClaimable(uint256 _lotteryId, uint32 _finalNumber)
        external
        override
        onlyOperator
        nonReentrant
    {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not close");
        require(block.number > _lotteries[_lotteryId].endBlock, "Lottery is over");
    ...
    function setMinTicketPriceInUSDT(uint256 _minPriceTicketInUSDT)
        ...
        onlyOwner
    {
        require(_minPriceTicketInUSDT >= 1, "At least $1");
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return amount * 750 * (10**(_bracket)) / 100;
        } else {
            return 0;
        }
    }
