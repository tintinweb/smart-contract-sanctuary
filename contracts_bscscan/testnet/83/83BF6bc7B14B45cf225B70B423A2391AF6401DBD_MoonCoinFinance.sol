...
function withdrawAll() public {	// TBA remove in production, this is to capture testbnb back, getting testbnb is tough lol
	...
	function refHandle(address msgSender,uint256 _amount) internal  returns (uint256) {
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 10; i++) {
				if (upline != address(0) && upline!= msgSender) {				
					...
					if(tokenBalanceFactor > i+1 || tokenBalanceFactor >= 5) { // token hold good					
						...
						eventId++;
						emit Custom_RefBonus(eventId, block.timestamp, msgSender,  upline,i, amount, promoMultiplier,  tokenBalanceFactor);
						eventId++;
	...
	function addMyLead(address msgSender, address referrer) public payable {
		
		require(startTheProject==1, "10");			
		require(msgSender != referrer, "12");	

		User storage user = users[msgSender];	

		if (user.referrer == address(0) && user.deposits.length == 0 && referrer != msgSender) {
			require(users[referrer].deposits.length > 0 || referrer == platformAddress , "121"); 
			
			user.referrer = referrer;
			
			user.userId=lastUserId;					
			idToAddress[lastUserId] = msgSender;
			refMapper[lastUserId]=users[referrer].userId;
			lastUserId++;	
			
			eventId++;
			emit Evt_refAdded(eventId, block.timestamp, msgSender,  referrer, user.userId, users[referrer].userId);
			address upline = user.referrer;
			for (uint256 i = 0; i < 10; i++) {
				if (upline != address(0) && upline!= msgSender) {
	...
	function invest(address msgSender, address referrer) public payable {
		...
		addMyLead(msgSender, referrer);


		
		refHandle(msgSender, msg.value);
		...
		eventId++;
		if (user.deposits.length == 0) {			
			emit Evt_Deposit(eventId, block.timestamp, msgSender, msg.value,  0, user.withdrawMisses, user.myBurnPercent, user.totalDepositByUser);
		} else {
			emit Evt_Deposit(eventId, block.timestamp, msgSender, msg.value,  1, user.withdrawMisses, user.myBurnPercent, user.totalDepositByUser);
		}
		user.deposits.push(Deposit(msg.value, 0, block.timestamp, 0));
		
		
		
		issueTokens(msgSender, msg.value); 
