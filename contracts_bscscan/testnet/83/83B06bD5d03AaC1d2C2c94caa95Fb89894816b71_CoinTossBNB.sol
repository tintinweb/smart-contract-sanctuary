...
// @title The Toss
// Author: Yakitori
contract CoinTossBNB is Ownable, ReentrancyGuard, Multicall, VRFConsumerBase {
  // Constants
  ...
  // Choose the result
  bool[2] public coinSides = [false, true];
  ...
  event ContractBroke(address user);
  ...
  struct Bet {
    ...
    bool face; // 1 bytes = 29 bytes
    bool resolved; // 1 bytes = 30 bytes
    uint80 amount; // 8 bytes = 38 bytes  = 2 storage slots
  } // Total 4 storages slots
  ...
  // @notice Pending pendingBets data
  mapping(bytes32 => Bet) public pendingBets; // set private
  // mapping between users and requestId list
  mapping(address => bytes32[]) private userBets;
  // @notice Array for failed bets, in case the contract is broke.
  bytes32[] public failedBets;

  // @notice Start the bet
  // @dev Set maximum bet size to 0 to pause it.
  function wager(bool face) external payable nonReentrant {
    // Conditions to wager
    require(msg.value <= MAXIMUM_BET_SIZE, 'Bet value is too large');
    require(msg.value >= MINIMUM_BET_SIZE, 'Bet value is too low');
    require(address(this).balance > msg.value * (2), 'Contract needs more funding');
    ...
    // Saving the bet
    pendingBets[requestId] = Bet(uint32(block.timestamp), payable(msg.sender), face, false, uint80(msg.value));
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    roll(randomness % 2, requestId);
  }

  // @notice Reveal the last pending bet of the user
  function roll(uint256 random, bytes32 requestId) internal nonReentrant {
    ...
    // Generate a random number - need to add more stuff in the abi
    uint256 result;

    // Check for the result
    if (coinSides[random] == bet.face) {
      result = ((bet.amount * (100 - FEE_NUMERATOR)) / 100) * 2;
    ...
    emit Roll(bet.user, bet.face, bet.amount, requestId, coinSides[random], result, bet.timestamp);
  }
  ...
  function modifyFeeNumerator(uint256 _feeNum) external onlyOwner {
    require(_feeNum < 15, 'Fee is faceped at 15%, choose a lower fee');
    FEE_NUMERATOR = _feeNum;
    emit UpdateFeeNum(msg.sender, FEE_NUMERATOR);
  }

  // Get an array of failed bets
  ...
  function cleanFailedBets() external onlyOwner {
    uint32 betsToClean = 0;
    for (uint32 i = 0; i == failedBets.length - 1; i++) {
      if (pendingBets[failedBets[i]].resolved) {
        betsToClean++;
      }
    }
    if (betsToClean == failedBets.length) {
      delete failedBets;
    }
  }

  // @notice Getter for the unresolved bets
  function getLastUserUnresolvedBets(address user, uint256 dataLength) external view returns (BetWithId[] memory data) {
    ...
    if (userRequestIdArray.length == 0) {
      ...
      if (userRequestIdArray.length < dataLength) {
        dataLength = userRequestIdArray.length;
      }

      uint256 counter;
      for (uint256 i = userRequestIdArray.length - dataLength; i <= userRequestIdArray.length - 1; i++) {
