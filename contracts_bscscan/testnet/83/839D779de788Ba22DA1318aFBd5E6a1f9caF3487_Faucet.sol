...
contract Faucet is Whitelist {
    ...
    {
        return (
            faucetInfo.usdt.balanceOf(_address),
            faucetInfo.busd.balanceOf(_address)
        );
    }

    function getCooldown(address _address)
        public view returns (uint256 cooldownUsdt, uint256 cooldownBusd)
    {
        return (
            cooldown[_address]["usdt"],
            cooldown[_address]["busd"]
        );
    }
    ...
    {
        (, uint256 busdBalance) = getBalance(msg.sender);
    ...
    {
        (uint256 usdtBalance, ) = getBalance(msg.sender);
