...
contract KawaiiAirdopV2 is Ownable, Pausable {
    ...
    event ClaimFullNFTAirdrop(address indexed user, uint256 amount);
    event ClaimHalfNFTAirdrop(address indexed user, uint256 amount);
    event ClaimOneNFTAirdrop(address indexed user, uint256 amount);
    event ClaimMixAirdrop(address indexed user, uint256 indexType);
    event ClaimKawaiiToken(address indexed user, uint256 amount);
    ...
    function claimToken(IERC20 kawaiiToken) external whenNotPaused {
        ...
        emit ClaimKawaiiToken(sender, amount);
    }
    ...
    function claimFullNFT(IERC1155 nftRegister) external whenNotPaused {
        ...
        emit ClaimFullNFTAirdrop(sender, tokenIds.length);
    }
    ...
    function claimHalfNFT(IERC1155 nftRegister) external whenNotPaused {
        ...
        _tokenIds = _remove(_tokenIds, indexWithStarRule);


        for (uint256 i = 0; i < 4; i++) {
            uint256 index = kawaiiRandomness.getRandomNumber(
                _tokenIds.length.sub(1),
                gasleft()
            ); //mint for user 4 item left
            nftRegister.mint(sender, _tokenIds[index], 1);
            //xoá phần tử ra khỏi mảng _tokenIds
            _tokenIds = _remove(_tokenIds, index);
        }
        emit ClaimHalfNFTAirdrop(sender, 5);
    }
    ...
    function claimOneNFT(IERC1155 nftRegister) external whenNotPaused {
        ...
        emit ClaimOneNFTAirdrop(sender, tokenIds[index]);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 index = _createMixAirdrop(nftRegister, kawaiiToken, sender);
        canClaimMix[sender] = false;
        emit ClaimMixAirdrop(sender, index);
        return index;
    }
    ...
    function _remove(uint256[] memory array, uint256 index)  private returns(uint256[] memory) {
