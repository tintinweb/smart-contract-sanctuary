...
contract PYDEXReferral is IPYDEXReferral, Ownable {
    ...
    function getMyFirstLevelCommissionRate(address _user) public view  returns(uint256){
        return refCommissionRate[_user] == 0?MIN_REFFERAL_COMMISSION:refCommissionRate[_user];
    }


    function removeReferrer(address addr) public onlyOwner  {
    ...
    function setCustomReferralCommission(uint256 rate,bool enableReferralCommission) public {
        require(rate <= MAX_REFFERAL_COMMISSION && rate <= MIN_REFFERAL_COMMISSION,"Invalid Rate");
        customReferralCommission[msg.sender] = rate;
        depositCommissionStatuses[msg.sender] = enableReferralCommission;
    }
