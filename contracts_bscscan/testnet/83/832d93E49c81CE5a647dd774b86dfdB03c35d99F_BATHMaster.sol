...
contract BATHMaster is Ownable, Withdrawable, ReentrancyGuard {
    ...
    function stake(uint256 amount) external nonReentrant {
        ...
        uint256 busdAmount = amount.div(bathPerUsd);
        busd.safeTransferFrom(msg.sender, address(this), busdAmount);
        if(feePermille > 0) {
            uint256 feeAmount = busdAmount * feePermille / 1000;
            busd.safeTransfer(treasury, feeAmount);
            // amount = amount - feeAmount.mul(bathPerUsd);
        }
    ...
    function redeem(uint256 amount) external nonReentrant {
        ...
        uint256 busdTreasuryAmount = amount.div(bathPerUsd) * treasuryPermille / 1000;
        uint256 busdTransferAmount = amount.div(bathPerUsd) * (1000 - treasuryPermille) / 1000;
