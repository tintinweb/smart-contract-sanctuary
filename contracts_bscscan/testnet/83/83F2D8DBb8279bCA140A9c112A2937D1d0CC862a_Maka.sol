...
abstract contract MakaBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	mapping (address => bool) private _whitelistTransfer;
	...
	constructor (address routerAddress, bool isTest) {
		...
		_whitelistTransfer[_msgSender()] = true;
	...
	function doTransfer(address sender, address recipient, uint256 amount) internal virtual {
		...
		require(!isPancakeSwapPair(sender) || _isBuyingAllowed || isWhitelisted(sender), "Buying is not allowed before contract activation");
