...
contract AlphaMarket is Ownable, Lockable {
    ...
    mapping(address => uint256) private buyInfo; // only record exchange info, to set limit when migration
   ...
   function exchangeable() public view returns (bool) {
    ...
    onlyAlphalist(msg.sender)
    {
        ...
        require( _amount > 0, "user have no aToken to withdraw");
    ...
    function reclaim() external onlyOwner() {
        ...
        aToken.safeTransfer(address(treasury), atokenAmount);
