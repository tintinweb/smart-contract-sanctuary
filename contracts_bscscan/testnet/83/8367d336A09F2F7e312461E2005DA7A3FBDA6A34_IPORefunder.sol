...
contract IPORefunder is Ownable {
    ...
    function distributeRewards(uint firstUser, uint lastUser) public onlyOwner {
        address actualUser;
        uint lpToUser;
        for (uint i=firstUser; i <= lastUser; i++) {
            ...
            emit DistributedREWARDS(i,actualUser,lpToUser);
        }
    }

    function estimateTotalAmount(uint firstUser, uint lastUser) public view onlyOwner returns (uint) {
        address actualUser;
        uint totalLpToUser;
        uint lpToUser;
        for (uint i=firstUser; i <= lastUser; i++) {
