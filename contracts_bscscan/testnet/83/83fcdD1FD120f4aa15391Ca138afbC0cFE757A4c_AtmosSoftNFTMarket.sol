...
contract AtmosSoftNFTMarket is Ownable,ReentrancyGuard, IERC721Receiver {
    ...
    event BuyListing(address _nft, uint256 _tokenId, uint256 _listingID);
    ...
    constructor(address _zapper, address _zapReciever, address _targetLP){
    ...
    function addListing(address _nft, uint256 _tokenId, uint256 _price, uint256 _rarity) payable public {
        ...
        //zap fee into LP
        zapper.zapIn{value: msg.value}(targetLP);
    ...
    function buyListing(address _nft, uint256 _tokenId, uint256 _listingID, uint256 _rarity) payable public {
        ...
        //zap purchase fee to LP
        
        //require fee is sent        
        require(msg.value >= buyingFeeForRarity[_rarity] + listing.price, "buyListing: Amount not provided for listing fee");

        //zap fee into LP
        zapper.zapIn{value: buyingFeeForRarity[_rarity]}(targetLP);
        
        //transfer purchase price to listee.
        listing.seller.transfer(listing.price);

        // user List update
        userListings[msg.sender][listingIdUserListingIndex[_listingID - 1]] = listing;
        ...
        emit BuyListing(_nft, _tokenId, _listingID);
    }
    ...
    function update(uint256 _listingId, uint256 _price) public { //user can update listing
        Listing storage listing = userListings[msg.sender][listingIdUserListingIndex[_listingId - 1]];
        require(listing.seller == msg.sender, "update: only seller can update listing");
        listing.price = _price;
        
        emit Update(_listingId, _price);
    }
    ...
    function setZapAddress(address newZap) external onlyOwner{ //ability to change the zapper
        zapper = IZapInterface(newZap);
        emit SetZapAddress(newZap);
    }
