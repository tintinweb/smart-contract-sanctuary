...
contract Token is ERC20, Ownable {
    ...
    address public defaultInvitationWallet;
    mapping(address => address) public invitation;
    mapping(address => bool) public notInvitation;

    // timestamp for when the token can be traded freely on PanackeSwap
    uint256 public tradingEnabledTimestamp = 1625400000; //July 4, 12:00 UTC, 2021
    ...
    constructor() public ERC20("PolyJoker", "PolyJoker") {
        ...
        defaultInvitationWallet = 0x2c206505038c04D56A66F58E19692CC1c55740aD;
        ...
        // YouSwap
        // dividendTracker.excludeFromDividends(address(0xA4CE57F063A610290EEEF0564B034278438D06CF));
    	// quickSwap
    ...
    receive() external payable {
  	...
  	function setExcludeFromDividends(address o) public onlyOwner{
  	    dividendTracker.excludeFromDividends(o);
  	}
    ...
    function setTradingEnabledTimestamp(uint256 time) public onlyOwner{
        tradingEnabledTimestamp = time;
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            uint256 invitationFees = 4;
            uint256 realTotalFees = totalFees.add(invitationFees);
            
        	uint256 fees = amount.mul(realTotalFees).div(100);
            ...
            uint256 iFees = fees.mul(invitationFees).div(realTotalFees);
            
            if(invitation[from] == address(0x0000000000000000000000000000000000000000)){
                invitation[from] = defaultInvitationWallet;
            }
            super._transfer(from, invitation[from], iFees);
    ...
    function setNotInvitation(address o, bool value) public onlyOwner {
        require(notInvitation[o] != value, "notInvitation is already set to that value");
        notInvitation[o] = value;
    }
    
    function getInvitation(address o)public returns(address){
        return invitation[o];
    }
    
    function setInvitation(address o)public {
        require(o != address(0), "invitation address the zero address");
        invitation[msg.sender] = o;
    }
...
contract TIKIDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("BUN_Dividend_Tracker", "BUN_Dividend_Tracker") {
    	claimWait = 3600;
