...
contract KsharkIDO is Ownable, ReentrancyGuard {
  using SafeMath for uint256;

  IKsharkHoldIDOStake public contractHoldStakeInterface;

  address payable public devAddress; // address of contract dev
  address public poolOwner; // Address user pool Owner and hold invest

  ERC20 public token; // token that will be sold
  ERC20 public stakingToken; // token that require staked for buy IDO
  ERC20 public tokenInvest; // token that require staked for buy IDO
  uint256 public amountRequireStakeToken; // Amount require staked
  address public unsoldTokensDumpAddress; // address where unsold tokens will be transferred to

  mapping(address => bool) public isStaked; // Check address user has staked before
  ...
  uint256 public startTimeStaking; // Time to start accept staking
  uint256 public endTimeStaking; // Time end accept staking
  ...
  event HasUserStaked(address user, uint256 amout);
  ...
  modifier isValidClaimPeriod() {
    require(block.timestamp >= claimTime, "Listing not started");
  ...
  modifier tokenStakedOnly() {
    require(isStaked[msg.sender] == true, "Address not staked token");
    _;
  }

  function setContractHoldStake(address _contractAddress) external onlyOwner {
    contractHoldStakeInterface = IKsharkHoldIDOStake(_contractAddress);
  }

  function setAddressDev(address _devAddress) external onlyDev {
    devAddress = payable(_devAddress);
  }

  function setTokenInvest(address _newTokenInvest) external onlyDev {
    tokenInvest = ERC20(_newTokenInvest);
  }

  // Set staking Token and amount require staking
  function setStakingTokenAndAmount(address _stakingToken, uint256 _amount)
    external
    onlyDev
  {
    require(_stakingToken != address(0), "Staking Token Error");

    stakingToken = ERC20(_stakingToken);
    amountRequireStakeToken = _amount;
  }

  // Set time accept staking
  function setTimeStaking(uint256 _timeStart, uint256 _timeEnd)
    external
    onlyDev
  {
    require(_timeEnd >= _timeStart);

    startTimeStaking = _timeStart;
    endTimeStaking = _timeEnd;
  }

  // Set address token IDO and address will receive token unsold
  ...
  function setGeneralInfo(
    ...
    tokensLeft = _totalTokens; // token left == totalTokens when init set
  ...
  function setRefundAllowed(bool _refundAllowed) external onlyDev {
  ...
  function getTokenAmount(uint256 _weiAmount) internal view returns (uint256) {
    return _weiAmount.mul(10**token.decimals()).div(tokenPriceInWei);
  }

  // staked token to participate ido
  // Need approve contract to token
  function stakeToken() public nonReentrant presaleIsNotCancelled {
    require(block.timestamp >= startTimeStaking, "Stake is not open now");
    require(block.timestamp <= endTimeStaking, "End time stake");
    require(isStaked[msg.sender] == false, "User has staking before!");

    // transfer to contract hold stake
    contractHoldStakeInterface.transferTokenStake(
      msg.sender,
      amountRequireStakeToken
    );

    isStaked[msg.sender] = true;

    emit HasUserStaked(msg.sender, amountRequireStakeToken);
  }
    ...
    presaleIsNotCancelled
  {
    ...
    require(totalCollectedWei.add(_amount) < hardCapInWei, "Hard cap reached");
    require(tokensLeft > 0, "Token sold out");
    require(
      _amount <= tokensLeft.mul(tokenPriceInWei),
      "Token left is not enough!!!"
    );

    // total amount invest per wallet can between minInvestInWei and maxInvestInWei
    // if amount not reached, user can invest multi time.
    uint256 totalInvestmentInWei = investments[msg.sender].add(_amount);
    require(_amount >= minInvestInWei, "Min investment not reached");
    require(
      maxInvestInWei == 0 || totalInvestmentInWei <= maxInvestInWei,
      "Max investment reached"
    );

    // Send token invest to wallet poolOwner
    tokenInvest.transferFrom(msg.sender, poolOwner, _amount);
  ...
  function transferUnsoldTokens() external onlyDev presaleIsNotCancelled {
