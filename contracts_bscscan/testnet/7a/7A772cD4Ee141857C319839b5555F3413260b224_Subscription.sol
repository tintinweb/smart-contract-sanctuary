...
contract Subscription is Ownable, Permission {
    ...
    function subscribe(uint256 _planId, address _token) external onlyAllow {
        ...
        if (_token == BUSDAddress) {
            uint256 BUSDRatio = IOraclePrice(oraclePriceAddress).getPrice("BUSD", "USD");
            ...
            amount = IOraclePrice(oraclePriceAddress).getAmountToken("BUSD", "USD", _token, BUSDAddress, plan.amount);
