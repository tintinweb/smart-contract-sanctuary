...
contract aaa {
    ...
    uint256 fee_percent10000 = 10000;
    ...
    //
    mapping(uint256 => bool) depositLimitMap;
    //rewardToken
    ...
    constructor() public {
        owner = msg.sender;
        admin = 0x3B205CEe4F2628DE9f6cA172d390B6BA49b9b3A4;
        _init();
    }
    ...
    function _init() internal {
        ...
        depositLimitMap[100 * ONE_TOKEN] = true;
        depositLimitMap[1000 * ONE_TOKEN] = true;
        depositLimitMap[10000 * ONE_TOKEN] = true;
        depositLimitMap[100000 * ONE_TOKEN] = true;
        depositLimitMap[1000000 * ONE_TOKEN] = true;
        depositLimitMap[10000000 * ONE_TOKEN] = true;

        setRandomRateRangeEncrypt2encrypt(20, 110);
    }
    ...
    function setRandomRateRangeEncrypt2encrypt(uint256 min, uint256 max) public onlyOwner {
        rateMin = min;
        rateMax = max;
    }
    ...
    function depositEncrypt2encrypt(uint256 amount, bytes32 withdrawHash, bytes32 feeHash, bytes32 callHash) public changeNonce {
        require(depositLimitMap[amount], "deposit limit");
    ...
    function withdrawEncrypt2encrypt(bytes32 h1) public changeNonce {
        ...
        if (passday == 0) {
            ...
            order.valid = 2;
            order.bonus = order.amount * order.rate * passday / fee_percent10000;
            historyTotalBonus = historyTotalBonus.add(order.bonus);
        }

        curCount = curCount.sub(1);
        curTotalAmount = curTotalAmount.sub(order.amount);

        safeTransfer(usdtToken, msg.sender, order.amount);
        User storage user = userMap[order.stakeAddress];
    ...
    function getOrderInfoEncrypt2encrypt(bytes32 hash1) public view returns (uint256, uint256, uint256, uint256, uint256, uint8, uint8){
        ...
        if (od.bonus == 0) {
            ...
            bonus = od.amount * od.rate * passday / fee_percent10000;
        } else {
            bonus = od.bonus;
        }
    ...
    function randomRate() public view returns (uint256){
        ...
        return num.mod(rateMax.sub(rateMin)).div(fee_percent10000).add(rateMin);
    }
    ...
    function getTotalInfoEncrypt2encrypt() public view returns (uint256, uint256, uint256){
        return (curCount, curTotalAmount, historyTotalBonus);
    }
