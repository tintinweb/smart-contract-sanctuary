...
contract TMHPrivateSale is Ownable{
    ...
    uint256 public refCommission = 0;
    
    mapping(address => uint256) private _balances;
    mapping (address=>uint) public refRewards;

    event TokensPurchased(address indexed purchaser, address indexed beneficiary, address referral, uint256 value, uint256 amount);
    ...
    function poolInfo() public view returns(
        ...
        totalTokenSale = totalToken;
        capReached = _weiRaised >= _hardCap;
        hasClose = block.timestamp > _closingTime;
    }
    ...
    function setRefCommission(uint _rate) external onlyOwner{
        refCommission = _rate;
    }
    ...
    function _toBNB(uint tokenAmount) private view returns(uint){
        return tokenAmount.mul(_rateNumerator).div(_rateDenominator);
    }
    ...
    function buyTokens(address referral) payable public {
        require(referral != _msgSender(), "buyer and referal is the same");
        ...
        if (referral != address(0) && referral != _msgSender()) {
            uint256 referCommision = tokenAmount.mul(refCommission).div(100);
            refRewards[referral] = refRewards[referral].add(referCommision);
        }
