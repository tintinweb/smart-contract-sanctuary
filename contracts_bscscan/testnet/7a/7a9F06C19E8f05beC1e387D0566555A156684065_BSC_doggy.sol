...
contract BSC_doggy is ERC721Enumerable, Ownable{
    ...
    constructor(address _beneficiary) ERC721("BSC_doggy", "BSC_doggy") {
        _baseTokenURI = "https://gateway.pinata.cloud/ipfs/QmY5GJbMyFwyXw9svFcv4HKEFGdbRyBVV3AGWkd7F3Za9M/";
    ...
    function getPrice(uint256 _id) public pure returns (uint256) {
        if (_id > 7500) {
            return 2e18;
        }else if (_id > 5000) {
            return 1.5e18;
    ...
    function mint_character() public payable returns (uint256){
        require(totalSupply() <= 10000, "enough minting");

        _tokenIds.increment();
        uint256 newCharacter = _tokenIds.current();
        uint256 price = getPrice(newCharacter);
