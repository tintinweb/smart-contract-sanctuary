...
contract MainMiner is ReentrancyGuard {
    ...
    uint256 _cs = 1800*1e40; // 32 + 8
    uint256 _maxhash = 4e25; // 7 + 18
    ...
    event WithDrawCredit(address indexed sender, uint256 jtamount, uint256 dtamount, uint256 cutamount, uint256 feerate);
    ...
    uint256[9] _holdhash = [0 * 1e18, 500 * 1e18, 1000 * 1e18, 3000 * 1e18, 5000 * 1e18, 10000 * 1e18, 15000 * 1e18, 20000 * 1e18,30000 * 1e18];
    ...
    function withDrawCredit() public nonReentrant returns (bool) {
        ...
        emit WithDrawCredit(user, amount[0], amount[1], cut, _feerate);
