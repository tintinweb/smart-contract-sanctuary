...
contract StarboundETF is ERC20, Ownable, Pausable {
    ...
    ) public ERC20 ("Starbound ETF", "sETF")
    {
        ...
        assetInfo[_primaryAsset] = AssetInfo(0, ERC20(_primaryAsset).decimals(), 0);
    ...
    function unwrap(uint _amount) external whenNotPaused whiteListed {
        ...
        for (uint i = 0; i < assets.length(); i++) {
            ...
            require (amountToRelease <= curBal, 'Insufficient asset fund');
    ...
    function calculateFundsLayout(address _asset, uint _amount, bool _flag) external view returns (uint newPrice, uint[] memory newWeights) {
        uint[] memory assetWeights = new uint[](assets.length());

        if (!assets.contains(_asset) || totalSupply() == 0) return (basePrice, assetWeights);

        uint totalValue = 0;
        uint[] memory assetValues = new uint[](assets.length());
        for (uint i = 0; i < assets.length(); i++) {
            address asset = assets.at(i);
            uint assetFund = totalFunds[asset];
            if (asset == _asset) {
                if (_flag == true) {
                    assetFund += _amount;
                } else {
                    assetFund = assetFund > _amount ? assetFund.sub(_amount) : 0;
                }
            }
            assetValues[i] = valueOfAsset(asset, wbnb, assetFund);
            totalValue += assetValues[i];
        }

        for (uint i = 0; i < assets.length(); i++) {
            assetWeights[i] = assetValues[i].mul(MAX_FEE).div(totalValue);
        }

        return (totalValue.mul(1e18).div(totalSupply()), assetWeights);
    }
    ...
    function _amountOfAsset(address _from, address _to, uint _amount) internal view returns (uint) {
        if (assetInfo[_from].weight == 0) return 0;
        return valueOfAsset(_from, _to, _amount).mul(assetInfo[_to].weight).div(assetInfo[_from].weight);
    }

    function _updateAssetWeights() internal {
        uint totalValue = 0;
        uint[] memory assetValues = new uint[](assets.length());
        for (uint i = 0; i < assets.length(); i++) {
            address asset = assets.at(i);
            assetValues[i] = valueOfAsset(asset, wbnb, totalFunds[asset]);
            totalValue += assetValues[i];
        }

        for (uint i = 0; i < assets.length(); i++) {
            if (totalValue == 0) assetInfo[assets.at(i)].weight = 0;
            else assetInfo[assets.at(i)].weight = assetValues[i].mul(MAX_FEE).div(totalValue);
        }
    }
    ...
    function updateAssetWeights() external whenPaused onlyKeeper {
        _updateAssetWeights();
    }

    // It should be called before funding from users at the first
    function addAsset(address _token) external whenPaused onlyOwner {
        require(assets.contains(_token) == false, "already existing asset");
        assets.add(_token);
        assetInfo[_token] = AssetInfo(0, 18, 0);
    }
