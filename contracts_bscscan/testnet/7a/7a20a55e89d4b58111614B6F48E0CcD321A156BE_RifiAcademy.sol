...
contract RifiAcademy is Signed, Ownable, Pausable {
    ...
    constructor(INFTPlayerCore _nftPlayer, INFTPlayerRandomness _nftRandomness, IBEP20 _rifi) public {
        rifi = _rifi;
        nftPlayer = _nftPlayer;
        nftRandomness = _nftRandomness;
    ...
    external {
        ...
        require(_inputMetadatas.length == _weights.length, "must same length");
        uint256 nonce = nonces[msg.sender]++;
        (uint8 v, bytes32 r, bytes32 s) = abi.decode(adminSignedData, (uint8, bytes32, bytes32));
        address signer = getSigner(
            keccak256(
                abi.encode(address(this), this.generateRandomMetadata.selector, _inputMetadatas, _weights, fee, nonce)
            ), v, r, s);
        ...
        uint256 rand = nftRandomness.getRandomNumber(totalWeight, gasleft());

        string memory selectedMetadata = _inputMetadatas[selectMetadata(_weights, rand)];

        nftPlayer.mint(msg.sender, selectedMetadata);
    }

    function selectMetadata(uint256[] memory _weights, uint256 rand) public pure returns (uint256){
        uint256 indexId;
