...
contract PrivateSale is Initializable , OwnableUpgradeable{
  ...
  mapping(address => UserData) public userDataLock;  
  ...
  function resetWrongClaim(address ref) public onlyOwner {
    userDataLock[ref].firstReleaseBlock = 0;
  }
  ...
  function addInvester(address ref, uint256 totalInvest) public onlyOwner {
    uint256 lockAmount = totalInvest * 925 /1000;
    userDataLock[ref].lockAmount = lockAmount;
    userDataLock[ref].firstRelease = totalInvest - lockAmount;
  }
  ...
  function removeWhiteList(address ref) public onlyOwner {
    userDataLock[ref].firstRelease = 0;
    userDataLock[ref].lockAmount = 0;
  }
  ...
  function getClaimable(address buyerAddress) public view returns (uint256) {
    ...
    uint256 totalLockAmount = userDataLock[buyerAddress].lockAmount;
    if (totalLockAmount == 0 ) {
        return 0;    
    }
    if (userDataLock[buyerAddress].firstReleaseBlock == 0 ) {
        return userDataLock[buyerAddress].firstRelease;    
    }

    uint256 userStartUnlockBlock = userDataLock[buyerAddress].firstReleaseBlock + totalCiffBlock;
    ...
    return fullclaimableAmount - userDataLock[buyerAddress].claimedAmount;
  }  
  ...
  function claim(address recipient) public unlockEnabled {
    require(userDataLock[recipient].lockAmount > 0, "the address is not valid!");
    uint256 claimableAmount = getClaimable(recipient);
    if (userDataLock[recipient].firstReleaseBlock == 0 ){
      userDataLock[recipient].firstReleaseBlock = block.number;
    } else {
      userDataLock[recipient].claimedAmount += claimableAmount;
    }
