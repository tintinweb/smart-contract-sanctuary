...
contract IANFT is BRC721Enumerable, Pausable {
  using Strings for uint256;
...
contract NFT721 is IANFT, ERC721Locker {
  using Address for address;
  uint256 private _totalBurned;
  mapping(uint256 => address) private _lockedBy;

  constructor() IANFT(_msgSender(), "https://", "NFT", "NFT NAME") {}
  ...
  function lock(uint256 tokenId, address locker) external override returns (bool) {
    ...
    if (locker.isContract()) {
      return
        IERC721Locker(locker).onERC721Locked(_msgSender(), locker, tokenId);
    } else {
      return true;
    }
  }

  function unlock(uint256 tokenId) external override returns (bool) {
    require(_exists(tokenId), "ERC721: operator query for nonexistent token");
    address _locker = _lockedBy[tokenId];
    require(_locker == _msgSender(), "caller is not locker nor admin");
    delete _lockedBy[tokenId];
    if (_locker.isContract()) {
      return
        IERC721Locker(_locker).onERC721Unlocked(ownerOf(tokenId), _locker, tokenId);
    } else {
      return true;
    }
  }
