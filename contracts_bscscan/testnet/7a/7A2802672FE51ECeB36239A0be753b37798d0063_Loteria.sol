...
Sell / Transfer from wallet to wallet:
4.5% Holders
4.5% lottery
6% marketing (Bnb)
2% liquidity
...
ABOUT THE LOTTERY:
Each buy and sell, a percentage goes to a random wallet that has at least 20 million tokens.
If the drawn wallet does not have the 20 million tokens, try again to choose the new wallet and if in the
second attempt does not find a wallet with more than 20 million tokens, the tokens will be burned.

ANTI-WHALE SYSTEM:
Above 250M tokens = 2.5% extra fee
Above 500M tokens = 5% extra fee
Above 1B tokens = 7.5% extra fee
Direct added to liquidity
...
Venda / Transferencia de carteira para carteira:
4.5% Holders
4.5% loteria
6% marketing (Bnb)
2% liquidez
...
SOBRE A LOTERIA:
Cada tcompra e venda, uma porcentagem vai para um carteira aleatório que tenha pelo menos 20 milhões de tokens
Se a carteira sorteada nao possuir os 20 milhoes de tokens, tenta novamente para escolher noca carteira e se na 
segunda tentativa não achar uma carteira com mais de 20 milhões de tokens, os tokens serão queimados.

SISTEMA ANTI-BALEIA:
Acima de 250M tokens = 2.5% de taxa extra
Acima de 500M tokens = 5% de taxa extra
Acima de 1B tokens = 7.5% de taxa extra
Adicionado direto a liquidez
...
contract Loteria is Context, IERC20, Ownable {
    ...
    string private constant _name = "Loteria";
    string private constant _symbol = "Loteria";

    uint256 public _minLottoBalance = 500 * 10**_decimals; // 50 Bilhões para participar da loteria
...
//        IUniswapV2Router02 _PancakeSwapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //BSC mainnet
        ...
        antiwhale_measures[2] = antiwhale({selling_treshold: _tTotal/100, extra_tax: 75});
