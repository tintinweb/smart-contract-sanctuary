...
import {IDividendDistributor} from "./interfaces/IDvdDistributor.sol";
...
contract ScGame is IBEP20, Auth {
    ...
    function setWhiteList(address holder, bool iswhitelist)
        external
        authorized
    {
        ...
        returns (bool)
    {
        return
            !(isFeeExempt[sender] ||
                isFeeExemptRecipient[recipient] ||
                isWhiteList[sender] ||
                isWhiteList[recipient]);
    }

    function getTotalFee(address sender, address receiver)
        public
        view
        returns (uint256)
    {
    ...
    function takeFee(
        ...
        uint256 feeAmount = amount.mul(getTotalFee(sender, receiver)).div(
            feeDenominator
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = swapThreshold
            ...
            .div(totalFee)
            .div(2);
        uint256 amountReflection = swapThreshold.mul(reflectionFee).div(
            totalFee
        );
    ...
    function triggerAdminBuyback(uint256 amount, bool triggerBuybackMultiplier)
        external
        authorized
    {
    ...
    function setIsFeeExemptRecipient(address holder, bool exempt)
        external
        authorized
    {
