...
ERC721HolderUpgradeable
{
    ...
    mapping (uint => uint) public cardsRemaining;
    mapping (uint => uint) public cardsRatio;
    ...
    uint[] private allCandidateCards;
    ...
    function fulfillRandomness(uint256 tokenId, address user, uint256 randomness) internal {
        ...
        burn(tokenId);
        uint i = cardIds.length();
        uint index = totalBlindBoxRemaining;
        do {
            index = _randModulus(user, randomness, i, totalBlindBoxRemaining);
            uint cardId = indexToCardId(index);

            uint originRatio = cardsRatio[cardId];
            uint curRatio = cardsRemaining[cardId] * 1e12 / totalBlindBoxRemaining;
            uint range = originRatio * _randModulus(user, randomness, i, 11) / 100;

            if (curRatio + range >= originRatio) {
                break;
            }
            i -= 1;
        } while (i > 0);

        uint selectedCardId = indexToCardId(index);

        cardsRemaining[selectedCardId] -= 1;
        totalBlindBoxRemaining -= 1;

        uint nftId = nftToken.mint(selectedCardId);
        nftToken.safeTransferFrom(address(this), user, nftId);
        nftIdtoCardId[nftId] = selectedCardId;
        emit Open(tokenId, selectedCardId, nftId);
    }
