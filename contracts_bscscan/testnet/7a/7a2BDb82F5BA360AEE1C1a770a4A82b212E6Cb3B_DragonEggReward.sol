...
contract DragonEggReward is  ReentrancyGuardUpgradeable, IDragonEggReward , OwnableUpgradeable {
...
//    }
    ...
    function setup(address _signer, address _beneficiary) public initializer {
        signer = _signer;
        beneficiary = _beneficiary;
    }
    ...
    function validateSig(uint salt, uint price, address buyToken, Sig memory sig) internal view returns(address) {
        ...
        for(uint i = 0; i < eggAddresses.length; i++) {
            ...
            if(keccak256(abi.encode(this, salt, egg, price, buyToken, msg.sender)).toString().recover(sig.v, sig.r, sig.s) == signer) {
                signedEgg = egg; // do not return to keep the same gas for all item
            }
        }
        require(signedEgg != address (0), "incorrect reward signature");
        return signedEgg;
    }
