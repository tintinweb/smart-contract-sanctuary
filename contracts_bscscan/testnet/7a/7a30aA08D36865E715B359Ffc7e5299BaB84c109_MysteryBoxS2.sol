...
contract MysteryBoxS2 is RandomGenerator, Context {
    ...
    // 二开
    mapping (address => mapping (uint => bool)) public secOpenBag;
    ...
    constructor(address KTA_, address Msbox_, address U_, address wallet_) {
        ...
        uint[7] memory amounts_ = [uint(90), uint(190), uint(350), uint(1000), uint(2000), uint(3000),uint(5200)];
        ...
        uint[4] memory secAmounts_ = [uint(7), uint(13), uint(24), uint(292)];
        ...
        for (uint i = 0; i < secCardBagCap; ++i) {
            ...
            secCardAmount = secCardAmount.add(amounts_[i]);
        }
    }
    ...
    function openBox() external openBoxStage returns (uint) {
        ...
        uint level = _randomCardLevel(firstCardAmount, firstCardBagCap, firstCardBag);
        uint cardId = firstCardBag[level].cardId;
        uint tokenId = KTA.mint(_msgSender(), cardId);

        if (level > 2) {
            secOpenBag[_msgSender()][tokenId] = true;
        }
    ...
    function openBoxBatch(uint amounts_) external openBoxStage returns (bool){
        ...
        for (uint i = 0; i < amounts_; ++i) {
            level = _randomCardLevel(firstCardAmount, firstCardBagCap, firstCardBag);
            if (level > 2) {
                secOpenBag[_msgSender()][tokenId] = true;
            }
    ...
    function secOpen(uint tokenId_, uint signtime_, bytes32 r, bytes32 s, uint8 v) external openBoxStage returns (uint) {
        require(block.timestamp <= signtime_, "Signature expired");

        bytes32 hash =  keccak256(abi.encodePacked(signtime_, _msgSender()));
        address a = ecrecover(hash, v, r, s);

        require(a == banker, "Invalid signature");
        require(secOpenBag[_msgSender()][tokenId_], "Invalid token id");

        KTA.burn(tokenId_);

        uint cardIndex;
        uint tokenId;
        uint level = _randomCardLevel(secCardAmount, secCardBagCap, secCardBag);

        for (uint i = 0; i < secCardBagCap; ++i){
            cardIndex = cardIndex.add(secCardBag[i].amount);
            if (level <= cardIndex) {
                secOpenBag[_msgSender()][tokenId] = false;

                uint cardId = secCardBag[level].cardId;
                if (cardId == 0){
    ...
    function _randomCardLevel(uint num_, uint cardCap_, mapping(uint => cardBagInfo) storage bag_) internal returns (uint) {
        uint level = randomCeil(num_);
        uint cardIndex;
        for (uint i = 0; i < cardCap_; ++i){
            cardIndex = cardIndex.add(bag_[i].amount);
            if (level <= cardIndex) {
                return i;
            }
        }
