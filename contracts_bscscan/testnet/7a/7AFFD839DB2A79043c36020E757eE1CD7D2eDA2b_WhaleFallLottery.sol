...
contract WhaleFallLottery is ReentrancyGuard, IWhaleFallLottery, Ownable {
    ...
    uint256 public maxPriceTicketInWhaleFall = 50 ether;
    uint256 public minPriceTicketInWhaleFall = 0.005 ether;
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                // A. No WhaleFall to distribute, they are added to the amount to withdraw to treasury address
            } else {
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId]
            .amountCollectedInWhaleFall - amountToShareToWinners);

        // Transfer WhaleFall to treasury address
        whaleFallToken.safeTransfer(
            treasuryAddress,
            amountToWithdrawToTreasury
        );
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInWhaleFall >= minPriceTicketInWhaleFall) &&
                (_priceTicketInWhaleFall <= maxPriceTicketInWhaleFall),
            "Outside of limits"
        );
    ...
    /**
     * @notice Set WhaleFall price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInWhaleFall: minimum price of a ticket in WhaleFall
     * @param _maxPriceTicketInWhaleFall: maximum price of a ticket in WhaleFall
     */
    function setMinAndMaxTicketPriceInWhaleFall(
        uint256 _minPriceTicketInWhaleFall,
        uint256 _maxPriceTicketInWhaleFall
    ) external onlyOwner {
        require(
            _minPriceTicketInWhaleFall <= _maxPriceTicketInWhaleFall,
            "minPrice must be < maxPrice"
        );

        minPriceTicketInWhaleFall = _minPriceTicketInWhaleFall;
        maxPriceTicketInWhaleFall = _maxPriceTicketInWhaleFall;
    }
