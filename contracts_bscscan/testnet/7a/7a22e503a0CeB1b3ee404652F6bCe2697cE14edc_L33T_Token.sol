...
contract ERC20 is Context, IERC20 {
    ...
    function unstakeTokens(uint256 amount) public {
        ...
        require(getUnlockTime(msg.sender) == 0, "Wait until staking lock is over.");
        withdrawEarnings();
        totalStaked = totalStaked.sub(amount);
        stakedAmount[msg.sender] = stakedAmount[msg.sender].sub(amount);
        uint256 unstakeTax = amount.div(10);
        uint256 addToBalance = amount.sub(unstakeTax);
        _balances[msg.sender] = _balances[msg.sender].add(addToBalance);
    }
    
    function getUnlockTime(address user) public view returns (uint256){
        uint256 passedBlocks = block.number.sub(stakedBlock[user]);
        if(passedBlocks >= 201600) return 0;
        else return (604800 - passedBlocks.mul(3));
    }
...
contract L33T_Token is ERC20 {
    constructor() ERC20("L33T Token", "L33T", 10000000000000000000000000) {
    }
}