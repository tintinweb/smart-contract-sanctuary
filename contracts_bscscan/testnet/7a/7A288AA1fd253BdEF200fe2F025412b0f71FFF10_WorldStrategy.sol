...
import "../library/AirdropEquation.sol";
...
contract WorldStrategy is IRewardStrategy, ERC165 {
    using SafeERC20 for IMintableToken;

    PoiCenter private _poiCenter;
    IMintableToken private _mintableToken;
    IERC721 private _erc721;
    Friends private _friends;
    CommunityVault private _communityVault;
    PoiStake private _poiStake;

    address public strategyFactoryAddr;

    uint256 public lastUpdateTime = 0;
    uint256 public rewardPerCVStored;
    uint256 public rewardRate;
    uint256 public timePeriodRate = 1;
    uint256 public baseTimePeriodRate = 1;
    mapping(uint256 => uint256) public poiRewardPerCVPaid;
    mapping(uint256 => uint256) public poiRewardPerToken;
    mapping(uint256 => mapping(address => uint256))
        public userRewardPerTokenPaid;
    mapping(uint256 => mapping(address => uint256))
        public holderRewardPerTokenPaid;
    uint256[5] public levelAmounts; //1,2,3,4,5
    mapping(uint256 => uint8) public poiLevelMap;

    uint256 public periodFinish = block.timestamp + 30 days;
    uint8 public roundTimes = 0;
    mapping(uint256 => mapping(address => uint256)) public holderRewards;
    mapping(uint256 => mapping(address => uint256)) public stackRewards;

    uint256 public constant BASE_REWARD_RATE = 1e27;
    bytes32 public constant REWARD_STRATEGY_FACTORY =
        keccak256("RewardStrategyFactory");

    event LogGetAirdropReward(
        address indexed who,
        uint256[] tokenIdArray,
        uint256[] holderRewardArray,
        uint256[] stakeRewardArray
    );
    event LogCheckDecline(uint8 roundTimes);

    modifier onlyFactory() {
        require(msg.sender == strategyFactoryAddr, "not factory");
        _;
    }

    constructor(
        PoiCenter pPoiCenter,
        IMintableToken pMintableToken,
        IERC721 pErc721,
        ...
        _poiCenter = pPoiCenter;
        _mintableToken = pMintableToken;
        _erc721 = pErc721;
        _friends = pFriends;
        _communityVault = pCommunityVault;
        _poiStake = pPoiStake;

        strategyFactoryAddr = address(pRewardStrategyFactory);
    }
    ...
    function onPoiChangeOwnership(
        ...
        if (from != address(0)) {
            updateAirdropReward(tokenId, from);
        }
        if (to != address(0)) {
            updateAirdropReward(tokenId, to);
        }
    }
    ...
    function onUpdateReward(uint256 tokenId, address account)
        ...
        onlyFactory()
    {
        updateAirdropReward(tokenId, account);
    }
    ...
    function onUpdateRewardRate(uint256 tokenId)
        ...
        onlyFactory()
    {
        uint256 newCv = _poiStake.getPoiCV(tokenId);
        uint8 newLevel = 0;
        if (newCv >= 20000 * 10**18) {
            newLevel = 1;
        } else if (newCv >= 10000 * 10**18 && newCv < 20000 * 10**18) {
            newLevel = 2;
        } else if (newCv >= 5000 * 10**18 && newCv < 10000 * 10**18) {
            newLevel = 3;
        } else if (newCv >= 1000 * 10**18 && newCv < 5000 * 10**18) {
            newLevel = 4;
        } else if (newCv > 0 && newCv < 1000 * 10**18) {
            newLevel = 5;
        }
        uint8 poiLevel = poiLevelMap[tokenId];
        if (poiLevel != newLevel) {
            if (poiLevel != 0) {
                levelAmounts[poiLevel - 1] -= 1;
            }
            if (newLevel != 0) {
                levelAmounts[newLevel - 1] += 1;
            }
            poiLevelMap[tokenId] = newLevel;
            console.log(
                "onUpdateRewardRate poiLevel %s newLevel %s",
                poiLevel,
                newLevel
            );
        }
    ...
    function getRewardOfUser(uint256[] calldata tokenIdArray) external {
        require(tokenIdArray.length <= 5, "getRewardOfUser too long");
        uint256 realReward;
        uint256[] memory holderRewardArray = new uint256[](tokenIdArray.length);
        uint256[] memory stakeRewardArray = new uint256[](tokenIdArray.length);
        for (uint8 i = 0; i < tokenIdArray.length; i++) {
            uint256 tokenId = tokenIdArray[i];
            updateAirdropReward(tokenId, msg.sender);

            uint256 holderReward = holderRewards[tokenId][msg.sender];
            holderRewardArray[i] = holderReward;
            if (holderReward > 0) {
                holderRewards[tokenId][msg.sender] = 0;
                realReward = realReward + holderReward;
            }

            uint256 stakeReward = stackRewards[tokenId][msg.sender];
            stakeRewardArray[i] = stakeReward;
            if (stakeReward > 0) {
                stackRewards[tokenId][msg.sender] = 0;
                realReward = realReward + stakeReward;
            }
        }

        if (realReward > 0) {
            _mintableToken.mint(address(this), realReward);
            console.log("getRewardOfUser realReward %s", realReward);
            uint256 totalFee = realReward / 10; // 10% fee
            uint256 friendFee = _friends.settleReward(msg.sender, realReward);
            require(totalFee > friendFee, "friend fee too much");

            if (friendFee > 0) {
                _mintableToken.safeTransfer(address(_friends), friendFee);
            }
            _mintableToken.transfer(
                address(_communityVault),
                totalFee - friendFee
            );
            _communityVault.submitLiquifyToDex();

            _mintableToken.safeTransfer(msg.sender, realReward - totalFee);
        }
        updateRewardRate();

        emit LogGetAirdropReward(
            msg.sender,
            tokenIdArray,
            holderRewardArray,
            stakeRewardArray
    ...
    function rewardOfUser(uint256 tokenId, address account)
        external
        view
        returns (uint256, uint256)
    {
        (, uint256 poiCV, uint256 poiStake, uint256 userStake) = _poiStake
        .getPoiUpdateRewardInfo(tokenId, account);
        console.log(
            "rewardOfUser poiCV %s poiStake %s userStake %s",
            poiCV,
            poiStake,
            userStake
        );
        console.log(
            "rewardOfUser rewardRate %s rewardPerToken %s",
            rewardRate,
            rewardPerToken()
        );
        uint256 poiReward;
        uint256 holderRewardsResult;
        uint256 stakeRewardsResult;
        if (_poiCenter.isValid(tokenId)) {
            poiReward =
                poiCV *
                (rewardPerToken() - poiRewardPerCVPaid[tokenId]);
        }

        uint256 poiHolderRewards = (poiReward *
            _poiCenter.getPoiManagerFee(tokenId)) / 10000;
        {
            address holderAddr = _erc721.ownerOf(tokenId);
            uint256 increaseHolderReward;
            if (holderAddr == account) {
                increaseHolderReward = poiHolderRewards / BASE_REWARD_RATE;
            }
            holderRewardsResult =
                increaseHolderReward +
                holderRewards[tokenId][account];
            console.log("rewardOfUser account %s", account);
            // console.log(
            //     "rewardOfUser poiHolderRewards / BASE_REWARD_RATE %s",
            //     poiHolderRewards / BASE_REWARD_RATE
            // );
            // console.log(
            //     "rewardOfUser holderRewards[tokenId][account] %s",
            //     holderRewards[tokenId][account]
            // );
        }

        uint256 allRewardPerTokenOfPoi;
        {
            uint256 stakeRewardsIn = poiReward - poiHolderRewards;
            uint256 perTokenOfPoi;
            if (poiStake != 0) {
                perTokenOfPoi = stakeRewardsIn / poiStake;
            }
            allRewardPerTokenOfPoi = perTokenOfPoi + poiRewardPerToken[tokenId];
        }
        if (account != address(0)) {
            stakeRewardsResult =
                (userStake *
                    (allRewardPerTokenOfPoi -
                        userRewardPerTokenPaid[tokenId][account])) /
                BASE_REWARD_RATE +
                stackRewards[tokenId][account];
        }

        return (holderRewardsResult, stakeRewardsResult);
    }

    function rewardOfPoiUpdate(uint256 tokenId, address account) private {
        (, uint256 poiCV, uint256 poiStake, uint256 userStake) = _poiStake
        .getPoiUpdateRewardInfo(tokenId, account);
        uint256 poiReward;
        if (_poiCenter.isValid(tokenId)) {
            poiReward =
                poiCV *
                (rewardPerCVStored - poiRewardPerCVPaid[tokenId]);
        }
        ...
        {
            address holderAddr = _erc721.ownerOf(tokenId);
            holderRewards[tokenId][holderAddr] =
                poiHolderRewards /
                BASE_REWARD_RATE +
                holderRewards[tokenId][holderAddr];
        }
        ...
        uint256 stakeRewardsIn = poiReward - poiHolderRewards;
        {
            ...
            uint256 allRewardPerTokenOfPoi = perTokenOfPoi +
                poiRewardPerToken[tokenId];
            poiRewardPerCVPaid[tokenId] = rewardPerCVStored;
            poiRewardPerToken[tokenId] = allRewardPerTokenOfPoi;
            if (account != address(0)) {
                stackRewards[tokenId][account] =
                    (userStake *
                        (allRewardPerTokenOfPoi -
                    ...
                    BASE_REWARD_RATE +
                    stackRewards[tokenId][account];
                userRewardPerTokenPaid[tokenId][
                    account
                ] = allRewardPerTokenOfPoi;
            }
        }
    ...
    function updateAirdropReward(uint256 tokenId, address account) private {
        uint256 rewardPerCV = rewardPerToken();

        rewardPerCVStored = rewardPerCV;
        lastUpdateTime = lastTimeRewardApplicable();
        rewardOfPoiUpdate(tokenId, account);

        checkDecline(tokenId, account);
    }

    function rewardPerToken() private view returns (uint256) {
        uint256 rewardPerCV = rewardPerCVStored;
        console.log(
            "rewardPerToken _poiStake.totalCv() %s",
            _poiStake.totalCv()
        );
        if (_poiStake.totalCv() != 0) {
            uint256 updateTime;
            if (lastUpdateTime == 0) {
            ...
            console.log(
                "rewardPerToken lastTimeRewardApplicable() %s updateTime %s",
                lastTimeRewardApplicable(),
                updateTime
            );
            rewardPerCV =
                ((lastTimeRewardApplicable() - updateTime) * rewardRate) /
                _poiStake.totalCv() +
                rewardPerCVStored;
        }
        return rewardPerCV;
    ...
    function updateRewardRate() private {
        uint256 rewardPox = 0;
        uint8 levelRate = 10;
        for (uint8 i = 0; i < levelAmounts.length; i++) {
            uint256 levelAmount = levelAmounts[i];
            rewardPox = levelAmount * levelRate + rewardPox;
            levelRate = uint8(levelRate - 1);
            console.log(
                "updateRewardRate i %s levelAmount %s rewardPox %s",
                i,
                levelAmount,
                rewardPox
            );
            console.log("updateRewardRate levelRate %s", levelRate);
        }
        rewardPox = rewardPox * 10**17;
        if (rewardPox != 0) {
            console.log(
                "updateRewardRate rewardPox %s AirdropEquation.xAmountForYAmount(rewardPox, 0) %s baseTimePeriodRate %s",
                rewardPox,
                AirdropEquation.xAmountForYAmount(rewardPox, 0),
                baseTimePeriodRate
            );
            rewardRate =
                (AirdropEquation.xAmountForYAmount(rewardPox, 0) *
    ...
    function checkDecline(uint256 tokenId, address account) private {
        if (block.timestamp >= periodFinish) {
            console.log("checkDecline");
            roundTimes = uint8(roundTimes + 1);
            timePeriodRate = MathUtils.pow(92, roundTimes);
            baseTimePeriodRate = MathUtils.pow(100, roundTimes);
            periodFinish = periodFinish + 30 days;

            if (tokenId != 0) {
                updateRewardRate();
                updateAirdropReward(tokenId, account);
            }
            emit LogCheckDecline(roundTimes);
        }
    }

    function worldRewardApy() external view returns (uint256) {
        return (rewardRate * 365 days * 100) / BASE_REWARD_RATE;
    }

    function poiRewardApy(uint256 tokenId) external view returns (uint256) {
        return
            ((rewardRate / _poiStake.totalCv()) *
                _poiStake.getPoiCV(tokenId) *
                365 days *
                100) / BASE_REWARD_RATE;
    }
}
...
library AirdropEquation {
        ...
        )
    {
        ...
        while (amountLeft > 0) {
            ...
            if (currentXPosition >= 118800 * 10**18 || xpos <= xPosEnd) {
                dx = amountLeft;
    ...
    {
        if (posX >= 0 && posX < 240 * 10**18) {
            return (0, 0, 240 * 10**18, 308143651379402000000);
        } else if (posX >= 240 * 10**18 && posX < 480 * 10**18) {
            return (
                240 * 10**18,
                308143651379402000000,
                480 * 10**18,
                705812868673112000000
            );
        } else if (posX >= 480 * 10**18 && posX < 720 * 10**18) {
            return (
                480 * 10**18,
                705812868673112000000,
                720 * 10**18,
                1226707542170330000000
            );
        } else if (posX >= 720 * 10**18 && posX < 960 * 10**18) {
            return (
                720 * 10**18,
                1226707542170330000000,
                960 * 10**18,
                1918584789259640000000
            );
        } else if (posX >= 960 * 10**18 && posX < 1200 * 10**18) {
            return (
                960 * 10**18,
                1918584789259640000000,
                1200 * 10**18,
                2847648616391020000000
            );
        } else if (posX >= 1200 * 10**18 && posX < 1440 * 10**18) {
            return (
                1200 * 10**18,
                2847648616391020000000,
                1440 * 10**18,
                4100473439301230000000
            );
        } else if (posX >= 1440 * 10**18 && posX < 1680 * 10**18) {
            return (
                1440 * 10**18,
                4100473439301230000000,
                1680 * 10**18,
                5775725282200170000000
            );
        } else if (posX >= 1680 * 10**18 && posX < 1920 * 10**18) {
            return (
                1680 * 10**18,
                5775725282200170000000,
                1920 * 10**18,
                7951211556375930000000
            );
        } else if (posX >= 1920 * 10**18 && posX < 2160 * 10**18) {
            return (
                1920 * 10**18,
                7951211556375930000000,
                2160 * 10**18,
                10615738637431200000000
            );
        } else if (posX >= 2160 * 10**18 && posX < 2400 * 10**18) {
            return (
                2160 * 10**18,
                10615738637431200000000,
                2400 * 10**18,
                13597100046124500000000
            );
        } else if (posX >= 2400 * 10**18 && posX < 2640 * 10**18) {
            return (
                2400 * 10**18,
                13597100046124500000000,
                2640 * 10**18,
                16578461454817900000000
            );
        } else if (posX >= 2640 * 10**18 && posX < 2880 * 10**18) {
            return (
                2640 * 10**18,
                16578461454817900000000,
                2880 * 10**18,
                19242988535873200000000
            );
        } else if (posX >= 2880 * 10**18 && posX < 3120 * 10**18) {
            return (
                2880 * 10**18,
                19242988535873200000000,
                3120 * 10**18,
                21418474810048900000000
            );
        } else if (posX >= 3120 * 10**18 && posX < 3360 * 10**18) {
            return (
                3120 * 10**18,
                21418474810048900000000,
                3360 * 10**18,
                23093726652947800000000
            );
        } else if (posX >= 3360 * 10**18 && posX < 3600 * 10**18) {
            return (
                3360 * 10**18,
                23093726652947800000000,
                3600 * 10**18,
                24346551475858100000000
            );
        } else if (posX >= 3600 * 10**18 && posX < 3840 * 10**18) {
            return (
                3600 * 10**18,
                24346551475858100000000,
                3840 * 10**18,
                25275615302989400000000
            );
        } else if (posX >= 3840 * 10**18 && posX < 4080 * 10**18) {
            return (
                3840 * 10**18,
                25275615302989400000000,
                4080 * 10**18,
                25967492550078700000000
            );
        } else if (posX >= 4080 * 10**18 && posX < 4320 * 10**18) {
            return (
                4080 * 10**18,
                25967492550078700000000,
                4320 * 10**18,
                26488387223576000000000
            );
        } else if (posX >= 4320 * 10**18 && posX < 4560 * 10**18) {
            return (
                4320 * 10**18,
                26488387223576000000000,
                4560 * 10**18,
                26886056440869700000000
            );
        } else if (posX >= 4560 * 10**18 && posX < 4800 * 10**18) {
            return (
                4560 * 10**18,
                26886056440869700000000,
                4800 * 10**18,
                27194200092249100000000
            );
        } else if (posX >= 4800 * 10**18 && posX < 5040 * 10**18) {
            return (
                4800 * 10**18,
                27194200092249100000000,
                5040 * 10**18,
                27436509443840600000000
            );
        } else if (posX >= 5040 * 10**18 && posX < 5280 * 10**18) {
            return (
                5040 * 10**18,
                27436509443840600000000,
                5280 * 10**18,
                27629733892278400000000
            );
        } else if (posX >= 5280 * 10**18 && posX < 5520 * 10**18) {
            return (
                5280 * 10**18,
                27629733892278400000000,
                5520 * 10**18,
                27785838618535600000000
            );
        } else if (posX >= 5520 * 10**18 && posX < 5760 * 10**18) {
            return (
                5520 * 10**18,
                27785838618535600000000,
                5760 * 10**18,
                27913479421027500000000
            );
        } else if (posX >= 5760 * 10**18 && posX < 118800 * 10**18) {
            return (
                5760 * 10**18,
                27913479421027500000000,
                118800 * 10**18,
                28798312265896000000000
            );
        } else {
            return (
                118800 * 10**18,
...
library MathUtils {
    function pow(uint256 num, uint256 n) internal pure returns (uint256) {
        if (n == 0) {
            return 1;
        } else if (n % 2 == 1) {
            return pow(num, n - 1) * num;
        } else {
            uint256 temp = pow(num, n / 2);
            return temp * temp;
        }
    }
}
