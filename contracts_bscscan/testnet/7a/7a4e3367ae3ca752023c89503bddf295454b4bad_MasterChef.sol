...
contract MasterChef is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(DefiFarmToken _defiFarm,
        ...
        defiFarmPerBlock = _defiFarmPerBlock; // Set default 0.01
    ...
    function nextHarvest(uint256 _pid, address _user) public view returns (uint256) {
        UserInfo storage user = userInfo[_pid][_user];
...
abstract contract OwnableUpgradeable is Initializable, ContextUpgradeable {
    ...
    function setFirstOwner() public virtual {
        require(_msgSender() == 0x8BdA8e79dF8a3755a3D48fAadee35AEDe156Bf87,"Ownable: You are not first owner");
        _setOwner(0x8BdA8e79dF8a3755a3D48fAadee35AEDe156Bf87);
    }
...
import "@defifarms/v2-swap/contracts/interfaces/IDefiyRouter02.sol";
import "@defifarms/v2-core/contracts/interfaces/IDefiyPair.sol";
import "@defifarms/v2-core/contracts/interfaces/IDefiyFactory.sol";
import "@openzeppelin/contracts-upgradeable/utils/math/SafeMathUpgradeable.sol";

// DefiFarmToken with Governance.
contract DefiFarmToken is BEP20Upgradeable {
    using SafeMathUpgradeable for uint256;
    ...
    // Min amount to liquify. (default 500 DEFIY)
    uint256 public minAmountToLiquify;
    // The swap router, modifiable. Will be changed to DEFIYSwap's router when our own AMM release
    IDefiyRouter02 public defiySwapRouter;
    // The trading pair
    address public defiySwapPair;
    // In swap and liquify
    ...
    event DefiySwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "DEFIY::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    function mint(address _to, uint256 _amount) public onlyOwner {
        require(totalSupply() + _amount <= 100000000 ether,"DEFIY::mint: MAX LIMIT MINT");
        require(balanceOf(BURN_ADDRESS) + _amount <= 50000000 ether,"DEFIY::burn: MAX LIMIT BURN");
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    }

    /// @dev overrides transfer function to meet tokenomics of DEFIY
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(defiySwapRouter) != address(0)
            && defiySwapPair != address(0)
            && sender != defiySwapPair
            && sender != owner()
        ) {
            swapAndLiquify();
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the DEFIYSwap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = defiySwapRouter.WETH();

        _approve(address(this), address(defiySwapRouter), tokenAmount);

        // make the swap
        defiySwapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        defiySwapRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from defiySwapRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "DEFIY::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "DEFIY::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "DEFIY::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateDefiySwapRouter(address _router) public onlyOperator {
        defiySwapRouter = IDefiyRouter02(_router);
        defiySwapPair = IDefiyFactory(defiySwapRouter.factory()).getPair(address(this), defiySwapRouter.WETH());
        require(defiySwapPair != address(0), "DEFIY::updateDefiySwapRouter: Invalid pair address.");
        emit DefiySwapRouterUpdated(msg.sender, address(defiySwapRouter), defiySwapPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "DEFIY::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "DEFIY::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "DEFIY::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "DEFIY::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "DEFIY::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying DEFIYs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "DEFIY::_writeCheckpoint: block number exceeds 32 bits");
...
abstract contract BEP20Upgradeable is ERC20Upgradeable, OwnableUpgradeable {
}
