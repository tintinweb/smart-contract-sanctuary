...
contract HoePool is Ownable {
    ...
    function lockValue() public view returns(uint256) {
        uint256 totalLock = 0;
        (address factory, address usdtToken) = manager.getFactory();
        for(uint256 i = 0; i < poolInfo.length; i++) {
            address lpToken = address(poolInfo[i].lpToken);
            uint256 amount = poolInfo[i].totalAmount;
            bool isPair = poolInfo[i].isPair;
            uint256 swapAmount = PriceLibrary.getPrice(factory, lpToken, address(usdtToken), amount, isPair);
            totalLock = totalLock.add(swapAmount);
        }
        return totalLock;
    }
}