...
contract MetaSoldierToken is TemplateERC20Token, Ownable {
    ...
    // Main
    //  IPancakeRouter02 public pancakeRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
...
function transferFrom(
    ...
    **/ 
    function addLiquidity(
        ...
        (amountA,amountB,liquidity) = pancakeRouter.addLiquidity(address(this),USDTAddress,amountADesired,amountBDesired,amountAMin,amountBMin,to,deadline);
         ...
         (amountA,amountB) = _getPairTokenSort(amountA,amountB);
    ...
    function _getPairTokenSort(uint256 token0,uint256 token1) private view  returns(uint256,uint256){
        IPancakePair pair = IPancakePair(swapPairAddress);
        address token0Address = pair.token0();
        if(token0Address != address(this)){
            uint256 tmep = token0;
            token0 = token1;
            token1 = tmep;
        }
    ...
    **/
    function removeLiquidity(uint liquidity,
      ...
      return _getPairTokenSort(amountA,amountB);
    }
