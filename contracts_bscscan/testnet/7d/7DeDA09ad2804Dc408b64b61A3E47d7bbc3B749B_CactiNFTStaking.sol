...
contract CactiNFTStaking is IERC721Receiver, ReentrancyGuard {
    ...
    constructor(
        ...
        stakingPeriods.push(StakingPeriod({ startBlock: block.number, endBlock: (block.number + 50) }));
        emit StakePeriod(block.number, (block.number + 50), activeStakingPeriodIndex);
    }
    ...
    function unStakeNFT(uint256 tokenId) external nonReentrant returns (bool) {
    ...
    function _unStakeNFT(uint256 tokenId) internal onlyStaker(tokenId) requireTimeElapsed(tokenId) returns (bool) {
        // require current block must be higher or equal than the farmingEndBlock
        require(block.number >= stakingPeriods[activeStakingPeriodIndex].endBlock, 'Unstake: Farming end block has not been passed yet');
        ...
        // delete user[msg.sender].tokens[tokenId];
    ...
    function harvest(uint256 tokenId) external nonReentrant onlyStaker(tokenId) requireTimeElapsed(tokenId) {
        // require current block must be higher or equal than the farmingEndBlock
        require(block.number >= stakingPeriods[activeStakingPeriodIndex].endBlock, 'Harvest: Farming end block has not been passed yet');
        // This 'payout first' should be safe as the function is nonReentrant
        _payoutStake(tokenId);

        // update receipt with a new block number
        receipt[tokenId].stakedFromBlock = stakingPeriods[activeStakingPeriodIndex].endBlock;
    }
    ...
    function _payoutStake(uint256 tokenId) internal {
        ...
        // If the getTimeStaked is <= 0 than sub(1) won't work because of safeMath. So that also means
        // he has no time staked so no rewards. Emit an event.
        if(_getTimeStaked(tokenId) <= 0) {
    ...
    function emergencyWithdrawToUser(address _user, uint256 tokenId) external nonReentrant onlyDao returns (bool) {
        delete receipt[tokenId];
    ...
    function getCurrentStakeEarned(uint256 tokenId) external view returns (uint256) {
    ...
    function setStakingPeriod(uint256 _startBlock, uint256 _endBlock, bool _activateNewIndex, uint256 _index) external onlyDao {
        ...
        if(!_activateNewIndex && _index == activeStakingPeriodIndex) {
