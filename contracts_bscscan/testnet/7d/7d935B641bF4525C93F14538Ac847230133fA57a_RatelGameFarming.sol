...
contract RatelGameFarming is ReentrancyGuard, Ownable, Pausable {
    ...
    event OnSetMaxLimitAmount(uint256 maxLimitAmount);
    event OnApprove(address receiver, uint256 amount);
    event OnWithdraw(address receiver, uint256 amount);
    event OnRevoked(address owner, uint256 balance);
    event OnNotEnoughWarn(uint256 balance); //服务端监控，如此出现则需要部署者铸造新的代币
    
    mapping(address => bool) private operators;   //操作员数组

    address operator_address; //接收手续费的操作员,中心化服务端需用这个操作员私钥签名

    //游戏代币合约地址
    address honeyRatelGameToken_address = payable(0x72F20393273875BF4Ba237dF9eb91C6e1bcd1bbA);
    
    // Ratel NFT 合约地址 
    address ratelNFT_Address;
    RatelNFT ratelNFT;

    uint256 private WITHDRAW_DURATION = 3600; //1209600; //提币间隔 14天, 测试的时候改为1个小时
    uint256 private fee = 5 * 10**15; //提现手续费, 初步0.005BNB

    struct EarnedData {
        uint256 lastWithdrawTime; //最后一次提币时间戳       
        uint256 amount;     // 可提的游戏币数量     
        bool isValid;        //是否有效              
    }
    mapping(address => EarnedData) private approveEarnedDatas;   

    constructor(address _ratelNFT_Address) {
        ratelNFT_Address = _ratelNFT_Address;
        ratelNFT = RatelNFT(payable(ratelNFT_Address));
    ...
    // 合约部署者设置手续费接收的操作员
    function setReceiveOperator(address _operator) external nonReentrant onlyOwner returns(bool){
    ...
    // 只允许部署者操作, paused() - contract pause status
    ...
    */
    ...
    // 服务端操作员操作允许用户提币
    function approve(address receiver, uint256 amount) external nonReentrant isOperator returns(bool){
        require(!paused(), "contract is paused");   
        require(amount>0, "amount must be more than 0");
        //调用NFT合约，判断receiver是否拥有NFT
        require(ratelNFT.balanceOf(receiver)>0, "Not valid address");

        if (approveEarnedDatas[receiver].isValid)  {
            approveEarnedDatas[receiver].amount = approveEarnedDatas[receiver].amount.add(amount);
        } else {
            EarnedData memory earnedData;

            earnedData.lastWithdrawTime = 0;
            earnedData.amount = amount;
            earnedData.isValid = true;
            approveEarnedDatas[receiver] = earnedData;

        }
        emit OnApprove(receiver, amount);
        return true;
    }

    //提币,需要玩家支付手续费, 有防重入攻击
    function withdrawGameToken(uint256 amount) 
        external 
        payable
        nonReentrant 
        returns(bool){
        ...
        require(amount>0, "Amount must be more than 0");
        require(msg.value >= fee, "Payment error");
        require(ratelNFT.balanceOf(msg.sender)>0, "Not valid address");

        uint256 balance = IERC20(honeyRatelGameToken_address).balanceOf(address(this));

        if (balance < amount) {
            emit OnNotEnoughWarn(balance);
            return false;
        }

        //将手续费转到中心化服务端操作员，用于日常gas费的支付
        payable(operator_address).transfer(msg.value);
    
        if (approveEarnedDatas[msg.sender].isValid) {
            require(amount <= approveEarnedDatas[msg.sender].amount, "Not enough balance");
            require(block.timestamp.sub(approveEarnedDatas[msg.sender].lastWithdrawTime) > WITHDRAW_DURATION, "Withdraw period should be over 10d");
            
            approveEarnedDatas[msg.sender].lastWithdrawTime = block.timestamp;
            approveEarnedDatas[msg.sender].amount = approveEarnedDatas[msg.sender].amount.sub(amount);

            IERC20(honeyRatelGameToken_address).safeTransfer(msg.sender, amount);
    ...
    //获取可提的游戏币数量
    function getApproveAmount() public view returns(uint256) {
       if (ratelNFT.balanceOf(msg.sender)==0) return 0;
       return approveEarnedDatas[msg.sender].amount;
    }
