...
contract AIDO is ReentrancyGuard, OwnableUpgradeable {
    ...
    // total amount of raising need to be raised
    ...
    function initialize(
        ...
        receivingCap = 80 ether;
        status = 0;
        totalRaiseAmount = 0;
        minBuyAmount = 1 ether; // 1 BNB
        maxBuyAmount = 2 ether;
    ...
    modifier onlyWhitelist() {
        require(whitelist[msg.sender] || msg.sender == owner(), "!admin");
        _;
    }

    function setMultiWhitelist(address[] memory wls, bool _isWhitelist)
        external
        onlyOwner
    {
        uint256 length = wls.length;
        require(length >= 1, "zero token");
        for (uint256 i = 0; i < length; i++) {
            whitelist[wls[i]] = _isWhitelist;
        }
    }
    ...
    function setSellingToken(address _sellingToken) external onlyOwner {
        sellingToken = _sellingToken;
    }
    ...
    modifier onlyFinished() {
        require(
            status == 3 && block.timestamp >= endTime,
            "ido is not finished"
        );
        _;
    }
    ...
    function setReceivingCap(uint256 _receivingCap) public onlyOwner {
        rate = _receivingCap;
    }
    ...
    function buy() external payable onlyActive onlyWhitelist nonReentrant {
        require(userInfo[msg.sender].amount <= 0, "you can't join IDO twice!");
        require(msg.value >= minBuyAmount, "value must >= min");
        require(msg.value <= maxBuyAmount, "value must <= max");
        require(totalRaiseAmount + msg.value <= receivingCap, "overcap");

        address payable wallet = payable(admin);
        wallet.transfer(msg.value);
        if (userInfo[msg.sender].amount == 0) {
            addressList.push(msg.sender);
        }
        userInfo[msg.sender].amount += msg.value;
        totalRaiseAmount += msg.value;
    ...
    function _tokenTransfer(address _account, uint256 _amount) internal {
        uint256 _releaseAmount = _amount;
    ...
    function getUserSellingTokenAmount(address user)
        public
        view
        returns (uint256)
    {
        // Return an offering amount equal to a proportion of the raising amount
        return userInfo[user].amount * rate;
    }
    ...
    function userTokenStatus(address _user)
        public
        view
        returns (uint256 receivingTokenAmount, uint256 sellingTokenHarvest)
    {
