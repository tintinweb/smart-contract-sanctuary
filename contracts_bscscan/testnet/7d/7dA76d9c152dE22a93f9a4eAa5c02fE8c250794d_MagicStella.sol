...
contract MagicStella is BEP20, Ownable {
	...
	bool public autoDistributionOfContractsTokens = true;
	...
	event updateStellaRouterEvent(address indexed newAddress, address indexed oldAddress);
	...
	function _removeRestrictionForEcoSystemWallets(IUniswapV2Router02 _stellaRouter) private {
		...
		excludeWalletFromFees(address(buyBackWallet), true);
		...
		isAllowedDuringDisabled[teamWallet] = true;
		isAllowedDuringDisabled[liquidityWallet] = true;
	}
	...
	function activeTrading(bool tradingStatus) external authorized {
	...
	function excludeMultipleWalletsFromFees(address[] calldata accounts, bool excluded) external authorized {
	...
	function allowWalletDuringContractDisabled(address wallet, bool status) external authorized {
	...
	function excludeWalletFromDividends(address account) external authorized {
	...
	function setIsMaxTokenByWalletExempt(address holder, bool exempt) external authorized {
	...
	function buyBackTokensWithNoFees() external payable returns (bool) {
		...
		stellaRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{ value: bnbAmount }( //try to swap for tokens, if it fails (bad contract, or whatever other reason, send BNB)
	...
	function _transfer(
		...
		if (from == address(stellaSwapV2Pair) || to == address(stellaSwapV2Pair)) {
			takeFee = true;
		}
