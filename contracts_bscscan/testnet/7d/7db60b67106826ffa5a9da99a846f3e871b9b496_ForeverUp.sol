...
contract BEP20 is Context, IBEP20, Ownable {
     ...
     */
     ...
     event DebugTr(string message,address n);
    function _transfer(
        address sender,
        address recipient,
        uint256 amount
...
contract ForeverUp is BEP20 {
    // Transfer tax rate in basis points.
    uint256 public transferTaxRate = 990;
    // Max transfer tax rate: 30%.
    uint256 public constant MAXIMUM_TRANSFER_TAX_RATE = 3000;
    // Diff between sell and buy tax // Should be divided by 2 for the real number
    uint16 public sellMultiplier = 6;
    ...
    // AntiPaperHands %
    uint public antiPaperHandsPct;
    // When was the last pump?
    uint public lastPump;
    // Locktime
    uint256 public _lockTimeOperator;
    // Lock var
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override 
        ...
        if (recipient == address(0)
            ...
            if (recipient==pair) {
                taxAmount = amount.mul(transferTaxRate).div(20000).mul(sellMultiplier);
            }
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        ...
        require(_maxTransferAmountRate >= 10, "Upforever::updateMaxTransferAmountRate: Max transfer amount rate must not be lower thant the minimum rate.");
    ...
    function updateBuyback(address payable _buyback) external onlyOwner {
        buyback = _buyback;
    }

    function updateSellMultiplier(uint16 _mul) external onlyOperator {
    require(transferTaxRate.div(20000).mul(sellMultiplier) <= 5000);
    sellMultiplier = _mul;
    }
    ...
    function GetStartTimeTrading(uint _multiplier) public view returns (uint256) {
    ...
    function lockOperator(uint256 time) public onlyOperator {
    _previousOperator = _operator;
    _operator = address(0);
    _lockTimeOperator = now + time;
    }

    function getOperatorUnlockTimeDays() public view returns (uint256) {
        if (_operator == address(0)) {
            if (_lockTimeOperator > now) {
                return (_lockTimeOperator.sub(now)).div(86400);
            }
            else {
                return 0;
            }
    ...
    function unlockOperator() public {
        require(_previousOperator == msg.sender, "You don't have permission to unlock");
        require(now > _lockTimeOperator , "Contract is locked");
    ...
    function YouGotToPumpItUp() external {
