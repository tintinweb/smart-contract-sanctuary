...
import "./TrainingCenter.sol";

contract TrainingManagement is TrainingCenter {
    ...
    modifier onlyCenterOwner(uint256 _trainingId){
        require(ownerOf(_trainingId) == msg.sender,"!Yours");
        _;
    }

    modifier monstOwner(uint256 _monsterId){
        require(IERC721(gameAddresses.getMonsterAddress()).ownerOf(_monsterId) == msg.sender,"!Yours");
        _;
    }

    // 0 = not params, 1= closed, 2 = Free, 3 = in use
    function getSpotStatus(uint256 _spotId,uint256 _trainingId) external view returns(uint256){
        return _getSpotStatus(_spotId,_trainingId);
    }
    ...
    function _getSpotStatus(uint256 _spotId,uint256 _trainingId) internal view returns(uint256 _state){
        ...
        if(t.price == 0){
            ...
            return 1;
        } else if(t.monsterId == 0 || block.timestamp >= t.endAt){
            return 2;
        } else {
            return 3;
        }
        ...
        onlyCenterOwner(_trainingId)
    {
        ...
        require(BZAI.transferFrom(msg.sender, _payment, _spotsQty.mul(addSpotPrice)));
        IPayments(_payment).distributeFees(_spotsQty.mul(addSpotPrice));
    ...
    function registerCoaching(uint256 _monsterId, uint256 _spotId, uint256 _trainingId) external monstOwner(_monsterId){
        TrainingInstance storage t = trainingSpots[_trainingId][_spotId];
        IBandZaiNFT I = IBandZaiNFT(gameAddresses.getMonsterAddress());
        require(t.coach.minLevelReq != 0,"!Exist");
        require(t.coach.coachId == 0, "Got coach");
        (uint256 _level, , , , , ) = I.getMonster(_monsterId);
        require(_level >= t.coach.minLevelReq, "!Level");
        require(I.isFree(_monsterId), "Monster!=Free");
        I.updateStatus(_monsterId, 2, _trainingId);
        t.coach.coachId = _monsterId;
        t.spotOpened = true;
    }

    function quiteCoaching(uint256 _monsterId, uint256 _spotId, uint256 _trainingId) external monstOwner(_monsterId){
        TrainingInstance storage t = trainingSpots[_trainingId][_spotId];
        IBandZaiNFT I = IBandZaiNFT(gameAddresses.getMonsterAddress());
        ...
        require(_level >= t.coach.minLevelReq, "!Level");
        t.coach.coachId = 0;
        I.updateStatus(_monsterId, 0, _trainingId);
        t.spotOpened = false;
    }
    ...
    monstOwner(_monsterId)
    {
        ...
        require(I.isFree(_monsterId), "M!=Free");
        I.updateStatus(_monsterId,1,_trainingId);
        require(BZAI.transferFrom(msg.sender, _paymentAddress, t.price));

        address ownerOfTC = ownerOf(
            _trainingId
        );
        ...
        if(t.coach.coachId != 0){
            uint256 coachPayment = ownerPayment.mul(t.coach.percentPayment).div(100);
            ownerPayment = ownerPayment.sub(coachPayment);
            address ownerOfCoach = IERC721(_monsterAddress).ownerOf(t.coach.coachId);
            IPayments(_paymentAddress).payOwner(ownerOfCoach, coachPayment);
        }

        IPayments(_paymentAddress).payOwner(ownerOfTC, ownerPayment);//9
        tcRevenues[_trainingId] = tcRevenues[
            _trainingId
        ]
        .add(t.price);
        ...
        t.endAt = block.timestamp.add(t.duration);
        t.monsterId = _monsterId;
    }

    function finishTraining(uint256 _spotId, uint256 _trainingId, uint256 _monsterId) external monstOwner(_monsterId){
        TrainingInstance storage t = trainingSpots[_trainingId][_spotId];
        if (block.timestamp >= t.endAt) {
            _updateMonster(t.monsterId, t.duration,t.coach.coachId);
        }

        IBandZaiNFT(gameAddresses.getMonsterAddress()).updateStatus(t.monsterId,0,0);
    ...
    function _updateMonster(uint256 _monsterId, uint256 _duration, uint256 _coachId)
        private
    {
        IBandZaiNFT I = IBandZaiNFT(gameAddresses.getMonsterAddress());
        (uint256 monsterLevel, , , , , ) = I.getMonster(_monsterId);

        if(_coachId != 0){
            (uint256 coachLevel, , , , , ) = I.getMonster(_coachId);
            if(coachLevel.sub(monsterLevel) > 0){
                _duration = _duration.mul(coachLevel.sub(monsterLevel).add(1));
            }
        }
        ...
        I.updateXp(_monsterId,_duration.mul(multiplierXp));
    }
}
...
contract TrainingCenter is ERC721, Ownable {
    ...
    modifier onlyAuth() {
        require(gameAddresses.getOpenAndCloseAddress() == msg.sender, "!Auth");
        _;
    }
    ...
    { 
        ...
        numberOfTrainingSpots[_newItemId] = 3;
