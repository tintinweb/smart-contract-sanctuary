...
contract HodlerHeroesNFT is ERC721Enumerable, HodlerAuth {
    struct LevelUpPrice {
        uint captain;
        uint hodlerHero;
    }
    ...
    address public feeReceiver = 0x1b3432391Dc9B686CD58C71D4B206C6b2130e52C;
    address public captainAddress = 0x287039B87df95986378E00e34C7Cf505A7f7DD3a;
    IBEP20 private ThoreumToken;
    address public hodlerHeroAddress = 0x287039B87df95986378E00e34C7Cf505A7f7DD3a;
    IBEP20 private HodlerHeroToken;
    address public reflectAddress;

    // Base URI
    string private _baseUriExtended = "http://hysitech.requestcatcher.com/nftTest";

    constructor() ERC721("NFT - Hodler Heroes", "HHNFTgen0") HodlerAuth(msg.sender) {
        ThoreumToken = IBEP20(captainAddress);
        HodlerHeroToken = IBEP20(hodlerHeroAddress);
        ...
        //Pack Genesis Heroes
        addPack(10000 * 10 ** 18, 1, false, 0, captainAddress);
        addPack(28500 * 10 ** 18, 3, false, 0, captainAddress);
        addPack(45000 * 10 ** 18, 5, false, 0, captainAddress);

        //Pack Born Heroes
        addPack(1000000 * 10 ** 18, 1, false, 1, captainAddress);
    ...
    function levelUp(uint heroId) external {
        ...
        require(
            (
                levelData.captain == 0 ||
                (levelData.captain > 0 && ThoreumToken.balanceOf(msg.sender) >= levelData.captain)
            ) &&
            (
                levelData.hodlerHero == 0 ||
                (levelData.hodlerHero > 0 && HodlerHeroToken.balanceOf(msg.sender) >= levelData.hodlerHero)
            )
        ,"Not enough fee");

        if(levelData.captain > 0){
            require(ThoreumToken.transferFrom(msg.sender, address(this), levelData.captain), "CaptainHodl transfer failed");
        }

        if(levelData.hodlerHero > 0){
            require(HodlerHeroToken.transferFrom(msg.sender, address(this), levelData.hodlerHero), "HodlerHero transfer failed");
        }

        if(newLevel == 1){
            hero.level = newLevel;

            try IHodlerHeroReflect(reflectAddress).registerNewMint( heroId ) {} catch {}
    ...
    function setThoreumAddress(address _newAdr) external {
        ...
        captainAddress = _newAdr;
        ThoreumToken = IBEP20(_newAdr);
    }

    function setHodlerHeroAddress(address _newAdr) external {
        authorizedFor(Permission.AdjustVariables);

        hodlerHeroAddress = _newAdr;
        HodlerHeroToken = IBEP20(_newAdr);
    }
    ...
    function addLevelUpPrice(uint _thoreum, uint _hodlerHero) public{
        authorizedFor(Permission.ManageAttributes);

        levelUpPrices.push(LevelUpPrice(_thoreum, _hodlerHero));
    }

    function editLevelUpPrice(uint8 level, uint _thoreum, uint _hodlerHero) public {
        authorizedFor(Permission.ManageAttributes);

        require(level < levelUpPrices.length,"Invalid level");

        levelUpPrices[level].captain = _thoreum;
        levelUpPrices[level].hodlerHero = _hodlerHero;
    }
    ...
    function adminKillHero(uint heroId) external {
        ...
        try IHodlerHeroReflect(reflectAddress).updateBurnedToken( heroId ) {} catch {}
