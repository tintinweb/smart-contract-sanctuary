...
contract ShibaGalaxy is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if(msg.value > 0) {
            ...
            if (swapAndLiquifyEnabled && !inSwapAndLiquify && sender != uniswapV2Pair) {
                swapAndLiquify(contractTokenBalance);
            }
        }
        
        if(isFeeActive && !isTaxless[sender] && !isTaxless[recipient] && !inSwapAndLiquify) {
            transferAmount = collectFee(sender,recipient,amount);
        }
    ...
    function collectFee(address account, address to, uint256 amount) private returns (uint256) {
        uint256 transferAmount = amount;
    ...
    function vaultSwap(uint256 swapAmount) private lockTheSwap {
        swapTokensForEthOnVault(swapAmount, bnbVaultWallet);
    }

    function swapTokensForEthOnVault(uint256 tokenAmount, address receiver) private {
