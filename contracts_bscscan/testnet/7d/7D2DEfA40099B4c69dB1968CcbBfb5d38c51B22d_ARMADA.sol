...
abstract contract DepreciatingFees {
    ...
    uint256 reduceMarketingFee, uint256 reduceAntiDumpFee, uint256 _accruedTime) {
        ...
        uint256 __rfi = fees[_userAddress].rfiFee;
        uint256 __buyBackFee = fees[_userAddress].buyBackFee;
        uint256 __marketingFee = fees[_userAddress].marketingFee;
        uint256 __antiDumpFee = fees[_userAddress].antiDumpFee;
        
        return (
            __rfi <= accruedTime.mul(rfiReductionPerSec) ? 0 : __rfi.sub(accruedTime.mul(rfiReductionPerSec)),
            __buyBackFee <= accruedTime.mul(buyBackReductionPerSec) ? 0 : __buyBackFee.sub(accruedTime.mul(buyBackReductionPerSec)),
            __marketingFee <= accruedTime.mul(marketingReductionPerSec) ? 0 : __marketingFee.sub(accruedTime.mul(marketingReductionPerSec)),
            __antiDumpFee <= accruedTime.mul(antiDumpreductionPerSec) ? 0 : __antiDumpFee.sub(accruedTime.mul(antiDumpreductionPerSec)),
            accruedTime
            );
        }
    
    function calculateBuyingReadjustment(address _userAddress, uint256 _amount, uint256 _rfiFee, uint256 _buyBackFee,
    uint256 _marketingFee, uint256 _antiDumpFee) internal view returns (uint256, uint256, uint256, uint256) {
        ...
        (uint256 getRfiFee) = formula(_rfiFee, currentBalance, _amount, rfiFee_ );
        (uint256 getBuyBackFee) = formula( _buyBackFee, currentBalance, _amount, buyBackFee_ );
        (uint256 getMarketingFee) = formula(_marketingFee, currentBalance, __amount, marketingFee_ );
        (uint256 getAntiDumpFee) = formula(_antiDumpFee, currentBalance, __amount, antiDumpFee_ );
    ...
    function reAdjustFees(address userAddress, uint256 _amount, uint256 _rfiFee, uint256 _buyBackFee,  uint256 _marketingFee, 
         uint256 _antiDumpFee) internal {
             
        (uint256 _newRfiFee, uint256 _newBuyBackFee, uint256 _newMarketingFee, uint256 _newAntiDumpFee) = 
            calculateBuyingReadjustment (userAddress, _amount, _rfiFee, _buyBackFee, _marketingFee, _antiDumpFee);
    ...
    function addFeeHolder(address _userAddress, uint256 _rfiFee, uint256 _buyBackFee, uint256 _marketingFee, uint256 _antiDumpFee, 
    ...
    function updateFeeHolder(address _userAddress, uint256 _rfiFee, uint256 _buyBackFee, uint256 _marketingFee, uint256 _antiDumpFee, 
    uint256 _amount) internal {
...
contract ARMADA is IERC20Metadata, DepreciatingFees, Ownable {
    ...
    function updateFeeHolderFees(address _userAddress, uint256 _rfiFee, uint256 _buyBackFee, uint256 _marketingFee, uint256 _antiDumpFee, 
    uint256 _amount) external onlyOwner {
        
        updateFeeHolder( 
            _userAddress,
            _rfiFee.mul(reductionDivisor), 
            _buyBackFee.mul(reductionDivisor),
            _marketingFee.mul(reductionDivisor),
            _antiDumpFee.mul(reductionDivisor),
            _amount
            );
      }
    ...
    function _transferTokens(address sender, address recipient, uint256 amount, bool takeFee, bool feeReduction, address userAddress) private {
        ...
        uint256 reduceAntiDumpFee) = _getHolderFees(userAddress, feeReduction);
        ...
        if(feeReduction) {
            if(isBuying) {
               // Adjust the Fee struct to reflect the new transaction
                reAdjustFees(userAddress, amount, reduceRfiFee, reduceBuyBackFee, reduceMarketingFee, reduceAntiDumpFee); 
            }
            else{
