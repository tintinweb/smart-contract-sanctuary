    ...
    EIP712Upgradeable
{
    ...
    function claim(
        ...
        emit ClaimMint(account, tokenAmount,startBlock, endBlock);
        uint256 referAmount = tokenAmount.mul(shareBase).div(referShare);
        if (refer == _msgSender() || refer == address(0)) {
            refer = funder;
        }
        _mint(refer, referAmount);
        emit ReferMint(refer,account, referAmount);
        uint256 funderAmount = tokenAmount.mul(shareBase).div(funderShare);
        _mint(funder, funderAmount);
        emit FunderMint(funder,account, funderAmount);
    }
    ...
    function setShare(uint256 newReferShare, uint256 newFunderShare) external override _onlyAdmin {
...
interface IWeb3GasV2 {
    ...
    function setShare(uint256 newReferShare, uint256 newFunderShare) external;
