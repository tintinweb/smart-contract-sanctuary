...
contract Game is SafeMath, ErrorReporter {
    ...
    function calculateCardIndexProfit(uint256 userCardIndex) public view returns (uint256) {
        uint256 profit = 0;
        uint256 j = 0;
    ...
    function calculateCardProfit(address hold, uint256 cardNum) public view returns (uint256) {
        ...
        uint256 userCardIndex = userCard[hold][cardNum];
        return calculateCardIndexProfit(userCardIndex);
    }


    function calculateMineIndexProfit(uint256 userCardIndex) public view returns (uint256){
        uint256 profit = 0;
        uint256 j = 0;
        uint256 lastBlockPower = 0;
        uint256 lastBlock = 0;
    ...
    function calculateMineProfit(address hold, uint256 cardNum) public view returns (uint256){
        uint256 userCardIndex = userCard[hold][cardNum];
        return calculateMineIndexProfit(userCardIndex);
    }
    ...
    function isMineIndex(uint256 cardIndex) public view returns (bool){
        uint256 endMineBlock = safeAdd(mineBlock[cardIndex], blockStep.mineBlockNum);
        return mineBlock[cardIndex] != 0 && endMineBlock > getBlockNumber();
    }

    function isMine(address user, uint256 cardNum) public view returns (bool){
        uint256 userCardIndex = userCard[user][cardNum];
        return isMineIndex(userCardIndex);

    }

    function getFreezeOf(address user, uint256 cardNum) public view returns (uint256){
        if (isMine(user, cardNum) && freezeOf[user][cardNum] > 0) {
            return freezeOf[user][cardNum];
        }
        return 0;
    }
