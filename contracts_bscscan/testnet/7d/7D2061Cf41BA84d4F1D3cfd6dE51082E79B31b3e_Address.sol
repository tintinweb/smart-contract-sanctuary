...
interface Cashier {
    ...
    function updateRewardsToken(address router, address token) external;
...
contract ReflectionToken is IERC20 {
    ...
    string constant _name = "PAMP BNB";
    ...
    uint256 private _burnFee = 0; // Adjusted by buys and sells.
    uint256 private _charityFee = 0; // Adjusted by buys and sells.
    uint256 private _totalFee = _liquidityFee + _reflectionFee + _marketingFee + _charityFee;
    ...
    uint256 constant public maxMarketingFee = 700;
    ...
    address payable private _marketingWallet = payable(0x8D4BD8dBbA85d0c66Ccb70a2fC8f4488a321277A);
    ...
    // Maximum wallet size is 3% of the total supply.
    ...
    constructor (uint _snipeBlockAmt, address cInitializer, uint256 _gasPriceLimit) payable {
...
//===============================================================================================================
    ...
    function setBuyTaxes(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee) external onlyOwner {
    ...
    function setSellTaxes(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee) external onlyOwner {
        require(liquidityFee <= maxLiquidityFee
                && reflectionFee <= maxReflectionFee
                && marketingFee <= maxMarketingFee
                );
    ...
    function getTokenLength() external view returns (uint256) {
        return reflector.getTokenLength();
    }
    ...
    function updateRewardsToken(address router, address token) external onlyOwner() {
        reflector.updateRewardsToken(router, token);
    }
    ...
    function adjustTaxes(address to) internal {
        if (lpPairs[to]) {
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if(_hasLimits(from, to)) {
            ...
            if (sameBlockActive) {
                if (lpPairs[from]){
                    require(lastTrade[to] != block.number, "Cannot trade more than once in the same block.");
                    lastTrade[to] = block.number;
                } else {
                    require(lastTrade[from] != block.number, "Cannot trade more than once in the same block.");
    ...
    function getTotalFee() public view returns (uint256) {
        return _reflectionFee + _liquidityFee + _marketingFee;
    }
    ...
    function addInitialTokens() external onlyOwner{
        addTokenToList(0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47, 0x10ED43C718714eb63d5aA57B78B54704E256024E); // ADA
        addTokenToList(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c, 0x10ED43C718714eb63d5aA57B78B54704E256024E); // BTC
        addTokenToList(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82, 0x10ED43C718714eb63d5aA57B78B54704E256024E); // CAKE
        addTokenToList(0xbA2aE424d960c26247Dd6c32edC70B295c744C43, 0x10ED43C718714eb63d5aA57B78B54704E256024E); // DOGE
        addTokenToList(0x2170Ed0880ac9A755fd29B2688956BD959F933F8, 0x10ED43C718714eb63d5aA57B78B54704E256024E); // ETH
    }
}