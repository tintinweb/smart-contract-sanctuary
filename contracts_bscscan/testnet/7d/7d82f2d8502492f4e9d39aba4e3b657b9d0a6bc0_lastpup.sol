...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minDistribution = 10000000 * (10**18); //must hold 10000000 token
...
contract lastpup is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "lastpup";
    string constant _symbol = "lastpup";
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // cooldown timer, so a bot doesnt do quick trades! 2min gap between 2 trades.
        if (sender == pair &&
            buyCooldownEnabled &&
            !isTimelockExempt[recipient]) {
            require(cooldownTimer[recipient] < block.timestamp,"Please wait for 2min between two buys");
