...
contract XYX is owned {
    ...
    struct allocationType
    {
        ...
        uint monthlyReleaseLimitInPercent; // In Percent up to 18 decimal Places
        uint allocatedTotal;
        uint releasedTotal;
    }
    ...
    function claimAllocation(uint allocationTypeIndex,uint _claimAmount) public returns(bool)
    {
        ...
        uint currentMonthCount = ((block.timestamp - releaseTime) / 300 seconds) + 1;
        uint amountClaimable = _allocationBalance[allocationTypeIndex][msg.sender] * allocationTypes[allocationTypeIndex].monthlyReleaseLimitInPercent / (10 ** 20);
