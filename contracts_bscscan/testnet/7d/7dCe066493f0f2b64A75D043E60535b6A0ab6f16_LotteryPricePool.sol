...
contract LotteryPricePool is ReentrancyGuard,Ownable,Pausable{
    ...
    uint256 public prizePeriodStart;
    uint256 public prizePeriodStartedAt;
    
    string public clientSeeds;
    ...
    constructor(
        ...
        prizePeriodStart = _prizePeriodStart;
    ...
    function updatePool(uint256 _pool,address _address,uint256 _amount,bool _status) internal {
        if(_status){
            ...
            if(_pool == userPool[_address]){
                ...
                poolSize[userPool[_address]] -= (_balances[_address]);
                poolSize[_pool] += _balances[_address];
                userPool[_address] = _pool;
            }
        } 
    ...
    function stake(address _address,uint256 amount) external updateReward(_address) onlyMembershipPool() {
    ...
    function getReward(address _address) public updateReward(_address) onlyMembershipPool() returns(uint256) {
        uint256 reward = rewards[_address];
        if (reward > 0) {
            rewards[_address] = 0;
            rewardToken.safeTransfer(address(membershipPool), reward);
            emit RewardPaid(msg.sender, reward);
        }
        return reward;
    }

    function exit(address _address) onlyMembershipPool() external {
    ...
    function changeClientSeeds(string memory _seed) public {
        require(prizePeriodStartedAt.add(prizePeriodSeconds) > block.timestamp,"before lottey distribution");
        clientSeeds = _seed;
    }
    ...
    function completeAward(string memory _serverSeeds) public onlyOwner() requireCanStartAward() whenNotPaused() {
        nonce++;
        uint256 randomNumber = uint256(keccak256(abi.encode(_serverSeeds,clientSeeds,nonce)));
