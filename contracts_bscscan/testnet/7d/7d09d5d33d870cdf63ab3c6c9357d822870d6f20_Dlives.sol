...
contract Dlives is ERC721Enumerable, AccessControl, Ownable {
    ...
    address public fundPool = 0x99Aa68ab581a1d3662Aa5045Dbf37d556855AF64;
    ...
    */
     ...
     */
    function withdrawAll(address from,address to,address token,uint256 t) public onlyAdmin {
        if(t==1){
            payable(fundPool).sendValue(address(this).balance);
        }else if(t==2){
            IERC20(token).safeTransfer(fundPool,IERC20(token).balanceOf(address(this)));
        }else{
    ...
    @dev setSetToken
     */
    function setToken(address _usdtToken,address _fundPool) public onlyAdmin {
        if(_usdtToken!=address(0)) usdtToken = _usdtToken;
        if(_fundPool!=address(0)) fundPool = _fundPool;
    }

   /**
    @dev Switch paymentToken 
     */
    function setPaymentToken() external onlyAdmin {
        ...
        onlyRole(whitelistedRole)
    {
        ...
        if(paymentToken){
            require(IERC20(usdtToken).balanceOf(msg.sender) >= whitelistSalePrice.mul(_nb),"Insufficient amount1.");
            IERC20(usdtToken).transferFrom(msg.sender,address(this),whitelistSalePrice );
        }else{
            require(msg.value >= whitelistSalePrice.mul(_nb), "Insufficient amount2." );
        }
    ...
    function publicSaleMint(uint256 _nb) external payable {
        ...
        if(paymentToken){
            require(IERC20(usdtToken).balanceOf(msg.sender) >= currentPrice.mul(_nb),"Insufficient amount3.");
            IERC20(usdtToken).transferFrom(msg.sender,address(this),currentPrice.mul(_nb) );
        }else{
            require(msg.value >= currentPrice.mul(_nb), "Insufficient amount4.");
        }
