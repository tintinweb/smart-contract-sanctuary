...
interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to daosSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // daosSwap must mint EXACTLY the same amount of daosSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// MasterChef is the master of daos. He can make daos and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once daos is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of daoss
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accdaosPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accdaosPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. daoss to distribute per block.
        uint256 lastRewardBlock; // Last block number that daoss distribution occurs.
        uint256 accdaosPerShare; // Accumulated daoss per share, times 1e12. See below.
    }
    // The daos TOKEN!
    DAOS public daos;
    // Dev address.
    address public devaddr;
    // Block number when bonus daos period ends.
    uint256 public bonusEndBlock;
    // daos tokens created per block.
    uint256 public daosPerBlock;
    // Bonus muliplier for early daos makers.
    ...
    // The block number when daos mining starts.
    ...
    constructor(
        DAOS _daos,
        address _devaddr,
        uint256 _daosPerBlock,
        uint256 _startBlock,
        uint256 _bonusEndBlock
    ) public {
        daos = _daos;
        devaddr = _devaddr;
        daosPerBlock = _daosPerBlock;
    ...
    // Update the given pool's DAOS allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending daoss on frontend.
    function pendingdaos(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accdaosPerShare = pool.accdaosPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier =
                getMultiplier(pool.lastRewardBlock, block.number);
            uint256 daosReward =
                multiplier.mul(daosPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accdaosPerShare = accdaosPerShare.add(
                daosReward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(accdaosPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        daos.mint(devaddr, daosReward.div(10));
        daos.mint(address(this), daosReward);
        pool.accdaosPerShare = pool.accdaosPerShare.add(
            daosReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for daos allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accdaosPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            safedaosTransfer(msg.sender, pending);
        }
        ...
        user.rewardDebt = user.amount.mul(pool.accdaosPerShare).div(1e12);
    ...
    // Safe daos transfer function, just in case if rounding error causes pool to not have enough daoss.
    function safedaosTransfer(address _to, uint256 _amount) internal {
        uint256 daosBal = daos.balanceOf(address(this));
        if (_amount > daosBal) {
            daos.transfer(_to, daosBal);
        } else {
            daos.transfer(_to, _amount);
        }
    }
...
// export const DAOSToken = '0x6b7Ccd19B78F57EFaa89e6291b8834831C2F539f'
