...
/**
 *    SaveCryptoWorld Tokenomics
 *    totalSupply = 100 Billion
 *     30% burn at launch
 *    Note:
 *      - 2% tx fee redistributed to token holders on each transaction.  
 *      - 5% liquidity fee is taken on each transaction and locked forever in the liquidity pool.
 *      - 8% donation fee is taken on each transaction.
 *      - Functionality for max wallet size has been added (5% of total supply)
 *      - Cleaned up solc warnings
 *      - Updated to compile under solc >=0.8.0
 *      SaveCryptoWorld.com
 *      t.me/SaveCryptoWorld
 */
...
contract SaveCryptoWorld is Context,IBEP20,Ownable {
    ...
    string private constant _name = "SaveCryptoWorld";
    string private constant _symbol = "SaveCW";
    ...
    function getCirculatingSupply() external view returns(uint256){
        return _tTotal - balanceOf(BURN_ADDRESS);
    }
