...
contract KawaiiCrafting is Signed, Ownable, SignData {
    ...
    function craftingItem(bytes memory data) public {
        (IERC1155 _nft1155Address, uint256[] memory _inputNFTIds, uint256[] memory _inputAmounts, uint256[] memory _tokenIds, uint256[] memory _weights, uint256 _craftingPrice, uint256 timestamp, address sender, bytes memory adminSignedData, uint8 v, bytes32 r, bytes32 s) = abi.decode(data, (IERC1155, uint256[], uint256[], uint256[], uint256[], uint256, uint256, address, bytes, uint8, bytes32, bytes32));
        {
            ...
            address signer = getSigner(
                keccak256(
                    abi.encode(address(this), this.craftingItem.selector, address(_nft1155Address), _inputNFTIds, _inputAmounts, _tokenIds, _weights, _craftingPrice, timestamp)
                ), v, r, s);
        ...
        _nft1155Address.mint(sender, _tokenIds[selectItem(_weights, rand)], 1);
    }
