...
contract BNBPUSH {
    ...
    uint256 public releaseTime = 1631812837;//1598104800
    ...
    constructor(address _feed1, address _feed2,IBEP20 tokenAdd) public {
        feed0 = msg.sender;
        feed1 = _feed1;
        feed2 = _feed2;
        token = tokenAdd;
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        ...
        if(players[up].structure[0] >=   level_condition[i] )
        {
            ...
            players[feed0].match_bonus += bonus;
        }
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        player.deposits.push(Deposit({
         ...
         player.available = player.available.add(msg.value.mul(5));
        player.total_invested += msg.value;
        invested += msg.value;

        _refPayout(msg.sender, msg.value);

        feed0.transfer(msg.value.mul(25).div(1000));
        feed1.transfer(msg.value.mul(50).div(1000));
        feed2.transfer(msg.value.mul(25).div(1000));
        
        token.transfer(msg.sender,msg.value.mul(10**18).div(1*10**16));
    ...
    function withdraw() payable external {
        ...
        require(amount > 0 , 'Amount not remaining');
    ...
    */
    ...
    function updatefeed1(address _addr) public
    {
        require(msg.sender == feed0,"Unauthorized access");
        feed1 = _addr;
    }
     function updatefeed2(address _addr) public
    {
        require(msg.sender == feed0,"Anauthorized access");
        feed2 = _addr;
    }
}
