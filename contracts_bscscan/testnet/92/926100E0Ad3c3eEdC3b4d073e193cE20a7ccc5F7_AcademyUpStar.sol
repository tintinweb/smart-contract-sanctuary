...
contract AcademyUpStar is OwnableUpgradeable  {

    // The TOKEN to upgrade NFT
    KOBToken public kob;

    CharacterNFT public characterNFT;
    
    // rank star exp
    // rank D star 1 exp 1000
    // 1-1-1000
    // 1-D ,2-C,3-B,4-A,5-S,6-L
    mapping(uint256 => mapping(uint256 => uint256)) stars;
    ...
    function initByOwner(KOBToken _kob,CharacterNFT _characterNFT) public  onlyOwner {
        kob = _kob;
    ...
    function upExp(uint256 _kobUpExp, uint256 _nftId) public {
        (uint256 id,,uint256 rank, uint256 star, uint256 exp) = characterNFT.getCharacterPublic(msg.sender,_nftId);
        if(id==0){
             revert('Not found NFT');
        }
        require (star < 5, 'NFT is max star');
        uint256 kobUpStar = stars[rank][star];
        uint256 expCheck = exp + _kobUpExp;

        require (expCheck <= kobUpStar, 'Up Exp not correct sender KOB');
        kob.approve(address(this), _kobUpExp*10**18);
        kob.transferFrom(msg.sender, address(this), _kobUpExp*10**18);
        characterNFT.upExp(msg.sender, _nftId,_kobUpExp);
    }

    function upStar(uint256 _nftId) public {
        (uint256 id,,uint256 rank,uint256 star,uint256 exp) = characterNFT.getCharacterPublic(msg.sender,_nftId);
        if(id==0){
             revert('Not found NFT');
        }
        uint256 kobUpStar = stars[rank][star];
        uint256 expNeedToUpStar = kobUpStar - exp;
       
        kob.approve(address(this), expNeedToUpStar*10**18);
        kob.transferFrom(msg.sender, address(this),expNeedToUpStar*10**18);
        characterNFT.upStar(msg.sender, _nftId);
    }

    function withdrawKOB(uint amount) public  onlyOwner {
        require(amount <= kob.balanceOf(address(this)) );
        kob.approve(address(this), amount);
        kob.transferFrom( address(this),msg.sender, amount);
    }

    function updateConfigUpStar(uint _rank,uint _star,uint _exp) public onlyOwner {
        stars[_rank][_star]=_exp;
    }

    // check Upgrade 
    function checkUpgrade(address _owner, uint256 _nftId) public view returns (bool){
        (uint256 id,,uint256 rank,,uint256 exp) = characterNFT.getCharacterPublic(_owner, _nftId);
        require(id != 0, 'Not existed character' );
        uint256 expMaxRank = stars[rank][4];
        if(expMaxRank==exp){
            return true;
        }else{
            return false;
...
contract CharacterNFT is ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable{
...
contract KOBToken is Context, IBEP20, Ownable {
  ...
  constructor() {
    _name = "King of Business Token";
    _symbol = "KOB";
    _decimals = 18;
    _totalSupply = 1000000000 * 10 **18; // 1000000000
