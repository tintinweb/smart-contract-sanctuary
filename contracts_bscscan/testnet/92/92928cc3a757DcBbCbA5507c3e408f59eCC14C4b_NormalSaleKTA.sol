...
contract NormalSaleBase {
    ...
    event Purchase(address indexed _buyer, uint[] _ids, uint[] _amounts, uint _u_amounts);
    ...
    function setU(address _U) public onlyOwner returns (bool) {
        U = IERC20(_U);
        return true;
    }
...
contract NormalSaleKTN is NormalSaleBase {
    ...
    function setKAKACard(address _KAKACard) public onlyOwner returns (bool) {
        KTN = IKTN(_KAKACard);
        return true;
    }
    ...
    function purchase(uint[] calldata _ids, uint[] calldata _amounts) external {

        //check if ids are of level 1-5 and amounts not exceed quota
        require(_ids.length == _amounts.length, "lengths of ids and quantity should be equal");

        // notify the custormer the usdt amount needed
        uint u_amount;

        for (uint i = 0; i < _ids.length; i++) {
            require(status[_ids[i]], "Not Open");
            require(_amounts[i] > 0, "quantity should be positive");
            require(inventory[_ids[i]] >= _amounts[i], "Out Limit");
            inventory[_ids[i]] = inventory[_ids[i]].sub(_amounts[i]);

            u_amount = u_amount.add(price[_ids[i]].mul(_amounts[i]));
        }

        U.safeTransferFrom(_msgSender(), wallet, u_amount);

        KTN.mintBatch(_msgSender(), _ids, _amounts);

        emit Purchase(_msgSender(), _ids, _amounts, u_amount);
    }
...
contract NormalSaleKTA is NormalSaleBase {
    ...
    function purchase(uint[] calldata _ids, uint[] calldata _amounts) external {
        ...
        for (uint i = 0; i < _ids.length; i++) {
            KTA.mintMulti(_msgSender(), _ids[i], _amounts[i]);
        }
