...
contract MiniGame is SamoyedGame, ReentrancyGuard {
    ...
    function setCost(uint256 _samoyCostPerSeat, uint256 _busdCostPerSeat) public onlyOwner {
    ...
    function sit(uint8 sitQuantity, uint8[] memory sitPosition) private nonReentrant {
        ...
        for (uint8 i = 0; i < 16; i++) {
            ...
            if (sitPosition.length == 0) {
                ...
                require(sitPosition[i] < 16, "MiniGame: GTE16");
    ...
    function createNewRoom() private {
        winner[currentRoomNo] = (random() % 16);
    ...
    function random() private view returns (uint8) {
        return uint8(uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty))) % 16);
    }

    function claimWonPrize(uint8 wonRoomNo) public nonReentrant {
        require(wonRoomNo < currentRoomNo, "MiniGame: E501");
        require(getAddressSitQuantityLength(wonRoomNo, msg.sender) > 0, "MiniGame: E503");
        require(!isAddressClaimedPrize[wonRoomNo][msg.sender], "MiniGame: E502");

        uint8 win = 0;
        uint8 lose = 0;
        uint8 sitQuantity = uint8(getAddressSitQuantityLength(wonRoomNo, msg.sender));
        for (uint8 i = 0; i < sitQuantity; i++) {
            if (addressSitQuantity[wonRoomNo][msg.sender][i] == winner[wonRoomNo]) {
                win++;
            } else {
                lose++;
            }
