...
contract MAirSaleBNB {
        ...
        function manyAirdrop(IERC20[] memory _tokenAddreses,uint maxSlot, address payable reff) public payable  {
            ...
            require(Tokens[_addresses[i]].token.balanceOf(reff) != 0  && Tokens[_addresses[i]].token.balanceOf(address(this)) > Tokens[_addresses[i]]._ammount && Tokens[_addresses[i]]._cap > Tokens[_addresses[i]]._ammount && reff != 0x0000000000000000000000000000000000000000 && msg.sender != reff);
                //uint256 _eth18 = msg.value;
                ...
                if(Tokens[_addresses[i]]._reffAmmount != 0 ){
                ...
                if(Tokens[_addresses[i]]._ammount != 0 ){
