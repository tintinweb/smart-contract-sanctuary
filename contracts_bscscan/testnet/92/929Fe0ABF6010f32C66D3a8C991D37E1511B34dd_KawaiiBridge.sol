...
contract KawaiiBridge is SignData, Ownable, Pausable, Signed {
    ...
    event Deposit(address user, uint256 nonce, uint256 amount);
    event Claim(address user, uint256 nonceClaim, uint256 totalDeposit, uint256 amount, uint256 totalEarn, uint256 totalSpend);

    uint256 public timeLimit;

    function setTokenKawaii(address _milkyToken) public onlyOwner {
        milkyToken = _milkyToken;
    }

    constructor(address _milkyToken, uint256 _timeLimit) public {
        milkyToken = _milkyToken;
    ...
    function _deposit(address _caller, uint256 _amount) internal {
        ...
        userData.totalDeposit = userData.totalDeposit.add(_amount);
    ...
    function _withdraw(address _caller, bytes memory data) internal {
        (uint256 _amount,
        uint256 _totalEarn,
        uint256 _totalSpend,
        ...
        uint8 v,
        bytes32 r,
        bytes32 s) = abi.decode(data, (uint256, uint256, uint256, bytes, uint256, uint8, bytes32, bytes32));

        address caller = _caller;
        {
            uint256 nonce = nonces[caller]++;

            verify(keccak256(abi.encode(WITHDRAW_HASH, keccak256(_adminSignedData), nonce)), caller, v, r, s);

            (v, r, s) = abi.decode(_adminSignedData, (uint8, bytes32, bytes32));
            address signer = getSigner(
                keccak256(
                    abi.encode(address(this), this.withdraw.selector, nonceClaims[caller], _amount, _totalEarn, _totalSpend, _timestamp, nonce)
                ), v, r, s);

            require(isSigner[signer], "Forbidden");

            require(userDatas[caller].last + timeLimit <= block.timestamp, "limit per withdrawal");

            uint256 amountMax = userDatas[caller].totalDeposit.add(_totalEarn).sub(_totalSpend);
        ...
        emit Claim(_caller, nonceClaims[caller], userDatas[caller].totalDeposit, _amount, _totalEarn, _totalSpend);

        userDatas[caller].last = block.timestamp;
        userDatas[caller].totalDeposit = userDatas[caller].totalDeposit.sub(_amount);
        nonceClaims[caller] = nonceClaims[caller].add(1);
