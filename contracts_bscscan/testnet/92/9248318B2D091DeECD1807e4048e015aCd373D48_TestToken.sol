...
abstract contract FinanceBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	function doTransfer(address sender, address recipient, uint256 amount) internal virtual {
		...
		uint256 swapThreshold = _tokenSwapThreshold;
		
		if (tokensAvailableForSwap >= swapThreshold) {
			// Limit to threshold
			tokensAvailableForSwap = swapThreshold;
		...
		feesPooled += feeAmount;
		_totalFeesPooled = feesPooled;
	...
	function executeSwap(uint256 amount) private {
		...
		doApprove(fft, address(_pancakeswapV2Router), amount);
	...
	function swapTokensForBNB(address token, uint256 amount) internal {
		...
		// Swap
		_pancakeswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(amount, 0, path, token, block.timestamp + 360);
	}
