...
contract Tokenka is ERC20, Ownable {
 ...
 function setAutoSendDividends(bool _autoSendDividends) public onlyOwner {
        autoSendDividends = _autoSendDividends;
    }
    ...
    function swapOnDemand(uint256 amount, bool sendToFee, bool sendDividends) external onlyOwner {
        swapping = true;
    ...
    function _transfer(
        ...
        if( feesEnabled) {
            ...
            if( canSwap &&
                ...
                swapping = true;
                if(feeMultiplier > 0) {
                    uint256 feeTokens = contractTokenBalance.mul(feeMultiplier).div(100);
                    swapAndSendToFee(feeTokens);
                }
                if( autoSendDividends){
                   sendDividendsToHolders();
                }
                swapping = false;
            }
