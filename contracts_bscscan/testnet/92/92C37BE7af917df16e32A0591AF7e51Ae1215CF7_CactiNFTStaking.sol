...
contract CactiNFTStaking is IERC721Receiver, ReentrancyGuard {
    ...
    uint256 public stakingPeriodIndex = 0;
    ...
    function initStaking() external nonReentrant onlyDao returns (bool) {
        stakingPeriods[stakingPeriodIndex].startBlock = block.number;
        stakingPeriods[stakingPeriodIndex].endBlock = block.number + 576000;
        emit StakePeriod(block.number, (block.number + 576000), stakingPeriodIndex);
        return true;
    }
    ...
    function _unStakeNFT(uint256 tokenId) internal onlyStaker(tokenId) requireTimeElapsed(tokenId) returns (bool) {
        ...
        require(block.number >= stakingPeriods[stakingPeriodIndex].endBlock, 'Unstake: Farming end block has not been passed yet');
    ...
    function harvest(uint256 tokenId) external nonReentrant onlyStaker(tokenId) requireTimeElapsed(tokenId) {
        // require current block must be higher or equal than the farmingEndBlock
        require(block.number >= stakingPeriods[stakingPeriodIndex].endBlock, 'Harvest: Farming end block has not been passed yet');
    ...
    function _stakeNFT(uint256 tokenId) internal returns (bool) {
        ...
        require(block.number < stakingPeriods[stakingPeriodIndex].startBlock, 'Stake: Farming has ended');
    ...
    function _getTimeStaked(uint256 tokenId) internal view returns (uint256) {
        ...
        // loop through all staking periods and check the time staked per period.
        uint256 totalTimeStaked = 0;
        for (uint256 i = stakingPeriods.length-1; i >= 0; i--) {
            if (receipt[tokenId].stakedFromBlock < stakingPeriods[i].startBlock) {
                // stakedFromBlock < stakingPeriod, so add all blocks for this period. 
                totalTimeStaked.add(stakingPeriods[i].endBlock).sub(stakingPeriods[i].startBlock);
            } else if (receipt[tokenId].stakedFromBlock >= stakingPeriods[i].startBlock && receipt[tokenId].stakedFromBlock <= stakingPeriods[i].endBlock) {
                // the stakedFromBlock is in the middle of the period, so calculate the exact time staked in that period
                totalTimeStaked.add(stakingPeriods[i].endBlock).sub(receipt[tokenId].stakedFromBlock);
            } else {
                // The staking started after this period was ended. The loop can be ended now since this accounts for all periods to come
                break;
            }
        }
    ...
    // Setters
    function setFarmingTimeBlocks(uint256 _startBlock, uint256 _endBlock, uint256 _index) external onlyDao {
        require(_startBlock != 0, 'setFarmingTimeBlocks: Cannot be 0');
        require(_endBlock != 0, 'setFarmingTimeBlocks: Cannot be 0');
