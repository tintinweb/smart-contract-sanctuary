...
contract ERC20TokenPlusMax is Context, IERC20Metadata, Ownable {
    ...
    address private addressV2Router =  payable(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    constructor()  {
        _name = "WhosYourDaddy2";
        _symbol = "WYD";
        _decimals = 8;
        _totalTokens = 1000000000 * 5 ** 6 * 5 ** 4;
        ...
        accountsExcludedFromFees[uniswapV2Pair] = true;
    ...
    */
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        require(amount <= _tokensOwned[sender], "Transfer amount would exceed balance");

        _tokensOwned[sender] = _tokensOwned[sender] - amount;

        uint256 remaining = amount;
        uint256 sellTax;
        uint256 buyTax;

        if (_taxesEnabled && !accountsExcludedFromFees[sender] && !isSwapAndLiquifyLocked)
        {
        ...
        if (amount > remaining)
        {
            //            Add the taxed tokens to _owner
    ...
    function _transfer_adapter(address sender, uint256 tax, uint256 amount) private returns (uint256) {
        ...
        if (isDevFundFeeEnabled || isMarketingFeeFundEnabled) {
            ...
            // if _devAndMarketingAmount is 0 then not needed to swap.
