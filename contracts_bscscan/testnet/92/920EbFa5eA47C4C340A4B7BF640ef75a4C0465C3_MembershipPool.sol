...
contract MembershipPool is ReentrancyGuard,Authorizable{
    ...
    function rewardMedalPerPool(uint _pool) public view returns(uint256) {
        uint256 _rewardMedalPoolTokenStored;
        if (selfBalance == 0) {
               return _rewardMedalPoolTokenStored;
        }
        else{
            if(pools[_pool].poolSize > 0){
                _rewardMedalPoolTokenStored = pools[_pool].rewardMedalPoolTokenStored.add(
                ((block.timestamp.sub(medalDistributeAt)).div(medalRewardDuration))
                );
            }
            if(pools[_pool].poolSize == 0){
                _rewardMedalPoolTokenStored = pools[_pool].rewardMedalPoolTokenStored;
            }
        }
        return _rewardMedalPoolTokenStored;
    }
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            ...
            members[account].userMedalRewardPoolTokenPaid[_pool] = pools[_pool].rewardMedalPoolTokenStored;
        }
