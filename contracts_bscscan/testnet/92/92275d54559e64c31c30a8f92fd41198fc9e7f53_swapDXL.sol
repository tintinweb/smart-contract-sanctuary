...
contract swapDXL is Ownable {
    ...
    function swapDXLForDEEDEEToken() external payable {
        require(!isContract(_msgSender()),"swapDXLForDEEDEEToken :: Caller must not be contract address");
        require(msg.value >= _minimumIvestment, "swapDXLForDEEDEEToken :: wallet should deposit > 0 DXL at a time");
        require(msg.value <= DXLLimit, "swapDXLForDEEDEEToken :: wallet can buy max 200 DXL");
	    require(DEEDEE.balanceOf(address(this)) >= msg.value,"insufficient DEEDEE amount on the contract");

        uint amountDeeDee = msg.value.div(DEEDEEPrice); 

    	//transfer DEEDEE to sender
      	require(DEEDEE.transfer(msg.sender, amountDeeDee),"swapDXLForDEEDEEToken :: DEEDEE transfer failed");        
    
    	DXL.transfer(address(this),msg.value); //transfer DXL to contract
        
        totalSoldDXL = totalSoldDXL.add(msg.value);
        totalSoldDEEDEE= totalSoldDEEDEE.add(amountDeeDee);
        
        emit swap( _msgSender(), amountDeeDee, msg.value);
    }
