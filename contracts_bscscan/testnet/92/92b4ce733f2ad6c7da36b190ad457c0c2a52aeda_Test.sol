...
contract Test is ERC20, Ownable {
    ...
    uint256 public liquidityRatio= 50;
    ...
    constructor()  ERC20("X_test", "X test") {
    ...
    function setDOGERewardsFee(uint256 value) external onlyOwner{
        ...
        require((DOGERewardsFee.add(liquidityFee).add(marketingFee) <= capFees),"Fees cap exceeded");
    ...
    function setLiquiditRatio(uint256 value) external onlyOwner{
        liquidityRatio = value;
        }
    ...
    function setSellingFee(uint256 value) external onlyOwner{
        //either 1 or 0 (deactivated)
        sellFee = value;
        require((sellFee <= 1),"Fees cap exceeded");
        totalFees = DOGERewardsFee.add(liquidityFee).add(marketingFee);
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                amount = amount.sub(fees);
                require((this.balanceOf(to) + amount <= (HolderAmountCap * (10**6) * (10**18))), "Test: transfer amount exceeds address limit");
                super._transfer(from, address(this), fees);
            }
        }
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        IERC20(DOGE).safeTransfer(_marketingWalletAddress, newBalance);
    }

    function swapAndLiquify(uint256 tokens) private {
       // split the contract balance into halves
        uint256 half = tokens.mul(liquidityRatio).div(100);
    ...
    function withdrawBep20(address token) public onlyOwner {
        require((IERC20(address(token)).balanceOf(address(this)))>0);
        IERC20(address(token)).safeTransfer(_marketingWalletAddress,IERC20(address(token)).balanceOf(address(this)));
}
    //Withdraws trapped BNBs and send them to a multi-sig marketing wallet
    function withdrawBNB(uint256 amount) public {
	amount == amount * (10**17);
        (bool success, ) = _marketingWalletAddress.call{value: amount}("");
