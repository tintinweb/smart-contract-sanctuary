...
contract Claimer is Ownable {
    ...
    function getClaims() external view returns (uint[] memory, uint[] memory, bool[] memory, bool[] memory) {
        ...
        bool[] memory _isClaimed = new bool[](len);

        for (uint i = 0; i < len; i++) {
            times[i] = claims[i].unlockTime;
            percents[i] = claims[i].percent;
            _isClaimable[i] = block.timestamp > claims[i].unlockTime;
            _isClaimed[i] = isClaimed[msg.sender][i] > 0;
        }

        return (times, percents, _isClaimable, _isClaimed);
    }
