...
contract A is BEP20('A', 'ADMRSL6V10'){
    ...
    bool internal _recipient_is_contract; 
    bool internal _sender_is_contract;
    
    function recipient_is_contract(
        address recipient
    ) internal virtual returns (bool) {
        if (recipient.isContract()) {
            _recipient_is_contract = true;
        }
    }

    function sender_is_contract(
        address sender
    ) internal virtual returns (bool) {
        if (sender.isContract()) {
            _sender_is_contract = true;
        }
    }
    ...
    modifier NoContractsOn(address sender, address recipient) {
            if (
               ...
               // && _recipient_is_contract == true
               // && (_recipient_is_contract == true || _sender_is_contract == true)
               // && _contractwhitelist[sender] == false 
               // && (recipient.isContract() = true  || sender.isContract() = true)
            ) {
    ...
    function PauseTheChain_Pause() public onlyOperator whenNotPaused {
        _pause();
    }

    function PauseTheChain_Unpause() public onlyOperator whenPaused {
        _unpause();
    }
