...
contract PQExchange is Ownable, ERC165, IERC721Receiver {
    ...
    event RelistItem(address owner, uint256 listingID, uint256 newExpiry);
    event CancelListing(address owner, uint256 listingID);
    event BuyItNow(uint256 listing, address from);
    event ClaimListing(address bidder, uint256 listingID);
    ...
    function listOutOfGameItem(address owner, address contractAddress, uint256 tokenID, uint256 buyItNowPrice, uint256 expiry) public {
        require(owner == msg.sender || super.owner() == msg.sender , "only the owner of this contract or the owner of the asset can list their item");
        ...
        Listing memory newListing = Listing(listingID, owner, TokenType.ERC721, contractAddress, tokenID, buyItNowPrice, 0, expiry, 1);
    ...
    function relistItem(address owner, uint256 listingID, uint256 newExpiry) public {
        Listing storage item = individualListings[listingID];
        require((owner == msg.sender || super.owner() == msg.sender) && owner == item.owner , "only the owner of this contract or the owner of the asset can relist their item");
        ...
        emit RelistItem(owner, listingID, newExpiry);
    }
    ...
    function claimListing(address bidder, uint256 listingID) public {
        ...
        listingIDs = newArray;
        emit ClaimListing(bidder, listingID);
    }
    
    function placeBid(address bidder, uint256 listingID, uint256 amount) public {
        require(bidder == msg.sender || super.owner() == msg.sender , "only the owner of this contract or the address owner of the bidding wallet can bid");
        Listing storage item = individualListings[listingID];
