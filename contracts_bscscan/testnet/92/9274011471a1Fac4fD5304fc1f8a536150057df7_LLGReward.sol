...
contract LLGReward is Ownable {
    ...
    address public taxWallet = 0x3F54E24c3E67179FcAb0367bac76F179cdc83f90;
    address public bonusWallet = 0xEB74663BD160d8b2082c00B730F7ab0e282cEE1A;
    ...
    bool public consecutive = true;
    bool public canGetBonus = true;

    mapping(uint => mapping(address => bool)) public rewarded;
    mapping(uint => mapping(address => uint256)) public rewardedTime;
    ...
    function setBonusWallet(address _address) external onlyOwner {
        bonusWallet = _address;
    }

    // for DEV only
    function setConsecutiveCount(address _of, uint _num) external onlyOwner {
        numConsecutiveWins[_of] = _num;
    }
    ...
    function offerWinningReward(uint256 _room, uint256 _passport, address _winnerAddr, bool isFriendMatch) external returns (uint) {
        ...
        uint diffDays = (block.timestamp - startTimeOfDay) / (24*60*60*1000);
        if(diffDays >= 1) {
            startTimeOfDay = startTimeOfDay + diffDays * 24 * 60 * 60 * 1000;
        }

        if(!isFriendMatch) {
            if((block.timestamp - startTimeOfDay) / (24*60*60*1000) >= 1) {
                startTimeOfDay += ((block.timestamp - startTimeOfDay) / (24*60*60*1000))*24*60*60;
    ...
    function giveBonusReward(address _playerAddr, uint256 _room, uint256 _passport, uint _numCWins) external returns (bool) {
        require(passport == _passport, "Not authorized player");
        require(bonusWallet != _playerAddr, "Player address should not be the same as bonus wallet.");

        randomMatches[_playerAddr].room = _room;

        numConsecutiveWins[_playerAddr] = _numCWins;
        if(numConsecutiveWins[_playerAddr] == 3) {
            llgContract.transferFrom(bonusWallet, _playerAddr, 50 * 10**9);
            return true;
        }
        else if(numConsecutiveWins[_playerAddr] == 5) {
            llgContract.transferFrom(bonusWallet, _playerAddr, 100 * 10**9);
            return true;
        } 
        else if(numConsecutiveWins[_playerAddr] == 10) {
            llgContract.transferFrom(bonusWallet, _playerAddr, 300 * 10**9);
        }
        else {
            return false;
        }
