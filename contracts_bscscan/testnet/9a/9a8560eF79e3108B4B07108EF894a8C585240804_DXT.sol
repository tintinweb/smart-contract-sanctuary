...
contract DXT{
   ...
   IBEP20 private dxtToken; 
 

    constructor(IBEP20 _dxtToken) public 
    {
        owner = msg.sender;
    ...
    function withdrawBalance(uint256 amt,uint8 _type) public 
    {
        ...
        dxtToken.transfer(msg.sender,amt);
    }
    ...
    function buyToken(uint tokenQty) public payable
	{
	     ...
	     dxtToken.transfer(msg.sender , tokenQty);
	...
	function sellToken(uint tokenQty) public payable 
	{
	    ...
	    require(dxtToken.balanceOf(userAddress)>=(tokenQty),"Low Balance");
	    require(dxtToken.allowance(userAddress,address(this))>=(tokenQty),"Approve your token First");
	    require(!isContract(userAddress),"Can not be contract");
	    
	    uint256 bnb_amt=calcBuyAmt(tokenQty);
	     
		 dxtToken.transferFrom(userAddress ,address(this), (tokenQty));
