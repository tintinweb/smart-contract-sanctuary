...
contract SmartWorld {
    ...
    event Registration(address userAddress, uint256 accountId, uint256 refId, uint side);
    event BuyLevel(uint256 accountId, uint level);
    event Withdraw(uint256 accountId, uint256 amount);
    event WithdrawRequest(uint256 accountId, uint256 amount);
    ...
    function regUser(uint256 _referrerID, uint side) external payable {
        require(msg.value == LevelPrice[1], "Incorrect Value");
        regUserInternal(msg.sender, _referrerID, side);
    }
    
    function regUserInternal(address payable userAddress, uint256 _referrerID, uint side) internal {
        ...
        emit Registration(userAddress, lastIDCount, originalReferrer, side);
    }
    ...
    function adjustment(address payable userAddress , uint256 _referrerID) public onlyDeployer {
        ...
        users[userAddress] = UserInfo;
        userAddressByID[lastIDCount] = userAddress;
        emit Registration(userAddress, lastIDCount, originalReferrer, 1);
    ...
    function check_slot_status(address userAddress, uint8 _level) public view returns (bool){
