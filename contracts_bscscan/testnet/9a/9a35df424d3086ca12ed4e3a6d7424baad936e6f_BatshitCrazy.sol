...
contract BatshitCrazy is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    // Friday, 17 September 2021 23:59:00
    uint public immutable forceStartTradingAt = 1631923140;
    uint256 public sellAmount = 0;
    uint256 public buyAmount = 0;
    uint256 private totalSellFees;
    uint256 private totalBuyFees;

    BatshitCrazyDividendTracker public dividendTracker;
    ...
    uint256 public maxSellTransactionAmount = 2500000;
    uint256 public swapTokensAtAmount = 200000 * (10**9);

    uint256 public operationsFees = 5;
    ...
    constructor() ERC20("Crazy Test", "CrazyT") {
        dividendTracker = new BatshitCrazyDividendTracker(payable(this));
        ...
        totalSellFees = BNBRewardsSellFee.add(liquidityFee).add(operationsFees);
        totalBuyFees = BNBRewardsBuyFee.add(liquidityFee).add(operationsFees);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        BatshitCrazyDividendTracker newDividendTracker = BatshitCrazyDividendTracker(
            payable(newAddress)
        );
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 totalFees = BNBRewardsFee.add(liquidityFee).add(operationsFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividendsFromBuy = address(this)
            ...
            .div(BNBRewardsBuyFee.add(operationsFees));
        ...
        uint256 dividendsFromSell = address(this)
            ...
            .div(BNBRewardsSellFee.add(operationsFees));
...
contract BatshitCrazyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BatshitCrazy public batshitCrazy;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "BatshitCrazy_Dividend_Tracker",
            "BatshitCrazy_Dividend_Tracker"
        )
    {
        batshitCrazy = BatshitCrazy(mainContract);
        minimumTokenBalanceForAutoDividends = 100000 * (10**9);
    ...
    function _transfer(
        ...
        require(false, "BatshitCrazy_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BatshitCrazy_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BatshitCrazy contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(batshitCrazy);

            uint256 prevBalance = batshitCrazy.balanceOf(address(this));
            ...
            {
                uint256 received = batshitCrazy.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    batshitCrazy.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
