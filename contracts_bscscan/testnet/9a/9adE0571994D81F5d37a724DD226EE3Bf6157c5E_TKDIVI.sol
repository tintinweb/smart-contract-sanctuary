...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public DividendTokenOne = address(0);
  address public DividendTokenTwo = address(0);
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      uint256 _withdraw1Half = _withdrawableDividend.div(2);
      uint256 _withdraw2Half = _withdrawableDividend.sub(_withdraw1Half);

        bool successOne = false;
        bool successTwo = false;

        if(DividendTokenOne == address(0)){
            (bool sent,) = user.call{value: _withdraw1Half }("");
            successOne = sent;
            emit DividendWithdrawn(user, _withdraw1Half, DividendTokenOne);     
        }else{
            successOne = IERC20(DividendTokenOne).transfer(user, _withdraw1Half);
            emit DividendWithdrawn(user, _withdraw1Half, DividendTokenOne);
        }        
        
        if(DividendTokenTwo == address(0)){
            (bool sent,) = user.call{value: _withdraw2Half }("");
            successTwo = sent;
            emit DividendWithdrawn(user, _withdraw2Half, DividendTokenTwo);     
        }else{
            successTwo = IERC20(DividendTokenTwo).transfer(user, _withdraw2Half);
            emit DividendWithdrawn(user, _withdraw2Half, DividendTokenTwo);
        }

      if(!successOne) {
        withdrawnDividends[user] = withdrawnDividends[user].sub(_withdraw1Half);
        return 0;
      }      
      
      if(!successTwo) {
        withdrawnDividends[user] = withdrawnDividends[user].sub(_withdraw2Half);
        return 0;
      }
...
contract TKDIVI is ERC20, Ownable {
    ...
    // added
    event UpdateDividendTokenOne(address indexed newAddress, address indexed oldAddress);    
    event UpdateDividendTokenTwo(address indexed newAddress, address indexed oldAddress);
    //
    ...
    constructor() public ERC20("G2DIVIPLAN", "G2DVPL") {
        // LP burn address
    ...
    function updateTokenForDividendOne(address newAddress) external onlyOwner {
        dividendTracker.updateTokenForDividendOne(newAddress);
        DividendTokenOne = newAddress;
        emit UpdateDividendTokenOne(newAddress, address(DividendTokenOne));
    }        
    
    function updateTokenForDividendTwo(address newAddress) external onlyOwner {
        dividendTracker.updateTokenForDividendTwo(newAddress);
        DividendTokenTwo = newAddress;
        emit UpdateDividendTokenTwo(newAddress, address(DividendTokenTwo));
    }    
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        bool successOne = false;
        bool successTwo = false;
        uint256 dividendsOne;
        uint256 dividendsTwo;
        uint256 tokens1Half = tokens.div(2);
        uint256 tokens2Half = tokens.sub(tokens1Half);

        if(DividendTokenOne != address(0)){
            swapTokensForTokens(tokens1Half, address(this), DividendTokenOne);
            dividendsOne = IERC20(DividendTokenOne).balanceOf(address(this));
            successOne = IERC20(DividendTokenOne).transfer(address(dividendTracker), dividendsOne);
        }else{
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(tokens1Half);
            uint256 newBalance = address(this).balance.sub(initialBalance);
            dividendsOne = newBalance;
            (bool sent,) = diviTracker.call{value: newBalance}("");
            successOne = sent;
        }        
        
        if(DividendTokenTwo != address(0)){
            swapTokensForTokens(tokens2Half, address(this), DividendTokenTwo);
            dividendsTwo = IERC20(DividendTokenTwo).balanceOf(address(this));
            successTwo = IERC20(DividendTokenTwo).transfer(address(dividendTracker), dividendsTwo);
        }else{
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(tokens);
            uint256 newBalance = address(this).balance.sub(initialBalance);
            dividendsTwo = newBalance;
            (bool sent,) = diviTracker.call{value: newBalance}("");
            successTwo = sent;
        }

        if (successOne) {
            dividendTracker.distributeTokenDividends(dividendsOne);
            emit SendDividends(tokens1Half, dividendsOne);
        }        
        
        if (successTwo) {
            dividendTracker.distributeTokenDividends(dividendsTwo);
            emit SendDividends(tokens2Half, dividendsTwo);
        }
    } 
    ...
    function swapTokensForTokens(uint256 tokenAmount, address recipient, address _DividendToken) private {
...

{
  ...
  "evmVersion": "istanbul",
  "libraries": {
    "/Users/fly/Desktop/dev/safeFlight/contracts/buybackTokenNoTxFeesWithBurnZeroFeeTwoDivV1.sol": {
      "IterableMapping": "0x5d60eF5238e35FbfDAD3E225A2C932E2a92CE092"
    }
  },
  "outputSelection": {
    "*": {
