...
// File: contracts\Giga Berlin-Brandenburg .sol

contract AlohaToken  is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	uint256 private constant MAX = ~uint256(0);
	
	// MAIN TOKEN PROPERTIES
	string private constant _name = "Aloha Token";
	...
	// CHARITY
	address payable private constant _charityAddress = payable(0x8F85dFBAce50732eD8B967A8f8317C80B8997708); // A percentage of the BNB pool will go to the charity address
	...
	// BUYBACK
	address payable private constant _buybackAddress = payable(0x8F85dFBAce50732eD8B967A8f8317C80B8997708); // A percentage of the BNB pool will go to the charity address
    ...
    // TOKENOMICS
    uint256 private constant _initialBurnPercentage = 90;
    uint256 private constant _initialLiquidityPercentage = 9;
	...
	constructor (address routerAddress) {
		...
		_balances[_msgSender()] = _totalTokens * 1 / 100 * currentRate;

        _balances[_burnWallet] = _totalTokens * 90 / 100 * currentRate;
		...
		emit Transfer(address(0), _msgSender(), _totalTokens * 10 /100);
        emit Transfer(_burnWallet, _msgSender(), _totalTokens * 90 /100);
    ...
    function addToLiquidity(uint256 bnbToBeAddedToLiquidity) public onlyOwner{
        require(bnbToBeAddedToLiquidity > 0, "BNB Amount must be larger than 0");
        // uint256 currentRate = calculateDistributionRate();

		uint256 tokensToAddAsLiquidity = _totalTokens * _initialLiquidityPercentage / 100;

        (,, uint liquidity) = _pancakeswapV2Router.addLiquidityETH{value: bnbToBeAddedToLiquidity}(address(this), tokensToAddAsLiquidity, 0, 0, owner(), block.timestamp + 360);


    }

    function getWEI() public view returns (address) {
