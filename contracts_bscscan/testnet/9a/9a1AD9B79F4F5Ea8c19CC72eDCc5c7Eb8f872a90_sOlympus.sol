...
contract sOlympus is IsOHM, ERC20Permit {
    ...
    function initialize(address _stakingContract, address _sFidlV1Contract) external {
        ...
        require(_sFidlV1Contract != address(0), "StakingV1");
    ...
    function _storeRebase(
        ...
        uint256 rebaseAmount_,
        uint256 epoch_
    ) internal {
        uint256 rebasePercent = rebaseAmount_.mul(1e18).div(previousCirculating_);
        rebases.push(
            Rebase({
                epoch: epoch_,
    ...
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(IsOHM(sFidlV1Contract).circulatingSupply());
            // .add(gOHM.balanceFrom(IERC20(address(gOHM)).totalSupply())).add(
