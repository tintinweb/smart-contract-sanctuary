...
contract TESTTOKEN is ERC20, Ownable {
    ...
    uint256 public immutable fixedSaleStartTimestamp = 1627277802; //
    ...
    uint256 public immutable tradingEnabledTimestamp = 1627277802; //
    ...
    constructor() public ERC20("TEST TOKEN", "TESTTOKEN") {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "TEST TOKEN: The dividend tracker already has that address");

        TESTTOKENDividendTracker newDividendTracker = TESTTOKENDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TEST TOKEN: The new dividend tracker must be owned by the TEST TOKEN token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TEST TOKEN: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TEST TOKEN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "TEST TOKEN: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TEST TOKEN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TEST TOKEN: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "TEST TOKEN: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "TEST TOKEN: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "TEST TOKEN: The fixed-sale is only available to certain participants at the start");
            }
...
contract TESTTOKENDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("TESTTOKEN_Dividend_Tracker", "TESTTOKEN_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public override {
        require(false, "TESTTOKEN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TEST TOKEN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "TESTTOKEN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
