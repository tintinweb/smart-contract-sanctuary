...
contract Countdown is ERC20 {
    using SafeMath for uint256;
    uint256 initialSupply = 1000000000 * (10 ** 18);
    uint countdown = 100;
    
    address devAddress = 0x0De8947c2ABd59C201e5EcE142bFCd22253BFC0d;

    mapping(address => uint256) private balances;
    mapping(address => mapping(address => uint256)) private allowed;

    constructor () ERC20("Countdown", "CDOWN") {
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        uint fee = amount.div(100).mul(countdown);
        uint amountToBurn = fee.div(2);
        uint devTax = fee - amountToBurn;
        uint amountToTransfer = amount - fee;
        
        balances[recipient] = balances[recipient].add(amountToTransfer);
        balances[address(0)] = balances[address(0)].add(amountToBurn);
        balances[devAddress] = balances[devAddress].add(devTax);
        
        emit Transfer(msg.sender, recipient, amountToTransfer); 
        emit Transfer(msg.sender, address(0), amountToBurn); 
        emit Transfer(msg.sender, devAddress, devTax);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        if (countdown < 0) {
            uint shareburn = amount.div(10);
            uint shareuser = amount - shareburn;
            allowed[sender][msg.sender] = allowed[sender][msg.sender].sub(amount);
            balances[recipient] = balances[recipient].add(shareuser);
            balances[address(0)] = balances[address(0)].add(shareburn);
            
            emit Transfer(sender, recipient, shareuser); 
            emit Transfer(msg.sender,address(0),shareburn);
        } else if (countdown == 0) {
            uint shareburn2 = amount;
            uint shareuser2 = 0;
            allowed[sender][msg.sender] = allowed[sender][msg.sender].sub(amount);
            balances[address(0)] = balances[address(0)].add(shareburn2);
            
            emit Transfer(msg.sender, recipient, shareuser2);
