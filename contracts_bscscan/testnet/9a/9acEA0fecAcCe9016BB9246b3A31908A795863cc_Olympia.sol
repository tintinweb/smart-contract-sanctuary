...
contract Olympia is ERC20, Ownable {
    ...
    event DividendsSentToDistributor(address indexed wallet, uint256 amount);
    ...
    function _swap() private swapping {
        ...
        uint256 liquidityEthAmount = ethAmount.sub(teamEthAmount).sub(providerEthAmount).sub(marketingEthAmount);
    ...
    function _sendFeesToWallet(address wallet, uint256 ethAmount) private {
        if (ethAmount > 0) {
            ...
            require(success, "Olympia: Fees transfer failed");

            emit FeesSentToWallet(wallet, ethAmount);
        }
    }

    function _sendDividends(uint256 ethAmount) private {
        if (ethAmount > 0) {
            (bool success, /* bytes memory data */) = _dividendTracker.call{value: ethAmount}("");
            require(success, "Olympia: Dividends transfer failed");

            emit DividendsSentToDistributor(_dividendTracker, ethAmount);
        }
    }
