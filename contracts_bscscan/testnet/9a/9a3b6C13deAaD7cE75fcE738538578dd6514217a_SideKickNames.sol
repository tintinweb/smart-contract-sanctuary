...
contract SideKickNames {
    ...
    function setNameRecord(string calldata _name) public returns (bool _success) {	
        ...
        if(_checkSubscribed(_addr) == 0){
            payRooted(_isNamed);
            //tier one subscription pays for name after first payment	
        }if (_checkSubscribed(_addr) == 1 && _isNamed == true && redeemedSubscriptionNaming[msg.sender].nameRedemption == true){
    ...
    function setNFT721(address _nftAddr) public returns (bool _success){
        ...
        if(_checkSubscribed(_addr) == 1 && getNFTbyHeroAddress(_addr) != address(0) && redeemedSubscriptionNaming[_addr].nftRedemption == true){
