...
// SyrupBar used for HAKU staking.
contract SyrupBar is ERC20("HakuSwapBar Token", "SYRUP") {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    ...
    // The HAKU TOKEN!
    HakuToken public cake;

    constructor(HakuToken _cake) public {
        cake = _cake;
    }

    // Safe cake transfer function, just in case if rounding error causes pool to not have enough HAKUs.
...
// MasterChef is the master of HAKU. He can make HAKU and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once HAKU is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of HAKUs
    ...
    // HAKU tokens created per second.
    uint256 public cakePerSecond;

    // set a max cake per second, which can never be higher than 10 per second
    uint256 public constant maxCakePerSecond = 10e18;
    
    // Bonus muliplier for early haku makers.
    ...
    // The block time when HAKU mining starts.
    uint256 public startTime;

    // The HAKU pool weight  default = 4  //20%
    uint256 public hakuPoolWeight = 4;
    ...
    // Update the given pool's HAKU allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    function updateStakingPool() internal {
        ...
        if (points != 0) {
            points = points.div(hakuPoolWeight);
    ...
    // View function to see pending HAKUs on frontend.
    ...
    function updatePool(uint256 _pid) public {
        ...
        // HakuSwap Tokenomics
        // 0.5 HAKU per second to farm & pools
        // 10% to treasury address
        // 5% to burn address
        // 5% is stored and used for future usecase(NFT, metaverse, game, etc)

        cake.mintFor(devaddr, cakeReward.div(10));
        cake.mintFor(burnaddr, cakeReward.div(20));
        cake.mintFor(reserveaddr, cakeReward.div(20));
    ...
    // Deposit LP tokens to MasterChef for HAKU allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_pid != 0, "deposit HAKU by staking");
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        require(_pid != 0, "withdraw HAKU by unstaking");
    ...
    // Stake HAKU tokens to MasterChef
    ...
    // Withdraw HAKU tokens from STAKING.
    ...
    // Safe haku transfer function, just in case if rounding error causes pool to not have enough HAKUs.
    ...
    function setCakePerSecond(uint256 _cakePerSecond) external onlyOwner {
        require(_cakePerSecond <= maxCakePerSecond, "setCakePerSecond: too many HAKU!");
    ...
    // Update hakuPoolWeight.
    function setHAKUPoolWeight(uint256 _weight) public onlyOwner {
        require(_weight > 0, "must be bigger than 0");
        hakuPoolWeight = _weight;
        updateStakingPool();
    }
}