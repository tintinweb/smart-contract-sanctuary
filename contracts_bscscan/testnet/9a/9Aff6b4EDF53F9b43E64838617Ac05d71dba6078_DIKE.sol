...
contract DIKE is Context, IBEP20, Ownable {
    ...
    bool public sellingStatus = false;
    bool public updateTransferStatus = false;
    ...
    constructor() {
        ...
        // By default owner and contract are whiteListed
    ...
    function _transfer(
        ...
        if (!updateTransferStatus) {
            if (_isWhiteListed[sender]) {
                require(
                    ...
                    "whitelisted memeber can tranfer to only whitelisted"
                );
                require(
                    _isGoldListed[recipient],
                    "whitelisted memeber can tranfer to only whitelisted"
                );
                _beforeTokenTransfer(sender, recipient, amount);
                uint256 senderBalance = _balances[sender];
        ...
        // if (_isWhiteListed[sender] == true) {
        //     require(_isWhiteListed[recipient] == true, "not whitelisted");
        // }

        if (!sellingStatus) {
            require(recipient != dexPair, "selling is not open yet");
            if (
                _isGoldListed[sender] == true || _isWhiteListed[sender] == true
            ) {
                recipient == dexPair;
            }
        }
