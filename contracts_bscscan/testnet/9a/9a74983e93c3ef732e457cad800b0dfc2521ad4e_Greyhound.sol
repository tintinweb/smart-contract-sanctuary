...
contract Greyhound is Context, IERC20, Ownable {
    ...
    // Set standard fee values,optimized for presale. No value set is by default 0
    ...
    uint256 public _buyCharityFee    = 5;

    // Set sell fee values    
    uint256 public _sellTaxFee       = 10;
    uint256 public _sellLiquidityFee = 0;
    uint256 public _sellCharityFee   = 0;
    
    uint256 private _previousTaxFee         = _taxFee;
    uint256 private _previousLiquidityFee   = _liquidityFee;
    uint256 private _previousCharityFee     = _charityFee;
    
    uint256 private constant _burnAmount = 10; 
    ...
    // set all special transfers to 0 for presale.
    bool public swapAndLiquifyEnabled = false;
    bool public makeBurnEnabled = false;
    event makeBurnEnabledUpdate(bool enabled);
    bool public makeCharityEnabled = false;
    event makeCharityEnabledUpdate(bool enabled);
    ...
    uint256 private numTokensSellToAddToLiquidity = 7777777777 * 10**9;
...
constructor (address CharityWallet) {
        ...
        // Exclude owner, this contract, the burn and the charity wallet from fee
        _isExcludedFromFee[owner]        = true;
        _isExcludedFromFee[address(this)]  = true;
        _isExcludedFromFee[_CharityWallet] = true;
        
        // Exclude owner, this contractm the burn and the charitywallet from Rewards
        // This is needed because big wallets like the burn wallet would take the biggest piece of the cake (rewards)
    ...
    function decimals() public view returns (uint256) { //uint 8 by default
        return _decimals;
    }
    ...
    function setFees(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyCharityFee) external onlyOwner {
        _buyTaxFee = buyTaxFee;
        _buyLiquidityFee = buyLiquidityFee;
        _buyCharityFee = buyCharityFee;
    }
   
    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellCharityFee) external onlyOwner {
   ...
   function setMakeBurnEnabled(bool _enabled) public onlyOwner {
       makeBurnEnabled = _enabled;
       emit makeBurnEnabledUpdate(_enabled);
   }
   function setMakeCharityEnabled(bool _enabled) public onlyOwner {
       makeCharityEnabled = _enabled;
       emit makeCharityEnabledUpdate(_enabled);
   }   
    ...
    // Returns the current supply in the correct dimension, depending on address is excluded from rewards or not
    ...
    function _transfer(
        ...
        // Take buy fee values when transfer comes to the token
        ...
        // Take sell fee values when transfer comes to Uniswap
            if(to == uniswapV2Pair){
                removeAllFee();
                _taxFee = _sellTaxFee;
...
//        
//        //decimals
//        uint256 res0 = Res0.mul(10**token1.decimals());
//        return (amount.mul(Res0)).div(Res1); // returns amount of GHDtoken needed to buy token1
//    }
    ...
    //
    // We need "t" Values for reward free wallets and we need "r" values for the wallets with rewards
    // The return of the getValue functions are used to transport into other functions like Transfer().
    // They are needed to display the correct token amount per wallet (differences between included and excluded addresses)
    // In this contract several functions from here are splitted (getTValues/1), because the maximum local variables
    // within functions is limited in solidity/the ethereum blockchain.
    ...
    function _sendToBurnWallet(uint256 tBurn) private {
    ...
    // Transfer functions to handle reward excluded and included adresses.
    ...
    // As in _tokenTransfer() described, excludedFrom-->Rewards<-- NOT Fee!
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        // If statements to save gas due to avoided transaction without value. Othervise 0 token are send via transfer function
        if(makeCharityEnabled) {
            _sendToCharity(tCharity, sender);         
        }
        if(makeBurnEnabled) {
            _sendToBurnWallet(tBurn);
        }
   ...
   function _transferStandard(address sender, address recipient, uint256 tAmount) private {
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount,) = _getValues1(tAmount);
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tCharity, uint256 tBurn) = _getValues(tAmount);
        uint256 currentRate = _getRate();
        uint256 rFee = tFee.mul(currentRate);
    ...
    // basically not really needed because everything is set up on contract creation. But useful in case of presale troubleshooting
    function prepareForPreSale() public onlyOwner {
        setSwapAndLiquifyEnabled(false);
        setMakeBurnEnabled(false);
        setMakeCharityEnabled(false);
    ...
    function afterPreSale() external onlyOwner {
        ...
        setMakeBurnEnabled(true);
        setMakeCharityEnabled(true);
        ...
        // 0,1% of total supply to avoid price dump
        _maxTxAmount = 7777777 * 10**9;
    }
    //
    function claimTokens(address sender) public onlyOwner {
