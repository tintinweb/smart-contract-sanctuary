...
contract FairLaunch is IFairLaunch, Ownable, ReentrancyGuard {
    ...
    function getPidList() external view returns (uint256[] memory) {
        uint256[] memory lst = pidList[msg.sender];
        return lst;
    }

    function getBalanceByPid(uint256 pid) external view returns (uint256) {

        UserInfo[] memory users = userInfo[pid][msg.sender];
        uint256 bal;
    ...
    function getUserInfoByPid(uint256 pid) external view returns (UserInfo[] memory) {
        UserInfo[] memory users = userInfo[pid][msg.sender];
        return users;
    }
    ...
    function _harvest(address _to, uint256 _pid, uint256 index) internal {
       ...
       // console.log("safeRabbitTransfer",pending);
