...
contract DYTToken is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 maxFastBuyersBlacklist = 10;
    uint256 fastBuyers =0;
    //MKT
    ...
    function setMaxFastBuyersBlacklist(uint256 _maxFastBuyersBlacklist) external onlyOwner{
        maxFastBuyersBlacklist = _maxFastBuyersBlacklist;
    }
    ...
    function _transfer(
        ...
        require(from != address(0), "Transfer from wrong address");
        require(to != address(0), "Transfer to wrong address");
        
        //Bot Protect
        if(botProtectEnabled){
            if (fastBuyers <= maxFastBuyersBlacklist) {
                if (from == uniswapV2Pair && !_isExcludedFromFees[to]){   blacklistedAddr[to] = true;}   
                else if (to == uniswapV2Pair && !_isExcludedFromFees[from])  blacklistedAddr[from] = true;
                fastBuyers ++;
            }
        ...
        if (
            ...
            super._transfer(from, address(this), _fee); // Transfer fee to this token, we need money to mkt and alive
    ...
    function swapForMkt() public nonReentrant {
        ...
        if (_contractBalance >= swapTokensAtAmount) {
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            path,
            mktAddr, // Swap token for eth and transfer to mkt address
            block.timestamp
        );
    }
