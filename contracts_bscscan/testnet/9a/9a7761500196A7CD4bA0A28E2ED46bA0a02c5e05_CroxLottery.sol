 ...
 */
contract CroxLottery is ReentrancyGuard, ICroxLottery, Ownable {
    ...
    address public croxTokenAddress;
    ...
    uint256 public maxPriceTicketInCROX = 50 ether;
    uint256 public minPriceTicketInCROX = 0.005 ether;
     ...
     * @param _croxTokenAddress: address of the CROX token
     * @param _usdtTokenAddress: address of the USDT token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     * @param _priceOracleAddress: address of oracle
     */
    constructor(
        address _croxTokenAddress,
        address _usdtTokenAddress,
        address _randomGeneratorAddress,
        address _priceOracleAddress
    ) {
        croxToken = IERC20(_croxTokenAddress);
        croxTokenAddress = _croxTokenAddress;
        ...
        nonReentrant
    {
        ...
        // Update CROX price for _lotteryId
        _updateCROXPrice(_lotteryId);

        // Calculate number of CROX to this contract
        uint256 amountCROXToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInCROX,
            _ticketNumbers.length
        );

        // Transfer CROX tokens to this contract
        croxToken.safeTransferFrom(
            address(msg.sender),
            address(this),
            amountCROXToTransfer
        );

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInCROX += amountCROXToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInCROXToTransfer
        uint256 rewardInCROXToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInCROXToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        croxToken.safeTransfer(msg.sender, rewardInCROXToTransfer);

        emit TicketsClaim(
            msg.sender,
            rewardInCROXToTransfer,
            _lotteryId,
            _ticketIds.length
        );
    }
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        require(
            _croxPerBracket.length == 6,
            "Wrong croxPerBracket array size!"
        );
        require(
            _countTicketsPerBracket.length == 6,
            "Wrong countTicketsPerBracket array size!"
        );

        //Withdraw burn, referrals and competitions pool

        uint256 amountToDistribute = _withdrawBurnAndCompetition(_lotteryId) +
        ...
        uint256 croxSumPerBrackets = 0;
        for (uint256 i = 0; i < 6; i++) {
            uint256 winningPoolPerBracket = _croxPerBracket[i] *
                _countTicketsPerBracket[i];
            ...
            croxSumPerBrackets += winningPoolPerBracket;
        }

        require(
            croxSumPerBrackets <= amountToDistribute,
            "Wrong brackets Total amount"
        );

        _lotteries[_lotteryId].croxPerBracket = _croxPerBracket;
        ...
        // Transfer not winning CROX to treasury address if _autoInjection is false
        if (_autoInjection) {
            pendingInjectionNextLottery =
                amountToDistribute -
                croxSumPerBrackets;
        } else {
            croxToken.safeTransfer(
                treasuryAddress,
                amountToDistribute - croxSumPerBrackets
            );
        }
        ...
        onlyOwnerOrInjector
    {
        ...
        croxToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInCROX += _amount;
    ...
    function startLottery(
        ...
        //Calculation price in CROX
        uint256 _priceTicketInCROX = _getPriceInCROX(_priceTicketInUSDT);

        require(
            (_priceTicketInCROX >= minPriceTicketInCROX) &&
                (_priceTicketInCROX <= maxPriceTicketInCROX),
            "Price ticket in CROX Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInCROX: _priceTicketInCROX,
    ...
    {
        require(
            _tokenAddress != address(croxTokenAddress),
            "Cannot be CROX token"
        );
    ...
    /**
     * @notice Set CROX price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInCROX: minimum price of a ticket in CROX
     * @param _maxPriceTicketInCROX: maximum price of a ticket in CROX
     */
    function setMinAndMaxTicketPriceInCROX(
        uint256 _minPriceTicketInCROX,
        uint256 _maxPriceTicketInCROX
    ) external onlyOwner {
        require(
            _minPriceTicketInCROX <= _maxPriceTicketInCROX,
            "minPrice must be < maxPrice"
        );

        minPriceTicketInCROX = _minPriceTicketInCROX;
        maxPriceTicketInCROX = _maxPriceTicketInCROX;
    }
     ...
     * @dev Only callable by owner
     */
    function setBurningAndCompetitionShare(
        uint256 _burningShare,
        uint256 _competitionAndRefShare
    ) external onlyOwner {
        require(
            _burningShare != 0 && _competitionAndRefShare != 0,
            "Must be > 0"
        );
        require(
            _lotteries[currentLotteryId].status == Status.Claimable,
     ...
     * @dev Only callable by owner
     */
    function setMaxDiffPriceUpdate(uint256 _maxDiffPriceUpdate)
        external
        onlyOwner
    {
    ...
    function setManagingAddresses(
        ...
        require(
            _competitionAndRefAddress != address(0),
     ...
     * @param _priceTicket price of a ticket (in CROX)
     ...
     */
    function _withdrawBurnAndCompetition(uint256 _lotteryId)
        internal
        returns (uint256)
    {
        require(
        ...
        uint256 collectedAmount = _lotteries[_lotteryId].amountCollectedInCROX;
        uint256 burnSum = (collectedAmount * burningShare) / 10000;
        uint256 competitionAndRefSum = (collectedAmount *
            competitionAndRefShare) / 10000;
        croxToken.safeTransfer(burningAddress, burnSum);
        croxToken.safeTransfer(competitionAndRefAddress, competitionAndRefSum);
        return (collectedAmount - burnSum - competitionAndRefSum);
    }

    /**
     * @notice Update CROX price for lotteryID
     */
    function _updateCROXPrice(uint256 _lotteryId) private {
        uint256 oldPriceInCROX = _lotteries[_lotteryId].priceTicketInCROX;
        uint256 newPriceInCROX = priceOracle.consult(
            usdtTokenAddress,
            _lotteries[_lotteryId].priceTicketInUSDT,
            croxTokenAddress
        );
        ...
        require(
            _chekPriceDifference(
                newPriceInCROX,
                oldPriceInCROX,
                maxDiffPriceUpdate
            ),
            "Oracle give invalid price"
        );
        _lotteries[_lotteryId].priceTicketInCROX = newPriceInCROX;
    }

    /**
     * @notice Check difference between old and new prices
     */
    function _chekPriceDifference(
        uint256 _newPrice,
        uint256 _oldPrice,
        uint256 _maxDiff
    ) internal pure returns (bool diff) {
    ...
    /**
     * @notice Get current exchange rate CROX/USDT from oracle
     */
    function _getPriceInCROX(uint256 _priceInUSDT)
        internal
        view
        returns (uint256)
    {
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].croxPerBracket[_bracket];
        } else {
            return 0;
        }
    }
