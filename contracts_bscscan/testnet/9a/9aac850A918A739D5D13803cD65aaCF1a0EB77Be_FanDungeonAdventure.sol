...
contract FanDungeonAdventure is ERC721Holder, HeroInfo, OperatorControl {
    ...
    function deposit(
        ...
        uint256 _duration = _takeReservedRewards(currentTeam, _dungeonId, _level);
        _userInfo[msg.sender].teamIds.push(currentTeam);
        emit Deposit(msg.sender, _tokenIds, _dungeonId, _level, currentTeam, _duration);
    ...
    function withdraw(uint256 _teamId) external {
        ...
        _earnCrystal(
            msg.sender,
            fanDungeonLand.ownerOf(teamInfo_.dungeonId),
            _crystal,
    ...
    // view functions
    function viewPendingRewards(uint256 _teamId)
        public
        view
    ...
    function viewTeam(uint256 _teamId) public
        view
        returns (TeamInfo memory)
    {
        return _teamInfo[_teamId];
    }
    ...
    function _takeReservedRewards(
        ...
        Dungeon memory dungeon = viewDungeon(_dungeonId);
        ...
        _teamInfo[_team].dungeonId = _dungeonId;
        _teamInfo[_team].dungeonLevel = _dungeonLevel;
        _teamInfo[_team].startTime = block.timestamp;
        _teamInfo[_team].endTime = _endTime;
        _teamInfo[_team].reservedCrystal = _reservedCrystal;
        _teamInfo[_team].reservedXP = _reservedXP;
