...
contract swapping {
    ...
    function doSwap(IPancakePair[] memory pairs, IBEP20 startToken, uint256 amountIn, uint256 minProfit) public
    {
        ...
        for(uint i = 0; i < pairs.length; i++)
        {
            ...
            swapExactTokensForTokens(address(pairs[i]), inAmount, 0, path, address(this));
            tokenIn = path[1];
            inAmount = IBEP20(tokenIn).balanceOf(address(this));
        }
        require(inAmount >= amountIn+minProfit,"No profit.");
        if(tokenIn == address(startToken))
        {
            startToken.transfer(msg.sender, inAmount);
        }
    }
}