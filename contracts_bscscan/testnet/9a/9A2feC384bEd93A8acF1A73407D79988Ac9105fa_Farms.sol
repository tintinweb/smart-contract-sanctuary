...
contract Farms is ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  event LogUpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accEVRYPerShare);
  ...
  function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
    ...
    if (block.number > pool.lastRewardBlock) {
      ...
      poolInfo[pid] = pool;
      emit LogUpdatePool(pid, pool.lastRewardBlock, stakeTokenSupply, pool.accEVRYPerShare);
    }
  }
  ...
  function _harvest(address to, uint256 pid) internal {
    ...
    require(_pendingEvry <= evry.balanceOf(address(this)), "Farms::_harvest::not enough evry");

    // Effects
    user.rewardDebt = accumulatedEvry;

    evry.transfer(to, _pendingEvry);
