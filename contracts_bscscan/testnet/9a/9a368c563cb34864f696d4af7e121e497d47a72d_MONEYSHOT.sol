...
contract MONEYSHOTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("MONEYSHOT_Dividend_Tracker", "MONEYSHOT_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MONEYSHOT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MONEYSHOT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MONEYSHOT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MONEYSHOT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "MONEYSHOT_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MONEYSHOT is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    MONEYSHOTDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 6966966966 * (10**18);
    uint256 public swapTokensAtAmount = 696696696 * (10**18);
    ...
    constructor() ERC20("MONEYSHOT", "$HOT") {
        BNBRewardsFee = 300;
        liquidityFee = 250;
        ...
        devWallet = payable(0xA47B3E535C1804a57B67c7bC48e887f0346A4078);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(devFee); // total fee transfer and buy

    	dividendTracker = new MONEYSHOTDividendTracker();
        ...
        */
        _mint(owner(), 696696696696 * (10**18));
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MONEYSHOT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MONEYSHOT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MONEYSHOT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MONEYSHOT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MONEYSHOT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MONEYSHOT: Cannot update gasForProcessing to same value");
