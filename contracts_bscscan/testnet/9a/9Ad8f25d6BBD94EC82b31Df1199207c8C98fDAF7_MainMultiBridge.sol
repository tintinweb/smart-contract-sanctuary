...
/**
 * wSCC MainMultiBridge
 *
 * Attributes:
 * - Locks wSCC tokens and supplies
 * - Unlocks wSCC tokens from an authorized member after a cross-chain migration
...
contract MainMultiBridge is MultiBridge, FeeAccrual {
    ...
    function lock(
        ...
        uint256 lockAmount = wSCC.balanceOf(address(this));
        wSCC.safeTransferFrom(msg.sender, address(this), amount);
        lockAmount = wSCC.balanceOf(address(this)).sub(lockAmount);
    ...
    function transferBridgeFees(uint256 amount) external onlyOwner() {
        wSCC.safeTransfer(msg.sender, amount);
    }
}
...
contract FeeAccrual {
    ...
    ERC20MintSnapshot internal immutable wSCC;
    ...
    constructor(ERC20MintSnapshot wroot) internal {
        wSCC = wroot;
    }
    ...
    function availableRedemption(uint256 num) public view returns (uint256) {
        ...
        // uint256 userMints =
        //     uint256(wSCC.getPriorMints(msg.sender, fees.blockNumber));
        // uint256 totalMints =
        //     uint256(wSCC.getPriorMints(TOTAL_MINT, fees.blockNumber));
        
        uint256 amount = uint256(fees.amount);

        // redeem bridge fees based on total supply owned
        uint256 userMints =
            uint256(wSCC.balanceOf(msg.sender));
        uint256 totalMints =
            uint256(wSCC.totalSupply());
    ...
    function redeemAll() external {
        ...
        wSCC.safeTransfer(msg.sender, availableFees);
