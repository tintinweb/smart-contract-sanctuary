...
contract Zap is IZap, Ownable {
     ...
     */
    function initialize(address ali, address usdt, address busd, address weth, address btcb) onlyOwner public {
    ...
    function zapInToken(address fromToken, uint256 amount, address toToken) external override notPaused {
        uint256 fromTokenBalanceBefore = IBEP20(fromToken).balanceOf(address(this));
        ...
        if (isLpToken(toToken)) {
            ...
            if (fromToken == token0 || fromToken == token1) {
                ...
                ( , , liquidity) = ROUTER.addLiquidity(fromToken, other, amount.sub(sellAmount), otherAmount, 0, 0, msg.sender, block.timestamp, 0);

                uint256 fromTokenBalanceAfter = IBEP20(fromToken).balanceOf(address(this));
                ...
                uint256 bnbAmount = fromToken == WBNB? _safeSwapToBNB(amount) : _swapTokenForBNB(fromToken, amount, address(this)); 
                liquidity = _swapBNBToLpToken(toToken, bnbAmount, msg.sender);
            }
        } else {
    ...
    function zapIn(address toToken) external payable override notPaused {
        uint liquidity = _swapBNBToLpToken(toToken, msg.value, msg.sender);
        emit ZapInBNB(toToken, msg.value , liquidity);
    }

    function zapOut(address fromToken, uint256 amount) external override notPaused {
        IBEP20(fromToken).safeTransferFrom(msg.sender, address(this), amount);
        _approveTokenIfNeeded(fromToken);
    ...
    function _swapBNBToLpToken(address lpToken, uint256 amount, address receiver) private returns(uint liquidity) {
        if (!isLpToken(lpToken)) {
            ...
            if (token0 == WBNB || token1 == WBNB) {
                ...
                liquidity = _addLiquidityBNB(token, amount, receiver);
            } else {
    ...
    */
	function _addLiquidityBNB(address token, uint amount, address receiver ) private returns(uint liquidity) {
