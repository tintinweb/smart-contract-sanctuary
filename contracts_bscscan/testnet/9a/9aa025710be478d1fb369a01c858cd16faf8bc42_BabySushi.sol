...
contract BabySushi is IERC20, Ownable {
    ...
    function claimAirdrop() external{
        ...
        Airdrop memory holder = airdrops[airdropAddress];
        require(swapEnabled, "You need to wait until launch time");
        require(holder.isAirDrop, "You are not in airdrop list");
        require(holder.totalClaimed < holder.totalToken, "You have no token to claim");
        require(holder.lastClaimTime + minBetweenClaim < block.timestamp, "Claim too fast");
        require(balanceOf(airdropWallet) >= holder.tokenPerClaim, "Not enough tokens to airdrop");
        
        _basicTransfer(airdropWallet, airdropAddress, holder.tokenPerClaim);
        ...
        holder.totalClaimed += holder.tokenPerClaim;
    }
