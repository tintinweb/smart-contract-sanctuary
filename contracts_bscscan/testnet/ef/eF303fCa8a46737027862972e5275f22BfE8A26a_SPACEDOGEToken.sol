...
contract SPACEDOGEToken is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public _dividendToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; //BUSD
    ...
    SDOGETokenDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SPACEDOGE", "SDOGE") {
        dividendTracker = new SDOGETokenDividendTracker();

         
        lotteryWallet = 0x80C9029bb2E804048c9566f8a4ADA23A9F1d6A33;
        marketingWallet = 0x80C9029bb2E804048c9566f8a4ADA23A9F1d6A33;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "SDOGE: The dividend tracker already has that address"
        );

        SDOGETokenDividendTracker newDividendTracker = SDOGETokenDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "SDOGE: The new dividend tracker must be owned by the SDOGE token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "SDOGE: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "SDOGE: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "SDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "SDOGE: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "SDOGE: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "SDOGE: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[to]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 bnbSwap = buyLotteryFee.add(
                    buyMarketingFee
                );
    ...
    function swapAndSendBnb(uint256 tokenAmount, uint256 totalFee) private {
        ...
        transferToWallet(
            payable(lotteryWallet),
            address(this).balance.div(totalFee).mul(buyLotteryFee)
        );
        transferToWallet(
...
contract SDOGETokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("SDOGE_Dividend_Tracker", "SDOGE_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "SDOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SDOGE contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "SDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "SDOGE_Dividend_Tracker: Cannot update claimWait to same value"
        );
