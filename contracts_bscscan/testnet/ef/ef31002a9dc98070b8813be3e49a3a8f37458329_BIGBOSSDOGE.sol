...
/* 
 *                            
🔥 HOLD BIGBOSSDOGE, EARN BNB! 🥳

💎 A BNB reflection token with auto-claim feature! 💫
👑 Simply hold $BBD tokens in your wallet and you'll earn BNB!💰🤯

📣 - 8% transaction fee redistribution in BNB to $BBD holders. 
🔑 - 4% transaction fee is automatically locked into liquidity. 
🏆 - Auto Payment every 60 minutes. 
🌏 - 4% funding to marketing campaign

✉️ Telegram: https://t.me/bigbossdogeofficial

📝 Twitter: 

🌐 Website: https://bigbossdogebsc.com                                                                  
 */
...
contract BIGBOSSDOGE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BIGBOSSDOGEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BIGBOSSDOGE", "BBD") {
    	...
    	dividendTracker = new BIGBOSSDOGEDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BIGBOSSDOGE: The dividend tracker already has that address");
 
        BIGBOSSDOGEDividendTracker newDividendTracker = BIGBOSSDOGEDividendTracker(payable(newAddress));
 
        require(newDividendTracker.owner() == address(this), "BIGBOSSDOGE: The new dividend tracker must be owned by the BIGBOSSDOGE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BIGBOSSDOGE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BIGBOSSDOGE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BIGBOSSDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BIGBOSSDOGE: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BIGBOSSDOGE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BIGBOSSDOGE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BIGBOSSDOGE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BIGBOSSDOGE: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "BIGBOSSDOGE: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "BIGBOSSDOGE: The fixed-sale is only available to certain participants at the start");
            }
...
contract BIGBOSSDOGEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BIGBOSSDOGE_Dividend_Tracker", "BIGBOSSDOGE_Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "BIGBOSSDOGE_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "BIGBOSSDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BIGBOSSDOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "BIGBOSSDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BIGBOSSDOGE_Dividend_Tracker: Cannot update claimWait to same value");
