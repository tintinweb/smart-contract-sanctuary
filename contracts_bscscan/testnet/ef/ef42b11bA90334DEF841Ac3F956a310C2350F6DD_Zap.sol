...
contract EstimateLpToken {
    using SafeMath for uint;
    using SafeBEP20 for IBEP20;
    
    IZap public ZAP;

    constructor(address _zap) public {
        ZAP = IZap(_zap);
    }

    function getEstimatedLpToken(address fromToken, address toToken, uint fromTokenAmount) public view returns (uint liquidity) {
        require(ZAP.getIsLpToken(toToken) ,'toToken must be LP token' );
        ...
        uint amount0 = fromToken == token0 ? reserve0 + swapAmount : reserve0 + getPoolAddedAmount(fromToken, token0, swapAmount);
        uint amount1 = fromToken == token1 ? reserve1 + swapAmount : reserve1 + getPoolAddedAmount(fromToken, token1, swapAmount);
    ...
    function getPoolAddedAmount(address fromToken, address toToken, uint swapAmount) public view returns (uint amount) {
        address[] memory path = getRoutePath(fromToken, toToken);
        amount = ZAP.getRouter().getAmountsOut(swapAmount, path)[1];
    }

    function getRoutePath(address fromToken, address toToken) public view returns (address[] memory path) {
        address intermediate = ZAP.routePair(fromToken);
        if (intermediate == address(0)) {
            intermediate = ZAP.routePair(toToken);
        }

        if (intermediate != address(0) && (fromToken == ZAP.getWBNB() || toToken == ZAP.getWBNB())) {
            path = new address[](3);
            path[0] = fromToken;
            path[1] = intermediate;
            path[2] = toToken;
            ...
            path[0] = fromToken;
            path[1] = toToken;
        } else if (intermediate != address(0) && ZAP.routePair(fromToken) == ZAP.routePair(toToken)) {
            path = new address[](3);
            path[0] = fromToken;
            path[1] = intermediate;
            path[2] = toToken;
        } else if (
            ZAP.routePair(fromToken) != address(0) &&
            ZAP.routePair(toToken) != address(0) &&
            ZAP.routePair(fromToken) != ZAP.routePair(toToken)
        ) {
            path = new address[](5);
            path[0] = fromToken;
            path[1] = ZAP.routePair(fromToken);
            path[2] = ZAP.getWBNB();
            path[3] = ZAP.routePair(toToken);
            path[4] = toToken;
        } else if (intermediate != address(0) && ZAP.routePair(fromToken) != address(0)) {
            path = new address[](4);
            path[0] = fromToken;
            path[1] = intermediate;
            path[2] = ZAP.getWBNB();
            path[3] = toToken;
        } else if (intermediate != address(0) && ZAP.routePair(toToken) != address(0)) {
            path = new address[](4);
            path[0] = fromToken;
            path[1] = ZAP.getWBNB();
            path[2] = intermediate;
            path[3] = toToken;
        } else if (fromToken == ZAP.getWBNB() || toToken == ZAP.getWBNB()) {
            path = new address[](2);
            path[0] = fromToken;
            path[1] = toToken;
        } else {
