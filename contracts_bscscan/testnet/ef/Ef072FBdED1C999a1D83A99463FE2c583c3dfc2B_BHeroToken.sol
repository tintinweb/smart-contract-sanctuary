...
contract BHeroToken is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  function createToken(address to, uint256 count) internal {
    require(to != address(0), "zero address");
    uint256 seed = Utils.randomSeed(count);
    for (uint256 i = 0; i < count; ++i) {
      uint256 id = tokenIdCounter.current();
      seed = createRandomToken(seed, id);
  ...
  function createRandomToken(uint256 seed, uint256 id) internal returns (uint256 nextSeed) {
