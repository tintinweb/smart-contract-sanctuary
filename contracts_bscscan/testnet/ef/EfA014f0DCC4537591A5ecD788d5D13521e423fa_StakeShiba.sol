...
contract StakeShiba {
    ...
    address payable public aiContract;
    ...
    // Separate return records
     ...
     mapping(address => uint) public levelIncomeGain;
    ...
    uint16[] percent_ =  [800,300,200,100,50,50,25,25];
    ...
    function withdrawDailyGain(uint stakeIndex) public returns(bool)
    {
      require(!isContract(msg.sender),  'No contract address allowed to withdraw');
        ...
        if(tim2 < tim + lD)
        {
          ...
          {
            if(checklimit(ref,amt * (userInfo[msg.sender].referralplace*10)/100))
            {
              sponsordailyGain[ref]+= amt * (userInfo[msg.sender].referralplace*10)/100;
              userInfo[ref].total_payouts += amt * (userInfo[msg.sender].referralplace*10)/100;
            }
          }
          else
          {
            if(checklimit(ref, amt))
            {
              sponsordailyGain[ref]+= amt;
              userInfo[ref].total_payouts += amt ;
            }
          }

          if(checklimit(msg.sender, amt * 98 / 100))
          {
            tokenBalanceLedger_[msg.sender] = tokenBalanceLedger_[msg.sender].add(amt * 98 / 100);
            userInfo[msg.sender].total_payouts += amt * 98 / 100;
            //tokenInterface(tokenAddress).transfer(msg.sender, amt * 98 / 100);
          }
          tokenBalanceLedger_[terminal] = tokenBalanceLedger_[terminal].add(amt / 100);
          tokenInterface(tokenAddress).transfer(terminal, amt / 100);
    ...
    constructor(address payable _aiContract, address _tokenAddress) public
    {
        terminal = msg.sender;
    ...
    function eligible(uint i, address ref) internal view returns(bool)
    {
        ...
        else if(i>7 && i < 11 && refCount[ref] == 3) return true;
        else return false;
    }
    ...
    function buy_(address useraddress, address _referredBy, uint256 tokenAmount) public onlyAdministrator returns(uint)
    {
        ...
        uint tkn = purchaseTokens(msgsender, tokenAmount, _referredBy);
        ...
        returns(uint256)
    {
      require(!isContract(msg.sender),  'No contract address allowed');
        ...
        userInfo[msg.sender].totalbus += tokenAmount;
        tokenAmount = tokenAmount.div(2);
        ...
        uint tkn = purchaseTokens(msg.sender, tokenAmount, _referredBy);
        return tkn;
    }
    ...
    function _AIpollDeposits(address _addr, uint256 _amount) private {
        ...
        ai_pool_users_refs_deposits_sum[ai_pool_cycle][_addr] += _amount;
    ...
    function purchaseTokens(address _customerAddress, uint256 _amountOfTokens, address _referredBy) internal returns(uint256)
    {
        //deduct commissions for referrals
        if(checklimit(_referredBy, _amountOfTokens/10))
        {
          ...
          sponsorGain[_referredBy] += _amountOfTokens/10;
          emit directPaid(_customerAddress, _referredBy ,_amountOfTokens / 10);
          tokenSupply_ = tokenSupply_.add(_amountOfTokens + (_amountOfTokens/10) );
          level1Holding_[_referredBy] +=_amountOfTokens;
          userInfo[_referredBy].total_payouts += (_amountOfTokens/10);
        }
    ...
    function distributeRewards(uint256 _amount, address _idToDistribute)  internal
    {
        for(uint i=0; i<8; i++)
        {
            ...
            uint pct;
            pct = percent_[i];
            if(value >= (1746 * (10** decimals)) && _holdingLevel1 >= (holding_[i]) )
            {
                if(eligible(i+1, _idToDistribute))
                {
    ...
    function setAiContract(address payable _aiContract) public onlyAdministrator returns(bool)
    {
        aiContract = _aiContract;
        return true;
    }
