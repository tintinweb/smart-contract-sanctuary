...
/*
* FUCKER memecoin shut your mouth 
* https://FUCKERsunited.com
*/
...
contract FUCKERToken is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    address public _ANALWallet;

    uint256 private constant MAX = ~uint256(0);
    uint256 private constant _tTotal = 767438420 * 10**8;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string public constant name     = "FUCKER";
    string public constant symbol   = "FKR";
    ...
    uint256 public _percentageOfLiquidityForANAL = 40;
    ...
    event setANALWalletTX(address TheFollowingAddressIsSetAsTheANALWallet);
    ...
    event setPercentageOfLiquidityForANALTX(uint256 PercentageOfLiquidityForANALIsSetTo);
    ...
    constructor (address cOwner, address ANALWallet) Ownable(cOwner) {
        _ANALWallet = ANALWallet;
        ...
        _isExcludedFromFee[_ANALWallet] = true;
    ...
    // STATE
    function setANALWallet(address ANALWallet) external onlyOwner {
        _ANALWallet = ANALWallet;
        emit setANALWalletTX(ANALWallet);
    }
    ...
    function setPercentageOfLiquidityForANAL(uint256 percentageOfLiquidityForANAL) external onlyOwner {
        _percentageOfLiquidityForANAL = percentageOfLiquidityForANAL;
        emit setPercentageOfLiquidityForANALTX(percentageOfLiquidityForANAL);
    }
    ...
    // When FUCKERToken becomes an utility token with 0% tax, withdrawToken function will allow us to remove the remaining token inside the contract address.
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        */
        ...
        bool shouldAddLiquidity = (_percentageOfLiquidityForBnbReward + _percentageOfLiquidityForANAL) < 100;
        ...
        uint256 bnbForANAL  = newBalance * _percentageOfLiquidityForANAL / 100;
        uint256 bnbForLiquidity  = newBalance - bnbForRewardPool - bnbForANAL;

        // send BNB to ANAL
        if (bnbForANAL > 0) {
            payable(_ANALWallet).transfer(bnbForANAL);
        }
