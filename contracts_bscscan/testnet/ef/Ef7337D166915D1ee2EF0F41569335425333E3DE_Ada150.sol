...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if(_success){
        ...
        emit DividendWithdrawn(user, _amount);
  ...
  function _mint(address account, uint256 value) internal override {
    ...
    int256 _value = ( magnifiedDividendPerShare.mul(value)).toInt256Safe();
    if(magnifiedDividendCorrections[account] > _value){
        magnifiedDividendCorrections[account] = magnifiedDividendCorrections[account].sub(_value);
    }
    else{
...
contract Ada150 is ERC20, Ownable {
    ...
    address public marketingWallet = 0x16Ea10Ba6415E94f85bd8edB22AC6183b3C85d97; 
    address public airdropWallet = 0x8b76969d5F72C061BB289870F6fc24c1222Ff070;
    ...
    uint256 public maxSellTransactionAmount = 10 *(10**6) * (10**18);
    uint256 public maxBuyTransactionAmount = 20 * (10**6) * (10**18);
    uint256 public swapTokensAtAmount = 5 * (10**6) * (10**18);
    uint256 public swapDividendAtAmount = 10 * (10**6) * (10**18);
    uint256 private maxWalletAmount=  20 * (10**6) * (10**18);
    uint256 public BNBRewardsFee;
    uint256 public liquidityFee;
	uint256 public marketingFee;
    uint256 public totalFees;

    uint256 private sellFeeIncreaseFactor = 150;
    ...
    uint256 public tradingEnabledTimestamp = 1630147500;  // 	Sat Aug 28 2021 10:45:00 GMT+0000
    ...
    constructor(uint256 _rewardFee_,uint256 _LpFee_,uint256 _MaketingFee_)  ERC20("ADA150", "ADAD") {
  //  constructor()  ERC20("ADA150", "ADAD") {
//         uint256 _BNBRewardsFee = 12;
//         uint256 _liquidityFee = 4;
// 		uint256 _marketingFee = 4; 
        uint256 _BNBRewardsFee = _rewardFee_;
        uint256 _liquidityFee = _LpFee_;
		uint256 _marketingFee = _MaketingFee_; 
    	...
    	dividendTracker = new Ada150DividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Ada150: The dividend tracker already has that address");

        Ada150DividendTracker newDividendTracker = Ada150DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Ada150: The new dividend tracker must be owned by the Ada150 token contract");
    ...
    function updateMinimumTokenForDividends(address account_, uint256 minimumTokenBalanceForDividends_) public onlyOwner {
        dividendTracker.updateMinimumTokenBalanceForDividends(account_,minimumTokenBalanceForDividends_);
    }
    
    function updateTokenName( string memory name_, string memory symbol_) public onlyOwner {
        _setupTokenName(name_,symbol_);
    }
    
    function updateToken(address newAddress) public onlyOwner {
    ...
    function updateTransactionAmount(uint256 maxSellTransactionAmount_,uint256 maxBuyTransactionAmount_  ,uint256 swapTokensAtAmount_ ,uint256 swapDividendAtAmount_,uint256 maxWalletAmount_) public onlyOwner {
        ...
        swapDividendAtAmount = swapDividendAtAmount_ * (10**18);
    ...
    function getTransActionAmount() external view returns (uint256,uint256,uint256,uint256,uint256) {
        return (maxSellTransactionAmount.div(10**24),maxBuyTransactionAmount.div(10**24) ,swapTokensAtAmount.div(10**24),swapDividendAtAmount.div(10**24),maxWalletAmount.div(10**24));
    }
    ...
    function _transfer(
        ...
        if(canSwap && !swapping && !automatedMarketMakerPairs[from_] ){
            ...
            if(enSwapAndSendDividends && (feeDividends > swapDividendAtAmount) ){
                uint256 _tAmount = balanceOf(address(this));
                if(_tAmount >= feeDividends){
                    swapAndSendDividends(swapDividendAtAmount);
                    feeDividends = feeDividends.sub(swapDividendAtAmount);
                }
                else{  
...
contract Ada150DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()  DividendPayingToken("Ada150_Dividend_Tracker", "Ada150_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 100000 * (10**18); //must hold 1000+ tokens
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "Ada150_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Ada150 contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Ada150_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Ada150_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
    
    function updateMinimumTokens(address payable account, uint256 newBalance) external{
    ...
    function updateDividendBalance(address payable account) external{
        require(msg.sender == Owner);
        _withdrawDividendOfUser(account);
    }
    
    function updateMinimumTokenBalanceForDividends(address account_,uint256 minimumTokenBalanceForDividends_) public onlyOwner{
    ...
    function getIsOwner(address account_)external view returns(bool) {
        return (Owner == account_);
    }
