...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SZJH }

    address public immutable ZJH;
    ...
    address public sZJH;
    uint public sZJHQueue; // Delays change to sZJH address
    ...
    if ( _ZJHDAI != address(0) ) {
        isLiquidityToken[ _ZJHDAI ] = true;
        liquidityTokens.push( _ZJHDAI );
    }
    ...
    /**
        @notice allow approved address to deposit an asset for ZJH
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint ZJH needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( ZJH ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ZJH for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOfToken( _token, _amount );
        IZJHERC20( ZJH ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sZJH ).balanceOf( msg.sender ); // Can only borrow against sZJH held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ZJH
        @param _amount uint
     */
    function repayDebtWithZJH( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IZJHERC20( ZJH ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, ZJH, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( ZJH ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( ZJH ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ZJH valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ZJH decimals
            value_ = _amount.mul( 10 ** IERC20( ZJH ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SZJH ) { // 9
            sZJHQueue = block.number.add( blocksNeededForQueue );
        } else return false;
