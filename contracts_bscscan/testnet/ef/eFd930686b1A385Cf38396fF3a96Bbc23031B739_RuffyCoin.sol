...
abstract contract RuffyCoinBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
  ...
  uint8 private _developerFee; //% of each transaction that will be used for developer
    ...
    require(developerFee >= 0 && developerFee <= 5, "Developer fee must be between 0% and 5%");
		require(additionalSellFee <= 10, "Additional sell fee cannot exceed 10%");
...
contract RuffyCoin is RuffyCoinBase {
	...
	MinimumTokenBalance private T1 = MinimumTokenBalance(1000000 * 10**decimals(), 75);
	MinimumTokenBalance private T2 = MinimumTokenBalance(10000000 * 10**decimals(), 50);
	MinimumTokenBalance private T3 = MinimumTokenBalance(100000000 * 10**decimals(), 0);
