...
contract RetroNeko is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        address distributor) public initializer {
        ...
        _swapThreshold = 5_000_000 * 10 ** decimals(); // 50M $RNK ( 0.05% )
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (amount > 0) {
            super._transfer(sender, recipient, amount.div(2));
        }
    ...
    function _shouldSetShare(address account) private view returns (bool) {
        return
            account != owner() && account != address(this) &&
            account != _router && account != _deadWallet &&
            !_isAutomatedMarketMakerPairs[account];
    }
