...
/*
    ...
    Total Supply: 1,000,000,000 ADEN
    
    Redistribution rule: No
    0% tax & fee on transactions
       
    Email: [email protected] - Tele: @adeneplatform
    Web: adene.io 
*/
...
contract ADENE is Context, IERC20, Ownable {
    ...
    uint256 private numTokensSellToAddToLiquidity = 2 * 10**7 * 10**18; // 2%
...
// This make it secure from bots
    ...
    modifier launchRestrict(address sender, address recipient, uint256 amount) {
        if (_tradingTime > now) {
            require(sender == owner() || recipient == owner(), "ADENE: transfers are disabled");
        } else if (_tradingTime <= now && _restrictionLiftTime > now) {
            require(amount <= _maxRestrictionAmount, "ADENE: amount greater than max limit in restricted mode");
            if (!_isWhitelisted[sender] && !_isWhitelisted[recipient]) {
                require(_lastTx[sender].add(60) <= now && _lastTx[recipient].add(60) <= now, "ADENE: only one tx/min in restricted mode");
                _lastTx[sender] = now;
                _lastTx[recipient] = now;
            } else if (!_isWhitelisted[recipient]){
                require(_lastTx[recipient].add(60) <= now, "ADENE: only one tx/min in restricted mode");
                _lastTx[recipient] = now;
            } else if (!_isWhitelisted[sender]) {
                require(_lastTx[sender].add(60) <= now, "ADENE: only one tx/min in restricted mode");
                _lastTx[sender] = now;
            }
        }
        _;
...
// Bot End here
    ...
    function _transfer(
        ...
        require(!_paused || from == owner() || to == owner(), "ADENE: Transfers are paused"); // pause check
