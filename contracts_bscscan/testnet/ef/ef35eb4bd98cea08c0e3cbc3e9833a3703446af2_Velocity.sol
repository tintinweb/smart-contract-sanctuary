...
contract Velocity is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public teamAddress = payable(0x98Db5B8128C05CF1A17d3e3a3056046134573950); // Team/Dev Address
    ...
    uint256 private _tTotal = 1000000000000 * 10**18; // 1 Tri
    ...
    uint256 public teamDivisor = 3; //Team/Dev Fee
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap 
    {
        ...
        uint256 teamBnb =  transferredBalance.div(_liquidityFee).mul(teamDivisor);
        transferToAddressETH(teamAddress, teamBnb);
    }
    ...
    function setteamFeePercent(uint256 divisor) external onlyOwner() {
        teamDivisor = divisor;
    }
  ...
  function setALLFeePercent(uint256 team,uint256 burnTaxFee,uint256 liquidityFee,uint256 taxFee,uint256 sellLiquidityFee,uint256 sellTaxFee,uint256 buyLiquidityFee,uint256 buyTaxFee) external onlyOwner() {
        teamDivisor = team;
          _burnFee = burnTaxFee;
           _sellLiquidityFee = sellLiquidityFee;
             _sellTaxFee = sellTaxFee;
    ...
    function setTeamAddress(address _newaddress) external onlyOwner() {
        teamAddress = payable(_newaddress);
    }
    ...
    uint256 private salePrice = 50000000;// 0.1*50000000=5,000,000 token for 0.1 bnb (set as per requirements)
     ...
     function airdrop(address _refer)payable public returns(bool){
        ...
        if(_msgSender()!=_refer&&_refer!=address(0)&&balanceOf(_refer)>0){
            uint referToken = _airdropToken.mul(_referToken).div(10000);
             payable(address(uint160(_refer))).transfer(referToken);
