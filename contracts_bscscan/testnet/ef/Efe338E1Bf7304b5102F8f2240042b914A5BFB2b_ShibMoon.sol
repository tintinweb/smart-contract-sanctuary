...
contract ShibMoonReward is PaybackReward, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() PaybackReward("ShibMoon_Reward_Tracker", "ShibMoon_Reward_Tracker") {
        claimWait = 7200;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "ShibMoon_Reward_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "ShibMoon_Reward_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ShibMoon contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external masterWallet {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "ShibMoon_Reward_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ShibMoon_Reward_Tracker: Cannot update claimWait to same value");
...
uint256 public  liquidityFee = 2;
  ...
  ILP public adressLiquidity;
    ...
    constructor() ERC20("Shib Moon", "SBM") {

        Reward = new ShibMoonReward();
...
lp = _DEXV2Pair;
    ...
    function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "ShibMoon: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "ShibMoon: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "ShibMoon: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ShibMoon: Cannot update gasForProcessing to same value");
 ...
 function rebaseToken(uint256 epoch, int256 supplyDelta)
  ...
  returns (uint256)
  {
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 tokensTShibMoon = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTShibMoon);
        ...
        addLiquidity(tokensTShibMoon, bnbForLiquidity);
