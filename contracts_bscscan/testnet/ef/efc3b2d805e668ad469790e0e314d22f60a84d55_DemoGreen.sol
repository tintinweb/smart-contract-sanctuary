...
contract DemoGreen is Context, IERC20, Ownable {
    ...
    event GetKingT(uint256 tTransferAmount, uint256 tFee, uint256 tSwap);
    event GetKingR(uint256 rAmount, uint256 rTransferAmount, uint256 rFee);
    event king(uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tSwap);
    event GetRate(uint256 a1, uint256 b1);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        emit king(rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tSwap);
        // _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _getValues(uint256 tAmount) private returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        emit GetKingT(tTransferAmount, tFee, tSwap);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tSwap, _getRate());
        (uint256 a, uint256 b) = _getCurrentSupply();
        emit GetRate(a, b);

        emit GetKingR(rAmount, rTransferAmount, rFee);
