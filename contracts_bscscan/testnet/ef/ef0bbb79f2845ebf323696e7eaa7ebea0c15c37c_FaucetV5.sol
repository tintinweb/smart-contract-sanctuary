...
contract FaucetV5 is OwnableUpgradeable {
    ...
    IToken private holdingToken; // TODO this will become SK
    ...
    function payOutRateOf(address _addr) public view returns (uint256) {
        uint256 rate = ratesController.payOutRateOf(_addr, payoutRate);
        return rate;
    }
    ...
    function payoutOf(address _addr) public view returns(uint256 payout, uint256 max_payout, uint256 net_payout, uint256 sustainability_fee) {
        ...
        if(users[_addr].payouts < max_payout) {
            ...
            share = users[_addr].deposits.mul(payoutRate).div(100e18).div(24 hours); //divide the profit by payout rate and seconds in the day
