...
contract BEP20 is Context, IBEP20, Ownable {
     ...
     * construction.
     */
    constructor(string memory tokenName, string memory tokenSymbol, uint256 maxSuppply)  {
        _name = tokenName;
        _symbol = tokenSymbol;
        _decimals = 18;
        _maxSupply = maxSuppply.mul(10**18);
    }
    ...
    */
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        require(totalSupply() + amount <= _maxSupply, "mint: request exceeds max supply");
   ...
   struct NftPoolInfo{
    ...
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            uint256 urCounter = 0;
            for(uint256 i = 0; i < mythicsTokens.length -1; i++){
                if(mythicsTokens[i] > 1500 )
                    usersURTokens[i] = (mythicsTokens[i]);
                    urCounter = urCounter + 1;
            }
            
            require(urCounter >= _amount, "deposit: Not enough Mythic NFTs");

            uint256[] memory pmotTokens = pmotNFT.getUserNftTokensForRarity(2,msg.sender);
            require(pmotTokens.length >= _amount, "deposit: Not enough Mythic NFTs");

            for(uint256 i = 0; i < _amount; i++){
                mythicNFT.safeTransferFrom(address(msg.sender), address(this), usersURTokens[i]);             
                pmotNFT.safeTransferFrom(address(msg.sender), address(this), pmotTokens[i]);             
                user.amountStaked = user.amountStaked + 1;           
                user.pmotUltraRaretokenIds.push(pmotTokens[i]);
                user.mythicUltraRaretokenIds.push(mythicsTokens[i]);
                pool.amountStaked = pool.amountStaked + 1;
            }
...
contract Zap is Ownable {
    ...
    // IApeRouter01 private constant ROUTER = IApeRouter01(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);


    //MainNet
    address private  WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address private  BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    IApeRouter01 private constant ROUTER = IApeRouter01(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function zapInToken(
        ...
        uint256 amount,
        address _to,
        address _recipient //TODO TESTing THIS.
    ) external {        
