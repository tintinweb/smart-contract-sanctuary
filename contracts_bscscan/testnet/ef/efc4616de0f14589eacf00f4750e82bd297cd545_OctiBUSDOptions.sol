...
/**
 * Octi
 * Copyright (C) 2020 Octi Protocol
 *
 ...
 * MBEPHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
...
// File: contracts/Pool/OctiBUSDPool.sol
 ...
 */
contract OctiBUSDPool is
    IBEPLiquidityPool,
    Ownable,
    BEP20("Octi BUSD LP Token", "OBUSD")
{
    using SafeMath for uint256;
    ...
    IBEP20 busdToken = IBEP20(0x95f3339475c04Fa95E8e91237C489957E446BACb);
    ...
    /*
     * @nonce A provider supplies BNB to the pool and receives writeBNB tokens
     ...
     */
    function provide(uint256 minMint, uint256 token) external  returns (uint256 mint) {
        busdToken.transferFrom(msg.sender,address(this),token);
        lastProvideTimestamp[msg.sender] = block.timestamp;
        investedAmount[msg.sender] += token;
        uint supply = totalSupply();
        uint balance = totalBalance();
        if (supply > 0 && balance > 0)
            mint = token.mul(supply).div(balance.sub(token));
        else
            mint = token.mul(INITIAL_RATE);

        require(mint >= minMint, "Pool: Mint limit is too large");
        require(mint > 0, "Pool: Amount is too small");

        _mint(msg.sender, mint);
        emit Provide(msg.sender, token, mint);
    }

    /*
     * @nonce Provider burns writeBNB and receives BNB from the pool
    ...
    /*
     * @nonce calls by OctiCallOptions to lock the funds
     * @param amount Amount of funds that should be locked in an option
     */
    function lock(uint id, uint256 amount, uint256 token) external override onlyOwner  {
        require(id == lockedLiquidity.length, "Wrong id");
        require(
            lockedAmount.add(amount).mul(10) <= totalBalance().sub(token).mul(8),
            "Pool Error: Amount is too large."
        );

        lockedLiquidity.push(LockedLiquidity(amount, token, true));
        lockedPremium = lockedPremium.add(token);
        lockedAmount = lockedAmount.add(amount);
    }

    /*
     * @nonce calls by OctiOptions to unlock the funds
    ...
    /*
     * @nonce calls by OctiCallOptions to send funds to liquidity providers after an option's expiration
     * @param to Provider
     * @param amount Funds that should be sent
     */
    function send(uint id, address to, uint256 amount)
        external
        override
        onlyOwner
    {
        ...
        busdToken.transfer(to,transferAmount);
    ...
    function totalBalance() public override view returns (uint256 balance) {
        return  busdToken.balanceOf(address(this)).sub(lockedPremium);
    }
...
// File: contracts/Options/OctiBUSDOptions.sol
...
/**
 * @author 0mllwntrmt3
 * @title Octi BNB (Ether) Bidirectional (Call and Put) Options
 * @notice Octi BNB Options Contract
 */
contract OctiBUSDOptions is Ownable, IOctiOptions {
    using SafeMath for uint256;

    address public settlementFeeRecipient;
    Option[] public override options;
    uint256 public impliedVolRate;
    uint256 public settlementPercent;
    uint256 public setoptionSize = 5e17;
    ...
    OctiBUSDOptions private oldOctiBUSDOptions;
    AggregatorV3Interface public priceProvider;
    OctiBUSDPool public pool;
    
    IBEP20 busdToken1 = IBEP20(0x95f3339475c04Fa95E8e91237C489957E446BACb);
    constructor(AggregatorV3Interface pp,address owner, OctiBUSDPool _pool) public {
        pool = _pool;
        priceProvider = pp;
        settlementFeeRecipient = owner;
        impliedVolRate = 4500;
        settlementPercent = 1;
    ...
    function transferPoolOwnership() external onlyOwner {
       // require(block.timestamp < contractCreationTimestamp);
    ...
    function setImpliedVolRate(uint256 value) external onlyOwner {
      ...
      function setSettlementPercent(uint256 value) external onlyOwner {
        settlementPercent = value;
      }

     function setOptionSize(uint256 value) external onlyOwner {
         setoptionSize = value;
     }

    function setSettlementFeeRecipient(address newOwner) external onlyOwner {
        require(address(newOwner) != address(0));
        settlementFeeRecipient = newOwner;
    }
    ...
    {
        require(amount <= setoptionSize,"Wrong option size");
        ...
        require(token >= total.mul(currentPrice.div(10**8)), "Wrong value");
        busdToken1.transferFrom(msg.sender,address(this),token);
        if (token > total.mul(currentPrice.div(10**8))) busdToken1.transfer(msg.sender,token - total.mul(currentPrice.div(10**8)) );
        ...
        Option memory option = Option(
            ...
            (total.sub(settlementFee)).mul(currentPrice.div(10**8)),
            block.timestamp + period,
            optionType
        );

        options.push(option);
        busdToken1.transfer(settlementFeeRecipient,settlementFee.mul(currentPrice.div(10**8)));
        busdToken1.transfer(address(pool),option.premium);
        pool.lock(optionID, option.lockedAmount,option.premium);
        emit Create(optionID, msg.sender, settlementFee.mul(currentPrice.div(10**8)), total.mul(currentPrice.div(10**8)));
    }
    ...
    function migrate(uint count) external onlyOwner {
        ...
        require(address(oldOctiBUSDOptions) != address(0));
        for (uint i = 0; i < count; i++){
            uint optionID = options.length;
            OctiBUSDOptions.Option memory option;
            (
                option.state,
                option.holder,
                option.strike,
                ...
                option.expiration,
                option.optionType
            ) = oldOctiBUSDOptions.options(optionID);
    ...
    function setOldOctiBUSDOptions(address oldAddr) external onlyOwner {
        require(address(oldOctiBUSDOptions) == address(0));
        oldOctiBUSDOptions = OctiBUSDOptions(oldAddr);
    }
    ...
    function getSettlementFee(uint256 amount)
        ...
        returns (uint256 fee)
    {
        uint256 getpercent = amount * settlementPercent;
