...
contract FactoryMigrationHelper {
  ...
  mapping(address => bool) public migrationExistence;
    ...
    uint256 divider,
    uint256 amountNewToken
  ) public payable {
		require(msg.value > normalMigrationFee, "You need to pay the fee to start the migration!");
    require(IERC20(newTokenAddress).balanceOf(msg.sender) >= amountNewToken, "You don't have enough new token!");

    address newMigration = address(
      new MigrationHelper(
        address(this),
        title,
        oldTokenAddress,
    ...
    migrationExistence[newMigration] = true;
		migrations.push(newMigration);
    // Transfer the tokens from msg.sender to the contract
    IERC20(newTokenAddress).safeTransferFrom(msg.sender, newMigration, amountNewToken);
...
contract MigrationHelper {
  using SafeERC20 for IERC20;
    ...
    require(IERC20(oldTokenAddress).balanceOf(msg.sender) > 0, "You don't have old token");
    ...
    IERC20(newTokenAddress).safeTransfer(msg.sender, newAmountToReceive);
	...
	function setNewAmountToReceive(address _receiver) private {
		newAmountToReceive = IERC20(oldTokenAddress).balanceOf(_receiver).div(10**decimalsOldToken).mul(10**decimalsNewToken).div(divider);
	}
}
