...
contract EtermonBox is Etermon {
    address private etermonMonster;
    address private etermonItem;
    bool private lock;

    function boxInitailize(address etermonMonster_, address etermonItem_, address mintFactory_) external{
        require(!lock);
        mintFactory = mintFactory_;
        _addMinter(mintFactory_);
        _addMinter(owner());
        etermonInitailize("EtermonBox","ETB");
        etermonMonster = etermonMonster_;
        etermonItem = etermonItem_;
        lock = true;
    }

    function openBox(uint256 tokenBurn,uint256 etermonMonsterQuantites, uint256 etermonItemQuantities) external {
        assert(
            !isBlackListed[msg.sender] &&
            etermonMonsterQuantites + etermonItemQuantities <= 3
        );

        _burn(tokenBurn);

        MintFactory(mintFactory).mintTo(msg.sender, etermonMonster, etermonMonsterQuantites);
        MintFactory(mintFactory).mintTo(msg.sender, etermonItem, etermonItemQuantities);
    }
}