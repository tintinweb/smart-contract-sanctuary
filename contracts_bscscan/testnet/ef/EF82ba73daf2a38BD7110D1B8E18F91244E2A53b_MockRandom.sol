...
abstract contract Random is Context, SafeOwnable {
    ...
    function requestRandom(uint256 _seed) private returns (bytes32) {
        emit RequestRandom(bytes32(requestId), _seed);
    ...
    function finishRandom(bytes32 _requestId) private {
...
contract MockRandom is Random {
    constructor(address _linkAccessor) Random(_linkAccessor) {
    }
}
