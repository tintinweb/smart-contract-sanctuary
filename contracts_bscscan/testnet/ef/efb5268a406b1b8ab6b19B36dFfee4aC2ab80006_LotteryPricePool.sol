...
contract LotteryPricePool is ReentrancyGuard,Ownable,Pausable{
    ...
    function changeMedal(address _medalContract) public onlyOwner(){
        medalContract = MedalNFT(_medalContract);
    }
    ...
    function getPool(address _address) public view returns(uint256){
        if(_balances[_address] >= viplevelLimit){
            return 4;
        }
        else if(_balances[_address] >= goldLevelLimit){
            return 3;
        }
        else if(_balances[_address] >= sliverLevelLimit){
            return 2;
        }
        else if(_balances[_address] >= greenLevelLimit){
            return 1;
        }
        else{
            return 0;
    ...
    function withdraw(address _address,uint256 amount) public nonReentrant() updateReward(_address) onlyMembershipPool() {
        require(amount > 0, "LotteryPricePool: Cannot withdraw zero amount");
        require(amount <= _balances[_address],"LotteryPricePool: Not a valid amount");
    ...
    function _notifyRewardAmount(uint256 reward,uint256[POOLS] memory poolShare) internal {
        ...
        require(_total == DENOMINATOR,"LotteryPricePool: not valid pool share");
        ...
        for(uint i=1 ; i< POOLS ; i++){
            ...
            require(rewardRate[i] <= balance.div(rewardsDuration), "LotteryPricePool: Provided reward too high");
        }
    ...
    modifier onlyMembershipPool() { 
      require (address(membershipPool) == msg.sender ,"LotteryPricePool: only call by membership Pool"); 
      _; 
    }
