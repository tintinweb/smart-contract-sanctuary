...
contract SideKickNames {
    ...
    {
        bool isNamed = _isNamed(msg.sender);
        
        payRooted(isNamed, msg.sender);
    ...
    function setNFT721(address _nftAddr) public returns (bool _success) {
        require(rooted.transferFrom(msg.sender, address(this), price), "Pay NFT fee");
        address _addr = msg.sender;
        return _setNFTRecord(_addr, _nftAddr);
    }

    function payRooted(bool isNamed, address payee) private returns (bool _success) {
        if (isNamed) {
            require(rooted.transferFrom(payee, address(this), taxedPrice));
        } else {
            require(rooted.transferFrom(payee, address(this), price));
        }
    ...
    function _isNamed (address _addr) private view returns (bool _named){
        string memory empty = '';
        string memory name = getNameByAddress(_addr);
