...
contract Exchange is OrderState, TransferSafe {
    ...
    constructor(address payable beneficiary, address payable buyerfeesigner, address jadeAddr, uint beneficiaryfee) public {
        beneficiaryAddress = beneficiary;
        buyerFeeSigner = buyerfeesigner;
        beneficiaryFee = beneficiaryfee;
        jade = IERC20(jadeAddr);
    }
    
    function sell(Order calldata order, Sig calldata sig, Sig calldata buyerFeeSig, uint256 royaltyFee, address payable royaltyReceipt)  external {
       require((block.timestamp <= order.expiryTime ), "signature expired" );
       require(order.key.buyAsset.token == address(jade));
    ...
    function buy(Order calldata order, Sig calldata sig, Sig calldata buyerFeeSig, uint256 royaltyFee, address payable royaltyReceipt) external {
       require( ( block.timestamp<= order.expiryTime ), "signature expired" );
       require(order.key.sellAsset.token == address(jade));
    ...
    function transferSellFee(Order calldata order,  address payable royaltyReceipt, uint256 royaltyFee, address buyer) internal {
        ...
        } else if(order.key.buyAsset.token == address(jade)){
            transferjadeFee(order.buying, order.key.owner, buyer, royaltyFee, royaltyReceipt);
        } else {
    ...
    function transferBuyFee(Order calldata order,  address payable royaltyReceipt, uint256 royaltyFee, address buyer) internal {
        if(order.key.sellAsset.token == address(jade)){
            transferjadeFee(order.selling, buyer, order.key.owner, royaltyFee, royaltyReceipt);
        } else {
            transferErc20Fee(order.key.sellAsset.token,order.selling, buyer, order.key.owner, royaltyFee, royaltyReceipt);
        }
    }
    
    function transferBidFee(address assest,address  seller, uint256 buyingAmount, address payable royaltyReceipt, uint256 royaltyFee, address buyer) internal {
        if(assest == address(jade)){
            transferjadeFee(buyingAmount, seller, buyer, royaltyFee, royaltyReceipt);
        } else {
            transferErc20Fee(assest, buyingAmount, seller, buyer,royaltyFee, royaltyReceipt);
        }
    }
    
    function transferEthFee(uint256 amount, address  _seller, uint256 royaltyFee, address payable royaltyReceipt ) internal{
        ...
        if(remaining > 0){
            jade.transfer(_seller,remaining);
        }
    }

    function transferjadeFee(uint256 amount, address  _seller, address buyer,uint256 royaltyFee, address  royaltyReceipt ) internal{
        (uint256 protocalfee, uint256 secoundaryFee, uint256 remaining) = transferFeeView(amount, royaltyFee);
        if(protocalfee > 0){
            require(jade.transferFrom(buyer,beneficiaryAddress,protocalfee),"invalid 1");
        }
        if( (secoundaryFee > 0) && (royaltyReceipt!= address(0x00)) ){
                require(jade.transferFrom(buyer,royaltyReceipt,secoundaryFee),"invalid 2");
        }
        if(remaining > 0){
            require(jade.transferFrom(buyer,_seller,remaining),"invalid 3");
        }
    }
