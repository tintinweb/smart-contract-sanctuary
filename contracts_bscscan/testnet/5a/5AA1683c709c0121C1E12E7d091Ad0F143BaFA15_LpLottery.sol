...
contract Lottery {
    ...
    mapping(uint256 => address) allParticipantsByLotteryId;
    mapping(address => bool) allParticipants;
    uint256 private timestampForLastEntry;
    uint256 private unstakingCount;

    constructor(
        uint256 _prizeValue,
        uint256 _minimumAmountInBusd,
        address _manager
    ) {
    ...
    function participate(uint256 _amount, address _particpant) external {
        ...
        for (uint256 i = 0; i < _tickets; i++) {
            allParticipantsByLotteryId[currentTicketId] = _particpant;
            currentTicketId++;
        }
        allParticipants[_particpant] = true;
        entriesRequired = entriesRequired.sub(_tickets);
        if (entriesRequired == 0) timestampForLastEntry = block.timestamp;
    ...
    function declareWinner() external restricted returns (address) {
        ...
        return allParticipantsByLotteryId[winnerTicketNo];
    }
    ...
    function getTimestampForLatsEntry() external view returns (uint256) {
        return timestampForLastEntry;
    }

    function getUnstakingCount() external view returns (uint256) {
        return unstakingCount;
    }

    function incrementUnstakingCount(uint256 delta) external {
        require(
            allParticipants[tx.origin],
            "The caller is not participant foor this lottery"
        );
        unstakingCount = unstakingCount.add(delta);
    }
...
contract LpLottery is PancakeClass {
    ...
    uint256 public totalUnstakingCount;
    ...
    function getTimestampForLastEntry(uint256 _lotteryId)
        external
        view
        returns (uint256)
    {
        return lotteryStructs[_lotteryId].getTimestampForLatsEntry();
    }
        ...
        nonReentrant
    {
        ...
        require(
            IERC20(tokenSafeMars).transferFrom(
                msg.sender,
                address(this),
                amount
            ),
            "Safemars transfer failed"
        );
        uint256 actualAmount = IERC20(tokenSafeMars)
            .balanceOf(address(this))
            .sub(initBalance);

        uint256 tokensGetInBusdForTotalAmount = convertSafeMarsToBUSD(
            actualAmount
        );
        ...
        if (
            ...
            uint256 tokensGetInSafermars = convertBUSDToSafeMars(
                tokensGetInBusdForTotalAmount
            );
            IERC20(tokenSafeMars).transfer(msg.sender, tokensGetInSafermars);
        }
        require(
            tokensGetInBusdForTotalAmount >=
                lotteryStructs[_lotteryId].getMinimumAmountInBusd(),
        ...
        uint256 entryFee = tokensGetInBusdForTotalAmount.mul(4).div(100);
        //uint256 tokensGetInBusd = convertSafeMarsToBUSD(entryFee);
        lotteryStructs[_lotteryId].participate(entryFee, msg.sender);

        uint256 stakingAmount = tokensGetInBusdForTotalAmount.sub(entryFee);
        stakeInBUSD(stakingAmount, msg.sender);
    }
    ...
    function exit(uint256 _lotteryId) public nonReentrant {
        ...
        totalUnstakingCount = totalUnstakingCount.add(1);
        lotteryStructs[_lotteryId].incrementUnstakingCount(1);
    }

    function getUnstakingCountByLottery(uint256 _lotteryId)
        external
        view
        returns (uint256)
    {
        return lotteryStructs[_lotteryId].getUnstakingCount();
    }
