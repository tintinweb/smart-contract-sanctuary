...
contract DSMerchNFT is ERC721URIStorage, ReentrancyGuard, Ownable {
    ...
    constructor() ERC721("Diamond Boyz Merch NFT", "DSNFT") {}
    
    function createToken(
        uint256 numberOfTokens,
        string calldata tokenURI
    ) external onlyOwner {
        require(numberOfTokens <= 50, "DSNFT: number of tokens should be less or equal 50 items by one call");
        require(numberOfTokens > 0, "DSNFT: number of tokens should not be zero");
    ...
    function burnToken(uint256 tokenId) external onlyOwner {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "DSNFT: caller is not owner nor approved");
        _burn(tokenId);
    ...
    function initialTicketSale(
        ...
        require(!tokenIdToSold[tokenId], "DS: This tokenId is already sold from initial ticket sale");
        require(amount >= getInitialPrice(), "DS: Please submit the asking price in order to complete the purchase");
        require(IERC20(dbzToken).allowance(_msgSender(), address(this)) == amount, "DS: marketplace should be approved as a spender");

        IERC20(dbzToken).safeTransferFrom(_msgSender(), owner(), amount);
    ...
    modifier whenNotPaused() {
        require(!paused(), "DS: sale is not started yet or token minting is paused");
        _;
    }

    modifier whenPaused() {
        require(paused(), "DS: sale is already started or token minting is not paused");
        _;
    }
