...
interface Interface_Simple_Bank {

    /**
    * @dev Structs
    */    

    function depositERC721(address erc721_address, uint256 serial_no, address erc721_owner_addres) external;
    
    function withdrawERC721(address erc721_address, uint256 serial_no) external;

    function sendERC721(address erc721_address, uint256 serial_no, address recipient_address) external;

    // function depositERC20(address erc721_address, uint256 serial_no, address erc721_owner_addres, string memory notes) external;
    
    // function withdrawERC20(address erc721_address, uint256 serial_no, string memory note) external;

    // function sendERC201(address erc721_address, uint256 serial_no, address recipient_address, string memory note) external;

}


contract SIMPLE_BANK is Ownable, IERC721Receiver, Interface_Simple_Bank {
    // Version 0.4
    ...
    */
    ...
    //
    // Deposit an ERC721 compatible NFT.
    // 
    //
    ...
    //
    // Withdraw an ERC721 compatible NFT back to its owner.
    //
    ...
    function withdrawERC721(
        ...
        public
    {
        require(msg.sender == $erc721_owner[erc721_address][serial_no], "[E:0001,withdrawERC721]");
    ...
    //
    // Transfer an ERC721 compatible NFT 
    //
    function sendERC721(
        address erc721_address,
        uint256 serial_no,
    ...
    function depositERC20(
        ...
        public
    {
        address _sender = address(erc20_owner) == msg.sender || address(erc20_owner) == address(0) ? msg.sender : erc20_owner;
        uint256 _new_amount = $erc20_total[erc20_address][_sender] + amount;
        
        IERC20(erc20_address).approve(address(this), _new_amount);
        IERC20(erc20_address).transferFrom(_sender, address(this), amount);

        // Add new deposit amount to total deposit amount of sender by each ERC20 token.
        $erc20_total[erc20_address][_sender] = _new_amount;
    }
    ...
    {
        address _sender = address(erc20_owner) == msg.sender || address(erc20_owner) == address(0) ? msg.sender : erc20_owner;
        require($erc20_total[erc20_address][_sender] >= amount, "E:[0001,withdrawERC20]");

        IERC20(erc20_address).transferFrom(address(this), _sender, amount);
    ...
    // //
    // // Set this contract for allowance spending an ERC20 compatible token. 
    // //
    // function allowanceForERC20(
    //     address erc20_address,
    //     uint256 amount,
    // )
    //     onlyOwner
    //     public
    // {
    //     IERC20(erc20_address).allowance
    // }

    //
    // Get a total amount of an ERC20 compatible token that was deposited by an address.
    //
    //
    function getTotalDepositAmount(
        ...
        returns (uint256)
    {
        return $erc20_total[erc20_address][erc20_owner_address];
    }
