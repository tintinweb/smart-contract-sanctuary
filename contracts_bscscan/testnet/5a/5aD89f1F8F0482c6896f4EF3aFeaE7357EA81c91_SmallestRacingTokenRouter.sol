...
contract SmallestRacingTokenRouter is ReentrancyGuard,Ownable,IRacingTokenRouter {
    ...
    mapping (uint32 => mapping(address => bool)) whiteListPlayers; 
    uint public minAndMaxParticipationFeeAmount = 0.5 ether;
    uint public maxDepositAmount = 1 ether;    
    uint256 public platformFee = 10;
    uint32 public totalParticipants = 2;
    ...
    address public racingTokenFactory = 0x34f4A0c47835fE34f90A0C5Fc7285D2D31eCB8AA;
    ...
    uint256 public maxLenghtRace = 5 minutes; // 5 minutes
    IERC20 public weth = IERC20(wethAddress);
    IRandomNumberGenerator public randomGenerator;
    uint32 private testWinnerCount;
    ...
    function startRace(uint32 _raceId,address[] calldata whiteList) external override onlyOwner()  {
      require(race[currentRaceId].status != Status.Open,"You can't start the race"); 
      require(whiteList.length == totalParticipants,"No less or more participants");
      require(race[_raceId].status == Status.Pending,"Not time to start race");
      
      currentRaceId=_raceId;
        for (uint i = 0; i < totalParticipants; i++) {
            whiteListPlayers[_raceId][whiteList[i]] = true;
        }
    ...
    function joinRace(uint32 _raceId,uint64[] calldata _raceTicket) external override payable nonReentrant {
        require(whiteListPlayers[_raceId][msg.sender] == true,"You are not on the whitelist");
    ...
    function getWhiteListPlayer(uint32 _raceId,address player) public view returns(bool) {
        return whiteListPlayers[_raceId][player];
    }
    ...
    function getRaceEndTime(uint32 _raceId) public view returns(uint256) {
        return block.timestamp - race[_raceId].endTime;
    }
