...
contract TokenSale is ReentrancyGuard {
  ...
  // The block number when TokenSale starts
  uint256 public startTime;
  // The block number when TokenSale ends
  ...
  // max deposit amount for per wallet
  uint256 public depositLimitPerWallet;
  ...
  constructor(
      ...
      uint256 _depositLimitPerWallet,
      ...
      totalAmount = 0;
      depositLimitPerWallet = _depositLimitPerWallet;
  ...
  function setDepositLimitPerWallet(uint256 _depositLimitPerWallet) public onlyAdmin {
    require (block.timestamp < startTime, 'no');
    depositLimitPerWallet= _depositLimitPerWallet;
  }

  function deposit(uint256 _amount) public {
    require (block.timestamp > startTime && block.timestamp < endTime, 'not ijo time');
    ...
    if (depositLimitPerWallet > 0) {
      require (userInfo[msg.sender].amount <= depositLimitPerWallet, 'exceeded max deposit amount');
    }
  ...
  // get the amount of TokenSale token you will get
  ...
  function setLiquidityIsCreated(bool _liquidityIsCreated) public onlyAdmin {
