...
contract SpintopToken is Ownable, BEP20Pausable, Blacklist {
    ...
    uint256 private _mintingAmount1 = 40000000000000000000000000;  //40million tokens
    uint256 private _mintingAmount2 = 2000000000000000000000000;
    uint256 private _mintingAmount3 = 1000000000000000000000000;
    uint256 private _mintingAmount4 = 1000000000000000000000000;
    uint256 private _mintingAmount5 = 1000000000000000000000000;
    uint256 private _mintingAmount6 = 83000000000000000000000; //83000 tokens

    ////* Addresses to mint initial token supply */////////
    //address address1 = 0xd93365a477F4a35D4F59Ff6472071429ad80F21c; //Staking&Liquidity
    //address address2 = 0xE6d8dd93c0FAa9263FD3569D6c5C1b58796b3824; //Treasury
    //address address3 = 0xC1193acc76F22cEE3153498d57959148cdeB1CBd; //Marketing
    //address address4 = 0x6407AfFBeCe77A5cbbDBA51dC9D079Cdcb6Ad43e; //Team
    //address address5 = 0x00a2753508abe1d45A967A73ce11c59C29914911; //IDO
    //address address6 = 0x98bd91a8bE0f451f0A06f88Dd06f85D192042173; //Seed Round
    //address address7 = 0xF0a693994b40a645DD99D83844BEBc29370e6126; //Initial Liquidity
    //address address8 = 0x5C7FfB85031E025Ba80EB450dCdf6fa1e536F5aC; //Airdrop&Bounty
    
     // Testing Addresses //
     address address1 = 0xEa2ED5Db0E74f198e56ceD89a014867a59D15C23;
     address address2 = 0x5FC61B62a957646C1cF01f21aFeA57BF63e3C8AE;
     address address3 = 0x85CbC73f02f69a3A05D1dCB3879F0CbFefA755Bf; 
     address address4 = 0x8A7F0BB09Ef723c582375211ff2aE63B5fd8a8C9;
     address address5 = 0x78b2c40ed1a57c4f0307851F50a21f59227723a9;
     address address6 = 0x09F12B460360AAbC8dBD26cb42ce326Cf44B9144;

    constructor() Ownable() BEP20('Spintop-Mock','SPIN10')
    {
        _setupDecimals(numDecimals);
        setupMintingDates();
        _mint(address1, _mintingAmount1); //40milliontokens minted.
        ...
        _circulatingSupply = _mintingAmount1 + _mintingAmount2 + _mintingAmount3 + _mintingAmount4 +_mintingAmount5 + _mintingAmount6;
    }
    ...
    /// @dev Setups minting dates and amounts, will be called only on initialization
        ...
        uint256 nextMintingAt = 1631722020; // 15 Sep 2021 19:07:00  MODIFY HERE
        for (uint8 i = 1; i <= 36; i++) {
            _mintingDates[i] = nextMintingAt;
            nextMintingAt += 30 days;
            if (i==1){
                uint256 mintingAmount = 44208000000000000000000000 ;
                _mintingAmounts[i] = mintingAmount;
                _remainingMintingAmount += mintingAmount;
            }
            if(i>1 && i<4){
                uint256 mintingAmount1 = 39208000000000000000000000;
                _mintingAmounts[i] = mintingAmount1;
                _remainingMintingAmount += mintingAmount1;
            }
            if(i<=4 && i<6){
                uint256 mintingAmount2 = 29208000000000000000000000;
                _mintingAmounts[i] = mintingAmount2;
                _remainingMintingAmount += mintingAmount2;
            }
            if (i >=6) {
                uint256 mintingAmount3 = 25250000000000000000000000;
                _mintingAmounts[i] = mintingAmount3;
                _remainingMintingAmount += mintingAmount3;
        }
            if (i==36){
                uint256 mintingAmount4 = 14417000000000000000000000;
                _mintingAmounts[i] = mintingAmount4;
                _remainingMintingAmount += mintingAmount4;
        }
    }
        }
        ...
        function mint(address account) public onlyOwner {
            ...
            require(_latestMintRound < 37, "Minting is over");
            _mint(account, _mintingAmounts[currentMintRound()]);
