...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    mapping (address => bool) public tokenAcceptable;
    ...
    function updateUniswapV2RouterTracker(address newAddress) public override onlyOwner {
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }


    function updateTokenOf(address account, address token) public override onlyOwner{
        require(tokenAcceptable[token], "Token is not acceptable");
        tokenOf[account] = token;
    }

    function updateTokenAcceptability(address token, bool value) public override onlyOwner{
        tokenAcceptable[token] = value;
    }

    function isTokenAcceptable(address token) public view override returns(bool){
        return tokenAcceptable[token];
    }
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            bool success;
            if (tokenOf[user] != address(0)) {
                success = swapETHForDivToken(tokenOf[user], _withdrawableDividend, user);
            } else {
    ...
    function swapETHForDivToken(address token, uint256 tokenAmount, address user) private returns(bool){
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor(address token) DividendPayingToken("Dividen_Tracker", "Dividend_Tracker") {
        claimWait = 3600;
    ...
    function sendTokens(address _token, uint256 _amount) public onlyOperator {
...
interface DividendPayingTokenInterface {
    ...
    function updateUniswapV2RouterTracker(address newAddress) external;

    function updateTokenOf(address account, address token) external;

    function updateTokenAcceptability(address token, bool value) external;

    function isTokenAcceptable(address token) external view returns(bool);
}
