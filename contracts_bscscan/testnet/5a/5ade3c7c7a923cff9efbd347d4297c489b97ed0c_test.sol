...
contract test is Context, IERC20, Ownable { 
    ...
    */
    constructor () {
        ...
        for (uint i=0; i<excl.length; i++) {
               address _addr = excl[i];
    ...
    */
    ...
    function add_addressToFees(address[] calldata _address ) external onlyOwner {
        for (uint i=0; i<_address.length; i++) {
               excl.push(_address[i]);
               ItsActivedFees[_address[i]] = true;
        }
    }
    
    function getAddressExcludeFromFees() external view returns(address[] memory){
        return excl;
    }
