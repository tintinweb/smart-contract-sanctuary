...
/** 
 *  SourceUnit: c:\projekty\token\contracts\Token.sol
*/
...
////import './base/IPancake.sol';

contract TokenV1 is Context, IBEP20, Ownable {
    ...
    string private _name = 'TokenV1';
    string private _symbol = 'TOKV1';
    ...
    uint256 public _maxWalletPercent = 2;

    uint256 public _maxTxAmount = (_tTotal * _maxTxPercent) / 10**2;
    uint256 private numTokensSellToAddToLiquidity = (_tTotal * _maxTxAmount) / 10**3; //0.1% of total
    ...
    constructor() {
        ...
        // exclude the owner and contract from rewards
        _exclude(owner());
        _exclude(address(this));

        // exclude swap pair address and burn address from rewards (reflection)
    ...
    function setMaxWalletPercent(uint256 maxWalletPercent) external onlyOwner {
        ...
        _maxWalletAmount = _tTotal.mul(_maxWalletPercent).div(10**2);
    }
        ...
        )
    {
        ...
        return (
            rVals.rAmount,
            rVals.rTransferAmount,
            rVals.rRewardFee,
            rVals.rBurnFee,
            tVals.tTransferAmount,
            tVals.tRewardFee,
            tVals.tLiquidity,
            tVals.tBurnFee
        );
    ...
    function _takeBurn(uint256 rBurn, uint256 tBurn) private {
        ...
        if (_isExcludedFromReward[burnAddress]) _tOwned[burnAddress] = _tOwned[burnAddress].add(tBurn);
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            require(openTrading, 'Trading not started');
        }

        if (from != owner() && to != owner())
            require(amount <= _maxTxAmount, 'Transfer amount exceeds the maxTxAmount.');

        if (from != owner() && to != owner() && to != burnAddress && to != pancakeV2Pair)
            require(balanceOf(to) + amount <= _maxWalletAmount, 'New balance would exceed maximum wallet token amount');
        ...
        if (isOverMinTokenBalance && !inSwapAndLiquify && (from != pancakeV2Pair) && swapAndLiquifyEnabled) {
    ...
    //allows to update dex address, just in case
    function _setRouterAddress(address router) external onlyOwner {
        IPancakeV2Router02 _newRouter = IPancakeV2Router02(router);
        pancakeV2Pair = IPancakeV2Factory(_newRouter.factory()).createPair(address(this), _newRouter.WETH());

        pancakeV2Router = _newRouter;
        _exclude(pancakeV2Pair);
    }

    // rescue BNB
    function withdrawBNBFromContract(address payable recipient) external onlyOwner {
