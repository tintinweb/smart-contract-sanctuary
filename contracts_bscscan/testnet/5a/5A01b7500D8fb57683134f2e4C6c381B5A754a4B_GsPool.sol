...
contract GsPool is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using SafeERC20 for IGSB;
    
    uint256 public constant MAX_MINT_AMOUNT = 30000000e18; // 该池最大挖币数，挖完为止，总量的10%，3000万
    uint256 public constant PER_DAY = 7500e18;
    uint256 public hasMintAmount; // 已挖出多少GSB
    uint256 public gsbPerBlock; // 每区块产币数
    ...
    constructor(IGSB _gsb, IERC20 _gs) {
        gsb = _gsb;
        gs = _gs;
        gsbPerBlock = PER_DAY / 28800;
    ...
    function setGsbPerBlock(uint256 _newPerBlock) public onlyOwner {
        update();
        gsbPerBlock = _newPerBlock;
    }
    ...
    function pending(address _user) public view returns (uint256) {
        ...
        if (user.amount > 0) {
            ...
            return user.amount.mul(_accGsbPerShare).div(1e12).sub(user.rewardDebt);
        }
        return 0;
    }
    
    function deposit(uint256 _amount) public notPause {
        require(_amount > 0, "amount is error");
        ...
        gs.safeTransferFrom(_user, address(this), _amount);
