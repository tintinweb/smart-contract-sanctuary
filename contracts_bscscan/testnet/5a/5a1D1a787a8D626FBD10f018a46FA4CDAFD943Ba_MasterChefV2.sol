...
// will be transferred to a governance smart contract once LIFE is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of LIFEs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. LIFEs to distribute per block.
    ...
    // The LIFE TOKEN!
    LifeToken public life;
    // Dev address.
    address public devaddr;
    // LIFE tokens created per block.
    uint256 public lifePerBlock;
    // Bonus muliplier for early life makers.
    ...
    // The block number when LIFE mining starts.
    ...
    // View function to see pending LIFEs on frontend.
    function pendingLife(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 lifeReward = multiplier
            .mul(lifePerBlock)
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
            accLifePerShare = accLifePerShare.add(
                lifeReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accLifePerShare = pool.accLifePerShare.add(
            lifeReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for LIFE allocation.
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accLifePerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeLifeTransfer(msg.sender, pending);
    ...
    function safeLifeTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > lifeBal) {
            transferSuccess = life.transfer(_to, lifeBal);
        } else {
            transferSuccess = life.transfer(_to, _amount);
        }
