...
contract SupeNFTMintGold is Ownable {
    ...
    uint256 public supe_threshold = 100 * 10**18;
    uint256 public nft_threshold = 3;
    ...
    mapping(address => mapping(address=>uint256)) public wait_claimed;
    mapping(address => mapping(address=>uint256)) public claimed;
    mapping(address => uint256) public total_accumulate;
    ...
    event InitializeParams(uint256 _interval, uint256 _supe_threshold, uint256 _nft_threshold);
    event StakeOne(address _user, uint256 _asset_id, uint256 _start_ts, uint256 _end_ts);
    ...
    function initialize_params(uint256 _interval, uint256 _supe_threshold, uint256 _nft_threshold) external onlyFactoryOrOwner{
        interval = _interval;
        supe_threshold = _supe_threshold;
        nft_threshold = _nft_threshold;
        emit InitializeParams(_interval, _supe_threshold, _nft_threshold);
    }
    ...
    function stake_one(uint256 _asset_id) public checkPaused{
        ...
        require(stakes[msg.sender][_asset_id].asset_id == 0, "asset has been staked");
        nft_token.safeTransferFrom(msg.sender, dev_pool, _asset_id);
        stakes[msg.sender][_asset_id] = StakeInfo({
            asset_id: _asset_id,
            start_ts: block.timestamp,
            end_ts: block.timestamp + interval
        });
        balances[msg.sender] = balances[msg.sender].add(1);
        emit StakeOne(msg.sender, _asset_id, block.timestamp, block.timestamp + interval);
    }
    ...
    function exit_one(uint256 _asset_id) public checkPaused{
        require(stakes[msg.sender][_asset_id].asset_id > 0, "asset has been exited");
        require(stakes[msg.sender][_asset_id].end_ts <= block.timestamp, "asset not end");
        nft_token.safeTransferFrom(dev_pool, msg.sender, _asset_id);
        uint256 _tlp;
        uint256 _frog;
        (_tlp, _frog) = _get_reward(msg.sender, _asset_id);
        if(_tlp > 0){
            wait_claimed[msg.sender][address(tlp_token)] = wait_claimed[msg.sender][address(tlp_token)].add(_tlp);
            total_accumulate[address(tlp_token)] = total_accumulate[address(tlp_token)].add(_tlp);
        }
        if(_frog > 0){
            wait_claimed[msg.sender][address(frog_token)] = wait_claimed[msg.sender][address(frog_token)].add(_frog);
            total_accumulate[address(frog_token)] = total_accumulate[address(frog_token)].add(_frog);
        }
        if(balances[msg.sender] >= 1){
            balances[msg.sender] = balances[msg.sender].sub(1);
        }
        emit ExitOne(msg.sender, _asset_id, stakes[msg.sender][_asset_id].start_ts, stakes[msg.sender][_asset_id].end_ts, _tlp, _frog);
        delete(stakes[msg.sender][_asset_id]);
    }
    function exit_stake(uint256[] memory ids) public {
        for (uint256 i = 0; i < ids.length; ++i) {
            exit_one(ids[i]);
            stake_one(ids[i]);
        }
    }
    function exit_stake_one(uint256 _asset_id) public checkPaused{
        exit_one(_asset_id);
        stake_one(_asset_id);
    }
    function _get_reward(address _account, uint256 _asset_id) internal view returns (uint256, uint256){
        uint256 _tlp;
        uint256 _frog;
        uint256 seed = uint256(keccak256(abi.encode(_account, _asset_id, block.timestamp, blockhash(block.number))));
        if(seed % 100 < 20){
            _frog = 10 * 10 ** 18;
        }
        _tlp = ((seed % 10001).add(10000)).mul(10 ** 18);
        return (_tlp, _frog);
    }
    function claim() public checkClaimEnable{
        uint256 _tlp = wait_claimed[msg.sender][address(tlp_token)];
        uint256 _frog = wait_claimed[msg.sender][address(frog_token)];
        require(_tlp > 0 || _frog > 0, "tlp and frog == 0");
        if(_tlp > 0){
            IERC20(tlp_token).mint(msg.sender, _tlp);
            wait_claimed[msg.sender][address(tlp_token)] = 0;
            claimed[msg.sender][address(tlp_token)] = claimed[msg.sender][address(tlp_token)].add(_tlp);
            total_claimed[address(tlp_token)] = total_claimed[address(tlp_token)].add(_tlp);
        }
        if(_frog > 0){
            //IERC20(frog_token).safeTransfer(msg.sender, _frog);
            IERC20(frog_token).mint(msg.sender, _frog);
            if(_frog.div(10) > 0){
                IERC20(frog_token).mint(mint_frog_pool, _frog.div(10));
            }
            wait_claimed[msg.sender][address(frog_token)] = 0;
            claimed[msg.sender][address(frog_token)] = claimed[msg.sender][address(frog_token)].add(_frog);
            total_claimed[address(frog_token)] = total_claimed[address(frog_token)].add(_frog);
        }
        emit Claimed(msg.sender, _tlp, _frog);
    }
