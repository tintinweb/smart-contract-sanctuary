...
contract Pledge is Ownable {
    ...
    modifier checkHolder{
        ...
        if (airDropInfo.status) {
        ...
        } else if (temp > 30000 && airDropInfo.round == 3) {
            airDropInfo.regTotal = 0;
            airDropInfo.status = true;
            airDropInfo.regTime = block.timestamp;
            airDropInfo.regEnd = block.timestamp + 2 days;
            airDropInfo.claimTime = block.timestamp + 2 days;
            airDropInfo.claimEnd = block.timestamp + 4 days;
            airDropInfo.round = 4;
            airDropInfo.toClaim = checkRoundTotal();
            airDropInfo.claimed = 0;
        } else if (temp > 20000 && airDropInfo.round == 2) {
            airDropInfo.regTotal = 0;
            airDropInfo.status = true;
            airDropInfo.regTime = block.timestamp;
            airDropInfo.regEnd = block.timestamp + 2 days;
            ...
            airDropInfo.toClaim = checkRoundTotal();
            airDropInfo.claimed = 0;
        } else if (temp > 10000 && airDropInfo.round == 1) {
            airDropInfo.regTotal = 0;
            airDropInfo.status = true;
            airDropInfo.regTime = block.timestamp;
            airDropInfo.regEnd = block.timestamp + 2 days;
    ...
    function coutingDebt() public view returns (uint debt_){
        debt_ = totalPower > 0 ? (rate * 4 / 10) * (block.timestamp - debt.timestamp) * Acc / totalPower + debt.debted : 0 + debt.debted;
    ...
    function activateInvite(address addr_, bytes32 r, bytes32 s, uint8 v) isStart checkTime external {
    ...
    function claimCommunity(uint total_, uint amount_, uint timestamp_, bytes32 r, bytes32 s, uint8 v) external  returns (uint out){
        ...
        SpeToken.transfer(msg.sender, amount_);
         if (amount_ > userClaim[msg.sender].quota) {
            ...
            userClaim[msg.sender].comTotal += userClaim[msg.sender].quota;
            require(userClaim[msg.sender].comTotal <= total_, '20');
            out = userClaim[msg.sender].quota;
            emit ClaimCommunity(msg.sender, userClaim[msg.sender].quota);
        } else {
            SpeToken.transfer(msg.sender, amount_);
    ...
    function claimIDO(uint total_, uint amount_, uint timestamp_, bytes32 r, bytes32 s, uint8 v) external {
        ...
        IDOTime[msg.sender] += block.timestamp + 86000;
    ...
    function checkUserTotalReward(address addr_) public view returns (uint out_){
        out_ = calculateAllSlotRewards(addr_) + userInfo[addr_].Claimed;
    }
    ...
    function claimAirDrop() public {
        ...
        SpeToken.transfer(msg.sender, tempAmount);
        userClaim[msg.sender].regAir[airDropInfo.round] = false;
    ...
    function checkRoundTotal() public view returns (uint out) {
    ...
    function checkToClaimAir() public view returns (uint out) {
        out = airDropInfo.toClaim;
    }

    function checkClaimedAir() public view returns (uint out) {
        out = airDropInfo.claimed;
    }

    function checkUserRegStatus(address addr_, uint round_) public view returns (bool out){
        out = userClaim[addr_].regAir[round_];
    }

    function coutingPower(uint amount_, uint cycle_) public view returns (uint out){
        out = amount_ * Coe[cycle_] / 10;
    }
