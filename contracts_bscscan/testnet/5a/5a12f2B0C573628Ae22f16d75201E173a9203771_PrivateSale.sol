...
contract PrivateSale is Ownable, OwnerPausable
{
    using SafeMath for uint256;

    uint256 public tokensAmount = 200000 * 10**18;
    ...
    function updateTokenContract(address contract_) external onlyOwner{
        require(address(tokenContract) != contract_);
        tokenContract = IERC20(contract_);
    }
    ...
    function getAmountTokensByContributionValue(address sender) private view returns(uint256){
        require(_usersDeposite[sender] > 0);
    ...
    // PRESALE FUNCTIONS
    function deposit() external payable whenNotPaused returns(bool success){
        require(msg.value > 0);
    ...
    function finilize() external onlyOwner{
        require(!finilized, 'Already finilized');
    ...
    function reclaimTokens() external whenPaused returns(bool success){
        require(finilized, 'Private sale is not finilaze');
        ...
        tokenContract.transfer(sender, tokens);
