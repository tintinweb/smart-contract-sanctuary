  ...
  */
  function calculateShareOfShareHolder(address shareholderAddress, bool checkReward, bool claimReward) private returns (uint256, uint256) {
    ...
    for (uint256 i = 0; i < _shareHoldersTxShares[shareholderAddress].length; i++) {
      ...
      uint256 blockTimestampLast = getBlockTimestamp();
      if (txDateTime.add(5 minutes) < blockTimestampLast) {
        bool rewardCheck = checkReward ? rewardDateTime.add(5 minutes) < blockTimestampLast : true;
  ...
  */
  function distributeLongDividend(address shareholder, uint256 longTermHolds) internal
  {
      if (longTermHolds > 0 && _snapshotLongTotalShares > 0 && _tSupply > 0) {
        uint256 busdBalance = BUSD.balanceOf(address(this)).mul(
          _tSupply.div(_tSupply.add(_mSupply))
        );
        if (busdBalance > 0) {
          uint256 perc = longTermHolds.div(_snapshotLongTotalShares).mul(10 ** 4);
          BUSD.transfer(shareholder, (busdBalance.mul(perc).div(10 ** 4)));
        }
      }
  }
  ...
  */
  function distributeMidDividend(address shareholder, uint256 midTermHolds) internal {
      if (midTermHolds > 0 && _snapshotMidTotalShares > 0 && _mSupply > 0) {
        uint256 busdBalance = BUSD.balanceOf(address(this)).mul(
          _mSupply.div(_mSupply.add(_tSupply))
        );
        if (busdBalance > 0) {
          uint256 perc = midTermHolds.div(_snapshotMidTotalShares).mul(10 ** 4);
          BUSD.transfer(shareholder, (busdBalance.mul(perc).div(10 ** 4)));
        }
      }
  }
