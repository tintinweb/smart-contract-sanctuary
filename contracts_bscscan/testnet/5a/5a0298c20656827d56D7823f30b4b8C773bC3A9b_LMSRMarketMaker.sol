...
contract MarketMaker is Ownable, ERC20, ERC1155TokenReceiver {
        ...
        atStage(Stage.Paused)
    {
        ...
        require(collateralToken.transferFrom(msg.sender, address(this), liquidityChange) && collateralToken.approve(address(pmSystem), liquidityChange));
        ...
        atStage(Stage.Paused)
    {
        ...
        withdrawFees();
        // Either add or subtract funding based off whether the fundingChange parameter is negative or positive
        uint liquidityChange = lpsChange;
        if (totalSupply() != 0) {
            liquidityChange = lpsChange.mul(funding).div(totalSupply());
        }
        mergePositionsThroughAllConditions(liquidityChange);
        funding = funding.sub(liquidityChange);
        _burn(msg.sender, lpsChange);
        require(collateralToken.transfer(owner(), liquidityChange));
