...
contract SUCSESSLIFE {
    ...
    function updateX2Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        require(users[userAddress].activeX2Levels[level]," User Level not activated");
          ...
          emit UserIncome(userAddress,referrerAddress,levelPrice[1],0,1,"Level Income");
              ...
              if(users[referrerAddress].activeX2Levels[level]) {
    ...
    function updateX3Referrer(address userAddress, address referrerAddress,uint8 matrix, uint8 level) private {
        ...
        if(users[referrerAddress].x3Matrix[matrix][level].referrals.length < member_count) {
          ...
          emit NewUserPlace(userAddress, referrerAddress, matrix, level, uint8(users[referrerAddress].x3Matrix[matrix][level].referrals.length));
            ...
            if(level<2 && users[referrerAddress]._holdMatrixAmount[matrix][level]>=matrixPrice[matrix][level+1]&&users[referrerAddress].x3Matrix[matrix][level].referrals.length==member_count)
            {
                    ...
                    busdToken.transfer(referrerAddress,users[referrerAddress]._holdMatrixAmount[matrix][level]);
                    emit UserIncome(referrerAddress,referrerAddress,users[referrerAddress]._holdMatrixAmount[matrix][level],matrix,level,"Global Matrix Income");
    ...
    function autoUpgrade(address _user,uint8 matrix, uint8 level) private {
        users[_user].activeX3Levels[matrix][level] = true;
        address freeX3Referrer = findFreeX3Referrer(matrix,level);
        users[_user].x3Matrix[matrix][level].currentReferrer = freeX3Referrer;
        updateX3Referrer(_user, freeX3Referrer,matrix, level);
        
        emit Upgrade(_user, matrix, level);
    }
