...
// File: contracts/libs/ISwampWolfReferral.sol
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 lpTotal;                // lp total in this pool.
    }

    // The SWAMPWOLF TOKEN!
    SwampWolfToken public swampWolf;
    // Dev address.
    ...
    // Bonus muliplier for early swampWolf makers.
    ...
    // DarkAgeOfBeast contract address.
    IDarkAgeOfBeast public darkAgeOfBeast;
    // SwampWolf referral contract address.
     ...
     *
     */
    function swampWolfPerBlock() public view returns (uint256) {
        // Block till update tokens per block ( 60sec x 60 min / 3sec : time per block  )
        uint256 blockToUpdate = 60 * 60 / 3;
        // Block since startBlock
        uint256 blockSinceStart = block.number - startBlock;
        // Block before deflationary
        uint256 blockBeforeDeflationary = blockToUpdate * 243;
        ...
        uint256 swampWolfPerBlockCalc = newTokenPerBlock.add(1);
     ...
     */
    function add(uint256 _allocPoint, IBEP20 _lpToken,  bool _withUpdate) public onlyAdmin {
        if (_withUpdate) {
            massUpdatePools();
        }
        IPancakePair lpPair = IPancakePair(address(_lpToken));
        address token0Address = lpPair.token0();
        uint16 _depositFeeBP = depositFeeBp;
        uint256 _harvestInterval = harvestInterval();
        if(token0Address != address(0) && token0Address == address(swampWolf) ){

            _depositFeeBP = 0;
            _harvestInterval = _harvestInterval.div(2);
            swampWolf.setNewTradingPair(address(_lpToken), true);
        }
    ...
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate) public onlyAdmin {
        ...
        poolInfo[_pid].depositFeeBP = depositFeeBp;
        uint256 _harvestInterval = harvestInterval();
        if(swampWolf.isSwapPair(address(poolInfo[_pid].lpToken))){
            poolInfo[_pid].depositFeeBP = 0;
            _harvestInterval = _harvestInterval.div(2);
        }
    ...
    */
    function pendingSwampWolf(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 swampWolfReward = swampWolfPerBlock().mul(pool.allocPoint).div(totalAllocPoint);
            accSwampWolfPerShare = accSwampWolfPerShare.add(swampWolfReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 swampWolfReward = swampWolfPerBlock().mul(pool.allocPoint).div(totalAllocPoint);
        swampWolf.mint(address(this), swampWolfReward);
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        payOrLockupPendingSwampWolf(_pid);
        if (_amount > 0) {
            ...
            if (!swampWolf.isSwapPair(address(pool.lpToken))) {
                ...
                pool.lpToken.safeTransfer(devAddress, devPart);
                pool.lpToken.safeTransfer(questAddress, questPart);
                pool.lpToken.safeTransfer(marketingAddress, marketingPart);
                if (feeRate != 0){
                    uint256 feePart = depositFeeBp.mul(feeRate).div(10000);
                    pool.lpToken.safeTransfer(feeAddress, feePart);
                }
     ...
     *
     */
    function payOrLockupPendingSwampWolf(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                safePayReferralCommission(msg.sender, totalRewards);
    ...
    /**
     * @dev Safe pay referral commission to multilevel referrers who referred this user.
     *
     */
    function safePayReferralCommission(address _user, uint256 _pending) internal {
        if (address(swampWolfReferral) != address(0) && referralCommissionRate > 0) {
            // The total commission amount
            uint256 totalCommissionAmount = _pending.mul(referralCommissionRate).div(10000);
            // The referrer commission rate in basis point
            uint16 commission0 = 5000; // 50% of totalCommissionAmount
            uint16 commission1 = 3000; // 30% of totalCommissionAmount
            uint16 commission2 = 2000; // 20% of totalCommissionAmount
            // The referrer list
            address referrer0 = swampWolfReferral.getReferrer(_user);       // direct referrer
            address referrer1 = swampWolfReferral.getReferrer(referrer0);   // referrer of the referrer
            address referrer2 = swampWolfReferral.getReferrer(referrer1);   // referrer of the referrer of the referrer
            // The commission amount by referrer
            uint256 commissionAmount0 = totalCommissionAmount.mul(commission0).div(10000);
            uint256 commissionAmount1 = totalCommissionAmount.mul(commission1).div(10000);
            uint256 commissionAmount2 = totalCommissionAmount.mul(commission2).div(10000);
            // Check for balance just in case if rounding error causes pool to not have enough SWAMPWOLF.
            uint256 swampWolfBal = swampWolf.balanceOf(address(this));
            if (referrer0 != address(0) && commissionAmount0 > 0) {
                if (commissionAmount0 > swampWolfBal) {
                    swampWolf.transfer(referrer0, swampWolfBal);
                } else {
                    swampWolf.transfer(referrer0, commissionAmount0);
                }
                swampWolfReferral.recordReferralCommission(referrer0, commissionAmount0);
                emit ReferralCommissionPaid(_user, referrer0, commissionAmount0);
            }
            if (referrer1 != address(0) && commissionAmount1 > 0) {
                if (commissionAmount1 > swampWolfBal) {
                    swampWolf.transfer(referrer1, swampWolfBal);
                } else {
                    swampWolf.transfer(referrer1, commissionAmount1);
                }
                swampWolfReferral.recordReferralCommission(referrer1, commissionAmount1);
                emit ReferralCommissionPaid(_user, referrer1, commissionAmount1);
            }
            if (referrer2 != address(0) && commissionAmount2 > 0) {
                if (commissionAmount2 > swampWolfBal) {
                    swampWolf.transfer(referrer2, swampWolfBal);
                } else {
                    swampWolf.transfer(referrer2, commissionAmount2);
                }
                swampWolfReferral.recordReferralCommission(referrer2, commissionAmount2);
                emit ReferralCommissionPaid(_user, referrer2, commissionAmount2);
            }
        }
    }
     ...
     * Must only be called by the Admin.
     */
    function setDevAddress(address _devAddress) public onlyAdmin {
        require(_devAddress != address(0), "setDevAddress: can't be ZERO address");
    ...
    function setMarketingAddress(address _marketingAddress) public onlyAdmin {
        require(_marketingAddress != address(0), "setMarketingAddress: can't be ZERO address");
        feeAddress = _marketingAddress;
    }
     ...
     * Must only be called by the Admin.
     */
    function setQuestAddress(address _questAddress) public onlyAdmin {
        require(_questAddress != address(0), "setQuestAddress: can't be ZERO address");
        questAddress = _questAddress;
    }
     ...
     */
    function setSwampWolfReferral(ISwampWolfReferral _swampWolfReferral) public onlyOwner {
     ...
     *
     * Must only be called by the Admin.
     * Mus be less than MAXIMUM_REFERRAL_COMMISSION_RATE.
     */
    function setReferralCommissionRate(uint16 _referralCommissionRate) public onlyAdmin{
     ...
     */
    function setDepositFeeBp(uint16 _depositFeeBp) public onlyAdmin{
    ...
    /**
     * @dev Update all commission rates in basis points
     *
     * Requirements
     *
     * Must only be called by the Admin.
     ...
     */
    function setFeeCommissionRates(uint16 _feeRate, uint16 _devRate, uint16 _questRate, uint16 _marketingRate) public onlyAdmin {
    ...
    function farmingStart() public onlyAdmin{
        require(startTime > block.timestamp && startBlock > block.number , "Farm already started !");
     ...
     */
    function transferAdmin(address newAdmin) public onlyAdmin {
        require(newAdmin != address(0), "transferAdmin : can't be ZERO address");
        emit AdminTransferred(_admin, newAdmin);
        _admin = newAdmin;
    }
