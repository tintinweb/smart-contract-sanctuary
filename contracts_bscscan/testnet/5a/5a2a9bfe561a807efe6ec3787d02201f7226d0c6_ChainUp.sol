...
abstract contract Ownable is Context {
    ...
    function owner_renounce() public virtual onlyOwner {
        emit owner_transferred(_owner, address(0));
        _owner = address(0);
    }

    function owner_transfer(address newOwner) public virtual onlyOwner {
...
//- This is "ChainUp Contract"
contract ChainUp is Context, Ownable{
    ...
    //- Login ChainUp
    function LoginChainUp(address _loginaddress) public {
        if (ChainUp_Accounts.length == 0) { revert("Not Login : Err 1"); }
    ...
    //-view returns(string memory, string memory, string memory, string memory, string memory, address)        
    function ViewChainUpAccountId(uint256 _id) public {
        if ( keccak256(abi.encodePacked(Global_UserName)) == keccak256(abi.encodePacked("")) ) {
