...
interface IPandoraERC1155 {
    ...
    function mint(address to, uint256 _id, uint256 _quantity, bytes memory _data) external;
    ...
    function totalBalance(address account, uint256[] memory ids) external view returns (uint256, uint256[] memory);
...
contract BurnRoomManager is SafeOwnable, Random {
    ...
    IPandoraERC1155 public nftToken;
    ...
    constructor(IPandoraERC1155 _nftToken, address _receiver, address _linkAccessor) Random(_linkAccessor) {
    ...
    function beginLoop(uint _rid) public {
        ...
        nftIDs[_rid][room.currentLoop] = nftToken.createBatch(uint256(-1), 0, nftTypes[_rid], "0x");
        room.openNum = 0;
    }
    ...
    function buyBlindBox(uint256 _rid, uint256 _loop, uint256 _num, address _to) external {
        ...
        require(totalBalance <= rangeInfo[_rid].length, "token alrady full");
