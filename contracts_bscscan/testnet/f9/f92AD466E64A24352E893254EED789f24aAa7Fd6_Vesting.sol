...
contract Vesting is Ownable {
    ...
    function getClaimableVestTokenByIndex(uint256 _index) public view returns (uint256 totalTokensToClaim) {
        ...
        uint32 startDate = _gmtMidnight(vest.dateStarted);
        if (block.timestamp >= (startDate + CLIFF_IN_MINUTES)) { 
            uint256 currentPeriod = _getCurrentPeriod(startDate);
    ...
    function _getCurrentPeriod(uint32 dateStartedGmtMidnight) private view returns (uint256 currentPeriod) {
        uint256 timePassed = (block.timestamp - (dateStartedGmtMidnight + CLIFF_IN_MINUTES)) / 60;
    ...
    function _getNextClaimDate(uint32 dateStarted) private view returns (uint256 nextClaimDate) {
        uint32 startDate = _gmtMidnight(dateStarted);
        uint256 currentPeriod = _getCurrentPeriod(startDate);
        nextClaimDate = (currentPeriod * CLIFF_IN_MINUTES) + (CLIFF_IN_MINUTES * 2) + startDate;
    }
