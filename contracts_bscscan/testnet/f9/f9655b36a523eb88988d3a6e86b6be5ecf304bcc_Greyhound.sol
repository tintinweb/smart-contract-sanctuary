...
//    }
    ...
    function _reflectFee(uint256 tBurn, uint256 rBurn, uint256 tFee, uint256 rFee, uint256 tCharity, uint256 rCharity) private {
        ...
        _rTotal         = _rTotal.sub(rCharity).sub(rFee);//.sub(rBurn); //?????
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _reflectFee(tBurn, rBurn, tFee, rFee, tCharity, rCharity);
   ...
   function _transferStandard(address sender, address recipient, uint256 tAmount) private {
    ...
    function burnFromContract(uint256 tAmount) public onlyOwner {
        require(tAmount <= _tTotal, "Amount must be less than supply");
        address sender = _msgSender();
