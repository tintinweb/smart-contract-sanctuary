...
contract Boardroom is ShareWrapper, ContractGuard, Operator {
    ...
    function getClaimEpoch(address _account) external view returns (uint256) {
        return directors[_account].epochTimerStart.add(rewardLockupEpochs);
    }

    function getWithdrawEpoch(address _account) external view returns (uint256) {
        return directors[_account].epochTimerStart.add(withdrawLockupEpochs);
    }
}