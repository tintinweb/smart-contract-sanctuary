...
contract testBNBividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("WISHDOGE_Dividend_Tracker", "WISHDOGE_Dividend_Tracker") {
        claimWait = 600;
    ...
    function withdrawDividend() public pure override {
        require(false, "WISHDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WISHDOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "WISHDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "WISHDOGE_Dividend_Tracker: Cannot update claimWait to same value");
...
contract testBNB is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    testBNBividendTracker public dividendTracker;
    ...
    constructor() ERC20("Doge The BNB", "DTBNB") {
        BNBRewardsFee = 8;
        liquidityFee = 3;
        extraFeeOnSell = 3; // extra fee on sell
        marketingFee = 3;
        marketingWallet = payable(0x5DA084A7b052c8F58B4e04A8aF67b11619F03244);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new testBNBividendTracker();
