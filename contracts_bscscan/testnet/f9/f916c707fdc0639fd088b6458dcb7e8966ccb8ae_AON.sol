...
library TransferHelper {
  ...
  address constant USDTAddr = address(0x846110eB11B1d0c7Ec47D02a25d60c232fa78c67);
...
contract AON is ITRC20, Owend{
    ...
    string private _name ="MUCNFT";
    string private _symbol="MUCNFT";
    uint256 private _decimals = 6;
    
    uint256 private _fee=5;
    address private _USDTAddr=address(0x846110eB11B1d0c7Ec47D02a25d60c232fa78c67);
    address private _platformAddress=address(0x78E596ff4459C007176ed6961b15724528e0B75B);
    
    constructor()public{
        token=ITRC20(_USDTAddr);
    }
    
    function buyBox(uint256 uAmount,address reciveAddress)public{
        require(uAmount>0,"price must more than the zero");
        uint256 feeAmount =uAmount.mul(_fee).div(100);
        uint256 reciveAmount=uAmount.sub(feeAmount);
        require(address(token).safeTransferFrom(msg.sender, _platformAddress, feeAmount));
        require(address(token).safeTransferFrom(msg.sender,reciveAddress,reciveAmount));
    }
    
    function setFee(uint256 fee)public onlyOwner{
        require(fee>=0,"fee more than the zero");
        require(fee<=100,"fee less than 100");
        _fee=fee;
    }
    function mint(address _to, uint256 _id, uint256 _quantity, bytes memory _data) public{
        require(_to!=address(0),"price must more than the zero");
    }
    function setPlatformAddress(address account) public onlyOwner{
    ...
    function _transfer(address _from,address _to,uint256 _value) private{
        ...
        require(_value > 0, "Transfer amount must be more than the zero");
