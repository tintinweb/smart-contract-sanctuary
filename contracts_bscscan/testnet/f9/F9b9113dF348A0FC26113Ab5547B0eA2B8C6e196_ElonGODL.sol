...
contract ElonGODL is IERC20, Ownable {
    ...
    address private _lotteryAddress = 0xA160DC6EEEd18B4Fd1bC034dA64cBDb530DABfBf; // for event transparency, not balance tracking
    uint256 private _lotteryBalance = 0;
    uint256 _lotteryContestantMinAmount = 1000000 * 10**9;
    IterableMapping.Map private _lotteryContestants;
    ...
    function _validateLotteryContestant(address account) private {
        if (account == _uniswapV2Pair || 
            _isExcluded[account] || 
            balanceOf(account) < _lotteryContestantMinAmount) 
        {
