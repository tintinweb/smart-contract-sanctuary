...
contract AKP is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public rewardBNBPcent = 3;
    uint256 public rewardBUSDPcent = 3;
    uint256 private numerator = 100;
    uint256 public trashPcent = numerator.div(200);
    ...
    function getRewardCycleBlock() public view returns (uint256) {
        ...
        uint256 eggs = _liquidityFee.sub(_liquidityPercent);
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        !(from == address(this) && to == address(pancakePair)) // swap 1 time
            ...
            uint256 liquidityPiece = contractTokenBalance.mul(liquidityPercent()).div(2*10**2);
			uint256 otherPiece = contractTokenBalance.mul(liquidityPercent()).div(2*10**2);
			uint256 rewardPiece = contractTokenBalance.sub(liquidityPiece).sub(otherPiece);
			uint256 swapBNB = rewardPiece.add(otherPiece);
			
			uint256 liquidityPortion = liquidityPercent().div(2).mul(100).div(_liquidityFee);
			...
			uint256 liquiditySlice = liquidityPortion.mul(liquified).div(ratio);
			
			// add liquidity to pancake
            Utils.addLiquidity(address(pancakeRouter), owner(), liquidityPiece, liquiditySlice);
			
			emit SwapAndLiquify(otherPiece, liquified, liquidityPiece);
        }
    }
...

{
  ...
  "libraries": {
    "contracts/TAR.sol": {
      "Utils": "0x75132d1fa536a456e772c67ce6b849cdd33ee0cd"
    }
  }
}