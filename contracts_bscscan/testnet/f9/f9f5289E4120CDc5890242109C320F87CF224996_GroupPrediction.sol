...
contract GroupPrediction is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    uint256 private ONE_HUNDRED_PERCENT = 10000;

    mapping(address => mapping(address => mapping(uint256 => DataTypes.GroupPrediction))) public predictions;
    mapping(address => mapping(uint256 => DataTypes.GroupPredictStats)) private predictStats;
     ...
     * @dev Predictions
     */
    function predict(uint256 _matchId, uint256 _option, address _token, uint256 _amount) public payable
    {
        ...
        require(_option < 3, 'option-invalid');
        ...
        emit PredictionCreated(_matchId, _token, _amount);
    }

    /**
     * @dev Calculates reward
     */
    function calculateReward(uint256 _matchId, address _user, address _token)
    public
    view
    returns (uint256 _reward)
    {
        ...
        uint256 _winSide = _score.firstTeam == _score.secondTeam ? 0 : _score.firstTeam > _score.secondTeam ? 1 : 2;
        uint256 _total = 0;
        for (uint256 i = 0; i < 3; i++) {
            _total += predictStats[_token][_matchId].predictionAmount[i];
        }
        _reward = predictions[_token][_user][_matchId].predictionAmount[_winSide] * _total / predictStats[_token][_matchId].predictionAmount[_winSide];
    }

    /**
     * @dev Claims reward
     */
    function claimReward(uint256 _matchId, address _token) external
    {
        DataTypes.Match memory _match = matchData.info(_matchId);
        ...
        uint256 _reward = calculateReward(_matchId, msg.sender, _token);
    ...
    function transferMoney(address _token, address _toAddress, uint256 _amount) internal {
        uint256 _fee = 0;
        if (feeRate > 0) {
            _fee = _amount * feeRate / ONE_HUNDRED_PERCENT;
        }
        if (_token == address(0)) {
            if (_fee > 0) {
                payable(feeCollector).transfer(_fee);
            }
            payable(_toAddress).transfer(_amount - _fee);
        } else {
            if (_fee > 0) {
                IERC20(feeCollector).safeTransfer(_toAddress, _fee);
            }
            IERC20(_token).safeTransfer(_toAddress, _amount - _fee);
        }
    }

    event PredictionCreated(uint256 matchId, address token, uint256 amount);
