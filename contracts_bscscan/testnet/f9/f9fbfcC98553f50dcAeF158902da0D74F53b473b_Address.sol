...
contract Traits is Ownable, ITraits {
  ...
  function drawSVG(uint256 tokenId) public view returns (string memory) {
    ...
    string memory svgString = string(abi.encodePacked(
      ...
      s.isEmployee ? drawTrait(traitData[1 + shift][s.head]) : drawTrait(traitData[1 + shift][s.alphaIndex]),
      s.isEmployee ? drawTrait(traitData[2 + shift][s.ears]) : '',
      drawTrait(traitData[3 + shift][s.eyes]),
      s.isEmployee ? drawTrait(traitData[4 + shift][s.nose]) : '',
      drawTrait(traitData[5 + shift][s.mouth]),
      s.isEmployee ? '' : drawTrait(traitData[6 + shift][s.neck]),
      s.isEmployee ? drawTrait(traitData[7 + shift][s.feet]) : ''
    ));
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    string memory metadata = string(abi.encodePacked(
      ...
      s.isEmployee ? 'Employee #' : 'Boss #',
      tokenId.toString(),
      '", "description": "Thousands of Employee and Wolves compete on a farm in the metaverse. A tempting prize of $WOOL awaits, with deadly high stakes. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Ethereum blockchain.", "image": "data:image/svg+xml;base64,',
...
contract Company is Ownable, IERC721Receiver, Pausable {
  ...
  function _claimProgrammerFromBarn(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (unstake) {
      ...
      _payBossTax(owed * WAG_CLAIM_TAX_PERCENTAGE / 100); // percentage tax to staked wolves
      owed = owed * (100 - WAG_CLAIM_TAX_PERCENTAGE) / 100; // remainder goes to Programmer owner
      barn[tokenId] = Stake({
        owner: _msgSender(),
        tokenId: uint16(tokenId),
        value: uint80(block.timestamp)
