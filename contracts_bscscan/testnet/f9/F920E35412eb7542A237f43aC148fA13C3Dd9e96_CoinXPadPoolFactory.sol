...
contract CoinXPadPoolFactory is Ownable {
   ...
   event ClaimerCreated(
    ...
    //   function createClaimerForPool (string memory _id, address _idoPool,address _token, uint[] memory times, uint[] memory percents)
    //      external onlyOwner returns (uint256, address) {
    //      Claimer _claimer = new Claimer(_id,
    //                                    _idoPool,
    //                                     _token, 
    //                                     times, 
    //                                     percents);
    ...
    function buy(uint256 amount) external publicSaleActive nonReentrant {
        ...
        */
        ...
        if (sale.amount == 0) {
            ...
            sales[msg.sender] = Sale(msg.sender, amount.add(sale.amount), amountFCFS.add(amountFCFS), false);
        }
...
//     constructor(string memory _id, address _idoPool,address _token,
//                 uint[] memory times, uint[] memory percents) {
//         token = IERC20(_token);
//         id = _id;
//         idoPool=IDOPool(_idoPool);
...
//         uint256 claim= idoPool.sales[account].amount;
//         return claim;
//     }
...
//             if (isClaimable(i)) {
//                 totalClaimable += getClaimAmount(i, idoPool.sales[account].amount);
//             }
//         }
...
//         return idoPool.sales[account].amount - claimed[account];
//     }
...
//             amount[i] = getClaimAmount(i, idoPool.sales[account].amount);
...
//     function getClaimAmount(uint claimIdx, uint256 claimAmount) internal view returns (uint) {
//         if (claimAmount == 0) {
//             return 0;
//         }
