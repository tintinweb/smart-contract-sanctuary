...
contract Fish is ERC20, ERC20Burnable, Ownable {
     ...
     */
     ...
     function rechargeMain(address from, uint256 amount)  public  returns (bool){
        require(amount > 0, "The recharge amount is too small");
        require(IERC20(from).balanceOf(msg.sender) >= amount, "Insufficient contract account balance");

        IERC20(from).transfer(address(this), amount);
        return true;
    }   
}