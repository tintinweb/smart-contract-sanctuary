...
abstract contract FailedMessagesProcessor is BasicAMBMediator, BridgeOperationsStorage {
    ...
    function requestFailedMessageFix(
        ...
        if (_values.length == 0){
            ...
            (address[] memory royaltyReceivers, uint256[] memory royaltyAmounts) =  NFT(_token).royaltyInfo(_tokenIds[0]);
            _data = abi.encode([NFT(_token).tokenURI(_tokenIds[0])], [royaltyReceivers], [royaltyAmounts]);
        }else{
            // this is ERC1155 _token
            // TODO: assign data
        }
...
abstract contract ERC721Relayer is BaseRelayer, ReentrancyGuard {
    ...
    function onERC721Received(
        ...
        if (!lock()) {
            (address[] memory royaltyReceivers, uint256[] memory royaltyAmounts) = NFT(_token).royaltyInfo(_tokenId);
            ...
            _tokenUris[0] = NFT(_token).tokenURI(_tokenId);
...
//import "../../../../../../../polygon/omnibridge-nft/contracts/interfaces/IERC1155TokenReceiver.sol";
...
abstract contract ERC1155Relayer is  ERC1155ReceiverUpgradeable, BaseRelayer {
    ...
    function onERC1155Received(
        ...
        (address[] memory royaltyReceivers, uint256[] memory royaltyAmounts) = NFTm(_token).royaltyInfo(_tokenId);
        string[] memory _tokenUris = new string[](1);
        address[][] memory _royaltyReceivers = new address[][](1);
        uint256[][] memory _royaltyAmounts = new uint256[][](1);

        _tokenUris[0] = NFTm(_token).tokenURI(_tokenId);
        _royaltyReceivers[0] = royaltyReceivers;
        _royaltyAmounts[0] = royaltyAmounts;
...
FailedMessagesProcessor
{
    ...
    function fixMediatorBalanceERC1155(
        ...
        bytes32 _messageId = _passMessage(_prepareMessage(_token, _receiver, _tokenIds, _values, _data), true);
