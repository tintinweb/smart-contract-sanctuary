...
abstract contract MerchantObject{
    ...
    function updateAccountLimits(uint256 subMaxRewardRate, uint subMaxDuration, uint subMaxReward) public onlyMainMerchant returns(uint code){
        if(_merchants[msg.sender].expiry < block.timestamp) return 1;
    ...
    function removeSubAccount(address merchantAddress) public onlyMainMerchant returns(uint code){
        ...
        _merchants[merchAddress].isSubAccountEnabled = false;
        _merchants[merchAddress].totalSubAccounts -= 1;
    }
    
    /**
     * @dev Automatic add member, add membership and support updating membership.
     */
    function addUpdateMembership(address memberAddress, uint256 rewardRate, uint duration, uint256 reward, string calldata campaign, string calldata membershipMeta)public onlyMerchant returns(uint code){
        ...
        if(_members[memberAddress].exist && _members[memberAddress].locked == true) return 1;// don't allowed to update if the member choose to lock their account.
        if(_merchants[merchAddress].usedRewards.add(rewardRate) > _merchants[merchAddress].allocatedRate) return 2;// merchant can not use more than the rewards they bought.
        if(_memberships[merchAddress][memberAddress].locked == true) return 3;// don't allow to update if the member choose to lock the specific membership.
        if(_merchants[merchAddress].expiry < time) return 4; // merchant account already expired;
        if(_currentRewardRate.add(rewardRate > 0 ?rewardRate : 0) > _maxGlobalRate) return 5;//don't allow if the additional minting rate will exceed global minting rate.
        if(_merchants[merchAddress].maxRewardPerAddress < rewardRate) return 6; //don't allow if the reward rate is more than the limit set for merchant;
        if(_merchants[merchAddress].subMaxRewardRate < rewardRate) return 7; //don't allow if the reward rate is more than the limit set by main account.
        if(_merchants[merchAddress].subMaxDuration < duration) return 8; //don't allow if the expiry duration is more than the limit set by main account.
        if(_merchants[merchAddress].subMaxReward < reward) return 9;  //don't allow if the reward is more than the limit set by main account.
        ...
        if(_merchants[msg.sender].isSubAccount == true && _merchants[msg.sender].isSubAccountEnabled == false) return 11; // block sub account from adding and updating membership is account is disabled.
        if(_merchants[merchAddress].allowLifeTime == false && _memberships[merchAddress][memberAddress].exist==false && duration == 0) return 12; //merchant is not allowed to set lifetime reqward;
    ...
    function getClaimable(address merchantAddress, address memberAddress) public view returns(uint code, uint256 unclaimed){
        ...
        if(_memberships[merchAddress][memberAddress].expiry < time && _memberships[merchAddress][memberAddress].rewardRate == 0) return (3, 0e9);
        uint span = 0;
