...
contract DividendDistributor is IDividendDistributor {
    ...
    constructor (address _router, address _dividend, address _weth, address[] memory _path) {
    ...
    function setPath(address[] memory _path) external onlyToken {
        path = _path;
    }
...
contract ProjectE is IBEP20, Auth {
    ...
    string constant _name    = "Dim Bulb";
    string constant _symbol  = "DBULB";
    ...
    constructor (address _dexRouter, address _teamWallet, address _dividend_token) Auth(msg.sender) {
        ...
        rewardsPath = [WETH, address(DIVIDEND_TOKEN)];
        //new address[](2);
        //rewardsPath[0] = WETH;
        //rewardsPath[1] = address(DIVIDEND_TOKEN);

        distributor = new DividendDistributor(routerAddress, DIVIDEND_TOKEN, WETH, rewardsPath);
    ...
    function getRewardsPath() external view returns(address[] memory) {
        return distributor.getPath();
    }
    ...
    function setRewardsPath(address[] memory _path) external authorized {
        distributor.setPath(_path);
    }
