    ...
    */
        ...
        public payable
    {
        ...
        for(uint i=0; i<crateData.rarities.length; i++) {
            TokenRarity rarity = crateData.rarities[i];

            for(uint j=1; j<=nextTokenId; j++) {   
                UmblData memory umblData = tokenUmblData[j];             
                if(umblData.status == TokenStatus.MINTED && rarity == umblData.rarity) {
                    availableTokenCount++;
                }
            }
        }
