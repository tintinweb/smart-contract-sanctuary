...
contract characterContract is ERC721Enumerable, ERC721URIStorage {
  // for inherit
  ...
  modifier onlyJedi () {
    require(_msgSender() == Jedi, "not Jedi's calling");
    _;
  }

  function setJedi(address newJedi_) public onlyJedi {
    Jedi = newJedi_;
  }

  function setSuperMinter(address newSuperMinter_) public onlyJedi {
    superMinter = newSuperMinter_;
  }

  function setMinter(address newMinter_, uint cardId_, uint amount_) public onlyJedi {
  ...
  function setMyBaseURI(string memory uri_) public onlyJedi {
    myBaseURI = uri_;
  }

  function newCard(string memory name_, uint cardId_, uint maxAmount_, string memory tokenURI_,uint rarity_, string memory faction_) public onlyJedi {
  ...
  function updateCard(string memory name_, uint cardId_, uint maxAmount_, string memory tokenURI_,uint rarity_, string memory faction_) public onlyJedi {
    require(cardId_ != 0 && cardInfoes[cardId_].cardId != 0, "ERC721: wrong cardId");
    ...
    cardInfoes[cardId_].tokenURI = tokenURI_;
    cardInfoes[cardId_].rarity = rarity_;
    cardInfoes[cardId_].faction = faction_;
  }
