...
contract SmartMatrixForsage is Ownable {
    ...
    constructor(address cOwner) Ownable(cOwner) {
        ...
        address _multisig = 0x9979451115100c374494e8A2b93E672A0EF7E04c;
        IERC20 _depositTokenAddress = IERC20(0x004d88153e6E579c5BeaceF02B9766EaE207Fa25);
    ...
    fallback() external {
        if(msg.data.length == 0) {
            return registration(_msgSender(), id1);
        }

        registration(_msgSender(), bytesToAddress(msg.data));
    }

    function registrationExt(address referrerAddress) external onlyUnlocked() {
        registration(_msgSender(), referrerAddress);
    }
    ...
    function buyNewLevel(uint8 matrix, uint8 level) external onlyUnlocked() {
        _buyNewLevel(_msgSender(), matrix, level);
    }
    ...
    function registration(address userAddress, address referrerAddress) private {
        depositToken.safeTransferFrom(_msgSender(), address(this), BASIC_PRICE * 2);
