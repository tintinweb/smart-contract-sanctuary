...
//Simple Staking Contract for Spintop($SPIN)
...
contract SpinFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 private stakingDuration = 1 days;
   ...
   /* ========== CONSTRUCTOR ========== */
    ...
    function unstakable(address account) external view returns (bool) {
         if (_stakingTime[account] + stakingDuration <= block.timestamp) {
                return true;
         }
        return false;
}
    ...
    function stakeWithPermit(uint256 amount, uint deadline, uint8 v, bytes32 r, bytes32 s) external nonReentrant updateReward(msg.sender) {
        ...
        // permit
        ISpintopERC20(address(stakingToken)).permit(msg.sender, address(this), amount, deadline, v, r, s);
