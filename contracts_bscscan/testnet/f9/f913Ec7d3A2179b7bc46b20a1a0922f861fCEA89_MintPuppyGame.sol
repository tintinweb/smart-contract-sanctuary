...
contract MintPuppyGame is SamoyedGame, ReentrancyGuard {
    ...
    event UpdateCost(uint256 _smoyCost, uint256 _busdCost);
    ...
    function setCost(uint256 _busdCost, uint256 _smoyCost) external onlyOwner {
        busdCost = _busdCost;
        smoyCost = _smoyCost;
        emit UpdateCost(_busdCost, _smoyCost);
    }

    function mintWithSMOY() external nonReentrant {
        uint256 totalCost = smoyCost;

        collectFee(smoyToken, totalCost);
        feeDistributeSMOY(smoyToken, totalCost);
        mintPuppy();
    }

    function mintWithBUSD() external nonReentrant {
        uint256 totalCost = busdCost;
        collectFee(busdToken, totalCost);
        feeDistributeBUSD(busdToken, totalCost);
        mintPuppy();
    }
    ...
    function feeDistributeSMOY(IBEP20 _token, uint256 _amount) private {
    ...
    function feeDistributeBUSD(IBEP20 _token, uint256 _amount) private {
        uint256 balance = _token.balanceOf(address(this));
        require(balance >= _amount, "MiniGame: INSUFFICIENT_BALANCE");
        TransferHelper.safeTransfer(address(_token), platformReserveAddress, _amount);
    }
