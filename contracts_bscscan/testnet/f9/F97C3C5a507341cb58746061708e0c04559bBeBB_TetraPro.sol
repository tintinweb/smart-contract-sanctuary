...
contract TetraPro is ERC20, AccessControl, Pausable {
    // CONFIG START

    uint256 denominator = 100;
    
    // TOKEN
    string tokenName = "TetraPro";
    string tokenSymbol = "TTP";
    uint256 tokenTotalSupply = 1_000_000_000 * (10**18);
    
    // ADRESSES
    address devWallet = 0x98eccbE01A6BBD97749aFB45F2177Aba68ac7245;
    address marketingWallet = 0xF2b9309A3F94593a2b5Ca4131cC41bdbf653fd89;
    ...
    uint256 public maxTxAmount = 5_000_000 * 10**18 + 1;
    uint256 public maxWalletAmount = 10_000_000 * 10**18 + 1;

    // REDISTRIBUTION
    address public token = 0x965F527D9159dCe6288a2219DB51fc6Eef120dD1;
	...
	function setWalletAddress(address payable _devWallet, address payable _marketingWallet) external onlyRole(OWNER_ROLE) {
         devWallet = _devWallet;
         marketingWallet = _marketingWallet;
    }

    function Sweep() external onlyRole(OWNER_ROLE) {
        //KEPT FOR ANY BNB SENT TO THE CONTRACT ADDRESS TO GO TO DEV FOR REFUND
        uint256 balance = address(this).balance;
        Address.sendValue(payable(devWallet), balance * 10**18);
    }
    ...
    function setTaxesBuy(uint256 dev, uint256 marketing, uint256 redistribution) external onlyRole(OWNER_ROLE) {
        devTaxBuy = dev;
        marketingTaxBuy = marketing;
        redistributeTaxBuy = redistribution;
    }

    function setTaxesSell(uint256 dev, uint256 marketing, uint256 redistribution) external onlyRole(OWNER_ROLE) {
        devTaxSell = dev;
