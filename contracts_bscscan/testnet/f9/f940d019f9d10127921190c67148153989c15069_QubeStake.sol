...
contract QubeStake is Pausable {
    ...
    function pendingReward(uint256 sid) public view returns (uint256 getAmountOut){        
        return (userInfo[sid].stakeAmount.mul(
                block.timestamp.sub(userInfo[sid].stakeTime)).mul(
                    rewardRoi)).div(1200 * monthDuration);
    }
    ...
    function stake(uint256 amount) public whenNotPaused {
        ...
        userInfo[internalTicket] = userData(_msgSender(),block.timestamp,block.timestamp,amount,0);
...
contract QubeStakeFactory is Ownable{        
    ...
    function createPool(
        ...
        IERC20(_rewardToken).transferFrom(_msgSender(),address(newQube),_amount);
