...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    address public constant DOT = address(0x7083609fCE4d1d8Dc0C979AAb8c869Ea2C873402); //DOT
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            bool success = false;
            try IERC20(DOT).transfer(user, _withdrawableDividend) {
                success = true;
            } catch {
                emit ErrorEmission("Could not transfer DOT Dividends (Dividends currently too low?)");
            }
...
contract DOTYFLECTDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    uint256 public constant minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    ...
    constructor() public DividendPayingToken("DOTYFlectDvt", "DOTYFlectDvt") {
        claimWait = 3600;
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "DotyFlect_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "DotyFlect_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DotyFlect contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DotyFlect_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DotyFlect_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DotyFlect is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DOTYFLECTDividendTracker public dividendTracker;
    ...
    address public _marketingWalletAddress = 0xaA4da54ebFB14Dae1C23CCe4AeA8493a0a645888;
    ...
    constructor() public ERC20("DotyFlect", "DotyFlect") {


        dividendTracker = new DOTYFLECTDividendTracker();
    ...
    function prepForLaunch() public onlyOwner {
        require(!tradingReady, "DotyFlect: Contract has already been preppared for trading.");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DotyFlect: Account is already the value of 'excluded'");
    ...
    function setMarketingWallet(address payable wallet) external onlyOwner{
        require(wallet != address(0), "DotyFlect: Marketing wallet cannot be 0!");
        require(wallet != deadWallet, "DotyFlect: Marketing wallet cannot be the dead wallet!");
        require(canChangeMarketingWallet == true, "DotyFlect: The ability change the marketing wallet has been disabled.");
        _marketingWalletAddress = wallet;
    }

    function setDOTRewardsFee(uint256 value) external onlyOwner{
        require(value <= 10, "DotyFlect: Maximum DOT Reward fee is 10%");
        require(canChangeFees == true, "DotyFlect: The ability to update or change fees has been disabled.");
        DOTRewardsFee = value;
        totalFees = DOTRewardsFee.add(liquidityFee).add(marketingFee);
    }

    function setLiquidityFee(uint256 value) external onlyOwner{
        require(value <= 4, "DotyFlect: Maximum Liquidity fee is 4%");
        require(canChangeFees == true, "DotyFlect: The ability to update or change fees has been disabled.");
        liquidityFee = value;
        totalFees = DOTRewardsFee.add(liquidityFee).add(marketingFee);
    }
    ...
    function setMarketingFee(uint256 value) external onlyOwner{
        require(value <= 5, "DotyFlect: Maximum Marketing fee is 5%");
        require(canChangeFees == true, "DotyFlect: The ability to update or change fees has been disabled.");
        marketingFee = value;
        totalFees = DOTRewardsFee.add(liquidityFee).add(marketingFee);
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DotyFlect: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function blacklistAddress(address account, bool value) public onlyOwner{
        require(canBlacklist == true, "DotyFlect: The ability to blacklist / amnesty accounts has been disabled.");
        if (value) {
            require(now < blacklistDeadline, "DotyFlect: The ability to blacklist accounts has been disabled.");
        }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DotyFlect: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DotyFlect: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DotyFlect: Cannot update gasForProcessing to same value");
