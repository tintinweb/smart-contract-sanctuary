...
contract VelaMarketplace is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // mapping(address => mapping(address => mapping(uint256 => uint256))) public currentBids; // user address => erc721 address => token id => bid id
    ...
    function updateErc20Whitelist(address[] memory erc20s, bool status) public onlyOwner {
    ...
    function _calculateSystemFee(uint256 price, uint256 feePercent) internal pure returns (uint256){
