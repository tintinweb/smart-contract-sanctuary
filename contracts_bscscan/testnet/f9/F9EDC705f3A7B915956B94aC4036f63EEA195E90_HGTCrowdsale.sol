...
// https://www.youtube.com/watch?v=PoI5SGE8150
// https://www.linkedin.com/pulse/how-write-test-real-world-crowdsale-contract-ethereum-sandeep-panda

// https://forum.openzeppelin.com/t/help-me-to-understand-liquidity-in-new-contracts/8390

// https://ethereum.stackexchange.com/questions/98531/price-conversion-using-chainlink-ethusd-price/98686

// https://www.youtube.com/watch?v=uPeRwDvkuCs
...
contract HGTCrowdsale is Crowdsale {
    ...
    address private bnbAddress = address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd);
    // IERC20 private usdtToken = IERC20(0x7ef95a0fee0dd31b22626fa2e10ee6a223f8a684);
    address usdtTokenAddress = address(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
    ...
    */
    function getPriceBnbUsd() public view returns (int)  {
        (
            uint80 roundID, 
            int price,
            uint startedAt,
    ...
    // Presale
    // -----------------------------------------
    function startICO(uint endDate, uint _minPurchase, uint _maxPurchase, uint256 _poolPercent) external icoNotActive() {
        ...
        // tokensPool = _availableTokens.mul(_poolPercent).div(10**2);
        minPurchase = _minPurchase;
        maxPurchase = _maxPurchase;
        availableTokensICO = _token.totalSupply();
    }

    function stopICO() external icoActive() {
    ...
    function IcoCompleted() external {
        uint256 usdtBalance = IERC20(usdtTokenAddress).balanceOf(address(this));
        transferToken(IERC20(usdtTokenAddress),_wallet,usdtBalance);
    }


    

    function IcoRefund() external {
    ...
    function _forwardFunds() internal override  {
     ...
     function _prepareRefund(address _walletAddress) internal {
        ...
        uint256 usdtBalance = IERC20(usdtTokenAddress).balanceOf(_walletAddress);
    ...
    function _getTokenAmount(uint256 weiAmount) internal view override returns (uint256) {
         // 2bnb x 410(usd price) /100(token price fixed)  = 8.2 token s
        int price_fixed = 99;

        // int min_required = 5; // minimum required in USD
        // La división de enteros de solidez se redondea automáticamente hacia 0.
        // https://ethereum.stackexchange.com/questions/98531/price-conversion-using-chainlink-ethusd-price/98686
        int _pricebnb = getPriceBnbUsd() / 10 ** 8; // price of 1 ether in USD 
        int price = _pricebnb / price_fixed;
        // require(msg.value >= min_required / _price, "NOT ENOUGH ETHER");
        return weiAmount.mul(uint256(price));
    }
    ...
    function swapETHForUSDT(uint256 amount) private {
        ...
        // https://github.com/pancakeswap/pancake-swap-interface-v1/issues/365
        // WBNB	BEP20	0xae13d989dac2f0debff460ac112a837c89baa7cd 
        // USDT	BEP20	0x7ef95a0fee0dd31b22626fa2e10ee6a223f8a684 
        // ETH	BEP20	0x8babbb98678facc7342735486c851abd7a0d17ca 
        // BUSD	BEP20	0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7 
        // DAI	BEP20	0x8a9424745056Eb399FD19a0EC26A14316684e274 
        ...
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(
            ...
            path,
            address(this), // Wallet address to recieve USDT
    ...
    function swapUSDTForETH(uint256 amount) private {
        ...
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(
            0, // accept any amount of Tokens
            path,
            address(this), // Wallet address to recieve USDT _wallet antes
            block.timestamp.add(300)
        );
...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    //  Amount token 
    uint256 internal  availableTokensICO;
    /**
    ...
    function buyTokens(address beneficiary) public nonReentrant payable {
        ...
        _updatePurchasingStateCustom(beneficiary, weiAmount, tokens);
    ...
    function _updatePurchasingStateCustom(address beneficiary, uint256 weiAmount, uint256 tokens) internal virtual {
        // solhint-disable-previous-line no-empty-blocks
