...
contract Zeus is IBEP20, Auth {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(inZenLaunchPhase && isZenblockActive()){
            ...
            emit log('Tax: default tax selected'); // TODO : REMOVE
    ...
    function startZenblock(uint256 duration) internal {
    ...
    function startZenLaunch(uint256 duration, uint256 phases) external authorized{
    ...
    function checkZenLaunchTxLimit(address recipient, uint256 amount) internal {
    ...
    function getZenLaunchTotalBuyAllowance(address holder) external view returns (uint256) {
        if(!isZenLaunchActive()) return 0;

        uint256 phases = getZenLaunchCounter(zenLaunchInterval);
        uint256 multiplier  = phases != 0 ? phases: 1 ;
        uint256 totalBuyAllowance  = zenLaunchBuyAllowance.mul(multiplier);
        if(totalBuyAllowance > _balances[holder]){
            return (totalBuyAllowance.sub(_balances[holder])) / (10 ** _decimals);
        }
        return 0;
    }
    ...
    function getHolderAllowance(address holderAddress) public view returns (uint256) {
        if(isDividendExempt[holderAddress]) return _maxTxAmount;
        return distributor.getUpdatedAllowance(holderAddress) / (10 ** _decimals);
    }


    function ConcatenateStrings(string memory a, string memory b) internal pure returns (string memory concatenatedString) {
    ...
    // ########################################
    function setDividendTokenAddress(address tokenAddress) external authorized {
        distributor.setDividendToken(tokenAddress);
    }
...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 allowanceDuration = 120; // 120 = 2min , 300 = 5 min, Change to 2 weeks 1210000
    ...
    // uint256 public minDistribution = 1 * (10 ** 18); //main-net
    uint256 public minDistribution = 1000 * (10 ** 9); //test-net
    ...
    //Allowance Functions
    function getAllowance(address shareholder) external view returns (uint256) {
        return shares[shareholder].allowance;
    }

    function hasEnoughAllowance(address shareholder, uint256 amount) external view returns (bool) {
        // console.log('hasEnoughAllowance() ------------------------------------------------------'); // TODO : REMOVE
        // console.log('- allowance: %s', shares[shareholder].allowance); // TODO : REMOVE
        return shares[shareholder].allowance > 0 && shares[shareholder].allowance >= amount;
    }

    function getUpdatedAllowance(address shareholder) external view returns (uint256) {
        //if share holder doesn't exist => return
        if(shares[shareholder].amount == 0 || shares[shareholder].lastTxOutDate == 0){ return 0; }

        //if its not time to add new allowance, return current allowance
        if(block.timestamp <= shares[shareholder].lastTxOutDate.add(allowanceDuration)) return shares[shareholder].allowance;
        
        //if its time to add allowance, calculate new allowance 
        return shares[shareholder].allowance.add(shares[shareholder].amount.div(allowanceDivisor).mul(block.timestamp.sub(shares[shareholder].lastTxOutDate).div(allowanceDuration)));
    }

    function updateAllowance(address shareholder) external onlyToken {
        ...
        bool isNowLessThanNewCycleEnd = block.timestamp <= shares[shareholder].lastTxOutDate.add(allowanceDuration);

        // console.log('- lastTxOutDate: %s', shares[shareholder].lastTxOutDate); // TODO : REMOVE
        // console.log('- block.timestamp: %s', block.timestamp); // TODO : REMOVE
        // console.log('- isNowLessThanNewCycleEnd: %s', isNowLessThanNewCycleEnd); // TODO : REMOVE

        //If no new cycle passed => return
        if(isNowLessThanNewCycleEnd) { return; }

        //ADD NEW ALLOWANCE

        //Calculate new allowance 
        uint256 sliceCountsToClaim = block.timestamp.sub(shares[shareholder].lastTxOutDate).div(allowanceDuration);
        uint256 newTotalAllowance = shares[shareholder].allowance.add(shares[shareholder].amount.div(allowanceDivisor).mul(sliceCountsToClaim));
    ...
    //Dividend
    function setDividendToken(address tokenAddress) external onlyToken {
