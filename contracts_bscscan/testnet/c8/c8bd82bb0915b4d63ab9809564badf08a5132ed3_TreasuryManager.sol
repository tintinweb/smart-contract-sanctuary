...
contract TreasuryManager is ITreasuryManager, Ownable, ReentrancyGuard {
    ...
    uint256 private rewardTimeAdd = 259200;
    // Use For Reset rewardTimeAdd
    uint256 private rewardTimeReset = 259200;
    ...
    uint256 private withdrawPeriod = 1209600;
    ...
    event ResetRewardTimeAdd(uint256 ResetRewardTimeAdd);
    event ClearPool(uint256 WBNB, uint256 LIGER);
    ...
    {
        uint256 nextRewardTime = lastRewardUpdate.add(rewardTimeAdd);
        return (
            totalUsers,
            totalJoined,
            totalLPSupply,
    ...
    // Internal Main Function To Calculate Reward
    // Received After Swap Liger To WBNB
    function updateReward() public override {
        uint256 RewardTime = lastRewardUpdate.add(rewardTimeAdd);
    ...
    function clearPool() external onlyOperatorOrOwner {
        if (totalLPSupply == 0) {
            ...
            emit ClearPool(BalanceWBNB, BalanceLIGER);
        }
    }
    ...
    // Reset RewardTime To Default Time 259600
    function resetRewardTime() external onlyOperatorOrOwner {
        rewardTimeAdd = rewardTimeReset;
        emit ResetRewardTimeAdd(rewardTimeReset);
    }
}