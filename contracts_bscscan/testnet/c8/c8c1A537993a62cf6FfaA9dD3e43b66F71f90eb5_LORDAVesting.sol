...
contract LORDAVesting is Ownable {
    IERC20 public immutable _lorda;

    uint256 public immutable _tgeClaimPercentage;
    ...
    constructor(
        address lorda,
        uint256 tgeClaimPercentage,
        uint256 startTime,
        uint256 endTime,
        uint256 periods
    ) {
        _lorda = IERC20(lorda);
        _tgeClaimPercentage = tgeClaimPercentage;
    ...
    function _claimableAmount(address account) private view returns (uint256) {
        if (block.timestamp < _startTime) {
            ...
            uint256 tgeUnlock = (_locked[account] * _tgeClaimPercentage) /
                10000;

            return
                tgeUnlock +
                (((_locked[account] - tgeUnlock) * passedPeriods) / _periods) -
                _released[account];
        }
    }
    ...
    function _passedPeriods() private view returns (uint256) {
        return
            (block.timestamp >= _endTime)
                ? _periods
                : ((block.timestamp - _startTime) * _periods) /
                    (_endTime - _startTime);
    }
