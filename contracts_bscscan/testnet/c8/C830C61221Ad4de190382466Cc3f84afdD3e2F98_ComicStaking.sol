       ...
       OuruNFT nftContract = OuruNFT(0x456b0DF1ceC361b6C1c997992415B606394197A6);
        ...
        for(uint256 i=0; i < rewardChanges.length; i++) {
           ...
           (uint256 level, uint256 tier) = nftContract.getHero(tokenID);
            ...
            if (blocksPassed >= 100 && blocksPassed <= 200) {
            rewards += (rewardChanges[i].rewardPerBlock * level * tier * blocksPassed * 80) /100;
            }
            else { rewards += rewardChanges[i].rewardPerBlock * level * tier * blocksPassed;}
        }
        return rewards;
    }      


    function calculateblock(uint256 tokenID) public view returns (uint256) {
        require(stakes[tokenID].lastClaimedBlock != 0, "token not staked");
