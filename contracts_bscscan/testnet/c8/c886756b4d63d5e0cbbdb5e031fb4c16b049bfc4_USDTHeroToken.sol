...
contract USDTHeroToken is Context, IBEP20, Ownable {
    ...
    string private _name = "USDTHeroToken";
    string private _symbol = "USDTH";
    uint8 private _decimals = 18;
    
    uint256 public _USDTFee = 8;
    uint256 private _previousBNBFee = _USDTFee;
    
    uint256 public _liquidityFee = 1;
    uint256 private _previousLiquidityFee = _liquidityFee;

    address public immutable USDT = address(0x337610d27c682E347C9cD60BD4b3b107C9d34dDd); //usdt testnet
    // address public immutable USDT = address(0x55d398326f99059fF775485246999027B3197955); // USDT
    address public _rewardWalletAddress = 0x5b769EC98d8EC4E3525445426a2c44FeF4268398;
    ...
    event UpdateFees(uint256 usdtFee, uint256 liquidityFee);
    ...
    constructor () {
        ...
        _isExcludedFromFee[_rewardWalletAddress] = true;
    ...
    function setFees(uint256 usdtFee, uint256 liquidityFee) external onlyOwner() {
        require(_USDTFee != usdtFee || _liquidityFee != liquidityFee);
        _USDTFee = usdtFee;
        _liquidityFee = liquidityFee;
        emit UpdateFees(usdtFee, liquidityFee);
    }
    ...
    function _getFeeValues(uint256 tAmount) private view returns (uint256) {
        uint256 fee = tAmount.mul(_USDTFee + _liquidityFee).div(10**2);
        uint256 tTransferAmount = tAmount.sub(fee);
        return tTransferAmount;
    }

    function removeAllFee() private {
        if(_USDTFee == 0 && _liquidityFee == 0) return;
        
        _previousBNBFee = _USDTFee;
    ...
    function swapAndCharge(uint256 tokenBalance) private lockTheSwap {
        ...
        uint256 liquidBalance = tokenBalance.mul(_liquidityFee).div(_liquidityFee + _USDTFee).div(2);
        ...
        uint256 feeBalance = tokenBalance-liquidBalance;
        swapAndSendToFee(feeBalance);

        _balances[address(this)] = _balances[address(this)].sub(tokenBalance);
        emit SwapAndCharged(tokenBalance, liquidBalance, address(this).balance, bnbForLiquid);
    }

    function swapAndSendToFee(uint256 tokens) private  {
        uint256 initialCAKEBalance = IBEP20(USDT).balanceOf(address(this));

        swapTokensForUsdt(tokens);
        uint256 newBalance = (IBEP20(USDT).balanceOf(address(this))).sub(initialCAKEBalance);
        IBEP20(USDT).transfer(_rewardWalletAddress, newBalance);
    }
