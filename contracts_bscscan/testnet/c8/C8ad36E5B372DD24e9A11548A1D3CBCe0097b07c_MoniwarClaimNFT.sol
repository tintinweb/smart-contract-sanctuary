...
contract MoniwarClaimNFT is Ownable, ReentrancyGuard {
    ...
    function checkBeforeClaim(address _user) public view returns (bool wl) {
        if(whiteList[_user] == true && claims[_user].owner == address(0)){
            return true;
        }
        return false;
    }

    function getHasClaim(address _user) public view returns (uint256 claim) {
        if(claims[_user].owner == address(0)){
            return 0;
        }
        return claims[_user].tokenId;
    }
    ...
    function claimNFT() public payable nonReentrant {
        require(whiteList[_msgSender()] == true, "not valid whitelist");
        require(claims[_msgSender()].owner == address(0), "already claimed");

        uint256 mowaBal = mowaToken.balanceOf(_msgSender());
        require(mowaBal >= feeClaim, "Not enough money to claim");
     ...
     function addWhiteList(address[] memory newWhiteList) public onlyOwner {
