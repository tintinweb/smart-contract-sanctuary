...
contract LinksPayment is Ownable {
    using SafeERC20 for IERC20;

    event TreasuryAddressChanged(address indexed sender,address oldTreasuryAddress, address newTreasuryAddress);
    event Transaction(address indexed sender, address indexed account, uint256 amount,uint256 nonce, uint256 deadline);
    ...
    function transact(address account, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external {
        
        IERC20Permit(address(links)).permit(account,address(this),value, deadline, v, r, s);
        IERC20(links).safeTransferFrom(msg.sender,treasuryAddress,value);
        ...
        // links.safeTransferFrom(account, treasuryAddress, amount);

        // emit Transaction(msg.sender, account, amount, nonce, deadline);
    }
