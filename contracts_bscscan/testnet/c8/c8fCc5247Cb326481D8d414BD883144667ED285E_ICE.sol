...
contract ICE is Context, IERC20, Ownable {
    ...
    string private _name = "IceIceBaby";
    ...
    address public stakingWallet = 0x89EC98A503f98D201df4743e797a6E5147428aa2;
    uint256 private _previousStakingFee = _StakingFee;
    
    
    uint256 public _WalletFee   = 50;
    address public extraWallet = 0x89EC98A503f98D201df4743e797a6E5147428aa2;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 walletAmt = amount.mul(_WalletFee).div(1000);
        
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(stakingAmt).sub(walletAmt).sub(BurnAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(stakingAmt).sub(walletAmt).sub(BurnAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(stakingAmt).sub(walletAmt).sub(BurnAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(stakingAmt).sub(walletAmt).sub(BurnAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(stakingAmt).sub(walletAmt).sub(BurnAmt)));
        }
        ...
        _transferStandard(sender, extraWallet, walletAmt);
