...
contract SantaStakingB is Ownable, ReentrancyGuard {
    ...
    function harvest(address _token) external onlyOwner {
        require(_token == address(stakingToken) || _token == address(rewardToken), "Not staking or reward token");
    ...
    function _distributeFees(uint256 earned) internal returns(uint256) {
        // buy back 30% 
        uint256 _distributed = buyBack(earned);

        // transfer 30% to dev wallet
        uint256 _amount = earned.mul(devRate).div(10000);
        if(_amount > 0) {
            if(address(reflectionToken) == address(0x0)) {
        ...
        if(_amount > 0) {
            if(address(reflectionToken) == address(0x0)) {
                ...
                reflectionToken.safeTransfer(poolA, _amount);
            }
            _distributed = _distributed.add(_amount);
        }
    ...
    function buyBack(uint256 earned) internal returns (uint256) {
        ...
        _safeSwap(_amount, reflectToStakedPath, buyBackAddress);
...
interface IStaking {
    ...
    function userInfo(address) external returns(uint256 amount, uint256 rewardDebt, uint256 reflectionDebt, uint256 lastStakedTime);
