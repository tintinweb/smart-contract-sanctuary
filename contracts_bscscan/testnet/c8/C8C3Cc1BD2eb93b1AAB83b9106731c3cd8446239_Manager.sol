...
contract Manager {
    ...
    function register(uint256 tokenId, int x, int y) public payable {
        require(msg.value >= 0.05 ether);
        IMonster monster = IMonster(monsterAddress);
        monster.create(msg.sender, tokenId, x, y, 15, 25);
        punkOf[msg.sender] = tokenId;
    }

    function pullFund(address payable addr) public {
        require(msg.sender == owner, "Only admin can pull funds.");
        addr.transfer(address(this).balance);
    }

    function isFreeze(int x, int y) public view returns (bool) {
        uint t = block.timestamp;
        uint start = t / 36000 * 36000;
        if (t - start > 1200) {
