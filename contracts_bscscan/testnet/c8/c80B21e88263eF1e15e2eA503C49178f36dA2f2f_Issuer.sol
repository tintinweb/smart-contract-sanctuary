...
contract Issuer is Owned, MixinSystemSettings, IIssuer {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[3] = CONTRACT_PHANTOMSTATE;
    ...
    function issuePhantsOnBehalf(
        address issueForAddress,
        address from,
        uint amount
    ) external onlyPhantom {
        _requireCanIssueOnBehalf(issueForAddress, from);
        _issuePhants(issueForAddress, amount, false);
    }

    function issueMaxPhantsOnBehalf(address issueForAddress, address from) external onlyPhantom {
        _requireCanIssueOnBehalf(issueForAddress, from);
        _issuePhants(issueForAddress, 0, true);
    }
    ...
    function burnPhantsOnBehalf(
        address burnForAddress,
        address from,
        uint amount
    ) external onlyPhantom {
        _requireCanBurnOnBehalf(burnForAddress, from);
        _voluntaryBurnPhants(burnForAddress, amount, false);
    }
    ...
    function burnPhantsToTargetOnBehalf(address burnForAddress, address from) external onlyPhantom {
        _requireCanBurnOnBehalf(burnForAddress, from);
        _voluntaryBurnPhants(burnForAddress, 0, true);
    }
