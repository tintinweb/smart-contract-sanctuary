...
contract RewardsDistributor is IRewardsDistributor {
    ...
    // the contract address for the WBNB reward tokens
    address addressWbnb            = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    
    address addressWbnbTestnet     = 0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F;
    ...
    IBEP20 addressRewardWBNB = IBEP20(addressWbnbTestnet); 
...
contract BitRides is IBEP20, Ownable {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender != owner() && recipient != address(this)  && recipient != address(addressDead) && recipient != _pancakeswapV2Pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver) {
            uint256 feeAmount = amount.mul(marketingFee).div(feeDenominator);
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(feeAmount);
            uint256 tosend = (address(this).balance).sub(initialBalance);
            (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: tosend, gas: 30000}("");
