...
contract BPContract is Ownable {
    struct TraderInfo {
        uint256 lastTrade;
        uint256 amount;
    }
    ...
    uint256 public maxBuy = 300 * 10**uint256(_decimals);
    uint256 public maxSell = 150 * 10**uint256(_decimals);
    uint256 public antiBotDuration = 1632461318;

    address public pancakeLiquidPair;
    uint256 public buyCooldown = 3 minutes;
    uint256 public sellCooldown = 3 minutes;

    mapping(address => mapping(string => TraderInfo)) public traders;
        ...
        if (block.timestamp < antiBotDuration) {
            if (_sender == pancakeLiquidPair) {
                if (_amount > maxBuy) revert("Buy amount limited ");
                    ...
                    traders[_recipient]["BUY"].lastTrade + buyCooldown >
                    block.timestamp
                ) {
                ...
                if (_amount > maxSell) revert("Sell amount limited ");
                    ...
                    traders[_sender]["SELL"].lastTrade + buyCooldown >
                    block.timestamp
                ) {
                    revert("You are cooldown to next trade!");
                } else {
