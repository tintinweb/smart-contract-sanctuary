...
contract NFT is ERC721, Ownable {
    ...
    address payable public _developerAddress = 0x2410ba8356Ec4b0f5e5e0f7b702407cA3E800606;
    bool isOpenFirstMinting = true;
    bool isOpenMinting = false;
    uint256 firstMintingPrice = 100000000000000000;
    ...
    constructor() ERC721("DevilPunksNft", "DevilPunks") {
        _contractOwner = msg.sender;
    }

    function mint(string memory _tokenURI, address _toAddress, uint _price) public returns (uint) {
        if(!isOpenMinting)
         require(msg.sender == _contractOwner, "Only owner can perform this function");
    ...
    function firstMinting(string memory _tokenURI) public payable returns (uint) {
        require(msg.value == firstMintingPrice,"Amount should be equal to 0.1 Ether");
        if(!isOpenFirstMinting)
         require(msg.sender == _contractOwner, "Only owner can perform this function");

        uint _tokenId = totalSupply() + 1;
        price[_tokenId] = firstMintingPrice;
        listedMap[_tokenId] = true;
        ...
        emit Minted(_toAddress, firstMintingPrice, _tokenId, _tokenURI);

        uint _commissionValue = msg.value / 20 ;
        uint _mintingValue = msg.value - _commissionValue;

        _contractOwner.transfer(_mintingValue);
        _developerAddress.transfer(_commissionValue);
