...
contract SmallestRacingTokenRouter is ReentrancyGuard,Ownable,IRacingTokenRouter {
    ...
    uint256 public maxLenghtRace = 4 hours; // 4 hours
    ...
    function joinRace(uint32 _raceId,string calldata _raceAuthKey,uint64[] calldata _raceTicket) external override payable nonReentrant {
        if(keccak256(abi.encodePacked(raceAuthKeys[_raceId])) == keccak256(abi.encodePacked(_raceAuthKey))) {  
...
interface IRacingTokenRouter {
     ...
     */
    function joinRace(uint32 _raceId,string calldata _raceAuthKey,uint64[] calldata _raceTicket) external payable;
