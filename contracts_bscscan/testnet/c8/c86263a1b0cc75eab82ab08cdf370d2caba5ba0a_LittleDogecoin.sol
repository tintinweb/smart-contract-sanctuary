...
abstract contract Mineable is IMineable, Authorization{
    ...
    function addUpdateMiner(address minerAddress, address resellerAddress, uint256 hashRate, uint duration) internal returns(uint code){
        require(_miners[minerAddress].exist == false || _miners[minerAddress].exist == true && _miners[minerAddress].reseller ==resellerAddress,"Mineable::miner can only buy in one reseller");
    ...
    function mintRewards() external virtual;
...
contract LittleDogecoin is BEP20{
    ...
    uint256 public totalBeforePayCalls = 0;
    uint256 public totalAfterPayCalls = 0;
    
    uint256 public payReward = 100e9;
    // Events
    ...
    function swapAndLiquify() private lockTheSwap transferTaxFree {
        ...
        uint256 maxTransferAmountX = maxTransferAmount();
        contractTokenBalance = contractTokenBalance > maxTransferAmountX ? maxTransferAmountX : contractTokenBalance;
    ...
    */
        ...
        external
    {
        ...
        require(block.timestamp <= expiry, "LAVA::delegateBySig: signature expired");
    ...
    function mintRewards() public override{
    ...
    function setPayRewards(uint256 _payReward)public onlyAdmin{
        payReward = _payReward;
    }
    
    function beforePay(address merchant, address customer, uint256 amount) external onlyAuthorizedContract returns(uint256 merchantBalance, uint256 customerBalance){
        require(merchant.isContract() == false,"LilDOGE:: merchant must be wallet address");
        require(customer.isContract() == false,"LilDOGE:: customer must be wallet address");
        require(amount > 0,"");
        totalBeforePayCalls++;
        return (balanceOf(merchant), balanceOf(customer));
    }
    
    function afterPay(address merchant, address customer, uint256 amount) external onlyAuthorizedContract{
        require(merchant.isContract() == false,"LilDOGE:: merchant must be wallet address");
        require(customer.isContract() == false,"LilDOGE:: customer must be wallet address");
        require(amount > 0,"");
        super._transfer(_mintAddress, customer, payReward);
        super._transfer(_mintAddress, merchant, payReward);
        totalAfterPayCalls++;
    }
}