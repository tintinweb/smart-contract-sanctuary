...
contract GymVaultsBank is ReentrancyGuard, Ownable, Relationship {
    ...
    uint256 public WITHDRAW_FEE_MAX = 10000;
    uint256 public WITHDRAW_FEE_UL = 8500;
    ...
    function updateRewardToken(
        ...
        require(rewardPoolInfo.rewardPerBlock == 0, "GymVaultsBank: Old pool still running");
        massUpdatePools();
        rewardPoolInfo.rewardToken = _rewardToken;
        rewardPoolInfo.rewardPerBlock = _rewardPerBlock;
        rewardPoolInfo.totalPaidRewards = 0;
    }
    ...
    function depositBNB( address _referrer) external payable{
        addRelationship(msg.sender, _referrer);
        distributeRewards( msg.value * RELATIONSHIP_REWARD / 100, address(0), msg.sender, false);
    ...
    function _deposit(uint256 _pid, uint256 _wantAmt, bool _isToken) private {
        ...
        require(!pausePool, "GymVaultsBank: Paused");
        updatePool(_pid);
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (_wantAmt > 0) {
            ...
            distributeRewards(_wantAmt * RELATIONSHIP_REWARD / 100, address(pool.want), msg.sender, true);
            }
    ...
    function _withdraw(uint256 _pid, uint256 _wantAmt, bool _isToken) private { 
        ...
        if (_wantAmt > 0) {
            ...
            if (_wantAmt > 0) {
                if(_isToken){
                    _wantAmt = _wantAmt * WITHDRAW_FEE / WITHDRAW_FEE_MAX;
                    pool.want.safeTransfer(address(msg.sender), _wantAmt);
                    pool.want.safeTransfer(TREASURY_ADDRESS, pool.want.balanceOf(address(this)));
    ...
    //     _withdraw($(GYM_VAULTS_BANK_CORE_POOL_ID), type(uint256).max, false);
    // }
    ...
    function getBNBAmount(address _user) public view returns (uint256 amountToBNB){
        ...
        uint256 pendingIbBNBUser = user.shares * alpacaToBNBAmount[1] / wantLockedTotal;
        uint256 pendingIbBNB = pendingIbBNBUser * totalSupply / totalToken;

        uint256 contractDepositAmount = IFairLaunchV1(0xac2fefDaF83285EA016BE3f5f1fb039eb800F43D).userInfo(1, pool.strategy).amount;
        uint256 amount = user.shares * contractDepositAmount / wantLockedTotal;
        
        uint256 amountToBNB = totalToken * (amount + pendingIbBNB) / totalSupply;
        return amountToBNB;
    ...
    // function setStopRewardPool( bool _stopRewardPool)
    //     external
    //     nonReentrant
    //     onlyOwner
    // {
    //     stopRewardPool = _stopRewardPool;
    // }

    // function setPausePool(bool _pausePool)
    //     external
    //     nonReentrant
    //     onlyOwner
    // {
    ...
    function setWithdrawFee(uint256 _fee) 
        ...
        onlyOwner
        {
            require(_fee >= WITHDRAW_FEE_UL && _fee <= WITHDRAW_FEE_MAX, "GymVaultsBank: Wrong range");
            WITHDRAW_FEE = _fee;
        }
}
...
contract Relationship {
    ...
    uint8[10] private RELATIONSHIP_DIRECT_REFERRAL_BONUS;
