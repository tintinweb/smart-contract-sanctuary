...
function buyBox(address buyer,uint256 rank) external onlyBoxNFTRoundOrOwner returns (uint256) {
        _tokenIds.increment();
        uint256 newBoxId = _tokenIds.current();
...
contract BoxNFTBuyRound1 is OwnableUpgradeable  {
...
function buyBox(uint256 rank) external payable returns (uint256) {
