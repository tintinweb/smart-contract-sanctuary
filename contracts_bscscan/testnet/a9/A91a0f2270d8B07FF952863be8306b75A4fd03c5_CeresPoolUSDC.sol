...
abstract contract CeresPool is AccessControl {
    ...
    function setMaintainer(address account) public onlyGovernance {
        maintainer = account;
    }
...
contract CeresPoolUSDC is CeresPool {
    ...
    function mint(uint256 usdcAmount, uint256 crsAmount, uint256 ascOut) internal override {
        ...
        uint256 ascToUsdc = ascOut * colRaito / CERES_PRECISION;
        uint256 ascToCrs = ascOut * (1e6 - colRaito) / CERES_PRECISION;

        address usdcStakingAddr = stakingAddress[usdcAddress];
        address crsStakingAddr = stakingAddress[crsAddress];
        
        // notify mint
        IMinter(usdcStakingAddr).notifyMint(ascToUsdc, usdcAmount);
        IMinter(crsStakingAddr).notifyMint(ascToCrs, crsAmount);
        
        // mint to
        coinASC.poolMint(usdcStakingAddr, ascToUsdc);
    ...
    function redeem(uint256 ascAmount, uint256 usdcOut, uint256 crsOut) internal override {
        ...
        // nofity redeem
        IRedeemer(ascStakingAddr).notifyRedeem(ascAmount, crsOut, usdcOut);
...
interface IMinter {
    ...
    function reinvestByStaking(address account) external returns (uint256);
}
