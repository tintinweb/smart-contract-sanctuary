...
contract DragonSlayer is ERC20, ERC20Burnable, Ownable, DragonSlayerInterface {
    ...
    function setManager(address _manager) public onlyOwner {
        require(!addressBlackList[_manager], "Address is blocked by owner");
        ...
        onlyOwner
    {
        ...
        for (uint256 index = 0; index < to.length; index++) {
            if (addressBlackList[address(to[index])]) {
    ...
    function setBotAddresses (address[] memory _addresses) external onlyOwner {
        ...
        for (uint256 index = 0; index < _addresses.length; index++) {
            if (addressBlackList[address(_addresses[index])]) {
                revert("Address is blocked by owner");
            }
    ...
    function addBotAddress (address _address) external onlyOwner {
        ...
        require(!addressBlackList[_address], "Address is blocked by owner");
    ...
    function rewards(address recipient, uint256 amount) override external onlySupporter {
        ...
        require(amount > 0, "not accept 0 value");
        require(!addressBlackList[recipient], "Address is blocked by owner");
    ...
    function _transfer(
        ...
        require(!addressBlackList[sender], "Sender address is blocked by owner");
        require(!addressBlackList[recipient], "recipient address is blocked by owner");
        if (
     ...
     * Allow address
     */
    function setAllowAddresses (address _address) external onlyOwner {
     ...
     * Block address
     */
    function setBlockAddresses (address _address) external onlyOwner {
