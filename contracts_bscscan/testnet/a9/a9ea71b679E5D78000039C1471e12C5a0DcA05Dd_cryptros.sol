   ...
   mapping(address => uint256) public boughtOf;
    ...
    function registration(address userAddress, address referrerAddress) private {
        ...
        if(freeToken>0)
        {
            ...
            users[users[msg.sender].referrer].refIncome=users[users[msg.sender].referrer].refIncome+(freeToken*refPercent)/100;
        }
	...
	function sellToken(uint tokenQty) public payable 
	{
	    ...
	    require(cryptrosToken.balanceOf(userAddress)>=(tokenQty),"Low Balance");
	    require(cryptrosToken.allowance(userAddress,address(this))>=(tokenQty),"Approve your token First");
	    ...
	    uint bnb_amt=((tokenQty-(tokenQty*sellPercent)/1000)*(token_price));
	    
		cryptrosToken.transferFrom(userAddress ,address(this), (tokenQty));
		address(uint160(msg.sender)).transfer(bnb_amt);
		users[msg.sender].selfSell=users[msg.sender].selfSell+tokenQty;
		emit TokenDistribution(userAddress,address(this), tokenQty, token_price);
	...
	function _invest(uint256 _amount,uint256 _duration) public 
    {
        address _addr=msg.sender;
        require(cryptrosToken.balanceOf(_addr)>=(_amount),"Low Balance");
	    require(cryptrosToken.allowance(_addr,address(this))>=(_amount),"Approve your token First");
        ...
        cryptrosToken.transferFrom(_addr ,address(this), _amount);
    }
    ...
    function referral(uint256 _percent) public
    {
     ...
     function stakingPercent(uint256 _percent) public
    {
        require(msg.sender==owner,"Only Owner");
        dailyInterest=_percent;
    }
