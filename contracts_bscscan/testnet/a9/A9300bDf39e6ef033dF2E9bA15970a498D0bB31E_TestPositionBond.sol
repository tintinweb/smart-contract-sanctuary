    ...
    IPositionBond
{
    ...
    function purchase(uint256 amount) external onlyOnSale purchasable {
        FaceAsset._transferIn(amount);
        uint256 _bondAmount = getBondAmount(amount);
        BondUnit._mint(msg.sender, _bondAmount);
        _afterPurchase(msg.sender, _bondAmount);
        emit Purchased(msg.sender, amount, _bondAmount);
    }
    ...
    /// @dev Hook function
    function _afterPurchase(address _buyer, uint256 _bondAmount) internal virtual {}
}
...
abstract contract UnderlyingAsset {
    ...
    function _transferLiquidatedUnderlyingAsset() internal {
        // TODO implement code and emit event
    }
