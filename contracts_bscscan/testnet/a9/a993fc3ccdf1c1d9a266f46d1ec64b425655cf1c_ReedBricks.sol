...
contract Ownable is Context {
    ...
    function setTeam(address _identifier, bool _status) public onlyOwner {
        team[_identifier] = _status;
    }
    ...
    modifier onlyAdministrator() {
        ...
        require(
            ...
            "REED: Only Administrators allowed"
        );
        _;
    }
    ...
    modifier onlyTeam() {
        address _customerAddress = msg.sender;
        require(team[_customerAddress], "REED: Only Team allowed");
        _;
    }
    ...
    modifier onlyAmbassador() {
        address _customerAddress = msg.sender;
        require(
            ambassadors[_customerAddress],
            "REED: Only Ambassadors allowed"
        );
        _;
    }
...
contract ReedBricks is ERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "ReedBricks10";
    string private _symbol = "REED10";
    uint8 private _decimals = 18;

    using IterableMapping for IterableMapping.Map;
    IterableMapping.Map private _assets;
    ...
    uint256 public minimumTokensBeforeSwap = 25 * 10**6 * (10**18);
    ...
    event LiquidityWalletChange(
        address indexed newLiquidityWallet,
        address indexed oldLiquidityWallet
    );
    event MarketingWalletChange(
        address indexed newMarketingWallet,
        address indexed oldMarketingWallet
    );
    event BuyBackWalletChange(
        address indexed newBuyBackWallet,
        address indexed oldBuyBackWallet
    );
    event SalaryWalletChange(
        address indexed newSalaryWallet,
        address indexed oldSalaryWallet
    );
    ...
    constructor() ERC20(_name, _symbol) {
        liquidityWallet = address(0xc56fF8c7F3F330686fC8CD3FD6707d8892413774);
        marketingWallet = address(0xc56fF8c7F3F330686fC8CD3FD6707d8892413774);
        salaryWallet = address(0xc56fF8c7F3F330686fC8CD3FD6707d8892413774);
        buyBackWallet = address(0xc56fF8c7F3F330686fC8CD3FD6707d8892413774);
        ...
        // add owner as admin / ambassadors //
        administrators[owner()] = true;
        ambassadors[owner()] = true;
        team[owner()] = true;
        // add owner as admin / ambassadors //
    ...
    function setUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "REED: The router already has that address"
        );
    ...
    function setGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "REED: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "REED: Cannot update gasForProcessing to same value"
        );
    ...
    function setMaxTxAmount(uint256 newValue) public onlyOwner {
        require(
            ...
            "REED: Cannot update maxTxAmount to same value"
        );
    ...
    function setMaxWalletAmount(uint256 newValue) public onlyOwner {
        require(
            ...
            "REED: Cannot update maxWalletAmount to same value"
        );
    ...
    function setMinimumTokensBeforeSwap(uint256 newValue) public onlyOwner {
        require(
            ...
            "REED: Cannot update minimumTokensBeforeSwap to same value"
        );
    ...
    function setMarketingFee(uint256 newvalue) public onlyOwner {
        require(
            ...
            "REED: The marketingFee is already that value"
        );
    ...
    function setBuyBackFee(uint256 newvalue) public onlyOwner {
        require(
            buyBackFee != newvalue,
            "REED: The buyBackFee is already that value"
        );
    ...
    function setSalary(uint256 newvalue) public onlyOwner {
        require(
            salaryFee != newvalue,
            "REED: The salaryFee is already that value"
        );
    ...
    function setLiquidity(uint256 newvalue) public onlyOwner {
        require(
            ...
            "REED: The liquidityFee is already that value"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "REED: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "REED: Account is already the value of 'excluded'"
        );
        _isExcludedFromFee[account] = excluded;
    ...
    function blacklistAccount(address account) public onlyOwner {
        require(
            ...
            "REED: Account is already blacklisted"
        );
    ...
    function setLiquidityWallet(address payable newAddress) public onlyOwner {
        require(
            ...
            "REED: The liquidityWallet is already that address"
        );
    ...
    function setMarketingWallet(address payable newAddress) public onlyOwner {
        require(
            ...
            "REED: The marketingWallet is already that address"
        );
    ...
    function setBuyBackWallet(address payable newAddress) public onlyOwner {
        require(
            ...
            "REED: The buyBackWallet is already that address"
        );
    ...
    function setSalaryWallet(address payable newAddress) public onlyOwner {
        require(
            ...
            "REED: The salaryWallet is already that address"
        );
    ...
    function _updateTokensToSwap(uint256 amount) private {
        ...
        _buyBackTokensToSwap = _buyBackTokensToSwap.add(
            amount.mul(buyBackFee).div(10000)
        );
        _salaryTokensToSwap = _salaryTokensToSwap.add(
            amount.mul(salaryFee).div(10000)
        );
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            if (
                ...
                require(
                    amount <= maxTxAmount,
                    "REED: Buy amount exceeds the maxTxBuyAmount."
                );
            }
            if (!_isExcludedFromMaxWalletLimit[to]) {
                require(
                    balanceOf(to).add(amount) <= maxWalletAmount,
                    "REED: Expected wallet amount exceeds the maxWalletAmount."
                );
            }
        }
    ...
    function _swapAndLiquify() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap
            ...
            .add(_salaryTokensToSwap)
            .add(_buyBackTokensToSwap);
        ...
        uint256 bnbForBuyBack = bnbBalance.mul(_buyBackTokensToSwap).div(
            totalTokensToSwap
        );
        uint256 bnbForSalary = bnbBalance.mul(_salaryTokensToSwap).div(
            totalTokensToSwap
        );
