...
contract PositionToken is Context, IERC20, Ownable, Pausable {
    ...
    function _mint(address receiver, uint256 amount) private {
        ...
        _rTotal = _rTotal.add(amount.mul(_currentRate));//_currentRate.//mul(tSupply).add(_getRExcluded());
    ...
    function _burn(uint256 amount) private {
        ...
        // rTotal should be increased as well
        _rTotal = _rTotal.sub(amount.mul(_currentRate)); //_currentRate.mul(tSupply).add(_getRExcluded());
        uint256 _newRate = _getRate();
