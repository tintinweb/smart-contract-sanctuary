...
contract IMO is ReentrancyGuard {
    ...
    function getFundLimit(address _user) public view returns (uint256) {
        ...
        if (getTier(_user) == Tier.One) {
            fundLimit = hardCap[1].mul(imoInfo[imoNumber].raisingAmount).div(1e6);   
        }
        else if (getTier(_user) == Tier.Two){
            fundLimit = hardCap[2].mul(imoInfo[imoNumber].raisingAmount).div(1e6); 
        }
        else if (getTier(_user) == Tier.Three){
            fundLimit = hardCap[3].mul(imoInfo[imoNumber].raisingAmount).div(1e6); 
        }
        else if (getTier(_user) == Tier.Four){
            fundLimit = hardCap[4].mul(imoInfo[imoNumber].raisingAmount).div(1e6);
        }
        else if (getTier(_user) == Tier.Five){
            fundLimit = hardCap[5].mul(imoInfo[imoNumber].raisingAmount).div(1e6); 
        }
        return fundLimit;
    }
