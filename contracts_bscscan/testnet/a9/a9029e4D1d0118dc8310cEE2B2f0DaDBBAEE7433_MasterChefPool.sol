...
contract MasterChefPool is BasePool {
    ...
    constructor( IBEP20 assetToken,
        ...
        address platformFeeAddress) 
            BasePool(assetToken,
                egoToken,
                initialVEGORate, 1, 1,platformFeeAddress)//rate+different in dec numbers
    {
    ...
    function _stakeAsset(uint256 assetStakeAmount, StakerInfo memory stakerInfo, CelebrityInfo memory celebrityInfo) internal virtual override{
        ...
        uint256 vBalance =  assetStakeAmount * 1e18 / _getRate();
    ...
    function _celebrityClaimAsset(address celebrity, CelebrityInfo memory celebrityInfo)  internal virtual override {
        ...
        {
            celebrityInfo.assetBalanceInfo.vBalance -= claimAmount.vClaimAmount;
    ...
    function _prepareBalance(uint256 amount) private{
        ...
        _masterChef.leaveStaking(amount-balance);
    }
    ...
    function _unstakeAsset(StakerInfo memory stakerInfo, CelebrityInfo memory celebrityInfo, uint256 amount) internal virtual override {
        ...
        uint256 newVBalance = stakerInfo.assetBalanceInfo.amount *1e18 / _getRate();
        uint256 vChange = amount * 1e18 / _getRate() ;
        stakerInfo.assetBalanceInfo.vBalance = newVBalance;
        if(celebrityInfo.assetBalanceInfo.vBalance > vChange)
            celebrityInfo.assetBalanceInfo.vBalance -= vChange;
        else
