...
contract DoobVault is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public immutable doob;
    address public immutable sDoob;
    ...
    function stakeWithPermit(
        ...
        IERC2612Permit(doob).permit(msg.sender, address(this), amount_, expiry, v, r, s);
    ...
    /**
        @notice stake doob to enter escrow
        @param amount_ uint
    ...
    function _stake(uint256 amount_, address recipient_) internal returns (uint256) {
        ...
        IERC20(doob).safeTransferFrom(msg.sender, address(this), amount_);
        ...
        if (escrowPeriod > 0) {
            stakeInfo[recipient_] = Claim({
                ...
                gons: info.gons.add(ISDoob(sDoob).gonsForBalance(amount_)),
                expiry: expiryEpoch,
                lock: false
            });

            IERC20(sDoob).safeTransfer(escrowContract, amount_);
        } else {
            IERC20(sDoob).safeTransfer(recipient_, amount_);
        }
    ...
    /**
        @notice forfeit sDoob in escrow and retrieve doob
     */
    function forfeit() external lock {
        ...
        if (info.gons > 0) {
            ...
            IERC20(doob).safeTransfer(msg.sender, info.deposit);
    ...
    function redeemWithPermit(
        ...
        IERC2612Permit(sDoob).permit(msg.sender, address(this), amount_, expiry, v, r, s);

        _redeem(amount_, trigger_);
    }

    /**
        @notice redeem sDoob for doob
        @param amount_ uint
        @param trigger_ bool
     */
    ...
    function _redeem(uint256 amount_, bool trigger_) internal {
        ...
        IERC20(sDoob).safeTransferFrom(msg.sender, address(this), amount_);
        IERC20(doob).safeTransfer(msg.sender, amount_);

        emit LogRedeem(msg.sender, amount_, trigger_);
    }

    /**
        @notice returns the sDoob index, which tracks rebase growth
        @return uint
     */
    function index() public view returns (uint256) {
        return ISDoob(sDoob).index();
    }
    ...
    function rebase() public {
        if (epoch.nextBlock <= block.number) {
            ...
            uint256 staked = ISDoob(sDoob).circulatingSupply();
            ...
            ISDoob(sDoob).rebase(prevRewards, prevEpoch);
    ...
    /**
        @notice returns contract doob holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns (uint256) {
        return IERC20(doob).balanceOf(address(this)).add(totalBonus);
    }
    ...
    function giveLockBonus(uint256 amount_) external {
        ...
        IERC20(sDoob).safeTransfer(locker, amount_);
    }
    ...
    function returnLockBonus(uint256 amount_) external {
        ...
        IERC20(sDoob).safeTransferFrom(locker, address(this), amount_);
    }
