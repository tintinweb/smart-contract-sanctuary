...
import "./interfaces/IElfCoin.sol";
import "./SafeMath.sol";
import "./DividendDistributor.sol";

contract ElfCoin is ERC20, Ownable, IElfCoin {
    ...
    uint256 public constant maxFee = 15;
    ...
    uint256 public maxTransactionAmount = _totalTokens.div(400); // 0.25%
    ...
    function setFees(uint256 liquidity, uint256 marketing, uint256 reflection, uint256 buyBack) external onlyOwner
    {
        totalFee = liquidity.add(marketing).add(reflection).add(buyBack);

        require(totalFee <= maxFee, "totalFee cannot exceed maxFee");
    ...
    function setGasForProcessingDividends(uint256 gas) external onlyOwner
    {
        _dividendDistributor.setGasForProcessing(gas);
    }

    function setMarketPair(address pair, bool enabled) external onlyOwner
    {
        require(
            pair != uniswapV2Pair,
    ...
    function setMinimumDividendDistributionPeriod(uint256 period) external onlyOwner
    {
        _dividendDistributor.setMinimumDistributionPeriod(period);
    }

    function setMinimumDividendDistribution(uint256 distribution) external onlyOwner
    {
        _dividendDistributor.setMinimumDistribution(distribution);
    }

    function setSniper(address account, bool isASniper) external onlyOwner
    {
        require(
    ...
    function _swapBack(uint256 numTokensToSwap) internal swapping
    {
        ...
        uint256 bnbMarketing = bnbAmount.mul(marketingFee).div(bnbTotalFee);
...
interface IElfCoin {
    ...
    /**
     * @dev Enables or disables trading for all those except excluded from fees.
     */
     ...
     */
    function setFees(uint256 liquidity, uint256 marketing, uint256 reflection, uint256 buyBack) external;
