...
contract Pazzive is ERC20, Ownable {
    ...
    event SetLockDurationToBonus(uint256 duration, uint256 bonus);
    event EnableSwapAndLiquify(bool enabled);
    event EnableTokenLocking(bool enabled);
...
//        require(trackerAddress != address(dividendTracker), "Pazzive: already current tracker");
//
//        IPazDividendTracker newDividendTracker = PazDividendTracker(payable(trackerAddress));
//
...
//
//        emit SetDividendTracker(trackerAddress, address(dividendTracker));
//
...
//    }
    ...
    */
    ...
    function enableTokenLocking(bool enable) public onlyOwner {
...
//    }
    ...
    function handleStaking(address from) internal {
        if (!swapPairs[from] && lockingEnabled) {
            require(userLockEndTime[from] <= block.timestamp, "Pazzive: Tokens are locked during staking period");
...
/* todo: have this separate for better debugging
...
//  function name (  ) external view returns ( string );
  ...
  function setPazAddress ( address pazAddress ) external;
