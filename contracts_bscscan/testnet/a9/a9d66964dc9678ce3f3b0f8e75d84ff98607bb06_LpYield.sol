...
contract LpYield is Initializable, JaxOwnable {
    ...
    uint totalAmount;
    uint totalBusdStaked;

    mapping(address => UserInfo) userInfo;

    address public lpToken;           // Address of LP token contract.
    ...
    event Set_Fair_Price(uint fairPrice);
    ...
    function initialize (address admin_address, address _router, address _BUSD, address _WJAX) external initializer {
        ...
        lpToken = IPancakeFactory(router.factory()).getPair(WJAX, BUSD);
    ...
    function set_token_addresses(address _WJAX, address _BUSD) external onlyAdmin {
        WJAX = _WJAX;
        BUSD = _BUSD;
        lpToken = IPancakeFactory(router.factory()).getPair(_WJAX, _BUSD);
        IERC20(lpToken).approve(address(router), type(uint256).max);
    ...
    function setFairPrice(uint _fairPrice) external onlyGovernor {
        fairPrice = _fairPrice;
        emit Set_Fair_Price(_fairPrice);
    }
    ...
    function depositBUSD(uint amount) external {
        ...
        require(amounts[1] * (10 ** (18 - IERC20(WJAX).decimals())) * 1e8 / amounts[0] <= fairPrice, "can't deposit");
    ...
    function withdraw() external {
        (bool ret, ) = address(this).call(abi.encodeWithSignature("harvest()"));
        ...
        require(amounts[0] * (10 ** (18 - IERC20(WJAX).decimals())) * 1e8 / amounts[1] >= fairPrice * 95 / 100, "can't withdraw");
    ...
    function get_liquidity_ratio() public view returns(uint) { //1e8
        ...
        return busd_liquidity / wjax_supply_in_busd;
    }
