...
contract bbusd is ERC20, Ownable {
    ...
    address public _dividendToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; // busd
    ...
    address public buyBackWallet = 0xB632522d5eCD928C642eA3e0E4c20B9fe0fA5297;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(multiplier >= 100 && multiplier <= 500, "bbusd: Sell transaction multipler must be between 100 (1x) and 500 (5x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setTreasuryDivisor(uint256 divisor) external onlyOwner {
  	    require(treasuryDivisor >= 0 && treasuryDivisor <= 100, "bbusd: Treasury divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "bbusd: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "bbusd: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "bbusd: Dividend reward tax must be between 0 and 10");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "bbusd: The liquidity wallet is already this address");
...
contract bbusdDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("bbusd_Dividend_Tracker", "bbusd_Dividend_Tracker") {
    	claimWait = 3600;
