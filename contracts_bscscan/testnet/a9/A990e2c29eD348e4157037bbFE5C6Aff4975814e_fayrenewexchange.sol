...
contract Sale is Ownable{
    ...
    function _saleToken1(address payable from, address payable admin,uint256 tokenId, uint256 amount) internal{
    ...
    function _saleToken2(address payable from, address payable admin,uint256 tokenId, uint256 amount) internal{
        require(amount> order_place[from][tokenId].price , "Insufficent found");
        require(checkTokenApproval(tokenId, from));
        address payable create = address(uint160(_creator[tokenId]));
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], serviceValue);
    ...
    function calc1(uint256 amount, uint256 royal, uint256 _serviceValue) public view returns(uint256, uint256, uint256){
        uint256 fee = percent(amount, _serviceValue.div(10));
        uint256 ser=fee.div(2);
        uint256 or_am = amount.sub(ser);
        uint256 roy = percent(or_am, royal);
...
contract fayrenewexchange is Sale{
    ...
    function saleToken1(address payable from, address payable admin,uint256 tokenId, uint256 amount, address token, uint256 _type, uint256 NOFToken) public payable{
       _saleToken1(from, admin, tokenId, amount);
       if(_type == 721){
           BEP721 tok= BEP721(token);
    ...
    function saleToken2(address payable from, address payable admin,uint256 tokenId, uint256 amount, address token, uint256 _type, uint256 NOFToken) public payable{
       _saleToken2(from, admin, tokenId, amount);
       if(_type == 721){
           BEP721 tok= BEP721(token);
            if(checkOrder[tokenId][from]==true){
                delete order_place[from][tokenId];
