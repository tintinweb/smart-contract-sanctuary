...
interface IDividendDistributor {
    ...
    function getInfo(bool showDecimal) external view returns(uint256, uint256, uint256, uint256, uint256);
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    function getInfo(bool hideDecimal) public view returns (uint256, uint256, uint256, uint256, uint256) {
        if(hideDecimal){ return (totalShares.div(10 ** 18), totalDividends.div(10 ** 18), totalDistributed.div(10 ** 18), dividendsPerShare.div(10 ** 18), minDistribution.div(10 ** 18)); } 
        else return (totalShares, totalDividends, totalDistributed, dividendsPerShare, minDistribution);
    }
}
...
contract Definitive is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Definitive";
    string constant _symbol = "DEF2";    
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(previousPrice > 0){ checkMarketCondition(); } 
...
//        emit Transfer(sender, recipient, amount);
    ...
    function getTotalFee(address sender, address receiver) public view returns (uint256) {
        if (dynamicFees && receiver == pair) { 
            ...
            else if (marketDowntrend == 4) { return totalFee.mul(400).div(100);  }
            else if (marketDowntrend == 5) { return totalFee.mul(500).div(100);  }
         }
       // a downtrend offers opportunities to buy tokens with very low fees
            ...
            else if (marketDowntrend == 2) { return totalFee.mul(20).div(100);  }
    ...
    function checkMarketCondition() internal {
          if (currentPrice.div(previousPrice).mul(100) <= 50) { marketDowntrend = 5; } // very very strong downtrend
          else if (currentPrice.div(previousPrice).mul(100) <= 60) { marketDowntrend = 4; } // very strong downtrend
          else if (currentPrice.div(previousPrice).mul(100) <= 70) { marketDowntrend = 3; } // strong downtrend
          else if (currentPrice.div(previousPrice).mul(100) <= 80) { marketDowntrend = 2; } // downtrend
          else if (currentPrice <= previousPrice) { marketDowntrend = 1; } // downtrend or not a uptrend
    ...
    function setDistributionCriteria(uint256 _minDistribution) external authorized {
        distributor.setDistributionCriteria(_minDistribution); // the minimum number of tokens to receive a share in the dividend
    }
    ...
    function getTotalFeesCollected() public view returns (uint256) {
        return totalFeesCollected.div(10 ** 18);
    }    
    
    function getDividendInfo(bool hideDecimal) public view returns (uint256 totalShares, uint256 totalDividends, uint256 totalDistributed, uint256 dividendsPerShare, uint256 minDistribution) {
