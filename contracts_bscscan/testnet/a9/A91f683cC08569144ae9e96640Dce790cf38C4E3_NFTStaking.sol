...
contract NFTStaking is ReentrancyGuard, DataStorage, Events, Ownable, Pausable {
    ...
    function unStake(uint256 poolId) external nonReentrant whenNotPaused {
        ...
        if (user.isUnStake == false && user.amount > 0 && ((pool.locked && block.timestamp >= pool.toTime) || !pool.locked)) {
            ...
            uint256 currentDividends = IUltisProxy(iUltisProxy).getUserDividends(user.start, user.checkpoint, user.amount, poolAmount, pool.rewardPerBlock, pool.toTime, TIME_STEP, user.nftId);
