...
contract HeroMallV3 is Ownable {
    ...
    uint256 private freeHeroSeries;
    uint256 private freeHeroRace;
    ...
    // address private usdtTokenAddress = 0x55d398326f99059fF775485246999027B3197955;//正式链usdt
    address private usdtTokenAddress = 0x337610d27c682E347C9cD60BD4b3b107C9d34dDd;//测试链usdt
    ...
    constructor(address _heroAddress,uint256 _periodTotalSupply,uint256 _period,uint256 _freeHeroSeries,uint256 _freeHeroRace){
        heroAddress = _heroAddress;
        periodTotalSupply = _periodTotalSupply;
        lastUpdateTime = block.timestamp;
        period = _period;
        freeHeroSeries = _freeHeroSeries;
        freeHeroRace = _freeHeroRace;
    }
    
    function recruit(uint256 priceIndex) public payable {
        require(isOpen,"the mall is Close");
        require(isPaidOpen,"can not recruit hero now");
        ...
        require(price.totalSupply>price.sellAmount,"this hero is sold out");
        ...
        require(heroDeploys.length>0,"all hero is sold out");
        uint256 index = getRandom() % length;
        string memory _name = heroDeploys[index].name;
        heroDeploys[index].index += 1;
        string memory name = string(abi.encodePacked(_name, "#", uint2str(heroDeploys[index].index)));

        if(price.currentPrice>0){
            if(price.currentToken!=address(0)){
                 if(price.currentToken == cwtTokenAddress){
                    IERC20(price.currentToken).transferFrom(msg.sender,receiverAddress,price.currentPrice);
                 }else{
                    IERC20(price.currentToken).transferFrom(msg.sender,address(this),price.currentPrice);
                 }   
            }else{
                require(msg.value == price.currentPrice,"price is worng");
            }
        }
        INFT(heroAddress).mint(msg.sender,name,1,price.rarity,price.series,price.race,price.mineralLevel);
        totalSupply += 1;
        periodSellAmount +=1;
        sellPrice[priceIndex].sellAmount +=1;
    }
    ...
    function recruitFree() public{
        ...
        INFT(heroAddress).mint(msg.sender,name,1,0,freeHeroSeries,freeHeroRace,1);
        totalSupply += 1;
        periodSellAmount +=1;
    ...
    function setFreeHeroSeries(uint256 _freeHeroSeries) public onlyController {
        freeHeroSeries = _freeHeroSeries;
    }

    function getFreeHeroSeries() public view returns (uint256) {
        return freeHeroSeries;
    }
    
    function setFreeHeroRace(uint256 _freeHeroRace) public onlyController {
        freeHeroRace = _freeHeroRace;
    }
    ...
    function withdrawToken(address tokenAddress,address account) public onlyOwner {
    ...
    function addPrice(address _exchangeToken,uint256 _rarity,uint256 _mineralLevel,uint256 _series,uint256 _race,uint256 _price,uint256 _totalSupply) public onlyController {
    ...
    function updatePrice(uint256 index,address _exchangeToken,uint256 _rarity,uint256 _mineralLevel,uint256 _series,uint256 _race,uint256 _price,uint256 _totalSupply) public onlyController {
        sellPrice[index].exchangeToken = _exchangeToken;
        sellPrice[index].rarity = _rarity;
        sellPrice[index].mineralLevel = _mineralLevel;
        sellPrice[index].series = _series;
        sellPrice[index].price = _price;
        sellPrice[index].totalSupply = _totalSupply;
        sellPrice[index].race = _race;
    }

    function deletePrice(uint256 index) public onlyController {
        uint256 length = sellPrice.length;
         for(uint256 idx = index; idx < length-1; idx++){
            sellPrice[idx] = sellPrice[idx+1];
         }
        sellPrice.pop();
    }
    ...
    function addHeroNameDeploy(string memory _name) public onlyController {
    ...
    function updateHeroNameDeploy(uint256 index,string memory _name,uint256 _index) public onlyController {
        heroDeploys[index].name = _name;
        heroDeploys[index].index = _index;
    }

    function deleteHeroNameDepoly(uint256 index) public onlyController {
         uint256 length = heroDeploys.length;
         for(uint256 idx = index; idx < length-1; idx++){
            heroDeploys[idx] = heroDeploys[idx+1];
         }
        heroDeploys.pop();
    }
    ...
    function getPriceByIndex(uint256 index) public view returns (CurrentPrice memory) {
        ...
        if(price.exchangeToken == usdtTokenAddress){
            _currentPrice = price.price * 1000;
            _currentToken = cwtTokenAddress;
            //  _currentPrice = price.price * _getPrice() / 1e18;
        }else{
            _currentPrice = price.price;
            _currentToken = price.exchangeToken;
        }
