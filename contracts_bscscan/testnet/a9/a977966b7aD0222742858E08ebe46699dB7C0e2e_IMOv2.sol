...
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
import "./IIMOFactory.sol";
import "./IIMOClonable.sol";
import "./IIMO.sol";
...
abstract contract IMO is IIMO, Initializable, OwnableUpgradeable{
    ...
    function getImo(uint256 imoIndex) public view returns(IIMOClonable){
        ...
        return IIMOClonable(_factory.imoAddresses(imoIndex));
    }
    ...
    function setImoFactory(address _factoryAddress) public onlyOwner{
        _factory = IIMOFactory(_factoryAddress);
    }
    ...
    function commit(uint256 imoIndex, uint256 amount) public{
        require(imoIndex < _factory.imoAddressesLength(), "IMO not found.");
...
interface IIMOFactory{
    ...
    function getImo(uint256 imoIndex) external view returns(address);
