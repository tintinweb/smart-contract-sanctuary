...
// File: contracts/LIMITv2DividendPayingToken.sol
pragma solidity ^0.7.6;
...
// File: contracts/LIMITv2.sol
pragma solidity ^0.7.6;
contract LIMITv2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LIMITv2DividendTracker public dividendTracker;
    ...
    constructor() public ERC20("GDIVIPLAN", "GDVPL") {
        ...
        dividendTracker = new LIMITv2DividendTracker();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "LIMITv2: The dividend tracker already has that address");

        LIMITv2DividendTracker newDividendTracker = LIMITv2DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "LIMITv2: The new dividend tracker must be owned by the LIMITv2 token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "LIMITv2: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "LIMITv2: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "LIMITv2: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LIMITv2: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LIMITv2: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "LIMITv2: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "LIMITv2: This account cannot send tokens until trading is enabled");
        }
...
contract LIMITv2DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("LIMITv2_Dividend_Tracker", "LIMITv2_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "LIMITv2_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "LIMITv2_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LIMITv2 contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "LIMITv2_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "LIMITv2_Dividend_Tracker: Cannot update claimWait to same value");
