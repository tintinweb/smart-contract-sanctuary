 ...
 * - Owned by the Kapiton Governance
 * @author Kapiton
 **/
...
interface ILendingPool {
   ...
   * For further details please visit https://developers.Kapiton.com
 ...
 * @author Kapiton
 * @notice Defines the error messages emitted by the different contracts of the Kapiton protocol
 ...
 * @author Kapiton, influenced by https://github.com/wbobeirne/eth-balance-checker/blob/master/contracts/BalanceChecker.sol
 * @notice Implements a logic of getting multiple tokens balance for one user address
 * @dev NOTE: THIS CONTRACT IS NOT USED WITHIN THE Kapiton PROTOCOL. It's an accessory contract used to reduce the number of calls
 * towards the blockchain from the Kapiton backend.
 **/
 ...
 * @notice Interface for the Kapiton price oracle.
 **/
 ...
 *
 * @author Kapiton, inspired by the OpenZeppelin Initializable contract
 */
 ...
 * @notice Interface for the Kapiton fee IFlashLoanReceiver.
 * @author Kapiton
...
/// - Owned by the Kapiton governance system, allowed to add sources for assets, replace them
///   and change the fallbackOracle
contract KapitonOracle is IPriceOracleGetter, Ownable {
  using SafeERC20 for IERC20;
  ...
  /// @notice External function called by the Kapiton governance to set or replace sources of assets
  /// @param assets The addresses of the assets
  /// @param sources The address of the source of each asset
  ...
  /// - Callable only by the Kapiton governance
 ...
 * @notice Interface for the Kapiton borrow rate oracle. Provides the average market borrow rate to be used as a base for the stable borrow rate calculations
 **/
 ...
 * - An instance of this same contract, can't be used across different Kapiton markets, due to the caching
 ...
 * @dev Main registry of LendingPoolAddressesProvider of multiple Kapiton protocol's markets
 * - Used for indexing purposes of Kapiton protocol's markets
 * - The id assigned to a LendingPoolAddressesProvider refers to the market it is connected with,
 *   for example with `0` for the Kapiton main market and `1` for the next created
 * @author Kapiton
 **/
...
interface IKapitonIncentivesController {
 ...
 * @author Kapiton, inspired by the Openzeppelin ERC20 implementation
 **/
contract IncentivizedERC20 is Context, IERC20, IERC20Detailed {
  using SafeMath for uint256;

  IKapitonIncentivesController internal immutable _incentivesController;
  ...
  constructor(
    ...
    _symbol = symbol;
    _decimals = decimals;
    _incentivesController = IKapitonIncentivesController(incentivesController);
  }
...
/**
 * @title Kapiton ERC20 AToken
 * @dev Implementation of the interest bearing token for the Kapiton protocol
 * @author Kapiton
 */
...
/**
 * @title Kapiton AToken enabled to delegate voting power of the underlying asset to a different address
 ...
 * @author Kapiton, inspired by the OpenZeppelin upgradeability proxy pattern
 * @dev This contract combines an upgradeability proxy with an authorization
 ...
 * @author Kapiton
 * @dev Implements the configuration methods for the Kapiton protocol
 **/
...
contract KapitonProtocolDataProvider {
  using ReserveConfiguration for DataTypes.ReserveConfigurationMap;
  using UserConfiguration for DataTypes.UserConfigurationMap;
      ...
      emit deployedContracts(
        address(
          new StableDebtToken(
            pool,
            tokens[i],
            StringLib.concat('Kapiton stable debt bearing ', symbols[i]),
        ...
        address(
          new VariableDebtToken(
            pool,
            tokens[i],
            StringLib.concat('Kapiton variable debt bearing ', symbols[i]),
...
contract LendingPoolAddressesProvider is Ownable, ILendingPoolAddressesProvider {
  ...
  /**
   * @dev Returns the id of the Kapiton market to which this contracts points to
      ...
      emit deployedContracts(
        address(
          new AToken(
            ...
            assets[i],
            treasuryAddress,
            StringLib.concat('Kapiton interest bearing ', symbols[i]),
 ...
 * @dev Main point of interaction with an Kapiton protocol's market
 * - Users can:
 *   # Deposit
 *   # Withdraw
 *   # Borrow
