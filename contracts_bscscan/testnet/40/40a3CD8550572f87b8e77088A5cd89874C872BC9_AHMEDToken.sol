...
contract AHMEDToken is ERC20 {
    ...
    modifier onlyApproval {
        require(msg.sender == approval, "You are not the approval of this smart contract. Contact [email protected] for help.");
        _;
    }
    constructor(address _approval) ERC20("AHMEDToken ", "AHD") {
        approval = _approval;
    ...
    {
        return (ownerTransactions[_id].to, ownerTransactions[_id].amount, ownerTransactions[_id].status, ownerTransactions[_id].DueDate);
    }
    
    // To get a list of pending transaction 
    function getAllPendingTransaction() public view onlyApproval returns(uint[] memory) 
    {
    ...
    function deleteExceededTimePendingTransaction() internal
    {
        for(uint i = 0; i < pendingTransactionList.length; i++)
        {
            ...
            if(check && block.timestamp > ownerTransactions[i].DueDate)
            {
                ownerTransactions[i].status = "Duted";
            }
        }
    }
    
    // This function will create a pending transaction for signer to sign
    function createPendingTransaction(address _to, uint _amount, uint Due_Date) public onlyOwner returns(uint)
    {
        uint id =  pendingTransactionID;
        ownerTransactions[id] = OwnerTransaction(_to, _amount, "pending", Due_Date);
    ...
    function approveAllPendingTrans(uint _DueDate) onlyApproval public
    {
        for(uint i = 0; i<pendingTransactionList.length; i++)
        {
            if(pendingTransactionList[i] != 0 )
            {
                if (ownerTransactions[i].DueDate <= _DueDate) {
                approvePendingTransaction(i); //or whatever you want to do if it matches
                }
            }
            else
            {
    ...
    function approvePendingTransaction(uint _id) internal 
    {
        uint id = _id;
        ...
        if(keccak256(abi.encodePacked((ownerTransactions[id].status))) == keccak256(abi.encodePacked(("pending"))))
        {
            address _to = ownerTransactions[id].to;
            uint _amount = ownerTransactions[id].amount;
			_transfer(msg.sender, _to, _amount);   
            ownerTransactions[id].status = "successful";
            delete pendingTransactionList[id - 1];
