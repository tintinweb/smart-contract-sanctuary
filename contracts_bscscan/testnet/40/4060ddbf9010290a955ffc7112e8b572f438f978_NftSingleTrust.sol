...
contract BEP721 is Context, BEP165, IBEP721, IBEP721Metadata {
    ...
    function safeTransferFrom(
        ...
        require(
            msg.sender == from || checker[tokenId][from] == true,
            "Not a Owner"
        );
    ...
    BEP721Burnable
{
    ...
    constructor(uint256 id, uint256 _serviceValue) BEP721("NTS", "NTS") {
        _tid = id;
    ...
    function mint(
        ...
        _royal[tokenId]["artist"].uaddress = royaddress[0];
        _royal[tokenId]["agent"].uaddress = royaddress[1];
        _royal[tokenId]["artist"].pecentr = royPer[0].mul(1e18);
        _royal[tokenId]["agent"].pecentr = royPer[1].mul(1e18);
        ...
        )
    {
        ...
        uint256 artist = pBEPent(amount, artist);
        uint256 agent = pBEPent(amount, agent);
        uint256 netamount = amount.sub(fee.add(artist).add(agent));
        fee = fee.add(fee);
        return (fee, artist, agent, netamount);
    }
    ...
    function newtokenasbid(
        ...
        t.transferFrom(msg.sender, _royal[tokenId]["agent"].uaddress, agent);
        t.transferFrom(msg.sender, _royal[tokenId]["artist"].uaddress, artist);
    ...
    function _acceptBId(
        ...
        require(
            approveValue >= _adminfee.add(agent.add(netamount).add(artist)),
            "Insufficient Balance"
        );
        t.transferFrom(from, admin, _adminfee);
        t.transferFrom(from, _royal[tokenId]["agent"].uaddress, agent);
        t.transferFrom(from, _royal[tokenId]["artist"].uaddress, artist);
    ...
    function _saleToken(
        ...
        require(
            msg.value == _adminfee.add(agent.add(netamount).add(artist)),
            "Insufficient Balance"
        );
        admin.transfer(_adminfee);
        create.transfer(artist);
        create2.transfer(agent);
