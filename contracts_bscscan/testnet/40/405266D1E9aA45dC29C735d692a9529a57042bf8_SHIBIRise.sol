...
contract SHIBIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SHIBI_Dividend_Tracker", "SHIBI_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "SHIBI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "SHIBI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SHIBI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "SHIBI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SHIBI_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SHIBIRise is ERC20, Ownable {
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    SHIBIDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SHIBIRise", "SHIBI") {

        dividendTracker = new SHIBIDividendTracker();
...
//Testnet
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SHIBI: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SHIBI: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SHIBI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SHIBI: Cannot update gasForProcessing to same value");
