...
contract Collector is Ownable {
    ...
    event SetProjectTwitter(uint256 id, string webset);
    ...
    event SetLaunchpadWebset(uint256 id, string desc);
    ...
    function setLaunchpadTelegram(uint256 id, string memory telegram) public onlyOwner {
        require(bytes(telegram).length <= 1024, "illegal desc");
        require(launchpads[id] != 0, "launchpad not exists");
        require(projectAvaliable[id], "launchpad not avaliable");
...
contract PoolInstance {
    ...
    mapping(uint256 => uint256) public sourceVoted;
    uint256 totalVoted;
    mapping(address => bool) public userFavorite;
    uint256 totalFavorite;
    ...
    function doVote(uint256 source, uint256 num) external {
        require(source < sources.length, "illegal source");
        sourceVoted[source] = sourceVoted[source].add(num);
        ...
        emit Vote(msg.sender, address(this), source, num);
    }
