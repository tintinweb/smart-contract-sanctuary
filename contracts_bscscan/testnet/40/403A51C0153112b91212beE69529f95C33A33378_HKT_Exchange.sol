...
contract HKT_Exchange is Ownable{
    ...
    function addToken0(address t0_) public onlyOwner{
        no.push(0);
    ...
    function closePair(uint id_) public onlyOwner checkRefrech returns(bool){
    ...
    // function swapExactToken0to1(uint amount_, address[] calldata path, address addr_) public checkRefrech {
    ...
    //     }
    //     uint _inAmount = amount_ * (100 - pairInfo[id].fee) / 100;
    //     require(pairInfo[id].dailyQuota + _inAmount < pairInfo[id].quota, 'out of quota');
    //     require(input == pairInfo[id].token0, 'wrong path');
    //     require(pairInfo[id].status, "null pair");
    //     require(calculate0to1(amount_, path) < IERC20(output).balanceOf(bank), 'out of Reserve');
    //     // uint _inAmount = amount_ * (100 - pairInfo[id].fee) / 100;
    //     pairInfo[id].dailyQuota += _inAmount;
    ...
    //     _swap(amount_, output, addr_, id, buy);
    //     emit SwapExactToken0to1(msg.sender,id,amount_);

    // }

    // function swapExactToken1to0(uint amount_, address[] calldata path, address addr_) public checkRefrech {
    //     require(amount_ > 0, 'no amount') ;
    //     require(path[0] != path[1], 'Exchange:  INVALID_PATH');
    //     address input;
    //     address output;
    ...
    //     }
    //     uint _inAmount = amount_ * (100 - pairInfo[id].fee) / 100;
    //     require(quota1to0[id].dailyQuota + _inAmount < quota1to0[id].quota, 'out of quota');
    //     require(pairInfo[id].tradeSwitch, "not open");
    //     require(pairInfo[id].status, "null pair");
    //     require(input == pairInfo[id].token1, 'wrong path');
    //     require(calculate1to0(amount_, path) < IERC20(output).balanceOf(bank), 'out of Reserve');
    //     // uint _inAmount = amount_ * (100 - pairInfo[id].fee) / 100;
    //     quota1to0[id].dailyQuota += amount_;
    //     uint de = DECIMALS(input).decimals();
    //     if (de != 18){
    //         uint x = Acc / 10 ** de;
    //         _inAmount = _inAmount / x ;
    ...
    //     uint sell = 2;
    //     IERC20(input).transferFrom(addr_, bank, _inAmount);
    //     _swap(amount_, output, addr_, id, sell);
    ...
    function calculateQuota(address[] calldata path) public view returns (uint _Q) {
        require(path[0] != path[1], 'Exchange:  INVALID_PATH');
        address input;
        address output;
        for (uint i; i < path.length - 1; i++){
        ...
        uint id = findID[input][output];
        if (input == pairInfo[id].token0){
            _Q = pairInfo[id].quota - pairInfo[id].dailyQuota;
        } else if(input == pairInfo[id].token1){
            _Q = quota1to0[id].quota - quota1to0[id].dailyQuota;
        }
    }
