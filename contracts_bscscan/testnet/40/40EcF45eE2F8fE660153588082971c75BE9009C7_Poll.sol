...
contract Poll  {
   ...
   mapping (address => bool) markVoter;
    ...
    modifier hasQuizStarted{
        require(time != 0, 'poll has not started');
        _;
        
    }
    
    modifier hasQuizEnded {
        require(block.timestamp > time, 'poll has not ended');
        _;
    ...
    function vote(string memory _option, address _receiver) public  hasVoted{
        require(time != 0, 'poll has not started');
        require(block.timestamp < time, 'poll has ended');
        require(TOBA1.balanceOf(msg.sender)/50**18 > 10);
        
        
         if(keccak256(abi.encodePacked((_option))) == keccak256(abi.encodePacked((A)))){
             TOBA1.transferFrom(msg.sender,_receiver, 50*10**18);
