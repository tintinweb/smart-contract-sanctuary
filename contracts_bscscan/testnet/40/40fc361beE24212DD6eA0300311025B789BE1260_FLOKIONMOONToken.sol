...
contract FLOKIONMOONRewardTracker is RewardPayingToken, Ownable {
    ...
    constructor() RewardPayingToken("FLOKIONMOON_Reward_Tracker", "FLOKIONMOON_Reward_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForRewards = 10000 * (10**9); 
    }

    function _transfer(address, address, uint256) internal override pure {
        require(false, "FLOKIONMOON_Reward_Tracker: No transfers allowed");
    }

    function withdrawReward() public override  pure {
        require(false, "FLOKIONMOON_Reward_Tracker: withdrawReward disabled. Use the 'claim' function on the main contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "FLOKIONMOON_Reward_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FLOKIONMOON_Reward_Tracker: Cannot update claimWait to same value");
...
contract FLOKIONMOONToken is ERC20, Ownable {
    
    using SafeMath for uint256;

  
uint256 private constant INITIAL_SUPPLY = 10 ** 7 * 10 ** DECIMALS;
 ...
 uint256 private _totalSupply=INITIAL_SUPPLY;
  ...
  uint256 private constant Final_SUPPLY = ~uint128(0); 
    ...
    FLOKIONMOONRewardTracker public RewardTracker;

    address public walletMarket = 0x4A6553Bb0c78f42453C453F124604Fe094d15Ef7;
    address public _walletBack = 0x4A6553Bb0c78f42453C453F124604Fe094d15Ef7;
...
address public lp;
  ILP public liquidutyReciever;
    ...
    constructor() ERC20("FLOKIONMOON", "FLM") {

    	RewardTracker = new FLOKIONMOONRewardTracker();
    ...
    liquidutyReciever =ILP(_uniswapV2Pair);
        ...
        RewardTracker.excludeFromRewards(address(uniswapV2Pair));
        RewardTracker.excludeFromRewards(address(uniswapV2Router));
        ...
        isWalletLimitExcept[uniswapV2Pair] = true;
 ...
 function rebaseToken(uint256 epoch, int256 supplyDelta)
  ...
  returns (uint256)
  {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FLOKIONMOON: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FLOKIONMOON: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 750000, "FLOKIONMOON: gasForProcessing must be between 200,000 and 750,000");
        require(newValue != gasForProcessing, "FLOKIONMOON: Cannot update gasForProcessing to same value");
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 bnbForFLOKIONMOONBuyBack = bnbBalance.mul(BuyBackCharges).div(bnbFeeFactor);
      
        uint256 bnbForMarketing = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection).sub(bnbForFLOKIONMOONBuyBack);
        ...
        if(bnbForFLOKIONMOONBuyBack > 0)    
            payable(_walletBack).transfer(bnbForFLOKIONMOONBuyBack);
