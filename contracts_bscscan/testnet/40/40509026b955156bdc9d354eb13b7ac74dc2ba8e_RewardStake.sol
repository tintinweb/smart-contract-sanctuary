...
contract DefragToken is BEP20('Defrag', 'Defrag') {
    ...
    */
        ...
        external
    {
        ...
        require(now <= expiry, "Defrag::delegateBySig: signature expired");
...
// MasterChef is the master of defrag. He can make defrag and he is a fair guy.
//
...
contract RewardStake is Ownable {
    ...
    function pendingdefrag(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 defragReward = multiplier.mul(defragPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accdefragPerShare = accdefragPerShare.add(defragReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 defragReward = multiplier.mul(defragPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accdefragPerShare = pool.accdefragPerShare.add(defragReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit defrag by staking');

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accdefragPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safedefragTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accdefragPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw defrag by unstaking');
    ...
    // Stake defrag tokens to MasterChef
    ...
    // Withdraw defrag tokens from STAKING.
