    ...
    function setWhiteListPaid(
        ...
        for (uint256 i = 0; i < _addressWhiteList.length; ++i) {
            ...
            nftsPaid = nftsPaid.sub(
                _allowances[i],
                "setWhiteListPaid: amount of token to mint exceeds the amount of tokens that have been prepaid"
            );
        }
    }
...
contract ElpisHeroMock is ERC721 {
    using Counters for Counters.Counter;

    Counters.Counter private _idTrackerHero;

    constructor() public ERC721("Elpis Hero", "EHO") {
        _idTrackerHero.increment();
    }

    function mint(address to) public {
        uint256 id = _idTrackerHero.current();
        _idTrackerHero.increment();
        _mint(to, id);
    }
}
