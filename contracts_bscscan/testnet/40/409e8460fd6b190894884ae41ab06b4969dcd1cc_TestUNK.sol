...
contract TestUNK {
    ...
    function purchase(uint8 _matrix, uint8 _slotId) external {
        ...
        uint commission = _activateSlots(msg.sender, _slotId, _matrix);
    ...
    function purchaseMatrix(uint8 _pool) external {
        ...
        (uint commission, uint _globlalreferrerpay, address _globalreferrer) = _activateMatrix(_pool, pools[_pool]);
        _processPayout(_globalreferrer, _globlalreferrerpay);
        _processPayout(admin, commission);
        emit EarnedProfit(_globalreferrer, msg.sender, 3, _pool, 0, _globlalreferrerpay);
    ...
    function _createAccount(address _addr, address _referrer) internal {
        ...
        users[_addr].workingPool[_addr].currentSlot = 1;
        users[_addr].autoPool[_addr].currentSlot = 1;

        (uint256 profitunilevel, ) = _sendLevelCommission(_addr, _referrer, 1, 1);
        (uint256 profitforcedmatrix, ) = _sendLevelCommission(_addr, _referrer, 1, 1);
        (uint commission, uint profitglobal,) = _activateMatrix(1, pools[1]);
        _processPayout(_referrer, (profitunilevel + profitforcedmatrix + profitglobal));
        
        uint256 _slotLimit = (slots[1].price * 90 / 100) * 2;
        uint256 _fee = _slotLimit * 10/100;
        _slotLimit = _slotLimit - (profitunilevel + profitforcedmatrix);
        _processPayout(admin, (commission + _fee + _slotLimit));
        
        emit Registration(_addr, _referrer, lastId);
    }


   function _activateSlots(address _addr, uint8 _slotId, uint8 _matrix) internal returns (uint) {
        if (_matrix == 1) {
    ...
    function _activateMatrix(uint8 _pool, uint256 _amount) internal returns (uint, uint, address) {
        ...
        uint _globlalreferrerpay = _amount * 80 / 100;
        ...
        _processPayout(_globalreferrer, _globlalreferrerpay);
        emit EarnedProfit(_globalreferrer, msg.sender, 3, _pool, 0, _globlalreferrerpay);
        return (commission, _globlalreferrerpay, _globalreferrer); 
    }
    ...
    function _sendDividends(address _addr, uint8 _slotId, uint8 _matrix) internal returns (uint, uint) {
        ...
        uint256 _fee = slots[_slotId].price - _slotLimit;
        
        if (_matrix == 2) {
            address _referrer = getReferrrers(1, _addr, 1);
            (uint256 profit, ) = _sendLevelCommission(_addr, _referrer, _matrix, _slotId);
            _processPayout(_referrer, profit);
            _slotLimit = _slotLimit - profit;
        }
        ...
        for (uint8 _level = 1; _level <= 10; _level++) {
            ...
            (uint _earning, uint _maxLimit, uint _bonus, uint8 _referrerslot) = _getLevelDistribution(_referrer, _slotId, _matrix, _level);
            
            if (_referrerslot < _slotId) {
    ...
    function _sendLevelCommission(address _addr, address _referrer, uint8 _matrix, uint8 _slotId) internal returns (uint256, address) {
        uint256 profit;
        uint256 commission;
        if (_referrer != address(0)) {
            (uint _earning, uint _maxLimit,,uint8 _referrerslot) = _getLevelDistribution(_referrer, _slotId, _matrix, 1);
            if (_slotId == 1) {
        ...
        if (profit > 0) {
            if (_matrix == 1) {
                users[_referrer].workingPool[_referrer].earning += profit; 
            }
            else {
                users[_referrer].autoPool[_referrer].earning += profit; 
            }   
        }
        return (profit, _referrer);
    }

    function _getLevelDistribution(address _referrer, uint8 _slotId, uint8 _matrix, uint8 _level) public view returns(uint256, uint256, uint256, uint8) {
        uint256 earning;
        uint256 maxLimit;
        uint256 bonus;
        uint8 slot;
        if (_matrix == 1) {
            ...
            slot = users[_referrer].workingPool[_referrer].currentSlot;
            for (uint8 i = 1; i <= slot; i++) {
            ...
            slot = users[_referrer].autoPool[_referrer].currentSlot;
            for (uint8 i = 1; i <= slot; i++) {
                maxLimit += slots[i].limit;
            }
        }
    ...
    function getUserDetails(address _addr) public view returns (uint256, uint8[3] memory, uint256[2] memory, address[2] memory, address[] memory, address[] memory) {
        User memory account = users[_addr];
        U1Matrix memory uone = users[_addr].workingPool[_addr];
        U2Matrix memory utwo = users[_addr].autoPool[_addr];
        GlobalPool memory matrix = users[_addr].matrixPool[_addr];

        return(
            account.id,
            [uone.currentSlot, utwo.currentSlot, matrix.level],
            [uone.earning, utwo.earning],
