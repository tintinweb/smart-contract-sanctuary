...
interface KojiEarth {
    function GetPending(address _shareholder) external returns (uint256);
}
...
contract KojiSwap is Ownable, ReentrancyGuard {
    ...
    function hasPendingDividends(address holder) public returns (bool) {
        uint256 tempdivs = kojiearth.GetPending(holder);(holder);
        if (tempdivs > 1) {
            return true;
        } else {
            return false;
    ...
    function getPendingDividends(address holder) public returns (uint256) {
