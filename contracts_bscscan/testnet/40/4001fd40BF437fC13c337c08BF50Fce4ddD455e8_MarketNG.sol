...
contract MarketNG is IERC721Receiver, IERC1155Receiver, ReentrancyGuard, Ownable, Pausable {
    ...
    function run(
        ...
        require(detail.deadline > block.timestamp, 'deadline reached');
        require(marketSigners[detail.signer], 'unknown market signer');

        _validateOpCode(detail.opcode);
        require(
            verify(sigDetail, keccak256(abi.encode(detail)), detail.signer),
        ...
        if (hasSignedIntention(detail.opcode)) {
            ...
            require(
                verify(sigIntent, keccak256(encodedInt), intent.user),
    ...
    function _newBuyDeal(
        ...
        require(endTimestamp > block.timestamp, 'buy has already ended');
