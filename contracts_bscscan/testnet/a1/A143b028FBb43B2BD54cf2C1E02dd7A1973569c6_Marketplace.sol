...
contract Marketplace is Ownable, Pausable, MarketplaceStorage, IERC721Receiver {
  ...
  /**
    ...
    * @param _ownerCutPerMillion - owner cut per million, the fee for listing a token
    */
  constructor (
  ...
  /**
    ...
    */
  function setPublicationFeeInEther(uint256 _publicationFee) public onlyOwner {
    ...
    )
  {
    ...
    require(_isOnOrder(_order), "Token not published");
  ...
  {    
    ...
    require(_nftContract.ownerOf(_tokenId) == _seller, "Only the owner can create orders");
    ...
    orderByTokenId[_nftAddress][_tokenId] = _order;
  ...
  {    
    ...
    // Require that all auctions have a duration of
    // at least one minute. (Keeps our math from getting hairy!)
    require(_duration >= 1 minutes, "Duration should greater one minute");
  ...
  function cancelOrder(address _nftAddress, uint256 _tokenId) public whenNotPaused {
    ...
    IERC721 _nftContract = IERC721(_nftAddress);
    _nftContract.safeTransferFrom(address(this), _order.seller, _tokenId);
    ...
    internal returns (Order memory)
  {
    ...
    require(_bidAmount >= _price, "Bid amount must be higher than the current price");
...
contract MarketplaceStorage {
  ...
  // From ERC721/ERC1155 registry tokenId to Order (to avoid asset collision)
