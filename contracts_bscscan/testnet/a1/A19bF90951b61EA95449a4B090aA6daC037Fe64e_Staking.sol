...
contract Staking is Ownable {
    ...
    event WithdrawalSuccessful(address _wallet, uint256 _amount);
    event HarvestSuccessfull(address _wallet, uint256 _amount);
    event RewardsRecalculated(bool _success);

    constructor(uint256 _startBlock, IERC20 _depositToken) payable {
        startBlock = _startBlock;
        tokensPerBlock = msg.value.div(10512000);
        lastBlock = _startBlock + 10512000;
        blocksLeft = lastBlock - startBlock;
        tokensPerBlock = address(this).balance*precisionLevel.div(10512000); // precision level??
    ...
    function deposit(uint256 _amount) public {
        ...
        require(block.number < lastBlock, "Error: Contract has died");
    ...
    function getRewardDebt(address _address) public view returns (uint256) {
        return stakingWallets[_address].rewardDebt;
    }
    ...
    function withdraw(uint256 _amount) public {
        ...
        stakingWallets[msg.sender].depositAmount -= _amount;
        _recalculateRewardForAllUsers();
        _amount += stakingWallets[msg.sender].rewardDebt;
        stakingWallets[msg.sender].rewardDebt = 0;
    ...
    function harvest() public {
        ...
        stakingWallets[msg.sender].rewardDebt = 0;
        payable(msg.sender).transfer(toHarvest);
    ...
    function _recalculateRewardForAllUsers() internal {
        ...
        for (uint256 x = 0; x<numberOfUsers; x++) {
            _claimableAmount = _claimableAmount.add(stakingWallets[allAddresses[x]].rewardDebt);
        }
        uint256 _leftAmount = address(this).balance.sub(_claimableAmount);
        blocksLeft = lastBlock.sub(block.timestamp);
        tokensPerBlock = _leftAmount.div(blocksLeft);
        ...
        for (uint256 i = 0; i<numberOfUsers; i++) {
            ...
            stakingWallets[allAddresses[i]].rewardDebt = stakingWallets[allAddresses[i]].rewardDebt.add(_oldRewardPerBlock.mul(blocksPassed).div(precisionLevel));
        }
