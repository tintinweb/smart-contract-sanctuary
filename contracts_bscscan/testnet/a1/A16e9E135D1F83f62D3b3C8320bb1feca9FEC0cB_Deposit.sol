...
contract Deposit is Ownable {
    ...
    function setToken(address BLK_, address USDB_, address FLY_) public onlyOwner {
        BLK = IERC20(BLK_);
        USDB = IERC20(USDB_);
        FLY = IERC20(FLY_);
    }
    ...
    function getBLKPrice(address pair) public view returns (uint price_){
        (uint reserve0, uint reserve1,) = PancakePair(pair).getReserves();
        price_ = reserve1 * decimal / reserve0;
        // reserve0 A, reserve 1 U, it is U price of A
        return price_;
    }
    ...
    function claimInterest(uint slot_) public {
        ...
        require(userSlot[msg.sender][slot_].endTime - block.timestamp < 359 days + 85800, 'too early');
        uint temp;
    ...
    function claimAll() public {
        for (uint i = 0; i < 5; i++) {
            if (userSlot[msg.sender][i].status && userSlot[msg.sender][i].endTime - block.timestamp < 359 days + 85800) {
                claimInterest(i);
            } else {
                continue;
            }
    ...
    function countingAll(address addr_) public view returns (uint){
        uint out;
        for (uint i = 0; i < 5; i++) {
            if (userSlot[addr_][i].status && userSlot[addr_][i].endTime - block.timestamp < 359 days + 85800) {
    ...
    function unDeposite(uint slot_) public {
        ...
        require(info.endTime - block.timestamp < 359 days + 85800, 'too early');
        if (info.round <= 4) {
    ...
    function setPair(address addr_) public onlyOwner {
        BLKPair = addr_;
    }
