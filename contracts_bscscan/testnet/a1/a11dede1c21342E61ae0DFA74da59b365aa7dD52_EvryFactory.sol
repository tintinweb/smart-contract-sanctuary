...
contract EvryFactory is IEvryFactory, Ownable {
    ...
    address public override ownerToCreationPair;
    ...
    // modifier locked(Functions _fn) {
    //     require(msg.sender == feeToSetter, "Evry: FORBIDDEN");
    //     _;
    // }
    ...
    constructor(
        ...
        ownerToCreationPair = _feeToSetter;
    ...
    {
        require(
            msg.sender == ownerToCreationPair,
            "Evry: FORBIDDEN_FOR_CREATE_PAIR"
        );
    ...
    function setFeeToPlatform(address _feeToPlatform) external override {
        require(msg.sender == feeToSetter, "Evry: FORBIDDEN");
    ...
    function setOwnerToCreationPair(address _address) external override {
        require(msg.sender == ownerToCreationPair, "Evry: FORBIDDEN");
        ownerToCreationPair = _address;
    }

    function setPlatformFee(uint256 feeBasis) external override {
        require(msg.sender == feeToSetter, "Evry: FORBIDDEN");
        feePlatformBasis = feeBasis;
    }

    function setLiquidityFee(uint256 feeBasis) external override {
        require(msg.sender == feeToSetter, "Evry: FORBIDDEN");
...
interface IEvryFactory {
    ...
    function ownerToCreationPair() external view returns (address);
    ...
    function setOwnerToCreationPair(address) external;
