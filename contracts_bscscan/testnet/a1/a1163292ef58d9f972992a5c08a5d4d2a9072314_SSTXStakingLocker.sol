...
contract SSTXStakingLocker is Ownable {
    ...
    // Token Address
    IBEP20 constant token = IBEP20(0x6B93817896F57eE4E260e14d348921af522931D1);

    struct User{
        bool hasLocked;
        uint32 snapshotTime;
        uint256 stakedValue; //what is staked value?
    ...
    event Withdrawn(address userAddress, uint32 timestamp, uint256 value);
     ...
     // LOCKER
     // 1) Requires the deploy address to have value greater than or equal to amount deposited in locker
     //??? what does this mean?require(!person.hasLocked, "Can not lock again!");
