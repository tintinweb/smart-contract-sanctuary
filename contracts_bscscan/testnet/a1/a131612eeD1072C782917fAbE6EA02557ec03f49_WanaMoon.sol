...
contract WanaMoon is Context, IERC20, Ownable{
    ...
    uint256 private maxAntiSnipelimit = 2500 * 10**3 * 10**9;
    ...
    function isBot(address _bot) external onlyOwner view returns(bool) {
        return _isBot[_bot];
    }
    ...
    function isFullyLaunched() external view onlyOwner returns(bool) {
        if (isLaunched && block.number <= lastSnipeBlock) {
            return true;
        }
        return false;
    }
    ...
    function _transfer(
        ...
        // 2 min dealy for each buy this is valid only for initial 24h
         if (isLaunched && block.timestamp <= endSnipeLimitPeriod && from == pancakeV2Pair && !_isBot[to]) {
             // till 24hr you can buy only buy max
             require(
                amount <= maxAntiSnipelimit,
                "Early limit exceeded"
            );
