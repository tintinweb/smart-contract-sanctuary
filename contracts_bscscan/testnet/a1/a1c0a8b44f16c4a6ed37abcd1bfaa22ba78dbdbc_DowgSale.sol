...
contract DowgSale is Ownable {
  using SafeMath for uint256;
  TeamToken token;
  uint256 public constant RATE = 15000; // Number of tokens per Ether
  ...
  function isActive() public view returns (bool) {
    return (
        block.timestamp > 1639250983 && block.timestamp < 1640374183 && initialized == true// Goal must not already be reached
    );
  }
