...
contract PrivateSaleTokenVesting is Ownable, ReentrancyGuard {
    ...
    IERC20 public token;
    // in percent for percent of total tokens that user will receive at first claim, 100 = 1%
    uint256 public firstRelease;
    // moment when releasing the first (firstRelease/100)% of the tokens
    uint256 public startTime;
    // total amount of vesting periods
    uint256 public totalPeriods;
    // time in seconds for every vesting period
    uint256 public timePerPeriod;
    // delay (seconds) from _startTime after that monthly vesting starts
    uint256 public cliff;
    // current token hold
    uint256 public totalTokens;
    // amount of available balance for add new user
    uint256 public availablePool;
    // turn on when the first time when any user withdrawn token from pool
     ...
     *@param _startTime moment when releasing the first (_firstRelease/100)% of the tokens
    ...
    function fundVesting(uint256 _totalTokens) public onlyOwner {
        ...
        availablePool = availablePool.add(_totalTokens);
    ...
    function claimTokens() public nonReentrant {
        ...
        uint256 tokensToClaim = availableTokensToClaim();
        if (tokensToClaim > 0) {
            token.transfer(_sender, tokensToClaim);
            updateUserInfo(user, tokensToClaim);
            emit TokenClaimed(_sender, tokensToClaim);
        }
    }

    /**
     * @return available tokens to claimed
     */
    function availableTokensToClaim() public view returns (uint256) {
        address _sender = msg.sender;
        ...
        // user not added or already claimed all
        if (user.tokensClaimed.add(1) > user.amount) {
            return 0;
        }
    ...
    function updateUserInfo(UserInfo storage _user, uint256 _claim) internal {
        _user.tokensClaimed = _user.tokensClaimed.add(_claim);
        totalTokens = totalTokens.sub(_claim);
        if (!hasWithdrawn) {
            hasWithdrawn = true;
        }
    }
    ...
    function setStartTime(uint256 _startTime) public onlyOwner {
        ...
        require(!hasWithdrawn, "Vesting already started");
    ...
    function addWhitelistUser(address _address, uint256 _amount) public onlyOwner {
        require(startTime > block.timestamp, "Vesting has started, cannot add whitelist");

        UserInfo storage user = userInfo[_address];

        // return amount if user already created before
        if (user.amount != 0) {
            availablePool = availablePool.add(user.amount);
        }

        require(availablePool >= _amount, "Not enough available balance");

        user.amount = _amount;
        user.tokensClaimed = 0;

        availablePool = availablePool.sub(_amount);

        emit AddWhitelistUser(_address, _amount);
    }

    function removeWhitelistUser(address _address) public onlyOwner {
        require(block.timestamp < startTime, "Vesting has started, cannot remove whitelist");

        UserInfo storage user = userInfo[_address];

        // remove user should return amount of removed user to  availablePool
        availablePool = availablePool.add(user.amount);
