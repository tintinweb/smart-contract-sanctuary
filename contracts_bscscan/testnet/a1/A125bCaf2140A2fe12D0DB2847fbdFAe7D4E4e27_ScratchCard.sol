...
contract ScratchCard is Context, Ownable{
	...
	mapping(address=>uint256) public lastClaimablePrize;
    ...
    function BuyTicket() public{
		...
		require(cardsSold<=maxCards && GameOn==true,"Conditions for game start not met: Game hasnt started or it has finished");//Checks if machine has sold out	
		 ...
		 cardsSold+=1;
			...
			//_currentplayers+=1;
			uint256 currentRandomNumber = 16;//randomNumber(10000);
			...
			if(currentRandomNumber%6==0){
				...
				if(currentRandomNumber%16==0){
				    setValueInSpot(playerAddress,2500*10**18);	
				    wonLast[msg.sender]=true;
			}
	...
	function startGame() public onlyOwner{
	 ...
	 function isWinnerLast(address playerAddress) public view returns (bool){
	     return wonLast[playerAddress];
	 }
	 function setValueInSpot(address playerAddress, uint256 amountToSet) public onlyOwner{
	     lastClaimablePrize[playerAddress] = amountToSet;
