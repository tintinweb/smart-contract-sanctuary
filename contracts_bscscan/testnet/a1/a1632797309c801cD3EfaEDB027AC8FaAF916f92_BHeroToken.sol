  ...
  IRandomRequester
{
  struct CreateTokenRequest {
    bytes32 requestId; // VRF Chainlink request ID.
  ...
  function increaseClaimableTokens(
    ...
    for (uint256 i = 0; i < recipients.length; ++i) {
      claimableTokens[recipients[i]][rarity] += count;
    }
  }
  ...
  function requestCreateToken(
    ...
    // Create request.
    tokenRequests[to].push(CreateTokenRequest(requestId, uint32(orderId), uint16(count), uint8(rarity)));
  ...
  function createToken(
    ...
    for (uint256 i = 0; i < count; ++i) {
      ...
      (seed, details) = createRandomToken(seed, id, rarity);
  ...
  function createRandomToken(
    ...
    uint256 id,
    uint256 rarity
  ) internal returns (uint256 nextSeed, uint256 encodedDetails) {
    BHeroDetails.Details memory details;
    (nextSeed, details) = design.createRandomToken(seed, id, rarity);
  ...
  function upgrade(uint256 baseId, uint256 materialId) external {
    ...
    address to = msg.sender;
    require(tokenOwners[baseId] == to && tokenOwners[materialId] == to, "Token not owned");
...
interface IRandomGenerator {
  function requestRandom() external returns (bytes32, uint256);

  function getResult(bytes32 requestId, uint256 orderId) external view returns (uint256);
}

interface IRandomRequester {
  function requestRandomFulfilled(bytes32 requestId) external;
}
...
contract RandomGenerator is VRFConsumerBase, IRandomGenerator {
  ...
  mapping(bytes32 => RandomResult) public results;
  ...
  /** Requests randomness */
  function requestRandom() public override returns (bytes32, uint256) {
    ...
    if (requesting) {
      ...
      currentOrderId = 0;
      emit RequestRandom(currentRequestId);
    }
    return (currentRequestId, currentOrderId);
  }

  /** Callback function used by VRF Coordinator */
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    requesting = false;
    results[requestId] = RandomResult(randomness, true);
    emit RequestRandomFulfilled(requestId);
    randomRequester.requestRandomFulfilled(requestId);
  }

  function getResult(bytes32 requestId, uint256 orderId) external view override returns (uint256) {
    RandomResult storage result = results[requestId];
    require(result.completed, "Random result is not available");
    return uint256(keccak256(abi.encode(result.random, orderId)));
  }
}
...
interface IBHeroDesign {
  ...
  function createRandomToken(
    ...
    uint256 id,
    uint256 rarity
  ) external view returns (uint256 nextSeed, BHeroDetails.Details memory details);
}
...
contract BHeroDesign is AccessControlUpgradeable, UUPSUpgradeable, IBHeroDesign {
  ...
  function createRandomToken(
    ...
    uint256 id,
    uint256 rarity
  ) external view override returns (uint256 nextSeed, BHeroDetails.Details memory details) {
    details.id = id;
