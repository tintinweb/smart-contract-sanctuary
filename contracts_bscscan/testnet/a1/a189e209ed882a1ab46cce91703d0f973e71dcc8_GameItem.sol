...
/** 
 *  SourceUnit: d:\Projects\ftribe\smart-contracts\contracts\nfts\GameItem.sol
*/
...
contract GameItem is AccessControlEnumerable, ERC721Enumerable, ERC721Burnable  {
    ...
    function setURI(string memory uri) public virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "GameItem: must have admin role to set");

        require(bytes(uri).length > 0, "GameItem: uri is invalid");
    ...
    function mint(uint256 tokenId, address to) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "GameItem: must have minter role to mint");

        _mint(to, tokenId);
    }

    function mint(uint256 tokenId, bytes memory signature) public virtual {
        require(!nonces[tokenId], "GameItem: nonce was used");

        address msgSender = _msgSender();

        bytes32 message = keccak256(abi.encodePacked(tokenId, msgSender, _chainId, this)).prefixed();

        require(hasRole(SIGNER_ROLE, message.recoverSigner(signature)), "GameItem: signature is invalid");
    ...
    function mintBatch(uint256[] memory tokenIds, address[] memory accounts) public virtual {
        ...
        require(length > 0 && length == accounts.length, "GameItem: array length is invalid");
