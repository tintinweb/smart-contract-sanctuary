...
contract ELVN is IERC20, Ownable {
    ...
    function _transfer(
        ...
        // people won't pay tax when they buying tokens from dexes
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || _isDex[from]) {
            takeFee = false;
        }
        ...
        returns (uint256[8] memory)
    {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(
            [tAmount, tFee, tLiquidity, tPool, tBurn, _getRate()]
        );
        return (
            [
                rAmount,
    ...
    function _getRValues(uint256[6] memory values)
        private
        pure
        returns (
            uint256,
    ...
    {
        uint256 rAmount = values[0].mul(values[5]); // tAmount
        uint256 rFee = values[1].mul(values[5]); // tFee
        uint256 rLiquidity = values[2].mul(values[5]); // tLiquidity
        uint256 rPool = values[3].mul(values[5]); // tPool
        uint256 rBurn = values[4].mul(values[5]);
    ...
    function _transferBothExcluded(
        ...
        if(values[7] != 0) _takeBurn(values[7], sender);
    ...
    function setDexAddr(address _who, bool _case) external onlyOwner {
        _isDex[_who] = _case;
    }
}