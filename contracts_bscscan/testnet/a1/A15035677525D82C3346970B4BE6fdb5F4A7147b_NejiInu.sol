...
contract NEJIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("NEJI_Dividend_Tracker", "NEJI_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "NEJI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "NEJI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NEJI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "NEJI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "NEJI_Dividend_Tracker: Cannot update claimWait to same value");
...
contract NejiInu is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _DevsFee) public onlyOwner {
        BNBRewardsFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        DevsFee = _DevsFee;

        totalFees = BNBRewardsFee.add(liquidityFee).add(DevsFee); // total fee transfer and buy
    }
    ...
    function setDevsWallet(address payable _newDevsWallet) public onlyOwner {
        DevsWallet = _newDevsWallet;
    }

    constructor() ERC20("NEJI INU", "NEJI") {
        BNBRewardsFee = 0;
        liquidityFee = 1;
        extraFeeOnSell = 0; // extra fee on sell
        DevsFee = 1;
        DevsWallet = payable(0x58842c8f3E3F29A855ee2690461bAd59a2195660);

        totalFees = BNBRewardsFee.add(liquidityFee).add(DevsFee); // total fee transfer and buy

    	dividendTracker = new NEJIDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "NEJI: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "NEJI: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "NEJI: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "NEJI: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "NEJI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "NEJI: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // worthy Devs fee
        uint256 DevsAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(DevsFee);
    ...
    function swapAndSendBNBToDevs(uint256 tokenAmount) private {
        swapTokensForBnb(tokenAmount, DevsWallet);
    }
