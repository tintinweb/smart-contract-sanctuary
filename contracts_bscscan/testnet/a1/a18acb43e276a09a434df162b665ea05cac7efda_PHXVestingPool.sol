...
//    using Fraction for fractionNumber;
    ...
    */
    function fixedLoge(uint256 _x) internal pure returns (uint256 logE) {
        ...
        for `ln(2)`. This one is used since itΓÇÖs the max accuracy 
        of Python `ln(2)`
    ...
    */
    ...
    /**
        fixedExp is a ΓÇÿprotectedΓÇÖ version of `fixedExpUnsafe`, which 
    ...
    function fixedExp(uint256 _x) internal pure returns (uint256) {
       ...
       /**
        ...
        def calculateFactorials():
            ΓÇ£ΓÇ¥"Method to print out the factorials for fixedExpΓÇ¥ΓÇ£ΓÇ¥
            ...
            print( ΓÇ£\n        ΓÇ£.join([ΓÇ£xi = (xi * _x) >> PRECISION;\n        res += xi * %s;ΓÇ¥ % hex(int(x)) for x in ni]))
