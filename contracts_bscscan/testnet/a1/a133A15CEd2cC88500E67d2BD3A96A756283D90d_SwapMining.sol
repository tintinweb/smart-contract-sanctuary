...
library HakuswapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal view returns (address pair) {
        return IHakuswapFactory(factory).getPair(tokenA, tokenB);
    }
    ...
    function getAggregationAmountsOut(address[] memory factories, uint[] memory fees, uint[] memory minAmounts, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts, address[] memory usedFactories) {
        require(path.length >= 2, 'HakuswapLibrary: INVALID_PATH');
        require(factories.length == fees.length && path.length == minAmounts.length, "HakuswapLibrary: INVALID_AMOUNT");
        require(factories.length >= 1, 'HakuswapLibrary: INVALID_FACTORY');
...
contract SwapMining is Ownable {
    ...
    uint256 public halvingPeriod = 5256000 * 3;
