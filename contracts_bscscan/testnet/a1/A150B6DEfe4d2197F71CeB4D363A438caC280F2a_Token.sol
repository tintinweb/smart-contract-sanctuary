...
contract Token is Context, IERC20, Ownable {
    ...
    uint256 private _total = 10000000 * 10 ** 6 * 10 ** 9;
    uint256 private _feeTotal;

    string private _name = "QB Token";
    string private _symbol = "QB";
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        _owned[address(this)] = _owned[address(this)].add(tLiquidity);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // 如果余额>numTokensSellToAddToLiquidity 那么就需要添加流动性
    ...
    function _getValues(uint256 amount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 transferAmount = amount.sub(fee).sub(liquidity);
        return (transferAmount, fee, liquidity);
    }

    function _reflectFee(uint256 fee) private {
        _owned[dao] = _owned[dao].add(fee);
