...
contract ERC_TOKEN is ERC20, Ownable, ReentrancyGuard {
    // public properties
        ...
        // addresses
            // contracts
                address public CONTRACT_UNISWAP_ROUTER = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
                address public CONTRACT_TOKEN_UNISWAP_PAIR;
                address public CONTRACT_TOKEN_BURN = 0x000000000000000000000000000000000000dEaD;
                address public CONTRACT_TOKEN_STAKE;
                address public CONTRACT_TOKEN_LOTTERY;
    ...
    // private properties
        ...
        // mappings (data storage)
            mapping(address => uint256) private _balances;
            mapping (address => bool) private _accountNoFees;
    ...
    constructor() ERC20("ERC_TOKEN", "ERC"){
        ...
        _uniswapRouter = IUniswapV2Router02(CONTRACT_UNISWAP_ROUTER);
        CONTRACT_TOKEN_UNISWAP_PAIR = IUniswapV2Factory(_uniswapRouter.factory()).createPair(
            address(this),
            CONTRACT_UNISWAP_ROUTER
        );
        _accountNoFees[address(this)] = true;
        emit Transfer(address(this), _msgSender(), _totalSupply);
    }

    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "ERC_TOKEN: Transfer from the zero address");
        require(to != address(0), "ERC_TOKEN: Transfer to the zero address");
        require(amount > 0, "ERC_TOKEN: Transfer amount must be greater than zero");
        uint256 contractTokenBalance = balanceOf(address(this));
        _swapAndLiquify(contractTokenBalance);
        bool taxes = true;
        if(_accountNoFees[from] || _accountNoFees[to]){
            taxes = false;
        }
        _tokenTransfer(from, to, amount, taxes);
        emit Transfer(address(this), _msgSender(), _totalSupply);
    }

    function _swapAndLiquify(uint256 contractTokenBalance) private swapLocking {
    ...
    function _swapTokensForNative(uint256 tokenAmount) private {
        ...
        _approve(address(this), address(CONTRACT_UNISWAP_ROUTER), tokenAmount);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool taxes) private {
        uint256 amountToSend = amount;
        if(taxes){
            uint256 amountTaxLiquidity = _tax(amount, taxLiquidity);
            uint256 amountTaxLottery = _tax(amount, taxLottery);
            uint256 amountTaxRedistribution = _tax(amount, taxRedistribution);
            uint256 amountTaxBurn = _tax(amount, taxBurn);            
            amountToSend = amount.sub(amountTaxLiquidity).sub(amountTaxLottery).sub(amountTaxRedistribution).sub(amountTaxBurn);
        }
        _balances[sender] = _balances[sender].sub(amountToSend);
