...
contract BettingPrediction is Ownable {
    ...
    function initialize(
        ...
        // operatorAddress=0xd3C1C1c23d9E689832C586D62Bb64620A3D7574f;
        operatorAddress=0x2486c562F0B41af2e28d899855980666390C92b2;
    }
    ...
    function setMatchStartTime(string memory _matchStartTime) external onlyOperator{
        matchStartTime = _matchStartTime;
    }
...
contract Factory{
    ...
    event bettingContractCreated(address indexed bettingAddress,uint256 eventId,string eventName,string eventCategory, int256 outcomeNumber, string positionAName, string positionBName, string positionCName, string matchStartTime);
    ...
    function createBettingContract(
        ...
        bytes32 salt = keccak256(abi.encodePacked(eventId, _eventName,_eventCategory, _outcomeNumber, _positionAName, _positionBName, _positionCName, _matchStartTime));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        IBettingPrediction(pair).initialize(eventId, _eventName,_eventCategory,_outcomeNumber, _positionAName, _positionBName, _positionCName, _matchStartTime);
        
        getBettingContract[pair]= eventId;
        allContracts.push(pair);
        
        emit bettingContractCreated(pair,eventId,_eventName,_eventCategory,_outcomeNumber, _positionAName, _positionBName, _positionCName, _matchStartTime);
    }
