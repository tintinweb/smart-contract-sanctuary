...
// File: contracts/SafeTokenV1Beta.sol
...
abstract contract Tokenomics {
    ...
    address internal greenDAOAddress =
        0x3D50fA80E27b3def024692B1daF30C9eE95f2423;
    address internal greenAllianceAddress =
        0xe9444d47f42475d8739Ec124f397FcfB1Ebaeb55;
    ...
    Tokenomics
{
    ...
    mapping(address => VestingItem) public deservers; // user -> vesting
    ...
    /// @param _beneficiary The community's id, to which the stake will belong to
    ...
    function deposit(
        ...
        deservers[_beneficiary].amount += _amount;
        // timeLocked = block.timestamp + 15 minutes;
        deservers[_beneficiary].lockExpires = uint128(
            block.timestamp + _timeLocked
        );
    ...
    /// @param _beneficiary The community's id, to which the vesting belongs
    /// @param _amount The amount to unstake
    function withdraw(address _beneficiary, uint256 _amount) external {
        VestingItem storage vestingData = deservers[_beneficiary];
        // Test timelock
        require(
            vestingData.lockExpires < block.timestamp,
            "Vesting still locked"
        );

        // Modify tne vesting details
        vestingData.amount -= _amount; // Will revert if the user tries to withdraw more than staked
        totalVesting -= _amount;
        // // Actually send the withdraw amount
        _transfer(_msgSender(), _beneficiary, _amount);
