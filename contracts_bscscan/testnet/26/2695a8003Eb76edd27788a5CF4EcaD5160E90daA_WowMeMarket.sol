...
// File: contracts/WowMeMarket.sol
...
contract WowMeMarket is OrderState, TransferSafe {
	...
	IWowMeMintableStore private _WowMeMintableStore;
	address public weth;
		...
		if (
			...
			if (order.orderType == 1 || order.orderType == 3) {
				if (!isStore) {
					...
					require(
						order.key.sellAsset.token == address(_WowMeMintableStore),
						"invalid sell asset"
					);
					erc1155safeMintTransferFrom(
						IWowMeMintableStore(order.key.sellAsset.token),
						order.key.owner,
						buyer,
						order.key.sellAsset.tokenId,
						1,
				...
				if (!isStore) {
					...
					require(
						order.key.buyAsset.token == address(_WowMeMintableStore),
						"invalid buy asset"
					);
					erc1155safeMintTransferFrom(
						IWowMeMintableStore(order.key.buyAsset.token),
						buyer,
						order.key.owner,
						order.key.buyAsset.tokenId,
						1,
	...
	function setMintableStore(address newMintableStore) external onlyOwner {
		...
		_WowMeMintableStore = IWowMeMintableStore(newMintableStore);
	...
	function WowMeMintableStore() external view returns (address) {
		return address(_WowMeMintableStore);
	}
}