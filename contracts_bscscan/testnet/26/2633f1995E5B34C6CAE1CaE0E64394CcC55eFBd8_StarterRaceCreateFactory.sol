...
// Custom
import "./interfaces/ITokenRacesCreateFactory.sol";
...
contract StarterRaceBasement is Ownable,IERC20,IERC20Metadata {
    ...
    function init(address tokenOwner, string memory symbols, string memory names, uint fixedSupply) external {
       require(msg.sender == address(ITokenRacesCreateFactory(msg.sender)), "Only StarterRaceCreateFactory"); 
...
contract StarterRaceCreateFactory is Ownable,ITokenRacesCreateFactory {    
    ...
    function startRacingToken(
        ...
        uint totalSupply,
        uint32 id) external override  returns (address starterRaceAddress) {
        require(msg.sender == tokenRacesRouter, "Only TokenRacesRouter");
        StarterRaceBasement starterRace = new StarterRaceBasement();
        starterRace.init(msg.sender, symbol, name, totalSupply);
        currentRaceId=id;
        return address(starterRace);
    }
...
interface ITokenRacesRouter {
    ...
    function setMaxLenghtRace(uint256 _maxLenght) external;
...
interface ITokenRacesCreateFactory {
