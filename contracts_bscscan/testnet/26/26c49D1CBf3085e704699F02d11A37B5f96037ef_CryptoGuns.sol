...
function duplicateUpgrade(uint _swatId, uint _targetId) public{
   ...
   require(ownerSwatCount[_msgSender()] > 0, "Swat Count not high enough");
      //ownerSwatCount[_msgSender()] = ownerSwatCount[_msgSender()] - 1;
...
//overrides duplicate functions begins here
  ...
  function  _burn(uint256 tokenId) internal virtual override(ERC721Upgradeable, ERC721URIStorageUpgradeable) {
           require(hasRole(BURNER_ROLE, msg.sender), "Must be assigned Burner role");
          uint256 index = swatIsAtIndex[tokenId];

        ownerSwatCount[msg.sender] = ownerSwatCount[msg.sender] - 1;
         delete userOwnedSwats[msg.sender][tokenId];
