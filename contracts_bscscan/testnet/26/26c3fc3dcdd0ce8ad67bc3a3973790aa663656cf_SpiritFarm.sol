...
contract SpiritFarm is Ownable {
    ...
    event AddressList(address indexed _account, address _inviteContract, address _spiritFragmentContract, address _dtuTokenContract, address _usdtTokenContract, address _oraclePairDtuAddress);
    ...
    event FarmYield(address indexed _account, uint256 _dayNum, uint256 _dailyReward, uint256 _farmJoinTotalCount);
    ...
    constructor () public {
          ...
          useAgentiaPayAmount = 400 * 10 ** 18;// pay 400 usdt
    }
    ...
    function toFarmYield() public returns (bool) {
        ...
        for(uint256 i=1;i<=farmJoinTotalCount;i++){
            if(block.timestamp<farmOrders[i].endTime){
                profitAmount = dailyReward.div(existSpiritCount);
        ...
        emit FarmYield(msg.sender,dayNum,dailyReward,farmJoinTotalCount);// set log
    ...
    function useAgentia(uint256 _index) public returns (bool) {
        ...
        // Transfer
        dtuTokenContract.safeTransferFrom(address(msg.sender), address(this), payDtuAmount);// Transfer dtu to farm address
    ...
    function setAddressList(address _inviteContract,address _spiritFragmentContract,address _dtuTokenContract,address _usdtTokenContract,address _oraclePairDtuAddress) public onlyOwner returns (bool) {
        ...
        emit AddressList(msg.sender, _inviteContract, _spiritFragmentContract, _dtuTokenContract, _usdtTokenContract, _oraclePairDtuAddress);
        return true;
    }
