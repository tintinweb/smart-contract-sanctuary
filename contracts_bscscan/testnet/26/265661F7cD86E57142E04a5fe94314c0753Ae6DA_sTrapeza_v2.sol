...
contract sTrapeza_v2 is IsFIDL, ERC20Permit {
    ...
    event LogContractsUpdated(address stakingContract, address prevsFIDLContract);
    ...
    address public prevsFIDLContract; // v1 contract 
    ...
    constructor(uint256 totalSupply_) ERC20("Staked OHM", "sOHM", 9) ERC20Permit("Staked OHM") {
    ...
    function setgFIDL(address _gFIDL) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gFIDL) == address(0), "gOHM:  gOHM already set");
        require(_gFIDL != address(0), "gOHM:  gOHM is not a valid contract");
        gFIDL = IgFIDL(_gFIDL);
    }

    // do this last
    function initialize(address stakingContract_, address prevsFIDLContract_ ) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");

        require(stakingContract_ != address(0), "Staking");
        stakingContract = stakingContract_;

        require(prevsFIDLContract_ != address(0), "prev sFIDL Contract");
        prevsFIDLContract = prevsFIDLContract_;

        // require(totalSupply_ != IsFIDL(prevsFIDLContract_).totalSupply(), "Incorrect totalSupply");
        ...
        emit LogContractsUpdated(stakingContract, prevsFIDLContract);
    ...
    // toG converts an sOHM balance to gFIDL terms. gFIDL is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gFIDL.balanceTo(amount);
    }

    // fromG converts a gFIDL balance to sOHM terms. sOHM is a 9 decimal token. balance given is in 9 decimal format.
    ...
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(IsFIDL(prevsFIDLContract).circulatingSupply()).add(gFIDL.balanceFrom(IERC20(address(gFIDL)).totalSupply())).add(
