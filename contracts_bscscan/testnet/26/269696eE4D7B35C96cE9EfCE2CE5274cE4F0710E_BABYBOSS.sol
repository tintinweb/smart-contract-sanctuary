...
contract BABYBOSS is Context, IBEP20, Ownable {
    ...
    uint256 private _numOfTokensToExchange = 3000000 * 10**6 * 10**9;
    ...
    function _getAntiDumpMultiplier() private view returns (uint256) {
        ...
        if (_penaltiesEnabled) {
            if (time_since_start > 1 * hour) { return (2);}
            else if (time_since_start > 2 * hour) { return (1);}
        }   else { return (3);}
    }
