...
contract TokenDividendTracker is DividendPayingToken, Ownable {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 60 && newClaimWait <= 259200, "77Exitus_Dividend_Tracker: claimWait must be between 1 minute and 3 days");
        require(newClaimWait != claimWait, "77Exitus_Dividend_Tracker: Cannot update claimWait to same value");
...
contract A77Exitus is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() public ERC20("77Exitus", "77Exitus") {

        _marketingWallet = 0x333e5922681EE29f515Db739e556c59133329921;
    	dividendTracker = new TokenDividendTracker("77Exitus_Dividend_Tracker", "77Exitus_Dividend_Tracker", DOGEToken);
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "77Exitus: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "77Exitus: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "77Exitus: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "77Exitus: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 2000000, "77Exitus: gasForProcessing must be between 200,000 and 2,000,000");
        require(newValue != gasForProcessing, "77Exitus: Cannot update gasForProcessing to same value");
