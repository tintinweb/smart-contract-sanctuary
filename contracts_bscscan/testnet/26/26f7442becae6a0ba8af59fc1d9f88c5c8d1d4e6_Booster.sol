...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public          rotatingThreshold = 1 * (10 ** 15);
    uint256 public          defaultThreshold = 1 * (10 ** 15);
 ...
 *
 */
contract Booster is IERC20, Context, Ownable, TokenRecover {
    ...
    uint    public  _spaceDustRate                  = 2;                   // Initially set to reward half the bonus allocation
        ...
        if(_enableMegaPay &&  ((!isMegaPayExempt[recipient] && (amount >= _megaPayMinimumSpend && _isBuy)) || (!isMegaPayExempt[sender] && (amount >= _megaPayMinimumTransfer && _isTransfer && _balances[sender] >= _megaPayMinimumHODL)))){
            ...
            if (_isTransfer){
                ...
                if (!disabledRewardDust[sender] && _spaceDust > 0) {
                    _balances[sender] = _balances[sender].add(_spaceDust.div(_spaceDustRate));
    ...
    function setDustDividers(uint256 _earnPayDustDivider,uint256 _earnPayDustMultiplier, uint256 _earnPayRewardRate, uint256 _dustDivider, uint256 _dustMultiplier, uint256 _rewardRateDivider, uint256 _rewardRateMultiplier) external onlyOwner {
        distributor.setDustDividers(_dustDivider, _dustMultiplier,  _rewardRateDivider, _rewardRateMultiplier);
        earnPayDustDivider = _earnPayDustDivider;
        earnPayDustMultiplier = _earnPayDustMultiplier;
        _spaceDustRate = _earnPayRewardRate;
    }
