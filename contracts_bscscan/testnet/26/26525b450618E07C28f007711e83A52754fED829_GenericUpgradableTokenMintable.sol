...
contract GenericUpgradableToken is ERC20BurnableUpgradeable {
    using SafeMath for uint256;
		...
		function init(string memory _name, string memory _symbol,
			uint256 _totalSupply, address owner)
		external {
...
contract GenericUpgradableTokenMintable is GenericUpgradableToken, IGenericUpgradableTokenMintable {
    ...
    /**
     * @dev Issues `amount` tokens from the caller.
     *
     * See {ERC20-_burn}.
     */
    function updateTotalSupply(address to, uint256 newTotalSupply) public override virtual initializer {
...
contract FerrumProxyTokenDeployer {
	...
	function deployToken(
		...
		uint256 totalSupply, address admin) external
	{
		bytes memory data = abi.encodeWithSignature(
			"init(string, string, uint256, address)", name, symbol, totalSupply, msg.sender);
	...
	function updateTotalSupplyMethodData(
		...
		data = abi.encodeWithSignature(
			"updateTotalSupply(address, uint256)", to, newTotalSupply);
	}
}
