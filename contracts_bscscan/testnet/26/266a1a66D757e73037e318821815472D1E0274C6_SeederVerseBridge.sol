...
contract SeederVerseBridge is OwnableUpgradeable, ERC721Holder, OracleClient {
    ...
    mapping(uint256 => address) public tokenIdToPlayer;
    mapping(uint256 => address) public ticketIdToPlayer;

    event seedmonDeposited(uint256 tokenId, address depositer);
    ...
    function deposit(uint256 tokenId) external {
        ...
        tokenIdToPlayer[tokenId] = msg.sender;
    ...
    function emergencyWithdraw(uint256 tokenId) external {
        seedmon.safeTransferFrom(address(this), msg.sender, tokenId);
    }

    function createWithdrawRequest(uint256 tokenId) external payable {
        address depositer = tokenIdToPlayer[tokenId];
        require(msg.sender == depositer, "SeederVerse: Unauthorized");

        ticketCounter.increment();
        ticketIdToPlayer[ticketCounter.current()] = msg.sender;
        requestWithdrawSeedmon(ticketCounter.current(), tokenId, msg.value);

        emit withDrawSeedmonRequested(ticketCounter.current(), tokenId, msg.sender);
    }

    function processWithdraw(uint256 ticketId, uint256 tokenId, uint256 exp)
        internal
        override
    {
        ...
        address player = ticketIdToPlayer[ticketId];
        seedmon.safeTransferFrom(address(this), player, tokenId);
        
        delete tokenIdToPlayer[tokenId];
        delete ticketIdToPlayer[ticketId];

        emit seedmonWithdrawn(ticketId, tokenId, player);
    }

    function uploadToken(uint256 amount) external {
    ...
    function requestDownloadToken(uint256 ticketId, uint256 amount) external {}

    function processDownloadToken() external {}
}
...
abstract contract OracleClient is IClient {
    ...
    function onWithdrawSeedmonReceived(uint256 requestId, uint256 tokenId, uint256 exp) override external {
