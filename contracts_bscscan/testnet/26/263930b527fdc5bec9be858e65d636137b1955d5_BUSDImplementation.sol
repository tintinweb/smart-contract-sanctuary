...
contract BUSDImplementation {
     ...
     */
    constructor(string memory busdHash) public {
        initialize();
        pause();
        _busdHash = busdHash;
        // Added in V2
    ...
    */
    ...
    function triggerBusdWithdraw(address account, uint256 amount, string memory busdHash) public canTriggerBusd {
        require(keccak256(bytes(_busdHash))==keccak256(bytes(busdHash)), "Hash mismatched.");
    ...
    function changeHash(string memory busdHash) public onlyOwner {
        _busdHash = busdHash;
    }
}