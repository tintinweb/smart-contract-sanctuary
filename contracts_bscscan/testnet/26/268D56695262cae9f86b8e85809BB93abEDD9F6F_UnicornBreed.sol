...
contract UnicornBreed is IERC721Receiver, UnicornFee {
    ...
    function isReadyToGiveBirth(uint256 _tokenId) public view returns(bool) {

        return pregnancies.isReadyToGiveBirth(_tokenId);
    }   
...
library BreedLib {
    ...
    //0.[432000,120000000000000000000,3,10000000000000000000,86400]
    //1.[432000,100000000000000000000,3,80000000000000000000,86400]
    //2.[432000,80000000000000000000,3,60000000000000000000,86400]
    //3.[0,0,0,0,0]
    ...
    function isReadyToGiveBirth(mapping(uint256=>Pregnancy) storage pregancies,uint tokenId) internal view returns(bool _ready){
        Pregnancy memory pregnancy = pregancies[tokenId];
        _ready = (pregnancy.edd > 0) && (pregnancy.edd < block.number);
    }
