...
/**
 * Name: Aetherius Junior
 * Symbol: JUNIOR
 * Telegram: https://t.me/aetheriusjunior
 * Twitter: https://twitter.com/AetheriusJunior 
 */
...
contract AETH_DISTRIBUTOR is IDividendDistributor {
...
contract AetheriusJunior is Context, IBEP20, Ownable, ReentrancyGuard{ 
    using SafeMath for uint256;

    address AETH = (0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
    address WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    ...
    string constant _name = "AetheriusJunior";
    ...
    mapping (address => bool) public isAETHDividendExempt;
    ...
    constructor () {
        ...
        distributor = new AETH_DISTRIBUTOR(address(router));

        isFeeExempt[msg.sender] = true;
        isTxLimitExempt[msg.sender] = true;
        isAETHDividendExempt[pair] = true;
        isAETHDividendExempt[address(this)] = true;
        isAETHDividendExempt[burnAddress] = true;
        isAETHDividendExempt[zeroAddress] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isAETHDividendExempt[sender]){ try distributor.setShare(sender, _balances[sender]) {} catch {} }
        if(!isAETHDividendExempt[recipient]){ try distributor.setShare(recipient, _balances[recipient]) {} catch {} }
    ...
    function setIsDividendExempt(address holder, bool exempt) public onlyOwner {
        ...
        isAETHDividendExempt[holder] = exempt;
        if(exempt){
    ...
    function setAETHDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external onlyOwner {
        distributor.setDistributionCriteria(_minPeriod, _minDistribution);
    }

    function setAETHDistributorSettings(uint256 gas) external onlyOwner {
        require(gas < 750000);
        distributorGas = gas;
    }
