...
contract MultiTransfer is Pausable {
  ...
  payable external whenNotPaused returns(bool)
  {
    ...
    // solhint-disable-next-line avoid-low-level-calls, avoid-call-value
    (bool success, ) = _address1.call{ value: _amount1 }("");
    require(success);

    // solhint-disable-next-line avoid-low-level-calls, avoid-call-value
    (bool success2, ) = _address2.call{ value: _amount2 }("");
    require(success2);
...
contract MultiTransferTokenEther is Pausable {
  ...
  ) payable external whenNotPaused
  {
    ...
    for (uint8 i; i < _addresses.length; i++) {
      ...
      // solhint-disable-next-line avoid-low-level-calls, avoid-call-value
      (bool success, ) = _addresses[i].call{ value: _amountsEther[i] }("");
      require(success);
...
contract MultiTransferTokenEtherEqual is Pausable {
  ...
  ) payable external whenNotPaused
  {
    ...
    for (uint8 i; i < _addresses.length; i++) {
      ...
      // solhint-disable-next-line avoid-low-level-calls, avoid-call-value
      (bool success, ) = _addresses[i].call{ value: _amountEther }("");
      require(success);
    }
  }
}
