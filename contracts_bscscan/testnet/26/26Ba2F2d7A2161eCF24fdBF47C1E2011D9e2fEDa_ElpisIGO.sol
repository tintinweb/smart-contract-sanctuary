...
contract ElpisIGO is Pausable, Ownable {
    ...
    //Maximum number of tokens in necessaryClaim
    uint256 private constant amountPerClaim = 20;
    ...
    //List of addresses that can collect pre-sale slots
    mapping(address => bool) private collectors;
    ...
    function setCollector(address[] calldata _collectors) external onlyOwner {
        for (uint256 i = 0; i < _collectors.length; ++i) {
            collectors[_collectors[i]] = true;
        }
    }
    ...
    //Collect remaining pre-sale slots after pre-sale
    function collectPreSaleSlots(uint256 amount)
        external
        payable
        whenNotPaused
        validTransaction(amount)
        onlyFullPayment(amount)
    {
        require(
            block.number > blockFinishPreSale,
            "collectPreSaleSlots: pre-sale is not over yet"
        );
        require(
            block.number <= blockFinishPublicSale,
            "collectPreSaleSlots: IGO has ended"
        );
        require(
            collectors[address(msg.sender)],
            "collectPreSaleSlots: caller is not collector"
        );
        nftsPreSale = nftsPreSale.sub(
            amount,
            "collectPreSaleSlots: amount of token to mint exceeds the amount of pre sale tokens"
        );
        transferHero(address(msg.sender), amount);
    }

    // Claim heroes for prepaid adddresses without caring about times. NECESSARY ONLY
