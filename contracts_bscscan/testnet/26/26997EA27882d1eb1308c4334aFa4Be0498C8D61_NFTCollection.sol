...
contract NFTCollection is BaseContract {
    ...
    function _beforeMint() internal override {
        require(!alreadyMinted[msg.sender], 'The account has already minted the NFT');
...
contract BaseContract is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function reserveTokens(uint amount, string[] memory _tokenURI) external onlyOwner maxSupplyCheck(amount) {
    ...
    function giftTokens(address[] memory addresses, string[] memory _tokenURI) external onlyOwner maxSupplyCheck(addresses.length) {
    ...
    function mint(uint amountToMint, string[] memory _tokenURI) external payable maxSupplyCheck(amountToMint) {
    ...
    function withdraw(address payable recepient) external onlyOwner {
        ...
        payable(recepient).transfer(balance);
    }
