 ...
 */
 ...
 contract MarketTradingNFT is ERC721("Market Trading NFT", "NDSKNFT") {
    ...
    //@notice event emitted when NDSK token address is updated
    event NDSKTokenAddressUpdated(
        IBEP20 _newNDSK
    );

    //@notice event emitted when NDSK required token amount is updated
    event NDSKTokenAmountUpdated(
        uint256 _newAmount
    );
    ...
    /// @notice Structure of an offer
    struct Offer {
        uint256 salePrice;
    }
     /// @notice NFT token ID -> Offer params
    mapping(uint256 => Offer) public offers;

    /// @dev Total required token amount to mint
    uint256 public TOKEN_REQ_AMOUNT = 10;
     ...
     */
    constructor(MarketTradingAccessControls _accessControls, IBEP20 _ndsk, address _devAddr) public {
    ...
    function mint(address _beneficiary, string calldata _tokenUri) external returns (uint256) {
        ...
        //     "MarketTradingNFT.mint: Sender must have the admin or minter role"
        // );
        address _postCreator = _msgSender();
        // Valid args
        _assertMintingParamsValid(_tokenUri, _postCreator);
        
        //check the user's ndsk token account
        _MintingNdskTokenValid(_postCreator);

        uint256 tokenAmount = TOKEN_REQ_AMOUNT.mul(1e18);
        //send ndsk token to admin address
        ndsk.safeTransferFrom(address(_postCreator), devAddr, tokenAmount);
        ...
        postCreators[tokenId] = _msgSender();
    ...
    @dev Everyone who is the owner of the NFT can create an offer
   */
  function createOffer(
    uint256 _tokenId,
    uint256 _salePrice
  ) external {
    require(_exists(_tokenId), "NDSKNFTMARKET: createOffer: TokenID does not exist");
    require(
      ownerOf(_tokenId) == _msgSender() && isApproved(_tokenId, address(this)), "NDSKNFTMARKET: createOffer: Not owner or contract is not approved"
    );
    require(offers[_tokenId].salePrice == 0, "NDSKNFTMARKET: createOffer: This token is already on Market");
    // create offer
    _createOffer(
      _tokenId,
      _salePrice
  ...
  /**
    @notice accept opened offer
    @param _index uint256
   */
  function confirmOffer(uint256 _index) external {
    // uint256 _tokenId = categories[_category].products[_index];
    uint256 _tokenId = _index;
    // require(_msgSender().isContract() == false, "NDSKNFTMARKET: confirmOffer: No contracts permitted");

    Offer storage offer = offers[_tokenId];
    // Ensure this contract is still approved to move the token
    require(isApproved(_tokenId, address(this)), "NDSKNFTMARKET.confirmOffer: offer not approved");
    // price in ndsk token
    uint256 _salePrice = offer.salePrice;
    require(ndsk.allowance(msg.sender, address(this)) >= _salePrice, "NDSKNFTMARKET: confirmOffer: ndsk is not allowanced enough to purchase NFT");

    ndsk.transferFrom(msg.sender, ownerOf(_tokenId), _salePrice);

    // Transfer NFT token to the purchaser
    _transfer(ownerOf(_tokenId), msg.sender, _tokenId);

    // Remove offer
    delete offers[_tokenId];
    //removeByNonce(_category, _tokenId);
    // categories[_category].products[_index] = categories[_category].products[categories[_category].products.length - 1];
    // delete categories[_category].products[categories[_category].products.length - 1];

    // categories[_category].products.pop();

    // delete categoryState[_tokenId];
  }

    /**
    @notice Cancels an inflight and un-resulted offer
    @dev Only admin
    @param _index Token ID of the NFT being offered
  */
  function cancelOffer(uint256 _index) external {
    ...
    Offer storage offer = offers[_tokenId];
    require(
      ownerOf(_tokenId) == _msgSender(), "NDSKNFTMARKET: cancelOffer: Not owner or contract is not approved"
    );
    require(offer.salePrice != 0, "NDSKNFTMARKET: cancelOffer: Offer does not exist");
    // require(_getNow() <= offer.endTime, "NDSKNFTMARKET: cancelOffer: Offer already closed");
    // Remove offer
    delete offers[_tokenId];
    // removeByNonce(_category, _tokenId);
    // categories[_category].products[_index] = categories[_category].products[categories[_category].products.length - 1];
    ...
    /**
     @notice Update a NDSK token address
     @dev Only the owner or an approved sender can call this method
     @param _newNDSK new NDSK token address
     */
    function updateNDSKAddress(IBEP20 _newNDSK) external {
        address operator = _msgSender();
        require(
            accessControls.hasAdminRole(_msgSender()) || accessControls.hasSmartContractRole(_msgSender()),
            "MarketTradingNFT.mint: Sender must have the admin or contract role"
        ...
        // Destroy token mappings
        ndsk = _newNDSK;
        emit NDSKTokenAddressUpdated(_newNDSK);
    }

    /**
     @notice Update a required token amount
     @dev Only the owner or an approved sender can call this method
     @param _newAmount set required NDSK token amount
     */
    function updateReqNDSKTokenAmount(uint256 _newAmount) external {
        address operator = _msgSender();
        require(
            accessControls.hasAdminRole(_msgSender()) || accessControls.hasSmartContractRole(_msgSender()),
            "MarketTradingNFT.mint: Sender must have the admin or contract role"
        ...
        // Destroy token mappings
        TOKEN_REQ_AMOUNT = _newAmount;
        emit NDSKTokenAmountUpdated(_newAmount);
    }
    ...
    /**
     @notice Check minter has enough ndsk token amount
     @param _postCreator Address supplied on minting
     */
    function _MintingNdskTokenValid(address _postCreator) view internal{
        require(ndsk.balanceOf(_postCreator) >= TOKEN_REQ_AMOUNT , "Required Token: minter's NDSK token amount is less than required amount");
    }

     /**
    @notice Private method to create an offer
    @param _tokenId token ID of the NFT being offered to marketplace
    @param _salePrice offered price on marketplace
   */
    function _createOffer(
        uint256 _tokenId,
        uint256 _salePrice
    ) private {
        uint256 price = _salePrice.mul(1e18);
        offers[_tokenId] = Offer({
        salePrice: price
        });
        // categories[_category].products.push(_tokenId);
        // categoryState[_tokenId] = _category;
        // offer is created
    }
