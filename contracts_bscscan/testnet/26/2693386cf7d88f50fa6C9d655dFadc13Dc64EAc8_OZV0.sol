...
contract OZV0 is OZV0Events, Ownable, ReentrancyGuard{
  ...
  uint public tokenCounter;               // Number of tokens added by admin
  ...
  uint []  internal   startingPrice ;                                                     // Starting Price in each category 
  uint []  internal   counters;                                                           // Slot Counters
  uint []  internal   limitInSlot;                                                        // Slot Limit
  ...
  mapping(uint => address) public tokenAddresses;                                         // Counter to token address mapping
  ...
  */
  ...
  function completeKYC(address _referrer, address user,bool check) public onlyOwnerAccess  {
  ...
  */
  ...
  function slotReservedByAdmin(address user, uint index) public onlyOwnerAccess nonReentrant {
    require(users[user].id ==0, "OZV0 : User does not exists");
    require((privateSaleInDollars <= maxInvestmentCap*999),"OZV0: Private sell end");
    require(counters[index] < limitInSlot[index], "OZV0 : Slots already sold");
    completeKYC(rootNode, user, false);
    counters[index]++;
    users[user].tokenPrice = uint64(startingPrice[index] + ((counters[index] - 1)*8));
    users[user].userType = uint16(index);
    users[user].lastInvestmentTimestamp = uint32(block.timestamp);
    users[user].mintedOnce = true;
    privateSaleInDollars += maxInvestmentCap;
    emit SlotReservedByAdmin(user, users[user].userType, users[user].tokenPrice, block.timestamp, counters[index]);
  }
  ...
  */
  ...
  function purchaseTokens(uint128 value, address tokenAddress, address _referrer) public nonReentrant {
    ...
    if(value >= (maxInvestmentCap*paymentTokenDecimals)) {
      ...
      require((value >= 100 * paymentTokenDecimals*dollarDecimals), "OZV0: Investment less then 100$ and multiple of 100$ is allowed");
    }
  ...
  */
  ...
  function flashSale(address user, address tokenAddress, uint128 value ) public nonReentrant{
    require((users[_msgSender()].exists() || users[_msgSender()].KYC),"OZV0:User does not exists");
  ...
  */
  ...
  function purchaseOZBonusTokens(uint128 value, address tokenAddress) public nonReentrant{
    ...
    emit BonusAllocation( _msgSender(), (currentTokenPrice()/2), _tokens, block.timestamp, value, paymentTokenDecimals, users[_msgSender()].investmentCounter);
  }
  ...
  */
  ...
  function calculateDrip(address user, uint64 counter) public  view  returns (uint) {
    ...
    if(daysOver > 30) {
      ...
      return daysOver <= 1490 ? (dailyIncome*(daysOver-30))/1490 :  (dailyIncome*(1490))/1490 ;
    }else{
      return 0;
    }
  ...
  */
  ...
  function saveDetailsOfUser(address _referrer, address user, bool check) internal {
  ...
  */
  ...
  function reffralBonus(uint128 value, uint paymentTokenDecimals, address _referrer, address user, address tokenAddress, bool referralIncome) internal {
    ...
    emit ReffralIncome(user, _referrer, referralAmount, (ozOperationsamount-referralAmount), paymentTokenDecimals,block.timestamp);
  }
  ...
  function sellToken(address[]  memory tokenAddresses, uint [] memory numberOfTokens, uint64 investmentCounter) public nonReentrant {
    ...
    if(dollars < maxInvestmentCap) {
      ...
      if(soldInDollars + dollars < maxInvestmentCap) {
       ...
       emit SellToken(_msgSender(), _tokens, currentTokenPrice(), block.timestamp, investmentCounter);
        ...
        emit SellToken(_msgSender(), offsetTokens, currentTokenPrice(), block.timestamp, investmentCounter);
      ...
      for(uint i = 1;i <= n;i++){
        ...
        emit SellToken(_msgSender(), tokens, currentTokenPrice(), block.timestamp, investmentCounter);
        _publicSaleInDollars -= (maxInvestmentCap);
        _tokens += tokens;
    ...
    if(investmentCounter == uint256(0)){
      console.log(OZT.balanceOf(_msgSender()),_tokens);
