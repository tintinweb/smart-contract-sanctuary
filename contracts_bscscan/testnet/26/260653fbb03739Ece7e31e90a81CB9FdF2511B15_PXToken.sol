...
contract PXToken is ERC20, AccessControl, Pausable {
    // CONFIG START

    uint256 denominator = 100;
    
    // TOKEN
    string tokenName = "PixelDoge";
    string tokenSymbol = "PXLD";
    ...
    function handleFees(address sender, address recipient, uint256 amount) internal returns (uint256 fee) {
        ...
        if(isBuy) {
            fees = amount * 10**18 / denominator * (devTaxBuy + marketingTaxBuy + communityTaxBuy) / 10**18;
            ...
            fees = amount * 10**18 / denominator * (devTaxSell + marketingTaxSell + communityTaxSell) / 10**18;
    ...
    function setTaxesBuy(uint256 dev, uint256 marketing, uint256 community) external onlyRole(OWNER_ROLE) {
        devTaxBuy = dev;
        marketingTaxBuy = marketing;
        communityTaxBuy = community;
    }

    function setTaxesSell(uint256 dev, uint256 marketing, uint256 community) external onlyRole(OWNER_ROLE) {
        devTaxSell = dev;
