...
function buyBox(uint256 rank) public payable returns (uint256 characterId,uint256 diceId) {
        require(msg.value!=0,'msg.value must be greater than 0');
        require(msg.value==prices[rank],'Not equal value to buy');
        require(rank<=8,'Rank must be less than 8');
            uint256 value = uint256(block.timestamp);
            uint256 randomCharacterType = (value << 30)%10+1;
            characterId=characterNFT.createCharacter(msg.sender,randomCharacterType,rank,1,0);
            diceId=diceNFT.createDice(msg.sender,rank);
    }
