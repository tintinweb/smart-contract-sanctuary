...
contract deployerCRDN {
    ...
    function createPreSaleBNB(
        ...
        uint256 _vestingPercent = _hundred.sub(_preSaleData[0]).mul(100).div(_preSaleData[1]);

        for(uint i=1 ; i <= _preSaleData[1] ; i++){
            _vestingTime.push(_preSaleData[2].add(_month.mul(i)));
        }
    ...
    function setMonth(uint256 _duration) external onlyAdmin{
        _month = _duration;
    }
...
contract preSaleBnb {
    ...
    function claim() public allowed {
        ...
        if (amountRaised >= softCap) {
            if (claimCount[msg.sender] != 0) {
                require(
                    block.timestamp >= vestingTime[claimCount[msg.sender]],
            ...
            if (claimCount[msg.sender] == 0) {
                ...
                transferAmount = activePercentAmount[msg.sender];
                // .mul(
                //     multiplier
                // );
                token.transfer(msg.sender, transferAmount);
    ...
    function withdrawAndInitializePool() public onlyTokenOwner allowed {
        ...
        if (amountRaised > softCap) {
            ...
            pancakePair = IPancakeFactory(routerAddress.factory()).getPair(
                address(token),
                routerAddress.WETH()
            );
