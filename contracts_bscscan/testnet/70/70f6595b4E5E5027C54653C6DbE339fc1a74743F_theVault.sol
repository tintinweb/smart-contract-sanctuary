...
interface ICONSTSWAP {
    function getMintAmount(address tokenAddr, uint256 tokenAmountIn) external view returns (uint256, uint256);

    function mint(address tokenAddr, uint256 tokenAmountIn, uint256 stalMintedMin) external view returns (uint256);
}
...
contract theVault is ReentrancyGuard, Pausable, Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    using Address for address;

    address public _constSwap;
    address public _stal;
    address public _ibSTAL;
    ...
    constructor(
        ...
        require(constSwap != address(0), 'INVALID_CONSTSWAP');
        require(stal != address(0), 'INVALID_STAL');
        require(ibStal != address(0), 'INVALID_ibSTAL');

        _constSwap = constSwap;
        _stal = stal;
        _ibSTAL = ibStal;
    }
    ...
    function getTotalSupply() public view returns (uint256) {
        return IibSTAL(_ibSTAL).totalSupply();
    }

    function getIbSTALPrice(uint256 ibAmount) external view returns (uint256) {
        uint256 totalBalance = getTotalBalance();
        uint256 totalSupply = IibSTAL(_ibSTAL).totalSupply();
    ...
    function deposit(
        ...
        toMint = _toMint;
        IibSTAL(_ibSTAL).mint(msg.sender, toMint);

        emit Deposited(
            msg.sender,
            amountIn,
            IibSTAL(_ibSTAL).totalSupply()
        );
    }
    ...
    function getMintAmountByOther(
        ...
        (_stalAmountMin, _fee) = ICONSTSWAP(_constSwap).getMintAmount(tokenAddr, tokenAmountIn);
    ...
    function depositByOther(
        ...
        IERC20(tokenAddr).safeApprove(address(_constSwap), 0);
        IERC20(tokenAddr).safeApprove(address(_constSwap), tokenAmountIn);
        uint256 stalAmount = ICONSTSWAP(_constSwap).mint(tokenAddr, tokenAmountIn, _stalAmountMin);
    ...
    function getBurnAmountByStal(
        ...
        amountOut = ibAmount.mul(totalBalance).div(totalSupply);
    }
    ...
    function withdraw(
        ...
        IibSTAL(_ibSTAL).burnFrom(msg.sender, ibAmount);
