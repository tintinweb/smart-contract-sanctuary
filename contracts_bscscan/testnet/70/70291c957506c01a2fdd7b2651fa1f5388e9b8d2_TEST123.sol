...
contract TEST123 is Context, IERC20, Ownable { 
    ...
    address payable private _devWalletAddress = payable(0x418D809612A84DEd80E3C4116305AdFAedddef6D);
    address payable private _marketingWalletAddress = payable(0xb6A17A36CCF042aA48e03C259bC1Db5620DF708b);
    address payable private _developmentWalletAddress = payable(0xC11C7F1C6496D3860421e7331659Fe0fc81FF056);
    ...
    uint256 public _banLiquidityFee = 3;
    uint256 private _totalBNBFees = _marketingfee + _developmentfee + _devFee + _liquidityFee;
    uint256 private _banTotalBNBFees = _marketingfee + _developmentfee + _devFee + _banLiquidityFee;
    uint256 private _previousTotalFee = _totalBNBFees;
    uint256 private _blocks = 0;
    uint256 private _banBlocks = 0;
    ...
    function setBanBlocks(uint256 blocks) external onlyOwner {
        _blocks = blocks;
    }
    
     function setTradingIsEnabled(bool _enabled) external onlyOwner {
        _banBlocks = block.timestamp + _blocks;
    ...
    function _transfer(
        ...
        uint256 banTokensForLP = ((amountToSwap * _banLiquidityFee) / _banTotalBNBFees) / 2;

        uint256 initialBalance = address(this).balance;

        if (block.timestamp < _banBlocks)
            swapTokensForEth(amountToSwap - banTokensForLP);
        ...
        uint256 banTotalBNBFee = _banTotalBNBFees - _banLiquidityFee / 2;

        uint256 bnbToBeAddedToLiquidity = ((newBalance * _liquidityFee) / totalBNBFee) / 2;
        
        uint256 banBNBToBeAddedToLiquidity = ((newBalance * _banLiquidityFee) / banTotalBNBFee) / 2;

        if (banBNBToBeAddedToLiquidity > 0 && block.timestamp < _banBlocks)
            addLiquidity(banTokensForLP, banBNBToBeAddedToLiquidity);
            
        if (bnbToBeAddedToLiquidity > 0 && block.timestamp > _banBlocks)
        ...
        if (block.timestamp < _banBlocks)
            emit SwapAndLiquify(amountToSwap, newBalance, banTokensForLP);
        else
