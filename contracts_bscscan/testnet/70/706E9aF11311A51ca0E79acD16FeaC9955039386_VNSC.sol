...
contract VNSC is Context, Ownable, ERC165, IERC721Metadata {
    ...
    // function userFirstList(address user) public returns(address[]) {
    //     return _userFirst[user];
    // }

    // function userSecondList(address user) public returns(address[]) {
    //     return _userSec[user];
    // }

    function userInvitor(address user) public returns(address) {
        return _firstInvitors[user];
    }

    function userReword(address user) public returns(uint256) {
        return _reword[user];
    }
    ...
    function getNFTPrice(uint256 coinType) public view returns (uint256) {
        ...
        if (coinType == 3) {
            return 80000000000000000000;     //8bnb
