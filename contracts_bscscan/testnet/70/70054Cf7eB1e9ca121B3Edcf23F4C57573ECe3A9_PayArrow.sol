...
contract PayArrow is Ownable {
    ...
    function getInvestorInfoByUID(uint256 _uid) public view returns (uint256, uint256, uint256,  uint256, uint256[] memory, uint256[] memory) {
     ...
     function getLevelInfoByUID(uint256 _uid) public view returns (uint256[] memory, uint256[] memory, uint256[] memory) {
       
        Objects.Investor storage investor = uid2Investor[_uid];
        uint256[] memory level_ref = new uint256[](13);
        uint256[] memory level_invest = new  uint256[](13);
        uint256[] memory level_income = new  uint256[](13);
        for (uint256 i = 0; i < 13; i++) 
        {
            uint256 level=i+1;
           level_ref[i]=investor.levelRef[level];
           level_invest[i]=investor.levelInvest[level];
           level_income[i]=investor.levelIncome[level];
        }
        return
        (
        level_ref,
    ...
    function _invest(address _addr, uint256 _planId, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        for(uint256 i=1; i<14; i++) 
        {
				if (upline.addr != address(0)) {
				        if(upline.partners>=i || upline.partners>=10 || upline.addr==owner)
				        {
					     pacToken.transfer(upline.addr,(_amount*1)/100);
					     upline.levelIncome[i]=upline.levelIncome[i]+((_amount*1)/100);
  ...
  function withdrawLostBNBFromBalance(address payable _sender,uint256 _amt) public {
        require(msg.sender == owner, "onlyOwner");
        _sender.transfer(_amt*1e6);
    }
    
    function withdrawLostTokenFromBalance(address payable _sender,uint256 _amt) public {
        require(msg.sender == owner, "onlyOwner");
        pacToken.transfer(_sender,_amt);
    }
     ...
     function multisendTOKEN(address payable[]  memory  _contributors, uint256[] memory _balances) public payable {
        ...
        uint256 i = 0;
        for (i; i < _contributors.length; i++) {
            pacToken.transfer(_contributors[i],_balances[i]);
        }
    }
