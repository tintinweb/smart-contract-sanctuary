...
contract Staking is Ownable, ReentrancyGuard {
    ...
    *   @dev Emitted when the subscription is stopped (400.000 tokens)
    ...
    uint256 private constant _MIN_STAKE_AMOUNT = 100 * 10 ** 18;

    uint256 private constant _MAX_STAKE_AMOUNT = 100000 * 10 ** 18;

    uint private constant _REFERALL_REWARD = 333; //0.333% per day

    uint256 private constant _MAX_TOKEN_SUPPLY_LIMIT =     50000000 * 10 ** 18;
    uint256 private constant _MIDTERM_TOKEN_SUPPLY_LIMIT = 40000000 * 10 ** 18;
    ...
    *   @return The amount of referral reward related to the given customer
    ...
    function calculatePeriods(uint _stakeID) public view returns (uint){
        ...
        uint total_period = start.sub(creation_time);
