...
contract HODL_ELON is Context, IERC20, Ownable { 
    ...
    mapping (address => bool) private _isSnipe;
    mapping (address => bool) private _preLaunchAccess;

    // Exempt from sell penalty
    ...
    address payable public Wallet_Marketing = payable(0xf9631AA0eb8b36d64d9b452C5eC743E84A7c16b0); 
    ...
    uint256 private _Fee_LP_Promo = _FeeMarketing + _FeeLiquidity;
    ...
    constructor () {
        ...
        //IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); // MAIN NET BSC
    ...
    */
    ...
    function set_Sellers_Wait(uint _waitForRewards) external onlyOwner {
        ...
        // Wait time is in seconds 60*60*24*7 (604800) is one week
        require(_waitForRewards <= 604800, "That's far too long! Be reasonable!");
        waitForRewards = _waitForRewards;

    }

    //xxxxxxxx - change meme addrese
    // The meme tokens
    IERC20 HODL_F = IERC20(0x6Cf61eC79c178F833E0b30d52Baa0558A8770d2f);
    IERC20 HODL_B = IERC20(0x6Cf61eC79c178F833E0b30d52Baa0558A8770d2f);
    IERC20 HODL_G = IERC20(0x6Cf61eC79c178F833E0b30d52Baa0558A8770d2f);
    ...
    // Allows holder to check if their rewards are active
    ...
    */
    ...
    // Only used to prevent double processing during high volatility - processing is triggered by counter
    uint256 public triggerTokens = 1000*10**_decimals;


    function Update_Trigger_Token_Amount(uint256 number_of_tokens) public onlyOwner() {
       triggerTokens = number_of_tokens*10**_decimals;
    }  
    ...
    */
    ...
    function restoreAllFee() private {
        ...
        _Fee_LP_Promo = _FeeMarketing + _FeeLiquidity;
    }
    ...
    function _transfer(
        ...
        uint256 amount
          ...
          if (launchPhase){
                ...
                if (!TradeOpen){
                    ...
                    require(_preLaunchAccess[from] || from == owner() || to == owner(), "Trade is not open yet, please come back later");
                    }
                ...
                if(TradeOpen){
                        ...
                        if (launchBlock + 1 > block.number){
                            ...
                            if(_isPair[to] || from != owner() || to != owner()){
                            _isSnipe[to] = true;
        ...
        */
        ...
        if (to != owner() &&
            ...
            require((heldTokens + amount) <= _maxWalletToken,"You are trying to buy too many tokens. You have reached the limit for one wallet.");
                ...
                if (heldTokens == 0 && !_isExempt[to]){
                    ...
                    if ((HODL_F.balanceOf(to) < HODL_F_amount) || 
                        (HODL_B.balanceOf(to) < HODL_B_amount) || 
                        (HODL_G.balanceOf(to) < HODL_G_amount)) {
        ...
        if (_isPair[to] && !_isExempt[from] && from != address(this)){
        ...
        {  
            ...
            // Prevent double triggers from un-mined counter updates when blockchain is slow and traffic is high
            if(contractTokenBalance > triggerTokens){
            txCount = 0;
    ...
    if (_FeeMarketing != 0 && _FeeLiquidity != 0){
        ...
        marketingBNB = address(this).balance;
        sendToWallet(Wallet_Marketing, marketingBNB);


    } else if (_FeeMarketing != 0 && _FeeLiquidity == 0){
        ...
        if(balanceBeforeSwap > 50000000000000000)
    ...
    Creating Auto Liquidity - And sending the LP tokens directly to burn
    ...
    */
        ...
        uint256 private rLiquidity; //LP and Marketing
        ...
        uint256 private tLiquidity; //LP and Marketing
   ...
   function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        tLiquidity = tAmount*_Fee_LP_Promo/100;
