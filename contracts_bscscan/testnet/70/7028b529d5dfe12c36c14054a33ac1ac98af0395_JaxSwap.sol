...
interface IJaxSwap {
  ...
  event Freeze_Vrp_Wjxn_Exchange(uint flag);
...
contract JaxSwap is IJaxSwap, Initializable {
  ...
  uint priceImpactLimit;

  mapping (address => uint) public wjxn_wjax_ratios;
  uint public _wjxn_wjax_collateralization_ratio;
  uint public _wjax_collateralization_ratio;
  uint public freeze_vrp_wjxn_exchange;
  ...
  mapping (address => JToken) jtokens;
  address[] public jtoken_addresses;

  modifier onlyAdmin() {
    require(IJaxAdmin(jaxAdmin).userIsAdmin(msg.sender), "Not_Admin"); //Only Admin can perform this operation.
    _;
  }

  modifier onlyGovernor() {
    require(IJaxAdmin(jaxAdmin).userIsGovernor(msg.sender), "Not_Governor"); //Only Governor can perform this operation.
    _;
  }

  modifier onlyAjaxPrime() {
    require(IJaxAdmin(jaxAdmin).userIsAjaxPrime(msg.sender), "Not_AjaxPrime"); //Only AjaxPrime can perform this operation.
    _;
  }

  modifier isActive() {
      require(IJaxAdmin(jaxAdmin).system_status() == 2, "Swap_Paused"); //Swap has been paused by Admin.
      _;
  }
  ...
  function setJaxAdmin(address newJaxAdmin) public onlyAdmin {
    jaxAdmin = newJaxAdmin;
  ...
  function set_jtoken(address token, string calldata name, uint jusd_ratio, uint markup_fee, address markup_fee_wallet) public onlyGovernor {
  ...
  function swap_wjxn_wjax(uint amount) public onlyGovernor {
    address[] memory path = new address[](2);
    path[0] = address(wjxn);
    path[1] = address(wjax);
    JaxLibrary.swapWithPriceImpactLimit(address(router), amount, priceImpactLimit, path, address(this));
    
    (uint wjax_lsc_ratio, ,) = show_reserves();

    require(wjax_lsc_ratio <= _wjax_collateralization_ratio * 110 / 100, "Low Collateral"); //Unable to withdraw as collateral is fine.
    emit Swap_Wjxn_Wjax(amount);
  }

  function swap_wjax_wjxn(uint amount) public onlyGovernor {
    ...
    require(wjax_lsc_ratio >= _wjax_collateralization_ratio, "Low Reserves"); //Unable to withdraw as reserves are low.
  ...
  function set_use_wjxn_usd_dex_pair(uint flag) public onlyGovernor {
    use_wjxn_usd_dex_pair = flag;
    emit Set_Use_Wjxn_Usd_Dex_Pair(flag);
  }

  function set_use_wjax_usd_dex_pair(uint flag) public onlyGovernor {
    use_wjax_usd_dex_pair = flag;
    emit Set_Use_Wjax_Usd_Dex_Pair(flag);
  }

  function set_wjxn_usd_ratio(uint ratio) public onlyGovernor {
    wjxn_usd_ratio = ratio;
    emit Set_Wjxn_Usd_Ratio(ratio);
  }

  function set_wjax_usd_ratio(uint ratio) public onlyGovernor {
  ...
  function get_wjxn_wjax_ratio(uint withdrawal_amount) public view returns (uint) {
    if( wjax.balanceOf(address(this)) == 0 ) return 1e8;
    if( wjxn.balanceOf(address(this)) == 0 ) return 0;
    return ((10 ** wjax.decimals()) * (wjxn.balanceOf(address(this)) - withdrawal_amount) 
        * get_wjxn_jusd_ratio() * 1e8) / (wjax.balanceOf(address(this)) * get_wjax_jusd_ratio());
  }
  ...
  function freeze_vrp_wjxn_swap(uint flag) public onlyGovernor {
    freeze_vrp_wjxn_exchange = flag;
    emit Freeze_Vrp_Wjxn_Exchange(flag);
  }

  function swap_wjxn_vrp(uint amountIn) public isActive returns (uint) {
    ...
    uint vrp_to_be_minted = amountIn * get_wjxn_vrp_ratio();
    vrp.mint(msg.sender, vrp_to_be_minted);
    emit Swap_WJXN_VRP(msg.sender, amountIn, vrp_to_be_minted);
    return vrp_to_be_minted;
  }

  function swap_vrp_wjxn(uint amountIn) public isActive returns (uint) {
    require(freeze_vrp_wjxn_exchange == 0, "Freeze VRP-WJXN Swap"); //VRP-WJXN exchange is not allowed now.
    require(amountIn > 0, "Zero AmountIn");
    require(wjxn.balanceOf(address(this))> 0, "No Reserves.");
    uint wjxn_to_be_withdrawn = amountIn * get_vrp_wjxn_ratio() / (10 ** (18 * 2));
    ...
    require(wjxn_wjax_ratio_now >= _wjxn_wjax_collateralization_ratio, "Low Reserves"); //Unable to withdraw as reserves are low.
  ...
  function set_wjax_collateralization_ratio(uint wjax_collateralization_ratio) public onlyGovernor {
    _wjax_collateralization_ratio = wjax_collateralization_ratio;
    emit Set_Wjax_Collateralization_Ratio(wjax_collateralization_ratio);
  }
  ...
  function swap_wjax_jusd(uint wjax_amount) public isActive{
    // Calculate fee
    uint fee_amount = wjax_amount * wjax_jusd_markup_fee / 1e8;
    ...
    uint jusd_amount = (wjax_amount - fee_amount) * get_wjax_jusd_ratio() / (10 ** wjax.decimals());

    jusd.mint(msg.sender, jusd_amount);
		emit Swap_WJAX_JUSD(msg.sender, wjax_amount, jusd_amount);
	}

  function swap_jusd_wjax(uint jusd_amount) public isActive returns (uint) {
		require(jusd.balanceOf(msg.sender) >= jusd_amount, "Insufficient fund");
    uint fee_amount = jusd_amount * wjax_jusd_markup_fee / 1e8;
    uint wjax_amount = (jusd_amount - fee_amount) * (10 ** wjax.decimals()) / get_wjax_jusd_ratio();
  ...
  function swap_jusd_jtoken(address jtoken, uint amountIn) public isActive returns (uint) {
    JToken storage token = jtokens[jtoken];
  ...
  function swap_jtoken_jusd(address jtoken, uint amountIn) public isActive returns (uint) {
    JToken storage token = jtokens[jtoken];
    uint ratio = token.jusd_ratio;
    require(ratio > 0, "Zero Ratio"); //ratio is not set for this token
  ...
  function swap_jtoken_busd(address jtoken, uint amountIn) public isActive returns (uint) {
    JToken storage token = jtokens[jtoken];
    ...
    uint fee_amount = (jusd_amountOut - jusd_markup_fee) * wjax_jusd_markup_fee / 1e8;
    uint wjax_amount = (jusd_amountOut - jusd_markup_fee - fee_amount) * (10 ** wjax.decimals()) / get_wjax_jusd_ratio();
		require(wjax.balanceOf(address(this)) >= wjax_amount, "Insufficient reserves");
    // jusd.burnFrom(msg.sender, jusd_amount);
    jusd.mint(wjax_jusd_markup_fee_wallet, fee_amount);
    // The recipient has to pay fee.
    ...
    uint[] memory amounts = JaxLibrary.swapWithPriceImpactLimit(address(router), wjax_amount, priceImpactLimit, path, msg.sender);
    
		emit Swap_JToken_BUSD(jtoken, msg.sender, amountIn, amounts[1]);
    return amounts[1];
  }

  function swap_busd_jtoken(address jtoken, uint amountIn) public isActive returns(uint) {
    JToken storage token = jtokens[jtoken];
    ...
    path[0] = address(busd);
    path[1] = address(wjax);
    uint[] memory amounts = JaxLibrary.swapWithPriceImpactLimit(address(router), amountIn, priceImpactLimit, path, msg.sender);
    // Calculate fee
    uint wjax_fee = amounts[1] * wjax_jusd_markup_fee / 1e8;
    // markup fee wallet will receive fee
    // pay fee
    uint wjax_amountOut = amounts[1] - wjax_fee;
    wjax.transferFrom(msg.sender, wjax_jusd_markup_fee_wallet, wjax_fee);
    // wjax.transferFrom(msg.sender, address(this), wjax_amountOut);

    uint jusd_amount = wjax_amountOut * get_wjax_jusd_ratio() / (10 ** wjax.decimals());
  ...
  function swap_jusd_busd(uint amountIn) public isActive returns (uint) {
    uint fee_amount = amountIn * wjax_jusd_markup_fee / 1e8;
    uint wjax_amount = (amountIn - fee_amount) * (10 ** wjax.decimals()) / get_wjax_jusd_ratio();
    jusd.burnFrom(msg.sender, amountIn);
    jusd.mint(wjax_jusd_markup_fee_wallet, fee_amount);
    // The recipient has to pay fee.
    wjax.transfer(msg.sender, wjax_amount);
  ...
  function swap_busd_jusd(uint amountIn) public isActive{
    ...
    uint jusd_amount = (amounts[1] - wjax_fee) * get_wjax_jusd_ratio() / (10 ** wjax.decimals());

    jusd.mint(msg.sender, jusd_amount);
		emit Swap_WJAX_JUSD(msg.sender, amountIn, jusd_amount);
	}

  function setPriceImpactLimit(uint limit) public onlyGovernor {
    priceImpactLimit = limit;
    emit Set_Price_Impact_Limit(limit);
  }

  function initialize(address _jaxAdmin, address pancakeRouter, address _busd) public initializer {
...
library JaxLibrary {
  ...
  function swapWithPriceImpactLimit(address router, uint amountIn, uint limit, address[] memory path, address to) external returns(uint[] memory) {
