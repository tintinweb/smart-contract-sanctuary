...
contract Farming is Auth, Random {
    ...
    function _mint(Box memory _box_) internal returns(Lummy memory) {
        ...
        uint256 farmLimit = _box_.price * 2;
        if (players[msg.sender].referrer == address(0)) {
            farmLimit = farmLimit * 80 / 100;
        }

        if (random <= one) {
            uint256 power = _box_.price / stars[Stars.One] / 60;
            return Lummy(_counter, msg.sender, _box_.rarity, Stars.One, power, false, 0, farmLimit, 0);
        } else if (random <= two) {
            uint256 power = _box_.price / stars[Stars.Two] / 60;
            return Lummy(_counter, msg.sender, _box_.rarity, Stars.Two, power, false, 0, farmLimit, 0);
        } else if (random <= three) {
            uint256 power = _box_.price / stars[Stars.Three] / 60;
            return Lummy(_counter, msg.sender, _box_.rarity, Stars.Three, power, false, 0, farmLimit, 0);
        } else if (random <= four) {
            uint256 power = _box_.price / stars[Stars.Four] / 60;
            return Lummy(_counter, msg.sender, _box_.rarity, Stars.Four, power, false, 0, farmLimit, 0);
        } else {
            uint256 power = _box_.price / stars[Stars.Five] / 60;
            return Lummy(_counter, msg.sender, _box_.rarity, Stars.Five, power, false, 0, farmLimit, 0);
        }
    }
