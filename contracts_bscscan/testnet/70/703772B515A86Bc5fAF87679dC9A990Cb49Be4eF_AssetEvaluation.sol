...
contract DFY_Physical_NFT is ERC1155Upgradeable, DFYAccessControl, PausableUpgradeable, ERC1155BurnableUpgradeable {
...
contract AssetEvaluation is ERC1155HolderUpgradeable, PausableUpgradeable, DFYAccessControl{
    ...
    DFY_Physical_NFT public dfy_physical_nft;
    ...
    // Mapping owner => tokenId => Asset
    mapping (address => mapping(uint256 => Asset)) public listAssetNFTByOwner;

    // Mapping evaluator to NFT 
    // address evaluator => tokenId => Asset
    mapping (address => mapping(uint256 => Asset)) listNFTByEvaluator;
    ...
    */
    ...
    function _setNFTAddress(address _newAddress) internal {
        dfy_physical_nft = DFY_Physical_NFT(_newAddress);
    }
    ...
    */
    ...
    function createNftToken(address _creator, uint256 _assetId, uint256 _evaluationId) external OnlyEOA onlyRole(EVALUATOR_ROLE) {
        ...
        // Create NFT
        uint256 mintedTokenId = dfy_physical_nft.mint(_creator, 1, _asset.assetDataCID, "");

        // Add token id to list token id of owner
        tokenIdListByOwner[_creator].push(mintedTokenId);

        // Add token id to list asset of owner
        listAssetNFTByOwner[_creator][mintedTokenId] = _asset;

        // Add token id to list nft of evaluator
        listNFTByEvaluator[msg.sender][mintedTokenId] = _asset;
        ...
        emit NftCreationSuccessful(_creator, mintedTokenId, _asset.assetDataCID, _asset.status);
    }
