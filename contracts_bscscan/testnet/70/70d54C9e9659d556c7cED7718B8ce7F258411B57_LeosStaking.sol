 ...
 * @title Leos Staking
 */
contract LeosStaking is Ownable {
    ...
    struct Plan {
        ...
        uint256 effectiveRewardPercentage; // Calculated reward percentage, with Leos Tax fee
    ...
    address public leosTokenAddress; /* LEOS Token Contract Address */
    address public rewardAccount; //Leos account from which reward amount will be sent to Stake holders
    ...
    constructor(address leosToken, address rewardsFrom) public {
        leosTokenAddress = leosToken;
        ...
        returns (uint256 stakeIndex)
    {
        ...
        require(!stakingPaused, "LeosStaking: Staking Paused");
        /* plan must be valid and should not be disabled for further stakes */
        require(
            plan.state == 1,
            "LeosStaking: Invalid or disabled staking plan"
        );
        require(
            _stakeAmount >= plan.minimumStake,
            "LeosStaking: Stake is below minimum allowed stake"
        );

        // Transfer tokens from stake holders Leos account to this contract account
        IBEP20(leosTokenAddress).transferFrom(
            _msgSender(),
            address(this),
            _stakeAmount
        );

        /* Reward amount is the rewardAmount + Leos Fee to transfer */
        rewardAmount = (
        ...
        require(
            ...
            "LeosStacking: Reached maximum stakes per user"
        );
    ...
    function withdrawStaking(uint256 _stakeIndex) public {
        ...
        require(
            _stakeIndex < MAX_NUM_OF_STAKES_PER_USER,
            "LeosStaking: Invalid stake index"
        );

        Stake storage stake = stakes[_msgSender()][_stakeIndex];
        require(stake.state == 1, "LeosStaking: Stake is not active");
        require(
            _isExpired(stake.createdAt, plans[stake.plan].durationInDays),
            "LeosStaking: Stake Plan not expired yet"
        );
        ...
        IBEP20(leosTokenAddress).transfer(_msgSender(), amount);
    ...
    function transferExcessReward(address _to) public onlyOwner {
        uint256 excessAmount = IBEP20(leosTokenAddress).balanceOf(
            address(this)
        );
        ...
        if (excessAmount > 0) {
            ...
            IBEP20(leosTokenAddress).transfer(_to, excessAmount);
        }
     ...
     * @param _effective_percentage is the reward percentage with Leos Fee of 3%
    ...
    function createPlan(
        ...
        require(_duration > 0, "LeosStaking: Duration in weeks can't be zero");
        require(_minimum_stake > 0, "LeosStaking: Minimum stake can't be zero");
        require(
            _reward_percentage > 0,
            "LeosStaking: Total reward percentage can't be zero"
        );
        require(plans[_name].state == 0, "LeosStaking: Plan already exists");
    ...
    function deletePlan(string memory _name) public onlyOwner {
        require(plans[_name].state > 0, "LeosStaking: Plan not found");
        require(plans[_name].usageCount == 0, "LeosStaking: Plan is in use");
    ...
    function disablePlan(string memory _name) public onlyOwner {
        require(plans[_name].state > 0, "LeosStaking: Plan doesn't exist");
