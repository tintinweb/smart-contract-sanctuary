...
contract FuniToken is BEP20('Funi Token', 'FUNI') {
    ...
    uint public maxSupply = 10**8 * 10**18; // scaled by 1e18. That means 100,000,000 FUNI
    ...
    function _mintForKeeper(uint256 _amount) private {
        require(keeper != address(0), 'FUNI::_mintForKeeper: keeper to the zero address');
        uint mintAmount = _amount.add(totalSupply()) > maxSupply ? maxSupply.sub(totalSupply()) : _amount;
