...
contract HOMLaunch is Launch, ReentrancyGuard {
    ...
    uint256[] public prices = [0.1 * 10**18, 0.3 * 10**18, 0.9 * 10**18, 2.4 * 10**18];
    ...
    function buyCrate(uint256 power) public payable nonReentrant {
        require(power < prices.length, 'not a valid power');
