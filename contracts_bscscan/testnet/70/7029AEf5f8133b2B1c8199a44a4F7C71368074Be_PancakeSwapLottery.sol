...
contract PancakeSwapLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    uint16 public useReferralDiscountRate = 100;
    // Max referral commission rate: 10%.
    uint16 public constant MAXIMUM_REFERRAL_COMMISSION_RATE = 1000;
    // Max referral discount rate: 10%.
    uint16 public constant MAXIMUM_USE_REFERRAL_DISCOUNT_RATE = 1000;
        ...
        nonReentrant
    {
        ...
        payReferralCommission(msg.sender, amountCakeToTransfer);
        // Calculate referral commission amount
        uint256 referralCommisionAmount = calculateReferralCommission(msg.sender, amountCakeToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInCake += amountCakeToTransfer - referralCommisionAmount;
    ...
    function payReferralCommission(address _user, uint256 _totalAmount) internal {
        if (address(Referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                cakeToken.transfer(referrer, commissionAmount);
