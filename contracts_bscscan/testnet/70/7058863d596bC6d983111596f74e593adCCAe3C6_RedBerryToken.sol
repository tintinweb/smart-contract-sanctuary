...
// File: contracts\RedBerryToken.sol
...
contract RedBerryToken is BEP20("RedBerry", "REDB") {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "REDBERRY::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "REDBERRY::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "REDBERRY::delegateBySig: signature expired"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "REDBERRY::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying REDBERRY (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "REDBERRY::_writeCheckpoint: block number exceeds 32 bits"
        );
...
// will be transferred to a governance smart contract once REDBERRY is sufficiently
...
contract MasterChef is Ownable {
    ...
    // REDBERRY tokens created per block.
    ...
    function updatePool(uint256 _pid) public {
        ...
        .mul(pool.allocPoint)
        .div(totalAllocPoint);
        RedBerry.mint(devaddr, toadReward.div(10));
        RedBerry.mint(address(this), toadReward);
    ...
    function deposit(
        ...
        if (
            _amount > 0 &&
            address(pantherReferral) != address(0) &&
            _referrer != address(0) &&
    ...
    // Safe toad transfer function, just in case if rounding error causes pool to not have enough REDBERRYs.
    ...
    function setPantherReferral(IPantherReferral _pantherReferral)
        public
        onlyOwner
    {
    ...
    function payReferralCommission(
        ...
        if (
            address(pantherReferral) != address(0) && referralCommissionRate > 0
        ) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                ...
                pantherReferral.recordReferralCommission(
                    referrer,
                    commissionAmount
                );
            }
