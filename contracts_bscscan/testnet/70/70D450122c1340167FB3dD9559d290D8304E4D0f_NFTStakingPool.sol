...
contract NFTStakingPool is Ownable, IERC721Receiver {
    ...
    constructor(address _usdc, address _pet, address _paymentToken) {
        usdc = IERC20(_usdc);
        pet = IPet(_pet);
    ...
    function buySlot(address to) public {
        ...
        require (n <= 5, 'Staking : buy > 5 slots');
    ...
    function deposit(uint256[] memory tokenIds, address to) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            pet.safeTransferFrom(msg.sender, address(this), tokenIds[i]);
        }

        uint256 power = NFTLib.getPower(EnumerableSet.values(user.nftIds), pet);
        uint256 incPower = power - user.power;
        user.power = power;
        totalPower += incPower;
        user.rewardDebt += int256(incPower * accRewardPerShare / ACC_REWARD_PRECISION);
        emit Deposit(msg.sender, incPower, to);
    }
    ...
    function withdraw(uint256[] memory tokenIds, address to) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (EnumerableSet.contains(user.nftIds, tokenId)) {
                ...
                pet.transferFrom(address(this), to, tokenId);
            }
        }
        uint256 power = NFTLib.getPower(EnumerableSet.values(user.nftIds), pet);
        ...
        // Effects
        emit Withdraw(msg.sender, withdrawPower, to);
    }
    ...
    function emergencyWithdraw(address to) public {
        ...
        emit EmergencyWithdraw(msg.sender,power, to);
    }
    ...
    function changePetAddress(address _newAddr) external onlyOwner {
        pet = IPet(_newAddr);
    }
...
interface IPet is IERC721{
    function create(address receiver, uint256 eggLevel) external;
    function evolve(address receiver, uint256 pet0Level, uint256 pet1Level) external;
...
library NFTLib {
    ...
    function optimizeEachLevel(NFTLib.info[] memory info, uint256 level, uint256 m,  uint256 n) internal pure returns (uint256){
        ...
        require(l > n + m, 'Lib: not enough pet');
        uint256 j = 0;
        for (uint256 i = 0; i < info.length; i++) {
            if (info[i].level == level) {
                tmp[j++] = info[i].power;
