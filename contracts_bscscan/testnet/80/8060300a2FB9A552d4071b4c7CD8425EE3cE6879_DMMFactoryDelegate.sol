...
contract DMMFactoryDelegate is IDMMFactoryDelegate, ManageUser {
    ...
    function setFeeConfiguration(address _feeAddress, uint16 _governmentFeeBps, uint32 _platformFeePercent) external override {
        require(msg.sender == feeSetter, "DMM: FORBIDDEN");
        require(_governmentFeeBps >= 0 && _governmentFeeBps < 2000, "DMM: INVALID FEE");
        require(_platformFeePercent >= 0 && _platformFeePercent <= 10000, "DMM: INVALID PLATFORM FEE");
        feeAddress = _feeAddress;
        governmentFeeBps = _governmentFeeBps;
        platformFeePercent = _platformFeePercent;

        emit SetFeeConfiguration(_feeAddress, _governmentFeeBps, _platformFeePercent);
    }
...
interface IDMMFactoryDelegate {
    function setFeeConfiguration(address _feeAddress, uint16 _governmentFeeBps, uint32 _platformFeePercent) external;
