...
contract EpicMarket is AccessControl {
    ...
    function getMarketId(address item, uint256 tokenId, uint256 amount) public view returns(bool, uint256){
        for(uint i = 0; i < items.length; i++){
            if(items[i]._tokenId == tokenId && items[i]._amount == amount && items[i]._item == item){
