...
contract StakingUSDC is Staking {
    ...
    function approvePool(uint256 amount) public onlyPools override {
        require(USDC.balanceOf(address(this)) > amount, "USDC staking balance is not enough!");
    ...
    function yieldAPR() public override view returns (uint256){
        if (_totalStaking > 0)
            return rewardRate * 31536000 * ceresAnchor.getCRSPrice() * CERES_PRECISION / _totalStaking / ceresAnchor.getUSDCPrice();
        else
            return 0;
    }
