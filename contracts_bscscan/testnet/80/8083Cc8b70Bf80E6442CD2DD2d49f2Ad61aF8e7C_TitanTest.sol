...
contract TitanTest is Context, IERC20, Ownable {
    ...
    // THIS FUNCTION WILL USE BY SMART CONTRACT IN SEED AND PRSALE ROUND
    ...
    function startBuying(uint256 tokenAmount, uint256 time, string memory _stage) public onlyOwner{
        require(keccak256(abi.encodePacked(_stage)) == keccak256(abi.encodePacked("seed")) ||
            keccak256(abi.encodePacked(_stage)) == keccak256(abi.encodePacked("presale")),"INVALID STAGE");
        
        if(keccak256(abi.encodePacked(_stage)) == keccak256(abi.encodePacked("seed"))){
            minLimit = 25000;
            maxLimit = 250000;
        }else if (keccak256(abi.encodePacked(_stage)) == keccak256(abi.encodePacked("presale"))){
            minLimit = 5000;
            maxLimit = 25000;
        }
        releaseTime = time;
        remainingTokens = tokenAmount;
        _approve(ownerAccount, address(this), tokenAmount);
    ...
    function buyToken(uint256 amountOfToken) public payable{
        ...
        require(amountOfToken >= minLimit && amountOfToken <= maxLimit,"You are passing wrong value.");
        address sender =  msg.sender; 
        uint256 quantity = amountOfToken;
