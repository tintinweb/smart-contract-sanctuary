...
contract Chainance is Context, Ownable{
    ...
    //- Login ChainUp
    function _LoginChainUp(address _address) internal AccountControl {
        for ( uint i =0; i < ChainUp_Accounts.length; i++) {
            if ( ChainUp_Accounts[i].AccountAddress == _address) {
    ...
    function LoginChainUp(address _address) external {
        _LoginChainUp(_address);
    }    
