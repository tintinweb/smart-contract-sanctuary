...
contract FlashLiquidate is Exponential,FlashLiquidateInterface,Ownable {
     ...
     */
    function liquidate(address borrower) public onlyOwner returns (uint) {
        uint err=_liquidateBorrowInternal(borrower);
        return err;
    }

    function _liquidateBorrowInternal(address borrower) internal onlyOwner returns (uint) {
        ...
        emit GetAccountInfo(filAmount,filMulColAmount, usdtAmount,contractUsdtBalance,maxClose);
        if (err != 0) {
            return err;
        }
    ...
    function _liquidateBorrowFresh(address borrower,uint maxClose) internal onlyOwner returns (uint) {
    ...
    function _getAccountInfo(address borrower) internal onlyOwner returns (uint,uint,uint,uint,uint,uint) {
    ...
    function _getBorrowInfo(address borrower)  internal returns (uint,uint,uint) {
        ...
        (uint usdtPrice,uint filPrice)=_getPriceInfo();
        //当前借款usdt的数量
        uint usdtBorrowBalance = CTokenInterface(eUSDTAddr).borrowBalanceCurrent(borrower);
        //usdt价值
        (mErr,usdtAmount)=mulExp3(Exp({mantissa: usdtBorrowBalance}),Exp({mantissa: usdtPrice}),Exp({mantissa: expScale}));
        if (mErr != MathError.NO_ERROR) {
            return (uint(Error.MATH_ERROR), 0, 0);
        }
        return (uint(Error.NO_ERROR), usdtBorrowBalance,usdtAmount.mantissa);
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _redeem(uint redeemTokens) internal onlyOwner returns (uint) {
