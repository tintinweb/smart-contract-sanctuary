...
contract HowlsCastle is Ownable, ReentrancyGuard {
    ...
    address internal callSource = 0x0000000000000000000000000000000000000000;
    ...
    function deposit(uint256 _pid, uint256 _amount, bool _boost, address _callSource, address _referrer) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (amount > 0 && address(calciferReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
        ...
        if (_boost) {
            ...
            require(_callSource == callSource,'deposit: FORBIDDEN BOOST CALL');
        }
    ...
    function payOrLockupPendingCALCIFER(uint256 _pid, bool _boost, uint256 _amount) internal {
        ...
        uint256 pending = pendingCALCIFER(_pid, msg.sender);
    ...
    function setCallSource(address _callSource) public onlyOwner {
        callSource = _callSource;
    }

    // Get callSource by the owner
    function getCallSource() public view onlyOwner returns (address) {
        return callSource;
    }
}