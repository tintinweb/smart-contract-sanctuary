...
contract zombieFactory is Ownable{
    ...
    function createBoss (uint32 _attributeId,uint32 _rare,uint32 _isready,uint32 _reward,uint32 _level, uint32 _energy,string memory _name) public  onlyOwner{
        boss.push(Boss(uint32(boss.length),_attributeId,_rare,_isready,_reward,_level,_energy,_name));

    }
    function resetBoss(uint32 _Id,uint32 _isready,uint32 _attackReward) public onlyOwner{
        boss[_Id].isready = _isready;
        boss[_Id].attackReward = _attackReward;
    }
    function _getRare(uint _rand) internal  pure returns(uint){
        uint _rare;
        if(_rand<=400){
            _rare = 1;
        }
...
contract zombieHelper is zombieFactory{
    ...
    function _levelUp(uint _zombieId) internal onlyOwnerOf(_zombieId){
    ...
    function _fusionRare(uint32 _rareOne,uint32 _rareTwo) public returns(uint32){
        uint32 _Rare;
    ...
    function multiply(uint _zombieId, uint _targetId) public multiplyOnlyOwnerOf(_zombieId,_targetId) returns(uint32){
        ...
        uint32 _rare = _fusionRare(myZombie.rare,targetZombie.rare);
        uint32 _res;
...
contract zombieBattlefiled is zombieOwnership {
    ...
    uint  public battleReward =  10000;
    uint public claimLimitTime = 86400;
    ...
    function setBattleReward(uint _battleReward) public onlyOwner {
     ...
     function setClaimLimitTime(uint _claimLimitTime) public onlyOwner {
        claimLimitTime = _claimLimitTime;
    }

    function getReward(uint _zombieId) public view returns(uint){
        uint reward=((block.timestamp - battleField[_zombieId].caculateTime)*zombies[_zombieId].energy*battleField[_zombieId].rewardProportion) / battleReward;
        return reward;
    }
    function claimTokens(uint _zombieId) public onlyOwnerOf(_zombieId){
        require(zombies[_zombieId].isBattling ==1);
        require(block.timestamp-battleField[_zombieId].caculateTime >claimLimitTime,'Clamin tokens must be > 1days');
    ...
    function joinBattlefield(uint _zombieId)public onlyOwnerOf(_zombieId){
        ...
        battleField[_zombieId] = battlefieldJoiner(payable(_msgSender()),block.timestamp,100);
