...
contract DinoMarketPlace is Pausable, Ownable {
  ...
  uint256 public eggPrice = 7500 * 10e18;
  ...
  /// @dev Set new egg price address
  /// @param _newEggPrice - Address of new erc721 token.
  function setEggPrice(uint256 _newEggPrice) external onlyOwner {
      eggPrice = _newEggPrice;
  }
  
  /// @dev Set total egg by genes of market
  /// @param _eggGenes - Dino egg genes
  /// @param _total - Total egg by genes
  function setTotalEggByGenes(uint256 _eggGenes, uint256 _total) external onlyAdmin {
  ...
  function buyEgg(uint256 _eggGenes, uint256 _eggAmount) external {
      ...
      IERC20(tokenAddress).transferFrom(msg.sender, address(this), _eggAmount * eggPrice);
