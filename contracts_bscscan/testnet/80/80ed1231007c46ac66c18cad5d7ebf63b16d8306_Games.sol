...
contract Games is Ownable {
    ...
    event JoinGame(address indexed _account, uint256 _gameJoinTotalCount, uint256 _gamesPayTotalAmount, uint256 _gameOpenPayAmount);
    event Recharge(address indexed _account, uint256 _gameRechargeCount, uint256 _gamesPayTotalAmount, uint256 _rechargeAmount);
    ...
    function joinGame() public returns (bool) {
        ...
        gamesPayTotalAmount += gameOpenPayAmount;
        ...
        emit JoinGame(msg.sender, gameJoinTotalCount, gamesPayTotalAmount, gameOpenPayAmount);// set log

        return true;// return result
    }

    function recharge(uint256 _rechargeAmount) public returns (bool) {
        // Data validation
        require(gameSwitchState,"-> gameSwitchState: game has not started yet.");
        require(gameAccountOf(msg.sender),"-> isJoin: account not exist.");
        require(cuseTokenContract.balanceOf(msg.sender)>=_rechargeAmount,"-> _rechargeAmount: Insufficient address token balance.");

        // Orders dispose
        gameRechargeCount += 1;// total number + 1
        gamesPayTotalAmount += _rechargeAmount;

        cuseTokenContract.safeTransferFrom(address(msg.sender), officialAddress, _rechargeAmount);// Transfer cuse to officialAddress
        emit Recharge(msg.sender, gameRechargeCount, gamesPayTotalAmount, _rechargeAmount);// set log
    ...
    function getGameBasic() public view returns (ERC20 CuseTokenContract,address OfficialAddress,bool GameSwitchState,uint256 GameStartTime,
        uint256 GamesPayTotalAmount,uint256 GameJoinTotalCount,uint256 GameRechargeCount,uint256 GameOpenPayAmount) {
        return (cuseTokenContract,officialAddress,gameSwitchState,gameStartTime,gamesPayTotalAmount,gameJoinTotalCount,gameRechargeCount,gameOpenPayAmount);
    }
