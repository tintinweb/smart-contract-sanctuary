...
contract KawaiiQuizAirdrop is Ownable, SignData {
    ...
    function setQuizAirdropNFT(address[] calldata receivers) external onlyOwner {
    ...
    function changeQuizAirdropNFT(address[] calldata receivers, bool[] calldata isClaims) external onlyOwner {
        uint256 length = receivers.length;
        for (uint i = 0; i < length; i++) {
