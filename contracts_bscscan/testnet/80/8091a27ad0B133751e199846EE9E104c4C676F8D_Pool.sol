...
contract Pool is Ownable {
    ...
    event LogDepositLp(uint256 indexed poolId, address, uint256 depositLpAmount, uint256 lastBlock, uint256 lpPrice);
    ...
    function addPool(IERC20 _lpToken, uint256 _lockBlockCount, uint256 _minDepositLimit, uint256 _minDepositPercent) public onlyOwner {
        ...
        poolInfoList.push(
            PoolInfo({
                ...
                minDepositPercent: _minDepositPercent,
                totalBalance: 0
            })
        );
    ...
    /// @param _minDepositLimit The minimum amount to deposit.
    function setPool(uint256 _poolId, uint256 _minDepositLimit, uint256 _lockBlockCount, uint256 _minDepositPercent) public onlyOwner {
        PoolInfo storage pool = poolInfoList[_poolId];
        pool.minDepositPercent = _minDepositPercent;
        pool.minDepositLimit = _minDepositLimit;
        pool.lockBlockCount = _lockBlockCount;
    ...
    function deposit(uint256 _poolId, uint256 _depositLpAmount) public {
        ...
        require(_depositLpAmount.mul(lpPrice) >= pool.minDepositLimit.mul(pool.minDepositPercent), "Pool: Insufficient amount of deposit");
        ...
        emit LogDepositLp(_poolId, msg.sender, _depositLpAmount, lastBlock, lpPrice);
    }
    ...
    function getLpPrice(uint256 _poolId) public view returns (uint256 lpPrice) {
        PoolInfo memory pool = poolInfoList[_poolId];
        (uint112 reserve0, ,) = IUniswapV2Pair(address(pool.lpToken)).getReserves();
        uint256 totalSupply = getLpTotalSupply(_poolId);
        uint256 divResult = reserve0.div(totalSupply);
        uint256 mulResult = divResult.mul(2);
        return mulResult;
    }
}
