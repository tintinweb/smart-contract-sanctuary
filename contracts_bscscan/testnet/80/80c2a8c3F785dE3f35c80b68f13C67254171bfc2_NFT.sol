    ...
    ERC721Pausable
{
    ...
    constructor(
        ...
        require(initialCap_ > 0, "ERC721PresetMinterPauserAutoId: cap is 0");
        ...
        _setupRole(CAP_MANAGER, _msgSender());
    }
    ...
    function _mint(address to_, uint256 tokenId_) internal virtual override {
        require(
            ...
            "ERC721PresetMinterPauserAutoId: cap exceeded"
        );
    ...
    function increaseCap(uint256 amount_) public onlyRole(CAP_MANAGER) {
        require(amount_ > 0, "ERC721PresetMinterPauserAutoId: amount is 0");
        require(
            cap() + amount_ <= type(uint256).max,
            "ERC721PresetMinterPauserAutoId: cap overflowed"
        );
     ...
     */
    function mint(address to_) public virtual onlyRole(MINTER_ROLE) returns (uint256) {
...
interface INFTArtifacts {
    ...
    function getRandomArtifact(uint256 randomNumber)
        external
        view
        returns (uint256 actifactId, uint256 actifactType);
...
contract NFTArtifactsManager {
    INFTArtifacts public artifacts;

    event UpdateArtifacts(address artifacts_);

    constructor(address artifacts_) {
        _updateArtifacts(artifacts_);
    }

    function _updateArtifacts(address artifacts_) internal {
        artifacts = INFTArtifacts(artifacts_);
        emit UpdateArtifacts(artifacts_);
    }
}
    ...
    NFTArtifactsManager
{
    ...
    struct Artifact {
        uint256 artifactId;
        uint256 artifactType; // 1: characters, 2: karts, 3: items
        bool presetType;
    }

    mapping(uint256 => Artifact) public tokenIdToArtifact;
    ...
    event TokenFulfilled(uint256 indexed tokenId, uint256 indexed artifactId);
    ...
    constructor(
        ...
        NFTArtifactsManager(artifacts_)
    {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(MANAGER_ROLE, _msgSender());
    }

    function updateArtifacts(address artifacts_) public onlyRole(MANAGER_ROLE) {
        require(
            artifacts_ != address(0),
            "NFT: artifacts_ is the zero address"
        );

        _updateArtifacts(artifacts_);
    }

    function updateRandomGenerator(address randomGenerator_) public onlyRole(MANAGER_ROLE) {
        require(
    ...
    function mintSpecificType(address to_, uint256 artifactType_) public {
        require(
            artifactType_ > uint256(0),
            "NFT: artifactType_ is 0"
        );

        uint256 _tokenId = super.mint(to_);
        Artifact storage _artifactInfo = tokenIdToArtifact[_tokenId];
        _artifactInfo.artifactType = artifactType_;
        _artifactInfo.presetType = true;
    }

    function unbox(uint256 tokenId_) public payable {
        address _owner = ownerOf(tokenId_);
        require(_owner == _msgSender(), "NFT: caller is not owner");
        require(
            tokenIdToArtifact[tokenId_].artifactId == uint256(0),
            "NFT: token is already unboxed"
        );
    ...
    {
        Artifact storage _artifactInfo = tokenIdToArtifact[tokenId_];

        if (_artifactInfo.artifactId == uint256(0)) {
            if (_artifactInfo.presetType == true) {
                uint256 _artifactId = artifacts.getRandomArtifactId(
                    randomness_,
                    _artifactInfo.artifactType
                );

                _artifactInfo.artifactId = _artifactId;
                delete _tokenIdToUnboxBlockNumber[tokenId_];

                emit TokenFulfilled(tokenId_, _artifactId);
            } else {
                (uint256 _artifactId, uint256 _artifactType) = artifacts
                    .getRandomArtifact(randomness_);

                _artifactInfo.artifactId = _artifactId;
                _artifactInfo.artifactType = _artifactType;
                _artifactInfo.presetType = false;
                delete _tokenIdToUnboxBlockNumber[tokenId_];
