...
contract MyNFTs is ERC1155, Ownable {
    //// this store the relation betwen the card and
    mapping(string => uint256) public ide;
    mapping(uint256 => string) public ideName;
    //represent how many Ides has been created
      ...
      function viewDeck1(address account)  public view returns( string memory deck)  {
            ...
            for (uint i = 1; i <= poolIde; i++) {                            
                 if( balanceOf(account,i) > 0){
                   ...
                   if(  i == poolIde ){
              message = string (abi.encodePacked(message,'{','"', "nombre",'"',':', '"',ideName[i],'"',',','"',"cantidad",'"',':', '"', value,'"','}' ));    
                     } else{ 
              message = string (abi.encodePacked(message,'{','"', "nombre",'"',':', '"',ideName[i],'"',',','"',"cantidad",'"',':', '"', value,'"','}',','));  
                     }
                }                   
