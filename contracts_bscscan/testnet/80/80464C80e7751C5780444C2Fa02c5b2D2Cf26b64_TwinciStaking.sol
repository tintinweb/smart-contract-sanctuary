...
// Twinci Stacking is a smart contract for distributing Twinci by asking user to stake the ERC20-based token.
contract TwinciStaking is ITwinciStaking, Ownable ,ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Twincis to distribute per block.
        uint256 lastRewardBlock; // Last block number that Twincis distribution occurs.
        uint256 accRewardPerShare; // Accumulated Twinci per share
    ...
    // Twinci tokens created per block.
    ...
    // The block number when Twinci mining starts.
    ...
    function pendingReward(uint256 _pid, address _user) external override view returns (uint256, address) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 TwinciReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            uint accRewardPerShare = pool.accRewardPerShare.add(TwinciReward);
            uint _reward = pool.accRewardPerShare.mul(_stakerWeight).div(pool.totalStakingWeight);
            accTwinciPerShare = accRewardPerShare.mul(_stakerWeight).div(pool.totalStakingWeight);
        }
        return (accTwinciPerShare, pool.stakeToken);
    }
    ...
    // Deposit Staking tokens to StakingToken for Twinci allocation.
