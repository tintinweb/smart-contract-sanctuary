...
// File: contracts/interfaces/IFuniSwapFactory.sol

pragma solidity >=0.5.0;

interface IFuniSwapFactory {
...
// File: contracts/interfaces/IFuniSwapPair.sol

pragma solidity >=0.5.0;

interface IFuniSwapPair {
...
// File: contracts/interfaces/IFuniSwapERC20.sol

pragma solidity >=0.5.0;

interface IFuniSwapERC20 {
    event Approval(address indexed owner, address indexed spender, uint value);
    event Transfer(address indexed from, address indexed to, uint value);
...
// File: contracts/FuniSwapERC20.sol
...
contract FuniSwapERC20 is IFuniSwapERC20 {
    ...
    string public constant name = 'FuniSwap LPs';
    string public constant symbol = 'FUNI-LP';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'FuniSwap: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'FuniSwap: INVALID_SIGNATURE');
...
// File: contracts/interfaces/IFuniSwapCallee.sol

pragma solidity >=0.5.0;

interface IFuniSwapCallee {
    function FuniSwapCall(address sender, uint amount0, uint amount1, bytes calldata data) external;
}




// File: contracts/FuniSwapPair.sol
...
contract FuniSwapPair is IFuniSwapPair, FuniSwapERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'FuniSwap: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    // modifier allowSwap(){
    //     require(now >= startingSwapTime, 'FuniSwap: not allow swap');
    //     _;
    // }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'FuniSwap: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'FuniSwap: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'FuniSwap: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IFuniSwapFactory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'FuniSwap: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'FuniSwap: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'FuniSwap: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        ...
        require(to != _token0 && to != _token1, 'FuniSwap: INVALID_TO');
        if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
        if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
        if (data.length > 0) IFuniSwapCallee(to).FuniSwapCall(msg.sender, amount0Out, amount1Out, data);
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this));
        }
        uint amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0;
        uint amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, 'FuniSwap: INSUFFICIENT_INPUT_AMOUNT');
...
// File: contracts/FuniSwapFactory.sol
