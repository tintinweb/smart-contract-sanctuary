...
contract NewAMasterChef is InitializeOwnable, DEFIToken, Initializable{
        ...
        )
    {
        ...
        todayAmount = (allocAmount > 0) ? (allocAmount - totalSupply) : 0;
        alreadyAmount = totalSupply;
        ...
        )
    {
        ...
        PoolInfo storage pool = poolInfo;
        uint256 mSupply = pool.mToken.balanceOf(address(this));
        uint256 allocAmount = (pool.accPerShare > 0) ? pool.accPerShare.mul(mSupply).div(1e12) : 0;


        treasuryUSD = pool.mToken.getTreasuryValueUSD();
        treasuryPrevUSD = 0;
        tokenTotalSupply = totalSupply;
        availableToClaim = (allocAmount > 0) ? (allocAmount - totalSupply) : 0;
        expectedClaim = totalSupply;
    ...
    function updatePool() public {
        ...
        uint256 lastAvalible = (pool.accPerShare > 0) ? pool.accPerShare.mul(mSupply).div(1e12) : 0;


        if (pool.lastDayAvailable + 1 days < block.timestamp && treasuryUSD > totalSupply + (lastAvalible - totalSupply) && mSupply > 0) {
                uint256 solosReward = treasuryUSD - (totalSupply + (lastAvalible - totalSupply))/ 10 ;
                ...
                history.push(
                    History({
                        available: (lastAvalible - totalSupply),
                        availableTotal: totalSupply + (lastAvalible - totalSupply),
                        tvl: treasuryUSD,
                        solosTotalSupply: totalSupply,
