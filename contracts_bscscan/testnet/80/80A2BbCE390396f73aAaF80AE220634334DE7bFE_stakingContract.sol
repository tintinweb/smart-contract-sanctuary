...
contract stakingContract {
    ...
    function withdraw() public returns(bool) {
        ...
        uint256 amount = rewardAmount + user[msg.sender].amount;
    ...
    function getUserDetails(address account) public view returns(userDetails memory, uint256) {
        uint256 reward = getRewards(account);
        return (userDetails(user[account].level, user[account].amount, user[account].initialTime, user[account].endTime, user[account].rewardAmount, user[account].withdrawAmount, user[account].status), reward);
    }

    function setrTokenAddress(address _token) public onlyOwner returns (bool){
        token = IERC20(_token);
        return true;
    }
