...
contract DistributorFactory {
  ...
  function setShare(address shareholder, uint256 amount) external onlyToken {
    ...
    for (uint256 i = 0; i < arrayLength; i++) {
      ...
      .distributorAddress.setShare(shareholder, amount);
    }
  }
  ...
  function process(uint256 gas) external onlyToken {
    ...
    for (uint256 i = 0; i < arrayLength; i++) {
      distributorsMapping[distributorsArrayOfKeys[i]]
      .distributorAddress.process(gas);
    }
  }
  ...
  function deposit() external payable onlyToken {
    ...
    for (uint256 i = 0; i < arrayLength; i++) {
      distributorsMapping[distributorsArrayOfKeys[i]]
      .distributorAddress.deposit{value: valuePerToken}();
    }
  }
...
contract SHELTERT0B2 is IBEP20, Auth {
  using SafeMath for uint256;
  ...
  string constant _name = 'SHELTERT0B2';
  string constant _symbol = 'SHELTERT0B2';
