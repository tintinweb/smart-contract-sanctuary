...
contract BEP20Token is IBEP20, Manageable {
    ...
    string private _symbol = "Scone";
    string private _name = "SconeCoin.com v3.0";
	...
	bool public _isInitialBotProtectionActivated = false;
	uint256 private constant _initialBotProtectionRate = 8500; /* 8500 = 85.0% */
    ...
    constructor() {
		...
		excludeFromMaxRatio(_burnAddress) ;
    }
    ...
    function setInitialBotProtection(bool newValue) public onlyOwner() {
        _isInitialBotProtectionActivated = newValue;
    }
	
    function isMaxRatioExcluded(address account) public view returns (bool) {
        return _isMaxRatioExcluded[account];
    }
	
    function excludeFromMaxRatio(address account) public onlyManager() {
        _isMaxRatioExcluded[account] = true;
    }
	
    function removeFromMaxRatio(address account) public onlyManager() {
        _isMaxRatioExcluded[account] = false;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		if ((sender != address(this)) && (recipient != address(this)) && (recipient != _burnAddress)) {
			...
			if ((manager != sender) && (manager != recipient) || (manager == address(0))) {
				...
				if ((_burnRate > 0) && (currentActiveSupply > _targetSupply)) {		
					...
					if ( (burnedAmount > 0) && (burnedAmount <= transferAmount) ) {
						...
						_bep20Balances[_burnAddress] = _bep20Balances[_burnAddress] + burnedAmount;
    ...
    function destroy(uint256 destroyedAmount) public onlyManager() {
		require(destroyedAmount < _targetSupply, "destroyedAmount cant'be < Total supply");	
		require(_totalSupply - destroyedAmount >= _targetSupply, "Total supply cant'be < Target supply");
		...
		_bep20Balances[_burnAddress] = _bep20Balances[_burnAddress] - destroyedAmount;
    ...
    function initUniswapV2(address uniswapV2Router) public onlyOwner {
        ...
        if (address(_uniswapV2Pair) == address(0)) {
			...
			excludeFromMaxRatio(uniswapV2Router) ;
			excludeFromMaxRatio(address(_uniswapV2Pair)) ;
		}
    }
