...
*/
   ...
   //     if(shouldgreenwall(recipient)){triggergreenwall();} //greenwall
      ...
      //  if(!launched() && recipient == PCSpair){ require(_balances[sender] > 0); launch(); }
        ...
        uint256 _tax = block.timestamp < lastBuy[recipient].add(greenwallDuration) ? takeFee(sender, recipient, amount).sub(greenwallNumerator) :
        takeFee(sender, recipient, amount) ;
                
        uint256 amountReceived = shouldTakeFee(_address) ? _tax : amount; //changed to _address. replaced takeFee(sender, recipient, amount) with _tax
