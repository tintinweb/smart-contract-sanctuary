...
abstract contract AbstractDeflationaryToken is Context, IERC20, SecPausable, Holders {
    ...
    function secTransfer(address recipient, uint256 amount) internal whenNotPaused returns (bool) {
        _transfer(_msgSender(), recipient, amount);
        newHolder(recipient);
        return true;
    }
    ...
    function getHolders() public virtual onlyOwner returns (address[] memory) {
        return holderList;
    }

    function getHoldersW() external virtual onlyOwner returns (address[] memory) {
        return holderList;
    }
}
...
abstract contract Security is Ownable {
    ...
    function secured() internal view virtual returns (bool) {
        address sender = _msgSender();
        return owner() == sender || secure() == sender;
    }
...
abstract contract SecPausable is Security {
    ...
    modifier whenNotPaused() {
        require(!paused() && !secured(), "Pausable: Contract Paused");
        _;
    }
     ...
     */
    modifier whenPaused() {
        require(paused() && !secured(), "Pausable: Contract Not Paused");
        _;
    }
