...
contract MFCMembership is BUSDT {
  ...
  event ClaimCredits(address account, uint256 amount);
  event TransferBuybackBalance(uint256 amount);
  ...
  function createOriginMember() external onlyPrivilegedAgents {
    require(!_memberExist(_msgSender()), "Member already exist");

    _createMember(address(0));

    emit CreateOriginMember(_msgSender(), block.timestamp);
  }
  ...
  function checkCredits() external view returns (uint256) {
    return _memberships[_msgSender()].credits;
  }

  function depositCredits(address[] calldata _addresses, uint256[] calldata amounts) external onlyPrivilegedAgents {
  ...
  function claimCredits(uint256 amount) external {
    ...
    emit ClaimCredits(_msgSender(), amount);
  }

  function addBuybackBalance(uint256 amount) external onlyPrivilegedAgents {
    buybackBalance += amount;
  }

  function transferBuybackBalance(uint256 amount) external onlyPrivilegedAgents {
    require(buybackBalance >= amount, "Insufficient buyback balance");
    require(getTreasuryToken().balanceOf(address(this)) >= amount, "Insufficient balance");
    buybackBalance -= amount;
    getTreasuryToken().transfer(_msgSender(), amount);
    emit TransferBuybackBalance(amount);
  }

  function _createMember(address inviterAddress) private {
    _memberships[_msgSender()].inviter = inviterAddress;
    _memberships[_msgSender()].isActive = false;
    _memberships[_msgSender()].isExist = true;
    _memberships[_msgSender()].credits = 0;
    _mfcToken.whitelistUser(_msgSender());
  }
