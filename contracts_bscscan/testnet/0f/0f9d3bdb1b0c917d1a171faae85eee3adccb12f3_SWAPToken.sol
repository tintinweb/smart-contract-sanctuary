...
/**
 *Submitted for verification at bscscan.com on 2020-08-13
*/
...
contract SWAPToken is ApproveAndCallFallBack, IBSC, Owned {
    ...
    event XXX(address indexed tokenOwner, uint256 amount, address tokenContract);
    ...
    function receiveApproval(address tokenOwner, uint256 amount, address tokenContract, bytes memory data) public override {
        emit XXX(tokenOwner, amount, tokenContract);
        uint8 mode = uint8(data[0]);
        require(mode == 0x10 || mode == 0xD0 || mode == 0xE0 || mode == 0xF0, "Mode not accepted");

        if(tokenContract == VICC) {
            uint amountExpected = amount.mulSafe(3).divSafe(2);
            balances[address(this)] = balances[address(this)].subSafe(amountExpected);
            balances[msg.sender] = balances[msg.sender].addSafe(amountExpected);
            emit Exchange(address(this), msg.sender, amount);
        }
    }
}