...
contract DMMFactory is IDMMFactory, ManageUser {
    ...
    event SetFeeConfiguration(address feeAddress, uint16 governmentFeeBps, uint32 platformFeePercent);
    ...
    override
    view
    returns (address _feeAddress, uint16 _governmentFeeBps, uint32 _platformFeePercent)
    {
...
interface IDMMFactory {
    ...
    function getFeeConfiguration() external view returns (address _feeAddress, uint16 _governmentFeeBps, uint32 _platformFeePercent);
...
contract DMMPool is IDMMPool, ERC20Permit, ReentrancyGuard, VolumeTrendRecorder {
    ...
    // function getPlatformFeePercent() public override view returns (uint32) {
    //     return platformFeePercent;
    // }

    function feeForFactory(uint256 _feeInPrecision, uint256 amount, uint32 platformFeePercent) private view returns (uint256) {
        uint256 precision = 10 ** 18;
        return _feeInPrecision.mul(amount).div(precision).mul(platformFeePercent).div(BPS);
    }

    function sendFeeToFactory(uint256 _feeInPrecision, IERC20 token, uint256 amount) private {
        (address feeTo,, uint32 platformFeePercent) = factory.getFeeConfiguration();
        if (feeTo != address(0)) {
            token.safeTransfer(feeTo, feeForFactory(_feeInPrecision, amount, platformFeePercent));
            _sync();
        }
    }
...

{
  ...
  "libraries": {
    "contracts/libraries/FeeFomula.sol": {
      "FeeFomula": "0x6302f360fcfebf9e704b211e95e95a1353dbd22d"
    }
  }
}