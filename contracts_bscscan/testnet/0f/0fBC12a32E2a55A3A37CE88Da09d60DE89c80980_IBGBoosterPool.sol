...
contract IBGBoosterPool is Ownable {
    using SafeMath for uint256;
    ...
    mapping(address => uint256) public depositCheckPoints;
    ...
    constructor(
        ...
        uint256 _startBlock,
        uint256 _poolValidity
    ) public {
        ibg = _ibg;
        feeAddress = _feeAddress;
        ibgPerBlock = _ibgPerBlock;
        startBlock = _startBlock;
        poolValidity = _poolValidity;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require(depositCheckPoints[msg.sender] == 0,"Can't do multiple deposits");
        updatePool(_pid);
        if (user.amount > 0) {
        ...
        if (_amount > 0) {
            ...
            depositCheckPoints[msg.sender] = block.timestamp;
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(depositCheckPoints[msg.sender].add(poolValidity) < block.timestamp,"Time lock has not passed yet");
        ...
        user.rewardDebt = user.amount.mul(pool.accIBGPerShare).div(1e12);
        depositCheckPoints[msg.sender] = 0;
