...
contract Link is LinkInfo, Initialized, Ilink {
    ...
    event Getamount(address weth, address to, uint amount );
    function _withdraw(address to, uint amount) internal{
        ...
        if (token == ETH){
             //if (true) revert("before wbnb withdraw");
             emit Getamount(weth, to, amount);
             IWETH(weth).withdraw(amount);
             payable(to).transfer(amount);
        }else if(token == luca){
            // if (true) revert("before, trader  withdraew");
            emit Getamount(wluca, to, amount);
