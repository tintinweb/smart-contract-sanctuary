...
import {MidoNFT} from './MidoNFT.sol';
import {IMidoTokenSale} from './interfaces/IMidoTokenSale.sol';
...
contract MidoNFTSale is IMidoTokenSale, Whitelist {
    ...
    SaleNFTConfig[] internal _saleConfigList;
    ...
    function newSale(
        ...
        require(_publicTime<=_endTime,'publicTime is greater than endTime');

        if(_saleConfigList.length>0){
            SaleNFTConfig memory lastConfig = _saleConfigList[currentId];
            require(_whitelistTime>lastConfig.endTime,'The opening time must be greater than the end time of the previous sale');
        }

        _saleConfigList.push(SaleNFTConfig({
        ...
        midoNFT.updateTotalSupplyWithNewSale(_maxSupply);
    }
     ...
     */
    function getSaleConfig(uint64 _index) external view returns(SaleNFTConfig memory config) {
        require(_index<=currentId,'_index is out of the list sale config');
    ...
    function _validateAndUpdateWithBuyAmount(address buyer, uint64 amount) internal {
        SaleNFTConfig memory config = _saleConfigList[currentId];
        ...
        if (config.whitelistTime <= timestamp && timestamp < config.publicTime) {
            ...
            require(
                _userRecord[buyer].whitelistBought + amount <= config.capPerWhitelistedAddress,
        ...
        if (config.publicTime <= timestamp && timestamp < config.endTime) {
            ...
            require(totalPaid == amount * config.salePrice, 'invalid paid value');
            require(
                _userRecord[buyer].publicBought + _userRecord[buyer].whitelistBought + amount <= config.capPerAddress,
                'normal cap reached'
            );
