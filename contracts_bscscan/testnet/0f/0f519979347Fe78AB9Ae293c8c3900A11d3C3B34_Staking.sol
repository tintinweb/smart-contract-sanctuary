...
contract Staking is Ownable, ReentrancyGuard {
    ...
    function harvest() external nonReentrant {
        ...
        IERC20(stakeToken).transferFrom(hotWallet, msg.sender, unlockedTokens.mul(apy.mul(1e12).div(1e12)).mul(daysStaked).div(noOfDaysYear).div(100));
        
        _isFullyClaimed();
        
        emit StakeHarvest(msg.sender, block.timestamp, unlockedTokens);
        emit RewardHarvest(msg.sender, block.timestamp, unlockedTokens.mul(apy.mul(1e12).div(1e12)).mul(daysStaked).div(noOfDaysYear).div(100));
    }
