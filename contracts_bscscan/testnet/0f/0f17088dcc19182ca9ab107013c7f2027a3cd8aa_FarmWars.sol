...
contract FMDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("FM_Dividend_Tracker", "FM_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "FM_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "FM_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SURF contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "FM_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FM_Dividend_Tracker: Cannot update claimWait to same value");
...
contract FarmWars is ERC20, Ownable {
   
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    FMDividendTracker public dividendTracker;

    address public _marketingWallet = 0x842C29f96792908958ce1019FC266f39931AEc49;
    address public _buybackWallet = 0xC397c96Cbc6E5189647a825F17575Dd52501120e ;

    bool public restrictWhales = false;

    uint256 public maxTxAmount = 10000000  * (10**18);
    uint256 public walletMax = 100000000  * (10**18);
    ...
    constructor() ERC20("FarmWars", "FM") {

        dividendTracker = new FMDividendTracker();
        BuyTotalFees = BuyRewardsFee.add(BuyLiquidityFee).add(BuyMarketingFee).add(BuyBuybackFee);
        SellTotalFees = SellRewardsFee.add(SellLiquidityFee).add(SellMarketingFee).add(SellBuybackFee);
    ...
    function updateBuyFees(uint256 newBuyRewardFee, uint256 newBuyLiqFee, uint256 newBuyMarketingFee , uint256 newBuyBuybackFee) public onlyOwner {
    ...
    function updateSellFees(uint256 newSellRewardFee, uint256 newSellLiqFee, uint256 newSellMarketingFee ,uint256 newSellBuybackFee) public onlyOwner {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FM: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FM: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FM: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FM: Cannot update gasForProcessing to same value");
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 bnbForBuyback = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection);
