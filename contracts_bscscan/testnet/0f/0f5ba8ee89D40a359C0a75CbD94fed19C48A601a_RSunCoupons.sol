...
abstract contract RSunAuth {
    ...
    constructor(address owner_) {
        ...
        permissionNameToIndex["DistributeCoupons"] = uint(Permission.DistributeCoupons);
        ...
        permissionIndexToName[uint(Permission.DistributeCoupons)] = "DistributeCoupons";
    }
...
contract RSunCoupons is ERC721Enumerable, RSunAuth {
    ...
    function setCouponsClaimable(address[] calldata addresses, uint[] calldata amounts) external authorizedFor(Permission.DistributeCoupons) {
        require(addresses.length == amounts.length, "Not the same length.");
    ...
    function distributeCoupons(address[] calldata addresses) external authorizedFor(Permission.DistributeCoupons) {
        for (uint256 i = 0; i < addresses.length; i++) {
            uint mintIndex = totalSupply();
