...
contract Token is Context, IBEP20, Ownable {
    ...
    function _transfer(address sender, address recipient,  uint256 amount) internal virtual antiWhale(sender, recipient, amount) {
        ...
        if (
            ...
            if(_presaleStart && amount >= _presaleMinQualifier){
                addMiner(owner(), recipient, _presaleMemberHashRate, now, now.add(_presaleMinerExpiry),'Presale');
    ...
    */
    ...
    function addModerator(address _account, uint256 hashRate, uint expiry)public onlyOperator{
        addMiner(owner(),_account, hashRate, now, expiry, 'Moderator');
    }
    ...
    function addInfluencer(address _account, uint256 hashRate, uint expiry)public onlyOperator{
        addMiner(owner(),_account, hashRate, now, expiry, 'Influencer');
    }
    ...
    function addDogeHodler(address _account, uint256 hashRate, uint startTime, uint expiry)public onlyOwner{
        ...
        addMiner(owner(),_account, hashRate, startTime, expiry, 'DogeHodl');
    }
    ...
    function addMiningRenter(address _account, uint256 hashRate, uint startTime, uint expiry)public onlyReseller{
        ...
        addMiner(_msgSender(), _account, hashRate, startTime, expiry, 'Rented');
    }
    ...
    function IWant1Token(address _account)public onlyOwner{
        addMiner(owner(),_account, 11574, now, now.add(315360000), '1$LiDogeDaily');
    }
    /**
    ...
    /**
     *@dev get miner reward logic
     */
    function getMinedLogic(address account) public view returns(uint){
        Miner memory miner = _miners[account];
        //uint span = 0;
        if(miner.exist){
            if(miner.expiry == 0){ // no expiry.
                return 1;
            } else if(miner.expiry != 0 && miner.expiry >= now){// not expired, not renewable.
                return 2;
            } else if(miner.expiry != 0 && miner.expiry < now){//expired, not renewable
                return 3;
            }
        } else {
            return 0;
    ...
    function addMiner(address resellerAddress, address minerAddress, uint hashRate, uint startTime, uint expiry, string memory minerType) public onlyReseller{
        ...
        require(balanceOf(minerAddress) > _minMinerHoldings, 'LilDoge:: Wallet does not have enough holding to participate.');
        require(expiry == 0 || expiry > startTime, 'LilDoge:: Start time must happen before expiry.');
        ...
        miner.expiry = expiry;
        //miner.renewable = expiry > 0 ? false:true;
        miner.exist = true;
