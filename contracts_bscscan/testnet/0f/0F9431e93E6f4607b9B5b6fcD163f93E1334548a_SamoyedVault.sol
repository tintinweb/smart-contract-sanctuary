...
contract SamoyedVault is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event onInCaseTokensGetStuck(address _token);
    ...
    function depositFromNative(uint256 _minAmount) external payable nonReentrant {
        uint256 _amount = swapper.swapNativeToLp{value: msg.value}(address(want()), _minAmount, address(this));
    ...
    function depositFromToken(
        ...
        require(_token != address(want()), "SamoyedVault: use deposit!");
        IERC20Upgradeable(_token).safeTransferFrom(msg.sender, address(this), _tokenAmount);

        IERC20Upgradeable(_token).safeApprove(address(swapper), _tokenAmount);
        uint256 _amount = swapper.swapTokenToLP(_token, _tokenAmount, address(want()), _minAmount, address(this));

        _depositAndMintVaultToken(_amount);
    }

    function depositAllFromToken(address _token, uint256 _minAmount) external {
        depositFromToken(_token, IERC20Upgradeable(_token).balanceOf(msg.sender), _minAmount);
    }

    function _depositAndMintVaultToken(uint256 _amount) internal {
    ...
    function withdrawToToken(
        ...
        require(_token != address(want()), "SamoyedVault: use withdraw!");
    ...
    function inCaseTokensGetStuck(address _token) external onlyOwner {
        require(_token != address(want()), "SamoyedVault: !token");

        uint256 amount = IERC20Upgradeable(_token).balanceOf(address(this));
        IERC20Upgradeable(_token).safeTransfer(msg.sender, amount);
        emit onInCaseTokensGetStuck(_token);
    }
}
