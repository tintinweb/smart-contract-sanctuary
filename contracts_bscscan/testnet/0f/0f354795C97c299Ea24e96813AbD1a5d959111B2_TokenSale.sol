...
contract TokenSale is ReentrancyGuard {
    ...
    constructor(
        ...
        require(busdRate > 0, "busdRate is 0");
    ...
    function setBusdRate(uint256 busdRate) external onlyOwner {
        _busdRate = busdRate;
    }

    function setBnbRate(uint256 bnbRate) external onlyOwner {
        _bnbRate = bnbRate;
    }
    ...
    function purchaseFromBusd(uint256 busdAmount) external whenActive {
        ...
        uint256 token = busdAmount * _busdRate;
        require(_tokenPurchased[msg.sender].add(token) <= _purchaseLimit, "Individual cap exceeded");
    ...
    function withdraw() external onlyOwner whenInactive {
        uint256 quillBalance = _quill.balanceOf(address(this));
        _quill.transfer(_admin, quillBalance);
    }
