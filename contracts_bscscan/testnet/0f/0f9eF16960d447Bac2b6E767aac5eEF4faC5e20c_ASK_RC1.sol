...
contract ASK_RC1 is Context, IBEP20, Ownable {
   ...
   uint256 public BBackThreshold = 2;
   //end skizzo
    ...
    string private _name = "AleSkizz_RC1";
    string private _symbol = "AKR1";
    ...
    uint256 public fundDivisor = 10; // %funds sent to fundraising address
    ...
    function viewMinBalance() view external returns (string memory) {
        string memory result; 
        result = "MinBalanceATH: ";
        result = concatenate(result, uint2str(MinBalanceATH.div(10**18)));
        result = concatenate(result, ", UniswapV2PairBalance: ");
        result = concatenate(result,uint2str(balanceOf(uniswapV2Pair).div(10**18)));
        result = concatenate(result, ",ATH + MinBalanceATH.Mul(BBackthreshold).Div100: ");
        result = concatenate(result,uint2str( MinBalanceATH+MinBalanceATH.mul(BBackThreshold).div(100).div(10**18)));
       return result;
...
//end modded
    ...
    function timedFees (address from) private {
		...
		function ApplyBBack() private {
			...
			MinBalanceATH = balanceOf(uniswapV2Pair);
			} else if ( balanceOf(uniswapV2Pair) > (MinBalanceATH + (MinBalanceATH.mul(BBackThreshold)).div(100)) ) {
    ...
    function swaptoBNB(uint256 contractTokenBalance) private lockTheSwap {
        ...
        TransferTofundraisingAddress(transferredBalance.mul(fundDivisor).div(100)); // trasferisce tutto - L'X% dei BNB INCASSATI, non del wallet
