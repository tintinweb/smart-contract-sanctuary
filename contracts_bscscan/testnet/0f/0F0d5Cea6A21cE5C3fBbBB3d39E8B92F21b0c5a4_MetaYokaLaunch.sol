...
contract MetaYokaLaunch is Launch {
    ...
    event BuyLootBox(address indexed _from, uint256 _lootBoxType, uint256 _amount);
    ...
    // functions ---
    function buyLootBox(uint256 _type, uint256 _quantity) public {
        takeQuota(_type, _quantity);
        _token.safeTransferFrom(msg.sender, address(this), _prices[_type] * _quantity);

        emit BuyLootBox(msg.sender, _type, _quantity);
    }
...
abstract contract Launch is Ownable, Pausable {
    ...
    modifier hasQuota(uint256 index, uint256 amount) {
        require(index >= 0 && index < quotas.length, 'not a correct index');
