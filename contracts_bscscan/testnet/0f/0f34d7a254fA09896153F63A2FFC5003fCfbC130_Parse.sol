...
contract Parse is Ownable {
    ...
    mapping(string=>bool) private chainDict;
    ...
    function setBatchInfo(uint256 nftId,Single[] memory list) public {
        ...
        for (uint i = 0; i < list.length; i++) {
            ...
            if (item.itype == 1) {
                if (!chainDict[item.key]) {continue;}
                ...
                if (itemInfo.chainMap[item.key] > 0) {
                    ...
                    itemInfo.chainMap[item.key] = itemInfo.walletList.length-1;
                ...
                if (itemInfo.textMap[item.key] > 0) {
                    ...
                    itemInfo.textMap[item.key] = itemInfo.textList.length-1;
