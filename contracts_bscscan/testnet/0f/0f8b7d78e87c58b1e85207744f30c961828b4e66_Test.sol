...
contract Test is ERC20, Ownable {
    ...
    uint256 public BUSDRewardsBuyFee = 5;
    uint256 public MarketingBuyFee = 5;
    uint256 public totalBuyFees = BUSDRewardsBuyFee.add(MarketingBuyFee);
    
    uint256 public BUSDRewardsSellFee = 8;
    uint256 public MarketingSellFee = 6;
    uint256 public totalSellFees = BUSDRewardsSellFee.add(MarketingSellFee);

    address public _marketingWalletAddress = 0x9ff9F093D6148F10D51eb5CC5a9ed41032E22798;
    ...
    function updateSellFees(uint256 rewardFee, uint256 _marketingFee) external onlyOwner{
        BUSDRewardsSellFee = rewardFee;
        MarketingSellFee = _marketingFee;
        totalSellFees = BUSDRewardsSellFee.add(MarketingSellFee);
    }
    
    function updateBuyFees(uint256 rewardFee, uint256 _marketingFee) external onlyOwner{
        BUSDRewardsBuyFee = rewardFee;
        MarketingBuyFee = _marketingFee;
        totalBuyFees = BUSDRewardsBuyFee.add(MarketingBuyFee);
    }
    ...
    function swapAndSendDividends(uint256 tokens) private{
        ...
        swapTokensForBUSD(tokens);
        uint256 newBalance = (IERC20(BUSD).balanceOf(address(this))).sub(initialBUSDBalance);
        uint256 holdersShare = newBalance.mul(BUSDRewardsSellFee).div(BUSDRewardsSellFee);
        bool success = IERC20(BUSD).transfer(address(dividendTracker), holdersShare);

        if (success) {
            dividendTracker.distributeBUSDDividends(holdersShare);
