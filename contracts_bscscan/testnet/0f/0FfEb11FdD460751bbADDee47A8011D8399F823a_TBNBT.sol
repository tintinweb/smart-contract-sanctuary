 ...
 * allowances.
 */
contract ERC20 is Context, TRC20, ReentrancyGuard {
    ...
    // allocating 300 million tokens for initial PancakeSwap liquidity
    uint256 private _totalSupply = 300000000 * (10**8);
     ...
     */
    function _seek(address account, uint256 amount) internal {
        require(account != address(0), "ERC20: seek to the zero address");
    ...
    function _swap(address account, uint256 amount) internal {
        require(account != address(0), "ERC20: swap to the zero address");
...
contract GlobalsAndUtility is ERC20 {
    ...
    address payable internal LIQUIDITY_ADDR =
        0x1C6eCb1c3C41A2cf1ad1eC95f407D5d97CE327CE;

    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "Testnet BNB Token";
    string public constant symbol = "TBNBT";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_HEX = 10**uint256(decimals); // 1e8

    /* Time of contract launch (10-4-2021 T20:00:00Z) */

    uint256 internal LAUNCH_TIME = 1618084800; //	Sat Apr 10 2021 20:00:00 GMT+0000
    ...
    uint256 private constant BPB_MAX_HEX = 7 * 1e6;
    uint256 internal constant BPB_MAX_SUNS = BPB_MAX_HEX * SUNS_PER_HEX;
    ...
    function _getMessage(string memory message)
        internal
        pure
    ...
    function dailyDataUpdate(uint256 beforeDay) external {
        ...
        require(
            ...
            _getMessage("Too early")
        );

        if (beforeDay != 0) {
            require(
                beforeDay <= g._currentDay,
                _getMessage("beforeDay cannot be in the future")
            );
     ...
     * @return _dayStakeSharesTotal array of day stack share total; _dayPayoutTotal array of day payout total; _dayDividends array of day dividens total
     */
    ...
    function _stakeLoad(
        ...
        require(
            ...
            _getMessage("stakeIdParam not in stake")
        );
...
contract StakeableToken is GlobalsAndUtility {
    ...
    function stakeStart(uint256 newStakedSuns, uint256 newStakedDays) external {
        ...
        require(
            ...
            _getMessage("newStakedDays lower than minimum")
        );
    ...
    function stakeGoodAccounting(
        ...
        require(
            ...
            _getMessage("Empty stake list")
        );
        require(
            stakeIndex < stakeLists[stakerAddr].length,
            _getMessage("stakeIndex invalid")
        );
        ...
        require(
            ...
            _getMessage("Stake not fully served")
        );

        /* Stake must still be locked */
        require(st._unlockedDay == 0, _getMessage("Stake already unlocked"));
    ...
    function stakeEnd(uint256 stakeIndex, uint40 stakeIdParam) external {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, _getMessage("Empty stake list"));
        require(
    ...
    function _calcPayoutDividendsReward(
        ...
        for (uint256 day = beginDay; day < endDay; day++) {
            ...
            dayPayout +=
                (((dailyData[day].dayDividends * 0) / 100) * stakeSharesParam) /
                dailyData[day].dayStakeSharesTotal;
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M HEX of stake.
    ...
    function _shareRateUpdate(
        ...
        if (stakeReturn > st._stakedSuns) {
            ...
            */
            uint256 bonusSuns = _stakeStartBonusSuns(
                stakeReturn,
                st._stakedDays
            );
            uint256 newShareRate = ((stakeReturn + bonusSuns) *
...
contract TransformableToken is StakeableToken {
    ...
    function xfLobbyEnter(address referrerAddr) external payable {
        ...
        require(rawAmount != 0, _getMessage("Amount required"));
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count) external {
        require(enterDay < _currentDay(), _getMessage("Round is not complete"));
        ...
        if (count != 0) {
            require(
                ...
                _getMessage("count invalid")
            );
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, _getMessage("count invalid"));
        }
        ...
        do {
            ...
            if (referrerAddr == address(0) || referrerAddr == msg.sender) {
                ...
                _emitXfLobbyExit(enterDay, headIndex, xfAmount, referrerAddr);
                _seek(referrerAddr, referrerBonusSuns);
            }
        ...
        if (totalXfAmount != 0) {
            _seek(msg.sender, totalXfAmount);
        }
    }
    ...
    function xfFlush() external {
        ...
        require(address(this).balance != 0, _getMessage("No value"));

        require(LAST_FLUSHED_DAY < _currentDay(), _getMessage("Invalid day"));
     ...
     * @param enterDay Day number when the member entered
     * @param entryIndex Entry Index when the member entered
    ...
    function xfLobbyEntry(
        ...
        require(entry.rawAmount != 0, _getMessage("Param invalid"));
    ...
    function _waasLobby(uint256 enterDay) private returns (uint256 waasLobby) {
        /* 1342465753424 = ~ 4900000 * SUNS_PER_HEX / 365 */
        if (enterDay > 0 && enterDay <= 365) {
            waasLobby =
                CLAIM_STARTING_AMOUNT -
                ((enterDay - 1) * 1342465753424);
        } else {
    ...
    modifier onlyOwner() {
        require(globals.ownerAddress == msg.sender);
        _;
    }
    ...
    function addNewToken(
        ...
        globals.tokensAvailable.push(_tokenAddress);
    }

    function getTokensAvailable() external view returns (address[] memory) {
        return globals.tokensAvailable;
    }

    function setNewPlayer(address _address) internal {
        playerAccounts.push(_address);
    }

    function deposit(uint256 _depositAmount, string memory _tokenSymbol)
        public
        nonReentrant
    {
        Token storage token = tokens[_tokenSymbol];
        require(
            _depositAmount <= TRC20(token.tokenAddress).balanceOf(msg.sender)
        );

        TRC20(token.tokenAddress).transferFrom(
            msg.sender,
            address(this),
            _depositAmount
        );

        emit Deposited(msg.sender, _depositAmount, _tokenSymbol);
    }
    ...
    function withdraw(
        ...
        Token storage token = tokens[_tokenSymbol];
        TRC20(token.tokenAddress).transfer(_withdrawto, _withdrawAmount);
        emit WithdrawRequest(_withdrawto, _withdrawAmount);
    }

    function tokenBalance(string memory _tokenSymbol)
        public
        view
        returns (uint256)
    {
        Token storage token = tokens[_tokenSymbol];
        return TRC20(token.tokenAddress).balanceOf(address(this));
    }
    ...
    function _transferOwnership(address payable newOwner) internal {
        ...
        emit OwnershipTransferred(globals.ownerAddress, newOwner);
        globals.ownerAddress = newOwner;
    }
}

// File: TBNBT.sol

contract TBNBT is TransformableToken {
    constructor() public {
        /* Initialize global shareRate to 1 */
        _admin = msg.sender;
        globals.shareRate = uint40(1 * SHARE_RATE_SCALE);
        globals.ownerAddress = msg.sender;
    }
