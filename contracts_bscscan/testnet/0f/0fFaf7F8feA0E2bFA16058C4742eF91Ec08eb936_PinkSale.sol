    ...
    Referral
{
        ...
        )
    {
        ...
        uint256 _amountPerBNB = amount / perTokenPrice;
        return (
            lpFee.mul(amount).div(100),
            (buyBacksAmount * _amountPerBNB) / 100,
            (marketingWallet * _amountPerBNB) / 100
        );
    }
    ...
    function processBNBTax(
        ...
        // swapAndLiquify(lpAmount); // returns lp tokens to the liquidity wallet
        payable(buyBackAddress).transfer(buyBacksAmount);
    ...
    function setBuyBackWalletAddress(address buyBackAddress)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function sellWithBNB(uint256 amount) public {
    ...
    function buy() public payable coolDown returns (uint256) {
        ...
        uint256 totalSupply = totalSupply();
        uint256 _totalBuyLimite = totalSupply.div(tokens).mul(100);
      
        if (listingTaxEnabled == true) {
            
            uint256 _totalTax = totalSupply.div(tokens).mul(100);
            ...
            if (_totalTax > MAX_WALLET) {
                ...
                _mint(msg.sender, buyer.mul(perTokenPrice));
                emit Transfer(
                    address(this),
                    msg.sender,
    ...
    // You decided that buybacks is better to make from contract functions?
    // Well, no. Better add button buybackWithdraw. So that I can withdraw BNBs from
    // the contract to the address I add.
    // 3. AntiDump
    // Make contract increase the sell amount 100 per 1 min till it reaches max sell
    // 0.5% from market cap.

    function buybackWithdraw() external {
