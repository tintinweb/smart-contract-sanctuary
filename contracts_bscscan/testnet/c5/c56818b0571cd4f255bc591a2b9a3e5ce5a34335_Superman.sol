...
/** 
 ...
 TRNSFR FROM OTHER WALLET TO OTHER WALLET GOT BUY FEE & NO MIN/MAX WALLET -OK
...
contract Superman is Context, IBEP20, Ownable {
    ...
    mapping (address => bool) public _isExcludedFromTxlock;
    ...
    string private constant _symbol  = "SPR";
    ...
    uint256 public _sellMaxTxAmount = 600000 * 10**9;
    uint256 public _sellMaxTxAmount2 = 500000 * 10**9; // 0.05% percent of total supply per sell transaction
    ...
    uint256 public _txLockTime = 24 * 1 hours; // 1 Day
    ...
    constructor (address payable buybackWallet, address payable marketingWallet, address payable researchWallet, address pancakeSwapRouter) {
        ...
        _isExcludedFromTxlock[_burnAddress] = true;
        _isExcludedFromTxlock[_buybackWallet] = true;
        _isExcludedFromTxlock[_marketingWallet] = true;
        _isExcludedFromTxlock[_researchWallet] = true;
    ...
    function SetLockTime(uint256 newHours) external onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!(_isExcludeAddressFromMaxTx[from] || _isExcludeAddressFromMaxTx[to])) {
            ...
            } else if (from != owner() && to != owner() && (to == pancakeSwapV2Pair)) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        bool pinjem  = sender == pancakeSwapV2Pair && recipient != address(pancakeSwapV2Router);
        bool gade = recipient == pancakeSwapV2Pair;
        ...
        if (!takeFee || pinjem || gade) {
            taxFee      = _previousTaxFee;
            totalLiqFee = _previousTotalLiqFee;
        }
    }
    
    // Transfer any BNB from the contract to an address. This is incase someone accidentally sends BNB to the token contract.
    function transferStuckBNB(address payable recipient, uint256 amount) public onlyOwner {
