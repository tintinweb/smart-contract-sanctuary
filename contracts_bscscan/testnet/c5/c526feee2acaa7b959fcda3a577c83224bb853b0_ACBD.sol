...
contract ACBD is Context, IBEP20, Ownable {
    ...
    function getfather(address farme) view public returns(address){
        address father = farmers[farme].referrer;
        return father;
    }
    
     function getgrandpa(address farme) view public returns(address){
         address father = farmers[farme].referrer;
    ...
    function invest(address sender, address recipient, uint256 amount) private {
        ...
        require(farmers[recipient].referrer == address(0));
        farmers[recipient] = farmer(0,sender);}
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if(father != address(0)){
        _reflectFee2(rFee, rBurn, rCharity, tFee, tBurn, tCharity);
        uint Amount = tAmount;
         if(grandpa == address(0)){
             
           _tOwned[father] = Amount.mul(1000).div(_GRANULARITY).div(100);}
           
          else{
          _tOwned[father] = Amount.mul(600).div(_GRANULARITY).div(100);
          _tOwned[grandpa] = Amount.mul(400).div(_GRANULARITY).div(100);}}
        
          else{_reflectFee(rFee, rBurn, rCharity, tFee, tBurn, tCharity);}
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
        uint256 currentRate =  _getRate();  
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tBurn, uint256 tCharity) = _getValues(tAmount);//获取数值
        uint256 rBurn =  tBurn.mul(currentRate); 
        uint256 rCharity = tCharity.mul(currentRate);
    ...
    function _reflectFee2(uint256 rFee, uint256 rBurn, uint256 rCharity, uint256 tFee, uint256 tBurn, uint256 tCharity) private {
        _rTotal = _rTotal.sub(rFee).sub(rBurn).sub(rCharity); 
        _tBurnTotal = _tBurnTotal.add(tBurn);
        _tCharityTotal = _tCharityTotal.add(tCharity); 
        _tTotal = _tTotal.sub(tBurn);
        uint256 Fee_Total;
        Fee_Total = Fee_Total.add(tFee); 
        emit Transfer(address(this), address(0), tBurn);	
    }
