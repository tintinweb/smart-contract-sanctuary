...
contract Super is Ownable, IBEP20, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    address PCS = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;

    mapping (address => uint256) _balances;
    mapping (address => mapping (address => uint256)) _allowances;

    string _name = "Super";
    string _symbol = "Super";
    ...
    mapping (address => bool) excludeMaxWal;
    ...
    constructor () {
        ...
        excludeMaxWal[liquidity] = true;
        excludeFee[liquidity] = true;
        
        
        excludeMaxTxn[DEV] = true;
        excludeMaxWal[DEV] = true;
        excludeFee[DEV] = true;
        
        excludeFee[owner_] = true;
        excludeMaxTxn[owner_] = true;
        excludeMaxWal[owner_] = true;

        excludeDividend[pair] = true;
        excludeMaxTxn[pair] = true;
        excludeMaxWal[pair] = true;
    ...
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        require(amount <= _maxWallet || excludeMaxWal[sender] || excludeMaxWal[recipient], "MaxWal Limit Exceeded");
        uint256 currentBalance = balanceOf(recipient);
       require(excludeMaxWal[recipient] || (currentBalance + amount <= _maxWallet));
    }
    ...
    function setTxWal(uint256 amount) external onlyOwner {
        require(amount >= _totalSupply *80 / 2000);
        _maxWallet = amount;
    }
