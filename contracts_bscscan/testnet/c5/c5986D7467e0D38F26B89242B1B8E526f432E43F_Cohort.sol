...
contract Cohort is ICohort, ReentrancyGuard {
    ...
    function batchImplementForDepositRequest() external onlyCohortOwner nonReentrant {
        for (uint256 ii = 0; ii < allRiskPools.length; ii++) {
            address pool = getRiskPool[allRiskPools[ii]];
            address[] memory requestUserList = IRiskPool(pool).depositRequestList();
            for (uint kk = 0; kk < requestUserList.length; kk++) {
                IRiskPool(pool).updateUserDeposit(requestUserList[kk]);
            }
            IRiskPool(pool).updateTotalDeposit();
        }
    }
    ...
    function batchImplementForWithdrawRequest() external onlyCohortOwner nonReentrant {
        for (uint256 ii = 0; ii < allRiskPools.length; ii++) {
            address pool = getRiskPool[allRiskPools[ii]];
            address[] memory requestUserList = IRiskPool(pool).withdrawRequestList();
            for (uint kk = 0; kk < requestUserList.length; kk++) {

                IRiskPool(pool).updateUserWithdraw(requestUserList[kk]);
            }
            IRiskPool(pool).updateTotalWithdraw();
        }
    }
