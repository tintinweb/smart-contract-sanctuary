...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     * Bnber and Wei. This is the value {ERC20} uses, unless this function is
     * overridden;
     *
...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  //  see https://github.com/bnbereum/EIPs/issues/1726#issuecomment-472352728
  ...
  /// @dev Distributes dividends whenever bnber is paid to this contract.
...
contract GFOXDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("GFOX_Dividend_Tracker", "GFOX_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100000000 * (10**18); //must hold 100000000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "GFOX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "GFOX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GFOX contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GFOX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "GFOX: Cannot update claimWait to same value");
...
contract GFOX is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    GFOXDividendTracker public dividendTracker;
    ...
    constructor() ERC20("GamblerFox", "GFOX") {
        BNBRewardsFee = 6;
        liquidityFee = 4;
        extraFeeOnSell = 0;
        marketingFee = 4;
        marketingWallet = payable(0x0146CC852f72612fb5de1f673F32C7A90bd4C972);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new GFOXDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GFOX: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GFOX: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GFOX: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GFOX: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GFOX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GFOX: Cannot update gasForProcessing to same value");
