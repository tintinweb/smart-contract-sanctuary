...
contract RetroNeko is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function _currentFees() private view returns (uint256, uint256, uint256, uint256, uint256) {
    ...
    function _totalBuyFees() private view returns (uint256) {
        return (
            _buyFees.liquidityFeesPerTenThousand
                ...
                .add(_buyFees.warFeesPerTenThousand));
    }
    ...
    function _totalSellFees() private view returns (uint256) {
        return (
            _sellFees.liquidityFeesPerTenThousand
                ...
                .add(_sellFees.warFeesPerTenThousand));
    }
    ...
    function _swapSendFeesAndLiquify() private swapping {
        ...
        uint256 warEthBalance = ethAmount.mul(warFeesPerTenThousand).div(totalFeesPerTenThousand);
        _sendFeesToWallet(_warWallet, warEthBalance);

        uint256 liquidityEthAmount = ethAmount.sub(teamEthAmount).sub(marketingEthAmount).sub(warEthBalance);
