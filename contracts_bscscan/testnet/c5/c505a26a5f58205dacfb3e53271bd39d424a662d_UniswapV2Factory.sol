...
contract UniswapV2ERC20 {
    ...
    string public constant name = 'Vinci Swap LP Token';
...
contract UniswapV2Pair is UniswapV2ERC20 {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    // uint liquidity = rootK.sub(rootKLast);
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        uint balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(IUniswapV2Factory(factory).feeRate()));
        uint balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(IUniswapV2Factory(factory).feeRate()));
