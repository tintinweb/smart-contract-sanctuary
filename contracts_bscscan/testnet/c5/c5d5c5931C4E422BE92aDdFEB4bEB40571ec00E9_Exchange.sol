...
contract Exchange is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function calcLoanAmountAndExchangeRate(
        ...
        if (collateralAddress == address(0)) {
            collateralToUSD =
                (uint256(RateBNBwithUSD()) * loanToValue * amount) / (100 * 10**5);
        } else {
            collateralToUSD =
                (uint256(getLatesPriceToUSD(collateralAddress))  * loanToValue * amount) / (100 * 10**5);
        }
        ...
        if (loanAsset == address(0)) {
            ...
            rateLoanAsset =
                uint256(getLatesPriceToUSD(loanAsset));
        }
