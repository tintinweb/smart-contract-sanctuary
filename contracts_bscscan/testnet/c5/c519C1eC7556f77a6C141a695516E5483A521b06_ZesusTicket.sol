...
contract ZesusTicket is ERC721Enumerable, Ownable {
  ...
  constructor(ESHK token, string memory baseURI)
    ERC721('eShark Token', 'ESHK')
  {
    _token = token;
    setBaseURI(baseURI);
  }
  ...
  function mintNFTs() public onlyOwner {
    require(
      ...
      'Mint NFT more than Max supply not allowed'
    );

    for (uint256 i = totalSupply(); i < MAX_SUPPLY; i++) {
      _mintSingleNFT();
    }
  }

  function _mintSingleNFT() public onlyOwner returns (uint256) {
