...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - see https://github.com/ETH/solidity/issues/2691
        return msg.data;
    }
}
...
library Address {
     ...
     *
     * https://eips.ETH.org/EIPS/eip-1884[EIP1884] increases the gas cost
...
contract SacredTails is Context, IBEP20, Ownable {
    ...
    string private _name = "Sacred Tails";
    ...
    uint256 public totalSellFees =  sellMarketingFee + selltournementFee;
    ...
    address payable public tournmentAddress;
    
    constructor (address _router,address payable _marketingAddress,address payable _tournmentAddress) public {
        ...
        tournmentAddress = _tournmentAddress;
        
        //exclude owner and this contract from fee
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[marketingAddress] = true;
        _isExcludedFromFee[tournmentAddress] = true;
    ...
    function setBuySwappingFeePercent(uint256 _buyMarketingFee, uint256 _buytournementFee) external onlyOwner() {
        buyMarketingFee = _buyMarketingFee;
        buytournementFee = _buytournementFee;
        totalBuyFees = buyMarketingFee + buytournementFee;
    }

    function setSellSwappingFeePercent(uint256 _sellMarketingFee, uint256 _selltournementFee) external onlyOwner() {
        sellMarketingFee = _sellMarketingFee;
        selltournementFee = _selltournementFee;
        totalSellFees = sellMarketingFee + selltournementFee;
    }

    function setTransferSwappingFeePercent(uint256 _transferMarketingFee, uint256 _transfertournementFee) external onlyOwner() {
    ...
    function setmarketingAddress(address payable _market) external onlyOwner(){
        marketingAddress = _market;
    }

    function settournmentAddress(address payable _tournment) external onlyOwner(){
        tournmentAddress = _tournment;
    }
    ...
    function _getTValues(uint256 tAmount, uint256 _type) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tSwapping);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tSwapping, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rSwapping = tSwapping.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rSwapping);
    ...
    function _takeSwapping(uint256 tSwapping) private {
        uint256 currentRate =  _getRate();
        uint256 rSwapping = tSwapping.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rSwapping);
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tSwapping);
    }
    ...
    function calculateTaxFee(uint256 _amount, uint256 _type) private view returns (uint256) {
        ...
        return _amount.mul(fees).div(10**2);
    }
   ...
   function _transfer(
        ...
        if(from == address(pancakeV2Pair)){ // Buy
            _type = 1;
        }
        else if(to == address(pancakeV2Pair)){ // Sell
            require(amount <= totalSupply().mul(5).div(1000), "BEP20 : Sell Amount Exceed");
            _type = 2;
        }
        else{                           // Transfer
            _type = 3;
    ...
    function _feeDistribution(uint256 _amount, uint256 _type) private {
        ...
        if(_type == 1 ) {
            marketFees    =    _amount.mul(buyMarketingFee).div(100);
            tournmentFees =    _amount.mul(buytournementFee).div(100);
        }
        else if (_type == 2) {
            marketFees    =    _amount.mul(sellMarketingFee).div(100);
            tournmentFees =    _amount.mul(selltournementFee).div(100);
        }
        else{
            marketFees    =    _amount.mul(transferMarketingFee).div(100);
            tournmentFees =    _amount.mul(transfertournementFee).div(100);
        }
        feesValues[marketingAddress]= feesValues[marketingAddress].add(marketFees);
        feesValues[tournmentAddress] = feesValues[tournmentAddress].add(tournmentFees);

    }

    function swapping()external payable{
        require(marketingAddress == _msgSender() || tournmentAddress == _msgSender() , "BEP20 : Only Marketing and Development Address");
        require(feesValues[_msgSender()] > 0 , "BEP20 : Invalid Amount");
        uint256 swapAmount;

        swapAmount = feesValues[_msgSender()];
        feesValues[_msgSender()] = 0;
        swapTokensForETH(_msgSender(),swapAmount);
    }
    ...
    function viewFee(address _user)public view returns(uint256){
