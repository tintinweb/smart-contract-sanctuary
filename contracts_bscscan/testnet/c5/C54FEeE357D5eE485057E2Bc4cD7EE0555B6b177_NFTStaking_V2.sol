...
contract NFTStaking_V2 is IERC721Receiver, ReentrancyGuard {
    ...
    uint256 public farmingEndTimestamp = block.timestamp + 20 days; // No more rewards after 20 days;
    ...
    function _stakeNFT(uint256 tokenId) internal returns (bool) {
        // require farming has not been ended
        require(block.timestamp <= farmingEndTimestamp, 'Stake: Farming has ended');
    ...
    // Setters
    function setFarmingEndTimestamp(uint256 _endTimeStamp) public onlyDao {
        require(_endTimeStamp != 0, 'setFarmingEndTimestamp: Cannot be 0');
        farmingEndTimestamp = _endTimeStamp;
    }
}