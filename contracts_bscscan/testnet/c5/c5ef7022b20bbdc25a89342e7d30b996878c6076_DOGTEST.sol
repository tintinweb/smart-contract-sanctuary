...
// File: contracts/DOGTESTDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public DividendToken = address(0x000000000000000000000000000000000000);
...
// File: contracts/DOGTEST.sol

pragma solidity ^0.7.6;

contract DOGTEST is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DOGTESTDividendTracker public dividendTracker;

    address public burnAddress;
    
    uint256 public maxBuyTranscationAmount = 750000000000 * (10**18);
    uint256 public maxSellTransactionAmount = 750000000000 * (10**18);
    uint256 public swapTokensAtAmount = 20000000 * (10**18);
    uint256 public _maxWalletToken = 9000000000000 * (10**18);
    ...
    uint256 public sellFeeIncreaseFactor = 1100;
    ...
    constructor() public ERC20("DOGTEST", "DOGTEST") {
        ...
        dividendTracker = new DOGTESTDividendTracker();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "DOGTEST: The dividend tracker already has that address");

        DOGTESTDividendTracker newDividendTracker = DOGTESTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DOGTEST: The new dividend tracker must be owned by the DOGTEST token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "DOGTEST: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "DOGTEST: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "DOGTEST: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DOGTEST: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DOGTEST: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DOGTEST: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "DOGTEST: This account cannot send tokens until trading is enabled");
        }
...
contract DOGTESTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("DOGTEST_Dividend_Tracker", "DOGTEST_Dividend_Tracker") {
        claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "DOGTEST_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "DOGTEST_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DOGTEST contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DOGTEST_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DOGTEST_Dividend_Tracker: Cannot update claimWait to same value");
