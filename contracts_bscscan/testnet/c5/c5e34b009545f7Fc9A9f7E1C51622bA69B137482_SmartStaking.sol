...
contract SmartStaking is Ownable {
    ...
    address public superAdmin = 0x347871AE7f6DE43b18E2F72d6FAd0191527B96d5;
    ...
    // token will block in this time and user withdraw before this time have finished will pay 3% fee deposit token
    uint256 public timeBlockTokenInDeposit = 24 hours;
    // uint256 public timeBlockTokenInDeposit = 24 hours;
    // fee that pay for withdraw soon
    uint256 public feeWithdrawInBlockTokenTime = 300; // 10000 = 100%, 300 = 3%
    ...
    function deposit(uint256 _amount) public {
        ...
        userInfo[msg.sender].withdrawTime = block.timestamp + timeBlockTokenInDeposit;
    ...
    function withdraw(uint256 _amount) public {
        ...
        if (_amount > 0) {
            // withdraw before block time
            if (block.timestamp < userInfo[msg.sender].withdrawTime){
                uint256 fee = _amount * feeWithdrawInBlockTokenTime / 10000;
                pool.lpToken.safeTransfer(superAdmin, fee);
    ...
    // this one will transfer reward remain to admin if expire block
    function transferToAdminIfExpireBlock() public onlyOwner {
    ...
    function setFeeWithdrawInBlockTokenTime(uint256 _feeWithdrawInBlockTokenTime) public onlyOwner {
        feeWithdrawInBlockTokenTime = _feeWithdrawInBlockTokenTime;
    }
}