...
// max amount of raising tokens  to be raised
  uint256 public maxCap = 1200000000000000000000;    // 1200 busd
  ...
  function deposit(uint256 _amount) public {
    ...
    // require(userInfo[msg.sender].amount.add(_amount) <= 2500000000000000000, 'maximum contribution reached');
...
//   // toremove
//   function modifyBlock(uint256 theStartBlock) public  {
//       startBlock = theStartBlock;
//   }
  
//     function modifyEndBlock(uint256 theEndBlock) public  {
//       endBlock = theEndBlock;
//   }
  
//   function modifyMinCap(uint256 theMaxRaised) public {
//       raisingAmount = theMaxRaised;
//   }
  
//     function modifyMaxCap(uint256 theminRaised) public {
//       maxCap = theminRaised;
//   }
...
//   // to remove end
  ...
  function harvest() public nonReentrant {
    ...
    if(totalAmount >= raisingAmount ){
        offeringToken.safeTransfer(address(msg.sender), offeringTokenAmount.mul(2)); // Get 2 tokens for every token
    }
    else {
  ...
  function getOfferingAmount(address _user) public view returns(uint256) {
    ...
    //   uint256 allocation = getUserAllocation(_user);
      return userInfo[msg.sender].amount.mul(2); // Get 2 for every token you put
    }
    else {
  ...
  function burnTreats() public onlyAdmin{
    require (block.number > endBlock, 'not withdrawal time');
    require (totalAmount >= raisingAmount, 'min cap not reached');
    uint256 totalToBurn = maxCap - totalAmount; 
    if(offeringToken.balanceOf(address(this)) > 0) {
      offeringToken.safeTransfer(0x000000000000000000000000000000000000dEaD, totalToBurn.mul(2)); // 2 tokens for every token
    }
