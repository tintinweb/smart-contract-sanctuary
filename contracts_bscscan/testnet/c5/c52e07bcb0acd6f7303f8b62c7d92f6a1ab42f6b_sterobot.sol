...
/*
 ________  _________  _______   ________  ________  ________  ________  _________   
|\   ____\|\___   ___\\  ___ \ |\   __  \|\   __  \|\   __  \|\   __  \|\___   ___\ 
\ \  \___|\|___ \  \_\ \   __/|\ \  \|\  \ \  \|\  \ \  \|\ /\ \  \|\  \|___ \  \_| 
 \ \_____  \   \ \  \ \ \  \_|/_\ \   _  _\ \  \\\  \ \   __  \ \  \\\  \   \ \  \  
  \|____|\  \   \ \  \ \ \  \_|\ \ \  \\  \\ \  \\\  \ \  \|\  \ \  \\\  \   \ \  \ 
    ____\_\  \   \ \__\ \ \_______\ \__\\ _\\ \_______\ \_______\ \_______\   \ \__\
   |\_________\   \|__|  \|_______|\|__|\|__|\|_______|\|_______|\|_______|    \|__|
   \|_________|                                                                     
...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "Test_Sterobot";
    string internal constant SYMBOL = "$SBT";
    ...
    uint256 internal constant maxTransactionAmount =  4000000000000*ZEROES; 

    uint256 internal constant maxWalletBalance = TOTAL_SUPPLY / 20; // 2% of the total supply
    
    uint256 internal constant numberOfTokensToSwapToLiquidity = 5000000000000*ZEROES; 

    address internal devAddress = 0x2b044809B70cF7baaB8c943C9931e0dE15c87E2C;
     
    address internal burnAddress = 0x000000000000000000000000000000000000dEaD;

     address internal marketingAddress = 0xa4d8057CCB1E92241de3521C772143F8933113c6;

    enum FeesEnum { Antiwhale, Burn, Liquidity, Rfi, External, ExternalToETH }
    struct Fee {
        FeesEnum name;
        uint256 value;
        address recipient;
    ...
    function _addFee(FeesEnum name, uint256 value, address recipient) private {
        fees.push( Fee(name, value, recipient, 0 ) );
        sumOfFees += value;
    }

    function _addFees() private {
        _addFee(FeesEnum.Rfi, 30, address(this) ); 
        _addFee(FeesEnum.Burn, 0, burnAddress); 
        _addFee(FeesEnum.Liquidity, 50, address(this) );
        _addFee(FeesEnum.External, 20, devAddress );
        _addFee(FeesEnum.External, 20, marketingAddress);
    }
    ...
    function _getFee(uint256 index) internal view returns (FeesEnum, uint256, address, uint256){
...
abstract contract Sterobot is BaseRfiToken, Liquifier, Antiwhale {
    ...
    function _takeTransactionFees(uint256 amount, uint256 currentRate) internal override {
        ...
        for (uint256 index = 0; index < feesCount; index++ ){
            (FeesEnum name, uint256 value, address recipient,) = _getFee(index);
            // no need to check value < 0 as the value is uint (i.e. from 0 to 2^256-1)
            if ( value == 0 ) continue;

            if ( name == FeesEnum.Rfi ){
                _redistribute( amount, currentRate, value, index );
            }
            else if ( name == FeesEnum.Burn ){
                _burn( amount, currentRate, value, index );
            }
            else if ( name == FeesEnum.Antiwhale){
                // TODO
            }
            else if ( name == FeesEnum.ExternalToETH){
...
contract sterobot is Sterobot{
    constructor() Sterobot(Env.Testnet){
