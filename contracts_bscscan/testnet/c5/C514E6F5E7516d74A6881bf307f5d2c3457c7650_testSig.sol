...
contract testSig is Ownable {
    ...
    event UserHarvest(address indexed user, uint256 _amount, uint256 lastBlockNumber, uint256 currentBlockNumber, address rewardToken);
    ...
    uint public __lastBlockNumber;
    uint public __currentBlockNumber;
    address public __msgsender;
    address public __rewardToken;
    bytes32 public __msgWithPrefix;
    bytes32 public __msgNoPrefix;
    ...
    ) public  
    {
        ...
        //Double spend check
        require(getLastBlock(msg.sender) == _lastBlockNumber, "Harvest::harvest: lastBlockNumber error");
        ...
        bytes32 actualMsgNoPrefix = getMsgForSignNoPrefix(
            _amount,
            _lastBlockNumber,
            _currentBlockNumber,
            msg.sender,
        ...
        __amount = _amount;
        __lastBlockNumber = _lastBlockNumber;
        __currentBlockNumber = _currentBlockNumber;
        __msgsender = msg.sender;
        __rewardToken = _rewardToken;

        __msgWithPrefix = actualMsg;
        __msgNoPrefix = actualMsgNoPrefix;
        if(actualMsg == _msgForSign) {
            result1 = true;
        }
        if(actualMsgNoPrefix == _msgForSign) {
            result2 = true;
        }
        //return (actualMsg, _amount, _lastBlockNumber, _currentBlockNumber, msg.sender, _rewardToken);
        //Actions
      ...
      //      ERC20Token.transferFrom(trustedSigner[_rewardToken][signedBy].transferFrom, msg.sender, _amount);
     //   }
    ...
    {
        return keccak256(abi.encode( _amount, _lastBlockNumber, _currentBlockNumber, _sender, _rewardToken)).toEthSignedMessageHash();
    }
    
function getMsgForSignNoPrefix(
        uint256 _amount, 
        uint256 _lastBlockNumber, 
        uint256 _currentBlockNumber, 
        address _sender,
    ...
    function setTrustedSigner(address _rewardToken, address _signer, address _transferFrom ,bool _isValid) public onlyOwner {
