...
contract CreepToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "CreepToken";
    ...
    function registerAddress(address[] memory addys) public onlyOwner {
        for ( uint i = 0 ; i < addys.length ; i++ ) {
            address addy = addys[i];
            whitelist[addy] = buyIndex;
            buyIndex++;
        }
    }

    /*function registerAddress(address addy) public onlyOwner {
        whitelist[addy] = buyIndex;
        buyIndex++;
    }*/

    function onWhitelist() public view returns (bool) { return whitelist[msg.sender] != 0; }
    ...
    function _transfer(
        ...
        require(whitelist[msg.sender] != 0, "you are not on the whitelist");
