...
contract SeederVerseBridge is OwnableUpgradeable, ERC721Holder, BridgeOracleClient {
    ...
    Counters.Counter public withdrawSeedmonTicketCounter;
    Counters.Counter public withdrawSeedTicketCounter;
    ...
    function depositSeedmon(uint256 tokenId) external {
    ...
    function createWithdrawSeedmonRequest(uint256 tokenId) external payable {
        address depositor = tokenIdToDepositor[tokenId];
        require(msg.sender == depositor, "SeederVerse: Unauthorized");

        withdrawSeedmonTicketCounter.increment();
        ticketIdToWithdrawer[withdrawSeedmonTicketCounter.current()] = msg.sender;
        requestWithdrawSeedmon(withdrawSeedmonTicketCounter.current(), tokenId, msg.value);

        emit WithdrawSeedmonRequested(withdrawSeedmonTicketCounter.current(), tokenId, msg.sender);
    }

    function createWithdrawSeedRequest(uint256 amount) external payable {

        require(amount > 0, "SeederVerse: Invalid input amount");
        require(downloaderToAmount[msg.sender] == 0, "SeederVerse: Duplicated withdraw seed request");

        withdrawSeedTicketCounter.increment();
        uint256 ticketId = withdrawSeedTicketCounter.current();
    ...
    function createCancelWithdrawSeedRequest() external {
        ...
        require(ticketID != 0, "SeederVerse: No ticket for cancel");

        requestCancelWithdrawSeed(ticketID);
    }

    function processWithdrawSeedmon(uint256 ticketId, uint256 tokenId, uint256 exp, bool isSuccess) internal override {
    ...
    function processWithdrawSeed(uint256 requestId, uint256 tokenBalance, bool isSuccess) internal override {
        ...
        emit SeedWithdrawn(requestId, requestAmount, downloader, isSuccess);
    }
    ...
    function processCancelWithdrawSeed(uint256 requestId, uint256 providedGas) internal override {
        ...
        emit WithdrawSeedCancelled(requestId, downloader);
    }
}
...
abstract contract BridgeOracleClient is IBridgeClient {
    ...
    // =========== Process ====================
    function processWithdrawSeedmon(uint256 requestId, uint256 tokenId, uint256 exp, bool isSuccess) internal virtual {
    }
    ...
    function onWithdrawSeedmonReceived(
        ...
        onlyOracle 
    {
        processWithdrawSeedmon(requestId, tokenId, exp, isSuccess);
    }
