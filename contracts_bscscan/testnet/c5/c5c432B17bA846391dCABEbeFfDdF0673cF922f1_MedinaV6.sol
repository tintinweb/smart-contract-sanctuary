...
contract MedinaV6 is Context, IERC20, Ownable {
    ...
    string private _name = "MedinaV6";
    string private _symbol = "MED6";
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tCharity, uint256 tMarketing) = _getValuesT(tAmount);
    ...
    function setGiveawayAddress(address payable giveaway) public onlyOwner {
        _charityAddress = giveaway;
    }
    ...
    function showGiveawayAddress() public view returns(address payable) {
...
//     }
    ...
    function _getTValuesCharityAndMarketing(uint256 tAmount) private view returns (uint256, uint256) {
    ...
    /* split into two function, one that returns rvariables and one that returns tvariables*/
    ...
    //     (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tCharity, tMarketing);
    ...
    function _getValuesR(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        (, uint256 tFee, uint256 tLiquidity, uint256 tCharity, uint256 tMarketing) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tCharity, tMarketing);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 tCharity, uint256 tMarketing) = _getTValuesCharityAndMarketing(tAmount);
        //uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tCharity).sub(tMarketing); split into two lines
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity);
         tTransferAmount = tTransferAmount.sub(tCharity).sub(tMarketing);
        return (tTransferAmount, tFee, tLiquidity, tCharity, tMarketing);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tCharity, uint256 tMarketing) private view returns (uint256, uint256, uint256) {
        uint256 currentRate = _getRate();
...
*/
    ...
    function _takeMarketing(uint256 tMarketing) private {
        uint256 currentRate =  _getRate();
        uint256 rMarketing = tMarketing.mul(currentRate);
        _rOwned[_marketingDevAddress] = _rOwned[_marketingDevAddress].add(rMarketing);
        if(_isExcluded[_marketingDevAddress])
            _tOwned[_marketingDevAddress] = _tOwned[_marketingDevAddress].add(rMarketing);
    }
    ...
    function calculateMarketingFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_marketingAndDevBudget).div(
            10**2
        );
    }
