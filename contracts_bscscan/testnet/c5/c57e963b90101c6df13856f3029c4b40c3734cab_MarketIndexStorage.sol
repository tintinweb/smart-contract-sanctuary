...
contract MarketIndexStorage {
    ...
    {
        require(tokens.contains(_address));
        uint categoryCountForToken = categoryMaxValues[_address].length;
        require(_categoryValues.length == categoryCountForToken);
        require(_categoryFiltersActive.length == categoryCountForToken);
        ...
        for (uint8 a = 0; a < categoryMaxValues[_address][0]; a++) {
            ...
            for (uint8 b = 0; b < categoryMaxValues[_address][1]; b++) {
                if (_categoryFiltersActive[1] && (_categoryValues[1] != b)) {
                    continue;
                }

                keyCategoryValues[0] = b;
                for (uint8 c = 0; c < categoryMaxValues[_address][2]; c++) {
                    if (_categoryFiltersActive[2] && (_categoryValues[2] != c)) {
                        continue;
                    }

                    keyCategoryValues[0] = c;
                    for (uint8 d = 0; d < categoryMaxValues[_address][3]; d++) {
                        if (_categoryFiltersActive[3] && (_categoryValues[3] != d)) {
                            continue;
                        }

                        keyCategoryValues[0] = d;
                        for (uint8 e = 0; e < categoryMaxValues[_address][4]; e++) {
                            if (_categoryFiltersActive[4] && (_categoryValues[4] != e)) {
                                continue;
                            }

                            keyCategoryValues[0] = e;
                            uint8[] memory key = _keyFromCategoryValues(_address, keyCategoryValues);
                            EnumerableSet.UintSet storage set = _getIndexBucket(_address, key);
