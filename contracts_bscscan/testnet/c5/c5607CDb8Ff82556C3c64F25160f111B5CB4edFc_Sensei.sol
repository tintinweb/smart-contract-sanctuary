...
contract Ownable is Context {
    ...
    modifier onlyOwner() {
     ...
     /**
     * @dev Leaves the contract without full owner. It will not be possible to call
...
contract Sensei is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000000000 * 10**3 * 10**2;
    ...
    uint256 public _maxTxAmount = 5000000 * 10**3 * 10**2; // 5 000 000 000 (+ 2 decimals)
    uint256 private numTokensSellToAddToLiquidity = 500000 * 10**3 * 10**2;  // 500 000 000 000 (+ 2 decimals)
    ...
    constructor () public {
        ...
        emit Transfer(address(0), 0xfED3051B554A9FCf8eDFbAd7a597f54c7d61814c, 10000000 * 10**3 * 10**2); // Team - Anubis - 1%
        emit Transfer(address(0), 0xBeC16697926f43a9c6de7817a8BCce335F366BcD, 5000000 * 10**3 * 10**2); // Team Member - 0,5%
        emit Transfer(address(0), 0xaBd5fe6a52F8a33c58a19254D6A1fcF4709C2526, 5000000 * 10**3 * 10**2); // Team Member - 0,5%
        emit Transfer(address(0), 0x3184bF7c88b72C470EC61EB8aeC7bD4278649986, 5000000 * 10**3 * 10**2); // Team Member - 0,5%
        emit Transfer(address(0), 0x1DfEDFEdfC4C428A0F9850ea89430CD906C4E69e, 5000000 * 10**3 * 10**2); // Team Member - 0,5%
        emit Transfer(address(0), 0x1FDe37b4fE004A28e57f7DCDfbCD8FfaB9a2b28C, 5000000 * 10**3 * 10**2); // Team Member - 0,5%
        emit Transfer(address(0), 0x2b055675cB9156D1e393a2BD8a89E0960EF5A061, 1000000 * 10**3 * 10**2); // Team Member - 0,1%
        emit Transfer(address(0), 0x10cD8079755bC23b7E644F60627478e62E8817D5, 5000000 * 10**3 * 10**2); // Dev - 0,5%
        emit Transfer(address(0), 0xb79B40D3798A84509Cb802adDD24D2422C9B73C6, 5000000 * 10**3 * 10**2); // Dev - 0,5%
        emit Transfer(address(0), 0xbfe92F7AF15441eBB41aC49902Bf1C073EA05285, 5000000 * 10**3 * 10**2); // Dev - 0,5%
        emit Transfer(address(0), 0xD6cC5aD9440E0339F2C7b7D192633bD0Fb1D8EA0, 5000000 * 10**3 * 10**2); // Dev - 0,5%
        emit Transfer(address(0), 0x055AC601eAa45EAc36466A67f535A02A6481Ba73, 12000000 * 10**4 * 10**2); // Future Marketing - 12%
        emit Transfer(address(0), 0x83A49af14B243c609F8e7f09dab13065bf4c826A, 30000000 * 10**3 * 10**2); // Future Development - 3%
        emit Transfer(address(0), 0x7cb8Bd57826c00672CFd03412468B7FBeF63279f, 64000000 * 10**3 * 10**2); // Future team 6,4%
        emit Transfer(address(0), 0xa3448570Ac5b48e7C40D2356e7dE8aCc9efe2C27, 80000000 * 10**3 * 10**2); // Future advisor 8%
    ...
    function _takeBurn(uint256 tBurn) private {
        ...
        if (isMember(tx.origin) && txAmountMember[tx.origin] < 2) {
            ...
            if (currentTxAmount > 0) {
                ...
                txAmountMember[tx.origin] = 1; // TO TEST
            }
    ...
    function burn(uint256 _value) public returns (bool) {
        ...
        require(_value <= senderBalance, "Not enough tokens");
        uint256 currentRate =  _getRate();
        ...
        decreaseROwned(_msgSender(), rValue);
        decreaseTOwned(_msgSender(), _value);
