...
contract LazerMerchantWalletController is Ownable, ILazerStorageSchema {
    ...
    function _updateStakeRecord(
        ...
        storageContract.updateStakeRecord(merchantId, walletId, derivativeBalance, derivativeTotalWithdrawn, derivativeTotalDeposits, stakeToken);
    }
    ...
    function getStakingRecordByWallet(address wallet) public view returns (StakingRecord memory) {
        ...
        return StakingRecord(merchantId, derivativeBalance, derivativeTotalWithdrawn, derivativeDeposits, stakeToken, walletId, exists);
    }
    ...
    function stakeFundsToProtocol(
        ...
        if (exists) _updateStakeRecord(stakeRecord);
            ...
            storageContract.createStakeRecord(
                ...
                stakeRecord.derivativeTotalDeposits,
    ...
    function _updateStakeRecordAfterDeposit(
        ...
        if (!exists) {
            StakingRecord memory stakingRecord = StakingRecord(merchant, sharesAmount, 0, sharesAmount, derivativeToken, wallet, true);
            return stakingRecord;
        } else {
            StakingRecord memory stakingRecord = getStakingRecordByWallet(wallet);

            stakingRecord.derivativeTotalDeposits = stakingRecord.derivativeTotalDeposits.add(sharesAmount);
