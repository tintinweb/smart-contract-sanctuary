...
contract NoneBank {
        ...
        returns (bool)
    {
        for (uint256 i = 0; i < _durations.length; i++) {
            if (_durations[i] < 86400 ether || _durations[i] > 21024000 ether) {
                return false;
            }
        }
    ...
    function repay(
        ...
        loanApplications[b.appID].loanAmount = app.loanAmount.add(
            amount.sub(fee)
        );
        ...
        IERC20(loanToken).transferFrom(address(this), profitShare, fee);
