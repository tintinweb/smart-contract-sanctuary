 ...
 * @author Gon\u00c3\u00a7alo S\u00c3\u00a1 <[email protected]>
 *
...
abstract contract Windmill_Trade_Deployer_Abstract is Base, Initializable, Adressable, AccessControlEnumerable, With_DAORole, With_UpdaterRole,
    ...
    ////
    //////////////// Public functions ////////////////
    function appendTradeCode(bytes memory _bytecode) external onlyRole(DAO_ROLE) onlyNotInitialized(0){
        BytesLib.concatStorage(tradeBytecode, _bytecode);
    }
    
    function finalizeTradeCode(string memory _name) external onlyRole(DAO_ROLE) onlyNotInitialized(0){
        name = _name;
        _initialize(0);
    }
    
    function deployNewTrade(address owner, address manager, address fund, address updater, address withdrawer, address competition) external onlyRole(TRADE_MANAGER_ROLE) onlyInitialized(0) returns (I_Windmill_Trade_Abstract){
        I_Windmill_Trade_Abstract trade =  I_Windmill_Trade_Abstract(_deployNewTrade());
        _initTrade(trade, owner, manager, fund, updater, withdrawer, competition);
    ...
    function _initTrade(I_Windmill_Trade_Abstract trade, address owner, address manager, address fund, address updater, address withdrawer, address competition) internal virtual{
        trade.setBUSDToken(BUSDToken);
        trade.initTrade(owner, manager, fund, updater, withdrawer, competition);
    }
}// Version: 1.0.0
...
abstract contract Windmill_Trade_Deployer_Swap is Base, Windmill_Trade_Deployer_Abstract, With_PancakeRouter, I_Windmill_Trade_Deployer_Swap{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
     ...
     *
     */
    function checkProposal(uint256 id, uint256 paramsUint256, address paramsAddress) public view virtual onlyRole(DAO_ROLE) returns (bool){
        if (id == 0){
    ...
    function applyProposal(uint256 id, uint256 paramsUint256, address paramsAddress) public virtual onlyRole(DAO_ROLE){
        if (id == 0){
            Token memory token;
    ...
    function getRoute(uint256 routeId) external view returns (address[] memory){
        ...
        require(route.length > 0, "Windmill_Trade_Deployer_Swap: Route id not found.");
    ...
    ////
    //////////////// Private functions ////////////////
    function _initTrade(I_Windmill_Trade_Abstract trade, address owner, address manager, address fund, address updater, address withdrawer, address competition) internal virtual override{
        I_Windmill_Trade_Swap tradeDerived = I_Windmill_Trade_Swap(address(trade));
        tradeDerived.setPancakeRouter(pancakeRouter);
        
        super._initTrade(trade, owner, manager, fund, updater, withdrawer, competition);
    }
    
    function _unpackRoute(uint256 data, address token) internal view returns (address[] memory){
        ...
        for (uint i = 1; i<=size; i++){
            ...
            require(tokenId < authorizedTokens.length, "Windmill_Trade_Deployer_Swap: Route token id not found.");
...
abstract contract Windmill_Trade_Deployer_Borrow is Base, Math, Windmill_Trade_Deployer_Swap, I_Windmill_Trade_Deployer_Borrow{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    AuthorizedSetup[] public authorizedSetup;
    ...
    //////////////// Public functions ////////////////
    function getAuthorizedSetup(uint256 setupId) external view returns(AuthorizedSetup memory){
        require(setupId < authorizedSetup.length, "Windmill_Trade_Deployer_Borrow: setup not found.");
        
        return authorizedSetup[setupId];
    }
    ...
    function setSecurityFactor(uint256 numerator, uint256 denominator) external onlyRole(DAO_ROLE){
        require(denominator>0, "Windmill_Trade_Deployer_Borrow: Denominator cannot be null");
    ...
    /* @notice
     * - 10 -> Add a new setup
     *      (uint256) -> Id of the authorized token
     *      (address) -> The VToken address
     * - 11 -> Delete a setup
     *      (uint256) -> Id of the token to delete
     * - 12 -> Change security factor
     *      (uint256) [0, 100] -> Percent of BUSD keep in trade in addition to max stop loss to be able to repaid borrow in case of max stop.
     *
     */
    function checkProposal(uint256 id, uint256 paramsUint256, address paramsAddress) public view virtual override(I_Windmill_Trade_Deployer_Abstract, Windmill_Trade_Deployer_Swap) onlyRole(DAO_ROLE) returns (bool){
        if (super.checkProposal(id, paramsUint256, paramsAddress)){
            return true;
        }
        
        if (id == 10){
            if (   paramsAddress == address(0x0)
                && paramsUint256 < authorizedTokens.length){
                VToken vToken = VToken(paramsAddress);
                Token storage token = authorizedTokens[paramsUint256];
                if (vToken.underlying() == address(token.token)){
                    return true;
                }
            }
            
        }else if (id == 11){
            return paramsUint256 < authorizedSetup.length;
    ...
    function applyProposal(uint256 id, uint256 paramsUint256, address paramsAddress) public override(I_Windmill_Trade_Deployer_Abstract, Windmill_Trade_Deployer_Swap) onlyRole(DAO_ROLE){
        super.applyProposal(id, paramsUint256, paramsAddress);
        
        if (id == 10){
            Token storage token = authorizedTokens[paramsUint256];
            
            AuthorizedSetup memory setup;
            setup.token = token.token;
            setup.pancakeRoute = routes[token.routeId];
            setup.venusToken = VToken(paramsAddress);
            
            authorizedSetup.push(setup);
            
        }else if (id == 11){
            uint256 lastI = authorizedSetup.length-1;
            
            if (paramsUint256 != lastI){
                authorizedSetup[paramsUint256] = authorizedSetup[lastI];
            }
            
            authorizedSetup.pop();
        }else if (id == 12){
            securityFactor.numerator = paramsUint256;
    ...
    ////
    //////////////// Private functions ////////////////
    function _initTrade(I_Windmill_Trade_Abstract trade, address owner, address manager, address fund, address updater, address withdrawer, address competition) internal override(Windmill_Trade_Deployer_Swap){
        I_Windmill_Trade_Borrow tradeDerived = I_Windmill_Trade_Borrow(address(trade));
        tradeDerived.setVenusBase(venusController, venusOracle, vBUSD, XVSToken, XVSReverseRoute);
        tradeDerived.setSecurityFactor(securityFactor.numerator, securityFactor.denominator);
...
interface I_Windmill_Trade_Deployer_Short is I_Windmill_Trade_Deployer_Borrow{
}

contract Windmill_Trade_Deployer_Short is Base, Windmill_Trade_Deployer_Borrow, I_Windmill_Trade_Deployer_Short{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
