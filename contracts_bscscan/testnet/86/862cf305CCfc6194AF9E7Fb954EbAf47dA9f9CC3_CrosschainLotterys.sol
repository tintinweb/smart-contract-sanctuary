...
/** @title PanPYESwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract CrosschainLotterys is ReentrancyGuard, ICrosschainLottery, Ownable {
    ...
    uint256 public maxPriceTicketInPYE = 50 ether;
    uint256 public minPriceTicketInPYE = 0.005 ether;

    uint256 public pendingInjectionNextLottery;

    uint256 public constant MIN_DISCOUNT_DIVISOR = 300;
    //uint256 public constant MIN_LENGTH_LOTTERY = 4 minutes; // 4 hours
   ...
   uint256 public constant MIN_LENGTH_LOTTERY = 120; // 4 hours
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInPYE;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] PYEPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInPYE;
        uint32 finalNumber;
    }
     ...
     * @param _PYETokenAddress: address of the PYE token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _PYETokenAddress, address _randomGeneratorAddress) {
        PYEToken = IERC20(_PYETokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of PYE to this contract
        uint256 amountPYEToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInPYE,
            _ticketNumbers.length
        );

        // Transfer PYE tokens to this contract
        PYEToken.safeTransferFrom(address(msg.sender), address(this), amountPYEToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInPYE += amountPYEToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInPYEToTransfer
        uint256 rewardInPYEToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInPYEToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        PYEToken.safeTransfer(msg.sender, rewardInPYEToTransfer);

        emit TicketsClaim(msg.sender, rewardInPYEToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInPYE) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in PYE for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].PYEPerBracket[j] =
                ...
                // A. No PYE to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].PYEPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInPYE - amountToShareToWinners);

        // Transfer PYE to treasury address
        PYEToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        PYEToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInPYE += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInPYE >= minPriceTicketInPYE) && (_priceTicketInPYE <= maxPriceTicketInPYE),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInPYE: _priceTicketInPYE,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            PYEPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInPYE: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(PYEToken), "Cannot be PYE token");
    ...
    /**
     * @notice Set PYE price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInPYE: minimum price of a ticket in PYE
     * @param _maxPriceTicketInPYE: maximum price of a ticket in PYE
     */
    function setMinAndMaxTicketPriceInPYE(uint256 _minPriceTicketInPYE, uint256 _maxPriceTicketInPYE)
        external
        onlyOwner
    {
        require(_minPriceTicketInPYE <= _maxPriceTicketInPYE, "minPrice must be < maxPrice");

        minPriceTicketInPYE = _minPriceTicketInPYE;
        maxPriceTicketInPYE = _maxPriceTicketInPYE;
    }
     ...
     * @param _priceTicket price of a ticket (in PYE)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].PYEPerBracket[_bracket];
        } else {
            return 0;
        }
    }
