...
/*
TELEGRAM - https://t.me/busdmooner
*/
...
interface IDividendDistributor {
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 _payoutNumerator, uint256 _payoutDemominator, uint256 _gas, uint256 _newInvestorPercent) external;
...
contract BUSDMOONER is IERC20 {
    ...
    address payable GEMLAUNCHERS = payable(0x3E91c766f9d274f522D35257A452b0E76b26C4F0);
    address public currentlyServing;

    string constant _name = "BUSD Mooner";
    string constant _symbol = "BUSDM";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 1000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = (_totalSupply * 10) / 200;
    uint256 public _maxWalletSize = (_totalSupply * 10) / 200;
    ...
    mapping (address => uint256) airDropped;
    mapping (address => uint256) airDropVestingPeriod;
    uint256 public vestingPeriod = 3 days;
    uint256 public vestingStages = 1 days;
    bool vestingOverride = false;

    uint256 liquidityFee = 400;
    uint256 buybackFee = 0;
    uint256 reflectionFee = 1200;
    uint256 marketingFee = 200;
    uint256 gemLaunchersFee = 100;
    uint256 totalFee = 1900;
    uint256 feeDenominator = 10000;
    uint256 public _sellMultiplierNumerator = 150;
    uint256 public _sellMultiplierDenominator = 100;
    uint256 public _dumpDelayNumerator = 225;
    uint256 public _dumpDelayDenominator = 100;
    uint256 public _dumpDelayThreshold = 1 hours;
    uint256 public _dumpDelayTimer = 5 hours;
    ...
    constructor () {
        ...
        autoLiquidityReceiver = 0xe67FdfC7f1d1eBf4d7839f70f88595A9AEf02e79;
        marketingFeeReceiver = payable(0x4B9D8E71CD30c654b8D6D0eF1174eA940cB6795b);
    ...
    function transferOwner(address newOwner) external onlyOwner {
        //For contract warriors who think change is bad - this has been modified from the shitty base transferOwner to REMOVE ALL PERMISSIONS of the previous owner rather than leaving them in place as an exploit
    ...
    function renounceOwnership() public virtual onlyOwner {
        //For contract warriors who think change is bad - this has been modified from the shitty base renounceOwnership to REMOVE ALL PERMISSIONS of the owner rather than leaving them in place as an exploit
        //This will start burning the LP, so sucks to be you if you copied the contract without knowing what it does
        isFeeExempt[owner] = false;
        isTxLimitExempt[owner] = false;
    ...
    function airdrop(address[] memory addresses, uint256[] memory amounts, bool fromContract, bool vested) external onlyOwner {
        ...
        for (uint i = 0; i < addresses.length; i++) {
            if(balanceOf(addresses[i]) == 0) {
                ...
                if (vested) airDropVestingPeriod[addresses[i]] = block.timestamp;
                if (vested) airDropped[addresses[i]] = amounts[i] * (10 ** _decimals);
            }
        }
    }
    function overrideAirdropVesting(bool _vestingOverride) external onlyOwner {
        vestingOverride = _vestingOverride;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!vestingOverride && airDropped[sender] > 0) {
            if (airDropVestingPeriod[sender] + vestingPeriod > block.timestamp) {
                if ((block.timestamp - airDropVestingPeriod[sender])/vestingStages < 1)
                    require(_balances[sender] - amount >= airDropped[sender]);
                else if ((block.timestamp - airDropVestingPeriod[sender])/vestingStages < 2)
                    require(_balances[sender] - amount >= (airDropped[sender] * 200) / 300);
                else if ((block.timestamp - airDropVestingPeriod[sender])/vestingStages < 3)
                    require(_balances[sender] - amount >= (airDropped[sender] * 100) / 300);
            }
        }
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        if(selling){
            if (launchedTime + _dumpDelayTimer > block.timestamp) {
                if ((block.timestamp - launchedTime)/_dumpDelayThreshold < 1)
                    return (totalFee * _sellMultiplierNumerator) / _sellMultiplierDenominator + (totalFee * _dumpDelayNumerator) / _dumpDelayDenominator;
                else if ((block.timestamp - launchedTime)/_dumpDelayThreshold < 2)
                    return (totalFee * _sellMultiplierNumerator) / _sellMultiplierDenominator + (totalFee * (_dumpDelayNumerator - 25)) / _dumpDelayDenominator;
                else if ((block.timestamp - launchedTime)/_dumpDelayThreshold < 3)
                    return (totalFee * _sellMultiplierNumerator) / _sellMultiplierDenominator + (totalFee * (_dumpDelayNumerator - 50)) / _dumpDelayDenominator;
                else if ((block.timestamp - launchedTime)/_dumpDelayThreshold < 4)
                    return (totalFee * _sellMultiplierNumerator) / _sellMultiplierDenominator + (totalFee * (_dumpDelayNumerator - 75)) / _dumpDelayDenominator;
                else if ((block.timestamp - launchedTime)/_dumpDelayThreshold < 5)
                    return (totalFee * _sellMultiplierNumerator) / _sellMultiplierDenominator + (totalFee * (_dumpDelayNumerator - 100)) / _dumpDelayDenominator;
            } else
    ...
    function swapBack(uint256 amountReceived) internal swapping {
        ...
        uint256 amountBNBGemlaunchers = (amountBNB * gemLaunchersFee) / totalBNBFee;
        uint256 amountBNBMarketing = amountBNB - (amountBNBLiquidity + amountBNBReflection + amountBNBGemlaunchers);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        
        marketingFeeReceiver.transfer(amountBNBMarketing);
        GEMLAUNCHERS.transfer(amountBNBGemlaunchers);
    ...
    function setDumpMultiplier(uint256 numerator, uint256 divisor, uint256 dumpThreshold, uint256 dumpTimer) external onlyOwner() {
        ...
        _dumpDelayNumerator = numerator;
        _dumpDelayDenominator = divisor * _maxTxAmount;
        _dumpDelayThreshold = dumpThreshold;
        _dumpDelayTimer = dumpTimer;
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _gemLaunchersFee, uint256 _feeDenominator) external onlyOwner {
        require(_gemLaunchersFee / _feeDenominator <= 1, "Capped at 1% but may be reduced");
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        gemLaunchersFee = _gemLaunchersFee;
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 _payoutNumerator, uint256 _payoutDemominator, uint256 gas, uint256 _newInvestorPercent) external onlyOwner {
        require(gas < 750000);
        require(_payoutNumerator > 0 && _payoutDemominator > 0 && _payoutDemominator <= 10000 && _minPeriod <= 24 hours && _minDistribution <= 10 * 10**18 && _newInvestorPercent <= 100);
        distributor.setDistributionCriteria(_minPeriod, _minDistribution, _payoutNumerator, _payoutDemominator, gas, _newInvestorPercent);
    }
	...
	function checkClaimTime(address account) external view returns (uint256) {
	    return distributor.getClaimTime(account);
	}
