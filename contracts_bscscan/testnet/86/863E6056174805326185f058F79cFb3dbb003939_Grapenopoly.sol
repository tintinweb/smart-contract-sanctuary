...
contract Grapenopoly
{
    ...
    function buyPropertyFromBank(uint256 _tokenId, address selectedTokenToPay) external
    {
        ...
        require(vaultBalances[msg.sender][selectedTokenToPay] >= SafeMath.safeMulFloat(nftPrice[_tokenId], getQuote(usdToken, selectedTokenToPay), ERC20(usdToken).decimals()), "NB"); //No balance for purchase
        ...
        if(address(selectedTokenToPay) != networkcoinaddress)
        {
            ...
            ERC20(selectedTokenToPay).transfer(feeandtaxreceiver, SafeMath.safeMulFloat(nftPrice[_tokenId], getQuote(usdToken, selectedTokenToPay), ERC20(usdToken).decimals()));
        }
        else
        {
            //Send Network Coin to Fee and Tax Receiver Address
            payable(feeandtaxreceiver).transfer(SafeMath.safeMulFloat(nftPrice[_tokenId], getQuote(usdToken, selectedTokenToPay), ERC20(usdToken).decimals()));
        }

        //Subtract amount of user balance
        vaultBalances[msg.sender][selectedTokenToPay] = SafeMath.safeSub(vaultBalances[msg.sender][selectedTokenToPay], SafeMath.safeMulFloat(nftPrice[_tokenId], getQuote(usdToken, selectedTokenToPay), ERC20(usdToken).decimals()) );
    ...
    function buyPropertyForSale(uint256 _tokenId, address selectedTokenToPay) external
    {
        ...
        require(vaultBalances[msg.sender][selectedTokenToPay] >= SafeMath.safeMulFloat(propertySalePrice[ _tokenId ], getQuote(usdToken, selectedTokenToPay), ERC20(usdToken).decimals()), "NB"); //No balance for purchase
        require(playerPosition[msg.sender] == _tokenId, 'ip'); //Invalid player position

        //*** Pay to Owner ***
        //Subtract amount of user balance
        vaultBalances[msg.sender][selectedTokenToPay] = SafeMath.safeSub(vaultBalances[msg.sender][selectedTokenToPay], SafeMath.safeMulFloat(propertySalePrice[ _tokenId ], getQuote(usdToken, selectedTokenToPay), ERC20(usdToken).decimals()) );

        //Add amount to NFT owner
        vaultBalances[ nftOwners[ _tokenId ] ][selectedTokenToPay] = SafeMath.safeAdd(vaultBalances[ nftOwners[ _tokenId ] ][selectedTokenToPay], SafeMath.safeMulFloat(propertySalePrice[ _tokenId ], getQuote(usdToken, selectedTokenToPay), ERC20(usdToken).decimals()));
    ...
    function buyHouseOrHotelForProperty(uint256 _tokenId, address selectedTokenToPayBuild) external
    {
        ...
        require(nftHouseAmount[ _tokenId ] < SafeMath.safeAdd(maxHouseAmount, 1), 'mx'); //Maximum builds reached
        require(nftOwners[ _tokenId ] == msg.sender, 'own'); //You don't own this property
        require(SafeMath.safeMulFloat(vaultBalances[msg.sender][selectedTokenToPayBuild], getQuote(usdToken, selectedTokenToPayBuild), ERC20(usdToken).decimals()) >= nftHousePrice[_tokenId][ SafeMath.safeAdd(nftHouseAmount[ _tokenId ], 1) ], "NB"); //No balance to build

        //Pay for the house or hotel
        vaultBalances[msg.sender][selectedTokenToPayBuild] = SafeMath.safeSub(vaultBalances[msg.sender][selectedTokenToPayBuild], nftHousePrice[_tokenId][ SafeMath.safeAdd(nftHouseAmount[ _tokenId ], 1) ]);

        //Update build index
        nftHouseAmount[ _tokenId ] = SafeMath.safeAdd(nftHouseAmount[ _tokenId ], 1);
    ...
    function sellHouseOrHotelProperty(uint256 _tokenId) external
    {
        ...
        vaultBalances[msg.sender][tokenToUseOnSellPropertyToBank] = SafeMath.safeAdd( vaultBalances[msg.sender][tokenToUseOnSellPropertyToBank], nftHousePrice[_tokenId][ nftHouseAmount[ _tokenId ] ]);

        //Update build index
        nftHouseAmount[ _tokenId ] = SafeMath.safeSub(nftHouseAmount[ _tokenId ], 1);
    ...
    function rollDices(address selectedTokenToPayRent) external
    {
        ...
        //require(id > SafeMath.safeMul(totalDices, diceFaces), "LP" ); //Low properties for dices rolling
        ...
        //result.faces = new uint[](totalDices);
        
        //uint walkSteps = 0;
        //result.steps = 0; //Dont need to init with zero

        // for(uint ix = 0; ix < totalDices; ix++)
        // {
        //     result.faces[ix] = SafeMath.safeAdd(Hlp.clockN(SafeMath.safeMul(diceNonce, 3), diceFaces), 1);
        //     if(SafeMath.safeAdd(diceNonce, 1) >= MAX_NONCE)
        //     {
        //         diceNonce = 0;
        //     }
        //     diceNonce++;
        //     result.steps = SafeMath.safeAdd(result.steps, result.faces[ix]);
        // }

        result.faces[0] = SafeMath.safeAdd(Hlp.clockN(SafeMath.safeMul(diceNonce, 3), diceFaces), 1);
        diceNonce++;
        result.faces[1] = SafeMath.safeAdd(Hlp.clockN(SafeMath.safeMul(diceNonce, 3), diceFaces), 1);
        diceNonce++;
        result.faces[2] = SafeMath.safeAdd(Hlp.clockN(SafeMath.safeMul(diceNonce, 3), diceFaces), 1);
        diceNonce++;
        ...
        */
        ...
        if(prizeWalkAmount > 0 && playerAccumulatedStepsFromLastPrize[msg.sender] >= prizeWalkAmount)
        {
            ...
            //Collect prize
            //uint256 prizeValue = uint256(keccak256(abi.encodePacked(SafeMath.safeAdd(block.timestamp, SafeMath.safeMul(prizeNonce, 7)), block.difficulty, msg.sender))) % prizeLandMaxPay;
        ...
        if(taxesWalkAmount > 0 && playerAccumulatedStepsFromLastTax[msg.sender] >= taxesWalkAmount)
        {
            ...
            //Pay tax
            //uint256 taxValue = uint256(keccak256(abi.encodePacked(SafeMath.safeAdd(block.timestamp, SafeMath.safeMul(prizeNonce, 5)), block.difficulty, msg.sender))) % taxesLandMaxPay;
    ...
    function getDiceFace() internal returns(uint)
    {
        ...
        //     SafeMath.safeAdd(block.timestamp, SafeMath.safeMul(diceNonce, 3) ), 
        //     block.difficulty, 
        //     msg.sender)
        // )) % diceFaces;

        uint vFace = Hlp.clockN(SafeMath.safeMul(diceNonce, 3), diceFaces);

        vFace = SafeMath.safeAdd(vFace, 1);
    ...
    */
    ...
    function mint(string calldata _nftName, string calldata _nftDescription, string calldata _nftImageURI, uint256 _nftPrice, uint _nftZone) external returns (uint256 tokenId)
    {
        ...
        //propertySalePrice[ id ] = 0; //Starting not for SALE //Dont need to init with zero
