...
contract DoughTokenCloneable is Initializable, ERC20Upgradeable, OwnableUpgradeable, VersionedUpgradeable, IDoughTokenCloneable  {
    ...
    function setFees (uint8 _liquidityFee, uint8 _marketingFee, uint8 _burnFee, uint8 _feeType) internal returns (uint256 totalFees) {
        ...
        //totalFeePerMille[_feeType] = uint16(totalFees * 10 + feeInfo[_feeType].platformFeePerMille);
    }
    ...
    function checkTransferRequirementsAndSwap (address from, address to, uint256 amount) internal returns (uint256 transferAmount, uint8 feeType) {
        ...
        feeType = automatedMarketMakerPairs[to] ? SELL : BUY;
...
contract DoughRewardCloneable is DoughTokenCloneable  {
    ...
    string private constant MIN_BALANCE_ISSUE = "Must be <=0.001% supply";
    ...
    function __DoughToken_init (SharedStructures.TokenInfo memory _tokenInfo) internal override {
        ...
        for (uint256 i; i < length; i++) {
            ...
            if (rewardToken == address(1)) {
                ...
                if (failed)
                    revert (createErrorMessage ("Reward needs BNB pair"));
            }
            ...
            if (rewardToken == address(this)) {
                ...
                require (nativeRewardTracker == address(0), createErrorMessage ("Too many native rewards"));
        ...
        if (!swapping) {
            ...
            for (uint256 i; i < length; i++) {
                try IDividendTrackerCloneable(otherFeeInfo[i].feeRecipient).process (gasPerTracker) returns (uint256 _iterations, uint256 _claims, uint256 _lastProcessedIndex) {
                    emit ProcessedDividendTracker (_iterations, _claims, _lastProcessedIndex, true, gasPerTracker, msg.sender);
                } catch {}
            }
        }
    }
