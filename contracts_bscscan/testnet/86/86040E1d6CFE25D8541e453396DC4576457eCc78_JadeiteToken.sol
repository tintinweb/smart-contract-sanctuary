...
contract JadeiteToken is Context, IERC20, Ownable {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        _takeBurnFee(sender,recipient,burnAmt);
        
        if (!takeFee) restoreAllFee();

    }
    
    function _takeBurnFee(address sender, address recipient, uint256 burnAmt) private {
        if(!_isExcludedFromFee[sender] && !_isExcluded[recipient] && _burnFee !=0 && balanceOf(address(0xdead)) < 9000 * 10**6 * 10**9) {
            emit Transfer(sender, address(0xdead), burnAmt);
        }        
    }  
