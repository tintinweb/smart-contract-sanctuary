...
contract EpicAuction is AccessControl {
    ...
    function winnerAcceptBid(uint256 marketId) public returns (bool, uint256) {
        ...
        require(items[marketId]._available == false, "Auction is active, You can't accept this bid");
        (, Item memory itemData) = _getItemInfo(marketId);
        // tranfer NFT
        if(itemData._itemType == TokenType.ERC1155){
    ...
    function getMarketId(address item, address owner, uint256 tokenId, uint256 amount, bool isAvailable, STATUS_TYPE status) public view returns(bool, uint256){
