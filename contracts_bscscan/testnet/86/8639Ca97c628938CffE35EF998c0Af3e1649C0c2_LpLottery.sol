...
contract Lottery {
    ...
    mapping(uint256 => address) public allParticipantsByLotteryId;
    mapping(address => bool) public allParticipants;
    ...
    function incrementUnstakingCount(uint256 delta) external {
        require(
            ...
            "The caller is not participant for this lottery"
        );
...
contract LpLottery is PancakeClass {
    ...
    function createContext(uint256 _prizeValue, uint256 _minimumAmountBusd)
        public
        restricted
    {
    ...
    function getMinimumAmountInSafemars(uint256 _lotteryId)
        external
        view
        returns (uint256 amountOut)
    {
        uint256 amountInBusd = lotteryStructs[_lotteryId]
            .getMinimumAmountInBusd();
        uint256 amountOutInSafemars = getEquivalentAmountInSafemarsForBusd(
            amountInBusd
        );
        return amountOutInSafemars;
    }
...
contract PancakeClass is ReentrancyGuard {
    ...
    function unstakeAllToSafeMars(address account) internal {
        ...
        (uint256 _amountSafeMars, uint256 _amountBUSD) = _removeLiquidity(
            usrBal.balanceLP
        );
    ...
    function convertBUSDToSafeMars(uint256 _amountIn)
        internal
        returns (uint256 amountOut)
    {
    ...
    function convertSafeMarsToBUSD(uint256 _amountIn)
        internal
        returns (uint256 amountOut)
    {
    ...
    function _removeLiquidity(uint256 _liquidity)
        private
        returns (uint256 _amountSafeMars, uint256 _amountBUSD)
    {
    ...
    function _sellXGetY(
        ...
        uint256 _amountXIn
    ) private returns (uint256 _amountYOut) {
    ...
    function _getPairAddress() private view returns (address) {
        ...
        IPancakePair pair = IPancakePair(
            factory.getPair(tokenSafeMars, tokenBUSD)
        );
        return address(pair);
    }
    ...
    function getEquivalentAmountInSafemarsForBusd(uint256 _amountIn)
        internal
        view
        returns (uint256)
    {
        ...
        t[0] = tokenBUSD;
        t[1] = tokenSafeMars;
        uint256[] memory amountsOut = router.getAmountsOut(_amountIn, t);
        return amountsOut[1];
    }
}
