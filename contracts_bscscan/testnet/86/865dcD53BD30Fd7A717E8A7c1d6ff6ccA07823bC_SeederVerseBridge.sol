...
contract SeederVerseBridge is OwnableUpgradeable, ERC721Holder, BridgeOracleClient {
    ...
    mapping(uint256 => bytes32) private seedmonIdToName;

    event SeedmonsDeposited(uint256[] tokenIds, bytes32[] names, address depositor);
    ...
    function emergencyWithdrawSeedmonForTest(uint256 tokenId) external {
        ...
        delete seedmonIdToName[tokenId];
    }

    //TODO Remove
    function setSeedTimelock(uint256 _timelock) external {
    ...
    function depositedSeedmon(address depositer) external view returns (uint256[] memory tokenIds, bytes32[] memory names) {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            names[i] = seedmonIdToName[tokenIds[i]];
        }
    ...
    function depositSeedmons(uint256[] memory tokenIds) external {
        bytes32[] memory names = new bytes32[](tokenIds.length);
        for (uint i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            seedmon.safeTransferFrom(msg.sender, address(this), tokenId);
            tokenIdToDepositor[tokenId] = msg.sender;
            names[i] = seedmon.tokenIdToName(tokenId);
            seedmonIdToName[tokenId] = names[i];
            addDepositedSeedmon(msg.sender, tokenId);
        }
        emit SeedmonsDeposited(tokenIds, names, msg.sender);
    }
