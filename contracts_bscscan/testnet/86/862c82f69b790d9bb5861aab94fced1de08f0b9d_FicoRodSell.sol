...
contract FicoRod is IFicoRod, ERC721, Ownable {
    ...
    constructor() ERC721("FicoRod", "FicoRod") {
        _operators[msg.sender] = true;
        setBaseUri("https://fishcrypto.io/");
    }
...
contract FicoRodSell is ReentrancyGuard, Runnable {
    address public _recepientAddress;

    IFicoRod public _ficoRodNft;

    uint256 public _startTime;
    uint256 public _price;
    uint256 public _rodToSell;
    uint256 public _purchasedBox;

    uint256 public _rodsPerUser;

    mapping(address => uint256) public _userBoxCounts;

    IERC20 public _ficoToken;

    constructor(
        address rodNFTAddress,
        address recepientAddress,
        address ficoTokenAddress,
        uint256 rodToSell, // 20000
        uint256 startTime, // 1637676000
        uint256 price, // 2 00000000000000000000
        uint256 boxPerUser // 5
    ) {
        require(rodNFTAddress != address(0), "rodNFTAddress is zero address");
        require(
            recepientAddress != address(0),
            "recepientAddress is zero address"
        );
        require(
        ...
        _ficoRodNft = IFicoRod(rodNFTAddress);
        _recepientAddress = recepientAddress;
        _ficoToken = IERC20(ficoTokenAddress);

        _rodToSell = rodToSell;
        _startTime = startTime;
        _price = price;

        _rodsPerUser = boxPerUser;
    }
    ...
    function purchase(uint256 quantity) external nonReentrant whenRunning {
        ...
        require(_purchasedBox < _rodToSell, "Sold out");
        require(
            _userBoxCounts[msg.sender] + quantity <= _rodsPerUser,
            "Reach limited box"
        );
        ...
        for (uint256 index = 0; index < quantity; index++) {
            //Mint NFT
            uint256 mysteryBoxTokenId = _ficoRodNft.mint(msg.sender);
        ...
        //Send fund to wallet
        //Transfer in end to prevent Reentrancy attack
        require(
            _ficoToken.transferFrom(msg.sender, _recepientAddress, totalPrice),
            "Can not transfer BUSD"
        );
    }

    function setFicoRodNftAddress(address newAddress) external onlyOwner {
        require(newAddress != address(0), "Address 0");
        _ficoRodNft = IFicoRod(newAddress);
    }

    function setFicoToken(address newAddress) external onlyOwner {
        require(newAddress != address(0), "Address 0");
        _ficoToken = IERC20(newAddress);
    }
    ...
    function setRodToSale(uint256 rodToSell) external onlyOwner {
        require(rodToSell > 0, "rodToSell is 0");
        _rodToSell = rodToSell;
    }
    ...
    function setRecepientTokenAddress(address recepientAddress)
        external
        onlyOwner
    {
    ...
    event SaleInfoSeted(uint256 startTime, uint256 price, uint256 rodToSell);
    event Purchased(
        address account,
        uint256 tokenId,
        uint256 price,
...
contract FicoMysteryBoxSell is ReentrancyGuard, Runnable {
    address public _recepientAddress;

    IFicoMysteryBox public _mysteryBoxNft;
    ...
    uint256 public _ficoHoldAmount;
    ...
    constructor(
        ...
        uint256 boxToSell, // 20000
        uint256 startTime, // 1637676000
        uint256 price, // 200000000000000000000
        uint256 boxPerUser, // 5
        uint256 ficoHoldAmount // 500000000000000000000
    ) {
        require(
        ...
        require(
            busdTokenAddress != address(0),
            "busdTokenAddress is zero address"
        );

        _mysteryBoxNft = IFicoMysteryBox(mysteryBoxNFTAddress);
        _recepientAddress = recepientAddress;
        ...
        _ficoHoldAmount = ficoHoldAmount;
    }
    ...
    function purchase(uint256 quantity) external nonReentrant whenRunning {
        ...
        require(
            _ficoToken.balanceOf(msg.sender) >= _ficoHoldAmount,
            "Hoding FICO is not enough"
        );
        ...
        for (uint256 index = 0; index < quantity; index++) {
            //Mint NFT
            uint256 mysteryBoxTokenId = _mysteryBoxNft.mint(msg.sender);
        ...
        //Transfer in end to prevent Reentrancy attack
        require(
            _busdToken.transferFrom(msg.sender, _recepientAddress, totalPrice),
            "Can not transfer BUSD"
        );
    }
    ...
    function setMysteryBoxNFT(address newAddress) external onlyOwner {
        require(newAddress != address(0), "Address 0");
        _mysteryBoxNft = IFicoMysteryBox(newAddress);
    }
...
contract FicoMysteryBox is IFicoMysteryBox, ERC721, Ownable {
    uint256 public totalSupply = 0;
    ...
    constructor() ERC721("FicoMysteryBox", "FicoMysteryBox") {
        _operators[msg.sender] = true;
        setBaseUri("https://fishcrypto.io/");
    }
