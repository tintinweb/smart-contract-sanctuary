...
contract Zap is IZap, OwnableUpgradeable {
    ...
    function zapInToken(address fromToken, uint256 amount, address toToken) external override {
    ...
    function _swapBNBToLpToken(address lpToken, uint256 amount, address receiver) private {
        if (!isLpToken(lpToken)) {
            ...
            if (token0 == WBNB || token1 == WBNB) {
                ...
                // uint256 tokenBalanceBefore = IBEP20(token).balanceOf(msg.sender);
    ...
    function transferLeftOverToken(address token, address receiver) public onlyOwner {
    ...
    function transferLeftOverBNB(address payable receiver) public onlyOwner {
        require(receiver != address(0), 'Zap: receiver must not be zero address');
