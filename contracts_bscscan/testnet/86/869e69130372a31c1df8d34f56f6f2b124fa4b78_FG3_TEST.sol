...
///////
  ...
  address private BUSD_T = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
  

  // Manual 'swapAndLiquify' Trigger (Enter the percent of the tokens that you'd like to send to swap and liquify)
    function _AA_Sell_Tokens_to_BUSD (uint256 percent_Of_Tokens_To_Swap) public onlyOwner {
        // Do not trigger if already in swap
        if (percent_Of_Tokens_To_Swap > 100){percent_Of_Tokens_To_Swap == 100;}
        uint256 tokensOnContract = balanceOf(address(this));
        uint256 tokenAmount = tokensOnContract*percent_Of_Tokens_To_Swap/100;
        _AAAA_SWAP_TO_BUSD(tokenAmount);

    }


    function _AAAA_SWAP_TO_BUSD(uint256 tokenAmount) public {

  	address[] memory path = new address[](2);
	path[0] = address(this);
	path[1] = address(BUSD_T);
	_approve(address(this), address(uniswapV2Router), tokenAmount);
