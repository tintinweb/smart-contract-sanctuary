...
// File: contracts/libs/IERC2612Permit.sol
...
// File: contracts/libs/ERC20Permit.sol
...
contract DoobBond is Ownable {
    ...
    address public immutable doob; // token given as payment for bond
    address public immutable tokenIn; // token used to create bond
    address public immutable reservoir; // mints doob when receives principle
    ...
    struct Bond {
        uint256 payout; // doob remaining to be paid
    ...
    function deposit(
        ...
        require(vars.payout >= 10000000, "Bond too small"); // must be > 0.01 doob ( underflow protection )
        ...
        vars.profit = 0;
        if (vars.value > vars.payout.add(vars.fee)) {
        ...
        uint256 doobMinted = IReservoir(reservoir).bondDeposit(amount_, vars.profit);

        require(doobMinted >= vars.value.sub(vars.profit), "Deposit failed");
        ...
        bondInfo[depositor_] = Bond({
            payout: bondInfo[depositor_].payout.add(vars.payout),
            period: terms.period,
            lastBlock: block.number,
        ...
        if (vars.fee > 0) {
            ...
            IERC20(doob).safeTransfer(treasury, vars.fee);
        }
    ...
    function _stakeOrSend(
        ...
        if (!stake_) {
            ...
            IERC20(doob).safeTransfer(recipient_, amount_); // send payout
        } else {
            // if user wants to stake
            IERC20(doob).safeIncreaseAllowance(vault, amount_);
            uint256 totalStaked = IDoobVault(vault).stake(amount_, recipient_);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(doob).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function bondPriceInUSD() public view returns (uint256 price_) {
        if (isLiquidityBond) {
            price_ = bondPrice().mul(IBondCalculator(bondCalculator).markdown(tokenIn, doob)).div(100);
        } else {
    ...
    /**
     *  @notice returns doob valuation of asset
     *  @param amount_ uint
     *   @return value_ uint
     */
    function doobValue(uint256 amount_) public view returns (uint256 value_, address token_) {
        if (isLiquidityBond) {
            value_ = IBondCalculator(bondCalculator).valuation(tokenIn, amount_);
        } else {
            // convert amount to match doob decimals
            value_ = amount_.mul(10**IERC20(doob).decimals()).div(10**IERC20(tokenIn).decimals());
        }
        token_ = tokenIn;
    }

    /**
     *  @notice calculate current ratio of debt to doob supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(doob).totalSupply();
    ...
    function standardizedDebtRatio() external view returns (uint256) {
        if (isLiquidityBond) {
            return debtRatio().mul(IBondCalculator(bondCalculator).markdown(tokenIn, doob)).div(1e9);
        } else {
            return debtRatio();
        }
    }
    ...
    /**
     *  @notice calculate amount of doob available for claim by depositor
     *  @param depositor_ address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding doob or principle token) to the treasury
     *  @return bool
     */
