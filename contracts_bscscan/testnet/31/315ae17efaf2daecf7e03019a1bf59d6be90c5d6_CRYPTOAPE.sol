...
contract CRYPTOAPE is Context, IERC20, Ownable {
    ...
    constructor () {
        _uniswapV2Router = IUniswapV2Router(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    ...
    function feeFor(uint256 amount) internal view returns (uint256) {
        return amount.mul(_feeTotal).div(100);
    }
    ...
    function swapToETH(uint256 amount) internal returns (uint256) {
        ...
        return address(this).balance.sub(oldBalance);
    }
    /** 
    ...
    function distribute(address from, uint256 amount) private returns (uint256 _totalFees) {
        ...
        /// @dev Get fee amounts
        uint256 toBurn = feesInContract.mul(_burnFee).div(100);
        uint256 _toLP = feesInContract.mul(_liquidityFee).div(100);
        uint256 toFundingWallet = feesInContract.mul(_fundingWalletFee).div(100);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256) {
        ...
        uint256 total = tAmount.add(tAmount.mul(_liquidityFee + _burnFee + _fundingWalletFee).div(90));

        /// @dev If trading isn't open don't take any reflection fees and don't count anything from total
        if (!_tradingOpen)
            total = tAmount;
        uint256 tFee = total.mul(_reflectionFee).div(100);
     ...
     */
    function changeValues(uint256 newFeeTotal, uint256 newBurnFee, uint256 newReflectionFee, uint256 newLiquidityFee, uint256 newFundingWalletFee, uint256 newMinimumToDistribute) external onlyOwner {
    ...
    function fees() external view returns (uint256, uint256, uint256, uint256, uint256)  {
        return (_feeTotal, _burnFee, _reflectionFee, _liquidityFee, _fundingWalletFee);
    }
