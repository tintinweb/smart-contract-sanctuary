...
contract CORSACAuction is IERC721Receiver,IERC1155Receiver {
    ...
    function createAuction( address _contractNFT, uint256 _tokenId, uint256 _price, uint256 _deadline, bool _isERC1155, address _sender, bool _isUSD ) public {
        require(!_contractsTokenIdsList[_contractNFT].contains(uint256(_sender).add(_tokenId)), "CORSAC: auction is already created");
        require(IERC20(_CSCT).balanceOf(_sender) >= (10 ** uint256(9)), "CORSAC: you must have 1 CSCT on account to start");
        if (_isERC1155) {
        ...
        IERC20(_CSCT).transferFrom(msg.sender, address(this), 10 ** uint256(9));
    ...
    function bid( address _contractNFT,uint256 _tokenId, uint256 _price, bool _isERC1155, address _sender ) public returns (bool, uint256, address, bool) {
        ...
        require(IERC20(_CSCT).balanceOf(_sender) >= _price, "CORSAC: you have not enough CORSAC");
    ...
    function cancelAuction( address _contractNFT, uint256 _tokenId, address _sender , bool _isERC1155 ) public {
        ...
        require(_contractsTokenIdsList[_contractNFT].contains(uint256(_sender).add(_tokenId)), "CORSAC: auction is not created");
