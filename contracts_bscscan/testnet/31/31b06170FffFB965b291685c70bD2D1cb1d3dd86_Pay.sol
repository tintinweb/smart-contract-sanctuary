...
contract Ownable is Context {
 ...
 interface tokenInterface
 {
    ...
    function idoParticipant(address _to) external returns (bool);
    function balanceOf(address _to) external returns (uint256);
    function includeIdoParticipant(address _add) external returns(bool);
    function taxActivate() external  returns(bool) ;
...
//------------------------------------------END---------------------------------------------------------
    ...
    function openLockedToken() public onlyOwner returns(bool){
        ...
        tokenInterface(tokenAddress).transfer(_lockedWallet,_lockedTokenAllot);
    ...
    function buyToken() payable public returns(uint)
    {
        ...
        require(tokenInterface(tokenAddress).balanceOf(msg.sender).add(tokenTotal)<=maximumTokenHold,"wallet reach maximum holdings");
        ...
        if(tokenInterface(tokenAddress).idoParticipant(msg.sender)==false){
            
            tokenInterface(tokenAddress).includeIdoParticipant(msg.sender);
        }
    ...
    {
        idoClosed = true;
        tokenInterface(tokenAddress).taxActivate();
