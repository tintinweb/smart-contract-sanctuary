...
interface IPokemonNFT is IERC721 {
...
contract  PokeFarmingBank is IERC721Locker, OwnableUpgradeable, ReentrancyGuard, ContractGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of PokeCashs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accPokecashPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPokeCashPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. PokeCashs to distribute per block.
        uint256 lastRewardTime; // Last timestamp that PokeCashs distribution occurs.
        uint256 accPokecashPerShare; // Accumulated PokeCashs per share, times 1e18. See below.
    ...
    address public pokecash; // PokeCash
    address public nft; // Pokemon
    ...
    // The block number when PokeCash mining starts.
    ...
    //   =============================================
    //   > LP Incentive (to Farm):  19,800,000 (41.66666%)  2 year
    //   > Dev + MKT:               1,880,000 (25%) 2 year
    //   > In-game Treasury:        15,840,000 (33.33333%)
    //   > Insurance Fund:          0  (0)
    ...
    function initialize(
        ...
        devRate = 2647; // 26.47%
        gameTreasuryRate = 2941; // 29.41%
        insuranceRate = 0; // 0%

        totalRewardPerSecond = _totalRewardPerSecond; // 0.278224 PokeCash/seconds
        _updateRewardPerSecond();
    ...
    function replacePoolLpToken(uint256 _pid, IERC20 _lpToken) external onlyOwner {
        ...
        require(pool.accPokecashPerShare == 0 && pool.totalLpSupply == 0, "Cant replace running pool");
        massUpdatePools();
    ...
    // View function to see pending PokeCashs on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accPokecashPerShare = pool.accPokecashPerShare;
        uint256 lpSupply = pool.totalLpSupplyBoosted;
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            uint256 _seconds = block.timestamp - pool.lastRewardTime;
            if (totalAllocPoint > 0) {
                uint256 _pokecashReward = (_seconds * rewardPerSecond * pool.allocPoint) / totalAllocPoint;
                accPokecashPerShare += (_pokecashReward * 1e18) / lpSupply;
            }
        }
        uint256 _totalReward = (_getAccountBoostedAmount(_user, user.amount) * accPokecashPerShare) / 1e18;
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _seconds = block.timestamp - pool.lastRewardTime;
            uint256 _pokecashReward = (_seconds * rewardPerSecond * pool.allocPoint) / totalAllocPoint;
            pool.accPokecashPerShare += (_pokecashReward * 1e18) / lpSupply;
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function _harvestReward(uint256 _pid, address _account) internal {
        ...
        if (_amount > 0) {
            ...
            PoolInfo storage pool = poolInfo[_pid];
            uint256 _totalReward = (_amount * pool.accPokecashPerShare) / 1e18;
            ...
            if (_claimableAmount > 0) {
                require(_claimableAmount * 200 <= IERC20(pokecash).totalSupply(), "Suspicious big reward amount!!"); // <= 0.5% total supply
                emit RewardPaid(_account, _claimableAmount);

                _topupFunds(_claimableAmount);

                _safePokeCashMint(address(this), _claimableAmount);

                if (noStakeFee > 0 && userStakedNft[_account] == 0) {
                    uint256 _fee = (_claimableAmount * noStakeFee) / 10000;
                    _claimableAmount -= _fee;
                    _safePokeCashBurn(_fee);
                }

                _safePokeCashTransfer(_account, _claimableAmount);
            }
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public notContract onlyOneBlock nonReentrant checkHalving {
        ...
        if (_amount > 0) {
            ...
            if (_amount > 0) {
                ...
                pool.totalLpSupplyBoosted += _getAccountBoostedAmount(msg.sender, _userAmount);
                user.amount += _userAmount;
                userLastDepositTime[_pid][msg.sender] = block.timestamp;
            }
        }
        user.rewardDebt = (_getAccountBoostedAmount(msg.sender, user.amount) * pool.accPokecashPerShare) / 1e18;
    ...
    function _withdraw(
        ...
        if (_amount > 0) {
            ...
            if (_earlyWithdrawFee > 0 && insuranceFund != address(0) && block.timestamp < unfrozenDepositTime(_pid, _account)) {
            ...
            pool.totalLpSupplyBoosted -= _getAccountBoostedAmount(_account, _amount);
            user.amount -= _amount;
            _lpToken.safeTransfer(_account, _sentAmount);
        }
        user.rewardDebt = (_getAccountBoostedAmount(_account, user.amount) * pool.accPokecashPerShare) / 1e18;
    ...
    function _safePokeCashTransfer(address _to, uint256 _amount) internal {
        uint256 _pokecashBal = IERC20(pokecash).balanceOf(address(this));
        if (_pokecashBal > 0) {
            if (_amount > _pokecashBal) {
                IERC20(pokecash).safeTransfer(_to, _pokecashBal);
            } else {
                IERC20(pokecash).safeTransfer(_to, _amount);
            }
        }
    }

    function _safePokeCashMint(address _to, uint256 _amount) internal {
        address _pokecash = pokecash;
        if (ICappedMintableBurnableERC20(_pokecash).minter(address(this)) && _to != address(0)) {
            uint256 _totalSupply = IERC20(_pokecash).totalSupply();
            uint256 _cap = ICappedMintableBurnableERC20(_pokecash).cap();
            uint256 _mintAmount = (_totalSupply + _amount <= _cap) ? _amount : (_cap - _totalSupply);
            if (_mintAmount > 0) {
                ICappedMintableBurnableERC20(_pokecash).mint(_to, _mintAmount);
            }
        }
    }

    function _safePokeCashBurn(uint256 _amount) internal {
        uint256 _pokecashBal = IERC20(pokecash).balanceOf(address(this));
        if (_pokecashBal > 0) {
            if (_amount > _pokecashBal) {
                ICappedMintableBurnableERC20(pokecash).burn(_pokecashBal);
            } else {
                ICappedMintableBurnableERC20(pokecash).burn(_amount);
            }
        }
    }
    ...
    function _topupFunds(uint256 _claimableAmount) internal {
        ...
        uint256 _totalMintAmount = _devAmount + _gameTreasuryAmount  + _insuranceAmount;
        if (
            ICappedMintableBurnableERC20(_pokecash).minter(address(this)) &&
            IERC20(_pokecash).totalSupply() + _totalMintAmount <= ICappedMintableBurnableERC20(_pokecash).cap()
        ) {
            ICappedMintableBurnableERC20(_pokecash).mint(devFund, _devAmount);
            ICappedMintableBurnableERC20(_pokecash).mint(gameTreasuryFund, _gameTreasuryAmount);
            ICappedMintableBurnableERC20(_pokecash).mint(insuranceFund, _insuranceAmount);
            totalDevFundAdded = totalDevFundAdded + _devAmount;
            totalGameTreasuryFundAdded = totalGameTreasuryFundAdded + _gameTreasuryAmount;
            totalInsuranceFundAdded = totalInsuranceFundAdded + _insuranceAmount;
        }
    }
    ...
    function emergencyWithdraw(uint256 _pid) external notContract onlyOneBlock nonReentrant {
        ...
        uint256 _amount = user.amount;
        uint256 _sentAmount = _amount;
        if (insuranceFund != address(0) && block.timestamp < unfrozenDepositTime(_pid, msg.sender)) {
            uint256 _earlyWithdrawFee = pool.earlyWithdrawFee;
    ...
    /* ========== NFT ========== */
    function getBoostedPercent(uint256 _tokenId) public view returns (uint256 _boosted) {
        if (_tokenId > 0) {
            (, , uint16 productivityStat, ) = IPokemonNFT(nft).getStats(_tokenId);
            // productivityStat range: (120, 392)
            // boosted range: (2%, 30%)
            if (productivityStat >= 400)
                _boosted = 3000; // 30%
            else if (productivityStat <= 120)
                _boosted = 200; // 2%
            else _boosted = 200 + (productivityStat - 120) * 10; // eg 392 -> 29.2%
        } else {
            _boosted = 0;
        }
    }

    function getAccountBoostedPercent(address _account) external view returns (uint256) {
        return getBoostedPercent(userStakedNft[_account]);
    }

    function _getAccountBoostedAmount(address _account, uint256 _amount) internal view returns (uint256) {
        return _getBoostedAmount(_amount, userStakedNft[_account]);
    }

    function _getBoostedAmount(uint256 _amount, uint256 _tokenId) internal view returns (uint256) {
        uint256 _boosted = getBoostedPercent(_tokenId);
        require(_boosted <= 10000, "max bonus 100%");
        return (_amount * (10000 + _boosted)) / 10000;
    }

    function _massUpdatePoolsWithNftLocked(address _account, uint256 _newLockedTokenId) internal {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (_amount > 0) {
                ...
                updatePool(pid);
                user.rewardDebt = (_getBoostedAmount(_amount, _newLockedTokenId) * pool.accPokecashPerShare) / 1e18;
            }
        }
    }
    ...
    function unlock(uint256 tokenId) external override nonReentrant {
        ...
        IPokemonNFT(nft).unlock(tokenId);
    }
