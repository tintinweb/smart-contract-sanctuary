...
/* Main Contract of PrimeToken starts here */
contract PrimeToken is Context, IERC20, Ownable {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if(_marketingFee>0){
            ...
            marketingAmount = amount.mul(_marketingFee).div(_marketingDenominator);
        }
        if(_liquidityFee>0){
            /* Calculate liquidity fee */
            liquidityAmount = amount.mul(_liquidityFee).div(_liquidityDenominator);
        }
        ...
        if (_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]) {
            ...
            _transferStandard(sender, recipient, (amount.sub(marketingAmount).sub(liquidityAmount)));
    ...
    /* Function     : Set Marketing Fee Denominator  */
    /* Parameters   : Denominator */
    /* Only Owner Function */
    function setMarketingDenominator(uint256 value) external onlyOwner() {
        _marketingDenominator = value;
    }
    
    /* Function     : Set Liquidity Fee Denominator  */
    /* Parameters   : Denominator */
    /* Only Owner Function */
    function setLiquidityDenominator(uint256 value) external onlyOwner() {
