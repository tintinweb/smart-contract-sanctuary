...
contract World_6_Game is Context, IBEP20, Ownable {
    ...
    function _getRTransferAmount(uint256 rAmount, uint256 rFee, uint256 tBurn, uint256 tCharity, uint256 currentRate) private pure returns (uint256) {
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rCharity = tCharity.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rBurn).sub(rCharity*5);
        return rTransferAmount;
    }
    ...
    function _sendToCharity(uint256 tCharity, address sender) private {
        ...
        _tOwned[devAddress] = _tOwned[devAddress].add(tCharity);
            ...
            _rOwned[repurchaseAddress] = _rOwned[repurchaseAddress].add(rCharity);
            _tOwned[repurchaseAddress] = _tOwned[repurchaseAddress].add(tCharity);
            
                     _rOwned[mktWallet] = _rOwned[mktWallet].add(rCharity*3);
                    _tOwned[mktWallet] = _tOwned[mktWallet].add(tCharity*3);
