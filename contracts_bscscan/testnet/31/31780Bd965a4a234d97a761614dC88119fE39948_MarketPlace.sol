...
contract MarketPlace is OwnableUpgradeable, ERC721Holder, PlatformRevenueBase {
    ...
    bool public isOpen;
    mapping(IERC20 => bool) public supportedPriceToken;
    Counters.Counter public selleingOrderID;
    ...
    function initialize(
        ...
        __Ownable_init();
        __PlatformRevenueBase_init(_treeWallet, _burnWallet, _feeWallet, _router);
    ...
    /// @param sellingToken currently only Seedmon address, plan to include another NFT seedmon
    /// @param sellingTokenID NFT ID related to sellingToken
    /// @param priceToken seller can choose the trade currency (not accept native token)
    /// @param priceAmount amount of currency needed for trade
    function selling(
        IERC721 sellingToken,
        ...
        require(
            sellingToken.ownerOf(sellingTokenID) == msg.sender,
            "MarketPlace: seller is not the token owner"
        );
        require(
            sellingToken.getApproved(sellingTokenID) == address(this) ||
                sellingToken.isApprovedForAll(msg.sender, address(this)),
            "MarketPlace: allowance is required for selling"
        );
    ...
    function buy(uint256 sellingOrderID) external {
        ...
        require(
            sellingOrder.priceToken.allowance(msg.sender, address(this)) >=
                sellingOrder.priceAmount &&
                sellingOrder.priceToken.balanceOf(msg.sender) >=
                sellingOrder.priceAmount,
            "MarketPlace: insufficient balance/allowance of buyer"
        );
        ...
        sellingOrder.priceToken.transferFrom(
            msg.sender,
            sellingOrder.seller,
            sellerAmount
        );
        // TODO: which operate cost sending to?, not 60,35,5
        sellingOrder.priceToken.transferFrom(
            msg.sender,
            address(this),
            operateAmount
        );
        distributeOperationFee(sellingOrder.priceToken);
        sellingOrder.token.safeTransferFrom(
            address(this),
            msg.sender,
...
abstract contract PlatformRevenueBase is OwnableUpgradeable {
    ...
    /// @dev pre-requisite is PlatformRevenueImplementer must approve _priceToken(BUSD) to router and treeRewardToken(LEAF) to TreeFarm
    function distributeOperationFee(IERC20 _priceToken) internal {
        
        uint256 priceOperateAmount = _priceToken.balanceOf(address(this));
        uint256 _priceOperateAmountTree = priceOperateAmount.mul(TREE_ALLOCATION).div(TOTAL_ALLOCATION);
        
        address[] memory treeRewardPath = new address[](2);
        treeRewardPath[0] = address(_priceToken);
        treeRewardPath[1] = address(treeRewardToken);
        _priceToken.safeApprove(address(router), MAXUINT);
        router.swapExactTokensForTokens(_priceOperateAmountTree, 1, treeRewardPath, address(this), MAXUINT);
        treeWallet.addReward(treeRewardToken.balanceOf(address(this)));

        _priceToken.safeTransfer(burnWallet, priceOperateAmount.mul(BURN_ALLOCATION).div(TOTAL_ALLOCATION));
        _priceToken.safeTransfer(feeWallet, priceOperateAmount.mul(FEE_ALLOCATION).div(TOTAL_ALLOCATION));
    }
