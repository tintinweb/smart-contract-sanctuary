...
contract SwapContract {
    ...
    function swap(address _tokenOut, uint _amountIn, uint _amountOutMin, address _to) external {
      ...
      IPancakeRouter(PANCAKE_V2_ROUTER).swapExactETHForTokens(_amountOutMin, path, _to, block.timestamp + 600);
    }
    
    //this function will return the minimum amount from a swap
    //input the 3 parameters below and it will return the minimum amount out
    //this is needed for the swap function above
    function getAmountOutMin(address _tokenOut, uint _amountIn) external view returns (uint) {
      //path is an array of addresses.
      //this path array will have 3 addresses [tokenIn, WETH, tokenOut]
      //the if statement below takes into account if token in or token out is WETH.  then the path is only 2 addresses
      address[] memory path;
