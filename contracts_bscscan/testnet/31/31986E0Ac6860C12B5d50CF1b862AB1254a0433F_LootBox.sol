...
contract LootBox is ERC1155, AccessControl {
    ...
    // id => reward contract
    mapping(uint256 => IERC721) private _rewardContracts;
    // id => NFT ids
    ...
    function mint(
        ...
        for (uint256 i = 0; i < rewardIds.length; i++) {
            rewardContract.transferFrom(_msgSender(), address(this), rewardIds[i]);

            _rewards[newTokenId].push(rewardIds[i]);
        }

        _rewards[newTokenId].shuffle();
        _rewardContracts[newTokenId] = rewardContract;
    ...
    function openBox(uint256 boxId) external returns (uint256) {
        ...
        _rewardContracts[boxId].transferFrom(address(this), _msgSender(), rewardId);
