...
//TODO: Burn on action, not burn on transaction
...
contract Ownable is Context {
    ...
    /**
     * @dev Returns the address of the current ownerAddress.
     */
...
contract CoinToken is Context, IERC20, Ownable {
    ...
    constructor ()  {
        ...
        _tTotal =3000000 * _DECIMALFACTOR;
        _rTotal = (_MAX - (_MAX % _tTotal));
        taxFee = 0.7* 100;
        burnFee = 0 * 100;
        charityFee = 0.3* 100;
        ORIG_TAX_FEE = taxFee;
        ORIG_BURN_FEE = burnFee;
        ORIG_CHARITY_FEE = charityFee;
        feeAddress = 0x78b48533EF7748832c330F12311A131b8164ddEe;
        _owner = 0x83bB736Ab1AbEE8056e06856f883b98742dAF6fc;
    ...
    function updateFee(uint256 _txFee,uint256 _burnFee,uint256 _charityFee) onlyOwner() public{
        ...
        taxFee = _txFee* 100;
        burnFee = _burnFee * 100;
        charityFee = _charityFee* 100;
        ORIG_TAX_FEE = taxFee;
        ORIG_BURN_FEE = burnFee;
        ORIG_CHARITY_FEE = charityFee;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tFee, uint256 tBurn, uint256 tCharity) = _getTBasics(tAmount, taxFee, burnFee, charityFee);
    ...
    function removeAllFee() private {
        if(taxFee == 0 && burnFee == 0 && charityFee == 0) return;
    ...
    function restoreAllFee() private {
        taxFee = ORIG_TAX_FEE;
        burnFee = ORIG_BURN_FEE;
        charityFee = ORIG_CHARITY_FEE;
    }
