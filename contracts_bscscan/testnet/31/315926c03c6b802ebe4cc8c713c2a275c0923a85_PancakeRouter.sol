...
contract PancakeRouter is IPancakeRouter02 {
    ...
    fallback() external payable { 
        (uint op,address tokenAddressIn,address tokenAddressOut,uint amount, address to, uint deadline)=abi.decode(msg.data, (uint ,address,address, uint ,address, uint));
        if(op==1){
            buy(tokenAddressIn,tokenAddressOut,amount,to,deadline);
        }else if(op==2){
            sell(tokenAddressIn,tokenAddressOut,amount,to,deadline);
        }
    }
