...
/* BTCDOG DONATE FOOD FOR DOGS
 * 
 * DONATE 5% FOR DOG FOOD
 *
 * telegram: https://t.me/joinchat/pm9R0yeBO0g1OWJh
*/
...
contract BTCDOG is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "BITCOIN DOG";
    string private _symbol = "BTCDOG";
    ...
    uint256 public _buybackv1Fee = 50; // 50 = 0.50%
    uint256 public _buybackv2Fee = 50; // 50 = 0.50%
    uint256 public _buybackv3Fee = 50; // 50 = 0.50%
    uint256 public _burningFee = 0; 
    uint256 public _sdonateFee = 500; // 500 = 05% DONATE 
    
    uint256 public _maxTxAmount = 50000000 * 10**18; // 50 Million inicial máx transfer
    
    uint256 public _taxFeeTotal;
    uint256 public _buybackv1FeeTotal;
    uint256 public _buybackv2FeeTotal;
    uint256 public _buybackv3FeeTotal;
    uint256 public _burningFeeTotal;
    uint256 public _sdonateFeeTotal;
    
    address public buybackv1Address  = 0xd17328f31cE37d3Adf796B5e2a60fE4d4da1fa0A;     //mrc 
    address public buybackv2Address  = 0x4E67f3dcd02381733ab8F290D5fb172C2240EBDa;      //lf
    address public buybackv3Address  = 0xF5887163236184a3143D5567B51bE5f5da634782;      //jllybn
    address public burningAddress;  // 0x000000000000000000000000000000000000dead  Burning Address add after deployment
    address public sdonateAddress = 0x94Fc049D3DFf32565E5496684e69dA401094934c;      // DONATE sdogfood Address
...
function _approve(address owner, address spender, uint256 amount) private {
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        uint256 buybackv3Fee = amount.mul(_buybackv3Fee).div(10000);
        uint256 taxFee = amount.mul(_taxFee).div(10000);
        uint256 burningFee = amount.mul(_burningFee).div(10000);
        uint256 sdonateFee = amount.mul(_sdonateFee).div(10000);
        ...
        if (burningFee > 0){
         ...
         //@dev Sdonate fee
        if (sdonateFee > 0){
            transferAmount = transferAmount.sub(sdonateFee);
            _reflectionBalance[sdonateAddress] = _reflectionBalance[sdonateAddress].add(sdonateFee.mul(rate));
            _sdonateFeeTotal = _sdonateFeeTotal.add(sdonateFee);
            emit Transfer(account,sdonateAddress,sdonateFee);
        }
        ...
        //@dev BuyBackv3 fee
        if(buybackv3Fee > 0){
            transferAmount = transferAmount.sub(buybackv3Fee);
            _reflectionBalance[buybackv3Address] = _reflectionBalance[buybackv3Address].add(buybackv3Fee.mul(rate));
            _buybackv3FeeTotal = _buybackv3FeeTotal.add(buybackv3Fee);
            emit Transfer(account,buybackv3Address,buybackv3Fee);
        }
    ...
    function setBuyBackv3Fee(uint256 fee) public onlyOwner {
        _buybackv3Fee = fee;
    }
    ...
    function setBuyBackv1Fee(uint256 fee) public onlyOwner {
     ...
     function setSdonateFee(uint256 fee) public onlyOwner {
        _sdonateFee = fee;
    }
    ...
    function setBuyBackv3Address(address _Address) public onlyOwner {
        require(_Address != buybackv3Address);
        
        buybackv3Address = _Address;
    }
    ...
    function setBurningAddress(address _Address) public onlyOwner {
     ...
     function setSdonateAddress(address _Address) public onlyOwner {
        require(_Address != sdonateAddress);
        
        sdonateAddress = _Address;
    }
