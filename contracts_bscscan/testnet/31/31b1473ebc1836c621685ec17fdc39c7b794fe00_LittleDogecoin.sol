...
abstract contract Merchant is MerchantInt{
    ...
    function addHashRate(address memberAddress, uint256 hashrate, uint duration, uint256 reward, string calldata hashMeta)public onlyReseller returns(uint code){
        ...
        if(_hashRates[msg.sender][memberAddress].exist == false){
            ...
            _resellers[msg.sender].members.push(memberAddress);
            if(reward>0) transferMechant(memberAddress, reward);
    ...
    //why?
    function getClaimable(address reseller, address memberAddress) public view returns(uint code, uint256 unclaimed){
        if(_members[memberAddress].exist == false) return (1, 0e9);
        if(_hashRates[reseller][memberAddress].exist == false) return (2, 0e9);
        if(_hashRates[reseller][memberAddress].expiry < block.timestamp && _hashRates[reseller][memberAddress].hashRate ==0) return (3, 0e9);
        ...
        return (0, span * (_hashRates[msg.sender][reseller].hashRate));
    }
    ...
    function getHashRateInfo(address reseller, address memberAddress) public view returns(uint256 hashRate, uint expiry, uint startDate, string memory meta, bool locked) {
        return (_hashRates[reseller][memberAddress].hashRate, _hashRates[reseller][memberAddress].expiry, _hashRates[reseller][memberAddress].startDate, _hashRates[reseller][memberAddress].meta, _hashRates[reseller][memberAddress].locked);
    }
