...
contract WORK is ERC721Holder,Ownable{
	...
	function userNft(address _user, uint256 _workId)public view returns(NftInfo[] memory){
		...
		for(uint256 i = 0; i < length; i++){
			nftInfos[i] = _nftMap(userWork[_user][_workId][i]);
		}
		return (nftInfos);
	...
	function _nftMap(uint256 _nftId) internal view returns(NftInfo memory){
		...
		(uint256 lever,string memory name, uint256 roleId, uint256 force, uint256 agile, uint256 physique, uint256 intellect, uint256 willpower, uint256 spirit) = nftContract.nftMap(_nftId);
