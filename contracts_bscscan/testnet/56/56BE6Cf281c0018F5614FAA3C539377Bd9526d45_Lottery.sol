...
contract Lottery is Ownable, Initializable {
    ...
    event LotteryCancled(address owner, bool isCancled);
    ...
    struct LotteryInfo {
        ...
        bool prizeInNFT;    // if this is false, x22 will be awarded, else the NFTs
        // string[] nftMetadata;
        uint256[] nftIDs;
        uint256[] nftQuantity;
    ...
    constructor(address x22, address randomGenerator, address nft)
    {
        _x22 = IERC20(x22);
        _randomGenerator = RandomNumberGenerator(randomGenerator);
        _nft = IERC1155(nft);
    }
    function createNewLottery(bool _prizeInNFT, uint8[] calldata _prizeDistribution,
        uint256[] calldata _nftIDs,uint256[] calldata _nftQuantity, uint256 _costPerTicket, uint256 _minTicketsToBuy,
        uint256 _closingTimestamp) external onlyOwner()
    {
        // checking for starting time is strictly greater than current time
        // if prizeInNFT is false, then prizeDistribution needs some value (non-zero)
        // else, _nftMetadata needs reference to NFTs
        ...
        if (_prizeInNFT) {
            // prizes in NFT
            // require(_nftMetadata.length == 3, "NFTs Distribution Invalid");
            require(_nftIDs.length == 3, "NFTs ID Invalid");
            require(_nftQuantity.length == 3, "NFT Quantity is invalid");
            // add nft metadata for three winners
        } else {
        ...
        allLotteries[lotteryId].prizeInNFT = _prizeInNFT;
        allLotteries[lotteryId].nftIDs.push(_nftIDs[0]);
        allLotteries[lotteryId].nftIDs.push(_nftIDs[1]);
        allLotteries[lotteryId].nftIDs.push(_nftIDs[2]);
    ...
    function costToBuyTickets(uint256 _lotteryId, uint256 _numberOfTickets) public view  returns (uint256 totalCost) {
        ...
        totalCost = costPerTicket * _numberOfTickets;
    }
    function buyTickets(uint256 _lotteryId, uint256 _numberOfTickets) external  {
        ...
        ticketsSold[_lotteryId] = ticketsSold[_lotteryId] + _numberOfTickets;

        // updating prizePool for the lotteryId
        prizePool[_lotteryId] = prizePool[_lotteryId] + totalCost;
    ...
    function drawWinners(uint256 _lotteryId, uint256 seed, uint256 seedDifference) external onlyOwner()  {
        ...
        for (uint i=0;i<3;i++) {
            ...
            seed = seed + seedDifference;
            uint256 index = _randomGenerator.fetchNumber();
            index = index % ticketsSold[_lotteryId];
        ...
        // check prize inNFT
        // if prizeisNFT, give them NFTs
        // else give them x22

        for (uint i=0;i<3;i++) {
                address user = winnersList[_lotteryId][i];
                uint256 nftID = allLotteries[_lotteryId].nftIDs[i];
                uint256 nftQuantity = allLotteries[_lotteryId].nftQuantity[i];
                uint256 amount = allLotteries[_lotteryId].prizeDistribution[i];
                ...
                if(nftQuantity>0){
                    _nft.safeTransferFrom(msg.sender, user, nftID, nftQuantity,"");
                }
    ...
    function cancle() external onlyOwner(){
         delete (allLotteries[lotteryId]);
         _lotteryIdCounter--;
         lotteryId = _lotteryIdCounter;
         emit LotteryCancled(msg.sender, true);
    }
