...
contract dappOptions {
    ...
    modifier ownerOrManager() {
        ...
        for(uint i = 0; i < manager.length; i++) {
            isManager = manager[i] == msg.sender ? true : false;
        }
        ...
        if(__roundGame[__timestamp].timestamp == __timestamp) {
            ...
            for(uint i=0; i<player.length; i++) {
                ...
                if(roundGameId != 0) {
                    ...
                    emit userOrderEvent(__userAddress, __userOrderRecord[__userAddress].tokenAddress,  __userOrderRecord[__userAddress].symbol, __userOrderRecord[__userAddress].positionSide, __userOrderRecord[__userAddress].entryPrice, __userOrderRecord[__userAddress].amount, endPrice, __win, roundGameId, block.timestamp);
