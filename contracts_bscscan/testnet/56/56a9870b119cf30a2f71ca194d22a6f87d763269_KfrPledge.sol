...
contract KfrPledge is Ownable{
    ...
    function getTotalReward(address _address) external view returns(uint){
        return totalReward[_address];
    }

    function getRecommendNum(address _address) external view returns(uint){
        return recommendNum[_address];
    }
    ...
    function setKfr(address _address) external onlyOwner returns (bool){
      kfr = IERC20(_address);
      return true;
    }
