...
contract Maka is MakaBase {
	...
	constructor (address routerAddress, bool isTest) MakaBase(routerAddress, isTest) {
	...
	function calculateClaimRewards(address ofAddress) public view returns (uint256, uint256, uint256) {
		...
		uint256 claimMakaRewards = 0;
		
		if (_rewardAsTokensEnabled) {
			claimMakaRewards = reward * (percentageMaka / 100);
		} else {
		    claimBnbRewards = reward - claimBusdRewards;
		}
...
abstract contract MakaBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	constructor (address routerAddress, bool isTest) {
		_balances[_msgSender()] = totalSupply();
		
		BUSD = isTest ? 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7 : 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
	...
	function approve(address spender, uint256 amount) public override returns (bool) {
	    require(_isBuyingAllowed, "Buying is not allowed before contract activation");
