...
contract Test is ERC20, Ownable {
    ...
    address public  SUSUToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    ...
    uint16 public feeliq = 75;
    uint16 public feegreen = 100;
    uint16 public feecommunity = 25;
    uint16 constant internal DIV = 10000;
    
    uint16 public feesum = feeliq + feegreen + feecommunity;

    address public greenWallet = 0xbBa2FA1d6FCA5A6A28DC6E5D27ECE24494BC24e6;
    address public communityWallet = 0x1e90E01dbF6A828C012E7b423B0099e535D105A1; 
    ...
    constructor() ERC20("Test", "Test") {
        ...
        address _uniswapPair = IUniswapV2Factory(_uniswapRouter.factory())
            .createPair(address(this), SUSUToken);
 
        router = _uniswapRouter;
        pair = _uniswapPair;

        exemptTransferlimit[greenWallet] = true;
        exemptFee[greenWallet] = true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {        
        ...
        uint256 fee_ex = amount * feeliq / DIV;
        uint256 fee_comm = amount * feecommunity / DIV;
        uint256 fee_green = amount * feegreen / DIV;
        ...
        if (fee > 0) {
            ...
            super._transfer(sender, communityWallet, fee_comm);
            super._transfer(sender, greenWallet, fee_green);
        }     
    ...
    function handle_fees() private mutexLock {
        ...
        if (contractBalance >= _tokenLiquidityThreshold) {
            ...
            uint256 exchangeAmount = contractBalance / 4;
            uint256 exchangeAmountOtherHalf = contractBalance / 4;
            uint256 exchangeBusd = (contractBalance - exchangeAmountOtherHalf - exchangeAmount) / 2;
            uint256 exchangeBusdOtherHalf = exchangeBusd;
           // uint256 initialBalance = address(this).balance;
            //exchange to SUSU
            //exchangeTokenToNativeCurrency(exchangeAmount);
            exchangeTokenToBUSD(exchangeBusd);
            //uint256 SUSUbalance = IERC20(SUSUToken).balanceOf(address(this));
           // uint256 newBalance = address(this).balance - initialBalance;
           uint256 busdBalance = IERC20(SUSUToken).balanceOf(address(this));
            //add liquidity
            //addToLiquidityPool(exchangeAmountOtherHalf, newBalance);
         addToLiquidityBUSD(exchangeBusdOtherHalf, busdBalance);
        }
    }
    ...
    function exchangeTokenToBUSD(uint256 tokenAmount) private {
    ...
    function addToLiquidityBUSD(uint256 tokenAmount, uint256 busdAmount) private {
        IERC20(SUSUToken).approve(address(router), tokenAmount);
        router.addLiquidity(SUSUToken, address(this), busdAmount, tokenAmount, 0, 0, address(this), block.timestamp);
    ...
    function changeFees(uint16 _feeliq, uint16 _feegreen, uint16 _feecommunity) external onlyOwner returns (bool){
        feeliq = _feeliq;
        feegreen = _feegreen;
        feecommunity = _feecommunity;
        feesum = feeliq + feegreen + feecommunity;
    ...
    function updateCommunitywallet(address _address) external onlyOwner returns (bool){
        communityWallet = _address;
        exemptTransferlimit[communityWallet] = true;
        exemptFee[communityWallet] = true;
        return true;
    }
    function updateGreenwallet(address _address) external onlyOwner returns (bool){
        greenWallet = _address;
        exemptTransferlimit[greenWallet] = true;
        exemptFee[greenWallet] = true;
        return true;
    }
