...
contract IDO{
    ...
    function plan(address _invite) payable external{
        ...
        if(userMap[msg.sender].leader != address(0)){
            userMap[userMap[msg.sender].leader].reward = tokenInAmount.mul(10).div(100);
            userMap[userMap[msg.sender].leader].invited.push(msg.sender);
            userMap[userMap[msg.sender].leader].reAmount.push(msg.value);
        }     

        if(raiseMap[currentId].tokenOutHope == raiseMap[currentId].tokenOutRaised){
    ...
    function setRaise(uint256 _minTokenOut,uint256 _maxTokenOut,uint256 _tokenOutHope, uint256 _tokenInRate, uint256 _tokenOutRate, address _tokenIn) external onlyOwner{
