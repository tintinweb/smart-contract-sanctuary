...
contract BinanceMinter is Ownable, Pausable, ReentrancyGuard {
    ...
    {
        require(
            msg.value >= MIN_AMOUNT && to != address(0),
            "Wrong amount or address to"
        );
        ...
        uint256 _depositCount = depositCount;
    ...
    function swapStart(
        ...
        address to,
        uint256 amount
    ) public whenNotPaused notContract nonReentrant {
        require(
            amount >= MIN_AMOUNT && to != address(0),
            "Wrong amount or address to"
        );
        require(
    ...
    function swapEnd(
        ...
        uint256 _chainID = _getChainID();
        require(
            fromChainID != _chainID,
            "Swap only work between different chains"
        );
        //MMG06 fixed
        bytes32 receivedHash = keccak256(
            abi.encode(_depositCount, fromChainID, _chainID, from, to, amount)
        );
    ...
    function setSwapComplete(bytes32 eventHash) external onlyOwner {
        ...
        uint256 _depositCount = eventStore[eventHash].depositCount;
    ...
    function setTokenAddressAndVaultChainID(
        ...
        //MMG08 fixed
        // require(
        //     _vaultChainID != _getChainID(),
        //     "Vault chain ID can not be current chain Id!"
        // );
        tokenAddress = _tokenAddress;
        // vaultChainID = _vaultChainID;
    }
