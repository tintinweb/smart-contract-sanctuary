...
contract FuturADA is IBEP20, Auth {
    ...
    address WBNB = 0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F;
    address BURN = 0x225F1792C5a8Fe4aeB7083800c5d8B007844494D;
    ...
    constructor () Auth(msg.sender) {
        ...
        isFeeExempt[BURN] = true;
        isTxLimitExempt[msg.sender] = true;
        isTxLimitExempt[BURN] = true;
        ...
        uint lockedforburn = _totalSupply.div(20); //5% of total supply to the lock for burn wallet

        _balances[msg.sender] = _totalSupply.sub(lockedforburn);
        emit Transfer(address(0), msg.sender, _totalSupply.sub(lockedforburn));

        _balances[BURN] = lockedforburn;
        emit Transfer(address(0), BURN, lockedforburn);
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // max wallet code
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(BURN) && recipient != pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver){
