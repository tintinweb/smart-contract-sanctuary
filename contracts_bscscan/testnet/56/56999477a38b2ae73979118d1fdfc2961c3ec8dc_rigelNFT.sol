...
contract rigelNFT is ERC1155, Ownable {
    ...
    constructor(address _giftDapp) public ERC1155() {
        _owner = _msgSender();
        giftDAppInterface = IGiftDApp(_giftDapp);
        Cards storage card = cards[0];
        card.numOfReferral = 5000;
        card.numOfSpin = 200000;
        card._price = 0.01E18;
    }

    function updateCards( uint256[] memory _id, uint256[] memory _newRef, uint256[] memory _newSpinCount, uint256[] memory _amount) public onlyOwner {
        
        require(_id.length == _newRef.length && _newSpinCount.length == _amount.length, "Rigel: Unequal Lenght...");
        for(uint256 i = 0; i < _id.length; i++) {
            Cards storage card = cards[i];
            card.numOfReferral = _newRef[i];
            card.numOfSpin = _newSpinCount[i];
            card._price = _amount[i];
        } 
    }

    function mint(address _to, uint256 _tokenId, string memory uri_, uint256 _refGiftDAppID) public returns (bool) {
        (uint8 bSpinAvlb, , uint8 rSpinAvlb, , , ) = getGiftData(_refGiftDAppID, _msgSender());
        Cards memory card = cards[_tokenId];
        require(card.numOfSpin != 0 && card.numOfReferral != 0, "Rigel: Can't...");
        if (bSpinAvlb == card.numOfSpin) {
            price[_tokenId] = card._price;
            _mint(_to, _tokenId, card._price, " ");
            _setURI(uri_);
    ...
    function getGiftData(uint256 _id, address _addr) 
        public 
        view 
        returns(uint8, uint8 , uint8 , uint8 , uint256 , uint256 )
    {
        IGiftDApp.UserClaimData memory cClaim = giftDAppInterface.projectClaims(_id, _addr);
