...
//////////CHANGE ABOVE//////////
    ...
    //////////CHANGE BELOW//////////
    constructor() ERC20("EarningUSDC", "EUSDC") { //Change to Name,Shortname
    	...
    	DividendToken = 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca; // This is the reward Address
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "Contract: airdropToWallets:: Arrays must be the same length");
        require(airdropWallets.length < 40, "Contract: airdropToWallets:: Cannot airdrop more than 40 wallets due to gas limit restrictions");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "Contract: The marketing wallet is already this address");
    ...
    function setAuthOnDividends(address account) public onlyOwner{
        RewardDividendTracker.setAuth(account);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "Contract: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Contract: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "Contract: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "Contract: Cannot update gasForProcessing to same value");
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
		(uint256 firstIterations, uint256 firstClaims, uint256 firstLastProcessedIndex) = RewardDividendTracker.process(gas);
		emit ProcessedRewardDividendTracker(firstIterations, firstClaims, firstLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "Contract: Trading has not started yet");
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        transferDividends(DividendToken, address(RewardDividendTracker), RewardDividendTracker, Dividends);
...
contract REWARDDividendTracker is DividendPayingToken, Ownable {
    ...
    //////////CHANGE BELOW//////////
    constructor() DividendPayingToken("RewardDividendTracker", "RewardDividendTracker", 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca) {  //CHANGE TO REWARD CONTRACT
    	claimWait = 1200;
    ...
    function withdrawDividend() pure public override {
        require(false, "Rewards_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Contract contract.");
    }
