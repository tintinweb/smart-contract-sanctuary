 ...
 * markets can exist.
 */
interface I_Windmill_Power is IAccessControlEnumerable, I_With_DAORole, IERC20{
    /**
...
contract Windmill_Stacking is Math, Adressable, AccessControlEnumerable, With_DAORole,
                              With_PWRToken, I_Windmill_Stacking{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    mapping(address=>uint256[]) public userGroupId;
    ...
    ////
    //////////////// Public functions ////////////////
    function setCurrentCycle(uint256 _currentCycle) external onlyRole(DAO_ROLE){
    ...
    function updateCycle(uint256 cycleId) external onlyRole(DAO_ROLE){
        CycleData storage data = cycleData[cycleId];
        data.sPWRSupply = sPWRSupply;
        data.totalPWRMinted = (PWRToken.totalSupply() * percentPWRStackingMint.numerator) / (percentPWRStackingMint.denominator);
    ...
    function updateOneStacking(uint256 groupId) external onlyRole(DAO_ROLE){
        require(groupId < stackingGroup.length, "Windmill_Stacking: Unknown stacking group.");

        _updateOneStacking(groupId);
    }

    function updateStacking() external onlyRole(DAO_ROLE){
    ...
    function _computeUnstacking(StackingGroup storage group, uint256 groupId, uint256 feesPWR) internal{
        ...
        for (uint cycle = group.startCycle; cycle <= group.endCycle; cycle++){
        ...
        PWRToken.transfer(group.userAddr, nbPWR);
    }
