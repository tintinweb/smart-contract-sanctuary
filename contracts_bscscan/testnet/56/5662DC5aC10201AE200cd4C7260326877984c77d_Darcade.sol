...
contract Darcade is ReentrancyGuard, Ownable, Machine {
    ...
    */
     ...
     */
    event CashOutInitiated(string session);
    ...
    */
    ...
    function withdraw(string memory sessionId) public nonReentrant payable {
        ...
        emit CashOutInitiated(sessionId);
    }

    modifier onlyProcessor() {
        require(msg.sender == _processingFeeWallet);
        _;
    }
     ...
     */
    function collectReward(string memory machineId, string memory gameId, string memory sessionId, uint256 amountInWei, address payable receiver) public onlyProcessor {
...
contract Machine {
    ...
    event MachineGameLiquidityReceived(string machineId, string gameId, address from, uint256 amount);
    event MachineGameRewardSent(string machineId, string gameId, address to, uint256 amount);

    /**
     * @dev Adds liquidity to a machine game.
     */
    function addLiquidityToMachineGame(string memory machineId, string memory gameId, IERC20 _token, uint256 amountInWei) public {
        require(amountInWei > 0, 'You must send an amount larger then zero');
        require(_token.allowance(msg.sender, address(this)) >= amountInWei, "You must execute approve on the token for the desired amount before adding liquidity!");
        _token.transferFrom(msg.sender, address(this), amountInWei);
        machines[machineId][gameId] = Liquidity(_token, machines[machineId][gameId].amountInWei += amountInWei);
        emit MachineGameLiquidityReceived(machineId, gameId, msg.sender, amountInWei);
    }
    ...
    function _collectReward(string memory machineId, string memory gameId, uint256 amountInWei, address payable receiver) internal {
        ...
        require(_rewardToken.balanceOf(address(this)) >= amountInWei, 'The Liquidity is too low to complete this transaction!');
