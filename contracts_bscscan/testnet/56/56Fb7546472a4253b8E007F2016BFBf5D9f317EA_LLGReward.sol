...
contract LLGReward is Ownable {
    ...
    address public clientWallet = 0x56118dD254445B66ef36ef680D01700976FbC556;
    ...
    function setClientWallet(address _address) external onlyOwner {
        clientWallet = _address;
    }
    ...
    //         llgContract.transfer(clientWallet, _funds * 10**9);
    // }

    function issuePassport(uint256 _passport) external {
        passport = _passport;
    }

    function offerWinningReward(uint256 _roomId, uint256 _passport, address _winnerAddr) external returns (uint256) {
        if(deposits[_roomId] > 0 && passport == _passport) {
            uint256 rewardAmount = deposits[_roomId] * (100 - taxPercent) / 100;
            uint256 fee = deposits[_roomId] * taxPercent / 100;

            llgContract.transfer(_winnerAddr, rewardAmount * 10**9);
            llgContract.transfer(clientWallet, fee * 10**9);
