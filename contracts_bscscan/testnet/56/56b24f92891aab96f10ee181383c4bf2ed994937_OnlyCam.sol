...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x4D3C412e684bfB512f415c7ec4481069F96EC3a7;
...
contract OnlyCam is Context, IERC20, Ownable {
    ...
    address public _developmentWalletAddress = 0x0b08d17596A6AB28a21709cf6fbA5Bba243b9d84;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "OnlyCam";
    ...
    function setdevelopmentWalletAddress(address newWallet) external onlyOwner() {
        _developmentWalletAddress = newWallet;
    }
    
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <=300 );// tax not more that 5%, 100=1%
        _taxFee = taxFee;
    }
    function setDevelopmentFeePercent(uint256 developmentFee) external onlyOwner() {
        require(developmentFee <=300 );// tax not more that 5%, 100=1%
        _developmentFee = developmentFee;
    }
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <=300 );// tax not more that 5%, 100=1%
