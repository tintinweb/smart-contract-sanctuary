...
contract ScrapYard is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 public _marketFee = 50; // 5% will be added to marketing wallet
    uint256 private _previousmarketFee = _marketFee;

    uint256 _totalFeePerTx = 100; // 10% by default
    ...
    function setSellFeeMultiplierNumerator(uint256 _sellFeeMultiplierNumerator) external onlyOwner {
        sellFeeMultiplierNumerator = _sellFeeMultiplierNumerator;
    }
    ...
    function _takeBothFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tBothFee = tAmount.mul(_totalFeePerTx).div(1e3);
        uint256 rBothFee = tBothFee.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rBothFee);
        if (_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(rBothFee);
        emit Transfer(_msgSender(), address(this), tBothFee);
    }
    ...
    function setSellFee(uint256 _sellFee) private {
        _previousLiquidityFee = _liquidityFee;
        _previousmarketFee = _marketFee;
        _previoustotalFeePerTx = _totalFeePerTx;

        uint256 lp = _liquidityFee.mul(100).div(_totalFeePerTx);
        uint256 mp = _marketFee.mul(100).div(_totalFeePerTx);

        _totalFeePerTx = _sellFee;
        _liquidityFee = _sellFee.mul(lp).div(100);
        _marketFee = _sellFee.mul(mp).div(100);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeBothFee(tAmount, currentRate);
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            totalPercent = lpPercent.add(mwPercent);
