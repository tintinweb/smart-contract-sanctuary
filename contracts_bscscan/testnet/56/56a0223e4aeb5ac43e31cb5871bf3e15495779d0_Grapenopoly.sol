...
contract Grapenopoly
{
    ...
    function rollDices(address selectedTokenToPayRent) external
    {
        ...
        {
            newPlayerPositionIndex = safeSub(safeAdd(currentPlayerPositionIndex, walkSteps), propList.length - 1);
        }
        else
        {
        ...
        if(prizeWalkAmount > 0 && playerAccumulatedStepsFromLastPrize[msg.sender] >= prizeWalkAmount)
        {
            ...
            //Collect prize
            uint256 prizeValue = uint256(keccak256(abi.encodePacked(safeAdd(block.timestamp, prizeNonce), block.difficulty, msg.sender))) % prizeLandMaxPay;
        ...
        if(taxesWalkAmount > 0 && playerAccumulatedStepsFromLastTax[msg.sender] >= taxesWalkAmount)
        {
            ...
            //Pay tax
            uint256 taxValue = uint256(keccak256(abi.encodePacked(safeAdd(block.timestamp, prizeNonce), block.difficulty, msg.sender))) % taxesLandMaxPay;
            if(safeAdd(prizeNonce, 1) >= MAX_NONCE)
            {
                prizeNonce = 0;
            }
        ...
        if(nftOwners[ playerPosition[msg.sender] ] != msg.sender && nftOwners[ playerPosition[msg.sender] ] != feeandtaxreceiver)
        {
            ...
            if(rentValueInUSD > 0)
            {
                uint256 rentValueInToken = safeMulFloat(rentValueInUSD, getQuote(usdToken, selectedTokenToPayRent), ERC20(usdToken).decimals());
