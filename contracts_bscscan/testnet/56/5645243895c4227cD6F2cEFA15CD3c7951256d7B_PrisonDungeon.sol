...
contract PrisonDungeon is Ownable {
    ...
    //============= public function ===============
    //=============================================
    function adventure(uint character_tokenid, uint bitdna_tokenid) public returns(bool) {
        require(msg.sender == tx.origin, "not eoa");
        require(msg.sender == character.ownerOf(character_tokenid), "only owner character NFT can be used");
        require(msg.sender == bitdna.ownerOf(bitdna_tokenid), "only owner BitDNA NFT can be used");
        ...
        // must core attr
        string memory dna = bitdna.binaryDna(bitdna_tokenid);
