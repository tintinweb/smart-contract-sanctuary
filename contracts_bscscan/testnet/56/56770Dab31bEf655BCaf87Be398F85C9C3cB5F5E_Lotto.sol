...
contract Lotto is IERC20 {
    ...
    uint256 private _ticketPrice;                           // Variable for price of each ticket
    uint256 private _purchaseTokenAmount;                   // variable for Amount of tokens per ticket purchase
    ...
    */
    ...
    //Internal function for checking values for checking value for purchaseTicket
    function getNumber(uint256 _number) internal returns(uint256){
       return  _number.div(6);
    }

    modifier purchaseValidator(uint256[] memory _num){
        require((_num.length == 6 || _num.length == 12 || _num.length == 18 || _num.length == 24 || _num.length == 30 || _num.length == 36 || _num.length == 42 || _num.length == 48 || _num.length == 54 || _num.length == 60) , "Invalid Value");
        _;
    }
    /**
    ...
    */
    function purchaseTicket(uint256[] memory _ticketNumbers) public payable purchaseValidator(_ticketNumbers) returns(bool){ 
        require(msg.value > getNumber(_ticketNumbers.length).mul(_ticketPrice), "Insufficient eth value");
        saleParticipants[saleIdNow] = 0;
