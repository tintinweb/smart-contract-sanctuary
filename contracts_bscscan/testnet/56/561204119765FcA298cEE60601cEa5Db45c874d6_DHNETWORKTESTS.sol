...
// simply hold $DHNETWORK and you'll receive CAKE automatically in your wallet.
//
// Hold Baby Cake and get rewarded in Cake on every transaction!
//
//
// 📱 Telegram: https://t.me/babyrworBSC
// 🌎 Website: https://www.babyrwor.app/
// 🌐 Twitter: https://twitter.com/BabyRworBSC
//
...
contract DHNETWORKTESTS is ERC20, Ownable {
    ...
    address public immutable CAKE = address(0xF9f93cF501BFaDB6494589Cb4b4C15dE49E85D0e); // CAKE TESTNET
    ...
    address public _marketingWalletAddress = 0x3d291865C135aD8c9146978467063b754d197729;
    ...
    mapping (address => uint256) public _lastSellsTimes;
    // store last transaction amount of wallets.
    mapping (address => uint256) public _lastSellsAmount;

    // store last transaction times of wallets.
    mapping (address => uint256) public _sellWait;
    ...
    mapping (address => SellsHistory[]) sellsHistoryPerAddress;
    */
    ...
    constructor() ERC20("Diamond Hands Network - Tests", "DHNETWORKTESTS") {

    	dividendTracker = new DHNETWORKTESTSDividendTracker();
    ...
    function updateSellWait(uint256 newSellWait) external onlyOwner {
        require(newSellWait >= 3600 && newSellWait <= 86400, "DHNETWORKTESTS: sellWait must be updated to between 1 and 24 hours");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DHNETWORKTESTS: gasForProcessing must be between 200,000 and 500,000");
    ...
    function canSell(address from, uint256 amount) public view returns(bool){
        ...
        if(amount > balanceOf(from).mul(_maxDailyTxPercentage).div(100)){
            return false;
        }
        if(_sellWait[from] > 0){
            return amount.add(_lastSellsAmount[from]) < balanceOf(from).add(_lastSellsAmount[from]).mul(_maxDailyTxPercentage).div(100);
        }
        return false;
    }

    function updateAddressLastSellData(address from, uint256 amount) private {
        _lastSellsAmount[from] = amount;
        if(_lastSellsTimes[from] == 0 || (_lastSellsTimes[from] != 0 && _sellWait[from] <= 0)){
            _sellWait[from] = sellWait;
        }else{
            _sellWait[from] = _sellWait[from].sub(block.timestamp.sub(_lastSellsTimes[from]));
        }
        _lastSellsTimes[from] = block.timestamp;
    }
    ...
    function _transfer(
        ...
        if (_firstTransferTime == 0){
            _firstTransferTime = block.timestamp;
        }

        // TODO check transfer to other wallets
        ...
        if (to != address(uniswapV2Pair) && from != address(uniswapV2Pair)){
            if(!canSell(from, amount)){
                revert("Sorry you can't make any transfer for now, you have already sold 10% of you bag. Check _sellWait for more info");
            }
        }

        if(from == address(uniswapV2Router) && balanceOf(to).add(amount) > maxWalletSize){
            revert("Wallet balance plus amount of transaction exceeds max wallet balance");
        }
...
contract DHNETWORKTESTSDividendTracker is Ownable, DividendPayingToken {
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "DHNETWORKTESTS_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "DHNETWORKTESTS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DHNETWORKTESTS contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DHNETWORKTESTS_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DHNETWORKTESTS_Dividend_Tracker: Cannot update claimWait to same value");
