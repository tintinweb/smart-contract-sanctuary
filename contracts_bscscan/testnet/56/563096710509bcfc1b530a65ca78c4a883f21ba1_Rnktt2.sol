...
contract Rnktt2{
    ...
    event ReffReward(address _user, uint256 _amount);
    
    constructor() public {
        owner = 0x03AA1cf0097041Df7Ee901518697bEDde21CCaB5;
        token = IBEP20(0x422CC0E87d888993dD2fDEF4E835a2e49317610B);
    ...
    function buyTokenBUSD(address _referrer,uint256 plan,uint256 Amount)  public {
        ...
        require(users[_referrer].isExist || _referrer == owner,"referrer  must be a Buyer or Owner");
        ...
        require(!users[msg.sender].Alreadybought,"you can onlybuy once");
        ...
        users[_referrer].referredamount += numberOfTokens;
        
        if(users[_referrer].referredamount >= minAmount && users[_referrer].referredamount <= 100000 )
        {
                uint256 temp = numberOfTokens.mul(referralPercent[0]).mul(1e18).div(100);
                token.transferFrom(owner, _referrer, temp);
                soldToken = soldToken.add(temp);
                soldToken = soldToken.add(numberOfTokens.mul(1e18));
            emit ReffReward(_referrer,temp);
        }else if(users[_referrer].referredamount >= 100001 && users[_referrer].referredamount <= 500000 )
        {
                uint256 temp = numberOfTokens.mul(referralPercent[1]).mul(1e18).div(100);
                token.transferFrom(owner,_referrer, temp);
                soldToken = soldToken.add(temp);
                soldToken = soldToken.add(numberOfTokens.mul(1e18));
            emit ReffReward(_referrer,temp);
        }else if(users[_referrer].referredamount >= 500001 && users[_referrer].referredamount <= 1000000 )
        {
                uint256 temp = numberOfTokens.mul(referralPercent[2]).mul(1e18).div(100);
                token.transferFrom(owner,_referrer, temp);
                soldToken = soldToken.add(temp);
                soldToken = soldToken.add(numberOfTokens.mul(1e18));
            emit ReffReward(_referrer,temp);
        }else if(users[_referrer].referredamount >= 1000001 )
        {
                uint256 temp = numberOfTokens.mul(referralPercent[3]).mul(1e18).div(100);
                token.transferFrom(owner, _referrer, temp);
                soldToken = soldToken.add(temp);
                soldToken = soldToken.add(numberOfTokens.mul(1e18));
            emit ReffReward(_referrer,temp);
        ...
        if(busdtotoken(Amount) >= 150000 && busdtotoken(Amount) < 300000){
            balances[msg.sender] = balances[msg.sender].add(numberOfTokens.mul(1e18));
            balances[msg.sender] = balances[msg.sender].add(numberOfTokens.mul(2).mul(1e18).div(100));
            uint256 temp = numberOfTokens.mul(2).mul(1e18).div(100);
            token.transferFrom(owner, address(this),temp);
        }else if(busdtotoken(Amount) >= 300000 && busdtotoken(Amount) < 500000){
            balances[msg.sender] = balances[msg.sender].add(numberOfTokens.mul(1e18));
            balances[msg.sender] = balances[msg.sender].add(numberOfTokens.mul(3).mul(1e18).div(100));
            uint256 temp = numberOfTokens.mul(3).mul(1e18).div(100);
            token.transferFrom(owner, address(this),temp);
        }else if(busdtotoken(Amount) >= 500000 && busdtotoken(Amount) < 1000000){
            balances[msg.sender] = balances[msg.sender].add(numberOfTokens.mul(1e18));
            balances[msg.sender] = balances[msg.sender].add(numberOfTokens.mul(5).mul(1e18).div(100));
            uint256 temp = numberOfTokens.mul(5).mul(1e18).div(100);
            token.transferFrom(owner, address(this),temp);
    ...
    function claim() public {
        ...
        require(balances[msg.sender] > 0,"BEP20: Do not have any tokens");
    ...
    function ClaimBusd() public {
        ...
        require(balances[msg.sender] > 0,"BEP20: Do not have any tokens");
        require(!users[msg.sender].claimed,"you can only claim once");
        require(users[msg.sender].claiminBusd,"you can only claim tgt token not BUSD authorized");
        uint256 userBalance = balances[msg.sender];
        balances[msg.sender] = 0;
        BUSD.transferFrom(owner,msg.sender, userBalance.div(tokenPrice));
        token.transfer(owner, userBalance);
        if(users[msg.sender].selectedplan != 3){
            BUSD.transferFrom(owner,msg.sender,(users[msg.sender].usertimelockBonus).div(tokenPrice));
            token.transfer(owner,users[msg.sender].usertimelockBonus);
        }
        claimed[msg.sender] = true;
        users[msg.sender].claimed = true;
        emit ClaimToken(msg.sender, userBalance);
