...
contract PepePrediction is OwnableUpgradeable,  PausableUpgradeable {
    ...
    uint256 public priceUpdateAllowance; // seconds
    ...
    function initialize(
        ...
        priceUpdateAllowance = _priceUpdateAllowance;
     ...
     * callable by admin. Deprecate, use Pancake instead
     */
     ...
     * callable by admin
     */
    function setOracleUpdateAllowance(uint256 _priceUpdateAllowance) external onlyAdmin {
        priceUpdateAllowance = _priceUpdateAllowance;
    }
    ...
    function _startRound(uint256 epoch) internal {
        ...
        round.lockBlock = block.number.add(intervalBlocks * 2);
        round.endBlock = block.number.add(intervalBlocks * 4);
        round.epoch = epoch;
        round.totalAmount = 0;
     ...
     */
    function _getPriceFromPancakeSwap() internal returns (int256) {
            address[] memory BNB_BUSD_Path = new address[](2);
            BNB_BUSD_Path[0] = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
            BNB_BUSD_Path[1] = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
            uint256[] memory amounts = pancakeRouter.getAmountsOut(1000, BNB_BUSD_Path);
            return int256(amounts[1]);
    }
