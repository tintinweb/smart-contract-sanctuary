...
contract MinerTycoon is Ownable {
    ...
    constructor () {
        ...
        mines[1] = Mine(1, 1e18, 30,  uint256(1e18).div(uint256(90)));

        mines[2] = Mine(2, 1e17, 3, uint(1e17).div(90));
    ...
    function buyMine(uint mid, uint8 colors) public {
        ...
        // myMines[ids] = MyMine(mid, block.timestamp, block.timestamp + 1 minutes, mines[mid].everyDay, colors, 0);
        ...
        for (uint8 j = 0; j < 3; j++) {
            ...
            if (j == 0) {
                ...
                users[welMember].receiveGold += 600;
    ...
    function signMine(uint id) public checkMine(id){
        ...
        require(myMines[id].checkTime < block.timestamp, "the mine can no to check ");
         ...
         if (ds > 180) {
            ...
            // myMines[id].checkTime = block.timestamp + 1 minutes;
