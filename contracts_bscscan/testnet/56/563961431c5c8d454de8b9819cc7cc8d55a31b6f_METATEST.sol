...
contract METATEST is Context, IERC20, Ownable {
    ...
    uint256 private minTokensSwapToProjectFunds = 100000 * 10**_decimals; // 0.01%
    ...
    function check_ExcludedFromReflections(address account) public view returns (bool) {
	...
	function A3_checkifBlacklisted(address account) public onlyManager returns (bool) {
        // True - account is blacklisted
        // False -  account is not blacklisted 
        check_address = account;  
        return isBlacklisted[account];
    }
    
    // Anti-Dump settings

    function B1_check_AntiDump_sellWait_Enabled() public onlyManager view returns (bool) {
    ...
    */
    ...
    function C22_isChargedFees(address account) public onlyManager {
        // Enable will be charged all fees (project and reflections fees)
        _isExcludedFromFee[account] = false;
    }
    function C22_isExcludeFromFees(address account) public onlyManager {
        // Exempt from paying any fees
        _isExcludedFromFee[account] = true;
    }
    function C24_checkif_ExcludedFromFees(address account) public onlyManager view returns(bool) {
    ...
    function F3_check_BridgeOrExchange(address account) public onlyManager view returns (bool) {
        return BridgeOrExchange[account];
    }
    function F4_get_BridgeOrExchange_ProjectFee(address account) public onlyManager view returns (uint256) {
        return BridgeOrExchange_ProjectFee[account];
    }
    function F5_get_BridgeOrExchange_ReflectionsFee(address account) public onlyManager view returns (uint256) {
