...
contract MonoStaking is ReentrancyGuard, DataStorage, Events, Ownable, Utils {
    ...
    address public monoToken = 0xD4099A517f2Fbe8a730d2ECaad1D0824B75e084a;
    Utils public utils = Utils(0xC79f069c81FB034C285D68db439c6D23128BbbFc);
    ...
    function _invest(
        ...
        if (user.amount == 0 && user.registerTime == 0) {
            (
                uint256 totalPayout,
    ...
    function withdraw(uint256 poolId) external nonReentrant {
        ...
        if (user.amount == 0 && user.registerTime == 0) {
            (
                ...
                uint256 checkpoint,
                uint256 rewardDebt
            ) = utils.getUserInfo(_msgSender(), poolId);
            user.totalPayout = totalPayout;
            user.registerTime = registerTime;
            user.lastStake = lastStake;
            user.start = start;
    ...
    function unStake(uint256 poolId) external nonReentrant {
        ...
        Pool storage pool = pools[poolId];
        if (user.isUnStake == false && user.amount > 0) {
            user.isUnStake = true;
            uint256 currentDividends = getUserDividends(_msgSender(), poolId)
                .add(user.rewardDebt);
            IBEP20(pool.tokenAddress).transfer(_msgSender(), user.amount);
            if (block.timestamp < pool.toTime) {
                feeUnlock = currentDividends.mul(UNSTAKE_FEE).div(
                    PERCENTS_DIVIDER
                );
            }
            if (feeUnlock > 0) {
            ...
            IBEP20(monoToken).transfer(
                _msgSender(),
                currentDividends.sub(feeUnlock)
            );
