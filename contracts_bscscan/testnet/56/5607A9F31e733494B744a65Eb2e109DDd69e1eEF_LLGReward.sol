...
contract LLGReward is Ownable {
    ...
    function getDeposits(uint256 _room) external onlyOwner view returns (uint256) {
        return deposits[_room];
    }

    function getNumOfConsecutiveWins(address _playerAddr) external onlyOwner view returns (uint) {
