...
/**
 ...
 / _, _/  __/ /__/ /_/ / |/ /  __/ /       / / / /_/ / ,< /  __/ / / /   
/_/_|_|\___/\___/\____/|___/\___/_/       /_/ _\____/_/|_|\___/_/ /_/    
  / ___// /_____ _/ /__(_)___  ____ _   _   _<  / __ \                   
  \__ \/ __/ __ `/ //_/ / __ \/ __ `/  | | / / / / / /                   
 ___/ / /_/ /_/ / ,< / / / / / /_/ /   | |/ / / /_/ /                    
/____/\__/\__,_/_/|_/_/_/ /_/\__, /    |___/_/\____/                     
             ______         /____/      __  __                __         
  __/|___/|_/ ____/___     / __ \__  __/ /_/ /_  ____  ____  / /_/|___/|_
 |    /    / / __/ __ \   / /_/ / / / / __/ __ \/ __ \/ __ \/ /    /    /
/_ __/_ __/ /_/ / /_/ /  / ____/ /_/ / /_/ / / / /_/ / / / /_/_ __/_ __| 
 |/   |/  \____/\____/  /_/    \__, /\__/_/ /_/\____/_/ /_(_) |/   |/    
                              /____/                                     
/**
 * @title RCVR Token
 ...
 * 9.0
 * Added in seperate reward pools for each token
 * Added in the zero removal feature for negative rebases
 * 10.0
 * Added support for Python External interfaces to handle the rebases
 * 
 */
...
contract RCVRStaking is Context, Ownable {
    ...
    address public RCVRAddress = 0x4f619840DAD5CAC0c2233d5F063edBC775eB0dF4;
    ...
    address public rcvrlpcontract = 0x4f619840DAD5CAC0c2233d5F063edBC775eB0dF4;
    ...
    uint private amountofbnbperrefelctor;
    ...
    uint private amountoftokens; //Amount of Tokens needed to qualify for reflection
    ...
    uint public totalamountocharity;
    ...
    bool public noremoval; //No removal of reward balance if negative rebase
    bool public skipnegrebase; // Variable to bypass the rebase if the price has dropped////
    uint private temp;
    uint private temp2;
    uint private temp3;
    //NB Addresses/////////
    ...
    address public riskytoken=0x2170Ed0880ac9A755fd29B2688956BD959F933F8; //Eth by default
    address payable public timeraddress=0xB283E7D0da95D78718B7E0713a8A08dFdE6b2241;
    ...
    mapping(address => uint256) internal riskyrewards; ///Safe Stake Pool
  ...
  constructor () public {
      ...
      safeoldprice =  8023062101082472022;
      riskyoldprice =  23133883578569;
  ...
  function configParams(uint option,bool _onoroff,uint _mode,address payable _sendto) public onlyOwner
  {
      ...
      if (option==12)
      {
          noremoval = _onoroff; //Feature to allow zero removal of rewards if negative rebase
      }
      if (option==13)
      {
          skipnegrebase = _onoroff;
      }
      if (option==14)
      {
          riskymultiplier = _mode;
      }
  ...
  ////////////STAKING FUNCTIONS//////////
   ...
   function removeStake(uint256 _stake) public
   {
       ...
       if(stakes[msg.sender] == 0) //Remove entire stake and rewards
       {
           ...
           temp = riskyrewards[msg.sender];
           _stake = _stake.add(temp);
           rewards[msg.sender] = 0; //Clear balance to ensure correct accounting
           riskyrewards[msg.sender] = 0;
       ...
       public payable
   {
      ...
      reward = reward.add(bonus); //Add the final figure
           ...
           if (bonus2 > 0)
          {
      reward2 = reward2.add(bonus2); //Add the final figure
          }
          }
          
      temptotal = temptotal.add(reward);
      temptotal = temptotal.add(reward2);
      RCVR(RCVRAddress).transfer(msg.sender,temptotal);
      ...
      if (autotransfer==true)
      {
          ...
          tempreward = tempreward.add(tempreward2);
       ...
       public payable
   {
       ...
       reward = rewards[msg.sender];
       riskyr = riskyrewards[msg.sender];
       require (RCVR(RCVRAddress).balanceOf(msg.sender) >= stakes[msg.sender],"B2lo4R(s)");
       rewards[msg.sender] = 0; //Clear balance to ensure correct accounting
       riskyrewards[msg.sender] = 0;
      ////Perform a check to ensure that balance is equal to original stack
      tempreward = reward.div(treasuryrcvr);
      tempreward2 = riskyr.div(treasuryrcvr);
      reward = reward.sub(tempreward);
      riskyr = riskyr.sub(tempreward2);
      templpbalance = RCVR(rcvrlpcontract).balanceOf(msg.sender);
      
     if(enableLPrewards==true)
      {
          bonus = getBonus(reward);
          bonus2 = getBonus(riskyr);
          if (bonus > 0)
          {
        reward = reward.add(bonus); //Add the final figure
          }
          if (bonus2 > 0)
          {
        riskyr = riskyr.add(bonus2);
          }
          }
      temptotal = temptotal.add(reward);
      temptotal = temptotal.add(riskyr);
      RCVR(RCVRAddress).transfer(msg.sender,temptotal); //send out both amounts
      /////Compounding Feature!///////
      tempstake = stakes[msg.sender];
      tempstake = tempstake.add(temptotal);
  ...
  function returnRCVRTokens(bool _returnall,uint _amountOfRCVR) public onlyOwner{
    ...
    function rewardOf(address _stakeholder) public view returns(uint256)
   {
    ...
    ///Function for user to check the amount of Staking rewards
    function riskyRewardOf(address _stakeholder) public view returns(uint256)
   {
       return riskyrewards[_stakeholder];
   }
    /////Functions to Verify total rewards for safe and Risky rewards
   function totalSafeRewards() public view returns(uint256)  ////Safe Staking
   {
    ...
    /////Functions to Verify total rewards for safe and Risky rewards
   function totalRiskyRewards() public view returns(uint256)  ////Safe Staking
   {
       uint256 _totalRewards = 0;
       for (uint256 s = 0; s < stakeholders.length; s += 1){
           _totalRewards = _totalRewards.add(riskyrewards[stakeholders[s]]);
       }
       return _totalRewards;
   }
   ...
   {
       uint temp;
       if (riskymultiplier==1)
       {
           temp = (stakes[_stakeholder].div(riskypercentage)).mul(1);
       }
       if (riskymultiplier==2)
       {
           temp = (stakes[_stakeholder].div(riskypercentage)).mul(2);
       }
       if (riskymultiplier==3)
       {
           temp = (stakes[_stakeholder].div(riskypercentage)).mul(3);
       }
       if (riskymultiplier==4)
       {
           temp = (stakes[_stakeholder].div(riskypercentage)).mul(4);
       }
       if (riskymultiplier==5)
       {
           temp = (stakes[_stakeholder].div(riskypercentage)).mul(5);
       }
       return temp; // Returns Value based at x% * multiplier
   }
   ...
   function rebase(bool _fromforced,bool _isRisky) private {
       ...
       int safeprice = 0;
       bool skiprebase;
       ///////Bool to not trigger rebase if minus///////
       ...
       /////////////////////Rewards calculation////////
       if (RiskyReward == false && skiprebase==false)  //////SafeBet Rewards
       {
      ...
      ////////////////////////Risky reward!!////////
      if (RiskyReward == true  && skiprebase==false)  //////Risky Bet
       {
          ...
          for (uint256 s = 0; s < stakeholders.length; s += 1){
           ...
           ///////////////////////////////////////////////////////////////////
             ...
             riskyrewards[stakeholder] =0; //Zero the reward
             removeStakeholder(stakeholder); //Remove the user
             stakes[stakeholder]=0; //Remove stakes entirely
            ...
            if (riskyremoved==false)
            {
               ...
               if (negrebase == true)
               {
                ...
                if (riskyrewards[stakeholder] >= reward) { //ensure that the users balance can ammodate the subtraction
                  if(noremoval==false) //New feature to not remove balance on price drop
                  {
  ...
  function enableRebase(bool _onoroff) public onlyOwner
  {
   ...
   function reflectBNB() public {
       ...
       totalbnbreflected += amounttoreflectors;
       ////////////for reporting//////
       amountofbnbperrefelctor = shareofBNB;
       //////////////////////////////
       for (uint256 s = 0; s < stakeholders.length; s += 1){
       ...
       if (sendtocharity==true)
       {
           ...
           totalamountocharity += amounttocharity;
       }
       //////////////////////////////
       ...
       //returns stats from reflecting for the website
    function refectStats(uint option) view public returns(uint)
    {
        uint temp;
        if (option==1)
        {
        ...
        if (option==2)
        {
         temp = amountofbnbperrefelctor;      
        }
        if (option==3)
        {
         temp = totalamountocharity;      
        }
        if (option==4)
        {
         temp = amounttoliquidty;      
        }
        if (option==5)
        {
         temp = totalbnbreflected;      
        }
        return temp;
    }
       ///Returns the address of a staker given an index
       function returnStaker(uint index) view public returns (address)
       {
           require(msg.sender == timeraddress || msg.sender == Owner,"Not Auth!");
           address temp;
           if(index < stakeholders.length)
           {
               temp = stakeholders[index];
           }
           else
           {
               temp = burnaddress;
           }
       ///return address in array
           return temp;
       }
       //function to set the reward of the staker, given add address, this is for external access via python///
       function setRewardBalance(uint pool,address staker,uint amount) public
       {
           require(msg.sender==timeraddress || msg.sender==Owner,"Not Auth Set R");
           if (pool==1) //Safe Pool
           {
           ...
           if (pool==2)
           {
               riskyrewards[staker]=amount;
           }
           if (pool==3)
           {
               //Zero Balance//
               riskyrewards[staker]=0;
               rewards[staker]=0;
           }
       }
      function getLastrebase() public view returns(bool)
      {
          
          return lastrebasewassafe;
      }
      function getOldSafePrice() public view returns(uint)
      {
          return  uint(safeoldprice);
      }
      function getOldRiskyPrice() public view returns(uint)
      {
          return riskyoldprice;
      }
      function setoldsafe(int value) public
      {
          safeoldprice = value;
      }
      function setoldrisky(uint value) public
      {
          riskyoldprice = value;
      }
