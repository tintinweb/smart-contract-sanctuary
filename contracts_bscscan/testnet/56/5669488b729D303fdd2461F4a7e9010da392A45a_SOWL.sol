...
contract SOWL is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 private _tTotal = 100000000000 * 1e12;
    ...
    uint256 public _maxSellAmount = _tTotal.div(10000000); // should be 0.00001% percent per transaction
    ...
    // for 1% input 100
    function setMaxTxLimits(uint256 maxTxAmount, uint256 maxSellAmount) public onlyOwner {
    ...
    function setFeePercent(uint256 liquidityFee, uint256 holderFee, uint256 charityFee, uint256 marketFee) external onlyOwner {
        _liquidityFee = liquidityFee;
        _holderFee = holderFee;
        _charityFee = charityFee;
        _marketFee = marketFee;
        totalFee = _liquidityFee.add(_holderFee).add(_charityFee).add(_marketFee);
        _totalFeePerTx = totalFee;
    }

    // numerator must be a multiple of 100
    function setSellFeeMultiplierNumerator(uint256 _duration, uint256 _numerator) external onlyOwner {
    ...
    function changeRouter(IPancakeRouter02 _pancakeRouter, address _pancakePair) external onlyOwner {
        pancakeRouter = _pancakeRouter;
    ...
    function _takeLiquidityFee(uint256 tAmount, uint256 currentRate) internal {
        ...
        uint256 tFee = feeAmount.mul(_liquidityFee.mul(100).div(totalFee)).div(100);
    ...
    function _reflectFee(uint256 tAmount, uint256 currentRate) private {
        uint256 feeAmount = getTotalFeePerTx(tAmount);
        uint256 tFee = feeAmount.mul(_holderFee.mul(100).div(totalFee)).div(100);
        uint256 rFee = tFee.mul(currentRate);
    ...
    function _transfer(
        ...
        if(_isExcludedFromMaxTx[from] == false && 
            ...
            if (!_tradingOpen){
                    require(from != pancakePair && to != pancakePair, "SOWL: Trading is not enabled");
            }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if (!takeFee)
            ...
            require(amount <= _maxSellAmount,"SOWL: amount exceeded max limit");
    ...
    function swapAndLiquify() private {
        ...
        if (contractTokenBalance >= _maxTxAmount) {
            ...
            if (buyBackEnabled && initialBalance > uint256(buyBackLowerLimit)) {
                ...
                Utils.swapETHForTokens(address(pancakeRouter), deadAddress, initialBalance.div(10));
            }
