...
contract XSPSender is Ownable
{
    ...
    modifier validLists(uint256 _contributorsLength, uint256 _balancesLength) {
        require(_contributorsLength > 0, "XSPSender: No contributors sent");
        require(
            _contributorsLength == _balancesLength,
            "XSPSender: Different arrays lengths"
        );
        _;
    }
    ...
    function multisendToken(
        ...
        uint256 _total
    ) external payable validLists(contributors.length, balances.length) hasFee(0) {
        ...
        require(
            total >= 0,
            "XSPSender: Wrong amount of total"
        );
    ...
    function multisendEth(
        ...
        uint256[] calldata balances,
        uint256 _total
    ) external payable validLists(contributors.length, balances.length) hasFee(_total) {
        uint256 total = _total;
