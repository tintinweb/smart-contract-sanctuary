...
import './FuckPresaleToken.sol';
import "@openzeppelin/contracts/access/Ownable.sol";


/**
 * @title FuckPresale 
 * FuckPresale allows investors to make
 * token purchases and assigns them tokens based
 * on a token per BNB rate. Funds collected are forwarded to a wallet 
 * as they arrive.
 */
 
contract FuckPresale is Pausable, Ownable{
  using SafeMath for uint256;

  // The token being sold
  FuckPresaleToken public token;
  ...
  constructor (address payable _wallet, uint256 _minInvestment, uint256 _cap, uint256 _rate) public {
    ...
    wallet = _wallet;
    rate = _rate;
    minInvestment = _minInvestment;  //minimum investment in wei  (=10 bnb)
    cap = _cap * (10**18);  //cap in tokens base units (=295257 tokens)
  }

  // creates presale token
  function createTokenContract() internal returns (FuckPresaleToken) {
    return new FuckPresaleToken();
  }
  ...
  function buyTokens(address beneficiary) public payable whenNotPaused {
    ...
    require(validPurchase(), "Check Presale Min Ammount, and that it's within Max Cap");
...
/**
 * @title FuckPresaleToken (FUCKPT) 
 * Standard Mintable BEP20 Token
 * https://github.com/ethereum/EIPs/issues/20
 * Based on code by FirstBlood:
 * https://github.com/Firstbloodio/token/blob/master/smart_contract/FirstBloodToken.sol
 */

contract FuckPresaleToken is BEP20('Fuck Presale Token', 'FUCKPT') {
