...
contract EpicHeroNFTv2 is ERC721Enumerable, EpicAuth {
    struct LevelUpPrice {
        uint thoreum;
        uint epicHero;
    }
    ...
    address public feeReceiver = 0xe6DC665091364f2dCDCDcE9F352440B7c7d64419;
    address public thoreumAddress = 0xfa766806aCa18749FB3A91b3AA28758D9836B00F;
    IBEP20 private ThoreumToken;
    address public epicHeroAddress = 0x9c07169708042De3479F3475E4125E79E425C009;
    IBEP20 private EpicHeroToken;
    address public reflectAddress;

    // Base URI
    string private _baseUriExtended = "http://hysitech.requestcatcher.com/nftTest";

    constructor() ERC721("Hodler Heroes NFT - Testnet", "HHNFT Testnet") EpicAuth(msg.sender) {
        ...
        //Pack Genesis Heroes
        addPack(500 * 10 ** 18, 25 * 10 ** 18, 1, false, 0, thoreumAddress, epicHeroAddress);
        addPack(1455 * 10 ** 18, 7275 * 10 ** 16, 3, false, 0, thoreumAddress, epicHeroAddress);
        addPack(2250 * 10 ** 18, 11875 * 10 ** 16, 5, false, 0, thoreumAddress, epicHeroAddress);
        addPack(22500 * 10 ** 18, 1125 * 10 ** 18, 50, false, 0, thoreumAddress, epicHeroAddress);

        //LevelUpPrice
        addLevelUpPrice(0, 25 * 10 ** 18);
        addLevelUpPrice(0, 10 * 10 ** 18);
        addLevelUpPrice(0, 25 * 10 ** 18);
        addLevelUpPrice(0, 75 * 10 ** 18);
        addLevelUpPrice(200 * 10 ** 18, 225 * 10 ** 18);
        addLevelUpPrice(250 * 10 ** 18, 250 * 10 ** 18);
        addLevelUpPrice(300 * 10 ** 18, 300 * 10 ** 18);
        addLevelUpPrice(350 * 10 ** 18, 350 * 10 ** 18);
        addLevelUpPrice(400 * 10 ** 18, 400 * 10 ** 18);
        addLevelUpPrice(450 * 10 ** 18, 450 * 10 ** 18);
        addLevelUpPrice(500 * 10 ** 18, 500 * 10 ** 18);
        addLevelUpPrice(550 * 10 ** 18, 550 * 10 ** 18);
        addLevelUpPrice(600 * 10 ** 18, 600 * 10 ** 18);
    ...
    function purchasePack(uint8 packId) external {
        ...
        require(IBEP20(pack.tokenAddress2).transferFrom(msg.sender, address(this), getPrice2(packId)), "Transfer failed");
    ...
    function getPrice2(uint8 packId) public view returns (uint price) {
        Pack memory pack = packTypes[packId];
        price = pack.basePrice2;
    }
    ...
    function addPack(uint232 basePrice, uint232 basePrice2, uint8 numberOfCards, bool saleRunning, uint8 cardSetId, address tokenAddress, address tokenAddress2) public {
        ...
        emit PackAdded(packTypes.length - 1, basePrice, basePrice2, numberOfCards, cardSetId, tokenAddress, tokenAddress2);
    }

    function editPack(uint8 packId, uint232 basePrice, uint232 basePrice2, uint8 numberOfCards, bool saleRunning, uint8 cardSetId, address tokenAddress, address tokenAddress2) external {
        authorizedFor(Permission.ManagePacks);

        packTypes[packId].basePrice = basePrice;
        packTypes[packId].basePrice2 = basePrice2;
        ...
        packTypes[packId].tokenAddress2 = tokenAddress2;

        emit PackEdited(packId, basePrice, basePrice2, numberOfCards, saleRunning, cardSetId, tokenAddress, tokenAddress2);
    }
    ...
    function adminSetRarities(uint[] memory heroIds, uint8[] memory rarities) external {
        ...
        require(heroIds.length == rarities.length, "Invalid array");

        for(uint i = 0; i < heroIds.length; i++){
            Hero storage hero = _heroes[heroIds[i]];

            require(rarities[i] <= maxRarity, "Invalid rarity");

            hero.rarity = rarities[i];
        }
    }
    ...
    function _singleMint(address recipient, uint8 packId, uint8 level, uint8 rarity) internal {
        require(_heroes.length == currentIndex, "Error length");
        require(level > 0 && level <= maxLevel && rarity <= maxRarity, "Error level or rarity");
