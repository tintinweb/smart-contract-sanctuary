...
contract Fund is ICrawler, PriceCrawler, IPFSCrawler {
    ...
    function iterateRequestId() internal returns(bytes memory reqId) {
        reqId = abi.encodePacked(address(this), blockhash(block.number - 1));
    }
     ...
     */
    function crawlerCallback(bytes memory _reqId, QueryType _type, bytes calldata _calldata) external override onlyResolver {
...
interface ICrawler {
    function crawlerCallback(bytes memory _reqId, QueryType _type, bytes calldata _calldata) external;
}
...
abstract contract Crawler is Ownable {
    ...
    event CrawlerQuery(bytes _reqId, address _caller, QueryType _type, bytes _calldata);
    ...
    function crawlerQuery(bytes memory _reqId, QueryType _type, bytes memory _calldata) internal {
