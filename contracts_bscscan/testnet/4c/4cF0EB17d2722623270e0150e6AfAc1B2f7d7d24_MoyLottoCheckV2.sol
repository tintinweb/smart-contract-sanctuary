...
contract MoyLottoCheckV2 is SamoyedLottoCheck {
    using SafeMath for uint256;
    PuppyStorage public puppyStorage;
    uint256 public smoyPrize;

    event onUpdateSmoyPrize(uint256 smoyPrize);
    ...
    function claimMultipleReward(uint256 _round, uint256[] memory _tokenIDArray) external nonReentrant {
        require(_round < MAXIMUM_ROUND, "MoyLottoChec: RoundNumber overflow");
        ...
        for (uint256 i = 0; i < _tokenIDArray.length; i++) {
            ...
            if (!isTokenAlreadyClaimedReward[_round][_tokenID]) {
                ...
                _transferSMOYTo(msg.sender, smoyPrize);
                emit onClaimReward(_round, _tokenID, msg.sender, smoyPrize, 0);
            } else {
    ...
    function updateSmoyPrize(uint256 _smoyPrize) external onlyOwner {
        smoyPrize = _smoyPrize;

        emit onUpdateSmoyPrize(smoyPrize);
    }
}
