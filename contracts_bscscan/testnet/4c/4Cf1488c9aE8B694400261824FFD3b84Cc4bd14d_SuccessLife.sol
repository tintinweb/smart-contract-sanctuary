...
contract SuccessLife {
    ...
    constructor (address _ownerAddress, IERC20 _busdAddress) {
        //default value
        LAST_LEVEL = 7;
        LAST_MATRIX = 5;
        lastUserId = 2;
    ...
    function Owner_registration(address userAddress, address referrerAddress)
        public
        onlyOwner
    {
        ...
        Owner_updateX2Referrer(userAddress, freeX2Referrer, 1);
    ...
    function Owner_updateX2Referrer(
        address userAddress,
        ...
        if (level == 1) {
            ...
            if (
                ...
                for (uint8 k = 1; k <= 30; k++) {
                    if (
                        !users[gullu].activeX2Levels[level] ||
                        users[gullu].partnersCount == 0
                    ) {
    ...
    function Owner_updateX3Referrer(
        address userAddress,
        address referrerAddress,
        uint8 matrix,
        uint8 level
    ...
    function Owner_UpgradeLevel(address _user, uint8 level) external onlyOwner {
        ...
        if (referrerAddress != address(0))
            Owner_updateX2Referrer(_user, referrerAddress, level);
    ...
    function Owner_UpgradeMatrix(address _user, uint8 matrix)
        external
        onlyOwner
    {
        require(
        ...
        Owner_updateX3Referrer(_user, freeX3Referrer, matrix, 1);
        emit Upgrade(_user, matrix, 1);
    }

    function Owner_matrixLevel2Upgrade(address _user, uint8 matrix)
        external
        onlyOwner
    {
        require(
    ...
    {
        require(
            busdToken.balanceOf(userAddress) >= (levelPrice[1]),
            "Low Balance"
        );
        require(
            busdToken.allowance(userAddress, address(this)) >= levelPrice[1],
    ...
    function UpgradeLevel(address _user, uint8 level) external payable {
        ...
        require(!users[_user].activeX2Levels[level], "Level already upgraded!");
        require(
            busdToken.allowance(_user, address(this)) >= levelPrice[level],
    ...
    function UpgradeMatrix(address _user, uint8 matrix) external payable {
        ...
        require(
            busdToken.allowance(_user, address(this)) >= matrixPrice[matrix][1],
            "Invalid Upgradation amount"
        );
        require(
            busdToken.balanceOf(_user) >= matrixPrice[matrix][1],
            "Low Balance"
        );
    ...
    function matrixLevel2Upgrade(address _user, uint8 matrix) external payable {
        ...
        //require(users[_user].x3Matrix[matrix][1].referrals.length >=3,"Matrix Level One is Incomplete");
        require(
            busdToken.allowance(_user, address(this)) >= matrixPrice[matrix][2],
            "Invalid Upgradation amount"
        );
        require(
            busdToken.balanceOf(_user) >= matrixPrice[matrix][2],
            "Low Balance"
        );
        busdToken.transferFrom(_user, address(this), matrixPrice[matrix][2]);
        uint256 newIndex = x3Index[matrix][2] + 1;
