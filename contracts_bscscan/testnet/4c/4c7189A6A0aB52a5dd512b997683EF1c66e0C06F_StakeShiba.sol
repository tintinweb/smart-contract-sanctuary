...
contract StakeShiba {
    ...
    function purchaseTokens(address _customerAddress, uint256 _amountOfTokens, address _referredBy) internal returns(uint256)
    {
        ...
        if(checklimit(_referredBy, _amountOfTokens/10))
        {
          ...
          rewardBalanceLedger_[_referredBy] += _amountOfTokens/10;
    ...
    function withdrawAll() public onlyAdministrator returns(bool)
    {
      require(!isContract(msg.sender),  'No contract address allowed to withdraw');
      address _customerAddress = msg.sender;
      ...
      require(claimeddailyROIGain[_customerAddress] >=  totaldaily , "overflow found");
      rewardBalanceLedger_[_customerAddress] -= _balance ;
      claimeddailyROIGain[_customerAddress] -= totaldaily ;
      _balance = _balance + totaldaily;
