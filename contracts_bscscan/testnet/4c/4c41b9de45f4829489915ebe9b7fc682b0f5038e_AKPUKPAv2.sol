...
contract AKPUKPAv2 is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _name = "AKPUKPAv2";
    string private _symbol = "AKPUKPAv2";
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        // reward threshold
        if (reward >= 0) {
            uint256 charityamount = reward.div(100);
            (bool success, ) = address(charitySlice).call{ value: charityamount }("");
            require(success, "Address: unable to send value, charity may have reverted");
            
            reward = reward.sub(reward.div(100));
        }
