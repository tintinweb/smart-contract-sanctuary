...
contract TierCalculator is AdminableUpgradeable {
    ...
    modifier onlyTokenOrTokenLpPool(address _pool) {
        ...
        // stakingToken or rewardToken is BLP or BLP UNI-V2 LP
        require(
            ...
            "Only pool with BLP or BLP UNI-V2 LP"
        );
        _;
    }
    ...
    function getTierIndex(address _user, address _deal) external view returns (bool success, uint256 tierIndex) {
        uint256 blpBalance = getLockedTokens(_user);
        ...
        for (uint256 i = 0; i < tiersLength; i++) {
            (uint256 blpAmount, , ,) = IDeal(_deal).allTiers(i);
            (uint256 curBlpAmount, , ,) = IDeal(_deal).allTiers(tierIndex);
            if (
                blpBalance >= blpAmount &&
                // check that next level is higher than current
                blpAmount >= curBlpAmount
            ) {
                tierIndex = i;
                success = true;
            }
        ...
        if (success) {
            tierIndex = maxNftTier == tierIndex ? tierIndex : 0;
    ...
    function getLockedTokens(address _user) public view returns (uint256 amount) {
        ...
        // calculate user's BLP on staking pools (max MAX_POOLS length)
        uint256 poolsLength = pools.length;
        for (uint256 i = 0; i < poolsLength; i++) {
            IStakingPool stakingPool = IStakingPool(pools[i]);

            // calculate user's staked BLP
            ...
            // calculate user's pending BLP reward
        ...
        // add user's locked BLP on locker to staked BLP (max MAX_POOLS length)
        uint256 lockersLength = lockers.length;
        for (uint256 i = 0; i < lockersLength; i++) {
            amount = amount.add(ILocker(lockers[i]).getLockedBLP(_user));
        }
    }
    ...
    function getMaxNftTier(address _user) public view returns (bool success, uint256 nftTier) {
        ...
        for (uint256 i = 0; i < lockers.length; i++) {
            (, uint256 tier, bool isDeposited) = ILocker(lockers[i]).userNFT(_user);
            if (isDeposited) {
                success = true;
            }
            if (tier > nftTier) {
    ...
    function isLocker(ILocker _locker) public view returns (bool) {
        try _locker.getLockedBLP(msg.sender) {
            return true;
        } catch {
            return false;
        }
    ...
    function _calculateTokens(
        ...
        // curToken is BLP UNI-V2 LP
        else if (isTokenUniLp(IUniswapV2Pair(curToken))) {
            // calculate BLP amount on UNI-V2 LP
