...
contract miniBAYIKEBO is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    minibayikeboDividendTracker public dividendTracker;
    ...
    constructor() ERC20("miniBAYIKEBO", "miniBAYIKEBO") {
        
        totalFees = BNBRewardsFee.add(liquidityFee);


    	dividendTracker = new minibayikeboDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "miniBAYIKEBO: The router already has that address");
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "miniBAYIKEBO: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "miniBAYIKEBO: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "miniBAYIKEBO: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 1000000, "miniBAYIKEBO: gasForProcessing must be between 200,000 and 1,000,000");
        require(newValue != gasForProcessing, "miniBAYIKEBO: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "miniBAYIKEBO: This account cannot send tokens until trading is enabled");
        }
...
contract minibayikeboDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("minibayikebo_Dividend_Tracker", "minibayikebo_Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "minibayikebo_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public override {
        require(false, "minibayikebo_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main minibayikebo contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 900 && newClaimWait <= 86400, "minibayikebo_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "minibayikebo_Dividend_Tracker: Cannot update claimWait to same value");
