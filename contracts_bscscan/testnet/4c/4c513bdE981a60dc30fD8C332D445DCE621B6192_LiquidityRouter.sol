...
library PancakeLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'5d613229d248b496d8091353a8285ec49f95bae13847a2be3d3369e135158b72' // init code hash
            ))));
    }
...
contract LiquidityRouter is IPancakeRouter02 {
    using SafeMath for uint;
    ...
    constructor(address _factory, address _WETH, address _moonToken) public {
        factory = _factory;
        WETH = _WETH;
    ...
    function removeLiquidity(
        ...
        if (tokenA == moonToken || tokenB == moonToken) {
            (amount0, amount1) = IPancakePair(pair).burn(address(this));
            TransferHelper.safeTransfer(token0, to, amount0);
            TransferHelper.safeTransfer(token1, to, amount1);
        } else {
            (amount0, amount1) = IPancakePair(pair).burn(to);
        }
