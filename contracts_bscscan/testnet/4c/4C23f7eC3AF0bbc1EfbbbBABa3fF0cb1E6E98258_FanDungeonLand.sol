...
// File contracts/FanDungeonLand.sol
...
contract FanDungeonLand is ERC721, ERC721Enumerable, OperatorControl {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    constructor() ERC721("FanDungeonLand", "FDL") {}
    ...
    // Keep track of type id for a given token Id
    mapping(uint256 => uint256) internal _typeIds;

    function getTypeId(uint256 _tokenId) external view returns (uint256) {
        return _typeIds[_tokenId];
    }

    function setTypeId(uint256 _tokenId, uint256 _typeId) external onlyOperator {
        _typeIds[_tokenId] = _typeId;
    }
