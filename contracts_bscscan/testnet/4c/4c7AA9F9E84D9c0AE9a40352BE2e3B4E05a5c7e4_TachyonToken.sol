...
contract TachyonToken is ERC20, Ownable {
    ...
    uint256 private _maxSellBatch = 99000000000000000100; //in TAC
    uint256 private _maxBuyBatch = 999000000000000000100; //in TAC
    ...
    uint256 FOUNDERS_PAYOUT = 810000000000000000000000; //0.81M in 36 times
    ...
    uint256 MARKETING_FUND_PAYOUT = 1795500000000000000000000; //1.7955M in 36 months
    uint256 ADVISORS_PARTNERS_PAYOUT = 900000000000000000000000; //0.9M in 36 times
    ...
    function addMFRewards(address to, uint256 value) external onlyOwner(){
    ...
    function syncMFRewards(uint256 _tokens) external payable{
    ...
    function MFToken(uint256 amount) external {
    ...
    function MFTokenWei(uint256 amount) external {
        amount = amount ;
        require(balanceOf(_msgSender()) >= amount, "TAC: recipient account doesn't have enough balance");
