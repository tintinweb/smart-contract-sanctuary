    ...
    function getTokensToClaim(address from) public view virtual returns (uint256) {
        if (block.timestamp < _firstClaimAvailable) {
            ...
            if(_claimed[msg.sender] == 0){
                return getFirstClaimable(msg.sender) + _historicalBalance(from).mul( block.timestamp.sub(_firstClaimAvailable) ).div(_duration);
            }
            return _historicalBalance(from).mul( block.timestamp.sub(_firstClaimAvailable) ).div(_duration);
        }
    }

    function getFirstClaimable(address from) internal view returns(uint){
        return _total[from].div(4);
    }
