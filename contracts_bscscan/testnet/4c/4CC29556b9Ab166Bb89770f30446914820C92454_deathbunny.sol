...
contract deathbunny is Context, IERC20, Ownable { 
    ...
    */
    ...
    function setToBnbEnabled(bool en) public {
        toBnbEnabled = en;
    }
    
    function setSendToWalletEnabled(bool en) public {
        sendToWalletEnabled = en;
    }    
    
    function setLiqAddenabled(bool en) public {
        liqAddenabled = en;
    }    
    ...
    */
    ...
    event BalanceUpdateFromSwapLiq(uint256 newBalance, uint256 devBalance, uint256 contractBalance);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) public lockTheSwap {
        ...
        // 33% goes to dev ~66% goes to liq
        //
        uint256 oneThird = contractTokenBalance.div(3);
        uint256 twoThirds = contractTokenBalance.sub(oneThird);
        ...
        // swap tokens for ETH
        if(toBnbEnabled) {
            swapTokensForBNB(twoThirds); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        }

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 devBalance = newBalance.div(2);

        emit BalanceUpdateFromSwapLiq(newBalance, devBalance, contractTokenBalance);
        
        if(sendToWalletEnabled) {
            sendToWallet(Wallet_Dev, devBalance);
        }
