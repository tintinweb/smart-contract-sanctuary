...
contract GameHouseEdge {
    ...
    uint256 LockLPPercent;
    uint256 BurnTokenPercent;
    uint256 DaoTreasuryBankRollPercent;

    uint256 burnBustPercent=6000;
    uint256 burnApePercent=2000;
    uint256 burnMfrmPercent=2000;
  ...
  // dao treasury  bankroll
    ...
    // constructor
      ...
      constructor(address _bust, address _busd, address _ape, address _mfrm, address _wbnb,address _bust_lp,address _bnbAPElp,address _bnbMFRMlp,address _bnb_lp, address _router, address _reward,address _lock,address payable _dao) public {
        owner = msg.sender;
        reward = _reward;
    ...
    function setLPAddress(address _bust, address _bnb,address _apeBNB,address _mfrmBNB) public restricted(){
          BUSD_BUST =
        IBiswapPair(_bust);
    ...
    function setPercent(uint _rewardPercent,uint _LockLPPercent, uint _BurnTokenPercent, uint _DaoTreasuryBankRollPercent) public restricted(){
        require(_rewardPercent+_LockLPPercent+_BurnTokenPercent+_DaoTreasuryBankRollPercent == 10000, "sum of percent should be eqqual to 10000");
    ...
    function distribution() public restricted() {
      ...
      */
        ...
        uint256 p2BUST = bustBal.mul(50).div(percentHelper);
        _getBUSDforBUST(p2BUST);
        uint256 BUSTaddliquidityAmount =bustBal.sub(p2BUST);
        addLiquidityBUSTBUSD(BUSTaddliquidityAmount);
        
            /**
      * @dev Distributed BUST/APE/MFRM Into 60/20/20
      */
        ...
        uint256 p3MFRM = DistributedBNBToBurnToken.mul(burnMfrmPercent).div(percentHelper);
                    ...
                    /**
      * @dev Swap BUST,APE and MFRM with BNB
      */
    ...
    function _getBUSDforBUST(uint256 _amount) internal {
        uint256 busd = router.getAmountsOut(_amount,BUSD_BUST_path)[1];
    ...
    function addLiquidityBNBBUST(uint256 value) internal{
        ...
        uint256 minbnb = value.mul(anum).div(aden);
    ...
    function addLiquidityBUSTBUSD(uint256 value) internal{
        (uint256 _reserve0, uint256 _reserve1, ) =BUSD_BUST.getReserves();
        uint256 busd = _reserve1.mul(value).div(_reserve0);
        uint256 minbusd = busd.mul(anum).div(aden);
        uint256 minbust = value.mul(anum).div(aden);
        uint256 time = block.timestamp + 1220;
