...
interface payBackHoldersInterface {
 ...
 function distributeDividends() external payable;
  ...
  function withdrawDividend() external;
   ...
   event payBackHoldersDistributed(
    address indexed from,
    uint256 weiAmount
  );

   event payBackHoldersWithdrawn(
    address indexed to,
    uint256 weiAmount
  );
}
interface payBackHoldersOptionalInterface {
 function withdrawableDividendOf(address _owner) external view returns(uint256);
function withdrawnDividendOf(address _owner) external view returns(uint256);
 function accumulativeDividendOf(address _owner) external view returns(uint256);
}
contract payBackHolders is ERC20, payBackHoldersInterface, payBackHoldersOptionalInterface {
  using SafeMath for uint256;
...
uint256 public totalpayBackHoldersDistributed;
...
function distributeDividends() public override payable {
 ...
 if (msg.value > 0) {
      ...
      emit payBackHoldersDistributed(msg.sender, msg.value);
 totalpayBackHoldersDistributed = totalpayBackHoldersDistributed.add(msg.value);
    }
  }
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      emit payBackHoldersWithdrawn(user, _withdrawableDividend);
...
contract DaddyFlokipayBack is payBackHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() payBackHolders("DaddyFloki_payBack_Tracker", "DaddyFloki_payBack_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "DaddyFloki_payBack_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "DaddyFloki_payBack_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DaddyFloki contract.");
    }
...
contract DaddyFloki is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    uint256 public  payBackFee = 5;
    uint256 public  TotalFees;
 uint256 public gasForProcessing = 300000;
    address public DEXV2Pair;
    PanCakeRouter02 public DEXV2Router;
    DaddyFlokipayBack public payBack;
   uint256 _totalSupply =80000000  * (10**9);
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Daddy Floki", "DDF") {
 
        TotalFees = payBackFee.add(liquidityFee).add(marketFee);
 ...
 DEXV2Router = _DEXV2Router;
        ...
        payBack = new DaddyFlokipayBack();
 payBack.excludeFromDividends(owner());
        payBack.excludeFromDividends(address(this));
        payBack.excludeFromDividends(address(DEXV2Pair));
        payBack.excludeFromDividends(address(DEXV2Router));
        payBack.excludeFromDividends(address(payBack));
        payBack.excludeFromDividends(0x000000000000000000000000000000000000dEaD);
        payBack.excludeFromDividends(0x0000000000000000000000000000000000000000);
lp = _DEXV2Pair;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
        ...
        if(Limited){
            payBack.excludeFromDividends(holder);
        }else{
             try payBack.includeInDividends(payable(holder), balanceOf(holder)) {} catch {}
        }
    }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "DaddyFloki: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "DaddyFloki: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "DaddyFloki: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DaddyFloki: Cannot update gasForProcessing to same value");
        emit GasForProcessingUpdated(newValue, gasForProcessing);
        gasForProcessing = newValue;
    }

    function updateClaimWait(uint256 claimWait) external masterWallet {
        payBack.updateClaimWait(claimWait);
    }

    function getClaimWait() external view returns(uint256) {
        return payBack.claimWait();
    }
  function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
 uint256 tokensTDaddyFloki = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTDaddyFloki);
...
uint256 bnbForLiquidity = bnbBalance.mul(liquidityFee).div(bnbFeeFactor).div(2);
        uint256 bnbForReflection = bnbBalance.mul(payBackFee).div(bnbFeeFactor);
        uint256 bnbForDev = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection);
addLiquidity(tokensTDaddyFloki, bnbForLiquidity);
payable(marketFeeReciever).transfer(bnbForDev);
 (bool success,) = address(payBack).call{value: bnbForReflection}("");
if(success) {
            emit transferDividends(tokensToLiquify.mul(payBackFee).div(TotalFees), bnbForReflection);
        }
    }
 ...
 DEXV2Router.addLiquidityETH{value: ethAmount}(
   ...
   function getAccountDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return payBack.getAccount(account);
    }
function getAccountDividendsInfoAtIndex(uint256 index)
        external view returns (
            ...
            uint256,
            uint256) {
        return payBack.getAccountAtIndex(index);
    }
  function processpayBack(uint256 gas) external {
        (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) = payBack.process(gas);
        emit ProcessedpayBack(iterations, claims, lastProcessedIndex, false, gas, tx.origin);
    }
 function claim() external {
        payBack.processAccount(payable(msg.sender), false);
    }
function getLastProcessedIndex() external view returns(uint256) {
        return payBack.getLastProcessedIndex();
    }
function getNumberOfDividendTokenHolders() external view returns(uint256) {
        return payBack.getNumberOfTokenHolders();
    }
...
try payBack.setBalance(payable(from), balanceOf(from)) {} catch {}
        try payBack.setBalance(payable(to), balanceOf(to)) {} catch {}
 if(!swapping) {
            uint256 gas = gasForProcessing;
try payBack.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                emit ProcessedpayBack(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
            }
            catch {}
        }
    }
