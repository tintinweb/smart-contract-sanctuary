...
/** 
 * Chunks: SafeEarn/EarnHub fork with integrated ERC-1363 (payable token) with improvements and additions (Whale timer & whale fee multiplier, reflection pool mechanism, bonuspay, merchant fee)
 */
 ...
 *  Additional Whale Timer & Fee Multiplier Mechanism + Custom Transfer Fee + Custom Merchant Fee + Reward Pool Mechanism added by the Chunks team
...
contract ERC20 is IERC20, Context, Ownable {
    ...
    constructor (string memory name_, string memory symbol_) {
        ...
        // Liquidity Pool Address for BNB -> Chunks
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        else if(!isBelowWhaleSellThreshold(sender, amount) && _isSell) {
            amountReceived = handleTransferBody(sender, recipient, amount);
            tryToProcess();
        }
        else if(shouldSwapBack(!_isTransfer)) { 
            swapBack();
            amountReceived = handleTransferBody(sender, recipient, amount);
            tryToProcess();
        }
        else if(shouldAutoBuyback(!_isTransfer)) { 
            triggerAutoBuyback(); 
            amountReceived = handleTransferBody(sender, recipient, amount);
            tryToProcess();
        }
    ...
    /** Basic Transfer with no swaps for BNB -> Chunks or Chunks -> BNB */
    ...
    function isBelowWhaleSellThreshold(address sender, uint256 amount) internal view returns (bool) {
        return _amountSold[sender].add(amount) < _whaleSellThreshold;
    }
    ...
    /** True if we should swap from Chunks => BNB, only swapsback during buy/sell to save transfer gas fees */
    ...
    /**
     *  Swaps Chunks for BNB if threshold is reached and the swap is enabled
    ...
    /** Should Chunks buy/burn right now? */
    ...
    /**
     * Buys Chunks with bnb in the contract and then sends to the dead wallet
     */ 
    ...
    /**
     * Buy and Burn Chunks with bnb stored in contract
     */
    function triggerChunksBuyback(uint256 amount) external onlyOwner {
        buyTokens(amount, DEAD);
        emit ChunksBuyBackAndBurn(amount);
    }
    ...
    /** Returns the Circulating Supply of Chunks ( supply not owned by Burn Wallet ) */
    ...
    function getPoolChanceValue() external view returns (uint) {
        return _poolChance;
    }
    function getPoolTransferChanceValue() external view returns (uint) {
        return _poolChance;
    }
    ...
    function getPoolThreshold() external view returns (uint256) {
        return _poolThreshold;
    }
    function getPoolMinimumSpend() external view returns (uint256) {
        return _poolMinimumSpend;
    }
    function getCurrentPoolMinimumTransfer() external view returns (uint256) {
        return _poolMinimumTransfer;
    }
    function getLastWinnerSummary() external view returns (address, uint256, uint) {
        return (_previousWinner, _previousWonAmount, _previousWinTime);
    }
    function getLastWinnerAddress() external view returns (address) {
        return _previousWinner;
    }
    function getLastWonAmount() external view returns (uint256) {
        return _previousWonAmount;
    }
    function getLastWinTime() external view returns (uint) {
        return _previousWinTime;
    }
    ...
    function areUserTransferDividendsEnabled(address holder) external view returns (bool) {
        return enabledTransferDividends[holder];
    }
    ...
    function areTransferEarningsEnabled(address holder) external view returns (bool) {
        return !disabledTransferEarnings[holder];
    }
    ...
    function getUserUnpaidReflectionEarnings(address holder) external view returns (uint256) {
    ...
    event ChunksBuyBackAndBurn(uint256 amountBNB);
...
contract Chunks is ERC1363, TokenRecover {
    constructor ()
        ERC1363("Chunks", "CHUNKS")
    {
    }
}