...
contract ImplAndTermsV2 is Storage, Ownable, ERC20Init {
    ...
    function stakeInternal(address staker, uint tokenAmount, uint holdTime, address referer, address influencer, bool donatsForDevelopers) internal {
        ...
        if (referer != address(0)) {
            require(holdTime >= 30 days, "ImplAndTerms::stakeInternal: holdtime with referer must be more than 30 days");

            uint refererBonus = calcRefererLPAmount(stakerLPAmount);
            stakeFresh(referer, 0, 0, refererBonus);

            stakerLPAmount += calcRefBonusTime(refererBonus, holdTime);
        }
        ...
        if (influencer != address(0)) {
            ...
            stakeFresh(influencer, 0, 0, calcInfluencerLPAmount(stakerLPAmount));
        }

        if (donatsForDevelopers) {
            stakeFresh(getDeveloperAddress(), 0, 0, calcDeveloperLPAmount(stakerLPAmount));
        }
    ...
    function calcRefBonusTime(uint lpAmount, uint holdTime) public view returns (uint) {
        return lpAmount * holdTime * timeBonusPercent / 100e18 / timeNormalizer;
    }
    ...
    function calcInfluencerLPAmount(uint lpAmount) public view returns (uint) {
        return lpAmount * influencerBonusPercent / 100e18;
    }

    function calcDeveloperLPAmount(uint lpAmount) public view returns (uint) {
        return lpAmount * developerBonusPercent / 100e18;
    }
