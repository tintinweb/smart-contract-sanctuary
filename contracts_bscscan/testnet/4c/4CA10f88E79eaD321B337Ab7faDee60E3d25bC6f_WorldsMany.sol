  ...
  */
  function removeShare(address shareholder, uint256 amount) external onlyToken returns (uint256, uint256, uint256) {
    ...
    { 
      ...
      uint256 blockTimestampLast = getBlockTimestamp();
      if (txDateTime.add(5 minutes) < blockTimestampLast)
      {
        if (txDateTime.add(10 minutes) < blockTimestampLast)
        {
  ...
  */
  function calculateShareOfShareHolder(address shareholderAddress, bool checkReward, bool claimReward) private returns (uint256, uint256) {
    ...
    for (uint256 i = 0; i < _shareHoldersTxShares[shareholderAddress].length; i++) {
      ...
      uint256 blockTimestampLast = getBlockTimestamp();
      if (txDateTime.add(5 minutes) < blockTimestampLast) {
        bool rewardCheck = checkReward ? rewardDateTime.sub(5 minutes) < txDateTime : true;
        if (txDateTime.add(10 minutes) < blockTimestampLast && rewardCheck)
        {
...
contract WorldsMany is IBEP20, Authorization {
  ...
  uint256 private _maxTxAmount = 6250 * 10**_decimals; //0.125%
  uint256 private _numTokensSellToAddToLiquidity = 25 * 10**_decimals; //0.0005%
  uint256 private _numTokensSellDistributor = 15 * 10**_decimals; //0.0003%
  ...
  */
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    // is the total reward supply of this contract address over the min number of
    // rewards that we need to initiate a swap
    // also, don't swap & liquify if sender is uniswap pair. 
    bool overMinTokenDistributorBalance = totalRewardSupply >= _numTokensSellDistributor;
    if (
        overMinTokenDistributorBalance &&
        !inSwapAndLiquify &&
        sender != uniswapV2Pair &&
        swapAndLiquifyEnabled
    ) {
  ...
  */
  function depositWallets() external authorized {
    ...
    if (totalAmount > 0) {
        ...
        _balances[address(this)] = _balances[address(this)].add(totalAmount);
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WBNB;
        ...
        BUSD.approve(msg.sender, busdAmount);
          if (lottoBalance > 0) {
            BUSD.transfer(
              _lottoAddress
