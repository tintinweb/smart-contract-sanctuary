  ...
  ITransferable
{
   ...
   * @dev Callable by users
   */
  function viewDeGrandPrize(uint256 _lotteryId)
    external
    view
    returns (DeGrandPrize memory)
  {
    return _deGrandPrizes[_lotteryId];
  }
  ...
  function setDeGrandPrize(
    ...
    require(
      _lotteryId == 1 || // First DeGrand stage
      (_lotteryId == currentLotteryId + 1 && _lotteries[
        currentLotteryId
      ].status == Status.Claimable) || // While DeLotto stage1 goes on
      _lotteries[_lotteryId].status == Status.Open, // Before DeGrand closes
      "DeGrand closed or drawed"
    );
    require(_maxNumberDeGrandWinners < 128, "Maximum limit of winners is 128");

    _deGrandPrizes[_lotteryId] = DeGrandPrize({
  ...
  function pickDeGrandWinners(
    ...
    uint256 maxWinnerCount = _deGrandPrizes[_lotteryId].maxWinnerCount;
