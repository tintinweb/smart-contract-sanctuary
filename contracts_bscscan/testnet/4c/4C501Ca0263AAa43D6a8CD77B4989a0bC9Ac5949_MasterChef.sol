...
contract MasterChef is BaseChef {
    constructor(
        ...
        BaseChef(
            ...
            _devAddress,
            _feeAddress,
            100000000000000000, // Tokens minted per block (0.1)
            5,  // Percent of the bonus tokens sent to dev address (5%)
            5,   // Percent of tokens minted that go to dev (5%)
            0x73feaa1eE314F8c655E354234017bE2193C9E24E, // Pancake Swap
...
contract BaseChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock; // Last block number that DIPs distribution occurs.
    ...
    uint256 public mintDevPerc;

    // Info for using the staked LP tokens to also get rewards from Pancake Swap
    IChef public bonusChef;
    IBEP20 public bonusToken;
    IBEP20 public feeRewardToken;
    ...
    constructor(
        ...
        uint256 _rewardFee,
        uint256 _mintDevPerc,
        address _bonusChefAddress,
        address _bonusTokenAddress,
        address _feeRewardToken
    ) public {
        require(_mintDevPerc <= 10, "max 10%");
        rewardFee = _rewardFee;
        mintDevPerc = _mintDevPerc;
        token = BaseToken(_tokenAddress);
        devAddress = _devAddress;
        feeAddress = _feeAddress;
        tokenPerBlock = _tokenPerBlock;
        bonusChef = IChef(_bonusChefAddress);
        bonusToken = IBEP20(_bonusTokenAddress);
        feeRewardToken = IBEP20(_feeRewardToken);
    }
    ...
    function initiate(uint256 _startBlock) public onlyOwner {
        require(
            _startBlock > block.number || poolInfo.length == 0,
            "start block passed"
        );
    ...
    function set(
        ...
        totalFeeBonusAllocPoint = totalFeeBonusAllocPoint
    ...
    // Amount of token rewards pending for a user
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && pool.totalcap != 0) {
            ...
            accDipPerShare = accDipPerShare.add(
                tokenReward.mul(1e12).div(pool.totalcap)
            );
        }
    ...
    // Amount of wBNB rewards from transfer fees pending for a user
    function pendingFeeEarned(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        return
            user.amount.mul(accFeeBonusPerShare).div(1e12).sub(
                user.feeBonusDebt
            );
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 tokenReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        token.mint(devAddress, tokenReward.div(mintDevPerc));
    ...
    function updateFeeBonusReward(uint256 _pid, uint256 _added) public {
        ...
        if (_added > 0 && pool.totalcap > 0) {
            ...
            pool.accFeeBonusPerShare = pool.accFeeBonusPerShare.add(
                added.mul(1e12).div(pool.totalcap).mul(pool.allocPoint).div(
                    totalFeeBonusAllocPoint
                )
            );
        }
    }
    ...
    function bonusDeposit(uint256 _pid, uint256 _amount) internal {
        ...
        if (pool.isBonus) {
            if (
                pool.lpToken.allowance(address(this), address(bonusChef)) <
                MAX_INT
            ) {
                pool.lpToken.approve(address(bonusChef), MAX_INT);
            }
            uint256 preharvest = bonusToken.balanceOf(address(this));
            bonusChef.deposit(pool.bonusId, _amount);
            updateBonusReward(
                _pid,
                bonusToken.balanceOf(address(this)).sub(preharvest)
            );
            if (user.amount > 0) {
                uint256 pending = user
                    .amount
        ...
        if (pool.isFeeBonus) {
            uint256 currentBalance = feeRewardToken.balanceOf(address(this));
    ...
    function bonusWithdraw(uint256 _pid, uint256 _amount) internal {
        ...
        if (pool.isBonus) {
            uint256 preharvest = bonusToken.balanceOf(address(this));
            bonusChef.withdraw(pool.bonusId, _amount);
            updateBonusReward(
                _pid,
                bonusToken.balanceOf(address(this)).sub(preharvest)
            );
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if (pool.isBonus) {
            bonusChef.withdraw(pool.bonusId, amount);
        }
    ...
    function safeDipTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > tokenBal) {
            ...
            token.mint(_to, _amount.sub(tokenBal));
        } else {
    ...
    function safeBonusTransfer(address _to, uint256 _amount) internal {
        uint256 earnBal = bonusToken.balanceOf(address(this));
        if (_amount > earnBal) {
            bonusToken.transfer(_to, earnBal);
        } else {
    ...
    function safeFeeBonusTransfer(address _to, uint256 _amount) internal {
        uint256 earnBal = feeRewardToken.balanceOf(address(this));
        if (_amount > earnBal) {
            feeRewardToken.transfer(_to, earnBal);
        } else {
            feeRewardToken.transfer(_to, _amount);
        }
    }
...
// Interface for the pancake swap chef to allow us to stake LP there for extra rewards
interface IChef {
    function deposit(
        uint256 _pid,
        uint256 _amount
