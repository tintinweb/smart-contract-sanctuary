...
contract NFTMarket is Ownable {
    ...
    mapping(address => mapping(uint256 => uint256)) listingIndex;
    ...
    function listForSale(IERC721Ownable _token, uint256 _tokenId, uint256 _askPrice) public tokenActive(_token) {
        ...
        // Add listing
        listingIndex[address(_token)][_tokenId] = listings[address(_token)].length;
        listings[address(_token)][listings[address(_token)].length] = Listing({
            owner: msg.sender,
            askPrice: _askPrice
        });
    ...
    function cancelListing(IERC721Ownable _token, uint256 _tokenId) public tokenActive(_token) {
        require(listings[address(_token)][listingIndex[address(_token)][_tokenId]].owner == msg.sender, "You don't own this token.");
        
        listings[address(_token)][listingIndex[address(_token)][_tokenId]].askPrice = 0;
    ...
    function updateListing(IERC721Ownable _token, uint256 _tokenId, uint256 _askPrice) public tokenActive(_token) {
        ...
        listings[address(_token)][listingIndex[address(_token)][_tokenId]].askPrice = _askPrice;
    }
    ...
    function buy(IERC721Ownable _token, uint256 _tokenId) public payable tokenActive(_token) {
        ...
        Collection storage collection = _collection(_token);
        uint256 askPrice = listings[address(_token)][listingIndex[address(_token)][_tokenId]].askPrice;
        ...
        payable(listings[address(_token)][listingIndex[address(_token)][_tokenId]].owner).transfer(msg.value-marketFee-collectionFee);

        _token.safeTransferFrom(address(this), msg.sender, _tokenId);

        
        listings[address(_token)][listingIndex[address(_token)][_tokenId]].owner = address(0);
        listings[address(_token)][listingIndex[address(_token)][_tokenId]].askPrice = 0;
    }
    ...
    function _isListed(IERC721Ownable _token, uint256 _tokenId) internal view returns (bool) {
        return listings[address(_token)][listingIndex[address(_token)][_tokenId]].askPrice > 0;
    }
    ...
    function collectionListings(address _token) external view returns (uint256[] memory) {
        uint256[] memory tokenIds;
        for(uint256 i; i < listings[_token].length; i++) {
            tokenIds[i] = listingIndex[_token][i];
        }
        return tokenIds;
    }
