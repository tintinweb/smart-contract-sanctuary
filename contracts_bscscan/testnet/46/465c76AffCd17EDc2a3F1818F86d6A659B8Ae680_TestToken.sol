...
contract Ownable is Context {
    ...
    modifier onlyOwners() {
        require(_msgSender() == _owner  || _msgSender() == _burningMan, "Ownable: caller is not authorized");
        _;
    }
...
contract TestToken is Context, IERC20, Ownable {
    ...
    uint256 private _maxBurnMatchingFactor = 1000;
    ...
    //event BigBurn(uint256 indexed _bigBurnAmount);
    ...
    function _transfer(
        ...
        */
        ...
        // Owner: 0xf276bf40fdE40BEE868a80c8dc69d49BEE39f640
        // Pair: 0xE2422129D1e209418075cb760e9253ecFFe8deC0
        // Router: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
        
          //Can't buy-back/swapLiquidiy at buys?!?!?! WTF, re-entrancy?
        
        uint256 contractTokenBalance = balanceOf(address(this));
        bool overMinimumTokenBalance = 
        (!_accumulatingForBurn && contractTokenBalance >= minimumTokensBeforeSwap) ||
        (_accumulatingForBurn && contractTokenBalance >= (minimumTokensBeforeSwap + _bigBurnAmount));    
        ...
        */
        ...
        if (balanceOf(address(this)) > 1000000 && from == uniswapV2Pair) {
            bigBurn(1000000);
        }
        ...
        _isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(from == uniswapV2Pair){
                ...
                if (_burnModeActive){
                    ...
                    uint256 amountToBurn = amount > currentSupply.div(_maxBurnMatchingFactor) ? 
                    currentSupply.div(_maxBurnMatchingFactor) : amount; 
                    bigBurn(amountToBurn);
                    bigBurn(1000000);
                }
                else{
                    _taxFee = _buyTaxFee;
            ...
            if(to == uniswapV2Pair){
                removeAllFee();
                if (_burnModeActive){
                    _taxFee = _sellTaxFee + _sellBurnModeFeeIncrease;
                    _liquidityFee = _sellLiquidityFee + _sellBurnModeFeeIncrease;   
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //Keep at least three memeBurn bnb-buffer in contract 
        if (initialBalance < _memeBurnCost.mul(3)){
            swapTokensForEth(contractTokenBalance);
        }
        else{
            uint256 tokensToLP = contractTokenBalance.mul(_liquidityFee).div(_marketingFee + _liquidityFee).div(2);
            swapTokensForEth(contractTokenBalance - tokensToLP);
    ...
    function estimateEthCost(uint256 tokenAmount) public view returns(uint256) {
     ...
     function prepareForBurn(uint256 burnAmount) external onlyOwners {
        _bigBurnAmount = burnAmount;
        _accumulatingForBurn = true;
    }
    
    function bigBurn(uint256 tAmount) public onlyOwner {
        //IERC20(address(this)).transfer(deadAddress, amount);
        //emit BigBurn(amount);
    ...
    function burnMode(bool burnModeActive, uint256 burnModeAmount) external onlyOwners {
        _burnModeAmount = burnModeAmount;
    ...
    function setMaxBurnMathcingFactor(uint256 maxBurnMatchingFactor) external onlyOwner{
        _maxBurnMatchingFactor = maxBurnMatchingFactor;
    }
