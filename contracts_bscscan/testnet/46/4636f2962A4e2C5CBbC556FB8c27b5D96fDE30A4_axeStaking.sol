...
contract axeStaking is AccessControlled {
    ...
    constructor(address _authority) AccessControlled(IAuthority(_authority)) {
        ...
        uint256 _epochLength = 2200;
        uint256 _firstEpochTime = block.timestamp + _epochLength;
        epoch = Epoch({length: _epochLength, number: 1, end: _firstEpochTime, distribute: 0});
    }
