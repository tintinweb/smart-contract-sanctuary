...
contract ExchangeWithAtomic is ExchangeWithOrionPool {
    mapping(bytes32 => LibAtomic.AtomicSwap) public atomicSwaps;
    ...
    function lockAtomicSwap(LibAtomic.AtomicSwap memory swap) public nonReentrant {
    ...
    function redeemAtomicOrder(LibAtomic.AtomicSwapOrder calldata order, bytes calldata secret) public nonReentrant {
        LibAtomic.doValidateRedeemAtomicOrder(order, secret, secrets);
        ...
        emit AtomicSwapRedeemed(order.receiver, order.secretHash, secret);
    }

    function claimAtomicSwap(bytes calldata secret, bytes calldata matcherSignature) public nonReentrant {
        LibAtomic.AtomicSwap storage swap = LibAtomic.doValidateClaimAtomicSwap(
                secret,
                matcherSignature,
                _allowedMatcher,
                atomicSwaps
    ...
    function refundAtomicSwap(bytes32 secretHash) public nonReentrant {
        LibAtomic.AtomicSwap storage swap = atomicSwaps[secretHash];
        require(swap.expiration/1000 < block.timestamp, "E17NE");
        _updateBalance(swap.sender, swap.asset, int(swap.amount));

        emit AtomicSwapRefunded(swap.sender, swap.secretHash);
    }
...
library LibAtomic {
    ...
    /*function doLockAtomicSwap(LibValidator.AtomicSwap memory swap,
            mapping(address => mapping(address => int192)) storage assetBalances) public {
        require(msg.sender == swap.sender, "E3C");
        _updateBalance(swap.sender, swap.asset, -1*int(swap.amount));
        require(checkPosition(swap.sender), "E1PA");
        require(assetBalances[swap.sender][swap.asset] > 0, "E1A");
    ...
    function doValidateRedeemAtomicOrder(
                AtomicSwapOrder calldata order,
    ...
    function doValidateClaimAtomicSwap(
            bytes calldata secret,
            bytes calldata matcherSignature,
            address allowedMatcher,
            mapping(bytes32 => AtomicSwap) storage atomicSwaps)
                public view returns(AtomicSwap storage swap) {
...
/*
    function refundAtomicSwap(bytes32 secretHash) public nonReentrant {
        LibValidator.AtomicSwap storage swap = atomicSwaps[secretHash];
        require(swap.expiration/1000 < block.timestamp, "E17NE");
        _updateBalance(swap.sender, swap.asset, int(swap.amount));
    ...
    function getEthSignedAtomicOrderHash(AtomicSwapOrder calldata _order) internal pure returns (bytes32) {
        return
        keccak256(
            abi.encodePacked(
                "atomicOrder",
    ...
    function validatelAtomic(AtomicSwapOrder calldata order, uint currentTime) internal pure returns (bool) {
...
contract ExchangeWithOrionPool is Exchange, IPoolSwapCallback {
    ...
    function fillThroughOrionPool(
        ...
        tmp.isInContractTrade = uint(getBalance(path[0], order.senderAddress)) >= tmp.amount_spend;
