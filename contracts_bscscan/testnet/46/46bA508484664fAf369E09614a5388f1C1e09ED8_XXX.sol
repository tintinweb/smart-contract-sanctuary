...
contract XXX is Context, IBEP20, Ownable {
...
contract Presale {
    ...
    constructor(address _tokenaddress, uint256 _tokenvalue) public {
        ...
        token = XXX(_tokenaddress);
    }
    
   
    function buyTokens(uint256 _totalvalue) public payable {
        require(token.balanceOf(address(this)) >= _totalvalue, "Smart Contract dont hold enough token");
        require(msg.value == _totalvalue * tokenprice, "You have insufficient balance");
        token.transfer(msg.sender, _totalvalue);
