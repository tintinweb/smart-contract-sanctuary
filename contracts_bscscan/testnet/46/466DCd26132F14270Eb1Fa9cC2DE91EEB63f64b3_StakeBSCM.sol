...
contract StakeBSCM is Ownable {
    ...
    function claimRewards() external {
        ...
        require(_staker.isPresent, "Staker not present");
        uint256 rewardsEarned = earnedRewards(msg.sender);
    ...
    function stake(uint256 amount) external {
        ...
        if (!_staker.isPresent) {
            ...
            _staker.rewardRedeemedAt = _staker.stakedAt;
        }
    ...
    /**
     * @dev To withdraw BSCMemepad from the contract
     *
     * Requirements:
    ...
    function withdraw(uint256 amount) external {
        ...
        require(daysStakedFor >= 7, "Cannot withdraw in cooldown period");

        require(_staker.amount > 0, "No Stakes");
        require(amount <= _staker.amount, "Withdraw amt > staked amt");
        ...
        emit Withdraw(msg.sender, amount, block.timestamp); //solhint-disable-line not-rely-on-time
    }

    /**
     * @dev Returns the caller's staking details
     */
    function myStakes() external view returns (uint256 stakedBSCMemepad, uint256 daysStakedFor) {
        Staker memory _staker = stakerInfo[msg.sender];
     ...
     */
    function calculateRewards(Staker memory _staker) internal view returns (uint256 reward) {
        uint256 daysStakedFor = stakedDays(_staker.rewardRedeemedAt);
        //solhint-disable-next-line not-rely-on-time
        reward = ((_staker.amount.mul(APY)).mul(daysStakedFor)).div(36500);
    }

    /**
     * @dev Returns the number of days for which BSCMemepad was staked
     */
    function stakedDays(uint256 timestamp) internal view returns (uint256) {
        //solhint-disable-next-line not-rely-on-time
