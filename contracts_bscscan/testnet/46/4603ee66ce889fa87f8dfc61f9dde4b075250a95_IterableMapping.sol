...
// File: contracts/EtherDogeDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable Baddalni = address(0xd66c6B4F0be8CE5b39D52E0Fd1344c389929B378);
...
contract EtherDoge is ERC20, Ownable {
    ...
    EtherDogeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("EtherDoge", "ETHERDOGE") {
        ...
        totalFees = _ETHRewardsFee.add(_liquidityFee).add(_marketingFee).add(_marketingBTCFee);

        dividendTracker = new EtherDogeDividendTracker();

        liquidityWallet = owner();
        marketingWallet = address(0xD604825ddd7413CCcf999D1fd2234BD14d0ca506);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "EtherDoge: The dividend tracker already has that address");

        EtherDogeDividendTracker newDividendTracker = EtherDogeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "EtherDoge: The new dividend tracker must be owned by the EtherDoge token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "EtherDoge: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "EtherDoge: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "EtherDoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "EtherDoge: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "EtherDoge: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "EtherDoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "EtherDoge: Cannot update gasForProcessing to same value");
    ...
    function setMarketingFee(uint256 _tokenFee, uint256 _btcFee) external onlyOwner {
        ...
        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee).add(marketingBTCFee);
    }

    function setBTCRewardFee(uint256 _btcRewardFee) external onlyOwner {
        ETHRewardsFee = _btcRewardFee;
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "EtherDoge: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 pieces = marketingBTCFee.add(ETHRewardsFee);
        uint256 btcBalance = IERC20(Baddalni).balanceOf(address(this));
        uint256 dividends = btcBalance.mul(ETHRewardsFee).div(pieces);
...
contract EtherDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("EtherDoge_Dividend_Tracker", "EtherDoge_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "EtherDoge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "EtherDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "EtherDoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EtherDoge_Dividend_Tracker: Cannot update claimWait to same value");
