...
contract MonsterSpawningPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The block number when MONSTER mining ends.
...
// File: contracts/MonsterPoolFactory.sol

pragma solidity 0.6.12;

contract MonsterPoolFactory is Ownable {
    event NewMonsterSpawningPoolContract(address indexed spawningPool);
    ...
    function deployPool(
        ...
        bytes memory bytecode = type(MonsterSpawningPool).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address spawningPoolAddress;

        assembly {
            spawningPoolAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        MonsterSpawningPool(spawningPoolAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewMonsterSpawningPoolContract(spawningPoolAddress);
    }
}
