...
contract Refferal is PausableUpgradeable, OwnableUpgradeable {
    ...
    uint refMotivationNum = 20;
    uint refMotivationAmount = 5 ether;
    IBEP20 public erc20Bonus;
    ...
    function initialize(address _signer, uint _newUserBonusAmount, IBEP20 _erc20Bonus, uint _minClaimReferBonusAmount) external initializer {
        signer = _signer;
    ...
    function getTotalUser(address _refferrer) public view returns(uint) {
        return referreds[_refferrer].length;
    }
    function claimReferBonusAmount() internal {
    ...
    function claimReward(uint _amount, string memory _claimId, uint8 v, bytes32 r, bytes32 s) internal {
    ...
    function claimRewardAndReferBonus(uint _amount, string memory _claimId, uint8 v, bytes32 r, bytes32 s) external {
        require(referBonusAmount[_msgSender()] + _amount >= minClaimReferBonusAmount, "Refferal: Not meet condition yet");
    ...
    function register(address _refferrer, uint _top, uint8 v, bytes32 r, bytes32 s) external {
        ...
        if(_refferrer != address(0)) {
            ...
            if(referreds[_refferrer].length == refMotivationNum) referBonusAmount[_refferrer] += refMotivationAmount;
            if(_top != 0 && _top <= 3) topRefers[_top] = TopRefer(_refferrer, _top);
        }
        users[users.length] = _msgSender();
        isReferrer[_msgSender()] = true;
        erc20Bonus.transfer(_msgSender(), newUserBonusAmount);
    }
    function register1() external {
//        require(permit(_msgSender(), _refferrer, _top, v, r, s), "Refferal: Invalid signature");
//        if(_refferrer != address(0)) {
//            require(isReferrer[_refferrer], "Refferal: Invalid Refferrer");
//            referBonusAmount[_refferrer] += newUserBonusAmount;
...
//        users[users.length] = _msgSender();
    ...
    function setErc20Bonus(IBEP20 _erc20Bonus) external onlyOwner {
        erc20Bonus = _erc20Bonus;
    }
    function setNewUserBonusAmount(uint _newUserBonusAmount) external onlyOwner {
        newUserBonusAmount = _newUserBonusAmount;
    }
    function setRefMotivation(uint _refMotivationNum, uint _refMotivationAmount) external onlyOwner {
        refMotivationNum = _refMotivationNum;
        refMotivationAmount = _refMotivationAmount;
    }
