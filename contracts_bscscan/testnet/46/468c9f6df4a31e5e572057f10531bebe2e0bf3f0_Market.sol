...
contract Market {
    ...
    function transferFrom (address _destination)  public payable{
        // @dosc set address destination  
        
        orders[ID].destination = _destination;
        ...
        orders[ID].walletAddress.transfer(address(this).balance);
        
        
        
         // @dosc send ERC20TOKEN from wallet sender to destination address walletl
        
        token.transferFrom(orders[ID].walletAddress, orders[ID].destination,  orders[ID].tokenAmount );
    }
    
    // @dosc Send ether from msg.sender to contract address
    
    function showOrderWalletAddress () public view returns(address){
        return orders[ID].walletAddress;
    }

    function showOrderDestinationAddress () public view returns(address){
        return orders[ID].destination;
    }
