...
contract MetaStarAuction is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) linkedListlLength;
    ...
    function enterBid(uint256 _amount, address referrer) external nonReentrant {
        uint256 _period = getCurrPeriod();
        uint256 _start = getStart(_period);
        uint256 _end = getEnd(_start);

        require(_period != 2, "The offering has not started yet");
        ...
        if (_referrers[account] == address(0) && referrer != address(0)) {
    ...
    function levelUp(uint256 tokenId, address referrer) public nonReentrant {
        require(
        ...
        uint256 rewardAmount = levelRewardPoints[level] * 1e18;

        POINT.transferFrom(account, address(this), amount);
        referrer = referrers(account);
        if (referrer != address(0)) {
            POINT.mint(referrer, rewardAmount);
        }
        emit LevelUp(account, tokenId, level.add(1));
    }

    function referrers(address account) public view returns (address) {
