...
contract FFLDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("FFL_Dividend_Tracker", "FFL_Dividend_Tracker") {
        claimWait = 7200;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "FFL_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "FFL_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FFL contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "FFL_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FFL_Dividend_Tracker: Cannot update claimWait to same value");
...
contract FFL is ERC20, Ownable {
 
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    FFLDividendTracker public dividendTracker;
  ...
  ILP public liquidutyReciever;
    ...
    constructor() ERC20("FLOKI FAMILY", "FFL") {

        dividendTracker = new FFLDividendTracker();
...
lp = _uniswapV2Pair;
 ...
 function rebaseToken(uint256 epoch, int256 supplyDelta)
  ...
  returns (uint256)
  {
    ...
    function readyForSale(uint256 token) external onlyOwner {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FFL: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FFL: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FFL: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FFL: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount ) internal override {
        ...
        if(takeFee) {
            ...
            uint256 SellFees = amount.mul(BuyTotalFees).div(100);
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 tokensToLP = tokensToLiquify.mul(BuyLiquidityFee).div(BuyTotalFees).div(2);
        ...
        uint256 bnbFeeFactor = BuyTotalFees.sub(BuyLiquidityFee.div(2));

        uint256 bnbForLiquidity = bnbBalance.mul(BuyLiquidityFee).div(bnbFeeFactor).div(2);
        uint256 bnbForReflection = bnbBalance.mul(BuyRewardsFee).div(bnbFeeFactor);
        ...
        if(success) {
            emit transferDividends(tokensToLiquify.mul(BuyRewardsFee).div(BuyTotalFees), bnbForReflection);
        }
    }
