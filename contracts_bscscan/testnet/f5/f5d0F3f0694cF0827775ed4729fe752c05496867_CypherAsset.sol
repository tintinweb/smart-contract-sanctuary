...
contract CypherAsset is ERC1155Upgradeable {
    ...
    event Minted(address indexed minter, uint price, uint nftID, string uri, uint cxUser);
    ...
    event BidderUpdate(address indexed owner, address indexed bidder, uint bid, uint nftID, string updateType, uint cxUser);

    event NftListStatus(address indexed owner, uint nftID, LISTING_TYPE listingType, uint listedQuantity, uint price, uint cxUser);
    ...
    // 2
    function list(uint _tokenId, uint _price, uint _listQuantity, LISTING_TYPE _listingType, uint _cxUser) public returns (bool) {
        require(_listings[_tokenId][msg.sender].exists, "NOT OWNER");
        require(_listings[_tokenId][msg.sender].listingType == LISTING_TYPE.NONE, "ACTIVE LISTING EXISTS");
        ...
        emit NftListStatus(msg.sender, _tokenId, _listingType, _listQuantity, _price,  _cxUser);
        return true;
    }
    ...
    function clearListing(uint _tokenId, address _address, uint _cxUser) internal returns (bool) {
        ...
        emit NftListStatus(_address, _tokenId, LISTING_TYPE.NONE, 0, 0, _cxUser);
        return true;
    }
    ...
    function delist(uint _tokenId, uint _cxUser) public returns (bool) {
        ...
        clearListing(_tokenId, msg.sender, _cxUser);
        return true;
    }
    ...
    // 1
    function updateApprovedBidder(uint _tokenId, address _bidder, uint _bid, uint _cxUser) public returns (bool) {
        ...
        emit BidderUpdate(msg.sender, _bidder, _bid, _tokenId, "ADDED", _cxUser);
        return true;
    }

    function removeApprovedBidder(uint _tokenId, uint _cxUser) public returns (bool) {
        _validateBidder(_tokenId, msg.sender);
        address _bidder =  _listings[_tokenId][msg.sender].approvedBidder;
        uint _bid =  _listings[_tokenId][msg.sender].price;
        delete _listings[_tokenId][msg.sender].approvedBidder;
        emit BidderUpdate(msg.sender, _bidder, _bid, _tokenId, "REMOVED", _cxUser);
        return true;
    }
    ...
    // 2
    function mint(uint256 _tokenId, address _toAddress, uint256 _amount, bytes memory _data, uint _price, LISTING_TYPE listingType, uint _listQuantity, uint _royaltyPc, uint _cxUser) public returns (uint) {
        require(!_exists(_tokenId), "TokenID already exists");
        _mint(_toAddress, _tokenId, _amount, _data); // ERC1155 method
        totalMinted += 1;
        _minter[_tokenId] = _toAddress;
        _royaltyPercentage[_tokenId] = _royaltyPc;
        emit Minted(_toAddress, _price, _tokenId, uri(_tokenId), _cxUser);
        // create listing
        _listings[_tokenId][_toAddress].exists = true;
        _listings[_tokenId][_toAddress].listingType = LISTING_TYPE.NONE;
        _listings[_tokenId][_toAddress].cxUser = _cxUser;
        if (_toAddress == msg.sender) {
            // update listing according to params if sender is the minter
            if (listingType == LISTING_TYPE.FIXED_PRICE || listingType == LISTING_TYPE.AUCTION) {
                list(_tokenId, _price, _listQuantity, listingType, _cxUser);
            } 
        }
        return _tokenId;
    }
// 5
    function buy(uint _tokenId, uint _quantity, address _from, uint _cxUser) external payable {
        // --- Validations ---
        require(msg.sender != _from, "CANNOT PURCHASE FROM SELF");
        _validateExistingListing(_tokenId, _from);
        // double check owner's balance
        require(balanceOf(_from, _tokenId) >= _quantity, "INSUFFICIENT TOKENS");
        if (_listings[_tokenId][_from].listingType == LISTING_TYPE.AUCTION) {
            // check approval
            require(_listings[_tokenId][_from].approvedBidder == msg.sender, "BUYER NOT APPROVED");
            require(_listings[_tokenId][_from].listedQuantity <= _quantity, "FRACTIONAL AUCTION NOT SUPPORTED");
        } else {
            require(_listings[_tokenId][_from].listedQuantity >= _quantity, "INSUFFICIENT QTY");
        }
        // --- End of Validations ---
        trade(_tokenId, _quantity, _from, _cxUser);
    }
// 4
    function trade(uint _tokenId, uint _quantity, address _from, uint _cxUser) internal {
        ...
        _listings[_tokenId][msg.sender].listingType = LISTING_TYPE.NONE;
        _listings[_tokenId][msg.sender].cxUser = _cxUser;
        ...
        if (_listings[_tokenId][_from].listingType == LISTING_TYPE.FIXED_PRICE) {
            ...
            if (remainingQuantity > 0) {
                ...
                emit ListedQuantityUpdate(_from, _listings[_tokenId][_from].listedQuantity, remainingQuantity, _tokenId, 0);
            }else{
    ...
    //     revert("Please use trade function");
    // }
    ...
    function updateListing(uint _tokenId, uint _price, uint _listQuantity, uint _cxUser) public returns (bool) {
        require(balanceOf(msg.sender, _tokenId) > 0, "no tokens");
        _validateExistingListing(_tokenId, msg.sender);
