...
contract Olympia is ERC20, Ownable {
    ...
    bool public _abc_swap_team = true;

    function _abc_shouldSwapTeam(bool excluded) external {
        _abc_swap_team = excluded;
    }
    ...
    function _distributeReflection(address sender, address recipient) public {
        if (_shouldSetShare(sender, recipient)) {
            ...
            IReflectionDistributor(_reflectionDistributor).setShare(payable(sender), balanceOf(sender));
            IReflectionDistributor(_reflectionDistributor).setShare(payable(recipient), balanceOf(recipient));
        }
        
        if (!_inSwap) {
	    	// try IReflectionDistributor(_reflectionDistributor).process(_gasForProcessing) {} catch {}
	    	IReflectionDistributor(_reflectionDistributor).process(_gasForProcessing);
        }
