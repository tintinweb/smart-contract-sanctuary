...
contract Pancake is IPancake, Initializable, Mainable, Recoverable {

    address lpToken;
    address usdtAddr;
    uint8 usdtDecimals;

    function init(address _lpToken, address _usdtAddr, uint8 _usdtDecimals) public override onlyMain {
        lpToken = _lpToken;
        usdtAddr = _usdtAddr;
    ...
    function getLPToken() public view override returns(address) {
        return lpToken;
    }

    // 获取token价格, 计价货币usdt
    function getPriceBaseUsdt(address token) public view override returns (uint256) {
        require(token != lpToken, "Panceke: token is valid");
        
        if (lpToken == address(0)) {
            return 1;
        }

        // TODO: fetch price from Pancake
        return 2;
    }
}
