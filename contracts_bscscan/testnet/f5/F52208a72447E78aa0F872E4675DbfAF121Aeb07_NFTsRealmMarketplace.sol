...
contract NFTsRealmMarketplace is ReentrancyGuard, Ownable {
    ...
    address public immutable PADPaymentAddress;
    ...
    constructor(
        ...
        PADPaymentAddress = padPaymentAddress_;
    ...
    function buyNew(uint256 _id, 
        ...
        if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("KCS")))) {
            ...
            transferKCSOrWKCS(payable(PADPaymentAddress), _commissionValue.div(2));

        } else {
            require(IERC20(tokenAddressMap[paymentType]).transferFrom(payable(msg.sender), _creator, _sellerValue));
            require(IERC20(tokenAddressMap[paymentType]).transferFrom(payable(msg.sender), payable(adminRecoveryAddress), _commissionValue.div(2)));
            require(IERC20(tokenAddressMap[paymentType]).transferFrom(payable(msg.sender), payable(PADPaymentAddress), _commissionValue.div(2)));        
        }
    ...
    function buy(uint256 _id, uint256 _price, string memory paymentType) external payable {
        ...
        if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("KCS")))) {
            uint256 _commissionValue = price[_id].mul(75).div(1000);
            ...
            uint256 _commissionValue = price[_id].mul(50).div(1000);
            uint256 _sellerValue = price[_id].sub(_commissionValue).sub(_royaltyValue);
            require(IERC20(tokenAddressMap[paymentType]).transferFrom(payable(msg.sender), payable(ownerMap[_id]), _sellerValue));
            require(IERC20(tokenAddressMap[paymentType]).transferFrom(payable(msg.sender), payable(creatorMap[_id]), _royaltyValue));
            require(IERC20(tokenAddressMap[paymentType]).transferFrom(payable(msg.sender), payable(adminRecoveryAddress), _commissionValue.div(2)));
            require(IERC20(tokenAddressMap[paymentType]).transferFrom(payable(msg.sender), payable(PADPaymentAddress), _commissionValue.div(2)));
        }
        NFTsRealm(nftContract).transferFrom(address(this), msg.sender, _id);
        ...
        onlyCreatorOrWinner(tokenId)
    {
        ...
        if(winner == address(0)) {
            ...
            if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("KCS")))) {
                uint256 _commissionValue = amount.mul(75).div(1000);
                transferKCSOrWKCS(payable(adminRecoveryAddress), _commissionValue.div(2));
                transferKCSOrWKCS(payable(PADPaymentAddress), _commissionValue.div(2));
                ...
                uint256 _commissionValue = amount.mul(50).div(1000);
                require(IERC20(tokenAddressMap[paymentType]).transfer(payable(adminRecoveryAddress), _commissionValue.div(2)));
                require(IERC20(tokenAddressMap[paymentType]).transfer(payable(PADPaymentAddress), _commissionValue.div(2)));
                if (Creator == creatorMap[tokenId]) {
                    require(IERC20(tokenAddressMap[paymentType]).transfer(payable(Creator), amount.sub(_commissionValue)));
                } else {
                    uint256 _royaltyValue = amount.mul(royaltyMap[tokenId]).div(100);
                    require(IERC20(tokenAddressMap[paymentType]).transfer(payable(creatorMap[tokenId]), _royaltyValue));
                    require(IERC20(tokenAddressMap[paymentType]).transfer(payable(Creator), amount.sub(_royaltyValue).sub(_commissionValue)));
                }
            }
