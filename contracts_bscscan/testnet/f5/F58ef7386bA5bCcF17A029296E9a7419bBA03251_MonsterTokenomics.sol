...
contract MonsterTokenomics is Ownable {
    ...
    uint256 public MAX_SUPPLY = 1000 * 10**6 * 10**18;

    uint256 public PUBLICSALE_ALLOCATION = MAX_SUPPLY.mul(20).div(100); // 20%
    uint256 public PUBLICSALE_REMAINING = PUBLICSALE_ALLOCATION;
    ...
    event DistributionPublicSale(address addr, uint256 amount);
    ...
    //
    function tgeUnicrypt(address _unicrypt) public onlyOwner {
        require(TGE_UNICRYPT > 0, "TGE Unicrypt minted");
        token.mintNomics(_unicrypt, TGE_UNICRYPT);
        TGE_UNICRYPT = 0;
    }
    ...
    function publicUnlock(address _addr, uint256 _amount) public onlyOwner {
        require(PUBLICSALE_REMAINING > _amount, "amount error");
        require(PUBLICSALE_REMAINING > 0, "insufficient amount");

        PUBLICSALE_REMAINING = PUBLICSALE_REMAINING.sub(_amount);
        token.mintNomics(_addr, _amount);
        emit DistributionPublicSale(_addr, _amount);
    }
    ...
    function devUnlock() public onlyOwner {
        ...
        if(DEV_ALLOCATION.sub(DEV_PAID) >= DEV_WEEK_MINT*2){
            ...
            emit DistributionPublicSale(_msgSender(), PRIVATE_WEEK_MINT);
