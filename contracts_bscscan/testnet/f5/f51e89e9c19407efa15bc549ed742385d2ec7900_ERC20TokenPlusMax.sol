    ...
    function _preProcessTransfer(address sender, address recipient, uint256 amount) private returns (uint256) {
        ...
        if (_devAndMarketingAmount > 0) {
            ...
            if (sender != uniswapV2Pair && totalAmountForDevAndMarketing > 0) {
                ...
                if (swapTokensForBNB(_currentDevAndMarketingAmountSnapShot)) {
                    uint256 transferredBalance = address(this).balance - initialBalance;
                    if (transferredBalance == 0) return amountWithFee;
                    devAndMarketingFundAddress.transfer(transferredBalance);
