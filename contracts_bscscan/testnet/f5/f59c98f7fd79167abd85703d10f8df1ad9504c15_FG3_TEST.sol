...
contract FG3_TEST is Context, IERC20, Ownable { 
    ...
    */
    ...
    function __a_process_into_BUSD (uint256 percent_Of_Tokens_To_Liquify) public onlyOwner {
        ...
        swapAndLiquify_BUSD(sendTokens);

    }

     function swapAndLiquify_BUSD(uint256 contractTokenBalance) private lockTheSwap {


        // Create the Auto LP if FoxGirl is 0
            uint256 half_LP = contractTokenBalance / 2;

            uint256 balanceBeforeSwap = BUSD_Token.balanceOf(address(this));
            __a_swapTokensForBUSD(half_LP);
            uint256 BUSD_LP = BUSD_Token.balanceOf(address(this)) - balanceBeforeSwap;
            __a_addBUSD_LP(half_LP, BUSD_LP);
//            emit SwapAndLiquify(half_LP, BUSD_LP, half_LP);
    ...
    */
    // END OF BUSD LP 
      ...
      // Check the balance of BUSD on the contract 

    IERC20 BUSD_Token = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
    uint256 contract_BUSD = BUSD_Token.balanceOf(address(this));


    /////// Need an extrenal function to send the native tokens to the BUSD swap
    ...
    function __a_swapTokensForBUSD(uint256 tokenAmount) public {
    ...
    function __a_addBUSD_LP(uint256 tokenAmount, uint256 amountBUSD) public {
