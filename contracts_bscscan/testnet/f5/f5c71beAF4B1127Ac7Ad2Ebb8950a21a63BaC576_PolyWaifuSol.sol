...
contract PolyWaifuSol is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function transferFrom(
        ...
        require(!_isLockTransferForAntiBot || sender == _lpCreatorAddress, "PWS: Antibot revert");
        
        if (isAddressLP(recipient)) {
            // SELL
            if (sender != _lpCreatorAddress) {
                require(_limiter.isValidSell(amount), "PWS: Sell limit reach");
            }
        }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        if (isAddressLP(_msgSender()) || isAddressLP(recipient)) {
            // BUY
            require(!_isLockTransferForAntiBot || recipient == _lpCreatorAddress, "PWS: Antibot revert");

            if (recipient != _lpCreatorAddress) {
                require(_limiter.isValidBuy(recipient, amount), "PWS: Buy limit reach");
            }
            ...
            require(recipient != address(this), "PWS: Failed to transfer");
...
contract Limiter is OwnableUpgradeable {
