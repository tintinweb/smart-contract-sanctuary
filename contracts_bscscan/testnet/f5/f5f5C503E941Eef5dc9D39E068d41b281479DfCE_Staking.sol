...
contract Staking is Ownable, ReentrancyGuard {
	...
	function _autoSwitchPool(bool force) private {
		...
		if (amount > _totalStaked.mul(2)) {
			uint256 namount = _totalStaked.mul(2);
			uint256 diff = amount.sub(namount);
			if (diff > toAdd) {
				diff = diff.sub(toAdd);
			}
			amount = namount;
			_accumulatedTokensByPeriod = _accumulatedTokensByPeriod.add(diff);
		} else {
