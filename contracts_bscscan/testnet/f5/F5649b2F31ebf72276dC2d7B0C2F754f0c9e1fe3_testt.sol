...
/**
 * @title testt contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract testt is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    // The IPFS hash for all testt concatenated stored here
    string public METADATA_PROVENANCE_HASH = "";

    constructor(string memory baseURI) ERC721("testt","TC")  {
        setBaseURI(baseURI);
    }
    
    function price() public view returns (uint256) {
        require(hasSaleStarted == true, "Sale hasn't started");
        require(totalSupply() < MAX_Supply, "All testt have been sold");

        if (totalSupply() > 100) {
            return 25000000000000000; //  1001-5200 0.025 ETH
        } else {
            return 0; //  0-1000 FREE
        }
    }
    ...
    function mint(uint256 numtestt) public payable {
        require(hasSaleStarted == true, "Sale hasn't started");
        require(totalSupply() < MAX_Supply, "All testt have been sold");
        require(numtestt > 0 && numtestt <= 24, "You can mint minimum 1, maximum 24 testt");
        require(totalSupply().add(numtestt) <= MAX_Supply, "Exceeds MAX_Supply");
        
        if (numtestt == 24) {
            require(msg.value >= price().mul(numtestt).mul(20).div(24), "Ether value sent is below the price");
        } else if (numtestt >= 7) {
            require(msg.value >= price().mul(numtestt).mul(6).div(7), "Ether value sent is below the price");
        } else {
            require(msg.value >= price().mul(numtestt), "Ether value sent is below the price");
        }
        
        for (uint i = 0; i < numtestt; i++) {
