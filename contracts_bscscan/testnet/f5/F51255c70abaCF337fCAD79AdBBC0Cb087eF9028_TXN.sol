...
contract TXN is owned {
    ...
    function getBenPer(uint _tkn)  public pure returns(uint){
        if(_tkn<100){
            return _tkn*100/100;
        }else if(_tkn>=100 && _tkn<3000){
            return _tkn*110/100;
        }else if(_tkn>=3000 && _tkn<10000){
            return _tkn*112/100;
        }else if(_tkn>=10000 && _tkn<25000){
            return _tkn*115/100;
        }else if(_tkn>=25000 && _tkn<50000){
            return _tkn*117/100;
        }else if(_tkn>=50000 && _tkn<100000){
            return _tkn*120/100;
        }
    }
    
    function sendToken(uint _tkn) public bonusRelease returns(bool success){
        return  my_token.transfer(msg.sender, _tkn);
    }
    
    
    function purchaseToken( uint _value) public payable returns(bool success){
        
        uint token_p = SafeMath.mul(_value , per_token);
        uint token_s = getBenPer(token_p);
        ...
        total_purchased = SafeMath.add(total_purchased,token_s);
