...
contract BHeroToken is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  function claim(uint256 count) external {
    ...
    require(totalMintCount > 0, "No token to claim");
  }
  ...
  function mint(uint256 count) external {
    ...
    address to = msg.sender;
    uint256 size = tokenIds[to].length + tokenRequests[to].length;
    require(size + count <= design.getTokenLimit(), "User limit reached");
  ...
  function processTokenRequests() external {
    address to = msg.sender;
    CreateTokenRequest[] memory requests = tokenRequests[to];
    delete tokenRequests[to];
    for (uint256 i = 0; i < requests.length; ++i) {
      CreateTokenRequest memory request = requests[i];
  ...
  function upgrade(uint256 baseId, uint256 materialId) external {
    ...
    require(baseLevel == materialLevel && baseLevel < 10, "Invalid token level");

    // Transfer coin token.
    uint256 cost = design.getUpgradeCost(baseLevel - 1);
...
library Utils {
  ...
  /// Random by weights.
  function randomByWeights(uint256 seed, uint256[] memory weights)
    internal
    view
...
interface IBHeroDesign {
  ...
  function getUpgradeCost(uint256 level) external view returns (uint256);
...
contract BHeroDesign is AccessControlUpgradeable, UUPSUpgradeable, IBHeroDesign {
  ...
  function getUpgradeCost(uint256 level) external view override returns (uint256) {
  ...
  function createRandomToken(
    ...
    (seed, details.abilities) = Utils.randomSampling(seed, abilityIds, stats.ability);
