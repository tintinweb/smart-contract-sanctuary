...
contract DividendDistributor is IDividendDistributor {
    ...
    constructor (address _router, address _dividend, address _weth) {
        ...
        DIVIDEND = IBEP20(_dividend);
        WETH = _weth;
        _token = msg.sender;
        minDistribution *= (10 ** DIVIDEND.decimals());
    }
...
contract ProjectE is IBEP20, Auth {
    ...
    string constant _name = "GuineaPig";
    ...
    uint256 reflectionFee  = 8;  //  8% reflects DIVIDEND back to holders
    ...
    constructor (address _dexRouter, address _marketingAddress, address _dividend) Auth(msg.sender) {
        DIVIDEND = _dividend;
        ...
        distributor = new DividendDistributor(routerAddress, DIVIDEND, WETH);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		// Max tx check
		require (amount <= maxTransactionSize || isTxLimitExempt[sender], "Max amount per transaction exceeded");
		...
		if (isSell && shouldSwapBack()) { swapBack(); }

        _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");

        uint256 amountReceived = amount;
        
        // Deduct fees unless the sender is exempt. Fees go to the contract address.
