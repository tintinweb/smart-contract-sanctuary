...
// File EpicHero/interfaces/IERC721.sol
...
// File EpicHero/libraries/SafeMathExt.sol
...
// File EpicHero/interfaces/IStrategy.sol
...
interface IStrategy {
    ...
    function deposit(address _userAddress, uint256[] memory _tokenIds) external returns (uint256);
    function withdraw(address _userAddress, uint256[] memory _tokenIds) external returns (uint256);
}


// File EpicHero/interfaces/ITreasury.sol
...
interface ITreasury {
    function claimTokens(address token, uint256 amount, address receiver) external;
}
...
// MasterChef is the master of EpicHero. He can make EpicHero and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once EPICHERO is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of EPICHERO
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accEpicHeroPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accEpicHeroPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. EPICHERO to distribute per block.
        uint256 lastRewardBlock;    // Last block number that EPICHERO distribution occurs.
        uint256 accEpicHeroPerShare; // Accumulated EPICHERO per share, times 1e12. See below.
        uint16 depositFeeBP;        // Deposit fee in basis points
        uint256 harvestInterval;    // Harvest interval in seconds
        uint8 minLevel;             // Min Level used for poolType ERC721
        uint8 minRarity;            // Min Rarity used for poolType ERC721
        uint256 totalLp;            // Total Token in Pool
        address strat;              // Contract calculate shares
    }

    struct NftOwner {
        address addr;               // user's address
        uint256 pid;                // pool Id
        uint64  index;              // _stakingHeroArray index of the array
    }
    ...
    // Owner Address => heroIds Array
    mapping (address => uint256[]) _stakingHeroArray;
    // heroId => Owner Address
    mapping (uint256 => NftOwner) _heroIdToOwnerAndIndex;
    // Total allocation points. Must be the sum of all allocation points in all pools.
    uint256 public totalAllocPoint = 0;
    // The block number when EPICHERO mining starts.
    uint256 public startBlock;
    // Total locked up rewards
    uint256 public totalLockedUpRewards;
    // Total EPICHERO in EPICHERO Pools (can be multiple pools)
    uint256 public totalEpicHeroInPools = 0;
    // EpicHero referral contract address.
    IEpicHeroReferral public epicHeroReferral;
    // Referral commission rate in basis points.
    uint16 public referralCommissionRate = 0;
    uint16 public devFee = 0;
    ...
    // Update the given pool's EPICHERO allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // View function to see pending EPICHERO on frontend.
    ...
    function massUpdatePools() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            totalReward.add(epicHeroReward.mul(devFee).div(10000));
        }
        if(totalReward > 0){
    ...
    function updatePool(uint256 _pid) public {
        ...
        safeEpicHeroTransfer(devAddress, epicHeroReward.div(10));
    }

    // Deposit LP tokens to MasterChef for EPICHERO allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(epicHeroReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            epicHeroReferral.recordReferral(msg.sender, _referrer);
        }
    ...
    // Deposit NFT to MasterChef for EPICHERO allocation.
    function depositNFT(uint256 _pid, uint256[] memory _heroIds, address _referrer) public nonReentrant {
        require(block.number >= startBlock, "MasterChef:: Can not deposit before farm start");
        ...
        if (_heroIds.length > 0 && address(epicHeroReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            epicHeroReferral.recordReferral(msg.sender, _referrer);
        }
        ...
        if (_heroIds.length > 0) {
            ...
            uint256 sharesAdded = IStrategy(pool.strat).deposit(msg.sender, _heroIds);
    ...
    function withdrawNFT(uint256 _pid, uint256[] memory _heroIds) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        require(sharesTotal > 0, "Withdraw: Shares total not enough");

        updatePool(_pid);
        payOrLockupPendingEpicHero(_pid);

        if(_heroIds.length > 0){
            uint256 sharesRemoved = IStrategy(pool.strat).withdraw(msg.sender, _heroIds);
    ...
    // Safe epicHero transfer function, just in case if rounding error causes pool do not have enough EPICHERO.
    function safeEpicHeroTransfer(address _to, uint256 _amount) internal {
        if (epicHero.balanceOf(address(treasury)) >= _amount) {
    ...
    // Update the epicHero referral contract address by the owner
    function setEpicHeroReferral(IEpicHeroReferral _epicHeroReferral) public onlyOwner {
        epicHeroReferral = _epicHeroReferral;
    }

    function retrieveTokens(address _token, uint amount) external onlyOperator {
    ...
    function retrieveBNB(uint amount) external onlyOperator{
    ...
    function emergencyRetrieveNfts(address _ntfAddress, uint256[] memory _heroIds) external onlyOperator {
        for (uint i = 0; i < _heroIds.length; i++) {
            IEpicHeroNFT(_ntfAddress).safeTransferFrom(address(this), msg.sender, _heroIds[i]);
        }
    }

    function emergencyRetrieveAllNfts(address _ntfAddress) external onlyOperator {
        IEpicHeroNFT nftContract = IEpicHeroNFT(_ntfAddress);

        uint256 balance = nftContract.balanceOf(address(this));
        for (uint i = 0; i < balance; i++) {
            uint heroId = nftContract.tokenOfOwnerByIndex(address(this), i);
            nftContract.safeTransferFrom(address(this), msg.sender, heroId);
        }
    }
    ...
    function setDevFee(uint16 _devFee) public onlyOperator {
        require(_devFee <= 10000, "Invalid devFee");
        devFee = _devFee;
    }

    // Pay referral commission to the referrer who referred this user.
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(epicHeroReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = epicHeroReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                safeEpicHeroTransfer(referrer, commissionAmount);
                epicHeroReferral.recordReferralCommission(referrer, commissionAmount);
    ...
    function updateEmissionRate(uint256 _epicHeroPerBlock) public onlyOperator {
