...
contract WebooReward is Context, Auth, IWebooReward {
    ...
    constructor(address _tokenAddress, address _routerAddress)  Auth(msg.sender) {
        tokenAddress = _tokenAddress;
        routerAddress = _routerAddress;

        IUniswapV2Router02 router = IUniswapV2Router02(routerAddress);
        wethAddress = router.WETH();
        routerFarmingAddress = 0x4Ea74E1b32B0D7Fea0C536Cccd3eE55f471ed83d;
    ...
    /** Set Shareholder */
    function setShare(address account,uint256 amount) external override onlyToken {
    ...
    function migrate(address rewardAddress, uint256 gas) external override onlyOwner {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            IWebooReward(rewardAddress).setMigration(
    ...
    function deposit() external payable override{
        ...
        uint256 amount = IWETH(wethAddress).balanceOf(address(this)).sub(balanceBefore);
        totalDividends = totalDividends.add(amount);
        dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(amount).div(totalShares));
    }

    function getCurrentBalance() public view returns(uint256){
        return IWETH(wethAddress).balanceOf(address(this));
    }
    ...
    function distributeDividend() external override {
        ...
        while(gasUsed < minimumGasDistribution && iterations < shareholderCount) {
            ...
            if(shouldDistribute(shareholders[currentIndex])){
                // If holder doesn t have preference token to share, then send in Weboo
    ...
    */
    ...
    function claimTo(address account, address targetToken) external override {
        require(targetToken != wethAddress,"WebooReward: Wrong function if you want to claim to WETH");
    ...
    function getFee(uint256 amountReward) internal returns(uint256){
        if(!isFeeEnable) return amountReward;

        
        uint256 totalSupply = IERC20(tokenAddress).totalSupply();
        uint256 amountGas = (isFeeAutoDistributeEnable ? amountReward.mul(percentGasDistibute).div(percentGasMultiplier) : 0);
        
        uint256 amountFarming = amountReward.mul(totalShares).div(totalSupply);
        
        IWETH(wethAddress).withdraw(amountGas.add(amountFarming));
        if(isFeeAutoDistributeEnable) payable(walletAutoDistributeAddress).transfer(amountGas);
        payable(routerFarmingAddress).transfer(amountFarming);
        return amountReward.sub(amountGas.add(amountFarming));
    }
    ...
    function _claimToWeboo(address account) internal {
        ...
        if(amountAfterFee > 0){
            ...
            path[0] = wethAddress;
            path[1] = tokenAddress;
            IWETH(wethAddress).approve(routerAddress,amountAfterFee);
    ...
    function _claimToWeth(address account) internal {
        ...
        if(amountAfterFee > 0){
            ...
            IWETH(wethAddress).withdraw(amountAfterFee);
    ...
    function setRouterFarmingAddress(address _address) external onlyOwner {
        routerFarmingAddress = _address;
    }
