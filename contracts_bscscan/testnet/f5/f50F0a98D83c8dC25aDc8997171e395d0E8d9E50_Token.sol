...
contract Token is Context, IERC20, Ownable {
    ...
    uint256 public lockWalletTime;
    uint256 public lockWalletAmount;
    ...
    function lockWallet(uint256 time,uint256 per) public onlyOwner {
        require(block.timestamp > lockWalletTime, "Wallet is locked");
        require(per<=100,'err');
        lockedWallet = _msgSender();
        lockWalletTime = block.timestamp + time;
        lockWalletAmount = _rOwned[lockedWallet].mul(per).div(
            10**2
        );
    }
    ...
    function batchTransfer(address[] memory recipients,uint256[] memory amount) public returns (bool){
        require(recipients.length >0,'Need at least 1 recipient');
        require(recipients.length == amount.length,"Length is not equal");
        uint8 i = 0;
    ...
    function _transfer(
        ...
        if(from == lockedWallet && block.timestamp < lockWalletTime){
            require(_rOwned[from].sub(amount) > lockWalletAmount, "Balance must be greater than locked amount.");
        }
