...
contract DSMarket is ReentrancyGuard, Ownable {
    ...
    function createMarketSale(
        ...
        // if (_amount != 0) {
        if (IERC20(_paymentToken) == dbzToken) {
            ...
            require(IERC20(dbzToken).allowance(msg.sender, address(this)) == _amount, "DSMarket: marketplace should be approved as a spender");
            
            if (platformFee > 0) {
                feeAmount = _platformPayment(priceDBZ);
                IERC20(dbzToken).safeTransferFrom(msg.sender, payable(feeRecipient), feeAmount);
            }
            
            IERC20(dbzToken).safeTransferFrom(msg.sender, tokenSeller, _amount - feeAmount);
