...
contract ExoticMonsterController is Ownable {
    ...
    uint256 public heroUniId = 0;
    uint256 public poolUniId = 0;
    ...
    constructor() {
        ...
        //初始化 五种英雄
        heros[1] = Hero(0,1,2,0,0,false);
        heros[2] = Hero(0,2,3,0,0,false);
        heros[3] = Hero(0,3,4,0,0,false);
        heros[4] = Hero(0,4,5,0,0,false);
        heros[5] = Hero(0,5,6,0,0,false);
    ...
    function inPool(uint256 poolid,uint256 amount,uint256 heroId) public {
        ...
        Pool memory newPool = pools[poolid];
        
        poolUniId++;
        
        newPool.uniId = poolUniId;
        
         //定期 
        if( 1 != poolid){
            userPools[msg.sender][newPool.uniId] = Pool(
                newPool.uniId,
                newPool.id,
                heroId,
                amount,
                //时间固定，直接计算利息
                amount.mul(newPool.rate).div(1000),
                newPool.circleTime,
                block.timestamp,
                newPool.rate
            );
    ...
    function outPool(uint256 poolId) public {
        ...
        //解锁加入矿池的英雄
        userHeros[msg.sender][userPools[msg.sender][poolId].heroId].isPooled = false;
    ...
    function _addHeroToUser(address user ) internal virtual{
       ...
       newHero.uniId = heroUniId;
    
       //更新用户矿池映射数据
       userHeros[user][newHero.uniId] = newHero;
    ...
    function _getHero() internal virtual returns (uint256) {
        ...
        uint256 number =  (uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty,msg.sender))).add(3000)) % 10000;
