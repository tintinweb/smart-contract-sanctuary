...
// File: AutoTransferSwap.sol

contract AutoTransferSwap is Ownable{
    using SafeMath for uint256;

    address public WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    address public BUSD = 0x1384561c7051509168c18C44e3Ef6545c97c8880;
    address public boxAddress = 0x31127F02210ce9119Fa436045F0898FbEca55E15;
    address public factory;
    address public mineTeamAddress;
    address public mineFoundationAddress;
    address public mineOtherAddress;

    event NewFactory(address oldFactory, address newFactory);
    event NewMineTeamAddress(address oldMineTeamAddress, address newMineTeamAddress);
    event NewMineFoundationAddress(address oldMineFoundationAddress, address newMineFoundationAddress);
    event NewMineOtherAddress(address oldMineOtherAddress, address newMineOtherAddress);
    ...
    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, 'AutoTransferSwap: EXPIRED');
        _;
    }
    ...
    function setMineTeamAddress(address _mineTeamAddress) external onlyOwner{
        address oldMineTeamAddress = mineTeamAddress;
        mineTeamAddress = _mineTeamAddress;
        emit NewMineTeamAddress(oldMineTeamAddress,_mineTeamAddress);
    }

    function setMineFoundationAddress(address _mineFoundationAddress) external onlyOwner{
        address oldMineFoundationAddress = mineFoundationAddress;
        mineFoundationAddress = _mineFoundationAddress;
        emit NewMineFoundationAddress(oldMineFoundationAddress,_mineFoundationAddress);
    }

    function setMineOtherAddress(address _mineOtherAddress) external onlyOwner{
        address oldMineOtherAddress = mineOtherAddress;
        mineOtherAddress = _mineOtherAddress;
        emit NewMineOtherAddress(oldMineOtherAddress,_mineOtherAddress);
    }
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'AutoTransferSwap: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function _removeLiquidity(
        ...
        require(amountA >= amountAMin, 'AutoTransferSwap: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'AutoTransferSwap: INSUFFICIENT_B_AMOUNT');
    }

    function removeLiquidity(address[] calldata pairs, address to, uint256 deadline) external ensure(deadline)  {
        ...
        swapToken(allTokens,to);
    }

    function swapToken(address[] memory allTokens, address to) internal {
        ...
        for (uint256 tokenId = 0; tokenId < tokenLength; ++tokenId) {
            ...
            if (token == address(0) || token == WBNB || token == BUSD) continue;
    ...
    function swapTargetPair(uint256 _balance, address _token) internal returns(bool) {
        ...
        pair = IBoxswapFactory(factory).getPair(_token, BUSD);
        if (pair != address(0)) {
            path[0] = _token;
            path[1] = BUSD;
            swapExactTokensForTokens(_balance, 0, path, address(this));
    ...
    function swapBusdForWBnb(address to) internal {
        uint256 busdBalance = IERC20(BUSD).balanceOf(address(this));
        if (busdBalance > 0) {
            return;
        }
        address pair = IBoxswapFactory(factory).getPair(BUSD, WBNB);
        if (pair == address(0)) {
            IERC20(BUSD).transfer(to, busdBalance);
            return;
        }
        address[] memory path = new address[](2);
        path[0] = BUSD;
        path[1] = WBNB;
        swapExactTokensForTokens(busdBalance, 0, path, address(this));
    }

    function swapWBnbForBox(address to) internal {
        uint256 WBnbBalance = WETH9(WBNB).balanceOf(address(this));
        if (WBnbBalance > 0) {
            // 60%
            uint256 targetAmount = WBnbBalance.mul(6).div(10);
            uint256 bnbAmount = WBnbBalance.mul(4).div(10);
            WETH9(WBNB).withdraw(bnbAmount);
            TransferHelper.safeTransferETH(to, bnbAmount);
            
            address[] memory path = new address[](2);
            // WBNB,boxAddress
            path[0] = WBNB;
            path[1] = boxAddress;
            swapExactTokensForTokens(targetAmount, 0, path, to);
        }
    }

    function transferMineFeeBox(uint256 amount) external {
        uint256 boxBalance = IERC20(boxAddress).balanceOf(msg.sender);
        require(amount <= boxBalance,'AutoTransferSwap: INSUFFICIENT_boxBalance');
        ...
        IERC20(boxAddress).transferFrom(msg.sender, mineTeamAddress, teamAmount);
        IERC20(boxAddress).transferFrom(msg.sender, mineFoundationAddress, foundationAmount);
        IERC20(boxAddress).transferFrom(msg.sender, mineOtherAddress, otherAmount);
