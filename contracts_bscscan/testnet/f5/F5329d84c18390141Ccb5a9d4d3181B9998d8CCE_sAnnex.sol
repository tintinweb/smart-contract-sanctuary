...
import "./interfaces/IgANN.sol";
import "./interfaces/IsANN.sol";
import "./interfaces/IStaking.sol";

contract sAnnex is IsANN, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgANN public gANN; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked ANN", "sANN", 9) ERC20Permit("Staked ANN") {
    ...
    function setgANN(address _gANN) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gANN) == address(0), "gANN:  gANN already set");
        require(_gANN != address(0), "gANN:  gANN is not a valid contract");
        gANN = IgANN(_gANN);
    }
    ...
    /**
        @notice increases rANN supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sANN of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sANN
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sANN: insufficient balance");
    }
    ...
    // toG converts an sANN balance to gANN terms. gANN is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gANN.balanceTo(amount);
    }

    // fromG converts a gANN balance to sANN terms. sANN is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gANN.balanceFrom(amount);
    }

    // Staking contract holds excess sANN
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gANN.balanceFrom(IERC20(address(gANN)).totalSupply())).add(
