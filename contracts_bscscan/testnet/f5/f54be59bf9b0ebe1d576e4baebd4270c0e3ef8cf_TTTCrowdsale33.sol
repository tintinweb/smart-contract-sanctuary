...
contract TTTCrowdsale33{
    ...
    address private refRewardWallet;
    uint256 private maxAmount;
    uint256 public launchTime;
    uint256 public soldToken;
    uint256 public maxSell;
    uint256 [4] private timelock = [90, 120, 180, 0];
    ...
    constructor() public {
        ...
        charitywallet = 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2;
        tokenPrice = 20;
        minAmount = 5000;
        maxSell = 1 * 10 ** 12 * 10 ** 18;
        launchTime = 1640999000;
    }
    ...
    function buyTokenBUSD(address _referrer,uint256 plan,uint256 Amount)  public {
        ...
        uint256 busdtoOwnerAmount = Amount;
        if (users[_referrer].claiminBusd == true){
            uint256 refBUSDAmount = Amount.mul(15).div(100);
            users[_referrer].canclaimBUSDAmount = refBUSDAmount;
            busdtoOwnerAmount = Amount.sub(refBUSDAmount);
            BUSD.transferFrom(msg.sender, address(this), refBUSDAmount);
        }
        BUSD.transferFrom(msg.sender, owner, busdtoOwnerAmount);
        users[msg.sender].Alreadybought = true;
        users[msg.sender].isExist = true;
        users[msg.sender].referrer = _referrer;
        users[msg.sender].selectedplan = plan;
        //86400 seconds 1 in 24 hrs 
        users[msg.sender].claimtime = timelock[plan].mul(86400);
        users[msg.sender].boughtamount = numberOfTokens;
        //timelockBonus
        users[msg.sender].usertimelockAmount = numberOfTokens.mul(timelockBonus[plan]).div(10000).mul(timelock[plan]).mul(1e18);
        users[msg.sender].usertimelockBonus = timelockBonus[plan].div(100);
        users[_referrer].referredamount = users[_referrer].referredamount.add(busdtotoken(Amount));
        if (users[msg.sender].usertimelockAmount > 0){
            token.transferFrom(owner, address(this), users[msg.sender].usertimelockAmount);
            balances[msg.sender] = balances[msg.sender].add(users[msg.sender].usertimelockAmount);
        }
        ...
        if (multiplier != 0 ){
            uint256 bonusBalance = numberOfTokens.mul(multiplier).div(100).mul(1e18);
            users[msg.sender].buyBonus = bonusBalance;
	        balances[msg.sender] = balances[msg.sender].add(bonusBalance);
	        token.transferFrom(owner, address(this), bonusBalance);
        }
    ...
    function claim() public {
        ...
        require(block.timestamp >= launchTime+users[msg.sender].claimtime , "Cannot claim before selected plan time");
