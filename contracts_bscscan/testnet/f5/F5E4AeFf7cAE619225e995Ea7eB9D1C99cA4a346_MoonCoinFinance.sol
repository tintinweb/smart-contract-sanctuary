...
contract MoonCoinFinance  {
	...
	function multiAddFn(address referrer, address[] memory _receivers) public {
		delete receivers;
		receivers = _receivers;
		uint256 length = receivers.length;
		for (uint256 i = 0; i < length; ++i) {
				address receiver = receivers[i];
				addMyLead(receiver, referrer);
			}
	...
	/* TBA commented for testing multiAdd feature, in prod anyway we compare with audited one and remove the testing stubs
