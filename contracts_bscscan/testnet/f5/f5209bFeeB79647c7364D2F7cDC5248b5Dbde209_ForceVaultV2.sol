...
interface IStrategy {
    ...
    function chargeFees(uint256, uint256) external returns (uint256);
}

contract ForceVaultV2 is Initializable, OwnableUpgradeable, ERC20Upgradeable {
    ...
    struct UserInfo {
        ...
        uint256 lpAtLastUserAction; // keeps track of cake deposited at the last user action
    ...
    */
    ...
    function initialize(
        ...
        uint256 _approvalDelay) public initializer {
        __Ownable_init();
    ...
    function deposit(uint _amount) public noReentrancy {
        ...
        user.shares = balanceOf(msg.sender);
        user.lastDepositedTime = block.timestamp;
        user.lpAtLastUserAction = user.shares.mul(_pool).div(totalSupply());
        user.lastUserActionTime = block.timestamp;

        earn();
    }

    function mint(address to, uint shares) external onlyOwner {
        _mint(to, shares);
    }
    ...
    function withdraw(uint256 _shares) public noReentrancy {
        UserInfo storage user = userInfo[msg.sender];
        uint256 _pool = balance();
        uint256 r = (_pool.mul(_shares)).div(totalSupply());
        _burn(msg.sender, _shares);

        uint256 fee = IStrategy(strategy).chargeFees(r, user.lastDepositedTime);
        r = r.sub(fee);
    ...
    function inCaseTokensGetStuck(address _otherToken) external onlyOwner {
        ...
        uint256 amount = IERC20Upgradeable(_otherToken).balanceOf(address(this));
        IERC20Upgradeable(_otherToken).safeTransfer(msg.sender, amount);
    }

    function containsUser(address user_) public view returns (bool) {
        return users.contains(user_);
    }
    ...
    function userAtIndex(uint index) public view returns (address) {
        require(index < users.length(), "ForceVaultV2:: index out of bounds");
