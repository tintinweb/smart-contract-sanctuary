...
// hbc 0xCCCbFF4ECE7E19D3fa12811E54536b2242807B09 invest bnb
...
contract HBCICO 
{
    ...
    function _invest(uint256 _planId,address referrer,uint256 withType) public{
        ...
        if(msg.sender!=owner)
        {
            ...
            uint256 _directincome= _amount*ref_percent/1000;
            hbcToken.transfer(users[msg.sender].referrer,_directincome);
            emit onDirectIncome(users[msg.sender].referrer,msg.sender,_directincome,block.timestamp);
        }
    ...
    function _calculatePoolReward(address _referrer,address  _from) private 
    {
        ...
        uint256 autop=13000000000000000000;
         ...
         while(i<5)
         {
            hbcToken.transfer(_referrer,(autop.div(tokenPrice)).mul(1e18)); 
            emit onPoolIncome(_referrer,  _from, (autop.div(tokenPrice)).mul(1e18), block.timestamp);
