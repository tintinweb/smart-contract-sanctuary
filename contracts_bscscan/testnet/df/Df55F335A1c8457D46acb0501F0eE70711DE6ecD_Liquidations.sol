...
contract Liquidations is Owned, MixinSystemSettings, ILiquidations {
    ...
    function usersFlaggedForLiquidation(uint256 offset, uint256 limit) external view returns (uint256 total, address[] memory users) {
        return liquidationsState().usersFlaggedForLiquidation(offset, limit);
    }

    // /**
    //  * @notice Query users available for liquidation.
    //  * @notice This API uses pagination.
    ...
    function usersAvailableForLiquidation(uint256 offset, uint256 limit) external view returns (address[] memory users) {
        ...
        while (true) {
            ...
            (, addresses) = liquidationsState().usersFlaggedForLiquidation(currentOffset, 100);
            currentOffset += 100;

            // no more addresses available
            // `result` is longer than `limit` so we copy it into a smaller array
            if (addresses.length == 0) {
                users = new address[](resultCount);
                for (uint256 ii = 0; ii < resultCount; ii++) { users[ii] = result[ii]; }
                return users;
            }

            for (uint256 ii = 0; ii < addresses.length; ii++) {
                // skip if not available for liquidation
                if (!this.isOpenForLiquidation(addresses[ii])) {
                    continue;
                }
                ...
                // skip offset
                if (readyCount <= offset) {
                    continue;
                }

                result[resultCount] = addresses[ii];
                resultCount += 1;
    ...
    // /**
    //  * @notice Query users available for liquidation.
    //  * @notice This API will collect a "page" of flagged accounts and filter out the ones not available for liquidation.
    //  */
    function usersAvailableForLiquidationInPage(uint256 offset, uint256 limit) external view returns (uint256 total, address[] memory users) {
        address[] memory addresses;
        (total, addresses) = liquidationsState().usersFlaggedForLiquidation(offset, limit);
        ...
        users = new address[](readyCount);
        uint256 current = 0;

        for (uint256 ii = 0; ii < addresses.length; ii++) {
            if (this.isOpenForLiquidation(addresses[ii])) {
                users[current] = addresses[ii];
                current += 1;
            }
        }
    }
