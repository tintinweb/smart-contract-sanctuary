...
contract Treasury is ITreasury, EmergencyWithdrawable {
    ...
    IStarlinkEngine public starlinkEngine;

    uint256 public investMagnitude = 800;
    uint256 public engineGasMagnitude = 100;
    uint256 public amountPendingInvestment;
    uint256 public amountPendingEngineGas;

    uint256 public nextFundsAllocationDate;
    uint256 public fundsAllocationPeriod = 30 minutes;

    event Deposited(address from, uint256 bnbAmount, address tokenAddress, uint256 tokenAmount);

    constructor(IInvestor _investor, IStarlinkEngine _starlinkEngine) {
        setInvestor(_investor);
        setStarlinkEngine(_starlinkEngine);
    }

    function deposit(address tokenAddress, uint256 amount) external override payable {
        require(msg.value > 0 || amount > 0, "Treasury: Invalid amounts");

        if (amount > 0) {
            IBEP20 token = IBEP20(tokenAddress);
            require(token.allowance(msg.sender, address(this)) >= amount, "Treasury: Not allowed");
            token.transferFrom(msg.sender, address(this), amount);
        }

       onDeposit(msg.sender, msg.value, tokenAddress, amount);
    }

    function onDeposit(address from, uint256 bnbAmount, address tokenAddress, uint256 tokenAmount) private {
        amountPendingInvestment += bnbAmount * investMagnitude / 1000;
        amountPendingEngineGas += bnbAmount * engineGasMagnitude / 1000;

        if (block.timestamp >= nextFundsAllocationDate) {
            nextFundsAllocationDate = block.timestamp + fundsAllocationPeriod;
            doAllocateFunds();
        }

        emit Deposited(from, bnbAmount, tokenAddress, tokenAmount);
    }

    function doAllocateFunds() private {
        investor.deposit{value: amountPendingInvestment}();
        starlinkEngine.donate{value: amountPendingEngineGas}();

        delete amountPendingInvestment;
        delete amountPendingEngineGas;
    }

    function setInvestor(IInvestor _investor) public onlyOwner {
        require(address(_investor) != address(0), "Treasury: Invalid address");
        investor = _investor;
    }

    function setStarlinkEngine(IStarlinkEngine _starlinkEngine) public onlyOwner {
        require(address(_starlinkEngine) != address(0), "Treasury: Invalid address");
        starlinkEngine = _starlinkEngine;
    }

    function setAllocationMagnitudes(uint256 invest, uint256 engineGas) external onlyOwner {
        require(invest + engineGas <= 1000, "Treasury: Out of range");
