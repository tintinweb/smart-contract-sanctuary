...
contract Bucket2D {
  ...
  function storePixels(pixelInput[] memory pixelInputs) external payable {
    ...
    _storeBucket(prevBucket, loadBucket);
    emit pixelsCommitted(pixelInputs);
  }
  ...
  function getFlatBuckets(uint256 fromBucket, uint256 toBucket) public view returns(uint16[16384] memory) {
    require(toBucket - fromBucket < 1024, "Too many buckets");
    ...
    while (i < toBucket) {
      ...
      while (j < 16) {
        uint256 index = ((i*16) + j);
        bucketRange[index] = thisBucket[j];
        j++;
      }
      j=0;
      i++;
