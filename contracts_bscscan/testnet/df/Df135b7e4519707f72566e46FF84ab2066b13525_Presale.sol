...
contract Presale is Whitelist {
    ...
    //uint256 public constant hardCap = 150000000000000000000000;
    uint256 public constant hardCap = 10000000000000000000; // testnet 10 busd hardcap
    ...
    function getTotalContribution(address account) public view returns (uint256){
    ...
    function contribute(uint256 amount) public active onlyWhitelisted {
        require(
            contribution[msg.sender] <= whitelist[msg.sender].maxContribution,
            "Maximum amount contributed"
        );
        require(
            contribution[msg.sender] + amount <=
                whitelist[msg.sender].maxContribution,
            "Contribution goes over limit"
        );
        require(
            baseToken.balanceOf(address(this)) <= hardCap,
            "Hard Cap reached"
        );
        
        baseToken.transferFrom(msg.sender, address(presaleWallet), amount);
        contribution[msg.sender] += amount;
...
contract Whitelist is Owned {
    ...
    modifier onlyWhitelisted() {
        require(whitelist[msg.sender].active, 'not whitelisted');
        _;
    }
     ...
     */
    function addAddressToWhitelist(address addr, uint256 maxContribution) public ownerOnly() returns(bool success) {
        if (!whitelist[addr].active) {
            whitelist[addr].active = true;
            whitelist[addr].maxContribution = maxContribution;
     ...
     */
    function addAddressesToWhitelist(address[] calldata addrs, uint256 maxContribution) public ownerOnly() returns(bool success) {
        for (uint256 i = 0; i < addrs.length; i++) {
            if (addAddressToWhitelist(addrs[i], maxContribution)) {
                success = true;
            }
        }
    }
