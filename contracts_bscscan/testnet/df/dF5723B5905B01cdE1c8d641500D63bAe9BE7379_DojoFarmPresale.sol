...
contract DojoFarmPresale is ReentrancyGuard {
    ...
    // Initial Price of presale DOJO, 2 BUSD
    uint256 public BUSDPerToken = 2;
    // Amount of BUSD received in presale
    uint256 busdReceived = 0;
    // BUSD token
    IBEP20 BUSD;
    // BUSD Address
    address busdAddress= 0x09784e1FA632cEb95943e0964b1326D565aF8424;
    ...
    constructor (address _DOJO, uint256 _startingTimestamp) public {
        DOJO = IBEP20(_DOJO);
    ...
    function buy (uint256 _amount, address beneficiary) public nonReentrant {
        ...
        if (totalTokensSold > 20000 ether){ //PRE1
            BUSDPerToken = 3; // tier 2 of presale
        } 
        
        if (totalTokensSold > 45000 ether){
            BUSDPerToken = 4; // tier 3 of presale
        }

        uint256 tokens = _amount.div(BUSDPerToken);

        require (busdReceived +  _amount <= 235000 ether, "Presale hardcap reached");
    ...
    function getDOJOTokensLeft() external view returns (uint256) {
        return DOJO.balanceOf(address(this));
    }

    function claimTokens (address claimer) external {
        require (isClaimActive, "Claim is not allowed yet");
        require (tokensOwned[msg.sender] > 0, "User should own some DOJO tokens");
        require (tokensUnclaimed[msg.sender] > 0, "User should have unclaimed DOJO tokens");
        require (DOJO.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough DOJO tokens to transfer.");

        DOJO.safeTransfer(msg.sender, tokensUnclaimed[msg.sender]);
    ...
    function withdrawUnsoldDOJO() external onlyOwner {
        uint256 amount = DOJO.balanceOf(address(this)) - totalTokensSold;
        DOJO.safeTransfer(msg.sender, amount);
    }

    function withdrawAllDOJO() external onlyOwner {
        DOJO.safeTransfer(msg.sender, DOJO.balanceOf(address(this)));
    }
}
