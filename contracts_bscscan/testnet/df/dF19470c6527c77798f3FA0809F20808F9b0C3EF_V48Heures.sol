...
contract V48Heures is IERC20 {
    ...
    uint256 private constant startingSupply = 48_000_000_000; // 48 billion, underscores aid readability

    uint8 private _decimals = 9;
    uint256 private _decimalsMul = _decimals;
    uint256 private _tTotal = startingSupply * (10 ** _decimalsMul);

    string constant _name = "V48Heures";
    string constant _symbol = "V48Heures";
    ...
    uint256 private _buyBackFees = 0;
    uint256 public _buyGivewayFee = 100;

    uint256 private _sellReflectionFee = 0;
    uint256 public _sellLiquidityFee = 2300;
    uint256 public _sellMarketingFee = 800;
    uint256 private _sellBuyBackFee = 0;
    uint256 public _sellGivewayFee = 900;

    uint256 private maxReflectionFee = 0;
    uint256 public maxLiquidityFee = 2300;
    uint256 public maxMarketingFee = 1000;
    uint256 private maxBuyBackFee = 0;
    uint256 public maxGivewayFee = 1000;
    ...
    uint256 private previousGivewayFee = _GivewayFee;
    ...
    uint256 private GivewayRatio = _sellGivewayFee;
    ...
    //Set this
    address payable private buyBackAddress = payable (0xbFD7c47b145D797cc7D0099A9d9A11b096952e6F);
    address private ZERO = 0x0000000000000000000000000000000000000000;

    address payable private _marketingWallet = payable (0xbFD7c47b145D797cc7D0099A9d9A11b096952e6F); //Put your marketing address here
    address payable private _GivewayWallet = payable (0xbFD7c47b145D797cc7D0099A9d9A11b096952e6F);// Enter Giveway wallet here also
    ...
    // Maximum wallet size is 0.48% of the total supply.
    uint256 private maxWalletPercent = 48; // Less fields to edit
    ...
    uint32 public cooldownTimerInterval = 0.8 minutes;
    ...
    function setBuyTaxes(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee, uint256 buyBackFees, uint256 GivewayFee) external onlyOwner {
        ...
        require(liquidityFee + reflectionFee + marketingFee + GivewayFee + _buyBackFees <= 5000);
    ...
    function setSellTaxes(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee, uint256 buyBackFee, uint256 GivewayFee) external onlyOwner {
        require(liquidityFee <= maxLiquidityFee
                && reflectionFee <= maxReflectionFee
                && marketingFee <= maxMarketingFee
                && GivewayFee <= maxGivewayFee
                && buyBackFee <= maxBuyBackFee);
        require(liquidityFee + reflectionFee + marketingFee + GivewayFee + buyBackFee <= 5000);
    ...
    function setRatios(uint256 reflection, uint256 liquidity, uint256 marketing, uint256 Giveway) external onlyOwner {
        reflectionRatio = reflection;
        liquidityRatio = liquidity;
        marketingRatio = marketing;
        GivewayRatio = Giveway;
    }
    ...
    function setGivewayWallet(address payable newWallet) external onlyOwner {
        require(_GivewayWallet != newWallet, "Wallet already set!");
        _GivewayWallet = payable(newWallet);
    }
    ...
    function getTotalFee() public view returns (uint256) {
        return _reflectionFee + _liquidityFee + _marketingFee + _GivewayFee;
    }
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        uint256 swapTotalFee = reflectionRatio + liquidityRatio + marketingRatio + GivewayRatio;
    ...
    function transferBNBOut(uint256 amount) internal {
        uint256 amountBNBMarketing = (amount * marketingRatio) / (marketingRatio + GivewayRatio);
        uint256 amountBNBGiveway = amount - amountBNBMarketing;
        _marketingWallet.transfer(amountBNBMarketing);
        _GivewayWallet.transfer(amountBNBGiveway);
    }
