...
contract DecBUSDNew {
	...
	address private tokenAddr = 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee; // BUSD
	IERC20 public token;

	uint256 constant public INVEST_MIN_AMOUNT = 0.001 ether; // 1 BUSD
	uint256[] public REFERRAL_PERCENTS = [70, 30, 20];
	uint256 constant public TOTAL_REF = 120;
	uint256 constant public CEO_FEE = 90;
	uint256 constant public DEV1_FEE = 10;
	...
	constructor(address payable ceoAddr, address payable dev1Addr, address payable dev2Addr, uint256 start) public {
		require(!isContract(ceoAddr) && !isContract(dev1Addr) && !isContract(dev2Addr));
		ceoWallet = ceoAddr;
		dev1Wallet = dev1Addr;
		dev2Wallet = dev2Addr;
	...
	function invest(address referrer, uint8 plan , uint256 amount) public {
		...
		uint256 d1Fee = amount.mul(DEV1_FEE).div(PERCENTS_DIVIDER);
		uint256 d2Fee = amount.mul(DEV2_FEE).div(PERCENTS_DIVIDER);
		token.safeTransfer(ceoWallet, ceo);
		token.safeTransfer(dev1Wallet, d1Fee);
		token.safeTransfer(dev2Wallet, d2Fee);
