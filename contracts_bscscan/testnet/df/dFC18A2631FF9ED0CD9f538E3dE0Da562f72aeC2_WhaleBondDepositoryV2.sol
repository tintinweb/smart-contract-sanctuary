...
/// @title Whale Bond Depository V2
/// Forked from: Olympus DAO

contract WhaleBondDepositoryV2 is IBondDepository, NoteKeeper {
/* ======== DEPENDENCIES ======== */
...
/* ======== CONSTRUCTOR ======== */
  ...
  constructor(
    IWhaleAuthority _authority,
    IERC20 _hump,
    IStaking _staking,
    ITreasury _treasury
  ) NoteKeeper(_authority, _hump, _staking, _treasury) {
    // save gas for users by bulk approving stake() transactions
    _hump.approve(address(_staking), 1e45);
  }
...
/* ======== DEPOSIT ======== */
   ...
   * @return payout_     the amount of HUMP due
  ...
  function deposit(
     ...
     * price = quote tokens : hump (i.e. 42069 DAI : HUMP)
     *
     * 1e18 = HUMP decimals (9) + price decimals (9)
     */
     ...
     *
     * this is either the number of HUMP that the market can sell
    ...
    // purchased, and how much HUMP has been sold
  ...
  function _tune(uint256 _id, uint48 _time) internal {
    ...
    if (_time >= meta.lastTune + meta.tuneInterval) {
      ...
      // hump decimals (9) + price decimals (9)
       ...
       * is 10,000 HUMP. max payout would be 1,000 HUMP (10,000 * 1 / 10).
       */  
...
/* ======== CREATE ======== */
   ...
   * @param _market      [capacity (in HUMP or quote), initial price / HUMP (9 decimals), debt buffer (3 decimals)]
  ...
  function create(
     ...
     *
     * 1e18 = hump decimals (9) + initial price decimals (9)
     */
     ...
     * interval. for example, if capacity is 1,000 HUMP, there are 10 days to conclusion, 
     * and the preferred deposit interval is 1 day, max payout would be 100 HUMP.
     */
    ...
    emit CreateMarket(id_, address(hump), address(_quoteToken), _market[1]);
  }
...
/* ======== EXTERNAL VIEW ======== */
   ...
   * @param _id          ID of market
   * @return             price for market in HUMP decimals
   *
   ...
   * @param _id          ID of market
   * @return             amount of HUMP to be paid in HUMP decimals
   *
   * @dev 1e18 = hump decimals (9) + market price decimals (9)
   */
   ...
   * @param _id          ID of market
   * @return             current debt for market in HUMP decimals
   */
   ...
   * @param _id          ID of market
   * @return             control variable for market in HUMP decimals
   */
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
  ...
  constructor (
    ...
    IStaking _staking,
    ITreasury _treasury
  ) FrontEndRewarder(_authority, _hump) {
    staking = _staking;
    treasury = _treasury;
  }
...
/* ========== ADD ========== */
   ...
   * @param _payout      the amount of HUMP due to the user
  ...
  function addNote(
    ...
    // note that only the payout gets staked (front end rewards are in HUMP)
...
/* ========== REDEEM ========== */
   ...
   * @return payout_     sum of payout sent, in HUMP
   */
  function redeem(address _user, uint256[] memory _indexes) public override returns (uint256 payout_) {
    ...
    staking.unwrap(_user, payout_); // unwrap and send payout as sHUMP
  }
   ...
   * @return             sum of payout sent, in HUMP
   */ 
...
/* ========== VIEW ========== */
   ...
   * @return payout_     the payout due, in HUMP
...
abstract contract FrontEndRewarder is WhaleAccessControlled {
  ...
  IERC20 internal immutable hump; // reward token

  constructor(
    IWhaleAuthority _authority, 
    IERC20 _hump
  ) WhaleAccessControlled(_authority) {
    hump = _hump;
  }
  ...
  function getReward() external {
    ...
    hump.transfer(msg.sender, reward);
  }
...
interface INoteKeeper {
  ...
  struct Note {
    uint256 payout; // HUMP remaining to be paid
