...
contract BlindBox {
    ...
    uint256 counter;
      for(uint256 i = 0; i < 3; i++){
       for (uint256 j = 0; j < 2 || (i ==2 && j < 2); j++) {
          ...
          uint256 randNumber =  (uint256(keccak256(abi.encode(randomValueMain, (block.timestamp + counter++ )))) % (i ==2 ? 10 : 10));
  ...
  function buyBox(address seller, string memory ownerId, string memory name, uint256 selectedTime, bool isPaid) public {
    uint256 dayCount = ( block.timestamp - boxDetails[name].initialize)/(60 * 12);
    require(Category[name].count > 0, "No NFT");
    require(!boxDetails[name].usedCountByday[dayCount].isSold,"Box soldout");
    require(boxDetails[name].initialize < block.timestamp && (boxDetails[name].initialize + (boxDetails[name].initialize * dayCount)) + 8 minutes <= block.timestamp , "Box not available");
  ...
  function isBoxSold(uint256 selectedTime, string memory name) public view returns(bool) {
    ...
    uint256 dayCount = ( currentTime - boxDetails[name].initialize)/(60 * 12);
    
    return Category[name].count == 0 || boxDetails[name].usedCountByday[dayCount].isSold || boxDetails[name].initialize >= currentTime || !(boxDetails[name].dailiyLimit > 0 && dayCount < Category[name].count / boxDetails[name].dailiyLimit) ||  (boxDetails[name].initialize + (boxDetails[name].initialize * dayCount)) + 8 minutes <= currentTime ;
  }
  function isSleepTime(uint256 selectedTime, string memory name) public view returns(bool) {
      uint256 currentTime = block.timestamp;
    uint256 dayCount = ( currentTime - boxDetails[name].initialize)/(60 * 12);
    
    return  (boxDetails[name].initialize + (boxDetails[name].initialize * dayCount)) + 8 minutes <= currentTime ;
