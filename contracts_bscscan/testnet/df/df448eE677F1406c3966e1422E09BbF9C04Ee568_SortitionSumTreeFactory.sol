...
contract PotStableCoinLover is PotController {//Valcontroller
    ...
    uint private _currentInvestedUndelying;
    ...
    function deposit(uint amount) public onlyValidState(PotConstant.PotState.Opened) onlyValidDeposit(amount) {
        ...
        stakingToken.safeTransferFrom(account, address(VENUS_STRATEGY), amount);
        VENUS_STRATEGY.doHardWork();
    ...
    function depositDonation(uint amount) public onlyWhitelisted {
        stakingToken.safeTransferFrom(msg.sender, address(VENUS_STRATEGY), amount);
        VENUS_STRATEGY.doHardWork();
    ...
    function closePot() external onlyOwner onlyValidState(PotConstant.PotState.Opened) {
        ...
        _currentInvestedUndelying = VENUS_STRATEGY.currentInvestedUnderlyingBalance();
    }
    ...
    function _overCook() private {
        ...
        _totalHarvested = VENUS_STRATEGY.currentInvestedUnderlyingBalance().sub(_currentInvestedUndelying);

        uint buyback = _totalHarvested.mul(burnRatio).div(100);
        _totalHarvested = _totalHarvested.sub(buyback);
        if (buyback > 0) {
            VENUS_STRATEGY.withdraw(buyback, address(this));
        }
