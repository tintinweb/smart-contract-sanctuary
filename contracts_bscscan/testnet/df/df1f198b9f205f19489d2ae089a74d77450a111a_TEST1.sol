...
contract TEST1 is IBEP20, Auth {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[sender] && recipient == bankAddress){ try bank.setShare(sender, amountReceived) {} catch {} }        
        
        checkMarketCondition();
        recalculationMaxTokenToPrint();	

        if(bank.shouldEstablishPool()){ try bank.establishPool() {} catch {} }
...
//        emit Transfer(sender, recipient, amount);
    ...
    function shouldPrintTokens() internal view returns (bool) {
    ...
    function triggerAutoBuyback() internal {
            buyTokens(autoBuybackAmount, burnerAddress); 
    ...
    function editBank(address newAddress) external authorized {
    ...
    function _print(address account, uint256 amount) internal {
        ...
        emit Printed(account, amount);
    }
	
    function _printToken(address account, uint256 amount) internal {
        require(account == bankAddress, "Only Bank Of TEST1 can print tokens");
		if(shouldPrintTokens()){ _print(account, amount); }
    }	
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        emit SentToHell(account, amount);
    }
    ...
    function provideLP() internal {
        ...
        uint256 adjustedIDOprice = _totalSupply / amountBNBLiquidity;
        uint256 amountTokenLiquidity = amountBNBLiquidity * adjustedIDOprice.mul(125).div(100);
        ...
        lockLPTime = block.timestamp + 0 days; //lock LP for 400 days
        launch();
    ...
    function IDO(address referrer) external payable returns(bool) {
        ...
        uint256 adjustedIDOprice = _totalSupply / address(this).balance;
        IDOprice = IDOprice.mul(adjustedIDOprice).div(IDOprice);
        }
        uint256 amount = (msg.value * IDOprice);

        if(balanceOf(referrer) >= 2000 * 10**18){
            payable(referrer).transfer(msg.value / 50);	
    ...
    function IDOSettings(uint256 hardCap_, uint256 IDOduration) external authorized {
        hardCap = hardCap_;
        IDOendDate = block.timestamp + IDOduration * 1 days; 
    }
    ...
    function feeInfo() public view returns (uint256 Liquidity_fee, uint256 Buyback_fee, uint256 Dividend_fee, uint256 Airdrop_fee, uint256 Total_fee, uint256 totalFeeForBuyers, uint256 totalFeeForSellers) {
        return (liquidityFee, burnFee, dividendFee, hedgeFundFee, totalFee, getTotalFee(pair, msg.sender), getTotalFee(msg.sender, pair));
    }    
    ...
    event printedTokens(address buyer, uint256 amountCRY);
    event Printed(address printer, uint256 amountCRY);
    event SentToHell(address account, uint256 amountCRY);
