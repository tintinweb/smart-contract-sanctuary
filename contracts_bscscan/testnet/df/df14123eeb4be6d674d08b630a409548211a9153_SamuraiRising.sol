...
contract SamuraiRising is ERC721Enumerable, RSunAuth {
    ...
    constructor() ERC721("SamuraiRising", "Samurai Rising") RSunAuth(msg.sender) {
        ...
        addCardSet(200, 1000, 10);
        addCardSet(40,  2000, 5);

        addPack(100 * 15  * 10 ** 11, 1,  true, 0);
        addPack(97  * 75  * 10 ** 11, 5,  true, 0);
        addPack(94  * 150 * 10 ** 11, 10, true, 0);
        addPack(91  * 225 * 10 ** 11, 15, true, 0);
        addPack(85  * 375 * 10 ** 11, 25, true, 0);

        addPack(40  * 10 ** 14, 1, true, 1);
        addPack(180 * 10 ** 14, 5, true, 1);

        addAttribute("name", 10 ** 15, rsunAdr);
        addAttribute("desc", 20 ** 15, rsunAdr);

        // setFeeReceivers([address(0x2921dd9580eE924A5a995Bf2606D537affDD5f2f), address(0x3011bA590a78e61E461BF953954A4402ae1dA7a0), address(0x884F86Bdb9766D9B30895bc9CF02A918BF267dda)], [10, 20, 70]);
    }
    ...
    function setRsunAddress(address newAdr) external {
        authorizedFor(Permission.AdjustVariables);
        rsunAdr = newAdr;
        rsun = IBEP20(newAdr);
    }
    ...
    function setFeeReceivers(address[] calldata receivers, uint96[] memory weights) public {
        authorizedFor(Permission.AdjustVariables);
