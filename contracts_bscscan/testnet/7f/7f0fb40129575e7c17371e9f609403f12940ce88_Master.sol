...
contract Master is Ownable {
    ...
    // Market oracle provides the GRV/USD exchange rate as an 18 decimal fixed point number.
    ...
    constructor(address _grv) public {
    ...
    function rebase() public {
        ...
        uint256 supplyAfterRebase = grv.rebase(epoch, supplyDelta);
    ...
    function adjustTargetRate() external
    {
      ...
      if (exchangeRate > targetRate){
        targetRate = exchangeRate.mul(8114135).div(8000000);
      }

    }

    // increment by 1.0142669 to achieve 12% every 8hrs
    function incrementTargetRate() internal {

      uint256 newRate = targetRate.mul(8114135).div(8000000);
    ...
    function getRebaseValues() public view returns (uint256, int256) {
        ...
        if (supplyDelta > 0 && grv.totalSupply().add(uint256(supplyDelta)) > MAX_SUPPLY) {
            supplyDelta = (MAX_SUPPLY.sub(grv.totalSupply())).toInt256Safe();
        }
        ...
        returns (int256)
    {
        ...
        return grv.totalSupply().toInt256Safe()
