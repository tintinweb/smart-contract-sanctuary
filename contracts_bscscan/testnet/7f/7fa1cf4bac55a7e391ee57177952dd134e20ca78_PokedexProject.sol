...
// File: contracts/PokedexProjectDividendPayingToken.sol
...
// File: contracts/PokedexProject.sol
pragma solidity ^0.7.6;

contract PokedexProject is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public buyTokenRewardsFee = 4;
    ...
    uint256 public tradingEnabledTimestamp = 1627909999;
    ...
    constructor() public ERC20("PokedexProject", "Wartortle") {
        // LP address receiver 
        burnAddress = address(0x8ceD7Db1f55b87dEC72D78a9A3e339aD71942d4F);
        uint256 _processDividendTime = block.timestamp;
        processDividendTime = _processDividendTime;

        dividendTracker = new PokedexProjectDividendTracker();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "PokedexProject: The dividend tracker already has that address");

        PokedexProjectDividendTracker newDividendTracker = PokedexProjectDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "PokedexProject: The new dividend tracker must be owned by the PokedexProject token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "PokedexProject: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "PokedexProject: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "PokedexProject: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PokedexProject: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PokedexProject: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PokedexProject: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "PokedexProject: This account cannot send tokens until trading is enabled");
        }
...
contract PokedexProjectDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("PokedexProject_Dividend_Tracker", "PokedexProject_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "PokedexProject_Dividend_Tracker: No transfers allowed");
    }
    function withdrawDividend() public override {
        require(false, "PokedexProject_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PokedexProject contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PokedexProject_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PokedexProject_Dividend_Tracker: Cannot update claimWait to same value");
