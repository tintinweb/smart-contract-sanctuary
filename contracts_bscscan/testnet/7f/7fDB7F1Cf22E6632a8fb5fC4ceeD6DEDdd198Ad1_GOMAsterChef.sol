...
contract GOMAsterChef is Ownable {
    ...
    // set a max rewardToken per second
    ...
    constructor() {
        stakedToken = IBEP20(0x167c835cf1a86302CE01FCEf1b597Be6B5B17C16); // GOMA
        rewardToken = IBEP20(0x2b9874162d9BEA24A1D79650BBe7A182352492BA); // TORII
        
        maxTokensPerSecond = 1000000000;
    ...
    // Changes reward per second, with a cap of maxtoken per second
    ...
    function setMinDepositAmount(uint256 _minDepositAmount) external onlyOwner {
        require(_minDepositAmount >= 100, "setMinDepositAmount: 100 is minimum!");
    ...
    // 
    function pendingRewardsOfUser(address _user) public view returns (uint256) {
        UserInfo storage user = userInfo[_user];
        
		uint256 _accRewardTokensPerShare = accRewardTokensPerShare;
        
		if (blockTimestamp() > lastRewardTimestamp && stakedTokenDeposied != 0) {
            uint256 tokenReward = getMultiplierNow() * tokensPerSecond;                
            _accRewardTokensPerShare = accRewardTokensPerShare + tokenReward;
            
            if (user.amount > 0) {
                return user.amount * _accRewardTokensPerShare / stakedTokenDeposied - user.rewardDebt;
            }
		}
    ...
    function _updatePool() internal {        
        ...
        accRewardTokensPerShare = accRewardTokensPerShare + (getMultiplierNow() * tokensPerSecond);
        lastRewardTimestamp = blockTimestamp();
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        if (stakedTokenDeposied > 0) {
            uint256 pending = user.amount * accRewardTokensPerShare / stakedTokenDeposied - user.rewardDebt;
            if (pending > 0) {
                safeRewardTransfer(msg.sender, pending);
            }
        }

        uint256 finalAmount = _amount - (_amount * taxPercent / 10000);
    ...
    function withdrawEmergency() public {
        ...
        stakedToken.safeTransfer(address(msg.sender), userAmount);
    ...
    function withdrawAllStakedTokens() external onlyOwner {
        ...
        stakedToken.safeTransfer(address(msg.sender), balanceOfStakedToken());
    }
