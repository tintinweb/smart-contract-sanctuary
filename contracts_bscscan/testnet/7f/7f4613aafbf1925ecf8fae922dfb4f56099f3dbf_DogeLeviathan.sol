...
/**
 DogeLeviathan : An experimental attempt at providing a set % of RoI to holders, paid in $Doge.
 Contract simply applies taxes to Txs, one in BNBs and another for Autoliq. BNBs are sent to
 an outer wallet and autoliq works as usual. 
 
 BNBs sent to the outer wallet are used to maintain a vault that is composed of DOGE, BUSD and BNB.
 This vault is maintained such that its $ value is always equal to 2 times the upcoming payout.
 Excess income is put to work on farms.
 If the vault's value is greater than 110% of the expected payout, the excess amount in used to
 buyback and burn $DogeLev
*/
...
contract DogeLeviathan is Context, IBEP20, Ownable {
    ...
    string private _name     = "DogeLeviathan";
    string private _symbol   = "DogeLev";
