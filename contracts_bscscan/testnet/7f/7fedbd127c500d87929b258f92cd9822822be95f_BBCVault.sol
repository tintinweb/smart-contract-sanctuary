...
// File: contracts/BBCVault.sol

pragma solidity 0.6.12;

contract BBCVault is Ownable, Pausable {
    ...
    IERC20 public immutable token; // BBC token
    ...
    /**
     * @notice Constructor
     * @param _token: BBC token contract
    ...
    /**
     * @notice Deposits funds into the BBC Vault
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.bbcAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Reinvests BBC tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the BBC Vault
     */
     ...
     * @return Expected reward to collect in BBC
     */
    function calculateHarvestBBCRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingBBC(0, address(this));
     ...
     */
    function calculateTotalPendingBBCRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingBBC(0, address(this));
        amount = amount.add(available());
    ...
    /**
     * @notice Withdraws from funds from the BBC Vault
