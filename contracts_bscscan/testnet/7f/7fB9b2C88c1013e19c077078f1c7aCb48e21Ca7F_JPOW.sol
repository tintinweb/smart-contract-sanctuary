...
contract JPOW is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 rewardDebtWSB; // Reward debt. See explanation below.
    ...
    address public WSB = 0x0A64f190eD7546abDE9D2D74fa272fdf05Df1F34;
    address public BNB = 0x16BE61eC53D92F864bD708c1804f7661Fc189aA6;
    address public fundSourceBNB; //source of WSB tokens to pull from
    address public fundSourceWSB;////source of BNB tokens to pull from
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && sharesTotal != 0) {
            ...
            uint userReward=user.shares.mul(accWSBPerShare).div(1e12).sub(user.rewardDebtWSB);

            uint reReward = userReward.mul(poolRewardPercentage).div(DENOMINATOR);
            uint bankrollReward=reReward.mul(bankroll).div(DENOMINATOR);
            uint daoReward=reReward.mul(dao).div(DENOMINATOR);
            reReward=reReward.sub(bankrollReward.add(daoReward));
            return reReward;
        }
        return user.shares.mul(accWSBPerShare).div(1e12).sub(user.rewardDebtWSB);
    }
        //  View function to see pending WSB on frontend.
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && sharesTotal != 0) {
            ...
            uint bankrollReward=userReward.mul(bankroll).div(DENOMINATOR);
            uint daoReward=userReward.mul(dao).div(DENOMINATOR);
            userReward=userReward.sub(bankrollReward.add(daoReward)); 
    ...
    function _updateRewardPerBlock() internal{
        ...
        if(lastRPBlock.add(rPInterval) < block.number){
            uint rewardGenerated = IERC20(WSB).balanceOf(rewardPool);
            uint increment = rewardGenerated.div(rPInterval);
            if(increment > 0){
                WSBPerBlock = defaultRewardValue.add(increment);
                IERC20(WSB).transferFrom(rewardPool, fundSourceWSB, rewardGenerated);
                lastRPBlock=block.number;

            }
            else{
                WSBPerBlock = defaultRewardValue;
                 lastRPBlock=block.number;
            }
        }
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (user.shares > 0) {
            ...
            if (pendingWSBD > 0) {
                uint poolRewardPart=pendingWSBD.mul(poolRewardPercentage).div(DENOMINATOR);
                uint stakeRewardPart=pendingWSBD.mul(DENOMINATOR.sub(poolRewardPercentage)).div(DENOMINATOR);
                uint bankrollReward=poolRewardPart.mul(bankroll).div(DENOMINATOR);
                uint daoReward=poolRewardPart.mul(dao).div(DENOMINATOR);
                poolRewardPart=poolRewardPart.sub(bankrollReward.add(daoReward));
                safeWSBTransfer(msg.sender,poolRewardPart);
                safeWSBTransfer(bankRollAddress,bankrollReward);
                safeWSBTransfer(daoAddress,daoReward);
                safeWSBTransfer(stkAddress, stakeRewardPart);
            }
            
            if (pendingBNBD > 0) {
                uint bankrollReward=pendingBNBD.mul(bankroll).div(DENOMINATOR);
                uint daoReward=pendingBNBD.mul(dao).div(DENOMINATOR);
                pendingBNBD=pendingBNBD.sub(bankrollReward.add(daoReward));
                safeBNBTransfer(msg.sender, pendingBNBD);
                safeBNBTransfer(bankRollAddress,bankrollReward);
        ...
        user.rewardDebtWSB = user.shares.mul(pool.accWSBPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (pendingWSBW > 0) {
                uint poolRewardPart=pendingWSBW.mul(poolRewardPercentage).div(DENOMINATOR);
                uint stakeRewardPart=pendingWSBW.mul(DENOMINATOR.sub(poolRewardPercentage)).div(DENOMINATOR);
                uint bankrollReward=poolRewardPart.mul(bankroll).div(DENOMINATOR);
                uint daoReward=poolRewardPart.mul(dao).div(DENOMINATOR);
                poolRewardPart=poolRewardPart.sub(bankrollReward.add(daoReward));
                safeWSBTransfer(msg.sender,poolRewardPart);
        ...
        if (pendingBNBW > 0) {
                uint bankrollReward=pendingBNBW.mul(bankroll).div(DENOMINATOR);
                uint daoReward=pendingBNBW.mul(dao).div(DENOMINATOR);
                pendingBNBW=pendingBNBW.sub(bankrollReward.add(daoReward));
    ...
    function getWSB(uint256 _WSBAmt) internal {
        IERC20(WSB).transferFrom(fundSourceBNB, address(this), _WSBAmt);
    }
    //gets BNB for distribution from external address
    function getBNB(uint256 _BNBAmt) internal {
        IERC20(BNB).transferFrom(fundSourceWSB, address(this), _BNBAmt);
    }
    ...
    function setWSBPerBlock(uint _WSBPerBlock) external onlyOwner {
        WSBPerBlock = _WSBPerBlock;
        defaultRewardValue = _WSBPerBlock;
    }
    ...
    function setFundSourceWSB(address _fundSourceWSB) external onlyOwner {
        fundSourceWSB = _fundSourceWSB;
    }
    ...
    function setBankRollAddress(address _bankRollAddress) external onlyOwner{
    ...
    function setBankRollPercentage(uint _percentage)external onlyOwner{
        require(_percentage<DENOMINATOR, "should be less");
        bankroll=_percentage;
    }
