  ...
  */
  function removeShare(address shareholder, uint256 amount) external onlyToken returns (uint256, uint256, uint256) {
    ...
    for (uint256 i = 0; i < _shareHoldersTxSharesLength; i++)
    {
      ...
      uint256 blockTimestampLast = getBlockTimestamp();
      if (share.txDateTime.add(7 days) < blockTimestampLast)
      {
        if (share.txDateTime.add(90 days) < blockTimestampLast)
        {
...
contract WrldsMany is IBEP20, Authorization {
  ...
  modifier lockTheRewardDistrubtor {
      inRewardDistrubtor = true;
      _;
      inRewardDistrubtor = false;
  }
  ...
  */
   ...
   */
  function sendBNBToDistributor(uint256 rewardFee) private lockTheRewardDistrubtor {
