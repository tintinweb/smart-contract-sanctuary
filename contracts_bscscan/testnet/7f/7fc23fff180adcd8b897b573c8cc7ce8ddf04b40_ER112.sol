...
contract ER112 is ERC20Burnable {
    uint256 private openTime;
    address private router;
    address public pair;

    constructor(address _router) ERC20("Coin", "CO") {
    ...
    returns (bool)
    {
        return (block.timestamp < openTime);
    }

    function _transfer(address sender, address recipient, uint256 amount)
    override internal virtual
    {
        // Check for first liquidity add and start timer
        if (openTime == 0) {
            if (msg.sender == router) {
                openTime = block.timestamp + 30 seconds;
                pair = recipient;
            }
