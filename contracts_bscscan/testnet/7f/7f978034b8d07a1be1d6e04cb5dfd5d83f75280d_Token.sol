...
contract Token is Context, IERC20, Ownable {
   ...
   //address public router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; //uniswap rinkeby
    address public router = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; // bsc testnet
    ...
    function setDevFeePercent(uint8 devFeePercent) external onlyOwner {
        require(devFeePercent >= 0 && devFeePercent <= maxWalletFee, "WF err");
        _devFee = devFeePercent;
    }

    function setMarketingFeePercent(uint8 marketingFeePercent)
        external
        onlyOwner
    {
        require(
            marketingFeePercent >= 0 && marketingFeePercent <= maxWalletFee,
            "WF err"
        );
        _marketingFee = marketingFeePercent;
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && onlyWhiteListed)
            require(
                findAddressOnArray(whitelist, from),
                "Whitelist mode is enabled, Only whitelisted addresses can do transfers."
            );
    ...
    function addAddressBlacklist(address addr) public onlyOwner {
        require(
            findAddressOnArray(whitelist, addr) == false,
            "the address has already been added to blacklist."
        );
        blacklist.push(addr);
    }

    function removeAddressBlacklist(address addr)
        public
        view
        onlyOwner
    ...
    {
        require(
            findAddressOnArray(blacklist, addr),
            "Not found address on blacklist."
        );
    ...
    function addAddressWhitelist(address addr) public onlyOwner {
        require(
            !findAddressOnArray(whitelist, addr),
            "the address has already been added to whitelist."
        );

        whitelist.push(addr);
    }

    function removeAddressWhitelist(address addr)
        public
        view
        onlyOwner
        returns (address[] memory)
    {
        require(
            findAddressOnArray(whitelist, addr),
            "Not found address on whitelist."
        );
    ...
    function removeElementFromAddressArray(address[] memory list, address addr)
        public
        pure
        returns (bool)
    {
    ...
    function findAddressOnArray(address[] memory list, address addr)
        public
        pure
        returns (bool)
    {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (_devFee != 0) {
            spentAmount = contractTokenBalance.div(totFee).mul(_devFee);
            _tokenTransferNoFee(address(this), devWallet, spentAmount);
