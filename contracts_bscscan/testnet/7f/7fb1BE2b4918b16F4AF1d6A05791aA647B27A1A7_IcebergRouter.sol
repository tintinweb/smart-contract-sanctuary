 ...
 * - CLE2: Caller does not have the appropriate role.
 * - CLE3: Address whitelist status is already set as passed vaule.
 * - CLE4: Captcha requirement already set as passed vaule.
 * - CLE5: Nonce in signature does not equal to the current account nonce.
 */
contract CaptchaLimited is AccessControl {
    ...
    constructor(address _captchaVerifier) public {
     ...
     */
    function captchaVerified(address _captchaVerifiedAddress) external {
        require(hasRole(CAPTCHA_VERIFIER_ROLE, msg.sender), "CaptchaLimited: CLE2");
     ...
     */
    function captchaVerifiedBySig(uint256 _nonce, address _account, bytes calldata sig) external {
     ...
     */
    function setIsWhitelisted(address _whitlistAddress, bool _isWhitelisted) external {
        require(hasRole(CAPTCHA_LIMITED_ADMIN_ROLE, msg.sender), "CaptchaLimited: CLE2");
     ...
     * - the caller must have the 'Captcha Limited Admin' role.
     */
    function setCaptchaRequired(bool _isCaptchaRequiered) external {
        require(hasRole(CAPTCHA_LIMITED_ADMIN_ROLE, msg.sender), "CaptchaLimited: CLE2");
    ...
    function isValidCaptchaSigData(uint256 _nonce, address _account, bytes memory sig) public view returns(bool) {
        ...
        require(hasRole(CAPTCHA_VERIFIER_ROLE, recoverSigner(message, sig)), "CaptchaLimited: CLE2");
        require(_nonce == accountsNonce[_account], "CaptchaLimited: CLE5");
        return true;
    }
...
// File contracts/IcebergRouter.sol

pragma solidity 0.6.6;


contract IcebergRouter is CaptchaLimited, UniswapV2Router02 {
    ...
    constructor(
        ...
        uint256 _feeBp,
        address _captchaVerifier
    ) UniswapV2Router02(_factory, _WETH) CaptchaLimited(_captchaVerifier) public {
        require(feeBp < FEE_PRECISION, "CaptchaLimitedSwap: CLSE1");
    ...
    function swapExactTokensForTokens(
        ...
        address to,
        uint deadline
    ) external override ensure(deadline) onlyCaptchaVerified(msg.sender) returns (uint[] memory amounts) {
        uint256 fee = amountIn.mul(feeBp) / FEE_PRECISION;
        TransferHelper.safeTransferFrom(path[0], msg.sender, feesCollector, fee);
        amounts = this.swapExactTokensForTokens(amountIn.sub(fee), amountOutMin, path, to, deadline);
    }
    ...
    function swapTokensForExactTokens(
        ...
        uint256 fee = amountInMax.mul(feeBp) / FEE_PRECISION;
        TransferHelper.safeTransferFrom(path[0], msg.sender, feesCollector, fee);
        amounts = this.swapTokensForExactTokens(amountOut, amountInMax.sub(fee), path, to, deadline);        
    }
    ...
    function swapExactETHForTokens(
        ...
        address to,
        uint deadline
    ) external override payable ensure(deadline) onlyCaptchaVerified(msg.sender) returns (uint[] memory amounts) {
        uint256 fee = msg.value.mul(feeBp) / FEE_PRECISION;
        require(path[0] == WETH, "UniswapV2Router: INVALID_PATH");
        amounts = UniswapV2Library.getAmountsOut(factory, msg.value.sub(fee), path);
        require(amounts[amounts.length - 1] >= amountOutMin, "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT");
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    ...
    function swapTokensForExactETH(
        ...
        uint256 fee = amountInMax.mul(feeBp) / FEE_PRECISION;
        TransferHelper.safeTransferFrom(path[0], msg.sender, feesCollector, fee);
        amounts = this.swapTokensForExactETH(amountOut, amountInMax.sub(fee), path, to, deadline);
    }
    ...
    function swapExactTokensForETH(
        ...
        uint256 fee = amountIn.mul(feeBp) / FEE_PRECISION;
        TransferHelper.safeTransferFrom(path[0], msg.sender, feesCollector, fee);
        amounts = this.swapExactTokensForETH(amountIn.sub(fee), amountOutMin, path, to, deadline);
    }
    ...
    function swapETHForExactTokens(
        ...
        address to,
        uint deadline
    ) external override ensure(deadline) onlyCaptchaVerified(msg.sender) payable returns (uint[] memory amounts) {
        uint256 fee = msg.value.mul(feeBp) / FEE_PRECISION;
        uint256 amountAfterFee = msg.value.sub(fee);
        require(path[0] == WETH, "UniswapV2Router: INVALID_PATH");
        amounts = UniswapV2Library.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountAfterFee, "UniswapV2Router: EXCESSIVE_INPUT_AMOUNT");
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
        if (amountAfterFee > amounts[0]) TransferHelper.safeTransferETH(msg.sender, amountAfterFee - amounts[0]);
        TransferHelper.safeTransferETH(feesCollector, fee);
    }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        address to,
        uint deadline
    ) external override ensure(deadline) onlyCaptchaVerified(msg.sender) {
        uint256 fee = amountIn.mul(feeBp) / FEE_PRECISION;
        TransferHelper.safeTransferFrom(path[0], msg.sender, feesCollector, fee);
        this.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountIn.sub(fee), amountOutMin, path, to, deadline);
    }
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        address to,
        uint deadline
    ) external override payable ensure(deadline) onlyCaptchaVerified(msg.sender) {
        uint256 fee = msg.value.mul(feeBp) / FEE_PRECISION;
        require(path[0] == WETH, "UniswapV2Router: INVALID_PATH");
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        uint256 fee = amountIn.mul(feeBp) / FEE_PRECISION;
        TransferHelper.safeTransferFrom(path[0], msg.sender, feesCollector, fee);
        this.swapExactTokensForETHSupportingFeeOnTransferTokens(amountIn.sub(fee), amountOutMin, path, to, deadline);
    }
}