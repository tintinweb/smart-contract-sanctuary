...
contract IBOFactory {
    ...
    address public adminAddress;
    uint256 public sponsorshipMultiplier;
    ...
    function setFeeRate(address addr, uint256 _feeRate) external {
        require(msg.sender == adminAddress, 'BenSwap: FORBIDDEN');
        getFeeRate[addr] = _feeRate;
    }
    ...
    function createIBO(address raisingToken, address offeringToken, uint256 _price, uint256 amount) external returns (address ibo) {
        ...
        IBO(ibo).initialize(raisingToken, offeringToken, _price, msg.sender, adminAddress, getFeeRate[raisingToken]);
