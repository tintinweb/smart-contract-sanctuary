...
contract FOFF is Context, IERC20, Ownable {
    ...
    string private constant _name = "FOFF16";
    string private constant _symbol = "FOFF16";
    ...
    function _maxTX() private view returns (uint256) {
        ...
        uint256 TX = (_tTotal - tokenFromReflection(_rOwned[burnAddress])).mul(1).div(10**2);
    ...
    function _maxWalletAmount() private view returns (uint256) {
        uint256 time_deployment = block.timestamp - _tradingStartTime;
        uint256 Wallet = (_tTotal - tokenFromReflection(_rOwned[burnAddress])).mul(2).div(10**2);
        if (time_deployment < 1 hours) {
            return Wallet;
        } else if (time_deployment < 3 hours) {
            return Wallet.mul(2);
  ...
  /*  function setSwapAndLiquifyEnabled(bool _swapAndLiquifyEnabled) external onlyOwner() {
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        ...
        _maxTX = _tTotal.mul(maxTxPercent).div(10**2);
    }
     */   
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || bypassesMaxTxAmount[from] || bypassesMaxTxAmount[to]){
            takeFee = false;
        }
        else{
            // Buy
