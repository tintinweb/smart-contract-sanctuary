...
contract cubefinanceDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("cubefinance_Dividend_Tracker", "cubefinance_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "cubefinance_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "cubefinance_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main cubefinance contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "cubefinance_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "cubefinance_Dividend_Tracker: Cannot update claimWait to same value");
...
contract CubeFinance is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _teamFee) public onlyOwner {
        BNBRewardsFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        teamFee = _teamFee;

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(teamFee); // total fee transfer and buy
    }
...
function setTeamWallet(address payable _newteamWallet) public onlyOwner {
    ...
    constructor() ERC20("Cube Finance", "$CUBE") {
        BNBRewardsFee = 15;
        liquidityFee = 0;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 3;
        marketingWallet = payable(0x58842c8f3E3F29A855ee2690461bAd59a2195660);
        teamFee = 2;
        teamWallet = payable(0x2CfCa43a5f375D6fFAa336f389e12c9c25E0F44B);
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(teamFee); // total fee transfer and buy
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "cubefinance: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "cubefinance: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "cubefinance: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "cubefinance: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "cubefinance: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "cubefinance: Cannot update gasForProcessing to same value");
