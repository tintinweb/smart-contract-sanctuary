...
contract StakingPool is IStakingPool, PermissionGroup {
    ...
    mapping(uint => bool) public canUpStar;
    ...
    /**
     * @dev Owner set new max apy
     * @param value new value max apy
     */
    ...
    /**
     * @dev Stake Kaby token for upgrade hero star or receive reward
     * @param heroId id of hero.
     * @param currentStar current star of hero.
     * @param amount amount of kabyToken to stake.
     * @param stakingType type staking: upgradeStar or noLockTime.
     */
    function stake(uint heroId, uint currentStar, uint amount, StakingType stakingType) external override {
        address account = msg.sender;
        if (stakingType == StakingType.upgradeStar) {
            uint acceptedTokenDecimal = ERC20(address(acceptedToken)).decimals();
            require(100000 * (10**acceptedTokenDecimal) <= amount && amount <= 1000000 * (10**acceptedTokenDecimal), "StakingPool: invalid amount");
            require(!_stakingHeros[account].contains(heroId), "StakingPool: Hero is staking for upgrade star");
        }
        _stake(account, currentStar, heroId, amount, stakingType);
    }

    /**
     * @dev Unstake from a hero.
     * @param heroId id of hero.
     * @param amount amount of kabyToken to unstake. 
     */
    function unstake(uint heroId, uint amount) external override {
        address account = msg.sender;
        StakingData memory stakingHero = stakingData[account][heroId];
        require(stakingHero.stakingType == StakingType.noLockTime, "StakingPool: Only no lock time");
        require(stakingHero.balance >= amount, "StakingPool: insufficient balance");
        _unstake(account, heroId, amount);
    }

    /**
     * @dev User claim their rewards.
     * @param heroId id of hero.
     */
    ...
    function upgradeStarForHero(uint heroId) external override {
        ...
        emit UpgradeStar(heroId, account, 1);
        _unstake(account, heroId, stakingHero.balance);
    }

    /**
     * @dev Get status of upgradeStar for hero and amount of reward
     * @param heroId id of hero.
     * @param account wallet address of user
     */
    function earned(uint heroId, address account) public view override returns (bool canUpgradeStar, uint tokenEarned) {
        StakingData memory stakingHero = stakingData[account][heroId];
        ...
        canUpgradeStar = stakingHero.stakingType == StakingType.upgradeStar && block.timestamp >= stakingHero.lockedTime;
        tokenEarned = stakingDuration / ONE_YEAR_IN_SECONDS * stakingHero.APY / 100;
    }

    /**
     * @dev Get staking data of an account and heroId 
     * @param heroId id of hero.
     * @param account wallet address of user
     */
    ...
    function _harvest(uint heroId, address account) private {
        (bool canUpgradeStar, uint tokenEarned) = earned(heroId, account);
    ...
    function _stake(address account, uint currentStar, uint heroId, uint amount, StakingType stakingType) private {
        StakingData storage stakingHero = stakingData[account][heroId];

        _harvest(heroId, account);

        if (stakingType == StakingType.noLockTime) {
            stakingHero.APY = BASE_APY;
            stakingHero.lockedTime = 0;
            stakingHero.stakingType = StakingType.noLockTime;
        } else {
            uint time = (currentStar ** 2) * 5000000 * 10**ERC20(address(acceptedToken)).decimals() / amount;
            uint apy = (time / (86400*30)) * 6;
            stakingHero.APY = apy > MIN_APY ? apy : MIN_APY;
            stakingHero.lockedTime = block.timestamp + time;
            stakingHero.stakingType = StakingType.upgradeStar;
        }
        ...
        emit Staked(heroId, account, currentStar, amount, stakingHero.lockedTime);
    }

    function _unstake(address account, uint heroId, uint amount) private {
        StakingData storage stakingHero = stakingData[account][heroId];
...
interface IStakingPool {
    event Staked(uint indexed heroId, address indexed account, uint currentStar, uint amount, uint lockedTime);
    ...
    event UpgradeStar(uint indexed heroId, address indexed account, uint amount);

    enum StakingType {
        noLockTime, upgradeStar
    }
     ...
     */
    function stake(uint heroId, uint currentStar, uint amount, StakingType stakingType) external;
     ...
     */
    function earned(uint heroId, address account) external view returns (bool canUpgradeStar, uint tokenEarned);
