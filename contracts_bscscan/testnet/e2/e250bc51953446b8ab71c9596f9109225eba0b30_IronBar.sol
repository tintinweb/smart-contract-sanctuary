...
contract PriceConsumerV3  {
    ...
    // Change the aggregator(BNB/USD) address on deploying mainnet
    //Aggregator for BSC Testnet : 0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526
...
contract IronBar is IERC20, PriceConsumerV3 {
    
    using SafeMath for uint256;
    address private _owner;
    string private _name = "IronBar";
    string private _symbol = "IB" ;
    uint8 private _decimals = 18;
    uint256 private _totalSupply;
    uint8 private tokenPrice;
    ...
    */
     ...
     */
      ...
      */
       ...
       * @return locking status in true or false  locked => true & unlocked => false
       */
     ...
     */
    function transfer(address to, uint256 value) public AllTransfersLockStatus checkLocking(msg.sender,value) override returns (bool) {
     ...
     */
    function transferFrom(address from, address to, uint256 value) public AllTransfersLockStatus checkLocking(from,value) override returns (bool) {
    ...
    function setTokenPrice(uint8 _tokenPrice) public onlyOwner returns (bool) {
        
        tokenPrice = _tokenPrice;
        return true;
    }
    function buyTokens(address _user, uint256 _amountInBNB) public returns (uint256) {

        uint256 amountInUsd = _amountInBNB.mul(getUsdPrice());
        tokenTransfer(_user,amountInUsd);

    }
    function tokenTransfer(address _address, uint256 _amountinUSD) public returns(uint256){

        uint256 totalTokenTransfer=_amountinUSD.div(tokenPrice);
        transfer(_address,totalTokenTransfer);
    ...
    function getTokenPrice()public view returns (uint8) {
