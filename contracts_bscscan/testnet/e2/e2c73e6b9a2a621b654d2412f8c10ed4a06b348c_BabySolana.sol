...
Symbol: BSol
Decimal: 9
Mket Address 0x7E9DFf15DD9F6934Cc71DEA7B582A00501028ACD

number of token can be 1T 
7-10% marketing wallet (adjustable) (Ok, please provide marketing address) 
1% token reflection to holders (adjustable) (Ok) 
 */ 

contract BabySolana is Context, IBEP20, Ownable {
    ...
    constructor ( )  {
		...
		_tTotal =1000000000000 * _DECIMALFACTOR;
		_rTotal = (_MAX - (_MAX % _tTotal));
		_TAX_FEE = 1 * 100; 
        _BURN_FEE = 0 * 100;
		_Marketing_FEE = 7 * 100;
		ORIG_TAX_FEE = _TAX_FEE;
		ORIG_BURN_FEE = _BURN_FEE;
		ORIG_Marketing_FEE = _Marketing_FEE;
		FeeAddress = 0x7E9DFf15DD9F6934Cc71DEA7B582A00501028ACD;
		_owner = owner();
	...
	function updateFee(uint256 _txFee,uint256 _burnFee,uint256 _MarketingFee) onlyOwner() public{
		require(_txFee < 100 && _burnFee < 100 && _MarketingFee < 100);
        _TAX_FEE = _txFee* 100; 
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _reflectFee(rFee, rBurn, tFee, tBurn, tMarketing);
    ...
    function _reflectFee(uint256 rFee, uint256 rBurn, uint256 tFee, uint256 tBurn, uint256 tMarketing) private {
    ...
    function _getTBasics(uint256 tAmount, uint256 taxFee, uint256 burnFee, uint256 MarketingFee) private view returns (uint256, uint256, uint256) {
    ...
    function _sendToMarketing(uint256 tMarketing, address sender) private {
        uint256 currentRate = _getRate();
        uint256 rMarketing = tMarketing.mul(currentRate);
        _rOwned[FeeAddress] = _rOwned[FeeAddress].add(rMarketing);
        _tOwned[FeeAddress] = _tOwned[FeeAddress].add(tMarketing);
