...
contract LatteSwapFactory is ILatteSwapFactory {
  ...
  function pairCodeHash() external pure returns (bytes32) {
    return keccak256(type(LatteSwapPair).creationCode);
  }

  function createPair(address tokenA, address tokenB) external override returns (address pair) {
    require(tokenA != tokenB, 'LatteSwapFactory::createPair::IDENTICAL_ADDRESSES');
    (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
    require(token0 != address(0), 'LatteSwapFactory::createPair::ZERO_ADDRESS');
    require(getPair[token0][token1] == address(0), 'LatteSwapFactory::createPair::PAIR_EXISTS'); // single check is sufficient
    bytes memory bytecode = type(LatteSwapPair).creationCode;
    bytes32 salt = keccak256(abi.encodePacked(token0, token1));
    assembly {
      pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
    }
    LatteSwapPair(pair).initialize(token0, token1);
  ...
  function setFeeTo(address _feeTo) external override {
    require(msg.sender == feeToSetter, 'LatteSwapFactory::setFeeTo::FORBIDDEN');
    feeTo = _feeTo;
  }

  function setFeeToSetter(address _feeToSetter) external override {
    require(msg.sender == feeToSetter, 'LatteSwapFactory::setFeeToSetter::FORBIDDEN');
    feeToSetter = _feeToSetter;
  }
}