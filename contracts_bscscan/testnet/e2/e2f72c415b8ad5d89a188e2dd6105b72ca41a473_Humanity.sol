...
contract Humanity is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Humanity";
    string constant _symbol = "HMN";
    ...
    function swapBack() internal swapping {
        ...
        (bool successDev, /* bytes memory data */) = payable(charityFeeReceiver).call{value: amountBNBDev, gas: 3000000}("");
    ...
    function setFees(uint256 _charityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator) external authorized capGasPrice {
        if(feesLocked) {
            require( _charityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee) < (_feeDenominator.div(4)) );
        }
        charityFee = _charityFee;
        buybackFee = _buybackFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        totalFee = _charityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee);
    ...
    function setCharityFeeReceiver(address _charityFeeReceiver) external authorized capGasPrice {
