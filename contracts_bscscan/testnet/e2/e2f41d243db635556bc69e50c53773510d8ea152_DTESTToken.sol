...
contract DTESTToken is Context, IERC20, Ownable {
    ...
    address payable _devAddress; // 0x5d6B9042FCE5d4eb614F9e77179cd71E05f8667a
    address payable _marketingAddress; // 0x82E6045fa2b4c0BEB36E7d5094C075eC9DafEB25
    ...
    uint256 public _maxTxAmount = 10000000000000 * 10**18; // 2%
    ...
    {
        require(
            block.timestamp > _tLocked[_msgSender()],
    ...
    function excludeOrIncludeFromMaxTx(address account, bool exclude)
        public
        onlyOwner
    {
    ...
    function setMinTokensToSwap(uint256 _minTokens) external onlyOwner {
        numTokensSellToAddToLiquidity = _minTokens * 10**18;
    }
    ...
    function calculateLiquidityPlusFees(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return _amount.mul(_liquidityFee + _devFee + _marketingFee).div(10**2);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingBalance = newBalance.div(totalLiqFee).mul(
            _marketingFee
        );
        uint256 marketingPortion = otherHalf.div(totalLiqFee).mul(
            _marketingFee
        );
