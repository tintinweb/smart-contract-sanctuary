...
contract WWinTokenLock is Ownable, ReentrancyGuard{
    ...
    function setLock(address user, uint256 amount, uint256 firstReleaseTime, uint256 firstReleaseRate, uint256 monthlyReleaseRate) public onlyOwner{
        require(amount > 0, "Amount > 0");
        require(firstReleaseTime > block.timestamp, "Time must be future");
        require(
            (firstReleaseRate < 10000 && monthlyReleaseRate > 0) || 
            (firstReleaseRate == 10000 && monthlyReleaseRate == 0), 
        ...
        _locks[user].monthlyReleaseRate = monthlyReleaseRate;
    ...
    function setLockBatch(address[] memory users, uint256[] memory amounts, uint256 firstReleaseTime, uint256 firstReleaseRate, uint256 monthlyReleaseRate) external onlyOwner{
        require(users.length == amounts.length, "Array shape not match");
        for(uint256 i=0;i<users.length;i++){
            setLock(users[i], amounts[i], firstReleaseTime, firstReleaseRate, monthlyReleaseRate);
        }
    }

    // NOTE: Method getters

    function getMonthSinceFirstRelease(address _user) internal view returns(uint256){
        return block.timestamp.sub(_locks[_user].firstReleaseTime).div(30 days);
    }

    function _getClaimableAmount(address _user) internal view returns(uint256){
        uint256 month = getMonthSinceFirstRelease(_user);
        if(block.timestamp < _locks[_user].firstReleaseTime)return 0;
        uint256 amount = 0;
        if(block.timestamp >= _locks[_user].firstReleaseTime){
            amount = amount.add(_locks[_user].initialAmount.mul(_locks[_user].firstReleaseRate).div(_rate_precision).div(1e2));
        }
        if(month > 0){
            amount = amount.add(_locks[_user].initialAmount.mul(_locks[_user].monthlyReleaseRate).mul(month).div(_rate_precision).div(1e2));
        }
        return amount;
    }
