...
contract FinaLockFarming is Initializable, OwnableUpgradeable, FinaFarming {
    ...
    function pendingLPRewardByTier(uint _pid, address _user) public view returns (uint[] memory pending_) {
        ...
        if(pending > 0){
            for(uint i = 0; i < depositTier.length; i++) {
    ...
    function pendingTimeForNextTier(uint _pid, address _user) public view returns (bool validUser_, uint pendingTime) {
        uint[] memory p = pendingLPReward(_pid,_user);
        uint pending = p[0];
        uint tmpTime;
        bool validUser;
        ...
        if(pending > 0 && block.timestamp < userLPInfo[_pid][_user].averageDepositedTime + depositTier[depositTier.length-1]){
            validUser = true;
            for(uint i = 0; i < depositTier.length - 1; i++) {
                if(block.timestamp >= userLPInfo[_pid][_user].averageDepositedTime + depositTier[i]){
                    tmpTime = block.timestamp - userLPInfo[_pid][_user].averageDepositedTime - depositTier[i];
                }
            }
        }
