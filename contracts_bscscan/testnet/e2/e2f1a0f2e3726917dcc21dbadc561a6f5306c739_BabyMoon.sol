...
contract BabyMoon is ERC20, Ownable {
    ...
    address public cardanoDividendToken;
    ...
    bool public cardanoDividendEnabled = false;
    bool public busdDividendEnabled = false;

    CardanoDividendTracker public cardanoDividendTracker;
    ...
    uint256 private cardanoDividendRewardsFee;
    uint256 private previousCardanoDividendRewardsFee;
    ...
    event UpdateCardanoDividendTracker(address indexed newAddress, address indexed oldAddress);
    ...
    event CardanoDividendEnabledUpdated(bool enabled);
    ...
    event ProcessedCardanoDividendTracker(
    	uint256 iterations,
    	uint256 claims,
    ...
    constructor() ERC20("BabyMoon", "$BMOON") {
    	cardanoDividendTracker = new CardanoDividendTracker();
    	...
    	cardanoDividendToken = 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47; // Binance-Peg Cardano
        ...
        excludeFromDividend(address(cardanoDividendTracker));
    ...
    receive() external payable {
  	...
  	function whitelistDxSale(address _presaleAddress, address _routerAddress) external onlyOwner {
        ...
        cardanoDividendTracker.excludeFromDividends(_presaleAddress);
        busdDividendTracker.excludeFromDividends(_presaleAddress);
        excludeFromFees(_presaleAddress, true);

        cardanoDividendTracker.excludeFromDividends(_routerAddress);
  	...
  	function prepareForPartherOrExchangeListing(address _partnerOrExchangeAddress) external onlyOwner {
  	    cardanoDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(busdDividendRewardsFee).add(cardanoDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setCardanoDividendEnabled(bool _enabled) external onlyOwner {
        require(cardanoDividendEnabled != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
            previousCardanoDividendRewardsFee = cardanoDividendRewardsFee;
            cardanoDividendRewardsFee = 0;
            cardanoDividendEnabled = _enabled;
        } else {
            cardanoDividendRewardsFee = previousCardanoDividendRewardsFee;
            totalFees = cardanoDividendRewardsFee.add(marketingFee).add(busdDividendRewardsFee).add(buyBackAndLiquidityFee);
            cardanoDividendEnabled = _enabled;
        }

        emit CardanoDividendEnabledUpdated(_enabled);
    }
    ...
    function setbusdDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = busdDividendRewardsFee.add(marketingFee).add(cardanoDividendRewardsFee).add(buyBackAndLiquidityFee);
            busdDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(busdDividendRewardsFee).add(cardanoDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateCardanoDividendRewardFee(uint8 newFee) external onlyOwner {
        cardanoDividendRewardsFee = newFee;
        totalFees = cardanoDividendRewardsFee.add(marketingFee).add(busdDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    
    function updateBusdDividendRewardFee(uint8 newFee) external onlyOwner {
              busdDividendRewardsFee = newFee;
        totalFees = busdDividendRewardsFee.add(cardanoDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }
    
    function updateMarketingFee(uint8 newFee) private onlyOwner {
        marketingFee = newFee;
        totalFees = marketingFee.add(cardanoDividendRewardsFee).add(busdDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(cardanoDividendRewardsFee).add(busdDividendRewardsFee).add(marketingFee);
    }
    ...
    function excludeFromDividend(address account) public onlyOwner {
        cardanoDividendTracker.excludeFromDividends(address(account));
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        ...
        if(value) {
            cardanoDividendTracker.excludeFromDividends(pair);
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        cardanoDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
        busdDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }

    function updateClaimWait(uint256 claimWait) external onlyOwner {
        cardanoDividendTracker.updateClaimWait(claimWait);
        busdDividendTracker.updateClaimWait(claimWait);
    }

    function getCardanoClaimWait() external view returns(uint256) {
        return cardanoDividendTracker.claimWait();
    }
    ...
    function getTotalCardanoDividendsDistributed() external view returns (uint256) {
        return cardanoDividendTracker.totalDividendsDistributed();
    }
    ...
    function withdrawableCardanoDividendOf(address account) external view returns(uint256) {
    	return cardanoDividendTracker.withdrawableDividendOf(account);
  	}
	...
	function cardanoDividendTokenBalanceOf(address account) external view returns (uint256) {
		return cardanoDividendTracker.balanceOf(account);
	}
    ...
    function getAccountCardanoDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return cardanoDividendTracker.getAccount(account);
    }
	...
	function getAccountCardanoDividendsInfoAtIndex(uint256 index)
        external view returns (
            address,
            int256,
            int256,
            ...
            uint256,
            uint256) {
    	return cardanoDividendTracker.getAccountAtIndex(index);
    }
	...
	function processDividendTracker(uint256 gas) external onlyOwner {
		(uint256 ethIterations, uint256 ethClaims, uint256 ethLastProcessedIndex) = cardanoDividendTracker.process(gas);
		emit ProcessedCardanoDividendTracker(ethIterations, ethClaims, ethLastProcessedIndex, false, gas, tx.origin);
    ...
    function claim() external {
		cardanoDividendTracker.processAccount(payable(msg.sender), false);
		busdDividendTracker.processAccount(payable(msg.sender), false);
    }
    function getLastCardanoDividendProcessedIndex() external view returns(uint256) {
    	return cardanoDividendTracker.getLastProcessedIndex();
    }
    ...
    function getNumberOfCardanoDividendTokenHolders() external view returns(uint256) {
        return cardanoDividendTracker.getNumberOfTokenHolders();
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (cardanoDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(cardanoDividendRewardsFee.add(busdDividendRewardsFee)).mul(cardanoDividendRewardsFee);
                    swapAndSendCardanoDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (busdDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(cardanoDividendRewardsFee.add(busdDividendRewardsFee)).mul(busdDividendRewardsFee);
        ...
        try cardanoDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try busdDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try cardanoDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        try busdDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}

        if(!swapping) {
	    	uint256 gas = gasForProcessing;

	    	try cardanoDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
	    		emit ProcessedCardanoDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
	    	}
	    	catch {
    ...
    function swapAndSendCardanoDividends(uint256 tokens) private {
        swapTokensForDividendToken(tokens, address(this), cardanoDividendToken);
        uint256 cardanoDividends = IERC20(cardanoDividendToken).balanceOf(address(this));
        transferDividends(cardanoDividendToken, address(cardanoDividendTracker), cardanoDividendTracker, cardanoDividends);
    }
...
contract CardanoDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() DividendPayingToken("BabyMoon_Cardano_Dividend_Tracker", "BCDT", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 100000 * (10**18); //must hold 100000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BabyMoon_Cardano_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BabyMoon_Cardano_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyMoon contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "BabyMoon_Cardano_Dividend_Tracker: Cannot update claimWait to same value");
