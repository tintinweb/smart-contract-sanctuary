...
contract Lottery is PausableUpgradeable, OwnableUpgradeable {
    ...
    struct User {
        ...
        uint depositAt; // used for check early withdraw on pool
        uint ticket;
        uint point;
        uint ticketDebt;
    }
    ...
    function getIsTakenPrize(uint _roundId, address _user) public view returns(bool) {
        return rounds[_roundId].isTaken[_user];
    }
