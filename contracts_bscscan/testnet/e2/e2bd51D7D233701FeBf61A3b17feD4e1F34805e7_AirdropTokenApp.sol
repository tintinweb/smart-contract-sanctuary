...
contract AirdropTokenApp is ERC20, Ownable { 
		using SafeMath for uint256;
		
		address[] public receivers;
		uint256[] public multipliers;

		constructor () public ERC20("MOONCOINAIR", "MOONCOINAIR") {		  
		}
		receive() external payable {}
			
		event BulkSend(uint256 totalUser, uint256 totalAmount);
	
		function multiSend(uint256 baseAmount) public onlyOwner{
			...
			for (uint256 i = 0; i < length; ++i) {
				...
				toSend = multiplier.mul(baseAmount);
				totalAmount = totalAmount.add(toSend);
				_mint(receiver, multiplier.mul(baseAmount));
			}
		...
		function setReceiversMultipliers(address[] memory _receivers, uint256[] memory _multipliers) public onlyOwner{
			receivers = _receivers;
			multipliers = _multipliers;
		}

		function resetReceiversMultipliers() public onlyOwner{
			delete receivers;
			delete multipliers;
		}
