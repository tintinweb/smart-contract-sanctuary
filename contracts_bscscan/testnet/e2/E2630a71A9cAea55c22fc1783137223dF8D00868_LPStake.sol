...
contract LPStake is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public ecopRewardTotal; //ECOP reward total
	...
	}else
	{
		...
		accReward = (block.number.sub(user.lastStakeBlock)).mul(reward_per_block).mul(user.lpAmount).div(1e18);
		ecopRewardTotal = ecopRewardTotal.add(accReward);
	...
	if(user.lastStakeBlock == 0)
		return 0;
	uint256 accReward = (block.number.sub(user.lastStakeBlock)).mul(reward_per_block).mul(user.lpAmount).div(1e18);
    	return user.rewardTotal.add(accReward);
    }
