 ...
 * Have fun !!!
 * 
 * The idea that unvaccinated semen will outperform Bitcoin and other crypto has floated around Crypto Twitter since COVID-19 vaccines rolled out. 
 * The meme is: the vaccines will go so wrong that women will be forced to pay top money for sperm from men who’ve opted out of The Great Reset.
 * 
 */
...
contract UNSperm is Context, IBEP20, Ownable {
    ...
    //return total fees distributed to influencers
	...
	function updateFee(uint256 _txFee,uint256 _burnFee,uint256 _charityFee) onlyOwner() public{
        require(_txFee <= 200, "TAX_FEE too high" );
        require(_burnFee <= 100, "BURN_FEE too high" );
        require(_charityFee <= 300, "CHARITY_FEE too high" );
    ...
    function _reflectFee(uint256 rFee, uint256 rBurn, uint256 rCharity, uint256 tFee, uint256 tBurn, uint256 tCharity) private {
        ...
        //Update total fees of charity for unregistred member - else update total fees of influencers for registered member
        if (isMember(tx.origin)) {
            _tInfluencersTotal = _tInfluencersTotal.add(tCharity);
        } else {
    ...
    function _getTBasics(uint256 tAmount, uint256 taxFee, uint256 burnFee, uint256 charityFee) private view returns (uint256, uint256, uint256) {
        ...
        //  divide burn by 2 for registered member (using tx.origin to take into account contract calls by swap function)
        if (isMember(tx.origin)) tBurn = tBurn.div(2);
        
        uint256 tCharity = ((tAmount.mul(charityFee)).div(_GRANULARITY)).div(100);
        
        // charity fees are 0 for influencers
        if (_isInfluencer[tx.origin]) tCharity = 0;
    ...
    function _sendToCharity(uint256 tCharity, address sender) private {
        ...
        //if user is a member, send fees to influencer - else, send to charity
        address feesReceiver = FeeAddress;
        if (isMember(tx.origin)) feesReceiver = _members[tx.origin];
        
        _rOwned[feesReceiver] = _rOwned[feesReceiver].add(rCharity);
        _tOwned[feesReceiver] = _tOwned[feesReceiver].add(tCharity);
        
        emit Transfer(sender, feesReceiver, tCharity);
    }
    ...
    /**
     *
     *  adding special functions to avoid getting stuck
     * 
    ...
    function withdrawTokens(address _tokenAddr, address _to) public onlyOwner {
        require(_tokenAddr != address(this), "Cannot transfer out this token from contract!");
        require(_tokenAddr.isContract(), "Need a contract address");
        Token(_tokenAddr).transfer(_to, Token(_tokenAddr).balanceOf(address(this)));
    }
