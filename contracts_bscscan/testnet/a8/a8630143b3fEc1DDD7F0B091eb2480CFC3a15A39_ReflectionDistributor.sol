...
contract ReflectionDistributor is Allowable, IReflectionDistributor {
    ...
    receive() external payable {
        ...
        _dividendsPerShare = _totalShares > 0 && ethAmount >= _dividendsPerShareAccuracyFactor
            ? _dividendsPerShare.add(ethAmount.div(_totalShares))
            : 0;
    }
    ...
    function process(uint256 gas) external override onlyAllowed {
        ...
        while (iterations < shareholderCount && gasUsed < gas) {
            ...
            if (_shares[currentShareHolder].amount != ownerAmount.div(_dividendsPerShareAccuracyFactor)) {
    ...
    function _setShare(address shareholder, uint256 amount) private {
        uint256 share = amount.div(_dividendsPerShareAccuracyFactor);
        uint256 previousShare = _shares[shareholder].amount;

        if (previousShare == 0 && share > 0) {
            _addShareholder(shareholder);
        }
        else if (previousShare > 0 && share == 0) {
            _removeShareholder(shareholder);
        }

        _shares[shareholder].amount = share;
        _totalShares = _totalShares.add(share).sub(previousShare);
        _shares[shareholder].totalExcluded = _getCumulativeDividends(previousShare);
    }
    ...
    function _distributeDividends(address shareholder) private {
        uint256 share = _getUnpaidEarnings(shareholder);
        uint256 amount = share.mul(_dividendsPerShareAccuracyFactor);
        ...
        _shares[shareholder].totalRealised = _shares[shareholder].totalRealised.add(share);
