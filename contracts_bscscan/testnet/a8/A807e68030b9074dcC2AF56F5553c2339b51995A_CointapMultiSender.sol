...
contract CointapMultiSender is Context, Ownable, SafeTransfer {
    ...
    address SERVICE_ADDRESS = address(0xbe44ddE2875D023D8de11518b4b4e351d6A7bC58);
    ...
    uint256 COST_PER_TX_DIAMOND = 0.001 * 10**18;
    ...
    function addManager(address newManager) public virtual onlyOwner returns (bool) {
        require(!_isManager(_msgSender()), "Manager: the address is already a manager");
        managers.push(newManager);
        return true;
    }

    function removeManager(address newManager) public virtual onlyOwner returns (bool) {
        require(_isManager(_msgSender()), "Manager: the address isn't a manager");
        for(uint8 i = 0; i < managers.length; i++) {
            if(managers[i] == newManager) {
                delete managers[i];
                break;
