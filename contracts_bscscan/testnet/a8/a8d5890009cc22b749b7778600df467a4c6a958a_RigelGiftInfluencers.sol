...
contract RigelGiftInfluencers is Ownable {
    ...
    mapping(uint256 => RewardProject) rewardProjMapping;
    ...
    function createRewardProject(
        ...
        { // to avoid stack too deep
            uint256 subFee = _subscriptionFee; // gas efficiency
            address tReceiver = _RGPTokenReceiver; // gas efficiency
        ...
        uint256 rpCounter = _rewardProjectCounter; // for gas efficiency 

        (bool status, uint256 sumOfWeights) =
            _setTickers(rpCounter, tickerInfo);
        require(status == true, "RigelGift: _setTickers fail");

        status = _setRewards(rpCounter, rewards);
        ...
        rewardProjMapping[rpCounter] = rewardProj;
        rewardProjects.push(rpCounter);

        emit RewardProjectCreate(_msgSender(), rpCounter);
        rpCounter = rpCounter.add(1);
    }
    ...
    {
        return abi.decode(tickerInfo, (address, uint256, uint256, bytes32));
    }
    ...
    // returns rewards for project
    function returnRewardProj(uint256 rew) public view returns(bool, address, uint256, uint256, uint256, uint256,uint256, uint256, uint256, string memory) {
        RewardProject memory r = rewardProjMapping[rew];
        
        string memory des = string(abi.encodePacked(r.description));
