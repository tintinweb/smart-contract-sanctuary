...
contract sTrapeza_v2 is IsFIDL, ERC20Permit {
    ...
    event LogStakingContractUpdated(address stakingContract, address sFIDLv1Contract);
    ...
    address public sFIDLv1Contract; // v1 contract 
    ...
    // do this last
    function initialize(address _stakingContract, address _sFIDLv1Contract) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");

        require(_stakingContract != address(0), "Staking");
        stakingContract = _stakingContract;

        require(_sFIDLv1Contract != address(0), "sFIDL v1");
        sFIDLv1Contract = _sFIDLv1Contract;

        _gonBalances[stakingContract] = TOTAL_GONS;

        emit Transfer(address(0x0), stakingContract, _totalSupply);
        emit LogStakingContractUpdated(stakingContract, sFIDLv1Contract);
    ...
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(IsFIDL(sFIDLv1Contract).circulatingSupply()).sub(gFIDL.balanceFrom(IERC20(address(gFIDL)).totalSupply()));
            // .add(gFIDL.balanceFrom(IERC20(address(gFIDL)).totalSupply())).add(
