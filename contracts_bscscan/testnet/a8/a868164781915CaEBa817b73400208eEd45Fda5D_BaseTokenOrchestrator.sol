 ...
 */
contract BaseToken is ERC20UpgradeSafe, OwnableUpgradeSafe {
...
//    );
    ...
    uint256 public mcapNow;
    int256 public mcapTotalNow;
    uint256 public tokenPriceNow;
...
//        minRebaseTimeIntervalSec = 1 days;
        minRebaseTimeIntervalSec = 2 hours;
        rebaseWindowOffsetSec = 1800;  // 10PM UTC
        rebaseWindowLengthSec = 30 minutes;
        lastRebaseTimestampSec = 0;
        mcapNow = 7599190141;
        mcapTotalNow = 1000000 * 10**9;
        tokenPriceNow = 1*10**18;
        epoch = 0;
...
//            .div(targetPricex1T);
        int256 referentialValue = mcap.mul(10000).div(mcapNow).toInt256Safe().sub(10000).mul(3).add(10000);
        if (referentialValue < 0) {
            return 0;
        }
        mcapNow = mcap;
        mcapTotalNow = referentialValue.mul(mcapTotalNow).div(10000);
        // 暂时不管价格变化
//        emit LogTest(-1,referentialValue, mcapTotalNow,mcapTotalNow.sub(BASE.totalSupply().toInt256Safe()),now);
        return mcapTotalNow.sub(BASE.totalSupply().toInt256Safe());
    }
...
contract BaseTokenOrchestrator is OwnableUpgradeSafe {
    ...
    address public LMRewardAddr;
    address public GenesisBNBAddr;
    address public GenesisUSDCAddr;
    ...
    function setTransferAddr(address LMPool_, address GenesisBNBPool_, address GenesisUSDCPool_, address BaseToken_) external onlyOwner{
        LMRewardAddr = LMPool_;
        GenesisBNBAddr = GenesisBNBPool_;
        GenesisUSDCAddr = GenesisUSDCPool_;
...
//        }
        ...
        uint256 amount = IERC20(BaseTokenAddress).balanceOf(address(this));
        uint256 amountLMReward = amount.mul(325).div(1000);
        uint256 amountGenesisBNBReward = amount.mul(250).div(1000);
        uint256 amountGenesisUSDCReward = amount.mul(250).div(1000);
        if(policy.epoch()<200){
            IERC20(BaseTokenAddress).transfer(LMRewardAddr, amountLMReward);
            IERC20(BaseTokenAddress).transfer(GenesisBNBAddr, amountGenesisBNBReward);
            IERC20(BaseTokenAddress).transfer(GenesisUSDCAddr, amountGenesisUSDCReward);
        }
    }
