...
contract GoldenGrainToken is BEP20 {
    ...
    // Min amount to liquify. (default 50 GRAIN)
    uint256 public minAmountToLiquify = 50 * 10**18; // INMUTABLE

    // PCS LP Token Address
    address public lpToken; // ONLY ONCE!

    address public uffToken;

    // Automatic swap and liquify enabled
    bool public swapAndLiquifyEnabled = false; // INMUTABLE
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (sender == owner() || recipient == owner() || transferTaxRate == 0 || _isExcludedFromFee[sender] || _isExcludedFromFee[recipient] || recipient == lpToken || sender == lpToken) {
            if (sender == lpToken) {
                if (recipient == address(PCSRouter)) {
                    super._transfer(sender, recipient, amount);
                }
            }
            if (sender == address(PCSRouter)) {
                super._transfer(sender, recipient, amount);
            }
        } else {
            // uint256 ethRouterBalance = address(PCSRouter).balance;
            require(tradingOpen == true, "Trading not yet open.");
            ...
            // Distributing UFFs (Liquify, Recipient)
    ...
    function swapAndLiquify() private lockTheSwap transferTaxFree {
        ...
        if (contractTokenBalance >= minAmountToLiquify) {
            ...
            uint256 halfPlusQuarter = half.add(liquifyAmount.div(4));
            ...
            // swap tokens for BNB
            swapTokensForEth(halfPlusQuarter);
            uint256 newBalanceAfterGrainSell = address(this).balance.sub(initialBalance);

            // swap bnb for uff
            swapEthForTokens(newBalanceAfterGrainSell.div(4));
            uint256 uffBalance = IBEP20(uffToken).balanceOf(address(this));

            // how much ETH did we just swap into?
            uint256 newBalance = newBalanceAfterGrainSell.div(4);
            
            // add liquidity
            addLiquidity(address(this), liquifyAmount.div(4), newBalance);
            addLiquidity(uffToken, uffBalance, newBalance);
