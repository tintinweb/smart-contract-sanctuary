...
contract RedRocket is Context, IERC20, Ownable {
    ...
    string private _name = "Red Rocket";
    string private _symbol = "RRKT";
    ...
    address private _operationsWalletAddress = 0x26121630b15691cA37c2789bc8c4B8B3a8F1e1a6;
    ...
    uint256 private _previousCharityFee = _operationsFee;
    ...
    constructor() public {
        ...
        address _uniswapV2RouterAddr=0xD99D1c33F9fC3444f8101754aBC46c52416550D1; //BNB test net
        ...
        startSale(block.number, 999999999, 0,1500000000*10**9, 2000000000000);
    }
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwner {
     ...
     //to recieve ETH from uniswapV2Router when swapping --> use cleanDust to recover ether
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tCharity = calculateOperationsFee(tAmount);
    ...
    function _takeCharity(uint256 tCharity) private {
        uint256 currentRate =  _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[_operationsWalletAddress] = _rOwned[_operationsWalletAddress].add(rCharity);
        if(_isExcludedFromReward[_operationsWalletAddress])
            _tOwned[_operationsWalletAddress] = _tOwned[_operationsWalletAddress].add(tCharity);
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _operationsFee== 0 && _burnFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousCharityFee = _operationsFee;
    ...
    function restoreAllFee() private {
        ...
        _operationsFee= _previousCharityFee;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if (tCharity > 0) emit Transfer(sender, _operationsWalletAddress, tCharity); 
    }
 ...
 function setsPrice(uint256 newSalePrice) external onlyOwner() {
        sPrice = newSalePrice;
    }   
