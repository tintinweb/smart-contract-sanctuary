...
contract LudoP2E is ERC20, Ownable {
    ...
    address payable public  marketingWallet = payable(0xCD543473B16f15a1950940Efb1317E4c34BE7985);
    ...
    function updateFees(uint256 _liquidityFee, uint256 _marketingFee) public onlyOwner {
        require(_liquidityFee <= 5 && _marketingFee <= 5, "each fee must be less than or equal to five");
    ...
    function setMaxSellTx(uint256 _maxSellTxAmount) public onlyOwner {
        require(_maxSellTxAmount > 0, "max transaction amount must be greater than zero");
        maxSellTransactionAmount = _maxSellTxAmount * (10**18);
    }
    
    function setMaxBuyTx(uint256 _maxBuyTxAmount) public onlyOwner {
        require(_maxBuyTxAmount > 0, "max transaction amount must be greater than zero");
    ...
    constructor() ERC20("LudoP2E", "$LudoP2E") {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "LudoP2E: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "LudoP2E: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "LudoP2E: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LudoP2E: Automated market maker pair is already set to that value");
