...
contract MonstropolyData is AccessControlProxyPausable, UUPSUpgradeableByRole { 
    ...
    function getRarityByRange(string calldata gen) public view returns(uint) {
        ...
        return getRarityByRange(parseInt(slice(_bytes, randLength*2, randLength)));
    }

    function getRarityByRangeVIP(string calldata gen) public view returns(uint) {
        bytes memory _bytes = bytes(gen);
        return getRarityByRangeVIP(parseInt(slice(_bytes, randLength*2, randLength)));
    }
    ...
    function _replaceInString(string calldata original_, string calldata insert_, uint index_) internal view returns(string memory) {
