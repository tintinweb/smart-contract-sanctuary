...
Tokenomic:
reward reflect each 10 minutes.
each: transfer, buy or sell (Pancakeswap) 
- 3% auto burn (Dead address)
- 3% Charity wallet
- 5% auto Reward holder in BUSD  */
...
contract Lanora is IBEP20, RSunAuth {
    using SafeMath for uint256;
    ...
    string constant _name = "Lanora";
    string constant _symbol = "LNA";
    ...
    uint256 public buybackFee = 300;  //3%
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _charityFee, uint256 _feeDenominator, uint256 _totalSellFee) external authorizedFor(Permission.AdjustContractVariables) {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        reflectionFee = _reflectionFee;
        charityFee = _charityFee;
        totalBuyFee = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_charityFee);
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _charityFeeReceiver) external authorizedFor(Permission.AdjustContractVariables) {
