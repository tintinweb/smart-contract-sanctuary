...
contract MasterChef is Ownable {
        ...
        if(length > 0){
            for (uint256 pid = 0; pid < length; ++pid) {
                ...
                if(amountHolder >= discount.totalHolder) {
                    feeAmount = _amount.mul(_withdrawFee.mul(discount.discountRate).div(100)).div(10000);
                    break ;
                }
            }
        } else {
            feeAmount = _amount.mul(_withdrawFee).div(10000);
        }
    ...
    function leaveStaking(uint256 _amount) public {
        ...
        if(_amount > 0) {
            user.amount = user.amount.sub(_amount);
            safePeachTransfer(msg.sender, _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accPeachPerShare).div(1e12);
