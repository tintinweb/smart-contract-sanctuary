...
contract Signed {
    ...
    function getSigner(bytes32 data, uint8 v, bytes32 r, bytes32 s) internal returns (address){
        require(!permitDoubleSpending[data] , "Forbidden double spending in admin");
...
contract KawaiiDelivery is Pausable, Signed, SignData {
    using SafeMath for uint256;
    ...
    constructor(IBEP20 _kawaiiToken, uint256 _minFee, uint256 _maxFee) public {
    ...
    function delivery(address _nft1155Address, uint256[] memory _tokenIds, uint256[] memory _rateItems, uint256[] memory _amounts, uint256 timestamp, address sender, bytes memory adminSignedData, uint8 v, bytes32 r, bytes32 s) external whenNotPaused {
        ...
        require(totalReward >= minFee && totalReward <= maxFee, "Invalid reward");
        IBEP20(kawaiiToken).safeTransfer(sender, totalReward);
