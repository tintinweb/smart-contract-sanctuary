    ...
    ERC20("Genesis NFT Pool", "GNFT-BSCS")
{
   ...
   //// extra state for genesis nft staking
    ...
    function updateTypePrice(uint256 _typeId, uint256 _price) public onlyOwner {
        typeToPrice[_typeId] = _price; 
    }
    //Nft to value of BSCS
    mapping(uint256 => uint256) private nftIdToBscsAmount;
  ...
  //// end extra info for genesis nft staking
    ...
    function deposit(uint256 _nftId, address _nftAddress) external nonReentrant {
        ...
        require(nft.nftType == 1 || nft.nftType == 2 || nft.nftType == 3 || nft.nftType == 4, 'Invalid NFT type');
    ...
    // type 1: skybar, 2: rocket, 3: moon, 4: mars
      ...
      onlyOwner
    {
        ...
        uint256 _price = typeToPrice[_nftType];
        require(_price > 0, "Please first setup nft type price");
        require(IERC721(_tokenAddress).ownerOf(_nftId) == _nftOwner, "Wrong token owner");
    ...
    function getStakedAmount(address _userAddr) public view returns(uint256) {
        return userStakedAmount[_userAddr];
    }
