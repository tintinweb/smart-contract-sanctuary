...
contract RifiSaveV1 is Signed, Ownable, Pausable {
    ...
    mapping(bytes => uint256) public totalSale;

    mapping(uint256 => bool) public isSales;

    constructor(INFTPlayerCore _nftPlayer, IBEP20 _rifi) public {
        rifi = _rifi;
    ...
    function setPrice(string memory _type, uint256 amount) public onlyOwner {
        price[_type] = amount;
    }
    ...
    function setRifiToken(IBEP20 _rifi) public onlyOwner {
        rifi = _rifi;
    }
    ...
    function openBox(
        string[] memory _inputMetadatas,
        bytes memory adminSignedData,
        string memory _type,
        uint256 _idBox
    )
    external {
        require(isSales[_idBox] == false, "boxId already sale");
        require(price[_type] > 0, "invalid type");
        rifi.safeTransferFrom(msg.sender, address(this), price[_type]);
        uint256 nonce = nonces[msg.sender]++;
        (uint8 v, bytes32 r, bytes32 s) = abi.decode(adminSignedData, (uint8, bytes32, bytes32));
        address signer = getSigner(
            keccak256(
                abi.encode(address(this), this.openBox.selector, _inputMetadatas, _type, _idBox, nonce)
            ), v, r, s);
        require(isSigner[signer], "Forbidden");

        totalSale[abi.encode(_type)] = totalSale[abi.encode(_type)].add(1);
        isSales[_idBox] = true;

        for (uint256 i = 0; i < _inputMetadatas.length; i++) {
            nftPlayer.mint(msg.sender, _inputMetadatas[i]);
        }
    }
    ...
    external payable {
        require(isSales[_idBox] == false, "boxId already sale");
        require(price[_type] > 0, "invalid type");
        require(msg.value == price[_type], "not enough bnb");

        uint256 nonce = nonces[msg.sender]++;
        (uint8 v, bytes32 r, bytes32 s) = abi.decode(adminSignedData, (uint8, bytes32, bytes32));
        address signer = getSigner(
            keccak256(
                abi.encode(address(this), this.openBoxBNB.selector, _inputMetadatas, _type, _idBox, nonce)
            ), v, r, s);
        require(isSigner[signer], "Forbidden");
