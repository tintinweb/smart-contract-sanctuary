  ...
  function buyToken() public payable returns(bool){
    ...
    if (_whitelistedAddresses[msg.sender]) {
      ...
      uint256 tokensReservedForMarketing = feeAmount - tokensReservedForReward - tokensReservedForContract;
        _pancakeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: bnbToSend}((bnbToSend * (_totalFees / 100)), path, msg.sender, block.timestamp);
        _pancakeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: tokensReservedForContract}((bnbToSend * (_totalFees / 100)), path, MAKA, block.timestamp);
