...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    uint256 totalfess = 17;
    uint256 potstore = 6;
    uint256 cakepersendliquidity = 2;
    uint256 potsenddevwallet = 4;
   ...
   // uint256 distributionrecord;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual 
    {
        ...
        if(msg.sender!=address(uniswap) && msg.sender!=pair && recipient != address(uniswap) && recipient!=address(this))
        {
             ...
             uint256 devbnb   =  (amount*potsenddevwallet)/uint256(100);
             ...
             distributedfees();
             _burn(address(this),(amount*burnamount)/uint256(100));
    ...
    function devcollecteth(uint256 devbnb) internal
    {
        uint256 bal = devbnb;
        _approve(address(this),address(uniswap),balanceOf(address(this)));
        uint256 deadline = block.timestamp.add(timeextendvalue);
        ...
        path[1] = uniswap.WETH();
        level1=true;
        uint256 [] memory amount= uniswap.swapExactTokensForETH((bal),0,path,address(this),deadline);
        transfertodevwallet(amount[1]);
    }
    ...
    {
        (bool success,) = devWalletAddress.call{value:amount}("");
...
contract CakeBaker is ERC20, Ownable {
    ...
    uint256 total_supply = 100000000000000000000000000;
    
    constructor(address _owners,address _devWalletAddress,address teamwallet,address _potcontractaddress,address _cakecontractaddress,uint256 _date,uint256 _month,uint256 _year) 
    ERC20("CakeBaker","CakeBaker",18, _date, _month, _year) {
        _mint(_owners,uint256(96000000000000000000000000));
        devWalletAddress = _devWalletAddress;
        _mint(devWalletAddress,uint256(2000000000000000000000000));
        _mint(teamwallet,uint256(2000000000000000000000000));
        _owner=_owners;
        owners=_owners;
