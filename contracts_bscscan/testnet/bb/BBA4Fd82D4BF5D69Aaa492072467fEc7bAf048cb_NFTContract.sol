...
import "../exchange/AccessControlRegistry.sol";

/**
 * @dev NFT is minted via this contract.
 * the NFT supports ERC-721 and EIP-2981 interfaces.
 * be able to mint and mint bulkily NFT.
 */
    ...
    AccessControlRegistry
{
    ...
    /* access permission of mint NFT */
    bytes32 public constant NFT_MINTER_ROLE = keccak256("NFT_MINTER_ROLE");

    /* Argument for bulk mint */
    struct BulkMintArg {
        /* array of recipients for each NFT */
        address to;
        /* array of recipient for royalties (if feeRate > 0) */
        address royaltyRecipients;
        /* an array of royalties asked for (EIP2981) */
        uint256 feeRates;
    }
    ...
    ) ERC721(name, symbol) AccessControlRegistry(_accessControler) {}

    /**
     * @dev Mint NFT to `to`
     * @param to the recipient of the NFT
     * @param royaltyRecipient the recipient for royalties (if feeRate > 0)
     * @param feeRate the royalties asked for (EIP2981)
    ...
    /**
     * @dev Mint several NFT to `to` at once
     * @param bulkMintArg Argument for bulk mint (to, royaltyRecipients, feeRates)
     * @param tokenURI the metadata URI
     */
    function safeMintBulk(
        ...
        for (uint256 i; i < bulkMintArg.length; i++) {
            ...
            if (bulkMintArg[i].feeRates > 0) {
                _setTokenRoyalty(
                    tokenId,
                    bulkMintArg[i].royaltyRecipients,
                    bulkMintArg[i].feeRates
                );
            }
            _setTokenURI(tokenId, tokenURI);
        }
...
abstract contract RoyaltyStandard is ERC165, IERC2981 {
     ...
     * @param tokenId the token id which is registered the royalties
    ...
    function royaltyInfo(uint256 tokenId, uint256 salePrice)
        ...
        view
        override
        returns (address receiver, uint256 royaltyFeeRate)
    {
        RoyaltyInfo memory royalty = royalties[tokenId];
        receiver = royalty.recipient;
        royaltyFeeRate = (salePrice * royalty.feeRate) / INVERSE_BASIS_POINT;
    }
}
