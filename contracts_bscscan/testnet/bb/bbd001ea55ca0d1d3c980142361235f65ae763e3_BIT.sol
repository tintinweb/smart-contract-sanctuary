...
contract BIT is IERC20, Ownable {
    ...
    modifier launchRestrict(address sender, address recipient, uint256 amount) {
        ...
        if (state == State.Restricted) {
            require(amount <= maxRestrictionAmount, "BIT: amount greater than max limit in restricted mode");
            if (!isWhitelisted[sender] && !isWhitelisted[recipient]) {
                require(lastTx[sender].add(60) <= block.timestamp && lastTx[recipient].add(60) <= block.timestamp, "BIT: only one tx/min in restricted mode");
                lastTx[sender] = block.timestamp;
                lastTx[recipient] = block.timestamp;
            } else if (!isWhitelisted[recipient]) {
                require(lastTx[recipient].add(60) <= block.timestamp, "BIT: only one tx/min in restricted mode");
                lastTx[recipient] = block.timestamp;
            } else if (!isWhitelisted[sender]) {
                require(lastTx[sender].add(60) <= block.timestamp, "BIT: only one tx/min in restricted mode");
                lastTx[sender] = block.timestamp;
            }
        }
        _;
