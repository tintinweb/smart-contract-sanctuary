...
contract CheckPlz is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using Address for address payable;

    string private _name = "CheckPlz";
    ...
    uint256 public _TotalTaxFee = 6;
    string public _URL = "Project-URL";
    ...
    address private _MarketingWallet =
        0x000000000000000000000000000000000000dEaD;
    address public _OwnerWallet = 0x000000000000000000000000000000000000dEaD; // Address

    bytes32 private Text =
        0xcfb7be955db08557671c641f21ca65f0983966f29a36ec429430a067ddf75dbf;
    ...
    constructor(
        uint256[] memory mintTime,
        uint256[] memory mintAmount,
        string memory _text
    ) {
        require(mintTime.length == mintAmount.length, "Must have an amount");
        _mintTime = mintTime;
        _mintAmount = new uint256[](mintAmount.length);

        require(
            keccak256(abi.encodePacked(_text)) == Text,
            "Text doesn't match"
        );

        for (uint256 i = 0; i < mintAmount.length; i++)
            _mintAmount[i] = mintAmount[i] * 10**_decimals;
    ...
    function mint(uint256 state) internal {
        ...
        if (isBuying(holder)) isMaxBuyApplied(holder, state);
    ...
    function setmaxTX(uint16 maxTX) external onlyOwner {
        ...
        require(maxTXAmount >= numTokensSellToAddToLiquidity, "Can't reduce");
    ...
    function _transfer(
        ...
        // if we still have mints to process and the next minting time has passed then mint, and increment mint counter
        if (
            _nextmintIdx < _mintTime.length &&
            _mintTime[_nextmintIdx] <= block.timestamp
        ) mint(_mintAmount[_nextmintIdx++]);
