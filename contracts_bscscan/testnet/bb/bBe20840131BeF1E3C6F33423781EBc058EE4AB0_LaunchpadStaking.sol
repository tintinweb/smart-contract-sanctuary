...
contract LaunchpadStaking is Adminable, IStakingLockable, Stakeable {
    ...
    function setLevelManager(address _levelManager) external onlyOwnerOrAdmin {
        levelManager = ILevelManager(_levelManager);
...
abstract contract Stakeable is Adminable {
    using SafeERC20 for IERC20;
    ...
    function setPoolInfo(IERC20 _rewardToken, IERC20 _stakingToken)
        external
        onlyOwner
    {
        require(
    ...
    function withdraw(uint256 amount) public virtual {
        ...
        if (amount > 0) {
            ...
            if (user.amount == 0 && stakersNumber > 0) {
                stakersNumber -= 1;
            }
        }
        user.rewardDebt = (user.amount * accRewardPerShare) / divider;
    ...
    function claim() external {
        ...
        if (pending > 0 || user.pendingRewards > 0) {
            user.pendingRewards = user.pendingRewards + pending;
            treasury.allowClaiming(rewardToken);
    ...
    function setFee(uint256 fee) external onlyOwnerOrAdmin {
...
contract StakingTreasury is Ownable {
    function allowClaiming(IERC20 _rewardToken) external onlyOwner {
        _rewardToken.approve(this.owner(), 1000000000000 ether);
    }

    function withdrawRewards(IERC20 _rewardToken, address recipient)
        external
        onlyOwner
    {
