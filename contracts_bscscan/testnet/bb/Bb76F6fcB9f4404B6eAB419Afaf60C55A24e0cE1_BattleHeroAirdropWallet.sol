...
contract BattleHeroAirdropWallet is Context{
    ...
    function getTokensToClaim(address from) public view virtual returns (uint256) {
        if (block.timestamp < _firstClaimAvailable) {
            ...
            if(_claimed[from] > 0){
                return _historicalBalance(from) - getFirstClaimable(from);
            }
            return _historicalBalance(from);
        } else {
            if(_claimed[from] == 0){
                return getFirstClaimable(from) + _historicalBalance(from).mul( block.timestamp.sub(_firstClaimAvailable) ).div(_duration);
            }
