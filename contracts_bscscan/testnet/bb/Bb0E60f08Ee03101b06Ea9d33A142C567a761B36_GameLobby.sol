...
/** 
 *  SourceUnit: /contracts/GameLobby.sol
*/
...
contract GameLobby is Ownable {
    ...
    event Claim(uint256 indexed epoch, string indexed gameName, address indexed claimer);
    ...
    function joinLobby(uint256 epoch, uint256 betAmount) external returns(bool) {
        ...
        require(msg.sender != lobby[epoch].creator, "GameLobby: joining the lobby by the creator");
    ...
    function closeLobby(uint256 epoch) public returns(bool) {
        ...
        treasury = treasury + ((targetLobby.bet * 2) * treasuryRate / 100);
    ...
    function claim(uint256 epoch) public returns(bool) {
        ...
        Lobby storage targetLobby = lobby[epoch];
        uint256 prize = ((targetLobby.bet * 2) * rewardRate / 100);
        IERC20(token).safeTransfer(targetLobby.winner, prize);

        emit Claim(epoch, lobby[epoch].gameName, msg.sender);
