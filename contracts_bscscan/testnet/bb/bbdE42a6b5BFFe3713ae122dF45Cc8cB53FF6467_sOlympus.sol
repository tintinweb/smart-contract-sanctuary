...
contract sOlympus is IsOHM, ERC20Permit {
    ...
    event LogStakingContractUpdated(address stakingContract, address stakingV1Contract);
    ...
    address public stakingV1Contract; // v1 contract 
    ...
    // do this last
    function initialize(address _stakingContract, address _stakingV1Contract) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");

        require(_stakingContract != address(0), "Staking");
        stakingContract = _stakingContract;

        require(_stakingV1Contract != address(0), "StakingV1");
        stakingV1Contract = _stakingV1Contract;

        _gonBalances[stakingContract] = TOTAL_GONS;

        emit Transfer(address(0x0), stakingContract, _totalSupply);
        emit LogStakingContractUpdated(stakingContract, stakingV1Contract);
    ...
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(IsOHM(stakingV1Contract).circulatingSupply());
