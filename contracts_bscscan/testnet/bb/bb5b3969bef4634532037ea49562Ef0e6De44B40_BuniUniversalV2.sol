...
contract BuniUniversalV2 is Initializable, AccessControlUpgradeable {
    ...
    // TrainersV2 public constant TRAINER_CONTRACT = TrainersV2(0x03DC86ab10011B02c5660263e344de84C6126ea0);
    // BunicornsV2 public constant BUNICORN_CONTRACT = BunicornsV2(0x32F487f404EAa05541D1F2F69a279c1D8e3f6813);
    // ITrainerRoller public constant TRAINER_ROLLER_CONTRACT = ITrainerRoller(0xDe6b8aF4bFFbA5Ab62BdB445439252653fA9c3E6);
    // IBunicornRoller public constant BUNICORN_ROLLER_CONTRACT = IBunicornRoller(0x623c1F377Aa79695bCa12A1ebd5D6E266971980E);
    ...
    // INFTNameTag public constant NAME_TAG_CONTRACT = INFTNameTag(0x59803E09606483e75A5010dB36C331b0A179192E);
    // EventTicket public constant EVENT_TICKET_CONTRACT = EventTicket(0x70a931eD026Da46B4Ab3116bd0B96C0582324791);

    // External contracts configuration for dev
    TrainersV2 public constant TRAINER_CONTRACT = TrainersV2(0x72179dB9633c2bF8862234a9Cd7dB35Fd0f08662);
    BunicornsV2 public constant BUNICORN_CONTRACT = BunicornsV2(0xe9E4F0027912Bcad386749Ed9ce9C5e0B68c757F);
    ITrainerRoller public constant TRAINER_ROLLER_CONTRACT = ITrainerRoller(0xaf8F08DF4D70f99d95E2264a379deadE758c0152);
    IBunicornRoller public constant BUNICORN_ROLLER_CONTRACT = IBunicornRoller(0x77532E5096D2f309e0A7c4fF29FC3eAF74AF4C64);
    IERC20 public constant BUNI_TOKEN_CONTRACT = IERC20(0x958b7F4D1eca6331C86D983b6229F6b23Ce2993a);
    IBurToken public constant BUR_TOKEN_CONTRACT = IBurToken(0x402bA638D4cc31e5bD1D2F36F66704a6C113e2Db);
    IPriceOracle public constant BUNI_PRICE_ORACLE = IPriceOracle(0x439B45Ef4F2eB800d066db1DC78085cb193123aF);
    IPriceOracle public constant BUR_PRICE_ORACLE = IPriceOracle(0x5FdbABfb1CaD851D4350962752D54B3e66cE2d22);
    ...
    EventTicket public constant EVENT_TICKET_CONTRACT = EventTicket(0xFb6316205A82FEEd3D18C1fC857D3A1A0654e1C2);
    ...
    /**
     * This define the upcoming & ongoing event
     * The convention: `{EVENT_TYPE_1_DIGIT}{COUNTER_2_DIGITS}`
     *   + Raid boss event has type=1
     *   + PVP event has type=2
     * For example:
     *   + eventId = 101 means RB1
     *   + eventId = 102 means RB2
     *   + eventId = 205 means PVP5
     * and so on...
     * The new event types will be determined later.
     */
    ...
    function _preMintBunicorns(int128 _feeInBuni, int128 _feeInBur, uint8 _times) internal view returns(uint256 _buniAmount, int128 _burAmount) {
        ...
        // check buni balance
        _buniAmount = usdToBuni(_feeInBuni.mul(ABDKMath64x64.divu(_times, 1)));
        require(BUNI_TOKEN_CONTRACT.balanceOf(msg.sender) >= _buniAmount, "insufficient buni");
        // check bur balance
        _burAmount = _feeInBur.mul(ABDKMath64x64.divu(_times, 1));
    ...
    function mintBunicorns(uint16 _gachaId, uint8 _times)
        public
        noEmergency
        onlyNonContract
        oncePerBlock(msg.sender)
    {
        // get fee
        (int128 _feeInBuni, int128 _feeInBur, uint256 _starsProbSpecs) = _getGachaSettings(_gachaId);
        (uint256 buniAmount, int128 burAmount) = _preMintBunicorns(_feeInBuni, _feeInBur, _times);
        ...
        for (uint256 i = 0; i < _times; i++) {
            ...
            BUNICORN_ROLLER_CONTRACT.mintOneRandomBunicornWithStarsProbSpecs(msg.sender, _seed, _starsProbSpecs);
        }
    ...
    /**
     * GACHA SPECS & CONVENTION
     * Just hard code for now until we can move all gacha logic out of game contract
     * Currently the available gachas are:
     *  1: normal common gacha:
     *    + probability: 51% / 33% / 13% / 2.7% / 0.3%
     *    + price: $25 buni + $25 bur
     *  2: normal epic gacha:
     *    + probability: 44% / 35% / 15% / 5% / 1%
     *    + price: $50 buni + $50 bur
     *
     * For event gacha, the convention is `{EVENT_ID_3_DIGITS}{COUNTER_2_DIGITS}`. For example:
     *  10101: rb1 event - TBD
     *  10201: rb2 event (gacha type 1) - TBD
     *  10202: rb2 event (gacha type 2) - TBD
     *  20101: pvp1 event - TBD
     *  20201: pvp2 event (gacha type 1) - TBD
     *  20202: pvp2 event (gacha type 2) - TBD
     *  20203: pvp2 event (gacha type 3) - TBD
     *  30101: sd1 event - TBD
     *  30201: sd2 event - TBD
     *  and so on..
     */

    // This return the USD that users have to pay for a given gachaId & the probabilities
    // See the probability specs in BunicornRoller
    function _getGachaSettings(uint16 _gachaId) private pure returns (int128 _usdBuni, int128 _usdBur, uint256 _starsProbSpecs) {
        if (_gachaId == 1) { // normal common gacha
            _usdBuni = ABDKMath64x64.divu(25, 1);
            _usdBur = ABDKMath64x64.divu(25, 1);
            _starsProbSpecs = (5100) | (3300 << 16) | (1300 << 32) | (270 << 48) | (30 << 64);
        }
        else if (_gachaId == 2) { // normal epic gacha
            _usdBuni = ABDKMath64x64.divu(50, 1);
            _usdBur = ABDKMath64x64.divu(50, 1);
            _starsProbSpecs = (4400) | (3500 << 16) | (1500 << 32) | (500 << 48) | (100 << 64);
        }
        else {
            require(false, "invalid gachaId");
        }
    }

    // For FE calls to retrieve & display gacha information
    function getGachaSettings(uint16 gachaId, uint8 times) external pure 
        returns (int128 usdBuni, int128 usdBur, uint16 prob1, uint16 prob2, uint16 prob3, uint16 prob4, uint16 prob5) {
        (int128 _usdBuni, int128 _usdBur, uint256 _starsProbSpecs) = _getGachaSettings(gachaId);
        usdBuni = _usdBuni.mul(ABDKMath64x64.fromUInt(times));
        usdBur = _usdBur.mul(ABDKMath64x64.fromUInt(times));
        prob5 = uint16((_starsProbSpecs >> 64) & 0xFFFF);
        prob4 = uint16((_starsProbSpecs >> 48) & 0xFFFF);
        prob3 = uint16((_starsProbSpecs >> 32) & 0xFFFF);
        prob2 = uint16((_starsProbSpecs >> 16) & 0xFFFF);
        prob1 = uint16(_starsProbSpecs & 0xFFFF);
    }
...
interface IBunicornRoller {
    ...
    function mintOneRandomBunicornWithStarsProbSpecs(address minter, uint256 randomSeed, uint256 starsProbSpecs) external returns(uint256);
...
contract EventTicket is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function initialize () public initializer {
        ...
        EVENT_TICKET_FEE_ONLY_USD = ABDKMath64x64.divu(15, 1); // 15.0 USD
    }
...
contract BunicornsV2 is IBunicornsV2, Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    struct EventMultiplier {
        uint16 eventId;
        int128 boostMultiplier;
    }

    mapping(uint256 => EventMultiplier) private eventMultipliers;
    ...
    function getEventMultiplier(uint256 id) public view returns(uint16 eventId, int128 boostMultiplier) {
        EventMultiplier memory eventMultiplier = eventMultipliers[id];
        return (eventMultiplier.eventId, eventMultiplier.boostMultiplier);
    }
    // =========================== GETTER, SETTER ===========================
    ...
    function mintOneEventBunicornBySpecs(
        address _minter, uint16 _props,
        uint16 _attr1, uint16 _attr2, uint16 _attr3,
        int128 _eventMultiplier, uint16 _eventId
    )
        external override
        onlyGameContract
        notInEmergencyPause
        returns(uint256)
    {
        ...
        eventMultipliers[tokenID] = EventMultiplier(_eventId, _eventMultiplier);
    ...
    function getPowerMultiplierForBattleBoss(uint256 id, uint16 eventId) public view returns(int128) {
        ...
        EventMultiplier memory eventMultiplier = eventMultipliers[id];
        if (eventId != eventMultiplier.eventId) {
            factor = ABDKMath64x64.divu(35, 100); // 0.35, decrease 65%
        }
        return _powerMultiplier.mul(eventMultiplier.boostMultiplier).mul(factor);
    }
