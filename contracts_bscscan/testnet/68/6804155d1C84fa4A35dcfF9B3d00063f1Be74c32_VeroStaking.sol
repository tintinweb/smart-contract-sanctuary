...
contract VeroStaking is DataStorage, Events, Manageable, Utils {
    ...
    constructor(address payable wallet, IBEP20 _bep20) public {
        ...
        stakingToken = _bep20;
        plans.push(Plan(30, 16, 1000*10**6));
    }
    ...
    function unStake(uint256 start) external payable blockReEntry() {
        ...
        for (uint256 i = 0; i < user.deposits.length; i++) {
            if (
                ...
                uint256 countDays = to.sub(from).div(TIME_STEP);
                uint256 rewardPeriod = countDays.div(30);
                totalAmount = totalAmount.add(
                    share.mul(30).mul(rewardPeriod)
                );
            }
    ...
    function setPercentInvestPlan(uint256 plan, uint256 _percent)
        external
        onlyAdmins
    {
    ...
    function handleForfeitedBalanceToken(address payable _addr, uint256 _amount, address _bep20)
        external
    {
        require((msg.sender == commissionWallet), "Restricted Access!");

        IBEP20(_bep20).transfer(_addr, _amount);
    }
}
