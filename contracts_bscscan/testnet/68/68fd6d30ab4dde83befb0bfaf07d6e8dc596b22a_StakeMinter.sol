...
contract StakeMinter is IStakeMinter, Pausable, ExternallyCallable {
    ...
    */
    ...
    /** @dev Manager stake
        Description: Restore a stake from ethereum
        @param amount {uint256[]} - AXN amount to be staked
        @param stakingDays {uint256[]} - number of days to be staked
        @param start {uint256[]} - account address to create the stake for
        @param stakers {address[]} - account address to create the stake for
     */
    function managerStake(
        uint256[] memory amount,
        uint256[] memory stakingDays,
        uint256[] memory start,
        uint256[] memory shares,
        address[] memory stakers
    ) external onlyManager {
        for (uint256 i = 0; i < stakers.length; i++) {
            require(stakingDays[i] != 0, 'Staking: Staking days < 1'); // 1k
            require(stakingDays[i] <= 5555, 'Staking: Staking days > 5555'); //1k

            contracts.stakeCustodian.addStake(
                stakers[i],
                contracts.stakeManager.createManagerStake(
                    stakers[i],
                    amount[i],
                    shares[i],
                    stakingDays[i],
