...
contract DNB is Context, IERC20, Ownable {
    ...
    address payable private _developmentWalletAddress = payable(0x5b68F7d5E5672745d249A5d6695Dc5E28DC2415C);
    ...
    string private _name = "DoNotBuyTest";
    ...
    function updateDevelopmentWalletAddress(address payable developmentWalletAddress) external onlyOwner() {
     ...
     function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        FinalFees memory vals;
        (vals.tTransferAmount, vals.tReflection, vals.tLiquidity, vals.tDev, vals.tDevelopment, vals.tMarketing) = _getTValues(tAmount);
        (vals.rAmount, vals.rTransferAmount, vals.rReflection) = _getRValues(tAmount, vals.tReflection, vals.tLiquidity, vals.tDev, vals.tDevelopment, vals.tMarketing, _getRate());
        return (vals.rAmount, vals.rTransferAmount, vals.rReflection, vals.tTransferAmount, vals.tReflection, vals.tLiquidity, vals.tDev, vals.tDevelopment, vals.tMarketing);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        FinalFees memory vals;
        vals.tReflection = calculateReflectionFee(tAmount);
        vals.tLiquidity = calculateLiquidityFee(tAmount);
        vals.tDev = calculateDevFee(tAmount);
        vals.tDevelopment = calculateDevelopmentFee(tAmount);
        vals.tMarketing = calculateMarketingFee(tAmount);
        vals.tTransferAmount = tAmount.sub(vals.tReflection).sub(vals.tLiquidity).sub(vals.tDev);
        vals.tTransferAmount -= vals.tDevelopment;
        vals.tTransferAmount -= vals.tMarketing;
        return (vals.tTransferAmount, vals.tReflection, vals.tLiquidity, vals.tDev, vals.tDevelopment, vals.tMarketing);
    }

    function _getRValues(uint256 tAmount, uint256 tReflection, uint256 tLiquidity, uint256 tDev, uint256 tDevelopment, uint256 tMarketing, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        FinalFees memory vals;
        vals.rAmount = tAmount.mul(currentRate);
        vals.rReflection = tReflection.mul(currentRate);
        vals.rLiquidity = tLiquidity.mul(currentRate);
        vals.rDev = tDev.mul(currentRate);
        vals.rDevelopment = tDevelopment.mul(currentRate);
        vals.rMarketing = tMarketing.mul(currentRate);
        vals.rTransferAmount = vals.rAmount.sub(vals.rReflection).sub(vals.rLiquidity).sub(vals.rDev).sub(vals.rDevelopment).sub(vals.rMarketing);
        return (vals.rAmount, vals.rTransferAmount, vals.rReflection);
    }
    ...
    function removeAllFee() private {
        if (_reflectionFee == 0 && _liquidityFee == 0 && _devFee == 0 && _developmentfee == 0 && _marketingfee == 0) return;
    ...
    function _transfer(
        ...
        if (!_exemptFromMaxWallet[to]) require(balanceOf(to).add(amount) <= maxWalletBalance, 'Wallet balance is exceeding maxWalletBalance'); 
        if (!tradingIsEnabled) {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rReflection, uint256 tTransferAmount, uint256 tReflection, uint256 tLiquidity, uint256 tDev, uint256 tDevelopment, uint256 tMarketing) = _getValues(tAmount);
