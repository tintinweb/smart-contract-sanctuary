...
contract Vault is ERC20 {
    ...
    event ParameterUpdated(address, address, address, uint256, uint256);
    ...
    function setParameters(
        ...
        emit ParameterUpdated(company, stakers, algoDev, percentDev, maxCap);
    }
    ...
    function depositBase(uint256 amount) public {
        ...
        if (position == 1) {
            soldAmount = soldAmount.add(amounts[2]);
        }
    ...
    function sell() public {
        ...
        if (amount > 0) {
            ...
            if (profit > percentMax) {
                ...
                uint256 profitAmount = amount.mul(profit.sub(percentMax)).div(profit);
    ...
    function takePerformanceFeesFromBaseToken(uint256 amount) private returns(uint256) {
        ...
        uint256 ubxtAmt = _after.sub(_before);
        uint256 feeAmount = _tokenbBefore.sub(_tokenbAfter);
    ...
    function buyParaswap(address augustusAddr, bytes memory swapCalldata) public {
        ...
        require(augustusAddr != address(0));
