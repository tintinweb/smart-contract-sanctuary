...
contract Ownable is Context {
    ...
    //TODO: Check if contract-permission necessary when calling from inside _transfer
...
contract TestToken is Context, IERC20, Ownable {
    ...
    address payable public _marketingAddress = payable(0x579e7889ee18882ECD32c910340CCE7E032ffC9C); // Marketing Address
    ...
    uint256 public _maxWalletAmount = 1000000000 * 10**6 * 10**9;
    bool public _useCustomMaxTx;
    bool public _useCustomMaxTk;
    
    uint256 private _bigBurnAmount;
    uint256 private _burnModeAmount;
    uint256 private _preparationRatePercent;
    uint256 private _bigBurnBufferPercent;
    uint256 private _memeBurnAmount = 420000000000;
    uint256 private _memeBurnCost;
    uint256 private _memeBurnBuffer = 10;
    uint256 private _maxBurnMatchingContractPercent = 10; 
    
    uint256 private burnEnd;
    uint256 private _minBurnTime = 3 minutes;
    uint256 private _burnTimeExtensionFactor = 120;
    
    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;
    address public _LPLockAddress;
    
    bool _automaticBigBurn;
    bool inSwapAndLiquify;
    bool inBuyBack;
    bool public _accumulatingForBurn;
    bool public _burnModeActive;
    ...
    constructor () {
        ...
        _LPLockAddress = owner(); //Requires change before launch
    ...
    function _transfer(
        ...
        require(to != address(0) || inSwapAndLiquify || inBuyBack, "ERC20: transfer to the zero address");
        */
        ...
        if (justLaunched){
            if (block.timestamp <= launchTime.add(300)){          // minutes [0,5]
                maxTxAmount = 10000000000 * 10 ** 9;              // 10 billion max tx
                maxWalletAmount = _tTotal.div(10000);             // 0.01% max wallet
            }
            else if (block.timestamp <= launchTime.add(900)){     // minutes (5,15]
                maxTxAmount = 30000000000 * 10 ** 9;              // 30 billion max tx
                maxWalletAmount = _tTotal.div(3333);              // 0.03% max wallet
            }
            else if (block.timestamp <= launchTime.add(6000)){     // minutes (15,60]
                maxTxAmount = 100000000000 * 10 ** 9;             // 100 billion max tx
                maxWalletAmount = _tTotal.div(1000);              // 0.1% max wallet
            }
            else{
                justLaunched = false;
                maxTxAmount = 300000000000 * 10 ** 9;             // 300 billion max tx
                maxWalletAmount = _tTotal.div(100);               // 1.0% max wallet
            }
        }
        else{
            // Restrictions are related to circulating supply post-launch
            maxTxAmount = _useCustomMaxTx ? _maxTxAmount : currentSupply.div(3333);        
            maxWalletAmount = _useCustomMaxTk ? _maxWalletAmount : currentSupply.div(100); 
        }
        
        if(from != owner() && to != owner() && 
        from != address(this) && to != uniswapV2Pair) {   
            require(amount <= maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            require(walletTokens.add(amount) <= maxWalletAmount, "Wallet amount exceeds the maxWalletAmount.");
        }
    ...
    function swapTokens(uint256 tokenAmountCandidate) private lockTheSwap {
        ...
        if (_accumulatingForBurn){
            ...
            uint256 totalBurnBuffer =  _bigBurnAmount.mul(_bigBurnBufferPercent).div(100); // Total amount for BigBurn and following BurnMode
        ...
        _memeBurnCost = estimateEthCost(_memeBurnAmount);
        
        if (initialBalance < _memeBurnCost.mul(_memeBurnBuffer)){
            swapTokensForEth(tokensToSwap);
        }
        else{
            uint256 tokensToLP = tokensToSwap.mul(_liquidityFee).div(_marketingFee + _liquidityFee).div(2);
            swapTokensForEth(tokensToSwap - tokensToLP);
            uint256 ethToTransfer = address(this).balance.sub(initialBalance);
            uint256 ethToLP = ethToTransfer.mul(_liquidityFee).div(_marketingFee + _liquidityFee).div(2);
            transferToAddressETH(_marketingAddress, ethToTransfer - ethToLP);
    ...
    function swapETHForExactTokens(uint256 tokenAmount) private lockTheBuyBack{       //X necessary
        // Generate the uniswap pair path of token -> WETH
        address[] memory path = new address[](2);
        
        path[0] = uniswapV2Router.WETH();
        path[1] = address(this);
        uint256 costEstimate = estimateEthCost(tokenAmount).mul(2);
    ...
    */
    ...
    function burn(uint256 tAmount) private onlyAuthorized {
        if (_burnModeActive && block.timestamp > burnEnd){
    ...
    function prepareForBurn(bool automaticBigBurn, uint256 burnAmount, 
     uint256 bigBurnBufferPercent, uint256 preparationRatePercent,  
     uint256 maxBurnMatchingContractPercent) external onlyAuthorized {
    ...
    function bigBurn() public onlyAuthorized {
        if (_accumulatingForBurn){
    ...
    function bigBurnReady() public view onlyAuthorized returns(bool){
        return balanceOf(address(this)) >= _bigBurnAmount.mul(_bigBurnBufferPercent).div(100);
    }
    
    function burnMode(bool burnModeActive) public onlyAuthorized {
        if (_accumulatingForBurn){
            _accumulatingForBurn = false;
        }
        _burnModeActive = burnModeActive;
        if (_burnModeActive){
        burnEnd = block.timestamp  + _minBurnTime +  
    ...
    function setBurnModeSettings(uint256 minBurnTime, uint256 burnTimeExtensionFactor) external onlyOwner{
    ...
    function launch() external onlyOwner {
       // require( _LPLockAddress != owner(), "Assign appropriate LP locker address before launch" );  //TODO
    ...
    function setBuyFee(uint256 buyReflectionFee, uint256 buyLiquidityFee) external onlyOwner {
        _buyReflectionFee = buyReflectionFee;
        _buyLiquidityFee = buyLiquidityFee;
    }
   
    function setSellFee(uint256 sellReflectionFee, uint256 sellLiquidityFee) external onlyOwner {
        _sellReflectionFee = sellReflectionFee;
        _sellLiquidityFee = sellLiquidityFee;
    }
    
    function setAddressFee(address addr, bool enable, uint256 addressReflectionFee, uint256 addressLiquidityFee) external onlyOwner {
        _addressFees[addr].enable = enable;
        _addressFees[addr]._reflectionFee = addressReflectionFee;
        _addressFees[addr]._liquidityFee = addressLiquidityFee;
    }
    
    function setBuyAddressFee(address addr, bool enable, uint256 addressReflectionFee, uint256 addressLiquidityFee) external onlyOwner {
        _addressFees[addr].enable = enable;
        _addressFees[addr]._buyReflectionFee = addressReflectionFee;
        _addressFees[addr]._buyLiquidityFee = addressLiquidityFee;
    }
    
    function setSellAddressFee(address addr, bool enable, uint256 addressReflectionFee, uint256 addressLiquidityFee) external onlyOwner {
        _addressFees[addr].enable = enable;
        _addressFees[addr]._sellReflectionFee = addressReflectionFee;
        _addressFees[addr]._sellLiquidityFee = addressLiquidityFee;
    }
    ...
    function setMaxTxAmount(bool useCustomMaxTx, uint256 maxTxAmount) external onlyOwner {
        _useCustomMaxTx = useCustomMaxTx;
        _maxTxAmount = maxTxAmount;
    }
    function setMaxTokenAmount(bool useCustomMaxTk, uint256 maxWalletAmount) external onlyOwner {
        _useCustomMaxTk = useCustomMaxTk;
    ...
    function setLPAddress(address LPLockAddress) external onlyOwner {
        _LPLockAddress = LPLockAddress;
    }
