...
contract Microtest is Ownable, IBEP20 {
    ...
    address private _projectDev = payable(0x5DCF0bb58eBa8AA185F956291518FBfE35dcAB16);
	address private _donationAddress = payable(0x9574546c24EC049D9eEfDd54043d3A89A5c9242b);
    
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tokenTotal = 99999999000000000000000000;
    uint256 private _reflectTotal = (MAX - (MAX % _tokenTotal));
    uint256 private _tokenFeeTotal;
    
    
    string public _name = "Microtest";
    string public _symbol = "MAA";
	...
	address public donationFeeAddress;
    ...
    constructor () {
        ...
        devFeeAddress = _projectDev;
		donationFeeAddress = _donationAddress;
        ...
        // _pancakeFactory = IPancakeFactory(_pancakeRouter.factory()).createPair(address(this), _pancakeRouter.WETH());
        
        //    address _pancakeFactory = payable(0x6725F303b657a9451d8BA641348b6761A6CC7a17);
        ...
        _isExcludedFromFee[_projectDev] = true;
	...
	function setDonationFee(uint256 dontaionFee) external onlyOwner() {
        _donationFee = dontaionFee;
    }
    ...
    function removeAllFee() private {
        if(_holderFee == 0 && _liquidityFee == 0 && _devFee == 0) return;
    ...
    function calculateDistributeFee(uint256 _amount) private view returns (uint256) {
        return _amount * _holderFee / 10**2;
    }
	...
	function calculateDonFee(uint256 _amount) private view returns (uint256) {
        return _amount * _devFee / 10**2;
    }
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 transferAmount) private {
		...
		_takeDevFee(feeDev);
		_takeDonationFee(feeDon);
        _takeLiquidity(feeLiquidity);
        _reflectFee(reflectFee, feeDistribute, feeDev, feeDon);
		
        emit Transfer(sender, recipient, tokenTransferAmount);
        emit Transfer(sender, _projectDev, feeDev);
		emit Transfer(sender, _donationAddress, feeDon);
    ...
    function _getValues(uint256 transferAmount) private view returns ( uint256 reflectAmount,uint256 reflectTransferAmount,uint256 reflectFee,uint256 tokenTransferAmount,uint256 feeDistribute,uint256 feeLiquidity,uint256 feeDev,uint256 feeDon) 
    {

        (tokenTransferAmount,feeDistribute,feeLiquidity,feeDev,feeDon) = _getTValues(transferAmount);

        (reflectAmount,reflectTransferAmount,reflectFee) = _getRValues(transferAmount,feeDistribute,feeLiquidity+feeDev+feeDon,_getRate());
       // return (reflectAmount, reflectTransferAmount, reflectFee, tokenTransferAmount, feeDistribute, feeLiquidity);
    }
    
    

    
    function _getTValues(uint256 transferAmount)private view returns (uint256, uint256, uint256, uint256, uint256) {
		...
		uint256 feeDon = calculateDonFee(transferAmount);

        uint256 tokenTransferAmount = transferAmount - feeDistribute -  feeLiquidity - feeDev - feeDon;

        return (tokenTransferAmount,feeDistribute,feeLiquidity,feeDev,feeDon);
    }
    
    function _getRValues( uint256 transferAmount, uint256 feeDistribute, uint256 feeTotal,uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function _takeDevFee(uint256 tDevFee) private {
        ...
        _reflectOwned[devFeeAddress] = _reflectOwned[devFeeAddress].add(rDevFee);
        if(_isExcluded[devFeeAddress])
            _tokenOwned[devFeeAddress] = _tokenOwned[devFeeAddress].add(tDevFee);
    }
	
	  function _takeDonationFee(uint256 tDonFee) private {
        uint256 currentRate =  _getRate();
        uint256 rDonFee = tDonFee.mul(currentRate);
        _reflectOwned[donationFeeAddress] = _reflectOwned[donationFeeAddress].add(rDonFee);
        if(_isExcluded[donationFeeAddress])
            _tokenOwned[donationFeeAddress] = _tokenOwned[donationFeeAddress].add(tDonFee);
    }
    ...
    function _reflectFee(uint256 reflectFee, uint256 feeDistribute, uint256 feeDev, uint256 feeDon) private {
        _reflectTotal = _reflectTotal - reflectFee;
        _tokenFeeTotal = _tokenFeeTotal + feeDistribute + feeDev + feeDon;
