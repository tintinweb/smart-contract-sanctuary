...
contract LuckyBlock is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using SafeCast for int256;

    string private _name = "LuckyBlock";
    string private _symbol = "LBlock";
    ...
    uint256 public lotteryPoolFee = 400;
    uint256 public nftRoyalPoolFee = 300;
    ...
    uint256 public maxTxAmount =
        _tokenTotal.mul(maxTxAmountPercentage).div(10000); // 0.05%
    ...
    constructor(address router) {
        // IPancakeswapV2Router02 _uniswapV2Router = IPancakeswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // Uniswap Router For Ethereum
        ...
        uniswapV2Router = _pancakeSwapV2Router;
    ...
    function setLotteryPoolFee(uint256 fee) external onlyOwner {
        lotteryPoolFee = fee; //4
    }

    function setNFTPoolFee(uint256 fee) external onlyOwner {
        nftRoyalPoolFee = fee; //4
    }
    ...
    function setTeamWallet(address wallet) external onlyOwner {
     ...
     function setDistributionWallet(address wallet) external onlyOwner {
        distributionWallet = wallet;
    }

    function setNFTPoolWallet(address wallet) external onlyOwner {
        nftRoyalPoolWallet = wallet;
    }

    function setLotteryPoolWallet(address wallet) external onlyOwner {
        lotteryPoolWallet = wallet;
    }
    ...
    function setSellCooldonwEnabled(bool value) external onlyOwnerAndAdmin {
    ...
    function claim(address payable destination) public onlyOwner {
