...
contract RonaCarrier is OwnableContext {
    ...
    // Fee Percentages
    uint256 private _feeServicePercentage;
    uint256 private _feeCharityPercentage;
    uint256 private _feeLiquidityPoolPercentage;
    uint256 private _feeHoldersPercentage;
    ...
    //address[] private _blockDistributionRecievers;
    //mapping(address => uint256) private _blockDistributionRecieversMap;
    //mapping(address => uint256) private _blockDistributionDebts;
    ...
    constructor(address carrierTokenAddress, address claimableTokenAddress) {
        ...
        _maxPurchaseRate = 7000000000*_carrierTokenDecimalFactor;

        //initialize fees and recievers
        _feeServicePercentage = 1;
        _feeCharityPercentage = 3;
        _feeLiquidityPoolPercentage = 3;
        _feeHoldersPercentage = 3;

        _serviceFeeReciever = address(0x98944821EA2db7030F7329640344F117A188588b);
        _charityFeeReciever = address(0x98944821EA2db7030F7329640344F117A188588b);

        // initialize purchase and liquidation
        _purchaseIsOpen = true;
        _liquidationIsOpen = true;
    ...
    function _block(uint256 distributionAmount, address blockInitiator, address blockCloser) internal {
        ...
        // TODO: Implement final version of blocking method
        // ...
    ...
    function carry(address from, address to, uint256 amount) external returns (bool) {
        ...
        // TODO: handle liquidity pool fee
    ...
    function updatePurchaseIsOpen(bool isPurchaseOpen) external onlyOwner returns (bool) {
        _purchaseIsOpen = isPurchaseOpen;
        return true;
    }

    function updatePurchaseRateMax(uint256 newPurchaseRateMax) external onlyOwner returns (bool) {
    ...
    function _liquidationRateDeducted(uint256 deduction) internal view returns (uint256) {
        return address(this).balance.mul(_carrierTokenDecimalFactor).div(_carrierToken.totalSupply().sub(_carrierToken.balanceOf(address(this)).add(deduction)));
    }
    ...
    function updateLiquidationIsOpen(bool isLiquidationOpen) external onlyOwner returns (bool) {
    ...
    function updateClaimIsOpen(bool isClaimOpen) external onlyOwner returns (bool) {
    ...
    function serviceFeeReciever() public view returns (address) {
        return _serviceFeeReciever;
    }

    function charityFeeReciever() public view returns (address) {
        return _charityFeeReciever;
    }

    // TODO add update methods for fee recievers
