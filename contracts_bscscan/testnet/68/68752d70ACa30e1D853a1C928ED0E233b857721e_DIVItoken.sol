...
contract DIVItoken is ERC20, Ownable {
    ...
    DIVItokenDividendTracker public dividendTracker;

    bool public presaleOver = true;
    ...
    address payable public marketingWallet = 0x460742e4025a6256DF01d03c96555107A63d6B08;

    uint256 public swapTokensAtAmount = 200000 * (10**18);
    uint256 public maxSellTransactionAmount = 1000000000 * (10**18);

    uint256 public BNBRewardsFee = 30;
    uint256 public DIVIRewardsFee = 30;
    ...
    constructor() ERC20("DIVItoken", "DIVItoken") {
        
        totalFees = BNBRewardsFee.add(liquidityFee);


    	dividendTracker = new DIVItokenDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DIVItoken: The router already has that address");
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DIVItoken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DIVItoken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DIVItoken: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 1000000, "DIVItoken: gasForProcessing must be between 200,000 and 1,000,000");
        require(newValue != gasForProcessing, "DIVItoken: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "DIVItoken: This account cannot send tokens until trading is enabled");
        }
        ...
        if(DIVIRewardsFee > 0 && !_isExcludedFromFees[to] && !swapping && !automatedMarketMakerPairs[to]) { // FEE DIVI TOKENS
            uint256 feesDIVI = amount.mul(DIVIRewardsFee).div(100);
        	amount = amount.sub(feesDIVI);
        	super._transfer(from, address(dividendTracker), feesDIVI);
        }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> DIVI swap when swap+liquify is triggered
...
contract DIVItokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DIVI_Track", "DIVI_Track") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 1 * (10**18); //must hold 1+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DIVI_Track: No transfers allowed");
    }
    ...
    function getMinimumBalanceToReceiveDividends() view public returns(uint256) {
        return minimumTokenBalanceForDividends;
    }

    function withdrawDividend() pure public override {
        require(false, "DIVI_Track: withdrawDividend disabled. Use the 'claim' function on the main DIVItoken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 900 && newClaimWait <= 86400, "DIVI_Track: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DIVI_Track: Cannot update claimWait to same value");
