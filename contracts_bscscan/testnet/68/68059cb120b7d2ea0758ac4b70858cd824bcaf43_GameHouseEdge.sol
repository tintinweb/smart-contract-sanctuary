...
contract GameHouseEdge {
    ...
    address[] public BUST_BUSD_path = [0x6e03884333a30eE91AFda92E429fF4FD95Dc2850,0x637F61C18Cd7259f7c5EA50591C7Befe6A2E0BfE];
    address[] public WBNB_BUST_path= [0x44Bc761E0B58Aa6727202eBd2B636DC924dA9f1a, 0x6e03884333a30eE91AFda92E429fF4FD95Dc2850];
    address[] public WBNB_APE_path;// = [0x44Bc761E0B58Aa6727202eBd2B636DC924dA9f1a, 0xe0c2a41a988E1bec6945deE4236bfF0e2e78e448];
    address[] public WBNB_MFRM_path;//= [0x44Bc761E0B58Aa6727202eBd2B636DC924dA9f1a, 0xeFc55FfA42D02dc3fC14B5a64cf0D174cdBA3324];
    address[] public BUST_WBNB_path= [0x6e03884333a30eE91AFda92E429fF4FD95Dc2850,0x44Bc761E0B58Aa6727202eBd2B636DC924dA9f1a];
    address[] public BUSD_WBNB_path=[0x637F61C18Cd7259f7c5EA50591C7Befe6A2E0BfE,0x6e03884333a30eE91AFda92E429fF4FD95Dc2850,0x44Bc761E0B58Aa6727202eBd2B636DC924dA9f1a];
    
    
    
    constructor(address _bust, address _busd, address _wbnb,address busd_bust_lp,address bust_bnb_lp, address _router, address _reward,address _lock,address payable _dao) public {
         owner = msg.sender;
         reward = _reward;
         ...
         BUSD  = IERC20(_busd);
         WBNB  = IERC20(_wbnb);
         BUSD_BUST =IBustPair(busd_bust_lp);
         BUST_BNB =IBustPair(bust_bnb_lp);
         lock = _lock;
         dao = _dao;
     ...
     function setTokenAddress(uint256 _bust, uint256 _busd, address _wbnb) public restricted(){
         BUST  = IERC20(_bust);
         BUSD  = IERC20(_busd);
         WBNB  = IERC20(_wbnb);
    ...
    function setLPAddress(address busd_bust, address bust_bnb) public restricted(){
        BUSD_BUST =IBustPair(busd_bust);
        BUST_BNB =IBustPair(bust_bnb);
    ...
    function setPathBNBAPE(address[] memory bnb_ape)public restricted(){
        WBNB_APE_path=bnb_ape;
    }
    
    function setPathBNBMFRM(address[] memory bnb_mfrm)public restricted(){
        WBNB_MFRM_path=bnb_mfrm;
    }
    
    function setPathBUSTBUSD(address[] memory _path)public restricted(){
        BUST_BUSD_path=_path;
    }
    
    function setPathBNBBUST(address[] memory _path)public restricted(){
        WBNB_BUST_path=_path;
    }
    
    function setPathBUSTBNB(address[] memory _path)public restricted(){
        BUST_WBNB_path=_path;
    }
    
    function setPathBUSDBNB(address[] memory _path) public restricted(){
        BUSD_WBNB_path=_path;
    }
    ...
    function setRouterAddress(address _routerAddress) public restricted(){
        router=IBustRouter02(_routerAddress);
    }
    ...
    function distribution() public restricted() {
        ...
        if(DistributedReward>0){
              payable(reward).transfer(DistributedReward); 
        }
         
        if(DistributedBNBToLockLP>0){
                ...
                _getBUSTforBNB(SubDistributedBNBBal,address(this));//convert 75 BNB into BUST
        ...
        if(p3BUST>0){
           _getBUSTforBNB(p3BUST,0x000000000000000000000000000000000000dEaD); 
          
        }  
        
        if(p3APE>0){
            _getAPEforBNB(p3APE,0x000000000000000000000000000000000000dEaD);
            
        }
        
        if(p3MFRM>0){
           _getMFRMforBNB(p3MFRM,0x000000000000000000000000000000000000dEaD);
           
        }
        
        if(DistributedBNBToDaoTreasury>0){
            _daoTreasuryBankRoll(DistributedBNBToDaoTreasury);
        }
        
        if(BUST.balanceOf(address(this))>0){
            _getBNBForBUST(BUST.balanceOf(address(this)));
        }
        
        if(BUSD.balanceOf(address(this))>0){
            _getBNBForBUSD(BUSD.balanceOf(address(this)));
        }
        emit gameHouseDistribution();
        
    }
    
    function _getBUSTforBNB(uint256 _amount,address _addr) internal  {
        uint256[] memory bustArray = router.getAmountsOut(_amount,WBNB_BUST_path);
        uint bust=bustArray[bustArray.length - 1];
    ...
    function _getBUSDforBUST(uint256 _amount) internal {
        uint256[] memory busdArray = router.getAmountsOut(_amount,BUST_BUSD_path);
        uint busd=busdArray[busdArray.length-1];
    ...
    function _getMFRMforBNB(uint256 _amount,address _addr) internal {
        uint256[] memory amountArray = router.getAmountsOut(_amount,WBNB_MFRM_path);
        uint mfrm=amountArray[amountArray.length -1];
    ...
    function _getAPEforBNB(uint256 _amount,address _addr) internal {
        uint256[] memory apeArray = router.getAmountsOut(_amount,WBNB_APE_path);
        uint ape=apeArray[apeArray.length-1];
    ...
    function _getBNBForBUST(uint _amount) internal{
        uint256[] memory bnbArray = router.getAmountsOut(_amount,BUST_WBNB_path);
        uint bnb=bnbArray[bnbArray.length-1];
        uint256 bnbmin = bnb.mul(snum).div(sden);
        uint256 time = block.timestamp + 1120;
     ...
     function _getBNBForBUSD(uint _amount) internal{
        uint256[]  memory bnbArray = router.getAmountsOut(_amount,BUSD_WBNB_path);
        uint bnb=bnbArray[bnbArray.length-1];
        uint256 bnbmin = bnb.mul(snum).div(sden);
        uint256 time = block.timestamp + 1120;
        BUSD.approve(address(router),_amount);
        router.swapExactTokensForETH(
            _amount,
            bnbmin,
            BUSD_WBNB_path,
