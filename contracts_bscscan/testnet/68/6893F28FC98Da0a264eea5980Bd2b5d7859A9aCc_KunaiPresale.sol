...
contract KunaiPresale is ReentrancyGuard {
  ...
  uint256 maxHardCap = 50000 ether; // 50k kunai
	...
	function buy(uint256 _amount, address beneficiary) public nonReentrant {
		...
		require (totalTokensSold + tokens <= maxHardCap, "Presale hardcap reached");
	...
	function getKUNAITokensLeft() external view returns (uint256) {
		return KUNAI.balanceOf(address(this));
	}

	function claimTokens(address claimer) external {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some KUNAI tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed KUNAI tokens");
		require(KUNAI.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough KUNAI tokens to transfer.");

		KUNAI.safeTransfer(msg.sender, tokensUnclaimed[msg.sender]);
	...
	function withdrawUnsoldKUNAI() external onlyOwner {
		uint256 amount = KUNAI.balanceOf(address(this)) - totalTokensSold;
		KUNAI.safeTransfer(msg.sender, amount);
	}

	function withdrawAllKUNAI() external onlyOwner {
		KUNAI.safeTransfer(msg.sender, KUNAI.balanceOf(address(this)));
	}
}
