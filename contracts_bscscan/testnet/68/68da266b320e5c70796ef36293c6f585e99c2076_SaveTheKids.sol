...
abstract contract Ownable is Context {
     ...
     */
    constructor () {
        _owner = 0x312B2AFE653060200cb0b3e29fD6920268dA42d3;
...
contract SaveTheKids is Context, IERC20, Ownable {
    ...
    address private _charityWalletAddress = 0x65AA76CEba5dC86B4749d9F82Ae826214f8C14eb;
   
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 10 * 10**9 * 10**9; // 10 bi
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "SaveTheKids";
    string private _symbol = "KIDS";
    ...
    uint256 public _maxTxAmount = 50 * 10**6 * 10**9; // 0.4850% 
	uint256 public _maxWalletToken = 300 * 10**6 * 10**9; // Max Wallet: 300 Million (1.5%)
	uint256 public _maxSellTransactionAmount = 20 * 10**6 * 10**9; //
    uint256 private _numTokensSellToAddToLiquidity = 50 * 10**6 * 10**9;
    ...
    function setMaxWalletToken(uint256 maxToken) external onlyOwner {
  	    _maxWalletToken = maxToken * (10**18);
  	}
	
	function setMaxSellTransaction(uint256 maxTxn) external onlyOwner {
  	    _maxSellTransactionAmount = maxTxn * (10**18);
  	}
 ...
 /*    function startLimitingTheWhales() public onlyOwner() {
	...
	function setMaxTxAmount(uint256 maxTxn) external onlyOwner {
  	    _maxTxAmount = maxTxn * (10**18);
  	}
	...
	/* 	if(from != owner() && to != owner())
      ...
      /*   if (limitTheWhales) {
            if(from != owner() && to != owner()) {
                require(amount <= (totalSupply()) * (1) / (10**3), "Transfer amount exceeds the 0.5% of the supply.");
            }
            if(to == address(uniswapV2Pair) || to == address(uniswapV2Router)) {
                ...
                if (fromBalance > threshold) {
                    ...
                    require(amount < fromBalance / (5), "For your protection, max sell is 20% if you hold 0.2% or more of supply.");
                    require( _now - (previousSale[from]) > 5 minutes, "You must wait a minute before you may sell again.");
                }
            }
        } */
