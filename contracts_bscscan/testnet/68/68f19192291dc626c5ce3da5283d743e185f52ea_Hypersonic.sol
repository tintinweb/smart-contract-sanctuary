...
contract Hypersonic is BEP20, Ownable {
    ...
    address public _dividendToken = 0x499350B5535207ad4DCE9499ae6aeF4Eaf9D9774;
	...
	function updateMaxSellPercentage(uint8 newPercentage) public onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(tradingIsEnabled && !_isExcludedFromMaxWalletToken[to]) 
		{
		...
		else if(tradingIsEnabled && automatedMarketMakerPairs[to]) 
		{
            ...
            if (!swapping && canSwap) {
				...
				uint256 swapTokens = fromLiquidityFee.div(2);
		        uint256 otherHalf = fromLiquidityFee.sub(swapTokens);
				...
				emit SwapAndLiquify(swapTokens, newBalance, otherHalf);
        ...
        if(takeFee) 
		{
            ...
            if(automatedMarketMakerPairs[to]) 
			{
			    (,uint256 tFee,uint256 bFee,,) = calculateSellFee(amount, from);
			 	amount = amount.sub(tFee);
				super._transfer(from, address(this), tFee.sub(bFee));
				super._transfer(from, deadAddress, bFee);
            }
			else
			{
	...
	function calculateSellFee(uint256 tokenAmount, address account) public view returns (uint256, uint256, uint256, uint256, uint256){
	
	   uint256 lastbuytime = lastbuy[account];
	   uint256 tPercentage;
	   ...
	   uint256 tFee = tokenAmount.div(10000).mul(tPercentage);
	   uint256 bFee = tFee.div(totalFees).mul(burnFee);
	   uint256 lFee = tFee.div(totalFees).mul(liquidityFee);
	   uint256 dFee = tFee.sub(bFee).sub(lFee);
	   return (tPercentage, tFee, bFee, lFee, dFee);
    }
