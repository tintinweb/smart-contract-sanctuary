...
contract LamboStake is Context, Ownable, ReentrancyGuard {
	 ...
	 */
	function createPool(uint256 poolIndex,address stakedToken,address rewardToken,uint256 unstakeRewardTax,uint256 stakePeriod,uint256 rewardTokensByPeriod) public onlyOwner returns (Pool memory) {
		...
		emit PoolAddedSuccessfully(poolIndex,stakedToken,rewardToken,_unstakeRewardTax[poolIndex],stakePeriod,rewardTokensByPeriod);
		Pool memory pool;
