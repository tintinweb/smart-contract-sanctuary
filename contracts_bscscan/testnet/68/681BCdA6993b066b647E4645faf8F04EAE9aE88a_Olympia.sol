...
contract Olympia is ERC20, Ownable {
    ...
    function _distributeReflection(address sender, address recipient) private {
        // if (_shouldSetShare(sender, recipient)) {
            try IReflectionDistributor(payable(_reflectionDistributor)).setShare(payable(sender), balanceOf(sender)) {} catch {}
            try IReflectionDistributor(payable(_reflectionDistributor)).setShare(payable(recipient), balanceOf(recipient)) {} catch {}
        // }
        
        if (!_inSwap) {
	    	try IReflectionDistributor(payable(_reflectionDistributor)).process(_gasForProcessing) {
