...
contract Lottery is Ownable, Pausable, ReentrancyGuard {
    ...
    event UnstakeDelayUpdated(uint unstakeDelay);
    ...
    function getAmountToStake(uint256 poolId)
        public
        view
        returns (uint256 amountToStake)
    {
        uint256 amount = tierCategory[poolId];
        return (amount);
    }
    ...
    function enterTheLottery(uint256 _poolId)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        require(
            ...
            "you have already staked in a pool"
        );

        // Does he have enough monies?
        require(
            orbit.balanceOf(msg.sender) >= amountToStake,
        ...
        stakePools[_poolId].push(
            userData(msg.sender, amountToStake, block.timestamp, _poolId)
        );
        userToTierStaked[msg.sender] = _poolId;
        // We store the position of the user in the array, to avoid a latter O(n) search
        userIndexMap[_poolId][msg.sender] = stakePools[_poolId].length - 1;
    ...
    function leaveLottery() external nonReentrant {
        ...
        // require(
        //     block.timestamp >= userEntry.lastStake + unstakeDelay,
        //     "please wait until the end of your unstaking delay"
        // );

        // We remove the entry
        userToTierStaked[msg.sender] = 0;

        stakePools[poolId][userIndex] = stakePools[poolId][
            stakePools[poolId].length - 1
        ];
        ...
        // We refund the deposit - minus x% if it's too early
        if (block.timestamp < userEntry.lastStake + unstakeDelay) {
            uint256 refund = userEntry.amountStaked*100/(1-penaltyRate);
            orbit.safeTransfer(msg.sender, refund);
        } else {
            orbit.safeTransfer(msg.sender, userEntry.amountStaked);
        }
    ...
    function drawWinner(
        ...
        require(
            orbit.allowance(mktWallet, address(this)) >= reward,
        ...
        IERC20 rewardToken = IERC20(rewardTokenAdress);
        // We send him the tokens
        rewardToken.safeTransferFrom(mktWallet, winner, reward);
    ...
    function drawWinnersArray(
        ...
        require(
            poolIds.length == rewards.length,
            "param 1 and 2 do not have the same length"
        );

        // We loop over the supplied arrays
        for (uint256 i = 0; i < poolIds.length; i++) {
            drawWinner(token, poolIds[i], rewards[i], ownerSalt);
        }
    }
    ...
    function random(uint256 ownerSalt, uint256 innerSalt)
        internal
        whenPaused
        returns (uint256 prng)
    {
    ...
    function updateUnstakeDelay(uint256 _delayInSeconds) external onlyOwner {
        unstakeDelay = _delayInSeconds;
    ...
    function updateTierCategory(uint256[] memory _tiersArray)
        external
        onlyOwner
    {
    ...
    function getWinnersList(uint256 nLast)
        external
        returns (winnerData[] memory winners)
    {
        ...
        for (uint256 i = 0; i < nLast; i++) {
            emit Debug("i", i);
            emit Debug("nlast", nLast);
            winners[i] = winnersList[winnersList.length - i - 1];
        }
    ...
    function pauseTheLottery() external onlyOwner {
        _pause();
    }

    function unPauseTheLottery() external onlyOwner {
        _unpause();
    }
    ...
    function seePoolData(uint256 poolId)
        external
        view
        returns (uint256 amountStaked, uint256 amountStakers)
    {
        amountStakers = stakePools[poolId].length;
        amountStaked = amountStakers * getAmountToStake(poolId);

        return (amountStaked, amountStakers);
    }
}
