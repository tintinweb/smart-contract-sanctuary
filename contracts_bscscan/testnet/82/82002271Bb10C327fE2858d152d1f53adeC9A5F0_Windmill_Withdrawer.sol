...
interface I_Windmill_Power is I_Math, IAccessControlEnumerable, I_With_DAORole, IERC20, I_With_FundContract{
    ...
    function MOVER_ROLE() external returns (bytes32);
...
event GasLog_With_PWRToken(uint256 line, uint256 gas);
    I_Windmill_Power public PWRToken;

    function setPWRToken(I_Windmill_Power token) external onlyRole(DAO_ROLE){
emit GasLog_With_PWRToken(15, gasleft());
        PWRToken = token;
emit GasLog_With_PWRToken(16, gasleft());
    }
}// Version: 1.0.0
...
event GasLog_Math(uint256 line, uint256 gas);
    /**
     * @notice Compute the number of digits in an uint256 number.
     *
     * Node that if number = 0, it returns 0.
     */
    function numDigits(uint256 number) internal returns (uint8) {
emit GasLog_Math(13, gasleft());
        uint8 digits = 0;
emit GasLog_Math(14, gasleft());
        while (number != 0) {
emit GasLog_Math(15, gasleft());
            number /= 10;
emit GasLog_Math(16, gasleft());
            digits++;
emit GasLog_Math(17, gasleft());
        }
        return digits;
emit GasLog_Math(19, gasleft());
    }
    
    function min(uint256 a, uint256 b) internal returns (uint256){
emit GasLog_Math(22, gasleft());
        if (a<b){
emit GasLog_Math(23, gasleft());
            return a;
emit GasLog_Math(24, gasleft());
        }
        return b;
emit GasLog_Math(26, gasleft());
    }
}
// Version: 1.0.0
...
interface I_Windmill_Fund is I_Math, I_Payable, IAccessControlEnumerable, I_With_DAORole, I_With_UpdaterRole, I_With_TradeManagerRole,
    ...
    function PWR_TOKEN_ROLE() external returns (bytes32);
    
    function WITHDRAWER_ROLE() external returns (bytes32);
    ...
    function getWithdrawFees() external returns(Fraction memory);
    
    function getDataPWRLock(address addr) external returns (uint256, uint256, uint256, uint256, uint256[2][] memory);
     ...
     */
    function getFundBUSD() external returns (uint256);
     ...
     */
    function getTotalPWR() external returns (uint256);

    /**
     * Compute The number of PWR that corresponds to "amountBUSD" BUSD.
     */
    function getPWRAmountFromBUSD(uint256 amountBUSD) external returns (uint256);

    /**
     * Compute The number of BUSD that corresponds to "amountPWR" PWR.
     */
    function getBUSDAmountFromPWR(uint256 amountPWR) external returns (uint256);
...
event GasLog_With_FundContract(uint256 line, uint256 gas);
    I_Windmill_Fund public fund;

    function setFund(address _fund) public onlyRole(DAO_ROLE){
emit GasLog_With_FundContract(15, gasleft());
        fund = I_Windmill_Fund(payable(_fund));
emit GasLog_With_FundContract(16, gasleft());
    }
}// Version: 1.0.0
...
interface I_Windmill_Withdrawer is I_Math, IAccessControlEnumerable, I_With_DAORole, I_With_TradeManagerRole, I_With_PWRToken,
    ...
    function getNbPWRToWithdraw(uint256 tradeId) external returns(uint256);
...
event GasLog_Windmill_Withdrawer(uint256 line, uint256 gas);
    ////
    ////
    ////
    //////////////// Public variables ////////////////
...
emit GasLog_Windmill_Withdrawer(46, gasleft());
        require(nbPWR > 0, "Windmill_Withdrawer: Cannot withdraw 0 PWR");
emit GasLog_Windmill_Withdrawer(47, gasleft());
        
        (, uint256 maxNbLockedPwr,, uint256 maxLockPercentNumerator, uint256[2][] memory lockDatas) = fund.getDataPWRLock(msg.sender);
emit GasLog_Windmill_Withdrawer(49, gasleft());
        
        require(nbPWR <= maxNbLockedPwr, "Windmill_Withdrawer: Not enough locked PWR");
emit GasLog_Windmill_Withdrawer(51, gasleft());
        
        uint256 l = lockDatas.length;
emit GasLog_Windmill_Withdrawer(53, gasleft());
        uint256 remainingPWR = nbPWR;
emit GasLog_Windmill_Withdrawer(54, gasleft());
        PendingWithdrawData storage data = pendingWithdrawData[msg.sender];
emit GasLog_Windmill_Withdrawer(55, gasleft());
        
        for(uint256 i=0; i<l; i++){
emit GasLog_Windmill_Withdrawer(57, gasleft());
            uint256 nbPartPWR = remainingPWR;
emit GasLog_Windmill_Withdrawer(58, gasleft());
            
            if (i < l-1){
emit GasLog_Windmill_Withdrawer(60, gasleft());
                nbPartPWR = (nbPWR * lockDatas[i][0]) / maxLockPercentNumerator;
emit GasLog_Windmill_Withdrawer(61, gasleft());
            }
            
            remainingPWR -= nbPartPWR;
emit GasLog_Windmill_Withdrawer(64, gasleft());
            
            WithdrawData storage d = withdrawData[lockDatas[i][1]];
emit GasLog_Windmill_Withdrawer(66, gasleft());
            
            require(!d.claimable, "Windmill_Withdrawer: Trade already closed");
emit GasLog_Windmill_Withdrawer(68, gasleft());
            
            d.totalPWR += nbPartPWR;
emit GasLog_Windmill_Withdrawer(70, gasleft());
            d.userPWR[msg.sender] += nbPartPWR;
emit GasLog_Windmill_Withdrawer(71, gasleft());
            
            if (data.tradeIdToPendingTradeIdShifted[lockDatas[i][1]] == 0){
emit GasLog_Windmill_Withdrawer(73, gasleft());
                data.pendingTrade[data.pendingTradeLength] = lockDatas[i][1];
emit GasLog_Windmill_Withdrawer(74, gasleft());
                data.pendingTradeLength += 1;
emit GasLog_Windmill_Withdrawer(75, gasleft());
    ...
    function getClaimableBUSD(address addr) external returns(uint256){
emit GasLog_Windmill_Withdrawer(86, gasleft());
        PendingWithdrawData storage pendingData = pendingWithdrawData[addr];
emit GasLog_Windmill_Withdrawer(87, gasleft());
        mapping(uint256=>uint256) storage pendingTrade = pendingData.pendingTrade;
emit GasLog_Windmill_Withdrawer(88, gasleft());
        
        uint256 totalClaimableBUSD;
emit GasLog_Windmill_Withdrawer(90, gasleft());
        
        for(uint256 i=0;i<pendingData.pendingTradeLength; i++){
emit GasLog_Windmill_Withdrawer(92, gasleft());
            WithdrawData storage data = withdrawData[pendingTrade[i]];
emit GasLog_Windmill_Withdrawer(93, gasleft());
            
            if (data.claimable && data.userPWR[addr]>0){
emit GasLog_Windmill_Withdrawer(95, gasleft());
                totalClaimableBUSD += data.totalBUSD * data.userPWR[addr] / data.totalPWR;
emit GasLog_Windmill_Withdrawer(96, gasleft());
            }
        }
        
        return totalClaimableBUSD;
emit GasLog_Windmill_Withdrawer(100, gasleft());
    }
    
    function claimWithdraw() external{
emit GasLog_Windmill_Withdrawer(103, gasleft());
        PendingWithdrawData storage pendingData = pendingWithdrawData[msg.sender];
emit GasLog_Windmill_Withdrawer(104, gasleft());
        mapping(uint256=>uint256) storage pendingTrade = pendingData.pendingTrade;
emit GasLog_Windmill_Withdrawer(105, gasleft());
        
        uint256 totalClaimableBUSD;
emit GasLog_Windmill_Withdrawer(107, gasleft());
        uint256 totalPWRToBurn;
emit GasLog_Windmill_Withdrawer(108, gasleft());
        
        uint256 i = pendingData.pendingTradeLength;
emit GasLog_Windmill_Withdrawer(110, gasleft());
        while (i>0){
emit GasLog_Windmill_Withdrawer(111, gasleft());
            i--;
emit GasLog_Windmill_Withdrawer(112, gasleft());
            
            WithdrawData storage data = withdrawData[pendingTrade[i]];
emit GasLog_Windmill_Withdrawer(114, gasleft());
            
            if (data.claimable && data.userPWR[msg.sender]>0){
emit GasLog_Windmill_Withdrawer(116, gasleft());
                totalPWRToBurn += data.userPWR[msg.sender];
emit GasLog_Windmill_Withdrawer(117, gasleft());
                totalClaimableBUSD += data.totalBUSD * data.userPWR[msg.sender] / data.totalPWR;
emit GasLog_Windmill_Withdrawer(118, gasleft());
                data.userPWR[msg.sender] = 0;
emit GasLog_Windmill_Withdrawer(119, gasleft());
                
                uint256 lastI = pendingData.pendingTradeLength-1;
emit GasLog_Windmill_Withdrawer(121, gasleft());
                pendingData.tradeIdToPendingTradeIdShifted[pendingTrade[i]] = 0;
                if (i != lastI){
emit GasLog_Windmill_Withdrawer(123, gasleft());
    ...
    function getNbPWRToWithdraw(uint256 tradeId) external returns(uint256){
emit GasLog_Windmill_Withdrawer(138, gasleft());
        return withdrawData[tradeId].totalPWR;
emit GasLog_Windmill_Withdrawer(139, gasleft());
    }
    
    function closeTrade(uint256 tradeId, uint256 transferedBUSD) external onlyRole(TRADE_ROLE){
emit GasLog_Windmill_Withdrawer(142, gasleft());
        WithdrawData storage data = withdrawData[tradeId];
emit GasLog_Windmill_Withdrawer(143, gasleft());
        data.totalBUSD = transferedBUSD;
emit GasLog_Windmill_Withdrawer(144, gasleft());
        data.claimable = true;
emit GasLog_Windmill_Withdrawer(145, gasleft());
        
        PWRToken.burnFrom(thisAddr, data.totalPWR);
emit GasLog_Windmill_Withdrawer(147, gasleft());
    }
