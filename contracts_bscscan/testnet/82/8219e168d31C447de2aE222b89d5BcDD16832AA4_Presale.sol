...
contract Presale is AccessControl {
    ...
    constructor(
        ...
        _setupRole(BUY_ROLE, _msgSender());
    ...
    function buy(uint256 buyNum,string memory users) external payable {
        require(buyNum >= 1 && buyNum <= 100000, "Maximum of 100000 at a time");
       
        require(curPrice > 0, "No price available");

        uint256 latestPrice = uint256(getLatestPrice());
        require(latestPrice > 0, "Invalid price");
        uint256 totalPrice = curPrice.mul(buyNum).mul(1e8).mul(1e18).div(
            latestPrice
        );
    ...
    function getPriceBuyNum(uint256 buyNum) external view returns (uint256){
        uint256 latestPrice = uint256(getLatestPrice());
        require(latestPrice > 0, "Invalid price");
        uint256 totalPrice = curPrice.mul(buyNum).mul(1e8).mul(1e18).div(
            latestPrice
        );
        return totalPrice.div(1e8).div(1e18);
    }
