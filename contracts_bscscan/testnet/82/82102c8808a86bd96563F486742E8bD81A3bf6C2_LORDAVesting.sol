...
contract LORDAVesting is Ownable {
    ...
    function _claimableAmount(address account) private view returns (uint256) {
        if (block.timestamp < _startTime) {
            return 0;
        } else if (block.timestamp >= _unlockTimes[_unlockTimes.length - 1]) {
            ...
            for (uint256 i = 0; i < _unlockTimes.length; i++) {
                if (block.timestamp >= _unlockTimes[i]) {
                    unlockedCount++;
                }
            }
            ...
            return
                unlockedCount == 0
                    ? tgeUnlock - _released[account]
                    : tgeUnlock +
                        (((_locked[account] - tgeUnlock) /
                            _unlockTimes.length) * unlockedCount) -
                        _released[account];
        }
    }
