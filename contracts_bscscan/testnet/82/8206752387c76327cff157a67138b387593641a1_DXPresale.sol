...
contract DXPresale is Owned {
   ...
   bool public isSuccess = false;
    ...
    constructor(address _token,uint256 _min,uint256 _max,uint256 _rate, uint256 _soft , uint256 _hard,uint256 _pancakeRate,address _router,uint256 _unlockon,bool isAuto) public {
        tokenAddress = _token;
        minEthLimit = _min;
        maxEthLimit = _max;
        ...
        tokenRatePerEthPancake = _pancakeRate.mul(10 ** (tokenDecimals));
  ...
  function claimTokens() public{
    ...
    function _addLiquidityToken() internal{
        uint256 tokenAmount1 = softCap.mul(tokenRatePerEthPancake); 
        uint256 tokenAmount2 = earnedCap.mul(tokenRatePerEthPancake);
        uint256 tokenAmount3 = hardCap.mul(tokenRatePerEthPancake);
        if(IToken(tokenAddress).balanceOf(address(this)) >= tokenAmount3){
            addLiquidity(tokenAmount3,hardCap);
        }else if(IToken(tokenAddress).balanceOf(address(this)) >= tokenAmount2){
            addLiquidity(tokenAmount2,earnedCap);
        }else{
            addLiquidity(tokenAmount1,softCap);
        }
        unlockOn = block.timestamp.add(unlockOn);
    }
    
    function checkTokentoAddLiquidty() public view returns(uint256) {
        if((earnedCap.mul(tokenRatePerEthPancake)) > IToken(tokenAddress).balanceOf(address(this))){
            return (earnedCap.mul(tokenRatePerEthPancake).div(10e18)).sub((IToken(tokenAddress).balanceOf(address(this))).div(10 ** tokenDecimals));
        }else{
            return uint256(0);
        }
    }
