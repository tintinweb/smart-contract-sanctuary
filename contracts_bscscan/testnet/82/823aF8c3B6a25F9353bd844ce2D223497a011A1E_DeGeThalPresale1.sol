...
contract DeGeThalPresale1 is Ownable {
    using SafeMath for uint256;
    ...
    event PrivatesaleFinalized(uint256 weiAmount);
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "DeGeThalPrivatesale: sale is not open");
        require(!hasEnded(), "DeGeThalPrivatesale: sale is over");
        require(
            weiAmount >= minContribution,
            "DeGeThalPrivatesale: min contribution criteria not met"
        );
        require(
            contributions[beneficiary].add(weiAmount) <= maxContribution,
            "DeGeThalPrivatesale: max contribution criteria not met"
        );
        this;
    }

    function claimTokens() external {
        require(hasEnded(), "DeGeThalPrivatesale: private sale is not over");
        require(
            softCapReached(),
            "DeGeThalPrivatesale: soft cap not reached, refund is available"
        );
        require(
            contributions[msg.sender] > 0,
            "DeGeThalPrivatesale: nothing to claim"
        );
    ...
    function claimRefund() external {
        require(hasEnded(), "DeGeThalPrivatesale: private sale is not over");
        require(!softCapReached(), "DeGeThalPrivatesale: soft cap not reached");
        require(
            contributions[msg.sender] > 0,
            "DeGeThalPrivatesale: nothing to claim"
        );
    ...
    function endPrivatesale() external onlyOwner {
        require(
            !finalized,
            "DeGeThalPrivatesale: private sale is already over"
        );
        finalized = true;
        if (weiRaised > softCap) {
            uint256 totalWeiRaised = address(this).balance;
            uint256 devFee = totalWeiRaised.mul(_devFee).div(10**2);
            uint256 marketingFee = totalWeiRaised.mul(_devFee).div(10**2);
        ...
        emit PrivatesaleFinalized(weiRaised);
    }

    function emergencyEndPrivatesale() external onlyOwner {
        finalized = true;
    ...
    function withdrawTokens() public onlyOwner {
        require(finalized, "DeGeThalPrivatesale: private sale is not over");
