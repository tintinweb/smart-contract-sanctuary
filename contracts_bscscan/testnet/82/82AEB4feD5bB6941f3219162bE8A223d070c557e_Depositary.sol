...
error BaksDAOInsufficientDeposit();
...
contract Depositary is Initializable, Governed {
    ...
    mapping(address => Magister) public magisters;
    ...
    function withdraw(uint256 depositId, uint256 amount) external {
        ...
        if (msg.sender == d.magister) {
            stablecoin.safeTransfer(d.magister, normalizedAmount);
            bonusToken.safeTransfer(d.magister, d.magisterAccruedBonusRewards);

            d.magisterAccruedRewards -= normalizedAmount;
            d.magisterAccruedBonusRewards = 0;
        } else {
            uint256 a = normalizedAmount;
            if (a > d.principal) {
                revert BaksDAOInsufficientDeposit();
            }

            if (p.isCompounding) {
                a += d.depositorAccruedRewards;
            ...
            stablecoin.safeTransfer(d.depositor, a);
            bonusToken.safeTransfer(d.depositor, d.depositorAccruedBonusRewards);
        ...
        )
    {
        ...
        uint256 totalRewards = d
            .principal
            .mul(p.calculateMultiplier((block.timestamp - d.lastInteractionAt).mulDiv(ONE, 365 days)))
        ...
        try priceOracle.getNormalizedPrice(bonusToken) returns (uint256 bonusTokenPrice) {
            depositorBonusReward = totalRewards.mulDiv(p.depositorBonusApr.mul(bonusTokenPrice), totalApr);
            magisterBonusReward = totalRewards.mulDiv(p.magisterBonusApr.mul(bonusTokenPrice), totalApr);
        } catch {}
    }
