...
contract Chef is ERC721Enumerable, Ownable {
    ...
    Taco public taco;
    address public KitchenAddress;

    uint256 public constant MAX_PER_MINT = 30;
    uint256 public constant MAX_BASE_SUPPLY = 10000;
    uint256[3] public MAX_BASE_SUPPLY_ARRAY = [5000,3000,2000];
    uint256 public constant MAX_PRESALE_SUPPLY = 500;
    uint256[3] public MAX_PRESALE_SUPPLY_ARRAY = [250,150,100];
    uint256[3] public BASE_MINT_PRICE_ARRAY = [2.5 ether,3 ether,3.5 ether];//2.5 AVAX, 3 AVAX, 3.5 AVAX
    uint256[3] public PRESALE_MINT_PRICE_ARRAY = [2 ether,2.5 ether,3 ether];//2 AVAX, 2.5 AVAX, 3 AVAX
    uint256 public constant UPGRADE_SALES_OFFSET = 2 days;

    uint256 public baseSupply;
    uint256[3] public baseSupply_array;
    uint256 public presaleSupply;
    uint256[3] public presaleSupply_array;
    ...
    string public constant BASE_URI = "https://taco.game/tokens/";

    constructor() ERC721("Taco Chef", "TACO-CHEF") {
        // supply and price are ignored for the base levels of chef and robot
        levels.push(Level({ supply: 0, maxSupply: MAX_BASE_SUPPLY_ARRAY[0], price: 0, yield: 1 }));
        levels.push(Level({ supply: 0, maxSupply: MAX_BASE_SUPPLY_ARRAY[1], price: 0, yield: 2 }));
        levels.push(Level({ supply: 0, maxSupply: MAX_BASE_SUPPLY_ARRAY[2], price: 0, yield: 3 }));
    }

    /* Minting of base chefs */

    function mintBase(uint16 _numTokens,  uint16 _levelCheck) external payable {
        require(msg.value == _numTokens * BASE_MINT_PRICE_ARRAY[_levelCheck], "Incorrect amount sent");
        require(baseSalesOpen(), "The main sale period is not open");
        require(baseSupply_array[_levelCheck] + _numTokens <= MAX_BASE_SUPPLY_ARRAY[_levelCheck], "Insufficient basesale supply of level1");
        _mintBaseTokens(_numTokens, _msgSender(), _levelCheck);
        baseSupply_array[_levelCheck] += _numTokens;
    }
    
    function presaleMintBase(uint16 _numTokens, uint16 _levelCheck) external payable {
        require(msg.value == _numTokens * PRESALE_MINT_PRICE_ARRAY[_levelCheck], "Incorrect amount sent");
        require(presaleOpen(), "The presale is not open");
        require(presaleSupply_array[_levelCheck] + _numTokens <= MAX_PRESALE_SUPPLY_ARRAY[_levelCheck], "Insufficient presale supply of level1");
        require(presaleSupply + _numTokens <= MAX_PRESALE_SUPPLY, "Insufficient presale supply");
        _mintBaseTokens(_numTokens, _msgSender(), _levelCheck);
        presaleSupply_array[_levelCheck] += _numTokens;
        presaleSupply += _numTokens;
    }

    function reserveBase(uint16 _numTokens, address _for, uint16 _levelCheck) external onlyOwner {
        _mintBaseTokens(_numTokens, _for, _levelCheck);
    }
    ...
    function _mintBaseTokens(uint16 _numTokens, address _for, uint16 _levelCheck) internal {
        ...
        for (uint i = 0; i < _numTokens; i++) {
            ...
            tokenLevel[tokenId] = _levelCheck;            
        }
    }

    /* Minting of upgrade chefs */
    ...
    function mintUpgrade(uint256 _level, uint16 _numTokens) external {
        ...
        require(_numTokens <= MAX_PER_MINT, "Too many purchases at once");
        require(_level < levels.length && _level > 2, "Invalid level");
        ...
        require(taco.balanceOf(msg.sender) >= totalCost, "Insufficient TACO balance");
        //100% burn
        taco.burn(msg.sender, totalCost);
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        if (isUpgrade(_tokenId)) {
            ...
            return string(abi.encodePacked(BASE_URI, "base/", _tokenId.toString(), "-chef.json"));
        }        
    }
    ...
    // function batchedChefsOfOwner(
    //     address _owner,
    //     uint256 _offset,
    //     uint256 _maxSize
    // ) public view returns (ChefInfo[] memory) {
    ...
    function setKitchenAddress(address _KitchenAddress) external onlyOwner {
        require(KitchenAddress == address(0), "Kitchen address already set");
        KitchenAddress = _KitchenAddress;
    }

    function setTaco(Taco _taco) external onlyOwner {
        taco = _taco;
    }
...
import "./14_Chamber.sol";

contract Taco is ERC20("Taco", "TACO"), Ownable, StakeableChamber{
    ...
    function setKitchenAddress(address _kitchenAddress) external onlyOwner {
        require(address(kitchenAddress) == address(0), "Kitchen address already set");
        kitchenAddress = _kitchenAddress;
    }
    ...
    function mint(address _to, uint256 _amount) external {
        require(_msgSender() == kitchenAddress, "Only the Kitchen contract can mint");
        _mint(_to, _amount);
    }

    function burn(address _from, uint256 _amount) external {
        require(_msgSender() == chefAddress || _msgSender() == vaultAddress, "Only the Chef and Vault contract can burn");
        _burn(_from, _amount);
    }
    ...
    function stakeChamber(uint256 _amount) public {
      
      require(_amount < balanceOf(msg.sender), "Taco Token: Cannot stake more than you own");
