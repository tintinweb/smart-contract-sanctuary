...
interface IWSPPNFT1155 {
    function creators(uint256) external returns (address);
}

pragma solidity 0.6.12;
contract WolfableMarketplace is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    /// @notice WSPP721 contract
    address public wspp721;
    ...
    function buyItem(
        ...
        // require(feeTransferSuccess, "WSPPMarketplace: Fee transfer failed");
        if (
            ...
            //     "WSPPMarketplace: Royalty fee transfer failed"
            // );
        }
        listedItem.payToken.safeTransferFrom(
            _msgSender(),
        ...
        // }("");
        // require(ownerTransferSuccess, "WSPPMarketplace: Owner transfer failed");
    ...
    function registerRoyalty(
        ...
        if (_supportERC721(_nftAddress)) {
            require(
                IERC721(wspp721).ownerOf(_tokenId) == _msgSender(),
                "Not owning the item."
            );
        } else if (_supportERC1155(_nftAddress)) {
            require(
                IWSPPNFT1155(_nftAddress).creators(_tokenId) == _msgSender(),
                "Not owning the item."
            );
        } else {
            revert("Invalid NFT address.");
    ...
    /**
     @notice Update WSPP721 contract
     @dev Only admin
     */
    function updateWSPP721(address _wspp721) external onlyOwner {
        require(
            IERC165(_wspp721).supportsInterface(INTERFACE_ID_ERC721),
            "Not ERC721"
        );
        wspp721 = _wspp721;
    }
