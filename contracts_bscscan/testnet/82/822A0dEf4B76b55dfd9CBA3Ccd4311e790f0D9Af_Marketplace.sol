  ...
  IMarketplace
{
  ...
  function removeFromSale(uint256 _id) external override {
    require(tokenSaleInfo[_id][msg.sender].status == 1, "Marketplace:removeFromSale01");
    if (tokenSaleInfo[_id][msg.sender].priceType == 1 || tokenSaleInfo[_id][msg.sender].priceType == 2) {
      require(
        activeBid[_id][msg.sender].bidder == address(0) || activeBid[_id][msg.sender].bidder == msg.sender,
        "Marketplace:removeFromSale02"
      );
    }
    _transferNFT(address(this), msg.sender, _id, tokenSaleInfo[_id][msg.sender].amount);
    _removeFromSale(_id, msg.sender);
  }
  ...
  function buy(
    ...
    IMaviaNFT(maviaNFTAddress).setTokenOwner(msg.sender, _id);
