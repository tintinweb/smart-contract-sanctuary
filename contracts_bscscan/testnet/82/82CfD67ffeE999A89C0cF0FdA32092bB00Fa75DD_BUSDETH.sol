...
// File: Quantum/IERC20Metadata.sol
...
contract BUSDETH is ERC20, Ownable {
    ...
    uint8 _decimals = 9;
    uint256 public maxBuyTranscationAmount = 10000000000000000 * (10**uint256(_decimals)); //100% of total supply
    uint256 public maxSellTransactionAmount = 1000000000000 * (10**uint256(_decimals)); //0.1% of total supply
    uint256 public swapTokensAtAmount = 2000000000000 * (10**uint256(_decimals)); //0.2% of total supply
    bool public enableSwapAndLiquify;

    uint256 public ethFee = 600;
    uint256 public busdFee = 200;
    uint256 public marketingFee = 200;
    uint256 public liquidityFee = 200;
    uint256 public totalFees = ethFee + busdFee + marketingFee + liquidityFee;
    uint256 constant public DENOMINATOR = 10_000;
    address public marketingWallet = 0xE8780d04AB0b7074e8b275ae1cf305B869276559;
...
//         IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
        ...
        _mint(owner(), 1_000_000_000_000_000 * (10**uint256(_decimals)));
    }
    ...
    function updateDividendRewardFee(uint256 busd, uint256 eth) public onlyOwner {
        require(eth + busd <= 3000, "Dividend reward tax must be <= 3000");
        totalFees = totalFees - (ethFee + busdFee) + (eth + busd);
        ethFee = eth;
        busdFee = busd;
    ...
    function updateLiquidityFee(uint256 newFee) public onlyOwner {
        require(newFee <= 4000, "liquidity fee must be less than 4000");
        totalFees = totalFees - liquidityFee + newFee;
        liquidityFee = newFee;
    }
    ...
    function _transfer(
        ...
        if (from != uniswapV2Pair && enableSwapAndLiquify && contractTokenBalance >= swapTokensAtAmount && !swapping) {
            ...
            uint256 onePart = swapTokensAtAmount / totalFees;
            ...
            uint256 bnbForBUSD = onePart * busdFee;
            uint256 bnbForLiquidity = onePart * liquidityFee / 2;
            uint256 bnbForMarketing = onePart * marketingFee;
            uint256 bnbForETH= contractBNBBalance - bnbForBUSD - bnbForMarketing - bnbForLiquidity;
