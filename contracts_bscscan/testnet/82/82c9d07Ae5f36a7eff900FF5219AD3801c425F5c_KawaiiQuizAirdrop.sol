    ...
    function claimAirdrop(IERC1155 nftRegister, address sender, uint8 v, bytes32 r, bytes32 s) external {
        ...
        for (uint256 i = 0; i < canClaim[sender]; i++) {
            ...
            if (index == 0) {
                ...
                idMint1 = chooseMaterial(materials.length, gasleft());
                idMint2 = chooseMaterial(materials.length, gasleft());
            } else if (index == 3) {
                idMint = chooseField(fields.length, gasleft());
            } else if (index == 4) {
                idMint = chooseDye(dyes.length, gasleft());
                idMint1 = chooseDye(dyes.length, gasleft());
                idMint2 = chooseDye(dyes.length, gasleft());
            }
            ...
            if (index == 2 || index == 4) {
                nftRegister.mint(sender, idMint1, 1);
                nftRegister.mint(sender, idMint2, 1);
                emit ClaimAirdrop(sender, idMint1);
                emit ClaimAirdrop(sender, idMint2);
            }
        }
        canClaim[sender] = 0;
    }
