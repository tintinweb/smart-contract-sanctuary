...
contract Ownable is Context {
    ...
    function extendLock(uint256 time) public virtual {
        require(_previousOwner == _msgSender(), 'Only previous owner can extend lock time');
        require(time + now > _lockTime, 'New unlock time must be later than current');
        _lockTime = now + time;
    }
    ...
    function unlock() public virtual {
        ...
        require(now > _lockTime , "Contract lock has not expired");
...
contract TokenDividendTracker is DividendPayingToken, Ownable {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 60 && newClaimWait <= 86400, "76ClaimLock_Dividend_Tracker: claimWait must be between 1 minute and 24 hours");
        require(newClaimWait != claimWait, "76ClaimLock_Dividend_Tracker: Cannot update claimWait to same value");
...
contract C76ClaimLock is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxSellTransactionAmount = 1500000 * (10**18);
    ...
    constructor() public ERC20("76ClaimLock", "76ClaimLock") {

        _marketingWallet = 0x333e5922681EE29f515Db739e556c59133329921;
    	dividendTracker = new TokenDividendTracker("76ClaimLock_Dividend_Tracker", "76ClaimLock_Dividend_Tracker", DOGEToken);
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "76ClaimLock: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "76ClaimLock: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "76ClaimLock: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "76ClaimLock: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 2000000, "76ClaimLock: gasForProcessing must be between 200,000 and 2,000,000");
        require(newValue != gasForProcessing, "76ClaimLock: Cannot update gasForProcessing to same value");
    ...
    function setMaxBuyOrSellTxAMount(uint256 amount) external onlyOwner{
    ...
    function setSwapBnbForDogeAmountInPlainBNB(uint256 amount) external onlyOwner {
        swapBnbForDogeAmount = amount * (10 ** 18);
    }
    
    function getSwapBnbForDogeAmountInPlainBNB() external view returns(uint256) {
        return swapBnbForDogeAmount.div(10 ** 18);
    }
    ...
    function _transfer(
        ...
        if( 
         ...
         if( 
            ...
            require(amount <= maxSellTransactionAmount, "Buy transfer amount exceeds maxAmount.");
        }
