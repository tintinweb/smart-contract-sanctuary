...
contract GameFactory is Ownable, AccessControlEnumerable {
    ...
    address public marketWallet; //payment token (Hifi token)
    ...
    uint256 public _withdrawFeeForBronzeItem;
    uint256 public _withdrawFeeForSilverItem;
    uint256 public _withdrawFeeForGoldItem;
   ...
   constructor(address _paymetTokenAddress, address _marketWallet, address _cmoAddress, address _cfoAddress) {
        ...
        _withdrawFeeForBronzeItem = 3;
        _withdrawFeeForSilverItem = 2;
        _withdrawFeeForGoldItem = 1;
  ...
  // Get Staked amount by user address
    ...
    */        
    function withdrawBoost(uint itemType, uint16 count) external {
        uint256 amount = 0;
        uint256 commissionFee = 0;
        require(count > 0 && count <= userBoostItemBalance[msg.sender][itemType], 'Invalid Item counts');
        if(itemType == 1) {
            amount  = _goldItemPrice.mul(count);
            commissionFee = amount.mul(_withdrawFeeForGoldItem).div(100);
        } else if (itemType == 2) {
            amount = _silverItemPrice.mul(count);
            commissionFee = amount.mul(_withdrawFeeForSilverItem).div(100);
        } else if (itemType == 3) {
            amount = _bronzeItemPrice.mul(count);
            commissionFee = amount.mul(_withdrawFeeForBronzeItem).div(100);
        } else {
            require(false, 'Invalid item type');
        }
        ...
        platformStatistic.totalStakedForBoost = platformStatistic.totalStakedForBoost.sub(amount);
        platformStatistic.totalCommission = platformStatistic.totalCommission.add(commissionFee);
    ...
    */
    function _setBaseStakeAmountForPlay(uint256 _newBaseStakeAmountForPlay) external  onlyRole(CFO_ROLE) {
        require(_newBaseStakeAmountForPlay > 0 && _newBaseStakeAmountForPlay <= 100000 * 10**18, 'Invalid Parameter');
    ...
    */
    function _setBaseStakeAmountForEarn(uint256 _newBaseStakeAmountForEarn) external  onlyRole(CFO_ROLE) {
        require(_newBaseStakeAmountForEarn > 0 && _newBaseStakeAmountForEarn <= 10000000 * 10**18, 'Invalid Parameter');
