...
contract TaxFreeTransfers is StarlinkComponent, ShopItemFulfilment {
    ...
    function taxExemptSell(uint256 xldAmount, uint256 amountOutMin) external payable notPaused notUnauthorizedContract nonReentrant process {
	...
	function swapBNBForXLD(uint256 bnbAmount, uint256 amountOutMin) internal returns(uint256) { 
		...
		_pancakeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{ value: bnbAmount }(amountOutMin, xldBnbPath, address(this), block.timestamp + 360);
	...
	function swapXLDForBNB(uint256 tokenAmount, uint256 amountOutMin) internal returns(uint256) {
		...
		_pancakeswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(tokenAmount, amountOutMin, xldBnbPath, address(this), block.timestamp + 360);
    ...
    function setPancakeSwapRouter(address routerAddress) public onlyOwner {
        ...
        xldBnbPath = new address[](2);
        xldBnbPath[0] = address(xld);
		xldBnbPath[1] = _pancakeswapV2Router.WETH();
	}
