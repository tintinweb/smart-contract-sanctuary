...
contract Olympia is ERC20, Ownable {
    ...
    struct Fee {
        uint256 reflectionFeePerThousand;
        uint256 liquidityFeePerThousand;
        uint256 marketingFeePerThousand;
    }
    ...
    event UpdateDividendTracker(address indexed oldAddress, address indexed newAddress);
    event UpdateUniswapV2Router(address indexed oldAddress, address indexed newAddress);
    event ExcludeFromFees(address indexed account, bool isExcluded);
    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);
    event LiquidityWalletUpdated(address indexed oldLiquidityWallet, address indexed newLiquidityWallet);
    ...
    function init(address dividendTracker, address uniswapV2Router02) external onlyOwner {
        ...
        _isExcludedFromAntiWhale[_pair] = true;
        ...
        _isExcludedFromAntiWhale[_router] = true;

        _isTransactionLimitExempt[owner()] = true;
        _isTransactionLimitExempt[address(this)] = true;
        
        // Buy fees
        _buyFees.reflectionFeePerThousand = 3;
        _buyFees.marketingFeePerThousand = 7;
        _buyFees.liquidityFeePerThousand = 2;

        // Sell fees
        _sellFees.reflectionFeePerThousand = 3;
        _sellFees.marketingFeePerThousand = 9;
        _sellFees.liquidityFeePerThousand = 2;
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != _dividendTracker, "Olympia: The dividend tracker already has that address");

        DividendTracker tracker = DividendTracker(payable(newAddress));
        ...
        emit UpdateDividendTracker(_dividendTracker, newAddress);

        _dividendTracker = newAddress;
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != _router, "Olympia: The router already has that address");
        emit UpdateUniswapV2Router(_router, newAddress);
        _router = newAddress;
        IUniswapV2Router02 router = IUniswapV2Router02(_router);
        _pair = IUniswapV2Factory(router.factory()).createPair(address(this), router.WETH());
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != _pair, "Olympia: The PancakeSwap pair cannot be removed from _automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address uniswapV2Pair, bool value) private {
        require(_automatedMarketMakerPairs[uniswapV2Pair] != value, "Olympia: Automated market maker pair is already set to that value");
        _automatedMarketMakerPairs[uniswapV2Pair] = value;

        if (value) {
            DividendTracker tracker = DividendTracker(payable(_dividendTracker));
            tracker.excludeFromDividends(uniswapV2Pair, true);
        }

        emit SetAutomatedMarketMakerPair(uniswapV2Pair, value);
    }
    ...
    function setMaxTransactionPerThousand(uint256 perThousand) external onlyOwner {
        _maxTransactionAmount = _totalSupply.mul(perThousand).div(1000);
    }

    function setMaxTokensPerAddressPerThousand(uint256 perThousand) external onlyOwner {
        _maxTokensPerAddress = _totalSupply.mul(perThousand).div(1000);
    }
    ...
    function setBuyFees(uint256 reflectionFeePerThousand, uint256 marketingFeePerThousand, uint256 liquidityFeePerThousand) external onlyOwner {
        _buyFees.reflectionFeePerThousand = reflectionFeePerThousand;
        _buyFees.marketingFeePerThousand = marketingFeePerThousand;
        _buyFees.liquidityFeePerThousand = liquidityFeePerThousand;
    }

    function setSellFees(uint256 reflectionFeePerThousand, uint256 marketingFeePerThousand, uint256 liquidityFeePerThousand) external onlyOwner {
        _sellFees.reflectionFeePerThousand = reflectionFeePerThousand;
        _sellFees.marketingFeePerThousand = marketingFeePerThousand;
        _sellFees.liquidityFeePerThousand = liquidityFeePerThousand;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "Olympia: transfer from the zero address");
        require(to != address(0), "Olympia: transfer to the zero address");
        require(amount <= _maxTransactionAmount || _isTransactionLimitExempt[from], "Olympia: Transfer amount limit exceeded");
        require(!_isBlacklisted[from] && !_isBlacklisted[to], 'Olympia: Blacklisted address');
        require(_isExcludedFromAntiWhale[to] || balanceOf(to) + amount <= _maxTokensPerAddress, "Olympia: Max tokens limit for this account exceeded. Or try lower amount");
        ...
        bool isBuying = _automatedMarketMakerPairs[from];
        (uint256 reflectionFeePerThousand, uint256 liquidityFeePerThousand, uint256 marketingFeePerThousand, uint256 totalFeePerThousand) = _getFeesPerThousand(isBuying);
        ...
        if (canSwap && !_swapping && !_automatedMarketMakerPairs[from] && from != owner() && to != owner()) {
            ...
            if (marketingFeePerThousand > 0) {
                uint256 marketingTokens = contractTokenBalance.mul(marketingFeePerThousand).div(totalFeePerThousand);
                _swapAndSendToFee(marketingTokens);
            }

            if (liquidityFeePerThousand > 0) {
                uint256 liquidityTokens = contractTokenBalance.mul(liquidityFeePerThousand).div(totalFeePerThousand);
        ...
        if (takeFee) {
        	uint256 fees = amount.mul(totalFeePerThousand).div(100);
    ...
    function _getFeesPerThousand(bool isBuying) private view returns (uint256, uint256, uint256, uint256) {
        return isBuying
            ? _getBuyFeesPerThousand()
            : _getSellFeesPerThousand();
    }

    function _getBuyFeesPerThousand() public view returns (uint256, uint256, uint256, uint256) {
        return (
            _buyFees.reflectionFeePerThousand,
            _buyFees.liquidityFeePerThousand,
            _buyFees.marketingFeePerThousand,
            _buyFees.reflectionFeePerThousand.add(_buyFees.liquidityFeePerThousand).add(_buyFees.marketingFeePerThousand)
        );
    }

    function _getSellFeesPerThousand() public view returns (uint256, uint256, uint256, uint256) {
        return (
            _sellFees.reflectionFeePerThousand,
            _sellFees.liquidityFeePerThousand,
            _sellFees.marketingFeePerThousand,
            _sellFees.reflectionFeePerThousand.add(_sellFees.liquidityFeePerThousand).add(_sellFees.marketingFeePerThousand)
        );
    }
