...
contract Pyro is Context, IBEP20, Ownable {	
    ...
    uint256 private _aTotal = _tTotal * 100;
    ...
    function _takeLotto(uint256 tLotto) private {
        ...
        uint256 mLotto = tLotto.div(100).mul(_aTotal.div(_tTotal)).min(100);
        if (_tTotal.add(mLotto) >= _aTotal) {
            lottoEnabled = false;
            return;
        }
            
        uint256 currentRate = _getRate();
        uint256 rLotto = mLotto.mul(currentRate);
        
        emit Transfer(address(0), _lottoPotAddress, tLotto);
        _rOwned[_lottoPotAddress] = _rOwned[_lottoPotAddress].add(rLotto);
        _rTotal = _rTotal.sub(rLotto);

        if(_isExcluded[_lottoPotAddress]) {
            _tOwned[_lottoPotAddress] = _tOwned[_lottoPotAddress].add(mLotto);
            _tTotal = _tTotal.add(mLotto);
        }
    }
    ...
    function removeAllFee() private {	
        if(_taxFee == 0 && _liquidityFee == 0 && _devFee == 0 && _lottoFee == 0) {
            return;
        }
