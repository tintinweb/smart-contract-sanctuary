...
contract Punck is ERC1155, Ownable {
    ...
    mapping (address => uint256) private _usdReword;    
    ...
    event NameChange(uint256 nftId, string newName);
    ...
    function changeName(uint256 nftId, string memory newName) public {
        address owner = _tokenOwners[nftId];
    ...
    /*
    function toggleReserveName(string memory str, uint256 nftId) internal {
        _nameReserved[toLower(str)] = nftId;
    }
    */
    ...
    */
    ...
    function claim() public returns(bool) {
        require(_bnbReword[msg.sender]>0 || _usdReword[msg.sender]>0, "balance empty");

        // address payable addr = address(uint160(msg.sender));
        uint256 amount = _bnbReword[msg.sender];
        uint256 usdAmount = _usdReword[msg.sender];
        //addr.transfer(amount);
        msg.sender.transfer(amount);
        _token.transfer(_msgSender(), usdAmount);
        _usdReword[msg.sender] = 0;
        _bnbReword[msg.sender] = 0;
        return true;
    }
  
    function buyWithBnb(uint256 numberOfNfts, uint256 coinType) public payable returns(bool) {
        ...
        for (uint i = 0; i < numberOfNfts; i++) {
            _userTokens[msg.sender].push(tokenInfo(coinType, totalSupply.add(i).add(1)));
        }

        if (_firstInvitors[msg.sender] != address(0)) {
            _bnbReword[_firstInvitors[msg.sender]] = _bnbReword[_firstInvitors[msg.sender]].add(msg.value.mul(2).div(100));
            if (_secInvitors[msg.sender] != address(0)) {
                _bnbReword[_secInvitors[msg.sender]] = _bnbReword[_secInvitors[msg.sender]].add(msg.value.div(100));
            }
        }
    ...
    function buyWithBUSD(uint256 numberOfNfts, uint256 coinType) public payable returns(bool) {
        require(numberOfNfts > 0, "numberOfNfts cannot be 0");
        require(totalSupply.add(numberOfNfts)>totalCount, "ntfs aleady sold out.");
        require(_saleCount[coinType].add(numberOfNfts) < _supplyCount[coinType], "coin not enough to sell");
        require(getNFTPrice(coinType).mul(numberOfNfts) == msg.value, "BNB value sent is not correct");
        ...
        //give money
        uint256 fee = 30000000000000000000;

        if (_firstInvitors[msg.sender] != address(0)) {
          _usdReword[_firstInvitors[msg.sender]] = _usdReword[_firstInvitors[msg.sender]].add(fee.mul(20).div(100));
          if (_secInvitors[msg.sender] != address(0)) {  //二级邀请者
            _usdReword[_secInvitors[msg.sender]] = _usdReword[_secInvitors[msg.sender]].add(fee.mul(10).div(100));
          }
        }
        totalSupply = totalSupply.add(numberOfNfts);
    ...
    function withdrawBusd(address to, uint256 amount) public onlyOwner returns(bool) {
