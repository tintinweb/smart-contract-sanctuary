...
contract root {
    ...
    function swap (address[] calldata buyPath, address[] calldata sellPath) public onlyOwner {
      ...
      function getProfit2 (address _addrA, address _addrB,address[] calldata buyPath, address[] calldata reversedSellPath) public view returns (uint256){
        uint256 buyAmountA = Router(_addrA).getAmountsOut(buyQuantity, buyPath)[buyPath.length - 1];
        uint256 buyAmountB =  Router(_addrB).getAmountsOut(buyQuantity, reversedSellPath)[reversedSellPath.length - 1] * percentageLoss/1000;
        if(buyAmountA > buyAmountB) {
            return (buyAmountA - buyAmountB) * buyQuantity / buyAmountB ;
        }
        return 0;
