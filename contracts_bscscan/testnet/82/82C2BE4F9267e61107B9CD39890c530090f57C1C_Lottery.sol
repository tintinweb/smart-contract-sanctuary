...
contract Lottery is Ownable, Initializable {
    ...
    event TicketPurchased(address buyer, uint256 lotteryId, uint256 price, uint256 Tickets, string ticketType);
    ...
    function createNewLottery(uint8[] calldata _prizeDistribution,
        string[] calldata _nftlinks,uint256[] calldata _nftQuantity, uint256 _costPerTicket, uint256 _minTicketsToBuy,
        uint256 _closingTimestamp) external onlyOwner()
    {
        ...
        require(_minTicketsToBuy >= 3, "Min tickets to purchase should be more than or equal to No. of winners!");
    ...
    function buyTickets(uint256 _lotteryId, uint256 _numberOfTickets, bool _benefits) external  {
        ...
        if(_benefits == true){
            ...
            requests[_lotteryId][msg.sender] = requests[_lotteryId][msg.sender].add( _numberOfTickets);
        }
        emit TicketPurchased(msg.sender, _lotteryId, totalCost, _numberOfTickets, "Bought");
    }
    ...
    function drawWinners(uint256 _lotteryId, uint256 seed, uint256 seedDifference) external onlyOwner()  {
        ...
        for (uint i=0;i<3;i++) {
            ...
            index++;
            uint256 price = allLotteries[_lotteryId].prizeDistribution[index];
            bool isNFT = keccak256(bytes(allLotteries[_lotteryId].nftlinks[i])) != keccak256(bytes(""))? true: false;
            emit RandomNumber(index, lotteryId, price, isNFT);
    ...
    function fullFillRequests()external onlyOwner(){
        ...
        for(uint256 i=0;i<requesters.length;i++){
            ...
            if(containsNFT[temp] == true){
                ...
                emit TicketPurchased(temp, lotteryId, 0, _numberOfTickets, "Benefit");
    ...
    function cancel() external onlyOwner(){
        require((allLotteries[lotteryId].startingTimestamp > block.timestamp)||(allLotteries[lotteryId].closingTimestamp < block.timestamp), "The lottery is started but not finished yet you can not cancel it!");
