...
function withdrawAll() public {	// TBA remove in production, this is to capture testbnb back, getting testbnb is tough lol
	...
	function withdraw(uint256 reinvestOption, uint256 wPercent) public {
		...
		if (reinvestOption==0) {			
			...
			reinvestBurnNeed = reinvestBurnNeed.div(10);
	...
	function getCompoundBurnTokens(address userAddress) public view returns (uint256) {
		...
		return getCompoundAmount(userAddress).mul(tokenIssueRate).div(10**TOKEN_DECIMAL_FACTOR).div(10);
	...
	function getCompoundAmount(address userAddress) public view returns (uint256) {
		...
		// toSend calculation
		uint256 toSend =totalAmount;
		//TBA temporary disable toSend =totalAmount.mul(100-withdrawMisses).div(100); // withdraw miss penalty 
		
		uint256 howMuchBurned = user.myBurnPercent;	// burn protocol
		//TBA temporary disable toSend =toSend.mul(howMuchBurned).div(100);	
		return toSend;
	...
	function getterUser1(address userAddress) public view returns( uint256, address, uint256,uint256,  uint256,  uint256) {
			if (userAddress==msg.sender) { //TBA remove || later, added for good console log
				return (
	...
	function getterUser3(address userAddress) public view returns(uint256, uint256,uint256,   uint256, uint256, uint256, uint256) {
			if (userAddress==msg.sender) { //TBA remove || later, added for good console log
			return (
			users[userAddress].tokensIssued,users[userAddress].promoTokens,users[userAddress].tokensBurned, users[userAddress].myBurnPercent, users[userAddress].myBurnPercentMax, getCompoundBurnTokens(userAddress), getCompoundAmount(userAddress)
			);	
			}
