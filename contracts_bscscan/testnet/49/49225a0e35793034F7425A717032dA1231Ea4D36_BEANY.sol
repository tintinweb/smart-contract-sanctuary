...
contract BEANY is IERC20Metadata, Ownable {
    ...
    address public charityWallet = 0x5A312c525cfff880A80f8fD1d4D709C05C2Ce1Ce; // Marketing Address
    ...
    uint256 public sellingRfiFee = 4;
    uint256 public buyBackFee = 3;
    uint256 public liquidityFee = 2;
    
    uint256 public buyersTotalFees = rfiFee.add(charityFee);
    uint256 public sellersTotalFees = sellingRfiFee.add(buyBackFee).add(liquidityFee);
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        ...
        require(!swapping, "BEANY: A swapping process is currently running, wait till that is complete");
        require(contractBalance >= amount, "BEANY: Insufficient Funds");
    ...
    function _takeFees(uint256 amount, uint256 currentRate, uint256 sumOfFees, bool isBuying) private {
        if ( sumOfFees > 0 && !isInPresale ){
            ...
            _takeFee( amount, currentRate, charityFee, charityWallet); // charity fee
            
            if(!isBuying) {
                _redistribute( amount, currentRate, sellingRfiFee);  // redistribute to holders
                _takeFee( amount, currentRate, buyBackFee, address(this)); // buy back fee
