...
contract vampDOGE is ERC20, Ownable {
    ...
    address payable public marketingWallet = 0x90B71Df497076fD558B6f93a45589911e1Ef15D5;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "vampDOGE: The router already has that address");
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "vampDOGE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "vampDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "vampDOGE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 1000000, "vampDOGE: gasForProcessing must be between 200,000 and 1,000,000");
        require(newValue != gasForProcessing, "vampDOGE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "vampDOGE: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndBuyback(uint256 tokenAmount) private {
        ...
        transferToAddressETH(marketingWallet, bnbSold.div(buybackFee).mul(manualBuybackFee));
...
contract vampdogeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("vampdoge_Dividend_Tracker", "vampdoge_Dividend_Tracker") {
    	claimWait = 1200;
    ...
    function withdrawDividend() public override {
        require(false, "vampdoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main vampdoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 900 && newClaimWait <= 86400, "vampdoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "vampdoge_Dividend_Tracker: Cannot update claimWait to same value");
