...
contract KawaiiMarketPlace is Pausable, SignData {
    ...
    enum Status{AUCTION, CLOSE, CANCEL, EXPIRE}
    struct Auction {
        uint256 tokenId;
        address seller;
        uint256 amount;
    ...
    external view returns (address seller, uint256 amount, Status status, uint256 startingPrice, uint256 endingPrice, uint256 duration, uint256 startedAt)
    {
        ...
        if (_auction.status == Status.AUCTION && block.timestamp > uint256(_auction.startedAt).add(_auction.duration)) _auction.status = Status.EXPIRE;
