...
// File: contracts/GeometryProfile.sol

pragma solidity ^0.6.0;

/** @title GeometryProfile.
@dev It is a contract for users to bind their address 
to a customizable profile by depositing a NFT.
*/
contract GeometryProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public gxoToken;
    ...
    uint256 public numberGXOToReactivate;
    uint256 public numberGXOToRegister;
    uint256 public numberGXOToUpdate;
    ...
    // Event to notify a user pausing their profile
    ...
    // Event to notify that a user is reactivating this profile
    ...
    // Event to notify that a user is pausing this profile
    event UserUpdate(
        address indexed userAddress,
        address nftAddress,
        uint256 tokenId
    ...
    constructor(
        IBEP20 _gxoToken,
        uint256 _numberGXOToReactivate,
        uint256 _numberGXOToRegister,
        uint256 _numberGXOToUpdate
    ) public {
        gxoToken = _gxoToken;
        numberGXOToReactivate = _numberGXOToReactivate;
        numberGXOToRegister = _numberGXOToRegister;
        numberGXOToUpdate = _numberGXOToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends GXO to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer GXO tokens to this contract
        gxoToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberGXOToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer GXO token to this address
        gxoToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberGXOToUpdate
    ...
    /**
     * @dev Claim GXO to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        gxoToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of GXO to register
     * Callable only by owner admins.
     */
    function updateNumberGXO(
        uint256 _newNumberGXOToReactivate,
        uint256 _newnumberGXOToRegister,
        uint256 _newNumberGXOToUpdate
    ) external onlyOwner {
        numberGXOToReactivate = _newNumberGXOToReactivate;
        numberGXOToRegister = _newnumberGXOToRegister;
        numberGXOToUpdate = _newNumberGXOToUpdate;
    }
