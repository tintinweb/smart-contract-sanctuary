...
contract Xgame is ERC721 {
    ...
    //read Role info from tokenId
    function roleInfo(uint _tokenId) external view returns (uint _xp, uint _class, uint _level) {
        _xp = xp[_tokenId];
        _class = class[_tokenId];
        _level = level[_tokenId];
    }
       //define Role name and class
