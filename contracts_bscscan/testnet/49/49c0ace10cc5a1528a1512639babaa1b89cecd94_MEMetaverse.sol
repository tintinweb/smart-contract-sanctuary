...
//remove ITOKEN since we don't need calim token here
contract MEMetaverse is Context, IBEP20, Ownable {
    ...
    */
    ...
    function transfer(address recipient, uint256 amount) external returns (bool) {
        if (_totalSupply > _limitSupply) {
            _transfer(_msgSender(), recipient, (amount.div(100)).mul(100));
            _burn(_msgSender(), (amount.div(100)).mul(0));
        } else {
