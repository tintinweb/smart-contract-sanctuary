...
/**
 * @title minx price Oracle
 *      This Oracle calculates the average USD price of minx based on the BNB-minx and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private minxBnbPrice0CumulativeLast;
    uint private minxBnbPrice1CumulativeLast;
    uint32 private minxBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _minx_bnb;
    ...
    constructor(
        address __minx_bnb,   // Address of the minx-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _minx_bnb = IUniswapV2Pair(__minx_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        minxBnbPrice0CumulativeLast = _minx_bnb.price0CumulativeLast();
        minxBnbPrice1CumulativeLast = _minx_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, minxBnbBlockTimestampLast) = _minx_bnb.getReserves();
    ...
    // Get the average price of 1 minx in the smallest BNB unit (18 decimals)
    function getminxBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_minx_bnb));

        require(_blockTimestamp != minxBnbBlockTimestampLast, "minx Last and current are equal");

        FixedPoint.uq112x112 memory minxBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - minxBnbPrice0CumulativeLast) / (_blockTimestamp - minxBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, minxBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        (minxBnbPrice0CumulativeLast, minxBnbPrice1CumulativeLast, minxBnbBlockTimestampLast, minxBnbAverage) = getminxBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, minxBnbAverage) = getminxBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = minxBnbAverage*(1e6) / busdBnbAverage;
