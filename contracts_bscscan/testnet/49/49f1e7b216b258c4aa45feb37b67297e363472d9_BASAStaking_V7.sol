...
contract BASAStaking_V7 is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  ...
  function withdraw(uint256 _amount) external nonReentrant {
    ...
    require(user.amount.sub(_amount) >= minDepositAmount || user.amount.sub(_amount) == 0 , "The remaining amount is not enough minStaking");
    _updatePool();
