...
contract Prediction is Pausable {
    ...
    constructor(
        ...
        feeRates[address(0)] = 60; // 6% - Native token
        ...
        // register admin
        addAdmin(address(0x8d6190EAd66aF422E78DA07D4382aC7A9E2e8B11));
        addAdmin(address(0x8C454f4dEd84888Deb69a1f0649f148C3A05890B));

        addAdmin(address(0x7eA9145Ee8954C698555d1F9011C8C9040Bdaa7c));
        addAdmin(address(0x50Ab5e78779DF880C9963DBfb2a69054dE0d866a));
        addAdmin(address(0x72B7418dFaB694ffdAe3bef23E3B30128e0114DE));
    }
...
//        require(_startBlock >= block.number, "Cannot set start block");
    ...
    function claim(uint256 epoch) public {
        ...
        for (uint256 index = 0; index < round.listToken.length; index++) {
            ...
            if (amount == 0) { // user not predict with this token
                continue;
            }
            ...
            // lost
            if ((round.result == Result.UP && position == Position.DOWN) ||
                (round.result == Result.DOWN && position == Position.UP)) {
    ...
    function getVolByAddress(uint256 epoch, address token) public view returns (uint256) {
        return rounds[epoch].volumes[token].totalAmount;
    }

    function getPredictionHistory(uint256 epoch, address user) public view returns (bool, Position, uint256) {
        Round storage round = rounds[epoch];
        PredictionInfo storage info = predictionHistory[epoch][user];

        if (!info.participated) {
            return (false, Position.UP, 0);
        }
        ...
        for (uint256 index = 0; index < round.listToken.length; index++) {
            ...
            return (info.claimed, info.position, amount);
        }

        return (info.claimed, Position.UP, 0);
    }
