...
/**
 * @notice Draw the final number, calculate reward in GOUDA per group, and make lottery claimable
 * @param _lotteryId: lottery id
 ...
 * @notice Inject funds
 * @param _lotteryId: lottery id
 * @param _amount: amount to inject in GOUDA token
 * @dev Callable by operator
 */
 ...
 * @param _priceTicketInGouda: price of a ticket in GOUDA
 ...
 * @param _seed: seed provided by the CowSwap lottery
 */
function getRandomNumber(uint256 _seed) external override {
require(msg.sender == cowSwapLottery, "Only cowSwapLottery");
...
/**
 * @notice Set the address for the cowSwapLottery
 * @param _cowSwapLottery: address of the CowSwap lottery
 */
function setLotteryAddress(address _cowSwapLottery) external onlyOwner {
cowSwapLottery = _cowSwapLottery;
}
...
latestLotteryId = ICowSwapLottery(cowSwapLottery).viewCurrentLotteryId();
}
}