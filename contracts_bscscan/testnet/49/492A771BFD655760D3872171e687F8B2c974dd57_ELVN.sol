...
contract ELVN is IERC20, Ownable {
    ...
    constructor(address _router, address _poolAddr) public {
        require(
            ...
            "ELVN: Zero address in constructor."
        );
        require(
    ...
    function removeAllFee() private {
        if (
            _taxFee == 0 && _liquidityFee == 0 && _poolFee == 0 && _burnFee == 0
        ) return;
        ...
        returns (uint256[8] memory)
    {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(
            [tAmount, tFee, tLiquidity, tPool, _getRate()]
        );
        return (
            [
                rAmount,
        ...
        )
    {
        ...
        values[0] = calculateTaxFee(tAmount);
        values[1] = calculateLiquidityFee(tAmount);
        values[2] = calculatePoolFee(tAmount);
        values[3] = calculateBurnFee(tAmount);
        values[4] = tAmount.sub(values[0]).sub(values[1]).sub(values[2]).sub(
            values[3]
        );
        return (values[4], values[0], values[1], values[2], values[3]);
    }

    function _getRValues(uint256[5] memory values)
        private
        pure
        returns (
            uint256,
    ...
    {
        uint256 rAmount = values[0].mul(values[4]); // tAmount
        uint256 rFee = values[1].mul(values[4]); // tFee
        uint256 rLiquidity = values[2].mul(values[4]); // tLiquidity
        uint256 rPool = values[3].mul(values[4]); // tPool
    ...
    function setBurnFee(uint256 _newFee) external onlyOwner {
        require(
            _newFee <= 100,
            "ELVN: Fee can't be bigger than 100!"
        );
        _burnFee = _newFee;
    }
