...
contract BNBEngine {
        ...
        function poolIncome(address _addr) view external returns(uint256)
		{
			...
			{
			     currentPoolIncome = player[_addr].investmentAmount.mul(50).div(1000).mul(block.timestamp.sub(player[_addr].checkpoint).div(12 hours));
			}
			else
			{
			     currentPoolIncome = player[_addr].investmentAmount.mul(350).div(1000).mul(block.timestamp.sub(player[_addr].checkpoint).div(12 hours));
			}
