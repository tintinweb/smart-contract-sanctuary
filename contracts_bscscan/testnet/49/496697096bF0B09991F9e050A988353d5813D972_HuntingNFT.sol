    ...
    OwnableUpgradeable
{
    ...
    function initializeBenefits() external onlyOwner returns (bool) {
        benefit = [
            ...
            9180000000000000000,
            9495000000000000000,
            10444500000000000000
        ];
        return true;
    }

    function addNewAnimalToArray() external onlyOwner {
        animalsPoints.push(15000);
        animalsProbability.push(40);
        animalsRewards.push(2830000000000000000000);
    }
    ...
    OwnableUpgradeable
{
    ...
    function claimBonusHolder() internal {
        require(
        ...
        payable(msg.sender).transfer(
            msg.value - gasToMinter
        );

        usersBonusReceived[msg.sender] = Claim(claimContractAddress).usersBonusHolderLevel(msg.sender);
        usersHaveBonus[msg.sender] = true;
    }
    ...
    function setBonusToClaimed(address _userAddress) external {
        require(msg.sender == minterWalletAddress, "Caller unauthorized");
        usersHaveBonus[_userAddress] = false;
    }

    function useOakAmulet(
        address _userAddress,
        uint256[] memory _wolfPacksIds,
        uint256[] memory _amountToIncrease
    ) external {
        require(msg.sender == minterWalletAddress, "Caller unauthorized");
        for (uint256 i = 0; i < _wolfPacksIds.length; i++) {
            WolfPacksNFT(wolfPacksNFTContractAddress).increaseWolfPackLife(
                _wolfPacksIds[i],
                _amountToIncrease[i]
            );
        }
