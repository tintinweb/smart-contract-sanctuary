...
contract LockedTokenSale {
    ...
    constructor(address _pancakeRouter, address _tokenVesting, address _ref, address _Token) {
    ...
    function getLockedTokenPrice(uint plan) public view returns (uint){
        ...
        // uint pancake_price = 1e15;
        uint price;
        if( plan == 1 ) { // R1 plan for regular investors
            price = pancake_price * 70 / 100;
            // $ 0.97 = 97 * 1e16
            uint price_limit = (97 * 1e16) * 1e8 / ref.latestAnswer();
            if( price < price_limit )
                price = price_limit;
        }
        if( plan == 2) { // R2 plan
            price = pancake_price * 50 / 100;
            // $ 0.87 = 87 * 1e16
            uint price_limit = (87 * 1e16) * 1e8 / ref.latestAnswer();
            if( price < price_limit )
                price = price_limit;
        }
        return price / 100;
    ...
    function sellLockedToken(uint plan, address referrer) public payable returns(uint){
        uint referral_value = msg.value * referral_ratio / 1e8;
        if(referrer != address(0x0) || referrer != msg.sender) {
            payable(referrer).transfer(referral_value);
        }
        uint value = msg.value - referral_value;
        ...
        uint256 endEmission = block.timestamp + 60 * 10 * plan;
    ...
    function bytesToAddress(bytes calldata source) internal pure returns(address) {
        uint result;
        uint mul = 1;
        for(uint i; i < 20; i++) {
            result = result + mul * uint8(source[19-i]);
            mul = mul * 256;
        }
    ...
    function approve() external {
        IERC20(token).approve(address(tokenVesting), 1e20);
    }
}
