...
interface GachaTicket is IERC20 {
    function mint(uint256 amount) external;
}

interface GachaCapsule is IERC721Enumerable {
    function mint (address _minter, uint256 _stars) external;
    function getStar(uint256 _tokenId) external view returns (uint256);
}
...
contract GachaponMachine {
    ...
    function _random() internal view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(block.timestamp + salt))) % 100;
    }
}