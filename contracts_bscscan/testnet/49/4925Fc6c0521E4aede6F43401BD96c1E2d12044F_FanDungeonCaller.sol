...
contract FanDungeonCaller {
        ...
        returns (Hero[] memory heroes)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            IFanDungeonHero.Stats memory stats = IFanDungeonHero(_target)
                .statsOf(tokenIds[i]);
            IFanDungeonHero.Part memory head = IFanDungeonHero(_target).partOf(
                tokenIds[i],
                HeroInfo.Position.head
            );
            IFanDungeonHero.Part memory upper = IFanDungeonHero(_target).partOf(
                tokenIds[i],
                HeroInfo.Position.upper
            );
            IFanDungeonHero.Part memory lower = IFanDungeonHero(_target).partOf(
                tokenIds[i],
                HeroInfo.Position.lower
            );
            IFanDungeonHero.Part memory hat = IFanDungeonHero(_target).partOf(
                tokenIds[i],
                HeroInfo.Position.hat
            );
            IFanDungeonHero.Part memory tool = IFanDungeonHero(_target).partOf(
                tokenIds[i],
                HeroInfo.Position.tool
            );
            heroes[i] = Hero({
                stats: stats,
                parts: Parts({
        ...
        returns (LandCaller[] memory lands)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ILandInfo.Land memory land = IFanDungeonLand(_target).landOf(
                tokenIds[i]
            );
            lands[i] = LandCaller({
                id: tokenIds[i],
                ...
                owner: IERC721(_target).ownerOf(tokenIds[i]),
                zone: land.zone,
                xPos: land.positionX,
                yPos: land.positionY
            });
    ...
    function getDecorations(address _target, uint256[] calldata tokenIds)
        external
        view
        returns (DecorationCaller[] memory decorations)
    {
        decorations = new DecorationCaller[](tokenIds.length);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            IFanDungeonDecoration.Decoration
                memory decoration = IFanDungeonDecoration(_target).decorationOf(
                    tokenIds[i]
                );
    ...
    function getDungeon(
        ...
        uint256 crystalPoolSize = IFanDungeonAdventure(_adventureAddress)
            .crystalPoolSize();
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            dungeons[i] = DungeonCaller({
                id: tokenIds[i],
                remainingCrystal: crystalPoolSize - dungeon.reservedCrystal,
                fee: stage.fee,
    ...
    // function getDungeonLevelInfo(uint32 start,uint32 end) external view returns(DungeonLevelInfoCaller memory) {
