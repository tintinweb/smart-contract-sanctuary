...
contract Olympia is ERC20, Ownable {
    ...
    address private _dividendTracker;
    ...
    uint256 public _swapThreshold = 0;
    ...
    address public _providerWallet = 0xe8C17dB3c4f158908593B6a3b880053E87C0ecF0;
    ...
    function init(address newDividendTracker, address newRouter) external onlyOwner {
    ...
    function _shouldSwap(address sender, address recipient) private view returns (bool) {
        // TODO: Check sender != owner() && recipient != owner()
        return balanceOf(
    ...
    function _swap() private swapping {
        ...
        uint256 liquidityTokenAmount = tokenBalance.mul(liquidityFeesPerTenThousand).div(totalFeesPerTenThousand);
        _swapAndLiquify(liquidityTokenAmount);

        uint256 teamTokenAmount = tokenBalance.mul(teamFeesPerTenThousand).div(totalFeesPerTenThousand);
        _swapAndSendFeesToWallet(_teamWallet, teamTokenAmount);

        uint256 providerTokenAmount = tokenBalance.mul(providerFeesPerTenThousand).div(totalFeesPerTenThousand);
        _swapAndSendFeesToWallet(_providerWallet, providerTokenAmount);

        uint256 marketingTokenAmount = tokenBalance.mul(marketingFeesPerTenThousand).div(totalFeesPerTenThousand);
        _swapAndSendFeesToWallet(_marketingWallet, marketingTokenAmount);

        uint256 remainingTokenBalance = balanceOf(address(this));
        _swapAndSendDividends(remainingTokenBalance);
    }

    function abc_swapAndLiquify() external onlyOwner {
        uint256 tokenBalance = balanceOf(address(this));
        (
            uint256 liquidityFeesPerTenThousand,
            /* uint256 teamFeesPerTenThousand */,
        ...
        uint256 liquidityTokenAmount = tokenBalance.mul(liquidityFeesPerTenThousand).div(totalFeesPerTenThousand);
        liquidityTokenAmount = tokenBalance;
        _swapAndLiquify(liquidityTokenAmount);
    }

    function abc_swapAndSendFeesToTeamWallet() external onlyOwner {
        uint256 tokenBalance = balanceOf(address(this));
        (
            /* uint256 liquidityFeesPerTenThousand */,
            uint256 teamFeesPerTenThousand,
        ...
        uint256 teamAmount = tokenBalance.mul(teamFeesPerTenThousand).div(totalFeesPerTenThousand);
        _swapAndSendFeesToWallet(_teamWallet, teamAmount);
    }

    function abc_swapAndSendFeesToProviderWallet() external onlyOwner {
        uint256 tokenBalance = balanceOf(address(this));
        (
            /* uint256 liquidityFeesPerTenThousand */,
            /* uint256 teamFeesPerTenThousand */,
        ...
        uint256 tokenAmount = tokenBalance.mul(providerFeesPerTenThousand).div(totalFeesPerTenThousand);
        _swapAndSendFeesToWallet(_providerWallet, tokenAmount);
    }

    function abc_swapAndSendFeesToMarketingWallet() external onlyOwner {
        uint256 tokenBalance = balanceOf(address(this));
        (
            /* uint256 liquidityFeesPerTenThousand */,
            /* uint256 teamFeesPerTenThousand */,
        ...
        uint256 tokenAmount = tokenBalance.mul(marketingFeesPerTenThousand).div(totalFeesPerTenThousand);
        _swapAndSendFeesToWallet(_marketingWallet, tokenAmount);
    }

    function abc_swapAndSendDividends() external onlyOwner {
        uint256 remainingTokenBalance = balanceOf(address(this));
        _swapAndSendDividends(remainingTokenBalance);
    }
    ...
    function _swapAndLiquify(uint256 tokenAmount) private {
        if (tokenAmount > 0) {
            ...
            _swapTokensForEth(halfTokenAmount);
            uint256 newEthBalance = address(this).balance.sub(initialEthBalance);
            _addLiquidity(otherHalfTokenAmount, newEthBalance);

            emit SwapAndLiquify(halfTokenAmount, newEthBalance, otherHalfTokenAmount);
        }
    }

    function _swapAndSendFeesToWallet(address wallet, uint256 tokenAmount) private {
        if (tokenAmount > 0) {
            uint256 initialEthBalance = address(this).balance;

            _swapTokensForEth(tokenAmount);
            uint256 newEthBalance = (address(this).balance).sub(initialEthBalance);
            payable(wallet).transfer(newEthBalance);
        }
    }
    ...
    function _swapAndSendDividends(uint256 tokenAmount) private {
        if (tokenAmount > 0) {
            uint256 initialEthBalance = address(this).balance;
            _swapTokensForEth(tokenAmount);
            uint256 dividends = (address(this).balance).sub(initialEthBalance);
    ...
    function _shouldTakeFees(address sender, address recipient) private view returns (bool) {
        // TODO: Check if no fee if recipient is excluded
        return !_inSwap && !_isExcludedFromFees[sender] && !_isExcludedFromFees[recipient];
    }
    ...
    function _distributeReflection(address sender, address recipient) private {
        DividendTracker tracker = DividendTracker(payable(_dividendTracker));
