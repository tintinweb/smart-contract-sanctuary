...
/**
 ...
 |  _ <| | __| |    / _` | |/ / _ \                                                  
 | |_) | | |_| |___| (_| |   <  __/                                                  
 |____/|_|\__|\_____\__,_|_|\_\___|  _                                         _     
 | |   | |         ___              | |                                       | |    
 | |__ | |_ ___   ( _ )     ___ __ _| | _____   _ __ _____      ____ _ _ __ __| |___ 
 | '_ \| __/ __|  / _ \/\  / __/ _` | |/ / _ \ | '__/ _ \ \ /\ / / _` | '__/ _` / __|
 | |_) | || (__  | (_>  < | (_| (_| |   <  __/ | | |  __/\ V  V / (_| | | | (_| \__ \
 |_.__/ \__\___|  \___/\/  \___\__,_|_|\_\___| |_|  \___| \_/\_/ \__,_|_|  \__,_|___/
                                                                                     
**/
 
/**
Telegram: https://t.me/BitCakeBSC
Website: https://bitcake.space
...
contract TEST is ERC20, Ownable {
    ...
    uint256 public cakeDividendRewardsFee = 7;
    uint256 public previousCakeDividendRewardsFee;
    uint256 public btcDividendRewardsFee = 7;
    uint256 public previousBtcDividendRewardsFee;
    uint256 public marketingFee = 2;
    uint256 public previousMarketingFee;
    uint256 public totalFees = cakeDividendRewardsFee.add(marketingFee).add(btcDividendRewardsFee).add(liquidityFee);
    ...
    constructor() ERC20("TEST" , "BTTEST") {
    	cakeDividendTracker = new CAKEDividendTracker();
    	btcDividendTracker = new BTCDividendTracker();
 
    	marketingWallet = 0xd8bDaD09352ded56F4008c82F4CF2FF61C27Dfb5;
    ...
    function setCakeDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = cakeDividendRewardsFee.add(marketingFee).add(btcDividendRewardsFee).add(liquidityFee);
            cakeDividendEnabled = _enabled;
        }
    ...
    function setBtcDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = btcDividendRewardsFee.add(marketingFee).add(cakeDividendRewardsFee).add(liquidityFee);
            btcDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(btcDividendRewardsFee).add(cakeDividendRewardsFee).add(liquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updatecakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "DualRewards: The dividend tracker already has that address");
 
        CAKEDividendTracker newcakeDividendTracker = CAKEDividendTracker(payable(newAddress));
 
        require(newcakeDividendTracker.owner() == address(this), "DualRewards: The new dividend tracker must be owned by the DualRewards token contract");
    ...
    function updatebtcDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(btcDividendTracker), "DualRewards: The dividend tracker already has that address");
 
        BTCDividendTracker newbtcDividendTracker = BTCDividendTracker(payable(newAddress));
 
        require(newbtcDividendTracker.owner() == address(this), "DualRewards: The new dividend tracker must be owned by the DualRewards token contract");
    ...
    function updateBtcDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = btcDividendRewardsFee.add(cakeDividendRewardsFee).add(marketingFee).add(liquidityFee);
    }
 
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        marketingFee = newFee;
        totalFees = marketingFee.add(cakeDividendRewardsFee).add(btcDividendRewardsFee).add(liquidityFee);
    }
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "DualRewards: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "DualRewards: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DualRewards: The BITCAKE pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "DualRewards: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "DualRewards: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "DualRewards: Trading has not started yet");
        ...
        if (!swapping && canSwap && from != uniswapV2Pair) {
            ...
            if (btcDividendEnabled) {
                uint256 btcTokens = contractTokenBalance.div(totalFees).mul(btcDividendRewardsFee);
...
contract CAKEDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("DualRewards_Cake_Dividend_Tracker", "DualRewards_Cake_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "DualRewards_Cake_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "DualRewards_Cake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DualRewards contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DualRewards_Cake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DualRewards_Cake_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BTCDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("DualRewards_Btc_Dividend_Tracker", "DualRewards_Btc_Dividend_Tracker", 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "DualRewards_Btc_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "DualRewards_Btc_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DualRewards contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DualRewards_Btc_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DualRewards_Btc_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
