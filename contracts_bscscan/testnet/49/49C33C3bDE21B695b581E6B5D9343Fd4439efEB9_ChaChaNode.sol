...
contract ChaChaNode is  Ownable {
    ...
    @dev setSetAddress
     */
    function setSetAddress(address _payToken,address _treasury) public onlyOwner {
     ...
     */
    function setPaymentToken() external onlyOwner {
    ...
    @dev Give sale price 
     */
    function getPublicSalePriceFor(uint256 index)
        public
        view
        returns (uint256)
    {
        uint256 diff  = index.div(increaseAmount);
        return initialSalePrice.add(diff.mul(increasePrice));
    }
    ...
    function buyNode() external payable {
        ...
        require(node[msg.sender].addr==address(0), "Is already  node.");
