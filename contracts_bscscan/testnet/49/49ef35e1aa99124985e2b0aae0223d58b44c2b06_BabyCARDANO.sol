...
library SafeMath {
    ...
    function div(uint256 a, uint256 b, string memory errorMessage) internal pure returns (uint256) {
        // Solidity only autoCARDANOally asserts when dividing by 0
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 CARDANO = IBEP20(0xaC2A889dEfE2205AA18BD46c0A24e8cAE7084DBB);
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = CARDANO.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(CARDANO);
        ...
        uint256 amount = CARDANO.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            CARDANO.transfer(shareholder, amount);
...
contract BabyCARDANO is IBEP20, Auth {
    using SafeMath for uint256;

    address CARDANO = 0xaC2A889dEfE2205AA18BD46c0A24e8cAE7084DBB;
    address WBNB = 0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "BabyCARDANO";
    string constant _symbol = "BabyCARDANO";
