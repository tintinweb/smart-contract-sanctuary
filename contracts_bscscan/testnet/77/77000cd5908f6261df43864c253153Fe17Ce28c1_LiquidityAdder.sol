...
contract LiquidityAdder is Auth {
	...
	function removeLiquidity(address lpToken, uint256 amount) internal {
		IDexPair t = IDexPair(lpToken);
		require(t.transferFrom(msg.sender, address(this), amount), "We didn't receive the LP tokens :(");
		uint256 totalLp = t.totalSupply();
		uint256 percentage = amount * 100 / totalLp;
		(, uint256 wbnb, ) = t.getReserves();
		uint256 bnb = percentage * wbnb / 100;
		router.removeLiquidityETHSupportingFeeOnTransferTokens(lpToken, bnb, 0, bnb, msg.sender, block.timestamp + 300);
	}
