...
contract PriceConsumerV3 {
    ...
    function updatePlan() external payable returns(uint256){
        uint256 _price = uint256(getLatestPrice());
        _price = 30000000000/_price;
        return _price;
    }
    
    function updatePlan2() external payable returns(bool){
        uint256 _price = uint256(getLatestPrice());
        _price = 30000000000/_price;
    ...
    function xxx(uint i) external returns(string memory){
        ...
        if(i == 1){
            uint256 _price = uint256(getLatestPrice());
            _price = (5000000000/_price)/10**8;
            payable(address(this)).transfer(_price);
            return "1";
        }else if(i==2){
             uint256 _price = uint256(getLatestPrice());
            _price = (30000000000/_price)/10**8;
            payable(address(this)).transfer(_price);
            return "1";
            return "2";
        }else{
