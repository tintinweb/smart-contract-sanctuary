...
contract WenDAOBondDepository is Ownable {
    ...
    address public immutable WenDAO; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints WDAO when receives principle
    ...
    struct Bond {
        uint payout; // WDAO remaining to be paid
    ...
    function setBondTerms ( PARAMETER _parameter, uint _input ) external onlyPolicy() {
        if ( _parameter == PARAMETER.VESTING ) { // 0
            require( _input >= 10800, "Vesting must be longer than 36 hours" );
    ...
    function setAdjustment ( 
        ...
        require( _increment <= terms.controlVariable.mul( 250 ).div( 1000 ), "Increment too large" );
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 WDAO ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) WDAO
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( WenDAO ).safeTransfer( DAO, fee ); 
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( WenDAO ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( WenDAO ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( WenDAO ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( WenDAO ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to WDAO supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( WenDAO ).totalSupply();
    ...
    /**
     *  @notice calculate amount of WDAO available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or WDAO) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != WenDAO );
