...
contract GymVaultsBank is ReentrancyGuard, Ownable, Relationship {
    ...
    uint256 public startBlock; // https://bscscan.com/block/countdown/4814500

    PoolInfo[] public poolInfo; // Info of each pool.
    mapping(uint256 => mapping(address => UserInfo)) public userInfo; // Info of each user that stakes LP tokens.
    RewardPoolInfo[] public rewardPoolInfo;
    uint256 public totalAllocPoint ; // Total allocation points. Must be the sum of all allocation points in all pools.
    // uint256 public unstakingFrozenTime;
    ...
    address public constant routerAddress = 0x367633909278A3C91f4cB130D8e56382F00D1071; // PancakeSwap: Router
    address public buyBack; //buy and burn contract address
    ...
    constructor(
        ...
        buyBack = 0x5852E79A6D495a2936Ee34b9E1960fE57D3Eb61e;
        totalAllocPoint = 0;
        ...
        addRewardPool(_gym, _gymRewardRate);
    ...
    function checkPoolDuplicate(IERC20 _want) public view {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].want != _want, "GymVaultsBank: existing pool?");
        }
    }
    ...
    function add(
        ...
        uint256 idx = poolInfo.length;
        poolInfo.push();
        PoolInfo storage pool = poolInfo[idx];
        pool.want = _want;
    ...
    function addRewardPool(address _rewardToken, uint256 _rewardPerBlock)
        public
        nonReentrant
        onlyOwner
    {
        require(
            ...
            "GymVaultsBank: Reward pool length > 16"
        );
        massUpdatePools();
        rewardPoolInfo.push(
            RewardPoolInfo({
    ...
    function updateRewardToken(
        ...
        require(_rewardPid >= 1, "GymVaultsBank: Core reward pool"); // GYM
        require(rewardPool.rewardPerBlock == 0, "GymVaultsBank: Old pool still running");
        massUpdatePools();
    ...
    function updateRewardPerBlock(uint256 _rewardPid, uint256 _rewardPerBlock)
        external
        nonReentrant
        onlyOwner
    {
        ...
        for (
            ...
            uint256 _reward =
                (multiplier * _rewardPerBlock * pool.allocPoint) /
                    totalAllocPoint;
            pool.accRewardPerShare[_rewardPid] =
                pool.accRewardPerShare[_rewardPid] +
                ((_reward * 1e18) / sharesTotal);
            pool.lastRewardBlock = block.number;
        }
    ...
    function _getReward(uint256 _pid) internal {
        ...
        for (
            ...
            if (!stopRewardPool[_rewardPid]) {
                uint256 _pending =
                    (_user.shares * _pool.accRewardPerShare[_rewardPid]) /
                        (1e18) -
                        (_user.rewardDebt[_rewardPid]);
                if (_pending > 0) {
                    RewardPoolInfo storage rewardPool =
                        rewardPoolInfo[_rewardPid];
                    address _rewardToken = rewardPool.rewardToken;
                    safeRewardTransfer(_rewardToken, msg.sender, _pending);
                    rewardPool.totalPaidRewards =
                        rewardPool.totalPaidRewards +
                        (_pending);
                    emit RewardPaid(
                        _rewardPid,
    ...
    // Want tokens moved from user -> GYMFarm (GYM allocation) -> Strat (compounding)
    ...
    function depositBNB() external payable nonReentrant onlyOnRelationship{
        IWETH(0xDfb1211E2694193df5765d54350e1145FD2404A1).deposit{value: msg.value}();
        _deposit(0, msg.value, false);
    }
    function _deposit(uint256 _pid, uint256 _wantAmt, bool _isToken) private {
        require(!pausePool[_pid], "GymVaultsBank: Paused");
        updatePool(_pid);
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (_wantAmt > 0) {
            ...
            distributeRewards(_wantAmt * RELATIONSHIP_REWARD / 100, address(pool.want), msg.sender);
            pool.want.safeTransfer(0x5852E79A6D495a2936Ee34b9E1960fE57D3Eb61e, _wantAmt * BUY_AND_BURN_GYM / 100);
            IBuyBack(buyBack).buyAndBurnToken(address(pool.want), _wantAmt * BUY_AND_BURN_GYM / 100, rewardPoolInfo[0].rewardToken);
        ...
        for (
            ...
            user.rewardDebt[_rewardPid] =
                (user.shares * (pool.accRewardPerShare[_rewardPid])) /
                (1e18);
        }
...
contract Relationship {
    ...
    uint8[5] private RELATIONSHIP_DIRECT_REFERRAL_BONUS;

    constructor() {
        RELATIONSHIP_DIRECT_REFERRAL_BONUS = [30, 25, 20, 15, 10];
        userToReferrer[msg.sender] = address(this);
    }


    modifier onlyOnRelationship() {
        require(userToReferrer[msg.sender] != address(0), "Relationship::Don't have Relationship");
        _;
    }
    ...
    function addRelationship(
        ...
        require(_user != address(0), "Relationship::user address can't be zero");
        require(_referrer != address(0), "Relationship::referrer address can't be zero");
        require(
    ...
    function distributeRewards(
        ...
        while (_index < RELATIONSHIP_DIRECT_REFERRAL_BONUS.length) {
            token.transfer(
                userToReferrer[_user],
                (_wantAmt * RELATIONSHIP_DIRECT_REFERRAL_BONUS[_index]) / 100
            );
