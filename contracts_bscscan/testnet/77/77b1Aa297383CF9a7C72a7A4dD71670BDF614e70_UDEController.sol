...
abstract contract UDEGO_TOKEN is IBEP20, Ownable {
    function mintUDE(address holder, uint256 amount) external virtual returns (bool);
    function referrer(address owner) external virtual returns (address);
}

contract UDEPOS is BEP20('UDEPOS Token', 'UDEGO_POS') {
    // The ude TOKEN!
    UDEGO_TOKEN public ude;
...
contract UDEController is Ownable {
    ...
    constructor(
        ...
        require(_lotteryAddr != address(0), "UDEController: _lotteryAddr should not be zero address");
    ...
    function setUDE(UDEGO_TOKEN _token) external onlyOwner {
        require(_token != UDEGO_TOKEN(0), "UDEController: _token should not be zero address");
