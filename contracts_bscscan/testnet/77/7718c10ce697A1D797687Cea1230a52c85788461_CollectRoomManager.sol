...
contract CollectRoomManager is SafeOwnable, Random {
    ...
    function add(
        ...
        IERC20[] memory _tokens, uint256[] memory _amounts, uint256[] memory _nftTypes, uint256[] memory _nftPercents
    ) external onlyOwner {
        ...
        for (uint i = 0; i < _nftTypes.length; i ++) {
            ...
            emit RoomRange(rid, _nftTypes[i], nftId, lastEndIndex, lastEndIndex.add(_nftPercents[i]));
