...
contract FOMOR3D is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 210 * 10**4 * 10**18;
    uint256 public _maxTxAmount = 210 * 10**4 * 10**18;
    uint256 public _numTokensToBeGroupLeader = 1000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "FOMO R3D";
    string private _symbol = "FMOR3D";
    ...
    IERC20 public usdt = IERC20(0x0C2Db8a1dD6a52e3f2fc2f39F3163C631d8541ee);  //IERC20(0x55d398326f99059fF775485246999027B3197955);
    ...
    uint256 public _numTokensToBeRefer = 1 * 10**18;
    ...
    event ReferRewards(address[REFER_DEPTH], uint256[REFER_DEPTH]);
    ...
    constructor (address developer) public {
        ...
        //exclude owner,  this contract,  and developer from fee
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee, bool hasRefer) public view returns(uint256) {
        ...
        (uint256[6] memory rValues,)  = _getValues(tAmount, _getFeeRates(hasRefer));
    ...
    function getValues(uint256 amount, bool hasRefer) public view returns (uint256[6] memory, uint256[6] memory){
        return _getValues(amount, _getFeeRates(hasRefer));
    }
    ...
    function _takeReferRewards(address recipient, uint256 rReferReward, uint256 tReferReward) private {
        ...
        for (uint256 i= 0; i < REFER_DEPTH - 1; i++){
            if (father != genesisAddress && father != address(0)){
        ...
        
        {
            ...
            for (uint256 i= 0; i < REFER_DEPTH - 1; i++){
                ...
                userInfo[beneficiaries[i]].referRewards[i] = userInfo[beneficiaries[i]].referRewards[i] + tRewards[i];    
            }
            ...
            userInfo[beneficiaries[_groupLeaderIndex]].referRewards[_groupLeaderIndex] = userInfo[beneficiaries[_groupLeaderIndex]].referRewards[_groupLeaderIndex] + tRemain;     
            tRewards[_groupLeaderIndex] = tRemain;
            
            emit ReferRewards(beneficiaries,tRewards);
        } 
    ...
    function _getFeeRates(bool hasRefer) private view returns (uint256[ALLFEE_NUMBER] memory){
    ...
    function _transfer(
        ...
        if (_lottery != address(0)){
            if (takeFee && from == uniswapV2Pair ) {
                ILottery(_lottery).transferNotify(userInfo[to].father, amount);
            }
        ...
        //register refer at the 1st token transfer
        if (!from.isContract() && !to.isContract() && balanceOf(to) == 0 && amount >= _numTokensToBeRefer){
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (takeFee) {
            ...
            feeRates = _getFeeRates(hasRefer);
        }
    ...
    function _transferToExcluded(address sender, address recipient, uint256 amount, uint256[ALLFEE_NUMBER] memory feeRates) private {
        ...
        //   {
        //         uint256 rLiquidity = rValues[REFER_INDEX];
        //         uint256 tLiquidity = tValues[REFER_INDEX];
