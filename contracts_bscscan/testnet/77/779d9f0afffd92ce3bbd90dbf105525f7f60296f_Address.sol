...
contract PumpkinPie is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 210000 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "PumpkinPie Token";
    ...
    uint256 public _taxFeeContract = 10;
    uint256 public _liquidityFeeContract = 2;
    
    uint256 public _liquidityFee = 2;
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public weeklyRewardRate = 10;
    uint256 public burnRate = 10;
    uint256 public playerRewardRate = 10;
	uint256 public fomoRate = 60;		//40% + 20%
    ...
    uint256 public maxRewardNum = 21;
    address public weeklyRewardWallet = 0x9999999999999999999999999999999999999999;
    address public burnWallet = 0x000000000000000000000000000000000000dEaD;
	address public USDT = 0xD13fc0C6A1c32b53b2303C1c0fB73881DB792A67;
	address public router = 0x65B879d01AbEf98Ae90F2D8261C2f355138E61f1;
    ...
    uint256 public fomoInterval = 1200;	//20min
    uint256 public _minFoMoBalance = 100000 * 10**9; 
    uint256 public numTokensSellToFoMo = 5250000 * 10**9;		//half of add liquidity
    ...
    uint256 public _maxTxAmount = 2100 * 10**6 * 10**9;			//1% max
    uint256 public numTokensSellToAddToLiquidity = 10500000 * 10**9;
    uint256 public minReferAmount = 1000 * 10**9;
    ...
    function setFomoAdmin(address _fomoAdmin) public onlyOwner {
        ...
        _isExcludedTopPlayer[fomoAdmin] = true;
        _isExcludedFromFee[fomoAdmin] = true;
    }
    ...
    function isTopPlayers(address addr) view public returns(bool) {
    	for(uint256 i = 0; i < topPlayers.length; i++){
    		if (addr == topPlayers[i]){
    			return true;
    		}
    	}
    	return false;
    ...
    function getTipTopPlayer() view public returns(address, uint256) {
    	...
    	uint256 amount = topPlayersAmounts[addr];
    ...
    function getPlayerBalance(address addr) view public returns(uint256) {
    	if(_plyrBalance[addr].lastUpdateTime < prevWeeklyRewardTime){
    		return 0;
    	}
    	uint256 amount = _plyrBalance[addr].totalAmountIn > _plyrBalance[addr].totalAmountOut ? _plyrBalance[addr].totalAmountIn.sub(_plyrBalance[addr].totalAmountOut) : 0;
    ...
    function setTaxFeePercent(uint256 taxFee, uint256 taxFeeContract) external onlyOwner() {
        _taxFee = taxFee;
         _taxFeeContract = taxFeeContract;
    }
    ...
    function setLiquidityFeePercent(uint256 liquidityFee,uint256 liquidityFeeConract) external onlyOwner() {
    ...
    function setNumTokensSellToAddToLiquidity(uint256 num0, uint256 num1) external onlyOwner {
        numTokensSellToAddToLiquidity = num0;
        numTokensSellToFoMo = num1;
    }
    ...
    function setUniswapV2Router(address _router) public onlyOwner {
    	router = _router;
    ...
    function _checkPlayerBalanceLimit(address addr, uint256 amount) view private returns(bool) {
    	uint256 balance = balanceOf(addr);
        if (balance < amount || balance.sub(amount) < _tTotal.div(1000000).mul(playerHoldingRatio)){
            return false;
        }
        return true;
    }
    ...
    function _playerReward(address recipient, uint256 rPlayer, uint256 tPlayer) private returns(uint256, uint256){   
        ...
        for (uint i = 0; i < maxPlayerLevel; i++){
            if (_checkPlayerBalanceLimit(addr, 0) == false){
                continue;
            }
    ...
    function _transfer(
		...
		require(from != burnWallet, "Invalid from");

		uint256 fomoAdminBalance = balanceOf(fomoAdmin);
        ...
        }else if (fomoAdminBalance >= numTokensSellToFoMo &&
        	!inSwapAndLiquify && 
        	from != uniswapV2Pair &&
        	from != fomoAdmin
        	){
	...
	function _awardsFoMo(uint256 amount,address _user) private {
		if (amount >= _minFoMoBalance ){
			if (block.timestamp > openTimeFoMo){
				...
				IFomo(fomoAdmin).transferNotify(_lastBuyAddress);
			}
	...
	function swapFomoReward() private lockTheSwap {
		IFomo(fomoAdmin).swap();
	}
    ...
    function _bindRefer(address sender, address recipient, uint256 amount) private {
        ...
        if(_checkPlayerBalanceLimit(sender, amount) == false){
        	return;
        }
	...
	function _rewardTopPlayers(address sender, address recipient) private{
        ...
        if (lastWeeklyRewardTime <= now && weeklyRewardEnabled == true){
        	...
        	_sendTopPlayerReward();
      	}		
	}
	
	function _updateTopPlayers(address sender, address recipient, uint256 rAmountOut, uint256 rAmountIn) private{
        if(_isExcludedTopPlayer[sender] || _isExcludedTopPlayer[recipient]){
            return;
        }
		...
		if(sender == uniswapV2Pair){
			...
			playerAddr = _plyr[recipient];
			playerAmount = rAmountIn.div(currentRate);
			swapIn = true;
		}else if (recipient == uniswapV2Pair){
			if (_plyr[sender] == address(0)){
				return;
			}
			playerAddr = _plyr[sender];
			playerAmount = rAmountOut.div(currentRate);
		}else{
			return;
		}
		
		if(_plyrBalance[playerAddr].lastUpdateTime < prevWeeklyRewardTime){
			//reset all in new period
			_plyrBalance[playerAddr].totalAmountIn = 0;
			_plyrBalance[playerAddr].totalAmountOut = 0;
			topPlayersAmounts[playerAddr] = 0; 				 
		}
		
		//update
		if (swapIn == true){
			_plyrBalance[playerAddr].totalAmountIn += playerAmount;	
		}else{
			_plyrBalance[playerAddr].totalAmountOut += playerAmount;
		}			
		_plyrBalance[playerAddr].lastUpdateTime = now;
		
		//rank
        uint256 amount = _plyrBalance[playerAddr].totalAmountIn > _plyrBalance[playerAddr].totalAmountOut ? _plyrBalance[playerAddr].totalAmountIn.sub(_plyrBalance[playerAddr].totalAmountOut) : 0;
        _rankTopPlayers(playerAddr, amount);
        
        _plyrInfo[playerAddr].amount = amount;		
	}
    ...
    function _rankTopPlayers(address addr, uint256 amount) private {
        if (topPlayersAmounts[addr] > 0) {
            ...
            if (topPlayers.length < maxRewardNum){
                ...
                //Reset old tip's amount
                topPlayersAmounts[topPlayers[topPlayerTip]] = 0;
    ...
    function _removeTopPlayer(address addr) private {
    	for (uint256 i = 0; i < topPlayers.length; i++){
    		if(topPlayers[i] == addr){
				topPlayers[i] = topPlayers[topPlayers.length - 1];
				topPlayers.pop();
				topPlayersAmounts[addr] = 0;
				return;
    		}	
    	}
    ...
    function _sendTopPlayerReward() private {        
    ...
    function refreshTopPlayer() public {		
    	address addr = msg.sender;
    	uint256 amount = getPlayerBalance(addr);
		if(amount > 0 && topPlayersAmounts[addr] == 0){
			_rankTopPlayers(addr, amount);
		}
    }    
}