...
contract SaleProvider is Ownable {
    ...
    /**
        ...
        Ví dụ : như ví dụ trên _numTokenSale = 1*10^18 và _numTokenAccept = 0.2*10^6
        muốn lấy phí 10% => _numFee = 0.2*10^6 * 10% = 0.2*10^5
    ...
    */
    ...
    constructor(IERC20Metadata _tokenSale,
        ...
        fee = (_numFee*10**tokenAccept.decimals())/_numTokenAccept;
    ...
    function buyTokens(uint256 numberOfTokens) public {
        ...
        require(amount != 0);
        uint256 feePayable = (amount*fee)/(10**tokenAccept.decimals());
    ...
    function updateFee(uint256 _numTokenAccept, uint256 _numFee) public onlyOwner() {
        require(_numFee > 0);
        require(_numTokenAccept > 0);
