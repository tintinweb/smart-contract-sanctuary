...
contract AlphaMarket is Ownable, Lockable {
    ...
    onlyAlphalist(msg.sender) {
        ...
        uint256 _prinAmount = calculator.principleAmount(_amount, marketPrice, aGIZA.decimals());
    ...
    function openRedeem() external onlyOwner() {
        require(block.number > exchangeEndBlock, "exchanging is not ended");
    ...
    onlyInitialized() 
    onlyAlphalist(msg.sender) {
        require(block.number > exchangeEndBlock, "exchanging has not ended");
        require(canRedeem, "redeem not open");

        uint256 _amount = senderInfo[msg.sender];
        aGIZA.safeTransfer(msg.sender, _amount);
    ...
    function reclaim() external onlyOwner() {
        require(block.number > exchangeEndBlock, "exchanging has not ended");
        require(!canRedeem, "redeem is opening");
        require(address(treasury)!=address(0), "treasury address is zero");
        
        uint256 agizaAmount = aGIZA.balanceOf(address(this));

        aGIZA.safeTransfer(address(treasury), agizaAmount);
