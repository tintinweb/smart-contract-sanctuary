...
contract BNBDistributor is IDividendDistributor {
    ...
    uint256 public minSwapBalance = 30 * 10 ** 9;
    ...
    function approveRewardSwap () external onlyToken {
...
contract AnjiToken is Context, IBEP20, Ownable {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (shouldTakeFee(sender, recipient)) {
            ...
            _balances[address(this)] = _balances[address(this)] + _bnbFee;
            distributorAmount = distributorAmount + _bnbFee;
    ...
    function swapBack() internal swapping {
        ...
        uint256 reward_amount = distributorAmount;

        uint256 amount = _balances[address(this)];
        uint256 amountToLiquify = amount.sub(reward_amount).div(2);
        uint256 amountToSwap = amountToLiquify.add(reward_amount);
        ...
        uint256 swapAmountBNB = address(this).balance.sub(balanceBefore);

        uint256 totalSwapFee = bnbReflectionFee.add(liquidityFee);

        uint256 totalBNBLiquidity = swapAmountBNB.div(100).mul(liquidityFee.div(totalSwapFee.div(100).mul(2).mul(2)));
        uint256 amountBNBLiquidity = totalBNBLiquidity.div(2);

        uint256 amountBNBRewards = swapAmountBNB.div(100).mul(bnbReflectionFee.div(totalSwapFee.div(100).mul(2).mul(2)));
        ...
        if (amountBNBRewards > 0){
            _approve(address(this), WBNB, amountBNBRewards);
            IBEP20(WBNB).transferFrom(address(this), address(bnbDistributor), amountBNBRewards);
            bnbDistributor.deposit(amountBNBRewards);
    ...
    function rewardSwap() public {
        bnbDistributor.deposited();
    }

    function rewardApprove() public {
        bnbDistributor.approveRewardSwap();
    }
