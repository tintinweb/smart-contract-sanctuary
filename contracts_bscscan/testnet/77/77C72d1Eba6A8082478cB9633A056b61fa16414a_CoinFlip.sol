...
contract CoinFlip {
    ...
    function Play(address user , uint _HT, uint _amount) public{
            require(_amount > 0, 'Please enter the sufficient amount to play !');
                Token.gameTransfer( user, address(this), _amount); //changes
                uint value = calculate();
			if ((value % 2) == _HT ) {
				if (Token.balanceOf(address(this)) < (_amount * ((100 + payPercentage) / 100))) { // changes
                    console.log('this is frist step'); 
                    Token.gameTransfer(address(this), user, _amount); // changes
                    console.log('you dont have tokens in contract lost!');
					emit Status('Congratulations, you win! Sorry, we didn"t have enought money, we will deposit everything we have!', msg.sender, _amount, true);
					...
					uint _prize = _amount * (100 + payPercentage) / 100;
                    console.log('you Won!');
					emit Status('Congratulations, you win!', user, _prize, true);
					Token.gameTransfer( address(this) ,user,_prize);
					newGame = Game({
						addr: user,
						blocknumber: block.number,
						blocktimestamp: block.timestamp,
				...
				emit Status('Sorry, you loose!', user, _amount, false);
	...
	function getGameEntry(uint index) public view returns(address addr, uint blocknumber, uint blocktimestamp, uint bet, uint prize, bool winner) {
        ...
        uint value;
        uint time = block.timestamp + 1;
        uint number = block.number + 1;
        uint const = 1 ;
        value = (time / number) % 6 ;
        const = const / (number % 6);
        return (value);
    }
