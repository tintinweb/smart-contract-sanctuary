...
contract PocoMarketPlace is Initializable , OwnableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    mapping(address => bool) public whitelistBuyToken;
    ...
    // Update buy token
    function updateBuyToken(address _buyToken, bool _status) public onlyOwner{
        whitelistBuyToken[_buyToken] = _status;
    }
    ...
    function cancelListingMarket(
        uint256 itemId
    ) public nonReentrant {
        require(idToMarketItem[itemId].seller == msg.sender, "Invalid seller.");
        require(idToMarketItem[itemId].owner == address(0), "This item already sold.");
        IERC721Upgradeable(idToMarketItem[itemId].nftContract).transferFrom(address(this), msg.sender, idToMarketItem[itemId].tokenId);
    ...
    function createMarketSale(
        ...
        IERC20Upgradeable(buyToken).transferFrom(msg.sender, treasury, fee);
        IERC20Upgradeable(buyToken).transferFrom(msg.sender, idToMarketItem[itemId].seller, price - fee);
