...
// import './interface/ISmartDEXChainFactory.sol';
...
contract TOKordinatorBSC is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    using PancakeLibrary for IPancakePair;
    // using SmartDEXChainLibrary for ISmartDEXChainPair;
    ...
    IPancakeFactory internal pancakeswap;
    // IPancakeFactory(0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f);
    // ISmartDEXChainFactory internal smartdex;
    // ISmartDEXChainFactory(0x87a58Cc6fE4A6d1290ff6DCd7a152F2a8777E433);
    IHelioswapFactory internal helioswap;
    // IHelioswapFactory(0x0d45f1F4987c939A58E3a3761CBfE72c441F6731);
    ...
    constructor(
        ...
        weth = IWETH(_weth);
        pancakeswap = IPancakeFactory(_pancakeswap);
        // smartdex = ISmartDEXChainFactory(_smartdex);
    ...
    function getPancakeswapAmountsOut(uint256 amountIn, IERC20[] memory path) public view returns (uint256[] memory) {
        IERC20[] memory realPath = formatPath(path);
        return PancakeLibrary.getAmountsOut(pancakeswap, amountIn, realPath);
    }

    // function getSmartDEXChainAmountsOut(uint256 amountIn, IERC20[] memory path) public view returns (uint256[] memory) {
    //     IERC20[] memory realPath = formatPath(path);
    //     return SmartDEXChainLibrary.getAmountsOut(smartdex, amountIn, realPath);
    // }
    ...
    function _swapOnPancakeswap(IERC20[] calldata path, uint256 amount) internal {
        IERC20[] memory realPath = formatPath(path);

        IPancakePair pair = pancakeswap.getPair(realPath[0], realPath[1]);
        uint256[] memory amounts = PancakeLibrary.getAmountsOut(pancakeswap, amount, realPath);
        ...
        for (uint256 i; i < realPath.length - 1; i++) {
            ...
            address to = i < realPath.length - 2
                ? address(pancakeswap.getPair(IERC20(output), realPath[i + 2]))
                : address(this);
            pancakeswap.getPair(IERC20(input), IERC20(output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    ...
    // function _swapOnSmartDEXChain(IERC20[] calldata path, uint256 amount) internal {
    //     IERC20[] memory realPath = formatPath(path);

    //     ISmartDEXChainPair pair = smartdex.getPair(realPath[0], realPath[1]);
    //     uint256[] memory amounts = SmartDEXChainLibrary.getAmountsOut(smartdex, amount, realPath);
    ...
    //             : (amountOut, uint256(0));
    //         address to = i < realPath.length - 2
    //             ? address(smartdex.getPair(IERC20(output), realPath[i + 2]))
    //             : address(this);
    //         smartdex.getPair(IERC20(input), IERC20(output)).swap(amount0Out, amount1Out, to, new bytes(0));
    //     }
...
library PancakeLibrary {
    ...
    function getAmountsOut(IPancakeFactory factory, uint amountIn, IERC20[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'TOKordinator: invalid path');
        amounts = new uint[](path.length);
        amounts[0] = amountIn;
        for (uint i; i < path.length - 1; i++) {
            IPancakePair pair = factory.getPair(path[i], path[i + 1]);
