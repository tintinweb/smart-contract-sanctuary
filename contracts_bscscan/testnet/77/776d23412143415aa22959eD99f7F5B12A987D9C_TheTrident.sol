...
contract TheTrident is Ownable, Pausable {
    ...
    uint256 internal constant devRate = 5; // adjusted by FEES_DIVISOR (0.5%)
    ...
    function _calculateRewards(uint256 epoch) internal {
        ...
        uint256 treasuryAmt;
        uint256 devAmount;
        uint256 zeroPointFive = devRate.div(10);
        uint256 _rewardRate = rewardRate.sub(zeroPointFive);
        ...
        if (round.closePrice > round.lockPrice) {
            ...
            devAmount = round.totalAmount.mul(devRate).div(FEES_DIVISOR);
        }
        // Bear wins
