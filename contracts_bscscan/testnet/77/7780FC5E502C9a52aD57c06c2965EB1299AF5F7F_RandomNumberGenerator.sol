...
contract RandomNumberGenerator is VRFConsumerBase, Ownable, IRandomNumberGenerator {
  ...
  /// @inheritdoc IRandomNumberGenerator
  function requestRandomNumber() external returns (uint16 _requestId) {
    if (!canRequestRandomNumber[msg.sender]) revert CallerCannotRequestNumber();
    if (_callbacks.length == 0) {
      /* 
        If there are no callbacks stored, then this is the first request. Therefore, we forward the request to Chainlink. 
        If there are other callbacks, then a request to Chainlink has already been made and we will wait for it. 
        The plan is to make only one call to Chainlink, and use the response as a seed to generate multiple random numbers.
        We are doing this as a way to try to spend less LINK tokens
      */
      _requestRandomness();
      emit RequestForwarded();
    }
    _callbacks.push(msg.sender);
    _requestId = _callbacks.length.toUint16(); // Use the amount of callbacks as the request id
  }
  ...
  function fulfillRandomness(bytes32, uint256 _randomness) internal override {
    address[] memory __callbacks = _callbacks;
    for (uint16 i; i < __callbacks.length; i++) {
      // We are using this method suggested by Chainlink to generate multiple random numbers from a random seed
      uint256 _randomNumber = uint256(keccak256(abi.encode(_randomness, i)));
      IRandomNumberConsumer(__callbacks[i]).consumeRandomNumber(i + 1, _randomNumber);
    }
    emit RandomnessFulfilled(__callbacks);
    delete _callbacks;
  }
}
