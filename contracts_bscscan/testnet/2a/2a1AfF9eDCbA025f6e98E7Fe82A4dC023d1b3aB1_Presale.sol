...
contract Presale is Ownable, ReentrancyGuard {
    ...
    function contributePublicSale(uint256 _saleId) external payable {
    ...
    function contributeTier1(uint256 _saleId) external payable {
        require(presaleTokens[_saleId] != address(0), "presale-not-exist");
        require(isDeposited[_saleId], "token-not-deposited-yet");
        require(
            block.timestamp >= startTime[_saleId] &&
                block.timestamp <= tier1Time[_saleId],
            "privatesale-not-active"
        );
        require(whitelist1[_saleId][msg.sender], "not-whitelist1");
        address _token = presaleTokens[_saleId];
        IERC20 token = IERC20(_token);
        uint256 decimals = token.decimals();
        userContributionBNB[_saleId][msg.sender] += msg.value;
    ...
    function contributeTier2(uint256 _saleId) external payable {
        require(presaleTokens[_saleId] != address(0), "presale-not-exist");
        require(isDeposited[_saleId], "token-not-deposited-yet");
        require(
            block.timestamp >= tier1Time[_saleId] &&
        ...
        require(whitelist2[_saleId][msg.sender], "not-whitelist2");
        address _token = presaleTokens[_saleId];
        IERC20 token = IERC20(_token);
        uint256 decimals = token.decimals();
        userContributionBNB[_saleId][msg.sender] += msg.value;
    ...
    function outsideContribute(uint256 _saleId) external payable {
        require(presaleTokens[_saleId] != address(0), "presale-not-exist");
        require(isDeposited[_saleId], "token-not-deposited-yet");
        require(
            block.timestamp >= startTime[_saleId] &&
        ...
        require(
            iswhitelist1[_saleId] && basewhitelist[msg.sender],
            "not-original-whitelist"
        );
        uint256 _length = outsideContribution[msg.sender].length;
        require(
            _length <= outsideContributionCount ||
                block.timestamp.sub(
