...
contract CompetitionPool is Ownable, ReentrancyGuard {
    ...
    function setRewardRates(
        ...
        require(
            ...
            "CP: Arrays must be of equal length"
        );
        require(
            _bracketsAmountToken[0] == 0,
            "CP: _bracketsAmountToken[0] must be Zero"
        );
        for (uint256 i = 0; i < _bracketsAmountToken.length - 1; i++) {
            require(
                _bracketsAmountToken[i] < _bracketsAmountToken[i + 1],
                "CP: _bracketsAmountToken must be sorted ascending"
            );
            require(
                _rewardRateOfcreator[i] + _rewardRateOfOwner[i] <=
                    10**(decimalOfRate + 2),
                "CP: rewardRateOfcreator + rewardRateOfOwner must be <=100%"
            );
        }
    ...
    function checkRefund(address _betting) external view returns (bool) {
        ...
        ) = ICompetitionContract(_betting).getDataToCheckRefund();
        if (
            block.timestamp >
