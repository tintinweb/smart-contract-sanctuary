 ...
 contract dividendMaster is Ownable, ReentrancyGuard{
    ...
    */
    ...
    function removeShares(uint256 _amount) external nonReentrant  {
        UserInfo storage user = userInfo[msg.sender];
        // adjust shares in case remaeket transfer
        if(user.shares == 0 || user.shares<xdvdToken.balanceOf(msg.sender))
        {
            user.shares = xdvdToken.balanceOf(msg.sender);
        }
        require(_amount > 0 , "DVDMaster::removeShares: Amount should more than 0");
        require(user.shares >= _amount, "DVDMaster::removeShare: Insufficient balance");
        require(depositedBalance()>=_amount, "DVDMaster::removeShare: Insufficient deposited token remain");
