...
contract Chrissi is Ownable, IBEP20, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    function setExcludeWalMultiple(address[] calldata _users, bool exempt) external onlyOwner {
        for(uint8 i = 0; i < _users.length; i++) {
            excludeMaxWal[_users[i]] = exempt;
        }
    }
