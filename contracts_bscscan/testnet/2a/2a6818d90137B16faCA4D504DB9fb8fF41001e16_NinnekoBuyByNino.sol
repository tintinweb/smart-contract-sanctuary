...
contract NinnekoBuyByNino is Ownable, Pausable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    NinnekoBox public ninnekoBox;
    IERC20 public NINOToken;

    uint256 public priceNinoPerBox;
    uint256 public maxBoxBuyByNinoPerUser;
    address public addressNinoReceiver;
    uint256 public maxBoxSale;
    uint256 public totalBoxSold;

    uint256[] public typeBoxs;

    mapping(address => uint256) public addressBoughtBoxNino; //(user => number box bought)
    ...
    constructor() {
        priceNinoPerBox = 999 * 10**18;
        maxBoxBuyByNinoPerUser = 6;
        addressNinoReceiver = owner();
        typeBoxs = [uint256(3)];
        maxBoxSale = 1000;
        ninnekoBox = NinnekoBox(0x5df4d874284DEcFc029a5426B147546D89d4Ef76);
        NINOToken = IERC20(0x6CAD12b3618a3C7ef1FEb6C91FdC3251f58c2a90);
    }
    ...
    function setPriceNinoPerBox(uint256 _price) external onlyOwner {
        priceNinoPerBox = _price;
    }

    function setMaxBoxBuyByNinoPerUser(uint256 _maxBuy) external onlyOwner {
    ...
    function setMaxBoxSell(uint256 _amount) external onlyOwner {
        maxBoxSale = _amount;
    }
    ...
    function buyNFTBox(uint256 _amount) external whenNotPaused nonReentrant {
        require(priceNinoPerBox > 0, "price invalid");
        require(totalBoxSold + _amount <= maxBoxSale, "Amount invalid");
        require(addressBoughtBoxNino[msg.sender] + _amount <= maxBoxBuyByNinoPerUser, "you can buy maximum 6 boxes per wallet");
        uint256 allowanceNino = NINOToken.allowance(msg.sender, address(this));
        require(allowanceNino >= priceNinoPerBox * _amount, "Token payment allowance is insufficient");

        for (uint256 i = 0; i < _amount; i++) {
            _mintBoxToAddress(msg.sender);
        }
        totalBoxSold = totalBoxSold + _amount;
        addressBoughtBoxNino[msg.sender] = addressBoughtBoxNino[msg.sender] + _amount;
        NINOToken.safeTransferFrom(msg.sender, addressNinoReceiver, priceNinoPerBox * _amount);
        emit BuyBox(msg.sender, _amount);
    }

    function setAddressNino(address _addNINO) external onlyOwner {
        NINOToken = IERC20(_addNINO);
    }

    function setNinnekoBoxAddress(address _add) external onlyOwner {
        ninnekoBox = NinnekoBox(_add);
    }

    function changeNinnekoBoxOwner(address _newOwner) external onlyOwner {
        ninnekoBox.transferOwnership(_newOwner);
    }

    function _mintBoxToAddress(address _to) private {
        ninnekoBox._mintBatchToAddress(_to, 2, typeBoxs);
    }

    function bathMintBox(address[] memory _listAdd) external onlyOwner {
        for (uint256 i = 0; i < _listAdd.length; i++) {
            ninnekoBox._mintBatchToAddress(_listAdd[i], 2, typeBoxs);
        }
    }
    ...
    function batchMintToAddress(
        ...
        ninnekoBox._mintBatchToAddress(_to, _typeBuy, _listTypeBoxs);
    }
