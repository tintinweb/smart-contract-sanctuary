...
contract Treasury is Ownable {
    ...
    address public KISS;
    uint256 public KISSQueue; // Delays change to KISS address
    ...
    constructor(
        ...
        isLiquidityToken[_LOVEDAI] = true;
        liquidityTokens.push(_LOVEDAI);
        bondCalculator[_LOVEDAI] = _bondCalculator;
    ...
    /**
        @notice allow approved address to deposit an asset for LOVE
        @param _amount uint
    ...
    function deposit(
        ...
        // mint LOVE needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IERC20Mintable(LOVE).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn LOVE for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf(_token, _amount);
        ILOVEERC20(LOVE).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(KISS).balanceOf(msg.sender); // Can only borrow against KISS held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with LOVE
        @param _amount uint
     */
    function repayDebtWithLOVE(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");

        ILOVEERC20(LOVE).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, LOVE, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(LOVE).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(LOVE).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns LOVE valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf(address _token, uint256 _amount) public view returns (uint256 value_) {
        if (isReserveToken[_token]) {
            // convert amount to match LOVE decimals
            value_ = _amount.mul(10**IERC20(LOVE).decimals()).div(10**IERC20(_token).decimals());
    ...
    function queue(MANAGING _managing, address _address) external onlyManager returns (bool) {
        ...
        } else if (_managing == MANAGING.KISS) {
            // 9
            KISSQueue = block.number.add(blocksNeededForQueue);
        } else return false;
