...
contract PokeBallNFT is ERC721Enumerable, Ownable, ERC721Pausable, ERC721Burnable  {
    ...
    event Migration(
        ...
        address owner,
        string hash_value,
        address indexed ntf_creator,
        uint256 exp,
        uint256 createdAt
    );
    event EXP(uint256 indexed tokenId, address owner, uint256 exp);
    event Spawn(uint256 indexed tokenId, address to, string hash_val, address indexed nft_creator);
    ...
    mapping(uint256 => Ball) internal balls;
    ...
    function spawn(address to, string memory hash_value, uint256 _price) public onlySpawner {
        require(_price >= MIN_PRICE, "Price belows under MIN_PRICE");

        uint256 nextTokenId = _getNextTokenId();
        _mint(to, nextTokenId);

        balls[nextTokenId] = Ball({
        ...
        emit Spawn(nextTokenId, to, hash_value, msg.sender);
    }
    ...
    function upExp(
        ...
        Ball storage ball = balls[_tokenId];
        ball.exp = ball.exp.add(_exp);
    ...
    function migrate(
        ...
        emit Migration(nextTokenId, to, hash_value, msg.sender, _exp, _createdAt);
    }

    function multiSpawn(address to, uint256 amount, string[] memory hash_list, uint256[] memory _prices) public onlySpawner {
        require(amount > 1, "require: multiple");
        for (uint256 index = 0; index < amount; index++) {
            spawn(to, hash_list[index], _prices[index]);
        }
    }
    ...
    function getBall(uint256 _tokenId) public view returns (Ball memory) {
        return balls[_tokenId];
    }
    ...
    function buyToken(uint256 _tokenId, uint256 _amount) public payable saleIsOpen {
        require(balls[_tokenId].price == _amount, "Price is not correct");
        BALL.transferFrom(msg.sender, address(this), _amount);


    }

    function _isAdmin (address spender) internal view returns (bool) {
        return spawners[spender] || migrator == spender || owner() == spender;
    }
    ...
    function transferFrom(
        ...
        require(_isAdmin(_msgSender()) || _isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
