...
contract Shop is Ownable{
    ...
    mapping (address => address[]) depositedNFT;
    ...
    event NFTsold (address _from, address to, address nft, uint256 NFT_ID, uint256 price);
    ...
    function startSale (uint256 _startPrice, uint256 _idFromNft, address _nft) public payable{
        ...
        require (erc721NFT.ownerOf(_idFromNft) == address(this), "you did not deposit your nft yet");
        saleId += 1;
    ...
    function depositNFT (uint256 _idFromNft, address _nft) public {

        ERC721 erc721NFT = ERC721(_nft);
        erc721NFT.approve(address(this), _idFromNft);
        erc721NFT.transferFrom(msg.sender, address(this), _idFromNft);

        if (erc721NFT.ownerOf(_idFromNft) == address(this))
