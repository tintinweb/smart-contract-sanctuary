...
contract GDogZillaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("GDogZillaDividendTracker", "GDogZilla_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "GDogZillaDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "GDogZillaDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main GDogZilla contract.");
    }
...
contract GDogZilla is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GDogZillaDividendTracker public dividendTracker;
    address public marketingWallet = 0x46E8DF31C0e2d98B047E97013A395d868A1dcCc5;
    address public buybackWallet = 0x89E4B6127e8c98C1E1cC62234F2F33505904a50f;
    ...
    constructor() ERC20("GDogZilla Token", "GDogZilla") {
        dividendTracker = new GDogZillaDividendTracker();
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "GDogZilla: The router already has that address");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "GDogZilla: The dividend tracker already has that address");

        GDogZillaDividendTracker newDividendTracker = GDogZillaDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "GDogZilla: The new dividend tracker must be owned by the GDogZilla token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GDogZilla: Account is already the value of 'excluded'");
    ...
    function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPeriodLimit[account] != excluded, "GDogZilla: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "GDogZilla: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GDogZilla: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GDogZilla: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "GDogZilla: This account cannot send tokens until trading is enabled");
        }
