...
contract BaseDeployer is Initializable, AccessControl {
    ...
    uint16[] private typeProbabilities = [1, 5, 10, 30, 54];
    uint8[] private buildTypes = [1, 2, 3, 4, 5];
    uint8[] private buildModels = [1, 2, 3, 4, 5];
    ...
    function getProbabilitiesTotal() public view returns (uint16) {
        uint16 adds = 0;
        for (uint8 i = 0; i < typeProbabilities.length; i++) {
            adds += typeProbabilities[i];
        }
        return adds;
    }
    ...
    function randomBuildType() public returns (uint8) {
        ...
        for (uint256 i = 0; i < typeProbabilities.length; i++) {
            if (
                momentAdd < random &&
                random <= (momentAdd + typeProbabilities[i])
            ) return buildTypes[i];
            momentAdd += typeProbabilities[i];
        }
