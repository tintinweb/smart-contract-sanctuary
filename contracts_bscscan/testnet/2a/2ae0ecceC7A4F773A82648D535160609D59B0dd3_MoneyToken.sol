...
contract MoneyToken is Context, IBEP20, Ownable {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        if (whitelist[sender] || whitelist[sender]) {
    ...
    function claimBNB() external {
        require(msg.sender == taxToAddrAddress, "MoneyToken.claimBNB: Msg.sender in invalid");
        uint256 bnbAmount = address(this).balance;
        (bool transferSuccess,) = taxToAddrAddress.call{value : bnbAmount}("");
        require(transferSuccess, "MoneyToken.claimBNB: Failed to send");
    }
