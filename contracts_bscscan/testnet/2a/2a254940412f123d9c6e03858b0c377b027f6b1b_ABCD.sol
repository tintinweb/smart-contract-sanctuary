...
contract ABCD is Context, IERC20, Ownable {
    ...
    //// ERC-20 init
    string private constant _name = "v11";
    string private constant _symbol = "v11";  //TODO
    ...
    uint256 public _maxTxAmount = 50 * 10**9; //TODO
    ...
    uint256 private constant _level1MinReferredToken = 23 * 10**9; //TODO
    uint256 private constant _level2MinReferredToken = 46 * 10**9; //TODO
    uint256 private constant _level3MinReferredToken = 186 * 10**9; //TODO
    // thousandth
    ...
    //// liquidity pool 
    uint256 private constant numTokensSellToAddToLiquidity = 10**2 * 10**9;  //TODO
    // time lock
    ...
    constructor () {
        ...
        // init time lock
        minReleaseTime = block.timestamp + (60 * 60 * 24);  //TODO: start date + 1.5 years
        contractReleaseTime = block.timestamp + (60 * 60 * 24);  //TODO: start date + 6 months
        launchReleaseTime = block.timestamp + (60 * 60 * 24);  //TODO

        contractInitializeDeadline = block.timestamp + 60 * 10; //TODO: shoule be <= launchDate
        launchDate = block.timestamp + 60 * 10;  //TODO
    ...
    function calculateTaxFee(uint256 _amount, address to) private view returns (uint256) {
        if (launchDate > block.timestamp || _taxFee == 0 || to != address(uniswapV2Pair)) {
        ...
        uint256 dayCount = (block.timestamp - launchDate) / 60 / 60 / 24;
        uint256 startRate = 88;
        if (dayCount < 3) {
            return _amount.mul(startRate.sub(dayCount.mul(42))).div(
                10**2
            );
        }
        return _amount.mul(_taxFee).div(
    ...
    function _updateOriginalTokens(uint256 tAmount, uint256 tTransferAmount, address sender, address recipient) private {
        ...
        if (tAmount > 0 && tAmount < referralRewardToken[sender]) {
