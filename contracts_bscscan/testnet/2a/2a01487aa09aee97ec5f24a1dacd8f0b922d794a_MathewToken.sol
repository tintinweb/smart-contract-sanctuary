...
contract MathewToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public restrictionPeriod = 1 hours;
    struct Sell {
        uint256 time;
        uint256 amount;
    }
    ...
    event UpdateWallets(address oldMarketing, address newMarketing, address oldfounderFee, address newfounderFee);
    ...
    event SetBuyFees(uint256 founderFeeFee, uint256 MarketingFee, uint256 DividendFee);

    event SetSellFees(uint256 sellfounderFee, uint256 SellMarketingFee, uint256 SellDividendFee);
    ...
    event CalculatedBNBForEachRecipient(uint256 forMarketing, uint256 forfounderFee, uint256 forDividends);
    ...
    constructor() ERC20("MathewToken", "Mathew") {
        dividendTracker = new MathewTokenDividendTracker();
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "MathewToken: The router already has that address");
    ...
    function setBuyFees(uint256 _founderFeeFee, uint256 _MarketingFee, uint256 _DividendFee)  external onlyOwner {
        founderFee = _founderFeeFee;
        marketingFee = _MarketingFee;
        dividendFee = _DividendFee;
        uint256 totalFees = founderFee + marketingFee + dividendFee;
        require(totalFees < 51, "Too much");
        emit SetBuyFees(_founderFeeFee, _MarketingFee, _DividendFee);
    }

    function setSellFees(uint256 _sellfounderFee, uint256 _SellMarketingFee, uint256 _SellDividendFee)  external onlyOwner {
        sellfounderFee = _sellfounderFee;
        sellMarketingFee = _SellMarketingFee;
        sellDividendFee = _SellDividendFee;
        uint256 totalFees = sellfounderFee + sellMarketingFee + sellDividendFee;
        require(totalFees < 51, "Too much");
        emit SetSellFees(_sellfounderFee, _SellMarketingFee, _SellDividendFee);
    }

    function setWallets(address newMarketingWallet, address newfoundersWallet) external onlyOwner {
        require(newfoundersWallet != address(0), 'founder Zero address');
        require(newMarketingWallet != address(0), 'Marketing Zero address');
        emit UpdateWallets(marketingWallet, newMarketingWallet, foundersWallet, newfoundersWallet);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MathewToken: The dividend tracker already has that address");

        MathewTokenDividendTracker newDividendTracker = MathewTokenDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MathewToken: The new dividend tracker must be owned by the MathewToken token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MathewToken: Account is already the value of 'excluded'");
    ...
    function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPeriodLimit[account] != excluded, "MathewToken: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "MathewToken: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MathewToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MathewToken: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "MathewToken: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndDistributeBNB(uint256 tokens) private {
        ...
        uint256 accTotal = accfounderFee.add(accDividendFee).add(accMarketingFee);
        uint256 forMarketing = balance.mul(accMarketingFee).div(accTotal);
        uint256 forfounderFee = balance.mul(accfounderFee).div(accTotal);
        uint256 forDividends = balance.sub(forfounderFee).sub(forMarketing);

        emit CalculatedBNBForEachRecipient(forMarketing, forfounderFee, forDividends);
        ...
        (success,) = address(foundersWallet).call{value: forfounderFee}("");

        if(success) {
            emit SwapAndSendTo(accfounderFee, forfounderFee, "ANTI DUMP");
            accfounderFee = 1;
        }
...
contract MathewTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MathewTokenDividendTracker", "Honey_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MathewTokenDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MathewTokenDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main Honey contract.");
    }
