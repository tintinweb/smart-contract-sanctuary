...
contract DemoGreel_PreSale is ReentrancyGuard, Context, Ownable {
    ...
    uint256 public _lastReceivedWei;
    ...
    event WalletTokensPurchased(address indexed purchaser, address indexed beneficiary, uint256 value, uint256 amount);
    event WeiAmountAre(uint256 weiAmount);
    event BeneficiaryIs(address beneficiary);
    event correspondingTokensAre(uint256 correspondingTokens);
    event TokensAre(uint256 tokens);
    event WeiRaisedAre(uint256 _weiRaised);
    event AvailableTokensICOBeforeAre(uint256 availableTokensICO);
    event AvailableTokensICOAfterAre(uint256 availableTokensICO);
    ...
    function buyTokens(address beneficiary) public nonReentrant icoActive payable {
        ...
        emit WeiAmountAre(weiAmount);
        emit BeneficiaryIs(beneficiary);
        _preValidatePurchase(beneficiary, weiAmount);

        _lastReceivedWei = weiAmount;
        correspondingTokens = _getTokenAmount(_lastReceivedWei);
        emit correspondingTokensAre(correspondingTokens);
        
        uint256 tokens = _getTokenAmount(weiAmount);
        emit TokensAre(tokens);

        _weiRaised = _weiRaised.add(weiAmount);
        emit WeiRaisedAre(_weiRaised);

        emit AvailableTokensICOBeforeAre(availableTokensICO);
        availableTokensICO = availableTokensICO.sub(tokens);
        emit AvailableTokensICOAfterAre(availableTokensICO);

        _processPurchase(beneficiary, tokens);
        emit TokensPurchased(_msgSender(), beneficiary, weiAmount, tokens);

        // _forwardFunds();
        // emit WalletTokensPurchased(_msgSender(), beneficiary, weiAmount, tokens);
    }
