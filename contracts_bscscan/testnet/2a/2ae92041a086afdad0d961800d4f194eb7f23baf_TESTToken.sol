...
contract TESTToken is ERC20, Ownable {
    ...
    address public team =       0xEeB9Ee700511F5C9e75c24dC3B4d4F0f0e7E8AA1;
    ...
    uint256 public algorithmFee = 5;
    uint256 public totalFees = BUSDRewardsFee.add(algorithmFee);

    address public _algoWalletAddress = 0x314D49C74e868987ECed32ec6918dBCadF33EFba;
    ...
    function setalgoWallet(address payable wallet) external onlyOwner{
        _algoWalletAddress = wallet;
    }

    function setTESTRewardsFee(uint256 value) external onlyOwner{
        BUSDRewardsFee = value;
        totalFees = BUSDRewardsFee.add(algorithmFee);
    }


    function setalgoFee(uint256 value) external onlyOwner{
        algorithmFee = value;
        totalFees = BUSDRewardsFee.add(algorithmFee);
    ...
    function _transfer(
        ...
        require(amount <= maxTxAmount , "Amount is out of limit");
        if(from == _algoWalletAddress)
        {
            require(block.timestamp >= (deploymentTime + 180 days) , "algo wallet is locked");
        }
        
        uint256 teamPercentage = amount.mul(2).div(100);
       super._transfer(from, to , teamPercentage);
        ...
        if( canSwap &&
            ...
            uint256 algoTokens = contractTokenBalance.mul(algorithmFee).div(totalFees);
            swapAndSendToFee(algoTokens);
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        uint256 initialTESTBalance = IERC20(BNB).balanceOf(address(this));

        swapTokensForBNB(tokens);
        uint256 newBalance = (IERC20(BNB).balanceOf(address(this))).sub(initialTESTBalance);
        IERC20(BNB).transfer(_algoWalletAddress, newBalance);
    }
