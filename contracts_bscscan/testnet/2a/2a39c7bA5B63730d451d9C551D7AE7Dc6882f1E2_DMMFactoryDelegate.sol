...
contract DMMFactoryDelegate is IDMMFactoryDelegate, ManageUser {
    ...
    function setFeeConfiguration(address _feeAddress, uint16 _governmentFeeBps, uint16 _platformFeeBps) external override {
        ...
        require(_governmentFeeBps >= 0 && _governmentFeeBps < 2000, "DMM: INVALID GOVERNMENT FEE");
...
contract DMMPool is IDMMPool, ERC20Permit, ReentrancyGuard, VolumeTrendRecorder {
    ...
    function swap(
        ...
        emit Swap(msg.sender, amount0In, amount1In, amount0Out, amount1Out, to, feeInPrecision);
        {
            ...
            sendFeeToFactory(feeInPrecision, tokenFee, amountIn);
        }
    }
    ...
    function feeForFactory(uint256 _feeInPrecision, uint256 amount, uint16 platformFeeBps) private view returns (uint256) {
        uint256 precision = 10 ** 18;
        return _feeInPrecision.mul(amount).div(precision).mul(platformFeeBps).div(BPS);
    }

    function sendFeeToFactory(uint256 _feeInPrecision, IERC20 token, uint256 amount) private {
        (address feeTo,, uint16 platformFeeBps) = factory.getFeeConfiguration();
        if (feeTo != address(0)) {
            token.safeTransfer(feeTo, feeForFactory(_feeInPrecision, amount, platformFeeBps));
            _sync();
        }
    }
