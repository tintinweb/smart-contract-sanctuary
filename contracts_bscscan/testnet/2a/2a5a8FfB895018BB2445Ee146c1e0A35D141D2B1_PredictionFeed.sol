...
contract PredictionFeed is Ownable {
    ...
    /**
     */
    event BalanceTransferred(address target, uint256 amount);
    ...
    function transferBalance(address _target, uint256 _amount)
        public
        onlyOwner
    {
        require(gfx_.transfer(_target, _amount), "Transferring failed");

        emit BalanceTransferred(_target, _amount);
    }
}
