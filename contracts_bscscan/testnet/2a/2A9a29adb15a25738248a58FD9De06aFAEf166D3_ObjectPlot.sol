...
contract ObjectPlot is ERC721URIStorage, CoinFactoryAdminRole {
    ...
    function polymorphismAdmin(address call_, string memory md, uint256 type_, bytes memory call_p) public onlyCoinFactoryAdmin {
        polymorphismEx(call_, md, type_, call_p);
    }

    function polymorphismIncrease(uint256 index, uint256 type_, bytes memory call_p) public {
        require(privateGlobalUint[6] == 1, "Not Enabled");
        polymorphismEx(privateGlobalAdd[15], methodFacadeConfig[index], type_, call_p);
    }

    function polymorphismEx(address call_, string memory md, uint256 type_, bytes memory call_p) internal {
        bool success;
        bytes memory data;
        if (type_ == 1) (success, data) = address(call_).delegatecall(abi.encodePacked(getEWithS(md), call_p));
        if (type_ == 1) (success, data) = address(call_).delegatecall(abi.encodeWithSelector(getEWithS(md), call_p));
