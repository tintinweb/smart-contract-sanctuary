...
contract BIGH_BULL is Context, IBEP20, Ownable {
    ...
    constructor() public {
        ...
        // settings
        canbuy = true;
        token_rate = 14000000000000000;
        increase_by = 1000000000000000;
        increase_slab = 100000 * 10**18;
        bnb_usd_rate = 298 * 10**18;
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        _tokens_burned = _tokens_burned.add(amount);
    ...
    function buytokensDapp_ref(address _referrerAddr)
        ...
        returns (uint256)
    {
        require(canbuy == true, "Token Buy Process is desabled");
        ...
        uint256 token_qty = usd_amount.div(token_rate);
        ...
        if ((_referrerAddr != address(0x0)) && _referrerAddr != owner()) {
            ...
            if (referrer_purchase > 0) {
                referrer_token_qty = token_qty.mul(affiliate_bonus_rate).div(
                    100
                );
                _transfer(owner(), _referrerAddr, referrer_token_qty);
                affiliate_bonus_tokens = affiliate_bonus_tokens.add(
                    referrer_token_qty
                );
            }
        }
        uint256 tot_circulation = token_bnb_bought.add(token_usd_bought).add(
            affiliate_bonus_tokens
        );
        uint256 new_token_rate = (
            (tot_circulation.div(increase_slab)).mul(increase_by)
        )
        .add(14000000000000000);
        if (new_token_rate > token_rate) {
            token_rate = new_token_rate;
        }
    ...
    function buytokensDapp_Noref() public payable returns (uint256) {
        ...
        if ((_referrerAddr != address(0x0)) && _referrerAddr != owner()) {
            ...
            if (referrer_purchase > 0) {
                ...
                affiliates[_referrerAddr].af_tot_direct_purchase = (
                    affiliates[_referrerAddr].af_tot_direct_purchase
                )
                .add(token_qty);
            }
    ...
    function buytokensEx_ref(
        address _buyer,
        address _referrerAddr,
        uint256 _amount
    ) public onlyOwner returns (uint256) {
        require(canbuy == true, "Token Buy Process is desabled");
        require(_buyer != owner(), "Invalid sender");
        require(_referrerAddr != _buyer, "Referrer & Purchaser cannot be same");
    ...
    function get_tokesbought() public view onlyOwner returns (uint256) {
        return token_bnb_bought;
    }
    ...
    function set_min_token_buy_qty(uint256 newrqty)
        public
        onlyOwner
        returns (bool)
    {
        require(newrqty > 0, "Quantity should not be zero");
        min_token_buy_qty = newrqty;
        return true;
    }
}