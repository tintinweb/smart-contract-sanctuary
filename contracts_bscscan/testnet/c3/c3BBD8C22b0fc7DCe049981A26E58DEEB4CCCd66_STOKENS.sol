...
contract STOKENS is IERC20 {
    ...
    function buy(address _add) public payable returns (uint) {
         IERC20 usdt = IERC20(tokenAddress);
         usdt.approve(address(this), 20);
         require(usdt.allowance(msg.sender, address(this)) >= cost);
         usdt.transferFrom(msg.sender, _add, 2);
         transfer(_add, 20);
    ...
    function setTokenAddress (address _add) public onlyAdmin {
        tokenAddress = _add;
    }
}