...
// Dependency file: contracts/interfaces/IBITNET.sol
...
// Root file: contracts/tokens/BITNET.sol
...
// import "contracts/interfaces/IBITNET.sol";
...
contract BITNET is ERC20Upgradeable, OwnableUpgradeable, IBITNET {
    using SafeMath for uint256;
    ...
    constructor() {
        implementation = address(new BITNETDividendTracker());
    }
    ...
    function initialize(
        ...
        dividendTracker = BITNETDividendTracker(
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "BITNET: The dividend tracker already has that address"
        );

        BITNETDividendTracker newDividendTracker = BITNETDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "BITNET: The new dividend tracker must be owned by the BITNET token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "BITNET: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "BITNET: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "BITNET: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "BITNET: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "BITNET: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "BITNET: Cannot update gasForProcessing to same value"
        );
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint256 initialBNBBalance = IERC20(rewardToken).balanceOf(
            address(this)
        );
...
contract BITNETDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BITNET contract."
        );
    }
