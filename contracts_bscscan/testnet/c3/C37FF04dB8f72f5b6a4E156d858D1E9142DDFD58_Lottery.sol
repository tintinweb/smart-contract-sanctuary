...
contract Lottery is LotteryOwnable, Initializable {
    ...
    uint8 public charifyFee;
    // Percentage of staking pool
    ...
    function reset() external onlyAdmin {
        ...
        //before reset lottery send charity and pool
        if(getTotalRewards(issueIndex - 1) > 0){
            uint256 charityAmount = getTotalRewards(issueIndex-1).mul(charifyFee).div(100);
            uint256 poolAmount = getTotalRewards(issueIndex-1).mul(poolFee).div(100);
            // send kapex to charity address
            kapex.safeTransfer(address(charityAddress), charityAmount);
            // send kapex to staking pool address
            kapex.safeTransfer(address(poolAddress), poolAmount);
        }
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        kapex.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        kapex.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            kapex.safeTransfer(address(msg.sender), reward);
        }
    ...
    function  multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            kapex.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        kapex.safeTransfer(address(msg.sender), _amount);
    ...
    function setAllocation(uint8 _allcation1, uint8 _allcation2, uint8 _allcation3) external onlyAdmin {
     ...
     // Update charity address by only admin.
    function setCharityAddress(address _charityAddress) public onlyOwner {
        charityAddress = _charityAddress;
    }

    // Update kapex staking pool address by only admin.
    function setPoolAddress(address _poolAddress) public onlyOwner {
        poolAddress = _poolAddress;
    }

    // Update charity Fee by only admin
    function setCharityFee(uint8 _charityFee) public onlyOwner {
        charifyFee = _charityFee;
    }

    // Update Staking pool Fee by only admin
    function setPoolFee(uint8 _poolFee) public onlyOwner {
        poolFee = _poolFee;
    }
...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("Summit Lottery Ticket", "SummitTicket") {}
