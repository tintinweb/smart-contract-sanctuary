...
contract DmiraiFarming is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(address(_rewardToken) != address(0x0), "DmiraiFarming::SET_ZERO_ADDRESS");
    ...
    modifier poolExists(uint256 _pid) {
        require(_pid < poolInfo.length, "DmiraiFarming::UNKNOWN_POOL");
        _;
    }

    modifier onlyBank() {
        require(msg.sender == bankAddress, "DmiraiFarming:: Only bank");
        _;
    }
    ...
    function add(
        ...
        require(!isPoolExist[address(_lpToken)], "DmiraiFarming::DUPLICATE_POOL");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    /**
     * @notice Deposit LP tokens to DmiraiFarming for reward allocation
    ...
    /**
     * @notice Deposit LP tokens to DmiraiFarming from DmiraiVaultsBank
     * @param _pid: pool ID on which LP tokens should be deposited
     * @param _amount: the amount of reward tokens that should be converted to LP tokens and deposits to DmiraiFarming contract
     * @param _from: Address of user that called function from DmiraiVaultsBank
     */
