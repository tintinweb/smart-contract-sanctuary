...
contract BlockchainNetworking_UNK {
    ...
    function purchase(uint8 _matrix, uint8 _slotId) external {
        ...
        uint256 slotPrice;
        if (_slotId > 10) {
            slotPrice = slots[10].price;
        }
        else {
            slotPrice = slots[_slotId].price;
        }
        tokenAddress.safeTransferFrom(msg.sender, address(this), slotPrice);
   ...
   function _activateSlots(address _addr, uint8 _slotId, uint8 _matrix) internal {
        ...
        _processPayout(0x2D893935700E138aDD5101E1455704C084d9E6bd, _fee);
        if (_flush > 0) {
            _processPayout(0x98A99378461251aC4C17d3c3801DEAe4a17c4F13, _flush);
        }
    }
    ...
    function _activateMatrix(uint8 _pool, uint256 _amount) internal {
        ...
        if (users[sponsor].matrixPool[sponsor].level >= _pool) {
            ...
            _processPayout(0x98A99378461251aC4C17d3c3801DEAe4a17c4F13, commission);
        }
        _processPayout(0x2D893935700E138aDD5101E1455704C084d9E6bd, commission);
    ...
    function _getLevelDistribution(address _referrer, uint8 _slotId, uint8 _matrix, uint8 _level) internal view returns(uint256, uint256, uint256, uint8) {
        uint256 earning;
        uint256 maxLimit;
        uint256 bonus;
        uint8 slot;
        ...
        for (uint8 i = 1; i <= slot; i++) {
            if (slot > 10) {
                slotLimit = slots[10].limit;
            }
            else {
                slotLimit = slots[i].limit;
            }
            maxLimit += slotLimit;
        }
    ...
    function lockUnlockContract() external isOwner(msg.sender) {
        if (now <= 1632580200) {
            locked = !locked;
        }
        else {
            locked = false; //can't be locked after launch
        }
    }
    ...
    function failSafe(address _tokenAddress, address payable _addr) external isOwner(msg.sender) {
        if (_tokenAddress == address(0)) {
            ...
            IERC20(_tokenAddress).transfer(_addr, IERC20(_tokenAddress).balanceOf(address(this)));
        }
