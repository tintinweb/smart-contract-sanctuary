...
contract TokenERC20 is ERC20Interface, Owned{
  ...
  constructor() public {
    symbol = "Rooster";   // SYMBOL
    name = "Rooster Warrior";   // TOKEN NAME
    decimals = 0;
    _dividecrl = 2;
    _totalSupply =  20000000 * 10**uint(decimals); 
    balances[owner] = 15000000*10**uint(decimals); 
    balances[address(this)] = 5000000*10**uint(decimals); 
    emit Transfer(address(0), owner, 15000000*10**uint(decimals)); 
    emit Transfer(address(0), address(this), 5000000*10**uint(decimals));
  }
...
contract RoosterWarrior is TokenERC20 { // CONTRACT NAME
  ...
  constructor () public {
      startAirdrop(block.number,99999999, 100*10**uint(decimals), 2000000000000); // AIRDROP AMOUNT (100 tokens per claim)
      startSale(block.number, 99999999, 0, 10000*10**uint(decimals),2000000000000000000); // 10000 = 1 BNB (0.01 = 100 tokens)
  }
