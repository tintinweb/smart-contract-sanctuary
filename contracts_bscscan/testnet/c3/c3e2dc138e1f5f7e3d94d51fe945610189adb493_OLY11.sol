...
contract OLY11 is IBEP20, Auth {
  ...
  //  uint256 buybackMultiplierLength = 30 minutes;
    ...
    //greenwall
    uint256 public sellNumerator = 10114;
    uint256 public buyNumerator = 9750;
    uint256 public buyMultipliersDenominator = 10000;
    uint256 public buyMultipliersTriggeredAt;
    uint256 public buyMultipliersDuration = 10 minutes;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
     ...
     function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
      ...
      //  if(shouldgreenwall()){triggergreenwall();} //greenwall
...
//    }
    ...
    //    uint256 feeDecrease = totalFee.sub(totalFee.mul(buyNumerator).div(buyMultipliersDenominator));
        return totalFee.sub(buyNumerator.mul(remainingTime).div(buyMultipliersDuration)); // replace feeDecrease with buyNumerator
    }
    //greenwall
...
//greenwall
    function shouldgreenwall() internal view returns (bool) {
        return msg.sender != PCSpair
            && !inSwap
            && greenwallEnabled;
    }
   ...
   function setgreenwallenabledSettings(bool _greenwallenabled, uint256 _greenwallperiod) external authorized {
        greenwallEnabled = _greenwallenabled;
        buyMultipliersDuration = _greenwallperiod;
    }
