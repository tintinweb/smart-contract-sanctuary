...
contract Distribute is Ownable {
    ...
    constructor(address _nftAddr, address _coinAddr, uint[][] memory _infos) {
        ...
        // todo minutes to days
        nextClaimTime = block.timestamp + 1 minutes;
    ...
    function addClaimInfo(uint _tokenId, uint _canClaimTotal, uint _claimPercent) internal {
        ...
        ClaimInfo memory _claimInfo = ClaimInfo({tokenId:_tokenId,canClaimTotal:_canClaimTotal * 18 ** 10,claimState:State.Active,
        claimPercent: _percentTemp,claimedQuantity: 0,laveQuantity: _canClaimTotal,
    ...
    function claim() hasClaimRight() public {
        ...
        //todo minutes to days
        uint _days = SafeMath.div((block.timestamp - gameStartTime), SECONDS_PER_MINUTE);
        //todo 1 to 30
        ...
        for(uint i=0;i<_myTokenIds.length; i++) {
            ...
            //todo minutes to days
            nextClaimTime = gameStartTime + (_canClaimeNumTotal + 1) * (1 minutes);
