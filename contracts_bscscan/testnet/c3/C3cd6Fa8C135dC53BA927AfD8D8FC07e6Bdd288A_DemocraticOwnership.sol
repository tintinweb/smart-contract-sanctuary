...
contract DemocraticOwnership {
    ...
    event EventCallRegistered (uint eventDate, string eventSCName, string eventFunctionName);

    function readAuthorizedAddress(address sender) external view returns (address) {
        require(ownership[sender], "Sender Must be Admin");
    ...
    function RegisterCall(address sender, string memory scname, string memory funcname) external {
        ...
        emit EventCallRegistered(time, scname, funcname);
    }
