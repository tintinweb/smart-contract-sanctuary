...
contract BotTrading is ReentrancyGuard, Ownable {
    ...
    uint256 public redistroRate = 5;
    uint256 public referralRate = 1;
    uint256 public minDeposit;
    bool public locked = false;
    uint256 public totalStake = 0;
    uint256 public rewardPerToken = 0;
    mapping(address => uint256) public stake;
    mapping(address => uint256) public refer;
    ...
    constructor(address _tokenAddr, uint256 _minDeposit) {
        token = IERC20(_tokenAddr);
        minDeposit = _minDeposit;
    }
    
    event staked(address _staker, uint256 _stakedAmt, uint256 _feeAmt, uint256 _redistroAmt);
    event stakedWithReferrer(address _staker, uint256 _stakedAmt, uint256 _feeAmt, uint256 _redistroAmt, uint256 _referralAmt);
    ...
    event referralRateSet(uint256 _newFee);
    ...
    event reInvestedWithRef(address _user, uint256 _amount, address _referrer, uint256 _referredAmt);
    

    // User functionality
    function depositStake(uint256 _amount) public onlyUnlock {
        require(token.allowance(msg.sender, address(this)) >= _amount, "Not enough allowance to stake");
        ...
        token.transferFrom(msg.sender, owner(), calcDepositFeeAmt(_amount));
        token.transferFrom(msg.sender, address(this), _amount - feeAmt);
    ...
    function depositStakeWithRef(uint256 _amount, address _referrer) public onlyUnlock {
        require(token.allowance(msg.sender, address(this)) >= _amount, "Not enough allowance to stake");
        require(_referrer != 0x0000000000000000000000000000000000000000, "Referral not valid: referrer is 0x0000000000000000000000000000000000000000");
        require(_referrer != address(0), "Referral not valid: referrer is 0 address");
        require(_referrer.balance != 0, "Referral is a empty address: 0 balance");
        require(_referrer != tx.origin, "Referral is not allowed to be sender");
        uint256 feeAmt = calcDepositFeeAmt(_amount);
        uint256 feeToOwner = calcDepositFeeAmt(_amount) - calcReferralAmt(_amount);
        uint256 feeToReferrer = calcReferralAmt(_amount);
        uint256 redistroAmt = calcRedistroAmt(_amount);
        ...
        token.transferFrom(msg.sender, owner(), feeToOwner);
        token.transferFrom(msg.sender, _referrer, feeToReferrer);
        token.transferFrom(msg.sender, address(this), _amount - feeAmt);
        ...
        // update refer amt
        refer[_referrer] = refer[_referrer] + feeToReferrer;

        emit stakedWithReferrer(msg.sender, _amount - feeAmt - redistroAmt, feeAmt, redistroAmt, feeToReferrer);
    }
    ...
    function withdrawStake(uint256 _amount) public onlyUnlock nonReentrant {
        ...
        token.transfer( msg.sender, _amount-fee-redistro);
    ...
    function withdrawReward() public onlyUnlock {
        ...
        token.transfer(msg.sender, reward - claimFeeAmt - redistroAmt);
        token.transfer(owner(), claimFeeAmt);
    ...
    function reInvestReward() public onlyUnlock {
        ...
        token.transfer(owner(), stakeFeeAmt);
    ...
    function reInvestRewardWithRef(address _referrer) public onlyUnlock {
        require(_referrer != 0x0000000000000000000000000000000000000000, "Referral not valid: referrer is 0x0000000000000000000000000000000000000000");
        require(_referrer != address(0), "Referral not valid: referrer is 0 address");
        require(_referrer.balance != 0, "Referral is a empty address: 0 balance");
        require(_referrer != tx.origin, "Referral is not allowed to be sender");
        ...
        uint256 reward = computeReward(msg.sender);
        uint256 stakeFeeAmt = calcDepositFeeAmt(reward);
        uint256 feeToOwner = calcDepositFeeAmt(reward) - calcReferralAmt(reward);
        uint256 feeToReferrer = calcReferralAmt(reward);
        uint256 redistroAmt = calcRedistroAmt(reward);

        // token.transfer(owner(), stakeFeeAmt);
        token.transfer(owner(), feeToOwner);
        token.transfer(_referrer, feeToReferrer);
        ...
        emit reInvestedWithRef(msg.sender, reward, _referrer, feeToReferrer);
    }
    
    // Owner (Trading bot) functionality
    function distribute(uint256 _amount) public onlyOwner {
        ...
        require(token.allowance(msg.sender, address(this)) >= _amount, "Not enough allowance to transfer reward. Make sure enough amount is approved");
    ...
    function setReferralRate(uint256 _newFee) external onlyOwner {
        require(_newFee <= depositFee, "Referral rate need to be less than deposit fee");
        referralRate = _newFee;
        emit referralRateSet(_newFee);
    }
    ...
    function calcReferralAmt(uint256 _amount) public view returns(uint256) {
        return _amount.mul(referralRate).div(100);
    }
}