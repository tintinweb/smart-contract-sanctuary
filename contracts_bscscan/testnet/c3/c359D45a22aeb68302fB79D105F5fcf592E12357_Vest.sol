...
contract Vest is Ownable {
    ...
    function claimPartner() public isLock{
        require (msg.sender == partnersWallet,"UnAuthorized user");
        require (block.timestamp >= creationTime + 730 days,"Vesting time");
        require (partnerTime[msg.sender] == 0 || block.timestamp >= partnerTime[msg.sender] + 30 days,"Invalid time");
        require (claimCount[msg.sender] < 10,"Claimed fully");
        partnerTime[msg.sender] = block.timestamp;
        uint amount = totalSupply * partnerPercent /100e18;
        token.transfer(msg.sender,amount*10/100);
        claimCount[msg.sender]++;
    }
