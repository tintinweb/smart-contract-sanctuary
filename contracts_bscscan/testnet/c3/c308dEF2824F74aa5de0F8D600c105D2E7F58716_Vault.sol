...
contract Vault {
    ...
    function getRewardsEarned(address staker) public view returns (uint256) {
        return stakersRewards[staker] + (balance[staker] * rewardsPerSAFUPerBlock * (block.number - lastUpdate[staker])).div(10e9);
    }
	...
	IBEP20 SAFU = IBEP20(0x377Cc56C4830717813fD358eBD3B4702f4D07BF7); //Test Net
    ...
    function stake(uint256 amount) public {
		...
		if (balance[msg.sender] > 0) {
			stakersRewards[msg.sender] += (balance[msg.sender] * rewardsPerSAFUPerBlock * (block.number - _lastUpdate)).div(10e9);
		} else {
    ...
    function withdraw(uint256 amount) public {
		...
		SAFU.transfer(msg.sender, amount.mul(101).div(100) + rewards);
    }
	...
	function modifyRewards(uint256 amount) public onlyOwner {
		...
		for (uint256 i = 0; i < stakers.length; i++) {
			...
			stakersRewards[stakers[i]] += (balance[stakers[i]] * rewardsPerSAFUPerBlock * (block.number - _lastUpdate)).div(10e9);
		}
