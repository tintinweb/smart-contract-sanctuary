...
import "./IPLTRandomNumberGenerator.sol";
import "./IPLTLottery.sol";
...
contract PLTLottery is ReentrancyGuard, IPLTLottery, Ownable {
    ...
    IERC1155 public poollotoNFT;
    uint256 public poollotoNFTFreeTicketId;
    ...
    struct Lottery {
        ...
        uint256[6] freeTicketsArray; // 0: 1 matching number // 5: 6 matching numbers
    ...
    event NewPoollotoNFT(address indexed poollotoNFT);
     ...
     * @param _useFreeTickets: to use the free tickets or not
    ...
    function buyTickets(
        ...
        (
            uint256 amountPLTToTransfer,
            uint256 amountPLTNftToTransfer
        ...
        pltToken.safeTransferFrom(
            address(msg.sender),
            address(this),
            amountPLTToTransfer
        );
        poollotoNFT.safeTransferFrom(
            address(this),
            address(msg.sender),
            poollotoNFTFreeTicketId,
            amountPLTNftToTransfer,
            ""
        );
    ...
    function claimTickets(
        ...
        uint256 rewardInPLTNftToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            if (_brackets[i] != 5) {
                ...
                require(
                    (rewardForTicketId == 0 && nftRewardForTicketId == 0),
            ...
            rewardInPLTNftToTransfer += nftRewardForTicketId;
        }

        if (rewardInPLTToTransfer > 0) {
            // Transfer money to msg.sender
            pltToken.safeTransfer(msg.sender, rewardInPLTToTransfer);
        }

        if (rewardInPLTNftToTransfer > 0) {
            // Transfer nft free tickets to msg.sender
            poollotoNFT.safeTransferFrom(
                address(this),
                address(msg.sender),
                poollotoNFTFreeTicketId,
                rewardInPLTNftToTransfer,
                ""
            );
        }
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInPLT) *
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId]
            .amountCollectedInPLT - amountToShareToWinners);
    ...
    /**
     * @notice Change the Poolloto NFT address
     * @param _poollotoNFT: address Poolloto NFT
     */
    function changePoollotoNFT(address _poollotoNFT) external onlyOwner {
        poollotoNFT = IERC1155(_poollotoNFT);
        emit NewPoollotoNFT(_poollotoNFT);
    }
     ...
     * @param _freeTicketsArray: the free tickets array
    ...
    function startLottery(
        ...
        uint256[6] calldata _freeTicketsArray,
        ...
        require(
            (_priceTicketInPLT >= minPriceTicketInPLT) &&
                (_priceTicketInPLT <= maxPriceTicketInPLT),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            freeTicketsArray: _freeTicketsArray,
    ...
    /**
     * @notice It allows the admin to recover wrong nft sent to the contract
     * @param _tokenAddress: the address of the nft to withdraw
     * @param _id: the address of the nft id to withdraw
     * @param _tokenAmount: the number of token amount to withdraw
     * @dev Only callable by owner.
     */
    function recoverWrongNFT(
        address _tokenAddress,
        uint256 _id,
        uint256 _tokenAmount
    ) external onlyOwner {
        require(
            _tokenAddress != address(poollotoNFT),
            "Cannot be poolloto NFT"
        );
     ...
     */
    function setMinAndMaxTicketPriceInPLT(
        uint256 _minPriceTicketInPLT,
        uint256 _maxPriceTicketInPLT
    ) external onlyOwner {
        require(
            _minPriceTicketInPLT <= _maxPriceTicketInPLT,
    ...
    /**
     * @notice Set poolloto NFT free ticket id
     * @dev Only callable by owner
     */
    function setPoollotoNFTFreeTicketId(uint256 _poollotoNFTFreeTicketId)
        external
        onlyOwner
    {
        poollotoNFTFreeTicketId = _poollotoNFTFreeTicketId;
    }
    ...
    function calculateTotalPriceForBulkTickets(
        ...
        require(
            _discountDivisor >= MIN_DISCOUNT_DIVISOR || useFreeTickets,
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return (
                _lotteries[_lotteryId].pltPerBracket[_bracket],
                _lotteries[_lotteryId].freeTicketsArray[_bracket]
            );
        } else {
            return (0, 0);
        }
    ...
    function _calculateTotalPriceForBulkTickets(
        ...
        if (_useFreeTickets) {
            uint256 freeTicket = poollotoNFT.balanceOf(
                msg.sender,
                poollotoNFTFreeTicketId
            );
            require(
                _numberTickets > freeTicket,
                "Number of tickets must be bigger then free tickets"
            );
            return ((_priceTicket * (_numberTickets - freeTicket)), freeTicket);
        } else {
            return (
                ((_priceTicket *
                    _numberTickets *
                    (_discountDivisor + 1 - _numberTickets)) /
                    _discountDivisor),
                0
            );
        }
