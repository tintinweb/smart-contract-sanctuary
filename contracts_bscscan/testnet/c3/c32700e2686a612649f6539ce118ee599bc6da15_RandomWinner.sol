...
contract RandomWinner is VRFConsumerBase {
    ...
    function addContestants(string[] memory name) public onlyOwner {
        uint i = 0;
        for(i; i < name.length; i++){
            contestants.push(name[i]);    
        }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        winner = contestants[randomness % (contestants.length + 1)];
        contestants = new string[](0);
    }
}