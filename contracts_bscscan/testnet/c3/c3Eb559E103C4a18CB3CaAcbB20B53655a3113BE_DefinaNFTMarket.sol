...
contract DefinaNFTMarket is Ownable, IERC721Receiver, Initializable, Pausable {
    ...
    event CleanFromWhiteList(address indexed _who);
    ...
    function setupWhitelist(address[] calldata candidates_) onlyOwner whenPaused external returns (bool) {
        ...
        for(uint i = 0; i < candidates_.length; i++){
            ...
            emit AddedToWhiteList(candidates_[i]);
        }
     ...
     */
    function cleanWhitelist() onlyOwner whenPaused public returns (bool) {
        ...
        for(uint i = 0; i < length; i++) {
            ...
            emit CleanFromWhiteList(candidate);
