...
contract BurningMoonDividentNFT is ERC165, IERC721, IERC721Metadata ,IERC721Enumerable,Ownable {
    ...
    function AirdropBM() public isClaiming{
        _distributeCompound();
        for(uint i=0;i<NFTHolders.length;i++)
            _payout(NFTHolders[i]);
    }

    function ClaimDividents() public isClaiming{
        _distributeCompound();
    ...
    function OwnerSetURI(string memory newURI, uint ID) public onlyOwner{
        require(_exists(ID));
        NFTData memory data=NFTs[ID];
        data.URI=newURI;
        NFTs[ID]=data;
    }
    function OwnerSetTaxes(uint8 taxes) public onlyOwner{
        Taxes=taxes;
    }
    function OwnerTransferSacrifice(address target,uint256  amount) public onlyOwner{
    ...
    function AddNFTs(NFTData[] memory data) public onlyOwner{
    ...
    function OwnerAddToPresale(address[] memory recipients, bool add) public onlyOwner{
    ...
    function AddToSale(uint256 ID, uint256 value, bool isForSale)public{
    ...
    function AddToSale(uint256 value, bool isForSale)public{
    ...
    function isInPresale(address account) public view returns (bool){
    ...
    function TokenForSaleAT(uint256 ID) public view returns(uint){
    ...
    function SetWhitelist(address[] memory addresses, bool add) public onlyOwner{
        for(uint i=0;i<addresses.length;i++)
            Whitelist[addresses[i]]=add;
    } 
    bool _presaleOpen;
    bool _onlyWhitelist;
    function SetPresale(bool PresaleOpen, bool OnlyWhitelist) public onlyOwner
    {
