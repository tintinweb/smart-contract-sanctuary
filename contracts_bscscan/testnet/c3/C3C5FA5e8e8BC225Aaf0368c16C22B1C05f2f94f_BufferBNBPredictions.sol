 ...
 */
contract BufferBNBPredictions is
    IBufferOptions,
    Ownable,
    ERC721,
    ERC721Enumerable,
    ...
    AccessControl
{
    bytes32 public constant PREDICTION_EXERCISER =
        keccak256("PREDICTION_EXERCISER");
    ...
    constructor(
        ...
        _setupRole(PREDICTION_EXERCISER, msg.sender);
     ...
     * @param expectedChange Percentage change in BNB/USD Price
    ...
    function create(
        ...
        (uint256 total, uint256 settlementFee, ) = fees(period, amount);
        ...
        require(
            expectedChange <= optionCollateralizationRatio,
            "expectedChange is too high"
        );
        ...
        Option memory option = Option(
            ...
            currentPrice,
            amount,
            mulDiv(amount, expectedChange, 100),
            total - settlementFee,
            block.timestamp + period,
            optionType
        );
        // if (msg.sender != owner()) {
        //     approve(owner(), optionID);
        // }
        ...
        uint256 stakingAmount = mulDiv(settlementFee, 75, 100);
        ...
        emit Create(optionID, msg.sender, stakingAmount, total);
    }

    /**
     * @notice Exercises/Unlocks an active option depending on the result
     * @param optionID ID of your option
     * @param isPredictionTrue: Boolean specifying if the price went above the predicted value in the defined period
     */
    function execute(uint256 optionID, bool isPredictionTrue) public {
        require(
            hasRole(PREDICTION_EXERCISER, msg.sender),
            "msg.sender is not allowed to excute the option contract"
        );

        Option storage option = options[optionID];

        // We need to wait till the option is expired and then only exercise it with the info from the exerciser
        require(
        ...
        if (isPredictionTrue) {
            ...
            uint256 profit = option.lockedAmount;
    ...
    /**
     * @notice Exercises/Unlocks an array of options
     * @param optionIDs array of options
     * @param isPredictionTruePerOption array of prediction results for each option
     */
    function executeAll(
        uint256[] calldata optionIDs,
        bool[] calldata isPredictionTruePerOption
    ) external {
        uint256 arrayLength = optionIDs.length;
        for (uint256 i = 0; i < arrayLength; i++) {
            execute(optionIDs[i], isPredictionTruePerOption[i]);
        }
    }
     ...
     */
    function fees(uint256 period, uint256 amount)
        public
        view
        returns (
            uint256 total,
    ...
    function getSettlementFee(uint256 amount)
        ...
        returns (uint256 fee)
    {
        return mulDiv(amount, settlementFeePercentage, 100);
    }
     ...
     */
    function getPeriodFee(uint256 amount, uint256 period)
        internal
        view
        returns (uint256 fee)
    {
        return (amount * sqrt(period) * impliedVolRate) / (PRICE_DECIMALS);
    }
     ...
     **/
    function createOptionFor(address holder) private returns (uint256 id) {
        id = nextTokenId++;
