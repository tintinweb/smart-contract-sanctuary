...
import "../standard/AntiBotStandardToken.sol";
import "../TattooAntiBot.sol";

contract AntiBotStandardTokenFactory {
...
//        TattooAntiBot tattooAntiBot = new TattooAntiBot();
        AntiBotStandardToken _antibotStandardToken = new AntiBotStandardToken(name, symbol, decimals, totalSupply, 0x56B9631C475342f3366e18eaaf828717a7d924c7);//address(tattooAntiBot));
        _antibotStandardToken.transfer(msg.sender, _antibotStandardToken.totalSupply());
        _antibotStandardToken.transferOwnership(msg.sender);
//        tattooAntiBot.setTokenOwner(msg.sender);
        feeReceiver.transfer(msg.value);
        emit TokenCreated(address(_antibotStandardToken));
    }
}
...
contract TattooAntiBot is ITattooAntiBot {
    ...
    address[] public _blacklistAddresses;
    ...
    function addManyToWhitelist(address[] memory _beneficiaries) external onlyOwner {
        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            if (whitelist[_beneficiaries[i]] == false)
            {
                whitelist[_beneficiaries[i]] = true;
                _whitelistAddresses.push(_beneficiaries[i]);
            }
        }
    }
    ...
    function removeManyFromWhitelist(address[] memory _beneficiaries) external onlyOwner {
        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            if (whitelist[_beneficiaries[i]] == true)
            {
                whitelist[_beneficiaries[i]] = false;
                uint j = find(_beneficiaries[i], _whitelistAddresses);
                delete _whitelistAddresses[j];
            }
        }
    }

    function addManyToBlacklist(address[] memory _beneficiaries) external onlyOwner {
        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            if (blacklist[_beneficiaries[i]] == false)
            {
                blacklist[_beneficiaries[i]] = true;
                _blacklistAddresses.push(_beneficiaries[i]);
            }
        }
    }

    function removeManyFromBlacklist(address[] memory _beneficiaries) external onlyOwner {
        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            if (blacklist[_beneficiaries[i]] == true)
            {
                blacklist[_beneficiaries[i]] = false;
                uint j = find(_beneficiaries[i], _blacklistAddresses);
                delete _blacklistAddresses[j];
            }
        }
    }

    function find(address recipient, address[] memory addresses) internal view returns(uint) {
    ...
    function getAntiBotData() external view returns (address[] memory, uint[] memory, address[] memory, address[] memory)
    {
        address[] memory _addresses = new address[](3);
        uint[] memory _amounts = new uint[](4);
        _addresses[0] = _router;
        _addresses[1] = _token;
        _addresses[2] = _pairToken;
        _amounts[0] = _amountLimitPerTrade;
        _amounts[1] = _amountToBeAddedPerBlock;
        _amounts[2] = _timeLimitPerTrade;
        _amounts[3] = _blockNumberToDisable;
        return (_addresses, _amounts, _whitelistAddresses, _blacklistAddresses);
    }
