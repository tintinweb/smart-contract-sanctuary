...
contract MOVX is Context, IBEP20, Ownable {
    ...
    uint256 public _buyTaxFee = 1; // Fee that gets re-distributed to holders.
    ...
    constructor (address payable buybackWallet, address payable marketingWallet, address payable researchWallet, address pancakeSwapRouter) {
        ...
        _isExcludedMaxWallet[address(_burnAddress)] = true;
        _isExcludedMaxWallet[address(_buybackWallet)] = true;
        _isExcludedMaxWallet[address(_marketingWallet)] = true;
        _isExcludedMaxWallet[address(_researchWallet)] = true;
    ...
    function setOtherFeePercent(uint256 Taxfee, uint256 BuybackFee, uint256 LiquidityFee, uint256 MarketingFee, uint256 ResearchFee) external onlyOwner() {
        _otherTaxFee = Taxfee;
        _otherBuybackFee = BuybackFee;
        _otherLiquidityFee = LiquidityFee;
        _otherMarketingFee = MarketingFee;
        _otherResearchFee = ResearchFee;
        totalOtherFee = _otherLiquidityFee.add(_otherBuybackFee).add(_otherMarketingFee).add(_otherResearchFee);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludeAddressFromMaxTx[from] && !_isExcludeAddressFromMaxTx[to] && from != owner() && to != owner()) {
        ...
        if(!_isExcludedMaxWallet[from] && !_isExcludedMaxWallet[to] && (to != pancakeSwapV2Pair) && from != owner() && to != owner() && (to != address(0))) {
