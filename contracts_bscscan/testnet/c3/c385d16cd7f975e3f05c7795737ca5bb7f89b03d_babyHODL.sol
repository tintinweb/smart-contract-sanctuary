...
contract babyHODLDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("babyHODL_Dividend_Tracker", "babyHODL_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "babyHODL_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "babyHODL_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main babyHODL contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "babyHODL_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "babyHODL_Dividend_Tracker: Cannot update claimWait to same value");
...
contract babyHODL is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    babyHODLDividendTracker public dividendTracker;
    ...
    constructor() ERC20("babyHODL", "babyHODL") {
        BNBRewardsFee = 8;
        liquidityFee = 3;
    	...
    	dividendTracker = new babyHODLDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "babyHODL: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "babyHODL: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "babyHODL: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "babyHODL: Automated market maker pair is already set to that value");
