...
contract ModelCollection is ERC721 {
    ...
    modifier onlyModel {
        require(msg.sender == modelAddress, "You are not model.");
        _;
    }
    ...
    modifier onlyPlatformOwner {
        require(msg.sender == master.getPlatformOwner(), "You are not platform owner.");
        _;
    }

    modifier lockInitializer {
        require(!initializerLocked, "You are locked.");
        _;
        initializerLocked = true;
    }

    modifier checkBlacklist {
        require(!master.modelIsBlacklisted(modelAddress), "You are not in black list.");
        _;
    }
    ...
    function addNft(
        ...
        master.logAddNft(nfts.length.sub(1), _uri, _mintCap, _purchaseTokenAddress, _purchaseTokenAmount, _description);
    ...
    function purchaseNft(uint256 nftId) checkBlacklist external returns (uint256) {
        ...
        require(nftId <= nfts.length.sub(1), "PurchaseNft: NFT Does not exist");
        ...
        nft.numberMinted = nft.numberMinted.add(1);
    ...
    function makePayment(address to, uint256 amount, address purchaseTokenAddress) internal {
        ...
        if (
            referrer != address(0)
            && block.timestamp.sub(initializedAt) < referralDuration
            && !master.modelIsBlacklisted(referrer)
        ) {
            referrerAmount = amount.mul(referrerFee).div(100);
            purchaseToken.transferFrom(msg.sender, referrer, referrerAmount);
            platformFee.sub(referrerFee);
            master.logReferralPay(msg.sender, referrer, referrerAmount, purchaseTokenAddress);
        }

        if (purchaseTokenAddress == cummies) {
            if (platformFee >= 5) {
                platformFee = platformFee.sub(5);
            } else {
                platformFee = 0;
            }
        }

        uint256 platformAmount = amount.mul(platformFee).div(100);
        ...
        if(purchaseTokenAddress == master.getMainToken()) {
            ...
            purchaseToken.transferFrom(msg.sender, master.getFeeAggregator(), platformAmount);
        }
        purchaseToken.transferFrom(msg.sender, to, amount.sub(platformAmount).sub(referrerAmount));
    }
    ...
    function _burn(uint256 tokenId) internal override {
        ...
        super._burn(tokenId);
        delete nfts[nftId];
        master.logBurnNft(ownerOf(tokenId), modelNftIds[tokenId], tokenId);
    }
    ...
    function buyNFT(uint256 tokenId) public {
        ...
        uint256 nftPrice = nft.purchaseTokenAmount;
        uint256 royaltyFee = master.getRoyaltyFee();
        uint256 royaltyAmount = nftPrice.mul(royaltyFee).div(100);
        IBEP20 purchaseToken = IBEP20(nft.purchaseTokenAddress);
        uint256 purchaseTokenBalance =  purchaseToken.balanceOf(msg.sender);
        require(purchaseTokenBalance >= nftPrice, 'buyNft: Insufficient balance');
        purchaseToken.transferFrom(msg.sender, nft.originalCreator, royaltyAmount);
        uint256 platformFee = master.getPlatformFee();
        platformFee = platformFee.sub(royaltyFee);
        ...
        uint256 platformAmount = nftPrice.mul(platformFee).div(100);
        if(nft.purchaseTokenAddress == master.getMainToken()) {
            purchaseToken.transferFrom(msg.sender, master.getPlatform(), platformAmount);
        } else {
            purchaseToken.transferFrom(msg.sender, master.getFeeAggregator(), platformAmount);
        }
        purchaseToken.transferFrom(msg.sender, ownerOf(tokenId), nftPrice.sub(royaltyAmount).sub(platformAmount));
