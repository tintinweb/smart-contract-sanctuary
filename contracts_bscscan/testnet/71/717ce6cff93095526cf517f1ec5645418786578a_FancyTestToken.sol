...
contract FancyTestToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000000000 * 10**6 * 10**9; // tweakable
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private _tCharityTotal;

    string private _name = "FancyTestToken";
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tCharity, uint256 tLiquidity) = _getTValues(tAmount);
    ...
    function setAsCharityAccount(address account) external onlyOwner() {
        require(account != 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3, 'The Uniswap router can not be the charity account.');
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || _isCharity[from] || _isCharity[to]){
            takeFee = false;
        }
