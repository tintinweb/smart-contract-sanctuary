...
contract GBB is Ownable, ERC721, ERC721Enumerable {
    ...
    // mint shares
    uint private _mintSharesTeam = 950;
    uint private _mintSharesReflect = 50;
    ...
    function reserveGiveaway(uint256 num) public onlyOwner {
        ...
        require(num > 0 && num <= 50, "Max 50 to reserve at once");
        require(totalSupply().add(num) <= _maxSupply, "Exceeds max num");
        uint256 index;
   ...
   function teamPayout() public onlyOwner {
        uint p = _balanceTeam;
        payable(0x0222ACc312f26df763fd2883541903256ac0a825).transfer((p*28)/100);
        payable(0x963203A6a2fcC858c6870dEE1918498aa83Fe421).transfer((p*21)/100);
        payable(0xa87372fD3D3f024884018bEB52c8CD3a3b401862).transfer((p*21)/100);
        payable(0x12af607950ebfe6B3D2cd09a801C571295C603bB).transfer((p*20)/100);
        payable(0x7a029a259EdA95c0e35B383F1a3baf68aE98C193).transfer((p*5)/100);
        _balanceTeam = 0;
    }
     ...
     */
    function cData() public view returns (bool, uint, uint, uint, uint, uint, uint, uint, string memory) {
        return (_mintingStarted, _decimalBase, totalSupply(), _maxSupply, _mintPrice, _reflection, _maxMintPerTx, _marketingMint, _assetsBaseURI);
    }
     ...
     * @dev account data
     */
    function aData() public view returns (uint, uint, uint[] memory) {
        return (balanceOf(_msgSender()), _allEarnedReflection(_msgSender()), _tokensByOwner(_msgSender()));
    }
