...
abstract contract RewardAdder is IOnChainStakingPool {
...
contract OnChainStakingPoolWithReward is OnChainStakingPool, RewardAdder {
    ...
    constructor (string memory name_,
        ...
        owner = msg.sender;
        earlyWithdrawAllowed = earlyWithdrawAllowed_;
    }

    function changeStakingStarts(uint stakingStarts_)
    external {
        require(msg.sender == owner, "OnChainStakingPool: Not allowed");
        stakingStarts = stakingStarts_;
        super.setStakingStarts(stakingStarts_);
    }

    function changeStakingEnds(uint stakingEnds_)
    external {
        require(msg.sender == owner, "OnChainStakingPool: Not allowed");
    ...
    function changeWithdrawStarts(uint withdrawStarts_)
    external {
        require(msg.sender == owner, "OnChainStakingPool: Not allowed");
    ...
    function changeWithdrawEnds(uint withdrawEnds_)
    external {
        require(msg.sender == owner, "OnChainStakingPool: Not allowed");
    ...
    returns (bool) {
        if (block.timestamp < withdrawEnds) {
            require(earlyWithdrawAllowed, "OnChainStaking: early withdraw not allowed");
        }
...
contract OnChainStakingPool is IOnChainStakingPool {
    ...
    address public owner; // Not using Ownable to save on deployment gas
