...
contract MinistoContract is Ownable {
    ...
    string public name = "Ministo Smart Contract_alpha2";
    string public symbol = "MISCT";
    ...
    */
    ...
    function CompraCotas(uint _cotas) public payable{
        require(msg.value >= valordacota*_cotas, "Saldo insuficiente");
        require(balances[address(this)] > 0, "Nao ha cotas para negociacao via contrato.");
        
        investidores[msg.sender].cotas += _cotas;
        uint valor = (totalSupply*_cotas)/100;
        investidores[msg.sender].investBalance += valor;
        ContractTransfer(msg.sender, valor);
        ContInvest(msg.sender);
       
        //para funcionar o resgate, deve colocar payable no endereço que será enviado o valor em ETHER 
        
        if(msg.value > valordacota*_cotas){
            uint troco = msg.value - valordacota*_cotas;
        ...
        emit InvestimentoRealizado(msg.sender, _cotas);
