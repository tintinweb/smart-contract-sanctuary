...
contract elfinStaking is Ownable {
    ...
    constructor(address elfinToken, address[] memory _token, uint256[] memory _totalLimit, uint256[] memory _minStakingAmount) public payable {
        _owner  = msg.sender;
        _cmo    = msg.sender;
    ...
    function withdraw(uint userStakingIndex) external {
        ...
        if(pool.stakingToken == IERC20(address(0))) {
            if(tokenApy > 0) {
                _msgSender().transfer(info.stakingAmount.add(((info.stakingAmount).mul(tokenApy)).div(10000)));
            } else {
    ...
    function injectBnb() external payable {
    }

    function withdrawBnb(uint256 amount) external onlyCmo {
        require(address(this).balance >= amount, "Not enough bnb");
        payable(_cmo).transfer(amount);
    }
