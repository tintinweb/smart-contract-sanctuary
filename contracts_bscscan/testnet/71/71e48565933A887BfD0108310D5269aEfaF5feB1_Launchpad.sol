...
contract Launchpad is Ownable, DataStorage, Access, Events {
    ...
    function _registerBuy(address _beneficiary, uint256 amount) internal {
        ...
        uint256 amountInvest = tokenAmount.mul(priceToken).div(10**IBEP20(saleToken).decimals());
        require(msg.value == PROJECT_FEE, "Required: Must be paid fee to register");
        require(
            buyToken.allowance(_beneficiary, address(this)) >= amountInvest,
        ...
        require(
            tokenAmount >= minInvest,
            "Requried: buy token not enough"
        );   
        require(
            tokenAmount <= maxInvest,
        ...
        tokenHolders[_beneficiary] = tokenHolders[_beneficiary].add(tokenAmount);
        // update state
        wasSale = wasSale.add(tokenAmount);
        user.owner = _beneficiary;
        user.amountInvest = user.amountInvest.add(amountInvest);
        user.tokenBuy = user.tokenBuy.add(tokenAmount);
        _forwardFunds(amountInvest, _beneficiary);
        _deliverTokens(msg.sender, tokenAmount);
        emit TokenPurchase(_beneficiary, amountInvest, tokenAmount);
