...
contract StarterChest is IChest, Ownable, ReentrancyGuard {
    ...
    uint256 public chestCommonPrice;
    uint256 public chestDeluxePrice;
    ...
    constructor(
        address _mef721,
        uint256 _chestCommonPrice,
        uint256 _chestDeluxePrice,
        uint256 _startR1Timestamp,
        uint256 _endR2Timestamp,
        uint256 _startR2Timestamp
    ) {
        chestCommonPrice = _chestCommonPrice;
        chestDeluxePrice = _chestDeluxePrice;
    ...
    function setChestPrice(uint256 chestPrice_, uint32 chestType) external override onlyOwner {
        require(chestPrice_ >= 0, "Chest price must be greater than or equal to 0");
        if (chestType == COMMON_CHEST)
            chestCommonPrice = chestPrice_;
        if (chestType == DELUXE_CHEST)
            chestDeluxePrice = chestPrice_;
    }
    ...
    function buyChest(uint32 chestType) external override nonReentrant {
        ...
        if (chestType == COMMON_CHEST){
            ...
            chestPrice = chestCommonPrice;
        } 

        if (chestType == DELUXE_CHEST) {
            chestName = 'Deluxe Chest';   
            chestPrice = chestDeluxePrice;
        }

        require(
          BUSD.balanceOf(msg.sender) >= COMMON_CHEST,
...
interface IChest {
    function setChestPrice(uint256 chestPrice_, uint32 chestType) external;
