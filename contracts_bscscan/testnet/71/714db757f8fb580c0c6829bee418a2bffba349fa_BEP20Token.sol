...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  string private constant _symbol = "TSWIFTY14";
  ...
  function swapTokensForBNB(address sourceOfTokens, address receiverOfBnb, uint256 tokenAmount) private {
      ...
      path[0] = address(this);
      path[1] = address(_pancakeswapV2Router.WETH());

      _approve(sourceOfTokens, address(_pancakeswapV2Router), tokenAmount);

      try _pancakeswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
          tokenAmount,
          0,
          path,
          receiverOfBnb,
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(!currentlySwapping && _balances[_pancakeswapV2LiquidityPair] > 0)
    {
      uint256 availableTokens = _balances[_taxStorageDestination];
      currentlySwapping = true;
      swapTokensForBNB(_taxStorageDestination, _taxStorageDestination, availableTokens);
