...
contract Prediction is Owned, Pausable {
    ...
    event SetRandomService(IRandom indexed oldService, address indexed newService);
    event SetPriceService(IPrice indexed oldPriceService, address indexed newPriceService);
    event SetTokenReward(IBEP20 indexed oldTokenReward, address indexed newTokenReward);
    event SetFeeRatio(uint256 indexed oldFeeRatio, uint256 indexed newFeeRatio);
    event SetLotteryRatio(uint256 indexed oldLotteryRatio, uint256 indexed newLotteryRatio);
    event SetImprovementDelta(uint256 indexed oldImprovementDelta, uint256 indexed newImprovementDelta);
    event SetBufferTimestamp(uint256 indexed oldBufferTimestamp, uint256 indexed newBufferTimestamp);
    event SetOperator(address indexed oldOperator, address indexed newOperator);
    event SetReferralRewardPerTicket(uint256 indexed oldReward, uint256 indexed newReward);
    ...
    function setRandomService(address _random) public onlyOwner {
        emit SetRandomService(randomService, _random);
    ...
    function setPriceService(address _price) public onlyOwner {
        emit SetPriceService(priceService, _price);
    ...
    function setTokenReward(address _token) public onlyOwner {
        emit SetTokenReward(tokenReward, _token);
    ...
    function setFeeRatio(uint256 _feeRatio) public onlyOwner {
        ...
        emit SetFeeRatio(feeRatio, _feeRatio);
        feeRatio = _feeRatio; 
    }
    ...
    function setLotteryRatio(uint256 _lotteryRatio) public onlyOwner {
        ...
        emit SetLotteryRatio(lotteryRatio,  _lotteryRatio);
        lotteryRatio = _lotteryRatio; 
    }
    ...
    function setImprovementDelta(uint256 _improvementDelta) public onlyOwner {
        emit SetImprovementDelta(improvementDelta, _improvementDelta);
        improvementDelta = _improvementDelta; 
    }
    ...
    function setBufferTimestamp(uint256 _bufferTimestamp) public onlyOwner {
        emit SetBufferTimestamp(bufferTimestamp, _bufferTimestamp);
    ...
    function setOperator(address _operator) public onlyOwner {
        emit SetOperator(operatorAddress,  _operator);
        operatorAddress = _operator; 
    }
    ...
    function setReferralRewardPerTicket(uint256 _referralRewardPerTicket) public onlyOwner {
        emit SetReferralRewardPerTicket(referralRewardPerTicket, _referralRewardPerTicket);
        referralRewardPerTicket = _referralRewardPerTicket; 
    }
     ...
     * @param _pid id of pool
     */
    function executeRound(uint256 _pid) external onlyOperator poolExisted(_pid) whenNotPaused(_pid) {
        require(
    ...
    function numbersDrawn(uint256 _pid, bytes32 _requestId, uint256 _randomNumber) external onlyRandomService() {
        ...
        if (block.timestamp > round.endTime.add(bufferTimestamp)) {
            return;
        }
        require(
    ...
    function _buyTicket(uint256 _pid, bool _label, address _buyer, bool _initial, address _referral) internal returns(uint256 _price) {
        ...
        uint256 tid = uint256(tickets[msg.sender][_pid][_currentEpoch].length);
