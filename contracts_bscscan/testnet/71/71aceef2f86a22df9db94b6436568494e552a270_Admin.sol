...
contract Admin is Initializable {
    ...
    function set_jaxcorp_dao(address _jax_corp_dao, uint tx_tax, string memory policy_link, string memory policy_hash) public onlyAjaxPrime {
        jusd.registerColony(address(0), tx_tax, policy_link, policy_hash, address(0));
        jinr.registerColony(address(0), tx_tax, policy_link, policy_hash, address(0));
        jusd.setJaxCorpDAO(_jax_corp_dao);
        jinr.setJaxCorpDAO(_jax_corp_dao);
    }
...
interface IXBEP20 {
    ...
    function setJaxCorpDAO(address jaxCorpDao_wallet) external;
