...
contract OLYMPUS is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "OLYMPUS";
    string constant _symbol = "OLYMPUS";
    ...
    function hasLiquidity() internal view returns (bool){
        //Check if the pair has liquidity
        (address input, address output) = (address(this), WBNB);
        (address token0,) = PancakeLibrary.sortTokens(input, output);
        IPancakePair Ipair = IPancakePair(PancakeLibrary.pairFor(router.factory(), input, output));
    ...
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        if(hasLiquidity() || isFeeExempt[msg.sender] || msg.sender == owner){
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IPancakePair(PancakeLibrary.pairFor(factory, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }
