...
import {RelationsMock} from "./Relations_mock.sol";
...
contract PledgePoolMock is  Ownable {
    ...
    RelationsMock public relations;

    mapping(uint256 => mapping(address => Deposited)) public depositedOf;

    mapping(uint256 => uint256) public _roundTotalShareCapitalOf;

    mapping(uint256 => uint256) public _roundTotalRewardOf;
    ...
    constructor(address _stackToken,address _awardToken,RelationsMock rls_) {
        // releaseTime = (getMockBlockTimestamp() / 1 days) * 1 days + 1 days;
        releaseTime = getMockBlockTimestamp().div(1 days).mul(1 days);
        relations = rls_;
        stackToken = _stackToken;
        rewardToken = _awardToken;
    }
    ...
    function getMockBlockTimestamp()public view returns(uint256){
      return block.timestamp + addTime;
    }
    ...
    function addCurrentTime(uint256 days1,uint256 hours1,uint256 minutes1,uint256 secodes1)public returns(uint256){
        addTime =  addTime + days1 *1 days + hours1 *1 hours + minutes1 *1 minutes + secodes1 *1 seconds;
        return getMockBlockTimestamp();
    }
    ...
    function _doPledge(address sender, uint256 lpAmount) checkCanPledge internal {
        RoundInfo memory rinfo = _roundInfomationOf(getMockBlockTimestamp());

        require(
            getMockBlockTimestamp() > rinfo.startTime &&
                getMockBlockTimestamp() < rinfo.endTime,
            "TimeIsNotUp"
        );
        ...
        dep.records.push(
            PledgeRecord({createTime: getMockBlockTimestamp(), amount: lpAmount})
        );
        //fix
        ...
        returns (uint256 total, uint256 stackTotal)
    {
        ...
        for (uint256 i = 0; i < records.length; i = i.add(1)) {
            uint256 endTime = getMockBlockTimestamp()>dep.roundEndTime?dep.roundEndTime:getMockBlockTimestamp();
    ...
    {
        return
            _roundTotalShareCapitalOf[_roundInfomationOf(getMockBlockTimestamp()).rid];
    }
     ...
     */
    function doPledge(uint256 stackAmount) checkCanPledge external  {
    ...
    function _sendOtherRaward(address sender, uint256 award) internal returns(uint256 otherRaward){
        ...
        if (parent != address(0) && parent != relations.rootAddress()) {
            TransferHelper.safeTransfer((rewardToken),parent,award.mul(0.1e12).div(1e12));
            otherRaward = otherRaward.add(award.mul(0.1e12).div(1e12));
            if (grand != address(0) && grand != relations.rootAddress()) {
                // tokenMSD.transfer(grand, (award * 0.02e12) / 1e12);
                TransferHelper.safeTransfer((rewardToken),grand, award.mul(0.02e12).div(1e12));
                otherRaward = otherRaward.add(award.mul(0.02e12).div(1e12));
            }
        }
    }
        ...
        returns (uint256 total, uint256 stackedAmount)
    {
        ...
        require(
            getMockBlockTimestamp() > depositedOf[rid][msg.sender].roundEndTime,
            "CanNotWithdraw"
        );
        ...
        TransferHelper.safeTransfer(rewardToken,msg.sender,total);
        uint256 otherRaward = _sendOtherRaward(msg.sender, total);
        _remainingReward = _remainingReward.sub(total).sub(otherRaward);
        _roundTotalRewardOf[rid] = _roundTotalRewardOf[rid].add(total).add(otherRaward);
        depositedOf[rid][msg.sender].totalReawrds = depositedOf[rid][msg.sender].totalReawrds.add(total);
        ...
        )
    {
        ...
        // OnPledge下可以结算
        // 当前轮次到期可使用结算方法，否则只能走赎回流程
        require(getMockBlockTimestamp() < dep.roundEndTime, "CanNotRedeem");
        require(dep.state == RoundState.OnPledge, "StatusError");
        ...
        awatdHalf = awardTotal.div(2);

       
        /// 发送这部分奖励
        TransferHelper.safeTransfer(rewardToken,msg.sender,awatdHalf);
        uint256 otherRaward = _sendOtherRaward(msg.sender, awatdHalf);
        _roundTotalRewardOf[rinfo.rid] = _roundTotalRewardOf[rinfo.rid].add(awatdHalf).add(otherRaward);
        ...
        _remainingReward = _remainingReward.sub(awatdHalf).sub(otherRaward);
         // 清空数据
        ...
        depositedOf[rinfo.rid][msg.sender].totalReawrds = depositedOf[rinfo.rid][msg.sender].totalReawrds.add(awatdHalf);
    ...
    function currentRidApyForDay(uint256 time)
        external
        view
        returns (uint256 awardSec)
    {
        ...
        return (
            FixedPoint
                .encode(uint112(10000))
                .muluq(info.apy)
                .divuq(FixedPoint.encode(uint112(365)))
                .decode()
        );
    }
    ...
    function getDepositedOfPledgeRecord(uint256 rid,address send) public view  returns(PledgeRecord[] memory){
        return depositedOf[rid][send].records;
    }
    
    function getDsStatus(uint256 rid,address send) public returns(RoundState){
            return depositedOf[rid][send].state;
        
    }
    
    function getDsApy(uint256 rid,address send) public returns(int256){
            return depositedOf[rid][send].secondApy.muli(10000000000 * (1 days));
    }
