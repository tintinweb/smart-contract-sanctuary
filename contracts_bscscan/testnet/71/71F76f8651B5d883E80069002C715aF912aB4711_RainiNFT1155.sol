...
contract RainiNFT1155 is ERC1155, AccessControl, ReentrancyGuard {
  ...
  uint256 public rainbowToEth = 1000000000;
  uint256 public unicornToEth = 1000000;
  ...
  constructor(string memory _uri, bytes1 _contractChar, string memory _contractURIString) 
    ERC1155(_uri) {
  ...
  modifier onlyBurner() {
    require(hasRole(BURNER_ROLE, _msgSender()), "RainiNFT1155: caller is not a burner");
    _;
  }
  ...
  function setEtherValues(uint256 _unicornToEth, uint256 _rainbowToEth)
    external onlyOwner {
  ...
  function setFees(uint256 _mintingFeeBasisPoints, uint256[] memory _fees) 
    external onlyOwner {
  ...
  function merge(uint256 _cardId, uint256 _level, uint256 _mintAmount, uint256[] memory _tokenIds, uint256[] memory _burnAmounts) 
    external payable nonReentrant {
      ...
      require(_level > 0 && _cardLevel.conversionRate > 0, "RainiNFT1155: merge not allowed");
      ...
      for (uint256 i = 0; i < _tokenIds.length; i++) {
        require(_burnAmounts[i] <= balanceOf(_msgSender(), _tokenIds[i]), "RainiNFT1155: not enough balance");
        TokenVars memory _tempTokenVars =  tokenVars[_tokenIds[i]];
        require(_tempTokenVars.cardId == _cardId, "RainiNFT1155: card mismatch");
        require(_tempTokenVars.level < _level, "RainiNFT1155: can only merge into higher levels");
      ...
      require(totalPointsBurned == cost, "RainiNFT1155: wrong number of tokens burned");
  ...
  function initCards(uint256[] memory _costInUnicorns, uint256[] memory _costInRainbows, uint256[] memory _maxMintsPerAddress,  uint16[] memory _maxSupply, uint256[] memory _allocation, string[] memory _pathUri, uint32[] memory _mintTimeStart, uint16[][] memory _conversionRates, bool[][] memory _isGrouped, uint256[][] memory _maxStamina)
    external onlyOwner() {
      ...
      require(_costInUnicorns.length == _costInRainbows.length, "RainiNFT1155: arrays length not equal");
      require(_costInUnicorns.length == _maxMintsPerAddress.length, "RainiNFT1155: arrays length not equal");
      require(_costInUnicorns.length == _pathUri.length, "RainiNFT1155: arrays length not equal");
      require(_costInUnicorns.length == _maxSupply.length, "RainiNFT1155: arrays length not equal");
      require(_costInUnicorns.length == _allocation.length, "RainiNFT1155: arrays length not equal");

      uint256 _maxCardId = maxCardId;
      uint256 _maxTokenId = maxTokenId;

      for (uint256 i; i < _costInUnicorns.length; i++) {
        require(_conversionRates[i].length == _isGrouped[i].length, "RainiNFT1155: arrays length not equal");
  ...
  function _mintToken(address _to, uint256 _cardId, uint256 _cardLevel, uint256 _amount, bytes1 _mintedContractChar, uint256 _number) private {
    ...
    require(_cardLevel > 0 || cardLevel.numberMinted + _amount <= card.maxSupply, "RainiNFT1155: total supply reached.");
  ...
  function burn(uint256 _tokenId, uint256 _amount, address _owner) 
    external onlyBurner {
      require(_amount <= balanceOf(_owner, _tokenId), "RainiNFT1155: not enough balance");
  ...
  function mintWithPoints(uint256[] memory _cardId, uint256[] memory _amount, bool[] memory _useUnicorns, address[] memory _rainbowPools, address[] memory _unicornPools)
    ...
    for (uint256 i = 0; i < _cardId.length; i++) {
      ...
      require(block.timestamp >= card.mintTimeStart, "PointNft1155Swap: Card not yet mintable");
      require(cardLevel.numberMinted + _amount[i] <= card.allocation, "PointNft1155Swap: Not enough tokens in supply");
      require(numberMintedByAddress[_msgSender()][_cardId[i]] + _amount[i] <= card.maxMintsPerAddress, "PointNft1155Swap: Max mints reached for address");

      if (_useUnicorns[i]) {
        require(card.costInUnicorns > 0, "PointNft1155Swap: unicorns not allowed");
        _totalPriceUnicorns += card.costInUnicorns * _amount[i] * POINT_COST_DECIMALS;
      } else {
        require(card.costInRainbows > 0, "PointNft1155Swap: rainbows not allowed");
    ...
    for (uint256 n = 0; n < 2; n++) {
      ...
      if (totalPrice > 0) {
        ...
        require(loopLength > 0, "PointNft1155Swap: invalid pools");

        for (uint256 i = 0; i < loopLength; i++) {
          IStakingPool pool;
          if (loopTypeUnicorns) {
            require((unicornPools[_unicornPools[i]]), "PointNft1155Swap: invalid unicorn pool");
            pool = IStakingPool(_unicornPools[i]);
          } else {
            require((rainbowPools[_rainbowPools[i]]), "PointNft1155Swap: invalid rainbow pool");
        ...
        if (remainingPrice > 0) {
          ...
          require(totalPrice - remainingPrice >= minPoints, "PointNft1155Swap: not enough balance");
          uint256 pointsToEth = loopTypeUnicorns ? unicornToEth : rainbowToEth;
          require(msg.value * pointsToEth > remainingPrice, "PointNft1155Swap: not enough balance");
    ...
    // Add minting fees
    _amountEthToWithdraw += ((_totalPriceRainbows / rainbowToEth + _totalPriceUnicorns / unicornToEth) * mintingFeeBasisPoints) / 10000;
