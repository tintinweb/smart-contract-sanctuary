...
interface ISuperDeedNFT {
    ...
    function setTotalRaise(uint value) external;
}
...
contract SuperDeedNFT is ISuperDeedNFT, ERC721Enumerable {
    ...
    address private _distributor; // distributor can set asset address and distribute/release tokens
    ...
    constructor(address minter, address distributor, string memory tokenSymbol, string memory deedName) ERC721(deedName, SUPER_DEED) {
        _minter = minter;    
    ...
    function tokenURI(uint256 /*tokenId*/) public view virtual override returns (string memory) {
        return  string(abi.encodePacked(BASE_URI, asset.symbol));
    }
    ...
    function setTotalRaise(uint value) external override onlyMinter {
    ...
    function distributeTokens(uint amount) external onlyDistributor {
        ...
        _tokenReleases.push(TokenRelease(block.timestamp, amount));
        totalTokensReleased += amount;
        
        ERC20(asset.tokenAddress).safeTransferFrom(msg.sender, address(this), amount);
    }
    
    function getClaimable(uint id) public view returns (uint amount, uint indexFrom, uint indexTo) {
        
        uint len = _tokenReleases.length;
        
        NftInfo memory item = _nftInfoMap[id];
        if (item.valid && len > item.nextClaimIndex) {
    ...
    function claim(uint id) external {
        ...
        (uint amt, , uint indexTo) = getClaimable(id);
    ...
    // Otherwise, the user should claim the tokens before combining. 
