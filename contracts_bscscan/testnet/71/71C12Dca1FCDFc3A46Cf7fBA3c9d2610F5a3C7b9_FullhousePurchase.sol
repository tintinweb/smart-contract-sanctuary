...
contract FullhousePurchase is Context, ReentrancyGuard {
   ...
   */
  function createOffer(uint256[] memory _nfts, uint256 _price, PaymentMethod _kind) external whenNotPaused {
  ...
  function buyOffer(uint256 _id, address creator, bytes memory _data) external payable nonReentrant whenNotPaused {
    ...
    if(kind == PaymentMethod.BNB) {
      ...
      _makeOfferPayment(buyer, creator, seller, buyPrice, kind);
    } else {
      _makeOfferPayment(buyer, creator, seller, price, kind);
    }
    ...
    emit BuyOffer(_id, seller, buyer, price, kind);
  }

  function _makeOfferPayment(address buyer, address creator, address seller, uint256 price, PaymentMethod kind) internal {
   ...
   */
  function createAuction(uint256[] memory _nfts, uint256 _minPrice, PaymentMethod _kind) external whenNotPaused {
  ...
  function resultAuction(uint256 _auctionId, address creator, bytes memory _data) external nonReentrant {
      ...
      // Make payment
      _makeAuctionPayment(winner, creator, seller, winningBid, kind);
      // Result the auction
  ...
  function _makeAuctionPayment(address winner, address creator, address seller, uint256 price, PaymentMethod kind) private {
    uint256 maxShare = 1000;
    // Ensure there is a winner
