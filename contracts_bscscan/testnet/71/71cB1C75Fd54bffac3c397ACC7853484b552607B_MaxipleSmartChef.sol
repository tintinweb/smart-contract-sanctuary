...
contract MaxipleSmartChef is MaxipleSmartChefStateUpgradeable {
    ...
    function withdraw(uint _pid, uint _period) external _validateParams( _pid, _period) {
        UserInfo storage _user = user[msg.sender][_pid][_period];
        PoolInfo storage _pool = _PoolInfo[_pid];
        
        require(_user._amount > 0,"MaxipleSmartChef :: _user._amount must not be zero");
        
        uint _reward = viewReward( _pid, _period);
        
        variables memory _var = variables(_user._amount, (_reward == 0) ? 0 : _reward, block.timestamp, (_reward > 0 ) ? 4 : 0, 0, _user._depositTime);
        ...
        if(_var._rewardAmount > 0){
            if((_var._depositTime+_poolPeriod[_period] > _var._currentTime)) _var._feePercent = 10;
    ...
    function viewReward( uint _pid, uint _period) public view returns (uint _reward) {
        UserInfo memory _user = user[msg.sender][_pid][_period];
