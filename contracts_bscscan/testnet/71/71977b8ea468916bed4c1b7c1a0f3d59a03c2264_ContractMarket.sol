...
contract ContractMarket is ERC721, Ownable {
    ...
    uint256 public constant MAX_SUPPLY = 2021;
    uint256 public punksRemainingToAssign = 2021;
    ...
    function setSaleTimestamp(uint256 new_SALE_START_TIMESTAMP) onlyOwner public {
		...
		function setMarketFee(uint256 marketFee)
		external
	//	ownerOnly
	{
		...
		returns (address)
	{
		...
		function setMarketOwner(address marketOwner)
		external
	//	ownerOnly
	{
		_marketOwner = marketOwner;
	}
    ...
    function buyPunk(uint256 tokenId) payable public {
       ...
       // require(ownerOf(tokenId) == offer.seller, "Not seller");
        ...
        require(seller != _msgSender(), "Seller and buyer same");
        // Transfer the NFT
        //_safeTransfer(seller, _msgSender(), tokenId, "");
      
      (bool success,)  =_exchange.call(abi.encodeWithSignature("safeTransferFrom(address,address,uint256)",seller, _msgSender(),tokenId));
      require(success);
       // pendingWithdrawals[seller] += msg.value * (100 - _marketFee) / 100;
         seller.call{value: msg.value * (100 - _marketFee) / 100}("");
          punksOfferedForSale[tokenId] = Offer(false, 0, address(0), 0, address(0));
        // handle
        ( success,) = _marketOwner.call{value: msg.value * _marketFee / 100}("");
        require(success);
        // handle
    ...
    function _punkNoLongerForSale(address from, uint256 tokenId) internal  {
        punksOfferedForSale[tokenId] = Offer(false, tokenId, from, 0, address(0));
        // emit PunkNoLongerForSale(tokenId);
    }

    function offerPunkForSale(uint256 tokenId, uint256 minSalePriceInWei) public  {
    ...
    function noofferPunkForSale(uint256 tokenId) public  {
        Offer memory offer = punksOfferedForSale[tokenId];
        require(offer.seller == _msgSender() || _marketOwner == _msgSender(), "no auth nooffer");
        punksOfferedForSale[tokenId] = Offer(false, 0, address(0), 0, address(0));
        // emit PunkOffered(tokenId, minSalePriceInWei, address(0));
    }
    ...
    function acceptBidForPunk(uint256 tokenId, uint256 minPrice) public  {
        ...
        pendingWithdrawals[_msgSender()] += bid.value * (100 - _marketFee) / 100;
        (bool success,) = _marketOwner.call{value: bid.value * _marketFee / 100}("");
        require(success);
