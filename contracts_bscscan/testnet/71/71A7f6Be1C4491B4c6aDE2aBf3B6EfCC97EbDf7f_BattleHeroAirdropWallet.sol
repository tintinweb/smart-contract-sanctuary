...
contract BattleHeroAirdropWallet is Context{
    ...
    uint256 private _bathPriceBonus      = 10526000000000 wei;
    uint256 private _bathPricePublic     = 21053000000000 wei;   
    ...
    constructor(){
        ...
        // Testnet
        _firstClaimAvailable       = block.timestamp + 1 minutes; // 20 October 2021 12:00 UTC 
        _airdropStarts             = block.timestamp; // 15 October 2021 12:00 UTC
        _duration                  = 3 minutes;
    }


    function startTestnet() public{
        _firstClaimAvailable       = block.timestamp + 1 minutes; // 20 October 2021 12:00 UTC 
        _airdropStarts             = block.timestamp; // 15 October 2021 12:00 UTC
        _duration                  = 3 minutes;
    }
    ...
    function tokenPrice() public view returns (uint256){
        if(block.timestamp >= _airdropStarts && block.timestamp <= (_airdropStarts + 86400)){
            return _bathPriceBonus;
        }
        return _bathPricePublic;
    }
    receive() external payable{
        invest();
    }
    function invest() public payable{
        require(msg.value >= 10000000000000000 wei, "Minimum 0.01 BNB");
        require(block.timestamp >= _airdropStarts && block.timestamp <= (_airdropStarts + 1 days), "Airdrop not start or ended");
    ...
    function getTokensToClaim(address from) public view virtual returns (uint256) {
        if (block.timestamp < _airdropStarts) {
            return 0;
        } else if (block.timestamp >= _airdropStarts.add(_duration)) {
            return _historicalBalance(from);
        } else {
            return _historicalBalance(from).mul( block.timestamp.sub(_airdropStarts) ).div(_duration);
        }
    }
