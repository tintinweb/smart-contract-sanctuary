...
abstract contract Tokenomics is Manageable {
    ...
    string internal constant NAME = "Pandamic Pump Token GlobalSellTax";
    string internal constant SYMBOL = "PPGTX";
    ...
    // sells have increased fees of +50%//
    uint256 internal constant sellFeeIncreaseFactor = 150; 

//==================================================================================================================================================================================================================//

    /**
     * @dev Sets the maximum transaction amount allowed in a transfer.
     * 
     * 0.5% of the total supply. 
     * 
     * NOTE: in order to have an unlimited max, set
     * `maxTransactionAmount = TOTAL_SUPPLY;`
     */
    uint256 internal constant maxTransactionAmount = TOTAL_SUPPLY / 150; // 0.6% of the total supply
//==================================================================================================================================================================================================================//    
    /**
     * @dev Set the maximum allowed balance in a wallet.
     * 
     * 1.5% of the total supply. f
     * 
     ...
     */
    uint256 internal constant maxWalletBalance = TOTAL_SUPPLY / 75; // 1.3% of the total supply
//==================================================================================================================================================================================================================//    
    /**
...
//==================================================================================================================================================================================================================//
        /**
         * The RFI recipient is ignored but a valid address value is required
         * 
         * The value of fees is given in part per 1000 (based on the value of FEES_DIVISOR),
         * e.g. 2% is 20
         */ 

        _addFee(FeeType.Burn, 0, burnAddress );
...
//==================================================================================================================================================================================================================//
    ...
    function _getFeePublic(uint256 index) public view returns (FeeType, uint256, address, uint256){
        Fee memory fee = _getFeeStruct(index);
        return ( fee.name, fee.value, fee.recipient, fee.total );
    }
    
    function _changeFee(uint256 index, uint256 value) external onlyManager{
...
//==================================================================================================================================================================================================================//
    /**
     * @dev BURN CURRENTLY DISABLED: If enabled in the future;
     ...
     * 2) Tokens in the burn address cannot be sold (which in turn drain the 
     *    liquidity pool)
     *
     *
     ...
     *
     * There is an extra check placed into the `transfer` function to makes sure the
     * burn address cannot withdraw the tokens.
     */
    ...
    function _burnTokens(address sender, uint256 tBurn, uint256 rBurn) internal {
        ...
        /**
         * @dev Does not reduce _totalSupply and/or _reflectedSupply. (soft) burning by sending
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if ( isInPresale ){ takeFee = false; }
            ...
            */
            ...
            /**
            * The pair is excluded from the max wallet balance check; 
        ...
        if(takeFee) {
            uint256 Fees = amount.mul(sumOfFees).div(FEES_DIVISOR);
            
            //if sell, multiply by 1.5//
            if(_isV2Pair(recipient)){
            Fees = Fees.mul(sellFeeIncreaseFactor).div(FEES_DIVISOR);
        }
}
    ...
    function _transferTokens(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        /**
         * sum of all fees to calculates the % of the total 
    ...
    function _getCurrentSupply() internal view returns(uint256, uint256) {
        ...
        /**
         * Removes balances of addresses excluded from rewards from
         * rSupply and tSupply, which increases the % of transaction fees
     ...
     * 
     * This contract (class) takes care of ONLY handling RFI, i.e. 
     * changing the rates and updating the holder's balance (via `_redistribute`). 
     * all other fees and taxes are handled in their individual contracts (classes).
     */ 
     ...
     * This allows rfi to redistribute the fee without 
...
abstract contract Liquifier is Ownable, Manageable {
    ...
    /**
     * NOTE: passing the `contractTokenBalance` here instead of creating `balanceOfDelegate`
     */
    function liquify(uint256 contractTokenBalance, address sender) internal {
        ...
        /**
         * - first checks if the contract has collected enough tokens to swap and liquify
         * - then checks if swap and liquify is enabled
         * - then makes sure not to get caught in a circular liquidity event
         * - finally, doesn't swap & liquify if the sender is the uniswap pair
         */
    ...
    function _swapAndLiquify(uint256 amount) private lockTheSwap {
        ...
        // capture the contract's current ETH or BNB balance.
        //captures the exact amount of ETH or BNB that the
        // swap creates, and does not make the liquidity event include any ETH or BNB that
        // has been manually sent to the contract
        uint256 initialBalance = address(this).balance;
        
        // swap tokens for ETH or BNB on BSC
        _swapTokensForEth(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH or BNB did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);

        // add liquidity to uniswap or PCS on BSC
    ...
    function _swapTokensForEth(uint256 tokenAmount) private {
        ...
        // generate the uniswap or PCS pair path of token -> weth or wbnb
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        (uint256 tokenAmountSent, uint256 ethAmountSent, uint256 liquidity) = _router.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
            // Bounds the extent to which the WETH/token or WBNB/token price can go up before the transaction reverts. 
            // Must be <= amountTokenDesired; 0 = accept any amount (slippage is inevitable)
            0,
            // Bounds the extent to which the token/WETH or WBNB/token price can go up before the transaction reverts.
            // 0 = accept any amount (slippage is inevitable)
            0,
            owner(),
            block.timestamp
        ...
        /**
         * The swapAndLiquify function converts half of the contractTokenBalance native tokens to ETH or BNB. 
         * For every swapAndLiquify function call, a small amount of ETH or BNB remains in the contract. 
         * This amount grows over time with the swapAndLiquify function being called throughout the life 
         * of the contract. Below are the methods to withdraw these funds after an ample amount of time for cleanup, 
         * otherwise the residual ETH or BNB will be locked in the contract forever.
         */
    ...
    /**
    * @dev Sets the uniswapV2 or PancakeSwapV2 pair (router & factory) for swapping and liquifying tokens
    */
    ...
    /**
     * @dev this delegates instead of having to extend `BaseRfiToken` to gain access 
...
abstract contract Antiwhale is Tokenomics {
     ...
     *      values to adjust the fees total for each tx. Leaving here for probable expansion in V2 Delta.
     */
...
//////////////////////////////////////////////////////////////////////////
//==================================================================================================================================================================================================================//
abstract contract PandamicPumpToken is BaseRfiToken, Liquifier, Antiwhale {
    ...
    constructor(Env _env){
        ...
        // exclude the pair address from rewards - doesn't redistribute
    ...
    /**
     * @dev Converts the fee amount of tokens into ETH/BNB
     * and sends to the recipient's wallet. Note that this reduces liquidity so it is not currently implemented.
     * It would be required to add a % into the liquidity fee for % to take out through
     * this method upon activiation in probable V2 Delta.
     */
...
//==================================================================================================================================================================================================================//
contract PPTGlobalSellTax is PandamicPumpToken{

    constructor() PandamicPumpToken(Env.Testnet){
