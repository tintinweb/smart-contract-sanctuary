...
contract StakingRewards is IStakingRewards, Ownable ,ReentrancyGuard {
    ...
    function pendingReward(uint256 _pid, address _user) external override view returns (uint256, address) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint _poolSharedPerBlock = rewardPerBlock.mul(pool.allocPoint).div(totalAllocPoint);
            uint _apr = calcApr(pool.totalStakingAmount, _poolSharedPerBlock);
            accRewardTokenPerShare = user.amount.mul(_completeBlocks).mul(_apr).div(PRECISION);
        }
        return (accRewardTokenPerShare, pool.stakeToken);
    }

    function calcApr(uint totalStakeAmount, uint poolRewardPerBlock) public view returns(uint) {

        if(totalStakeAmount == 0) {
            return 0;
        }
        return poolRewardPerBlock.mul(PRECISION).div(totalStakeAmount); // avoid 0 result
    }
