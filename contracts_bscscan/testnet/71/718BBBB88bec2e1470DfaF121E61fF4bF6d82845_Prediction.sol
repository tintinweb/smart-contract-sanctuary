...
contract Prediction is IPrediction, Ownable, Pausable {
    ...
    function addReferral(address _user, address _referral) public onlyOwner {
        referral[_user] = _referral;
    }
    ...
    function numbersDrawn(uint256 _pid, uint256 _epoch, bytes32 _requestId, uint256 _randomNumber) external override {
        ...
        if (specifications[_pid][_epoch].lotteryWinningRequestId == _requestId) {
            _numbersDrawn(_pid, _epoch, _randomNumber);
        }
    ...
    function claim(uint256 _pid, uint256 _epoch, address _to, uint256[] memory _tids) external poolExisted(_pid) {
        ...
        achievementService.increaseTotalWin(msg.sender, _pid, _epoch, reward + lotteryReward);
    }
    ...
    function _numbersDrawn(uint256 _pid, uint256 _epoch, uint256 _randomNumber) internal {
        require(
    ...
    function _lockRound(uint256 _pid, uint256 _epoch, uint256 _price) internal {
        ...
        if (
            ...
            (bytes32 requestId, uint256 randomness, bool isWating) = 
                randomService.getRandomNumber(_pid, _epoch);
            if (isWating) {
                specifications[_pid][_epoch].lotteryWinningRequestId = requestId;
            } else {
                _numbersDrawn(_pid, _epoch, randomness);
            }
        }
    }
    ...
    function _calculateRewards(uint256 _pid, uint256 _epoch) internal {
        ...
        lotteryReward = total * lotteryRatio / oneRatio;
        // Blue wins
...
interface IAchievement {
    ...
    function increaseTotalWin(address user, uint256 pid, uint256 epoch, uint256 amount) external returns(uint256);
...
interface IRandom {
    function getRandomNumber(uint256 pid, uint256 epoch) external returns(bytes32 requestId, uint256 randomness, bool isWating);
}
