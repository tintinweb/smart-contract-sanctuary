...
contract NFT is Ownable, ERC721Token, ERC721Holder {
    using Strings for string;
    using Integers for uint;

    function NFT () ERC721Token("NFT" ,"NFT") public {
    ...
    function purchaseNFTWithBMA(uint256 _tokenId, uint256 _amount) public {
        uint256 priceItemEther = tokenIdToSellingItem[_tokenId].price;
        address sellerAddress = tokenIdToSellingItem[_tokenId].seller;
        bool resaleT = tokenIdToSellingItem[_tokenId].resale;
        ...
        if(resaleT){
          require(addressVBMACredit.transferFrom(msg.sender, sellerAddress, priceItemEther), "Transfer error.");
        }else{
          require(addressBMAToken.transferFrom(msg.sender, artistWallet, priceItemEther.mul(artistTax).div(100)), "Transfer error.");
          require(addressBMAToken.transferFrom(msg.sender, teamWallet, priceItemEther.mul(teamTax).div(100)), "Transfer error.");
          require(addressBMAToken.transferFrom(msg.sender, treasuryWallet, priceItemEther.mul(treasuryTax).div(100)), "Transfer error.");
          require(addressBMAToken.transferFrom(msg.sender, taxWallet, priceItemEther.mul(taxTax).div(100)), "Transfer error.");
          require(addressBMAToken.transferFrom(msg.sender, BMAWallet, priceItemEther.mul(BMATax).div(100)), "Transfer error.");
        }

        // cashback
        require(addressVBMACredit.mint(msg.sender, priceItemEther.mul(taxCashBack).div(100)), "Transfer cashback error.");
