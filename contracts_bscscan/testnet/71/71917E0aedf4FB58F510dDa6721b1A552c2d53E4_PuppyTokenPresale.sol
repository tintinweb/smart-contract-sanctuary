...
// File: contracts/PuppyTokenPresale.sol
...
contract PuppyTokenPresale is Crowdsale, AllowanceCrowdsale, CapperRole {

    using SafeMath for uint256;

    // Track investor contributions
    uint256 public investorMaxCap = 3000000000000000000;// 3

    mapping(address => uint256) private _contributions;
    mapping(address => uint256) private _caps;

    constructor(uint256 rate, address payable wallet, IERC20 token, address tokenWallet, uint256 _investorMaxCapInWei)
        AllowanceCrowdsale(tokenWallet)
        Crowdsale(rate, wallet, token)
    public {
        investorMaxCap = _investorMaxCapInWei;
    }
    ...
    */
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal view {
        super._preValidatePurchase(beneficiary, weiAmount);
        // solhint-disable-next-line max-line-length
        require(_contributions[beneficiary].add(weiAmount) <= getCap(beneficiary), "PuppyTokenPresale: beneficiary's cap exceeded");
    }
