...
contract Rubix is Context, IBEP20, Ownable {
  using SafeMath for uint256;
   ...
   event TransferToLottery(address indexed receiver, uint256 value);
   event emergencyWithdrawn(address indexed user, uint256 value);
    ...
    mapping(uint => PoolData) public _PoolData;
    
    mapping(uint => uint256) public _Lottery;
  ...
  uint private index;
  uint internal poolDuration = 100; //3888000
  uint internal depositFee = 50000 * 10**8;
  uint internal lotteryTicketFee = 100000 * 10**8;
  ...
  function changeLotteryAddress(address _newAddress) public onlyOwner {
  ...
  function Deposit(uint256 _amount) external {
        ...
        _balances[address(1)] = _balances[address(1)].add(_amount.add(_amount.mul(5000).div(100000)));
        
        //Send fee to the lottery
        _balances[lotteryAddress] = _balances[lotteryAddress].add(depositFee);
        ...
        //Adds Pool Rewards
        _PoolData[currentPool].PoolRewards = _PoolData[currentPool].PoolRewards.add(_amount.mul(5000).div(100000));
 ...
 function _poolShare(address _stakeholder) public view returns(uint256) {
  ...
  function emergencyWithdraw() public {
      ...
      uint256 withdrawFee = _stakingBalance[msg.sender].mul(1450).div(100000);
  ...
  function estimatedApr(uint256 _amount) public view returns(uint) {
      ...
      uint _aprFromDeposit = _amount.mul(9000).div(100000);
  ...
  function OldRbxbalanceOf(address _user) public view returns(uint256) {
     return _oldRBX.balanceOf(_user).div(1e10).mul(1000);
  }
  
  function claimNewRbx(address _user) public {
      require(msg.sender == _user, "Not holder");
      require(metadataOldRbx[_user] != false, "New token already claimed");
      require(vestingTimestamp < now, "You can't claim yet!");
      
      //18 decimals to 1
      uint convertedBalance = OldRbxbalanceOf(_user).div(1e10);
      
      uint256 bonusForLocking = convertedBalance.mul(3).div(100);
