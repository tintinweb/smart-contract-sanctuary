...
library SafeBEP20 {
    ...
    function safeTransferFrom(
        ...
        _callOptionalReturn(token, abi.encodeWithSelector(token.transferFrom.selector, from, to, value), "safeTransferForm");
    }
    ...
    /**
     * @dev Same as {xref-SafeBEP20-_callOptionalReturn-IBEP20-bytes-}[`_callOptionalReturn`], but with
     * `errorMessage` as a fallback revert reason when `target` reverts.
     */
    function _callOptionalReturn(IBEP20 token, bytes memory data, string memory errorMessage) private {
        // We need to perform a low level call here, to bypass Solidity's return data size checking mechanism, since
        // we're implementing it ourselves. We use {Address.functionCall} to perform this call, which verifies that
        // the target address contains contract code and also asserts for success in the low-level call.

        bytes memory returndata = address(token).functionCall(data, string(abi.encodePacked("SafeBEP20: ", errorMessage)));
        if (returndata.length > 0) {
            // Return data is optional
            // solhint-disable-next-line max-line-length
            require(abi.decode(returndata, (bool)), "SafeBEP20: BEP20 operation did not succeed");
