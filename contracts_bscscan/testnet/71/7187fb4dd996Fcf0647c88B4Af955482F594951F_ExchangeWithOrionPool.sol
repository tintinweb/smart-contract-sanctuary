...
contract ExchangeWithOrionPool is Exchange {
    ...
    {
        /*if(token != address(0) && !orionpoolAllowances[_orionpoolRouter][token])
        {
            IERC20(token).safeIncreaseAllowance(_orionpoolRouter, type(uint).max);
            orionpoolAllowances[_orionpoolRouter][token] = true;
        }*/
        require(token != address(0), 'Cannot approve on 0 address');
        uint256 currentAllowance = IERC20(token).allowance(address(this), _orionpoolRouter);
        if (currentAllowance < type(uint).max) {
            IERC20(token).approve(_orionpoolRouter, type(uint).max);
        }
    ...
    {
        require(msg.sender == _orionpoolRouter || msg.sender == _ethWrapper, "NPF");
    }
    ...
    /* function fillThroughOrionPoolApprove(
        ...
        if (order.buySide == 1) {
            require(_isRetailUser(order.senderAddress, order.quoteAsset), 'E1');
        }
        else {
            require(_isRetailUser(order.senderAddress, order.baseAsset), 'E1');
        }
        _fillThroughOrionPool(order, filledAmount, blockchainFee, path, true);
     }*/
    ...
    function fillThroughOrionPool(
        ...
        //_fillThroughOrionPool(order, filledAmount, blockchainFee, path, false);
        if (order.buySide == 1) {
            require(_isRetailUser(order.senderAddress, order.quoteAsset), 'E1');
        }
        else {
    ...
    function _fillThroughOrionPool(
            LibValidator.Order memory order,
            uint112 filledAmount,
            uint64 blockchainFee,
            address[] calldata path,
        ...
        if(order.buySide==1){ /* NOTE BUY ORDER ************************************************************/
            ...
            if (approve) {
                _completeDeficit(order.quoteAsset, int192(_amountQuote), order.senderAddress);
            }
            ...
            returns(uint[] memory amounts)
            {
                ...
                require(_filledPrice<= order.price, "EX");
                exec_data.filledPrice = uint64(_filledPrice);
                updateOrderBalance(order, filledAmount, exec_data.amountQuote, kBuy, approve);
            }
            catch(bytes memory)
            {
                filledAmount = 0;
                ...
                _payMatcherFee(
                    order.senderAddress, order.matcherFeeAsset, order.matcherAddress, order.matcherFee, approve
                );
            }

            require(checkPosition(order.senderAddress), "E3G");
            ...
            if (approve) {
                _completeDeficit(order.baseAsset, filledAmount, order.senderAddress);
            }
            ...
            returns (uint[] memory amounts)
            {
                ...
                updateOrderBalance(order, filledAmount, exec_data.amountQuote, kSell, approve);
            }
            catch(bytes memory)
            {
                filledAmount = 0;
...
library LibValidator {
    ...
    //TODO add hash explanations
    bytes32 public constant DOMAIN_SALT = 0xf2d857f4a3edcb9b78b4d503bfe733db1e3f6cdc2b7971ee739626c97e86a557;
    bytes32 public constant EIP712_DOMAIN_TYPEHASH = 0xa604fff5a27d5951f334ccda7abff3286a8af29caeeb196a6f2b40a1dce7612b;
    bytes32 public constant ORDER_TYPEHASH = 0xb5132db62dfceb466f2f8aee7a039db36a99772e5a9771d28388a5f9baad7c54;
    bytes32 public constant DOMAIN_SEPARATOR = 0x77a8aca299fc6e8f5f7e7ebdca28484843025e9725fce77f4ae0f6bdb8146cc6;
...
contract Exchange is OrionVault, ReentrancyGuard {
    ...
    address constant _ethWrapper = 0x23eE96bEaAB62abE126AA192e677c52bB7d274F0;
    ...
    */
     ...
     */
    function _isRetailUser(address user, address asset) internal returns (bool isRetail) {
        isRetail = assetBalances[user][asset] >= 0;
    }

    function _payMatcherFee(address user, address feeAsset, address matcher, int192 feeAmount, bool retail) internal {
        int192 feeBalance = assetBalances[user][feeAsset];

        if (retail) {
            _completeDeficit(feeAsset, feeAmount, user);
            assetBalances[user][feeAsset] -= feeAmount;
            assetBalances[matcher][feeAsset] += feeAmount;
        }
        else {
            bool feeInLiabilities = feeBalance < 0;
            assetBalances[user][feeAsset] -= feeAmount;
            int192 feeBalanceAfter = assetBalances[user][feeAsset];
            bool enteringLiability = feeBalanceAfter < 0;

            if(!feeInLiabilities && enteringLiability) {
                setLiability(user, feeAsset, feeBalanceAfter);
            }
    ...
    function fillOrders(
        ...
        //_fillOrders(buyOrder, sellOrder, filledPrice, filledAmount, false);
        require(_isRetailUser(buyOrder.senderAddress, buyOrder.quoteAsset), 'E1');
        require(_isRetailUser(sellOrder.senderAddress, sellOrder.quoteAsset), 'E1');
        _fillOrders(buyOrder, sellOrder, filledPrice, filledAmount, true);
    }


/*    function fillOrdersApprove(
        LibValidator.Order memory buyOrder,
        LibValidator.Order memory sellOrder,
        uint64 filledPrice,
        uint112 filledAmount
    ...
    function _fillOrders(
        ...
        if (approve) {
            _completeDeficit(buyOrder.quoteAsset, amountQuote, buyOrder.senderAddress);
            _completeDeficit(sellOrder.baseAsset, filledAmount, sellOrder.senderAddress);
        }

        //Prepay trade if the balance is not enough and the user is retail

        // Update User's balances
        updateOrderBalance(buyOrder, filledAmount, amountQuote, kBuy | kCorrectMatcherFeeByOrderAmount, approve);
        updateOrderBalance(sellOrder, filledAmount, amountQuote, kSell | kCorrectMatcherFeeByOrderAmount, approve);
        require(checkPosition(buyOrder.senderAddress), "E3G");
        require(checkPosition(sellOrder.senderAddress), "E3G");
    ...
    /**
     * @dev Function to complete the deficit required in approve method trades
     */
    function _completeDeficit(address asset, int192 desiredAmount, address user) internal {
        int192 balance = assetBalances[user][asset];
        ...
        int192 deficit = desiredAmount - balance;
        assetBalances[user][asset] += deficit;
        int112 deficitInBase = LibUnitConverter.decimalToBaseUnit(asset, uint(deficit));
        _transferFrom(asset, user, uint(deficitInBase));
    }

    /**
    * @dev Required to adjust for special case, when custom wrapped ethereum is used
    */
    function _transferFrom(address asset, address from, uint256 amount) internal {
        if (asset == address(0)) {
            IERC20(_ethWrapper).transferFrom(from, address(this), amount);
        }
        else {
    ...
    function updateOrderBalance(
        ...
        if(filledAmount > 0)
        {
            ...
            //Retail liability protection is implied, because the transferFrom covers the trade
            if(!(firstBalance<0) && (temp<0) && !approve){
        ...
        // User pay for fees
       _payMatcherFee(user, order.matcherFeeAsset, order.matcherAddress, order.matcherFee, approve);
    }
    ...
    /* Error Codes
        E1: Insufficient Balance, flavor S - stake, L - liabilities
