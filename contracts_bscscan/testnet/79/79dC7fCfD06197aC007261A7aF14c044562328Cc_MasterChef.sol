...
// MasterChef is the master of BullDoge. He can make BullDoge and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once BULLDOGE is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BULLDOGE
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBullDogePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBullDogePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. BULLDOGE to distribute per block.
        uint256 lastRewardBlock;    // Last block number that BULLDOGE distribution occurs.
        uint256 accBullDogePerShare; // Accumulated BULLDOGE per share, times 1e12. See below.
    ...
    // BULLDOGE tokens created per block.
    uint256 public bullDogePerBlock;
    uint256 public constant MAX_BULLDOGE_PER_BLOCK = 2000 * 10 ** 9;
    // Bonus multiplier for early bulldoge makers.
    ...
    // The block number when BULLDOGE mining starts.
    uint256 public startBlock;
    // Total locked up rewards
    uint256 public totalLockedUpRewards;
    // Total BULLDOGE in BULLDOGE Pools (can be multiple pools)
    uint256 public totalBullDogeInPools = 0;
    ...
    constructor(
        ...
        bullDoge = _bullDoge;
        bullDogePerBlock = _bullDogePerBlock;
    ...
    //actual Bulldoge left in MasterChef can be used in rewards, must excluding all in bulldoge pools
    //this function is for safety check only not used anywhere
    function remainRewards() external view returns (uint256) {
        return bullDoge.balanceOf(address(this)).sub(totalBullDogeInPools);
    }
    ...
    // Update the given pool's BULLDOGE allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending BULLDOGE on frontend.
    function pendingBullDoge(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBullDogePerShare = pool.accBullDogePerShare;
        uint256 lpSupply = pool.totalLp;

        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 bullDogeReward = multiplier.mul(bullDogePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accBullDogePerShare = accBullDogePerShare.add(bullDogeReward.mul(1e12).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accBullDogePerShare).div(1e12).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest BULLDOGE.
    ...
    function massUpdatePools() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            pool.accBullDogePerShare = pool.accBullDogePerShare.add(bullDogeReward.mul(1e12).div(pool.totalLp));
            pool.lastRewardBlock = block.number;

            totalReward.add(bullDogeReward.div(10));
        }

        safeBullDogeTransfer(devAddress, totalReward);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        safeBullDogeTransfer(devAddress, bullDogeReward.div(10));
    }

    // Deposit LP tokens to MasterChef for BULLDOGE allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        payOrLockupPendingBullDoge(_pid);
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                if (address(pool.lpToken) == address(bullDoge)) {
                    totalBullDogeInPools = totalBullDogeInPools.add(_amount).sub(depositFee);
                }
            } else {
                ...
                if (address(pool.lpToken) == address(bullDoge)) {
                    totalBullDogeInPools = totalBullDogeInPools.add(_amount);
                }
            }
        }
        user.rewardDebt = user.amount.mul(pool.accBullDogePerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(bullDoge)) {
                totalBullDogeInPools = totalBullDogeInPools.sub(_amount);
            }
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if (address(pool.lpToken) == address(bullDoge)) {
            totalBullDogeInPools = totalBullDogeInPools.sub(amount);
        }
    ...
    // Pay or lockup pending BULLDOGE.
    function payOrLockupPendingBullDoge(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accBullDogePerShare).div(1e12).sub(user.rewardDebt);
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeBullDogeTransfer(msg.sender, totalRewards);
            }
        } else if (pending > 0) {
    ...
    // Safe bulldoge transfer function, just in case if rounding error causes pool do not have enough BULLDOGE.
    function safeBullDogeTransfer(address _to, uint256 _amount) internal {
        if (bullDoge.balanceOf(address(this)) > totalBullDogeInPools) {
            //bullDogeBal = total bulldoge in MasterChef - total bulldoge in bulldoge pools, this will make sure that MasterChef never transfer rewards from deposited bulldoge pools
            uint256 bullDogeBal = bullDoge.balanceOf(address(this)).sub(totalBullDogeInPools);
            if (_amount >= bullDogeBal) {
                bullDoge.transfer(_to, bullDogeBal);
            } else if (_amount > 0) {
                bullDoge.transfer(_to, _amount);
            }
        }
    }
    ...
    function updateEmissionRate(uint256 _bullDogePerBlock) public onlyOperator {
        require(_bullDogePerBlock <= MAX_BULLDOGE_PER_BLOCK, "BULLDOGE per block too high");
        massUpdatePools();

        emit EmissionRateUpdated(msg.sender, bullDogePerBlock, _bullDogePerBlock);
        bullDogePerBlock = _bullDogePerBlock;
    }
