...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "FullTest3";
    string internal constant SYMBOL = "FT3";
    ...
    // maxTransactionAmount = 1.4% total supply
    uint256 internal constant maxTransactionAmount = (TOTAL_SUPPLY * 14) / 10**3;
    // maxWalletBalance = 2% total supply
    uint256 internal constant maxWalletBalance = (TOTAL_SUPPLY * 18) / 10**3;
    ...
    enum FeeType { Liquidity, Rfi, MarketingBNB }
    struct Fee {
        FeeType name;
        uint256 value;
        address recipient;
    ...
    function _addFees() private {
        ...
        _addFee(FeeType.MarketingBNB, 20, address(this) );
    }
...
abstract contract Liquifier is Ownable, Manageable {
    ...
    function _swapAndLiquify(uint256 amount) private lockTheSwap {
        ...
        // sub marketing cut from amount to swap
        // fee has to be manually set because liq & marketing fees come in as one.
        uint256 marketingTokens = amount.mul(20).div(10**2);
        uint256 liquidityTokens = amount.sub(marketingTokens);
        
        // split the contract balance into halves
        uint256 tokensForLiquidity = liquidityTokens.div(2);
        uint256 bnbForLiquidity = liquidityTokens.sub(tokensForLiquidity);
        
        //add marketing cut back for swap
        uint256 tokensToSwap = bnbForLiquidity.add(marketingTokens);
        ...
        // swap tokens for ETH
        _swapTokensForEth(tokensToSwap); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);

        // remove marketing cut from total and swap the liquidity amounts
        uint256 liquidityBNB = newBalance.mul(bnbForLiquidity).div(tokensToSwap);
        
        // add liquidity to pancakeswap
        _addLiquidity(tokensForLiquidity, liquidityBNB);
        
        emit SwapAndLiquify(tokensForLiquidity, newBalance, bnbForLiquidity);
    }
    ...
    *    safety in the event pancakeswap v3 code breaks swaps in the same way v1 to v2 broke safemoon.
    *    
...
abstract contract Linker is Tokenomics {

    function _getLinkedFees(uint256, uint256) internal view returns (uint256){
        return sumOfFees;
    }
}

abstract contract FullTestBuild is BaseRfiToken, Liquifier, Linker {
    ...
    function _getSumOfFees(address sender, uint256 amount) internal view override returns (uint256){ 
        return _getLinkedFees(balanceOf(sender), amount); 
    }
...
contract FullTest is FullTestBuild{

    constructor() FullTestBuild(Env.Testnet){
