 ...
 *
 * #TODO:
 * - RescueBNB (rip Burn pioneer) -- Still not sure if this is even possible..
 * - Fix Liquidity and burn(?)
 * - 
 * - Seperate max TX for buy and sell -- This could be done rather easily
 * - Make the setMaxTx so we can use decimals (.5%) -- Maybe jeet this from another contract
 * -----------------------------------------------------------------------
 * - Dynamic fees, more fee when selling in < x amount of time
 * - Can we check our own price?
 */
...
contract COIN is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    // Max TX in tokens
    uint256 public _maxSellTxAmount = 2 * 10**9;
    uint256 public _maxBuyTxAmount = 3 * 10**9;
