...
contract TTHStake is Owned , ReentrancyGuard {
    ...
    function calculateEarnings(address _stakeholder, uint256 _sidRandomer) public view returns(uint) {
        uint activeDays = (block.timestamp.sub(getStaking(_stakeholder,_sidRandomer))).div(600);
    ...
    function withdrawEarnings() external returns (uint success) {
        ...
        // uint remainder = (block.timestamp.sub(lastClock[msg.sender])).mod(600);
        //
        // lastClock[msg.sender] = block.timestamp.sub(remainder);
    ...
    function getStaking(address _addressr,uint256 _idRandomer) public view returns(uint){
      require(_addressr != msg.sender, 'Must be same User.');
        for(uint i = 0;i < Staker[_addressr].length;i++){
          if(_idRandomer == Staker[_addressr][i].idRandomer){
            return Staker[_addressr][i].rewardClocks;
          }
        }
    }
    ...
    function calcStakingAll(address _addressr) public view returns(uint){
      uint dd = 0;
        for(uint i = 0;i < Staker[_addressr].length;i++){
          uint activeDays = (block.timestamp.sub(Staker[_addressr][i].rewardClocks)).div(600);
          dd = dd + ((Staker[_addressr][i].totalStakeds).mul(dailyROI).mul(activeDays)).div(10000);
        }
        return dd;
    }
