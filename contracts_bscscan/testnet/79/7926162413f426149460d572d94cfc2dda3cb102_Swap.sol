...
contract Swap is Ownable {
    ...
    event SwitchState(address indexed _account, bool _oneSwapSwitchState, bool _twoSwapSwitchState, bool _threeSwapSwitchState);
    ...
    function joinSwap(uint256 _payUsdtAmount,uint256 _swapId) public returns (bool) {
        ...
        cuseTokenContract.safeTransfer(address(msg.sender),swapCuseAmount);// Transfer cuse to user address
        ...
        swapOrders[swapNowJoinTotalCount] = SwapOrder(swapNowJoinTotalCount,msg.sender,block.timestamp,_swapId,_payUsdtAmount,swapCuseAmount);// add swapOrders
    ...
    function setSwapSwitchState(bool _oneSwapSwitchState,bool _twoSwapSwitchState,bool _threeSwapSwitchState) public onlyOwner returns (bool) {
        oneSwapSwitchState = _oneSwapSwitchState;
        twoSwapSwitchState = _twoSwapSwitchState;
        threeSwapSwitchState = _threeSwapSwitchState;
        emit SwitchState(msg.sender, _oneSwapSwitchState,_twoSwapSwitchState,_threeSwapSwitchState);
        return true;
    }
}