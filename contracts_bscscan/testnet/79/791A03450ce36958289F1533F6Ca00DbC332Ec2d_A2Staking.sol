...
contract A2Staking is Context, Ownable {
    ...
    event UnstakeIntention(address indexed token, uint256 indexed pool, address indexed beneficiary, uint256 stake, uint256 unstakeAmount);
    ...
    struct Stake {
        ...
        uint256 unstakeIntentionAmount;
    }
     ...
     */
    function unstakeIntention(uint256 poolId, uint256 stakeId, uint256 amount) external {
        unstakeIntentionInternal(_msgSender(), poolId, stakeId, amount);
    }
    ...
    function stakeInternal(address beneficiary, uint256 poolId, uint256 amount) internal {
        ...
        userStakes.push(Stake({
            ...
            unstakeIntentionAmount:0
        }));
        pool.tvl = pool.tvl.add(amount);
        emit Staked(address(pool.token), poolId, beneficiary, stakeId, amount, pool.duration);
    }

    function unstakeIntentionInternal(address beneficiary, uint256 poolId, uint256 stakeId, uint256 amount) internal {
        Pool storage pool = pools[poolId];
        require(address(pool.token) != address(0), "pool not found");
        Stake[] storage userStakes = stakes[beneficiary][poolId];
        ...
        require(amount <= stakee.amount, "unstake amount too high");
        if(amount == 0) {
            // Clear unstake intention
            stakee.unstakeIntentionTimestamp = 0;
        } else if(stakee.unstakeIntentionAmount == 0 || stakee.unstakeIntentionAmount < amount) {
            //Update timestamp only if unstake was not set or amount is higher than previous intention
            stakee.unstakeIntentionTimestamp = block.timestamp;
        }
        stakee.unstakeIntentionAmount = amount;
        emit UnstakeIntention(address(pool.token), poolId, beneficiary, stakeId, amount);
    }
    ...
    function adminUnstakeExactStake(address beneficiary, uint256 poolId, uint256 stakeId, uint256 unstakeAmount) external onlyOwner {
        ...
        stakee.unstakeIntentionAmount = 0;
        stakee.unstakeIntentionTimestamp = 0;
    ...
    returns(uint256 userAmount, uint256 penaltyAmount) {
        ...
        if(pool.unstakeCliff > 0) {
            if(!ignoreUnstake) {
                require(stakee.unstakeIntentionAmount >= amount, "amount is higher than intention");
