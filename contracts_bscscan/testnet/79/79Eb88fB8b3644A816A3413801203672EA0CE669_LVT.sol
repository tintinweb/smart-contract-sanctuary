...
contract LVT is ERC20, Ownable, PaymentSplitter {
    ...
    function sellTokensForEth() external {
        ...
        require(
            ...
            "Can't sell token if you don't have node"
        );
        require(
            getNodesLastSells() + getSellTime() <= block.timestamp, 
            "Can sell once by day"
        );
        if (swapLiquify) {
            uint256 feeAmount;
            if (cashoutFee > 0) {
        ...
        swapAndSendToFee(distributionPool, rewardAmount);
        nodeRewardManager._sellTokensInAllNodes(sender);
    }
    ...
    function getNodesLastSells() public view returns (uint256) {
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        require(
            nodeRewardManager._isNodeOwner(_msgSender()),
            "NO NODE OWNER"
        );
        return nodeRewardManager._getNodesLastSellTime(_msgSender());
    }
