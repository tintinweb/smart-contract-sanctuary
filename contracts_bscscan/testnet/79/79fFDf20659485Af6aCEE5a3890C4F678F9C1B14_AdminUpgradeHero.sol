...
contract AdminUpgradeHero is AccessControl {
    ...
    // conditionOfCommon(listHeroId[0])
    function checkCommon(uint256 tokenId ) internal view returns(bool){
        uint256 tierMain = queryNumberTier(heroesNFT.getHeroesNumber(tokenId).tier);
        uint256 tierBasicMain = queryNumberTier(heroesNFT.getHeroesNumber(tokenId).tierBasic);
        // if(tierBasicMain == 0){
        //     require(tierMain <= 4, "Tier common max level");
        // }
        if(tierBasicMain == 0 && tierMain >= 4){
            return false;
        }else{
            return true;
        }
    }
    function upgradeHero(uint256[] memory listHeroId) public {
        uint256 heroesTierMain = listHeroId[0];  
        require(checkCommon(heroesTierMain), "Tier common max level");   
