...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SNYMPH }

    address public immutable NYMPH;
    ...
    address public sNYMPH;
    uint public sNYMPHQueue; // Delays change to sNYMPH address
    ...
    /**
        @notice allow approved address to deposit an asset for NYMPH
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint NYMPH needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( NYMPH ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn NYMPH for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        INymphERC20( NYMPH ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sNYMPH ).balanceOf( msg.sender ); // Can only borrow against sNYMPH held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with NYMPH
        @param _amount uint
     */
    function repayDebtWithNYMPH( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        INymphERC20( NYMPH ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, NYMPH, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( NYMPH ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( NYMPH ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns NYMPH valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match NYMPH decimals
            value_ = _amount.mul( 10 ** IERC20( NYMPH ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SNYMPH ) { // 9
            sNYMPHQueue = block.number.add( blocksNeededForQueue );
        } else return false;
