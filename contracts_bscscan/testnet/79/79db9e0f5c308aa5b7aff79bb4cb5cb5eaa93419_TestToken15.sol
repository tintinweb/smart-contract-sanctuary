...
contract TestToken15 is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 500000000 * 10**9; // fixed total supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string public constant name     = "TestToken15";
    string public constant symbol   = "TT15";
    uint8  public constant decimals = 9;
    
    // default transfer fees
    uint256 public _taxFeeTransfer       = 100; 
    uint256 public _liquidityFeeTransfer = 400;
    uint256 public _marketingFeeTransfer = 200; 

    // default buy fees
    uint256 public _taxFeeBuy       = 100; 
    uint256 public _liquidityFeeBuy = 400; 
    uint256 public _marketingFeeBuy = 200; 

    // default sell fees
    uint256 public _taxFeeSell       = 100; 
    uint256 public _liquidityFeeSell = 400; 
    uint256 public _marketingFeeSell = 200; 

    uint256 public _maxTxAmount     = _tTotal / 2;
    uint256 public _minTokenBalance = _tTotal / 2000;
	
	// We're a legit project, so
	// We guarantee dev will never be able to set total fees more than 30%
	uint256 public _maxTaxFee = 3000;
    ...
    function setTransferFee(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner {
		// Dev can't change tax fee more than _maxTaxFee
		require(taxFee + liquidityFee + marketingFee <= _maxTaxFee, "Tax fee must less than maxTaxFee");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
		// default fees are transfer fees
        ...
        } // otherwise, it uses Transfer fees
