...
contract Staking is Ownable, ReentrancyGuard {
	...
	address private shibacashTokenAddress;
	...
	constructor(address _leashAddress) public {
	...
	function setShibacashTokenAddress(address tokenAddress) public onlyAuthorizedCallers {
		shibacashTokenAddress = tokenAddress;
	}
	...
	function setPancakePair(address wallet) public onlyAuthorizedCallers {
		pancakePair = wallet;
	}
	...
	function _getLeashSupply() private view returns (uint256) {
		...
		uint balance = leash.balanceOf(0x000000000000000000000000000000000000dEaD);
		if (balance < supply) {
			supply = supply.sub(balance);
		}
		balance = leash.balanceOf(leashAddress); 
		if (balance < supply) {
			supply = supply.sub(balance);
		}
		balance = leash.balanceOf(pancakePair); 
		if (balance < supply) {
			supply = supply.sub(balance);
		}
		return supply;
...
contract LeashCash is Context, IBEP20, Ownable, ReentrancyGuard {
	...
	uint256 private _maxPriceImpact = 20000; // 2% max price impact
	...
	constructor (
		...
		staking = new Staking(address(this));
	...
	function canSellNow(address account) public view returns (bool) {
		return _nextAllowedTransferToPancake[account] <= block.timestamp && _nextAllowedTransferToPancakePriceImpact[account] <= _maxPriceImpact;
	}
	...
	function getMaxPriceImpact() public view returns (uint256) {
	...
	function getMinTokenNumberToSell() public view returns (uint256) {
		return _computeAmountFromPriceImpact(_maxPriceImpactForSwapAndLiquify);
	}
	...
	function setMaxPriceImpact(uint256 maxPriceImpact) public onlyAuthorizedCallers {
		_maxPriceImpact = maxPriceImpact;
	}
	...
	function _computePriceImpact(uint256 amount) private view returns (uint256) {
	...
	function _computeAmountFromPriceImpact(uint256 priceImpact) private view returns (uint256) {
		uint256 startBnbs = IBEP20(addressWBNB).balanceOf(address(pancakePair));
	...
	function _transfer(
		...
		uint256 priceImpact = _computePriceImpact(amount);
		if (_maxPriceImpact > 0 && !_isExcludedFromMaxTx[from] && !_isExcludedFromMaxTx[to]) {
			require(priceImpact <= _maxPriceImpact,"Price impact too high for selling !");
		}
		...
		if (limitSellByTimeUnit && oneSellEveryX != 0 && !_isSellWhiteListed[from] && _applyFeeFor[to]) {
			if (_nextAllowedTransferToPancake[from] != 0) {
				if (_nextAllowedTransferToPancake[from] > block.timestamp) {
					require(_nextAllowedTransferToPancakePriceImpact[from].add(priceImpact) < _maxPriceImpact, "Error: One sell transaction every oneSellEveryX time for max price impact");
	...
	function swapAndLiquify(address from, address to) private {
		...
		uint256 _maxToSell = _computeAmountFromPriceImpact(_maxPriceImpactForSwapAndLiquify);
		if (_maxToSell == 0) {
			return;
		}
	...
	function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
		...
		if (_marketingBNB >= _minBNBToSendToMarketingWallet) {
			_sendBNBToMarketingWallet(_marketingBNB);
			_marketingBNB = 0;
		}
	...
	function activateContract() public onlyOwner {
		...
		staking.setShibacashTokenAddress(shibacashTokenAddress);
		staking.setAuthorizedCallers(address(this),true);
		staking.setAuthorizedCallers(owner(),true);
		staking.setMarketingWallet(marketingWallet);
		staking.poolEnabled(true);
		staking.setPancakePair(address(pancakePair));
