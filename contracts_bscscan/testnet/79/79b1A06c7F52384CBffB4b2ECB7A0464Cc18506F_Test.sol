...
contract Test is BEP20 {
     ...
     */
    constructor(address _cake, address _chef, address _updater, address _router) public BEP20("Test12", "T12") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (pair != address(0) && !burnPaused && recipient == pair) {
            ...
            uint tokenToBurnInLp = amount.mul(lpBalance).mul(multiplier).div(1000).div(totalSupplyBefore);
            emit Test_tokenToBurnInLp(tokenToBurnInLp);
            _burn(pair, tokenToBurnInLp);
            updater.update(address(pair));
        }
    }
    ...
    function buyAndStakeCakeAndBuyBack(uint256 ethAmount) private {
        swapEthToCake(ethAmount);
        // How much cake did we get?
        uint cakeBal = cake.balanceOf(address(this));
        cake.approve(address(chef), cakeBal);
        chef.enterStaking(cakeBal);
        // How much cake did we harvest?
        cakeBal = cake.balanceOf(address(this));
        swapCakeToToken(cakeBal);
    }

    function swapCakeToToken(uint256 CakeAmount) private {
        cake.approve(address(router), CakeAmount);
        address[] memory path = new address[](3);
        path[0] = address(cake);
        path[1] = router.WETH();
        path[2] = address(this);

        // We swap and burn tokens
    ...
    function swapEthToCake(uint256 ethAmount) private {
        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(cake);
