...
interface ILakeBundleMarketplace {
...
/**
 * @notice Lake auction contract for NFTs
 */
contract LakeAuction is Ownable, ReentrancyGuard {
    ...
    constructor(address payable _treasury) public {
        require(
            _treasury != address(0),
            "LakeAuction: Invalid Platform treasury"
        );
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    ILakeTokenRegistry(addressRegistry.tokenRegistry()).enabled(
                        _payToken
                    )),
            "invalid pay token"
        );
        ...
        nonReentrant
    {
        ...
        if (winningBid > auction.reservePrice) {
            ...
            if (auction.payToken == address(0)) {
                // Send platform fee
                (bool platformTransferSuccess, ) = treasury.call{
        ...
        ILakeMarketplace marketplace = ILakeMarketplace(
        ...
        ILakeBundleMarketplace(addressRegistry.bundleMarketplace())
