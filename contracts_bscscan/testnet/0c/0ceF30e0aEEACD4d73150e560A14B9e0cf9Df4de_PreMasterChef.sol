...
// CDzToken with Governance.
contract CDzToken is BEP20('CDzExchange Token', 'CDZ') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "CDZ::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "CDZ::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "CDZ::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "CDZ::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying CDZs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "CDZ::_writeCheckpoint: block number exceeds 32 bits");
...
contract PreMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // @notice The total of CDZ Token by PreMasterChef rewarded.
    ...
    // @notice Withdraw LP Tokens from MasterChef and harvest the reward.
    // @param `_pid` The index of pool.
    // @param `_amount` LP Token amount to withdraw.
