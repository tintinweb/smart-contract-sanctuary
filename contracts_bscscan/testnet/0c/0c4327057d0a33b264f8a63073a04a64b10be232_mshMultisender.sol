...
// File: contracts/mshMultisender.sol
...
contract mshMultisender is Ownable
{
    ...
    modifier hasFee(uint256 ethToDistribute) {
        if (fee > 0) {
            require(
                ...
                "mshMultisender: No fee");
        }
        _;
    }

    modifier validLists(uint256 _contributorsLength, uint256 _balancesLength) {
        require(_contributorsLength > 0, "mshMultisender: No contributors sent");
        require(
            _contributorsLength == _balancesLength,
            "mshMultisender: Different arrays lengths"
        );
        _;
    }
        ...
        onlyOwner
    {
        require(
            isInitialized == false,
            "mshMultisender: Already initialized"
        );
    ...
    function multisendToken(
        ...
        require(
            total >= 0,
            "mshMultisender: Wrong amount of total"
        );
