...
contract MobileChainLeague is Context, IERC20, Ownable {
    ...
    uint256 public bagihasil     = 0;
    uint256 private _previousholder = bagihasil;
    
    uint256 public likuiditas = 0;
    uint256 private _previousLiquidity = likuiditas;

    uint256 public bakarMCL      = 0;
    uint256 private _previousbakarMCL = bakarMCL;

    uint256 public ekosistem   = 0;
    address public dompetekosistem = 0xe21dA53539B4bC9bE832FBA039B6c71C14E5A334;
    uint256 private _previousecosystem = ekosistem;
    ...
    uint256 public maksimaltransaksi = 50000000 * 10**18;
    ...
    function tidaktermasukdalambagihasil(address account) public view returns (bool) {
        return _isExcluded[account];
    }

    function totalbiaya() public view returns (uint256) {
        return _tFeeTotal;
    }
    ...
    function tidaktermasukbiaya(address account) public view returns(bool) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        if(_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]){
            ...
            require(amount <= maksimaltransaksi, "Transfer amount exceeds the maxTxAmount.");
        }
        
        //Hitung jumlah luka bakar dan jumlah Amal Bakar
        uint256 burnAmt = amount.mul(bakarMCL).div(100);
        uint256 BurnCharityAmt = amount.mul(ekosistem).div(100);
        ...
        //Hapus sementara biaya transfer ke alamat pembakaran dan dompet ekosistem
        bagihasil = 0;
        likuiditas = 0;

        //transfer untuk membakar dan dompet ekosistem
        _transferStandard(sender, address(0), burnAmt);
        _transferStandard(sender, dompetekosistem, BurnCharityAmt);

        //pulihkan biaya pajak dan likuiditas
        bagihasil = _previousholder;
        likuiditas = _previousLiquidity;
    ...
    function enableAllFees() external onlyOwner() {
        bagihasil       = 2;
        _previousholder = bagihasil;
        likuiditas = 2;
        _previousLiquidity = likuiditas;
        ekosistem   = 8;
        _previousecosystem = ekosistem;
        bakarMCL      = 0;
        _previousbakarMCL = bagihasil;
        inSwapAndLiquify = true;
    ...
    function setdompetekosistem(address newWallet) external onlyOwner() {
        dompetekosistem = newWallet;
    }
   
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        require(maxTxPercent > 10, "Cannot set transaction amount less than 10 percent!");
        maksimaltransaksi = _tTotal.mul(maxTxPercent).div(
            10**2
        );
    }
