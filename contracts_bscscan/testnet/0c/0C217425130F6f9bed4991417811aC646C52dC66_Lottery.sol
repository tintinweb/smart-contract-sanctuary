...
contract LotteryNFT is ERC721("Winery Lottery Ticket", "WLT"), Ownable {
    ...
    function multiClaimReward(uint256[] memory _tokenRewardIds) external onlyOwner {
        for (uint i = 0; i < _tokenRewardIds.length; i++) {
            claimInfo[_tokenRewardIds[i]] = true;
        }
    }
...
contract Lottery is LotteryOwnable, Initializable {
    ...
    event Reset(uint256 indexed issueIndex, uint256 unclaimableRewards, uint256 burnAmount);
    ...
    function initialize(
        ...
        rolloverAllocation = 15;
        _setAllocation(50, 12, 8);
        initOwner(_owner);
    }
    ...
    function reset() external onlyAdmin {
        ...
        // Calculate unclaimable rewards from previous issueIndex
        for (uint i = 0; i < allocation.length; i++) {
            if (getMatchingRewardAmount(issueIndex-1, 4 - i) == 0) {
                rolloverAmount = rolloverAmount.add((previousRewards.mul(allocation[i])).div(100));
            }
        }
        // Calculate roll over balance 
        rolloverAmount = rolloverAmount.add((previousRewards.mul(rolloverAllocation)).div(100));

        if(rolloverAmount > 0) {
            internalBuy(rolloverAmount, nullTicket);
        }

        uint256 toBurn = getTotalRewards(issueIndex-1).mul(burnAllocation()).div(100);
        winery.transfer(burnAddress, toBurn);

        emit Reset(issueIndex, rolloverAmount, toBurn);
    }
    ...
    function approveTransfer(uint256 _price) public  returns(bool) {
        winery.approve(address(this), _price);
        return true;
    }
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external returns(uint256){
        require(!drawed(), 'drawed, can not buy now');
        ...
        winery.transferFrom(address(msg.sender),address(this), _price);
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        winery.transfer(address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            winery.transfer(address(msg.sender), reward);
        }
    ...
    function  multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            winery.transfer(address(msg.sender), totalReward);
        }
    ...
    function _setAllocation(uint8 _allocation1, uint8 _allocation2, uint8 _allocation3) internal {
        uint8 totalAllocation = uint8(_allocation1.add(_allocation2.add(_allocation3).add(rolloverAllocation)));
