...
import "./IDragonStakingRewards.sol";
import "./Domain.sol";
...
contract DragonFarming is IERC721Receiver, IDragonStakingRewards, Ownable ,ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //   1. The pool's `accRewardPerShare` (and `accRewardPerShare`) gets updated.
    ...
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256[] tokenIds);
    ...
    function pendingReward(uint256 _pid, address _user) external override view returns (uint256, address) {
        ...
        accRewardPerShare = user.amount.mul(accRewardPerShare).div(PRECISION).sub(user.rewardDebt);
        return (accRewardPerShare, pool.stakeToken);
    ...
    function deposit(uint256 _pid, uint256[] calldata _tokenIds) nonReentrant isActive public override {
        address _for = msg.sender;
        uint numberOfTokens = _tokenIds.length;
        require(numberOfTokens > 0, "_tokenIds: at least one");
        PoolInfo storage pool = poolInfo[_pid];
        ...
        user.amount = user.amount.add(numberOfTokens);
        pool.totalStakingAmount = pool.totalStakingAmount.add(numberOfTokens);
        user.lastBlock = block.number;
        for(uint i = 0; i < _tokenIds.length; i++) {
            user.tokenIds.push(_tokenIds[i]);
            erc721SafeTransferFrom(IERC721(pool.stakeToken), msg.sender, address(this), _tokenIds[i]);
        }
    ...
    function _withdraw(address _for, uint256 _pid, uint256 _amount) internal {
        ...
        for (uint i = 0; i < _amount; i++) {
            uint tokenId = user.tokenIds[user.tokenIds.length -1];
            erc721SafeTransferFrom(token, address(this), address(msg.sender), tokenId);
            tokenIds[i] = tokenId;
            user.tokenIds.pop();
        }
    ...
    function _harvest(address _to, uint256 _pid) internal {
        ...
        require(pending <= IERC20(rewardToken).balanceOf(address(this)), "Not enough reward token");
        safeTransfer(_to, pending);
    }
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        pool.totalStakingAmount = pool.totalStakingAmount.sub(tokenIds.length);

        emit EmergencyWithdraw(msg.sender, _pid, tokenIds);
    }
    ...
    function getStakeTokens(uint _pid, address _account) override public view returns(address token, uint[] memory tokenIds) {
        return (poolInfo[_pid].stakeToken,  userInfo[_pid][_account].tokenIds);
    }
}
...
interface IDragonStakingRewards {
    ...
    function getStakeTokens(uint _pid, address _account) external view returns(address token, uint[] memory tokenIds);
}
