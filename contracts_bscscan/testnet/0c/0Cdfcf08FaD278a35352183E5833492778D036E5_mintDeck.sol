 ...
 function _addBlock(   
        ...
        // ) public returns (uint256 initTime, uint256 newTime ){      
             ...
             if(LockCard[account][id].time > 0 ){
                initTime = (LockCard[account][id].time - refDay) / 1 minutes; //mro
                newTime = (block.timestamp - refDay) / 1 minutes; 
...
contract mintDeck is ERC1155, Ownable {
    ...
    constructor() ERC1155("https://lasthorde.com/NFTs/{id}.json") {
          poolIde = 0;         
          createIde( "Skeleton");
          mint( msg.sender,"Skeleton",10);
          createIde( "Bruja");
          mint( msg.sender,"Bruja",10);
          createIde( "Orco");
          mint( msg.sender,"Orco",10);
        }
      
      function mint( address account, string memory id, uint256 amount) public  onlyOwner  {   
           if(ide[id] > 0 ){
       ...
       function viewDeck1(address account)  public view returns( string memory deck)  {
             ...
             initTime = 0;
             newTime = 0;
             message = string (abi.encodePacked(message,'{', '"', "cartas", '"',':', '['));
             newTime = (block.timestamp - refDay)/ 1 minutes; 
            for (uint i = 1; i <= poolIde; i++) {    
                 ...
                 if( balanceOf(account,i) > 0){
                   ...
                   if (LockCard[account][i].time > 0){
                     initTime = (LockCard[account][i].time - refDay) / 1 minutes; 
                     
                      if ( newTime  <= initTime   ){   //mro
                   ...
                   message = string (abi.encodePacked(message,'{','"', "nombre",'"',':', '"',ideName[i],'"',',','"',"cantidad",'"',':', '"', value,'"',',','"',"idLock",'"',':', '"' ,idLock,'"' ,'}' ));                                   
                  newline = 1;
                }                   
               }     
               message = string (abi.encodePacked(message,']','}')); 
