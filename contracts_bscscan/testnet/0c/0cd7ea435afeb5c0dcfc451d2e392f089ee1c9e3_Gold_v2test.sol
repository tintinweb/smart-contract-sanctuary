...
contract Gold_v2test is Initializable, AccessControlUpgradeable, ERC20Upgradeable {
     ...
     * @dev _amount has to include 18 decimals
     */
    function mintWithBurn(address _to, uint _amount) external onlyRole(MINTER_ROLE) onlyRole(BURNER_ROLE) {
        require(totalSupply() + _amount <= MAX_SUPPLY, "Gold: max supply reached");

        uint burnAmount = _amount / 2;
        _mint(_to, _amount);
