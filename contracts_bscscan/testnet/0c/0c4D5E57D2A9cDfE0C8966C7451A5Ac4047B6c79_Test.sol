...
contract Test is ERC20, Ownable, Reentrancy {
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if (automatedMarketMakerPairs[from]){
                require((IERC20(address(this)).balanceOf(to).add(amount) <= (HolderAmountCap * (10**6) * (10**18))), "Test: transfer amount exceeds address limit");
            }
            super._transfer(payable (from), payable(address(this)), fees);
        }

        super._transfer(payable(from), payable(to), amount);
    ...
    function swapAndLiquify(uint256 tokens) private {
       // split the contract balance into halves
        uint256 half = tokens.mul(50).div(100);
    ...
    function withdrawBep20(address token) public onlyOwner nonReentrant{
        ...
        IERC20(address(token)).safeTransfer(payable(_marketingWalletAddress),IERC20(address(token)).balanceOf(address(this)));
}
