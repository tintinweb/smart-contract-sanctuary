...
import "@passive-income/wyvern-v3/contracts/registry/ProxyRegistry.sol";
import "@passive-income/wyvern-v3/contracts/registry/AuthenticatedProxy.sol";

contract PSIProxyRegistry is ProxyRegistry {
    string public constant name = "PSI NaaS Wyvern Protocol Proxy Registry";
     ...
     * @param authAddresses Addresses of the contract to grant authentication
     */
    function grantInitialAuthentication(address[] calldata authAddresses) public onlyOwner {
        require(
            !initialAddressSet,
            "Proxy Registry initial address already set"
        );
        initialAddressSet = true;
        for (uint256 idx = 0; idx < authAddresses.length; idx++) {
            contracts[authAddresses[idx]] = true;
        }
    }
}
