...
contract WhaleNetworkGovernance is Context, Ownable {
    ...
    function getVote(uint256 proposalId)public view returns(Vote){
        require( _proposalVotingStatus[proposalId][_msgSender()], "You have not voted on this proposal");
        for(uint256 i =0; i< _votes[proposalId].length; i++){
            if(_votes[proposalId][i].voter == _msgSender()){
