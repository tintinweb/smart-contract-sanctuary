...
contract BisharesUniswapRouterMinter is BMath {
  ...
  function swapExactETHForTokensAndMint(
    ...
    require(path[0] == weth, 'BisharesUniswapRouterMinter: INVALID_PATH');
  ...
  function swapETHForTokensAndMintExact(
    ...
    require(amounts[0] <= msg.value, 'BisharesUniswapRouterMinter: EXCESSIVE_INPUT_AMOUNT');
  ...
  function swapTokensForTokensAndMintExact(
    ...
    uint amountOut = _tokenInGivenPoolOut(indexPool, swapTokenOut, poolAmountOut);
    uint[] memory amounts = UniswapV2Library.getAmountsIn(factory, amountOut, path);
    require(amounts[0] <= amountInMax, 'BisharesUniswapRouterMinter: EXCESSIVE_INPUT_AMOUNT');
    TransferHelper.safeTransferFrom(
      path[0], msg.sender, UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]
    );
    _swap(amounts, path);
