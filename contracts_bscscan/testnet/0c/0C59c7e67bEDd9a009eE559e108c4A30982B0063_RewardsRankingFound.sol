...
contract RewardsRankingFound is Ownable { 
    ...
    uint256 public lastDailyRewardDistributionTimestamp;
    uint256 public lastWeeklyRewardDistributionTimestamp;
    ...
    function _updateRewards() private {
        address reserve = gameAddresses.getReserveChallengeAddress();
        if(IReserveForChalengeRewards(reserve).getRewardFinished()){
            reserveFinished = true;
            return;
        } else if(block.timestamp >= IReserveForChalengeRewards(reserve).getNextUpdateTimestamp()){
             IReserveForChalengeRewards(reserve).updateRewards();
        } else {
            return;
        }
    }

    function getDailyRewards() external onlyGame returns(uint256){
        require(block.timestamp >= lastDailyRewardDistributionTimestamp.add(1 days));
        lastDailyRewardDistributionTimestamp = block.timestamp;
        _updateRewards();
        uint256 _toSend = BZAI.balanceOf(address(this)).div(1000);
        require(BZAI.transfer(msg.sender,_toSend));
        return _toSend;
    }

    function getWeeklyRewards() external onlyGame returns(uint256){
        require(block.timestamp >= lastWeeklyRewardDistributionTimestamp.add(7 days));
        lastWeeklyRewardDistributionTimestamp = block.timestamp;
        _updateRewards();
        uint256 _toSend = BZAI.balanceOf(address(this)).div(1000);
        require(BZAI.transfer(msg.sender,_toSend));
        return _toSend;
    ...
    function getFightingWinRewards() external onlyGame returns(uint256){
