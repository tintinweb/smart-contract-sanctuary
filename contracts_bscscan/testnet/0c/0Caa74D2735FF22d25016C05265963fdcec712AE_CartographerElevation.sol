...
import "./ICartographerElevation.sol";
import "hardhat/console.sol";

contract CartographerElevation is Ownable, Initializable, ReentrancyGuard, ICartographerElevation {
    ...
    mapping(address => mapping(uint8 => uint8)) public override userTotem;
    ...
    function initialize(address _IElevationHelper) external initializer onlyCartographer() override {
        require(_IElevationHelper != address(0), "Contract is zero");
        elevationHelper = IElevationHelper(_IElevationHelper);
    }
    ...
    modifier validTotem(uint8 elevation, uint8 totem) {
        require(totem < elevationHelper.getElevationTotemCount(elevation), "Invalid totem");
        _;
    }
    ...
    function add(uint8 _pid, bool _live, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate, uint8 _elevation, uint256 _cakeChefPid) external onlyCartographer() nonDuplicated(_lpToken, _elevation) override {
        if (_withUpdate) {
        ...
        elevationPoolInfo[_pid] = ElevationPoolInfo({
            ...
            totemLpSupplies : new uint256[](elevationHelper.getElevationTotemCount(_elevation)),
            roundRewards : 0,
            totemRoundRewards : new uint256[](elevationHelper.getElevationTotemCount(_elevation)),
    ...
    function set(uint8 _pid, bool _live, uint16 _depositFeeBP, bool _withUpdate) public onlyCartographer() poolExists(_pid) override {
    ...
    function rolloverAllRoundsAtElevation(uint8 _elevation) external onlyCartographer() override {
        uint8[] memory poolsAtElev = poolsAtElevation[_elevation];
        uint256 currRound = elevationHelper.getElevationRound(_elevation);
    ...
    function endRoundAndStartNext(uint8 _pid, uint256 currRound, uint256 nextRound) internal poolExistsAndEnabled(_pid) {
        ...
        uint256 winningTotemRoundRewards = pool.totemRoundRewards[elevationHelper.getRoundWinningTotem(pool.elevation, currRound)];
        ...
        pool.roundRewards = 0;
        pool.totemRoundRewards = new uint256[](elevationHelper.getElevationTotemCount(pool.elevation));
    }
    ...
    function getPendingAndVestingSummit(uint8 _pid, address _userAdd) external view onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) override returns (uint256, uint256, uint256, uint256) {
        ElevationPoolInfo storage pool = elevationPoolInfo[_pid];
    ...
    function getUserHypotheticalReward(ElevationPoolInfo memory pool, UserInfo storage user, uint256 accSummitPerShare) internal view returns (uint256) {
        uint256 currRound = elevationHelper.getElevationRound(pool.elevation);
    ...
    function getHypotheticalSummit(uint8 _pid, address _userAdd) external view poolExists(_pid) validUserAdd(_userAdd) override returns (uint256, uint256) {
        ...
        return (
            ...
            rew.mul(liveRoundRewards).div(liveTotemRoundRewards)
        );
    }
    ...
    function getRoundWinnings(ElevationPoolInfo storage pool, UserInfo storage user, uint256 roundIndex, uint256 prevRoundIndex, address userAdd) internal view returns (uint256) {
        if (userTotem[userAdd][pool.elevation] != elevationHelper.getRoundWinningTotem(pool.elevation, roundIndex)) { return 0; }
    ...
    function getElevationPendingSummit(ElevationPoolInfo storage pool, UserInfo storage user, address userAdd) internal view poolExists(pool.pid) returns (uint256) {
        ...
        if (user.staked == 0 || user.prevInteractedRound == currRound) { return avail; }
    ...
    function updateUserRoundInteraction(ElevationPoolInfo storage pool, UserInfo storage user, uint256 amount, bool isDeposit) internal {
        // Deposit Only
        // Initialize round interaction
        uint256 currRound = elevationHelper.getElevationRound(pool.elevation);
        if (isDeposit && user.staked == 0) {
            user.roundRew = 0;
            user.staked = amount;
            user.roundDebt = amount.mul(pool.accSummitPerShare).div(1e12);
        ...
        if (user.prevInteractedRound == currRound && user.staked > 0) {
            uint256 staked = user.staked;
            user.roundRew = user.roundRew.add(staked.mul(pool.accSummitPerShare).div(1e12).sub(user.roundDebt));
    ...
    function switchAllTotems(uint8 _elevation, uint8 _newTotem, address _userAdd) external nonReentrant onlyCartographer() validTotem(_elevation, _newTotem) validUserAdd(_userAdd) elevationNotLockedUntilRollover(_elevation) override {
    ...
    function getIsTotemInUse(uint8 _elevation, address _userAdd) external view override returns (bool) {
    ...
    function deposit(uint8 _pid, uint256 _amount, uint8 _totem, address _userAdd) external nonReentrant onlyCartographer() poolExistsAndEnabled(_pid) validTotem(elevationPoolInfo[_pid].elevation, _totem) validUserAdd(_userAdd) elevationNotLockedUntilRollover(elevationPoolInfo[_pid].elevation) override returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
        ElevationPoolInfo storage pool = elevationPoolInfo[_pid];
    ...
    function switchElevationValidate(uint8 _pid, address _lpToken, uint256 _cakeChefPid, uint8 _totem, address _userAdd) external nonReentrant onlyCartographer() poolExistsAndEnabled(_pid) validTotem(elevationPoolInfo[_pid].elevation, _totem) elevationNotLockedUntilRollover(elevationPoolInfo[_pid].elevation) override {
    ...
    function switchElevationDeposit(uint8 _pid, uint256 _amount, uint8 _totem, address _userAdd) external nonReentrant onlyCartographer() override returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
        ElevationPoolInfo storage pool = elevationPoolInfo[_pid];
    ...
    function withdraw(uint8 _pid, uint256 _amount, address _userAdd) external nonReentrant onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) elevationNotLockedUntilRollover(elevationPoolInfo[_pid].elevation) override returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
    ...
    function switchElevationWithdraw(uint8 _pid, uint256 _amount, address _userAdd) external nonReentrant onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) elevationNotLockedUntilRollover(elevationPoolInfo[_pid].elevation) override returns (address, uint256, uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];       
    ...
    function emergencyWithdraw(uint8 _pid, address _userAdd) external nonReentrant onlyCartographer() poolExistsAndEnabled(_pid) validUserAdd(_userAdd) elevationNotLockedUntilRollover(elevationPoolInfo[_pid].elevation) override returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
    ...
    function totemRoundRewards(uint8 _pid) public view poolExists(_pid) returns (uint256[11] memory) {
        ElevationPoolInfo storage pool = elevationPoolInfo[_pid];
        uint8 totemCount = elevationHelper.getElevationTotemCount(pool.elevation);
        uint256 summitReward = cartographer.getSummitReward(pool.lastRewardBlock, pool.lpToken, pool.elevation);
...
interface ICartographerElevation {
    function userTotem(address userAdd, uint8 elevation) external view returns (uint8);
    function getIsTotemInUse(uint8 _elevation, address _userAdd) external view returns (bool);
    function initialize(address _IElevationHelper) external;
    
    function add(uint8 _pid, bool _live, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate, uint8 _elevation, uint256 _cakeChefPid) external;
    function set(uint8 _pid, bool _live, uint16 _depositFeeBP, bool _withUpdate) external;
    function rolloverAllRoundsAtElevation(uint8 _elevation) external;
    function getPendingAndVestingSummit(uint8 _pid, address _userAdd) external view returns (uint256, uint256, uint256, uint256);
    ...
    function getHypotheticalSummit(uint8 _pid, address _userAdd) external view returns (uint256, uint256);
    function massUpdatePools() external;
    function switchAllTotems(uint8 _elevation, uint8 _newTotem, address _userAdd) external;
    function deposit(uint8 _pid, uint256 _amount, uint8 _totem, address _userAdd) external returns (uint256);
    ...
    function switchElevationValidate(uint8 _pid, address _lpToken, uint256 _cakeChefPid, uint8 _totem, address _userAdd) external;
    function switchElevationDeposit(uint8 _newPid, uint256 _amount, uint8 _totem, address _userAdd) external returns (uint256);
    function withdraw(uint8 _pid, uint256 _amount, address _userAdd) external returns (uint256);
    function emergencyWithdraw(uint8 _pid, address _userAdd) external returns (uint256);
}
