...
contract WORK is ERC721Holder,Ownable{
    ...
    IERC721 public nftContract = IERC721(0xDF2a6948f3eb8ac292d6810f697Efd5e9A20BAca);
    IERC20 public gheAddress = IERC20(0x8333Fe622Be1669aeFe182F382Da441400Dfa7cB);
	IERC20  public ghcAddress = IERC20(0xCB829819787e48D0eBb272DA7424AeB33d91731c);
	...
	constructor() {
		...
		// Odd job
		_setWorkForCondition(1,2,0,0,0,0,0,0,0,roleRestA);
		_setWorkForReward(1,25 * 10 ** 4 * 10**18,0,0,0,10000);
		
		// Lumberjack
		_setWorkForCondition(2,2,0,86,0,61,0,0,0,roleRestB);
		_setWorkForReward(2, 25 * 10 ** 4 * 10**18,1,85,15 * 10 **4 *10**18,10000);
		
		//Winemaker
		_setWorkForCondition(3,2,0,0,0,0,86,0,61,roleRestC);
		_setWorkForReward(3, 25 * 10 ** 4 * 10**18,4,85,15 * 10 **4 *10**18,10000);
		
		//hunter
		_setWorkForCondition(4,2,0,61,0,86,0,0,0,roleRestD);
		_setWorkForReward(4, 25 * 10 ** 4 * 10**18,3,85,15 * 10 **4 *10**18,10000);
		
		//Scroll Transcript
		_setWorkForCondition(5,2,0,0,61,0,0,0,86,roleRestE);
		_setWorkForReward(5, 25 * 10 ** 4 * 10**18,6,85,15 * 10 **4 *10**18,10000);
		
		//Legendary Realm
		_setWorkForCondition(6,2,0,0,86,0,61,0,0,roleRestF);
		_setWorkForReward(6, 25 * 10 ** 4 * 10**18,2,85,15 * 10 **4 *10**18,10000);
	...
	function removeByIndex(address _user, uint256 _workId, uint256 index) internal{
        ...
        userWork[_user][_workId][index] = 0;
    ...
    function withdrawal(uint256 _nftId) public {
        ...
        gheAddress.transfer(msg.sender,allSalary); 
        userMap[msg.sender][_nftId].lastblockNum = block.number;
	...
	function userNft(address _user, uint256 _workId)public view returns(NftInfo[] memory){
		
		uint256 length  = userWork[_user][_workId].length;
    ...
    function CrossTransfer(address token, address beneficiary) public onlyOwner {
    ...
    function CrossTransferNft(IERC721 token, address beneficiary, uint256 nftId) public onlyOwner {
