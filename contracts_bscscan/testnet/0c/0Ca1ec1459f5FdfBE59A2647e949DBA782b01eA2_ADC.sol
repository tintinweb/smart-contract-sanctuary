...
contract ADCReward is RewardHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() RewardHolders("ADC_Reward_Tracker", "ADC_Reward_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "ADC_Reward_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "ADC_Reward_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ADC contract.");
    }
...
uint256 public  marketFee = 3;
 ...
 address public lp;
  ...
  constructor() ERC20("Ada Cake", "ADC") {
...
emit RebaseUpdate(epoch, _totalSupply);
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "ADC: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "ADC: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "ADC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ADC: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTADC = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTADC);
...
addLiquidity(tokensTADC, bnbForLiquidity);
