...
contract DecubateCrowdfunding is IDecubateFactory, Ownable, ReentrancyGuard {
  ...
  function addDCBAgreement(
    ...
    require(_endDate > block.timestamp, "Agreement end date passed");
   ...
   *
   */
  function joinDCBAgreement(uint256 _investFund)
    external
    override
    nonReentrant
    returns (bool)
  {
    ...
    require(dcbAgreement.active, "Agreement does not exist");
    ...
    require(
      dcbAgreement.token.balanceOf(address(msg.sender)) >= _investFund,
      "Not enough funds"
    );
    ...
    if (_useBrackets && dcbAgreement.startDate.add(4 hours) < block.timestamp) {
      ...
      (bool flag, , ) = _bracketAlloc.getTierOfUser(msg.sender);
    ...
    dcbAgreement.totalInvestFund += _investFund;
...
interface IDecubateFactory {
  ...
  function joinDCBAgreement(uint256 _investFund) external returns (bool);
