...
BUSD Farmer by Moonfarms

~ PLANS:
  plan 1 3%  200 days 600% ROI
  plan 2 7%   40 days 280% ROI
  plan 3 6.5%   60 days  390% ROI
  plan 4 6%   80 days  480% ROI
...
~ ANTI-WHALE CONTROL FEATURES:
  ...
  Max withdraw amount of 12000 BUSD per day.
...
contract busdavaxmix {
	...
	uint256 public MAX_WITHDRAW = 12000 ether;
	...
	uint256 public LOTTERY_TICKET_PRICE = 10 * 1e18; /** 10 BUSD **/
	...
	constructor(address ownerAddress, address payable projectAddress, address payable marketingAddress, address payable communityAddress) {
        ...
        plans.push(Plan(200, 30, 10 * 1e18 ether, 0, 0, 0, 0, true)); // 10 * 1e18 10
        plans.push(Plan(40,  70, 10 * 1e18 ether, 0, 0, 0, 0, true));
        plans.push(Plan(60,  65, 10 * 1e18 ether, 0, 0, 0, 0, true));
        plans.push(Plan(80,  60, 10 * 1e18 ether, 0, 0, 0, 0, true));
	}
    ...
    function ADD_NEW_PLAN(uint256 time, uint256 percent, uint256 mininvest, bool planActivated) external {
        require(msg.sender == contractOwner, "Admin use only");
