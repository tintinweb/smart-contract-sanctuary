...
contract KTest_Liquidity_Provider is Context, Ownable{
    ...
    uint256 public _feeMarketing = 1500; // marketing fee 15%

    uint256 public _feeDev = 500; // dev fee 5%

    uint256 public _feeKodaBnbLiquidity = 7000; // BNB to koda liquidity fee 70%

    uint256 public _feeKapexBNBLiquidity = 0; // BNB to Kapex liquidity fee 10%

    uint256 public _feeKodaStakingPool = 0; // koda staking pool fee 0%

    uint256 private _feeTotalBNBDeductable = _feeMarketing.add(_feeKodaBnbLiquidity).add(_feeKapexBNBLiquidity).add(_feeDev).add(_feeKodaStakingPool).add(_feeBurn); // all liquidity + dev + marketing fee on each transaction

    address payable public _devAddress                  = payable(0xdE2379768295879F71d6886EFCD9c5A580884C26); // TO-DO dev Address
    address payable public _kodaTokenAddress            = payable(0x0530d9BC784248D29d7809629b21aDD0301c8D3D); // TO-DO koda token Address
    address payable public _marketingAddress            = payable(0xdE2379768295879F71d6886EFCD9c5A580884C26); // TO-DO Marketing Address
    address payable public _kapexTokenAddress           = payable(0xF1610b0224657Ce827Be377c60e800eb18A72837); // TO-DO kapex token Address
    address payable public _lpTokensLockAddress         = payable(0xdd304b63303302B5904f84f3324aBB6d1d3eA010); // TO-DO to lock lp tokens 
    address payable public _kodaStakingPoolAddress      = payable(0xc262B612BB9fc186713d31874688A4E93B1df6b1); // TO-DO koda staking pool address
    address payable public _stakingRewardTokenAddress   = payable(0xF1610b0224657Ce827Be377c60e800eb18A72837); // TO-DO address of token that will be rewarded for staking koda
    address payable constant public _burnAddress        = payable(0x000000000000000000000000000000000000dEaD); // Burn Address

    IBEP20 private kodaToken = IBEP20(_kodaTokenAddress);
    IBEP20 private kapexToken = IBEP20(_kapexTokenAddress);
    IBEP20 private stakingRewardToken = IBEP20(_stakingRewardTokenAddress);
    ...
    bool public isStakingRewardInBNB = false; // to check if to send BNB to staking pool or tokens
    ...
    constructor () {
        ISummitSwapRouter02 _summitSwapRouter = ISummitSwapRouter02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    ...
    function setMarketingFeePercent(uint256 Fee) external onlyOwner {
        _feeMarketing = Fee;
        _feeTotalBNBDeductable = _feeKodaBnbLiquidity.add(_feeKapexBNBLiquidity).add(_feeMarketing).add(_feeDev).add(_feeKodaStakingPool).add(_feeBurn);
    }
    ...
    function setLpTokensLockAddress(address payable lpTokensAddress) external onlyOwner {
        _lpTokensLockAddress = lpTokensAddress;
    }
    ...
    function setKapexTokenAddress(address payable kapexAddress) external onlyOwner {
        _kapexTokenAddress = kapexAddress;
        kapexToken = IBEP20(_kapexTokenAddress); 
    }
    ...
    function setStakingRewardTokenAddress(address payable rewardTokenAddress) external onlyOwner {
        _stakingRewardTokenAddress = rewardTokenAddress;
        stakingRewardToken = IBEP20(_stakingRewardTokenAddress); 
    }
    ...
    function _swapAndLiquify(uint256 tokenBalance) private lockTheSwap {
        ...
        uint256 swapPercent = _feeDev.add(_feeMarketing).add(_feeKodaStakingPool).add(_feeKapexBNBLiquidity).add(_feeKodaBnbLiquidity/2);
        uint256 swapTokens = tokenBalance.mul(swapPercent).div(_feeTotalBNBDeductable);
        uint256 burnTokens = tokenBalance.mul(_feeBurn).div(_feeTotalBNBDeductable);
        uint256 liquidityTokens = tokenBalance.sub(swapTokens).sub(burnTokens);
        
        if(burnTokens > 0)
            kodaToken.transfer(_burnAddress, burnTokens);
        ...
        if(_feeDev > 0)
        {
            uint256 devAmount = transferredBalance.mul(_feeDev).div(swapPercent);
        ...
        if(_feeMarketing > 0)
        {
            uint256 marketingAmount = transferredBalance.mul(_feeMarketing).div(swapPercent);

            _marketingAddress.transfer(marketingAmount);
        }

        if(_feeKodaBnbLiquidity > 0)
        {
            uint256 bnbLiquidityAmount = transferredBalance.mul(_feeKodaBnbLiquidity/2).div(swapPercent);

            addLiquidityBNB(_lpTokensLockAddress, _kodaTokenAddress, liquidityTokens, bnbLiquidityAmount);

            emit SwapAndLiquify(liquidityTokens, bnbLiquidityAmount, liquidityTokens);
        }

        if(_feeKapexBNBLiquidity > 0)
        {
            uint256 bnbToKapexSwapAmount = transferredBalance.mul(_feeKapexBNBLiquidity/2).div(swapPercent);

            swapBNBForTokens(_kapexTokenAddress, bnbToKapexSwapAmount);

            uint256 kapexTokenAmount = kapexToken.balanceOf(address(this));
            kapexToken.approve(address(summitSwapRouter), kapexTokenAmount);
            addLiquidityBNB(_lpTokensLockAddress, _kapexTokenAddress, kapexTokenAmount, bnbToKapexSwapAmount);
        }

        if(_feeKodaStakingPool > 0)
        {
            uint256 kodaStakingPoolAmount = transferredBalance.mul(_feeKodaStakingPool).div(swapPercent);

            if(isStakingRewardInBNB)
                _kodaStakingPoolAddress.transfer(kodaStakingPoolAmount);
            else {
                swapBNBForTokens(_stakingRewardTokenAddress, kodaStakingPoolAmount);

                kodaStakingPoolAmount = stakingRewardToken.balanceOf(address(this));
                stakingRewardToken.transfer(_kodaStakingPoolAddress, kodaStakingPoolAmount);
            }
        }
    }
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
        ...
        path[0] = _kodaTokenAddress;
        path[1] = summitSwapRouter.WETH();

        kodaToken.approve(address(summitSwapRouter), MAX);
