...
contract GameHouseEdge {
    ...
    uint public slippage=100; // 1%
    
    // percent declaration
    uint256 rewardPercent=2500;
    uint256 LockLPPercent=2500;
    uint256 BurnTokenPercent=2500;
    uint256 DaoTreasuryBankRollPercent=2500;
  ...
  // dao treasury  bankroll
    ...
    //setter 
    function setFraction(uint _snum, uint _sden, uint _rnum, uint _rden,uint _slippage) public restricted(){
        snum= _snum;
        sden = _sden;
        anum = _rnum;
        aden = _rden;
      ...
      */
       ...
       uint256 bustBal = BUST.balanceOf(address(this));
        ...
        if(bustAmount<=BUST.balanceOf(address(this))){
            addLiquidityBUSTBUSD(BUSD.balanceOf(address(this)));
        }
        else{
            addLiquidityBUSTBUSD(BUST.balanceOf(address(this)));
        }
    ...
    function addLiquidityBUSTBUSD(uint256 value) internal{
        ...
        uint256 bust = _reserve0.mul(value).div(_reserve1);
        
        uint256 minbust = bust.mul(anum).div(aden);
        uint256 minbusd = value.mul(anum).div(aden);
        uint256 time = block.timestamp + 1220;
        BUST.approve(address(router),bust);
        BUSD.approve(address(router),value);
        router.addLiquidity(
            address(BUSD),
            address(BUST),
            value,
