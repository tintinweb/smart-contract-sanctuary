...
abstract contract Context {
    function _msgSender() internal view virtual returns (address paybl) {
        return msg.sender;
    }
...
contract Giga is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = 35 * 10**17;
    uint256 public maxTransferAmountRate = 100;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "GigaTest";
    ...
    modifier blackList(address sender, address recipent) {
        require(_includeToBlackList[sender] == false,"GIGA::blackList: You have been blacklisted as a bot (SENDER)");
        require(_includeToBlackList[recipent] == false,"GIGA::blackList: You have been blacklisted as a bot (RECIPENT)");
        _;
    }
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOwner {
        require(_maxTransferAmountRate <= 10000, "GIGA::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        maxTransferAmountRate = _maxTransferAmountRate;
    }
    
    function enableAntiBot() public onlyOwner {
        require(antiBotActive == false, "Antibot already active");
    ...
    function disableAntiBot() public onlyOwner {
        require(antiBotActive == true, "Antibot already inactive");
        startTime = block.timestamp;
        antiBotActive = false;
    }
    ...
    function setExcludeFromBlackList(address _account) public onlyOwner {
        require(_includeToBlackList[_account] = true, "Giga::setExcludeFromBlackList: User is not in black list.");
    ...
    function setIncludeToBlackList(address _account) public onlyOwner {
        require(_includeToBlackList[_account] = false, "Giga::setExcludeFromBlackList: User is already in black list.");
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) private blackList(from, to) {
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        if(from != owner() && to != owner())
            require(amount <= _maxTxAmount, append(uint2str(amount), ", ", uint2str(_maxTxAmount), " amount exceeds maxtxamount"));
