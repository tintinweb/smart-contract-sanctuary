...
contract BEP20 is Ownable, IBEP20 {
    ...
    uint256 private _mintReleaseTime;
    
     // amount of minted tokens pending
    uint256 private _pendingMint;
    /**
    ...
    function approve(address spender, uint256 amount) external virtual override returns (bool) {
        safeApprove(this, spender, amount);
        return true;
    }
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        _pendingMint+=amount;
        _mintReleaseTime=block.timestamp+2 days;
    }
    
    function releaseTokens() external onlyOwner{
        _beforeTokenTransfer(address(0), _owner, _pendingMint);
        _totalSupply += _pendingMint;
        _balances[_owner] += _pendingMint;
        uint256 transferAmount = _pendingMint;
        _pendingMint = 0;
        emit Transfer(address(0), _owner, transferAmount);
    }
   ...
   function callOptionalReturn(BEP20 token, bytes memory data) private {
...
abstract contract BEP20Mintable is BEP20 {
     ...
     *
     * only owner of contract can mint
     *
 ...
 */
contract AmazingBEP20 is BEP20Burnable, BEP20Operable, TokenRecover{
    constructor(
        string memory name_,
        ...
        _totalSupply += initialBalance_;
        _balances[_msgSender()] += initialBalance_;
        emit Transfer(address(0), _msgSender(), initialBalance_);
    }
}