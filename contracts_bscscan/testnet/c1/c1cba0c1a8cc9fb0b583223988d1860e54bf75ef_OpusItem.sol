...
contract OpusItem is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    event MintItem(uint256 tokenId, address author, uint256 price, address token,  uint256 copyrightFee, string  tokenURI);
    event SetPlatformFee(uint256 fee);
    event FlipSale(bool sale, uint256 price, address token);
    ...
    function mintItem(address author, uint256 price, address token, uint256 copyrightFee, string memory tokenURI)
    ...
    returns (uint256)
    {
        ...
        emit MintItem(newItemId, author, price, token, copyrightFee, tokenURI);
        return newItemId;
    }
    ...
    function flipSale(uint256 tokenId, bool onSale, uint256 price, address token) public nonReentrant  {
        ...
        emit FlipSale(onSale, price, token);
    }
    ...
    function feePrice(uint256 tokenId) public view returns(uint256 copyrightFee, uint256 platformFeePrice) {
        ...
        platformFeePrice = _price[tokenId].mul(_platformFee).div(10**feeDecimals);
    }
    ...
    function publishAuction(uint256 tokenId, address currency, uint256 minPrice, uint256 plusPrice) public nonReentrant returns(uint256 auctionId) {
        require(_exists(tokenId), "nonexistent token");
        require(ownerOf(tokenId) == _msgSender() || _msgSender() == owner(), "caller is not the token owner");
    ...
    function joinAuction(uint256 tokenId, uint256 amount) public nonReentrant {
        require(_exists(tokenId), "nonexistent token");
        require(ownerOf(tokenId) != _msgSender(), "the token is already yours");
        ...
        if (logs.length > 0) {
            for (uint256 i=0; i <= logs.length-1; i++) {
                if (i == logs.length-1) {
                    ...
                    uint256 platformFeePrice = logs[i].price.mul(_platformFee).div(10**feeDecimals);
