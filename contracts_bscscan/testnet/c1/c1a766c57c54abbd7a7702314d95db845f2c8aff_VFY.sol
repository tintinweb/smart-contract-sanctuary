...
// File: contracts/VFYDividendPayingToken.sol
pragma solidity ^0.7.6;
...
// File: contracts/VFY.sol
pragma solidity ^0.7.6;
contract VFY is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    VFYDividendTracker public dividendTracker;

    address public burnAddress;
    
    uint256 public maxBuyTransactionAmount = 50000000000000 * (10**18);
    uint256 public maxSellTransactionAmount = 50000000000000 * (10**18);
    uint256 public swapTokensAtAmount = 15000000 * (10**18);
    uint256 public _maxWalletToken = 50000000000000 * (10**18);
    ...
    address payable public RD;
    address payable public charityAddress;
    address public RDTokenAddressForFee = address(this);
    address public charityTokenAddressForFee = address(this);
    // transfer fees
    uint256 public RDFee = 0;
    uint256 public charityFee = 0;
    uint256 public tokenRewardsFee = 0;
    uint256 public liquidityFee = 0;
    uint256 public burnFee = 0;
    uint256 public totalFees = tokenRewardsFee.add(liquidityFee).add(RDFee).add(charityFee).add(burnFee);
    // buy fees
    uint256 public buyRDFee = 2;
    uint256 public buyCharityFee = 2;
    uint256 public buyTokenRewardsFee = 0;
    uint256 public buyLiquidityFee = 1;
    uint256 public buyBurnFee = 1;
    uint256 public buyTotalFees = buyTokenRewardsFee.add(buyLiquidityFee).add(buyRDFee).add(buyCharityFee).add(buyBurnFee);
    // sell fees
    uint256 public sellRDFee = 2;
    ...
    uint256 public sellTotalFees = sellTokenRewardsFee.add(sellLiquidityFee).add(sellRDFee).add(sellCharityFee).add(sellBurnFee);
    // fee tracker
    uint256 private previousRDFee = RDFee;
    ...
    function updateRDAddress(address payable newAddress) external onlyOwner {
        _isExcludedFromFees[RD] = false;
        RD = newAddress;
        _isExcludedFromFees[newAddress] = true;
        //dividendTracker.excludeFromDividends(R&D);     
    }    

    function updateCharityAddress(address payable newAddress) external onlyOwner {  
        _isExcludedFromFees[charityAddress] = false;
    ...
    function updateFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _RDFee, uint256 _charityFee, uint256 _burnFee) external onlyOwner {
        tokenRewardsFee = _tokenRewardsFee;
        liquidityFee = _liquidityFee;
        RDFee = _RDFee;
        charityFee = _charityFee;
        burnFee = _burnFee;
        totalFees = _tokenRewardsFee.add(_liquidityFee).add(_RDFee).add(_charityFee).add(_burnFee);
        previousRDFee = _RDFee;
        ...
        previousTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_RDFee).add(_charityFee).add(_burnFee);
    }    
    
    function updateBuyFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _RDFee, uint256 _charityFee, uint256 _burnFee) external onlyOwner {
        buyTokenRewardsFee = _tokenRewardsFee;
        buyLiquidityFee = _liquidityFee;
        buyRDFee = _RDFee;
        buyCharityFee = _charityFee;
        buyBurnFee = _burnFee;
        buyTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_RDFee).add(_charityFee).add(_burnFee);
    }    
    
    function updateSellFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _RDFee, uint256 _charityFee, uint256 _burnFee) external onlyOwner {
        sellTokenRewardsFee = _tokenRewardsFee;
        sellLiquidityFee = _liquidityFee;
        sellRDFee = _RDFee;
        sellCharityFee = _charityFee;
        sellBurnFee = _burnFee;
        sellTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_RDFee).add(_charityFee).add(_burnFee);
    }
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "VFY: The dividend tracker already has that address");

        VFYDividendTracker newDividendTracker = VFYDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "VFY: The new dividend tracker must be owned by the VFY token contract");
    ...
    function updateRouter(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "VFY: The router already has that address");
        emit UpdateRouter(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }    
    
    function updatePair(address newAddress) external onlyOwner {
        require(newAddress != uniswapV2Pair , "VFY: The pair already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "VFY: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "VFY: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "VFY: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "VFY: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "VFY: Cannot update gasForProcessing to same value");
    ...
    function swapAndLiquify(uint256 tokens) private  {
        ...
        if(RDFee != 0){
            RDFeeFull = tokens.mul(RDFee).div(100);
        }
        if(charityFee != 0){
            charityFeeFull = tokens.mul(charityFee).div(100);
        }

        uint256 tokensAfterFee = tokens - RDFeeFull - charityFeeFull;
        ...
        uint256 RDFeeBalance;
        uint256 charityFeeBalance;

        if(RDFee != 0){
            RDFeeBalance = newBalance.mul(RDFee).div(100);
        }
        if(charityFee != 0){
            charityFeeBalance = newBalance.mul(charityFee).div(100);
        }
        
        uint256 finalBalance = newBalance - RDFeeBalance - charityFeeBalance;
        uint256 finalHalf = otherHalf;

        if(RDFeeBalance != 0){
        if(RDTokenAddressForFee != address(0)){
            swapEthForTokens(RDFeeBalance, RDTokenAddressForFee, RD);
        }else{
            (bool sent,) = RD.call{value: RDFeeBalance}("");
        }}
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "VFY: This account cannot send tokens until trading is enabled");
        }
        ...
        if(takeFee || buyOrSell ) {
            ...
            uint256 calcFee = totalFees - burnFee - RDFee - charityFee;
            uint256 feeAmount = amount.mul(calcFee).div(100);
            uint burnAmount = amount.mul(burnFee).div(100);
            uint RDAmount = amount.mul(RDFee).div(100);
            uint charityAmount = amount.mul(charityFee).div(100);
            amount = amount - feeAmount - burnAmount - RDAmount - charityAmount;
            super._transfer(from, address(this), feeAmount);
            super._transfer(from, RD, RDAmount);
            super._transfer(from, charityAddress, charityAmount);
...
contract VFYDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("VFY_Dividend_Tracker", "VFY_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "VFY_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "VFY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main VFY contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "VFY_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "VFY_Dividend_Tracker: Cannot update claimWait to same value");
