...
contract ApidaiStake is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor (uint256 _maxTokenStake, address _apidaiAddress, address _BUSD) {
        ...
        planDetails[1] = PlanInfo({planID: 1, planDays: 30, planPercentage: 25});
        planDetails[2] = PlanInfo({planID: 2, planDays: 60, planPercentage: 33});
        planDetails[3] = PlanInfo({planID: 3, planDays: 90, planPercentage: 40});
    }
    ...
    function stakeApidai(uint256 _tokenAmount) external nonReentrant whenNotPaused {
        ...
        userTotalBalance[msg.sender] = userTotalBalance[msg.sender].add(_tokenAmount);
    ...
    function unstakeApidai(uint256 _stakeID) external nonReentrant whenNotPaused {
        ...
        userTotalBalance[msg.sender] = userTotalBalance[msg.sender].sub(user.stakeAmount);
