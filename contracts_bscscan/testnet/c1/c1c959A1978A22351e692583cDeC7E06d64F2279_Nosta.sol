...
/**
    ...
    */
    ...
    event WeEnterInBuyCondition(address _from, address _to);
    event weCheckIfLastSellInUnderFortyEightHour(address _from, address _to);
    event weEnterInAutoLiqCondition(address _from, address _to);
    // Modifiers 
    ...
    */
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // BUY - No TAX
        // Check if msg.sender isBlackListed
        if (from == _pancakeswapV2LiquidityPair) {
             emit WeEnterInBuyCondition(from, to);
            ...
            if (delta > 0 && delta <= TWO_DAYS) {
                // blacklist; 
                emit weCheckIfLastSellInUnderFortyEightHour(from, to);
        ...
        if (_enableLiquidity && tokenBalance >= _tokenSwapThreshold && !currentlySwapping && from != _pancakeswapV2LiquidityPair) {
            ...
            emit weEnterInAutoLiqCondition(from, to);
        ...
        // if (to == _pancakeswapV2LiquidityPair) {
    ...
    */
    ...
    function _priceImpactTax(uint256 amount) public returns(bool) { 
        ...
        if(receivedBUSD <= 0) return (false);
        else {
            uint256 _new_price   = amount.div(receivedBUSD);
            uint256 _delta_price = (_new_price.div(_market_price)).mul(10**_DECIMALS);
            uint256 _portion     =  uint256(1).mul(10**_DECIMALS);
            
            uint256 _price_impact = (_portion).sub(_delta_price).mul(100);
        
            emit Watch3(_price_impact, _reserveA, _reserveB, _constant, _market_price, _reserveA_new, _reserveB_new, receivedBUSD, _new_price);
