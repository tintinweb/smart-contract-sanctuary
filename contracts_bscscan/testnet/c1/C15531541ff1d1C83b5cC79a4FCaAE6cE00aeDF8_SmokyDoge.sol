...
/*
SmokyDoge Coin features:
Different buy/sell transaction tax
   buy transaction tax
 	5% fee auto added to the liquidity pool (12% total tax)
 	2% fee auto distributed to all hodlers
 	1% fee added to development and team wallet
 	2% fee added to marketing wallet
 	2% fee added to wallet for buyback SmokyDoge in dip
 	
Penalty for sell during the initial days (16% total tax)
 	6% fee auto added to the liquidity pool
 	4% fee auto distributed to all hodlers
 	2% fee added to development and team wallet
 	2% fee added to marketing wallet
 	2% fee added to wallet for buyback SmokyDoge in dip
 	Join our Community: https://t.me/Smokydoge
*/
...
contract SmokyDoge is Context, IBEP20, Ownable {
    ...
    // ********************************* START VARIABLES **********************************
    string private _name = "SmokyDoge Coin";                                          // name
    string private _symbol = "SmokyDoge";                                             // symbol
    uint256 private _tTotal = 1000000 * 10**3 * 10**uint256(_decimals);          // total supply
    ...
    uint256 public _taxFee4Sellers = 4;
    ...
    uint[] public _percentages = [32,34,34];                                             // % of liquidity fee split to wallet(s)
    address payable[] public _wallets = [                                                // wallet(s) to receive liquidity fee split
        payable(0xDA8B4999BB6D48295dEE8E542B1a86C308672363),                             // Dev+Team wallet
        payable(0xa42F1d5bcf97123601d92137C71418305690944f),                             // Marketing wallet
        payable(0xb2C86CEE1Bb0A5D0A4FB71E570c7f57068507319)                              // Buyback wallet
    ];
    // ********************************** END VARIABLES ***********************************
    ...
    function _goLive() external onlyOwner() {
        require(!isTokenLive, "SmokyDoge is already live");
        isTokenLive = true;
    }
    ...
    function _transfer(
        ...
        require(_isExcludedFromFee[from] || isTokenLive, "SmokyDoge is not live yet");
