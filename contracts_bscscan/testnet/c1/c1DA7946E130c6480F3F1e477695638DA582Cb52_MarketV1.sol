...
contract MarketV1 is Ownable, Pausable, ReentrancyGuard {
    ...
    MyMasterWarCore public nft;
    IMarketAccessManager public accessManager;
    ...
    function setMyMasterWarCore(MyMasterWarCore _nft) public onlyOwner {
        nft = _nft;
    }
    ...
    {
        require(nft.exists(_nftId), "Error, wrong nftId");
        return (
            items[_nftId].owner,
    ...
    function listing(
        ...
        require(_nftIds.length == _amounts.length, "Input invalid");

        for (uint256 i = 0; i < _nftIds.length; i++) {
            require(nft.exists(_nftIds[i]), "Error, wrong nftId");
            require(
                nft.ownerOf(_nftIds[i]) == _msgSender(),
                "Error, you are not the owner"
            );
            require(
                items[_nftIds[i]].owner == address(0),
                "Error, item listing already"
            );

            items[_nftIds[i]] = Item(_msgSender(), _currencies[i], _amounts[i]);
            //transfer NFT for market contract
            nft.transferFrom(_msgSender(), address(this), _nftIds[i]);
            emit Listing(
                _msgSender(),
                _nftIds[i],
                _currencies[i],
        ...
        nonReentrant
    {
        ...
        _validate(_id, _amount);

        address previousOwner = nft.ownerOf(_id);
        address newOwner = _msgSender();

        uint256 commissionAmount;
        uint256 sellerAmount;
        (commissionAmount, sellerAmount) = _trade(
            _id,
            items[_id].currency,
            _amount
        );
    ...
    function _validate(uint256 _id, uint256 _amount) internal view {
        require(nft.exists(_id), "Error, wrong nftId");
        require(items[_id].owner != address(0), "Item not listed currently");
        require(_msgSender() != nft.ownerOf(_id), "Can not buy what you own");
        if (items[_id].currency == address(0)) {
            require(
                msg.value >= items[_id].price,
                "Error, the amount is lower"
            );
        } else {
            require(_amount >= items[_id].price, "Error, the amount is lower");
        }
    }
    ...
    function _trade(
        ...
        address nftOwner = items[_id].owner;

        nft.transferFrom(address(this), buyer, _id);
        uint256 _commissionAmount = (_amount * commission) / 10000;
        uint256 _sellerAmount = _amount - _commissionAmount;

        if (items[_id].currency == address(0)) {
            payable(nftOwner).transfer(_sellerAmount);
            payable(owner()).transfer(_commissionAmount);
        } else {
            IBEP20(_currency).transferFrom(buyer, nftOwner, _sellerAmount);
            IBEP20(_currency).transferFrom(buyer, owner(), _commissionAmount);
        ...
        delete items[_id];
        return (_commissionAmount, _sellerAmount);
    }

    function updatePrice(uint256[] memory _nftIds, uint256[] memory _prices)
        public
        whenNotPaused
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < _nftIds.length; i++) {
            uint256 oldPrice = items[_nftIds[i]].price;
            require(
                _msgSender() == items[_nftIds[i]].owner,
                "Error, you are not the owner"
            );
            items[_nftIds[i]].price = _prices[i];
    ...
    function unListing(uint256[] memory _nftIds)
        public
        whenNotPaused
        returns (bool)
    {
        for (uint256 i = 0; i < _nftIds.length; i++) {
            ...
            delete items[_nftIds[i]];

            nft.transferFrom(address(this), _msgSender(), _nftIds[i]);

            emit UnListing(_msgSender(), _nftIds[i], block.timestamp);
        }
...
contract MyMasterWarCore is ERC721, Ownable {
    ...
    event Detroy(address indexed caller, uint256 indexed nftId, uint256 time);
    ...
    function born(address _toAddress, uint256 _gene)
        external
        returns (uint256)
    {
        require(
    ...
    function detroy(uint256 _nftId) external {
        require(
            MATAccessManager.isDetroyAllowed(_msgSender(), _nftId),
            "Not have detroy permisison"
        );
        _burn(_nftId);
        delete myMasterWars[_nftId];

        //charge fee
        // coreFee.chargeDetroyFee(_msgSender(), _nftId);
        emit Detroy(_msgSender(), _nftId, block.timestamp);
    }
...
interface IMATAccessManager {
    ...
    function isDetroyAllowed(address _caller, uint256 _nftId)
        external
        view
        returns (bool);
}
