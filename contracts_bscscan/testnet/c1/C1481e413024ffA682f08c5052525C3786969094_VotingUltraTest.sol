...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x594d7a19209258583550311501c865D9b5fB6dF0;
...
contract VotingUltraTest is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0x594d7a19209258583550311501c865D9b5fB6dF0;
    ...
    string private _name = "Voting test token";
    ...
    uint256 public _maxTxAmount = _tTotal / 1000;
    ...
    struct Poll { 
        ...
        bool isClosed;
        string[] pollOptions;
        mapping(string => uint256) pollVotes;
    }
    Poll[] private _polls;
    uint256 public _currentPollId;
    mapping(address => mapping(uint256 => bool)) private _voteParticipation;
    ...
    /* Voting methods */
    function _startNewPoll(string memory pollTitle) external onlyOwner() {
        require(_polls[_polls.length - 1].isClosed == true, "Lastest poll is NOT closed yet.");        
        Poll storage newPoll = _polls.push();
        newPoll.id = _currentPollId;
        newPoll.title = pollTitle;
        newPoll.isClosed = false;
        _currentPollId = _currentPollId.add(1);
    } 

    function _addVoteOption(string memory newOption) external onlyOwner() {
        require(_polls[_polls.length - 1].isClosed == false, "Lastest poll is already closed.");        
        _polls[_polls.length - 1].pollOptions.push(newOption);
        _polls[_polls.length - 1].pollVotes[newOption] = 0;
    }

    function _closeCurrentPoll() external onlyOwner() {
        require(_polls[_polls.length - 1].isClosed == false, "Lastest poll is already closed.");            
       _polls[_polls.length - 1].isClosed = true;
    }   

    function _getVoteOptions(uint256 pollId) external view returns(string[] memory){
        require(_polls.length > pollId, "Poll does NOT exists with the requested id.");        
        return _polls[pollId].pollOptions;
    }

    function _getResultOfPoll(uint256 pollId, string memory pollOption) external view returns(uint256){
        require(_polls[pollId].isClosed == true, "Requested poll is NOT closed yet.");        
        return _polls[pollId].pollVotes[pollOption];
    }

    function _vote(address sender, string memory option) external {
        require(_polls[_polls.length - 1].isClosed == false, "Lastest poll is already closed.");
        require(_voteParticipation[sender][_currentPollId] == false, "Sender already voted in the current poll.");        
        _polls[_polls.length - 1].pollVotes[option] = _polls[_polls.length - 1].pollVotes[option].add(balanceOf(sender));
        _voteParticipation[sender][_currentPollId] = true;
    }  
}