...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "CRYPTOMINI: The router already has that address");
  ...
  constructor(string memory _name, string memory _symbol, uint8 _decimals) ERC20(_name, _symbol, _decimals) {
    ammIsWhiteListed[address(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3)] = true; // PCS V2 router @note enabled
    ...
    // ammIsWhiteListed[address(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3)] = true; // PCS Testnet router @note disabled
  }
...
contract CRYPTOMINI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MINIDIVIDEND public dividendTracker;
    ...
    constructor() ERC20("CRYPTOMINI", "MINI", 9) {

        dividendTracker = new MINIDIVIDEND();
        ...
        */
        _mint(owner(), 10000000000 * (10**9)); // @note Equals 1e+18 | Set supply to 1,000,000,000 (without override)
    }
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
        require(isContract(ammAddress), "CRYPTOMINI: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CRYPTOMINI: The dividend tracker already has that address");

        MINIDIVIDEND newDividendTracker = MINIDIVIDEND(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CRYPTOMINI: The new dividend tracker must be owned by the CRYPTOMINI token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "CRYPTOMINI: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require (airdropWallets.length == amount.length, "CRYPTOMINI: airdropToWallets:: Arrays must be the same length");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "CRYPTOMINI: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "CRYPTOMINI: The operations wallet is already this address");
    ...
    function updateMigrationSwapContract(address newMigrationSwapContract) external onlyOwner {
        require(newMigrationSwapContract != migrationSwapContract, "CRYPTOMINI: The migration swap contract is already this address");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "CRYPTOMINI: The buyback wallet is already this address");
    ...
    function updateFees(uint256 bnbRewardPerc, uint256 liquidityPerc, uint256 operationsPerc, uint256 buyBackPerc) external onlyOwner {
        require (operationsPerc.add(buyBackPerc) <= liquidityPerc, "CRYPTOMINI: updateFees:: Liquidity Perc must be equal to or higher than operations and buyback combined.");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CRYPTOMINI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CRYPTOMINI: Cannot update gasForProcessing to same value");
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "CRYPTOMINI: setRewardToken:: Address is a wallet, not a contract.");
        require(rewardTokenAddress != address(this), "CRYPTOMINI: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isBlacklistedToken(rewardTokenAddress), "CRYPTOMINI: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "CRYPTOMINI: setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(uniswapV2Router), "CRYPTOMINI: setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "CRYPTOMINI: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isBlacklistedToken(rewardTokenAddress), "CRYPTOMINI: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
        require(isAMMWhitelisted(ammContractAddress) == true, "CRYPTOMINI: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userRawBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "CRYPTOMINI: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CRYPTOMINI: Automated market maker pair is already set to that value");
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if (liquidityFee > 0) {
            ...
            if (operationsFee > 0) {
                ...
                require(success, "CRYPTOMINI: SwapAndLiquify:: Unable to send BNB to Operations Wallet");
            }
            ...
            if (buyBackFee > 0) {
                ...
                require(success, "CRYPTOMINI: SwapAndLiquify:: Unable to send BNB to BuyBack Wallet");
            }
...
contract MINIDIVIDEND is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MINID", "MINID", 9) {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) pure internal override {
        require(false, "FDT: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "FDT: withdrawDividend disabled. Use the 'claim' function on the main CRYPTOMINI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FDT: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FDT: Cannot update claimWait to same value");
