  ...
  //address public constant USDT = address(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
    ...
    function _priceImpactTax(uint256 amount) public view returns (bool) {
        ...
        (uint256 _reserveA, uint256 _reserveB) = getReserves(
            ...
            BUSD
				...
				require(_reserveA > 0, "probleme with _reserveA = 0");
				require(_reserveB > 0, "probleme with _reserveB = 0");

        uint256 _constant = IUniswapV2Pair(stellaSwapV2Pair).kLast();

				require(_constant > 0, "probleme with constant = 0");

        
        uint256 _market_price = wdiv(_reserveA, _reserveB);

				require(_market_price > 0, "probleme with _market_price = 0");
        ...
        if (_reserveA == 0 && _reserveB == 0) {
            ...
            uint256 _reserveA_new = _reserveA.sub(amount);
						require(_reserveA_new > 0, "probleme with _reserveA_new = 0");
            uint256 _reserveB_new = wdiv(_constant, _reserveA_new);

            if (_reserveB >= _reserveB_new) return false;
            uint256 receivedBUSD = _reserveB_new.sub(_reserveB);

						require(receivedBUSD > 0, "probleme with receivedBUSD = 0");

            uint256 _new_price =  wmul(wdiv(amount, receivedBUSD), 10**18);
            uint256 _delta_price =  wdiv(_new_price, _market_price);
    ...
    function _priceImpactTaxV2(uint256 amount) public view returns (uint256, uint256, uint256) {
        ...
        uint256 _constant = IUniswapV2Pair(stellaSwapV2Pair).kLast();
				...
				require(_constant > 0, "probleme with _constant = 0");
