...
import "./qrawler/impl/PriceQrawler.sol";
import "./qrawler/impl/IPFSQrawler.sol";
import "./qrawler/IQrawler.sol";

contract Fund is IQrawler, PriceQrawler, IPFSCrawler {
    ...
    function queryPriceOracle() onlyOwner external {
        qrawlerQuery(iterateRequestId(), QueryType.PriceOracle, "");
    }

    function queryIPFS(string memory ipfsUrl) onlyOwner external {
        requestCounter.increment();
        qrawlerQuery(iterateRequestId(), QueryType.IPFS, abi.encode(ipfsUrl));
    }

    /**
     * @dev An implementation of the IQrawler callback function will be called after Qrawler finished external querying. 
     *
     * Requirements:
     *
     * - resolverAddress must be correct
     */
    function qrawlerCallback(string memory _reqId, QueryType _type, bytes calldata _calldata) external override onlyResolver {
...
abstract contract PriceQrawler is Qrawler {
    ...
    function getPriceOracle() public view returns (PriceOracle memory priceOracle) {
        require(encodedPriceOracle.length > 0, "Qrawler: invalid price data");
        ...
        require(decoder.finished(), "Qrawler: data decode not finished");
    } 
...
abstract contract IPFSCrawler is Qrawler {
...
interface IQrawler {
    function qrawlerCallback(string memory _reqId, QueryType _type, bytes calldata _calldata) external;
}
...
abstract contract Qrawler is Ownable {

    address resolverAddress;

    event QrawlerQuery(string _reqId, address _caller, QueryType _type, bytes _calldata);
    event QrawlerCallback();

    modifier onlyResolver() {
        require(msg.sender == resolverAddress, "Qrawler: invalid resolver address");
        _;
    }
    ...
    function qrawlerQuery(string memory _reqId, QueryType _type, bytes memory _calldata) internal {
        emit QrawlerQuery(_reqId, address(this), _type, _calldata);
    }
