...
contract CORSACNFT is BEP721,Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    address private _CSCT;

    constructor(string memory name, string memory symbol, address CSCT) public BEP721(name, symbol) {
        _CSCT = CSCT;
    }

    function mint(string memory tokenUri, uint256 price) public virtual{
        // We cannot just use balanceOf to create the new tokenId because tokens
        // can be burned (destroyed), so we need a separate counter.
        require(IBEP20(_CSCT).balanceOf(msg.sender) > price, "CSCT: Insufficient CSCT");
        IBEP20(_CSCT).transferFrom(msg.sender, address(this), price);
    ...
    function withdraw() public onlyOwner {
        uint256 balance = IBEP20(_CSCT).balanceOf(address(this));
        IBEP20(_CSCT).transfer( msg.sender, balance);
    }
}