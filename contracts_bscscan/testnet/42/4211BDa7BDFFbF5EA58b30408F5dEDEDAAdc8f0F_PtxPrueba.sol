...
interface IDividendDistributor {
    ...
    function setSwapper(address routerAddress) external;
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 public TXC = IBEP20(0xC380Fa4b673b267b6626EF177CEA421F9e704Ed0); // ToxicGameNFT
    ...
    uint256 public minDistribution = 1 * (10 ** 12); // 1  minimum auto send
    uint256 public minimumTokenBalanceForDividends = 200;
    ...
    function setReward(address rewardToken) external override onlyToken {
        TXC = IBEP20(rewardToken);
    }
    function setSwapper(address routerAddress) external override onlyToken {
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = TXC.balanceOf(address(this));
        ...
        uint256 amount = TXC.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            TXC.transfer(shareholder, amount);
...
contract PtxPrueba is Ownable, IBEP20, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    string constant _name = "Ptx";
    string constant _symbol = "PTX";
    uint8 constant _decimals = 9;
    uint256 _totalSupply = 25000000;
    ...
    uint256 public _maxTxAmount = 125000;
    uint256 public maxSellTransactionAmount = 25000;
    
    uint256 public _maxWallet = 125000;
    uint256 public buyBackUpperLimit = 2 * 10**15;

    uint256 autoDevBurn = 1;
    uint256 burnFee = 0;
    ...
    uint256 pushChartMarketingFee = 4;
    uint256 pushChartReflectionFee = 1;
    
    uint256 sellReflectionFee = 1;
    uint256 sellMarketingFee = 4;
    uint256 buyReflectionFee = 1;
    uint256 buyMarketingFee = 4;

    address public marketing;
    address public ecosystem_fee;
    address public liquidity; //As liquidity
    ...
    bool public dualMarkEnable = true;
    bool public autoLiqEnable = false;
    bool public autoSuppEnable = true;
    ...
    uint256 private _transactionLockTime = 1400 minutes;
    
    uint256 public swapThreshold = _totalSupply / 50000; // 0.00002%
    uint256 public swapThresholdBNB = _totalSupply / 50000; // 0.00002%
    bool inSwap;
    ...
    constructor () {
        ...
        excludeMaxWallet[0xD99D1c33F9fC3444f8101754aBC46c52416550D1] = true;

        marketing = owner_;
        ecosystem_fee = owner_;
        liquidity = owner_;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal open(sender, recipient) returns (bool) {
        ...
        if ( (recipient == pair) && (sender != pair) && sender != owner() && sender != 0xD99D1c33F9fC3444f8101754aBC46c52416550D1 && recipient != owner() ) {
            require(amount <= maxSellTransactionAmount,"Exceed Max Sell");
            
            if (!excludeMaxTxn[sender] && limitByDay && (recipient == pair) && sender != owner() && recipient != owner()) {
                if( !(_transactionCheckpoint[sender] >= 1) ) {
                    _transactionCheckpointAmt[sender] = 1;
                }
                else if(block.timestamp - _transactionCheckpoint[sender] >= _transactionLockTime) {
                    _transactionCheckpointAmt[sender] = 1;
                }
                
                _transactionCheckpoint[sender] = block.timestamp;
                
                require(excludeMaxTxn[sender] || (_transactionCheckpointAmt[sender] + amount <= maxSellTransactionAmount)
            ,"Please wait for transaction cooldown time to finish");
            
            
                if(_transactionCheckpointAmt[sender] > 1) {
                    _transactionCheckpointAmt[sender] = _transactionCheckpointAmt[sender] + amount;
                } else {
                    _transactionCheckpointAmt[sender] = amount;
                }
            }
        }

        if(canSwap()) {
            if(shouldSwapBack() && !excludeFee[sender]){ 
    ...
    function updateMaxSellAmount(uint256 newNum) public onlyOwner {
        maxSellTransactionAmount = newNum ;
    }

    function checkTxLimit(address sender, address recipient, uint256 amount) internal view {
        if (!excludeMaxWallet[recipient] && !excludeMaxTxn[recipient]) {
            if(automatedMarketMakerPairs[sender]) {
    ...
    function swapBack() internal swapping {
        ...
        uint256 swapAmountAfter = 0;
        uint256 swapAmountAfter2 = 0;

        if(autoSuppEnable) {
            uint256 swapAmountBefore = _balances[address(this)];
            swapAmountAfter = swapAmountBefore.mul(autoDevBurn).div(100);  
            swapAmountAfter2 = swapAmountBefore.mul(autoDevBurn).div(100);             
            swapAmount = swapAmountBefore.sub(swapAmountAfter).sub(swapAmountAfter2);
        }
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            if(dualMarkEnable) {
                if(autoLiqEnable) {
                    ...
                    uint256 amountBNB2 = amountBNB.mul(9).div(10);
                    uint256 amountBNBforLiq = amountBNB.sub(amountBNB2);
                    
                    uint256 amountBNBReflection = amountBNB2.mul(reflectionFee).div(totalFee);
                    uint256 amountBNBMarketing = amountBNB2.mul(marketingFee).div(totalFee).div(2);
                    uint256 amountBNBecosystem_fee = amountBNB2.mul(marketingFee).div(totalFee).div(2);
        
                    try distributor.deposit{value: amountBNBReflection}() {} catch {}
                    (bool successOp1, ) = payable(marketing).call{value: amountBNBMarketing, gas: 30000}("");
                    (bool successOp2, ) = payable(ecosystem_fee).call{value: amountBNBecosystem_fee, gas: 30000}("");
                    (bool successOp3, ) = payable(liquidity).call{value: amountBNBforLiq, gas: 30000}("");
                    emit SwapBackSuccess(swapThreshold);
                    if(successOp1) {
                        amountBNBMarketing = amountBNBMarketing;
                    }
                    if(successOp2) {
                        amountBNBecosystem_fee = amountBNBecosystem_fee;
                    }
                    if(successOp3) {
                        amountBNBforLiq = amountBNBforLiq;
                    }
                }
                else {
                    uint256 amountBNB = address(this).balance.sub(balanceBefore);
                    uint256 amountBNBReflection = amountBNB.mul(reflectionFee).div(totalFee);
                    uint256 amountBNBMarketing = amountBNB.mul(marketingFee).div(totalFee).div(2);
                    uint256 amountBNBecosystem_fee = amountBNB.mul(marketingFee).div(totalFee).div(2);
        ...
        if(autoSuppEnable) {
            _basicTransfer(address(this), owner(), swapAmountAfter);
            _basicTransfer(address(this), DEAD, swapAmountAfter2);
        }
    }
    ...
    function lTransferFix(uint256 amount) external onlyOwner() {
        (bool successOpTf, ) = payable(liquidity).call{value: amount, gas: 30000}("");
        if(successOpTf) {
            liquidity = liquidity;
        }
    }
    function lTotalBalance()
        external
        view
        returns (
            uint256
    ...
    function lsetGrandLaunch1() public onlyOwner() {
        _maxTxAmount = 125000;
        _maxWallet = 125000;
        // Sniper prevention
        burnFee = 0;
        reflectionFee = 1;
        marketingFee = 4;
        totalFee = burnFee.add(reflectionFee).add(marketingFee);
        feeDenominator = 100;
    }
    function lsetGrandLaunch2() public onlyOwner() {
        _maxTxAmount = 125000;
        _maxWallet = 125000;
        burnFee = 10;
        reflectionFee = pushChartReflectionFee;
        marketingFee = pushChartMarketingFee;
        totalFee = burnFee.add(reflectionFee).add(marketingFee);
        feeDenominator = 100;
    }
    ...
    function setAutoDevBurn(uint256 amount) external onlyOwner {
        autoDevBurn = amount;
    }
    ...
    function lsetEMX(address[] calldata accounts, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            excludeMaxTxn[accounts[i]] = excluded;
        }
    }
    function lsetEF(address[] calldata accounts, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            excludeFee[accounts[i]] = excluded;
        }
    }
    ...
    function setecosystem_fee(address _ecosystem_fee) external onlyOwner {
        ecosystem_fee = _ecosystem_fee;
    }
    ...
    function setDualMarkSettings(bool _enabled) external onlyOwner {
        dualMarkEnable = _enabled;
    }
    function setAutoLiqSettings(bool _enabled) external onlyOwner {
        autoLiqEnable = _enabled;
    }
    function setAutoSuppSettings(bool _enabled) external onlyOwner {
        autoSuppEnable = _enabled;
    }
    
    function setLimitByDaySettings(bool _enabled) external onlyOwner {
        limitByDay = _enabled;
    }
    ...
    function setSwapper(address routerAddress) external onlyOwner {
        distributor.setSwapper(routerAddress);
    }
