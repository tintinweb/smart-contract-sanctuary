...
contract SwapRouter is ISwapRouter02, Ownable {
    ...
    receive() external payable {
        assert(msg.sender == WETH); // only accept OKT via fallback from the WETH contract
    }
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            if (tradingPool != address(0)) {
                ITradingPool(tradingPool).enter(msg.sender, input, output, amountOut);
            }
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            if (tradingPool != address(0)) {
                ITradingPool(tradingPool).enter(msg.sender, input, output, amountOutput);
            }
            (uint256 amount0Out, uint256 amount1Out) =
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"0a5e5e629cea451fb8659e47bde11a8d64bb578102b4dbb4c2ea43846be4e155" // init code hash
                    )
                )
            )
        );
