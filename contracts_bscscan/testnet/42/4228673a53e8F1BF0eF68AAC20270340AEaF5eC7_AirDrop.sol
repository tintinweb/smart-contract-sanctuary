...
contract AirDrop is Context {
    ...
    //////////////////////////// ACCESS CONTROL VARS ////////////////////////////
    address public directorAddress = _msgSender();
    ...
    modifier OnlyDirector() {
        require(directorAddress == _msgSender(), "Caller must be the Director");
        _;
    }

    modifier OnlyStaff(address tokenAddress) {
        address managerAddress = tokenAddressToManagerAddress[tokenAddress];
        require(managerAddress == _msgSender() || directorAddress == _msgSender(), "Caller must be Director or Manager");
        _;
    }
    //////////////////////////// ACCESS CONTROL MODIFIERS ////////////////////////////
    ...
    function TransferDirectorAccount(address newDirectorAddress) public virtual OnlyDirector() {
        directorAddress = newDirectorAddress;
    }
    ...
    event InitilizationOfAirdrops(address indexed initializerAddress, uint256 currentBlockTime);
    event AirDropClaimed(address indexed claimer, uint256 indexed amountOfAirdropTokenToClaim, uint256 currentBlockTime);
    event AirdropTokenAddressChanged(address indexed oldAirdropTokenAddress, address indexed newAirdropTokenAddress, uint256 currentBlockTime);
    ...
    function InitializeTokenAirDropAddressesAndAmounts(address tokenAddress, address[] memory addressesToAirDrop, uint256[] memory amountsToAirDrop) external OnlyStaff(tokenAddress) { 
        ...
        emit InitilizationOfAirdrops(_msgSender(), GetCurrentBlockTime());
    }
    ...
    // TODO - write steps of how to use normal system
    // TODO - write steps of how to use timer system

    mapping(address => mapping(address => uint256)) public userAddressToTokenAddressToAmountTimerSystem;
    ...
    mapping(address => mapping(address => uint256)) public userAddressToTokenAddressToLeftToCliam;    // the amount of token the user has currently claimed
    ...
    function InitializeTokenAirDropAddressesAndAmountsTimerSystem(address tokenAddress, address[] memory addressesToAirDrop, uint256[] memory amountsToAirDrop) external OnlyStaff(tokenAddress) { 
        for(uint i = 0; i < addressesToAirDrop.length; i++){
            userAddressToTokenAddressToAmountTimerSystem[addressesToAirDrop[i]][tokenAddress] = amountsToAirDrop[i];
            userAddressToTokenAddressToClaimedAmount[addressesToAirDrop[i]][tokenAddress] = 0;      // resets claimed amount to 0
            userAddressToTokenAddressToLeftToCliam[addressesToAirDrop[i]][tokenAddress] = amountsToAirDrop[i];  // resets to total amount as none has been claimed
            userAddressToTokenAddressToIsFullyClaimed[addressesToAirDrop[i]][tokenAddress] = false; // resets total claim to false
        }
        emit InitilizationOfAirdrops(_msgSender(), GetCurrentBlockTime());
    }

    function BalanceOfAirdropTokenForUserAddressTimeSystem(address account, address tokenForAirdrop) public view returns (uint256) {
        uint256 airdropBalance = userAddressToTokenAddressToAmountTimerSystem[account][tokenForAirdrop];
        return airdropBalance;  
    }
    ...
    function LeftBalanceOfAirdropTokenForUserAddressTimeSystem(address account, address tokenForAirdrop) public view returns (uint256) {
        uint256 airdropLeftBalance = userAddressToTokenAddressToAmountTimerSystem[account][tokenForAirdrop];
    ...
    function percentOfAirdropAvailableToClaim(address tokenForAirdrop) public view returns (uint256) {
        ...
        uint256 percentOfAirDropAvailable = cyclesElapsed.div(cyclesOfAirdrop).mul(100);
    ...
    function AirDropClaimTimerSystem(address tokenForAirdrop) public {    
        ...
        uint256 amountOfAirdropTokenToClaimTotal = userAddressToTokenAddressToAmountTimerSystem[claimer][tokenForAirdrop];  // get it into a uint256 var for ease of use
        ...
        userAddressToTokenAddressToLeftToCliam[claimer][tokenForAirdrop] = amountOfAirdropTokenToClaimTotal.sub(userAddressToTokenAddressToClaimedAmount[claimer][tokenForAirdrop]);
 
        IERC20(tokenForAirdrop).safeTransfer(PayableInputAddress(claimer), amountOfAirdropTokenToClaimTotal);

        emit AirDropClaimed(claimer, amountOfAirDropToGive, GetCurrentBlockTime());
    ...
    function RescueAllETHSentToContractAddress() external OnlyDirector()  {   
        ...
        PayableInputAddress(directorAddress).transfer(balanceOfContract);
        emit ETHwithdrawnRecovered(directorAddress, balanceOfContract, GetCurrentBlockTime());
    }

    function RescueAmountETHSentToContractAddress(uint256 amountToRescue) external OnlyDirector()  {   
        PayableInputAddress(directorAddress).transfer(amountToRescue);
        emit ETHwithdrawnRecovered(directorAddress, amountToRescue, GetCurrentBlockTime());
    }
    ...
    function RescueAllTokenSentToContractAddressAsDirector(IERC20 tokenToWithdraw) external OnlyDirector() {
        ...
        tokenToWithdraw.safeTransfer(PayableInputAddress(directorAddress), balanceOfContract);
        emit ERC20tokenWithdrawnRecovered(directorAddress, balanceOfContract, GetCurrentBlockTime());
    }
    ...
    function RescueAmountTokenSentToContractAddressAsDirector(IERC20 tokenToWithdraw, uint256 amountToRescue) external OnlyDirector() {
        tokenToWithdraw.safeTransfer(PayableInputAddress(directorAddress), amountToRescue);
        emit ERC20tokenWithdrawnRecovered(directorAddress, amountToRescue, GetCurrentBlockTime());
    }
    ...
    function RescueAllTokenSentToContractAddressAsManager(IERC20 tokenToWithdraw) external OnlyStaff(address(tokenToWithdraw)) {
        ...
        uint256 balanceOfContract = tokenToWithdraw.balanceOf(address(this));
        tokenToWithdraw.safeTransfer(PayableInputAddress(directorAddress), balanceOfContract);
        emit ERC20tokenWithdrawnRecovered(managerOfToken, balanceOfContract, GetCurrentBlockTime());
    }
    ...
    function RescueAmountTokenSentToContractAddressAsManager(IERC20 tokenToWithdraw, uint256 amountToRescue) external OnlyStaff(address(tokenToWithdraw)) {
        ...
        address managerOfToken = tokenAddressToManagerAddress[tokenAddress];
        tokenToWithdraw.safeTransfer(PayableInputAddress(directorAddress), amountToRescue);
        emit ERC20tokenWithdrawnRecovered(managerOfToken, amountToRescue, GetCurrentBlockTime());
    }
    //////////////////////////// RESCUE FUNCTIONS ////////////////////////////
