...
contract EGM{
   ...
   IBEP20 private egmToken; 
   event onBuy(address buyer , uint256 amount);
   mapping(address => uint256) public boughtOf;

    constructor(address ownerAddress,IBEP20 _egmToken) public 
    {
        owner = ownerAddress;
	...
	function buyToken(uint tokenQty,address _referrer) public payable
	{
		 ...
		 egmToken.transfer(msg.sender , (tokenQty));
		 if(msg.sender!=owner)
		 {
		    egmToken.transfer(users[msg.sender].referrer , ((tokenQty*refPercent)/100));
		    users[users[msg.sender].referrer].refIncome=users[users[msg.sender].referrer].refIncome+(tokenQty*refPercent)/100;
		 }
