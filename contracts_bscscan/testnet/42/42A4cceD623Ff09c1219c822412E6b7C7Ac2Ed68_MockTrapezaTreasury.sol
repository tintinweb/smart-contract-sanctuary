...
contract MockTrapezaTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sFIDL;
  uint256 public sFIDLQueue; // Delays change to sFIDL address
  ...
  constructor(
    ...
    // isLiquidityToken[ _FIDLBUSD ] = true;
    // liquidityTokens.push( _FIDLBUSD );

    blocksNeededForQueue = _blocksNeededForQueue;
  }

  /**
        @notice allow approved address to deposit an asset for FIDL
        @param _amount uint
  ...
  function deposit(
    ...
    (_token, _amount);
    // mint FIDL needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(FIDL).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn FIDL for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    IFIDLERC20(FIDL).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sFIDL).balanceOf(msg.sender); // Can only borrow against sFIDL held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with FIDL
        @param _amount uint
     */
  function repayDebtWithFIDL(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    IFIDLERC20(FIDL).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, FIDL, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(FIDL).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(FIDL).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns FIDL valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match FIDL decimals
      value_ = _amount.mul(10**IERC20(FIDL).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SFIDL) {
      // 9
      sFIDLQueue = block.number.add(blocksNeededForQueue);
    } else return false;
