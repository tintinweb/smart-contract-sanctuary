...
interface IWhiteList {
    function whiteListValid(address user) external view returns(uint256);
}

pragma experimental ABIEncoderV2;

contract Monster is ERC1155, Ownable {
    ...
    bool public isWhiteOpen = false;

    IWhiteList private _white = IWhiteList(0x3563EF7653261Fdc4c0Fa36849485Ddc619F290d);

    mapping (uint256 => bool) public _openWhiteMap;
    ...
    uint256 public totalCount = 45000;
    uint256 public totalSupply = 0;

    constructor() public ERC1155("https://game.example/api/item/{id}.json") {
        _supplyCount[6] = 20000;  //godzilla
        _tokenUri[6] = "https://upgrade.ozlys.com/icon8/icon8.png";
        _supplyCount[5] = 11000;  //tyranno
        _tokenUri[5] = "https://upgrade.ozlys.com/icon8/icon7.png";
        _supplyCount[4] = 4500;  //tyranno
        _tokenUri[4] = "https://upgrade.ozlys.com/icon8/icon6.png";
        _supplyCount[3] = 5850;  //turtle
        _tokenUri[3] = "https://upgrade.ozlys.com/icon8/icon5.png";
        _supplyCount[2] = 6750;  //smaug
        _tokenUri[2] = "https://upgrade.ozlys.com/icon8/icon4.png";
        _supplyCount[1] = 24750;  //alien
        _tokenUri[1] = "https://upgrade.ozlys.com/icon8/icon3.png";
    }

    function setCoinUri(uint256 coinType, string memory uri) public onlyOwner {
        _tokenUri[coinType] = uri;
    }

    function tokenUri(uint256 coinType) public view returns(string memory _uri) {
        return _tokenUri[coinType];
    }

    function setOpenWhiteType(uint256[] memory types) public onlyOwner returns(bool) {
        for (uint i = 1; i < 7; ++i) {
            _openWhiteMap[i] = false;
        }
        for (uint i = 0; i < types.length; i++) {
            _openWhiteMap[types[i]] = true;
        }
        return true;
    }
    ...
    function setWhiteStatus(bool state) public onlyOwner {
        isWhiteOpen = state;
    }
    ...
    function buyCoin(uint256 numberOfNfts, uint256 coinType) public payable returns(bool) {
        require(isStartSale, "sale not started.");
    ...
    function claimWhite() public returns(bool) {
        require(isWhiteOpen, "activity not exist.");
        uint256 validTime = _white.whiteListValid(msg.sender);
        require(validTime <block.timestamp, "user not have permission to claim.");
      
        uint256 openCount = 0;
        for (uint i = 1; i < 7; i++) {
            if (_openWhiteMap[i]) {openCount = openCount.add(1);}
        }
        require(openCount>0, "whitelist not open.");

        uint256 random = uint256(keccak256(abi.encodePacked(block.difficulty, now)));
        uint256 coinType = random.mod(openCount);
        
        require(_saleCount[coinType].add(1) < _supplyCount[coinType], "coin not enough to sell");

        _mint(msg.sender, coinType, 1, _tokenUri[coinType], "");
        _saleCount[coinType] = _saleCount[coinType].add(1);
