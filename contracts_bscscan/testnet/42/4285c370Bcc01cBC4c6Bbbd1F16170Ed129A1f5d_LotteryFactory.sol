...
contract LotteryFactory is OnlyType,ILotteryFactory {
    ...
    function startOrStop()external override onlyType {
        ...
        _startOrStop(lotteryId);
    }
    
    function _startOrStop(uint256 _lotteryId) private{
    ...
    function _createTicket(uint _tType,address _to,uint256 _amount) private{
    ...
    function _isBuyTicket(address _from,uint _tType)private view returns (bool){
        LotteryInfo memory lottery = lotteries[_tType];
    ...
    function _addPhase(LotteryInfo storage lottery) private {
        require(lottery.started,"lottery not start."); 
        if(lottery.isFree){
    ...
    function _publish(uint _salt)private {
        ...
        LotteryInfo memory lottery = lotteries[tType];
        if(lottery.currentPhaseId==0||lottery.inTicket==0){
            return;
        }
        require(lottery.started,"lottery not start."); 
