...
import './libs/introspection/IERC165.sol';
...
import './WagashiProfile.sol';

pragma solidity ^0.6.12;

contract ClaimBackWagashi is Ownable {
    using SafeBEP20 for IBEP20;

    IBEP20 public wagashiToken;
    WagashiProfile wagashiProfile;

    uint256 public numberWagashi;
    uint256 public thresholdUser;

    mapping(address => bool) public hasClaimed;

    constructor(
        IBEP20 _wagashiToken,
        address _wagashiProfileAddress,
        uint256 _numberWagashi,
        uint256 _thresholdUser
    ) public {
        wagashiToken = _wagashiToken;
        wagashiProfile = WagashiProfile(_wagashiProfileAddress);
        numberWagashi = _numberWagashi;
        thresholdUser = _thresholdUser;
    }

    function getWagashiBack() external {
        // 1. Check if she has registered
        require(wagashiProfile.hasRegistered(_msgSender()), "not active");

        // 2. Check if she has claimed
        require(!hasClaimed[_msgSender()], "has claimed WAGASHI");

        // 3. Check if she is active
        uint256 userId;
        (userId, , , , , ) = wagashiProfile.getUserProfile(_msgSender());
        ...
        // Transfer WAGASHI tokens from this contract
        wagashiToken.safeTransfer(_msgSender(), numberWagashi);
    }

    /**
     * @dev Claim WAGASHI back.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        wagashiToken.safeTransfer(_msgSender(), _amount);
    }

    function canClaim(address _userAddress) external view returns (bool) {
        if (!wagashiProfile.hasRegistered(_userAddress)) {
            return false;
        } else if (hasClaimed[_userAddress]) {
            return false;
        } else {
            uint256 userId;
            (userId, , , , , ) = wagashiProfile.getUserProfile(_userAddress);
...
contract WagashiProfile is AccessControl, ERC721Holder {
    ...
    uint256 public numberWagashiToReactivate;
    uint256 public numberWagashiToRegister;
    uint256 public numberWagashiToUpdate;
    ...
    constructor(
        IBEP20 _wagashiToken,
        uint256 _numberWagashiToReactivate,
        uint256 _numberWagashiToRegister,
        uint256 _numberWagashiToUpdate
    ) public {
        wagashiToken = _wagashiToken;
        numberWagashiToReactivate = _numberWagashiToReactivate;
        numberWagashiToRegister = _numberWagashiToRegister;
        numberWagashiToUpdate = _numberWagashiToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends WAGASHI to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer WAGASHI tokens to this contract
        wagashiToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberWagashiToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer WAGASHI token to this address
        wagashiToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberWagashiToUpdate
        );
    ...
    /**
     * @dev Claim WAGASHI to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        wagashiToken.safeTransfer(_msgSender(), _amount);
    ...
    /**
     * @dev Update the number of WAGASHI to register
     * Callable only by owner admins.
     */
    function updateNumberWagashi(
        uint256 _newNumberWagashiToReactivate,
        uint256 _newNumberWagashiToRegister,
        uint256 _newNumberWagashiToUpdate
    ) external onlyOwner {
        numberWagashiToReactivate = _newNumberWagashiToReactivate;
        numberWagashiToRegister = _newNumberWagashiToRegister;
        numberWagashiToUpdate = _newNumberWagashiToUpdate;
    }
