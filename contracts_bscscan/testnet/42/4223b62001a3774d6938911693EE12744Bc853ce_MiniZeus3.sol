...
contract MiniZeus3 is Context, IERC20, Ownable {
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _txFee,uint256 _lpFee,uint256 _DexFee,address routerAddress,address feeaddress,address tokenOwner) {
        _name = _NAME;
        _symbol = _SYMBOL;
        _decimals = 9;
        ...
        numTokensSellToAddToLiquidity = 50000000000000 * 10 ** 9;
    ...
    function _takeDev(uint256 tDev) private {
        ...
        /*if(devFeeEnabled) {
            uint256 swapTokens = contractTokenBalance.div(totalFees).mul(devFee);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 half = contractTokenBalance.div(6); // 1/6 for half liq
        uint256 otherHalf = contractTokenBalance.div(6); // 1/6 for half liq 
        uint256 devFeeBNB = contractTokenBalance.sub(half).sub(otherHalf); // 4/6 for dev fee
        ...
        swapTokensAndSendToDev(devFeeBNB);
    ...
    function swapTokensAndSendToDev(uint256 tokenAmount) private {
        swapTokensForEth(tokenAmount);
        payable(_devWalletAddress).transfer(address(this).balance);
    }
    ...
    function withdrawStuckBNB() public {
        require(_msgSender() == _devWalletAddress, "only dev can withdraw");
