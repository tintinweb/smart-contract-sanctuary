...
// File: contracts/M7Finance_contracts/autovault/IStrategy.sol
pragma solidity ^0.6.0;
...
// File: contracts/M7Finance_contracts/autovault/CIVault.sol
pragma solidity ^0.6.0;
 ...
 */
contract CIVault is ERC20, Ownable {
    ...
    IERC20 public wbnb = IERC20(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd);//0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c
    //
    ...
    function directDeposit(uint256 _amount) public {

        uint256 _pool = balance();

        require(_amount > 0, "Cannot swap 0");
        uint256 userBalance = IERC20(wbnb).balanceOf(msg.sender);
        require(userBalance >= _amount, "_amount must be greater than userBalance");
        ...
        _approve(address(this), address(pancakeswapV2Router), wbnbBal);
        ...
        uint256 gotAmount = token.balanceOf(address(this));

        uint256 shares = 0;
        if (totalSupply() == 0) {
            shares = gotAmount;
        } else {
            shares = (gotAmount.mul(totalSupply())).div(_pool);
        }
        _mint(msg.sender, shares);
    ...
    function testDirectDeposit(uint256 _amount) public payable{
