...
contract NFTsRealmMarketplace is ReentrancyGuard, Ownable {
    ...
    function buyNew(uint256 _id, 
        ...
        if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("KCS")))) {
            ...
            require(IERC20(tokenAddressMap[paymentType]).transferFrom(payable(msg.sender), payable(payoutAddressMap[paymentType]), _commissionValue.div(2)));        
        }
        ...
        onlyCreatorOrWinner(tokenId)
    {
        ...
        if(winner == address(0)) {
            ...
            if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("KCS")))) {
                ...
                require(IERC20(tokenAddressMap[paymentType]).transfer(payable(payoutAddressMap[paymentType]), _commissionValue.div(2)));
                if (Creator == creatorMap[tokenId]) {
