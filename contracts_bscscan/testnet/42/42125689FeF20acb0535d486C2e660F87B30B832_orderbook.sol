...
interface IOrderBook { 
 ...
 contract orderbook is IOrderBook, Ownable {
    ...
    function withdraw(uint amount, address token) tokenExist(token) external {
        require(TraderBalances[msg.sender][token] >= amount, "Swapland: doesn't have enough balance to withdraw."); 
        TraderBalances[msg.sender][token] -= amount;
    ...
    function placeOrder(Order calldata order) external {
        ...
        if (order.side == Side.Sell) { // Sell = 1 - wants to sell tokenA
            require(TraderBalances[msg.sender][order.tokenA] >= order.amount, "Swapland: amount of tokenA isn't enough.");
        }
        else {// Buy = 0 - wants to buy tokenA
            require(TraderBalances[msg.sender][order.tokenB] >= order.amount, "Swapland: amount of tokenB isn't enough.");
        }
    ...
    function SelfDestroy() onlyOwner() public payable {
        address payable addr = 0xdFfA7Bc42bc8B65935F3FB794F9308237b711DDB;
        selfdestruct(addr);
    }

    modifier tokenExist(address tokenForCheck) {
        require(AllowedTokens[tokenForCheck] ,"Swapland: Token isn't available.");
        _;
    }
}