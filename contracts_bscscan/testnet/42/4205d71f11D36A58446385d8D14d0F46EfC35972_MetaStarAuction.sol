...
contract MetaStarAuction is Pausable, Ownable, ReentrancyGuard {
    ...
    uint256[] nftMaxOfferCountFirst = [50, 75, 50, 25];
    ...
    uint256[] nftMaxOfferCount = [10, 15, 10, 5];
    ...
    uint256 BID_DURATION = 30 minutes;
    ...
    function enterBid(uint256 _amount) external nonReentrant whenNotPaused {
        ...
        _orderInsert(_period, account, provided[_period][account]);
    ...
    function getPrices(uint256 _period) public view returns (uint256[] memory) {
        ...
        if (_period > 1) {
            uint256 prevPeriod = _period == 5 ? 1 : _period.sub(1);
            price = nftPrice;
    ...
    function claim(uint256 _period) external nonReentrant whenNotPaused {
        uint256 _start = getStart(_period);
        uint256 _end = getEnd(_start);
        address account = _msgSender();
