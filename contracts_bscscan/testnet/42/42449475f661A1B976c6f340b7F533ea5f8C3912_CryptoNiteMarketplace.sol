...
contract CryptoNiteMarketplace is ReentrancyGuard, Ownable {
    ...
    uint256 public marketFeeForBNB = 100;
    ...
    function buyNew(
        ...
        if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("BNB")))) {
            ...
            transferBNBOrWBNB(payable(_creator), _sellerValue);
            transferBNBOrWBNB(payable(adminRecoveryAddress), _commissionValue.div(2));
            transferBNBOrWBNB(payable(payoutAddressMap[paymentType]), _commissionValue.div(2));
