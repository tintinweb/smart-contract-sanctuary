...
contract NFTMedal is Authorizable, ERC1155 {
    ...
    constructor(string memory _baseURI,uint _totalMedalType) public ERC1155(_baseURI) {
        name = "4PlayMedal";
        symbol = "4PLM";
        _setURI(_baseURI);
    ...
    function  changeMedalTypes(uint _totalMedalType) public onlyOwner(){
        totalMedals = _totalMedalType;
    }
    ...
    function dropMedal(address[] memory _addresses,uint256[] memory  _quantities,uint256 _id) public {
    ...
    function batchMint(address _to,uint256[] memory _ids,uint256[] memory _quantities) public onlyAuthorized() {
        require(_to != address(0),"NFTMedal: enter valid address");
    ...
    function burn(address _account,uint256 _id,uint256 _amount) public validTokenId(_id) {
    ...
    function batchBurn(address _account,uint256[] memory _ids,uint256[] memory _quantities) public {
        require(msg.sender == _account || isApprovedForAll(_account,msg.sender),"NFTMedal: caller is not owner nor approved");
        for (uint256 i = 0; i < _ids.length; i++) {
            uint256 _id = _ids[i];
