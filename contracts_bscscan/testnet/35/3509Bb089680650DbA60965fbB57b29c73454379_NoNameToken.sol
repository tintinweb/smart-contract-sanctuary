...
contract NoNameToken is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'NoNameToken';
    string private constant _symbol = 'NNT';
    uint8 private constant _decimals = 9;
    uint256 public constant InitialSupply= 100000000 * 10**_decimals; //100,000,000

    //Divider for the MaxBalance based on circulating Supply 
    uint8 public constant BalanceLimitDivider=50; // 2,000,000 (2%)
    //Divider for sellLimit based on circulating Supply 
    uint16 public constant SellLimitDivider=400; // 250,000 (0.25%)
    ...
    uint16 public constant MaxSellLockTime= 20; // 20 Seconds
    ...
    uint16 public constant MaxBuyLockTime= 5; // 5 Seconds
    ...
    //DevWallets
    address public TeamWallet=payable(0xAaA6f5E740c595a1B0004E71f28FC0021C7B7044);
    //TestNet
	...
	uint256 private antiWhale = _circulatingSupply; // Set as circulating supply at start. You can set this as " = AMOUNT * 10**_decimals;" if not using auto tax changes. Otherwise, change below in the isSell/isBuy section.
	
	//Tax options -- 0 = Disbaled, 1 = Enabled -- Time in seconds
	uint256 private enableSellCount = 0;
	uint256 private enableBuyCount = 0;
	uint256 private enableSellTimestamp = 1;
	uint256 private enableBuyTimestamp = 1;
	uint256 private taxChangeTime = 600; // 10 Minutes in seconds.
	
	//Sell & Buy Counter Variables (DO NOT CHANGE)
	uint256 private sellCount = 0; //DO NOT CHANGE, THIS IS FOR COUNTING SELLS FOR AUTO TAX CHANGES
	uint256 private buyCount = 0; //DO NOT CHANGE, THIS IS FOR COUNTING BUYS FOR AUTO TAX CHANGES
	uint256 private tradingEnabledAt; //DO NOT CHANGE, THIS IS FOR HOLDING THE TIMESTAMP
    ...
    constructor () {
		...
		//Sets sellLockTime
        sellLockTime=20; // 20 Seconds
		
		//Sets buyLockTime
        buyLockTime=2; // 2 Seconds

        //Set Starting Tax (FOR AUTO TAX CHANGES: Set these to your higher or lower tax rates, then below in the "isSell & inBuy" sections set what you want them to change to afterwards.)
        //Auto tax changes must be enabled above to work.
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            //These are what your taxes will be changed to after default 2 sells. Change your taxes as you see fit, up or down. If you want to change when they change modify 
            //the "if(sellCount == 2)" to whatever number you want.
            if(sellCount == 2 && enableSellCount == 1) {
                _buyTax=5;
                _sellTax=5;
                _transferTax=5;
                ...
                antiWhale = 1000000 * 10**_decimals;
            }
            sellCount = sellCount + 1;
            //Timed tax changes.
            if(block.timestamp >= tradingEnabledAt + taxChangeTime && enableSellTimestamp == 1) {
                _buyTax=5;
                _sellTax=5;
                _transferTax=5;
                ...
                enableSellTimestamp = 0;
                enableBuyTimestamp = 0;
                antiWhale = 1000000 * 10**_decimals;
            }
            ...
            //These are what your taxes will be changed to after default 2 sells. Change your taxes as you see fit, up or down. If you want to change when they change modify 
            //the "if(sellCount == 2)" to whatever number you want.
            if(buyCount == 2 && enableBuyCount == 1) {
                _buyTax=5;
                _sellTax=5;
                _transferTax=5;
            ...
            buyCount = buyCount + 1;
            //Timed tax changes
            if(block.timestamp >= tradingEnabledAt + taxChangeTime && enableBuyTimestamp == 1) {
                _buyTax=5;
                _sellTax=5;
                _transferTax=5;
            ...
            if(amount<=10**(_decimals)) claimUSDT(sender);
            //Checks If the recipient balance(excluding Taxes) would exceed Balance Limit
            require(recipientBalance+amount<=balanceLimit,"whale protection");
    ...
    //marketing share starts at 80% will be changed 20 min after launch
    ...
    //gets the not dividends of a staker that aren't in the toBePaid mapping 
    //returns wrong value for excluded accounts
    function _newDividendsOf(address staker) private view returns (uint256) {
    ...
    event OnWithdrawUSDT(uint256 amount, address recipient);
    
    //withdraws all dividends of address
    function claimUSDT(address addr) private{
        ...
        path[1] = 0x55d398326f99059fF775485246999027B3197955; //BUSDT
        ...
        emit OnWithdrawUSDT(amount, addr);
        _isWithdrawing=false;
    }
    ...
    function getSellCount() public view returns (uint256 _sellCount){
        return (sellCount);
    }
    
    function getBuyCount() public view returns (uint256 _buyCount){
        return (buyCount);
    }
    
    function getTaxChangeTime() public view returns (uint256 _buySellRatio){
        return (tradingEnabledAt + taxChangeTime);
    }
    ...
    function withdrawBUSDT() public{
        claimUSDT(msg.sender);
    }
    ...
    function TeamWithdrawALLMarketingBNB() public onlyOwner{
        ...
        address[] memory path = new address[](2);
        path[0] = _pancakeRouter.WETH(); //BNB
        path[1] = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684; //USDT(Testnet Address)

        _pancakeRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount / 2}(
        0,
        path,
        TeamWallet,
        block.timestamp);
