...
contract KiBridgeV8b {
  ...
  function calculateToAllChainableBalance(
    ...
    returns (uint256 amount) 
  {
    if (liquidityInfo[_token].toAllChainableBalance + _fulfilled >= _pening) return _amount;
    if (liquidityInfo[_token].toAllChainableBalance + _fulfilled < _pening) return 0;
  }
