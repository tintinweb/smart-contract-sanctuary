...
import "./sub_contracts/WJXN_VRP.sol";
import "./sub_contracts/WJAX_JUSD.sol";
import "./sub_contracts/JUSD_JINR.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";

contract Admin is Initializable, Governance, WJXN_VRP, WJAX_JUSD, JUSD_JINR {
    ...
    function initialize() public initializer {
        ...
        rate_decimal = 8;
        // WJAX_JUSD exchange
        wjax_jusd_markup_fee = 1000000;
        wjax_jusd_markup_fee_wallet = sender;
        wjax_bnb_pair = 0xa7405ae3D18330c979AD5Ee7CF970e67220e0021;
        usd_bnb_pair = 0x58F876857a02D6762E0101bb5C46A8c1ED44Dc16;
        // JUSD_JINR exchange
        usd_inr_rate = 7500000000;
    ...
    function pause_system(uint flag) public onlyAdmin {
...
contract WJXN_VRP is Governance{

    function withdraw_wjxn(uint _amount) public onlyGovernor {
        require(validate_wjxn_withdrawal() == true, "validate_wjxn_withdrawal failed");
    ...
    function exchange_wjxn_vrp(uint wjxn_amount) public isActive {
        require(wjxn_amount > 0, "WJXN amount must not be zero");
        require(wjxn.balanceOf(msg.sender) >= wjxn_amount, "Insufficient fund");
        wjxn.transferFrom(msg.sender, address(this), wjxn_amount);
        uint wjxn_vrp_ratio = vrp.totalSupply() / wjxn.balanceOf(address(this));
        if( vrp.totalSupply() == 0 ){
            wjxn_vrp_ratio = (10 ** vrp.decimals());
        }
        uint vrp_to_be_minted = wjxn_amount * wjxn_vrp_ratio;
        vrp.mint(msg.sender, vrp_to_be_minted);
    }

    function exchange_vrp_wjxn(uint vrp_amount) public isActive {
        require(vrp_amount > 0, "VRP amount must not be zero");
        require(wjxn.balanceOf(address(this))> 0, "Insufficient supply");
        uint vrp_wjxn_ratio = wjxn.balanceOf(address(this)) * (10 ** (vrp.decimals()*2) ) / vrp.totalSupply();
        uint wjxn_to_be_withdrawn = vrp_amount * vrp_wjxn_ratio / (10 ** (vrp.decimals()*2) );
        require(wjxn.balanceOf(address(this))>= wjxn_to_be_withdrawn, "Insufficient supply");
...
contract WJAX_JUSD is Governance {

    address wjax_bnb_pair; // = 0xa7405ae3D18330c979AD5Ee7CF970e67220e0021;
    address usd_bnb_pair; // = 0x58F876857a02D6762E0101bb5C46A8c1ED44Dc16;
    
    function withdraw_wjax(uint _amount) public onlyGovernor {
        require(validate_wjax_withdrawal() == true, "validate_wjax_withdrawal failed");
	...
	/*
        buy XUSD from BEE
	*/
	function exchange_wjax_jusd(uint jusd_amount) public isActive {
        uint256 wjax_price = get_wjax_bnb_price();
        uint256 busd_price = get_usd_bnb_price();
        uint256 wjax_amount = jusd_amount * busd_price / wjax_price;
        // Calculate fee
        uint256 fee_amount = calculate_wjax_jusd_markup_fee(wjax_amount);
        // markup fee wallet will receive fee
		require(wjax.balanceOf(msg.sender) >= wjax_amount + fee_amount, "Insufficient fund");
        // pay fee
        wjax.transferFrom(msg.sender, wjax_jusd_markup_fee_wallet, fee_amount);
        wjax.transferFrom(msg.sender, address(this), wjax_amount);

        jusd.mint(msg.sender, jusd_amount);
		emit Exchange_WJAX_JUSD(msg.sender, jusd_amount, wjax_amount);
	}

    /*
        sell XUSD and receive wjax
	*/
	function exchange_jusd_wjax(uint jusd_amount) public isActive {
        uint256 wjax_price = get_wjax_bnb_price();
        uint256 busd_price = get_usd_bnb_price();
        uint256 wjax_amount = jusd_amount * busd_price / wjax_price;
		require(wjax.balanceOf(address(this)) >= wjax_amount, "Insufficient supply");
		require(jusd.balanceOf(msg.sender) >= jusd_amount, "Insufficient fund");
        jusd.burnFrom(msg.sender, jusd_amount);
        uint256 fee_amount = calculate_wjax_jusd_markup_fee(jusd_amount);
        // The recipient has to pay fee. 
        wjax.transfer(wjax_jusd_markup_fee_wallet, fee_amount);
        wjax.transfer(msg.sender, wjax_amount - fee_amount);
		emit Exchange_JUSD_WJAX(msg.sender, jusd_amount, wjax_amount);
	}

    function set_wjax_jusd_markup_fee(uint _wjax_jusd_markup_fee) public onlyGovernor {
        wjax_jusd_markup_fee = _wjax_jusd_markup_fee;
    }

    function set_wjax_jusd_markup_fee_wallet(address _wallet) public onlyGovernor {
    ...
    function get_usd_bnb_price() internal returns(uint)
    {
        IPancakeswapV2Pair pair = IPancakeswapV2Pair(usd_bnb_pair);
    ...
    function get_wjax_bnb_price() internal returns(uint)
    {
        IPancakeswapV2Pair pair = IPancakeswapV2Pair(wjax_bnb_pair);
        (uint Res0, uint Res1,) = pair.getReserves();

        // decimals
        if( pair.token1() == address(wjax) ){
            return Res0 * (10**wjax.decimals()) /Res1;
        }
        return Res1 * (10**wjax.decimals()) / Res0; // return amount of token0 needed to buy token1
    }

    function get_wjax_busd_price() public view returns(uint)
    {
        return (10 ** 18) * 2435 / 5;
    }
...
contract JUSD_JINR is Governance {
    
    uint public usd_inr_rate;
    uint public rate_decimal;
    ...
    function convert_jusd_jinr(uint _amount) public isActive {
        uint256 jinr_amount = _amount * usd_inr_rate / (10 ** rate_decimal);
    ...
    function convert_jinr_jusd(uint _amount) public isActive {
        uint256 jusd_amount = _amount / usd_inr_rate * (10 ** rate_decimal);
        // Calculate Fee on receiver side
    ...
    function set_usd_inr_rate(uint _rate) public onlyGovernor {
        require(validate_conversion_rates(_rate) == true, "USD-INR Conversion rate is not valid.");
        usd_inr_rate = _rate;
    }

    function set_jusd_jinr_markup_fee(uint _markup_fee) public onlyGovernor {
        jusd_jinr_markup_fee = _markup_fee;
    }

    function set_jusd_jinr_markup_fee_wallet(address _wallet) public onlyGovernor {
...
import "../tokens/WJAX.sol";
import "../tokens/VRP.sol";
...
contract Governance {
    ...
    modifier onlyAdmin() {
        require(admin == msg.sender, "not Admin of this contract");
        _;
    }

    modifier isActive() {
        require(current_system_state == 0, "current system state is not active");
        _;
    }
    ...
    modifier onlyGovernor() {
        require(governor == msg.sender, "not Governor of this contract");
        _;
    }
    ...
    function validate_wjxn_withdrawal() public view onlyGovernor returns (bool) {
        return true;
    }

    function validate_wjax_withdrawal() public view onlyGovernor returns (bool) {
        return true;
    }

    function validate_conversion_rates(uint _rate) public view onlyGovernor returns (bool) {
        return true;
    }

    function set_document(string memory _readme_hash, string memory _readme_link) public onlyGovernor {
        readme_hash = _readme_hash;
        readme_link = _readme_link;
    }

    function set_jinr_transaction_fee(uint _tx_fee) public onlyGovernor {
        jinr.setTransactionFee(_tx_fee);
    }

    function set_jinr_transaction_fee_wallet(address _wallet) public onlyGovernor {
        jinr.setTransactionFeeWallet(_wallet);
    }

    function set_jusd_transaction_fee(uint _tx_fee) public onlyGovernor {
        jusd.setTransactionFee(_tx_fee);
    }

    function set_jusd_transaction_fee_wallet(address _wallet) public onlyGovernor {
        jusd.setTransactionFeeWallet(_wallet);
    }
    ...
    function transferOwnershipOfTokens(address newOwner) public onlyAdmin {
        wjax.transferOwnership(newOwner);
...
import "../lib/BEP20Fee.sol";

contract WJAX is BEP20, BEP20Fee {

    constructor() 
        BEP20("WJAX token", "WJAX")
        payable
    {
        _setupDecimals(18);
