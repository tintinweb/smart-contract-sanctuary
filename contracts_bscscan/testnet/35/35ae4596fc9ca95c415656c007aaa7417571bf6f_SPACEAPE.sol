...
import "./SPACEAPEDividendPayingToken.sol";
...
contract SPACEAPE is ERC20, Ownable {
    ...
    address public immutable BANANA = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); // BANANA
	address payable public MarketingWallet = payable(0x5134789f1FDE0CDe4944481716f85D40b9E870F3); // Marketing Address

    bool private swapping;

    SPACEAPEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("TestToken", "TestkToken") {
        ...
        dividendTracker = new SPACEAPEDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SPACEAPE: The dividend tracker already has that address");

        SPACEAPEDividendTracker newDividendTracker = SPACEAPEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SPACEAPE: The new dividend tracker must be owned by the SPACEAPE token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SPACEAPE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PancakeSwapV2Pair, "SPACEAPE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SPACEAPE: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SPACEAPE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SPACEAPE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SPACEAPE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!tradingOpen) {
            require(canTransferBeforetradingOpen[from], "SPACEAPE: This account cannot send tokens until trading is enabled");
        }
...
contract SPACEAPEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() DividendPayingToken("SPACEAPE_Dividend_Tracker", "SPACEAPE_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(true, "SPACEAPE_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "SPACEAPE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SPACEAPE_Dividend_Tracker: Cannot update claimWait to same value");
