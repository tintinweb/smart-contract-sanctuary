    ...
    AccessControl
{
    ...
    mapping(address => bool) public autoExeciseStatus;

    event AutoExeciseStatusChange(address indexed account, bool status);

    function setAutoExeciseStatus(bool status) public {
        autoExeciseStatus[msg.sender] = status;
        emit AutoExeciseStatusChange(msg.sender, status);
    }
...
contract BufferBNBOptions is BaseBufferBNBOptions
{
    ...
    function canExercise(uint256 optionID) internal view returns (bool){
        ...
        bool isAutoExerciseTrue = autoExeciseStatus[tokenOwner] && msg.sender == owner();
