...
//    address BTCB = 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c;
    address BTCB = 0xBeaf31689314EC4A367Aa42Ea2B18BC39793B8b0;
//    address ETH = 0x2170Ed0880ac9A755fd29B2688956BD959F933F8;
    address ETH = 0xBeaf31689314EC4A367Aa42Ea2B18BC39793B8b0;
    ...
    function distributeDividendsBTCB(uint256 amount) public override onlyOwner {
    ...
    function distributeDividendsETH(uint256 amount) public override onlyOwner {
        require(totalSupply() > 0);

        if (amount > 0) {
            magnifiedDividendPerShare.ETH = magnifiedDividendPerShare.ETH.add(
                (amount).mul(magnitude) / totalSupply()
            );
            emit DividendsDistributed(msg.sender, amount, "ETH");

            _totalDividendsDistributed.ETH = _totalDividendsDistributed.ETH.add(amount);
        }
    }
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (Tokens memory withdrawable) {
        ...
        if (withdrawable.ETH > 0 ) {
            withdrawnDividends[user].ETH = withdrawnDividends[user].ETH.add(withdrawable.ETH);
            emit DividendWithdrawn(user, withdrawable.ETH, "ETH");

            bool success = IERC20(ETH).transfer(user, withdrawable.ETH);
            if(!success) {
                withdrawnDividends[user].ETH = withdrawnDividends[user].ETH.sub(withdrawable.ETH);
                withdrawable.ETH = 0;
            }
        }
    ...
    function withdrawableDividendOf(address _owner) public view override returns(Tokens memory acc) {
        ...
        acc.ETH -= withdrawnDividends[_owner].ETH;
    ...
    function accumulativeDividendOf(address _owner) public view override returns(Tokens memory) {
        Tokens memory acc;
        acc.ETH = magnifiedDividendPerShare.ETH.mul(balanceOf(_owner)).toInt256Safe()
        .add(magnifiedDividendCorrections[_owner].ETH).toUint256Safe() / magnitude;
    ...
    function _mint(address account, uint256 value) internal override {
        ...
        magnifiedDividendCorrections[account].ETH = magnifiedDividendCorrections[account].ETH
        .sub((magnifiedDividendPerShare.ETH.mul(value)).toInt256Safe() );
    ...
    function _burn(address account, uint256 value) internal override {
        ...
        magnifiedDividendCorrections[account].ETH = magnifiedDividendCorrections[account].ETH
        .add((magnifiedDividendPerShare.ETH.mul(value)).toInt256Safe() );
        magnifiedDividendCorrections[account].BTCB = magnifiedDividendCorrections[account].BTCB
...
contract DividendTracker is DividendPayingToken {
    ...
    function processAccount(address payable account, bool automatic) public onlyOwner returns (bool) {
        ...
        if(w.ETH > 0) {
            emit Claim(account, w.ETH, automatic);
        } else {
            res = false;
        }
        if(w.BTCB > 0) {
...
interface IDividendPayingToken is AdditionalTypes {
    ...
    function distributeDividendsBTCB(uint256 amount) external;
    function distributeDividendsETH(uint256 amount) external;
