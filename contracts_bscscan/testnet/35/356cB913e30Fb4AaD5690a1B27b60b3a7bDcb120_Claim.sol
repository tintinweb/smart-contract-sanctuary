    ...
    OwnableUpgradeable
{
    ...
    function increaseWolfPackLink(uint256 _wolfPackId, uint256 _amountToIncrease) public {
        wolfPackLinkDays[_wolfPackId] = wolfPackLinkDays[_wolfPackId] + _amountToIncrease;
        emit IncreasedLink(_wolfPackId, _amountToIncrease);
    }

    function getWolfPackLength(uint256 _wolfPackId) public view returns (uint256) {
    ...
    function getConfigurationLinkDays(uint256 _amountOfDays) external view returns (bool) {
    ...
    OwnableUpgradeable
{
        ...
        returns (uint256 commission)
    {
        if (_positionToAttack < 5) {
            ...
            return gasToMinter + ((gasToMinter * 1000) / 10000);
        } else if (_positionToAttack >= 10 && _positionToAttack <= 14) {
            return gasToMinter + ((gasToMinter * 12000) / 10000);
        } else if (_positionToAttack >= 15 && _positionToAttack <= 19) {
            return gasToMinter + ((gasToMinter * 20000) / 10000);
        } else if (_positionToAttack >= 20 && _positionToAttack <= 24) {
            return gasToMinter + ((gasToMinter * 30000) / 10000);
        } else if (_positionToAttack >= 25 && _positionToAttack <= 29) {
            return gasToMinter + ((gasToMinter * 50000) / 10000);
        }
    }
