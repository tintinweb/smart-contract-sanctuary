...
contract DinoMarketplaceUpgradeable is Pausable, Initializable {
  ...
  modifier onlyMarketManger() {
      require(msg.sender == marketManagerAddress, "need_market_manager_permission");
      _;
  }
  /// @dev Set new market manager address
  /// @param _newMarketManagerAddress - Address of new admin.
  function setMarketManagerAddress(address _newMarketManagerAddress) external onlyMarketManger {
  ...
  function setAdmin(address _adminAddress, bool _isAdmin) external onlyMarketManger {
  ...
  function setEggPriceByGenes(uint256 _eggGenes, uint256 _eggPrice) external onlyAdmin {
  ...
  function setTotalSellingEggByGenes(uint256 _eggGenes, uint256 _total) external onlyAdmin {
      totalSellingEggByGenes[_eggGenes] = _total;
  }
  
  function setSkipHatchCooldownPrice(uint256 _newSkipHatchCooldownPrice) external onlyAdmin {
  ...
  function getEggsByOwner(address _owner) external view returns(uint256[] memory) {
  ...
  function updateEggStatus(uint256 _eggId, bool _isAvailable) external onlyAdmin returns(uint256) {
  ...
  function withdrawBalance(address _to, uint256 _amount) external onlyMarketManger returns(bool) {
  ...
  /// @param _to - Receiver address
  function withdrawAllBalance(address _to) external onlyMarketManger returns(bool) {
  ...
  function cancelAuction(uint256 _tokenId) public {
    ...
    require(msg.sender == _auction.seller || admins[msg.sender], "dont_have_pemission");
  ...
  function cancelAllAuction() external onlyAdmin {
      uint256[] memory dinosOwnedByMarket = IDinoLandNFT(nftAddress).getDinosByOwner(address(this));
