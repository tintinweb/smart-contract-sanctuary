...
contract PeopleFutures {
    ...
    function initialPayment(address _sponsor, string memory _contractId) external onlyAdmin() {
    ...
    function reportEarnings(string memory _contractId, uint256 _earnings) external onlyAdmin() {
    ...
    function verifyEarningsReport(string memory _contractId,uint256 _earnings) external onlyAdmin() {
        uint256 currentYear = DateTime.getYear(block.timestamp);
    ...
    function payPortionOfEarnings(string memory _contractId) external onlyAdmin() {
        uint256 currentYear = DateTime.getYear(block.timestamp);
    ...
    function getTotalPayment(string memory _contractId) external onlyAdmin() view returns(uint256) {
        return escrows[_contractId].totalPayment;
    }

    function getEscrowPercent(string memory _contractId) external onlyAdmin() view returns(uint256) {
        return escrows[_contractId].escrowPercent;
    }

    function getProjectedEarnings(string memory _contractId) external onlyAdmin() view returns(uint256[] memory) {
        return escrows[_contractId].projectedEarnings;
    }

    function getCurrentYear() external onlyAdmin() view returns(uint256) {
        return DateTime.getYear(block.timestamp);
    }

    function getSponsor(string memory _contractId) external onlyAdmin() view returns(address) {
        return escrows[_contractId].sponsor;
    }

    function getNumberOfYears(string memory _contractId) external onlyAdmin() view returns(uint256) {
        return escrows[_contractId].numberOfYears;
    }

    function getBaseEscrow(string memory _contractId) external onlyAdmin() view returns(uint256[] memory) {
        return escrows[_contractId].baseEscrowBalances;
    }

    function getProjectedPayout(string memory _contractId) external onlyAdmin() view returns(uint256[] memory) {
        return escrows[_contractId].projectedPayout;
    }

    function getPrompti() external onlyAdmin() view returns(address){
        return prompti;
    }
    ...
    function balanceOf(address account) external onlyAdmin() view returns (uint256) {
        return balances[account];
    }


    function deposit(address _owner, uint _amount) external onlyAdmin() {
