...
contract BondingCalculator is IBondingCalculator {
    ...
    address public immutable AlphaOX;

    constructor( address _AlphaOX ) {
        require( _AlphaOX != address(0) );
        AlphaOX = _AlphaOX;
    }
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == AlphaOX ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( AlphaOX ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}