...
contract Staking is AccessControl, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        if (_poolId == 10) { 
            _amount == msg.value;
            //require(_amount > 0, "Staking: amount of ETH != _stake");
        } else {
            require(msg.value == 0, "Staking: you shouldn`t use ETH");
        }
    ...
    function setTokensForPool(uint8 _poolId, address _stake, address _reward) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "Staking: have no rights"
        );
        if(_stake != address(0)) pools[_poolId].stakingToken = _stake;
        if(_reward != address(0))pools[_poolId].rewardsToken = _reward;
    }
