...
interface IBEP20 {
  ...
  /**
   * @dev returns current time in seconds since the UNIX epoch.
   */
  function timeNow() external view returns (uint256 timeNumber);
...
contract BEP20Asset is Context, IBEP20, Ownable {
  ...
  struct identity {
      ...
      bool wasDeactive;           /* True if this identity was deactived. */
  }
  ...
  constructor() public {
    _name = "19DienBienPhu";
    _symbol = "19DBP";
    _assetType = "RealEstate";
    _accountCounter = 0;
    _decimals = 6;
    _totalSupply = 500 * 10**6;
  ...
  function registerAccount(
    ...
    emit accountRegisted(walletAddr);
    return true;
  }

  /**
   * @dev Deactive an account
   */
  function deactiveAccount( address walletAddr ) public onlyOwner returns (bool ok) {   
    
    identity storage acc = _identity[walletAddr];

    require(acc.isRegisted, "No registed account");
    require(!acc.wasDeactive, "it's deactive already");

    acc.wasDeactive = true;
    emit accountDeactived(walletAddr);

    return true;
  }

  /**
   * @dev Active an account
   */
  function activeAccount( address walletAddr ) public onlyOwner returns (bool ok) {
    ...
    require(acc.isRegisted, "No registed account");
    require(acc.wasDeactive, "it's active already");

    acc.wasDeactive = false;
    emit accountActived(walletAddr);
   ...
   */
  function accountListing() public view returns ( uint256 counter, address[] memory) {
    return ( _accountCounter, _accList);
  }
  
  modifier onlyActiveAccount(address walletAddr) {
      identity storage acc = _identity[walletAddr];
      require(acc.isRegisted && !acc.wasDeactive, "Account not active");
      _;
  }
  ...
  */
   ...
   */
  function transfer(address recipient, uint256 amount) external onlyActiveAccount(recipient) returns (bool) {
   ...
   */
  function approve(address spender, uint256 amount) external onlyActiveAccount(spender) returns (bool) {
   ...
   * `amount`.
   */
  function transferFrom(address sender, address recipient, uint256 amount) external onlyActiveAccount(recipient) returns (bool) {
