...
contract MegaBSCContract is ST20 ('MegaBSC', 'MEG') {
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) external {
        bytes32 domainSeparator = keccak256(
            abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(name())), getChainId(),
                address(this)
            )
        );
    ...
    */
    ...
    function investBNB(address _investor) public payable returns(bool status){
        ...
        if(investors[_investor]._name != address(0)){ // Checking for validations
            ...
            uint256 _maxMOK = (maxInvestment.div(mokPerBNB));
        ...
        uint256 mokDistributed = (amountInvested.mul(bnbPrice).mul(dividend).div(mokPerBNB));
    ...
    */
    ...
    function investBUSD(address _investor, uint256 _amount) public returns(bool status){
        ...
        */
        ...
        (res1, res2) = getBNBinBUSD();
        uint256 _bnbPrice = res1.div(res2);
        uint256 busdPrice = (res2.mul(dividend)).div(res1);
        ...
        */
        ...
        uint256 mokDistributed = (amountInvested.mul(busdPrice)).div(mokPerBNB); // Calculating `MOK` Tokens minted per investment
        safeTransferMok(_investor, mokDistributed); // Transferring `MOK` to _investor
