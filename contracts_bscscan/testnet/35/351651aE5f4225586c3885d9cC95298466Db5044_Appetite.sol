...
contract Appetite is Context, IERC20, Ownable {
    ...
    bool private _antiWhaleEnabled;
    ...
    function setRateIndexOne(uint8 index, uint32 value) external onlyOwner {
        ...
        if (index <= 1)
            require(
                value <= 100000,
                "Value for rate must be less than 100000 (100%)"
            );

        if (index == 0 || index == 1) {
            if (index == 0) {
                _setRateIndexOne(0, value);
                _setRateIndexOne(1, 100000 - value);
            } else {
                _setRateIndexOne(0, 100000 - value);
    ...
    /// Burn tokens from the initial support stream.
    function burnTokens(uint64 amount)
        external
        LockSwap
        onlyOwner
    {
     ...
     */
    function sellTokens(uint64 amount) external LockSwap onlyOwner {
    ...
    function useContractBNB(
        ...
        if (marketingRate > 0) {
            uint256 marketingBNB = (amount * (marketingRate * 1000)) / denominator;
        ...
        /// Gives available amount of tokens if we can otherwise no LP can happen (requires tokens to pair)
        if (
            liquidityRate > 0 &&
            ...
            _addLiquidity(
                _tokenBalance[address(this)] - _tokensForInitialSupport,
                (amount * (liquidityRate  * 1000)) / denominator
            );
        }
    }
     ...
     */
    function getRatesIndexOne() external view returns (uint32[7] memory) {
        bytes32[2] memory contractRates = _contractRates;
    ...
    function getAntiWhale() external view returns (bool) {
    ...
    // Conveniently display collected prize pot
    function getPrizePoolPot() external view returns (uint128) {
    ...
    function getReflectionAmountFromToken(uint128 amount)
        ...
        returns (uint128)
    {
        uint128 reflectedAmount = amount * getReflectionRate();
        require(
    ...
    function _transfer(
        ...
        if (!_lockSwap) {
            ...
            if (to == _pancakePair) {
                ...
                if (burnRate > 0) {
                    uint128 amountToBurn = (amount * burnRate) / 100000;

                    // Burn support tokens allocated to contract
                    if (
                        _tokensForInitialSupport >= amountToBurn &&
                        _tokenBalance[address(this)] >= amountToBurn
                    ) {
                        _burn(amountToBurn);
                        _tokensForInitialSupport -= uint64(amountToBurn);
                    }
                }
            }

            _handlePrizePool(from, prizePoolFee, amount);
        } // !_lockSwap

        bool antiWhaleCheck = _antiWhaleEnabled &&
            to != _pancakePair &&
            to != burnAddress &&
            to != address(this);
    ...
    function _finishTransfer(
        ...
        uint128 antiWhaleRate = _tokenSupply *
            uint32(bytes4(contractRatesOne << 160));
        ...
        if (!_isExcluded[to].fromReward) {
            ...
            if (antiWhaleCheck && antiWhaleRate > 0) {
                require(
                    _reflectionBalance[to] / getReflectionRate() <=
                        antiWhaleRate / 100000,
            ...
            if (antiWhaleCheck && antiWhaleRate > 0) {
                require(
                    _tokenBalance[to] <= antiWhaleRate / 100000,
        ...
        )
    {
        ...
        if (charityRate > 0) charityFee = (amount * charityRate) / denominator;
    ...
    function _getLiquidityFee(
        ...
        if (liquidityRate > 0) {
            ...
            liquidityFee =
                ((bnbToSplit * (liquidityRate * 100000)) / commonDenominator) /
                100000;
        }
    }
