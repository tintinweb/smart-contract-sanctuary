...
contract Prediction is IPrediction, Ownable, Pausable {
    ...
    function depositReward(uint256 _pid, uint256 amount) external poolExisted(_pid) onlyOwner whenNotPaused(_pid) {
        uint256 _epoch = currentEpoch[_pid];
        Specification storage specification = specifications[_pid][_epoch];
        specification.additionalReward += amount;
    ...
    function buyTicket(uint256 _pid, bool _label, address _referral) public poolExisted(_pid) whenNotPaused(_pid) returns(uint256 _price) {
        require(msg.sender != _referral || referral[msg.sender] != address(0x0), "PREDICTION: Yourself not allowed for referral");
        require(_bettable(_pid, currentEpoch[_pid]), "PREDICTION: Round not bettable");
        uint256 price = _buyTicket(_pid, _label, msg.sender, _referral);
        achievementService.updateHistory(msg.sender, _pid, currentEpoch[_pid], price, _label);
        return price;
    }
    ...
    function claimable(uint256 _pid, uint256 _epoch, uint256 _tid, address _user) public view returns (bool) {
        ...
        return 
            specification.oracleCalled && (
                ...
                (_tid == specification.lotteryWinningNumber)
            );
    }
    ...
    function claim(uint256 _pid, uint256 _epoch, address _to, uint256[] memory _tids) external poolExisted(_pid) {
        ...
        for (uint256 i = 0; i < _tids.length; i++) {
            ...
            require(!ticket.claimed, "PREDICTION: Rewards claimed");
            require(ticket.lotteryNumber >= 2 * pools[_pid].ticketNumberInit, "PREDICTION: Cannot claim initialized ticket");
            ...
            if (specifications[_pid][_epoch].oracleCalled == true) {
                ...
                if (_tid == specification.lotteryWinningNumber) {
     ...
     * @param _to wallet address of receiver
     */
    function claimReferralReward(address _to) external {
        ...
        IBEP20(perry).safeTransferFrom(owner(), _to, currentRewardAmount);
     ...
     */
    function getNextTicketPrice(address _buyer, uint256 _pid, bool _label) public view poolExisted(_pid) returns(uint256 price, uint256 fee, uint256 reduceRate) {
        (price, fee, ,reduceRate) = _nextTicket(_buyer, _pid, _label);
    }
    ...
    function _startRound(uint256 _pid, uint256 _epoch, uint256 _price) internal {
        ...
        for (uint256 i = 0; i < pool.ticketNumberInit; i++) {
            _initTicket(_pid, true);
            _initTicket(_pid, false);
        }
    }
    ...
    function _calculateRewards(uint256 _pid, uint256 _epoch) internal {
        ...
        if (lotteryReward > 0 && specification.lotteryWinningNumber > 0) {
            achievementService.increaseWinningLotteryCount(ticketOwner[specification.lotteryWinningNumber]);
        }
        emit RewardsCalculated(_pid, _epoch, rewardAmount, lotteryReward, specification.lotteryWinningNumber, result);
    }
     ...
     * @param _referral wallet address of referral
     */
    function _buyTicket(uint256 _pid, bool _label, address _buyer, address _referral) internal returns(uint256 _price) {
        uint256 _currentEpoch = currentEpoch[_pid];
        Specification storage specification = specifications[_pid][_currentEpoch];

        (uint256 price, uint256 fee, uint256 newPool, uint256 reduceRate) = _nextTicket(_buyer, _pid, _label);
        ...
        _calculateReferralReward(_buyer, _referral);

        tokenReward.safeTransferFrom(_buyer, address(this), price);
        perry.safeTransferFrom(_buyer, address(this), fee);
        
        _storgeTicket(_buyer, _pid, _currentEpoch, _label, price, fee);
        return (price);
    }

    function _nextTicket(address _buyer, uint256 _pid, bool _label) internal view returns(uint256 price, uint256 fee, uint256 newPool, uint256 reduceRate) {
        uint256 _currentEpoch = currentEpoch[_pid];
        Pool memory pool = pools[_pid];
        Specification memory specification = specifications[_pid][_currentEpoch];
        if (_label) {
            newPool = (specification.bluePool / specification.blueTicketNumber + improvementDelta) * specification.blueTicketNumber + pool.ticketPriceStart;
            price = newPool - specification.bluePool;
        } else {
            newPool = (specification.redPool/ specification.redTicketNumber + improvementDelta) * specification.redTicketNumber + pool.ticketPriceStart;
            price = newPool - specification.redPool;
        }
        fee = pool.ticketFeeStart * price / pool.ticketPriceStart;
        reduceRate = achievementService.userReduce(_buyer);
    }

    function _initTicket(uint256 _pid, bool _label) internal {
        uint256 _currentEpoch = currentEpoch[_pid];
        Pool memory pool = pools[_pid];
        Specification storage specification = specifications[_pid][_currentEpoch];
        address _buyer = owner();
        uint256 price = pool.ticketPriceStart;
        if (_label == true) {
            specification.bluePool = specification.bluePool + price;
            specification.blueTicketNumber++;
        } else {
            specification.redPool = specification.redPool + price;
            specification.redTicketNumber++;
        }
        specification.feePool += pool.ticketFeeStart;
        
        tokenReward.safeTransferFrom(_buyer, address(this), price);
        perry.safeTransferFrom(_buyer, address(this), pool.ticketFeeStart);
        
        _storgeTicket(_buyer, _pid, _currentEpoch, _label, price, pool.ticketFeeStart);
    }
    
    function _storgeTicket(address _buyer, uint256 _pid, uint256 _epoch, bool _label, uint256 _price, uint256 _fee) internal {
        Specification storage specification = specifications[_pid][_epoch];
        Ticket memory ticket = Ticket({
            label: _label,
            price: _price,
            fee: _fee,
            lotteryNumber: specification.blueTicketNumber + specification.redTicketNumber - 1,
            claimed: false
        });
        
        uint256 tid = tickets[_buyer][_pid][_epoch].length;
        tickets[_buyer][_pid][_epoch].push(ticket);
        ticketOwner[tid] = _buyer;
        if (tid == 99) {
            achievementService.setFirstBuy100(_buyer);
        }
        emit NewTicket(_buyer, _label, _pid, _epoch, tid, ticket.lotteryNumber, _price, _fee, block.timestamp);
    }

    function _calculateReferralReward(address _buyer, address _referral) internal {
        ...
        if (referral[_buyer] != address(0x0)) {
            referralReward[referral[_buyer]] = referralReward[referral[_buyer]] + referralRewardPerTicket;
        }
    }
    ...
    function _split(uint256 _randomNumber, uint256 _pid, uint256 _epoch) internal view returns(uint256) {
        ...
        // Calculation range
        uint256 range = specification.blueTicketNumber + specification.redTicketNumber - (2 * pool.ticketNumberInit);
