...
contract Barn is Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
    ...
    // todo 临时改回去
    uint256 public constant MINIMUM_TO_EXIT = 2 hours;
    // wolves take a 20% tax on all $WOOL claimed
    uint256 public constant WOOL_CLAIM_TAX_PERCENTAGE = 20;
    // sheep burn a 5% ftax on all $WOOL claimed
    uint256 public constant SHEEP_CLAIM_BURN_PERCENTAGE = 5;
   ...
   */
    function claimManyFromBarnAndPack(uint16[] calldata tokenIds, bool unstake) external whenNotPaused _updateEarnings nonReentrant {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            if (isSheep(tokenIds[i])) {
                 (uint256 owed1, uint256 burned1) = _claimSheepFromBarn(tokenIds[i], unstake);
                 owed += owed1;
                 burned += burned1;
            } else {
   ...
   // todo 添加了return
    function _claimSheepFromBarn(uint256 tokenId, bool unstake) internal returns (uint256 owed, uint256 burned) {
        ...
        if (totalWoolEarned < MAXIMUM_GLOBAL_WOOL) {
            ...
            // todo 换成小时
            owed = (block.timestamp - stake.value) * DAILY_WOOL_RATE / 1 hours;
            ...
            // todo 小时
            owed = (lastClaimTimestamp - stake.value) * DAILY_WOOL_RATE / 1 hours;
        }
        if (unstake) {
            if (random(tokenId) & 1 == 1) { // 50% chance of 90% $WOOL stolen, 8% burn, 2% mercy
                _payWolfTax(owed * 90 / 100);
                burned = owed * 8 / 100;
                owed = owed * 2 / 100;
   ...
   */
     ...
     */
     // todo days -> hours 换了间隔
    modifier _updateEarnings() {
        if (totalWoolEarned < MAXIMUM_GLOBAL_WOOL) {
            totalWoolEarned +=
            (block.timestamp - lastClaimTimestamp)
            * totalSheepStaked
            * DAILY_WOOL_RATE / 1 hours;
