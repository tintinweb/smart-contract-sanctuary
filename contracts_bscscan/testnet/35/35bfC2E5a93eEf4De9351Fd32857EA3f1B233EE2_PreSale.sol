...
//change this name
contract TestPresale is BEP20, BEP20Detailed {
  ...
  constructor () BEP20Detailed("TestPresale", "Test", 18) {
      _owner = msg.sender;
...
contract PreSale is Ownable{
  ...
  TestPresale public token;

    uint256 public totalAmountAvailable = 400000000000000 * 10 ** 18;
  ...
  uint public minBuy = 3 ether /100;
 ...
 // change type of last parameter to match the above token
  constructor(uint256 _rate, address payable _wallet, TestPresale _token) {
    require(_rate > 0);
    ...
    wallet = _wallet;
    token = _token;
    token._transfer(owner(), address(this), totalAmountAvailable);
  }

  fallback () external payable {
    buyTokens();
    buyForaFriend(msg.sender);
  }
  ...
  function reset(uint256 _rate, address payable _wallet) external onlyOwner{
    rate = _rate;
    wallet = _wallet;
   
  } 
  function restockSupply(uint256 _amount) external onlyOwner{
      totalAmountAvailable = totalAmountAvailable.add(_amount);
      token._transfer(owner(), address(this), _amount);
  }
  function buyTokens() public payable {
    require(msg.value >= minBuy, 'Amount too small' );
  ...
  function buyForaFriend(address _beneficiary) public payable {
    require(msg.value >= minBuy, 'Amount too small' );
    uint256 weiAmount = msg.value;
    _preValidatePurchase(_beneficiary, weiAmount);
  ...
  function _processPurchase(
    ...
    internal
  {
      uint256 maxtokenHold  = _getTokenAmount(3000000000000000000);
