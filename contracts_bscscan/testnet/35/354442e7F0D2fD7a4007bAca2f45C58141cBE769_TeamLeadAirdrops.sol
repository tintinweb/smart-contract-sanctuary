...
contract TeamLeadAirdrops  {
	...
	event BulkSend(uint256 totalUser, uint256 totalAmount);
	...
	function getChainID() public pure returns (uint256) {
		...
		function multiSend(address[] memory _receivers, uint256[] memory _amounts, uint256 _totalCost) public {
			...
			for (uint256 i = 0; i < length; ++i) {
				...
				totalAmount = totalAmount.add(toSend);
				PlatformTokenApp.transfer(receiver, toSend);
				emit AirdropSent(msg.sender, receiver, toSend);
			}
			
			emit BulkSend(receivers.length, totalAmount);
