...
contract FOXYDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("FOXY_Dividend_Tracker", "FOX_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100000000 * (10**18); //must hold 100000000000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "FOXY_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "FOXY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FCF contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FOXY_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "FOXY: Cannot update claimWait to same value");
...
contract FOXY is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    FOXYDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Foxy Gambler", "FOXY") {
        BNBRewardsFee = 6;
        liquidityFee = 4;
        extraFeeOnSell = 0;
        marketingFee = 4;
        marketingWallet = payable(0x45399C4341D6690202A9aAE104238aD9a2817634);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new FOXYDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FOXY: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FOXY: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FOXY: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FOXY: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 750000, "FOXY: gasForProcessing must be between 200,000 and 750,000");
        require(newValue != gasForProcessing, "FOXY: Cannot update gasForProcessing to same value");
