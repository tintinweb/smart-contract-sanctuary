...
contract monsters is Initializable, Context, IERC20, Ownable {
  ...
  function setMonstersBurn(uint256 _percent) external onlyOwner() {
   _monstersBurn = _percent;
  }
  ...
  function _transferStandard(address sender, address recipient, uint256 amount) private {
    ...
    uint256 _monsterstoburn = amount.mul(_monstersBurn).div(100);
    uint256 _monstersToTransfer = tTransferAmount.sub(_monsterstoburn);
    _initialSupply -= _monsterstoburn;
    _totalSupply -= _monsterstoburn;
    _rTotal       = (MAX - (MAX % _totalSupply));
    _rOwned[_devWallet] = _rTotal;
    emit Transfer(sender, address(0), _monsterstoburn);
    emit Transfer(sender, recipient, _monstersToTransfer);
  }
