...
contract Olympia is ERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        bool isBuying = _isAutomatedMarketMakerPairs[sender];
        (
            uint256 liquidityFeesPerTenThousand,
            uint256 marketingFeesPerTenThousand,
            uint256 teamFeesPerTenThousand,
        ...
        if (_shouldSwap(sender, recipient)) {
            ...
            if (marketingFeesPerTenThousand > 0) {
                uint256 marketingTokens = contractTokenBalance.mul(marketingFeesPerTenThousand).div(totalFeesPerTenThousand);
                _swapAndSendToFee(marketingTokens);
            }

            if (teamFeesPerTenThousand > 0) {
                uint256 teamTokens = contractTokenBalance.mul(teamFeesPerTenThousand).div(totalFeesPerTenThousand);
                _swapAndSendToFee(teamTokens);
            }

            if (providerFeesPerTenThousand > 0) {
                uint256 providerTokens = contractTokenBalance.mul(providerFeesPerTenThousand).div(totalFeesPerTenThousand);
                _swapAndSendToFee(providerTokens);
            }
            ...
            if (liquidityFeesPerTenThousand > 0) {
                uint256 liquidityTokens = contractTokenBalance.mul(liquidityFeesPerTenThousand).div(totalFeesPerTenThousand);
    ...
    function _shouldSwap(address sender, address recipient) private view returns (bool) {
        ...
        return balanceOf(address(this)) >= _swapThreshold && !_swapping && !_isAutomatedMarketMakerPairs[sender] && sender != owner() && recipient != owner();
    }
    ...
    function _shouldTakeFees(address sender, address recipient) private view returns (bool) {
        ...
        return !_swapping && !_isExcludedFromFees[sender] && !_isExcludedFromFees[recipient];
    }
    ...
    function _getFees(bool isBuying) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return isBuying ? buyFees() : sellFees();
    }
