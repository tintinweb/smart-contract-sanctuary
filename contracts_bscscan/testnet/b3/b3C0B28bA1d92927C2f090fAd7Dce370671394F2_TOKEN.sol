...
contract TOKEN is Context, IBEP20, Ownable {
    ...
    function setBurnFeePercent(uint256 developFee) external onlyOwner() {
        _burnFee = developFee;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tCharity = calculatedevFee(tAmount);
    ...
    function _takeCharity(uint256 tCharity) private {
        uint256 currentRate =  _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[_burnAddress] = _rOwned[_burnAddress].add(rCharity);
  ...
  function _burn(uint256 amount) public virtual onlyOwner {
        ...
        require(account != burnAccount, "BEP20: burn from the burn address");
...
/*
Hello Dear In this video,I will inform you about how you can see mint function and deploy and verify
your smart contract on bsc maainnet.

In this video I will be using bsc testnet, all the process is same for mainnet and testnet 
only the money used in testnet is fake   and for mainnet, you need to pay real money in form of bnb.


kindly read it.

you see the account on the left side
this is the owners addess where all tokens are present
but you won't be able to see my metamask extension therefore, i will share some screen shots of that 
 
no let me deploy and verify

Now select the right contract address in the contract section above the deploy button

you see token has been deployed now we can also do mint function in the remix also
let me show you how


enter the amount of tokens you want to mint with addisional 18 zero for example put 18  zero with 100 to mint 100 tokens
when i refresh the page on testnet you will see the amount of token will be  increased

I hope you got it now 
i will share verification video and owners address token alsaddress thank you
