...
contract baseBonus {      
    ...
    function _userRegister(address _userAddress,address _oneAddress) internal {
        if(!isUserExists(_userAddress)) {
            users[_userAddress] = User({oneAddress:_oneAddress,isUsed:1,mp:0,receiveTime:0,waitMp:0,commissions:0});
        }           
...
contract wolBonus is baseBonus{    
    ...
    // --------------------
    constructor(WERC20 _hostAddress,address _hostUserAddress,address _inputAddress,WERC20 _payContractAddress) {
        ...
        updateHostAddress(_hostAddress,_hostUserAddress,_inputAddress,_payContractAddress);
    }       
    //修改主币合约地址 和入金账号
    function updateHostAddress(WERC20 _hostAddress,address _hostUserAddress,address _inputAddress,WERC20 _payContractAddress) checkOwner public {
    ...
    // 更改产品分红及算力释放时间
    function changeProductMpSendDay(uint256 _number) checkOwner public {
        productMpSendDay = _number;
    } 
    // 更改加权分红池分销比例
    ...
    function buyProduct(uint productId,uint number) checkRegister public payable{
        ...
        uint256 price = products[_index].price.mul(number);
        require(price > 0);
        _safeTransfer(payContractAddress,msg.sender,inputAddress,price.mul(10**8));
    ...
    function userWithdraw() public {
        require(users[msg.sender].commissions > 0);
        emit WithdrawLog(msg.sender,users[msg.sender].commissions,block.timestamp);   
