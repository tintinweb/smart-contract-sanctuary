...
abstract contract FrozenableToken is Ownable
{
    ...
    function freezeAccount(address to, bool freeze) public {
        ...
        require(to != address(0), "not to self");
...
contract hiDollar is ERC20, ERC20Burnable, Pausable, FrozenableToken {
    ...
    function setFeeCollector(address newCollector) public onlyOwner {
        require(newCollector != address(0), "hiDollar: new collector is the zero address");
        require(newCollector != feeCollector, "hiDollar: new collector is same as the old one");
    ...
    function setUserFeeRatio(address user, uint256 ratio, bool enabled) public onlyOwner {
        require(user != address(0), "hiDollar: user can not be zero address");
    ...
    function setHolder(address newHolder) public onlyOwner {
        require(newHolder != address(0), "hiDollar: new holder is the zero address");
        require(newHolder != holder, "hiDollar: new holder is same as the old one");
        holder = newHolder;
    }
    ...
    function _transfer(
        ...
        if (fee > 0) {
            require(balanceOf(sender) >= amount + fees, "hiDollar: transfer amount and friction fee exceed balance");
        }
    ...
    function collectFee(uint256 amount) public returns (bool) {
        require(msg.sender == feeCollector, "hiDollar: only fee collector can collect fee");
        require(amount <= fees, "hiDollar: the amount exceeds the collectable fee");
        fees -= amount;
    ...
    */
    function proposeMint(uint256 amount) public returns(bool) {
        ...
        //mint once for a propoer at a time otherwise would be overwritten
