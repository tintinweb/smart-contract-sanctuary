...
contract DeployTesting is Context, IERC20, Ownable {
    ...
    address private _devWalletAddress = 0x4f6C16Dedd6219c0C57C11040e4F8a50BA6Dd477;
    ...
    string private _name = "DeployTesting";
    string private _symbol = "DTST";
    ...
    uint256 public maxWallet = 30000000000000  * 10**18;
    ...
    function setdevWalletAddress(address newAdd) external onlyOwner() {
        _devWalletAddress = newAdd;
    }

    function setdevWalletAddress(uint256 newVal) external onlyOwner() {
        numTokensSellToAddToLiquidity = newVal;
    }
   
    function setMaxWallet(uint256 newVal) external onlyOwner() {
        maxWallet = newVal;
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && !isExcludedFromFee(from) && !isExcludedFromFee(to)){ 
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            require(balanceOf(msg.sender) <= maxWallet, "Wallet exceeds Max Wallet limit.");
        }
