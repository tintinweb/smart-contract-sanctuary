...
contract Loot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint16 public referralShare = 2000; // as 20.00%
    IReferral public referralContract;

    event PriceUpdated(uint256 newPrice);
    event MaxSupplyUpdated(uint256 newMaxSupply);
    event ReferralContractUpdated(address newReferralContract);
    ...
    string[] private chestArmor = [
        ...
        "Redpilled Husk",
        "Based Armor",
        "Cringe Leather Armor",
        "Hard Leather Armor",
        "Leather Armor",
        "Blockchain Chestplate",
        "Based Chestplate",
        "Cool Mail",
        "(Block)Chain Mail",
        "JUST Mail"
    ...
    function mint(address referrer) external payable nonReentrant {
        ...
        require(
            maxSupply >= (this.totalSupply() + count),
            "Over purchasing, will sell out"
        );
        ...
        if (
            ...
            referralContract.recordReferral(to, referrer);
        }
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            // URI generated on call at `tokenUri`

            payReferral(to, msg.value); // pay to's referrer
        }
    }
    ...
    function setReferralContract(address newReferralContract) public onlyOwner {
        referralContract = IReferral(newReferralContract);
        emit ReferralContractUpdated(newReferralContract);
    }

    function setReferralShare(uint16 newReferralShare) public onlyOwner {
        referralShare = newReferralShare;
        emit ReferralShareUpdated(newReferralShare);
    }

    function payReferral(address referee, uint256 amount) internal {
        if (address(referralContract) != address(0) && referralShare > 0) {
            address referrer = referralContract.getReferrer(referee);
            uint256 shareAmount = (amount * referralShare) / 1e4; // 1e2 for percent and 1e2 for 2 decimals
            ...
            if (referrer != address(0) && shareAmount > 0) {
                ...
                emit ReferralSharePaid(referee, referrer, shareAmount);
            }
        }
    }
...
// File @openzeppelin/contracts/token/ERC20/[email protected] solidity 0.8.7;
