...
contract AdvensturesSlot is Context, PartnerOwnable  { 
    ...
    event wins_out(address sender, WINS wins);
    ...
    function get_fee_amount(uint256 _domainId, uint256 _tokenId) external view returns(uint256 _fee_amount){ 
        address _address = tokensInfo[_tokenId].tokens_address;
        _fee_amount = domainInfo[_domainId].domain_fee_amount[_address];
    } 
    
    function get_fee_amount(uint256 _domainId, address _tokenAddress) external view returns(uint256 _fee_amount){ 
    ...
    function bet(uint[] calldata bets,uint domainId, uint tokensId) external  
        ...
        if( tokensInfo[tokensId].tokens_cake_balance + tot_bet < wins.wins_result + fee2 ) { 
            tokensInfo[tokensId].tokens_cake_balance = 0;
        } else { 
            tokensInfo[tokensId].tokens_cake_balance += tot_bet - fee2 - wins.wins_result;
        }
