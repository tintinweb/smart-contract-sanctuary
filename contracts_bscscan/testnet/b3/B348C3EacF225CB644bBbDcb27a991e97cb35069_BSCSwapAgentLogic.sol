...
// -----------------------------------------------------------------------------------------------------
// BSCSwapAgentLogic Contract
//
//
// This contract is modified from the BSCSwapAgentImpl contract in Open Source Ethereum and Binance 
// Smart Chain Bi-directional bridge Solution by BINANCE CHAIN
// (https://github.com/binance-chain/eth-bsc-swap-contracts/blob/master/contracts/BSCSwapAgentImpl.sol)
//                        
// July 15 2021                                  
// -----------------------------------------------------------------------------------------------------

contract  BSCSwapAgentLogic is OwnableUpgradeable {
    ...
    function __BSCSwapAgentLogic_init(address bep20Impl, uint256 fee, address bep20ProxyAdminAddr) public initializer {
    ...
    event SwapPairCreated(bytes32 indexed ethRegisterTxHash, address indexed bep20Addr, address indexed erc20Addr, string symbol, string name);
    ...
    event BEP20ImplementationUpdated(address indexed _oldBep20Impl, address indexed _newBep20Impl, uint256 _updatedAt);
    event BEP20ProxyAdminUpdated(address indexed _oldBep20ProxyAdmin, address indexed _newBep20ProxyAdmin, uint256 _updatedAt);
    ...
    function swapBSC2ETH(address bep20Addr, uint256 amount) payable external notContract returns (bool) {
        ...
        IERC20Upgradeable(bep20Addr).safeTransferFrom(msg.sender, address(this), amount);
     ...
     * @dev createSwapPair
     */
    function createSwapPair(bytes32 ethTxHash, address erc20Addr, string calldata name, string calldata symbol) onlyOwner external returns (address) {
        ...
        token.__BEP20HCurrencyImplementation_init(name, symbol);

        swapMappingETH2BSC[erc20Addr] = address(token);
        swapMappingBSC2ETH[address(token)] = erc20Addr;

        emit SwapPairCreated(ethTxHash, address(token), erc20Addr, symbol, name);
    ...
    /**
     * @dev Upgrade to new BEP20Implementation
     */
    function updateBEP20Implementation(address _newBEP20Impl) onlyOwner external {
        emit BEP20ImplementationUpdated(bep20Implementation, _newBEP20Impl, block.timestamp);
        bep20Implementation = _newBEP20Impl;
    }

    /**
     * @dev Upgrade to new BEP20ProxyAdmin
     */
    function updateBEP20ProxyAdmin(address _newBEP20ProxyAdmin) onlyOwner external {
        emit BEP20ProxyAdminUpdated(bep20ProxyAdmin, _newBEP20ProxyAdmin, block.timestamp);
        bep20ProxyAdmin = _newBEP20ProxyAdmin;
    }
...
interface IProxyInitialize {
    function __BEP20HCurrencyImplementation_init(string calldata name, string calldata symbol) external;
}
