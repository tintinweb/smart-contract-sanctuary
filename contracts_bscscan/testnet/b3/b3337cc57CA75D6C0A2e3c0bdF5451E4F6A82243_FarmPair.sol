...
contract FarmPair is IFarmPair, FarmHashrate, Initializable, Ownable, Recoverable {
    ...
    // 用户所有币对算力
    mapping(address => uint256) public pairHashrateMap;

    // 用户单个币对算力, useraddr -> token0 -> token1 -> hashrate
    mapping(address => mapping(address => mapping(address => uint256))) public singlePairHashrateMap;
    ...
    // 返佣数量, 解押些会扣除, useraddr -> genUser.addr -> hashratecommission
    ...
    function stakePair(address token0, uint256 amount0, address token1, uint256 amount1, uint8 scaleType) public override needInit {
        ...
        // 累计总投资
        farmUser.incrementInvest(_msgSender(), hashrate);

        // 增加用户币对算力，用户币对总算力, 全网币对总算力
        singlePairHashrateMap[userAddr][token0][token1] = SafeMath.add(singlePairHashrateMap[userAddr][token0][token1], hashrate);
        pairHashrateMap[userAddr] = SafeMath.add(pairHashrateMap[userAddr], hashrate);       
        totalHashrate = SafeMath.add(totalHashrate, hashrate);

        // 算力返佣
        if (hc.rebate == 1) {
            Model.User memory genUser = farmUser.getInviterUser(_msgSender());
            for (uint8 gen = 1; gen <= appData.getMaxGen(); gen++) {
                ...
                uint256 hashrateCommission = SafeMath.div(SafeMath.mul(hashrate, commissionRate), 1000);  
                pairHashrateMap[genUser.addr] = SafeMath.add(pairHashrateMap[genUser.addr], hashrateCommission);
                // 记录用户算力
                recordUserHashrate(Model.CATEGORY_PAIR, genUser.addr, pairHashrateMap[genUser.addr]);

                totalHashrate = SafeMath.add(totalHashrate, hashrateCommission);
                commissionMap[_msgSender()][genUser.addr] = SafeMath.add(commissionMap[_msgSender()][genUser.addr], hashrateCommission);
                genUser = farmUser.getInviterUser(genUser.inviterAddr);
            }
        }

        // 记录用户及全网算力
        recordHashrate(Model.CATEGORY_PAIR, _msgSender(), pairHashrateMap[_msgSender()], totalHashrate);
    ...
    function unstakePair(address token0, address token1) public override needInit {
        ...
        // 减少币对池算力
        uint256 hashrate = singlePairHashrateMap[userAddr][token0][token1];
        singlePairHashrateMap[userAddr][token0][token1] = 0;
        pairHashrateMap[userAddr] = SafeMath.sub(pairHashrateMap[userAddr], hashrate);
        ...
        for (uint8 gen = 1; gen <= appData.getMaxGen(); gen++) {
            ...
            uint256 hashrateCommission = commissionMap[_msgSender()][genUser.addr];
            if (hashrateCommission == 0) {
                continue;
            }

            singlePairHashrateMap[genUser.addr][token0][token1] = SafeMath.sub(singlePairHashrateMap[genUser.addr][token0][token1], hashrateCommission);
            pairHashrateMap[genUser.addr] = SafeMath.sub(pairHashrateMap[userAddr], hashrateCommission);
            // 记录用户算力
            recordUserHashrate(Model.CATEGORY_PAIR, genUser.addr, pairHashrateMap[genUser.addr]);
    ...
    function getUserStakeAmount(address userAddr, address token0, address token1) public view returns(uint256, uint256) {
        uint256[2] storage amounts = pairPoolMap[token0][token1][userAddr];
...
interface IFarmUser is IFarmHashrate {
    ...
    function upgradeLevel(uint8 levelNo, address payToken) external;
    ...
    function incrementInvest(address userAddr, uint256 usdtAmount) external;
    function incrementYield(address userAddr, address rewardToken, uint256 rewardTokenAmount) external;
