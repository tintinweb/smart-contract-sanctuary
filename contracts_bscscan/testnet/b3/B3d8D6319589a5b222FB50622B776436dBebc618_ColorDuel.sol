...
contract ColorDuel is IERC20, Auth {
    ...
    InitialDistribution internal _intitialDistribution =
        InitialDistribution(
            ...
            uint256(45).mul(_totalSupply).div(1e2), // Public Presale -- 45% total supply
            uint256(31).mul(_totalSupply).div(1e2)  // Public Listing -- 31% total supply
        );
...
contract DividendDistributor is IDividendDistributor {
    ...
    function deposit() external payable override onlyToken {
        ...
        uint256 amount = RIVAL.balanceOf(address(this)).sub(balanceBefore);

        totalDividendsRival = totalDividendsRival.add(amount);
        dividendsPerShareRival = dividendsPerShareRival.add(dividendsPerShareAccuracyFactor.mul(amount).div(totalShares));

        totalDividendsBNB = totalDividendsBNB.add(msg.value.sub(rivalFee));
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amountRival > 0){
            ...
            (bool success, bytes memory data) = address(RIVAL).call{gas: 300000}(abi.encodeWithSelector(0xa9059cbb, shareholder, amountRival));
    ...
    function getUnpaidEarnings(address shareholder, uint256 dividendsPerShare, uint256 shareholderTotalExcluded) public view returns (uint256) {
    ...
    function setRival(address _rival) external onlyToken {
        RIVAL = IERC20(_rival);
    }
