...
contract PrivatePresale {
    ...
    // @dev privatePrice in cents (20 = $0.2)
    uint256 private privatePrice = 20;
    // @dev total amount of deposit token collected
    uint256 public totalCollected;
    // @dev used to calculate amount of tokens received
    uint256 public previousCollected = 0;
    // @dev max deposit from one wallet
    uint256 public walletLimit = 4*10**(18);
    // @dev softCap is not used in this sc
    uint256 public softCap = 5*10**(18);
    // @dev hardCap is not used
    uint256 public hardCap;
    // @dev amount of transactions
    ...
    // @dev % of tokens to collect after TGE (15% = 15000)
    uint256 private tgeToCollect = 15000;
    // @dev % of tokens to collect after 1st collect (10.625% = 10625)
    uint256 private normalCollect = 10625;

    // @dev when tgeHappened sets to true, block.timestamp is stored
    uint256 private tgeTime;
    // @dev Collect1 -> Collect8 – 2 weeks after each other (10.625% collect)
    ...
    function devCheck() public onlyOwner {
        uint256 txCollected = 1*10**18;
        txCount ++;
    ...
    // @dev Returns the amount of tokens mistakingly sent to this address (not through function privateRound())
    ...
    // @dev Returns presale token address
    function returnToken() public view virtual returns (IERC20) {
        return _returnToken;
    }

    // @dev returns collect times (9 values returnes)
    ...
    // @dev used to claim refund by wallet if softCap was not met
    // @notice Requires refundProcess to be set to true
    function claimRefund() public {
        require(refundProcess == true, "RefundError: You're not able to claim refund");
    ...
    // @dev Main function to send deposit token to
    function privateRound() public payable {
        require(privateOpen,"PrivateRound: Private Round is Closed");

        // @notice calculates the amount of deposit token received
    ...
    // @dev Changes Private Round status (true = open, false = closed)
    ...
    // @dev Closes presale. Transfers deposit token to owner, closes private round
    // @notice To initiate claim, setTGEHappened() must be called
    ...
    // @dev Sets tgeHappened to true and claimTokens() could be used
    ...
    // @dev Returns amount left to collect by the wallet
    function amountToCollect(address _address) public view returns (uint256) {
        return inPresale[_address].newAmount;
    }

    //@dev Returns initial deposit amount in deposit tokens
    ...
    // @dev Transfers return token to the owner
    // @notice could be used after all claims have happened
    // @notice do not forget about decimals
    ...
    function claimTokens() public {
        ...
        if (inPresale[msg.sender].numOfCollectTX == 0) {
            ...
            // @dev checks if time for the next collected is >= block.timestamp
    ...
    // @dev Reverts if funds are sent directly to the smart contract
