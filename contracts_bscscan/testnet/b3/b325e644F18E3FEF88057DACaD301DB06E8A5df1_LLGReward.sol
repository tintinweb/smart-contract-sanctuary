...
contract LLGReward is Ownable {
    IERC20 public llgContract;
    address private LLGContractAddr;
    ...
    constructor(address _llg) {
        llgContract = IERC20(_llg);
        LLGContractAddr = _llg;
    ...
    function depositForRoom(uint256 _roomId, uint256 _funds) external returns (bool) {
        // if(llgContract.approve(LLGContractAddr, _funds)) {
            // llgContract.transferFrom(_playerAddr, devWallet, _funds * 10**9);
            llgContract.transfer(devWallet, _funds * 10**9);
