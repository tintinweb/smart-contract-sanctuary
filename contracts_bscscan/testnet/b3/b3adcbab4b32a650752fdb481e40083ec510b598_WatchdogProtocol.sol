...
contract WatchdogProtocol is IBEP20, Auth {
    ...
    function swapBack() internal swapping {
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            // console.log("swapBack(): amountBNB =", amountBNB);

            uint256 totalBNBFee = totalBuyFee.sub(dynamicLiquidityFee.div(2));
            // console.log("swapBack(): totalBNBFee =", totalBNBFee);

            uint256 amountBNBLiquidity = amountBNB.mul(dynamicLiquidityFee).div(totalBNBFee).div(2);
            // console.log("swapBack(): amountBNBLiquidity =", amountBNBLiquidity);
            uint256 amountBNBBuyback = amountBNB.mul(buybackFee).div(totalBNBFee);
            // console.log("swapBack(): amountBNBBuyback =", amountBNBBuyback);
            uint256 amountBNBToBUSD = amountBNB.sub(amountBNBLiquidity).sub(amountBNBBuyback);
            // console.log("swapBack(): amountBNBToBUSD =", amountBNBToBUSD);

            uint busdDiff = swapToBusd(amountBNBToBUSD);
            // console.log("swapBack(): busdDiff =", busdDiff);

            uint totalFeeBUSD = utilityFee.add(reflectionFee);
            // console.log("swapBack(): totalFeeBUSD =", totalFeeBUSD);
            uint busdToDistributor = busdDiff.mul(reflectionFee).div(totalFeeBUSD);
            // console.log("swapBack(): busdToDistributor =", busdToDistributor);
            ...
            if(amountToLiquify > 0){
                // console.log("swapBack(): about to liquify. amountToLiquify =", amountToLiquify);
                try router.addLiquidityETH{ value: amountBNBLiquidity }(
                    ...
                    // console.log("swapBack(): liquified. amountBNBLiquidity =", amountBNBLiquidity);
                } catch {
                    emit AutoLiquify(0, 0);
                    // console.log("swapBack(): failed to liquify");
                }
            }
    ...
    function swapToBusd(uint amount) internal returns (uint resultingAmt) {
        ...
        // console.log("swapToBusd(): busdBefore =", busdBefore);
    ...
    function pushFees() public {
        ...
        // console.log("pushFees(): toBeDistributed =", toBeDistributed);
        ...
        for (uint256 i = 0; i < feeReceivers.length; i++) {
            ...
            try IBEP20(BUSD).transfer(feeReceivers[i].adr, amt) {
                ...
                // console.log("pushFees(): pushed amt =", amt);
            } catch {
                // console.log("pushFees(): push failed with amt =", amt);
            }
        }
        // console.log("pushFees(): pushed fees.");
    }
