...
contract BuyNFT is Ownable {
    ...
    uint256 public immutable priceLast;
    uint256 public purchaseLimit = 5000;
    uint256 public delayTime = 3*24*60*60;
    //uint256 public constant deadline = 1638316800; //latest start time is 2021-12-01 00:00:00 GMT
    uint256 public deadline = 1638316800; //latest start time is 2021-12-01 00:00:00 GMT

    event  BuyOneNFT(address indexed user, uint256 tokenID);

    constructor(IERC721 _NFTContract, IERC20 _tokenContract, uint256 _priceNFT, uint256 _priceLast) {
        NFTContract = _NFTContract;
        tokenContract = _tokenContract;
        priceNFT = _priceNFT;
        priceLast = _priceLast;
        receiver = msg.sender;
    }
    ...
    function buyOneNFT() public payable {
        ...
        if (block.timestamp < deadline) {
            ...
            require(msg.value == priceLast, "Transfer asset's value error.");
        }
        buyCount++;
    ...
    function setBuyCount(uint256 _buyCount) public onlyOwner() {
        buyCount = _buyCount;
    }
