...
contract SpaceHunterNFT is ERC721Pausable, AccessControl, Ownable {
    ...
    mapping(uint256 => uint8) public rateRewards;
    event SetRateReward(uint256 indexed tokenId, uint8 rateReward);
    ...
    function getRateReward(uint256 _tokenId) public view returns(uint8) {
        return rateRewards[_tokenId];
    }

    function setRateReward(uint256 _tokenId, uint8 _rateReward) public onlyOwner {
        rateRewards[_tokenId] = _rateReward;
        emit SetRateReward(_tokenId, _rateReward);
    }
