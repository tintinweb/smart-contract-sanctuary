...
interface IERC721 is IERC165 {
    ...
    function safeMint(address to) external view returns (uint256 balance);

    function safeMint(address to,bytes memory _data) external view returns (uint256 balance); 
...
contract Vehicle is Ownable {
    ...
    function setERC721Token(IERC721 _token) public onlyOwner {
        ERC721Token = _token;
    }
    ...
    function addVehicle(uint8 id,
        ...
        if(Detail.index==0){
            vehicleList.push(VehicleConfig(id, grade,name,value,url,currency,vehicleList.length,status));
            VehicleDetail[id]=VehicleConfig(id,grade,name,value,url,currency,vehicleList.length,status);
        }else{
            vehicleList[Detail.index.sub(1)]=VehicleConfig(id, grade,name,value,url,currency,Detail.index,status);
            VehicleDetail[id]=VehicleConfig(id,grade,name,value,url,currency,Detail.index,status);
        }
    }
    ...
    function buyVehicle(uint8 id,uint8 num) public {
        ...
        currency.safeTransferFrom(msg.sender, address(account), amount);
        for (uint256 i = 1; i <= num; i++) {
           createOrder(msg.sender, id,detail.grade, price,detail.name, currency);
        } 
    }


    function createOrder(address sender, uint8 id,uint8 grade,uint256 price,string memory name,IERC20 currency) private {
        uint256 tokenId= ERC721Token.safeMint(sender);
        myOrders[sender].push(OrderInfo(tokenId, id,grade,name));
        TokenToIndex[tokenId] = uint64(myOrders[sender].length - 1);
        emit BuyVehicle(msg.sender,id,grade,price,name,currency);
    }
