...
contract Staking is Ownable, ReentrancyGuard {
    ...
    function harvestHalf() external nonReentrant {
        require(UserInfos[msg.sender].isStaked, "HarvestHalf:: No Stake Found");
        require(
            !UserInfos[msg.sender].isHalfClaimed,
            "HarvestHalf:: Already Harvested Half"
        );
    ...
    function _harvestRewardsForHalf(uint256 tokenToReward) internal {
        ...
        UserInfos[msg.sender].currentLockedTokens.sub(
            _tokenToReward.mul(apy.div(100)).mul(_daysStaked).div(noOfDaysYear)
        );
        // Updated Unlocked tokens
        UserInfos[msg.sender].currentUnlockedTokens.add(
            _tokenToReward.mul(apy.div(100)).mul(_daysStaked).div(noOfDaysYear)
        );
    ...
    function harvestDaily() external nonReentrant {
        require(
            UserInfos[msg.sender].isStaked,
            "HarvestDaily:: No Stake Found"
        );
        require(
            !UserInfos[msg.sender].isFullClaimed,
            "HarvestDaily:: Fully Harvested "
        );
        require(
            UserInfos[msg.sender].isHalfClaimed,
            "HarvestDaily:: Harvest Half First"
        );
        require(
            UserInfos[msg.sender].noOfWithdrawls <= 99,
            "HarvestDaily:: Invalid"
        );
    ...
    function _harvestDaily() internal {
        ...
        UserInfos[msg.sender].currentLockedTokens = UserInfos[msg.sender]
            .currentLockedTokens
            .sub(
                _tokenToReward.mul(apy.div(100)).mul(_daysStaked).div(
                    noOfDaysYear
                )
            );
        ...
        emit DailyStakeHarvest(
            msg.sender,
            block.timestamp,
            _tokenToReward.mul(1).div(100)
        );
        emit DailyRewardHarvest(
            msg.sender,
            block.timestamp,
            _tokenToReward.mul(apy.div(100)).mul(_daysStaked).div(noOfDaysYear)
        );
