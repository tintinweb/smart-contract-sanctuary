...
contract TestToken is IERC20, IERC20Metadata, Ownable {
    ...
    mapping(address => bool) public _isServiceWallet;

    mapping(address => bool) public _isExcluded;

    mapping(address => uint256[]) public _dailySales;
    ...
    uint256[4] public _saleFees = [10,30,30,30];

    uint256 public _userFee = 99; // fee for user-to-user transfer
    ...
    function _transfer(
        ...
        if (isUser(sender) && isUser(recipient)) {
            ...
            if (_dailySales[sender].length == 4) {
                require(_dailySales[sender][0] < block.timestamp-3600, "Only 4 sells per day allowed");
                delete _dailySales[sender];
                _dailySales[sender][0] = block.timestamp;
            } else if (_dailySales[sender].length > 0) {
                require(_dailySales[sender][_dailySales[sender].length-1] < block.timestamp-60, "1 hour delay between sales required");
