...
contract PazDividendTracker is DividendPayingToken, Ownable {
    ...
    event DividendReinvested(address indexed account, uint256 busdAmount, address rewardTokenAddress);
...
//        if(rewardToken == address(0) || rewardToken == BUSD){
//            return router;
//        }
//        address rewardTokenRouter = tokenRouter[rewardToken];
//        if(rewardTokenRouter == address(0)){
//            return router;
//        }
...
//    }
    ...
    function getRouter(address token) internal view returns (ISwapRouter _router){
        address routerAddress = tokenRouter[token];
        return routerAddress == address(0) || routerAddress == address(router) ? router : ISwapRouter(routerAddress);
    }
    ...
    function swapBusdForTokens(address user, address token, uint256 amount) internal returns (uint256) {
        ...
        try _router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount, 0, path, address(user), block.timestamp){
            userSwappedBusd[user]+=amount;
            swapSuccess = true;
            emit DividendReinvested(user, amount, token);
