...
contract EVERTIKI is ERC20, Ownable {
    ...
    uint256 public fixedSaleStartTime = 1624842900;
    uint256 public tradingEnabledTime = 1624910400;
    ...
    event FixedSaleStartTimeUpdated(uint256 indexed newValue, uint256 indexed oldValue);
    event TradingEnabledTimeUpdated(uint256 indexed newValue, uint256 indexed oldValue);
    ...
    constructor() public ERC20("EverTiki-V2", "EVERTIKI-V2") {
    ...
    function updateFixedSaleStartTime(uint256 newValue) public onlyOwner {
        require(newValue != fixedSaleStartTime, "Token: Cannot update FixedSaleStartTime to same value");
        emit FixedSaleStartTimeUpdated(newValue, fixedSaleStartTime);
        fixedSaleStartTime = newValue;
    }
    function updateTradingEnabledTime(uint256 newValue) public onlyOwner {
        require(newValue != tradingEnabledTime, "Token: Cannot update TradingEnabledTime to same value");
        emit TradingEnabledTimeUpdated(newValue, tradingEnabledTime);
        tradingEnabledTime = newValue;
    }
    ...
    function _transfer(
        ...
        if (isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTime, "EverTiki: fixed-sale has not started yet");
            bool openToEveryone = block.timestamp.sub(fixedSaleStartTime) >= fixedSaleEarlyParticipantDuration ||
        ...
        if (takeFee) {
            ...
            //         // amount = amount.sub(buyBackAmount); // @bug will further deduct the transfer amount by the buyback amount
            //     }
            // }
        }
...
contract TokenDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("EverTiki_Dividend_Tracker-V2", "EverTiki_Dividend_Tracker-V2") {
        claimWait = 3600;
