...
// OriRewardContract with Governance.
contract OriRewardContract is BEP20('OriRewardContract', 'ORI') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
...
contract MasterChef is Ownable {
    ...
    // The ori TOKEN!
    OrichainsToken public ori;
    // The Ori Reward TOKEN!
    OriRewardContract public oriRewardContract;
    // Dev address.
    ...
    constructor(
        OrichainsToken _ori,
        OriRewardContract _oriRewardContract,
        address _devaddr,
    ...
    function updatePool(uint256 _pid) public {
        ...
        ori.mint(address(oriRewardContract), oriReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if(_pid == 4 || _pid == 5 || _pid == 8 || _pid == 9) {
    ...
    function enterStaking(uint256 _amount) public {
        ...
        oriRewardContract.mint(msg.sender, _amount);
    ...
    function leaveStaking(uint256 _amount) public {
        ...
        oriRewardContract.burn(msg.sender, _amount);
    ...
    function safeoriTransfer(address _to, uint256 _amount) internal {
        oriRewardContract.safeoriTransfer(_to, _amount);
    }
