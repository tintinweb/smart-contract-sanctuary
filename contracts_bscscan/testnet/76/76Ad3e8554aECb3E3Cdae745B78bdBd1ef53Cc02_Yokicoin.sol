...
// This function calls lp pair and checks for the current price of YOKI token in BUSD
    ...
    function busdPricePerYOKI(uint256 tokenAmount) internal view returns(uint256){
        ...
        if(token0 == address(this)){
            sellPrice = IPancakeRouter(routerAddress).getAmountIn(tokenAmount, reserve1, reserve0);
            return sellPrice;
        }
        sellPrice = IPancakeRouter(routerAddress).getAmountIn(tokenAmount, reserve0, reserve1);
        return sellPrice;
    }
