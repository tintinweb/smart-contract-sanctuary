...
contract VIGOURBNB {
    ...
    uint256[7] public defaultPackages = [0.05 ether, 0.1 ether,0.2 ether,0.4 ether ,0.6 ether,1 ether,4 ether];
  ...
  function _refPayout(address _addr, uint256 _amount) internal {
    ...
    function invest(address referrer) public payable {
		require(msg.value >= INVEST_MIN_AMOUNT,'Min invesment 0.05 BNB');
  ...
  function UserInfo(address _user) public view returns(uint256 uplineincome,uint256 downlineincome,uint256 availableForWithdraw){
      uplineincome = GetUplineIncomeByUserId(_user);
      downlineincome = GetDownlineIncomeByUserId(_user);
      availableForWithdraw = uplineincome.sub(users[_user].singleUplineBonusTaken) + downlineincome.sub(users[_user].singleDownlineBonusTaken)  + users[_user].referrerBonus;
  }
  ...
  function getEligibleWithdrawal(address _user) public view returns(uint8 reivest, uint8 withdrwal){
      ...
      uint256 TotalDeposit = users[_user].amount;
      if(users[_user].refs[0] >=4 && (TotalDeposit >=0.05 ether && TotalDeposit < 4 ether)){
          reivest = 50;
          withdrwal = 50;
  ...
  function _safeTransfer(address payable _to, uint _amount) internal returns (uint256 amount) {
   ...
   function referral_stage(address _user)external view returns(uint[6] memory _noOfUser, uint256[6] memory _investment, uint256[6] memory _bonus){
    ...
    function GetSingleLegFamily(address _user) public view returns(address[] memory _address,uint256[] memory invesment,uint256[] memory income,uint256[] memory level){
      ...
      for(uint i=uplineCount-1;i>=0;i--){
        if(upline == address(0))
            break;
        else{
