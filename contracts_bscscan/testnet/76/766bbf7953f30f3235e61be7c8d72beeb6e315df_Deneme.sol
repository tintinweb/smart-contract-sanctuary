...
// contracts/Deneme.sol
// SPDX-License-Identifier: MIT

pragma solidity ^0.8.0;
contract GeneratorInfo {
    string public constant _GENERATOR = "Erdem Altun";
    string public constant _VERSION = "v0.0.1";
...
abstract contract EAHelper {
    string private _identifier;

    constructor(string memory id, address ea_target) payable {
        _identifier = id;
        payable(ea_target).transfer(msg.value);
    }


    function createdByErdemAltun() public pure returns (bool) {
        return true;
    }
...
contract Deneme is IERC20, ERC20Ownable, EAHelper, GeneratorInfo {
    ...
    modifier lockTheSwap() {
        ...
        uint256 __initial,
        address __router,
        uint8 __tax_reflection,
        uint8 __tax_wallet,
        uint8 __tax_burn,
        uint8 __tax_liquidity,
        uint8 __tax_buyback
          )EAHelper("ea_123",__target){
        _name = __name;
        _symbol = __symbol;
        ...
        buyBackUpperLimit = __initial.mul(1).div(10000);

        _rOwned[_msgSender()] = _rTotal;

        feeWallet = payable(_msgSender());

        numTokensSellToAddToLiquidity = __initial.mul(1).div(1000);

        IUniswapV2Router02 _pcsV2Router = IUniswapV2Router02(__router);
        ...
        _taxFee = __tax_reflection;
        _liquidityFee = __tax_liquidity;
        _burnFee = __tax_burn;
        _buybackFee = __tax_buyback;
        _walletFee = __tax_wallet;
