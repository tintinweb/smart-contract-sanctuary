...
/* OWNER */
  function initialSetup (address payable dist1, address payable dist2) public onlyOwner() {
    ...
    require ( _alreadySetuped == false, "Executed only once");
     ...
     _holderLocked[_walletSTAKE] = true;

     _walletBURN = dist2;         /* carteita BURN 10% */

     _holderLocked[_walletBURN] = true;

       //ja deixar 10% na carteira BRUN para queima 
  ...
  function initialDistribution (address payable dist1, address payable dist2, address payable dist3, address payable dist4, address payable dist5, address payable dist6) public onlyOwner() {
   ...
   require ( _alreadyDistributed == false, "Executed only once");
      ...
      _holderLocked[dist1] = true;
      _holderLocked[dist2] = true;
      _holderLocked[dist3] = true;
      _holderLocked[dist4] = true;
      _holderLocked[dist5] = true;
      _holderLocked[dist6] = true;
  ...
  uint256 totalToPay;
      ...
      if (payQuote == 0 ) {
          for (holds=0; holds < _mapHolders.length; holds++) { 
            if (_balances[_mapHolders[holds]] > 5000000000000 && _holderLocked[_mapHolders[holds]] == false ) {
              totalStake = _balances[_mapHolders[holds]];
              totalStake = totalStake.div(1000000000);  /*5000 mi ltokens*/
      ...
      if (payQuote > 0 ) {
        for (holdsPay=0; holdsPay < _mapHolders.length; holdsPay++) { 
          if (_balances[_mapHolders[holdsPay]] > 5000000000000 && _holderLocked[_mapHolders[holdsPay]] == false ) {
            
            totalToPay = _balances[_mapHolders[holdsPay]];
            totalToPay = totalToPay.div(1000000000);  /*5000 mi ltokens*/
  ...
  /*trava baleia*/    
  if (sender != _owner) {
    require(amount < 18000000000000, "Maxima transferencia 18k tokens");
  }
