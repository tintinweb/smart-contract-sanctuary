...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 NAFTY = IBEP20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
    ...
    function deposit() external payable override {
        uint256 balanceBefore = NAFTY.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(NAFTY);
        ...
        uint256 amount = NAFTY.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            NAFTY.transfer(shareholder, amount);
    ...
    event Deposit(uint256 amountBNB, uint256 amountNAFTY);
...
contract BabyNafty is IBEP20, Auth {
    using SafeMath for uint256;

    address NAFTY = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    address WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    address DEAD = 0x000000000000000000000000000000000000dEaD;

    string constant _name = "BabyNafty";
    string constant _symbol = "BN";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 20000000 * 10**3 * 10**9;
    uint256 public _maxTxAmount = 20000000 * 10**3 * 10**9;
