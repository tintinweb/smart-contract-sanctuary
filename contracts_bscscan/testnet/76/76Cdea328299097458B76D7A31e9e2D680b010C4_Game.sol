...
abstract contract OracleClient is IClient {
    ...
    function requestRandomSeedmon(uint256 requestId, uint256 min, uint256 max, uint256 providedGas) internal {
        require(requestId > 0, "OracleClient: Invalid request Id for requesting random number");

        oracle.requestRandomSeedmon{value: providedGas}(requestId, min, max);
    }
