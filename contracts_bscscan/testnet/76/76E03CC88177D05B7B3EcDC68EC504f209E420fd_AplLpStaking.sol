...
contract AplLpStaking is Pausable, Ownable {
    ...
    function lpUnStaking(uint256 _amount, uint256 index) public {
        ...
        require(_amount <= stakingList[msg.sender][index].amount, "Not enough lp token");
            lpToken.transferFrom(address(this), msg.sender, _amount);
            cpToken.transferFrom(msg.sender, address(this), _amount);
            balanceOfLpToken[msg.sender] = balanceOfLpToken[msg.sender].sub(_amount);
            if(stakingList[msg.sender][index].amount==_amount){
                ...
                stakingList[msg.sender][index].amount = stakingList[msg.sender][index].amount.sub(_amount);
            }
            balanceOfComputePower[msg.sender] = balanceOfComputePower[msg.sender].sub(_amount);
