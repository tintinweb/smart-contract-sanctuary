...
contract OLPPaybackTrack is PaybackReward, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() PaybackReward("OLP_Reward_Tracker", "OLP_Reward_Tracker") {
        claimWait = 7200;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "OLP_Reward_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "OLP_Reward_Tracker: withdrawDividend disabled. Use the 'claim' function on the main OLP contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "OLP_Reward_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "OLP_Reward_Tracker: Cannot update claimWait to same value");
...
contract OLP is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    OLPPaybackTrack public PaybackTrack;
...
uint256 public  payBackPercent = 6;
    ...
    uint256 public  marketingPercent = 3;
  ...
  ILP public adressLiquidity;
    ...
    constructor() ERC20("ONLY PUMP", "OLP") {

        PaybackTrack = new OLPPaybackTrack();
 ...
 function rebaseToken(uint256 epoch, int256 supplyDelta)
  ...
  returns (uint256)
  {
    ...
    function updateDEXV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(DEXV2Router), "OLP: The router already has that address");
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 tokensToLP = tokensToLiquify.mul(liquidityTax).div(TotalFees).div(2);
