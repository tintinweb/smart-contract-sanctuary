...
contract DinoMarketPlace is Pausable, Ownable {
  ...
  function buyEgg(uint256 _eggGenes, uint256 _eggAmount) external {
      ...
      if(eggPriceByGenes[_eggGenes] > 0) {
        IERC20(tokenAddress).transferFrom(msg.sender, address(this), _eggAmount * eggPriceByGenes[_eggGenes]);
      } else {
        IERC20(tokenAddress).transferFrom(msg.sender, address(this), _eggAmount * defaultEggPrice);
      }
      
      totalEggByGenes[_eggGenes].sub(_eggAmount);
