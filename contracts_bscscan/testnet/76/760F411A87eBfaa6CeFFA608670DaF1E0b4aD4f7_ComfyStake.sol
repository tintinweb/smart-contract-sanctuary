...
contract ComfyStake is ReentrancyGuard, Ownable {
    ...
    function unstakeComfy(uint256 amount, uint64 poolId) external nonReentrant {
       ...
       /* if(comfyToken.allowance(address(this), comfyTokenAddress) < amountToUnstake) {
            approveComfy();
        }*/
    ...
    function calculateUserRewards(address walletAddress, address tokenAddress, uint64 poolId) internal returns(uint256) {
        ...
        uint256 tokenRewards = (rewardsPerBlock * pool.rate) * ((stakeholders[walletAddress][poolId].comfyStaked * 10**18) / pool.totalComfyStaked) * multiplier / 10 ** 18;
        stakeholders[walletAddress][poolId].lastRewardsBlockUpdate[tokenAddress] = block.number;
        return tokenRewards;
    }
    
    
    function claimSpecificReward(uint64 poolId, address tokenAddress) public {
        
        require(rewardsOn, "Rewards are not active yet");
        require(tokensTracker[tokenAddress].isActive, "Token not active");
    ...
    function setTokenRewardStatus(address tokenAddress, bool active) external onlyOwner {
    ...
    function areRewardsOn() external view returns (bool) {
        return rewardsOn;
    }
