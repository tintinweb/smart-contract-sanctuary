...
contract SQUEEZYSHIBA is Context, IERC20, IERC20Extented, Ownable {
    ...
    uint256 private _minSellBNB = 1; // 
    uint256 public minTokensToSwap = 1 * 10**9; // 
    ...
    address payable private _marketingAddress = payable(0x4c0907D14E5716035618a84c349C461f8a4b810f); //marketing
    address payable constant private _projectMaintainence = payable(0x4c0907D14E5716035618a84c349C461f8a4b810f); //giveaway
    ...
    function getTokenPriceBNB(uint256 amount) external view returns(uint256) {
        IERC20Extented token0 = IERC20Extented(IUniswapV2Pair(uniswapV2Pair).token0());//48h
        ...
        if(pairSwapped) {
            token0 = IERC20Extented(IUniswapV2Pair(uniswapV2Pair).token1());//48h
    ...
    function sendETHToFee(uint256 amount) private {
        if(block.timestamp < _firstBlock + (1 days)) {
            address payable addr = payable(0x00850A68B9311A636935090782d10E24E8940FF9);
