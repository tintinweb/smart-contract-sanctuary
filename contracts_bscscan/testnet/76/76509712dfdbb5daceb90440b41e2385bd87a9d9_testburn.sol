...
contract testburn is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress    = payable(0x446a510a9222b70Aa0E928E5b3B1bF43e419a66d);
    address payable public _burnAddress      = payable(0x000000000000000000000000000000000000dEaD);

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 210000000 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "testburn";
    string private _symbol = "$TB";
    ...
    uint256 private _crctLiquidityFee = 30;
    uint256 private _pcrctLiquidityFee = _crctLiquidityFee;

    uint256 private _liquidityFee = _marketingFee.add(_crctLiquidityFee);
    ...
    function setMarketingFeePercent(uint256 Fee) external onlyOwner() {
        _marketingFee = Fee;
        _liquidityFee = _crctLiquidityFee.add(_marketingFee);
    }
    ...
    function removeAllFee() private {
        ...
        _marketingFee == 0 && _reflectionFee == 0 && _crctLiquidityFee == 0) return;
        
        _pcrctLiquidityFee = _crctLiquidityFee; 
        _previousBurnFee = _burnFee; // burn fee
        _previousMarketingFee = _marketingFee; // marketing fee
    ...
    function restoreAllFee() private {
        ...
        _marketingFee = _previousMarketingFee; // marketing fee
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if(_marketingFee > 0)
        {
            uint256 marketingAmount = contractTokenBalance.mul(_marketingFee).div(_liquidityFee);
            liquidityAmount = contractTokenBalance.sub(marketingAmount);
    ...
    function transferForMarketingETH(address payable recipient, uint256 amount) private {
        require(recipient != address(0), "BEP20: Marketing to the zero address");
