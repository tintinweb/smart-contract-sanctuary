    ...
    PausableUpgradeable
{
    ...
    function performAddLp() public payable {
        require(msg.value >= minimumFightTax, "Tax");

        if (address(this).balance > 2 * 10**17) {
            // 0.2 BNB
            // generate the pancake pair path of token -> weth
            address[] memory path = PancakeUtil.getPath(pancakeRouter.WETH(), address(xBlade));

            // make the swap
            pancakeRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{
                value: address(this).balance.div(2)
            }(
            ...
            pancakeRouter.addLiquidityETH{value: address(this).balance.div(2)}(
                address(xBlade),
