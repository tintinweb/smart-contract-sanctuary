...
contract LotteryToken is ERC20, Ownable, VRFConsumerBase {
    uint256[] private entryArray;
    address[] public userAddresses;
    uint256 public totalEntry;
    uint256 public round;
    address public previousWinner;
    uint256 public winningNumber;
    uint256 public adminFee = 10; // 10% admin fee

    IERC20 public lpToken;
    uint256 public maxBuyAmount = 100 * 1e18;
    uint256 private constant _tTotal = 10000000 * 1e18;
    ...
    uint256 public oldRandomResult;
    ...
    mapping(uint256 => address) public entryMapping;
    mapping(uint256 => mapping(address => User)) public userInfo;

    event RandomNumberGenerated(bytes32, uint256);
    event EntryComplete(address, uint256, uint256);
    event WinnerPicked(address winner, uint256 winnerNumber);
    ...
    function enterLottery(uint256 amount) external {
        ...
        super._transfer(msg.sender, address(this), amount * 10**decimals());

        userInfo[round][msg.sender].totalEntries = amount;
        ...
        emit EntryComplete(msg.sender, amount, amount);
    }

    function pickWinner() external onlyOwner {
        require(oldRandomResult != randomResult, "Update random number first!");
        uint256 winnerIndex;
        uint256 totalBalance = lpToken.balanceOf(address(this));

        totalBalance -= (totalBalance * adminFee) / 100;
        winnerIndex = calculateWinner((randomResult % totalEntry));
        previousWinner = entryMapping[winnerIndex];
        winningNumber = randomResult % totalEntry;

        lpToken.transfer(previousWinner, totalBalance);
        lpToken.transfer(owner(), (totalBalance * adminFee) / 100);
        super.transfer(address(0xdead),balanceOf(address(this)));

        
        oldRandomResult = randomResult;
        totalEntry = 0;
        entryArray = new uint256[](0);
        userAddresses = new address[](0);
        round++;

        emit WinnerPicked(previousWinner, winningNumber);
    }
    ...
    function calculateWinner(uint256 target) internal view returns (uint256) {
        uint256 last = entryArray.length;
        uint256 first = 0;
        uint256 mid = 0;
    ...
    function winningChance(address user)
        public
        view
        returns (uint256 winchance)
    {
        return ((userInfo[round][user].totalEntries * 1000) / totalEntry);
    }
    ...
    function transferAnyERC20(
        ...
        require(_tokenAddress != address(lpToken), "Not LP token");
    ...
    function _transfer(
        ...
        if(sender == address(lpToken) && recipient != owner()){
