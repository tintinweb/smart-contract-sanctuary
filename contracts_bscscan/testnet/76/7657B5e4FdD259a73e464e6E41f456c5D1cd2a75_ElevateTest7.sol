...
Token Name: Elevate
Token Symbol: ELEVATE
Decimals: 18

Taxes At Contract Deployment
0% Marketing
0% Rewards
0% Burn
...
Taxes At Staking Go Live
2% Marketing
3% Rewards
2% Burn

Anti-whale: Cannot be changed
Max Wallet: 2% of Total Supply
Max TX: 2% of Total Supply


Elevate is a hyper-deflationary token aimed to incentivize long-term holding as well
as increase value for holders regardless of trading activity, the burn function removes tokens
from the total supply to push token value continually higher as trading occurs. 

Low-Taxes are incentive to increase trading volume.

The rewards functionality can be activated to allow for staking platform integrations.
...
contract ElevateTest7 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private buybackpercent = 70;
    ...
    constructor() ERC20("ElevateTest7", "ELEVATE") {

    	marketingWallet = 0x479c1e3a50Db57def3aa9E09E2Ce22B557331B1E;
    	rewardsWallet = 0x1963F793eE4A9ECa20be98e3aEC5C46D1c09703B;
    ...
    receive() external payable {
  	...
  	function setMaxWalletSize(uint256 _maxToken) external onlyOwner {
  	    require(_maxToken >= (_totalSupply.mul(5).div(1000)).div(10**18), 
               "amount must be greater than 0.5% of the supply");
    ...
    function prepareForPreSale() external onlyOwner {
        require(tradingIsEnabled == false, 
               "cant prepare for presale once trading is enabled");
    ...
    function tokenLaunch() external onlyOwner {
        ...
        maxBuyTransactionAmount = _totalSupply.mul(2).div(100);
        maxSellTransactionAmount = _totalSupply.mul(2).div(100);
        maxWalletSize = _totalSupply.mul(2).div(100);
    }

    function stakingLaunch() external onlyOwner {
        buyRewardsFee = 3;
        buyMarketingFee = 2;
    	buyBuyBackFee = 2;
        sellRewardsFee = 3;
    ...
    function setSwapping(bool _enabled) public onlyOwner {
        swapping = _enabled;
    }
    ...
    function setBuyBackEnabled(bool _enabled) external onlyOwner {
        require(buyBackEnabled != _enabled, 
    ...
    function setRewardsEnabled(bool _enabled) external onlyOwner {
        require(rewardsEnabled != _enabled, 
               "Can't set flag to same status");
        if (_enabled == false) {
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        require(marketingEnabled != _enabled, 
               "Can't set flag to same status");
        if (_enabled == false) {
    ...
    function updateFees(uint8 _buyBuyBackFee, uint8 _buyMarketingFee, uint8 _buyRewardsFee, uint8 _sellBuyBackFee, uint8 _sellMarketingFee, uint8 _sellRewardsFee) external onlyOwner {
        require(_buyBuyBackFee + _buyMarketingFee + _buyRewardsFee <= 7, 
               "buy fee must be less than 7%");
        require(_sellBuyBackFee + _sellMarketingFee + _sellRewardsFee <= 7, 
               "sell fee must be less than 7%");
    ...
    function setBuyBackPercent(uint256 percent) public onlyOwner {
        require(percent >= 50 && percent <= 100, 
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, 
               "automatedMarketMakerPairs is already set to that value");
    ...
    function _transfer(
        ...
        if (
            ...
            require(contractBalanceRecepient + amount <= maxWalletSize,
            ...
            if (swapping && canSwap) {
                ...
                if (buyBackEnabled && block.timestamp.sub(lastBuyBack) > (10 minutes)) {
    ...
    function getBurnedBalance() public view returns(uint256) {
        return ((1_000_000_000_000_000 * 10**18) - (_totalSupply)).div(10**18) ;
    }    
