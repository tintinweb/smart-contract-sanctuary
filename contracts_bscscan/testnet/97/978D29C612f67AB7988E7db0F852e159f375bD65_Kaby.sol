...
contract Kaby is IKaby, ERC721, AcceptedToken, ReentrancyGuard {
    ...
    function claimHero(uint versionId, uint amount, uint skillId) external override {
        ...
        for (uint i = 0; i < amount; i++) {
            uint heroId = _createHero(skillId);
    ...
    function getHero(uint heroId) external view override returns (
        ...
        uint skillCount = _heroskills[heroId].length();
        uint[] memory skillIds = new uint[](skillCount);
        for (uint i = 0; i < skillCount; i++) {
            skillIds[i] = _heroskills[heroId].at(i);
        }
    ...
    function _createHero(uint skillId) private returns (uint heroId) {
        _heros.push(Hero("", 1, 0, 0, 0, 0));
        heroId = _heros.length - 1;

        IEquipment.ItemType itemType = equipmentContract.getItemType(skillId);
        EnumerableSet.UintSet storage skills = _heroskills[heroId];

        require(itemType == IEquipment.ItemType.SKILL_BOOK, "Kaby: invalid skill book");

        bool isSuccess = skills.add(skillId);
        if (!isSuccess) revert("Kaby: already have this skill");

        uint[] memory skillIds = new uint[](1);
        skillIds[0] = skillId;
        equipmentContract.putItemsIntoStorage(msg.sender, skillIds);

        emit HeroCreated(heroId, skillId);
    }
...
interface IEquipment {
    enum ItemType { WEAPON, ARMOR, FOOTWEAR, RING, SKILL_BOOK }
...
interface IKaby {
    ...
    event HeroCreated(uint indexed heroId, uint indexed skillId);
     ...
     */
    function claimHero(uint versionId, uint amount, uint skillId) external;
