...
// File: anpans-anpan-vault/contracts/interfaces/IMasterChef.sol
...
// File: anpans-anpan-vault/contracts/AnpanVault.sol
...
contract AnpansProfile is AccessControl, ERC721Holder {
    ...
    function createProfile(
        ...
        anpanToken.safeTransferFrom(_msgSender(), address(this), numberAnpanToRegister);
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        anpanToken.safeTransferFrom(_msgSender(), address(this), numberAnpanToUpdate);
    ...
    function reactivateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        anpanToken.safeTransferFrom(_msgSender(), address(this), numberAnpanToReactivate);
...
// File: contracts/BunnySpecialAnpanVault.sol

pragma solidity ^0.6.12;

/**
 * @title BunnySpecialAnpanVault.
 * @notice It is a contract for users to mint Anpan Vault collectible.
 */
contract BunnySpecialAnpanVault is Ownable {
    using SafeMath for uint256;

    BunnyMintingStation public bunnyMintingStation;
    AnpanVault public anpanVault;
    ...
    constructor(
        ...
        anpanVault = AnpanVault(_anpanVault);
        bunnyMintingStation = BunnyMintingStation(_bunnyMintingStation);
        anpansProfile = AnpansProfile(_anpansProfile);
        endBlock = _endBlock;
    ...
    function mintNFT() external {
        ...
        bool isUserActive;
        (, , , , , isUserActive) = anpansProfile.getUserProfile(msg.sender);
        ...
        // Increase point on AnpanSwap profile, for a given campaignId.
        anpansProfile.increaseUserPoints(msg.sender, numberPoints, campaignId);

        emit BunnyMint(msg.sender, tokenId, bunnyId);
    }

    /**
     * @notice Change the campaignId for AnpanSwap Profile.
    ...
    /**
     * @notice Change the number of points for AnpanSwap Profile.
     * @dev Only callable by owner.
     */
    ...
    function _canClaim(address _userAddress) internal view returns (bool) {
        if (hasClaimed[_userAddress]) {
            ...
            if (!anpansProfile.getUserStatus(_userAddress)) {
                return false;
            } else {
                uint256 lastDepositedTime;
                (, lastDepositedTime, , ) = anpanVault.userInfo(_userAddress);
