...
contract BDCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BDC_Dividend_Tracker", "BDC_Dividend_Tracker") {
        claimWait = 14400;
    ...
    function withdrawDividend() public pure override {
        require(false, "BDC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BDC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BDC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "BDC_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BDC is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    BDCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Blood Donation Coin", "BDC") {
        BNBRewardsFee = 2;
        liquidityFee = 2;
        extraFeeOnSell = 6; // extra fee on sell
        CharityFee = 2;
        CharityWallet = payable(0x9bcBE4Af33C43E12c82d2fcD77DF6AF7F82174A2); // Acc1

        totalFees = BNBRewardsFee.add(liquidityFee).add(CharityFee); // total fee transfer and buy

    	dividendTracker = new BDCDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BDC: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BDC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BDC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BDC: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BDC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BDC: Cannot update gasForProcessing to same value");
