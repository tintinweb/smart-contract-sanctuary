...
contract GameFactory is Ownable, AccessControlEnumerable {
    ...
    event TokensDeposited(uint256 amount, uint stakeType, address indexed wallet);
    ...
    event SoldBoostItem(address indexed userAddress, uint256 itemType, uint256 count);
    event StakedWithdrawn(uint256 amount, uint stakeType, address indexed wallet);
    ...
    uint256 public constant BOOST_EXPIRE_TIME = 21 days;
  ...
  // Get Staked amount by user address
    ...
    */
    function stakeTokens(uint stakeType, uint256 amount) external {
    ...
    */      
    function stakeForBoost(uint itemType) external {
        require(isWhitelisted(msg.sender), 'You are not allowed to stake');
        ...
        platformStatistic.totalCommission = platformStatistic.totalCommission.add(amount);
        userBoostItemBalance[msg.sender][itemType] = userBoostItemBalance[msg.sender][itemType].add(1);
        boostItemExpireTime[msg.sender][itemType] = block.timestamp.add(BOOST_EXPIRE_TIME);
    ...
    */    
    function burn(address from, uint256 amount) internal {
        require(IERC20(paymentTokenAddress).transferFrom(from, address(0x000000000000000000000000000000000000dEaD), amount));
    }
    /**
    ...
    */
    function withdrawStakedToken(uint stakeType, uint256 amount) external {
        ...
        if(stakeType == 1) {
            ...
            platformStatistic.totalStakedForPlay = platformStatistic.totalStakedForPlay.sub(amount);
            platformStatistic.totalWithdrawn = platformStatistic.totalWithdrawn.add(amount.sub(commissionFee));
            platformStatistic.totalCommission = platformStatistic.totalCommission.add(commissionFee);
            emit StakedWithdrawn(amount.sub(commissionFee), stakeType,  msg.sender);
        }
        if(stakeType == 2) {
            ...
            platformStatistic.totalStakedForEarn = platformStatistic.totalStakedForEarn.sub(amount);
            platformStatistic.totalWithdrawn = platformStatistic.totalWithdrawn.add(amount.sub(commissionFee));
            platformStatistic.totalCommission = platformStatistic.totalCommission.add(commissionFee);
            emit StakedWithdrawn(amount.sub(commissionFee), stakeType,  msg.sender);
        }
    ...
    */
    function _setBaseStakeAmountForPlay(uint256 _newBaseStakeAmountForPlay) external  onlyRole(CFO_ROLE) {
        require(_newBaseStakeAmountForPlay >= 100 && _newBaseStakeAmountForPlay <= 1000000 * 10**18, 'Invalid Parameter');
    ...
    */
    function _setBaseStakeAmountForEarn(uint256 _newBaseStakeAmountForEarn) external  onlyRole(CFO_ROLE) {
        require(_newBaseStakeAmountForEarn >= 1000 && _newBaseStakeAmountForEarn <= 10000000 * 10**18, 'Invalid Parameter');
    ...
    */
    function _setThawingPeriod(uint256 _newThawingPeriod) external  onlyRole(CMO_ROLE) {
        require(_newThawingPeriod > 0 && _newThawingPeriod <= 86400, 'Invalid Parameter');
    ...
    function _setAddRewardCandidatePeriod(uint256 _newAddRewardCandidatePeriod) external onlyOwner {
        require(_newAddRewardCandidatePeriod > 0 && _newAddRewardCandidatePeriod <= 172800, 'Invalid Parameter');
    ...
    function batchAddRewardCandidates(address[] memory userAddresses, uint256[] memory approveAmounts) external onlyOwner {
        ...
        for (uint i = 0; i < userAddresses.length; i ++) {
            if (isWhitelisted(msg.sender)) {
                totalApprovedTokenAmount = totalApprovedTokenAmount.add(approveAmounts[i]);
            }
        }
        uint256 availableBalance = IERC20(paymentTokenAddress).balanceOf(address(this)).add(platformStatistic.totalRewarded).sub(platformStatistic.totalApproved);
        require(availableBalance >= totalApprovedTokenAmount, 'Game Factory Contract does not have sufficient Balance');
        for (uint i = 0; i < userAddresses.length; i ++) {
            if (isWhitelisted(msg.sender)) {
                //Capping user earnings for the reward cycle.
                uint rewardedAmount = approveAmounts[i] > maxUserEarningPerDay ? maxUserEarningPerDay : approveAmounts[i];
                rewardCandidates[userAddresses[i]].approvedAmount = rewardCandidates[userAddresses[i]].approvedAmount.add(rewardedAmount);
                rewardCandidates[userAddresses[i]].lastRewardedTime = block.timestamp;
            }
        }
        // Genesis Mining
        platformStatistic.totalApproved = platformStatistic.totalApproved.add(totalApprovedTokenAmount);
        _lastAddCandidateOperationTime = block.timestamp;
        emit RewardCandidatesAdded(userAddresses.length, totalApprovedTokenAmount);
    }
    ...
    */
    function _setMaxUserEarningPerDay(uint256 _maxUserEarningPerDay) external onlyOwner {
    ...
    */
    function withdrawFee(address _to, uint256 amount) external onlyOwner {
        uint256 availableCommissionBalance = platformStatistic.totalCommission.sub(platformStatistic.totalCommissionWithdrawn);
        uint256 contractTokenBalance = IERC20(paymentTokenAddress).balanceOf(address(this));
        require(availableCommissionBalance > amount, "insufficient commission balance");
        require(contractTokenBalance > amount, "insufficient contract balance");
        if (isBurnable) {
          burn(address(this), amount.mul(_burnFee).div(100));  
          require(IERC20(paymentTokenAddress).transfer(_to, amount.sub(amount.mul(_burnFee).div(100))));
        ...
        platformStatistic.totalCommissionWithdrawn = platformStatistic.totalCommissionWithdrawn.add(amount);
