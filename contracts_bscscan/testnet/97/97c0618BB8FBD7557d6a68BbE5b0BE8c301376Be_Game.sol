...
contract Game is Ownable, ERC721Token("Game" ,"GAME"), ERC721Holder {
    ...
    uint256 public levelChar1 = 1000000000000000000;
    uint256 public levelChar2 = 1500000000000000000;
    uint256 public levelChar3 = 1800000000000000000;
    uint256 public levelChar4 = 2100000000000000000;
    uint256 public levelChar5 = 2300000000000000000;
    uint256 public levelChar6 = 2800000000000000000;
    uint256 public levelChar7 = 3300000000000000000;
    uint256 public levelChar8 = 4000000000000000000;
    ...
    function setInitFight(uint256 _value) public onlyOwner {
        initFight = _value;
    }
    ...
    function mintCharacter(uint256 _amount, string _ipfsHash, string _name, string _description) public {
        require(priceCharacter == _amount);
        require(ipfsHashToTokenId[_ipfsHash] == 0);

        require(addressPaymentToken.transferFrom(msg.sender, address(this), _amount), "Transfer for mint error.");
    ...
    function purchaseCharacter(uint256 _tokenId, uint256 _amount) public {
        ...
        require(addressPaymentToken.transfer(sellerAddress, priceItemEtherFee), "Transfer error.");
        require(addressPaymentToken.transfer(address(this), feeToOwnerItem), "Transfer error.");
    ...
    function addPower(uint256 _tokenId) public onlyOwnerOf(_tokenId) {

        if(characters[_tokenId].power == 200){
            require(addressPaymentToken.transfer(msg.sender, levelChar1), "Transfer error.");
            characters[_tokenId].power = 300;
        }

        if(characters[_tokenId].power == 300){
            require(addressPaymentToken.transfer(msg.sender, levelChar2), "Transfer error.");
            characters[_tokenId].power = 400;
        }

        if(characters[_tokenId].power == 400){
            require(addressPaymentToken.transfer(msg.sender, levelChar3), "Transfer error.");
            characters[_tokenId].power = 500;
        }

        if(characters[_tokenId].power == 500){
            require(addressPaymentToken.transfer(msg.sender, levelChar4), "Transfer error.");
            characters[_tokenId].power = 600;
        }

        if(characters[_tokenId].power == 600){
            require(addressPaymentToken.transfer(msg.sender, levelChar5), "Transfer error.");
            characters[_tokenId].power = 700;
        }

        if(characters[_tokenId].power == 700){
            require(addressPaymentToken.transfer(msg.sender, levelChar6), "Transfer error.");
            characters[_tokenId].power = 800;
        }

        if(characters[_tokenId].power == 800){
            require(addressPaymentToken.transfer(msg.sender, levelChar7), "Transfer error.");
            characters[_tokenId].power = 900;
        }

        if(characters[_tokenId].power == 900){
            require(addressPaymentToken.transfer(msg.sender, levelChar8), "Transfer error.");
            characters[_tokenId].power = 10000;
        }

        characters[_tokenId].power = characters[_tokenId].power;
    }

    function addFight(uint256 _tokenId) public onlyOwnerOf(_tokenId) {

        if(characters[_tokenId].power == 200){
            require(addressPaymentToken.transfer(msg.sender, powerChar1), "Transfer error.");
            characters[_tokenId].amountFight = characters[_tokenId].amountFight + 10;
        }
        ...
        characters[_tokenId].amountFight = characters[_tokenId].amountFight;
    }
