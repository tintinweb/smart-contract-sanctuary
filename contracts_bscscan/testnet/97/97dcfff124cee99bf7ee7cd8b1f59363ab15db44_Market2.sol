...
contract Market2 {
  event ItemSold(address indexed _buyer, uint256 indexed _id, uint256 _value);
  function buy(uint256 _id, address seller, uint256 _price) public payable {
    emit ItemSold(seller, _id, _price);
  }
  event ItemListed(address indexed _from, uint256 indexed _id, uint256 _value);
  function sell(uint256 _id, uint256 _price, address seller) public payable {
    emit ItemListed(seller, _id, _price);
  }
  event ItemDelisted(uint256 indexed _id);
  function delist(uint256 _id) public payable {
    emit ItemDelisted(_id);
  }
}