...
// File: contracts/AnpanSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title AnpanSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract AnpanSwapLottery is ReentrancyGuard, IAnpanSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInAnpan = 50 ether;
    uint256 public minPriceTicketInAnpan = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInAnpan;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] anpanPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInAnpan;
        uint32 finalNumber;
    }
     ...
     * @param _anpanTokenAddress: address of the ANPAN token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _anpanTokenAddress, address _randomGeneratorAddress) {
        anpanToken = IERC20(_anpanTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of ANPAN to this contract
        uint256 amountAnpanToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInAnpan,
            _ticketNumbers.length
        );

        // Transfer anpan tokens to this contract
        anpanToken.safeTransferFrom(address(msg.sender), address(this), amountAnpanToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInAnpan += amountAnpanToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInAnpanToTransfer
        uint256 rewardInAnpanToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInAnpanToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        anpanToken.safeTransfer(msg.sender, rewardInAnpanToTransfer);

        emit TicketsClaim(msg.sender, rewardInAnpanToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInAnpan) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in ANPAN for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].anpanPerBracket[j] =
                ...
                // A. No ANPAN to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].anpanPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInAnpan - amountToShareToWinners);

        // Transfer ANPAN to treasury address
        anpanToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        anpanToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInAnpan += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInAnpan >= minPriceTicketInAnpan) && (_priceTicketInAnpan <= maxPriceTicketInAnpan),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInAnpan: _priceTicketInAnpan,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            anpanPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInAnpan: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(anpanToken), "Cannot be ANPAN token");
    ...
    /**
     * @notice Set ANPAN price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInAnpan: minimum price of a ticket in ANPAN
     * @param _maxPriceTicketInAnpan: maximum price of a ticket in ANPAN
     */
    function setMinAndMaxTicketPriceInAnpan(uint256 _minPriceTicketInAnpan, uint256 _maxPriceTicketInAnpan)
        external
        onlyOwner
    {
        require(_minPriceTicketInAnpan <= _maxPriceTicketInAnpan, "minPrice must be < maxPrice");

        minPriceTicketInAnpan = _minPriceTicketInAnpan;
        maxPriceTicketInAnpan = _maxPriceTicketInAnpan;
    }
     ...
     * @param _priceTicket price of a ticket (in ANPAN)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].anpanPerBracket[_bracket];
        } else {
            return 0;
        }
    }
