...
// Dependency file: contracts/IPoolManager.sol
...
// Root file: contracts/PinkLock.sol
...
// import "contracts/IPoolManager.sol";
...
contract PinkLock is Ownable, ReentrancyGuard {
  ...
  function getCumulativeLpTokenLockInfo(uint256 start, uint256 end)
    ...
    returns (CumulativeLockInfo[] memory)
  {
    require(start < end, "Start should not be before end");
    if (end > _lpLockedTokens.length()) {
      end = _lpLockedTokens.length() - 1;
    }

    CumulativeLockInfo[] memory lockInfo = new CumulativeLockInfo[](
      end - start
    );
