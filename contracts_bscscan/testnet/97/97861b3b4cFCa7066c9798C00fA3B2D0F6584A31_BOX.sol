...
contract BOX is ERC721Enumerable, Ownable {
    ...
    function _updateType(uint256 _type) internal {
        Type storage t = types[_type];
        t.remain--;
    }
    /**
     ...
     * @dev Callable by owner
     */
    function mint(uint256 _type, address _to, uint256 _tokenId) external onlyMinter {
        require(totalSupply() < maxSupply, "BOX: Total supply reached");
        require(types[_type].remain > 0, "Sold out");
        _mint(_to, _tokenId);
        tokenHash[_tokenId] = types[_type].hash;
        _updateType(_type);
    }
    ...
    function setType(string memory _name, string memory _hash, uint256 _maxSupply) public onlyOwner {
        types.push(Type(_name, _hash, _maxSupply, _maxSupply));
    }
