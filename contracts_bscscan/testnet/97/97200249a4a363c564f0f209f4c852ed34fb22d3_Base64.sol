...
contract rarity is ERC721Enumerable, IRarity {
    address public govern_token;
    ...
    constructor(address _govern_token)  {
        govern_token = _govern_token;
        owner = msg.sender;
    }


    function adventure(uint _summoner) external override {
    ...
    function level_up(uint _summoner) external override {
        require(_isApprovedOrOwner(msg.sender, _summoner));
    ...
    function summon(uint _class) external override {
        require(1 <= _class && _class <= 11);
        // transfer mint fee
        TransferHelper.safeTransferFrom(govern_token, msg.sender, address(this), mint_fee);
    ...
    function withdraw(uint amount, address receiver) external {
        require(msg.sender == owner, 'not owner call');
        uint balance = IERC20(govern_token).balanceOf(address(this));
        require(amount <= balance, 'balance not enough');
        TransferHelper.safeTransfer(govern_token, receiver, amount);
    }

    function get_all_summoners(address _owner) view external returns (uint[] memory) {
        uint count = ERC721.balanceOf(_owner);
    ...
    function get_summoners_page(address _owner, uint page, uint size) view external returns (uint[] memory) {
        require(page > 0 && size > 0, 'invalid param');
        uint count = ERC721.balanceOf(_owner);
        uint end = page * size;
        if (end > count) {
            size = size - (end - count);
        ...
        for (uint i = 0; i < size; i++) {
            res[i] = tokenOfOwnerByIndex(_owner, start + i);
        }
        return res;
    }

    function xp_required(uint current_level) public pure returns (uint xp_to_next_level) {
        xp_to_next_level = current_level * 1000e18;
        for (uint i = 1; i < current_level; i++) {
