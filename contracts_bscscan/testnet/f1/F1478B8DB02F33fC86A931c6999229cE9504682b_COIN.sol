...
/**
 * PoC Core contract for Binance Smart Chain
 *
 * #TODO:
 * X Add sell cooldown
 * - Add liq fee
 * - Lock trades by default and start it
 * - Withdraw bnb from contract (rip Burn pioneer)
 * - Add different tax for buy/sell/transfer
 * - Add small buy cooldown against bots
 * - Production/dgevelopment bool
 * - ------------------------------------------------
 * - (?)Earn
 * - (?)Swap tax to bnb to marketing (we dont want to dump our own coins)
 * - Endgame: Buyback when a certain % of locked holders is reached
 */
...
contract COIN is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    //@note: We add a mapping for the addresses that are sell locked
    ...
    //@note: If we add an address then its exclude from fees, should we do that?
    //@note: We could make a real 'janitoir' out of it being only able to do mandatory stuff 
    // without being able to disrupt trading
    address POC_WALLET = 0x0000000000000000000000000000000000000000;


    /* Token name */
    string private _name = "Test3 Token";
    ...
    // Cooldowns
    //@note: We can lower this but never higher
    ...
    constructor() {
        ...
        _isExcludedFromFee[POC_WALLET] = true;
        // Init
    ...
    function SellLockTime(uint256 sellLockSeconds)public onlyOwner{
        ...
        )
    {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rHODLrFee) = _getRValues(tAmount, tHODLrFee, tBurn, tLiquidity, _getRate());
    ...
    function removeAllFee() private {
        if (_rewardFee == 0 && _burnFee == 0 && _liquidityFee ==0) return;
    ...
    function _transfer(
        ...
        //@TODO: Fix this
        bool isBuy=from==uniswapV2Pair|| from == ROUTER;
        bool isSell=to==uniswapV2Pair|| to == ROUTER;
        ...
        if (isSell){
            // Check sell lock
            //@note: Check if they are excluded from  sell lock,
            // if not give an error when the sell lock isn't over
            // Otherwise set a new timestamp + sellLockTime and do the transfer
