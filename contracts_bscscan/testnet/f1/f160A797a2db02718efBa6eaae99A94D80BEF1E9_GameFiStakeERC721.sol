...
import './interfaces/IGameFiStakeERC721.sol';
import './interfaces/IGameFiMaster.sol';
import './interfaces/IGetStakingPower.sol';

contract GameFiStakeERC721 is
    IGameFiStakeERC721,
    ERC20,
    Ownable,
    ERC721Holder,
    ...
    ReentrancyGuard
{
    ...
    uint256 accGfiPerShare; // Accumulated GFIs per share, times 1e12. See below.
    uint256 public constant accGfiPerShareMultiple = 1E20;
    uint256 public lastRewardBlock;
    // total has stake to GameFiMaster stakingPower
    uint256 public totalStakingPower;
    IERC721 public immutable erc721;
    address public constant gameFiToken = 0x9C4569339848c70d3FE7Da7D31ba00D80b0b1c1d;
    IGameFiMaster public immutable gameFiMaster;
    ...
    constructor(
        ...
        address _gameFiMaster,
        address _erc721,
        address _getStakingPower,
        bool _isMintPowerTokenEveryTimes
    ) public ERC20(_name, _symbol) {
        gameFiMaster = IGameFiMaster(_gameFiMaster);
    ...
    // View function to see pending GFIs on frontend.
    function pendingGfi(address _user) external view override returns (uint256) {
        UserInfo memory userInfo = _userInfoMap[_user];
        uint256 _accGfiPerShare = accGfiPerShare;
        if (totalStakingPower != 0) {
            uint256 totalPendingGfi = gameFiMaster.pendingToken(address(this), address(this));
            _accGfiPerShare = _accGfiPerShare.add(
                totalPendingGfi.mul(accGfiPerShareMultiple).div(totalStakingPower)
            );
        }
        return userInfo.stakingPower.mul(_accGfiPerShare).div(accGfiPerShareMultiple).sub(userInfo.rewardDebt);
    }
    ...
    function updateStaking() public override {
        ...
        (, uint256 lastRewardDebt) = gameFiMaster.poolUserInfoMap(address(this), address(this));
        gameFiMaster.stake(address(this), 0);
        (, uint256 newRewardDebt) = gameFiMaster.poolUserInfoMap(address(this), address(this));
        accGfiPerShare = accGfiPerShare.add(
            newRewardDebt.sub(lastRewardDebt).mul(accGfiPerShareMultiple).div(totalStakingPower)
        );
        lastRewardBlock = block.number;
    }
    ...
    function _harvest(UserInfo storage userInfo) internal {
        ...
        if (userInfo.stakingPower != 0) {
            uint256 pending = userInfo.stakingPower.mul(accGfiPerShare).div(accGfiPerShareMultiple).sub(
                userInfo.rewardDebt
            );
            if (pending != 0) {
                safeGfiTransfer(_msgSender(), pending);
    ...
    function harvest() external override {
        ...
        _harvest(userInfo);
        userInfo.rewardDebt = userInfo.stakingPower.mul(accGfiPerShare).div(accGfiPerShareMultiple);
    }
    ...
    function stake(uint256 _tokenId) public override nonReentrant whenNotPaused {
        ...
        gameFiMaster.stake(address(this), stakingPower);
    ...
    function unstake(uint256 _tokenId) public override nonReentrant {
        ...
        gameFiMaster.unstake(address(this), stakingPower);
    ...
    function _approveToMasterIfNecessary(uint256 amount) internal {
        uint256 currentAllowance = allowance(address(this), address(gameFiMaster));
        if (currentAllowance < amount) {
            _approve(address(this), address(gameFiMaster), 2**256 - 1 - currentAllowance);
        }
    }
    ...
    function emergencyUnstakeAllFromGfi(uint256 _amount) external override nonReentrant onlyOwner whenPaused {
        gameFiMaster.emergencyUnstake(address(this), _amount);
        emit EmergencyUnstakeAllFromGfi(_msgSender(), _amount);
    }

    function safeGfiTransfer(address _to, uint256 _amount) internal {
        uint256 gfiBal = IERC20(gameFiToken).balanceOf(address(this));
        if (_amount > gfiBal) {
            IERC20(gameFiToken).transfer(_to, gfiBal);
        } else {
            IERC20(gameFiToken).transfer(_to, _amount);
        }
    }
...
interface IGameFiStakeERC721 {
    ...
    event EmergencyUnstakeAllFromGfi(address indexed user, uint256 amount);

    function getStakingPower(uint256 _tokenId) external view returns (uint256);

    function pendingGfi(address _user) external view returns (uint256);
    ...
    function emergencyUnstakeAllFromGfi(uint256 _amount) external;
