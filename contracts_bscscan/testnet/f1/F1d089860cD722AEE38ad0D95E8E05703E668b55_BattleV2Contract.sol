...
contract BattleV2Contract is VRFConsumerBase, Ownable {
    ...
    function _claimNFT(uint256 _battleId) private {
        ...
        require(!winner.isClaimedNFT, "BattleV2Contract: Already Claimed NFT");
    ...
    function _claimGFX(uint256 _battleId) private {
        Participant storage winner = battleHistories[_battleId];
        require(!winner.isClaimedGFX, "BattleV2Contract: Already Claimed GFX");
        ...
        _checkValidBattleId(_battleId)
    {
        require(
            insurances[msg.sender] > block.timestamp,
            "BattleV2Contract: Insurace period expired"
        );
        require(
            battles[_battleId].status == STATUS.END,
        ...
        for (uint256 i = 0; i < participants[_battleId].length; i++) {
            if (participants[_battleId][i].nftOwner == msg.sender) {
                require(
                    !participants[_battleId][i].isClaimedNFT,
                    "BattleV2Contract: Already claimed"
                );
                ...
                participants[_battleId][i].isClaimedNFT = true;
                isClaimed = true;
