...
contract Pool is IPool, OwnableUpgradeable, ReentrancyGuard {
    ...
    event Mint(address indexed account, uint256 assetAmount, uint256 collateralAmount, uint256 shareAmount);
    event Redeem(address indexed account, uint256 assetAmount, uint256 collateralAmount, uint256 shareAmount);
    event CollectRedemption(address indexed account, uint256 collateralAmount, uint256 shareAmount);
    event TreasuryMintByShares(address indexed receiver, uint256 assetAmount, uint256 shareAmount);
    ...
    function getCollateralPrice() public view override returns (uint256) {
        return (collateralOracle == address(0)) ? PRICE_PRECISION : IOracle(collateralOracle).consult(); // WMATIC = 1$
    }

    function getSharePrice() public view override returns (uint256) {
        return (shareOracle == address(0)) ? PRICE_PRECISION * 10 : IOracle(shareOracle).consult(); // Minimum 10$
    }

    function getTrueSharePrice() public view returns (uint256) {
        return (shareOracle == address(0)) ? PRICE_PRECISION * 5 : IOracle(shareOracle).consultTrue(); // Constant 5$
    }

    function getEffectiveCollateralRatio() external view override returns (uint256) {
        uint256 _collateral_bal = ITreasury(treasury).globalCollateralBalance();
        uint256 _total_asset_supply = IERC20(asset).totalSupply();
        return (_collateral_bal * (10**missing_decimals) * 10000) / _total_asset_supply;
    }
    ...
    function calcMintInput(uint256 _assetAmount) public view override returns (uint256 _collateralAmount, uint256 _shareAmount) {
        uint256 _collateral_price = getCollateralPrice();
        uint256 _share_price = getTrueSharePrice();
        uint256 _assetFullValue = (_assetAmount * _collateral_price) / PRICE_PRECISION;
        uint256 _collateralFullValue = (_assetFullValue * targetCollateralRatio) / 10000;
        _collateralAmount = (_collateralFullValue * PRICE_PRECISION) / _collateral_price / (10**missing_decimals);
        uint256 _required_shareAmount = ((_assetFullValue - _collateralFullValue) * PRICE_PRECISION) / _share_price;
        uint256 _mintingFee = ITreasury(treasury).minting_fee();
        uint256 _shareFee = (_required_shareAmount * _mintingFee) / 10000;
        _shareAmount = _required_shareAmount + _shareFee;
        uint256 _collateralFee = (_collateralAmount * _mintingFee) / 10000;
        _collateralAmount += _collateralFee;
    }

    function calcMintOutputFromCollateral(uint256 _collateralAmount) public view override returns (uint256 _assetAmount, uint256 _shareAmount) {
        uint256 _collateral_price = getCollateralPrice();
        uint256 _share_price = getTrueSharePrice();
        uint256 _collateralFullValue = (_collateralAmount * (10**missing_decimals) * _collateral_price) / PRICE_PRECISION;
        uint256 _assetFullValue = (_collateralFullValue * 10000) / targetCollateralRatio;
        _assetAmount = (_assetFullValue * PRICE_PRECISION) / _collateral_price;
        uint256 _shareFullValue = _assetFullValue - _collateralFullValue;
        _shareAmount = (_shareFullValue * PRICE_PRECISION) / _share_price;
        uint256 _mintingFee = ITreasury(treasury).minting_fee();
        _assetAmount = (_assetAmount * 10000) / (10000 + _mintingFee) - 1;
    }

    function calcMintOutputFromShare(uint256 _shareAmount) public view override returns (uint256 _assetAmount, uint256 _collateralAmount) {
        uint256 _collateral_price = getCollateralPrice();
        uint256 _share_price = getTrueSharePrice();
        uint256 _shareFullValue = (_shareAmount * _share_price) / PRICE_PRECISION;
        uint256 _assetFullValue = (_shareFullValue * 10000) / (10000 - targetCollateralRatio);
        _assetAmount = (_assetFullValue * PRICE_PRECISION) / _collateral_price;
        uint256 _collateralFullValue = _assetFullValue - _shareFullValue;
        _collateralAmount = (_collateralFullValue * PRICE_PRECISION) / (10**missing_decimals) / _collateral_price;
        uint256 _mintingFee = ITreasury(treasury).minting_fee();
        _assetAmount = (_assetAmount * 10000) / (10000 + _mintingFee) - 1;
    }

    function calcRedeemOutput(uint256 _assetAmount) public view override returns (uint256 _collateralAmount, uint256 _shareAmount) {
        uint256 _collateral_bal = ITreasury(treasury).globalCollateralBalance();
        uint256 _share_bal = ITreasury(treasury).globalShareBalance();
        uint256 _total_asset_supply = IERC20(asset).totalSupply();
        uint256 _outputRatio = (_assetAmount * 1e18) / _total_asset_supply;
        _collateralAmount = (_collateral_bal * _outputRatio) / 1e18;
        uint256 _share_out = (_share_bal * _outputRatio) / 1e18;
        uint256 _redemptionFee = ITreasury(treasury).redemption_fee();
        uint256 _shareFee = (_share_out * _redemptionFee) / 10000;
        _shareAmount = _share_out - _shareFee;
        uint256 _collateralFee = (_collateralAmount * _redemptionFee) / 10000;
        _collateralAmount -= _collateralFee;
    }
        ...
        )
    {
        ...
        (_assetOut, _required_shareAmount) = calcMintOutputFromCollateral(_collateralAmount);
        if (_required_shareAmount > _shareAmount) {
            (_assetOut, _required_collateralAmount) = calcMintOutputFromShare(_shareAmount);
            require(_required_collateralAmount <= _collateralAmount, "invalid input quantities");
        }
        require(_assetOut >= _assetOutMin, "slippage");

        (_required_collateralAmount, _required_shareAmount) = calcMintInput(_assetOut);
        ...
        require(_assetOut <= (_collateralAmount * (10**missing_decimals) * 12600) / 10000, "Insanely big _assetOut"); // double check - we dont want to mint too much asset
        ...
        IAsset(asset).poolMint(msg.sender, _assetOut);
        emit Mint(msg.sender, _assetOut, _required_collateralAmount, _required_shareAmount);
    }
    ...
    ) external nonReentrant returns (uint256 _collateral_out, uint256 _share_out) {
        require(redeem_paused == false, "Redeeming is paused");
        trimExtraToTreasury();

        (_collateral_out, _share_out) = calcRedeemOutput(_assetAmount);
        require(_collateral_out >= _collateral_out_min, "short of collateral");
        require(_share_out >= _share_out_min, "short of share");
        require(_collateral_out * (10**missing_decimals) <= (_assetAmount * 10100) / 10000, "Insanely big _collateral_out"); // double check - we dont want to redeem too much collateral
        IAsset(asset).poolBurnFrom(msg.sender, _assetAmount);

        if (_collateral_out > 0) {
            redeem_collateral_balances[msg.sender] += _collateral_out;
            unclaimed_pool_collateral += _collateral_out;
        }
        if (_share_out > 0) {
            redeem_share_balances[msg.sender] += _share_out;
            unclaimed_pool_share += _share_out;
        }
        last_redeemed[msg.sender] = block.timestamp;
        emit Redeem(msg.sender, _assetAmount, _collateral_out, _share_out);
    }
    ...
    function treasuryMintByShares(address _receiver, uint256 _assetAmount) external override onlyTreasury returns (uint256 _shareAmount) {
        require(mint_paused == false, "Minting is paused");
        mintedBySharesAmount += _assetAmount;
        require(mintedBySharesAmount <= mintedBySharesCap, "exceeds cap");
        trimExtraToTreasury();
        uint256 _share_price = getSharePrice();
        _shareAmount = (_assetAmount * PRICE_PRECISION) / _share_price;
        _transferToReserve(share, msg.sender, _shareAmount);
        IAsset(asset).poolMint(_receiver, _assetAmount);
        emit TreasuryMintByShares(_receiver, _assetAmount, _shareAmount);
    }

    function trimExtraToTreasury() public returns (uint256 _collateralAmount, uint256 _shareAmount) {
        uint256 _collateral_price = getCollateralPrice();
        uint256 _collateral_bal = ITreasury(treasury).globalCollateralBalance();
        uint256 _collateralFullValue = (_collateral_bal * (10**missing_decimals) * _collateral_price) / PRICE_PRECISION;
        uint256 _total_asset_supply = IERC20(asset).totalSupply();
        uint256 _total_asset_FullValue = (_total_asset_supply * _collateral_price) / PRICE_PRECISION;
        uint256 _share_bal = ITreasury(treasury).globalShareBalance();
        address _profitSharingFund = ITreasury(treasury).profitSharingFund();
        if (_collateralFullValue >= _total_asset_FullValue) {
            _collateralAmount = ((_collateralFullValue - _total_asset_FullValue) * PRICE_PRECISION) / (10**missing_decimals) / _collateral_price;
            if (_collateralAmount > 0) {
            ...
            uint256 _total_share_value = (_share_true_price * _share_bal) / PRICE_PRECISION;
            uint256 _total_reserve_value = _collateralFullValue + _total_share_value;
            if (_total_reserve_value > _total_asset_FullValue) {
                uint256 _extra_value_from_reserve = _total_reserve_value - _total_asset_FullValue;
                _shareAmount = (_extra_value_from_reserve * PRICE_PRECISION) / _share_true_price;
                _requestTransferFromReserve(share, _profitSharingFund, _shareAmount);
            }
        }
    }
    ...
    function _transferToReserve(
        ...
        if (_amount > 0) {
            ...
            if (_token == share) {
                ITreasury(treasury).reserveReceiveShares(_amount);
            }
        }
    }
