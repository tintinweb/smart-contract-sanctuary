...
contract ParabolicGame is Context, Ownable {
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;
    string private constant _name = "ParabolicGame";
    IPancakeRouter02 public pancakeswapV2Router;
    address public tokenAddress;
    address public withdrawerAddress;
    bool private isAvailable = false;
    event DepositParabolic(address player, uint256 amount);
    event WithdrawAll(address player, uint256 amount);
    event Withdraw(address player, uint256 amount);
    event Claim(address recipient, uint256 amount);
    event WithdrawerUpdated(address withdrawer);
    constructor() public {
        tokenAddress = 0xf6a59Af78E64b8B0BA15B2eDa2032376ee1e3bBe;
        withdrawerAddress = 0x296ed45f8344313b064332A3Fd85dF93F5051F99;
        //test net
    ...
    function setWithdrawerAddress(address withdrawer) external onlyOwner {
        withdrawerAddress = withdrawer;
        emit WithdrawerUpdated(withdrawer);
    }
    function depositParabolic(uint256 amount) public {
        IBEP20 token = IBEP20(tokenAddress);
        token.transferFrom(_msgSender(), address(this), amount);
        _balances[address(this)] = _balances[address(this)].add(amount);
        emit DepositParabolic(_msgSender(), amount);
    }
    function totalBalance() public view returns (uint256) {
        return _balances[address(this)];
    }
    function withdrawBNB() external onlyOwner {
        require(_msgSender() == owner(), "You can not run this function");
        (bool success, ) = payable(_msgSender()).call{value: address(this).balance}("");
        require(success == true);
    }
    function withdrawToken(address player, uint256 amount) public {
        require(_msgSender() == withdrawerAddress || _msgSender() == owner(), "You can not run this function");
        require(_balances[address(this)] > amount, "Insufficiant balance" );
        IBEP20 token = IBEP20(tokenAddress);
