...
contract Esgtroller is EsgtrollerV5Storage, EsgtrollerInterface, EsgtrollerErrorReporter, ExponentialNoError {
     ...
     */
    function liquidateCalculateSeizeTokens(address eTokenBorrowed, address eTokenCollateral, uint actualRepayAmount) external  returns (uint, uint) {
     ...
     */
    function claimEsg(address holder) internal {
     ...
     */
    function claimEsg(address holder, EToken[] memory eTokens) internal {
     ...
     */
    function claimEsg(address[] memory holders, EToken[] memory eTokens, bool borrowers, bool suppliers) internal {
     ...
     * @param amount The amount of ESG to (possibly) transfer
     */
    function _grantEsg(address recipient, uint amount) external {
     ...
     * @param esgSpeed New ESG speed for market
     */
    function _setEsgSpeed(EToken eToken, uint esgSpeed) external {
     ...
     */
    function _setContributorEsgSpeed(address contributor, uint esgSpeed) external {
        require(adminOrInitializing(), "only admin can set esg speed");
     ...
     */
    function getEsgAddress() internal pure returns (address) {
//        return 0x96a16178edAFF58736567Cfcaff570C06617F0Fd;
        return 0x2287818F3f8d7A51b885DB5D6bFEeab936a16040;
    }
}
...
contract PriceOracle {
    ...
    /**
      ...
      */
    function getUnderlyingPrice(EToken eToken) public returns (uint);
}
...
contract InterestRateModel {
    ...
    /**
      ...
      */
    function getBorrowRate(uint cash, uint borrows, uint reserves) public returns (uint);
...
contract ESG {
    ...
    uint public constant totalSupply = 100000000e18; // 100 million ESG
...
contract ETokenInterface is ETokenStorage {
    /**
    ...
    function exchangeRateCurrent() internal returns (uint);
    function exchangeRateStored() public returns (uint);
    ...
    function _setEsgtroller(EsgtrollerInterface newEsgtroller) internal returns (uint);
...
contract EToken is ETokenInterface, Exponential, TokenErrorReporter {
     ...
     */
    function exchangeRateCurrent() internal nonReentrant  returns (uint) {
        require(accrueInterest() == uint(Error.NO_ERROR), "accrue interest failed");
    ...
    function redeemInternal(uint redeemTokens) internal nonReentrant returns (uint) {
        ...
        return redeemFresh(address(uint160(msg.sender)), redeemTokens, 0);
    }
    ...
    function redeemUnderlyingInternal(uint redeemAmount) internal nonReentrant returns (uint) {
        ...
        // redeemFresh emits redeem-specific logs on errors, so we don't need to
        return redeemFresh(address(uint160(msg.sender)), 0, redeemAmount);
    }
    ...
    function borrowInternal(uint borrowAmount) internal nonReentrant returns (uint) {
        ...
        return borrowFresh(address(uint160(msg.sender)), borrowAmount);
    }
