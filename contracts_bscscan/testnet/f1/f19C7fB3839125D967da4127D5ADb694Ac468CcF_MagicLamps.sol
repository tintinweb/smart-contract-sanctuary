...
import "./IMagicLamps.sol";
import "./MagicLampsERC721.sol";
import "./IALDN.sol";
 ...
 */
contract MagicLamps is IMagicLamps, MagicLampsERC721 {
    ...
    uint256 public constant MAX_MAGICLAMP_SUPPLY = 11111;
    ...
    uint256 public startingIndexBlock; // TODO: remove
    uint256 public startingIndex; // TODO: remove
    ...
    event DistributeReferralRewards (uint256 indexed magicLampIndex, address indexed tokenAddress, uint256 amount);
     ...
     */
    constructor (string memory name_, string memory symbol_, address aladdin, address genie, address weth) MagicLampsERC721(name_, symbol_) {
        aladdinToken = aladdin;
        genieToken = genie;
        wethToken = IWETH(weth);

        //////////////  #tier      start id    end id  price(milliether)   ALDN reward amount(ALDN)
        TIERS.push(Tier(1,         0,          999,       100 * 10**15,    uint256(  515463917525 * 10**9) / 1000));
        TIERS.push(Tier(2,         1000,       2999,      200 * 10**15,    uint256( 2061855670103 * 10**9) / 2000));
        TIERS.push(Tier(3,         3000,       5999,      500 * 10**15,    uint256( 7731958762886 * 10**9) / 3000));
        TIERS.push(Tier(4,         6000,       8999,     1000 * 10**15,    uint256(15463917525773 * 10**9) / 3000));
        TIERS.push(Tier(5,         9000,       10999,    2000 * 10**15,    uint256(20618556701030 * 10**9) / 2000));
        TIERS.push(Tier(6,         11000,      11099,    5000 * 10**15,    uint256( 2577319587628 * 10**9) / 100));
        TIERS.push(Tier(7,         11100,      11109,   10000 * 10**15,    uint256(  515463917525 * 10**9) / 10));
        TIERS.push(Tier(8,         11110,      11110,  100000 * 10**15,    uint256(  515463917525 * 10**9) / 1));
    ...
    function initMagicLampWalletAddress(address newMagicLampWallet) public onlyOwner {
        require(address(magicLampWallet) == address(0), "Already set");
    ...
    function distributeReferralRewards(uint256 startMagicLampId, uint256 endMagicLampId) external onlyOwner {
        ...
        for (uint256 i = startMagicLampId; i <= endMagicLampId; i++) {
            ...
            if (amount > 0) {
                magicLampWallet.addTokenBalance(address(this), i, magicLampWallet.tokenTypeERC20(), address(wethToken), amount);                
                distributionAmount = distributionAmount.add(amount);
                delete referralRewards[owner];
                emit DistributeReferralRewards(i, address(wethToken), amount);
            }
        }

        if (distributionAmount > 0) {
            wethToken.deposit{ value: distributionAmount }();
            wethToken.transfer(address(magicLampWallet), distributionAmount);
    ...
    function mintMagicLamp(uint256 count, address referrer) public payable {
        ...
        require(totalSupply().add(count) <= MAX_MAGICLAMP_SUPPLY, "Exceeds max supply");
        require(estimateMagicLampPurchaseAmount(count) == msg.value, "Ether value incorrect");

        uint256 aldnRewardAmount = estimateALDNRewardAmount(count);

	    // require(aldnRewardAmount.mul(count) <= IERC20(aladdinToken).balanceOf(address(this)), "Magiclamp ALDN Token balance should be grather then supplied token");
        IALDN(aladdinToken).transfer(_msgSender(), aldnRewardAmount);
    ...
    /**
     * @dev Withdraws ALDN to treasury after sale ended
     */
    function emergencyWithdrawALDN()  public onlyOwner {
        require(totalSupply() >= MAX_MAGICLAMP_SUPPLY, "Magiclamps::emergencyWithdrawALDN: Sale has not ended");
        require(IERC20(aladdinToken).balanceOf(address(this)) > 0, "Magiclamps::emergencyWithdrawALDN: No ALDN Token balance");
        IALDN(aladdinToken).transfer(treasuryFundAddress, IERC20(aladdinToken).balanceOf(address(this)));
    }
    ...
    function _rewardReferral(address referrer, address referee, uint256 referralAmount) internal {
        uint256 referrerBalance = MagicLampsERC721.balanceOf(referrer);
    ...
    function changeName(uint256 tokenId, string memory newName) public {
        require(_msgSender() == ownerOf(tokenId), "MagicLampsERC721: caller is not the owner");
    ...
    function _percent(uint256 amount, uint256 fraction) internal pure returns(uint256) {
...
interface IMagicLamps is IERC721Enumerable {
 ...
 * {ERC721Enumerable}.
 */
contract MagicLampsERC721 is ERC165, IERC721, IERC721Metadata, Ownable {
    using Address for address;
    ...
    function balanceOf(address owner) public view virtual override returns (uint256) {
        require(owner != address(0), "MagicLampsERC721: balance query for the zero address");
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        return _tokenOwners.get(tokenId, "MagicLampsERC721: owner query for nonexistent token");
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = MagicLampsERC721.ownerOf(tokenId);
        require(to != owner, "MagicLampsERC721: approval to current owner");

        require(_msgSender() == owner || isApprovedForAll(owner, _msgSender()),
            "MagicLampsERC721: approve caller is not owner nor approved for all"
        );
    ...
    function getApproved(uint256 tokenId) public view virtual override returns (address) {
        require(exists(tokenId), "MagicLampsERC721: approved query for nonexistent token");
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(operator != _msgSender(), "MagicLampsERC721: approve to caller");
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual override {
        //solhint-disable-next-line max-line-length
        require(_isApprovedOrOwner(_msgSender(), tokenId), "MagicLampsERC721: transfer caller is not owner nor approved");
     ...
     * are aware of the MagicLampsERC721 protocol to prevent tokens from being forever locked.
     *
    ...
    function _safeTransfer(address from, address to, uint256 tokenId, bytes memory _data) internal virtual {
        _transfer(from, to, tokenId);
        require(_checkOnERC721Received(from, to, tokenId, _data), "MagicLampsERC721: transfer to non ERC721Receiver implementer");
    }
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view virtual returns (bool) {
        require(exists(tokenId), "MagicLampsERC721: operator query for nonexistent token");
        address owner = MagicLampsERC721.ownerOf(tokenId);
    ...
    function _safeMint(address to, uint256 tokenId, bytes memory _data) internal virtual {
        _mint(to, tokenId);
        require(_checkOnERC721Received(address(0), to, tokenId, _data), "MagicLampsERC721: transfer to non ERC721Receiver implementer");
    }
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        require(to != address(0), "MagicLampsERC721: mint to the zero address");
        require(!exists(tokenId), "MagicLampsERC721: token already minted");
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        require(MagicLampsERC721.ownerOf(tokenId) == from, "MagicLampsERC721: transfer of token that is not own");
        require(to != address(0), "MagicLampsERC721: transfer to the zero address");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(MagicLampsERC721.ownerOf(tokenId), to, tokenId);
    }
        ...
        private returns (bool)
    {
        if (to.isContract()) {
            try IERC721Receiver(to).onERC721Received(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert("MagicLampsERC721: transfer to non ERC721Receiver implementer");
                } else {
