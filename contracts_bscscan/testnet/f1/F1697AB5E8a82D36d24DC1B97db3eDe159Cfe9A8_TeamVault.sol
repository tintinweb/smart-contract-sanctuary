...
contract TeamVault is Ownable {
    ...
    */
     ...
     * @return bool true
    */
    function distributeTeamWallet() public returns (bool) {
        require((teamVault[_msgSender()]._isExist) || (_msgSender() == owner()), "TeamVault :: distributeTeamWallet : invalid team vault");
        require((startTime != 0) && (startTime.add(timeLock) < block.timestamp), "TeamVault :: distributeTeamWallet : Requires 180 days to pass");
        require(IERC20(DXB).balanceOf(address(this)) > 0, "TeamVault :: distributeTeamWallet : insufficient balance to distribute");
        
        uint _balance = IERC20(DXB).balanceOf(address(this));
        uint _halfBalance = _balance.div(2);
        
        _safeTransferDXB( teamWalletsList[0], _halfBalance);
        _safeTransferDXB( teamWalletsList[1], _halfBalance.div(2));
        _safeTransferDXB( teamWalletsList[2], _halfBalance.div(2));
