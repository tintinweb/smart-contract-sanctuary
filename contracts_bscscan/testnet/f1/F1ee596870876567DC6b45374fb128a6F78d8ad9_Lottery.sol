...
abstract contract Timing {
    uint32 public constant LOTTERY_DURATION =  2 hours; //12 hours;  // 12 hours
    uint32 public constant SUPER_LOTTERY_DURATION = 4 hours; //24 hours; //24 hours
...
contract Lottery is Timing, Ownable, ILottery{
    ...
    uint256 private numTokensToLottery = 1000 * 10**18;
    mapping(uint256 => mapping(address =>uint256)) public buyin; //round => address =>amount
    ...
    uint256[SUPER_LOTTRY_CANDIDATES_NUM] public superLotteryRewardPercent = [45, 25, 15, 10, 5];
    ...
    function transferNotify(address account, uint256 amount) override public{
        ...
        updateLastHeartBeat();
        updateLottery(lotteryRound, account, amount);
        updateSuperLottery(superLotteryRound, account, amount, block.timestamp);
        emit TransferNotify(account, amount, block.timestamp);
    }
