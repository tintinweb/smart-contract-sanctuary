...
contract CeresAnchor is AccessControl {
    ...
    address public usdcChainlinkAddress;
    uint8 public usdcPriceDecimals;

    IOracle public ascUsdcOracle;
    address public ascUsdcOracleAddress;

    IOracle public crsUsdcOracle;
    address public crsUsdcOracleAddress;
    ...
    // Setting of oracles
    // ------------------------------------------------------------------------
    function setAscUsdcOracle(address oracleAddr) public onlyGovernance {
        ascUsdcOracle = IOracle(oracleAddr);
        ascUsdcOracleAddress = oracleAddr;
    }

    function setCrsUsdcOracle(address oracleAddr) public onlyGovernance {
        crsUsdcOracle = IOracle(oracleAddr);
        crsUsdcOracleAddress = oracleAddr;
    }

    function setUsdcChainLink(address chainlinkAddress) public onlyGovernance {
        usdcChainlink = IChainlink(chainlinkAddress);
        usdcChainlinkAddress = chainlinkAddress;
    ...
    function getUSDCPrice() public view returns (uint256) {
        return uint256(usdcChainlink.getLatestPrice()) * (CERES_PRECISION) / (uint256(10) ** usdcPriceDecimals);
    }
    ...
    function oraclePrice(Coin choice) internal view returns (uint256) {
        ...
        uint256 usdcPriceInUSD = uint256(usdcChainlink.getLatestPrice()) * (CERES_PRECISION) / (uint256(10) ** usdcPriceDecimals);
        ...
        if (choice == Coin.ASC) {
            priceVsUsdc = uint256(ascUsdcOracle.consult(usdcAddress, CERES_PRECISION));
        } else if (choice == Coin.CRS) {
            priceVsUsdc = uint256(crsUsdcOracle.consult(usdcAddress, CERES_PRECISION));
        }

        else revert("INVALID COIN!");

        // return in 1e6 format
        return usdcPriceInUSD * CERES_PRECISION / priceVsUsdc;
    }
