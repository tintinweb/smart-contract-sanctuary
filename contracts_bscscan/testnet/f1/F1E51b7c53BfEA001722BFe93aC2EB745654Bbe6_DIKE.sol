...
contract DIKE is Context, IBEP20, IBEP20Metadata, Ownable {
    ...
    bool public sellingStatus = false; //once switched on, can never be switched off.
    ...
    function _transfer(
        ...
        if( _isGoldListed [sender] == false  && _isGoldListed[recipient] == false)
        {   
            if( _isWhiteListed [sender] == false){
                 if (!sellingStatus ) {

                 require(recipient != dexPair, "Trading is not enabled");
                }
            }
        }
