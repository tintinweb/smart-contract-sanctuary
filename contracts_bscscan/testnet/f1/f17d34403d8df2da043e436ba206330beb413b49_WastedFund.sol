...
contract AcceptedToken is Ownable {
    ...
    function collectTokenAsPrice(uint amount, address destAddress) internal {
        require(acceptedToken.balanceOf(msg.sender) >= amount, "AcceptedToken: insufficient token balance");
        acceptedToken.safeTransferFrom(msg.sender, destAddress, amount);
    }
...
// File: contracts/WastedFund.sol

pragma solidity ^0.8.7;


contract WastedFund is AcceptedToken {
    using SafeERC20 for IERC20;
    ...
    mapping(address => uint) seller;

    constructor(IERC20 tokenAccept) AcceptedToken(tokenAccept) {
    }
    function deposit(Seller[] memory info) external {
        for(uint i = 0; i < info.length; i++) {
            collectTokenAsPrice(info[i].amount, address(this));
            seller[info[i].owner] += info[i].amount;
        }
    }

    function withdraw() external {
        require(seller[msg.sender] != 0, "invalid address");
        acceptedToken.transfer(msg.sender, seller[msg.sender]);
        seller[msg.sender] = 0;
    }
}