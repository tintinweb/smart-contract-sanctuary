...
contract BondMilkyLp is Ownable {
    ...
    function deposit(
        ...
        if (fee != 0) {// fee is transferred to dao
            IERC20(milky).safeTransfer(DAO, fee);
        }
        ...
        console.log(bondInfo[_depositor].vesting);
    ...
    function stakeOrSend(address _recipient, bool _stake, uint _amount) internal returns (uint) {
        if (!_stake) {// if user does not want to stake
            IERC20(milky).transfer(_recipient, _amount);
            // send payout
        } else {// if user wants to stake
            IERC20(milky).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint) {
        return IERC20(milky).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function payoutFor(uint _value) public view returns (uint) {
        if (terms.controlVariable < 10000) {
