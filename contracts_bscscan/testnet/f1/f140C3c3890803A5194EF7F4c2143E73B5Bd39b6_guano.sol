...
contract guano is IBEP20, Auth {
    ...
    string constant _name = "guano";
    string constant _symbol = "guano";
    ...
    event FeesModifiedEvent(uint totalFee, uint liquidityFee, uint reflectionFee, uint marketingFee);
    ...
    function shouldSwapBackDebugg() external authorized view returns (bool) {
        return msg.sender != pair
        && !inSwap
        && swapEnabled
        && _balances[address(this)] >= swapThreshold;
    ...
    function swapBack() internal swapping {
        // Every time our contract reaches the swap threshold we want to redistribute an amount of tokens equal to the swapThreshold
        
        // If we have more liquidity than needed then we do not distribute tokens to the liquidity pool
        uint256 dynamicLiquidityFee = isOverLiquified(targetLiquidity, targetLiquidityDenominator) ? 0 : liquidityFee; 
        uint256 amountToLiquify = swapThreshold.mul(dynamicLiquidityFee).div(totalFee).div(2);  // The reason why we divide by two is because we need to provide a collateral as BNB, so half the tokens to liquify will be sold for BNB to then add the equivalent (in value) of tokens of BNB and Guano
        
        // The remaining tokens (the ones that do not go to the liquidity pool) will be swapped for BNBs to redistribute
        ...
        // We sell the Guano tokens for BNB to distribute
        ...
        // The amount to distribute is the difference between the BNBs we had before and after swapping the swswapThreshold
        ...
        // The distributor is in charge of purchasing the token to distribute and then send that token proportionally to the holders
        ...
        // We just add more BNB + Guanos to the liquidity pool
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator) external authorized {
        ...
        emit FeesModifiedEvent(totalFee, liquidityFee, reflectionFee, marketingFee);
    }
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amount) external authorized {
        ...
        swapThreshold = _amount * 30 / 10000;
    }
