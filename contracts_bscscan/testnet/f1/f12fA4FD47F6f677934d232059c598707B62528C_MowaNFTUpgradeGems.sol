...
contract MowaNFTUpgradeGems is Ownable, ReentrancyGuard {
    ...
    address private feeWallet = 0x8b9588F69e04D69655e0d866cD701844177360A7;
    uint256 private feeUpgrade = 2 * 10**18;
    ...
    function upgradeGems(uint256 tokenId) public nonReentrant {
        require(blackList[_msgSender()] == false, "owner has been locked");

        if(feeUpgrade > 0){
            uint256 mowaBal = mowaToken.balanceOf(_msgSender());
            require(mowaBal >= feeUpgrade, "Insufficient funds in the account");
            mowaToken.transferFrom(_msgSender(), feeWallet, feeUpgrade);
        }
        ...
        uint256 qty = mowaItemsNFT.balanceOf(_msgSender(), tokenId);
        require(qty >= 2, "Not enough gems");
        character = character.add(1);

        uint256 tokenIdNew = mowaItemsCore.getTokenWithCharacter(character);
        if (tokenIdNew == 0) {
        ...
        mowaItemsCore.safeMintNFT(_msgSender(), tokenIdNew, 1);
        mowaItemsCore.burnNFT(_msgSender(), tokenId, 2);
    }
}
