...
contract SwapContract is Ownable {
    ...
    uint256[] public contributingAmounts;
    ...
    function addLucky(uint256 amount) public {
        IERC20(Lucky).transferFrom(msg.sender, address(this), amount);

        if (userIndex[msg.sender] == 0) {
            userIndex[msg.sender] = totalUsers + 1;
            totalUsers = totalUsers + 1;
            contributingWallets.push(msg.sender);
            contributingAmounts.push(amount);
        } else {
            contributingAmounts[userIndex[msg.sender]-1] = contributingAmounts[userIndex[msg.sender]-1] + amount;
        }
        
    }

    function getContributingWallets() public view onlyOwner returns(address[] memory, uint256[] memory) {
        return(contributingWallets, contributingAmounts);
    }
