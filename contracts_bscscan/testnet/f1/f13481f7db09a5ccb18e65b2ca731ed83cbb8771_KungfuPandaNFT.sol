...
contract KungfuPandaNFT is ERC721URIStorage, Ownable {

    event GetNFT(address indexed user, uint256 indexed tokenId, uint256 types);
    // event GetNFT(uint256[5] indexed types);
	...
	constructor() ERC721("KungfuPandaNFT", "KPN"){
		counter = 0;

        BMV = ERC20(0xb3438B2Da776631965E3AB6895cA202728bF9888);
	}
    ...
    function openBlindBox(uint256 boxTypes) public returns (uint256[5] memory types){
        ...
        if(boxTypes == 1){
            ...
            BMV.transferFrom(msg.sender, address(this), 100000000000000 * 10 **6);
        ...
        if(boxTypes == 2){
            ...
            BMV.transferFrom(msg.sender, address(this), 250000000000000 * 10 **6);
        ...
        if(boxTypes == 3){
            ...
            BMV.transferFrom(msg.sender, address(this), 400000000000000 * 10 **6);
    ...
    //预售铸造NFT
    function createNFTs() internal virtual returns (uint256){
    ...
    //控制合约铸造NFT
    // function createNFT(address user, uint256 NFTType) public onlyController returns (string memory){
