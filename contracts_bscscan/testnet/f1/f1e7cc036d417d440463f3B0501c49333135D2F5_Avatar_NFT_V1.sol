...
contract Avatar_NFT_V1 is Initializable,ContextUpgradeable, AccessControlEnumerableUpgradeable, ERC1155BurnableUpgradeable,ERC1155PausableUpgradeable
{
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    address public superOwner; 

    function initialize(string memory uri, address _owner) public {
        _avatar_nft(uri);
        superOwner = _owner; 
    }
    ...
    function _avatar_nft(string memory uri) internal initializer {
    ...
    function _avatar_nft_uri(string memory _uri) internal initializer {
        _setupRole(DEFAULT_ADMIN_ROLE, superOwner);
        _setupRole(MINTER_ROLE, superOwner);
        _setupRole(PAUSER_ROLE, superOwner);
    }
    ...
    function mint(
        ...
        emit Mint(id, tokenuri, owner[id], newNft);
    }
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            owner[ids[i]] = ownerAddress;
            royalty[ids[i]][ownerAddress] = Royalty;
            emit Mint(ids[i], newuri, owner[ids[i]], newNft);
        }
    }
