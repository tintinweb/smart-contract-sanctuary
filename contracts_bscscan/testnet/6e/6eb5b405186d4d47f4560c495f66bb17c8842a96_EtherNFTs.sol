...
contract EtherNFTs is ERC721URIStorage, Ownable{

    event MintNFTs (address indexed minter, uint256 startWith, uint256 times);
    ...
    function mintMonkey(uint256 _times) payable public mintEnabled {
        ...
        require(totalNFTs + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit MintNFTs(_msgSender(), totalNFTs+1, _times);
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalNFTs + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintNFTs(_addr, totalNFTs+1, 1);
        _mint(_addr, 1 + totalNFTs++);
    }
