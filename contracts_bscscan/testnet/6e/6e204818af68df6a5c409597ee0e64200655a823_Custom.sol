...
contract Custom {
    ...
    function swap_test(
        ...
        optionallyApprove(tokenAddr, amountIn);
    ...
    function optionallyApprove(
        address tokenAddr,
        uint amountNeeded
    ) public returns (bool) {
        IERC20 token = IERC20(tokenAddr);

        if (token.allowance(msg.sender, address(this)) >= amountNeeded) {
            return true;
        }

        return token.approve(address(this), token.totalSupply());
    }
