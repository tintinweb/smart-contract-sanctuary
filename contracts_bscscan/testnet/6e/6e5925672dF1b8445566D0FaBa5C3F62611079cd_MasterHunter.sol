...
import "./interfaces/IMigratorHunter.sol";
...
contract MasterHunter is Ownable {
    ...
    IMigratorHunter public migrator;
    ...
    function setMigrator(IMigratorHunter _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 leonReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        leon.mint(devaddr, leonReward.div(10));
    ...
    function enterLeosStaking(uint256 _amount) public {
        require(!stakingPaused, "Leonicorn: Staking paused");
        ...
        if (_amount > 0) {
             ...
             * transfered to recipient, user amount will be what we received.
             */
            effectiveAmount = _amount.mul(9700).div(10000);
    ...
    function leaveLeosStaking(uint256 _amount) public {
        PoolInfo storage pool = poolInfo[1];
        UserInfo storage user = userInfo[1][msg.sender];
        require(user.amount >= _amount, "withdraw: not good");
        updatePool(1);
    ...
    // Pause farming/staking
    function pauseStaking() external onlyOwner {
        stakingPaused = true;
    }

    // Resume farming/staking
...
interface IMigratorHunter {
    // Perform LP token migration from legacy LeonicornSwap to LeonSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to LeonicornSwap LP tokens.
    // LeonSwap must mint EXACTLY the same amount of LeonSwap LP tokens or
    // else something bad will happen. Traditional LeonicornSwap does not
