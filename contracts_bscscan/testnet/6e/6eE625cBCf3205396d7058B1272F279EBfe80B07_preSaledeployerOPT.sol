...
contract preSaledeployerOPT {
    ...
    constructor() {
        admin = payable(0x9fCbfe41188E5852b147549F3a2eAf5881aE748f); //0x2dC900A489DA7d5Eb5E1eAc7F6B9f9246f4dD16d
        projectCoinFee = 3;
        projectTokenFee = 3;
        userFee = 10;
    ...
    // 14 => _softCapSeed,
    // 15 => _listingPrice,
    // 16 => _totalTokenPublic = 0
    // 17 => _totalTokenPrivate = 0
    // 18 => _totalTokenSeed = 0
    ...
    function createPreSale(
        ...
        _preSaleData[16] = getNumberOfTokensForSale(
            _preSaleData[6],
            _preSaleData[7]
        ).add(
                getNumberOfTokensForListing(
                    _preSaleData[15],
                    _preSaleData[7],
                    _preSaleData[3]
                )
            );
        _preSaleData[16] = _preSaleData[16].mul(
            10**(IERC20(_token).decimals())
        );
        _preSaleData[17] = getNumberOfTokensForSale(
            _preSaleData[9],
            _preSaleData[10]
        ).add(
                getNumberOfTokensForListing(
        ...
        _preSaleData[17] = _preSaleData[17].mul(
            10**(IERC20(_token).decimals())
        );
        _preSaleData[18] = getNumberOfTokensForSale(
            _preSaleData[12],
            _preSaleData[13]
        ).add(
                getNumberOfTokensForListing(
        ...
        _preSaleData[18] = _preSaleData[18].mul(
            10**(IERC20(_token).decimals())
        );
        ...
        _token.transferFrom(
            msg.sender,
            preSaleContract,
            _preSaleData[16].add(_preSaleData[17]).add(_preSaleData[18])
        );
        ...
        returns (uint256)
    {
        ...
        tokensForSale = tokensForSale.add(
            tokensForSale.mul(projectCoinFee).div(100)
        );
    ...
    function setData(
        ...
        projectCoinFee = _percent1;
        projectTokenFee = _percent2;
        userFee = _percent3;
        admin = _admin;
        routerAddress = _router;
        emit SetData(_admin, _router);
    }
...
contract preSale {
    ...
    event Initialize(
        ...
        uint256[] indexed _data
    );

    event AddLiquidity(uint256 indexed tokenAmount, uint256 indexed coinAmount);
    event RemoveStuckBnb();
    event RemoveStuckToken();
    event SetAllow(bool _enable);
    event SetWhiteListSeed(address[] indexed _users);
    event SetWhiteListPrivate(address[] indexed _users);
    event EndVoting();
    event StartVoting(uint256 _endTime);
...
*/
    ...
    function initialize(
        ...
        require(msg.sender == deployer, "PS: FORBIDDEN"); // sufficient check
    ...
    function buyToken(uint256 _type, uint256 _amount) public allowed {
        require(block.timestamp < preSaleEndTime, "PS: Time over"); // time check
        ...
        if (_type == 1) {
            ...
            require(whiteListPrivate[msg.sender], "PS: Not whiteListed");
            ...
            require(whiteListSeed[msg.sender], "PS: Not whiteListed");
    ...
    function claim() public allowed nonReentrant {
        require(
            block.timestamp > preSaleEndTime,
            "PS: Presale time not over"
        );
        require(canClaim, "PS: Wait for the owner to end preSale");
        require(
            tokenBalancePublic[msg.sender]
                .add(tokenBalancePrivate[msg.sender])
                .add(tokenBalanceSeed[msg.sender]) > 0,
            "PS: No claim able balance"
        );
        require(
            claimCount[msg.sender] <= currentClaimCycle,
            "PS: you have claimed in this vesting"
        );

        // require(
        //     block.timestamp >= preSaleEndTime + vestingTime,
        //     "PS: Wait for next claim date"
        // );
        ...
        if (
            ...
            emit TokenClaimed(msg.sender, tokenBalancePublic[msg.sender]);
        } else {
        ...
        if (
            ...
            emit TokenClaimed(msg.sender, tokenBalancePrivate[msg.sender]);
        } else {
        ...
        if (
            ...
            emit TokenClaimed(msg.sender, tokenBalanceSeed[msg.sender]);
        } else {
    ...
    function endPreSale() public onlyTokenOwner allowed {
        require(!presaleEnd, "PS: Presale ended");
        if (voteUp < voteDown) {
        ...
        if (!profitClaim) {
            ...
            canClaim = true;
            if (adminFeeCounter > 0  && f == true)token.transfer(admin, adminFeeCounter);
            adminFeeCounter = 0;
        }
    }
    ...
    function vote(bool _vote) public {
        require(
            ...
            "VT: Voter must be a holder"
        );
        require(
            !usersVoting[msg.sender][currentVotingCycle].voteCasted,
            "VT: cast a vote"
        );
        require(votingStatus, "VT: Not Allowed");
        require(
    ...
    function addLiquidity(uint256 tokenAmount, uint256 coinAmount) internal {
        ...
        emit AddLiquidity(tokenAmount, coinAmount);
    }

    function startVoting(uint256 _endTime) external onlyAdmin {
        require(!votingStatus, "VT: started");
        require(
        ...
        currentVotingCycle++;
        emit StartVoting(_endTime);
    }

    function endVoting() external onlyAdmin {
        require(votingStatus, "VT: ended");
        votingStatus = false;
    ...
    function setWhiteListPrivate(address[] calldata _users)
        external
        onlyTokenOwner
    {
        ...
        emit SetWhiteListPrivate(_users);
    }

    function setWhiteListSeed(address[] calldata _users)
        external
        onlyTokenOwner
    {
        for (uint256 i = 0; i < _users.length; i++) {
    ...
    function removeStuck() external onlyAdmin {
        payable(admin).sendValue(address(this).balance);
        emit RemoveStuckBnb();
    }
