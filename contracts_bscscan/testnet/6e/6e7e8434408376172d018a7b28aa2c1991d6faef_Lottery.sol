...
contract Lottery {
    ...
    constructor(address _lotteryToken, uint256 _entryAmount, uint _cap) {
        owner = msg.sender;
    ...
    function closeEntry() private {
        state = State.CLOSING;
        uint index = _randomModulo(participants.length);
    ...
    function sendFundsToWinner() onlyOwner external {
        require(state == State.CLOSING, 'Can only send funds to winner when Lottery is closed');
        lotteryToken.transfer(lastWinner,lotteryToken.balanceOf(address(this)));
        for(uint i = 0; i < participants.length; i++) {
            userEnteredActualLottery[participants[i]] = false; 
        }
        delete participants;
        state = State.ENDED;
    }
    ...
    function changeCap(uint _cap) onlyOwner public {
        cap = _cap;
    }
