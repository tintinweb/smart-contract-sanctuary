...
import "./DragonKartNFTCore.sol";
import "./base/ItemFactoryManager.sol";
import "./DragonKartNFTManager.sol";

contract DragonKartNFTUpgrader is
    ...
    ItemFactoryManager
{
    ...
    bytes32 public constant RATIO_CONTROL_ROLE = keccak256("RATIO_CONTROL_ROLE");
    bytes32 public constant LEVEL_UPGRADE_ROLE = keccak256("LEVEL_UPGRADE_ROLE");

    DragonKartNFTCore private _nftCore;
    DragonKartNFTManager private _dragonKartManager;
    uint256 private _enhancementRatioRate;
    // uint256 private _fusionRatioRate;
    ...
    function updateLevelBatch(uint256[] memory tokenIds_, uint256 level_)
        external
        view
        onlyRole(LEVEL_UPGRADE_ROLE)
    {
        require(tokenIds_.length > 0, "Upgrader: tokenId array is empty.");
        require(level_ > 0, "Upgrader: level_ is 0.");
        for (uint256 _index = 0; _index < tokenIds_.length; _index++) {
            (, uint256 _artifacts) = _nftCore.tokenMetaData(tokenIds_[_index]);
            _addArtifactValue(_artifacts, 8, 8, level_);
        }
    }

    function setEnhancementRatioRate(uint256 ratioRate_) onlyRole(RATIO_CONTROL_ROLE) external {
        _enhancementRatioRate = ratioRate_;
        emit SetEnhancementRatioRate(ratioRate_);
    }

    // function setFusionRatioRate(uint256 ratioRate_) external {
    //     _fusionRatioRate = ratioRate_;
    //     emit SetFusionRatioRate(ratioRate_);
    // }

    function setTransformationRatioRate(uint256 ratioRate_) onlyRole(RATIO_CONTROL_ROLE) external {
        _transformationRatioRate = ratioRate_;
        emit SetTransformationRatioRate(ratioRate_);
    }

    function setCloningRatioRate(uint256 ratioRate_) onlyRole(RATIO_CONTROL_ROLE) external {
        ...
        onlyRole(CONTRACT_UPGRADER)
    {
        require(
            ...
            "Upgrader: randomGenerator_ is the zero address"
        );

        _updateRandomGenerator(randomGenerator_);
    }

    function updateManager(address dragonKartManager_)
        public
        onlyRole(CONTRACT_UPGRADER)
    {
        require(
            dragonKartManager_ != address(0),
            "Upgrader: dragonKartManager_ is the zero address"
        );
        _dragonKartManager = DragonKartNFTManager(dragonKartManager_);
    }
        ...
        onlyRole(CONTRACT_UPGRADER)
    {
        require(
            nftCore_ != address(0),
            "Upgrader: nftCore_ is the zero address"
        );
        ...
        returns (uint256)
    {
        ...
        require(
            ...
            "Upgrader: 1st id is not a charm"
        );
        ...
        require(
            ...
            "Upgrader: 2nd id is not a charm"
        );
        require(
            _charmId1 != _charmId2,
            "Upgrader: 1st charm is the same as 2nd charm"
        );
    ...
    // function _calculateFusionSuccessRate(
    //     uint256 level1_,
    //     uint256 level2_,
    //     uint256 upgraderLevel_,
    //     uint256[] memory charmTokenIds_
    // ) private view returns (uint256) {
    //     return
    //         (_fusionRatioRate + uint256(75) * upgraderLevel_ + _validatingCharm(charmTokenIds_)) /
    //         (level1_ - level2_ + 1);
    // }
    ...
    function _echancement(
        ...
        bool _isUsingMagicCharm = (_charmId1 == 6 || _charmId2 == 6);
    ...
    function _transformation(
        ...
        if (isSucceeding_) {
            ...
            _createRandomAccessory(tokenId_, artifacts_, _itemType);
        }
        return tokenId_;
    }
    ...
    function _cloning(
        ...
        uint256 artifacts_,
        uint256 upgraderId_,
        uint256 upgraderArtifacts_,
        bool isSucceeding_
    ) private returns (uint256) {
        if (isSucceeding_) {
            uint256 _itemType = _getArtifactValue(artifacts_, 16, 8);
            upgraderArtifacts_ = artifacts_;
            _takeRandomFee();
            randomGenerator.requestRandomNumber(tokenId_);
            //create random accessory
            _createRandomAccessory(upgraderId_, upgraderArtifacts_, _itemType);
            return upgraderId_;
        } else {
            return tokenId_;
        }
    ...
    function upgrade(
        ...
        uint256[] memory charmTokenIds_,
        uint256 upgraderId_
    ) public onlyTokenOwner(tokenId_) returns (uint256) {
        require(tokenId_ > 0, "Upgrader: missing token ID.");

        (, uint256 _artifacts) = _nftCore.tokenMetaData(tokenId_);
        uint256 _level = _getArtifactValue(_artifacts, 8, 8);

        (, uint256 _upgraderArtifacts) = _nftCore.tokenMetaData(upgraderId_);
        uint256 _upgraderType = _getArtifactValue(_upgraderArtifacts, 16, 8);
        uint256 _upgraderId = _getArtifactValue(_upgraderArtifacts, 24, 16);
        uint256 _upgraderLevel = _getArtifactValue(_upgraderArtifacts, 8, 8);

        require(
            _upgraderType == 10 && _upgraderId >= 1 && _upgraderId <= 4,
            "Upgrader: Token is not an upgrader"
        );
        ...
        if (_upgraderId == 1) {
            ...
            bool _isSucceeding = _randomNumber > 0 && _randomNumber <= _rate;
            ...
            _nftCore.burn(_upgraderId);
        } else if (_upgraderId == 2) {
            //fusion
        } else if (_upgraderId == 3) {
            ...
            bool _isSucceeding = _randomNumber > 0 && _randomNumber <= _rate;
            _resultToken = _transformation(tokenId_, _artifacts, _isSucceeding);
            _nftCore.burn(_upgraderId);
        } else {
