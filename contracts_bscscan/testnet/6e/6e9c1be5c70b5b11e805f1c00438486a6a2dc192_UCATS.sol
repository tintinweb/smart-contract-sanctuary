...
contract UCATS is Context, IERC20, Ownable {
    ...
    address[] public _whilelist;
    bool public _isNeedCheckTransferWhitelist = false;
    ...
    constructor (
      ...
      address serviceAddr_
        ...
        // Add owner, swaprouter to whitelist
        _whilelist.push(ownerAddr_);
        _whilelist.push(routerAddr_);
    ...
    function isAllowTranfer(address account) public view returns (bool) {
      if(!_isNeedCheckTransferWhitelist) {
        return true;
      }
      for (uint256 i = 0; i < _whilelist.length; i++) {
        if(_whilelist[i] == account) {
          return true;
        }
      }
      return false;
    ...
    function allowTransfer(address[] memory accounts) external onlyOwner() returns (uint256) {
        uint8 count  = 0;
        for (uint256 i = 0; i < accounts.length; i++) {
            if (!isAllowTranfer(accounts[i])) {
                _whilelist.push(accounts[i]);
                count++;
            }
        }
        return count;
    ...
    function denyTransfer(address[] memory accounts) external onlyOwner() returns (uint256) {
        uint8 count  = 0;
        for (uint256 i = 0; i < accounts.length; i++) {
            if (isAllowTranfer(accounts[i])) {
              delete _whilelist[i];
    ...
    function fetchWhitelist() public view onlyOwner() returns (string memory) {
      string memory res = '';
      for (uint i = 0; i < _whilelist.length; i++) {
         res = string(abi.encodePacked(res, _whilelist[i], ','));
      }
      return res;
    }

    function setCheckWhitelist(bool newValue) external onlyOwner() {
      _isNeedCheckTransferWhitelist = newValue;
    }
    ...
    function _transfer(
        ...
        require(isAllowTranfer(from), "The address not allowed for transfer");
