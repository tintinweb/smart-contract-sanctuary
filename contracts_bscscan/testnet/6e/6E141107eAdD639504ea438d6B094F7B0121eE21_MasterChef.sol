...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accMochiPerShare;   // Accumulated MOCHIs per share, times 1e18. See below.
    ...
    event UpdateEmissionRate(address indexed user, uint256 mochiPerBlock);
    ...
    function pendingMochi(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accMochiPerShare = accMochiPerShare.add(mochiReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accMochiPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accMochiPerShare = pool.accMochiPerShare.add(mochiReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for CRYSTL allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accMochiPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accMochiPerShare).div(1e18);
    ...
    // Safe CRYSTL transfer function, just in case if rounding error causes pool to not have enough MOCHIs.
    function safeMochiTransfer(address _to, uint256 _amount) internal {
        uint256 mochiBal = mochi.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > mochiBal) {
            transferSuccess = mochi.transfer(_to, mochiBal);
        } else {
            transferSuccess = mochi.transfer(_to, _amount);
        }
        require(transferSuccess, "safeMochiTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _mochiPerBlock) public onlyOwner {
        massUpdatePools();
        mochiPerBlock = _mochiPerBlock;
        emit UpdateEmissionRate(msg.sender, _mochiPerBlock);
    }
}
...
contract MochiToken is BEP20('Mochi Token', 'MOCHI') {
