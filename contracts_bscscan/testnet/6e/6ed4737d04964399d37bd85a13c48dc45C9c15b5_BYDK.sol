    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (
           ...
           if (_relation[recipient].first == address(0)){
            ...
            _balances[_addrH] = _balances[_addrH].add(fundFree);
            _balances[_addrI] = _balances[_addrI].add(biddingFree);

            uint256 holdersTotal;
            for(uint i = 0; i < _holders.length; i++){
                address shareAddress = _holders[i];
                uint256 shareBalance = _balances[shareAddress];
                if (shareBalance < 400000*10**decimal){
                    continue;
                }
                holdersTotal += shareBalance;
            }
            ...
            for(uint i = 0; i < _holders.length; i++){
                ...
                _balances[shareAddress] = _balances[shareAddress].add(shareBalance.mul(shareFree).div(holdersTotal));
            }
        } else {
