...
contract MembershipPool is ReentrancyGuard,Authorizable{
    ...
    uint256 public medalRewardDuration = 7 days;
    ...
    mapping(address => uint256[POOLS_INDEX]) userRewardMedal;
    ...
    constructor(
        ...
        require(_medalDistributeAt >= block.timestamp,"Membership Pool: _medalDistributeAt must be greater than current timestamp");
    ...
    function earnedMedal(address account,uint256 pool) public view returns (uint256) {
        ...
        if(pool == getPool(account)){
            uint medalAmount = block.timestamp.sub(lastTimeMedalGet[account]).div(medalRewardDuration);
            return userRewardMedal[account][pool].add(medalAmount);
        }
        return userRewardMedal[account][pool]; 
    }
    ...
    function _updateRewardNFT(address _address) internal {
        if(lastTimeMedalGet[_address] == 0){
            lastTimeMedalGet[_address] = medalDistributeAt;
        }
        if(lastTimeMedalGet[_address].add(medalRewardDuration) < block.timestamp){
            uint256 _index = getPool(_address);
            if(_index != 0){
                uint medalAmount = block.timestamp.sub(lastTimeMedalGet[_address]).div(medalRewardDuration);
                userRewardMedal[_address][_index] = userRewardMedal[_address][_index].add(medalAmount);
                lastTimeMedalGet[_address] = medalDistributeAt;
                //emit MedalRewardAdded(_address,medalAmount,_index,block.timestamp);
            }
            else{
    ...
    function notifyMedalRewardTime(uint256 _medalDistributeAt) public onlyAuthorized() updateReward(address(0)){
        require(_medalDistributeAt > medalDistributeAt,"Membership Pool: disribution must be greater then privious one");
        medalDistributeAt = _medalDistributeAt;
    }
    ...
    function setMedalRewardsDuration(uint256 _rewardsDuration) external onlyOwner() {
        require(
            _rewardsDuration != 0,
    ...
    //event MedalRewardAdded(address user,uint256 _medalAmount,uint256 _poolMedal,uint256 _atTime);
}