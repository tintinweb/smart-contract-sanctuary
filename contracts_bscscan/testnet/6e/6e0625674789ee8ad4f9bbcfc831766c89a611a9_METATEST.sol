...
contract Ownable is Context {
    ...
    function F52_Owner_Change(address newOwner) public virtual onlyOwner {
        // IMPORTANT: 
    ...
    function F54_Change_Security_Manager(address newSecurityManager)  public virtual onlySecurityManager {
        // IMPORTANT: 
        // Usually one needs also to execute F28_Enable_Must_Pay_Fees function
...
contract METATEST is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router)) {
                ...
                // Resetting these so make sure that _takeProjectFee works as  
    ...
    function F45_Add_Bridge_Or_Exchange(address account, uint256 proj_fee, uint256 reflections_fee) external onlyOwner {
        ...
        require(account != _marketingWallet, "Marketing wallet is not allowed");	
        require(account != _blockchainSupportWallet, "Blockchain Support wallet is not allowed");
    ...
    function F51_Rescue_Tokens_Sent_To_This_Contract_By_Mistake(IERC20 token, address receiver, uint256 amount) external onlyOwner {
