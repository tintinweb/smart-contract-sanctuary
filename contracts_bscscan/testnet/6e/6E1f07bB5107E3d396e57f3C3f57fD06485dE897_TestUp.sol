...
contract TestUp is BEP20 {
    ...
    uint256 public minAmountToLiquify = 5e6 ether;
    // Burn address
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        // Antiwhale only for selling (sad!)
        if (antiWhaleActivated && recipient == pair) {
            if (
                _excludedFromAntiWhale[sender] == false
                && ( _excludedFromAntiWhale[recipient] == false
                || recipient == pair) // Pair isn't excluded if we buy, onky if we sell
            ) {
                require(amount <= maxTransferAmount(), "UpForever::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
     ...
     */
    constructor(address _router) public BEP20("Test12", "T12") {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private transferTaxFree {
        ...
        emit Debug("Liquidity Added", ethAmount);
    }
    ...
    function updateRouter(address _router) public onlyOperator {
        ...
        setExcludedFromAntiWhale(pair, true);
