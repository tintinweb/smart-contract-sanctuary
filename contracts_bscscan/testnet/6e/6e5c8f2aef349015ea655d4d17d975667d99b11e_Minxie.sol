...
/**
 * @title Minxie ERC20 token
 * @dev
 *      Based on the Ampleforth & Safemoon protocol.
 */
contract Minxie is ERC20Detailed, Ownable {
    ...
    address public masterMinx;

    // LP atomic sync
    address public lp;
    mLP public lpContract;

    modifier onlymasterMinx() {
        require(msg.sender == masterMinx);
        _;
    }
    ...
    uint256 private constant fragmentalSupply = 10**15 * 10**DECIMALS;
    ...
    address payable public iceBlueMinxAddress;
    ...
    // TOTAL_MINX is a multiple of fragmentalSupply so that _minxPerFragment is an integer.
    // Use the highest value that fits in a uint256 for max granularity.
    uint256 private constant TOTAL_MINX = MAX_UINT256 - (MAX_UINT256 % fragmentalSupply);

    // MAX_SUPPLY = maximum integer < (sqrt(4*TOTAL_MINX + 1) - 1) / 2
    uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

    uint256 private _totalSupply;
    uint256 private _minxPerFragment;
    mapping(address => uint256) private _gonBalances;

    // This is denominated in Fragments, because the minx-fragments conversion might change before
    // it's fully paid.
    mapping (address => mapping (address => uint256)) private _allowedFragments;

    constructor (address router, address payable _iceBlueMinxAddress)
        ERC20Detailed("Minxies", "MINXS", uint8(DECIMALS))
        payable
        public
    {
        iceBlueMinxAddress = _iceBlueMinxAddress;
        ...
        _totalSupply = fragmentalSupply;
        _gonBalances[msg.sender] = TOTAL_MINX;
        _minxPerFragment = TOTAL_MINX.div(_totalSupply);
    ...
    /**
     * @notice Sets a new masterMinx
     */
    function setmasterMinx(address _masterMinx)
        external
        onlyOwner
        returns (uint256)
    {
        masterMinx = _masterMinx;
    }
    ...
    function balanceOf(address who)
        ...
        returns (uint256)
    {
        return _gonBalances[who].div(_minxPerFragment);
    }
   ...
   returns (bool)
   {
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
      (uint256 tTransferAmount, uint256 tFee) = _getTValues(amount);
          uint256 gonDeduct = amount.mul(_minxPerFragment);
          uint256 gonValue = tTransferAmount.mul(_minxPerFragment);
    ...
    function _transferExcluded(address sender, address recipient, uint256 amount) private {
          uint256 gonValue = amount.mul(_minxPerFragment);
    ...
    function _takeFee(uint256 tFee) private {
        uint256 rFee = tFee.mul(_minxPerFragment);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // Transfer to iceBlueMinx address
        transferToAddressETH(iceBlueMinxAddress, sharedETH);
