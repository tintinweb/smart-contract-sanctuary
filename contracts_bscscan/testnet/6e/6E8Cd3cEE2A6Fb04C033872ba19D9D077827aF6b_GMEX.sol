...
contract GMEX is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 amountToCharity = rLiquidity.div(8).mul(5);
        uint256 RLiq = rLiquidity.sub(amountToCharity);
        _rOwned[address(this)] = _rOwned[address(this)].add(RLiq);
        uint256 tokenToCharity = tokenFromReflection(amountToCharity);
        sendBNBToCharity(tokenToCharity);
