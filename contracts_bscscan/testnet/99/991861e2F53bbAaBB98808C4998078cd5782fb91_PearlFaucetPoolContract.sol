...
contract PearlFaucetPoolContract {
    ...
    function invest(uint256 _tokenAmount) public {
        ...
        _tokenAmount = _tokenAmount.sub(_tokenAmount.mul(slippage).div(percentDivider));
    ...
    function withdraw() public {
        ...
        totalAmount = totalAmount.sub(totalAmount.mul(slippage).div(percentDivider));
    ...
    function reinvestStake() public returns (bool) {
        ...
        totalWithdrawn = totalWithdrawn.add(totalAmount);
        totalAmount = totalAmount.sub(totalAmount.mul(slippage.div(2)).div(percentDivider));
    ...
    function updateReferrer(address _newReferrer) public {
        ...
        require(users[msg.sender].referrer == address(0),"already have referrer");
