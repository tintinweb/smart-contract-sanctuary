...
contract SignData {
    ...
    constructor() internal {
        ...
        CONVERT_HASH = keccak256("Data(bytes adminSignedData,uint256 nonce)");
    }
...
contract KawaiiMinting is Signed, Ownable, SignData {
    using SafeMath for uint256;
    ...
    constructor(IBEP20 _kawaiiToken) public {
    ...
    function convertItem(address _nft1155Address, uint256 _tokenId, uint256 _amount, uint256 _timestamp, address _sender, bytes memory _adminSignedData, uint8 v, bytes32 r, bytes32 s) public {
        verify(keccak256(abi.encode(CONVERT_HASH, keccak256(_adminSignedData), nonces[_sender])), _sender, v, r, s);
        (v, r, s) = abi.decode(_adminSignedData, (uint8, bytes32, bytes32));
        address signer = getSigner(
            keccak256(
                abi.encode(address(this), this.convertItem.selector, _nft1155Address, _tokenId, _amount, _timestamp, nonces[_sender]++)
            ), v, r, s);
        require(isSigner[signer] == true, "Forbidden");

        uint256 amountPay = prices[_tokenId].mul(_amount);
        if (amountPay > 0) {
            kawaiiToken.safeTransferFrom(_sender, address(this), amountPay);
        }
        IERC1155(_nft1155Address).mint(_sender, _tokenId, _amount);
        emit  Convert(_sender, _tokenId, _amount);
    }
