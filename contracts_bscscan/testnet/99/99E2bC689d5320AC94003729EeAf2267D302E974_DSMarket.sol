...
contract DSMarket is ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;
    using Counters for Counters.Counter;
    Counters.Counter private _itemIds;
    Counters.Counter private _itemsSold;

    enum Currency { DBZ, BNB }
    ...
    mapping(address => mapping(uint256 => uint256)) private nftContractToMarketItemId;
    ...
    event UpdateListingFeeRecipient(address listingFeeRecipient);
    ...
    function setPaymentToken(address _paymentToken) external onlyOwner {
        dbzToken = IERC20(_paymentToken);
    }

    function getItemPrice(uint256 _itemId) external view returns (uint256, uint256) {
        return (tokenIdToMarketItem[_itemId].priceDBZ, tokenIdToMarketItem[_itemId].priceBNB);
    }
 
    function updateItemPrice(uint256 _itemId, uint256 _newPriceDBZ, uint256 _newPriceBNB) external {
        address tokenSeller = tokenIdToMarketItem[_itemId].seller;
        require(_msgSender() == tokenSeller, "DSMarket: should be token seller");
        MarketItem storage item = tokenIdToMarketItem[_itemId];
        item.priceDBZ = _newPriceDBZ;
        item.priceBNB = _newPriceBNB;

        emit MarketItemPriceUpdated(_itemId, _newPriceDBZ, _newPriceBNB);
    }
    ...
    function createMarketItem(
        ...
        require(
            ...
            _msgSender() == IERC721(_nftContract).getApproved(_tokenId), 
        ...
        require(_priceDBZ > 0 || _priceBNB > 0, "DSMarket: price must be greater than zero");
        require(msg.value >= listingFee, "DSMarket: not enough bnb for listing item");
        ...
        nftContractToMarketItemId[_nftContract][_tokenId] = _itemId;
    ...
    function getMarketItemId(address _nftContract, uint256 _tokenId) view external returns (uint256) {
        uint256 itemId = nftContractToMarketItemId[_nftContract][_tokenId];
        return itemId;
    }
    ...
    function createMarketSale(
        ...
        uint256 priceDBZ = tokenIdToMarketItem[_itemId].priceDBZ;
        uint256 priceBNB = tokenIdToMarketItem[_itemId].priceBNB;

        if (_amount != 0) {
            require(priceDBZ != 0, "DSMarket: market token price can not be zero");
            require(_amount == priceDBZ, "DSMarket: please submit the asking price in order to complete the purchase");
            require(IERC20(dbzToken).allowance(_msgSender(), address(this)) == _amount, "DSMarket: marketplace should be approved as a spender");
            
            IERC20(dbzToken).safeTransferFrom(_msgSender(), tokenSeller, _amount);
    ...
    function cancelMarketItem(uint256 _itemId) external nonReentrant {
        ...
        if (_msgSender() == marketItemCreator) {
            IERC721(nftContract).safeTransferFrom(address(this), _msgSender(), tokenId);
    ...
    // listingFee == 50000000000000000 wei, 0,05 bnb
    function updateListingFee(uint256 _listingFee) external onlyOwner {
        listingFee = _listingFee;
        emit UpdateListingFee(_listingFee);
    }

    function updateListingFeeRecipient(address payable _listingFeeRecipient) external onlyOwner {
        feeRecipient = _listingFeeRecipient;
        emit UpdateListingFeeRecipient(_listingFeeRecipient);
    }
    ...
    function fetchPurchasedNFTs() external view returns (MarketItem[] memory) {
        ...
        for (uint i = 0; i < totalItemCount; i++) {
            if (tokenIdToMarketItem[i + 1].owner == _msgSender()) {
            itemCount += 1;
            }
        }
    ...
    function fetchItemsCreated() external view returns (MarketItem[] memory) {
        ...
        for (uint i = 0; i < totalItemCount; i++) {
            if (tokenIdToMarketItem[i + 1].seller == _msgSender()) {
            itemCount += 1;
            }
        }
