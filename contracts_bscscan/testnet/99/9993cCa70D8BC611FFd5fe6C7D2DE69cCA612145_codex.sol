...
contract codex is Context, IERC20, Ownable {
	...
	address payable private _devWallet = payable(0xCf670c98C4C4163d8Da291b20d5893eA2Cb28af8);
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tmarketing, uint256 tDead, uint256 tdev) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tmarketing, data.tdev, data.tDead);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 totalFee = tFee.add(tLiquidity).add(tmarketing).add(tdev).add(tDead);
        uint256 tTransferAmount = tAmount.sub(totalFee);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tmarketing, tdev, tDead, 0));
    }
    ...
    function _getRValues( TData memory _data) private pure returns (uint256, uint256, uint256) {
		...
		uint256 totalFee = rFee.add(rLiquidity).add(rmarketing).add(rdev).add(rDead);
        uint256 rTransferAmount = rAmount.sub(totalFee);
