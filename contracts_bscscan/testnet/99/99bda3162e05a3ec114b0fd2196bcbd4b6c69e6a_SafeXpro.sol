...
contract SafeXpro is Context, IBEP20, Ownable {
    ...
    constructor (address buybackWallet, address marketingWallet, address researchWallet, address pancakeSwapRouter) {
        _rOwned[_msgSender()] = _rTotal;
		
        _buybackWallet = payable(buybackWallet);
        _marketingWallet = payable(marketingWallet);
        _researchWallet = payable(researchWallet);
    ...
    function SafeTransferTokens(IBEP20 token, address recipient, uint256 amount) external onlyOwner {
        token.safeTransfer(recipient, amount);
    }

    function updateGas(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000);
        emit GasForProcessingUpdated(newValue, divgas);
        divgas = newValue;
    }
    ...
    function StopLockTime() external onlyOwner() {
        lockTime = false;
    }
	...
	function excludeAddressFromMaxTx(address account) external onlyOwner {
        _isExcludeAddressFromMaxTx[account] = true;
    }
    
    function includeAddressFromMaxTx(address account) external onlyOwner {
    ...
    function setNumTokensSellToAddToLiquidity(uint256 amount) external onlyOwner() {
        numTokensSellToAddToLiquidity = amount.mul(10**9);
    }
    ...
    function setMaxTxAmount(uint256 buyTxAmount, uint256 sellTxAmount, uint256 sellTxAmount2, uint256 maxWalletAmount, uint256 minWalletAmount) external onlyOwner() {
        _buyMaxTxAmount = buyTxAmount.mul(10**9);
        _sellMaxTxAmount = sellTxAmount.mul(10**9);
        _sellMaxTxAmount2 = sellTxAmount2.mul(10**9);
        _maxWalletAmount = maxWalletAmount.mul(10**9);
        _minWalletAmount = minWalletAmount.mul(10**9);
    }
	
	function setWalletAddress(address buybackWallet, address marketingWallet, address researchWallet) external onlyOwner() {
         _buybackWallet = payable(buybackWallet);
         _marketingWallet = payable(marketingWallet);
         _researchWallet = payable(researchWallet);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        require(tradingOn || canTransferBeforeTradingOn[from], "Trading still OFF");
        require(!_isLockedWallet[from], "Locked wallet cant call this function");
        require(from != address(0), "BEP20: transfer from the zero address");
        require(to != address(0), "BEP20: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        if(!(_isExcludeAddressFromMaxTx[from] || _isExcludeAddressFromMaxTx[to])) {
            require(amount <= _buyMaxTxAmount, "Exceeds maxTxAmount");
            if (to != pancakeSwapV2Pair) {		            
		        ...
		        require(amount <= _sellMaxTxAmount, "Exceeds Sell maxTxAmount");
            ...
            if (!_isExcludedFromTxlock[from] && lockTime && (to == pancakeSwapV2Pair) && (from != pancakeSwapV2Pair) && from != owner() && to != owner()) {
                ...
                require(_isExcludedFromTxlock[from] || (_transactionCheckpointAmt[from].add(amount) <= _sellMaxTxAmount2), "Please wait for transaction cooldown time to finish");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        (_buybackWallet.call{gas: divgas, value: ethForBuyback}(""));
        (_marketingWallet.call{gas: divgas, value: ethForMarketing}(""));
        (_researchWallet.call{gas: divgas, value: ethForResearch}(""));
    ...
    function updatePCSRouter(address newAddress) external onlyOwner {
