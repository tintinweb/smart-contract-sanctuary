...
interface IBEP20 {
  ...
  */
  ...
  function pix(uint amount_pix) external payable returns (bool);
...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  /* 7EX implementation */
  uint public amount_bnb;
  uint public token_price;
  uint public sell_des;
  /* ------------------ */
  ...
  constructor() public {
    ...
    _symbol = "7EX";
    _decimals = 2;
    _totalSupply = 20000 * 10 ** uint(_decimals);
  ...
  function withdraw() public payable returns(bool) {    
    require(msg.value > 0);
    require(_balances[address(msg.sender)] > msg.value);
    
    uint bnbs;
    bnbs = msg.value - (msg.value / sell_des);
    
    require(amount_bnb > bnbs);
   ...
   * @dev PIX 7EX Token.
   */
  function pix(uint amount_pix) onlyOwner public payable returns(bool) {
    
    require(amount_pix > 0);
    require(amount_bnb >= amount_pix);
    
    amount_bnb -= amount_pix;
    msg.sender.transfer(amount_pix);
