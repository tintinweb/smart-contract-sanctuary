...
contract UCTReward is RewardHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() RewardHolders("UCT_Reward_Tracker", "UCT_Reward_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "UCT_Reward_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "UCT_Reward_Tracker: withdrawDividend disabled. Use the 'claim' function on the main UCT contract.");
    }
...
contract UCT is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    UCTReward public Reward;
   uint256 _totalSupply =50000000  * (10**9);
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Up Candy", "UCT") {
...
emit RebaseUpdate(epoch, _totalSupply);
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "UCT: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "UCT: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "UCT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "UCT: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTUCT = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTUCT);
...
addLiquidity(tokensTUCT, bnbForLiquidity);
