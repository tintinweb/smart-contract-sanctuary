...
contract PerformanceDistribution is Ownable, ReentrancyGuard {
  ...
  {
    uint256 length = _distributionlength();
    address[] memory addrs = new address[](length);
    uint256[] memory amount = new uint256[](length);
    uint256[] memory remainingAmount = new uint256[](length);
    uint256 count = 0;

    for (uint256 i = 0; i < members.length; i++) {
      for (uint256 j = 0; j < distributionInfo[members[i]].length; j++) {
        DistributionInfo storage distribution = distributionInfo[members[i]][j];
        addrs[count] = members[i];
        amount[count] = distribution.amount;
        remainingAmount[count] = distribution.remainingAmount;
        count++;
      }
    }

    // return (addrs, amount, remainingAmount);
    return (addrs);
  }

  function _distributionlength() internal view returns (uint256) {
    uint256 length = 0;
