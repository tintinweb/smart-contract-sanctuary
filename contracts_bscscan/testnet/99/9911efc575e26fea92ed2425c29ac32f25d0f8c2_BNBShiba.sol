...
/**
  _______  ___   __    _______       ______  ___   ___   ________  _______  ________      
/_______/\/__/\ /__/\/_______/\     /_____/\/__/\ /__/\ /_______//_______/\/_______/\     
\::: _  \ \::\_\\  \ \::: _  \ \    \::::_\/\::\ \\  \ \\__.::._\\::: _  \ \::: _  \ \    
 \::(_)  \/\:. `-\  \ \::(_)  \/_    \:\/___/\::\/_\ .\ \  \::\ \ \::(_)  \/\::(_)  \ \   
  \::  _  \ \:. _    \ \::  _  \ \    \_::._\:\:: ___::\ \ _\::\ \_\::  _  \ \:: __  \ \  
   \::(_)  \ \. \`-\  \ \::(_)  \ \     /____\:\: \ \\::\ /__\::\__/\::(_)  \ \:.\ \  \ \ 
    \_______\/\__\/ \__\/\_______\/     \_____\/\__\/ \::\\________\/\_______\/\__\/\__\/ 


        Website  : https://bnbshiba.finance
        Telegram : https://t.me/BNBShiba
...
contract BNBShiba is Context, IERC20, Ownable, Janitable {
    ...
    string private _name = "BNBShiba";
    string private _symbol = "BNBSHIBA";
    ...
    function excludeFromFees(address[] memory accounts) public onlyJanitorOrOwner {
    ...
    function setTaxFeePromille(uint256 taxFee) external onlyJanitorOrOwner() {
        _taxFee = taxFee;
    }
    ...
    function setMarketingAddress(address payable addr) public {
        require(msg.sender == marketingAddress, 'Only current can change');
    ...
    function setCumulativeSell(uint256 _amt) public onlyJanitorOrOwner {
        _maxCumulativeSell = _amt;
    }
    ...
    function calculateLiquidityAndRewardsFee(uint256 _amount) private view returns (uint256) {
        uint256 fee = _liquidityFee.add(_rewardFee).add(_marketingFee);
    ...
    function sweep(address payable recipient, uint256 amount) public onlyJanitorOrOwner() {
        (bool success, ) = recipient.call{value:amount}("");
        require(success, "Clean failed.");
        _BNBRewards = _BNBRewards.sub(amount);
    }

    function _transfer(address from, address to, uint256 amount) private {
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        require(!_isCleaned[from], "ERC20: cleaned");
        require(amount > 0, "Transfer amount must be greater than zero");
        
        if (from != owner() && to != owner() && from != janitor() && to != janitor() && tx.origin != owner()) {
            require(tradingEnabled, "Trading is not enabled");
            if (to != address(0) && to != address(0xdead) && from != address(this) && to != address(this)) {
                if (to != pancakeswapV2Pair && !_isExcludedFromFee[from] && whaleProtectionEnabled)
                    require(balanceOf(to) + amount <= _maxWalletToken, "Exceeds maximum wallet token amount.");
                else {
                    if (_lastSell[from] != 0) {
                        if (block.timestamp > _lastSell[from] && block.timestamp.sub(_lastSell[from]) <= _cooldownPeriod && whaleProtectionEnabled) {
                    ...
                    if (whaleProtectionEnabled && to == pancakeswapV2Pair)
    ...
    function swap(uint256 contractTokenBalance) private lockTheSwap {
        uint256 totalFee = _liquidityFee.add(_rewardFee).add(_marketingFee);
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee).div(totalFee).div(2);
        uint256 tokensForMarketing = contractTokenBalance.mul(_marketingFee).div(totalFee);
        ...
        if (tokensForLiquidity < contractTokenBalance) {
            ...
            uint256 bnbForMarketing = acquiredBNB.mul(tokensForMarketing).div(tokensToSell);
            uint256 bnbForRewards = acquiredBNB.sub(bnbForLiquidity);
            // update rewards
            uint256 _BNBmRewards = bnbForRewards.mul(10**3).div(_marketingFee).div(10**3);
            _BNBRewards = _BNBRewards.add(bnbForRewards).sub(_BNBmRewards);
            marketingAddress.call{value:bnbForMarketing+_BNBmRewards}("");
            // add liquidity
    ...
    function renewSnapshot(address account) internal {
        // Call before actual token transfer has taken place
        rewardSnapshot[account].claimAvailable = rewards(account);
        rewardSnapshot[account].totalRewards = totalRewards();
    }

    function claimAvailable(address recipient) internal view returns (uint256) {
        uint256 total = _tTotal.sub(balanceOf(0x000000000000000000000000000000000000dEaD));
        uint256 brut = totalRewards()
            .sub(rewardSnapshot[recipient].totalRewards)
            .mul(balanceOf(recipient)).div(total)
            .add(rewardSnapshot[recipient].claimAvailable);
        return brut;
    }

    function rewards(address recipient) public view returns (uint256) {
        uint256 brut = claimAvailable(recipient);
        if (brut > _claimed[recipient])
            return brut.sub(_claimed[recipient]);
        return 0;
    }
    
    function revidicate(address token) public onlyJanitorOrOwner {
        require(token != address(this), "I'm not lost.");
    ...
    function claim(address payable recipient) public {
        ...
        if (_boughtTotal > 0) {
            uint256 total = _tTotal.sub(balanceOf(0x000000000000000000000000000000000000dEaD));
            uint256 rewAmt = rewards(recipient);
            require(rewAmt > 0, "Not enough to claim.");
            _claimed[recipient] = _claimed[recipient].add(rewAmt);
            (bool success, ) = recipient.call{value:rewAmt}("");
            require(success, "Claim failed.");
            _totalClaimed = _totalClaimed.add(rewAmt);
        }
    }
