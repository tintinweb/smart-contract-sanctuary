...
contract StoneNFT is ERC721 {

  uint256 public latestTokenId;

  event EvolverChanged(address newEvolver);
  ...
  constructor(address _manager) public ERC721("Stone", "STONE", _manager){  
  }
  ...
  function mint(address receiver) external onlySpawner {
    uint256 nextTokenId = _getNextTokenId();
    _mint(receiver, nextTokenId);
    _incrementTokenId();
  }

  function burn(uint256 tokenId) external onlySpawner {
    _burn(tokenId);
  }
