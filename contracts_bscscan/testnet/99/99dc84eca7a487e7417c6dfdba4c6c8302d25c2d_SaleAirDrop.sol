...
contract SaleAirDrop is Ownable{
    ...
    */
    ...
    function signPartner() external returns(Partner memory) {
        require(_isPartner[msg.sender] == 0, "Already sign");
        _newPartner(msg.sender, 0, 0, 0, 0, false);
        Partner memory partner;
        partner.wallet = msg.sender;
        partner.rebate = 0;
        partner.rebateAirDrop = 0;
        partner.rebateAnoter = 0;
        partner.rebateBuyer = 0;
        partner.customRebate = false;
        return partner;
    }
    ...
    function claimAirDrop(uint256 id) public payable returns(address) {
    ...
    function setTestMode(bool trueOrFalse) external onlyOwner {
        _testMode = trueOrFalse;
    }
