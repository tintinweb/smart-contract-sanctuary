...
contract DroidBot is ERC721Burnable, Ownable
{
    ...
    /*----------------------------CONSTRUCTOR----------------------------*/
    constructor(address _dataStorage, string memory _baseURI) ERC721("DroidBot NFT Token", "DBOT")
    {
    ...
    function mintDroidBotWithLevel(address _receiver, uint256 _level) external onlyMinter {
        totalSupply++;
        uint256 _tokenId = totalSupply;
        _mint(_receiver, _tokenId);
        uint256 _power = IDataStorage(dataStorage).getDroidBotPower(_level);

        nftInfo[_tokenId] = NFTLib.info({
            level : _level,
            power : _power
        });
        emit DroidBotCreated(_receiver, _tokenId, _level, _power);
    }

    function create(address _receiver, uint256 _pandoBoxLevel) external onlyMinter{
        require(_receiver != address(0), 'DroidBotNFT: _receiver is the zero address');
        uint256[] memory _creatingProbability = IDataStorage(dataStorage).getDroidBotCreatingProbability(_pandoBoxLevel);
        ...
        for (uint256 i = 0; i < _creatingProbability.length; i++) {
            ...
            if (_cur >= _randSeed) {
               ...
               _mint(_receiver, _tokenId);
                ...
                emit DroidBotCreated(_receiver, _tokenId, i, _power);
                break;
            }
        }
    }

    function evolve(address _receiver, uint256 _droid0Level, uint256 _droid1Level) external onlyMinter {
        require(_receiver != address(0), 'DroidBotNFT: _receiver is the zero address');
        uint256[] memory _evolvingProbability = IDataStorage(dataStorage).getDroidBotEvolvingProbability(_droid0Level, _droid1Level);
        uint256 _randSeed = Random.computerSeed() % PRECISION + 1;
        uint256 _cur = 0;
        for (uint256 i = 0; i < _evolvingProbability.length; i++) {
            ...
            if (_cur >= _randSeed) {
                ...
                emit DroidBotEvolved(_receiver, i, _droid0Level, _droid1Level, _tokenId, _power);
                break;
            }
        }
    }
