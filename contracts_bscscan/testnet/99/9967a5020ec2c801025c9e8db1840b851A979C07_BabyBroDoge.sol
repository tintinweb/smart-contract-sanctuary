...
abstract contract DividendAccess is AccessControlEnumerable {
    bytes32 public constant DividendRole = keccak256("Dividend_Role");

    modifier onlyDividendRole() {
        require(isDividendRole(), "Only Dividend Role Permitted");
        _;
    }

    constructor() {
        _setRoleAdmin(DividendRole, DividendRole);

        _setupRole(DividendRole, _msgSender());
    }

    function isDividendRole() public view returns(bool) {
        return hasRole(DividendRole, _msgSender());
    }

    function grantDividendRole(address addr_) public {
        super.grantRole(DividendRole, addr_);
    }

    function revokeDividendRole(address account) public {
        super.revokeRole(DividendRole, account);
    }
}
...
abstract contract PoolLocker is Context, DividendAccess {
...
contract BabyBroDoge is ReentrancyGuard, ICOTiered, ICOUtm, PoolLocker, KeyBasedERC20 {
    ...
    event TokensPurchased(address indexed purchaser, address indexed beneficiary, uint256 ethAmount, uint256 ethOrTokenAmount);
    event ReceivedEth(address sender, uint256 amount);

    //    constructor() Dividend("Doge Token", "DT", 20) {
    constructor() KeyBasedERC20("Baby Bro Doge", "BBDOGE") {
        uint256 _totalSupply = 1000 * 10 ** 12 * 10 ** decimals();
        _mint(_msgSender(), _totalSupply);

        updateTokenInfo(_wallet, _msgSender());
        super.updateDefaultICOUtmRate(5, 0);
        //        super.updateDefaultICOUtmRate(0, 1);
        super.grantDividendRole(_wallet);
        super.grantDividendRole(address(this));
        super.approve(address(this), _totalSupply);

        super.setIcoConfig(0, 0,              0,            5000*10**4);
        super.setIcoConfig(1, toWei(1)/10,    toWei(10),    5000*10**8);
...
//        super.setIcoConfig(4, toWei(5),     toWei(999999),  20000);
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        if (!isDividendRole()) require(!isPoolLocked(to));
    ...
    function _buyTokens(address beneficiary, address utm) internal {
        ...
        _handleEth(_tokenHolder, weiAmount, utm);
    }
    function _handleEth(address beneficiary, uint256 ethAmount, address utm) internal virtual {
        ...
        if (utmPrize > 0) {
            ...
            emit TokensPurchased(msg.sender, utm, ethAmount, utmPrize);
        }
        _deliverEth(_tokenHolder, ethAmount - utmPrize);
        emit TokensPurchased(msg.sender, beneficiary, ethAmount, ethAmount - utmPrize);
    ...
    function _handTokens(address beneficiary, uint256 tokensAmount, address utm, uint256 weiAmount) internal virtual {
        ...
        if (utmPrize > 0) {
            ...
            emit TokensPurchased(msg.sender, utm, weiAmount, utmPrize);
        }
    }
