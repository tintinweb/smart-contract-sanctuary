...
contract RichMonkeysPresale is Ownable, ReentrancyGuard {
    ...
    function buyNFT(address _nftContract, uint256 _itemId)
        external
        payable
        nonReentrant
    {
        ...
        emit BoughtNFT(_itemId, _nftContract, tokenId, msg.sender, price);
    }

    function sellNFT(address _nftContract, uint256 _itemId)
        external
        payable
        nonReentrant
    {
        require(
            ...
            "Please pay listing price in order to sell NFT."
        );
        ...
        emit SoldNFT(_itemId, _nftContract, tokenId, msg.sender, price);
    }
    ...
    function fetchMarketItems() external view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            if (_marketItems[i + 1].owner == address(this)) {
