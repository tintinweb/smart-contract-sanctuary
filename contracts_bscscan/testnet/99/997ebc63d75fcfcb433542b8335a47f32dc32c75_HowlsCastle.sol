...
contract HowlsCastle is Ownable, ReentrancyGuard {
    ...
    function getNativePoolLPInUSD(uint256 _pid) public view returns(uint256) {
        require(getTokenUSDPair("BNB") != address(0x0), "getNativePoolLPInUSD: BNB-BUSD pair is mandatory");
        ...
        uint256 priceCalciferInBNB = getTokenPrice(CALCIFIRE.uniswapV2Pair(), 1);
        nativePriceUSD = (priceCalciferInBNB.mul(priceBNBinUSD)).div(10**CALCIFIRE.decimals());

        //native pool
        if (lpAddress == address(CALCIFIRE)) {
          poolAmountInUSD = user.amount.mul(nativePriceUSD).div(10**CALCIFIRE.decimals());
          return poolAmountInUSD;
        }

        //native farms
        totalTokens = CALCIFIRE.balanceOf(lpAddress);
        userLPinCalcifer = user.amount.div(lpTotalSupply);
    ...
    function deposit(uint256 _pid, uint256 _amount, bool _boost, address _referrer) public nonReentrant {
        ...
        if (_boost) {
            ...
            if (_boostLimitsEnabled) {
              require(getNativePoolLPInUSD(_pid) >= getPoolBoostUSDLimits(user.boost), 'deposit:BOOST USD LIMIT RESTRICTION');
            }
        }
    ...
    function payOrLockupPendingCALCIFIRE(uint256 _pid, bool _boost, uint256 _amount) internal {
        ...
        if (pending > 0 || user.rewardLockedUp > 0) {
            ...
            totalRewards = totalRewards.add(user.rewardLockedUp).add(user.boostLockedUp);
