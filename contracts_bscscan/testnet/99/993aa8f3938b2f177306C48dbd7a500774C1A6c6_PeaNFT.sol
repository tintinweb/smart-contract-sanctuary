...
contract PeaNFT is PeaManager, ERC721 {
    ...
    function destroy(uint256 tokenId) public onlyFusioner {
        _burn(tokenId);
    }
    ...
    function levelUp(
        ...
        Pean storage pean = peans[_tokenId];
        require(uint256(pean.level) < 5, "Already max level");
        pean.level = Level(uint256(pean.level).add(1));
        emit LevelUp(_tokenId);
    }
