    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public constant MAX_BORROW = 8000; // 80%
    uint256 public loanApplicationID;
    ...
    struct LoanApplication {
        ...
        // LoanCondition[] conditions; // user later
        uint256 interateRate;
        uint256 duration;
        bool openNewLoan;
    }
    ...
    event CreateLoanApp(address indexed owner, uint256 indexed loanID);
    ...
    modifier transferToBank(address token, uint256 value) {
        if (token == address(0)) {
            require(msg.value == value, "amount!");
        } else {
    ...
    function createLoanApp(
        ...
        uint256 _interateRate,
        uint256 _duration
    ) public payable transferToBank(_loanToken, _loanAmount) {
        require(
            !isDuplicatedLoanApplication(_loanToken),
            "createLoanApp: Duplicated"
        );

        loanApplicationID++;
        uint256 id = loanApplicationID;
        ...
        emit CreateLoanApp(msg.sender, id);
    }
    ...
    function borrow(
        ...
        public
        payable
        transferToBank(_collateralToken, _collateralAmount)
        returns (uint256)
    {
        LoanApplication memory app = loanApplications[_appID];
        require(app.loanToken != address(0), "no loan app");
        ...
        uint256 collateralPrice = oracle.price(_collateralAmount, path);
        uint256 max = collateralPrice.mul(MAX_BORROW).div(10000);
        console.log("borrow max: ", max);
        require(_borrowAmount <= max, "borrow: max");
    ...
    function interestAmount(uint256 bid) public view returns (uint256) {
        ...
        if (block.number <= b.lastPayBlock) {
            return 0;
        }

        uint256 timePast = block.number.sub(b.lastPayBlock);
        uint256 totalInterate = b.borrowAmount.mul(b.interateRate).div(10000);
        uint256 currentInterate = totalInterate.mul(timePast).div(b.duration);

        return currentInterate;
    }
    ...
    function repay(
        ...
        require(app.loanToken == loanToken, "invalid loan token");
        // console.log("repay block number2: ", block.number);
        uint256 interate = interestAmount(bid);

        uint256 principal = 0;
        if (amount > interate) {
            principal = amount.sub(interate);
        } else {
            principal = interate.sub(interate);
        }
...
contract Oracle is Ownable, ReentrancyGuard {
    ...
    function price(uint256 amountIn, address[] calldata path)
        public
        view
        returns (uint256)
    {
