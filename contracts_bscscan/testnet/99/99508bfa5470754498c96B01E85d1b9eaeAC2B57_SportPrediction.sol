...
contract SportPrediction is Ownable {
    ...
    event NewWinner(uint256 matchId, address owner, uint256 amount);

    event NewLoser(uint256 matchId, address owner);
    ...
    function participateMatch(
        ...
        emit NewParticipant(_matchId, msg.sender, _amount, _vote);
    ...
    function setWinner(uint256 _matchId, MatchResult _matchResult) private {
        if (_matchResult == MatchResult.AB) {
            for (uint256 i = 0; i < participants[_matchId].length; i++) {
                ...
                winner.amount = participants[_matchId][i].amount;
            ...
            for (uint256 i = 0; i < participants[_matchId].length; i++) {
                if (participants[_matchId][i].vote == _matchResult) {
                    ...
                    emit NewLoser(_matchId, participants[_matchId][i].owner);
                }
            }
        }
    }
