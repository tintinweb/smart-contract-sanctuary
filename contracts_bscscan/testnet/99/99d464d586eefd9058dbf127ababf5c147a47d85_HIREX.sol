...
contract HIREX is Context, IBEP20, Ownable {
    ...
    uint256 public buytaxFee          = 1; // Fee that gets re-distributed to holders.
    uint256 public buyliquidityFee    = 2;
    uint256 private buymarketingFee   = 2;
    uint256 private buyresearchFee    = 2;
    ...
    uint256 public othertaxFee        = 1;
    uint256 public otherliquidityFee  = 2;
    uint256 private othermarketingFee = 2;
    uint256 private otherresearchFee  = 2;
    ...
    uint256 public _buyMaxTxAmount     = 2000000 * 10**9;
    uint256 public _sellMaxTxAmount    = 1000000 * 10**9; // 0.3% of total supply
    uint256 public _otherMaxTxAmount   = _tTotal;
   
    uint256 private numTokensSellToAddToLiquidity = 800000 * 10**9; // this tx amount will trigger swap and add liquidity
    ...
    function setMaxTxPercent(uint256 buyTxPercent, uint256 sellTxPercent, uint256 otherTxPercent) external onlyOwner() {
    ...
    function setOtherFeePercent(uint256 OtherTaxfee, uint256 OtherLiquidityFee, uint256 OtherMarketingFee, uint256 OtherResearchFee) external onlyOwner() {
    othertaxFee = OtherTaxfee;
    otherliquidityFee = OtherLiquidityFee;
    othermarketingFee = OtherMarketingFee;
    otherresearchFee = OtherResearchFee;
    }

    function setMaxWalletPercent(uint256 maxWalletAmount) external onlyOwner() {
        require(maxWalletAmount > 0.000, "% must be greater than 0.000");
        _maxWalletAmount = _tTotal.mul(maxWalletAmount).div(10**2);
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !_isExcludeAddressFromMaxTx[from] && !_isExcludeAddressFromMaxTx[to]) {
            ...
            require(balanceOf(to).add(amount) <= _maxWalletAmount, "Max Wallet Reach, wait till check off");        
		}
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if (from == pancakeSwapV2Pair) { // Buy
                ...
                taxFee = othertaxFee;
                liquidityFee = otherliquidityFee;
                marketingFee = othermarketingFee;
                researchFee = otherresearchFee;
            }
