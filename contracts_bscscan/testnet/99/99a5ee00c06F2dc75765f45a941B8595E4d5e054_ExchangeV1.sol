...
contract ExchangeV1 is Ownable {
    ...
    address public arcadeAddress = address(0x0);
    address public busdAddress = address(0x0);
    address public factoryAddress = address(0x0);
    address public bnbAddress = address(0x0);

    constructor() public {
    }
    
    /**
     * @dev Exchange ERC721 token and Arcade token(ERC20)
     * Transfer ERC721 token from `owner` to `buyer`.
     * Transfer Arcade token from `buyer` to `owner`.
     */
    function exchange(
        ...
        IERC20(arcadeAddress).transferFrom(buyer, owner, buyValue);
     ...
     /**
     * @dev Exchange ERC721 token and BUSD token(ERC20)
     * Transfer ERC721 token from `owner` to `buyer`.
     * Transfer BUSD token from `buyer` to `owner`.
     */
     ...
     * List ERC721 token with arcadePrice
     */
    function SellRequest(
        ...
        sellRequests[_msgSender()][token][tokenId] = arcadePrice;
    }
     ...
     * check if the arcadePrice is correct.
     */
    function validateBuyRequest(
        ...
        require(
            sellRequests[owner][token][tokenId] == arcadePrice, 
            "Amount is incorrect."
        );
    }
    ...
    /**
     * @dev Check the validation of buy request.
     * check if the BUSD is correct.
     */
    function validateBuyBUSDRequest(
        address owner, 
        address token, 
        uint256 tokenId, 
        uint256 busdPrice
        ...
        require(
            sellRequests[owner][token][tokenId].mul(getRate()) == busdPrice.mul(10 ** 18), 
            "Amount is incorrect."
        );
    }
    
    /**
     * @dev Set Arcade Address
     */
    function setArcadeAddress(address _address) public onlyOwner {
        arcadeAddress = _address;
    }
     ...
     */
    function setBNBAddress(address _address) public onlyOwner {
        bnbAddress = _address;
    }
    ...
    function getLiquidityInfo(
        ...
        address pairAddress = IUniswapV2Factory(factoryAddress).getPair(token1, token2);
    ...
    function getBNBPrice() public view returns (uint256) {
        (uint256 bnbReserve, uint256 busdReserve) = getLiquidityInfo(bnbAddress, busdAddress);
        return busdReserve.mul(10 ** 18).div(bnbReserve);
    }
    
    /**
     * @dev Get Arcade price in USD
     */
    function getRate() public view returns (uint256) {
        (uint256 arcadeReserve, uint256 bnbReserve) = getLiquidityInfo(arcadeAddress, bnbAddress);
