    ...
    ReentrancyGuardUpgradeable
{
    ...
    event BridgeOutFor(address indexed to, uint256 amount);
    ...
    uint256 public accBridgeOut;
    mapping(address => uint256) private _accBridgeOutByUser;
    ...
    function initialize(IBEP20 _zmn) public initializer {
        __Ownable_init();
        zmn = _zmn;
    }
    ...
    function getAccBridgeOutByUser(address _user)
        external
        view
        returns (uint256)
    {
        return _accBridgeOutByUser[_user];
    }
    ...
    function bridgeOutFor(address _to, uint256 _accIn)
        public
        nonReentrant
        onlyOwner
    {
        uint256 _accOut = _accBridgeOutByUser[_to];
        require(_accIn > _accOut, "No credit amount");

        // add debit
        uint256 _amount = _accIn.sub(_accOut);
        _accBridgeOutByUser[_to] = _accIn;
        accBridgeOut = accBridgeOut.add(_amount);

        // transfer token
        zmn.safeTransfer(address(_to), _amount);
        emit BridgeOutFor(_to, _amount);
    }

    // Withdraw ZMN to safe wallet. EMERGENCY ONLY.
