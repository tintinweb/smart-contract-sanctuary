...
// File: contracts/DoubleOwnerDividendPayingToken.sol
...
// File: contracts/DoubleOwner.sol

// 

pragma solidity ^0.7.6;

contract DoubleOwner is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DoubleOwnerDividendTracker public dividendTracker;
    address public liquidityWallet;
    address public marketingWallet;
    address public charityWallet;
    uint256 public maxSellTransactionAmount = 50000000 * (10**18);
    uint256 public swapTokensAtAmount = 2000000 * (10**18);
    uint256 public _maxWalletToken = 1000000000 * (10**18);// Anti-whale 1% max wallet amount
    uint256 public BTCRewardsFee;
    uint256 public liquidityFee;
    uint256 public marketingFee;
    uint256 public marketingBTCFee;
    uint256 public charityBTCFee;
    ...
    constructor() ERC20("DoubleOwner", "DBON") {
        uint256 _BTCRewardsFee = 4;
        uint256 _liquidityFee = 3;
        uint256 _marketingFee = 1;
        uint256 _marketingBTCFee = 3;
        uint256 _charityBTCFee = 3;

        BTCRewardsFee = _BTCRewardsFee;
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        marketingBTCFee = _marketingBTCFee;
        _charityBTCFee = _charityBTCFee;
 
        totalFees = _BTCRewardsFee.add(_liquidityFee).add(_marketingFee).add(_marketingBTCFee).add(_charityBTCFee);

        dividendTracker = new DoubleOwnerDividendTracker();

        liquidityWallet = owner();
        marketingWallet = address(0x486F1bCC300ab3724ea65C32Bf0ed085aC4Fd7aE);
        charityWallet   = address(0x868367d6107426bbb2795D6C2C376d19bc3D182b);
        ...
        */
        _create(owner(), 100000000000 * (10**18));
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DoubleOwner: The dividend tracker already has that address");

        DoubleOwnerDividendTracker newDividendTracker = DoubleOwnerDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DoubleOwner: The new dividend tracker must be owned by the DoubleOwner token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DoubleOwner: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DoubleOwner: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DoubleOwner: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DoubleOwner: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "DoubleOwner: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DoubleOwner: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DoubleOwner: Cannot update gasForProcessing to same value");
    ...
    function setMarketingFee(uint256 _tokenFee, uint256 _btcFee) external onlyOwner {
        ...
        charityBTCFee  = _btcFee;

        totalFees = BTCRewardsFee.add(liquidityFee).add(marketingFee).add(marketingBTCFee).add(charityBTCFee);
    }
...
function setCharityWallet(address _address) public onlyOwner {
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "DoubleOwner: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 btcBalance = IERC20(BTCB).balanceOf(address(this));
        ...
        success = IERC20(BTCB).transfer(marketingWallet, marketings);
        if (success) {
...
contract DoubleOwnerDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DoubleOwner_Dividend_Tracker", "DoubleOwner_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DoubleOwner_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "DoubleOwner_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DoubleOwner contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DoubleOwner_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DoubleOwner_Dividend_Tracker: Cannot update claimWait to same value");
