...
contract MockBond {
    ...
    function setReservoir(address _reservoir) public {
        reservoir = _reservoir;
    }

    function setLiquidityBond(bool isLiquidityBond_) public {
        isLiquidityBond = isLiquidityBond_;
    }

    function gyroValue(uint256 amount_) public view returns (uint256 value_, address token_) {
        value_ = amount_.mul(10**9).div(10**IERC20(tokenIn).decimals());
        token_ = tokenIn;
    }

    function deposit(uint256 amount_, uint256 profit_) public {
        IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), amount_);
        IERC20(tokenIn).approve(address(reservoir), amount_);
        IReservoir(reservoir).bondDeposit(amount_, profit_);
    }
}
