...
contract MockOMNIA is MockOMNIABase, AutoClaimDetails {
    ...
    function setOSC(address osc_) external onlyOwner {
    ...
    function _transfer(
        ...
        bool isSelling = isPancakeSwapPair(recipient) && isListed();
        if (isSelling) {
...
abstract contract MockOMNIABase is Transfers {
...
abstract contract AutoClaimDetails is AutoClaim {
    ...
    */
        ...
        returns (uint256)
    {
        ...
        excludedFromCirculation += balanceOf(DX_SALE_LOCK);
...
abstract contract Transfers is SwapExecutorDetails {
    ...
    function _transfer(
        ...
        if (isPancakeSwapPair(sender) && block.timestamp < listingDate) {
            revert("Buying not allowed before listing date");
        }
        if (isPancakeSwapPair(recipient) && block.timestamp < listingDate) {
            revert("Selling are not allowed before listing date");
        }
...
abstract contract SwapsManager is ERC20PresetFixedSupply, RouterManager {
    ...
    bool private _isUnlisted = true;
    bool private _isListed = false;
    ...
    function isUnlisted() public view returns (bool) {
        return _isUnlisted;
    }
...
abstract contract RewardManager is Ownable {
...
abstract contract RewardCycle is Ownable {
    uint256 private _rewardCyclePeriod = 1; // Can claim rewards every 6h
