...
contract EliteWorldFarm is Initializable, OwnableUpgradeable {
    ...
    struct PoolInfo {
        address lpToken;
        uint256 rewardRate; // reward in days
        uint256 rewardInterval; // days
    }
    ...
    struct UserInfo {
        ...
        // pending reward = user.amount * pool.rewardRate * (current lastTime - last timestamp) /pool.rewardInterval
    }
    ...
    mapping(address => uint256) private realizedReward;
        ...
        returns (uint256)
    {
        ...
        return userInfos[pid][_user].amount;
    }
    ...
    function setLpToken(
        ...
        uint256 pid = getPid(_lpToken);
        poolInfos[pid].rewardRate = _rewardRate;
        poolInfos[pid].rewardInterval = _rewardInterval;
    }
        ...
        returns (uint256)
    {
        ...
        UserInfo storage user = userInfos[pid][_user];
        uint256 lpSupply = IERC20Upgradeable(pool.lpToken).balanceOf(
            address(this)
        );
        uint256 reward = 0;
        if (block.timestamp > user.lastTime && lpSupply != 0) {
            reward = block
                .timestamp
                .sub(user.lastTime)
                .mul(pool.rewardRate)
        ...
        return DecimalMath.mulFloor(user.amount, reward);
    }
        ...
        returns (uint256)
    {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            if (userInfos[pid][_user].amount == 0) {
                continue; // save gas
            }
            PoolInfo storage pool = poolInfos[pid];
            UserInfo storage user = userInfos[pid][_user];
            ...
            totalReward = totalReward.add(
                DecimalMath.mulFloor(user.amount, reward)
            );
        }
        return totalReward;
    }
    ...
    function withdraw(address _lpToken, uint256 _amount) public {
        ...
        user.lastTime = block.timestamp;
        IERC20Upgradeable(pool.lpToken).safeTransfer(
            address(msg.sender),
            _amount
        );
    ...
    function claim(address _lpToken) public {
        uint256 pid = getPid(_lpToken);
        if (userInfos[pid][msg.sender].amount == 0) {
            return; // save gas
        }
        PoolInfo storage pool = poolInfos[pid];
        UserInfo storage user = userInfos[pid][msg.sender];
    ...
    function _safeReward(address _to, uint256 _pending) internal {
        _balances[_to] = _balances[_to].add(_pending);
        realizedReward[_to] = realizedReward[_to].add(_pending);
    }
}
