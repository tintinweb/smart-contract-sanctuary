...
contract Auction is Ownable {
    ...
    event AddressList(address indexed _account, address _macTokenContract, address _magicNftContract, address _platinumNftContract, address _officialAddress, address _platinumShareAddress);
    ...
    function joinAuction(uint256 _nftType,uint256 _tokenId,uint256 _auctionAmount,uint256 _endTime) public returns (bool) {
          // Data validation
          require(auctionSwitchState,"-> auctionSwitchState: auction has not started yet.");
          require(_endTime>=block.timestamp.add(dayTime),"-> _endTime: Cannot be less than the minimum auction time.");
          require(_endTime<=block.timestamp.add(dayTime.mul(30)),"-> _endTime: Cannot be greater than the maximum auction time.");
          ...
          auctionOrders[auctionJoinTotalCount] = AuctionOrder(auctionJoinTotalCount,msg.sender,0,_nftType,_tokenId,block.timestamp,_endTime,_auctionAmount,nftValue.mul(2),0,address(0),_auctionAmount,0);// add auctionOrders
    ...
    function getAuctionBasic() public view returns (uint256 DayTime,ERC20 MacTokenContract,ERC721 MagicNftContract,ERC721 PlatinumNftContract,address OfficialAddress,address PlatinumShareAddress,
        bool AuctionSwitchState,uint256 AuctionStartTime,uint256 AuctionJoinTotalCount,uint256 NowAuctionFee) {
        return (dayTime,macTokenContract,magicNftContract,platinumNftContract,officialAddress,platinumShareAddress,auctionSwitchState,auctionStartTime,auctionJoinTotalCount,auctionFee);
    }
    ...
    function auctionOrdersOf(uint256 _orderIndex) public view returns (uint256 Index,address Account,uint256 Status,uint256 TokenId,uint256 NftType,
        uint256 EndTime,uint256 AuctionAmount,uint256 MaxAmount,uint256 PayFeeAmount,address LastUser,uint256 LastAmount,uint256 LastTime){
        AuctionOrder storage order = auctionOrders[_orderIndex];
        return (order.index,order.account,order.status,order.tokenId,order.nftType,order.endTime,order.auctionAmount,order.maxAmount,order.payFeeAmount,
    ...
    function setAddressList(address _macTokenContract,address _magicNftContract,address _platinumNftContract,address _officialAddress,address _platinumShareAddress) public onlyOwner returns (bool) {
        ...
        emit AddressList(msg.sender, _macTokenContract, _magicNftContract, _platinumNftContract, _officialAddress, _platinumShareAddress);
        return true;
    }
