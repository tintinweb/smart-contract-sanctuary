...
contract A_Solc8_V23 is BEP20('ArrgghhV23', 'ADMRSL6V23'){
    using Address for address;
    using SafeMath for uint256;
    ...
    event ExcludedFromAntiWhale(address indexed owner, bool value);
    ...
    modifier MintWrappersOn(address recipient) {
            if ( _mint_wrappers_stopped == true) {
                if  ((_minter_role_address[recipient] == false) || (recipient.isContract() && _contractwhitelist[recipient] == false))
                   {
    ...
    function H3_contractwhitelistUpdate(address _account, bool _excluded) public onlyOwner whenApproveSpendableAddressOn {
        _contractwhitelist[_account] = _excluded;
        emit ContractWhitelistUpdated(_account, _excluded);
    }
    
    function H4_isContractWhiteListed(address _account) public view onlyOwner returns (bool) {
        return _contractwhitelist[_account];
    }
    ...
    function isExcludedFromAntiWhale(address _account) public virtual onlyOwner returns (bool) {
        return _excludedFromAntiWhale[_account];
    }
    
    function H5_minterRoleUpdate(address _account, bool _excluded) public onlyOwner whenApproveSpendableAddressOn {
        _minter_role_address[_account] = _excluded;
        emit MinterRoleUpdated(_account, _excluded);
    }

    function H6_isMinterRole(address user) public virtual onlyOwner returns (bool) {
