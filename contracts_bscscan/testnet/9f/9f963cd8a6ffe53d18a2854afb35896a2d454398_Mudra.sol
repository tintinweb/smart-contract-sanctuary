...
library SafeMath {
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        // See: https://github.com/OpenZeppelin/ro/me/o/do/ge/pull/522
        if (a == 0) {
            return 0;
        }
...
contract Mudra is IERC20 {
    ...
    uint256 private constant startingSupply = 500000000;
    
    uint8 private _decimals = 18;
    uint256 private _decimalsMul = _decimals;
    uint256 private _tTotal = startingSupply * (10 ** _decimalsMul);

    string constant _name = "Mudra";
    string constant _symbol = "MDR";

    uint256 private _loveFee = 0; // Adjusted by buys and sells.
    uint256 private _flirtFee = 0; // Adjusted by buys and sells.
    uint256 private _kissFee = 0; // Adjusted by buys and sells.
    uint256 private _totalFee = _flirtFee + _loveFee + _kissFee;
    uint256 public masterTaxDivisor = 10000;

    uint256 public _buyLoveFee = 1000;
    uint256 public _buyFlirtFee = 500;
    uint256 public _buyKissFee = 300;

    uint256 public _sellLoveFee = 1100;
    uint256 public _sellFlirtFee = 600;
    uint256 public _sellKissFee = 400;

    uint256 constant public maxLoveFee = 2000;
    uint256 constant public maxFlirtFee = 1500;
    uint256 constant public maxKissFee = 1500;
    ...
    address payable private _marketingWallet = payable(0x38C7372894654EEA7945d0D9c0bfBe2579B2a864);
    ...
    constructor () payable {
        ...
        reflector = Cashier(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
...
//===============================================================================================================
    ...
    function setBuyTaxes(uint256 flirtFee, uint256 loveFee, uint256 kissFee) external onlyOwner {
        require(flirtFee <= maxFlirtFee && loveFee <= maxLoveFee && kissFee <= maxKissFee);
        require(flirtFee + loveFee + kissFee <= 5000);
        _buyFlirtFee = flirtFee;
        _buyLoveFee = loveFee;
        _buyKissFee = kissFee;
    }

    function setSellTaxes(uint256 flirtFee, uint256 loveFee, uint256 kissFee) external onlyOwner {
        require(flirtFee <= maxFlirtFee && loveFee <= maxLoveFee  && kissFee <= maxKissFee);
        require(flirtFee + loveFee + kissFee <= 5000);
        _sellFlirtFee = flirtFee;
        _sellLoveFee = loveFee;
        _sellKissFee = kissFee;
    }
    ...
    function adjustTaxes(address from, address to) internal {
        if (lpPairs[to]) {
            _loveFee = _sellLoveFee;
            _flirtFee = _sellFlirtFee;
            _kissFee = _sellKissFee;
        } else if (lpPairs[from]) {
            _loveFee = _buyLoveFee;
            _flirtFee = _buyFlirtFee;
            _kissFee = _buyKissFee;
        } else {
            _loveFee = 0;
            _flirtFee = 0;
            _kissFee = 0;
    ...
    function getTotalFee() public view returns (uint256) {
        return _loveFee + _flirtFee + _kissFee;
    }
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        uint256 dynamicLiquidityFee = isOverLiquified(targetLiquidity, targetLiquidityDenominator) ? 0 : _flirtFee;
        ...
        uint256 amountBNBReflection = amountBNB * _loveFee / totalBNBFee;
