...
contract Kodi is ERC20, Ownable {
    ...
    constructor() public ERC20("KODITEST", "KODITEST") {
    ...
    function blacklistAccount(address account) public onlyOwner {
        ...
        require((!isTradingEnabled && !_isLanched) || (_isLanched && currentTimestamp.sub(_launchStartTimestamp) < _blacklistTimeLimit) , "KODI: Time to blacklist accounts has expired");
    ...
    function isInRoar() external view returns (bool) {
        ...
        if(currentTimestamp.sub(_roarStartTimestamp) < _roar1.timeInPeriod.add(_roar2.timeInPeriod).add(_roar3.timeInPeriod)) {
            return true;
        } else {
            return false;
        }
    ...
    function isInLaunch() external view returns (bool) {
        ...
        if(_isLanched && (timeSinceLaunch < _launch1.timeInPeriod.add(_launch2.timeInPeriod).add(_launch3.timeInPeriod) || blocksSinceLaunch < _launch1.blocksInPeriod )) {
            return true;
        } else {
            return false;
        }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()) {
            ...
            if (_isInLaunch) {
                if (currentTimestamp.sub(_launchStartTimestamp) <= 300) {
                    require(isBuyFromLp && currentTimestamp.sub(_buyTimesInLaunch[to]) > 60, "KODI: Cannot buy more than once per min in first 5min of launch");
                }
            }
        ...
        if ( to != owner() && _isInLaunch && isBuyFromLp && currentTimestamp.sub(_launchStartTimestamp) <= 300 && currentTimestamp.sub(_buyTimesInLaunch[to]) > 60) {
    ...
    function _isInLaunch1() internal view returns(bool) {
        if(block.number.sub(_launchBlockNumber) < _launch1.blocksInPeriod) {
            return true;
        } else {
            return false;
        }
    ...
    function _isInLaunch2() internal view returns(bool) {
        uint256 currentTimestamp = !isTradingEnabled && _tradingPausedTimestamp > _launchStartTimestamp  ? _tradingPausedTimestamp : _getNow();
        if (currentTimestamp.sub(_launchStartTimestamp) < _launch1.timeInPeriod && block.number.sub(_launchBlockNumber) > _launch1.blocksInPeriod ) {
            return true;
        } else {
            return false;
        }
    ...
    function _isInLaunch3() internal view returns(bool) {
        uint256 currentTimestamp = !isTradingEnabled && _tradingPausedTimestamp > _launchStartTimestamp  ? _tradingPausedTimestamp : _getNow();
        uint256 timeSinceLaunch = currentTimestamp.sub(_launchStartTimestamp);
        if (timeSinceLaunch > _launch2.timeInPeriod && timeSinceLaunch < _launch3.timeInPeriod.add(_launch2.timeInPeriod) && block.number.sub(_launchBlockNumber) > _launch1.blocksInPeriod) {
            return true;
        } else {
            return false;
        }
    ...
    function _isInRoar1() internal view returns (bool) {
        uint256 currentTimestamp = !isTradingEnabled && _tradingPausedTimestamp > _roarStartTimestamp  ? _tradingPausedTimestamp : _getNow();
        if(currentTimestamp.sub(_roarStartTimestamp) < _roar1.timeInPeriod) {
            return true;
        } else {
            return false;
        }
    ...
    function _isInRoar2() internal view returns (bool) {
        uint256 currentTimestamp = !isTradingEnabled && _tradingPausedTimestamp > _roarStartTimestamp  ? _tradingPausedTimestamp : _getNow();
        if(currentTimestamp.sub(_roarStartTimestamp) > _roar1.timeInPeriod && currentTimestamp.sub(_roarStartTimestamp) < _roar1.timeInPeriod.add(_roar2.timeInPeriod)) {
            return true;
        } else {
            return false;
        }
    ...
    function _isInRoar3() internal view returns (bool) {
        uint256 currentTimestamp = !isTradingEnabled && _tradingPausedTimestamp > _roarStartTimestamp  ? _tradingPausedTimestamp : _getNow();
        if(currentTimestamp.sub(_roarStartTimestamp) > _roar1.timeInPeriod.add(_roar2.timeInPeriod) && currentTimestamp.sub(_roarStartTimestamp) < _roar1.timeInPeriod.add(_roar2.timeInPeriod).add(_roar3.timeInPeriod)) {
            return true;
        } else {
            return false;
        }
