...
contract preSaledeployerOPT {
    ...
    constructor() {
        admin = payable(0x7863d1C6f74Ebb0d4C7c0bdBE40EEB963dE005A0); //0x171C10C05bbf0B87907561Ee50D6a2Bb6aB17502
        projectCoinFee = 3;
        projectTokenFee = 3;
        userFee = 10;
        ...
        returns (address preSaleContract)
    {
        ...
        _preSaleData[16] = _preSaleData[16]*(
        ...
        _preSaleData[17] = _preSaleData[17]*(
            10**(IERC20(_token).decimals())
        );
        ...
        _preSaleData[18] = _preSaleData[18]*(
            10**(IERC20(_token).decimals())
        );
    ...
    function getNumberOfTokensForSale(uint256 _tokenPrice, uint256 _hardCap)
        ...
        returns (uint256)
    {
        uint256 tokensForSale = _hardCap*(_tokenPrice)*(1e8)/(1e18);
        tokensForSale = tokensForSale+(
            tokensForSale*(projectCoinFee)/(100)
        );
        return tokensForSale/(1e8);
    }
    ...
    function getNumberOfTokensForListing(
        ...
        uint256 tokensForListing = (_hardCap*(_liquidityPercent)/(100))
            *(_listingPrice)
            *(1e8)
            /(1e18);
        return tokensForListing/(1e8);
    }
