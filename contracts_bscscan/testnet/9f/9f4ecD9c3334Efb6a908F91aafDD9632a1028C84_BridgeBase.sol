...
contract BridgeBase is Context, Ownable {
    ...
    event WalletTransfer(address from, uint256 amount, address indexed to);
    ...
    function getUserWalletLimit(address account)
        external
        view
        returns (Limit memory)
    {
        ...
        if (block.timestamp.sub(limit.lastTransfered) > 1 days) {
            ...
            limit.lastTransfered =
                1 days -
                block.timestamp.sub(limit.lastTransfered);
        }
    ...
    function walletTransfer(address to, uint256 amount) external {
        ...
        uint256 tokenLimit = getTokenAmount(20000000000000000000);
        require(
            limit.amount + amount < tokenLimit,
            "Exceeds token Limit for today!"
        );
