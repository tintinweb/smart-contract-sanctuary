...
contract Staking is Ownable {
    ...
    function stake(uint8 _pid, uint256 _amount) external returns(bool) {
        require(poolInfo[_pid].depositToken.allowance(msg.sender,address(this)) >= _amount,
        "Token : Set allowance first!");

        bool success = poolInfo[_pid].depositToken.transferFrom(msg.sender,address(this),_amount);
    ...
    */
    ...
    function unStake(uint8 _pid, uint256 _amount) external returns(bool) {
        ...
        uint256 feeAmount = _amount.mul(feePercent).div(1000);

        safeTransfer(pool.depositToken,feeAddress,feeAmount);
    ...
    function _claim(uint8 _pid, address _addr) internal { 
        ...
        if(amount > 0){
            ...
            safeTransfer(poolInfo[_pid].rewardToken,_addr,amount);
    ...
    /**
    *
    * @dev View function which calculated claimable amount of a user
    *
    * @param _pid id of the pool
    * @param _addr address of the user 
    *
    * @return value Claimable amount of user
    */
