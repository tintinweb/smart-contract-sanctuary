...
contract ERC20SimpleSwap {
  ...
  */
  function _cashChequeInternal(
    ...
    require(issuer == recoverEIP712(chequeHash(address(this), beneficiary, cumulativePayout), issuerSig), "invalid issuer signature");
    ...
    require (totalPayout > 0, "totalPayout is zero");

    require(token.transfer(recipient, totalPayout), "transfer failed");

    emit ChequeCashed(beneficiary, recipient, msg.sender, totalPayout, cumulativePayout);
  }
  ...
  */
  function cashCheque(address recipient, uint cumulativePayout, bytes memory issuerSig) public {
...
contract SimpleSwapFactory {
  ...
  */
  ...
  public returns (address) {
    require(deployedContracts[issuer] == address(0), "deployed");

    address contractAddress = Clones.cloneDeterministic(master, keccak256(abi.encode(msg.sender, salt)));
    ERC20SimpleSwap(contractAddress).init(issuer, ERC20Address);
    deployedContracts[issuer] = contractAddress;
