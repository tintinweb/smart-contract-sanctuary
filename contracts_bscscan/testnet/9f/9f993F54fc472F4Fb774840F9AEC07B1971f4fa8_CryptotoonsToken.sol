...
contract CryptotoonsToken is ERC721, Ownable {
    ...
    uint256 fee = 0.043 ether;
    uint256 fee_level = 0.011 ether;
    // Estructura de datos con las propiedades del ctt (personaje)
    struct Ctt {
        string name;
        uint256 id;
        uint256 dna;
    ...
    event NewCtt(address indexed owner, uint256 id, uint256 dna);
    ...
    // Creación del Token NFT (ctt/personaje)
    function _createCtt(string memory _name) internal {
        uint8 randRarity = uint8(_createRandomNum(100));
        uint randDna = _createRandomNum(10**16);
        Ctt memory newCtt = Ctt(_name, COUNTER, randDna, 1, randRarity);
        ctts.push(newCtt);
        _safeMint(msg.sender, COUNTER);
        emit NewCtt(msg.sender, COUNTER, randDna);
        COUNTER++;
    }

    // ============================================
    // Creación de un personaje aleatorio
    function createRandomCtt(string memory _name) public payable {
        require(msg.value >= fee);
        _createCtt(_name);
    }

    // Obtención de todos los ctts (personajes)
    function getCtts() public view returns (Ctt [] memory) {
        return ctts;
    }
    ...
    function getOwnerCtts(address _owner) public view returns (Ctt [] memory) {
        Ctt [] memory result = new Ctt [] (balanceOf(_owner));
        uint256 counter = 0;
        for (uint256 i = 0; i <ctts.length; i++) {
            if (ownerOf(i) == _owner) {
                result[counter] = ctts[i];
                counter++;
            }
        }
        return result;
    ...
    function levelUp(uint256 _cttId) public payable {
        require(ownerOf(_cttId) == msg.sender);
		require(msg.value >= fee_level);
        Ctt storage ctt = ctts[_cttId];
        ctt.level++;
    }
