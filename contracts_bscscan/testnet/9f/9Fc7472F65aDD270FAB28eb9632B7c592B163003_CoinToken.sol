...
contract CoinToken is Context, IBEP20, Ownable {
    ...
    function csproMiningStart(uint256 mineamount) public returns (uint256) {
      uint256 realMineAmount = mineamount * _DECIMALFACTOR;     
      require(realMineAmount >= 5000 * _DECIMALFACTOR); 
    ...
    function miningGetBack() public returns(uint256) {
    ...
    function havetoSign(uint256 _bnumber) public returns (uint256)  { 
       require(stakerStatus(msg.sender) == true);
    ...
    function getCspro(uint256 _bnumber) public returns(uint256) { 
       require(stakerStatus(msg.sender) == true);
       require((block.number-1) - _bnumber  > 200);        
       require(uint256(blockhash(_bnumber)) % nRewarMod == 1);
       require(nStockDetails[msg.sender]._stocktime + nWtime > block.timestamp  ); 
       ...
       uint256 halving = lastBlock / 180;   
       uint256 totalRA = 120000 * _DECIMALFACTOR;
       if(halving==0)
       {
           totalRA = 120000 * _DECIMALFACTOR;
       }
       else if(halving==1)
       {
           totalRA = 170000 * _DECIMALFACTOR;
       }
       else if(halving==2)
       {
           totalRA = 150000 * _DECIMALFACTOR;
       }
       else if(halving==3)
       {
           totalRA = 120000 * _DECIMALFACTOR;
       ...
       {
           totalRA = 75000 * _DECIMALFACTOR;
       }
       else if(halving==6)
       {
           totalRA = 50000 * _DECIMALFACTOR;
       }
       else if(halving==7)
       {
           totalRA = 25000 * _DECIMALFACTOR;
       }
       else if(halving==8)
       {
           totalRA = 15000 * _DECIMALFACTOR;
       }
       else if(halving==9)
       {
           totalRA = 10000 * _DECIMALFACTOR;
       }
       else
       {
            totalRA = 0;
