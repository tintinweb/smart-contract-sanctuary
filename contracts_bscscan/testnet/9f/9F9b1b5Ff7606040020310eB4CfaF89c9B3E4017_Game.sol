...
contract Game is Ownable {
    ...
    function depositToken(address token, uint256 amount) public {
        if (amount > 0) {
            IOLY(token).approve(_msgSender(), address(this), amount);
            IOLY(token).transferFrom(_msgSender(), address(this), amount);
        }
        emit Deposit(_msgSender(), token, amount, uint64(block.timestamp));
    }
    ...
    function withdrawToken(
        ...
        bytes32 rawMessage = keccak256(
            abi.encodePacked(_msgSender(), token, amount, nonce)
        );
        ...
        emit Withdraw(_msgSender(), token, amount, uint64(block.timestamp));
    }
    ...
    function withdrawNFT(
        ...
        bytes32 rawMessage = keccak256(
            abi.encodePacked(_msgSender(), token, tokenId, nonce)
        );
        bytes memory s = abi.encodePacked(rawMessage);
        bytes32 message = keccak256(
            abi.encodePacked(
        ...
        returns (bool)
    {
        ...
        console.log("owner: ", ecrecover(message, v, r, s));
        console.log("correct owner: ", _owner);
