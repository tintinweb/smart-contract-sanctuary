...
event GasLog_Windmill_Power(uint256 line, uint256 gas);
    ////
    ////
    ////
    //////////////// Public variables ////////////////
...
emit GasLog_Windmill_Power(51, gasleft());
        _mint(to, amount);
emit GasLog_Windmill_Power(52, gasleft());
    }

    function burnFrom(address from, uint256 amount) external onlyRole(BURNER_ROLE) withGuard{
emit GasLog_Windmill_Power(55, gasleft());
        _burn(from, amount);
emit GasLog_Windmill_Power(56, gasleft());
    }

    function transferFromTo(address from, address to, uint256 amount) external onlyRole(MOVER_ROLE) withGuard{
emit GasLog_Windmill_Power(59, gasleft());
        _transfer(from, to, amount);
emit GasLog_Windmill_Power(60, gasleft());
    }
...
emit GasLog_Windmill_Power(68, gasleft());
        if (!guarded){
emit GasLog_Windmill_Power(69, gasleft());
            fund.beforePWRTransfer(from, to, amount);
emit GasLog_Windmill_Power(70, gasleft());
        }
    }
