...
abstract contract ERC20 is ERC20Data, Domain {
    ...
    function transfer(address to, uint256 amount) public returns (bool) {
        ...
        if (amount != 0) {
            ...
            require(srcBalance >= amount, "balance too low");
            if (msg.sender != to) {
                require(to != address(0), "zero address"); // Moved down so low balance calls safe some gas
...
abstract contract BoringOwnable is BoringOwnableData {
    ...
    function claimOwnership() public {
        ...
        // Checks
        require(msg.sender == _pendingOwner, "no auth");
...
contract KashiPair is ERC20, BoringOwnable, IMasterContract {
    ...
    uint256 private constant CLOSED_COLLATERIZATION_RATE = 67000; // 67%
    ...
    modifier solvent() {
        _;
        require(_isSolvent(msg.sender, false, exchangeRate), "user insolvent");
    }
    ...
    function _addTokens(
        ...
        if (skim) {
            require(share <= bentoBox.balanceOf(token, address(this)).sub(total), "Skim too much");
        } else {
    ...
    function _removeAsset(address to, uint256 fraction) internal returns (uint256 share) {
        ...
        require(_totalAsset.base >= 1000, "below minimum");
    ...
    function _call(
        ...
        require(callee != address(bentoBox) && callee != address(this), "can't call");
    ...
    function cook(
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (action == ACTION_ADD_COLLATERAL) {
                ...
                require((!must_update || updated) && rate > minRate && (maxRate == 0 || rate > maxRate), "rate not ok");
    ...
    function liquidate(
        ...
        require(allBorrowAmount != 0, "all are solvent");
        ...
        if (!open) {
            ...
            require(masterContract.swappers(swapper), "Invalid swapper");
