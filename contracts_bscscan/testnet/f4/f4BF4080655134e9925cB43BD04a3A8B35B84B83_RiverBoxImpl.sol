...
import "../interfaces/IRiverBoxExchange.sol";
    ...
    UUPSUpgradeable
{
    ...
    address public internalExchangeAddress;
    ...
    function stockLocation(uint256 stockIdx) public view returns (uint256) {
        return uint256(_stockLocations[stockIdx]);
    }

    function batchStockInfo(uint256 startIndex, uint256 length) public view returns (uint256[] memory) {
        require(startIndex < _stockLocations.length , "startIndex over stockLocationLength");
        require(length > 0 , "length cannot equal 0");
        uint256 endIndex = MathUpgradeable.min(startIndex + length, _stockLocations.length);
        uint256[] memory ret = new uint256[](endIndex-startIndex);
        for (uint256 idx = startIndex; idx <= endIndex; ++idx) {
            ret[idx] = _stockLocations[idx];
        }
        return ret;
    }
        ...
        )
    {
        ...
        return (
            _tokenDetailsMapping[tokenId],
            ownerOf(tokenId),
            tokenURI(tokenId),
            IRiverBoxExchange(internalExchangeAddress).dealIdOfTokenId(tokenId)
        );
    }
    ...
    /**
     * @dev Fetch tokenIds from a giver owner by index pagination
     */
    function tokenOfOwnerByIndexPagination(
        address owner,
        uint256 startIndex,
        uint256 length
    ) public view override returns (uint256[] memory) {
        uint256 endIndex = MathUpgradeable.min(startIndex + length, balanceOf(owner));
        uint256[] memory ids = new uint256[](endIndex.sub(startIndex));
        uint256 count = 0;
        for (uint256 i = startIndex; i < endIndex; ++i) {
            ids[count] = tokenOfOwnerByIndex(owner, i);
            ++count;
        }
        return ids;
    }
    ...
    /**
     * @dev Fetch tokenIds from all tokens by index pagination
     */
    function tokenByIndexPagination(uint256 startIndex, uint256 length)
        public
        view
        override
        returns (uint256[] memory)
    {
        uint256 endIndex = MathUpgradeable.min(startIndex + length, totalSupply());
        uint256[] memory ids = new uint256[](endIndex.sub(startIndex));
        uint256 count = 0;
        for (uint256 i = startIndex; i < endIndex; ++i) {
            ids[count] = tokenByIndex(i);
            ++count;
        }
        return ids;
    }
    ...
    function isApprovedForAll(address owner, address operator) public view override(ERC721Upgradeable) returns (bool) {
        // Whitelist internal exchange
        if (internalExchangeAddress == operator) {
            return true;
        }
    ...
    function _beforeTokenTransfer(
        ...
        require(
            !IRiverBoxExchange(internalExchangeAddress).tokenListed(tokenId),
            "cannot transfer a token that is listed in internal exchange"
        );
    ...
    function buy(uint256 quality, address referrer) public payable override whenNotPaused notContract nonReentrant {
        ...
        for (uint256 i = 0; i < promotion; ++i) {
            ...
            Product memory newItem = Product(pawnLocationId, 0, signature, block.timestamp, new uint256[](0));
    ...
    function fuse(uint256[] memory tokenIds) public override whenNotPaused notContract nonReentrant {
        ...
        uint16 parentLocationId = _tokenHierarchyMapping[_tokenDetailsMapping[tokenIds[0]].locationId].parentLocationId;
        Product memory newItem = Product(parentLocationId, 0, 0, block.timestamp, tokenIds);
        for (uint256 i = 0; i < tokenIds.length; ++i) {
    ...
    function setPawnLocationStock(uint16 locationId, uint16 locationIdStock) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        for (uint16 i = 0; i < locationIdStock; ++i) {
    ...
    /**
     * @dev Set internal exchange contract address
     * @param newAddress new exchange address
     */
    function setInternalExchange(address newAddress) public override {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        internalExchangeAddress = newAddress;
    }
    ...
    function debugMint(uint16 locationId, address receiver) public {
        ...
        Product memory newItem = Product(locationId, 0, signature, block.timestamp, new uint256[](0));
...
interface IRiverBox {
    ...
    function tokenByIndexPagination(uint256 startIndex, uint256 length) external view returns (uint256[] memory);
    ...
    function setInternalExchange(address newAddress) external;
...
interface IRiverBoxExchange {
    ...
    /* ================ EVENTS ================ */
    event DealPosted(address indexed seller, uint256 indexed tokenId, uint256 price, uint256 indexed dealId);

    event DealCanceled(uint256 indexed dealId);

    event DealBought(uint256 indexed dealId, address indexed buyer);

    /* ================ VIEWS ================ */
    function dealIdOfTokenId(uint256 tokenId) external view returns (uint256);

    function tokenListed(uint256 tokenId) external view returns (bool);

    function dealDetail(uint256 dealId) external view returns (Deal memory);

    function dealOfSellerByIndex(address seller, uint256 index) external view returns (uint256);

    function totalDeals() external view returns (uint256);

    function dealByIndex(uint256 index) external view returns (uint256);

    function ownerOf(uint256 dealId) external view returns (address);
    ...
    function postDeal(uint256 tokenId, uint256 price) external;

    function cancelDeal(uint256 dealId) external;

    function buyDeal(uint256 dealId) external payable;
