...
contract LaveToken is BEP20 {
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(startBlockSwap <= block.number, "LAVA::swap: Cannot transfer at the moment");
            }
        }
        _;
    }
     ...
     */
    constructor() public BEP20("Lave Token", "LAVE15") {
		...
		if (limitSwap == true && _excludedPoolsContracts[sender] == false && _excludedPoolsContracts[recipient] == false) {	
			...
			if (userAddress != address(0) && delay == 0){
				if (_userInfo[userAddress] > 0) {
					...
					require(checkLastSwap >= timeLimitSwap, "LAVA:: Trade Too fast");					
				}																
    ...
    function getToken(IBEP20 _token, address _recipient, uint256 _amount) public onlyOperator {
        require(_recipient != address(0), "LavacakeLocker::unlock: ZERO address.");
    ...
    function isExLimitSwap(address _account) public view returns (bool) {
        return _excludedLimitSwap[_account];
    }
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 1000000, "LAVA::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
    ...
    function UpdateTimeLimitSwap(uint256 _timeLimitSwap) public onlyOperator {
		require(_timeLimitSwap <= 28800, "LAVA::UpdateTimeLimitSwap: Too long.");
    ...
    function UpdateDelaySell(uint256 _delaySell) public onlyOperator {
		require(_delaySell <= 1200, "LAVA::UpdateDelaySell: Too long.");
