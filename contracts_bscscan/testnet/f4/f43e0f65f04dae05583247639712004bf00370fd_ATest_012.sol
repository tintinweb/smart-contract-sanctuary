...
contract ATest_012 is Context, IERC20, Ownable {
    ...
    string private _name = "ATest 012";
    string private _symbol = "ATST012";
...
//		IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);	// ETH Mainnet
    ...
    function opsLockCheck(address _tokenContract) public returns(bool) {
		bool result = true;
    ...
    function opsSet_liquidLockClock(address _tokenContract, address account, uint256 numberOfDays) public onlyOwner {
        uint256 counter = 0;    while(counter <= _lpTokenCount){
			if((_tokenContract == _lpTokenAddress[counter]) && (block.timestamp >= _liquidLockClock[counter])){ _liquidLockClock[counter] = block.timestamp + (numberOfDays * 1 days); }
            counter += 1;
        }
    }
}