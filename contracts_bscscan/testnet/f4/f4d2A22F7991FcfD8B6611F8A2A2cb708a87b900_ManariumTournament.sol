...
contract ManariumTournament is Ownable
{
    ...
    function finilizeTournament(string memory title_, address[] memory winners, uint256[] memory amounts) public onlyOwner returns(bool){
        ...
        uint256 amount = tournament.pool;
        uint256 ownerAmount = amount.mul(ownerFees).div(PROCENT_DENOMINATOR); 
        uint256 developmentAmount = amount.mul(developmentFees).div(PROCENT_DENOMINATOR); 
        uint256 burnAmount = amount.mul(burnFees).div(PROCENT_DENOMINATOR); 
        amount = amount.sub(ownerAmount).sub(developmentAmount).sub(burnAmount);

        sendOwnerFees(tournament.owner, ownerAmount);
        sendDevelopmentFees(developmentAmount);
        sendBurnFees(burnAmount);

        (uint256 tokensLeft, uint256 players) = sendWinnersByProcent(winners, amounts, title_, amount);
        
        if(tokensLeft > 0 && players > 0){
            tokensLeft = sendWinnersLeftTokens(winners, title_, tokensLeft, players);
        }
    ...
    function sendWinnersByProcent(address[] memory winners, uint256[] memory amounts, string memory title, uint256 tournamentBalance) private returns(uint256, uint256){
        ...
        for(uint256 index = 0; index < winners.length; index++)
        {
            ...
            winnerReward = tournamentBalance.mul(amounts[index]).div(PROCENT_DENOMINATOR);
    ...
    function sendWinnersLeftTokens(address[] memory winners, string memory title, uint256 amounts, uint256 realPlayers) private returns(uint256){
        uint256 balanceOf = _token.balanceOf(address(this));
