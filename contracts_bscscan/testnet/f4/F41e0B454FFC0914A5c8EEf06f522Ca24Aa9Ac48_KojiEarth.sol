...
contract DividendDistributor is IDividendDistributor {
    ...
    function setShare(address shareholder, uint256 amount) external override onlyToken {
        ...
        if(amount == 0 && shares[shareholder].heldAmount > 0) {      
            ...
            shares[shareholder].totalUnrealized = getUnpaidEarnings(shareholder);    
        ...
        if(amount > 0 && shares[shareholder].heldAmount == 0) {
            ...
            if (amount < minHoldAmountForRewards) {
                ...
                //we only want to add the holder to the index if he is eligible for rewards
        ...
        if(amount > 0 && shares[shareholder].heldAmount > 0) {
            ...
            if (amount > minHoldAmountForRewards && shares[shareholder].heldAmount > minHoldAmountForRewards) {
                if (amount < shares[shareholder].heldAmount) { //sold


                    if(getUnpaidEarnings(shareholder) > shares[shareholder].totalUnrealized) {
                        shares[shareholder].totalUnrealized = getUnpaidEarnings(shareholder);
                    }
                ...
                if (amount > shares[shareholder].heldAmount) { //bought/reinvest
            ...
            if (amount < minHoldAmountForRewards && shares[shareholder].heldAmount > minHoldAmountForRewards) {
                if(shares[shareholder].totalUnrealized > minDistribution) {
    ...
    function distributeAll(uint256 gas) external {
     ...
     function distributeDividend(address shareholder, uint256 percent) public {
        ...
        uint256 remaining = getUnpaidEarnings(shareholder);
        
        uint256 amount = remaining.mul(percent).div(100);
        ...
        if(amount > 0) {
            ...
            //shares[shareholder].totalExcluded = shares[shareholder].totalExcluded.add(netamount.add(1));
            shares[shareholder].totalUnrealized = remaining;
            shares[shareholder].totalRealized = shares[shareholder].totalRealized.add(netamount.add(1));
    ...
    function getUnpaidEarnings(address shareholder) public view returns (uint256) {
        if(shares[shareholder].amount == 0){ return shares[shareholder].totalUnrealized; }

        uint256 shareholderTotalDividends = getCumulativeDividends(shares[shareholder].amount).add(shares[shareholder].totalUnrealized);
    ...
    function holderInfo(address _holder) external view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return (shares[_holder].amount, shares[_holder].heldAmount, shares[_holder].unpaidDividends, shares[_holder].totalExcluded, shares[_holder].totalUnrealized, shares[_holder].totalRealized);
    }
    ...
    function viewAnomaly() external view returns (uint256) {
        return totalAnomaly;
    }
...
contract KojiEarth is IBEP20, Auth {
    ...
    string constant _symbol = "KOJI DT1.11";
    ...
    function ViewHolderInfo(address _address) external view returns (uint256 amount, uint256 held, uint256 unpaid, uint256 excluded, uint256 unrealised, uint256 realised) {
    ...
    function ViewtheAnomaly() external view returns (uint256) {
       return distributor.viewAnomaly();
    }
}