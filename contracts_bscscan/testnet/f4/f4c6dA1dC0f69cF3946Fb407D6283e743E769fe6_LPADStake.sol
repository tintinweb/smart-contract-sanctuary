...
interface IVerify {
    function isValidSigner(uint _amount, address _caller, bytes memory _signature) external pure returns(bool);
}

contract MyToken is ERC20 {
    constructor() ERC20("The Professor's Coin", "TPS") {
...
contract LPADStake {
    ...
    event ClaimReward(address _receiver, uint amount);
    
    constructor(address tokenAddress_, address verifyAddress_) {
     ...
     */
    function stakeCoins(uint _amount) external {
        require(_amount != 0, "LPADStake: _amount should non-zero!");
        require(IERC20(_tokenAddress).balanceOf(msg.sender) >= _amount, "LPADStake: Insufficient Funds!");
        require(IERC20(_tokenAddress).transferFrom(msg.sender, address(this), _amount));
        if(_stakes[msg.sender] == 0) _stakeHolders.push(msg.sender); 
        _stakes[msg.sender] = _stakes[msg.sender].add(_amount);
     ...
     */
    function unstakeCoins(uint _amount) external {
        require(_amount != 0, "LPADStake: _amount should non-zero!");
        bool _isExist = isStakeHolder(msg.sender);
        require(_isExist == true, "LPADStake: You don't have any stake!");
        require(IERC20(_tokenAddress).balanceOf(address(this)) >= _amount, "LPADStake: Insufficient tokens!");
        require(_stakes[msg.sender] >= _amount, "LPADStake: Not Enough Stakes!");
    ...
    function removeStakeHolder(address _stakeHolder) private {
        for(uint index = 0; index < _stakeHolders.length; index++) {
    ...
    /**
     * @notice It shows weather theaddress is a valid stakeholder or not
     ...
     */
    function claimReward(uint _amount, bytes memory _signature) public {
        require(_amount != 0, "LPADStake: _amount should non-zero!");
        bool _isExist = isStakeHolder(msg.sender); 
        require(_isExist == true, "LPADStake: Unauthorized access!");
        require(_amount <= IERC20(_tokenAddress).balanceOf(address(this)), "LPADStake: Insufficient funds!");
        require(IVerify(_verifyAddress).isValidSigner(_amount, msg.sender, _signature) == true, "LPADStake: You haven't signed the message!");
