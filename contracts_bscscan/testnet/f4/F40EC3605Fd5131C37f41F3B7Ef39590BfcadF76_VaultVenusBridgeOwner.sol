...
contract VaultVenusBridgeOwner is WhitelistUpgradeable {
  ...
  VAIVault private constant XVS_HOLD_XVS_Claim =
    VAIVault(0x7Db4f5cC3bBA3e12FF1F528D2e3417afb0a57118);
  ...
  function getXVS(address vault) public view returns (uint256 xvsBalance) {
    VaultVenusBridge.MarketInfo memory market = VENUS_BRIDGE.infoOf(vault);
    address[] memory vTokens = new address[](1);
    vTokens[0] = market.vToken;
  ...
  function harvestBehalf(address vault) public returns (uint256 XVSBalance) {
    ...
    uint256 xvsBefore = XVS.balanceOf(msg.sender);
    VENUS_UNITROLLER.claimVenus(address(VENUS_BRIDGE), vTokens);

    // XVS_HOLD_XVS_Claim.claim();
    uint256 xvsBalance = XVS.balanceOf(msg.sender).sub(xvsBefore);
    // if (xvsBalance > 0) {
