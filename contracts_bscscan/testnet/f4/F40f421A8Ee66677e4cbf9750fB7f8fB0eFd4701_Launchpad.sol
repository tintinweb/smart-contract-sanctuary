...
contract Launchpad is Ownable, DataStorage, Access, Events {
    ...
    function _registerBuy(address _beneficiary, uint256 amount) internal {
        ...
        require(
            buyToken.allowance(_beneficiary, address(this)) >= amount.mul(priceToken).div(10**IBEP20(saleToken).decimals()),
        ...
        _forwardFunds(weiAmount.mul(priceToken).div(10**IBEP20(saleToken).decimals()), _beneficiary);
        _deliverTokens(msg.sender, user.tokenBuy);
        emit TokenPurchase(_beneficiary, weiAmount.mul(priceToken).div(10**IBEP20(saleToken).decimals()), weiAmount);
