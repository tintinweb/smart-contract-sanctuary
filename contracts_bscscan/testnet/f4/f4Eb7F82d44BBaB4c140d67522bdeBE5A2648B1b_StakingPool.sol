...
contract StakingPool is AccessControl, ReentrancyGuard {
    ...
    mapping(uint256 => uint256[2]) public durations;
    ...
    event Deposit(address indexed account, uint256 indexed stakeId, uint256 sssAmount, uint256 vesssAmount, uint256 durationIndex);
    event Withdraw(address indexed account, uint256 indexed stakeId, uint256 sssAmount, uint256 vesssAmount);
    event Harvest(address indexed account, address indexed token, uint256 amount);
    event SetDuration(uint256 indexed durationIndex, uint256 indexed second, uint256 indexed divisor);
    ...
    constructor(
        ...
        require(lockTimes.length > 0, "lockTimes empty");
        require(lockTimes.length == divisors.length, "length not match");
        // if (lockTimes.length == 0 || lockTimes.length != divisors.length) {
        //     revert();
        // }
        ...
        */
        ...
        for (uint8 i = 0; i < lockTimes.length; i++) {
            durations[i + 1] = [lockTimes[i], divisors[i]];
        }
    ...
    function setDuration(
        ...
        onlyOwner
    {
        durations[durationIndex] = [second, divisor];
        emit SetDuration(durationIndex, second, divisor);
    }
    ...
    function deposit(uint256 sssAmount, uint256 durationIndex) external nonReentrant {
        require(sssAmount > 0, "EAMOUNT");
        require(durations[durationIndex][1] > 0, "EDURATIONINDEX");

        stakingToken.safeTransferFrom(_msgSender(), address(this), sssAmount);

        uint256 vesssAmount = _sssToVesss(sssAmount, durationIndex);
        Asset memory asset = Asset (
            _stakeIdCounter.current(),
            sssAmount,
            vesssAmount,
            block.timestamp,
            block.timestamp + durations[durationIndex][0]
        );
        Staker storage staker = users[_msgSender()];
        staker.stakedAssets.push(asset);

        require(users[_msgSender()].power + vesssAmount >= users[_msgSender()].power, "EOVERFLOW");
        require(totalPower + vesssAmount >= totalPower, "EOVERFLOW");

        users[_msgSender()].power += vesssAmount;
        totalPower += vesssAmount;
        _updateUserDebt(_msgSender(), vesssAmount, true);

        totalStaked += sssAmount;

        _stakeIdCounter.increment();

        emit Deposit(_msgSender(), asset.stakeId, sssAmount, vesssAmount, durationIndex);
    }
    ...
    function withdraw(uint256 stakeId) external nonReentrant {
        ...
        require(users[_msgSender()].power - asset.vesssAmount <= users[_msgSender()].power, "EUNDERFLOW");
        require(totalPower - asset.vesssAmount <= totalPower, "EUNDERFLOW");

        _removeAsset(_msgSender(), stakeId);

        users[_msgSender()].power -= asset.vesssAmount;
        totalPower -= asset.vesssAmount;
        _updateUserDebt(_msgSender(), asset.vesssAmount, false);

        stakingToken.safeTransfer(_msgSender(), asset.sssAmount);

        totalStaked -= asset.sssAmount;

        emit Withdraw(_msgSender(), stakeId, asset.sssAmount, asset.vesssAmount);
    }
    ...
    function harvestAll(address to) public {
        for(uint i = 0; i < rewardList.length; i++) {
            ...
            if(_pendingReward > 0) {
                if (token == wbnb) {
                    (bool success,) = wbnb.call(abi.encodeWithSignature("withdraw(uint256)", _pendingReward));
                    require(success, "EWITHDRWAWBNB");
            ...
            emit Harvest(_msgSender(), token, _pendingReward);
        }
    }
    ...
    function _sssToVesss(uint256 sssAmount, uint256 duration) internal view returns(uint256) {
        return sssAmount / durations[duration][1];
    }
}
