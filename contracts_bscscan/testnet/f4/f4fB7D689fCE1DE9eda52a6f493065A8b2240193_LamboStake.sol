...
contract LamboStake is Context, Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) private _stakePools;
    mapping(address => uint256) private _currentStakePools;
    mapping(address => uint256) private _currentTotalStakePools;
    mapping(address => uint256) private _previousStakePools;
    mapping(address => uint256) private _previousTotalStakePools;
    mapping(address => uint256) private _totalStakePools;
    mapping(address => uint256) private _lastSwitchPools;
    mapping(address => mapping(address => uint256)) private _userStakes;
    mapping(address => mapping(address => uint256)) private nextAvailableClaimDate;
    address private retrieveFundWallet;
    uint256 stakePeriod = 7 days;
    ...
    function _poolTokens(address tokenAddress,uint256 amount) private {
        ...
        _stakePools[tokenAddress] = _stakePools[tokenAddress] + amount;
    }
    
    function _switchPool(address tokenAddress) private {
        uint256 previous = _previousStakePools[tokenAddress];
        _previousStakePools[tokenAddress] = _currentStakePools[tokenAddress];
        _previousTotalStakePools[tokenAddress] = _previousStakePools[tokenAddress];
        // add previous previous to current stake
        _currentStakePools[tokenAddress] = _stakePools[tokenAddress] + previous;
        _currentTotalStakePools[tokenAddress] = _currentStakePools[tokenAddress];
        _stakePools[tokenAddress] = 0;
    ...
    function resetPoolTokens(address tokenAddress) public onlyAuthorizedCallers {
        _previousStakePools[tokenAddress] = 0;
        _previousTotalStakePools[tokenAddress] = 0;
        _currentStakePools[tokenAddress] = 0;
        _currentTotalStakePools[tokenAddress] = 0;
    ...
    function switchPool(address tokenAddress) external onlyAuthorizedCallers nonReentrant {
       	_switchPool(tokenAddress);
    }
    ...
    function stakeTokens(address tokenAddress,uint256 amount) external isHuman nonReentrant {
        ...
        uint256 tax = amount.mul(20000).div(1000000);
       	amount = amount - tax;
       	_stakePools[tokenAddress] = _stakePools[tokenAddress] + tax;
        _userStakes[tokenAddress][msg.sender] = amount;
        _totalStakePools[tokenAddress] = _totalStakePools[tokenAddress] + amount;
        nextAvailableClaimDate[tokenAddress][msg.sender] = block.timestamp + (stakePeriod * (1 days));
        emit StakeTokenSuccessfully(tokenAddress,msg.sender, totalAmount, tax, amount, nextAvailableClaimDate[tokenAddress][msg.sender]);
    }
    
    function canClaim(address tokenAddress,address account) external view returns (bool) {
      	return nextAvailableClaimDate[tokenAddress][account] <= block.timestamp;  
    }
    
    function stakePools(address tokenAddress) external view returns (uint256) {
      	return _stakePools[tokenAddress];  
    }

    function currentStakePools(address tokenAddress) external view returns (uint256) {
      	return _currentStakePools[tokenAddress];  
    }

    function currentTotalStakePools(address tokenAddress) external view returns (uint256) {
      	return _currentTotalStakePools[tokenAddress];  
    }

    function previousStakePools(address tokenAddress) external view returns (uint256) {
      	return _previousStakePools[tokenAddress];  
    }

    function previousTotalStakePools(address tokenAddress) external view returns (uint256) {
      	return _previousTotalStakePools[tokenAddress];  
    }

    function lastSwitchPools(address tokenAddress) external view returns (uint256) {
      	return _lastSwitchPools[tokenAddress];  
    }

    function userStakes(address tokenAddress,address account) external view returns (uint256) {
      	return _userStakes[tokenAddress][account];  
    }

    function nextClaimDate(address tokenAddress,address account) external view returns (uint256) {
      	return nextAvailableClaimDate[tokenAddress][account];  
    }

    function totalStakePools(address tokenAddress) external view returns (uint256) {
      	return _totalStakePools[tokenAddress];  
    }

    function estimatedRewards(address tokenAddress,address account) public view returns (uint256) {
        uint256 poolAmount = _currentStakePools[tokenAddress];
        bool previous = false;
        if (nextAvailableClaimDate[tokenAddress][account] <= _lastSwitchPools[tokenAddress]) {
            // new pools has been added since, so estimate from previous pool
            poolAmount = _previousStakePools[tokenAddress];
            previous = true;
        }
        if (poolAmount > 0) {
			uint256 stakerBalance = _userStakes[tokenAddress][account];
	        uint256 rewardPercentage = stakerBalance.mul(1000000).div(_totalStakePools[tokenAddress]);
    ...
    function claimRewards(address tokenAddress) public isHuman nonReentrant {
		...
		if (_totalStakePools[tokenAddress] > maxToRetrieve) {
		    _totalStakePools[tokenAddress] = maxToRetrieve;
		}
        if (nextAvailableClaimDate[tokenAddress][msg.sender] <= block.timestamp) {
            uint256 reward = estimatedRewards(tokenAddress,msg.sender);
            bool previous = nextAvailableClaimDate[tokenAddress][msg.sender] <= _lastSwitchPools[tokenAddress];
	        nextAvailableClaimDate[tokenAddress][msg.sender] = block.timestamp + (stakePeriod * (1 days));
            if (reward > 0) {
		        if (previous) {
			        _previousStakePools[tokenAddress] = _previousStakePools[tokenAddress] - reward;
		        } else {
			        _currentStakePools[tokenAddress] = _currentStakePools[tokenAddress] - reward;
		        }
		        _userStakes[tokenAddress][msg.sender] = _userStakes[tokenAddress][msg.sender] + reward;
		        emit ClaimRewardSuccessfully(tokenAddress,msg.sender, reward, nextAvailableClaimDate[tokenAddress][msg.sender]);
            }
		}
    }
    ...
    function _unstakeTokens(address tokenAddress,address account) private {
        uint256 maxToRetrieve = IBEP20(tokenAddress).balanceOf(address(this));
        uint256 maxToRetrievePool = _totalStakePools[tokenAddress];
        ...
        // take tax fee
        uint256 tax = stakerBalance.mul(20000).div(1000000);
       	stakerBalance = stakerBalance - tax;
		require(stakerBalance > 0,"Error no tokens to send.");
       	_stakePools[tokenAddress] = _stakePools[tokenAddress] + tax;
		_totalStakePools[tokenAddress] = _totalStakePools[tokenAddress]-stakerBalance;
		_userStakes[tokenAddress][account] = 0;
    ...
    function setNextClaimDate(address tokenAddress,address account,uint256 when) external nonReentrant onlyAuthorizedCallers {
		nextAvailableClaimDate[tokenAddress][account] = when;
	}
    ...
    function retrievePoolTokens(address tokenAddress) external nonReentrant onlyAuthorizedCallers {
        uint256 maxToRetrieve = IBEP20(tokenAddress).balanceOf(address(this));
        uint256 toRetrieve = _currentStakePools[tokenAddress] + _previousStakePools[tokenAddress];
