...
contract BHOLaunchpad is Ownable, Pausable, Whitelist, SharedConstants {
    ...
    mapping(address => uint256) private amountsStaked;
    ...
    constructor(
        ...
        // require(
        //     _launchpadData.startAt > block.timestamp,
        //     "Start time must be after now"
        // );
        // require(
        //     _launchpadData.startAt < _launchpadData.endAt,
        //     "Start time must be less than end time"
        // );
        require(
            _launchpadData.softCap > 0 && _launchpadData.hardCap > 0,
            "Soft and Hard cap must be other than 0"
        );
        require(
            _launchpadData.softCap < _launchpadData.hardCap,
            "Soft cap must be less than hard cap"
        );
        require(
            _launchpadData.minBuy > 0 && _launchpadData.maxBuy > 0,
            "Min and Max must be other than 0"
        );
        require(
            _launchpadData.minBuy < _launchpadData.maxBuy,
        ...
        require(
            _vestingTimeline.percents.sum() == 100,
    ...
    function finalize() public onlyOwner {
        require(
            ...
            "Launchpad is finalized"
        );
        require(
            status != Statuses.CANCELLED,
            "Launchpad is cancelled"
        );
        require(
            capacity >= launchpadData.softCap,
            "Capacity user bought less than soft cap"
        );

        // Get 98% BNB to FACTORY_ADDRESS
        uint256 amountToken = capacity.mul(98).div(100);
        launchpadData.payInToken.transfer(owner(), amountToken);

        // Get remain BNB include fee emergency withdraw and 2% capacity
    ...
    function cancel() public onlyOwner {
        ...
        if (block.timestamp < launchpadData.startAt) {
            ...
            require(
                block.timestamp > launchpadData.endAt,
                "Launchpad do not end yet"
            );
            status = Statuses.CANCELLED;
            // Refund BNB for user
        }
     ...
     */
    function registration(uint256 _amount) external checkWhoCanBuy {
        ...
        // max register user can buy
        address buyer = _msgSender();
        amountsStaked[buyer] = amountsStaked[buyer].add(_amount);
        ...
        returns (uint256 amount, uint256 claimAt)
    {
        ...
        address buyer = _msgSender();
        uint256 amountUserStaked = amountsStaked[buyer];
        ...
        uint256 amountWillVesting = amountOfUser.mul(percent).div(100);
    ...
    function emergencyWithdraw() external {
        require(block.timestamp < launchpadData.endAt, "Launchpad is ended");
        require(
            status != Statuses.FINZALIZE,
            "Launchpad is finalized"
        );
        ...
        uint256 amountFinal = amountUserStaked
            .mul(EMERGENCY_WITHDRAW_PORTION)
            .div(100);

        amountsStaked[buyer] = 0;
    ...
    function _handleUnsoldToken() internal {
        ...
        uint256 remainToken = remainCap.mul(launchpadData.presaleRate).div(
            launchpadData.payInToken.decimals()
        );
