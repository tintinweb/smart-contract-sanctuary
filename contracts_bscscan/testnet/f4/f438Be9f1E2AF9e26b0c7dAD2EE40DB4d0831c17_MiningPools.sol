...
contract MiningPools is AccessControl {
    ...
    function _initMaterial(address[12] memory _token) private {
        material[0] = Material(1, Mint(_token[0]));
        material[1] = Material(10, Mint(_token[1]));
        material[2] = Material(10, Mint(_token[2]));
        material[3] = Material(10, Mint(_token[3]));
        material[4] = Material(10, Mint(_token[4]));
        material[5] = Material(150, Mint(_token[5]));
        material[6] = Material(150, Mint(_token[6]));
        material[7] = Material(200, Mint(_token[7]));
        material[8] = Material(1500, Mint(_token[8]));
        material[9] = Material(1500, Mint(_token[9]));
        material[10] = Material(1750, Mint(_token[10]));
        material[11] = Material(20000, Mint(_token[11]));
    }
    ...
    constructor(address _token, address[12] memory _material) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        token = IERC20(_token);
        _initMaterial(_material);
    }
    ...
    function exchange(uint256 _mid, uint256[12] memory amount) public {
        ...
        uint256 point;
        for (uint256 i = 0; i < 12; i++) {
            point = point.add(amount[i].mul(material[i].weight));
        }
        ...
        for (uint256 j = 0; j < 12; j++) {
            if (amount[j] > 0) {
                ...
                material[j].token.mint(msg.sender, amount[j]);
            }
        }
    }
    ...
    function stake(uint256 _mid, uint256 _amount) public {
        ...
        updateMine(_mid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(mine.pointPerShare).div(1e12).sub(
                user.rewardDebt
            );
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(mine.pointPerShare).div(1e12);
        ...
        returns (uint256)
    {
        ...
        if (block.number > mine.lastRewardBlock && mine.totalSupply != 0) {
            ...
            pointPerShare = mine.pointPerShare.add(
                pointAdded.mul(1e12).div(mine.totalSupply)
            );
        }
        return user.amount.mul(pointPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function getTotalPointPerBlock(uint256 _mid) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < 12; i++) {
            totalPoint = totalPoint.add(
                materialsOutput[i].mul(material[i].weight)
            );
        }
        return totalPoint;
    }
