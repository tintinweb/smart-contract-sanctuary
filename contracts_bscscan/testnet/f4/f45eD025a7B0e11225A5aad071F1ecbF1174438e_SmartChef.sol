...
contract SmartChef is Ownable {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 flokimarsReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accRewardPerShare = accRewardPerShare.add(flokimarsReward.mul(1e18).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 flokimarsReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accRewardPerShare = pool.accRewardPerShare.add(flokimarsReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }


    // Stake ROCKET tokens to SmartChef
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                uint256 depositFee = _sendAmount.mul(pool.depositFeeBP).div(10000);
                pool.lpToken.safeTransfer(feeAddress, depositFee);
                user.amount = user.amount.add(_sendAmount).sub(depositFee);
            } else {
    ...
    // Withdraw ROCKET tokens from STAKING.
