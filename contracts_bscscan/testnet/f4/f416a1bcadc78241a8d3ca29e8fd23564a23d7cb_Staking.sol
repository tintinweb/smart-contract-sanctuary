...
contract Staking{
    ...
    event redeem(address _user, address _nft_addr, uint _nft_id, uint256 _time, uint256 _order_id, uint256 _amount);
    ...
    // 贖回
    function Redeem_NFT(uint256 _order_id) public returns (bool){
        ...
        if(order.withdraw_time <= _etime)
        {
            ...
            // 寫入事件
            //emit redeem(msg.sender, order.nft_addr, order.nft_id, _etime,_order_id, _amount);
        ...
        else
        {
            ...
            if(NFT_OF[msg.sender].length >=1 && NFT_IF[msg.sender].length >=1 && NFT_H[msg.sender].length >=1)
            {
                ...
                if(order.nft_type % 10 ==0 || order.nft_type % 10 ==1 || order.nft_type % 10 ==5)
                {
                    ...
                    // 寫入事件
                    //emit redeem(msg.sender, order.nft_addr, order.nft_id, _etime, _order_id, _amount_0);//寫入個別的利息
                    emit redeem(msg.sender, order2.nft_addr, order2.nft_id, _etime, order2_id, _amount_1);//寫入個別的利息
                    emit redeem(msg.sender, order3.nft_addr, order3.nft_id, _etime, order3_id, _amount_2);//寫入個別的利息
