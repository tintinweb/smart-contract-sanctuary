...
contract MybiToken is Context, IERC20, Ownable {
    ...
    string private constant _name = "My Bingo v1";
    string private constant _symbol = "MYBI";
    ...
    uint256 public _mybiFee = 8;
    uint256 public _previousMybiFee = _mybiFee;
    
    uint256 public totalToMybi = 0;
    address public MYBI_ADDRESS;
    ...
    function setMybiWallet(address _mybiWallet) external onlyOwner {
        MYBI_ADDRESS= _mybiWallet;
    }
    ...
    function setMybiFeePercent(uint256 mybiFee) external onlyOwner() {
        _mybiFee = mybiFee;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (TValuesStruct memory) {
         ...
         TValuesStruct memory tValues = TValuesStruct(
             {
                 ...
                 tMybi: calculateMybiFee(tAmount),
        ...
        tValues.tTransferAmount = tValues.tTransferAmount.sub(tValues.tMybi).sub(tValues.tBurn);
    ...
    function _getRValues(uint256 tAmount, TValuesStruct memory tValues, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rMybi = tValues.tMybi.mul(currentRate);
        uint256 rBurn = tValues.tBurn.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rMarketing).sub(rMybi).sub(rBurn);
    ...
    function _takeMybi(uint256 tMybi) private {
        uint256 currentRate =  _getRate();
        uint256 rMybi = tMybi.mul(currentRate);
        _rOwned[MYBI_ADDRESS] = _rOwned[MYBI_ADDRESS].add(rMybi);
        totalToMybi = totalToMybi.add(rMybi);
        if(_isExcluded[MYBI_ADDRESS])
            _tOwned[MYBI_ADDRESS] = _tOwned[MYBI_ADDRESS].add(tMybi);
    }
    ...
    function calculateMybiFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_mybiFee).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _marketingFee == 0 && _mybiFee == 0 && _burnFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousMarketingFee = _marketingFee;
        _previousMybiFee = _mybiFee;
