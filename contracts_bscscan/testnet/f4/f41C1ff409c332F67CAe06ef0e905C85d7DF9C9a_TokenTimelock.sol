...
contract TokenTimelock is Ownable {
    ...
    uint256 constant public START_TIME = 1641394800; // 15:00:00 GMT 5/1/2022
    ...
    function release() public {
        ...
        uint256 amount = numberOfToken[msg.sender].mul(cliff).div(243);
        if (countRelease[msg.sender] + cliff >= 243){
    ...
    function getBalanceAviable(address _addr) external view returns(uint256) {
        uint256 cliff = block.timestamp.sub(nextRelease[_addr]).div(PERIOD) + 1;
        uint256 amount = numberOfToken[_addr].mul(cliff).div(243);
        return amount;
    }
