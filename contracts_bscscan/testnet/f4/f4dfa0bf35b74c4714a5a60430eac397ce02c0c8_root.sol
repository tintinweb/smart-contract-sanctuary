...
contract root {
    ...
    function estProfit (address[] calldata buyPath, address[] calldata reversedSellPath) public view returns (uint256){
        uint256 buyAmountA = routerA.getAmountsOut(buyQuantity, buyPath)[buyPath.length - 1];
        uint256 buyAmountB = routerB.getAmountsOut(buyQuantity, reversedSellPath)[reversedSellPath.length - 1];
        if(buyAmountA > buyAmountB) {
            return (buyAmountA - buyAmountB) * buyQuantity / (buyAmountB) ;
        }
        return 0;
    }
    ...
    function estProfit2 (address _addrA, address _addrB,address[] calldata buyPath, address[] calldata reversedSellPath) public view returns (uint256){
        uint256 buyAmountA = Router(_addrA).getAmountsOut(buyQuantity, buyPath)[buyPath.length - 1];
        uint256 buyAmountB =  Router(_addrB).getAmountsOut(buyQuantity, reversedSellPath)[reversedSellPath.length - 1];
        if(buyAmountA > buyAmountB) {
            return (buyAmountA - buyAmountB) * buyQuantity / buyAmountB ;
        }
        return 0;
    ...
    function swap2 (address _addrA, address _addrB, address[] calldata buyPath, address[] calldata sellPath) public onlyOwner {
        require(buyPath[buyPath.length - 1] == sellPath[0]);
        require(buyPath[0] == sellPath[sellPath.length - 1]);
        routerA = Router(_addrA);
        routerB = Router(_addrB);
        uint256 buyAmount = routerA.getAmountsOut(buyQuantity, buyPath)[buyPath.length - 1];
        IBEP20 tokenToSell = IBEP20(sellPath[0]);
        IBEP20 tokenToSpend = IBEP20(buyPath[0]);
        uint256 startingAmount = tokenToSpend.balanceOf(to);
