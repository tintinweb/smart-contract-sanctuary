  ...
  ReentrancyGuardUpgradeable
{
  ...
  function initialize(
    ...
    uint16 marketingFeeBps_,    // marketing fee bps        (3    %)  300
  ...
  { 
    ...
    antiBot.setTokenOwner(_msgSender());
    // antiBot.setTokenOwner(owner_);

    // OwnableUpgradeable.transferOwnership(payable(owner_));
    ...
    private
  {
    ...
    swapTokensForEth(tFee, _supportAddress);
  }
    ...
    private
  {
    if (tMarketing > 0) {
      ...
      swapTokensForEth(tMarketing, _marketingAddress);
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
    ...
    // swap tokens for BNB
    swapTokensForEth(half, address(this)); // <- this breaks the BNB -> NEFTI swap when swap+liquify is triggered
  ...
  function recallTax() public nonReentrant onlyOwner { _transfer(address(this), _msgSender(), balanceOf(address(this))); }
