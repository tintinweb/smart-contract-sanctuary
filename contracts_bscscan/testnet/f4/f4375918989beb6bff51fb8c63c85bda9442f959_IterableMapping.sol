...
contract WISHDOGE is ERC20, Ownable, SafeToken, LockToken {
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _BBFee) public onlyOwner {
        ...
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(BBFee); // total fee transfer and buy
    }
    ...
    constructor() ERC20("Wish Doge Dragon", "WISHDOGE") {
        BNBRewardsFee = 6;
        ...
        marketingFee = 2;
        BBFee = 1;
        marketingWallet = payable(0xf416398740D8d98a76CC9A3ae29B4C2A84b4B30f);
        BBWallet = payable(0xB35092f3Dc661fa3625c8fC47229c8Ad0f552e7E);
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) open(from, to) whenNotPaused() internal override {
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            {
                uint256 extraFeeN = extraFee/2;
                swapAndSendBNBToMarketing(extraFeeN); // swap the marketing fee on second
            }
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // BB fee
        uint256 BBAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(BBFee);
        BBWallet.transfer(BBAmount);
