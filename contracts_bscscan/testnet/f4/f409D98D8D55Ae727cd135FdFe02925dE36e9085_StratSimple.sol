...
contract StratSimple is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public totalAmountWithdraw=0;
    ...
    function withdrawLoan(uint256 _withdrawAmt, address _withdrawAddress) public onlyGovernance {
       require(_withdrawAmt.add(totalAmountWithdraw)<wantLockedTotal.mul(withdrawPercentage).div(Denominator));
       totalAmountWithdraw=totalAmountWithdraw.add(_withdrawAmt);
       IERC20(wantAddress).safeTransfer(_withdrawAddress, _withdrawAmt);

    }


    function repayLoan(uint256 _withdrawAmt) public  {
        require(_withdrawAmt>0,"should bw greater than zero");
       totalAmountWithdraw=totalAmountWithdraw.sub(_withdrawAmt);
       IERC20(wantAddress).safeTransferFrom(msg.sender,address(this), _withdrawAmt);
    ...
    function setGov(address _govAddress) public onlyGovernance {
      ...
      function setWithdrawPercentage(uint256 _withdrawPercentage) public onlyGovernance {
          require(_withdrawPercentage<Denominator, "Should be less");
        //   require(_withdrawPercentage<30,"should be less than 30");
        withdrawPercentage=_withdrawPercentage;
    }
