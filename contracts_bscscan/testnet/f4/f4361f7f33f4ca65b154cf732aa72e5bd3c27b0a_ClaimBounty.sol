...
contract ClaimBounty is Ownable, Pausable {
    ...
    constructor(address _erc1155, address  _treasury, address _erc20) public{
    ...
    function claim(uint256[] memory _nftId, uint256[] memory _amount) public {

        uint256 totalPrize = 0;
        for(uint i = 0 ; i <=_nftId.length ; i++){
            if(nftIdWithBonus == _nftId[i]){
                 totalPrize = totalPrize.add(nftIdToPrize[_nftId[i]].mul(weeklyBonusMultiplier));
            }else{
                 totalPrize = totalPrize.add(nftIdToPrize[_nftId[i]]);
            }
           
        }
        erc1155.burnBatch(msg.sender, _nftId, _amount);
        prizeErc20.safeTransferFrom(treasury, msg.sender, totalPrize);
    }
