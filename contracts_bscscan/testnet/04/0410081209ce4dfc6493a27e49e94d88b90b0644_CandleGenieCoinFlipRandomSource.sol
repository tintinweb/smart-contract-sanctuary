...
contract CandleGenieCoinFlipRandomSource is VRFConsumerBase, ReentrancyGuard, Ownable 
{
    ...
    // Brute Attack Shield
    bool bruteAttackSafeMode = true;
    uint8 public bruteAttackLevel = 2;
     
    uint8 public Level1BruteAttackArmTreshold; 
    uint8 public Level2BruteAttackArmTreshold = 3; 

    uint8 public Level1BruteAttackTimeTreshold;
        ...
        )
    {
        ...
        // Hard Coded Contract
        address gameAddress = 0x18324b762940262B0C40E1b76d3387a9262E8C3e;
        gameContract = CandleGenieCoinFlip(gameAddress);
    }
    ...
    function finalizeFlip(uint256 requestId, uint256 randomness, int flipResult) internal
    {
        ...
        // bot or hacker attacks. We have defined several rules that must any placed flip have to
        // pass it before count as valid one. These securtiy checks prevent contract from bankrupt
        // and malicious attacks by attackers.
        //----------------------------------------------------------------------------------------

        CandleGenieCoinFlip.Bet memory currentBet = gameContract.getBet(requestId);
        require(currentBet.betAmount != 0 ,"RandomSource: Bet not found !");
        
 
        // LEVEL 1 / REGULAR BRUTE MODE
        if (bruteAttackSafeMode && bruteAttackLevel == 1)
        {
        
            CandleGenieCoinFlip.Bet memory previousBet = gameContract.getBet(gameContract.getUserBetId(currentBet.user, gameContract.getUserBetsLength(currentBet.user) - 1));
                 
            // 1 - Multiple flips in same user same block [ILLEGAL]
            if (previousBet.betAmount != 0)
            {
                if (currentBet.flipTimestamp - previousBet.flipTimestamp <= Level1BruteAttackTimeTreshold)
                {
                    gameContract.Drop(uint256(requestId), revertResult(currentBet.guess));  
                    return;
                }
            }
            
            // 2 - N > Limit Continious Wins [ILLEGAL]
        ...
        // LEVEL 2 / HEAVY BRUTE MODE
        if (bruteAttackSafeMode && bruteAttackLevel == 2)
        {
            // Dicing Defense For Heavy Brute Attack
            uint256 pointer =  randomness % 10 + 1;
            for (uint i = 0; i < Level2BruteAttackArmTreshold; i++) 
            {
                uint256 dice =  randomness % 10 + 1;
                if (dice == pointer)
                {
                    gameContract.Drop(uint256(requestId), getFlipPosition(currentBet.guess));  
                    return;  
                }
            }
