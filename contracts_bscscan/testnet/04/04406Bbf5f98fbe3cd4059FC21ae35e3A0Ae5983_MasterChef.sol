...
contract HulkToken is BEP20('Hulk', 'HULK') {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    event UpdateEmissionRate(address indexed user, uint256 hulkPerBlock);
    ...
    function safeHulkTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > hulkBal) {
            transferSuccess = hulk.transfer(_to, hulkBal);
        } else {
            transferSuccess = hulk.transfer(_to, _amount);
        }
        require(transferSuccess, "safeHulkTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _hulkPerBlock) public onlyOwner {
        massUpdatePools();
        hulkPerBlock = _hulkPerBlock;
        emit UpdateEmissionRate(msg.sender, _hulkPerBlock);
    }
