...
import "../../v2/tokens/Seedmon.sol";
import "@openzeppelin/contracts/utils/Address.sol";
import "../../v1/tokens/SeedmonAttribute.sol";

contract SeedmonV2Minter is Ownable {
   
    Seedmon public seedmonV2;
    constructor(Seedmon _seedmonV2){
        seedmonV2 = _seedmonV2;
    }

    function processCreation(bytes32 seedmonName, uint256 dob, uint256[] memory stats) external onlyOwner{
        SeedmonAttribute.UniqueProfile memory unique = getUnique(dob);
        seedmonV2.mint(msg.sender, seedmonName, unique, getSeedmonStat(stats));
    }

    function getUnique(uint256 dob) internal pure returns (SeedmonAttribute.UniqueProfile memory) {
        return SeedmonAttribute.UniqueProfile({
            birthTimestamp: dob,
            fatherTokenId: 0,
            motherTokenId: 0,
            numberOfFused: 0,
    ...
    function getSeedmonStat(uint256[] memory bonusStats) internal pure returns (SeedmonAttribute.Stat memory){
        return SeedmonAttribute.Stat({
...
contract Seedmon is ERC721Enumerable, TrustCaller {
    ...
    function burn(uint256 tokenId) external {
        ...
        require(ERC721.ownerOf(tokenId) == _msgSender(), "Seedmon: burn of token that is not owner");
