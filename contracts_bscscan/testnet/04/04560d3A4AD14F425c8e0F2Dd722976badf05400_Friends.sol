...
contract Friends is Ownable {
    ...
    function takeInvestment(uint256 _packetId) public{
        ...
        uint256 _multiplier;
        if(block.timestamp > userPackets[msg.sender][_packetId].finishTime && lastUpdate[msg.sender][_packetId] > userPackets[msg.sender][_packetId].finishTime) {
            _multiplier = 1;
        }else if(block.timestamp < userPackets[msg.sender][_packetId].finishTime) {
            _multiplier = block.timestamp - lastUpdate[msg.sender][_packetId];
        } else {
            _multiplier = userPackets[msg.sender][_packetId].finishTime - lastUpdate[msg.sender][_packetId];
        }

        uint256 _earned = userPackets[msg.sender][_packetId].invested.mul(DAILY_REWARD).mul(_multiplier).div(120).div(100);
    ...
    function totalClaimable(uint256 _packetId, address _user) public view returns(uint256) {
        ...
        uint256 _multiplier;
        if(block.timestamp > userPackets[_user][_packetId].finishTime && lastUpdate[_user][_packetId] > userPackets[_user][_packetId].finishTime) {
            _multiplier = 1;
        }else if(block.timestamp < userPackets[_user][_packetId].finishTime) {
            _multiplier = block.timestamp - lastUpdate[_user][_packetId];
        } else {
            _multiplier = userPackets[_user][_packetId].finishTime - lastUpdate[_user][_packetId];
        }

        uint256 _reward = userPackets[_user][_packetId].invested.mul(DAILY_REWARD).mul(_multiplier).div(120).div(100);
    ...
    function getActivePackets(address _user) public view returns(Packet[] memory) {
        ...
        for(uint i=0; i<_allPackets.length; i++) {
            if(_allPackets[i].finishTime > block.timestamp || _allPackets[i].paid < (_allPackets[i].invested.mul(97).div(50))) {
                _size++;
            }
        }
