...
contract NFTStaking is ReentrancyGuard, DataStorage, Events, Ownable, Pausable {
    ...
    address public nft = 0x9ED64a7cd17CA9E166b088616703BADbB26E62E5;
    ...
    function deposit(uint8 poolId, uint256 _amount) external nonReentrant whenNotPaused {
    ...
    function depositNFT(uint8 poolId, uint256 _nftId) external nonReentrant whenNotPaused {
        require(poolId != 0, "Invalid plan");
    ...
    function _deposit(
        ...
        if(user.amount > 0) {
            uint256 currentDividends = IUltisProxy(iUltisProxy).getUserDividends(user.start, user.checkpoint, user.amount, pool.totalAmount, pool.rewardPerBlock, pool.toTime, TIME_STEP, user.nftId);
    ...
    function withdraw(uint256 poolId) external nonReentrant whenNotPaused {
        ...
        if (user.amount > 0) {
            ...
            if (block.timestamp < pool.toTime) {
                feeWithdraw = currentDividends.add(user.rewardDebt).mul(WIHDRAW_FEE).div(
                    PERCENTS_DIVIDER
                );
            }
            ...
            user.totalPayout = user.totalPayout.add(currentDividends).add(user.rewardDebt);
            user.rewardDebt = 0;
    ...
    function unStake(uint256 poolId) external nonReentrant whenNotPaused {
        ...
        UserInfo storage user = userInfos[poolId][_msgSender()];
        Pool storage pool = pools[poolId];
        if (user.isUnStake == false && user.amount > 0 && ((pool.locked && block.timestamp >= pool.toTime) || !pool.locked)) {
            user.isUnStake = true;
            uint256 currentDividends = IUltisProxy(iUltisProxy).getUserDividends(user.start, user.checkpoint, user.amount, pool.totalAmount, pool.rewardPerBlock, pool.toTime, TIME_STEP, user.nftId);
            IERC20(pool.tokenAddress).transfer(_msgSender(), user.amount);
            if (block.timestamp < pool.toTime) {
                feeUnlock = currentDividends.add(user.rewardDebt).mul(UNSTAKE_FEE).div(
                    PERCENTS_DIVIDER
                );
            }
            if (feeUnlock > 0) {
