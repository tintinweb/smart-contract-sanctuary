...
contract CoinToken is Context, IBEP20, Ownable {
    ...
    mapping (address => bool) private _isSigned;
    ...
    uint256 private immutable _GRANULARITY = 100;
    ...
    function addSigner(address account) external onlyOwner() {
        require(account != address(0x0) && !_isSigner[account], "Account is already added.");
        _isSigner[account] = true;
        signers.push(account);
    }

    modifier onlyUnlocked() {
        require(block.timestamp > _lockStartTime+ _lockDuration, "Locked");
        _;
    }

    function sign() external {
        require(_isSigner[msg.sender], "Account is not a signer.");
        _isSigned[msg.sender] = true;
    }
    
    function lock(uint256 durationInHours) onlyOwner external {
        uint count_signed = 0;
        for (uint i = 0; i < signers.length; i++) {
            if (_isSigned[signers[i]]) {
                count_signed++;
            }
        }

        require(signers.length != 0 && signers.length == count_signed, "All signers need to sign.");
        _lockStartTime = block.timestamp;
        _lockDuration = durationInHours* 3600;

        for (uint i = 0; i < signers.length; i++) {
            _isSigned[signers[i]] = false;
        }
    }
    ...
    function setAsCharityAccount(address account) external onlyOwner() onlyUnlocked {
	...
	function updateFee(uint256 _txFee,uint256 _burnFee,uint256 _charityFee) onlyOwner() external onlyUnlocked {
        _TAX_FEE = _txFee* 100; 
        _BURN_FEE = _burnFee * 100;
		_CHARITY_FEE = _charityFee* 100;
		ORIG_TAX_FEE = _TAX_FEE;
    ...
    function mint(address account, uint256 amount) onlyOwner() external onlyUnlocked {
        _tTotal = _tTotal.add(amount);
        if (_isExcluded[account])
            _tOwned[account] = _tOwned[account].add(amount);
        _rOwned[account] = _rOwned[account].add(amount.mul(_getRate()));
    ...
    function restoreAllFee() private onlyUnlocked {
