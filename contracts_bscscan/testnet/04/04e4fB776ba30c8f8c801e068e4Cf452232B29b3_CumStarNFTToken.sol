...
// File: contracts/src/token/CumStarNFTToken/CumStarNFTToken.sol
...
contract CumStarNFTToken is CumStarNFTTokenModel, TokenOwnable {
    address _stateContractAddress;

    /**
     * @dev Initializes CumStar NFT Token
     */
    ...
    function mint(address tokenOwner, string memory tokenUri) external onlyMinter returns (uint256) {
        require(tokenOwner != address(0), "Ownable: Token Owner Address is the Zero Address");

        uint256 tokenId = CumStarNFTTokenStateInterface(_stateContractAddress).mintToken(tokenOwner, tokenUri);
        ...
        CumStarNFTTokenStateInterface(_stateContractAddress).addNewNFTState(token);

        return token.tokenId;
    }

    function getNextTokenId() external onlyMinter returns (uint256) {
        return CumStarNFTTokenStateInterface(_stateContractAddress).getNextTokenId();
    }
}