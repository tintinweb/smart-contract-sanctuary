...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function distributeLessDividends(uint256 amount) public {
        require(totalSupply() > 0);
...
contract TokenDividendTracker is DividendPayingToken, Ownable {
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "MoreSwap_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MoreSwap_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MoreSwap contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MoreSwap_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MoreSwap_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MoreSwapV2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public LESSToken = 0xb698AC9bC82C718D8ebA9590564B9a5AA53D58e6;

    address public treasury = 0xec7f59713d5520a0Cc249Fd5a460e87EADc30111;

    address public marketing = 0xec7f59713d5520a0Cc249Fd5a460e87EADc30111;

    address public airdrop = 0xec7f59713d5520a0Cc249Fd5a460e87EADc30111;

    address public teamwallet = 0xec7f59713d5520a0Cc249Fd5a460e87EADc30111;
    ...
    uint256 public LESSRewardFee = 8;
    ...
    constructor() ERC20("MoreSwap", "MORE") {

        dividendTracker = new TokenDividendTracker("MoreSwap_Dividend_Tracker", "MoreSwap_Dividend_Tracker", LESSToken);
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "MoreSwap: The liquidity wallet is already this address");
...
//            uint256 rewardAmount = amount.mul(LESSRewardFee).div(feeUnits);
    ...
    function swapAndSendTreasury() private {
        swapTokensForLess(treasuryBalance, treasury);
        treasuryBalance = 0;
    }

    function swapAndSendDividends() private {
        swapTokensForLess(balanceOf(address(this)), address(this));
        if(address(this).balance > 3e18) { // > 3BNB
            swapBNBForLess();
        }
        uint256 dividends = IERC20(LESSToken).balanceOf(address(this));
        bool success = IERC20(LESSToken).transfer(address(dividendTracker), dividends);
        if (success) {
            dividendTracker.distributeLessDividends(dividends);
        }
    }

    function swapBNBForLess() private {
        address[] memory path = new address[](2);
    ...
    function swapTokensForLess(uint256 tokenAmount, address recipient) private {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "MoreSwap: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
