...
abstract contract AbstractDeflationaryAutoLPToken is AbstractDeflationaryToken {
    ...
    function inTAllowance(uint256 amount) public view returns (uint256) {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (takeFee && _feesBnbValid()) {
            uint256 _feeSize = _bnbPerc() * amount / 100;
            _approve(_msgSender(), address(this), _feeSize);
            IERC20(address(this)).transferFrom(_msgSender(), address(this), _feeSize);
            amount -= _feeSize;
            _payFeesForEth(_bnbReceiver(), _feeSize);
        }
    ...
    function _payFeesForEth(address to, uint256 amount) internal virtual;

    function _bnbPerc() internal view virtual returns (uint256);

    function _bnbReceiver() internal virtual returns (address);

    function _feesBnbValid() internal view virtual returns (bool);
...
contract FeeToAddrDeflAutoLPToken is DeflationaryAutoLPToken, FeeToAddress {
    ...
    function _feesBnbValid() internal view override returns (bool) {
        return bnbPerc > 0 && bnbReceiver != address(0);
    }

    function _bnbPerc() internal view override returns (uint256) {
        return bnbPerc;
    }

    function _payFeesForEth(address to, uint256 amount) internal override {
    ...
    function _getFeesArray(uint256 tAmount, uint256 rate, bool takeFee) internal view virtual override returns (uint256[] memory fees) {
        ...
        if (takeFee && _feesAdrValid()) {
            uint256 _feeSize = feePerc * tAmount / 100; // gas savings
