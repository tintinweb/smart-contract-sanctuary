...
contract BNBHMarket is Initializable, AccessControlUpgradeable, IERC721ReceiverUpgradeable {
    ...
    event NewListing(address seller, uint256 tokenId, uint256 bnbPrice, uint256 bnbhPrice);
    event ListingPriceChange(address seller, uint256 tokenId, uint256 bnbPrice, uint256 bnbhPrice);
    event CancelledListing(address seller, uint256 tokenId);
    event PurchaseListing(address buyer, address seller, uint256 tokenId, uint256 bnbPrice, uint256 bnbhPrice, uint256 tax);
    function initialize (IBEP20 _bnbhToken, IBNBHCharacter _characters) public initializer {
    ...
    function getCharacterListingIDsPage(uint256 _limit, uint256 _pageNumber, uint256 _minLevel, uint256 _maxLevel, uint256 rarity)
        public
        view
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < listedTokenIDs.length() && counter < pageEnd; i++) {
            (uint256 characterLevel, uint256 heroRarity) = characters.getLevelAndRarity(listedTokenIDs.at(i));
            if ((rarity == 0 || heroRarity == rarity) && characterLevel >= _minLevel && characterLevel <= _maxLevel){
    ...
    function getCharactersForPage(uint256 _limit, uint256 _pageNumber, uint256 _minLevel, uint256 _maxLevel, uint256 rarity) public view returns (MarketHero[] memory) {
        uint256[] memory tokenIds = getCharacterListingIDsPage(_limit, _pageNumber, _minLevel, _maxLevel, rarity);
    ...
    function getCharacterDataByIds(uint256[] memory tokenIds) public view returns (MarketHero[] memory){
        ...
        for (uint256 index = 0; index < tokenIds.length; index ++) {
            HeroLibrary.Hero memory hero = characters.getHero(tokenIds[index], false);
            Listing memory listing = listings[tokenIds[index]];
            hero.hp = characters.calcHpForFreeHeroes(tokenIds[index], block.timestamp - listing.listingTime);
            marketHeroes[index] = MarketHero(hero.name, hero.heroType, hero.xp, hero.attack, hero.armor, hero.speed, hero.hp, hero.tokenId, listings[tokenIds[index]].seller, listings[tokenIds[index]].price, hero.level, hero.heroClass);
        }
        return marketHeroes;
    }
        ...
        isNotListed(_id)
    {
        ...
        require(_price > 0.01 ether, "The price should be minimum 0.01 BNB");
        ...
        isSeller(_id)
    {
        require (maintenanceMode == false);
        require(bannedList[msg.sender] == false && isContract(msg.sender) == false);
        require(_newPrice >= 0.01 ether, "The price should be minimum 0.01 BNB");
    ...
    function purchaseListing(uint256 _id) public isListed(_id) {
        ...
        bnbhToken.transferFrom(msg.sender, listing.seller, amount.sub(tax));
        delete listings[_id];
        listedTokenIDs.remove(_id);
        emit PurchaseListing(msg.sender, listing.seller, _id, listing.price, amount, tax);
    }
