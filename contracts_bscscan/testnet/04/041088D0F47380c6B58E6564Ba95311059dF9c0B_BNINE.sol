...
// Thanks to support B9 (BNINE) – a community driven fork project for high reflection passive income strategy aiming auto distribution of BNB. 
// 
// The token applies a tax on every transaction so to achieve at least:
// 	9% goes to the liquidity pool;
// 	9% goes to the reward pool;
// 	2% fee to apply on sell transactions; Half of it will go to liquidity, other half to rewards.
// 
// Disclaimer: The crypto market carries an intense volatility and is a high risk environment; DYOR.
// 
// 99,999,999 Total Supply 
// 
// Name: B9
// Symbol: BNINE
// Decimals: 9
// 
// The contract is verified on BSC scan.

import "./BNINEBase.sol";


// Implements rewards & burns
contract BNINE is BNINEBase {

	// REWARD CYCLE
	uint256 private _rewardCyclePeriod = 10 minutes; // The duration of the reward cycle (e.g. can claim rewards every nine days)
	...
	uint256 private _globalRewardDampeningPercentage = 1; // Rewards are reduced by 3% at the start to fill the main BNB pool faster and ensure consistency in rewards
	...
	uint256 private _totalBNBAsBNINEClaimed; // The total number of BNB that was converted to BNINE and claimed by all addresses
	...
	uint256 private _gradualBurnTimespan = 1 hours; //Burn every 9 days by default
	uint256 private _lastBurnDate; //The last burn date
	uint256 private _minBnbPoolSizeBeforeBurn = 1 ether; //The minimum amount of BNB that need to be in the pool before initiating gradual burns
	...
	mapping(address => uint256) private _bnbAsBNINEClaimed; // The amount of BNB converted to BNINE and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy BNINE automatically
	...
	bool private _reimburseAfterBNINEClaimFailure; // If true, and BNINE reward claim portion fails, the portion will be given as BNB instead
	...
	// Calls the core class for BNINE
	constructor (address routerAddress) BNINEBase(routerAddress) {
		...
		// If someone sends or receives more than 9% of their balance in a transaction, their reward cycle date will increase accordingly
		setRewardCycleExtensionThreshold(9);
	}
	...
	function onActivated() internal override {
		...
		setReimburseAfterBNINEClaimFailure(true);
		setMinRewardBalance(999 * 10**decimals());  //At least 999 tokens are required to be eligible for rewards
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough BNINE balance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim BNINE tokens
		if (!claimBNINE(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterBNINEClaimFailure) {
	...
	function claimBNINE(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsBNINEClaimed[user] += bnbAmount;
		_totalBNBAsBNINEClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $BNINE on a zero balance address:
		...
		// Depending on the % of $BNINE tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsBNINE(address byAddress) public view returns (uint256) {
		return _bnbAsBNINEClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsBNINE() public view returns (uint256) {
		return _totalBNBAsBNINEClaimed;
	}
	...
	function setRewardCyclePeriod(uint256 period) public onlyOwner {
		require(period > 0 && period <= 9 days, "Value out of range");
	...
	// Will be used to exclude any pre-sale/private sales fees/token vesting addresses from rewards
	...
	function reimburseAfterBNINEClaimFailure() public view returns(bool) {
		return _reimburseAfterBNINEClaimFailure;
	}


	function setReimburseAfterBNINEClaimFailure(bool value) public onlyOwner {
		_reimburseAfterBNINEClaimFailure = value;
	}
...
abstract contract BNINEBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	// MAIN TOKEN PROPERTIES
	string private constant NAME = "B9";
	string private constant SYMBOL = "BNINE";
	...
	uint256 private constant _totalTokens = 99999999 * 10**DECIMALS;	//99,999,999 total supply
	...
	uint256 private _tokenSwapThreshold = _totalTokens / 1000000; //There should be at least 0.0001% of the total supply in the contract before triggering a swap
	...
	constructor (address routerAddress) {
		...
		// Initialize PancakeSwap V2 router and BNINE <-> BNB pair.  Router address will be: 0x10ed43c718714eb63d5aa57b78b54704e256024e or for testnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
		setPancakeSwapRouter(routerAddress);

		// Initially set values are 9% liquidity fee, 9% reward fee, 2% additional sell fee (adjustable range between 1% to 9%)
		setFees(9, 9, 2);
	...
	function activate() public onlyOwner {
		...
		setTransactionLimit(1000000); // only 0.01% of the total supply can be sold at once
		activateBuying();
		onActivated();
	}
	...
	// This function swaps a {tokenAmount} of BNINE tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for BNINE -> WBNB
	...
	// Returns how many more $BNINE tokens are needed in the contract before triggering a swap
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 additionalSellFee) public onlyOwner {
		require(liquidityFee >= 1 && liquidityFee <= 9, "Liquidity fee must be between 1% and 9%");
		require(rewardFee >= 1 && rewardFee <= 18, "Reward fee must be between 1% and 18%");
		require(additionalSellFee <= 9, "Additional sell fee cannot exceed 9%");
		require(liquidityFee + rewardFee + additionalSellFee <= 36, "Total fees cannot exceed 36%");
	...
	function setTransactionLimit(uint256 limit) public onlyOwner {
		require(limit >= 1 && limit <= 1000000, "Limit must be greater than 0.01%");
