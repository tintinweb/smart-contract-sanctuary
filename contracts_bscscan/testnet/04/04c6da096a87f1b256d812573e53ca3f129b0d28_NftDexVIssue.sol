 ...
 // events
  ...
  function claimAuction(uint256 _tokenId) public {
    ...
    if(_royality[_tokenId] > 0) {
      mainPerecentage = SafeMath.sub(SafeMath.sub(1000,SafeMath.mul(_royality[_tokenId],100)),platformPerecentage);
  ...
  function buyNFT(address nft_a,uint256 tokenId, address buyer) public returns (bool) {
        ...
        if(_royality[tokenId] > 0) {
          mainPerecentage = SafeMath.sub(SafeMath.sub(1000,SafeMath.mul(_royality[tokenId],100)),platformPerecentage); //50
