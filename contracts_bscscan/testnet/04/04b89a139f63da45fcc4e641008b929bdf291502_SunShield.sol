...
abstract contract SunShieldBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	string private constant SYMBOL = "SSHLD";
    ...
    uint8 private _moonshieldFee; //% of each transaction that will be used for Moonshield BNB pool
	uint8 private _poolFee; //The total fee to be taken and added to the pool, this includes all fees

	uint256 private constant _totalTokens = 2000000000000000 * 10**DECIMALS;	//2 quadrillion total supply
    ...
    address private _moonshieldAddress = 0xF565aaf0b8EB813a1c8C956D2C59F1ce27FD2366;
	...
	constructor (address routerAddress, address pinkAntiBot_) {
		...
		// Initialize PancakeSwap V2 router and SSHLD <-> BNB pair.  Router address will be: 0x10ed43c718714eb63d5aa57b78b54704e256024e or for testnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
		setPancakeSwapRouter(routerAddress);

		// 1% liquidity fee, 10% reward fee, 3% buyback fee, 2% marketing fee, 1% moonshield fee, 2% additional sell fee
		setFees(1, 10, 3, 2, 1, 2);
	...
	function executeSwap(uint256 amount) private {
		...
		uint256 tokensReservedForMoonshield = amount * _moonshieldFee / _poolFee;
		uint256 tokensReservedForReward = amount - tokensReservedForLiquidity - tokensReservedForBuyback - tokensReservedForMarketing - tokensReservedForMoonshield;
		...
		uint256 tokensToSwap = tokensReservedForReward + tokensToSwapForLiquidity + tokensReservedForBuyback + tokensReservedForMarketing + tokensReservedForMoonshield;
		...
		//send bnb to Moonshield BNB pool
		uint256 bnbToBeSendToMoonshield = bnbSwapped * tokensReservedForMoonshield / tokensToSwap;
		(sent, ) = _moonshieldAddress.call{value: bnbToBeSendToMoonshield}("");
		require(sent, "Failed to send BNB to Moonshield BNB pool");
	...
	// This function swaps a {tokenAmount} of SSHLD tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for SSHLD -> WBNB
	...
	// Returns how many more $SSHLD tokens are needed in the contract before triggering a swap
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 buybackFee, uint8 marketingFee, uint8 moonshieldFee, uint8 additionalSellFee) public onlyOwner {
        ...
        require(buybackFee >= 1 && buybackFee <= 5, "Buyback fee must be between 1% and 5%");
        require(marketingFee >= 1 && marketingFee <= 5, "Marketing fee must be between 1% and 5%");
        require(moonshieldFee >= 1 && moonshieldFee <= 5, "Moonshield fee must be between 1% and 5%");
		require(additionalSellFee <= 10, "Additional sell fee cannot exceed 5%");
        ...
        _moonshieldFee = moonshieldFee;
		_additionalSellFee = additionalSellFee;
		
		// Enforce invariant
		_poolFee = _rewardFee + _liquidityFee + _buybackFee + _marketingFee + _moonshieldFee;
	}
...
contract SunShield is SunShieldBase {
	...
	uint256 private _totalBNBAsSSHLDClaimed; // The total number of BNB that was converted to SSHLD and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsSSHLDClaimed; // The amount of BNB converted to SSHLD and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy SSHLD automatically
	...
	bool private _reimburseAfterSSHLDClaimFailure; // If true, and SSHLD reward claim portion fails, the portion will be given as BNB instead
	...
	constructor (address routerAddress, address pinkAntiBot_) SunShieldBase(routerAddress, pinkAntiBot_) {
	...
	function onActivated() internal override {
		...
		setReimburseAfterSSHLDClaimFailure(true);
		setMinRewardBalance(5000000000 * 10**decimals());  //At least 50 billion tokens are required to be eligible for rewards
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough SSHLD balance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim SSHLD tokens
		if (!claimSSHLD(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterSSHLDClaimFailure) {
	...
	function claimSSHLD(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsSSHLDClaimed[user] += bnbAmount;
		_totalBNBAsSSHLDClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $SSHLD on a zero balance address:
		...
		// Depending on the % of $SSHLD tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsSSHLD(address byAddress) public view returns (uint256) {
		return _bnbAsSSHLDClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsSSHLD() public view returns (uint256) {
		return _totalBNBAsSSHLDClaimed;
	}
	...
	function reimburseAfterSSHLDClaimFailure() public view returns(bool) {
		return _reimburseAfterSSHLDClaimFailure;
	}


	function setReimburseAfterSSHLDClaimFailure(bool value) public onlyOwner {
		_reimburseAfterSSHLDClaimFailure = value;
	}
