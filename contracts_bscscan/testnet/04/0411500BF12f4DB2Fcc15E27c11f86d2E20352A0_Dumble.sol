...
contract Dumble is ERC20, ERC20Burnable, AccessControl {
    ...
    bytes32 public constant ALLOW_LIST = keccak256("ALLOW_LIST");
    ...
    constructor(
        ...
        _mint(_treasury, 7777777 * (10**uint256(18)));
    ...
    function addExcluded(address _address) public virtual onlyRole(OWNER_ROLE) {
        _excluded(_address, true);
    }

    function removeExcluded(address _address)
        public
        virtual
        onlyRole(OWNER_ROLE)
    {
    ...
    function _excluded(address _address, bool _exclude) internal {
        _isExcludedFromFee[_address] = _exclude;
    }
    ...
    function _taxTransfer(
        ...
        if (isExcluded(sender)) return 0;
        if (isExcluded(recipient)) return 0;
