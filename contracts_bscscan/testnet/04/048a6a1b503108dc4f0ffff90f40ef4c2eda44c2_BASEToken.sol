...
BASE is a Hyper-deflationary token that rewards investors for holding tokens while also
...
contract BASEToken is Context, IERC20, Ownable {
    ...
    constructor (string memory _n, string memory _s, uint8 _d, uint256 _ts, uint256 _tax, uint256 _bb, uint256 _mkt, address _ma) {
        ...
        ecosystemAddress = payable(0xE48f45BdEc07D7305dEC77A726D54eC4353B0668);
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(ecosystemAddress, transferredBalance.div(_liquidityFee).mul(ecosystemFee));
        transferToAddressETH(marketingAddress, transferredBalance.div(_liquidityFee).mul(marketingFee.sub(ecosystemFee)));
    ...
    function setMarketingFee(uint256 _marketingFee) external onlyOwner() {
        require(_marketingFee >= ecosystemFee, 'Marketing fee can not be less than 5 percent');
