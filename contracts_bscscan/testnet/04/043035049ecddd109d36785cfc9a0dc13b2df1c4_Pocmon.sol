...
contract Pocmon is Context, IERC20, Ownable {
    ...
    uint256 private minimumTokensBeforeSwap = 10**6 * 10**9; 
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee > 10, 'You have reached fee limit');
        _taxFee = taxFee;
    }
    
    function setBurnFeePercent(uint256 burnFee) external onlyOwner() {
        require(burnFee > 10, 'You have reached fee limit');
        _burnFee = burnFee;
    }
    
    function setDonationFeePercent(uint256 DonationFee) external onlyOwner() {
        require(DonationFee > 10, 'You have reached fee limit');
        _donationFee = DonationFee;
    }

    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= 5 * 10**5 * 10**9 , 'maxTxAmount should be greater than 500000e9');
    ...
    function _setCharityWallet(address payable _charityAddress) external onlyOwner() {
        charityAddress = _charityAddress;
    }
