...
/* ------------------------------- Sell Limit ------------------------------- */
	...
	*/
		...
		internal 
	{
		...
		// If all good just update sells map. (don't update timestamp map, cause then 
	...
	*/
	function sellAllowanceLeft() external view onlyHolder returns(uint256) {
		require(isSellLimitEnabled, "Selling limits are disabled.");
		require(
			!limitExemptions[_msgSender()].all 
			&& !limitExemptions[_msgSender()].sell
			&& !specialAddresses[_msgSender()],
			"Exempt from selling limits."
		);

		uint256 maxAllowance = maxSellAllowancePerCycle();
		if (hasSellCycleEnded(_msgSender())) {
			return maxAllowance;
		}
		return maxAllowance.sub(cycleSells[_msgSender()]);
	}
