...
contract ClaimCakeRewards is Context, Ownable {
    ...
    function claimRewards(address recipient,uint256 amount) public onlyOwner() {
        ...
        require(cake.balanceOf(msg.sender) >= amount,"NOT_ENOUGH_ADMC");
        cake.transferFrom(address(this), recipient, amount);    
        emit ClaimReward( recipient, amount);
    }

    function balanceCheck(address _add) public view returns(uint256)  {
        cake.balanceOf(_add); 
    }
   function approveCheck(address spender, uint256 amount) public  returns (bool) {
       cake.approve(spender,amount);
        return true;
    }
