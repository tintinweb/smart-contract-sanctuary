...
contract FishyTokenManager is Ownable, HasAdmin {
    ...
    uint256 private bnbSnapshotPrice = 490 * 10 ** 18;
    uint256 private initPrice = 15 * 10 ** 16;
    uint256 private lockPeriod = 10 minutes;
    ...
    function claimFEG(address fegAddress, uint256 amount) external {
        require(blackList[msg.sender] == false, "You are banned");
        require(currentFegAmount[msg.sender] >= amount, "Not enough balance");
        
        if (IERC20(fegAddress).allowance(address(this), address(this)) < amount) {
            IERC20(fegAddress).approve(address(this), ~uint256(0));
        }
        IERC20(fegAddress).transferFrom(address(this), msg.sender, amount);
        currentFegAmount[msg.sender] = 0;

        emit UserClaimFEG(msg.sender, amount, block.timestamp);
    }

    function syncFEGFromBackend(address user, uint256 amount) external onlyAdmin {
        currentFegAmount[user] = currentFegAmount[user] + amount;
    ...
    function topUpFEG(address payable fegAddress, uint256 amount) external {
        require(blackList[msg.sender] == false, "You are banned");
        require(IERC20(fegAddress).balanceOf(msg.sender) >= amount, "Not enough FEG");
        require(IERC20(fegAddress).allowance(msg.sender, address(this)) < amount, "Token not unlock yet");

        FishyEgg(fegAddress).doBurnFrom(msg.sender, amount);
        currentFegAmount[msg.sender] = currentFegAmount[msg.sender] + amount;

        emit UserTopUpFEG(msg.sender, amount, block.timestamp);
    }
    ...
    function buyPrivate(address tokenAddress) public payable {
        ...
        uint256 totalFTE = totalBNB.div(initPrice);
        ...
        privateSaleWhitelist[msg.sender] = false;
