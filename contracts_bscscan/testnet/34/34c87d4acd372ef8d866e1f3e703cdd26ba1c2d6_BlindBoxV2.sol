...
interface IERC20 {
    ...
    function checkTokenOwnerIsArtist(uint256 tokenId) external returns(bool);
}
...
contract BlindBoxV2 {
  ...
  event AddAgent(string agentId, address _agentAddress, string artistId, uint256 _percentage,uint256 _companyPercentage, uint256 _artistPercentage);
 ...
 function init1() public {
    ...
    ALIA = IERC20(0x8D8108A9cFA5a669300074A602f36AF3252B7533);
    xanaliaDex = 0x4F77EA3cf1c328D0d51e58F3517acFc2260a1251;
    LPAlia=IERC20(0x52826ee949d3e1C3908F288B74b98742b262f3f1);
    LPBNB=IERC20(0xe230E414f3AC65854772cF24C061776A58893aC2);
    isInitialized1=true;
  }
  ...
  function buyBox(address seller, string memory ownerId, string memory name) public {
    ...
    randomValue = uint256(keccak256(abi.encodePacked(now, block.difficulty, msg.sender, num))) % Category[name].count;
  ...
  function addAgent(string memory agentId, address _agentAddress, string memory artistId, uint256 _percentage, uint256 _companyPercentage, uint256 _artistPercentage) adminAdd public {
    ...
    _artistDetails[artistId]._agents[ _artistDetails[artistId].count] = agent(agentId, _agentAddress, _percentage);
    if(_agentAddress == address(0x0)){
      _artistDetails[artistId]._agents[ _artistDetails[artistId].count]._address = NFTInterface(xanaliaDex).nonCryptoNFTVault();
    }
    ...
    emit AddAgent(agentId, _agentAddress, artistId, _percentage, _companyPercentage, _artistPercentage);
  }
  ...
  function addNonCryptoAuthor(string calldata artistId, uint256 tokenId, bool _isArtist) external returns(bool) {
    require(msg.sender == xanaliaDex);
      ...
      _artistDetails[artistId] = percentages( 970, _artistDetails[artistId].companyPercentage, _artistDetails[artistId].count);
      }
    return true;
  }
  ...
  function tranferAliaArtist(address buyer, uint256 price, address nftVaultAddress, uint256 tokenId ) external returns(bool) {
    ...
    ALIA.transferFrom(buyer, nftVaultAddress, (price * _artistDetails[artistId].artistPercentage)/1000);
      ALIA.transferFrom(buyer, companyAddress, (price * _artistDetails[artistId].companyPercentage)/1000);
      for(uint256 i =1; i <= _artistDetails[artistId].count; i++){
        ALIA.transferFrom(buyer, _artistDetails[artistId]._agents[i]._address, (price * _artistDetails[artistId]._agents[i].percentage)/1000);
        if(_artistDetails[artistId]._agents[i]._address == nftVaultAddress){
          NFTInterface(xanaliaDex).updateAliaBalance(_artistDetails[artistId]._agents[i].id,(price * _artistDetails[artistId]._agents[i].percentage)/1000);
        }
      }
    return true;
  }
  ...
  function getnonCryptoNFTVault() public view returns(address){
    return  NFTInterface(xanaliaDex).nonCryptoNFTVault();
