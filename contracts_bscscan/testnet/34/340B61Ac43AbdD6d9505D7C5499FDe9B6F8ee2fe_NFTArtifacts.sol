...
contract NFTArtifacts is Ownable, INFTArtifacts, INFTArtifactsMetadata {
    ...
    mapping(uint256 => uint256) private _artifactIdToType;
    ...
    function addArtifact(
        uint256 artifactId_,
        uint256 rarity_,
        uint256 artifactType_ // 1: characters, 2: karts, 3: items
    ) public onlyOwner {
        require(
        ...
        _artifactIdToType[artifactId_] = artifactType_;
        ...
        RarityInfo
            storage _rarityInfoForSpecificType = _artifactsForSpecificType
                .artifactIdToRarity[artifactId_];
    ...
    function getRandomArtifact(uint256 randomNumber_)
        public
        view
        returns (uint256 _artifactId, uint256 _artifactType)
    {
        require(
            _totalRarity > 0,
            "NFTArtifactsManager::getRandomArtifact: add actifacts before using function!"
        );
        ...
        for (uint256 i = 0; i < _artifactIds.length; i++) {
            ...
            if (
                ...
                _artifactType = _artifactIdToType[_artifactId];
                break;
            }
        }
    }
