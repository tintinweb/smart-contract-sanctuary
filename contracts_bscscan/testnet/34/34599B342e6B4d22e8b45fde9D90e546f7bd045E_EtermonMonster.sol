...
contract ERC721 is ERC165, IERC721, IERC721Metadata, IERC721Enumerable, BlackList {
    ...
    event BurnerAdded(address indexed burnerAddress);
    event BurnerRemoved(address indexed burnerAddress);

    bytes4 private constant _ERC721_RECEIVED = 0x150b7a02;

    function checkBurner(address paymentMethod) public view returns (bool) {
        return burner.has(paymentMethod);
    }

    function addBurner(address paymentMethod) public onlyOwner {
        burner.add(paymentMethod);
        emit BurnerAdded(paymentMethod);
    }

    function removeBurner(address paymentMethod) external onlyOwner {
        burner.remove(paymentMethod);
        emit BurnerRemoved(paymentMethod);
    }
    ...
    function _burn(uint256 tokenId) internal {
        ...
        require(_isApprovedOrOwner(msg.sender, tokenId) || checkBurner(msg.sender), "ERC721: burn caller is not owner nor approved");
        address owner = ownerOf(tokenId);
...
contract Etermon is ERC721, AccessRole {
...
// File: contracts/Etermon/EtermonMonster/EtermonMonster.sol

pragma solidity 0.8.0;


contract EtermonMonster is Etermon {
    constructor() Etermon("EtermonMonster","ETM"){
