  ...
  ReentrancyGuardUpgradeable
{
  ...
  // LP 1% on each transaction
  address public lpAddress;
  uint public lpTax; // 100 means 1%
   ...
   */
  function __WagyuPresenter_init(address _token, address _router) public initializer {
    __Ownable_init();
    __CoinDexTools_init();
    __ReentrancyGuard_init();
    token = _token;
    setRouter(_router);
    updateLPTax(100, true);
    // Exclude addresses
  ...
  function _taxCollector(uint _amount) internal returns (uint) {
    uint taxAmount_ = (_amount * lpTax) / _RATE_NOMINATOR;
    lpPending += taxAmount_;
    return taxAmount_;
  }
   ...
   */
  function updateLPTax(uint _lpTax, bool _isAutoLP) public onlyOwner {
    lpTax = _lpTax;
    isAutoLP = _isAutoLP;
  }
  ...
  function _solvePendingTaxWithBalance(uint _debt) private {
    ...
    if (lpPending > 0) {
      _swapAndLiquify(token, lpPending, owner());
      bal_ -= lpPending;
      lpPending = 0;
    }
  }
