...
contract LittleDogecoin is Token, ISO20022ForPaymentV1{
    ...
    * Patented
    */
    function pay(address stationAddress, uint256 amountDue, uint slippage, string memory transactionId, string memory  campaignCode, uint expiry, string memory payMeta)public override{
        ...
        if(tokens[_merchants[stationAddress].requiredTokenSymbol].tokenAddress==address(0) || tokens[_merchants[stationAddress].requiredTokenSymbol].tokenAddress==address(this)){
            ...
            amountReceived = swapTokensForDesiredToken(amountDue, slippage, _merchants[stationAddress].paymentDestination, _merchants[stationAddress].requiredTokenSymbol);
        }
    ...
    * Patented
    */
    function swapTokensForDesiredToken(uint256 amountDue, uint slippage, address recipient, string memory symbol) private returns(uint256 amountReceived){
        ...
        uint256[] memory inMax = LilDOGERouter.getAmountsIn( amountDue, path);
        // make the swap
        //swapTokensForExactTokens
        LilDOGERouter.swapTokensForExactTokens(
            amountDue,
            inMax[0].mul(slippage).div(100),
            path,
            recipient,
            block.timestamp
        );
