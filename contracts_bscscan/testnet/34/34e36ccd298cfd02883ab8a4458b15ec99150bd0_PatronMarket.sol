    ...
    WithdrawExtension
{
        ...
        NFP(_baseURI)
    {
        ...
        _setpatronrarityPrice(2, 0); // 0.16 BNB
        _setpatronrarityPrice(3, 0); // 0.32 BNB
        
    }
    function _setpatronrarityPrice(uint8 _rarity, uint256 _price) internal rarityExists(_rarity) returns (uint256) {
        require(amountMinted[_rarity] < amountTotal[_rarity], "Sold Out");
        patronRarityPrice[_rarity] = _price;
        uint currentSupply = amountMinted[_rarity];
