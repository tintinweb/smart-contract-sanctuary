...
// File: @pantherswap-libs\panther-swap-core\contracts\interfaces\IPantherFactory.sol
...
// File: @pantherswap-libs\panther-swap-core\contracts\interfaces\IPantherPair.sol
...
// File: contracts\libraries\PantherLibrary.sol
...
library PantherLibrary {
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"9097386609f20a042d957cdb9aed4221ed617c5a03826d6afd0c990f15d131a4" // init code hash
                    )
                )
            )
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IPantherPair(
            pairFor(factory, tokenA, tokenB)
        ).getReserves();
    ...
    function quote(
        ...
        require(
            ...
            "PantherLibrary: INSUFFICIENT_LIQUIDITY"
        );
...
// File: contracts\interfaces\ISafedexRouter.sol

pragma solidity >=0.6.2;

interface ISafedexRouter {
...
// File: contracts\SafedexRouter01.sol
...
contract SafedexRouter01 is ISafedexRouter {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = PantherLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = PantherLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "PantherRouter: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = PantherLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                ...
                require(
                    ...
                    "PantherRouter: INSUFFICIENT_A_AMOUNT"
                );
        ...
        returns (uint256 amountA, uint256 amountB)
    {
        ...
        (uint256 amount0, uint256 amount1) = IPantherPair(pair).burn(to);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? PantherLibrary.pairFor(factory, output, path[i + 2])
                : _to;
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            PantherLibrary.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        require(
            ...
            "PantherRouter: EXCESSIVE_INPUT_AMOUNT"
        );
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
