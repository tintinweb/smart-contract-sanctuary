...
/** 
 * Crypto: SafeEarn/EarnHub fork with integrated ERC-1363 (payable token) with improvements and additions (Whale timer & whale fee multiplier, reflection pool mechanism, bonuspay, merchant fee)
 */
interface IDividendDistributor {
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minPeriodCustom, uint256 _minDistribution, uint256 _minDistributionCustom) external;
...
contract DividendDistributor is IDividendDistributor {
    ...
    address TOK2 = 0x6E2c3779b281d0449009f08a3373d3e873aCd532;
    // bnb address
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minPeriodCustom, uint256 _minDistribution, uint256 _minDistributionCustom) external override onlyToken {
    ...
    function deposit(bool _enabledCustomRewards) external payable override onlyToken {
        if(_enabledCustomRewards) {
            uint256 balanceBefore = IERC20(TOK2).balanceOf(address(this));
            buyDepositTokens(TOK2);
            uint256 amount = IERC20(TOK2).balanceOf(address(this)).sub(balanceBefore);
 ...
 * Contract: Crypto 
 * 
 *  This ERC-1363 payable token was forked from SafeVault (who took it from SafeEarn/EarnHub - credit woofydev) with further tweaks / improvements to gas & further utility and functionality.
 *  Additional Whale Timer & Fee Multiplier Mechanism + Custom Transfer Fee + Custom Merchant Fee + Reward Pool Mechanism added by the Crypto team
 *  Payout dividends & earnings during transfer toggles, 
...
contract ERC20 is IERC20, Context, Ownable {
    ...
    address public SWAPPER; //Needs to be updated to start
    // token data
    ...
    uint256 private _megaPay                       = 0;                   // How many reflections are in the megapay pool
    uint256 private _buyBonus                      = 0;                   // How many reflections are in the bonus pool
    ...
    uint256 customTokenRewardBalance = 0; // Tracks Crypto reflections that will be converted to custom token
    uint256 tokenRewardBalance       = 0; // Tracks Crypto reflections that will be converted to default token
    // in charge of swapping from Crypto -> BNB to fund Crypto reflections + sustaining
    ...
    constructor (string memory name_, string memory symbol_) {
        ...
        // Liquidity Pool Address for BNB -> Crypto
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(_isTransfer){
            ...
            emit Transfer(sender, recipient, amountReceived);
            return true;
        } //Should SwapBack (swaps CRYPTO for BNB)?
        else if(shouldSwapBack(!_isTransfer, customRewardsEnabled)) { 
            swapBack(customRewardsEnabled);
        } //Should AutoBuyBack (buy & burn)?
        else if(!_isTransfer && !inSwap && autoBuybackEnabled && autoBuybackBlockLast + autoBuybackBlockPeriod <= block.number && address(this).balance >= autoBuybackAmount) { 
            triggerAutoBuyback(); 
        } //process as normal
        amountReceived = handleTransferBody(sender, recipient, amount);
        tryToProcess(customRewardsEnabled);
    ...
    function handleTransferBody(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        uint256 amountReceived; 
        if(recipient == SWAPPER || sender == SWAPPER) { //takes interface fees
                uint256 feeAmount = amount.mul(interfaceFee).div(feeDenominator);
                uint256 feePart = feeAmount.div(2);
                customTokenRewardBalance = customTokenRewardBalance.add(feePart);
                tokenRewardBalance = tokenRewardBalance.add(feeAmount.sub(feePart));
                _balances[address(this)] = _balances[address(this)].add(feeAmount);//remove pool allocations from total fee amount & add to contract balance
                amountReceived = amount.sub(feeAmount); //subtract total fee amount that was removed for taxes
        }
        else {
            amountReceived = !isFeeExempt[sender] ? takeFee(sender, recipient, amount) : amount; // !isFeeExempt[sender] == should take fee?
        }
    ...
    /** Basic Transfer with no swaps for BNB -> Crypto or Crypto -> BNB */
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        if(_enableMegaPay) {
            ...
            uint256 _buyBonusAdd = (receiver == pair) ? poolAllocation.div(2) : 0; //if it's a sell allocate 1/2 of the pool allocation for the buy bonus
            _buyBonus = _buyBonus.add(_buyBonusAdd);
        ...
        if((sender == pair)){
            if(enabledCustomRewards[receiver]) { customTokenRewardBalance = customTokenRewardBalance.add(_finalFeeAmount);//remove pool allocations from total fee amount & add to custom token balance
            }
    ...
    /** True if we should swap from Crypto => BNB, only swapsback during buy/sell to save transfer gas fees */
    function shouldSwapBack(bool _isBuyOrSell, bool customRewardsEnabled) internal view returns (bool) {
    ...
    /**
     *  Swaps Crypto for BNB if threshold is reached and the swap is enabled
    ...
    function swapBack(bool customRewardsEnabled) internal swapping {
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            {
                customTokenRewardBalance = customTokenRewardBalance.sub(amountToSwap);
            }
            else {
                tokenRewardBalance = tokenRewardBalance.sub(amountToSwap);
            }
        } catch{}
    ...
    function transferToDistributorAndSustaining(uint256 distributorBNB, uint256 sustainingBNB, bool customRewardsEnabled) internal {
        if (allowTransferToSustaining) {
            ...
            if(customRewardsEnabled) {
                try distributor.deposit{value: distributorBNB.add(sustainingBNB)}(customRewardsEnabled) {totalBNBTokenReflectionsCustom = totalBNBTokenReflectionsCustom.add(distributorBNB).add(sustainingBNB);} catch {}
            }
            else {
                try distributor.deposit{value: distributorBNB.add(sustainingBNB)}(customRewardsEnabled) {totalBNBTokenReflections = totalBNBTokenReflections.add(distributorBNB).add(sustainingBNB);} catch {}
            }
        }
    }
    ...
    /**
     * Buys Crypto with bnb in the contract and then sends to the dead wallet
     */ 
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minPeriodCustom, uint256 _minDistribution, uint256 _minDistributionCustom) external onlyOwner {
        distributor.setDistributionCriteria(_minPeriod, _minPeriodCustom, _minDistribution, _minDistributionCustom);
        emit DistributionCriteriaSet(_minPeriod, _minPeriodCustom, _minDistribution, _minDistributionCustom);
    }
    ...
    /**
     * Buy and Burn Crypto with bnb stored in contract
     */
    function triggerCryptoBuyback(uint256 amount) external onlyOwner {
        buyTokens(amount, DEAD);
        emit CryptoBuyBackAndBurn(amount);
    }
    ...
    function setTokenContractAddressRotating(address nToken) external onlyOwner {
        ...
        emit SwappedRotatingTokenAddress(nToken);
    }
    ...
    function switchReflections() external {
        ...
        distributor.setShare(msg.sender, 0, true, false); //removes from regular shareholder list
    ...
    function disableCustomReward() external {
        require(enabledCustomRewards[msg.sender], 'not currently enabled');
        require(!isDividendExempt[msg.sender], 'address is dividend exempt');
        enabledCustomRewards[msg.sender] = false;
        distributor.claimDividend(enabledCustomRewards[msg.sender]);
        distributor.setShare(msg.sender, 0, true, true); //removes from custom token shareholder list
    ...
    /** Returns the Circulating Supply of Crypto ( supply not owned by Burn Wallet ) */
    ...
    function inWhaleTimer(address account) external view returns (bool, uint256, uint) {
        return ((block.timestamp.sub(_timeSinceFirstSell[account]) > 0 && _amountSold[account] > _whaleSellThreshold), _amountSold[account], _timeSinceFirstSell[account]);
    }
    ...
    function getMegaPayPools() external view returns (uint256, uint256) {
        //Returns the total reflection pool value
        return (_megaPay, _buyBonus);
    }
    ...
    function getLastMegaPayWinnerSummary() external view returns (address, uint256, uint) {
    ...
    function getTransferSettings(address holder) external view returns (bool, bool) {
        return (enabledTransferDividends[holder],!disabledTransferEarnings[holder]);
    }
...
/*
    function contributeToRewardPool(uint256 megaPayAmount, uint256 bonusBuyAmount) external returns(bool){
        require(megaPayAmount > 0 || bonusBuyAmount > 0, 'Amount needs to be greater than 0 for one field');
        require(_balances[msg.sender] >= megaPayAmount.add(bonusBuyAmount), 'user does not own enough tokens');
        _balances[msg.sender] = _balances[msg.sender].sub(megaPayAmount, 'cannot have negative tokens').sub(bonusBuyAmount, 'cannot have negative tokens');
        _megaPay = _megaPay.add(megaPayAmount);
        _buyBonus = _buyBonus.add(bonusBuyAmount);
        emit ContributedToPool(msg.sender, megaPayAmount, bonusBuyAmount);
        return true;
    }
*/
    ...
    event DistributionCriteriaSet(uint256 _minPeriod, uint256 _minPeriodCustom, uint256 _minDistribution, uint256 _minDistributionCustom);
    ...
    event MegaPayEnabled(bool enablePool);
    ...
    event ContributedToPool(address sender, uint256 megaPayAmount, uint256 bonusBuyAmount);
    event AutoLiquify(uint256 amountBNB, uint256 amountSR);
    event CryptoBuyBackAndBurn(uint256 amountBNB);
    event SwappedTokenAddresses(address newToken);
    event SwappedRotatingTokenAddress(address newToken);
...
contract Crypto is ERC1363, TokenRecover {
    constructor ()
        ERC1363("Crypto", "CRYP")
    {
    }
