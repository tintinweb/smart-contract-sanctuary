...
contract GEMUNIMarketplace is IGEMUNIMarketplace, PermissionGroupUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function putOnSale(address token, uint tokenId, uint price, uint startTime, uint expirationTime) external override whenNotPaused {
        ...
        if(token == address(geniPass)) {
            IGENIPass.PassType passType = geniPass.getPass(tokenId).passType;
    ...
    function takeOffer(address token, uint tokenId, address buyer, uint takeOfferPrice) external override whenNotPaused nonReentrant {
        ...
        if(saleInfo.seller == address(0)) {
            require(msg.sender == geniPass.ownerOf(tokenId) || msg.sender == gemuniItem.ownerOf(tokenId), "GEMUNIMarketplace: not owner");
            require(buyer != msg.sender, "GEMUNIMarketplace: buyer can not take offer");
        } else {
    ...
    function withdrawNftEmergency(address token, uint tokenId, address recipient) external whenPaused onlyOperator {
        require(token == address(geniPass) || token == address(gemuniItem), "GENIMarketplace: invalid token");
        SaleInfo memory saleInfo = saleInfos[token][tokenId];
        ...
        } else if(token == address(gemuniItem)){
            gemuniItem.transferFrom(address(this), recipient, tokenId);
            emit withdrawNft(token, saleInfo.seller, recipient, tokenId);
        }
        delete saleInfos[token][tokenId];
    }
...
interface IGENIPassSale {
    ...
    event PassBoughtWithReferal(uint indexed passId, address buyer, address seller, address referal, uint nonce, uint256 price, uint discountedPrice, uint referalBonus);
    ...
    function purchaseWithReferal(uint passId, uint buyPrice, address referalAddress, uint nonce, bytes memory signature) external payable;
}
...
contract GENIPassSale is IGENIPassSale, IERC721Receiver, PermissionGroup, ReentrancyGuard, Pausable {
    ...
    uint public referalBonusLv0;
    uint public referalBonusLv1;
    uint public referalBonusLv2;
    uint public referalBonusLv3;
    uint public referalBonusLv4;
    ...
    constructor(
        ...
        address _busd,
        address _bnb, 
        uint _referalBonusLv0,
        uint _referalBonusLv1,
        uint _referalBonusLv2,
        uint _referalBonusLv3,
        uint _referalBonusLv4
    ) {
        geni = geniAddr;
        ...
        busd = _busd;
        bnb = _bnb;
        referalBonusLv0 = _referalBonusLv0;
        referalBonusLv1 = _referalBonusLv1;
        referalBonusLv2 = _referalBonusLv2;
        referalBonusLv3 = _referalBonusLv3;
        referalBonusLv4 = _referalBonusLv4;
    }
    ...
    function setReferalBonusLevel0(uint _rate) external onlyOperator {
        require(_rate <= 1000000, "GENIPassSale: percentage not greater than 100.0000 %");
        referalBonusLv0 = _rate;
    }

    function setReferalBonusLevel1(uint _rate) external onlyOperator {
        require(_rate <= 1000000, "GENIPassSale: percentage not greater than 100.0000 %");
        referalBonusLv1 = _rate;
    }

    function setReferalBonusLevel2(uint _rate) external onlyOperator {
        require(_rate <= 1000000, "GENIPassSale: percentage not greater than 100.0000 %");
        referalBonusLv2 = _rate;
    }

    function setReferalBonusLevel3(uint _rate) external onlyOperator {
        require(_rate <= 1000000, "GENIPassSale: percentage not greater than 100.0000 %");
        referalBonusLv3 = _rate;
    }

    function setReferalBonusLevel4(uint _rate) external onlyOperator {
        require(_rate <= 1000000, "GENIPassSale: percentage not greater than 100.0000 %");
        referalBonusLv4 = _rate;
    }
    ...
    function purchaseWithReferal(uint passId, uint buyPrice, address referalAddress, uint nonce, bytes memory signature) external payable override whenNotPaused nonReentrant {
        SaleInfo storage saleInfo = saleInfos[passId];
        ...
        address recoverAddress = VerifySignature.recoverSigner(VerifySignature.getEthSignedMessageHash(getMessageHash(referalAddress, nonce)), signature);
        ...
        uint passAmount = passCount[referalAddress][monthIndex];
        
        salePrice = payPrice * (1000000 -  getDiscountReferal(passAmount)) / decimalRate / 100;
        if (saleInfo.priceType == IGENIPass.PriceType.GENI) {
            geni.safeTransferFrom(buyer, treasury, salePrice);
            if(getDiscountReferal(passAmount) > 0){
        ...
        passCount[referalAddress][monthIndex] +=  1;
        emit PassBoughtWithReferal(passId, buyer, treasury, referalAddress, nonce, saleInfo.price, payPrice, payPrice - salePrice);
        delete saleInfos[passId];
    }
    ...
    function getDiscountReferal(uint amount) internal returns (uint referalBonus){
        if (amount >= 1 && amount <= 4)
            referalBonus = referalBonusLv0;
        else if (amount >= 5 && amount <= 30)
            referalBonus = referalBonusLv1;
        else if (amount >= 31 && amount <= 50)
            referalBonus = referalBonusLv2;
        else if (amount >= 51 && amount <= 100)
            referalBonus = referalBonusLv3;
        else if (amount > 100)
            referalBonus = referalBonusLv4;
    }
}
