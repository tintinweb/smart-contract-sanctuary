...
contract AceFARM is Ownable {

address _aceTokenAddress;
address _adminAddress;
uint256 _minConversion;
mapping(uint256 => bool) usedNonces;
event BuyAce(address _receiver, uint256 amount);

constructor(
        address token
    ) {
        _aceTokenAddress = token;
        _adminAddress = owner();
        _minConversion = 10 * 10**18;
    }

    function convertToAceToken(
        address _receiver,
        uint256 _amount,
        uint256 _nonce,
        ...
        require(IBEP20(_aceTokenAddress).transfer(_receiver, _amount));
        usedNonces[_nonce] = true;

        emit BuyAce(_receiver, _amount);
    }
    ...
    function setAceTokenAddress(address token) external onlyOwner {
        _aceTokenAddress = token;
    }
    ...
    function setAceTokenRate(uint256 amount) external onlyOwner {
