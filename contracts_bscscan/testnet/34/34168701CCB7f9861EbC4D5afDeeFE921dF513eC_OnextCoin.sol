...
contract TokenDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(string memory _name, string memory _symbol, address rewardToken) public DividendPayingToken(_name, _symbol, rewardToken) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1 * (10 ** 32); // must hold 0.0001%
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "OnextCoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "OnextCoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main OnextCoin contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "OnextCoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "OnextCoin_Dividend_Tracker: Cannot update claimWait to same value");
...
contract OnextCoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxSellTransactionAmount = 1 * (10 ** 35); // 1% of total supply - could be less
    ...
    uint256 public swapTokensAtAmount = 5 * (10 ** 31); //0.0005% set low so that swaps are often, seems okay in unit/volume tests
    ...
    constructor() ERC20("ONEXT", "$ONEXt") {

        dividendTracker = new TokenDividendTracker("OnextCoin_Dividend_Tracker", "OnextCoin_Dividend_Tracker", REWARDToken);
...
// add dividendtracker update function for future upgradeability & dApps
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "OnextCoin: The dividend tracker already has that address");

        TokenDividendTracker newDividendTracker = TokenDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "OnextCoin: The new dividend tracker must be owned by the OnextCoin token contract");
...
// add uniswap router upgrade function in case of pancakeswap update
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "OnextCoin: The router already has that address");
...
// add reward exclusion function for future dApps etc @dev note cannot re-add - will throw a gas error if address already added
    ...
    //require(dividendTracker.excludedFromDividends[_excludeRewardsAddr] != true, "OnextCoin: Account is already the value of 'excluded'");
...
// add fee exclusion function for future dApps etc
    ...
    require(_isExcludedFromFees[_excludeFeesAddr] != true, "OnextCoin: Account is already the value of 'excluded'");
        _isExcludedFromFees[_excludeFeesAddr] = true;
    }

// add fee inclusion function fo future dApps etc
    function includeInFees(address account) public onlyOwner {
    require(_isExcludedFromFees[account] != false, "OnextCoin: Account is already included / not 'excluded'");
...
// add max sell/transaction amount setting
    function setMaxSellTransactionAmount(uint256 _maxSellTransactionAmount) public onlyOwner() {
        require(maxSellTransactionAmount != _maxSellTransactionAmount, "OnextCoin: The entered amouont is already '_maxSellTransactionAmount'");
...
// add change reward token function
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "OnextCoin: The liquidity wallet is already this address");
...
//            uint256 rewardAmount = amount.mul(BUSDRewardFee).div(feeUnits);
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "OnextCoin: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
