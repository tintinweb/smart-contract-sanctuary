...
contract SWP {
    ...
    function approvePancake(address _token) public onlyOwnerOrSelf returns (bool _success) { 
    ...
    function buyToken(address[] memory path, uint256 _bnb) public onlyOwner returns (bool _success) {
    ...
    function checkWeights() public onlyOwnerOrSelf returns (bool _success) { 
        total = 0;
        for (uint i=0; i<assets.length; i++) {
            total += weights[assets[i]];
        }
        require(total <= 100, "Sum of weights exceeds 100 %");
        weights[WBNB] = 100 - total;
        return true;
    }
    ...
    function desiredHoldings(uint256 _AUM, uint16 _w, uint256 _amount0, uint256 _amount1) pure private returns (uint256 _desired) {
     ...
     // Re-balancing
    function rebalance(uint256 _AUM) public returns (bool _success) {
        uint b = 0;
        uint s = 0;
        uint256 amount;
        address[] memory path    = new address[](2);
        ...
        for (uint i=0; i<assets.length; i++) {
            ...
            if (_AUM==0) {
                ...
                if (amount > H) {
                    ...
                    delta = delta.mul(amounts[1]).div(amounts[0]); // (in units of WBNB)
