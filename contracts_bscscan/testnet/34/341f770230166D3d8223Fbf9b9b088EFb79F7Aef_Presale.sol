...
WBNB 0x47DDeDcad24FC3706D18E275D1fCBd59dBcb003b
IUniswapV2Router02 0x10ED43C718714eb63d5aA57B78B54704E256024E
*/


/* Testnet addresses
ATPAD 0xa3DdBD75CeAF723B375F620a3727Ca16bA21A7A6
Presale 0x18d4cBa77996B89ecEff0414fa6e7b4FE12839aE
StakePool 0x27e550f5589a546743F7692Aa1b75084C8EF8253

MYTOK 0x5D0dE7882155D9e7a3789b3Fe68D06813BA3dbd5

BNB 0x07e6F49DA68e29EF2FAA5bE1056301c863041a4a
TEST-WBNB (WBNB) 0x1024efDD8317773E50aF245177D5e99B3360d25B
  ...
  //  address immutable keeper;
    ...
    function totalInvestable(address _investor) public view returns (uint) {
        ...
        for (uint i = 0; i < tierCount; i++) {
            ...
            balance = balance.div(10**18);
            amount += hardCap.mul(allocPoint).div(totalAllocation).mul(balance).div(tierTotalAvailable);
        }

        return amount;
    }
    function calculatePriceOfTokens(address _investor) public view returns (uint) {
        uint tierCount = tiers.length;
        uint inWeiTotalSupply = totalSupply.div(10**18);
        uint userAmount;
       
        
        uint priceOfToken = inWeiTotalSupply.div(hardCap);
        ...
        if(balance < 0) continue;
         userAmount += tokensPerParticipants(i);
        }
        
        uint userPriceOfToken = userAmount.mul(priceOfToken);
        
  
        return userPriceOfToken;
    }
    ...
    function getParticipants()public view   returns (uint) {
         ...
         for(uint i = 0; i < tierCount; i++) {
             ...
             totalParticipants += stakePool.userCount(i);
    ...
    function calculateTotalWeight() public view  returns(uint){
        uint tierCount = tiers.length;
        uint userAllocation;
        ...
        for(uint i = 0; i < tierCount; i++ ) {
             uint tierTotalAvailable = tiers[i].available;
             uint users = stakePool.userCount(i);

            if (tierTotalAvailable == 0) continue;
            userAllocation += tiers[i].allocation.mul(users);
    ...
    function weightEquation() public view  returns(uint) {
        uint totalWeight = calculateTotalWeight();
        uint inWeiTotalSupply = totalSupply.div(10**18);
        
        return inWeiTotalSupply.div(totalWeight);
        
    }
    
    function poolEquation(uint _pid) public view  returns(uint) {
        

        
         
             
             return  stakePool.userCount(_pid).mul(tiers[_pid].allocation);
    ...
    function tokensPerTier(uint _pid) public view returns(uint) {
        
       return poolEquation(_pid).mul(weightEquation());
        
        
    } 
    
    function tokensPerParticipants(uint _pid) public view returns(uint) {
        
        
        uint tokenssPerTier = tokensPerTier(_pid);
        
        if(tokenssPerTier <= 0) {
            return 0;
        }   else{
            
            return tokenssPerTier.div(stakePool.userCount(_pid));
    ...
    // function calculate(address _investor) public view returns (uint) {
        
    //           uint tierCount = tiers.length;
    //           uint totalPoolWeight;
    //           uint totalParticipants;
    //           uint weightEquation;
    ...
    //     for(uint i = 0; i < tierCount; i++) {
    //          totalPoolWeight += tiers[i].allocation;
    //          totalParticipants  += stakePool.userCount(i);
    //          uint participants  = stakePool.userCount(i);
    //          uint poolWeight = tiers[i].allocation;
    //          uint funcPoolEquation = participants * poolWeight;
             
    //          uint tokensPerTier = funcPoolEquation.mul(weightEquation);
    //           uint tierTotalAvailable = tiers[i].available;
    //           if (tierTotalAvailable == 0) continue;
             
    //           uint tokensPerParticipants = tokensPerTier.div(participants);
    //           calculatePriceOfTokens(tokensPerParticipants);
            
    //     }
        
    //     uint poolEquation = totalParticipants * totalPoolWeight;
    //       weightEquation = totalSupply.div(poolEquation);
    ...
    function deposit(uint amount) external onlyKeeper whenNotStarted {
        require(amount > 0, "!amount not ok");
       ...
       function approveDeposit(uint amount) external onlyKeeper whenNotStarted {
        require(amount > 0, "!amount not ok");

       // uint before = wantToken.balanceOf(address(this));
       // wantToken.safeTransferFrom(msg.sender, address(this), amount);
        wantToken.approve(msg.sender, amount);
    }
    ...
    function setTotalSupply(uint amount) external onlyKeeper whenNotStarted {
        totalSupply = wantToken.balanceOf(address(this));
        if(totalSupply > 0) totalSupply = amount;
    }
