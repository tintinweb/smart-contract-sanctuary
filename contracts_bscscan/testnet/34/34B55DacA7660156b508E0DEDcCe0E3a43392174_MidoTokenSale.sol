...
contract MidoTokenSale is IMidoTokenSale, Whitelist {
    ...
    function withdrawTokenFunds(address recipient, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function updateSaleEndTime(uint64 _endTime) external onlyRole(DEFAULT_ADMIN_ROLE) {
...
contract TokenVestingLock is AccessControl  {
    ...
    // duration of the period in which the tokens will vest
    uint public constant VESTING_DURATION = 365 days;
    // total of token vesting in vesting Duration of each address
    mapping(address => uint) public totalTokenVesting;
    // amount of token vested of each address
    mapping(address => uint) public tokenVested;
    // the last time to claim to vest of  each address
    ...
    function isUnlockedTime() public view returns (bool) {
        return block.timestamp > unlockTime;
    }

    function setUnLockTime(uint _unlockTimestamp) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_unlockTimestamp > block.timestamp && _unlockTimestamp > unlockTime , "Cannot set an unlock time in past!");
        unlockTime = _unlockTimestamp;
    }

    function setTotalVesting(address user, uint64 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        totalTokenVesting[user]+=amount;
    }
    /**
    * @dev Calculates the amount that has already vested.
    * timeDiff is Time from the last claimed time until now
    * amountRemainingVesting is amount of remaining token unvesting
    * remainingVestingTime  is remaining time since the last claimed time
    */
    function vestedAmount(address user) internal returns (uint){
        uint timeDiff               = block.timestamp - lastClaimedTime[user];
        uint amountRemainingVesting = totalTokenVesting[user] - tokenVested[user];
        uint remainingVestingTime   = unlockTime + VESTING_DURATION - lastClaimedTime[user];
        uint _vestedAmount          = amountRemainingVesting/remainingVestingTime*timeDiff;
        lastClaimedTime[user]       = block.timestamp;
        tokenVested[user]          +=_vestedAmount;
...
import {
    TokenVestingLock
} from './utils/TokenVestingLock.sol';

contract MidoToken is ERC20, TokenVestingLock {
    
    uint256 public MAX_SUPPLY = 10000000 * 10 ** 18; // 1b
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant SALER_ROLE = keccak256("SALER_ROLE");
    ...
    function addSaler(address saler) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _setupRole(SALER_ROLE, saler);
    }

    /**
     * @dev Call to mint new tokens, only by Saler.
     *  Can mint up to maxSupply tokens.
    ...
    /**
     * @dev Call to claim token vested.
     */
    function claim(address account) external onlyRole(SALER_ROLE) {
        uint _vestedAmount = vestedAmount(account);
        transfer(account, _vestedAmount);
    }


    /**
     * @dev override function _transfer to check transfer is locked
     */
    function _transfer(address sender,address recipient,uint256 amount)
        internal virtual override
    {
        require(isUnlockedTime(), "Not Unlocked Yet!");
