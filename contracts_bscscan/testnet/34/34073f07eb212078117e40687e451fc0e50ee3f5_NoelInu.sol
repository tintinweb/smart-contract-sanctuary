...
contract NoelInu is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable private marketingAddress =
        payable(0x2f2Dafd27d8D74453D587740767c635C069acD86); // Marketing Address
                
    address payable private liquidityAddress =
        payable(0x2f2Dafd27d8D74453D587740767c635C069acD86); // Liquidity Address
    ...
    uint256 private constant _tTotal = 1e12 * 1e18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    bool public limitsInEffect = true;
    
    string private constant _name = "NoelInu";
    ...
    uint256 public _maxWalletSize = _tTotal * 2 / 100; // 2% maxWallet (only checked on buys)
            ...
            if (
                ...
                if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]) {
                    ...
                    require(
                        ...
                        "Exceeds maximum wallet token amount (2%)"
                    );                    
    ...
    function setBuyFee(uint256 buyMarketingFee, uint256 buyLiquidityFee)
        external
        onlyOwner
    {
        _buyLiquidityFee = buyLiquidityFee;
        _buyMarketingFee = buyMarketingFee;
        require(_buyLiquidityFee + _buyMarketingFee <= 20, "Must keep taxes below 20%");
        emit SetBuyFee(buyMarketingFee, buyLiquidityFee);
    }

    function setSellFee(uint256 sellMarketingFee, uint256 sellLiquidityFee)
        external
        onlyOwner
    {
        _sellLiquidityFee = sellLiquidityFee;
        _sellMarketingFee = sellMarketingFee;
        require(_sellLiquidityFee + _sellMarketingFee <= 20, "Must keep taxes below 20%");
