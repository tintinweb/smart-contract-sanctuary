...
contract TimelockController_RewardsDistributor is AccessControl, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    address payable public AUTOStakingAddress;
    address public wbnbAddress;

    uint256 public AUTOStakingRewardsFactor; // X/10,000 of rewards go to AUTO staking vault. The rest goes to treasury.
     ...
     * @dev Emitted when a call is scheduled as part of operation `id`.
     */
    event SetScheduled(bytes32 indexed id, uint256 indexed index, uint256 _pid, uint256 _allocPoint, bool _withUpdate, bytes32 predecessor, uint256 delay);
    ...
    event SetScheduled(bytes32 indexed id, uint256 indexed index, address target, uint256 value, bytes data, bytes32 predecessor, uint256 delay);

    event AUTOStakingRewardsFactorChange(uint256 oldAUTOStakingRewardsFactor, uint256 newAUTOStakingRewardsFactor);
    ...
    constructor(
        ...
        address payable _AUTOStakingAddress,
        uint256 _AUTOStakingRewardsFactor,
        uint256 _minDelay,
        ...
        AUTOStakingAddress = _AUTOStakingAddress;
        AUTOStakingRewardsFactor = _AUTOStakingRewardsFactor;
        minDelay = _minDelay;
    ...
    function setAUTOStakingRewardsFactor(uint256 newAUTOStakingRewardsFactor) external virtual {
        require(msg.sender == address(this), "TimelockController: caller must be timelock");
        require(newAUTOStakingRewardsFactor <= 1000, "newAUTOStakingRewardsFactor > 1000");
        emit AUTOStakingRewardsFactorChange(AUTOStakingRewardsFactor, newAUTOStakingRewardsFactor);
        AUTOStakingRewardsFactor = newAUTOStakingRewardsFactor;
    }
    ...
    function scheduleSet(
        ...
        bytes32 id = keccak256(abi.encode(_autofarmAddress, _pid, _allocPoint, _withUpdate, predecessor, salt));

        require(_timestamps[id] == 0, "TimelockController: operation already scheduled");

        _timestamps[id] = SafeMath.add(block.timestamp, minDelayReduced);
        emit SetScheduled(id, 0, _pid, _allocPoint, _withUpdate, predecessor, minDelayReduced);
    }
     ...
     */
    function setTreasuryAddress(address payable _treasuryAddress) public onlyRole(EXECUTOR_ROLE) {
        treasuryAddress = _treasuryAddress;
    }

    // function setAUTOStaking(address payable _AUTOStakingAddress, uint256 _AUTOStakingRewardsFactor) public onlyRole(EXECUTOR_ROLE) {
    //     AUTOStakingAddress = _AUTOStakingAddress;
    //     AUTOStakingRewardsFactor = _AUTOStakingRewardsFactor;
    // }
    ...
    function leverageOnce(address _stratAddress) public onlyRole(EXECUTOR_ROLE) {
    ...
    // // In case new vaults require functions without a timelock as well, hoping to avoid having multiple timelock contracts
    function noTimeLockFunc1(address _stratAddress) public onlyRole(EXECUTOR_ROLE) {
        IStrategy(_stratAddress).noTimeLockFunc1();
    }

    function noTimeLockFunc2(address _stratAddress) public onlyRole(EXECUTOR_ROLE) {
        IStrategy(_stratAddress).noTimeLockFunc2();
    }

    function noTimeLockFunc3(address _stratAddress) public onlyRole(EXECUTOR_ROLE) {
        IStrategy(_stratAddress).noTimeLockFunc3();
    }

    function convertTokenToWBNB(
        ...
        uint256 tokenStartAmt = IERC20(tokenStartAddress).balanceOf(address(this));
        if (tokenStartAmt > 0) {
            IERC20(tokenStartAddress).safeApprove(_uniRouterAddress, 0);
            IERC20(tokenStartAddress).safeIncreaseAllowance(_uniRouterAddress, tokenStartAmt);

            // Swap all tokens to tokens
            _safeSwap(_uniRouterAddress, tokenStartAmt, _slippageFactor, _path, address(this), block.timestamp.add(600));
        }
    }

    function distributeRewards() public onlyRole(EXECUTOR_ROLE) returns (uint256) {
        _wrapBNB();

        uint256 rewardsAmt = IERC20(wbnbAddress).balanceOf(address(this));

        if (AUTOStakingRewardsFactor > 0) {
            uint256 AUTOStakingRewards = rewardsAmt.mul(AUTOStakingRewardsFactor).div(1000);
            IERC20(wbnbAddress).safeTransfer(AUTOStakingAddress, AUTOStakingRewards);
            rewardsAmt = rewardsAmt.sub(AUTOStakingRewards);
        }

        IERC20(wbnbAddress).safeTransfer(treasuryAddress, rewardsAmt);
    }
