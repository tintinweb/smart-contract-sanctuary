...
contract Star_Cattle is ERC721Enumerable, Ownable {
    ...
    function mintNormall(address player, uint gender_, uint life_, uint energy_, uint growth_,uint star, uint[2] memory parents) public {
        ...
        cowInfoes[currentId].growth = growth_;
        cowInfoes[currentId].star = star;
        cowInfoes[currentId].parents = parents;
        deadTime[currentId] = block.timestamp + maxLife * life_ / 100;
        if (cowInfoes[currentId].gender == 1) {
            ...
            emit BornNormalBull(player, currentId, cowInfoes[currentId].life, cowInfoes[currentId].energy, cowInfoes[currentId].growth,star, parents, deadTime[currentId], cowInfoes[currentId].attack, cowInfoes[currentId].defense, cowInfoes[currentId].stamina);


        } else {
            cowInfoes[currentId].milk = 60 + rand(26);
            cowInfoes[currentId].milkRate = 60 + rand(26);
            emit BornNormalCow(player, currentId, cowInfoes[currentId].life, cowInfoes[currentId].energy, cowInfoes[currentId].growth,star, parents, deadTime[currentId], cowInfoes[currentId].milk, cowInfoes[currentId].milkRate);
        }
        currentId ++;
    ...
    function mint(address player) public {
        ...
        cowInfoes[currentId].growth = 86 + rand(14);
        cowInfoes[currentId].checkTime = block.timestamp;
        cowInfoes[currentId].isAdult = true;
        deadTime[currentId] = block.timestamp + 36000 days;
        if (cowInfoes[currentId].gender == 1) {
            cowInfoes[currentId].attack = 86 + rand(14);
            cowInfoes[currentId].stamina = 86 + rand(14);
            cowInfoes[currentId].defense = 86 + rand(14);
            maleCreation++;
            emit BornCreationBull(player, currentId, maleCreation,cowInfoes[currentId].life, cowInfoes[currentId].energy, cowInfoes[currentId].growth,deadTime[currentId], cowInfoes[currentId].attack, cowInfoes[currentId].defense, cowInfoes[currentId].stamina);

        } else {
            cowInfoes[currentId].milk = 86 + rand(14);
            cowInfoes[currentId].milkRate = 86 + rand(14);
            femaleCreation ++;
            emit BornCreationCow(player, currentId, femaleCreation, cowInfoes[currentId].life, cowInfoes[currentId].energy, cowInfoes[currentId].growth,deadTime[currentId], cowInfoes[currentId].milk, cowInfoes[currentId].milkRate);
        }
