...
contract newBank {
    ...
    function onoff(bool start ) public onlyCmo() {
        _isStart =  start;
    }
    ...
    function inject(uint256 reward) public onlyCmo() {
        _balance += reward;
        _supportToken.transferFrom(msg.sender,address(this),reward);
    }
    ...
    function withdrawtest(uint256 index) public {
       ...
       // require(block.timestamp >( _user[msg.sender][index].startTime + (_user[msg.sender][index].period).mul(86400) ));
        ...
        _supportToken.transfer(msg.sender,amount);
    ...
    function getRate(uint256 period) public view returns(uint256 rate){
        return _rate[period];
    }
