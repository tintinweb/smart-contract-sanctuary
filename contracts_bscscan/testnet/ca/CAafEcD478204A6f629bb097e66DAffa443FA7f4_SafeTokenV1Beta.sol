    ...
    ReentrancyGuard
{
    ...
    function addNftBeats(
        ...
        require(
            _nftToken.ownerOf(_tokenId) == _msgSender,
            "you don't own this NFT token"
        );

        token2value[_msgSender][_tokenId] = _amount;
    ...
    function createVestingSchedule(
        ...
        uint256 _tokenId,
        uint256 _duration, // track duration in seconds
        uint256 _amount
        ...
        require(!isBeneficiaryCurrentlyVesting(_beneficiary),
        'beneficiary is already vesting');

        require(_amount > 0, "TokenVesting: amount must be > 0");
        bytes32 vestingScheduleId = this.computeNextVestingScheduleIdForHolder(
            _beneficiary
        );
        //require(vestingSchedules[vestingScheduleId])

        uint256 _vestingDurationInSeconds = _duration * 1 seconds;
        ...
        emit ScheduleAdded(_beneficiary, vestingScheduleId, _amount);
        ...
        onlyIfVestingScheduleNotRevoked(vestingScheduleId)
    {
        ...
        uint256 PaidNftListeningValue = token2value[vestingSchedule.beneficiary][vestingSchedule.tokenId];
        ...
        uint256 vestedAmount = _computeReleasableAmount(vestingSchedule, PaidNftListeningValue);
        require(
            vestedAmount >= amount,
        ...
        token2value[vestingSchedule.beneficiary][vestingSchedule.tokenId].sub(amount);
        ...
        returns (uint256)
    {
        ...
        return _computeReleasableAmount(vestingSchedule, vestingSchedule.tokenId);
    }
     ...
     */
    function _computeReleasableAmount(VestingSchedule memory vestingSchedule, uint256 paidNftValue)
        internal
        view
        returns (uint256)
    {
        ...
        if (vestingSchedule.revoked == true) {
            ...
            if(currentTime > vestingSchedule.start + vestingSchedule.duration){
                ...
                if(paidNftValue > vestingSchedule.amountTotal){
