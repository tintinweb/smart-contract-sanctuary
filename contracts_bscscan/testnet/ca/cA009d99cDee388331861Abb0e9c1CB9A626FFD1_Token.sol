...
contract Token is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    constructor (address payable _walletAaddress,address payable _marketWallet,address payable _Bwallet) {
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_taxFee.add(_liquidityFee).add(WalletAfee).add(_marketFee).add(walletBfee)).div(1e3);
        return percentage;
    }
