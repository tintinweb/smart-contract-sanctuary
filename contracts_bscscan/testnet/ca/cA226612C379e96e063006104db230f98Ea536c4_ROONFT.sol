...
* 主合约
*/
contract ROONFT is ERC721Pausable, AccessControl, Ownable {
    ...
    // token编号
    mapping(uint256 => string) private _tokenNumbers;

    event Burn(address indexed sender, uint256 tokenId);
    event FeeERC20AddressTransferred(address indexed previousERC20, address indexed newERC20);
    ...
    constructor(
        ...
        address _feeERC20Address,
        address payable _mintFeeAddr,
        uint256 _mintFeeAmount
    ) public ERC721(name, symbol) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(UPDATE_TOKEN_URI_ROLE, _msgSender());
        _setupRole(PAUSED_ROLE, _msgSender());
        feeERC20Address = _feeERC20Address;
        quoteErc20 = IERC20(_feeERC20Address);
    ...
    function mint(address to, string memory _tokenURI) public payable returns (uint256 tokenId) {
        ...
        if(feeERC20Address == address(0)){
            ...
            mintFeeAddr.transfer(mintFeeAmount);
            _msgSender().transfer(msg.value - mintFeeAmount);
        }else{
            quoteErc20 = IERC20(feeERC20Address);
            require(quoteErc20.balanceOf(_msgSender()) >= mintFeeAmount, 'your price is too low');
            if (mintFeeAmount != 0) {
                quoteErc20.transferFrom(_msgSender() ,mintFeeAddr, mintFeeAmount);
            }
        }
        ...
        setTokenNumber(tokenId, uint2str(tokenId));
    }

    // 获取用户所有 NFT列表
    function tokensOfOwner(address owner) public view returns (uint256[] memory){
        uint256 _length = balanceOf(owner);
        uint256[] memory tokens = new uint256[](_length);
        if (_length > 0) {
    ...
    function tokenNumber(uint256 tokenId) public view returns (string memory){
        return _tokenNumbers[tokenId];
    }

    function setTokenNumber(uint256 tokenId, string memory _tokenNumber) public {
        require(hasRole(UPDATE_TOKEN_URI_ROLE, _msgSender()), 'Must have update token uri role');
        _tokenNumbers[tokenId] = _tokenNumber;
    }
    ...
    function transferFeeERC20Address(address _feeERC20Address) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), 'Must have admin role');
        address previous = feeERC20Address;
        feeERC20Address = _feeERC20Address;
        quoteErc20 = IERC20(_feeERC20Address);
        emit FeeERC20AddressTransferred(previous, _feeERC20Address);
    }

    function transferMintFeeAddress(address payable _mintFeeAddr) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), 'Must have admin role');
        mintFeeAddr = _mintFeeAddr;
