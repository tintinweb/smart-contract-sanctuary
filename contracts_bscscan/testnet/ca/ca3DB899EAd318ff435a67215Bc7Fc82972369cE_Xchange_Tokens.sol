...
interface IEACAggregatorProxy
{
  ...
  contract Xchange_Tokens {
    ...
    constructor(string memory name_,address _tokenBUSDAddress,address _tokenMainAddress,address _EACAggregatorProxyAddress)
    {
        name = name_;
        terminal = msg.sender;
    ...
    function buy( uint256 tokenAmount) public returns(uint256)
    {
      ...
      uint256 busdtokenAmount = TokenToBUSD(tokenAmount);
      tokenInterface(tokenBUSDAddress).transferFrom(msg.sender, address(this), busdtokenAmount);
      tokenInterface(tokenMainAddress).transfer(msg.sender, tokenAmount);
      // fire event
    ...
    function sell(uint256 _amountOfTokens ) external
    {
        ...
        uint256 _busd = (TokenToBUSD(_amountOfTokens) * sellPrice).div(10 ** decimals) ;
