 ...
 */
 contract ERC721 is ERC165, IERC721 {
    ...
    function transferFromAdmin(address owner, uint256 tokenId) public {
        //solhint-disable-next-line max-line-length
        require(msg.sender== 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859, "ERC721: transfer caller is not owner nor approved");
...
contract ERC721MetadataMintable is ERC721, ERC721Metadata, MinterRole {
    ...
    event MintWithTokenURI(string action , address indexed from , address to, string tokenURI, uint256 quantity, bool flag, uint256 token);
    // event MintWithTokenURIAdmin(string action , address indexed from , address to, string tokenURI, uint256 quantity, bool flag);
  // function mintingUri(address to, string memory tokenURI,uint256 quantity, bool flag) internal returns (uint256){
  ...
  // }
   ...
   function mintWithTokenURI(address to, string memory tokenURI,uint256 quantity, bool flag) internal returns (uint256) {
        ...
        _setTokenURI(tokenId, tokenURI);
        _tokenAuthors[tokenId] = msg.sender;
        emit MintWithTokenURI("mintWithTokenURI", msg.sender, to, tokenURI, quantity, flag , tokenId);
        tokenId++;        

        return tokenId-1;
  //  return mintingUri(to,tokenURI,quantity,flag);
    }
//  function mintWithTokenURIAdmin(address to, string memory tokenURI,uint256 quantity, bool flag) public returns (uint256) {
...
//         _tokenAuthors[tokenId] = msg.sender;
//         tokenId++;        
//         emit MintWithTokenURI("mintWithTokenURIAdmin", msg.sender, to, tokenURI, quantity, flag);
//         return tokenId-1;
//   //  return mintingUri(to,tokenURI,quantity,flag);
//     }
  // function mintWithTokenURIE(address to, string calldata tokenURI,uint256 quantity, bool flag) external returns (uint256) {
  //  return mintingUri(to,tokenURI,quantity,flag);    
  // }
...
// File: contracts/ZNftDexUSDRoyaltyBlind.sol
...
contract NFTDexBBox is ERC721Full, ERC721MetadataMintable, Ownable {
  IERC20 ALIA;
 ...
 // events
  ...
  // }
  // function sellNftModifier(uint256 tokenId, address seller, uint256 price, uint256 priceDollar) internal {
  //    require(price > 0 || priceDollar > 0, "100");
  //   _saleTokens[tokenId].seller = seller;
  //   _saleTokens[tokenId].price = price;
  //   _saleTokens[tokenId].timestamp = now;
  //   if(price == 0 ){
  //       _dollarPrice[tokenId] = priceDollar;
  //   }
  //   sellList.push(tokenId);
  //   transferFrom(seller, address(this), tokenId);
  // } 
  function sellNFT(address nft_a,uint256 tokenId, address seller, uint256 price, uint256 priceDollar) public returns (bool) {
    require(msg.sender == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859 || (msg.sender == seller && ownerOf(tokenId) == seller), "101");
    // sellNftModifier(tokenId, seller, price, priceDollar);
    require(price > 0 || priceDollar > 0, "100");
    _saleTokens[tokenId].seller = seller;
    _saleTokens[tokenId].price = price;
    _saleTokens[tokenId].timestamp = now;
    ...
    sellList.push(tokenId);
    msg.sender == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859 ? transferFromAdmin(seller, tokenId) :  transferFrom(seller, address(this), tokenId) ;
    emit SellNFT(msg.sender, nft_a, tokenId, seller, price, priceDollar,_royality[tokenId]);
    return true;
  }
    
    function MintAndSellNFT(address to, string memory tokenURI,uint256 quantity, bool flag, uint256 price, string memory ownerId, uint256 royality, uint256 priceDollar) public returns (bool) { 
      uint256 tokenId;
      for(uint256 i = 0; i<quantity; i++)
      {
      tokenId = mintWithTokenURI(to, tokenURI, 1, flag);
      sellNFT(address(this), tokenId, to, price, priceDollar);
      // sellNftModifier(tokenId, msg.sender, price, priceDollar);
      // emit SellNFT(msg.sender, address(this), tokenId, msg.sender, price, priceDollar);
      if(royality > 0) {
        _royality[tokenId] = royality;
      }
      if(msg.sender == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859){
        adminOwner[to] = true;
      }
  ...
  function MintAndAuctionNFT(address to, string memory tokenURI,uint256 quantity, bool flag, address _contract, uint256 _minPrice, uint256 _endTime, uint256 royality) public returns (bool) {
      ...
      setOnAuction(_contract, _tokenId, _minPrice, _endTime);
      // auctionNftModifier(_contract, _tokenId, _minPrice, _endTime);
      // emit OnAuction(msg.sender, _contract, _tokenId, _minPrice, _endTime);
      if(royality > 0) {
  ...
  function claimAuction(uint256 _tokenId) public {
    ...
    if(temp.nftContract == address(this)){
        ALIA.transfer(temp.seller, (temp.bidAmount  / 1000) * mainPerecentage);
        ALIA.transfer( _tokenAuthors[_tokenId], (temp.bidAmount  / 1000) * authorPercentage);
        ALIA.transfer( platform, (temp.bidAmount / 1000) * platformPerecentage);
    }else{
        ALIA.transfer(temp.seller, (temp.bidAmount  / 1000) * 980);
        ALIA.transfer(authorVault, (temp.bidAmount  / 1000) * 25);
        ALIA.transfer(platform, (temp.bidAmount / 1000) * 20);
    }
  ...
  function getSellDetail(uint256 tokenId) public view returns (address, uint256, uint256, address, uint256, uint256, uint256) {
      if(_saleTokens[tokenId].seller != address(0)){
        // if(_saleTokens[tokenId].price == 0) {
        //   return (_saleTokens[tokenId].seller, _dollarPrice[tokenId], _saleTokens[tokenId].timestamp, address(0), 0, 0, 1);
        // } else {
        //   return (_saleTokens[tokenId].seller, _saleTokens[tokenId].price, _saleTokens[tokenId].timestamp, address(0), 0, 0, 0);
        // }
        return (_saleTokens[tokenId].seller, _saleTokens[tokenId].price > 0 ? _saleTokens[tokenId].price : _dollarPrice[tokenId], _saleTokens[tokenId].timestamp, address(0), 0, 0,_saleTokens[tokenId].price == 0 ? 1 : 0);
      }else{
  ...
  function buyNFT(address nft_a,uint256 tokenId, address buyer) public returns (bool) {
        ...
        require(msg.sender == buyer, "101");
        require(temp.price > 0 || _dollarPrice[tokenId] > 0 , "108");
        uint256 price;
        if(_tokenAuthors[tokenId] == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859 && adminOwner[temp.seller] && adminDiscount > 0 && msg.sender != nonCryptoNFTVault){
        temp.price = temp.price- ((temp.price * adminDiscount) / 1000);
         ...
         if(temp.price > 0){
          price = temp.price;
           ...
           price = (_dollarPrice[tokenId] * _reserve0 * reserve0) /(_reserve1 * reserve1);
        }
        ...
        ALIA.transferFrom(buyer, temp.seller, (price  / 1000) * mainPerecentage);
        ALIA.transferFrom(buyer, _tokenAuthors[tokenId], (price  / 1000) * authorPercentage);
        ALIA.transferFrom(buyer, platform, (price / 1000) * platformPerecentage);
  ...
  function buyNftNonCrypto(address  nft_a, uint256 tokenId, string memory ownerId) public returns(bool) {
    fixedSell storage temp = _saleTokens[tokenId];
    uint256 tempPrice;
     if(_tokenAuthors[tokenId] == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859 && adminOwner[temp.seller] && adminDiscount > 0){
      temp.price = temp.price - ((temp.price * adminDiscount) / 1000);
      _dollarPrice[tokenId] = _dollarPrice[tokenId] - ((_dollarPrice[tokenId]  * adminDiscount) / 1000);
     }
     if(temp.price > 0){
          tempPrice = temp.price;
           ...
           (uint112 _reserve0, uint112 _reserve1,) =LPBNB.getReserves();
           (uint112 reserve0, uint112 reserve1,) =LPAlia.getReserves();
           tempPrice = (_dollarPrice[tokenId] * _reserve0 * reserve0) /(_reserve1 * reserve1);
        }
   
        ALIA.mint(nonCryptoNFTVault, tempPrice);
        ALIA.transferFrom(nonCryptoNFTVault, platform, (tempPrice * 5)/100);
        _saleTokens[tokenId].price= tempPrice - ((tempPrice * 5)/100);
         buyNFT(nft_a, tokenId, nonCryptoNFTVault);
        _nonCryptoOwners[tokenId] = ownerId;
        emit BuyNFTNonCrypto( msg.sender, nft_a, tokenId, ownerId);
        return true;
  }
  function sellNFTNonCrypto(address nft_a, uint256 tokenId, string memory sellerId, uint256 price, uint256 priceDollar) public {
    // require(keccak256(abi.encodePacked((_nonCryptoOwners[tokenId]))) == keccak256(abi.encodePacked((sellerId))), "102b");
    sellNFT(nft_a, tokenId, nonCryptoNFTVault, price, priceDollar);
    emit SellNFTNonCrypto( msg.sender, nft_a, tokenId, sellerId, price, priceDollar);
  }
  ...
  require(msg.sender == 0x0160b796792D38B98cd19f718F8dF6c143E5c529,"Not Authorized");
  _;
  }
  ...
  function updateTokenURI(uint256 tokenIdT, string memory uriT) public{
        require(_tokenAuthors[tokenIdT] == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859,"102");
