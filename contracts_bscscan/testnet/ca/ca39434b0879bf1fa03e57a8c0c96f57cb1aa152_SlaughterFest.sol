...
contract SlaughterFest is IBEP20, Ownable
{
    ...
    uint256 constant SlaughterTime=10 minutes; //Selling is able to be done after 10 minutes.
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            require(Slaughterfest); //Must be after slaughtertime for selling to be enabled.
            tax=_getsellTax();
    ...
    function _getsellTax() private view returns (uint8)
    {
        if(block.timestamp>(launchTimestamp+SlaughterTime)){
            uint8 tax=_calculateSlaughterTax();
            return tax;
        }
        return _sellTax;
    }
    ...
    function _calculateSlaughterTax() private view returns (uint8){
        uint256 timeSinceSlaughter=block.timestamp-launchTimestamp-SlaughterTime;
        uint8 Tax=uint8(SlaughterMaxTax-((SlaughterMaxTax-_sellTax)*timeSinceSlaughter/SlaughterTime));
        return Tax;
