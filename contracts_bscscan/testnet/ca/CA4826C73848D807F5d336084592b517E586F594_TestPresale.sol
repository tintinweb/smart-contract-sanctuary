...
contract TestPresale is Ownable {
    ...
    function _buyTokens(address beneficiary, uint256 weiAmount) internal {
        ...
        claimedTokens[beneficiary] = tokens;
        token.transfer(beneficiary, tokens);
        
        emit TokenPurchase(beneficiary, weiAmount);
        emit TokenClaim(beneficiary, tokens);
    }
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "TestPresale: sale is not open");
        require(!hasEnded(), "TestPresale: sale is over");
        require(
            weiAmount >= minContribution,
            "TestPresale: min contribution criteria not met"
        );
        require(
            contributions[beneficiary].add(weiAmount) <= maxContribution,
            "TestPresale: max contribution criteria not met"
        );
        this;
    }
    ...
    function withdrawTokens() public onlyOwner {
        require(finalized, "TestPresale: presale is not over");
