...
contract CryptoBlades is Initializable, AccessControlUpgradeable {
    ...
    function initialize(
        ...
        burFightRewardGasOffset = ABDKMath64x64.divu(23177, 100000); // 0.0539 x 4.3
        burFightRewardBaseline = ABDKMath64x64.divu(344, 1000); // 0.08 x 4.3

        // migrateTo_5e833b0
        durabilityCostFight = 1;

        // migrateTo_801f279
        _burReforgeCharacterFees = [
            ABDKMath64x64.divu(2, 10),  // level 0-10:      0.2 usd;
            ABDKMath64x64.divu(5, 10),  // level 11-50:     0.5 usd;
            ABDKMath64x64.divu(10, 10), // level 51-100:    1.0 usd;
            ABDKMath64x64.divu(15, 10), // level 101-150:   1.5 usd;
            ABDKMath64x64.divu(20, 10), // level 151-200:   2.0 usd;
            ABDKMath64x64.divu(25, 10)  // level 201-250:   2.5 usd;
        ];
        burReforgeWeaponFee = ABDKMath64x64.divu(5, 10);//0.5 usd;
    ...
    int128 public burReforgeWeaponFee;
    ...
    int128[6] private _burReforgeCharacterFees;

    event FightOutcome(address indexed owner, uint256 indexed character, uint256 weapon, uint32 target, uint24 playerRoll, uint24 enemyRoll, uint16 xpGain, uint256 burGain);
    ...
    function fight(uint256 char, uint256 wep, uint32 target, uint8 fightMultiplier)
        external
    ...
    function mintCharacter() public onlyNonContract oncePerBlock(msg.sender) {
        ...
        uint256 buniAmount = usdToBuni(mintCharacterFee);
    ...
    function reforgeCharacter(uint256 reforgeID, uint256 burnID)
        public
        isCharacterOwner(reforgeID)
        isCharacterOwner(burnID)
        requestPayBurFromPlayer(_calculateReforgeCharacterFee(reforgeID))
    {
        _payBurContract(msg.sender, _calculateReforgeCharacterFee(reforgeID));
        characters.reforge(reforgeID, burnID);
    }

    function _calculateReforgeCharacterFee(uint256 reforgeID) internal view returns(int128) {
        uint8 charLevel = characters.getLevel(reforgeID);
        if (charLevel >= 250) {
            return 0;
        }
        else if (charLevel <= 9) {
            return _burReforgeCharacterFees[0];
        }
        else {
            uint8 index = (charLevel / 50) * 50;
            return _burReforgeCharacterFees[index + 1];
        }
    }
    ...
    {
        uint256 buniAmount = usdToBuni(mintWeaponFee);
        require(buniToken.balanceOf(msg.sender) >= buniAmount);
    ...
    function reforgeWeapon(uint256 reforgeID, uint256 burnID)
        ...
        requestPayBurFromPlayer(burReforgeWeaponFee)
    {
        _payBurContract(msg.sender, burReforgeWeaponFee);
    ...
    function _requestPayBurFromPlayer(int128 usdAmount) internal view {
        ...
        require(buniToken.balanceOf(msg.sender) >= fromUserWallet);
    }
    ...
    function setReforgeCharacterValues(uint256[6] memory cents) public restricted {
        for (uint8 i = 0; i < 6; i++) {
            _burReforgeCharacterFees[i] = ABDKMath64x64.divu(cents[i], 100);
        }
    }

    function setReforgeWeaponValue(uint256 cents) public restricted {
        burReforgeWeaponFee = ABDKMath64x64.divu(cents, 100);
    }
    ...
    function claimXpRewards() public {
        ...
        for(uint256 i = 0; i < characters.balanceOf(msg.sender); i++) {
            ...
            uint256 maxGainXp = characters.getMaxGainXp(char);

            if (xpRewardsToClaim > maxGainXp) {
                xpRewards[char] = xpRewardsToClaim.sub(maxGainXp);
    ...
    function getReforgeCharacterFees() external view returns(int128[6] memory fees) {
        for (uint8 i = 0; i < _burReforgeCharacterFees.length; i++) {
            fees[i] = _burReforgeCharacterFees[i];
        }
    }
...
contract Characters is Initializable, ERC721Upgradeable, AccessControlUpgradeable, ITransferCooldownable {
    ...
    function initialize () public initializer {
        __ERC721_init("BuniUniversal Trainer", "BUT");
        __AccessControl_init_unchained();
    ...
    */
    ...
    mapping(uint256 => uint8) private _charReforgedLevel;
    ...
    function mintCharacterWithElement(address minter, uint256 seed, uint8 element) public restricted {
        uint256 tokenID = tokens.length;
    ...
    function getMaxGainXp(uint256 id) public view returns(uint256) {
        ...
        if (char.level < 255) {
            uint maxGainXp = 0;
            for (uint8 i = char.level; i <= _levelMax(id); i++) {
                maxGainXp = maxGainXp.add(experienceTable[i]);
            }
            if (maxGainXp > char.xp) {
                return maxGainXp.sub(char.xp).sub(1);
            }
            else {
                return 0;
            }
    ...
    function hasReforged(uint256 id) public view returns(bool) {
        uint8 charLevel = tokens[id].level;
        return _charReforgedLevel[id] == _reforgeMaxLevel(charLevel);
    }

    function reforge(uint256 reforgeID, uint256 burnID) public restricted {
        uint8 charLevel = tokens[reforgeID].level;
        uint8 burnCharLevel = tokens[burnID].level;

        require(charLevel < 255 && charLevel.add(1).div(10) < experienceTable.length.div(10), "Reforge un-necessary");
        require(
            _reforgeMinLevel(charLevel) <= burnCharLevel && burnCharLevel <= charLevel,
            string(abi.encodePacked("Need burn trainer level ", _reforgeMinLevel(charLevel), "-" , charLevel))
        );
        if (_charReforgedLevel[reforgeID] != 0) {
            require(_charReforgedLevel[reforgeID] < charLevel, "Reforged");
        }

        _charReforgedLevel[reforgeID] = _reforgeMaxLevel(charLevel);
        _burn(burnID);
    ...
    function _levelMax(uint256 id) internal view returns(uint8) {
        ...
        if (_charReforgedLevel[id] == maxLevel) {
            if (maxLevel == 9) {
                return 49;
            }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        if(to != address(0) && to != address(0x000000000000000000000000000000000000dEaD) && !hasRole(NO_OWNED_LIMIT, to)) {
            require(balanceOf(to) < characterLimit, "Recv has too many trainers");
        }
...
contract Weapons is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function initialize () public initializer {
        __ERC721_init("BuniUniversal BuniCorn", "BUB");
        __AccessControl_init_unchained();
    ...
    */
    ...
    function mintWeaponByElement(address minter, uint256 seed, uint256 element) public restricted returns(uint256) {
        uint256 stars;
        uint256 roll = seed % 100;
        // will need revision, possibly manual configuration if we support more than 5 stars
        if(roll < 1) {
        ...
        return mintWeaponByElementWithStars(minter, stars, seed, element);
    }
    ...
    function mintWeaponByElementWithStars(address minter, uint256 stars, uint256 seed, uint256 element) public restricted returns(uint256) {
        require(stars < 8, "Stars parameter too high! (max 7)");
        (uint16 stat1, uint16 stat2, uint16 stat3) = getStatRolls(stars, seed);
