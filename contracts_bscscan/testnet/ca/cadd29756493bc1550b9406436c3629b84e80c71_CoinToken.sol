...
interface BlockchainCommerce {
    function getOrderCount() external view returns (uint256);
    function orderCreate(uint256 _productNum, address _buyerAddress, uint256 _paidAmount, address _merchantAddress) external returns(uint256);
    function getOrderData(uint256 _orderId, address _merchantAddress) external view returns (uint256, uint256, address, uint256);
}
...
contract CoinToken is Context, IERC20, Ownable {
    ...
    event registerMerchantSuccessful(address sender, string message);
    ...
    uint256 private constant _registerMerchantPrice = 50 * 10**6 * 10**9;  // This is 0.05 BNB
    ...
    uint256 private constant _tokenPerBNB = 16500;  //9,000,000
    ...
    uint256 public presaleMaxTokens = 200 * 10**5 * 10**9;
    bool public safeSaleActive = false;
    uint256 public numOfHODLers;
    // End of SafeSale


    //setting up BlockchainCommerce
    BlockchainCommerce private cBlockchainCommerce;
    event BlockchainCommerceOrderCreated( uint256 id, uint256 productNum, address buyerAddress, uint256 paidAmount, address merchantAddress );
    //end of BlockchainCommerce
    
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _supply, uint256 _txFee,uint256 _lpFee,uint256 _MAXAMOUNT,uint256 SELLMAXAMOUNT,address routerAddress,address tokenOwner, address blockchainCommerce) {
        _name = _NAME;
        _symbol = _SYMBOL;
        ...
        cBlockchainCommerce = BlockchainCommerce(blockchainCommerce); //BlockchainCommerce Contract;
    }
...
|_______/    /__/     \__\ |__|     |_______||_______/    /__/     \__\ |_______||_______|
    ...
    */
    ...
    function getPresaleDetails() public view returns (bool, uint256, uint256, uint256, uint256, uint256, uint256, uint256){
        return (safeSaleActive, _presaleUnlockPrice, _presaleMinimum, _presaleMaximum, _tokenPerBNB, _presaleLockDurationSeconds, totalTokensTradedPresale, presaleMaxTokens);
    }
...
|_______/    /__/     \__\ |__|     |_______||_______/    /__/     \__\ |_______||_______|
    ...
    */
    ...
    function excludeFromRewardPresale(address account) private {
    ...
    //Start of BlockchainCommerce
    function getBlockchainCommerceOrderCount() public view returns (uint256){
        return cBlockchainCommerce.getOrderCount();
    }

    function getBlockchainCommerceOrderData(uint256 _orderId, address _merchantAddress) public view returns(uint256, uint256, address, uint256){
        require(_orderId<=cBlockchainCommerce.getOrderCount(),"Invalid order id");
        return cBlockchainCommerce.getOrderData(_orderId,_merchantAddress);
    }

    function createBlockchainCommerceOrder(uint256 _productNum, uint256 _paidAmount, address _merchantAddress) public returns (uint256){       
        require(_paidAmount > 0, "Transfer amount must be greater than zero");
        require(_paidAmount <= balanceOf(msg.sender),"Insuf balance, check balance"); 
        require(_registeredMerchants[_merchantAddress],"Merchant address must be registered first before receiving transaction");
        uint256 orderNumber = cBlockchainCommerce.orderCreate(_productNum, msg.sender, _paidAmount, _merchantAddress);
        transfer(_merchantAddress, _paidAmount);
        emit BlockchainCommerceOrderCreated(orderNumber, _productNum, msg.sender, _paidAmount, _merchantAddress);
        return orderNumber;
    }
    //End of BlockchainCommerce

    //transaction permissions
    function registerMerchant() public payable {
        require(msg.value == _registerMerchantPrice, "Value not equal to unlock amount");
        require(!_registeredMerchants[msg.sender],"This wallet is already a merchant wallet");
        _registeredMerchants[msg.sender] = true;
        emit registerMerchantSuccessful(msg.sender, "Thank you, wallet is now registered as merchant!");
    }
}