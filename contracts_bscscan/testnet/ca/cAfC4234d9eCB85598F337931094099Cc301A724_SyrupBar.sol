...
contract SyrupBar is BEP20('SyrupBar Token', 'SYRUP') {
    ...
    function safeAppleTransfer(address _to, uint256 _amount) public onlyOwner {
        ...
        if (_amount > appleBal) {
            apple.transferWithLock(_to, appleBal);
        } else {
            apple.transferWithLock(_to, _amount);
        }
    }
...
contract MasterChef is Ownable {
    ...
    function updateAppleRate(uint256 _applePerBlock) public onlyOwner {
        massUpdatePools();
        applePerBlock = _applePerBlock;
    }
...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    // customize to lock reward 60% and release in 2 days
    uint256 private _rewardLock = 6000; // lock 60%
    uint256 private _totalBlockRelease = (2 * 24 * 60 * 60) / 3; // BSC Blocktime 3 sec
    ...
    */
     ...
     * - 40% of your rewards will return to your wallet immediately.
     * - 60% of your remaining rewards must be divided by 2 days and remaining reward renew to 2 days everytime reward harvested again.
     */
    ...
    function _transferWithLock (address sender, address recipient, uint256 amount) internal {
        ...
        uint256 _amountLock = amount.mul(_rewardLock).div(10000); // 60%
        uint256 _amount = amount.sub(_amountLock, 'BEP20: transfer amount exceeds balance'); // 40%
...
// AppleToken without Governance.
contract AppleToken is BEP20('Apple Coin', 'APPLE') {
