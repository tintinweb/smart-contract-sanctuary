...
contract MinterPocoEvent is Initializable , OwnableUpgradeable{
    ...
    event SpinTicketNoelResult(uint256 indexed resultType, address indexed minter, uint256 indexed nftId);
    ...
    function spinTicketNoelEvent() public onlyNonContract {
        ...
        uint256 randomValue = PocoRandom(pocoRandom).getPocoRandom(1000);
        if (randomValue == 1) {
            // Get character
            uint256 skin;
            uint256 skill;
            ...
            if (mintedCharacter < limitCharacter) {
                ...
                emit SpinTicketNoelResult(2, msg.sender, IPocoCharacter(pocoCharacter).currentTokenIDCounter() - 1);
                mintedCharacter += 1;
            }
        }
            ...
            if (mintedItem < limitItem) {
                ...
                emit SpinTicketNoelResult(3, msg.sender, IPocoEquiment(pocoEquiment).currentTokenIDCounter() - 1);
                mintedItem += 1;
            }
        }
        else {
            // 1 is spin ticket got nothing. Unlucky 
            emit SpinTicketNoelResult(1, msg.sender, 0);
        }
