  ...
  CoinDexTools
{
  ...
  // To exclude current staking to shared reward
  // users[address][teamIndex] => cycle index
  mapping(address => mapping(uint => uint)) private userDepositCycles;
  // users[address][teamIndex] => deposit amount with cycle
  mapping(address => mapping(uint => uint)) private userDepositAmount;
  ...
  function deposit(
    ...
    if (userDepositCycles[_msgSender()][_teamIndex] != cycleIndex) {
      userDepositCycles[_msgSender()][_teamIndex] = cycleIndex;
      userDepositAmount[_msgSender()][_teamIndex] = _amount;
    } else {
      userDepositAmount[_msgSender()][_teamIndex] += _amount;
    }
  ...
  function getPendingReward(address _account, uint _teamIndex) public view returns (uint) {
    ...
    if (teams[_teamIndex].totalAmount > 0) {
      ...
      if (userDepositCycles[_msgSender()][_teamIndex] == cycleIndex) {
        userTotalAmount_ -= userDepositAmount[_msgSender()][_teamIndex];
      }

      percentage_ = (userTotalAmount_ * _PRECISION_FACTOR) / teams[_teamIndex].totalAmount;
    }
