...
contract Lottery is Ownable, Pausable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) public poolLastDraw; // when was the last draw for the specific pool
    ...
    event PenaltyWalletUpdated(address penaltyWallet);
    event UnlockUpdated(bool unlock);
    event UnlockWindowUpdated(bool unlockWindow);
    ...
    function getTierValue(address account) public view returns (uint256 value) {
        uint256 amountOfTokenInWallet = orbit.balanceOf(account);
        uint256 tier;
        for (uint256 i = 0; i < tierCategory.length - 1; i++) {
            if (amountOfTokenInWallet >= tierCategory[i+1]) {
                tier = i + 1;
            }
        }
        return (tier);
        ...
        nonReentrant
    {
        ...
        uint256 beforeAmount = orbit.balanceOf(address(this));
        // We transfer the assets to the contract
        orbit.safeTransferFrom(msg.sender, address(this), amountToStake);
        uint256 afterAmount = orbit.balanceOf(address(this));
        require(afterAmount - beforeAmount == amountToStake, "did not receive enough, please disable fees");
    ...
    function leaveLottery() external nonReentrant {
        ...
        if (unlockWindow && !unlocked) {
            if (block.timestamp < poolLastDraw[poolId] + unstakeDelay) {
                uint256 refund = userEntry.amountStaked*(100-penaltyRate)/100;
                orbit.safeTransfer(msg.sender, refund);
                orbit.safeTransfer(penaltyWallet, userEntry.amountStaked-refund); 
            } else {
                orbit.safeTransfer(msg.sender, userEntry.amountStaked);
            }
        } else if (!unlockWindow && !unlocked) {
    ...
    function drawWinner(
        ...
        poolLastDraw[poolId] = block.timestamp;
    ...
    function drawWinnersArray(
        ...
        for (uint256 i = 0; i < poolIds.length; i++) {
            drawWinner(token, poolIds[i], rewards[i], ownerSalt+i);
        }
    }
    ...
    function updatePenaltyWallet(address _newPenaltyWallet) external onlyOwner {
        penaltyWallet = _newPenaltyWallet;
    ...
    function updateUnlock(bool _isUnlocked) external onlyOwner{
        unlocked = _isUnlocked;
        emit UnlockUpdated(_isUnlocked);
    }

    function updateUnlockWindow(bool _isUnlockWindow) external onlyOwner {
        unlockWindow = _isUnlockWindow;
        emit UnlockWindowUpdated(_isUnlockWindow);
    }
    ...
    function seePoolUsers(uint256 startIndex, uint256 endIndex, uint256 poolId) 
        external 
        view 
        returns(userData[] memory users) {

        if (endIndex > stakePools[poolId].length-1) {
            endIndex = stakePools[poolId].length-1;
        }
        
        users = new userData[](endIndex-startIndex+1);

        for (uint256 i = 0; i<endIndex-startIndex+1;i++) {
            uint256 index = i + startIndex;
            users[index] = stakePools[poolId][index];
        }
