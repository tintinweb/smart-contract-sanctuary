...
interface IApestributor {
    function startApestribution() external;
    function setApestributionParameters(uint256 _minPeriod, uint256 _minDistribution, uint256 _gas) external;
...
contract Apex is IERC20, Ownable {
    ...
    uint256 _maxBuyTxAmount = (_totalSupply * 1) / 250;
    uint256 _maxSellTxAmount = (_totalSupply * 1) / 100;
    uint256 _maxWalletSize = (_totalSupply * 2) / 2;
    ...
    IApestributor public apestributor;
    //uint256 delayApestribution = 60 minutes;
    bool public autoApeStream = true;
    uint256 lastStream;
    ...
    function startApestribution() external onlyOwner {
        apestributor.startApestribution();
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[sender]){ try apestributor.setShares(sender, _balances[sender]) {} catch {} }
        if(!isDividendExempt[recipient]){ try apestributor.setShares(recipient, _balances[recipient]) {} catch {} }

        if (processEnabled)
            try apestributor.process() {} catch {}
    ...
    function swapBack(uint256 amount) internal swapping {
        ...
        if (autoApeStream && balanceBefore > 0 && lastStream + streamPeriod < block.timestamp) {
            uint256 apeStream = (balanceBefore * streamPercent) / 100;
            try apestributor.deposit{value: apeStream}() {} catch {}
        }
        try apestributor.deposit{value: amountBNBDistribution}() {} catch {}
    ...
    function setup(address _developer, address distributor) external onlyOwner {
        ...
        apestributor = IApestributor(distributor);
        developer = payable(_developer);
    }
    
    function updateDistributor(address distributor) external onlyOwner {
        emit UpdatedSettings('Migrated Distributor', [Log(concatenate('Old Distributor: ',toString(abi.encodePacked(address(apestributor)))), 1),Log(concatenate('New Distributor: ',toString(abi.encodePacked(distributor))), 1), Log('', 0)]);
        apestributor.migrate(distributor);
        apestributor = IApestributor(distributor);
    }
    ...
    function manualStream(uint256 _percent) external onlyOwner {
        uint256 apeStream = (address(this).balance * _percent) / 100;
        apestributor.deposit{value: apeStream}();
        lastStream = block.timestamp;
    }
    ...
    function resetShares(address shareholder) external onlyOwner {
        if(!isDividendExempt[shareholder]){ apestributor.setShares(shareholder, _balances[shareholder]); }
        else apestributor.setShares(shareholder, 0);
    }
    ...
    function setIsDividendExempt(address holder, bool exempt) external onlyOwner {
        ...
        if(exempt){
            apestributor.setShares(holder, 0);
        }else{
            apestributor.setShares(holder, _balances[holder]);
        }
    ...
    function setApeStreamSettings(bool _enabled, uint256 _period, uint256 _percent) external onlyOwner {
        require(_percent <= 100);
        autoApeStream = _enabled;
        streamPeriod = _period;
        streamPercent = _percent;
        emit UpdatedSettings('ApeStream Settings', [Log('Enabled', _enabled ? 1 : 0),Log('Period', _period), Log('Percent', _percent)]);
    }
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 gas) external onlyOwner {
        ...
        apestributor.setApestributionParameters(_minPeriod, _minDistribution, gas);
    ...
    function getPoolStatistics() external view returns (uint256 totalAmount, uint256 totalClaimed, uint256 holders) {
        totalAmount = apestributor.getTotalRewards();
        totalClaimed = apestributor.getTotalRewarded();
        holders = apestributor.countShareholders();
    }
    
    function getWalletStatistics(address wallet) external view returns (uint256 pending, uint256 claimed) {
	    pending = apestributor.getUnpaidRewards(wallet);
	    claimed = apestributor.getPaidRewards(wallet);
	}

	function claimDividends() external {
	    apestributor.claim(msg.sender);
	    try apestributor.process() {} catch {}
	}
