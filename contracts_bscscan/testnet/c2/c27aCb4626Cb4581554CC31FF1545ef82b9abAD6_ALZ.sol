...
//         }
   ...
   function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        uint256 time_since_start = block.timestamp - _start_timestamp;
             ...
             if(time_since_start<8*hour){
                chtAmount = _calculateChtAmount(amount,sender,recipient,true);
                mktAmount = _calculateMktAmount(amount);
                burnAmount = _calculateBurnAmount(amount);
                taxAmount = mktAmount + burnAmount+chtAmount;
             }else {
                chtAmount = _calculateChtAmount(amount,sender,recipient,false);
                mktAmount = _calculateMktAmount(amount);
                burnAmount = _calculateBurnAmount(amount);
                taxAmount = mktAmount + burnAmount+chtAmount;
             }
            ...
            emit Transfer(sender, address(this), chtAmount);
            emit Transfer(sender, _marketing, mktAmount);
            emit Transfer(sender, _burnDead, burnAmount);
