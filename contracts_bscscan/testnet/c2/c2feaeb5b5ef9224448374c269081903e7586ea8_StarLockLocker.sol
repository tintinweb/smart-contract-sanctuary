...
contract StarLockLocker is Ownable, ReentrancyGuard {
    ...
    uint256 public bnbFee = 0.1 ether;
    // base 1000, 0.5% = value * 5 / 1000
    uint256 public lpFeeThousandth = 5;
    ...
    function lockTokens(
        ...
        if (_feeInBnb) {
            ...
            _nt = block.timestamp + 120;
        } else {
            uint256 fee = lockAmount.mul(lpFeeThousandth).div(1000);
        ...
        if(_nt>0)
        {
            lockedToken[_id].unlockTime = _nt;
        }
    ...
    function setLpFee(uint256 percent) external onlyOwner {
        ...
        lpFeeThousandth = percent;
    }
    
    function setDepositsPer(uint256 dPer) external onlyOwner {
        require(dPer > 0, 'dPer is too small');
        depositper = dPer;
    }
