...
contract Ownable is Context {
    ...
    constructor () {
        _owner = payable(0x607541193dd9f7D3409f97b587EE3ab3d515C271);
...
contract ScrapYard is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 50 * 1e9 * 1e9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "ScrapYard";
    string private _symbol = "SCRAP";
    ...
    bool public reflectionFeesdiabled = true;
    ...
    uint256 _totalFeePerTx = 150; // 10% by default
    ...
    uint256 sellFeeMultiplierNumerator = 200;
    uint256 sellFeeMultiplierDenominator = 100;
    uint256 sellFeeMultiplierTriggeredAt;
    uint256 sellFeeMultiplierLength = 30 minutes;
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns (uint256) {
        ...
        if (!deductTransferFee) {
            ...
            uint256 rAmount = tAmount.mul(_getRate());
            uint256 rTransferAmount = rAmount.sub(getTotalFeePerTx(tAmount).mul(_getRate()));
    ...
    function setFeePercent(uint256 liquidityFee, uint256 marketFee, uint256 giveawayFee) external onlyOwner {
    ...
    function setSellFeeMultiplierNumerator(uint256 _currentFeeMultiplierNumerator) external onlyOwner {
        sellFeeMultiplierNumerator = _currentFeeMultiplierNumerator;
    }
    ...
    function setGiveawayAddress(address payable _giveawayAddress) external onlyOwner {
        payoutWallet = _giveawayAddress;
    }
    ...
    function getSellFee() public view returns (uint256) {
        uint256 remainingTime = sellFeeMultiplierTriggeredAt.add(sellFeeMultiplierLength).sub(block.timestamp);
        uint256 feeIncrease = _totalFeePerTx.mul(sellFeeMultiplierNumerator).div(sellFeeMultiplierDenominator).sub(_totalFeePerTx);
        return _totalFeePerTx.add(feeIncrease.mul(remainingTime).div(sellFeeMultiplierLength));
    }
    ...
    function _takeAllFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tAllFee = tAmount.mul(_totalFeePerTx).div(1e3);
        uint256 rAllFee = tAllFee.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rAllFee);
        if (_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(rAllFee);
        emit Transfer(_msgSender(), address(this), tAllFee);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if (!takeFee)
            setAllFee(0);
        else if(recipient == pancakePair && sellFeeMultiplierTriggeredAt.add(sellFeeMultiplierLength) > block.timestamp){ 
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        uint256 rTransferAmount = rAmount.sub(getTotalFeePerTx(tAmount).mul(currentRate));
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            uint256 lpPercent = _liquidityFee.mul(1e4).div(_totalFeePerTx).div(2);
            uint256 mPercent = _marketFee.mul(1e4).div(_totalFeePerTx);
            uint256 gPercent = _giveawayFee.mul(1e4).div(_totalFeePerTx);
            ...
            uint256 totalPercent = lpPercent.add(mPercent).add(gPercent);

            uint256 bnbToBeAddedToLiquidity = getContractBalance().mul(lpPercent).div(totalPercent);
            ...
            payoutWallet.transfer(getContractBalance().mul(gPercent).div(totalPercent));
            marketWallet.transfer(getContractBalance());
            
            emit SwapAndLiquify(tokenAmountToBeSwapped, getContractBalance(), otherPiece);
        }
    }
