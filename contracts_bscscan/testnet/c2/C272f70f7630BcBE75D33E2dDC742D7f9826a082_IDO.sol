...
contract IDO is Ownable {
    ...
    function buy(uint _pid) external {
        ...
        _pool.idoToken2Buy.transferFrom(_msgSender(), _pool.idoUnlock, _pool.amount * _pool.tokenBuy2IDOtoken);
    ...
    function set(uint _pid, uint _amount, bool _status, bool _isOverWhitelist) external onlyOwner {
    ...
    function addPool(IERC20 _idoToken, IERC20 _idoToken2buy, uint _tokenBuy2IDOtoken, uint _amount, uint _totalAmount, address _idoUnlock, string memory _keyType, uint _startTime, uint _endTime, bool _isOverWhitelist) external onlyOwner {
        require(_idoUnlock != address(0), 'IDO: invalid unlock address');
        poolInfo.push(PoolInfo(_idoToken, _idoToken2buy, _tokenBuy2IDOtoken, _amount, _totalAmount, _totalAmount, _idoUnlock, _keyType, _startTime, _endTime, true, _isOverWhitelist));
    }
