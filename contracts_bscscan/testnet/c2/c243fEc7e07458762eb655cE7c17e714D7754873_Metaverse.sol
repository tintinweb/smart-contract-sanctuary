...
contract Metaverse is AccessControlEnumerableUpgradeable, ERC721Holder, PausableUpgradeable
{
        ...
        address _collector) public initializer
    {
        __AccessControlEnumerable_init_unchained();
        __Pausable_init_unchained();
        __Metaverse_init_unchained(_admin, _land, _usd, _collector);
    }
        ...
        address _collector) internal initializer
    {
        ...
        land = _land;
        usd = _usd;
        pricePerPoint[_usd] = 3e18;  // 3 tokens;
    ...
    function buyLand(string[] calldata points, address token) external whenNotPaused {
        require(points.length > 0, "invalid points");
        require(token == usd, "unsupport token");
    ...
    function setSupportUSDToken(address _usd) external onlyAdmin {
        require(_usd != address(0), "invalid address");
        require(_usd != usd, "same address");
        usd = _usd;
    }

    function setPricePerPoint(uint256 price) external onlyAdmin {
        require(price != 0, "invalid price");
        pricePerPoint[usd] = price;
    }
}