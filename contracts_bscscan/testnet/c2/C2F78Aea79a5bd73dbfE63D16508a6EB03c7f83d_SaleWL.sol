...
contract SaleWL {
    ...
    function invest(uint256 _amount) public {
        ...
        if(block.timestamp < _START_TIME.add(_SALE_DAYS)){
            if(users[msg.sender].round == 0){
                limit = _basevar.mul(weight).div(10);
            }
            if(users[msg.sender].round == _round){
                require((_basevar.mul(weight).div(10)) >= (_amount.add(users[msg.sender].invested[_round])),"You cannot invest");
                limit = (_basevar.mul(weight).div(10)).sub(users[msg.sender].invested[_round]);
            }
        }
    ...
    function setNextRound(uint256 _x) public {
        require(msg.sender == _owner);
        _nextRound = _x;
    }
