...
contract CA is ERC721, Ownable {
    ...
    event Operate(address indexed addr,string base64str);

    constructor() ERC721("CA", "Crazy Alpaca") {
    ...
    function updateCAOwner(uint _tokenId,address _toAddress) public onlyOwner returns (bool) {
        _transfer(ownerOf(_tokenId),_toAddress, _tokenId);
        return true;
    }
    function operate(string memory base64str)public returns (bool){
        emit Operate(msg.sender,base64str);
        return true;
    }
}