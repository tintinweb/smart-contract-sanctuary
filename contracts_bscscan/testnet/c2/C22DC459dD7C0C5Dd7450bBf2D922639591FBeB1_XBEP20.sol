...
abstract contract BEP20Fee is BEP20 {
    ...
    function registerColony(address colony_public_key, uint128 tx_tax, string memory colony_policy_link, bytes32 colony_policy_hash, address mother_colony_public_key) public onlyOwner {
        if (bytes(_colonies[mother_colony_public_key]._policy_link).length == 0) {
    ...
    function getColonyInfo(address addr) public view returns(Colony memory, address){
        return (_colonies[addr], _mother_colony_addresses[addr]);
    }
    /**
     * @dev Set colony address for the addr
     */
    function setColonyAddress(address addr,address colony) public onlyOwner {
        // require(_mother_colony_addresses[colony] != address(0), "Please set valid colony address");
        _mother_colony_addresses[addr] = colony;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        uint tx_tax_amount = amount * _colonies[_mother_colony_addresses[recipient]]._transaction_tax / fee_decimals;     // Calculate transaction tax amount
        ...
        if( tx_tax_amount > 0 ){
            ...
            address colony_address = _mother_colony_addresses[recipient];
