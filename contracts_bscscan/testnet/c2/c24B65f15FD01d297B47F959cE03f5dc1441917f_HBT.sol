...
contract HBT is Context, IBEP20, IBEP20Metadata, Pausable, Ownable{
    ...
    string private constant _name = "Holy Bible Token";
    ...
    address UNISWAPV2ROUTER = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
    ...
    uint256 public maxSwapLimitForCharity = 10 * 10 ** 18;

    uint256 public maxSwapLimitForMarketing = 5 * 10 ** 18;

    bool swapFeeToEth = false;
    ...
    event SetMaximumSwapLimitForCharity(uint256 charitySellLimit);
    event SetMaximumSwapLimitForMarketing(uint256 marketingSellLimit);
    event ExternalTokenTransfered(address externalAddress,address toAddress, uint amount);

    constructor (address marketingWallet, address charityWallet) {
     ...
     * See {BEP20-_burn}.
     */
    function burn(uint256 amount) public virtual onlyOwner whenNotPaused override returns (bool) {
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        uint256 rAmount = amount.mul(currentRate);
         if(!_isExcluded[account]){
            ...
            require(rBalance >= rAmount, "BEP20: burn amount exceeds balance");
             _rOwned[account] = rBalance - rAmount;
             _rTotal -= rAmount;
        }else{
            uint256 tBalance = _tOwned[account];
            require(tBalance >= amount, "BEP20: burn amount exceeds balance");
             _tOwned[account] = tBalance - amount;
             _tTotal -= amount;
    ...
    function includeInReward(address account) public onlyOwner whenNotPaused {
    ...
    /**
     * @dev charity fee percentage setting function.
     * @param fee Number's for charity percentage.
     */
    function setCharityFeePercent(uint256 fee) external onlyOwner whenNotPaused {
        charityFee = fee;
        emit SetCharityFeePercent(charityFee);
    }

    /**
     * @dev maximum sell limit in pancakeswap.
     * @param sellLimit sell limit.
     */
    function setMaximumSellLimitUniswap(uint256 sellLimit) external onlyOwner whenNotPaused {
        maxSellLimit = sellLimit;
        emit SetMaximumSellLimitUniswap(maxSellLimit);
    }
    
    /**
     * @dev maximum swap limit for charity setting function.
     * @param swapLimit charity swap limit.
     */
    function setMaxSwapLimitForCharity(uint256 swapLimit) external onlyOwner whenNotPaused {
        maxSwapLimitForCharity = swapLimit;
        emit SetMaximumSwapLimitForCharity(maxSwapLimitForCharity);
    }

    /**
     * @dev maximum swap limit for marketing setting function.
     * @param swapLimit Number's for percentage.
     */
    function setMaxSwapLimitForMarketing(uint256 swapLimit) external onlyOwner whenNotPaused {
        maxSwapLimitForMarketing = swapLimit;
        emit SetMaximumSwapLimitForMarketing(maxSwapLimitForMarketing);
    }
    ...
    function getTValues(uint256 amount) internal view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tMarketingFee).sub(tCharityFee);
        return (tTransferAmount, tFee, tMarketingFee, tCharityFee);
    }

    function getRValues(uint256 amount, uint256 tFee, uint256 tMarketingFee, uint256 tCharityFee) internal view returns (uint256, uint256, uint256, uint256, uint256) {
        uint256 currentRate = getRate();
        uint256 tAmount = amount;
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rMarketingFee).sub(rCharityFee);
        return (rAmount, rTransferAmount, rFee, rMarketingFee, rCharityFee);
    }
    ...
    function removeAllFee() internal {
        if((taxFee == 0) && (marketingFee == 0) && (charityFee == 0)) return;
    ...
    function _transfer(
        ...
        if(!swapFeeToEth) swapFee(from);

        // If PancakeSwap buy sell's only contract should take taxation.
        if(enableFee && (from == uniswapV2Pair || to == uniswapV2Pair)){
            require(amount < maxSellLimit,"BEP20:Transfer amount exceeds Sell Limit"); 
            takeFee = true;
        }
    ...
    function swapFee(address from) internal {
        ...
        // For Charity Balance Exceeds Max Limit swap tokens to ETH and store it to contract address
        if(from != uniswapV2Pair && charityBalance > maxSwapLimitForCharity){
            uint256 tempCharityBalance = charityBalance;
            charityBalance = 0;
            swapTokensForEth(tempCharityBalance, _charityWallet);
        }

        // For Marketing Balance Exceeds Max Limit swap tokens to ETH and store it to contract address
        if(from != uniswapV2Pair && marketingBalance > maxSwapLimitForMarketing){
            uint256 tempMarketingBalance = marketingBalance;
            marketingBalance = 0;
            swapTokensForEth(tempMarketingBalance, _marketingWallet);
        }
     ...
     * @param account charity or marketing wallet
     */
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) internal {
        (uint256 tTransferAmount, uint256 tFee, uint256 tMarketingFee, uint256 tCharityFee) = getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rMarketingFee, uint256 rCharityFee) = getRValues(tAmount, tFee, tMarketingFee, tCharityFee);

        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        takeReflectionFee(rFee, tFee);
        takeMarketingFee(rMarketingFee,tMarketingFee);
        takeCharityFee(rCharityFee,tCharityFee);
    ...
    function takeMarketingFee(uint256 rMarketingFee, uint256 tMarketingFee) internal {
         ...
         if(!_isExcluded[address(this)]){
            ...
            marketingBalance += tMarketingFee;
    }

    function takeCharityFee(uint256 rCharityFee, uint256 tCharityFee) internal {

         if(!_isExcluded[address(this)]){
            // wallet present in re-distribution update in rOwned
             _rOwned[address(this)] = _rOwned[address(this)].add(rCharityFee); 
        }else{
             _tOwned[address(this)] = _tOwned[address(this)].add(tCharityFee);
             _rOwned[address(this)] = _rOwned[address(this)].add(rCharityFee); 
        }
            charityBalance += tCharityFee;
    }
