...
contract Lfg is ERC20 {
  ...
  uint256 private _initialSupply = 1000000000000000;

  constructor() ERC20("LToken", "LT") {
  ...
  function ensureOneHuman(address _to, address _from) internal virtual returns (address) {
    require(!Address.isContract(_to) || !Address.isContract(_from), 'SORRY: No bots allowed!');
  ...
  function ensureOneTxPerBlock(address addr) internal virtual {
    ...
    require(isNewBlock, 'SORRY: Only one transaction per block!');
  }
...
/** 
 * If you've scrolled down till here and/or read the whole contract: RESPECT! You are a smart human being!!! Hopefully we'll meet at Sam's yacht party in the not too distant future! RTTv2 FOREVER!!! 🚀🚀🚀
*/