...
interface IERC721 is IERC165 {
    ...
    function burn(address _from, uint256 _tokenId) external returns (bool);
}
...
contract IssueAndUpgradeHero is AccessControl {
    ...
    mapping(uint256 => Heroes) public heroesNumber; // tokenId => Heroes
    ...
    modifier conditionOfCommon(uint256 tokenId) {
        uint256 tierMain = queryNumberTier(tier[tokenId]);
        uint256 tierBasicMain = queryNumberTier(tierBasic[tokenId]);
        if(tierBasicMain == 0){
    ...
    function addHeroes(uint256[] memory id, string[] memory name, string[] memory race, string[] memory class) public {
        ...
        for(uint256 i = 0; i < name.length ; i ++) {
            // heroes.push(Heroes(name[i], race[i]));
    ...
    function issueHero(uint256 _tokenId, uint256 _numberHero, string memory _tier) public {
        ...
        string memory uriToken = string(abi.encodePacked(uri, uint2str(_tokenId)));
        heroesNFT.mint(msg.sender, _tokenId, uriToken);
        tier[_tokenId] = _tier;
        tierBasic[_tokenId] = _tier;
        heroesNumber[_tokenId] = heroes[_numberHero];
        emit issue(
            msg.sender,
            _tokenId,
    ...
    function upgradeHero(uint256[] memory listHeroId) public conditionOfCommon(listHeroId[0]){
        ...
        require(keccak256(bytes( tier[heroesTierMain] )) != keccak256(bytes( tierName[tierName.length - 1].tierName )), "You are already at the highest level");
        require(queryNumberTier(tier[heroesTierMain]) != 100, "Tier not found");
        uint256 tierMain = queryNumberTier(tier[heroesTierMain]);    
        ...
        if(requestHeroOrRace == 0){
            ...
            for(uint256 i = 1; i < listHeroId.length ; i++){
                require(keccak256(bytes( heroesNumber[heroesTierMain].name )) == keccak256(bytes( heroesNumber[listHeroId[i]].name )), "Not the same heroes");
                require(queryNumberTier(tier[listHeroId[i]]) == heroesTierFood, "Tier food not is incorrect");
                heroesNFT.burn(msg.sender, listHeroId[i]);
                delete tier[listHeroId[i]];
                delete tierBasic[listHeroId[i]];
        ...
        if(requestHeroOrRace == 1){
            //same race
            for(uint256 i = 1; i < listHeroId.length ; i++){
                require(keccak256(bytes( heroesNumber[heroesTierMain].race )) == keccak256(bytes( heroesNumber[listHeroId[i]].race )), "Not the same race");
                require(queryNumberTier(tier[listHeroId[i]]) == heroesTierFood, "Tier food not is incorrect");
                heroesNFT.burn(msg.sender, listHeroId[i]);
                delete tier[listHeroId[i]];
                delete tierBasic[listHeroId[i]];
        ...
        // Update Heroes
        tier[heroesTierMain] = tierName[tierMain + 1].tierName;
        emit ascend(
            msg.sender,
            heroesTierMain,
            tier[heroesTierMain]
