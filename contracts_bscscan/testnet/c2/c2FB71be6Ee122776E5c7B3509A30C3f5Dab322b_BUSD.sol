...
contract BUSD is LibNote {
  ...
  event Transfer(address indexed src, address indexed dst, uint wad);
    ...
    modifier auth {
        require(wards[msg.sender] == 1, "BUSD/not-authorized");
        _;
    }

    // --- ERC20 Data ---
    string  public constant name     = "MOCK BUSD";
    string  public constant symbol   = "MOCKBUSD";
    string  public constant version  = "1";
    uint8   public constant decimals = 18;
    uint256 public totalSupply;
    uint public dailyBusdLimit;

    mapping (address => uint)                      public balanceOf;
    mapping (address => mapping (address => uint)) private allowances;
    mapping (address => uint)                      public nonces;
    mapping (address => uint)                      public lastMintRestart;
    mapping (address => uint)                      public busdMintedToday;
    ...
    constructor(uint256 chainId_) {
        ...
        dailyBusdLimit = 10000000000000000000000;
    }
    ...
    function transferFrom(address src, address dst, uint wad) public returns (bool) {
      ...
      require(balanceOf[src] >= wad, "BUSD/insufficient-balance");
        if (src != msg.sender && _allowance( src, msg.sender ) != uint(-1)) {
            require(_allowance( src, msg.sender ) >= wad, "BUSD/insufficient-allowance");
    ...
    function adjustDailyBusdLimit(uint _limit) external auth {
        dailyBusdLimit = _limit;
    }

    function mint(address usr, uint wad) external {

      if(wards[msg.sender] == 0) {
        require(add(wad, busdMintedToday[msg.sender]) <= dailyBusdLimit || sub(block.number, lastMintRestart[msg.sender]) >= 6500 && wad <= dailyBusdLimit, "Over daily Busd Limit");
        if( sub(block.number, lastMintRestart[msg.sender]) >= 6500 ) {
            busdMintedToday[msg.sender] = wad;
            lastMintRestart[msg.sender] = block.number;
        } else {
            busdMintedToday[msg.sender] = add(busdMintedToday[msg.sender], wad);
        }
      }
    ...
    function burn(address usr, uint wad) external {
        require(balanceOf[usr] >= wad, "BUSD/insufficient-balance");
        if (usr != msg.sender && _allowance( usr, msg.sender ) != uint(-1)) {
            require(_allowance( usr, msg.sender ) >= wad, "BUSD/insufficient-allowance");
                    ...
                    bool allowed, uint8 v, bytes32 r, bytes32 s) external
    {
        ...
        require(holder != address(0), "BUSD/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "BUSD/invalid-permit");
        require(expiry == 0 || block.timestamp <= expiry, "BUSD/permit-expired");
        require(nonce == nonces[holder]++, "BUSD/invalid-nonce");
