...
Initializable
{
    ...
    function setMaxMintAmount(uint maxMintAmount_) onlyRole(ADMIN_ROLE) whenPaused public {
        require(maxMintAmount_ != 0, "The maxMintAmount is 0");
        maxMintAmount = maxMintAmount_;
        emit SetMaxMintAmount(maxMintAmount_);
    }
    ...
    function mintMulti(uint amount) whenNotPaused onlyEOA external returns(uint[] memory tokens_){
        require(amount > 0 && amount <= maxMintAmount, "BlindBox: missing amount or exceeding max amount");
        uint[] memory tokenIds_;
        for (uint i = 0; i < amount; i++) {
            _mint(_msgSender(), _tokenIdTracker.current());
            tokenIds_[i] = _tokenIdTracker.current();
    ...
    function mintOne() whenNotPaused onlyEOA external{
        _mint(_msgSender(), _tokenIdTracker.current());
        emit Mint(_msgSender(),_tokenIdTracker.current());
        _tokenIdTracker.increment();
        uint cost = nftPrice;
