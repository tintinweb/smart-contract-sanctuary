...
contract Swaraj is Context, IERC20, Ownable {
    ...
    string private _name = "Swaraj";
    string private _symbol = "SWRJ";
    ...
    SellWhaleFee public sellWhaleFee;
    ...
    address public _marketingWallet = payable(address(0x629706f83721DD0F0D0faF2f6D0c70eEe0C4e7Df));
    address public _rewardWallet = payable(address(0x17A22378f263DE8987A342ABB5365A1FC0a25bB4));

    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;

    uint256 public _maxTxAmount = 1 * 10**10 * 10**18;
    uint256 private numTokensSellToAddToLiquidity = 1 * 10**10 * 10**18;
    ...
    constructor() {
        ...
        sellFee.rewardFee = 50;

        sellWhaleFee.liquidityFee = 350;
        sellWhaleFee.marketingFee = 200;
        sellFee.rewardFee = 50;
        ...
        _tOwned[_msgSender()] = _tOwned[_msgSender()].add(_tTotal);
    ...
    function setBuyFee(uint16 liq, uint16 market, uint16 reward) external onlyOwner {
        buyFee.liquidityFee = liq;
        buyFee.marketingFee = market;
        buyFee.rewardFee = reward;
    }

    function setSellFee(uint16 liq, uint16 market, uint16 reward) external onlyOwner {
        sellFee.liquidityFee = liq;
        sellFee.marketingFee = market;
        sellFee.rewardFee = reward;
    }

    function setSellWhaleFee(uint16 liq, uint16 market, uint16 reward) external onlyOwner {
        sellWhaleFee.liquidityFee = liq;
        sellWhaleFee.marketingFee = market;
        sellWhaleFee.rewardFee = reward;
    }
    ...
    function setAntiWhaleLimit(uint256 _antiWhaleLimit) external onlyOwner{
        ...
        )
    {
        ...
        return (tTransferAmount, tLiquidity, tReward, tMarketing);
    }
