...
contract TestMoon is Context, IERC20, Ownable {
    ...
    string private constant _name = "TestMoon";
    string private constant _symbol = "TESTMOON";
    ...
    feeRatesStruct public feeRates = feeRatesStruct(
     {rfi: 0,
      liquidity: 3,
      burn: 3,
      community: 6}); //32 bytes - perfect, as it should be
    ...
    address public communityWallet = 0x6d1DA4082A2F50e0A1c5557825F843916b613830;
    ...
    constructor () {
        ...
        IUniswapV2Router02 _PancakeSwapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); //BSC Mainnet
    ...
    */
    function setRfiRatesPercents(uint8 _rfi, uint8 _lp, uint8 _burn, uint8 _community) public onlyOwner {
      feeRates.rfi = _rfi;
      feeRates.liquidity = _lp;
      feeRates.burn = _burn;
      feeRates.community = _community;
      emit FeesChanged();
    }
   ...
   function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tCommunity = tAmount.mul(feeRates.community).div(100);

        s.tTransferAmount = tAmount.sub(s.tRfi).sub(s.tLiquidity).sub(s.tBurn).sub(s.tCommunity);
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi) {
        ...
        uint256 rCommunity = s.tCommunity.mul(currentRate);

        rTransferAmount = rAmount.sub(rRfi).sub(rLiquidity).sub(rBurn).sub(rCommunity);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        reflectCommunityandBurnFee(s.tCommunity,s.tBurn);

        emit Transfer(sender, recipient, s.tTransferAmount);
    }

    function reflectCommunityandBurnFee(uint256 tCommunity, uint256 tBurn) private {
        uint256 currentRate =  _getRate();
