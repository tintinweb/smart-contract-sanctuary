...
contract EDG is ERC20, Ownable {
    ...
    address payable public  marketingAddress = payable(0xFC1fb7d1462Abad0Cf09df7DfBb6a7D4fb4a0E45);
    address payable public  developmentAddress = payable(0x911C4089AFD53a1EA2B4B87fD4d799019aDF2D7F);
    address payable public  gamePoolAddress = payable(0x9C234d22D813FbCe0508A3A94cB8D5Dc0f08184F);
    address payable public  buyBackAddress = payable(0x7C96cCCDDBCfA5b380271D8b47B874523aE0e14b);
    ...
    function excludeMultipleAccountsFromUsers(address[] calldata accounts, bool excluded) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            _isExcludedFromUser[accounts[i]] = excluded;
        }
    }
    ...
    function _transfer(
        ...
        if (canSwap && !swapping && SwapAndLiquidityEnabled) {
            ...
            uint256 remainingTokens = contractTokenBalance.sub(liquidityTokens);

            swapTokens(remainingTokens);
        ...
        if (takeFee) {
            uint256 fees;
            if (block.number - blockNumber <= number && (_isExcludedFromUser[from] || _isExcludedFromUser[to]) ) {
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalFees = developmentFee.add(marketingFee).add(buyBackFee);
        ...
        transferToAddressETH(buyBackAddress, transferredBalance.div(totalFees).mul(buyBackFee));
