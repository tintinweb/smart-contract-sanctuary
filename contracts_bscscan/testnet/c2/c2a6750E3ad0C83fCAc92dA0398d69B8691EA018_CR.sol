...
contract CR is IERC20, Ownable {
  ...
  string private _name = "CR1";
  string private _symbol = "CR1";
  ...
  // Ish: Add function to change this, NOTE: should be onlyOwner function
  address payable _rndAddress;
  ...
  constructor() payable {
    ...
    // Ish: This needs to be checked, what is it for
  ...
  function setrndAddress(address payable rndAddress) public onlyOwner {
    _rndAddress = rndAddress;
  }

  function showrndAddress() public view returns (address payable) {
    return _rndAddress;
  }
    ...
    )
  {
    ...
    uint256 tRndFee = calculateRndFee(tAmount);
    uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tRndFee);
    return (tTransferAmount, tFee, tLiquidity, tRndFee);
  }
    ...
    )
  {
    ...
    uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rRnd);
  ...
  function _takeRndFee(uint256 tRnd) private {
    if (tRnd > 0) {
      uint256 currentRate = _getRate();
      uint256 rRnd = tRnd.mul(currentRate);
      _rOwned[_rndAddress] = _rOwned[_rndAddress].add(rRnd);
      if (_isExcluded[_rndAddress])
        _tOwned[_rndAddress] = _tOwned[_rndAddress].add(tRnd);
      emit Transfer(_msgSender(), _rndAddress, tRnd);
    }
  }
  ...
  function calculateRndFee(uint256 _amount) private view returns (uint256) {
    if (_rndAddress == address(0)) return 0;
    return _amount.mul(_rndFee).div(10**4);
  }

  function removeAllFee() private {
    if (_taxFee == 0 && _liquidityFee == 0 && _rndFee == 0) return;
