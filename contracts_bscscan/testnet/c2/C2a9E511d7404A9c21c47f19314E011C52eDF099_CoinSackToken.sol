...
contract CoinSackToken is IBEP20, Manageable {
    ...
    uint256 public _minPendingFeesForLiquidation = 750000 * 10**_decimals;
    ...
    uint256 public _pendingTotalFeesForLiquidation = 0;


    // auto token buybacks
    bool public _isAutoBuybackEnabled = false;
    uint256 public _minReserveWETHForBuyback = uint256(1 * 10**18).div(2);
    uint256 public _maxReserveWETHPerBuyback = uint256(1 * 10**18).div(6);
    ...
    function _transfer(address from, address to, uint256 tAmount) private {
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to] && _areFeesEnabled){
            if(_isAutoFeeLiquifyEnabled) {
                if(!_isInternalLiquifyLocked && (_pendingManagementFeesForLiquidation + _pendingReserveFeesForLiquidation) >= _minPendingFeesForLiquidation && to == address(_pancakePair)){
                ...
                _pendingTotalFeesForLiquidation = _pendingManagementFeesForLiquidation + _pendingReserveFeesForLiquidation;
            } else {
        ...
        // perform auto buyback
        if(_isAutoBuybackEnabled && to == address(_pancakePair) && address(this).balance >= _minReserveWETHForBuyback){
            uint256 amountReserveWETHForBuyback = address(this).balance.div(100).mul(_autoBuybackFactor);
            if(amountReserveWETHForBuyback > _maxReserveWETHPerBuyback) {
                amountReserveWETHForBuyback = _maxReserveWETHPerBuyback;
            }
            _buybackTokens(amountReserveWETHForBuyback);
        }
    ...
    function _liquidateFeesInternal(uint256 tManagementFeeAmount, uint256 tReserveFeeAmount) private internalLiquifyLock() {
        ...
        _pancakeRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(tTotalFeeAmount, 0, path, address(this), block.timestamp.add(100));
        ...
        emit SwapAndLiquifyTokens(tTotalFeeAmount, postSwapContractBalanceDifference);
    }
    ...
    function _reinjectTokensAsLiquidity(uint256 reserveWETHToUse, uint256 reserveTokenToUse) private {
        require(address(this).balance >= reserveWETHToUse, "not enough WETH in reserve");
        require(balanceOf(address(this)).sub(_pendingTotalFeesForLiquidation) >= reserveTokenToUse, "not enough token in reserve");

        _approve(address(this), address(_pancakeRouter), reserveTokenToUse);

        _pancakeRouter.addLiquidityETH{value: reserveWETHToUse}(address(this), reserveTokenToUse, 0, 0, address(this), block.timestamp.add(100));
    }
    ...
    function includeInReflections(address account) public onlyManagement() returns (bool) {
        ...
        for(uint256 i = 0; i < _excludedFromReflections.length; i++) {
            if(_excludedFromReflections[i] == account){
                ...
                _rOwned[account] = _tOwned[account].mul(_getCurrentReflectionRate());
                _tOwned[account] = 0;
    ...
    /* @dev: */
    function setMinReserveWETHForBuyback(uint256 minReserveWETHForBuyback) public onlyManagement() returns (bool) {
        _minReserveWETHForBuyback = minReserveWETHForBuyback;
        return true;
    }

    /* @dev: */
    function setMaxReserveWETHPerBuyback(uint256 maxReserveWETHPerBuyback) public onlyManagement() returns (bool) {
        _maxReserveWETHPerBuyback = maxReserveWETHPerBuyback;
        return true;
    }
    ...
    /* @dev: */
    event SwapAndLiquifyTokens(uint256 amountTSwapped, uint256 amountETHRecieved);
