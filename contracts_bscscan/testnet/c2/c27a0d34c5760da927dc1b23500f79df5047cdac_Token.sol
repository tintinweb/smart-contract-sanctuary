...
contract Token is Context, IBEP20, Ownable {
    ...
    uint256 public _minMinerHoldings = 1000; //minimum token miner needs to hold before registration.
    ...
    */
    ...
    function updateMyHashBalance(address resellerAddress) public onlyReseller{
        address[] memory myminers = _resellers[resellerAddress].minerAddresses;
        uint256 totalHash=0;
        uint tCustomers=0;
        if(myminers.length>0){
            for(uint i = 0; i < myminers.length; i++){
                if(_miners[myminers[i]].expiry == 0 || _miners[myminers[i]].expiry > now){
                    totalHash += _miners[myminers[i]].hashRate;
                    tCustomers += 1;
                }
            }
        }
    ...
    function setMintinHashRate(uint256 hashRate) public onlyOwner {
