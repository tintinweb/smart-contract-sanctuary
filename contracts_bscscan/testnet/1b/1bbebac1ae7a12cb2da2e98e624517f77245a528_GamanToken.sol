...
/** 
   ...
   300,000,000,000,000 total supply
   3,000,000,000,000 tokens limitation for trade, which is 1% of the total supply
...
contract GamanToken is Context, IERC20, Ownable {
    ...
    address public _donationWalletAddress = 0xefa62ef5319FaC1cBA030C598D4Fb57e318f4759;
	
	address public _marketingWalletAddress = 0xC3375174076c7F2EaBF1a90278fF7A62D4FBCF36;
   
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 300000000 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "GamanToken";
    string private _symbol = "GAM88";
    ...
    function setDonationFeePercent(uint256 donationFee) external onlyOwner() {
		 ...
		 _liquidityFee = donationFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        _liquidityFee = liquidityFee;
		_donationFee = liquidityFee;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity.mul(2)).sub(tDonation);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tDonation,uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity.mul(2)).sub(rDonation);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _donationFee == 0 && _marketingFee == 0) return;
