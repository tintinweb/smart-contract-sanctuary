...
contract DigitalBEP721 is Ownable{
    ...
    function balanceOf(address owner) public view  returns (uint256) {
        require(owner != address(0), "BEP21: balance query for the zero address");
    ...
    function setApprovalForAll(address from, address to, bool approved, uint256 tokenId) public {
        ...
        require(to != msg.sender, "BEP721: approve to caller");
        _approve(to, tokenId);
...
contract UserDigitalBEP721 is DigitalBEP721{
  event CreateBEP721(address indexed creator, string name, string symbol);
    constructor(
        string memory name,
        string memory symbol
    ) DigitalBEP721(name, symbol) public{
         emit CreateBEP721(msg.sender, name, symbol);
    }     
}