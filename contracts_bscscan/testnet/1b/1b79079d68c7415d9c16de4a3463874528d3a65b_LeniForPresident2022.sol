...
contract LeniForPresident2022 is Context, IBEP20, Ownable {
    using SafeMath for uint256;

    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping(address => uint256) private donorsList;
    ...
    uint256 constant MINCONTRIBUTION = 0.01 ether;
    uint256 totalCont = 0;
    uint256 totalTokens = 0;
    uint256 totalDonors = 0;
    
    uint exchangeRate = 25000;
    
    address private creator;
    
    constructor() public {
        _name = 'LeniForPresident2022';
        _symbol = 'LENI';
        _decimals = 18;
    ...
    function contribute() public payable {
        ...
        require(msg.value >= MINCONTRIBUTION,"Invalid BNB Contribution!");
        ...
        totalCont = totalCont.add(msg.value);
        totalTokens = totalTokens.add(tokens);
        
        if(!isDonor(msg.sender)) // new donor? update record
        {
            totalDonors = totalDonors.add(1);
            donorsList[msg.sender] = totalDonors;
        }
    }
    ...
    function unlockContribution(uint perc) public onlyOwner returns (bool success) {
        address payable addr = address(uint160(creator));
        addr.transfer( address(this).balance * perc / 100 );
        return true;
    }
    
    function isDonor(address addr) public view returns (bool) {
        return (donorsList[addr] != 0);
    }
    
    function donorNo(address addr) public view returns(uint) {
         return donorsList[addr];
    }
