...
contract Faucet is Whitelist {
    ...
    FaucetContract public faucetInfo;

    uint256 internal constant WEI_PRECISION = 10**18;

    constructor() public {
        faucetInfo.tokenAmount = 10000 * WEI_PRECISION;
        faucetInfo.waitTime = 60 minutes;
    }

    event FaucetTransfer(address indexed to, uint256 value, uint256 timestamp);

    /* Map cooldown user faucet time for each token */
    mapping(address => mapping(string => uint256)) cooldown;
    ...
    modifier checkAllowedToWithdraw(address _address, string memory _tokenSymbol) {
        uint256 addressCooldown = cooldown[_address][_tokenSymbol];
        require(addressCooldown == 0 || addressCooldown <= block.timestamp, "withdraw unavaildable due to cooldown");
        _;
    }
    ...
    function getBalance() 
     public 
     view 
     returns (uint256 usdt, uint256 busd) 
    {
        return (
    ...
    function faucetSendBUSD()
        public
        checkAllowedToWithdraw(msg.sender, "busd")
    {
        (, uint256 busdBalance) = getBalance();
        require(busdBalance >= faucetInfo.tokenAmount, "insufficient BUSD");

        faucetInfo.busd.safeTransfer(
            msg.sender,
            faucetInfo.tokenAmount
        );
        cooldown[msg.sender]["busd"] = block.timestamp + faucetInfo.waitTime;

        emit FaucetTransfer(msg.sender, faucetInfo.tokenAmount, block.timestamp);
    }

    function faucetSendUSDT()
        public
        checkAllowedToWithdraw(msg.sender, "usdt")
    {
        (uint256 usdtBalance, ) = getBalance();
        ...
        cooldown[msg.sender]["usdt"] = block.timestamp + faucetInfo.waitTime;
