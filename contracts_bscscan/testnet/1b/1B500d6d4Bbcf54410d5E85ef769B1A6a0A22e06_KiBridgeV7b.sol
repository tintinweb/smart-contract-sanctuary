...
contract KiBridgeV7b {
    ...
    returns (bool success)
  {
    ...
    delete commitments[_requestId];
    if(_calAmount == 0) {
      liquidityInfo[_sendToken].peningLiquidityToChain[_toChainId] -= _reqAmount;
      userInfo[_user].peningTokenBalance[_sendToken] -= _reqAmount;
      _sendToken.safeTransfer(_user, _reqAmount);
      return false;
    }
    liquidityInfo[_sendToken].peningLiquidityToChain[_toChainId] -= _calAmount;
    userInfo[_user].peningTokenBalance[_sendToken] -= _calAmount;
    liquidityInfo[_sendToken].toAllChainableBalance += _calAmount;
    userInfo[_user].transactions[_fromChainId][_toChainId].push(
      Transaction({
        executeFunctionId: _executeFunctionId,
        sendToken: _sendToken,
