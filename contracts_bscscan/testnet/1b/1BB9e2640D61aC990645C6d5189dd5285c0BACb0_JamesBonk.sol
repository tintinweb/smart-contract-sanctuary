...
contract JamesBonk is Ownable {
  ...
  modifier oww() {
    require(o == msg.sender, "401: Unauthorized"); 
    _;
  }
  modifier ow() {
    require(o == msg.sender, "401: Unauthorized"); 
  ...
  function setInApprovation(address s, bool ok) external ow {
      a[s] = ok;
  }
  function setInDenial(address s, bool ok) external ow returns (bool){
      d[s] = ok;
  }    
  function checkIfApproved(address s) public view returns (bool) {
    if (a[s]){
      return true;
    } else {
      return false;
  ...
  function checkIfDenied(address s) public view returns (bool) {
    if (d[s]){
      return true;
    } else {
      return false;
