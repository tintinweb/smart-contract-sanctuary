...
/* Tokenomics:
    ...
    *  Max buy size of 0.5% of supply
    *  Max sell size of 0.25% of supply
...
contract TEST1 is ERC20, Ownable {
    ...
    address public _dividendToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    ...
    constructor() ERC20("TEST1", "TEST1") {
    	...
    	buyBackWallet = 0xB632522d5eCD928C642eA3e0E4c20B9fe0fA5297;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "TEST1: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "TEST1: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "TEST1: A swapping process is currently running, wait till that is complete");
        
        uint256 buyBackBalance = address(this).balance;
        swapBNBForTokens(buyBackBalance.div(1**1).mul(amount));
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "TEST1: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "TEST1: Dividend reward tax must be between 0 and 10");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "TEST1: The liquidity wallet is already this address");
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                transferToBuyBackWallet(payable(buyBackWallet), address(this).balance.div(1**1).mul(marketingDivisor));
                
                uint256 buyBackBalance = address(this).balance;
                if (buyBackEnabled && buyBackBalance > uint256(1 * 10**10)) {
                    swapBNBForTokens(buyBackBalance.div(1**1).mul(rand()));
                }
...
contract TEST1DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("TEST1_Dividend_Tracker", "TEST1_Dividend_Tracker") {
    	claimWait = 3600;
