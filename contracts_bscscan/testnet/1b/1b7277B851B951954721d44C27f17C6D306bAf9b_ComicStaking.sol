       ...
       OuruNFT nftContract = OuruNFT(0x456b0DF1ceC361b6C1c997992415B606394197A6);
        ...
        for(uint256 i=0; i < rewardChanges.length; i++) {
           ...
           (uint256 level, uint256 tier) = nftContract.getHero(tokenID);
            ...
            //rewards += ((rewardChanges[i].rewardPerBlock + ( tier + level )) * 1/200) * blocksPassed;  
            rewards += rewardChanges[i].rewardPerBlock * level * tier * blocksPassed;
            //rewards += rewardChanges[i].rewardPerBlock + ((level* + tier) * 1e16) * blocksPassed;
        }
        return rewards;
    }      
