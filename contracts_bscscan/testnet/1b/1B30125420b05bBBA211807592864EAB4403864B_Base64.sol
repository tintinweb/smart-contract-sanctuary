...
contract rarity_attributes {
    ...
    struct ability_score {
        uint32 strength;//力量
        uint32 dexterity;//灵巧
        uint32 constitution;//宪法
        uint32 intelligence;//智力
        uint32 wisdom;//智慧
        uint32 charisma;//魅力
    }
    
    mapping(uint => ability_score) public ability_scores;//能力分数
    mapping(uint => uint) public level_points_spent;//花费的等级点数
    ...
    //计算积分购买
    function calculate_point_buy(uint _str, uint _dex, uint _const, uint _int, uint _wis, uint _cha) public pure returns (uint) {//力量/灵巧/宪法/智力/智慧/魅力
        return calc(_str)+calc(_dex)+calc(_const)+calc(_int)+calc(_wis)+calc(_cha);
    }
    //计算
    function calc(uint score) public pure returns (uint) {//分数
        if (score <= 14) {
            return score - 8;
        } else {
            return ((score - 8)**2)/6;
    ...
    function _increase_base(uint _summoner) internal {
        ...
        uint _points_spent = level_points_spent[_summoner];//花费的等级点数
        require(abilities_by_level(rm.level(_summoner)) - _points_spent > 0);//能力等级
        level_points_spent[_summoner] = _points_spent+1;//花费的等级点数+1
    }
    
    //增加强度
    function increase_strength(uint _summoner) external {
        _increase_base(_summoner);//增加基本
        ability_score storage _attr = ability_scores[_summoner];//能力分数
