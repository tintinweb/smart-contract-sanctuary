...
contract Rainbow is ReentrancyGuard, IRainbow, Ownable {
    ...
    event LotteryOpen(uint256 indexed lotteryId, uint256 startBlock, uint256 endBlock, uint256 firstTicketId);
    event LotteryNumberDrawn(uint256 indexed lotteryId, uint256 finalNumber);
    event NewOperatorAddress(address operator);
    event TicketsPurchase(address indexed buyer, uint256 indexed lotteryId, uint256 ticketId, uint256 amount);
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            emit TicketsPurchase(msg.sender, currentLotteryId, thisTicketNumber, _amount[i]);
        }
    }
