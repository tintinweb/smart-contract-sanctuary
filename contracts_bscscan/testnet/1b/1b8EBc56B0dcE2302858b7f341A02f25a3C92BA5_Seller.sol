...
contract Seller is Ownable {
    ...
    IERC20 public _sellToken;
    uint256 public _salePrice;
    uint256 public _salePriceDiv;

    function buy(address refer) external payable returns (uint256 amount) {

        amount = uint256(msg.value).mul(_salePrice).div(_salePriceDiv);
        uint256 referAmount = uint256(amount).mul(5).div(100);
        require(amount+referAmount<=_sellToken.balanceOf(address(this)),"Out of balance");
        _sellToken.transfer(msg.sender,amount);
        _sellToken.transfer(refer,referAmount);
    }
    function setSalePrice(uint256 salePrice, uint256 salePriceDiv) external onlyOwner {
        _salePrice = salePrice;
        _salePriceDiv = salePriceDiv;
    }
    function changeSellToken(IERC20 token) external onlyOwner {
        _sellToken = token;
    }
