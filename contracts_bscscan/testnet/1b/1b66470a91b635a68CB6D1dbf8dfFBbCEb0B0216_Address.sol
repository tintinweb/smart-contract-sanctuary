...
// ----------------------------------------------- NCOIN ---------------------------------------------------------
// ---------------------------------------------------------------------------------------------------------------
contract NCOIN is BEP20, Pausable { 
	using SafeMath for uint256; 
	...
	uint256 public taxOperationalPercent; 
	...
	mapping (address => bool) public liquidityOperatorsList;
	mapping (address => bool) public liquidityProvidersList;	
	...
	bool public allowLiquidityAdding;
    bool public swapAndLiquifyEnabled;
	uint256 public addToLiquidityAmount;

	modifier onlyOperator() {
        require(operatorsList[_msgSender()] || _owner == _msgSender(), 'NCOIN: caller is not the operator/owner');
        _;
    }
	...
	constructor(){
		...
		liquidityOperatorsList[address(pancakeSwapV2Router)] = true;
		...
		taxLiquidityPercent = 50; // 0,5%	
		taxOperationalPercent = 50; // 0,5%

		taxExcludedList[address(this)] = true; 		 
		
		_owner = _msgSender();		
		taxExcludedList[_owner] = true;
		liquidityProvidersList[_owner] = true;
		operatorsList[_owner] = true;		
		operationalWallet = _owner;		
		_approve(_owner, address(pancakeSwapV2Router), MAX_INT);
   	}
	...
	function addLockedLiquidity(uint256 tokenAmount, uint256 bnbAmount) public payable lockLiquidity onlyOperator {
		...
		require(bnbAmount <= address(this).balance, 'NCOIN: not enough BNB');
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
		require(sender != address(0), 'NCOIN: transfer from the zero address');
        require(recipient != address(0), 'NCOIN: transfer to the zero address');
		if ( paused ) {
		    require(operatorsList[sender], "NCOIN: sender not whitelisted to transfer when paused");
		}
		if ( !allowLiquidityAdding && liquidityOperatorsList[recipient]) {
		    require(liquidityProvidersList[sender], "NCOIN: sender not whitelisted to add liquidity");
		}
		...
		if ( 
            ...
            require(amount <= maxTransferAmount, "NCOIN: sell transfer amount exceeds the max allowed");
			if ( amount > maxTaxFreeTransferAmount ) {
				takeTax = true;
			}
        }
        ...
        _balances[sender] = _balances[sender].sub(amount, 'NCOIN: transfer amount exceeds balance');
		...
		if ( takeTax ) { 
            ...
            if (taxOperationalPercent != 0) {
				uint256 taxOperationalAmount = amount.mul(taxOperationalPercent).div(10000);
				amount = amount.sub(taxOperationalAmount);
				_balances[operationalWallet] = _balances[operationalWallet].add(taxOperationalAmount);
				emit Transfer(sender, operationalWallet, taxOperationalAmount);
			}									
        }
	...
	function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, 'NCOIN: transfer amount exceeds allowance') );
        return true;
    }

	function lockLpTokens(uint256 unlockTimestamp) external onlyOwner {
		require(lpUnlockTimestamp <= block.timestamp, 'NCOIN: already locked');
    ...
    function setTaxLiquidityPercent(uint256 newTaxPercent) external onlyOwner {
		require((taxOperationalPercent + newTaxPercent) <= 1200, 'NCOIN: sum of taxes can`t be more than 1200 (12%)');
		taxLiquidityPercent = newTaxPercent;	
	}

	function setTaxMarketingPercent(uint256 newTaxPercent) external onlyOwner {
		require((taxLiquidityPercent + newTaxPercent) <= 1200, 'NCOIN: sum of taxes can`t be more than 1200 (12%)');
		taxOperationalPercent = newTaxPercent;	
	}
	...
	function setMarketingWallet(address account) external onlyOwner {
		taxExcludedList[operationalWallet] = false;
		operationalWallet = account;
		taxExcludedList[operationalWallet] = true;			
	}

	function toggleExchangesList(address account) external onlyOwner returns (bool) {
		require(account != pancakeSwapV2Pair, 'NCOIN: pancakeSwapV2Pair can`t be removed from list');
	...
	function toggleLiquidityOperatorsList(address account) external onlyOwner returns (bool) {
		require(account != address(pancakeSwapV2Router), 'NCOIN: pancakeSwapV2Router can`t be removed from list');
		liquidityOperatorsList[account] = !liquidityOperatorsList[account];	
		return liquidityOperatorsList[account];
	}

	function toggleLiquidityProvidersList(address account) external onlyOwner returns (bool) {
		liquidityProvidersList[account] = !liquidityProvidersList[account];	
		return liquidityProvidersList[account];
	}

	function toggleAllowLiquidityAdding() external onlyOwner returns (bool) {
		allowLiquidityAdding = !allowLiquidityAdding;
		return allowLiquidityAdding;	
	}	
	...
	function availableTokensToMint() public view returns (uint256) {
   	...
   	function mint(uint256 amount) public onlyOperator {
		require(_totalSupply.add(amount) <= MAX_SYPPLY, 'NCOIN: exceed max supply');
    ...
    function mintTo(address recipient, uint256 amount) public onlyOperator {
   	...
   	function mintToMultiple(address[] memory recipients, uint256[] memory amounts) public onlyOperator returns (uint256 amountTotal) {
        ...
        require(cnt > 0 && cnt <= 255, 'NCOIN: number or recipients must be more then 0 and not much than 255');
        require(amounts.length == recipients.length, 'NCOIN: number or recipients must be equal to number of amounts');
        for ( uint i = 0; i < cnt; i++ ){
			require(amounts[i] != 0, 'NCOIN: you can`t drop 0');
	...
	function recoverTokens(address token, uint256 amount) external onlyOwner {
        if (token == pancakeSwapV2Pair) {
			require(lpUnlockTimestamp <= block.timestamp, 'NCOIN: can`t withdraw LP tokens before unlock time');			
		}				
		IBEP20(token).safeTransfer(_msgSender(), amount);        
    }

	function recoverBnb(uint256 amount) external onlyOwner {		
		require(amount <= balanceOfBnb(), 'NCOIN: transfer amount exceeds BNB balance');
        (bool sent,) = _msgSender().call{ value: amount }("");
        require(sent, 'NCOIN: failed');       
    }
