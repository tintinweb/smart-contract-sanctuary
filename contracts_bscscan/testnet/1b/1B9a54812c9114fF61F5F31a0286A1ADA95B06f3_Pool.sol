...
contract Pool is Ownable {
    ...
    /// @notice The records of deposit by user.
    DepositRecord[] public depositRecords;

    /// @notice Info of each user that stakes LP tokens.
    mapping (uint256 => mapping (address => DepositRecord[])) public userDepositRecordList;


    event LogAddPool(uint256 indexed poolId, IERC20 _lpToken, uint256 lockBlockCount, uint256 minDepositLimit);
    event LogSetPool(uint256 indexed poolId, uint256 minDepositLimit);
    event LogDepositLp(uint256 indexed poolId, address, uint256 depositLpAmount, uint256 lastBlock);
    event LogWithdrawLp(uint256 indexed poolId, address, uint256 IERC20, uint256 lastBlock);

    event LogGetLpPrice(bool success, bytes returnData);
    ...
    function addPool(IERC20 _lpToken, uint256 _lockBlockCount, uint256 _minDepositLimit) public onlyOwner {
        ...
        emit LogAddPool(lpToken.length.sub(1), _lpToken, _lockBlockCount, _minDepositLimit);
    }
    ...
    /// @param _minDepositLimit The minimum amount to deposit.
    function setPool(uint256 _poolId, uint256 _minDepositLimit) public onlyOwner {
        PoolInfo storage pool = poolInfoList[_poolId];
        pool.minDepositLimit = _minDepositLimit;
        emit LogSetPool(_poolId, _minDepositLimit);
    }
    ...
    function deposit(uint256 _poolId, uint256 _depositLpAmount) public {
        ...
        PoolInfo storage pool = poolInfoList[_poolId];
        require(_depositLpAmount >= pool.minDepositLimit, "Pool: _depositAmount >= pool.minDepositLimit");
        ...
        userDepositRecordList[_poolId][msg.sender].push(
            DepositRecord({
                operateType: 1,
        ...
        emit LogDepositLp(_poolId, msg.sender, _depositLpAmount, lastBlock);
    }


    function withdraw(uint256 _poolId, uint256 _depositRecordId) public {

        DepositRecord storage userDepositRecord = userDepositRecordList[_poolId][msg.sender][_depositRecordId];
        ...
        uint256 totalBalance = pool.totalBalance.sub(userDepositRecord.userDepositAmount);
        pool.totalBalance = totalBalance;
    }


    function poolLength() public view returns (uint256 pools) {
        pools = poolInfoList.length;
    }
    ...
    function getDepositRecordList(uint256 _poolId) public view returns (DepositRecord[] memory) {
        return userDepositRecordList[_poolId][msg.sender];
    }
    ...
    function getLpPrice(uint256 _poolId) public payable returns (bool success, bytes memory resultData) {
        PoolInfo memory pool = poolInfoList[_poolId];

        (bool success, bytes memory resultData) = address(pool.lpToken).call(
            abi.encodeWithSignature("getReserves()")
        );

        LogGetLpPrice(success, resultData);
