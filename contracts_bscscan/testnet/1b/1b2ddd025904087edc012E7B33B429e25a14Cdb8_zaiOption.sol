...
contract zaiOption is Ownable, Pausable, ReentrancyGuard {
    ...
    address public treasuryAddress = 0x41d771508338FE1cc1aA89E90b11374FA12E0B50;
    address public devAddress = 0x41d771508338FE1cc1aA89E90b11374FA12E0B50;
    address public staffAddress = 0x41d771508338FE1cc1aA89E90b11374FA12E0B50;
    address public buybackAddress = 0x41d771508338FE1cc1aA89E90b11374FA12E0B50;
    address public buybackAddress2 = 0x41d771508338FE1cc1aA89E90b11374FA12E0B50;
    ...
    // 4% for Marketing
    uint16 public constant MktFee = 400;

    // 2% for buyback and burn
    uint16 public constant BuyBackFee = 200;

    // 1% for staff on distribution
    uint16 public constant StaffFee = 100;

    // 1% for development on distribution
    uint16 public constant DevFee = 100;
    ...
    uint256 public commissionBBAmt;
     ...
     * @param epoch: epoch
     */
    function betBear(uint256 epoch,address _referrer) external payable whenNotPaused nonReentrant notContract {
        ...
        //Record Referral
        if (msg.value > 0 && address(zaiReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
     ...
     * @param epoch: epoch
     */
    function betBull(uint256 epoch,address _referrer) external payable whenNotPaused nonReentrant notContract {
        require(epoch == currentEpoch, "Bet is too early/late");
        require(_bettable(epoch), "Round not bettable");
        require(msg.value >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(ledger[epoch][msg.sender].amount == 0, "Can only bet once per round");
    ...
    function claimTreasury() external nonReentrant onlyAdmin {
        ...
        uint256 treasuryAmt = (currentTreasuryAmount * 5000) / 10000; // 4% total > 50% of 8%
        uint256 devAmt = (currentTreasuryAmount * 1250) / 10000; // 1% total > 12.5% of 8%
        uint256 staffAmt = (currentTreasuryAmount * 1250) / 10000; // 1% total > 12.5% of 8%
        uint256 buybackAmt = (currentTreasuryAmount * 2500) / 10000; // 2% total > 25% of 8%
       
        //BuyBack Additional
        if(commissionBBAmt > 0){

            _safeTransferBNB(buybackAddress2, commissionBBAmt);
            commissionBBAmt = 0;

        }
          
        _safeTransferBNB(treasuryAddress, treasuryAmt);
        _safeTransferBNB(devAddress, devAmt);
        _safeTransferBNB(staffAddress, staffAmt);
        _safeTransferBNB(buybackAddress, buybackAmt);
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(zaiReferral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                _safeTransferBNB(referrer, commissionAmount);
                zaiReferral.recordReferralCommission(referrer, commissionAmount);
                emit ReferralCommissionPaid(_user, referrer, commissionAmount);
            } else{
                // add to BuyBack Amount
                commissionBBAmt += commissionAmount;
            }
        }
    }
}
