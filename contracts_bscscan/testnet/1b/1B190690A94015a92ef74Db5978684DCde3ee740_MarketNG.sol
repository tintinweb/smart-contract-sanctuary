...
contract MarketNG is IERC721Receiver, IERC1155Receiver, ReentrancyGuard, Ownable, Pausable {
    ...
    struct Intention {
        ...
        uint256 deadline; // deadline for both direct sell and auction
        bytes32 salt;
        uint8 kind;
    }
    ...
    function run(
        ...
        if (detail.opcode == OP_COMPLETE_SELL) {
            ...
            _assertSender(detail.caller);
            require(detail.caller == intent.user, 'seller only');
    ...
    function _isNative(IERC20 currency) internal returns (bool) {
