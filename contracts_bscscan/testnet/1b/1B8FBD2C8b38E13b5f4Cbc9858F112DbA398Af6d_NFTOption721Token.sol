...
contract NFTOption721Token is ERC721PresetMinterPauserAutoId, ERC721URIStorage, ReentrancyGuard {
    ...
    uint256 public constant SPRING = 1;
    uint256 public constant SUMMER = 2;
    uint256 public constant AUTUMN = 3;
    uint256 public constant WINTER = 4;
   
    uint256 public constant DFA_AMOUNT = 1000*10**18; 
    bytes32 public constant ADMIN_ROLE = keccak256("NFT721_ADMIN_ROLE");

    uint256 public createdNumber;          
     ...
     */
    event OnNFTTokenMinted(address indexed to, uint256 indexed seasonType, uint256 indexed nftID, string tokenURI);
     ...
     */
    function mintNFT(uint256 seasonType, address _to, string memory _tokenURI) public onlyAdmin returns(bool) {
        require(_to != address(0), "invalid address");
        require(usd_prices[seasonType] > 0, "No price setting for the NFT type");
        require(nft_dueday[seasonType] > 0, "No due day setting for the NFT type");
        require(nft_dueday[seasonType] > block.timestamp, "Due day must be in the future");     
        ...
        //2. Create NFT
        uint256 _tokenId = prefix_number + totalSupply().add(1);
        _mint(_to, _tokenId);       
        _setTokenURI(_tokenId, _tokenURI);
        emit OnNFTTokenMinted(_to,seasonType, _tokenId, _tokenURI);

        //3. make a record of locked DFA amount for this NFT.
        lockedDFAAmout = lockedDFAAmout.add(DFA_AMOUNT);

        //4. save nft information
        NFTInfo memory nftinfo = NFTInfo(_tokenId, nft_dueday[seasonType], DFA_AMOUNT, usd_prices[seasonType]);
        nfts_info[_tokenId] = nftinfo;
        
        //5. increase counter of minted tokens
        createdNumber = createdNumber.add(1);
        return true;
    }  
