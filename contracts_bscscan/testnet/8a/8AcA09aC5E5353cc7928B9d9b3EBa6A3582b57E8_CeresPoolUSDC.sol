...
contract ASC is ERC20MintBurn, AccessControl {
    ...
    uint256 public constant genesisSupply = 10000000e18;
    ...
    function removePool(address poolAddress) public onlyGovernance {
        ...
        for (uint i = 0; i < poolsArray.length; i++) {
            if (poolsArray[i] == poolAddress) {
                ...
                // will leave a null at this index
                break;
            }
        }
    }
...
import "../common/ERC20MintBurn.sol";
import "../common/AccessControl.sol";
import "../oracle/UsdcUsdChainLinkConsumer.sol";
import "./ASC.sol";

contract CRS is ERC20MintBurn, AccessControl {
    ...
    modifier onlyStaking() {
        require(staking[msg.sender] == true, "Only staking!");
        _;
    }
    ...
    // Add staking
    // ------------------------------------------------------------------------
    function addStaking(address stakingAddress) public onlyGovernance {
        stakingArray.push(stakingAddress);
        staking[stakingAddress] = true;
    }
    ...
    // ------------------------------------------------------------------------
    // Called by staking, mint new CRS
    // ------------------------------------------------------------------------
    function stakingMint(address to, uint256 amount) public onlyStaking {
        super._mint(to, amount);
        emit CRSMinted(msg.sender, to, amount);
    }
...
import "../interface/IOracle.sol";
import "../library/PoolLibrary.sol";

abstract contract CeresPool is AccessControl {
    ...
    address public ceresAnchorAddress;
    ...
    //****************
    modifier onlyGovernance() {
        require(msg.sender == maintainer || msg.sender == owner, "Only Governance!");
        _;
    }
    ...
    // Set staking address
    // ------------------------------------------------------------------------
    function setStakingAddress(address tokenAddress, address _stakingAddress) public onlyGovernance {
        stakingAddress[tokenAddress] = _stakingAddress;
    }
    ...
    function setCeresAnchor(address anchorAddress) public onlyGovernance {
        ...
        ceresAnchorAddress = anchorAddress;
    }
    ...
    // EVENTS
    //****************
    event PoolMint(uint256 collateralAmount, uint256 crsAmount, uint256 ascOut);
    event PoolRedeem(uint256 ascAmount, uint256 usdcOut, uint256 crsOut);
}
...
contract CeresPoolUSDC is CeresPool {
    ...
    // Constructor
    // ------------------------------------------------------------------------
    constructor(address usdc, address asc, address crs) {
    ...
    function determine() external onlyGovernance override {
        ...
        if (ascPrice > PRICE_TARGET + ceresAnchor.priceBand()) {
            console.log("mint caused:", ascPrice);
            mint(85000000000000000000, 15000000000000000000, 100000000000000000000);
        } else if (ascPrice < PRICE_TARGET - ceresAnchor.priceBand()) {
            console.log("redeem caused:", ascPrice);
            redeem(100000000000000000000, 85000000000000000000, 15000000000000000000);
        }
    ...
    function mint(uint256 usdcAmount, uint256 crsAmount, uint256 ascOut) internal override {

        // approve staking vault to this pool
        address usdcStakingAddr = stakingAddress[usdcAddress];
        IStaking usdcStaking = IStaking(usdcStakingAddr);
        usdcStaking.approvePool(usdcAmount);

        address crsStakingAddr = stakingAddress[crsAddress];
        IStaking crsStaking = IStaking(crsStakingAddr);
        crsStaking.approvePool(crsAmount);

        // collateral save
        USDC.transferFrom(usdcStakingAddr, address(this), usdcAmount);

        // crs burn
        coinCRS.poolBurnFrom(crsStakingAddr, crsAmount);

        // mint to 2 staking address according to collateral ratio
        uint256 colRaito = ceresAnchor.collateralRatio();
        coinASC.poolMint(usdcStakingAddr, ascOut * colRaito / CERES_PRECISION);
        coinASC.poolMint(crsStakingAddr, ascOut * (1e6 - colRaito) / CERES_PRECISION);

        emit PoolMint(usdcAmount, crsAmount, ascOut);
    }

    function redeem(uint256 ascAmount, uint256 usdcOut, uint256 crsOut) internal override {

        // approve staking vault to this pool
        address ascStakingAddr = stakingAddress[ascAddress];
        IStaking ascStaking = IStaking(ascStakingAddr);
        ascStaking.approvePool(ascAmount);

        // asc burn
        coinASC.poolBurnFrom(ascStakingAddr, ascAmount);

        // collateral transfer to staking
        USDC.transfer(ascStakingAddr, usdcOut);

        // crs mint to staking
        coinCRS.poolMint(ascStakingAddr, crsOut);

        emit PoolRedeem(ascAmount, usdcOut, crsOut);
    }
}
...
interface IStaking {
    ...
    function approvePool(uint256 shareAmount) external;

    function stakingYield() external;

    function setYieldInterval(uint256 newInterval) external;

    function setYieldPercent(uint256 newPercent) external;
...
library PoolLibrary {
    ...
    function calcMint1t1ASC(uint256 col_price, uint256 collateral_amount_d18) public pure returns (uint256) {
        return (collateral_amount_d18.mul(col_price)).div(1e6);
    }

    function calcMintAlgorithmicASC(uint256 css_price_usd, uint256 css_amount_d18) public pure returns (uint256) {
        return css_amount_d18.mul(css_price_usd).div(1e6);
    }


    function calcMintFractionalASC(MintCRS_Params memory params) internal pure returns (uint256, uint256) {
    ...
    function calcRedeem1t1ASC(uint256 col_price_usd, uint256 ASC_amount) public pure returns (uint256) {
        return ASC_amount.mul(1e6).div(col_price_usd);
    }
    ...
    function calcRecollateralizeASCInner(
        uint256 collateral_amount,
        uint256 col_price,
        uint256 global_collat_value,
        uint256 ASC_total_supply,
        uint256 global_collateral_ratio
    ) public pure returns (uint256, uint256) {
        uint256 collat_value_attempted = collateral_amount.mul(col_price).div(1e6);
        uint256 effective_collateral_ratio = global_collat_value.mul(1e6).div(ASC_total_supply);
        uint256 recollat_possible = (global_collateral_ratio.mul(ASC_total_supply).sub(ASC_total_supply.mul(effective_collateral_ratio))).div(1e6);
...
contract UsdcUsdChainLinkConsumer is IChainlink{
