...
import './libraries/SDBswapLibrary.sol';
import './interfaces/ISDBswapRouter01.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWETH.sol';

contract SDBswapRouter01 is ISDBswapRouter01 {
    ...
    function addLiquidityETH(
        ...
        address pair = SDBswapLibrary.pairFor(factory, token, WETH);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            ISDBswapPair(SDBswapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, SDBswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    {
        require(path[0] == WETH, 'SDBswapRouter: INVALID_PATH');
        amounts = SDBswapLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'SDBswapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(SDBswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'SDBswapRouter: INVALID_PATH');
        amounts = SDBswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'SDBswapRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, SDBswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public pure override returns (uint amountIn) {
        return SDBswapLibrary.getAmountOut(amountOut, reserveIn, reserveOut);
    }
...
library SDBswapLibrary {
    ...
    // вычисляет адрес CREATE2 для пары без каких-либо внешних вызовов
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'e66a0a260f387b4ee2264552dda7b65a2da0da7c3972a3688956a1fb755c79f0' // init code hash
            ))));
    }
    // выбирает и сортирует резервы для пары
    // fetches and sorts the reserves for a pair
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        (address token0,) = sortTokens(tokenA, tokenB);
        pairFor(factory, tokenA, tokenB);
        (uint reserve0, uint reserve1,) = ISDBswapPair(pairFor(factory, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }
    // с учетом некоторой суммы актива и резервов пары возвращает эквивалентную сумму другого актива
...
//// с учетом входной суммы актива и парных резервов возвращает максимальную выходную сумму другого актива
