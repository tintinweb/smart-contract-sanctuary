...
contract AIRDROP_TOKEN  
{
 ...
 function multisendTOKEN(address payable[]  memory  _contributors, uint256[] memory _amount) public payable {
        ...
        for (i; i < _contributors.length; i++) {
            ...
            total = total.sub(_amount[i]);
         	MyToken.transferFrom(msg.sender, _contributors[i], _amount[i]);
        }
