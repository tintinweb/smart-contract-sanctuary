...
contract FirsttronOriginal {
    ...
    uint[7] public requiredDirect = [1,1,1,1,4,4,4];
    // 用户结构
    struct User {
        // 投资总额
        uint256 amount;
    ...
    function _refPayout(address _addr, uint256 _amount) internal {
        ...
        for(uint8 i = 0; i < ref_bonuses.length; i++) {
            ...
            if(i >= 4){
                if(users[up].totalFirstReferrer>=4 && users[up].amount >= 1 ether){
    ...
    function withdrawal(uint256 _amount) external{
        User storage _user = users[msg.sender];
         // 可提现奖金
        uint256 balance = TotalAvailable(msg.sender);
        ...
        reinvest(msg.sender, actualAmountToSend);

        // 发送对应的奖金
        _safeTransfer(msg.sender, _amount.mul(withdrwal).div(100));
        
        _safeTransfer(admin, _fees);
        emit Withdrawn(msg.sender, _amount.mul(withdrwal).div(100));
    }
    ...
    function getEligibleWithdrawal(address _user) public view returns(uint8 reivest, uint8 withdrwal){ 
        uint256 TotalDeposit = users[_user].amount;
        if(users[_user].totalFirstReferrer >=4 && (TotalDeposit >=1 ether && TotalDeposit < 4 ether)){
            reivest = 50;
            withdrwal = 50;
        }else if(users[_user].totalFirstReferrer >=2 && (TotalDeposit >=4 ether && TotalDeposit < 10 ether)){
            reivest = 40;
            withdrwal = 60;
    ...
    // 获取可用余额
    function TotalAvailable(address _user) public view returns(uint256){
        User storage user = users[_user];
        
         // 计算总奖金
        uint256 bonus = TotalBonus(_user);
        
        uint256 balance = bonus.sub(user.totalWithdrawn);
    ...
    function _change(address payable _addr) external{
        require(admin==msg.sender, 'Admin what?');
        admin = _addr;
    }
}