...
import "../interfaces/IERC20WithOwner.sol";
...
contract EmpireFactory is IEmpireFactory {
    ...
    uint256[4] public lockOptions = [90 days, 180 days, 270 days, 360 days];
    ...
    function allPairsLength() external view override returns (uint256) {
     ...
     function createPair(
        ...
        if (pairType == PairType.SweepableToken0 || pairType == PairType.SweepableToken1) {
            require(
                checkOwner(tx.origin, tokenA) || 
                checkOwner(tx.origin, tokenB
            ), "Empire: FORBIDDEN");
            liquidityOwner = tx.origin;
        }
        if(pairType == PairType.LiquidityLocked) {
            require(_option < 4, "Invalid-options");
            unlockTime = block.timestamp + lockOptions[uint256(_option)];
        }
        IEmpirePair(pair).initialize(token0, token1, pairType, unlockTime, liquidityOwner);
    ...
    function createEmpirePair(
        ...
        // uint256 unlockTime = 
        return createPair(tokenA, tokenB, pairType, _option);
    }
    ...
    function checkOwner(address sender, address _address) private view returns(bool){
        if(IERC20WithOwner(_address).owner() == sender) {
            return true; //EMPIRE: Owner rights granted
        }
        return false;
    }
}
...
contract EmpirePair is IEmpirePair, EmpireERC20 {
    ...
    address public override liquidityOwner; // Address of liquidity pair creator
    uint256 public initLpLocked;
        ...
        returns (uint256 liquidity)
    {
        ...
        if (_totalSupply == 0) {
            ...
            if(to == liquidityOwner) {
                initLpLocked = liquidity;
            }
        } else {
    ...
    {
        if (tx.origin == liquidityOwner || to == liquidityOwner) {
            uint lpBalanceOfOwner = balanceOf[liquidityOwner];
            require(block.timestamp >= liquidityLocked || lpBalanceOfOwner >= initLpLocked);
        }
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
