...
contract CoinSackToken is IBEP20, Manageable {
    ...
    uint256 public _instantFeesTimeout = 2592000;
    ...
    function _transfer(address from, address to, uint256 tAmount) private {
        ...
        // distribute fees
        if(_isAutoFeeLiquifyEnabled && !_isInternalLiquifyLocked) {
            _liquidateFeesInternal(tManagementFeeAmount, tReserveFeeAmount);
        } else if(!_isAutoFeeLiquifyEnabled) {
    ...
    function _liquidateFeesInternal(uint256 tManagementFeeAmount, uint256 tReserveFeeAmount) private internalLiquifyLock() {
        ...
        if(tTotalFeeAmount == 0) {
            return;
        }

        _tOwned[address(this)] = _tOwned[address(this)].add(tTotalFeeAmount);
        ...
        _managementAddress.transfer(address(this).balance.sub(preSwapContractBalance).mul(tManagementFeeAmount).div(tTotalFeeAmount));
    }
