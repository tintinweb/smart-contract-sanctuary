...
contract Denominator is AccessControl, ReentrancyGuard {
    ...
    constructor(address _seed, address _metaSeed, uint256 _denomenateValue) {
    ...
    function denominate(uint256 _amount) public nonReentrant {
        require(_amount > 0, "Zero amount");
        IERC20(seedAddress).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 denomenated = _amount / denomenateValue;
        IERC20MintableBurnable(metaSeedAddress).transfer(msg.sender, denomenated);
    }

    function setDenomenateValue(uint256 _value) public onlyRole(ADMIN_ROLE) {
