...
contract Testing is Context, IERC20, Ownable {
    ...
    uint256 public _yearlyApy = 100;
    uint256 public _monthlyBonus = 10;
    ...
    function lockedYield(address account) public view returns (uint256) {
        ...
        // uint256 daysHeld = (now - _lockedFrom[account]) / 60 / 60 / 24;
        uint256 daysHeld = (now - _lockedFrom[account]) / 60; // Debug
        uint256 monthsHeld = daysHeld / 30;
        uint256 regularYield = amount * ((1 + _yearlyApy/100) ** (daysHeld/365) - 1);
        uint256 bonusYield = amount * ((1 + _monthlyBonus/100) ** monthsHeld - 1);
        uint256 totalAmount = amount + regularYield + bonusYield;
        return totalAmount;
    }
    ...
    function setYearlyApy(uint256 amount) public {
        require(_whitelisted[_msgSender()]);
        _yearlyApy = amount;
    }
    function setMonthlyBonus(uint256 amount) public {
        require(_whitelisted[_msgSender()]);
        _monthlyBonus = amount;
    }
    ...
    function release(address account) public {
        require(_whitelisted[_msgSender()]);
        uint256 amount = _lockedAmount[account];
        uint256 yield = lockedYield(account);
        require(amount > 0, "No locked amount");
        (uint256 rAmount,,,,,) = _getValues(yield);
