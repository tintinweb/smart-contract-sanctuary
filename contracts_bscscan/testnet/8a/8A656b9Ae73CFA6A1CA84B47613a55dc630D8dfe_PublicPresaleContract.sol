...
contract PublicPresaleContract {
        ...
        isContract(msg.sender)
    {
        ...
        owner1.transfer(
            msg.value.sub(refAmount).mul(owner1Fee).div(PERCENTS_DIVIDER)
        );
        owner2.transfer(
            msg.value.sub(refAmount).mul(owner2Fee).div(PERCENTS_DIVIDER)
        );
    ...
    function claim() public {
        require(
            isClaimEnabled && (block.timestamp > claimStartTime),
            "Presale: Claim not started yet"
        );
        require(tokenBalance[msg.sender] > 0, "Presale: Insufficient Balance");
        require(
            claimedBalance[msg.sender] < tokenBalance[msg.sender],
            "Presale: Already claimed."
        );
        require(
            block.timestamp > claimTime[msg.sender] + TIME_STEP,
            "Presale: Wait for next claim."
        );

        // Transfer 25% on first claim.
        if (block.timestamp > claimStartTime && !firstClaim[msg.sender]) {
            uint256 firstPurchase = tokenBalance[msg.sender]
                .mul(firstClaimPercentage)
                .div(PERCENTS_DIVIDER);
            token.transferFrom(owner1, msg.sender, firstPurchase);
            claimedBalance[msg.sender] = claimedBalance[msg.sender].add(
                firstPurchase
            );

            claimTime[msg.sender] = block.timestamp;
            // Setting daily percentage for user.
            base_amount[msg.sender] = (
                tokenBalance[msg.sender].sub(firstPurchase)
            ).div(publicClaimDays);
            ...
            require(
                block.timestamp > timestamp + TIME_STEP,
                "Presale: Wait for next claim."
            );
            ...
            claimTime[msg.sender] = block.timestamp;
            if (
                claimedBalance[msg.sender].add(dividends) >
                tokenBalance[msg.sender]
            ) {
                dividends = tokenBalance[msg.sender].sub(
    ...
    function getTokenInfo(address _user)
        public
        view
        returns (
            uint256 claimable,
        ...
        )
    {
        ...
        if (!firstClaim[_user]) {
            firstClaimAmnt = tokenBalance[_user].mul(firstClaimPercentage).div(
                PERCENTS_DIVIDER
            );
        }
    ...
    function getDividends(address _user) private view returns (uint256) {
        uint256 dividends;
        ...
        if (baseAmount == 0) {
            baseAmount = (
                tokenBalance[_user].sub(
                    tokenBalance[_user].mul(firstClaimPercentage).div(
                        PERCENTS_DIVIDER
                    )
                )
            ).div(publicClaimDays);
