...
contract ChildDeployer is IChildDeployer {
  ...
  function deploy(address mom, string memory name, uint256 birthYear) internal returns (address house) {
    ...
    house = address(new SonContract{salt: keccak256(abi.encode(mom, name, birthYear))}(address(this)));
    delete parameters;
  }
}
...
contract SonContract {
  ...
  constructor(address deployer) {
    ...
    (address _mom, string memory _name, uint256 _birthYear) = IChildDeployer(deployer).parameters();
