...
contract Activity is AccessControl, MultiSig {
    ...
    uint256 public sendAccountLimit = 500;
    mapping(string => bool) public activities;
    ...
    event SetSendAccountLimit(uint256 limit_);
    ...
    function setSendAccountLimit(uint256 limit) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (sendAccountLimit != limit) {
            sendAccountLimit = limit;
        ...
        external
    {
        ...
        RewardRecord memory record = rewardRecords[txId_];
        if (record.token == address(0)) {
            ...
            rewardRecords[txId_] = record;
        }

        if (_sign(txId_)) {
            _sendRewards(actityId_, record.token, record.amount, record.to);
        }
    }
        ...
        internal 
    {
        ...
        emit SendRewards(actityId_, token_, to_, amount_);
    }
}
