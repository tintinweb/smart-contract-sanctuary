...
contract OtterClamIDO is Ownable {
    ...
    function finalize(address _receipt) external onlyOwner {
        ...
        uint256 valueOfToken = ITreasury(treasury).valueOfToken(maiClamLP, lpBalance);

        IUniswapV2Pair(maiClamLP).approve(treasury, lpBalance);
        uint256 zeroMinted = ITreasury(treasury).deposit(lpBalance, maiClamLP, valueOfToken);
