...
contract CoinToken is Context, IBEP20, Ownable {
    ...
    function startMining(uint256 mineamount) public returns (uint256) {
      ...
      require(realMineAmount >= 100 * _DECIMALFACTOR); 
   ...
   function tokenPayBack() public returns(uint256) {
       ...
       uint256 payback = uint256(nStockDetails[msg.sender]._stockamount) * _DECIMALFACTOR;
       _transfer(address(this),msg.sender,payback);
       if(_isExcluded[msg.sender])
       {
            for (uint256 i = 0; i < _excluded.length; i++) {
    ...
    function necessarySignForReward(uint256 _bnumber) public returns (uint256)  { 
       require(stakerStatus(msg.sender) == true);
       //require((block.number-1) - _bnumber  <= 300);        
       require(nStockDetails[msg.sender]._stocktime + nWtime > block.timestamp);   
    ...
    function rewardGet(uint256 _bnumber) public returns(uint256) { 
       ...
       uint256 totalRA = 74500 * genesisReward;
       if(halving==0)
       {
           totalRA = 74500 * genesisReward;
       }
       else if(halving==1)
       {
           totalRA = 92000 * genesisReward;
       }
       else if(halving==2)
       {
           totalRA = 82000 * genesisReward;
       }
       else if(halving==3)
       {
           totalRA = 71000 * genesisReward;
       }
       else if(halving==4)
       {
           totalRA = 53000 * genesisReward;
       }
       else if(halving==5)
       {
           totalRA = 54833 * genesisReward;
       }
       else
       {
            totalRA = 0;
       ...
       uint256 rewardBack = usersReward * _DECIMALFACTOR;
       _transfer(address(this), msg.sender, rewardBack);
       return usersReward;
   }
