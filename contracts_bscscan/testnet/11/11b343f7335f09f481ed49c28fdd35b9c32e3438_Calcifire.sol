...
contract Calcifire is Context, IERC20, Ownable {
    ...
    mapping (string => address) private _boostPairUSD;
    ...
    // Add a new BoostPair
    function addBoostPair(address pairAddress, string memory tokenSymbol) public onlyOperator {
        require(_boostPairUSD[tokenSymbol] == address(0x0), "addBoostPair: same pair cant be added twice");
        _boostPairUSD[tokenSymbol] = pairAddress;
    }

    // Get BoostPair by token symbol
    function getBoostPair(string memory tokenSymbol) public view returns(address) {
        return(_boostPairUSD[tokenSymbol]);
    }
