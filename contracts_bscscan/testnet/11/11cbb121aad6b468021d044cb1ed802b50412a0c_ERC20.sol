  ...
  , IERC20Metadata
{
  ...
  function setTax(uint256 tax_, uint256 divider_) public onlyOwner swapping { _tax = tax_;_taxDivider = divider_; }
  function setTaxCollector(address input_) public onlyOwner swapping { require(input_ != address(0), "Zero Address."); _taxCollector = input_; }
  ...
  constructor(
    ...
    _mint(_msgSender(), TOTALSUPPLY_ * 10 ** uint256(_decimals));
  }
  ...
  function approveMax(address spender) public virtual returns (bool) {
    _approve(_msgSender(), spender, ~uint256(0));
    return true;
  }
  ...
  function takeFee (uint256 amount_) private returns(uint256){
    if (_tax == 0 || _taxCollector == address(0)) return amount_;
  ...
  function distributeFee () private swapping {
    if (_tax == 0 || _taxCollector == address(0)) return;
    uint256 swapAmount = _balances[address(this)];
    
    if (_balances[address(this)] > 0 && _tax > 0 && _taxCollector != address(0)) {
