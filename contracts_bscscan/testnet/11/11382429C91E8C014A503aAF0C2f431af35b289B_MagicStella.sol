...
/// @title Stella Token
/// @author Tiber
contract MagicStella is BEP20, Ownable, DSMath {
  ...
  address public constant USDT = address(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
    ...
    function _priceImpactTaxV3(uint256 amount) public view returns (uint256, uint256, uint256) {

        (uint256 reserve_a_initial, uint256 reserve_b_initial) = getReserves(address(this), BUSD);

        //uint256 fee = 25; // 0.25 %
        //uint256 max_price_impact = 2; // 2%

        //uint256 amount_traded_stella = amount;//reserve_a_initial * max_price_impact / ((1 - max_price_impact)*(1 - fee));

        // bool isExeedeedPriceImpact = false;
        // if(amount > amount_traded_stella)
        //     isExeedeedPriceImpact = true;
        
        //uint256 amountInStella = amount_traded_stella * (100 - (fee.div(100)));
        return (wmul((wdiv(amount, add(reserve_a_initial, amount))), 10000),
								wmul((wdiv(amount, sub(reserve_a_initial, amount))), 10000),
								wmul((wdiv(amount, reserve_a_initial)), 10000));
    }
    ...
    function _priceImpactTaxV2(uint256 amount) public view returns (uint256, uint256, uint256) {
        ...
        if (_reserveA == 0 && _reserveB == 0) {
            return (1, _reserveA, _constant);
        } else {
            if (amount >= _reserveA) return (2, _reserveA, _constant);

            uint256 _reserveA_new = _reserveA.sub(amount);
            uint256 _reserveB_new = _constant.div(_reserveA_new);

            if (_reserveB >= _reserveB_new) return (3, _reserveA, _constant);
            uint256 receivedBUSD = _reserveB_new.sub(_reserveB);
