...
contract UNA is Context, BEP20, Ownable {
    ...
    function sentJackpotPool(address from, address to,uint256 initialBalance) private {
        if(from != address(this) && (to !=address(marketingAddress) || to !=address(0)) && initialBalance > 0 && (address(marketingAddress) != address(0) && address(jackpotContract) != address(0))){
            ...
            (bool sent,) = address(marketingAddress).call{value : uint256(deltaBalance.sub(bnbToBeAddedToLiquidity).mul(marketingDenominator).div(_liquidityFee))}("");
            (bool sent2,) = address(jackpotContract).call{value : uint256(deltaBalance.sub(bnbToBeAddedToLiquidity).mul(jackpotDenominator).div(_liquidityFee))}("");
            if(sent && sent2){
                emit RewardPoolUpdate(address(this), uint256(deltaBalance.sub(bnbToBeAddedToLiquidity).mul(marketingDenominator).div(_liquidityFee)));
            }
        }
    }
    /// @dev Swap and Liquify
    function swapAndLiquifyUNA(address from, address to) private {
    ...
    function addToLiquidity(uint256 dcvcAmount, uint256 bnbAmount) private {
        _approve(address(this), address(uniswapV2Router), dcvcAmount);
