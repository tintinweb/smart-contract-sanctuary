...
contract Curiosity is Context, IERC20, Ownable {
    ...
    uint private startingSupply = 100000000000;  // total supply
    ...
    string private _name = "Curiosity"; 
    string private _symbol = "CURIO";
    ...
    address payable public _marketingWallet = payable(0x0C9186611Cfc19B11D712112fE33E86BF3eB9678); //select your marketing address
    address payable public _developerWallet = payable(0x17AE8871759a0A477c6018DC12048E7e702FaD4F); // development team address
    ...
    // Maximum wallet size is 0.5% of the total supply.
    ...
    constructor (uint256 _snipeBlockAmt) {
        ...
        _isExcluded[_developerWallet] = true; 
        _excluded.push(_developerWallet);
        // DxLocker Address (BSC)
        _isExcludedFromFee[0x17AE8871759a0A477c6018DC12048E7e702FaD4F] = true;
        _isExcluded[0x17AE8871759a0A477c6018DC12048E7e702FaD4F] = true;
        _excluded.push(0x17AE8871759a0A477c6018DC12048E7e702FaD4F);
    ...
    function setBuyTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee, uint256 burnFee) external onlyOwner() {
        ...
        require(burnFee <= maxBurnFee); // Prevents owner from abusing fees.
    ...
    function setSellTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee, uint256 burnFee) external onlyOwner() {
        require(reflectFee <= maxReflectFee); // Prevents owner from abusing fees.
        require(liquidityFee <= maxLiquidityFee); // Prevents owner from abusing fees.
        require(marketingFee <= maxMarketingFee); // Prevents owner from abusing fees.
        require(burnFee <= maxBurnFee); // Prevents owner from abusing fees.
    ...
    function setBNBRatio (uint256 liquidityRatio, uint256 marketingRatio) external onlyOwner() {
        require (marketingRatio < liquidityRatio);
        _liquidityRatio = liquidityRatio;
        _marketingRatio = marketingRatio;
    }
    ...
    function setMarketingWallet(address payable newWallet) external onlyOwner {
     ...
     function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //Send to Investor address
        transferToAddressETH(_marketingWallet, transferredBalance.div(_liquidityFee).mul(_buyMarketingFee));  
...
,        address to,
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        if (_marketingRatio + _liquidityRatio == 0)
            return;
        uint256 toMarketing = contractTokenBalance.mul(_marketingRatio).div(_marketingRatio.add(_liquidityRatio));
    ...
    function calculateBurnFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_burnFee).div(masterTaxDivisor);
    }
    ...
    function adjustTaxes(address from, address to, uint256 amount, bool takeFee) internal {
        if (!takeFee) {
            ...
            if (amount >= (_tTotal / 100)) {
                _reflectFee = _sellReflectFee * 1;
                _liquidityFee = _sellLiquidityFee * 6;
                _marketingFee = _sellMarketingFee * 3;
                _burnFee = _sellBurnFee * 1;
            } else {
