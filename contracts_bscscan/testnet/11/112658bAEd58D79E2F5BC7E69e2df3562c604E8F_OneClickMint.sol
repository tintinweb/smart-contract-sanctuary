...
contract OneClickMint is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 private constant SLIPPAGE_MAX = 1e17; // 10%
    uint256 private constant OFFSET_MAX = 5e15; // 5%
    ...
    function initialize(
        ...
        // to avoid "Not enough Share inputted"
        // the more offset, the more remaining share received
        offset = 11e14; // 0.11%
    ...
    function quickMint(uint256 _collateralAmount, uint256 _kusdOutMin)
        external
        ...
        nonReentrant
    {
        ...
        uint256 _priceCollateral = oracle.consult(address(collateral), 1e18);

        require(_sharePrice > 0, "Invalid share price");
        require(_priceCollateral > 0, "Invalid collateral price");

        uint256 _swapCollateralAmount = _collateralAmount
            .mul(PRICE_PRECISION - _tcr + offset)
            .div(PRICE_PRECISION);
        ...
        uint256 _collateralValue = _collateralAmount.mul(_priceCollateral).div(
            PRICE_PRECISION
        );

        uint256 _shareValue = (_collateralValue * (RATIO_PRECISION - _tcr)) /
            RATIO_PRECISION;

        uint256 _minShareAmount = (_shareValue *
            PRICE_PRECISION *
            (RATIO_PRECISION - slippage)) /
            _sharePrice /
            RATIO_PRECISION;
    ...
    function setOffset(uint256 _offset) external {
        require(hasRole(MAINTAINER, msg.sender), "Caller is not a maintainer");
        require(_offset <= OFFSET_MAX, "OFFSET TOO HIGH");
        offset = _offset;
    }
