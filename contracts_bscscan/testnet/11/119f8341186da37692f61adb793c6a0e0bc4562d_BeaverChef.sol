...
// Dependency file: contracts/interfaces/IPoolStrategy.sol
...
interface IPoolStrategy {
    ...
    event ProfitPaid(address indexed user, uint256 indexed profit);
    event Harvested(uint256 indexed profitAmount, uint256 indexed beaverAmount);
}


// Root file: contracts/beaver/BeaverChef.sol
...
// import "contracts/interfaces/IPoolStrategy.sol";

contract BeaverChef is IBeaverChef, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    /* ========== STATE VARIABLES ========== */

    IBeaverToken public beaver;
    address[] private _vaultList;
    mapping(address => VaultInfo) vaults;
    mapping(address => mapping(address => UserInfo)) vaultUsers;

    IBeaverMinter public minter;
    ...
    modifier onlyVaults() {
        require(vaults[msg.sender].token != address(0), "BeaverChef: caller is not on the vault");
        _;
    }
    ...
    modifier updateRewards(address vault) {
        ...
        if (block.number > vaultInfo.lastRewardBlock) {
            ...
            if (tokenSupply > 0) {
                ...
                uint256 rewards = multiplier.mul(beaverPerBlock()).mul(vaultInfo.allocPoint).div(totalAllocPoint);
                vaultInfo.accBeaverPerShare = vaultInfo.accBeaverPerShare.add(rewards.mul(1e12).div(tokenSupply));
            }
    ...
    event BeaverRewardPaid(address indexed user, address indexed vault, uint256 amount);

    /* ========== INITIALIZER ========== */

    constructor(IBeaverToken _beaver, uint256 _startBlock) {
        beaver = _beaver;
    ...
    function beaverPerBlock() public view override returns (uint256) {
        if (address(minter) != address(0)) {
            return minter.beaverPerBlock();
        }
    ...
    function tokenSupplyOf(address vault) public view returns (uint256) {
        return IPoolStrategy(vault).totalSupply();
    }
    ...
    function pendingBeaver(address vault, address user) public view override returns (uint256) {
        UserInfo storage userInfo = vaultUsers[vault][user];
        VaultInfo storage vaultInfo = vaults[vault];

        uint256 accBeaverPerShare = vaultInfo.accBeaverPerShare;
        uint256 tokenSupply = tokenSupplyOf(vault);
        if (block.number > vaultInfo.lastRewardBlock && tokenSupply > 0) {
            uint256 multiplier = timeMultiplier(vaultInfo.lastRewardBlock, block.number);
            uint256 beaverRewards = multiplier.mul(beaverPerBlock()).mul(vaultInfo.allocPoint).div(totalAllocPoint);
            accBeaverPerShare = accBeaverPerShare.add(beaverRewards.mul(1e12).div(tokenSupply));
        }
        return userInfo.pending.add(userInfo.balance.mul(accBeaverPerShare).div(1e12).sub(userInfo.rewardPaid));
    }
    ...
    function addVault(
        ...
        require(vaults[vault].token == address(0), "BeaverChef: vault is already set");
        bulkUpdateRewards();
    ...
    function updateVault(address vault, uint256 allocPoint) public onlyOwner {
        require(vaults[vault].token != address(0), "BeaverChef: vault must be set");
        bulkUpdateRewards();
    ...
    function setMinter(address _minter) external onlyOwner {
        require(address(minter) == address(0), "BeaverChef: setMinter only once");
        minter = IBeaverMinter(_minter);
    }
    ...
    function notifyDeposited(address user, uint256 amount) external override onlyVaults updateRewards(msg.sender) {
        ...
        uint256 pending = userInfo.balance.mul(vaultInfo.accBeaverPerShare).div(1e12).sub(userInfo.rewardPaid);
        userInfo.pending = userInfo.pending.add(pending);
        userInfo.balance = userInfo.balance.add(amount);
        userInfo.rewardPaid = userInfo.balance.mul(vaultInfo.accBeaverPerShare).div(1e12);
    ...
    function safeBeaverTransfer(address user) external override onlyVaults updateRewards(msg.sender) returns (uint256) {
        UserInfo storage userInfo = vaultUsers[msg.sender][user];
        VaultInfo storage vaultInfo = vaults[msg.sender];
        ...
        minter.mintFor(user, msg.sender, amount);
        emit BeaverRewardPaid(user, msg.sender, amount);
        return amount;
    }

    function bulkUpdateRewards() public override {
    ...
    function recoverToken(address _token, uint256 amount) external onlyOwner {
        require(_token != address(beaver), "BeaverChef: cannot recover beaver token");
