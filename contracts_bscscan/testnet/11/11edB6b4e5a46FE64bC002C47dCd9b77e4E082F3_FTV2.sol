...
import "./abstracts/core/Supply.sol";
...
contract FTV2 is IERC20Metadata, Context, Ownable, ReentrancyGuard, Tokenomics, RFI, Supply, Liquify, Expensify, Buyback, Collateralize, Distribution {
...
/* ---------------------------- Custom functions ---------------------------- */
	 ...
	 */
	function maxSellAlowancePerDay() public view returns(uint256) {
		// 0.1% of total circulating supply.
		return totalCirculatingSupply().mul(1).div(1000);
	}

	/* -------------------------------- Overrides ------------------------------- */

	function midTransferTriggers(address from) internal override {
		uint256 contractTokenBalance = balanceOf(address(this));
		if (canLiquify(from, contractTokenBalance)) {

			// TODO: investigate minToLiquify(used traditionally) vs accumulatedForLiquidity
			swapAndLiquify(accumulatedForLiquidity);
		}

		if (canExpensify(from, contractTokenBalance)) {

			// TODO: investigate minTo...(used traditionally) vs accumulatedFor...
			expensify(accumulatedForExpenses);
		}

		if (canSellForBuyback(from, contractTokenBalance)) {

			// TODO: investigate minTo...(used traditionally) vs accumulatedFor...
			sellForBuyback(accumulatedForBuyback);
		}

		if (canSellForCollateral(from, contractTokenBalance)) {

			// TODO: investigate minTo...(used traditionally) vs accumulatedFor...
			sellForCollateral(accumulatedForCollateral);
		}

		if (canSellForDistribution(from, contractTokenBalance)) {
...
/* --------------------------- Triggers and limits -------------------------- */
	...
	// Wallet hard cap 0.1% of total supply
	uint256 public maxWalletSize = _tTotal.mul(1).div(100);
	// 0.05% per transaction
	uint256 public maxTxAmount = maxWalletSize;
...
/* --------------------------------- Helpers -------------------------------- */
	...
	/**
	* @notice Swaps one wallet with another in _isExcludedFromFee mapping.
	*/
	function swapExcludedFromFee(address newWallet, address oldWallet) internal {
		// Switch wallets in excluded from fees mapping.
...
abstract contract RFI is IERC20, Ownable, Tokenomics, Pancake {
	...
	constructor() {
		...
		// Exclude owner and this contract from any fees
...
/* --------------------------------- Custom --------------------------------- */
	...
	function _transfer(
		...
		// Ensure max transaction amount and wallet cap
		if (
			from != owner() &&
			to != owner() &&
			to != address(0) &&
			...
			require(
				toBalance + amount <= maxWalletSize,
				"Exceeds maximum wallet size allowed."
			);
		}

		midTransferTriggers(from);

		// if (canLiquify(from)) {

		// 	// TODO: uncomment this before testing on testnet and mainnet !!!!!!!!!!!!!!!!!!!!!!!
		// 	// TODO: investigate minToLiquify(used traditionally) vs accumulatedForLiquidity
		// 	// swapAndLiquify(accumulatedForLiquidity);
		// }

		// if (canExpensify(from)) {

		// 	// TODO: investigate minTo...(used traditionally) vs accumulatedFor...
		// 	expensify(accumulatedForExpenses);
		// }

		// if (canSellForBuyback(from)) {

		// 	// TODO: investigate minTo...(used traditionally) vs accumulatedFor...
		// 	sellForBuyback(accumulatedForBuyback);
		// }

		// if (canSellForCollateral(from)) {

		// 	// TODO: investigate minTo...(used traditionally) vs accumulatedFor...
		// 	sellForCollateral(accumulatedForCollateral);
		// }

		// if (canSellForDistribution(from)) {

		// 	// TODO: investigate minTo...(used traditionally) vs accumulatedFor...
		// 	sellForDistribution(accumulatedForDistribution);
		// }

		// By default take the fees on every transaction...
		bool takeFee = true;
		// ... but if sender or receiver is excluded from fees then don't take.
	 ...
	 * @param takeFee Toggle on/off fees.
	 */
	...
	function midTransferTriggers(address from) virtual internal {
...
abstract contract Liquify is Tokenomics, Pancake, RFI {
	using SafeMath for uint256;
	...
	* liquidity event.
	* @param from address of a transaction sender passed from _transfer function.
	*/
	function canLiquify(address from, uint256 contractTokenBalance) 
		internal 
		view
		returns(bool) 
	{
		...
		return !inSwapAndLiquify 
			&& contractTokenBalance >= accumulatedForLiquidity 
...
abstract contract Expensify is Ownable, Helpers, Tokenomics, Pancake {
	...
	// Prevents infinite _transfer loops.
	bool internal inExpensify;
	...
	* @param from address of a transaction sender passed from _transfer function.
	*/
	function canExpensify(address from, uint256 contractTokenBalance) 
		internal 
		view
		returns(bool) 
	{
		...
		return !inExpensify
			&& contractTokenBalance >= accumulatedForExpenses 
			&& accumulatedForExpenses >= minToExpenses
			&& from != uniswapV2Pair;
	}

/* -------------------------------- Modifiers ------------------------------- */
	modifier lockTheExpensify {
		inExpensify = true;
		_;
		inExpensify = false;
	}
...
abstract contract Buyback is Ownable, Helpers, Tokenomics, Pancake, RFI {
	...
	* @param from address of a transaction sender passed from _transfer function.
	*/
	function canSellForBuyback(address from, uint256 contractTokenBalance) 
		internal
		view
		returns(bool) 
	{
		...
		// TODO: double check if need to check all inExpensify inSwapAndLiquify etc.
		return !inSellingForBuyback
			&& contractTokenBalance >= accumulatedForBuyback 
	...
	*/
	function sellForBuyback(
		uint256 tokenAmount
	) internal lockTheSellForBuyback {
		// Must approve before swapping.
		rfiApprove(address(this), address(uniswapV2Router), tokenAmount);
...
/* -------------------------------- Modifiers ------------------------------- */
	modifier lockTheSellForBuyback {
		inSellingForBuyback = true;
		_;
		inSellingForBuyback = false;
	}
...
abstract contract Collateralize is Ownable, Helpers, Tokenomics, Pancake, RFI {
	using SafeMath for uint256;
	...
	// Prevents infinite _transfer loops.
	bool internal inSellingForCollateral;
	...
	* @param from address of a transaction sender passed from _transfer function.
	*/
	function canSellForCollateral(address from, uint256 contractTokenBalance) 
		internal 
		view
		returns(bool) 
	{
		...
		// TODO: double check if need to check all inExpensify inSwapAndLiquify etc.
		return !inSellingForCollateral
			&& contractTokenBalance >= accumulatedForCollateral 
	...
	*/
	function sellForCollateral(
		uint256 tokenAmount
	) internal lockTheSellForCollateral {
		uint256 tokensSold;
		uint256 bnbReceived;
...
/* -------------------------------- Modifiers ------------------------------- */
	modifier lockTheSellForCollateral {
		inSellingForCollateral = true;
		_;
		inSellingForCollateral = false;
	}
...
abstract contract Distribution is Ownable, Tokenomics, Pancake, RFI, Supply {
	...
	* @param from address of a transaction sender passed from _transfer function.
	*/
	function canSellForDistribution(address from, uint256 contractTokenBalance) 
		internal 
		view
		returns(bool) 
	{
		...
		// TODO: double check if need to check all inExpensify inSwapAndLiquify etc.
		return !inSellingForDistribution
			&& contractTokenBalance >= accumulatedForDistribution 
	...
	*/
	function sellForDistribution(
		uint256 tokenAmount
	) internal lockTheSellForDistribution {
		// Must approve before swapping.
		rfiApprove(address(this), address(uniswapV2Router), tokenAmount);
		uint256 bnbReceived = swapTokensForBnb(tokenAmount);
		// Increment BNB accumulator
	...
	*/
	function calcClaimableShare(address holderAddr) public view returns(uint256) {
		...
		uint256 bnbShare = balanceOf(holderAddr).mul(100).div(totalHoldingAmount);
		uint256 bnbToSend = claimableDistribution.mul(bnbShare).div(100);
	...
	*/
		...
		returns(uint256)
	{
		...
		require(!hasAlreadyClaimed(_msgSender()), "Already claimed in the current cycle.");
		if (hasCyclePassed()) {
			// Reset with same cycle hours.
			resetClaimDistributionCycle(claimCycleHours);
		}
		uint256 bnbShare = calcClaimableShare(_msgSender());
		payable(_msgSender()).transfer(bnbShare);
		claimedTimestamp[_msgSender()] = block.timestamp;
		return bnbShare;
	}

/* -------------------------------- Modifiers ------------------------------- */

	modifier lockTheSellForDistribution {
		inSellingForDistribution = true;
		_;
		inSellingForDistribution = false;
	}
...
abstract contract Pancake is Ownable {
	...
	// This will be set when we call initPCSRouter().
	...
	*/
	function initPCSRouter(address router) 
		external
		onlyOwner
	{
	...
	/* ------------------------- Buy/sell differentiator ------------------------ */
	// To keep track of buy/sel destinations.
