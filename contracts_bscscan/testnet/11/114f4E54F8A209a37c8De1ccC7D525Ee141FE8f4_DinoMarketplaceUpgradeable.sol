...
contract DinoMarketplaceUpgradeable is Pausable, Ownable, Initializable {
  ...
  function setAdminAddress(address _newAdminAddress) external onlyAdminOrOwner {
      adminAddress = _newAdminAddress;
  }
  
  /// @dev Set new token address
  /// @param _newTokenAddress - Address of new erc20 token.
  function setTokenAddress(address _newTokenAddress) external onlyAdminOrOwner {
      tokenAddress = _newTokenAddress;
  }
  
  /// @dev Set new nft address
  /// @param _newNftAddress - Address of new erc721 token.
  function setNftAddress(address _newNftAddress) external onlyAdminOrOwner {
  ...
  function disableEgg(uint256 _eggId) external onlyAdminOrOwner {
      Egg storage egg = eggs[_eggId];
      require(_eggId <= eggs.length -1, "egg_not_exist");
