...
contract Staking is Ownable, ReentrancyGuard {
    ...
    function claculateAPR() external {
        // Calculate on the go APR
    }
    ...
    function harvestRewards() internal {
        ...
        uint256 _currentBlock = block.number;
        uint256 _difference = _currentBlock.sub(_lastWithdrawlBlock);
        uint256 _userStaked = UserInfos[msg.sender].currentUnlockedTokens;
        uint256 _noOfWithdrawls = UserInfos[msg.sender].noOfWithdrawls;
        require(
            _noOfBlocksPerDay < _difference,
            "HarvestRewards:: Withdrawing too early"
        );

        // divide the difference with no.of blocks, that will be no.of days that haven't claimed
        uint256 _unClaimedDays = _difference.div(_noOfBlocksPerDay);
        UserInfos[msg.sender].currentUnlockedTokens = _userStaked.sub(
            apy.mul(_userStaked.mul(_unClaimedDays)).div(100).div(365)
        );

        // Update User stake
        UserInfos[msg.sender].noOfWithdrawls = _noOfWithdrawls.add(
            _unClaimedDays
        );
        ...
        if (!UserInfos[msg.sender].isHalfClaimed) {
            IERC20(stakeToken).transferFrom(
                ...
                apy
                    .mul(
                        UserInfos[msg.sender].initialTokens.div(2).mul(
                            _unClaimedDays
                        )
                    )
                    .div(100)
