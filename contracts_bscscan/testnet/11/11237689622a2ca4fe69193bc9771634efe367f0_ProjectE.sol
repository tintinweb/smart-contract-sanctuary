...
contract ProjectE is IBEP20, Auth {
    ...
    string constant _name    = "Widgets";
    string constant _symbol  = "WIDGET";
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		// Max tx check
		require (amount <= maxTransactionSize || isTxLimitExempt[sender] || !maxPerTransactionEnabled, "Max amount per transaction exceeded");

        // Max wallet check excluding pair and router
        bool isSell = (recipient == pair || recipient == routerAddress);
		if (!isSell) {
			require ((_balances[recipient] + amount) <= maxWalletBalance || !maxPerWalletEnabled, "Max balance per wallet exceeded");
		}
		
		if (isSell && msg.sender != pair && !inSwap && swapEnabled && _balances[address(this)] >= swapThreshold) {
            swapBack();
        }
        ...
        if (!isFeeExempt[sender] && feesEnabled) {
    ...
    function setPresaleSettings(bool _enabled, address presale) external authorized {
