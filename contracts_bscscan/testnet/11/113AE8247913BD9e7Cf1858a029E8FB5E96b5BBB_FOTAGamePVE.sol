...
contract FOTAGamePVE is Auth, EIP712Upgradeable {
  ...
  uint public totalStations;
  uint public energyConsumption;
  mapping (address => uint) public nonces;

  event TotalStationsUpdated(uint totalStations, uint timestamp);
  ...
  function finishGame(uint _station, bool _win, uint8 _currentEnergy, bytes memory _signature) external {
    _validateFinishGame(_station, _win, _currentEnergy, _signature);
  }

  // PRIVATE FUNCTIONS

  function _validateFinishGame(uint _station, bool _win, uint8 _currentEnergy, bytes memory _signature) private {
    require(_station <= totalStations, "Station invalid");
    energyManager.updateEnergy(msg.sender, _currentEnergy, energyConsumption);
    bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
        keccak256("FinishGame(address user,string station,uint256 nonce,bool win)"),
        msg.sender,
        _station,
        nonces[msg.sender],
        _win
  ...
  function updateTotalStations(uint _totalStations) external onlyMainAdmin {
    totalStations = _totalStations;
    emit TotalStationsUpdated(totalStations, block.timestamp);
  }
...
interface IEnergyManager {
  function updateEnergy(address _user, uint _amount, uint _consumeAmount) external;
}
