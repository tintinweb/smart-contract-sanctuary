...
contract MultiSend is Context, Ownable {
   ...
   function transferBulk(address token, address[] memory _tos, uint256[] memory _values) public virtual returns(bool) {
      ...
      payable(SERVICE_ADDRESS).transfer(this.estFeeTransferBulk(_tos.length));
      return true;
   }
