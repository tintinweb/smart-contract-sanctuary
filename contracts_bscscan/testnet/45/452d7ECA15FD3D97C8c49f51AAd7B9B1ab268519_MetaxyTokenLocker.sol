...
interface ITokenLocker {
    ...
    event BatchUnLock(uint256[] seasonIDs, address indexed to, uint256 value);
}

contract MetaxyTokenLocker is ITokenLocker, Ownable, ReentrancyGuard {
    ...
    constructor(address _dorToken) public {
    ...
    function unlock(
        ...
        bytes32 node = keccak256(abi.encodePacked(_index, _account, _dorAmount));
        require(
            MerkleProof.verify(_merkleProof, merkleRoots[_seasonID], node),
            "MerkleDistributor: Invalid proof."
        );
        
        _unlock(_seasonID, _account, _dorAmount);
    }
    ...
    function batchUnlock(
        ...
        emit BatchUnLock(_seasonIDs, account, dorAmount);
    }
    ...
    function _unlock(uint256 _seasonID, address _account, uint256 _dorAmount) internal {
        require(_dorAmount > 0, "zero unlock");
        DOR.safeTransfer(_account, _dorAmount);
        claimed[_seasonID][_account] = 1;
        emit UnLock(_seasonID, _account, _dorAmount);
    }
}
