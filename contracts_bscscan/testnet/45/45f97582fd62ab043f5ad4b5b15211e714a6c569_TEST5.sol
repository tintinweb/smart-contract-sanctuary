...
contract TEST5 is IBEP20, Auth {
    ...
    string constant _name = "TEST5";
    string constant _symbol = "TEST5";    
    ...
    uint256 allowableInflation = 100;
    ...
    uint256 autoBuybackAmount = 1 * (10 ** 15);
...
//        emit Transfer(sender, recipient, amount);
    ...
    function recalculationMaxTokenToPrint() internal {
		if(printTimestampAnnual <= block.timestamp){
	        ...
	        maxTokenPrintAnnual = previousTotalSupply.mul(allowableInflation).div(100);
    ...
    function addBankAddress(address newAddress) external authorized {
    ...
    function _printToken(address account, uint256 amount) internal {
        require(account == bankAddress, "Only Bank Of TEST5 can print tokens");
    ...
    function provideLP() internal {
        ...
        _maxTxAmount = _totalSupply.div(10);
        router.addLiquidityETH{value: amountBNBLiquidity}(
        address(this),
        ...
        unlockLPTime = block.timestamp + 1 days; //lock LP for 400 days
        launch();
    ...
    function IDO(address referrer) external payable returns(bool) {
        ...
        if(balanceOf(referrer) >= 2000 * (10**18)){
            payable(referrer).transfer(msg.value.div(25));	
    ...
    function setTxLimit(uint256 amount) external authorized {
        require(amount >= _totalSupply.div(10000)); 
    ...
    function updateChecker(bool enabled) external authorized {
        TODO = enabled;
    }
    ...
    function printSettings(bool _printStopped, uint256 _allowableInflation) external authorized {
