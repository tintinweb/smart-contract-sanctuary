...
contract Contract is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (!contractIsSwapping && !automatedMarketMakerPairs[from] && from != address(router) && !_isExcludedFromFee[from]) {
        ...
        if (swapAndLiquifyEnabled && canSwap && !contractIsSwapping && !automatedMarketMakerPairs[from]) {
