...
interface IStake{
    function isWhitelist(address account) external view returns (uint256);
}
...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    uint256 public claiminterval = 150 days;
    uint256 public initialclaimpercent = 50;
    uint256 public refundStartDate;
    uint256 public claimStartDate;
    bool public readfromStake;
    address public stakeaddress;
    uint256 public tier1Amount = 0.5 ether;
    uint256 public tier2Amount = 0.25 ether;
    uint256 public tier3Amount = 0 ether;
    ...
    function FinishICO() external onlyOwner icoActive(){
        endICO = 0;
        claimStartDate = block.timestamp;
    }
    
    function RefundICO() external onlyOwner icoActive(){
        endICO = 0;
        startRefund = true;
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal view {
        ...
        if(readfromStake){
            IStake stakecont = IStake(stakeaddress);
            if(stakecont.isWhitelist(msg.sender) == 1){
                require(_contributions[beneficiary].add(weiAmount)<= tier1Amount, 'can\'t buy more than: maxPurchase');
            }
            else if(stakecont.isWhitelist(msg.sender) == 2){
                require(_contributions[beneficiary].add(weiAmount)<= tier2Amount, 'can\'t buy more than: maxPurchase');
            }
            else if(stakecont.isWhitelist(msg.sender) == 3){
                require(_contributions[beneficiary].add(weiAmount)<= tier3Amount, 'can\'t buy more than: maxPurchase');
            }
            else{
                require(false, 'Stake more tokens');
            }
        }
        this; 
    }
    ...
    function seRefundstartDate(uint256 value) public onlyOwner {
    ...
    function setTier1Amount(uint256 value) external onlyOwner{
        tier1Amount = value;
    }
    function setTier2Amount(uint256 value) external onlyOwner{
        tier2Amount = value;
    }
    
    function setTier3Amount(uint256 value) external onlyOwner{
        tier3Amount = value;
    }
    function setStakeaddress(address value) external onlyOwner{
        stakeaddress = value;
    }
    function setReadfromStake(bool value) external onlyOwner{
        readfromStake = value;
    }
