...
contract ExtendedReflections is Context, IERC20, Ownable {
    ...
    mapping (address => bool) public isExcludedFromTokenLimit;
    ...
    */
    ...
    constructor (string memory name_, string memory symbol_, uint32 decimals_, uint256 tokenSupply_) {
        ...
        isExcludedFromTokenLimit[owner()] = true;
        isExcludedFromTokenLimit[burnAccount] = true;
        isExcludedFromTokenLimit[address(this)] = true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (!isExcludedFromTokenLimit[recipient])
            require(balanceOf(recipient) + amount <= getMaxWalletAmount(), "Anti-whale: Recipient cannot exceed maximum wallet limit");
    ...
    function _getTValues(ValuesFromAmount memory values, bool deductTransferFee) view private {
        ...
        if (deductTransferFee) {
            ...
            // calculate fee
            values.tBurnFee = _calculateTax(values.amount, getTax(0), 1);
            values.tLiquifyFee = _calculateTax(values.amount, getTax(1), 1);
            values.tRewardFee = _calculateTax(values.amount, getTax(2), 1);
            values.tAdminFee = _calculateTax(values.amount, getTax(3), 1);
    ...
    */
    ...
    function enableAutoSwapAndLiquify(address routerAddress, uint256 minTokensBeforeSwap_) public onlyOwner {
        ...
        isExcludedFromTokenLimit[address(uniswapV2Router)] = true;
        isExcludedFromTokenLimit[_uniswapV2Pair] = true;
    ...
    function enableAdminTax(address adminAddress_) public onlyOwner {
    ...
    uint256 maxWalletAmountPercent = 1;

    uint32[4][5] private _taxTable;

    function setMaxWalletAmount(uint256 _maxWalletAmount) public onlyOwner {
        maxWalletAmountPercent = _maxWalletAmount;
    }

    function getMaxWalletAmount() public view returns (uint256){
        return _currentSupply*maxWalletAmountPercent/100;
    }

    function excludeAccountFromLimit(address _account) public onlyOwner {
        require(isExcludedFromTokenLimit[_account], "Anti-whale: Account already exluded");
        isExcludedFromTokenLimit[_account] = true;
    }

    function includeAccountInLimit(address _account) public onlyOwner {
        require(!isExcludedFromTokenLimit[_account], "Anti-whale: Account already included");
        isExcludedFromTokenLimit[_account] = false;
    }

    function initTaxTable() public onlyOwner {
        _taxTable[0] = [50,30,20,20];
        _taxTable[1] = [40,20,25,15];
        _taxTable[2] = [30,10,30,10];
        _taxTable[3] = [20,5,30,5];
        _taxTable[4] = [0,0,0,0];
    }

    function getTax(uint32 _taxIndex) internal view returns (uint32 tax) {
        if (_currentSupply > 50000000000000 * (10 ** _decimals))
            return _taxTable[0][_taxIndex];
        if (_currentSupply <= 50000000000000 * (10 ** _decimals) && _currentSupply > 25000000000000 * (10 ** _decimals))
            return _taxTable[1][_taxIndex];
        if (_currentSupply <= 25000000000000 * (10 ** _decimals) && _currentSupply > 1000000000000 * (10 ** _decimals))
            return _taxTable[2][_taxIndex];
        if (_currentSupply <= 1000000000000 * (10 ** _decimals) && _currentSupply > 100000000 * (10 ** _decimals))
            return _taxTable[3][_taxIndex];
        if (_currentSupply <= 100000000 * (10 ** _decimals))
            return _taxTable[4][_taxIndex];
    }
}

contract CustomTaxToken is ExtendedReflections {

    uint256 private _tokenSupply = 100000000000000;

    uint32 private _decimals = 18;

    uint256 private _minTokensBeforeSwap = 100 * (10 ** _decimals);
    ...
    address private _adminAddress = 0xAeb8E5C62760737fB4546D3eDAb07F1BD2D56d1e;

    constructor () ExtendedReflections("Name", "SYMBOL", _decimals, _tokenSupply) {
        enableAutoBurn();
        enableReward();
        enableAdminTax(_adminAddress);
        enableAutoSwapAndLiquify(_routerAddress, _minTokensBeforeSwap);
    }
}