...
contract HIREX is Context, IBEP20, Ownable {
    ...
    uint256 public _maxWalletAmount   =   10000000 * 10**9; // 1% max
    uint256 public _buyMaxTxAmount    =    2000000 * 10**9; // 0.2% max buy tx
    uint256 public _sellMaxTxAmount   =     100000 * 10**9; // 0.01% max sell tx
    uint256 private _otherMaxTxAmount = 1000000000 * 10**9; // total supply
       
    uint256 private numTokensSellToAddToLiquidity = 70000 * 10**9; // this tx amount will trigger swap and add liquidity
    ...
    constructor (address payable marketingWallet, address payable researchWallet, address pancakeSwapRouter) {
        ...
        _isExcludeAddressFromMaxTx[burnAddress] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
		...
		if (maxWalletMode == MaxWalletMode.HARD && to != pancakeSwapV2Pair && !_isExcludedFromMaxWallet[to]) {
