...
contract TestPrivateSale is Owned {
    using SafeMath for uint256;
    
    bool public isPrivateSaleOpen;
    
    address public tokenAddress = 0xA4336466Ba06EC959091234a21cBEAC2943E0384;
    uint256 public tokenDecimals = 18;
    
    uint256 public tokenRatePerEth = 100000000000000000000000;
    uint256 public rateDecimals = 0;
    
    uint256 public minEthLimit = 1e17; // 0.1 BNB
    uint256 public maxEthLimit = 4e17; // 0.4 BNB

    uint256 public dividingFactor = 10;
    ...
    function setDividingFactor(uint256 amount) external onlyOwner {
        dividingFactor = amount;    
    }
    ...
    function distributeTokens(address[] calldata _addresses) external onlyOwner {
         for (uint i=0; i<_addresses.length; i++) {
            ...
            if (tokenAmount > 0) {
                require(IToken(tokenAddress).transfer(_addresses[i], tokenAmount/dividingFactor), "Insufficient balance of presale contract!");
                balanceOf[_addresses[i]] = tokenAmount - tokenAmount/dividingFactor;
            }
         }
    }
