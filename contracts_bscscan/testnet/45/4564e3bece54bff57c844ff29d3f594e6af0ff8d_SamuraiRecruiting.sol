...
contract SamuraiRecruiting is RSunAuth, IERC721Receiver {
    ...
    address public rsunAdr = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public infAdr = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public samuraiAdr = 0x14a3Ee3771845cee9EA2D49Fcca8DDA58f5D5D8b; // 0xC3c3B849ED5164Fb626c4a4F78e0675907B2C94E Mainnet

    uint public maxUsages = 10;
    uint public maxUsagesGeneral = 30; // generals can be used 30 times
    uint public rsunCost = 15000 gwei; // 15k RSUN ~ 15-20$

    uint public lockDurationBase = 20;
    uint public lockDurationPerUsage = 10;
    ...
    function _recruit(uint[] memory ids, address user) internal {
        ...
        missions.push(Mission(getIdsAsSingleUint(ids), uint40(block.timestamp), getLockDuration(ids), false));
    ...
    function retrieveSamurai(uint missionId) external {
        Mission memory s = missions[missionId];
    ...
    function getAllMissionsOfUser(address user) external view returns(OutputMission[] memory missions_) {
        ...
        for (uint256 i = 0; i < missionIds.length; i++) {
            ...
            missions_[i] = OutputMission(missionIds[i], ids, mission.startTimestamp, mission.duration, mission.retrieved);
        }
    }

    function setLockDurations(uint base, uint perUsage) external {
        authorizedFor(Permission.AdjustVariables);
