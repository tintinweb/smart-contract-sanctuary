...
contract Token2 is ERC20, AccessControl {

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant GOVERNOR_ROLE = keccak256("GOVENOR_ROLE");

    uint internal _cap = 1_000_000_000 * 10 ** 18;

    constructor() public ERC20("TestToken", "TestToken") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function mint(address _to, uint256 _amount) public {
        require(hasRole(MINTER_ROLE, msg.sender), "Caller is not a minter");
        require(totalSupply().add(_amount) <= cap(), "Out Of the Cap");
        _mint(_to, _amount);
    }

    function increaseCap(uint256 _increaseNum) public {
        ...
        external
    {
        ...
        require(now <= expiry, "UP::delegateBySig: signature expired");
