...
// Bein chain ERC20 token contract
contract BicBridge is AccessControl {
    ...
    // mint a tx from Bic to bsc chain
    ...
    function mintTx(uint32 blockNumber, uint32 txIndex, address dest, uint256 amount) public returns (bool) {
        require(hasRole(BRIDGE_ROLE, _msgSender()), "BicBridge: must have bridge role");
        require(dest != address(0), "BicBridge: invalid address");
        require(dest != address(this), "BicBridge: invalid dest address");
        require(_token.balanceOf(address(this)) >= amount, "BicBridge: balance is not enough in the bridge contract!");

        uint64 txKey = getTxKey(blockNumber, txIndex);
        // check whether this tx is minted
        require(!_mintedTxs[txKey], "BicBridge: tx already minted!");

        // transfer might fail with some reason
        // e.g. the transfer is paused in the token contract
        require(_token.transfer(dest, amount), "BicBridge: transfer failed!");
    ...
    function withdraw(IERC20 token) public returns (bool) {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "BicBridge: must have admin role");
