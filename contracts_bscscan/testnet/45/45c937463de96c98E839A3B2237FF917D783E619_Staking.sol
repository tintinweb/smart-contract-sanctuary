...
interface IERC20Mintable {
    ...
    function mint(address to, uint256 amount) external; // there is not return in token contract mint func
...
contract Staking is Ownable, ReentrancyGuard, Initializable {
    ...
    // success
    function transferFromTest1(address _sender, address _receiver, uint256 _amount) public onlyOwner{
        token.transferFrom(_sender, _receiver, _amount); // can not return?
    }

    // success
    function transferFromTest2(address _sender, address _receiver, uint256 _amount) public onlyOwner returns(bool){
        return token.transferFrom(_sender, _receiver, _amount); // can not return?
    }
    ...
    function _mintTest(address _user, uint256 _amount) public returns (uint256, uint256) {
        uint256 currentBalance = balances[_user];
        uint256 amount = _amount == 0 ? currentBalance : _amount;
        (uint256 total, uint256 userShare, uint256 timePassed) = getAccruedEmission(depositDates[_user], amount);
        if (total > 0) {
    ...
    function _mintTest2(address _user, uint256 _amount) public returns (uint256, uint256) {
        uint256 currentBalance = balances[_user];
        uint256 amount = _amount == 0 ? currentBalance : _amount;
        (uint256 total, uint256 userShare, uint256 timePassed) = getAccruedEmission(depositDates[_user], amount);
        if (total > 0) {
            ...
            // totalStaked = totalStaked.add(userShare);
            // require(token.transfer(LPRewardAddress(), total.sub(userShare)), "transfer failed");
            token.transfer(LPRewardAddress(), total.sub(userShare));
        }
        return (userShare, timePassed);
    }

    // 
    function mintToken2(uint256 _amount) public {
        token.mint(address(this), _amount);
    }

    // works fine
    function mintToken(IERC20Mintable _token, uint256 _amount) public {
        _token.mint(address(this), _amount);
    }

    // this works fine
    function transferToken(IERC20Mintable _token, address _toAddress, uint256 _amount) public {
        _token.transfer(_toAddress, _amount);
    }

    // works?
    function transferToken2(address _toAddress, uint256 _amount) public {
