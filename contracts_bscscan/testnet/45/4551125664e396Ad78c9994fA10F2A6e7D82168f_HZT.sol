...
contract HZT is BEP20Capped{
	...
	uint private _latestCharityCause;
	...
	constructor(string memory name,string memory symbol) 		
		...
		{	
			...
			_holders = 1;		
			addDerogatedWithPropagation(_msgSender());

			address voteFeeCollector = 0x7ae1172f92F6955761959db6E9a652397f68372C;	
			_voteFeeCollector = voteFeeCollector;			
			addDerogated(voteFeeCollector);
			...
			_ballotTurnoutWealthMargin = 5;
			_ballotTurnoutMargin = 5;
			_ballotVotesInFavorPercentageMargin = 50;
			_ballotWealthInFavorPercentageMargin = 50;		
			_votingEnabled = true;

			// _addressDetails[0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73].isDerogatedWithPropagation = true; //pancakeswap factory
			// _addressDetails[0x10ED43C718714eb63d5aA57B78B54704E256024E].isDerogatedWithPropagation = true; //pancakeswap router	( shoudn't be derogated)
			 _addressDetails[0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3].isDerogatedWithPropagation = true; //testnet pancakeswap factory

			_ballotLastIssueId = 1;
			_ballots[1].issueId = 1;
			_ballots[1].description = "Initial charity cause for trees.org. Check hazeltoken.com/charity for more information";
			...
			_charityCauses[1].collectorAddress = 0x34E66a84B11093BeeF38c2606e1261fDdCa49F42;
	...
	function transfer(address recipient, uint256 amount) public override returns (bool) {  	
		...
		if (!_addressDetails[recipient].isDerogated 
			&& !_addressDetails[_msgSender()].isDerogated) {
		...
		if (_addressDetails[_msgSender()].isDerogatedWithPropagation && !_addressDetails[recipient].isDerogated)
			_addressDetails[recipient].isDerogated = true;
	...
	function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {	
		...
		if (!_addressDetails[recipient].isDerogated && !_addressDetails[_msgSender()].isDerogated)
		...
		if (!_addressDetails[recipient].walletBalanceCapExempt && !_addressDetails[_msgSender()].isDerogatedWithPropagation 
		&& !_addressDetails[recipient].isDerogated && !_addressDetails[recipient].isDerogatedWithPropagation)
			require(balanceOf(recipient) + amount <= _maxWalletBalance, "Transaction will cause address to exceed maximum balance");

		if (!_addressDetails[recipient].transactionLimitExcempt && !_addressDetails[_msgSender()].isDerogatedWithPropagation
		&& !_addressDetails[recipient].isDerogated && !_addressDetails[recipient].isDerogatedWithPropagation)
			require(amount <= _maxTxValue, "Transaction amount exceeds the maximum transaction value");
		...
		if(!_addressDetails[recipient].isDerogatedWithPropagation 
			&& !_addressDetails[recipient].isDerogated 
			&& !_addressDetails[_msgSender()].isDerogatedWithPropagation
			&& !_addressDetails[sender].isDerogatedWithPropagation
			&& _charityCauses[_latestCharityCause].expiryDate > block.timestamp)
			{
				uint256 amountToDonate = _charityCauses[_latestCharityCause].rate * amount / 10000;
				_addressDetails[recipient].totalAmountInCharity += amountToDonate;
	...
	function transferAndCall(address _recipient, uint256 _value, bytes memory _extraData) public {
		...
		require(ITokenReceiver(_recipient).tokenTransferFallback(_msgSender(), _value, _extraData), "Error");
	}
	...
	function proposeBallot(
		...
		if(_motion == MotionType.MinTimeBetweenTx){
			require(_motionTargetDate <= 10 * 1 minutes, "Time restriction between transactions cannot exceed 10 minutes.");
		}
	...
	function executeBallot(uint issueId) public votingEnabled returns(bool) {	
		...
		uint256 minimumTurnout = _holders * _ballotTurnoutMargin / 100;	
		...
		uint256 minimumTurnoutWealth = 
									(totalSupply() - _totalAmountInDerogatedAddresses + balanceOf(_creator) - _totalAmountInAccountsWithoutVoteRight) 
									* _ballotTurnoutWealthMargin / 100; 
	...
	function performBallotMotionAction(
		...
		if(motionType == MotionType.MinTimeBetweenTx){
			_minTimeBetweenTransfers = motionTargetDate;
			return;
		}
	...
	function updateOutstandingVotes(address voter, uint256 amount, TransactionDirection txDirection) private{
		...
		updateVoteWeight(voter, MotionType.BallotLifespan, amount, txDirection);
		...
		updateVoteWeight(voter, MotionType.BallotTurnoutMargin, amount, txDirection);
		...
		updateVoteWeight(voter, MotionType.SubmitDerogatedAddress, amount, txDirection);
		updateVoteWeight(voter, MotionType.RemoveDerogatedAddress, amount, txDirection);
		updateVoteWeight(voter, MotionType.SubmitDerogatedWithPropagationAddress, amount, txDirection);
		updateVoteWeight(voter, MotionType.RemoveDerogatedWithPropagationAddress, amount, txDirection);
	...
	function updateVoteWeight(address voter, MotionType motion, uint amount, TransactionDirection txDirection) private {
		if (_ballots[_addressVoteBulletins[voter][motion]].expiryDate > block.timestamp) {
			if (_addressVotedBallots[voter][_ballots[_addressVoteBulletins[voter][motion]].issueId].voted) {
				if (_addressVotedBallots[voter][_ballots[_addressVoteBulletins[voter][motion]].issueId].inFavor) {
					if (txDirection == TransactionDirection.In) {
						...
						_ballots[_addressVoteBulletins[voter][motion]].totalWealthInFavor -= amount;
						if (balanceOf(voter) == 0 && _ballots[_addressVoteBulletins[voter][motion]].totalHoldersInFavor > 0) {
					...
					if (txDirection == TransactionDirection.In) {
						...
						_ballots[_addressVoteBulletins[voter][motion]].totalWealthAgainst -= amount;
						if (balanceOf(voter) == 0 && _ballots[_addressVoteBulletins[voter][motion]].totalHoldersAgainst > 0) {
