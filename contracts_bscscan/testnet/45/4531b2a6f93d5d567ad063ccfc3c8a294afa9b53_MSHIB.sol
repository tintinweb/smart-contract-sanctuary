...
abstract contract MSHIBBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	// MAIN TOKEN PROPERTIES
	string private constant NAME = "MadShibaX2";
	string private constant SYMBOL = "MSHIBX2";
	uint8 private constant DECIMALS = 9;
	uint256 private constant _totalTokens = 100_000_000 * 10**DECIMALS;	//total supply

   // marketing and dev address
    address private _marketingWallet = 0x1c8dC23400Eb78D7AeB14A3ee086E1882993Dd3e;
    address private _devAddress = 0x3e2D9F782153D4423B3Ce9E7175b3DeC80dF5eC1;
	...
	constructor (address routerAddress) {
		...
		// Initialize PancakeSwap V2 router and MSHIB <-> BNB pair.
		setPancakeSwapRouter(routerAddress);

		_maxHoldAmount = 1_200_000 * 10**DECIMALS; //cannot hold more than 1.2% in single wallet
	...
	function activate() public onlyOwner {
		...
		setTransactionSellLimit(400_000 * 10**DECIMALS); //cannot sell more than 0.40% at once
		setTransactionBuyLimit(999_000 * 10**DECIMALS); //cannot buy more than 0.99% at once
		activateBuying(true);
		onActivated();
	}
	...
	// Returns how many more $MSHIB tokens are needed in the contract before triggering a swap
	...
	function setPancakeswapV2Pair(address pancakeswapV2PairAddress) public onlyOwner {
        _pancakeswapV2Pair = pancakeswapV2PairAddress;
    }
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 marketingFee, uint8 devFee) public onlyOwner {
		...
		require(rewardFee >= 0 && rewardFee <= 5, "Reward fee must be between 1% and 10%");
        require(marketingFee >= 0 && marketingFee <= 10, "Marketing fee must be between 0% and 10%");
        require(devFee >= 0 && devFee <= 5, "Dev fee must be between 1% and 10%");
...
contract MSHIB is MSHIBBase {
	...
	uint256 private _totalBNBAsMSHIBClaimed; // The total number of BNB that was converted to MSHIB and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsMSHIBClaimed; // The amount of BNB converted to MSHIB and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy MSHIB automatically
	...
	bool private _reimburseAfterMSHIBClaimFailure; // If true, and MSHIB reward claim portion fails, the portion will be given as BNB instead
	...
	constructor (address routerAddress) MSHIBBase(routerAddress) {
	...
	function onActivated() internal override {
		...
		setReimburseAfterMSHIBClaimFailure(true);
	...
	function onBeforeTransfer(address sender, address recipient, uint256 amount) internal override {
		...
		bool isSelling = isPancakeSwapPair(recipient);
		if (!isSelling) {
			// Wait for a dip, MSHIB diamond hands
			return;
		}
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough MSHIB balance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim MSHIB tokens
		if (!claimMSHIB(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterMSHIBClaimFailure) {
	...
	function claimMSHIB(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsMSHIBClaimed[user] += bnbAmount;
		_totalBNBAsMSHIBClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $MSHIB on a zero balance address:
		...
		// Depending on the % of $MSHIB tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsMSHIB(address byAddress) public view returns (uint256) {
		return _bnbAsMSHIBClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsMSHIB() public view returns (uint256) {
		return _totalBNBAsMSHIBClaimed;
	}
	...
	function reimburseAfterMSHIBClaimFailure() public view returns(bool) {
		return _reimburseAfterMSHIBClaimFailure;
	}


	function setReimburseAfterMSHIBClaimFailure(bool value) public onlyOwner {
		_reimburseAfterMSHIBClaimFailure = value;
	}
	...
	//Rescue BNB & other tokens which are sent to the contract
