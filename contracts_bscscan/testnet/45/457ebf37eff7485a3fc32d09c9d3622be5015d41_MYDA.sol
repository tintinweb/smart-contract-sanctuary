...
interface AFTS {
    ...
    function mint(bytes32 txid, uint256 swapNonce, uint256 amount, uint256 fees, uint256 fromId, uint256 toId, bytes memory sig) external returns (bool);   
...
contract MYDA is Context, AFTS {
    ...
    constructor () {
        ...
        _general._validatorApi = "https://validator.exosis.org/";
        _general._totalSupply = 210000000*1e18;
        ...
        _stakeConfig[1].collateral = 1000*1e18;
        _stakeConfig[2].collateral = 5000*1e18;
        _stakeConfig[3].collateral = 50000*1e18;
        ...
        stakeAddress = 0xEB8BDE562B7d93AF17583CF5680fDA2A2db6f83B;
        _transfer(address(0), _msgSender(), 210000000*1e18, 0);
    }
    ...
    function _stakeProcess(address recipient, uint256 tier, uint256 qty, address refAdd, uint256 method) internal virtual {
        ...
        collateral = (method < 2)?_stakeConfig[tier].collateral * qty:_stake[recipient][tier].collateral;
        ...
        if(method < 2){
            ...
            _transfer(recipient, address(0), collateral, 0);
            ...
            _stake[recipient][tier].collateral += collateral;
            if(method == 1){
            ...
            if(method == 2){
                _transfer(address(0), recipient, collateral + stakeReward + refRew, 0);
                ...
                if(_general._totalSupply < _general._maxSupply){
                    if((_general._totalSupply + stakeReward + refRew) > _genConfig["maxSupply"]){
                        remainSupply = _genConfig["maxSupply"] - _general._totalSupply;
                        if(remainSupply > 0){
                    ...
                    if(_user[stakeAddress]._balances < (stakeReward + refRew)){
                        if(_user[stakeAddress]._balances > 0){
                            stakeReward = _user[stakeAddress]._balances;
                            refRew = 0;
                        } else { 
                            stakeReward = 0;
                            refRew = 0;
                    ...
                    if(stakeReward > 0){
                        _transfer(stakeAddress, recipient, stakeReward + refRew, 1);
                    }
                }
    ...
    function mint(bytes32 txid, uint256 swapNonce, uint256 amount, uint256 fees, uint256 fromId, uint256 toId, bytes memory sig) public virtual override returns (bool) {
        _mint(_msgSender(), txid, swapNonce, amount, fees, fromId, toId, sig);
        return true;
    }  
    
    function _mint(address sender, bytes32 txid, uint256 swapNonce, uint256 amount, uint256 fees, uint256 fromId, uint256 toId, bytes memory sig) internal {
        require(!_general._isPaused, "Contract is Paused");
        require(_swapIn[sender][txid] == 0, "Already Swapped");
        _sigValidate(sig, keccak256(abi.encodePacked(sender, txid, swapNonce, amount, fees, fromId, toId, true)), _general._validatorAdd);
