...
// File: contracts/APWarsWarMachineV3.sol
 ...
 */
contract APWarsWarMachineV3 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function deposit(IAPWarsUnit _unit, uint256 _amount) public nonReentrant {
        ...
        attackPowerByAddress[team][msg.sender] = attackPowerByAddress[team][
            msg.sender
        ].add(troopAttackPower).add(troopDefensePower);
        attackPower[team] = attackPower[team].add(troopAttackPower).add(
            troopDefensePower
        );
        defensePowerByAddress[team][msg.sender] = defensePowerByAddress[team][
            msg.sender
        ].add(troopAttackPower).add(troopDefensePower);
        defensePower[team] = defensePower[team].add(troopAttackPower).add(
            troopDefensePower
        );
        ...
        internal
    {
        ...
        uint256 randomTeamSource = random(
        ...
        uint256 randomBadLuckSource = random(
            _externalRandomSource,
            salt,
            ONE_HUNDRED_PERCENT
        );
    ...
    function _calculateTroopImprovement() internal {
        ...
        for (uint256 i = 0; i < allowedTeamTokenAddresses[TEAM_A].length; i++) {
            IAPWarsUnit unit = IAPWarsUnit(
                allowedTeamTokenAddresses[TEAM_A][i]
            );
            ...
            if (unit.getTroopImproveFactor() > 0 && balance > 0) {
                ...
                attackPower[TEAM_A] = attackPower[TEAM_A].add(
                    attackImprovement
                );
                ...
                defensePower[TEAM_A] = defensePower[TEAM_A].add(
                    defenseImprovement
                );
            }
        }
        ...
        for (uint256 i = 0; i < allowedTeamTokenAddresses[TEAM_B].length; i++) {
            IAPWarsUnit unit = IAPWarsUnit(
                allowedTeamTokenAddresses[TEAM_B][i]
            );
            ...
            if (unit.getTroopImproveFactor() > 0 && balance > 0) {
                ...
                attackPower[TEAM_B] = attackPower[TEAM_B].add(
                    attackImprovement
                );
                ...
                defensePower[TEAM_B] = defensePower[TEAM_B].add(
                    defenseImprovement
                );
            }
        }
    ...
    function _calculateLuckImpact() internal {
        ...
        uint256 attackerPowerByLuck = initialAttackTeamPower
            .mul(war.attackerLuck)
            .div(ONE_HUNDRED_PERCENT);
        uint256 defensePowerByLuck = initialDefenseTeamPower
            .mul(war.defenderLuck)
            .div(ONE_HUNDRED_PERCENT);
    ...
    function _calculateMoraleImpact() internal {
        ...
        uint256 attackerMoraleImpactPerc = initialAttackTeamPower
            .mul(ONE_HUNDRED_PERCENT)
            .div(initialDefenseTeamPower);
        uint256 defenseMoraleImpactPerc = initialDefenseTeamPower
            .mul(ONE_HUNDRED_PERCENT)
            .div(initialAttackTeamPower);

        uint256 attackerMoraleImpact = initialAttackTeamPower
            .mul(attackerMoraleImpactPerc)
            .div(ONE_HUNDRED_PERCENT)
            .div(TEN_PERCENT);
        uint256 defenseMoraleImpact = initialDefenseTeamPower
        ...
        if (attackerMoraleImpactPerc > ONE_HUNDRED_PERCENT) {
            if (war.finalAttackPower > attackerMoraleImpact) {
            ...
            if (war.finalDefensePower > defenseMoraleImpact) {
    ...
    function _calculateLosses() internal {
        ...
        if (war.finalAttackPower == 0) {
            ...
            war.percAttackerLosses = war.percAttackerLosses.add(
                war.attackerCasualty
            );
        ...
        if (war.finalDefensePower == 0) {
            ...
            war.percDefenderLosses = war.percDefenderLosses.add(
                war.defenderCasualty
            );
    ...
    function finishSecondRound(bytes32 _externalRandomSource) public onlyOwner {
        ...
        uint256 losses = war.winner == war.attackerTeam
            ? war.percAttackerLosses
            : war.percDefenderLosses;
        uint256 newLosses = losses
    ...
    function withdraw(IAPWarsUnit _unit) public nonReentrant {
        ...
        uint256 toBurnPerc = team == war.attackerTeam
            ? war.percAttackerLosses
            : war.percDefenderLosses;
        ...
        uint256 amountToBurn = depositAmount.mul(toBurnPerc).div(
            ONE_HUNDRED_PERCENT
        );
    ...
    function getWithdrawPrize(address _player)
        public
        view
        returns (
            uint256 net,
        ...
        )
    {
        ...
        teamTotalPower = isAttacker
            ? initialAttackPower[war.winner]
            : initialDefensePower[war.winner];
        userTotalPower = isAttacker
            ? attackPowerByAddress[war.winner][_player]
            : defensePowerByAddress[war.winner][_player];

        userShare = userTotalPower.mul(ONE_HUNDRED_PERCENT).div(teamTotalPower);
        userPrize = totalPrize.mul(userShare).div(ONE_HUNDRED_PERCENT);
        amountToBurn = userPrize
            .mul(
