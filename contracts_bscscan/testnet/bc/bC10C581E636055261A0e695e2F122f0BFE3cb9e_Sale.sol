...
contract Sale is Owned {
    ...
    bool public saleCancelled = false;

    uint256 rate = 1; // 1 BNB = 1 token

    uint256 public softCap = 2 * 10 ** 18; // 2 BNBs
    ...
    receive() external payable saleIsOpen {
        ...
        require(IERC20(TOKEN).balanceOf(address(this)) >= totalInvestment, "Insufficient tokens in sale");
    }

    function purchaseToken(uint256 _amount, uint256 _tokens) private {
        purchasedTokens[msg.sender] = purchasedTokens[msg.sender].add(_tokens);
    ...
    function claimTokens() external {
        require(saleFinished == true, "Sale has not been finished");
        uint256 toClaim = purchasedTokens[msg.sender];
    ...
    function refund() external {
        require(saleCancelled == true, "Sale has not been cancelled");
        uint256 investment = purchasedTokens[msg.sender];
        require(investment > 0, "nothing to be refunded");
        purchasedTokens[msg.sender] = 0;
        msg.sender.transfer(investment);
    }

    function finishSale() external onlyOwner{
        if (totalInvestment >= softCap) {
            saleFinished = true;
        } else {
            saleCancelled = true;
        }
    ...
    function getFunds() external onlyOwner{
        require(saleFinished == true, "Sale has not been finished");
        owner.transfer(totalInvestment);
    }
    ...
    function balanceOf(address tokenOwner) external view returns (uint256 balance) {
        return purchasedTokens[tokenOwner];
    }

    modifier saleIsOpen{
        require(saleFinished == false, "Sale has already been finished");
