...
contract Nuke {
    ...
    function playGame(uint _tokenAmount) public whenNotPaused{
        ...
        uint balance = nukeToken.balanceOf(address(msg.sender));
        require(balance >= playAmount, "Your NUKE token balance is not sufficient");
        require(nukeToken.transferFrom(msg.sender, nukeFundWallet, (playAmount/2)), "Fund Wal Fail");
        require(nukeToken.transferFrom(msg.sender, address(this),  (playAmount/2)),"Cont Wal Fail");
