...
contract TestADA is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "TestADA";
    string constant _symbol = "TESTADA";
    uint8 constant _decimals = 4;

    uint256 _totalSupply = 1 * 10**12 * (10 ** _decimals);
    uint256 public _maxTxAmount = _totalSupply * 1 / 100;      // max wallet 1%
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver){
            ...
            require((heldTokens + amount) <= _maxWalletToken,"Max Wallet size exceeded");}
