    ...
    ERC721Pausable
{
    ...
    constructor(
        ...
        require(initialCap_ > 0, "DragonKartNFT: cap is 0");
    ...
    function increaseCap(uint256 amount_) public onlyOwner {
        require(amount_ > 0, "DragonKartNFT: amount is 0");
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId_),
            "DragonKartNFT: URI query for nonexistent token"
        );
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    tokenId_.toString(),
                    "/",
                    _items[tokenId_].artifacts.toString()
                )
            );
    }

    function tokenURIs(address owner_)
        public
        view
        returns (string[] memory _tokenURIs)
    {
        uint256 _balance = balanceOf(owner_);
        for (uint256 _index = 0; _index < _balance; _index++) {
            uint256 _tokenId = tokenOfOwnerByIndex(owner_, _index);
            string memory _tokenURI = tokenURI(_tokenId);
            _tokenURIs[_index] = _tokenURI;
        }
    }
...
contract DragonKartNFTGovernanceCore is
    AccessControlEnumerable,
    RandomConsumerBase,
    ItemFactoryManager
{
    ...
    DragonKartNFT immutable nftCore;
    EnumerableSet.UintSet private _supportedBoxTypes;
    ...
    {
        nftCore = new DragonKartNFT(name_, symbol_, baseTokenURI_, initialCap_);
    ...
    // NFT core features
    function upgradeGovernanceCore(address newGovernanceCore_)
        external
        onlyRole(CONTRACT_UPGRADER)
    {
        nftCore.transferOwnership(newGovernanceCore_);
    }
