...
contract LamboStake is Context, Ownable, ReentrancyGuard {
    ...
    function stakeTokens(address tokenAddress,uint256 amount) external isHuman nonReentrant {
        ...
        emit StakeTokenSuccessfully(tokenAddress,msg.sender, amount, nextAvailableClaimDate[tokenAddress][msg.sender]);
    }
    
    function canClaim(address tokenAddress) external view returns (bool) {
      	return nextAvailableClaimDate[tokenAddress][msg.sender] <= block.timestamp;  
    }
    
    function nextClaimDate(address tokenAddress) external view returns (uint256) {
      	return nextAvailableClaimDate[tokenAddress][msg.sender];  
    }
    ...
    function userStakes(address tokenAddress) external view returns (uint256) {
      	return _userStakes[tokenAddress][msg.sender];  
    }
