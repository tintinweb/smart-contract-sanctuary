...
contract CUREStaking is Ownable {
    ...
    event Harvest(uint256 indexed pId, address indexed to, uint256 rewardAmount);
    event EmergencyWithdraw(uint256 indexed pId, address indexed to, uint256 amount);
    event PoolAdded(uint256 indexed pId);
    event RewardDistributorModified(address indexed oldValue, address indexed newValue);
    event PoolEndTimeModified(uint256 indexed oldValue, uint256 indexed newValue);
    ...
    function deposit(uint256 pId, uint256 amount) external {
        ...
        user.amount += amount;
        user.rewardDebt +=
            (amount * pool.accRewardsPerShare) /
    ...
    /**
    @notice Allow users to withdraw specified amount os tokens from specified pool.
    Requirements:
    - Locking period should have expired or pool is closed.
     */
    ...
    /**
    @notice Allows user to harvest its rewards
    Requirements:
    - Locking period should have expired.
     */
    ...
    function emergencyWithdraw(uint256 pId) external {
        ...
        // Calls
        poolInfo[pId].token.safeTransfer(msg.sender, amount);
    }
    ...
    function addPool(
        ...
        poolInfo.push(
            PoolInfo(apy, apyDenominator, 0, 0, startTime, 0, lockDays * 1 days, token)
        );
