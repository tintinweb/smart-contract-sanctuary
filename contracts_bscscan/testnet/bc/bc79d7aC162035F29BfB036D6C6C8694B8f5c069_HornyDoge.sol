...
contract HornyDoge is Context, IBEP20, Ownable {
    ...
    string private _name = "HornyDoge";
    string private _symbol = "$HORNYDOGE";
    ...
    function setFees(uint256 liquidityFee, uint256 marketingFee, uint256 buybackFee, uint256 reflectionFee, uint256 devFee) external onlyOwner {
        _marketingFee = marketingFee;
        _buyBackFee = buybackFee;
        _reflectionFee = reflectionFee;
        _devFee = devFee;
        _liquidityFee = liquidityFee + marketingFee + buybackFee + reflectionFee + devFee;
    }

    function setFeeReceivers(address payable _buyBackFeeReceiver, address payable _marketingFeeReceiver, address payable _devFeeReceiver) external onlyOwner {
    ...
    function _getTValues(address sender, uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tFee = calculateTaxFee(sender, tAmount);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tReflection, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).add(rReflection);
    ...
    function calculateTaxFee(address sender, uint256 _amount) private view returns (uint256) {
    ...
    // Calculates the token allocation for buyback, marketing, reflection and dev
    function calculateTokenAllocationForBuyBackMarketingReflectionAndDev(uint256 tokenAmount) private view returns (uint256) {
        uint256 buyBack_Marketing_Dev_Reflection_Fees = _buyBackFee.add(_marketingFee).add(_reflectionFee).add(_devFee);
        return tokenAmount.div(_liquidityFee).mul(buyBack_Marketing_Dev_Reflection_Fees);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 allocationForBuyBackMarketingReflectionAndDev = calculateTokenAllocationForBuyBackMarketingReflectionAndDev(contractTokenBalance);
        ...
        uint256 half = contractTokenBalance.sub(allocationForBuyBackMarketingReflectionAndDev).div(2);
        uint256 otherHalf = contractTokenBalance.sub(allocationForBuyBackMarketingReflectionAndDev).sub(half);

        // Add half and allocation for marketing to be swapped into bnb
        uint256 tokensToBeSwappedIntoBNB = half.add(allocationForBuyBackMarketingReflectionAndDev);
