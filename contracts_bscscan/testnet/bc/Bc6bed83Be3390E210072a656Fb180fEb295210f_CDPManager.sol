...
interface IVault {
    ...
    function UpdatesStabilityFeeOnPayback ( address asset, address user ) external;
    ...
    function moveCDP( address moveFrom, address asset, address user, uint amount ) external returns(bool);
    function maxPayback(address asset, address user, uint amount) external view returns (uint _maxLimit);
}
...
contract CDPManager is ReentrancyGuard {
    ...
    **/
     ...
     **/
    constructor(address _vaultManagerParameters, address _oracleRegistry, address _cdpRegistry, address _collateralRegistry) {
        require(
            _vaultManagerParameters != address(0) && 
            _oracleRegistry != address(0) &&                                                                                                                   
            _cdpRegistry != address(0),
                "CryptoPeso protocol : INVALID_ARGS"
        );
    ...
    receive() external payable {
        require(msg.sender == WETH, "CryptoPeso protocol : RESTRICTED");
    }
    ...
    function join(address asset, uint assetAmount, uint usdpAmount) public nonReentrant checkpoint(asset, msg.sender) {
        require(usdpAmount != 0 || assetAmount != 0, "CryptoPeso protocol : USELESS_TX");
        require(collateralRegistry.isCollateral(asset), "CryptoPeso protocol : is not actived as collateral");

        require(IToken(asset).decimals() <= 18, "CryptoPeso protocol : NOT_SUPPORTED_DECIMALS");
    ...
    function join_Eth(uint usdpAmount) external payable {
        ...
        if (msg.value != 0) {
            ...
            require(IWETH(WETH).transfer(msg.sender, msg.value), "CryptoPeso protocol : WETH_TRANSFER_FAILED");
        }
    ...
    function exit(address asset, uint assetAmount, uint usdpAmount) public nonReentrant checkpoint(asset, msg.sender) returns (uint) {
        ...
        require(assetAmount != 0 || usdpAmount != 0, "CryptoPeso protocol : USELESS_TX");
    
        vault.UpdatesStabilityFeeOnPayback( asset, msg.sender);
        ...
        if (usdpAmount > vault.maxPayback( asset, msg.sender, usdpAmount)) { usdpAmount = vault.maxPayback( asset, msg.sender, usdpAmount); }
    ...
    function exit_Eth(uint ethAmount, uint usdpAmount) public returns (uint) {
        ...
        require(IWETH(WETH).transferFrom(msg.sender, address(this), ethAmount), "CryptoPeso protocol : WETH_TRANSFER_FROM_FAILED");
        IWETH(WETH).withdraw(ethAmount);
        (bool success, ) = msg.sender.call{value:ethAmount}("");
        require(success, "CryptoPeso protocol : ETH_TRANSFER_FAILED");
        return usdpAmount;
    }
    ...
    function moveCDP(address _moveFrom, address _asset, uint _amount) external returns (bool) {
        return vault.moveCDP(_moveFrom, _asset, msg.sender, _amount);
    }
    ...
    function _ensurePositionCollateralization(address asset, address owner) internal view {
        ...
        require(vault.getTotalDebt(asset, owner) <= usdLimit, "CryptoPeso protocol : UNDERCOLLATERALIZED");
    }
    ...
    function triggerLiquidation(address asset, address owner) external nonReentrant {
        ...
        require(_isLiquidatablePosition(asset, owner, usdValue_q112), "CryptoPeso protocol : SAFE_POSITION");
        
        uint debt = vault.getTotalDebt(asset, owner);
        
        uint initialLiquidationPrice = (debt.mul(vaultManagerParameters.liquidationRatio(asset)).mul(Q112)).div(usdValue_q112);
    ...
    function _ensureOracle(address asset) internal view {
        ...
        require(oracleType != 0, "CryptoPeso protocol : INVALID_ORACLE_TYPE");
        address oracle = oracleRegistry.oracleByType(oracleType);
        require(oracle != address(0), "CryptoPeso protocol : DISABLED_ORACLE");
    }
