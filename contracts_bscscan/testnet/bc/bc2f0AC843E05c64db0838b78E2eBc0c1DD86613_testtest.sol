...
contract testtest is Context, IBEP20, Ownable { // no spaces aloud for name here
    ...
    string private constant _name     = "testtest"; // name can have spaces.
    string private constant _symbol   = "TTT"; // symbol cannot have spaces.
    uint8  private constant _decimals = 9; // do not change unless you wanna redo safemath
    
    uint256 public  _percentageOfLiquidityForTeam       = 2500;
    uint256 public  _percentageOfLiquidityForMarketing = 2500;
    uint256 public  _percentageOfLiquidityForGiveaway   = 2500; //auto LP is whats left over from the value of 10,000 - LP here would be 2500

    // transfer fee only tax this if u want people to be taxed for moving tokens from wallet to wallet
    uint256 public  _taxFee       = 0; // this is token reflections
    uint256 public  _liquidityFee = 0; // this is divided between team, marketing, giveaway and auto LP

    // buy fee
    uint256 public  _taxFeeBuy       = 0; // reflections
    uint256 public  _liquidityFeeBuy = 15; // this is divided between team, marketing, giveaway and auto LP

    // sell fee
    uint256 public  _taxFeeSell       = 0;
    uint256 public  _liquidityFeeSell = 15; // these are percentage values 0-100
    
    uint256 public  _maxTxAmount     = _tTotal * 1000 / 10000; // set at 10%
    ...
    // anti whale
    bool    public _isAntiWhaleEnabled = true; //max wallet is on, change to false for off
    ...
    event GiveawaySent(address to, uint256 bnbSent);
    ...
    constructor () {
        ...
        // change this 
        _teamWallet       = 0x82137667E08ffbdB32Ba92baCda41FC9ce64E77b;
        _marketingWallet = 0xb6c2612A4c81705244B8505D437F82AC491431cD;
        _giveawayWallet   = 0xfAEc145ab00A2C3f43c1e18aF775Cbf864d2360d;
    ...
    function setAddresses(address teamWallet, address marketingWallet, address giveawayWallet) external onlyOwner {
        _teamWallet       = teamWallet;
        _marketingWallet = marketingWallet;
        _giveawayWallet   = giveawayWallet;
    }

    function setLiquidityPercentages(uint256 teamFee, uint256 marketingFee, uint256 giveawayFee) external onlyOwner {
        _percentageOfLiquidityForTeam        = teamFee;
        _percentageOfLiquidityForMarketing  = marketingFee;
        _percentageOfLiquidityForGiveaway    = giveawayFee;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 bnbForGiveaway   = newBalance / 10000 * _percentageOfLiquidityForGiveaway;
        uint256 bnbForLiquidity = newBalance - bnbForTeam - bnbForMarketing - bnbForGiveaway;
        ...
        if ( bnbForGiveaway != 0 ) {
            emit GiveawaySent(_giveawayWallet, bnbForGiveaway);
            payable(_giveawayWallet).transfer(bnbForGiveaway);
        }
