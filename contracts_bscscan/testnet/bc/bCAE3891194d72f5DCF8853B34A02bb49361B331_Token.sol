...
contract Token is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function setupFee(address feeAddress,string calldata feeName,uint256 fee) public onlyOwner {
    ...
    /* OWNER Functions */
    function addToWhitelist(address addy) public onlyOwner returns(bool) {return _addToWhitelist(addy);}
    function setWhitelistActive(bool flag) public onlyOwner {_whitelist.active = flag;}
    function _addToWhitelist(address addy) private onlyOwner returns(bool) {
        require(_whitelist.positions[addy] == 0, "address is already on the whitelist");
    ...
    function _takeFees(address recipient,uint256 amount) private {
        for ( uint i = 0 ; i < activeFees.length ; i++ ) {
          ...
          //_balancesByAddress[balanceType.address] += fee;
          //emit Transfer(recipient, balanceType.address, fee);
        }
    }
