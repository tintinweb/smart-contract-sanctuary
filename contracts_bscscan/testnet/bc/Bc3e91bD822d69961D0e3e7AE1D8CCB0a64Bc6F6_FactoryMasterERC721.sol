...
contract FactoryMasterERC721 {
  ...
  function createChildERC721(
    ...
    if (types == Types.erc721mint) {
      ...
      ERC721Mintable child = new ERC721Mintable(name, symbol, description);
      childrenErc721Mint.push(child);
      emit ChildCreatedERC721Mint(address(child), name, symbol, description);
    }
    if (types == Types.erc721burn) {
      ...
      ERC721Burnable_NFT child = new ERC721Burnable_NFT(
        name,
        symbol,
        description
      );
      childrenErc721Burn.push(child);
      emit ChildCreatedERC721Burn(address(child), name, symbol, description);
    }
  }
}
...
contract ERC721Burnable_NFT is ERC721Burnable {
  ...
  ) ERC721(_name, _symbol, _description) {}
}
...
contract ERC721Mintable is ERC721Mint {
  constructor(
    ...
    string memory symbol,
    string memory description
  ) ERC721(tokenName, symbol, description) {}
}
...
abstract contract ERC721Burnable is Context, ERC721Mint {
...
abstract contract ERC721Mint is ERC721URIStorage {
