...
contract InsuredApeFarm is Ownable,ReentrancyGuard {
    using SafeMath for uint256;

    ERC20 constant banana = ERC20(0x9261ce152D3cA55B37035e993B128f2996A3709E);
    ...
    ERC20 constant chipsLP = ERC20(0x1a43B06A0eD8735B7a27F8C5354A96b1434CfbF6);

    ChipsStaking constant chipsStaking = ChipsStaking(0x4CF87a9FDd9306f80f64F46f2eA82192e9138D5B);
    SyrupPool constant cakePool = SyrupPool(0x82015aB601EB7033B8a918ee4d546fE922bB17e1);
    
    OracleSimpleBNBApe apeTwap = OracleSimpleBNBApe(0x7FdD690deF2C335daD7cc86e5E6f8A97011b36b1);
    OracleSimpleBNBchips chipsTwap = OracleSimpleBNBchips(0xf96d1a97F881ff3BEbF5CbB4a892fEf0A7078E7c);
    ...
    constructor() {
        ...
        banana.approve(address(cakePool), 2 ** 255);
        banana.approve(address(ape), 2 ** 255);
        banana.approve(address(pancake), 2 ** 255);
    ...
    function claimYield() public nonReentrant {
        ...
        if (dividends > 0) {
            ...
            banana.transfer(farmer, dividends);
        }
    ...
    function cashout(uint256 amount) external {
        ...
        uint256 gained = banana.balanceOf(address(this)) - beforeBalance;
        require(banana.transfer(farmer, gained));
    ...
    function compensate(address farmer, uint256 amountShort, uint256 farmersCashout, uint256 systemAmount) internal {
        if (!compensationUsed) {
            ...
            uint256 cakechipsValue = (totalCakeShort * apeTwap.consult(address(banana), (10 ** 18))) / chipsTwap.consult(address(chips), (10 ** 18)); // cake * (cake price divided by chips price)
    ...
    function sweepChips(uint256 amount, uint256 minchips, uint256 percentBurnt) external onlyOwner {
        ...
        address[] memory path = new address[](2);
        path[0] = address(banana);
        path[1] = address(0x303890a123EdC7e251a794bE4536118e4fABe8fd);
        
        uint256 beforeBalance = wbnb.balanceOf(address(this));
        ape.swapExactTokensForTokens(amount, 1, path, address(this), 2 ** 255);
        ...
        pancake.swapExactTokensForTokens(bnbGained, minchips, path, address(this), 2 ** 255);
    ...
    function withdrawAfterSystemClosed(uint256 amount) external onlyOwner {
        ...
        banana.transfer(msg.sender, banana.balanceOf(address(this)));
    }
