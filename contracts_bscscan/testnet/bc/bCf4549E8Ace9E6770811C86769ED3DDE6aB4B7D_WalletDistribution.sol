...
contract WalletDistribution is Ownable {
    ...
    function setTeamAddresses(address _team1Addr, address _team2Addr) external onlyAuth{
        require(block.timestamp >= lastTeamAddressChange.add(1 days), 'To soon to change again');
    ...
    function validateChange() external onlyAuth{
        ...
        require(block.timestamp >= lastTeamAddressChange.add(7 days), "No change to validate");
        lastTeamAddressChange = block.timestamp;
        pendingChange = false;
        _team1 = _futurTeam1;
        _team2 = _futurTeam2;
    ...
    function payOwner() external onlyAuth returns(bool){
        ...
        require(block.timestamp >= lastPayment.add(7 days));
        ...
        require(BZAI.transfer(_team1, _toDistribute.div(2)));
        return(BZAI.transfer(_team2, _toDistribute.div(2)));
