...
contract HIREX is Context, IBEP20, Ownable {
    ...
    uint256 private totalLiqFee = liquidityFee.add(marketingFee).add(buybackFee).add(researchFee);
    ...
    uint256 public _maxTxAmount = 1000000 * 10**9; // Max buy per 1x txn 0,1% from Total Supply
    uint256 private numTokensSellToAddToLiquidity = 1000000 * 10**9; // Max sell per 1x txn 0,1% to LP
    ...
    constructor (address payable marketingWallet, address payable buybackWallet, address payable researchWallet, address pancakeSwapRouter) {
    ...
    function setLiquidityFeePercent(uint256 LiquidityFee) external onlyOwner() {
        ...
        totalLiqFee = liquidityFee.add(marketingFee).add(buybackFee).add(researchFee);
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            uint256 fees = amount.mul(totalLiqFee).div(100);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 ethForLiquidity = balanceDiff.sub(ethForMarketing).sub(ethForBuyback).sub(ethForResearch);
