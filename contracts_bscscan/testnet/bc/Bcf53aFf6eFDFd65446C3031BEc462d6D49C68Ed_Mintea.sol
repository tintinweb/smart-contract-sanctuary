...
contract Mintea is Context, IERC20, Ownable {
    ...
    address public marketingAndTeamFeeWallet = 0x733040F39EAFa802C096b2768784E2fad4C40136;
...
//        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);  // PCS V2
    ...
    function _distributeFee(uint256 rFee, uint256 rMarketingAndTeam, uint256 tFee, uint256 tMarketingAndTeam) private {
    ...
    function _getValues(uint256 tAmount) private view returns (ValuesStruct memory) {
        ...
        RValuesStruct memory rvs = _getRValues(tAmount, tvs.tFee, tvs.tMarketingAndTeam, tvs.tLiquidity, _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (TValuesStruct memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tMarketingAndTeam).sub(tLiquidity);
        return TValuesStruct(tTransferAmount, tFee, tMarketingAndTeam, tLiquidity);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tMarketingAndTeam, uint256 tLiquidity, uint256 currentRate) private pure returns (RValuesStruct memory) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rMarketingAndTeam = tMarketingAndTeam.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rMarketingAndTeam);
        return RValuesStruct(rAmount, rTransferAmount, rFee, rMarketingAndTeam, rLiquidity);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        _distributeFee(vs.rFee, vs.rMarketingAndTeam, vs.tFee, vs.tMarketingAndTeam);
