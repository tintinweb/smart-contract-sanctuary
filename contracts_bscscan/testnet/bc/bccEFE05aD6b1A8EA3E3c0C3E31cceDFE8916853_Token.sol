...
contract Token is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    function setDivider(uint256 amount) external onlyOwner {
    ...
    function freezeAndUnfreeze(address account, bool state) external onlyOwner {
        isFrozen[account] = state;
    }

    function setAntiBots(bool state) external onlyOwner {
        antiBots = state;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            // anti Sniper bots mechanisam
            if (!antiBots && from != owner()) {
                require(to != pancakePair, "BEP20: AntiBots disabled");
            }
        }
