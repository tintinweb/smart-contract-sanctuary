...
contract Charge is ERC20Burnable, AccessControlEnumerable, IShare {
	...
	function mint(address recipient_, uint256 amount_)
		...
		returns (bool)
	{
		require(totalSupply() + amount_ <= maxCap, 'Exceeds max cap');

		uint256 newMintTotalForMinter = _mintedAmount[_msgSender()] + amount_;
		require(
	...
	{
		return
			(totalSupply() + amount_ <= maxCap) &&
			(_mintedAmount[minter_] + amount_ <= _mintLimit[minter_]);
	}
}
