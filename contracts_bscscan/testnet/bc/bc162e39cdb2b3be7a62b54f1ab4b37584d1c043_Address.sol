...
// added functionnalities by the Glassswap's devs
//
...
contract SmartChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. GlasSwaps to distribute per block.
        uint256 lastRewardBlock;  // Last block number that BEARs distribution occurs.
        uint256 accBEARPerShare; // Accumulated BEARs per share, times 1e12. See below.
    ...
    // The block number when BEAR mining starts.
    uint256 public startBlock;
    // The block number when BEAR mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 BEARReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accBEARPerShare = accBEARPerShare.add(BEARReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accBEARPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // Withdraw BEAR tokens from STAKING.
