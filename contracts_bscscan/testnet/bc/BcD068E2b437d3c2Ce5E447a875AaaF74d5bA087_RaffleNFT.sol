...
contract RaffleNFT is Ownable, Pausable, Claimable, VRFConsumerBase {
	...
	mapping(bytes32 => uint8) public tiers;
	// Mapping of Participants per Tier
	mapping(bytes32 => mapping(uint8 => address[])) public participants;
	// Mapping of Winners per Tier
	mapping(bytes32 => mapping(uint8 => address)) public winnersMap;

	//Event
	event RequesId(bytes32 requestId, address[] listParticipants);
	event RandomResult(bytes32 requestId, uint8 tier, uint256 randomResult, address[] listParticipants, uint256 winnerNumber, address winner);
	...
	function getRequestRandomness(address[] calldata _participants, uint8[] calldata _tier, uint8 _tiers)
	...
	function fulfillRandomness(bytes32 _requestId, uint256 _randomness)
    	internal override
		{
			uint256[] memory randomResult = expand(_randomness, tiers[_requestId]);
			for (uint8 i = 1; i <= tiers[_requestId]; i++)
			{
				address[] memory _participants = participants[_requestId][i];
				uint256 numberWinner = randomResult[uint(i)].mod(uint(_participants.length)).add(1);
				winnersMap[_requestId][i] = _participants[numberWinner];
				emit RandomResult(_requestId, i, randomResult[uint(i)], _participants, numberWinner , winnersMap[_requestId][i]);
			}
		}
}
