...
contract Presale is Ownable, Whitelisted, ReentrancyGuard {
    ...
    IERC20 public token;
    address public Payee = 0xeE42F50308e824ee4902D503b6334a814cF62290; // wallet that recieves the bnb and liquidity
    ...
    uint256 public novaStage = 3600; //1800 = 30 minutes for nova/snova holders to buy first
    ...
    uint256 public liqPercent = 83; //amount of bnb used for liquidity div 100
    uint256 public liqTokens = 42; // amount of tokens used for liquidity div 100
    uint256 public fee = 1; // launchpad fee div 100
    
    uint256 public maxBuy = 1500000000000000000; // 2000000000000000000 = 2 bnb make sure to add 1 to avoid overflow on UI
    ...
    uint256 public novaSwapRate = 833333333333; // tokens per bnb snova stage
    uint256 public swapRate = 625000000000; // tokens per bnb normal stage
    uint256 public publicLaunchRate = 500000000000; // tokens per bnb for liquidity
    ...
    function swap() public payable onlyWhitelisted nonReentrant{
        uint256 quota = token.balanceOf(address(this)) - (fee/100) * (100 - liqTokens) / 100; // tokens available for purchase = total deposited mins 1% usage fee and liquidityTokens
        uint256 outAmount;
        ...
        if (block.timestamp < startBlock + novaStage ) {
            ...
            outAmount = msg.value * novaSwapRate;
        }
            else {
                outAmount = msg.value * swapRate;
            }
    ...
    function addLiquidity () external onlyOwner nonReentrant {
        ...
        uint256 feeBNB = address(this).balance * fee / 100;
        uint256 feeToken = token.balanceOf(address(this)) * fee / 100;

        uint256 bnbLiquidity = (address(this).balance - feeBNB) * liqPercent / 100;
        uint256 tokenLiquidity = bnbLiquidity * publicLaunchRate;
   ...
   function withdrawBNB() external nonReentrant{
    ...
    function emergencyWithdrawTokens () external nonReentrant {
        require(msg.sender == address(Payee), 'You cannot withdraw tokens');
        require(presaleFailed == true, 'Presale not failed');
