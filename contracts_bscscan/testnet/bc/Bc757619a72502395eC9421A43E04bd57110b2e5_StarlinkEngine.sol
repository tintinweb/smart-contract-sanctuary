...
contract StarlinkEngine is IStarlinkEngine, AccessControlled, EmergencyWithdrawable {
    ...
    uint256 lastFundsProcessTime;
    uint256 gasForFundsMagnitude = 300;
    uint256 processFundsPeriod = 1 minutes;
    ...
    function addGas(uint256 gas) public override nonReentrant notUnauthorizedContract {
        uint256 gasForFunds = gas * gasForFundsMagnitude / 1000;
        uint256 gasRemaining = processFunds(gasForFunds);
        processQueue(gasRemaining);
    }

    function buyGas(uint256 gas) external onlyAdmins refundGasCost {
        addGas(gas);
        emit GasBought(gas);
    }

    function donate() external payable override {
        require(msg.value > 0, "StarlinkEngine: Invalid amount");
        emit Donated(tx.origin, msg.value);
    }

    function setOptions(uint256 _gas, uint256 _processFundsPeriod) external onlyOwner {
        require(_gas <= 1000, "StarlinkEngine: Invalid value");
        gasForFundsMagnitude = _gas;
        processFundsPeriod = _processFundsPeriod;
    }

    function processQueue(uint256 gas) internal {
        if (gas > 0) {
            try xld.processRewardClaimQueue(gas) { } catch { }
        }
    }

    function processFunds(uint256 gas) internal returns (uint256) {
        uint256 gasLeft = gasleft();
        if (block.timestamp - lastFundsProcessTime >= processFundsPeriod) {
            if (gas > 0) {
                try starlink.processFunds(gas) { } catch { }
            }
        }
