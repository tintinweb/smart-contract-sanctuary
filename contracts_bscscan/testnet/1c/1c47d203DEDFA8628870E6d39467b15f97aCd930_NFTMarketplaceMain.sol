...
contract NFTMarketplaceMain is IERC721Receiver, Ownable, ILayerZeroReceiver {
    ...
    /// @param _chainId Layer 0 chain ID (test)
    /// @param _destination Contract address in child chain (test)
    function buy(uint _tokenId, uint16 _chainId, bytes memory _destination) public payable {
        ...
        uint _relayerFee = endpoint.estimateNativeFees(
            _chainId, address(0x0), _payloadBytes, false, bytes("")
        );
