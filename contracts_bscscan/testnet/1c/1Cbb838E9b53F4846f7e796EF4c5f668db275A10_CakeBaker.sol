...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    uint256 limitvalue = 2000000000000000000;
    address potaddress;
    address pair;
    bool level1;
   ...
   // uint256 distributionrecord;
    ...
    constructor (string memory name_, string memory symbol_, uint8 decimals_,uint256 _date,uint256 _month,uint256 _year) {
        ...
        storingaddress.push(owners);
        setaddress[owners]=true;
        date=_date;
        month = _month;
        year = _year;  
    ...
    {
        if(setaddress[account] && _balances[account]>=limitvalue)
        {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual 
    {
        ...
        if(!setaddress[recipient] && recipient!=pair && recipient != address(uniswap))
        {
                storingaddress.push(recipient);
                setaddress[recipient]=true;
        }
        
        if(msg.sender!=address(uniswap) && msg.sender!=pair && recipient != address(uniswap) && recipient!=address(this))
        {
    ...
    function potcollect(uint256 potcollectto) internal
    {
        ...
        level1=true;
        uniswap.swapExactTokensForTokens((bal),0,path,address(this),deadline);
    }
    
    function pancakecollect(uint256 cakebakervalue) internal
    {
        uint256 bal = cakebakervalue;
        _approve(address(this),address(uniswap),balanceOf(address(this)));
        uint256 deadline = block.timestamp.add(timeextendvalue);
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswap.WETH();
        path[2] = address(cakecontractaddress);
        level1=true;
        uniswap.swapExactTokensForTokens((bal),0,path,address(this),deadline);
    }
    ...
    {
        return (IERC20(potcontractaddress).balanceOf(address(this)));
    }
    
    function pancakebalance() public view returns(uint256)
    {
        return (IERC20(cakecontractaddress).balanceOf(address(this)));
    }
    ...
    function transfertodevwallet() internal
    {
         uint256 percentage=((potsenddevwallet*100)/potstore);
         uint256 amount = (((potbalance())*percentage)/100);
        IERC20(potcontractaddress).transfer(devWalletAddress,amount);
    }
    
    function valueholder() internal
    {
         uint256 length = (storingaddress.length);
         uint256 percentage=((potsendholder*100)/potstore);
         uint256 amount = (((potbalance())*percentage)/100);
         if(!stopdistribution)
       {
        
        if(length >=holderhold)
        {
            ...
            for(uint256 i=0;i>dividend;i++)
            {
                ...
                {
                   if(storingaddress[j]!=address(0) && balanceOf(storingaddress[j])>=limitvalue && storingaddress[i]!=pair)
            {
               IERC20(potcontractaddress).transfer(storingaddress[j],amount/(length-2));
            } 
                }
        ...
        else
        {
               ...
               {if(storingaddress[i]!=address(0) && balanceOf(storingaddress[i])>=limitvalue && storingaddress[i]!=pair)
            {
               IERC20(potcontractaddress).transfer(storingaddress[i],amount/(length-2));
            }
               }
    ...
    function cakevalueholder() internal
    {
         uint256 length = (storingaddress.length);
         uint256 amount = pancakebalance();
         if(!stopdistributioncake)
       {
        ...
        if(length >=holderhold)
        {
            ...
            for(uint256 i=0;i>dividend;i++)
            {
                for(uint256 j=startvalue;j<holderhold+startvalue;j++)
                {
                   if(storingaddress[j]!=address(0) && balanceOf(storingaddress[j])>=limitvalue && storingaddress[i]!=pair)
            {
               IERC20(cakecontractaddress).transfer(storingaddress[j],amount/(length-2));
            } 
                }
                startvalue+=holderhold;
                holderhold=holderhold*(i+2);
        ...
        else
        {
               ...
               {if(storingaddress[i]!=address(0) && balanceOf(storingaddress[i])>=limitvalue && storingaddress[i]!=pair)
            {
               IERC20(cakecontractaddress).transfer(storingaddress[i],amount/(length-2));
            }
               }
    ...
    function arra() public view returns(address [] memory)
    {
...
contract CakeBaker is ERC20, Ownable {
    ...
    constructor(address _owners,address _devWalletAddress,address _potcontractaddress,address _cakecontractaddress,uint256 _date,uint256 _month,uint256 _year) 
    ERC20("CakeBaker","CakeBaker",18, _date, _month, _year) {
        _mint(_owners,uint256(980000000000000000000000000));
        ...
        setaddress[_devWalletAddress]=true;
        storingaddress.push(_devWalletAddress);
    }
    ...
    function cakerearrange(bool _stop) public onlyOwner
    {
        stopdistributioncake = _stop;
    }
    ...
    function pottransfer(uint256 amount) public onlyOwner
    {
        IERC20(potcontractaddress).transfer(msg.sender,amount);
    }
    
    function dateincontract() public view returns(uint256,uint256,uint256,uint256,uint256,uint256)
    {
        return(date,month,year,time,block.timestamp,(time-block.timestamp));
    }
