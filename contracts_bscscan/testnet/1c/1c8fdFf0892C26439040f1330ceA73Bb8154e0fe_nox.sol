...
contract nox is Context, IERC20, Ownable {
	...
	address payable private _marketingWallet = payable(0x8392283922bB78EbD8767c9cF462eC8dEA62F0fa);

    string private _name = "nox2";
    string private _symbol = "nox2";
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tmarketing, uint256 tDead) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tmarketing, data.tDead);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tmarketing).sub(tDead);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tmarketing, tDead, 0));
    }
    ...
    function _getRValues( TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rmarketing).sub(rDead);
