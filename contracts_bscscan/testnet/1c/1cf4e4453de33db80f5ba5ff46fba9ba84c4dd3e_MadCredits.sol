...
contract MadCredits is IBEP20, Auth {
    ...
    function swapBack() public swapping {
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            if (reflectFee > 0) {
                ...
                try distributor.deposit(busdToDistributor) {
                    emit DividendDeposit(block.timestamp, busdToDistributor);
                } catch {}
            }
    ...
    event DividendDeposit(uint256 timestamp, uint256 amountDeposited);
    event BasicTransfer(address sender, address recipient, uint256 amount);
    event ProcessingDividends();
}