...
contract KiBridgeV22 is KiBridgeV1Factory {
  ...
  struct TokenInfo {
    ...
    mapping(uint256 => bytes4) fromChainExecuteFunctionId;
  ...
  function setTokenInfo(
    ...
    permittedTokenOutFunctionId(_executeFunctionId)
  {
    require(_fromChainId != block.chainid && _toChainId != block.chainid, "Wrong chainId");
    tokenInfo[_token].liquidable = _liquidable;
    tokenInfo[_token].tokenInFunctionId = _tokenInFunctionId;
    tokenInfo[_token].fromChainExecuteFunctionId[_fromChainId] = _executeFunctionId;
    tokenInfo[_token].toChainToken[_toChainId] = _toChainToken;
  }
  function updatePendingFulfilled(
    IERC20 _token,
    uint256 _chainId,
    uint256 _amount
  ) private {
  ...
  function updateFulfilledPending(
    IERC20 _token,
    uint256 _chainId,
    uint256 _amount
  ) private {
    ...
    external
  {
    ...
    updatePendingFulfilled(liquidityRequest.token,liquidityRequest.toChainId,liquidityRequest.amount);
    ...
    returns (bool success, bytes memory result)
  {
    ...
    if (tokenInfo[_receiveToken].liquidable) {
      updatePendingFulfilled(_receiveToken,_fromChainId,_amount);
    }
    // solhint-disable-next-line avoid-low-level-calls
    return address(this).delegatecall(
      abi.encodeWithSelector(
      tokenInfo[_receiveToken].fromChainExecuteFunctionId[_fromChainId],
      _user,
      _receiveToken,
      _amount));
  }
    ...
    returns (bool success)
  {
    ...
    if (fromChainId != toChainId) updatePendingFulfilled(token,toChainId,refund);
    ...
    checkAmount(_amount)
  {
    ...
    updateFulfilledPending(_token,_toChainId,_amount);
    ...
    checkAmount(_amount)
  {
    ...
    liquidityInfo[_token].toAllChainableBalance -= _amount;
    liquidityInRemoveToChain(_specId, _toChainId, _token, _amount);
  }
  function removeLiquidityWithPermit(
    ...
    removeLiquidity(_specId, _toChainId, _token, _amount);
  }
    ...
    returns (uint256 amount) 
  {
    ...
    if (liquidityInfo[_token].toAllChainableBalance + _fulfilled < _pending + _amount) return 0;
  }
    ...
    ) 
  {
    return (
      ...
      tokenInfo[_token].fromChainExecuteFunctionId[_fromChainId],
      tokenInfo[_token].toChainToken[_toChainId]
    );
  }
