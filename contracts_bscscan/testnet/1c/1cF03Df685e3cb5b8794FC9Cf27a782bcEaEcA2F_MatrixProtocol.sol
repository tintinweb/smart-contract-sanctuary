...
contract MatrixProtocol is IBEP20, LPSwapSupport, AntiLPSniper {
    ...
    constructor (address _routerAddress, address _tokenOwner) public payable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if(from == pancakePair && !tradingOpen && antiSniperEnabled){
                ...
                require(tradingOpen, "Trading has not yet opened");
            }
            ...
            if(from == pancakePair){ // Buy
                ...
                (rAmount, tTransferAmount, rTransferAmount) = takeFees(amount, true);
            } else {
                (rAmount, tTransferAmount, rTransferAmount) = takeFees(amount, false);
            }
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        } else if(contractTokenBalance >= minTokenSpendAmount){
            swapTokensForCurrencyAdv(address(this), tokenTracker.marketing, address(marketingWallet));
            swapTokensForCurrencyAdv(address(this), tokenTracker.charity, address(charityWallet));
            tokenTracker.marketing = 0;
            tokenTracker.charity = 0;
        }
    }

    function takeFees(uint256 amount, bool isSell) private returns(uint256 rAmount, uint256 tTransferAmount, uint256 rTransferAmount){
        Fees memory _fees = isSell ? sellFees : buyFees;
    ...
    function updateBuyFees(uint256 reflectionFee, uint256 liquidityFee, uint256 charityFee, uint256 marketingFee, uint256 newFeeDivisor) public onlyOwner {
        buyFees = Fees({
    ...
    function updateSellFees(uint256 reflectionFee, uint256 liquidityFee, uint256 charityFee, uint256 marketingFee, uint256 newFeeDivisor) public onlyOwner {
        buyFees = Fees({
            reflection: reflectionFee,
            liquidity: liquidityFee,
            charity: charityFee,
