...
contract MagicStella is BEP20, Ownable {
  ...
  uint256 public _sellFee = 700;
	uint256 public _MaxPurcentFee = 10000;
  ...
  address public constant USDT = address(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
	...
	function _transfer(
      ...
      if (_isSellerTransfer(to) && _priceImpactTax(amount)) {
        localFeesRation = localFeesRation.add(_sellFee);
			...
			uint256 fees = (amount.mul(localFeesRation)).div(_MaxPurcentFee);
    ...
    //     console.log(`Price impact when trading ${amount_traded_usdt} USDT: ${price_impact_trade_usdt*100}%`);
		...
		function _GetFees(uint256 amount) public view returns (uint256, uint256, uint256){
      ...
      if (_priceImpactTax(amount)) {
        localFeesRation = localFeesRation.add(_sellFee);
			...
			return (amount.sub(fees), fees, localFeesRation);
		}
