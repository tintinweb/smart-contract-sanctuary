...
contract PlayToWinGames {
    ...
    event ConfirmGame(address indexed player, string indexed game, uint profit, bytes choice, uint game_choice, bytes32 seed, bool status, bool draw,  uint timestamp);
    event ConfirmGameRolling(address indexed player, string  game, uint profit, bytes min, bytes max, uint game_choice, bytes32 seed, bool status, bool draw,  uint timestamp);
    ...
    function game_rolling(bytes memory _choice1,bytes memory _choice2, bytes32 seed) public betInRange payable {
        ...
        //Checking that bids are in the right range
        require(user_choice >= 1 && user_choice2 <=100 && ((user_choice2 - user_choice) <= 94));
    ...
    function confirm(bytes32 seed, bytes32 _s) public onlyCroupier {
        ...
        if (game.game_title == Games.CoinFlip){
            ...
            if(game.result == user_choice){
                ...
                emit ConfirmGame(game.player, 'CoinFlip', profit, game.choice1, game.result, game.seed, true, false, now);
            }else{
                game.state = GameState.Lose;
                emit ConfirmGame(game.player, 'CoinFlip', 0, game.choice1, game.result, game.seed, false, false, now);
            }
        //KNB game algorithm
            ...
            if(game.result != user_choice){
                if (user_choice == 1 && game.result == 2 || user_choice == 2 && game.result == 3 || user_choice == 3 && game.result == 1) {
                    ...
                    game.profit = profit;
                    game.player.transfer(profit);
                    emit ConfirmGame(game.player, 'KNB', profit, game.choice1, game.result, game.seed, true, false, now);
                }
                else{
                    game.state = GameState.Lose;
                    emit ConfirmGame(game.player, 'KNB', 0, game.choice1, game.result, game.seed, false, false, now);
                }
            }else{
                ...
                emit ConfirmGame(game.player, 'KNB', profit, game.choice1, game.result, game.seed, false, true, now);
            }
        //Dice game algorithm
            ...
            if(profit > 0){
                ...
                game.profit = profit;
                game.player.transfer(profit);
                emit ConfirmGame(game.player, 'Dice', profit, game.choice1, game.result, game.seed, true, false, now);
            }else{
                game.state = GameState.Lose;
                emit ConfirmGame(game.player, 'Dice', 0, game.choice1, game.result, game.seed, false, false, now);
            }
        }
            ...
            if(profit > 0){
                ...
                emit ConfirmGameRolling(game.player, 'Rolling', profit, game.choice1, game.choice2, game.result, game.seed, true, false, now);
            }else{
                game.state = GameState.Lose;
                emit ConfirmGameRolling(game.player, 'Rolling', 0, game.choice1, game.choice2, game.result, game.seed, false, false, now);
            }
        }
