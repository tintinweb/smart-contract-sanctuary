...
contract Velocity is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 5 * 10**12 * 10**18;
    ...
    uint256 public _TeamWallet = 0;
    uint256 private _previousTeamWalletFee = _TeamWallet;

    uint256 public _liquidityFee = 0;
    uint256 private _previousLiquidityFee = _liquidityFee;
    address public teamwalletAdd = 0xcd85C5859D04BF27ba05b782c8b13e176f93bd34;
    ...
    function setTeamWalletAdd(address addr) external onlyOwner() {
        teamwalletAdd = addr;
    }

    function setTeamWalletFeePercent(uint256 TeamWallet) external onlyOwner() {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 teamwallet = calculateTeamWalletFee(tAmount);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(teamwallet).sub(tLiquidity);
        return (tTransferAmount, tFee, teamwallet, tLiquidity);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 teamwallet, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rTeamWallet = teamwallet.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rTeamWallet);
    ...
    function _teamwallet(uint256 teamwallet) private {
        uint256 currentRate =  _getRate();
        uint256 rTeamWallet = teamwallet.mul(currentRate);
        _rOwned[teamwalletAdd] = _rOwned[teamwalletAdd].add(rTeamWallet);
    ...
    function calculateTeamWalletFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_TeamWallet).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _TeamWallet == 0 && _liquidityFee == 0) return;

        _previousTaxFee = _taxFee;
        _previousTeamWalletFee = _TeamWallet;
    ...
    function restoreAllFee() private {
        ...
        _TeamWallet = _previousTeamWalletFee;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 teamwallet, uint256 tLiquidity) = _getValues(tAmount);
        ...
        emit Transfer(sender, teamwalletAdd, teamwallet);
