...
contract EverRise is Context, IERC20, Ownable {
    ...
    function setLiquidityFee(uint256 liquidityFee) external onlyOwner() {
        require(_buyBackFee.add(_marketingFee).add(liquidityFee) <= _fundingFee);
    ...
    function setMarketingFee(uint256 marketingFee) external onlyOwner() {
        require(_liquidityFee.add(_buyBackFee).add(marketingFee) <= _fundingFee);
        _marketingFee = marketingFee;
    }
    
    function setBuyBackFee(uint256 buyBackFee) external onlyOwner() {
        require(_liquidityFee.add(_marketingFee).add(buyBackFee) <= _fundingFee);
