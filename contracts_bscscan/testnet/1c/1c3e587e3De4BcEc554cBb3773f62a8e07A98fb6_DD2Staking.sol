...
contract DD2Staking is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accCakePerShare; // Accumulated CAKEs per share, times 1e2. See below.
        bool exist;
    }
    ...
    function pendingCake(uint256 _pid, address _user) external view existPool(_pid) returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accCakePerShare = accCakePerShare.add(cakeReward.mul(1e2).div(lpSupply));
        }
        return user.amount.mul(accCakePerShare).div(1e2).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public existPool(_pid) {
        ...
        pool.accCakePerShare = pool.accCakePerShare.add(cakeReward.mul(1e2).div(lpSupply));
    ...
    function deposit(uint256 _pid, uint256 _amount) public existPool(_pid) {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accCakePerShare).div(1e2).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accCakePerShare).div(1e2);
        user.mintNFT = true;
...
//    }
    ...
    function getReward(uint _pid) external view returns (uint256) {
//        require(poolInfo[_pid].exists, "PoolInfo does not exist.");
    ...
    function getStaking(uint _pid) external view existPool(_pid) returns (uint256) {
//        require(poolInfo[_pid].exists, "PoolInfo does not exist.");
        require (_pid != 0, 'withdraw CAKE by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function harvest(uint _pid) public existPool(_pid) {
//        require(poolInfo[_pid].exists, "PoolInfo does not exist.");
    ...
    function reCalReward(uint _pid) public existPool(_pid) {
//        require(poolInfo[_pid].exists, "PoolInfo does not exist.");
...
//                rewardToken.safeTransfer(address(msg.sender), pending);
    ...
    function getD2NFT(uint _pid) public existPool(_pid) returns (uint256) {
        //check > day
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
