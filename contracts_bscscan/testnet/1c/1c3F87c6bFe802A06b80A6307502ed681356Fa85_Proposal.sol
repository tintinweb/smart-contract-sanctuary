...
contract Proposal is Ownable {
    ...
    function checkHoldings(address account) public view isActive returns (uint) {
        ...
        uint totalSupply;
        if (circulation == 0) {
            totalSupply = KTA.totalSupply() - KTA.burned();
        } else {
            totalSupply = circulation;
        }

        return balanceOf >= totalSupply * ratio / 100 ? 0 : 1;
    }
    ...
    function vote(uint pId_, uint[] calldata options_, uint[][] calldata tickets_) public isActive returns (bool) {
        ...
        require(voteInfo[pId_].startTime <= block.timestamp && block.timestamp <= voteInfo[pId_].endTime , "wrong time");
