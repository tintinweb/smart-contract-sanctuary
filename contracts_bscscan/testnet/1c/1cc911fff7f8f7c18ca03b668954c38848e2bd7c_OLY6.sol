...
/**
 remove txlimit to remove lp
 swapThreshold determines auto lp and conversion to dividend
 feeMultipliersDuration - 180 min
 
 WIP
 whitelist not working for owner
 isFeeExempt not working for owner and all (sell is working)
 txlimitexempt not working for owner and all ( sell is working) <- mightbe due to issell
...
contract OLY6 is IBEP20, Auth {
    ...
    string constant _name = "OLY6";
    string constant _symbol = "OLY6";
  ...
  //  uint256 buybackMultiplierLength = 30 minutes;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
     ...
     function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
      ...
      //  require( balanceOf(recipient) + amount <= maxWallet,"Exceeds maximum wallet token amount (2%)"); // added for max wallet
