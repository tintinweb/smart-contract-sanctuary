...
contract IBGBooster is Ownable {
    ...
    mapping(address => uint256) private depositCheckpoint;
    mapping(address => bool) private withdrawalsList;

    mapping(address => ClaimInfo) private claimInfo;

    uint256 public ONE_DAY = 1;
    uint256 public lockPeriod = uint256(90).mul(ONE_DAY);
    uint256 public withdrawInstallmentInterval = uint256(7).mul(ONE_DAY);
    ...
    event onPanalty(uint256 panaltyAmount,address user);
    event onInstallmentPaid(address user,uint256 rewardAmount,uint256 principalAmount,uint256 claimNumber);
    ...
    function setDepositEnalbed(bool isEnabled) public onlyOwner {
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require( depositCheckpoint[msg.sender] == 0, "multiple deposits are not allowed");
        updatePool(_pid);
        if (user.amount > 0) {
    ...
    function withdraw(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(withdrawalsList[msg.sender] == true, 'cant use this function use claim function');
        ...
        user.amount =0;
        user.rewardDebt = user.amount.mul(pool.accIBGPerShare).div(1e12);
        handleWithdrawal(rewardAmount,principalAmount,msg.sender);
    }


    function handleWithdrawal(uint256 rewardAmount ,uint256 principalAmount,address user) internal{


        bool isPreMature = depositCheckpoint[user].add(lockPeriod) > block.timestamp;
        if(isPreMature){
            uint256 panaltyAmount = rewardAmount.mul(prematurePaneltyPercent).div(100);
            if(panaltyAmount > 0){
                safeIBGTransfer(feeAddress, panaltyAmount);
                emit onPanalty(panaltyAmount,user);
            }
        ...
        if(paymentSum > 0){
            safeIBGTransfer(user, paymentSum);
        }
        ...
        withdrawalsList[msg.sender] = true;
        emit onInstallmentPaid(user,rewardAmountInstallment,principalInstallment,1);
    }


    function claimInstallment() public {
        ClaimInfo storage _claimInfo = claimInfo[msg.sender];
        ...
        require(_claimInfo.lastClaimed.add(withdrawInstallmentInterval) <block.timestamp," Withdrawal intrval not passed yet ");
        ...
        emit onInstallmentPaid(msg.sender,rewardAmountInstallment,principalInstallment,_claimInfo.claims);
