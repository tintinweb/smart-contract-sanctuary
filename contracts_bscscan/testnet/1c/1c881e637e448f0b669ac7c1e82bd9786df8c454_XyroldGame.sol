...
contract XyroldGame {  
    ...
    function GameInfo(uint _gameId) view public returns (uint256 ID, address player1, address player2, address winner, uint256 prize, string memory status) {
        Game storage _getGame = GamesPool[_gameId];
        string memory _status = "Closed";
        if(_getGame.status == 1){
            _status = "Open";
        }
        return (_getGame.ID, _getGame.creator, _getGame.oponent,  _getGame.winner, (_getGame.bet * 2), _status);
    }
    ...
    function RoundChoices(uint _gameId, address _player) view public returns (uint256 ID, string memory round1, string memory round2, string memory round3) {
        Game storage _getGame = GamesPool[_gameId];
        return (_getGame.ID, getchoicename(_getGame.details[_player].round1), getchoicename(_getGame.details[_player].round1),  getchoicename(_getGame.details[_player].round1));
    }
