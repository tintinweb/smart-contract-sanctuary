...
contract WhaleNetworkGovernance is Context, Ownable {
    ...
    mapping(uint256 => mapping(address => VoteStatus)) _voter;
    ...
    function castVote(uint256 proposalId, uint256 vote)external returns(bool){
        //vote = 0 - NO
        //vote = 1 - YES
        require(
        ...
        require( !_proposalVotingStatus[proposalId][_msgSender()], "You have already voted on this proposal");
        ...
        if(vote == 1){
            ...
            emit VoteCast(_msgSender(), _token.balanceOf(_msgSender()), Vote.FOR);
        }
        if(vote == 0){
            proposalVote.against += _token.balanceOf(_msgSender());
            emit VoteCast(_msgSender(), _token.balanceOf(_msgSender()), Vote.AGAINST);
        }
        return true;
    ...
    function getResult(uint256 proposalId) public view returns(VoteStatus memory){
        require(
            proposalId > 0 && proposalId < _nextProposalId,
            "WhaleNetworkGovernance: Invalid proposal Id"
        );
