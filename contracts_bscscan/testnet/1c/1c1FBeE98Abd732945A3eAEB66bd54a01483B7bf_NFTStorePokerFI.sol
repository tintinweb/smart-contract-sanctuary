...
contract NFTStorePokerFI is ReentrancyGuard, Ownable {
  ...
  struct MarketItem {
    ...
    uint256 price;
      ...
      uint lote;                  /*lote paginacao*/
  }

  /*Complementos estrutura de carteiras por TokenID*/
  ...
  /*Complementos estrutura de NFT campeonatos por TokenID*/
  struct GameTokenId {
    uint256 tokenId;    /*NFT contract*/    
    uint256 userID;    /*posicaoglobal*/    
    uint256 itemID;    /*Store-ID*/
    uint256 gameID;    /*ptChamp*/
  }
  ...
  function setupWallets (address payable walletAdm ,address payable walletDev, address payable walletGame ) public onlyOwner() {
   require( _owner == msg.sender,"security guaranteed administrator only");
    _wallet1 = walletAdm;
    _wallet3 = walletDev;
  ...
  function createMarketItem( 
    ...
    uint behavior;
    if ( _owner == msg.sender ) { behavior = 0; } 
...
//    require(idToMarketItem[itemId].behavior == 1 || idToMarketItem[itemId].behavior == 0, "Only game's and user's NFT itens can to sell in this function");
  ...
  function fetchGameItems() public view returns (GameTokenId[] memory) {
    uint itemCount = _itemIds.current();
    uint unsoldItemCount = _itemIds.current() - _itemsSold.current();
    uint currentIndex = 0;

    GameTokenId[] memory items = new GameTokenId[](unsoldItemCount);


    for (uint i = 0; i < itemCount; i++) {
      if (idToMarketItem[i + 1].owner == address(0)) {

        uint currentId = idToGameTokenId[i + 1].tokenId;
