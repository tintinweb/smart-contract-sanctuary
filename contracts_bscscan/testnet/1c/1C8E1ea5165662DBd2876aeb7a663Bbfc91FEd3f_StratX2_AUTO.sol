...
abstract contract StratX2 is Ownable, ReentrancyGuard, Pausable {
    ...
    function deposit(address _userAddress, uint256 _wantAmt) public virtual onlyOwner nonReentrant whenNotPaused returns (uint256) {
    ...
    function withdraw(address _userAddress, uint256 _wantAmt) public virtual onlyOwner nonReentrant returns (uint256) {
    ...
    function earn() public virtual nonReentrant whenNotPaused {
        ...
        if (earnedAddress != token0Address) {
            ...
            _safeSwap(uniRouterAddress, earnedAmt.div(2), slippageFactor, earnedToToken0Path, address(this), block.timestamp.add(600));
        }

        if (earnedAddress != token1Address) {
            // Swap half earned to token1
            _safeSwap(uniRouterAddress, earnedAmt.div(2), slippageFactor, earnedToToken1Path, address(this), block.timestamp.add(600));
        }
        ...
        if (token0Amt > 0 && token1Amt > 0) {
            ...
            IPancakeRouter02(uniRouterAddress).addLiquidity(token0Address, token1Address, token0Amt, token1Amt, 0, 0, address(this), block.timestamp.add(600));
        }
    ...
    function buyBack(uint256 _earnedAmt) internal virtual returns (uint256) {
        ...
        if (earnedAddress == AUTOAddress) {
            ...
            _safeSwap(uniRouterAddress, buyBackAmt, slippageFactor, earnedToAUTOPath, buyBackAddress, block.timestamp.add(600));
        }
    ...
    function _safeSwap(
        ...
        IPancakeRouter02(_uniRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(_amountIn, amountOut.mul(_slippageFactor).div(1000), _path, _to, _deadline);
    }
}
...
contract StratX2_AUTO is StratX2 {
    ...
    function deposit(address _userAddress, uint256 _wantAmt) public override onlyOwner nonReentrant whenNotPaused returns (uint256) {
    ...
    function withdraw(address _userAddress, uint256 _wantAmt) public override onlyOwner nonReentrant returns (uint256) {
        require((userLastDepositedTimestamp[_userAddress].add(minTimeToWithdraw)) < block.timestamp, "too early!");
    ...
    function earn() public override nonReentrant whenNotPaused {
        ...
        IERC20(earnedAddress).safeApprove(uniRouterAddress, 0);
        IERC20(earnedAddress).safeIncreaseAllowance(uniRouterAddress, earnedAmt);
        _safeSwap(uniRouterAddress, earnedAmt, slippageFactor, earnedToAUTOPath, address(this), block.timestamp.add(600));
