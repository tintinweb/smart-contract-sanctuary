...
contract Manarium is ERC20, Ownable
{
    ...
    uint8 constant public PRIZE_POOL_FEES = 10; // 1%
    uint8 constant public DISTRIBUTION_FEES = 5; // 0.5%
    uint8 constant public DEVELOPMENT_FEES = 5; // 0.5%
    uint8 constant public TOTAL_FEES = 20; // 2%
    uint16 constant public PROCENT_DENOMINATOR = 1000; // 100%
    ...
    address constant public REFERRAL_WALLET = 0xd962c2cB2B006049B2f7B2e2a142a4020804d413;
    address constant public AIRDROP_WALLET = 0xa3D296669d4a7268fc85Ac6261b31Ca8E6AEebc8;
    ...
    uint256 private _distributorGas = 500000; // 500 000
    ...
    constructor() ERC20("Manarium", "ARI"){
        ...
        _router = IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); // PANCAKE SWAP
        _pair = IDEXFactory(_router.factory()).createPair(WBNB, address(this));
        _distributor = new DividendDistributor();

        _excludeFromFeesAndDividends(address(this), true);
        _excludeFromFeesAndDividends(address(0), true);
        _excludeFromFeesAndDividends(address(_distributor), true);
        _excludeFromFeesAndDividends(msg.sender, true);
        _excludeFromFeesAndDividends(DEVELOPMENT_WALLET, true);
        _excludeFromFeesAndDividends(REFERRAL_WALLET, true);
        _excludeFromFeesAndDividends(PRIZE_POOL_WALLET, true);
        _excludeFromFeesAndDividends(AIRDROP_WALLET, true);
    ...
    function excludeFromFeesAndDividends(address account_, bool excluded_) public onlyOwner{
        _excludeFromFeesAndDividends(account_, excluded_);
    }

    function _excludeFromFeesAndDividends(address account_, bool excluded_) private {
    ...
    function referralTransfer(address receiver, uint256 amount) public onlyOwner referral returns(bool)
    {
        require(amount > 0);
        require(balanceOf(PRIZE_POOL_WALLET) >= amount);
        bool result = transferFrom(PRIZE_POOL_WALLET, receiver, amount);
    ...
    internal override {
        ...
        if(_inSwap || _inAirdropTransfer || _inReferralTransfer) 
    ...
    function swapBack() internal swapping {
        ...
        uint256 developmentAmount = amount.mul(DEVELOPMENT_FEES).div(TOTAL_FEES);
        uint256 prizePoolAmount = amount.mul(PRIZE_POOL_FEES).div(TOTAL_FEES);
        uint256 distributionAmount = amount.mul(DISTRIBUTION_FEES).div(TOTAL_FEES);

        swapTokensForDevelopment(developmentAmount);
        sendToDistribtuion(distributionAmount);
        sendToPrizePool(prizePoolAmount);
    }

    function swapTokensForDevelopment(uint256 amount) private returns (bool){
        uint256 amountBNB = swapTokens(amount);
        (bool success, ) = payable(DEVELOPMENT_WALLET).call{value: amountBNB, gas: 30000}("");
        return success;
    }

    function sendToDistribtuion(uint256 amount) private{
        _transfer(address(this), address(_distributor), amount);
        try _distributor.deposit(amount) {} catch {}
    ...
    function setSwapBackSettings(uint256 _amount) external onlyOwner {
     ...
     function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external onlyOwner {
        _distributor.setDistributionCriteria(_minPeriod, _minDistribution);
    }
