...
contract Buds is Context, IERC20, Ownable, VRFConsumerBase(0xa555fC018435bef5A13C6c6870a9d4C11DEC329C, 0x84b9B910527Ad5C03A9Ca831909E21e236EA7b06) {
    ...
    //1)when BNB is recieved from pancakeSwap (buds going out BNB in)
    //2)when ownerContract sends coins to an address
        //Subtraction from sumOfBalances\\
    //1)when users sells their coins to pancakeswap (buds coming in BNB out)
    ...
    constructor(
        ...
        balances[ownerContract] = supply - supply/100;
        reservedDividendsForHOLDRs = supply/100;
    ...
    function viewCharityWallets() public view returns(address, address) {
        return(goodBudsWallet, smokeOutWallet);
    }
    ...
    //ADDITIONAL FUNCTIONALITY HERE NEEDED FOR PANCAKESWAP
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        //below sender and if-else statement is only for testing purposes
        address sender;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        if (randomness < firstThirdOfMax) {
            ...
            distributeLottery(randomness, requestId);
        }
    }
    ...
    function distributeLottery(uint randomnValue, bytes32 requestId) public returns (address, uint) {
        uint secondThirdOfMax = 2*maxInteger/3;
...
contract Derived is Buds {
    ...
    constructor(
        ...
        address manualLiquidityWallet, 
        address zombie) 
        Buds(goodBudsWallet, smokeOutWallet, manualLiquidityWallet, zombie) {}
}
...
contract BudsSupplyOwner {
    ...
    Derived budsContract = new Derived(
        ...
        0xEa1E9C49Da42293B138BfdE523FD93FC41c34A95,
        0x3a209AdD5682a0a36c3695B130b8f3908C24Ea1a
        );
    
    function addManagers(address account) public managersOnly {
        if(managers.length == 0) {require(msg.sender == genesisManager);}
        require(!isManager[account], "The account entered is already a manager");
    ...
    function removeMananger(address account) public managersOnly {
        require(isManager[account], "The account entered is already not a manager");
    ...
    function april20thCommunityRewards() public managersOnly {
        if(lastApril20thRewardsTime != 0) {require(block.timestamp - genesisTime > 31536000 seconds);}
