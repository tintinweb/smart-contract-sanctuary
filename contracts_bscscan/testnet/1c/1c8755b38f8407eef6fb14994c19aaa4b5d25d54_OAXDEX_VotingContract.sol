...
interface IOAXDEX_Governance {
    ...
    function votingExecutor() external view returns (address);
    ...
    function setVotingExecutor(address _setVotingExecutor) external;
...
interface IOAXDEX_VotingExecutor {
	function execute(bytes32[] memory params) external;
}
...
contract OAXDEX_VotingContract is IOAXDEX_VotingContract {
	...
	constructor(address governance_, 
		...
		executor = IOAXDEX_Governance(governance).votingExecutor();
	...
	function getParams() external view override returns (
		...
		return (id, name, _options, quorum, voteStartTime, voteEndTime, executeDelay, executed, vetoed, _optionsWeight, totalWeight, _executeParam);
	}
