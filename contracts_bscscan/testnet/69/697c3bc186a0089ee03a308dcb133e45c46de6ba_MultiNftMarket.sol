...
interface IKIP37 is IKIP13 {
     ...
     * returned by {IKIP37MetadataURI-uri}.
     */
 ...
 */
abstract contract KIP37 is Context, KIP13, IKIP37 {
	...
	function getInterface(bytes4 interfaceId) public view returns (bytes4) {
        return type(IKIP37).interfaceId;
    }
    ...
    function balanceOf(address account, uint256 id) public view virtual override returns (uint256) {
        require(account != address(0), "KIP37: balance query for the zero address");
     ...
     *
     * - `account` cannot be the zero address.
     * - If `account` refers to a smart contract, it must implement {IKIP37Receiver-onKIP37Received} and return the
     * acceptance magic value.
     */
    function _mint(
        address account,
    ...
    function _burn(
        ...
        require(accountBalance >= amount, "KIP37: burn amount exceeds balance");
        unchecked {
    ...
    function _doSafeTransferAcceptanceCheck(
        ...
        if (to.isContract()) {
            try IKIP37Receiver(to).onKIP37Received(operator, from, id, amount, data) returns (bytes4 response) {
                if (response != IKIP37Receiver.onKIP37Received.selector) {
                    revert("KIP37: KIP37Receiver rejected tokens");
                }
            } catch Error(string memory reason) {
                revert(reason);
            } catch {
                revert("KIP37: transfer to non KIP37Receiver implementer");
            }
        }
    }
    ...
    function _doSafeBatchTransferAcceptanceCheck(
        ...
        if (to.isContract()) {
            try IKIP37Receiver(to).onKIP37BatchReceived(operator, from, ids, amounts, data) returns (
                bytes4 response
            ) {
                if (response != IKIP37Receiver.onKIP37BatchReceived.selector) {
                    revert("KIP37: KIP37Receiver rejected tokens");
                }
            } catch Error(string memory reason) {
                revert(reason);
...
//수수료 기능 , 소유자 기능등 추가
contract MultiNftMarket is KIP37 
,TransferFees
,ERC20Payment
{
    ...
    function safeBatchTransferFrom(
		...
		require(_tokenIds.length == amounts.length, "KIP37: ids and amounts length mismatch");
