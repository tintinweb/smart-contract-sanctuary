...
contract EarnOtherFixedAPR is Ownable, ReentrancyGuard {
  ...
  */
  ...
  function deposit(uint256 _amount) external nonReentrant {
    require(block.number >= startBlock, "EarnOther::deposit:: not allow before start period");
  ...
  function harvest() public {
    ...
    if (user.amount > 0 && block.number > claimableBlock) {
      ...
      require(_reward <= rewardToken.balanceOf(address(this)), "EarnOther::harvest:: insufficeint reward");
  ...
  function emergencyWithdraw() external nonReentrant {
    ...
    /// @dev before end distribution period, allow emergency when remaining reward less than user's claim balance only
    if (block.number < endBlock) {
      require(
        rewards > rewardToken.balanceOf(address(this)),
        "EarnOther::emergencyWithdraw:: not allow before end period"
      );
    }
