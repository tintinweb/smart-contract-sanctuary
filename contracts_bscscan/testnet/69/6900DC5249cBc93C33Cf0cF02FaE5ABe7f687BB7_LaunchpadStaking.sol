...
contract LaunchpadStaking is Adminable, IStakingLockable, Stakeable {
    ...
    function isLocked(address account) public view override returns (bool) {
        return address(levelManager) != address(0) && levelManager.isLocked(account);
    }
...
contract StakingTreasury is Ownable {
    ...
    function withdrawRewards(IERC20 _rewardToken, address recipient) external onlyOwner {
...
abstract contract Stakeable is Adminable {
    ...
    uint256 public rewardPerBlock = 1 * 1e9; // 1 token (TODO e9 é do trustpad)
