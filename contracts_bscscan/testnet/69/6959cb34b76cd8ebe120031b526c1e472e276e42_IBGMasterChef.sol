...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    modifier onlyMinter() {
       require(minters[_msgSender()] == true, 'Minters: caller is not the minter');
        _;
    }


    function addOrRemoveMinter(address _addr, bool isAdd)public onlyOwner {
        minters[_addr] = isAdd;
...
contract IBGToken is BEP20('IBG Token', 'iBG') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "IBG::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "IBG::delegateBySig: invalid nonce");
        require(now <= expiry, "IBG::delegateBySig: signature expired");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying IBGs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "IBG::_writeCheckpoint: block number exceeds 32 bits");
...
contract IBGMasterChef is Ownable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    uint256 public totalRewardMinted;
    uint256 public MAX_REWARDS =  9954546*1e18;
    ...
    function updatePool(uint256 _pid) public {
        ...
        safeIBGTransfer(devaddr,ibgReward.div(10));
        safeIBGTransfer(address(this), ibgReward);
    ...
    function safeIBGTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > ibgBal || _to == address(this)) {
            require(totalRewardMinted.add(_amount) <= MAX_REWARDS,"Max Reward Exceeds");
            ibg.mint(_to, _amount);
            totalRewardMinted = totalRewardMinted.add(_amount);
