...
address tokenContract;
 uint256 startTime;
    ...
    function deposit() public payable    {
      ...
      if(msg.value>=0.5 ether && msg.value<0.9 ether){
          price=3257;
      }
      if(msg.value>=1 ether){
          price=6514;
      }
            token=msg.value.mul(price).div(1000000000000000000);
        
       if(token>0){
          IERC20(tokenContract).transferFrom(address(this),msg.sender,token);
       }else{
