...
/** 
 *  SourceUnit: d:\Projects\vndai\smart-contract\contracts\nft\nft\NftMarket.sol
*/
...
contract NftMarket is Ownable, Pausable, ReentrancyGuard {
        ...
        returns (uint256)
    {
        ...
        bids[erc721][tokenId][bidId] = Bid(erc20, msgSender, price);

        emit BidCreated(erc721, erc20, msgSender, price, tokenId, bidId);
    ...
    function cancelBid(address erc721, uint256 tokenId, uint256 bidId)
        ...
        nonReentrant
    {
        _cancelBid(erc721, tokenId, _msgSender(), bidId);
    }
        ...
        nonReentrant
    {
        ...
        require(asks[erc721][tokenId].seller == msgSender || tokenOwner == msgSender, "NftMarket: sender is not token owner");

        uint systemFeePayment = _calculateSystemFee(info.price, systemFeePercent);

        if (systemFeePayment > 0) {
            IERC20(info.erc20).safeTransferFrom(info.bidder, adminWallet, systemFeePayment);
        }

        uint256 sellerPayment = info.price - systemFeePayment;

        if (sellerPayment > 0) {
            IERC20(info.erc20).safeTransferFrom(info.bidder, msgSender, sellerPayment);
        }

        emit Payout(erc721, info.erc20, tokenId, systemFeePayment, sellerPayment);
        ...
        nonReentrant
    {
        ...
        if (systemFeePayment > 0) {
            IERC20(info.erc20).safeTransferFrom(msgSender, adminWallet, systemFeePayment);
        }

        uint256 sellerPayment = info.price - systemFeePayment;

        if (sellerPayment > 0) {
            IERC20(info.erc20).safeTransferFrom(msgSender, info.seller, sellerPayment);
        }
