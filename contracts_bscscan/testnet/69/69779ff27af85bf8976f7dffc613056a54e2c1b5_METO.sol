...
//MetaOcean V1.2 - Get Ready for The Most Epic P2E game in the crypto space!
//Breed your Squibbles NFTs using $METO
//Fight, Stake, Trade them on our upcoming MetaMarket
//Get into MetaOcean Gem before it isn't too late!
//Website: meta-ocean.space
//Telegram: @metaocean
//Twitter: @meta_ocean
...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     *
     * Tokens usually opt for a value of 13, imitating the relationship between
...
contract METODividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("METO_Dividend_Tracker", "METO_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "METO_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "METO_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main METO contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "METO_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "METO: Cannot update claimWait to same value");
...
contract METO is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    METODividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 30000000 * (10**18);
    ...
    constructor() ERC20("MetaOcean", "METO") {
        BNBRewardsFee = 8;
        liquidityFee = 2;
        extraFeeOnSell = 0; 
        marketingFee = 5;
        marketingWallet = payable(0xd0C82EcB0eB0047109D3e1BF6fA6204f3dAEa171);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new METODividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "METO: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "METO: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "METO: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "METO: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "METO: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "METO: Cannot update gasForProcessing to same value");
