...
contract SousChef {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;   // How many SWIFSANDWICH tokens the user has provided.
        uint256 rewardDebt;  // Reward debt. See explanation below.
        uint256 rewardPending;
        //
        // We do some fancy math here. Basically, any point in time, the amount of SWIFSANDWICHs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accRewardPerShare) - user.rewardDebt + user.rewardPending
        //
        // Whenever a user deposits or withdraws SWIFSANDWICH tokens to a pool. Here's what happens:
    ...
    // The SWIFSANDWICH TOKEN!
    IBEP20 public swifsandwich;
    // rewards created per block.
    uint256 public rewardPerBlock;

    // Info.
    PoolInfo public poolInfo;
    // Info of each user that stakes SwifSandwich tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = swifsandwich.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 syrupSupply = swifsandwich.balanceOf(address(this));
        if (syrupSupply == 0) {
    ...
    // Deposit SwifSandwich tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        swifsandwich.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw SwifSandwich tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        swifsandwich.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        swifsandwich.safeTransfer(address(msg.sender), user.amount);
