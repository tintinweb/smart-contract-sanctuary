...
/**
 * Doge Pool is the irst ever token you will be rewarded in 2 decent coins $ETH and $ADA
 * 
 * 
 * 18% Tax
 * 10% in rewards (5% in $ADA, 5% in $ETH)
 * 3 % in LP building
 * 5 % in Marketing
 * 
 * http://t.me/DogePoolOfficial 
 * 0.2% max wallet (2,000,000,000)
 * 0.1% max buy (1,000,000,000)
 * 0.05% max sell (500,000,000)
*/
...
contract DogePool is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxBuyTranscationAmount = totalSupply().mul(1).div(1000);
    uint256 public maxSellTransactionAmount = totalSupply().mul(5).div(10000);
    uint256 public maxWalletBalance = totalSupply().mul(2).div(1000);
    ...
    uint256 public adaDividendRewardsFee = 5;
    uint256 public previousAdaDividendRewardsFee;
    uint256 public marketingFee = 5;
    uint256 public previousMarketingFee;
    uint256 public totalFees = ethDividendRewardsFee.add(marketingFee).add(adaDividendRewardsFee).add(liquidityFee);
    ...
    constructor() ERC20("Doge Pool", "DogePool") {
    	ethDividendTracker = new ETHDividendTracker();
    	adaDividendTracker = new ADADividendTracker();
 
    	marketingWallet = 0x528eeE52CaB44C0D932675455eA6C1Ea94702865;
    ...
    receive() external payable {
  	...
  	function whitelistDxSale(address _presaleAddress, address _routerAddress) external onlyOwner {
        ...
        adaDividendTracker.excludeFromDividends(_presaleAddress);
        excludeFromFees(_presaleAddress, true);
 
        ethDividendTracker.excludeFromDividends(_routerAddress);
        adaDividendTracker.excludeFromDividends(_routerAddress);
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "DogePool: The marketing wallet is already this address");
    ...
    function setAuthOnDividends(address account) public onlyOwner{
        ...
        adaDividendTracker.setAuth(account);
    }
    ...
    function setEthDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = ethDividendRewardsFee.add(marketingFee).add(adaDividendRewardsFee).add(liquidityFee);
            ethDividendEnabled = _enabled;
        }
    ...
    function setAdaDividendEnabled(bool _enabled) external onlyOwner {
        require(adaDividendEnabled != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
            ...
            totalFees = adaDividendRewardsFee.add(marketingFee).add(ethDividendRewardsFee).add(liquidityFee);
            adaDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(adaDividendRewardsFee).add(ethDividendRewardsFee).add(liquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateethDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "DogePool: The dividend tracker already has that address");
 
        ETHDividendTracker newethDividendTracker = ETHDividendTracker(payable(newAddress));
 
        require(newethDividendTracker.owner() == address(this), "DogePool: The new dividend tracker must be owned by the DogePool token contract");
    ...
    function updateadaDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(adaDividendTracker), "DogePool: The dividend tracker already has that address");
 
        ADADividendTracker newadaDividendTracker = ADADividendTracker(payable(newAddress));
 
        require(newadaDividendTracker.owner() == address(this), "DogePool: The new dividend tracker must be owned by the DogePool token contract");
    ...
    function updateAdaDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = adaDividendRewardsFee.add(ethDividendRewardsFee).add(marketingFee).add(liquidityFee);
    }
 
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        marketingFee = newFee;
        totalFees = marketingFee.add(ethDividendRewardsFee).add(adaDividendRewardsFee).add(liquidityFee);
    }
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "DogePool: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "DogePool: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DogePool: The PanadaSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "DogePool: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "DogePool: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "DogePool: Trading has not started yet");
        ...
        if (!swapping && canSwap && from != uniswapV2Pair) {
            ...
            if (adaDividendEnabled) {
                uint256 adaTokens = contractTokenBalance.div(totalFees).mul(adaDividendRewardsFee);
                swapAndSendAdaDividends(adaTokens);
            }
...
contract ETHDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DogePool_Eth_Dividend_Tracker", "DogePool_Eth_Dividend_Tracker", 0xBC537C876510083f7D2406C12fdd93cbe8Da4b8A) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "DogePool_Eth_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "DogePool_Eth_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DogePool contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DogePool_Eth_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DogePool_Eth_Dividend_Tracker: Cannot update claimWait to same value");
...
contract ADADividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterableMapping for IterableMapping.Map;
    ...
    constructor() DividendPayingToken("DogePool_Ada_Dividend_Tracker", "DogePool_Ada_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "DogePool_Ada_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "DogePool_Ada_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DogePool contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DogePool_Ada_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DogePool_Ada_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
