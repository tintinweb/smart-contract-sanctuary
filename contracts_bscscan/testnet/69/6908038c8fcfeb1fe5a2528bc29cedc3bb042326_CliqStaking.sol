...
/**
 * @title CliqStaking
 * @dev   Cliq Staking Contract for token staking
 */
contract CliqStaking {
  ...
  */
  ...
  // function 
  function setManager( 
  uint256 tokenStakingCount, 
  uint256 tokenTotalDays, 
  address tokenStakingAddress, 
  uint256 tokenStakingEndTime,
  uint256 tokenStakingStartTime,
  uint256 usertokens,
  bool transactionstatus) external onlyOwner returns(bool){
    ...
    _TokenTransactionstatus[_tokenStakingCount] = transactionstatus;
  ...
  function withdrawToken() external onlyOwner returns(bool){
      itoken.transfer(msg.sender,address(this).balance);
      return true;
  } 
