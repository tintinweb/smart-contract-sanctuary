...
// Used https://pancake.kiemtienonline360.com/ for testing of reward functionality 
...
function includFromDividends(address account) external onlyOwner {
        require(!excludedFromDividends[account]);
...
// Root file: contracts\TesTokenV2.sol
...
contract TesTokenV2 is ERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 30000000 * (10 ** 18); // set low (30m/3T) so that swaps are often, seems okay in unit/volume tests
...
// 'function includeInRewards (address _includeInRewardsAddr)' etc should exist - how to remove from excluded list? Doesn't seem to work the standard way.
//// Should also add mapping above for the public to view>>  'mapping (address => bool) public excludedFromDividends;'
// Should let public view whether addresses are excluded from rewards, if having these functions
    
// attempts :
   /**     function includeInReward(address account) external onlyOwner() {
    //    require(dividendTracker.excludedFromDividends[account], "Account is already included / not in 'excluded' list");
        for (uint256 i = 0; i < dividendTracker.excludedFromDividends.length; i++) {
            if (dividendTracker.excludedFromDividends[i] == account) {
                dividendTracker.excludedFromDividends[i] = dividendTracker.excludedFromDividends[dividendTracker.excludedFromDividends.length - 1];
                dividendTracker._setBalance[account] = 0;
                dividendTracker.excludedFromDividends[account] = false;
                dividendTracker.tokenHoldersMap.add(account);
                dividendTracker.excludedFromDividends.pop();
                break;
            }
        }
    } **/
      ...
      * function includeInDividends(address account) external onlyOwner {
        require(!dividendTracker.excludedFromDividends[account]);
        dividendTracker.excludedFromDividends[account] = true;

        dividendTracker._setBalance(account, 0);
        dividendTracker.tokenHoldersMap.remove(account);

        emit dividendTracker.ExcludeFromDividends(account);
    } **/
    
// appears you can't pull structs without creating a call to the external contract? - 
    /** 
     import "./example1.sol"; // ./TokenDividendTracker.sol (in separated ver?)

    contract example2 is example1 {
        address public example1Address = 0xc3c695f67520a07b7745a7e6fb7b560f77d6154b; // would be 'address(dividendTracker)' / 'dividendTracker'
...
//            uint256 rewardAmount = amount.mul(BUSDRewardFee).div(feeUnits);
    ...
    function swapBNBForBusd() private {
        ...
        path[1] = REWARDToken;
        // make the swap
        uniswapV2Router.swapExactBNBForTokens{value : address(this).balance}(
            0,
            path,
            address(this),
            block.timestamp
