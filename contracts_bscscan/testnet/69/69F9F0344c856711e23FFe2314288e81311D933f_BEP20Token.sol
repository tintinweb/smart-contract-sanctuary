...
library SafeMath {
    ...
    function div(uint x, uint y) internal pure returns (uint z) {
        require(y != 0, unicode"denominateur can't be equal zero");
        z = x / y;
    }
}
...
contract BEP20Token is Context, IERC20, Ownable {
  ...
  */
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
    ...
    _approve(sender, _msgSender(), SafeMath.sub(_allowances[sender][_msgSender()], amount));
    return true;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    _balances[recipient] = SafeMath.sub(_balances[recipient], amount);
  ...
  function _burnFrom(address account, uint256 amount) internal {
    _burn(account, amount);
    _approve(account, _msgSender(), SafeMath.sub(_allowances[account][_msgSender()], amount));
  }
}