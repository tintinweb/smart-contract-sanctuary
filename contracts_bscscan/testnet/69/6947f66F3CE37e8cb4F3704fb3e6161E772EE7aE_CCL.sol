 ...
 //  uint256 private BNBCost; // Fees in BNB
    ...
    function lastApplications(address _addr) external view returns (string memory, string memory, address, bool, address, string memory) {
        if(owner() == _msgSender()) {
            ...
            return (applicantName, applicantEmail, requestedRAddress, isContract, contractAddress, _Chain); } else { 
                revert("This function only for CCL");
            }
    }
    ...
    function _isSusAddress(address _addr) external view returns (bool) {
        ...
        bool showResults =  ltsHOLDINGS(caller);
        if(!showResults) {
            revert("Calling this function requires LTS Holdings");
        } else { 
            return(isSuspected[_addr]);
        }
    }
    
        // RETURNS SUSPECTED ADDRESSES/NEWLY DEPLOYED CONTRACTS
