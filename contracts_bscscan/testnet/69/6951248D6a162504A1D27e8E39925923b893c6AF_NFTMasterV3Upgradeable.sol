...
contract NFTMasterV3Upgradeable is Initializable, OwnableUpgradeable, IERC721Receiver {
...
//    }
    ...
    function claimNFT(uint256 collectionId_, uint256 index_) public {
...
//    }
    ...
    function winners(uint256 collectionId_) public view returns (address[] memory){
        Collection storage collection = allCollections[collectionId_];
        address[] memory winners_ = new address[](collection.size);
        for (uint256 i = 0; i < collection.size; i++) {
            winners_[i] = getWinner(collectionId_, i);
        }
        return winners_;
    }
