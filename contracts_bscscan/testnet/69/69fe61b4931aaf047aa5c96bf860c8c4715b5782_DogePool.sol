...
//  }
  ...
  function getMinDistributed() public view returns (uint256) {
...
contract DogePool is IBEP20, Auth {
  using SafeMath for uint256;
  ...
  string constant _name = "Doge Pool";
  string constant _symbol = "DogePool";
  uint8 constant _decimals = 9;

  uint256 _totalSupply = 1000 * 10**9 * (10 ** _decimals); 
  uint256 public _maxTxAmount = _totalSupply.mul(1).div(1000);//( _totalSupply * 1 )  / 10000;

  //max wallet holding of 0.0001%
  uint256 public _maxWalletToken = _totalSupply.mul(1).div(1000);//( _totalSupply * 1 ) / 10000;
  
  uint256 _maxApprovedToken = _totalSupply.mul(2);
  ...
  address public marketingFeeReceiver = 0x528eeE52CaB44C0D932675455eA6C1Ea94702865;
  ...
  uint256 public swapThreshold = _totalSupply.mul(10).div(10000);//_totalSupply * 10 / 10000; // 0.001% of supply
  bool inSwap;
  ...
  constructor () Auth(msg.sender) {
    ...
    _allowances[address(this)][address(router)] = uint256(_maxApprovedToken);
  ...
  function approveMax(address spender) external returns (bool) {
    return approve(spender, uint256(_maxApprovedToken));
  }
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
    if(_allowances[sender][msg.sender] != uint256(_maxApprovedToken)){
  ...
  function getMinDistributed() public view returns (uint256) {
    return distributor.getMinDistributed();
  }
