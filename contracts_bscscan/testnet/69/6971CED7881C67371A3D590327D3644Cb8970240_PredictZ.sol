...
contract PredictZ is Ownable {
    ...
    event CloseMatch(address indexed setter,uint256 matchId, Result result);
    event Bet(address indexed better, uint256 matchId, uint256 teamId);
    ...
    constructor (address token_, address treasuryAddress_) {
        token = IERC20(token_);
    ...
    function createMatch(uint256 leagueId,uint256 teamAId,uint256 teamBId,uint256 endTime) external onlyOwner {
    ...
    function editMatch(uint256 matchId,uint256 endTime) external onlyOwner {
        require(endTime!=0,"PredictZ: invalid endTime");
    ...
    function closeMatch(uint256 matchId,Result result, uint256 scoreTeamA, uint256 scoreTeamB) external onlyOwner {
        require(matches[matchId].status==Status.Pending,"PredictZ: status must pending");
        require(result!=Result.Pending,"PredictZ: result must not equal pending");
        ...
        if (result!=Result.Cancelled) {
            ...
            IERC20(token).safeTransfer(treasuryAddress,(totalBet*20)/100);
        }
        
        emit CloseMatch(msg.sender,matchId,result);
    }
    
    function bet(uint256 matchId, uint256 teamId,uint256 amount) external {
        require(amount>0,"PredictZ: min amount");
        require(matches[matchId].endTime>block.timestamp,"PredictZ: time's up");
        require(matches[matchId].status==Status.Pending,"PredictZ: status must pending");
        userBets[matchId][msg.sender][teamId] += amount;
        IERC20(token).safeTransferFrom(msg.sender, address(this), amount);

        if (teamId==0) {
            matches[matchId].accDrawBets = matches[matchId].accDrawBets + amount;
        } else if (teamId==matches[matchId].teamAId) {
            matches[matchId].accTeamABets = matches[matchId].accTeamABets + amount;
        } else if (teamId==matches[matchId].teamBId) {
            matches[matchId].accTeamBBets = matches[matchId].accTeamBBets + amount;
        } else {
            revert("PredictZ: invalid team");
        }

        emit Bet(msg.sender,matchId,teamId);
    }
    ...
    function viewPrize(uint256 matchId,address user) public view returns(uint256) {
        ...
        if (matches[matchId].result==Result.Draw) {
            amount = (((totalBet * 8) / 10) * userBets[matchId][user][0])/(matches[matchId].accDrawBets);

        } else if (matches[matchId].result==Result.TeamAWins) {
            amount = (((totalBet * 8) / 10) * userBets[matchId][user][matches[matchId].teamAId])/(matches[matchId].accTeamABets);

        } else if (matches[matchId].result==Result.TeamBWins) {
            amount = (((totalBet * 8) / 10) * userBets[matchId][user][matches[matchId].teamBId])/(matches[matchId].accTeamBBets);
