...
contract ADACatto is ERC20, Ownable {
    ...
    constructor() ERC20("ADACatto", "CATTO") {
    	firstDividendTracker = new FIRSTDividendTracker();

    	marketingWallet = 0xBFD36C53835C252872a92D0b8a8717Ac45994ef7;
      //Testnet BUSD
    ...
    function updatefirstDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(firstDividendTracker), "ADACatto: The dividend tracker already has that address");

        FIRSTDividendTracker newfirstDividendTracker = FIRSTDividendTracker(payable(newAddress));

        require(newfirstDividendTracker.owner() == address(this), "ADACatto: The new dividend tracker must be owned by the ADACatto token contract");
