...
contract MinoWarNFT is IMinoCore, ERC721, Ownable {
    ...
    mapping(uint256 => Mino) public minoFactory;
    ...
    event AddMinoUser(uint256 indexed tokenId);
    ...
    function changeLevel(
        ...
        Mino memory mino = minoFactory[_tokenId];
        mino.level = _level;
        User storage userInfo = users[_owner];
        for (uint256 index = 0; index < userInfo.minos.length; index++) {
            if(userInfo.minos[index].tokenId == _tokenId) {
                userInfo.minos[index].level = _level;
            }
        } 
    ...
    function changeClass(
        ...
        for (uint256 index = 0; index < userInfo.minos.length; index++) {
            if(userInfo.minos[index].tokenId == _tokenId) {
                userInfo.minos[index].class = _class;
            }
        } 
    ...
    function changeSkill(
        ...
        for (uint256 index = 0; index < userInfo.minos.length; index++) {
            if(userInfo.minos[index].tokenId == _tokenId) {
                userInfo.minos[index].skill = _skill;
            }
        } 

        emit UpdateSkill(_tokenId, _skill);
    }
    ...
    function changeCharacter(
        ...
        Mino memory mino = minoFactory[_tokenId];
        mino.character = _character;
        User storage userInfo = users[_owner];
        for (uint256 index = 0; index < userInfo.minos.length; index++) {
            if(userInfo.minos[index].tokenId == _tokenId) {
                userInfo.minos[index].character = _character;
            }
        } 

        emit UpdateCharacter(_tokenId, _character);
    }
    ...
    function changeStar(
        ...
        for (uint256 index = 0; index < userInfo.minos.length; index++) {
            if(userInfo.minos[index].tokenId == _tokenId) {
                userInfo.minos[index].star = _star;
            }
        } 

        emit UpdateStar(_tokenId, _star);
    }
    ...
    function setNFTForUser(
        ...
        user.minos.push(_mino);
        emit AddMinoUser(_tokenId);
    }
    ...
    function getAllNFT(uint256 _fromTokenId, uint256 _toTokenId)
        ...
        returns (Mino[] memory)
    {
        Mino[] memory allMino = new Mino[](totalSupply());
        uint256 count = 0;
        for (uint256 index = _fromTokenId; index <= _toTokenId; index++) {
            allMino[count] = minoFactory[index];
            ++count;
        }
        return allMino;
    }
    ...
    function _transfer(address from, address to, uint256 tokenId) internal override(ERC721) {
        ...
        for (uint256 index = 0; index < userFrom.minos.length; index++) {
            if(userFrom.minos[index].tokenId == tokenId) {
                delete userFrom.minos[index];
            }
        }
        Mino memory mino = minoFactory[tokenId];
        User storage userTo = users[from];
        userTo.minos.push(mino);
    }
}
