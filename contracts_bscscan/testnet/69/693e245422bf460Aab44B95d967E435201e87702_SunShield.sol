...
abstract contract SunShieldBase is
    Context,
    IERC20Metadata,
    Ownable,
    ReentrancyGuard
{
    ...
    address private _marketingWallet =
        0xD746931F94286b5bFdB2d135C73f0E085c62109C;
    address private _moonshieldAddress =
        0xB63191dB354231e097b6dd3Fd5C0057E41ac4A5F;
    ...
    constructor(address routerAddress) {
        ...
        // 1% liquidity fee, 12% reward fee, 3% buyback fee, 2% marketing fee, 1% moonshield fee, 6% additional sell fee
        setFees(1, 12, 3, 2, 1, 6);
    ...
    function executeSwap(uint256 amount) private {
        ...
        uint256 bnbToBeSendToMoonshield = (bnbSwapped *
            tokensReservedForMoonshield) / tokensToSwap;
    ...
    function setFees(
        ...
        uint8 marketingFee,
        uint8 moonshieldFee,
        uint8 additionalSellFee
    ) public onlyOwner {
        require(
            liquidityFee >= 1 && liquidityFee <= 8,
            "Liquidity fee must be between 1% and 8%"
        );
        require(
            rewardFee >= 1 && rewardFee <= 15,
            "Reward fee must be between 1% and 15%"
        );
        require(
            buybackFee >= 1 && buybackFee <= 5,
            "Buyback fee must be between 1% and 5%"
        );
        require(
            marketingFee >= 1 && marketingFee <= 5,
            "Marketing fee must be between 1% and 5%"
        );
        require(
            moonshieldFee >= 1 && moonshieldFee <= 5,
            "Moonshield fee must be between 1% and 5%"
        );
        require(
            additionalSellFee <= 10,
            "Additional sell fee cannot exceed 5%"
        );
        require(
            liquidityFee + rewardFee + additionalSellFee <= 25,
            "Total fees cannot exceed 25%"
        );
...
contract SunShield is SunShieldBase {
    ...
    function claimReward() external isHuman nonReentrant {
        claimReward(msg.sender);
    }

    function claimReward(address user) public {
        require(
            msg.sender == user || isClaimApproved(user, msg.sender),
        ...
        require(
            ...
            "Address is excluded from rewards, make sure there is enough SSHLD balance"
        );
    ...
    function claimSSHLD(address user, uint256 bnbAmount)
        private
        returns (bool)
    {
        if (bnbAmount == 0) {
    ...
    function bnbRewardClaimedAsSSHLD(address byAddress)
        public
        view
        returns (uint256)
    {
