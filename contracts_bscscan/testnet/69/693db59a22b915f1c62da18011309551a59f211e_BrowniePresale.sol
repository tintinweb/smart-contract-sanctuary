...
contract BrowniePresale is ReentrancyGuard {
    ...
    function buy (uint256 _amount, address beneficiary) public nonReentrant {
        ...
        require (_amount >= 20 ether, "Amount is lower than min value");
        //Maximum presale purchase per transaction
        require (_amount <= 5000 ether, "Amount is greater than max value");
        //Presale HardCap
        require (busdReceived +  _amount <= 1000000 ether, "Presale hardcap reached");
        //Max presale purchase per wallet
        require (tokensOwned[_buyer] <= 5000 ether, "Max limit per wallet hit!");
