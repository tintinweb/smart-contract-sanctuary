...
/// @notice maintains roles, access control, lp, genesisGroup, and the treasury
contract Core is ICore, Permissions, Initializable {

    
    /// @notice the address of the LP contract
    IERC20 public override lp;

    function init(address _lp) external override initializer {
    ...
    /// @notice sets Lp address to a new address
    /// @param token new Lp address
    function setLp(address token) external override onlyGovernor {
        _setLp(token);
    }

    /// @notice sends Lp tokens from treasury to an address
    /// @param to the address to send LP to
    /// @param amount the amount of LP to send
    function allocateLp(address to, uint256 amount)
        external
        override
        onlyGovernor
    {
        IERC20 _lp = lp;
        require(
            _lp.balanceOf(address(this)) >= amount,
            "Core: Not enough Lp"
        );

        _lp.transfer(to, amount);

        emit LpAllocation(to, amount);
    }


    function _setLp(address token) internal {
        lp = IERC20(token);
        emit LpUpdate(token);
    }
}
