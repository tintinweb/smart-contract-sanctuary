...
contract SweetKeeper is Ownable, KeeperCompatibleInterface {
    ...
    modifier onlyKeeperAndModerator() {
        require(msg.sender == keeper || msg.sender == moderator, "SweetKeeper::onlyKeeperAndModerator: Not keeper or moderator");
        _;
    }
    ...
    function checkUpkeep(
        ...
        for (uint16 index = 0; index < sweetVaults.length; ++index) {
            ...
            (uint platformOutput, uint keeperOutput, uint burnOutput, uint fieldOutput) = _getExpectedOutputs(vault);
            ...
            if (
                ...
                tempCompoundInfo.minFieldOutputs[sweetVaultsLength] = fieldOutput.mul(slippageFactor).div(10000);
        ...
        if (legacyVaultsLength > 0 || sweetVaultsLength > 0) {
            ...
            for (uint16 index = 0; index < sweetVaultsLength; ++index) {
                ...
                compoundInfo.minFieldOutputs[index] = tempCompoundInfo.minFieldOutputs[index];
            }
    ...
    function performUpkeep(
        bytes calldata performData
    ) external override onlyKeeperAndModerator {
        (
    ...
    function _getExpectedOutputs(
        ...
        try ISweetVault(_vault).getExpectedOutputs() returns (
            ...
            return (platformOutput, keeperOutput, burnOutput, fieldOutput);
        }
        catch (bytes memory) {
        }
