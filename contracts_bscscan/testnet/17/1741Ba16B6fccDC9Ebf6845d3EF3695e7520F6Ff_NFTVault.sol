 ...
 */
contract NFTVault is  AccessControl,Ownable,IERC721Receiver{


    bytes32 public constant USER_ROLE = keccak256("USER_ROLE");
    bytes32 public constant MANAGER_ROLE = keccak256("MANAFER_ROLE");
    ...
    function _setMarketStartTimestamp(uint256 startTimestamp) external{
        MARKET_START_TIMESTAMP = startTimestamp;
    }

    function _setMarketEndTimestamp(uint256 endTimestamp) external{
        MARKET_END_TIMESTAMP = endTimestamp;
    }

    function _setWLStartTimestamp(uint256 startTimestamp) external{
        WHITELIST_START_TIMESTAMP = startTimestamp;
    }
    
    function _setWLEndTimestamp(uint256 endTimestamp) external{
    ...
    /// --------- BASIC USER ACTION -------
    /// -----------------------------------
    function batchApproveForNFT(uint256 [] calldata tokenIds) external {
    ...
    function depositeNFT(address _token,uint256 _tokenId) public onlyOwner{
    ...
    function batchDepositeNFT(address _token,uint256[] calldata _tokenIds) public onlyOwner{
        require(_token==base_nft,"must deposite nft with right type ");
        ...
        for (uint256 i=0;i<length;i++){
            ...
            supply++;
            token_id_queue.push(_tokenIds[tid]);
        }
    ...
    function buyOne(address account)  public onlyRole(USER_ROLE){
        require(IERC20(pay_coin).balanceOf(account)>=pay_amount,"account balance not enough");
        IERC20(pay_coin).transferFrom(account, address(this), pay_amount);
    ...
    function registerWhiteList() public {
        require(white_list_status,"white list not open");
