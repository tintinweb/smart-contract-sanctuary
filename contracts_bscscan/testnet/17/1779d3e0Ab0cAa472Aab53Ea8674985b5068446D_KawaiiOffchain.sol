...
contract KawaiiOffchain is Signed, Pausable, SignData {
    using SafeMath for uint256;
    ...
    function convertOffchain(bytes memory data) external whenNotPaused {
        (IERC1155 _nft1155Address,
        uint256[] memory _tokenBurnIds,
        uint256[] memory _amountBurn,
    ...
    address signer = getSigner(
            keccak256(
                abi.encode(address(this), this.convertOffchain.selector, _tokenBurnIds, _amountBurn, _tokenIds, _amount, _timestamp, nonce)
            ), v, r, s);
        require(isSigner[signer], "Forbidden");
        require(_tokenBurnIds.length == _amountBurn.length, "token id burn must same length with amount");
        require(_tokenIds.length == _amount.length, "token id must same length with amount");

        for (uint256 i = 0; i < _tokenBurnIds.length; i++) {
            IERC1155(_nft1155Address).burn(_sender, _tokenBurnIds[i], _amountBurn[i]);
        }

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            IERC1155(_nft1155Address).mint(_sender, _tokenIds[i], _amount[i]);
        }
