...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    receive () external payable {
        if(endICO > 0 && block.timestamp < endICO  && block.timestamp >= startDate){
            buyTokens(_msgSender());
        }
        else if(block.timestamp >= endICO){
            revert('Pre-Sale is closed');
        }
        else
        {
            revert('The presale has not started yet.');
        }
    }
    
    
    //Start Pre-Sale
    function startICO(uint _startDate, uint endDate, uint _minPurchase, uint _maxPurchase, uint _softCap, uint _hardCap) external onlyOwner icoNotActive() {
    ...
    function setStartRefund(bool value) external onlyOwner{
        startRefund = value;
    }
