...
// File: profile-nft-gamification/contracts/crossChainProfile.sol

pragma solidity ^0.6.0;

/** @title CrossChainProfile.
 * @notice It is a contract for users to bind their address
 * to a customizable profile by depositing a NFT.
 */
contract CrossChainProfile is AccessControl, ERC721Holder {
    ...
    uint256 public numberPYEToReactivate;
    uint256 public numberPYEToRegister;
    uint256 public numberPYEToUpdate;
    ...
    constructor(
        IBEP20 _PYEToken,
        uint256 _numberPYEToReactivate,
        uint256 _numberPYEToRegister,
        uint256 _numberPYEToUpdate
    ) public {
        PYEToken = _PYEToken;
        numberPYEToReactivate = _numberPYEToReactivate;
        numberPYEToRegister = _numberPYEToRegister;
        numberPYEToUpdate = _numberPYEToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends PYE to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        PYEToken.safeTransferFrom(_msgSender(), address(this), numberPYEToRegister);
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer PYE token to this address
        PYEToken.safeTransferFrom(_msgSender(), address(this), numberPYEToUpdate);
    ...
    function reactivateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        PYEToken.safeTransferFrom(_msgSender(), address(this), numberPYEToReactivate);
    ...
    /**
     * @dev Claim PYE to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        PYEToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of PYE to register
     * Callable only by owner admins.
     */
    function updateNumberPYE(
        uint256 _newNumberPYEToReactivate,
        uint256 _newNumberPYEToRegister,
        uint256 _newNumberPYEToUpdate
    ) external onlyOwner {
        numberPYEToReactivate = _newNumberPYEToReactivate;
        numberPYEToRegister = _newNumberPYEToRegister;
        numberPYEToUpdate = _newNumberPYEToUpdate;
    }
 ...
 * other CrossChainProfile requirements.
 */
contract IFOV2 is IIFOV2, ReentrancyGuard, Ownable {
    ...
    // CrossChainProfile
    CrossChainProfile public crossChainProfile;
     ...
     * @param _crossChainProfileAddress: the address of the crossChainProfile
    ...
    constructor(
        ...
        address _crossChainProfileAddress,
        uint256 _startBlock,
        uint256 _endBlock,
        ...
        crossChainProfile = CrossChainProfile(_crossChainProfileAddress);
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        require(crossChainProfile.getUserStatus(msg.sender), "Must have an active profile");
    ...
    function _claimPoints(address _user) internal {
        if (!_hasClaimedPoints[_user]) {
            ...
            if (sumPools > thresholdPoints) {
                ...
                crossChainProfile.increaseUserPoints(msg.sender, numberPoints, campaignId);
            }
        }
    }
