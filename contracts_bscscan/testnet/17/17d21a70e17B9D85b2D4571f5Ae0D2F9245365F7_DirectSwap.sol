...
contract DirectSwap is Ownable  {
    ...
    function _swapDirect(address _from, uint amount, address _to) internal returns (uint){
        ...
        uint[] memory amounts = ROUTER.swapExactTokensForTokens(amount, 0, path, address(this), block.timestamp + 100);
    ...
    function swapDirect(address _from,address _to, uint256 _amount, uint256 _minOutAmount) public {
        IERC20(_from).safeTransferFrom(msg.sender, address(this), _amount);
        
        uint256 swapAmount = _swapDirect(_from, _amount, _to);
        require(swapAmount >= _minOutAmount, string(abi.encodePacked("slippage exceeds, can only receive ", uint2str(swapAmount))));
