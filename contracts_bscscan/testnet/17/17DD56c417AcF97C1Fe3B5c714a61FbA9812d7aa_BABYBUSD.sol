...
interface IDividendPayingToken {
  ...
  /// @notice Distributes busd to token holders as dividends.
  /// @dev SHOULD distribute the paid busd to token holders as dividends.
  ///  SHOULD NOT directly transfer busd to token holders in this function.
  ///  MUST emit a `DividendsDistributed` event when the amount of distributed busd is greater than 0.
  function distributeDividends() external payable;

  /// @notice Withdraws the busd distributed to the sender.
  /// @dev SHOULD transfer `dividendOf(msg.sender)` wei to `msg.sender`, and `dividendOf(msg.sender)` SHOULD be 0 after the transfer.
  ///  MUST emit a `DividendWithdrawn` event if the amount of busd transferred is greater than 0.
  function withdrawDividend() external;

  /// @dev This event MUST emit when busd is distributed to token holders.
  /// @param from The address which sends busd to this contract.
  /// @param weiAmount The amount of distributed busd in wei.
  ...
  /// @param to The address which withdraws busd from this contract.
  /// @param weiAmount The amount of withdrawn busd in wei.
...
@author Roger Wu (https://github.com/roger-wu)
@dev A mintable ERC20 token that allows anyone to pay and distribute busd
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  // With `magnitude`, we can properly distribute dividends even if the amount of received busd is small.
  ...
  /// About undistributed busd:
  ///   In each distribution, there is a small amount of busd not distributed,
  ///     the magnified amount of which is
  ///     `(msg.value * magnitude) % totalSupply()`.
  ///   With a well-chosen `magnitude`, the amount of undistributed busd
  ///     (de-magnified) in a distribution can be less than 1 wei.
  ///   We can actually keep track of the undistributed busd in a distribution
  ///     and try to distribute it in the next distribution,
  ///     but keeping track of such data on-chain costs much more than
  ///     the saved busd, so we don't do that.
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0x158CDc1320f1D5E9758faB2BacB90dFE305A972A, "Only owner can change dividend contract address");
...
contract BABYBUSD is ERC20, Ownable {
    ...
    constructor() ERC20("BABYBUSD", "BBBUSD") {
    	busdDividendTracker = new BusdDividendTracker();

    	marketingWallet = 0x021087FF9b8E7Eab771E6d719E01f96617Fa93aA;
    	teamWallet = 0x7caBD0765110913a294CEd8e8c86Dfe9e18486C9;
    	busdDividendToken = 0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47;
    ...
    receive() external payable {
  	...
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "BBBUSD: The team wallet is already this address");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "BBBUSD: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function updateBusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "BBBUSD: The dividend tracker already has that address");

        BusdDividendTracker newBusdDividendTracker = BusdDividendTracker(payable(newAddress));

        require(newBusdDividendTracker.owner() == address(this), "BBBUSD: The new dividend tracker must be owned by the BBBUSD token contract");
    ...
    function updateBusdDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "BBBUSD: Fee must be less than 6%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "BBBUSD: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "BBBUSD: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BBBUSD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "BBBUSD: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "BBBUSD: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "BBBUSD: Trading has not started yet");
...
contract BusdDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BBBUSD_Busd_Dividend_Tracker", "BBBUSD_Busd_Dividend_Tracker", 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BBBUSD_Busd_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BBBUSD_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BBBUSD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BBBUSD_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BBBUSD_Busd_Dividend_Tracker: Cannot update claimWait to same value");
