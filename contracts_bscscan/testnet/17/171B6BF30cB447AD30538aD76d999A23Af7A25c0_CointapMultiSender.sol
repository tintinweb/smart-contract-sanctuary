...
contract CointapMultiSender is Context, Ownable, SafeTransfer {
    ...
    function _estFeeTransferBulk(uint256 noOfTxs, address token) internal view returns (uint256) {
        ...
        if(noOfTxs <= 5) {
            return SERVICE_BASE_COST * MULTIPLIER_TRIAL;
            // return _feeAfterDiscount(totalFee, token);
        }
        if(noOfTxs <= 50) {
            return SERVICE_BASE_COST * MULTIPLIER_PREMEUM;
            // return _feeAfterDiscount(totalFee, token);
        }
        return (SERVICE_BASE_COST * MULTIPLIER_PREMEUM) + (COST_PER_TX_DIAMOND * (noOfTxs - 50));
        // return _feeAfterDiscount(totalFee, token);
    }
