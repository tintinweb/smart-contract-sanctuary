...
contract SherlockDoge is Context, IBEP20, Ownable {
    ...
    uint256 public numTokensSellToAddToLiquidity = 100 * 10**15 * 10**9;
        ...
        )
    {
        ...
        uint256 tLiquidity = calculateLiquidityFee(tAmount, heldLessThan24Hours, isSecondHolder);
    ...
    function calculateLiquidityFee(uint256 _amount, bool _heldLessThan24Hours, bool _isSecondHolder)
        private
        view
        returns (uint256)
    {
        if (_heldLessThan24Hours) {
            if (_isSecondHolder) {
                return _amount.mul(_liquidityFee + 20).div(10**2);
            } else {
                return _amount.mul(_liquidityFee + 3).div(10**2);
            }
        } else {
    ...
    function calculateBurnFee(uint256 _amount, bool _heldLessThan24Hours, bool _isSecondHolder)
        private
        view
        returns (uint256)
    {
    ...
    function calculateCommunityFee(uint256 _amount, bool _heldLessThan24Hours, bool _isSecondHolder)
        private
        view
        returns (uint256)
    {
    ...
    function calculateMarketingFee(uint256 _amount, bool _heldLessThan24Hours, bool _isSecondHolder)
        private
        view
        returns (uint256)
    {
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            bool secondHolder = _secondHolder[_msgSender()];

            uint256 toMarketingWallet = calculateMarketingFee(amount, heldLessThan24Hours, secondHolder);
            uint256 toCommunityWallet = calculateCommunityFee(
                amount,
                heldLessThan24Hours,
                secondHolder
            );
            uint256 toLiquidity = calculateLiquidityFee(amount, heldLessThan24Hours, secondHolder);
            uint256 toBurn = calculateBurnFee(amount, heldLessThan24Hours, secondHolder);
