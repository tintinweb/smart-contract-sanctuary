...
contract Zap is IZap, OwnableUpgradeable {
    ...
    uint public constant SWAP_FEE_PERCENT = 20; // 0.2 %
    ...
    */
     ...
     */
    function initialize(address _factory, address _router, address _wbnb, address ali, address usdt, address busd, address eth, address btcb , address baby, address raca, address beta) public initializer {
        __Ownable_init();
    ...
    function calculatePriceImpact(address fromToken, uint amountIn, address toToken) private view returns (uint priceImpact) {
        address[] memory path = routePath(fromToken, toToken);

        uint midPrice = MULTIPLIER;

        for (uint i = 0; i < path.length - 1; i++) {
            (uint reserveA, uint reserveB) = AlitaLibrary.getReserves(factory, path[i], path[i + 1]);
        ...
        uint[] memory amountOuts = AlitaLibrary.getAmountsOut(factory, amountIn, path);
    ...
    function getPriceImpactWhenZapIn(address fromToken, uint amountIn, address toToken) public view returns (uint[] memory priceImpacts) {
        priceImpacts = new uint[](3);
        ...
        if (isLpToken(toToken)) {
            ...
            if (fromToken == token0 || fromToken == token1) {
                ...
                uint wbnbAmount = fromToken != wbnb ? getPoolAddedAmount(fromToken, wbnb, amountIn) : amountIn;
    ...
    function getPriceImpactWhenZapOut(address fromToken, uint amountIn) public view returns (uint priceImpact) {
