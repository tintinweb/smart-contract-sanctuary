...
contract ClosedSeaNFTController is ERC721Holder, Ownable, Pausable {
    ...
    IWETH public WETH;
    address quoteTokenAndETHPairAddress;
    ...
    function buyToken(address _tokenAddr, uint256 _tokenId) public whenNotPaused {
        buyTokenTo(_tokenAddr, _tokenId, _msgSender());
    }

    function buyTokenWithETH(address _tokenAddr, uint256 _tokenId) public payable whenNotPaused {
    ...
    function buyTokenTo(address _tokenAddr, uint256 _tokenId, address _to) public whenNotPaused {
        require(_msgSender() != address(0) && _msgSender() != address(this), 'Wrong msg sender');
        bytes32 key = _getKey(_tokenAddr, _tokenId);
        require(!_userBids[_msgSender()].contains(key), 'You must cancel your bid first');

        (uint256 price, , ) = _asksMap.get(key);
        uint256 priceInQuoteToken = price.mul(getQuoteTokenPriceInETH()).div(1e36);
        quoteErc20.safeTransferFrom(_msgSender(), _tokenSellers[key], priceInQuoteToken);

        IERC721(_tokenAddr).safeTransferFrom(_tokenSellers[key], _to, _tokenId);
        _asksMap.remove(key);
        _userSellingTokens[_tokenSellers[key]].remove(key);
        emit Trade(_tokenSellers[key], _to, _tokenId, price, 0);
        delete _tokenSellers[key];
    }
    ...
    function readyToSellToken(address _tokenAddr, uint256 _tokenId, uint256 _price) public whenNotPaused {
        readyToSellTokenTo(_tokenAddr, _tokenId, _price, address(_msgSender()));
    }
    ...
    function bidToken(address _tokenAddr, uint256 _tokenId, uint256 _price, bool _withEther) public payable whenNotPaused {
        ...
        if (_withEther) {
            require(msg.value >= _price.add(_price.mul(feePercent).div(100)), '!price');
        } else {
    ...
    function updateBidPrice(address _tokenAddr, uint256 _tokenId, uint256 _price) public payable whenNotPaused {
        ...
        if (_price > bidEntry.price) {
            if (bidEntry.withEther) {
                require(msg.value >= _price.sub(bidEntry.price).add(_price.sub(bidEntry.price).mul(feePercent).div(100)), '!price');
            } else {
    ...
    function sellTokenTo(address _tokenAddr, uint256 _tokenId, address _to) public whenNotPaused {
        ...
        if (bidEntry.withEther) {
            ...
            feeAmount = price.mul(feePercent).div(100);
            if (feeAmount != 0) {
    ...
    function getQuoteTokenPriceInETH() public view returns (uint) {
        if (quoteTokenAndETHPairAddress != address(0)) {
            uint pairBalInWETH = WETH.balanceOf(quoteTokenAndETHPairAddress);
            uint pairBalInQuote = quoteErc20.balanceOf(quoteTokenAndETHPairAddress);
            return pairBalInWETH.mul(1e18).div(pairBalInQuote);
        }
        return 0;
    }

    function updatePair(address _factory) external onlyOwner {
        address _QuoteTokenAndETHPair = IUniswapV2Factory(_factory).getPair(address(WETH), address(quoteErc20));
        quoteTokenAndETHPairAddress = _QuoteTokenAndETHPair;
    }
}