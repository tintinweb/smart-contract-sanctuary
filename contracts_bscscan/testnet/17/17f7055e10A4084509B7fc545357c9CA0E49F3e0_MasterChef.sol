...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;  
     ...
     // Loyalty pool address
    address public LOYALTY_POOL;  
    ...
    event Claim(address indexed user, uint256 indexed pid, uint256 pendingAmount, uint256 penaltyAmount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event SetFeeAddress(address indexed user, address indexed newAddress);
    event SetDevAddress(address indexed user, address indexed newAddress);
    event SetTreasuryAddress(address indexed user, address indexed newAddress);
    event SetLoyaltyAddress(address indexed user, address indexed newAddress);
    event UpdateEmissionRate(address indexed user, uint256 rewardPerBlock);
    event UpdateLockTime(address indexed user, uint256 locktime);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 DFLReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        DFL.transfer(devAddress, DFLReward.mul(10).div(100));
		DFL.transfer(treasuryAddress, DFLReward.mul(10).div(100));
        // DFL.transfer(address(this), DFLReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        user.unlockTimestamp = block.timestamp.add(lockDuration);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accRewardPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                user.totalPending = user.totalPending.add(pending);
    ...
    //User claim pending rewards
    function claim(uint256 _pid) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        uint256 pending = user.amount.mul(pool.accRewardPerShare).div(1e18).sub(user.rewardDebt);
        uint256 payout = user.totalPending.add(pending);
        uint256 penalty = 0;
        if(block.timestamp < user.unlockTimestamp){
            penalty = payout.sub(payout.div(2));
            payout = payout.div(2);
        }
        if (payout > 0 || penalty > 0) {
            safeTokenTransfer(LOYALTY_POOL, penalty);
            safeTokenTransfer(msg.sender, payout);
        }
        user.unlockTimestamp = block.timestamp.add(lockDuration);
        user.totalPending = 0;
        
        emit Claim(msg.sender, _pid, user.totalPending, penalty);
    }
    ...
    // Withdraw DFL incase of emergency.
    function emergencyRemoval(uint256 _amount) external nonReentrant onlyOwner {
    ...
    function setLoyaltyAddress(address _loyaltyAddress) external onlyOwner {
        require(_loyaltyAddress != address(0),"non-zero");
        LOYALTY_POOL = _loyaltyAddress;
        emit SetLoyaltyAddress(msg.sender, _loyaltyAddress);
    }
    ...
    function updateLocktime(uint256 _lockTime) external onlyOwner {
        ...
        emit UpdateLockTime(msg.sender, _lockTime);
    }
