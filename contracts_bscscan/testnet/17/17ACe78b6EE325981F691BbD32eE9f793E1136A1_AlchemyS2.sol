...
contract AlchemyS2 is ERC721Holder, Ownable, Divestor, RandomGenerator {
    ...
    uint public totalRepoU; 
    uint public totalPoundage;   
    ...
    function setRate(uint rate_, uint premium_) external onlyOwner {
        meta.rate = rate_;
        meta.premium = premium_;
    }
    ...
    function repurchase(uint tokenId_) public returns(uint) {
        ...
        uint poundage = PoolInfoes[poolId].price / 100 * meta.rate;
