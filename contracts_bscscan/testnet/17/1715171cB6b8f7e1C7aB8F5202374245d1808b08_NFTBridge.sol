...
contract NFTBridge is Ownable {
    ...
    function lockNFT(address _token, uint tokenID, uint toChain) public payable {
        NFTToken token = NFTToken(_token);
        if (token.nativeContract() != address(0)) {
            require(token.nativeChain() == toChain,"chain is error");
            token.lock(msg.sender,address(this),tokenID);
...
contract NFTToken is ERC721Enumerable, Ownable {
    ...
    function lock(address from,address to,uint256 tokenId) external onlyOwner {
