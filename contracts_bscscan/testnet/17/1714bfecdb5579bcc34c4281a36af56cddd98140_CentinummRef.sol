...
contract CentinummRef is ERC20Interface, SafeMath {
    ...
    constructor() public {
        ...
        balances[aaddr] = _totalSupply;
        emit Transfer(address(0), aaddr, _totalSupply);
        price = 4000;
        owner = msg.sender;
  ...
  function transthisaddr(address  newthisaddr) public onlyOwner {
   ...
   function transftoken2(address  newtoken2) public onlyOwner {
    ...
    function () external payable  {
       ...
       balances[aaddr] = safeSub(balances[aaddr], start);
        balances[msg.sender] = safeAdd(balances[msg.sender], start);
        emit Transfer(aaddr, msg.sender, start);
