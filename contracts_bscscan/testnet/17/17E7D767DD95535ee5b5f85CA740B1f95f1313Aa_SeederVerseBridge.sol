...
contract SeederVerseBridge is OwnableUpgradeable, ERC721Holder, OracleClient {
    ...
    event seedmonWithdrawn(uint256 tokenId, address withdrawer);
    event seedUploaded(uint256 amount, address uploader);

    function initialize(Seedmon _seedmon, Seed _seed) external initializer {
        seedmon = _seedmon;
        seed = _seed;
    }
        ...
        override
    {
        ...
        emit seedmonWithdrawn(tokenId, msg.sender);
    }
...
interface IClient {
    ...
    function onWithdrawSeedmonReceived(uint256 requestId, uint256 seedmonId, uint256 exp) external;
}
...
interface IOracle {
    ...
    function requestWithdrawSeedmon(uint256 requestId, uint256 tokenId) payable external;
}
