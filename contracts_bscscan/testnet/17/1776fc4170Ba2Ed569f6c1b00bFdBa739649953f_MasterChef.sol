...
// MasterChef is the master of UNIC. He can make UNIC and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once UNIC is sufficiently
...
contract MasterChef is Ownable {
    ...
    // The UNIC TOKEN!
    UNICORNTOKEN public unic;
    // Dev address.
    address public devaddr;
    // UNIC tokens created per block.
    uint256 public unicPerBlock;
    // Bonus muliplier for early unic makers.
    ...
    // Update the given pool's UNIC allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingUNIC(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 unicReward = multiplier.mul(unicPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updateEmissionRate(uint256 _unicPerBlock) public onlyOwner {
        massUpdatePools();
        unicPerBlock = _unicPerBlock;
    }
}
...
contract UNICORNTOKEN is BEP20('UNICORNTOKEN', 'UNIC') {
