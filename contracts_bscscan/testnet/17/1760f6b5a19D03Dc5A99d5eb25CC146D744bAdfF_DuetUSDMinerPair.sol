...
contract DuetUSDMinerPair is  Ownable, TokenRecipient  {
  ...
  constructor(address _stableToken, address _dusd, address _feeTo) {
  ...
  function checkRedeemLimit(uint amount) internal {
    uint redeemLimit = IERC20(stableToken).balanceOf(address(this)) * redeemPercent / PERCENT_BASE;
    require(amount <= redeemLimit, "insufficient liquidity");
  }
