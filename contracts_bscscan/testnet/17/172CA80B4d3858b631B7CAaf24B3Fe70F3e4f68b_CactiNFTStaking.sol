...
contract CactiNFTStaking is IERC721Receiver, ReentrancyGuard {
    ...
    constructor(
        ...
        calcRewardPerBlock();
        //stakingPeriods.push(StakingPeriod({ startBlock: block.number, endBlock: (block.number + period) })); 
        //emit StakePeriod(block.number, (block.number + period), activeStakingPeriodIndex);
    }
    ...
    function calcRewardPerBlock() private onlyDao {
    ...
    function calculateRewards () private onlyDao {
    ...
    function _payoutStake(uint256 tokenId) internal {
        ...
        //     payout = timeStaked.mul(rewardsPerBlock);
        // }
    ...
    function getCurrentStakeEarned(uint256 tokenId) external view returns (uint256) {
        return _getTimeStaked(tokenId).mul(rewardsPerBlock);
    }
    ...
    function getCurrentStakeEarnedOnAllTokens(address _wallet) external view returns (uint256) {
        ...
        for (uint256 index = 0; index < lengthStaked; index++) {
            ...
            staked = staked.add(_getTimeStaked(token).mul(rewardsPerBlock));
        }
        return staked;
    }
    ...
    function setUsersTokenStake(uint256 _rewardsPerBlock) external onlyDao {
        require(_rewardsPerBlock != 0, 'setrewardsPerBlock: Cannot be 0');
        rewardsPerBlock = _rewardsPerBlock;
