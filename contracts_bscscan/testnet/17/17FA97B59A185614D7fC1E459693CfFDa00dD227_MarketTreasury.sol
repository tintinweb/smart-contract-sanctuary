...
contract MarketTreasury is Ownable, Lockable {
    ...
    event BurnTokens(uint256 _aTokenAmount, uint256 pTokenAmount);
    ...
    // the extra aTokne and pToken will be transfered to 0x0
    function burnTokens() external onlyOwner() onlyInitialized() {
        uint256 _aBalance = aToken.balanceOf(address(this));
        uint256 _pBalance = pToken.balanceOf(address(this));

        aToken.safeTransfer(address(0), _aBalance);
        pToken.safeTransfer(address(0), _pBalance);

        emit BurnTokens(_aBalance, _pBalance);
    }
