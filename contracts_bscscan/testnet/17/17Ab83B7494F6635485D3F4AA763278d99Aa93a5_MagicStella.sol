...
contract MagicStella is BEP20, Ownable {
    ...
    function _priceImpactTaxV2(uint256 amount) public view returns (bool isExeedeedPriceImpact, uint256 priceImpactForAmount) {

        (uint256 reserve_a_initial, uint256 reserve_b_initial) = getReserves(
            address(this),
            BUSD
        );

        uint256 fee = 25; // 0.25 %
        uint256 max_price_impact = 2; // 2%

        uint256 amount_traded_stella = reserve_a_initial * max_price_impact / ((1 - max_price_impact)*(1 - fee));

        isExeedeedPriceImpact = false;
        if(amount > amount_traded_stella)
            isExeedeedPriceImpact = true;
        
        uint256 amountInStella = amount_traded_stella * (1 - fee);
        uint256 price_impact_trade_stella = amountInStella / (reserve_a_initial + amountInStella);


        return (isExeedeedPriceImpact, price_impact_trade_stella);
    }
