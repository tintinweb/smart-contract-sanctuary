...
contract OlympusTokenMigrator {
    ...
    constructor(
        ...
        approveForRouter(_FIDL, address(this));
        approveForRouter(_gFIDL, address(this));
        approveForRouter(_oldsFIDL, address(this));
    }
    ...
    function migrate(
        ...
        emit CheckTest(gAmount);
        emit CheckTest(3);
        oldStaking.unstake(_amount, false);

        emit CheckTest(4);
        FIDL.transfer(address(newStaking), _amount);
    ...
    function mintGFIdl (uint256 _amount) external {
        emit CheckTest(1); //
        uint256 gAmount = gFIDL.balanceTo(_amount);
