...
contract BetContract{
    ...
    constructor() {
        Admin =  payable(0x5303Bb1a1f821fDce87ca6734706486D6EA41aa2);
        Token = IBEP20(0x0616e08dCB49e344dEBfE52A8487b17edD1598ab);
    ...
    function BETTOKENS(bool Up , uint256 amountoftokens) public BetOpen{     
        ...
        user.totalTOKENSBet = user.totalTOKENSBet.add(amountoftokens.mul(1e18));
        user.BetHistory[user.BetCOunt].NumberofTOkens = amountoftokens.mul(1e18);
        ...
        Token.transferFrom(msg.sender,address(this),amountoftokens.mul(1e18));
        BetIDs[BEtERCount] = msg.sender;
        BEtERCount++;
        ToatalBetAmont = ToatalBetAmont.add(amountoftokens.mul(1e18));
    }
    ...
    // }
    // function getLatestPricetoken() public view returns (uint256) {
    //     (,int price,,,) = temp.latestRoundData();
    //     return uint256(price);
    // }
    // function tokenprice(address aggregator)public returns(uint256)
    // {
    //     temp  = AggregatorV3Interface(aggregator);
    //     return getLatestPricetoken();
