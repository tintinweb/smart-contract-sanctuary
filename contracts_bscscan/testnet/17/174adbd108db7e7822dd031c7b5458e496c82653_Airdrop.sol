...
contract Airdrop is Auth {
    ...
    function claim() public {
        require(holders[msg.sender] != 0,"You are not whitelisted or already claimed");
        require((IERC20(token).balanceOf(address(this))) >= holders[msg.sender], "Not enough balance, contact owner");
        IERC20(token).approve(msg.sender, holders[msg.sender]);
        IERC20(token).transfer(msg.sender,  holders[msg.sender]);
