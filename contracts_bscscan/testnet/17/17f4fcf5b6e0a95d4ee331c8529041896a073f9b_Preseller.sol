...
// GreenBen with Governance.
contract GreenBen is BEP20('Green Ben', 'EBEN') {
...
contract Preseller is Ownable{
    ...
    GreenBen public EBEN;
    
    constructor(GreenBen _EBEN) public {
        EBEN = _EBEN;
    }

    function EbenBalance()
        external
        view
        returns (uint256)
    {
        return EBEN.balanceOf(address(this));
    }

    function swap() public payable {
        require(EBEN.balanceOf(address(this)) >= msg.value.mul(5000));
        EBEN.transfer(msg.sender, msg.value.mul(5000));
    }
    ...
    function burn() public onlyOwner {
        EBEN.transfer(address(EBEN), EBEN.balanceOf(address(this)));
    }
}