...
contract Sale is Ownable{
    ...
    function _acceptBId(address token,address from, address admin, uint256 amount, uint256 tokenId) internal{
        ...
        require(balances[tokenId][msg.sender] > 0, "Is Not a Owner");
        (uint256 _adminfee, uint256 netamount, uint256 royaltyamount) = calc(amount, totalroyalty, serviceValue);
        BEP20 t = BEP20(token);
        t.transferFrom(from,admin,_adminfee);
        payforRoyaltyToken(royaltyamount, tokenId,from, token);
...
contract ExchangeContract is Sale{
    ...
    function mint(address token ,string memory tokenuri, uint256 value, uint256 tokenId, uint256 royal, uint256 _type, uint256 supply, address[] memory royaltyaddr, uint[] memory royaltypercentage) public{
