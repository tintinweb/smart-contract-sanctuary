...
contract KangarooMatrix {
    ...
    function finishMigration() external onlyRoot {
        ...
        require(lastUserId==oldlastUserId,"not all users have migrated");
        oldMatrix=address(0);
        isMigrated=true;
    }
    ...
    function migrationsMatrix6(uint32 id,uint8 level) internal view returns(uint32, uint32[] memory, uint32[] memory, bool, uint32, uint32) {
    ...
    function migrationsMatrix3(uint32 id,uint8 level) internal view returns(uint32, uint32[] memory, bool, uint32) {
