    ...
    AccessControlUpgradeable
{
    ...
    function initialize(
        ...
        Trainers _trainersContract,
        Bunicorns _bunicornsContract,
    ...
    Bunicorns internal bunicorns;
    Trainers internal trainers;
    MysteryBox internal mysteryBox;
    ...
    function getBunicornListingIDsPage(IERC721 _tokenAddress, uint8 _limit, uint256 _pageNumber, uint8 _trait, uint8 _stars)
        public
        view
        returns (uint256[] memory)
    {
        EnumerableSet.UintSet storage set = listedTokenIDs[address(_tokenAddress)];
        uint256 matchingBunicornsAmount = getNumberOfBunicornListings(_tokenAddress, _trait, _stars);
        uint256 pageEnd = _limit * (_pageNumber + 1);
        uint256 tokensSize = matchingBunicornsAmount >= pageEnd ? _limit : matchingBunicornsAmount.sub(_limit * _pageNumber);
        uint256[] memory tokens = new uint256[](tokensSize);

        uint256 counter = 0;
        uint8 tokenIterator = 0;
        for (uint256 i = 0; i < set.length() && counter < pageEnd; i++) {
            uint8 bunicornTrait = bunicorns.getTrait(set.at(i));
            uint8 bunicornStars = bunicorns.getStars(set.at(i));
            if((_trait == 255 || bunicornTrait == _trait) && (_stars == 255 || bunicornStars == _stars)) {
    ...
    function getTrainerListingIDsPage(IERC721 _tokenAddress, uint8 _limit, uint256 _pageNumber, uint8 _trait, uint8 _minLevel, uint8 _maxLevel)
        public
        view
        returns (uint256[] memory)
    {
        EnumerableSet.UintSet storage set = listedTokenIDs[address(_tokenAddress)];
        uint256 matchingTrainersAmount = getNumberOfTrainerListings(_tokenAddress, _trait, _minLevel, _maxLevel);
        uint256 pageEnd = _limit * (_pageNumber + 1);
        uint256 tokensSize = matchingTrainersAmount >= pageEnd ? _limit : matchingTrainersAmount.sub(_limit * _pageNumber);
        uint256[] memory tokens = new uint256[](tokensSize);

        uint256 counter = 0;
        uint8 tokenIterator = 0;
        for (uint256 i = 0; i < set.length() && counter < pageEnd; i++) {
            uint8 trainerTrait = trainers.getTrait(set.at(i));
            uint8 trainerLevel = trainers.getLevel(set.at(i));
             if((_trait == 255 || trainerTrait == _trait) && (_minLevel == 255 || _maxLevel == 255 || (trainerLevel >= _minLevel && trainerLevel <= _maxLevel))) {
                if(counter >= pageEnd - _limit) {
                    tokens[tokenIterator] = set.at(i);
                    tokenIterator++;
                }
    ...
    function getVBuniListingIDsPage(IERC721 _tokenAddress, uint8 _limit, uint256 _pageNumber, uint256 _minBuniAmount, uint256 _maxBuniAmount)
        public
        view
        returns (uint256[] memory)
    {
        EnumerableSet.UintSet storage set = listedTokenIDs[address(_tokenAddress)];
        uint256 matchingVBuniAmount = getNumberOfVBuniListings(_tokenAddress, _minBuniAmount, _maxBuniAmount);
        uint256 pageEnd = _limit * (_pageNumber + 1);
        uint256 tokensSize = matchingVBuniAmount >= pageEnd ? _limit : matchingVBuniAmount.sub(_limit * _pageNumber);
        uint256[] memory tokens = new uint256[](tokensSize);

        uint256 counter = 0;
        uint8 tokenIterator = 0;
        for (uint256 i = 0; i < set.length() && counter < pageEnd; i++) {
            (, uint256 amount,,) = IvBuni(address(_tokenAddress)).getTokenInfo(set.at(i));
            if(_minBuniAmount < amount && amount < _maxBuniAmount) {
                if(counter >= pageEnd - _limit) {
                    tokens[tokenIterator] = set.at(i);
                    tokenIterator++;
                }
    ...
    function getNumberOfTrainerListings(IERC721 _tokenAddress, uint8 _trait, uint8 _minLevel, uint8 _maxLevel)
        public
        view
        returns (uint256)
    {
        ...
        uint8 trainerTrait;
        for(uint256 i = 0; i < listedTokens.length(); i++) {
            trainerLevel = trainers.getLevel(listedTokens.at(i));
            trainerTrait = trainers.getTrait(listedTokens.at(i));
            if((_trait == 255 || trainerTrait == _trait) && (_minLevel == 255 || _maxLevel == 255 || (trainerLevel >= _minLevel && trainerLevel <= _maxLevel))) {
                counter++;
            }
        }
    ...
    function getNumberOfBunicornListings(IERC721 _tokenAddress, uint8 _trait, uint8 _stars)
        public
        view
        returns (uint256)
    {
        ...
        uint8 bunicornStars;
        for(uint256 i = 0; i < listedTokens.length(); i++) {
            bunicornTrait = bunicorns.getTrait(listedTokens.at(i));
            bunicornStars = bunicorns.getStars(listedTokens.at(i));
            if((_trait == 255 || bunicornTrait == _trait) && (_stars == 255 || bunicornStars == _stars)) {
                counter++;
            }
        }
    ...
    function getNumberOfVBuniListings(IERC721 _tokenAddress, uint256 _minBuniAmount, uint256 _maxBuniAmount)
        public
        view
        returns (uint256)
    {
        ...
        uint256 amount;
        for(uint256 i = 0; i < listedTokens.length(); i++) {
            (, amount,,) = IvBuni(address(_tokenAddress)).getTokenInfo(listedTokens.at(i));
            if(_minBuniAmount < amount && amount < _maxBuniAmount) {
                counter++;
            }
        }
    ...
    function recoverBuni(uint256 amount) public restricted {
        buniToken.safeTransfer(msg.sender, amount); // dont expect we'll hold tokens here but might as well
    }
