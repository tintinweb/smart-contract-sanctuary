...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minDistribution = 1 * (10 **9); // 1  minimum auto send
    uint256 public minimumTokenBalanceForDividends = 300 * (10 ** 9) ;
...
contract cainotoo is Ownable, IBEP20, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    string constant _name = "Nutulumonun";
    string constant _symbol = "Netninonese";
    uint8 constant _decimals = 9;
    uint256 _totalSupply = 25000000 * (10 ** _decimals);
    ...
    uint256 public maxSellTransactionAmount = 10000 * (10 ** _decimals);
    uint256 public _maxWallet = 160000 * (10 ** _decimals);

    uint256 public previus_maxTxAmount = _maxTxAmount;
    uint256 public previusmaxSellTransactionAmount = maxSellTransactionAmount;
    uint256 public previus_maxWallet = _maxWallet;


    uint256 public buyBackUpperLimit = 2 * 10**15;

    uint256 autoDevBurn = 0;
    uint256 burnFee = 0;
    ...
    uint256 previusreflectionFee = reflectionFee;
    uint256 previusMarketingFee = marketingFee;
    ...
    uint256 public swapThresholdBNB = _totalSupply / 5000; 
    bool inSwap;
    ...
    function presale(bool _presale) external onlyOwner {
        if (_presale) {
           ...
           swapThresholdBNB = _totalSupply;
           autoSuppEnable = false;
           buyBackEnable = false;
           dualMarkEnable = false;

        } else {
            swapEnabled = true;
            reflectionFee = previusreflectionFee;
            marketingFee = previusMarketingFee;
            feeDenominator = 100;
            _maxTxAmount = previus_maxWallet;
            maxSellTransactionAmount = previusmaxSellTransactionAmount;
            _maxWallet = previus_maxWallet;
           swapThreshold = _totalSupply ;
           swapThresholdBNB = _totalSupply;
           autoSuppEnable = true;
           buyBackEnable = true;
    ...
    function updateMaxTxAmount(uint256 Amount) public onlyOwner {
        _maxTxAmount = Amount ;
    }
