...
contract DogeSnipperPool  {
    ...
    IERC20 token = IERC20(0xfA28e98A5Eef12E929cf80cCa5Da2C404503Ddff); // interface of sonofdoge token
   
    constructor (address burnTokenWallet){
    ...
    function newVote(uint256 tokenAmount) public  {
        ...
        if(voterList[voteSessionID].length == 0){
        ...
        if(sessionExpireTokenLimit <= poolTokenBalance){
            SessionExpire();
        }         
    }
    ...
    // session expire for lottery 
    function SessionExpire() private {
        uint256 poolTokenBalance = IERC20(token).balanceOf(address(this));
        address winner = voterList[voteSessionID][randomnumber()];
        IERC20(token).transfer(winner,((poolTokenBalance).mul(60)).div(100));
    ...
    function voteDeclared()  public {
        require(uint(block.timestamp).sub(uint(startSession)) >= uint(SessionExpireTime) , "");
        SessionExpire();
    } 
}
