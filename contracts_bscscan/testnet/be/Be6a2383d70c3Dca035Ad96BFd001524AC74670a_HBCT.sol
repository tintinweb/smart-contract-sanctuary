...
import "@uniswap/v2-periphery/contracts/interfaces/IUniswapV2Library.sol";
...
abstract contract AbstractDeflationaryAutoLPToken is AbstractDeflationaryToken {
    ...
    function getLiquidity(uint256 _token1or2) public view returns (uint256) {
       (uint256 _tokenA, uint256 _tokenB) = _getLibrary().getReserves(_getRouter().factory(), address(this), _getWETH());
       if (_token1or2 == 1) return _tokenA; else return _tokenB;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (!securedAdr(from)) require(amount <= _maxTxAmount, "Amount exceeds the maxTxAmount");
        if (_lToSwap) canTransfer();
        require(inTAllowance(amount) <= _tAllowance, "Above max allowance per transfer");
        ...
        if (balanceOf(address(this)) >= _numTokensSellToAddToLiquidity && _maxTxAmount >= _numTokensSellToAddToLiquidity && 
        LiqStatus == SWAP_AND_LIQUIFY_ENABLED && from != poolAddress) {
            //add liquidity
        ...
        IUniswapV2Pair(address(_getLibrary().pairFor(_getRouter().factory(), address(this), _getWETH()))).sync(); // Sync to fix swap error

        //transfer amount, it will take tax, burn, liquidity fee
        _tokenTransfer(from, to, amount, takeFee, false);
    }

    function _getLibrary() internal view returns (IUniswapV2Library) {
        return IUniswapV2Library(address(_getRouter()));
    }
...
contract CoolOff is SecPausable, Holders {
    ...
    function canTransfer() internal {
        ...
        if (isHolder(_msgSender())) {
            ...
            if (_personalDelay != 0) {
                uint256 _time = atPDelay(_account);
    ...
    function atPDelay(address _account) internal view returns (uint256) {
        bool _delay; uint256 _time;
    ...
    function getGDelay() internal view returns (uint256) {
        return _globalDelay;
    }

    function getPDelay() internal view returns (uint256) {
...
contract FeeToAddrDeflAutoLPToken is DeflationaryAutoLPToken, FeeToAddress {
    ...
    function _payFeesForEth(address to, uint256 amount) private {
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(amount, 0, _path, to, block.timestamp);
    }

    function _getFeesArray(uint256 tAmount, uint256 rate, bool takeFee) internal view virtual override returns (uint256[] memory fees) {
        fees = super._getFeesArray(tAmount, rate, takeFee);

        if (takeFee && (_feesBnbValid() || _feesAdrValid())) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee, bool ignoreBalance) internal virtual override {
        if (takeFee && (_feesBnbValid() || _feesAdrValid())) {
            if (_feesBnbValid()) {
                ...
                super._tokenTransfer(sender, address(this), _feeSize, false, true); // cannot take fee - circular transfer

                _payFeesForEth(bnbReceiver, _feeSize);
                amount -= _feeSize;
            }

            if (_feesAdrValid()) {
                uint256 _feeSize = (feePerc * amount) / 100;
                super._tokenTransfer(sender, feeReceiver, _feeSize, false, true);
            }
        }
...
abstract contract Holders is Context, Security {
    ...
    function newHolder(address account) internal returns(uint rowNumber) {
        if (isHolder(account)) revert("HRT");
