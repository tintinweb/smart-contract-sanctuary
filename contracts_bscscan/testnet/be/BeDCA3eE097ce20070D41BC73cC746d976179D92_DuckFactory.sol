...
contract DuckFactory is Ownable {
    ...
    function transferDuckiesOwnership(address _newFactory) public onlyOwner() {
        IDuckies(tokenAddress).transferOwnership(_newFactory);
    }

    function openTrading(uint256 _tokenAmount, uint256 _bnbAmount) external onlyOwner() {
        require(!tradingOpen,"1: trading is already open");
        require(pancakeV2Pair == address(0), "2: trading is already open");
    ...
    /* function burnToken(uint256 _burnAmount) public onlyOwner() {
        IDuckies(tokenAddress)._burn(address(this),_burnAmount);
    } */
    ...
    function burnToken(uint256 _burnAmount) external onlyOwner() {
        IERC20(tokenAddress).transfer(address(0), _burnAmount);
    }
    // Admin functions: End
