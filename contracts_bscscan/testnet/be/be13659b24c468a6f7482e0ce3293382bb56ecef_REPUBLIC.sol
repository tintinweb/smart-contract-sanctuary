...
contract REPUBLIC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    REPUBLICDividendTracker public dividendTracker;
    ...
    uint256 public maxSellTransactionAmount = 1 * 10**7 * (10**9);
    uint256 public maxSellTransactionAmount2 = 1 * 10**7 * (10**9);
    uint256 public maxSellTransactionAmount3 = 2 * 10**6 * (10**9);
    uint256 public maxBuyTransactionAmount = 10**7 * (10**9);
    uint256 public swapTokensAtAmount = 1000 * (10**9); // 0.0001%
    uint256 public maxWalletToken = 51 * 10**6 * (10**9);  // 5.1%
    ...
    uint256 public minNotZero = 3;
   	uint256 public _transactionLockTime = 15 minutes;
    ...
    uint256 public snipeBNBRewardsSellFee = 30;
    ...
    constructor() ERC20("REPUBLIC", "RPC") {
        dividendTracker = new REPUBLICDividendTracker(payable(this));

        liquidityWallet = owner();

        //to change
        // Testnet Router : 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
        // Mainnet Router : 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
    ...
    function preLaunch() external onlyOwner {
        maxBuyTransactionAmount = 1 * 10**8 * 10**9;
    ...
    function updateTransactionLockTime(uint256 newNum) public onlyOwner {
        _transactionLockTime = newNum;
    } 
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        REPUBLICDividendTracker newDividendTracker = REPUBLICDividendTracker(
            payable(newAddress)
        );
...
contract REPUBLICDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "DREPUBLIC_Dividend_Tracker",
            "REPUBLIC_Dividend_Tracker"
        )
    {
        rEPUBLIC = REPUBLIC(mainContract);
        minimumTokenBalanceForAutoDividends = 50000 * (10**9); // 0.005%
        minimumTokenBalanceForDividends = 10000 * (10**9); // 0.001%
    ...
    function _transfer(
        ...
        require(false, "REPUBLIC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "REPUBLIC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main REPUBLIC contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(rEPUBLIC);

            uint256 prevBalance = rEPUBLIC.balanceOf(address(this));
            ...
            {
                uint256 received = rEPUBLIC.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    rEPUBLIC.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
