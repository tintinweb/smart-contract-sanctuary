...
contract LadyLassie is Context, IERC20, Ownable {
    ...
    uint public additionalLeechBlocks = 300;
    ...
    if(block.number <= antileech_block + additionalLeechBlocks) {
    ...
    function getantileech_block() public view returns(uint)
    {return antileech_block;}
    ...
    function setAdditionalLeechBlocks(uint256 value) external onlyOwner {
        additionalLeechBlocks = value;
    }

    function setTradingStatus(bool state, bool _activateLeechShield) external onlyOwner{
        tradingEnabled = state;
        swapEnabled = state;
        if(state == true && _activateLeechShield == true) antileech_block = block.number;
