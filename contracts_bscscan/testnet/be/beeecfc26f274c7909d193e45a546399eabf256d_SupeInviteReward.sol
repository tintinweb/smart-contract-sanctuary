...
contract SupeInviteReward is Ownable {
    ...
    function initialize(IStats _stats, IERC20 _lion, IERC20 _supe, IERC20 _tlp) external onlyFactoryOrOwner{
        stats = _stats;
        lion = _lion;
        supe = _supe;
        tlp = _tlp;
    }
    function initializeLion(uint256 per, uint256 max) external onlyFactoryOrOwner{
        PER_LION = per;
        MAX_LION = max;
    }
    function initializeTlp(uint256 per, uint256 max) external onlyFactoryOrOwner{
        PER_TLP = per;
        MAX_TLP = max;
    }
    function initializeSupe(uint256 per, uint256 max) external onlyFactoryOrOwner{
        PER_SUPE = per;
        MAX_SUPE = max;
    }
