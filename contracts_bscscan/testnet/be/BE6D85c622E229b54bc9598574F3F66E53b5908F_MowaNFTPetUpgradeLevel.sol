...
contract MowaNFTPetUpgradeLevel is Ownable, ReentrancyGuard {
    ...
    function getPriceUpgrade() public view returns (uint256) {
        uint256 checkTimeFee = block.timestamp.sub(3600);
        if (checkTimeFee < userUpgrade[_msgSender()].timeUpgrade) {
            return feeUpgrade.add(userUpgrade[_msgSender()].priceUpgrade).add(2);
        }
        return 2;
    }
    ...
    function petUpgradeLevel(uint256 tokenIdPet, uint256 tokenIdGems, uint256 tokenIdCharm) public nonReentrant {
        ...
        uint256 checkTimeFee = block.timestamp.sub(3600);
        if (checkTimeFee < userUpgrade[_msgSender()].timeUpgrade) {
            userUpgrade[_msgSender()].priceUpgrade = userUpgrade[_msgSender()].priceUpgrade.add(2);
            feeUpgrade = feeUpgrade.add(userUpgrade[_msgSender()].priceUpgrade);
        } else {
        ...
        level = level.add(1);
        uint256 levelGems = mowaItemsCore.getNFT(tokenIdGems).level;
        uint256 checkQtyGems = mowaItemsNFT.balanceOf(_msgSender(), tokenIdGems);
        qtyGems = qtyGems.add(level.sub(2));
        require(checkQtyGems >= qtyGems, "Not enough gems");
        require(levelGems == level, "Level Gems not equal");

        bool upgrade = false;
        if (level >= 5) {
            uint256 percentUpgrade = 0;
            if (tokenIdCharm > 0) {
                uint256 qtyCharm = mowaItemsNFT.balanceOf(_msgSender(), tokenIdCharm);
                require(qtyCharm >= 1, "Not enough lucky charm");
                uint256 levelCharm = mowaItemsCore.getNFT(tokenIdCharm).level;
                percentUpgrade = levelCharm.mul(20);
            ...
            if(percentUpgrade >= 100){
                ...
                if(rand >= 1 && rand <= percentUpgrade.sub(40)){
                    upgrade = true;
                }
            }
        } else {
