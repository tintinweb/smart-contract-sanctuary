...
import "./LazyNFTStorage.sol";
import "./MsgContext.sol";

// import "hardhat/console.sol";

contract LazyNFT is Initializable,
                    UUPSUpgradeable,
  ...
  bytes32 public constant LazyNFT_TYPEHASH =
        keccak256("NFTVoucher(uint256 tokenId,uint256 minPrice,address creator,address publisher,uint256 ratio,string uri)");
  ...
  // }
    ...
    function LazyNFT_init(address minter) private {
      _setupRole(MINTER_ROLE, minter);
      // _setRoleAdmin(MINTER_ROLE, msg.sender);
      // 部署者为管理员
      _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    ...
    function initialize() public payable initializer {
        __ERC721_init("DigitalPlanetNFT", "DPNFT");
        __ERC721URIStorage_init();
        __EIP712_init(SIGNING_DOMAIN, SIGNATURE_VERSION);
        __AccessControl_init();
        __Ownable_init();

        LazyNFT_init(msg.sender);
    }
  ...
  // 获取出版社-艺术家的版税分成
  function getPubishFeeRatio(address _pub, address _creator) public view returns (uint256) {
      uint ratio = publisherCreatorRatio[_pub][_creator];
      if (ratio > 0) {
        return ratio;
      }
  ...
  function redeem(address redeemer, NFTVoucher calldata voucher) public payable returns (uint256) {
    // console.log("redeem: %s %d", redeemer, voucher.tokenId);
    ...
    // 国内版 不校验价格
    // require(msg.value >= voucher.msinPrice, "Insufficient funds to redeem");
    ...
    // 设置 creator
    nftCreators[voucher.tokenId] = NFTCreator(voucher.creator, voucher.publisher, 10);
    nftSales[voucher.tokenId] = 1;
    ...
    uint amt = msg.value;
    if (feeTo != address(0)) {
        uint fee = msg.value.mul(firstSaleFee).div(DENOMINATOR);
        amt = amt.sub(fee);
        payable(feeTo).transfer(fee);
    }

    // pendingWithdrawals[feeTo] += fee; // .transfer()
    address publisher = voucher.publisher;
    uint256 ratio = voucher.ratio;
    if (publisher != address(0) && ratio > 0) {
        uint publisherFee = msg.value.mul(ratio).div(DENOMINATOR);
        if (publisherFee > 0) {
          amt = amt.sub(publisherFee);
          payable(publisher).transfer(publisherFee);
        }
    }
    payable(voucher.creator).transfer(amt);
    */
    emit Redeem(signer, voucher.publisher, redeemer, voucher.tokenId, voucher.minPrice);
    // pendingWithdrawals[signer] += msg.value.sub(fee);
  ...
  function verify(NFTVoucher calldata voucher) external view returns (address) {
  ...
  // 设置出版社分成比例
  function _setNFTPublisherRatio(
      uint256 _tokenId,
      uint256 _ratio
    ) internal validNFToken(_tokenId) {
  ...
  function setTrustForwarder(address to) public onlyRole(DEFAULT_ADMIN_ROLE) {
      _setTrustForwarder(to);
  }

  // 设置已经 mint 的商品的版税分成
  function setNFTPublisherRatio(
      uint256 _tokenId,
      uint256 _ratio
  ...
  function setFirstSaleFee(uint fee) external onlyRole(DEFAULT_ADMIN_ROLE) {
      require(fee < DENOMINATOR, "fee too high");
      firstSaleFee = fee;
  }

  function setPublishRatio(uint _ratio) external onlyRole(PUBLISHER_ROLE) {
...
contract LazyNFTStorageV1 {
  bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
  bytes32 public constant PUBLISHER_ROLE = keccak256("PUBLISHER_ROLE");
  string internal constant SIGNING_DOMAIN = "LazyNFT-Voucher";
...
abstract contract MsgContext {
    ...
    function _setTrustForwarder(address _forwarder) internal virtual {
    ...
    function _metaTxSender() internal view virtual returns (address sender) {
    ...
    function _metaTxData() internal view virtual returns (bytes calldata) {
        if (isTrustedForwarder(msg.sender)) {
