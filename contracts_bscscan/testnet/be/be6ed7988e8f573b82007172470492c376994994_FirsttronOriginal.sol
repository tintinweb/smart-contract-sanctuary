...
contract FirsttronOriginal {
  ...
  function _refPayout(address _addr, uint256 _amount) internal {
        ...
        for(uint8 i = 0; i < ref_bonuses.length; i++) {
            ...
            if(users[up].refs[0] >= i.add(1)){  // need to be change here after discussion
  ...
  function withdrawal() external{
    ...
    _user.totalWithdrawn=TotalBonus;
  ...
  function _safeTransfer(address payable _to, uint _amount) internal returns (uint256 amount) {
   ...
   function referral_count(address _upline,uint _index)external view returns(uint){
       return users[_upline].refs[_index];
   }
