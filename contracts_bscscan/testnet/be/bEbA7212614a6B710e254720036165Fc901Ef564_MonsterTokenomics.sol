...
contract MonsterTokenomics is Ownable {
    ...
    function privateUnlock() public onlyOwner {
        ...
        console.log("Remain %s" , PRIVATE_REMAINING);
    ...
    function devUnlock() public onlyOwner {
        ...
        if(DEV_ALLOCATION.sub(DEV_PAID) >= DEV_WEEK_MINT*2){
            token.mintNomics(bossAddress, DEV_WEEK_MINT);
            DEV_PAID = DEV_PAID.add(DEV_WEEK_MINT);
        } else{
            token.mintNomics(bossAddress, DEV_ALLOCATION.sub(DEV_PAID));
            DEV_PAID = DEV_ALLOCATION;
        }
    }

    function airdropMint(address _addr, uint256 _amount) public onlyOwner {
...
contract MockERC20 is ERC20, Ownable  {
    ...
    event Mint(address indexed add, uint256 amount);
