...
contract BlindBoxSale is ERC1155Holder, Ownable {
    ...
    uint256 public diffDay = 120 minutes;
   ...
   function _transferNftToAuctionContract(
    ...
    function saleWithTimeLimits()
        ...
        view
    {
        uint256 diffTime  = block.timestamp.sub(saleStartTime).mod(120 minutes  * diffDay);
        ...
        view returns(uint256)
    {
        ...
        if(block.timestamp.sub(saleStartTime)<= 120 minutes ){
            ...
            timeIndex = block.timestamp.sub(saleStartTime).div(120 minutes  * diffDay).mul(120 minutes  * diffDay).add(saleStartTime);
        }
        return(timeIndex);
    }
        ...
        saleWithinLimits(_nftContractAddress,_tokenId,_nftSeller,_amount)
    {
        ...
        if (_isERC20Auction(payToken)) {
            if(_recipient==address(0)){
                ...
                IERC20(payToken).transferFrom(msg.sender,_recipient,_amount.mul(_buyNowPrice) );
            }
        } else {
            ...
            if(_recipient==address(0)){
                ...
                payable(_recipient).transfer(_amount.mul(_buyNowPrice));
            }
        }
        ...
        uint256 timeIndex = getTimeIndex();
        saleAmounts[timeIndex] = saleAmounts[timeIndex].add(_amount);        
