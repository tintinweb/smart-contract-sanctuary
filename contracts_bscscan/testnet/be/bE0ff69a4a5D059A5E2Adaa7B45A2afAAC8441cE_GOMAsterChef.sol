...
contract GOMAsterChef is Ownable {
    ...
    constructor() {
        ...
        stakedToken = IBEP20(0x651A275E536bEf67D17aC7B32c1C7f48c610773a); // GOMA
        rewardToken = IBEP20(0x6Ea9E7324EcF0b78e5A6cA9993d4BaFE8ef8B4c7); // AGR
        stakedTokenOwner = 0xf7a6799E164685Ef752e7121eC6CBf47D6B67dD5;
    ...
    function pendingRewardsOfUser(address _user) public view returns (uint256 real, uint256 reflection) {
		...
		if (stakedTokenDeposied != 0 && user.amount != 0 && blockTimestamp() > lastRewardTimestamp) {
            ...
            //real = ((user.amount * _accRewardTokensPerShare) / stakedTokenDeposied) - user.rewardDebt;            
        }
		real = ((user.amount * _accRewardTokensPerShare) / 1e12) - user.rewardDebt;
    ...
    function pendingRewardsAll() public view returns (uint256) {
        if (stakedTokenDeposied != 0) {
            ...
            return accRewardTokensPerShare + (_mul * tokensPerSecond) - claimedRewardTokens;
        }
        return 0;
    }
    ...
    function getPending(UserInfo storage user) internal view returns (uint256) {
        //return ((user.amount * accRewardTokensPerShare) / stakedTokenDeposied) - user.rewardDebt;
