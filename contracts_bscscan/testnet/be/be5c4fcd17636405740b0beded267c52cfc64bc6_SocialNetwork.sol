...
contract SocialNetwork {
    ...
    function addBug(string calldata bugName) public {
        ...
        UserBugs.push(msg.sender);
        Bug storage _bug = bugs[msg.sender];
        _bug.name = bugName;
        _bug.belongTo = msg.sender;
    }
    ...
    function getBug(address owner) public view returns (string memory icerik, address benim){
        benim = bugs[owner].belongTo;
        Bug storage _bug = bugs[owner];
        icerik = _bug.name;
        benim = _bug.belongTo;
