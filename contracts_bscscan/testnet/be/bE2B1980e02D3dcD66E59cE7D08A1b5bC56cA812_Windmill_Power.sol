...
/**
 * @notice Windmill_Power (PWR) is the ERC20 token representing
...
contract Windmill_Power is ERC20, AccessControlEnumerable {
     ...
     * in exchange of depositing BUSD in the fund.

     * When Windmill_Fund use this method, this do not change the PWR price
     * because there is the corresponding amount of BUSD that have been added
     * to the fund.
     */
     ...
     * Windmill_Fund use this method to lock the PWR from direct
     * withdraw. There is two main reason for this to happen :
     *
     * - PWR are lock from user to Windmill_Trade contract when starting
     * a new trade. The corresponding BUSD from Windmill_Fund are also
     * allocated to the trade. These locked PWR are returned at the end of
     * the trade.
     *
     * - PWR are stacked by the user. These PWR are returned at the end of
     * the stacking period. Note that returned PWR can be still locked in a
     * trade, that will be return at the end of trade.
     *
     *
     * Note that in order to compute the true number of PWR owned by a user,
     * checks for locked or stacked PWR have to be done. The amount of PWR
     * token represent only free PWR tokens.
     */
