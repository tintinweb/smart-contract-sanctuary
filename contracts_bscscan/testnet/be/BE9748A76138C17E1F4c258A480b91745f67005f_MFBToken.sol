...
// File: contracts/contracts/token/ERC20/extensions/IERC20Metadata.sol
...
contract MFBToken is ERC20, Ownable{  
    ...
    //initialize the address setting
    function initialize_adr (address pool_adr, address box_adr, address market_adr, address exchange_adr) public onlyOperator {
        //require(is_init == false, "this contract is initialized already!");
        _pool_adr = pool_adr;
        _box_adr = box_adr;
        _market_adr = market_adr;
        _exchange_adr = exchange_adr;
        is_init = true;
    }
    //end setting
    function approve2Contract(address adr,uint amount) external returns (uint){
        ...
        if(msg.sender == _exchange_adr){
            _approve(adr,_exchange_adr,amount);
        }
        return allowance(msg.sender,adr);
    }
    function cleanApprove(address adr) external returns (uint){
        require(is_init == true,"contract initialize is not done yet");
        bool flag = false;
        if(msg.sender == _pool_adr){
            flag = true;
