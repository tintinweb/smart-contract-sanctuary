...
contract MetaMillionaire is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = payable(0x6f11b5A9d2c997ccC7F3359218a32aC47Daf7796); // Marketing Address W4

    address payable public liquidityAddress =
        payable(0x49C20eD42ebFbBE28559e6f59A5230370fA42373); // Liquidity Address
                
    address payable public dev1Address =
        payable(0xC87F925F8F868f52a63c8fAC2a6A8bD0339C16A6); // Dev1 Address
    
    address payable public dev2Address =
        payable(0x3104fc22e21B2F1Ef539869d49817f533Bb1630D); // Dev2 Address
    ...
    string private constant _name = "MetaMillionaire";
    ...
    uint256 public _buyDev1Fee = 15;
    uint256 public _buyDev2Fee = 15;

    uint256 public _sellTaxFee = 20;
    uint256 public _sellLiquidityFee = 30;
    uint256 public _sellMarketingFee = 30;
    uint256 public _sellDev1Fee = 15;
    uint256 public _sellDev2Fee = 15;
    ...
    uint256 private gasPriceLimit = 75 * 1 gwei; // 75 = BEP20, 300=ERC20
    
    // store addresses that a automatic market maker pairs. Any transfer *to* these addresses
    // could be subject to a maximum transfer amount
    mapping (address => bool) public automatedMarketMakerPairs;

    uint256 public minimumTokensBeforeSwap = _tTotal * 5 / 100000; // 0.00005%
    ...
    constructor() payable {
        ...
        maxBuyAmount = _tTotal * 25 / 10000; // 0.25% maxTransactionAmountTxn
        maxSellAmount = _tTotal * 25 / 10000; // 0.25% maxTransactionAmountTxn
    ...
    function updateMaxBuyAmount(uint256 newNum) external onlyOwner { 
        require(newNum >= (_tTotal * 25 / 10000)/10**9, "Cannot set max buy amount lower than 0.025%"); 
    ...
    function updateMaxSellAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (_tTotal * 25 / 10000)/10*9, "Cannot set max sell amount lower than 0.025%");
      ...
      // If any account belongs to _isExcludedFromFee account then remove the fee
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyDev1Fee + _buyDev2Fee;
                if(_liquidityFee > 0){
                    buyOrSellSwitch = BUY;
                }
            } 
            // Sell
                ...
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellDev1Fee + _sellDev2Fee;
                if(_liquidityFee > 0){
    ...
    function swapBack() private lockTheSwap {
        ...
        bool success;
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _dev1TokensToSwap + _dev2TokensToSwap + _marketingTokensToSwap;
        ...
        uint256 bnbForDev1 = bnbBalance.mul(_dev1TokensToSwap).div(totalTokensToSwap);
        uint256 bnbForDev2 = bnbBalance.mul(_dev2TokensToSwap).div(totalTokensToSwap);
        
        uint256 bnbForLiquidity = bnbBalance - bnbForMarketing - bnbForDev1 - bnbForDev2;
    ...
    (success,) = address(dev1Address).call{value: bnbForDev1}("");
        (success,) = address(dev2Address).call{value: bnbForDev2}("");
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyDev1Fee, uint256 buyDev2Fee)
        external
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyDev1Fee + _buyDev2Fee <= 200, "Must keep taxes below 20%");
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellDev1Fee, uint256 sellDev2Fee)
        external
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellDev1Fee + _sellDev2Fee <= 300, "Must keep taxes below 30%");
    }
...
//      IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); // mainnet
        ...
        liquidityAddress = payable(address(liquidityAddress));
        return true;
    }
