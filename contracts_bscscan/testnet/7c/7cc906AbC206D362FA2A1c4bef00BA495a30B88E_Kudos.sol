...
contract Kudos is Context, IKRC20, Ownable {
    ...
    uint256 private _competitionLength = 120; //CHANGEME make this an hour - 3600
    ...
    constructor (address platformWallet) {
        ...
        _isExcluded[address(0x0000000000000000000000000000000000000000)] = true;
    ...
    function _transfer (address from, address to, uint256 amount) private {
        ...
        if ((contractTokenBalance >= _numTokensSellToAddToLiquidity || block.timestamp >= _competitionEndTime) && !_inSwapAndLiquify && from != _kuswapV2Pair && _swapAndLiquifyEnabled)
    ...
    function _takeKCSFees (uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (contractTokenBalance > 0) {
            ...
            competitionBalance = kcsRemainder.mul(_competitionFeePercentage).div(feeDivisor);
    ...
    function _tokenTransfer (address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        if (!_isExcluded[trader] && amm == _kuswapV2Pair) {
            ...
            if (_tradingStats[trader].volume > _currentLeader.volume || _currentLeader.lastTradeEpochEnd != _competitionEndTime) {
                _currentLeader.lastTradeEpochEnd = _tradingStats[trader].lastTradeEpochEnd;
                _currentLeader.volume = _tradingStats[trader].volume;
                _currentLeader.rewardAddress = _tradingStats[trader].rewardAddress;
            }
        }
