...
contract DragonaireNFTCoreTest is
    Ownable,
    ERC721Enumerable,
    ERC721Burnable,
    ERC721Pausable
{
    ...
    string private name_ = "DragonNFT";
    string private symbol_ = "DRG";
    string private baseTokenURI_ = "https://nft.dragonaire.io/";
    uint256 private initialCap_ = 10;
    ...
    constructor() ERC721(name_, symbol_) {
        require(initialCap_ > 0, "DragonaireNFTCore: cap is 0");
    ...
    function _mint(address to_, uint256 tokenId_) internal virtual override {
        require(
            ...
            "DragonaireNFTCore: cap exceeded"
        );
    ...
    function increaseCap(uint256 amount_) public onlyOwner {
        require(amount_ > 0, "DragonaireNFTCore: amount is 0");

        uint256 newCap = cap() + amount_;
        _updateCap(newCap);
    }

    function updateBaseTokenURI(string memory _baseTokenURI_) public onlyOwner {
    ...
    function mintBatch(address to_, uint256 amount_) public onlyOwner {
        require(amount_ > 0, "DragonaireNFT: amount_ is 0");
    ...
    function tokenURI(uint256 tokenId_) public view virtual override returns (string memory) {
        require(
            _exists(tokenId_),
            "DragonaireNFTCore: URI query for nonexistent token"
        );
