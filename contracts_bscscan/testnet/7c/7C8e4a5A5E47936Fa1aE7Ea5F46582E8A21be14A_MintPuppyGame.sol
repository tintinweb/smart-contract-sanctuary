...
contract MintPuppyGame is Ownable, ReentrancyGuard {
    ...
    uint256 public smoyCost = 8000 ether;
    ...
    event onUpdateCost(uint256 oldBusdCost, uint256 newBusdCost, uint256 oldSmoyCost, uint256 newSmoyCost);
    ...
    function mintWithSMOY() external nonReentrant returns (uint256) {
        _collectFee(smoyToken, smoyCost);
    ...
    function withdrawSmoy() external onlyOwner nonReentrant {
        uint256 balance = smoyToken.balanceOf(address(this));
        require(balance > 0, "MintPuppyGame: INSUFFICIENT_SMOY");
        ...
        if (_feeCollectorAmount > 0)
            TransferHelper.safeTransfer(address(smoyToken), feeCollectorAddress, _feeCollectorAmount);
        if (_platformReserveAmount > 0)
            TransferHelper.safeTransfer(address(smoyToken), platformReserveAddress, _platformReserveAmount);
        if (_godfatherPrizeAmount > 0)
            TransferHelper.safeTransfer(address(smoyToken), godfatherPrizeAddress, _godfatherPrizeAmount);
        if (_puppyPrizeAmount > 0)
            TransferHelper.safeTransfer(address(smoyToken), puppyPrizeAddress, _puppyPrizeAmount);

        emit onWithdrawBalance(msg.sender, "SMOY", balance);
    }
    ...
    function setCost(uint256 _busdCost, uint256 _smoyCost) external onlyOwner {
        uint256 oldSmoyCost = smoyCost;
        uint256 oldBusdCost = busdCost;
        smoyCost = _smoyCost;
        busdCost = _busdCost;

        emit onUpdateCost(oldBusdCost, busdCost, oldSmoyCost, smoyCost);
    }
