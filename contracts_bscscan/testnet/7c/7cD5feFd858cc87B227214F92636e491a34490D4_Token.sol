...
contract Token is Context, Ownable, IBEP20 {
  ...
  uint256 private _tTotal = 10 ** 12 * _decimalFactor;
  uint256 private _rTotal = MAX - (MAX % _tTotal);
  uint256 private _totalReflections;

  // Token limits
  uint256 public _maxTxAmount = 10 ** 9 * _decimalFactor;
  uint256 public _tokenSwapThreshold = 100 * 10 ** 6 * _decimalFactor;

  // Fees
  uint256 _taxFee = 8;
  uint256 _sellTaxFee = 12;
  uint256 _whaleSellTaxFee = 20;
  ...
  // Anti-whale
  uint256 public _whaleSellThreshold = 500 * 10**6 * _decimalFactor;
  uint public _whaleTimer = DAY;
  ...
  uint256 public _buybackUpperLimit = 1 * 10 ** 6 * _decimalFactor;
  ...
  constructor() {
    ...
    IUniswapV2Router02 pancakeswapV2Router = IUniswapV2Router02(0xdc4904b5f716Ff30d8495e35dC99c109bb5eCf81);
  ...
  function transferFrom(address spender, address recipient, uint256 amount) external override returns(bool){
    ...
    require(_allowances[spender][_msgSender()] >= amount, "BEP20: You cannot spend that much on this account");
    
    _transfer(spender, recipient, amount);
    
    _approve(spender, _msgSender(), _allowances[spender][_msgSender()].sub(amount, "BEP20: Transfer amount exceeds allowance"));
    return true;
  }
  ...
  function withdrawForeignToken(address token) public onlyOwner {
    ...
    IERC20(address(token)).transfer(_msgSender(), IERC20(token).balanceOf(address(this)));
  }

  function reflect(uint256 tAmount) public {
    require(!_isExcluded[_msgSender()], "Excluded addresses cannot call this function");
    (uint256 rAmount,,,,) = _getValues(tAmount);
    _rBalances[_msgSender()] = _rBalances[_msgSender()].sub(rAmount);
  ...
  function setBuyback(bool buyback) external onlyOwner {
    _enableBuyback = buyback;
  }

  function setBuybackBNBThreshold(uint256 thresholdAmount) external onlyOwner {
    _buybackBNBThreshold = thresholdAmount;
  }

  function setBuybackPercent(uint256 percentage) external onlyOwner {
    _buybackFee = percentage;
  }

  function setTaxFeePercent(uint256 percentage) external onlyOwner {
    _taxFee = percentage;
  }

  function setSellTaxFeePercent(uint256 percentage) external onlyOwner {
    _sellTaxFee = percentage;
  }

  function setWhaleTaxFeePercent(uint256 percentage) external onlyOwner {
    _whaleSellTaxFee = percentage;
  }

  function setTreasuryFeePercent(uint256 percentage) external onlyOwner {
    _treasuryFee = percentage;
  }

  function setBurnFeePercent(uint256 percentage) external onlyOwner {
    _burnFee = percentage;
  }

  function setBuybackUpperLimit(uint256 limitAmount) external onlyOwner {
    _buybackUpperLimit = limitAmount;
  }

  function setLiquidity(bool liquidity) external onlyOwner {
    _enableLiquidity = liquidity;
  }

  function setWhaleSellThreshold(uint256 whaleThreshold) external onlyOwner {
    _whaleSellThreshold = whaleThreshold;
  }

  function setWhaleTimer(uint time) external onlyOwner {
    _whaleTimer = time;
  }
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    require(owner != address(0), "BEP20: approve cannot be done from zero address");
    require(spender != address(0), "BEP20: approve cannot be to zero address");
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    require(amount > 0, "BEP20: Transfer must be greater than zero");

    if (sender != owner() && recipient != owner()) {
      require(amount <= _maxTxAmount, "BEP20: Transfer amount exceeds max transaxtion amount");
    }
    ...
    if (_enableLiquidity && tokenBalance >= _tokenSwapThreshold && !currentlySwapping && sender != _pancakeswapV2LiquidityPair) {
      tokenBalance = _tokenSwapThreshold;
      swapAndLiquify(tokenBalance);
    }

    if (_enableBuyback && !currentlySwapping && recipient == _pancakeswapV2LiquidityPair) {
    ...
    if (takeFee && recipient == _pancakeswapV2LiquidityPair) {
      ...
      if (delta > 0 && delta < _whaleTimer && _timeSinceFirstSell[sender] != 0) {
  ...
  function _tokenTransfer(address sender, address recipient, uint256 tAmount) private {
    ...
    if (tFee > 0) {
      ...
      _burnTokens(tPortion * _burnFee);
      _addToTreasury(tPortion * _treasuryFee);

      if (recipient == _pancakeswapV2LiquidityPair) {
        _reflectTokens(tPortion * _sellReflectionFee);

        _addToContract(tFee.sub(tPortion * _burnFee).sub(tPortion * _treasuryFee).sub(tPortion * _sellReflectionFee));
      } else {
        _reflectTokens(tPortion * _reflectionFee);

        _addToContract(tFee.sub(tPortion * _burnFee).sub(tPortion * _treasuryFee).sub(tPortion * _reflectionFee));
      }
    }
  ...
  function swapAndLiquify(uint256 tokenAmount) private lockSwapping {
    uint256 sixth = tokenAmount.div(6);
    uint256 swapAmount = tokenAmount.sub(sixth);
    uint256 initialBalance = address(this).balance;

    swapTokensForBNB(swapAmount);

    uint256 receiveBNB = address(this).balance.sub(initialBalance);
    uint256 liquidityBNB = receiveBNB.div(5);

    addLiquidity(sixth, liquidityBNB);

    uint256 marketingBNB = receiveBNB.div(5).mul(2);

    transferBNBToAddress(_marketingAddress, marketingBNB);

    emit SwapAndLiquify(swapAmount, liquidityBNB, sixth);
  }
  ...
  function _burnTokens(uint256 tFee) private {
    if (tFee > 0) {
      ...
      _rBalances[_burnAddress] = _rBalances[_burnAddress].add(rFee);
      if (_isExcluded[_burnAddress]) {
        _tBalances[_burnAddress] = _tBalances[_burnAddress].add(tFee);
      }
    }
  }

  function _addToTreasury(uint256 tFee) private {
    if (tFee > 0) {
      uint256 rFee = tFee.mul(_getRate());
      _rBalances[_treasuryAddress] = _rBalances[_treasuryAddress].add(rFee);
      if (_isExcluded[_treasuryAddress]) {
        _tBalances[_treasuryAddress] = _tBalances[_treasuryAddress].add(tFee);
      }
    }
  }
  ...
  function _addToContract(uint256 tAmount) private {
    uint256 rAmount = tAmount.mul(_getRate());
    _rBalances[address(this)] = _rBalances[address(this)].add(rAmount);
    if (_isExcluded[address(this)]) {
      _tBalances[address(this)] = _tBalances[address(this)].add(tAmount);
    }
  }
  ...
  function _getCurrentSupply() private view returns (uint256, uint256) {
    ...
    for (uint i = 0; i < _excluded.length; i++) {
      if (_rBalances[_excluded[i]] > rSupply || _tBalances[_excluded[i]] > tSupply) return (_rTotal, _tTotal);
