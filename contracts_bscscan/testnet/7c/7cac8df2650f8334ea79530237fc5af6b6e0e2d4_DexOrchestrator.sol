...
contract DexOrchestrator {
    ...
    function fs_callback(
        ...
        // Defining facts
        uint256 borrowedAmount = _amount0 == 0 ? _amount1 : _amount0;
        address token0;
        uint256 i;
        ...
        for (i = actions.length; i > 0; i--) {
            ...
            actions[i - 1].amount0Out = actions[i - 1].path0 == token0
                ? 0
                : amountNeeded;
            actions[i - 1].amount1Out = actions[i - 1].path0 == token0
                ? amountNeeded
                : 0;
        }

        require(
            borrowedAmount > amountNeeded,
            "PairOrchestrator: WONT_GET_ENOUGH_TOKENS"
        );
        uint256 profit = borrowedAmount - amountNeeded;
