...
contract Payments is Ownable {
    ...
    function payOwner(address _owner, uint256 _amount) external onlyAuth returns(bool){
        uint256 _toOwner = _amount.mul(8000).div(10000);
        uint256 _toDistribute = _amount.sub(_toOwner);

        // 80% for owner of Nuresery/Training Center or Forge
        require(BZAI.transfer(_owner, _toOwner));

        // 5 % for teams members
        require(BZAI.transfer(gameAddresses.getTeamAddress(), _toDistribute.div(4)));

        address _stakingAddress = gameAddresses.getStakingAddress();
        // 5 % for stakers
        IStaking(_stakingAddress).receiveFees(_toDistribute.div(4));
        require(BZAI.transfer(_stakingAddress, _toDistribute.div(4)));

        // 5%  for reward challenge in game (daily and weekly ranking)
        require(BZAI.transfer(gameAddresses.getChallengeRewardsAddress(), _toDistribute.div(4)));
        // 5% for winning contract (where each win in game give 0.1% of the win pool to the user)
        return (BZAI.transfer(gameAddresses.getWinRewardsAddress(), _toDistribute.div(4)));
    }

    function distributeFees(uint256 _amount) external onlyAuth returns(bool){
        uint256 toStakers = _amount.mul(8000).div(10000);
        uint256 _toDistribute = _amount.mul(1000).div(10000);
        ...
        // 5%  for reward challenge in game (daily and weekly ranking)
        require(BZAI.transfer(gameAddresses.getChallengeRewardsAddress(), _toDistribute.div(2)));
        // 5% for winning contract (where each win in game give 0.1% of the win pool to the user)
        return (BZAI.transfer(gameAddresses.getWinRewardsAddress(), _toDistribute.div(2)));
    }
