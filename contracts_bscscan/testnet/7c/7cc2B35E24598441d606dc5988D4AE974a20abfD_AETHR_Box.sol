    ...
    Ownable
{
    ...
    constructor(
        ...
        address _ATHToken,
        uint256 _totalBox,
        uint256 _totalBoxPremium,
        uint256 _priceBox,
        uint256 _priceBoxPremium

    ) ERC721("Aethr Box", "ATHB") {

        BUSD = IERC20(_BUSD);
        ATH = IERC20(_ATHToken);

        totalBox = _totalBox;
        totalBoxPremium = _totalBoxPremium;

        priceBox = _priceBox ** uint256(16); // ** uint256(18) when deploy
        priceBoxPremium = _priceBoxPremium ** uint256(16) * 100; // ** uint256(18) when deploy
    ...
    modifier checkSaleRequirements(address _msgSender) {
        uint256 allowToPayAmount = BUSD.allowance(
            _msgSender,
            address(this)
        );
        require(
        ...
        require(block.timestamp >= startDate && block.timestamp < endDate, 'Sale time passed');
    ...
    modifier checkSalePremiumRequirements(address _msgSender) {
        uint256 allowToPayAmount = ATH.allowance(
            _msgSender,
            address(this)
        );
        require(
    ...
    modifier checkClaimRequirements(uint256 totalClaim,address addressClaim){
        ...
        require(
            BUSD.balanceOf(address(this)) >= totalClaim,
            'Box Payment: Invalid balanceOf'
        );
        require(
    ...
    function claimPool(uint256 totalClaim,address addressClaim) public checkClaimRequirements(totalClaim,addressClaim) returns (uint256) {
        poolUser[addressClaim] -= totalClaim;
        BUSD.transferFrom(address(this),addressClaim, totalClaim);
        return poolUser[addressClaim];
    }

    function boxFree() public checkClaimBoxRequirements(_msgSender()) {
        _tokenIds.increment();
    ...
    function boxBasic(address to) public checkSaleRequirements(_msgSender()) virtual {
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        require(!_exists(newItemId), "Box Payment: must have unique tokenId");
        BUSD.transferFrom(_msgSender(), address(this), priceBox);
        _mint(to, newItemId);
        poolUser[_msgSender()] += priceBox;
        emit BuyBox(
            newItemId,
            _msgSender()
        );
    ...
    function boxPremium(address to) public checkSalePremiumRequirements(_msgSender()) virtual {
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        require(!_exists(newItemId), "Box Payment: must have unique tokenId");
    ...
    function openBox(uint256 boxId) public checkOpenBox(boxId) {
        FactoryNFT.mint(_msgSender(),boxId);
