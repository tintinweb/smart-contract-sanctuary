...
contract VestingVault is Ownable {
    ...
    public view returns (uint, uint, uint, uint, uint[] memory, uint[] memory, uint, uint) {
        ...
        return (grant.value, grant.vestingStart, grant.vestingCliff, grant.vestingDuration, grant.scheduleTimes,
        grant.scheduleValues, grant.transferred, grant.grantedCount);
    }
    ...
    function grant(
        ...
        uint[] memory _scheduleValues) public onlyOwner isOpen returns (uint256) {
        ...
        if (grants[_to].value == 0) {
          ...
          // if (_level == 2) {
            ...
            emit NewGrant(_to, _value, _start, _duration, _cliff, _scheduleTimes, _scheduleValues);
        }
        else {
            ...
            // if (grants[_to].level == 2) {
                ...
                for (uint i = 0; i < _scheduleTimes.length; i++) {
                    ...
                    if (i > 0) {
                        // require(_scheduleTimes[i] > _scheduleTimes[i - 1], "ScheduleTimes should be sorted by ASC");
                        shouldSort = _scheduleTimes[i] < _scheduleTimes[i - 1] || _scheduleTimes[i] < grants[_to].scheduleTimes[length - 1];
                    }
            ...
            emit AddGrant(_to, _value, _start, _duration, _cliff, _scheduleTimes, _scheduleValues);
        }
