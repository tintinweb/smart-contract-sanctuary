...
contract Fund is Crawler, ICrawler {
    ...
    bytes __encodedExecutorMessage;
    ...
    function executeExecutor() onlyOwner public {
        recentReqId = iterateRequestId();
        crawlerQuery(recentReqId, BridgeExecutor, abi.encode(__encodedIPFS));
    }
    ...
    function getExecutorInfo() external view returns(bytes memory) {
        return __encodedExecutorMessage;
    }
    ...
    function crawlerCallback(bytes32 _reqId, bytes32 _type, bytes calldata _calldata) external override onlyResolver {
        ...
        if (_calldata.length == 0) {
            ...
            } else if (_type == BridgeExecutor) {
                __encodedExecutorMessage = _calldata;
            } else {
