...
contract ContExchange is Ownable, ReentrancyGuard {
    ...
    constructor (address _nftAddress, address _busdAddress, address _contAddress) {
        require(_nftAddress != address(0), "ContExchange: nftAddress is the zero address");
        require(_busdAddress != address(0), "ContExchange: busdAddress is the zero address");
    ...
    function sellToken(uint256 tokenId, NftPrice memory nftPrice) public {
        require(msg.sender != address(0) && msg.sender != address(this), "ContExchange: sender is the zero address");
        require(nftAddress.ownerOf(tokenId) == msg.sender, "ContExchange: sender is not owner ");
        require(nftPrice.price > 0, "ContExchange: NFT price must then ZERO");

        NftPrice memory nftPriceExisted = nftSellPrices[tokenId];
        require(nftPriceExisted.price == 0, "ContExchange: NFT is selling");
    ...
    function buyToken(uint256 tokenId) payable public nonReentrant {
        ...
        if (nftPrice.token == CirculatingToken.BUSD) {
            require(busdAddress.balanceOf(msg.sender) >= nftPrice.price, "ContExchange: BUSD amount is less than price");

            busdAddress.transferFrom(msg.sender, addressSeller, nftPrice.price);
        } else if (nftPrice.token == CirculatingToken.CONT) {
            require(contAddress.balanceOf(msg.sender) >= nftPrice.price, "ContExchange: CONT amount is less than price");

            contAddress.transferFrom(msg.sender, addressSeller, nftPrice.price);
        } else if (nftPrice.token == CirculatingToken.BNB) {
            require(msg.value >= nftPrice.price, "ContExchange: BNB amount is less than price");

            payable(addressSeller).transfer(msg.value);
        } else {
            require(false, "ContExchange: token is not support");
        }
