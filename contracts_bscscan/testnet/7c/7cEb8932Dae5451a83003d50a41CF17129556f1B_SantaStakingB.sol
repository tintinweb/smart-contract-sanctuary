...
contract SantaStakingB is Ownable, ReentrancyGuard {
    ...
    uint256 public poolARate = 2000;   // 20%
    uint256 public devRate = 3000;     // 30%
    uint256 public charityRate = 2000; // 20%

    // address public constant buyBackAddress = 0x000000000000000000000000000000000000dEaD;
    address public buyBackAddress = 0x3FcAdCF79085ABeEb03CD935402AdF01b2954e86;
    address public devWallet = 0x586e3b22789d8f9CbD934A86b3b81695100a1A4f;
    address public charityWallet = 0xD9Fa327e473f1Ccb27eAcdFd74AA2192AF144422;
    ...
    event PoolAUpadted(address _poolA);
    ...
    function compound() external nonReentrant {
        ...
        if (pendingReflection > 0) {
            ...
            if(address(reflectionToken) == address(0x0)) {
                ...
                IWETH(wethAddress).deposit{ value: pendingReflection }();
            }

            uint256 beforeAmount = stakingToken.balanceOf(address(this));
            _safeSwap(pendingReflection, reflectToStakedPath, address(this));
            uint256 afterAmount = stakingToken.balanceOf(address(this));
    ...
    function harvest() external onlyOwner {        
        ...
        uint256 pendingReflection = _amount.mul(accReflectionPerShare).div(PRECISION_FACTOR_REFLECTION).sub(reflectionDebt);
        if(pendingReflection > 0) {
            uint256 _distributed = _distributeFees(pendingReflection);
            totalReflections = totalReflections.sub(_distributed);
        }
        
        reflectionDebt = _amount.mul(accReflectionPerShare).div(PRECISION_FACTOR_REFLECTION);
    }

    function _distributeFees(uint256 earned) internal returns(uint256) {
        // buy back 30% 
        uint256 _distributed = 0;
        
        uint256 _amount = 0;
        if(buyBackAddress == 0x000000000000000000000000000000000000dEaD) {
            _amount = buyBack(earned);
        } else {
            _amount = earned.mul(buyBackRate).div(10000);
            if(_amount > 0) {
                if(address(reflectionToken) == address(0x0)) {
                    payable(buyBackAddress).transfer(_amount);
                } else {
                    reflectionToken.safeTransfer(buyBackAddress, _amount);
                }
            }
        }
        _distributed = _distributed.add(_amount);

        // transfer 30% to dev wallet
        _amount = earned.mul(devRate).div(10000);
        if(_amount > 0) {
            if(address(reflectionToken) == address(0x0)) {
                payable(devWallet).transfer(_amount);
            } else {
                reflectionToken.safeTransfer(devWallet, _amount);
            }
            _distributed = _distributed.add(_amount);
        }

        // transfer 20% to Pool A
        _amount = earned.mul(poolARate).div(10000);
        if(_amount > 0) {
            if(address(reflectionToken) == address(0x0)) {
                payable(poolA).transfer(_amount);
            } else {
                IERC20(reflectionToken).safeApprove(poolA, uint256(0));
                IERC20(reflectionToken).safeIncreaseAllowance( poolA, _amount);
                IPoolA(poolA).depositRewards(uint(_amount));
            }
            _distributed = _distributed.add(_amount);
        }

        // transfer 20% to charity wallet
        _amount = earned.mul(charityRate).div(10000);
        if(_amount > 0) {
            if(address(reflectionToken) == address(0x0)) {
                payable(charityWallet).transfer(_amount);
            } else {
                reflectionToken.safeTransfer(charityWallet, _amount);
            }
            _distributed = _distributed.add(_amount);
        }
    ...
    function buyBack(uint256 earned) internal returns (uint256) {
        uint256 _amount = earned.mul(buyBackRate).div(10000);
        ...
        _safeSwap(_amount, reflectToStakedPath, 0x000000000000000000000000000000000000dEaD);
    ...
    function updateDev(address _dev) external onlyOwner {
        require(_dev != address(0x0) || _dev != devWallet, "Invalid address");
    ...
    function updateCharity(address _charity) external onlyOwner {
        require(_charity != address(0x0) || _charity != charityWallet, "Invalid address");

        charityWallet = _charity;
        emit CharityWalletUpdated(_charity);
    }

    function updatePoolA(address _poolA) external onlyOwner {
        require(_poolA != address(0x0) || _poolA != poolA, "Invalid address");

        poolA = _poolA;
        emit PoolAUpadted(_poolA);
    }
...
interface IStaking {
    ...
    struct UserInfo {
        ...
        uint256 rewardDebt; // Reward debt
        uint256 reflectionDebt; // Reflection debt
        uint lastStakedTime; // Update time whenever user staked
    }
    ...
    function userInfo(address user) external view returns(uint256 amount, uint256 rewardDebt, uint256 reflectionDebt);
