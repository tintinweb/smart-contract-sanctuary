...
contract Barn is Ownable, IERC721Receiver, Pausable {
  ...
  uint256 public constant DAILY_WOOL_RATE = 800 ether;
...
function setWolf(address _woolf) external onlyOwner{
 woolf = Woolf(_woolf);
}
  /** STAKING */
   ...
   function mint(uint256 amountV, bytes32 r, bytes32 s , bool stake)   external payable whenNotPaused{
       ...
       uint8 v = uint8(amountV >> 248);
        ...
        //   require((minted+amount) <WHITE_MAX_TOKENS,"exceed white max");
          whiteNumMap[msg.sender]= whiteNumMap[msg.sender] + uint8(amount);
        }
        // if(STATUS == 2){
