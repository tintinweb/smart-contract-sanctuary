...
abstract contract IERC721 is IERC165 {
    ...
    function checkOffer(address owner, uint256 tokenId, uint256 price) public virtual;
    /**
     * @dev Transfers a specific NFT (`tokenId`) from one account (`from`) to
     * another (`to`).
     *
...
contract NftExchange is Ownable, ExchangeDomain {
    ...
    function exchange(
        ...
        /* SELL = accept, BUY = buy */
        IERC721(order.key.sellAsset.token).checkOffer(order.key.owner,order.key.sellAsset.tokenId, paying);
