...
import "./IDOPrediction.sol";
import "./IDOTokenBank.sol";


contract IDOPredictionFactory is PoolCreator{
    // TODO: implement interface and add it 
    TotemToken public immutable totemToken;
    RewardManager public immutable rewardManager;
    IDOTokenBank public immutable idoTokenBank;
    ...
    event PoolCreated(
        ...
        uint256 stakeApr,
        uint256 totemPrizeUnit,
        uint256 idoTokenUnit,
    ...
    function create(
        ...
        rewardManager.addPool(newIDOPrediction);
        idoTokenBank.addIDOPredictionWithToken(newIDOPrediction, idoToken);

        return newIDOPrediction;
    }
...
// TODO: provide an interface to work with
import "./RewardManager.sol";
import "./IDOTokenBank.sol";

contract IDOPrediction is
    Context,
    Ownable
{
    ...
    // TODO: use interfaces instead of contract
    TotemToken public immutable totemToken;
    RewardManager public immutable rewardManager;
    IDOTokenBank public immutable idoTokenBank;
    ...
    // prizeUint is the unit of TOTM that will be given to winners 
    // and multiply by 2 if user have staked more than an amount
    uint256 public immutable totemPrizeUnit;
    // idoTokenUnit is the total amount of the ido token that a winner can buy
    uint256 public immutable idoTokenUnit;
    ...
    ) 
    {
        ...
        totemPrizeUnit = _variables[4];
        idoTokenUnit = _variables[5];
    ...
    function stake(uint256 _amount, uint256 _pricePrediction1, uint256 _pricePrediction2) external {
        ...
        // TODO: we should decide to get the only a part of TOTMs or nothing of it    
        require(
        ...
        _stake(_amount, _pricePrediction1, _pricePrediction2);
    ...
    function _stake(uint256 _amount, uint256 _pricePrediction1, uint256 _pricePrediction2) internal {
        stakers.push(
            _msgSender()
        );
        ...
        predictions[_msgSender()] = StakeWithPrediction({
    ...
    // TODO: I think only purchasers should get the totem prize
    function purchaseIDOToken(uint256 _usdAmount) external {
        require(
            isMatured, 
        ...
        require(
            ...
            "0671 public sale time not reached"
        );
        ...
        require(
            idoAmount > 0, 
            "0672 user hasn't any ido token to withdraw"
        );

        uint256 usdAmount = usdPriceForIDO(idoAmount);
        ...
        // TODO: is it needed to check the purchaser is staked in the pool or not
        ...
        emit PurchaseIDOToken(_msgSender(), _usdAmount, idoAmount);
    ...
    function updateIDOPrices(uint256 _maturingPrice, uint256 _purchasePrice) external onlyPoolCreator {

        require(!idoIsStartDateSet, "0680 ido is scheduled");
    ...
    function _getStakingReturnPerStake(address staker)
        ...
        returns (uint256)
    {
        // TODO: I think we should give no staking return if the user had unstaked
        ...
        returns (uint256)
    {
        if (_rank <= 25) {
            ...
            if (_stakedAmount > 2500*(10**18)) {
                return 2*totemPrizeUnit;
            } else {
                return totemPrizeUnit;
            }
        }
    ...
    // TODO: merge these 2 functions to another function
    ...
    function setIDOScheduleParameters (
        ...
        require(!isMatured || idoStartDate > block.timestamp);
        require(_idoStartDate > block.timestamp);
