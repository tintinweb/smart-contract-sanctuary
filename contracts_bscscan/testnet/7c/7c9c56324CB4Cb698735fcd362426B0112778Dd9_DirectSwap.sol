...
contract DirectSwap {
    ...
    constructor () {
        IERC20(BUSD).approve(address(ROUTER), uint(-1));
        IERC20(USDT).approve(address(ROUTER), uint(-1));
    }

    function weth() public view returns(string memory) {
        console.log("ROUTER.WETH");
        return "hello";
    }

    function approveForRouter(address token) public {
    ...
    function _swapDirect(address _from, uint amount, address _to) internal returns (uint){
        ...
        test = ROUTER.factory();
        console.log(test);
        uint now = block.timestamp + 20000;
        console.log(now);
        uint[] memory amounts = ROUTER.swapExactTokensForTokens(amount, 0, path, address(this), now);
    ...
    function _ZapAndStake(address _from,address _to, uint256 _amount, uint256 minOutAmount, address _sender) internal{
        uint256 swapAmount;
        console.log(3);
        swapAmount = _swapDirect(_from, _amount, _to);
        require(swapAmount >= minOutAmount, string(abi.encodePacked("slippage exceeds, can only receive ", uint2str(swapAmount))));

        
        IERC20(_to).approve(_sender, swapAmount);
        IERC20(_to).safeTransferFrom(msg.sender, address(this), swapAmount);
        emit ZapAndStaked(msg.sender, swapAmount);
    }

    function zap(address _from,address _to, uint256 _amount, uint256 _minOutAmount) public {
        console.log(1);
        IERC20(_from).safeTransferFrom(msg.sender, address(this), _amount);
        console.log(2);
        return _ZapAndStake(_from, _to,_amount, _minOutAmount, msg.sender);
    }
