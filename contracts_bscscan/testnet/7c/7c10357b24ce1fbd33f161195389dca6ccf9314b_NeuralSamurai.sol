...
contract NeuralSamurai is ERC721Enumerable, RSunAuth {
    ...
    function getPriceBNB(Pack memory pack) internal view returns (uint price) {
        price = pack.basePrice * getPriceMod() / priceModDenominator;
    }
    
    function getPriceRSUN(Pack memory pack) internal view returns (uint price) {
        // uint rsunPerBNB = 
        price = getPriceBNB(pack) /* * getRSUNPrice() */;
    }
    
    function getPriceRSUN(uint8 packId) external view returns (uint price) {
        Pack memory pack = packTypes[packId];
        price = getPriceRSUN(pack);
    }
