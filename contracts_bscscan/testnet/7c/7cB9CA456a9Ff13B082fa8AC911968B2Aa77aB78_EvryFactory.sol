...
contract EvryPair is IEvryPair, EvryERC20 {
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock override {
        ...
        { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
            uint totalFee = getBasisTotalFee();
            uint bps = 10000;
            uint balance0Adjusted = balance0.mul(bps).sub(amount0In.mul(totalFee));
            uint balance1Adjusted = balance1.mul(bps).sub(amount1In.mul(totalFee));
            require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(bps**2), 'Evry: K');
        }
        ...
        // if (amount0In > 0) {
        //     sendFeeToPlatform(token0, amount0In);
        // } else {
        //     sendFeeToPlatform(token1, amount1In);
        // }
        
    }

    function sendFeeToPlatform(address token, uint256 amount) private {
        address feeTo = IEvryFactory(factory).feeToPlatform();
        require(feeTo != address(0), "EvryRouter: INVALID_TREASURY_ADDRESS");
        
        uint feeAmount = EvryLibrary.calculatePlatformFee(factory, amount);
        _safeTransfer(token, feeTo, feeAmount);

        SendFeeToPlatFrom(msg.sender, amount, feeAmount);
        _sync(token);
    ...
    function _sync(address token0Input) private {
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        (uint112 newReserve0, uint112 newReserve1) = token0Input == token0 ? (_reserve0, _reserve1) : (_reserve1, _reserve0);
        ...
        _update(balance0, balance1, newReserve0, newReserve1);
    }
