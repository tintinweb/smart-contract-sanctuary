...
contract BattleHeroBreeder is Ownable {
    ...
    function unlockChest(ChestType chestType) public{
        require(hasSuficientTokens(msg.sender, chestType) == true);
        ...
        (bool success, bytes memory result) = address(bhAddress).call{gas: 900000}(abi.encodeWithSignature("burn(uint256)", chestPrices[chestType]));
