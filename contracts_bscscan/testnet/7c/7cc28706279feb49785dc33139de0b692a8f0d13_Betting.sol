...
function getPlayerMatchInfo(address account, uint16 _matchSelected) public view returns (uint256, uint16, uint){
  return (playerInfo[account].amountBet, playerInfo[account].resultSelected, playerInfo[account].numFunders);
  return (playerInfo[_matchSelected].amountBet, playerInfo[_matchSelected].resultSelected, playerInfo[_matchSelected].numFunders);
}
