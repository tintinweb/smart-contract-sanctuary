  ...
  INEFTiLANDToken
{
    ...
    private
  {
    ...
    require( _isSuspended[from] == 0 || _isSuspended[from] > block.timestamp, "Suspended time range");
    /* solhint-disable-next-line not-rely-on-time */
    require( _isSuspended[to]   == 0 || _isSuspended[to]   > block.timestamp, "Suspended time range");
