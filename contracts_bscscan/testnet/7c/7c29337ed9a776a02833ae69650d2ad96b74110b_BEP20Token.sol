...
contract BEP20Token is  IBEP20Fee{
    ...
    function getAllFees() external view returns (feeStructure memory) {
       return allFees[0];
    }  
    function getDivider() external view returns (uint256) {
       return divider;
    }  
    function checkCalculateTheHighwayRobberyTax(uint256 amount) external view returns (uint256[] memory) {
        uint[] memory feesToPay;
        //emit Log("Divider",divider);
        feesToPay[0]=0;
        return (feesToPay);
