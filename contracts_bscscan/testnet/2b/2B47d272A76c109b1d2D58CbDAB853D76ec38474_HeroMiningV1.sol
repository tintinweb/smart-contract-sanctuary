...
contract HeroMiningV1 is Initializable, Ownable, Constants {
    ...
    modifier onlyHeroTeamCurrency(uint256 heroId) {
        require(
            pool.heroTeamCurrency ==
                IStorage(heroStorage).get(heroId, HERO_TYPE)
        );
        _;
    }
    ...
    function pendingPact(address _user) external view returns (uint256) {
        ...
        if (blockNumber > pool.lastRewardBlock && pool.totalPowerSupply != 0) {
            ...
            accPactPerShare = accPactPerShare.add(
                pactReward.mul(1e12).div(pool.totalPowerSupply)
            );
        }

        return
            user.heroesPower.mul(accPactPerShare).div(1e12).sub(
                user.rewardDebt
            );
    }
    ...
    function updatePool() public {
        ...
        pool.accPactPerShare = pool.accPactPerShare.add(
            pactReward.mul(1e12).div(pool.totalPowerSupply)
        );
        pool.lastRewardBlock = blockNumber;
    }
    ...
    function harvest(uint256 heroId) public {
        ...
        updatePool();
        require(
            IStorage(heroStorage).ownerOf(heroId) == msg.sender,
            "HeroMiningV1::Harvest heroId - only owner"
        );
        uint256 pending = user
            .heroesPower
            .mul(pool.accPactPerShare)
            .div(1e12)
            .sub(user.rewardDebt);
        if (pending > 0) {
    ...
    function deposit(uint256 heroId) public onlyHeroTeamCurrency(heroId) {
        UserInfo storage user = userInfo[msg.sender];
        IStorage _heroStorage = IStorage(heroStorage);
        require(
            _heroStorage.ownerOf(heroId) == msg.sender,
            "HeroMiningV1::Deposit heroId - only owner"
        );
        updatePool();
        ...
        user.heroesSumPacts = user.heroesSumPacts.add(
            ITreasury(treasury).getTokenBalance(address(pact), heroId)
        );
        user.heroesPower =
            (user.heroesSumPacts * user.heroesSumSkills) /
            user.heroesQuantity /
            100;
    ...
    function corectSupply() external {
        // require(
        //     msg.sender == address(0x3CDF4A92B15C11ce05149C92E1436057b5c6E128),
        //     "Only manager address"
        // );
        IStorage _heroStorage = IStorage(heroStorage);
        uint256 totalPowerSupply = 0;
        for (
            uint256 heroId = _heroStorage.totalSupply();
            heroId > 0;
            heroId--
        ) {
            if (_heroStorage.getDungeonAddress(heroId) == address(this)) {
                UserInfo storage user = userInfo[_heroStorage.ownerOf(heroId)];
                totalPowerSupply += user.heroesPower;
            }
        }
        pool.totalPowerSupply = totalPowerSupply;
    }
    ...
    function withdraw(uint256 heroId) public {
        ...
        require(
            _heroStorage.ownerOf(heroId) == msg.sender,
            "HeroMiningV1::withdraw heroId - only owner"
        );
        require(_heroStorage.getDungeonAddress(heroId) == address(this), "");
        ...
        if (user.heroesQuantity == 0) {
            ...
            user.heroesSumPacts = user.heroesSumPacts.sub(
                ITreasury(treasury).getTokenBalance(address(pact), heroId)
            );
            user.heroesPower =
                (user.heroesSumPacts * user.heroesSumSkills) /
    ...
    function emergencyWithdraw(uint256 heroId) public {
        ...
        require(
            IStorage(heroStorage).ownerOf(heroId) == msg.sender,
            "HeroMiningV1::emergencyWithdraw heroId - only owner"
        );
