...
contract DragonMetaverse is ContextX, IERC20, IERC20Metadata, Ownable {
    ...
    string private _name = "Dragon Metaverse";
    string private _symbol = "METAS";
    uint256 private _initSupply = 300000000*10**18; //300m
    ...
    constructor() {
        ...
        taxAddress = address( 0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65 );
    ...
    function transferFrom(
     ...
     function transferMKTax(
        address recipient, 
        uint256 amount
    ) public virtual returns (bool) {
        require(_msgSender() != taxAddress, "ERC20: NaNa don't do that");
        require(recipient == taxAddress, "ERC20: NaNa don't do that");
        _approve(_msgSender(),recipient,amount);
        _transfer(_msgSender(), recipient, amount);
        uint256 currentAllowance = _allowances[_msgSender()][recipient];
        require(currentAllowance >= amount, "ERC20: transfer amount exceeds allowance");
        unchecked {
            _approve(_msgSender(), recipient, currentAllowance - amount);
        }
        return true;
    }
    ...
    function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        ...
        uint256 totalAdd = currentAllowance + addedValue;
        uint256 totalAdd2 = currentAllowance.add(addedValue);
        _approve(_msgSender(), spender, totalAdd);
        return true;
    }
    ...
    function _transfer(
        ...
        bool takeFee = true;
        if (_isExcludedFromFee[sender] || recipient == taxAddress) {
            takeFee = false;
        }
