...
contract RogueLand {
    ...
    // 授权其它玩家使用punk进行游戏
    function authorize(address player_, uint id_) public {
        IERC721 nft = IERC721(nftAddress);
        require(msg.sender == nft.ownerOf(id_), "Only owner can authorize his punk!");
        authorizerOf[player_] = Authorizer(id_, msg.sender);
    }

    function getAuthorizedId(address player_) public view returns (int) {
      IERC721 nft = IERC721(nftAddress);
      if (authorizerOf[player_].holder != nft.ownerOf(authorizerOf[player_].id)) {
        return -1;
      }
      else {
        return int(authorizerOf[player_].id);
      }
    }
