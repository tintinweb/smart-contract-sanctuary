...
contract StakeERC20 is Access{
    ...
    address public rewardsAccount = 0xfDcc2805819ddfc3aeeE63eEEc5e5E7C11658AE0;
    ...
    function setRewardAccount(address a) external onlyOwner {
        rewardsAccount = a;
    }
    ...
    function _claim() internal update {
        ...
        if (rewards > 1e18) {
            ...
            BBT20.mint(msg.sender, rewards - invitePart);
            users[msg.sender].totalClaim += rewards - invitePart;

            if (inviter == address(0)){
                BBT20.mint(rewardsAccount, invitePart);
            }else{
    ...
    function getBaseInfo(address account) external view returns (uint dailySupply_, uint blockSupply_, uint totalPledge_, uint userPledge_, uint rewardsToClaim_, uint totalClaim_, uint inviteRewards_, uint todayInviteRewards_) {
        dailySupply_ = dailySupply;
