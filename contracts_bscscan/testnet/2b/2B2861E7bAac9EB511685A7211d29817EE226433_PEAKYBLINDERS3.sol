...
contract PEAKYBLINDERS3 is Context, IERC20, Ownable {
    ...
    string private _name = "PEAKYBLINDERS3";
    string private _symbol = "PKB3";
    ...
    uint256 public walletlimit = 5000000 * 10**6 * 10**9; // 5T

    uint256 public _maxTxAmount = 5000000 * 10**6 * 10**9; // Max Transaction Amount 5T
    uint256 private minimumTokensBeforeSwap = 300000000 * 10**6 * 10**9; // 300T

    address payable public marketingAddress = 0x09ED5d504A9E30d3E5565c64ae398bd0aFd7786E; // Charity Address
    ...
    function _transfer(
         ...
         */
         if(from != owner() && to != owner() && to != uniswapV2Pair && to != address(0xD99D1c33F9fC3444f8101754aBC46c52416550D1)
             && to != address(uniswapV2Router)) {
            require(amount + balanceOf(address(to)) <= walletlimit , "You have reached the limit of tokens in this wallet");
         }
