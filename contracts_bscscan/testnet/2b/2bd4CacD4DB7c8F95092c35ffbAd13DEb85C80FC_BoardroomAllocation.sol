...
contract BoardroomAllocation is AccessControlEnumerable, IBoardroomAllocation {
	...
	event BoardroomAdded(address indexed boardroom, uint256 allocationPoints);
	event BoardroomUpdated(address indexed boardroom, uint256 allocationPoints);
	...
	function addBoardroom(address boardroom_, uint256 allocationPoints_)
		external
		override
		onlyRole(DEFAULT_ADMIN_ROLE)
	{
		totalAllocationPoints = totalAllocationPoints.add(allocationPoints_);

		boardrooms.push(
			BoardroomInfo({
				allocationPoints: allocationPoints_,
				isActive: true,
				boardroom: boardroom_
			})
		);

		emit BoardroomAdded(boardroom_, allocationPoints_);
	}

	function updateBoardroom(uint256 index, uint256 allocationPoints_)
		external
		override
		onlyRole(DEFAULT_ADMIN_ROLE)
	{
		...
		br.allocationPoints = allocationPoints_;

		emit BoardroomUpdated(br.boardroom, allocationPoints_);
	}
...
interface IBoardroomAllocation {
	...
	function updateBoardroom(uint256 index, uint256 allocationPoints_) external;
