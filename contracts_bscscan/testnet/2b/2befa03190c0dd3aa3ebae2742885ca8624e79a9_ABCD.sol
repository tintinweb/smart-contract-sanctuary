...
contract ABCD is Context, IERC20, Ownable {
    ...
    function _transferStandardTest(address sender, address recipient, uint256 tAmount) public payable{ //tAmount = 1000      1
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tDev, uint256 tReferrer, uint256 tReferree) = _getTValues(tAmount, recipient); // 880, 50, 50, 20     0.88 0.05, 0.05, 0.02
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tDev, tReferrer, tReferree, _getRate()); // 1000*10^53, 880*10^53, 50*10^53     10, 8.8, 0.5
        // update log
        emit TransferDetailBeforeReflection(tAmount, tTransferAmount, tFee, tLiquidity, tDev, tReferrer, tReferree);
    ...
    function _getTValues(uint256 tAmount, address to) public view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tDev, uint256 tReferrer, uint256 tReferree, uint256 currentRate) public pure returns (uint256, uint256, uint256) {
    ...
    //ttesting
    function _getValueEst(uint256 tAmount, address to) public view returns (uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tDev, uint256 tReferrer, uint256 tReferree) = _getTValues(tAmount, to);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tDev, tReferrer, tReferree, _getRate());
        return (rAmount, rTransferAmount, rFee);
    }
    ...
    function distributionOfTokensTest1Ref(address account, address referrer_) public view returns 
    (uint256 transferedToken, uint256 referralToken, uint256 reflectionToken,
    uint256 ERtransferedToken, uint256 ERreferralToken, uint256 ERreflectionToken
    )
    {
        uint256[2] memory _referralToken;
        uint256[2] memory _transferedToken;
        uint256[2] memory _reflectionToken;
        (_transferedToken[0], _referralToken[0], _reflectionToken[0]) = distributionOfTokens(account);
        (_transferedToken[1], _referralToken[1], _reflectionToken[1]) = distributionOfTokens(referrer_);
        return (_transferedToken[0], _referralToken[0], _reflectionToken[0],
        _transferedToken[1], _referralToken[1], _reflectionToken[1]
        );
    }
    function distributionOfTokensTest2Con(uint256 doesntmatter) public view returns 
    (uint256 ABCDtransferedToken, uint256 ABCDreferralToken, uint256 ABCDreflectionToken,
     uint256 DEVtransferedToken, uint256 DEVreferralToken, uint256 DEVreflectionToken
    )
    {
        uint256[2] memory _referralToken;
        uint256[2] memory _transferedToken;
        uint256[2] memory _reflectionToken;
        (_transferedToken[0], _referralToken[0], _reflectionToken[0]) = distributionOfTokens(address(this));
        (_transferedToken[1], _referralToken[1], _reflectionToken[1]) = distributionOfTokens(_devAddress);
        return (_transferedToken[0], _referralToken[0], _reflectionToken[0],
        _transferedToken[1], _referralToken[1], _reflectionToken[1]
        );
    }
    function distributionOfTokensTest3LP(address lp, address random) public view returns 
    (uint256 LPtransferedToken, uint256 LPreferralToken, uint256 LPreflectionToken,
    uint256 RANtransferedToken, uint256 RANreferralToken, uint256 RANreflectionToken
    )
    {
        uint256[2] memory _referralToken;
        uint256[2] memory _transferedToken;
        uint256[2] memory _reflectionToken;
        (_transferedToken[0], _referralToken[0], _reflectionToken[0]) = distributionOfTokens(lp);
        (_transferedToken[1], _referralToken[1], _reflectionToken[1]) = distributionOfTokens(random);
        return (_transferedToken[0], _referralToken[0], _reflectionToken[0],
        _transferedToken[1], _referralToken[1], _reflectionToken[1]
        );
    }
    ...
    // TODO: just a view function, we can add it back if we are going to add this API to the web service
    // function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
    //     require(tAmount <= _tTotal, "Amount must be less than supply");

    //     (, uint256 tFee, uint256 tLiquidity, uint256 tDev, uint256 tReferrer, uint256 tReferree) = _getTValues(tAmount, address(0));
