...
contract PresaleFactory {
    ...
    mapping (address => Presale[]) public presaleAddresses;
    mapping (address => mapping (address => Presale)) public presales;
    event MetaCoinCreated(Presale presaleCoin);

    address private tattooAppOwner;

    constructor(address _tattooAppOwner) {
        tattooAppOwner = _tattooAppOwner;
    }

    function createPresale(address tokenAddress) external {
        Presale presaleCoin = new Presale(payable(tattooAppOwner), IERC20(tokenAddress), IERC20(tokenAddress).decimals());
        presaleAddresses[tokenAddress].push(presaleCoin);
        presales[tokenAddress][address(presaleCoin)] = presaleCoin;
        emit MetaCoinCreated(presaleCoin);
    }
    ...
    function startPresale(
        ...
        RouterType router) external {
        presales[tokenAddress][presaleAddress].startPresale(tokenData,
            presaleData,
            socialData,
            refund_type,
            router);
        emit MetaCoinCreated(presales[tokenAddress][presaleAddress]);
    }
...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    RefundType public _refund;
    RouterType public _router;
    ...
    function startPresale(
        ...
        string[] memory socialData,
        RefundType refund_type,
        RouterType router) external icoNotActive() {
        startRefund = false;
        _refundStartDate = 0;
    ...
    function setWalletReceiver(address payable newWallet) external {
        _wallet = newWallet;
    }
