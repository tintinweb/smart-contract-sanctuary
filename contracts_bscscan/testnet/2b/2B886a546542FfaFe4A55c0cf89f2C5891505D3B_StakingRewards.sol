...
contract StakingRewards is  ReentrancyGuard,Ownable {
    ...
    uint256 public holdingTime = 30 days;
    ...
    function _calculateFeeAmount(address from,uint256 amount) internal returns (uint256) {
        ...
        if(block.timestamp.sub(userLastStackedTime[from]) < holdingTime)
        {
            ...
            {
                    stakingToken.safeTransferFrom(from,community,totalFeeAmount);
            }
        }
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        require(_totalSupply < IERC20(tokenAddress).totalSupply().sub(tokenAmount), "Cannot withdraw staking token");
