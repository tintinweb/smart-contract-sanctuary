...
contract INTISwap {
    ...
    uint256 public startDate = 1629291840; // Wednesday, 18 August 2021 13:04:00
    uint256 public totalTokensToSell = 2600 * 10**6 * 10**18; // 2,600,000,000,000 tokens for sell
    uint256 public tokenPerPRE = 1 * 10**18; // 1 INTF = 1 INT(I)
    uint256 public minPerTransaction = 0; // min amount per transaction
    uint256 public maxPerUser = 13 * 10**6 * 10**18; // max amount per user
    ...
    modifier checkSaleRequirements(uint256 buyAmount) {
        require(now >= startDate, "Presale has not started");
        require(saleEnded == false, "Presale has ended");
        require(
