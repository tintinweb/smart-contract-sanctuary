...
contract ItemUpgrading is MarketAuth, EIP712Upgradeable {
  ...
  function _upgradeItem(
    ...
    success = _isSuccess(itemClass, _seed, _acceptedRatio);
...
//    uint _tokenIdFormula,
    ...
    uint tokenIdFormula = _upgradingInfo.tokenIdFormula;

    uint upgradeFeeWhenFailed = _getUpgradeFeeWhenFailed(_ownPriceFeeWhenSuccess, _willKeepMaterialIdIfFailed);

    if (!_success) {
      itemNft.updateOwnPrice(_willKeepMaterialIdIfFailed, _ownPriceFeeWhenSuccess);
...
interface IGameNFT is IERC721Upgradeable {
  ...
  function getHero(uint _tokenId) external view returns (uint8, string memory, uint16, uint, uint8, uint32);
