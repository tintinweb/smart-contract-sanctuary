...
contract ClosedSeaNFTController is ERC721Holder, Ownable, Pausable {
    ...
    function buyToken(address _tokenAddr, uint256 _tokenId) public payable whenNotPaused {
        ...
        if (_tokenAskedWithEther[key]) {
            ...
            uint256 rPrice = price.add(price.mul(ISeaToken(address(quoteErc20)).transferTaxRate()).div(10000));
            IERC20(quoteErc20).transferFrom(_msgSender(), seller, rPrice);
        }
    ...
    function bidToken(address _tokenAddr, uint256 _tokenId, uint256 _price, bool _withEther) public payable whenNotPaused {
        ...
        if (_withEther) {
            ...
            uint256 rPrice = _price.add(_price.mul(ISeaToken(address(quoteErc20)).transferTaxRate()).div(10000));
            quoteErc20.safeTransferFrom(address(_msgSender()), address(this), rPrice);
        }
    ...
    function updateBidPrice(address _tokenAddr, uint256 _tokenId, uint256 _price) public payable whenNotPaused {
        ...
        if (_price > bidEntry.price) {
            if (bidEntry.withEther) {
                ...
                uint256 rPrice = _price.sub(bidEntry.price).add(_price.sub(bidEntry.price).mul(ISeaToken(address(quoteErc20)).transferTaxRate()).div(10000));
                quoteErc20.safeTransferFrom(address(_msgSender()), address(this), rPrice);
            }
        } else {
            if (bidEntry.withEther) {
