...
contract Ibnb_presale is Ownable {
  ...
  uint256 public presale_token_per_BNB = 345;  //pre-sale price (630B/1BNB) AKA (630*10**9*10**9)/10**18
  uint256 public whitelist_token_per_BNB = 500; 
  uint256 public public_token_per_BNB = 345; //public pancake listing
...
// -- Presale flow --
  ...
  function buy() external payable ongoingSale {
    ...
    require(msg.value >= (whiteListed_adr? minBuyBNBPrivate : minBuyBNBPublic) , "Sale: Under min amount"); // 1 bnb min (public presale) 2 bnb min (private presale)
  ...
  function allowanceLeftInBNB() external view returns (uint256) {
    return 2*10**18 - amountBoughtBNB[msg.sender];
  }
