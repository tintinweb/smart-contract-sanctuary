...
contract Prediction is IPrediction, Ownable, Pausable {
     ...
     */
    function buyTicket(uint256 _pid, bool _label, address _referral, uint8 _quantity) public poolExisted(_pid) whenNotPaused(_pid) returns(uint256 _price, uint256 _fee) {
        ...
        (uint256 price, uint256 fee) = _buyTicket(_pid, _label, msg.sender, _referral, _quantity);
        achievementService.updateHistory(msg.sender, _pid, currentEpoch[_pid], price, _label, _quantity);
        return (price, fee);
    }
    ...
    function claimable(uint256 _pid, uint256 _epoch, uint256 _tid, address _user) public view returns (bool) {
        ...
        return 
            specification.oracleCalled && (
                ...
                (specification.lotteryWinningNumber - bought.quantity >= _tid && _tid >= specification.lotteryWinningNumber)
            );
    }
    ...
    function claim(uint256 _pid, uint256 _epoch, address _to, uint256[] memory _tids) external poolExisted(_pid) {
        ...
        for (uint256 i = 0; i < _tids.length; i++) {
            ...
            if (specifications[_pid][_epoch].oracleCalled == true) {
                ...
                uint256 from = _tid - bought.quantity;
                uint256 to = _tid;
     ...
     * @param _to wallet address of receiver
     * @param _amount Amount want to claim
     */
    function claimReferralReward(address _to, uint256 _amount) external {
        require(referralReward[msg.sender] > 0, "PREDICTION: No reward yet");
        require(_amount <= referralReward[msg.sender], "PREDICTION: Reward is not enought");
        referralReward[msg.sender] -= _amount;
        IBEP20(perry).safeTransferFrom(owner(), _to, _amount);

        emit ClaimReferralReward(msg.sender, _to, _amount);
    }
    ...
    function _startRound(uint256 _pid, uint256 _epoch, uint256 _price) internal {
        ...
        _initTicket(_pid, true, pool.ticketNumberInit);
        _initTicket(_pid, false, pool.ticketNumberInit);
    }
    ...
    function _buyTicket(uint256 _pid, bool _label, address _buyer, address _referral, uint256 _quantity) internal returns(uint256 _price, uint256 _fee) {
        uint256 _currentEpoch = currentEpoch[_pid];
        Specification storage specification = specifications[_pid][_currentEpoch];

        (uint256 price, uint256 fee, uint256 newPool, uint256 reduceRate) = _nextTicket(_buyer, _pid, _label, _quantity);
        ...
        _calculateReferralReward(_buyer, _referral, _quantity);
    ...
    function _initTicket(uint256 _pid, bool _label, uint256 _quantity) internal {
        uint256 _currentEpoch = currentEpoch[_pid];
        Pool memory pool = pools[_pid];
        Specification storage specification = specifications[_pid][_currentEpoch];
        address _buyer = owner();
        uint256 price = pool.ticketPriceStart * _quantity;
        uint256 fee = pool.ticketFeeStart * _quantity;
        if (_label == true) {
        ...
        _storgeTicket(_buyer, _pid, _currentEpoch, _label, _quantity, price, pool.ticketFeeStart);
    }
    ...
    function _calculateReferralReward(address _buyer, address _referral, uint256 _quantity) internal {
        ...
        if (referral[_buyer] != address(0x0)) {
            referralReward[referral[_buyer]] += referralRewardPerTicket * _quantity;
        }
    }
...
interface IAchievement {
    ...
    function updateHistory(address user, uint256 pid, uint256 epoch, uint256 price, bool label, uint8 quantity) external;
}
