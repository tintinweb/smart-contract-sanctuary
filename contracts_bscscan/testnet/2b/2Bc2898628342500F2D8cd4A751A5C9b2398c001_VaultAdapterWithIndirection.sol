...
contract VaultAdapterWithIndirection is VaultAdapter {
    using FixedPointMath for FixedPointMath.FixedDecimal;
    using SafeERC20 for IDetailedERC20;
    using SafeERC20 for IyVaultV2;
    using SafeMath for uint256;

    constructor(IyVaultV2 _vault, address _admin) VaultAdapter(_vault, _admin) {
    }
...
contract VaultAdapter is IVaultAdapter {
  ...
  constructor(IyVaultV2 _vault, address _admin) {
    vault = _vault;
    admin = _admin;
    updateApproval();
  ...
  function updateApproval() public {
    ...
    IDetailedERC20(_token).safeApprove(address(vault), uint256(2**256 - 1));
  }
