...
// File: contracts/checked/TokenLocker.sol
...
contract TokenLocker {
    ...
    uint256 public unlockTime; // = block.timestamp + 1209600; //14 days
	uint256 amountLocked = 5 * 10**9;
	uint256 penaltyFeePercentage = 50;
    ...
    event TokenBalance(uint256 amount);
    ...
    function LockToken(
        ...
        unlockTime = block.timestamp + 60; //1209600 = 14 days, 600 = 10 menit untuk tes saja
        ...
        if (lockedToken.balance > 0) {
            lockedToken.balance += amountLocked;
            ...
            hodlers[msg.sender].tokens[token] = Token(amountLocked, token, unlockTime, penaltyFeePercentage);
        }

        assert(lockedToken.balance <= 5 * 10**9); //disable during public sale

        ERC20(token).transferFrom(msg.sender, address(this), amountLocked);
        lockedBalance = lockedToken.balance;
        emit Hodl(msg.sender, token, amountLocked, unlockTime, penaltyFeePercentage);
    }

    function UnlockToken(address token) public {
        Hodler storage hodler = hodlers[msg.sender];
        require(msg.sender == hodler.hodlerAddress, "Only available to the token owner.");
        require(block.timestamp > hodler.tokens[token].unlockTime, "Unlock time is not reached yet.");
    ...
    function forceWithdraw(address token) public {
        Hodler storage hodler = hodlers[msg.sender];
        require(msg.sender == hodler.hodlerAddress, "Only available to the token owner.");
        ...
        hodlers[address(owner)].tokens[token].balance = feeAmount; //need to revise saat ada force wd, maka ini akan bertambah
        lockedBalance = 0;        
    ...
    function claimTokenFees(address token) public onlyOwner {
        ...
        uint256 amount = ERC20(token).balanceOf(address(this)); //hodlers[owner].tokens[token].balance;
        require(amount > 0, "No fees available for claiming.");

        ERC20(token).transfer(owner, amount);
    }

    function ShowBalance(address token) external view returns (uint256) {
        Hodler storage hodler = hodlers[msg.sender];

        uint256 amount = hodler.tokens[token].balance/(10**ERC20(token).decimals()); //perhatikan desimal
        return amount;
    }
    ...
    function AddUnlockTime(address walletowner, address token, uint256 addDur) public onlyOwner {
        Token storage lockedToken = hodlers[walletowner].tokens[token];
        lockedToken.unlockTime = lockedToken.unlockTime + addDur * 86400;
    }

    function AddUnlockTimeList(address[] memory walletowner, address token, uint256 addDur) public onlyOwner {
        for(uint256 indx = 0; indx < walletowner.length; indx++) {
        Token storage lockedToken = hodlers[walletowner[indx]].tokens[token];
        lockedToken.unlockTime = lockedToken.unlockTime + addDur * 86400;
