...
contract Weapons is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    */
    ...
    function _decrementDustSuppliesCheck(address playerAddress, uint32 amountLB, uint32 amount4B, uint32 amount5B) internal view {
    ...
    function _incrementDustSuppliesCheck(address playerAddress, uint32 amountLB, uint32 amount4B, uint32 amount5B) internal view {
        uint32[] memory supplies = getDustSupplies(playerAddress);
...
contract CryptoBlades is Initializable, AccessControlUpgradeable {
    ...
    function initialize(
        ...
        // migrateTo_801f279
        burReforgeCharacterFee = ABDKMath64x64.divu(5, 10); //0.3 usd;

        burBurnWeaponFee = ABDKMath64x64.divu(2, 10);//0.2 usd;
        burReforgeWeaponWithDustFee = ABDKMath64x64.divu(3, 10);//0.3 usd;

        burReforgeWeaponFee = burBurnWeaponFee + burReforgeWeaponWithDustFee;//0.5 usd;
    ...
    int128 public burReforgeCharacterFee;
    ...
    int128 public burBurnWeaponFee;
    int128 public burReforgeWeaponWithDustFee;
    ...
    // event InGameOnlyFundsGiven(address indexed to, uint256 buniAmount);
    ...
    function reforgeCharacter(uint256 reforgeID, uint256 burnID)
        ...
        requestPayBurFromPlayer(burReforgeCharacterFee)
    {
        _payBurContract(msg.sender, burReforgeCharacterFee);
    ...
    function burnWeapon(uint256 burnID)
        ...
        requestPayBurFromPlayer(burBurnWeaponFee)
    {
        _payBurContract(msg.sender, burBurnWeaponFee);
        weapons.burn(burnID);
    }
    ...
    function burnWeapons(uint256[] memory burnIDs)
        ...
        requestPayBurFromPlayer(burBurnWeaponFee.mul(ABDKMath64x64.fromUInt(burnIDs.length)))
    {
        _payBurContract(msg.sender, burBurnWeaponFee.mul(ABDKMath64x64.fromUInt(burnIDs.length)));
    ...
    function reforgeWeaponWithDust(uint256 reforgeID, uint8 amountLB, uint8 amount4B, uint8 amount5B)
        public
        isWeaponOwner(reforgeID)
        requestPayBurFromPlayer(burReforgeWeaponWithDustFee)
    {
        _payBurContract(msg.sender, burReforgeWeaponWithDustFee);
    ...
    function setReforgeCharacterValue(uint256 cents) public restricted {
        burReforgeCharacterFee = ABDKMath64x64.divu(cents, 100);
    }

    function setBurnWeaponValue(uint256 cents) public restricted {
        burBurnWeaponFee = ABDKMath64x64.divu(cents, 100);
    }

    function setReforgeWeaponValue(uint256 cents) public restricted {
        int128 newReforgeWeaponFee = ABDKMath64x64.divu(cents, 100);
        require(newReforgeWeaponFee > burBurnWeaponFee, "Reforge fee must include burn fee");
        burReforgeWeaponWithDustFee = newReforgeWeaponFee - burBurnWeaponFee;
        burReforgeWeaponFee = newReforgeWeaponFee;
    }

    function setReforgeWeaponWithDustValue(uint256 cents) public restricted {
        burReforgeWeaponWithDustFee = ABDKMath64x64.divu(cents, 100);
        burReforgeWeaponFee = burBurnWeaponFee + burReforgeWeaponWithDustFee;
    }
