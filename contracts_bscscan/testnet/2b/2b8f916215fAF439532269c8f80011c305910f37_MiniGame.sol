...
contract MiniGame is SamoyedGame, ReentrancyGuard {
    uint256 public currentRoomNo;
    uint256 public samoyCostPerSeat;
    uint256 public busdCostPerSeat;
    uint8 public immutable maxSeat;
    uint256 public minimumSeatToImmediatelyBeginRound;
    ...
    constructor(
        ...
        IBEP20 _samoyAddress,
        IBEP20 _busdAddress,
        address _cashier
    ) {
        godfatherStorage = _godfatherStorage;
        puppyStorage = _puppyStorage;
        samoyToken = _samoyAddress;
        busdToken = _busdAddress;
        cashier = _cashier;
        samoyCostPerSeat = 200 ether;
        busdCostPerSeat = 100 ether;

        currentRoomNo = 1;
        maxSeat = 16;
        minimumSeatToImmediatelyBeginRound = 4;
    }

    function setCost(uint256 _samoyCostPerSeat, uint256 _busdCostPerSeat) external onlyOwner {
        samoyCostPerSeat = _samoyCostPerSeat;
        busdCostPerSeat = _busdCostPerSeat;
    }

    function setMinimumSeatToImmediatelyBeginRound(uint256 _minimumSeatToImmediatelyBeginRound) external onlyOwner {
    ...
    function getAddressSitQuantityLength(uint256 roomNo, address sitterAddress) public view returns (uint8) {
        // for (uint8 i=0;i<uint8(addressSitQuantity[roomNo][sitterAddress].length);i++){
        //     console.log("At: %d",addressSitQuantity[roomNo][sitterAddress][i]);
        // }
        return uint8(addressSitQuantity[roomNo][sitterAddress].length);
    }

    function sitWithSamoy(uint8 sitQuantity, uint8[] memory sitPosition) external {
        collectFee(samoyToken, cashier, sitQuantity * samoyCostPerSeat);
        sit(sitQuantity, sitPosition);
    }

    function sitWithBUSD(uint8 sitQuantity, uint8[] memory sitPosition) external {
        collectFee(busdToken, cashier, sitQuantity * busdCostPerSeat);
        sit(sitQuantity, sitPosition);
    }
    ...
    function sit(uint8 sitQuantity, uint8[] memory sitPosition) private nonReentrant {
        ...
        for (uint8 i = 0; i < maxSeat; i++) {
            ...
            // console.log("Room : %d, Pos: %d, Addr: %s", currentRoomNo, willSitAtPosition, msg.sender);
    ...
    function createNewRoom() private {
        winner[currentRoomNo] = random();

        //console.log("CurrentRoom : %d, Winner %d", currentRoomNo, winner[currentRoomNo]);
        emit wonEvent(sitAddress[currentRoomNo][winner[currentRoomNo]], currentRoomNo, winner[currentRoomNo]);
        currentRoomNo++;
    }
    ...
    function random() private view returns (uint8) {
        return uint8(uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty))) % maxSeat);
    }

    function claimWonPrize(uint8 wonRoomNo) public nonReentrant {
        require(wonRoomNo < currentRoomNo, "MiniGame: E501");
        require(!isAddressClaimedPrize[wonRoomNo][msg.sender], "MiniGame: E502");
        uint8 sitQuantity = getAddressSitQuantityLength(wonRoomNo, msg.sender);
        require(sitQuantity > 0, "MiniGame: E503");
        ...
        // console.log("Won lose: %d, win: %d", lose, win);
        if (win > 0) {
            mintPuppy();
        }
        if (lose > 0) {
...
abstract contract SamoyedGame is Ownable, ISamoyedGame {
    address internal cashier;
    GodfatherStorage internal godfatherStorage;
    PuppyStorage internal puppyStorage;
    IBEP20 internal samoyToken;

    function getSamoyToken() external view override returns (IBEP20) {
        return samoyToken;
    }
    ...
    function getCashier() external view override returns (address) {
        return cashier;
    }
    ...
    function setCashier(address _cashier) external override onlyOwner {
        cashier = _cashier;
    }
    ...
    function collectFee(
        ...
        TransferHelper.safeTransferFrom(address(_token), msg.sender, _collectorAddress, _amount);
    }
}
...
interface ISamoyedGame {
    ...
    function getSamoyToken() external view returns (IBEP20);
...
contract GodfatherStorage is SamoyedStorage {
    ...
    function bulkMint(
        ...
        require(tcur >= tmin);
        require(tcur + (trv * mintQuantity) - 1 <= tmax);
    ...
    function getBaseTicket(uint256 ticket) public pure returns (uint256) {
        ...
        if (foundTicket) {
            // console.log("ticket : %d", ticket);
            // console.log("mod : %d", ticket % trv[i]);
            // console.log("baseTicket : %d", ticket - (ticket % trv[i]));
            return (ticket - (ticket % trv[i]));
            // return tStore[ticket-(ticket%trv[i])].length;
        } else {
            return 0;
        }
    }
    ...
    // function howManyWon(uint48 baseTicket) public view returns (uint256) {
    //     return tStore[baseTicket].length;
    // }

    // function upgradeToR(address player, uint8 pr,uint256 rc1,uint256 rc2,uint256 ru1,uint256 rr1,uint256 rsr1) public{
    //     //1 Rare + 1 Uncommon + 2 Common = SR
    //     if(pr>=2)
    //     {
    //         require(nStore[rc1]>=tmin[0] && nStore[rc1]<=tmax[0] );
    //         require(nStore[rc2]>=tmin[0] && nStore[rc2]<=tmax[0] );
    //         require(nStore[ru1]>=tmin[1] && nStore[ru1]<=tmax[1] );
    //         _burn(rc1);
    //         _burn(rc2);
    //         _burn(ru1);
    //     }
    //     if(pr>=3){
    //         require(nStore[rr1]>=tmin[2] && nStore[rr1]<=tmax[2] );
    //         _burn(rr1);
    //     }
    //     if(pr>=4){
    //         require(nStore[rsr1]>=tmin[3] && nStore[rsr1]<=tmax[3] );
    //         _burn(rsr1);
    //     }
    //     mint(player,pr);
    // }
    ...
    function getTicketMin() private pure returns (uint256[6] memory) {
        return [uint256(1), 3145728, 6291456, 9437184, 12582912, 15728640];
    }

    function getTicketMax() private pure returns (uint256[6] memory) {
        return [uint256(3145727), 6291455, 9437183, 12582911, 15728639, 16777215];
    }

    function getTicketValue() private pure returns (uint256[6] memory) {
        return [uint256(1), 2, 4, 8, 16, 256];
    }
}

contract GodfatherStorageCaller {
    function mintAirDropItem(
        GodfatherStorage _godfatherStorage,
        uint256 r,
        uint256 quantity
    ) public {
        uint256[] memory res = _godfatherStorage.bulkMint(msg.sender, r, quantity);
        for (uint256 i = 0; i < res.length; i++) {
            // console.log("TokenID: %s", res[i]);
        }
    }
}
...
contract PuppyStorage is SamoyedStorage {
    ...
    constructor() ERC721("Puppy", "PUP") {
        puppyCounter = PuppyCounter({currentPack: 1, nextTicketNumber: 1});
    }
    ...
    function mint(address player) public nonReentrant returns (uint256) {
        ...
        if (_puppyCounter.nextTicketNumber > 255) {
            // Start New Pack
            puppyCounter = PuppyCounter({currentPack: ++_puppyCounter.currentPack, nextTicketNumber: 1});
            // console.log("== Before == pack %d, number %d", --_puppyCounter.currentPack, _puppyCounter.nextTicketNumber);
            // console.log("== After == pack %d, number %d", puppyCounter.currentPack, puppyCounter.nextTicketNumber);
        } else {
