...
contract Crycle is Ownable {
    ...
    function startVote(uint256 beginAt, uint256 countAt, uint256 finishAt) external onlyOwner {
        ...
        voteInfo.push(VoteInfo({
            ...
            countAt: countAt,
            finishAt: finishAt,
            reward: 1000/*sds.balanceOf(address(this))*/
        }));
        ...
        */
        emit NewVoteInfo(_currentVoteId, beginAt, countAt, finishAt, 1000/*sds.balanceOf(address(this))*/);
    }
    ...
    function doCount(address[] memory _crycles) external onlyOwner {
        ...
        for (uint i = 0; i < _crycles.length; i ++) {
            totalVotes = totalVotes.add(crycleVote[voteId][_crycles[i]]);
        }
        if (totalVotes == 0) {
            return;
        }
        for (uint i = 0; i < _crycles.length; i ++) {
            voteReward[voteId][_crycles[i]] = _voteInfo.reward.mul(crycleVote[voteId][_crycles[i]]).div(totalVotes); 
        }
    }
