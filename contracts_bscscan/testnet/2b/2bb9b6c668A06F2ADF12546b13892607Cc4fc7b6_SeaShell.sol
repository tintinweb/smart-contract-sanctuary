...
contract SeaShell is ERC20 {
    ...
    //查看收益
    function getprofit(address account) public view returns (uint256 balance_,uint256 fee_,uint256 blocknumber_) {
        ...
        for(uint i=0;i<recs.length;i++){
            ...
            if(rec.profitBlock < rec.lastRewardBlock){
                ...
                uint256 profit;
                uint256 prop = rec.amount.mul(1e10).div(destroytotalnumber);
                if(blocknumber.sub(rec.startBlock) <= section_block){
                    profit = prop.mul(yield1.mul(blocknumber.sub(rec.profitBlock))).div(1e10);
                    ...
                    if(rec.profitBlock.sub(rec.startBlock) <= section_block){
                        ...
                        profit = prop.mul(yield1.mul(bnumber)).div(1e10);
                        profit = profit.add(prop.mul(yield2.mul(blocknumber.sub(rec.startBlock.add(section_block)))).div(1e10));
                    }else{
                        profit = prop.mul(yield2.mul(blocknumber.sub(rec.profitBlock))).div(1e10);
                    }
                }else{
                    if(rec.profitBlock.sub(rec.startBlock) <= section_block){
                        uint256 bnumber = section_block.sub(rec.profitBlock.sub(rec.startBlock));
                        profit = prop.mul(yield1.mul(bnumber)).div(1e10);
                        profit = profit.add(prop.mul(yield2.mul(section_block)).div(1e10));
                        profit = profit.add(prop.mul(yield3.mul(blocknumber.sub(rec.startBlock.add(section_block.mul(2))))).div(1e10));
                        ...
                        profit = prop.mul(yield2.mul(bnumber)).div(1e10);
                        profit = profit.add(prop.mul(yield3.mul(blocknumber.sub(rec.startBlock.add(section_block.mul(2))))).div(1e10));
                    }else if(rec.profitBlock.sub(rec.startBlock) <= section_block.mul(3)){
                        profit = prop.mul(yield3.mul(blocknumber.sub(rec.profitBlock))).div(1e10);
                    }else{
                        profit = prop.mul(yield3.mul(rec.lastRewardBlock.sub(rec.profitBlock))).div(1e10);
                    }
                }
        ...
        return (balance_,fee_,block.number);
    }
