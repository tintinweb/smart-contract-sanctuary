    ...
    function claim() public allowed {
        ...
        if (
            ...
            if (claimCount[msg.sender] == 0) {
                ...
                remainingCoins = coinBalancePublic[msg.sender]
                    .sub(
                        coinBalancePublic[msg.sender].mul(adminFeePercent).div(
                            100
                        )
                    );
                coinBalancePublic[msg.sender] = remainingCoins
                    .sub(
                        remainingCoins.mul(liquidityPercent).div(
                            100
                        )
                    );
        ...
        if (
            ...
            if (claimCount[msg.sender] == 0) {
                ...
                uint256 remainingCoins;
                remainingCoins = coinBalancePrivate[msg.sender]
                    .sub(
                        coinBalancePrivate[msg.sender].mul(adminFeePercent).div(
                            100
                        )
                    );
                coinBalancePrivate[msg.sender] = remainingCoins
                    .sub(
                        remainingCoins.mul(liquidityPercent).div(
                            100
                        )
        ...
        if (
            ...
            if (claimCount[msg.sender] == 0) {
                ...
                uint256 remainingCoins;
                remainingCoins = coinBalanceSeed[msg.sender]
                    .sub(
                        coinBalanceSeed[msg.sender].mul(adminFeePercent).div(
                            100
                        )
                    );
                coinBalanceSeed[msg.sender] = remainingCoins
                    .sub(
                        remainingCoins.mul(liquidityPercent).div(
                            100
                        )
        ...
        if (tokenPricePublic != 0) {
            if (amountRaisedPublic >= softCapPublic) {
                if (!profitClaim) {
                    ...
                    _adminFee = amountRaisedPublic.mul(adminFeePercent).div(
                        100
                    );
                    ...
                    refundToken = totalTokenPublic
                        ...
                        .sub(liquidityToken)
                        .sub(soldTokensPublic.mul(adminFeePercent).div(100));
                    if (refundToken > 0)
                        token.transfer(tokenOwner, refundToken);
                    remainingCoin = amountRaisedPublic.sub(_adminFee).sub(
                        liquidityCoin
                    );
                    tokenOwnerProfitPublic = remainingCoin
                        .mul(vestingPercent)
                        .div(100);
                    tokenOwner.transfer(tokenOwnerProfitPublic);
                    ...
                    if (getContractcoinBalance() > tokenOwnerProfitPublic) {
                        ...
                        tokenOwner.transfer(getContractcoinBalance());
                    }
                }
                claimCount[address(this)]++;
            } else {
                if (!publicRefundClaimed) {
                    // totalTokenPublic = coinToToken(
                    //     hardCapPublic,
                    //     tokenPricePublic
        ...
        if (tokenPricePrivate != 0) {
            if (amountRaisedPrivate >= softCapPrivate) {
                if (!profitClaim) {
                    ...
                    _adminFee = amountRaisedPrivate.mul(adminFeePercent).div(
                        100
                    );
                    admin.transfer(_adminFee);
                    token.transfer(
                    ...
                    refundToken = totalTokenPrivate
                        ...
                        .sub(liquidityToken)
                        .sub(soldTokensPrivate.mul(adminFeePercent).div(100));
                    if (refundToken > 0)
                        token.transfer(tokenOwner, refundToken);
                    remainingCoin = amountRaisedPrivate.sub(_adminFee).sub(
                        liquidityCoin
                    );
                    tokenOwnerProfitPrivate = remainingCoin
                        .mul(vestingPercent)
                        .div(100);
                    tokenOwner.transfer(tokenOwnerProfitPrivate);
                ...
                if (!privateRefundClaimed) {
                    // uint256 numberOfTokens = coinToToken(
                    //     hardCapPrivate,
                    //     tokenPricePrivate
                    // );
        ...
        if (tokenPriceSeed != 0) {
            if (amountRaisedSeed >= softCapSeed) {
                if (!profitClaim) {
                    ...
                    _adminFee = amountRaisedSeed.mul(adminFeePercent).div(100);
                    admin.transfer(_adminFee);
                    token.transfer(
                        admin,
                    ...
                    refundToken = totalTokenSeed
                        .sub(soldTokensSeed)
                        .sub(liquidityToken)
                        .sub(soldTokensSeed.mul(adminFeePercent).div(100));
                    if (refundToken > 0)
                        token.transfer(tokenOwner, refundToken);
                    remainingCoin = amountRaisedSeed.sub(_adminFee).sub(
                        liquidityCoin
                    );
                    tokenOwnerProfitSeed = remainingCoin
                        .mul(vestingPercent)
                        .div(100);
                    tokenOwner.transfer(tokenOwnerProfitSeed);
