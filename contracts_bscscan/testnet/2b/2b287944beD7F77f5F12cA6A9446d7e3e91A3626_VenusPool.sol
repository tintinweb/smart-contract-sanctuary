...
contract VenusPool is BasePool {

    IVenus private _venus;
    constructor( IBEP20 assetToken,
        ...
        address platformFeeAddress) 
            BasePool(assetToken,
                egoToken,
                initialVEGORate, 1000, 1e12, platformFeeAddress)
        {
        _venus = venus;
        assetToken.approve(address(_venus), MAX_INT);
    }

    function _stakeAsset(uint256 assetStakeAmount, StakerInfo memory stakerInfo, CelebrityInfo memory celebrityInfo) internal virtual override{
        require(_assetToken.transferFrom(msg.sender, address(this), assetStakeAmount));
        //stake on venus  vtoken = actualMintAmount / exchangeRate
        uint256 vBalance = _venus.balanceOf(address(this));
        require(_venus.mint(assetStakeAmount) == 0, "stake: mint fail");
        vBalance = _venus.balanceOf(address(this)) - vBalance;

        stakerInfo.assetBalanceInfo.amount += assetStakeAmount;
        stakerInfo.assetBalanceInfo.vBalance += vBalance;
       
        celebrityInfo.assetBalanceInfo.amount += assetStakeAmount;
        celebrityInfo.assetBalanceInfo.vBalance += vBalance - _getExitRatio(vBalance);
    }
    
    function _celebrityClaimAsset(address celebrity, CelebrityInfo memory celebrityInfo)  internal virtual override {
        ClaimAmount memory venusClaim = calculateCelebrityAssetClaimAmount(celebrity);
        if (venusClaim.claimAmount > 0)
        {
            celebrityInfo.assetBalanceInfo.vBalance -= venusClaim.vClaimAmount;

            uint256 balance = _assetToken.balanceOf(address(this));
            require(_venus.redeem(venusClaim.vClaimAmount) == 0, "unstake: redeem error");
            balance = _assetToken.balanceOf(address(this)) - balance;

            uint256 systemReward = balance * PLATFORM_FEE / DEFAULT_BASE;
            require(_assetToken.transfer(celebrity, balance - systemReward));
            require(_assetToken.transfer(_platformFeeAddress, systemReward));
        }
    }

    function _unstakeAsset(StakerInfo memory stakerInfo, CelebrityInfo memory celebrityInfo, uint256 amount) internal virtual override {
        //-------------venus unstake--------------
        uint256 venusRate = _venus.exchangeRateStored();

        //stake on venus  vtoken = actualMintAmount / exchangeRate
        // actualMintAmount = v* exchangeRate;
        uint256 avalibleVenusWithdrawal = stakerInfo.assetBalanceInfo.vBalance * venusRate / 1e18;
        uint256 originalWithExitFee = stakerInfo.assetBalanceInfo.amount + _getExitRatio(stakerInfo.assetBalanceInfo.amount);
        if(amount > stakerInfo.assetBalanceInfo.amount)
            amount = stakerInfo.assetBalanceInfo.amount;
        if(amount < originalWithExitFee){
            originalWithExitFee = amount +  _getExitRatio(amount);
        }
        uint256 celebrityDecreaseVBalance = 0;

        uint256 balance = _assetToken.balanceOf(address(this));
        if (avalibleVenusWithdrawal >= originalWithExitFee) {
            celebrityDecreaseVBalance = (originalWithExitFee * DEFAULT_OFFSET) * 1e18 / venusRate / DEFAULT_BASE;
        }
        else {
            amount = stakerInfo.assetBalanceInfo.amount;
            celebrityDecreaseVBalance = stakerInfo.assetBalanceInfo.vBalance;
        }
        require(_venus.redeem(celebrityDecreaseVBalance) == 0, "unstake: redeem error");
        balance = _assetToken.balanceOf(address(this)) - balance;

        //return funds in asset to the staker
        require(_assetToken.transfer(msg.sender, balance));
        //-------------venus unstake--------------


        //-----------venus calc---------------
        uint256 vAmount = amount * 1e18 / venusRate;
        if( celebrityInfo.assetBalanceInfo.vBalance > vAmount)
            celebrityInfo.assetBalanceInfo.vBalance -= vAmount;
        else
            celebrityInfo.assetBalanceInfo.vBalance = 0;
        celebrityInfo.assetBalanceInfo.amount -= amount;
        
        if(vAmount > stakerInfo.assetBalanceInfo.vBalance)
            stakerInfo.assetBalanceInfo.vBalance = 0;
        else
            stakerInfo.assetBalanceInfo.vBalance -= celebrityDecreaseVBalance;
        stakerInfo.assetBalanceInfo.amount -= amount;
        //-----------venus  calc---------------
    }

    function _getRate() view internal virtual override returns(uint256){
        return _venus.exchangeRateStored();
    }

    function _getExitRatio(uint256 amount)
    public
    view
    returns (uint256)
    {
        uint256 redeemFee =
        (amount * DEFAULT_OFFSET * IComptroller(_venus.comptroller()).treasuryPercent()) / DEFAULT_BASE / 1e18;
        return redeemFee;
    }
}
...
interface IVenus is IVenusBase {
    function mint(uint mintAmount) external returns (uint);
}

interface IBNBVenus is IVenusBase {
...
abstract contract BasePool is Ownable {
    ...
    uint256 constant MAXapr = 3 * DEFAULT_BASE;
    ...
    // In pool assets
    uint256 private _totalVEGOInCirculation;
    ...
    //rate wei asset\ego rate
    function setEgoAssetRate(uint256 rate) external onlyOwner {
        AssetRate = rate;
    }
    ...
    function stake(
        ...
        _stakeAsset(assetStakeAmount, stakerInfo, celebrityInfo);
        uint256 egoAmount = assetStakeAmount * AssetRate * DecimalAdjustment / DEFAULT_BASE;
    ...
    function celebrityClaim(address celebrity) external {
    ...
    function calculateCelebrityNetAssetReward(address celebrity) view public returns (ClaimAmount memory result){
        CelebrityInfo memory celebrityInfo = CelerbrityInfo[celebrity];
        result = _calculateClaimAmount(celebrityInfo.assetBalanceInfo, _getRate(), 1e18);
        result.claimAmount = result.claimAmount * (DEFAULT_BASE - PLATFORM_FEE) / DEFAULT_BASE;
        return result;
    }

    function calculateCelebrityAssetClaimAmount(address celebrity) view public returns (ClaimAmount memory result){
        CelebrityInfo memory celebrityInfo = CelerbrityInfo[celebrity];
        return _calculateClaimAmount(celebrityInfo.assetBalanceInfo, _getRate(), 1e18);
    }

    function unstake(address celebrity, uint256 assetStakeAmount) external {
        StakerInfo memory stakerInfo = StakersInfo[msg.sender][celebrity];
        CelebrityInfo memory celebrityInfo = CelerbrityInfo[celebrity];
        require(stakerInfo.stake, "unstake: not staked");
        uint256 assetOriginal =stakerInfo.egoBalanceInfoOriginal.amount * DEFAULT_BASE / AssetRate / DecimalAdjustment;
        uint256 egoAmount = 0;

        if(assetStakeAmount > assetOriginal) {
            assetStakeAmount = stakerInfo.egoBalanceInfoOriginal.amount * DEFAULT_BASE / AssetRate / DecimalAdjustment;
        }
        egoAmount = assetStakeAmount * AssetRate * DecimalAdjustment / DEFAULT_BASE;

        // update rate inside
        
        stakerClaim(msg.sender, celebrity);

        if(stakerInfo.egoBalanceInfoOriginal.amount > egoAmount)
            stakerInfo.egoBalanceInfoOriginal.amount -= egoAmount;
        else 
            stakerInfo.egoBalanceInfoOriginal.amount = 0;

        uint256 vChange = egoAmount * DEFAULT_BASE / CurrentRate;
        stakerInfo.egoBalanceInfoOriginal.vBalance -= vChange;

        uint256 stakerEgoChange = egoAmount * (DEFAULT_BASE - stakerInfo.celebrityRewardPercent) / DEFAULT_BASE;
        ...
        uint256 celebrityChange = egoAmount * stakerInfo.celebrityRewardPercent / DEFAULT_BASE;
        ...
        _unstakeAsset(stakerInfo, celebrityInfo, assetStakeAmount);
    ...
    function calculateAPR() public view returns (uint256) {
        if (TotalStaked == 0)
            return MAXapr;
        uint256 totalStakedEgo = TotalStaked * AssetRate * DecimalAdjustment / DEFAULT_BASE;
    ...
    function _stakeAsset(uint256 assetStakeAmount, StakerInfo memory stakerInfo,   CelebrityInfo memory celebrityInfo) internal virtual;
    function _celebrityClaimAsset(address celebrity, CelebrityInfo memory celebrityInfo)  internal virtual;
    function _unstakeAsset(StakerInfo memory stakerInfo, CelebrityInfo memory celebrityInfo, uint256 amount)  internal virtual;
