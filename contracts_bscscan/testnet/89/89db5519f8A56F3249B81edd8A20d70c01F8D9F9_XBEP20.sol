...
abstract contract BEP20Fee is BEP20 {
    ...
    address public _transaction_tax_wallet;
    ...
    function setTransactionTax(uint tx_tax, address wallet) public onlyOwner {
        _transaction_tax = tx_tax;
        _transaction_tax_wallet = wallet;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        address referrer = _referrer[recipient];
        uint total_referral_fees = 0;
        uint ref_fee_paid;
        (referrer, ref_fee_paid) = transferReferralFee(referrer, 70 * tx_fee_amount * _referral_fee / (10 ** _fee_decimal) / 100);
        total_referral_fees = total_referral_fees + ref_fee_paid;
        (referrer, ref_fee_paid) = transferReferralFee(referrer, 10 * tx_fee_amount * _referral_fee / (10 ** _fee_decimal) / 100);
        total_referral_fees = total_referral_fees + ref_fee_paid;
        (referrer, ref_fee_paid) = transferReferralFee(referrer, 10 * tx_fee_amount * _referral_fee / (10 ** _fee_decimal) / 100);
        total_referral_fees = total_referral_fees + ref_fee_paid;
        (, ref_fee_paid) = transferReferralFee(referrer, 10 * tx_fee_amount * _referral_fee / (10 ** _fee_decimal) / 100);
        total_referral_fees = total_referral_fees + ref_fee_paid;
        ...
        super.transfer(_transaction_tax_wallet, amount * _transaction_tax / (10 ** _fee_decimal));  // transaction tax
        super.transfer(recipient, amount - tx_fee_amount - amount * _ubi_tax / (10 ** _fee_decimal) - amount * _transaction_tax / (10 ** _fee_decimal));
        // // // set _referrer as first sender.
        if(_referrer[recipient] == address(0) && _referrer[msg.sender] != recipient && recipient != msg.sender && amount >= _referrer_amount_threshold) {
            _referrer[recipient] = msg.sender;
        }
        // super.transfer(recipient, amount);
        return true;
    }

    function transferReferralFee(address referrer, uint amount) internal returns (address, uint) {
        if(referrer == address(0))
            return (address(0), 0);
        super.transfer(referrer, amount);
        return (_referrer[referrer], amount);
    }
    ...
    function calculateTxFee(uint256 amount) internal view returns (uint256) {
        uint tx_fee = amount * _transaction_fee / (10 ** _fee_decimal);
        if(tx_fee > _transaction_fee_cap) {
            tx_fee = _transaction_fee_cap;
        }
        return tx_fee;
    }
