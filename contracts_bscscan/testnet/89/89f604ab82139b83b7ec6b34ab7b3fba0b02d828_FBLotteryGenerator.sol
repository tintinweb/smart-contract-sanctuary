...
contract FBLotteryGenerator is VRFConsumerBase, Ownable {
    using Address for address payable;
    using SafeERC20 for IERC20;
    
    FBLottery[] public lotteries;
    
    uint256 public entryFee = 1 ether / 100; // 0.01 BNB
    uint256 public maxEntries = 25;
    uint256 public minEntriesToBeWinnable = 25;
    address public treasuryWallet = address(0x54b72d910ADF7b7C9657cFdf0678a22BE05965D0);

    enum LotteryState { NOT_STARTED, STARTED, ENDED }
    LotteryState currentLotteryState = LotteryState.NOT_STARTED;
    uint256 public currentLotteryID;
    uint256 public currentLotteryEndTime;
    FBLottery public currentLottery;
    ...
    function createLottery (uint256 endTime) external payable onlyOwner {
        require (currentLotteryState == LotteryState.NOT_STARTED, "FBLotteryGenerator: Can't start a lottery whilst one is still in progress");
        //require (endTime > block.timestamp + 15 minutes, "FBLotteryGenerator: Lottery must last longer than 15 minutes"); //CHANGEME removed for testing
        require (endTime < block.timestamp + 1 weeks, "FBLotteryGenerator: Lottery must last less than one week");
        
        currentLotteryID = lotteries.length;
        currentLottery = new FBLottery (currentLotteryID, entryFee, minEntriesToBeWinnable, maxEntries);
        lotteries.push(currentLottery);
        currentLotteryEndTime = endTime;
    ...
    function endLottery (uint256 newEndTimeForFailedLottery) external onlyOwner returns (bool){
        require (currentLotteryEndTime > block.timestamp, "FBLotteryGenerator: Cannot end a lottery before its end time");
        require (currentLotteryState != LotteryState.ENDED, "FBLotteryGenerator: Lottery already ended and waiting for result");
        currentLotteryState = LotteryState.ENDED;
        
        if (currentLottery.totalTicketsHeld() < currentLottery.minEntriesToBeWinnable()) {
            //require (newEndTimeForFailedLottery > block.timestamp + 15 minutes, "FBLotteryGenerator: Lottery must last longer than 15 minutes"); //CHANGEME removed for testing
            require (newEndTimeForFailedLottery < block.timestamp + 1 weeks, "FBLotteryGenerator: Lottery must last less than one week");
    ...
    function declareLottery() external onlyOwner {
        require (lastLotteryResultReturned == currentLotteryID, "FBLotteryGenerator: no result returned for current lottery yet");
        require (currentLotteryState == LotteryState.ENDED, "FBLotteryGenerator: cannot declare winner until lottery has ended");
    ...
    function setEntryFee (uint256 newEntryFee) external onlyOwner {
        require (newEntryFee > 0, "FBLotteryGenerator: fee must be > 0");
    ...
    function setMaxEntries (uint256 newMaxEntries) external onlyOwner {
        require (newMaxEntries > 0, "FBLotteryGenerator: max entries per user must be > 0");
    ...
    function setTreasuryWallet (address newTreasuryWallet) external onlyOwner {
        require (newTreasuryWallet != address(0), "FBLotteryGenerator: treasury wallet can't be the zero address");
        emit TreasuryWalletUpdated (treasuryWallet, newTreasuryWallet);
        treasuryWallet = newTreasuryWallet;
    }
    
    function getCurrentLotteryStats() public view returns (address lotteryAddress, uint256 lotteryID, uint256 potentialWinnings, uint256 currentTicketsHeld, uint256 endTime, bool isLive) {
        lotteryAddress = address(currentLottery);
        lotteryID = currentLotteryID;
        potentialWinnings = currentLottery.winnings();
        currentTicketsHeld = currentLottery.totalTicketsHeld();
        isLive = currentLottery.isLotteryLive();
        endTime = currentLottery.endTime();
    }
    
    function getLotteryStats (uint256 lotteryID) public view returns (address lotteryAddress, address winner, uint256 winnings, uint256 ticketsHeld, uint256 endTime, bool isLive) {
        require (lotteryID < lotteries.length, "FBLotteryGenerator: No lottery with that ID exists");
    ...
    function enterLottery() public payable returns (uint256 numberOfTicketsBought) {
        require (currentLotteryEndTime <= block.timestamp, "FBLotteryGenerator: Lottery has expired");
        require (currentLotteryState == LotteryState.STARTED, "FBLotteryGenerator: Lottery can't be entered");
        numberOfTicketsBought = msg.value % currentLottery.entryFee();
        
        require (currentLottery.ticketsHeld(msg.sender) + numberOfTicketsBought <= currentLottery.maxEntries(), "FBLotteryGenerator: user has bought too many tickets");
        
        uint256 rrp = currentLottery.entryFee() * numberOfTicketsBought; 
    ...
    function getRandomNumber (uint256 seed) internal {
        require (keyHash != bytes32(0), "FBLotteryGenerator: Must have valid key hash");
        require (LINK.balanceOf (address(this)) >= linkFee, "FBLotteryGenerator: Not enough LINK tokens");

        latestRequestId = requestRandomness (keyHash, linkFee, seed);
    }
    
    function fulfillRandomness (bytes32 requestId, uint256 randomness) internal override {
        require (latestRequestId == requestId, "FBLotteryGenerator: Wrong requestID");
        lotteryResult = randomness;
        lastLotteryResultReturned = currentLotteryID;
    }
    
    function withdrawTokens (address token) external onlyOwner {
        require (token != address (0), "FBLotteryGenerator: can't withdraw token of zero address");
    ...
    function withdrawEth() external onlyOwner {
        require (address(this).balance > 0, "FBLotteryGenerator: No Eth to withdraw");
        payable(owner()).sendValue (address(this).balance);
    }
    
    function withdrawTokens (address token, uint256 lotteryID) external onlyOwner {
        require (lotteryID < lotteries.length, "FBLotteryGenerator: Lottery ID does not exist");
...
contract FBLottery is Ownable {
    ...
    constructor (uint256 _lotteryID, uint256 _entryFee, uint256 _minEntriesToBeWinnable, uint256 _maxEntries) {
