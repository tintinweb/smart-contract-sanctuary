...
/**
    ...
    */
    ...
    bool public currentlySwapping = false;
    ...
    // Token Limits
    uint256 public _tokenSwapThreshold; // TODO 
    ...
    event Watch3(uint256 _price_impact);
    ...
    */
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (takeFee && to == _pancakeswapV2LiquidityPair) {
            ...
            if (delta > 0 && delta <= TWO_DAYS && _timeLastSell[from] != 0) {
                // blacklist; 
                _isBlacklisted[from] = true;
            } else {
                // Otherwise we reset the time of the last sell 
                _isBlacklisted[from] = false; 
                _timeLastSell[from] = block.timestamp;
            }
            
            // if price impact is more than 2% then tax more fees
            if (_beforeTokenTransfer(amount)) {
        ...
        // Check that the othersPortions of contract balance has reached the threshold required to execute a swap and liquify event
        ...
        // TODO seuil ???
        if (_enableLiquidity && !currentlySwapping && from != _pancakeswapV2LiquidityPair) {
            emit Watch1(from, to);
            swapAndSendToFee(_othersPortions);  // swapAndLiquidify 
        }
    ...
    */
    ...
    function _beforeTokenTransfer(uint256 amount) private returns(bool) { 
        ...
        uint256 _reserveA_new = _reserveA.add(amount);
        uint256 _reserveB_new = _constant.div(_reserveA_new);
        
        uint256 receivedBUSD = _reserveB.sub(_reserveB_new);
        uint256 _new_price   = amount.div(receivedBUSD);
        
        uint256 _price_impact = (uint256(1).sub(_new_price.div(_market_price))).mul(100);
        
        emit Watch3(_price_impact);

        return (_price_impact > 2);
    }
