...
contract Team is AccessControl {
    ...
    uint256 public lockAmount = 170000000000000000000000000; // 170,000,000 HE
    ...
    function addTeam(address[] memory _account, uint256[] memory _amount) public {
        ...
        for(uint256 i =0 ; i < _account.length ; i ++) {
            ...
            debirdMember[address(_account[i])] = debirdMember[address(_account[i])].add(_amount[i]*10**decimalHeroes);
    ...
    function getBalanceToken(address _address) public view returns( uint256 ) {
        ...
        if(block.timestamp > lockTime){
            ...
            amount = debirdMember[address(_address)].mul(claimPeriod).div(vestingPeriod) > withdrawalAmount[address(_address)] ? (debirdMember[address(_address)].mul(claimPeriod).div(vestingPeriod) - withdrawalAmount[address(_address)]) : 0;
        }
        return amount;
    }
