...
library Genes {
    ...
    function random(uint256 _modulus) public view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, msg.sender))) % _modulus;
    }
    ...
    function mix(uint256 _genId1, uint256 _genId2) public view returns (uint256) {
    ...
    function mixGenes(
        ...
        for (uint256 i = 0; i < 6; i++) {
            if (i == 0) {
                ...
                uint256[] memory res = remix(genes, bGenes);
                child[3] = res[0];
                child[5] = res[1];
                child[7] = res[2];
                //
                if (inArray(bGenes, child[3])) {
                    child[4] = 1;
                }

                if (inArray(bGenes, child[5])) {
                    child[6] = 1;
                }

                if (inArray(bGenes, child[7])) {
                    child[8] = 1;
                }
            } else if (i == 1) {
                //mouth
                ...
                child[40] = res[2];
                //
                if (inArray(bGenes, child[36])) {
                    child[37] = 1;
                }

                if (inArray(bGenes, child[38])) {
                    child[39] = 1;
                }

                if (inArray(bGenes, child[40])) {
                    child[41] = 1;
                }
            } else if (i == 2) {
                //hair
                ...
                if (inArray(bGenes, child[9])) {
                    child[10] = 1;
                }

                if (inArray(bGenes, child[11])) {
                    child[12] = 1;
                }

                if (inArray(bGenes, child[13])) {
                    child[14] = 1;
                }
            } else if (i == 3) {
                //hand
                ...
                if (inArray(bGenes, child[15])) {
                    child[16] = 1;
                }

                if (inArray(bGenes, child[18])) {
                    child[19] = 1;
                }

                if (inArray(bGenes, child[21])) {
                    child[22] = 1;
                }
                //
                ...
                if (inArray(bGenes, child[24])) {
                    child[25] = 1;
                }

                if (inArray(bGenes, child[26])) {
                    child[27] = 1;
                }

                if (inArray(bGenes, child[28])) {
                    child[29] = 1;
                }
            } else if (i == 5) {
                // tail
                ...
                if (inArray(bGenes, child[30])) {
                    child[31] = 1;
                }

                if (inArray(bGenes, child[32])) {
                    child[33] = 1;
                }

                if (inArray(bGenes, child[34])) {
                    child[35] = 1;
                }
            }
        }
    ...
    function remix(uint256[6] memory _genes, uint256[] memory _bGenes) private view returns (uint256[] memory) {
        uint256[6] memory geneRatios = [uint256(36), 10, 4, 36, 10, 4];
        uint256[] memory chosen = pickNFromList(3, _genes, geneRatios);
        if (inArray(_bGenes, chosen[0]) && random(100) < 98) {
...
contract Ninneko is Initializable, ERC721Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function breed(uint256 _pet1Id, uint256 _pet2Id) external whenNotPausedBreed validateBreed(_pet1Id, _pet2Id) {
        ...
        uint256 genChid = Genes.mix(pets[_pet1Id].geneId, pets[_pet2Id].geneId);
        uint256 petId = _createPet(_pet1Id, _pet2Id, 2, genChid); // generation is always = 2
    ...
    function putListPetOnSale(uint256[] memory _listPetId, uint256 _petPrice) external whenNotPaused onlyListPetOwner(_listPetId) {
    ...
    function putOnSale(uint256 _petId, uint256 _price) external whenNotPaused onlyPetOwner(_petId) {
    ...
    function buyPet(uint256 _petId) external payable whenNotPaused nonReentrant {
