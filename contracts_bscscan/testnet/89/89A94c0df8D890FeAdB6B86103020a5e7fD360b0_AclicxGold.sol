...
contract AclicxGold {
    ...
    uint256 public aclicx_rate = 50; // 5$
    ...
    event Testing(address indexed addr,uint256 amount);
    ...
    constructor(IBEP20 tokenAdd) public {
        ...
        tarifs.push(Tarif(400, 333, 2500)); //0.83% for 400 days  = 333%
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        // uint256 token_value_in_USD = token_quantity.mul(aclicx_rate).div(10);
        ...
        uint256 token_value_in_USD  = tarifs[_tarif].min_inv;
        uint256  aclicx_eq_to_package = token_value_in_USD.div(aclicx_rate).mul(10);
        emit Testing(msg.sender,aclicx_eq_to_package);

        token.transferFrom(msg.sender, address(this), aclicx_eq_to_package);
        Player storage player = players[msg.sender];
            
        Deposit storage dep = player.deposits[player.deposits.length.sub(1)];
        //uint256 time_end = dep.time + tarif.life_days * 86400;
        require(dep.tarif < _tarif, "Purhase above amount package");
    ...
    function withdraw() payable external {
       ...
       require(amount > 10 * 10 ** 18,'Less then the min withdrawl');
        ...
        amount = amount.div(aclicx_rate).mul(10);
        token.transfer(owner,amount.mul(10).div(100));
        amount = amount.sub(amount.mul(10).div(100));
    ...
    function _send_gi(address _addr, uint256 _amount) private {
        ...
        for(uint8 i = 0; i < GI_PERCENT.length; i++) {
           ...
           if(players[up].structure[0] < i+1) continue;
    ...
    function upgrade_phase(uint256 new_value) public
    {
        require(msg.sender == owner,"unauthorized call");
        aclicx_rate = new_value;
