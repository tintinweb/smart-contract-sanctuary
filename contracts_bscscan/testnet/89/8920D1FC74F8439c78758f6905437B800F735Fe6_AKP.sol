...
contract AKP is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        !(from == address(this) && to == address(pancakePair)) // swap 1 time
            ...
            uint256 liquidityPiece = contractTokenBalance.mul(_liquidityPercent).div(2*10**2);
			uint256 otherPiece = contractTokenBalance.mul(_liquidityPercent).div(2*10**2);
			uint256 rewardPiece = contractTokenBalance.sub(liquidityPiece).sub(otherPiece);
			uint256 swapBNB = rewardPiece.add(otherPiece);
			
			uint256 liquidityPortion = _liquidityPercent.div(2).mul(100).div(_liquidityFee);
...

{
  ...
  "libraries": {
    "contracts/TAR.sol": {
      "Utils": "0x67c5a260545cf72498358bc2beacb399af8ef15b"
    }
  }
}