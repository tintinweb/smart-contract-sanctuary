...
/*
	...
	function getterAddress(uint256 userId) public view returns( address) {			
	...
	function getterUser4(address userAddress) public view returns(uint256,uint256, uint256,uint256) {
			if (userAddress==msg.sender) {
			return (
			users[userAddress].userId,users[userAddress].promoTokensTotal,users[userAddress].tokensMissed,users[userAddress].tokensEarned
			);	
			}
