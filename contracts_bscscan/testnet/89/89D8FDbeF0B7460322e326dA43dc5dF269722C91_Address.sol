...
contract TheRewardToken is Context, IERC20, Ownable {
    ...
    address payable public taxAddress =
        payable(0x449Ee4496EB3bCb2ed5e61cd0d3B5508D4A69800);
    ...
    uint256 private _tTotal = 1 * 10**12;
    // TODO VALIDATE
    uint256 private _rTotal = 1 * 10**12;
    uint256 private _tTotalFee;

    string private _name = "The Reward Token";
    string private _symbol = "RWT";
    ...
    uint256 public _maxTransferAmount = 5 * 10**10;
    ...
    function deliver(uint256 tAmount) public {
        ...
        _tTotalFee = _tTotalFee.add(tAmount);
    }
    ...
    function _transfer(
        ...
        bool doSwapAndLiquify = true;
        if (
            _isExcludedFromSwapAndLiquify[to] ||
            _isExcludedFromSwapAndLiquify[from]
        ) {
            doSwapAndLiquify = false;
        }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddress(
            taxAddress,
            transferredBalance.mul(_sellTaxFee + _sellLiquidityFee).div(100)
        );
    }
    ...
    function _tokenTransfer(
        ...
        bool isSenderExcludedFromFee = _isExcludedFromFee[sender];
        bool isRecipientExcludedFromFee = _isExcludedFromFee[recipient];

        if (isSenderExcludedFromFee && isRecipientExcludedFromFee) {
    ...
    function setMaxTransferAmount(uint256 maxTxAmount) external onlyOwner {
        _maxTransferAmount = maxTxAmount;
    }
    ...
    function transferToAddress(address payable recipient, uint256 amount)
        private
    {
