...
library PancakeLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'e995614e43fce4f1d5d0fec0b2feb018247061d6ef044fa9200b18dca752e66b'   // Change to INIT_CODE_PAIR_HASH of Pancake Factory
            ))));
    }
...
contract PancakeRouter01 is IPancakeRouter01 {
    ...
    event A(address a, uint amountA, uint amountB);
    ...
    event Pair(address token_address, address sender_address, address pair, uint amount);
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = PancakeLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        emit Pair(path[0], msg.sender,PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        TransferHelper.safeTransferFrom(path[0], msg.sender, PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        _swap(amounts, path, address(this));
        IWETH(WETH).withdraw(amounts[amounts.length - 1]);
        TransferHelper.safeTransferETH(to, amounts[amounts.length - 1]);
