...
contract StarlinkBounty is StarlinkComponent {
    ...
    uint256 public xldRewardMagnitude = 500;

    event BountyClaimed(uint256 xpAmount, uint256 xldAmount);

    constructor(IXLD xld, IStarlinkEngine engine, ILeveling _leveling) StarlinkComponent(xld, engine) {
        setLeveling(_leveling);
        processGas = 5000000;
    }

    function claim() external notUnauthorizedContract {
        ...
        uint256 xpAmount = _xpReward(usedGas);
        if (xpAmount > 0) {
            leveling.grantXp(msg.sender, xpAmount, xpReasonId);
        }
        

        uint256 xldAmount = _xldReward(usedGas);
        if (xldAmount > 0) {
            xld.transfer(msg.sender, xldAmount);
        }

        emit BountyClaimed(xpAmount, xldAmount);
    }
