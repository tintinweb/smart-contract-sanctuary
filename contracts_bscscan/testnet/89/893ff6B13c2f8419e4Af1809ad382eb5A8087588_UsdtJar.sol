...
contract UsdtJar is Context {
    ...
    mapping (address => uint256) private _stakedToken; //stake
    mapping (address => uint256) private _accruedUBXT; //Tracks reward
    mapping (address => uint256) private _stakeEntry; //S naught
    
    //Staking pool
    uint256 public _stakedTokenPool; //T
    ...
    function withdrawAll() external {
        ...
        uint256 reward = _calculateReward(msg.sender);
        _accruedUBXT[msg.sender] = _accruedUBXT[msg.sender].add(reward);
        token.transfer(msg.sender, _stakedToken[msg.sender]);
        _stakedTokenPool = _stakedTokenPool.sub(_stakedToken[msg.sender]);
        _stakedToken[msg.sender] = _stakedToken[msg.sender].sub(_stakedToken[msg.sender]);
    ...
    {
        return _stakedToken[msg.sender];
    }
