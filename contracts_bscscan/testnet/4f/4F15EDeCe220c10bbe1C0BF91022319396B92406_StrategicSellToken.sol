...
/**
 * @title Strategic Sell Poco token 
 * @dev Contract
 * - Validate whitelist seller
 * - Validate timelock
 * @author Poco
 **/
contract StrategicSellToken is VersionedInitializable{
   ...
   */
  function addInvester(address ref, uint256 totalInvest) public onlyAdmin {
    whiteList[ref] = RefInfo(true, totalInvest, totalInvest);
    uint256 lockAmount = totalInvest.mul(8).div(10);
  ...
  function getClaimable(address buyerAddress) public view returns (uint256) {
    ...
    uint256 totalLockAmount = SellLockUser[buyerAddress].lockAmount + SellLockUser[buyerAddress].firstRelease;
   ...
   * @return true if the transfer succeeds, false otherwise
   **/
  function buyTokenByUSD(address recipient, address ref, uint256 usdAmount, address usdToken) public sellEnabled returns (bool) {
    require(whiteList[ref].enable, "Ref invalid");
