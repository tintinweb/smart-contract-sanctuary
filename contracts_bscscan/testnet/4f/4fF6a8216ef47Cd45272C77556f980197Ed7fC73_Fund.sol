...
contract Fund is Crawler, ICrawler {
    ...
    bytes __encodedIPFS;
    mapping (string => bytes) balanceOfFund;
    ...
    function getBalanceOf(string memory _address) external view returns(Balance.BalanceInfo memory){
        return balanceOfFund[_address].parseBalanceInfo();
    }
    ...
    function crawlerCallback(bytes32 _reqId, bytes32 _type, bytes calldata _calldata) external override onlyResolver {
        ...
        if (_calldata.length == 0) {
            ...
            if (_type == BridgePriceOracle) {
                ...
                Balance.Updated memory fundInfo = abi.decode(_calldata, (Balance.Updated));
                balanceOfFund[fundInfo.fundAddress] = fundInfo.balanceData;
            } else {
...
library Balance {
    ...
    function parseBalanceInfo(bytes memory _data) internal pure returns (BalanceInfo memory balanceInfo_) {
        require(_data.length > 0, "parseBalanceInfo: invalid encoded price data");
