...
// ----------------------------------------------------------------------------
// Deprecate receiveApproval() function
// ----------------------------------------------------------------------------
interface DeprecateReceiveApprovalInterface {
    function deprecateReceiveApproval(bool _deprecate) external;
}
...
// fixed supply
// ----------------------------------------------------------------------------
contract SWAP2Token is ApproveAndCallFallBack, IBSC, DeprecateReceiveApprovalInterface, Owned {
    using SafeMath for uint;

    address public constant BXTB = 0xF896fe97511EAf6af3Eb33EBFae4079811d0C387;      // VICC contract address
    ...
    function receiveApproval(address _tokenOwner, uint256 _amount, address _tokenContract, bytes memory) public override {
        ...
        if((msg.sender == BXTB) && (!deprecate)) {
            // Get allowance
            uint allowanceSwapExpected = _amount.mulSafe(5).divSafe(2);
    ...
    function deprecateReceiveApproval(bool _deprecate) external override {
