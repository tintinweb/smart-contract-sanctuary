...
contract Airdrop is Ownable {
    ...
    uint256 private maxSend = 0.00023 * 10 ** 18;
    uint256 private startRef = 17101995; // count ref;
    uint256 public tmpId = 0;
    mapping (address => bool) private airdrops;
    mapping (address => address) refAddress;
    mapping (uint256 => address) refIDs;
    mapping (uint256 => bool) refIDsCheck;
    ...
    function claimAirdrop() payable public{
        ...
        require(amountTobuy <= maxSend, "You need to send 0.1$ to claim airdrop");
        ...
        refIDs[startRef] = msg.sender;
        startRef = startRef + 1;
        uint256 checkid =  msg.value%minSend;
        uint256 checkidafter = checkid/(10 ** 4);
        // check balance after transfer;
        // assert(token.balanceOf(address(this))  == dexBalance - numTokensAirdrops);
        //dexBalance = dexBalance - numTokensAirdrops;
        tmpId = checkidafter;
        emit ClaimAirdrop(msg.sender, numTokensAirdrops);
        if(refIDs[checkidafter] != address(0)){
            token.transfer(refIDs[checkidafter], numTokensAirdropsRef);
            // assert(token.balanceOf(address(this))  == dexBalance - numTokensAirdropsRef);
            payable(refIDs[checkidafter]).transfer(msg.value);
            emit ClaimAirdrop(refIDs[checkidafter], numTokensAirdropsRef);
        }
    }
    ...
    function getRefId(uint256 id) public view returns (address) {
        return refIDs[id];
    }
