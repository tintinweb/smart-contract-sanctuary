...
// MasterChef is the master of MorbexSwap Token (TNDR). He can make TNDR and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accMorbexPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accMorbexPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accMorbexPerShare; // Accumulated TNDRs per share, times 1e12. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
    }

    // The MorbexSwap Token!
    BEXToken public morbex;
    // Dev address.
    address public devAddr;
    // TNDR tokens created per block.
    uint256 public morbexPerBlock;
    // Deposit Fee address
    ...
    uint256 public constant MINIMUM_EMISSION_RATE = 6000 finney;
    ...
    constructor(
        ...
        morbexPerBlock = INITIAL_EMISSION_RATE;
    ...
    function pendingMorbex(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accMorbexPerShare = pool.accMorbexPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier =
                getMultiplier(pool.lastRewardBlock, block.number);
            uint256 morbexReward =
                multiplier.mul(morbexPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accMorbexPerShare = accMorbexPerShare.add(
                morbexReward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(accMorbexPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        morbex.mint(devAddr, morbexReward.div(10));
        morbex.mint(address(this), morbexReward);
        pool.accMorbexPerShare = pool.accMorbexPerShare.add(
            morbexReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accMorbexPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
                safeMorbexTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accMorbexPerShare).div(1e12);
    ...
    // Safe morbex transfer function, just in case if rounding error causes pool to not have enough TNDRs.
    function safeMorbexTransfer(address _to, uint256 _amount) internal {
        uint256 morbexBal = morbex.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > morbexBal) {
            transferSuccess = morbex.transfer(_to, morbexBal);
        } else {
            transferSuccess = morbex.transfer(_to, _amount);
        }
        require(transferSuccess, "safeMorbexTransfer: transfer failed.");
    }
    ...
    function updateEmissionRate() public {
        ...
        require(morbexPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = morbexPerBlock;
        ...
        if (newEmissionRate >= morbexPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = morbexPerBlock;
        morbexPerBlock = newEmissionRate;
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            morbex.mint(referrer, refBonusEarned);
