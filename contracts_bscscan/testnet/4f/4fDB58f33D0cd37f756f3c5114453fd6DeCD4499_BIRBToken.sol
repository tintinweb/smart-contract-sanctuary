...
contract BIRBToken is BEP20, Ownable {
    ...
    modifier when3DaysBetweenLastSnapshot() {
        require((block.timestamp - lastRewardTime) >= 10 minutes, "BIRBToken: not enough days since last snapshot taken.");
        _;
    }
    ...
    function getHatchAmount() public view returns (uint256) {
        if (paused) return 0;
        require((block.timestamp - lastHatchTime) >= 300, "BIRBToken: Already Hatched");
