...
contract PM is IERC721Receiver, Ownable {
    ...
    {
        return sellTokenArray.length;
    }
     
    function getApprovedToken(uint256 _i)
        public
        view
        returns(string memory,uint256 ,address )
    {
       ...
       address   _tokenAddress  ; 
           ...
           address _token =  sellTokenArray[_i] ;
           if(sellToken[_token]){
               _tokenSymbol =  IERC20(_token).symbol();
               _tokenFee  = tokenfee[_token];
               _tokenAddress  = _token;
   
           }
      
       return (_tokenSymbol,_tokenFee,_tokenAddress) ;
