...
// This contract locks coinswap liquidity tokens. Used to give investors peace of mind a token team has locked liquidity
// and that the css tokens cannot be removed from coinswap until the specified unlock date has been reached.
...
contract CoinswapLocker is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;

  ICoinSwapFactory public coinswapFactory;
  ...
  mapping(address => TokenLock[]) public tokenLocks; //map css pair to all its locks
  
  struct FeeStruct {
    uint256 bnbFee; // Small bnb fee to prevent spam on the platform
    IBEPBurn secondaryFeeToken; // CSS token
    uint256 secondaryTokenFee; // optional, CSS token
    uint256 secondaryTokenDiscount; // discount on liquidity fee for burning secondaryToken
    uint256 liquidityFee; // fee on css liquidity tokens
   ...
   * @param _lpToken the css token address
   * @param _amount amount of LP tokens to lock
   * @param _unlock_date the unix timestamp (in seconds) until unlock
   * @param _referral the referrer address if any or address(0) for none
   * @param _fee_in_bnb fees can be paid in bnb or in a secondary token such as CSS with a discount on css tokens
  ...
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _referral, bool _fee_in_bnb, address payable _withdrawer) external payable nonReentrant {
    ...
    // ensure this pair is a css pair by querying the factory
    ICoinSwapPair lpair = ICoinSwapPair(address(_lpToken));
    address factoryPairAddress = coinswapFactory.getPair(lpair.token0(), lpair.token1());
    require(factoryPairAddress == address(_lpToken), 'NOT css');
    ...
    // record the lock for the csspair
