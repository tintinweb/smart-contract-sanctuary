...
contract ContractTokenTest is Context, IBEP20, Ownable {
  ...
  constructor() public {
    ...
    _symbol = "TKTK1";
    _decimals = 18;
    _totalSupply = 200000000000000 * 10 ** 18;
    _totalLock =  160000000000000 * 10 ** 18;
   _statusLock = true;
   _totalAvailable = 40000000000000 * 10 ** 18;
 ...
 function disbursement(address owner,uint256 amount) public returns (bool) {
    _disbursement(owner,amount);
    return true;
  }
 function lockSupply(address owner) public returns (bool) {
    _lock(owner);
    return true;
  }
 function unlockSupply(address owner) public returns (bool) {
    _unlock(owner);
    return true;
  }
 ...
 function _buyToken(address investor, uint256 amount) internal {
    require(investor != address(0), "BEP20: transfer to the zero address");
 ...
 function _disbursement(address owner,uint256 amount) internal {
 ...
 function _lock(address owner) internal {
      require(owner != address(0), "BEP20: approve from the zero address");
     _statusLock = true;
  }
 function _unlock(address owner) internal {
    require(owner != address(0), "BEP20: approve from the zero address");
     _statusLock = false;
  }
