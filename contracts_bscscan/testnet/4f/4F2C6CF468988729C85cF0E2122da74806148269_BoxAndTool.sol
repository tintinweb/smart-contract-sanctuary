...
contract BoxAndTool{
    ...
    mapping(uint256=>address) public supportToken;
    // mapping(uint256=>uint256) public surplusSupply;
    mapping(uint256=>uint256) toolPrices;
    mapping(address=>bool) isBuyBossBox;
    address usdtErc20;
    address mpcErc20;
    address token1155;
    address manager;
    uint256 public totalTrading;
    //mapping(uint256=>uint256) bossRate;
    uint256[5] bossRate=[15,20,30,15,20];
    uint256[5] public bossSupply;
    uint256[4] landRate=[40,30,20,10];
    uint256[4] public landSupply;
    uint256[5] workerRate=[15,20,30,10,25];//请确认
    uint256[5] public workerSupply;
    uint256 number = 1111;
    constructor(){
        manager = msg.sender;
    }
    ...
    function setToolPrices(uint256 [] memory ids,uint256[] memory prices) public onlyManager{
        for(uint i=0; i<ids.length; i++){
                toolPrices[ids[i]] = prices[i].mul(1e17);
        }
    }

    function setBossSupply(uint256[] memory amounts) public onlyManager{
        require(amounts.length<=5,"BoxAndTool:Data wrong");
        for(uint i=0; i<amounts.length; i++){
                bossSupply[i] = amounts[i];
        }
    }

    function setLandSupply(uint256[] memory amounts) public onlyManager{
        require(amounts.length<=4,"BoxAndTool:Data wrong");
        for(uint i=0; i<amounts.length; i++){
                landSupply[i] = amounts[i];
        }
    }

    function setWorkerSupply(uint256[] memory amounts) public onlyManager{
        require(amounts.length<=5,"BoxAndTool:Data wrong");
        for(uint i=0; i<amounts.length; i++){
                workerSupply[i] = amounts[i];
        }
    }
    ...
    function randBoss(uint256 length) public view returns(uint256){
        uint256 random = uint256(keccak256(abi.encodePacked(block.difficulty,block.timestamp)));
        return random%length;
    }

    function geBosstMintAssetId() internal view returns(uint256 id){
        uint256 range = 0;
        for(uint i=0; i<bossRate.length; i++){
            if(bossSupply[i]>0){
                range = range.add(bossRate[i]);
            }
        }
        if(range>0){
            uint256 random = randBoss(range);
            for(uint i=0; i<bossRate.length; i++){
                if(bossSupply[i]>0){
                    uint256 min = 0;
                    uint256 max = bossRate[i];
                    for(uint j=0; j<i; j++){
                        if(bossSupply[j]>0){
                            min = min + bossRate[j];
                            max = max + bossRate[j];
    ...
    function openNftBoxForBoss(uint256 price) public returns(uint256){
        require(isBuyBossBox[msg.sender]==false,"BoxAndTool:Only once");
        ...
        uint256 assetId = geBosstMintAssetId();
        require(assetId>0 && assetId<6, "BoxAndTool:Wrong asset");
        IERC1155(token1155).mintNftForBoxAndTools(msg.sender, assetId, 1);
        bossSupply[assetId-1] = bossSupply[assetId-1].sub(1);
        isBuyBossBox[msg.sender] = true;
        updateTotalTrading(supportToken[1],price);
        return assetId;
    }

    function getLandMintAssetId() internal view returns(uint256 id){
        uint256 range = 0;
        for(uint i=0; i<landRate.length; i++){
            if(landSupply[i]>0){
                range = range.add(landRate[i]);
            }
        }
        if(range>0){
            uint256 random = randBoss(range);
    ...
    function openNftBoxForLand(uint256 price) public  returns(uint256){
        ...
        uint256 assetId = getLandMintAssetId();
        require(assetId>5 && assetId<10, "BoxAndTool:Wrong asset");
        IERC1155(token1155).mintNftForBoxAndTools(msg.sender, assetId, 1);
        landSupply[assetId-6] = landSupply[assetId-6].sub(1);
        updateTotalTrading(supportToken[1],price);
        return assetId;
    }

    function getWorkerMintAssetId() internal view returns(uint256 id){
        uint256 range = 0;
        for(uint i=0; i<workerRate.length; i++){
            if(workerSupply[i]>0){
                range = range.add(workerRate[i]);
            }
        }
        if(range>0){
            uint256 random = randBoss(range);
            for(uint i=0; i<workerRate.length; i++){
                if(workerSupply[i]>0){
                    uint256 min = 0;
                    uint256 max = workerRate[i];
                    for(uint j=0; j<i; j++){
                        if(workerSupply[j]>0){
                            min = min + workerRate[j];
                            max = max + workerRate[j];
                        }
                    }
                    if(min < random && random <= max){
                        id = i+10;
    ...
    function openNftBoxForWorker(uint256 price) public  returns(uint256){
        require(price >= boxOfWorkerPrice, "BoxAndTool:Wrong price");
        require(block.timestamp < workerEndTime, "BoxAndTool:Time wrong");
        uint256 assetId = getWorkerMintAssetId();
        require(assetId>9 && assetId<15,"BoxAndTool:Wrong asset");
        require(IERC20(supportToken[3]).transferFrom(msg.sender,address(this),price),"BoxAndTool:Wrong price");
        IERC1155(token1155).mintNftForBoxAndTools(msg.sender, assetId, 1);
        workerSupply[assetId-10] = workerSupply[assetId-10].sub(1);
        updateTotalTrading(supportToken[3],price);
        return assetId;
    }

    function getToolPrices(uint256 assetId,uint256 amount) public view returns(uint256){
        return toolPrices[assetId].mul(amount);
    }
    ...
    function getBossBoxSurplusSupply() public view returns(uint256){
        uint256 surplus = 0;
        for(uint i=0; i<bossSupply.length; i++){
            surplus = surplus.add(bossSupply[i]);
        }
        return surplus;
    }
    
    function getLandBoxSurplusSupply() public view returns(uint256){
        uint256 surplus = 0;
        for(uint i=0; i<landSupply.length; i++){
            surplus = surplus.add(landSupply[i]);
        }
        return surplus;
    }
    
    function getWorkerBoxSurplusSupply() public view returns(uint256){
        uint256 surplus = 0;
        for(uint i=0; i<workerSupply.length; i++){
            surplus = surplus.add(workerSupply[i]);
        }
        return surplus;
    }
    ...
    //0xcd8AfcbFa9921964ff41FF1BF2469c1B37AC66DD
