...
contract RandomNumberConsumer is VRFConsumerBase {
    ...
    uint256 public i;
    uint256[] public randomNumArray;
    uint256 scrollNum;
    bytes32[]  public requestIds;
    mapping(address => uint256) public randomNumAddressMapping;
     ...
     * Requests randomness 
     */
    function getRandomNumber(address origin, uint256 scrollNumb) public returns (bytes32[] memory requestId, uint256[] memory randomNumArrays ) {
        require(LINK.balanceOf(address(this)) > fee, "Not enough LINK - fill contract with faucet");
        scrollNum = scrollNumb;
        delete randomNumArray;
        for(i=0; i< scrollNumb; i++)
      { 
          requestIds.push(requestRandomness(keyHash, fee));
          randomNumAddressMapping[origin] = randomResult;
          
      }
         return (requestIds, randomNumArray);
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        randomResult = randomness.mod(10000000000000000);
        randomNumArray.push(randomResult);
         id =requestId;
    }
...
contract BuyScrolls is Ownable, RandomNumberConsumer {
    ...
    function buy(
        ...
        uint256 split;
        bytes32[] memory ids;
        uint256[] memory randomNumArray;
        split = _tokenAmount.mul(25).div(100);

       (ids,randomNumArray) = getRandomNumber(msg.sender,_noofScrolls);
        _randomNumbers = randomNumArray;
                
       usersRandomNum[msg.sender] = _randomNumbers;
