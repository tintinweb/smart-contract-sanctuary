...
contract WSPPStore is Context, ERC721Holder, Ownable, Pausable, ReentrancyGuard{
    ...
    function initialize(address feeTokenAddr,address nftAddr) external onlyOwner {
        feeToken = IFeeToken(feeTokenAddr);
        nft = IERC721(nftAddr);
    }
    ...
    function sell(uint256 tokenId, uint256 price) external whenNotPaused {
    ...
    function buy(uint256 tokenId) external whenNotPaused {
        ...
        feeToken.transferFrom(_msgSender(), tokenNFT.owner, tokenNFT.price);
    ...
    function back(address target, uint256 tokenId) external onlyOwner {
        require(_tokenOwners[tokenId].owner == address(0), "NFTStore:token is locked");

        nft.safeTransferFrom(address(this), target, tokenId);
    }

    function getNFT(uint256 tokenId) external view returns(uint256 price, address owner) {
        NFT memory tokenNFT = _tokenOwners[tokenId];
    ...
    function safeUnlock(uint256 tokenId) external whenPaused {
        require(_tokenOwners[tokenId].owner == _msgSender(), "NFTStore: transfer of token that is not own");
        nft.safeTransferFrom(address(this), _msgSender(), tokenId);
    }
