...
contract KAKAAirDrop is Ownable, Divestor {
    ...
    // 1 - KTA
    // 2 - KTN
    mapping(uint8 => address) addrList;
    
    mapping(uint => bool) airdroped;
    ...
    constructor() {
        meta.isOpen = true;
        addrList[0] = 0x26a1BdFa3bb86b2744c4A42EBfDd205761d13a8a;
        addrList[1] = 0x646b31b5dBA095537eE5f101F57062338D3C2ed7;
        addrList[2] = 0xc79b746B58110344454991c146bD2f6fA61dAEaF;
    }
    ...
    function claimERC20(uint airdropId_,  uint8 fromAddrId_, uint amount_, bytes32 r_, bytes32 s_, uint8 v_) onlyBanker isOpen public {
        bytes32 hash =  keccak256(abi.encodePacked(airdropId_, fromAddrId_, amount_));
    ...
    function airdropKTA(uint airdropId_, uint8 fromAddrId_, uint[] calldata cardIds_, uint[] calldata amounts_, bytes32 r_, bytes32 s_, uint8 v_) onlyBanker isOpen public {
        require(cardIds_.length == amounts_.length, "unequal length");
        bytes32 hash =  keccak256(abi.encodePacked(airdropId_, fromAddrId_, cardIds_, amounts_));
        address a = ecrecover(hash, v_, r_, s_);
        // require(a == meta.banker, "Invalid signature");
        require(!airdroped[airdropId_], "already received");
        ...
        //     KTA.mintMulti(_msgSender(), cardIds_[i], amounts_[i]);
        // }
    }

    function airdropKTN(uint airdropId_, uint8 fromAddrId_, uint[] calldata cardIds_, uint[] calldata amounts_, bytes32 r_, bytes32 s_, uint8 v_) onlyBanker public {
        require(cardIds_.length == amounts_.length, "unequal length");
        bytes32 hash =  keccak256(abi.encodePacked(airdropId_, fromAddrId_, cardIds_, amounts_));
        address a = ecrecover(hash, v_, r_, s_);
        // require(a == meta.banker, "Invalid signature");
        ...
        // IKTN(addrList[fromAddrId_]).mintBatch(_msgSender(), cardIds_, amounts_);
    }
}
