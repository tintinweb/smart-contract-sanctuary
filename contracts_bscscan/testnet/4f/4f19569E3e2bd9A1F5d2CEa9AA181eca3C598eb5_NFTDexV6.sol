...
contract ERC721Enumerable is ERC165, ERC721, IERC721Enumerable {
    ...
    function enumerableInit() internal{
         _registerInterface(_INTERFACE_ID_ERC721_ENUMERABLE);
    }
...
contract ERC721Metadata is ERC165, ERC721, IERC721Metadata {
    ...
    function metaDataInit(string memory name, string memory symbol) internal{
        _name = name;
        _symbol = symbol;
...
contract ERC721MetadataMintable is ERC721, ERC721Metadata, MinterRole {
   ...
   function mintWithTokenURI(address to, string memory tokenURI,uint256 quantity, bool flag) internal returns (uint256) {
    ...
    function SetAuthors(uint256 tokenId, address _address) public {
       _tokenAuthors[tokenId] = _address;
    }
...
// File: contracts/NFTDexBinance.sol
...
contract NFTDexV6 is ERC721Full, ERC721MetadataMintable, Ownable {
  IERC20 ALIA;
  ...
  IERC20 BNB;
  IERC20 BUSD;
  mapping (uint256 => uint112) public allowedCurrency;
  constructor() ERC721Full("ALIA", "ALIA") public {
  
  }

  event SellNFT(address indexed from, address nft_a, uint256 tokenId, address seller, uint256 price, uint256 dollarPrice, uint256 royalty, uint112 _currency);
  ...
  event SellNFTNonCrypto( address indexed from, address nft_a, uint256 tokenId, string seller, uint256 price, uint256 dollarPrice, uint112 _currency);
  ...
  //  erc165Init();
  //  metaDataInit("ALIA", "ALIA");
  //  erc721Init();
  //  enumerableInit();
   
    // _ERC721_RECEIVED = 0x150b7a02;
    // blindBoxAddress=0x313Df3fE7c83d927D633b9a75e8A9580F59ae79B;
    BNB = IERC20(0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F);
    BUSD = IERC20(0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47);
  }
  
  function sellNFT(address nft_a,uint256 tokenId, address seller, uint256 price, uint256 priceDollar, uint112 _currency) public{
    require(msg.sender == admin || (msg.sender == seller && ownerOf(tokenId) == seller), "101");
    require(_currency >= 0 && _currency <= 2, "113");
    ...
    allowedCurrency[tokenId] = _currency;
    msg.sender == admin ? transferFromAdmin(seller, tokenId) :  transferFrom(seller, address(this), tokenId) ;
    emit SellNFT(msg.sender, nft_a, tokenId, seller, price, priceDollar,_royality[tokenId], _currency);
  }
  function MintAndSellNFT(address to, string memory tokenURI,uint256 quantity, bool flag, uint256 price, string memory ownerId, uint256 royality, uint256 priceDollar, bool _isArtist, uint112 _currency)  public { 
    uint256 tokenId;
    for(uint256 i = 0; i<quantity; i++){
     tokenId = mintWithTokenURI(to, tokenURI, 1, flag);
     sellNFT(address(this), tokenId, to, price, priceDollar, _currency);
     ...
     if(msg.sender == nonCryptoNFTVault){
      ...
      emit SellNFTNonCrypto(msg.sender, address(this), tokenId, ownerId, price, priceDollar, _currency);
     }
    }
 }
 function setOnAuction(address _contract,uint256 _tokenId, uint256 _minPrice, uint256 priceDollar, uint256 _endTime, uint112 _currency) public {
  ...
  _auctionTokens[_tokenId].seller = msg.sender;
      ...
      allowedCurrency[_tokenId] = _currency;
 ...
 function MintAndAuctionNFT(address to, string memory tokenURI,uint256 quantity, bool flag, address _contract, uint256 _minPrice, string memory ownerId, uint256 _endTime, uint256 royality, uint256 priceDollar, uint112 _currency)  public {
    uint256 _tokenId;
      for(uint256 i = 0; i<quantity; i++)
      {
      _tokenId = mintWithTokenURI(to, tokenURI, 1, flag);
      setOnAuction(_contract, _tokenId, _minPrice, priceDollar, _endTime, _currency);
      if(royality > 0) {
  ...
  function placeBid(uint256 _tokenId, uint256 _amount, bool awardType, address from)  public{
    ...
    // allowedCurrency[_tokenId] == 1 && BUSD.transferFrom(msg.sender, address(this), _amount);
    if(temp.bidAmount > 0){
       ALIA.transfer(temp.bidder, temp.bidAmount);
      // allowedCurrency[_tokenId] == 1 && BUSD.transfer(temp.bidder, temp.bidAmount);
    }
  ...
  function buyNFT(address nft_a,uint256 tokenId, string memory ownerId) public{
        ...
        require(allowedCurrency[tokenId] <= 1, "115");
        require(msg.sender != nonCryptoNFTVault || allowedCurrency[tokenId] == 0, "116");
        uint256 price;
        ...
        IERC20 currency = allowedCurrency[tokenId] == 1 ? BUSD : ALIA;
        if(msg.sender == nonCryptoNFTVault) {
          ...
          ALIA.transferFrom(msg.sender,  _tokenAuthors[tokenId], (price  / 1000) * authorPercentage);
  ...
  function buyNFTPayable(address nft_a,uint256 tokenId, address buyer) public payable {
        fixedSell storage temp = _saleTokens[tokenId];
        require(msg.sender == buyer, "NFT: 101");
        require(allowedCurrency[tokenId] == 2, "117");
        uint256 before_bal = BNB.balanceOf(address(this));
        BNB.deposit.value(msg.value)();
        uint256 after_bal = BNB.balanceOf(address(this));
        require(temp.price == (after_bal - before_bal), "NFT 108");
        ...
        BNB.withdraw((temp.price / 1000) * mainPerecentage);
    	  uint256 newBalance = address(this).balance.sub(initialBalance);
    	  address payable seller = address(uint160(temp.seller));
        seller.transfer(newBalance);
        
        //author fee
        
        initialBalance = address(this).balance;
        BNB.withdraw((temp.price / 1000) * authorPercentage);
        ...
        //platform fee
        initialBalance = address(this).balance;
        BNB.withdraw((temp.price / 1000) * platformPerecentage);
    	  newBalance = address(this).balance.sub(initialBalance);
  ...
  function sellNFTNonCrypto(uint256 tokenId, string memory sellerId, uint256 price, uint256 priceDollar, uint112 _currency) public {
    sellNFT(address(this), tokenId, nonCryptoNFTVault, price, priceDollar, _currency);
    emit SellNFTNonCrypto( msg.sender, address(this), tokenId, sellerId, price, priceDollar, _currency);
  }
  ...
  function withdrawAlia(address to, uint256 amount, string memory userId) public {
    ...
    ALIA.transferFrom(nonCryptoNFTVault, to, amount - ((amount / 100) * 5));
    _nonCryptoWallet[userId] -= amount;
  }
  // function burn(uint256 tokenId) public onlyAdminMinter {
  //       _burn(tokenId);
  //   }
