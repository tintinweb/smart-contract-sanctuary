...
contract AAA_MONKEY is Context, IERC20, Ownable {
    ...
    string private  _symbol             = "G3.5c";

    // 3.5 -- testing GENxBNB exchange 
    // 0 to disable conversion
    // an integer to convert only fixed number of tokens
    uint256 public _fee_dev_convert_limit = _supply_total / 200;
    uint256 public _fee_marketing_convert_limit = _supply_total / 1000;
...
/*  CORE INTERFACE FUNCTION */
    ...
    function ___wallets () public view returns(
        ...
        return ( _fee_reflection, _fee_liquidity, _fee_marketing, _fee_giveaway, _fee_dev, _wallet_marketing, _wallet_dev, _wallet_giveaway);
    }
...
// Reflect Finance core code
    ...
    function _getValues(uint256 tAmount) private view returns (
        ...
        rAmount         = tTransferAmount * _getRate();

        rTransferAmount = rAmount - (tDev + tGiveaway + tLiquidity + tMarketing + tReflection) * _getRate();
...
// Fee & Wallet Related
    ...
    function _fees_to_bnb(uint256 tokensToConvert, address feeWallet) private {
