...
contract KAKAAirDrop is Ownable, Divestor {
    ...
    mapping(uint => bool) public airdroped;
    ...
    constructor() {
        ...
        addrList[1] = 0xC529341E0A13e6D539bCcC41D814eB24a50f35C5;
        addrList[2] = 0x3D37d5118a78509c69D3e3F7d6c707F8979539d3;
        addrList[3] = 0x3D7bDcF5e0Bb389DE1c4D12Ee6a88B90E14c6486;  
    }

    function setAddr(uint8 addrId_, address addr_) public onlyOwner {
        addrList[addrId_] = addr_;
    }
    ...
    event ClaimERC20(uint indexed airdropId, address indexed account);
    event ClaimERC721(uint indexed airdropId, address indexed account);
    event ClaimERC1155(uint indexed airdropId, address indexed account);

    event Debug(bytes32 indexed hash, address indexed account);

    function _claimERC20(uint airdropId_, uint fromAddrId_, uint amount_) private {
        IERC20(addrList[fromAddrId_]).transfer(_msgSender(), amount_ * 1 ether);
        emit ClaimERC20(airdropId_, _msgSender());
    }

    function _claimERC721(uint airdropId_, uint fromAddrId_, uint cardId_, uint tokenId, uint amounts_) private {
        IKTA KTA = IKTA(addrList[fromAddrId_]);
        if (tokenId == 0) {
            KTA.mintMulti(_msgSender(), cardId_, amounts_);
        } else {
            KTA.mintWithId(_msgSender(), cardId_, tokenId);
        }
        emit ClaimERC721(airdropId_, _msgSender());
    }

    function _claimERC1155(uint airdropId_, uint fromAddrId_, uint cardId_, uint  amounts_) private {
        IKTN(addrList[fromAddrId_]).mint(_msgSender(), cardId_, amounts_);
        emit ClaimERC1155(airdropId_, _msgSender());
    }

    function airdrop(uint airdropId_, uint category_, uint fromAddrId_, uint cardId_, uint tokenId, uint amounts_, bytes32 r_, bytes32 s_, uint8 v_) onlyBanker isOpen public { 
        bytes32 hash =  keccak256(abi.encodePacked(airdropId_, category_, fromAddrId_, cardId_, tokenId, amounts_, _msgSender()));
        address a = ecrecover(hash, v_, r_, s_);
        require(a == meta.banker, "Invalid signature");
        require(!airdroped[airdropId_], "already received");
        airdroped[airdropId_] = true;
        emit Debug(hash, a);

        if (category_ == 1) {
            _claimERC20(airdropId_, fromAddrId_, amounts_);
            return;
        } 
        if (category_ == 2) {
        ...
        if (category_ == 3) {
            _claimERC721(airdropId_, fromAddrId_, cardId_, tokenId, amounts_);
            return;
        } 
    }
