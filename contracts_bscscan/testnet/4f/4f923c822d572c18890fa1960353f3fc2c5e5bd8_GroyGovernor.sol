...
contract GroyGovernor is Ownable, ReentrancyGuard {
    ...
    function estimateRewards(uint _initiativeId) external view initiativeExists(_initiativeId, initiativeCount) returns(uint) {
        ...
        // uint accRewardPerShare = pool.accRewardPerShare + reward * 1e12 / pool.currentVotes;
    ...
    function _updatePool(uint endBlock, PoolInfo storage pool, ActionType actionType, uint amount) private {
        uint oldTotal = pool.currentVotes;
        if (actionType == ActionType.WITHDRAW) {
            pool.currentVotes -= amount;
