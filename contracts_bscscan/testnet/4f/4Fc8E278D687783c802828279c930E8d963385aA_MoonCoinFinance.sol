...
contract MoonCoinFinance  {
	...
	// (array elements in double qotes)
		...
		if (getChainID() == 56) { // TBA mainnet
            NATIVE_DECIMALS = 18; 
			INVEST_MIN_AMOUNT = 0.0001 ether;
			TIME_STEP = 1 days;
	...
	function issueTokens(address _userAddress, uint256 _amount) internal { 
		...
		PlatformTokenApp.transfer(msg.sender, noOfTokensToGive); // TBA reentrancy check 1, not possible because of internal function
		}		
		...
		PlatformTokenApp.transferFrom(msg.sender, address(0x000000000000000000000000000000000000dEaD), _amount); // TBA reentrancy check 2, do they want to burn more lol
	...
	function addMyLead(address investor, address referrer) public  {
		...
		//TBA onboarding can start before starting project staking				
	...
	function invest(address investor, address referrer) public payable {
		...
		user.bnbDeposit = user.bnbDeposit.add(msg.value); // TBA reentrancy check 3, need to send more deposits, so we are ok
	...
	function withdrawReferral() public {		
		...
		if(refAmount > 0) {
			...
			msg.sender.transfer(refAmount); // TBA reentrancy check 4, variable updated before call, so ok
		}
		...
		if(user.promoTokens > 0) {
			...
			user.promoTokens = 0;		
			PlatformTokenApp.transfer(msg.sender, claimCoins); // TBA reentrancy check 5, variable updated before call, so ok
		}
	...
	function withdraw(uint256 reinvestOption, uint256 wPercent) public {
		...
		if (reinvestOption==0) {
			...
			msg.sender.transfer(realSend); // TBA reentrancy check 6, variable updated before call, so ok
			...
			user.bnbDeposit = user.bnbDeposit.add(toSend); // TBA reentrancy check 7, variable updated before call, so ok
