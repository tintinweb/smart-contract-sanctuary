...
contract MoonCoinFinance  {
	...
	event CustomRefBonus(uint256 eventId, uint256 timestamp,  address indexed user, address indexed upline, uint256 i, uint256 amount, uint256 promoMultiplier, uint256 tokenBalance, uint256 tokenBalanceFactor);
	event CustomRefBonus1(uint256 eventId, uint256 tmpEarn,uint256 tmpMiss,uint256 tokensEarned,uint256 tokensMissed);
	...
	event CustomBuyBackTokenBurn(uint256 eventId, uint256 timestamp,  address indexed user, uint256 tokenToBurn, uint256 howMuchToBuyAtDex, uint256 buyBackBurned);
			...
			for (uint256 i = 0; i < 10; i++) {
				if (upline != address(0) && upline!= msg.sender) {				
					...
					if(tokenBalanceFactor > i+1 || tokenBalanceFactor >= 5) { // token hold good					
						...
						eventId++;
						emit CustomRefBonus(eventId, block.timestamp, msg.sender,  upline,i, amount, promoMultiplier, tokenBalance, tokenBalanceFactor);
						eventId++;
						emit CustomRefBonus1(eventId,  tmpEarn, tmpMiss, users[upline].tokensEarned, users[upline].tokensMissed);
...
/*
	...
	function tokenBurn() private  {	
		...
		buyBackBurned = buyBackBurned.add(tokenToBurn);
		eventId++;
		emit CustomBuyBackTokenBurn(eventId, block.timestamp, msg.sender, tokenToBurn, howMuchToBuyAtDex, buyBackBurned);
		deadWalletTransfer(tokenToBurn); 
