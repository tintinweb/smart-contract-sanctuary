...
contract SwapGateway is Pausable, Ownable {
    ...
    function swap(uint256 userId) external payable whenNotPaused {
        ...
        uint256 amount = ((bnbValue * _rate) / bnb_modulus) * token_modulus;
        require(bnbValue >= 0.01 ether, "Too small bnb");
        require(
            IBEP20(_token).balanceOf(address(this)) >= lockedBalances + amount,
            "Token sold out"
        );
        emit Swap(msg.sender, userId, bnbValue, amount);

        // Reset lock status when user deposit more
        lockedBalances += amount;
        user2Balances[msg.sender] += amount;
        ...
        user2ReleaseAmount[msg.sender] = amount / 10;
        _release();
    }
