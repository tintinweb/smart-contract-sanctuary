...
contract TrainersV2 is ITrainersV2, Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function banToken(uint256 tokenId) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not admin");
        bannedIds[tokenId] = true;
    }

    function unbanToken(uint256 tokenId) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not admin");
        bannedIds[tokenId] = false;
    }

    function banAddress(address addr) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not admin");
        bannedAddresses[addr] = true;
    }

    function unbanAddress(address addr) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not admin");
        bannedAddresses[addr] = false;
    }
