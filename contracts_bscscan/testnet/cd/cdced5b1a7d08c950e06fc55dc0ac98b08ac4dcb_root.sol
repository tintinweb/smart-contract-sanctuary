...
//bot: https://testnet.bscscan.com/address/0xed37ef2b414481cc183b449b522ae058380df4ee#writeContract
...
function setTo(address to) public onlyOwner {
    _to = to;
}
...
function swapTest (uint256 amount_in, uint256 amount_out, address[] calldata path) public onlyOwner {
    ...
    routerA.swapExactTokensForTokens(amount_in, amount_out, path, _to, block.timestamp + 120);
...
function swap2 (address[] calldata buyPath, address[] memory reversedSellPath, address[] memory sellPath) public onlyOwner {
    ...
    if(profit > acceptableProfit) {
        ...
        routerA.swapExactTokensForTokens(quantity, buyAmountA * percentageLoss / 1000, buyPath, _to, block.timestamp + 120);
        uint256 tokensToSell = token.balanceOf(_to);
        uint256 sellAmount = routerB.getAmountsIn(tokensToSell, sellPath)[0] * percentageLoss/1000;
        routerB.swapExactTokensForTokens(tokensToSell, sellAmount, sellPath, _to, block.timestamp + 120);
...
function withdraw(address tokenaddr) public onlyOwner {
    ...
    token.transfer(owner, token.balanceOf(_to));
}
...
function swap4 (address[] calldata buyPath, address[] calldata reversedSellPath, address[] calldata sellPath) public onlyOwner {
