...
contract Airdrops is IAirdrops {
    ...
    constructor(address _staking, address _admin, address _lpToken) {
    ...
    function setLptoken(address _address) external validation(_address) onlyOperator {
        lpToken = IERC20(_address);
    }

    function setStaking(address _address) external validation(_address) onlyOperator {
    ...
    function depositAssets(address _pool, uint256 _tokenAmount, uint256 _lpAmount)
        external
        override
        payable
        onlyExits(_pool)
    ...
    function claim(
        ...
        require(saleStates[_pool].root != bytes32(0), "pool does not have root");
        ...
        if (_tokenAmount > 0) {
            IERC20(p.token).safeTransfer(_account, _tokenAmount);
        }
        if (_lpAmount > 0) {
    ...
    function takeLocked(address _pool) external onlyExits(_pool) onlyOperator {
        ...
        if (tokenAmount > 0) {
            IERC20(p.token).safeTransfer(admin.wallet(), tokenAmount);
        }
        if (tokenAmount > 0) {
    ...
    function _canApply(IStaking.Tiers _tier, IStaking.LockLvl _level)
        internal
        pure
        returns (bool can)
    {
        if (
            _level == IStaking.LockLvl.THIRD ||
            (uint8(_level) > 0 && uint8(_tier) > 2)
        ) {
            can = true;
        }
    }
