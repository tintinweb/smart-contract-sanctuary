...
import "./SigTokenBsc.sol";

contract CashBackBsc is Ownable {
    address public immutable caller;
    SigTokenBsc public immutable sigToken;

    // caller - address of sig3pool, so contact knows whom to pay and whom to not
    constructor (
        SigTokenBsc _sigToken,
        address _caller
    ) public {
        sigToken = _sigToken;
        caller = _caller;
    ...
    */
    ...
    // the function pays to tx.origin ether/bnb/etc based of cashback settings
    function payCashbackEther(int128 coinId, uint256 amount) external {
...
contract SigTokenBsc is ERC20, Ownable, ERC20Burnable {
    // Maximum number of SIG be minted ever on ethereum main-net,
    // guaranteed by the cryptography of the SIG smart-contract.
    // Bsc mint is limited by that
    uint256 constant MAX_SIG_SUPPLY = 16_816_816 * 1e18;
    ...
    function mint(address _to, uint256 _amount) public onlyMinter {
        require(totalSupply() + _amount <= MAX_SIG_SUPPLY );
        _mint(_to, _amount);
