...
contract AnjiToken is Context, IBEP20, Ownable {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBLiquidity = swapedBNBAmount;

        if (swapedBNBAmount > 0) {
            if (bnbSwapMarketingAmount > 0) {
                amountBNBLiquidity = amountBNBLiquidity - bnbSwapMarketingAmount;

                // Update Marketing BNB Pool
                marketingPool = marketingPool + bnbSwapMarketingAmount;
            }

            if (bnbSwapCharityAmount > 0) {
                amountBNBLiquidity = amountBNBLiquidity - bnbSwapCharityAmount;

                // Update Charity BNB Pool
                charityPool = charityPool + bnbSwapCharityAmount;
            }

            if (bnbSwapBnbAmount > 0) {
                amountBNBLiquidity = amountBNBLiquidity - bnbSwapBnbAmount;

                // Send BNB for Rewards
                IBEP20(WBNB).transfer(address(bnbDistributor), bnbSwapBnbAmount);
    ...
    function claimPools() external {
        if (marketingPool > 0) {
            // Send BNB for Marketing
            IBEP20(WBNB).transfer(address(marketingFeeReceiver), marketingPool);
        }

        if (charityPool > 0) {
            // Send BNB for Charity
            IBEP20(WBNB).transfer(address(charityFeeReceiver), charityPool);
        }
    }
