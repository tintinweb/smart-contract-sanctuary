...
contract KineSwap is AccessControl, Pausable, ERC1155Holder {
    ...
    event RuleUpdated(uint256 tokenId);
    event Swapped(address indexed account, uint256 tokenId, uint256 amount);
    event Merged(address indexed account, uint256 tokenId);
    ...
    function updateAsset(uint256 tokenId, address[] memory tokens, uint256[] memory amounts) external onlyRole(MANAGER_ROLE) {
    ...
    function updateRules(uint256 tokenId, uint256[] memory tokens) external onlyRole(MANAGER_ROLE) {
        rules[tokenId] = tokens;

        emit RuleUpdated(tokenId);
    }
    ...
    function swap(uint256 tokenId, uint256 amount) external whenNotPaused {
        ...
        for (uint256 i = 0; i < data.length; i++) {
            if (data[i].token == WETH) {
                ...
                IERC20(data[i].token).transfer(_msgSender(), data[i].amount * amount);
            }
        }
        totalSupply += amount;
    ...
    function merge(uint256 tokenId, uint256[] memory tokens) external whenNotPaused {
        ...
        emit Merged(_msgSender(), tokenId);
    }

    function withdraw(address payable account, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        Address.sendValue(account, amount);
    }

    function withdrawToken(address token, address account, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
