...
contract BalanceScanner {
  ...
  /**
   * @notice Get the ERC-721 token balance ids from specified contract for a single owner
   * @param owner The address of the token owner
   * @param token The address of the ERC-721 token contract
   * @param balance The balance of the ERC-721 token
   * @return results The ERC-721 token #ids
   */
  function tokenBalanceAll(address owner, address token, uint256 balance) external view returns (Result[] memory results) {
    results = new Result[](balance);

    for (uint256 i = 0; i < balance; i++) {
      bytes memory data = abi.encodeWithSignature("tokenOfOwnerByIndex(address,uint256)", owner, i);
      results[i] = staticCall(token, data, 20000);
    }
  }
