    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(uint256 => uint256) public healthPotionPrices;
    mapping(uint256 => uint256) public healthPotionQuantities;
    mapping(uint256 => uint256) public shieldPotionPrices;
    mapping(uint256 => uint256) public shieldPotionQuantities;
        ...
        returns (bool)
    {
        ...
        require(msg.value >= (commissionInBNB * _amount), "Not enough gas");
    ...
    function buyHealthPotionsWithCWOLF(uint256 _healthPotionType, uint8 _amount)
        external
        payable
        whenNotPaused
        nonReentrant
    {
        require(
            _amount <= healthPotionQuantities[_healthPotionType],
            "Invalid amount"
        );
        uint256 amountCWOLF = Variables(VariablesContractAddress)
            .getDollarsInCWOLF(healthPotionPrices[_healthPotionType]) * _amount;
        ...
        getValuesForHealthPotion(_healthPotionType, _amount);

        healthPotionQuantities[_healthPotionType] -= _amount;
    }

    function buyHealthPotionsWithBNB(uint256 _healthPotionType, uint8 _amount)
        external
        payable
        whenNotPaused
        nonReentrant
    {
        ...
        uint256 dollarsDiscount = healthPotionPrices[_healthPotionType] * discount / 10000;
        uint256 amountInBNB = Variables(VariablesContractAddress)
            .getDollarsInBNB(dollarsDiscount) * _amount;

        payable(commissionWalletAddress).transfer((amountInBNB * _amount));
        payable(msg.sender).transfer(
            msg.value - (amountInBNB * _amount)
        );
    ...
    function getValuesForHealthPotion(uint256 _amount, uint256 _objectType)
        internal
    {
        for (uint256 index = 0; index < _amount; index++) {
            uint256 _tokenId = mint(msg.sender);
            (
                elementType[_tokenId],
                class[_tokenId],
                object[_tokenId],
                value[_tokenId]
            ) = ConsumibleNFTHelper(consumibleNFTHelperContractAddress)
                .generateHealthPotion(_objectType);
            generated[_tokenId] = true;
        }
    }

    function buyShieldPotionsWithCWOLF(uint256 _shieldPotionType, uint8 _amount)
        external
        payable
        whenNotPaused
        nonReentrant
    {
        require(
            _amount <= shieldPotionQuantities[_shieldPotionType],
            "Invalid amount"
        );
        uint256 amountCWOLF = Variables(VariablesContractAddress)
            .getDollarsInCWOLF(shieldPotionPrices[_shieldPotionType]) * _amount;
        ...
        getValuesForShieldPotion(_shieldPotionType, _amount);

        shieldPotionQuantities[_shieldPotionType] -= _amount;
    }

    function buyShieldPotionsWithBNB(uint256 _shieldPotionType, uint8 _amount)
        external
        payable
        whenNotPaused
        nonReentrant
    {
        ...
        uint256 dollarsDiscount = shieldPotionPrices[_shieldPotionType] * discount / 10000;
        uint256 amountInBNB = Variables(VariablesContractAddress)
            .getDollarsInBNB(dollarsDiscount) * _amount;
    ...
    function getValuesForShieldPotion(uint256 _amount, uint256 _objectType)
        internal
    {
        for (uint256 index = 0; index < _amount; index++) {
            uint256 _tokenId = mint(msg.sender);
            (
                ...
                .generateShieldPotion(_objectType);
            generated[_tokenId] = true;
        }
    }
    ...
    function getHealthPotion(uint256 _potionSelected)
        internal view
        returns (
        ...
        )
    {
        return
            ConsumibleNFTHelper(consumibleNFTHelperContractAddress)
                .generateHealthPotion(_potionSelected);
    }
    ...
    function setHealthPotionsPricesAndQuantities(uint256 _object, uint256 _price, uint256 _quantity) external onlyOwner {
        healthPotionPrices[_object] = _price;
        healthPotionQuantities[_object] = _quantity;
    }

    function setShieldPotionsPricesAndQuantities(uint256 _object, uint256 _price, uint256 _quantity) external onlyOwner {
        shieldPotionPrices[_object] = _price;
        shieldPotionQuantities[_object] = _quantity;
    }
