...
contract Vault is ERC20 {
    ...
    function depositQuote(uint256 amount) public {
        ...
        uint256 expectedBase = amounts[2];
        require (maxCap == 0 || _pool + expectedBase < maxCap, "The vault reached the max cap");

        // 1. transfer amount from sender to this vault
        uint256 _before = IERC20(tokenB).balanceOf(address(this));
        IERC20(tokenB).safeTransferFrom(msg.sender, address(this), amount);
        uint256 _after = IERC20(tokenB).balanceOf(address(this));
        amount = _after.sub(_before); // Additional check for deflationary tokens

        // 2. swap tokenB to tokenA
        _before = IERC20(tokenA).balanceOf(address(this));
        _swapPancakeswap(tokenB, tokenA, amount);
        _after = IERC20(tokenA).balanceOf(address(this));
        amount = _after.sub(_before);

        // 3. calculate share and send back xUBXT
        uint256 shares = 0;
        if (totalSupply() == 0) {
            shares = amount;
        } else {
    ...
    function withdraw(uint256 shares) public  {
        ...
        // 2. burn these shares from the sender wallet
        _burn(msg.sender, shares);

        // 3. if the vault is in open position (status == 1), update the soldAmount (remove the amount related to the shares)
        if (position == 1) { // the trade is in open position
            ...
            if (_profit > percentMax) {
                ...
                uint256 profitAmount = amountB.mul(_profit.sub(percentMax)).div(_profit);
                uint256 feeAmount = takePerformanceFeesFromTokenB(profitAmount);
                amountB = amountB.sub(feeAmount);
            }

            soldAmount = soldAmount.sub(thisSoldAmount);
        }
        
        // 4. send tokenA and B to the sender wallet
        if (amountA > 0) {
    ...
    function takePerformanceFeesFromTokenB(uint256 amount) private returns(uint256) {
        ...
        uint256 _total = stakersAmount.add(devAmount).add(burnAmount);
        uint256 _before = IERC20(ubxt).balanceOf(address(this));
        uint256 _tokenbBefore = IERC20(tokenB).balanceOf(address(this));
        _swapPancakeswap(tokenB, ubxt, _total);
        uint256 _after = IERC20(ubxt).balanceOf(address(this));
        uint256 _tokenbAfter = IERC20(tokenB).balanceOf(address(this));
