...
/**
 * @title PredictionCAKE
 */
contract PredictionCAKE is Ownable, Pausable, ReentrancyGuard {
     ...
     * @param _token: Wagering / Claim token
     ...
     * @param epoch: epoch
     * @param betAmount: Wagers with a ERC20 token instead of the native BNB token
     */
    function betBear(uint256 epoch, uint256 betAmount) external whenNotPaused nonReentrant notContract {
        require(epoch == currentEpoch, "Bet is too early/late");
        require(_bettable(epoch), "Round not bettable");
        require(betAmount >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(token.balanceOf(msg.sender) >= betAmount, "You don't have enough balance to wager");
     ...
     * @param epoch: epoch
     */
    function betBull(uint256 epoch, uint256 betAmount) external whenNotPaused nonReentrant notContract {
        require(epoch == currentEpoch, "Bet is too early/late");
        require(_bettable(epoch), "Round not bettable");
        require(betAmount >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(token.balanceOf(msg.sender) >= betAmount, "You don't have enough balance to wager");
    ...
    function claim(uint256[] calldata epochs) external nonReentrant notContract {
        ...
        if (reward > 0) {
            _safeTransferTokens(address(msg.sender), reward);
        }
    }
    ...
    function claimTreasury() external nonReentrant onlyAdmin {
        ...
        treasuryAmount = 0;
        _safeTransferTokens(adminAddress, currentTreasuryAmount);
    ...
    */
     ...
     */
    function _safeTransferTokens(address to, uint256 value) internal {
