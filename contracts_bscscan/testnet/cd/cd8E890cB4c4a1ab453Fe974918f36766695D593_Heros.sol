...
contract Heros is Ownable, ERC721URIStorage {
  ...
  mapping(uint256 => BoxInfo) public boxs;
  ...
  function setBox(uint256 typeBox, uint256 priceBox, string memory boxUri) external onlyOwner {
    BoxInfo memory newBox = BoxInfo({price:priceBox,boxURI:boxUri});
    boxs[typeBox] = newBox;
  }

  function boxPrice(uint256 typeBox) public view returns (uint256){
    return boxs[typeBox].price;
  }

  function mint(uint256 typeBox, uint256 amount) public {
