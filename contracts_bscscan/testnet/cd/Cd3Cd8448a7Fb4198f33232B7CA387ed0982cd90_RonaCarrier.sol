...
contract RonaCarrier is OwnableContext {
    ...
    constructor(address carrierTokenAddress, uint256 initialMaxPurchaseRate, address claimableTokenAddress) {
        ...
        _maxPurchaseRate = initialMaxPurchaseRate*_carrierTokenDecimalFactor;
    ...
    function carry(address from, address to, uint256 amount) external returns (bool) {
        ...
        _carrierToken.transfer(to, amount - (serviceFeeAmount + charityFeeAmount + holdersFeeAmount + liquidityPoolFeeAmount) + _reflect(holdersFeeAmount, from));
    ...
    function _purchaseRate() internal view returns (uint256) {
        uint256 currentPurchaseRate = _carrierToken.balanceOf(address(this)).sub(_reflectionPoolHoldings).mul(_liquidityTokenDecimalFactor).div(address(this).balance).div(_carrierTokenDecimalFactor);
    ...
    function _purchaseRateDeducted(uint256 deduction) internal view returns (uint256) {
        uint256 currentPurchaseRate = _carrierToken.balanceOf(address(this)).sub(_reflectionPoolHoldings).mul(_liquidityTokenDecimalFactor).div(address(this).balance.sub(deduction)).div(_carrierTokenDecimalFactor);
        if(currentPurchaseRate > _maxPurchaseRate){
            return _maxPurchaseRate;
        } else {
            return currentPurchaseRate;
    ...
    function liquidate(uint256 tokensToLiquidate) public returns (bool) {
        ...
        _carrierToken.transferFrom(_msgSender(), address(this), tokensToLiquidate - _reflect(holdersFeeAmount, _msgSender()));
    ...
    function _liquidationRate() internal view returns (uint256) {
        return address(this).balance.mul(_carrierTokenDecimalFactor).div(_carrierToken.totalSupply().sub(_carrierToken.balanceOf(address(this)).sub(_reflectionPoolHoldings))).div(_liquidityTokenDecimalFactor);
    }

    function _liquidationRateDeducted(uint256 deduction) internal view returns (uint256) {
        return address(this).balance.mul(_carrierTokenDecimalFactor).div(_carrierToken.totalSupply().sub(_carrierToken.balanceOf(address(this)).sub(_reflectionPoolHoldings).add(deduction))).div(_liquidityTokenDecimalFactor);
    }
