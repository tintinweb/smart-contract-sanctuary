...
contract Lottery is Ownable {
    ...
    mapping(uint => mapping(uint => mapping(address => ticket))) userTickets; // seri => timestamp => user => ticket
   ...
   function getMessageHash(uint timestamp, string memory result) public pure returns (bytes32) {
    ...
    function getUserTickets(uint _seri, address _user, uint _timestamd) public view returns(ticket memory) {
        return userTickets[_seri][_timestamd][_user];
    }
    ...
    function buyCarryOver(uint _seri, string[] memory _number, uint[] memory _numTicket, uint _assetIndex, uint _timestamp, uint totalTicket) public payable{
        ...
        userTickets[_seri][_timestamp][msg.sender] = ticket(_number, _numTicket);
        ...
        if(seriAssetRemain[_seri][_assetIndex] == 0) series[_seri].assetIndex.push(_assetIndex);
        uint assetRemain = assetAmount.sub(shareAffiliateAmount).sub(takeTokenAmount);
        seriAssetRemain[_seri][_assetIndex] += assetRemain;
        series[_seri].soldTicket += totalTicket;
    }
    function buy(uint _seri, string[] memory _number, uint[] memory _numTicket, uint _assetIndex, uint _timestamp, uint totalTicket) public payable{
        require(_number.length == _numTicket.length, 'invalid array length');
        uint assetPerTicket = ticket2Asset(_seri, priceFeeds[_assetIndex]);
        userTickets[_seri][_timestamp][msg.sender] = ticket(_number, _numTicket);
        require(series[_seri].soldTicket + totalTicket <= series[_seri].max2sale, 'over max2sale');
