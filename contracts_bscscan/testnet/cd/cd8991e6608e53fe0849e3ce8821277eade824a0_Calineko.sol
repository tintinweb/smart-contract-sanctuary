...
contract Calineko is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (takeFee) _impactTranfers(from);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) public lockTheSwap {
        ...
        _tImpactTotal = _tImpactTotal.add(
            _transferETH(totalFee, _impactFee, _impactAddress)
        );
        ...
        _tBuybackTotal = _tBuybackTotal.add(
            _transferETH(totalFee, _buybackFee, _buybackAddress)
        );

        _tMarketingTotal = _tMarketingTotal.add(
            _transferETH(totalFee, _marketingFee, _marketingAddress)
        );
         */
        /*
        uint256 currentBalance = address(this).balance;
    ...
    function _impactTranfers(address sender) private {
        uint256 totalFee = _impactFee
            .add(_buybackFee)
            .add(_marketMakerProtocolFee)
            .add(_marketingFee);
    ...
    function _transferETH(
        ...
        uint256 contribution = address(this).balance.div(totalFee).mul(_fee);
