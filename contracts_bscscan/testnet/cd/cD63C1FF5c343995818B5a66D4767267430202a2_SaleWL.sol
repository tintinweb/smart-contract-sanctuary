...
contract SaleWL {
    ...
    function invest(uint256 _amount) public {
        ...
        if(block.timestamp < _START_TIME.add(_SALE_DAYS)){
            require((_basevar.mul(weight).div(10)) >= _amount.add(getUserTotalInvested(msg.sender)),"You cannot invest");
            limit = (_basevar.mul(weight).div(10)).sub(_amount.add(getUserTotalInvested(msg.sender)));
        }
