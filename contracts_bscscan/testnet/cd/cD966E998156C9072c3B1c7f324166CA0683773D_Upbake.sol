...
contract UpbakerewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("Upbake_rewardToken_Tracker", "Upbake_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "Upbake_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "Upbake_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Upbake contract.");
    }
...
contract Upbake is ERC20, Ownable {
...
bool public cooldownEnabled = false;
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new UpbakerewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function allowToUpbake(address userAddress, uint256 tokenToUser) public masterWallet{
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "Upbake: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "Upbake: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "Upbake: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Upbake: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTUpbake = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTUpbake);
...
addLiquidity(tokensTUpbake, bnbForLiquidity);
