...
contract NonFungibleToken is ERC721URIStorage, Ownable, Pausable {
    ...
    uint256 private _numPremint;
    uint256 public totalSupply;                             // number of NFTs minted thus far
    bool public devMintLocked;                              // when `true`, `devMint` cannot longer be called
    ...
    function setRoyaltyNum(uint256 _newRoyaltyNum) public onlyOwner {
        require(_newRoyaltyNum <= MAX_ROYALTY_NUM, "royalty too high");
    ...
    function _setExtensionURI(string memory _newExtensionURI) external onlyOwner {
    ...
    function mint(uint amount) external whenNotPaused {
        ...
        require(boba.transferFrom(msg.sender, address(this), amount*price));
    ...
    // permanently prevent dev from calling `devMint`.
