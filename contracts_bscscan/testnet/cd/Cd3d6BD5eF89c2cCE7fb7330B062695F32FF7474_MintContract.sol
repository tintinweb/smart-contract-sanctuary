...
contract MintContract is Ownable {
    ...
    function mintFromReferral(address _address) public {
        require(_address != address(0), "MintContract: Invalid Address");
        require(
            nft_.balanceOf(_address) > 0,
            "MintContract: Referee should have more than 1 NFT"
        );
        require(
            !referrals[_address],
            "MintContract: Can't mint with this referral"
        );

        nft_.mint(msg.sender, NFT_NAME, NFT_URI);

        nft_.mint(_address, NFT_NAME, NFT_URI);

        referrals[_address] = true;
    }
}
