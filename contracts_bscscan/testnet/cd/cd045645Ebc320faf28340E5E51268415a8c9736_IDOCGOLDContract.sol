...
contract IDOCGOLDContract {
   ...
   */
  function updateRateRef(uint256 _f1_rate,uint256 _f1_rate_airdrop) public onlyIDOAdmin {
    f1_rate = _f1_rate;
   ...
   */
  function updateRateRef(uint256 _amountClaimAirDrop) public onlyIDOAdmin {
  ...
  function buyIDO(address recipient, uint256 buy_amount, address _referrer) public ifNotPaused returns (uint256) {
    ...
    uint256 sold_amount = buy_amount * PERCENTS_DIVIDER / tokenRate;
    ...
    if (referrers[msg.sender] != address(0) && f1_rate > 0 ){
      uint256 f1_reward = sold_amount * f1_rate / PERCENTS_DIVIDER;
