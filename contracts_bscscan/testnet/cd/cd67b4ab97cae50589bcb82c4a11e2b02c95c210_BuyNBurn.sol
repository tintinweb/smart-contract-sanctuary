...
contract BuyNBurn is Context, IERC20, Ownable {
    ...
    uint256 private maxWalletBalance = 1; // Less fields to edit
    uint256 private _maxWalletBalance = (_tTotal * maxWalletBalance) / maxTxDivisor;
    uint256 private _previous_maxWalletBalance = _maxWalletBalance;
    uint256 public _maxWalletBalanceAmountUI = (startingSupply * maxWalletBalance) / maxTxDivisor; // Actual amount for UI's    
    ...
    function setmaxWalletBalance(uint256 percent, uint256 divisor) external onlyOwner() {
        require(divisor <= 10000); // Cannot set lower than 0.01%
        _maxWalletBalance = _tTotal.mul(percent).div(divisor);
        _maxWalletBalanceAmountUI = startingSupply.mul(percent).div(divisor);
    }
