...
contract PooAdsPromotions is Context, IERC20, Ownable, Initializable {
    ...
    string private _name = "POOADS PROMOTIONS";
    string private _symbol = "POOADS";
    ...
    uint256 public _pooAdsFee = 4;
    uint256 private _previousPooAdsFee = _pooAdsFee;
    ...
    uint256 private feeDivisor = _liquidityFee.add(_pooAdsFee);

    IUniswapV2Router02 public immutable uniswapV2Router;
    address public immutable uniswapV2Pair;
    address payable public _pooAdsWalletAddress;
    ...
    uint256 private numTokensMax = 55000 * 10 ** 9; //0.055%
    uint256 private numTokensSellToAddToLiquidity = 50000 * 10 ** 9; //0.05% 
    ...
    function initialize(
        address payable pooAdsWalletAddress, 
        address payable developmentWalletAddress
    ) external initializer() onlyOwner() {
        _isExcludedFromMAxTx[owner()] = true;
        _pooAdsWalletAddress = pooAdsWalletAddress;
    ...
    function setPooAdsFeePercent(uint256 pooAdsFee) external onlyOwner() {
        _pooAdsFee = pooAdsFee;
    }
    ...
    function setNumTokensSell(uint256 _numTokensMax, uint256 _numTokens) external onlyOwner() {
    ...
    function viewNumTokens() external view returns (uint256, uint256) {
        return (numTokensMax, numTokensSellToAddToLiquidity);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateLiquidityPooFee(tAmount);
        uint256 tDev = calculateDevelopmentBurnFee(tAmount);
    ...
    function _takeLiquidity(uint256 tLiquidity, uint256 tDev) private {
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 divisor = _developmentFee.add(_burnFee);
        if (divisor < 1) {
            divisor = 1;
        }
        uint256 tDevelopment = tDev.div(divisor).mul(_developmentFee);
        uint256 tBurn = tDev.sub(tDevelopment);
    ...
    function calculateDevelopmentBurnFee(uint256 _amount) private view returns (uint256) {
        uint256 fee = _developmentFee.add(_burnFee);
        return _amount.mul(fee).div(
            10**2
        );
    }

    function calculateLiquidityPooFee(uint256 _amount) private view returns (uint256) {
        uint256 fee = _liquidityFee.add(_pooAdsFee);
        return _amount.mul(fee).div(
            10**2
        );
    }
    ...
    function restoreAllFee() private {
        ...
        _pooAdsFee = _previousPooAdsFee;
        _developmentFee = _previousDevelopmentFee;
        _burnFee = _previousBurnFee;
    }
    
    function sendBNBToPooAds() private { 
        _pooAdsWalletAddress.transfer(address(this).balance); 
    }
    
    function _setPooAdsWallet(address payable pooAdsWalletAddress) external onlyOwner() {
        _pooAdsWalletAddress = pooAdsWalletAddress;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 lpTokens = numTokensSellToAddToLiquidity.div(feeDivisor).mul(_liquidityFee);
        ...
        uint256 eth = newBalance.div(feeDivisor).mul(_liquidityFee);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeLiquidity(tLiquidity, tDev);
