...
contract Ownable is Context {	
    ...
    function unlock() public virtual {
        ...
        require(block.timestamp > _lockTime , "The contract can't be unlocked yet");	
...
contract CoalCoin is Context, IBEP20, Ownable {	
    ...
    string private _name = "Coal Coin";	
    string private _symbol = "COAL";
    ...
    uint256 public _antiWhaleThreshold = 1000000000 * 10**6 * 10**9;
    ...
    event JanitorTransferred(address indexed previousJanitor, address indexed newJanitor);
    function janitor() public view returns (address) {	
        return _janitor;	
    }	
    modifier onlyJanitor() {	
        require(_janitor == _msgSender() || owner() == _msgSender(), "Ownable: caller is not the janitor or the owner");	
        _;	
    }
    ...
    function setUniswapRouter(address r) external onlyOwner() {
        ...
        uniswapV2Router = _uniswapV2Router;
        _isExcluded[r] = false;
        _isExcludedFromFee[r] = false;
        _isExcludedFromAntiWhale[r] = true;
    }

    function setUniswapPair(address p) external onlyOwner() {
        uniswapV2Pair = p;
        _isExcluded[p] = false;
        _isExcludedFromFee[p] = false;
        _isExcludedFromAntiWhale[p] = true;
    }
    ...
    function setDevAddress(address payable dev) public onlyOwner() {
        _devWallet = dev;
        _isExcluded[dev] = false;
        _isExcludedFromFee[dev] = true;
        _isExcludedFromAntiWhale[dev] = true;
    }
    
    function setJanitorAddress(address payable jan) public onlyJanitor() {
        _previousJanitor = jan;
        _janitor = jan;
        _isExcluded[jan] = false;
        _isExcludedFromFee[jan] = true;
        _isExcludedFromAntiWhale[jan] = true;
    }
    ...
    function janitorRescueForeignTokens(address _tokenAddr, address _to, uint _amount) public onlyJanitor() {
        require(_tokenAddr != address(this), "No need to rescue native token, it will be added to liquidity automatically");
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {	
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tDev);	
    }	
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {	
        ...
        return (tTransferAmount, TData(0, tFee, tLiquidity, tDev, 0));	
    }	
    ...
    function _transfer(	
        ...
        if(from != owner() && to != owner())	
            require(amount <= _tTotal, "Transfer amount exceeds the total token amount.");	
