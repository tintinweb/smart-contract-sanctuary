...
// File contracts/FishingTownAuction.sol
...
contract FishingTownAuction is Pausable, ERC721Holder, AccessControl {
    using SafeERC20 for IERC20;

    event WhitelistedTokenChanged(address indexed tokenAddress, bool isWhitelisted);
    ...
    constructor(
        ...
        require(_busd != address(0), "cannot be zero");
    ...
    function sell(
        ...
        require(price % 1 ether == 0, "price cannot be in decimals");
        require(duration % 1 hours == 0, "duration must be in hours");
        require(duration >= 1 hours && duration <= 7 days, "duration must be between 1 and 168 hours");
    ...
    function cancel(uint256 auctionId) external {
        Auction memory auction = auctionOf(auctionId);
        ...
        _auctions[auctionId].state = AuctionState.Cancelled;

        emit Cancel(msg.sender, auctionId, block.timestamp);
    }

    function bid(uint256 auctionId, uint256 price) external {
        Auction memory auction = auctionOf(auctionId);
        require(
        ...
        if (auction.bidder != address(0)) {
            busd.safeTransfer(auction.bidder, auction.price);
        }
        ...
        if (auction.endTime < block.timestamp + 10 minutes) {
            _auctions[auctionId].endTime = block.timestamp + 10 minutes;
        }
    ...
    function claim(uint256 auctionId) external {
        ...
        uint256 toTreasury = _calculateFee(auction.price, auction.fee);
        busd.safeTransfer(treasuryAddress, toTreasury);
        busd.safeTransfer(auction.seller, auction.price - toTreasury);
        IERC721(auction.tokenAddress).safeTransferFrom(
            address(this),
            auction.bidder,
            auction.tokenId
        );
        _auctions[auctionId].state = AuctionState.Claimed;
    ...
    {
        require(_newTreasuryAddress != address(0), "cannot be zero");
    ...
    function setWhitelistToken(address _tokenAddress, bool _isWhitelisted)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _whitelistedTokens[_tokenAddress] = _isWhitelisted;
        emit WhitelistedTokenChanged(_tokenAddress, _isWhitelisted);
    }

    function auctionOf(uint256 _auctionId)
        public
        view
    ...
    function isWhitelistedToken(address _tokenAddress)
        public
        view
        returns (bool)
    {
