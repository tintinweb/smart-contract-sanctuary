...
contract SamuraiRecruiting is RSunAuth, IERC721Receiver {
    ...
    struct OutputMission { // just for being read from the frontend
        uint missionId;
        uint[] cardIds;
        uint40 startTimestamp;
        uint32 duration;
    ...
    address public infAdr = 0x2993321E2Dbc1E1FD3115B083147813fCac9b868;
    address public samuraiAdr = 0xC3c3B849ED5164Fb626c4a4F78e0675907B2C94E;

    uint public maxUsages = 10;
    uint public maxUsagesGeneral = 30;
    uint public stopIncreasingAt = 10;
    uint public rsunCost = 15000 gwei; // 15k RSUN ~ 15-20$

    uint public lockDurationBase = 8 hours;
    uint public lockDurationPerUsage = 4 hours;

    uint public lockDurationBaseGeneral = 6 hours;
    uint public lockDurationPerUsageGeneral = 2 hours;

    uint public infCostBase = 2000 gwei;
    uint public infCostPerUsage = 2000 gwei;

    uint public infCostBaseGeneral = 1000 gwei;
    uint public infCostPerUsageGeneral = 1000 gwei;

    mapping(uint => uint) public usages; // how often a samurai has been used in recruiting
    mapping(address => uint[]) public missionOwnerships;
    Mission[] public missions;

    ISamuraiRising samurai;
    IMysteriousCrates crates;

    event DebugLog(string message, uint value);
    event RecruitmentStarted(address indexed user, uint[] cardIds, uint indexed missionId);
    event RecruitmentFinished(address indexed user, uint indexed missionId);
    ...
    function recruit(uint[] memory ids) external {
        ...
        missionOwnerships[user].push(missions.length);
        missions.push(Mission(getIdsAsSingleUint(ids), uint40(block.timestamp), getLockDuration(ids), false, user));
        ...
        emit RecruitmentStarted(user, ids, missions.length - 1);
    }

    function _checkSamurai(uint[] memory ids) internal view {
        require(ids.length == 3, "Recruiting takes 3 samurai");

        for (uint256 i = 0; i < ids.length; i++) {
            uint maxUses = maxUsages;
            if (ids[i] < 5000) { // if it's a general
                maxUses = maxUsagesGeneral;
            }

            require(usages[ids[i]] < maxUses, "Recruitment limit of one or more samurai exceeded");
        }
    }

    function retrieveSamurai(uint missionId) external {
        Mission memory mission = missions[missionId];
        require(mission.owner == msg.sender, "Only mission owner can retrieve samurai.");
        require(block.timestamp >= mission.startTimestamp + mission.duration, "Staking period hasn't ended yet");
        require(mission.retrieved == false, "Samurai have already been retrieved");
        
        uint[] memory ids = getIdsAsArray(missionId);

        missions[missionId].retrieved = true;

        for (uint256 i = 0; i < ids.length; i++) {
            samurai.safeTransferFrom(address(this), msg.sender, ids[i]);
        }

        crates.mintCrate(msg.sender, ids);
        emit RecruitmentFinished(msg.sender, missionId);
    }

    function getLockDuration(uint[] memory ids) public view returns (uint32 dur) {
        for (uint256 i = 0; i < ids.length; i++) {
            uint id = ids[i];
            uint timesUsed = usages[id];
            
            if (id < 5000) {
                if (timesUsed < stopIncreasingAt) {
                    dur += uint32(lockDurationBaseGeneral + timesUsed * lockDurationPerUsageGeneral);
                } else {
                    dur += uint32(lockDurationBaseGeneral + stopIncreasingAt * lockDurationPerUsageGeneral);
                }
            } else {
                dur += uint32(lockDurationBase + timesUsed * lockDurationPerUsage);
            }
        }
    }

    function getUnlockTime(uint missionId) public view returns (uint time) {
        Mission memory mission = missions[missionId];
        return mission.startTimestamp + mission.duration;
    }
    ...
    function getInfCost(uint[] memory ids) public view returns (uint infCost) {
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (id < 5000) {
                if (timesUsed < stopIncreasingAt) {
                    infCost += infCostBaseGeneral + (timesUsed * infCostPerUsageGeneral);
                } else {
                    infCost += infCostBaseGeneral + (stopIncreasingAt * infCostPerUsageGeneral);
                }
            } else {
                infCost += infCostBase + (timesUsed * infCostPerUsage);
            }
        }
    }
    ...
    function getIdsAsArray(uint missionId) public view returns(uint[] memory arr) {
        uint72 ids = missions[missionId].cardIds;
    ...
    function getMission(uint id) external view returns(OutputMission memory mission_) {
        Mission memory mission = missions[id];
        uint[] memory ids = _getIdsAsArray(mission.cardIds);

        mission_ = OutputMission(id, ids, mission.startTimestamp, mission.duration, mission.retrieved, mission.owner);
    }

    function getAllMissionsOfUser(address user) external view returns(OutputMission[] memory missions_) {
        uint[] memory missionIds = missionOwnerships[user];
        
        missions_ = new OutputMission[](missionIds.length);
        for (uint256 i = 0; i < missionIds.length; i++) {
            Mission memory mission = missions[missionIds[i]];
            uint[] memory ids = _getIdsAsArray(mission.cardIds);

            missions_[i] = OutputMission(missionIds[i], ids, mission.startTimestamp, mission.duration, mission.retrieved, mission.owner);
        }
    }

    function setLockDurations(uint base, uint perUsage, uint baseGeneral, uint perUsageGeneral) external {
        authorizedFor(Permission.AdjustVariables);
        
        lockDurationBase = base;
        lockDurationPerUsage = perUsage;

        lockDurationBaseGeneral = baseGeneral;
        lockDurationPerUsageGeneral = perUsageGeneral;
    }

    function setInfCosts(uint base, uint perUsage, uint baseGeneral, uint perUsageGeneral) external {
        authorizedFor(Permission.AdjustVariables);

        infCostBase = base;
        infCostPerUsage = perUsage;

        infCostBaseGeneral = baseGeneral;
        infCostPerUsageGeneral = perUsageGeneral;
    }

    function setMaxUsages(uint normal, uint general) external {
        authorizedFor(Permission.AdjustVariables);
        maxUsages = normal;
        maxUsagesGeneral = general;
    }

    function setStopIncreasingAt(uint num) external {
        authorizedFor(Permission.AdjustVariables);
        stopIncreasingAt = num;
    }
    ...
    function setInfAdr(address inf_) external {
        authorizedFor(Permission.AdjustVariables);
        infAdr = inf_;
    }
