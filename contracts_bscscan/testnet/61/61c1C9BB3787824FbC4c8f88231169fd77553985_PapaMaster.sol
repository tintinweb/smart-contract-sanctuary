...
contract PapaMaster is Ownable, ReentrancyGuard {
    ...
    // Bonus muliplier for early dogejr makers.
    ...
    event EmissionRateUpdated(address indexed user, uint256 previousAmount, uint256 newAmount);
    ...
    // Update the given pool's DogeJr allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint256 _harvestInterval,
    ...
    function updatePool(uint256 _pid) public validatePool(_pid) {
        ...
        if (tokenReward.add(tokenReward.div(10)) <= token.maxSupply().sub(token.totalSupply())) { // check total supply
    ...
    function deposit(
        uint256 _pid, // index of poolInfo
        uint256 _amount
   ...
   // Pay or lockup pending Rewards.
    ...
    // Safe token transfer function, just in case if rounding error causes pool to not have enough DogeJr.
...
contract DogeJR is BEP20("DogeJR", "DOGEJR") {
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), "GOONG::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "GOONG::delegateBySig: invalid nonce");
    ...
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "GOONG::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, "GOONG::_writeCheckpoint: block number exceeds 32 bits");
