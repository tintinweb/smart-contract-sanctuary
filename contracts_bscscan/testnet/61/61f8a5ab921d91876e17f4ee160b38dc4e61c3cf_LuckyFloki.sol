...
contract LuckyFloki is ERC20, Ownable {
        ...
        if(limitsInEffect){
            if (
                ...
                //when sell
                        ...
                        //IF LUCKY HOUR - 3 hours, remove from lotto.
                        if (overMinLottoBuying) {
    ...
    function getlastlottoWinner() public view returns(uint) {
        uint256 Winner = _lastLottoWinnerAmount;
        return Winner;
    }
    ...
    function drawLotto(uint256 amount) external onlyOwner {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        swapBnbForRewardToken(lotteryAmount);
        tokenBalance = IERC20(token).balanceOf(address(this));
        success = IERC20(token).transfer(address(lotteryWallet), tokenBalance);
