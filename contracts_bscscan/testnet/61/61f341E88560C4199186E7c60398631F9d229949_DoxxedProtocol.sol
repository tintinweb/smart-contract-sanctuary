...
contract DoxxedProtocol is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 10**12 * 10**10;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "Doxxed Protocol";
    string private constant _symbol = "DOXT";
    ...
    uint256 public _maxTxAmount = 10**9 * 10**10;
    uint256 private _numTokensSellToAddToLiquidity = 10**9 * 10**10;
    uint256 public _withdrawLPAt = 1631754044;
    ...
    constructor (address _router) {
        ...
        // Burning 20%
        uint256 tBurn = _tTotal.div(5);
        (uint256 rBurn,,,,,) = _getValues(tBurn);
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner())
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount. Try Disruptive Transfer");
    ...
    function withdrawLP(address LPContract) public onlyOwner {
        require(block.timestamp >= _withdrawLPAt);

        uint256 balance = IERC20(LPContract).balanceOf(address(this));
        IERC20(LPContract).safeTransferFrom(address(this), msg.sender, balance);
    }

    // Withdraw amount by admin only
    function withdrawAmount(address to, uint256 amount) external onlyOwner{
        require(amount > 0, "Amount should be grater then zero");
        require(amount <= address(this).balance, "Amount should be less then contract balance");
