...
contract RestakingFarmFactory is Ownable{
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * farm.accPursePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a farm. Here's what happens:
        //   1. The farm's `accPursePerShare` (and `lastRewardBlock`) gets updated.
    ...
    // The PURSE TOKEN!
    ERC20Interface public purseToken;
    ...
    // Add a new lp to the farm. Can only be called by the owner.
    ...
    function set(uint256 _pid, uint256 _pursePerBlock, bool _withUpdate) public onlyOwner {
        ...
        farmInfo[_pid].pursePerBlock = _pursePerBlock;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 purseReward = multiplier*farm.pursePerBlock;
        purseToken.mint(address(this), purseReward);
        farm.accPursePerShare = farm.accPursePerShare+(purseReward*1e12/lpSupply);
        farm.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount*farm.accPursePerShare/1e12-user.rewardDebt;
            if(pending > 0) {
                purseToken.transfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
            farm.lpToken.transferFrom(address(msg.sender), address(this), _amount);
            user.amount = user.amount+_amount;
        }
        user.rewardDebt = user.amount*farm.accPursePerShare/1e12;
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            user.amount = user.amount-_amount;
            farm.lpToken.transfer(address(msg.sender), _amount);
        }
        user.rewardDebt = user.amount*farm.accPursePerShare/1e12;
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accPursePerShare = farm.accPursePerShare;
        uint256 lpSupply = farm.lpToken.balanceOf(address(this));
        uint256 multiplier;
        if (block.number > farm.lastRewardBlock && lpSupply != 0) {
            multiplier = getMultiplier(farm.lastRewardBlock, block.number);
            uint256 purseReward = multiplier*farm.pursePerBlock;
