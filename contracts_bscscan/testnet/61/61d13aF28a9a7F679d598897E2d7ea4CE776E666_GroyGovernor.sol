...
/** 
 *  SourceUnit: /app/contracts/GroyGovernor.sol
*/
...
contract GroyGovernor is Ownable, ReentrancyGuard {
    ...
    function decreaseVote(uint optionId, uint amount) external optionExists(optionId, optionCount) nonReentrant {
        ...
        _requireVoteIsActive(initiative, initiative.allowEarlyWithdrawal);
        mapping(address => uint) storage votesOnOptionId = currentVotes[optionId];
        require(votesOnOptionId[msg.sender] >= amount, "Vote removal amount too large");
        Option storage option = options[optionId];

        PoolInfo storage pool = poolInfo[initiativeId];
        UserInfo storage user = userInfo[initiativeId][msg.sender];        

        // update pool
        updatePool(initiativeId);

        option.totalVotes -= amount; // TODO: Examine cost benefits of forcing the client to calculate this value?

        votesOnOptionId[msg.sender] -= amount; // <== clients could use this value for each 

        uint256 pending = user.amount * pool.accRewardPerShare / 1e12 - user.rewardDebt;
        user.amount -= amount;
        user.rewardDebt = user.amount * pool.accRewardPerShare / 1e12;
        pool.totalVotes -= amount;
    ...
    function increaseVote(uint optionId, uint amount) external optionExists(optionId, optionCount) nonReentrant {
        ...
        uint initiativeId = optionToInitiativeId[optionId];
        Initiative memory initiative = initiatives[initiativeId];
        _requireVoteIsActive(initiative, initiative.allowLateDeposit);
        mapping(address => uint) storage votesOnOptionId = currentVotes[optionId];
        Option storage option = options[optionId];

        // uint senderBalance = IERC20(groyContractAddress).balanceOf(msg.sender);
        // // console.log("AMOUNT: %s", amount);
        // require(senderBalance >= amount, "Vote amount exceeds balance");

        // uint currentAllowance = IERC20(groyContractAddress).allowance(msg.sender, address(this));
        // // console.log("CURRENT_ALLOWANCE: %s", currentAllowance);
        // require(currentAllowance >= amount, "Vote amount exceeds allowance");
        ...
        if (block.number >= pool.lastRewardBlock && priorAmount > 0) {
            uint256 pending = priorAmount * pool.accRewardPerShare / 1e12 - priorReward;
            IERC20(pool.rewardToken).safeTransfer(msg.sender, pending);
        }
    ...
    function claimRewards(uint initiativeId) external initiativeExists(initiativeId, initiativeCount) nonReentrant {
        ...
        // claim current rewards
        // TODO: Create test that will try accumulation after initiative endBlock
        if (block.number >= pool.lastRewardBlock && user.amount > 0) {
            uint256 pending = user.amount * pool.accRewardPerShare / 1e12 - user.rewardDebt;
            user.rewardDebt = user.amount * pool.accRewardPerShare / 1e12;
            IERC20(pool.rewardToken).safeTransfer(msg.sender, pending);
        }
    ...
    function updatePool(uint256 initiativeId) public {
        PoolInfo storage pool = poolInfo[initiativeId];
        ...
        pool.accRewardPerShare += reward * 1e12 / pool.totalVotes;
        pool.lastRewardBlock = block.number;
    }

    function _requireVoteIsActive(Initiative memory initiative, bool disableLock) private view {
        require(block.number <= initiative.endBlock, "Vote has ended");
        require(disableLock || (block.number <= initiative.lockBlock), "Vote is locked");
    ...
    // TODO: consider reducing size for more efficient storage
    //       consider using bitmask + bitmap instead of bools, those take up 8-bits
    /*
    ...
    */
    struct Initiative { // Similar to a proposal
        ...
        uint lockBlock;
        uint endBlock;
        bool allowLateDeposit;
        bool allowEarlyWithdrawal;
    }

    struct PoolInfo { // Reward info. mapping to the initiative
        address rewardToken;
        uint rewardAmount;
    ...
    /// @dev Storage of Initiative ID to Reward PoolInfo objects (structs)
    mapping (uint => PoolInfo) public poolInfo;

    mapping (address => uint[]) public initiatorToInitiatives;

    // TODO: See if events can be used to replace these...
    ...
    function allowLateDeposit(uint initiativeId, bool allow) external 
        onlyOwner
        initiativeExists(initiativeId, initiativeCount)
    {
        Initiative storage initiative = initiatives[initiativeId];
        initiative.allowLateDeposit = allow;
    }

    function allowEarlyWithdrawal(uint initiativeId, bool allow) external onlyOwner initiativeExists(initiativeId, initiativeCount) {
        Initiative storage initiative = initiatives[initiativeId];
        initiative.allowEarlyWithdrawal = allow;
    }



    // TODO: Should we allow anyone to create an initiative (i.e. Initiatives would have Owners)?
    function createInitiative(uint startBlock, uint lockBlock, uint endBlock, address rewardToken, uint rewardAmount) external onlyOwner nonReentrant {
        require(creationEnabled, "Creation is disabled");
        require(startBlock > block.number, "Start block is in the past");
        require(startBlock <= lockBlock, "Start block > lock block");
        require(lockBlock <= endBlock, "Lock block > end block");
        ...
        initiative.allowLateDeposit = false; // default
        initiative.allowEarlyWithdrawal = false; // default

        PoolInfo storage pool = poolInfo[id];
        pool.rewardToken = rewardToken;
        pool.rewardAmount = rewardAmount;
        pool.lastRewardBlock = block.number > lockBlock + 1 ? block.number : lockBlock + 1;
        pool.rewardPerBlock = rewardAmount / (endBlock - lockBlock);
        ...
        emit InitiativeCreated(
            ...
            initiative.endBlock,
            initiative.allowLateDeposit,
            initiative.allowEarlyWithdrawal,
            pool.rewardToken,
            pool.rewardAmount
        );
    ...
    event InitiativeCreated(
        ...
        bool allowLateDeposit, bool allowEarlyWithdrawal,
    ...
    // TODO: Add bitmask and bitmap for data compression? 
    event IncreaseVote(
    ...
    /// @dev Storage of Initiative ID to Voter ID/Address to the receipt object for their vote
    mapping (uint => mapping(address => uint)) public currentVotes; // current user votes
    // mapping (uint => mapping (address => Receipt)) public receipts;

    // TODO: See if events can be used to replace these...
    //       Use events to track the count of votes at given blocks
    ...
    function createOption(string memory description, uint initiativeId) external onlyOwner initiativeExists(initiativeId, initiativeCount) {
        require(creationEnabled, "Creation is disabled");
        uint id = optionCount;
        Option storage option = options[optionCount++];
        option.description = description;
        option.totalVotes = 0; // TODO: Not needed, check if uses extra Gas or is optimized away
