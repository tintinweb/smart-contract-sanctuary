...
contract MorbexToken is BEP20('MORBEX TOKEN', 'BEX') {
        ...
        external
    {
        ...
        require(signatory != address(0), "MasterChef: delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "MasterChef: delegateBySig: invalid nonce");
        require(now <= expiry, "MasterChef: delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "MasterChef: getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "MasterChef: _writeCheckpoint: block number exceeds 32 bits");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {IBEP20 lpToken; uint256 allocPoint; uint256 lastRewardBlock; uint256 accBexPerShare; uint16 depositFeeBP; uint16 withdrawFeeBP;}

    MorbexToken public bex;
    address public devAddr;
    uint256 public bexPerBlock;
    ...
    uint256 public constant INITIAL_EMISSION_RATE = 12 ether;
    uint256 public constant MINIMUM_EMISSION_RATE = 500 finney;
    uint256 public constant EMISSION_REDUCTION_PERIOD_BLOCKS = 576000;
    ...
    constructor(MorbexToken _bex, address _devAddr, address _feeAddress, uint256 _startBlock) public {bex = _bex; devAddr = _devAddr; feeAddress = _feeAddress; bexPerBlock = INITIAL_EMISSION_RATE; startBlock = _startBlock;}
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, uint16 _withdrawFeeBP, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        ...
        require(_withdrawFeeBP <= MAXIMUM_WITHDRAWAL_FEE_BP, "add: invalid withdraw fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function pendingBex(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBexPerShare = pool.accBexPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 bexReward = multiplier.mul(bexPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accBexPerShare = accBexPerShare.add(bexReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accBexPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 bexReward = multiplier.mul(bexPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        bex.mint(devAddr, bexReward.div(10));
        bex.mint(address(this), bexReward);
        pool.accBexPerShare = pool.accBexPerShare.add(bexReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accBexPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safeBexTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accBexPerShare).div(1e12);
    ...
    function safeBexTransfer(address _to, uint256 _amount) internal {
        uint256 bexBal = bex.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > bexBal) {
            transferSuccess = bex.transfer(_to, bexBal);
        } else {
            transferSuccess = bex.transfer(_to, _amount);
        }
        require(transferSuccess, "safeBexTransfer: transfer failed.");
    }
    ...
    function updateEmissionRate() public {
        ...
        require(bexPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = bexPerBlock;
        ...
        if (newEmissionRate >= bexPerBlock) {
            return;
        }
        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = bexPerBlock;
        bexPerBlock = newEmissionRate;
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            bex.mint(referrer, refBonusEarned);
