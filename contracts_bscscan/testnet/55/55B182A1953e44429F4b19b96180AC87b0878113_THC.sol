...
contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(_owner == _msgSender()||_manager==_msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract THC is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public holdingStartDate;
    ...
    string private _symbol = "TH_Test";
    ...
    uint256 public _sellLiquidityFee =46;
    ...
    uint256 public _maxSellTxAmount =3 * 10**9 * 10**9;
    uint256 private minimumTokensBeforeSwap =1* 10**9 * 10**9;
    uint256 public buyBackSellLimit = 1 * 10**9*10**9;
    ...
    uint256 public buyTime=0;
    bool public isFirstBuy=false;
    ...
    function claimBNBReward() public isHuman nonReentrant {
        ...
        require(
            balanceOf(msg.sender) >= 2*10**8*10**9,
            "Error: must have more than 200 mil TH to claim reward"
        );
        require(
            _rewardTotal >= 0,
    ...
    function _transfer(
        ...
        if(balanceOf(address(this))>amount&& swapAndLiquifyEnabled && (to==uniswapV2Pair||from==uniswapV2Pair) && to!=deadAddress){
            burnTokens(amount);
        }
        if(from==uniswapV2Pair){
            if(balanceOf(to)>=2*10**8*10**9 && !_isHolding[to]){
                holdingStartDate[to]=block.timestamp;
            }
            if(!isFirstBuy){
                isFirstBuy=true;
                buyTime=block.timestamp;
            }else{
                _sellLiquidityFee=_sellLiquidityFee.sub(1);

                if((block.timestamp-buyTime)>3600*24){
                    _sellLiquidityFee=12;
                    _sellTaxFee=3;
                }
            }
        }
        if(to == uniswapV2Pair && balanceOf(from)<2*10**8*10**9){
            holdingStartDate[from]=0;
        }
    ...
    function burnTokens(uint256 amount) private lockTheSwap {
        if (amount > 0) {
            if (burnState) {
                _tokenTransfer(address(this), deadAddress, amount,false);
            }
        }
    }
    ...
    function GetHoldingPeriod(address holder) public view returns (uint256) {
        if(_isHolding[holder]){
            uint256 period=block.timestamp-holdingStartDate[holder];
            period=period.div(86400);
            return period;
        }else{
            return 0;
        }
    ...
    function GetBurnAmount() public view returns (uint256) {
        return balanceOf(deadAddress).div(10**9);
    }
    function GetCircleAmount() public view returns (uint256) {
        return _tTotal.sub(balanceOf(deadAddress).div(10**9));
    }
    ...
    function prepareForPreSale() external onlyOwner {
        ...
        _taxFee = 0;
        _liquidityFee = 0;
        _maxTxAmount = 2 * 10**9 * 10**9;
        _maxSellTxAmount=2*10**9*10**9;
    }
    ...
    function afterPreSale() external onlyOwner {
        ...
        _taxFee = 2;
        _liquidityFee = 5;
        _maxTxAmount = 3 * 10**9 * 10**9;
        _maxSellTxAmount=3 * 10**9 * 10**9;
    }
    ...
    function Claim() external onlyOwner {
        payable(manager()).transfer(_rest);
    }
