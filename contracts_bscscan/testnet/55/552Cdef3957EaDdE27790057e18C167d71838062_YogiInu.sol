...
contract YogiInu is IBEP20, Auth {
    ...
    string constant _name = "test21";
    string constant _symbol = "test21";
    ...
    uint256 startSellLiquidityFee = 500;

    uint256 burnFee = 400;
    uint256 startSellBurnFee = 500;

    uint256 marketingFee = 400;
    uint256 startSellMarketingFee = 2000;

    uint256 public totalFee = 1200;
    uint256 public startSellTotalFee = 3000;
    ...
    uint256 start = 1639751400;
    uint256 WLTime = 15 minutes;
    uint256 botTime = 5 seconds;
    uint256 highTaxTime = 20 minutes;
    ...
    event intLog(string name, uint256 val);

    constructor () Auth(msg.sender) {
        WL[0x934d30fC73685675645b09d41b8a7F1e57A10ea2] = true;
        ...
        _balances[msg.sender] = _totalSupply * 49 / 100;
        emit Transfer(address(0), msg.sender, _totalSupply * 49 / 100);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && !authorizations[recipient] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver){
            ...
            if (block.timestamp < start.add(WLTime)) {
                maxTokens = _maxWLTxAmount.div(feeDenominator.sub(totalFee)).mul(feeDenominator).add(10 ** _decimals);
            } else {
                maxTokens = _maxTxAmount.div(feeDenominator.sub(totalFee)).mul(feeDenominator).add(10 ** _decimals);
            }
            emit intLog("maxTokens", maxTokens);
            require((heldTokens + amount) <= maxTokens,"Total Holding is currently limited, you can not buy that much.");
        }
    ...
    function checkTxLimit(address sender, address recipient, uint256 amount) internal view {
        require(amount <= _maxTxAmount.div(feeDenominator.sub(totalFee)).mul(feeDenominator).add(10 ** _decimals)  || isTxLimitExempt[sender] || authorizations[sender] || authorizations[recipient], "TX Limit Exceeded");
    }
