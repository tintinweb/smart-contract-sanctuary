...
contract bgdsvipMultisender is Ownable
{
...
//    event ClaimedTokens(address token, address payable ownerPayable, uint256 amount);
    ...
    modifier validLists(uint256 _contributorsLength, uint256 _balancesLength) {
        require(_contributorsLength > 0, "bgdsvipMultisender: No contributors sent");
        require(
            _contributorsLength == _balancesLength,
            "bgdsvipMultisender: Different arrays lengths"
        );
        _;
    }
        ...
        onlyOwner
    {
        require(
            isInitialized == false,
            "bgdsvipMultisender: Already initialized"
        );
    ...
    function multisendToken(
        ...
        uint256 _total
    ) external payable validLists(contributors.length, balances.length) {
        ...
        require(
            total >= 0,
            "bgdsvipMultisender: Wrong amount of total"
        );
