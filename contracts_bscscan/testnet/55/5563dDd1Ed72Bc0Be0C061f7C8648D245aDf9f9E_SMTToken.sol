...
contract SMTToken is Context, IBEP20, Ownable {
    ...
    address public _archivementSystemAddress;
    address public _licenseSystemAddress;
    ...
    uint256 public SELL_LICENSE_SYSTEM_FEE = 20;
    ...
    uint256 public TRANSFER_GOLDEN_TREE_POOL_FEE = 80;

    uint256 public _numTokensSwap = 50000e18;
    ...
    event TaxAddressesUpdated(address referral, address goldenTree, address dev, address archivement, address license);
     ...
     *
     */
    constructor(address busd, uint256 totalSupply, address referral, address goldenTree, address dev, address archivement, address license) public {
        _name = "SMT Token";
        _symbol = "SMT";
        _decimals = 18;
        ...
        _devAddress = dev;
        _archivementSystemAddress = archivement;
        _licenseSystemAddress = license;
        ...
        _excludedFromFee[_archivementSystemAddress] = true;
        _excludedFromFee[_licenseSystemAddress] = true;
    ...
    */
    ...
    function _transfer (address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (_excludedFromFee[sender] || _excludedFromFee[recipient]) {
            ...
            if (sender == _uniswapV2ETHPair || sender == _uniswapV2BUSDPair) {
                // buy transfer
                uint256 taxAmount = amount.mul(_buyTaxFee).div(100);
                uint256 recvAmount = amount.sub(taxAmount);
                _balances[recipient] = _balances[recipient].add(recvAmount);

                distributeBuyTax(taxAmount);
                
            } else if (recipient == _uniswapV2ETHPair || recipient == _uniswapV2BUSDPair) {
                // sell transfer
                uint256 taxAmount = amount.mul(_sellTaxFee).div(100);
                uint256 recvAmount = amount.sub(taxAmount);
                _balances[recipient] = _balances[recipient].add(recvAmount);

                distributeSellTax(taxAmount);
                ...
                distributeTransferTax(taxAmount);           
    ...
    function distributeSellTax (uint256 amount) internal {
        ...
        uint256 licenseAmount = amount.mul(SELL_LICENSE_SYSTEM_FEE).div(100);
        uint256 burnAmount = amount.mul(SELL_BURN_FEE).div(100);
        uint256 archivementAmount = amount.mul(SELL_ARCHIVEMENT_FEE).div(100);

        _balances[_devAddress] = _balances[_devAddress].add(devAmount);
        _balances[_licenseSystemAddress] = _balances[_licenseSystemAddress].add(licenseAmount);
        ...
        if (overMinTokenBalance && 
            _swapEnabled && 
            !_inSwapEnabled) {
            swapAndDistributeForSell(_numTokensSwap);
        }

        _balances[address(this)] = _balances[address(this)].add(goldenTreeAmount).add(archivementAmount);
    ...
    function swapAndDistributeForSell (uint256 amount) internal lockTheSwap {
        ...
        uint256 archivementAmount = amount.sub(goldenPoolAmount);
    ...
    function distributeBuyTax (uint256 amount) internal {
        ...
        uint256 archivementAmount = amount.mul(BUY_ARCHIVEMENT_FEE).div(100);
        ...
        if (overMinTokenBalance && 
            _swapEnabled && 
            !_inSwapEnabled) {
            swapAndDistributeForBuy(_numTokensSwap);
        }
    ...
    function distributeTransferTax (uint256 amount) internal {
        ...
        uint256 archivementAmount = amount.mul(TRANFER_ARCHIVEMENT_FEE).div(100);
        ...
        if (overMinTokenBalance && 
            _swapEnabled && 
            !_inSwapEnabled) {
            swapAndDistributeForTransfer(_numTokensSwap);
        }
     ...
     */
    function setTaxAddresses (address referral, address goldenTree, address dev, address archivement, address license) external onlyOperator {
        ...
        if (_archivementSystemAddress != archivement) {
            _archivementSystemAddress = archivement;
            _excludedFromFee[archivement] = true;
        }
        if (_licenseSystemAddress != license) {
            _licenseSystemAddress = license;
            _excludedFromFee[license] = true;
        }
        emit TaxAddressesUpdated(referral, goldenTree, dev, archivement, license);
    }
