...
contract Marketplace is Ownable, Pausable, FeeManager, IMarketplace, ERC721Holder {
    ...
    //IERC20 public acceptedToken = IERC20(0xCB829819787e48D0eBb272DA7424AeB33d91731c);
	address public nftAddress  = 0xDF2a6948f3eb8ac292d6810f697Efd5e9A20BAca;
    ...
    {
        _createOrder(nftAddress, _assetId, _priceInWei);
    }
        ...
        public whenNotPaused
    {
        ...
        require(orderByAssetId[nftAddress][_assetId].id != 0, "Marketplace: asset not published");
        require(orderByAssetId[nftAddress][_assetId].seller == msg.sender, "Marketplace: sender not allowed");
        require(_priceInWei > 0, "Marketplace: Price should be bigger than 0");


        orderByAssetId[nftAddress][_assetId].price = _priceInWei;
        

        emit OrderUpdated(orderByAssetId[nftAddress][_assetId].id, _priceInWei);
    }



    function safeExecuteOrder(uint256 _assetId)payable public whenNotPaused{
	...
	function _nftMap(uint256 _assetId) internal view returns(NftInfo memory){
		NftInfo memory nftInfo;
		(uint256 level,string memory name, uint256 roleId, uint256 force, uint256 agile, uint256 physique, uint256 intellect, uint256 willpower, uint256 spirit) = IERC721(nftAddress).nftMap(_assetId);
        nftInfo.nftId = _assetId;		
		nftInfo.level = level;
		nftInfo.name = name;
		nftInfo.role = roleId;
		nftInfo.force = force;
