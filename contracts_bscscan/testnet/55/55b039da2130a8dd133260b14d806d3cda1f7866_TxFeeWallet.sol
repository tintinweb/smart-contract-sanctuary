...
contract TxFeeWallet is Initializable, JaxOwnable {
    ...
    function pay_yield(uint amount) public onlyGovernor {
        swap_tokens();
        ...
        for (uint i=0; i < yieldLength; i++) {
            ...
            if(yield.isContract) {
                IYield(yield.yield_address).deposit_reward(amount * yield.allocPoint / totalAllocPoint);
            } else {
                IERC20(rewardToken).transfer(yield.yield_address, amount * yield.allocPoint / totalAllocPoint);
            }
        }
        emit Pay_Yield(amount);
    }
