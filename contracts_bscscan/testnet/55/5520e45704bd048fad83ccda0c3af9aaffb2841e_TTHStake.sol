...
contract TTHStake is Owned , ReentrancyGuard {
    ...
    function unstake(address _addressr,uint256 _idRandom) nonReentrant() external onlyRegistered() {
    ...
    function withdrawEarnings() nonReentrant() external returns (bool success) {
    ...
    function Relock(address _addressr,uint idrandom) nonReentrant() public returns (bool success){
      require(_addressr == msg.sender, 'Must be same User.');
        ...
        for(uint i = 0;i < Staker[_addressr].length;i++){
          ...
          if(Staker[_addressr][i].stakeDayss == 1){
            ...
            if(block.timestamp < (Staker[_addressr][i].lastClocks.add(Staker[_addressr][i].stakeDayss.mul(86400)))){
              ...
              dd = dd + ((Staker[_addressr][i].totalStakeds).mul(dailyROI).mul(activeDays)).div(100000).div(1);
            }else{
              //revert("fuck you");
              uint activeDays = (Staker[_addressr][i].lastClocks.add(Staker[_addressr][i].stakeDayss.mul(86400)).sub(Staker[_addressr][i].rewardClocks)).div(120);
              dd = dd + ((Staker[_addressr][i].totalStakeds).mul(dailyROI).mul(activeDays)).div(100000).div(1);
    ...
    function filter(uint _amount) nonReentrant() external onlyOwner returns (bool success) {
