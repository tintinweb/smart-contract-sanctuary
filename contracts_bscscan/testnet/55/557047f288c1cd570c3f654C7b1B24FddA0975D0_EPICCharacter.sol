...
contract EPICCharacter is ERC721, VRFConsumerBase, AccessControl {
    ...
    string public baseURI =
        "https://gateway.pinata.cloud/ipfs/QmW292Ybh9CjrECx1CT3k3efzje2aNPJhQi7z7Txw7cNQM/";
    mapping(uint256 => string) tokenURIs;
    uint256[] public unclaimed;

    constructor(address _token)
        ERC721("EPICCharacter", "EPICC")
        VRFConsumerBase(
    ...
    modifier onlyOwner() {
        require(
            ...
            "RandomNFT.onlyOwner: caller is not the owner"
        );
        _;
    }
    ...
    modifier onlySteward() {
        require(
            hasRole("steward", msg.sender),
            "RandomNFT.onlySteward: caller is not the steward"
        );
        _;
    }
    ...
    function redeemTokenForNFT() public {
        ...
        require(
            balance >= 1,
            "RandomNFT.redeemTokenForNFT: balance less than one"
        );

        token.transferFrom(msg.sender, address(0), balance * 1e18);
        require(
            LINK.balanceOf(address(this)) >= fee * balance,
    ...
    function premint(uint256 tokenId, string memory _tokenURI)
        public
        onlyOwner
    {
        ...
        override(VRFConsumerBase)
    {
        ...
        require(
            to == address(0),
            "fulfillRandomness: requestId does not exist"
        );
    ...
    function setFee(uint256 _fee) public onlySteward {
        fee = _fee;
    }

    // method to recover unused LINK tokens
    function transferLink(address to, uint256 value) public onlySteward {
