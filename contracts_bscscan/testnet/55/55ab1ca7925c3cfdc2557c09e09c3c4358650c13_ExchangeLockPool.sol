...
contract ExchangeLockPool is Ownable {
    ...
    uint256 private welfareRate = 3000;
    ...
    function buy(uint256 exchangeAmount, address invitor) public {
        ...
        require(isOpen,"this pool is close");
        if (maxBuyTimes > 0) {
            require(maxBuyTimes > buyTimes[msg.sender], "buy times is not enough");
        }
        ...
        require(balance() >= withdrawAmount + invitorSendAmoumt, "totalSupply is not enough");
        ...
        if (invitorSendAmoumt > 0) {
            ERC20(withdrawToken).transfer(invitor, invitorSendAmoumt);
        }
    ...
    function withdrawAll(address tokenAddress, address account) public onlyOwner {
