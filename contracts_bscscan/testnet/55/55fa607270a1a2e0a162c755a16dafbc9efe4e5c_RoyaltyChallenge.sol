...
contract RoyaltyChallenge is ERC1155, Ownable, IRoyalty {
    using SafeMath for uint256;
    ...
    address payable recipient = 0x58B7C05f739f5af18513b421CeA2d8fE85544c2F;
    ...
    constructor() public ERC1155("https://gateway.pinata.cloud/ipfs/QmSb8FZBPJA3mxmYnMP5SGWiMwvFtYbNokTp8PU4453ax4/token/") {
    ...
    function setRoyalty(
        ...
        if (_royaltyStrategy == RoyaltyLibrary.Strategy.ROYALTY_STRATEGY) {
            require(
                ...
                "Royalty: Total fee bps should not exceed 10000"
            );
            for (uint256 i = 0; i < ids.length; i++) {
                _setRoyalty(ids[i], sumRoyaltyShareBps, RoyaltyLibrary.Strategy.ROYALTY_STRATEGY);
            }
        } else if (_royaltyStrategy == RoyaltyLibrary.Strategy.PROFIT_DISTRIBUTION_STRATEGY) {
            require(
                sumRoyaltyShareBps == 10 ** 4,
                "Royalty: Total fee bps should be 10000"
            );
            for (uint256 i = 0; i < ids.length; i++) {
                _setRoyalty(ids[i], _royaltyBps, RoyaltyLibrary.Strategy.PROFIT_DISTRIBUTION_STRATEGY);
            }
        } else {
            revert("Royalty: Royalty option does not exist");
        }
        for (uint256 i = 0; i < ids.length; i++) {
            _addRoyaltyShares(ids[i], _royaltyShares);
        }
    }
