...
contract Strategic is AccessControl {
    ...
    uint256 public lockPeriod = 7200 ; // 120m ,2592000; // 30 days
    uint256 public totalPoolLock = 150000000000000000000000000; // 150,000,000 HE
    ...
    function availableAmount(address _address) public view returns( uint256 ) {
        ...
        if(block.timestamp > lockTime){
            ...
            uint256 totalAmount90Percent = totalAmount[address(_address)].mul(9).div(10);
            remainCurrent = totalAmount90Percent.mul(claimPeriod).div(vestingPeriod);
        }
        uint256 availableAmount =  remainCurrent + totalAmount[address(_address)].div(10);
        amount = availableAmount >  withdrawnAmount[address(_address)] ? availableAmount.sub(withdrawnAmount[address(_address)]) : 0;
        return amount;
    }
