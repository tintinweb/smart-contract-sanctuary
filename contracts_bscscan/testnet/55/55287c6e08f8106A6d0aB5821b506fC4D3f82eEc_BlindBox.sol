...
contract BlindBox {
  ...
  function buyBoxOld(address seller, string memory ownerId, string memory name, uint256 selectedTime, bool isPaid) public {
    ...
    for(uint256 i =0; i<Category[name].countNFT; i++)
    {
     ...
     tokenId = IERC20(xanaliaDex).blindBox(seller, Category[name].tokenUris[randNumber], true, msg.sender, ownerId, name);
     temp.tokenUriUsedCount[Category[name].tokenUris[randNumber]]++;
     if(temp.tokenUriUsedCount[Category[name].tokenUris[randNumber]] >= 2){
      
      emit TokenUriUsed(name, Category[name].tokenUris[randNumber], randNumber);
      Category[name].tokenUris[randNumber] = Category[name].tokenUris[(temp.dailiyLimit * dayCount) + temp.usedCountByday[dayCount].count - 1];
