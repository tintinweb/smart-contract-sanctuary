    ...
    FixinTokenSpender
{
    ...
    function getNativeOrderInfoV2(LibNativeOrder.NativeOrder memory order, uint128 amount)
        public
        view
        returns (LibNativeOrder.OrderInfo memory orderInfo)
    {
        // Recover maker and compute order hash.
        orderInfo.orderHash = getNativeOrderHash(order);
        uint256 minValidSalt = LibNativeOrdersStorage.getStorage()
            .limitOrdersMakerToMakerTokenToTakerTokenToMinValidOrderSalt
                [order.maker]
                [address(order.makerToken)]
    ...
    function getNativeOrderHash(LibNativeOrder.NativeOrder memory order)
        public
        view
        returns (bytes32 orderHash)
    {
        return _getEIP712Hash(
            LibNativeOrder.getNativeOrderStructHash(order)
        );
    }
    ...
    /// @param minValidSalt The minimum valid salt for the maker and pair combination.
