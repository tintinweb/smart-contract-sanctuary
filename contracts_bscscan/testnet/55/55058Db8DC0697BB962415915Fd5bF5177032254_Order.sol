...
contract Owner {
    ...
    modifier nonContract() {
        require(_contractCallable || notContract(msg.sender), "non contract");
        _;
    }
...
// File: contracts/Order.sol

pragma solidity ^0.8.0;

interface ITemplar {
    function maxEnergy() external view returns (uint256);
...
interface IGTOKEN721 is IERC721 {
...
contract Order is Owner {
    ...
    event FillOrder(uint256 indexed tokenId, address indexed buyer);
    ...
    uint256 public feeAccumulate;
    uint256 public deadFeeAccumulate;
    uint256 public lpFeeAccumulate;
    ...
    function setTemplarAddress(address _templar) external onlyOwner {
        templar = ITemplar(_templar);
    }
    ...
    function placeOrder(uint256 _tokenId, uint256 _price) external nonContract {
        (, uint256 energy, , uint256 levelCode) = templar.getTokenInfo(
            _tokenId
        );
        require(levelCode > 0, "open first");
        require(energy == templar.maxEnergy(), "full energy");
        require(
            GTOKEN721.ownerOf(_tokenId) == msg.sender,
            "Only owner can sell"
        );
        require(_price > 0, "Cannot sell without fee");
        GTOKEN721.transferFrom(msg.sender, address(this), _tokenId);
        ...
        emit PlaceOrder(_tokenId, msg.sender, _price, levelCode);
    }
    ...
    {
        (fightAt, energy, , levelCode) = templar.getTokenInfo(tokenId);
        price = markets[tokenId].price;
        return (fightAt, energy, levelCode, price);
    }

    function ordersOf(address _seller) public view returns (uint256) {
        return sellerTokens[_seller].length();
    }

    function cancelOrder(uint256 _tokenId) public nonContract {
        ...
        GTOKEN721.transferFrom(address(this), msg.sender, _tokenId);
    ...
    function fillOrder(uint256 _tokenId, uint256 _price) public nonContract {
        require(tokenSales.contains(_tokenId), "not sale");

        ItemSale storage itemSale = markets[_tokenId];
        require(_price == itemSale.price, "price is unexpected");

        uint256 feeAmount = (itemSale.price * fee) / 100;
        GTOKEN20.transferFrom(msg.sender, feeAccount, feeAmount);
        uint256 deadAmount = (itemSale.price * deadfee) / 100;
        GTOKEN20.transferFrom(msg.sender, deadAddress, deadAmount);
        uint256 lpAmount = (itemSale.price * lpfee) / 100;
        GTOKEN20.transferFrom(msg.sender, pancakeswapPair, lpAmount);

        GTOKEN20.transferFrom(
            msg.sender,
            itemSale.owner,
            itemSale.price - feeAmount - deadAmount - lpAmount
        );
        feeAccumulate += feeAmount;
        deadFeeAccumulate += deadAmount;
        lpFeeAccumulate += lpAmount;
        ...
        emit FillOrder(_tokenId, msg.sender);
    }
    ...
    function myInventory(address owner, uint256 levelcode)
        external
        view
        returns (uint256[] memory available, uint256[] memory marketplace)
    {
        uint256[] memory a = GTOKEN721.getTokens(owner);
        uint256[] memory m = sellerTokens[owner].values();
        if (levelcode == 0) {
            return (a, m);
        }

        available = new uint256[](a.length);
        marketplace = new uint256[](m.length);

        uint256 counter = 0;
        for (uint256 i = 0; i < a.length; i++) {
            ( , , , uint256 levelCode) = templar.getTokenInfo(a[i]);
        ...
        for (uint256 i = 0; i < m.length; i++) {
            ( , , , uint256 levelCode) = templar.getTokenInfo(m[i]);
            if (levelCode == levelcode) {
                marketplace[mcounter] = m[i];
                mcounter++;
            }
        }
        return (available, marketplace);
    }

    function allMarketplace() external view returns (uint256[] memory) {
        return tokenSales.values();
    }
