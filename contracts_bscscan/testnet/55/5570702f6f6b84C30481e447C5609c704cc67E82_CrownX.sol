        ...
        if (users[referrerAddress].x3Matrix[matrix][level].referrals.length <2**level) {
            ...
            if (users[referrerAddress].x3Matrix[matrix][level].referrals.length ==2**level) {
                ...
                if(matrixLevel[matrix]>=level+1){
                    ...
                    users[referrerAddress]._holdMatrixAmount[matrix][level]=users[referrerAddress]._holdMatrixAmount[matrix][level]-matrixPrice[matrix+1][1];
                    emit UserIncome(referrerAddress,referrerAddress,users[referrerAddress]._holdMatrixAmount[matrix][level], matrix, level, "Matrix Income");
                    users[referrerAddress].activeX3Levels[matrix+1][1]=true;
                    x3CurrentvId[matrix+1][1] = x3CurrentvId[matrix+1][1] + 1;
                    emit Upgrade(referrerAddress, matrix+1, 1);
                    users[referrerAddress]._holdMatrixAmount[matrix][level]=0;
                    uint256 newIndex = x3Index[matrix+1][1] + 1;
                    x3vId_number[matrix+1][1][newIndex] = referrerAddress;
                    x3Index[matrix+1][1] = newIndex;
                    address freeX3Refferrer = findFreeX3Referrer(matrix+1,1);
                    updateX3Referrer(referrerAddress,freeX3Refferrer,matrix+1,1);
                }
            }
        }
    }
