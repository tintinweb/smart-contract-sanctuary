    ...
    IChainLinkOracleSportData
{
    ...
    uint256 public payment = (1 * LINK_DIVISIBILITY) / 10;
    ...
    function setJobId(bytes32 _jobId, uint256 _payment) public {
        jobId = _jobId;
        payment = _payment;
    }
    ...
    function setPayment(uint256 _payment) public {
        payment = _payment;
    }
    ...
    function setToken(address _linkToken) public {
        setChainlinkToken(_linkToken);
    }

    function requestData(uint256 _matchId) public override returns (bytes32) {
        require(
            pool.checkBettingContractExist(msg.sender),
            "You don't have right call requestData"
        );
        ...
        request.add("matchId", _matchId.toString());
    ...
    function fulfill(bytes32 _requestId, bytes memory _score)
        public
        recordChainlinkFulfillment(_requestId)
    {
        ...
        results[_requestId] = bytesToUint256Array(_score);
        emit ReceiveData(_requestId, _score);
        ...
        returns (uint256[] memory)
    {
        ...
        uint256[] memory dataList = new uint256[](dataNb);
        uint256 index = 0;
