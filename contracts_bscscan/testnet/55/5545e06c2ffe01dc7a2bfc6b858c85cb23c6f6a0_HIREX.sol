...
contract HIREX is Context, IERC20, Ownable {
    ...
    string private _name    = "HIRE-X";
    string private _symbol  = "HIREX";
    ...
    uint256 public buyBackSellLimit = 1000000;
    ...
    uint256 public antiSnipingTime = 40 seconds; 
    ...
    constructor (address payable MarketingWallet, address BurnAddress, address UniSwapRouter) {

        _rOwned[_msgSender()] = _rTotal;

        marketingWallet = MarketingWallet;
        burnAddress = BurnAddress; // 0x000000000000000000000000000000000000dEaD
    ...
    function _addSniperInList(address account) external onlyOwner() {
        require(account != address(uniswapV2Router), 'We can not blacklist uniswapV2Router');
    ...
    function _transfer(
        ...
        if(_isExcludedFromMaxTx[from] == false && 
            ...
            require(amount <= _maxTxAmount,"ERC20: amount exceeded max limit");

            if (!_tradingOpen){
                    require(to != uniswapV2Pair, "Trading is not enabled");
            }

            if (block.timestamp < _launchTime + antiSnipingTime && from != address(uniswapV2Router)) {
    ...
    function changeRouter(address _router) public onlyOwner returns(address _pair) {
    ...
    function setWalletFee(address _address, bool _enable, uint256 _walletTaxFee, uint256 _walletLiquidityFee) external onlyOwner {
        _addressFees[_address].enable = _enable;
        _addressFees[_address]._taxFee = _walletTaxFee;
        _addressFees[_address]._liquidityFee = _walletLiquidityFee;
    }
    
    function setBuyWalletFee(address _address, bool _enable, uint256 _walletTaxFee, uint256 _walletLiquidityFee) external onlyOwner {
        _addressFees[_address].enable = _enable;
        _addressFees[_address]._buyTaxFee = _walletTaxFee;
        _addressFees[_address]._buyLiquidityFee = _walletLiquidityFee;
    }
    
    function setSellWalletFee(address _address, bool _enable, uint256 _walletTaxFee, uint256 _walletLiquidityFee) external onlyOwner {
        _addressFees[_address].enable = _enable;
        _addressFees[_address]._sellTaxFee = _walletTaxFee;
        _addressFees[_address]._sellLiquidityFee = _walletLiquidityFee;
    }
