...
contract HONEYPAD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    HONEYPADDividendTracker public dividendTracker;
    address public marketingWallet = 0xC58fe07095511D20963CadB03F66042699127687;
    address public buybackWallet = 0x9392c6FBE6a7bE44F491D597217ceA634c916Ff6;

    uint256 public maxTransactionAmount;
    uint256 public maxHoldingAmount;
    uint256 public swapTokensAtAmount = 200000 * (10**decimals());

    uint256 public buyback = 5;
    uint256 public marketingFee = 5;
    uint256 public dividendFee = 5;
    uint256 public totalFees = buyback + marketingFee + dividendFee;


    uint256 public sellBuybackFee = 7;
    uint256 public sellMarketingFee = 6;
    uint256 public sellDividendFee = 7;
    uint256 public totalSellFees = sellBuybackFee + sellMarketingFee + sellDividendFee;

    uint256 public accBuybackFee;
    uint256 public accMarketingFee;
    uint256 public accDividendFee;

    mapping(address => uint256[]) private accLastPeriodSells;
    ...
    uint256 public immutable tradingEnabledTimestamp;
    ...
    event SetMaxTxPercent(uint256 OldPercent, uint256 NewPercent);
    event SetMaxHoldingPercent(uint256 OldPercent, uint256 NewPercent);
    ...
    constructor(uint256 _tradingEnabledTimestamp) ERC20("HONEYPAD", "$HONEY") {
        tradingEnabledTimestamp = block.timestamp > _tradingEnabledTimestamp ? block.timestamp : _tradingEnabledTimestamp;
        dividendTracker = new HONEYPADDividendTracker();
...
//        updatePancakeRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function setMaxTxPercent(uint256 percent)  external onlyOwner {
        uint256 newAmount = totalSupply().mul(percent).div(100);
        emit SetMaxTxPercent(maxTransactionAmount, newAmount);
        maxTransactionAmount = newAmount;
    }

    function setMaxHoldingPercent(uint256 percent)  external onlyOwner {
        uint256 newAmount = totalSupply().mul(percent).div(100);
        emit SetMaxHoldingPercent(maxHoldingAmount, newAmount);
    ...
    function setBuyFees(uint256 _BuybackFee, uint256 _MarketingFee, uint256 _DividendFee)  external onlyOwner {
        ...
        totalFees = buyback + marketingFee + dividendFee;
    ...
    function setSellFees(uint256 _SellBuybackFee, uint256 _SellMarketingFee, uint256 _SellDividendFee)  external onlyOwner {
        ...
        totalSellFees = sellBuybackFee + sellMarketingFee + sellDividendFee;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "HONEYPAD: The dividend tracker already has that address");

        HONEYPADDividendTracker newDividendTracker = HONEYPADDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "HONEYPAD: The new dividend tracker must be owned by the HONEYPAD token contract");
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "HONEYPAD: The router already has that address");
        ...
        _isExcludedFromPeriodLimit[address(pancakePair)] = true;

    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "HONEYPAD: Account is already the value of 'excluded'");
    ...
    function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPeriodLimit[account] != excluded, "HONEYPAD: Account is already the value of 'excluded'");
    ...
    function excludeFromAmountLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromAmountLimit[account] != excluded, "HONEYPAD: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "HONEYPAD: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "HONEYPAD: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "HONEYPAD: This account cannot send tokens until trading is enabled");
        }
        ...
        if( !swapping &&
        tradingIsEnabled &&
        !_isExcludedFromAmountLimit[from] &&
        !_isExcludedFromAmountLimit[to] ) {
        ...
        if(takeFee) {
            uint256 fees;
            if (to == pancakePair) {
                accBuybackFee = accBuybackFee.add(amount.mul(sellBuybackFee).div(100));
                accMarketingFee = accMarketingFee.add(amount.mul(sellMarketingFee).div(100));
                accDividendFee = accDividendFee.add(amount.mul(sellDividendFee).div(100));
                fees = amount.mul(totalSellFees).div(100);
            } else {
                accBuybackFee = accBuybackFee.add(amount.mul(buyback).div(100));
                accMarketingFee = accMarketingFee.add(amount.mul(marketingFee).div(100));
                accDividendFee = accDividendFee.add(amount.mul(dividendFee).div(100));
        ...
        if (!_isExcludedFromAmountLimit[to]) {
...
contract HONEYPADDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("HONEYPADDividendTracker", "HONEY_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "HONEYPADDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "HONEYPADDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main HONEY contract.");
    }
