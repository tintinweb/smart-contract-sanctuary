...
library Strings {
    ...
    function toString(uint256 value) internal pure returns (string memory) {
        ...
        temp = value;
        while (temp != 0) {
            //buffer[index--] = bytes(uint8(48 + (temp % 10)));
            temp /= 10;
        }
...
contract BuyScrolls is Ownable, RandomNumberConsumer{
    ...
    mapping(uint256 => uint256) public usersRandomNum;
    ...
    function buy(
        ...
        public returns (bool) {
        require(_tokenAmount == _noofScrolls.mul(scrollPrice), "Invalid amount");
        uint256 split;
    ...
    function mintCity() public returns (uint256 tokenId) {
        require(noOfCities[msg.sender] > 0, "Purchage City to Mint NFT");
        require(randomResult > 0);

        tokenId = scrollNFT.newItem(msg.sender, randomResult);
        getRandomNumber();
        usersRandomNum[tokenId] = randomResult;
        NFTInfo[tokenId] = randomResult;
        
        noOfCities[msg.sender] = noOfCities[msg.sender].sub(1);
        emit MintCity(tokenId);
        return tokenId;
    }
