...
/// Main contract of the EuterpeToken.
/// Operators (usually manager contract) can mint tokens and distribute to users.
/// Owner can add/remove Operators.
contract EuterpeToken is ERC20("EuterpeToken", "EUTERPE"), Mintable {
    constructor(uint _totalSupplyHardLimit) Mintable(_totalSupplyHardLimit){
    }
}
...
abstract contract Mintable is ERC20, Ownable{
    uint public totalSupplyHardLimit;
     ...
     * @param _totalSupplyHardLimit Total hard limit supply of Token
     */
    constructor(uint256 _totalSupplyHardLimit){
        totalSupplyHardLimit = _totalSupplyHardLimit;
    ...
    function mint(address _to, uint256 _amount) external onlyMinter {
        require(_amount + totalSupply() <= totalSupplyHardLimit, "exceed max supply");
        _mint(_to, _amount);
    }
