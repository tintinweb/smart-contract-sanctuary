...
contract PositionToken is Context, IERC20, Ownable {
    ...
    bool private _isRegisterWhitelistSaleDistribution;
    ...
    function distributeWhitelistSale(address _receiver, uint _value) public {
        require(_isRegisterWhitelistSaleDistribution, "not registered");
    ...
    function registerWhitelistSaleDistribution() public onlyOwner {
        require(!_isRegisterWhitelistSaleDistribution, "Already registered");
        uint256 _currentRate = _getRate();
        _tTotal = _tTotal.add(WHITELIST_SALE_AMOUNT);
        // rTotal should increase as well
        _rTotal = _currentRate.mul(_tTotal);
        _isRegisterWhitelistSaleDistribution = true;
    }
    ...
    function setWatcher(address _newKeeper) public onlyOwner {
