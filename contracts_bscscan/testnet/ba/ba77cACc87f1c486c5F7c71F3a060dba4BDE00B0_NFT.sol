...
contract NFT is ERC721, Ownable{
   ...
   */
    ...
    modifier onlyBox {
        require(box == msg.sender, "Ownable: caller is not the box");
        _;
    }

    function mint(address to, uint256 rarity) external onlyMinter returns(uint256){
        uint256 tokenId = _tokenIdTracker.current();
        _tokenIdTracker.increment();
        _mint(to, tokenId);
        Nft memory nft_ = Nft(rarity);
        nfts[tokenId] = nft_;
        return tokenId;
    }

    function boxRandomMint(address to, uint256 sourceId) external onlyBox returns(uint256){
        uint256 tokenId = _tokenIdTracker.current();
        _tokenIdTracker.increment();
        _mint(to, tokenId);
        uint256 rarity = random(sourceId, to, 4);
        Nft memory nft_ = Nft(rarity + 1);
        nfts[tokenId] = nft_;
        return tokenId;
    }


    function random(uint256 seed, address salt, uint256 total)
        internal
        view
        returns (uint256)
    {
        return uint256(keccak256(abi.encodePacked(block.timestamp, seed, salt))) % total;
    }
