...
// File: openzeppelin-solidity\contracts\token\ERC721\IERC721Enumerable.sol
...
// File: openzeppelin-solidity\contracts\token\ERC721\ERC721Enumerable.sol
...
// File: openzeppelin-solidity\contracts\token\ERC721\IERC721Metadata.sol
...
// File: openzeppelin-solidity\contracts\token\ERC721\ERC721MetadataMintable.sol
...
contract ERC721MetadataMintable is ERC721, ERC721Metadata, MinterRole {
  ...
  //   string royalty;
  // }
  // mapping (address => artistInfo) private artist;
  //  modifier onlyArtist() {
  //   require(msg.sender == artist[msg.sender].artistAddress, "only artist can mint NFTs");
  //   _;
  // }
  
  // function registerArtist(string memory logo, string memory name, string memory icon, string memory royalty) public {
  //   require(msg.sender != artist[msg.sender].artistAddress);
  //   artist[msg.sender].artistAddress = msg.sender;
  //   artist[msg.sender].logo = logo;
  //   artist[msg.sender].name = name;
  //   artist[msg.sender].icon = icon;
  //   artist[msg.sender].royalty = royalty;
  // }
  // onlyArtist
...
// File: contracts\LPBNBInterface.sol
...
contract NftDexV4 is ERC721Full, ERC721MetadataMintable, Ownable {
  IERC20 ALIA;
 ...
 // events
  ...
  event SellNFT(address indexed from, address nft_a, uint256 tokenId, address seller, uint256 price, uint256 dollarPrice);
  ...
  function sellNftModifier(uint256 tokenId, address seller, uint256 price, uint256 priceDollar) internal {
     require(price > 0 || priceDollar > 0, "NFT: 100");
  ...
  function sellNFT(address nft_a,uint256 tokenId, address seller, uint256 price, uint256 priceDollar) public returns (bool) {
    ...
    emit SellNFT(msg.sender, nft_a, tokenId, seller, price, priceDollar);
    return true;
  }
    
    function MintAndSellNFT(address to, string memory tokenURI,uint256 quantity, bool flag, uint256 price, string memory ownerId, uint256 priceDollar) public returns (bool) { 
      uint256 tokenId;
  ...
  function updatePrice(uint256 tokenId, uint256 newPrice, uint256 newDollarPrice) public{
    ...
    require(newPrice > 0 || newDollarPrice > 0,"NFT: 111");
    if(_saleTokens[tokenId].seller != address(0)){
          if(newPrice == 0){
  ...
  function buyNFT(address nft_a,uint256 tokenId, address buyer) public returns (bool) {
        ...
        require(_saleTokens[tokenId].price > 0 || _dollarPrice[tokenId] > 0 , "NFT: 108");
        uint256 price;
        if(_saleTokens[tokenId].price > 0){
          price = _saleTokens[tokenId].price;
        } else {
           (uint112 _reserve0, uint112 _reserve1,) =LPBNB.getReserves();
           (uint112 reserve0, uint112 reserve1,) =LPAlia.getReserves();
           price = (_dollarPrice[tokenId]/ ((_reserve1/_reserve0) * (reserve1 /reserve0)));
        }
        ALIA.transferFrom(buyer, _saleTokens[tokenId].seller, (price  / 1000) * mainPerecentage);
  ...
  function init4() public {
    require(!isInitialized4);
    LPAlia=LPInterface(0x52826ee949d3e1C3908F288B74b98742b262f3f1);
    LPBNB=LPBNBInterface(0xe230E414f3AC65854772cF24C061776A58893aC2);
    isInitialized2=true;
  }

  function getPrice(uint256 tokenId) public view returns(uint256, uint112, uint112, uint112, uint112) {
           ( uint112 _reserve0, uint112 _reserve1,) =LPBNB.getReserves();
           (uint112 reserve0, uint112 reserve1,) =LPAlia.getReserves();
          return  ( (_dollarPrice[tokenId] * _reserve0 * reserve0) /(_reserve1 * reserve1), _reserve0, _reserve1, reserve0, reserve1 );
  }
}