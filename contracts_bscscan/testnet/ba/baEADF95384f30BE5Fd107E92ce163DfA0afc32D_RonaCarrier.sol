...
contract RonaCarrier is OwnableContext {
    ...
    uint256 private _blockDistributionPoolHoldings = 0;
    uint256 private _blockDistributionDividendsCount = 0;
    mapping(address => uint256) private _blockDistributionDividendsCountPrevious;
    uint256 private _blockDistributionNumberRecievers = 0;
    ...
    // ----- BLOCKED DISTRIBUTON FUNCTIONS ----- //
    // ----------------------------------------- //
    function _block(uint256 distributionAmount, address blockInitiator, address blockCloser) internal {
       _blockDistributionPoolHoldings = _blockDistributionPoolHoldings.add(distributionAmount);
       _blockDistributionDividendsCount = _blockDistributionDividendsCount.add(distributionAmount);

       if(_blockDistributionDividendsCountPrevious[blockInitiator] == 0 && _carrierToken.balanceOf(blockInitiator) > 0) {
            _blockDistributionDividendsCountPrevious[blockInitiator] = _blockDistributionDividendsCount;
            _blockDistributionNumberRecievers = _blockDistributionNumberRecievers.add(1);
       } else if(_carrierToken.balanceOf(blockInitiator) > 0){
           uint256 owedToInitiator = _blockDistributionDividendsCount.sub(_blockDistributionDividendsCountPrevious[blockInitiator]).div(_blockDistributionNumberRecievers);
           if(owedToInitiator > 0) {
               _carrierToken.transfer(blockInitiator, owedToInitiator);
               _blockDistributionPoolHoldings = _blockDistributionPoolHoldings.sub(owedToInitiator);
               _blockDistributionDividendsCountPrevious[blockInitiator] = _blockDistributionDividendsCount;
           }
       } else {
           _blockDistributionDividendsCountPrevious[blockInitiator] = 0;
           _blockDistributionNumberRecievers = _blockDistributionNumberRecievers.sub(1);
       }

       if(blockInitiator != blockCloser) {
           if(_blockDistributionDividendsCountPrevious[blockCloser] == 0 && _carrierToken.balanceOf(blockCloser) > 0) {
               _blockDistributionDividendsCountPrevious[blockCloser] = _blockDistributionDividendsCount;
               _blockDistributionNumberRecievers = _blockDistributionNumberRecievers.add(1);
           } else if(_carrierToken.balanceOf(blockCloser) > 0) {
               uint256 owedToCloser = _blockDistributionDividendsCount.sub(_blockDistributionDividendsCountPrevious[blockCloser]).div(_blockDistributionNumberRecievers);
               if(owedToCloser > 0){
                    _carrierToken.transfer(blockCloser, owedToCloser);
                    _blockDistributionPoolHoldings = _blockDistributionPoolHoldings.sub(owedToCloser);
                    _blockDistributionDividendsCountPrevious[blockCloser] = _blockDistributionDividendsCount;
               }
           } else {
               _blockDistributionDividendsCountPrevious[blockCloser] = 0;
                _blockDistributionNumberRecievers = _blockDistributionNumberRecievers.sub(1);
           }
       }
    }
    ...
    function carry(address from, address to, uint256 amount) external returns (bool) {
        ...
        _block(holdersFeeAmount, from, to);

        emit Carry(from, to, amount);

        return true;
    }
    
    event Carry(address from, address to, uint256 amount);
    ...
    function purchase() public payable returns (bool) {
        ...
        _block(holdersFeeAmount, _msgSender(), _msgSender());
    ...
    function _purchaseRate() internal view returns (uint256) {
        uint256 currentPurchaseRate = _carrierToken.balanceOf(address(this)).sub(_blockDistributionPoolHoldings).mul(_liquidityTokenDecimalFactor).div(address(this).balance);
    ...
    function _purchaseRateDeducted(uint256 deduction) internal view returns (uint256) {
        uint256 currentPurchaseRate = _carrierToken.balanceOf(address(this)).sub(_blockDistributionPoolHoldings).mul(_liquidityTokenDecimalFactor).div(address(this).balance.sub(deduction));
        if(currentPurchaseRate > _maxPurchaseRate){
            return _maxPurchaseRate;
        } else {
            return currentPurchaseRate;
    ...
    function liquidate(uint256 tokensToLiquidate) public returns (bool) {
        ...
        require(avalible >= owed, "Liquidate: not enough carrier liquidity avalible");
    ...
    function _liquidateInternal(address payable recipiant, uint256 tokensToLiquidate, uint256 deduction) internal {
        ...
        require(avalible >= owed, "Liquidate internal: not enough carrier liquidity avalible");
    ...
    function _liquidationRate() internal view returns (uint256) {
        return address(this).balance.mul(_carrierTokenDecimalFactor).div(_carrierToken.totalSupply().sub(_carrierToken.balanceOf(address(this)).sub(_blockDistributionPoolHoldings)));
    }

    function _liquidationRateDeducted(uint256 deduction) internal view returns (uint256) {
        return address(this).balance.mul(_carrierTokenDecimalFactor).div(_carrierToken.totalSupply().sub(_carrierToken.balanceOf(address(this)).sub(_blockDistributionPoolHoldings).add(deduction)));
    }
    ...
    function claim(uint256 tokensToClaim) public returns (bool) {
        ...
        require(avalible >= tokensToClaim, "Claim: not enough carrier token avalible");
