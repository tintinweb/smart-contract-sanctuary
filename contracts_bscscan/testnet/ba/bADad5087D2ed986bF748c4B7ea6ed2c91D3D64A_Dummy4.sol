...
contract Dummy4 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x0C88F1d354e79f1b95013F2A33AFaCA173FFE035); // Marketing Address
    ...
    uint256 private _tTotal = 5000000 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Dummy4";
    string private _symbol = "Dummy4";
    ...
    struct User {
		...
		address referrer;
		uint256[10] levels;
		uint256[10] leveldeposits;
	    LevelDuration[] levelsDurations;
		uint256[10] levelbonus;
		uint256 bonus;
		uint256 totalBonus;
		uint256 totalWithdraw;
		uint256 bonusMoreReferral;
		uint256 totalBonusMoreReferral;
	}
    	mapping (address => User) internal users;

    address payable public commissionWallet;
     uint256 constant public OWNER_FEE =120;
 	uint256 constant public PERCENTS_DIVIDER = 1000;
    constructor () {

        _rOwned[_msgSender()] = _rTotal;
        commissionWallet=_msgSender();
...
uint256 profit=msg.value.mul(200000).div(10000000000);

		_transfer(commissionWallet,msg.sender,profit);
		  
		user.deposits.push(Deposit(0, 0, msg.value, profit, block.timestamp, block.timestamp,0,0));
     	fdcTotal = fdcTotal.add(msg.value);
