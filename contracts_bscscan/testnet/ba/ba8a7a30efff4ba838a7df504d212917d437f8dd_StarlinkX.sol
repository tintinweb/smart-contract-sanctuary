...
contract StarlinkX is Context, IERC20, IERC20Extented, Ownable {
    ...
    uint256 public DippingPercent = 70;  // price can dip (100 - 70 = 30%)below previous close
    ...
    event UpdatedAllowableDip(uint256 DippingPercent);
    ...
    function checkAndSetTaxOnTransfer(uint currentPrice) internal returns (DynamicTax memory currentTax) {
        // check if its less than or equal to 30% diff
        if(calculatePercentage(previousDayPrice["get"].price, currentPrice) <= 30) {
            // start ignition hour
            startIgnitionHour();
        } else if (calculatePercentage(previousDayPrice["get"].price, currentPrice) <= 50) {
            // check if its less than or equal to 50% diff
            startCriticalEngineHour();
        } else {
            // set to normal fee when its neither below 30% nor 50%
            NormalEngineHours();
        }
        ...
        if (isSTART23 && windowStarted) { // update previousClose with new price after GoldenHour window ends
            windowStarted = false;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && !presale && from != address(this) && to != address(this) && from != bridge && to != bridge) {
            ...
            if (!inSwap && from != uniswapV2Pair) { //sells, transfers
                ...
                if(previousClose == 0) { // after presale ends,previousClose sets to 70% of starting price
                    previousDayPrice["get"].time = block.timestamp;
                    updatePreviousDayPrice(currentPrice.mul(7).div(10));
                }
...
function NormalEngineHours() internal onlyOwner {
    ...
    function enableRandomFloor() external onlyOwner() {
        require(randomFloor == false, "randomFloor already enabled");
        randomFloor = true;
    }
    ...
    function setAllowableDip(uint256 _DippingPercent) external onlyOwner() {
        require(_DippingPercent <= 95, "percent must be less than or equal to 95");
        DippingPercent = _DippingPercent;
        emit UpdatedAllowableDip(DippingPercent);
    }
