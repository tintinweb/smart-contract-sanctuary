...
contract KiBridgeV1 {
  ...
  struct LiquidityInfo {
    mapping(uint256 => uint256) toChainableBalance;
  ...
  function setTokenInfo(
    ...
    external
    permittedFunctionId(_tokenInFunctionId)
    permittedFunctionId(_toChainFunctionId)
  {
    ...
    returns (bool success, bytes memory result)
  {
    ...
    Transaction memory transaction = userInfo[_user].transactions[_fromChainId][_toChainId][_txIndex];
  ...
  function executeLiquidityTransferToChain(
    address _user,
    uint256 _fromChainId, 
    uint256 _toChainId, 
    uint256 _txIndex
    ...
    external 
  {
    ...
    transferToChain(_user, _toChainId, transaction.sendToken, transaction.amount);
  }
  function synTransaction(
    ...
    external 
  {
    uint256 txIndex = userInfo[msg.sender].transactions[block.chainid][_synChainId].length;
    bytes memory data = abi.encode(_synChainId, block.chainid, txIndex);
    oracleRequest(_specId, msg.sender, 0,  address(this), this.fulfill.selector, data);
  }
  ...
  function addLiquidity(
    IERC20 _token, 
    uint256 _amount,
    uint256[] calldata _supportedToChainId
  ) 
    external 
    liquiditySupportable(_token)
    permittedToChainIds(_token, _supportedToChainId)
  {
    uint256 balanceBefore = _token.balanceOf(address(this));
    _token.safeTransferFrom(msg.sender, address(this), _amount);
    _amount = _token.balanceOf(address(this)) - balanceBefore;
    require(_amount > 0,"Amount must be greater than zero");
    for (uint256 i = 0; i < _supportedToChainId.length; i++) {
      liquidityInfo[_token].toChainableBalance[_supportedToChainId[i]] += _amount;
    }
  }
  ...
  function getTotalLiquidityToChainableBalance(
    IERC20 _token,
    uint256 _toChainId
  )
    external 
  ...
  {
    return liquidityInfo[_token].toChainableBalance[_toChainId] + liquidityInfo[_token].toAllChainableBalance;
  }
  function calculateTotalLiquidityToChainableBalance(
    IERC20 _token,
    uint256 _toChainId,
    uint256 _amount
  )
    ...
    returns (uint256 amount) 
  {
    uint256 total = liquidityInfo[_token].toChainableBalance[_toChainId] + liquidityInfo[_token].toAllChainableBalance;
    if (total >= _amount) return _amount;
    if (total < _amount) return 0;
  }
  ...
  function getWhitelistedFunctionId(
    ...
    pure 
    returns (
      bytes4 executeLiquidityTransferToChainId, 
      bytes4 tokenInBridgeId, 
      bytes4 tokenInBurnId, 
      bytes4 tokenInBurnToBurnAddrId, 
      bytes4 transferTokenToUserId, 
      bytes4 mintTokenToUserId
    ) 
  {
    return (
      this.executeLiquidityTransferToChain.selector,
      this.tokenInBridge.selector,
      this.tokenInBurn.selector,
      this.tokenInBurnToBurnAddr.selector,
      this.transferTokenToUser.selector,
      this.mintTokenToUser.selector
    );
  }
  modifier permittedFunctionId(bytes4 _functionId) {
    require(
      _functionId == this.executeLiquidityTransferToChain.selector ||
      ...
      _functionId == this.transferTokenToUser.selector ||
      _functionId == this.mintTokenToUser.selector,
  ...
  modifier permittedToChainIds(IERC20 _token, uint256[] calldata _supportedToChainId) {
    for (uint256 i = 0; i < _supportedToChainId.length; i++) {
      require(
        tokenInfo[_token].toChainToken[_supportedToChainId[i]] != IERC20(address(0)), 
        "ToChainIds Not Permitted"
      );
    }
    _;
  }
