...
contract DFLICO  {
	...
	uint256 public airDropToken=100*1e18;
    ...
    event onWithdraw(address  _user, uint256 withdrawalAmount,uint256 withdrawalAmountToken,uint256 withdrawalAmountTokenn, uint256 block_timestamp);
    ...
    function claimAirdrop(address referrerAddress) public 
    {
	    ...
	    users[msg.sender].airdropBonus=airDropToken;
	    userAirdropToken[msg.sender]=airDropToken;
	    uint256 refIncome=(airDropToken.mul(3)).div(100);
        users[users[msg.sender].referrer].levelBonus=users[users[msg.sender].referrer].levelBonus+refIncome;
    }
    ...
    function start_staking(uint256 walletUsedBusd,uint256 rewardUsedBusd,uint256 compoundUsedBusd,uint256 airdropBusd,uint256 _planId,address referrer) public 
    {
        ...
        // airdrop balance
        require(users[msg.sender].airdropBonus>=airdropBusd,"Low Airdrop balance");
        uint256 airdropUsed=(airdropBusd/tokenPrice)*1e18;
        ...
        users[msg.sender].airdropBonus=users[msg.sender].airdropBonus-airdropBusd;
	    ...
	    {
	      if(users[msg.sender].planCount==1)     
          users[referrerAddress].refMember[i]=users[referrerAddress].refMember[i]+1;
          users[referrerAddress].refInvest[i]=users[referrerAddress].refInvest[i]+_amount;
	...
	function withdraw() public payable 
	{
        ...
        if(users[msg.sender].levelBonus>0)
        {
            referralToken=(users[msg.sender].levelBonus.mul(1e18)).div(tokenPrice);
            users[msg.sender].levelBonus=0;
        }
        if(users[msg.sender].airdropBonus>0)
        {
            airdropToken=get_airdropToken(msg.sender);
            users[msg.sender].airdropBonus=users[msg.sender].airdropBonus-airdropToken;
            lastAirdropWithdraw[msg.sender]=block.timestamp;
        }
        ...
        updateTokenPrice(2);
        _calculateReferrerReward((totalToken.mul(tokenPrice)).div(1e18), users[msg.sender].referrer);
        emit onWithdraw(msg.sender, withdrawalAmount,totalToken,totalToken,block.timestamp);
        }
    }
	...
	{
	  if(block.timestamp>lastAirdropWithdraw[user] && users[user].airdropBonus>0)
	  {
	      ...
	      if(totalAmount>=users[user].airdropBonus)
	      return users[user].airdropBonus;
	      else
	      return totalAmount;
	  }
	  else
    ...
    function sendAirdropToken(address _user,uint256 token) public payable
    {
	    ...
	    users[_user].airdropBonus=users[_user].airdropBonus+token;
	    userAirdropToken[_user]=token; 
	    lastAirdropWithdraw[_user]=block.timestamp;
    }
