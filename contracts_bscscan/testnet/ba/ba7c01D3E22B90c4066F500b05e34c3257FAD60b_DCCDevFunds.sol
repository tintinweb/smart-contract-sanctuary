...
contract DCCDevFunds is Context, Ownable {
  ...
  mapping(address => uint256) public funderIndex;
  Funder[] funderData;
  ...
  function addFunder(
    ...
    if (funderIndex[_address] == 0 && _address != _thisAddress()) {
      ...
      _funder.data.push(Balance(_address, amount));
    }
  ...
  function addFunds(address _address, uint256 amount) external returns (bool) {
    ...
    uint256 allowance = IERC20(_address).allowance(_msgSender(), _thisAddress());
