...
contract Appetite is Context, IERC20, Ownable {
    ...
    uint8 private _reflectionMultiplier = 100; // Min 1; Max 255
    ...
    function setReflectionMultiplier(uint8 newMultiplier) external onlyOwner {
        require(
            newMultiplier >= 1 && newMultiplier < ~uint8(0),
            "Limit exceeds allowed amount"
        );

        _reflectionMultiplier = newMultiplier;
    }
    ...
    function setRateIndexOne(uint8 index, uint32 value) external onlyOwner {
        ...
        if (index <= 3)
            require(value <= _DENOMINATOR, "Value for rate too high");

        if (index == 0 || index == 1) {
            if (index == 0) {
                _setRate(0, value);
                _setRate(1, _DENOMINATOR - value);
            } else {
                _setRate(0, _DENOMINATOR - value);
                _setRate(1, value);
            }
        } else if (index == 2 || index == 3) {
            if (index == 2) {
                _setRate(2, value);
                _setRate(3, _DENOMINATOR - value);
            } else {
                _setRate(2, _DENOMINATOR - value);
                _setRate(3, value);
            }
        } else {
    ...
    function swapTokensForBNBAndSend(uint128 amount) public LockSwap onlyOwner {
        ...
        if (liquidityRate > 0) {
            liquidityFee = (amount * liquidityRate) / _DENOMINATOR;
    ...
    /**
     * Send Charity Rate
     * Send Reflection Rate
     * Buy Charity Rate
     * Buy Reflection Rate
     * Buy Marketing Rate
     * Buy Support Stream Rate
     * Sell Charity Rate
     * Sell Reflection Rate
     * Sell Marketing Rate
     * Sell Support Stream Rate
     * Anti Whale Rate
     */
    ...
    function _transfer(
        ...
        if (!_lockSwap) {
            ...
            uint8 transferType = _getTransferType(from, to, pancakePair);
            if (transferType != 1) {
                uint128 charityFee;
    ...
    function _initialSupportBurn(
        ...
        if (
            ...
            uint128 amountToBurn = (amount *
                uint32(bytes4(contractRatesOne << 224))) / _DENOMINATOR;
    ...
    function _prepAndSellTokens(
        ...
        uint128 liquidityFee;
        if (liquidityRate > 0) {
            liquidityFee = ((tokensToSell * liquidityRate) / _DENOMINATOR) / 2;
    ...
    function finishTransfer(
        ...
        uint128 reflectionFeeWithRate = (reflectionFee * _reflectionMultiplier) *
            getReflectionRate();
        ...
        if (antiWhaleCheck) {
            uint128 antiWhaleLimit = (_tokenSupply *
                uint16(bytes2(contractRatesZero << 160))) / _DENOMINATOR;
    ...
    function _getTransferType(
        ...
        address to,
        address pancakePair
    ) private view returns (uint8 transferType) {
        ...
        if (isExcludedFrom.fromFee || isExcludedTo.fromFee) {
            ...
            transferType = to == pancakePair ? 4 : 3;
        }
    }
    ...
    function _setTokenTrackers(
        ...
        trackers.tokensForSelling += charityFee + marketingFee;

        if (supportFee > 0) {
            (uint128 buybackFee, uint128 liquidityFee) = _getSupportFees(
                transferType,
                supportFee
            );
            ...
            trackers.tokensForSelling += buybackFee + liquidityFee;
        }
    ...
    function _getInitialFees(uint128 amount, uint8 transferType)
        private
        view
        returns (
            uint128 charityFee,
        ...
        )
    {
        ...
        if (charityRate > 0) {
            charityFee = (amount * charityRate) / _DENOMINATOR;
        }

        if (reflectionRate > 0) {
            reflectionFee = (amount * reflectionRate) / _DENOMINATOR;
        }

        if (marketingRate > 0) {
            marketingFee = (amount * marketingRate) / _DENOMINATOR;
        }

        if (supportStreamRate > 0) {
            supportStreamFee = (amount * supportStreamRate) / _DENOMINATOR;
        }
    }

    function _getRates(uint8 transferType)
        private
        view
        returns (
            uint16 charityRate,
    ...
    function _getSupportFees(uint8 transferType, uint128 supportFee)
        private
        view
        returns (uint128 buybackFee, uint128 liquidityFee)
    {
        uint32 buybackRate;
        uint32 liquidityRate;
        ...
        if (liquidityRate > 0)
            liquidityFee = ((supportFee * liquidityRate) / _DENOMINATOR) / 2;
        if (buybackRate > 0)
            buybackFee = (supportFee * buybackRate) / _DENOMINATOR;
    }
