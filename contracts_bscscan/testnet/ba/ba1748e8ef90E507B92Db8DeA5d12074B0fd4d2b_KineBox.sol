...
contract KineBox is AccessControl, Pausable, ERC1155Holder {
    ...
    function addMaxSupply(uint256 amount) external onlyRole(MANAGER_ROLE){
        uint256 old = maxSupply;
        maxSupply += amount;

        emit AddedMaxSupply(old,maxSupply);
    }
    ...
    function updateRate(uint256 level, uint256[] memory tokens, uint256[] memory values) external onlyRole(MANAGER_ROLE) {
        ...
        require(totalRates == 10000, "Invalid values");
    ...
    function open(uint256 seed) external whenNotPaused {
        ...
        uint256 randomNumber = _randomNumber(seed, 10000);
