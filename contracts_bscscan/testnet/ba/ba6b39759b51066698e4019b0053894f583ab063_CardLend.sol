...
contract CardLend is Ownable {
    ...
    constructor () public {
          ...
          clearingReceivingAddress = address(this);
    ...
    function getOraclePairCardUsdt() public view returns (uint256) {
          ...
          return pairCard.mul(1000000000).mul(1000000).div(pairUsdt);// add .mul(1000000000)
    }

    function updateOraclePairCardUsdt() public returns (bool) {
          uint256 pairCard = cardTokenContract.balanceOf(oraclePairCardAddress);
          uint256 pairUsdt = usdtTokenContract.balanceOf(oraclePairCardAddress);
          cardPriceUsdt = pairCard.mul(1000000000).mul(1000000).div(pairUsdt);// add .mul(1000000000)
    ...
    function lendClearing(address _account, uint256 _lendAccountJoinCount) public returns (bool) {
        ...
        // lendClearing
        uint256 clearingPrice = order.pledgeAmount.mul(1000000000).mul(10000).div(exitAmount).mul(1000000).div(10000);// add .mul(1000000000)
    ...
    function joinLend(uint256 _joinCardAmount,uint256 _lendingRate) public returns (bool) {
        ...
        uint256 lendAmount = _joinCardAmount.mul(1000000000).div(cardPriceUsdt).mul(1000000).mul(_lendingRate).div(100);// add .mul(1000000000)
        ...
        uint256 clearingPrice = _joinCardAmount.mul(1000000000).mul(100000000).div(lendAmount).mul(1000000).div(100000000);// add .mul(1000000000)
