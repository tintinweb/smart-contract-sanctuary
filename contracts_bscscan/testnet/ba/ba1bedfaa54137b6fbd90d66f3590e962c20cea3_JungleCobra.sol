...
contract JungleCobra is BEP20{
    ...
    // Contract address that need sto be exclued from Tax, like Presale Cotract!
    address public noTaxContractAddr = 0xBB7672E9Dac09326C327f9408c64A2051C34Cba3;
    ...
    // Constructs the JungleCobra contract.
    
    constructor() public BEP20('JungleCobra', 'COBRA') {
        ...
        _excludedFromAntiWhale[noTaxContractAddr] = true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        if (recipient == BURN_ADDRESS || recipient == FEE_ADDRESS || sender == DEV_ADDRESS  || sender == noTaxContractAddr ) {
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= MAXIMUM_TRANSFER_RATE, "COBRA::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxTransferAmountRate >= MINIMUM_TRANSFER_RATE, "COBRA::updateMaxTransferAmountRate: Max transfer amount rate can not be bleow minimum rate.");
    ...
    function SetContractAddress(address _Contract) public onlyOperator {
        noTaxContractAddr = _Contract;
        emit SetCONTRACTAddress(msg.sender, _Contract);
    }
...
// FutureMasterChef is the master of Cobra. He can make Cobra and he is a fair guy.
//
...
contract FutureMasterChefCobra is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // Max possibe emission rate: 10 COBRA per block.
    uint256 public constant MAXIMUM_EMISSION_RATE = 1000 finney;

    // Initial emission rate: 0.5 COBRA per block.
    uint256 public constant INITIAL_EMISSION_RATE = 500 finney;
    
    // Minimum emission rate: 0.00 COBRA per block.
    uint256 public constant MINIMUM_EMISSION_RATE = 0 finney;
    
    // Minimum emission rate: 0.00 COBRA per block.
    uint256 public constant STOP_EMISSION_RATE = 0 finney;
    
    // Reduce emission every 28800 blocks ~ 24 hours.
    uint256 public constant EMISSION_REDUCTION_PERIOD_BLOCKS = 28;
    ...
    constructor(
        ...
        cobraPerBlock = INITIAL_EMISSION_RATE;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cobraReward = multiplier.mul(cobraPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        uint256 feeAmount = cobraReward.mul(45).div(1000);
        uint256 markAmount = cobraReward.mul(15).div(1000);
        
        cobra.mint(feeAddress, feeAmount);
        cobra.mint(markAddress, markAmount);
    ...
    // Deposit LP tokens to FutureMasterChef for COBRA allocation.
    ...
    function safeCobraTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > cobraBal) {
            transferSuccess = cobra.transfer(_to, cobraBal);
        } else {
            transferSuccess = cobra.transfer(_to, _amount);
        }
        require(transferSuccess, "safeCobraTransfer: Transfer failed");
    }
   ...
   function updateEmissionRate() public {
        ...
        if ((cobraPerBlock <= MINIMUM_EMISSION_RATE)){
            return;
        }
        ...
        if (currentIndex <= lastReductionPeriodIndex) {
           ...
           uint256 newEmissionRate = cobraPerBlock;
           
           if ( newEmissionRate > 0)
           {
                uint256 rewardPerBlock = INITIAL_EMISSION_RATE;
                if(currentIndex <=1)
                {
                     rewardPerBlock = INITIAL_EMISSION_RATE.mul(25).div(10); 
                }
                else if (currentIndex > 18 && currentIndex <= 21 )
                {
                    rewardPerBlock = INITIAL_EMISSION_RATE.mul(10);
                }
                else if (currentIndex > 21)
                {
                    rewardPerBlock = STOP_EMISSION_RATE; // assuming constant STOP_EMISSION_RATE = 0
                }
                newEmissionRate = rewardPerBlock;
           }


        if (newEmissionRate != STOP_EMISSION_RATE) 
        {
          newEmissionRate = newEmissionRate < MINIMUM_EMISSION_RATE ? MINIMUM_EMISSION_RATE : newEmissionRate;
        }

        if (newEmissionRate >= MAXIMUM_EMISSION_RATE) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = cobraPerBlock;
        cobraPerBlock = newEmissionRate;
