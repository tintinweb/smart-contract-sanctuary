...
interface ILimitrMarketTokenETH {
    ...
    /// @return The order ID
    function newOrderForETHWithPointer(
        uint256 price,
        uint256 amount,
        address trader,
        uint256 deadline,
    ...
    /// @return The order ID
    function newOrderForETHWithPointers(
        uint256 price,
        uint256 amount,
        address trader,
        uint256 deadline,
    ...
    /// @return cost The amount of counter token spent
    /// @return received The amount of base token received
    function tradeETHAtAveragePrice(
...
/// @author Limitr
/// @title Trade market contract for Limitr
contract LimitrMarketTokenETH is LimitrMarketTokenToken,  ILimitrMarketTokenETH {
    ...
    {
        require(
            ILimitrFactory(_factory).weth() == _counterToken,
            'counter token is not WETH'
        );
    }

    mapping(uint256 => bool) _ETHOrders;
    ...
    {
        orderID = newOrder(price, amount, trader, deadline);
        _ETHOrders[orderID] = true;
    }
    ...
    function newOrderForETHWithPointer(
        ...
        returns (uint256 orderID)
    {
        orderID = newOrderWithPointer(price, amount, trader, deadline, pointer);
        _ETHOrders[orderID] = true;
    }
    ...
    function newOrderForETHWithPointers(
        ...
        returns (uint256 orderID)
    {
        orderID = newOrderWithPointers(price, amount, trader, deadline, pointers);
        _ETHOrders[orderID] = true;
    }


    function _removeOrder(uint256 orderID) internal override {
        LimitrMarketTokenToken._removeOrder(orderID);
        delete _ETHOrders[orderID];
    }
    ...
    {
        Trade memory trade = Trade(0, 0, afterFee(msg.value));
        while (trade.availableCounter > 0) {
            if (!_tradeETHFirstOrderMaxPrice(maxPrice, trade, receiver)) {
                break;
            }
        }
        require(trade.counter > 0 && trade.base > 0, 'No trade');
        uint256 fee = _collectETHFees(trade);
        uint256 spent = trade.counter + fee;
        payable(msg.sender).transfer(msg.value - spent);
        return (spent, trade.base);
    }

    function _collectETHFees(Trade memory trade) internal returns (uint256 fee) {
        fee = feeFor(trade.counter);
        if (fee > 0) { _wrapDepositCounterToken(fee); }
        emit FeeReceived(msg.sender, fee);
    }

    function _wrapDepositCounterToken(uint256 amount)
        internal
        postBalanceCheck(counterToken, _expectedCounterBalance + amount)
    {
        _expectedCounterBalance += amount;
        IWETH(counterToken).deposit{value: amount}();
    }

    /// @dev trade the first order at a max price
    function _tradeETHFirstOrderMaxPrice(
        uint256 maxPrice,
        Trade memory trade,
        address receiver
    ) internal returns (bool) {
        ...
        uint256 maxAmount = returnOf(trade.availableCounter, _order.price);
        // remaining amount of counter token
        return _tradeETHOrder(orderID, _order, trade, maxAmount, receiver);
    }


    function _tradeETHOrder(
        uint256 orderID,
        Order memory _order,
        Trade memory trade,
        uint256 maxAmount,
        ...
        _withdrawBaseToken(receiver, amt);
        // transfer counter token to the order trader
        if (_ETHOrders[orderID]) {
            payable(_order.trader).transfer(cost);
        }
        else {
            IWETH(counterToken).deposit{value: cost}();
            _tokenTransfer(counterToken, _order.trader, cost);
        }
        // remove order or update amount
        if (newAmount == 0) { _removeOrder(orderID); }
        else { order[orderID].amount = newAmount; }
        ...
        returns (uint256 cost, uint256 received)
    {
        Trade memory trade = Trade(0, 0, afterFee(msg.value));
        while (trade.availableCounter > 0) {
            if (!_tradeETHFirstOrderAveragePrice(maxAveragePrice, trade, receiver)) {
                break;
            }
        }
        require(trade.counter > 0 && trade.base > 0, 'No trade');
    ...
    /// @dev trade the first order at an average price
    function _tradeETHFirstOrderAveragePrice(
        uint256 maxAveragePrice,
        Trade memory trade,
        address receiver
    ) internal returns (bool) {
...
contract LimitrDeployerTokenETH is ILimitrDeployer {
    function createMarket(
        address factory,
        address baseToken,
        address counterToken
        ...
        returns (address)
    {
        return address(new LimitrMarketTokenETH(factory, baseToken, counterToken));
    }
}