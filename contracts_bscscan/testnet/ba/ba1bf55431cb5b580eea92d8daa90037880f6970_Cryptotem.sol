...
contract CryptotemDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Cryptotem_Dividend_Tracker", "Cryptotem_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Cryptotem_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Cryptotem_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Cryptotem contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Cryptotem_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "Cryptotem_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Cryptotem is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CryptotemDividendTracker public dividendTracker;
    ...
    uint256 public CryptotemFee;
    uint256 public MarketingFee;
    uint256 public totalFees;
    address payable public  CryptotemWallet;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _CryptotemFee, uint256 _MarketingFee) public onlyOwner {
        BNBRewardsFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        CryptotemFee = _CryptotemFee;
        MarketingFee = _MarketingFee;

        totalFees = BNBRewardsFee.add(liquidityFee).add(CryptotemFee).add(MarketingFee); // total fee transfer and buy
    }
    ...
    function setCryptotemWallet(address payable _newAddress) public onlyOwner {
        CryptotemWallet = _newAddress;
    }
    ...
    constructor() ERC20("Cryptotem", "TOTEM") {
        BNBRewardsFee = 4;
        liquidityFee = 4;
        CryptotemFee = 2;
        MarketingFee = 4;
        CryptotemWallet = payable(0x06AEBDCed640101B8C0F0990Ff445EAD9d2acf5b);
        MarketingWallet = payable(0xa21C65BcBDBD049458fcE583C374b0f89d1Cda50);

        totalFees = BNBRewardsFee.add(liquidityFee).add(CryptotemFee).add(MarketingFee); // total fee transfer and buy

    	dividendTracker = new CryptotemDividendTracker();
        ...
        excludeFromFees(CryptotemWallet, true);
        ...
        _isExcludedFromMaxTx[CryptotemWallet] = true;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Cryptotem: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Cryptotem: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Cryptotem: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Cryptotem: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Cryptotem: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Cryptotem: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // Cryptotem Share
        uint256 CryptotemAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee).sub(MarketingFee)).mul(CryptotemFee);
        CryptotemWallet.transfer(CryptotemAmount);
        
        // Marketing Share
        uint256 MarketingAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee).sub(CryptotemFee)).mul(MarketingFee);
