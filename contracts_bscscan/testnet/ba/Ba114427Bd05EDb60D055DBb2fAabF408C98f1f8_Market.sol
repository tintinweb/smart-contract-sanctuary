...
contract Market is ReentrancyGuard, Ownable, Pausable {
    ...
    // @notice SafeMars token address
    address private constant SAFEMARS_TOKEN = address(0x0ce1DB7B70cA557bf3Ee2087e5FDf0d7F7224722);
    ...
    event EvSettingsUpdated(address feeAddress, address bookkeeper);
    ...
    function updateSettings(
        ...
        emit EvSettingsUpdated(feeAddress, bookkeeper_);
    }
    ...
    function _offerSell(
        ...
        // added by @blkdot
        require(IERC20(SAFEMARS_TOKEN).allowance(msg.sender, address(this)) == 0, "thank you but seller should not pay");
    ...
    function _offerBuy(IERC721 nft, uint256 tokenId, uint256 price) internal {
        // @notice get the balance of safemars token and transfer to the contract
        require(IERC20(SAFEMARS_TOKEN).balanceOf(msg.sender) >= price, "Not enough balance!");
        require(IERC20(SAFEMARS_TOKEN).allowance(msg.sender, address(this)) >= price, "buyer should pay");
        IERC20(SAFEMARS_TOKEN).transferFrom(msg.sender, address(this), price);
    ...
    function _acceptSell(uint256 id) internal {
        ...
        // @notice get the balance of safemars token and transfer to the contract
        require(IERC20(SAFEMARS_TOKEN).balanceOf(msg.sender) >= _offer.price, "Not enough balance!");
        require(IERC20(SAFEMARS_TOKEN).allowance(msg.sender, address(this)) >= _offer.price, "buyer should pay");
        IERC20(SAFEMARS_TOKEN).transferFrom(msg.sender, address(this), _offer.price);
