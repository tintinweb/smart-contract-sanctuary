...
// dev functions     
    ...
    function start(
        ...
        require(_vestPercentageWholeNumber <= 50 && _vestPercentageWholeNumber > 0, 'Vest vestPercentage cannot be larger than 50%!');
        require(_vestTimeInEpoch < 1000, 'Set a vestTime larger than 1000!');
    ...
    function buyCurrentSale(uint _purchaseAmount) external onlyInvestors() saleActive() {
        ...
        if(tokenForContribution == address(0)){
            sender.transfer(_purchaseAmount);
        }else{
