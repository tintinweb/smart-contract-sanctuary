...
contract FenrirFee is BEP20{
    ...
    uint16 public constant BURN_RATE = 250;
    
    // Transfer FEE Rate: 1.5%.
    uint16 public constant FEE_RATE = 150;
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "FANRIR::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }   _;
    }
    
    // Constructs the Fenrir contract.
    constructor() public BEP20('Fenrir FarmWiki.xyz', 'FANRIR') {
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= MAXIMUM_TRANSFER_RATE, "FANRIR::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxTransferAmountRate >= MINIMUM_TRANSFER_RATE, "FANRIR::updateMaxTransferAmountRate: Max transfer amount rate can not be bleow minimum rate.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "FANRIR::transferOperator: new operator is the zero address");
