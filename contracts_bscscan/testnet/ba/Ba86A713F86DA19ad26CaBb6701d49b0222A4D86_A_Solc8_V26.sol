...
contract MultiSig is Context {
   ...
   // @dev Emitted when MultiSiging is paused by Operator.
    ...
    modifier whenMultiSigOff() {
        require(_multisig_stopped, "ADMIRE::MultiSig::Error: MultiSig Denied: Permission Revoked");
        _;
    }
 ...
 * the owner.
 */
contract Ownable is Context, MultiSig {
   ...
   // @dev ____________________Functions__________________________
    ...
    modifier onlyMultiSig() {
        require(multisig() == _msgSender(), "Ownable: caller is not the multisig address");
        _;
    }
    ...
    function transferOperator(address newOperator) public virtual onlyOperator {
        ...
        _setOwner(newOperator);
    }

        function transferMultiSig(address newMultiSig) public virtual onlyMultiSig {
        require(newMultiSig != address(0), "Ownable: new multisig is the zero address");
        _setOwner(newMultiSig);
    }
...
contract A_Solc8_V26 is BEP20("ArghV26", "ADMRSL6V25") {
    using Address for address;
    using SafeMath for uint256;
    ...
    function A1_PauseTheChain_Pause() public onlyMultiSig whenNotPaused {
        _pause();
    }

    function A2_PauseTheChain_Unpause() public onlyMultiSig whenPaused {
        _unpause();
    }
    ...
    function C1_MintLockOutDevWallet_TurnOn()
        public
        onlyMultiSig
        whenMintHardlockOn
    {
    ...
    function C2_MintLockOutDevWallet_AllowMinting()
        public
        onlyMultiSig
        whenMintHardlockOff
    {
    ...
    function D1_Revoke_Change_The_Operator()
        public
        onlyMultiSig
        whenChangeOperatorOn
    {
        _stop_change_operator();
    }

    function D2_Allow_Change_The_Operator()
        public
        onlyMultiSig
        whenChangeOperatorOff
    {
    ...
    function maxTransferAmount() public view returns (uint256) {
        return
            totalSupply().mul(maxTransferAmountRate).div(1000000000000000000);
    }
