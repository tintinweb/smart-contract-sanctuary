...
contract DonutCat is Context, IERC20, Ownable {
    ...
    uint256 private _totalFee; // 15% total team fee
    ...
    function removeFee() private {
        if (_totalFee == 0 && _taxFee == 0) return;
        _taxFee = 0;
        _totalFee = 0;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) {
                ...
                if (contractETHBalance > 5 * 10**17) { // Send Fee to team when the contract BNB balance is over 1 BNB
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tTeam) = _getTValues(tAmount, _taxFee, _totalFee);
        uint256 currentRate = _getRate();
