    ...
    AdventureInfo
{
    ...
    function deposit(
        ...
        _addReservedCrystalToDungeon(_dungeonId,_team.reservedCrystal,_timestamp);
        ...
        returns (uint256 level)
    {
        ...
        uint256 levelDungeonCap = fanDungeonStage.levelCapOf(_dungeonId);
    ...
    function _calculateReserveRewards(Team memory _team, uint64 _timestamp)
        internal view returns (Team memory) 
    {
        ...
        uint64 durationCrystal = _calculateDuration(_remainingCrystal, _maxCrystalPerTurn, _maxCrystalPerTurn) * fanDungeonStage.bonusOf(_team.dungeonId).duration / 10000;
        uint64 durationXp = _calculateDuration(1000000 ether, _maxCrystalPerTurn, _maxXpPerTurn) * fanDungeonStage.bonusOf(_team.dungeonId).duration / 10000;

        _team.endTimeCrystal = _timestamp + durationCrystal;
        _team.endTimeXP = _timestamp + durationXp;

        _team.reservedCrystal = uint256(durationCrystal) * _dungeonLevelInfo[_team.dungeonLevel].crystalPerSecond;
        _team.reservedXP = durationXp * _dungeonLevelInfo[_team.dungeonLevel].xpPerSecond;

        return _team;
    }

    function _calculateDuration(uint256 _remainingUnit,uint256 _maxUnitPerTurn, uint256 _unitPerSecond) internal pure returns (uint64 duration) {
