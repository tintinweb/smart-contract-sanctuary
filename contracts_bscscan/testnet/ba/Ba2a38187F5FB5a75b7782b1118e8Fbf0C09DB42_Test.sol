...
contract Test is Context, IERC20, Ownable {
    ...
    uint256 public _donationFee = 7;
    uint256 private _previousDonationFee = _donationFee;
    
    uint256 public _maxTxAmount = 10000000 * 10**9;
    uint256 private minimumTokensBeforeSwap = 50;

    address payable public charityAddress = 0xc016aCC8Db9Bfd7E035343C379c6291AcC10D068;
    ...
    function totalBurn() public view returns (uint256) { return _tBurnTotal; }
    function totalDonationBNB() public view returns (uint256) { return _tDonationTotal; }
    ...
    // allow people to donate to charity
    function DonateToCharity(uint256 amount) public {
        _transfer(_msgSender(), charityAddress, amount);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) { return _amount.mul(_donationFee).div(10**2); }
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() { if (taxFee < 10) { _taxFee = taxFee; } }
    function setBurnFeePercent(uint256 burnFee) external onlyOwner() { if (burnFee < 10) { _burnFee = burnFee; } }
    function setDonationFeePercent(uint256 DonationFee) external onlyOwner() { if (DonationFee < 10) { _donationFee = DonationFee; } }
    function setMaxTxPercent(uint256 maxTxPercent, uint256 maxTxDecimals) external onlyOwner() { _maxTxAmount = _tTotal.mul(maxTxPercent).div( 10**(uint256(maxTxDecimals) + 2) ); }
    function setNumTokensBeforeSwap(uint256 _minimumTokensBeforeSwap) external onlyOwner() { minimumTokensBeforeSwap = _minimumTokensBeforeSwap; }
    ...
    function transferAnyBEP20Tokens(address _tokenAddr, address _to, uint _amount) public onlyOwner() { require(_tokenAddr != address(this), "Cannot transfer out LOTTO!"); Token(_tokenAddr).transfer(_to, _amount); }
    function BNBToCharityWallet() external onlyOwner() { address payable _owner = payable(msg.sender); _owner.transfer(address(this).balance); }
    function TransferCharityETH(address payable recipient, uint256 amount) private { recipient.transfer(amount); }
