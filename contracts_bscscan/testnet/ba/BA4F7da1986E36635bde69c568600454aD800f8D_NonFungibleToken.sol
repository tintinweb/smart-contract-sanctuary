 ...
 */
contract NonFungibleToken is ERC721URIStorage, Ownable, Pausable{

    // general constants
    uint256 public COLLECTION_MAX_ITEMS;                    // maximum number of items in the collection
    ...
    address payable internal receiveEthAddress;
    ...
    constructor(string memory name_,
                ...
                address payable _receiveEthAddress,
    ...
    function setCollectMaxItems(uint256 _collectionMaxItems) external onlyOwner {
      require(_collectionMaxItems >= totalSupply, "max items lower than total supply");
      COLLECTION_MAX_ITEMS = _collectionMaxItems;
    }
    ...
    function mint() external payable whenNotPaused {
        ...
        uint256 _numToMint = _getNumToMint();
    ...
    function withdraw() external payable onlyOwner returns (bool success) {
        (success,) = payable(receiveEthAddress).call{value: address(this).balance}("");
    }
    ...
    function _getNumToMint() private view returns(uint256 _numToMint) {
        _numToMint = msg.value / price;
