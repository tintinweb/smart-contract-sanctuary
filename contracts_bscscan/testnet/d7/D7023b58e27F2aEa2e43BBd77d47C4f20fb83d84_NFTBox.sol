...
contract NFTBox is Ownable, Pausable, ReentrancyGuard {
     ...
     * @dev Sale Diamond Box
     */
    function buyStone(uint256 _quantity) public payable nonReentrant whenNotPaused {
        require(CURRENT_BOX.add(1) <= TOTAL_BOX, "box already sold out");
        uint256 typeBox = 3;
    ...
    function _buyStone(uint256 _typeBox, uint256 _quantity) internal {
        ...
        require(
            nftToken.allowance(msg.sender, address(this)) >= saleBoxs[_typeBox].price.mul(_quantity),
            "Token allowance too low"
        );
        require(msg.value == feeSale, "Amount of BNB sent is not correct.");
        nftToken.transferFrom(msg.sender, saleWallet, saleBoxs[_typeBox].price.mul(_quantity));
        if (feeSale > 0) {
            feeWallet.transfer(feeSale);
        }
        randManager.randMod(_msgSender(), 10000);
        ...
        nft.setNFTFactory(nftItem, tokenId);
        nft.setNFTForUser(nftItem, tokenId, _msgSender());
        emit Sale(tokenId, _msgSender(), saleBoxs[_typeBox].price.mul(_quantity), _typeBox);
    }
