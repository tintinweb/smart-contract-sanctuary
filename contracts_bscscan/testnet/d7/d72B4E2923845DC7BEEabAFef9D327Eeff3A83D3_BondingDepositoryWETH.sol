...
contract BondingDepositoryWETH is Ownable {
    ...
    address public immutable OAS; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints OAS when receives principle
    ...
    struct Bond {
        uint256 payout; // OAS remaining to be paid
    ...
    function deposit(
        ...
        require(payout >= 10000000, "Bond too small"); // must be > 0.01 OAS ( underflow protection )
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(OAS).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(OAS).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(OAS).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(OAS).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    /**
     *  @notice calculate current bond premium
     *  @return premium_ uint
     */
    function bondPremium() public view returns (uint256 premium_) {
        premium_ = terms.controlVariable.mul(debtRatio()).div(1e5);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to OAS supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(OAS).totalSupply();
    ...
    /**
     *  @notice calculate amount of OAS available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or OAS) to the DAO
     *  @return bool
     */
