...
contract OrionGovernance is IOrionGovernance, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    uint64  extra_fee_seconds;
    uint16  extra_fee_percent;  //  0 - 1000, where 1000
    ...
    function withdraw(uint56 removing_amount) override public nonReentrant updateReward(msg.sender)
    {
        ...
        UserVault memory vault;
        vault.amount = removing_amount;
        vault.created_time = uint64(block.timestamp);
        vaults_[msg.sender].push(vault);
    ...
    function vaultWithdraw(uint index) public nonReentrant
    {
        ...
        vault.amount = uint56(
            uint(vault.amount).mul(1000 - fee_percent).div(1000)
        );
        ...
        staking_token_.safeTransfer(msg.sender, vault.amount);
    }

    //  Only owner could set vault parameters
    function setVaultParameters(uint16 fee_percent, uint64 fee_seconds) external onlyOwner
    {
        require(fee_percent < 1000, "VF_1");
        extra_fee_percent = fee_percent;
        extra_fee_seconds = fee_seconds;
    }
