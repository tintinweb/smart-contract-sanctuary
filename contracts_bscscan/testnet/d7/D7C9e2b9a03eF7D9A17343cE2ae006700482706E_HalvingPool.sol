...
contract HalvingPool is Ownable {
    ...
    // The stake token
    IERC20 immutable public STAKE_TOKEN;
    // The reward token
    IERC20 immutable public REWARD_TOKEN;

    // Blocks to halving.
    uint256 immutable public BLOCKS_TO_HALVING;

    uint8 immutable public TOTAL_HALVINGS;

    // Reward tokens created per block.
    uint256 public rewardPerBlock;

    uint256 public currentHalving = 1;
    ...
    constructor(
        ...
        uint256 _startBlock,
        uint256 _blocksToHalving,
        uint8 _totalHavlings
    ) {
        STAKE_TOKEN = _stakeToken;
        REWARD_TOKEN = _rewardToken;
        rewardPerBlock = _rewardPerBlock;
        startBlock = _startBlock;
        BLOCKS_TO_HALVING = _blocksToHalving;
        TOTAL_HALVINGS = _totalHavlings;
    ...
    // Return reward multiplier over the given _from to _to block.
    function calculatedHalving() public view returns (uint256 totalHalving) {
        totalHalving = 1;
        if (startBlock < block.number) {
            uint256 halving = (block.number - startBlock) / BLOCKS_TO_HALVING;
            totalHalving = totalHalving + halving;
        }
    }

    function updateHalving() public {
        if (currentHalving != calculatedHalving() && currentHalving < TOTAL_HALVINGS) {
            currentHalving ++;
    ...
    function deposit(uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            STAKE_TOKEN.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    /// @param _startBlock The block to start mining
    ...
    function sweepToken(IERC20 token) external onlyOwner {
        require(address(token) != address(STAKE_TOKEN) && address(token) != address(REWARD_TOKEN), "can not sweep stake or reward token");
