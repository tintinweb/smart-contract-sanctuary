...
contract tokenExchange is ReentrancyGuarded, Ownable {
    
    using SafeBEP20 for BEP20Token; //SafeBEP20 library used for safe transfer of tokens
    event Received(address,uint256); // BNBer received event triggered when BNBer is added to this vault
    BEP20Token public tokenContractAddress; // The unique token contract address of the token pool
    uint256 public maxBNBlimit; // the max BNBer limit in wei to be exchanged
    uint256 public minBNBlimit; // the minimu BNBer limit in wei to be exchanged
    uint256 public tokenexchangerate ;
    constructor()
    {
        tokenContractAddress = BEP20Token(0x070343ADbe4C227f8807dc1cd5e954d914C2953B);
        maxBNBlimit = 1000000000000000000; // 1 BNB in wei
        minBNBlimit = 100000000000000000; // 0.1 BNB in wei
        tokenexchangerate = 20000000; // 1 BNB = 20M tokens
    }
    
    receive() external payable {
     
     require(msg.value >= minBNBlimit && msg.value <= maxBNBlimit, "Minimum 0.1 BNB and maximum 1 BNB accepted");
     uint256 tokenstotransfer = msg.value * tokenexchangerate;
     require(tokenContractAddress.balanceOf(address(this)) >= tokenstotransfer, "Tokens not available");
     SafeBEP20.safeTransfer(tokenContractAddress, msg.sender, tokenstotransfer);
    ...
    function withdrawTokenFromcontract(BEP20Token _token, uint256 _tamount) public onlyOwner{
        require(_token.balanceOf(address(this)) > _tamount, "Contract doesn't have enough tokens");
        SafeBEP20.safeTransfer(_token, owner(), _tamount);
    }
