...
contract Charge60Strategy is StratManager {
	...
	function withdraw(uint256 _amount) external {
		...
		IMasterCharge(masterCharge).withdraw(_amount.sub(wantBal), poolId);
	...
	function APR() external view returns (uint256) {
		return IMasterCharge(masterCharge).APR(poolId);
	}
	...
	function TVL() public view returns (uint256) {
		...
		return
			_amount
				.mul(IMasterCharge(masterCharge).stakedTokenPrice(poolId))
				.div(1e18);
	}

	function stakedTokenPrice() external view returns (uint256) {
		return IMasterCharge(masterCharge).stakedTokenPrice(poolId);
	}
}
