...
contract BSB is Percentage , Verifier{
   ...
   /*================================
    ...
    address [] public AchieveOneBNB;
    address [] public AchievethreeBNB;
    address [] public AchieveTenBNB;
   ...
   function Chackdistributing(address _add) internal {
        ...
        if(referralBalance_[_add] >= 0.001 ether){
            AchieveOneBNB.push(_add); 
        }
        if(referralBalance_[_add] >= 0.003 ether){
            AchievethreeBNB.push(_add);
        }
        if(referralBalance_[_add] >= 0.010 ether){
    ...
    function distributeRefferalFisrst(uint256 _trx, address __upline) private{
		...
		uint256 totalrefs = SafeMath.add(totalrefs,_allaff);
        ...
        public
    {
        ...
        uint256 _dividends=SafeMath.mul(onePercent(_ethereum),5);
        uint256 _taxedEthereum =calculateBNBReceived(_amountOfTokens);
        uint256 ownerDividend;
        ownerDividend=SafeMath.mul(onePercent(_dividends),8);
    ...
    function Totaldistribute_Byreferral(address _customerAddress) public view returns(uint256){
        
        return users[_customerAddress].Totaldistribute;
    }
