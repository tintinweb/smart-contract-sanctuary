...
contract BSCWin is Context, IERC20, Ownable {
    ...
    uint256 public initialLotteryTime = block.timestamp;
    ...
    function setInitialLotteryTime(uint256 timestamp) public onlyOwner {
        initialLotteryTime = timestamp;
        lastLotteryTime = timestamp;
    }

    function isTimeForLottery() public view returns(bool) {
        if (totalWinners == 0 && (block.timestamp > initialLotteryTime)) {
            return true;
        }
    ...
    function lotteryCheck() internal {
        ...
        if (winnerValid) {
            ...
            lastLotteryTime = currentLotteryStart;
