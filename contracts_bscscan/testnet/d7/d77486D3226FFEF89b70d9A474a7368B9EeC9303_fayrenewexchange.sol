...
contract fayrenewexchange is Sale{
    ...
    function saleToken4(address payable from, address payable admin,uint256 tokenId, uint256 amount, address token, uint256 _type, uint256 NOFToken) public payable{
       _saleToken1(from, admin, tokenId, amount);
       if(_type == 721){
           BEP721 tok= BEP721(token);
            if(checkOrder[tokenId][from]==true){
