...
contract Name is Context, IERC20, Ownable {
    ...
    string private _name = "NewDoge";
    string private _symbol = "NewDoge";
    ...
    address public marketingWallet = 0xd528aA5D295f0217327647A4B608F341D9eDf749;
    ...
    {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount);
        (tTransferAmount, rTransferAmount) = takeBurn(tTransferAmount, rTransferAmount, tAmount);
        (tTransferAmount, rTransferAmount) = takeMarketing(tTransferAmount, rTransferAmount, tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, tFee);
    ...
    function takeBurn(uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private
    ...
    function takeMarketing(uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private
    returns (uint256, uint256)
    {
