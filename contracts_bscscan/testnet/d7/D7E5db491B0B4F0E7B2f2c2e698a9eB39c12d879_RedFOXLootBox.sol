...
contract RedFOXLootBox is VRFConsumerBase, Ownable {
    ...
    event ClassCardIdsChanged(Class class, uint256[] newIds);
    event PackOpened(address indexed opener, bytes32 requestId);
    event NewPackAdded(
        Pack indexed pack,
        uint256 price,
        uint256 draws,
     ...
     * fee = 0.1 * 10 ** 18;
     */
    constructor(address _redFoxCollectible)
        VRFConsumerBase(
    ...
    /// @notice Open 1 pack
    /// @dev Open pack will use PackSetting to use
    /// @param _pack Pack number
    function openPack(Pack _pack) external payable packEnabled(_pack) {
        PackSettings memory pack = packToSettings[uint256(_pack)];
        ...
        emit PackOpened(msg.sender, requestId);
    }

    /// @notice Claim pack fee from user for building platform
    /// 100 % boxes fee will be reinvest and building RedFox platfomr
    /// @dev Only owner can claim
    /// @param _amount Amount of boxes fee
    function claimBoxFee(uint256 _amount) external onlyOwner {
        require(
            address(this).balance >= _amount,
            "RedFOXLootbox: Amount exceed balance"
        );
    ...
    /// @notice Get card ids by class
    /// @dev Class in range [0:NUM_CLASSES]
    /// @param _class Class number
    ...
    /// @notice Force remove id from classes
    /// @dev Only Owner can remove card id to each class. Duplicate ids is not allowed
    /// @param _class Class of new card
    /// @param _cardId Id of card
    function changeClassCardIds(Class _class, uint256[] memory _cardId)
        public
        onlyOwner
    {
        classToIds[_class] = _cardId;

        emit ClassCardIdsChanged(_class, _cardId);
    }

    /// @notice Admin setting pack configurations. _classProbabilities MUST has sum of 10000
    ...
    function addNewPack(
        ...
        emit NewPackAdded(_pack, _price, _numberOfDraws, _classProbabilities, _guarantees);
    }

    /// @notice Return the pack setting of _pack
    /// @dev Pack in range [0:NUM_PACKS] are valid
    /// @param _pack Uint pack enum
    /// @return price Price of the pack
    /// @return numberOfDraws Number of cards can receive
    /// @return classProbabilities Rate of each class
    /// @return hasGuaranteedClasses Is guaranteed any class
    /// @return guarantees Array of guarantee classes
    ...
    /// @notice Function that expand one random number to multple numbers
    /// @dev For information, see https://docs.chain.link/docs/chainlink-vrf-best-practices/
    /// @param _randomValue Random number, always come from VRF using truly data
    /// @param _n Number of expand values that return in array
    /// @param _seed Generate different seeds for each random
    /// @return expandedValues Uint array of random numbers generated from 1 random number.
    function expand(
        uint256 _randomValue,
        uint256 _n,
        uint256 _seed
    ) internal view returns (uint256[] memory expandedValues) {
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomness)
        ...
        override
    {
        (address owner, bytes32 requestId, Pack pack, ) = getRequestOwner(
            _requestId
        );
    ...
    function openRandomCards(
        ...
        for (uint256 i = 0; i < guarantees.length; i++) {
            if (guarantees[i] > 0) {
                randomNumbers = pickRandomIn(_randomness, guarantees[i], i);
                seed += i;
        ...
        if (remainCards > 0) {
            ...
            for (uint256 i = 0; i < remainCards; i++) {
                ...
                Class cardClass = getClassByProbabilities(
                    randomNumber,
                    classProbabilities
                );
                remainCardIds[i] = getRandomCardIdByClass(
                    cardClass,
                    randomNumber
                );
                minted += 1;
    ...
    function pickRandomIn(
        ...
        uint256 _numbers,
        uint256 _seed
    ) internal view returns (uint256[] memory randomResults) {
    ...
    function getClassByProbabilities(
        uint256 _random,
        uint16[NUM_CLASSES] memory _classProbabilities
    ) internal pure returns (Class _class) {
        uint256 value = _random.mod(INVERSE_BASIS_POINT);
        for (uint256 i = _classProbabilities.length - 1; i > 0; i--) {
            uint256 probability = _classProbabilities[i];
            if (value < probability) {
                return Class(i);
            } else {
                value = value.sub(probability);
            }
        }
