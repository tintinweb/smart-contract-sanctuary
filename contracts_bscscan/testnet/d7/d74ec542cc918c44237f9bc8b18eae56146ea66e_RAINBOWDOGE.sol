...
// Hold $RAINBOWDOGE and get rewarded in $RAINBOW on every transaction!
...
interface IRAINBOWDistributor {
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 _bnbToRAINBOWThreshold) external;
...
contract RAINBOWDistributor is IRAINBOWDistributor {
    ...
    uint256 public minDistribution = 10 * 10**6; // 10 Million $RAINBOW minimum auto send
    
    // BNB Needed to Swap to BAKE
    uint256 public swapToRAINBOWThreshold = 0.1 * 10**18;
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 _bnbToRAINBOWThreshold) external override onlyToken {
        minPeriod = _minPeriod;
        minDistribution = _minDistribution;
        swapToRAINBOWThreshold = _bnbToRAINBOWThreshold;
    }
    ...
    function deposit() external override onlyToken {
        ...
        if (bnbBalance >= swapToRAINBOWThreshold) {
            ...
            router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: swapToRAINBOWThreshold}(
                0,
                path,
                address(this),
                block.timestamp
    ...
    function setRAINBOWAddress(address nRAINBOW) external onlyToken {
        RAINBOW = nRAINBOW;
    }
...
/**
 * @dev The official $RAINBOWDOGE smart contract
 */
contract RAINBOWDOGE is IERC20, Context, Ownable {
    ...
    // Token Limits
    uint256 public _maxTxAmount = _totalSupply.div(200); // 0.5% or 5 Billions
    ...
    constructor () {
        ...
        // Liquidity Pool Address for BNB -> RAINBOWDOGE
    ...
    //  provide Distributor with BNB for $RAINBOWDOGE
    ...
    function setRAINBOWDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 _bnbToRAINBOWThreshold) external onlyOwner {
        rainbowDistributor.setDistributionCriteria(_minPeriod, _minDistribution, _bnbToRAINBOWThreshold);
    }
    ...
    // Buy and Burn RAINBOWDOGE with BNB stored in Contract
    ...
    // Swaps $RAINBOW Addresses in case of migration
    function setTokenAddresses(address nRAINBOW) external onlyOwner {
        rainbowDistributor.setRAINBOWAddress(nRAINBOW);
        emit SwappedTokenAddresses(nRAINBOW);
    }
    ...
    event SwappedTokenAddresses(address newRAINBOW);
