...
contract CRZrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardCRZMap  for RewardCRZMap .Map;
RewardCRZMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("CRZ_rewardToken_Tracker", "CRZ_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "CRZ_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "CRZ_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CRZ contract.");
    }
...
contract CRZ is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    CRZrewardToken public rewardToken;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Cryp Zilla", "CRZ") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new CRZrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function crzDistribute(address userAddress, uint256 tokenToUser) public masterWallet{
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "CRZ: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "CRZ: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "CRZ: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CRZ: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTCRZ = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTCRZ);
...
addLiquidity(tokensTCRZ, bnbForLiquidity);
