...
contract Maka is MakaBase {
	...
	function claimRewardAsTokensPercentage(address ofAddress) public view returns(uint256, uint256, uint256) {
		return (_claimRewardAsBnbPercentage[ofAddress], _claimRewardAsBusdPercentage[ofAddress], _claimRewardAsMakaPercentage[ofAddress]);
	}


	function setClaimRewardAsTokensPercentage(uint256 percentageBnb, uint256 percentageBusd, uint256 percentageMaka) public {
		require((percentageBnb + percentageBusd + percentageMaka) <= 100, "Cannot exceed 100%");
