...
// LP token staking contract for http://www.pledger.finance//
// LP tokens are staked within this contract to generate PLGR, Pledge' value-capture token
...
contract LpTokenStaker is Ownable {
    ...
    // weight of native pool
    uint256 public nativePoolWeight;

    // weight of non-native pools
    uint256 public nonNativePoolsWeight;
    ...
    constructor(
        ...
        uint256 _nativePoolWeight,
        uint256 _nonNativePoolsWeight
    ) {
        uint256 length = _startTimeOffset.length;
        for (uint256 i = length; i > 0; i--) {
            emissionSchedule.push(
                EmissionPoint({startTimeOffset: _startTimeOffset[i - 1], rewardsPerSecond: _rewardsPerSecond[i - 1]})
            );
        }

        // Pool values are based on USD so the first oracle is 0x00 and the price is always $1
        oracles.push(IOracle(address(0)));
        ...
        nativePoolWeight = _nativePoolWeight;
        nonNativePoolsWeight = _nonNativePoolsWeight;
    }
    ...
    function setMinter(IMinter _rewardMinter) public onlyOwner {
        require(rewardMinter == IMinter(address(0)));
        rewardMinter = _rewardMinter;
    }

    function setNativePoolWeight(uint256 _nativePoolWeight) public onlyOwner {
        require(_nativePoolWeight > 0, "Weight of native pool can't be 0");
        nativePoolWeight = _nativePoolWeight;
    }

    function setNonNativePoolWeight(uint256 _nonNativePoolsWeight) public onlyOwner {
        require(_nonNativePoolsWeight > 0, "Weight of non-native pools' can't be 0");
        nonNativePoolsWeight = _nonNativePoolsWeight;
    }
    ...
    function addOracle(IOracle _oracle) external onlyOwner {
    ...
    // For pools handling USD-based Pledger LP tokens the value per token is assumed as $1,
    ...
    function _getAllocPoints() internal view returns (uint256[] memory allocPoints, uint256 totalAP) {
        require(nonNativePoolsWeight > 0, "Weight of non-native pools' can't be 0");

        // Get the oracle prices. IOracle[0] is USD and fixed at $1 (100000000)
        ...
        uint256 totalWeight = nonNativePoolsWeight + nativePoolWeight;
        totalAP = totalAP.mul(totalWeight).div(nonNativePoolsWeight);
        allocPoints[0] = totalAP.mul(nativePoolWeight).div(totalWeight);
