  ...
  INEFTiLANDToken
{
  ...
  // address internal DEAD; address internal ZERO; uint256 private MAX;
  /* solhint-disable-next-line var-name-mixedcase */
  ...
  function initialize(
    ...
    uint16 taxFeeBps_,          // tax fee bpsamount        (4 %)     400
    ...
    address charityAddress_,    // address of the charity
      // 0x95178b52E58d5527c9a7f2a38AA4068Bc8D76FDF (TestNet)
  ...
  { 
    ...
    if (charityAddress_ == ZERO) {
      ...
      require(charityFeeBps_ == 0, "Invalid Charity address to ZERO and Charity percent more than 0");
    }
    /* solhint-disable-next-line reason-string */
  ...
  function suspend(address account, uint256 valueTo)
    ...
    { return ((_isSuspended[account] > 0), _isSuspended[account]); }
    ...
    private
  {
    ...
    if (!(_isSuspended[from] > 0) && !(_isSuspended[to] > 0)) {
      require(
        /* solhint-disable-next-line not-rely-on-time */
        (_isSuspended[from] > block.timestamp) || (_isSuspended[to] > block.timestamp),
        "Suspended time range"
      );
    }
