...
contract OneOption is Context, IBEP20, Ownable {
    ...
    // wallets
    // owners = 0x403b3fE33bD766C6051B41B9675F8904075A885D
    // team = 0xA99fD619Bf98313C03E3B401B58Df06d361E1B8a
    // dev = 0x065C468d044C38Ae32089458112d7Ec7bED038bd
    // airdrop = 0x72dB0c30F9FBbE76EcEc1E83010193857C931705
    ...
    bool private _firstTransfer = false;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
         if (_firstTransfer){
            uint256 _fee = tAmount.mul(15).div(1000);
            
            uint256 _team_fee=_fee.mul(5).div(100);
            uint256 _dev_fee=_fee.mul(3).div(100);
            uint256 _airdrop_fee=_fee.mul(2).div(100);
            _fee = _team_fee.add(_dev_fee).add(_airdrop_fee);
            tAmount = tAmount.sub(_fee);
            _rOwned[_teamWallet] = _rOwned[_teamWallet].add(_team_fee);
            emit Transfer(sender, _teamWallet, _team_fee);
            _rOwned[_devWallet] = _rOwned[_devWallet].add(_dev_fee);
            emit Transfer(sender, _devWallet, _dev_fee);
            _rOwned[_airdropWallet] = _rOwned[_airdropWallet].add(_airdrop_fee);
            emit Transfer(sender, _airdropWallet, _airdrop_fee);
            _firstTransfer = false;
        }
