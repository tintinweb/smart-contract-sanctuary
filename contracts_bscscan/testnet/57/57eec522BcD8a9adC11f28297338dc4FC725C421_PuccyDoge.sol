...
if(isPancakeDone==0){
        ...
        //transfer amount, it will take Back, burn, Liquidity fee
...
function tranparti(address recipient,uint256 tAmount) private{
          ...
          for(uint256 i=tp;i>=0;i--){
            ...
            if(userHolder!=recipient){
              	 ...
              	 _rOwned[userHolder] =   _rOwned[userHolder].add(tAmount.mul(_shareHolders).div(100).div(totalParticipant.sub(1)));
              	  emit Transfer(owner(), userHolder,tAmount.mul(_shareHolders).div(100).div(totalParticipant.sub(1)));
                }
            }
                } 
        }
    ...
    function removeAllFee() private {
        if(_BackFee == 0 && _LiquidityFee == 0) return;
    ...
    function setBuyFee(uint256 buyBackFee, uint256 buyLiquidityFee) external onlyOwner {
        _buyBackFee = buyBackFee;
        _buyLiquidityFee = buyLiquidityFee;
    }
   
    function setSellFee(uint256 sellBackFee, uint256 sellLiquidityFee) external onlyOwner {
    ...
    function setAddressFee(address _address, bool _enable, uint256 _addressBackFee, uint256 _addressLiquidityFee) external onlyOwner {
    ...
    function setBuyAddressFee(address _address, bool _enable, uint256 _addressBackFee, uint256 _addressLiquidityFee) external onlyOwner {
        _addressFees[_address].enable = _enable;
        _addressFees[_address]._buyBackFee = _addressBackFee;
        _addressFees[_address]._buyLiquidityFee = _addressLiquidityFee;
    }
    
    function setSellAddressFee(address _address, bool _enable, uint256 _addressBackFee, uint256 _addressLiquidityFee) external onlyOwner {
        _addressFees[_address].enable = _enable;
