...
// 	uint256 constant public CONTRACT_BALANCE_STEP = 1000000 trx;
	...
	uint256 [] public REF=[6,6,6,6,6,6,6,6,6,6,1,1,1,1,1];
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if (user.deposits[i].withdrawn < (user.deposits[i].amount.mul(210)).div(100)) {
				...
				if (user.deposits[i].withdrawn.add(dividends) > (user.deposits[i].amount.mul(210)).div(100)) {
				    // _remaining=(user.deposits[i].withdrawn.add(dividends).sub((user.deposits[i].amount.mul(210)).div(100)));
	...
	function _matchBonus(address _user, uint256 _amount) private {
        ...
        for (uint i = 1; i <= 15; i++) { // For matching bonus
            ...
            users[owner].matchBonus = users[owner].matchBonus.add(_amount.mul(REF[i]).div(100)); 
                ...
                if (i>=11) {
                    ...
                    users[up].matchBonus = users[up].matchBonus.add(_amount.mul(REF[i]).div(100));
                ...
                else if (users[up].referrals >= i+1&&isActive(up)) {
