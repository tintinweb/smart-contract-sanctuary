...
contract AnjiToken is Context, IBEP20, Ownable {
    ...
    function swapBack() internal swapping {
        ...
        uint256 bnbSwapMarketingAmount = swapedBNBAmount.mul(marketingFee).div(denom); // BNB for Marketing
        uint256 bnbSwapCharityAmount = swapedBNBAmount.mul(charityFee).div(denom); // BNB for Charity
        uint256 bnbSwapBnbAmount = swapedBNBAmount.mul(bnbReflectionFee).div(denom); // BNB for BNB Rewards

        if (swapedBNBAmount > 0) {
            if (bnbSwapMarketingAmount > 0) {
                // Send BNB for Marketing
                payable(marketingFeeReceiver).call{value: bnbSwapMarketingAmount, gas: 30000}("");
            }

            if (bnbSwapCharityAmount > 0) {
                // Send BNB for Charity
                payable(charityFeeReceiver).call{value: bnbSwapCharityAmount, gas: 30000}("");
            }

            if (bnbSwapBnbAmount > 0) {
                // Send BNB for Rewards
                payable(bnbDistributor).call{value: bnbSwapBnbAmount, gas: 30000}("");
    ...
    function depositExternalBNB() external payable {
        payable(bnbDistributor).call{value: msg.value, gas: 30000}("");
