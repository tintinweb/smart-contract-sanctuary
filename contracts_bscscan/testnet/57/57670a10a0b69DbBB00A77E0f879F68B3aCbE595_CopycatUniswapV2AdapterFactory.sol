...
contract CopycatLeader is ERC20('', ''), Ownable, ICopycatLeader, CopycatEmergency, ReentrancyGuard {
  ...
  address public mainToken = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;
  ...
  event SetMainToken(address indexed setter, address oldToken, address newToken);
  function setMainToken(address token) external onlyOwner {
    emit SetMainToken(msg.sender, mainToken, token);
    mainToken = token;
  }
  ...
  function depositTo(address to, uint256 shareMin) override public payable nonReentrant returns (uint256 totalShare, uint256 shareRatio) {
    require(!disabled, "D");
  ...
  function withdrawTo(address to, uint256 amount, uint256 bnbMin) override nonReentrant public returns (uint256 outputAmount, uint256 shareRatio) {
  ...
  function toLeader(uint256 adapterId, uint256 amountBnb, uint256 bnbMin) public override nonReentrant onlyOwner returns(uint256) {
    require(!disabled, "D");
