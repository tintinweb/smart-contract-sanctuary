...
contract Presale is ReentrancyGuard, Context, Ownable, Admined {
    ...
    event DelWhitelisted(address indexed addr, uint max);
    ...
    receive () external payable {
        ...
        // Add to whitelist
    function addtoWhitelist(address addr, uint max) public onlyAdmin {
        require(!seale);
    ...
    function removefromWhitelist(address addr, uint max) public onlyAdmin {
        delete Whitelist[addr];
        delete max;
        emit DelWhitelisted(addr, max);
    }
