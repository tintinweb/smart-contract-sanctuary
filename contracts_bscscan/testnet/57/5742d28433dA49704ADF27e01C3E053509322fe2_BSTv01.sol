...
contract BSTv01 is RedistributableToken {

  /// @custom:oz-upgrades-unsafe-allow constructor
  constructor() initializer {}

  function initialize() initializer public {
    RedistributableToken.initialize("Bet Stable Token", 1_000_000, "BST");
    _burnPercent = 0;
...
contract RedistributableToken is Initializable, BEP20, Burnable, Pausable, Ownable, UUPS {
  ...
  function burnPercent() public view virtual returns (uint) {
    return _burnPercent;
  }

  function redistributionPercent() public view virtual returns (uint) {
    return _redistributionPercent;
  }
  ...
  function _rateAfterDistribution(uint rewardFee, uint sendAmount) internal view returns (uint) {
    uint activeSupply = _supply.active();
    return IntegralMath.mulDivC(_currentRate, activeSupply - sendAmount, activeSupply + rewardFee - sendAmount);
  }
  ...
  function _transfer(address from, address to, uint amount ) internal override virtual {
      ...
      if(rewardFee > 0) emit Distributed(from, rewardFee);
      ...
      uint amountBefore = amount;
      uint amountAfter  = amount - rewardFee - burnAmount;

      emit Debug("from before", _book[from].balance());
      (_book[from], supply)  = _subtract(sender, amountBefore, supply);
      emit Debug("from after ", _book[from].balance());

      if(rewardFee > 0) _currentRate = _rateAfterDistribution(rewardFee, amount);

      emit Debug("to before", _book[to].balance());
      (_book[to], supply)    = _add(recipient, amountAfter, supply);
      emit Debug("to after ", _book[to].balance());

      _supply = supply.increaseActive(rewardFee);

      emit Transfer(from, to, amountAfter);
      _afterTokenTransfer(from, to, amountAfter);
  }
  ...
  function setPermissions(
    ...
    if( holder.mustHideBalance() != hideBalance ) {
      uint supply = _supply;
      if(hideBalance) {
        supply.decreaseActive(balance);
        supply.increaseHidden(balance);
      } else {
        supply.decreaseHidden(balance);
        supply.increaseActive(balance);
      }
      _supply = supply;
    }

    if( holder.mustBurnReward() != burnReward ) {
      if(burnReward) {
        // set rate from MAX_VAL/currentRate to 1 virtually
        // before: balance * MAX_VAL/currentRate
        // after: balance
        balance = _fromReflected(balance);
        // balance = balance*currentRate/MAX_VAL
      } else {
        // set rate from 1 to MAX_VAL/currentRate virtually
        // before: balance
        // after: balance * MAX_VAL/currentRate
        balance = _toReflected(balance);
        // balance = balance*MAX_VAL/currentRate
      }
    }
  ...
  function permissions(address account) public view virtual returns (
    bool sendWithoutFee,
    bool sendWithoutBurn,
    bool receiveWithoutFee,
    bool receiveWithoutBurn,
    ...
    uint holder = _book[account];
    sendWithoutFee = holder.mustSendWithoutFee();
    sendWithoutBurn = holder.mustSendWithoutBurn();
    receiveWithoutFee = holder.mustReceiveWithoutFee();
    receiveWithoutBurn = holder.mustReceiveWithoutBurn();
    burnReward = holder.mustBurnReward();
    hideBalance = holder.mustHideBalance();
  }
