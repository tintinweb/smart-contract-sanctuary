...
import "./ICatNIPNFT.sol";   // withdraw airdropped token
import "./ICatNIP.sol";   // withdraw airdropped token


/*

This contract will provide a passphrase to the caller, IF he is a verified reader
...
contract CodeForCats is Context {
    ...
    string public nameOfContract = "Code For Cats";
    ...
    //////////////////////////// ACCESS CONTROL VARS ////////////////////////////
    address public directorAccount = _msgSender(); // CHANGEIT - get the right director account
    mapping(address => bool) public isAuthorizedToView;
    address public catNIPnftAddress;
    address public catNIPcurrencyAddress;
    //////////////////////////// ACCESS CONTROL VARS ////////////////////////////
    ...
    function GetCodeForCats(uint256 codeForStats, uint256 codeKEK, uint256 codeForKEK) public view returns (uint256) {
        if(codeForStats != 77789503485094328023901){
            return 0;
        }
        if(codeKEK != 89389328093890){
            return 0;
        }
        ...
        return GetCurrentBlockTime().add(block.number).add(777).add( ICatNIPNFT(catNIPnftAddress).randomNumber() ).add( ICatNIP(catNIPcurrencyAddress).randomNumber() );
