...
contract CorgiShop is Ownable {
    ...
    mapping(uint256 => EggBox) eggBoxs;
    ...
    function buyEggBox(uint8 _comboType, address _referral) public {
        ...
        eggBoxLength++;
        eggBoxs[eggBoxLength].owner = msg.sender;
        eggBoxs[eggBoxLength].isOpened = false;

        for (uint256 i = 0; i < _comboType; i++) {
            uint256 eggType = random();
            corgiPet.mint(msg.sender, eggType);
        }
        emit BuyEggBoxEvent(msg.sender, _comboType, eggBoxLength, price);
    }
...
interface ICorgiPet {
    function mint(address _owner, uint256 _eggType) external;
}
