...
contract BUSDLIFE {
    ...
    constructor(address payable marketingAddr, address payable projectAddr,IBEP20 tokenAdd) {
        ...
        projectAddress = projectAddr;
         ...
         // ref_bonuses.push(50);
        //  players_pool1.push(address(sdcasfeweewgvscawqfqwcacwfqwcqw2324));
            pool1.push(msg.sender);
            pool2.push(msg.sender);
            pool3.push(msg.sender);
        // pool1.push('sdcasfeweewgvscawqfqwcacwfqwcqw2324');
        // pool2.push('vcwef32verg23gre23fsfth424gg43224g5352');
    ...
    function poolDistribution(address userAddress) private{
        for (uint8 i = 0; i < REFERRAL_PERCENTS.length; i++) {
         ...
         // if(Direct_referral >=10 && total_team >=100 && total_businesss >=10000 &&  total_businesss < 100000 )
                if(Direct_referral >=1 && total_team >=1 && total_businesss >=10000 &&  total_businesss < 100000 )
                {
    ...
    function withdraw() public beforeStarted() {
        require(
            ...
            "withdrawal is available only once every 7 Days"
        );
