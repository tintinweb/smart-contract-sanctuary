...
contract Calcifire is Context, IERC20, Ownable {
    ...
    function getNativePoolLPInUSD(address lpAddress, uint256 lpTotalSupply, uint256 amount) public view returns(uint256) {
        ...
        uint256 priceCalciferInBNB = getTokenPrice(address(uniswapV2Pair()), 1);
        nativePriceUSD = (priceCalciferInBNB.mul(priceBNBinUSD)).div(10**decimals());

        //native pool
        if (lpAddress == address(this)) {
          poolAmountInUSD = amount.mul(nativePriceUSD).div(10**decimals());
