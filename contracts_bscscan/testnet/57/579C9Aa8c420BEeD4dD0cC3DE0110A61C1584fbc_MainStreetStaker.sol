...
contract MainStreetStaker is Ownable {
    ...
    address public BANANA_POOL;
    address public GNANA_POOL;
    address public APESWAP_ROUTER;
    ...
    function setDistributionPercentage(uint percentage) public onlyOwner() {
        require(percentage > 0 && percentage < 100, "percentage should be between 1-99");
    ...
    function buyBanana(uint forBNBAmount) public payable returns(uint boughtBananaAmount) {
    ...
    function buyGanana(uint forBananaAmount) public returns(uint boughtGnanaAmount) {
        IApeSwapTreasury(APESWAP_TREASURY).buy(forBananaAmount);
        return IBEP20(GNANA_TOKEN).balanceOf(address(this));
    }

    function stakeBanana(uint amount) public returns (uint stakedBananaAmount) {
        IBananaPool(BANANA_POOL).enterStaking(amount);
        return amount;
    }

    function harvestBanana() public returns (uint harvestedAmount) {
    ...
    // uint _percent is for the percentage of banana staked, returns unstaked + harvested banana
    function withdrawBanana(uint percent) public onlyOwner() {
        require(percent > 0 && percent < 101, "incorrect percentage");
    ...
    function stakeGanana(uint amount) public returns (uint stakedGananaAmount) {
        IGnanaPool(GNANA_POOL).deposit(amount);
        return amount;
    }

    function harvestGnana() public returns(uint harvestedAmount) {
    ...
    // uint _percent is for the percentage of gnana staked, returns unstaked gnana
    function withdrawGnana(uint _percent) public onlyOwner() {
        require(_percent > 0 && _percent < 101, "incorrect percentage");
    ...
    function deposit() external payable {
        require(NFT_PRICE > 0, "NFT minting price not set yet");
        require(msg.value >= NFT_PRICE, "not enough balance");
        ...
        TOTAL_GNANA_STAKED += stakeGanana(gnanaBought);
    }
