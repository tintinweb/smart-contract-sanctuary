...
contract Calcifire is Context, IERC20, Ownable {
    ...
    uint256 private _tokenTotal = 3000000 * 10**18;
    ...
    uint256 public _taxFee = 1; // Rewards
    uint256 private _previousTaxFee = _taxFee;
    uint256 public _sellTaxFee = 1; // Rewards
    ...
    uint256 public _sellCommunityFee = 0;
    ...
    bool public autoSellForTreasury = true;
    ...
    // rateRatio keeps tokenSupply and reflectSupply ratio fixed while token is mintable
    uint256 _rateRatio = 1;
    ...
    constructor (address treasuryAddress, address communityAddress, address liquidityAddress) public {
        ...
        _reflectOwned[owner()] = _reflectTotal;
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        uint256 ownedAmount = tokenFromReflection(_reflectOwned[account]).add(amount);

        _tokenTotal = _tokenTotal.add(amount);

        // rateRatio keeps tokenSupply and reflectSupply ratio fixed while token is mintable
        _rateRatio =  currentRate.div(_getRate());

        _reflectOwned[account] = ownedAmount.mul(_getRate().div(_rateRatio));
   ...
   function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
    ...
    function _getRate() private view returns(uint256) {
        ...
        return (reflectSupply.div(tokenSupply)).mul(_rateRatio);
    }
