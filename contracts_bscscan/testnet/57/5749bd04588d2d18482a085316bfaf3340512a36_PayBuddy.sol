...
contract PayBuddyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("PayBuddy_Dividend_Tracker", "PayBuddy_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "PayBuddy_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "PayBuddy_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PayBuddy contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PayBuddy_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "PayBuddy_Dividend_Tracker: Cannot update claimWait to same value");
...
contract PayBuddy is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    PayBuddyDividendTracker public dividendTracker;

    uint256 public _totalSupply = 100_000_000_000 * 10**9;
    ...
    constructor() ERC20("Pay Buddy", "PayBuddy") 
    {
        BNBRewardsFee = 3;
        liquidityFee = 3;
        marketingFee = 3;
        ...
        marketingWallet = payable(0x0572689b9Cb91789325C32E5f4b3d2b6A4e7D526);
        developerWallet = payable(0xdb995a5e8564Af80C411374fAf951cE267558DAA);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(developerFee); // total fee transfer and buy

    	dividendTracker = new PayBuddyDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PayBuddy: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PayBuddy: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PayBuddy: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PayBuddy: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PayBuddy: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PayBuddy: Cannot update gasForProcessing to same value");
