...
contract iMOON is ERC20Detailed, Ownable {
  ...
  uint256 private startingSupply = INITIAL_FRAGMENTS_SUPPLY;

  uint256 public transactionTax = 1500;
  ...
  address payable public marketingAddress;
    ...
    uint256 private _maxTxAmount = _totalSupply.div(maxTxDivider);
  ...
  mapping (address => bool) private _isAirDropWallet;

  bool private privateSaleDropCompleted = false;

  // TOTAL_iMOON is a multiple of INITIAL_FRAGMENTS_SUPPLY so that _imoonPerFragment is an integer.
  // Use the highest value that fits in a uint256 for max granularity.
  uint256 private constant TOTAL_iMOON = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

  // MAX_SUPPLY = maximum integer < (sqrt(4*TOTAL_iMOON + 1) - 1) / 2
  uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

  uint256 private _totalSupply;
  uint256 private _imoonPerFragment;
  mapping(address => uint256) private _imoonBalances;
  ...
  ERC20Detailed("iMoon", "iMOON", uint8(DECIMALS))
  payable
  public
  {
    ...
    _imoonBalances[msg.sender] = TOTAL_iMOON;
    _imoonPerFragment = TOTAL_iMOON.div(_totalSupply);
...
*/
  ...
  returns (uint256)
  {
    return _imoonBalances[who].div(_imoonPerFragment);
  }
  ...
  returns (bool)
  {
    ...
    require(value > 0);
    if(limiton == true && _hasLimits(from, to))
            require(value <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        if(limiton == true && _hasLimits(from, to)

        ) {
            uint256 contractBalanceRecepient = balanceOf(to);
            require(contractBalanceRecepient + value <= _maxWalletSize, "Transfer amount exceeds the maxWalletSize.");
        }
  ...
  function _transferStandard(address sender, address recipient, uint256 amount) private {
    ...
    uint256 imoonDeduct = amount.mul(_imoonPerFragment);
    uint256 imoonValue = tTransferAmount.mul(_imoonPerFragment);
    _imoonBalances[sender] = _imoonBalances[sender].sub(imoonDeduct);
    _imoonBalances[recipient] = _imoonBalances[recipient].add(imoonValue);
    _takeFee(sender, tFee);
    emit Transfer(sender, recipient, amount);
  }

  function _transferExcluded(address sender, address recipient, uint256 amount) private {
    uint256 imoonValue = amount.mul(_imoonPerFragment);
    _imoonBalances[sender] = _imoonBalances[sender].sub(imoonValue);
    _imoonBalances[recipient] = _imoonBalances[recipient].add(imoonValue);
    emit Transfer(sender, recipient, amount);
  }
  ...
  function setlimitson(bool _enabled) public onlyOwner {
    limiton = _enabled;
    }
  ...
  function _takeFee(address sender, uint256 tFee) private {
    uint256 rFee = tFee.mul(_imoonPerFragment);
    _imoonBalances[address(this)] = _imoonBalances[address(this)].add(rFee);
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
    ...
    // <- this breaks the ETH -> iMOON swap when swap+liquify is triggered
 ...
 */
  ...
  function airDropWallet(address _addr) public {
     _isAirDropWallet[_addr] = true;
  }
  
  function isAirDropWallet(address account) public view returns (bool) {
        return _isAirDropWallet[account];
  }
