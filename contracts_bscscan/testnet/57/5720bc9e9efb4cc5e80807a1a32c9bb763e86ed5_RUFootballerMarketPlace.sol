...
contract RUFootballerMarketPlace is Pausable {
    ...
    function setOwnerCut(uint256 _ownerCut) public onlyOwner {
        require(_ownerCut <= 10000, "Owner cut must be less than or equal 10000");
        ownerCut = _ownerCut;
    }
    ...
    modifier canBeStoredWith64Bits(uint256 _value) {
        require(_value <= 18446744073709551615, "Can not Be Stored With 64Bits");
        _;
    }

    modifier canBeStoredWith128Bits(uint256 _value) {
        require(_value < 340282366920938463463374607431768211455, "Can not Be Stored With 128Bits");
        _;
    }
    ...
    function getAuction(
        ...
        require(_isOnAuction(_auction), "auction is not started");
        return (
        _auction.seller,
    ...
    canBeStoredWith64Bits(_duration) {
        ...
        require(_owns(_nftAddress, _seller, _tokenId), "nft token is not seller");
    ...
    function cancelAuction(address _nftAddress, uint256 _tokenId) external {
        Auction storage _auction = auctions[_nftAddress][_tokenId];
        require(_isOnAuction(_auction), "Auction is not start");
        require(msg.sender == _auction.seller, "Sender is not auction seller");
    ...
    function _addAuction(
        ...
        require(_auction.duration >= 1 minutes, "Auction duration must greater than 1 minutes");
    ...
    function _cancelAuction(address _nftAddress, uint256 _tokenId, address _seller) internal {
        Auction storage _auction = auctions[_nftAddress][_tokenId];
        uint64  startedAt = _auction.startedAt;
        _removeAuction(_nftAddress, _tokenId);
        _transfer(_nftAddress, _seller, _tokenId);
        emit AuctionCancelled(_nftAddress, _tokenId, startedAt, uint64(block.timestamp));
    }
    ...
    function _bid(
        ...
        require(_bidAmount >= _price, "Bid Amount must greater than or equal auction price");
        ...
        // Tell the world!
        emit AuctionSuccessful(_nftAddress, _tokenId, _price, msg.sender, uint64(block.timestamp), startedAt, uint64(block.timestamp));
