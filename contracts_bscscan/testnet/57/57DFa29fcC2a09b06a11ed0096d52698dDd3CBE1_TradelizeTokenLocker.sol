...
contract TradelizeTokenLocker is Ownable {
    ...
    function unClaimedAmount(address _holder) public view returns (uint96) {
        ...
        // initial approved amount (for all holders of this lock type and before linear vesting)
        uint96 initialApprovedAmount = lockConfig.supply.sub(lockConfig.linearVestingAmount).toUInt96();
        
        uint96 currentLinearVestingAmount;
        
        // computation current linear vesting amount (for all holders of this lock type)
        if (block.number > linearVestingStartBlock && lockConfig.linearVestingAmount > 0) {
            if (block.number < linearVestingStartBlock + lockConfig.linearVestingPeriod) {
                currentLinearVestingAmount = Math.min(block.number, linearVestingStartBlock.add(lockConfig.linearVestingPeriod)).
                    sub(linearVestingStartBlock).
                    mul(lockConfig.linearVestingAmount.div(lockConfig.linearVestingPeriod)).toUInt96();
            } else {
                currentLinearVestingAmount = lockConfig.linearVestingAmount;
            }
        }
        
        return initialApprovedAmount.
                  add(currentLinearVestingAmount).
