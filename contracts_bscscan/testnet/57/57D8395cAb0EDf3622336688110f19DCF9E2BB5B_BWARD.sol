...
Total Supply:       - 1.000.000.000 BWARD

40% Public Sale:    -   400.000.000 BWARD
20% Burn/dEaD:      -   200.000.000 BWARD
20% Pancake LP      -   200.000.000 BWARD
10% Private Sale:   -   100.000.000 BWARD

 4% Marketing:      -    40.000.000 BWARD
 3% Team:           -    30.000.000 BWARD
 3% Dev:            -    30.000.000 BWARD
_____________________________________________________________________________________________________________________________

14% Reward Fee:     - 14% of each transaction that will be used for BNB reward pool
 4% liquidity Fee:  - 4% of each transaction that will be added as liquidity
 4% Sell Fee:       - Additional 4% fee to apply on sell transactions. Half of it will go to liquidity, other half to rewards
_____________________________________________________________________________________________________________________________

Website  - https://bwardtoken.com/
Telegram - https://t.me/BardTokenOfficial
...
// File: contracts\BWARDBase.sol


// Base class that implements: BEP20 interface, fees & swaps
abstract contract BWARDBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	// MAIN TOKEN PROPERTIES
	string private constant NAME = "BWARD";
	string private constant SYMBOL = "BWARD";
	...
	uint256 private constant _totalTokens = 1000000000 * 10**DECIMALS;	//1 billion total supply
	...
	uint256 private _tokenSwapThreshold = _totalTokens / 100; //There should be at least 0.0001% of the total supply in the contract before triggering a swap
	...
	constructor (address routerAddress) {
		...
		// Initialize PancakeSwap V2 router and BWARD <-> BNB pair.  Router address will be: 0x10ed43c718714eb63d5aa57b78b54704e256024e or for testnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
		setPancakeSwapRouter(routerAddress);

		// 4% liquidity fee, 14% reward fee, 4% additional sell fee
		setFees(4, 14, 4);
	...
	function activate() public onlyOwner {
		...
		setTransactionLimit(1); // only 0.1% of the total supply can be sold at once
		activateBuying();
		onActivated();
	}
	...
	// This function swaps a {tokenAmount} of BWARD tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for BWARD -> WBNB
	...
	// Returns how many more $BWARD tokens are needed in the contract before triggering a swap
...
// File: contracts\BWARD.sol

// Implements rewards & burns
contract BWARD is BWARDBase {
	...
	uint256 private _totalBNBAsBWARDClaimed; // The total number of BNB that was converted to BWARD and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsBWARDClaimed; // The amount of BNB converted to BWARD and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy BWARD automatically
	...
	uint256 private _globalRewardDampeningPercentage = 4; // Rewards are reduced by 4% at the start to fill the main BNB pool faster and ensure consistency in rewards
	...
	bool private _reimburseAfterBWARDClaimFailure; // If true, and BWARD reward claim portion fails, the portion will be given as BNB instead
	...
	constructor (address routerAddress) BWARDBase(routerAddress) {
	...
	function onActivated() internal override {
		...
		setReimburseAfterBWARDClaimFailure(true);
		setMinRewardBalance(5 * 10**decimals());  //At least 5000 tokens are required to be eligible for rewards
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough BWARD balance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim BWARD tokens
		if (!claimBWARD(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterBWARDClaimFailure) {
	...
	function claimBWARD(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsBWARDClaimed[user] += bnbAmount;
		_totalBNBAsBWARDClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $BWARD on a zero balance address:
		...
		// Depending on the % of $BWARD tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsBWARD(address byAddress) public view returns (uint256) {
		return _bnbAsBWARDClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsBWARD() public view returns (uint256) {
		return _totalBNBAsBWARDClaimed;
	}
	...
	function reimburseAfterBWARDClaimFailure() public view returns(bool) {
		return _reimburseAfterBWARDClaimFailure;
	}


	function setReimburseAfterBWARDClaimFailure(bool value) public onlyOwner {
		_reimburseAfterBWARDClaimFailure = value;
	}
