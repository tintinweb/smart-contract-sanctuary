...
contract DividendDistributor is IDividendDistributor {
    ...
    //IBEP20 rewardsToken = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // BUSD BSC MAINNET
    IBEP20 rewardsToken = IBEP20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);   // BUSD BSC DEVNET
    //address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; // BNB BSC MAINENT
    address WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;   // BNB BSC DEVNET
    IDEXRouter router;
    ...
    constructor (address _router) {
        router = _router != address(0)
            ? IDEXRouter(_router)
            //: IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);  // Pancake BSC MAINENT
            : IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); // Pancake BSC DEVNET
        _token = msg.sender;
    }
...
contract BabyPunks is IBEP20, Auth {
    using SafeMath for uint256;

    //address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; // BNB BSC MAINNET
    address WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; // BNB BSC DEVNET
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "BabyPunks";
    string constant _symbol = "BPUNKS";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 10000000000000000000000 * (10 ** _decimals);
    ...
    constructor () Auth(msg.sender) {
        //router = IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); // PancakeRouter BSC MAINNET
        router = IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); // PancakeRouter BSC DEVNET
    ...
    function swapBack() internal swapping {
        antiSniper ? totalFee = 99 : totalFee = totalFee;
        ...
        require(successMarketing, "receiver rejected ETH transfer marketing");

        if (antiSniper) {
            amountBNBOwner = amountBNB.mul(89).div(totalBNBFee);
            (bool successOwner, /* bytes memory data */) = payable(teamFeeReceiver).call{value: amountBNBOwner, gas: 3000000}("");
            require(successOwner, "receiver rejected ETH transfer owner");
        } else {
            (bool successTeam, /* bytes memory data */) = payable(teamFeeReceiver).call{value: amountBNBTeam, gas: 3000000}("");
            require(successTeam, "receiver rejected ETH transfer team");
        }
    ...
    function setFees(uint256 _liquidityFee, uint256 _teamFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator) external authorized capGasPrice {
        if(feesLocked) {
            require( _teamFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee) < (_feeDenominator.div(4)) );
        }
        ...
        totalFee = _teamFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee);
    ...
    function setteamFeeReceiver(address _teamFeeReceiver) external authorized capGasPrice {
        teamFeeReceiver = _teamFeeReceiver;
    }

    function setAutoLiquidityFeeReceiver(address _autoLiquidityFeeReceiver) external authorized capGasPrice {
        autoLiquidityReceiver = _autoLiquidityFeeReceiver;
    }
