...
import "./ERC721.sol";
import "./HobbsBronze.sol";

contract NFTFeeClaim is TokensRecoverable
{
    ...
    address public bronzeNFTAddress;
    //pass in nft id, spit back the available claim
    mapping(uint => uint256) public availableClaimBronze;
    ...
    /* function setRootFeederAddress(address _rootFeederAddress) public onlyOwner
    {
        rootFeederAddress = _rootFeederAddress;
    } */
    function setBronzeNFTAddress(address _bronzeNFTAddress) public onlyOwner
    {
        bronzeNFTAddress = _bronzeNFTAddress;
    }
    ...
    function depositFees(uint256 _amount) public 
    {
        calculatePayouts(_amount);
        IERC20(paymentToken).transferFrom(msg.sender, address(this), _amount);
    }

    //todo make below private/internal
    function calculatePayouts(uint256 _amount) public 
    {
        uint totalMinted = HobbsBronze(bronzeNFTAddress).totalMinted();
        //todo safemt
        uint amountPerBronze = _amount / totalMinted;
        for (uint i = 1; i <= totalMinted; i++)
        {
            uint nftId = i;
                availableClaimBronze[nftId] += amountPerBronze;
        }
    }

    function claimPayout(uint256 _nftId) public 
    {
        require (availableClaimBronze[_nftId] > 0, "No payout available");
        require(checkNFTBalance(msg.sender, _nftId), "No NFT held or incorrect NFT ID");
        uint payment = availableClaimBronze[_nftId];
        availableClaimBronze[_nftId] = 0;
        IERC20(paymentToken).transfer(msg.sender, payment);
        
    }
    function checkNFTBalance(address _address, uint256 _nftId) public view returns (bool)
    {
        return HobbsBronze(bronzeNFTAddress).ownerOf(_nftId) == _address;
    }

    //ToDO test Functions delete later
    function viewTotalMintedNFTS() public view returns (uint)
    {
        return HobbsBronze(bronzeNFTAddress).totalMinted();
    }
   ...
   /*  function setFees(IERC20 token, uint256 burnRate, address[] memory collectors, uint256[] memory rates) public onlyOwner // 100% = 10000
    {
