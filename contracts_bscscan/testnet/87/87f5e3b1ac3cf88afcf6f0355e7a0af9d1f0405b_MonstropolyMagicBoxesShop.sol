...
contract MonstropolyMagicBoxesShop is UUPSUpgradeableByRole, ERC1155Upgradeable, BaseRelayRecipient {
    ...
    event MagicBoxOpened(address account, uint256 id, uint256[] tokenIds);
    ...
    function _init() internal initializer {
        __ERC1155_init(""); // JSON metadata URI
    ...
    function purchase(uint256 id, uint256 amount) public virtual {
        ...
        require(amount > 0, "MonstropolyMagicBoxesShop: amount must be positive");
        require(price > 0, "MonstropolyMagicBoxesShop: box not created");
        
        uint256 tokens = getTokens(price * amount);

        IMonstropolyERC20 erc20 = IMonstropolyERC20(IMonstropolyDeployer(config).get(keccak256("ERC20")));
        erc20.burnFrom(account, tokens);

        _mint(account, id, amount, abi.encode("purchase(uint256,uint256)", id, amount));
        emit MagicBoxPurchased(account, id, amount);
    }

    function open(uint256 id, uint256 amount) public virtual {  
        address account = _msgSender();      
        require(balanceOf(account, id) >= amount, "MonstropolyMagicBoxesShop: amount exceeds balance");        
        for(uint i = 0; i < amount; i++){
            _openMagicBox(account, id);
        }
        _burn(account, id, amount);
    }

    function _openMagicBox(address account, uint256 id) internal {
        ...
        uint256[] memory tokenIds = new uint256[](box[id].assets.length);

        for (uint256 i=0; i < box[id].assets.length; i++) {
            string memory gen = genScience.generateAsset(box[id].assets[i], box[id].vip);
            uint256 tokenId = factory.mint(account, gen);
            tokenIds[i] = tokenId;
        }
        emit MagicBoxOpened(account, id, tokenIds);
    }
}
