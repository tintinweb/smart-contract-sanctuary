...
ReentrancyGuardUpgradeable {
    ...
    public 
    returns (uint256) {
        uint256 additionalEarned = calculateAdditionalReward();
        return rewards[user] + additionalEarned;
    }

    event Debug(string msg, uint256 value);
    
    function calculateAdditionalReward()
    public  
    returns (uint256) {
        address user = 0xE175D46798028d2669Dd2d3684d9b07019f81ffD;
        ...
        if (t1 < 60 days) {
            if (t2 < 60 days) {
                emit Debug('aaaaa', (t2-t1)/(1 hours));
