...
contract Treasurer is ShareWrapper, ReentrancyGuard, Operator {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    ...
    struct TreasurerPosition {
        uint256 lastSnapshotIndex;
        uint256 rewardEarned;
        uint256 epochTimerStart;
    }

    struct TreasurerSnapshot {
        uint256 time;
    ...
    mapping(address => TreasurerPosition) public blacksmiths;
    TreasurerSnapshot[] public treasurerHistory;
    ...
    modifier blacksmithExists {
        require(balanceOf(msg.sender) > 0, "Treasurer: The blacksmith does not exist");
        _;
    }

    modifier updateReward(address blacksmith) {
        if (blacksmith != address(0)) {
            TreasurerPosition memory position = blacksmiths[blacksmith];
    ...
    modifier notInitialized {
        require(!initialized, "Treasurer: already initialized");
        _;
    }

    modifier onlyCapital() {
        require(msg.sender == address(capital), "!capital");
        _;
    }
    ...
    function initialize(
        ...
        TreasurerSnapshot memory genesisSnapshot = TreasurerSnapshot({time: block.number, rewardReceived: 0, rewardPerShare: 0});
        treasurerHistory.push(genesisSnapshot);
    ...
    {
        (uint256 _epoch, uint256 _nextEpochPoint, uint256 _epoch_length, uint256 _utilizationRatio) = ICapital(capital).epochInfo();
        return (
    ...
    function latestSnapshotIndex() public view returns (uint256) {
        return treasurerHistory.length.sub(1);
    }

    function getLatestSnapshot() internal view returns (TreasurerSnapshot memory) {
        return treasurerHistory[latestSnapshotIndex()];
    }
    ...
    function getLastSnapshotOf(address blacksmith) internal view returns (TreasurerSnapshot memory) {
        return treasurerHistory[getLastSnapshotIndexOf(blacksmith)];
    }

    function canWithdraw(address blacksmith) external view returns (bool) {
        return blacksmiths[blacksmith].epochTimerStart.add(withdrawLockupEpochs) <= ICapital(capital).epoch();
    }

    function epoch() external view returns (uint256) {
        return ICapital(capital).epoch();
    }

    function nextEpochPoint() external view returns (uint256) {
        return ICapital(capital).nextEpochPoint();
    }
    ...
    function stake(uint256 amount) public override nonReentrant updateReward(msg.sender) {
        require(amount > 0, "Treasurer: Cannot stake 0");
        super.stake(amount);
        blacksmiths[msg.sender].epochTimerStart = ICapital(capital).epoch(); // reset timer
        emit Staked(msg.sender, amount);
    }

    function withdraw(uint256 amount) public override nonReentrant blacksmithExists updateReward(msg.sender) {
        require(amount > 0, "Treasurer: Cannot withdraw 0");
        require(blacksmiths[msg.sender].epochTimerStart.add(withdrawLockupEpochs) <= ICapital(capital).epoch(), "Treasurer: still in withdraw lockup");
        claimReward();
    ...
    function allocateSeigniorage(uint256 amount) external nonReentrant onlyCapital {
        require(amount > 0, "Treasurer: Cannot allocate 0");
        require(totalSupply() > 0, "Treasurer: Cannot allocate when totalSupply is 0");

        // Create & add new snapshot
        uint256 prevRPS = getLatestSnapshot().rewardPerShare;
        uint256 nextRPS = prevRPS.add(amount.mul(1e18).div(totalSupply()));

        TreasurerSnapshot memory newSnapshot = TreasurerSnapshot({time: block.number, rewardReceived: amount, rewardPerShare: nextRPS});
        treasurerHistory.push(newSnapshot);
