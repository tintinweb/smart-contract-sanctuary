...
contract VOYCEVote is Ownable {
    ...
    function initialize(
        ...
        IERC20 tokenToHold_,
        uint256 amountToHold_
    ) external initializer {
        require(startTime_ > block.timestamp, "wrong starting block");
        require(endTime_ > startTime_, "wrong end block");
        require(charityNames.length >= 2, "2 or more options required");
        startTime = startTime_;
        endTime = endTime_;
        quorum = quorum_;
        for (uint8 i; i < charityNames.length; i++) {
            bytes32 hash = keccak256(abi.encodePacked(charityNames[i]));
            require(!charityExists[hash], "repeated charity name");
            charityExists[hash] = true;
            options.push(CharityOption(charityNames[i]));
        }
    ...
    function removeCharity(uint8 index) external onlyOwner {
        require(block.timestamp < startTime, "Poll already started");
        require(options.length > 2, "cannot delete, 2 or more options required");
        CharityOption memory option = options[index];
        bytes32 hash = keccak256(abi.encodePacked(option.charityName));
        charityExists[hash] = false;
        options[index] = options[options.length - 1];
        options.pop();
    }
