...
contract Monster is Ownable, ERC20 {
    using SafeMath for uint256;
    IManager public manager;

    uint256 public amountPlayToEarn = 260 * 10**6 * 10**18;
    uint256 public playToEarnReward;
    uint256 internal amountFarm = 130 * 10**6 * 10**18;
    uint256 private farmReward;

    uint256 public tokenForMkt = 2 * 10**6 * 10**18;
    ...
    constructor(string memory name, string memory symbol) ERC20(name, symbol) {
        addressForMkt = _msgSender();
    }

    modifier onlyFarmOwners() {
        require(manager.farmOwners(_msgSender()), "must be farmer");
        _;
    }

    modifier onlyEvolver() {
        require(manager.evolvers(_msgSender()), "must be evolver");
        _;
    }
    ...
    function farm(address recipient, uint256 amount) external onlyFarmOwners {
        ...
        // mint from 13% farming tokenomic, it's mint for you.
    ...
    function win(address winner, uint256 reward) external onlyEvolver {
        ...
        // mint from 13% fighting tokenomic, it's for you.
