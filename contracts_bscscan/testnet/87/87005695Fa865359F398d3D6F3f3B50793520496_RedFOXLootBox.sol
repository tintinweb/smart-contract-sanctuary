...
contract RedFOXLootBox is VRFConsumerBase, Ownable {
    ...
    uint256[] public commonCardIds;
    uint256[] public unCommonCardIds;
    uint256[] public rareCardIds;
    uint256[] public ultraRareCardIds;
    uint256[] public slammerCardIds;
    uint256[] public slammerGoldCardIds;
    ...
    struct OpenedPack {
        ...
        //
        uint16[] commonCards;
        uint16[] unCommonCards;
        uint16[] rareCards;
        uint16[] ultraRareCards;
        uint16[] slammerCards;
        uint16[] slammerGoldCards;
    }
    ...
    {   
        ...
        // Link class with cards
        classToIds[Class.Common] = commonCardIds;
        classToIds[Class.Uncommon] = unCommonCardIds;
        classToIds[Class.Rare] = rareCardIds;
        classToIds[Class.UltraRare] = ultraRareCardIds;
        classToIds[Class.Slammer] = slammerCardIds;
        classToIds[Class.SlammerGold] = slammerGoldCardIds;
    ...
    function createCard(Class _class) public onlyOwner {
        uint256 cardId = redFoxCollectible.create(msg.sender, 0, "");
        uint256[] storage cardType = classToIds[_class];
        cardType.push(cardId);
    }
    ...
    function getPackSetting(Pack _pack)
        ...
        returns (
            uint256 price,
            uint256 numberOfDraws,
            uint16[NUM_CLASSES] memory classProbabilitiesass,
            bool hasGuaranteedClasses,
        ...
        override
    {
        ...
        userNonce[owner] = userNonce[owner].add(_randomness.mod(10));
    }
    ...
    function openRandomCards(
        ...
        for (uint256 i = 0; i < packSettings.numberOfDraws; i++) {}
    }
    ...
    function pickRandomIn(
        ...
        uint256[] memory classIds = classToIds[_class];

        uint256[] memory randomResults = expand(
            _randomness,
            _numbers,
            _randomness.mod((uint256(_class)).add(1))
        );

        return randomResults;
    }

    function getRandomCardIdsByClass(Class _class, uint256[] memory _numbers) public view returns (uint256[] memory ids) {
        ids = new uint256[](_numbers.length);
        
        for (uint256 i = 0; i < _numbers.length; i++) {
            ids[i] = _numbers[i].mod(classToIds[_class].length);
        }
