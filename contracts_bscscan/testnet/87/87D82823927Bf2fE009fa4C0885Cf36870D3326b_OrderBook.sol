...
contract OrderBook {
    ...
    constructor() {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256("1"),
                chainId,
                0x1C56346CD2A2Bf3202F771f50d3D14a367B48070// address(this)
            )
        );
    }
    ...
    // Creates an order
    function createOrder(Orders.Order memory order) public returns (bytes32) {
        order.validate();
    ...
    function hashOfOrder(Orders.Order memory order)
        public
        pure
        returns (bytes32)
    {
    ...
    function recoverOrder(Orders.Order memory order)
        public
        view
        returns (address)
    {
        bytes32 hash = order.hash();
        return EIP712.recover(DOMAIN_SEPARATOR, hash, order.v, order.r, order.s);
    }
}
