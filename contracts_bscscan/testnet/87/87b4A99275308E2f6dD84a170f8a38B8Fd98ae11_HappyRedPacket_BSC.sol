...
import {IBEP20MetadataUpgradeable} from "./interface/IBEP20MetadataUpgradeable.sol";
import {SafeBEP20Upgradeable} from "./interface/SafeBEP20Upgradeable.sol";
...
contract HappyRedPacket_BSC is Initializable, UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    external payable {
        ...
        if (_token_type == 0)
            ...
            uint256 balance_before_transfer = IBEP20MetadataUpgradeable(_token_addr).balanceOf(address(this));
            IBEP20MetadataUpgradeable(_token_addr).safeTransferFrom(msg.sender, address(this), _total_tokens);
            uint256 balance_after_transfer = IBEP20MetadataUpgradeable(_token_addr).balanceOf(address(this));
    ...
    function claim(bytes32 id, address payable recipient, uint8 _v, bytes32 _r, bytes32 _s)
    external returns (uint claimed) {
        ...
        if (ifrandom == 1) {
            if (total_number - claimed_number == 1)
                ...
                claimed_tokens = random(nonce) % SafeMathUpgradeable.div(SafeMathUpgradeable.mul(remaining_tokens, 2), total_number - claimed_number);
            if (claimed_tokens == 0)
                claimed_tokens = 1;
        } else {
            if (total_number - claimed_number == 1)
    ...
    **/
    ...
    function transfer_token(address token_address, address recipient_address, uint amount) internal{
        IBEP20MetadataUpgradeable(token_address).safeTransfer(recipient_address, amount);
    }

    function random(uint32 nonce_rand) internal view returns (uint rand) {
        return uint(keccak256(abi.encodePacked(nonce_rand, msg.sender, block.timestamp))) + 1 ;
    }
