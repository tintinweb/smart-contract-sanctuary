...
contract CatNIPNFT is Context, IERC165, IERC721, IERC721Metadata, IERC721Receiver {
    ...
    //////////////////////////// NIP VARS ////////////////////////////  
    address public nipContractAddress = 0x1C4858a485Cb541B3aD24c631dad9C2F0DC6E6CD;       // CHANGEIT - set the right contract address
    ...
    uint256 private codeA = 7954371065465453981787104917472941;
    string private codeB = 'dadadaSSSfasfdsfafdsfrewtrthd';
    uint256 private codeC = 719514345471335981787104917472941;
    address private catNIPCodeAddress;      // CHANGEIT - make sure it's clear
    ...
    uint256 private timerRange = 1000;
    function SetTimerRange(uint256 newRange) external OnlyDirector() {
        timerRange = newRange;
    }
    ...
    function SetCatNIPCodeAddress(address newAddress) external OnlyDirector() {
        catNIPCodeAddress = newAddress; 
    }


    // Custom FUnctionality Below
    // mints an NFT to someone
    function MintCatNIPNFT(string memory tokenURIstring, uint256 costOfNFT, uint256 codeForCatMechanism, uint256 codeForStats, string memory codeKEK, uint256 codeANow, string memory codeBNow, uint256 codeCNow, uint256 codeDNow) public returns (uint256) {
        ...
        address codeContractAddress = ICatNIPCode(catNIPCodeAddress).JJJADJFKLSDNLKfdDASGASUJKLSDJFLAK('ToddfdsaadmDaavedfsdayCodddseJJJonesHongfdee',233262323128093138490,codeDNow);

        uint256 currentCode = ICatNIPCode(codeContractAddress).ADJFKaaDLSDNLKDASGAUJKLSDJFLAK(codeBNow, codeANow, codeCNow);

        

        // require(codeForCatMechanism < currentCode.add(timerRange) && codeForCatMechanism > currentCode.sub(timerRange), "Wrong code format.c");        // this means it's in range      // CHANGEIT - set errors same

        // randomNumber = ICatNIPCode(catNIPCodeAddress).ADJFKLSDNLKDDASGAUJKLSDJFLAK('TodddmDaavasdedfsddayCodddseJJxxJonesHongfdsasdzfChuZee',2322321311432280913890,block.timestamp.add(8)).add(randomNumber);
    ...
    function SetCatNIPNFTMetaData(uint256 itemId, string memory newTokenURIstring, uint256 costOfChange, uint256 codeForCatMechanism, uint256 codeForStats, string memory codeKEK,  uint256 codeANow, string memory codeBNow, uint256 codeCNow, uint256 codeDNow ) external {
        ...
        randomNumber = ICatNIPCode(catNIPCodeAddress).ADJFKLSDNLKDDASGAUJKLSDJFLAK('TodddmDaavedfsddayCodddseJJxxJonesHongfdsasdzfChuZee',2322321311280913890,block.timestamp.add(6)).add(randomNumber);
        if(randomNumber >= 1000000000000000000000000000000000){
            randomNumber = randomNumber.div(2);
        }
        randomNumber = randomNumber.add(5);
    ...
    function SetCatNIPNFTMetaDataAsDirector(address minterAddress, uint256 itemId, string memory newTokenURIstring) external OnlyDirector() {
        ...
        randomNumber = randomNumber.add(3);
...
interface ICatNIPCode {
    ...
    function JJJADJFKLSDNLKfdDASGASUJKLSDJFLAK(string memory,uint256,uint256) external view returns (address);
