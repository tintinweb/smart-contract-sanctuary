...
contract LitedexRouter is ILitedexRouter02 {
    ...
    function addLiquidityETH(
        ...
        address pair = LitedexLibrary.pairFor(factory, token, WBNB);
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WBNB, 'Litedex: Invalid Path');
        amounts = LitedexLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'Litedex: Insufficient Output Amount');
        IWBNB(WBNB).deposit{value: amounts[0]}();
        assert(IWBNB(WBNB).transfer(LitedexLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WBNB, 'Litedex: Invalid Path');
        amounts = LitedexLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'Litedex: Excessive Input Amount');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, LitedexLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        ensure(deadline)
    {
        ...
        assert(IWBNB(WBNB).transfer(LitedexLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IBEP20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IBEP20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
...
library LitedexLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'8f72911e593e733c4cc22e7b1422613343a3f91b61c639b23ffceefadbbdc752' // init code hash
            ))));
    }
