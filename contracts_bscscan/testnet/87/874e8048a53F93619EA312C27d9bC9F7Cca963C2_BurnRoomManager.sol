...
contract BurnRoomManager is SafeOwnable, Random {
    ...
    event OpenBlindBox(uint rid, uint loop, address to, uint nftId, uint typeId, uint num);
    ...
    mapping(uint256 => mapping(uint256 => bool)) public allOpened;
    ...
    function finishRandom(bytes32 _requestId) internal override {
        ...
        for (uint i = 0; i < random.num; i ++) {
            ...
            for (; rangeIndex < rangeInfo[random.rid].length; rangeIndex ++) {
                if (nftRange >= rangeInfo[random.rid][rangeIndex].startIndex && nftRange < rangeInfo[random.rid][rangeIndex].endIndex) {
                    ...
                    emit OpenBlindBox(random.rid, random.loop, random.to, nftId, rangeInfo[random.rid][rangeIndex].nftType, 1);
                    break;
                }
            }
        ...
        RoomInfo storage room = roomInfo[random.rid];
        if (random.loop < room.currentLoop || room.openNum == room.totalNum) {
            allOpened[random.rid][random.loop] = true; 
        }
        for (uint i = random.loop; i < room.currentLoop; i ++) {
            if (allOpened[random.rid][i]) {
                if (roomReward[random.rid][i] > 0 && winerNum[random.rid][i] == 0) {
                    roomReward[random.rid][i + 1] = roomReward[random.rid][i];
                    roomReward[random.rid][i] = 0;
                }
            }
        }
