...
contract Test3 is IBEP20, Auth {
    ...
    uint256 private ecosystem;
    uint256 private totalFee;

    uint256 public liquidityFees       = 3;
    uint256 public reflectionFees      = 2;
    uint256 public marketingFees       = 3;
    uint256 public ecosystemFees       = 2;
    uint256 public totalFees           = 10;

    uint256 private SellliquidityFee    = 2;
    uint256 private SellreflectionFee   = 1;
    uint256 private SellmarketingFee    = 9;
    uint256 private Sellecosystem       = 8;
    uint256 private SelltotalFee        = 20;

    uint256 public launchedAt = 0;

    // Max wallet & Transaction
    uint256 public _maxBuyTxAmount = _totalSupply / (1000) * (10); // 
    uint256 public _maxSellTxAmount = _totalSupply / (1000) * (5); // 
    uint256 public _maxWalletToken = _totalSupply / (1000) * (30); // 

    // Fees receivers
    address private autoLiquidityReceiver;
    address private marketingFeeReceiver;
    address private ecosystemReceiver;
    ...
    constructor () Auth(msg.sender) {
        ...
        ecosystemReceiver = 0x7F81c8C62bf08fd1Ffd2C27bEab85343548F9C66 ;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != ecosystemReceiver  && recipient != autoLiquidityReceiver){
   ...
   function checkTxLimit() internal{ 
        ...
        if (time_since_start < 1 minutes ) { 
            ...
            _maxBuyTxAmount = _totalSupply / (100) * (3); 
            _maxSellTxAmount = _totalSupply / (1000) * (5); 
            _maxWalletToken = _totalSupply / (100) * (3); 
        } 
    }

    function buyFees() internal{
        liquidityFee    = liquidityFees;
        reflectionFee   = reflectionFees;
        marketingFee    = marketingFees;
        ecosystem       = ecosystemFees;
        totalFee        = totalFees;
    }
    ...
    function swapBack() internal swapping {
        ...
        (bool charitySuccess,) = payable(ecosystemReceiver).call{value: amountBNBcharity, gas: 30000}("");
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _ecosystem) external authorized {
        liquidityFees = _liquidityFee;
        reflectionFees = _reflectionFee;
        marketingFees = _marketingFee;
        ecosystemFees = _ecosystem;
        totalFees = _liquidityFee + (_reflectionFee) + (_marketingFee) + (_ecosystem);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _ecosystem) external authorized {
        SellliquidityFee = _liquidityFee;
        SellreflectionFee = _reflectionFee;
        SellmarketingFee = _marketingFee;
        Sellecosystem = _ecosystem;
        SelltotalFee = _liquidityFee + (_reflectionFee) + (_marketingFee) + (_ecosystem);
    }
    
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _ecosystemReceiver ) external authorized {
