...
contract MysteryCapsule is AccessControlUpgradeable {
    ...
    function setMaxRate(uint8 _maxRate) public restricted {
        maxRate = _maxRate;
    }

    function setRate(uint256 rate, uint256 value) public restricted {
        require(rate > 0, "NSR0");
    ...
    function _getRarity(uint256 seed) private view returns (uint8) {
        ...
        uint256 current = 0;
        for (uint8 rate = maxRate; rate >= 0; rate--) {
    ...
    function _createSpaceship(address player) private returns (uint256) {
        ...
        uint8 rate = _getRarity(seed);
        if (rate == 0) {
            uint256 shipFee = usdToSip(mintSpaceshipFee);
            token.transfer(player, shipFee.div(100).mul(RandomUtil.randomSeededMinMax(30, 80, seed)));
            return 2**256 - 1;
        }
