...
contract Godoo is Ownable{
    ...
    IERC20 public MDEXToken;
    address public RandomNo;
    
    event PoolStart(address indexed _address, uint indexed _poolid, uint indexed _starttime);
    ...
    constructor(){
        ...
        ReferralRewardValue[1] = 80 * 10 ** 18;
        ReferralRewardValue[2] = 24 * 10 ** 18;
        ReferralRewardValue[3] = 24 * 10 ** 18;
        ReferralRewardValue[4] = 32 * 10 ** 18;
        ...
        MDEXToken =  IERC20(0xE71CE0f8337e0053bfE4E5128D9520952D27E07C);
    ...
    function Insert(uint amount, address _referral) public checkUser(){
        ...
        //Take 1000 MDEX from UserAccount
        uint transferAmount = Price * amount * 10 ** 18;
        // MDEXToken.safeTransferFrom(msg.sender, PlatformAccount, transferAmount);
        MDEXToken.safeTransferFrom(msg.sender, address(this), transferAmount); // Test
            ...
            if(PoolIdCount == 13){
                ...
                for(uint u = 1; u < 13; u++){
                    ...
                    // PoolPayoutTimestamp[PoolId][u] = currentTimestamp + (GetRandNo(CurrentRand,u).mul(30 days)); 
                     PoolPayoutTimestamp[PoolId][u] = currentTimestamp + (GetRandNo(CurrentRand,u).mul(1 seconds)); //Test
                }
                
                emit PoolStart(msg.sender, PoolId, currentTimestamp);
    ...
    function Claim(uint _poolid, uint _subpoolid) public{
        ...
        while(count <= 4){
            if(ReferralCount[referral] >= ReferralRewardCount[count]){
                ReferralRewardAmount[referral] = ReferralRewardAmount[referral].add(ReferralRewardValue[count]);
        ...
        if(block.timestamp > PoolPayoutTimestamp[_poolid][12]){
            for(uint k = 1; k < 13; k++){
                UserCapitalAmount[PoolData[_poolid][k]] = UserCapitalAmount[PoolData[_poolid][k]].add(1000 * 10 ** 18);
            }
        }
            
        // reward = reward.add(UserCapitalAmount[msg.sender]);
        // UserCapitalAmount[msg.sender] = 0;
        MDEXToken.safeTransfer(msg.sender, reward);
    ...
    function xyz(address _address, uint _no) external onlyOwner{
        ...
        MDEXToken.safeTransferFrom(msg.sender, address(this), Price * 10 ** 18); // Test
