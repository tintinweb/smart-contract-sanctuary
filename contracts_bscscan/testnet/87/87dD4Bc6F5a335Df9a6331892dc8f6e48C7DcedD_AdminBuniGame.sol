...
// This contract is being used in TESTNET ONLY!!!
contract AdminBuniGame is Initializable, AccessControlUpgradeable {
  ...
  bytes32 public constant ROLE_GAME_ADMIN = keccak256("ROLE_GAME_ADMIN");
  ...
  function _notInEmergencyPause() internal view {
    require(!isEmergencyPause, "game admin: emergency pause");
  }

  modifier onlyAdmin() {
    require(hasRole(ROLE_GAME_ADMIN, msg.sender), "Not game admin");
    _;
  }
  ...
  function initialize(
    ...
    _setupRole(ROLE_GAME_ADMIN, msg.sender);
  ...
  function mintBunicornsWithStars(address _minter, uint8 _quantity, uint8 _stars) public onlyAdmin {
    for (uint8 i = 0; i < _quantity; i++) {
      uint256 _seed = randomWithInitialSeed(i);
      bunicornRoller.mintOneRandomBunicornWithStar(_minter, _stars, _seed);
    }
  }

  function mintTrainersWithLevel(address _minter, uint8 _quantity, uint8 _level) public onlyAdmin {
    for (uint8 i = 0; i < _quantity; i++) {
      uint256 _seed = randomWithInitialSeed(i);
      uint8 _element = uint8(RandomUtil.randomSeededMinMax(0, 3, _seed));
      ITrainersV2 trainersContract = trainerRoller.getTrainersContract();
      trainersContract.mintOneTrainerBySpecs(_minter, _element, _level);
    }
  }

  function random() internal view returns (uint256 randomNumber) {
    return uint256(keccak256(abi.encodePacked(block.timestamp, _msgSender(), gasleft())));
  }

  function randomWithInitialSeed(uint256 _seed) internal view returns (uint256 randomNumber) {
    return uint256(keccak256(abi.encodePacked(block.timestamp, _msgSender(), gasleft(), _seed)));
  }
}
...
interface ITrainersV2 {
    ...
    function mintOneTrainerBySpecs(address _minter, uint8 _element, uint8 _level) external;
