...
// test contract by dapprex.com - Dapp Build Company
...
contract DAPPREX is ERC721 {
    address payable public _contractOwner;
    uint public _dappCommision = 2;
    ...
    function mint(string memory _tokenURI, address _toAddress, uint _price, uint _royalty, uint _reward) public payable returns (uint) {
        require(msg.value >= 1, "Error, the storage fee is lower");
        require((_royalty+_reward) >= 6, "Error, the commissions is too high");
    ...
    function _trade(uint _id) internal {
        ...
        address payable _reward = payable(reward[_id]);
