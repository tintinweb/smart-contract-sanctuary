...
contract BurningMoonDividentNFT is ERC165, IERC721, IERC721Metadata ,IERC721Enumerable,Ownable {
    ...
    // Token name
    string private _name="Luckypig x BurningMoon";
    // Token symbol
    string private _symbol="LuckypigXBM";
    ...
    uint256 public currentValue=5*10**12; //0.000005 BNB for first NFT
    ...
    address constant TokenAddress=0xF16e1Ad313E3Bf4E3381b58731b45fA116ECF53f;//Luckypig
    //TestNet
    ...
    function OwnerClaimBNB() external isInFunction{
        ...
        uint ReferalBNB=amount*ReferalShare/1000;
        uint ArtistBNB=amount*ArtistShare/1000;
        amount=amount-ReferalBNB-ArtistBNB;
        (bool sent,)=owner().call{value:amount}("");
        if(Referal!=address(0))(sent,)=owner().call{value:ReferalBNB}("");
        if(Artist!=address(0))(sent,)=owner().call{value:ArtistBNB}("");
        sent=true;
    }
    ...
    uint256 public MaxNFTCount=100000;
    ...
    function _recalculateShares(address account) private{
        ...
        for(uint i=0;i<counts.length;i++){
            uint count=counts[i];
            if(count>0){
                uint NewShares=getSharesOfNFT(i)*counts[i];
