...
contract Marketplace is Ownable, Pausable, MarketplaceStorage {
    ...
    internal
  {
    ...
    require(nftRegistry.ownerOf(assetId) == sender, "Only the owner can create orders");
    require(
      nftRegistry.getApproved(assetId) == address(this) || nftRegistry.isApprovedForAll(sender, address(this)),
   ...
   internal returns (Order memory)
  {
    ...
    ERC1155Verifiable nftRegistry = ERC1155Verifiable(nftAddress);
    ...
    require(nftRegistry.balanceOf(seller, assetId) > 0, "The seller is no longer the owner");
...
interface ERC1155Verifiable is IERC1155 {
...
contract MarketplaceStorage {
  ...
  // From ERC1155 registry assetId to Order (to avoid asset collision)
