...
contract Lottery is Ownable, Initializable {
    ...
    mapping(address => uint256[]) private lotteryOfAddress;
    ...
    function buyTickets(
        ...
        for (uint256 i = 0; i < _numberOfTickets; i++) {
            ...
            lotteryOfAddress[msg.sender].push(_ticketIdCounter);
            _ticketIdCounter++;
        }
    ...
    function drawWinners(uint256 _lotteryId) external onlyOwner {
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            AllIndex[i]++;
            uint256 price = allLotteries[_lotteryId].prizeDistribution[
                AllIndex[i]
            ];
    ...
    function FectchTicketOfAddress(address user)
        external
        view
        returns (uint256[] memory)
    {
        return lotteryOfAddress[user];
    }
}