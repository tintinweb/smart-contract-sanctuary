...
contract TrustMultiV1 is Initializable,ERC1155Upgradeable, OwnableUpgradeable, ERC1155BurnableUpgradeable {
    ...
    function initialize(uint256 id, uint256 _serviceValue,string memory _name, string memory _symbol) public initializer  {
    ...
    function mint(
        ...
        _royal[id]["artist"].uaddress = royaddress[0];
        _royal[id]["agent"].uaddress = royaddress[1];
        _royal[id]["artist"].pecentr = royPer[0].mul(1e18);
        _royal[id]["agent"].pecentr = royPer[1].mul(1e18);
        if (value != 0) {
            orderPlace(id, value);
        }
    }
    ...
    function _acceptBId(
        ...
        require(
            approveValue >= _adminfee.add(artist.add(netamount).add(artist)),
            "Insufficient Balance"
        );
