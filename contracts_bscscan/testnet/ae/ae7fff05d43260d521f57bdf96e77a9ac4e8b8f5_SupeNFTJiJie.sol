...
contract SupeNFTJiJie is Ownable {
    ...
    function _get_reward(uint256 _round_id, uint256 _asset_id) internal{
        ...
        if(attend_count == 1){
            ...
            emit CreateReward(_round_id, 0, account, asset_id, total_tlp, total_frog);
            ...
            uint256 third_index;
            seed = uint256(keccak256(abi.encode(msg.sender, pools[_round_id].attend_assets[first_index], _round_id + 1, _asset_id, block.timestamp, blockhash(block.number))));
            if(first_index == 0){
                second_index = first_index + 1 + seed % (attend_count - 1);
                if(second_index != attend_count -1){
                    seed = uint256(keccak256(abi.encode(msg.sender, _asset_id, pools[_round_id].attend_assets[first_index], pools[_round_id].attend_assets[second_index], _round_id + 2, _asset_id, block.timestamp, blockhash(block.number))));
