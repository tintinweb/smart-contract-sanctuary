...
contract FRKNLKN is Context, IBEP20, Ownable {
    ...
    uint256 public _numOfTokensToExchangeForBUSD = _tTotal  * 5 / (10**5);  // 0.005%
    //buy
    ...
    address public _resAndDevWalletAddress;
...
constructor (string memory Name, string memory Symbol, uint256 taxFee, uint256 marketFee, uint256 gameFee, uint256 resAndDevFee, uint256 nftFee,
                address marketingWalletAddress, address gameWalletAddress, address resAndDevWalletAddress, address nftWalletAddress ) {
        _name = Name;
        _symbol = Symbol;
    ...
    function setResAndDevWallet(address resAndDevWallet) external onlyOwner() {
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 busdBalance = address(this).balance;
        
        uint256 busdForGame = busdBalance.mul(_gameFee).div(_sellTotalFee);
        uint256 busdForResAndDev = busdBalance.mul(_resAndDevFee).div(_sellTotalFee);
        uint256 busdForNft = busdBalance.mul(_nftFee).div(_sellTotalFee);

        payable(_gameWalletAddress).transfer(busdForGame);
        payable(_resAndDevWalletAddress).transfer(busdForResAndDev);
        payable(_nftWalletAddress).transfer(busdForNft);
    }
    ...
    function _takeBuyFee(address sender, uint256 amount) private returns (uint256) {
        if(_marketFee != 0){
            ...
            _taxFee = _previousTaxFee;
            amount = amount.sub(marketAmt);
        }
        return amount;
    }
    function _takeSellFee(uint256 amount) private view returns (uint256) {
        if (_sellTotalFee != 0){
            amount = amount.sub(amount.div(_sellTotalFee));
        }
        return amount;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        bool overMinTokenBalance = contractTokenBalance >= _numOfTokensToExchangeForBUSD;
        ...
        if (!takeFee) removeAllFee();
            ...
            if (
                ...
                amount = _takeSellFee(amount);
            }
            else {
                if(sender == uniswapV2Pair) checkMaxTXAmount(sender, recipient, amount, true);
