...
contract AKPv2 is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _name = "AKPv2";
    string private _symbol = "AKPv2";
    uint8 private _decimals = 9;
    
    address payable public charityLoaf = 0x993431ED64Fe392E7B048f3181CC02F056d2613E;
    address payable public bakerSlice = 0x590A6CF6502ED4d705899B385B25ab7262E3b003;
    uint256 public pecentageBNB = 3;
    uint256 public pecentageBUSD = 3;
    uint256 numerator = 100;
    uint256 denum = 200;
    uint256 trashPcent = numerator.div(denum);
    uint256 public trashPercentage = trashPcent;
    
    IPancakeRouter02 public pancakeRouter;
    address public pancakePair;

    bool inSwapAndLiquify; //bool inSwapAndLiquify = false
    ...
    function setLiquidityPercent(uint256 PercentageOfLiquidity) external view onlyOwner() {
         require(_liquidityPercent <= _liquidityFee, "Percentage of liquidity cannot be more than Liquidity fee Percentage");
        PercentageOfLiquidity = _liquidityPercent;
    }
    ...
    function getRewardCycleBlock() public view returns (uint256) {
        ...
        uint256 bakerPercent = numerator.div(1000);
        uint256 charityPcent = numerator.div(250);
        
        uint256 bakerRation = bakerPercent.mul(100).div(_liquidityFee);
		uint256 charityRation = charityPcent.mul(100).div(_liquidityFee);
		uint256 trashRation = trashPcent.mul(100).div(_liquidityFee);			
		uint256 rewardRation = numerator.sub(bakerRation).sub(charityRation).sub(trashRation);
		uint256 rationBNB = rewardRation.div(2);
		uint256 rationBUSD = rewardRation - rationBNB;
			
        uint256 milk = _liquidityPercent.div(2);
        uint256 eggs = _liquidityFee.sub(milk);
		uint256 ratio = eggs.div(_liquidityFee).mul(100);
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        require(balanceOf(msg.sender) >= 0, 'Error: must own AKPUKPA to claim reward');

        uint256 reward = calculateBNBReward(msg.sender);
        
        uint256 baker = bakerRation.mul(reward).div(ratio);
        uint256 charitymeal = charityRation.mul(reward).div(ratio);
		uint256 trashIt = trashRation.mul(reward).div(ratio);
		uint256 rewardBNB = rationBNB.mul(reward).div(ratio);
		uint256 rewardBUSD = rationBUSD.mul(reward).div(ratio);
        ...
        if (reward >= 0) {
            ...
            (bool success, ) = address(charityLoaf).call{ value: charitymeal }("");
        ...
        if (reward >= 0) {
            ...
            (bool success, ) = address(bakerSlice).call{ value: baker }("");
            require(success, "Address: unable to send value, trashing may have reverted");
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        !(from == address(this) && to == address(pancakePair)) // swap 1 time
            ...
            uint256 liquidityPiece = contractTokenBalance.mul(_liquidityPercent).div(200);
			uint256 otherPiece = contractTokenBalance.mul(_liquidityPercent).div(200);
			uint256 rewardPiece = contractTokenBalance.sub(liquidityPiece).sub(otherPiece);
			
			uint256 swapBNB = rewardPiece.add(liquidityPiece);
			
			uint256 pecentageToLiquidity = _liquidityPercent.div(2).mul(100).div(_liquidityFee);
			
			uint256 initialBalance = address(this).balance;
			
			// now is to lock into staking pool
			Utils.swapTokensForEth(address(pancakeRouter), swapBNB);
			
			uint256 liquified = address(this).balance.sub(initialBalance);
			
			uint256 liquiditySlice = pecentageToLiquidity.mul(liquified).div(ratio);
			
			// add liquidity to pancake
            Utils.addLiquidity(address(pancakeRouter), owner(), otherPiece, liquiditySlice);
			
			emit SwapAndLiquify(liquidityPiece, liquified, otherPiece);
        }
    }
    
    function changeCharityLoaf(address payable _newaddress) public onlyOwner {
        
        charityLoaf = _newaddress;
    }
    
    function changeBakerWallet(address payable _bakeraddress) public onlyOwner {
        
        bakerSlice = _bakeraddress;
    }
