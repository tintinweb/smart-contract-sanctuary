...
using SafeMath for uint256;
  ...
  event CreateCategory(string title,string description,string image, uint256 price, uint256 usdPrice, uint256 countNFT);
  ...
  event BuyBox(address buyer, string ownerId, string name, string tokenURI );
  constructor() public{
  ...
  function createCategory(string memory name, string memory title, string memory description, string memory image, uint256 price, uint256 usdPrice, uint256 countNFT) public adminAdd {
    ...
    emit CreateCategory(Category[name].title, Category[name].description, Category[name].image, Category[name].price, Category[name].usdPrice, Category[name].countNFT);
  }
  ...
  function buyBox(string memory ownerId, string memory name) public {
    ...
    NFTInterface(xanaliaDex).mintAliaForNonCrypto(price,msg.sender);
    ...
    NFTInterface(xanaliaDex).blindBox(msg.sender, Category[name].tokenUris[randomValue], 1, true, ownerId);
    emit BuyBox(msg.sender, ownerId, name,Category[name].tokenUris[randomValue]);
    }
