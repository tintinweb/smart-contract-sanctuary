...
contract Ownable is Context {
    ...
    constructor () {
        _owner = payable(0x64cB30A482e15F27e2db28c64DBB12D247F1f33c);
...
contract ElecticEra is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 10000000 * 1e18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Electic Era";
    string private _symbol = "EEFI";
    ...
    bool public reflectionFeesdiabled = false;  // fee is enabled by default
    ...
    uint256 public _holderFee = 30; // 3% will be distributed to holders 

    uint256 public _totalFeePerTx = 50; // 5% by default
    ...
    function setFeePercent(uint256 liquidityFee, uint256 holderFee) external onlyOwner {
        _liquidityFee = liquidityFee;
        _holderFee = holderFee;
        _totalFeePerTx = liquidityFee.add(holderFee);
    }

    // numerator must be a multiple of 100
    function setSellFeeMultiplierNumerator(uint256 _numerator, uint256 _duration) external onlyOwner {
    ...
    function startTrading() external onlyOwner {
        require(!_tradingOpen,"Can not run twice");
        _tradingOpen = true;
    ...
    function _takeLiquidityFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 bothFee = getTotalFeePerTx(tAmount);
        uint256 tFee = bothFee.mul(_liquidityFee.div(_totalFeePerTx).mul(100)).div(100);
    ...
    function _reflectFee(uint256 tAmount, uint256 currentRate) private {
        uint256 bothFee = getTotalFeePerTx(tAmount);
        uint256 tFee = bothFee.mul(_holderFee.div(_totalFeePerTx).mul(100)).div(100);
        uint256 rFee = tFee.mul(currentRate);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        if(takeFee){
            require(balanceOf(sender) >= amount.add(getTotalFeePerTx(amount)),"Insufficient funds for transfer");
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        uint256 currentRate = _getRate();
        uint256 rReceivingAmount = tAmount.mul(currentRate);
        uint256 rSendingAmount = tAmount.add(getTotalFeePerTx(tAmount)).mul(currentRate);
        _rOwned[sender] = _rOwned[sender].sub(rSendingAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rReceivingAmount);
        if(_totalFeePerTx != 0){
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
        uint256 currentRate = _getRate();
        uint256 tSendingAmount = tAmount.add(getTotalFeePerTx(tAmount));
        uint256 rReceivingAmount = tAmount.mul(currentRate);
        uint256 rSendingAmount = tAmount.add(getTotalFeePerTx(tAmount)).mul(currentRate);
        _tOwned[sender] = _tOwned[sender].sub(tSendingAmount);
        _rOwned[sender] = _rOwned[sender].sub(rSendingAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rReceivingAmount);
        if(_totalFeePerTx != 0){
            _takeLiquidityFee(tAmount, currentRate);
        ...
        emit Transfer(sender, recipient, tSendingAmount);
    }
    ...
    function swapAndLiquify() private {
        ...
        if (contractTokenBalance >= _maxTxAmount) {
            ...
            if (buyBackEnabled && initialBalance > uint256(buyBackLowerLimit)) {
                ...
                Utils.swapETHForTokens(address(pancakeRouter), deadAddress, initialBalance);
            }
