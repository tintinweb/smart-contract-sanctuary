...
contract Bucks is Context, IBEP20, Ownable {
    ...
    uint256 public _holderFee = 100;
    uint256 private _previousTaxFee = _holderFee;
    
    uint256 public _liquidityFee = 200;
    uint256 private _previousLiquidityFee = _liquidityFee;
	
	uint256 public _devFee = 95; 
	uint256 private _previousdevFee = _devFee;
	
	uint256 public _charityFee = 50; 
	uint256 private _previouscharityFee = _charityFee;
	
	uint256 public _rewardFee = 200; 
	uint256 private _previousrewardFee = _rewardFee;	
	
	address payable public devWallet=0x47d0DdaDe3586F5bb8818715bE5CAA6b516005f7;
    ...
    uint256 public _maxTxAmount = 100000000*1e4;
    uint256 private _numTokensSellToAddToLiquidity = 100000*1e4;
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_holderFee.add(_liquidityFee).add(_devFee).add(_charityFee).add(_rewardFee)).div(10000);
        return percentage;
    }
    ...
    function _reflectFee(uint256 tAmount) private {
        uint256 tFee = tAmount.mul(_holderFee).div(10000);
    ...
    function _takeDevFee(uint256 tAmount) private {
        uint256 tDevFee = tAmount.mul(_devFee).div(10000);
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 reward=giveTransferReward(amount);
            //add liquidity
            swapAndLiquify(reward);
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // newBalance=newBalance.sub(newBalance.mul(_charityFee).div(100));
