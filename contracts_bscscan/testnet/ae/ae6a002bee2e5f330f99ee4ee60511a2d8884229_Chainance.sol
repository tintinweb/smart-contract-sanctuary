...
contract Chainance is Context, Ownable{
    ...
    modifier CreateAccountControl(string memory username, string memory email) {
        for ( uint i = 0; i < AccountIndex.length; i++ ) {
            //- Address Control
            if ( AccountStruct[AccountIndex[i]].Account_Address == msg.sender ) revert("err_code : 3");        
            //- UserName Control
            if ( keccak256(abi.encodePacked(AccountStruct[AccountIndex[i]].UserName)) == keccak256(abi.encodePacked(username)) ) revert("err_code : 4");
            //- EMail Control
            if ( keccak256(abi.encodePacked(AccountStruct[AccountIndex[i]].Email)) == keccak256(abi.encodePacked(email)) ) revert("err_code : 5");
        } _;
    }
    
    //- Create Account Control Algorithm 
    function CreateAccountController(string memory username, string memory email) public view returns(uint) {
        uint result = 0;
        for ( uint i = 0; i < AccountIndex.length; i++ ) {
            //- Address Control
            if ( AccountStruct[AccountIndex[i]].Account_Address == msg.sender ) result=result+1; 
            //- UserName Control
            if ( keccak256(abi.encodePacked(AccountStruct[AccountIndex[i]].UserName)) == keccak256(abi.encodePacked(username)) ) result=result+1;
            //- EMail Control
            if ( keccak256(abi.encodePacked(AccountStruct[AccountIndex[i]].Email)) == keccak256(abi.encodePacked(email)) ) result=result+1;
        }
        return result;
    }
    
    //- Login Chainance
    function Login() public AccountControl view returns(string memory) {
        if ( AccountStruct[msg.sender].Account_Address == msg.sender ) return(AccountStruct[msg.sender].UserName);
    ...
    function CreateAccount(string memory _UserName, string memory _NameSurname, string memory _Birthday, string memory _Email, string memory _Description, string memory _WebSite) public 
        CreateAccountControl(_UserName, _Email) {
    ...
    //- Get Account
    function getAccount() public view returns(uint256, uint256, string memory, string memory, string memory, string memory, string memory, string memory, address) {
        require(AccountStruct[msg.sender].Account_Address == msg.sender, "err_code : 6"); //- No account found at Wallet address.
    ...
    //- Get Profile Image's
    function getAccountImage() public view returns(uint256, uint256, string memory, string memory, address) {
        require(ImageStruct[msg.sender].Image_Address == msg.sender, "err_code : 6");
    ...
    //- Get Data
    function getData() public view returns(uint256, uint256, string memory, string memory, string memory, address) {
        require(DataStruct[msg.sender].Data_Address == msg.sender, "err_code : 6");
