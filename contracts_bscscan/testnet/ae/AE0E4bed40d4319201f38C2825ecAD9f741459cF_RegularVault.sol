...
contract RegularVault is VaultBase, IBotVaultEvent {
    ...
    modifier nonRestricted(uint vID) {
        require(!vaultData().option[vID].restricted, 'RegularVault: vault is restricted');
        _;
    }
    ...
    function pendingReward(uint vID, address account) public view _validVault(vID) returns(uint) {
        return _pendingReward(_vaultOf(vID), account);
    }
    ...
    function updateReward(uint vID, uint assetAmount) external _validVault(vID) botOnly {
        Vault storage vault = _vaultOf(vID);
        // IERC20(vault.data.asset).safeTransferFrom(_msgSender(), address(this), assetAmount);
        if (vault.totalDeposit > 0)
            vault.accRewardPerShare += assetAmount * 1e12 / vault.totalDeposit;
        else
    ...
    function claimReward(uint vID, address account) external _validVault(vID) {
        ...
        _claimReward(vault, account);
        user.debt = user.deposit * vault.accRewardPerShare / 1e12;
    }

    function _pendingReward(Vault storage vault, address account) internal view returns(uint) {
        UserInfo storage user = vault.users[account];
        uint reward = vault.accRewardPerShare * user.deposit / 1e12;
        if (reward < user.debt)
            return 0;
        return reward - user.debt; 
    }

    function _claimReward(Vault storage vault, address account) internal {
    ...
    function _withdraw(uint vID, address account, uint amount) internal {
        ...
        require(block.timestamp >= user.lastDepositTime + user.lockPeriod, "BotVault: deposit locked");

        _claimReward(vault, account);

        if (amount > 0) {
            require(amount <= user.deposit, 'BotVault: amount exceeded deposit');
            user.deposit -= amount;
            vault.totalDeposit -= amount;
        }
        user.debt = user.deposit * vault.accRewardPerShare / 1e12;
