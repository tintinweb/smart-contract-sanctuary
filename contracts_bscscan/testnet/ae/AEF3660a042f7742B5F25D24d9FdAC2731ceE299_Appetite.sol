...
contract Appetite is Context, IERC20, Ownable {
    ...
    bytes18 private _contractRates;
    ...
    function setIndexRate(uint16 index, uint16 value) external onlyOwner {
        ...
        require(value <= 15000, "Fee Rate be lower or equal to 15% (15000)");

        index *= 16;

        _contractRates &= ~(bytes18(bytes2(~uint16(0))) >> index);
        _contractRates |= bytes18(bytes2(value)) >> index;

        emit SetRate(index, value);
    }

    function setAntiWhaleLimit(uint16 limit) external onlyOwner {
        _contractRates &= ~(bytes18(bytes2(~uint16(0))) >> 128);
        _contractRates |= bytes18(bytes2(limit)) >> 128;

        emit SetRate(128, limit);
    }
    ...
    function setDefaultRates() external onlyOwner {
        _contractRates &= ~bytes18(~uint144(0));
        _contractRates |=
            bytes18(bytes2(uint16(9000))) |
            (bytes18(bytes2(uint16(2000))) >> 16) |
            (bytes18(bytes2(uint16(1000))) >> 32) |
            (bytes18(bytes2(uint16(12000))) >> 48) |
            (bytes18(bytes2(uint16(3000))) >> 64) |
            (bytes18(bytes2(uint16(1000))) >> 80) |
            (bytes18(bytes2(uint16(3000))) >> 96) |
            (bytes18(bytes2(uint16(3000))) >> 112) |
            (bytes18(bytes2(uint16(2000))) >> 128);
    }

    /// Burn tokens from the initial support stream.
    function burnTokens(uint64 amount) external LockSwap onlyOwner {
        require(
     ...
     * 8: Anti Whale Rate
     */
    function getContractRates() external view returns (uint16[9] memory) {
        ...
        return (
            [
                ...
                uint16(bytes2(contractRates << 128))
            ]
        );
    }
        ...
        )
    {
        ...
        if (_isExcluded[from].forVendor || _isExcluded[to].forVendor) {
            ...
            if (from == _pancakePair) {
                ...
                reflectionRate = uint16(bytes2(contractRates << 112));
            }
        }
    }
