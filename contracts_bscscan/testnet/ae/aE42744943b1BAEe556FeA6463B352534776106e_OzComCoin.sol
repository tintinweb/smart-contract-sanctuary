...
//   / __ \____  / ____/___  ____ ___  / ____/___  (_)___ 
//  / / / /_  / / /   / __ \/ __ `__ \/ /   / __ \/ / __ \
// / /_/ / / /_/ /___/ /_/ / / / / / / /___/ /_/ / / / / /
// \____/ /___/\____/\____/_/ /_/ /_/\____/\____/_/_/ /_/ 
//           @Haxbind - Intervention Finance
...
contract OzComCoin is Context, IERC20, Ownable {
    ...
    address private _marketingWalletAddress = 0x0DF5D55F783CdD336702B8f0cD36F9aE39aC808e;
    ...
    string private _name = "OzComCoin";
    ...
    uint256 private _maxTxAmount = _tTotal.div(10**2);
    uint256 private numTokensSellToAddToLiquidity = 7000 * (10 ** 9);
    ...
    function totalRedistribution() public view returns (uint256) {
        return _tFeeTotal.div(10 ** 9);
    }

    function _allFees() public view returns(uint256 Marketing, uint256 Redistributions, uint256 Liquidity){
        return(_marketingFee, _taxFee, _liquidityFee);
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing) {
        ...
        rFee = tFee.mul(_getRate());
        rTransferAmount = rAmount.sub(rFee).sub(_getRValues(tLiquidity, tMarketing, _getRate()));
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing) {
        ...
        tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tMarketing.mul(2));
        return (tTransferAmount, tFee, tLiquidity, tMarketing);
    }

    function _getRValues(uint256 tLiquidity, uint256 tMarketing, uint256 currentRate) private pure returns (uint256 rTransferAmount) {
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rMarketing = tMarketing.mul(currentRate);

        return rLiquidity.add(rMarketing);
    }
