...
pragma solidity ^0.6.12;
    ...
    contract MUNCH is Context, IERC20, Ownable {
        ...
        uint256 private _devandbuybackFee = 11;
        ...
        function swapTokensForEth(uint256 tokenAmount) private lockTheSwap{
         ...
         function sendETHToBuyback(uint256 amount) private {
            _buybackWalletAddress.transfer(amount.sub(amount).add(100).div(_devandbuybackFee).mul(_buybackFee).mul(amount).div(100));
            devAddress.transfer(amount.sub(amount).add(100).div(_devandbuybackFee).mul(_devFee).mul(amount).div(100));
        }
