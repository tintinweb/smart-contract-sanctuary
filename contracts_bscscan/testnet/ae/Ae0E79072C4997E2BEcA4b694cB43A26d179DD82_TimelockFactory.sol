...
contract TimelockFactory is Ownable {
    ...
    Timelock[] public _timelocks;

    event TimelockCreated(address childAddress);

    constructor(ESHK token) {
        _token = token;

    }

    function createTimelock(uint256 releaseTime, uint256 releaseAmount) public onlyOwner returns (address) {
        Timelock timelock = new Timelock(_token, releaseTime);
        _timelocks.push(timelock);
        _token.transfer(address(timelock), releaseAmount);
    //    emit TimelockCreated(address(timelock));
       return address(timelock);
    }

    function createTimelocks(uint256[] memory releaseTimes, uint256[] memory releaseAmounts) public onlyOwner {
        for(uint32 i = 0; i < releaseTimes.length; i++){
            Timelock timelock = new Timelock(_token, releaseTimes[i]);
            _timelocks.push(timelock);
            _token.transfer(address(timelock), releaseAmounts[i]);
            emit TimelockCreated(address(timelock));
        }
    }
    ...
    function getListTimelock() public view returns(Timelock[] memory) {
        return _timelocks;
    }
