...
contract KabyHero is IKabyHero, ERC721, AcceptedToken, ReentrancyGuard {
    ...
    constructor(
        ...
        _uri = baseURI;
        versions.push(Version(0, 0, 0, maxSupply, maxForSell, maxSupply - maxForSell, salePrice, startTime, revealTime, provenance));
     ...
     * @param maxForSell new maxForSell value
    ...
    function addNewVersion(
        ...
        require(latestVersion.currentSell + latestVersion.currentPresent == latestVersion.maxSupply);
    ...
    function claimHero(uint versionId, uint amount) external override {
        ...
        if (version.startingIndex == 0 && (version.currentSell + version.currentPresent == version.maxSupply || block.timestamp >= version.revealTime)) {
            _finalizeStartingIndex(versionId, version);
        }
    }
    
    /**
     * @dev Owner request to present hero to specific account
     * @param versionId id of version 
     * @param amount amount of hero want to present
     * @param account address of user will receive 
     */
    function presentHero(uint versionId, uint amount, address account) external onlyOwner override {
        Version storage version = versions[versionId];

        require(amount > 0 && amount <= 50, "KabyHero: amount out of range");
        require(block.timestamp >= version.startTime, "KabyHero: Sale has not started");
        require(version.currentPresent + amount <= version.maxForPresent, "KabyHero: sold out, cannot present");
     ...
     * @return gem return detail info of hero. Current is a array has info about gems 
     */
...
interface IKabyHero {
    ...
    /**
     * @notice Present Heros from Owner to user.
     */
    function presentHero(uint versionId, uint amount, address account) external;
