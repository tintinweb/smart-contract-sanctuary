...
contract NftCarsPacks is Ownable {
  ...
  INFTCARS public NFTCARS = INFTCARS(NFT_CARS);
  ...
  */
  function purchasePack(uint256 numberOfTokens) public payable returns (uint256[] memory) {
    ...
    for (uint256 i = 0; i < numberOfTokens; i++) {
      uint256 tokenIdToSend = NFTCARS.tokenOfOwnerByIndex(tokensHolder, i);
      tokenIdsToSend[i] = tokenIdToSend;
    }

    for (uint256 i = 0; i < tokenIdsToSend.length; i++) {
      NFTCARS.safeTransferFrom(tokensHolder, _msgSender(), tokenIdsToSend[i]);
    }
