...
contract Calcifire is Context, IERC20, Ownable {
   ...
   function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
    ...
    function _transfer(
        ...
        (,,, uint256 liquidityAmount) = _getFeeAmounts(amount);

        uint256 contractTokenBalance = balanceOf(address(this));

        bool overMinTokenBalance = contractTokenBalance >= liquidityAmount;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
    ...
    function swapAndLiquify(uint256 amount) private lockTheSwap {
        ...
        (uint256 treasuryAmount,uint256 communityAmount, uint256 burnAmount, uint256 liquidityAmount) = _getFeeAmounts(amount);
