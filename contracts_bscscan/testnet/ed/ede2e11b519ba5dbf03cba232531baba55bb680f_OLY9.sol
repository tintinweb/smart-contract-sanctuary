...
/**
 ...
 // exclude pcs router and lp pair for maxwallet
 // exempt max wallet for new wallet to receive token
...
add show maxwallet 
owner whitelist, maxwallet, txlimit, feeexempt not working
...
contract OLY9 is IBEP20, Auth {
    ...
    string constant _name = "OLY9";
    string constant _symbol = "OLY9";
...
//    }
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        if (!selling && totalBuyFee >= 500 && feeMultipliersTriggeredAt.add(feeMultipliersDuration) < block.timestamp && buyMultipliersTriggeredAt.add(buyMultipliersDuration) > block.timestamp) { return getbuyfee();} // greenwall
        if (selling && totalSellFee <= 3300 && feeMultipliersTriggeredAt.add(feeMultipliersDuration) < block.timestamp && buyMultipliersTriggeredAt.add(buyMultipliersDuration) > block.timestamp) { return getsellfee();} // greenwall
