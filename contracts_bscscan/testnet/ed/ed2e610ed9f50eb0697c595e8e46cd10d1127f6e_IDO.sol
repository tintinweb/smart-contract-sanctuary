...
contract IDO {
    ...
    function buyToken(address referer) public payable returns(bool) {
        ...
        if(block.timestamp - startTime > 120 minutes && block.timestamp - startTime <= 240 minutes){
            buyPrice = 75;
            bonusPercent = 7;
        }
        
        if(block.timestamp - startTime > 240 minutes && block.timestamp - startTime <= 360 minutes){
            buyPrice = 95;
            bonusPercent = 5;
        }
        
        if(block.timestamp - startTime > 360 minutes && block.timestamp - startTime <= 480 minutes){
            buyPrice = 295;
            bonusPercent = 0;
        }
        
        if(block.timestamp - startTime > 480 minutes){
            buyPrice = 495;
            bonusPercent = 0;
        }
