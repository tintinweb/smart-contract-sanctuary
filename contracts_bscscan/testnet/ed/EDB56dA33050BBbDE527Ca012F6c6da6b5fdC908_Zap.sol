...
contract Zap is IZap, OwnableUpgradeable {
     ...
     */
    function initialize(address ali, address usdt, address busd, address weth, address btcb) external initializer {
        __Ownable_init();
    ...
    function zapInToken( address fromToken, uint256 amount, address toToken) external override notPaused {
        ...
        if (isLpToken(toToken)) {
            ...
            if (fromToken == token0 || fromToken == token1) {
                ...
                ROUTER.addLiquidity(fromToken, other, amount.sub(sellAmount), otherAmount,0,0,msg.sender, block.timestamp, 0);
            } else {
                uint256 bnbAmount = fromToken == WBNB
                    ? _safeSwapToBNB(amount)
                    : _swapTokenForBNB(fromToken, amount, address(this));
                _swapBNBToLpToken(toToken, bnbAmount, msg.sender);
            }
        } else {
    ...
    function zapIn(address toToken) external payable override {
        _swapBNBToLpToken(toToken, msg.value, msg.sender);
    }

    function zapOut(address fromToken, uint256 amount) external override {
        IBEP20(fromToken).safeTransferFrom(msg.sender, address(this), amount);
        _approveTokenIfNeeded(fromToken);
    ...
    function _transferLeftOverToken(address token, address receiver, uint256 amount) private {
        ...
        uint256 currentBalance = IBEP20(token).balanceOf(address(this));
    ...
    function _transferLeftOverBNB(address payable receiver, uint256 amount) private {
        require(receiver != address(0), 'Zap: receiver must not be zero address');
    ...
    function _swapBNBToLpToken(address lpToken, uint256 amount, address receiver) private {
        if (!isLpToken(lpToken)) {
            ...
            if (token0 == WBNB || token1 == WBNB) {
                ...
                uint256 tokenBalanceBefore = IBEP20(token).balanceOf(address(this));

                ROUTER.addLiquidityETH{value: amount.sub(swapValue)}(token,tokenAmount,0,0,receiver,block.timestamp,0);

                uint256 tokenBalanceAfter = IBEP20(token).balanceOf(address(this));
                ...
                _swapTokenToLpTokenFresh(token0, token1, token0Amount, token1Amount, receiver);
    ...
    /* 
    * Swap a token (not includes WBNB) to LP token. 
    * This function to avoid CompilerError: Stack too deep, try removing local variables on Solidity 
    */
    function _swapTokenToLpTokenFresh(address token0, address token1, uint token0Amount, uint token1Amount, address receiver ) private {
        uint256 token0BalanceBefore = IBEP20(token0).balanceOf(address(this));
        uint256 token1BalanceBefore = IBEP20(token1).balanceOf(address(this));
		
		emit LogUint('token0BalanceBefore', token0BalanceBefore);
		emit LogUint('token1BalanceBefore', token1BalanceBefore);

        ROUTER.addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, receiver, block.timestamp, 0);

        uint256 token0BalanceAfter = IBEP20(token0).balanceOf(address(this));
        uint256 token1BalanceAfter = IBEP20(token1).balanceOf(address(this));

		emit LogUint('token0BalanceAfter', token0BalanceAfter);
		emit LogUint('token1BalanceAfter', token1BalanceAfter);
    ...
    function _swap(address fromToken, uint256 amount, address toToken, address receiver) private returns (uint256) {
        address intermediate = routePairAddresses[fromToken];
    ...
    function saveLeftOverBNB(address payable receiver, uint256 amount) public onlyOwner {
