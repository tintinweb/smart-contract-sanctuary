...
contract Cars is ERC20, Ownable {
    ...
    uint128 public minTokensBeforeSwap = 1000 * 10**6;
    ...
    constructor(
        ...
        _mint(address(0x55f856842437aAB72afbddeCF0ca882Bf2ADA952), 20800000000 * 10**6); // OWNER ADDRESS     - 208,000,000
        _mint(address(0x673eBA8066C49c34b72Fd391F193aad306F3eDf2), 68250000000 * 10**6); // ADMIN ADDRESS     -  68,250,000
        _mint(address(0x693F9b9E5322Aa510Ca3D3fA51b677D0DA02F121), 16250000000 * 10**6); // MARKETING ADDRESS -  16,250,000 
        _mint(address(0x6c9c7C8A9088a516B253B918CD5cDE15AE20Ab9f), 32500000000 * 10**6); // RESERVA ADDRESS   -  32,500,000
        
        _ownerWallet = 0x55f856842437aAB72afbddeCF0ca882Bf2ADA952; //OWNER WALLET FOR REFLECTION
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_feesDisabled && !inSwapAndLiquify) {
            ...
            super._transfer(from, _ownerWallet, ownerAmount);
    ...
    function setFees(uint8 _feeDecimals, uint32 _liquidityFee, uint32 _ownerFee)
        public
        onlyOwner
    {
    ...
    function changeOwnerWallet(address payable newAddress) public onlyOwner{
        _ownerWallet = newAddress;
    }
