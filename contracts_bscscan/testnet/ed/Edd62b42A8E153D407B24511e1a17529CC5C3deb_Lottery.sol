...
interface IChest{
    function mint(address recipient, uint chestIndex) external;
}
...
contract SellTime is Ownable, Random{
    ...
    uint public lotterySellStartTime = 1637337600;
    uint public lotterySellEndTime = 9999999999;
...
interface ILottery2 {
    ...
    event GetLottery(address buyer, uint time);
}
...
contract Lottery is Pausable, SellTime, ILottery2{
    ...
    function lotteryByUsdt() external whenNotPaused lotterySellTime{
        ...
        _lottery();
        emit GetLottery(msg.sender, now);
    }
...
//_mint
    ...
    function _mintChest(address to, uint chestIndex) private{
        chestContract.mint(to, chestIndex);
    }
