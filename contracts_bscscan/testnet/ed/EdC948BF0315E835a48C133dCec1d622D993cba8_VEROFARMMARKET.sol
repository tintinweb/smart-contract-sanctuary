...
contract VEROFARMMARKET is Ownable {
    ...
    mapping(address => VEROFARMNFT.User) internal users;

    constructor(address payable _feeWallet, address _veroNFT) {
        feeWallet = _feeWallet;
        veroNFT = VEROFARMNFT(_veroNFT);
    }
    ...
    function placeOrder(
        ...
        VEROFARMNFT.User storage user = users[_msgSender()];
        VEROFARMNFT.Seed[] memory seeds;
        if (user.owner == address(0)) {
            seeds = veroNFT.getUser(_msgSender()).seeds;
            for (uint256 index = 0; index < seeds.length; index++) {
                user.seeds.push(seeds[index]);
            }
        } 
        user.owner = _msgSender();
    ...
    function fillOrder(uint256 _tokenId, address _token) public payable {
        ...
        if (user.owner == address(0)) {
            seeds = veroNFT.getUser(_msgSender()).seeds;
            VEROFARMNFT.Seed memory seed = veroNFT.getNFT(_tokenId);
            user.seeds.push(seed);
            for (uint256 index = 0; index < seeds.length; index++) {
                user.seeds.push(seeds[index]);
            }
        } else {
        ...
        VEROFARMNFT.Seed[] memory seedFrom = users[itemSale.owner].seeds;
        for (uint256 index = 0; index < seedFrom.length; index++) {
            if (seedFrom[index].tokenId == _tokenId) {
                delete users[itemSale.owner].seeds[index];
            }
        }
        user.owner = _msgSender();
    ...
    function getAllSale(uint256 fromTokenId, uint256 toTokenId)
        public
        view
        returns (uint256[] memory)
    {
        ...
        for (uint256 index = 0; index < itemsSale.length; index++) {
            if (
                itemsSale[index] >= fromTokenId && itemsSale[index] <= toTokenId
            ) {
    ...
    function getUser(address _userAddress)
        public
        view
        returns (VEROFARMNFT.User memory userInfo)
    {
        if (users[_userAddress].owner == address(0)) {
            userInfo = veroNFT.getUser(_userAddress);
        } else {
