 ...
 * @title LibraCash
 */
contract LibraCash is ERC20, Ownable {
    ...
    uint256 private _softLimit = 2100000000000000000000000;
    uint256 private _hardLimit = 2100000000000000000000000;

    uint256 private _baseSlab = 0;
    uint256 private _basePriceWei = 24480000000000000;
    ...
    uint256 private _maxSellBatch = 1000000000000000000000; //in LBC
    uint256 private _maxBuyBatch = 5000000000000000000000; //in LBC

    address payable _commissionReceiver;
    address payable _feeReceiver;

    address payable PROJECT_DEVELOPMENT; //15%
    address payable FOUNDERS_TEAM; //10%
    address payable PUBLIC_LISTINGS; //30%
    address payable ADVISORS; //5%
    address payable STAKEHOLDERS; //10%
    ...
    constructor (
        ...
        address payable projectDevelopment,
        address payable foundersTeam,
        ...
        FOUNDERS_TEAM = foundersTeam;
        PUBLIC_LISTINGS = publicListings;
        ADVISORS = advisors;
        STAKEHOLDERS = stakeholders;

        //distribute shares
        transferTo(PROJECT_DEVELOPMENT, 315000000000000000000000 , false);
        transferTo(FOUNDERS_TEAM, 210000000000000000000000 , false);
        transferTo(PUBLIC_LISTINGS, 630000000000000000000000 , false);
        transferTo(ADVISORS, 105000000000000000000000 , false);
        transferTo(STAKEHOLDERS, 210000000000000000000000 , false);
    }
    ...
    function buyTokens() external payable{
        ...
        require(token_wei <= _maxBuyBatch, "LBC: invalid buy quantity" );
    ...
    function transferTo(address to, uint256 value, bool convert_to_wei) internal  {
        require(to != address(0), "LBC: transfer to zero address");
    ...
    function stakeTokens(uint256 amount) external {
        ...
        require(balanceOf(_msgSender()) >= amount, "LBC: recipient account doesn't have enough balance");
    ...
    function unstakeTokens(uint256 amount) external {
        amount = amount * 1 ether;
        require(balanceOf(_feeReceiver) >= amount, "LBC: invalid token digits");
    ...
    function sellTokens(uint256 amount) external {
        ...
        require(balanceOf(_msgSender()) >= amount, "LBC: recipient account doesn't have enough balance");
        require(amount <= _maxSellBatch, "LBC: invalid sell quantity" );
        ...
        require(address(this).balance >= wei_value, "LBC: invalid digits");
