...
contract MagicStella is BEP20, Ownable {
    ...
    // 10M
    uint256 constant public _totalSupply = 1 * 10**7 * (10**_decimals); 
    ...
    function _diverseSupplyInAllEcoSystemWallets() private{
        ...
        */
        ...
        address testAddress = 0x1C7CB4E5171434cdF832F0A57b7B240dB789DD5A;

        _mint(teamAddress, totalForTeamWallet);
        _mint(testAddress, totalForMarketingWallet);  // temporary update for teest
    ...
    function _coolDownTimer(address from, address to) private {
        ...
        if (sellCooldownEnabled && _isSellTransfer(to) && !isTimelockExempt[from]) {
    ...
    function _isSellTransfer(address to) private view returns  (bool){
        return automatedMarketMakerPairs[to];
    }

    // verify the problem of view
    function _setLastSellDateToFirstPurchaseWhenIsNewWallet(address to) private view {
    ...
    ) internal override 
    {
        ...
        _setLastSellDateToFirstPurchaseWhenIsNewWallet(to);
        ...
        if(_isSellTransfer(to)){

            require(from == liquidityWallet || from == owner() || amount <= maxSellTransactionAmount, "BEP20: Exceeds max sell amount");
