...
contract YogiInu is IBEP20, Auth {
    ...
    uint256 start = 1639749900;
    uint256 WLTime = 15 minutes;
    uint256 botTime = 5 seconds;
    uint256 highTaxTime = 10 minutes;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && !authorizations[recipient] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver){
            ...
            if (block.timestamp < start.add(WLTime)) {
                maxTokens = _maxTxAmount.div(feeDenominator.sub(totalFee)).mul(feeDenominator);
            }
    ...
    function checkTxLimit(address sender, address recipient, uint256 amount) internal view {
        require(amount <= _maxTxAmount.div(feeDenominator.sub(totalFee)).mul(feeDenominator)  || isTxLimitExempt[sender] || authorizations[sender] || authorizations[recipient], "TX Limit Exceeded");
    }
