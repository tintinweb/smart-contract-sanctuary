...
contract CUSDStaking is Staking, IRedeemer {

    IERC20 public USDC;
    address public usdcAddress;

    uint256 public redeemPerShareStoredCPT;
    uint256 public redeemPerShareStoredUSDC;

    mapping(address => uint256) public userRedeemPerSharePaidCPT;
    mapping(address => uint256) public userRedeemPerSharePaidUSDC;

    mapping(address => uint256) public redeemsCPT;
    mapping(address => uint256) public redeemsUSDC;

    uint256 public lastRedeemAmountCPT;
    uint256 public lastRedeemAmountUSDC;


    modifier updateRedeem(address account){
        redeemsCPT[account] = redeemEarnedCPT(account);
        redeemsUSDC[account] = redeemEarnedUSDC(account);
        userRedeemPerSharePaidCPT[account] = redeemPerShareStoredCPT;
        userRedeemPerSharePaidUSDC[account] = redeemPerShareStoredUSDC;
        _;
    }
    ...
    function redeemEarnedCPT(address account) public view returns (uint256) {
        return _shareBalances[account] * (redeemPerShareStoredCPT - userRedeemPerSharePaidCPT[account])
        / SHARE_PRECISION + redeemsCPT[account];
    }


    function redeemEarnedUSDC(address account) public view returns (uint256) {
        return _shareBalances[account] * (redeemPerShareStoredUSDC - userRedeemPerSharePaidUSDC[account])
        / SHARE_PRECISION + redeemsUSDC[account];
    }


    function notifyRedeem(uint256 cusdAmount, uint256 cptAmount, uint256 usdcAmount) public override onlyPools {

        if (cusdAmount > 0) {

            coinCUSD.burn(cusdAmount);

            uint256 leftRatio = (_totalStaking - cusdAmount) * SHARE_PRECISION / _totalStaking;
            uint256 newUnusedRatio = shareUnusedRatio * leftRatio / SHARE_PRECISION;
            if (newUnusedRatio > 0)
                shareUnusedRatio = newUnusedRatio;

            _totalStaking -= cusdAmount;
        }

        if (cptAmount > 0) {
            lastRedeemAmountCPT = cptAmount;
            redeemPerShareStoredCPT += cptAmount * SHARE_PRECISION / _totalShare;
        }
        if (usdcAmount > 0) {
            lastRedeemAmountUSDC = usdcAmount;
            redeemPerShareStoredUSDC += usdcAmount * SHARE_PRECISION / _totalShare;
        }

    }


    function stake(uint256 amount) external override nonReentrant whenNotPaused updateReward(msg.sender) updateRedeem(msg.sender) {
        require(amount > 0, "cannot stake 0");

        SafeERC20.safeTransferFrom(coinCUSD, msg.sender, address(this), amount);
        _stake(msg.sender, amount);
    }

    function stakeWithReferral(uint256 amount, address referral) external nonReentrant whenNotPaused updateReward(msg.sender) updateRedeem(msg.sender) {
        require(amount > 0, "cannot stake 0");

        uint256 stakeAmount = _transferWithReferral(coinCUSD, amount, referral);
    ...
    function reinvestRedeem() public override nonReentrant updateRedeem(msg.sender) {
        ...
        uint256 redeemUSDC = redeemsUSDC[msg.sender];
        if (redeemUSDC > 0) {
            redeemsUSDC[msg.sender] = 0;
            address stakingUSDC = stakingAddresses[usdcAddress];
            SafeERC20.safeTransfer(USDC, stakingUSDC, redeemUSDC);
            IStaking(stakingUSDC).notifyReinvest(msg.sender, redeemUSDC);
        }

        uint256 redeemCPT = redeemsCPT[msg.sender];
        if (redeemCPT > 0) {
            redeemsCPT[msg.sender] = 0;
            address stakingCPT = stakingAddresses[cptAddress];
            SafeERC20.safeTransfer(coinCPT, stakingCPT, redeemCPT);
            IStaking(stakingCPT).notifyReinvest(msg.sender, redeemCPT);
        }
    }
    ...
    function notifyReinvest(address account, uint256 amount) external override updateReward(account) updateRedeem(account) onlyStakings {
        require(_totalStaking + amount <= coinCUSD.balanceOf(address(this)), "Notify amount exceeds balance!");
    ...
    function withdraw(uint256 shareAmount) public override nonReentrant updateReward(msg.sender) updateRedeem(msg.sender) {
        ...
        SafeERC20.safeTransfer(coinCUSD, msg.sender, withdrawAmount);
        emit Withdrawn(msg.sender, shareAmount);
    }


    function getRedeemWithPercent(uint256 percent) public override nonReentrant updateRedeem(msg.sender) {
        require(percent > 0 && percent <= 100, "percent wrong");
        uint256 redeemCPT = redeemsCPT[msg.sender] * percent / 100;
        uint256 redeemUSDC = redeemsUSDC[msg.sender] * percent / 100;

        if (redeemCPT > 0) {
            redeemsCPT[msg.sender] -= redeemCPT;
            SafeERC20.safeTransfer(coinCPT, msg.sender, redeemCPT);
        }
        if (redeemUSDC > 0) {
            redeemsUSDC[msg.sender] -= redeemUSDC;
            SafeERC20.safeTransfer(USDC, msg.sender, redeemUSDC);
        }
    ...
    function yieldAPR() public override view returns (uint256){
        if (_totalStaking > 0)
            return rewardRate * 31536000 * capcPricer.getCPTPrice() * CAPC_DECIMAL / _totalStaking / capcPricer.getCUSDPrice();
        else
            return 0;
    }
