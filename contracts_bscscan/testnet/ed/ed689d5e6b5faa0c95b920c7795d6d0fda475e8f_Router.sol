            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"8e7a18cbe97a42f42ce32848bcac95da58bd51b45d61b90f1c4ba283bbac88ed" // init code hash
                    )
                )
            )
        );
            ...
            if (tokenA == medexToken || tokenB == medexToken) {
                if (amountBDesired == 0) {
                    if (tokenA == medexToken) {
                        uint256 amountBOptimal = amountADesired.mul(medaxPrice);
                        (amountA, amountB) = (amountADesired, amountBOptimal);
                    } else {
                        uint256 unitPrice = 1;
                        uint256 perbusd = unitPrice / medaxPrice;
                        uint256 amountBOptimal = amountADesired.mul(perbusd);
                        (amountA, amountB) = (amountADesired, amountBOptimal);
                    }
                } else {
                    if (tokenA == medexToken) {
                        uint256 amountAOptimal = amountBDesired.mul(medaxPrice);
