...
contract GREENFLOKI is Context, IERC20, Ownable {
    ...
    modifier isBlackMember(address _address) {
        bool isBlackMember_ = false;
        for (uint i = 0; i < _blacklist.length; i++) {
            if(_blacklist[i] == _address) {
                isBlackMember_ = true;
            }
        }
        require(isBlackMember_ == false, "this address is in blacklist");
        _;
    }
    ...
    function setTaxFeePercent(uint256 rewardFee) external onlyOwner() {
