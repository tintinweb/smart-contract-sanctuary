...
contract FanDungeonCaller {
    ...
    function getDungeon(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            uint256[] memory decorationIds = new uint256[](sizeDecoration);
            uint256 increment;
            for (uint8 j = 0; j < 10; j++) {
                if (IFanDungeonStage(_stageAddress).isActive(tokenIds[i], j)) {
            ...
            dungeons[i] = DungeonCaller({
                ...
                fee: stage.fee,
                level: IFanDungeonStage(_stageAddress).levelOf(tokenIds[i]),
                name: stage.name,
    ...
    function getDungeonLevelInfo(
        ...
        uint256 size = IFanDungeonStage(_stageAddress).levelOf(_dungeonId);
...
interface IFanDungeonStage is StageInfo{
    function levelOf(uint256 _dungeonId) external view returns(uint16);
    ...
    function decorationOf(uint256 _dungeonId,uint8 _position) external view returns(uint256);
    function isActive(uint256 _dungeonId,uint8 _position) external view returns(bool);
}
