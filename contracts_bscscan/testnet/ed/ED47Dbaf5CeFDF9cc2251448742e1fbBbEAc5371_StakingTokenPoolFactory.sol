...
contract StakingTokenPool is Ownable, Initializable {
...
contract StakingTokenPoolFactory {
  ...
  function deployPoolContract(
    ...
    StakingTokenPool pool = new StakingTokenPool();
  ...
  function deployComputedDefaultPoolContract(
        IERC20 _stakeToken,
        IERC20 _rewardToken,
        uint256 _startBlock,
        uint256 _blocksDuration,
        uint256 _totalRewards) public {
    (uint256 rewardPerBlock, uint256 bonusEndBlock) = calculatedConfig(_totalRewards, _startBlock, _blocksDuration);
    deployPoolContract(_stakeToken, _rewardToken, rewardPerBlock, _startBlock, bonusEndBlock, defaultOwner);
  }
