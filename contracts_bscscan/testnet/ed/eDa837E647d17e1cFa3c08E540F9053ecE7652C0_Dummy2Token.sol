...
contract Dummy2RewardTracker is RewardPayingToken, Ownable {
    ...
    constructor() RewardPayingToken("Dummy2_Reward_Tracker", "Dummy2_Reward_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForRewards = 10000 * (10**9); 
    }

    function _transfer(address, address, uint256) internal override pure {
        require(false, "Dummy2_Reward_Tracker: No transfers allowed");
    }

    function withdrawReward() public override  pure {
        require(false, "Dummy2_Reward_Tracker: withdrawReward disabled. Use the 'claim' function on the main contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "Dummy2_Reward_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Dummy2_Reward_Tracker: Cannot update claimWait to same value");
...
contract Dummy2Token is BEP20, Ownable {
...
uint256 private constant INITIAL_SUPPLY = 10 ** 7 * 10 ** DECIMALS;
 ...
 uint256 private _totalSupply=INITIAL_SUPPLY;
  ...
  uint256 private constant Final_SUPPLY = ~uint128(0); 
    ...
    Dummy2RewardTracker public RewardTracker;

    address public walletMarket = 0x0FFE2F9A2e1c0C5f1Ba74447b30c83910AD821D8;
    address public _walletBack = 0xC0b703061a0C15401e38DF62A0BA712a3347512a;
...
address public lp;
  ILP public liquidutyReciever;
    ...
    constructor() BEP20("Dummy2", "Dummy2") {

    	RewardTracker = new Dummy2RewardTracker();
 ...
 function rebaseToken(uint256 epoch, int256 supplyDelta)
  ...
  returns (uint256)
  {
    ...
    function updateIDEXV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(IDEXV2Router), "Dummy2: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Dummy2: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 750000, "Dummy2: gasForProcessing must be between 200,000 and 750,000");
        require(newValue != gasForProcessing, "Dummy2: Cannot update gasForProcessing to same value");
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 bnbForDummy2BuyBack = bnbBalance.mul(BuyBackCharges).div(bnbFeeFactor);
      
        uint256 bnbForMarketing = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection).sub(bnbForDummy2BuyBack);
        ...
        if(bnbForDummy2BuyBack > 0)    
            payable(_walletBack).transfer(bnbForDummy2BuyBack);
