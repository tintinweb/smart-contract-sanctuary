...
contract WxsToken is Ownable {
    ...
    event WxsFarmAddress(address indexed _sender, address indexed _wxsFarmAddress);
    ...
    function _transfer(address _sender, address _recipient, uint256 _amount) internal {
        ...
        if(pancakePairAddressOf(_recipient)){
            ...
            balances[wxsFarmAddress] += _amount.mul(15).div(100);// WxsFarmAddress 15%
            emit Transfer(_sender, wxsFarmAddress, _amount.mul(15).div(100));

            tradeFeeTotal += _amount.mul(15).div(100);
            emit TradeFee(_sender, wxsFarmAddress, _amount.mul(15).div(100));// add trade fee log
            ...
            balances[wxsFarmAddress] += _amount.mul(10).div(100);// WxsFarmAddress 10%
            emit Transfer(_sender, wxsFarmAddress, _amount.mul(10).div(100));

            tradeFeeTotal += _amount.mul(10).div(100);
            emit TradeFee(_sender, wxsFarmAddress, _amount.mul(10).div(100));// add trade fee log
    ...
    function setWxsFarmAddress(address _wxsFarmAddress) public onlyOwner returns (bool) {
        wxsFarmAddress = _wxsFarmAddress;
        emit WxsFarmAddress(msg.sender, _wxsFarmAddress);
        return true;
    }
