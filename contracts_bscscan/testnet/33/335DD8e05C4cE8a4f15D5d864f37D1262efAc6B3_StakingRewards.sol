...
contract StakingRewards is Ownable, ReentrancyGuard {
    ...
    function earned(address _account) public view returns (uint256) {
        ...
        (rewardPerToken() - userInfo[_account].userRewardPerTokenPaid)) * userInfo[_account].multiplier.div(10) / 1e18) +
    ...
    function checkPeriodUnlock(address _account, uint256 _time) public returns (uint256) {
    ...
    function depositTokenReward(uint256 _amount) public onlyOwner
    {
        _totalSupply += _amount;
        stakingToken.transferFrom(msg.sender, address(this), _amount);
    }

    function withdrawTokenReward(uint256 _amount) public onlyOwner
    {
        _totalSupply -= _amount;
