...
contract NICKMULTIPLE is BEP1155, Ownable, BEP1155Burnable{
    event Approve(
    ...
    function saleWithToken(
        ...
        string memory bidtoken
            ...
            require(amount == order_place[from][tokenId].price.mul(nooftoken) , "Amount Mismatchess");
    ...
    function _saleToken(
        ...
        if(keccak256(abi.encodePacked((bidtoken))) == keccak256(abi.encodePacked(("BNB")))){   
            require( msg.value == val, "Value doesnot matchess");
            ...
            require( msg.value == _adminfee.add(roy.add(netamount)), "Admin feee is not the same");
            ...
            require( approveValue >= val, "Approval is Invalid");
            (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], serviceValue);
            require( approveValue >= _adminfee.add(roy.add(netamount)), "Approval value not matching with Admin");
    ...
    function _acceptBId(string memory tokenAss,address from, uint256 amount, uint256 tokenId, address admin) internal{
        ...
        require( approveValue >= val, "Approval issue value");
        require(_balances[tokenId][msg.sender] > 0, "Is Not a Owner");
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], serviceValue);
        require( approveValue >= _adminfee.add(roy.add(netamount)), "Approve Value is Invalid");
