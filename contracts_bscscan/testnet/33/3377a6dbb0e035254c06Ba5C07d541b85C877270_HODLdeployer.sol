...
contract HODLdeployer is ReentrancyGuard {
    ...
    function createPreSaleBUSD(
        ...
        bytes memory bytecode = type(preSaleBusd).creationCode;
...
contract preSaleBnb is ReentrancyGuard {
    ...
    constructor() {
        ...
        stake = IHodlStake(0xAD6D8503627D6855A000aF56492CF8ec637475A9);
        adminFeePercent = 2;
    }
    ...
    function buyToken() public payable allowed{
        ...
        require(numberOfTokens.add(balances[msg.sender]) <= tierAmount[userTier],"HODL: Amount exceeded tier limit");
    ...
    function userContribution(address _user) public view returns(uint256){
        uint256 busdAmount = balances[_user].mul(1000).div(tokenPrice).div(10 ** token.decimals());
...
contract preSaleBusd is ReentrancyGuard {
    ...
    // to buy token during preSale time => for web3 use
    function buyToken(uint256 _amountBusd) public allowed{
    ...
    function withdrawAndInitializePool() public onlyTokenOwner allowed{
        require(block.timestamp > preSaleTime,"HODL: PreSale not over yet");
        uint256 amountRaised = getContractBusdBalance();
    ...
    function migrateBusdFunds() external onlyTokenOwner allowed{
        require(getCurrentTime() > preSaleTime,"HODL: Time error");
        tokenOwner.transfer(getContractBusdBalance());
    }
