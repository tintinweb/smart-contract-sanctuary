...
contract VendingMachine is OwnableUpgradeable, OracleClient {
    ...
    IERC20 public token;
    uint256 constant MINIMUM_GAS_PRICE = 4e15;
    ...
    mapping(uint256 => uint256) public privLevelToMinBonus;
    mapping(uint256 => uint256) public privLevelToMaxBonus;
    ...
    mapping(address => uint256) private playerToTicketID;
        ...
        override
    {
        ...
        token.approve(address(treeWallet), (ticket.amount * TREE_ALLOCATION) / TOTAL_ALLOCATION);
        treeWallet.addReward( (ticket.amount * TREE_ALLOCATION) / TOTAL_ALLOCATION);
        token.transfer(burnWallet, (ticket.amount * BURN_ALLOCATION) / TOTAL_ALLOCATION);
        token.transfer(feeWallet, (ticket.amount * FEE_ALLOCATION) / TOTAL_ALLOCATION);
        ...
        override
    {
        ...
        emit RefundTicket(ticket.player, ticket.amount, ticket.ticketID);
    }
...
contract Seedmon is ERC721Enumerable, TrustCaller {
    ...
    function adjustSeedmon(bytes32 name, SeedmonAttribute.Profile calldata profile_, SeedmonAttribute.Stat calldata stat_, SeedmonAttribute.BaseAction calldata baseAction_) external onlyOwner {
