...
contract SupeNFTMintGold is Ownable {
    ...
    event StakeOne(uint256 _asset_id, uint256 _start_ts, uint256 _end_ts);
    event ExitOne(uint256 _asset_id, uint256 _start_ts, uint256 _end_ts, uint256 _tlp, uint256 _frog);
    ...
    function stake_one(uint256 _asset_id) public checkPaused{
        ...
        emit StakeOne(_asset_id, block.timestamp, block.timestamp + interval);
    }
    ...
    function exit_one(uint256 _asset_id) public checkPaused{
        ...
        if(_frog > 0){
            wait_claimed[msg.sender][address(frog_token)] = wait_claimed[msg.sender][address(frog_token)].add(_tlp);
        }
        if(balances[msg.sender] >= 1){
            balances[msg.sender] = balances[msg.sender].sub(1);
        }
        emit ExitOne(_asset_id, stakes[msg.sender][_asset_id].start_ts, stakes[msg.sender][_asset_id].end_ts, _tlp, _frog);
    ...
    function _get_reward(address _account, uint256 _asset_id) internal view returns (uint256, uint256){
        ...
        _tlp = ((seed % 9000).add(1000)).mul(10 ** 18);
        return (_tlp, _frog);
    }
