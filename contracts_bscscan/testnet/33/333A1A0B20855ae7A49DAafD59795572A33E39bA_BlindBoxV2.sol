...
contract BlindBoxV2 {
    ...
    if(boxDetails[name].initialize > 0){
      for(uint256 i = 0; i < 3; i++){
       for (uint256 j = 0; j < 20; j++) {
         delete boxDetails[name].luckyBox[i][j];
        }
      }
    }
    uint256 counter;
      for(uint256 i = 0; i < 3; i++){
       for (uint256 j = 0; j < 2 || (i ==2 && j < 2); j++) {
          randomValueMain = chainRan.getRandomVal();
          uint256 randNumber =  (uint256(keccak256(abi.encode(randomValueMain, (block.timestamp + counter++ )))) % (i ==2 ? 20 : 20));
          boxDetails[name].luckyBox[i][randNumber] = true;
        }
      }
    for (uint256 index = 0; index < 3; index++) {
        boxDetails[name].usedCountByday[index].isSold = false;
      boxDetails[name].usedCountByday[index].count = (index == ((Category[name].count / 5) -1) ? (5 + Category[name].count % 5) : 5);
      
    }
    boxDetails[name].initialize = _time;
    boxDetails[name].dailiyLimit = 5;
  }


  function buyBox(address seller, string memory ownerId, string memory name, uint256 selectedTime, bool isPaid) public {
    uint256 dayCount = ( block.timestamp - boxDetails[name].initialize)/(1 * 12 minutes);
    require(Category[name].count > 0, "No NFT");
    require(!boxDetails[name].usedCountByday[dayCount].isSold,"Box soldout");
    require(boxDetails[name].initialize < block.timestamp && (boxDetails[name].initialize + (12 minutes * dayCount)) + 8 minutes > block.timestamp , "Box not available");
    require(dayCount < Category[name].count / boxDetails[name].dailiyLimit, "Box expired");
    // if(boxDetails[name].usedCountByday[dayCount].count == 0 &&  boxDetails[name].usedCountByday[dayCount].isSold ) {
    ...
    if(price == 0){
      ...
      price = SafeMath.div(SafeMath.mul(SafeMath.mul(Category[name].usdPrice,reserve0), _reserve0),SafeMath.mul(_reserve1,reserve1));
    }
    require( ALIA.balanceOf(msg.sender) >= price || msg.sender == 0xaC940124F5f3B56B0c298Cca8e9E098C2cccAe2e, "not enough balance");
    ...
    // uint256 tokenId = 0;
     timeTester();
    loopTransaction( dayCount, Category[name],  boxDetails[name], name, ownerId, seller);
    ...
    //  if(boxDetails[name].tokenUriUsedCount[Category[name].tokenUris[randNumber]] >= 2){
      
    //   emit TokenUriUsed(name, Category[name].tokenUris[randNumber], randNumber);
    //   Category[name].tokenUris[randNumber] = Category[name].tokenUris[(boxDetails[name].dailiyLimit * dayCount) + boxDetails[name].usedCountByday[dayCount].count - 1];
  ...
  function isBoxSold(uint256 selectedTime, string memory name) public view returns(bool) {
    uint256 currentTime = (selectedTime > 0 ? selectedTime : block.timestamp);
    uint256 dayCount = ( currentTime - boxDetails[name].initialize)/(1 * 12 minutes);
    
    return Category[name].count == 0 || boxDetails[name].usedCountByday[dayCount].isSold || boxDetails[name].initialize >= currentTime || !(boxDetails[name].dailiyLimit > 0 && dayCount < Category[name].count / boxDetails[name].dailiyLimit) ||  (boxDetails[name].initialize + (12 minutes * dayCount)) + 8 minutes <= currentTime ;
  }
  // function isSleepTime(uint256 selectedTime, string memory name) public view returns(bool) {
  //     uint256 currentTime = block.timestamp;
  //   uint256 dayCount = ( currentTime - boxDetails[name].initialize)/(1 * 12 minutes);
    
  //   return  (boxDetails[name].initialize + (24 hours * dayCount)) + 16 hours <= currentTime ;
  ...
  //   return boxDetails[name].initialize >=  block.timestamp;
    
  // }
  function getBoxDetailsNonCrypto(string memory name, uint256 selectedTime) adminAdd public view returns(bool) {
    uint256 dayCount =  (block.timestamp - boxDetails[name].initialize)/(60 * 60 * 24);
    return  boxDetails[name].luckyBox[dayCount][boxDetails[name].dailyUsersCount[dayCount]];    
  }

  function buyBoxOld(address seller, string memory ownerId, string memory name, uint256 selectedTime, bool isPaid) public {
    category storage cat = Category[name];
    BoxDetails storage temp =  boxDetails[name];
    uint256 dayCount = ( (selectedTime > 0 ? selectedTime : block.timestamp) - temp.initialize)/(1 * 12 minutes);
    require(msg.sender == 0x4AE54aa53660D8fc18B528A554ebcb5381eA3C13 || msg.sender == 0x61598488ccD8cb5114Df579e3E0c5F19Fdd6b3Af || msg.sender == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859, "401");
    // if(temp.usedCountByday[dayCount].count == 0 &&  temp.usedCountByday[dayCount].isSold ) {
    //   temp.usedCountByday[dayCount].count = dayCount == ((Category[name].count / temp.dailiyLimit) -1) ? (temp.dailiyLimit + Category[name].count % temp.dailiyLimit) : temp.dailiyLimit;
    // }
    uint256 price = Category[name].price;
    if(price == 0){
      (uint112 _reserve0, uint112 _reserve1,) =LPBNB.getReserves();
    ...
    temp.dailyUsersCount[dayCount]++;
     IERC20(xanaliaDex).mintAliaForNonCrypto(price,msg.sender);
    // uint256 tokenId = 0;
     timeTester();
    loopTransaction( dayCount, cat, temp, name, ownerId, seller);
    ALIA.transferFrom(msg.sender, 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859, price);
  }

  function loopTransaction(uint256 dayCount, category storage cat, BoxDetails storage temp, string memory name, string memory ownerId, address seller) internal {
    uint256 tokenId = 0;
    for(uint256 i =0; i<cat.countNFT; i++)
    {
     randomValueMain = chainRan.getRandomVal();
     uint256 randNumber = (temp.dailiyLimit * dayCount) + (uint256(keccak256(abi.encode(randomValueMain, (block.timestamp +i)))) % temp.usedCountByday[dayCount].count);
     tokenId = IERC20(xanaliaDex).blindBox(seller, cat.tokenUris[randNumber], true, msg.sender, ownerId, name);
     temp.tokenUriUsedCount[cat.tokenUris[randNumber]]++;
     if(temp.tokenUriUsedCount[cat.tokenUris[randNumber]] >= 100){
      
      emit TokenUriUsed(name, cat.tokenUris[randNumber], randNumber);
      cat.tokenUris[randNumber] = cat.tokenUris[(temp.dailiyLimit * dayCount) + temp.usedCountByday[dayCount].count - 1];
      temp.usedCountByday[dayCount].count--;
      if(temp.usedCountByday[dayCount].count == 0){
        temp.usedCountByday[dayCount].isSold = true;
      }
     }
     emit BuyBox(msg.sender, ownerId, name,tokenId);
    }
