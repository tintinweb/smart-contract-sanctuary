...
contract TokenContract is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 500 * 1e12 * 1e18; // 500 trillion total supply
    ...
    address public marketWallet; // market wallet wallet address
    address public burnAddress = 0x000000000000000000000000000000000000dEaD; // dead address

    uint256 public minTokenToSwap = _tTotal.div(100000); // 0.001 of total supply amount will trigger the swap and add liquidity
    ...
    // double sell tax fee
    uint256 public redistributionFeeOnSelling = 40; // 4% will be distributed among holder as token divideneds
    uint256 public liquidityFeeOnSelling = 40; // 4% will be added to the liquidity pool
    uint256 public marketFeeOnSelling = 100; // 10% will go to the market wallet address
    uint256 public burnFeeOnSelling = 20; // 2% will go to the burn wallet address

    // normal tax fee
    uint256 public redistributionFee = 20; // 20% will be distributed among holder as token divideneds
    uint256 public liquidityFee = 20; // 2% will be added to the liquidity pool
    uint256 public marketFee = 50; // 5% will go to the market wallet address
    uint256 public burnFee = 10; // 1% will go to the burn wallet address
    ...
    function _tokenTransfer(
        ...
        if (sender == dexPair && takeFee) {
            ...
            if(block.timestamp < _lastBuyTime[msg.sender].add(5 minutes)){
                setSellFee();
            } else {
                setNormalFee();
            }
    ...
    function _transferStandard(
        ...
        _takeLpFee(sender, tAmount, currentRate);
    ...
    // take fees for liquidity pool
    function _takeLpFee(address sender, uint256 tAmount, uint256 currentRate) internal {
    ...
    function _takeBurnFee(address sender, uint256 tAmount, uint256 currentRate) internal {
        ...
        uint256 rFee = tFee.mul(currentRate);
        _rOwned[burnAddress] = _rOwned[burnAddress].add(rFee);
