...
import "../pools/StakingPenaltyPool.sol";

import "../interfaces/IStakeFactory.sol";

contract StakePenaltyFactory is IStakeFactory {
    ...
    function createStakingPool(
        ...
        require(msg.sender == stakeMaster, "Only stakeAdminMaster can call");

        StakingPenaltyPool stakingPenaltyPool = new StakingPenaltyPool(
            _stakingToken,
            _poolToken,
            _startTime,
            _finishTime,
        ...
        if(_creator != IStakeMaster(stakeMaster).owner()) {
            stakingPenaltyPool.setAdminPermission(_creator, true);
            stakingPenaltyPool.transferOwnership(IStakeMaster(stakeMaster).owner());
        } else {
            stakingPenaltyPool.transferOwnership(_creator);
        }

        return address(stakingPenaltyPool);
    }
}
...
contract StakingPenaltyPool is StakingPool {
    ...
    mapping(address => UserLockerInfo) public userLockerInfo; // Info per each user
    ...
    struct UserLockerInfo {
        ...
        mapping(uint256 => DepositInfo) deposits; // User's deposits
    }
        ...
        _feeTo)
    {
        ...
        // TODO: think about transfer this logic to constructor of StakeAdminMaster
    ...
    function stakeTokens(uint256 _amountToStake) public override {
        ...
        uint256 processedAmount = _amountToStake.sub(feeAmount);

        UserLockerInfo storage userLocker = userLockerInfo[msg.sender];
        userLocker.deposits[userLocker.depositTail] = DepositInfo({
        amount : processedAmount,
        time : now
        });
        userLocker.depositTail = userLocker.depositTail.add(1);
    ...
    function withdrawStake(uint256 _amount) public override {
        ...
        (uint256 sumWithdrawAmount, uint256 sumFeeAmount) = calcWithdrawAmount(msg.sender, _amount);
        stakingToken.safeTransfer(feeTo, sumFeeAmount);
        user.amount = user.amount.sub(sumFeeAmount);

        super.withdrawStake(sumWithdrawAmount);
    }
    ...
    function emergencyWithdraw() public override {
        ...
        ( , uint256 sumFeeAmount) = calcWithdrawAmount(msg.sender, user.amount);
        stakingToken.safeTransfer(feeTo, sumFeeAmount);
        user.amount = user.amount.sub(sumFeeAmount);
    ...
    function calcWithdrawAmount(address _user, uint256 _amount) public returns (uint256, uint256) {
        UserLockerInfo storage userLocker = userLockerInfo[_user];

        uint256 sumWithdrawAmount = 0;
        uint256 sumFeeAmount = 0;

        for (uint256 i = userLocker.depositHead; i < userLocker.depositTail; i++) {
            DepositInfo storage userDeposit = userLocker.deposits[i];
            uint256 depositSubAmount = Math.min(userDeposit.amount, _amount);

            uint256 penaltyBP = getPenaltyBP(userDeposit.time);
            uint256 feeAmount = depositSubAmount.mul(penaltyBP).div(MAX_BPS);
            uint256 withdrawAmount = depositSubAmount.sub(feeAmount);

            sumFeeAmount = sumFeeAmount.add(feeAmount);
            sumWithdrawAmount = sumWithdrawAmount.add(withdrawAmount);

            if (userDeposit.amount > _amount) {
                userDeposit.amount = userDeposit.amount.sub(depositSubAmount);
            } else {
                delete userLocker.deposits[i];
                userLocker.depositHead = userLocker.depositHead.add(1);
            }

            _amount = _amount.sub(depositSubAmount);
    ...
    function getPenalties(address _receiver, uint256 _amount) external view returns (uint256, uint256) {
        UserLockerInfo memory user = userLockerInfo[_receiver];

        uint256 sumWithdrawAmount = 0;
        uint256 sumFeeAmount = 0;

        for (uint256 i = user.depositHead; i < user.depositTail; i++) {
            DepositInfo memory userDeposit = userLockerInfo[_receiver].deposits[i];
            uint256 depositSubAmount = Math.min(userDeposit.amount, _amount);
    ...
    function getDeposit(address _user, uint256 _id) external  view returns (uint256, uint256)    {
        DepositInfo memory userDeposit = userLockerInfo[_user].deposits[_id];
        return (userDeposit.amount, userDeposit.time);
    }

    function updatePenalty(uint256 _index, uint256 _duration, uint256 _penaltyBP) external onlyOwner {
