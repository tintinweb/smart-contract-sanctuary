...
contract AntiBotLiquidityGeneratorToken is IERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        require(fees[0] >= 0 && fees[0] <= 10**4, "Invalid tax fee");
        require(
            fees[1] >= 0 && fees[1] <= 10**4,
            "Invalid liquidity fee"
        );
        require(
            fees[2] >= 0 && fees[2] <= 10**4,
            "Invalid charity fee"
        );
    ...
    function _transfer(
        ...
        if (enableAntiBot) {
            tattooAntiBot.onPreTransferCheck(from, to, amount);
        }
