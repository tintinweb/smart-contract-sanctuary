...
contract MonoStaking is ReentrancyGuard, DataStorage, Events, Ownable, Utils {
    ...
    address public monoToken = 0x44d8DF9034F35eeDC972614d489E4E2ABBE5b993;
    ...
    function invest(uint8 poolId, uint256 _amount) external nonReentrant {
    ...
    function _invest(
        ...
        if (user.amount > 0) {
            user.rewardDebt = user.rewardDebt.add(
                getUserDividends(_msgSender(), poolId)
            );
        }
        ...
        emit NewDeposit(userAddress, poolId, _amount, PROJECT_FEE);
    }
    ...
    function withdraw(uint256 poolId) external nonReentrant {
        ...
        UserInfo storage user = userInfos[poolId][_msgSender()];
        uint256 currentDividends = getUserDividends(_msgSender(), poolId).add(
            user.rewardDebt
        );
        user.rewardDebt = 0;
        ...
        if (block.timestamp < pool.toTime) {
            feeWithdraw = currentDividends.mul(WIHDRAW_FEE).div(
                PERCENTS_DIVIDER
            );
        }
        IBEP20(monoToken).transfer(
            _msgSender(),
            currentDividends.sub(feeWithdraw)
        );
        if (feeWithdraw > 0) {
            IBEP20(monoToken).transfer(commissionWallet, feeWithdraw);
    ...
    function unStake(uint256 poolId) external nonReentrant {
        ...
        UserInfo storage user = userInfos[poolId][_msgSender()];
        Pool memory pool = pools[poolId];
        if (user.isUnStake == false) {
            user.isUnStake = true;
            uint256 currentDividends = getUserDividends(_msgSender(), poolId)
                .add(user.rewardDebt);
            IBEP20(pool.tokenAddress).transfer(_msgSender(), user.amount);
            IBEP20(monoToken).transfer(_msgSender(), currentDividends.sub(feeUnlock));
            ...
            if(block.timestamp < pool.toTime) {
                feeUnlock = currentDividends.mul(UNSTAKE_FEE).div(PERCENTS_DIVIDER);
            }
            if (feeUnlock > 0) {
                IBEP20(monoToken).transfer(commissionWallet, feeUnlock);
...
contract Utils is DataStorage {
    ...
    function getUserDividends(address userAddress, uint256 poolId)
        ...
        returns (uint256)
    {
        UserInfo storage user = userInfos[poolId][userAddress];
        Pool memory pool = pools[poolId];

        uint256 totalAmount = 0;
        uint256 fromTime = user.start > user.checkpoint
            ? user.start
            : user.checkpoint;
        uint256 toTime = block.timestamp < pool.toTime
            ? block.timestamp
            : pool.toTime;

        uint256 period = toTime.sub(fromTime).div(TIME_STEP);
        totalAmount = pool.rewardPerDay.mul(period).mul(user.amount).div(pool.totalAmount);
