...
// File: contracts/interface/ILPTokenSGCPoolForFactory.sol

// SPDX-License-Identifier: MIT

pragma solidity ^0.8.0;

interface ILPTokenSGCPoolForFactory{
	function consumeForFactory(address addr, uint256 amount) external;
	//  function getStakeHeros(address addr) external view returns(uint256[] memory);
}


// File: contracts/interface/IHeroTokenSGCPool.sol
...
// File: contracts/interface/IEquipIntegralToken.sol
...
// File: contracts/interface/IRewardToken.sol
...
// File: contracts/interface/IWarManager.sol
...
// File: contracts/interface/IConditionAdditionFilter.sol
...
// File: contracts/interface/INFTMarket.sol
...
// File: contracts/interface/ICardSlotManager.sol
...
// File: contracts/interface/ISGCToken.sol
...
// File: contracts/interface/IEquipManager.sol
...
// File: contracts/interface/IChecker.sol
...
// File: contracts/interface/IHeroManager.sol
...
// File: contracts/interface/IPowerEnhancer.sol
...
// File: contracts/interface/IEquipToken.sol
...
// File: contracts/library/ContractManagerUtil.sol
...
// File: contracts/interface/IContractManager.sol
...
// File: contracts/library/HeroTokenUtil.sol
...
// File: contracts/interface/IHeroToken.sol
...
// File: contracts/interface/IHeroFactory.sol
...
interface IHeroFactory{
    struct HeroBlindbox{
        uint256   parValue;
        uint256[] starRates;
        uint256[] starStepRates;
        uint256[] qualityRates;
        uint256[] qualityStepRates;
    }
    
    struct CustomizeBlindbox{
        uint256 minParValue;
        uint256 minStar;
        uint256[] starRates;
        uint256[] starStepRates;
...
// File: contracts/factory/HeroFactory.sol
...
contract HeroFactory is AccessControlEnumerable {
    using HeroTokenUtil for IHeroToken; 
    using ContractManagerUtil for IContractManager;
    using SafeMath for uint256;
    using SafeERC20 for ISGCToken;
    
    bytes32 public constant GOVERNOR_ROLE = keccak256("GOVERNOR_ROLE");
    
    IContractManager public contractManager;
    
    mapping(IHeroToken.HeroQuality => uint256[]) qualityToBaseIds;
    mapping(uint256 => IHeroFactory.HeroBlindbox) public heroBlindboxs;
    
    uint256 public maxStar = 5;
    uint256[] public customizeBlindboxQualityRates;
    uint256[] public customizeBlindboxQualityStepRates;
    
    uint256 internal seedFactor = 0;
    
    constructor(address _contractManager){
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(GOVERNOR_ROLE,  _msgSender());
        setContractManager(_contractManager);
        uint256[] memory tmpCustomizeBlindboxQualityRates = new uint256[](3);
        tmpCustomizeBlindboxQualityRates[0] = 70;
        tmpCustomizeBlindboxQualityRates[1] = 25;
        tmpCustomizeBlindboxQualityRates[2] = 5;
        setCustomizeBlindboxQualityRates(tmpCustomizeBlindboxQualityRates);
    }
    
    function setCustomizeBlindboxQualityRates(uint256[] memory _customizeBlindboxQualityRates) onlyRole(GOVERNOR_ROLE) public{
        require(_customizeBlindboxQualityRates.length == 3);
        customizeBlindboxQualityRates = _customizeBlindboxQualityRates;
        customizeBlindboxQualityStepRates = _customizeBlindboxQualityRates;
        for(uint256 i = 1; i < _customizeBlindboxQualityRates.length; i++){
            customizeBlindboxQualityStepRates[i] += customizeBlindboxQualityStepRates[i - 1];
        }
    }
    ...
    function addHeroBaseIds(IHeroToken.HeroQuality quality, uint256[]calldata baseIds) onlyRole(GOVERNOR_ROLE) public{
        qualityToBaseIds[quality] = baseIds;
    }
    
    function getHeroBaseIds(IHeroToken.HeroQuality quality) view public returns(uint256 [] memory){
        return qualityToBaseIds[quality];
    }
    
    function getHeroBlindbox(uint256 parValue) view public returns(uint256, uint256[] memory,uint256[] memory){
        IHeroFactory.HeroBlindbox storage heroBlindbox = heroBlindboxs[parValue];
        return (heroBlindbox.parValue, heroBlindbox.starRates, heroBlindbox.qualityRates);
    }
    
    function addHeroBlindbox(uint256 parValue, uint256[] calldata starRates,uint256[] calldata qualityRates) onlyRole(GOVERNOR_ROLE) public{
        require(starRates.length == 5);
        require(qualityRates.length == 3);
        require(parValue > 0);
        
        IHeroFactory.HeroBlindbox storage heroBlindbox = heroBlindboxs[parValue];
        if(heroBlindbox.parValue > 0){
            delete heroBlindboxs[parValue];
            heroBlindbox = heroBlindboxs[parValue];
        }
        
        heroBlindbox.parValue = parValue;
        heroBlindbox.starRates = starRates;
        heroBlindbox.starStepRates = starRates;
        for(uint256 i = 1; i < heroBlindbox.starStepRates.length;i++){
            heroBlindbox.starStepRates[i] += heroBlindbox.starStepRates[i-1];
        }
        
        heroBlindbox.qualityRates = qualityRates;
        heroBlindbox.qualityStepRates = qualityRates;
        for(uint256 i = 1; i < heroBlindbox.qualityStepRates.length;i++){
            heroBlindbox.qualityStepRates[i] += heroBlindbox.qualityStepRates[i-1];
        }
    }
    
    function removeHeroBlindbox(uint256 parValue) onlyRole(GOVERNOR_ROLE) public{
        //  require(parValue > 0);
        IHeroFactory.HeroBlindbox storage heroBlindbox = heroBlindboxs[parValue];
        if(heroBlindbox.parValue > 0){
            delete heroBlindboxs[parValue];
    ...
    function openFixedHeroBlindboxMoreTimes(uint256 parValue, uint256 times) public{
        require(times >= 1);
        for(uint256 i = 0; i < times; i++){
            openFixedHeroBlindbox(parValue);
        }
    }
    
    function openFixedHeroBlindbox(uint256 parValue) public{
        require(tx.origin == msg.sender, "dont supoort contract call!");
        require(contractManager._heroSellFeeAddr() != address(0x0), "HeroSellFeeAddr must be set!");
        IHeroFactory.HeroBlindbox storage heroBlindbox = heroBlindboxs[parValue];
        require(heroBlindbox.parValue == parValue, "this fixed blind box is not exist!");
        
        address operator = _msgSender();
        contractManager.sgcToken().safeTransferFrom(operator, contractManager._heroSellFeeAddr(), parValue);
        
        uint256 star  = generateStar(heroBlindbox.starStepRates);
        IHeroToken.HeroQuality quality = generateQuality(heroBlindbox.qualityStepRates);
        uint256 baseId = generateBaseID(quality);
        bool fromFixed = true;
        if(star == 1){
            parValue = 5;
        }else if(star == 2){
        ...
        uint256 sgcDecimals = uint256(contractManager.sgcToken().decimals());
        uint256 decimals    = 10**sgcDecimals;
        parValue *= decimals;
        
        contractManager.heroToken().mint(operator, star, quality, parValue, contractManager._sgcToken(), baseId, fromFixed);
    }
    
    function openCustomizeBlindboxMoreTime(uint256 parValue, uint256 times) public{
        require(times >= 1);
        for(uint256 i = 0; i < times; i++){
            openCustomizeBlindbox(parValue);
        }
    }
    
    function openCustomizeBlindbox(uint256 parValue) public{
        require(tx.origin == msg.sender, "dont supoort contract call!");
        require(contractManager._heroSellFeeAddr() != address(0x0), "HeroSellFeeAddr must be set!");
        
        IHeroFactory.CustomizeBlindbox memory customizeBlindbox = getCustomizeBlindbox(parValue);
        address operator = _msgSender();
        //  contractManager.sgcToken().destroy(operator, parValue);
        contractManager.sgcToken().safeTransferFrom(operator, contractManager._heroSellFeeAddr(), parValue);
        uint256 star   = customizeBlindbox.minStar + generateStarForCustomizeBlindbox(customizeBlindbox.starStepRates) - 1;
        IHeroToken.HeroQuality quality = generateQuality(customizeBlindboxQualityStepRates);
        uint256 baseId = generateBaseID(quality);
        bool fromFixed = false;
        contractManager.heroToken().mint(operator, star, quality, parValue, contractManager._sgcToken(), baseId, fromFixed);
    }
    
    function openCustomizeBlindboxForPoolMoreTime(address pool, uint256 parValue, uint256 times) public{
        require(times >= 1);
        for(uint256 i = 0; i < times; i++){
            openCustomizeBlindboxForPool(pool, parValue);
        }
    }
    
    function openCustomizeBlindboxForPool(address pool, uint256 parValue) public {
        require(tx.origin == msg.sender, "dont supoort contract call!");
        //  require(contractManager._heroSellFeeAddr() != address(0x0), "HeroSellFeeAddr must be set!");
        
        IHeroFactory.CustomizeBlindbox memory customizeBlindbox = getCustomizeBlindbox(parValue);
        address operator = _msgSender();
        ILPTokenSGCPoolForFactory(pool).consumeForFactory(operator, parValue);
    ...
    function getCustomizeBlindbox(uint256 parValue) view internal returns(IHeroFactory.CustomizeBlindbox memory customizeBlindbox){
        uint256 sgcDecimals = uint256(contractManager.sgcToken().decimals());
        uint256 decimals    = 10**sgcDecimals;
        require(parValue >= 5*decimals, "too less money");
        uint256 totalRate   = 100*decimals;
        
        if(parValue < 10*decimals){
            customizeBlindbox.minStar = 1;
            customizeBlindbox.starRates = new uint256[](5);
            customizeBlindbox.starRates[0] = totalRate - parValue + 5*decimals;
            customizeBlindbox.starRates[1] = (totalRate - customizeBlindbox.starRates[0])*2/5;
            customizeBlindbox.starRates[2] = (totalRate - customizeBlindbox.starRates[0])*3/10;
            customizeBlindbox.starRates[3] = (totalRate - customizeBlindbox.starRates[0])*1/5;
            customizeBlindbox.starRates[4] = (totalRate - customizeBlindbox.starRates[0])*1/10;
        }else if(parValue < 20*decimals){
            customizeBlindbox.minStar = 2;
            customizeBlindbox.starRates = new uint256[](4);
            customizeBlindbox.starRates[0] = totalRate - parValue + 10*decimals;
            customizeBlindbox.starRates[1] = (totalRate - customizeBlindbox.starRates[0])*1/2;
            customizeBlindbox.starRates[2] = (totalRate - customizeBlindbox.starRates[0])*1/3;
            customizeBlindbox.starRates[3] = (totalRate - customizeBlindbox.starRates[0])*1/6;
        }else if(parValue < 40*decimals){
            customizeBlindbox.minStar = 3;
            customizeBlindbox.starRates = new uint256[](3);
            customizeBlindbox.starRates[0] = totalRate - parValue + 20*decimals;
            customizeBlindbox.starRates[1] = (totalRate - customizeBlindbox.starRates[0])*2/3;
            customizeBlindbox.starRates[2] = (totalRate - customizeBlindbox.starRates[0])*1/3;
        }else if(parValue < 80*decimals){
            customizeBlindbox.minStar = 4;
            customizeBlindbox.starRates = new uint256[](2);
            customizeBlindbox.starRates[0] = totalRate - parValue + 40*decimals;
            customizeBlindbox.starRates[1] = (totalRate - customizeBlindbox.starRates[0]);
        }else{
            customizeBlindbox.minStar     = 5;
            customizeBlindbox.starRates   = new uint256[](1);
            customizeBlindbox.starRates[0]= totalRate;
        }
        
        customizeBlindbox.starStepRates = new uint256[](customizeBlindbox.starRates.length);
        for(uint256 i = 0; i < customizeBlindbox.starRates.length; i++){
            customizeBlindbox.starStepRates[i] = customizeBlindbox.starStepRates[i];
            if(i > 0){
                customizeBlindbox.starStepRates[i] += customizeBlindbox.starStepRates[i - 1];
            }
        }
    }
    
    function generateBaseID(IHeroToken.HeroQuality quality)  internal returns(uint256){
        uint256 seed = computerSeed();
        uint256[] storage baseIds = qualityToBaseIds[quality];
        return baseIds[seed%baseIds.length];
    }
    
    function generateStarForCustomizeBlindbox(uint256[] memory starStepRates)  internal returns(uint256){
        uint256 seed = computerSeed()%(starStepRates[starStepRates.length - 1] + 1);
        for(uint256 i = 0; i < starStepRates.length; i++){
            if(starStepRates[i] >= seed){
                return i + 1;
            }
        }
        
        return starStepRates.length;
    }
    
    function generateStar(uint256[] storage starStepRates) internal returns(uint256){
        uint256 seed = computerSeed()%(starStepRates[starStepRates.length - 1] + 1);
        for(uint256 i = 0; i < starStepRates.length; i++){
            if(starStepRates[i] >= seed){
                return i + 1;
    ...
    function generateQuality(uint256[] storage qualityStepRates) internal returns(IHeroToken.HeroQuality){
        uint256 seed = computerSeed()%(qualityStepRates[qualityStepRates.length - 1] + 1);
        for(uint256 i = 0; i < qualityStepRates.length; i++){
            if(qualityStepRates[i] >= seed){
                return IHeroToken.HeroQuality(i);
            }
        }
    
        return IHeroToken.HeroQuality(qualityStepRates.length - 1);
    }
    
    function computerSeed() private returns (uint256) {
        seedFactor++;
        // from fomo3D
