...
*/
contract OreInstallmentLockerVault is BaseVault {
    using SafeMathUpgradeable for uint;
    using SafeBEP20 for IBEP20;

    struct UserInstallmentLockerInfo {
        uint period;
        uint boostRate;
        uint balance;
    ...
    uint[] public periods;
    mapping(uint => PeriodLockerInfo) public periodLockerInfos;

    mapping(address => UserInstallmentLockerInfo[]) public balances;

    event OreInstallmentLockerVaultDeposited(address user, uint period, uint amount);
    event OreInstallmentLockerVaultWithdrawn(address user, uint amount);
    event OreEmissionRateChanged(uint oldEmissionRate, uint newEmissionRate);
    event OreLockerInfosChanged(uint[] indexed periods, PeriodLockerInfo[] indexed periodLockerInfos);

    function initialize(address _chef, address _token, uint _oreTokenId, uint _emissionRate) public initializer {
        __OreInstallmentLockerVault_init(_chef, _token, _oreTokenId, _emissionRate);
    }

    function __OreInstallmentLockerVault_init(address _chef, address _token, uint _oreTokenId, uint _emissionRate) internal initializer {
        __Ownable_init_unchained();
        __Context_init_unchained();
        __Pausable_init_unchained();
        __ReentrancyGuard_init_unchained();

        __OreInstallmentLockerVault_init_unchained(_chef, _token, _oreTokenId, _emissionRate);
    }

    function __OreInstallmentLockerVault_init_unchained(address _chef, address _token, uint _oreTokenId, uint _emissionRate) internal initializer {
        setChef(_chef);
        setToken(_token);
    ...
    function setLockerPeroids(uint[] memory _periods, PeriodLockerInfo[] memory _periodLockerInfos) public onlyOwner {
        require(_periods.length > 0, "invalid periods");
        require(_periods.length == _periodLockerInfos.length, "periods and boostRates should have same length");
        delete periods;

        for (uint i=0; i<_periods.length; i++) {
            periodLockerInfos[_periods[i]] = _periodLockerInfos[i];

            periods.push(_periods[i]);
        }
        emit OreLockerInfosChanged(_periods, _periodLockerInfos);
    }
    ...
    function balanceOf(address _user) public view override returns (uint) {
        UserInstallmentLockerInfo[] storage lockerInfos = balances[_user];
        uint _balance = 0;
        for (uint i=0; i< lockerInfos.length; i++) {
            UserInstallmentLockerInfo storage lockerInfo = lockerInfos[i];
    ...
    function unlockedBalance() public view returns (uint) {
        UserInstallmentLockerInfo[] storage lockerInfos = balances[msg.sender];
        uint _balance = 0;
        for (uint i=0; i<lockerInfos.length; i++) {
            UserInstallmentLockerInfo storage lockerInfo = lockerInfos[i];
            _balance = _balance.add(_getCanWithdrawBalanceFromLockerInfo(lockerInfo));
        }
        return _balance;
    }
    ...
    function _deposit(uint periodIndex, uint _amount) private whenNotPaused {
        require(periods.length > periodIndex && periods[periodIndex] > 0 && _amount > 0, "invalid deposit parameters");

        IBEP20(token).safeTransferFrom(msg.sender, address(this), _amount);
        totalAmount = totalAmount.add(_amount);
        
        UserInstallmentLockerInfo memory newLocker = UserInstallmentLockerInfo(periods[periodIndex], 
                                                periodLockerInfos[periods[periodIndex]].lockerBooster,
                                                _amount,
                                                periodLockerInfos[periods[periodIndex]].installmentCount,
                                                _amount,
                                                block.timestamp);
        balances[msg.sender].push(newLocker);
        
        IEBChef(chef).vaultDeposited(msg.sender, _amount);
        IEBChef(chef).safeMint1155(msg.sender, oreTokenId, rewardOreAmount(_amount, periodLockerInfos[periods[periodIndex]].lockerBooster));

        emit OreInstallmentLockerVaultDeposited(msg.sender, periods[periodIndex], _amount);
    }

    //determine how many installment passed theoretically
    function _getCanWithdrawBalanceFromLockerInfo(UserInstallmentLockerInfo memory lockerInfo) private view returns (uint canWithdrawBalance) {
        if (block.timestamp >= lockerInfo.depositTimestamp && lockerInfo.balance > 0) {
            uint passedTime = block.timestamp.sub(lockerInfo.depositTimestamp);
            if (passedTime > lockerInfo.period) {
                passedTime = lockerInfo.period;
            }
            uint installmentPassedCount = passedTime.div(lockerInfo.period.div(lockerInfo.installmentCount));
            uint theoreticallyUnlockedBalance = lockerInfo.depositBalance.mul(installmentPassedCount).div(lockerInfo.installmentCount);
            //require(lockerInfo.balance == lockerInfo.depositBalance - theoreticallyUnlockedBalance, "remain balance should larger equal than theoreticallyUnlockedBalance");

            canWithdrawBalance = theoreticallyUnlockedBalance.sub(lockerInfo.depositBalance.sub(lockerInfo.balance));
        }
    }

    function _withdraw(uint _amount) private whenNotPaused {
        uint amountCanWithdraw = unlockedBalance();
        require(amountCanWithdraw > 0 && _amount <= amountCanWithdraw, "no token to withdraw");
        ...
        for (uint i=0; i<lockerInfos.length; i++) {
            ...
            if (block.timestamp >= lockerInfo.depositTimestamp) {
                uint canWithdrawBalance = _getCanWithdrawBalanceFromLockerInfo(lockerInfo);
                if (withdrawingAmount > canWithdrawBalance) {
                    withdrawingAmount = withdrawingAmount.sub(canWithdrawBalance);
                    lockerInfo.balance = lockerInfo.balance.sub(canWithdrawBalance);
                    require(lockerInfo.balance == 0, "single locker balance should be 0");
                } else {
        ...
        emit OreInstallmentLockerVaultWithdrawn(msg.sender, withdrawingAmountMark);
    }
}
