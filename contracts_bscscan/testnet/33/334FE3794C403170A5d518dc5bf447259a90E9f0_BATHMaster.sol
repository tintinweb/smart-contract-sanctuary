...
contract BATHMaster is Ownable, Withdrawable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    
    IBATH public immutable bath;
    IERC20 public busd;
    IERC20 public wwin;
    ...
    uint public wwinPermille = 100;
    uint public treasuryPermille = 7;
    uint public feePermille = 0;
    uint256 public bathPerUsd = 30;
    ...
    event BATHWithdrawn(uint256 amount);
    event SwapPathChanged(address[] swapPath);
    event BATHPermilleChanged(uint256 wwinPermille);
    ...
    constructor(IBATH _bath, IERC20 _busd, IERC20 _wwin, address _treasury, uint256 _maxStakeAmount) public {
        require(
            address(_bath) != address(0) &&
            address(_busd) != address(0) &&
            address(_wwin) != address(0) &&
        ...
        treasury = _treasury;
        swapPath = [address(busd), address(wwin)];
    ...
    function setwwinPermille(uint _wwinPermille) external onlyOwner {
        require(_wwinPermille <= 500, 'wwinPermille too high!');
        wwinPermille = _wwinPermille;
        
        emit BATHPermilleChanged(wwinPermille);
    }
    ...
    function stake(uint256 amount) external nonReentrant {
        ...
        uint256 wwinAmount = amount * wwinPermille / 1000;
        wwin.safeTransferFrom(msg.sender, address(this), wwinAmount);
        ...
        //     block.timestamp
        // );
        bath.mint(msg.sender, amount);
        
        emit Stake(msg.sender, amount);
    }
    
    function redeem(uint256 amount) external nonReentrant {
        // uint256 busdTransferAmount = amount * (1000 - wwinPermille - treasuryPermille) / 1000;
        amount = amount.mul(bathPerUsd);
        uint256 busdTreasuryAmount = amount * treasuryPermille / 1000;
        uint256 busdTransferAmount = amount * (1000 - treasuryPermille) / 1000;
        uint256 wwinTransferAmount = wwin.balanceOf(address(this)) * amount / bath.totalSupply();

        bath.burn(msg.sender, amount);
        busd.safeTransfer(treasury, busdTreasuryAmount);
        busd.safeTransfer(msg.sender, busdTransferAmount);
        // wex.safeTransfer(msg.sender, wwinTransferAmount);
        wwin.safeTransferFrom(msg.sender, address(this), wwinTransferAmount);
    ...
    function withdrawBATH(uint256 amount) external onlyWithdrawer {
        wwin.safeTransfer(msg.sender, amount);
        emit BATHWithdrawn(amount);
    }
}
