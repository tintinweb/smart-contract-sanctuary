...
import "./StakingPoolImplementation.sol";
...
contract StakingPoolImplementation is StakingPoolStorageStructure {
    ...
    function claimReward() external {
        ...
        if (isMatured) {
            if (usdPrizeAmount > 0) {
                if (wrappedTokenReward > 0) {

                    ClaimRewardLib.withdrawPrizeReward(predictions[_msgSender()]);

                    require(wrappedToken.transfer(_msgSender(), wrappedTokenReward), "0320");
                }
            }
        ...
        if (isMatured) {
            ...
            if (usdPrizeAmount > 0) {
                if (wrappedTokenReward > 0) {
                    ...
                    IndexedClaimRewardLib.withdrawIndexedPrizeReward(
                        predictions[_msgSender()], 
                        stakeIndex
                    );

                    require(wrappedToken.transfer(_msgSender(), wrappedTokenReward), "0330");
                }
            }
