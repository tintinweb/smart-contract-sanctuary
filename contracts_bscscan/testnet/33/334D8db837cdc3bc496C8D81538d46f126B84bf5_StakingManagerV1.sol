...
contract StakingManagerV1 is IStakingManagerV1, Governable, Pausable, IDCounter, ReentrancyGuard {
  ...
  function _getAvailableRewards() private view returns (uint256) {
    ...
    if (rewards > _maximumRewardsForDistribution && rewards < _waitingRewards) {
      return _maximumRewardsForDistribution;
    }

    if (rewards > _waitingRewards) {
      return _waitingRewards;
    }
  ...
  function getSplitStakingRewardsForAddress(address account) external view override returns (uint256 pending, uint256 claimed) {
    (,,uint256 soloPending,uint256 soloClaimed) = _staking[_soloStakingId].getStakingDataForAccount(account);
    (,,uint256 lpPending,uint256 lpClaimed) = _staking[_lpStakingId].getStakingDataForAccount(account);
...
contract StakingV1 is Authorizable, Pausable, ReentrancyGuard {
  /** libraries */
  using Address for address payable;
  ...
  function accuracy() external virtual view returns (uint256) {
    return _accuracy;
  }

  function setAccuracy(uint256 value) external virtual onlyAuthorized {
    _rewardsPerToken = _rewardsPerToken * value / _accuracy;
    _accuracy = value;
  }

  function setAutoClaimOnDeposit(bool value) external virtual onlyAuthorized {
    _autoClaimOnDeposit = value;
  }

  function setAutoClaimOnClaim(bool value) external virtual onlyAuthorized {
    _autoClaimOnClaim = value;
  }

  function getAutoClaimOptOut(address account) external virtual view returns (bool) {
    return _stakers[account].autoClaimOptOut;
  }

  function setAutoClaimOptOut(bool value) external virtual {
   ...
   */
  function removeLockDuration() external virtual onlyAuthorized {
    _lockDurationDays = 0;
  }

  function getPlaceInQueue(address account) external virtual view returns (uint256) {
  ...
  function autoClaimGasLimit() external virtual view returns (uint256) {
    return _autoClaimGasLimit;
  }

  function setAutoClaimGasLimit(uint256 value) external virtual onlyAuthorized {
  ...
  function getStakingData() external virtual view returns (
    address stakedToken,
    string memory name,
    uint8 decimals,
    uint256 totalStaked,
  ...
  function getStakingDataForAccount(address account) external virtual view returns (
    uint256 amount,
  ...
  function _pending(address account) internal virtual view returns (uint256) {
    ...
    return excluded > rewards ? 0 : rewards - excluded;
  }

  function pending(address account) external virtual view returns (uint256) {
  ...
  function claimFor(address account, bool revertOnFailure, bool doAutoClaim) external virtual nonReentrant {
    if (revertOnFailure)
  ...
  function getUnlockTime(address account) external virtual view returns (uint64) {
  ...
  function processAutoClaim() external virtual nonReentrant {
    _autoClaim();
  }
  ...
  function _getRewardsPerToken() internal virtual view returns (uint256) {
    ...
    if (currentBalance <= _lastBalance || _totalStaked == 0)
      return 0;

    return _rewardsPerToken + (((currentBalance - _lastBalance) * _accuracy) / _totalStaked);
  }
  ...
  function _updateRewards() internal virtual {
    ...
    if (currentBalance > _lastBalance && _totalStaked != 0)
      _rewardsPerToken += ((currentBalance - _lastBalance) * _accuracy) / _totalStaked;
