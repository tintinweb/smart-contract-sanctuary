...
abstract contract BEP20Fee is BEP20 {
    ...
    struct Colony {
         ...
         string _policy_link;
         address _mother_colony_address;
    }
    ...
    function registerColony(address colony_public_key, uint128 tx_tax, string memory colony_policy_link, bytes32 colony_policy_hash, address mother_colony_public_key) public onlyOwner {
        ...
        if (_colonies[mother_colony_public_key]._level == 0) {
            _colonies[colony_public_key]._mother_colony_address = address(0);
            ...
            if (_colonies[mother_colony_public_key]._level < _colonies[colony_public_key]._level || _colonies[colony_public_key]._level == 0) {
                _colonies[colony_public_key]._mother_colony_address = mother_colony_public_key;
    ...
    function getColonyInfo(address addr) public view returns(Colony memory){        
        return _colonies[addr];
    }
    ...
    function setColonyAddress(address addr,address colony) public onlyOwner {
        require(_colonies[addr]._level == 0, "Colony can't be a user");
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(colony_address == address(0)) {
            colony_address = _colonies[recipient]._mother_colony_address;
        }

        Colony memory colony = _colonies[colony_address];
        ...
        uint tx_tax_amount = amount * colony._transaction_tax / fee_decimals;     // Calculate transaction tax amount
        ...
        if( tx_tax_amount > 0 ){
            ...
            while( colony_address != address(0) && level++ <= 5 ){
                ...
                colony_address = colony._mother_colony_address;
