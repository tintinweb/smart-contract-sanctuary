...
// File: contracts/PayADADividendPayingToken.sol
...
contract PayADA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address private _marketingWallet = address(0x000000000000000000000000000000000000dEaD);
    address private _devWallet = address(0x9f3Fe74c4feD70badbbF78D01136Cbce8A9E142e);

    bool private swapping;
    bool public airdrop;

    PayADADividendTracker public dividendTracker;

    address public liquidityWallet;
    address public marketingWallet;
    address public devWallet = _devWallet;

    uint256 public maxBuyTranscationAmount = 1000000000000000 * (10**18); // 0.3% of total supply
    uint256 public maxSellTransactionAmount = 1000000000000000 * (10**18); // 0.3% of total supply
    ...
    constructor() public ERC20("PayADA", "PayADA") {
        uint256 _BNBRewardsFee = 9;
        uint256 _liquidityFee = 9;
    	...
    	dividendTracker = new PayADADividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PayADA: The dividend tracker already has that address");

        PayADADividendTracker newDividendTracker = PayADADividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "PayADA: The new dividend tracker must be owned by the PayADA token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PayADA: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PayADA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PayADA: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "PayADA: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PayADA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PayADA: Cannot update gasForProcessing to same value");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PayADA: Account is already the value of 'excluded'");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "PayADA: This account cannot send tokens until trading is enabled");
        }
...
contract PayADADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("PayADA_Dividend_Tracker", "PayADA_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "PayADA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "PayADA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PayADA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PayADA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PayADA_Dividend_Tracker: Cannot update claimWait to same value");
