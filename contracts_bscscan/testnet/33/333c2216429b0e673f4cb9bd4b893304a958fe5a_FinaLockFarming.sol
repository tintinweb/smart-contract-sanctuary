...
contract FinaLockFarming is Initializable, OwnableUpgradeable, FinaFarming {
    ...
    function initialize(iFina finaToken_, IERC20Upgradeable secondToken_, address devAddr_,
    ...
    function withdrawLP(uint _pid, uint _amount) external virtual override onlyEOA whenNotPaused {
        ...
        if(pending > 0 && block.timestamp >= user.averageDepositedTime + depositTier[3]) {
            //full rewards if after tier 4
            require(finaToken.transferFrom(address(this),_msgSender(), pending));
            secondRewardToken.safeTransferFrom(address(this), _msgSender(), pendingExtra);
        } else if(pending > 0 && block.timestamp >= user.averageDepositedTime + depositTier[2]){
            //tier 3 rewards if after tier 3
            finaToken.transferFrom(address(this), _msgSender(), pending * rewardRatioByTier[3] / 10000);
            secondRewardToken.safeTransferFrom(address(this), _msgSender(), pendingExtra * rewardRatioByTier[3] / 10000);
        } else if(pending > 0 && block.timestamp>= user.averageDepositedTime + depositTier[1]) {
            //tier 2 rewards if after tier 2
            finaToken.transferFrom(address(this),_msgSender(), pending * rewardRatioByTier[2] / 10000);
            secondRewardToken.safeTransferFrom(address(this), _msgSender(), pendingExtra * rewardRatioByTier[2] / 10000);
        } else if(pending > 0 && block.timestamp>= user.averageDepositedTime + depositTier[0]){
            //tier 1 rewards if after tier 1
            finaToken.transferFrom(address(this),_msgSender(), pending * rewardRatioByTier[1] / 10000);
            secondRewardToken.safeTransferFrom(address(this), _msgSender(), pendingExtra * rewardRatioByTier[1] / 10000);
        } else if(pending > 0 && block.timestamp < user.averageDepositedTime + depositTier[0]){
            //tier 0 rewards if before tier 1
            finaToken.transferFrom(address(this),_msgSender(), pending * rewardRatioByTier[0] / 10000);
            secondRewardToken.safeTransferFrom(address(this), _msgSender(), pendingExtra * rewardRatioByTier[0] / 10000);
        }
        if(_amount > 0) {
    ...
    function setDepositTier(uint index_, uint seconds_) onlyOwner external {
        require(index_ != 0, "The index is null");
        depositTier[index_] = seconds_ ;//uint in seconds
    }

    function setRatioByTier(uint index_, uint ratio_) onlyOwner external {
        require(index_ != 0, "The index is null");
        rewardRatioByTier[index_] = ratio_;
    }
    function foo() external {
        rewardRatioByTier[0] = 0;
    }
}
...
interface iFina{
    //remember to set this contract as a "MINTER_ROLE" from FinaToken contract.
    function mint(address who, uint amount) external returns (bool);
...
contract FinaFarming is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function __FinaFarming_init(iFina finaToken_, IERC20Upgradeable secondToken_, address devAddr_,
    ...
    function pendingLPReward(uint _pid, address _user) external view returns (uint[] memory pending) {
        LPPoolInfo storage lpPool = lpPoolInfo[_pid];
    ...
    function claimLPReward(uint _pid) external onlyEOA {
        //use depositLP(_pid, 0) to claim pending reward
    }
    ...
    function setFinaAddress(iFina token_) onlyOwner external {
        require(token_ != iFina(address(0)), "The address of token is null");
        finaToken = token_;
    }
