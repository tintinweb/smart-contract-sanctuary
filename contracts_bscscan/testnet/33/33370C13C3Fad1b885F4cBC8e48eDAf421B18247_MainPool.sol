...
contract MainPool is Ownable {
    ...
    function transferToPool(
        ...
        if (_tokenAddress == IDOLAddress) {
            ...
            // fee
            feeAmount = _amount.mul(fee).div(100); 
            token.transferFrom(_userAddress, address(this), _amount);
            token.transferFrom(_userAddress, feeAddress, feeAmount);
            finalAmount = _amount.sub(feeAmount);
        }
        
        emit TransferToPool(_userAddress, _creatorAddress, _tokenAddress, finalAmount, feeAmount);
        ...
        for(uint256 i = 0; i < tcArr.length; i++ ) {
            ...
            if (_tokenAddress == tc.tokenAddress) {
                tc.amount = tc.amount.add(finalAmount);
                tokenExist = true;
            }
        }
        
        if (!tokenExist) {
            creatorBal[_creatorAddress].push(tokenClaim(_tokenAddress, finalAmount, block.number));
        }
    }
