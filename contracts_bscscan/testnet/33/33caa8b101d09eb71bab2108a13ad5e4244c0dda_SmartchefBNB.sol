...
contract SmartchefBNB is Ownable {
    using SafeMath for uint256;
    ...
    function deposit() public payable {
        ...
        if (user.amount > 0) {
            ...
            if(pendingRewardToken > 0 && rewardToken.balanceOf(address(this)) > pendingRewardToken) {
                // compound
                pendingRewardToken = compound(pendingRewardToken);
                safeTransferBNB(address(msg.sender), pendingRewardToken);
            }
        }
    ...
    function emergencyWithdraw() public {
        ...
        IWBNB(WBNB).withdraw(user.amount);
        uint256 amount = takeWithDrawFee(user.amount);
        safeTransferBNB(address(msg.sender), amount);
    ...
    function takeDepositFee(uint256 _amount) internal returns (uint256) {
        ...
        assert(IWBNB(WBNB).transfer(feeAddress1,feeAmount.div(2)));
        assert(IWBNB(WBNB).transfer(feeAddress2,feeAmount.div(2)));
    ...
    function takeWithDrawFee(uint256 _amount) internal returns (uint256) {
        ...
        safeTransferBNB(feeAddress1, feeAmount.div(2));
        safeTransferBNB(feeAddress2, feeAmount.div(2));
