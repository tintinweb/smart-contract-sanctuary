...
contract FlokiDAOToken is Divine {
  ...
  using SafeMath for uint256;
    ...
    constructor() Divine("FlokiDAOToken", "FLOK", 9) {
    }

    function whitelistState() public virtual returns(bool){
      return enforceWhitelist;
    }

    function toggleEnforceWhitelist_OVAULT() external onlyVault {
        enforceWhitelist
            ? enforceWhitelist = false
            : enforceWhitelist = true;
    }

    function toggleWhitelisted_OVAULT(address holder) external onlyVault {
        whitelist[holder]
