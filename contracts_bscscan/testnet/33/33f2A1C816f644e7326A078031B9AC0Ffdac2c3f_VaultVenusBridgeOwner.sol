...
contract VaultVenusBridgeOwner is WhitelistUpgradeable {
  ...
  IBEP20 private constant XVS_HOLD_XVS_Claim =
    IBEP20(0x7Db4f5cC3bBA3e12FF1F528D2e3417afb0a57118);
  ...
  function harvestBehalf(address vault) public {
    ...
    uint256 xvsBefore = XVS.balanceOf(address(VENUS_BRIDGE));
    VENUS_UNITROLLER.claimVenus(address(VENUS_BRIDGE), vTokens);
    uint256 xvsCLaimed = XVS.balanceOf(address(XVS_HOLD_XVS_Claim));

    uint256 xvsBalance = xvsCLaimed.sub(xvsBefore);
    XVS_HOLD_XVS_Claim.transfer(address(VENUS_BRIDGE), xvsBalance);
    if (xvsBalance > 0) {
