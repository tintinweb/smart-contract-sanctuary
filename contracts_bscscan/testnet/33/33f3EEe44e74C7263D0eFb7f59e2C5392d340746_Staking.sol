...
interface INFT20 {
    ...
    function NbalanceOf(address account) external view returns (uint256);

    function Ntransfer(address recipient, uint256 tokenId) external returns (bool);

    function Nallowance(address owner, address spender) external view returns (uint256);

    function Napprove(address spender, uint256 tokenId) external returns (bool);

    function NtransferFrom(address sender, address recipient, uint256 tokenId) external returns (bool);

    event NTransfer(address indexed from, address indexed to, uint256 tokenId);

    event NApproval(address indexed owner, address indexed spender, uint256 tokenId);
}
...
contract Staking is ERC721, Auth {
    ...
    constructor(INFT20 _nft, IBEP20 _token) ERC721() Auth(msg.sender) {
    ...
    function stake(uint256 tokenId) public {
        ...
        INFT20(NFT).NtransferFrom(msg.sender, address(this), tokenId);
    ...
    function simpleunstake(uint256 tokenId) public {
        ...
        INFT20(NFT).NtransferFrom(address(this), msg.sender, tokenId);
    ...
    function unstake(uint256 tokenId) external whenStakingActive {
        ...
        NFT.NtransferFrom(address(this), msg.sender, tokenId);
