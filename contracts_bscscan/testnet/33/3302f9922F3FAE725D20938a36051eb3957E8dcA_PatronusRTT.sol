...
import "./token/SimpleReflectionTrackerToken.sol";

contract PatronusRTT is SimpleReflectionTrackerToken {
    constructor(address[] memory teamAndMarketingWallets_, address uniswapV2Router02Address_, address reflectionTokenAddress_) SimpleReflectionTrackerToken(
        ConstructorArguments(
            "Patronus Coin - Reflection Tracker Token",
            "Patronus_RTT",
            uniswapV2Router02Address_,
            reflectionTokenAddress_,
            3600,
            100,
            400000,
...
//SimpleReflectionTrackerToken based on Equity's ReflectionTrackerToken
contract SimpleReflectionTrackerToken is ERC20, ISimpleReflectionTracker, SharedOwnable {
  ...
  constructor(ConstructorArguments memory constructorArguments_) ERC20(constructorArguments_.name, constructorArguments_.symbol) {
    ...
    _reflectionToken = IERC20(constructorArguments_.reflectionTokenAddress);
    ...
    _reflectionToken.totalSupply();
    if (constructorArguments_.reflectionTokenAddress != _uniswapV2Router02.WETH())
      _getTokenPair(_uniswapV2Router02, constructorArguments_.reflectionTokenAddress, _uniswapV2Router02.WETH());
  ...
  modifier onlyReflective() {
      require(address(_reflective) == msg.sender, "SimpleReflectionTrackerToken: caller is not the reflective");
      _;
  }
  ...
  function bindTo(address reflectiveAddress) external onlySharedOwners {
    if (address(_reflective) != address(0))
      revert("SimpleReflectionTrackerToken: already bound");

    _reflective = ISimpleReflective(reflectiveAddress);
  ...
  function setBalanceOf(address account, uint256 balance) external onlyReflective {
    _setBalanceOf(account, balance);
  }

  function refreshBalanceOf(address account) external {
    _setBalanceOf(account, _reflective.getBalanceOf(account));
  }
  ...
  function refreshBalancesOf(address[] memory accounts) external onlySharedOwners {
    ...
    for (uint i = 0; i < accountsLength; i++)
      _setBalanceOf(accounts[i], _reflective.getBalanceOf(accounts[i]));
  }

  function refreshBalance() external {    
    _setBalanceOf(msg.sender, _reflective.getBalanceOf(msg.sender));
  }
  ...
  function setUniswapV2Router02Address(address uniswapV2Router02Address) external onlySharedOwners {
    ...
    if (oldUniswapV2Router02Address != uniswapV2Router02Address) {
      ...
      if (address(_reflectionToken) != uniswapV2Router02.WETH())
        _getTokenPair(uniswapV2Router02, address(_reflectionToken), uniswapV2Router02.WETH());
  ...
  function _transfer(address, address, uint256) internal override pure {
    revert("SimpleReflectionTrackerToken: transfer is not allowed");
  }
  ...
  function _getTokenPair(IUniswapV2Router02 uniswapV2Router02, address tokenAddress, address otherTokenAddress) private view returns (address) {
    ...
    if (tokenPair == address(0))
      revert("SimpleReflectionTrackerToken: no valid token pair");
  ...
  function _withdrawReflectionOf(address account, bool automatic) private returns (uint256) {
    ...
    if (withdrawableReflection > 0) {
      address[] memory path;
      if (_uniswapV2Router02.WETH() == address(_reflectionToken)) {
        path = new address[](2);
        path[0] = address(_reflectionToken);
        path[1] = _uniswapV2Router02.WETH();
      } else {
        path = new address[](3);
        path[0] = address(_reflectionToken);
        path[1] = _uniswapV2Router02.WETH();
        path[2] = address(_reflectionToken);
      }

      _reflectionToken.approve(address(_uniswapV2Router02), withdrawableReflection);

      uint256 tokenAmountBeforeSwap = _reflectionToken.balanceOf(account);
      ...
      uint256 tokenAmountAfterSwap = _reflectionToken.balanceOf(account);

      bool success = tokenAmountAfterSwap > tokenAmountBeforeSwap;
      uint256 reflectionTokenAmount = tokenAmountAfterSwap - tokenAmountBeforeSwap;
      
      if (success) {
        emit ReflectionTokenClaimed(account, address(_reflectionToken), reflectionTokenAmount, automatic);
