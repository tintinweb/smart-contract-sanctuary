...
contract TestDoge is Context, IERC20, Ownable {
    ...
    uint256 private _maxTxAmount = 3000000 * 10**6 * 10**9;           //3 Trillion
    uint256 private _maxTxDivisor = 3333;                             //3 Trillion at launch
    uint256 private _maxWalletAmount = 1000000000 * 10**6 * 10**9;    //1 Quadrillion
    uint256 private _maxWalletDivisor = 100;                          //1 Quadrillion at launch
    ...
    function _transfer(
        ...
        if (justLaunched){
            if (block.timestamp <= launchTime.add(600)){          // minutes [0,10]
                maxTxAmount = 20000000000 * 10 ** 9;              // 20 billion max tx
                maxWalletAmount = _tTotal.div(10000);             // 0.01% max wallet, 100 billion
            }
            else if (block.timestamp <= launchTime.add(1200)){     // minutes (10,20]
                maxTxAmount = 60000000000 * 10 ** 9;              // 60 billion max tx
                maxWalletAmount = _tTotal.div(3333);              // 0.03% max wallet, 300 billion
            }
            else if (block.timestamp <= launchTime.add(3600)){    // minutes (20,60]
        ...
        _isExcludedFromFee[from] || _isExcludedFromFee[to]) && from != _DXLockerAddress){
            takeFee = false;
        }
            ...
            if(to == uniswapV2Pair){
                ...
                if (_burnModeActive){
                    ...
                    if (address(this).balance > memeBurnCost){
    ...
    function swapTokens(uint256 tokenAmountCandidate) private lockTheSwap {
        ...
        if (initialBalance < memeBurnCost.mul(_memeBurnBuffer)){
            ...
            uint256 tokensToLP = tokensToSwap.mul(_liquidityFee).div(_LPMarketingFee).div(2);
