...
// File: contracts\ChadFinanceToken.sol

pragma solidity 0.6.12;


// SpiritToken with Governance.
contract ChadFinanceToken is BEP20('ChadFinanceToken', 'CHAD') {
    ...
    */
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "CHAD::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "CHAD::_writeCheckpoint: block number exceeds 32 bits");
...
// File: contracts\ChadFinanceMasterChad.sol
...
// MasterChad is the master of CHAD. He can make CHAD and he is a fair guy.
//
...
contract MasterChad is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of CHAD
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock;  // Last block number that CHAD distribution occurs.
        uint256 accChadPerShare;   // Accumulated Chads per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The CHAD TOKEN!
    ChadFinanceToken public CHAD;
    // Dev address.
    ...
    // Update the given pool's CHAD allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending CHAD on frontend.
    function pendingChad(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accChadPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // Deposit LP tokens to MasterChef for CHAD allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accChadPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
    ...
    function updateEmissionRate(uint256 _ChadPerBlock) public onlyOwner {
        massUpdatePools();
        ChadPerBlock = _ChadPerBlock;
    }
}