...
contract Lottery is Ownable {
	...
	uint public entryFee; // 0.05 BNB 5000000000000000 
	uint public ownerCut; // 0.01 BNB 1000000000000000 
	...
	event NumberDrawn(address winnerAddress, uint winningNumber);
    ...
    //constructor
	constructor (uint _entryFee, uint _ownerCut, uint _maxNumberOfEntries, address _bank) Ownable() {
		require(_entryFee > 0, "Entry fee must be greater than 0");
		require(_ownerCut < _entryFee, "Entry fee must be greater than owner cut");
		require(_bank != address(0), "Random number generator must be valid address");
		entryFee = _entryFee;
		ownerCut = _ownerCut;
		_bankAddress = _bank;
    ...
    function submitNumber(uint _number) public payable isState(LotteryState.Open) {
		...
		// players.push(Player({location: msg.sender, number:_number}));
		players.push(msg.sender);
		playerNumbers.push(_number);
		
		payable(_bankAddress).transfer(ownerCut);
		emit NewEntry(msg.sender, _number);
		if(players.length > maxNumberOfEntries){
		    _drawNumber();
		}
	}

	function _drawNumber() private isState(LotteryState.Open) {
		_changeState(LotteryState.Closed);
		//
		uint winningNumber = _pickWinner();
		address winningAddress = players[winningNumber];
		uint playerNumber = playerNumbers[winningNumber];
		
		winners.push(Player({location: winningAddress, number:playerNumber}));
		
		emit NumberDrawn(winningAddress, winningNumber);
		_payout(winningAddress);
		_resetLottery();
    ...
    function random() private view returns (uint) {
        ...
        for(uint i = 0; i < playerNumbers.length; i++){
            salt += playerNumbers[i] * i;
        }
        return uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, salt)));
    }
    
    function _pickWinner() private view returns (uint){
        return random().mod(players.length);
    }
    
    function _resetLottery() private{
        players = new address[](0);
        playerNumbers = new uint[](0);
    }
    
    function showWinners() public view returns ( address[] memory, uint[] memory){
        address[] memory addrs = new address[](winners.length);
        uint[]  memory numbers = new uint[](winners.length);
        
        for (uint i = 0; i < winners.length; i++) {
            Player storage person = winners[i];
            addrs[i] = person.location;
            numbers[i] = person.number;
        }
        
        return (addrs, numbers);
    }
    
    function setMaxNumberOfEntries(uint _maxNumberOfEntries) public onlyOwner{
        maxNumberOfEntries = _maxNumberOfEntries;
    }
    ...
    function setOwnerCut(uint _ownerCut) public onlyOwner{
        ownerCut = _ownerCut;
    }
