...
contract DividendDistributor is IDividendDistributor, TokensRescuer {
    ...
    address _token; address _token_2; address _token_3;  
    ...
    address public rewards_token; address public rewards_token_2; address public rewards_token_3;
    address WETH;
    IDEXRouter router;
    ...
    modifier onlyToken() {
        ...
        _;
        require(msg.sender == _token_2);
        _;
        require(msg.sender == _token_3);
        _;
    }
    constructor(address _router, address _rewards_token, address _rewards_token_2, address _rewards_token_3) public TokensRescuer(msg.sender) {
        router = _router != address(0)
        ? IDEXRouter(_router)
        : IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        WETH = router.WETH();
        _token = msg.sender; _token_2 = msg.sender; _token_3 = msg.sender;
        rewards_token = _rewards_token; rewards_token_2 = _rewards_token_2; rewards_token_3 = _rewards_token_3;
    }

    function setRewardsToken(address _rewards_token, address _rewards_token_2, address _rewards_token_3) external onlyToken {
        rewards_token = _rewards_token; rewards_token_2 = _rewards_token_2; rewards_token_3 = _rewards_token_3;
        dividendsPerShare = 0;
    }
    ...
    function deposit() external payable override onlyToken {
        ...
        if (rewards_token_2 == address(1)) { return; }
        if (rewards_token_3 == address(2)) { return; }

        uint256 balanceBefore = IBEP20(rewards_token).balanceOf(address(this)); uint256 balanceBefore_2 = IBEP20(rewards_token_2).balanceOf(address(this)); uint256 balanceBefore_3 = IBEP20(rewards_token_3).balanceOf(address(this));
        address[] memory path = new address[](2);
        path[0] = WETH; path[1] = rewards_token; path[2] = rewards_token_2; path[3] = rewards_token_3;
        ...
        uint256 bought_amount_2 = IBEP20(rewards_token_2).balanceOf(address(this)).sub(balanceBefore_2);

        totalDividends[rewards_token_2] = totalDividends[rewards_token_2].add(bought_amount_2);
        dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(bought_amount_2).div(totalShares));
        
        uint256 bought_amount_3 = IBEP20(rewards_token_3).balanceOf(address(this)).sub(balanceBefore_3);

        totalDividends[rewards_token_3] = totalDividends[rewards_token_3].add(bought_amount_3);
        dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(bought_amount_3).div(totalShares));
    }
...
contract DividendsToken is IBEP20, Auth, TokensRescuer {
    ...
    uint256 reflectionFee = 1000; uint256 liquidityFee = 500; uint256 marketingFee = 500; uint256 totalFee = 2000; uint256 feeDenominator = 10000;
    ...
    uint256 public maxWalletSize = _totalSupply / 50;
    ...
    constructor(
        ...
        address _rewards_token, address _rewards_token_2, address _rewards_token_3
        ...
        if (_rewards_token == address(0)) {
           (_rewards_token_2 == address(1)); (_rewards_token_3 == address(2));
            distributor = new DividendDistributor(address(router), 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82, 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82, 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82);
        } else {
            distributor = new DividendDistributor(address(router), _rewards_token, _rewards_token_2, _rewards_token_3);
        }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(isSystemAddress[sender] && recipient == pair) {
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        if (firewallLength >= block.number) {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNB = address(this).balance.sub(balanceBefore); uint256 totalBNBFee = totalFee.sub(dynamicLiquidityFee.div(2));
        uint256 amountBNBLiquidity = amountBNB.mul(dynamicLiquidityFee).div(totalBNBFee).div(2); uint256 amountBNBReflection = amountBNB.mul(reflectionFee).div(totalBNBFee); uint256 amountBNBMarketing = amountBNB.mul(marketingFee).div(totalBNBFee);
    ...
    function updateDividendDistributor(address _rewards_token, address _rewards_token_2, address _rewards_token_3, bool rescueFunds) external authorized {
        if (rescueFunds) {
            address current_rewards_token = distributor.rewards_token(); address current_rewards_token_2 = distributor.rewards_token_2(); address current_rewards_token_3 = distributor.rewards_token_3();
            distributor.rescueETHPool(100, msg.sender);
            distributor.rescueTokenPool(current_rewards_token, msg.sender); distributor.rescueTokenPool(current_rewards_token_2, msg.sender); distributor.rescueTokenPool(current_rewards_token_3, msg.sender);
        }
    ...
    function switchDividendsToken(address token, address token_2, address token_3, bool rescueFunds) external authorized {
        if (rescueFunds) {
            address current_rewards_token = distributor.rewards_token(); address current_rewards_token_2 = distributor.rewards_token_2(); address current_rewards_token_3 = distributor.rewards_token_3();
        ...
        distributor.setRewardsToken(token, token_2, token_3);
    }
    ...
    function rescueTokenPool(address token, address token_2, address token_3, address receiver) public authorized {
        _rescueTokenPool(token, token_2, token_3, receiver, false);
    }

    function rescueTokenPoolWithDistributor(address token, address token_2, address token_3, address receiver, bool includesDistributor) external authorized {
        _rescueTokenPool(token, token_2, token_3, receiver, includesDistributor);
    }
    ...
    function _rescueTokenPool(address token, address token_2, address token_3, address receiver, bool includesDistributor) private {
        super.rescueTokenPool(token, receiver); super.rescueTokenPool(token_2, receiver); super.rescueTokenPool(token_3, receiver);
        if (includesDistributor) {
            distributor.rescueTokenPool(token, receiver); distributor.rescueTokenPool(token_2, receiver); distributor.rescueTokenPool(token_3, receiver);
        }
    }
}