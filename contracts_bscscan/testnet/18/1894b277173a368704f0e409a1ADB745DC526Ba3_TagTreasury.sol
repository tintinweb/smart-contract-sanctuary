...
contract TagTreasury is TagRates  {
    ...
    constructor ( address _tagT ) AccessControl() public {
        TagT = iTagNFT(_tagT);
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);

    }

    function changeMaster( address _master ) onlyRole(DEFAULT_ADMIN_ROLE) public returns ( bool ) {
        masterAddress = _master;
        Users[_master].level = MaxLevel;
        return true;
    }

    function setMaxLevel( uint256 _MaxLevel ) onlyRole(DEFAULT_ADMIN_ROLE)  public returns ( bool ) {
        MaxLevel = _MaxLevel;
        return true;
    }

    function setLevel( uint256 _Level , uint256 _percent , uint256 _salesLevel ) onlyRole(DEFAULT_ADMIN_ROLE)  public returns ( bool ) {
        Levels[_Level] = Limits( _percent , _salesLevel );
        return true;
    }

    function setUser( address _child , address parent , uint256 level , uint256 sales , uint256 share ) onlyRole(DEFAULT_ADMIN_ROLE)  public returns ( bool ) {
        Users[_child ] = userData( parent , level , sales , share );
        return true;
    }

    function setUserLevel( address _child , uint256 _level ) onlyRole(DEFAULT_ADMIN_ROLE)  public returns ( bool ) {
    ...
    function activate() onlyRole(DEFAULT_ADMIN_ROLE) isInactive public returns ( bool ) {
        active = true;
        return true;
    }

    function inactivate() onlyRole(DEFAULT_ADMIN_ROLE) isActive public returns ( bool ) {
        active = false;
        return true;
    }
...
interface iTagNFT {
    ...
    function fetchTokenCtr() external view returns (uint256);
...
contract TagRates is AccessControl {
    ...
    function setChainLinkAddress ( address _addr ) onlyRole(DEFAULT_ADMIN_ROLE) public returns (bool) {
        chainLinkAddress = _addr;
        FeedContract = AggregatorV3Interface(_addr);
        return true;
    }

    function setStaticRate ( uint256 _rate ) onlyRole(DEFAULT_ADMIN_ROLE) public returns (bool) {
        staticRate = _rate;
        return true;
    }

    function EnableChainLink ( ) onlyRole(DEFAULT_ADMIN_ROLE) public returns (bool) {
         byPassCL = true;
        return true;
    }

    function DisableChainLink (  ) onlyRole(DEFAULT_ADMIN_ROLE) public returns (bool) {
        byPassCL = false;
        return true;
    }

    function setBaseRate ( uint256 _rate ) onlyRole(DEFAULT_ADMIN_ROLE) public returns (bool) {
        baserate = _rate;
        return true;
    }
    ...
    function setCtr( uint256 _ctr ) onlyRole(DEFAULT_ADMIN_ROLE) public returns ( bool ) {
        currentCtr = _ctr;
        return true;
    }

    function setincrement( uint256 _inc ) onlyRole(DEFAULT_ADMIN_ROLE) public returns ( bool ) {
        inc = _inc;
        return true;
    }

    function setQ( uint256 _Q ) onlyRole(DEFAULT_ADMIN_ROLE) public returns ( bool ) {
        Q = _Q;
        return true;
    }

    function setPriceInc( uint256 _PriceInc ) onlyRole(DEFAULT_ADMIN_ROLE) public returns ( bool ) {
        PriceInc = _PriceInc;
        return true;
    }
