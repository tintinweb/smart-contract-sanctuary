...
contract SousChef {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;   // How many BONEKING tokens the user has provided.
        uint256 rewardDebt;  // Reward debt. See explanation below.
        uint256 rewardPending;
        //
        // We do some fancy math here. Basically, any point in time, the amount of BONEKINGs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accRewardPerShare) - user.rewardDebt + user.rewardPending
        //
        // Whenever a user deposits or withdraws BONEKING tokens to a pool. Here's what happens:
    ...
    // The BONEKING TOKEN!
    IBEP20 public boneking;
    // rewards created per block.
    uint256 public rewardPerBlock;

    // Info.
    PoolInfo public poolInfo;
    // Info of each user that stakes BoneKing tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = boneking.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 bonekingSupply = boneking.balanceOf(address(this));
        if (bonekingSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(bonekingSupply));
        poolInfo.lastRewardBlock = block.number;
    }


    // Deposit BoneKing tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        boneking.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw BoneKing tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        boneking.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        boneking.safeTransfer(address(msg.sender), user.amount);
