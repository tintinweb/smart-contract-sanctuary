...
contract TESTCAKE2 is ERC20, Ownable {
    ...
    RewManagerInterface public MDIV = RewManagerInterface(0xf03375667E569f03870FC71e377b25a10898267A); // TODO
    address public CAKE = address(0x9BAC35cccb8A7de5fDa6425B6B05e13604a378c2);

    uint256 public swapTokensAtAmount = 1000000 * (10**18);
    
    mapping(address => bool) public _isBlacklisted;

    uint256 private _cakeBuyFee = 6;
    uint256 private _devBuyFee = 1;
    uint256 private _marketingBuyFee = 3;
    
    uint256 private _minRewSellFee = 6;
    uint256 private _maxRewSellFee = 40;
    uint256 private _devSellFee = 3;
    uint256 private _marketingSellFee = 5;


    uint256 private _cakeRewardsBalance = 0;
    uint256 private _rewRewardsBalance = 0;
    uint256 private _marketingFeeBalance = 0;
    uint256 private _devFeeBalance = 0;

    address payable public _marketingWalletAddress = 0x4b24df2fbC81C437E4F6732edD9558DA8ED816a2; // TODO
    address payable public _devWalletAddress = 0xD0CEA25E0573130f90B045261580F17b1C9c7984; // TODO
    ...
    event TransferInitDebug(address from, address to,
                           uint256 amount,
                           bool ammFrom, bool ammTo, bool swapping);
                           
    event TransferTakingFee(bool takeFee);
    event SellFeeDebug(uint256 rewSellFee, uint256 daysSinceLastBuy, uint256 feeReduction);

    constructor() public ERC20("TEST CAKE 2", "TESTCAKE2") {

    	rewDividendTracker = new RewDividendTracker(MDIV);
    	cakeDividendTracker = new CakeDividendTracker(CAKE);

    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);  // TODO check this
    ...
    function updateRewDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(rewDividendTracker), "TESTCAKE: The RewDoge dividend tracker already has that address");

        RewDividendTracker newDividendTracker = RewDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TESTCAKE: The new dividend tracker must be owned by the TESTCAKE token contract");
    ...
    function updateCakeDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(cakeDividendTracker), "TESTCAKE: The Cake dividend tracker already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TESTCAKE: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TESTCAKE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TESTCAKE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TESTCAKE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 100000 && newValue <= 800000, "TESTCAKE: gasForProcessing must be between 100,000 and 800,000");
        require(newValue != gasForProcessing, "TESTCAKE: Cannot update gasForProcessing to same value");
        emit GasForProcessingUpdated(newValue, gasForProcessing);
        gasForProcessing = newValue;
    }

    function updateRewClaimWait(uint256 claimWait) external onlyOwner {
        rewDividendTracker.updateClaimWait(claimWait);
    }

    function updateCakeClaimWait(uint256 claimWait) external onlyOwner {
        cakeDividendTracker.updateClaimWait(claimWait);
    }
    
    function getRewClaimWait() external view returns(uint256) {
        return rewDividendTracker.claimWait();
    }
    ...
    function _transfer(
        ...
        emit TransferInitDebug(from,to,amount,automatedMarketMakerPairs[from],automatedMarketMakerPairs[to],swapping);
        ...
        // ********************************************************************************
        // This logic transfers out the tokens sitting in the contract, which were taken
        // to pay for dividends/fees from previous transactions.  Tokens taken for dividends
        // are converted to their respective currencies and sent to the appropriate dividend
        // tracker to be distributed to holders later.  Tokens taken for fee are converted to 
        // ETH and sent to the dev/marketing wallets.
        if( !swapping &&
            ...
            if(_devFeeBalance >= swapTokensAtAmount) {
        ...
        // ********************************************************************************
        // This this logic takes dividend/fee tokens from the current transaction and puts it
        // in the contract
        ...
        emit TransferTakingFee(takeFee);
        if(takeFee) {
            uint256 totalFeeTokens = 0;
            if(automatedMarketMakerPairs[to]) {
                // Sell fees
                uint256 rewSellFee = _maxRewSellFee;

                uint256 daysSinceLastBuy = block.timestamp.sub(lastBuy[from]).div(10 minutes);
                uint256 feeReduction = daysSinceLastBuy.mul(4);
                
                // This math avoids unsigned underflow and provides a floor of _minRewSellFee
                if(feeReduction >= rewSellFee.sub(_minRewSellFee)) {
                    rewSellFee = _minRewSellFee;
                } else {
                    rewSellFee = rewSellFee.sub(feeReduction);
                }
                
                emit SellFeeDebug(rewSellFee,daysSinceLastBuy,feeReduction);
                
                totalFeeTokens = _takeFee(from, amount, 0, _minRewSellFee, _marketingSellFee, _devSellFee);
            } else {
                // Buy fees
        ...
        // ********************************************************************************
        // This does the actual requested transfer, updates the dividend trackers' record
        // of who holds tokens, and resets the sell tax cooldown on the person who
        // just received tokens.
        ...
        // ********************************************************************************
        // Finally, we trigger the dividend contracts to pay out some of their accumulated
        // dividends.
    ...
    function swapAndSendToFee(uint256 marketingTokens, uint256 devTokens) private  {
        // The marketing and dev wallets don't get identical amounts, so we need to do
        // a bit of math here.
        //
        // We first convert all the marketing and dev tokens to ETH, then we compute the ratio 
        // of that ETH that should go to marketing, send it, and then send the balance to dev.
        //
        // It would be a little simpler to convert the tokens separately, but we'd pay
        // more in fees/gas and would be doing two sells of our token instead of one.
        uint256 totalFeeTokens = marketingTokens.add(devTokens);
        swapTokensForEth(totalFeeTokens);
        uint256 totalEth = address(this).balance;
        uint256 marketingEth = totalEth.mul(marketingTokens).div(totalFeeTokens);
    ...
    function swapAndSendCakeDividends(uint256 tokens) private{
        ...
        if (success) {
            cakeDividendTracker.distributeDividends(dividends);
...
contract RewDividendTracker is Ownable, ERC20 {
    ...
    constructor(RewManagerInterface dw) public ERC20("TESTCAKE_Rew_Dividend_Tracker", "TESTCAKE_Rew_Dividend_Tracker") {
        MDIV = dw;
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }
    
    // Only the owner can transfer these tokens, since they are just for internal
    // TestCake accounting.
    function transfer(address, uint256) public override returns(bool) {
        require(false, "TESTCAKE_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TESTCAKE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TESTCAKE_Dividend_Tracker: Cannot update claimWait to same value");
...
// Just keeping this logic from BABYCAKE for the Cake dividend tracker.  It could be cleaned up.
contract CakeDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(address token) public DividendPayingToken("TESTCAKE_Dividend_Tracker", "TESTCAKE_Dividend_Tracker", token) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }
    ...
    function withdrawDividend() public override {
        require(false, "TESTCAKE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TESTCAKE contract.");
    }
