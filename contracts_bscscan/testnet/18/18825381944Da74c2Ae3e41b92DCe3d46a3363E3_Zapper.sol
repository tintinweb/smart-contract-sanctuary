...
contract Zapper is Ownable, IZapper {
	...
	constructor(
		...
		IERC20(_BTD).safeApprove(address(ROUTER), type(uint128).max);
		IERC20(_BTS).safeApprove(address(ROUTER), type(uint128).max);
		IERC20(_BUSD).safeApprove(address(ROUTER), type(uint128).max);

		//approve to breakLP
		IERC20(_BTDBUSD).safeApprove(address(ROUTER), type(uint128).max);
		IERC20(_BTSBUSD).safeApprove(address(ROUTER), type(uint128).max);
	...
	function zapBNBToLP(address _to) external payable override {
	...
	function breakLP(address _from, uint256 amount) external override {
		IERC20(_from).safeTransferFrom(msg.sender, address(this), amount);
	...
	function _sendDustBack(address _to) internal {
		IPancakePair pair = IPancakePair(_to);
		address token0 = pair.token0();
		address token1 = pair.token1();
