...
contract LazerMerchantWalletController is Ownable, ILazerStorageSchema {
    ...
    function setPrecision(uint256 _precision) public {
        precision = _precision;
    }

    function setPercentValue(uint256 _percentValue) public {
    ...
    function createMerchantsWallet(uint256 _merchantsID, uint256 createdAt) external returns (address) {
        ERC20Wallet wallet = new ERC20Wallet(owner, WalletAdmin);
        storageContract.createWalletMapping(_merchantsID, address(wallet), createdAt);
        storageContract.createWalletMappingToWalletID(_merchantsID, address(wallet), createdAt);
    ...
    function withdrawFunds(
        ...
        require(wallet.tokenBalanceOf(tokenAddress) > 0, "MerchantWallet: Wallet is empty");
    ...
    function stakeFundsToProtocol(
        ...
        require(wallet.tokenBalanceOf(token) > amount, "LazerMerchantWallet: Wallet does not have the amount");
        ...
        wallet.transferFrom(token, address(this), amount);

        IVToken _vToken = IVToken(vToken);

        IERC20 _token = IERC20(token);

        _token.safeApprove(address(_vToken), amount);
        ...
        _vToken.transfer(address(wallet), _vToken.balanceOf(address(this)));

        //wallet balance after stake
        uint256 walletBalance = wallet.tokenBalanceOf(token);

        //update the staking record
        StakingRecord memory stakeRecord = _updateStakeRecordAfterDeposit(
            walletId, vToken, amount, _vToken.balanceOf(address(this)), merchantId);

        bool exists = storageContract.doesStakeRecordExist(merchantId);
        ...
        if (exists) _updateStakeRecord(stakeRecord);
            ...
            storageContract.createStakeRecord(
                ...
                stakeRecord.derivativeWithdrawn,
                stakeRecord.derivativeDeposits,
                stakeRecord.underlyingWithdrawn,
                stakeRecord.underlyingBalance,
                stakeRecord.stakeToken
            );
        }
        emit UnderlyingAssetDeposited(address(wallet), amount, _vToken.balanceOf(address(this)), walletBalance);
        return true;
    }
    ...
    function withdrawStakeFromProtocol(
        ...
        bool isApproveSuccessful = wallet.approveAddress(vToken, sharesAmount, address(this));

        require(isApproveSuccessful, "LazerMerchantWallet: Wallet does not have enough allowance");

        wallet.transferFrom(vToken, address(this), sharesAmount);
        ...
        IVToken _vToken = IVToken(vToken);
        _vToken.redeem(sharesAmount);

        uint256 underlyingBalance = _token.balanceOf(address(this));
        _token.safeTransfer(address(wallet), underlyingBalance);

        StakingRecord memory stakeRecord = _updateStakeRecordAfterWithdrawal(walletId, underlyingBalance, sharesAmount);
    ...
    function _updateStakeRecordAfterWithdrawal(
        ...
        stakingRecord.derivativeWithdrawn = stakingRecord.derivativeWithdrawn.add(sharesAmount);

        stakingRecord.underlyingWithdrawn = stakingRecord.underlyingWithdrawn.add(underlyingAmountWithdrawn);

        stakingRecord.derivativeBalance = stakingRecord.derivativeBalance.sub(sharesAmount);

        stakingRecord.underlyingBalance = stakingRecord.underlyingBalance.sub(underlyingAmountWithdrawn);
    ...
    function _updateStakeRecordAfterDeposit(
        ...
        bool exists = storageContract.doesStakeRecordExist(merchant);
        if (!exists) {
            ...
            stakingRecord.amountStaked = stakingRecord.amountStaked.add(underlyingAmountDeposited);
    ...
    function getPricePerFullShare(address vToken) external view returns (uint256) {
        IVToken _vToken = IVToken(vToken);
        uint256 cash = _vToken.getCash();

        uint256 totalBorrows = _vToken.totalBorrows();

        uint256 totalReserves = _vToken.totalReserves();

        uint256 totalSupply = _vToken.totalSupply();

        uint256 pricePerFullShare = (cash.add(totalBorrows).sub(totalReserves)).div(totalSupply);
        return pricePerFullShare;
    }

    function getGrossRevenue(address account, address vToken) public view returns (uint256) {
        IVToken _vToken = IVToken(vToken);
        //  Get the price per full share
        uint256 price = _vToken.exchangeRateCurrent();

        //  Get the balance of yDai in this users address
        uint256 balanceShares = _vToken.balanceOf(account);
...
interface IERC20Wallet {
     ...
     */
    function tokenBalanceOf(address tokenAddress) external view returns (uint256);

    /**
     * @dev Returns the bnb balance for this contract
     */
    function bnbBalanceOf() external view returns (uint256);
    function approveAddress(address tokenAddress, uint256 amount, address spender) external returns (bool success);
    function transferFrom(address tokenAddress, address to, uint256 amount) external returns (bool success);
...
interface ILazerStorage is ILazerStorageSchema {
    ...
    function doesStakeRecordExist(uint256 merchantId) external view returns (bool);

    function getRecordIndex(uint256 merchantId) external view returns (uint256);
...
contract ERC20Wallet is Ownable {
        ...
        )
    {
        ...
        uint256 walletBalanceAfterCommissionFeeWithdrawal = token.balanceOf(recepient);
    ...
    function approveAddress(address tokenAddress, uint256 amount, address spender) public returns (bool success) {
        IERC20 tokenContract = IERC20(tokenAddress);
    ...
    function transferFrom(address tokenAddress, address to, uint256 amount) public returns (bool success) {
        IERC20 tokenContract = IERC20(tokenAddress);
