...
contract MultiRewards is ReentrancyGuard, Pausable {
    ...
    //mapping(address => mapping(address => uint256[])) public userRewardPerTokenPaidForStake;
    ...
    // this has to work without state variable upto date
    ...
    function unlockedReward(address account, address _rewardsToken) public view returns (uint256) {
        ...
        require(actualAmount <= reward, "internal consistency");
        return actualAmount;
    }    
    
    // the discrepancy should be 0 in theory   
    function checkTotalReward(address account, address _rewardsToken) public view returns (uint256, uint256, uint256, uint256, uint256) {
        uint256 reward = earned( account,  _rewardsToken );  // this function does not rely on the state variables
        ...
        uint256 stake2R = 0;
        for (uint j; j < userStakes[account].length; j++) {
            uint256 outAmount = earned_for_stake( account, _rewardsToken, j); // this function does not rely on state variables  
    	    actualAmount = actualAmount.add(outAmount);
    	    if (j ==0)
    	        stake0R = outAmount;
    	    else if (j==1)
        ...
        return (reward, actualAmount, stake0R, stake1R, stake2R);
    }  
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        require(actualAmount <= amount && actualAmount <= _balances[msg.sender], "No unlocked stake available or internal error");    
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        for (uint i; i < rewardTokens.length; i++) {
            ...
            for (uint j; j < userStakes[msg.sender].length; j++) {
                ...
                if (outAmount > 0) {
    	   	        rewardsForStake[msg.sender][_rewardsToken][j] = 0;
    		        actualAmount = actualAmount.add(outAmount);
                }
            }
            require(actualAmount <= reward, "No unlocked reward available or internal error");
        ...
        for (uint i; i < rewardTokens.length; i++) {
            ...
            if (account != address(0)) {
                ...
                for (uint j; j < userStakes[account].length; j++) {
                    ...
                    //userRewardPerTokenPaidForStake[account][token][j] = rewardData[token].rewardPerTokenStored;   
                }
