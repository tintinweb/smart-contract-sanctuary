...
contract LuckyStaking {
    ...
    {
        uint256 formattedAmount=amount*1000000000000000000;
        depositHistory.push(formattedAmount);
        depositHistoryAddress.push(msg.sender);
        depositHistoryTS.push(block.number);
        CNR.transferFrom(msg.sender,address(this),formattedAmount);
        lastClaimed[msg.sender]=block.number;
        blockSinceDeposit[msg.sender]=block.number;
        stake[msg.sender]+=formattedAmount;
        totalDeposits+=formattedAmount;
        poolShare[msg.sender]=stake[msg.sender]/totalDeposits;
