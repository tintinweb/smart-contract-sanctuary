...
contract ExtendedReflections is Context, IERC20, Ownable {
    ...
    */
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (!buying && !selling && _marketingRewardEnabled && _autoSwapAndLiquifyEnabled) {
            uint256 contractBalance = _tokenBalances[address(this)];
            bool overMinTokensBeforeSwaps = contractBalance >= _minTokensBeforeSwap;

            if (overMinTokensBeforeSwaps && !_inSwapAndLiquify && _msgSender() != _uniswapV2Pair) {
