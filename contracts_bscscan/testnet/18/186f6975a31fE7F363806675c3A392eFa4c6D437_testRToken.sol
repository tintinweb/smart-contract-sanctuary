...
// ZAIF token with Governance and reflect fees.
contract testRToken is RGBEP202('TestR Token', 'TSTR', 0x4305a48AD5ed01F56038693De8d92E36E89588b4) {
 ...
 * allowances. See {IBEP20-approve}.
 */
contract RGBEP202 is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;
    ...
    uint256 private constant MAX = ~uint256(0);
    uint256 private _maxMintable = 170000000 * 10 ** 18; // 170 million max mintable (180 million max - 10 million pre sale)
    uint256 private _tTotal = 9969000 * 10 ** 18; // 10 million pre sale > v2 started at 9.969.000 million
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    // Events
    event updatedToken(address indexed owner, uint256 amount);
    event TransferTaxFeeUpdated(address indexed owner, uint256 previousTaxFee, uint256 newTaxFee);
    event TransferBurnFeeUpdated(address indexed owner, uint256 previousBurnFee, uint256 newBurnFee);
    event TransferLiquidityFeeUpdated(address indexed owner, uint256 _previousLiquidityFee, uint256 newLiquidityFee);
    event TransferMarketingFeeUpdated(address indexed owner, uint256 _previousMarketingFee, uint256 newMarketingFee);
  ...
  //  event sendViaCall(address indexed to, uint256 amount);
    /**
     * @dev Sets the values for {name} ,{symbol} and {v1 token}, initializes {decimals} with
     * a default value of 18.
     *
     * To select a different value for {decimals}, use {_setupDecimals}.
     *
     ...
     * construction.
     */
    constructor(string memory name, string memory symbol , address payable _v1Token) public {
        _name = name;
        _symbol = symbol;
        _decimals = 18;

        v1Token = ZaifToken(_v1Token);
    ...
    //    pancakeRouter = _pancakeRouter;
        ...
        _operator = msg.sender;
         ...
         //get Total to swap from old token to new token 
         _tOwned[address(this)] = _tTotal;     
        
        
         emit Transfer(address(0), address(this), _tTotal);

         // will be reenabled after upgrade all users
         removeAllFee();

        // Airdrop - Presale amounts commented on v2
   ...
   function name() public view override returns (string memory) {
    ...
    function updateToken() external nonReentrant {
        uint256 amount = v1Token.balanceOf(msg.sender);  
        v1Token.transferFrom(msg.sender, BURN_ADDRESS, amount);
        transfer(msg.sender, amount);
        emit updatedToken(msg.sender, amount);
    }

    //Presale Functions only  (disabled on v2)
    ...
    //Burn Tokens that are not bought in presale / not updated
    function burn(uint256 _amount) public onlyOwner {     
        require(_tOwned[address(this)] >= _amount,'Burning more than the current tokens on contract!');
        transfer(BURN_ADDRESS, _amount);
        emit Transfer(address(this), BURN_ADDRESS, _amount);
        _tTotal = _tTotal.sub(_amount);
        _tOwned[address(this)].sub(_amount);
    }
