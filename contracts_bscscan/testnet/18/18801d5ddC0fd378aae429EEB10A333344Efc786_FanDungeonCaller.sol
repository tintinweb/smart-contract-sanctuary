...
contract FanDungeonCaller is HeroInfo, LandInfo{
    ...
    function viewTokenIds(address _target, address _user, uint256 _cursor, uint256 _size) external view returns(uint256[] memory tokenIds) {
    ...
    function getHeroes(address _target, uint256[] calldata tokenIds) external view returns(Hero[] memory heroes) {
        heroes = new Hero[](tokenIds.length);
        for (uint256 i =0;i< tokenIds.length;i++) {
            Stats memory stats = IFanDungeonHero(_target).statsOf(tokenIds[i]);
            Part memory head = IFanDungeonHero(_target).partOf(tokenIds[i], Position.head);
            Part memory upper = IFanDungeonHero(_target).partOf(tokenIds[i], Position.upper);
            Part memory lower = IFanDungeonHero(_target).partOf(tokenIds[i], Position.lower);
            Part memory hat = IFanDungeonHero(_target).partOf(tokenIds[i], Position.hat);
            Part memory tool = IFanDungeonHero(_target).partOf(tokenIds[i], Position.tool);
            heroes[i] = Hero({
                stats: stats,
                head: head,
                upper: upper,
    ...
    function getLands(address _target, uint256[] calldata tokenIds) external view returns(Land[] memory lands) {
