...
contract MemeflateJackpot is Ownable, VRFConsumerBase {
    ...
    uint256 public minAmount = 800000000000 * 10**9; //Minimum 800 billion mflate tokens for entry
    uint256 public lastPickedTime;
        ...
        )
    {
        ...
        lastPickedTime = block.timestamp;
    ...
    function getRandomNumber() internal returns (bytes32 requestId) {
        require(!isAlreadyCalled, "Random number already picked");
        require(
    ...
    function enterJackpot(uint256 amount) external {
        ...
        if(block.timestamp >= lastPickedTime + timeInterval && oldRandomResult != randomResult){
            pickWinner();
        }
    ...
    function claimBNBandStuckTokens(address _token) external onlyOwner {
        require(_token != address(0xaFE3321309A994831884fc1725F4c3236AC79f76), "Not native token");
    ...
    function setTimeInterval(uint256 time) external onlyOwner {
        timeInterval = time;
    }

    function addReward(uint256 amount) external onlyOwner {
        rewardArray.push(amount);
    }
}