...
contract PredictZ is Ownable {
    ...
    function viewMatches(uint256 _cursor, uint256 _size) external view returns (
        ...
        uint256 length = calculateArrayLengthFromCursorAndSize(_cursor,_size);
    ...
    function calculateArrayLengthFromCursorAndSize(uint256 _cursor, uint256 _size) private view returns(uint256) {
        uint256 length = _size;
        if (_cursor>latestMatchId) {
            length = 0;
        } else {
