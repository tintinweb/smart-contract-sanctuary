...
contract CobainReward is Context, IERC20, Ownable, Versionable {
    using SafeMath for uint256;
    ...
    string private _name = "Cobain Reward Network Finance";
    string private _symbol = "COBAINRW";
    uint8 private _decimals = 10;

    // 6 decimal precisions
    uint256 public constant _percentFactor = 100000000;

    uint256 public _taxFee = 2000000;
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 4000000;
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _burnFee = 1000000;
    uint256 private _previousBurnFee = _burnFee;
    address public _burnAddr;

    uint256 public _teamFee = 1000000;
    uint256 private _previousTeamFee = _teamFee;
    address public _teamAddr;

    uint256 public _marketingFee = 3000000;
    uint256 private _previousMarketingFee = _marketingFee;
    address public _marketingAddr;

    uint256 public _devFee = 500000;
    ...
    uint256 public _maxTxAmount = 4 * 10**7 * 10**6 * 10**9;
    uint256 public _minTxAmount = _percentFactor;
    ...
    function setFeesAdress(address burnAddr, address teamAddr, address marketingAddr, address devAddr) external onlyOwner(){
        _burnAddr = burnAddr;
		_teamAddr = teamAddr;
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() pctInput(liquidityFee) {
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() pctInput(taxFee) {
        _taxFee = taxFee;
    }
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() pctInput(maxTxPercent) {
        _maxTxAmount = _tTotal.div(_percentFactor).mul(maxTxPercent);
    ...
    function _transfer(address from, address to, uint256 amount, bool noEvent) private {
        ...
        require(amount >= _minTxAmount, "Transfer amount must be greater than factor");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool noEvent) private {
        if(!takeFee)
            removeAllFee();
