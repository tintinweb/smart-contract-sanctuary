...
contract EarnOtherNoFixedAPR is Ownable, ReentrancyGuard {
  ...
  function _updatePool() internal {
    ...
    accTokenPerShare = accTokenPerShare.add(rewards.mul(PRECISION_FACTOR).div(totalStaked));
    lastRewardBlock = block.number;
  }
  ...
  function deposit(uint256 _amount) external nonReentrant {
    ...
    if (user.amount > 0) {
      uint256 rewards = user.amount.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
      if (rewards > 0) {
        require(isTransferable(rewards), "EarnOtherNoFixedAPR::deposit:: insufficient reward");
  ...
  function withdraw(uint256 _amount) external nonReentrant {
    UserInfo storage user = userInfo[msg.sender];
    require(user.amount >= _amount, "EarnOtherNoFixedAPR::withdraw:: amount exceeds");

    _updatePool();

    uint256 rewards = user.amount.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
    if (rewards > 0) {
      require(isTransferable(rewards), "EarnOtherNoFixedAPR::withdraw:: insufficient reward");
      rewardToken.safeTransfer(msg.sender, rewards);
    }
  ...
  function pendingReward(address _for) external view returns (uint256) {
    require(_for != address(0), "EarnOtherNoFixedAPR::pendingReward:: bad address");
    ...
    if (block.number > lastRewardBlock && totalStaked != 0) {
      uint256 blocks = block.number.sub(lastRewardBlock);
      uint256 reward = blocks.mul(rewardPerBlock).mul(PRECISION_FACTOR);
      lAccTokenPerShare = lAccTokenPerShare.add(reward.div(totalStaked));
    }
    uint256 _pendingReward = (user.amount.mul(lAccTokenPerShare) / PRECISION_FACTOR).sub(user.rewardDebt);
  ...
  function harvest() external {
    ...
    UserInfo storage user = userInfo[msg.sender];
    uint256 accumulatedReward = user.amount.mul(accTokenPerShare).div(PRECISION_FACTOR);
    uint256 _pendingReward = accumulatedReward.sub(user.rewardDebt);
    if (_pendingReward == 0) {
      return;
    }

    require(isTransferable(_pendingReward), "EarnOtherNoFixedAPR::harvest:: insufficient reward");
  ...
  function isTransferable(uint256 _rewards) internal view returns (bool) {
    if (isMultiPool) {
