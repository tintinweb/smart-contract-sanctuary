...
contract BearHuntToken is Context, IERC20, Ownable {
    ...
    // 100,000,000,000,000 BHUNT Total supply
    uint256 private _tTotal = 100000000 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Bear Hunt Token";
    string private _symbol = "BHUNT";
    ...
    // 500,000,000,000 BHUNT Max tx amount
    uint256 public _maxTxAmount = 500000 * 10**6 * 10**9;
    // 50,000,000,000 BHUNT Max tx amount
    ...
    function setDevFeeAddress(address dev) external onlyOwner {
        _devFeeAddress = dev;
    }
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwner {
        require(uniswapV2Pair != address(0), "Zero pair");
        swapAndLiquifyEnabled = _enabled;
        emit SwapAndLiquifyEnabledUpdated(_enabled);
    }

    function setRouterAndPair(address _router, address _pair) public onlyOwner {
    ...
    function _takeDev(uint256 tDev) private {
        uint256 currentRate = _getRate();
        uint256 rDev = tDev.mul(currentRate);
        _rOwned[address(_devFeeAddress)] = _rOwned[address(_devFeeAddress)].add(rDev);
        if (_isExcluded[address(_devFeeAddress)])
            _tOwned[address(_devFeeAddress)] = _tOwned[address(_devFeeAddress)].add(tDev);
    }
