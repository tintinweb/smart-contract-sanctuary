...
import "./stake/PoiStakeChallengeReward.sol";
import "./challenge/PoiChallenge.sol";
import "./CommunityVault.sol";
import "./library/Logistic.sol";

contract AppToolView {
    enum RoleType {
        NONE,
        OWNER,
        STAKER,
    ...
    PoiChallenge public poiChallenge;
    ...
    constructor(
        ...
        require(address(pParameterizer) != address(0), "zero address");
        ...
        require(address(pVoting) != address(0), "zero address");
        ...
        friends = pFriends;
        communityVault = pCommunityVault;
        pPoiStakeChallenge = pPoiStakeChallenge;
        poiStake = pPoiStake;
        poiChallenge = pPoiChallenge;
        voting = pVoting;
    }
    ...
    function getRole(
        ...
        address userAddr,
        uint256 pollId
    ) external view returns (RoleType) {
        address holderAddr = poi.ownerOf(tokenId);
        uint256 stakeAmount = poiStake.userStake(tokenId, userAddr);

        if (userAddr == holderAddr && stakeAmount > 0) {
            return RoleType.OWNERSTAKER;
        }

        if (userAddr == holderAddr) {
            return RoleType.OWNER;
        }

        if (stakeAmount > 0) {
            return RoleType.STAKER;
        }

        address challengerAddr = poiChallenge.getChallenger(pollId);
        if (userAddr == challengerAddr) {
            return RoleType.CHALLENGER;
        }

        if (voting.didCommit(pollId, userAddr)) {
            return RoleType.VOTER;
        }
    ...
    function xAmountForYAmount(uint256 xAmount, uint256 currentXPosition)
        ...
        returns (uint256)
    {
        return Logistic.xAmountForYAmount(xAmount, currentXPosition);
    }

    function getChallengeState(uint256 pollId)
        external
        view
        returns (VoteState, uint256, uint256)
    {
        (uint256 commitEndDate, uint256 revealEndDate, , , , ) = voting.pollMap(
            pollId
        );

        (, , , , , , , uint8 resolved, , ) = poiChallenge.challenges(pollId);

        uint256 progress = 0;
        string[] memory names = new string[](2);
        names[0] = "commitStageLen";
        names[1] = "revealStageLen";
        uint256[] memory params = parameterizer.getByList(names);

        uint256 startTime = 0;

        if(voting.pollExists(pollId)){
            startTime = commitEndDate - params[0];
        }

        if (voting.pollExists(pollId) && !voting.isExpired(commitEndDate)) {
            progress =
                ((params[0] - (commitEndDate - block.timestamp)) * 100) /
                params[0];
            return (VoteState.VOTING, progress, startTime);
        }

        if (voting.pollExists(pollId) && !voting.isExpired(revealEndDate)) {
            progress =
                ((params[1] - (revealEndDate - block.timestamp)) * 100) /
                params[1];
            return (VoteState.REVEALING, progress, startTime);
        }

        if (
            voting.pollExists(pollId) &&
            voting.isExpired(revealEndDate) &&
            resolved == 1
        ) {
            return (VoteState.UPDATED, progress, startTime);
        }

        if (voting.pollExists(pollId) && voting.isExpired(revealEndDate)) {
            return (VoteState.REVEALEd, progress, startTime);
        }

        return (VoteState.NONE, progress, startTime);
    }
}
...
contract PoiChallenge is Ownable, IPoiChallenge {
    address public constant BURN_ADDRESS =
        address(0x000000000000000000000000000000000000dEaD);
    ...
    struct ChallengeModel {
        ...
        uint256 challengeAmount; // 挑战金额
        bytes32 postscript; // 挑战宣言 hash
        bytes32 subject; // 挑战主题
        uint16 challengeAmountRateForWinner; /// 胜方获得挑战金的百分比
        uint16 votesLoseRateForWinner; /// 胜方获得 败选方tokens 的百分比
        uint8 resolved; // 是否处理了
        uint256 votesRewardAmount; // 投票败选总token数
        uint256 challengeRewardAmount; // 挑战金奖励
        mapping(address => bool) tokenClaims; // Indicates whether a voter has claimed a reward yet
    }
    // pollId => Challenge
    mapping(uint256 => ChallengeModel) public challenges;
    // tokenId => pollId  POI最近的挑战
    mapping(uint256 => uint256) public poiRecentChallenge;

    event LogNewChallenge(
    ...
    event LogChallengeFailed(
        ...
        uint256 votesRewardAmount,
        uint256 burn
    );
    event LogChallengeSucceeded(
        uint256 indexed tokenId,
        uint256 indexed challengeID,
        uint256 rewardAmount,
        uint256 votesRewardAmount,
    ...
    /**
    发起挑战
    @param tokenId poi token id
    @param subject challenge subject
    @param postscript challenger words, hash
   */
    function challenge(
        uint256 tokenId,
        bytes32 subject,
        ...
        require(
            poiCenter.canbeChallengeSubject(subject),
            "subject not support challenge"
        );
        // 上次挑战还没完成
        uint256 recentPoll = poiRecentChallenge[tokenId];
        if (recentPoll > 0) {
            require(
                challenges[recentPoll].resolved == 1,
                "recent challenge not resolved"
            );
        }
        // 不允许持有者挑战
        require(
            !poiCenter.isOwnedBy(tokenId, _msgSender()),
            "owner not allowed"
        );
        // 不允许抵押者挑战
        require(
            poiStake.userStake(tokenId, _msgSender()) == 0,
            "staker not allowed"
        );

        string[] memory names = new string[](6);
        names[0] = "voteQuorum";
        names[1] = "punishRateOnNotReveal";
        names[2] = "commitStageLen";
        names[3] = "revealStageLen";
        names[4] = "challengeAmountRateForWinner";
        names[5] = "votesLoseRateForWinner";
        uint256[] memory params = parameterizer.getByList(names);
        require(
            params[1] <= 10000 && params[4] <= 10000 && params[5] <= 10000,
            "too big"
        );
        ...
        // 1、 锁定poicenter tokenId 相同 amount token
        uint256 challengeAmount = poiStake.lockByNewChallenge(tokenId);

        // 2、 新建challenge
        ChallengeModel storage challengeObj = challenges[pollId];
        challengeObj.tokenId = tokenId;
        challengeObj.challenger = msg.sender;
        challengeObj.postscript = postscript;
        challengeObj.challengeAmount = challengeAmount;
        challengeObj.subject = subject;
        challengeObj.challengeAmountRateForWinner = uint16(params[4]);
        challengeObj.votesLoseRateForWinner = uint16(params[5]);

        // 记录最近一次挑战
        poiRecentChallenge[tokenId] = pollId;

        // 转账
        if (challengeAmount > 0)
            token.transferFrom(msg.sender, address(this), challengeAmount);

        emit LogNewChallenge(
            msg.sender,
            tokenId,
            pollId,
            challengeAmount,
   ...
   */
    function commitVote(
        ...
        // 不允许挑战者投票
        require(
            challenges[pollId].challenger != _msgSender(),
            "challenger not allowed"
        );
        uint256 tokenId = challenges[pollId].tokenId;
        // 不允许持有者投票
        require(
            !poiCenter.isOwnedBy(tokenId, _msgSender()),
            "owner not allowed"
        ...
        voting.commitVote(pollId, secretHash, numTokens, _msgSender());

        token.transferFrom(_msgSender(), address(this), numTokens);
    }
    ...
    function revealVote(
        ...
        voting.revealVote(pollId, voteOption, salt, _msgSender());
    }

    /**
    没揭露，取回投票token
     */
    function rescueTokens(uint256 pollId) external {
        (uint256 rescueAmount, uint256 punishAmount) = voting
        .settleRescueTokens(pollId, _msgSender());
        if (rescueAmount > 0) token.transfer(_msgSender(), rescueAmount);
        // 如果不揭露，惩罚
        if (punishAmount > 0)
            token.transfer(address(communityVault), punishAmount);
    }

    /**
    揭露期结束，调用此函数计票并记录相应的奖励数据
    update challenge status, any one call at appropriate time
     */
    function updateStatus(uint256 pollId) public {
        ChallengeModel storage challengeInst = challenges[pollId];
        uint256 tokenId = challengeInst.tokenId;
        require(tokenId > 0, "no poll");
        require(challengeInst.resolved != 1, "resolved");

        // 标记已解决
        challengeInst.resolved = 1;

        (uint256 votesFor, uint256 votesAgainst) = voting
        .getTotalNumberOfTokensForOption(pollId);

        (bool isPassed, bool isValid) = voting.isPassed(pollId);
        if (!isValid) {
            poiStake.unlockAssetOnDrawGame(tokenId);
            emit LogChallengeInvalid(tokenId, pollId);
            return;
        }

        uint256 orignVotesReward;
        if (isPassed) {
            // 挑战者胜出, 将 锁定的token转入this，当作奖励发放
            poiStake.punishOnChallangeLose(tokenId);
            // 标记为无效
            poiCenter.setInvalidByChallenge(tokenId);

            orignVotesReward = votesAgainst;
        } else {
            orignVotesReward = votesFor;
        }

        uint256 burnRate = parameterizer.getAndCheck(
            "challengeRewardBurnRate",
            0,
            10000,
            6
        ); // < 100%
        uint256 burnVotesReward = (orignVotesReward * burnRate) / 10000;
        uint256 burnChallengeReward = (challengeInst.challengeAmount *
            burnRate) / 10000;
        challengeInst.votesRewardAmount = orignVotesReward - burnVotesReward;
        challengeInst.challengeRewardAmount =
            challengeInst.challengeAmount -
            burnChallengeReward;
        uint256 burnAmount = burnChallengeReward + burnVotesReward;
        if (burnAmount > 0) token.transfer(BURN_ADDRESS, burnAmount);

        if (isPassed) {
            emit LogChallengeSucceeded(
                tokenId,
                pollId,
                challengeInst.challengeRewardAmount,
                challengeInst.votesRewardAmount,
                burnAmount
            );
        } else {
            uint256 rewardAmount = winnerReward(
                challengeInst.challengeRewardAmount,
                challengeInst.votesRewardAmount,
                challengeInst.challengeAmountRateForWinner,
                challengeInst.votesLoseRateForWinner
            );
            // 解冻, 发放奖励
            token.approve(address(poiStake), rewardAmount);
            poiStake.awardOnChallangeWin(tokenId, rewardAmount);
    ...
    提取奖励
   */
    function claimReward(uint256 pollId) external {
        ChallengeModel storage challengeInst = challenges[pollId];
        require(challengeInst.resolved == 1, "not resolved");

        address sender = _msgSender();
        require(!challengeInst.tokenClaims[sender], "claimed");

        challengeInst.tokenClaims[sender] = true;
        uint256 challengeAmount = challengeInst.challengeAmount;
        ...
        if (!isValid) {
            ...
            if (challengeInst.challenger == sender) {
                withdrawAmount = challengeAmount;
            } else {
                (uint256 votesFor, ) = voting.getVotesFor(pollId, sender);
                if (votesFor > 0) {
                    withdrawAmount = votesFor;
                } else {
                    (uint256 votesAginst, ) = voting.getVotesAginst(
                        pollId,
                        sender
                    );
                    if (votesAginst > 0) {
                        withdrawAmount = votesAginst;
                    }
                }
            }
            require(withdrawAmount > 0, "not involved");

            token.transfer(sender, withdrawAmount);

            emit LogClaimReward(sender, withdrawAmount, 0, 0);
            return;
        }

        if (isPassed) {
            if (challengeInst.challenger == sender) {
                stakeAmount = challengeAmount;
                ...
                (uint256 votesFor, uint256 totalVotesFor) = voting.getVotesFor(
                    pollId,
                    sender
                );
                require(votesFor > 0, "not vote for");
                stakeAmount = votesFor;
                rewardAmount = voteWinnerReward(
                    votesFor,
                    totalVotesFor,
                    challengeInst.challengeRewardAmount,
                    challengeInst.votesRewardAmount,
            ...
            (uint256 votesAginst, uint256 totalVotesAginst) = voting
            .getVotesAginst(pollId, sender);
            require(votesAginst > 0, "not vote aginst");
            stakeAmount = votesAginst;
            rewardAmount = voteWinnerReward(
                votesAginst,
                totalVotesAginst,
                challengeInst.challengeRewardAmount,
        ...
        uint256 totalFee = (rewardAmount * 1000) / 10000; // 10% fee
        uint256 friendFee = friends.settleReward(sender, rewardAmount);
        require(totalFee > friendFee, "friend fee too much");
        ...
        token.transfer(sender, stakeAmount + rewardAmount - totalFee);

        emit LogClaimReward(sender, stakeAmount, rewardAmount, totalFee);
    }
    ...
    function winnerReward(
        ...
        return (cAmount * cAmountRate + vLose * (10000 - vLoseRate)) / 10000;
    }

    function voteWinnerReward(
        uint256 weight,
        uint256 totalWeight,
        uint256 cAmount,
        uint256 vLose,
        ...
        return
            ((vLose * vLoseRate + cAmount * (10000 - cAmountRate)) * weight) /
            (10000 * totalWeight);
    }

    function rewardOf(uint256 pollId) external view returns (uint256) {
        ChallengeModel storage challengeInst = challenges[pollId];

        if (challengeInst.resolved != 1) {
            return 0;
        }

        address sender = _msgSender();
        if (challengeInst.tokenClaims[sender]) {
            return 0;
        }
    ...
    function isResolved(uint256 pollId) external view returns (bool) {
        return challenges[pollId].resolved == 1;
    }

    function isChallenging(uint256 tokenId)
        external
        view
        override
        returns (bool)
    {
        uint256 recentPoll = poiRecentChallenge[tokenId];
        if (recentPoll > 0) {
            return challenges[recentPoll].resolved == 0;
        }
        return false;
    }

    function getChallenger(uint256 pollId) external view returns (address) {
        return challenges[pollId].challenger;
    }
}
