...
contract LazerMerchantWalletController is Ownable, ILazerStorageSchema {
    ...
    function stakeFundsToProtocol(
        ...
        StakingRecord memory stakeRecord = _updateStakeRecordAfterDeposit(walletId, vToken, amount, _vToken.balanceOf(address(wallet)), merchantId);
        ...
        emit UnderlyingAssetDeposited(address(wallet), amount, _vToken.balanceOf(address(wallet)), walletBalance);
        return true;
    }
    ...
    function _updateStakeRecordAfterDeposit(
        ...
        if (!exists) {
            StakingRecord memory stakingRecord = StakingRecord(merchant, amountStaked, sharesAmount, 0, sharesAmount, 0, vToken, wallet, true);
            return stakingRecord;
        } else {
            StakingRecord memory stakingRecord = getStakingRecordByWallet(wallet);
