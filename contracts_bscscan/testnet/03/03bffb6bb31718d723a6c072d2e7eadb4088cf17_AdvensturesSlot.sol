...
contract AdvensturesSlot is Context, PartnerOwnable  { 
    ...
    event bet_returned(address sender, uint tot_bet, uint win_tot, uint fee, uint returned, WINS wins);
    ...
    function bet(uint[] calldata bets,uint domainId, uint tokensId) external  
        ...
        emit bet_returned(msg.sender, tot_bet, win_tot, fee1, wins.wins_result, wins);
        _wins = wins; 
