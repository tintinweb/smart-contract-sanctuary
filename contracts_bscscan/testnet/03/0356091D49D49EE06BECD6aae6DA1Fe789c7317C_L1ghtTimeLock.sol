...
contract L1ghtTimeLock {
    ...
    constructor (IERC20 _l1ght) {

        l1ght = _l1ght;
        uint currentTime = block.timestamp;

        createLock(_l1ght, msg.sender, currentTime + 365 days);
        createLock(_l1ght, msg.sender, currentTime + 2 * 365 days);
        createLock(_l1ght, msg.sender, currentTime + 3 * 365 days);
        createLock(_l1ght, msg.sender, currentTime + 4 * 365 days);
        createLock(_l1ght, msg.sender, currentTime + 5 * 365 days);
    }
    ...
    function getLockAddress(uint lock) external view returns (address) {
        require(lock <= 4, "getLockAddress: lock does not exist");
    ...
    function forwardTokens() external {
        uint tokens = l1ght.balanceOf(address(this));

        require(tokens > 0, "forwardTokens: no l1ght tokens!");

        for (uint256 index = 0; index <= 4; index++) {
            l1ght.transfer(address(Locks[index]), tokens / 5);
        }

        // just incase theres any boos left from rounding
        uint leftover = l1ght.balanceOf(address(this));

        if (leftover > 0) {
            l1ght.transfer(address(Locks[4]), leftover);
        }
    }
