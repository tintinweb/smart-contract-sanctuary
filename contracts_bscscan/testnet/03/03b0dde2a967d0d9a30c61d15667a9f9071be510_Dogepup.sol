...
contract Dogepup is ERC20, Ownable {
    ...
    DogepupDividendTracker public dividendTracker;
    ...
    uint256 public immutable fixedSaleStartTimestamp = 1635172855; //October 25, 2:40.55 GMT, 2021
    ...
    uint256 public immutable tradingEnabledTimestamp = 1635172855; //October 26, 2:40.55 GMT, 2021
    ...
    constructor() public ERC20("Dogepup", "Dogepup") {
        ...
        dividendTracker = new DogepupDividendTracker();
        ...
        address _bounceFixedSaleWallet = 0x06951d8cEef22779081451d73C9f48d2D46eCd84;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Dogepup: The dividend tracker already has that address");

        DogepupDividendTracker newDividendTracker = DogepupDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Dogepup: The new dividend tracker must be owned by the Dogepup token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Dogepup: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Dogepup: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Dogepup: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Dogepup: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Dogepup: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Dogepup: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Dogepup: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "Dogepup: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "Dogepup: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "Dogepup: The fixed-sale is only available to certain participants at the start");
            }
...
contract DogepupDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("Dogepup_Dividend_Tracker", "Dogepup_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100 * (10**18); //must hold 100+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "Dogepup_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "Dogepup_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Dogepup contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Dogepup_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Dogepup_Dividend_Tracker: Cannot update claimWait to same value");
