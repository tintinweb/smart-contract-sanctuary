...
contract METATEST is Context, IERC20, Ownable {
    ...
    function F08_Set_Normal_Waiting_Time_Between_Sells(uint256 normal_wait_seconds) external onlyOwner {
        ...
        // then set it to zero --> normal wait_wait_seconds = 0
        normal_waiting_time_between_sells = normal_wait_seconds;
    }
    function F09_Set_Impact2_Longer_Waiting_Time_Before_Next_Sell(uint256 longer_wait_seconds) external onlyOwner {           
        //Examples:   Must wait 3 days --> longer_wait_seconds = 259200
        //                      7 days --> longer_wait_seconds = 604800
        //
        // To disable this feature i.e. to have the normal waiting time 
        // for impact2 then set this to zero --> longer_wait_secs = 0
        impact2_longer_waiting_time_before_next_sell = longer_wait_seconds;
    }
    ...
    function F51_Rescue_Other_Tokens_Sent_To_This_Contract(IERC20 token, address receiver, uint256 amount) external onlyOwner {
       ...
       function F52_Owner_Change_by_Owner(address newOwner) public virtual onlyOwner {
