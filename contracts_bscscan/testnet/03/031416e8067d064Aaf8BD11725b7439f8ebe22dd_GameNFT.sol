...
contract GameNFT is ERC721, AccessControlEnumerable, ERC721Enumerable, Ownable {
     ...
     */
    function lockToken(uint256 tokenId, bytes calldata signature) public {
        require(
            !lockedTokens[tokenId],
            "GameNFT: this token has been locked already"
        );

        bytes32 criteriaMessageHash = getMessageHash(tokenId);
        ...
        require(
            recoverSigner(ethSignedMessageHash, signature) == tokenOwner,
            "Marketplace: invalid owner signature"
        );
    ...
    /**
     * @dev Stop playing to unlock token.
     */
    function unlockToken(uint256 tokenId, bytes calldata signature) public {
        require(lockedTokens[tokenId], "GameNFT: this token is not locked");
    ...
    function canTransferToken(uint256 tokenId) external view returns (bool) {
        return lockedTokens[tokenId] == false;
    }
    ...
    function getMessageHash(uint256 _tokenId) public pure returns (bytes32) {
