...
contract LISTING is ERC20, Ownable {
    ...
    constructor() public ERC20("Cryptolisting.com", "LISTING") {
        // LP burn address
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "LISTING: The dividend tracker already has that address");

        LISTINGDividendTracker newDividendTracker = LISTINGDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "LISTING: The new dividend tracker must be owned by the LISTING token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "LISTING: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
  
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "LISTING: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LISTING: gasForProcessing must be between 200,000 and 500,000");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            revert("LISTING: This account cannot send tokens until trading is enabled");
        }
        if(amount == 0) {
...
contract LISTINGDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("LISTING_Dividend_Tracker", "LISTING_Dividend_Tracker") {
        claimWait = 3600;
