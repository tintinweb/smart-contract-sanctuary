...
contract PreSaleHouse is Ownable {
    using SafeMath for uint256;
    ...
    address public masterChefAddress = 0x15E3b8Ce984E94FC81CcAC3CF3d48C26cC89507e;
    // public masterChef;
    ...
    function deposit() public payable {
        ...
        uint256 newBalance = deposits[msg.sender].add(msg.value);

        (uint256 i, uint256 s,uint256 a) = IMasterChef(masterChefAddress).userInfo(0, msg.sender);
        uint256 stakedBalance = i;
        testVariable = stakedBalance;
        if(stakedBalance >= 2 && stakedBalance <= 60000000000 && newBalance <= 1*1e18){
            deposits[msg.sender] = newBalance;
            emit Deposited(msg.sender, msg.value);

        } else if(stakedBalance >=60000000001 && stakedBalance < 100000000000 && newBalance <= 3*1e18){
            deposits[msg.sender] = newBalance;
            emit Deposited(msg.sender, msg.value);

        } else if(stakedBalance >= 100000000000 && newBalance <= 10*1e18){
            deposits[msg.sender] = newBalance;
            emit Deposited(msg.sender, msg.value);

        } else {

            revert("Did not meet conditions");
        }
//
