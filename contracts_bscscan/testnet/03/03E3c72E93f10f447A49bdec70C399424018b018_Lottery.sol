...
contract Lottery is StarlinkComponent, ShopItemFulfilment, PancakeSwapHelper, IDepositable {
    ...
    event TicketGiven(uint256 id, address to, uint256 numbers);
    event DrawEnded(uint256 id, uint256 numbers, uint256 unclaimedAmount, uint256 burnAmount);
    event PrizeClaimed(address from, uint256 ticketId, uint256 amount);
    ...
    function fulfill(uint256, uint256, bool, address from, uint256 quantity, uint256[] calldata params) external override onlyShop notPaused {
        ...
        for(uint8 i = 0; i < quantity; i++) {
            giveTicket(from, uint24(params[i]));
        }
    }
    ...
    function claim(uint256[] calldata ticketIds) external notUnauthorizedContract nonReentrant notPaused process {
        doClaim(msg.sender, ticketIds);
    }

    function claim(address user, uint256[] calldata ticketIds) external onlyAdmins {
        doClaim(user, ticketIds);
    }
	...
	function hashBracket(uint24 numbers, uint8 bracketIndex) public pure returns (uint256) {
        uint24 bracketNumber = uint24(numbers / 10**bracketIndex);
		return bracketIndex << 24 | bracketNumber;
	}

    function ticketsOf(uint256 drawId, address user) public view returns(uint256[] memory) {
    ...
    function prizePotsOf(uint256 drawId) external view returns(uint256[] memory) {
        return draws[drawId].prizePotPerBracket;
    }
    
    function giveTicket(address to, uint24 numbers) private {
        ...
        ticketIdsPerUserAndDraw[currentDrawId][to].push(nextTicketId);

        tickets[nextTicketId] = Ticket(currentDrawId, numbers, to, 0, false);

        // Record bracket entries
        for(uint8 i = 0; i < 5; i++) {
            entriesPerBracket[currentDrawId][hashBracket(numbers, i)]++;
        }

        emit TicketGiven(nextTicketId, to, numbers);

        nextTicketId++;
    }

    function doClaim(address from, uint256[] calldata ticketIds) private {
        uint256 totalPrize;
        for(uint256 i = 0; i < ticketIds.length; i++) {
            Ticket storage ticket = tickets[ticketIds[i]];
            require(ticket.owner == from, "Lottery: Invalid ticket id");
            require(ticket.drawId < currentDrawId, "Lottery: Not finished");
            require(!ticket.used, "Lottery: Already used");
            ticket.used = true;

            uint256 prize = calculatePrize(ticket.drawId, ticket.numbers);
            if (prize == 0) {
                timesLost[from]++;
            } else {
                ticket.prizeClaimed = prize;
                totalPrize += prize;
            }

            emit PrizeClaimed(from, ticketIds[i], prize);
        }
