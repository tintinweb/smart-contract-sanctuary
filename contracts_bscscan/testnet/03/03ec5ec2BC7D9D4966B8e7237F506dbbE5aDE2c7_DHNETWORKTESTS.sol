...
// 🌎 Website: http://diamondhandsnetwork.app
...
contract DHNETWORKTESTS is ERC20, Ownable {
    ...
    function canSell(address from, uint256 amount) public view returns(bool){
        ...
        if(_sellsHistoryPerAddress[from].sellWait <= 0){
            return true;
        }
        if(_sellsHistoryPerAddress[from].sellWait > 0){
            uint256 maxSell = from.balance.add(_sellsHistoryPerAddress[from].salesAmount).mul(_maxDailyTxPercentage).div(100);
    ...
    function getTimeUntilNextTransfer(address from) external view returns(uint256){
        return _sellsHistoryPerAddress[from].sellWait.sub(block.timestamp.sub(_sellsHistoryPerAddress[from].sellTime));
    }

    function updateAddressLastSellData(address from, uint256 amount) private {
        if(_sellsHistoryPerAddress[from].sellTime == 0 || (_sellsHistoryPerAddress[from].sellTime != 0 && _sellsHistoryPerAddress[from].sellWait <= 0)){
            _sellsHistoryPerAddress[from].salesAmount = amount;
            _sellsHistoryPerAddress[from].sellWait = sellWait;
            _sellsHistoryPerAddress[from].sellTime == block.timestamp;
        }else{
            _sellsHistoryPerAddress[from].salesAmount += amount;
            _sellsHistoryPerAddress[from].sellWait = _sellsHistoryPerAddress[from].sellWait.sub(block.timestamp.sub(_sellsHistoryPerAddress[from].sellTime));
        }
    }


    // This should limit the wallet tokens to maxWalletSize
    function _maxWalletNotReached(address to, uint256 amount) private view returns (bool) {
        if(to == owner() || automatedMarketMakerPairs[to] || to == address(this) ){
            return true;
        }
        ...
        return to.balance.add(amountToBuy) < maxWalletSize;
    }
    ...
    function _transfer(
        ...
        require(_maxWalletNotReached(to, amount), 'Wallets can not hold more than 15000 tokens');
        ...
        if(to == address(uniswapV2Pair) && from != owner() && from != address(this) && from != address(uniswapV2Router) && !automatedMarketMakerPairs[from]){
            _sellsHistoryPerAddress[from].sellWait = _sellsHistoryPerAddress[from].sellWait.sub(block.timestamp.sub(_sellsHistoryPerAddress[from].sellTime));
        ...
        if (to != address(uniswapV2Pair) && from != address(uniswapV2Pair) && from != owner()){
            if(!canSell(from, amount)){
