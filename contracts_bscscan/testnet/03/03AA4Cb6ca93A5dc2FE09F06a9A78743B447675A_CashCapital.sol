...
contract CashCapital {
    ...
    uint256 public orbitIncome=200*1e18;
    BEP20 BUSD;
    ...
    function startBoard() external payable {
            ...
            require(BUSD.balanceOf(msg.sender)>=(200*1e18),"Low Balance in wallet");
            require(BUSD.allowance(msg.sender,address(this))>=(200*1e18),"Approve Your Token First");
            BUSD.transferFrom(msg.sender,address(this),200*1e18);
    ...
    function updateBoard(address _user) private 
    {
            ...
            else
            {
                ...
                {
                    BUSD.transfer(topReferrer,orbitIncome);  
                }
                if(_length==4)
                {
    ...
    function usersX3Matrix(address userAddress, uint8 level) public view returns(address, address[] memory, bool) {
     ...
     function usersX6Matrix(address userAddress, uint8 level) public view returns(address, address[] memory,address[] memory, bool,uint256) {
      ...
      function getUserBoard(address userAddress, uint8 level) public view returns(uint256, uint256[] memory,uint256[] memory, bool) {
        ...
        if(users[userAddress].x6Matrix[level].referrals.length<2)
