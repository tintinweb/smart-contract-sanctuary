...
contract DividendDistributor is IDividendDistributor {
    ...
    function distributeAll(uint256 gas) external {
     ...
     function distributeDividend(address shareholder, uint256 percent) public {
        ...
        uint256 netamount = amount.mul(percent).div(100);
        
        if(netamount > 0) {
            
            netamount = amount.sub(1); //this is so we aren't short on dust in the holding wallet
            ...
            shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount);
            shares[shareholder].totalUnrealized = shares[shareholder].totalExcluded.sub(shares[shareholder].totalRealized);
    ...
    function reinvestDividend(address shareholder, uint256 percent, uint256 minOut) public {
        ...
        uint256 amount = getUnpaidEarnings(shareholder).mul(percent).div(100);
...
contract KojiEarth is IBEP20, Auth {
    ...
    string constant _symbol = "KOJI DT1.2";
