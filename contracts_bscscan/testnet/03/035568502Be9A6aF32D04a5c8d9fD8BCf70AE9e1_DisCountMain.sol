...
contract DisCountMain is Ownable, Pausable{
    ...
    mapping (address => bool) public reservedToken;
    ...
    function reservedTokenUpdate(address token,bool status) public onlyOwner{
        reservedToken[token] = status;
    }
    ...
    function poolWhiteListUpdate(address pool,bool status) public {
        ...
        poolInfo[pool].isWhiteList = status;
    }
    
    function addWhiteListForPool(address pool,address[] memory accounts) public {
        require(poolInfo[pool].isWhiteList, "whitelist is not enable");
        require(msg.sender == owner() || msg.sender == IDiscountPool(pool).poolAdmin(), "unable to access");
    ...
    function createPool(
        ...
        uint256 minimumDeposit,
        uint256 maximumDeposit,
        uint256 buyBackFee) external payable whenNotPaused {
        require(reservedToken[token[0]], "Not able to add this from token");
        require(block.timestamp < startTime && startTime< endTime && startTime.add(maxSellDuration) >= endTime, "invalid time duration");
    ...
    function isPoolBlock(address pool) public view returns (bool) {
