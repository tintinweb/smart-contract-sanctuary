...
// File: contracts/libs/TWAPOracleUpdater.sol
...
// File: contracts/Doob.sol


pragma solidity ^0.7.5;


contract Doob is TWAPOracleUpdater {
    using SafeMath for uint256;
    address[] public nonCirculatingAddresses;
    
    constructor() TWAPOracleUpdater("Doob", "DOOB", 9) {}
    ...
    function _nonCirculatingSupply() internal view returns (uint256) {
        uint256 nonCirculatingDoob;

        for (uint256 i = 0; i < nonCirculatingAddresses.length; i = i.add(1)) {
            nonCirculatingDoob = nonCirculatingDoob.add(balanceOf(nonCirculatingAddresses[i]));
        }

        return nonCirculatingDoob;
    }
