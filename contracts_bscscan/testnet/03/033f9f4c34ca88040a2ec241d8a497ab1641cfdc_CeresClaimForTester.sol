...
contract CeresClaimForTester is Ownable{
    ...
    constructor(address _token, uint256 _endTime, Claim[] memory _claims){

        token = IERC20(_token);
        endTime = _endTime;

        for (uint256 i = 0; i < _claims.length; i++)
            claimAmount[_claims[i].account] = _claims[i].amount;
    }
    ...
    function end() external onlyOwner {
        require(block.timestamp >= endTime, "claim is not end!");
