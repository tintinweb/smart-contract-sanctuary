...
contract MowaNFTPetCombine is Ownable, ReentrancyGuard {
    ...
    address private feeWallet = 0xB89E7C6eB6FeF36fd30A86dc9614E753d81E6b3C;
    uint256 private feeCombine = 2 * 10 ** 18;
    ...
    // character => MowaPet
    mapping(uint256 => MowaPet) mowaPet;
    ...
    function setMowaPet(
        ...
        mowaPet[character] = MowaPet(level, skill, star, character, class);
    }
    ...
    function setFeeCombine(uint256 _feeCombine) onlyOwner public {
        feeCombine = _feeCombine.mul(1e18);
    }

    function petCombine(uint256 tokenIdPetFragment, uint256 tokenIdSoul) public nonReentrant {
        require(blackList[_msgSender()] == false, "owner has been locked");

        if (feeCombine > 0) {
            uint256 mowaBal = mowaToken.balanceOf(_msgSender());
            require(mowaBal >= feeCombine, "Insufficient funds in the account");
            mowaToken.transferFrom(_msgSender(), feeWallet, feeCombine);
        }

        uint256 characterPet = mowaItemsCore.getNFT(tokenIdPetFragment).characterPet;
        require(characterPet > 0, "Pet Fragment not found");

        uint256 star = mowaItemsCore.getNFT(tokenIdPetFragment).star;
        uint256 qtyFragment = star.mul(10);

        uint256 qtyPetFragment = mowaItemsNFT.balanceOf(_msgSender(), tokenIdPetFragment);
        require(qtyPetFragment >= qtyFragment, "Not enough Pet Fragment");

        uint256 qtySoul = mowaItemsNFT.balanceOf(_msgSender(), tokenIdSoul);
        require(qtySoul >= 1, "Not enough Soul");

        mowaItemsCore.burnNFT(_msgSender(), tokenIdPetFragment, qtyFragment);
        mowaItemsCore.burnNFT(_msgSender(), tokenIdSoul, 1);

        uint256 tokenIdNew = mowaCore.getNextNFTId();
        mowaCore.safeMintNFT(_msgSender(), tokenIdNew);
        Mowa memory mowa = Mowa(
            tokenIdNew,
            mowaPet[characterPet].level,
            mowaPet[characterPet].skill,
            mowaPet[characterPet].star,
            mowaPet[characterPet].character,
            mowaPet[characterPet].class,
            block.timestamp
        );
