...
contract GoBrandonCoin is Context, IERC20, Ownable, ReentrancyGuard {
	...
	address BURN_ADDRESS = 0x0000000000000000000000000000000000000001;
	 address internal LiquidtyFeeAddress;
	...
	constructor (uint _total_Supply,
	                 uint Max_Tx_Amount,
	                 address  _LiquidtyFee_Address,
                     address  _MarketingFee_Address,
                     address  _PromotionFeeAddress,
                     uint256 _Reward_Fee_percent ,
                     uint8 Decimals,
                     string memory coin_name,
                     string memory coin_symbol ) {
	    
	    LiquidtyFeeAddress = _LiquidtyFee_Address;
	    ...
	    _symbol = coin_symbol;
	    _decimals = Decimals;
	    _rewardFee = _Reward_Fee_percent;
	    _tTotal = _total_Supply * 10**_decimals;
	...
	function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
		...
		(uint256 rAmount, uint256 rTransferAmount, uint256 rHODLrFee) = _getRValues(tAmount, tHODLrFee, _getRate());
	...
	function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
		...
		uint256 tTransferAmount = tAmount.sub(tHODLrFee.mul(4));
		return (tTransferAmount, tHODLrFee, tBurn);
	}

	function _getRValues(uint256 tAmount, uint256 tHODLrFee, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
		uint256 rAmount = tAmount.mul(currentRate);
		uint256 rHODLrFee = tHODLrFee.mul(currentRate);
		//uint256 rBurn = tBurn.mul(currentRate);
		uint256 rTransferAmount = rAmount.sub(rHODLrFee.mul(4));
	...
	function _transferStandard(address sender, address recipient, uint256 tAmount) private {
		...
		_rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
		 ...
		 _rOwned[MarketingFeeAddress] = _rOwned[MarketingFeeAddress].add(rHODLrFee);
         _rOwned[PromotionFeeAddress] = _rOwned[PromotionFeeAddress].add(rHODLrFee);
		 _rOwned[LiquidtyFeeAddress] = _rOwned[LiquidtyFeeAddress].add(rHODLrFee);
	//	_transferBurn(tBurn);
		_HODLrFee(rHODLrFee, tHODLrFee);
	//	emit TransferBurn(sender, BURN_ADDRESS, tBurn);
		emit Transfer(sender, recipient, tTransferAmount);
