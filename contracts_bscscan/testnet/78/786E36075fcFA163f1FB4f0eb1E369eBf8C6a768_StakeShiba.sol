...
contract StakeShiba {
    ...
    constructor(address _aiContract, address _tokenAddress, address _ownTokenAddress) public
    {
        terminal = msg.sender;
    ...
    function withdrawAll() public returns(bool)
    {
      ...
      if(totaldaily>0)
      {
        ...
        {
          if(checklimit(ref, totaldaily * (userInfo[msg.sender].referralplace*10)/100) || ref==terminal)
          {
        ...
        {
          if(checklimit(ref, totaldaily)  || ref==terminal)
          {
    ...
    function Claim(uint stakeIndex) public returns(bool)
    {
      require(!isContract(msg.sender),  'No contract address allowed to withdraw');
        ...
        if(tim2 < tim + lD)
        {
          ...
          {
            amount = amount.sub(totalAirdrop[msg.sender]);
          }
    ...
    function _AIpollDeposits(address _addr, uint256 _amount) private {
      ...
      function _drawPool() private {
          ...
          for(uint8 i = 0; i < pool_bonuses.length; i++) {
              ...
              if(checklimit(pool_top[i], win) || pool_top[i] == terminal)
              {
                pool_balance -= win;
    ...
    function purchaseTokens(address _customerAddress, uint256 _amountOfTokens, address _referredBy, uint256 mainamount) internal returns(uint256)
    {
        ...
        if(checklimit(_referredBy, directincome) || _referredBy == terminal)
        {
    ...
    function distributeRewards(uint256 _amount, address _idToDistribute)  internal
    {
        ...
        for(uint i=0; i<7; i++)
        {
           ...
           // uint256 _holdingLevel1 = level1Holding_[_idToDistribute];
            ...
            //{
                if(checklimit(_idToDistribute, _amount*pct/10000) || _idToDistribute == terminal)
                {
