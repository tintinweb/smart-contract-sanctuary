...
contract ImplAndTerms is Storage, Ownable, ERC20Init {
    ...
    function stakeFresh(address staker, uint stakeAmount, uint holdTime, uint lpAmountOut) internal {
        ...
        userStakes[staker].push(StakeData({stakeAmount: stakeAmount, lpAmount: lpAmountOut, stakeTime: block.timestamp, holdTime: holdTime, active: true}));

        emit Stake(staker, userStakes[staker].length, stakeAmount, holdTime, lpAmountOut);
    }

    function calcAllLPAmountOut(uint amountIn, uint holdTime, uint rate_) public view returns (uint, uint, uint, uint, uint) {
        uint stakerLpAmountOut = calcStakerLPAmount(amountIn, holdTime, rate_);
        uint refererLpAmountOut = calcRefererLPAmount(amountIn, rate_);
        uint influencerLpAmountOut = calcInfluencerLPAmount(amountIn, rate_);
        uint developerLpAmountOut = calcDeveloperLPAmount(amountIn, rate_);
    ...
    function calcStakerLPAmount(uint amountIn, uint holdTime, uint rate_) public view returns (uint) {
        return amountIn * rate_ + calcBonusTime(amountIn, holdTime, rate_);
    }

    function calcBonusTime(uint amountIn, uint holdTime, uint rate_) public view returns (uint) {
        return amountIn * rate_ * holdTime * timeBonusPercent / 100e18 / timeNormalizer;
    }

    function calcRefererLPAmount(uint amountIn, uint rate_) public view returns (uint) {
        return amountIn * rate_ * refererBonusPercent / 100e18;
    }

    function calcInfluencerLPAmount(uint amountIn, uint rate_) public view returns (uint) {
        return amountIn * rate_ * influencerBonusPercent / 100e18;
    }

    function calcDeveloperLPAmount(uint amountIn, uint rate_) public view returns (uint) {
        return amountIn * rate_ * developerBonusPercent / 100e18;
    }
    ...
    function calcAmountOut(uint lpAmountIn, uint currentTimestamp, uint stakeTime, uint holdTime) public view returns (uint) {
        uint tokenAmountOut = totalStaked * lpAmountIn / totalSupply;

        uint feeAmount = calcFee(lpAmountIn, currentTimestamp, stakeTime, holdTime);

        return tokenAmountOut > feeAmount ? tokenAmountOut - feeAmount : 0;
    }

    function calcFee(uint lpAmountIn, uint currentTimestamp, uint stakeTime, uint holdTime) public view returns (uint) {
        ...
        return totalStaked * lpAmountIn * unHoldFee * delta / totalSupply / holdTime / 100e18;
    }

    function calcProfit(address user, uint userStakeId) public view returns (uint, uint) {
        ...
        uint tokenAmountOut = totalStaked * lpAmountIn / totalSupply;
        uint feeAmount = calcFee(lpAmountIn, block.timestamp, stakeTime, holdTime);
    ...
    function getTokenAmountAfterUnstake(address user, uint stakeUserId) public view returns (uint) {
    ...
    function getTokenAmountAfterAllUnstakes(address user) public view returns (uint) {
        ...
        for (uint i = 0; i < userStakes[user].length; i++) {
            stakeTokenAmountOut += getTokenAmountAfterUnstake(user, i);
        }
