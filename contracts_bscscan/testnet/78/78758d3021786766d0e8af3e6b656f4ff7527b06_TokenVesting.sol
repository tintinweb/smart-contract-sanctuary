...
contract TokenVesting is Ownable, ReentrancyGuard {
  ...
  constructor (ICashsafeAdmin _safeAdmins) {
    CASH_ADMINS = _safeAdmins;
    FEES.tokenFee = 35;
    FEES.feeAddress = payable(0x2854fd5E2FcE375D931b3a17FB53d70d2b681BaE);
  ...
  // Callable by CASH_ADMINS or whitelisted contracts (such as presale contracts)
  function editZeroFeeWhitelist (address _token, bool _add) external {
    require(CASH_ADMINS.userIsAdmin(msg.sender) || TOKEN_WHITELISTERS.contains(msg.sender), 'ADMIN');
    if (_add) {
      ZERO_FEE_WHITELIST.add(_token);
    } else {
