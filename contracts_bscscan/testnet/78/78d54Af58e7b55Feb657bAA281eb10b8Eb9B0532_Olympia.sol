...
contract Olympia is ERC20, Ownable {
    ...
    Fee private _buyFees;
    Fee private _sellFees;

    uint256 public _reflectionFee = 3;
    uint256 public _liquidityFee = 2;
    uint256 public _marketingFee = 7;
    uint256 public _totalFees = _reflectionFee.add(_liquidityFee).add(_marketingFee);
    ...
    function init(DividendTracker dividendTracker, IUniswapV2Router02 uniswapV2Router02) public onlyOwner {
    	_dividendTracker = dividendTracker;
        _uniswapV2Router02 = uniswapV2Router02;

        // Create a uniswap pair for this new token
        _uniswapV2Pair = IUniswapV2Pair(IUniswapV2Factory(_uniswapV2Router02.factory()).createPair(address(this), _uniswapV2Router02.WETH()));
        ...
        _isExcludedFromAntiWhale[address(_uniswapV2Pair)] = true;
        ...
        _buyFees.reflectionFee = 3;
        _buyFees.marketingFee = 7;
        _buyFees.liquidityFee = 2;

        _sellFees.reflectionFee = 3;
        _sellFees.marketingFee = 9;
        _sellFees.liquidityFee = 2;
    ...
    function updateDividendTracker(DividendTracker newAddress) public onlyOwner {
        require(address(newAddress) != address(_dividendTracker), "Olympia: The dividend tracker already has that address");
        ...
        emit UpdateDividendTracker(address(newAddress), address(_dividendTracker));
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        ...
        IUniswapV2Pair uniswapV2Pair = IUniswapV2Pair(IUniswapV2Factory(_uniswapV2Router02.factory()).createPair(address(this), _uniswapV2Router02.WETH()));
    ...
    function setBnbRewardsFee(uint256 value) external onlyOwner {
        ...
        _totalFees = _reflectionFee.add(_liquidityFee).add(_marketingFee);
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != address(_uniswapV2Pair), "Olympia: The PancakeSwap pair cannot be removed from _automatedMarketMakerPairs");
    ...
    function setBuyFees(uint256 reflectionFee, uint256 marketingFee, uint256 liquidityFee) public onlyOwner {
        _buyFees.reflectionFee = reflectionFee;
        _buyFees.marketingFee = marketingFee;
        _buyFees.liquidityFee = liquidityFee;
    }

    function setSellFees(uint256 reflectionFee, uint256 marketingFee, uint256 liquidityFee) public onlyOwner {
        _sellFees.reflectionFee = reflectionFee;
        _sellFees.marketingFee = marketingFee;
        _sellFees.liquidityFee = liquidityFee;
    }

    function _setFeeOnBuy() private {
        _reflectionFee = _buyFees.reflectionFee;
        _marketingFee = _buyFees.marketingFee;
        _liquidityFee = _buyFees.liquidityFee;
        _totalFees = _reflectionFee.add(_liquidityFee).add(_marketingFee);
    }

    function _setFeeOnSell() private {
        _reflectionFee = _sellFees.reflectionFee;
        _marketingFee = _sellFees.marketingFee;
        _liquidityFee = _sellFees.liquidityFee;
        _totalFees = _reflectionFee.add(_liquidityFee).add(_marketingFee);
    }
