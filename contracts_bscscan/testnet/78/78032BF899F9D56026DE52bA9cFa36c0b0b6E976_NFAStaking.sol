...
contract NFAStaking is Ownable, Initializable {
    using SafeERC20 for IERC20;

    // Info of each user.
    struct UserInfo {
        uint256 amount;         // How many NFTs tokens the user has provided.
        uint256[] stakedIds;    // IDs of staked NFTs.
        uint256 rewardDebt;     // Reward debt. See explanation below.
    }
    mapping (uint256 => uint256) public userNfts;
    ...
    /// @param _ids The NFT ids to deposit
    function deposit(uint256[] calldata _ids) public {
        ...
        if (user.amount > 0) {
         ...
         for (uint256 i=0; i<_ids.length; i++) {
            NFT_CONTRACT.transferFrom(address(msg.sender), address(this), _ids[i]);
            user.stakedIds[user.amount] = _ids[i];
            userNfts[_ids[i]] = user.amount;
            user.amount++;
            totalStaked++;
        }
    ...
    /// @param _ids The nft ids to deposit
    function withdraw(uint256[] calldata _ids) public {
        UserInfo storage user = userInfo[msg.sender];
        ...
        for (uint256 i=0; i<_ids.length; i++) {
            require(user.stakedIds[userNfts[_ids[i]]] == _ids[i], "Unauthorizd id");
            NFT_CONTRACT.transferFrom(address(this), address(msg.sender), _ids[i]);
            user.amount--;
            totalStaked--;            
            delete user.stakedIds[userNfts[_ids[i]]];
            delete userNfts[_ids[i]];
        }
    ...
    function emergencyWithdraw(uint256[] calldata _ids) external {
        UserInfo storage user = userInfo[msg.sender];
        for (uint256 i=0; i<_ids.length; i++) {
            require(user.stakedIds[userNfts[_ids[i]]] == _ids[i], "Unauthorizd id");
            NFT_CONTRACT.transferFrom(address(this), address(msg.sender), _ids[i]);
