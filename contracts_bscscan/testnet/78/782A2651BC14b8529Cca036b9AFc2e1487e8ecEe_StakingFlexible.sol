...
contract StakingFlexible is Pausable, Ownable {
    using SafeERC20 for IERC20;

    // store information about users stakes
    mapping(address => StakeInfo[]) public usersStake;

    uint256 public emissionPerSecond; // total emission per second for all options
    ...
    constructor(address _stakeToken, uint256 _initEpd) {
        stakeToken = _stakeToken;
        emissionPerSecond = _initEpd / 24 / 60 / 60;
    }
    ...
    function freeAmount() public view returns (uint256) {
        return IERC20(stakeToken).balanceOf(address(this)) - totalStaked;
    }
    ...
    /// @param amount - amount of tokens,
    function stakeFor(uint256 amount, address account) external onlyOwner {
        require(amount > 0, "!amount");
        IERC20(stakeToken).safeTransferFrom(msg.sender, address(this), amount);
        _stakeFor(amount, account);
    }
    ...
    function _stakeFor(uint256 amount, address account) internal {
        _updateIndex();
    ...
    function withdraw(uint16 stakeIndex) external whenNotPaused {
        ...
        (uint256 rewards, , bool withdrawable, ) = calcRewards(s);
    ...
    function claimRewards(uint16 stakeIndex) external whenNotPaused {
        StakeInfo[] storage stakeInfoList = usersStake[msg.sender];
        require(stakeInfoList.length > stakeIndex, "!index");
        ...
        (uint256 rewards, bool claimable, , ) = calcRewards(s);
    ...
    function setTotalEmissionPerSecond(uint256 emission) external onlyOwner {
        _updateIndex();
    ...
    function withdrawExtraTokens(address token, uint256 amount)
        ...
        onlyOwner
    {
        if (token == stakeToken) require(amount <= freeAmount(), "!free");
        IERC20(token).safeTransfer(msg.sender, amount);
    }

    /// @notice calculate rewards and check if user can claim/withdraw tokens
    function calcRewards(StakeInfo memory s)
        internal
        view
        returns (
            uint256 rewards,
            bool claimable,
            bool withdrawable,
            uint256 endOfLastPeriod
        )
    {
        rewards = ((cumulativeIndex - s.lastCI) * s.amount) / 10**18;

        endOfLastPeriod = block.timestamp;
        claimable = rewards > 0;
        withdrawable = true;

        if (rewards > s.claimed) rewards -= s.claimed;
        else rewards = 0;

        claimable = rewards > 0;

        console.log(
            "rewards: %s, withdrawable: %s, claimable: %s",
            rewards,
            withdrawable,
            claimable
        );
    ...
    function calcRewardsByIndex(address stakingOf, uint16 stakeIndex)
        public
        view
        returns (
            uint256 rewards,
    ...
    {
        return calcRewards(usersStake[stakingOf][stakeIndex]);
    }

    /// @notice Calculate cumulative index
    function calculateCumulativeIndex() public view returns (uint256 index) {
        if (totalStaked > 0) {
            index =
                cumulativeIndex +
    ...
    function _updateIndex() internal {
        cumulativeIndex = calculateCumulativeIndex();
