...
contract ForeverBNB1 {
    ...
    constructor(address v1, uint lastUserId) {
        
        foreverV1 = ForeverV1(v1);
        ...
        userCurrentId = lastUserId;
        oldUsersLastId = lastUserId;
    ...
    function updateLastId(uint _last) external onlyOwner returns(bool) {
        oldUsersLastId = _last;
        return true;
    }
    ...
    function compoundDeposit() external isLock payable {
        ...
        if(oldExist == true) {
            ...
            (, , , _endROITime,) = foreverV1.viewCompoundDetails(msg.sender); 
        }
        
        else {
            _refAddr = users[msg.sender].referrerAddress;
            _endROITime = compoundDetails[msg.sender].endROITime;
        }
        ...
        for(uint8 i=0; i<5; i++) {
            ...
            (oldExist, , , , oldReferrerAddress , ,  ) = foreverV1.users(_refAddr);
