...
───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
   ...
   #A - 🅰🆃🅻🅰🅽🆃🅸🆂 🅱🆂🅲 - Welcome to the future of AR crypto gaming:
   3.5% fee is taken to fund the creation of Atlantis studios
   3.5% fee is held in trust, and will be used to provide value into the game world (treasures)
   3% fee automatically distributed to all holders
   Giant Whale protections for the benefit of the long term holders and unique anti-bot code. We still want good whales
   
   change-log:
   
   tax is static but can be modified, its capped on 5%
   liquidity or, for this contract studio/game fee can also be changed, but its capped on 10%
   pancake router has setter function for easy migration
   i left mess in important variables, like total supply, and so on, it needs to be changed
   decimals are now 18 but everything that was using decimals for modulo is replaced with 2 
   there is a small warning on setTax function, that can be ignored
   
   PLEASE, nothing is guaranteed, DYOR before launching!
...
contract Atlantis is Context, IERC20, Ownable {
    ...
    uint256 private constant MAX = ~uint256(0); // keep it positive 
    uint256 private _tTotal = 200000000000 * 10**_decimals; // total supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal)); // wallet holdings
    uint256 private _tFeeTotal;

    string private _name = "AtlantisBSC";
    ...
    uint256 private minTokensBeforeSwap = 300000 * 10**_decimals; // 300k for lower supply

    uint256 startDate;
    address private _pancakeRouter = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; //testnet
    ...
    constructor () {
        _rOwned[_msgSender()] = _rTotal;
         ...
         IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_pancakeRouter); // Pancake
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        ...
        // using for loop can be problematic if excluded list is long 
        // in general, just subing excluded from reward reflection
        for (uint256 i = 0; i < _excluded.length; i++) {
