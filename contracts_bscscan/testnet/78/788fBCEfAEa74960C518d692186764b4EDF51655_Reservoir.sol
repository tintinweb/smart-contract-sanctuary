...
// File: contracts/interfaces/IDoobBond.sol
...
interface IDoobBond {
    ...
    function doobValue(uint256 amount_) external view returns (uint256 value_, address token_);
}
// File: contracts/interfaces/IDoob.sol
...
contract Reservoir is IReservoir, Ownable {
    ...
    address public sDoob;
    uint256 public sDoobQueue; // Delays change to sDoob address
    ...
    constructor(
        ...
        require(doob_ != address(0), "Doob undefined");
        doob = doob_;
    ...
    /**
        @notice allow approved address to deposit an asset for doob
        @param amount_ uint
        @param profit_ uint
        @return send_ uint
     */
    ...
    {
        (uint256 value, address token) = IDoobBond(msg.sender).doobValue(amount_);
    ...
    function deposit(
        ...
        uint256 value = _doobValueOf(token_, amount_);
    ...
    function _deposit(
        ...
        // mint doob needed and store amount of rewards for distribution
        send_ = value.sub(profit_);
        IERC20Mintable(doob).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn doob for collateral tokens
        @param bond_ address
        @param amount_ uint
     */
    function withdraw(address bond_, uint256 amount_) external isSpender() validate(bond_) {
        (uint256 value, address token) = IDoobBond(bond_).doobValue(amount_);
        require(_listContains(collateralTokens, token), "Collateral token only");

        IDoob(doob).burnFrom(msg.sender, value);
    ...
    function incurDebt(address bond_, uint256 amount_) external isDebtor() validate(bond_) {
        require(sDoob != address(0), "sDoob undefined");

        (uint256 value, address token) = IDoobBond(bond_).doobValue(amount_);
        require(_listContains(collateralTokens, token), "Collateral token only");

        uint256 maximumDebt = IERC20(sDoob).balanceOf(msg.sender); // Can only borrow against sDoob held
    ...
    /**
        @notice allow approved address to repay borrowed collaterals with doob
        @param amount_ uint
     */
    function repayDebtWithDoob(uint256 amount_) external isDebtor() {
        IDoob(doob).burnFrom(msg.sender, amount_);

        debtorBalances[msg.sender] = debtorBalances[msg.sender].sub(amount_);
        totalDebt = totalDebt.sub(amount_);

        emit LogRepayDebt(msg.sender, doob, amount_, amount_);
    }
    ...
    function manage(address bond_, uint256 amount_) external validate(bond_) {
        if (IDoobBond(bond_).isLiquidityBond()) {
    ...
    function mintRewards(address recipient_, uint256 amount_) external override isRewardManager() returns (uint256) {
        ...
        IERC20Mintable(doob).mint(recipient_, amount_);
    ...
    function excessAssets() public view returns (uint256) {
        uint256 net = IERC20(doob).totalSupply().sub(totalDebt);
    ...
    function auditAssets() external onlyOwner() {
        ...
        for (uint256 i = 0; i < bonds.length; i++) {
            IDoobBond bond = IDoobBond(bonds[i]);
            (value, ) = bond.doobValue(IERC20(bond.tokenIn()).balanceOf(address(this)));
            assets = assets.add(value);
        }
        for (uint256 i = 0; i < collateralTokens.length; i++) {
            value = _doobValueOf(collateralTokens[i], IERC20(collateralTokens[i]).balanceOf(address(this)));
            assets = assets.add(value);
        }
        totalAssets = assets;
    ...
    function queue(TARGETS target_, address address_) external onlyOwner() returns (bool) {
        ...
        } else if (target_ == TARGETS.S_DOOB) {
            // 8
            sDoobQueue = block.number.add(queueLength);
        } else return false;
     ...
     */
    function _doobValueOf(address token_, uint256 amount_) internal view returns (uint256 value_) {
        value_ = amount_.mul(10**IERC20(doob).decimals()).div(10**IERC20(token_).decimals());
    }
}