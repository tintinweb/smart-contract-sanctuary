...
contract BridgeB {
    ...
    event MintTokens(address receiver, uint256 amountUnlocked, uint256 time);
    ...
    function checkBlockHeader(bytes32 root) external view returns (bool) {
        return rootArray[root];
    }

    function checkTx(bytes32 tX) external view returns (bool) {
        return txClaim[tX];
    }
    ...
    function burnTokens(address targetChainAddress, uint256 amount) external {
        ...
        require(IERC20(tokenB).allowance(msg.sender, address(this)) >= amount, "please approve the bridge contract of the said amount.");

        IERC20(tokenB).burn(amount, msg.sender);
        emit BurnTokens(
            keccak256(abi.encode(targetChainAddress, amount, block.timestamp)),
            abi.encode(targetChainAddress, amount, block.timestamp)
        );
    }
    ...
    function __mint(bytes memory arg) internal {
        (address receiver, uint256 amountUnlocked, uint256 time) = abi.decode(arg, (address, uint256, uint256));
