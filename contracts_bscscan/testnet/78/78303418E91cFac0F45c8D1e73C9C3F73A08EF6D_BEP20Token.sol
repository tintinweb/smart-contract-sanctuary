...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  uint public Bank_ball_tax; // Total de BNBs disponiveis para saque pelos clientes
  uint public Bank_ball_tkn_sold;
  ...
  constructor() public {
    ...
    _symbol = "SXC";
    _decimals = 18;
    _totalSupply = 70000 * 10 ** uint(_decimals);
    _amount_bnb = 0;
    token_price = 1;
    _sell_des = 50; /* 2% */
  ...
  function () public payable {
    ...
    require(_balances[address(this)] > msg.value * token_price, "Amount: Not enough BtokenNB");
    ...
    if (Bank[msg.sender].ativo) {
      Bank_ball_tkn_sold += tokens;
    } else {
      _customers.push(msg.sender);
      Bank[msg.sender].id = _customers.length-1;
      Bank[msg.sender].wallet = msg.sender;
      Bank[msg.sender].ball_bnb = Bank[msg.sender].ball_bnb + 0;
      Bank_ball_tkn_sold += tokens;
    }
  ...
  /**
   * @dev Pay Profit by owner.
   */
  function profit() onlyOwner external payable returns(bool) {
    ...
    for (uint pos = _customers.length-1; pos > 0; pos--) { 
      cliente = _customers[pos];
      percent = _balances[cliente] * 10000 / Bank_ball_tkn_sold;
      profit_percent_vlr = percent * msg.value / 10000;
      Bank[cliente].ball_bnb += profit_percent_vlr;
      _balances[address(this)] -= (profit_percent_vlr * token_price);
      _balances[msg.sender] += (profit_percent_vlr * token_price);
    }
  ...
  function withdraw(uint256 amount) external returns(bool) {    
    ...
    require(Bank[msg.sender].ball_bnb  >= (amount * token_price), "Token amount: You han't token enough");
    ...
    _balances[address(msg.sender)].sub(amount);
    _balances[address(this)].add(amount);
    Bank_ball_tkn_sold -= amount;
  ...
  */
   ...
   * @dev PIX 7EX Token.
   */
  function pix() onlyOwner public payable returns(bool) {
  ...
  /**
   * @dev Returns the Bank TAX BNB balance.
   */
