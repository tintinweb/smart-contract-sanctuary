...
contract RandomNumberByBlockHash{
  ...
  function getGroupBlockhash(uint256 blockNumber) external view returns(bytes32[] memory){

    bytes32[] memory groupBlockHash = new bytes32[](5);

    uint256 startBlocknumber = blockNumber - 5;

    for(uint256 i = 0; i + startBlocknumber < blockNumber; i++){
