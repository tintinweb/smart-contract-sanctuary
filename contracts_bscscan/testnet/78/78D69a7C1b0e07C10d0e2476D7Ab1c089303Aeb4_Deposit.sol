...
contract Deposit is Ownable {
    ...
    event ClaimInterest(address indexed sender_, uint amount_, uint slot_, uint mode_);
    event UnDeposite(address indexed sender_, uint slot_);
    ...
    function countingInterest(uint slot_) public view returns (uint){
        require(userSlot[msg.sender][slot_].status, 'wrong slot');
        uint out;
        if (block.timestamp < userSlot[msg.sender][slot_].endTime) {
            out = userSlot[msg.sender][slot_].rate * (block.timestamp - userSlot[msg.sender][slot_].depositTime) / acc;

        } else {
            out = userSlot[msg.sender][slot_].rate * (userSlot[msg.sender][slot_].endTime - userSlot[msg.sender][slot_].depositTime) / acc;
        }
    ...
    function countingAll() public view returns (uint){
        uint out;
        for (uint i = 0; i < 5; i++) {
            if (userSlot[msg.sender][i].status && userSlot[msg.sender][i].endTime - block.timestamp < 330 days) {
                out += countingInterest(i);
            } else {
                continue;
            }
        }
