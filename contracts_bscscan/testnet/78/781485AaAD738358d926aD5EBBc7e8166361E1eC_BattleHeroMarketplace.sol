...
contract BattleHeroMarketplace is
    ...
    enum AcceptedToken {BATH, USDT, BNB}
    struct Auction{
    ...
    constructor(address erc721, address usdt, address bath){
    ...
    modifier isSetup() {
        ...
        require(address(_usdt) != address(0), "Setup not correctly");        
        require(address(_bath) != address(0), "Setup not correctly");        
        _;
    }
    function setERC721(address erc721) public{
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()));
        _erc721 = IBattleHeroFactory(erc721);
    }
    function setUSDT(address usdt) public{
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()));
        _usdt = USDT(usdt);
    }
    ...
    function buyNFT(uint256 index) isSetup public payable  returns(bool){       
        Auction memory auction = auctions[index];
        require(isSelling(auctions[index].hero.index) == true, "Currently not selling");   
        uint256 fee = (auctions[index].price / 100) * 5;   
        _erc721.unlockHero(auction.hero.index);        
        if(auctions[index].token == AcceptedToken.USDT){
            require(_usdt.allowance(msg.sender, address(this)) > auctions[index].price, "Insufficient funds USDT");
            // Transfer 95% USDT to owner of NFT
            _usdt.transferFrom(msg.sender, auctions[index].hero.owner, auctions[index].price - fee);
            // Transfer 5% USDT of fees to fee address
            _usdt.transferFrom(msg.sender, _feeAddress, fee); 
            // Transfer NFT to buyer
            _erc721.transferFrom(auctions[index].hero.owner, msg.sender, auctions[index].hero.index);
        ...
        if(auctions[index].token == AcceptedToken.BNB){
            require(msg.value > auctions[index].price, "Insufficient funds BNB");
            // Transfer 95% BNB to owner of NFT
            payable(auctions[index].hero.owner).transfer(msg.value - fee);
            // Transfer 5% BNB to owner of NFT
    ...
    function isSelling(uint256 tokenId) public view returns(bool){        
        ...
        for(uint i = 0; i <= auctions.length - 1; i++){
            if(auctions[i].hero.index == tokenId){
                exists = true;
            }            
        }
        return exists;
