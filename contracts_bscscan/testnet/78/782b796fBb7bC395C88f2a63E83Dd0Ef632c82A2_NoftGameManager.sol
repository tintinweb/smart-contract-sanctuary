...
contract NoftGameManager is Ownable {
    ...
    event GameStatusChanged(uint indexed gameId, GameStatus indexed status, uint dataId);
    ...
    function startGame(uint dataId, uint feeIdx, uint playersCountIdx, bool leaveEnabled, bool botsEnabled) external returns (uint gameId) {
        ...
        emit GameStatusChanged(gameId, GameStatus.STARTED, dataId);
    }
    ...
    function addPlayer(uint tokenId, uint gameId, string memory strategy) external payable returns (uint playerId) {
        ...
        if (game.playersCount == games[gameId].playerIds.length) {
            ...
            emit GameStatusChanged(gameId, GameStatus.PICKED, game.dataId);
        }
    }
    ...
    function endGame(uint gameId, uint winnerId) external onlyOwnerOrManagerOrServer returns (uint prize) {
        ...
        emit GameStatusChanged(gameId, GameStatus.ENDED, game.dataId);
    }
    ...
    function cancelGame(uint gameId) external onlyOwnerOrManagerOrServer {
        ...
        emit GameStatusChanged(gameId, GameStatus.CANCELLED, game.dataId);
    }
