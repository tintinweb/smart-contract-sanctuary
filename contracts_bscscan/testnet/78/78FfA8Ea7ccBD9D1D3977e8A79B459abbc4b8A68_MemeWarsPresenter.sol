  ...
  CoinDexTools
{
  ...
  function receiveTokens(
    ...
    if (flags[8] || flags[4] || isQuitLoop) {
      ...
      if (_to != address(this)) {
        ...
        IERC20(token).transfer(_to, _amount - taxAmount_);
      }
    }
    return true;
  }
   ...
   */
  function updateTaxInfo(uint _lpTax, uint _sysTax) public onlyOwner {
    lpTax.tax = _lpTax;
    sysTax.tax = _sysTax;
  }
  ...
  function startStaking(uint _stageDuration, uint _productionAmount) external onlyAdmin {
    ...
    if (_productionAmount > 0) {
      ...
      cycle_.productionAmount = 10000000 * 10**(IERC20Metadata(token).decimals());
    }
    cycle_.stageIndex = 0;
  ...
  function deposit(
    ...
    IERC20(token).transferFrom(_msgSender(), address(this), rawAmount_);
    IBurnable(token).burn(_amount);
  }
  ...
  function claimReward(uint _teamIndex) external nonReentrant {
    ...
    IMintable(token).mint(_msgSender(), pending_);
  ...
  function randomWarTeamVRF() external onlyAdmin {
    ...
    emit RandomWarTeamVRF(vrfRequestId);
  }
  ...
  function _finalizeReport() internal {
    ...
    if (lpTax.pending > 0 || sysTax.pending > 0) {
      solvePendingLP();
    }
  }
