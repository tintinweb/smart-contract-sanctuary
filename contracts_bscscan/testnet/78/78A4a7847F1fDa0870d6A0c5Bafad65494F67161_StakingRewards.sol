...
contract StakingRewards is IStakingRewards, Ownable ,ReentrancyGuard {
    ...
    function deposit(uint256 _pid, uint256 _amount) nonReentrant isActive public override {
        ...
        if (user.amount > 0) _harvest(_for, _pid, true);
    ...
    function harvest(uint256 _pid) public override {
        updatePool(_pid);
        _harvest(msg.sender, _pid, false);
    }


    function _harvest(address _to, uint256 _pid, bool restake) internal {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        // if re-stake then add reward into current staking amount
        if(restake) {
            user.amount = user.amount.add(_reward);
            pool.totalStakingAmount = pool.totalStakingAmount.add(_reward);
        } else {
