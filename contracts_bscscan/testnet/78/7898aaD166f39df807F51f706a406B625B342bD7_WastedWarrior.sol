...
contract WastedWarrior is  IWastedWarrior, ERC721Upgradeable, ReentrancyGuardUpgradeable, AcceptedToken {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using EnumerableSet for EnumerableSet.UintSet;
    using SafeMathUpgradeable for uint;
    
    modifier onlyWarriorOwner(uint warriorId) {
        require(ownerOf(warriorId) == msg.sender, "WAW: Caller isn't owner of warrior");
        _;
    }

    modifier onlyNotBlacklist(uint warriorId) {
    ...
    uint public marketFeeInPercent;
    uint public serviceFeeInToken;
    uint public plasticPackageFee;
    uint public steelPackageFee;
    uint public goldPackageFee;
    uint public platinumPackageFee;
    ...
    function initialize (
        ...
        __ERC721_init("WastedWarrior", "WAW");
        ...
        steelPackageFee = 0.15 * 1e18;
        goldPackageFee = 0.2 * 1e18;
        platinumPackageFee = 0.3 * 1e18;
        maxLevel = 100;
    }


    function addWarriorToBlacklist(uint warriorId) external onlyOwner {
    ...
    function removeWarriorFromBlacklist(uint warriorId) external onlyOwner {
        require(warriorId < _warriors.length, "WAW: warrior invalid");
    ...
    */
    function getPlasticPackageFee() external override view returns(uint) {
    ...
    */
    function getSteelPackageFee() external override view returns(uint) {
    ...
    */
    function getGoldPackageFee() external override view returns(uint) {
    ...
    */
    function getPlatinumPackageFee() external override view returns(uint) {
        return platinumPackageFee;
    }

    function getInfoCollaborator(address addressCollab) external override view returns(Collaborator memory) {
    ...
    function listing(uint warriorId, uint price) external override onlyNotBlacklist(warriorId) onlyWarriorOwner(warriorId) {
        Warrior memory warrior = _warriors[warriorId];
        require(price > 0, "WAW: invalid price");
        require(warrior.weapon == 0 && warrior.armor == 0 && warrior.accessory == 0, "WAW: remove equipment first");
    ...
    function _createWarrior(bool isBreed, bool isFusion, uint packageType, address buyer) private returns (uint warriorId) {
    ...
    function _removeWastedEquipment(uint warriorId, uint[] memory itemIds) private {
        require(warriorsOnSale[warriorId] == 0, "WAW: cannot change items while on sale");
        ...
        for (uint i = 0; i < itemIds.length; i++) {
            ...
            require(!itemSet[uint(itemType)], "WAW: duplicate itemType");
    ...
    function _setWastedEquipment(uint warriorId, uint[] memory itemIds) private {
        ...
        for (uint i = 0; i < itemIds.length; i++) {
            ...
            require(itemId != 0, "WAW: invalid itemId");
            require(!itemSet[uint(itemType)], "WAW: duplicated");
            require(wastedEquipment.ownerOf(itemId) == msg.sender, "WAW: only owner");

            if (itemType == IWastedEquipment.ItemType.WEAPON) {
                require(warrior.weapon == 0, "WAW:weapon is equipped");
                warrior.weapon = itemId;
                itemSet[uint(IWastedEquipment.ItemType.WEAPON)] = true;
            } else if (itemType == IWastedEquipment.ItemType.ARMOR) {
                require(warrior.armor == 0, "WAW: armor is equipped");
                warrior.armor = itemId;
                itemSet[uint(IWastedEquipment.ItemType.ARMOR)] = true;
            } else if (itemType == IWastedEquipment.ItemType.ACCESSORY) {
                require(warrior.accessory == 0, "WAW:accessory is equipped");
                warrior.accessory = itemId;
                itemSet[uint(IWastedEquipment.ItemType.ACCESSORY)] = true;
            } else {
                require(false, "WAW: invalid item");
            }
        }
    }
    
    function updateCollaborator(address collaborator, uint totalSupplyPlasticPackages, uint totalSupplySteelPackages, uint totalSupplyGoldPackages, uint totalSupplyPlatinumPackages) external onlyOwner {
        require(totalSupplyPlasticPackages != 0 && totalSupplySteelPackages != 0 && totalSupplyGoldPackages != 0 && totalSupplyPlatinumPackages != 0, "WAW: invalid" );
        ...
        collabInfo.totalSupplySteelPackages = totalSupplyPlasticPackages;
        collabInfo.totalSupplyGoldPackages = totalSupplyPlasticPackages;
        collabInfo.totalSupplyPlatinumPackages = totalSupplyPlasticPackages;
    }
    
    function addCollaborator(address collaborator, uint totalSupplyPlasticPackages, uint totalSupplySteelPackages, uint totalSupplyGoldPackages, uint totalSupplyPlatinumPackages) external override onlyOwner {
        require(totalSupplyPlasticPackages != 0 && totalSupplySteelPackages != 0 && totalSupplyGoldPackages != 0 && totalSupplyPlatinumPackages != 0, "WAW: invalid" );
        
        collaborators[collaborator] = Collaborator(totalSupplyPlasticPackages, totalSupplySteelPackages, totalSupplyGoldPackages, totalSupplyPlatinumPackages, 0, 0, 0, 0);
    }
    ...
    function buy(uint warriorId) external override payable onlyNotBlacklist(warriorId) nonReentrant {
    ...
    function offer(uint warriorId, uint offerPrice) external override onlyNotBlacklist(warriorId) nonReentrant payable {
        address buyer = msg.sender;
        ...
        require(msg.value == requiredValue, "WAW: sent value invalid");
    ...
    function abortOffer(uint warriorId) external override nonReentrant {
        ...
        require(offerPrice > 0, "WAW: offer not found");
    ...
    function levelUp(uint warriorId, uint amount) external override onlyOperator {
        Warrior storage warrior = _warriors[warriorId];
        uint newLevel = warrior.level.add(amount);

        require(amount > 0);
        require(newLevel <= maxLevel, "WAW: max level reached");
    ...
    function adoptPet(uint warriorId, uint petId) external override onlyNotBlacklist(warriorId) onlyWarriorOwner(warriorId) {
        require(wastedPet.ownerOf(petId) == msg.sender, "WAW: not pet owner");
        require(_warriorsWithPet[warriorId] == 0, "WAW: abandonPet before");
    ...
    function abandonPet(uint warriorId) external override onlyNotBlacklist(warriorId) onlyWarriorOwner(warriorId) {
    ...
    function rename( uint warriorId, string memory replaceName ) external override onlyNotBlacklist(warriorId) onlyWarriorOwner(warriorId) collectTokenAsFee(serviceFeeInToken, owner()) {
        require(_validateStr(replaceName), "WAW: invalid name");
        require(usedNames[replaceName] == false, "WAW: name already exist");
    ...
    function equipItems(uint warriorId, uint[] memory itemIds) external override onlyNotBlacklist(warriorId) onlyWarriorOwner(warriorId) {
    ...
    function removeItems(uint warriorId, uint[] memory itemIds) external override onlyNotBlacklist(warriorId) onlyWarriorOwner(warriorId) {
    ...
    function mintFor(address buyer, uint amount, uint rarityPackage) external override {
        Collaborator storage collabInfo = collaborators[msg.sender];

        require(rarityPackage >= 1 && rarityPackage <= 4, "WAW: invalid Package");

        if(rarityPackage == uint(IWastedWarrior.PackageRarity.PLASTIC)) {
            require(collabInfo.mintedPlasticPackages.add(amount) <= collabInfo.totalSupplyPlasticPackages, "WAW: sold out");
            collabInfo.mintedPlasticPackages = collabInfo.mintedPlasticPackages.add(amount);
        } else if(rarityPackage == uint(IWastedWarrior.PackageRarity.STEEL)) {
            require(collabInfo.mintedSteelPackages.add(amount) <= collabInfo.totalSupplySteelPackages, "WAW: sold out");
            collabInfo.mintedSteelPackages = collabInfo.mintedSteelPackages.add(amount);
        } else if (rarityPackage == uint(IWastedWarrior.PackageRarity.GOLD)) {
            require(collabInfo.mintedGoldPackages.add(amount) <= collabInfo.totalSupplyGoldPackages, "WAW: sold out");
            collabInfo.mintedGoldPackages = collabInfo.mintedGoldPackages.add(amount);
        } else if (rarityPackage == uint(IWastedWarrior.PackageRarity.PLATINUM)) {
            require(collabInfo.mintedPlatinumPackages.add(amount) <= collabInfo.totalSupplyPlatinumPackages, "WAW: sold out");
            collabInfo.mintedPlatinumPackages = collabInfo.mintedPlatinumPackages.add(amount);
        }

        
        for (uint i = 0; i < amount; i++) {
            uint warriorId = _createWarrior(false, false, rarityPackage, buyer);
    ...
    function breedingWarrior(uint fatherId, uint motherId, address owner) external onlyOperator override  {
        require(fatherId != motherId);
        require(ownerOf(fatherId) == owner && ownerOf(motherId) == owner);
        
        uint warriorId = _createWarrior(true, false, 0, owner);
        _safeMint(owner, warriorId);

        emit BreedingWarrior(fatherId, motherId, warriorId, owner);
    }
    
    function fusionWarrior(uint firstWarriorId, uint secondWarriorId, address owner) external override onlyOperator {
        require(firstWarriorId != secondWarriorId);
        require(ownerOf(firstWarriorId) == owner && ownerOf(secondWarriorId) == owner);
        
        uint warriorId = _createWarrior(false, true, 0, owner);
        _safeMint(owner, warriorId);
