    ...
    PausableUpgradeable
{
    ...
    function currentRound() public view returns (uint256) {
        ...
        return
            (block.timestamp.sub(saleStartTime)).div(ROUND_TIME).mul(100).div(
                100
            );
    }

    function endRoundTime(uint256 round) public view returns (uint256) {
        return saleStartTime.add(round.mul(ROUND_TIME));
    }

    function withdrawTimeLeft(uint256 round) public view returns (uint256) {
        uint256 withdrawTime = saleStartTime.add(round.mul(ROUND_TIME));
    ...
    function canWithdraw(uint256 round) public view returns (bool) {
        return
            withdrawTimeLeft(round) == 0 &&
            !depositTable[round][msg.sender].claimed;
    }
    ...
    function getOrderByRound(
        ...
        if (length > lastOrderIds[round] - cursor) {
            length = lastOrderIds[round] - cursor;
        }
        Order[] memory values = new Order[](length);
        for (uint256 i = 0; i < length; i++) {
            address addressByIndex = indexes[round][i];
    ...
    function getAddressIndexes(uint256 round, uint256 i)
        public
        view
        returns (address)
    {
        return indexes[round][i];
    }
    ...
    function deposit(uint256 round)
        public
        payable
        onlyNonContract
        whenNotPaused
    ...
    function withdraw(uint256 round) public onlyNonContract whenNotPaused {
        require(canWithdraw(round), "Cannot withdraw");
        ...
        if (token.allowance(address(this), msg.sender) == 0) {
            token.approve(msg.sender, ~uint256(0));
        }
        Order storage order = depositTable[round][msg.sender];
        order.claimed = true;
    ...
    function initialPriceInRound(uint256 round) public view returns (uint256) {
        ...
        return
            lastRoundPrice.sub(firstRoundPrice).mul(MAX_ROUND.sub(MIN_ROUND)) +
            initialPriceInRound(round - 1);
    }

    function maxVolumeInRound(uint256 round) public view returns (uint256) {
        return AMOUNT_TOKEN_SALE_PER_ROUND.mul(initialPriceInRound(round));
    }
    ...
    {
        return
            depositTable[round][account].depositValue.div(
                initialPriceInRound(round)
            );
    }
...

{
  ...
  "libraries": {
    "/Users/tamnguyen/Documents/project/xbn/xblade-token-distribution/contracts/lib/Utils.sol": {
      "Utils": "0x7616501B59A69c81d015eFE9247AE3F1CC339914"
    }
  },
  "outputSelection": {
    "*": {
