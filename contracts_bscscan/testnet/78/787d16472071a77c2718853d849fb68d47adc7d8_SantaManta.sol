...
contract SantaManta is IBEP20, Auth {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
       ...
       function shouldSetShares(address account) private view returns (bool) {
    ...
    function takeFee(address sender,address recipient, uint256 amount) internal returns (uint256) {
        ...
        function getTotalTax(address sender, address recipient) private view returns (uint256) {
        if (sender == pair) {
            return buyTax;
        } else if (recipient == pair) {
            if(inCreaseFactorEnabled){
            return sellTax.mul(inCreaseFactor);
            }else{
            return sellTax;}
        } else {
            return transferTax;
