...
contract Staking is AccessControl, ReentrancyGuard {
    ...
    */
    ...
    function _calculateReward(
        ...
        rewardAmount =  (userSign.amount * pools[_poolId].APY * (block.timestamp - userSign.start))/(100*365); // /(100*60*60*24*365);
        if( _poolId == 2 || _poolId == 3 || _poolId == 4 || _poolId == 11 || _poolId == 10 || _poolId == 9) rewardAmount = rewardAmount/10000000000;
    ...
    function getProcessInfoForUser(address _investor, uint8 _poolId)
        external
        view
        returns (uint256 amount, uint256 rewardGot, uint256 start)
    {
        UserInfo storage userSign = processForInvestor[_investor][_poolId];
        ...
        nonReentrant
    {
        ...
        require( processForInvestor[investor][_poolId].amount == 0, "Staking: stake exists");
        ...
        if( poolSign.timeLockUp > 0) {
            processForInvestor[investor][_poolId].timeLocked[0] = block.timestamp;
    ...
    function removePartOfStake(
        ...
        uint8 _poolId,
        uint256 _amount
    ) external nonReentrant onlyHolderOrAdmin(_investor) {
        ...
        nonReentrant
    {
        ...
        userSign.amount += _amount;
        poolSign.amountStaked += _amount;
        userSign.rewardGot = userSign.rewardGot + calculateReward(_poolId, _amount, userSign.start);
    ...
    function calculateReward(
        ...
        rewardAmount =
            (_amount * pools[_poolId].APY * (block.timestamp - _start))/(100*365); // /(100*60*60*24*365);
        if( _poolId == 2 || _poolId == 3 || _poolId == 4 || _poolId == 11 || _poolId == 10 || _poolId == 9) rewardAmount = rewardAmount/10000000000;
    }
    ...
    function unlock(uint8 _poolId, address _investor, uint256 i) internal returns(uint index) {
        UserInfo storage userSign = processForInvestor[_investor][_poolId];
        PoolInfo storage poolSign = pools[_poolId];
