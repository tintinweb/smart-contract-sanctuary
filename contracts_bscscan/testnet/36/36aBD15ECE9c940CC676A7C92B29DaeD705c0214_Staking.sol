...
contract Staking is Pausable, Ownable {
    ...
    function addPool(address _staking, 
        address _reward, 
        uint256 _period, 
        uint256 _percent,
        uint256 _amount
    ...
    function remainSeconds(uint256 _pid, address account) public view returns(uint256) {
        ...
        if(stakedAt != 0 && userInfo[_pid][account].balance > 0) {
            uint256 passed = block.timestamp.sub(stakedAt);
            return  passed >= poolInfo[_pid].stakingPeriod ? 0 : poolInfo[_pid].stakingPeriod.sub(passed);
        }
    ...
    function stake(uint256 _pid) public whenNotPaused {
        ...
        uint256 passed = block.timestamp.sub(uInfo.stakedAt);
        require(passed >= pInfo.stakingPeriod, "locked yet");
        ...
        pInfo.totalStaked = pInfo.totalStaked.add(stakingAmount);
    ...
    function withdraw(uint256 _pid) public {
        ...
        uint256 rewards = stakingAmount.mul(pInfo.rewardPercent).div(PERCENTS_DIVIDER);
        if(rewards > 0) {
        ...
        uInfo.balance = 0;
        uInfo.stakedAt = 0;
        pInfo.totalStaked = pInfo.totalStaked.sub(stakingAmount);
