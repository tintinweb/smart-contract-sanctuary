...
contract DEX is Ownable {
  ...
  mapping (uint256=> mapping(uint256 => bool)) _allowedCurrencies;
...
//   mapping(uint256 => mapping(address => offers)) _offers;
  ...
  event UpdatePrice(address indexed from, uint256 tokenId, uint256 newPrice, bool isDollar, address nftContract);
...
//   event Offer(uint256 tokenId, address indexed from, uint256 currencyType, uint256 offer, uint256 index);
  ...
  function init() public {
    ...
    XNFT = INFT(0xbFdbbe7Dcc62E9723EccBe670c3ED2932F8f251B); 
...
*
 */
  function mintNFT(address collection, address to, string memory tokenURI ) isValid(collection) public {
    ...
    // for (uint256 j = 0; j < quantity; j++){
        INFT(collection).mint(to, string(abi.encodePacked("https://ipfs.infura.io:5001/api/v0/cat?arg=", tokenURI)));
    // }
    emit NFT(collection, msg.sender, 1, tokenURI);
  }
  /***
 ...
 */
  ...
  function sellNFT(address nft_a,uint256 tokenId, address seller, uint256 price, uint256 baseCurrency, uint256[] memory allowedCurrencies) isValid(nft_a) public{
    ...
    for(uint256 i = 0; i< allowedCurrencies.length; i++){
      ...
      _allowedCurrencies[tokenId][allowedCurrencies[i]] = true;
    }
    require(isValid,"122");
    ...
    if(nft_a == address(this)){
        // if(msg.sender == admin) {
        //     XNFT.transferFromAdmin(address(this), tokenId);
        // } else {
        //       XNFT.transferFrom(seller, address(this), tokenId);
        // }
    ...
    emit SellNFT(msg.sender, nft_a, tokenId, seller, price, false, royality);
  }

  //mint & sell own/xanalia collection only
  function MintAndSellNFT(address to, string memory tokenURI, uint256 price, string memory ownerId, uint256 royality, uint256 currencyType, bool _isArtist, uint256[] memory allowedCurrencies)  public { 
    uint256 tokenId;
     ...
     sellNFT(address(this), tokenId, to, price, currencyType, allowedCurrencies);
     ...
     if(msg.sender == nonCryptoNFTVault){
      ...
      emit SellNFTNonCrypto(msg.sender, address(this), tokenId, ownerId, price, false);
    }
 }
 ...
 function MintAndAuctionNFT(address to, string memory tokenURI, address _contract, uint256 _minPrice, string memory ownerId, uint256 _endTime, uint256 royality, bool isDollar) public {
    ...
    require(_contract == address(this), "MintAndAuctionNFT function only supports non-user defined collection");
    uint256 _tokenId;
 ...
 // added _contract param in function to support generalization
  ...
  function claimAuction(address _contract, uint256 _tokenId, bool awardType, string memory ownerId, address from) isValid(_contract) public {
   ...
   if(blindRAddress == address(0x0)){
    ...
    if(_contract == address(this)){ // currently only supporting royality for non-user defined collection
...
//  // added nftContract param in function to support generalization
 ...
 // added nftContract param in function to support generalization
  function updatePrice(address nftContract, uint256 tokenId, uint256 newPrice, uint256 baseCurrency, uint256[] memory allowedCurrencies) isValid(nftContract)  public{
    ...
    emit UpdatePrice(msg.sender, tokenId, newPrice, false, nftContract); // added nftContract here as well
  }
  function calculatePrice(uint256 _price, uint256 base, uint256 currencyType, uint256 tokenId, address seller, address nft_a) internal view returns(uint256 price) {
    price = _price;
     (uint112 _reserve0, uint112 _reserve1,) =LPBNB.getReserves();
      (uint112 reserve0, uint112 reserve1,) =LPAlia.getReserves();
    if(nft_a == address(this) && _tokenAuthors[tokenId][address(this)]._address == admin && adminOwner[seller] && adminDiscount > 0){ // getAuthor() can break generalization if isn't supported in Collection.sol. SOLUTION: royality isn't paying for user-defined collections
  ...
  function getPercentages(uint256 tokenId, address nft_a) internal view returns(uint256 mainPerecentage, uint256 authorPercentage, address blindRAddress) {
    if(_tokenAuthors[tokenId][nft_a].royalty > 0 && nft_a == address(this)) { // royality for XNFT only (non-user defined collection)
  ...
  function buyNFT(address nft_a,uint256 tokenId, string memory ownerId, uint256 currencyType) isValid(nft_a) public{
        ...
        require(_allowedCurrencies[tokenId][currencyType] && currencyType != 2, "123");
  ...
  function buyNFTBnb(address nft_a,uint256 tokenId, string memory ownerId) isValid(nft_a) payable public{
        fixedSell storage temp = _saleTokens[tokenId][nft_a];
        AuthorDetails storage author = _tokenAuthors[tokenId][address(this)];
        require(_allowedCurrencies[tokenId][2], "123");
        ...
        if( nft_a == address(this)) {
         bnbTransfer(address(uint160(blindRAddress)), authorPercentage, price);
        }
        bnbTransfer(address(uint160(temp.seller)), mainPerecentage, price);
        clearMapping(tokenId, nft_a);
        // in first argument there should seller not buyer/msg.sender, is it intentional ??
        
  }  
  function getBoxAddress (uint256 tokenId) internal view returns(address) {
      return revenueAddressBlindBox[boxNameByToken[tokenId]];
  }
  function bnbTransfer(address payable _address, uint256 percentage, uint256 price) internal {
      uint256 initialBalance;
        uint256 newBalance;
       initialBalance = address(this).balance;
          BNB.withdraw((price / 1000) * percentage);
    	  newBalance = address(this).balance.sub(initialBalance);
          _address.transfer(newBalance);
  }
  function clearMapping(uint256 tokenId, address nft_a) internal {
      INFT(nft_a).transferFrom(address(this), msg.sender, tokenId);
        delete _saleTokens[tokenId][nft_a];
        for(uint256 i = 0; i <=2 ; i++) {
            _allowedCurrencies[tokenId][i] = false;
        }
        // _allowedCurrencies[tokenId][1] = false;
        // _allowedCurrencies[tokenId][2] = false;
        emit BuyNFT(msg.sender, nft_a, tokenId, msg.sender);
