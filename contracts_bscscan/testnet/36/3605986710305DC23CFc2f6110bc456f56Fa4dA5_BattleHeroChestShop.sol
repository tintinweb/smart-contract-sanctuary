...
contract BattleHeroChestShop is Context, AccessControlEnumerable {
    ...
    mapping(uint256 => uint256) _presaleCap;
    mapping(uint256 => mapping(uint256 => uint256)) _presaleSold;

    mapping(address => mapping(uint256 => uint256)) _amounts;
    ...
    function _createChest(
        ...
        uint256 chestId = uint256(keccak256(
    ...
    function mint(uint256 chestId, uint256 quantity) internal {
    ...
    function burn(uint256 chestId, uint256 amount) public {
    ...
    function buyChest(uint256 chestId, uint256 amount) public payable {
        require(
    ...
    function chestsId() public view returns(uint256[] memory) { 
        return _chestsIds;
    }

    function buyFirstRound(uint256 chestId) public payable {
        uint amount = 1;
        require(
    ...
    function buySecondRound(uint256 chestId) public payable {
        uint amount = 1;
        require(
            block.timestamp >= (_presaleStart + 1 days) &&
    ...
    function buyThirdRound(uint256 chestId) public payable {
        uint amount = 1;
        require(
            block.timestamp >= (_presaleStart + 2 days) &&
