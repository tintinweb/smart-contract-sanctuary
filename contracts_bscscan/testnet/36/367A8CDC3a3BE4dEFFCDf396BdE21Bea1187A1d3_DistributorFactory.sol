...
import "./distributor/BasicDistributor.sol";
import "./distributor/LinearDistributor.sol";
import "./distributor/TrancheDistributor.sol";
...
contract DistributorFactory is Operators, IDistributorFactory {
    ...
    mapping(string => DistributorType) distributorTypes;

    function createDistributor(
        string calldata _poolID,
        DistributorType _distributorType
    ) external override onlyOperator {
        IDistributor distributor;
        if (_distributorType == DistributorType.BASIC) {
            distributor = new BasicDistributor(
                ITokenSale(tokenSalePool.getTokenSaleContractAddress(_poolID))
            );
        } else if (_distributorType == DistributorType.LINEAR) {
            distributor = new LinearDistributor(
                ITokenSale(tokenSalePool.getTokenSaleContractAddress(_poolID))
            );
        } else if (_distributorType == DistributorType.TRANCHE) {
            distributor = new TrancheDistributor(
                ITokenSale(tokenSalePool.getTokenSaleContractAddress(_poolID))
            );
        } else {
            revert("unsupport distributor type");
        }
        distributors[_poolID] = distributor;
        distributorTypes[_poolID] = _distributorType;
    ...
    function setLinearReleaseInfo(
        ...
        require(
            distributorTypes[_poolID] == DistributorType.LINEAR,
            "pool was not created with linear distributor"
        );
        distributors[_poolID].setLinearReleaseInfo(
            _campaignID,
            _startTime,
            _endTime
        );
    ...
    function setLinearReleaseInfoSameInManyCampaigns(
        string calldata _poolID,
        string[] calldata _campaignIDs,
        uint256 _startTime,
        uint256 _endTime
    ...
    function setTrancheReleaseInfo(
        string calldata _poolID,
        string calldata _campaignID,
        uint256[] calldata _trancheTimestamps,
        uint32[] calldata _percentageOfTranches
    ) public override onlyOperator {
        require(
            distributorTypes[_poolID] == DistributorType.TRANCHE,
            "pool was not created with tranche distributor"
        );
        distributors[_poolID].setTrancheReleaseInfo(
            _campaignID,
            _trancheTimestamps,
    ...
    function setTrancheReleaseInfoSameInManyCampaigns(
        string calldata _poolID,
        string[] calldata _campaignIDs,
        uint256[] calldata _trancheTimestamps,
        uint32[] calldata _percentageOfTranches
...
contract BasicDistributor is BaseDistributor {
    using SafeERC20 for IERC20;

    mapping(string => mapping(address => bool)) public isWithdraw;
        ...
        nonReentrant
    {
        ...
        require(!isWithdraw[_campaignID][_user], "already withdraw");
        isWithdraw[_campaignID][_user] = true;
        ITokenSale.UserInfo memory _userInfo = tokenSale.getUserInfo(
            _campaignID,
            _user
        );
        ITokenSale.Campaign memory _campaign = tokenSale.getCampaign(
            _campaignID
        );
        require(block.timestamp > _campaign.endTime, "not start claiming yet");
        _safeTransfer(_campaign.token, _user, _userInfo.allocation);
        emit Withdraw(_user, _campaignID, _userInfo.allocation);
    }
    ...
    function getWithdrawedAmount(string calldata _campaignID, address _user)
        ...
        returns (uint256)
    {
        if (isWithdraw[_campaignID][_user]) {
            ITokenSale.UserInfo memory _userInfo = tokenSale.getUserInfo(
                _campaignID,
                _user
            );
...
contract LinearDistributor is BaseDistributor {
    using SafeERC20 for IERC20;
    ...
    mapping(string => mapping(address => uint256)) public claimedAmount;
    mapping(string => ReleaseInfo) public releaseInfo;
    mapping(string => bool) public activatedCampaigns;
    ...
    function setLinearReleaseInfo(
        ...
        require(!activatedCampaigns[_campaignID], "already set");
        ReleaseInfo storage info = releaseInfo[_campaignID];
        if (info.startTime > 0) {
            require(_startTime < info.startTime, "already start");
        }
        info.startTime = _startTime;
        info.endTime = _endTime;
        activatedCampaigns[_campaignID] = true;
        emit LinearReleaseInfoSet(_campaignID, _startTime, _endTime);
    }
        ...
        nonReentrant
    {
        ...
        // console.log("Contract time: ", block.timestamp);
        // console.log("Claimable: ", _amount);
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp < _info.startTime) {
            return 0;
        }
        ...
        uint256 timestamp = _min(block.timestamp, _info.endTime);

        // console.log(
        //     "timestamp - _info.startTime",
        //     timestamp - _info.startTime,
        //     "_info.endTime - _info.startTime",
        //     _info.endTime - _info.startTime
        // );

        return
            ((userInfo.allocation * (timestamp - _info.startTime)) /
                (_info.endTime - _info.startTime)) -
            claimedAmount[_campaignID][_user];
    }
...
contract TrancheDistributor is BaseDistributor {
    using SafeERC20 for IERC20;

    struct ReleaseInfo {
        uint256[] trancheTimestamps;
    ...
    function setTrancheReleaseInfo(
        ...
        ReleaseInfo storage info = releaseInfo[_campaignID];
        require(info.trancheTimestamps.length == 0, "already set tranches");
        uint32 i;
        uint32 percentageSum = 0;
        require(
            _trancheTimestamps.length == _percentageOfTranches.length,
            "number of timestamps must be equal to number of tranche percentages"
        );
        ...
        info.trancheTimestamps = _trancheTimestamps;
        info.percentageOfTranches = _percentageOfTranches;
        emit TrancheReleaseInfoSet(
            _campaignID,
            _trancheTimestamps,
            _percentageOfTranches
        );
    ...
    {
        ReleaseInfo memory _info = releaseInfo[_campaignID];
        if (block.timestamp < _info.trancheTimestamps[0]) {
            return 0;
        }
        ITokenSale.UserInfo memory userInfo = tokenSale.getUserInfo(
            _campaignID,
        ...
        uint32 totalClaimablePercentage = 0;
        for (uint32 i = 0; i < _info.trancheTimestamps.length; i++) {
            if (block.timestamp >= _info.trancheTimestamps[i]) {
                totalClaimablePercentage += _info.percentageOfTranches[i];
            }
        }

        return
            ((userInfo.allocation * totalClaimablePercentage) / 100) -
            claimedAmount[_campaignID][_user];
    }
}
...
abstract contract BaseDistributor is IDistributor, Ownable, ReentrancyGuard {
    ...
    event LinearReleaseInfoSet(
        string _campaignID,
        uint256 _startTime,
        uint256 _endTime
    );

    event TrancheReleaseInfoSet(
        string _campaignID,
        uint256[] _trancheTimestamps,
        uint32[] _percentageOfTranches
    );
