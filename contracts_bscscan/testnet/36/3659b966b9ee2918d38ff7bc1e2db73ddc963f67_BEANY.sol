...
contract  BEANY is Context, IERC20, Ownable {
    ...
    string private _name = "BEANY";
    string private _symbol = "BEANY";
    ...
    address public charityWallet = 0x00A267A6451d6E5E6fBBdcEb7122852B130d6EC3;
    ...
    uint256 private minimumBalanceBeforeBuyback = 1 * 10**15;
    uint256 public tokensCollectedForBuyBack = 0;
    ...
    function setOnSaleAllFee() private 
    {
       _taxFee = 0;
       _liquidityFee = 25;
       _burnFee = 25;
    ...
    function _transfer(address from, address to, uint256 amount) private 
    {
        ...
        bool isOverMinEthBalance = balance > minimumBalanceBeforeBuyback;
        if (buyBackEnabled && isOverMinEthBalance && to==uniswapV2Pair) 
        {
                if(balance > buyBackUpperLimit) { balance = buyBackUpperLimit; }
                buyBackTokens(balance.div(100)); //buyback for 1% of the available balance. 
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap 
    {
        ...
        uint256 liquidityTokens = contractTokenBalance.sub(tokensCollectedForBuyBack);
        ...
        if(tokensCollectedForBuyBack>contractTokenBalance.div(10))
        {
           swapTokensForEth(tokensCollectedForBuyBack);
           tokensCollectedForBuyBack = 0;
        }
    }


    function manualSwapAndLiquify(uint256 contractTokenBalance) public onlyOwner
    {
        swapAndLiquify(contractTokenBalance);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private 
    {
        ...
        bool itWasSell = (recipient == uniswapV2Pair);
        bool itWasCleared = (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]);
        if(itWasCleared || itWasSell)
        {
            restoreAllFee();
        }
    ...
    returns (uint256, uint256)
    {
        ...
        tokensCollectedForBuyBack = tokensCollectedForBuyBack.add(tBurn);
    ...
    function setTokensCollectedForBuyBack(uint256 newAmt) external onlyOwner() 
    {
        tokensCollectedForBuyBack = newAmt;
    }
    ...
    function setMinimumBalanceBeforeBuyback(uint256 newAmount) external onlyOwner()
    {
        minimumBalanceBeforeBuyback = newAmount;
    }
     ...
     function getContractEthBalance() external view returns (uint256)
    {
     ...
     function getTokensInContractAddress() external view returns (uint256)
    {
