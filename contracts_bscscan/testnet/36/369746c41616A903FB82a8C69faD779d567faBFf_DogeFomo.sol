...
contract DogeFomo is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private _lottoReward;
    ...
    uint256 private _minLottoBalance = 10000 * (10**18);
    uint256 private _minimumBuyForPotEligibility = 100000 * (10**18);
    ...
    constructor () public {
        gameSettings = GameSettings(
            8000000 * 10**18, //maxTxAmount is 2 million tokens
            500000 * 10**18, //tokenSwapThreshold is 2000000 tokens
            100000 * 10**18, //minimumBuyForPotEligibility is 100000 tokens
            35, //increaseRatio 0.035% for minimumBuyForPotEligibility
            60 * 60 * 1, // initTimeLeft is 4 hours
            60 * 60 * 8, //maxTimeLeft is 8 hours
            10, //extraTimeEachPot is 30 seconds
    ...
    function updateGameSettings(
        ...
        if(maxTxAmount > 0)  {
            require(maxTxAmount >= 8000000 * 10**18 && maxTxAmount <= 10000000 * 10**18);
            gameSettings.maxTxAmount = maxTxAmount;
        }
        if(tokenSwapThreshold > 0)  {
            require(tokenSwapThreshold >= 500000 * 10**18 && tokenSwapThreshold <= 1000000 * 10**18);
            gameSettings.tokenSwapThreshold = tokenSwapThreshold;
        }
        if(minimumBuyForPotEligibility > 0)  {
            require(minimumBuyForPotEligibility >= 100000 * 10**18 && minimumBuyForPotEligibility <= 100000 * 10**18);
    ...
    function lotteryReward(address account) public view returns (uint256) {
        return _lottoReward[account];
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidityAndPot, data.tLotto, data.tDev);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidityAndPot).sub(tDev).sub(tLotto);
        return (tTransferAmount, TData(0, tFee, tLiquidityAndPot, tLotto, tDev, 0));
    }
    ...
    function _getRValues(TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidityAndPot).sub(rLotto).sub(rDev);
    ...
    function _takeLotto(uint256 tLotto) private {
        ...
        _lottoReward[_lottoWalletAddress] = _lottoReward[_lottoWalletAddress].add(tLotto);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _potFee == 0 && _devFee == 0 && _lottoFee == 0) return;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidityAndPot, uint256 tLotto, uint256 tDev) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
