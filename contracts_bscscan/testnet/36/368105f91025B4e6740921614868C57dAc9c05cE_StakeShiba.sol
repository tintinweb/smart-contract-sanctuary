...
contract StakeShiba {
    ...
    uint256 public tokenprice = 7015000000000;
    ...
    =            VIEW FUNCTIONS            =
    ==========================================*/
    function Claimable(uint stakeIndex) public view returns(uint256)
    {
        uint256 amount = stakeInfo[msg.sender][stakeIndex].amount;
        uint256 amt;
    ...
    {
      if(((userInfo[_addr].total_payouts.add(amount)) <= userInfo[_addr].wid_limit) || _addr == terminal)
      {
         return amount;
      }
      else
    ...
    function buy(address _referredBy,uint256 tokenAmount) external returns(uint256)
    {
      require(!safeguard);
    ...
    function withdrawAll() external returns(bool)
    {
      ...
      require(rewardBalanceLedger_[_customerAddress] + totaldaily >= minWithdraw.div(tokenprice), "beyond withdraw limit");
      ...
      uint256 amt;
      uint256 usdamt;
      uint256 usdamount = totaldaily.mul(tokenprice);
      if(totaldaily>0)
      {
    ...
    function Claim(uint stakeIndex) external returns(bool)
    {
      ...
      require(!isContract(msg.sender),  'No contract address allowed to withdraw');
        ...
        if(tim2 < tim + lD)
        {
          ...
          //amt = (amount * extraROI(vmainamt) / 10000) * daysPassed ;
          uint256 usdamt = (usdamount * extraROI(vmainamt) / 10000) * daysPassed ;
          uint256 amt = (usdamt).div(tokenprice);
    ...
    function _AIpollDeposits(address _addr, uint256 _amount) private {
        ...
        ai_pool_users_refs_deposits_sum[ai_pool_cycle][_addr] += _amount.mul(tokenprice);
      ...
      function drawAIPool() public onlyAdministrator returns(bool)
      {
        ...
        if(ai_pool_last_draw + 3600 < block.timestamp)
        {
          _drawAIPool();
        }
        return true;
