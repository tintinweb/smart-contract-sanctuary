...
contract EcosyToken is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "EcosyToken";
    ...
    function swapBack() internal swapping {
        ...
        uint256 tokensToLiquidity = contractBnbBalance.mul(liquidityFee).div(
            totalFee
        );
        // calculate total swap fee
        uint256 totalSwapFee = totalFee.sub(liquidityFee);
        ...
        uint256 amountBNBReflection = swappedBnbAmount.mul(reflectionFee).div(
            totalSwapFee
        );
        // calculate marketing total
        uint256 totalBnbMarketing = swappedBnbAmount.mul(marketingFee).div(
            totalSwapFee
        );
        ...
        // calculate amount to send dev wallet
        uint256 amountBnbDev = swappedBnbAmount.mul(devFee).div(totalSwapFee);
