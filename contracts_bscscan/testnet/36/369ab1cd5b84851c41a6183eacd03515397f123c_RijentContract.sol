...
contract RijentContract {    
    ...
    function packageDetails(address addr) public view returns(uint[] memory packageAmt, uint[] memory lockingType){
        User storage userData = user[addr];
        uint len = userData.packages.length;
        packageAmt     = new uint[](len);
        lockingType    = new uint[](len);
        for(uint i = 0; i < len; i++){
            packageAmt[i] = userData.packages[i].packageAmt;
            lockingType[i] = userData.packages[i].lockingType; 
        }
        return (packageAmt, lockingType);
    }
    // Set depositStatus
