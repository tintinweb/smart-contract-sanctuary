...
contract HareV2 is ContextUpgradeable, IBEP20, OwnableUpgradeable {
    ...
    address payable public _routerAddress;
    ...
    function initialize(
        ...
        _routerAddress = payable(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            if (!_isExcludedFromLimits[from] || !_isExcludedFromLimits[to]) {
                require(
                    amount <= _maxTxAmount,
                    "Transfer exceeds max Tx amount."
                );

                require(
                    balanceOf(to) + amount <= maxWalletAmount,
                    "Receiver would exceed max wallet size"
                );
            }
    ...
    function setPairAddress(address newPairAddress) public onlyOwner {
        require(address(newPairAddress) != address(0), "Address cannot be 0");
    ...
    function updateRouterAddress() external onlyOwner {
