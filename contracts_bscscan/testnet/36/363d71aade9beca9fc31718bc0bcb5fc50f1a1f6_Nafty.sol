...
contract Nafty is Context, IERC20, Ownable {
    ...
    address public marketingAddress = 0xb5A6c2B9DF35E4c8643156F54bBD3C5E3A39c749;
    address public operationsAddress = 0xa2Dc5743C1426C62Ed6CDBe4355E3E474DFfB187;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "Nafty";
    string private constant _symbol = "Nafty";
    ...
    function setFeeRates(uint256 _rfi, uint256 _operations, uint256 _marketing, uint256 _liquidity, uint256 _burn) external onlyOwner {
        feeRates.rfi = _rfi;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _operations, uint256 _marketing, uint256 _liquidity, uint256 _burn) external onlyOwner{
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rOperations, to_return.rMarketing, to_return.rLiquidity, to_return.rBurn) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tTransferAmount = tAmount-s.tRfi-s.tOperations-s.tMarketing-s.tLiquidity-s.tBurn;
        }
        else{
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rOperations,uint256 rMarketing, uint256 rLiquidity, uint256 rBurn) {
        ...
        rTransferAmount =  rAmount-rRfi-rOperations-rMarketing-rLiquidity-rBurn;
        return (rAmount, rTransferAmount, rRfi,rOperations,rMarketing,rLiquidity, rBurn);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        emit Transfer(sender, address(this), s.tLiquidity + s.tOperations + s.tBurn);
