...
contract OracleV3 is IOracleV3, OracleV2 { 
    ...
    event WithdrawSeedmonRequested(
        address requester, 
        uint256 requestId, 
        uint256 tokenId, 
        address withdrawer, 
    ...
    event CancelDownloadSeed(address requester, uint256 requestId, uint256 providedGas);

    function cancelDownloadSeed(uint256 requestId) external override {
        ...
        IClientV3(msg.sender).onCancelDownloadSeedReceived{value: providedGas}(requestId);

        emit CancelDownloadSeed(msg.sender, requestId, providedGas);
    }

    function requestWithdrawSeedmon(uint256 requestId, uint256 tokenId, address withdrawer) 
        external 
        payable  
        override
        onlyOpen 
        requireGas 
    {
        ...
        require(!isRequestPending(msg.sender, requestId), "Oracle: Get duplicated withdraw seedmon request"); 
        ...
        emit WithdrawSeedmonRequested(requester, requestId, tokenId, withdrawer, providedGas);
    }

    function requestDownloadSeed(uint256 requestId, uint256 requestAmount, address downloader)
        external
        payable
        override
        onlyOpen
    ...
    {
        require(requestId > 0, "Oracle: Invalid request is zero");
        require(!isRequestPending(msg.sender, requestId), "Oracle: Get duplicated download seed request"); 
        ...
        emit DownloadSeedRequested(requester, requestId, requestAmount, downloader, providedGas);
    }

    // =============== Prophet method ================
    function provideWithdrawSeedmon(address requester, uint256 requestId, uint256 tokenId, uint256 exp, bool isSuccess) 
        external 
        onlyProphet 
    {
        ...
        IClientV3(requester).onWithdrawSeedmonReceived(requestId, tokenId, exp, isSuccess);
        payable(msg.sender).transfer(providedGas);

        emit ProphetResponse(requester, requestId);  
    }

    function provideDownloadSeed(address requester, uint256 requestId, uint256 tokenBalance, bool isSuccess)
        external
        onlyProphet
    {
        require(isRequestPending(requester, requestId), "Oracle: Transaction was refunded or responded"); 
        ...
        IClientV3(requester).onDownloadSeedReceived(requestId, tokenBalance, isSuccess);
        payable(msg.sender).transfer(providedGas);
...
interface IOracleV3 is IOracleV2 {
    function requestWithdrawSeedmon(uint256 requestId, uint256 tokenId, address withdrawer) external payable;
    function requestDownloadSeed(uint256 requestId, uint256 requestAmount, address downloader) external payable;
    function cancelDownloadSeed(uint256 requestId) external;
}
...
interface IClientV3 is IClientV2 {
    function onWithdrawSeedmonReceived(uint256 requestId, uint256 tokenId, uint256 exp, bool isSuccess) external;
    function onDownloadSeedReceived(uint256 requestId, uint256 tokenBalance, bool isSuccess) external;
    function onCancelDownloadSeedReceived(uint256 requestId) external payable;
}
