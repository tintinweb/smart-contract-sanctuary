...
contract Investment {
    ...
    //           investmentAmount:0
    //      });
    function getX (uint256  id) public view returns (string memory, uint256,uint256,uint256,uint256[] memory,bool,uint256,uint256) {

        X memory x=groupX[id];
        return (x.name,x.parent,x.refererId,x.plan,x.nodes,x.isValue,x.balance,x.investmentAmount);
    }
    
    function getXmax (uint256  id) public view returns (string memory, uint256,uint256,uint256,uint256[] memory,bool,uint256,uint256,uint256) {

        Xmax memory x=groupXmax[id];
        return (x.name,x.parent,x.refererId,x.plan,x.nodes,x.isValue,x.balance,x.investmentAmount,x.totalUsersReferred);
    }
    ...
    function getMyXMatrixParentReferer(uint256 id) public view returns ( uint256,uint256){
        // uint256 userId=address_Id[msg.sender].uniqueId;
        return (groupX[id].parent,groupX[id].refererId);
    }
    
    function getMyXMaxMatrixParentReferer(uint256 id) public view returns ( uint256,uint256){
        // uint256 userId=address_Id[msg.sender].uniqueId;
        return (groupXmax[id].parent,groupXmax[id].refererId);
    }
