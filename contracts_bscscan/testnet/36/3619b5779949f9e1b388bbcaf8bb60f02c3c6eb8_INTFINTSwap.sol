...
contract INTFINTSwap {
    ...
    uint256 public startDate = 1628604000; // Tuesday, 10 August 2021 14:00:00

    uint256 public totalTokensToSell = 2600000000000000000000000000; // Total tokens for swap
    uint256 public tokenPerPRE = 1 * 10**18; // 1 INTF2 = 1 INTVault
    uint256 public minPerTransaction = 0; // min amount per transaction
    uint256 public maxPerUser = 8000000 * 10**18; // max amount per user
    ...
    modifier checkSaleRequirements(uint256 buyAmount) {
        require(now >= startDate, "Swap has not started");
        require(saleEnded == false, "Swap has ended");
        require(
    ...
    constructor(address _TOKEN, address _PRESALETOKEN) public {
        owner = msg.sender;
        TOKEN = IBEP20(_TOKEN);
    ...
    function buyWithPRE(uint256 buyAmount)
        public
        checkSaleRequirements(buyAmount)
    {
        uint256 amount = calculatePREAmount(buyAmount);
        require(
            PRESALETOKEN.balanceOf(msg.sender) >= amount,
        ...
        require(
            sumSoFar <= maxPerUser,
            "Greater than the maximum purchase limit"
        );

        tokenPerAddresses[msg.sender] = sumSoFar;
        totalSold = totalSold.add(buyAmount);

        PRESALETOKEN.transferFrom(msg.sender, address(this), amount);
    ...
    function setTokenPricePerPRE(uint256 _tokenPerPRE) public {
        require(msg.sender == owner, "Only owner allowed");
        require(_tokenPerPRE > 0, "Invalid TOKEN price per PRESALETOKEN");
    ...
    function withdrawCollectedPreTokens() public {
        require(msg.sender == owner, "Only owner allowed");
        uint256 collectedPreAmount = PRESALETOKEN.balanceOf(address(this));
        require(collectedPreAmount > 0, "Insufficient balance");
        PRESALETOKEN.transfer(owner, collectedPreAmount);
    }
    ...
    function calculateTokenAmount(uint256 preAmount)
        public
        view
        returns (uint256)
    {
    ...
    function calculatePREAmount(uint256 tokenAmount)
        public
        view
        returns (uint256)
    {
        require(
            tokenPerPRE > 0,
            "TOKEN price per PRE should be greater than 0"
        );
