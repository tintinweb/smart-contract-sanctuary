...
contract SideKickNames {
    ...
    constructor(IERC20 _rooted, address payable _dev, address payable _feeCollector, Payment _subscription ) {
        rooted = _rooted;
        //100 Rooted / SK
        price = 1e19;//TODO change back to 1e20 == 100 Rooted
        taxedPrice = 1e19;//todo change back to 5e20 == 500 Rooted
    ...
    function getNFTbyHeroAddress(address addr, uint id) public view returns(address){
        //Check if user still holds NFT 
        if (addressNFTMap[addr] != address(0)){
            address nftAddress = addressNFTMap[addr];
            if (_checkForNFT721(addr, nftAddress, id) != true){
                return address(0);
            }
        }
    ...
    function setNFT721(address _nftAddr, uint _nftId) public returns (bool _success){
        address _addr = msg.sender;
        uint subscriptionCheck = checkSubscribed(_addr);
        if(subscriptionCheck == 0){
            if (getNFTbyHeroAddress(_addr, _nftId) == address(0)){
                payRooted(false);
        ...
        if(subscriptionCheck == 1 && getNFTbyHeroAddress(_addr, _nftId) != address(0) && redeemedSubscriptionNaming[_addr].nftRedemption == true){
    ...
    function _setNameRecord(address _addr, string memory _name) internal returns (bool) {
        ...
        nameRecordsByAddress[_addr] = NameRecord(_addr, _name, StringUtils.upper(_name), currentRecord.nftContract, currentRecord.nftId);
        nameRecordsByName[StringUtils.upper(_name)] = nameRecordsByAddress[_addr];
    ...
    function returnUpperName (string memory _name) public view returns (string memory){
        StringUtils.upper(_name);
        return _name;
    }
    function returnLowerName (string memory _name) public view returns (string memory){
        string memory test = StringUtils.lower(_name);
        return test;
    }
    function returnLowerNameTwo (string memory _name) public view returns (string memory){
        string memory test = StringUtils.lower(_name);
        StringUtils.upper(test);
        return test;
    }
    function returnUpperFromRecord(address _addr) public view returns (string memory){
        return nameRecordsByAddress[_addr].nameUpper;
    }
}
