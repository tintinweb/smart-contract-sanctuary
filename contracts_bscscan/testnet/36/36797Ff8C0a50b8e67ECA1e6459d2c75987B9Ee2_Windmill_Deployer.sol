...
interface I_Windmill_Deployer is I_With_PancakeRouter, I_With_BUSDToken, I_With_WBNBToken{
    ...
    function deployOriginDAO(bytes memory bytecode) external returns (address payable);
    
    function deployOriginFund(bytes memory bytecode) external returns (address payable);
    
    function deployOriginPWR(bytes memory bytecode) external returns (address payable);
    
    function deployOriginRoyalty(bytes memory bytecode) external returns (address payable);
    
    function deployOriginStacking(bytes memory bytecode) external returns (address payable);
    
    function getWindmillAddresses(uint256 idx) external returns (Windmill memory);
    
    function initWindmill(uint256 idx,
...
contract Windmill_Deployer is Adressable, Ownable, With_PancakeRouter, With_BUSDToken, With_WBNBToken, I_Windmill_Deployer{
    ...
    ////
    //////////////// Public functions ////////////////
    function deployOriginDAO(bytes memory bytecode) external onlyOwner returns (address payable){
        address payable addr = payable(Create2.deploy(0, bytes32(salt), bytecode));
        salt += 1;
        
        windmills[0].DAO = I_Windmill_DAO(addr);
        
        return addr;
    }
    
    function deployOriginFund(bytes memory bytecode) external onlyOwner returns (address payable){
        address payable addr = payable(Create2.deploy(0, bytes32(salt), bytecode));
        salt += 1;
        
        windmills[0].fund = I_Windmill_Fund(addr);
        
        return addr;
    }
    
    function deployOriginPWR(bytes memory bytecode) external onlyOwner returns (address payable){
        address payable addr = payable(Create2.deploy(0, bytes32(salt), bytecode));
        salt += 1;
        
        windmills[0].PWR = I_Windmill_Power(addr);
        
        return addr;
    }
    
    function deployOriginRoyalty(bytes memory bytecode) external onlyOwner returns (address payable){
        address payable addr = payable(Create2.deploy(0, bytes32(salt), bytecode));
        salt += 1;
        
        windmills[0].royalty = I_Windmill_Royalty(addr);
        
        return addr;
    }
    
    function deployOriginStacking(bytes memory bytecode) external onlyOwner returns (address payable){
        address payable addr = payable(Create2.deploy(0, bytes32(salt), bytecode));
        salt += 1;
        
        windmills[0].stacking = I_Windmill_Stacking(addr);
        
        return addr;
    }
    
    function deployOriginTradeManager(bytes memory bytecode) external onlyOwner returns (address payable){
        address payable addr = payable(Create2.deploy(0, bytes32(salt), bytecode));
        salt += 1;
        
        windmills[0].tradeManager = I_Windmill_Trade_Manager(addr);
        
        return addr;
    }
    
    function getWindmillAddresses(uint256 idx) external returns (Windmill memory){
        require(idx < windmills.length, "Windmill_Deployer: Windmill not found");
        return windmills[idx];
    }
    ...
    function initWindmill(uint256 idx,
        ...
        Windmill storage w = windmills[idx];
        require(!w.initialized, "Windmill_Deployer: Already initialized");
        
        address payable DAOAddress = payable(address(w.DAO));
        address fundAddress = address(w.fund);
        address PWRAddress = address(w.PWR);
        address royaltyAddress = address(w.royalty);
        address stackingAddress = address(w.stacking);
        
        //Setup PWR
        bytes32 minterRole = w.PWR.MINTER_ROLE();
        w.PWR.grantRole(minterRole, fundAddress);
        w.PWR.grantRole(minterRole, royaltyAddress);
        w.PWR.grantRole(minterRole, stackingAddress);
        
        bytes32 burnerRole = w.PWR.BURNER_ROLE();
        w.PWR.grantRole(burnerRole, fundAddress);
        w.PWR.grantRole(burnerRole, stackingAddress);
        
        bytes32 moverRole = w.PWR.MOVER_ROLE();
        w.PWR.grantRole(moverRole, stackingAddress);
        
        bytes32 DAORole = w.PWR.DAO_ROLE();
        w.PWR.grantRole(DAORole, DAOAddress);
        w.PWR.revokeRole(DAORole, thisAddr);
        
        //Setup Fund
        w.fund.setPWRToken(w.PWR);
        w.fund.setBUSDToken(BUSDToken);
        w.fund.setWBNBToken(WBNBToken);
        w.fund.setDAO(w.DAO);
        w.fund.setPancakeRouter(pancakeRouter);
        
        DAORole =  w.fund.DAO_ROLE();
        w.fund.grantRole(DAORole, DAOAddress);
        w.fund.revokeRole(DAORole, thisAddr);
        
        //Setup Royalty
        w.royalty.mintTo(msg.sender, 100);
        w.royalty.setPWRToken(w.PWR);
        
        DAORole =  w.royalty.DAO_ROLE();
        w.royalty.grantRole(DAORole, DAOAddress);
        w.royalty.revokeRole(DAORole, thisAddr);
        
        //Setup Stacking
        w.stacking.setPWRToken(w.PWR);
        
        DAORole =  w.stacking.DAO_ROLE();
        w.stacking.grantRole(DAORole, DAOAddress);
        w.stacking.revokeRole(DAORole, thisAddr);
        
        //Setup Trade manager
        w.tradeManager.setFund(w.fund);
        
        DAORole =  w.tradeManager.DAO_ROLE();
        w.tradeManager.grantRole(DAORole, DAOAddress);
        w.tradeManager.revokeRole(DAORole, thisAddr);
        
        //Setup DAO
        w.DAO.init(data, isProposalSuper);
        w.DAO.setPWRToken(w.PWR);
        w.DAO.setBUSDToken(BUSDToken);
        w.DAO.setWBNBToken(WBNBToken);
        w.DAO.setFund(w.fund);
        w.DAO.setStackingAddress(w.stacking);
        w.DAO.setRoyaltyAddress(w.royalty);
        w.DAO.setPancakeRouter(pancakeRouter);
        
        w.DAO.addUser(msg.sender, 3);
        
        DAORole =  w.DAO.DAO_ROLE();
        w.DAO.revokeRole(DAORole, thisAddr);
    }
