...
contract StakingBUSD is Staking, IMinter {
    ...
    // Notify auto mint
    // ------------------------------------------------------------------------
    function notifyMint(uint256 ascAmount, uint256 BUSDAmount) public override onlyPools {

        if (BUSDAmount > 0) {

            SafeERC20.safeTransfer(BUSD, msg.sender, BUSDAmount);

            uint256 leftRatio = (_totalStaking - BUSDAmount) * SHARE_PRECISION / _totalStaking;
            uint256 newUnusedRatio = shareUnusedRatio * leftRatio / SHARE_PRECISION;
            if (newUnusedRatio > 0)
                shareUnusedRatio = newUnusedRatio;

            _totalStaking -= BUSDAmount;
        }
    ...
    function notifyReinvest(address account, uint256 amount) external override updateMint(account) updateReward(account) onlyStakings {
        require(_totalStaking + amount <= BUSD.balanceOf(address(this)), "Notify amount exceeds balance!");
    ...
    function withdraw(uint256 shareAmount) public override nonReentrant updateReward(msg.sender) updateMint(msg.sender) {
        ...
        SafeERC20.safeTransfer(BUSD, msg.sender, withdrawAmount);
    ...
    function claimMintWithPercent(uint256 percent) public override nonReentrant updateMint(msg.sender) {
        ...
        if (mint > 0) {
            ...
            SafeERC20.safeTransfer(coinASC, msg.sender, mint);
