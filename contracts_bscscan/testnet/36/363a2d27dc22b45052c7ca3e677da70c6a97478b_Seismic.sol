...
//  IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); 
    ...
    */
    ...
    function fees_set_Reflection_Percent(uint256 FeeReflection) external onlyOwner() {

        // Buyer protection - reflection fee can not be lowered below 1% - total fees can not be increased above 10%
        uint256 limitCheck = _FeeMarketing+_FeeDev+_FeeLiquidity+_FeeBurnTokens+FeeReflection;
        if (limitCheck <= maxPossibleFee && FeeReflection >= minPossibleReflect){
            _FeeReflection = FeeReflection;
            _FeesTotal = _FeeMarketing+_FeeDev+_FeeLiquidity+_FeeBurnTokens+_FeeReflection;}
    }

    function fees_set_BurnTokens_Percent(uint256 FeeBurnTokens) external onlyOwner() {

        // Buyer protection - total fees can not be increased above 10%
        uint256 limitCheck = _FeeMarketing+_FeeDev+_FeeLiquidity+_FeeReflection+FeeBurnTokens;
        if (limitCheck <= maxPossibleFee){
            _FeeBurnTokens = FeeBurnTokens;
            _FeesTotal = _FeeMarketing+_FeeDev+_FeeLiquidity+_FeeBurnTokens+_FeeReflection;}
    }

    
    function fees_set_Liquidity_Percent(uint256 FeeLiquidity) external onlyOwner() {

        // Buyer protection - total fees can not be increased above 10%
        uint256 limitCheck = _FeeMarketing+_FeeDev+FeeLiquidity+_FeeReflection+_FeeBurnTokens;
        if (limitCheck <= maxPossibleFee){
    ...
    function fees_set_Marketing_Percent(uint256 FeeMarketing) external onlyOwner() {
        
        // Buyer protection - total fees can not be increased above 10%
        uint256 limitCheck = FeeMarketing+_FeeDev+_FeeLiquidity+_FeeReflection+_FeeBurnTokens;
        if (limitCheck <= maxPossibleFee){
