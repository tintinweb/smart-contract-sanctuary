...
// File: contracts/BabyCryptoDividendPayingToken.sol
...
// File: contracts/BabyCrypto.sol
pragma solidity ^0.7.6;

contract BabyCrypto is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BabyCryptoDividendTracker public dividendTracker;

    address public burnAddress;
    
    uint256 public maxBuyTranscationAmount = 10000 * (10**9);
    ...
    constructor() public ERC20("BabyCrypto", "BabyCrypto") {
        // LP address receiver 
        burnAddress = address(0xad978911fDd460fCaF37E8990eDfE601d032aAC8);
        uint256 _processDividendTime = block.timestamp;
        processDividendTime = _processDividendTime;

        dividendTracker = new BabyCryptoDividendTracker();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "BabyCrypto: The dividend tracker already has that address");

        BabyCryptoDividendTracker newDividendTracker = BabyCryptoDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyCrypto: The new dividend tracker must be owned by the BabyCrypto token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyCrypto: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "BabyCrypto: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "BabyCrypto: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyCrypto: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyCrypto: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyCrypto: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BabyCrypto: This account cannot send tokens until trading is enabled");
        }
...
contract BabyCryptoDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BabyCrypto_Dividend_Tracker", "BabyCrypto_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "BabyCrypto_Dividend_Tracker: No transfers allowed");
    }
    function withdrawDividend() public override {
        require(false, "BabyCrypto_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyCrypto contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyCrypto_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyCrypto_Dividend_Tracker: Cannot update claimWait to same value");
