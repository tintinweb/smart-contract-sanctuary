...
contract AdventureFlokiPaybackTrack is PaybackReward, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() PaybackReward("AdventureFloki_Reward_Tracker", "AdventureFloki_Reward_Tracker") {
        claimWait = 7200;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "AdventureFloki_Reward_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "AdventureFloki_Reward_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AdventureFloki contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "AdventureFloki_Reward_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "AdventureFloki_Reward_Tracker: Cannot update claimWait to same value");
...
contract AdventureFloki is ERC20, Ownable {
...
uint256 public  preward = 8;
  ...
  ILP public adressLiquidity;
    ...
    constructor() ERC20("Adventure Floki", "ADF") {

        PaybackTrack = new AdventureFlokiPaybackTrack();
        TotalFees = preward.add(liquidityTax).add(marketingPercent);
 ...
 function rebaseToken(uint256 epoch, int256 supplyDelta)
  ...
  returns (uint256)
  {
    ...
    function updateDEXV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(DEXV2Router), "AdventureFloki: The router already has that address");
        emit UpdateDEXV2Router(newAddress, address(DEXV2Router));
        DEXV2Router = IDEXV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "AdventureFloki: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "AdventureFloki: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "AdventureFloki: Cannot update gasForProcessing to same value");
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 tokensTAdventureFloki = tokensToLiquify.mul(liquidityTax).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTAdventureFloki);
        ...
        uint256 bnbForReflection = bnbBalance.mul(preward).div(bnbFeeFactor);
        uint256 bnbForMarketing = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection);

        addLiquidity(tokensTAdventureFloki, bnbForLiquidity);
        ...
        if(success) {
            emit transferDividends(tokensToLiquify.mul(preward).div(TotalFees), bnbForReflection);
        }
    }
