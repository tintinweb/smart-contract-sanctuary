...
contract BNBHero is Context, IBEP20, Ownable {
    ...
    address public bnbPoolWallet = 0xC9A9f5157592deCAB20EC204cF8355Aa29dF66C5;
    ...
    uint256 public _BNBBuyFee = 7;
    uint256 private _previousBNBBuyFee = _BNBBuyFee;
    
    uint256 public _BNBSellFee = 9;
    uint256 private _previousBNBSellFee = _BNBSellFee;
    ...
    event UpdateFees(uint256 bnbBuyFee, uint256 bnbSellFee, uint256 liquidityBuyFee, uint256 liquiditySellFee);
    ...
    event UpdateBNBPoolWallet(address account);
    ...
    function setBNBPoolWallet(address account) external onlyOwner {
        require(account != bnbPoolWallet, 'This address was already used');
        bnbPoolWallet = account;
        emit UpdateBNBPoolWallet(account);
    }
    ...
    function setFees(uint256 bnbBuyFee, uint256 bnbSellFee, uint256 liquidityBuyFee, uint256 liquiditySellFee) external onlyOwner() {
        _BNBBuyFee = bnbBuyFee;
        _BNBSellFee = bnbSellFee;
        _liquidityBuyFee = liquidityBuyFee;
        _liquiditySellFee = liquiditySellFee;
        emit UpdateFees(bnbBuyFee, bnbSellFee, liquidityBuyFee, liquiditySellFee);
    }
    ...
    function _getFeeValues(uint256 tAmount, bool isBuy) private view returns (uint256, uint256, uint256) {
        uint256 tBNBFee = tAmount.mul(_BNBBuyFee).div(10**2);
        uint256 tLiquidity = tAmount.mul(_liquidityBuyFee).div(10**2);
        uint256 tTransferAmount = tAmount.sub(tBNBFee).sub(tLiquidity);
        if (isBuy == false) {
            tBNBFee = tAmount.mul(_BNBSellFee).div(10**2);
            tLiquidity = tAmount.mul(_liquiditySellFee).div(10**2);
            tTransferAmount = tAmount.sub(tBNBFee).sub(tLiquidity);
        }
        return (tTransferAmount, tBNBFee, tLiquidity);
    }

    function removeAllFee() private {
        if(_BNBBuyFee == 0 && _BNBSellFee == 0 && _liquiditySellFee == 0 && _liquidityBuyFee == 0) return;
        
        _previousBNBBuyFee = _BNBBuyFee;
        _previousBNBSellFee = _BNBSellFee;
    ...
    function restoreAllFee() private {
        _BNBBuyFee = _previousBNBBuyFee;
        _BNBSellFee = _previousBNBSellFee;
    ...
    function swapAndChargeBNBPool(uint256 tokenBalance) private lockTheSwap {
        ...
        (bool success, ) = payable(bnbPoolWallet).call{value: newBalance}("");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        (uint256 tTransferAmount, uint256 tBNBFee, uint256 tLiquidityFee) = _getFeeValues(amount, isBuy);
        ...
        // _tOwned[address(this)] = tLiquidityFee.add(tBNBFee);
