...
contract PoolService is Ownable, PoolData {
    ...
    function addPoolService(IERC20 _lpToken, uint256 _lockBlockCount, uint256 _depositLimit, uint256 _firstDepositLimit) public {
    ...
    function setPoolService(uint256 _poolId, uint256 _minDepositLimit, uint256 _minFirstDepositLimit) public {
...
contract PoolData is Ownable {
    ...
    event LogSetMTokenAddress(address mTokenAddress);
    event LogUpgradeService(address serviceAddress);
    ...
    function getServiceAddress() public view returns (address) {
        return serviceAddress;
    }
    function upgradeService(address _serviceAddress) public onlyOwner {
        serviceAddress = _serviceAddress;
        emit LogUpgradeService(_serviceAddress);
    }
    ...
    /// @param _firstDepositLimit The minimum amount to first deposit.
    function addPool(IERC20 _lpToken, uint256 _lockBlockCount, uint256 _depositLimit, uint256 _firstDepositLimit) public onlyOwner {
        (bool success,) = address(serviceAddress).delegatecall(
            abi.encodeWithSignature("addPoolService(IERC20,uint256,uint256,uint256)", _lpToken, _lockBlockCount, _depositLimit, _firstDepositLimit)
        );
        require(success, 'Service Error!');
    ...
    function setPool(uint256 _poolId, uint256 _minDepositLimit, uint256 _minFirstDepositLimit) public onlyOwner {
        (bool success,) = address(serviceAddress).delegatecall(
            abi.encodeWithSignature("addPoolService(uint256,uint256,uint256)", _poolId, _minDepositLimit, _minFirstDepositLimit)
        );
        require(success, 'Service Error!');
