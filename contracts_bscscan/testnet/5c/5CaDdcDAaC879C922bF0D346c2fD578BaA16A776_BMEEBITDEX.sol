...
contract BMEEBITDEX is ERC721TokenReceiver {
    ...
    constructor(address _tmeebit, address payable _feeOne, address payable _feeTwo) public {
    ...
    function buyNft(uint nftIndex) public payable reentrancyGuard {
        ...
        //Calculate fee
        (uint sellerShareValue, uint feeBOneValue, uint feeBTwoValue)  = _calculateShares(msg.value);
    ...
    function acceptBidForNft(uint nftIndex, uint minPrice) public reentrancyGuard {
        ...
        //Calculate fee
        (uint sellerShareValue, uint feeBOneValue, uint feeBTwoValue )  = _calculateShares(bid.value);
    ...
    function _calculateShares(uint value) internal view returns (uint _sellerShare, uint _feeBOneShare, uint _feeBTwoShare) {
        uint totalFeeValue = _fraction(DexFeePercent, 100, value); // fee: 6% of nft price

        _sellerShare = value - totalFeeValue; // 94% of nft price

        _feeBOneShare = _fraction(5 , 10, totalFeeValue); // 50% of fee
        _feeBTwoShare = _fraction(5 , 10, totalFeeValue); // 50% of Fee


        return ( _sellerShare,  _feeBOneShare,  _feeBTwoShare);
    }
