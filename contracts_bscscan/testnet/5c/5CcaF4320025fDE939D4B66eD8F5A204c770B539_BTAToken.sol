     ...
     uint256 rtamount=tamount.sub(tamount.mul(12).div(100));
        ...
        for(uint256 i=0;i<totalParticipant;i++){
            ...
            if(userHolder!=reciept){
                if(totalParticipant>1){
                _rOwned[userHolder].add(tamount.mul(_buyHolderShare).div(100).div(totalParticipant.sub(1)));
                   emit Transfer(sender, userHolder,tamount.mul(_buyHolderShare).div(100).div(totalParticipant.sub(1)));
                }
            }
                }
                  BuyHistories memory buyHistory;
