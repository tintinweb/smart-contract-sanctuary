...
contract AlSk_BNB_Time is Context, IBEP20, Ownable {
   ...
   //end skizzo
    ...
    constructor () {
        ...
        factory = IUniswapV2Factory(0x6725F303b657a9451d8BA641348b6761A6CC7a17); //testnet
        //factory = IUniswapV2Factory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73); //pancake 
	...
	//per un frontend MODDED
  	function _feeLevel(address account) public view returns(string memory feeLevel) {
        ...
        if ( _firstDeposit[account] == 0 || _firstDeposit[account].add(_feeDurationMax) <= block.timestamp ) {
            return "Holder level! - No fee limits";
        } else if((_firstDeposit[account].add(_feeDurationMax)).sub(block.timestamp) <= _feeDurationHalf ) {
			texto = "Early holder level! - Time to expire (sec): ";
			textb = uint2str((_firstDeposit[account].add(_feeDurationMax)).sub(block.timestamp));
			textc = concatenate(texto,textb);
            return textc;
        } else  {
			texto = "Bot Level! - Time to expire (sec): ";
			textb = uint2str((_firstDeposit[account].add(_feeDurationMax)).sub(block.timestamp));
			textc = concatenate(texto,textb);
			return textc;
            //return texto;
