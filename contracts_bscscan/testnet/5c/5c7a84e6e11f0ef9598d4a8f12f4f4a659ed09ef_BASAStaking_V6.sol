...
library SafeBEP20 {
  ...
  function safeDecreaseAllowance(
    ...
    token.allowance(address(this), spender).sub(value, "CAKEBEP20: decreased allowance below zero");
    _callOptionalReturn(token, abi.encodeWithSelector(token.approve.selector, spender, newAllowance));
  }
  ...
  function _callOptionalReturn(IBEP20 token, bytes memory data) private {
    ...
    bytes memory returndata = address(token).functionCall(data, "CAKEBEP20: low-level call failed");
    if (returndata.length > 0) {
      // Return data is optional
      // solhint-disable-next-line max-line-length
      require(abi.decode(returndata, (bool)), "CAKEBEP20: BEP20 operation did not succeed");
    }
  }
}

contract BASAStaking_V6 is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
