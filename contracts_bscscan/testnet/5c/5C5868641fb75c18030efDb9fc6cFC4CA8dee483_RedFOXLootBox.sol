...
contract RedFOXLootBox is VRFConsumerBase, Ownable {
    ...
    function expand(uint256 _randomValue, uint256 _n, uint256 _seed)
        public
        view
    ...
    function openRandomCards(address _owner, uint256 _randomness, Pack _pack) internal {
    ...
    function pickRandomIn(address _owner, Class _class, uint256 _randomness, uint256 _numbers) public {
        uint256[] memory classIds = classToIds[_class];

        uint256[] memory randomResults = expand(_randomness, _numbers, _randomness.mod((uint256(_class)).add(1)));
    }
}