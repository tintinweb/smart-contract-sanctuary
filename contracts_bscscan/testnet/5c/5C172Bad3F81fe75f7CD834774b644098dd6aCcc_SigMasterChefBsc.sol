...
contract SigMasterChefBsc is Ownable {
    ...
    // bsc block number is store as uint32
    // eth block number is stored as uint64
...
//    address public sharedVault;
    ...
    uint256 public immutable startBscBlock;
    ...
    // Because this is BSC, we need to know approximate block number on the Ethereum
    // because SigToken which actually mints there according to it
    uint256 bscBlockA = 7_777_777; // May-27-2021 04:04:53 PM +UTC https://bscscan.com/block/7777777
    uint256 ethBlockA = 12_517_323; // May-27-2021 04:04:53 PM +UTC https://etherscan.io/block/12517323
    uint256 bscBlockB = 9494487; // Jul-26-2021 01:08:17 PM +UTC https://bscscan.com/block/9494487
    uint256 ethBlockB = 12901994; // Jul-26-2021 01:08:16 PM +UTC https://etherscan.io/block/12901994
    function updateReferenceBlockA(uint256 _bscBlockA, uint256 _ethBlockA) external onlyOwner {
        bscBlockA = _bscBlockA;
        ethBlockA = _ethBlockA;
    }
    function updateReferenceBlockB(uint256 _bscBlockB, uint256 _ethBlockB) external onlyOwner {
        bscBlockB = _bscBlockB;
        ethBlockB = _ethBlockB;
    }
    function bscToEthBlockNumber(uint256 bscBlock) public view returns (uint256) {
        int256 a = int256(bscBlock) - int256(bscBlockA);
        int256 b = int256(ethBlockB) - int256(ethBlockA);
        int256 c = int256(bscBlockB) - int256(bscBlockA);
        return uint256(int256(ethBlockA) + (a * b / c));
    }
    function ethToBscBlockNumber(uint256 ethBlock) public view returns (uint256) {
        int256 a = int256(ethBlock) - int256(ethBlockA);
        int256 b = int256(bscBlockB) - int256(bscBlockA);
        int256 c = int256(ethBlockB) - int256(ethBlockA);
        return uint256(int256(bscBlockA) + (a * b / c));
    }

    // function which returns approximate rewards of corresponding function
    // at https://etherscan.io/address/0x98C32b59a0AC00Cd33750427b1A317eBcf84D0F7
    function totalRewardAtBlock(uint256 atBscBlock) public view returns (uint256) {
        uint256 atEthBlock = bscToEthBlockNumber(atBscBlock);
        if (atEthBlock < startBscBlock) return 0;
        uint256 t = (atEthBlock - startBscBlock).div(rewardTimeFactor);
...
//        address _sharedVault,
        uint256 _startEthBlock
    ) public {
        SIG_TOKEN = _sigToken;
//        devaddr = _devaddr;
//        sharedVault = _sharedVault;
        startBscBlock = ethToBscBlockNumber(_startEthBlock);
    }

    // Faction of total mint which is belong to BSC chain. Same about of token gets lock on Ethereum bridge
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        ...
        uint256 lastRewardBlock = block.number > startBscBlock ? block.number : startBscBlock;
