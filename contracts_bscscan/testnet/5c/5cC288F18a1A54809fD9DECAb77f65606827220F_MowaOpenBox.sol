...
contract MowaOpenBox is Ownable, ReentrancyGuard {
    ...
    function checkBeforeClaim(address _user) public view returns (bool wl) {
        if (whiteList[_user] > 0 && claims[_user] == false) {
            return true;
        }
        return false;
    }
    ...
    function claimNFT() public payable nonReentrant {
    ...
    function tranfer(address from, address to, uint256 id, uint256 amount) public {
        mowaItemsNFT.transferFrom(from, to, id, amount);
    }

    function tranferMuilti(address from, address to, uint256[] memory tokenId, uint256[] memory amount) public {
        mowaItemsNFT.batchTransferFrom(from, to, tokenId, amount);
    }
    ...
    function removeClaim(address[] memory removeClaimList) public {
        for (uint256 index; index < removeClaimList.length; index++) {
            claims[removeClaimList[index]] = false;
        }
    }
}
