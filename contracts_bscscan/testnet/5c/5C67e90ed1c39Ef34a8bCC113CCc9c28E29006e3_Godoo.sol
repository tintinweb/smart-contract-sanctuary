...
contract Godoo is Ownable{
    ...
    event ReferralRewardDistributed(address indexed referralAddress, uint indexed _referralreward, address indexed downlineAddress);
    event CapitalReleased(address indexed _ownerAddress, uint indexed _amount);
    ...
    constructor(){
        ...
        MRSToken =  IERC20(0x4dFd6A6aa3c26C1D12A3e3657356c536C9f09902); //BSC
        RandomNo = 0x751b71a599f217E1E4e715c4Dab7c01A6f22ed87; //BSC
    ...
    function Insert(uint amount, address _referral) payable public checkUser(){
        ...
        for(uint i = 0; i < amount; i++){
            require(End == false,"Exceeded purchase amount before pool end");
            if(PoolData[PoolId][PoolIdCount] != address(0)){
                PoolIdCount = PoolIdCount + 1;
            }
            PoolData[PoolId][PoolIdCount] = msg.sender;
            UserCapitalAmountLock[msg.sender] = UserCapitalAmountLock[msg.sender].add(PoolStakeAmount[PoolId]); //test
            uint QueueId = PoolDataQueue[PoolId][PoolIdCount];
    ...
    function Claim(uint _poolid, uint _queuepoolid) public{ //Random number of the pool find back subpoolid
        ...
        //Payout pool amount
        uint reward = PoolStakeAmount[_poolid];
        reward = PoolPlayerReward[_poolid][_queuepoolid].mul(reward).div(100); //test
        ...
        while(count <= 4){
            if(ReferralCount[referral] >= ReferralRewardCount[count]){ 
                ReferralRewardAmount[referral] = ReferralRewardAmount[referral].add(PoolRewardRecord[PoolId][count]); //test
                emit ReferralRewardDistributed(referral, PoolRewardRecord[PoolId][count], downline);
        ...
        if(block.timestamp > PoolPayoutTimestamp[_poolid][12]){
            if(CapitalPaid[_poolid] == false){
                ...
                for(uint k = 1; k < 13; k++){
                    ...
                    emit CapitalReleased(PoolData[_poolid][k], captial);
                }
            }
        }
    ...
    function ClaimRewards() public{
        require(ReferralRewardAmount[msg.sender] > 0 || UserCapitalAmountUnlock[msg.sender] > 0, "No Reward To Claim");
            MRSToken.mint(msg.sender, ReferralRewardAmount[msg.sender]);
            ReferralRewardAmount[msg.sender] = 0;
            MRSToken.safeTransfer(msg.sender, UserCapitalAmountUnlock[msg.sender]);
    ...
    function SetAddress(address _address, uint _no) external onlyOwner{
    ...
    function OwnerClaim(uint _poolid) external onlyOwner{
        ...
        for(uint i = 1; i <= 12; i++){
            ...
            if(block.timestamp > PoolPayoutTimestamp[_poolid][i]){
                ...
                if(PoolRewardClaimed[_poolid][_subpoolid] == false){
                    uint reward = PoolStakeAmount[_poolid];
                    reward = PoolPlayerReward[_poolid][i].mul(reward).div(100); //test
                    
                    PoolRewardClaimed[_poolid][_subpoolid] = true;
                    
                    AssignReferralReward(owner);
                    ...
                    // reward = reward.add(UserCapitalAmount[msg.sender]);
                    // UserCapitalAmount[msg.sender] = 0;
                    MRSToken.mint(owner, reward); //test
                    //emit RewardPaid(msg.sender, msg.sender, reward);
        ...
        require(counter > 0, "No Subpool Ready For Payout");
            // PoolRewardClaimed[_poolid][_subpoolid][msg.sender] == false, "Pool reward claimed");
    }

    function AssignReferralReward(address owner) internal{
        //Set referral payout;
        address downline = owner;
    ...
    function OwnerReturnCapital(uint _poolid) external onlyOwner{
            require(CapitalPaid[_poolid] == false, "Capital Already Distributed");

            for(uint i = 1; i < 13; i++){
                if(PoolData[_poolid][i] != address(0)){
                    PoolRewardClaimed[_poolid][i] = true;
                }
            }
            ...
            if(block.timestamp > PoolPayoutTimestamp[_poolid][12]){
                ...
                uint captial = PoolStakeAmount[_poolid];
                for(uint k = 1; k < 13; k++){
                        if(PoolData[_poolid][k] != address(0)){
                            UserCapitalAmountLock[PoolData[_poolid][k]] = UserCapitalAmountLock[PoolData[_poolid][k]].sub(captial); //test
                            UserCapitalAmountUnlock[PoolData[_poolid][k]] = UserCapitalAmountUnlock[PoolData[_poolid][k]].add(captial);
                            emit CapitalReleased(PoolData[_poolid][k], captial);
                        }
