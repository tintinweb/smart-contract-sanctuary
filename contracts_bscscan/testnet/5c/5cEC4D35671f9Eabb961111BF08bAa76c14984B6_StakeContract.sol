...
contract StakeContract is Ownable {
    ...
    function rewardBalanceOf(address _user) external view returns (uint256) {
        uint256 reward = (_stakedBalance[_user].div(totalpoolstacked)).mul(rewardperblock);
        ...
        if ( lastRewardBlock < lastblock) {
            ...
            for (uint256 id = 0; id < length; ++id) {
                ...
                if (_stakedBalance[user] == 0){
                    ...
                    uint256 reward = (_stakedBalance[user].div(totalpoolstacked)).mul(rewardperblock);
