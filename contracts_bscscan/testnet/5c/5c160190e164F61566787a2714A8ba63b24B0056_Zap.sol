...
contract Zap is IZap, Ownable {
    ...
    */
    ...
    function getEstimatedLpToken(address fromToken, uint fromTokenAmount, address toToken) external view returns (uint liquidity) {
        require(isLpToken(toToken), 'toToken must be LP token');
        
        IAlitaPair pair = IAlitaPair(toToken);
        address token0 = pair.token0();
        address token1 = pair.token1();
        uint totalSupply = pair.totalSupply();
        uint swapAmount = fromTokenAmount.div(2);
        uint amount0 = fromToken == token0 ? swapAmount : getPoolAddedAmount(fromToken, token0, swapAmount);
        uint amount1 = fromToken == token1 ? swapAmount : getPoolAddedAmount(fromToken, token1, swapAmount);
        if (totalSupply == 0) {
    ...
    function getPoolAddedAmount(address fromToken, address toToken, uint swapAmount) public view returns (uint amount) {
        ...
        amount = ROUTER.getAmountsOut(swapAmount, path)[1];
    }
