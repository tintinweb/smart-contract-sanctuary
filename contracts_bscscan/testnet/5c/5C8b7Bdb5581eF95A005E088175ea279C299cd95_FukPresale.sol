...
contract FukPresale {
  ...
  constructor () public {
    presaleSize = 150000000000 * multiplier; //the amount of tokens that are going to be added to the presale contract, change only the 70000, keep the multiplier as it is basically the 18 decimals
    token = TOKEN(0x45BE773c07C1c66d18A969A5Ab9990Bf56E851b1); //fuk token address
    ceoAddress = msg.sender;
    buyLimit = SafeMath.mul(1235294117, multiplier); //buy limit in fuk
    bnbPrice = 350; //bnb current price, you can also update it afterwards, used to calculate the token price in bnb
