...
contract BabyOHM is Context, IERC20, Ownable {
    ...
    function excludeFromMax(address account) public onlyOwner {
		require(!isExcludedFromMax[account], "Account is already excluded from limits");
		isExcludedFromMax[account] = true; 
	}

    function includeInMax(address account) public onlyOwner {
		require(isExcludedFromMax[account], "Account is not excluded from limits");
		isExcludedFromMax[account] = false; 
	}

	function _isExcludedFromMax(address account) public view returns (bool) {
		return isExcludedFromMax[account]; 
	}
