...
contract Templar is Owner {
    ...
    constructor(
        ...
        setMissionInfo(1, 3000, 50e18);
        setMissionInfo(2, 2000, 100e18);
        setMissionInfo(3, 1000, 150e18);
        setMissionInfo(4, 600, 200e18);
        ...
        checkPaused
    {
        ...
        if(supplyInfo[supplyId].token != address(2)) {
            require(accountInfo[msg.sender].inviter != address(0), "no bind");
        } else {
            require(block.timestamp >= supplyInfo[supplyId].startTime, "no start");
            require(block.timestamp <= supplyInfo[supplyId].endTime, "have end");
        }
        ...
        if(supplyInfo[supplyId].token != address(2)) {
            inviter = accountInfo[msg.sender].inviter;
        ...
        token.transferFrom(msg.sender, feeAccount, fee - inviterPart);
            ...
            (bool success, ) = feeAccount.call{value: fee}(new bytes(0));
    ...
    function getIsFlag() public view returns (bool) {
        return isFlag;
    }
    ...
    function setSupplyInfo(
        ...
        supplyInfo[id].startTime = startTime;
        supplyInfo[id].endTime = endTime;
    ...
    function setIsFlag(bool isFlag_) public onlyOwner {
        isFlag = isFlag_;
    }
