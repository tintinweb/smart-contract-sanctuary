...
contract BondCalculator is IBondCalculator {
    ...
    function markdown(address pair_) external view override returns (uint256) {
        (uint256 reserve0, uint256 reserve1, ) = IUniswapV2Pair(pair_).getReserves();

        uint256 reserve;
        if (IUniswapV2Pair(pair_).token0() == gyro) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20(gyro).decimals())).div(getTotalValue(pair_));
    }
}
