...
contract PriceOracle {
    ...
    /**
      * @notice Get the underlying price of a wlToken asset
...
/**
 * @title welnance's WBep20 Contract
 * @notice WLTokens which wrap an EIP-20 underlying
 * @author welnance
 */
contract WBep20 is WLToken, WBep20Interface {
    /**
    ...
    function initialize(address underlying_,
        ...
        // WLToken initialize does the bulk of the work
    ...
    /**
     * @notice Sender supplies assets into the market and receiver receives wlTokens in exchange
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param receiver the account which is receiving the wlTokens
     * @param mintAmount The amount of the underlying asset to supply
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrow(address borrower, uint repayAmount, WLTokenInterface wlTokenCollateral) external returns (uint) {
        (uint err,) = liquidateBorrowInternal(borrower, repayAmount, wlTokenCollateral);
        return err;
    }
...
/**
 * @title Welnance's WLToken Contract
 * @notice Abstract base for WLTokens
 * @author Welnance
 */
contract WLToken is WLTokenInterface, Exponential, TokenErrorReporter {
    /**
    ...
    function getAccountSnapshot(address account) external view returns (uint, uint, uint, uint) {
        uint wlTokenBalance = accountTokens[account];
        uint borrowBalance;
        ...
        return (uint(Error.NO_ERROR), wlTokenBalance, borrowBalance, exchangeRateMantissa);
    }
    ...
    /**
     * @notice Returns the current per-block borrow interest rate for this wlToken
    ...
    /**
     * @notice Returns the current per-block supply interest rate for this wlToken
    ...
    /**
     * @notice Calculates the exchange rate from the underlying to the WLToken
    ...
    /**
     * @notice Get cash balance of this wlToken in the underlying asset
    ...
    /**
     * @notice User supplies assets into the market and receives wlTokens in exchange
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
         ...
         *  Note: The wlToken must handle variations between BEP-20 and BNB underlying.
         *  `doTransferIn` reverts if anything goes wrong, since we can't be sure if
         *  side-effects occurred. The function returns the amount actually transferred,
         *  in case of a fee. On success, the wlToken holds an additional `actualMintAmount`
         *  of cash.
         */
        vars.actualMintAmount = doTransferIn(minter, mintAmount);

        /*
         * We get the current exchange rate and calculate the number of wlTokens to be minted:
        ...
        /*
         * We calculate the new total supply of wlTokens and minter token balance, checking for overflow:
     ...
     * @notice Sender supplies assets into the market and receiver receives wlTokens in exchange
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param receiver The address of the account which is receiving the wlTokens
     * @param mintAmount The amount of the underlying asset to supply
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    ...
    /**
     * @notice Payer supplies assets into the market and receiver receives wlTokens in exchange
     * @dev Assumes interest has already been accrued up to the current block
     * @param payer The address of the account which is paying the underlying token
     * @param receiver The address of the account which is receiving wlToken
     * @param mintAmount The amount of the underlying asset to supply
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    function mintBehalfFresh(address payer, address receiver, uint mintAmount) internal returns (uint, uint) {
        ...
        /*
         * We calculate the new total supply of wlTokens and receiver token balance, checking for overflow:
         *  totalSupplyNew = totalSupply + mintTokens
     ...
     * @notice Sender redeems wlTokens in exchange for a specified amount of underlying asset
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param redeemAmount The amount of underlying to receive from redeeming wlTokens
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    ...
    /**
     * @notice User redeems wlTokens in exchange for the underlying asset
     * @dev Assumes interest has already been accrued up to the current block
     * @param redeemer The address of the account which is redeeming the tokens
     * @param redeemTokensIn The number of wlTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be non-zero)
     * @param redeemAmountIn The number of underlying tokens to receive from redeeming wlTokens (only one of redeemTokensIn or redeemAmountIn may be non-zero)
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
         ...
         *  Note: The wlToken must handle variations between BEP-20 and BNB underlying.
         *  On success, the wlToken has redeemAmount less of cash.
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
         ...
         *  Note: The wlToken must handle variations between BEP-20 and BNB underlying.
         *  On success, the wlToken borrowAmount less of cash.
         *  doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
         */
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
         ...
         *  Note: The wlToken must handle variations between BEP-20 and BNB underlying.
         *  On success, the wlToken holds an additional repayAmount of cash.
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, WLTokenInterface wlTokenCollateral) internal nonReentrant returns (uint, uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
        ...
        error = wlTokenCollateral.accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted liquidation failed
        ...
        return liquidateBorrowFresh(msg.sender, borrower, repayAmount, wlTokenCollateral);
    }
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, WLTokenInterface wlTokenCollateral) internal returns (uint, uint) {
        /* Fail if liquidate not allowed */
        uint allowed = comptroller.liquidateBorrowAllowed(address(this), address(wlTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
        ...
        /* Verify wlTokenCollateral market's block number equals current block number */
        if (wlTokenCollateral.accrualBlockNumber() != getBlockNumber()) {
        ...
        (uint amountSeizeError, uint seizeTokens) = comptroller.liquidateCalculateSeizeTokens(address(this), address(wlTokenCollateral), actualRepayAmount);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");

        /* Revert if borrower collateral token balance < seizeTokens */
        require(wlTokenCollateral.balanceOf(borrower) >= seizeTokens, "LIQUIDATE_SEIZE_TOO_MUCH");

        // If this is also the collateral, run seizeInternal to avoid re-entrancy, otherwise make an external call
        uint seizeError;
        if (address(wlTokenCollateral) == address(this)) {
            seizeError = seizeInternal(address(this), liquidator, borrower, seizeTokens);
        } else {
            seizeError = wlTokenCollateral.seize(liquidator, borrower, seizeTokens);
        }
        ...
        emit LiquidateBorrow(liquidator, borrower, actualRepayAmount, address(wlTokenCollateral), seizeTokens);

        /* We call the defense hook */
        comptroller.liquidateBorrowVerify(address(this), address(wlTokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
     ...
     * @dev Will fail unless called by another wlToken during the process of liquidation.
     *  Its absolutely critical to use msg.sender as the borrowed wlToken and not a parameter.
     * @param liquidator The account receiving seized collateral
     * @param borrower The account having collateral seized
     * @param seizeTokens The number of wlTokens to seize
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    ...
    /**
     * @notice Transfers collateral tokens (this market) to the liquidator.
     * @dev Called only during an in-kind liquidation, or by liquidateBorrow during the liquidation of another WLToken.
     *  Its absolutely critical to use msg.sender as the seizer wlToken and not a parameter.
     * @param seizerToken The contract seizing the collateral (i.e. borrowed wlToken)
     * @param liquidator The account receiving seized collateral
     * @param borrower The account having collateral seized
     * @param seizeTokens The number of wlTokens to seize
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
    ...
    function _addReservesFresh(uint addAmount) internal returns (uint, uint) {
         ...
         *  Note: The wlToken must handle variations between BEP-20 and BNB underlying.
         *  On success, the wlToken holds an additional addAmount of cash.
         *  doTransferIn reverts if anything goes wrong, since we can't be sure if side effects occurred.
         *  it returns the amount actually transferred, in case of a fee.
         */
...
contract WLTokenStorage {
    ...
    /**
     * @notice Contract which oversees inter-wlToken operations
     */
    ...
    /**
     * @notice Initial exchange rate used when minting the first WLTokens (used when totalSupply = 0)
     */
...
contract WLTokenInterface is WLTokenStorage {
    /**
     * @notice Indicator that this is a WLToken contract (for inspection)
     */
    bool public constant isWLToken = true;
     ...
     */
    event LiquidateBorrow(address liquidator, address borrower, uint repayAmount, address wlTokenCollateral, uint seizeTokens);
...
contract WBep20Storage {
    /**
     * @notice Underlying asset for this WLToken
     */
    address public underlying;
}

contract WBep20Interface is WBep20Storage {
...
contract WDelegatorInterface is WDelegationStorage {
    /**
...
contract WDelegateInterface is WDelegationStorage {
    /**
...
/**
  * @title Welnance's InterestRateModel Interface
  * @author Welnance
  */
