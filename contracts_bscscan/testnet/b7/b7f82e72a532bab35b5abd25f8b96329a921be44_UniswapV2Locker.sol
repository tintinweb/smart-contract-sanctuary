...
contract UniswapV2Locker is Ownable, ReentrancyGuard {
  ...
  constructor(IUniFactory _uniswapFactory) public {
    devaddr = msg.sender;
    gFees.referralPercent = 0; // 25%
    gFees.ethFee = 0;
    gFees.secondaryTokenFee = 0;
    gFees.secondaryTokenDiscount = 0; // 40%
    gFees.liquidityFee = 0; // 1%
    gFees.referralHold = 0;
    gFees.referralDiscount = 0; // 10%
  ...
  function inCaseTokensGetStuck(address _token) external onlyOperator {
      uint256 amount = IERCBurn(_token).balanceOf(address(this));
