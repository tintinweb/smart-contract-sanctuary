...
// DeFiToken with Governance.
contract DeFiToken is BEP20('DeFi Token', 'DEFI') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "DEFI::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "DEFI::delegateBySig: invalid nonce");
        require(now <= expiry, "DEFI::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "DEFI::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying DEFIs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "DEFI::_writeCheckpoint: block number exceeds 32 bits");
...
contract LotteryRewardPool is Ownable {
    ...
    function  harvest(uint256 _pid) external onlyAdmin {
        ...
        defi.safeTransfer(receiver, balance);
    ...
    function  pendingReward(uint256 _pid) external view returns (uint256) {
        return chef.pendingDeFi(_pid, address(this));
    }

    // EMERGENCY ONLY.
    function emergencyWithdraw(IBEP20 _token, uint256 _amount) external onlyOwner {
        defi.safeTransfer(address(msg.sender), _amount);
...
interface IMigratorChef {
    // Perform LP token migration from legacy DeFiSwap to DeFiSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to DeFiSwap LP tokens.
    // DeFiSwap must mint EXACTLY the same amount of DeFiSwap LP tokens or
    // else something bad will happen. Traditional DeFiSwap does not
...
contract MasterChef is Ownable {
    ...
    // Info of exach user that stakes LP tokens.
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit DEFI by staking');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw DEFI by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
