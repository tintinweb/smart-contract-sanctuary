...
contract BSC_Defi {
    ...
    function buyEggs(address ref) public payable{
        ...
        /** send referral bonus from new deposits. **/
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                /** use token value to give referral bonuses **/
                uint256 eggsReward = eggsBought.mul(REFERRAL).div(PERCENTS_DIVIDER);
                users[upline].claimedEggs = users[upline].claimedEggs.add(eggsReward);
                users[upline].referralEggRewards = users[upline].referralEggRewards.add(eggsReward);
                totalRefBonus = totalRefBonus.add(eggsReward);
            }
        }
