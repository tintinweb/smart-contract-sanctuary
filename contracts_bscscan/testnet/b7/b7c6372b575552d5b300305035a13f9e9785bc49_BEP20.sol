...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    // customize to lock reward 70% and release in 7days
    uint256 private _rewardLock = 7000; // lock 70%
    uint256 private _totalBlockRelease = 7 * 24 * 60 * 60 / 5; // BKC Blocktime 5 sec
...
// TukTukToken with Governance.
contract TukTukToken is BEP20('TukTuk', 'TUK') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "TUK::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "TUK::delegateBySig: invalid nonce");
        require(now <= expiry, "TUK::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "TUK::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying TUKs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "TUK::_writeCheckpoint: block number exceeds 32 bits");
