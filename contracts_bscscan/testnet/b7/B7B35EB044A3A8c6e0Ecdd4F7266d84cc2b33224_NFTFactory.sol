...
contract NFTFactory is Context, ReentrancyGuard, VRFConsumerBase {
  ...
  function getNFTInfo(uint256 _id) public view returns(uint256 id, uint256 edition, uint256 num, uint256 kind) {
    NFTInfo storage nft = nfts[_id];
    require(nft.cardInfoId >= 0, "NFT doesn't exist on this id");
    CardInfo storage card = deckInfo[nft.cardInfoId];
    return (_id, nft.edition, card.num, card.kind);
  }
  ...
  function upgradeCards(uint256[] memory _ids, bytes memory _data) external whenNotPaused {
    ...
    emit CardUpgraded(_ids, id, edition, newCardInfo.num, newCardInfo.kind);
  }
  ...
  function generateCardInfos() internal {
    ...
    for(uint256 i=0; i<4; i++) {
      CardInfo storage aceCard = deckInfo[48 + i];
      aceCard.num = 1;
      aceCard.kind = i;
    ...
    CardInfo storage jokerCard = deckInfo[52];
    jokerCard.num = 14;
    jokerCard.kind = 0;
    jokerCard.qty = 4;
