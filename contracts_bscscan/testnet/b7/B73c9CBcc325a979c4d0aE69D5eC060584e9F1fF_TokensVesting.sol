...
contract TokensVesting is Ownable, ITokensVesting {
    ...
    function development() external view returns (uint256) {
        return _getTotalAmountByParticipant(Participant.Development);
    }
    ...
    function developmentReleased() external view returns (uint256) {
        return _getReleasedAmountByParticipant(Participant.Development);
    }
    ...
    function releaseDevelopment() public {
        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            if (_beneficiaries[i].participant == Participant.Development) {
                release(i);
            }
        }
    }
    ...
    function _releasableAmount(
        ...
        return
            _vestedAmount(totalAmount_, tgeAmount_, cliff_, duration) -
            releasedAmount_;
    }
    ...
    function _vestedAmount(
        ...
        if (block.timestamp < genesisTimestamp) {
            ...
            if (timeLeftAfterStart < cliff) {
                ...
                return
                    ((gaps * (totalAmount - tgeAmount)) / duration) *
                    GAP +
                    tgeAmount;
            }
        }
