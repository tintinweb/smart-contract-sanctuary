...
abstract contract Earning is OwnableUpgradeable, IEarning {
    ...
    function _updateState() internal {
        ...
        if (accDeposit != 0)
            accRewardPerShare = accRewardPerShare + (tokenReward * 1e12) / accDeposit;
    }
...
contract MinterV2 is IMinter, Initializable, OwnableUpgradeable {
    ...
    function getAccumulatedRewardFarm(uint256 _lastAccumulatedReward, uint256 _blockNumber, address _addr) external view returns (uint256, uint256) {
        uint256 _accFarmReward = accFarmReward;
        uint256 pendingReward;
        if (accMultiplierForFarms != 0)
            _accFarmReward = accFarmReward + (Y * (_blockNumber - lastFarmRewardBlock)) / accMultiplierForFarms;
        pendingReward = (_accFarmReward - _lastAccumulatedReward) * farm[_addr].multiplier;
        return (pendingReward, _accFarmReward);
    }

    function getAccumulatedRewardStaking(uint256 _lastAccumulatedReward, uint256 blockNumber, address _addr) external view returns (uint256, uint256) {
    ...
    function getStatus(
        ...
        if (_divisor != 0)
            f.rewardPerBlock = (farm[_farm].multiplier * _factor) / _divisor;
    ...
    function updateFarming() internal {
        if (accMultiplierForFarms != 0)
            accFarmReward = accFarmReward + (Y * (block.number - lastFarmRewardBlock)) / accMultiplierForFarms;
