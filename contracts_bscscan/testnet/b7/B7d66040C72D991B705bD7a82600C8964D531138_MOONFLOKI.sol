...
interface RewardTokenOptionalInterface {
 function withdrawableDividendOf(address _owner) external view returns(uint256);
function withdrawnDividendOf(address _owner) external view returns(uint256);
 function accumulativeDividendOf(address _owner) external view returns(uint256);
}

contract RewardToken is BEP20, RewardTokenInterface, RewardTokenOptionalInterface {
  using SafeMath for uint256;
...
contract MOONFLOKIRewardTracker is RewardToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() RewardToken("MOONFLOKI_Reward_Tracker", "MOONFLOKI_Reward_Tracker") {
        claimWait = 7200;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "MOONFLOKI_Reward_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "MOONFLOKI_Reward_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MOONFLOKI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "MOONFLOKI_Reward_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MOONFLOKI_Reward_Tracker: Cannot update claimWait to same value");
...
contract MOONFLOKI is BEP20, Ownable {
 event UpdateUniswapV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    using SafeMath for uint256;
    uint256 public  Rewardtax = 4;
    uint256 public  Liquiditytax = 4;
    uint256 public  Marketingtax = 4;
    uint256 public  TotalFees;
    uint256 public gasForProcessing = 300000;
    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    MOONFLOKIRewardTracker public RewardTracker;
  ...
  ILP public liquidutyReciever;
    ...
    constructor() BEP20("MOON FLOKI", "MNF") {
        ...
        RewardTracker = new MOONFLOKIRewardTracker();
        TotalFees = Rewardtax.add(Liquiditytax).add(Marketingtax);
        ...
        RewardTracker.excludeFromDividends(address(uniswapV2Pair));
        RewardTracker.excludeFromDividends(address(uniswapV2Router));
        RewardTracker.excludeFromDividends(address(RewardTracker));
        RewardTracker.excludeFromDividends(0x000000000000000000000000000000000000dEaD);
        RewardTracker.excludeFromDividends(0x0000000000000000000000000000000000000000);
lp = _uniswapV2Pair;
 ...
 function rebaseToken(uint256 epoch, int256 supplyDelta)
  ...
  returns (uint256)
  {
    ...
    function changeIsDividendExempt(address holder, bool exempt) external onlyOwner {
        ...
        if(exempt){
            RewardTracker.excludeFromDividends(holder);
        }else{
             try RewardTracker.includeInDividends(payable(holder), balanceOf(holder)) {} catch {}
        }
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MOONFLOKI: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MOONFLOKI: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MOONFLOKI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MOONFLOKI: Cannot update gasForProcessing to same value");
    ...
    function getTotalDividendsDistributed() external view returns (uint256) {
        return RewardTracker.totalDividendsDistributed();
    }

    function withdrawableDividendOf(address account) public view returns(uint256) {
        return RewardTracker.withdrawableDividendOf(account);
    }
