...
contract CRTStake is Owned {
    
    //initializing safe computations
    using SafeMath for uint;

    //CRT contract address
    address public crt;
    //total amount of staked crt
     ...
     * transfers CRT from sender's address into the smart contract
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IERC20(crt).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        //makes sure amount is more than the registration fee and the minimum deposit
        require(_amount >= registrationTax.add(minimumStakeValue), "Must send at least enough CRT to pay registration fee.");
        //makes sure smart contract transfers CRT from user
        require(IERC20(crt).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
        ...
        //update the total staked CRT amount in the pool
    ...
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IERC20(crt).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(crt).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient CRT balance in pool');
        ...
        IERC20(crt).transfer(msg.sender, totalReward);
        //emit event
    ...
    function rewardPool() external view onlyOwner() returns(uint claimable) {
        return (IERC20(crt).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        //makes sure _amount is not more than required balance
        require((IERC20(crt).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient CRT balance in pool');
        //transfers _amount to _address
        IERC20(crt).transfer(msg.sender, _amount);
        //emit event
