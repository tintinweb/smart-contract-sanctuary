...
contract preSaledeployerOPT is ReentrancyGuard {
    ...
    function getTotalNumberOfTokens(
        ...
        uint256 tokensForSell = _hardCap.mul(_tokenPrice).mul(1e8).div(1e18);
        tokensForSell = tokensForSell.add(tokensForSell.mul(adminFeePercent).div(100));
        uint256 tokensForListing = (_hardCap.mul(_liquidityPercent).div(100)).mul(_listingPrice).mul(1e8).div(1e18);
        return tokensForSell.add(tokensForListing).div(1e8);
    }
