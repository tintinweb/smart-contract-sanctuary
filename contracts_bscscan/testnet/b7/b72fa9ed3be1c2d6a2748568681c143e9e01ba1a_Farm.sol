...
contract Farm is Ownable {
    ...
    constructor () public {
          ...
          genesisAddress = address(0xF92294D80Fa5B755dE0f95492065FBda6E45a4d9);
    ...
    function claim(uint256 _orderIndex) public returns (bool) {
        ...
        if(order.exitProfitIndex==0){
            ...
            profitAmount = order.nftValue.mul(order.apy).div(100).div(365).div(dayTime).mul(exitDiff);
    ...
    function profitAmountOf(uint256 _orderIndex) public view returns (uint256 ProfitAmount) {
        ...
        if(order.exitProfitIndex==0){
            ...
            return order.nftValue.mul(order.apy).div(100).div(365).div(dayTime).mul(exitDiff);
        }else if(order.exitProfitIndex==1){
    ...
    function joinFarm(address _inviterAddress,uint256 _tokenId) public returns (bool) {
        ...
        uint256 nftValue;
        if(_tokenId<=8){
            nftValue = 50000*10**18;
            tokenApy = 800;
        }else if(_tokenId>=9&&_tokenId<=16){
            nftValue = 20000*10**18;
            tokenApy = 500;
        }else if(_tokenId>=17&&_tokenId<=24){
            nftValue = 10000*10**18;
            tokenApy = 450;
        }else if(_tokenId>=25&&_tokenId<=32){
            nftValue = 5000*10**18;
            tokenApy = 360;
        }else if(_tokenId>=33&&_tokenId<=40){
            nftValue = 3000*10**18;
            tokenApy = 300;
        }else if(_tokenId>=41&&_tokenId<=80){
            nftValue = 2000*10**18;
            tokenApy = 280;
        }else if(_tokenId>=81&&_tokenId<=8000){
            nftValue = 1000*10**18;
            tokenApy = 200;
        }else{
            require(false,"-> _tokenId: Unable to get APY for TokenID.");
        }

        farmOrders[farmNowTotalCount] = FarmOrder(farmNowTotalCount,msg.sender,true,block.timestamp,0,_tokenId,nftValue,tokenApy,0,0,block.timestamp);// add farmOrders
    ...
    function farmOrdersOf(uint256 _orderIndex) public view returns (uint256 Index,address Account,bool IsExist,uint256 JoinTime,uint256 ExitTime,uint256 TokenId,uint256 NftValue,uint256 Apy,uint256 ProfitAmount,
        uint256 ExitProfitIndex,uint256 LastProfitTime){
        FarmOrder storage order = farmOrders[_orderIndex];
        return (order.index,order.account,order.isExist,order.joinTime,order.exitTime,order.tokenId,order.nftValue,order.apy,order.profitAmount,order.exitProfitIndex,order.lastProfitTime);
    }
