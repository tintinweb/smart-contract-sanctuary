...
contract KISHUCAKEPUNKSFEMALE is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable  {
    ...
    uint256 public retirementEligibility = 120 days;
    uint256 public breederEligibility = 90 days;
    uint256 public releasePrice;
    uint256 public slotsused;
    ...
    constructor() ERC721("KISHUCAKE PUNKS FEMALE", "KISHUCAKE PUNKS FEMALE") {
        ...
        minikishuAddress = 0x8E0B0510Eb2669eAAD50B70E1FE998D1af7957bA;
        minikishuAddress = 0x26bf6087906C7eaf89Aa8ff14CaED55e5B05e4d8; // REMOVE FOR MAINNET
       
        stakingAmountRequired = 50000000 * 10 ** 9;
        femalebreedingtokenAddress = 0xB4dd34cF0943579E69bcD0131A870Ce9d1c3CEe9;
        stakingforfemalestime = 365 * 3 * 1 days;
        stakingforfemalestime = 10 minutes; //DELETE THIS LINE ON MAINNET
       
        releasePrice = 5000000000000000;
        bondContractAddress = 0xac73Dc223e1307Be5Ea4Cc5ef0c61657a62A4711;
        max_quantity = 1200;
        stakePrice = 500000000000000000;
        minikishuReserveAddress = 0xC6baF6c531beb5E7395ADc15C0f21083dd522c96; // change to minikkishu reserv address
    }
   ...
   function setEcosystemContracts ( address[] memory _ecosystemcontracts ) public onlyOwner {
    ...
    function stakeForFemales() public payable returns(uint256)  {
          ...
          femaleStakes[femaleStakeCount].stakeamount = stakingAmountRequired;
          femaleStakes[femaleStakeCount].lastrelease = block.timestamp;
          femaleStakes[femaleStakeCount].nextrelease = block.timestamp + 30 minutes; //change to days
          minikishuStaked[msg.sender] += stakingAmountRequired;
    ...
    function closeMyStake ( uint256 _stakenumber ) public {
        ...
        minikishuStaked[msg.sender] -= stakingAmountRequired;
        ERC20 _minikishu = ERC20 ( minikishuAddress );
        _minikishu.transfer( msg.sender, femaleStakes[femaleStakeCount].stakeamount);
    ...
    function bondOut ( uint256 _stakenumber ) public {
        ...
        _bondcontract.BondOut ( msg.sender, stakingAmountRequired );
        
        uint256 _amount = femaleStakes[femaleStakeCount].stakeamount;
        femaleStakes[femaleStakeCount].stakeamount = 0;
        
         ERC20 _minikishu = ERC20 ( minikishuAddress );
        _minikishu.transfer( bondContractAddress, _amount);
         minikishuStaked[msg.sender] -= _amount;
        femaleStakes[femaleStakeCount].bondedOut = true;
        femaleStakes[femaleStakeCount].closed = true;
    ...
    function mintMyFemale(uint256 _stakenumber ) public   {
        ...
        ERC20 _minikishu = ERC20 ( minikishuAddress );
        _minikishu.transferFrom ( msg.sender, address(this), releaseCost(_stakenumber) );  
        _minikishu.transfer ( minikishuReserveAddress, releaseCost(_stakenumber) );   
        ...
        femaleStakes[_stakenumber].nextrelease = block.timestamp + 30 minutes; // change to days
        FemaleBreederCount++;
     ...
     function forwardDevShare( uint256 _amount ) internal {
        if ( _amount == 0 ) return;
        uint256 _share = (_amount *33) / 100;
        
        dev1.transfer(_share);
        dev2.transfer(_share);
        dev3.transfer(_share);
    }
    ...
    function releaseCost( uint256 _stakenumber ) public view  returns(uint256){
        ...
        //uint256 _multiple = _timeleft / 86400; // uncomment when live
        uint256 _multiple = _timeleft / 60; // delete line only for testing
