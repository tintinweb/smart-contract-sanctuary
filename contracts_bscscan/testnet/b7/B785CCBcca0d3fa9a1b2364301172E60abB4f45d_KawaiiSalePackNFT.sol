...
contract KawaiiSalePackNFT is Pausable, SignData {
    ...
    constructor(IERC20 _tokenPayment, IKawaiiRandomness _kawaiiRandomness) public {
    ...
    function createPack(uint256 _numberTree, uint256 _numberAnimal, uint256 _numberField, uint256 _price, uint256 _incrementPrice, uint256 _limit) external onlyOwner {
        require(_numberAnimal < _numberTree, "Animal must < tree");
        packs.push(PackData(_numberTree, _numberAnimal, _numberField, _price, _incrementPrice, _limit, 0));

    }

    function updatePack(uint256 _pack, uint256 _numberTree, uint256 _numberAnimal, uint256 _numberField, uint256 _price, uint256 _incrementPrice, uint256 _limit, uint256 _purchasedNumber) external onlyOwner {
        require(_numberAnimal < _numberTree, "Animal must < tree");
        packs[_pack] = PackData(_numberTree, _numberAnimal, _numberField, _price, _incrementPrice, _limit, _purchasedNumber);
    }
    ...
    function getPrice(uint256 price, uint256 incrementPrice, uint256 amount, uint256 purchasedPackNumber) public view returns (uint256){
    ...
    function buyPack(IERC1155 nftRegister, address sender, uint256 pack, uint256 amount, uint256 price, uint8 v, bytes32 r, bytes32 s) external whenNotPaused {
        ...
        {
            uint256 purchasedPackNumberAfter = packData.purchasedPackNumber.add(amount);
            require(purchasedPackNumberAfter <= packData.limit, "Exceed limit in this sale");
            packs[pack].purchasedPackNumber = packData.purchasedPackNumber.add(amount);
        }

        {
            uint256 amountPay = getPrice(packData.price, packData.incrementPrice, amount, packData.purchasedPackNumber);
