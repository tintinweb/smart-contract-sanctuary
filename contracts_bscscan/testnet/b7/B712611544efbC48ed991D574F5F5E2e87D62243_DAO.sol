...
contract DAO is Ownable {
    ...
    event LogAddResolution(uint256 resolutionId, int executeType, uint256 totalVotesCount, Votes[] votes);
    ...
    /// Resolution
    function addResolution(int _executeType, string memory _resolutionTitle, string memory _resolutionContent, uint256 _voteValidTime, uint256 _executeValidTime) public onlyOwner {
        ...
        resolutionContentList[_resolutionId] = ResolutionContent({
            ...
            voteValidTime: _resolutionId.add(_voteValidTime),
            executeValidTime: _resolutionId.add(_executeValidTime),
            executeHash: "",
            ceateTime: _ceateTime
        });

        emit LogAddResolution(_resolutionId, _executeType, resolutionVotesList[_resolutionId].length, resolutionVotesList[_resolutionId]);
    }
    ...
    function memberVotes(uint256 _resolutionId, int _agree, string memory _comments) public returns (address) {
        ...
        resolutionVotesList[_resolutionId][_memberIndex].voteTime = block.number;
    ...
    function executeResolution(uint256 _resolutionId, string memory _executeHash) public onlyOwner {
        ...
        resolution.executeTime = block.number;
