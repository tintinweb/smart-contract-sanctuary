...
contract GNLRStaking is Ownable {
    ...
    function updatePool() public {
        ...
        pool.accGNLRPerShare = pool.accGNLRPerShare.add(LumiReward.mul(1e12).div(totalStaked));
        pool.lastRewardBlock = block.number;
    }
    ...
    function stake(uint256 _amount) public {
        ...
        updatePool();
        // if (user.amount > 0) { // user has staked already

        //     // uint256 pending = user.amount.mul(pool.accGNLRPerShare).div(1e12).sub(user.rewardDebt);
        //     // if(pending > 0 && user.lock && block.timestamp >= user.end) {
