...
// File: contracts/contracts/interfaces/IToken.sol
...
// File: contracts/contracts/interfaces/IPYESwapCallee.sol
...
// File: contracts/contracts/libraries/UQ112x112.sol
...
// File: contracts/contracts/interfaces/IPYESwapERC20.sol
...
// File: contracts/contracts/PYESwapERC20.sol
...
// File: contracts/contracts/interfaces/IPYESwapPair.sol
...
// File: contracts/contracts/interfaces/IPYESwapFactory.sol
...
// File: contracts/contracts/PYESwapPair.sol
...
contract PYESwapPair is IPYESwapPair, PYESwapERC20 {
    ...
    uint public constant TOTAL_FEE = 1400;
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        if (amount0Out > 0) {
            ...
            if(baseToken == token0) {
                uint fee = _amount0Out.mul(TOTAL_FEE).div(10 ** 4);
                IERC20(_token0).approve(_token1, fee);
                IToken(_token1).depositLPFee(fee, _token0);

                _safeTransfer(_token0, to, _amount0Out.sub(fee));
            } else {
        ...
        if (amount1Out > 0) {
            ...
            if(baseToken == token1) {
                uint fee = _amount1Out.mul(TOTAL_FEE).div(10 ** 4);
                IERC20(_token1).approve(_token0, fee);
                IToken(_token0).depositLPFee(fee, _token1);

                _safeTransfer(_token1, to, _amount1Out.sub(fee));
            } else {
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'PYESwap: K');
        }
...
// File: contracts/contracts/PYESwapFactory.sol
