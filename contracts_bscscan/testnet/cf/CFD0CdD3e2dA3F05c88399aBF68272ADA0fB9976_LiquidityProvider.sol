    ...
    OwnableUpgradeable
{
    ...
    function addExchange(IUniswapV2Router02 _router, IUniswapV2ERC20 _weth) public onlyOwner {
        apis[_apiCount].router = _router;
        apis[_apiCount].factory = IUniswapV2Factory(
            apis[_apiCount].router.factory()
        );
        apis[_apiCount].wethToken = _weth;
        _apiCount++;
    }

    function initialize(IUniswapV2Router02 _router, IUniswapV2ERC20 _weth) public initializer {
        OwnableUpgradeable.__Ownable_init();
        addExchange(_router, _weth);
    }
