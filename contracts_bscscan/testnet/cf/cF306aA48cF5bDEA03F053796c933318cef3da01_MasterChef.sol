...
contract MasterChef is Ownable {
    ...
    event Staked(address indexed user, uint256 indexed fid, uint256 amount);
    event WithdrawnAndHarvest(
    ...
    constructor(
        ...
        xStand _xStand,
        address _devAddr,
        uint256 _standPerBlock //, // uint256 _startBlock
    ) public {
        stand = _stand;
        xSTAND = _xStand;
        devaddr = _devAddr;
    ...
    function harvest(uint256 fid) public {
        ...
        uint256 standReward = accumulateStand.sub(user.rewardDebt);
        uint256 giftReward = user.rewards;
        ...
        emit Harvest(msg.sender, fid, standReward, giftReward);
    }

    function withdrawAndHarvest(uint256 fid, uint256 amount) external {
        FarmInfo memory farm = farmInfo[fid];
        UserInfo storage user = userInfo[fid][msg.sender];
        require(
            user.amount >= amount,
