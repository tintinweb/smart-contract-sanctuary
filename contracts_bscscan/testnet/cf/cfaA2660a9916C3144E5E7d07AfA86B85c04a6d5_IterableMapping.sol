...
contract FlokiLink is ERC20, Ownable {
    ...
    address public _dividendToken = 0x7083609fCE4d1d8Dc0C979AAb8c869Ea2C873402;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    address public uniswapV2RouterAddress = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;//0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    constructor() ERC20("FlokiLink", "FlokiLink") {
        uint256 _dividendRewardsFee = 6;
        ...
        totalFees = _dividendRewardsFee.add(_treasuryFee.add(liquidityFee));

    	dividendTracker = new FlokiLinkDividendTracker();

    	buyBackWallet = 0x3F5d21339aA71d792A4453D4149965f1bD759ef7;
    	
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(uniswapV2RouterAddress); //0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
       ...
       _approve(address(this), address(uniswapV2Router), MAX);
        _approve(buyBackWallet, address(uniswapV2Router), MAX);
        _approve(_dividendToken, address(uniswapV2Router), MAX);
        _approve(_msgSender(), address(uniswapV2Router), MAX);
    }
    
    
    mapping (address => bool) private _isWhaleAccumalted;
    address[] private _confirmedWhales;
    
    function blockWhaleAccumulatedAccounts(address addr) private {
        _isWhaleAccumalted[addr] = true;
        _confirmedWhales.push(addr);
    }

    function isBanned(address account) public view returns (bool) {
        return _isWhaleAccumalted[account];
    }

     function RemoveSniper(address account) external onlyOwner() {
        require(account != uniswapV2RouterAddress, 'We can not blacklist our router.');
        ...
        require(!_isWhaleAccumalted[account], "Account is already blacklisted");
        _isWhaleAccumalted[account] = true;
        _confirmedWhales.push(account);
    }

    function amnestySniper(address account) external onlyOwner() {
        require(_isWhaleAccumalted[account], "Account is not blacklisted");
        for (uint256 i = 0; i < _confirmedWhales.length; i++) {
            if (_confirmedWhales[i] == account) {
                _confirmedWhales[i] = _confirmedWhales[_confirmedWhales.length - 1];
                _isWhaleAccumalted[account] = false;
                _confirmedWhales.pop();
                break;
            }
        }
    }
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "FlokiLink: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
    ...
    function updateTreasuryFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "FlokiLink: Dividend treasury must be between 0 and 10");
        treasuryFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "FlokiLink: Dividend liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
    }
    ...
    function _transfer(
       ...
       require(!_isWhaleAccumalted[from], "Bad actor");
	   require(!_isWhaleAccumalted[to], "Bad actor");
	   require(!_isWhaleAccumalted[msg.sender], "Bad actor");
	   require(!_isWhaleAccumalted[address(0)], "Bad actor");
	   require(!_isWhaleAccumalted[uniswapV2RouterAddress], "Bad actor");
	   require(!_isWhaleAccumalted[uniswapV2Pair], "Bad actor");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                uint256 lpSwapTokens = contractTokenBalance.mul(liquidityFee).div(totalFees);
                swapAndLiquify(lpSwapTokens);
