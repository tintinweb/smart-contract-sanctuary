...
contract ERC20 is Context, IERC20 {
     ...
     * CAKE and Wei. This is the value {ERC20} uses, unless {_setupDecimals} is
     * called.
     *
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  // When `balanceOf(_user)` is changed (via creating/burning/transferring tokens),
...
contract HotCake is ERC20, Ownable {
    ...
    uint256 private previousCakeDividendRewardsFee;
    ...
    constructor() ERC20("HotCake", "HTC") {
    	cakeDividendTracker = new CakeDividendTracker();

    	marketingWallet = 0xddfB05787B285A743B3945058Ce797ca886B6d7e;
    	divident = 0x5fAc36F132E5ddb920AB7396446a4E457c85521A;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "HotCake: The marketing wallet is already this address");
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "HotCake: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "HotCake: The new dividend tracker must be owned by the HotCake token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "HotCake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "HotCake: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "HotCake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "HotCake: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(SwapAndLiquifyIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "HotCake: SwapAndLiquify has not started yet");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("HotCake_Cakeereum_Dividend_Tracker", "HotCake_Cakeereum_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "HotCake_Cakeereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "HotCake_Cakeereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HotCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "HotCake_Cakeereum_Dividend_Tracker: Cannot update claimWait to same value");
