...
contract GABX  
{
   ...
   IBEP20 private busdToken; 
    ...
    function buyToken(uint256 amount,address referrer) public payable
	{
	     ...
	     users[msg.sender].selfBuyUsd=users[msg.sender].selfBuyUsd+buy_amt;
	...
	{
	  uint256 actualToken= _investment/1e18;
	  uint256 refPercent;
	  if(actualToken>=10 && actualToken<=99)
	  {
	    refPercent=5;  
	  }
	  
	  if(actualToken>=100 && actualToken<=499)
	  {
	    refPercent=6;  
	  }
	  
	  if(actualToken>=500 && actualToken<=999)
	  {
	     refPercent=7; 
	  }
	  
	  if(actualToken>=1000 && actualToken<=2499)
	  {
	     refPercent=8; 
	  }
	  
	  if(actualToken>=2500 && actualToken<=4999)
	  {
	      refPercent=9;
	  }
	  if(actualToken>=5000)
	  ...
	  gabxToken.transfer(_referrer , (_investment*refPercent)/100);
	  users[_referrer].refIncome=users[_referrer].refIncome+(_investment*refPercent)/100;
    }
