...
contract ExoticMonsterController is Ownable {
    ...
    //查询用户英雄
    // function myHero(uint256 userHeroId) public returns (uint256 id, uint256 totalTimes, uint256 usedTimes, uint256 lastUsedTime,bool isPooled) {
        
    //     // msg.sender 为客户（调用合约）的地址             
    //     Hero memory userHero = users[msg.sender].myHeros[userHeroId];
        
    //     users[msg.sender].id = userHero.id;
    //     users[msg.sender].totalTimes = userHero.totalTimes;  
    //     users[msg.sender].usedTimes = userHero.usedTimes; 
    //     users[msg.sender].lastUsedTime = userHero.lastUsedTime; 
    //     users[msg.sender].isPooled = userHero.isPooled; 
        
    //     uint256 id = userHero.id;
    //     uint256 totalTimes = userHero.totalTimes;  
    //     uint256 usedTimes = userHero.usedTimes; 
    //     uint256 lastUsedTime = userHero.lastUsedTime; 
    //     bool isPooled = userHero.isPooled; 
        
    //     return (id, totalTimes, usedTimes, lastUsedTime, isPooled);
                
    // }
    
    function myHero(uint256 userHeroId) public view returns (Hero memory) {
        ...
        return users[msg.sender].myHeros[userHeroId];
                
    }
    
    //查询用户投资池
    function myPool(uint256 poolId) public  view returns (Pool memory)  {
        
        // msg.sender 为客户（调用合约）的地址 
        // Pool memory userPool = users[msg.sender].myPools[poolId];
        
        return users[msg.sender].myPools[poolId];
                
    }
    
    // function findUser(address user) public  view returns (User memory)  {
        
    //     // msg.sender 为客户（调用合约）的地址 
    //     // User memory userPool = users[user];
    ...
    function inPool(uint256 poolid,uint256 amount,uint256 userHeroId) public {
        ...
        // MyPool memory newPool = MyPool({
        //         msg.sender,
        //         users[msg.sender].poolIds,
        //         pools[poolid].id,
        //         userHeroId,
        //         amount,
        //         users[msg.sender].myPools[users[msg.sender].poolIds].price,
        //         pools[poolid].circleTime,
        //         users[msg.sender].myPools[users[msg.sender].poolIds].joinTime,
        //         pools[poolid].rate
        // });
    ...
    function outPool(uint256 poolId) public {
        ...
        Pool memory myPool = users[msg.sender].myPools[poolId];
    ...
    function selectMonster(uint256 monsterID,uint256 userHeroId) public {
        ...
        if(userHero.usedTimes < userHero.totalTimes){
            ...
            //更新用户英雄数据
            users[msg.sender].myHeros[userHeroId] = userHero;
