...
contract Shop is IShop, PancakeSwapHelper, EmergencyWithdrawable {
    struct ShopItem {
        bool allowXldPayment;
        bool allowBnbPayment;
        bool bulkAllowed;
        uint256 price; // Price is in BNB
        uint256 id;
        uint256 val1;
        uint256 val2;
        uint112 req1;
    ...
    mapping(uint256 => IShopItemFulfilment) public fullfilmentSystems;
    mapping(uint256 => IDepositable) public fundReceivers;
    ...
    function addItem(uint8 typeId, uint256 val1, uint256 val2, uint112 req1, uint112 req2, uint256 price, uint8 discountRate, bool allowXldPayment, bool allowBnbPayment, bool allowBulk, address fulfilment, address fundReceiver) external override onlyAdmins {
        ShopItem storage item = items[nextId];
        require(item.id == 0, "Shop: Item already exists");
        require(fundReceiver != address(0), "Shop: Cannot deposit funds to zero address");
        
        item.id = nextId;
        item.bulkAllowed = allowBulk;
        item.allowXldPayment = allowXldPayment;
        item.allowBnbPayment = allowBnbPayment;
        item.price = price;
        item.val1 = val1;
        item.val2 = val2;
        item.req1 = req1;
        ...
        fundReceivers[item.id] = IDepositable(fundReceiver);

        if (fulfilment != address(0)) {
            fullfilmentSystems[item.id] = IShopItemFulfilment(fulfilment);
        }

        nextId++;
    }

    function editItem(uint256 id, uint8 typeId, uint256 val1, uint256 val2, uint112 req1, uint112 req2, uint256 price, uint8 discountRate, bool allowXldPayment, bool allowBnbPayment, bool allowBulk) external override onlyAdmins {
        ShopItem storage item = items[id];
        require(id > 0 && item.id == id, "Shop: Item does not exist");
        require(price > 0, "Shop: Invalid price");
    ...
    function itemInfo(uint256 id) external override view returns(uint256, uint256, uint256, uint256, uint256) {
        ShopItem storage item = items[id];
        return (item.typeId, item.val1, item.val2, item.req1, item.req2);
    }

    function setFulfilmentSystem(uint256 id, address fulfilment) external override onlyAdmins {
        ShopItem storage item = items[id];
        require(id > 0 && item.id == id, "Shop: Item does not exist");

        fullfilmentSystems[id] = IShopItemFulfilment(fulfilment);
    }

    function setFundsReceiver(uint256 id, address receiver) external override onlyAdmins {
        ShopItem storage item = items[id];
        require(id > 0 && item.id == id, "Shop: Item does not exist");
        require(receiver != address(0), "Shop: Cannot deposit funds to zero address");

        fundReceivers[id] = IDepositable(receiver);
    }

    function deleteItem(uint256 id) public onlyAdmins {
        delete items[id];
    }

    function setGlobalDiscountRate(uint8 rate) public onlyAdmins {
        require(rate <= 100, "Shop: Invalid value");
        globalDiscountRate = rate;
    }

    function setSlippage(uint8 _slippage) public onlyAdmins {
        require(_slippage <= 1000, "Shop: Invalid value");
        slippage = _slippage;
    }

    function priceOf(uint256 itemId) public view returns (uint256) {
        return priceOf(items[itemId]);
    }

    function xldPriceOf(uint256 itemId) public view returns (uint256) {
        return xldPriceOf(items[itemId]);
    }
    ...
    function buyItem(uint256 id, uint256 quantity, uint256[] calldata params) external payable notUnauthorizedContract nonReentrant notPaused {
        ...
        require(item.allowBnbPayment, "Shop: BNB payment is not allowed");
        require(msg.value == price, "Shop: Incorrect pay amount");
        require(quantity >= 1, "Shop: Invalid quantity");
        require(quantity == 1 || item.bulkAllowed, "Shop: Bulk not allowed");
        require(params.length == quantity, "Shop: Invalid params");

        IDepositable receiver = fundReceivers[id];
        receiver.deposit{value: msg.value}(address(0), msg.value);
        fundsReceivedBNB[address(receiver)] += msg.value;

        IShopItemFulfilment fulfilment = fullfilmentSystems[id];
        if (address(fulfilment) != address(0)) {
            fulfilment.fulfill(id, msg.value, false, msg.sender, quantity, params);
        }
    ...
    function buyItemWithXLD(uint256 id, uint256 quantity, uint256 amount, uint256[] calldata params) external notUnauthorizedContract nonReentrant notPaused {
        ...
        require(item.allowXldPayment, "Shop: XLD payment is not allowed");
        ...
        if (address(fulfilment) != address(0)) {
            xld.transferFrom(msg.sender, address(fulfilment), amount);
            fulfilment.fulfill(id, amount, true, msg.sender, quantity, params);
        }

        emit ItemBought(id, amount, false, msg.sender);
    }

    function priceOf(ShopItem storage item) internal view returns (uint256) {
        uint256 discount = item.price * discountRateOf(item) / 100;
    ...
    function xldPriceOf(ShopItem storage item) internal view returns (uint256) {
        uint256 xldPrice = calculateSwapAmountFromBNBToToken(address(xld), item.price);
        uint256 discount = xldPrice * discountRateOf(item) / 100;
        if (discount > xldPrice) {
            return 0;
        }

        return xldPrice - discount;
    }
