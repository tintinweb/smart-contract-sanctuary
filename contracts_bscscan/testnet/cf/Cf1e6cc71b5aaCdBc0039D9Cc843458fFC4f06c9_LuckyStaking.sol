...
contract LuckyStaking {
    ...
    mapping (address=>int256) lastClaimed;
    ...
    function modifyWaitBlock(int256 newWaitBlocks) public {
        if(msg.sender==owner){
    ...
    function blocksLeftUntilPayout(address user) public view returns (int256){
        int256 blocksLeft=(lastClaimed[user]+waitBlocks)-int256(block.number);
        return blocksLeft;
    }
    ...
    function getDaysPassed (address user) public view returns (int256){
                    int256 daysPassed=((int256(block.number)-int256(lastClaimed[user]))/int256(waitBlocks));
    return daysPassed;
    }

    function getLastClaimed (address user) public view returns (uint256) {
        return uint256(lastClaimed[user]);
    }

    function currentClaimableDividends (address user) public view returns (uint256){
            uint256 daysPassed=((block.number-uint256(lastClaimed[user]))/uint256(waitBlocks));
    ...
    function claimDividends() public{
        if((uint256(lastClaimed[msg.sender])+uint256(waitBlocks))<block.number){
            uint256 daysPassed=((block.number-uint256(lastClaimed[msg.sender]))/divideByDays);
            uint256 reward=(daysPassed*(((CNR.balanceOf(address(this))-totalDeposits)*poolShare[msg.sender])/divideByDays))/10000;
            uint256 rewardPostFees=reward-((reward/100)*(ownerFee+dividendFee));
    ...
    function reinvestDividends() public {
             if((uint256(lastClaimed[msg.sender])+uint256(waitBlocks))<block.number){
            uint256 daysPassed=((block.number-uint256(lastClaimed[msg.sender]))/uint256(waitBlocks));
            ...
            lastClaimed[msg.sender]=int256(block.number);
