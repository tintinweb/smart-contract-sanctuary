...
contract TheFryingPan is Ownable, IERC721Receiver, ReentrancyGuard {
	...
	function delBuildNumber21() internal pure {} //etherscan trick
	...
	/**
	 * Get owed $GREASE from staked tokenId
	 * @param tokenId staked token id
	 * @return owed in $GREASE
	 */
	function getOwedGrease(uint256 tokenId) public view returns (uint256 owed) {
		require(bacon.ownerOf(tokenId) == address(this), "Token is not staked.");

		Stake memory _stake = Pan[tokenId]; // default to pan
