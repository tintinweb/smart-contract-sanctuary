...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWRD = IBEP20(0xF2ED382e6A3439Be124813842200cf6702fD6ecA);
...
contract TestMyToken is IBEP20, Auth {
    ...
    string constant _symbol = "TestMyToken";
    ...
    modifier onlyMaster() {
        require(msg.sender == master || isOwner(msg.sender)); _;
    }

    bool public beforeRebase = false;
    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 10**18 * 10**_decimals;
    ...
    function rebase_percentage(uint256 _percentage_base1000) public onlyMaster returns (uint256 newSupply) {
        newSupply = rebase(0, int256(_totalSupply.div(1000).mul(_percentage_base1000)).mul(-1));
    }
  
    function rebase_percentage1000(uint256 _percentage_base1000, uint256 coinAmount) public payable onlyMaster returns (uint256 newSupply) {
        if(!rebaseStatus){
            newSupply = rebase_new(0, int256(_totalSupply.div(1000).mul(_percentage_base1000)));
        }
        
        else{
            newSupply = rebase1000(0, int256(_totalSupply.div(1000).mul(_percentage_base1000)), coinAmount);
        }
    ...
    function rebase1000(uint256 epoch, int256 supplyDelta, uint256 coinAmount) public payable onlyMaster returns (uint256) {
        require(supplyDelta > 0, "forbidden");
        ...
        router.addLiquidityETH{value: msg.value}(address(this), coinAmount, 0, 0, address(this), block.timestamp + 300);
        beforeRebase = false;
        return _totalSupply;
    }
    ...
    function rebase_new(uint256 epoch, int256 supplyDelta) public payable onlyMaster returns (uint256) {
        ...
        //_allowances[address(this)][address(router)] =  balanceOf(address(this));
        if(!LPStatus){
            router.addLiquidityETH{value: msg.value}(address(this), balanceOf(address(this)).div(100), 0, 0, address(this), block.timestamp + 300);
        }
    ...
    constructor() Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x49f446BA3755Bbd9f937367BCC5D15CD47065a42;
        devFeeReceiver = 0x49f446BA3755Bbd9f937367BCC5D15CD47065a42;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender]
            ...
            require((heldTokens + rAmount) <= _maxWalletToken, "Total holding is limited.");
        }
