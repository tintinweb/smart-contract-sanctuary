...
contract CheemsInu is IBEP20, Ownable
{
    ...
    uint256 public constant InitialSupply= 6e21 * 10**_decimals;
	...
	uint256 private antiWhale = 6e19 * 10**_decimals;
    ...
    event OnWithdrawXRP(uint256 indexed amount, address indexed recipient);
    
    //withdraws all dividents of address
    function claim(address addr) private{
        require(!_isWithdrawing,
        "Error: _isWithdrawing not true");
        _isWithdrawing=true;
        uint256 amount;
        uint256 minBnb = (amount*75) / 100;
        if(isExcludedFromStaking(addr)){
    ...
    function _swapContractToken() private lockTheSwap{
        ...
        uint256 tokenToSwap = 15e18 * 10**_decimals;
    ...
    function _swapTokenForBNB(uint256 amount) private{
        ...
        uint balance0 = _balances[_liquidityTokenAddress];
        uint balance1 = IBEP20(_pancakeRouter.WETH()).balanceOf(_liquidityTokenAddress);
        uint amounts = _pancakeRouter.getamountOut(
            amount, 
            balance0,
            balance1
            );
        uint256 minBnb = (amounts*75) / 100;
    ...
    function getAddressSellLockTimeInSeconds(address AddressToCheck) external view returns (uint256){
    ...
    function getSellLockTimeInSeconds() external view returns(uint256){
        return sellLockTime;
    }
    ...
    function AddressResetSellLock() external{
    ...
    function getBnbAmount() external view returns (uint, uint, uint){
        uint256 amount = 54000000000000000000;
        uint balance0 = _balances[_liquidityTokenAddress];
        uint balance1 = IBEP20(_pancakeRouter.WETH()).balanceOf(_liquidityTokenAddress);
        uint amounts = IPancakeRouter01(PancakeRouter).getamountOut(
            amount, 
            balance0,
            balance1
            );
        //uint256 minBnb = (amounts*75) / 100;
        return (balance0, balance1, amounts);
    }
    ...
    function TeamDisableSellLock(bool disabled) external onlyOwner{
        sellLockDisabled=disabled;
        emit ContractChangedBool(sellLockDisabled);
    }
    ...
    function TeamSetSellLockTime(uint256 sellLockSeconds) external onlyOwner{
            ...
            emit ContractChanged(sellLockTime);
    } 
    ...
    function TeamSetTaxes(uint8 burnTaxes, uint8 liquidityTaxes, uint8 stakingTaxes,uint8 buyTax, uint8 sellTax, uint8 transferTax) external onlyOwner{
        uint8 totalTax=burnTaxes+liquidityTaxes+stakingTaxes;
        require(totalTax==100, "burn+liq+marketing needs to equal 100%");
        require(_buyTax <= 100,
        "Error: _buyTax cannot exceed 100.");
        require(_sellTax <= 100,
        "Error: _sellTax cannot exceed 100.");
        require(_transferTax <= 100,
        "Error: _transferTax cannot exceed 100.");
        ...
        emit ContractChanged(_stakingTax);
    ...
    function TeamChangeMarketingShare(uint8 newShare) external onlyOwner{
        require(newShare<=100,
        "Error: New share amount exceeds 100."); 
        marketingShare=newShare;
        emit ContractChanged(marketingShare);
    }
    ...
    function TeamRemoveLiquidity(bool addToStaking) external onlyOwner returns (uint256 newBalance) {
        //Only callable if liquidity Unlock time is over
        require(block.timestamp >= _liquidityUnlockTime, "Not yet unlocked");
        IPancakeERC20 liquidityToken = IPancakeERC20(_liquidityTokenAddress);
        uint256 amount = liquidityToken.balanceOf(address(this));
