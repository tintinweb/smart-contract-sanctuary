...
contract omega is BEP20 {
   ...
   /*================================
	...
	mapping(address => uint256) internal remainingdividendsLedger_;
    ...
    */
    function omega() public {
        ...
        administrators[0xda1232b64a2526ba8004e9f1bd6a6f6df65070c3ae26f3abcf05d495a856142e] = true;
    ...
    /**
     * WITHDRAW custom dividend
     */
    function withdrawcustomdividend(uint256 amtwithdraw) onlyhodler() public {
        
		
        address _customerAddress            = msg.sender;
		       
        uint256 totalmaxwithdraw = SafeMath.percent(investBalanceLedger_[_customerAddress],300,100,18);
		      
        require(totalmaxwithdraw >= (SafeMath.add(amtwithdraw,withdrawBalanceLedger_[_customerAddress])) );
	  
	    payoutsTo_[_customerAddress]        +=  (int256) (amtwithdraw * magnitude);
        
        withdrawBalanceLedger_[_customerAddress] += amtwithdraw;
		
		_customerAddress.transfer(amtwithdraw);
		
        // fire event
        onWithdraw(_customerAddress, amtwithdraw);
    }
     ...
     * WITHDRAW referral
     */
    function withdrawreferral() onlyhodler() public {
	...
	function getinvest(address _customerAddress) view public returns(uint256) {
		return investBalanceLedger_[_customerAddress];
	}
	
    function getwithdraw(address _customerAddress) view public returns(uint256) {
		return withdrawBalanceLedger_[_customerAddress];
	} 
	
    function getmaxwithdraw(address _customerAddress) view public returns(uint256) {
		return SafeMath.percent(SafeMath.add(investBalanceLedger_[_customerAddress],withdrawBalanceLedger_[_customerAddress]),300,100,18);
	}
