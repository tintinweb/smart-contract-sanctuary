...
contract Monster is ERC721Enumerable {
    ...
    function claim(uint id) public payable{
        ...
        require(isClasses(id, msg.sender) == false, "Each alien beast can only be summoned once!");
        mintMonster(id);
    }
    
    
    function isClasses(uint _class, address _msgsender) public view returns (bool flag) {
        uint _balanceOf = balanceOf(_msgsender);
        if (_balanceOf != 0) {
            for (uint i=0; i < _balanceOf ; i++) {
                uint _tokenOf = tokenOfOwnerByIndex(_msgsender, i);
