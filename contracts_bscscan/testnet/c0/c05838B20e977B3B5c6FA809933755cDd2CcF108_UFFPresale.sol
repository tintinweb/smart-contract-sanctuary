...
/**
 * @title UFFPresale 
 * UFFPresale allows investors to make
 * token purchases and assigns them tokens based
 * on a token per BNB rate. Funds collected are forwarded to a wallet 
 * as they arrive.
 */
 
contract UFFPresale is Pausable, Ownable{
  using SafeMath for uint256;

  // Burn address
  address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;

  // The Holding Cap Percent
  uint holdingCapPercent = 5;

  // The token being sold
  UFFToken public token;
  ...
  // softcap - above this presale can end earlier
   ...
   * @param _wallet who receives invested bnb
   * @param _minInvestment is the minimum amount of bnb that can be sent to the contract
   * @param _cap above which the crowdsale is closed
   * @param _rate is the amounts of tokens given for 1 bnb
   */ 

  constructor (address payable _wallet, uint256 _minInvestment, uint256 _cap, uint256 _rate, address payable _uffAddress, address _senderFrom) public {
    require(_wallet != address(0));
    require(_minInvestment >= 0);
    require(_cap > 0);

    token = UFFToken(_uffAddress);
    wallet = _wallet;
    rate = _rate;
    minInvestment = _minInvestment;  //minimum investment in wei  (=0.1 bnb)
    cap = _cap * (10**18);  //cap in tokens base units (=150,000 tokens)
    softcap = 1 * (10**18);
    senderFrom = _senderFrom;
  }

  // creates presale token
  // function createTokenContract() internal returns (UFFToken) {
  //   return new UFFToken();
  // }
  ...
  function buyTokens(address beneficiary) public payable whenNotPaused {
    ...
    token.transferFrom(senderFrom, beneficiary, tokens);
    TokenPurchase(msg.sender, beneficiary, weiAmount, tokens);
    forwardFunds();
  }

  // send bnb to the fund collection wallet
  ...
  function validPurchase() internal view returns (bool) {
    require(token.balanceOf(msg.sender) + (msg.value * rate) <= _getMaxHolding(), "Max holding cap breached.");
    ...
    bool withinMaxPurchase = msg.value <= (5 * (10**18));

    return (notSmallAmount && withinCap && withinMaxPurchase);
  }

  // allow owner to finalize the presale once the presale is ended
  function finalize(address newOwner) public onlyOwner {
    require(!isFinalized);
    require(hasEnded());

    // token.finishMinting();
    uint leftOverBalance = token.allowance(senderFrom, address(this));
    token.transferFrom(senderFrom, BURN_ADDRESS, leftOverBalance);
  ...
  function setContactInformation(string calldata info) public onlyOwner {
    string memory contactInformation = info;
  }

  //return true if crowdsale event has ended
  function hasEnded() public view returns (bool) {
    bool capReached = (weiRaised.mul(rate) >= softcap);
    return capReached;
  }

  function _getMaxHolding() view internal returns (uint256) {
    return (cap * holdingCapPercent) / 100;
  }
}
