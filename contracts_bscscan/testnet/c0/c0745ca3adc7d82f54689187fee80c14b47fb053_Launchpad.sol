...
contract Launchpad {
    ...
    function buyToken() public payable returns (bool) {
        ...
        if(buyPrice == 11){
            ...
            buyer[msg.sender].count += 5;
            buyer[msg.sender].claimStatus.push(true);
            buyer[msg.sender].claimStatus.push(false);
            buyer[msg.sender].claimStatus.push(false);
            buyer[msg.sender].claimStatus.push(false);
            buyer[msg.sender].claimStatus.push(false);
            buyer[msg.sender].claimStatus.push(false);
    ...
    function showBool() public view returns (uint[] memory, uint[] memory, bool[] memory, uint[] memory) {
        address user = msg.sender;
        bool[] memory claims = new bool[](buyer[user].bought.length);
        uint[] memory tokens = new uint[](buyer[user].count);
        uint[] memory dates = new uint[](buyer[user].count);
        uint[] memory price = new uint[](buyer[user].buyPrice.length);
        
        for(uint j = 0; j < buyer[user].bought.length; j++){
            claims[j] = buyer[user].claimStatus[j];
            price[j] = buyer[user].buyPrice[j];
        }
        ...
        return(tokens, dates, claims, price);
    }
