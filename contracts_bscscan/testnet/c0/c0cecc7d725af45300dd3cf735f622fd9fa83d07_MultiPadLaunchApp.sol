...
abstract contract IMainToken {
    function balanceOf(address) public virtual pure returns (uint256);
    function allowance(address, address) public virtual pure returns (uint256);
    function transfer(address, uint256) public virtual pure returns (bool);
    function transferFrom(address, address, uint256) public virtual pure returns (bool);
    function approve(address , uint256) public virtual pure returns (bool);
 }
...
contract MultiPadLaunchApp is IBEP20 {
    ...
    */
    ...
    function claimPurchasedTokens(uint256 _lockNumber) external validateClaim(msg.sender,_lockNumber) returns (bool){
        _transfer(msg.sender,address(this),_finalSoldAmount[msg.sender].mul(20).div(100));
        iMainToken.transferFrom(IDOAddress, msg.sender,_finalSoldAmount[msg.sender].mul(20).div(100));
        _recordSale[msg.sender] = _recordSale[msg.sender].sub(_finalSoldAmount[msg.sender].mul(20).div(100));
        reEntrance[msg.sender][_lockNumber] = true;
        _claimedByUser[msg.sender][_lockNumber] = _finalSoldAmount[msg.sender].mul(20).div(100);
    }
    ...
    modifier validateClaim(address _userAddress, uint256 _lockNumber)
    {
        ...
        require(_lockNumber <= 1 && _lockNumber > 0);
     ...
     */
    function getUserTokensByAdd(address _userAddress) external view returns(uint256 _div1){
        uint256 division = _finalSoldAmount[_userAddress].div(20)*100;
        _div1 = division;
        return(_div1);
    }
    ...
    function getClaimDates() view external returns(uint256){
        return(saleEndTime.add(5400));
    }
    
    /*
     * @dev Get claimed tokens by user address
     */
    function getClaimedTokensHistory(address _userAddress,uint256 n) view external returns(uint256){
        return(_claimedByUser[_userAddress][n]);
    }
