...
contract Marketplace is Ownable , NftLegendCard{
    using SafeMath for uint;
    using SafeBEP20 for IBEP20;
    IBEP20 public tokennfl;
	NftLegendCard public tokenerc721;
    uint256 feeListMarket = 20000000;
    ...
    mapping(address => MarketInfo[]) public findMarketByAddress;
    // mapping(uint256 => MarketInfo) public findMarketById;
    ...
    function getMarketPlaceInfo() public view returns (MarketInfo[]  memory) {
        return marketInfo;
    }
    function getMarketPlaceInfoBuyAddress(address _address) public view returns (MarketInfo[]  memory) {
        return findMarketByAddress[_address];
    }
    function listMarket(uint256 _tokenId, uint256 _price , bool _ads) public {
        require(_price > 0, 'need price > 0');
        uint256 lengthMarket = marketInfo.length;
        tokenerc721.transferFrom(msg.sender, address(this), _tokenId);
        marketInfo.push(MarketInfo(lengthMarket, msg.sender, _tokenId, _price, _ads, 0));
        findMarketByAddress[msg.sender].push(MarketInfo(lengthMarket, msg.sender, _tokenId, _price, _ads, 0));
    }

    function cancelMarket(uint256 _id) public {
        require(marketInfo[_id].owner == msg.sender && marketInfo[_id].status == 0 , 'You cant cancel market');
        marketInfo[_id].status = 1;
        tokenerc721.transferFrom(address(this) , msg.sender, marketInfo[_id].tokenId);
    }

    function buyTokenIdMarket(uint256 _id) public {
        require(marketInfo[_id].status == 0, 'TokenId soldout');
        tokennfl.safeTransferFrom(msg.sender, marketInfo[_id].owner , marketInfo[_id].price);
        marketInfo[_id].status = 2;
        tokenerc721.transferFrom(address(this) , msg.sender, marketInfo[_id].tokenId);
    }   
