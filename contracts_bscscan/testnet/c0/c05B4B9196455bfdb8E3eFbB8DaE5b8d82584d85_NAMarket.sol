  ...
  */
  ...
  function payout(
      ...
      (bool success, ) = payable(address(this)).call{
        value: bidPrice.add(calculateFee(bidPrice))}("");

        totalMarketVolume.add(bidPrice);
       
        if (!success) {
          emit BidFailed(auctionId, bidPrice);
          require(false, "Payable Bid Failed");
        } else {
  ...
  (Auction[] memory, uint256 nextOffset, uint256 totalCount) {
        ...
        //sort
        Auction[] memory sortValues = new Auction[] (resultLen);
        for (uint256 i = 0; i < resultLen; i++) {
          if (Sort.NEW == sort)  {
            sortValues = sortMap(result, resultLen, Sort.NEW);
          } else if (Sort.END == sort)  {
            sortValues = sortMap(result, resultLen, Sort.END);
          } else if (Sort.ASC == sort)  {
            sortValues = sortMap(result, resultLen, Sort.ASC);
          } else {
            sortValues = sortMap(result, resultLen, Sort.DESC);
          }
        }
        ...
        if (limit > resultLen - offset) {
            limit = resultLen - offset;
        }
       
        Auction[] memory newAuctions = new Auction[] (sortValues.length > limit ? limit: sortValues.length);

        if (sortValues.length > limit) {
          for (uint256 i = 0; i < limit; i++) {
            newAuctions[i] = sortValues[offset+i];
          }
          return (newAuctions, offset + limit, resultLen);
        } else {
          return (sortValues, offset + limit, resultLen);
        }
