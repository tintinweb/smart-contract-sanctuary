...
interface IBisharesUniswapV2Oracle {
    ...
    function getPriceObservationInWindow(address token, uint256 priceKey)
        external
        view
        returns (PriceLibrary.PriceObservation memory);
    ...
    function canUpdatePrices(address[] calldata tokens)
        external
        view
        returns (bool[] memory);
...
interface IDelegateCallProxyManager {
     ...
     */
    function deployProxyOneToOne(bytes32 suppliedSalt, address implementation)
        external
     ...
     */
    function isImplementationLocked(bytes32 implementationID)
        external
        view
        returns (bool);
     ...
     */
    function isImplementationLocked(address proxyAddress)
        external
        view
        returns (bool);
     ...
     */
    function getImplementationHolder(bytes32 implementationID)
        external
        view
        returns (address);
...
contract DelegateCallProxyManyToOne is Proxy {
    ...
    constructor() public {
        ...
        _implementationHolder = ProxyDeployer(msg.sender)
...
library SaltyLib {
    ...
    function deriveManyToOneSalt(
        ...
        return
            keccak256(
                abi.encodePacked(originator, implementationID, suppliedSalt)
            );
    }
     ...
     * @param suppliedSalt Salt provided by the account requesting deployment.
     */
    function deriveOneToOneSalt(address originator, bytes32 suppliedSalt)
        internal
        pure
        returns (bytes32)
    {
    ...
    function computeProxyAddressOneToOne(
        ...
        return
            Create2.computeAddress(
                salt,
                CodeHashes.ONE_TO_ONE_CODEHASH,
                deployer
            );
    }
    ...
    function computeProxyAddressManyToOne(
        ...
        return
            Create2.computeAddress(
                salt,
                CodeHashes.MANY_TO_ONE_CODEHASH,
                deployer
            );
    }
...
interface IIndexPool {
    ...
    function setExitFeeRecipient(address exitFeeRecipient, bool additional)
        external;
...
library MCapSqrtLibrary {
    ...
    function computeMarketCapSqrts(
        ...
        for (uint256 i = 0; i < len; i++) {
            uint256 marketCap = computeAverageMarketCap(
                tokens[i],
                averagePrices[i]
            );
...
contract OwnableProxy is Context {
    ...
    function _initializeOwnership() internal {
        require(
            _owner == address(0),
            "Ownable: owner has already been initialized"
        );
...
contract MarketCapSortedTokenCategories is OwnableProxy {
     ...
     */
    function addToken(uint256 categoryID, address token)
        external
        onlyOwner
        validCategory(categoryID)
    {
    ...
    {
        require(
            _categoryTokens[categoryID].length + tokens.length <=
                MAX_CATEGORY_TOKENS,
            "ERR_MAX_CATEGORY_TOKENS"
        );
     ...
     */
    function removeToken(uint256 categoryID, address token)
        external
        onlyOwner
        validCategory(categoryID)
    {
     ...
     */
    function orderCategoryTokensByMarketCap(uint256 categoryID)
        external
        validCategory(categoryID)
    {
        ...
        uint144[] memory marketCaps = computeAverageMarketCaps(
            categoryTokens,
            oracle
        );
        for (uint256 i = 1; i < len; i++) {
            ...
            uint256 j = i - 1;
            while (int256(j) >= 0 && marketCaps[j] < cap) {
...
contract MarketCapSqrtController is MarketCapSortedTokenCategories {
    ...
    bytes32 internal constant INITIALIZER_IMPLEMENTATION_ID = keccak256(
        "PoolInitializer.sol"
    );

    // Identifier for the unbound token seller implementation on the proxy manager.
    bytes32 internal constant SELLER_IMPLEMENTATION_ID = keccak256(
    ...
    ) public MarketCapSortedTokenCategories() {
        _factory = factory;
        ...
        returns (address poolAddress, address initializerAddress)
    {
        ...
        emit NewPoolInitializer(poolAddress, initializerAddress);
    }
    ...
    function finishPreparedIndexPool(
        ...
        uint256 valueSum;
        uint144[] memory ethValues = IBisharesUniswapV2Oracle(oracleAddress)
     ...
     */
    function updateSellerPremium(address tokenSeller, uint8 premiumPercent)
        external
        onlyOwner
    {
     ...
     */
    function setMaxPoolTokens(address poolAddress, uint256 maxPoolTokens)
        external
        onlyOwner
        _havePool(poolAddress)
    {
    ...
    function setExitFeeReciver(
        ...
        IIndexPool(poolAddress).setExitFeeRecipient(
            exitFeeRecipient,
            additional
        );
    }
        ...
        _havePool(address(pool))
    {
        ...
        PriceLibrary.TwoWayAveragePrice memory price = IBisharesUniswapV2Oracle(
            oracleAddress
        )
        ...
        uint256 minimumBalance = price.computeAverageTokensForEth(poolValue) /
            100;
    ...
    function delegateCompLikeTokenFromPool(
        ...
        address token,
        address delegatee
    ) external onlyOwner _havePool(pool) {
    ...
    function reindexPool(address poolAddress) external {
        ...
        address oracleAddress = IIndexPool(poolAddress).oracle();
         ...
         = IBisharesUniswapV2Oracle(oracleAddress).computeTwoWayAveragePrices(
            tokens,
            SHORT_TWAP_MIN_TIME_ELAPSED,
            SHORT_TWAP_MAX_TIME_ELAPSED
        );
        FixedPoint.uq112x112[] memory weights = MCapSqrtLibrary
            .computeTokenWeights(tokens, prices);
        ...
        for (uint256 i = 0; i < size; i++) {
            ...
            minimumBalances[i] =
                prices[i].computeAverageTokensForEth(totalValue) /
                100;
    ...
    function reweighPool(address poolAddress) external {
        ...
        address[] memory tokens = IIndexPool(poolAddress)
    ...
    function getInitialTokensAndBalances(
        ...
        tokens = getTopCategoryTokens(categoryID, indexSize);
            ...
            PriceLibrary.TwoWayAveragePrice[] memory prices
         = _categoryOracles[categoryID].computeTwoWayAveragePrices(
            tokens,
            SHORT_TWAP_MIN_TIME_ELAPSED,
            SHORT_TWAP_MAX_TIME_ELAPSED
        );
     ...
     */
    function _estimatePoolValue(IIndexPool pool)
        internal
        view
        returns (uint144)
    {
        ...
        return
            IBisharesUniswapV2Oracle(oracleAddress).computeAverageEthForTokens(
                token,
                value,
                SHORT_TWAP_MIN_TIME_ELAPSED,
                SHORT_TWAP_MAX_TIME_ELAPSED
