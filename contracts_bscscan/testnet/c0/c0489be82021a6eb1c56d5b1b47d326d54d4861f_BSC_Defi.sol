...
contract BSC_Defi {
    ...
    // will auto execute, when condition is met. buy, hatch and sell, can be triggered manually buy admin no user action.
    function chooseWinner() public {
       ...
       for(uint256 i = 0; i < participants; i++){
           if((random >= init_range[i]) && (random <= end_range[i])){
               ...
               user.totalLotteryBonus = user.totalLotteryBonus.add(eggValue);
               totalLotteryBonus = totalLotteryBonus.add(eggValue);
               uint256 proj = eggValue.mul(LOTTERY).div(PERCENTS_DIVIDER);
               project.transfer(proj);

               //record round
               lotteryHistory.push(LotteryHistory(lotteryRound, winnerAddress, eggValue, participants, totalTickets));
    ...
    function getDailyCompoundBonus(address _adr, uint256 amount) public view returns(uint256){
        if(users[_adr].dailyCompoundBonus == 0) {
            ...
            uint256 totalBonus = users[_adr].dailyCompoundBonus.mul(COMPOUND_BONUS); // add compound bonus percentage
    ...
    function getEggsSinceLastHatch(address adr) public view returns(uint256){
        ...
        uint256 cutoffTime = min(secondsSinceLastHatch, CUTOFF_STEP); //GET SMALLEST TIME.
    ...
    /** setup **/
    function SET_PRE_START(address _addr) external {
    ...
    **/
    ...
    function SET_MAX_LOTTERY_PARTICIPANTS(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value >= 10 && value <= 100); /** min 10, max 100 **/
