...
contract FirstGovernanceTokenSale {
    ...
    function _getPrice(uint256 amount) private view returns(uint256) {
        return amount.mul(tokenSaleCost);
    }
    ...
    function getAmountInWei(uint256 amount) external view returns(uint256) {
        uint256 amountInWei = amount.mul(1e8);
        return amountInWei;
    }
    ...
    function buyToken(uint256 amount) external payable {
        ...
        require(availableToSaleAmount > amountInWei, "sale: not enough tokens to sell");

        uint256 totalCost = _getPrice(amountInWei);
        require(msg.value >= totalCost, "sale: value sent is less than the total cost tokens");

        token.safeTransfer(msg.sender, amountInWei);
