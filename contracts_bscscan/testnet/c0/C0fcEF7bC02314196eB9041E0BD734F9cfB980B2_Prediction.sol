...
contract Prediction is IPrediction, Ownable, Pausable {
    ...
    function _nextTicket(address _buyer, uint256 _pid, bool _label, uint256 _quantity) internal view returns(uint256 price, uint256 fee, uint256 newPool, uint256 reduceRate) {
        ...
        if (_label) {
            (uint256 previousPrice, ) = _ticketPriceAtIndex(_pid, specification.blueTicketNumber);
            uint256 improvementDeltaValue = previousPrice * improvementDelta / oneRatio;
            newPool = 
            ...
            (uint256 previousPrice, ) = _ticketPriceAtIndex(_pid, specification.redTicketNumber);
            uint256 improvementDeltaValue = previousPrice * improvementDelta / oneRatio;
            newPool = 
    ...
    function _ticketPriceAtIndex(uint256 _pid, uint256 _index) public view returns(uint256 price, uint256 fee) {
        require(_index > 1);
        Pool memory pool = pools[_pid];
    
        uint256 improvementDeltaValue = pool.ticketPriceStart * improvementDelta / oneRatio;
        uint256 oldPool = pool.ticketPriceStart * pool.ticketNumberInit;

        uint256 newPool = 
            (oldPool / pool.ticketNumberInit + improvementDeltaValue * _index) * pool.ticketNumberInit + 
                pool.ticketPriceStart * _index +
                improvementDeltaValue * (_index - 1) * _index / 2;
        price = newPool - oldPool;
        fee = pool.ticketFeeStart * price / pool.ticketPriceStart;
    }
