...
contract ClockAuctionBase {
        ...
        returns (uint256)
    {
        ...
        if (price > 0) {
            ...
            require(IERC20(CPCToken).transfer(seller, sellerProceeds));
        }

        uint256 bidExcess = _bidAmount - price;

        require(IERC20(CPCToken).transfer(msg.sender, bidExcess));
...
contract ClockAuction is Pausable, ClockAuctionBase, IClockAuction {
    ...
    function bid(uint256 _tokenId, uint256 _amount)
        ...
        whenNotPaused
    {
        // require(IERC20(CPCToken).transfer(address(this), _amount));
...
contract CPCRewards is ClockAuction {
    ...
    function bid(uint256 _tokenId, uint256 _amount)
        override external
    {
        require(IERC20(CPCToken).transferFrom(msg.sender, address(this), _amount));
