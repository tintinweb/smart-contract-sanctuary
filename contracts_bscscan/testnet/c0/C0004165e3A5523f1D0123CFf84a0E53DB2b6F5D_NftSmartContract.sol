...
contract NftSmartContract is ERC721URIStorage, Ownable {
  ...
  modifier isWhiteListedAddress(){
    require(whiteListed[msg.sender],"Users not whitelisted");
    _;
  }     
  ...
  function setSale(bool shouldOpen) external onlyOwner {
    sale = shouldOpen;
  }

  function addWhitelistAddress (address user) public onlyOwner {
    require(!whiteListed[user], "Not authorized to use this function");
    whiteListed[user] = true;
  }

  function claimFreeMint (address user, string memory tokenURI) public isWhiteListedAddress returns (uint256) {
    _tokenIds.increment();
