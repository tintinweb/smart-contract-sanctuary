...
interface IMintableToken is IERC20 {
  ...
  function transferBonus(address _receiver, uint256 _amount) external;
...
//------------------------------------------END---------------------------------------------------------
    ...
    uint256 private _tTotal = 50 * 10**9 * 10**9; // 50,000,000,000,000000000 50 billion max total supply
    ...
    function transferBonus(address _receiver, uint256 _amount) external override onlyMinter {
    ...
    constructor () public {
        ...
        // transfer all token to tokenContract
        ...
        //exclude wallet/ contract from fee
    ...
    function initializeFund() external onlyOwner returns(bool){
        
        // this all Transfer are internal so no one call from outside with address(this) contract
        //==> IDO Transfer
        _transfer(address(this),_msgSender(), idoAllotToken);
        // ===> LP Transfer
        _transfer(address(this),_lpWallet,_lpAllot);
        // ===> DEV/MARKETING Transfer
        _transfer(address(this),_devMarketingWallet,_devMarketingAllot);
        // ===> TEAM Transfer
        _transfer(address(this),_teamWallet,_teamAllot);
        // ===> CHARITY Transfer
        _transfer(address(this),_charityWallet,_charityAllot);
