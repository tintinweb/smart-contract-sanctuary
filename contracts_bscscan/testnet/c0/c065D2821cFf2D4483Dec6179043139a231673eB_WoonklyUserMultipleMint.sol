...
contract Woo1155 is Ownable{
    ...
    address public _exchange;
     ...
     constructor(string memory _name, string memory _symbol, address exchange) public{
        name = _name;
        symbol = _symbol;
        _exchange = exchange;
    }
    ...
    function safeTransferFrom(address _from, address _to, uint256 _id,uint256  _value) public {
        require(msg.sender == _exchange, "You are Not Allowed This Process");
        require(_to != address(0x0), "_to must be non-zero.");
        require(_from == msg.sender || operatorApproval[_creator[_id]][_exchange] == true, "Need operator approval for 3rd party transfers.");
    ...
    */
    ...
    function burn(address from, uint256 _id, uint256 _value) public{
        ...
        require(operatorApproval[_creator[_id]][_exchange] == true, "Need operator approval for 3rd party burns.");
        balances[_id][from] = balances[_id][from].sub(_value);
        if(totalQuantity[_id] == _value){
             operatorApproval[_creator[_id]][_exchange] == false;
             delete _creator[_id];
        }
...
contract WoonklyUserMultipleMint is Woo1155{
    string public name;
    string public symbol;

    mapping (uint256 => string) private _tokenURIs;

    constructor(string memory _name, string memory _symbol, address exchange) Woo1155(name, symbol, exchange) public{
        name = _name;
        symbol = _symbol;
    }
}