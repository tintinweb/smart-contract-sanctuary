...
// 
contract BoxMarketplace is Ownable, Pausable, ERC1155Holder, Initializable {
    ...
    function buy(uint256 _saleId, uint256 _quantity) external payable whenNotPaused {
        ...
        require(info.quantity.sub(info.numSold) >= _quantity, 'Not Enough Box To Buy');
    ...
    function updateSaleQuantity(uint256 _saleId, uint256 _quantity) external whenNotPaused returns (bool) {
        ...
        require(info.seller == _msgSender(), 'Not the owner of this item');
        require(info.status == 1, 'Sale Is Unavailable');
        require(_quantity >= info.numSold, 'Invalid Quantity');
    ...
    function cancelListed(uint256 _saleId) external whenNotPaused returns (bool) {
        ...
        if (info.quantity > info.numSold) {
            require(mysteryBoxNFT.balanceOf(address(this), info.mysteryBoxId) >= info.quantity.sub(info.numSold), 'Out Of Box For Cancel');
            mysteryBoxNFT.safeTransferFrom(address(this), _msgSender(), info.mysteryBoxId, info.quantity.sub(info.numSold),
                abi.encodePacked(keccak256("onERC1155Received(address,address,uint256,uint256,bytes)")));
        }
    ...
    function adminCancelList(uint256 _saleId) external onlyOwner {
        ...
        if (info.quantity > info.numSold) {
            require(mysteryBoxNFT.balanceOf(address(this), info.mysteryBoxId) >= info.quantity.sub(info.numSold), 'Out Of Box For Cancel');
            mysteryBoxNFT.safeTransferFrom(address(this), info.seller, info.mysteryBoxId, info.quantity.sub(info.numSold),
                abi.encodePacked(keccak256("onERC1155Received(address,address,uint256,uint256,bytes)")));
        }
