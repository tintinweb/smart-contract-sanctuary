...
/**
   ...
   0.3% fee auto distribute to _adminFee
   0.5% fee auto Burn
...
// ----------------------------------------IDO GLOBAL DATA STORAGE ------------------------------------------
  ...
  mapping (address => bool) private _isIdoParticipant;
  
      // Public variables for IDO

   	uint256 public exchangeRate = 10000000;         // exchange rate  1 BNB = 10000000 tokens 
   	uint256 public idoBNBReceived;                  // how many BNB Received through IDO
   	uint256 public totalTokenSold;                  // how many tokens sold
	uint256 public minimumContribution = 10**16;    // Minimum amount to invest - 0.01 BNB (in 9 decimal format)
    uint256 public maximumContribution = 50**17;    // Maximum amount to inveest - 0.5 BNB
    uint256 public maximumTokenHold = 15*10**6**9;   // Maximum amount a wallet can hold 15 million
    bool    public idoClosed;
    uint256 public idoAllotToken      = 15 * 10**9 * 10**9; // 15 billion for IDO
...
//------------------------------------------END---------------------------------------------------------
    ...
    uint256 private _mTotal = 50 * 10**9 * 10**9; // 50,000,000,000,000000000 50 billion max total supply
    uint256 private _tTotal ; // 50,000,000,000,000000000 50 billion total supply
    uint256 private _rTotal = (MAX - (MAX % _mTotal));
    ...
    uint256 public  _bountyBonusAllot = 75 * 10**8 * 10**9;   //  7.5 billion
    uint256 public  _totalBountyDistributed;                 // no of distribute token
    uint256 public  _lpAllot         = 10 * 10**9 * 10**9;  //  10 billion
    uint256 public  _devMarketingAllot = 75 * 10**8 * 10**9; //  7.5 billion
    uint256 public  _teamAllot            = 25 * 10**8 * 10**9; //  2.5 billion
    uint256 public  _charityAllot         = 25 * 10**8 * 10**9; //  2.5 billion
    uint256 public  _lockedTokenAllot     = 5 *  10**9 * 10**9;   // 5 billion
    ...
    uint256 public _burnFee= 500;  //0.5 
    uint256 public _adminFee=300;  //0.3
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        require(_tTotal.add(amount)<=_mTotal,"Can't mint more token");
    ...
    function mint(address _receiver, uint256 _amount) external override onlyMinter {
        require(_totalBountyDistributed<=_bountyBonusAllot);
    ...
    constructor () public {
        ...
        //locked _lockedPeriod
        
        _lockedPeriod= block.timestamp+157784760; // 5 years
        
        // mint all allocated token here 
        
        // ==> IDO MINTING
        _mint(_msgSender(), idoAllotToken);
        // ===> LP MINTING
        _mint(_lpWallet,_lpAllot);
        // ===> DEV/MARKETING MINTING
        _mint(_devMarketingWallet,_devMarketingAllot);
        // ===> TEAM MINTING
        _mint(_teamWallet,_teamAllot);
        // ===> CHARITY MINTING
        _mint(_charityWallet,_charityAllot);
        
        emit Transfer(address(0), _msgSender(), _tTotal);
    }
    
    function claimLockedToken() public onlyOwner returns(bool){
    ...
    function _burn(uint256 _value) internal returns (bool success) {
        ...
        _transfer(msg.sender,deadAddress,_value);  // Subtract from the sender
        _tTotal = _tTotal.sub(_value);           // Updates totalSupply
    ...
    function deliver(uint256 tAmount) public {
        ...
        require(!_isExcluded[sender] && _isIdoParticipant[sender]==true, "Excluded addresses cannot call this function");
    ...
    function excludeFromReward(address account) public onlyOwner() {
        ...
        require(!_isExcluded[account] && _isIdoParticipant[account]==true, "Account is already excluded");
    ...
    function _transfer(
        ...
        if( _isExcludedFromFee[from] || _isExcludedFromFee[to] || !_isIdoParticipant[from] || !_isIdoParticipant[to] ){
        ...
        _tokenTransfer(from,owner(),adminFee,takeFee);
        amount-=adminFee;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient] && !_isIdoParticipant[sender] && _isIdoParticipant[recipient]) {
            _transferFromExcluded(sender, recipient, amount);
        } else if (!_isExcluded[sender] && _isExcluded[recipient] && _isIdoParticipant[sender] && !_isIdoParticipant[recipient]) {
    ...
    function buyToken() payable public returns(uint)
    {
        ...
        require(balanceOf(msg.sender)<=maximumTokenHold,"wallet reach maximum holdings");
        ...
        _mint(msg.sender,tokenTotal);
        
        if(_isIdoParticipant[msg.sender]==false){
            
            _isIdoParticipant[msg.sender]=true;
        }
