...
//
contract MarsHome is ERC20, Ownable {
    ...
    address payable public  marketingWallet = payable(0xFC1fb7d1462Abad0Cf09df7DfBb6a7D4fb4a0E45);
    address payable public  developmentWallet = payable(0x911C4089AFD53a1EA2B4B87fD4d799019aDF2D7F);
    address payable public  gamePoolAddress = payable(0x9C234d22D813FbCe0508A3A94cB8D5Dc0f08184F);


    uint256 public swapTokensAtAmount = 50000000000 * (10 ** 9);
    uint256 public swapTokensAtAmountMax = 50000000000 * (10 ** 9);
    ...
    bool public sendToGamePool = false;
    ...
    constructor()  ERC20('MarsHome', 'MarsHome') {
...
//        uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function setBuyDevelopmentFee(uint256 value) external onlyOwner {
        buyDevelopmentFee = value;
    }

    function setSellDevelopmentFee(uint256 value) external onlyOwner {
        sellDevelopmentFee = value;
    }
    ...
    function setSwapTokensAmountMax(uint256 _amount) public onlyOwner {
        require(_amount > swapTokensAtAmount, "Max amount must be greater than minimum");
    ...
    function updateDevelopmentWallet(address payable newDevelopmentWallet) public onlyOwner {
    ...
    function _transfer(
        ...
        if (canSwap && !swapping && !automatedMarketMakerPairs[from] && SwapAndLiquidityEnabled) {
            swapping = true;
            // Set number of tokens to sell to swapTokensAtAmount
            contractTokenBalance = swapTokensAtAmount;
            uint256 totalFees = sellDevelopmentFee.add(sellMarketingFee).add(liquidityFee);

            uint256 marketingTokens = contractTokenBalance.mul(sellMarketingFee).div(totalFees);
            swapTokensForEth(marketingTokens, marketingWallet);

            uint256 countLiquidityFees = contractTokenBalance.mul(liquidityFee).div(totalFees);
            swapAndLiquify(countLiquidityFees);


            uint256 developmentTokens = contractTokenBalance.sub(marketingTokens).sub(countLiquidityFees);
        ...
        if (block.number - blockNumber <= 10) {
            ...
            require(afterBalance <= 250000000000 * (10 ** 9), 'Owned amount exceeds the maxOwnedAmount');
        }
        ...
        if (takeFee) {
            ...
            if (block.number - blockNumber <= 5) {
                ...
                if (from == uniswapV2Pair && to != address(uniswapV2Router)) {
                    uint256 totalFees = buyDevelopmentFee.add(buyMarketingFee).add(liquidityFee);
