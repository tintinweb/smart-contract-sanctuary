...
contract HowlsCastle is Ownable, ReentrancyGuard {
    ...
    function pedingCalciferBoosted(uint256 _pid, address _user) public view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accCALCIFERPerShare = pool.accCALCIFERPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
    ...
    function deposit(uint256 _pid, uint256 _amount, bool _boost, string memory _callSource, address _referrer) public nonReentrant {
        ...
        payOrLockupPendingCALCIFER(_pid, _boost, amount);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        payOrLockupPendingCALCIFER(_pid, false, amount);
        ...
        if (pending > 0 || user.rewardLockedUp > 0) {
            ...
            //Check if user has Boost and add to total rewards
             if (userBoost[msg.sender] > 0) {
                ...
                totalRewards = pending.add(boostAmount);
             }
            ...
            totalRewards = pending.add(user.rewardLockedUp).add(user.boostLockedUp);
