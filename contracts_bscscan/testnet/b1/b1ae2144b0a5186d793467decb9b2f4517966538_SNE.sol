...
contract SNE is BEP20, Ownable {
    ...
    SNEDividendTracker public dividendTracker;
    ...
    constructor() BEP20("Sonic Engine", "SNE") {
    	...
    	dividendTracker = new SNEDividendTracker();
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "SNE: A swapping process is currently running, wait till that is complete");
        uint256 buyBackBalance = address(this).balance;
        swapBNBForTokens(buyBackBalance.div(10**2).mul(amount));
    }
	
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SNE: The dividend tracker already has that address");

        SNEDividendTracker newDividendTracker = SNEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SNE: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10000, "SNE: Dividend reward fee must be between 0 and 10000");
        dividendRewardsFee = newFee;
    }
    
    function updateBurnFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10000, "SNE: Burn fee must be between 0 and 10000");
        burnFee = newFee;
    }
	
	function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10000, "SNE: Liquidity fee must be between 0 and 10000");
        liquidityFee = newFee;
    }
	
	function updateMaxSellPercentage(uint8 newPercentage) public onlyOwner {
        require(newPercentage >= 0 && newPercentage <= 10000, "SNE: Sell percentage must be between 0 and 10000");
        maxSellPercentage = newPercentage;
    }
	
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SNE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SNE: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;
        emit ExcludeFromFees(account, excluded);
    }
	
	function excludeFromMaxWalletToken(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxWalletToken[account] != excluded, "SNE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SNE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
	
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SNE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SNE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SNE: Cannot update gasForProcessing to same value");
...
contract SNEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SNE_Dividend_Tracker", "SNE_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200 * (10**18);
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "SNE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "SNE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SNE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SNE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SNE_Dividend_Tracker: Cannot update claimWait to same value");
