...
contract MetaMinerFarmUpgradeable is
    OwnableUpgradeable,
    PausableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    uint256[] strengthMin;
    uint256[] energyMin;
    uint256[] strengthRange;
    uint256[] energyRange;

    uint256[] INIT_OUTPUT_PER_WEEKLY;
    uint256[] OUTPUT_PER_WEEKLY;

    uint256 public generation;
    uint256 private nonce;
    uint256 priceCounter;

    uint256 public minerPriceForUSDT; // META
    ...
    event IncreaseStock(address indexed user, uint256 starId, uint256 count);
    ...
    function initialize(
        ...
        generation = 1;
        strengthMin = [10, 16, 8, 24, 12];
        energyMin = [50, 106, 42, 180, 72];
        strengthRange = [4, 8, 4, 12, 6];
        energyRange = [20, 28, 16, 40, 16];
        INIT_OUTPUT_PER_WEEKLY = [500, 600, 700, 800, 1000];
        OUTPUT_PER_WEEKLY = [100, 150, 200, 300, 500];

        minerPriceForUSDT = 15 * 1e17; // META
        minerPrice = 5 * 1e17;
        totalPrice = 16 * 1e18;
        stockPrice = 2 * 1e17;
        MINER_CLASS_PROBABILITY = [
            32,
            36,
            40,
            44,
    ...
    function increaseStock(uint256 starId, uint256 count)
        external
        whenNotPaused
    {
        address account = _msgSender();
        stock[starId] = stock[starId].add(count);
        uint256 totalAmount = stockPrice.mul(count);
        META.safeTransferFrom(account, address(FUND_ADDR), totalAmount);

        emit IncreaseStock(account, starId, count);
    }
    ...
    function batchBuy(
        ...
        require(areaId <= MINE_FIELD_COUNT, "areaId invalid.");
        require(count > 0 && count <= 10, "Max 10");
        require(count <= stock[starId], "Inventory shortage");

        for (uint256 i = 0; i < count; i++) {
            _buyMiner(starId, areaId, amount, metaAmount);
        }
        emit BatchBuy(_msgSender(), starId, areaId, amount, metaAmount, count);
    }
    ...
    function _buyMiner(
        ...
        uint256 price = _sharedRevenue(starId);
        uint256 mAmount = 0;
        if (totalPrice.mul(PRECISION).div(price) > minerPriceForUSDT) {
            mAmount = totalPrice.mul(PRECISION).div(price).sub(
                minerPriceForUSDT
            );
        }

        metaAmount = minerPrice > mAmount ? minerPrice : mAmount;
    ...
    function _sharedRevenue(uint256 starId) internal returns (uint256) {
        uint256 ownerRate = 5;
        ...
        USDT.transferFrom(
            _msgSender(),
            BUY_BACK_BURN_ADDR,
            payment.sub(ownerFee).sub(buyBackFee)
        );
        priceCounter++;
