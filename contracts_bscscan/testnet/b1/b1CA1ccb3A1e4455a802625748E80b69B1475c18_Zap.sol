...
contract Zap is IZap, OwnableUpgradeable {
    ...
    /*
     * initialize the addrees list of BEP20 token (not LP token or flip)
     */
    function initialize(address[] memory _tokens) external initializer {
        __Ownable_init();
    ...
    function zapInToken(address _from,uint256 amount,address _to) external override {
        ...
        if (isFlip(_to)) {
            IAlitaPair pair = IAlitaPair(_to);
            ...
            if (_from == token0 || _from == token1) {
                ...
                ROUTER.addLiquidity(_from,other,amount.sub(sellAmount),otherAmount,0,0,msg.sender,block.timestamp,0);
            } else {
    ...
    function zapOut(address _from, uint256 amount) external override {
        ...
        if (!isFlip(_from)) {
            ...
            IAlitaPair pair = IAlitaPair(_from);
            address token0 = pair.token0();
            address token1 = pair.token1();
    ...
    function _swapBNBToFlip(address flip,uint256 amount,address receiver) private {
        if (!isFlip(flip)) {
            _swapBNBForToken(flip, amount, receiver);
        } else {
            // flip
            IAlitaPair pair = IAlitaPair(flip);
            address token0 = pair.token0();
            address token1 = pair.token1();
            if (token0 == WBNB || token1 == WBNB) {
