...
contract Ateron is IERC20, Ownable {
    ...
    constructor() public {
        ...
        _owned[msg.sender] = crowdSaleSupply;
        emit Transfer(address(0), msg.sender, crowdSaleSupply);
    ...
    function _distributePrivateSale(uint256 amount) private {
        ...
        for(uint i = 0; i < _walletsForPrivateSale.length; i ++){
            _owned[_walletsForPrivateSale[i]] = amountPerWallet;
            emit Transfer(address(0), _walletsForPrivateSale[i], amountPerWallet);
        }
    }
}