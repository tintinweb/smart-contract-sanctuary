...
contract DividendPayingToken is ERC20, IDividendPayingToken {
    ...
    address public adminAddress = 0x2454e383BdE9401C13f00555D004592A94D7cB2d;
...
contract UsdtDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("USDT_Dividend_Tracker", "USDT_Dividend_Tracker", 0x337610d27c682E347C9cD60BD4b3b107C9d34dDd) {
        claimWait = 3600;
...
contract AppleSwap is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool public sendUsdtInTx = false;
    ...
    address public teamWallet = 0x1A0074d0Ce8395B2B230E03c29778b55Eb5dc999;
    address public marketingWallet = 0x9700F51C88D60F659D34aC70531636410bC66D51;

    address public usdtDividendToken = 0x337610d27c682E347C9cD60BD4b3b107C9d34dDd;
    UsdtDividendTracker public ustDividendTracker;

    //USDT => testnet
    address public wbnbReflectToken = 0x337610d27c682E347C9cD60BD4b3b107C9d34dDd;
    address public wbnbReflectTracker = 0xAc710bE8c571d510b91262cE60994ea5A524BD44;
    address public toBurnAllianceToken = 0x337610d27c682E347C9cD60BD4b3b107C9d34dDd;
    address public boardTreasury = 0x9d65d45E4627dF9C7c96E782CF27B892e188D2AE;
    ...
    uint256 public wbnbReflectRewardsFee = 0;
    uint256 public usdtDividendRewardsFee = 400;
    uint256 public marketingFee = 400;
    uint256 public boardTreasuryFee = 0;
    ...
    uint256 public maxBuyTransactionAmount = 600000 * 10 ** 18;
    uint256 public maxSellTransactionAmount = 600000 * 10 ** 18;
    uint256 public swapTokensAtAmount = 50000 * 10 ** 18;
    uint256 public maxWalletToken = 600000 * 10 ** 18;
    ...
    constructor() ERC20("AppleSwap", "APPLE") {
        ustDividendTracker = new UsdtDividendTracker();
        ...
        excludeFromDividend(address(ustDividendTracker));
    ...
    function prepareForPartner(address _partnerOrExchangeAddress) external onlyOwner {
        ustDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
    ...
    function setUsdtDividendToken(address _newContract) external onlyOwner {
        usdtDividendToken = _newContract;
        ustDividendTracker.setDividendTokenAddress(_newContract);
    }

    function setMinUsdtBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        ustDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }
    ...
    function setBoardTreasuryFee(uint256 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        boardTreasuryFee = newFee;
        _updateTotalFee();
    }
    ...
    function setUsdtDividendTracker(address newAddress) external onlyOwner {
        UsdtDividendTracker newUsdtDividendTracker = UsdtDividendTracker(payable(newAddress));

        require(newUsdtDividendTracker.owner() == address(this), "must be owner");
        ...
        newUsdtDividendTracker.excludeFromDividends(address(dexRouter));
        newUsdtDividendTracker.excludeFromDividends(address(deadAddress));

        ustDividendTracker = newUsdtDividendTracker;
    }

    function setUsdtDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        ...
        if(value) {
            ustDividendTracker.excludeFromDividends(pair);
        }
    ...
    function setMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        ustDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }

    function setClaimWait(uint256 claimWait) external onlyOwner {
        ustDividendTracker.updateClaimWait(claimWait);
    }
    ...
    function excludeFromDividend(address account) public onlyOwner {
        ustDividendTracker.excludeFromDividends(address(account));
    }

    function processDividendTracker(uint256 gas) external onlyOwner {
        (uint256 aIterations, uint256 aClaims, uint256 aLastProcessedIndex) = ustDividendTracker.process(gas);
        emit ProcessedUsdtDividendTracker(aIterations, aClaims, aLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function claim() external {
        ustDividendTracker.processAccount(payable(msg.sender), false);
    }
    ...
    function _updateTotalFee() internal {
        ...
        .add(liquidityFee)
        .add(marketingFee)
        .add(usdtDividendRewardsFee)
        .add(wbnbReflectRewardsFee)
        .add(toBurnAllianceFee)
        .add(toBurnTokenFee)
        .add(boardTreasuryFee);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if(boardTreasuryFee > 0){
                    uint256 tokensToTreasury = contractTokenBalance.mul(boardTreasuryFee).div(totalFees);
                    super._transfer(address(this), boardTreasury, tokensToTreasury);
                }

                if (usdtDividendRewardsFee > 0) {
                    uint256 sellTokens = contractTokenBalance.mul(usdtDividendRewardsFee).div(totalFees);
                    _swapAndSendUsdtDividends(sellTokens.sub(1300));
                }
        ...
        try ustDividendTracker.setBalance(payable(from), balanceOf(from), sendUsdtInTx) {} catch {}
        try ustDividendTracker.setBalance(payable(to), balanceOf(to), sendUsdtInTx) {} catch {}

        if(!swapping && to != deadAddress && sendUsdtInTx && usdtDividendRewardsFee > 0){
            uint256 gas = gasForProcessing;

            try ustDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
    ...
    function _swapAndSendUsdtDividends(uint256 tokens) private {
        uint256 beforeAmount = IERC20(usdtDividendToken).balanceOf(address(ustDividendTracker));

        _swapTokensForDividendToken(tokens, address(ustDividendTracker), usdtDividendToken);

        uint256 usdtDividends = IERC20(usdtDividendToken).balanceOf(address(ustDividendTracker)).sub(beforeAmount);

        if(usdtDividends > 0){
            ustDividendTracker.distributeDividends(usdtDividends);
            emit SendUsdtDividends(usdtDividends);
        }
    }
    ...
    event SendUsdtDividends(uint256 amount);
    event SendWbnbDividends(uint256 amount);
    event ProcessedUsdtDividendTracker(uint256 iterations, uint256 claims, uint256 lastProcessedIndex, bool indexed automatic, uint256 gas, address indexed processor);
}