...
interface IBaseDoNFT is IERC721Receiver {
    ...
    function mintFromNow(uint256 tokenId,uint256 durationId,uint64 duration,address to)external returns(uint256 tid);
    ...
    function getOrignalNftId(uint256 tokenId) external view returns(uint256);

    function checkIn(address to,uint256 tokenId,uint256 durationId) external;

    function getUser(uint256 tokenId) external returns(address);
...
contract BaseDoNFT is OwnableContract,ReentrancyGuard,ERC721Enumerable,IBaseDoNFT {
    ...
    function mintFromNow(uint256 tokenId,uint256 durationId,uint64 duration,address to)public returns(uint256 tid){
        tid = mint(tokenId, durationId, uint64(block.timestamp), uint64(block.timestamp+duration), to);
    }
    ...
    function getUser(uint256 tokenId) public view returns(address){
        return checkInUserMapping[tokenId];
    }
...
// File: contracts/VirtualDoNFT.sol
