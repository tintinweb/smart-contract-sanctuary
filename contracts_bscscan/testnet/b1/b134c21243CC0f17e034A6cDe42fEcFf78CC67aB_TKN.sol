...
abstract contract VestController is ERC20 {
    ...
    function _vestingSchedule(uint64 timestamp) internal view virtual returns (uint256) {
        if (timestamp < start()) {
            ...
            return (_totalTokens * (timestamp - start())) / duration();
        }
    }
    ...
    function releaseVesting() public virtual {
        require(_vestingReleased < _totalTokens, "No more tokens left to vest!");
        uint64 timestamp = uint64(block.timestamp);
        uint256 releasable = vestedAmount(timestamp) - released();
        if(releasable < 0){
            releasable = 0;
        }
...
contract TKN is ERC20, Ownable, VestController {
    ...
    uint256 public _maxWalletToken = 200000000 * (10**9);
    // admin wallets
    ...
    constructor() ERC20("TokenVest","TKN",9) {
        // vest duration - 48 months
        uint64 _vestTime = 126227664;
        // LP burn address
        burnAddress = address(0xdead);
        SwapRouter02 _swapV2Router = SwapRouter02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
        ...
        uint256 _totalSupplyMaster = 50000000000 * (10**9);
        ...
        _startVesting(owner(),uint64(block.timestamp),_vestTime,_totalSupplyVesting);
    }
    ...
    function updateMaxWalletAmount(uint256 newAmountNoDecimials)
        external
        onlyOwner
    {
        ...
        onlyOwner
    {
        ...
        //dividendTracker.excludeFromDividends(teamAddress);
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if (sellTeamFee != 0) {
            teamFeeFull = tokens.mul(sellTeamFee).div(100);
        }

        uint256 tokensAfterFee = tokens - teamFeeFull;
    ...
    function _transfer(
        ...
        if (takeFee && sellTotalFees != 0) {
            if (sellBurnFee == 0) {
                uint256 feeAmount = amount.mul(sellTotalFees).div(100);
