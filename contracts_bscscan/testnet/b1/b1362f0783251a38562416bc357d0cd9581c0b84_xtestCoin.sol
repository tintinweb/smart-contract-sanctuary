...
contract xtestCoin is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "XTST token";
    string private _symbol = "$XTST2";
    ...
    function _transfer(
        ...
        if (
            !_tradingOpen && 
            !(from == owner() && to == address(pancakeRouter)) &&
            !(from == address(pancakeRouter) && to == pancakePair)
        ) {
            require(
