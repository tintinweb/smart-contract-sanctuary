...
import "../libraries/utils/EnumerableMap.sol";
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    //  * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
    //  */
    function _initialize_erc721 (string memory name, string memory symbol) internal {
        _name = name;
        _symbol = symbol;
...
contract ERC721WithBalancesLike1155 is ManagerIsOwner, IBalancesLikeERC1155, ERC721
{
    ...
    function mint(address to, uint256[9] memory keys, uint256[9] memory amounts)
        public override
        onlyManager
        returns (uint256)
    {
    ...
    function setMany(uint256 tokenId, uint256[9] memory keys, uint256[9] memory values)
        public override
        onlyManager tokenIdExists(tokenId)
    {
...
abstract contract DungeonsWhitelist is Ownable {
    ...
    modifier onlyDungeonsWhitelist() {
        require(isWhitelisted(msg.sender), "only Dungeon Whitelist");
        _;
    }
