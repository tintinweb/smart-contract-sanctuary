...
contract PLTLottery is ReentrancyGuard, IPLTLottery, Ownable {
    ...
    struct Lottery {
        ...
        uint256[10] ticketPerPool;
        uint256[10] countWinnersPerPool;
    ...
    function buyTickets(uint256 _lotteryId, uint8[] calldata _ticketNumbers)
        external
        override
        notContract
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            require((thisTicketNumber >= 0) && (thisTicketNumber <= 9), "Outside range");

            _lotteries[_lotteryId].ticketPerPool[thisTicketNumber]++;
    ...
    function claimTickets(
        uint256 _lotteryId,
        uint256[] calldata _ticketIds) external override notContract nonReentrant {
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            require(rewardForTicketId != 0, "No prize for one of the given tickets");
    ...
    function drawFinalNumberAndMakeLotteryClaimable(uint256 _lotteryId, bool _autoInjection)
        ...
        uint32 winningNumber = finalNumber % 10;
        ...
        // no winners
        if(_lotteries[_lotteryId].ticketPerPool[winningNumber] == 0) {
            amountToWithdrawToTreasury = _lotteries[_lotteryId].amountCollectedInPLT;
        }

        // Update internal statuses for lottery
        _lotteries[_lotteryId].winningNumber = winningNumber;
        ...
        emit LotteryNumberDrawn(currentLotteryId, finalNumber,  _lotteries[_lotteryId].ticketPerPool[winningNumber]);
    }
    ...
    function startLottery(
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            ticketPerPool: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0),uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerPool: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0),uint256(0), uint256(0), uint256(0), uint256(0)],
    ...
    /**
     * @notice View rewards for a given ticketand lottery id
    ...
    function viewRewardsForTicketId(
        uint256 _lotteryId,
        uint256 _ticketId) external view returns (uint256) {
    ...
    function _calculateRewardsForTicketId(
        ...
        uint32 winningNumber = _lotteries[_lotteryId].winningNumber;

        // Retrieve the user number combination from the ticketId
        uint32 winningTicketNumber = _tickets[_ticketId].number;

        if(winningNumber == winningTicketNumber) {
            _lotteries[_lotteryId].amountToShareToWinners / ( _lotteries[_lotteryId].ticketPerPool[winningNumber]);
        } else {
            return 0;
        }
    }
