...
contract IrishShibaCoin is IBEP20, Ownable {
    ...
    uint public constant InitialSupply = 777 * 10**6 * 10**_decimals;     //equals 777,000,000 Token
    ...
    uint256 public _maxTxAmount = 777 * 10**4 * 10**18;   // 1%
    uint256 private numTokensSellToAddToLiquidity = 777 * 10**3 * 10**18;  // 0.1%
    ...
    uint public _dailyTax;

    uint constant public prizeFee = 5000;       // 50%
    uint public liquidityFee = 2500;   // 25%
    uint public coFounderFee = 2500;   // 25%

    function setFees(uint liquidityFee_, uint coFounderFee_) public onlyCoFounder {
        liquidityFee = liquidityFee_;
        coFounderFee = coFounderFee_;
    }

    function changeCoFounder(address newWallet) public onlyCoFounder {
        coFounder = newWallet;
    }
    function changeDevWallet(address newWallet) public onlyCoFounder {
        devWallet = newWallet;
    }
    //modifier for functions only the team can call
    modifier onlyCoFounder() {
        require(coFounder == msg.sender || msg.sender == owner() , "Caller not Cofounder");
        _;
    }
    //Constructor///////////////////////////////////////////////////////////////////////////////////////////
    constructor () {
    ...
    function processDailyPrize(address winner) public onlyCoFounder {
        require(winner == address(0), "Error: Invalid winner.");
        uint winnerPrize = _calculateFee(_dailyTax, prizeFee, 10000);
        _balances[winner] = _balances[winner] + winnerPrize;
        uint liquidityAmount = _calculateFee(_dailyTax, liquidityFee, 10000);
        swapAndLiquify(liquidityAmount);
        uint coFounderAmount = _calculateFee(_dailyTax, coFounderFee, 10000);
        _balances[coFounder] = _balances[coFounder] + coFounderAmount;
        uint devAmount = _dailyTax - (winnerPrize+liquidityAmount+coFounderAmount);
        _balances[devWallet] = _balances[devWallet] + devAmount;
        _dailyTax = 0;
    }
    ...
    function _taxedTransfer(address sender, address recipient, uint amount) private{
        ...
        if ( (sender!=_pancakePairAddress) && (!manualSwap) && (!_isSwappingContractModifier) && overMinTokenBalance )
    ...
    function switchManualSwap(bool manual) public onlyCoFounder {
        manualSwap=manual;
    }
    ...
    function setTaxes(uint buy, uint sell, uint transfer_) public onlyCoFounder {        
        buyTax=buy;
        sellTax=sell;
    ...
    function setAMM(address AMM, bool Add) public onlyCoFounder {
    ...
    function excludeAccountFromFees(address account, bool exclude) public onlyCoFounder {
