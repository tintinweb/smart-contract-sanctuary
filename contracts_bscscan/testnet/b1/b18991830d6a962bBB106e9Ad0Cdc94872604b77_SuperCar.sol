...
contract LockToken is Ownable {
    ...
    function isIncludeToWhiteList(address account) public view returns(bool)
    {
...
contract SuperCar is Context, IERC20, LockToken {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x5892c26B03426a38829eB94A3D798F68050af7dF); // Marketing wallet Address.
    address payable public developerAddress = payable(0x0Cd2D9184DA4Da804b1303dDBfa25Cf1e43BE001); // developer wallet address. 
    ...
    uint256 private _tTotal = 1_000_000_000 * 10**9;  //total supply. 
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "SuperCar6"; //token name
    string private _symbol = "SUPC6"; // token symbol. 
    ...
    uint256 private _saleMarketingFee = 1;
    uint256 private _saleDeveloperFee = 1;
    uint256 private _saleBurnFee = 1;

    uint256 public _maxTxAmount = 1_000_000_0 * 10**9;
    uint256 private minimumTokensBeforeSwap = 10_000 * 10**9; 
    ...
    constructor () {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);//test net swap exchange.
        //IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);//pancakeswap main net. 
        //IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xBBe737384C2A26B15E23a181BDfBd9Ec49E00248);//pinksale test router.
    ...
    function updateRouterAddress(address _router) public onlyOwner
    {
        ...
        try IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH()) returns(address _pair)
        {
    ...
    function isExcludeFromMaxTxLimit(address account) public view returns(bool) {
    ...
    {
        uint256 __totalFee = _liquidityFee.add(_marketingFee).add(_developerFee);
        uint256 halfLiquidityTokens = contractTokenBalance.div(__totalFee).mul(_liquidityFee).div(2);
        ...
        uint256 bnbForLiquidity = newBalance.div(__totalFee).mul(_liquidityFee).div(2);
        // add liquidity to uniswap
        addLiquidity(bnbForLiquidity, bnbForLiquidity);
        emit SwapAndLiquify(halfLiquidityTokens, bnbForLiquidity, halfLiquidityTokens);
        uint256 bnbForMarketing = newBalance.div(__totalFee).mul(_marketingFee);
        uint256 bnbForDeveloper = newBalance.sub(bnbForLiquidity).sub(bnbForMarketing);
        marketingAddress.transfer(bnbForMarketing);
        developerAddress.transfer(bnbForDeveloper);
    }
    ...
    {
        _amount = _amount.mul(100).div(100-_burnFee);
        return _amount.mul(_liquidityFee.add(_marketingFee).add(_developerFee)).div(100);
    }
    ...
    function setAllSaleFeesPercents(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 developerFee, uint256 burnFee) 
    external onlyOwner
    {
        _saleTaxFee = taxFee;
...
//////--------  ANTI WHALE -------/////--------
    ...
    uint256 public maxLimit =  _tTotal.div(100).mul(2); //2%
