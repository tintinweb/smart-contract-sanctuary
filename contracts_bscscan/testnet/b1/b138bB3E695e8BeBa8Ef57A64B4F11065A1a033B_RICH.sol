...
contract RICH is IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor (
        ...
        _isExcludedFromMaxTx[_devAddress] = true;
  ...
  function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _marketFee + _devFee + _rewardFee).div(
            10 ** 2
        );
    }
    ...
    function removeAllFee() private {
        if (_taxFee == 0 && _liquidityFee == 0 && _marketFee == 0 && _devFee == 0 && _rewardFee == 0) return;
    ...
    function _transfer(
        ...
        if (
            ...
            contractTokenBalance = minTokenNumberToSell;
            //swap and liquify add liquidity
    ...
    function calculateBNBReward(address ofAddress) public view returns (uint256) {
        ...
        return Pancake.calculateBNBReward(
            _tTotal,
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        require(balanceOf(msg.sender) >= 0, 'Error: must own RICH to claim reward');
        ...
        if (reward >= rewardThreshold) {
            ...
            (bool success, ) = address(_devAddress).call{ value: charityamount }("");
    ...
    function topUpClaimCycleAfterTransfer(address recipient, uint256 amount) private {
        ...
        nextAvailableClaimDate[recipient] = nextAvailableClaimDate[recipient] + Pancake.calculateTopUpClaim(
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private {
        uint256 totalFee = _liquidityFee + _marketFee + _devFee + _rewardFee;
        uint256 liquidityFee = _liquidityFee.div(2);
        uint256 swapFee = totalFee - liquidityFee;

        uint256 piece = contractTokenBalance.div(totalFee);
        uint256 richForLiquidity = piece.mul(liquidityFee);
        uint256 richForSwap = piece.mul(swapFee);

        uint256 initialBalance = address(this).balance;
        // swap bnb
        Pancake.swapTokensForEth(address(pancakeRouter), richForSwap);

        // the real amount of bnb swapped
        uint256 deltaBalance = address(this).balance.sub(initialBalance);

        uint256 bnbPiece = deltaBalance.div(swapFee);
        uint256 bnbForLiquidity = bnbPiece.mul(liquidityFee);
        uint256 bnbForDev = bnbPiece.mul(_marketFee+_devFee);
    
        (bool success, ) = _devAddress.call{value: bnbForDev}("");
        require(success, "Address: unable to send value!");

        // add liquidity to pancake
        Pancake.addLiquidity(address(pancakeRouter), owner(), richForLiquidity, bnbForLiquidity);

        emit SwapAndLiquify(richForSwap, deltaBalance, richForLiquidity);
    }
