...
contract ShibaMonSale is DataStorage, Access, Events, Manageable {
    ...
    function _buyToken(address _beneficiary) internal {
        ...
        require(
            msg.value <= maxInvest,
            "Requried: Amount to buy token one time too much"
        );
        require(
            user.amountInvest.add(msg.value) <= maxBuyAvailable,
    ...
    function setMaxBuyAvailable(uint256 _amount) external onlyAdmins {
        maxBuyAvailable = _amount;
    }    
