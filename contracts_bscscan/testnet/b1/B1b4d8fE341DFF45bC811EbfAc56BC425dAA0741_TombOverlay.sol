...
contract TombOverlay is Ownable, VRFConsumerBase {
    uint bracketBStart = 1000;      // The precentage of the pool required to be in the second bracket
    uint bracketCStart = 2000;      // The precentage of the pool required to be in the third bracket
    ...
    function addPool(
        ...
        // Lowest bracket: 80% common, 10% uncommon, 5% rare, 5% legendary, 0% mythic
    ...
    function priceInBnb(uint256 _price) public view returns(uint) {
        return priceConsumer.usdToBnb(_price);
    }
    ...
    function startMinting(uint _pid) public payable canStartMinting(_pid) returns (bytes32) {
        require(msg.value >= priceInBnb(mintingFee), 'Minting: Insufficient BNB for minting fee');
