...
contract LevelUp is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public minTokenNumberToSell = _totalSupply.div(10000); // 0.01% max tx amount will trigger swap and add liquidity
    ...
    uint256 public additionalFeeTime = 24 hours;
    ...
    function setExcludeFromReward(address _address, bool value)
        public
        onlyOwner
    {
        _isExcludedFromReward[_address] = value;
    }
    ...
    function setFeePercent(
        uint256 _sellFee,
        uint256 _additionalFeeTime
    ) external onlyOwner {
        additionalFeeTime = _additionalFeeTime;
        _totalSellFeePerTx = _sellFee;
    }
    ...
    function _tokenTransfer(
        ...
        if (!takeFee) {
            ...
            if (block.timestamp < launchTime.add(additionalFeeTime)) {
                uint256 _devFee = amount.mul(_totalSellFeePerTx).div(1e3);
                uint256 _totalFee = amount.mul(_totalFeePerTx).div(1e3);
                uint256 transferAmount = amount.sub(_totalFee).sub(_devFee);
                _balances[sender] = _balances[sender].sub(amount);
                ...
                // approve contract
                _approve(address(this), address(uniSwapV2Router), _devFee);
    ...
    function claimEthReward() public isHuman nonReentrant {
        ...
        require(
            !_isExcludedFromReward[msg.sender],
            "LVLUP: excluded from reward"
        );
    ...
    function migrateFunds(uint256 amount) external onlyOwner {
