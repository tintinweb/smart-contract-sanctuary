...
contract WJAX is BEP20 {
  ...
  event Set_Jax_Planet(address jax_planet);
  ...
  modifier onlyAdmin() {
    require(jaxAdmin.userIsAdmin(msg.sender), "Only JaxAdmin Contract");
    _;
  }

  modifier onlyAjaxPrime() {
    require(jaxAdmin.userIsAjaxPrime(msg.sender), "Only JaxSwap can perform this operation.");
    _;
  }
  ...
  function setJaxPlanet(address _jaxPlanet) public onlyAdmin {
    jaxPlanet = IJaxPlanet(_jaxPlanet);
    emit Set_Jax_Planet(_jaxPlanet);
  }

  function setGateKeeper(address _gateKeeper) public onlyAjaxPrime {
    gateKeeper = _gateKeeper;
    emit Set_Gate_Keeper(_gateKeeper);
  }

  function set_active_status(Status status) public onlyAdmin { 
    active_status = status;
    emit Set_Active_Status(status);
  }

  function setTransactionFee(uint tx_fee, uint tx_fee_cap, address wallet) public onlyJaxAdmin {
  ...
  /**
    ...
    */
  function setReferralFee(uint _referral_fee, uint _referrer_amount_threshold) public onlyJaxAdmin {
  ...
  /**
    * @dev Set cashback
    */
  function setCashback(uint cashback_percent) public onlyJaxAdmin {
