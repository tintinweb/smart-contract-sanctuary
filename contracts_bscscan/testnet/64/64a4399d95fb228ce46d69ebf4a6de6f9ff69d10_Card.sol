...
contract Card is IERC721Receiver {
    ...
    mapping(address => mapping(uint8 => uint256)) _userBox;
    ...
    // 用户的盲盒
    function boxOf(address account, uint8 index) public view returns (uint256){
        return _userBox[account][index];
    }
    ...
    // 购买盲盒
    function buyBox(uint8 index) public onlyActivate {
        require(index < 5, "Index must less than 5");
        require(_userBox[msg.sender][index] == 0, "Already have box");
    ...
    // 给用户添加盲盒
    function addBox(address account, uint8 index, uint256 tokenId) public onlyOwner {
        _userBox[account][index] = tokenId;
    }

    // 打开盲盒
    function openBox(uint8 index) public onlyActivate {
        require(index < 5, "Index must less than 5");
        require(_userBox[msg.sender][index] != 0, "Have no box");
        IERC721(nft).transferFrom(address(this), msg.sender, _userBox[msg.sender][index]);
        _tokenLife[_userBox[msg.sender][index]] = block.timestamp;
        _userBox[msg.sender][index] = 0;
    }
    ...
    function geneChange(uint256 tokenId, bool boom) public onlyActivate {
        require(IERC721(nft).ownerOf(tokenId) == msg.sender, "Token not belong to you");
    ...
    function play(uint256[] memory tokenIds) public onlyActivate {
        ...
        IERC721 ierc = IERC721(nft);
        require(ierc.balanceOf(msg.sender) >= 3, "Not enough tokens");
        ...
        if (ierc.ownerOf(tokenIds[0]) != msg.sender || ierc.ownerOf(tokenIds[1]) != msg.sender || ierc.ownerOf(tokenIds[2]) != msg.sender) {
        ...
        if (_tokenLife[tokenIds[0]] == 0 || _tokenLife[tokenIds[1]] == 0 || _tokenLife[tokenIds[2]] == 0) {
            revert("Wrong token id");
        }
        nftf.transferFrom(msg.sender, address(this), 700000000 * 10 ** nftf.decimals());
    ...
    function buyLife(uint64 num) public onlyActivate {
        usdt.transferFrom(msg.sender, first, 100 * num * 10 ** usdt.decimals());
    ...
    function rest(uint256 tokenId) public onlyActivate {
        require(IERC721(nft).ownerOf(tokenId) == msg.sender, "Token not belong to you");
        require(block.timestamp.sub(_tokenLife[tokenId]) < 5184000, "Please remain token life");
        require(block.timestamp % 86400 >= 0 && block.timestamp % 86400 <= 300, "Wrong time");
        DayRest memory info = _tokenDayInfo[tokenId][day];
        if (info.times != 0) {
            require(block.timestamp - info.point <= 3900, "Must rest continuous");
        }
        nftf.transferFrom(msg.sender, address(this), 1163750000 * 10 ** nftf.decimals());
        _tokenDayInfo[tokenId][day].times++;
        _tokenDayInfo[tokenId][day].point = uint64(block.timestamp);
        _tokenDayInfo[tokenId][day].amount += 1163750000 * 10 ** nftf.decimals();
    ...
    function restAmount(uint256 tokenId) public view returns (uint256, uint16){
        ...
        for (; startDay <= day; startDay++) {
            total = total.add(_dayRate(tokenId, startDay, total));
            totalTimes += _tokenDayInfo[tokenId][startDay].times;
        }
        return (total, totalTimes);
    }

    function _dayRate(uint256 tokenId, uint256 _day, uint256 lastDayAmount) private view returns (uint256){
        ...
        return (dr.amount.add(lastDayAmount)).mul(100 + rate + level).div(100);
    }
    ...
    function restTake(uint256 tokenId) public onlyActivate {
        ...
        nftf.transfer(msg.sender, amount.mul(100 - rate).div(100));
    ...
    function settle(address winner, address loser) public onlyOwner {
        nftf.transfer(winner, 901600000 * 10 ** nftf.decimals());
        nftf.transfer(loser, 386400000 * 10 ** nftf.decimals());
        _play[winner] = false;
        _play[loser] = false;
    }
