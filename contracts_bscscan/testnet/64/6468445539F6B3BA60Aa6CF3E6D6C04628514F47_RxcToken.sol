...
contract RxcToken is Context, IBEP20, Ownable, ReentrancyGuard {
  ...
  //setting up SafeSale token locks
    ...
    uint256 private constant _presaleUnlockPrice = 5 * 10**6 * 10**9;  // This is 0.05 BNB
    uint256 private constant _presaleMinimum = 100 * 10**6 * 10**9;  // This is 0.1 BNB - must include 18 zeroes
    uint256 private constant _presaleMaximum = 3 * 10**9 * 10**9;  // This is 3 BNB - must include 18 zeroes
    ...
    mapping (address => bool) private _systemsAddresses;
  ...
  constructor(
    ...
    _symbol = "RXC";
    _decimals = 9;
    _totalSupply = __totalSupply.sub(_presaleMaxTokens) * 10**9;//1000000
    ...
    treasuryAddress = msg.sender;//0xEF7e457508601C9D76428D2503E7A3e4c0FB5358
    ...
    _balances[developersAddress] = _developersInitialBalance;
    _balances[operationsAddress] = _operationsInitialBalance;
  ...
  function setOperationsAddress(address _operationsAddress) public onlyOwner {
      _isSpecialAddress[operationsAddress] = false;
      operationsAddress = _operationsAddress;
      _isSpecialAddress[_operationsAddress] = true;
  }

  function setDevelopersAddress(address _developersAddress) public onlyOwner {
      _isSpecialAddress[developersAddress] = false;
      developersAddress = _developersAddress;
      _isSpecialAddress[_developersAddress] = true;
  }

  function setMarketingAddress(address _marketingAddress) public onlyOwner {
      _isSpecialAddress[marketingAddress] = false;
      marketingAddress = _marketingAddress;
      _isSpecialAddress[_marketingAddress] = true;
  }

  function setTreasuryAddress(address _treasuryAddress) public onlyOwner {
      _isSpecialAddress[treasuryAddress] = false;
      treasuryAddress = _treasuryAddress;
      _isSpecialAddress[_treasuryAddress] = true;
  }
    ...
    function setSystemAddress(address systemAddress, bool toggleSystemAddress) public onlyOwner {
      _systemsAddresses[systemAddress] = toggleSystemAddress;
    }
  ...
  function transfer(address recipient, uint256 amount) external override returns (bool) {
    _transfer(_msgSender(), _msgSender(), recipient, amount);
    return true;
  }
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
    _transfer(_msgSender(), sender, recipient, amount);
   ...
   * - `sender` must have a balance of at least `amount`.
   */
  function _transfer(address actualSender, address sender, address recipient, uint256 amount) internal {
    ...
    bool maxTxAmountAllowed = false;
    
    if(_isSpecialAddress[sender] || _systemsAddresses[actualSender]){
        maxTxAmountAllowed = true;
    }
    
    if(!maxTxAmountAllowed){
  ...
  function _getCurrentBalance(address account) private view returns(uint256){
    if(_isSpecialAddress[account]){
      if(account == developersAddress){
        ...
        .sub(getTokenLockAmount(account,block.timestamp))
      ...
      return _balances[account].sub(_specialExpenses[account]);
    }
    
    if (tokenLocks[account].amount != 0){
      return _balances[account].sub(getTokenLockAmount(account,block.timestamp));
    } 
    return _balances[account];
  }
  ...
  function getLockedWalletDetails(address account) public view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
      ...
      return (_startTime, _endTime, _totalAmount, _lockedAmount, _accessibleAmount, currentTime);
  }
  ...
  function getTokenLockAccessible(address account) public view returns (uint256) {
  ...
  function getTokenLockAmount(address account, uint256 currentTime) public view returns (uint256) {
      if (currentTime > tokenLocks[account].endTime) return 0;
      return ((tokenLocks[account].amount * ((tokenLocks[account].endTime - currentTime) * 100) / (tokenLocks[account].endTime - tokenLocks[account].startTime))) / 100;
  }

  // @dev allow use mint function until presalemaxtokens have ran out
  ...
  function acceptPresaleUnlockPayment() public nonReentrant payable {
  ...
  */
  function acceptPresalePayment() public nonReentrant payable {
      require(safeSaleActive, "Presale is not active at this time");
      ...
      require(totalTokensTradedPresale.add(_tokensFromPayment) <= presaleMaxTokens, "Presale is Full"); //presale has been maxed out!
      uint256 _lockEndTime = _presaleLockDurationSeconds + block.timestamp; // How long to lock for
      numOfHODLers++;
      assignTokenLock(msg.sender, _tokensFromPayment, block.timestamp, _lockEndTime ,"presale",false);
