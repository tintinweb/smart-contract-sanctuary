...
contract PrivateSell is ExtendedAccessControl{
	...
	{
		require(membersAddress.length == membersAmount.length,"Amount and members length can't be different");

		for(uint256 i=0;i<membersAddress.length;i++){

			require(amountOfWhitelistedTokens + membersAmount[i] <= AMOUNT_OF_TOKENS_IN_SALE,"amount exceed total tokens in sale");
			amountOfWhitelistedTokens += membersAmount[i];
			_whitelist[membersAddress[i]] += membersAmount[i];
	...
	addressIsInWhitelistOrExceedBuyTime(msg.sender,_amount) payable {
		...
		require(_amount <= remainingTokens(),"you can't buy more tokens than you have");
	...
	function vestedAmount(address _address) public view returns(uint256) {
		if(block.timestamp < PRESALE_START_TIMESTAMP){
			...
			return ((_tokenBuyed[_address] - _tokenBuyed[_address] /10 ) * (block.timestamp - PRESALE_START_TIMESTAMP)) / TOTAL_VESTING_TIME + _tokenBuyed[_address] / 10;
		}
	}
