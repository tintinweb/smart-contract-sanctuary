...
contract LocalToken is BEP20("2local Token", "2LC") {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "2LC::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "2LC::delegateBySig: invalid nonce");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "2LC::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying 2LCs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "2LC::_writeCheckpoint: block number exceeds 32 bits");
...
contract Staking2local is Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        // bonusRateForTime[30] = 4;
        // bonusRateForTime[60] = 12;
        // bonusRateForTime[90] = 24;
        // bonusRateForTime[360] = 100;
        bonusRateForTime[5] = 4;
        bonusRateForTime[10] = 12;
        bonusRateForTime[20] = 24;
        bonusRateForTime[25] = 100;
    }
    ...
    function getBonusRateFromLockedDate(uint256 _days) public view returns (uint256 bonusRate) {
        if (_days >= 5 && _days < 10) {
            bonusRate = bonusRateForTime[5];
        }
        else if (_days >= 10 && _days < 20) {
            bonusRate = bonusRateForTime[10];
        }
        else if (_days >= 20 && _days < 25) {
            bonusRate = bonusRateForTime[20];
        }
        else if (_days >= 25) {
            bonusRate = bonusRateForTime[25];   
        }
    }

    // View function to see pending 2LCs on frontend.
            ...
            uint256 currentTime = block.timestamp;
                for (uint256 i = 0; i < user.userRewardInfo.length; i++ ) {
                    ...
                    if (currentTime > rewardInfo.canHarvestTimestamp) {
                        ...
                        lockedDays = lockedDays.div(1 minutes);
    ...
    // function calculateUserRewardInfo(uint256 timestamp) public view returns(uint256) {
    //     return 0;
    // }
    ...
    function deposit(uint256 _pid, uint256 _amount, uint256 _days) public {
        ...
        if(_amount > 0) {
            ...
            user.userRewardInfo.push(UserRewardInfo({
                ...
                canHarvestTimestamp: block.timestamp.add(_days * 1 minutes),
                bonusRate: bonusRateForTime[_days],
                isPaid: false
                }));
        }
        // user.rewardDebt = user.amount.mul(pool.accLocalPerShare).div(1e12);
