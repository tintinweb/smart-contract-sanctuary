...
contract CollectRoomExtention is SafeOwnable {
    using SafeMath for uint256;
    using Strings for uint256;

    event RoomExtention(address roomManager, uint rid, bytes32 roomId, bytes32 name, string rules, string logo, uint position, bool display);
    ...
    function addOrSetRoomInfo(
        ...
        emit RoomExtention(roomManager, rid, roomId, name, rules, logo, position, display);
    }
}
