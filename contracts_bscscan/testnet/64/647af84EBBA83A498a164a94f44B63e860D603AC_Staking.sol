...
interface IDividendDistributor {
    ...
    function depositToken(address from, uint256 _amount) external;
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWDS = IBEP20(0x0687fA128095c2baa8E7D87D0FEd1b4BD3acA333);
    ...
    function depositToken(address from, uint256 _amount) external override onlyToken {
        uint256 balanceBefore = RWDS.balanceOf(address(this));
        RWDS.approve(from, _amount);
        RWDS.transferFrom(from, address(this), _amount);
        uint256 amount = RWDS.balanceOf(address(this)).sub(balanceBefore);
        totalDividends = totalDividends.add(amount);
        dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(amount).div(totalShares));
        currentBalance = RWDS.balanceOf(address(this));
...
contract Staking is Auth {
    ...
    function _addStake(address from, uint256 _amount) internal {
        totalStaked += _amount;
        stakeValue[msg.sender] += _amount;
        distributor.setShare(from, stakeValue[from]);
    }

    function settok(address _tok, address _rew) external authorized {
        token = IBEP20(_tok);
        RWDS = IBEP20(_rew);
    }

    function depositviaBNB() external payable authorized {
        distributor.depositBNB{value: msg.value};
    }

    function depositviaTokenWallet(uint256 _amount) external authorized {
        require(RWDS.balanceOf(msg.sender) >= _amount);
        distributor.depositToken(msg.sender, _amount);
    }
