...
contract buyback is Context, IERC20, Ownable {
    ...
    function checkBot(address recipient) private {
        if (block.number < launchedAt + limitBlock) {
            //fast bot sell
    ...
    function _transfer(
        ...
        // require(swapAndLiquifyEnabled, "Trading has been paused");
        ...
        // uint256 minimumTokensBeforeSwap = amount.mul(_buyBackRate).div(10**2);
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                if (_startTimeForSwap + _intervalMinutesForSwap <= block.timestamp) {
                    ...
                    swapTokens(amount.mul(_buyBackRate).div(10**2));    
                }  
