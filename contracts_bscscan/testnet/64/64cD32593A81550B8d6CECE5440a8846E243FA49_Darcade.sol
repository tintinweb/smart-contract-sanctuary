...
contract Darcade is ReentrancyGuard, Ownable, Machine {
    ...
    */
    ...
    /**
     * @dev {PrizeDistributionComplete} Fires when the prizes have been distributed
     */
    event PrizeDistributionComplete(string machineId, string gameId, string leaderboardId);
    ...
    */
    ...
    /**
     * @dev Adds the different elements of the array and return its sum
     */
     ...
     */
    function distributePrizes(string memory machineId, string memory gameId, string memory leaderboardId, address payable[] memory addrs, uint[] memory amntsInWei, IERC20 _prizeToken) public onlyProcessor {
        require(addrs.length == amntsInWei.length, "The length of two array should be the same");
        uint256 totalAmnt = sum(amntsInWei);
        uint256 _ourTokenBalance = _prizeToken.balanceOf(address(this));
        require(_ourTokenBalance >= totalAmnt, "We do not have enough of these tokens to distribute!");
        for (uint i=0; i < addrs.length; i++) {
            _prizeToken.transfer(addrs[i], amntsInWei[i]);
        }
        emit PrizeDistributionComplete(machineId, gameId, leaderboardId);
    }
