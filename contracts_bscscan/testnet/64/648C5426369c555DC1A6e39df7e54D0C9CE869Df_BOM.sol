...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "BOMToken: You are not owner");
        _;
    }
...
contract BOM is ERC20, Ownable {
    string public name = "BOM Token";
    ...
    function transfer(address _to, uint256 _value) public {
        require(balances[msg.sender] >= _value, "BOMToken: Insufficient balance");
        _transfer(msg.sender, _to, _value);
    }

    function approve(address _spender, uint256 _amount) public {
        require(_spender != address(0), "Zero address error");
        require((allowed[msg.sender][_spender] == 0) || (_amount == 0), "BOMToken: Approve amount error");
        allowed[msg.sender][_spender] = _amount;
        emit Approval(msg.sender, _spender, _amount);
    }

    function transferFrom(address _from, address _to, uint256 _value) public {
        require(balances[_from] >= _value && allowed[_from][msg.sender] >= _value, "BOMToken: Insufficient balance");
    ...
    function mint(address _owner, uint256 _value) public {
        require(msg.sender == mintAddress, "BOMToken: You are not mint address");
        _mint(_owner, _value);
    }
