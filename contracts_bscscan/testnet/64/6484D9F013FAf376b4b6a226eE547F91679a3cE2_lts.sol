...
contract lts is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public _totalSupply =  1000000000 * 10 ** 18; // 1 Billion LTS TOKENS IN LUNCH
    ...
    function setFeeStatus(bool feesStatus) external onlyOwner() nonReentrant returns (bool) {
    ...
    function manualBurn(uint256 tokenBurnAmount) external onlyOwner() nonReentrant returns (bool) {
        ...
        _balances[_msgSender()] = _balances[_msgSender()].sub(tokenBurnAmount);
        ...
        emit Transfer(_msgSender(), burnAddress, tokenBurnAmount);
        return true;
    }
}