...
interface IYokiToken {
    ...
    function buyTradeTransfer(address credit, uint256 amount) external returns(bool);
    function sellTradeTransfer(address debit, uint256 amount) external returns(bool);
}
...
// This function calls lp pair and checks for the current price of YOKI token in BUSD
    ...
    function buyTokens(uint256 yokiAmount) external {
        ...
        // yokiPriceInBUSD = sellPrice(1e18);
        ...
        // _transfer(address(this), msg.sender, yokiAmount);
        
        IYokiToken(yokiWallet).buyTradeTransfer(msg.sender, yokiAmount);    // makes the transfers
        uint256 pricePerUnit = busdAmount / yokiAmount;
    ...
    function sellTokens(uint256 yokiAmount) external {
        ...
        IYokiToken(yokiWallet).sellTradeTransfer(yokiWallet, yokiAmount);    // makes the transfers
