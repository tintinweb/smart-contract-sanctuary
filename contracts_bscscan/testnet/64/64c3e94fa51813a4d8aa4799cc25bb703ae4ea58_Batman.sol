...
/** 
 ...
 MANUAL SWAP -
 SWEEP -
 OWNER & PRESALE WALLET CAN TRANSFER BEFORE TRADING-ON -
 LOCK WALLET CAN BUY, CANT SELL, CANT TRANSFER TO OTHER WALLET -
 TRNSFR FROM OWNER TO BURN ADDRESS NO FEE -
 TRNSFR FROM OWNER TO OTHER WALLET NO FEE & NO MAX WALLET -
 TRNSFR FROM OTHER WALLET TO OTHER WALLET GOT BUY FEE & NO MIN/MAX WALLET -
...
contract Batman is Context, IBEP20, Ownable {
    ...
    function emergencyWithdraw(uint256 amount, address _token) public onlyOwner {
        IBEP20 token = IBEP20(_token);
        require(amount >= token.balanceOf(address(this)), 'The contract balance is too low');
        token.transfer(msg.sender, amount);
    }

    function transferContractToken(address token, address recipient, uint256 amount) public onlyOwner returns(bool _send){
    ...
    function setMaxTxAmount(uint256 buyTxAmount, uint256 sellTxAmount, uint256 maxWalletAmount, uint256 minWalletAmount) external onlyOwner() {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        bool bau  = sender == pancakeSwapV2Pair && recipient != address(pancakeSwapV2Router);
        bool sempit = recipient == pancakeSwapV2Pair;
