    ...
    BurnableToken
{
    ...
    function _priceImpactTax(uint256 amount) public view returns (bool) {
        ...
        uint256 _reserveA_converted = _reserveA.div(10**10);
        
        if (_reserveA_converted == 0 && _reserveB == 0) {
            return false;
        } else {
            if (amount >= _reserveA_converted) return false;

            uint256 _reserveA_new = _reserveA_converted.sub(amount);
