...
contract KawaiiSalePackBNBDiscount is Pausable, SignData {
    ...
    PackData public pack;
    mapping(address => bool) public isDiscounts;
    ...
    event BuyPack(address buyer, uint256 animals, uint256 plants, uint256 fieldIds);
    ...
    function setField(uint256[] calldata _fieldIds) external onlyOwner {
        fields = _fieldIds;
    }
    ...
    function setPack(uint256 _numberTree, uint256 _numberAnimal, uint256 _numberField, uint256 _price, uint96 _startTime, uint96 _endTime) external onlyOwner {
        pack = PackData(_numberTree, _numberAnimal, _numberField, _price, _startTime, _endTime);
    }

    function setListDiscount(address[] memory _accounts, bool isDiscount) external onlyOwner {
        for (uint256 i = 0; i < _accounts.length; i++) {
            isDiscounts[_accounts[i]] = isDiscount;
        }
    }
    ...
    function buyPackBNB(IERC1155 nftRegister, address payable sender, uint256 amount, uint8 v, bytes32 r, bytes32 s) payable external whenNotPaused {

        verify(keccak256(abi.encode(BUY_PACK_HASH, sender, 0, amount, nonces[sender]++)), sender, v, r, s);

        require(isDiscounts[sender] == true, "user not discount");

        require(purchasedInAccount[sender].add(amount) <= limitPerAccount,"Exceed limit in the account");
        purchasedInAccount[sender] = purchasedInAccount[sender].add(amount);

        require(uint96(block.timestamp) >= pack.startTime && uint96(block.timestamp) <= pack.endTime, "Expired");

        uint256 amountPay = pack.price.mul(amount);

        require(msg.value >= amountPay, "Exceed price");

        uint256 change = msg.value.sub(amountPay);
        ...
        for (uint256 i = 0; i < amount; i++) {
            (animalId, treesId, fieldId) = randomItem();
            _mintItem(nftRegister, sender, animalId, treesId, fieldId);
            emit BuyPack(sender, animalId, treesId, fieldId);
        }
    }


    function _mintItem(IERC1155 nftRegister, address sender, uint256 animalMint, uint256 treesMint, uint256 fieldMint) internal {
        nftRegister.mint(sender, animalMint, 1);
        nftRegister.mint(sender, treesMint, 1);
        nftRegister.mint(sender, fieldMint, 1);
    }

    function randomItem() internal returns (uint256, uint256, uint256){

        uint256 animalId;
        uint256 treeId;
        uint256 fieldId;

        animalId = chooseAnimal(animals.length, gasleft());

        treeId = chooseTree(animalId, gasleft());

        fieldId = chooseField(fields.length, gasleft());

        return (animalId, treeId, fieldId);
    }
