...
contract SATVDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SATV_Dividend_Tracker", "SATV_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "SATV_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "SATV_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SURF contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "SATV_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SATV_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SARTURNV is ERC20, Ownable {
   
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    SATVDividendTracker public dividendTracker;

    address public _marketingWallet = 0xF2B4d039A9D1A093BC9aF8e806b01ad91579343e;
    address public _charityWallet = 0xF2B4d039A9D1A093BC9aF8e806b01ad91579343e ;
    ...
    constructor() ERC20("SARTURNV", "SATV") {

        dividendTracker = new SATVDividendTracker();
        BuyTotalFees = BuyRewardsFee.add(BuyLiquidityFee).add(BuyMarketingFee).add(BuyCharityFee);
        SellTotalFees = SellRewardsFee.add(SellLiquidityFee).add(SellMarketingFee).add(SellCharityFee);
    ...
    function updateBuyFees(uint256 newBuyRewardFee, uint256 newBuyLiqFee, uint256 newBuyMarketingFee , uint256 newBuyCharityFee) public onlyOwner {
    ...
    function updateSellFees(uint256 newSellRewardFee, uint256 newSellLiqFee, uint256 newSellMarketingFee ,uint256 newSellCharityFee) public onlyOwner {
