...
abstract contract StellarDiamondBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	string private constant NAME = "LLG Token";
	string private constant SYMBOL = "LLG";
	uint8 private constant DECIMALS = 9;
	uint8 private _totalFee; //% of each transaction that will be added as liquidity
	uint8 private _marketingFee; //% of each transaction that will be added as liquidity
	uint8 private _devFee; //% of each transaction that will be added as liquidity
	...
	address private _marketingWalletAddress = 0x5EbE51508d1449958e1D058Ddd7643D1245eC9a1;
    address private _devWalletAddress = 0x61151B9B1071c7d0583eD5850B688DAcA0bcBeb0;

	uint256 private constant _totalTokens = 100000000 * 10**DECIMALS;	//1 trillion total supply
	...
	constructor (address routerAddress) {
		...
		// 3% liquidity fee, 3% marketing fee, 4% dev fee
		setFees(3, 3, 4);
	...
	function doTransfer(address sender, address recipient, uint256 amount) internal virtual {
		...
		// Perform a swap if needed.  A swap in the context of this contract is the process of swapping the contract's token balance with BNBs 
		// in order to provide liquidity and increase the reward pool
		...
		uint256 bnbSwapped = swapTokensForBNB(feeAmount);
		payable(_marketingWalletAddress).transfer(bnbSwapped * 3 / 10);
		payable(_devWalletAddress).transfer(bnbSwapped * 4 / 10);
	...
	function updateBalances(address sender, address recipient, uint256 sentAmount, uint256 feeAmount) private {
		...
		_balances[address(this)] += feeAmount * 3 /10;
	...
	function calculateFeeRate(address sender, address recipient) private view returns(uint256) {
		...
		if (applyFees) {
			if (isPancakeSwapPair(recipient)) {
				...
				return _marketingFee + _devFee + _liquidityFee;
			}
	...
	function setFees(uint8 liquidityFee, uint8 marketingFee, uint8 devFee) public onlyOwner {
		require(liquidityFee >= 1 && liquidityFee <= 5, "Liquidity fee must be between 1% and 5%");
		require(marketingFee >= 1 && marketingFee <= 5, "marketing fee must be between 1% and 5%");
		require(devFee >= 1 && devFee <= 5, "Dev fee must be between 1% and 5%");
