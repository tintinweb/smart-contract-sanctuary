...
contract CakeToken is BEP20('PancakeSwap Token', 'Cake') {
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes) internal {
        ...
        if (nCheckpoints > 0 && checkpoints[delegatee][nCheckpoints - 1].fromBlock == blockNumber) checkpoints[delegatee][nCheckpoints - 1].votes = newVotes;
        else {
...
contract MasterChef is Ownable {
    ...
    constructor(CakeToken _cake, SyrupBar _syrup, address _devaddr, uint256 _cakePerBlock, uint256 _startBlock) {
        cake = _cake;
        syrup = _syrup;
        devaddr = _devaddr;
    ...
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate) public onlyOwner {
        ...
        if (prevAllocPoint != _allocPoint) updateStakingPool();
    }

    function updateStakingPool() internal {
        uint256 length = poolInfo.length;
        uint256 points = 0;
        for (uint256 pid = 1; pid < length; ++pid) points += poolInfo[pid].allocPoint;
        if (points != 0) {
            points /= 3;
