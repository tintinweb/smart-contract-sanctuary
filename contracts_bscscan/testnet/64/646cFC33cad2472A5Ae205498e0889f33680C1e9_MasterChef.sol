...
import '@galaxyswap/galaxy-swap-lib/contracts/math/SafeMath.sol';
import '@galaxyswap/galaxy-swap-lib/contracts/token/BEP20/IBEP20.sol';
import '@galaxyswap/galaxy-swap-lib/contracts/token/BEP20/SafeBEP20.sol';
import '@galaxyswap/galaxy-swap-lib/contracts/access/Ownable.sol';
...
interface IMigratorChef {
    // Perform LP token migration from legacy GalaxySwap to GalaxySwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to GalaxySwap LP tokens.
    // GalaxySwap must mint EXACTLY the same amount of GalaxySwap LP tokens or
    // else something bad will happen. Traditional GalaxySwap does not
...
contract MasterChef is Ownable {
    ...
    // Update the given pool's GALAXY allocation point. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 galaxyReward = multiplier.mul(galaxyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        galaxy.mint(devaddr, galaxyReward.div(10));
        galaxy.mint(address(syrup), galaxyReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit GALAXY by staking');
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                safeGalaxyTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw GALAXY by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Stake GALAXY tokens to MasterChef
    ...
    function safeGalaxyTransfer(address _to, uint256 _amount) internal {
        syrup.safeGalaxyTransfer(_to, _amount);
    }
...
import "@galaxyswap/galaxy-swap-lib/contracts/token/BEP20/BEP20.sol";

// GalaxyToken with Governance.
contract GalaxyToken is BEP20('GalaxySwap Token', 'Galaxy') {
    ...
    */
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying GALAXYs (not scaled);
...
contract SyrupBar is BEP20('SyrupBar Token', 'SYRUP') {
    ...
    // Safe galaxy transfer function, just in case if rounding error causes pool to not have enough GALAXYs.
    function safeGalaxyTransfer(address _to, uint256 _amount) public onlyOwner {
