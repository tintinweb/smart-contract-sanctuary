...
contract HolyBed is Member, OwnableOperatorRole, SignHelper {
    ...
    mapping(uint256 => uint256) public totemCompoundSupply;
    uint256 public tokenFee = 10;

    event FaceUpToGod(
        address indexed owner,
        uint256 oblationOne,
        uint256 oblationTwo,
        uint256 oblationThree
    );

    event Compound(
        address indexed owner,
        uint256 resultTokenId,
        uint256[] oblationTokens
    );

    function changeTokenFee(uint256 fee) public CheckPermit("Config") {
        tokenFee = fee;
    }
    ...
    function checkTokenIds(uint256[] memory tokenIds, address _owner)
        internal
        returns (bool)
    {
        IDiaoTotem totem = IDiaoTotem(getMember("DiaoTotem"));

        (uint256 race, uint256 design, uint256 rarety, ) = totem.parseTokenId(
            tokenIds[0]
        );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            (uint256 _race, uint256 _design, uint256 _rarety, ) = totem
            ...
            totem.safeTransferFrom(_owner, address(0x00), tokenId);
        }
        return true;
    }
    ...
    function compound(
        ...
        //  代币转移
        IERC20 token = IERC20(getMember("DiaoCoin"));
        require(
            token.transferFrom(_owner, address(0x00), tokenFee * 10**18),
            "HolyBed: have not enough DiaoCoin"
        );

        require(tokenIds.length == 3, "HolyBed: Compound token amount wrong");
        IDiaoTotem totem = IDiaoTotem(getMember("DiaoTotem"));
        require(totem.isApprovedForAll(msg.sender, address(this)));

        require(
            checkSign(tokenIds.length, v, r, s),
            "HolyWall: have wrong sign"
        );

        require(
            checkTokenIds(tokenIds, _owner),
            "HolyBed: Compound token type wrong"
        );
        ...
        // Face up to God
        if (rarety == 4) {
            emit FaceUpToGod(_owner, tokenIds[0], tokenIds[1], tokenIds[2]);
            return;
        }

        uint256 totemSupplyIndex = tokenIds[0] / 10**3;
        totemCompoundSupply[totemSupplyIndex + 1] =
            totemCompoundSupply[totemSupplyIndex + 1] +
            1;
        uint256 newRaretySupplys = totemCompoundSupply[totemSupplyIndex + 1];

        uint256 originalSupply = totem.getOriginalSupply(rarety + 1);

        uint256 newTokenId = totem.buildNewTokenId(
            race,
            rarety + 1,
            design,
            originalSupply + newRaretySupplys
        );

        totem.mint(_owner, newTokenId);

        emit Compound(_owner, newTokenId, tokenIds);
    }
}
