...
contract HIREX is Context, IBEP20, Ownable {
    ...
    uint256 public _buyMaxTxAmount    =    3000000 * 10**9; // 0.2% max buy tx
    uint256 public _sellMaxTxAmount   =    1500000 * 10**9; // 0.01% max sell tx
    uint256 private _otherMaxTxAmount = 1000000000 * 10**9; // total supply
   
    uint256 private numTokensSellToAddToLiquidity = 1000000 * 10**9; // this tx amount will trigger swap and add liquidity
    ...
    function setMaxTxPercent(uint256 maxBuyTxPercent, uint256 maxSellTxPercent, uint256 maxOtherTxPercent) external onlyOwner() {
        require(maxBuyTxPercent > 0.000, "% must be greater than 0.000");
        require(maxSellTxPercent > 0.000, "% must be greater than 0.000");
        require(maxOtherTxPercent > 0.000, "% must be greater than 0.000");
        _buyMaxTxAmount = _tTotal.mul(maxBuyTxPercent).div(10**2);
        _sellMaxTxAmount = _tTotal.mul(maxSellTxPercent).div(10**2);
        _otherMaxTxAmount = _tTotal.mul(maxOtherTxPercent).div(10**2);
    }   
    ...
    function _transfer(
        ...
        if(!(_isExcludeAddressFromMaxTx[from] || _isExcludeAddressFromMaxTx[to])) {
            ...
            require(balanceOf(to).add(amount) <= _maxTxAmount, "Already bought maxTxAmount, wait till check off");          
		}        
