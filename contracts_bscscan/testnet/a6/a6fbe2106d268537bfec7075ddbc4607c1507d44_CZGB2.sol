...
contract CZGB2 is Context, IERC20, Ownable {
    ...
    string private _name = "CAESARZILLA";
    string private _symbol = "CZILLA";
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateLiquidityPlustreasuryFee(tAmount);
    ...
    function calculateLiquidityPlustreasuryFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _treasuryFee + _marketingBudget).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingBudget == 0 && _treasuryFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _marketingBudget + _liquidityFee + _treasuryFee;
        ...
        // calculate the portions of the liquidity to add to treasury
        ...
        uint256 finalBalance = newBalance.sub(treasuryBalance).sub(marketingBalance);
        uint256 finalHalf = otherHalf.sub(treasuryPortion).sub(marketingPortion);
        
        
        
        (bool sent, bytes memory data) = _treasuryAddress.call{value: treasuryBalance}("");
        if(sent){
            _tokenTransfer(address(this), 0x0000000000000000000000000000000000000001, treasuryPortion, false);
            emit Transfer(address(this), 0x0000000000000000000000000000000000000001, treasuryPortion);
        } else {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount, address treasury) private {
