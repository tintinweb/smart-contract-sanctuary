...
contract MagicLampWallet is ERC165, IERC1155Receiver, IERC721Receiver, Wallet {
     ...
     */
    function withdrawERC20(address nft, uint256 id, address tokens, uint256 amounts)
    public  {
        ...
        //for (uint256 i = 0; i < tokens.length; i++) {
            IERC20 token = IERC20(tokens);

            token.transfer(to, amounts);

            _subTokenBalance(nft, id, TOKEN_TYPE_ERC20, tokens, amounts);

            emit MagicLampWalletERC20Withdrawn(_msgSender(), nft, id, tokens, amounts, to);
        //}
    }
    ...
    function depositERC721(address nft, uint256 id, address token, uint256[] memory tokenIds) external  {
        ...
        _addTokenBalance(nft, id, TOKEN_TYPE_ERC721, token, tokenIds.length);
    }

    /**
     * @dev Withdraws ERC721 token from wallet.
     */
    function withdrawERC721(address nft, uint256 id, address token, uint256 tokenIds)
    public {
        onlyWalletOwner(nft, id);
        unlocked(nft, id);
        IERC721 iToken = IERC721(token);
        ...
        //for (uint256 i = 0; i < tokenIds.length; i++) {
            require(iToken.ownerOf(tokenIds) == address(this));

            iToken.safeTransferFrom(address(this), to, tokenIds);

            _popTokenId(nft, id, token, tokenIds);

            emit MagicLampWalletERC721Withdrawn(_msgSender(), nft, id, token, tokenIds, to);
        //}
        _subTokenBalance(nft, id, TOKEN_TYPE_ERC721, token, tokenIds);
    }
    ...
    public {
       ...
       // }
        _addTokenBalance(toNFT, toId, TOKEN_TYPE_ERC721, token, 1);
        _subTokenBalance(fromNFT, fromId, TOKEN_TYPE_ERC721, token, 1);
    }
     ...
     */
    function withdrawERC1155(address nft, uint256 id, address token, uint256 tokenId, uint256 amount)
    public {
        onlyWalletOwner(nft, id);
        unlocked(nft, id);
        IERC1155 iToken = IERC1155(token);
    ...
    function withdrawAll(address nft, uint256 id, address to) external  {
       ...
       // require(erc20Addresses.length > 0 && erc20Addresses.length == erc20Balances.length, "MagicLampWallet::withdrawERC20: invalid parameters");
        for(uint256 i = 0; i < erc20Addresses.length; i++){
            withdrawERC20(nft, id, erc20Addresses[i], erc20Balances[i]);
        }
        ...
        for (uint256 a = 0; a < erc721Addresses.length; a++) {
            ...
            for(uint256 b = 0; b < ids.length; b++){
                withdrawERC721(nft, id, erc721Addresses[a], ids[b]);
            }
        }
        ...
        for (uint256 a = 0; a < erc1155Addresses.length; a++) {
            ...
            for(uint256 i= 0; i < tokenBalances.length; i++){
                withdrawERC1155(nft, id, erc1155Addresses[a], ids[i], tokenBalances[i]);
            }
        }
    }
    ...
    function transferAll(address fromNFT,uint256 fromId,address toNFT,uint256 toId) external{
        ...
        //require(erc20Addresses.length > 0 && erc20Addresses.length == erc20Balances.length, "MagicLampWallet::transferERC20: invalid parameters");
        for(uint256 i = 0; i < erc20Addresses.length; i++){
        ...
        for (uint256 a = 0; a < erc721Addresses.length; a++) {
            ...
            //require(ids.length > 0, "MagicLampWallet::transferERC721: invalid parameters");
            for(uint256 b = 0; b < ids.length; b++){
