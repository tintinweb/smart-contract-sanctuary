...
contract RainiStakingPool is AccessControl, ReentrancyGuard {
  ...
  event RainbowToBnbSet(uint256 rainbowToBnb);
  ...
  function setRainbowToBnb(uint256 _rainbowToBnb)
    external onlyOwner {
      rainbowToBnb = _rainbowToBnb;
      
      emit RainbowToBnbSet(_rainbowToBnb);
  }
  ...
  function buyRainbow(uint256 _amount) 
    external payable {
      ...
      require(msg.value.mul(rainbowToBnb) >= _amount, "RainiStakingPool: not enough bnb");

      balances[_msgSender()] = balances[_msgSender()].add(_amount);

      uint256 refund = msg.value.sub(_amount.div(rainbowToBnb));
      if(refund > 0) {
