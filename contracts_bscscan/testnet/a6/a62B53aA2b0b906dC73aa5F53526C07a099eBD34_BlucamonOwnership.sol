...
abstract contract BlucamonFactory is BlucaDependency {
    ...
    mapping(uint256 => address) blucamonToOwner;
    mapping(address => uint256[]) ownerToBlucamons;
    ...
    function spawnBlucamonEgg(uint256 _id) internal {
        ...
        ownerToBlucamons[msg.sender].push(_id);
...
contract BlucamonOwnership is ERC721URIStorage, BlucamonAirdrop {
    ...
    function mintBlucamon(address _address, string memory _tokenUri) external onlyWhitelistSetter{
        blucamons.push(Blucamon(true));
        blucamonId = blucamonId++;
        indexMapping[blucamonId] = blucamons.length - 1;
        ownerToBlucamons[_address].push(blucamonId);
        blucamonToOwner[blucamonId] = _address;
