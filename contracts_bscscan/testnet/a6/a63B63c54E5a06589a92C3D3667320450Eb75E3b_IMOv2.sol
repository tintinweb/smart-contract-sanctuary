...
abstract contract IMO is IIMO, Initializable, OwnableUpgradeable{
    ...
    function setDepositWrapFee(uint256 fee) public onlyOwner{
        depositWrapFee = fee;
    }
    ...
    function setWithdrawWrapFee(uint256 fee) public onlyOwner{
        withdrawWrapFee = fee;
    }
    ...
    function getImo(uint256 imoIndex) public view returns(IMOClonable){
    ...
    function newImoFactory() public onlyOwner returns(IMOFactory){
        _factory = new IMOFactory();
        return _factory;
    }
    ...
    function commit(uint256 imoIndex, uint256 amount) public{
        ...
        IMOClonable imo = getImo(imoIndex);
...
contract IMOFactory is Initializable, OwnableUpgradeable{
    ...
    function createIMO(
        ...
        require(_hardCap.length == 5, "Invalid hardcap array length.");
...
contract IMOClonable is Initializable, OwnableUpgradeable{
    ...
    function initialize(
        ...
        hardCap.push(_hardCap[0]);
        hardCap.push(_hardCap[1]);
        hardCap.push(_hardCap[2]);
        hardCap.push(_hardCap[3]);
        hardCap.push(_hardCap[4]);
    }
    ...
    function setHardCap(uint256[] memory _hardCap) public onlyOwner{
        ...
        for(uint32 i=0;i<_hardCap.length;i++){
            if(hardCap.length < (i+1))
                hardCap.push(_hardCap[i]);
            else
