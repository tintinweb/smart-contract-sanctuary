...
contract BuniUniversal is Initializable, AccessControlUpgradeable {
    ...
    // durability in 1 stars
    uint8 durabilityCostFight;

    mapping(uint8 => int128) private _burReforgeTrainerFees;
    int128[8] private _burReforgeBunicornFees;

    int128 public mintBunicornFeeBur;

    int128 public mintTrainerFeeBur;

    bool public isEmergency;

    event BattleOutcome(address indexed owner, uint256 indexed trainer, uint256 bunicorn, uint32 enemy, uint24 playerRoll, uint24 enemyRoll, uint16 xpGain, uint256 burGain);
    ...
    function unpackPlayerData(uint96 playerData)
        public pure returns (uint8 trainerElement, uint24 basePowerLevel, uint64 timestamp) {
    ...
    function getBunicornBattleData(uint256 bunicorn, uint8 trainerElement, uint8 battleMultiplier) internal returns(
        int128 bunicornMultTarget,
        int128 bunicornMultBattle,
        uint24 bunicornBonusPower,
        uint8 bunicornTrait
    ) {
        return bunicorns.getBattleDataAndDrainDurability(bunicorn, trainerElement,
            (durabilityCostFight - bunicorns.getStars(bunicorn)) * battleMultiplier);
    }

    function battle(uint256 trainer, uint256 bunicorn, uint32 enemy, uint8 battleMultiplier)
        external
        noEmergency
        battleCheck(trainer, bunicorn) {

        require(battleMultiplier >= 1 && battleMultiplier <= 5);

        (uint8 trainerTrait, uint24 basePowerLevel, uint64 timestamp) =
            unpackPlayerData(trainers.getBattleDataAndDrainStamina(trainer, staminaCostFight * battleMultiplier));

        (int128 bunicornMultTarget,
            int128 bunicornMultBattle,
            uint24 bunicornBonusPower,
            uint8 bunicornTrait) = getBunicornBattleData(bunicorn, trainerTrait, battleMultiplier);
        ...
        performBattle(
            trainer,
            bunicorn,
            getPlayerPower(basePowerLevel, bunicornMultBattle, bunicornBonusPower),
            uint24(trainerTrait | (uint24(bunicornTrait) << 8) | (enemy & 0xFF000000) >> 8),
    ...
    function checkEnemy(
        ...
        uint32[4] memory targets = getEnemiesInternal(
    ...
    function performBattle(
        ...
        uint24 enemyPower,
        uint8 battleMultiplier
    ) private {
        uint256 seed = uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), msg.sender, nonce)));
        uint24 playerRoll = getPlayerPowerRoll(playerBattlePower,traitsCWE,seed);
        uint24 enemyRoll = getEnemyPowerRoll(enemyPower, RandomUtil.combineSeeds(seed,1));

        uint16 xp = getXpGainForBattle(playerBattlePower, enemyPower) * battleMultiplier;
        // reward by buni universal reward token
        uint256 tokens = usdToBur(getTokenGainForBattle(enemyPower, battleMultiplier));
        ...
        emit BattleOutcome(msg.sender, trainer, bunicorn, (enemyPower | ((uint32(traitsCWE) << 8) & 0xFF000000)), playerRoll, enemyRoll, xp, tokens);
    }
    ...
    function getTokenGainForBattle(uint24 enemyPower, uint8 battleMultiplier) internal view returns (int128) {
        return burFightRewardGasOffset.add(
            burFightRewardBaseline.mul(
                ABDKMath64x64.sqrt(
                    // Performance optimization: 1000 = getPowerAtLevel(0)
                    ABDKMath64x64.divu(enemyPower, 1000)
                )
            ).mul(ABDKMath64x64.fromUInt(battleMultiplier))
        );
    }

    function getXpGainForBattle(uint24 playerPower, uint24 enemyPower) internal view returns (uint16) {
        return uint16(ABDKMath64x64.divu(enemyPower, playerPower).mulu(fightXpGain));
    }
    ...
    function getPlayerPowerRoll(
        ...
        return uint24(getPlayerElementBonusForBattle(traitsCWE).mulu(playerPower));
    }

    function getEnemyPowerRoll(uint24 enemyPower, uint256 seed) internal pure returns(uint24) {
        // roll for battles
    ...
    function getPlayerElementBonusForBattle(uint24 traitsCWE) public view returns (int128) {
        int128 traitBonus = oneFrac;
        uint8 trainerElement = uint8(traitsCWE & 0xFF);
        if(trainerElement == (traitsCWE >> 8) & 0xFF/*bunicornTrait*/) {
            traitBonus = traitBonus.add(fightTraitBonus);
        }
        if(isElementEffective(trainerElement, uint8(traitsCWE >> 16)/*enemy*/)) {
            traitBonus = traitBonus.add(fightTraitBonus);
        }
        else if(isElementEffective(uint8(traitsCWE >> 16)/*enemy*/, trainerElement)) {
    ...
    function getEnemies(uint256 trainer, uint256 bunicorn) public view returns (uint32[4] memory) {
        (int128 bunicornMultTarget,,
            uint24 bunicornBonusPower,
            ) = bunicorns.getBattleData(bunicorn, trainers.getTrait(trainer));

        return getEnemiesInternal(
    ...
    function getEnemiesInternal(uint24 playerPower,
        uint64 staminaTimestamp,
        uint256 currentHour
    ...
    function isElementEffective(uint8 attacker, uint8 defender) public pure returns (bool) {
    ...
    function mintTrainer()
        ...
        requestPayBurFromPlayer(mintTrainerFeeBur) {
        ...
        _payBurContract(msg.sender, mintTrainerFeeBur);
    ...
    function fuseTrainer(uint256 fusionID, uint256 burnID)
        public
        noEmergency
        isTrainerOwner(fusionID)
        isTrainerOwner(burnID)
        requestPayBurFromPlayer(calculateFuseTrainerFee(fusionID))
    {
        _payBurContract(msg.sender, calculateFuseTrainerFee(fusionID));
        trainers.fusion(fusionID, burnID);
        nonce = nonce + 1;
    }

    function calculateFuseTrainerFee(uint256 id) public view returns(int128 fee) {
        uint8 trainerLevel = trainers.getLevel(id);
        uint8[] memory milestones = trainers.getFusionMilestones();
    ...
    function mintBunicornWithBur()
        ...
        onlyNonContract
        oncePerBlock(msg.sender)
        requestPayBurFromPlayer(mintBunicornFeeBur)
    {
        _payBurContract(msg.sender, mintBunicornFeeBur);
    ...
    function enhanceBunicorn(uint256 enhanceID, uint256 burnID)
        ...
        requestPayBurFromPlayer(calculateEnhanceBunicornFee(burnID))
    {
        _payBurContract(msg.sender, calculateEnhanceBunicornFee(burnID));
        bunicorns.enhance(enhanceID, burnID);
        nonce = nonce + 1;
    }
    ...
    function migrateRandoms(IRandoms _newRandoms) external noEmergency {
    ...
    function approveContractBunicornFor(uint256 bunicornID, address playerAddress) public restricted noEmergency {
        _approveContractBunicornFor(bunicornID, playerAddress);
    }

    function payContractTokenOnly(address playerAddress, uint256 convertedAmount) public restricted noEmergency {
    ...
    function setTrainerMintValue(uint256 buniCents, uint256 burCents) public restricted noEmergency {
        mintTrainerFee = ABDKMath64x64.divu(buniCents, 100);
        mintTrainerFeeBur = ABDKMath64x64.divu(burCents, 100);
    }

    function setBattleRewardBaselineValue(uint256 tenthcents) public restricted noEmergency {
        burFightRewardBaseline = ABDKMath64x64.divu(tenthcents, 1000); // !!! THIS TAKES TENTH OF CENTS !!!
    }

    function setBattleRewardGasOffsetValue(uint256 cents) public restricted noEmergency {
        burFightRewardGasOffset = ABDKMath64x64.divu(cents, 100);
    }

    function setBunicornMintValue(uint256 cents) public restricted noEmergency {
        mintBunicornFee = ABDKMath64x64.divu(cents, 100);
    }

    function setBunicornMintValueWithBUur(uint256 cents) public restricted noEmergency {
        mintBunicornFeeBur = ABDKMath64x64.divu(cents, 100);
    }
    ...
    function setFuseTrainerFeeValues(uint256[] memory cents) public restricted noEmergency {
        uint8[] memory milestones = trainers.getFusionMilestones();
    ...
    function getEnhanceBunicornFees() external view returns(int128[8] memory fees) {
        for (uint8 i = 0; i < _burReforgeBunicornFees.length; i++) {
            fees[i] = _burReforgeBunicornFees[i];
        }
    }

    function setEnhanceBunicornFeeValues(uint256[8] memory cents) public restricted noEmergency {
        for (uint8 i = 0; i < 8; i++) {
            _burReforgeBunicornFees[i] = ABDKMath64x64.divu(cents[i], 100);
        }
    }

    function setStaminaCostBattle(uint8 points) public restricted noEmergency {
        staminaCostFight = points;
    }

    function setDurabilityCostBattle(uint8 points) public restricted noEmergency {
        durabilityCostFight = points;
    }

    function setBattleXpGain(uint256 average) public restricted noEmergency {
        fightXpGain = average;
    }

    function setTrainerLimit(uint256 max) public restricted noEmergency {
    ...
    function claimXpRewards() public noEmergency {
    ...
    function getDurabilityCostBattle() public view returns(uint8) {
        return durabilityCostFight;
    }
    ...
    // =========================== MIGRATOR ===========================
    // TODO: Revive this later, once everything of v2 is ok
    // For now we don't remove old data yet
    // function burnTokenRewardsByMigrator(address playerAddress) external onlyMigratorContract {
    //     burRewards[playerAddress] = 0;
    //     _burRewardsClaimTaxTimerStart[playerAddress] = 0;
    // }

    // function burnXpRewardsByMigrator(uint256 trainer) external onlyMigratorContract {
    //     xpRewards[trainer] = 0;
    // }
    // =========================== MIGRATOR ===========================
...
contract Trainers is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    */
    ...
    function setEmergency(bool _isEmergency) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "trainer: not admin");
        isEmergency = _isEmergency;
    }
    ...
    function _restricted() internal view {
        require(hasRole(GAME_ADMIN, msg.sender), "trainer: not game admin");
    }
    ...
    function _noEmergency() internal view {
        require(!isEmergency, "trainer: emergency pause");
    }
    ...
    function mint(address minter, uint256 seed) public restricted noEmergency {
    ...
    function mintTrainerWithElement(address minter, uint256, uint8 element) public restricted noEmergency {
        uint256 tokenID = tokens.length;
    ...
    function setTrait(uint256 id, uint8 trait) public restricted noEmergency {
    ...
    function gainXp(uint256 id, uint16 xp) public restricted noEmergency {
    ...
    function setStaminaTimestamp(uint256 id, uint64 timestamp) public restricted noEmergency {
    ...
    function getBattleDataAndDrainStamina(uint256 id, uint8 amount) public restricted noEmergency returns(uint96) {
        Trainer storage trainer = tokens[id];
    ...
    function fusion(uint256 fusionID, uint256 burnID) public restricted noEmergency {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal noEmergency override {
    ...
    // We still do not burn token for now
    // TODO: revive this function and burn trainers when everything of v2 is ok
...
contract Bunicorns is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function setEmergency(bool _isEmergency) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "bunicorn: not admin");
        isEmergency = _isEmergency;
    }
    ...
    */
    ...
    function _restricted() internal view {
        require(hasRole(GAME_ADMIN, msg.sender), "bunicorn: not game admin");
    }
    ...
    function _onlyMigratorContract() internal view {
        require(hasRole(ROLE_MIGRATOR_CONTRACT, msg.sender), "bunicorn: not migrator");
    }
    ...
    function _noEmergency() internal view {
        require(!isEmergency, "bunicorn: emergency pause");
    }
    ...
    function mint(address minter, uint256 seed) public restricted noEmergency returns(uint256) {
        uint256 stars;
    ...
    function mintBunicornByElement(address minter, uint256 seed, uint256 element) public restricted noEmergency returns(uint256) {
        uint256 stars;
    ...
    function mintBunicornWithStars(address minter, uint256 stars, uint256 seed) public restricted noEmergency returns(uint256) {
    ...
    function mintBunicornByElementWithStars(address minter, uint256 stars, uint256 seed, uint256 element) public restricted noEmergency returns(uint256) {
        require(stars < 8, "Stars parameter too high! (max 7)");
        (uint16 stat1, uint16 stat2, uint16 stat3) = getStatRolls(stars, seed);
    ...
    function enhance(uint256 enhanceID, uint256 burnID) public restricted noEmergency {
    ...
    function getBattleDataAndDrainDurability(uint256 id, uint8 trainerTrait, uint8 drainAmount) public
        restricted noEmergency noFreshLookup(id)
    ...
    function drainDurability(uint256 id, uint8 amount) public restricted noEmergency {
        uint8 durabilityPoints = getDurabilityPointsFromTimestamp(durabilityTimestamp[id]);
    ...
    function setBurnPointMultiplier(uint256 multiplier) public restricted noEmergency {
        burnPointMultiplier = multiplier;
    }
    function setLowStarBurnPowerPerPoint(uint256 powerPerBurnPoint) public restricted noEmergency {
        lowStarBurnPowerPerPoint = powerPerBurnPoint;
    }
    function setFourStarBurnPowerPerPoint(uint256 powerPerBurnPoint) public restricted noEmergency {
        fourStarBurnPowerPerPoint = powerPerBurnPoint;
    }
    function setFiveStarBurnPowerPerPoint(uint256 powerPerBurnPoint) public restricted noEmergency {
    ...
    function setDurabilityTimestamp(uint256 id, uint64 timestamp) public restricted noEmergency {
    ...
    // We still do not burn the token at this moment
    // TODO: revive this later, after everything of v2 is ok
    // function burnByMigrator(uint256 _tokenId) external onlyMigratorContract {
    //     _burn(_tokenId);
    // }
    // =========================== MIGRATOR ===========================
