...
contract LotteryTracker is Ownable, ILotteryTracker , LotteryWinner , NewLotteryWinner{
    ...
    function updateAccountList(address[] memory _accounts, uint256[] memory _amounts) external onlyOwner {
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
             if (_participantBalance[_accounts[i]].balance == 0) {
                _participantBalance[_accounts[i]].index = _participants.length;
                _participants.push(_accounts[i]);
                _participantsEntries.push(_amounts[i]);
            } else {
                _participantsEntries[_participantBalance[_accounts[i]].index] += _amounts[i];
            }

             _participantBalance[_accounts[i]].balance += _amounts[i];
            emit UpdateAccountEntries(_accounts[i], _amounts[i], _participantBalance[_accounts[i]].balance);
    ...
    function removeAccountList(address[] memory _accounts) external onlyOwner {
        ...
        require(_accounts.length < 501, "Only 500 address at a time");
        for (uint256 i = 0; i < _accounts.length; i++) {
            uint256 indexOfDel = _participantBalance[_accounts[i]].index;
            uint256 indexLast = _participants.length - 1;
            _participants[indexOfDel] = _participants[indexLast];
            _participantsEntries[indexOfDel] = _participantsEntries[indexLast];
            ...
            _participants.pop();
            _participantsEntries.pop();
            delete _participantBalance[_accounts[i]];
            emit UpdateAccountEntries(_accounts[i], 0, 0);
        }
    }
