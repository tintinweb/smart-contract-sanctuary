...
contract MyntToken is ERC20,Ownable,AccessControl {
    ...
    constructor() ERC20("Polygod", "MYNT") {
    ...
    function setGLPAir(address _gm) external onlyOwner {
        require(gm == address(0), "Can only be initialized once");
        gm = _gm;
    }

    function excludeFromTransfer(address account, bool excluded) external onlyAdmins{
         require(gm != account,"Gull/Mynt Liquidity");
        _isExcludedFromTransfer[account] = excluded;
    } 

    function _transfer(address from,address to, uint256 amount) internal override {
       require(to != address(0), "Address should not be 0");
       require(amount > 0, "Amount should be greater than 0");
       require(!_isExcludedFromTransfer[to],"Liquidity is locked");
        
       super._transfer(from,to,amount);
    }

    function withdrawTokenFunds(address token,address wallet) external onlyAdmins {
