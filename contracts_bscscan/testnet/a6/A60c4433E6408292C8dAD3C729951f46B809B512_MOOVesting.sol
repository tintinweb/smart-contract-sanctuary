...
contract MOOVesting is AccessControl {
  ...
  function queueEmergencyWithdraw() external onlyRole(DEFAULT_ADMIN_ROLE) {
    // Emergency
    require(
      block.timestamp <= tgeTimestamp || // emergencyWithdraw can be executed before TGE
        block.timestamp >= tgeTimestamp + (30 * 41 days), // emergencyWithdraw can be also executed after 41 months
      "MOOVesting: Emergency withdraw should before TGE or 41 months after TGE"
    );
