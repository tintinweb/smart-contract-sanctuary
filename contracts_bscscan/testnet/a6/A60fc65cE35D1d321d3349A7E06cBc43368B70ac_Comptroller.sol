...
contract Comptroller is ComptrollerV1Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    event AppleSpeedUpdated(AToken indexed aToken, uint newSpeed);
    event DistributedSupplierApple(AToken indexed aToken, address indexed supplier, uint appleDelta, uint appleSupplyIndex);
    event DistributedBorrowerApple(AToken indexed aToken, address indexed borrower, uint appleDelta, uint appleBorrowIndex);
    ...
    uint224 public constant appleInitialIndex = 1e36;
    ...
    modifier onlyListedMarket(AToken aToken) {
        require(markets[address(aToken)].isListed, "apple market is not listed");
        _;
    }
    ...
    function mintAllowed(address aToken, address minter, uint mintAmount) external onlyProtocolAllowed returns (uint) {
        ...
        updateAppleSupplyIndex(aToken);
        distributeSupplierApple(aToken, minter);
        return uint(Error.NO_ERROR);
    }
    ...
    function redeemAllowed(address aToken, address redeemer, uint redeemTokens) external onlyProtocolAllowed returns (uint) {
        ...
        updateAppleSupplyIndex(aToken);
        distributeSupplierApple(aToken, redeemer);
        return uint(Error.NO_ERROR);
    }
    ...
    function borrowAllowed(address aToken, address borrower, uint borrowAmount) external onlyProtocolAllowed returns (uint) {
        ...
        updateAppleBorrowIndex(aToken, borrowIndex);
        distributeBorrowerApple(aToken, borrower, borrowIndex);
        return uint(Error.NO_ERROR);
    }
    ...
    function liquidateBorrowAllowed(
        ...
        uint repayAmount) external onlyProtocolAllowed returns (uint) {
        liquidator;
        if (!markets[aTokenCollateral].isListed) {
            return uint(Error.MARKET_NOT_LISTED);
        }
    ...
    function seizeAllowed(
        ...
        updateAppleSupplyIndex(aTokenCollateral);
        distributeSupplierApple(aTokenCollateral, borrower);
        distributeSupplierApple(aTokenCollateral, liquidator);
        return uint(Error.NO_ERROR);
    }
    ...
    function transferAllowed(address aToken, address src, address dst, uint transferTokens) external onlyProtocolAllowed returns (uint) {
        ...
        updateAppleSupplyIndex(aToken);
        distributeSupplierApple(aToken, src);
        distributeSupplierApple(aToken, dst);
        return uint(Error.NO_ERROR);
    }
    ...
    function _supportMarket(AToken aToken) external returns (uint) {
        ...
        markets[address(aToken)] = Market({isListed: true, isApple: false, collateralFactorMantissa: 0});
    ...
    function setAppleSpeedInternal(AToken aToken, uint appleSpeed) internal {
        uint currentAppleSpeed = appleSpeeds[address(aToken)];
        if (currentAppleSpeed != 0) {
            // note that APPLE speed could be set to 0 to halt liquidity rewards for a market
            Exp memory borrowIndex = Exp({mantissa: aToken.borrowIndex()});
            updateAppleSupplyIndex(address(aToken));
            updateAppleBorrowIndex(address(aToken), borrowIndex);
        } else if (appleSpeed != 0) {
            // Add the APPLE market
            Market storage market = markets[address(aToken)];
            require(market.isListed == true, "apple market is not listed");
            if (appleSupplyState[address(aToken)].index == 0 && appleSupplyState[address(aToken)].block == 0) {
                appleSupplyState[address(aToken)] = AppleMarketState({
                    index: appleInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }
        if (appleBorrowState[address(aToken)].index == 0 && appleBorrowState[address(aToken)].block == 0) {
                appleBorrowState[address(aToken)] = AppleMarketState({
                    index: appleInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }
        }
        if (currentAppleSpeed != appleSpeed) {
            appleSpeeds[address(aToken)] = appleSpeed;
            emit AppleSpeedUpdated(aToken, appleSpeed);
        }
    }
    function updateAppleSupplyIndex(address aToken) internal {
        AppleMarketState storage supplyState = appleSupplyState[aToken];
        uint supplySpeed = appleSpeeds[aToken];
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, uint(supplyState.block));
        if (deltaBlocks > 0 && supplySpeed > 0) {
            uint supplyTokens = AToken(aToken).totalSupply();
            uint appleAccrued = mul_(deltaBlocks, supplySpeed);
            Double memory ratio = supplyTokens > 0 ? fraction(appleAccrued, supplyTokens) : Double({mantissa: 0});
            Double memory index = add_(Double({mantissa: supplyState.index}), ratio);
            appleSupplyState[aToken] = AppleMarketState({
    ...
    function updateAppleBorrowIndex(address aToken, Exp memory marketBorrowIndex) internal {
        AppleMarketState storage borrowState = appleBorrowState[aToken];
        uint borrowSpeed = appleSpeeds[aToken];
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, uint(borrowState.block));
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            uint borrowAmount = div_(AToken(aToken).totalBorrows(), marketBorrowIndex);
            uint appleAccrued = mul_(deltaBlocks, borrowSpeed);
            Double memory ratio = borrowAmount > 0 ? fraction(appleAccrued, borrowAmount) : Double({mantissa: 0});
            Double memory index = add_(Double({mantissa: borrowState.index}), ratio);
            appleBorrowState[aToken] = AppleMarketState({
                index: safe224(index.mantissa, "new index overflows"),
                block: safe32(blockNumber, "block number overflows")
            });
        } else if (deltaBlocks > 0) {
    ...
    function distributeSupplierApple(address aToken, address supplier) internal {
        AppleMarketState storage supplyState = appleSupplyState[aToken];
        Double memory supplyIndex = Double({mantissa: supplyState.index});
        Double memory supplierIndex = Double({mantissa: appleSupplierIndex[aToken][supplier]});
        appleSupplierIndex[aToken][supplier] = supplyIndex.mantissa;
        if (supplierIndex.mantissa == 0 && supplyIndex.mantissa > 0) {
            supplierIndex.mantissa = appleInitialIndex;
        }
        Double memory deltaIndex = sub_(supplyIndex, supplierIndex);
        uint supplierTokens = AToken(aToken).balanceOf(supplier);
        uint supplierDelta = mul_(supplierTokens, deltaIndex);
        uint supplierAccrued = add_(appleAccrued[supplier], supplierDelta);
        appleAccrued[supplier] = supplierAccrued;
        emit DistributedSupplierApple(AToken(aToken), supplier, supplierDelta, supplyIndex.mantissa);
    }

    function distributeBorrowerApple(address aToken, address borrower, Exp memory marketBorrowIndex) internal {
        AppleMarketState storage borrowState = appleBorrowState[aToken];
        Double memory borrowIndex = Double({mantissa: borrowState.index});
        Double memory borrowerIndex = Double({mantissa: appleBorrowerIndex[aToken][borrower]});
        appleBorrowerIndex[aToken][borrower] = borrowIndex.mantissa;
        if (borrowerIndex.mantissa > 0) {
            Double memory deltaIndex = sub_(borrowIndex, borrowerIndex);
            uint borrowerAmount = div_(AToken(aToken).borrowBalanceStored(borrower), marketBorrowIndex);
            uint borrowerDelta = mul_(borrowerAmount, deltaIndex);
            uint borrowerAccrued = add_(appleAccrued[borrower], borrowerDelta);
            appleAccrued[borrower] = borrowerAccrued;
            emit DistributedBorrowerApple(AToken(aToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }
    function claimApple(address holder) public {
        return claimApple(holder, allMarkets);
    }
    function claimApple(address holder, AToken[] memory aTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimApple(holders, aTokens, true, true);
    }
    function claimApple(address[] memory holders, AToken[] memory aTokens, bool borrowers, bool suppliers) public {
        uint j;
        for (j = 0; j < holders.length; j++) {
            appleAccrued[holders[j]] = grantAPPLEInternal(holders[j], appleAccrued[holders[j]]);
        }
        for (uint i = 0; i < aTokens.length; i++) {
            ...
            if (borrowers) {
                ...
                updateAppleBorrowIndex(address(aToken), borrowIndex);
                for (j = 0; j < holders.length; j++) {
                    distributeBorrowerApple(address(aToken), holders[j], borrowIndex);
                    appleAccrued[holders[j]] = grantAPPLEInternal(holders[j], appleAccrued[holders[j]]);
                }
            }
            if (suppliers) {
                updateAppleSupplyIndex(address(aToken));
                for (j = 0; j < holders.length; j++) {
                    distributeSupplierApple(address(aToken), holders[j]);
                    appleAccrued[holders[j]] = grantAPPLEInternal(holders[j], appleAccrued[holders[j]]);
                }
            }
        }
    }
    function grantAPPLEInternal(address user, uint amount) internal returns (uint) {
        APPLE apple = APPLE(getAppleAddress());
        uint appleRemaining = apple.balanceOf(address(this));
        if (amount > 0 && amount <= appleRemaining) {
            apple.transfer(user, amount);
            return 0;
        }
        return amount;
    }
    /*** Apple Distribution Admin ***/
    function _setAppleSpeed(AToken aToken, uint appleSpeed) public {
        require(adminOrInitializing(), "only admin can set apple speed");
        setAppleSpeedInternal(aToken, appleSpeed);
    }
    ...
    function getAppleAddress() public view returns (address) {
        // APPLE
        return 0x3e3183DF92E9D93b36DD290810454e569693EfEE;
    }
}
...
contract AToken is ATokenInterface, Exponential, TokenErrorReporter {
    ...
    /**
     * @notice Sender supplies assets into the market and receiver receives aTokens in exchange
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param receiver The address of the account which is receiving the aTokens
     * @param mintAmount The amount of the underlying asset to supply
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    ...
    /**
     * @notice Payer supplies assets into the market and receiver receives aTokens in exchange
     * @dev Assumes interest has already been accrued up to the current block
     * @param payer The address of the account which is paying the underlying token
     * @param receiver The address of the account which is receiving aToken
     * @param mintAmount The amount of the underlying asset to supply
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    function mintBehalfFresh(address payer, address receiver, uint mintAmount) internal returns (uint, uint) {
        ...
        /*
         * We calculate the new total supply of aTokens and receiver token balance, checking for overflow:
         *  totalSupplyNew = totalSupply + mintTokens
...
contract ComptrollerV1Storage is UnitrollerAdminStorage {
    ...
    struct Market {
        ...
        /// @notice Whether or not this market receives APPLE
        bool isApple;
    }
    ...
    struct AppleMarketState {
        /// @notice The market's last updated appleBorrowIndex or appleSupplyIndex
        uint224 index;
    ...
    /// @notice The rate at which the flywheel distributes APPLE, per block
    uint public appleRate;

    /// @notice The portion of appleRate that each market currently receives
    mapping(address => uint) public appleSpeeds;

    /// @notice The Apple market supply state for each market
    mapping(address => AppleMarketState) public appleSupplyState;

    /// @notice The Apple market borrow state for each market
    mapping(address => AppleMarketState) public appleBorrowState;

    /// @notice The Apple supply index for each market for each supplier as of the last time they accrued APPLE
    mapping(address => mapping(address => uint)) public appleSupplierIndex;

    /// @notice The Apple borrow index for each market for each borrower as of the last time they accrued APPLE
    mapping(address => mapping(address => uint)) public appleBorrowerIndex;

    /// @notice The APPLE accrued but not yet transferred to each user
    mapping(address => uint) public appleAccrued;
    ...
    /// @notice The portion of APPLE that each contributor receives per block
    mapping(address => uint) public appleContributorSpeeds;

    /// @notice Last block at which a contributor's APPLE rewards have been allocated
...
contract APPLE is Tokenlock {
    /// @notice BEP-20 token name for this token
    string public constant name = "Apple Coin";
    ...
    uint public constant totalSupply = 30000000e18; // 30 million APPLE
    ...
    /**
     * @notice Construct a new APPLE token
    ...
    function approve(address spender, uint rawAmount) external validLock returns (bool) {
        ...
        if (rawAmount == uint(-1)) {
            ...
            amount = safe96(rawAmount, "APPLE::approve: amount exceeds 96 bits");
        }
    ...
    function transfer(address dst, uint rawAmount) external validLock returns (bool) {
        uint96 amount = safe96(rawAmount, "APPLE::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external validLock returns (bool) {
        ...
        uint96 amount = safe96(rawAmount, "APPLE::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != uint96(-1)) {
            uint96 newAllowance = sub96(spenderAllowance, amount, "APPLE::transferFrom: transfer amount exceeds spender allowance");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(src != address(0), "APPLE::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "APPLE::_transferTokens: cannot transfer to the zero address");

        balances[src] = sub96(balances[src], amount, "APPLE::_transferTokens: transfer amount exceeds balance");
        balances[dst] = add96(balances[dst], amount, "APPLE::_transferTokens: transfer amount overflows");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "APPLE::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "APPLE::_moveVotes: vote amount overflows");
...
/**
  * @title Apple's InterestRateModel Interface
  * @author Apple
  */
