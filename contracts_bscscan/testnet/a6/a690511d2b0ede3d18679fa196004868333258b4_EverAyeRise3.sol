...
3) Anti-Bot Tax. For the first ten minutes, a 99% tax will apply that slowly decreases over time.
   The tax will drop linearly approximately 1% every 6 seconds until it reaches the floor at 10%.
*/
...
contract EverAyeRise3 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x765cee43Ebe112dc97783cc9D4C8C773af2900ea); // Marketing Address
    ...
    string private _name = "EverAyeRise3";
    string private _symbol = "EverAyeRise3";
    ...
    function _calculateLaunchTax() private view returns (uint256){
        if(block.timestamp>launchTimestamp+BotTaxTime) return _liquidityFee;
        uint256 timeSinceLaunch=block.timestamp-launchTimestamp;
        uint8 tax=uint8(BotMaxTax-((BotMaxTax-_liquidityFee)*timeSinceLaunch/BotTaxTime));
        return tax;
    }
    ...
    function getTaxes() public view returns(uint256 liquidityTax){
        uint256 buyTax;
        if(_botProtection) buyTax=_calculateLaunchTax();
        else buyTax= _liquidityFee;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        //Disable Bot Protection if timer passed.   
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        ...
        if(!_botProtection) return _amount.mul(_liquidityFee).div(10**2);
        if(block.timestamp<(launchTimestamp+BotTaxTime)){
            uint256 tax=_calculateLaunchTax();
