...
contract OlympusStaking is Ownable {
    ...
    /**
        @notice stake OX to enter warmup
        @param _amount uint
        @return bool
     */
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( IsOHM( xOX ).gonsForBalance( _amount ) ),
    ...
    /**
        @notice retrieve xOX from warmup
    ...
    function claim ( address _recipient ) public {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            IWarmup( warmupContract ).retrieve( _recipient, IsOHM( xOX ).balanceForGons( info.gons ) );
        }
    }

    /**
        @notice forfeit xOX in warmup and retrieve OX
     */
    function forfeit() external {
        Claim memory info = warmupInfo[ msg.sender ];
        delete warmupInfo[ msg.sender ];

        IWarmup( warmupContract ).retrieve( address(this), IsOHM( xOX ).balanceForGons( info.gons ) );
    ...
    /**
        @notice returns the xOX index, which tracks rebase growth
        @return uint
     */
    function index() public view returns ( uint ) {
        return IsOHM( xOX ).index();
    }
    ...
    function rebase() public {
        if( epoch.endBlock <= block.number ) {
            ...
            IsOHM( xOX ).rebase( epoch.distribute, epoch.number );
            ...
            uint staked = IsOHM( xOX ).circulatingSupply();
    ...
    /**
        @notice returns contract OX holdings, including bonuses provided
        @return uint
     */
