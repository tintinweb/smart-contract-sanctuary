...
import {StarNFTV1} from "StarNFTV2.sol";

contract StarNFTNaiveFactory {
    mapping(address => mapping(uint => address)) public nftMap; // we allow same address to create NFTs 
    mapping(address => uint) public nftNumber;
    ...
    event NFTCreated(address indexed minter, address indexed creator, address owner, address starAddr, uint);
...
//        manager = _contract_manager;
    ...
    function getOneNFT(address _creator, uint idx) external view returns (address) {
        return nftMap[_creator][idx];
    }

    function getRecentNFT(address _creator) external view returns (address) {
        uint idx = nftNumber[_creator] - 1;
        return nftMap[_creator][idx];
    }

    function createStarNFT(address _minter, address _owner) external onlyNoPaused returns (address starAddr) {
        uint count = nftNumber[_owner];
        bytes32 salt = keccak256(abi.encodePacked(_owner, count));

        bytes memory bytecode = type(StarNFTV1).creationCode;
        ...
        StarNFTV1 snf = StarNFTV1(starAddr);
        // StarNFTV1(point)(cap) share the same ABI
        snf.addMinter(_minter);
        snf.setURI(string(abi.encodePacked(base_uri, "0x", toAsciiString(starAddr), "/")));
        snf.transferOwnership(_owner);

        emit NFTCreated(_minter, msg.sender, _owner, starAddr, allNFTs.length);
