...
contract AIRTStaking is Ownable {
    using SafeMath for uint256;

    address public rtoken;
    address public AIRTAddress;
    ...
    function changeAIRTAddress(address _AIRTAddress) public onlyOwner {
        AIRTAddress = _AIRTAddress;
    }
    ...
    function stake(uint256[] memory tokenIds) public {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(IERC721(AIRTAddress).ownerOf(tokenIds[i]) != msg.sender) continue;

            IERC721(AIRTAddress).transferFrom(address(msg.sender), address(this), tokenIds[i]);
    ...
    function unstake(uint256[] memory tokenIds) public {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            (bool _isStaked,) = getStakingItemInfo(msg.sender, tokenIds[i]);
            if(!_isStaked) continue;
            if(IERC721(AIRTAddress).ownerOf(tokenIds[i]) != address(this)) continue;
            ...
            if(stakingAmount[msg.sender] > 0)
                stakingAmount[msg.sender] = stakingAmount[msg.sender] - 1;
            IERC721(AIRTAddress).transferFrom(address(this), msg.sender, tokenIds[i]);
