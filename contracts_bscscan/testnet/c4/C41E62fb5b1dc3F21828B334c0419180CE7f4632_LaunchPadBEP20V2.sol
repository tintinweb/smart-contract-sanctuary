...
contract LaunchPadBEP20V2 is Ownable, Initializable {
    // Contract libs
        ...
        returns (uint256, uint256)
    {
        ...
        if (info.vestDuration == 0) {
            return (info.amount, launchPadTGE);
        }

        // Vesting has not started yet
        if (block.timestamp < launchPadTGE) {
            return (info.released, info.lastReleasedAt);
        }

        // Vesting is done
        if (block.timestamp >= launchPadTGE.add(info.vestDuration)) {
            return (info.amount, launchPadTGE.add(info.vestDuration));
        }
