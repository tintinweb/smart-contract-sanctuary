...
contract CandleGenieCoinFlip is Ownable, ReentrancyGuard 
{
    ...
    {
        require(Bets[betId][user].amount != 0, "Bet not found");  
        require(Bets[betId][user].paid == false, "Bet already paid");  
          
        Bet storage bet = Bets[betId][user];
    ...
    function Flip(int position) external payable nonReentrant notContract 
    {
        ...
        require(Bets[currentId][msg.sender].amount == 0, "Can only bet after previous bet finished");
        ...
        // Storing Bet
        Bet storage bet = Bets[currentId][msg.sender];
        bet.Id = currentId;
