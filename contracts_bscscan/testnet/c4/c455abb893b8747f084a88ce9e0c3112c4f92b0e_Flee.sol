...
contract ERC20 is Context, IERC20 {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        /*
            NOTE: Mint function in this contract or any KEK rewards contract is necessary for the tokenomics
            to work. It is not intended to mint any tokens except for the rewards tracker and KEK-lps for marketing.
        */
    }
...
contract Flee is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Flee", "KEK") {
    	firstDividendTracker = new FIRSTDividendTracker();
 
    	marketingWallet = 0x7304314748DC184151afe347Ab44D9a40Dfd6f45;
    	firstDividendToken = 0x2B3F34e9D4b127797CE6244Ea341a83733ddd6E4;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "Flee: The marketing wallet is already this address");
    ...
    function updatefirstDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(firstDividendTracker), "Flee: The dividend tracker already has that address");
 
        FIRSTDividendTracker newfirstDividendTracker = FIRSTDividendTracker(payable(newAddress));
 
        require(newfirstDividendTracker.owner() == address(this), "Flee: The new dividend tracker must be owned by the Flee token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "Flee: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "Flee: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Flee: The PanKEKSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "Flee: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "Flee: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "Flee: Trading has not started yet");
...
contract FIRSTDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Flee_Dividend_Tracker", "Flee_Dividend_Tracker", 0x2B3F34e9D4b127797CE6244Ea341a83733ddd6E4) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "Flee_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "Flee_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Flee contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Flee_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Flee_Dividend_Tracker: Cannot update claimWait to same value");
