...
contract EpicAuction is AccessControl {
    ...
    function isExistItem(address item, uint256 tokenId, uint256 amount) public returns(bool){
        for(uint i = 0; i < items.length; i++){
            if(items[i]._owner == msg.sender && items[i]._tokenId == tokenId && items[i]._amount == amount && items[i]._item == item){
                emit ExistItem(items[i]._item, msg.sender, true);
                return true;
            }
        }
        emit ExistItem(address(0), address(0), false);
        return false;
    }
}
