...
contract RoyalWeedCoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RWCDividendTracker public dividendTracker;
    ...
    uint256 public swapTokensAtAmount = 1836900000 * (10**9);
    uint256 public maxBuyAmount = 36738000 * 10**9;
    uint256 public maxSellAmount = 18369000 * 10**9;
    uint256 public maxWalletAmount = 36738000 * 10**9;
    uint256 public buybackLimit = 1 ether;

    address public marketingWallet = address(0x3B77232474f97C648d491560536E147479B028aE); //marketing wallet
    address public developmentWallet = address(0x8CA73906151f79f068Ed1fA750e6731444F061Da); //development wallet
    ...
    uint16 public developmentFee = 1000; // 10% fee
    uint16 public buybackFee = 100; // 1% fee
    uint16 private totalFees =
        rewardsFee + liquidityFee + marketingFee + developmentFee + buybackFee; // 22% TotalFee
    ...
    constructor() ERC20("RoyalWeedCoin", "RWC") {
        dividendTracker = new RWCDividendTracker();
        ...
        */
        _mint(owner(), 1836900000 * (10**9)); // Total-Supply 1.8369 billion Token.
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        RWCDividendTracker newDividendTracker = RWCDividendTracker(
            payable(newAddress)
        );
    ...
    function setFees(
      ...
      require(_market <= 500, "5% max marketing fee");
      require(_liq <= 500, "5% max liq fee");
      require(_reward <= 1000, "10% max reward fee");
      require(_develop <= 1000, "10% max develop fee");
    ...
    function setMaxBuy(uint256 amount) external onlyOwner {
        require ( amount <= 36738000 * 10**9, "Can't set higher than 36.738 million tokens, Anti-Whale");
        maxBuyAmount = amount;
    }
    ...
    function setMaxSell(uint256 amount) external onlyOwner {
        require ( amount <= 18369000 * 10**9, "Can't set higher than 18.369 million tokens, Anti-Dump");
...
contract RWCDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("RoyalWeedCoin_Dividend_Tracker", "RWC_Dividend_Tracker")
    {
        claimWait = 3690;
        minimumTokenBalanceForDividends = 183690 * (10**9); //must hold 183690+ tokens
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            newClaimWait >= 3690 && newClaimWait <= 86400,
            "RWC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
