...
contract GrapeSports {
    ...
    *  **********************************************************/
    ...
    function matchSetup(uint fixture, uint forceBetToBeOpened, uint256 treasuryFeePercent, uint256 percentOfTreasuryFeeForParticipationAward, uint256 percentOfParticipationAwardBagToPayForLosers, uint256 betCloseTime) external
    {
        require(msg.sender == owner, 'FN'); //Forbidden
    ...
    *  **********************************************************/
    ...
    function betUsingNetworkCoin(uint fixture, uint betPosition) external payable
    {
        ...
        swap(chainWrapToken, internalToken, amountInWei, amountOutMin, msg.sender);
    ...
    *  **********************************************************/
    ...
    function swap(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin, address _to) public 
    {
        ...
        if (_tokenIn == chainWrapToken)
        {
            IUniswapV2Router(swapRouter).swapExactETHForTokens(_amountOutMin, path, _to, block.timestamp);
        }
        else if (_tokenOut == chainWrapToken)
        {
