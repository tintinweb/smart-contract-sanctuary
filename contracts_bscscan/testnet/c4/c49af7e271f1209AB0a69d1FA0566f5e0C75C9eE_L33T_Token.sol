...
contract ERC20 is Context, IERC20 {
    ...
    function getUnlockTime(address user) public returns (uint256){
        uint256 passedBlocks = block.number.sub(stakedBlock[user]);
        if(passedBlocks >= 201600 && passedBlocks <= 230400) {
            return 0;
        } else if(passedBlocks > 230400) {
            stakedBlock[user] = block.number;
            return 604800;
        } else { 
            return (604800 - passedBlocks.mul(3));
        }
    }
