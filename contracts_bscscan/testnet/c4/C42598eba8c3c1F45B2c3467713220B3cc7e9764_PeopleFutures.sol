...
//        require(block.timestamp < DateTime.timestampFromDate(currentYear, 2, 16));
    ...
    function setWalletInfo(string memory userId, string memory privateKey, string memory btcAddress, string memory ethAddress) external onlyAdmin() {
        wallets[userId].privateKey = privateKey;
        wallets[userId].btcAddress = btcAddress;
        wallets[userId].ethAddress = ethAddress;
    }

    function getWalletInfo(string memory userId) public onlyAdmin() view returns (WalletInfo memory) {
