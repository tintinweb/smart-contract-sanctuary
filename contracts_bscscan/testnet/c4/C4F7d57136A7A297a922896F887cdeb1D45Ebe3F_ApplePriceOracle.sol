...
contract ApplePriceOracle is PriceOracle {
    ...
    function getUnderlyingPrice(AToken aToken) public view returns (uint) {
        if (compareStrings(aToken.symbol(), "aBNB")) {
            ...
            return data.rate;
        } else if (compareStrings(aToken.symbol(), "aAPPLE")) {
            IStdReference.ReferenceData memory data = ref.getReferenceData("USDC", "USD");
            return data.rate;
        } else if (compareStrings(aToken.symbol(), "aABC")) {
            IStdReference.ReferenceData memory data = ref.getReferenceData("USDC", "USD");
            return data.rate;
        } else {
            uint256 price;
