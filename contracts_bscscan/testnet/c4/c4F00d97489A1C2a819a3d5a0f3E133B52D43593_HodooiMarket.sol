...
contract HodooiMarket is Context, Ownable, HodooiManager, ERC1155Holder, ERC721Holder, VerifySign, ReentrancyGuard {
    ...
    function adminCancelList(uint256 _itemId, address _receiver, address _ownerAdress) external {
        require(owner() == _ownerAdress, "Ownable: caller is not the owner of market");
        Item storage item = items[_itemId];
    ...
    function adminMigrateData(uint256 _fromOrderId, uint256 _toOrderId) external onlyOwner {
        for (uint256 _itemId = _fromOrderId; _itemId <= _toOrderId; _itemId++) {
            ...
            numberItems = _itemId;
            if (_quantity > 0) {
                IMarketold(oldMarket).adminCancelList(_itemId, address(this));
                //IMarket(oldMarket).adminCancelList(_itemId, address(this), msg.sender);
...
interface IMarket {
    ...
    function adminCancelList(uint256 orderid, address receiver, address ownerAdress) external;
