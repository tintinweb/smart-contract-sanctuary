...
contract Emariswap is Context, IERC20, Ownable {
    ...
    string private _name = "Emariswap";
    string private _symbol = "eMari";
    ...
    function _takeBurn(uint256 tBurn) private {
        uint256 currentRate =  _getRate();
        uint256 rBurn = tBurn.mul(currentRate);
        _rOwned[_MarketingWalletAddress] = _rOwned[_MarketingWalletAddress].add(rBurn);
        if(_isExcluded[_MarketingWalletAddress])
            _tOwned[_MarketingWalletAddress] = _tOwned[_MarketingWalletAddress].add(tBurn);
    }
