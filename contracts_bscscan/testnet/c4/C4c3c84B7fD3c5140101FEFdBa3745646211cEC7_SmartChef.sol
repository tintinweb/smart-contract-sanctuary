...
contract SmartChef is Ownable {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 depositTokenReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accRewardPerShare = accRewardPerShare.add(depositTokenReward.mul(1e18).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 depositTokenReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accRewardPerShare = pool.accRewardPerShare.add(depositTokenReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            if(user.amount == 0 && user.isLocked == false) {
                user.isLocked = true;
                user.locktime = now + pool.lockDuration;
            }
    ...
    function withdraw(uint256 _amount) public {
        ...
        if(_amount > 0) {
            require(now > user.locktime, "withdraw: not finished locktime yet");
