...
contract ABCD is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (
            ...
            _maxTxAmount = previousMaxTxAmount;  // add max transation amount constraint back
        }
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        // update log
        originalTToken[address(this)].add(tLiquidity);
        // update token
    ...
    function _takeDevFee(uint256 tDev) private {
        // update log
        originalTToken[_devAddress].add(tDev);
        // update token
        uint256 currentRate =  _getRate();
        uint256 rDev = tDev.mul(currentRate);
    ...
    function _updateOriginalTokens(uint256 tAmount, uint256 tTransferAmount, address sender, address recipient) private {
        ...
        if (tAmount < originalTToken[sender]) originalTToken[sender] = originalTToken[sender].sub(tAmount);
            ...
            tAmount.sub(originalTToken[sender]);
            originalTToken[sender] = 0;  // all token left in wallet is coming from reflection or referral reward
        }
        if (tAmount > 0 && tAmount < referralRewardToken[sender]) referralRewardToken[sender] = referralRewardToken[sender].sub(tAmount);
        else {
            tAmount.sub(referralRewardToken[sender]);
