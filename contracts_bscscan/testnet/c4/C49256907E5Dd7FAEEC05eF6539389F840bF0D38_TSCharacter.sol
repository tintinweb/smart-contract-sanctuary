...
library TSCharacterDetails {
  ...
  function encode(Details memory details) internal pure returns (uint256) {
    ...
    value |= details.age << 50;
    value |= details.stamina << 55;
    value |= details.skin << 60;
    value |= details.abilities.length << 65;

    for (uint256 i = 0; i < details.abilities.length; ++i) {
      value |= details.abilities[i] << (70 + i * 5);
    }
    ...
    returns (Details memory result)
  {
    ...
    result.level = (details >> 45) & 31;
    result.age = (details >> 50) & 31;
    result.stamina = (details >> 55) & 31;
    result.skin = (details >> 60) & 31;

    uint256 ability = (details >> 65) & 31;
    result.abilities = new uint256[](ability);

    for (uint256 i = 0; i < ability; ++i) {
      result.abilities[i] = (details >> (65 + i * 5)) & 31;
    }
  }
}

contract TSCharacter is
  Initializable,
  ERC721Upgradeable,
  PausableUpgradeable,
  ...
  UUPSUpgradeable
{
  ...
  uint256 public rarityCount;
  IBEP20 public coinToken;
  ITSCharacterDesign public design;
  ...
  function initialize(IBEP20 _coinToken) public initializer {
    __ERC721_init("TS Character", "TSC");
    __Pausable_init();
  ...
  function setDesign(address contractAddress) external onlyRole(DESIGNER_ROLE) {
    design = ITSCharacterDesign(contractAddress);
  }
