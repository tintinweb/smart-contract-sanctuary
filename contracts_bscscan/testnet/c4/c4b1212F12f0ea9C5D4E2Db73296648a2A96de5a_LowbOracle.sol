...
contract LowbOracle is Ownable {
  ...
  mapping(uint256=>uint256) transactionBlocks;
  mapping(address=>uint256) latestTransaction;
  ...
  function depositFor(uint256 _amount, address _for) public returns (uint256) {
    ...
    latestTransaction[msg.sender] = totalTransactions;
    emit DepositForEvent(_amount-fees, _for, totalTransactions);
