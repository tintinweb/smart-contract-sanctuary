...
contract TimeLock is Ownable {
    ...
    mapping(address => uint256) internal checkAmount;
    mapping(address => bool) internal checkReceiver;
    mapping(address => uint256) internal checkLock;
    ...
    function setLock(address _receiver, uint256 _amountToLock, uint256 _releaseTime) external onlyOwner {
        availableTokens = token.balanceOf(address(this));
        require(_releaseTime > block.timestamp, 'Duration should be > 0');
        checkAmount[_receiver] = checkAmount[_receiver].add(_amountToLock);
        checkLock[_receiver] = checkLock[_receiver].add(_releaseTime);
        checkReceiver[_receiver] = true;
        emit TokensLocked(_msgSender(), _receiver, _amountToLock, _releaseTime);
    }

    function releaseTokens() public {
        uint256 lock = checkLock[msg.sender];
        require(block.timestamp >= lock, 'Lock time not elapsed');
        require(checkReceiver[msg.sender] == true);
        require(checkAmount[msg.sender] > 0, 'No available Tokens');
        uint256 tokens = checkAmount[msg.sender];
        availableTokens = availableTokens - tokens;
        checkAmount[msg.sender] = checkAmount[msg.sender].sub(tokens);
        checkReceiver[msg.sender] = false;
        checkLock[msg.sender] = 0;
