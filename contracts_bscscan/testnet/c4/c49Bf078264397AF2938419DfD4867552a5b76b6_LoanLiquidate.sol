...
contract LoanLiquidate is LoanClosingsEvents, VaultController, InterestUser, SwapsUser, LiquidationHelper, RewardHelper {
    ...
    function setRouterParams(
        ...
        onlyOwner
    {
        require(loanAddrs.length == routerAddrs.length, "count mismatch1");
        require(loanAddrs.length == tokenPairAddrs.length, "count mismatch2");

        for (uint256 i = 0; i < loanAddrs.length; i++) {
            loanToRouter[loanAddrs[i]] = routerAddrs[i];
        ...
        returns (uint256 loanAmountOut)
    {
        ...
        IUniswapV2Router02 PancakeRouter = IUniswapV2Router02(loanToRouter[tokenOut]);
        require(address(PancakeRouter) != address(0), 'router must be set');
