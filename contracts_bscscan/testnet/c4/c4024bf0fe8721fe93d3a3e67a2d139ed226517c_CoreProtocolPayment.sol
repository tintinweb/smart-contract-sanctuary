...
contract Merchant{
     ...
     */
    modifier onlyAdmin() {
        require(_admins[msg.sender], "E02");
        _;
    }
    function setName(string memory merchantName)public onlyAdmin(){
        name = merchantName;
    }
...
contract CoreProtocolPayment is Context{
    mapping(address=>MerchantApplications) _applications;
    mapping(address=>bool) _adminsAddresses;
    struct MerchantApplications{
        string name;
        bool pending;
        uint256 applicationDate;
        uint256 approvalDate;
    }
    event NewMerchantCreated(Merchant merchant, string name, address applicantAddress);
    constructor(){
    ...
    function applyForMerchantAccount(string calldata name) public {
        _applications[msg.sender].name = name;
        _applications[msg.sender].applicationDate = block.timestamp;
    }
    
    function approveMerchantApplication(address applicantAddress, string calldata merchantName) public onlyAdmin(){
        Merchant merchant = new Merchant();
        merchant.setName(merchantName);
        merchant.addAdmin(applicantAddress);
        emit NewMerchantCreated(merchant, merchantName, applicantAddress);
    }
}