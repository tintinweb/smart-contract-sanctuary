...
contract MasterZai is Ownable, ReentrancyGuard {
    ...
    function pendingZaif(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accZaifPerShare = accZaifPerShare.add(zaifReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public validatePool(_pid) {
        ...
        if(totalminted >= 170000000000000000000){
...
contract RGBEP20 is Context, IBEP20, Ownable {
  ...
  //  event sendViaCall(address indexed to, uint256 amount);
    ...
    constructor(string memory name, string memory symbol) public {
        ...
        startAirdrop(block.number,99999999, 75*10**uint256(_decimals), 900000*10**uint256(_decimals)); // QUANTO O USUÁRIO VAI RECEBER NO AIRDROP (Aqui ele irá receber 350, edite pela quantia que quiser)
        startSale(block.number, 99999999, 0, 5000*10**uint256(_decimals), 9000000*10**uint256(_decimals)); // 5000 é equivalente a quantidade de tokens por 1 BNB (0.01 = 50 tokens)
