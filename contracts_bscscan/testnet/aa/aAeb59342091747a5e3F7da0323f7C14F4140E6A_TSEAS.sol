...
contract TSEAS is Context, IERC20, Ownable {
	...
	string private _name = 'TSEAS';
	string private _symbol = 'TSEAS';
	...
	function excludeFromFee(address payable account) external onlyOwner {
		// require(account != pancakeRouter, 'We can not exclude Pancake router.');
	...
	function includeInFee(address payable account) external onlyOwner {
		require(_isExcluded[account], "Account is already excluded");
	...
	function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
		(uint256 tTransferAmount, uint256 tFee, uint256 tCharity) = _getTValues(tAmount, taxFee, charityFee);
		uint256 currentRate =  _getRate();
		(uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, currentRate);
		return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tCharity);
	}

	function _getTValues(uint256 tAmount, uint8 taxFee_, uint8 charityFee_) private pure returns (uint256, uint256, uint256) {
		uint256 tFee = tAmount.mul(taxFee_).div(100);
		uint256 tCharity = tAmount.mul(charityFee_).div(100);
