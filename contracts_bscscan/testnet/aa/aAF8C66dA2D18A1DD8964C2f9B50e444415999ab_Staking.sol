...
contract Staking is AccessControl, ReentrancyGuard {
    ...
    function setAvailability(bool[] calldata state)
        external
        onlyRole(ADMIN_ROLE)
    {
        if (isStakeAvailable != state[0]) isStakeAvailable = state[0];
        if (isUnstakeAvailable != state[1]) isUnstakeAvailable = state[1];
        if (isClaimAvailable != state[2]) isClaimAvailable = state[2];
    }
    ...
    function _produced() internal view returns (uint256) {
        ...
        /* 
			...
			if halfingDuration is about three month on the output we will have 20 years max staking-live
