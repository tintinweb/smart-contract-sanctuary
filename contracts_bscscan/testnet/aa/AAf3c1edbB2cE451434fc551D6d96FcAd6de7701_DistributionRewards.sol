...
contract DistributionRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Ownable {
    ...
    function stake(address user, uint256 amount) external override onlyVault nonReentrant updateReward(user) {
    ...
    function withdraw(address user, uint256 amount) public override onlyVault nonReentrant updateReward(user) {
    ...
    modifier onlyVault() {
        require(msg.sender == vault, "Invalid Access: Can only be accessed by Vaults");
        _;
    }
