...
contract BlucamonAirdrop is BlucamonFactory {
    ...
    modifier onlyAirdropWhitelist(address _address) {
        require(
            whitelistEggDetail[_address].id != 0,
            "address must in whitelist"
        );
        _;
    }
    ...
    BlucamonAirdrop
{
    ...
    function summon(uint256 _id) external payable onlyOwnerOf(_id) {
        require(
            !blucamons[indexMapping[_id]].isSummoned,
    ...
    function mintBlucamon(address _address, string memory _tokenUri)
        external
        onlyWhitelistSetter
    {
        blucamons.push(Blucamon(true));
        blucamonId = blucamonId + 1;
        indexMapping[blucamonId] = blucamons.length - 1;
        _safeMint(_address, blucamonId);
        _setTokenURI(blucamonId, _tokenUri);
        emit SummonBlucamon(blucamonId);
    }

    function getBlucamons(address _address) external view returns (uint256[] memory) {
        uint256 blucamonCount = balanceOf(_address);
        if (blucamonCount == 0) {
            return new uint256[](1);
        }
        uint256[] memory blucamonIdList = new uint256[](blucamonCount);
        for (uint256 index = 0; index < blucamonCount; index.add(1)) {
            blucamonIdList[index] = tokenOfOwnerByIndex(_address, index);
        }

        return blucamonIdList;
    }
}
