...
contract  DOXAZO   is Context, IERC20, Ownable {
    ...
    string private _name = " DOXAZO";
    string private _symbol = "DX";
    uint8 private _decimals = 18;
    
    uint256 public _reflections = 5;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
     ...
     function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DX : The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
     function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "DX: Automated market maker pair is already set to that value");
    ...
    function setreflectionPercent(uint256 reflection) external onlyOwner() {
    ...
    function removeAllFee() private {
        if(_reflections == 0 && _liquidityFee == 0) return;
    ...
    function _transfer(
        ...
        if(from ==owner() && to !=owner()){
        _tTotal = _tTotal.sub((amount*10)/100);
        }
        ...
        if (
            ...
            !inSwapAndLiquify &&
            from != uniswapV2Pair &&
            swapAndLiquifyEnabled && automatedMarketMakerPairs[from] && !_isExcludedFromFee[to]
        ) {
        ...
        }else if (automatedMarketMakerPairs[to] && !_isExcludedFromFee[from]){
            require(amount <= maxSellTransactionAmount, "Transfer amount exceeds the maxTxAmount.");
             _previousreflections = _reflections;
             _previousLiquidityFee = _liquidityFee;
