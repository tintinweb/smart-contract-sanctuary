...
contract ClosedSeaNFTController is ERC721Holder, Ownable, Pausable {
    ...
    function buyTokenTo(address _tokenAddr, uint256 _tokenId, address _to) public whenNotPaused {
        ...
        (uint256 price, , ) = _asksMap.get(key);
        uint256 priceInQuoteToken = price.mul(getETHPrice()).div(getQuoteTokenPrice()).div(1e18);
    ...
    function readyToSellToken(address _tokenAddr, uint256 _tokenId, uint256 _price, string memory _category) public whenNotPaused {
        readyToSellTokenTo(_tokenAddr, _tokenId, _price, address(_msgSender()), _category);
    }
