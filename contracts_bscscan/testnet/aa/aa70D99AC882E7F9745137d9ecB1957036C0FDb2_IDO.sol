...
contract IDO {
    ...
    // Puchase
    event Puchase(address sender, uint256 amount);
     ...
     * @param _puchaseAmount: the number of puchase token to withdraw (18 decimals)
     * @param _offerAmount: the number of offering amount to withdraw (18 decimals)
     */
    function finalWithdraw(uint256 _puchaseAmount, uint256 _offerAmount) public onlyOwner {
        if (_puchaseAmount > 0){
            require(_puchaseAmount <= purchaseToken.balanceOf(address(this)), "Not enough puchase tokens");
            purchaseToken.transferFrom(address(this), adminAddress, _puchaseAmount);
        }
        if (_offerAmount > 0){
        ...
        emit FinalWithdraw(_puchaseAmount, _offerAmount);
    }
    /**
     * @dev It allows users to puchase tokens 
     * @param _amount: the number of token M (18 decimals)
     */
    function puchase(uint256 _amount) public {
        ...
        emit Puchase(msg.sender, _amount);
    }

    /**
     * @dev puchase tokens balance
     */
    ...
    function _safeTransferFrom(uint amount) private {
        bool sent = purchaseToken.transferFrom(msg.sender, address(this), amount);
