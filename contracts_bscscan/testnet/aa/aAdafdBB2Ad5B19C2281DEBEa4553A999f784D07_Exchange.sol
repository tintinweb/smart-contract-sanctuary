...
contract Exchange is Ownable {
    ...
    ///////////// Fees ///////////////////////////////
    // Fee in per thousand: /1000
    uint256 public defaultFeeRate;
    uint256 public mallFeeRate;
    //////////////////////////////////////////////////
    ...
    function setFees(uint256 _defaultFeeRate, uint256 _mallFeeRate) external onlyOwner {
        defaultFeeRate = _defaultFeeRate;
        mallFeeRate = _mallFeeRate;
    }
    ...
    function _generateFees(ExchangeData memory data) internal view returns (address payable creator, uint256[2] memory _fees){
        ...
        // Pay in MALL
        if (IERC20(data.priceToken) == mall) {
            tradingFee = price.mul(mallFeeRate).div(1000);
        } else {
            tradingFee = price.mul(defaultFeeRate).div(1000);
        }
    ...
    function _transferCost(ExchangeData memory data) internal {
        ...
        uint256 toSeller = price.sub(fees[0]).sub(fees[1]);

        if (IERC20(data.priceToken) == IERC20(address(0))) {
            require(msg.value >= price, "Order is underpriced");
    ...
    function _generateFeesAu(BidData memory data) internal view returns (address payable creator, uint256[2] memory _fees){
    ...
    function _transferCostAu(BidData memory data) internal {
        uint256 price = data.price;
        (address payable creator, uint256[2] memory fees) = _generateFeesAu(data);
        ...
        erc20TransferProxy.erc20safeTransferFrom(IERC20(data.priceToken), data.winner, beneficiary, toBeneficiary);
        emit FeesPaid(beneficiary, toBeneficiary);
    }
