...
contract ImplAndTerms is Storage, Ownable, ERC20Init {
    ...
    function stake(uint tokenAmount) public {
        stakeInternal(msg.sender, tokenAmount, 0, address(0));
    }

    function stake(uint tokenAmount, uint holdTime) public {
        stakeInternal(msg.sender, tokenAmount, holdTime, address(0));
    }

    function stake(uint tokenAmount, uint holdTime, address referer) public {
        stakeInternal(msg.sender, tokenAmount, holdTime, referer);
    }

    function stakeInternal(address staker, uint tokenAmount, uint holdTime, address referer) internal {
        require(
            referer != staker,
