...
// SHIBAPUMP BEP20 Token Available on PancakeSwap
// https://shibapumpkin.com/  <--- Website
// https://t.me/ShibaPUMPkinbsc <--- Telegram
...
contract SHIBAPUMPDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("SHIBAPUMP_Dividend_Tracker", "SHIBAPUMP_Dividend_Tracker", 9) {
    	claimWait = 14400;
        minimumTokenBalanceForDividends = 1000000 * (10**9); //must hold 10000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "SHIBAPUMP_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "SHIBAPUMP_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SHIBAPUMP contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SHIBAPUMP_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SHIBAPUMP_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SHIBAPUMPToken is BEP20, Ownable {
    using SafeMath for uint256;

    IPancakeswapV2Router02 public pancakeswapV2Router;
    address public immutable pancakeswapV2Pair;
    address public marketingWallet = 0xF141B2f76bc8574C75f94EeE371158289c752A5e;

    bool private swapping;

    SHIBAPUMPDividendTracker public dividendTracker;
    ...
    // sells have fees of 8 and 4 (10 * 1.2 and 5 * 1.2)
    ...
    constructor() public BEP20("ShibaPUMPkin", "SHIBAPUMP", 9) {
    	...
    	dividendTracker = new SHIBAPUMPDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SHIBAPUMP: The dividend tracker already has that address");

        SHIBAPUMPDividendTracker newDividendTracker = SHIBAPUMPDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SHIBAPUMP: The new dividend tracker must be owned by the SHIBAPUMP token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "SHIBAPUMP: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SHIBAPUMP: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "SHIBAPUMP: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SHIBAPUMP: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SHIBAPUMP: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SHIBAPUMP: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SHIBAPUMP: Cannot update gasForProcessing to same value");
