...
contract JokerCoin is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    string private _name = "Joker Coin";
    ...
    uint256 public _JokerSpinFee = 40; // 4% will be added to the liquidity pool
    uint256 public _marketFee = 10; // 1% will go to the market address
    uint256 public stakingFee = 20; // 2% will go to the charity address

    uint256 public _marketFeeOnSelling = 10; // 2% will be added to the liquidity pool
    uint256 public _devFee = 20; // 2% will go to the dev address
    uint256 public _buyBackFee = 40; // 4% will be added to pool for BNB reward
    ...
    constructor(address payable _devWallet,address payable _buyBackWallet,address payable _JokerSpinWallet,address payable _marketWallet,address payable _stakingWallet)
    {
    ...
    function setBuyFeePercent(
        uint256 JokerSpinFee,
        uint256 _stakingFee,
        uint256 marketFee
    ) external onlyOwner {
        _JokerSpinFee = JokerSpinFee;
    ...
    function setSellFeePercent(
        uint256 devFee,
        uint256 marketFeeOnSelling,
        uint256 buyBackFee
    ) external onlyOwner {
        _devFee = devFee;
        _marketFeeOnSelling = marketFeeOnSelling;
        ...
        onlyOwner
    {
     ...
     function setJokerSpinWallet(address payable _JokerSpinWallet) external onlyOwner {
        JokerSpinWallet = _JokerSpinWallet;
    }
    ...
    function totalBuyFeePerTx(uint256 amount) internal view returns (uint256) {
        uint256 fee = amount
            .mul(
                _JokerSpinFee.add(stakingFee).add(
                    _marketFee
                )
            )
            .div(1e3);
    ...
    function _transfer(
        ...
        require(amount > 0, "joker: transfer amount must be greater than zero");
        require(balanceOf(from) >= amount, "joker: not enough balance");
    ...
    function _tokenTransfer(
        ...
        if (!takeFee) {
            ...
            _balances[sender] = _balances[sender].sub(amount);
            _balances[JokerSpinWallet] = _balances[JokerSpinWallet].add(amount.mul(_JokerSpinFee).div(1e3));
            _balances[stakingWallet]= _balances[JokerSpinWallet].add(amount.mul(stakingFee).div(1e3));
            _balances[address(this)]= _balances[address(this)].add(amount.mul(_marketFee).div(1e3));
            uint256 allFee = totalBuyFeePerTx(amount);
            uint256 tTransferAmount = amount.sub(allFee);
           _balances[recipient] = _balances[recipient].add(tTransferAmount);
            emit Transfer(sender, recipient, tTransferAmount);
            emit Transfer(sender, address(this), _marketFee);
            emit Transfer(sender, address(JokerSpinWallet), _JokerSpinFee);
            emit Transfer(sender, address(stakingWallet), stakingFee);
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            uint256 otherPiece = contractTokenBalance.mul(totalPercent).div(1e4);
            ...
            totalPercent = bbPercent.add(dvPercent).add(
                mwPercent
            );
            ...
            devWallet.transfer(
                deltaBalance.mul(dvPercent).div(totalPercent)
            );
            ...
            buyBackWallet.transfer(
                deltaBalance.mul(bbPercent).div(totalPercent)
            );
