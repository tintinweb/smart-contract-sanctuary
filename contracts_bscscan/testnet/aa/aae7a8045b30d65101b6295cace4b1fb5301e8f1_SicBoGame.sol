...
contract SicBoGame  {
    ...
    function deposit(uint256 _bid, uint256 _value) public lock canDeposit{
        ...
        if(_bid==0){
            addTicket(BidTicket(_bid,block.timestamp,_value,currentRound.totalAmountLow,round));
        }
        else{
            addTicket(BidTicket(_bid,block.timestamp,_value,currentRound.totalAmountHigh,round));
        }
        //
