    ...
    PausableUpgradeable
{
    ...
    function deposit(
        ...
        require(
            verify(msg.sender, address(this), _message, _signature),
            "Tx not signed"
        );
        ...
        nonReentrant
    {
        ...
        if (rounds[round].totalDeposit <= maxVolumeInRound(round)) {
            withdrawTokenAmount = calculateExactTokenAmount(
                round,
                depositTable[round][msg.sender].depositValue
            );
        } else {
            uint256 buyableBnb = getBuyableBnb(round);
            if (depositTable[round][msg.sender].depositValue > buyableBnb) {
                withdrawTokenAmount = calculateExactTokenAmount(
                    round,
                    buyableBnb
                );
                refund(
                    msg.sender,
                    depositTable[round][msg.sender].depositValue.sub(buyableBnb)
                );
            } else {
                withdrawTokenAmount = calculateExactTokenAmount(
                    round,
    ...
    function calculateExactTokenAmount(uint256 round, uint256 amountBnb)
        ...
        returns (uint256)
    {
        return amountBnb.div(initialPriceInRound(round)).mul(10**18);
    }
...

{
  ...
  "libraries": {
    "/Users/tamnguyen/Documents/project/xbn/xblade-token-distribution/contracts/lib/Utils.sol": {
      "Utils": "0xc1C94F8D0B26c1c0F3F6468b4857E77EfF72fB0e"
    }
  },
  "outputSelection": {
    "*": {
