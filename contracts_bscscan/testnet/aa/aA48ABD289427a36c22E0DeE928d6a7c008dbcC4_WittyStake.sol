...
contract WittyStake {
    ...
    event Stake(address indexed from,address  referer,uint amount,uint plan,uint time);
    ...
    function stake(uint _plan,uint _amount,address _mainRef,address[] memory _ref,uint[] memory _commissionAmt,uint _flag) public isLock isContractCheck(msg.sender) {
        ...
        _refPayout(_ref,_commissionAmt,_plan);
        
        emit Stake(msg.sender,_mainRef,invest[msg.sender][_plan].depositAmount,_plan,block.timestamp);

    }
    
    function _refPayout(address[] memory _ref ,uint[] memory _amt,uint _plan) internal {
    ...
    function payout(address _user,uint _plan) external view returns(uint _payout, uint _maxPayout) {
        require(invest[_user][_plan].depositAmount > 0,"Not yet deposit");
            ...
            if (invest[_user][_plan].depositPayouts < _maxPayout) {
                _payout = ((amount.mul(roiPercentage[_plan]).div(100e18)).mul((block.timestamp.sub(invest[_user][_plan].depositTime)).div(payoutDuration))).sub(invest[_user][_plan].depositPayouts);
