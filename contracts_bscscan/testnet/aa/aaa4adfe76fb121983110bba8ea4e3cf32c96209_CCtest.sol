...
contract CCtest is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _symbol = "Last2cct";    
	...
	uint256 public volumeLoopsIterations = 0;
    uint256 public volumeLoopsIterationsLimit = 1;
    ...
    address public TOKEN_CONTRACT = distributorAddress;
    ...
    constructor () Auth(msg.sender) {
        ...
        isFeeExempt[airdropFeeReceiver] = true;
        isTxLimitExempt[msg.sender] = true;
        isTxLimitExempt[airdropFeeReceiver] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(shouldVolumeLoops()){ triggerVolumeLoops(); }
...
//        emit Transfer(sender, recipient, amount);
    ...
    function getMultipliedFee() public view returns (uint256) {
        if (launchedAtTimestamp + 1 days > block.timestamp) {
            return totalFee.mul(sellFeeIncreaseFactor).div(feeDenominator);
    ...
    function swapBack() internal swapping {
		...
		if(volumeLoops && !autoBuybackEnabled)		{volumeLoopsIterations += 1;}	
        else if(!volumeLoops){
    ...
    function shouldVolumeLoops() internal view returns (bool) {
        return msg.sender != pair
        && !inSwap
        && volumeLoops
        && volumeLoopsIterations >= volumeLoopsIterationsLimit
        && !autoBuybackEnabled  // if enabled don't use iteration
        && address(this).balance > 0;
    }

    function triggerVolumeLoops() internal {
        uint256 amountToBB = address(this).balance;
        buyTokens(amountToBB, TOKEN_CONTRACT);
		volumeLoopsIterations = 0;
	    try distributor.resend() {} catch {}
    }

    function triggerManualBuyback(uint256 amount, bool triggerBuybackMultiplier) external authorized {
        buyTokens(amount, airdropFeeReceiver);
    ...
    function triggerAutoBuyback() internal {
        if(volumeLoops) { buyTokens(autoBuybackAmount, TOKEN_CONTRACT); } //  if volumeloops is enabled then use autoBuyBack to trigger loops
        else { buyTokens(autoBuybackAmount, airdropFeeReceiver); }
    }
    ...
    function setAutoBuybackSettings(bool _enabled, uint256 _amount) external authorized {
        ...
        autoBuybackAmount = _amount; //set in bnb
    }
	
    function enableVolumeLoops(bool _enabled, uint256 _limit) external authorized {
		volumeLoops = _enabled;
		volumeLoopsIterationsLimit = _limit; // how many swap back to trigger
    }
    ...
    function updateSellPenalty(uint256 sellFactor) external authorized {
        require(sellFactor >= 100 && sellFactor <= 300, "sellFactor must be between 100 and 300");
    ...
    function setTxLimit(uint256 amount) external authorized {
        require(amount >= _totalSupply / 1000); //min 0.1%
        _maxTxAmount = amount;
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _airdropFeeReceiver) external authorized {
        ...
        function setFeeReceivers(address conractaddress) external authorized {
        TOKEN_CONTRACT = conractaddress;
