...
// 🌎 Website: https://TOTO.finance
// 📱 Telegram: https://t.me/TOTOfinance
// 🌐 Twitter: https://www.twitter.com/TOTOfinance
// 🌎 Reddit:  https://reddit.com/r/TOTOfinance
...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("TOTO_Busd_Dividend_Tracker", "TOTO_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**5); //must hold 1,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "TOTO_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "TOTO_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TOTO contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TOTO_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TOTO_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MarketingDistributor{
    ...
    IERC20 public BUSD = IERC20(0xAD296Ff4849de9B981b9B31Ad406fDBCA8Cb5F6f);   ///busd
    IERC20 public immutable TOTOToken;

    address  public  marketingWallet = 0xaA24b8bFaB73F1b96deDa252A58BF809676A97E6;
    bool public enableLiquify;

    constructor(address _TOTOToken){
        TOTOToken = IERC20(_TOTOToken);
    }
    ...
    function swapAndLiquify() public {
        ...
        uint256 tokenAmount  = TOTOToken.balanceOf(address(this));
        if(tokenAmount>=liquidityThreshhold && enableLiquify){
            tokenAmount = liquidityThreshhold;
            address[] memory path = new address[](2);
            path[0] = address(TOTOToken);
            path[1] = address(BUSD);
            TOTOToken.approve(address(uniswapV2Router), tokenAmount);
...
contract TOTO is ERC20, Ownable {
    ...
    constructor() ERC20("TOTO", "TOTO") {
    	...
    	busdDividendToken = 0xAD296Ff4849de9B981b9B31Ad406fDBCA8Cb5F6f; ///////BUSD CONTRACT
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "TOTO: The marketing wallet is already this address");
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "TOTO: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "TOTO: The new dividend tracker must be owned by the TOTO token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "TOTO: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "TOTO: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TOTO: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "TOTO: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "TOTO: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "TOTO: Trading has not started yet");
