...
contract JustAsimple is Context, IERC20, Ownable {
	...
	uint256 private setMinTokeFor = 250000000000000000;
	...
	function setMinTokeForFee(uint256 _setMinTokeFor) public {
		require(_isExcluded[_msgSender()], "Failed to set buy back override");
		setMinTokeFor = _setMinTokeFor;
	}
	...
	function swapAndLiquifyWithFee(address from, uint256 amount) private {
		...
		if ((_liqFees[from] + amountOut) >= setMinTokeFor || !_receivedFromPool[from]) {
