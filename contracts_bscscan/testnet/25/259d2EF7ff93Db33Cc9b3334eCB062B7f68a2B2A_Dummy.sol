...
contract DummyReward is RewardHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() RewardHolders("Dummy_Reward_Tracker", "Dummy_Reward_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "Dummy_Reward_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "Dummy_Reward_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Dummy contract.");
    }
...
function updateGasForProcessing(uint256 newValue) public masterWallet {
 ...
 uint256 tokensTDummy = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
