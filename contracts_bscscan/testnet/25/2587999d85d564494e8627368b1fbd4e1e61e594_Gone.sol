...
contract Gone is Context, IERC20, Ownable {
    ...
    string private _name = "Gone";
    string private _symbol = "GONE";
    ...
    function setCharityFeePercent(uint256 charityFee) external onlyOwner() {
        require(charityFee <= 50, 'Charity fees can not be higher than 50 [5%]');
        _charityFee = charityFee;
    }
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 100, 'Liquidity fees can not be higher than 100 [10%]');
