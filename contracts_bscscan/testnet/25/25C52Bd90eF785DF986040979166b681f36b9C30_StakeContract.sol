...
contract StakeContract {
    ...
    function stake(uint256 amount, uint256 plan) public {
        ...
        for (uint256 i; i < 4; i++) {
            user.stakerecord[user.stakecount].durationsParts[
                plan
            ] = partsDuration[plan][i];
            user.stakerecord[user.stakecount].percentageParts[
                    plan
                ] = partsPercentage[plan][i];
        }
    ...
    function nextWithdrawTime(address add,uint256 count) external view returns (uint256) {
