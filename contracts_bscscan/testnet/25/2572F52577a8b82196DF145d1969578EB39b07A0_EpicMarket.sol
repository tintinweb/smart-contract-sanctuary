...
contract EpicMarket is AccessControl {
    ...
    function makeOffer(uint256 marketId, uint256 price, uint256 expiration, uint256 amount) public onlyExistItem(marketId){
        ...
        (, Item memory itemData) = _getItemInfo(marketId);
        require(itemData._price * amount <= IERC20(itemData._token).balanceOf(msg.sender), "Balance is not enough");
    ...
    function closeOffer(uint256 offerId, uint256 marketId) public onlyExistItem(marketId){
    ...
    function cancelItem(uint256 marketId) public onlyItemOwner(marketId)  {
        ...
        require(IERC20(itemData._token).balanceOf(msg.sender) >=  wlToken.getFee(itemData._price), "Balance is not enough to cancel");
        IERC20(itemData._token).transferFrom(msg.sender, _wallet, wlToken.getFee(itemData._price));
