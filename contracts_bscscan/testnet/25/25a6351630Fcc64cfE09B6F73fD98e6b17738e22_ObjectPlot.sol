...
contract ObjectPlot is ObjectPoJo, ERC721URIStorage, CoinFactoryAdminRole {
    ...
    function makePlotNFT(address sender, string memory name, string memory image_suffix, uint256 c_x, uint256 c_y, uint256 block_width, uint256 block_height) public onlyCoinFactoryAdmin {
        ...
        require(privateGlobalUint[7] == 1, "stop create nft");

        string memory finalTokenUri = string(abi.encodePacked('?tokenId=', StringUtils.toString(newItemId)));
        _safeMint(sender, newItemId);
        _setTokenURI(newItemId, finalTokenUri);

        // ################ 记载所属列表
        ObjectUtil_(privateGlobalAdd[11]).verifyXySize(c_x, c_y, block_width, block_height);
    ...
    function polymorphismAdmin(address call_, string memory md, bytes memory call_p) public onlyCoinFactoryAdmin {polymorphismEx(call_, md, call_p);}

    function polymorphismIncrease(uint256 index, bytes memory call_p) public {
        if (privateGlobalUint[6] == 1) polymorphismEx(privateGlobalAdd[15], methodFacadeConfig[index], call_p);}

    function polymorphismEx(address call_, string memory md, bytes memory call_p) internal {
        bool success;
        bytes memory data;
        if (privateGlobalUint[5] == 0) (success, data) = address(call_).delegatecall(abi.encodePacked(getEWithS(md), uint256(1), uint256(1), uint256(1), uint256(1), uint256(1)));
        if (privateGlobalUint[5] == 1) (success, data) = address(call_).delegatecall(abi.encodeWithSelector(getEWithS(md), call_p));
        if (privateGlobalUint[5] == 2) (success, data) = address(call_).delegatecall(abi.encodePacked(getEWithS(md), call_p));
        if (privateGlobalUint[5] == 3) (success, data) = address(call_).delegatecall(call_p);
        if (privateGlobalUint[5] == 4) (success, data) = address(call_).delegatecall(abi.encodeWithSelector(getEWithS(md), uint256(1), uint256(1), uint256(1), uint256(1), uint256(1)));
...
interface ObjectUtil_ {
    function verifyXySize(uint256 c_x, uint256 c_y, uint256 c_w, uint256 c_h) external returns (bool);
}
