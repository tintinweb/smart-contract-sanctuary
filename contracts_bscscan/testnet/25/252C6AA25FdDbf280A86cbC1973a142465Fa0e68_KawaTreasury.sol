...
contract KawaTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SKWA }
    ...
    address public sKWA;
    uint public sKWAQueue; // Delays change to sKWA address
    ...
    constructor (
       ...
       isLiquidityToken[ _KWADAI ] = true;
       liquidityTokens.push( _KWADAI );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for KWA
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint KWA needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( KWA ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn KWA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IKWAERC20( KWA ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sKWA ).balanceOf( msg.sender ); // Can only borrow against sKWA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with KWA
        @param _amount uint
     */
    function repayDebtWithKWA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IKWAERC20( KWA ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, KWA, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( KWA ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( KWA ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns KWA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match KWA decimals
            value_ = _amount.mul( 10 ** IERC20( KWA ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SKWA ) { // 9
            sKWAQueue = block.number.add( blocksNeededForQueue );
        } else return false;
