 ...
 */
contract WestyPrivateSale is Ownable, ERC20("WESTY Private", "WESTY_PRV", 18) {
  using SafeMath for uint256;
  ...
  address public wallet = 0x98F774afA4F270E25aBFfF9d506beFeecC1FD6F9;

  // Presale start time
  uint256 public openingTime = 1639588800; //  Friday, December 17, 2021 5:00:00 (GMT)

  // Presale end time
  uint256 public closingTime = goal; // After 50 BNB is raised

  // How many token for a 1 BNB
  uint256 public rate = 200000000 ether;
  ...
  */
  ...
  function finalize() onlyOwner public {
    require(!isFinalized);
    require(hasClosed() || capReached() || cap.sub(weiRaised) < minContribution);
  ...
  */
  function removeManyFromWhitelist(address[] memory _beneficiaries) external onlyOwner {
    for (uint256 i = 0; i < _beneficiaries.length; i++) {
  ...
  function _preValidatePurchase(
    ...
    internal
    view
    onlyWhileOpen isWhitelisted(_beneficiary)
  {
