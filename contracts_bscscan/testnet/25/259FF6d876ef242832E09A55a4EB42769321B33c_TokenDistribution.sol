...
contract TokenDistribution is AccessControl {
    ...
    constructor(
        ...
        require(_timePeriod > 0, "Must be valid timePeriod");
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        if (timeRelease == 0) {
            require(
                allotted.add(amount) <= (capped - 2_000_000 * 1e18),
                "Full out"
            );
        } else {
            require(allotted.add(amount) <= capped, "Full out");
    ...
    function getClaimAmount(address user) public view returns (uint256) {
        if (block.timestamp < timeRelease || timeRelease == 0) return 0;
        Investor storage investor = investors[user];
        if (investor.packageId == 0) return 0;
        Package storage pack = packages[investor.packageId];
    ...
    function allocationFor(address user, uint256 amount) public {
        require(!suspended[msg.sender], "Must be not suspended");
        ...
        if (claimed > 0) {
            token.safeTransferFrom(msg.sender, user, claimed);
        }
    }
    ...
    function _setupInitPackage() private {
        ...
        // Advisors
        addPackage(0, 360 * DAY, 1080 * DAY);
        // Seed Round
        addPackage(5, 180 * DAY, 720 * DAY);
        // Private Round
        addPackage(10, 180 * DAY, 540 * DAY);
    }
    ...
    function addPackage(
        ...
        require(unlockPercent <= 100, "Must be valid percent");
    ...
    function suspend(address user, bool status)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function addLiquidity(uint256 amountBUSD, uint256 amountMinToken)
        public
