...
contract BROKEN is Context, IBEP20, Ownable {
    ...
    string private _name = "BRK";
    string private _symbol = "BRK";
    ...
    uint256 public timeDay = 10 * 60;       // 1h
    uint256 public timeWeek = 3 * 60 * 60;  // 2h
    uint256 public timeMonth = 4 * 60 * 60; // 4h
    ...
    function loteryDraw(uint tier) private lockTheSwap{
        ...
        if(tier == 1){
            while(found == false){
                tryNo = tryNo + 1;
                if(tryNo >= 10){unsuccessful += 1; break;}
                uint256 winnerIndex = random(0, tier1Cadidates.length - 1, current + tryNo);
                if (tier1Cadidates[winnerIndex] !=  address(this) &&
            ...
            while(found = false){
                ...
                if (tier2Cadidates[winnerIndex] !=  address(this) &&
            ...
            while(found == false){
                ...
                if (tier3Cadidates[winnerIndex] !=  address(this) &&
    ...
    IBEP20 public cakeToken = IBEP20(0x6a2139A693cF4bEB7fF3784105264b477C9382e8);
