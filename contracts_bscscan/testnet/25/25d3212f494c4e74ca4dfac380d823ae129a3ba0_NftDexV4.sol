...
contract NftDexV4 is ERC721Full, ERC721MetadataMintable, Ownable {
 ...
 // events
  ...
  event BlindBoxNonCrypto(address indexed from, string to, string tokenURI, uint256 quantity, bool flag);
  ...
  function buyNFT(address nft_a,uint256 tokenId, address buyer) public returns (bool) {
        ...
        if(_tokenAuthors[tokenId] == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859 && adminDiscount > 0){
            price = (price * adminDiscount) / 1000;
        }
        }
  ...
  function buyNftNonCrypto(address  nft_a, uint256 tokenId, string memory ownerId) public returns(bool) {
    uint256 tempPrice;
     if(_saleTokens[tokenId].price > 0){
          tempPrice = _saleTokens[tokenId].price;
        } else {
           (uint112 _reserve0, uint112 _reserve1,) =LPBNB.getReserves();
           (uint112 reserve0, uint112 reserve1,) =LPAlia.getReserves();
    ...
    if(_tokenAuthors[tokenId] == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859 && adminDiscount > 0){
      tempPrice = (tempPrice * adminDiscount) / 1000;
    }
  ...
  function blindBox(address to, string calldata tokenURI, uint256 quantity, bool flag, string calldata ownerId) external returns(bool){
  uint256 tokenId;
  tokenId = mintWithTokenURI(to, tokenURI, 1, flag);
      if(to == nonCryptoNFTVault){
        emit BlindBoxNonCrypto(msg.sender, ownerId, tokenURI, quantity, flag);
          _nonCryptoOwners[tokenId] = ownerId;
        }
      tokenMapping[tokenURI].inserted = true;
      tokenMapping[tokenURI].commercialUse = flag;
