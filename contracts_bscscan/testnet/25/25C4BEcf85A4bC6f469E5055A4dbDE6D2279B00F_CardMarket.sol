...
contract CardMarket {
  ...
  constructor(ERC721URIStorage _nft, IERC20 _token ) {
      nft = _nft;
      token = _token;
  }

  modifier OnlyItemOwner(uint256 tokenId){
    require(nft.ownerOf(tokenId) == msg.sender, "Sender does not own the item");
    _;
  }

  modifier HasTransferApproval(uint256 tokenId){
    require(nft.getApproved(tokenId) == address(this), "Market is not approved");
    _;
  }
  
  modifier HasTokenTransferApproval(address sender, uint256 amount){
   require(token.allowance(sender, address(this))>=amount ,"Token must be approved");
    _;
  }
  ...
  function buyItem(uint256 id) 
    ...
    HasTokenTransferApproval(msg.sender, itemsForSale[id].price)
    payable 
    external {
    
        //require(msg.value >= itemsForSale[id].price, "Not enough funds sent");
        require(msg.sender != itemsForSale[id].seller);
        token.transferFrom(msg.sender, address(this), itemsForSale[id].price );
        itemsForSale[id].isSold = true;
        activeItems[itemsForSale[id].tokenId] = false;
        nft.safeTransferFrom(itemsForSale[id].seller, msg.sender, itemsForSale[id].tokenId);
