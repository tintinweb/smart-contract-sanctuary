...
contract ForthBoxNFT_Swap is INftSwap, Ownable, ReentrancyGuard,IERC721Receiver {
    ...
    constructor() {
        ...
        nftToken = IERC721(0xb4989439bBcfDd43a10c3e93761143d90215aF0e);
    ...
    function getSellInfo(uint256 tokenId) public view returns (address addrs,uint256 prices,uint256 time){
        require(_exists(tokenId), "ForthBoxNFT_Swap: token none exist");
        return (_owners[tokenId],_sellPrice[tokenId],_sellTime[tokenId]);
    }

    function getSellInfos(uint256[] calldata tokenIdArr) public view returns ( address[] memory addrs,uint256[] memory prices,uint256[] memory times){
    ...
    function sells(uint256[] calldata tokenIds,uint256[] calldata prices ) public nonReentrant returns (bool){
        require(isWhiteContract(_msgSender()), "ForthBoxNFT_Swap: Contract not in white list!");
    ...
    function withdraws(uint256[] calldata tokenIds) public nonReentrant returns (bool){
        require(isWhiteContract(_msgSender()), "ForthBoxNFT_Swap: Contract not in white list!");
        require(tokenIds.length<=200, "ForthBoxNFT_Swap: num exceed 200!");
        for(uint256 i=0; i<tokenIds.length; ++i) {
    ...
    function _buy(uint256 tokenId) private{
        ...
        uint256 feeFundMeta = fee.mul(10).div(100);
        _burn(tokenId);

        fbxToken.safeTransferFrom(_msgSender(), ownerToken, price0.sub(fee));
        fbxToken.safeTransferFrom(_msgSender(), fundAdress, feeFundMeta);
        fbxToken.safeTransferFrom(_msgSender(), metaBoxAdress, feeFundMeta);
        fbxToken.safeTransferFrom(_msgSender(), address(0), fee.sub(feeFundMeta).sub(feeFundMeta));
    ...
    function buys(uint256[] calldata tokenIds) public nonReentrant returns (bool){
        require(isWhiteContract(_msgSender()), "ForthBoxNFT_Swap: Contract not in white list!");
    ...
    function addWhiteAccount(address account) external onlyOwner{
        ...
        require(account.isContract(), "ForthBoxNFT_Swap: not Contract Adress");
