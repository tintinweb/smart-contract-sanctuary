    ...
    ERC721URIStorage
{
    ...
    constructor(address marketplace_) ERC721("Non-Fungible Token", "NFT") {
    ...
    function mint(string memory seed) public virtual returns (uint256) {
        uint256 tokenId = _tokenIds.current();

        _tokenIds.increment();

        string memory tokenHash = _toHex(keccak256(abi.encode(seed, tokenId)));
        string memory tokenURI_ = string(abi.encodePacked(tokenHash, ".svg"));
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://avatars.dicebear.com/api/open-peeps/";
    }
    ...
    // Conversion from byte32 to string.
