...
contract BabyFalconDoge is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;
    address payable public marketingAddress =
        payable(0x63a5fA299a75B6c59ceB27Da6b61E2eB72AdE439); // Marketing Address
    address public immutable deadAddress =
        0x000000000000000000000000000000000000dEaD;
    bool public swapping = false;
    BFDOGEDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("Baby Falcon Doge", "BFDOGE") {
        uint256 _BNBRewardsFee = 0;
        uint256 _liquidityFee = 0;
        uint256 _marketingDivisor = 0;
        ...
        dividendTracker = new BFDOGEDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "BFDOGE: The dividend tracker already has that address"
        );

        BFDOGEDividendTracker newDividendTracker = BFDOGEDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "BFDOGE: The new dividend tracker must be owned by the BFDOGE token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "BFDOGE: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "BFDOGE: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "BFDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "BFDOGE: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "BFDOGE: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "BFDOGE: Cannot update gasForProcessing to same value"
        );
...
contract BFDOGEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor()
        public
        DividendPayingToken("BFDOGE_Dividend_Tracker", "BFDOGE_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "BFDOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(
            false,
            "BFDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BFDOGE contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "BFDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "BFDOGE_Dividend_Tracker: Cannot update claimWait to same value"
        );
