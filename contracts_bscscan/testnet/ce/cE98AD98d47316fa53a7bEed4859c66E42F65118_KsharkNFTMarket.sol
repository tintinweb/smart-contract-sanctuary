...
contract KsharkNFTMarket is IERC721Receiver, ReentrancyGuard {
  ...
  // boolean is have reward for seller or buyer
  ...
  function setDandyAddress(address addr)
    external
    onlyGovernance
    validAddress(addr)
  {
  ...
  function startSales(
    ...
    // Check start Time + duration > now
    require(startTime.add(durationTime) > now, "invalid start time");
    // Check durationTime > minDuration
    ...
    checkTime(index)
  {
    ...
    if (currencyAddr == address(0x0)) {
      // msg.value is ether send when call func has payable
      require(
