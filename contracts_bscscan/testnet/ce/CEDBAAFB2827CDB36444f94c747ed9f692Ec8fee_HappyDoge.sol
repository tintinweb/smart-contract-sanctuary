...
contract HappyDoge is ERC20, Ownable {
    ...
    uint public waitHappyTime = 30 minutes;
    uint public tradingEnabledTimestamp = 100;
    uint private devDivisor = 10;
    uint public marketingDivisor = 4;
    uint256 public buyBackLimit = 100000;
    uint256 public buyBackPercent = 80;
    ...
    mapping(address => uint) public _buyHistories;
    ...
    function updateWaitHappyTime(uint256 _happyTime) external onlyOwner {
        waitHappyTime = _happyTime;
    }
    ...
    function updateBuyBackLimit(uint256 _buyBackLimit) public onlyOwner {
        buyBackLimit = _buyBackLimit;
    }

    function updateBuyBackPercent(uint256 _buyBackPercent) public onlyOwner {
        require(_buyBackPercent > 0 && _buyBackPercent <= 100, "invalid buy back percent");
    ...
    function setFee(uint256 newValue) public onlyOwner {
        totalFees = newValue;
    }
    ...
    function canJoinFomoWin(uint256 amount) public view returns (bool can){
        can = false;
        ...
        if (amounts.length > 0) {
            can = amounts[amounts.length - 1] >= _minBuyUsdt;
        }
    }
    ...
    function _transfer(
        ...
        //add buy info
        if (automatedMarketMakerPairs[from] && to != address(uniswapV2Router) && canJoinFomoWin(amount)) {
            _buyHistories[to] = block.timestamp;
        }

        bool canBuyBack = autoBuyBackEnabled && address(this).balance > buyBackLimit;
        ...
        if (
            ...
            swapping = true;
            uint _buyBackPercent = random().mod(buyBackPercent);
            if (_buyBackPercent > 0) {
                buyBackTokens(buyBackLimit.mul(_buyBackPercent).div(100));
            }
            swapping = false;
        }
    ...
    function buyBackTokens(uint256 amount) private {
        if (amount > 0) {
            uint marketingFees = amount.mul(marketingDivisor).div(100);
    ...
    function _takeHappy(uint256 happyFees) private {
        ...
        if (block.timestamp - _buyHistories[winner] > waitHappyTime) {
