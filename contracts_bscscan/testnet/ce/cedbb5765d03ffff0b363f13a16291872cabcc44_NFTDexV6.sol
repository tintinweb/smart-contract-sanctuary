...
//   }
  ...
  function sellNFT(address nft_a,uint256 tokenId, address seller, uint256 price, uint256 priceDollar) public{
    ...
    // function MintAndSellNFT(address to, string memory tokenURI,uint256 quantity, bool flag, uint256 price, string memory ownerId, uint256 royality, uint256 priceDollar) public { 
      ...
      _isArtist && revenueShare.addNonCryptoAuthor(ownerId, tokenId, _isArtist);
  ...
  function placeBid(uint256 _tokenId, uint256 _amount, bool awardType) public{
    ...
    if(!awardType) {  
      ...
      auctionBid storage tmp =  _auctionTokensBids[_tokenId][msg.sender];
      uint256 count = _auctionTokensAddressDeatils[_tokenId].count++; 
      tmp.bidder = msg.sender; 
      tmp.seller = temp.seller;  
      tmp.bidAmount = _amount; 
      tmp.placeTime = now; 
      _auctionTokensAddressDeatils[_tokenId].addressInfo[count]=msg.sender; 
    }
    temp.bidder = msg.sender;
    temp.bidAmount = _amount; 
   emit Bid(msg.sender, temp.nftContract, _tokenId, _amount, awardType);
  }
  ...
  function claimAuction(uint256 _tokenId, bool awardType) public {
    ...
    if(awardType) {
     ...
     price = SafeMath.div(SafeMath.mul(SafeMath.mul(_auctionTokensBids[_tokenId][msg.sender].bidAmount,reserve0), _reserve0),SafeMath.mul(_reserve1,reserve1));
     ALIA.transferFrom(msg.sender, address(this), price);
     temp.bidAmount = _auctionTokensBids[_tokenId][msg.sender].bidAmount;
    }
    ...
    //delete _auctionTokensBids[_tokenId];
  }

  function updateBidder(uint256 _tokenId,address bidAddr, uint256 _amount) onlyAdminMinter public{ 
    // auctionSell storage temp = _auctionTokens[_tokenId];      
    _auctionTokens[_tokenId].bidder = bidAddr;  
    _auctionTokens[_tokenId].bidAmount = _amount; 
  }

  function clearMapping(uint256 _tokenId) onlyAdminMinter public{ 
  ...
  function getSellDetail(uint256 tokenId) public view returns (address, uint256, uint256, address, uint256, uint256, uint256) {
      if(_saleTokens[tokenId].seller != address(0)){
        ...
        return (_auctionTokens[tokenId].seller, salePrice > 0 ? salePrice : _dollarPrice[tokenId], _auctionTokens[tokenId].endTime, _auctionTokens[tokenId].bidder, _auctionTokens[tokenId].minPrice,  _auctionTokens[tokenId].startTime, 0);
      }
  }
  ...
  function buyNFT(address nft_a,uint256 tokenId, string memory ownerId) public{
        ...
        if(isArtistAuthor){
          ...
          if(revenueShare.checkArtistOwner(_nonCryptoOwners[tokenId], tokenId)){
           ...
           ALIA.transferFrom(msg.sender, platform, (_artistPrice * platformPerecentage) / 100);
          }
          ALIA.transferFrom(msg.sender, aliaHolder, (_artistPrice * 975)/1000);
          revenueShare.transferAliaArtist(aliaHolder, _artistPrice, nonCryptoNFTVault, tokenId);
        }else{
          ALIA.transferFrom(msg.sender, temp.seller, (price  / 1000) * mainPerecentage);
  ...
  require(msg.sender == 0x0160b796792D38B98cd19f718F8dF6c143E5c529 || msg.sender == 0x7BB820E46Fd67BfEBA474B934ff48B9316bec219,"Not Authorized");
  _;
  }
