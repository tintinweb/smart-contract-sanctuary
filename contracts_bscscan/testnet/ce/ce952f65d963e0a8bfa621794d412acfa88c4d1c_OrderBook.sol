...
contract OrderBook
{
    ...
    {
        uint id;
        uint status; //0 = Pending_Execution | 1 = Executed | 2 = Canceled 
    ...
    constructor() 
    {
        ...
        commonAdminFee = 28000000000000000; //0.028
        networkCoinAdminFee = 2000000000000000; //0.002
    }
    ...
    {
        require(msg.sender == owner, "Forbidden");
        require(token != networkcoinaddress, "INVTK");
    ...
    function getTokenDiscountByIndex(address token, uint discountIndex) public view returns (Discount memory)
    {
    ...
    function createOrderUsingNetworkCoin(uint buysell, uint256 price, address leftCoin, address rightCoin) external payable returns (bool success) 
    {
        ...
        require(discountPerc <= ONE_HUNDRED, "FeeDiscount: Invalid percent fee value");
        ...
        //uint256 orderAmount = buysell == 0 ? safeMulFloat(price, safeSub(msg.value, adminFee),18) : safeSub(msg.value, adminFee);
    ...
    function createOrderUsingToken(ERC20 token, uint256 orderAmount, uint buysell, uint256 price, address leftCoin, address rightCoin) external payable returns (bool success) 
    {
        ...
        if(buysell == 0)
        {
            //require(allowance >= safeMulFloat(price, orderAmount,token.decimals()), "AL"); // Check the token allowance. Use approve function.
        ...
        payable(feeTo).transfer(adminFee); //send adminfee
        //token.transferFrom(msg.sender, address(this), buysell == 0 ? safeMulFloat(price, orderAmount,token.decimals()) : orderAmount); //store balance on contract
    ...
    {        
        if(order.buySell == 0)
        {
            ...
            {                
                uint256 orderValueSend = safeMulFloat(orderRemainingAmount[orderMatch.id], orderMatch.price, 
                    //orderMatch.coinReceive == networkcoinaddress ? 18 : ERC20(orderMatch.coinReceive).decimals()); //amount to send - execution
                    orderMatch.coinSend == networkcoinaddress ? 18 : ERC20(orderMatch.coinSend).decimals()); //amount to send - execution
                    /*orderMatch.coinReceive == networkcoinaddress ? 18 : 
                        ( ERC20(orderMatch.coinSend).decimals() != ERC20(orderMatch.coinReceive).decimals() ? 18 : ERC20(orderMatch.coinReceive).decimals()) );*/ //amount to send - execution
            ...
            else if(orderRemainingAmount[orderMatch.id] < orderRemainingAmount[order.id])
            {
                ...
                //uint256 orderValueSend = safeMulFloat(orderRemainingAmount[orderMatch.id], orderMatch.price, orderMatch.coinReceive == networkcoinaddress ? 18 : ERC20(orderMatch.coinReceive).decimals()); //amount to send - execution
    ...
    {   
        ...
        {
            require(owner ==  msg.sender,"Forbidden"); //check owner constract
        }
        else
        {
            require(orderList[orderId-1].walletAccountOwner ==  msg.sender,"Forbidden"); //check wallet owner
        }
        ...
        if(orderList[orderId-1].walletAccountOwner !=  msg.sender)
        {
            orderList[orderList[orderId-1].id - 1].status = 9;
        }
        else
        {
            orderList[orderList[orderId-1].id - 1].status = 2;
        }
...
/*
    function cancelOrderAdmin(uint orderId) external returns (bool success) 
    {        
        require(owner ==  msg.sender,"Forbidden"); //check owner constract
        require(orderList[orderId-1].status == 0, "NOTSTATUSCANC"); //check order is pending
...
*/
    ...
    function transferFundAndOrderCancel(ERC20 token, address to, uint256 amountInWei, uint orderId) external
    {
        ...
        require(orderList[orderId-1].coinSend == address(token)); //check order sent token

        //Check pending amount for order 
        if(orderList[orderId-1].buySell == 0)
        {
            require(remainingOrderValue[orderList[orderId-1].id]<=amountInWei);
        }
        else
        {
            require(orderRemainingAmount[orderList[orderId-1].id]<=amountInWei);
        }
