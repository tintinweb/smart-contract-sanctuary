...
contract Loot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    bytes32 public constant MINT_TYPEHASH = keccak256(
        "Mint(address loot,address to,uint256 tokenId,uint256 deadline)"
    );
    ...
    function mintHash(address loot, address to, uint tokenId, uint256 deadline) internal pure returns (bytes32) {
        return keccak256(abi.encode(
            MINT_TYPEHASH,
            loot,
            to,
    ...
    function claim(uint256 tokenId, bytes memory ownerSig, uint256 deadline) public nonReentrant {
        require(owner() == recoverEIP712(mintHash(address(this), _msgSender(), tokenId, deadline), ownerSig), "invalid owner signature");
        require(deadline >= block.timestamp, 'WYNLoot: EXPIRED');
    ...
    constructor() ERC721("WYN Loot", "WYN Loot") Ownable() {}
}
