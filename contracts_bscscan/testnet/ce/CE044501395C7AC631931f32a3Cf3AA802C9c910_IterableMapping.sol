...
/* 
    ...
    _Welcome to Translunar_
        The cryptocurrency for interplanetary human beings


    _How we designed Translunar_
        Translunar coin exonomics are designed to reward long term holders, 
        avoid fraudulent market activity, establish a functional anti-whale
        system and to ensure maximal gain potential for all holders.    
        
        We are happy to have you on board for the mission.
    
    
    _Visit us_
        https://translunarcoin.com/
        https://t.me/translunarcoin/
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;  //Wrapped BNB (WBNB)
...
contract Translunar is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public _dividendToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; //Wrapped BNB (WBNB)
    ...
    TransLunarDividendTracker public dividendTracker;
    ...
    constructor() ERC20("TranslunarCoin", "TLC") {
    	...
    	dividendTracker = new TransLunarDividendTracker();

    	buyBackWallet = 0xfaD7e634b3C26C6A7734d923601CB4982B3A020C; 

    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); 
        // Testnet Address: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "TransLunarCoin: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "TransLunarCoin: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "TransLunarCoin: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "TransLunarCoin: The dividend tracker already has that address");

        TransLunarDividendTracker newDividendTracker = TransLunarDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TransLunarCoin: The new dividend tracker must be owned by the TransLunarCoin token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "TransLunarCoin: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TransLunarCoin: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TransLunarCoin: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TransLunarCoin: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TransLunarCoin: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "TransLunarCoin: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TransLunarCoin: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TransLunarCoin: Cannot update gasForProcessing to same value");
...
contract TransLunarDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("TransLunarCoin_Dividend_Tracker", "TransLunarCoin_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "TransLunarCoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "TransLunarCoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TransLunarCoin contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TransLunarCoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TransLunarCoin_Dividend_Tracker: Cannot update claimWait to same value");
