...
contract AdvarToken is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxnAmount = 3000000 * 10 ** 9;
    uint256 private numTokensSellToAddToLiquidity = 5000000 * 10**6 * 10**9;

    uint256 public taxFee = 10; // 10% tax for transfer
    ...
    modifier antiBot {
        require(block.number.sub(lastBlock[_msgSender()]) >= 3, "ADVAR V2: This is Bot");
        _;
    ...
    function _transfer(address from, address to, uint256 amount) private antiBot {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        _takeDev(taxAmount.mul(devFee).div(10));
        _takeMarket(taxAmount.mul(marketingFee).div(10));
        _takeLiquidity(sender, taxAmount.mul(liquidityFee).div(10));
    }
    ...
    function _takeMarket(uint256 amount) private {
