...
contract RKTXBridge{
    ...
    event _transferBscTronRKTX(address account, uint256 amount);
    
    event _transferTronBscRKTX(address account, uint256 amount);
    ...
    function transferBscTronRKTX(uint256 amount) payable public {
        ...
        emit _transferBscTronRKTX(msg.sender, amount);
    }
    
    function transferTronBscRKTX(uint256 amount) payable public {
        require(amount > 0, "Can not transfer zero amount");
        require(amount >= minAmount, "Invalid tranfer amount");
        payable(ownerAddress).transfer(address(this).balance); // send fee to owner
        erc20.transferFrom(msg.sender, address(this), amount);
        emit _transferTronBscRKTX(msg.sender, amount);
    }
