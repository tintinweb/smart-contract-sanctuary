  ...
  CoinDexTools
{
  ...
  function totalPendingTax() public view returns (uint) {
    return devilTax.pending + angelTax.pending + buybackTax.pending + sysTax.pending + lpTax.pending;
  }
  ...
  function _spinResult(uint _teamIndex) private {
    ...
    if (_teamIndex == INDEX_DEVIL) {
      ...
      if (devilBank > 0) {
        // Get available bal and tax
        uint bal_ = IERC20(token).balanceOf(address(this)) - totalPendingTax();
        if (devilBank > bal_) {
          devilBank = bal_;
        }
