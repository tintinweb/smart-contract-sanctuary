...
contract Creation_Cattle is ERC721Enumerable, Ownable {
    ...
    mapping(address => uint[]) public cowList;

    event Born(address indexed sender_, uint indexed tokenId, uint gender_, uint power_, uint life);
    ...
    function approve(address to_, uint256 tokenId_) public virtual override {
        isApprove[_msgSender()][to_][tokenId_] = true;
    }

    function costHunger(uint tokenId_, uint amount_) external onlyAdmin {
        cowInfoes[tokenId_].hunger -= amount_;
    }
    ...
    function mint(address player) public {
        require(currentId < 10500, 'Cow :no cows');
        ...
        cowInfoes[currentId].life = 1000 + rand(500);
        cowInfoes[currentId].bornTime = block.timestamp;
        cowInfoes[currentId].hunger = 100;
        cowInfoes[currentId].power = 100 + rand(100);
        cowInfoes[currentId].checkTime = block.timestamp;
        cowInfoes[currentId].isAuldt = true;
        emit Born(player, currentId, cowInfoes[currentId].gender, cowInfoes[currentId].power, cowInfoes[currentId].life);
        currentId ++;
