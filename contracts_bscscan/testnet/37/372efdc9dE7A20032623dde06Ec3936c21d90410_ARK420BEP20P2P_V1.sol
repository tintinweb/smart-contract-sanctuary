...
contract ARK420BEP20P2P_V1 is Ownable1 {    
    struct Trade {
        address initiator;
    ...
    constructor(address ark420Wbnb, address support, uint _supportRewardRate) {
        require(Address.isContract(ark420Wbnb), "ARK420BEP20P2P_V1: Not contract");
        ARK420_WBNB = IARK420_WBNB(ark420Wbnb);
        require(support != address(0), "ARK420BEP20P2P_V1: Zero address");
        supportAddress = support;
        require(_supportRewardRate >= 0 && _supportRewardRate <= 100, "ARK420BEP20P2P_V1: Reward rate should be from 0 to 100");
    ...
    function createTradeBNB(address askedAsset, bool proposedAssetVest, uint askedAmount, uint minTradeAskedAmount, uint maxTotalPurchaseAskedAmount, address[] memory rewardWallets, uint deadline) onlyOwner payable external returns (uint tradeId) {
        require(Address.isContract(askedAsset), "ARK420BEP20P2P_V1: Not contract");
        ARK420_WBNB.deposit{value: msg.value}();
        tradeId = _createTrade(address(ARK420_WBNB), msg.value, askedAsset, proposedAssetVest, askedAmount, minTradeAskedAmount, maxTotalPurchaseAskedAmount, rewardWallets, deadline);   
    }
    ...
    function supportTradeBNB(uint tradeId, uint partialAmount) payable external lock {
        ...
        uint toSupport = partialAmount * supportRewardRate / 100;
        if (supportRewardRate > 0)
        TransferHelper.safeTransferBNB(supportAddress, toSupport);

        if (trade.rewardWallets.length > 0) {
            uint toRewards = partialAmount - toSupport;
            for (uint i = 0; i < trade.rewardWallets.length; i++) {
                TransferHelper.safeTransferBNB(trade.rewardWallets[i], toRewards / trade.rewardWallets.length);
            }
        } else TransferHelper.safeTransferBNB(trade.initiator, partialAmount);
