    ...
    OwnableUpgradeable
{
        ...
        returns (bool)
    {
        ...
        require(
            WolfPacksNFT(WolfPackNFTContractAddress).ownerOf(_wolfPackId) == msg.sender,
            "You are not the owner"
        );

        // FUNCIÓN DE COMPROBACIONES DEL ATAQUE
        ...
        // Llamar a la función de check checkWolfPack
        checkWolfPack(pointsOfWolfPack, _positionToAttack);
    ...
    function checkWolfPack(uint256 _pointsOfWolfPack, uint256 _positionToAttack)
        internal
        returns (bool)
    {
        ...
        require(
            _pointsOfWolfPack >= animalsPointsToAttack,
            "Not enough points to attack this animal"
        );

        // TODO: Comprobar lastHunting

        // TODO: Comprobar la vida de la manada ¿?

        return true;

    }

    function getWinProbability(uint256 _pointsOfWolfPack, uint256 _positionToAttack ) public returns (uint256) {
        ...
        // TODO: Descomentar ambos para seguir trabajando (Ahora no abre el nodo por errores)
        //uint256 excessPoints = animalsPoints - animalsProbability;
    ...
    /*
    function generateResult(uint256 _tokenId, bytes32 _seed)
        external
        returns (bool)
    {
        require(_tokenId != 0, "Not allowed");
        require(
        ...
        emit GeneratedResultNFT(_tokenId);
    ...
    ReentrancyGuardUpgradeable
{
    ...
    // Mapping para ver si la información del lobo ha sido generada
    // TODO: Añadir a los test
    mapping(uint256 => bool) public generated;

    event MintedNFT(address indexed to, uint256 indexed id);
    event GeneratedNFT(uint256 indexed id);

    // VER QUÉ OTROS EVENTOS NECESITA EL FRONT
    // POR EJEMPLO, EL CAMBIO DE PRECIOS

    // TODO:  FALTA MODIFICADOR
        ...
        returns (bool)
    {
        ...
        require(
            msg.value >= ((gasToMinter * _amount) + commission),
            "Not enough gas"
        );
        ...
        returns (uint256[] memory)
    {
        ...
        // properties[6] = generated[_tokenId];
    ...
    function generateValuesWolf(uint256[] memory _tokenIds, bytes32 _seed)
        ...
        returns (bool)
    {
        // IBAI:  Preguntar cómo haría para implementar los require con un for en el caso de pasar un array
        for (uint256 index = 0; index < _tokenIds.length; index++) {
            ...
            // Este casting se podría subir hasta la raíz?
            ...
            (
                ...
                // !!! Muy importante: Añadir un keccak256 con el index para aleatoriezar los lobos
                // Pero no sé como hacer para modificar el seed
            ) = wolfNFTHelper.generateWolf(_seed );

            /*
            console.log(breedGenerated);
            console.log(genderGenerated);
            console.log(levelGenerated);
            console.log(attackGenerated);
            console.log(defenseGenerated);
            */
            ...
            // No tengo claro que los eventos funcionen dentro del for.
            // Igual hay que devolver un evento con el array de salida.
    ...
    function changeCommission(uint256 _newValue)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function changeBoxPriceCWOLF(uint256 _newValue)
        external
        onlyOwner
        returns (bool)
    {
        boxPriceCWOLF = _newValue;
        return true;
    }
    ...
    // TODO:  Función para modificar las propiedades del lobo desde el backend o DESDE UN CONTRATO AUXILIAR, por ejemplo, CAZA
    function modifyWolfProperties(uint256 _tokenId, uint256[6] memory _data)
        external
        returns (bool)
    {
        // require dirección backend
        return true;
    }

    // Esta función tiene que llamar al contrato MaterialsNFT y mirar si el lobo está utilizado
    /*
    function _beforeTokenTransfer(
        address _from,
        address _to,
        ...
        if (_from != address(0) && _to != address(0) && locked) {
            revert("Not allowed");
        }
    ...
    */
    ...
    // Función para transferir el lobo (mirar que no esté locked)

    // Función para cambiar las características del lobo (requerir address del backend ??? )

    // Precio de CWOLF obtenible desde otro contrato
    // El script tendría que consultar varias fuentes de datos, como PANCAKE, BIGQUERY Y 1Inch
    // Si hay diferencias significativas no actualizar y notificar.
}
    ...
    OwnableUpgradeable
{
    ...
    mapping(uint256 => uint256) public wolfPackLink;
    ...
    function mintWithCWOLF()
        ...
        returns (bool)
    {
        require(msg.value >= gasToMinter + commission, "Not enough gas");
        payable(minterWalletAddress).transfer(gasToMinter);
        payable(commissionWalletAddress).transfer(commission);
        payable(msg.sender).transfer(msg.value - (gasToMinter + commission));
        ...
        returns (bool)
    {
        ...
        ); //Wolf must be property of msg.sender
        require(
        ...
        // Sumar los puntos de ataque y defensa del lobo en wolfPackLife
        ...
        returns (bool)
    {
        ...
        ); // Material must be property of msg.sender
        require(
            getTotalMaterialsInWolfPack(_wolfPackId) < 20,
            "Limit 20 reached"
        ); // Limit of Materials per WolfPack < 20
    ...
    function buyEnergy(uint256 _wolfPackId, uint256 _amount) external {
        uint256 price = Variables(VariablesContractAddress).price();
        require((price * _amount) >= 1 ether, "The amount is not valid"); // TODO: Revisar esto
        wolfPackEnergy[_wolfPackId] =
            wolfPackEnergy[_wolfPackId] +
            (price * _amount);
        emit IncreasedEnergy(_wolfPackId, (price * _amount));
    }
    ...
    function buyWolfPackLink(uint256 _wolfPackId, uint256 _amountOfDays)
        external
    {
        require(wolfPackLink[_wolfPackId] == 0, "You cannot buy link now");
        require(
            configurationLinkDays[_amountOfDays] == true,
            "Error: Amount of days incorrect"
        );
        uint256 priceOfCWOLFInDollar = Variables(VariablesContractAddress).price();
        ...
        wolfPackLink[_wolfPackId] = wolfPackLink[_wolfPackId] + _amountOfDays;
        emit IncreasedLink(_wolfPackId, wolfPackLink[_wolfPackId]);
    }

    function decreasedWolfPackLink(
        uint256 _wolfPackId,
        uint256 _amountToDecrease
    ) public {
        require(
            wolfPackLink[_wolfPackId] - _amountToDecrease >= 0,
            "Error: Amount to decrease greater than wolfpack link"
        );
        wolfPackLink[_wolfPackId] =
            wolfPackLink[_wolfPackId] -
            _amountToDecrease;
...
contract WolfsNFTHelper is Initializable, OwnableUpgradeable {
    ...
    */
    ...
    function generateGender(bytes32 _seed) internal view returns (uint256) {
        uint256 randomNumber = Random.randomMinMax(_seed, 0, 1);
        ...
        returns (bool)
    {
        ...
        // TODO:  Revisar esta asignación directa
    ...
    function updateAttackDefenseLevels(uint256[][] memory _newData)
        ...
        returns (bool)
    {
        // Comprobar dimensiones y asignación
    ...
    OwnableUpgradeable
{
    ...
    event MintedNFT(address indexed _to, uint256 indexed _id);
    event GeneratedNFT(uint256 indexed _id);
    ...
    function generateValuesMaterials(uint256 _tokenId, bytes32 _seed)
        external
        returns (bool)
    {
        require(_tokenId != 0, "Not allowed");
        ...
        slots[_tokenId] = Random.randomMinMax(_seed, 1, 6);
        emit GeneratedNFT(_tokenId);
        return true;
    }
...
contract Variables is OwnableUpgradeable {
    ...
    function setCWolfPriceInDollars(uint256 _newPrice) public onlyOwner {
        price = _newPrice;
        emit PriceChanged(_newPrice);
    }

    function getDollarsInCWOLF() public view returns(uint256){
        return 1 / price;
    }
}