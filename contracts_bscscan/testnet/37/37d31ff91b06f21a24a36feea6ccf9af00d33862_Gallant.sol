...
contract Gallant is Context, IERC20, Ownable {
    ...
    constructor () {
        ...
        _isCommunity[address(this)] = true;
        ...
        putYourGenAway[Wallet_Burn] = true; 
        putYourGenAway[Wallet_Marketing] = true; 
        putYourGenAway[Wallet_Dev] = true; 
    ...
    //set Only Community Members <---whitelist!
    function safeLaunch_Pause_Contract(bool _enabled) public onlyOwner {
        contract_Paused = _enabled;
    }
    ...
    function fees_setFeeBurnPercent(uint256 FeeBurn) external onlyOwner() {
        if (FeeBurn < 11) {_FeeLiquidity = FeeBurn;}
    }
    ...
    function _transfer(
        ...
        if (to != owner() || from != owner()) {require(!contract_Paused, "Contract is currently paused.");}
        ...
        if (onlyCommunity){require(_isCommunity[to], "Sale restricted to whitelsited wallets! Pass your wallet to an admin, or come back later");}
