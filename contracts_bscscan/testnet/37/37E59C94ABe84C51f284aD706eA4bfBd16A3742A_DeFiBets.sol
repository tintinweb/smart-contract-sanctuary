...
contract DeFiBets is Context, IERC20, Ownable{
    ...
    function getTransferExclusions(address account) external view returns (bool) {
    ...
    function _transfer(
        ...
        // 2 min dealy for each buy this is valid only for initial 24h
         if (isLaunched && block.timestamp <= endSnipeLimitPeriod && from == pancakeV2Pair && !_isBot[to]) {
             //not required to check for bots
            if (lastBuyTime[to] <= endSnipeLimitPeriod - 2 minutes) {
                // require 2 min wait for 24hr for each buy
                require(
                    lastBuyTime[to] + 2 minutes < block.timestamp,
                    "Cooldown 2 min for buy"
                );
            }
