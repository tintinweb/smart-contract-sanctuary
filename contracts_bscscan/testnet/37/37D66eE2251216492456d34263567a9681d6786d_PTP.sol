...
contract PTP is ERC20, Ownable {
    ...
    mapping (address => referalsRecord[]) public referalUsersRecords;
    ...
    function claimStakeReward(uint256 id) public  {
        ...
        if(block.timestamp > myRecord[id].endDayTimeStamp){
            ...
            perDayDeduction = perDayDeduction.div(1000);
            uint256 subtractedValue = reward.div(100);
            subtractedValue = subtractedValue.mul(perDayDeduction);
            subtractedValue = subtractedValue.mul(dayAfterEnd);
            reward = reward.sub(subtractedValue);
    ...
    function createFreeStake(address user,uint balance, address referAddress) public onlyOwner{
        ...
        referalAddress[user] = referAddress;
      
    }

    function refreAddress(address sender) public view returns(address){
    ...
    function findDay() public view returns(uint) {
      ...
      function distributeFreeStake( ) public onlyOwner{
        ...
        address referalUserAddress = refreAddress(freeStakeUsers[i]);
        if(referalUserAddress == address(0)){
        uint256 stakeAmount = fixAmount.mul(68);
        stakeAmount = stakeAmount.div(100); 
        uint256 mintAmount = fixAmount.mul(32);
        ...
        uint256 stakeAmount = fixAmount.mul(80);
        stakeAmount = stakeAmount.div(100); 
        ...
        freeStakes[freeStakeUsers[i]] = freeStakes[freeStakeUsers[i]].add(stakeAmount);
        //Referal Bonus
        _mint(referalUserAddress,mintAmount);
        //User Bonus
        _mint(freeStakeUsers[i],mintAmount);
        createFreeStakeRecord(freeStakeUsers[i],mintAmount);
        createReferalRecords(freeStakeUsers[i],mintAmount,referalUserAddress);
        }
        }
    ...
    function createReferalRecords(address freeStakeHolder, uint256 mintedAmount, address referalUserAddress) internal onlyOwner{

        uint day = findDay();
        referalsRecord memory myRecord = referalsRecord({refferingAddress:freeStakeHolder,awardedPTP:mintedAmount,day:day});
        referalUsersRecords[referalUserAddress].push(myRecord);
    }


    function getReferalRecord(address userAddress) public view returns (referalsRecord[] memory referal) {
        
        return referalUsersRecords[userAddress];
    }
