...
contract Malbourne is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public  _launchTime =block.timestamp; 
    uint256[4] public _days=[5 minutes,8 minutes,11 minutes,13 minutes];

    string private _name = "Malbourne";
    string private _symbol = "Mal";
    uint8 private _decimals = 9;

    IPancakeRouter02 public pancakeRouter;
    address public immutable pancakePair;
    address payable public charityAddress=payable(0x1dce55E0382A075dd8ADc7A726984122145964AC);
    ...
    uint256 public _charityFee = 430;   
    uint256 private _previousCharityFee = _charityFee;
    
 
    
    uint256 public _burnFee = 20;  // 2% will go to the Dead address to deflate token
    ...
    function setCharityFeePercent(uint256 charityFee1,uint256 charityFee2,uint256 charityFee3,uint256 charityFee4 ) external onlyOwner(){
        _days[0] = charityFee1;
        _days[1] = charityFee2;
        _days[2] = charityFee3;
        _days[3] = charityFee4;
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_taxFee.add(_liquidityFee).add(_charityFee).add(_burnFee)).div(1e3);
        return percentage;
    }
    ...
    function _transfer(
        ...
        if(_isExcludedFromMaxTx[from] == false && 
          ...
          if( block.timestamp >= _launchTime &&  block.timestamp <= _days[0]){
                  _charityFee = 430; 
        }else if( block.timestamp > _launchTime + _days[0] && block.timestamp <= _launchTime + _days[1]){
                  _charityFee = 230; 
    	}else if(block.timestamp > _launchTime + _days[1] && block.timestamp <= _launchTime + _days[2]){
                  _charityFee = 180; 
    	}else if(block.timestamp > _launchTime + _days[2] && block.timestamp >= _launchTime + _days[3]){
                  _charityFee = 80; 
        } 
