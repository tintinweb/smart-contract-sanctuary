...
contract NaaSPrivate is NaaS {
    ...
    function mint(
        ...
        require(isMinter[data.minter], "NOT_ALLOWED");
    ...
    IERC1155Tradeable
{
    ...
    function mint(
        ...
        require(data.minter == _msgSender() || isApprovedForAll(data.minter, _msgSender()), "UNAUTHORIZED");
    ...
    function mintNFTs(
        ...
        for (uint256 tokenIdx = 0; tokenIdx < data.collection.tokens.length; tokenIdx++) {
            if (exists(data.collection.tokens[tokenIdx].id)) continue;

            LibERC1155Lazy.Token memory token = data.collection.tokens[tokenIdx];
            _setMultiTokenProperties(data, token, hash, false);
            ids[currentId] = token.id;
            amounts[currentId] = 1;
            currentId++;

            if (currentId == amount) break;
        }

        require(currentId == amount, "TOTAL_SUPPLY_EXCEEDED");
    ...
    function nftsLeft(
        ...
        uint256 tokensToCreate = data.collection.tokens.length;
        for (uint256 tokenIdx = 0; tokenIdx < data.collection.tokens.length; tokenIdx++) {
            if (exists(data.collection.tokens[tokenIdx].id)) tokensToCreate--;
        }
    ...
    function mintMultiple(
        ...
        for (uint256 idIdx = 0; idIdx < ids.length; idIdx++) {
            ...
            for (uint256 tokenIdx = 0; tokenIdx < data.collection.tokens.length; tokenIdx++) {
                ...
                _setMultiTokenProperties(data, token, hash, exists(token.id));
                found = true;
                break;
            }

            require(found, string(abi.encodePacked("TOKEN_NOT_FOUND")));
        }
    ...
    function _validateCreators(
        ...
        require(_creators.length > 0, "NO_CREATORS");
        require(_creators.length == signatures.length, "INVALID_SIGNATURES_COUNT");
...
abstract contract ERC1155Collection is ERC1155Proxy, IERC1155Collection {
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC1155Proxy, IERC165Upgradeable)
        returns (bool)
    {
        return
