...
contract DarkMatter is ERC20 ('DarkMatter', 'KDM'), MinterRole {
   ...
   event SetDeflationController(address indexed _addresss);
    ...
    uint256 public constant Max_Supply=85* (10 ** 6) * (10 ** 18); // 85.000.000 KDMs
    uint256 private _burnTotal;
    
    
    function mint(address _to, uint256 _amount) public onlyMinter {
        require(totalSupply().add(_amount) <= Max_Supply, "KDM: Minting this amount exceeds hard cap");
        _mint(_to, _amount);
  ...
  function setDeflationController(address _address ) external onlyMinter {
