...
library Address {
    ...
    function functionCallWithValue(address target, bytes memory data, uint256 value) internal returns (bytes memory) {
        return functionCallWithValue(target, data, value, "Address: low-level call with value failed. The calling contract must have an ETH balance of at least `value`. the called Solidity function must be `payable`.");
    }
...
interface IADAdistributor {
    function startADAdistribution() external;
    function setADAdistributionParameters(uint256 _minPeriod, uint256 _minDistribution, uint256 _gas) external;
...
contract perro is IERC20, Ownable {
    ...
    string constant _name = "perro";
    string constant _symbol = "perro";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1000000000 * (10 ** _decimals); // 1 billion tokens
    uint256 _maxBuyTxAmount = (_totalSupply * 100) / 100; //100% of the total supply
    uint256 _maxSellTxAmount = (_totalSupply * 100) / 100; //100% of the total supply
    uint256 _maxWalletSize = (_totalSupply * 100) / 100; //100% of the total supply
    ...
    uint256 distributionFee = 800; // 8.0% for ADA rewards to Holders
    uint256 marketingFee = 400; // 4.0%
    uint256 bonusRewardsFee = 200; // 2.0%
    uint256 totalFee = liquidityFee + distributionFee + marketingFee + bonusRewardsFee; // 16.0%
    uint256 feeDenominator = 10000;
    uint256 paperhandsFee = 400; //4.0% additional fee on sells

    address public autoLiquidityReceiver;
    address payable public marketingFeeReceiver;
    address payable public bonusRewardsReceiver;
    ...
    bool startYourEngines = false;
    uint256 public rateLimit = 0 seconds; 

    IADAdistributor public adadistributor;
    bool public autoADAStream = true;
    uint256 lastStream;
    uint256 streamPeriod = 2 minutes; 
    ...
    function openTrading(uint256 _deadBlocks) external onlyOwner {
        require(!startYourEngines && _deadBlocks < 7);
        deadBlocks = _deadBlocks;
        startYourEngines = true;
        launchedAt = block.number;
    }
    
    function startADAdistribution() external onlyOwner {
        adadistributor.startADAdistribution();
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!startYourEngines){ require(liquidityCreator[sender] || liquidityCreator[recipient], "Trading not open yet."); }
        ...
        if(!isDividendExempt[sender]){ try adadistributor.setShares(sender, _balances[sender]) {} catch {} }
        if(!isDividendExempt[recipient]){ try adadistributor.setShares(recipient, _balances[recipient]) {} catch {} }

        if (processEnabled)
            try adadistributor.process() {} catch {}
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        if (selling) return totalFee + paperhandsFee; 
        return totalFee;
    }
    ...
    function swapBack(uint256 amount) internal swapping {
        ...
        uint256 amountBNBBonus = (amountBNB * bonusRewardsFee) / totalBNBFee;

        if (autoADAStream && balanceBefore > 0 && lastStream + streamPeriod < block.timestamp) {
            uint256 adaStream = (balanceBefore * streamPercent) / 100;
            try adadistributor.deposit{value: adaStream}() {} catch {}
        }
        try adadistributor.deposit{value: amountBNBDistribution}() {} catch {}
        lastStream = block.timestamp;
        
        marketingFeeReceiver.transfer(amountBNBMarketing);
        bonusRewardsReceiver.transfer(amountBNBBonus);
        ...
        emit FundsDistributed(amountBNBLiquidity, amountToLiquify, amountBNBDistribution, amountBNBMarketing, amountBNBBonus);
    }
    
    function setup(address _bonusRewardsReceiver, address distributor) external onlyOwner {
        require(!launched());
        //0x3ee2200efb3400fabb9aacf31297cbdd1d435d47 - Binance Pegged ADA contract address
        adadistributor = IADAdistributor(distributor);
        bonusRewardsReceiver = payable(_bonusRewardsReceiver);
    }
    
    function updateDistributor(address distributor) external onlyOwner {
        emit UpdatedSettings('Migrated Distributor', [Log(concatenate('Old Distributor: ',toString(abi.encodePacked(address(adadistributor)))), 1),Log(concatenate('New Distributor: ',toString(abi.encodePacked(distributor))), 1), Log('', 0)]);
        adadistributor.migrate(distributor);
        adadistributor = IADAdistributor(distributor);
    }
    ...
    function manualStream(uint256 _percent) external onlyOwner {
        uint256 adaStream = (address(this).balance * _percent) / 100;
        adadistributor.deposit{value: adaStream}();
        lastStream = block.timestamp;
    }
    ...
    function resetShares(address shareholder) external onlyOwner {
        if(!isDividendExempt[shareholder]){ adadistributor.setShares(shareholder, _balances[shareholder]); }
        else adadistributor.setShares(shareholder, 0);
    }
    ...
    function setIsDividendExempt(address holder, bool exempt) external onlyOwner {
        ...
        if(exempt){
            adadistributor.setShares(holder, 0);
        }else{
            adadistributor.setShares(holder, _balances[holder]);
        }
    ...
    function setFees(uint256 _liquidityFee, uint256 _distributionFee, uint256 _marketingFee, uint256 _bonusRewardsFee, uint256 _feeDenominator, uint256 _paperhandsFee) external onlyOwner {
        require(_bonusRewardsFee * 100 / _feeDenominator <= 1);
        liquidityFee = _liquidityFee;
        distributionFee = _distributionFee;
        marketingFee = _marketingFee;
        bonusRewardsFee = _bonusRewardsFee;
        totalFee = _liquidityFee + _distributionFee + _marketingFee + _bonusRewardsFee;
        paperhandsFee = _paperhandsFee;
        require(paperhandsFee <= totalFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator / 4);
        emit UpdatedSettings('Fees', [Log('Total Buy Fee Percent', (totalFee - paperhandsFee) * 100 / feeDenominator), Log('Total Sell Fee Percent', (totalFee + paperhandsFee) * 100 / feeDenominator), Log('Distribution Percent', _distributionFee * 100 / feeDenominator)]);
    }
    ...
    function setAdaStreamSettings(bool _enabled, uint256 _period, uint256 _percent) external onlyOwner {
        require(_percent <= 100);
        autoADAStream = _enabled;
        streamPeriod = _period;
        streamPercent = _percent;
        emit UpdatedSettings('AdaStream Settings', [Log('Enabled', _enabled ? 1 : 0),Log('Period', _period), Log('Percent', _percent)]);
    }
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 gas) external onlyOwner {
        ...
        adadistributor.setADAdistributionParameters(_minPeriod, _minDistribution, gas);
    ...
    function getPoolStatistics() external view returns (uint256 totalAmount, uint256 totalClaimed, uint256 holders) {
        totalAmount = adadistributor.getTotalRewards();
        totalClaimed = adadistributor.getTotalRewarded();
        holders = adadistributor.countShareholders();
    }
    
    function getWalletStatistics(address wallet) external view returns (uint256 pending, uint256 claimed) {
	    pending = adadistributor.getUnpaidRewards(wallet);
	    claimed = adadistributor.getPaidRewards(wallet);
	}

	function claimDividends() external {
	    adadistributor.claim(msg.sender);
	    try adadistributor.process() {} catch {}
	}
