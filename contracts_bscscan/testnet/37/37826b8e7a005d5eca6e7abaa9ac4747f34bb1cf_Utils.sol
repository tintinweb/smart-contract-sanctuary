...
contract Dalmatian is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public constant CURELLA_WALLET = payable(address(0xE3d3D329e2287A7922D35F1D23e0FC146a7c25C9));
    address public constant MARKETING_WALLET = address(0xcA3c07f6764b7c6Ce9555279dE9ec82455e12725);
    ...
    uint256 public _maxTxAmount = _tTotal; // should be 0.005% percent per transaction, will be set again at activateContract() function
    uint256 public disruptiveCoverageFee = 4 ether; // antiwhale
    ...
    bool public reflectionFeesdisabled = false;//this means enabled for now
   
    uint256 public _taxFee = 0;
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 0; // 2.5% will be added pool, 2.5% will be sent to marketing, 2% for buyback
    uint256 private _previousLiquidityFee = _liquidityFee;
    
    uint256 public _bnbFee = 0; // 10% for the BNB rewards
    uint256 private _previousBNBFee = _bnbFee;
    
    uint256 private totalCollectedFees = 0; // For helping in collected fees calculation
    uint256 public minTokenNumberToSell = _tTotal.mul(1).div(100000); // 0.001% max tx amount will trigger swap and add liquidity

    string private _name = "Dalmatian INU";
    string private _symbol = "DALMI";
    ...
    constructor (
        ...
        _isExcludedFromFee[CURELLA_WALLET] = true;
        ...
        _isExcludedFromMaxTx[address(CURELLA_WALLET)] = true;
    ...
    ///@dev Returns the total tax Fee collected by the contract
    ...
    ///@dev Returns reflections from the amount of tokens and fees
    ...
    ///@dev The amount of tokens from reflections amount. Used in balance calculation
    ...
    ///@dev Exclude a user from tax fees reward
    ...
    ///@dev Exempt a user from tax fees
    function excludeFromFee(address account) external onlyOwner {
        _isExcludedFromFee[account] = true;
    }

    ///@dev Include a user in tax fees
    ...
    ///@dev Turn on the functionality to add liquidity, buyback and burn
  ...
  function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tBNB);
    ...
    function _transfer(
        ...
        //take fees if buy/sell
        if(from == address(pancakePair) || to == address(pancakePair)) {
            takeFee = true;

            //SELL
            if(to == address(pancakePair)) {
                //10% rewards for holders whenever someone sell
                _taxFee = 10;
        ...
        //transfer amount, it will take tax, BNB, liquidity fee
    ...
    ///@dev Returns the reward cycle time 
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        require(balanceOf(msg.sender) > 0, 'Error: must own DALMI to claim reward');

        uint256 reward = calculateBNBReward(msg.sender);

        //Fees for Curella Wallet
        (bool sentTreasury,) = address(CURELLA_WALLET).call{value : reward.div(10)}("");
        require(sentTreasury, 'Error: Cannot send to Curella Wallet');
    ...
    ///@dev To transfer more than max tx amount, pay additional 4 BNB to this function
    function disruptiveTransfer(address recipient, uint256 amount) external payable returns (bool) {
        _transfer(_msgSender(), recipient, amount, msg.value);
        return true;
    }

    ///@dev Buy Back -> Burn -> Add Liquidity -> Buy BNB to reward users
    function swapAndLiquify(address from, address to) private {
        ...
        !(from == address(this) && to == address(pancakePair)) // swap 1 time
            ...
            inSwapAndLiquify = true; // Security
            
            //to calculate totalCollectedFees in percent (% x 1000)
            uint256 feesPercent = totalCollectedFees.mul(1000000).div(balanceOf(address(this)));
            // only sell for minTokenNumberToSell, decouple from _maxTxAmount
            contractTokenBalance = minTokenNumberToSell;

            //liquidity Tokens 2.5%
            uint256 swapTokens = contractTokenBalance.mul(feesPercent).mul(5).div(9).div(1000000);

            //tokens for burn 2%
            uint256 burnTokens = contractTokenBalance.mul(feesPercent).mul(4).div(9).div(1000000);

            swapBurnAndLiquify(burnTokens,swapTokens);
            totalCollectedFees = totalCollectedFees.sub(swapTokens).sub(burnTokens);
            //This amount will be used to reward users
            uint256 sellTokens = contractTokenBalance.sub(swapTokens).sub(burnTokens);
            Utils.swapTokensForEth(address(pancakeRouter), sellTokens);
            
            inSwapAndLiquify = false;
        }
    }

    ///@dev BuyBack tokens and then add liquidity portion to liquidity
    function swapBurnAndLiquify(uint256 burnAmount,uint256 liq) private {
        ...
        // swap tokens for ETH (buyback and also for ETH)
        Utils.swapTokensForEth(address(pancakeRouter), EthPortion);
        ...
        //BuyBack and Burn
        Utils.swapETHForTokens(address(pancakeRouter), address(0xdEaD), burnAmountEth);

        //Adding to liquidity
        Utils.addLiquidity(address(pancakeRouter), owner(), liqPortion, liqBalance);
        
        emit SwapBurnLiquify(liqPortion, newBalance, liqBalance,burnAmountEth);
    }

    ///@dev To Activate the contract with the settings. Call the after deployement
    function activateContract() external onlyOwner {
        ...
        // protocol
        disruptiveCoverageFee = 4 ether;
        disruptiveTransferEnabledFrom = block.timestamp;
        setMaxTxPercent(5);// set to 0.005% of total Supply
        setSwapAndLiquifyEnabled(true);
         _taxFee = 10; // Fees for sell. Goes back to holders
         _previousTaxFee = _taxFee;
    
         _liquidityFee = 7; // 2.5% + 2.5% + 2%
         _previousLiquidityFee = _liquidityFee;
    
    
         _bnbFee = 11; // 10% BNB per transaction
         _previousBNBFee = _bnbFee;
    ...
    ///@dev To change the reward cycle time
