...
contract PlanetSandboxToken is ERC20, ReentrancyGuard, Ownable {
    ...
    function setTokenVestingTime(uint256 _vestingTime) public onlyOwner {
        require(_vestingTime >= block.timestamp, "PlanetSandbox: Token listing must be in future date");
    ...
    function addVestingToken(
        ...
        require(totalSupply().add(amount) <= TOTAL_SUPPLY, "PlanetSandbox: Max supply exceeded");
    ...
    function revokeVestingToken(address user) external onlyOwner {
        ...
        require(totalSupply().add(claimableAmount) <= TOTAL_SUPPLY, "PlanetSandbox: Max supply exceeded");
    ...
    function _getVestingClaimableAmount(address user) internal view returns (uint256 claimableAmount) {
        ...
        uint256 tgeRounds = ((block.timestamp.sub(info.startTimeVesting)).div(30 days)).add(1);

        if (info.tgePercent > 0) {
            if (tgeRounds <= info.releaseTgeRounds) {
                tgeReleasedAmount = info.amount.mul(info.tgePercent).div(100).mul(tgeRounds).div(info.releaseTgeRounds);
            } else {
        ...
        if (block.timestamp >= releaseTime) {
            uint256 roundsPassed = ((block.timestamp.sub(releaseTime)).div(info.daysPerRound.mul(1 days))).add(1);

            if (roundsPassed >= info.releaseTotalRounds) {
                roundReleasedAmount = info.amount.sub(tgeReleasedAmount);
            } else {
                roundReleasedAmount = info.amount.sub(tgeReleasedAmount).mul(roundsPassed).div(info.releaseTotalRounds);
            }
        }
    ...
    function withdrawERC20(address token, uint256 amount) external onlyOwner {
        require(amount > 0, "PlanetSandbox: Amount must be greater than 0");
        require(IERC20(token).balanceOf(address(this)) >= amount, "PlanetSandbox: ERC20 not enough balance");
        require(IERC20(token).transfer(_msgSender(), amount), "PlanetSandbox: transfer ERC20 failed");
    }
