...
// File: contracts/SuperBTCDividendPayingToken.sol
...
// File: contracts/SuperBTC.sol

// 

pragma solidity ^0.7.6;

contract SuperBTC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SuperBTCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SuperBTC", "SBTC") {
        ...
        dividendTracker = new SuperBTCDividendTracker();

        liquidityWallet = owner();
        marketingWallet = address(0x34cd7f2bADf0532181c554F1394725Ac41E15bD7);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SuperBTC: The dividend tracker already has that address");

        SuperBTCDividendTracker newDividendTracker = SuperBTCDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SuperBTC: The new dividend tracker must be owned by the SuperBTC token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SuperBTC: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SuperBTC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SuperBTC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SuperBTC: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SuperBTC: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SuperBTC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SuperBTC: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "SuperBTC: This account cannot send tokens until trading is enabled");
        }
...
contract SuperBTCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SuperBTC_Dividend_Tracker", "SuperBTC_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "SuperBTC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "SuperBTC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SuperBTC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SuperBTC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SuperBTC_Dividend_Tracker: Cannot update claimWait to same value");
