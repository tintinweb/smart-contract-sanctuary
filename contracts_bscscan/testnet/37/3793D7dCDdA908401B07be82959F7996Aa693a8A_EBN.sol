...
⭐️⭐️EBUY Network Features:
                 
- Auto yield and liquidity generation 
- Customize fees taken to reward holders
- Customize fees to generate liquidty
- Buyback function is turned on, tokens are bought back from the market, resulting in an immediate effect on the price.
- Burn The tokens bought through buyback are immediately burned. This creates a true burn and guarantees the price per token will increase every time a buyback is activated.
- Total Supply - 10 Trillion


Please note there are always risks associated with smart-contracts. Please use at your own risk. EBN Token is not a registered broker, analyst or investment advisor. Every functionality that we provide on this smart contract is purely for experiment and educational purposes. We do not accept any liability for any loss or damage whatsoever caused in reliance upong using this smart contract 
...
contract EBN is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using EnumerableSet for EnumerableSet.AddressSet;
    
    address payable public marketingAddress = payable(0x8B99F3660622e21f2910ECCA7fBe51d654a1517D); // Marketing Address to be given here.
    ...
    string private _name = "EBUY Network";
    string private _symbol = "EBN";
    ...
    function tokenEmergencySafe(IBEP20 token,address account,uint256 amount) public onlyOwner {
    ...
    function setMarketingAddress(address _marketingAddress) external onlyOwner() {
        require(_marketingAddress != address(0), "unable to set zeroth address");
