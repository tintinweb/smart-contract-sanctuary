...
contract LOTEST3 is Context, IERC20, Ownable { 
    ...
    bool public antiSniperArmed = false;
...
//        uint256 sTeamFee
        ...
        _developmentFee = bDevelopmentFee;
        _devFee = bTeamFee;
        uint256 enoughFees = (bLiquidityFee + bMarketingFee + bDevelopmentFee + bTeamFee);
        require (enoughFees <= MAX_FEE_RATE, "Total fees must 40% at max");
        _totalBNBFees = enoughFees;
        _previousTotalFee = enoughFees;
 /*     newTotalFees = newLiquidityFee.add(newMarketingFee).add(newDevFee).add(newTeamFee).add(newReflectionFee);
        require(newTotalFees <= MAX_FEE_RATE, "Total Fees Cannot Be Over 40%");
        previousReflectionsFee = newReflectionFee;
        previousBuyBackAndLiquidityFee = newLiquidityFee;
        previousMarketingFee = newMarketingFee;
        previousDevelopementFee = newDevFee;
        previousDevTeamFee = newTeamFee;
 */       
    }
    
    function setTradingIsEnabled(bool _enabled, uint256 sniperBlocks) external onlyOwner {
    uint256 checkinOnSomething = block.timestamp;
    antiSniperBlock = (checkinOnSomething + sniperBlocks);
...
//FIX IS MY SWAP AND LIQ FUNCTION STOPPING TRADING
        if (snipeBalance > 0) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(!takeFee && (antiSniperArmed = false)) {
            restoreAllFee();
        }
    }
//FIX Change taxes and check sniper time in calculateLiquidityFee function
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        // FIX THIS IS WHERE THE FEE IS GETTING CALCULATED LAST POINT OF ENTRY BEFORE GETTING FEES  TURNED OFF SWAPANDLIQ ALSO
        if (antiSniperArmed = true) {
            _whatTimeIsIt = block.timestamp;
            if (antiSniperBlock >= _whatTimeIsIt) {
            _totalBNBFees = 30;
            } else if (antiSniperBlock <= _whatTimeIsIt) {
            _previousTotalFee = _marketingFee + _developmentFee + _devFee + _liquidityFee;
            _totalBNBFees = _previousTotalFee;
            _reflectionFee = _previousReflectionFee;
