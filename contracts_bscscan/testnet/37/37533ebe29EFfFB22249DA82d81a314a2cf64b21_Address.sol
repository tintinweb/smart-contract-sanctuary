...
contract YuGiOhToken is Context, IERC20, Ownable {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tBurn, uint256 tLiquidity, uint256 tMarketing) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rBurn) = _getRValues(tAmount, tBurn, tLiquidity, tMarketing, _getRate());
        return (rAmount, rTransferAmount, rBurn, tTransferAmount, tBurn, tLiquidity, tMarketing);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        return (tTransferAmount, tBurn, tLiquidity, tMarketing);
    }

    function _getRValues(uint256 tAmount, uint256 tBurn, uint256 tLiquidity, uint256 tMarketing, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rMarketing = tMarketing.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rBurn).sub(rLiquidity).sub(rMarketing);
