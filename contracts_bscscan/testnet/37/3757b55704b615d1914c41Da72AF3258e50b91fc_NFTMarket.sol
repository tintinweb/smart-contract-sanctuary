...
contract NFTMarket is ReentrancyGuard, Ownable{
  ...
  function buyItem(
    ...
    uint256 amountTominter = (msg.value)*(royalty)/(100);
    uint256 amountToSeller = (msg.value)-(amountTominter);
  ...
  function EndSale(uint256 itemId) public nonReentrant{
      require(msg.sender==idToMarketItem[itemId].seller && idToMarketItem[itemId].sold == false);
  ...
  function fetchMarketItems() public view returns (MarketItem[] memory) {
    ...
    for (uint i = 0; i < itemCount; i++) {
      if (idToMarketItem[i + 1].owner == treasury) {
        uint currentId = i + 1;
