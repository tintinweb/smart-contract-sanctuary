...
contract MockedGaur is Gaur {
  constructor(uint256 _taxRate, address _taxCollectorAddress) public Gaur(_taxRate, _taxCollectorAddress) {}
...
contract Gaur is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 GAUR-WETH LP -> GAUR pool
    uint256 public constant INITIAL_GAUR_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the GAUR ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("gaur.money", "GAUR") {
        // Mints 1 GAUR to contract creator for initial pool setup
    ...
    function _getGaurPrice() internal view returns (uint256 _gaurPrice) {
        try IOracle(gaurOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Gaur: failed to fetch GAUR price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _gaurPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_gaurPrice >= taxTiersTwaps[tierId]) {
    ...
    function enableAutoCalculateTax() external onlyTaxOffice returns (bool) {
        autoCalculateTax = true;
        return true;
    }

    function disableAutoCalculateTax() external onlyTaxOffice returns (bool) {
        autoCalculateTax = false;
        return true;
    }

    function setGaurOracle(address _gaurOracle) external onlyOperatorOrTaxOffice returns (bool) {
        require(_gaurOracle != address(0), "oracle address cannot be 0 address");
        gaurOracle = _gaurOracle;
        return true;
    }

    function setTaxOffice(address _taxOffice) external onlyOperatorOrTaxOffice returns (bool) {
    ...
    function setTaxCollectorAddress(address _taxCollectorAddress) external onlyTaxOffice returns (bool) {
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");
        taxCollectorAddress = _taxCollectorAddress;
        return true;
    }

    function setTaxRate(uint256 _taxRate) external onlyTaxOffice returns (bool) {
    ...
    /**
     * @notice Operator mints GAUR to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of GAUR to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentGaurPrice = _getGaurPrice();
            currentTaxRate = _updateTaxRate(currentGaurPrice);
            if (currentGaurPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_gaurPool != address(0), "!_gaurPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_gaurPool, INITIAL_GAUR_POOL_DISTRIBUTION);
