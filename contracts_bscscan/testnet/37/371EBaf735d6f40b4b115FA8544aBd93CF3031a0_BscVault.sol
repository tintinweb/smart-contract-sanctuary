...
contract BscVault is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint;//using math opertions 
    using SafeERC20 for IERC20;//erc20 functions 

    uint public constant MIN_AMOUNT = 1e18;//minimum amount
    uint public swapCommission;//Commision swap 
    ...
    function addNewChain(uint chainID, address minter, address token) public onlyOwner returns(bool){
        ...
        require(registeredChains[chainID].minter == address(0), 'ChainID has already been registered');//if it equals to address 0 then it is not registered
    ...
    function changeActivationChain(uint chainID, bool activate) public onlyOwner {
        ...
        require(registeredChains[chainID].minter != address(0), 'Chain is not registered');//should be registered 
    ...
    function swapStart(
        ...
        require(IERC20(rootToken).allowance(msg.sender, address(this)) >= amount, "not enough allowance");//msg.sender needs to approve the contract to spend the amount 
        ...
        //BVG02 fixed
        registeredChains[toChainID].depositCount = registeredChains[toChainID].depositCount.add(1);//how many members are deposited on this chain
        uint _depositCount = registeredChains[toChainID].depositCount;//store the deposit count
        uint _chainID = _getChainID();//get the chain id
        ...
        //BVG07 fixed
        bytes32 eventHash = keccak256(abi.encode(_depositCount, _chainID, toChainID, msg.sender, to, amount));//encode and convert it to the hash
        require(eventStore[eventHash].depositCount == 0,
            "It's available just 1 swap with same: chainID, depositCount, from, to, amount");//a single change can change the hash so the same chainids does not have the same deposit count
        eventStore[eventHash] = eventStr;//store in the event str
        emit SwapStart(eventHash, _depositCount, toChainID, msg.sender, to, amount);//emit the event
    }
    //BVG10 fixed
    function swapEnd(
        bytes32 eventHash,
        ...
        require(eventStore[receivedHash].isCompleted == false, "Swap was ended before!");//it will be completed after the minting is completed
        EventStr memory eventStr = EventStr({
        ...
        if(swapCommission > 0 && to != commissionReceiver){
            uint commission = _commissionCalculate(amount);//take the commision amount
            amount = amount.sub(commission);
