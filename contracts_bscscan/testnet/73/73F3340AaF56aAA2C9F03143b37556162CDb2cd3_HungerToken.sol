...
contract HungerToken is Context, IERC20, Ownable {
    ...
    struct Reflections {
        uint128 reflectionSupply;
        uint128 reflectionsCirculating;
        uint128 reflectionFeesEarned;
    }
    ...
    function setContractRate(uint16 index, uint16 value) external onlyOwner {
        ...
        if (index == 0 || index == 3) {
            require(
                value <= 18000,
                "Combined Sell/Buy Fee Rate must be equal to or lower than 18% (18000)"
            );
        } else if (index == 6) {
            require(
                value <= 12000,
                "Send Fee Rate must be equal to or lower than 12% (12000)"
            );
        } else if (index != 7) {
            require(
                value <= 6000,
    ...
    function setLuckyShotThreshold(uint128 minimumTokensAccumulated)
        external
        onlyOwner
    {
    ...
    function setLuckyShotMinimumSpend(uint64 minimumSpendAmount)
        external
        onlyOwner
    {
    ...
    {
        require(
            amount <= _reflections.reflectionSupply,
        ...
        returns (uint256 reflectedAmount)
    {
        ...
        require(
            reflectedAmount <= _reflections.reflectionSupply,
            "Amount must be less than total reflections"
        );
    }
    ...
    function _transfer(
        ...
        uint128 reflectionRate = _getReflectionRate();
        require(
        ...
        if (!_lockSwap) {
            ...
            if (!(isExcludedFrom.fromFee || isExcludedTo.fromFee)) {
                (
                    ...
                    reflectionFee,
                    luckyShotFee
                ) = _splitAndDetermineFees(
                    from,
                    to,
                    amount,
                    [isExcludedFrom, isExcludedFrom]
                );
        ...
        if (!isExcludedTo.fromReward) {
            ...
            _tokenBalance[to] += amount - tokenFees;
        ...
        if (!_lockSwap) _handleLuckyShot(from, to, luckyShotFee, amount);

        _handleReflections(isExcludedFrom, isExcludedTo, reflectionRate, reflectionFee);
    ...
    function _handleReflections(
        IsExcluded memory isExcludedFrom,
        IsExcluded memory isExcludedTo,
        uint128 reflectionRate,
        uint256 reflectionFee
    ) private {
        uint128 reflectionFeeWithRate = uint128(reflectionFee) * reflectionRate;
        Reflections memory reflections = _reflections;

        if (reflectionFeeWithRate > 0) {
            if (isExcludedFrom.fromReward && !isExcludedTo.fromReward){
                reflections.reflectionsCirculating += reflectionFeeWithRate;
                reflections.reflectionSupply += reflectionFeeWithRate;
            } else if (!isExcludedFrom.fromReward && isExcludedTo.fromReward) {
                reflections.reflectionsCirculating -= reflectionFeeWithRate;
                reflections.reflectionSupply -= reflectionFeeWithRate;
            }

            if (reflections.reflectionsCirculating >= reflectionFeeWithRate)
                reflections.reflectionsCirculating -= reflectionFeeWithRate;
    ...
    function _getReflectionRate() private view returns (uint128) {
        if (_reflections.reflectionsCirculating > _reflections.reflectionSupply) {
            return _reflections.reflectionsCirculating / _reflections.reflectionSupply;
        } else {
            return (_MAX - (_MAX % _reflections.reflectionSupply)) / _reflections.reflectionSupply;
        }
    }
