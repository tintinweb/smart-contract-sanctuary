...
contract Staking is Ownable, ReentrancyGuard {
    ...
    uint256 public lpRewardCap = 14700 * (10 ** 18);
    uint256 public lpRewardBotPerSecond = 2893518518518518; // 250 * (10 **18) / (3600 * 24)
    ...
    function stakeBT(uint256 amount, address node) public nonReentrant {
        ...
        if (user.btLastRewardTime > 0) {
            ...
            if (pending > 0) {
                safeBotTransfer(sender, pending);
            }
        }
    ...
    function pendingRewardBT(address owner) public view returns (uint256) {
        ...
        if (user.btLastRewardTime > 0) {
            ...
            if (btRewardEndTime > 0) {
                // pending reward
                uint256 secds = btLastRewardTime.sub(user.btLastRewardTime);
                pending = secds.mul(btRewardBotPerSecond).mul(user.btAmount).div(PRECISION);

                secds = btRewardEndTime.sub(btLastRewardTime);
                uint256 reward = secds.mul(btRewardBotPerSecond).mul(btTotalStaked).div(PRECISION);
                reward = reward.mul(user.btAmount).div(btTotalStaked);
    ...
    function pendingRewardBEE(address owner) public view returns (uint256) {
        ...
        if (user.beeLastRewardTime > 0) {
            ...
            if (beeRewardEndTime > 0) {
                ...
                secds = beeRewardEndTime.sub(beeLastRewardTime);
                uint256 reward = secds.mul(beeRewardBotPerSecond).mul(beeTotalStaked).div(PRECISION);
                reward = reward.mul(user.beeAmount).div(beeTotalStaked);
                pending = pending.add(reward);
            } else {
                uint256 secds = block.timestamp.sub(user.beeLastRewardTime);
                pending = secds.mul(beeRewardBotPerSecond).mul(user.beeAmount).div(PRECISION);
