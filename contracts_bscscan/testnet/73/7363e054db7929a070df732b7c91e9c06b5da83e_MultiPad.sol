...
contract MultiPad is IBEP20 {
    ...
    */
    ...
    modifier validateClaim(address _userAddress, uint256 _lockNumber)
    {
        ...
        if(block.timestamp > saleEndTime + 300 && _lockNumber == 1){
            require(reEntrance[_userAddress][_lockNumber] != true, "Insufficient Unlocked Tokens");
        }
        if(block.timestamp > saleEndTime + 600 && _lockNumber == 2){
            require(reEntrance[_userAddress][_lockNumber] != true , "Insufficient Unlocked Tokens");
        }
        if(block.timestamp > saleEndTime + 900 && _lockNumber == 3){
            require(reEntrance[_userAddress][_lockNumber] != true ,  "Insufficient Unlocked Tokens");
        }
        if(block.timestamp > saleEndTime + 1200 && _lockNumber == 4){
            require(reEntrance[_userAddress][_lockNumber] != true,  "Insufficient Unlocked Tokens");
        }
        else{
           require(block.timestamp > saleEndTime + 1200, "Fully unlock not reached");
        }
        _;
    }
