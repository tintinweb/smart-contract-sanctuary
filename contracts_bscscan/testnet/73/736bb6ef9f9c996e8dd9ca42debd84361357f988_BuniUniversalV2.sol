...
contract TrainersV2 is ITrainersV2, Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function get(uint256 id) public view noFreshLookup(id) 
        returns (uint16 _exp, uint8 _level, uint8 _element, uint64 _staminaTimestamp, uint24 _power) {
        Trainer memory trainer = trainers[id];
        return (trainer.exp, trainer.level, trainer.element, staminaTimestamp[id], uint24(powers[trainer.level]));
    }
...
contract NFTNameTag is Initializable, AccessControlUpgradeable {
    ...
    uint8 public constant TAG_MINIMUM_LENGTH = 6;
    uint8 public constant TAG_MAXIMUM_LENGTH = 15;
    ...
    function initialize () public initializer {
        ...
        RENAME_TAG_FEE_ONLY_USD = ABDKMath64x64.divu(10, 1); // 10.0 USD
    }

    int128 public RENAME_TAG_FEE_ONLY_USD;

    mapping(address => mapping(uint256 => string)) private tags;

    // address is IERC721
    EnumerableSet.AddressSet private supportedTokens;

    event TagRenamed(address indexed token, address indexed owner, uint256 indexed tokenId);
    ...
    function _onlyGameContract() internal view {
        require(hasRole(ROLE_GAME_CONTRACT, msg.sender), "not game contract");
    }

    modifier isNFTRegistered(address _tokenAddress) {
        _isNFTRegistered(_tokenAddress);
        _;
    }

    function _isNFTRegistered(address _tokenAddress) internal view {
        require(supportedTokens.contains(_tokenAddress), "un-registered");
    }

    function registerNFT(IERC721 _tokenAddress) external restricted {
        require(!supportedTokens.contains(address(_tokenAddress)), "registered");

        supportedTokens.add(address(_tokenAddress));
    }

    function unregisterNFT(IERC721 _tokenAddress) external restricted {
        require(supportedTokens.contains(address(_tokenAddress)), "un-registered");

        supportedTokens.remove(address(_tokenAddress));
    }

    function setRenameTagValue(uint256 cents) external restricted {
        RENAME_TAG_FEE_ONLY_USD = ABDKMath64x64.divu(cents, 100);
    }

    function getNameTag(address _tokenAddress, uint256 _tokenId) external view returns(string memory) {
        return tags[_tokenAddress][_tokenId];
    }

    function setNameTag(address _tokenAddress, uint256 _tokenId, string calldata _tag)
        external
        onlyGameContract
        isNFTRegistered(_tokenAddress) {

        require(bytes(_tag).length >= TAG_MINIMUM_LENGTH, "Tag too short");
        require(bytes(_tag).length <= TAG_MAXIMUM_LENGTH, "Tag too long");

        tags[_tokenAddress][_tokenId] = _tag;
        emit TagRenamed(_tokenAddress, IERC721(_tokenAddress).ownerOf(_tokenId) , _tokenId);
    }
}
...
contract BuniUniversalV2 is Initializable, AccessControlUpgradeable {
    ...
    // External contracts configuration for testnet
    ...
    NFTNameTag public constant NAME_TAG_CONTRACT = NFTNameTag(0x1aE4A04CDaB183261cDF55b5890A527e0eb78bCe); // TODO: tmp address

    // TODO: External contracts configuration for mainnet
    ...
    // These one for 
    // TrainersV2 public trainers;
    // BunicornsV2 public bunicorns;
    ...
    // IBurToken public burToken; // reward token
    // IPriceOracle public priceOracleBuniPerUsd;
    // IPriceOracle public priceOracleBurPerUsd;
    ...
    // Fee to hunt bunicorn, paid in buni only
    int128 public BUNICORN_MINT_FEE_BY_BUNI_ONLY_USD;

    // Fee to hunt bunicorn, paid in bur only
    ...
    // =========================== NFT NAME TAG ===========================
    function getTrainerTag(uint256 _tokenId) external view returns(string memory) {
        return NAME_TAG_CONTRACT.getNameTag(address(TRAINER_CONTRACT), _tokenId);
    }
    ...
    function setTrainerTag(uint256 _tokenId, string calldata _tag)
        ...
        int128 fee = NAME_TAG_CONTRACT.RENAME_TAG_FEE_ONLY_USD();
    ...
    function getBunicornTag(uint256 _tokenId) external view returns(string memory) {
        return NAME_TAG_CONTRACT.getNameTag(address(BUNICORN_CONTRACT), _tokenId);
    }
    ...
    // =========================== MIGRATOR ===========================
    function setGlobalStaminaByMigrator(address _playerAddress) external onlyMigratorContract {
        if (globalStaminaTimestamp[_playerAddress] == 0) {
            globalStaminaTimestamp[_playerAddress] = uint64(now.sub(_getMaxGlobalStaminaCooldown()));
        }
    }

    function setTokenRewardsByMigrator(address _playerAddress, uint256 _amount, uint256 _timerStart) external onlyMigratorContract {
        burRewards[_playerAddress] = burRewards[_playerAddress].add(_amount);
        if (_burRewardsClaimTaxTimerStart[_playerAddress] == 0 || _timerStart < _burRewardsClaimTaxTimerStart[_playerAddress]) {
            _burRewardsClaimTaxTimerStart[_playerAddress] = _timerStart;
        }
    }

    function setExpRewardsByMigrator(uint256 _trainer, uint256 _exp) external onlyMigratorContract {
        expRewards[_trainer] = expRewards[_trainer].add(_exp);
    }
    // =========================== MIGRATOR ===========================
}