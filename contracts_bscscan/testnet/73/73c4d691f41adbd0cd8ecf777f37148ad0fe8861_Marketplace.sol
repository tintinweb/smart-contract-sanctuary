...
contract Marketplace is IERC721Receiver, Auth {
    ...
    address public nftAddress = 0xd1b21361B10874259aD092326B57C4c8E91f852a; // testnet crate

    IERC20 token;
    IERC721 nftContract;
    
    event TradeOpened(uint tradeId, address indexed seller, uint indexed nftId, uint price);
    event TradeClosed(uint tradeId, address indexed seller, address indexed buyer, uint indexed nftId, uint price);
    event TradeCancelled(uint tradeId, address indexed seller, uint indexed nftId, uint price);
    ...
    /**
     * Can fail if passing callstack limits. Use getOpenTradesBounded() if needed.
     */
    ...
    function openTrade(uint24 nftTokenId, uint200 price) external {
        ...
        emit TradeOpened(id, msg.sender, nftTokenId, price);
    }
    ...
    function executeTrade(uint tradeId) external {
        ...
        emit TradeClosed(tradeId, trade.poster, msg.sender, trade.nftTokenId, trade.nftTokenPrice);
    }
    ...
    function cancelTrade(uint tradeId) external {
        ...
        emit TradeCancelled(tradeId, trade.poster, trade.nftTokenId, trade.nftTokenPrice);
    }
