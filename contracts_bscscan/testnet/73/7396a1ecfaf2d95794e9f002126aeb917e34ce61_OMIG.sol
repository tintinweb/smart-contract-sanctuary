...
contract OMIG is Context, IERC20, Ownable {
    ...
    bool public semilock = false;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 100 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tRewardTotal;

    string private _name = "OMIG";
    string private _symbol = "OMIG";
    ...
    // function setSemiLockOpen() public onlyOwner {
    //     semilock = true;
    // }

    // function setSemiLockClose() public onlyOwner {
    //     semilock = false;
    // }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        } else if (!_isExcluded[sender] && !_isExcluded[recipient] && !semilock) {
            _transferStandard(sender, recipient, amount);
        } 
        else if (!_isExcludedFromFee[sender] && !_isExcludedFromMaxWallet[recipient]) {
            _transferStandardMaxWallet(sender, recipient, amount);
        } 
        // else if (!_isExcluded[sender] && !_isExcluded[recipient] && semilock) {
        //     _transferStandardSemi(sender, recipient, amount);
        // }
         else if (_isExcluded[sender] && _isExcluded[recipient]) {
    ...
    function _transferStandardMaxWallet(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rDistribution, uint256 tTransferAmount, uint256 tDistribution, uint256 tMarketingAndBurn) = _getValues(tAmount);
        require(balanceOf(recipient)+tTransferAmount <= _getMaxHolding(), "Max holding cap breached.");
