    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(uint256 => uint8) public wolfPackPotionSlot;
    uint8 public slotLimitForPotions;
    ...
    function initialize(address _wolfPackNFTContractAddress, address _consumibleNFTContractAddress) public initializer {
        ...
        slotLimitForPotions = 5;
    }

    function useHealthPotionPercentage(uint256 _wolfPackId, uint256 _tokenId) public {
        require(WolfPacksNFT(wolfPackNFTContractAddress).ownerOf(_wolfPackId) == msg.sender, "Owner of WolfPack failed");
        require(ConsumibleNFT(consumibleNFTContractAddress).ownerOf(_tokenId) == msg.sender, "Owner of Consumible failed");
        require(wolfPackPotionSlot[_wolfPackId] <= slotLimitForPotions, "Reached max amount of potions");
        uint256 _initialLife = WolfPacksNFT(wolfPackNFTContractAddress).wolfPackInitialLife(_wolfPackId);
        uint256 _life = WolfPacksNFT(wolfPackNFTContractAddress).wolfPackLife(_wolfPackId);
        require(_life < _initialLife, "WolfPack has all the life");
        (uint256 _type, uint256 _class, uint256 _object, uint256 _value) = getConsumibleProperties(_tokenId);
        require(_type == 0 && _class == 0 && _object >= 0 && _object <= 3, "Incorrect potion");
        uint256 _amountToIncrease = _initialLife * _value / 10000;
        increaseLife(_wolfPackId, _initialLife, _life, _amountToIncrease);
        ConsumibleNFT(consumibleNFTContractAddress).burnConsumible(_tokenId);
        wolfPackPotionSlot[_wolfPackId] += 1;
    }
    ...
    function useHealthPotionPoints(uint256 _wolfPackId, uint256 _tokenId) public {
        ...
        require(_life < _initialLife, "WolfPack has all the life");
        (uint256 _type, uint256 _class, uint256 _object, uint256 _value) = getConsumibleProperties(_tokenId);
        require(_type == 0 && _class == 0 && _object >= 4 && _object <= 9, "Incorrect potion");
        increaseLife(_wolfPackId, _initialLife, _life, _value);
        ConsumibleNFT(consumibleNFTContractAddress).burnConsumible(_tokenId);
        wolfPackPotionSlot[_wolfPackId] += 1;
    }

    function useRevivePotion(uint256 _wolfPackId, uint256 _tokenId) public {
        require(WolfPacksNFT(wolfPackNFTContractAddress).ownerOf(_wolfPackId) == msg.sender, "Owner of WolfPack failed");
        require(ConsumibleNFT(consumibleNFTContractAddress).ownerOf(_tokenId) == msg.sender, "Owner of Consumible failed");
        (uint256 _type, uint256 _class, uint256 _object, uint256 _value) = getConsumibleProperties(_tokenId);
        require((_type == 0 && _class == 2 && _object == 0) || (_type == 1 && _class == 0 && _object == 0), "Incorrect potion");
        require(WolfPacksNFT(wolfPackNFTContractAddress).wolfPackLife(_wolfPackId) == 0, "Wolfpack must be died");
        uint256 _initialLife = WolfPacksNFT(wolfPackNFTContractAddress).wolfPackInitialLife(_wolfPackId);
        uint256 _amountToIncrease = _initialLife * _value / 10000;
        increaseLife(_wolfPackId, _initialLife, 0, _amountToIncrease);
        ConsumibleNFT(consumibleNFTContractAddress).burnConsumible(_tokenId);
        wolfPackPotionSlot[_wolfPackId] = 0;
    }
    
    function increaseLife(uint256 _wolfPackId, uint256 _initialLife, uint256 _life, uint256 _amountToIncrease) internal {
    ...
    function useShieldPotion(uint256 _wolfPackId, uint256 _tokenId) public {
        ...
        require(wolfPackPotionSlot[_wolfPackId] <= slotLimitForPotions, "Reached max amount of potions");
        (uint256 _type, uint256 _class, uint256 _object, uint256 _value) = getConsumibleProperties(_tokenId);
        require(_type == 0 && _class == 1 && _object >= 0 && _object <= 2, "Incorrect potion");
        uint256 _wolfPackLifeActual = WolfPacksNFT(wolfPackNFTContractAddress).wolfPackLife(_wolfPackId);
        require(
            _wolfPackLifeActual == 
            WolfPacksNFT(wolfPackNFTContractAddress).wolfPackInitialLife(_wolfPackId), "Life must be initial life");
        uint256 _amountToIncrease = _wolfPackLifeActual * _value / 10000;
        WolfPacksNFT(wolfPackNFTContractAddress).increaseWolfPackLife(_wolfPackId, _amountToIncrease);
        ConsumibleNFT(consumibleNFTContractAddress).burnConsumible(_tokenId);
        wolfPackPotionSlot[_wolfPackId] += 1;
    }

    function getConsumibleProperties(uint256 _tokenId) internal view returns (uint256 _type, uint256 _class, uint256 _object, uint256 _value) {
        return (
            ConsumibleNFT(consumibleNFTContractAddress).getConsumibleProperties(_tokenId)[0],
            ConsumibleNFT(consumibleNFTContractAddress).getConsumibleProperties(_tokenId)[1],
            ConsumibleNFT(consumibleNFTContractAddress).getConsumibleProperties(_tokenId)[2],
            ConsumibleNFT(consumibleNFTContractAddress).getConsumibleProperties(_tokenId)[3]
        );
    }

    function setSlotLimitForPotions(uint8 _newLimit) external onlyOwner {
        slotLimitForPotions = _newLimit;
    }
    ...
    ReentrancyGuardUpgradeable
{
        ...
        nonReentrant
    {
        ...
        uint256 dollarsDiscount = (healthPotionPrices[_healthPotionType] * (10000 - discount)) / 10000;
    ...
    function getValuesForHealthPotion(uint256 _healthPotionType, uint256 _amount)
        internal
    {
        for (uint256 index = 0; index < _amount; index++) {
            ...
            (
                ...
                .generateHealthPotion(_healthPotionType);
    ...
    function calculatePriceInBNB(uint256 _healthPotionType, uint8 _amount)
        public
        view
        returns (uint256)
    {
    ...
    function burnConsumible(uint256 _tokenId) public returns (bool) {
        require(ownerOf(_tokenId) == msg.sender || msg.sender == inventoryContractAddress, "Caller unauthorized to burn");
        safeTransferFrom(ownerOf(_tokenId), rewardsPoolAddress, _tokenId);
        return true;
    }
    ...
    {
        require(_exists(_tokenId), "Token does not exist");
        uint256[] memory properties = new uint256[](4);
