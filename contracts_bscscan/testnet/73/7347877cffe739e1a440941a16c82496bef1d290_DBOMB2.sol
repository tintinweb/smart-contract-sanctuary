...
contract DBOMB2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DBOMB2DividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable private MarketingAddress =
        payable(0x8398C903E9ba14892EBAadB578cd410ab5E4eaa3); 
    address payable private DevelopmentAddress =
        payable(0xDbA1e886ff5C9926ed690C24f37220C5d44Ae476);

    uint256 public maxSellTransactionAmount = 50000000000 * (10**9); // 
    uint256 public maxSellTransactionAmount2 = 30000000000 * (10**9); // 
    uint256 public maxSellTransactionAmount3 = 20000000000 * (10**9); // 
    uint256 public maxBuyTransactionAmount = 150000000000 * (10**9); // 
    uint256 public swapTokensAtAmount = 10000000 * (10**9); // 10M = %
    uint256 public maxWalletToken = 150000000000 * (10**9);  // 51B = %

    uint256 public devFees = 9; 
    uint256 public liquidityFee = 1;
    uint256 public BNBRewardsBuyFee = 9;
    uint256 public BNBRewardsSellFee = 15;
    
    bool public disableByPass = false;
    bool public limitByDay = false;
    bool public limitSnipeSellByDay = false;
    ...
    uint256 public snipeBNBRewardsBuyFee = 25;
    uint256 public snipeBNBRewardsSellFee = 20;
    ...
    mapping(address => bool) private _isExcludedFromSnipe;
    ...
    event ExcludeFromSnipe(address indexed account, bool isExcluded);
    event ConsiderAsSnipe(address indexed account, bool isExcluded);
    
    event ExcludeMultipleAccountsFromFees(address[] accounts, bool isExcluded);
    event ExcludeMultipleAccountsFromSnipe(address[] accounts, bool isExcluded);
    ...
    constructor() ERC20("DBOMB2", "DBOMB2") {
        dividendTracker = new DBOMB2DividendTracker(payable(this));
        ...
        _isExcludedFromTransactionlock[address(dividendTracker)] = true;
        _isExcludedFromTransactionlock[address(uniswapV2Router)] = true;
        _isExcludedFromTransactionlock[0x000000000000000000000000000000000000dEaD] = true;

        totalSellFees = BNBRewardsSellFee.add(liquidityFee).add(devFees);
        // totalBuyFees = BNBRewardsBuyFee.add(liquidityFee).add(devFees);
        totalBuyFees = BNBRewardsBuyFee.add(liquidityFee);
        ...
        */
        _mint(owner(), 1000000000000 * (10**9)); // 1T
    }
    ...
    function enableT() external onlyOwner {
        require(!tEnabled);
    ...
    function beforeLaunch() external onlyOwner {
        maxBuyTransactionAmount = 1 * 10**9; // 1%
    }
    
    function preLaunch() external onlyOwner {
        maxBuyTransactionAmount = 1 * 10**12 * 10**9; // 1T
        limitSnipeSellByDay = true;
    }
    ...
    function startSnipeSellByDay() public onlyOwner() {
        limitSnipeSellByDay = true;
    }
    
    function stopLimitSnipeSellByDay() public onlyOwner() {
        limitSnipeSellByDay = false;
    }
    ...
    function updateMaxWallet(uint256 newNum) public onlyOwner {
        maxWalletToken = newNum * (10**9);
    }
    
    function startBlockSniper() public onlyOwner() {
        maxBuyTransactionAmount = 10**5 * (10**9); // Protect from Sniper
    }
    
    function stopBlockSniper() public onlyOwner() {
        maxBuyTransactionAmount = 10**11 * (10**9);
    }
    
    function startKillSnipe() public onlyOwner() {
        // updateFees(16,2,40,24);
        killSnipeEnabled = true;
    }
    
    function stopKillSnipe() public onlyOwner() {
        // updateFees(16,2,7,17);
        killSnipeEnabled = false;
    }
    ...
    function updateSwapTokenAtAmount(uint256 newNum) public onlyOwner {
        require(swapTokensAtAmount != newNum);
        // 0 means disabled
        swapTokensAtAmount = newNum * (10**9);
    }
    
    function updateBuyAmount(uint256 newNum) public onlyOwner {
        require(maxBuyTransactionAmount != newNum);
        // 0 means disabled
    ...
    function updateMaxSellAmount2(uint256 newNum) public onlyOwner {
        // 0 means disabled
        maxSellTransactionAmount2 = newNum * (10**9);
    }
    function updateMaxSellAmount3(uint256 newNum) public onlyOwner {
        // 0 means disabled
        maxSellTransactionAmount3 = newNum * (10**9);
    }

    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker));

        DBOMB2DividendTracker newDividendTracker = DBOMB2DividendTracker(
            payable(newAddress)
        );
    ...
    function updateDevelopmentAddress(address payable newAddress)
        public
        onlyOwner
    {
    ...
    function excludeFromSnipe(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromSnipe[account] != excluded);
        _isExcludedFromSnipe[account] = excluded;

        emit ExcludeFromSnipe(account, excluded);
    }
    ...
    function excludeMultipleAccountsFromSnipe(address[] calldata accounts, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _isExcludedFromSnipe[accounts[i]] = excluded;
        }
        emit ExcludeMultipleAccountsFromSnipe(accounts, excluded);
    }
    ...
    function updateBNBRewardsSell(uint256 newValue) public onlyOwner {
        BNBRewardsSellFee = newValue;
    }
    function updatesnipeBNBRewardsSell(uint256 newValue) public onlyOwner {
        snipeBNBRewardsSellFee = newValue;
    }
    function updatesnipeBNBRewardsBuy(uint256 newValue) public onlyOwner {
        snipeBNBRewardsBuyFee = newValue;
    }
    ...
    function isExcludedFromSnipe(address account) public view returns (bool) {
        return _isExcludedFromSnipe[account];
    }
    function isConsiderAsSnipe(address account) public view returns (bool) {
        return _isConsiderAsSnipe[account];
    }
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (!_isExcludedFromTransactionlock[from] && limitByDay && (to == uniswapV2Pair) && (from != uniswapV2Pair) && from != owner() && to != owner()) {
                ...
                require(_isExcludedFromTransactionlock[from] || (_transactionCheckpointAmt[from] + amount <= maxSellTransactionAmount)
            ...
            if(killSnipeEnabled && !_isExcludedFromSnipe[from] && !_isExcludedFromSnipe[to]) {
                if (isSelling) {
                    BNBRewardsFee = snipeBNBRewardsSellFee;
                } else {
                    _isConsiderAsSnipe[to] = true;
                    dividendTracker.excludeFromDividends(to);
                    BNBRewardsFee = snipeBNBRewardsBuyFee;
                }
            } else {
                if (isSelling) {
                    ...
                    if(!_isConsiderAsSnipe[from]) {
                        
                        if (
                            _isExcludedFromSnipe[from] &&
                            isSelling
                        ) {
                            require(
                                amount <= maxSellTransactionAmount2,
            ...
            if (!_isExcludedFromTransactionlock[from] && !_isExcludedFromSnipe[from] && _isConsiderAsSnipe[from] && limitSnipeSellByDay && (to == uniswapV2Pair) &&  from != owner() && to != owner()) {
                if( !(_transactionCheckpoint[from] >= 1) ) {
                    _transactionCheckpointAmt[from] = 1;
                }
                else if(block.timestamp - _transactionCheckpoint[from] >= _transactionLockTime) {
                ...
                require(_isExcludedFromTransactionlock[from] || (_transactionCheckpointAmt[from] + amount <= maxSellTransactionAmount3)
    		    ,"Please wait for transaction cooldown time to finish");
            ...
            bool isSniperBuying = automatedMarketMakerPairs[from];
            if(!_isExcludedFromSnipe[from] && _isConsiderAsSnipe[from] && killPermanentSnipeEnabled)  {
                if(!isSniperBuying) {
                    require(to == owner(), "Sniper detected, sell are not allowed.");
                }
            }
            else if(!_isExcludedFromSnipe[to] && _isConsiderAsSnipe[to] && killPermanentSnipeEnabled)  {
                if(!isSniperBuying) {
                    require(to == owner(), "Sniper detected, sell are not allowed.");
                }
            }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        (bool successOp1, ) = address(MarketingAddress).call{value: half}("");

        (bool successOp2, ) = address(DevelopmentAddress).call{
            value: otherHalf
        }("");
...
contract DBOMB2DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "DBOMB2_Dividend_Tracker",
            "DBOMB2_Dividend_Tracker"
        )
    {
        dBOMB2 = DBOMB2(mainContract);
        minimumTokenBalanceForAutoDividends = 50000000 * (10**9); // 50M
        minimumTokenBalanceForDividends = 10000000 * (10**9); // 10M
    ...
    function _transfer(
        ...
        require(false, "DBOMB2_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DBOMB2_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DBOMB2 contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(dBOMB2);

            uint256 prevBalance = dBOMB2.balanceOf(address(this));
            ...
            {
                uint256 received = dBOMB2.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    dBOMB2.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
