...
contract NFT is ERC721, Ownable {
    ...
    // 查询用户所有的token以及详情; 已销毁的前端不显示
    // 参数1: 用户地址
    function getUserAllToken(address _userAddress) external view returns (tokenStruct[] memory _r, uint256[] memory _o) {
    ...
    function userTakeRed2(address _address) private {
        ...
        if(_amount > 0) {
            (bool success1, ) = usdtAddress.call(
                abi.encodeWithSelector(TRANSFERFROM, address(this), _address, _amount)
            );
            if(!success1) {
                revert("NFT: transfer fail 2");
            }
        }
        // 从新赋值用户的accSushi
