...
*Auto BNB Reward 10% *************
*Every 30 Minutes*****************
*Telegram: https://t.me/SuperCarToken
* ********************************
* ********************************
*/
...
contract SuperCarBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("SuperCarBNB_Dividend_Tracker", "SuperCarBNB_Dividend_Tracker") {
        claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SuperCarBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SuperCarBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SuperCarBNB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "SuperCarBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "SuperCarBNB_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SuperCarBNB is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    SuperCarBNBDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SuperCarBNB", "SC-BNB") {
        BNBRewardsFee = 10;
        liquidityFee = 2;
        extraFeeOnSell = 3; // extra fee on sell
        marketingFee = 3;
        marketingWallet = payable(0xab2736151542e8cdd0A98794a301A58bc6a1b532);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new SuperCarBNBDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SuperCarBNB: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SuperCarBNB: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SuperCarBNBB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SuperCarBNB: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SuperCarBNB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SuperCarBNB: Cannot update gasForProcessing to same value");
