...
abstract contract Gas_Refundable is ReentrancyGuard{
    ...
    function getRefundableGas(address addr) public view returns (uint256){
        return _refundData[addr].usedGas * refundBNBBonusRatioNumerator / refundBNBBonusRatioDenominator;
    }
...
contract Windmill_DAO is Ownable, Gas_Refundable {
    ...
    uint256 public constant nbProposals = 23;

    bool[nbProposals] public isProposalSuper;

    Windmill_Power public PWRToken;
    Windmill_Fund public fund;
    Windmill_Royalty public royalty;
    ...
    function init(uint256 _nbProposalPerUserPerCycle,
                  ...
                  uint256 _cycleDurationNbBlock,
                  bool[nbProposals] calldata _isProposalSuper) external onlyOwner{
        currentCycle = 0;
        nbProposalPerUserPerCycle = _nbProposalPerUserPerCycle;
        maxNbOpenProposals = _maxNbOpenProposals;

        voteBlockDuration = _voteBlockDuration;
        quorum = _quorum;
        voteMajorityPercent = _voteMajorityPercent;

        superVoteBlockDuration = _superVoteBlockDuration;
        superQuorum = _superQuorum;
        superVoteMajorityPercent = _superVoteMajorityPercent;

        cycleDurationNbBlock = _cycleDurationNbBlock;
        currentCycleEndBlock = block.number + cycleDurationNbBlock;

        isProposalSuper = _isProposalSuper;
    }
     ...
     */
    function setRoyaltydAddress(Windmill_Royalty _royalty) external onlyOwner{
        royalty = _royalty;
    }
    ...
    function updateRoyaltydAddress(Windmill_Royalty _royalty) internal{
        Windmill_Royalty oldRoyalty = royalty;
        royalty = _royalty;

        royalty.setPWRToken(PWRToken);
        royalty.setRoyaltyRatio(oldRoyalty.percentPWRRoyaltyMintNumerator(),
                                oldRoyalty.percentPWRRoyaltyMintDenominator());
    ...
    function applyProposal(Proposal storage proposal) internal{
        if (proposal.id == 0){
            ...
            updateRoyaltydAddress(Windmill_Royalty(payable(proposal.paramsAddress)));
        }
    }
    ...
    function checkProposalParameters(Proposal memory proposal) internal view returns (bool){
        if (proposal.id == 0){
            ...
            Windmill_Royalty newRoyalty = Windmill_Royalty(payable(proposal.paramsAddress));
            if (   proposal.paramsUint256 != 0
