...
contract MineralPool is Ownable {
    ...
    uint256 private poolProfession;
    ...
    constructor(uint256 _poolProfession, address _stakeToken, address _rewardsContract, address _empowerContract, uint256 _startTime, uint256 _duration, uint256 _rewardsAmount) {
        poolProfession = _poolProfession;
        stakeToken = _stakeToken;
        rewardsContract = _rewardsContract;
        empowerContract = _empowerContract;
        startTime = _startTime;
        duration = _duration;
        _notifyRewardsAmount(_rewardsAmount);
    }
    ...
    modifier updateRewards(uint256 tokenId) {
        rewardsPerUnitStored = rewardsPerUint();
    ...
    function getPoolProfession() public view returns (uint256) {
        return poolProfession;
    }
    ...
    function getMiningStartTime() public view returns (uint256) {
        return startTime;
    }

    function getMiningDuration() public view returns (uint256) {
        return duration;
    }

    function getMiningPeriodFinish() public view returns (uint256) {
        return periodFinish;
    }
    ...
    function getDailyRewards() public view returns (uint256) {
        return rewardsRate.mul(dailySeconds);
    }
    ...
    function getProducedRewards() public view returns (uint256) {
        return rewardsAmount.sub(getAvailableRewards());
    }
    ...
    function getAvailableRewards() public view returns (uint256) {
        ...
        if(block.timestamp < startTime){
            ...
            if(block.timestamp < periodFinish){
                availableRewards = rewardsRate.mul(periodFinish.sub(block.timestamp));
            }
        }
    ...
    function notifyRewardsAmount(uint256 rewards) public onlyController {
        _notifyRewardsAmount(rewards);
    }

    function stake(uint256 tokenId) public updateRewards(tokenId) checkStart {
        ...
        bool isMatchProfession = false;
        if(tokenParmas.profession == poolProfession){
            isMatchProfession = true;
        }
        
        uint256 capacity = ICalcFormula(empowerContract).calcCapacity(tokenParmas.level, isMatchProfession);
    ...
    function earned(uint256 tokenId) public view returns (uint256) {
        return capacityOf(tokenId)
                    .mul(rewardsPerUint().sub(tokensRewardsPerUnitPaid[tokenId]))
    ...
    function claimAllReward() public checkStart {
    ...
    function withdrawAllAndClaim() public checkStart {
    ...
    function _notifyRewardsAmount(uint256 rewards) private updateRewards(0){
        if (block.timestamp > startTime) {
            if (block.timestamp >= periodFinish) {
                rewardsRate = rewards.div(duration);
                rewardsAmount = rewards;
            } else {
                uint256 remaining = periodFinish.sub(block.timestamp);
                uint256 leftover = remaining.mul(rewardsRate);
                rewardsAmount = rewards.add(leftover);
                rewardsRate = rewardsAmount.div(duration);
            }
    ...
    function _claimReward(uint256 tokenId) private updateRewards(tokenId) checkStart {
        _claimTokenReward(tokenId);
    }

    function _withdrawAndClaim(uint256 tokenId) private updateRewards(tokenId) checkStart {
        _claimTokenReward(tokenId);
        ...
        removeToken(msg.sender, tokenId);
    ...
    function lastTimeRewardsApplicable() private view returns (uint256) {
        return Math.min(block.timestamp, periodFinish);
    }

    function rewardsPerUint() private view returns (uint256) {
        if (totalCapacity() == 0) {
            return rewardsPerUnitStored;
        }
        return
        rewardsPerUnitStored.add(
            lastTimeRewardsApplicable()
            .sub(lastUpdateTime)
            .mul(rewardsRate)
            .mul(1e18)
            .div(totalCapacity())
    ...
    function removeToken(address account, uint256 tokenId) private {
