...
contract Windmill_Fund is AccessControlEnumerable {
     ...
     */
    function setBUSDToken(ERC20 token) external onlyRole(DAO_ROLE){
        BUSDToken = token;
    }
...
contract Windmill_DAO is Ownable {
     ...
     */
    function setBUSDToken(ERC20 _BUSDToken) external onlyOwner{
    ...
    function checkProposalParameters(Proposal memory proposal) internal view returns (bool){
        if (proposal.id == 0){
            ...
            Windmill_Fund newFund = Windmill_Fund(proposal.paramsAddress);
