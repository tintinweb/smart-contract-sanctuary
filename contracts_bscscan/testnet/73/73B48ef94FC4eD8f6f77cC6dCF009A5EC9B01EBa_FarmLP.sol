...
contract FarmLP is FarmHashrate, IFarmLP, Initializable, Ownable, Recoverable {
    using SafeERC20 for IERC20;

    address public lpToken;

    // LP池, useraddr -> lpamount
    mapping(address => uint256) public lpPoolMap;

    // useraddr -> hashrate
    mapping(address => uint256) public lpHashrateMap;

    // 返佣数量, 解押些会扣除, useraddr -> genUser.addr -> hashratecommission
    mapping(address => mapping(address => uint256)) commissionMap;

    // sender -> lptoken -> lpamount -> hashrate
    event LPStaked(address, address, uint256, uint256);
    event LPUnstaked(address, address, uint256, uint256);
    ...
    function init(IAppData _appData, IPancake _pancake, IFarmUser _farmUser) public onlyOwner {
        ...
        lpToken = pancake.getUsdtLPToken();
    ...
    // 质押LP
    function stakeLP(uint256 amount) external override needInit {
        Model.HashrateConf memory hc = appData.getHashrateConf(Model.CATEGORY_LP);
        if (hc.invited == 1) {
            require(farmUser.existUser(_msgSender()), "user not exists");
        }
        
        // 检查地址LP数量
        uint256 lpBalance = IERC20(lpToken).balanceOf(_msgSender());
        uint256 allowance = IERC20(lpToken).allowance(_msgSender(), address(this));
        require(lpBalance >= amount && allowance >= amount, "LP balance or allowance insufficient");

        // 增加质押数量
        lpPoolMap[_msgSender()] = SafeMath.add(lpPoolMap[_msgSender()], amount);
        IERC20(lpToken).safeTransferFrom(_msgSender(), address(this), amount);

        // 计算算力
        (uint256 reserveUsdt, uint256 reserveQuote) = pancake.getUsdtLPTokenAmounts(amount);
        uint256 hashrate = calcTokenHashrate(pancake.getUsdtToken(), reserveUsdt, pancake.getQuoteToken(), reserveQuote);
        if (hashrate == 0) {
            return;
        }

        lpHashrateMap[_msgSender()] = SafeMath.add(lpHashrateMap[_msgSender()], hashrate);
        ...
        if (hc.rebate == 1)
        {
            ...
            for (uint8 gen = 1; gen <= appData.getMaxGen(); gen++) {
                ...
                lpHashrateMap[genUser.addr] = SafeMath.add(lpHashrateMap[genUser.addr], hashrateCommission);
                // 记录用户算力
                recordUserHashrate(Model.CATEGORY_LP, genUser.addr, lpHashrateMap[genUser.addr]);
        ...
        // 记录用户及全网算力
        recordHashrate(Model.CATEGORY_LP, _msgSender(), lpHashrateMap[_msgSender()], totalHashrate);
        emit LPStaked(_msgSender(), lpToken, amount, hashrate);
    }

    // 解押LP
    function unstakeLP() external override needInit {
        require(lpPoolMap[_msgSender()] > 0, "User lp balance is 0");
        IERC20(lpToken).safeTransfer(_msgSender(), lpPoolMap[_msgSender()]);
        lpPoolMap[_msgSender()] = 0;

        uint256 lpHashrate = lpHashrateMap[_msgSender()];
        if (lpHashrate == 0) {
            emit LPUnstaked(lpToken, _msgSender(), lpPoolMap[_msgSender()], lpHashrate);
            return;
        }

        lpHashrateMap[_msgSender()] = 0;
        totalHashrate = SafeMath.sub(totalHashrate, lpHashrate);
        ...
        for (uint8 gen = 1; gen <= appData.getMaxGen(); gen++) {
            ...
            lpHashrateMap[genUser.addr] = SafeMath.sub(lpHashrateMap[genUser.addr], hashrateCommission);
            // 记录用户算力
            recordUserHashrate(Model.CATEGORY_LP, genUser.addr, lpHashrateMap[genUser.addr]);
    ...
    function calcTokenHashrate(address token0, uint256 amount0, address token1, uint256 amount1) public view returns(uint256) {
        ...
        uint256 hashrate = SafeMath.mul(SafeMath.add(usdtAmount0, usdtAmount1), appData.getLPMultiple());
        hashrate = SafeMath.div(hashrate, 100 * (10 ** pancake.getUsdtDecimals()));
        return hashrate;
    }
}
