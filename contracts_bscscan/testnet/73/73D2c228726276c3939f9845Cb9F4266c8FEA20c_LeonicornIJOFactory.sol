...
import "./interfaces/ILeonicornIJO.sol";
 ...
 * @title LeonicornIJO
 */
contract LeonicornIJO is ILeonicornIJO, ReentrancyGuard, Ownable {
    ...
    // The address of the IJO factory
    address public immutable IJO_FACTORY;

    // Max blocks (for sanity checks)
    uint256 public MAX_BUFFER_BLOCKS;

    // The LP token used for buying offeringTokens, it could be a single token or an LP token
    ...
    ILPStaking public stakingContract;

    // Percent of staking required for participate in IJO.
    uint256 public percentOfStakes;

    // Whether it is initialized
    bool public isInitialized;

    // The block number when IJO starts
    uint256 public startBlock;

    // The block number when IJO ends
     ...
     * @param _offeringToken: the token that is offered for the IJO
     * @param _startBlock: the start block for the IJO
     * @param _endBlock: the end block for the IJO
    ...
    function initialize(
        ...
        require(msg.sender == IJO_FACTORY, "Operations: Not factory");
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        /*
         * Check if the user has staked in staking contract for participating in this.
         */
        if (address(stakingContract) != address(0)) {
            uint256 userAmount = _userInfo[msg.sender][0].amountPool + _userInfo[msg.sender][1].amountPool;
            uint256 required = userAmount.mul(percentOfStakes).div(10000);
            if (required > 0) {
                uint256 stakes = stakingContract.getUserStakedAmount(msg.sender);
                require(stakes >= required, "Not sufficient staking amount"); 
            }
        }	
    ...
    function setPool(
        ...
        require(block.number < startBlock, "Operations: IJO has started");
    ...
    /**
     * @notice Set staking contract address and percent of staking tokens required to stake
     * for participating in the IJO.
     * @param _staking staking contract address
     * @param _perOfStakes percentage of staking
     */
    function setStakingParams(address _staking, uint256 _perOfStakes) external onlyOwner {
	stakingContract = ILPStaking(_staking);
        percentOfStakes = _perOfStakes;	
    }
...
import "./LeonicornIJO.sol";

/**
 * @title LeonicornIJOFactroy
 */
contract LeonicornIJOFactory is Ownable {
    using SafeERC20 for IERC20;

    uint256 public constant MAX_BUFFER_BLOCKS = 200000; // 200,000 blocks (6-7 days on BSC)

    event AdminTokenRecovery(address indexed tokenRecovered, uint256 amount);
    event NewIJOContract(address indexed ijoAddress);

    /**
     * @notice It creates the IJO contract and initializes the contract.
     * @param _lpToken: the LP token used
     * @param _offeringToken: the token that is offered for the IJO
     * @param _startBlock: the start block for the IJO
     * @param _endBlock: the end block for the IJO
    ...
    function createIJO(
        ...
        bytes memory bytecode = type(LeonicornIJO).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_lpToken, _offeringToken, _startBlock));
        address ijoAddress;

        assembly {
            ijoAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        LeonicornIJO(ijoAddress).initialize(
            _lpToken,
            _offeringToken,
            _startBlock,
            _endBlock,
        ...
        emit NewIJOContract(ijoAddress);
    }
...
/** @title ILeonicornIJO.
 * @notice It is an interface for IJOV2.sol
 */
interface ILeonicornIJO {
    /**
     * @notice It allows users to deposit LP tokens to pool
     * @param _amount: the number of LP token used (18 decimals)
     * @param _pid: poolId
