...
contract BetManager is Context, Ownable {
  ...
  function hashReserveInfo(address sender, uint256 order) private pure returns (bytes32) {
  ...
  function verifyReserveSignature(bytes32 _hash, bytes memory signature) private view returns (bool) {
