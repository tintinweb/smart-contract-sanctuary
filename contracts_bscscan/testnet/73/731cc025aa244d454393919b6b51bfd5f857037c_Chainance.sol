...
contract Chainance is Context, Ownable{
    ...
    function CreateAccountControlAlgo(address myaddress, string memory username, string memory email) public AccountControl view returns(uint) {
        uint result = 0;
        for ( uint i = 0; i < ChainUp_Accounts.length; i++ ) {
            //- Address Control
            if ( ChainUp_Accounts[i].AccountAddress == myaddress ) result=result+1; 
            //- UserName Control
            if ( keccak256(abi.encodePacked(ChainUp_Accounts[i].UserName)) == keccak256(abi.encodePacked(username)) ) result=result+1;
            //- EMail Control
            if ( keccak256(abi.encodePacked(ChainUp_Accounts[i].Email)) == keccak256(abi.encodePacked(email)) ) result=result+1;
        }
        return result;
    }
