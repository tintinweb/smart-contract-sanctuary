...
contract SimpleDice is GambleBase, AccessControlEnumerable {
    ...
    function setBurnPerc(address _tokenAddress, uint256 _perc) external onlyAdmin {
        require(_perc <= 10000, "use percentage points");
    ...
    *
    **/
    function startBet(IERC20 _token, uint256[4] memory _rollIntegerVariables, bytes32 _userSeed, bytes32 _houseCommit) external noContract {
    ...
    function finishBet(IERC20 _token, address _gambler,uint256 _blockNumber, uint256[4] memory _rollIntegerVariables, bytes32 _userSeed, bytes32 _houseReveal) external onlyCoupier {
        ...
        if (betObject.rollIntegerVariables[1] == 0 && roll < betObject.rollIntegerVariables[0]) {
            payout = getWinAmount(betObject.rollIntegerVariables[2], _rollIntegerVariables[3], betObject.rollIntegerVariables[0]);
        }
        else if (betObject.rollIntegerVariables[1] != 0 && roll > betObject.rollIntegerVariables[0]) {
            payout = getWinAmount(betObject.rollIntegerVariables[2], _rollIntegerVariables[3], _rollIntegerVariables[3] - 1 - betObject.rollIntegerVariables[0]);
        }
