...
contract TestToken is Context, IERC20, Ownable {
    ...
    mapping(address => TimeOfTransfer) time;
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        if (transferLock == true){
            ...
            time[recipient].transferTime = block.timestamp;
            return true;
        }
        else{
            _transfer(_msgSender(), recipient, amount);
    ...
    function setTransferLockStatus(bool status) public onlyOwner returns (bool) {
        transferLock = status;
        return true;
    }
