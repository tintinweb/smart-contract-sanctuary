...
abstract contract RuffyCoinBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	// MAIN TOKEN PROPERTIES
	string private constant NAME = "Ruffy Coin";
	string private constant SYMBOL = "RUFFY";
  ...
  uint8 private _developerFee; //% of each transaction that will be used for developer
	uint8 private _poolFee; //The total fee to be taken and added to the pool, this includes all fees

	uint256 private constant _totalTokens = 100000000000000 * 10**DECIMALS;	//100 trillion total supply
	...
	// marketing and developer address
	address private _marketingWallet = 0x0b50bdC52A5190E4407f75abab4094C4E39Dc9f6;
	address private _developerWallet = 0x53Ea97382D1C69fe011567432f8c5B9513b3AB82;
	...
	function executeSwap(uint256 amount) private {
		...
		uint256 tokensReservedForDeveloper = amount * _developerFee / _poolFee;
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 buybackFee, uint8 marketingFee, uint8 developerFee, uint8 additionalSellFee) public onlyOwner {
		require(liquidityFee >= 0 && liquidityFee <= 5, "Liquidity fee must be between 0% and 8%");
    ...
    require(developerFee >= 0 && developerFee <= 5, "Developer fee must be between 0% and 5%");
		...
		// Enforce invariant
		_poolFee = _rewardFee + _liquidityFee + _buybackFee + _marketingFee + _developerFee;
	}
...
contract RuffyCoin is RuffyCoinBase {
	...
	address private _rewardTokenAddress = 0x241d44A548066AFdCD4550F0d1324295954cE342;
	...
	MinimumTokenBalance private T1 = MinimumTokenBalance(100000000 * 10**decimals(), 25);
	MinimumTokenBalance private T2 = MinimumTokenBalance(1000000000 * 10**decimals(), 50);
	MinimumTokenBalance private T3 = MinimumTokenBalance(50000000000 * 10**decimals(), 100);
  ...
  function totalBNBClaimedAsRewardToken() public view returns (uint256) {
	...
	function setClaimRewardAsToken(address tokenAddr) public onlyOwner {
		address oldTokenAddr = claimRewardAsToken();
	...
	function getMinimumBalance() public view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
		return (T1.balanceAmount, T1.taxFeePercent, T2.balanceAmount, T2.taxFeePercent, T3.balanceAmount, T3.taxFeePercent);
	}

	function setMinimumBalance(uint256 balanceAmount1, uint256 taxFeePercent1, uint256 balanceAmount2, uint256 taxFeePercent2, uint256 balanceAmount3, uint256 taxFeePercent3) public onlyOwner {
