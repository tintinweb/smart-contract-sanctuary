...
contract TwapPriceOracle {
    ...
    constructor (
        ...
        pair = IPancakeFactory(_pancakeFactory).getPair(_token0, _token1);
        token0 = _token0;
        token1 = _token1;
        price0CumulativeLast = IPancakePair(pair).price0CumulativeLast(); // fetch the current accumulated price value (1 / 0)
        price1CumulativeLast = IPancakePair(pair).price1CumulativeLast(); // fetch the current accumulated price value (0 / 1)
        uint112 reserve0;
        uint112 reserve1;
