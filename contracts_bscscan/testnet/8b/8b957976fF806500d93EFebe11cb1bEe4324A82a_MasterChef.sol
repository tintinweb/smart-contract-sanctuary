...
// MasterChef is the master of OMC. He can make OMC and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once OMC is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of OMC
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accOmcPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accOmcPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. OMCs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that OMCs distribution occurs.
        uint256 accOmcPerShare;   // Accumulated OMCs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The OMC TOKEN!
    OmadarToken public omc;
    // Dev address.
    address public devaddr;
    // OMC tokens created per block.
    uint256 public omcPerBlock;
    // Bonus muliplier for early omc makers.
    ...
    // The block number when OMC mining starts.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _omcPerBlock,
        uint256 _startBlock
    ) public {
        omc = _omc;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        omcPerBlock = _omcPerBlock;
    ...
    // Update the given pool's OMC allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending OMCs on frontend.
    function pendingOmc(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accOmcPerShare = pool.accOmcPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 omcReward = multiplier.mul(omcPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accOmcPerShare = accOmcPerShare.add(omcReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accOmcPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 omcReward = multiplier.mul(omcPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        omc.mint(devaddr, omcReward.div(10));
        omc.mint(address(this), omcReward);
        pool.accOmcPerShare = pool.accOmcPerShare.add(omcReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for OMC allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accOmcPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeOmcTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accOmcPerShare).div(1e12);
    ...
    // Safe omc transfer function, just in case if rounding error causes pool to not have enough OMCs.
    function safeOmcTransfer(address _to, uint256 _amount) internal {
        uint256 omcBal = omc.balanceOf(address(this));
        if (_amount > omcBal) {
            omc.transferWithLock(_to, omcBal);
        } else {
            omc.transferWithLock(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _omcPerBlock) public onlyOwner {
        massUpdatePools();
        omcPerBlock = _omcPerBlock;
    }
    
    // Setup Omadar Token Reward Release
    function setOmcRewardLock(uint256 lock) public onlyOwner {
        omc.setRewardLock(lock);
    }
    
    function setOmcTotalBlockRelease(uint256 totalBlockRelease) public onlyOwner {
        omc.setTotalBlockRelease(totalBlockRelease);
    }
}
...
// OmadarToken without Governance.
contract OmadarToken is BEP20('Extended OMADAR COIN', 'OMC') {
