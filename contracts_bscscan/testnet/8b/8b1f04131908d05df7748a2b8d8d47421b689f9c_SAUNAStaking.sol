...
contract SAUNAStaking is Ownable, ReentrancyGuard {
    ...
    // Total compounding tokens
    uint256 public totalCompoundingTokens;
    ...
    struct UserInfo {
        ...
        uint256 shares; // number of shares for a user in auto-compounding feature
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(
            totalStakingTokens.add(_amount) <= maxStakingTokens,
        ...
        if (_amount > 0) {
            ...
            if (totalStakingTokens != 0 && totalCompoundingTokens != 0) {
                currentShares = (_amount.mul(totalStakingTokens)).div(
                    totalCompoundingTokens
                );
            } else {
            ...
            // Increase total staking token amount & total compounding token amount
            totalStakingTokens = totalStakingTokens.add(currentShares);
            totalCompoundingTokens = totalCompoundingTokens.add(_amount);

            user.amount = user.shares.mul(totalCompoundingTokens).div(
                totalStakingTokens
            );
        }
    ...
    function claim() external nonReentrant {
        ...
        totalCompoundingTokens = totalCompoundingTokens.add(claimedAmount);
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        require(
            ...
            "Total staking tokens higher than the amount to withdraw"
        );
        ...
        if (_amount > 0) {
            if (canWithdraw(msg.sender)) {
                uint256 currentAmount = (totalCompoundingTokens.mul(_amount))
                    .div(totalStakingTokens);
                user.shares = user.shares.sub(_amount);
                totalStakingTokens = totalStakingTokens.sub(_amount);
                totalCompoundingTokens = totalCompoundingTokens.sub(
                    currentAmount
                );
