...
contract LuckyMarketPlaceV2 is IBidNFT ,ISwapNFT, ERC721Holder, Ownable, Pausable {
   ...
   function updateBidPrice(uint256 _tokenId, uint256 _price) public override whenNotPaused {
    ...
    function swapNFT(uint256 _nftA,uint256 _nftB, address _userB) public  override whenNotPaused {
        ...
        (,uint256 _indexOwner) = getOfferOnwerByTokenId(_nftA,_userB);        
        delOfferOwnerByTokenIdAndIndex(_indexOwner,_userB);        
    ...
    function cancelOfferNFT(uint256 _tokenId) public  override whenNotPaused {
        ...
        // remove owner offer
        (,uint256  _indexOwner) = getOfferOnwerByTokenId(_tokenId,_msgSender());    
        delOfferOwnerByTokenIdAndIndex( _indexOwner,_msgSender());       
        emit CancelOfferNFT(_msgSender(), _tokenId);
    }
    ...
    function delOfferOwnerByTokenIdAndIndex(uint256  _indexOwner, address _userOffer) internal {
        uint256 len = _offerOwnerMap[_userOffer].length;
        for (uint256 i = _indexOwner; i < len - 1; i++) {
            _offerOwnerMap[_userOffer][i] = _offerOwnerMap[_userOffer][i + 1];
        }
        _offerOwnerMap[_userOffer].pop();
    }        
    ...
    function getOfferOnwerByTokenId(uint256 _tokenId, address _useOffer)
            public
            view
            returns (OfferOwner memory,uint256)
        {
            // find the index of the offer
            OfferOwner[] memory offerEntries = _offerOwnerMap[address(_useOffer)];
            uint256 len = offerEntries.length;
            uint256 _index;
