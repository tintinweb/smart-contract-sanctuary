...
contract Trading2local is Ownable {
    using SafeMath for uint256;
    ...
    modifier onlyEOA() {
        require(msg.sender == tx.origin, "Trading2local: must use EOA");
        _;
    }

    modifier onlyOperator() {
        require(msg.sender == tradingOperator, "Trading2local: must use EOA");
        _;
    }

    uint private unlocked = 1;
    modifier lock() {
        require(unlocked == 1, 'Trading2local: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    constructor(
        address _tradingOperator,
        address payable _routerAddress,
        address _factory,
        address _feeAddress
    ) public {
        feeAddress = _feeAddress;
        tradingOperator = _tradingOperator;
    ...
    function _swap(
        ...
        // X1 - X5: OK
        IPancakePair pair =
            IPancakePair(factory.getPair(fromToken, toToken));
        require(address(pair) != address(0), "Trading2Local: Cannot convert");
        ...
        uint amountInput = IBEP20(fromToken).balanceOf(address(pair)).sub(reserveInput); // calculate amount that was transferred, this accounts for transfer taxes
    ...
    function stopTrading() external onlyOperator() {
        require(isGoPhase == true, "Can't stop trading.");
        ...
        // }
        exchangeRate = exchangeRate.mul(previousBUSDAmount).div(amountBUSD);
