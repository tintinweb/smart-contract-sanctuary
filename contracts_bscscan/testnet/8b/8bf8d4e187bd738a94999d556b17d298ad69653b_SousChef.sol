...
contract SousChef {
    ...
    // Info of each user that stakes Honey tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = honey.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 honeySupply = honey.balanceOf(address(this));
        if (honeySupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(honeySupply));
        poolInfo.lastRewardBlock = block.number;
    }

    // Deposit Honey tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];

        updatePool();
        honey.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Honey tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        honey.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        honey.safeTransfer(address(msg.sender), user.amount);
