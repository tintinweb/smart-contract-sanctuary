...
contract KODIDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("KODI_Dividend_Tracker", "KODI_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function getAccount(address _account)
        ...
        index = 1;// tokenHoldersMap.getIndexOfKey(account);
    ...
    function process(uint256 gas) public returns (uint256, uint256, uint256) {
    	uint256 numberOfTokenHolders = 100;// tokenHoldersMap.keys.length;
...
contract TBNB is ERC20, Ownable {
    ...
    address public _devWallet = 0xb289bd090CAbECA807F3BD890221d705d44aF90C;
    address public _marketingWallet = 0xed344d0b7DA5d31A83c95b73eC5C50936C1d43f1;

    bool public restrictWhales = true;

    uint256 public maxTxAmount = 100000000000 * (10**9);
    uint256 public walletMax = 100000000000 * (10**9);
    ...
    function updateFees(uint256 newRewardFee, uint256 newLiqFee, uint256 newMarketingFee, uint256 newDevFee) public onlyOwner {
        BNBRewardsFee = newRewardFee;
    ...
    function excludeFromDividends(address holder) public onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount ) internal override {
        ...
        if(cooldownEnabled)
        {
            if(from == uniswapV2Pair && !isTimelockExempt[to]) //BUY
            {
            ...
            else if(to == uniswapV2Pair && !isTimelockExempt[from])//SELL
            {
    ...
    function swapBack(uint256 tokensToLiquify) private {

        uint256 tokensToLP = tokensToLiquify.mul(liquidityFee).div(totalFees).div(2);
        ...
        uint256 bnbForReflection = bnbBalance.mul(BNBRewardsFee).div(bnbFeeFactor);
        uint256 bnbForDev = bnbBalance.mul(devFee).div(bnbFeeFactor);
        uint256 bnbForMarketing = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection).sub(bnbForDev);
        ...
        if(success) {
   	 		emit SendDividends(tokensToLiquify.mul(BNBRewardsFee).div(totalFees), bnbForReflection);
        }
    }
    ...
    function prepareForPreSale() external onlyOwner {
        ...
        maxTxAmount = 100000000000 * (10**9);
        walletMax = 100000000000 * (10**9);
    }
    ...
    function prepareForLaunch() external onlyOwner {
        ...
        maxTxAmount = 500000000 * (10**9);
        walletMax = 1000000000 * (10**9);
    }
