...
contract BSCTest1 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("BSCTest1", "BSCT1") {
        dividendTracker = new CrazyDividendTracker(payable(this));
        ...
        _isExcludedFromFees[address(operations1Address)] = true;
        _isExcludedFromFees[address(operations2Address)] = true;

        totalSellFees = BNBRewardsSellFee.add(liquidityFee).add(opFees);
        totalBuyFees = BNBRewardsBuyFee.add(liquidityFee).add(opFees);
    ...
    function updateMaxAmount(uint256 newNum) public onlyOwner {
        ...
        require(
            maxSellTransactionAmount == 0 ||
                maxSellTransactionAmount >= 5000000 * (10**9),
            "max sell amount must either be disabled or greater than 5 million"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        CrazyDividendTracker newDividendTracker = CrazyDividendTracker(
            payable(newAddress)
        );
    ...
    function updateFees(
        ...
        require(operating <= 5, "operation fees can't exceed 5%");
        ...
        emit UpdateFees(operating, liquidity, BNBRewardsBuy, BNBRewardsSell);
    }
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 totalFees = BNBRewardsFee.add(liquidityFee).add(opFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividendsFromBuy = address(this)
            ...
            .div(BNBRewardsBuyFee.add(opFees));
        ...
        uint256 dividendsFromSell = address(this)
            ...
            .div(BNBRewardsSellFee.add(opFees));
...
contract CrazyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "Crazy_Dividend_Tracker",
            "Crazy_Dividend_Tracker"
        )
    {
        bscTest1 = BSCTest1(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BSCTest1_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BSCTest1_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BSCTest1 contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(bscTest1);

            uint256 prevBalance = bscTest1.balanceOf(address(this));
            ...
            {
                uint256 received = bscTest1.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    bscTest1.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
