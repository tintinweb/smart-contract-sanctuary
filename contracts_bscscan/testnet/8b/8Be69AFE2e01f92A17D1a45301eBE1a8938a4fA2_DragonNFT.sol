...
contract ERC721 is Context, ERC165, IERC721, IERC721Enumerable {
    ...
    event Birth(address owner, uint256 tokenId, uint256 dragonId);
    ...
    struct Dragon {
        ...
        // The timestamp from the block when this Dragon came into existence.
        uint64 birthTime;
    }

    /*** STORAGE ***/
    /// @dev An array containing the Dragon struct for all Dragons in existence. The ID
    ///  of each Dragon is actually an index into this array. Note that ID 0 is a negaDragon,
    ///  the unDragon.
    Dragon[] dragons;
    ...
    * @dev See {IERC721Enumerable-symbol}.
     ...
     * Emits a {Transfer} event.
     */
    function _safeMint(address to, uint256 tokenId, uint256 dragonId) internal virtual {
        _safeMint(to, tokenId, dragonId,"");
    }
     ...
     */
    function _safeMint(address to, uint256 tokenId, uint256 dragonId, bytes memory _data) internal virtual {
        _mintDragon(to, tokenId, dragonId);
     ...
     * Emits a {Transfer} event.
     */
    function _mintDragon(address to, uint256 tokenId, uint256 dragonId) internal virtual {
        ...
        emit Birth(to, tokenId, _dragon.dragonId);
...
contract DragonNFT is ERC721Pausable, AccessControl, Ownable {
    ...
    address public DragonBattlesToken = 0x12909F1C54D10D5A28a043Af2d924BFEf9ad1616;
    ...
    function addDragonId(uint256[] calldata _dragonId) external onlyOwner {
        for(uint i=0; i<_dragonId.length; i++) {
            rangeOfId.push(_dragonId[i]);
        }
    }
    ...
    function createDragon(address _owner, uint256 _amount) external {
        require(_amount >= price, "INVALID AMOUNT");
        require(rangeOfId.length > 0, "INSUFFFICIENT Dragon ID");
        require(IERC20(DragonBattlesToken).transferFrom(msg.sender, address(this), _amount));
        
        uint256 tokenId = nextTokenId;
        uint256 index = _randomDragonId();
        uint256 dragonId = rangeOfId[index];

        _mintDragon(_owner, tokenId, dragonId);
        _remove(index);
    ...
    function mintDragonFromSwap(address _owner, uint256 _dragonId) external onlySwap {
        uint256 tokenId = nextTokenId;

        _mintDragon(_owner, tokenId, _dragonId);
    ...
    function burnDragon(uint256 _tokenId) external {
        ...
        IERC20(DragonBattlesToken).transfer(msg.sender, amount);
    }
    ...
    function getDragon(uint256 _id)
        ...
        Dragon storage dragon = dragons[_id - 1];

        birthTime = uint256(dragon.birthTime);
        dragonId = dragon.dragonId;
    }
    
    function _randomDragonId() internal returns (uint256) {
    ...
    function getBalance() public view returns(uint256) {
        return IERC20(DragonBattlesToken).balanceOf(address(this));
    }
    
    function withdrawBalance() external onlyOwner {
        IERC20(DragonBattlesToken).transfer(owner(), getBalance());
    }
