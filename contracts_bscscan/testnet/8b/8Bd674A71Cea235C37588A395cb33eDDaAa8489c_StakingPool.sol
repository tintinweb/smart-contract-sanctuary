...
contract StakingPool {
  ...
  uint public lastUpdatePoolTime;
  ...
  function getStakingTokenBalance() external view returns (uint) {
  ...
  function updateRewardRate(uint _amount) external {
    ...
    uint remaining = endTime.sub(block.timestamp).mul(rewardRate);
    rewardRate = _amount.add(remaining).div(endTime);
    lastUpdatePoolTime = block.timestamp;
  }
  ...
  function updateReward(address _user) internal {
    ...
    if (_user != address(0)) {
      userReward[_user] = userReward[_user].add(rewardPerToken);
    }
  ...
  function getRewardPerToken() public view returns (uint) {
    ...
    return rewardPerToken.add(
      block.timestamp.sub(lastUpdatePoolTime)
      .mul(rewardRate).mul(1e18)
      .div(stakingSupply)
    );
  }
