...
// File: contracts\libs\IArfiliatePool.sol

pragma solidity ^0.8.0;

interface IArfiliatePool {
    function deposit(address _account, uint256 _amount) external view returns (uint256);
}
...
contract Merchant is Pausable {
    ...
    IArfiliatePool public arfiliatePool;
    address public arfiliatorWallet;
    ...
    event ArfiliatePoolUpdated(
        IArfiliatePool previousPool,
        IArfiliatePool newPool
    );
    ...
    function initialize(
        ...
        IArfiliatePool _arfiliatePool,
        address _arfiliatorWallet,
        address _admin
    ) external {
        ...
        require(
            _wpayTokenAddress != address(0),
            "Mearchant:: invalid WPAY token address"
        );
        require(
        ...
        require(
            _arfiliatorWallet != address(0),
            "Merchant::invalid arfiliator wallet address"
        );
        ...
        arfiliatePool = _arfiliatePool;
        arfiliatorWallet = _arfiliatorWallet;
    ...
    function handleFee() internal {
        ...
        if (feeProcessingMethod == FeeMethod.ARFLIQU) {
            require(
                address(wpayBnbPair) != address(0),
                "Invalid WPAY-BNB Pair"
            );

            // 55% of staked fee is swapped to WPAY token
            uint256 buyupBalance = etherBalance.mul(55).div(100);
            swapBnbToWpay(buyupBalance);

            uint256 wpayBalance = WPAY_TOKEN.balanceOf(address(this));

            WPAY_TOKEN.approve(address(arfiliatePool), wpayBalance.mul(10).div(55));
            // 5% amount of WPAY token is deposited to arfiliate pool for merchant
            uint256 refSourceFeeBalance = wpayBalance.mul(5).div(55);
            if (refSourceFeeBalance > 0) {
                arfiliatePool.deposit(merchantWallet, refSourceFeeBalance);
            }

            // 5% amount of WPAY token is deposited to arfiliate pool for arfiliator
            uint256 refTargetFeeBalance = wpayBalance.mul(5).div(55);
            if (refTargetFeeBalance > 0) {
                arfiliatePool.deposit(arfiliatorWallet, refTargetFeeBalance);
            }

            // WPAY + BNB to liquidity
            uint256 liqifyBalance = WPAY_TOKEN.balanceOf(address(this));
            // Add liquidity
    ...
    function updateArfiliatePool(IArfiliatePool _arfiliatePool)
        external
        onlyOwner
    {
        require(
            address(arfiliatePool) != address(_arfiliatePool),
            "Mearchant:: already set"
        );
        emit ArfiliatePoolUpdated(arfiliatePool, _arfiliatePool);
        arfiliatePool = _arfiliatePool;
    }
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            stakingContract.balanceOf(_msgSender()) >= wpayBalanceForFreeTransaction
        ) {
            return 0;
        }
