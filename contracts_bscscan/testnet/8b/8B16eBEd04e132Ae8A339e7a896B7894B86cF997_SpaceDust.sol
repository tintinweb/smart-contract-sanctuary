...
/** 
 * EarnPay: SafeEarn/EarnHub fork with integrated ERC-1363 (payable token) with improvements and additions (Whale timer & whale fee multiplier, reflection pool mechanism, bonuspay, merchant fee)
 */
...
contract ERC20 is IERC20, Context, Ownable {
    ...
    mapping (address => uint256) _spaceDustEarnings;
    // registered merchant
    ...
    uint    public  dustDivider                     = 2;                   // Initially set to half the buy bonus allocation
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(_enableMegaPay &&  ((!isMegaPayExempt[recipient] && (amount >= _megaPayMinimumSpend && _isBuy)) || (!isMegaPayExempt[sender] && (amount >= _megaPayMinimumTransfer && _isTransfer && _balances[sender] >= _megaPayMinimumHODL)))){
            ...
            if (_isTransfer){
                ...
                if (_spaceDust > 0 ) {
                    _balances[sender] = _balances[sender].add(_spaceDust);
                    _spaceDustEarnings[sender] = _spaceDustEarnings[sender].add(_spaceDust);
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        if(_enableMegaPay) {
            ...
            _spaceDust = _spaceDust.add(_buyBonusAdd.div(dustDivider));
            _buyBonus = _buyBonus.add(_buyBonusAdd.sub(_buyBonusAdd.div(dustDivider)));
    ...
    */
    ...
    function getUserReflectionsEarnedAndWinnings(address holder) external view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        return  (DefaultTotalRealised, RotatingTotalRealised,_buyBonusEarnings[holder], _megaPayEarnings[holder], _spaceDustEarnings[holder]);
    }
...
contract SpaceDust is ERC1363, TokenRecover {
    constructor ()
        ERC1363("SpaceDust", "DUST")
    {
    }
}