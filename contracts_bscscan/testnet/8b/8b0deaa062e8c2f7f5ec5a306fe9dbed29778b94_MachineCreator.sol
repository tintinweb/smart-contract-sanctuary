...
contract MachineGift is ERC721Enumerable {
    struct NFT {
        address creator;
        uint256 price;
    }
    ...
    constructor(address _machineCreator) ERC721("test", "TST") {
        machineCreator = _machineCreator;
    }

    function mint(uint256 _price, string memory _uri)
        public
...
contract MachineCreator is Ownable, IERC721Receiver, ERC1155Holder {
    ...
    mapping(uint256 => ClawMachine) public machineById;
    mapping(uint256 => NFT) public NFTById;

    mapping(uint256 => bool) public isMachineEmpty;
    mapping(address => bool) public isAcceptedNFT;
    mapping(address => uint256[]) public userMachines;

    constructor() {
        priceLevels = [5, 50, 500, 5000, 50000];
        licensePrices = [10, 100, 1000, 10000, 100000];         
    }

    function addTicketAccepted (address _ticket) external onlyOwner{
    ...
    function createMachine(
    ...
    ) external returns (uint256 machineId) {
        ...
        machineById[machineId] = ClawMachine({
            name: _name,
            owner: msg.sender,
        ...
        MachineCount++;
        activeMachineIndex[machineId] = activeMachines.length;
        activeMachines.push(machineId);
    }

    function batchAddToMachine ( uint256 _machineId, uint256[] memory _ids, address[] memory _addresses) external {
        require(machineById[_machineId].owner == msg.sender, "Only the owner can add to the machine");
        for (uint256 i = 0; i < _ids.length; i++) {
            addNFTtoMachine(_ids[i], _addresses[i], _machineId);
        }
    }

    function addNFTtoMachine(
        uint256 _id,
        address _address,
        uint256 _machineId
    ) internal returns (uint256 NFTid) {
        require(
            block.timestamp < machineById[_machineId].expirationDate,
            "Machine is expired"
        );
        uint256 machineSize = machineById[_machineId].size;
        uint256 tokenPrice = MachineGift(_address).getPrice(_id);
        require(
            priceLevels[machineSize] >= tokenPrice,
            "Gift price to high for this machine size"
        );
    ...
    function play(uint256 _machineId) external {
        require(!isMachineEmpty[_machineId], "This machine is empty");
        require(
            block.timestamp < machineById[_machineId].expirationDate,
            "Machine is expired"
        );
        require(
            block.timestamp > machineById[_machineId].lockedTime,
            "Machine Locked: Someone is playing!"
        );

        ticketsContract.safeTransferFrom(
            msg.sender,
            machineById[_machineId].owner,
            0,
            machineById[_machineId].price,
            "0x00"
        );
    ...
    function claimReward(uint256[] memory _NFTIds, uint256 _machineId)
        external
    {
        require(
            machineById[_machineId].currentPlayer == msg.sender,
        ...
        machineById[_machineId].itemsAvailable -= _NFTIds.length;
    ...
    function destroyMachine(uint256 _machineId, uint256[] memory _NFTIds)
        external
    {
        require(
            machineById[_machineId].owner == msg.sender,
            "only the machine owner can destroy it"
        );
        require(
            block.timestamp > machineById[_machineId].expirationDate,
            "Machine is not expired"
        );

        uint256 itemsAvailable = machineById[_machineId].itemsAvailable;
        if (itemsAvailable > 0) {
            require(
                _NFTIds.length == itemsAvailable,
                "withdraw all items available before destroy"
            );
    ...
    function withdrawFromMachine(uint256[] memory _NFTIds, uint256 _machineId)
        external
    {
        require(
            machineById[_machineId].owner == msg.sender,
            "only the machine owner can withdraw"
        );
        require(
            block.timestamp > machineById[_machineId].expirationDate,
            "Machine is not expired"
    ...
    function transferNFT(uint256 NFTid, uint256 _machineId) internal {
        ...
        require(
            nft.machineId == _machineId,
            "this token doesn't below to this machine"
        );
    ...
    function removeFromActiveMachine(uint256 _machineId) internal {
        ...
        activeMachines.pop();
        isMachineEmpty[_machineId] = true;
    }
    ...
    function machineNFTs(uint256 _machineId)
        external
        view
        returns (uint256[] memory)
    {
    ...
    function machineNFTsData(uint256 _machineId)
        external
        view
        returns (NFT[] memory)
    {
    ...
    function psuedoRandomness(uint256 _random) public view returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
