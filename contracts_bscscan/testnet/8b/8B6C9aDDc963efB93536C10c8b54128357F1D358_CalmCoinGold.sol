...
contract Gold is  CalmCoinGold{
    /* * @notice We usually require to know who are all the stakeholders.
     */
      address[] internal stakeholders;
     CalmCoinGold public CCS;
     /* * @notice The accumulated rewards for each stakeholder.
    */
     mapping(address => uint256) public stakes;
    function rewardCCS(address _reward)onlyOwner public{
    CCS = CalmCoinGold(_reward);
    }
   
   function rewards(address stakeholder) public view returns(uint256){
      return CCS.balanceOf(stakeholder);
   }
     ...
     function distributeRewards()public onlyOwner
   {
       for (uint256 s = 0; s < stakeholders.length; s += 1){
          // address stakeholder = stakeholders[s];
           ...
           CCS._mint(stakeholders[s], silver);
       }
   }
