...
contract pepeFlokiDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("pepeFloki_Dividend_Tracker", "pepeFloki_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "pepeFloki_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "pepeFloki_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main pepeFloki contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "pepeFloki_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "pepeFloki_Dividend_Tracker: Cannot update claimWait to same value");
...
contract pepeFloki is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    pepeFlokiDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 10**6 * (10**18);
    uint256 public maxBuyTransactionAmount = 10**6 * (10**18);
    ...
    constructor() ERC20("pepeFloki", "pepeFLOKI") {
        BNBRewardsFee = 8;
        liquidityFee = 2;
        marketingFee = 6;
        marketingWallet = payable(0x6881a5a2c88309df3E9C1B052916a6Ef9a7f3eCB);
        maxWalletToken = 30000000 * (10**18);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new pepeFlokiDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "pepeFloki: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "pepeFloki: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "pepeFloki: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "pepeFloki: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "pepeFloki: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "pepeFloki: Cannot update gasForProcessing to same value");
