...
// MasterChef is the master of PSP. He can make PSP and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once PSP is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of PSPs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accPspPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPspPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. PSPs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that PSPs distribution occurs.
        uint256 accPspPerShare;   // Accumulated PSPs per share, times 1e12. See below.
    ...
    // PSP tokens created per block.
    uint256 public pspPerBlock;
    // Bonus muliplier for early psp makers.
    ...
    // The block number when PSP mining starts.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _pspPerBlock,
        uint256 _startBlock
    ) public {
        psp = _psp;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        pspPerBlock = _pspPerBlock;
    ...
    // Update the given pool's PSP allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending PSPs on frontend.
    function pendingPsp(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accPspPerShare = pool.accPspPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 pspReward = multiplier.mul(pspPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accPspPerShare = accPspPerShare.add(pspReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accPspPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 pspReward = multiplier.mul(pspPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        psp.mint(devaddr, pspReward.div(10));
        psp.mint(address(this), pspReward);
        pool.accPspPerShare = pool.accPspPerShare.add(pspReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for PSP allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accPspPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safePspTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accPspPerShare).div(1e12);
    ...
    // Safe psp transfer function, just in case if rounding error causes pool to not have enough PSPs.
    function safePspTransfer(address _to, uint256 _amount) internal {
        uint256 pspBal = psp.balanceOf(address(this));
        if (_amount > pspBal) {
            psp.transferWithLock(_to, pspBal);
        } else {
            psp.transferWithLock(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _pspPerBlock) public onlyOwner {
        massUpdatePools();
        pspPerBlock = _pspPerBlock;
    }
    
    // Setup PSP Token Reward Release 
    function setPspRewardLock(uint256 lock) public onlyOwner {
        psp.setRewardLock(lock);
    }
    
    function setPspTotalBlockRelease(uint256 totalBlockRelease) public onlyOwner {
        psp.setTotalBlockRelease(totalBlockRelease);
    }
}
...
contract PSPToken is BEP20('PSP Token', 'PSP') {
