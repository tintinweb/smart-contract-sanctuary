...
//////////CHANGE ABOVE//////////
    ...
    uint256 public totalFees = DividendRewardsFee.add(marketingFee).add(liquidityFee);
    ...
    constructor() ERC20("TestDev", "DevF") {
    	...
    	marketingWallet = 0x40eA6A1471202FFBa9a051973e6eB75531291F1e; // Marketing Wallet
    	DividendToken = 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca; // $Token Address
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "DevFeeToken: The marketing wallet is already this address");
    ...
    function setAuthOnDividends(address account) public onlyOwner{
        TokenDividendTracker.setAuth(account);
    }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(DividendRewardsFee).add(liquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateTokenDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(TokenDividendTracker), "DevFeeToken: The dividend tracker already has that address");
 
        TOKENDividendTracker newTokenDividendTracker = TOKENDividendTracker(payable(newAddress));
 
        require(newTokenDividendTracker.owner() == address(this), "DevFeeToken: The new dividend tracker must be owned by the DevFeeToken token contract");
        ...
        emit UpdateTokenDividendTracker(newAddress, address(TokenDividendTracker));
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "DevFeeToken: Cannot update gasForProcessing to same value");
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
		(uint256 firstIterations, uint256 firstClaims, uint256 firstLastProcessedIndex) = TokenDividendTracker.process(gas);
		emit ProcessedTokenDividendTracker(firstIterations, firstClaims, firstLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function _transfer(
        ...
        if (!swapping && canSwap && from != uniswapV2Pair) {
            ...
            if (DividendEnabled) {
                uint256 firstTokens = contractTokenBalance.div(totalFees).mul(DividendRewardsFee);
                swapAndSendDividends(firstTokens);
            }
        ...
        try TokenDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try TokenDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
 
        if(!swapping) {
	    	uint256 gas = gasForProcessing;
 
	    	try TokenDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
	    		emit ProcessedTokenDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
	    	}
	    	catch {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        transferDividends(DividendToken, address(TokenDividendTracker), TokenDividendTracker, Dividends);
    }
...
contract TOKENDividendTracker is DividendPayingToken, Ownable {
    ...
    //set divident tracker name, alice token address
    constructor() DividendPayingToken("Rewards_Dividend_Tracker", "Rewards_Dividend_Tracker", 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "Rewards_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "Rewards_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DevFeeToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Rewards_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Rewards_Dividend_Tracker: Cannot update claimWait to same value");
