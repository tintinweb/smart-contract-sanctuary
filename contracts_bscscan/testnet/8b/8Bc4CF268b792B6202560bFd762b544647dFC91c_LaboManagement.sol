...
contract LaboManagement is Laboratory {
    // Les potions donnent un X3 sur l'xp gagné en combat

    using SafeMath for uint256;
    uint256 potionsStandardPrice = 100 * 1E18;
    uint256 workingSpotPrice = 10000 * 1E18;
    ...
    modifier onlyLaboOwner(uint256 _laboId) {
        require(ownerOf(_laboId)==msg.sender);
        _;
    }

    function setPotionStandardPrice(uint256 _price) public onlyOwner {
    ...
    function addWorkingSpotToLab(uint256 _laboId) external onlyLaboOwner(_laboId) {
        require(numberOfWorkingSpots[_laboId] <= 9, "Max spots already hit");
        address _paymentAddress = gameAddresses.getPaymentsAddress();
        require(BZAI.transferFrom(msg.sender,_paymentAddress , workingSpotPrice));
        IPayments(_paymentAddress).distributeFees(workingSpotPrice);
        numberOfWorkingSpots[_laboId] =numberOfWorkingSpots[_laboId].add(1);
    }

    function workInASpot(uint256 _monsterId, uint256 _laboId, uint256 _spotId) external {
        address _monsterAddress = gameAddresses.getMonsterAddress();
        require(IERC721(_monsterAddress).ownerOf(_monsterId) == msg.sender,"Not your monster");
        require(IBandZaiNFT(_monsterAddress).isFree(_monsterId),"Not Free");
        require(numberOfWorkingSpots[_laboId] >=_spotId, "Not enough spots");
        WorkInstance storage w = workingSpot[_laboId][_spotId];
        require(w.monsterId == 0 || block.timestamp > w.beginingAt.add(1 days), "Spot not free");
        if(block.timestamp < w.beginingAt.add(1 days).add(600)){
            require(
                IOracle(gameAddresses.getOracleAddress()).
                    getRandom(keccak256(abi.encodePacked(_monsterId,block.timestamp,msg.sender))).mod(100)
                <= 5,
        ...
        if(w.monsterId != 0){
            _updateMonster(w.monsterId,block.timestamp.sub(w.beginingAt));
        }else{
            employees[_laboId] = employees[_laboId].add(1);
        }
        w.monsterId = _monsterId;
        w.beginingAt = block.timestamp;
        IBandZaiNFT(gameAddresses.getMonsterAddress()).updateStatus(_monsterId,3,_laboId);
    }

    function stopWorking(uint256 _monsterId, uint256 _laboId, uint256 _spotId) external {
        require(IERC721(gameAddresses.getMonsterAddress()).ownerOf(_monsterId) == msg.sender,"Not your monster");
        WorkInstance storage w = workingSpot[_laboId][_spotId];
        require(w.monsterId == _monsterId,"Not your monster");
        _updateMonster(_monsterId,block.timestamp.sub(w.beginingAt));
        employees[_laboId] = employees[_laboId].sub(1);
        _updateCredits(_laboId);
    }

    function _updateMonster(uint256 _monsterId, uint256 _xpAlchemy) internal {
        IBandZaiNFT(gameAddresses.getMonsterAddress()).updateStatus(_monsterId,0,0);
        IBandZaiNFT(gameAddresses.getMonsterAddress()).updateAlchemyXp(_monsterId,_xpAlchemy.div(1000)); //1pts = 1000 seconds
    }

    function buyStandardPotion(uint256 _potionType) public returns(uint256){
        require(_potionType <= 4, "Doesn't exist");
        require(BZAI.transferFrom(msg.sender, gameAddresses.getPaymentsAddress(), potionsStandardPrice));
        IPayments(gameAddresses.getPaymentsAddress()).distributeFees(potionsStandardPrice);
    ...
    function getCredit(uint256 _laboId) external view returns (uint256) {
        uint256 _credits;
        if(_creditsLastUpdate[_laboId] == 0){
            _credits = 0;
        }else{
            _credits = _potionsCredits[_laboId].add(
            (block.timestamp.sub(_creditsLastUpdate[_laboId])).mul(employees[_laboId])
        );
        }
        return _credits;
    }
    ...
    function createPotion(
        ...
        require(_price > potionsStandardPrice, "You can't sell lower than game");
        require(IOpenAndClose(gameAddresses.getOpenAndCloseAddress()).canLaboSell(_laboId), "You can't");
        _updateCredits(_laboId);

        _potionsCredits[_laboId] = _potionsCredits[_laboId].sub(_power.mul(10000), "Not enough credits");
        createdPotionsForLab[_laboId] = createdPotionsForLab[_laboId].add(1);

        return IPotions(IAddresses(gameAddresses).getPotionAddress()).mintPotion(_laboId,_price,_type,_power);
    ...
    function buyPotion(uint256 _potionId) public {
        ...
        uint256 [3] memory potion = IPotions(_potionAddress).getPotion(_potionId); // 0: FromLabo ; 1: Price ; 2: numberOfUse
        require(BZAI.transferFrom(msg.sender, gameAddresses.getPaymentsAddress(), potion[1]));
        address ownerOfLabo = ownerOf(potion[0]);

        IPayments(gameAddresses.getPaymentsAddress()).payOwner(ownerOfLabo, potion[1]);
        laboratoryRevenues[potion[1]] = laboratoryRevenues[potion[0]].add(potion[1]);

        IERC721(_potionAddress).transferFrom(address(this),msg.sender, _potionId);
    }

    function _updateCredits(uint256 _laboId) internal {
        uint256 _timePassed = block.timestamp.sub(_creditsLastUpdate[_laboId]);
        _creditsLastUpdate[_laboId] = block.timestamp;
        _potionsCredits[_laboId] = _potionsCredits[_laboId].add(_timePassed.mul(employees[_laboId])); 
    }
}
...
contract Laboratory is ERC721,ERC721Holder, Ownable {
    ...
    mapping(uint256 => uint256) public numberOfWorkingSpots;
    mapping(uint256 => uint256) _creditsLastUpdate; // timestamp - creditLastUpdate + potionsCredits

    constructor() ERC721("LABO_NFT", "LAB") {}

    modifier onlyAuth() {
        require(msg.sender == gameAddresses.getOpenAndCloseAddress(), "!Auth");
        _;
    }
        ...
        returns (uint256)
    {
        ...
        numberOfWorkingSpots[_newItemId] = 5;
...
interface IBandZaiNFT {
    ...
    function getMonsterLevel(uint256 _tokenId)external view returns (uint256 level);

    function getMonsterPowers(uint256 _tokenId)external view returns (uint256[2] memory);
...
interface INursery {
    function mintNursery(address _to) external returns (uint256);
...
interface IPotions {
    ...
    function getPotion(uint256 _itemId) external view returns(uint256[3] memory);
...
interface IAddresses {
    ...
    function getInGameRewardsAddress() external view returns(address);
    ...
    function getReserveRewardsAddress() external view returns(address);
