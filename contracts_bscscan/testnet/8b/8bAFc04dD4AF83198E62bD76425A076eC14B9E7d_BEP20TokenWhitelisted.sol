...
/** 
 *  SourceUnit: c:\Users\anike\Work\akru-contracts\src\staking\StakingRewards.sol
*/
...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Ownable, BaseRelayRecipient {
    ...
    function stake(uint256 amount) external override nonReentrant updateReward(_msgSender()) {
        require(block.timestamp <= periodFinish, "Staking period is over.");
        require(amount > 0, "Cannot stake 0");
        _totalSupply = _totalSupply.add(amount);
        _balances[_msgSender()] = _balances[_msgSender()].add(amount);
        stakingToken.transferFrom(_msgSender(), address(this), amount);
    ...
    function exit() external override {
        require(block.timestamp > periodFinish, "Staking period not over yet. Withdraw once staking period is over");
