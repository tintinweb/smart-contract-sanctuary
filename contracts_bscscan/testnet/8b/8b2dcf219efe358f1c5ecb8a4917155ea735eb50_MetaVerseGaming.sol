...
contract MetaVerseGaming is Ownable, ERC20, ERC20Burnable {
    using SafeMath for uint256;

    mapping(address => bool) private excluded_fee_accounts;
    ...
    uint256 constant  _tokenSupply = 100_000_000_000;

    uint256 _perFeeBurn = 1;
    uint256 _perFeeDev = 4;
    uint256 _perFeeMarketing = 8;


    bool _setup = false;
    bool _burnEnable = true;
    bool _isActiveMarketingFee = true;
    bool _isActiveDevelopmentFee = true;
    bool _checkSwapAndLiqidifyLock;


    IUniswapV2Router02 public  uniswapV2Router;

    address public  uniswapPair;   // uniswap pair

    address payable marketing_dev_address;   // marketing and development address

    address owner_liquidity_address;    // owner address

    uint256 private t_amount_liquidity; // total amount to liquidity.
    uint256 private t_amount_dev_marketing; // total amount to development and marketing.
    ...
    constructor() ERC20(_tokenName, _tokenSymbol) {
        ...
        _totalSupply = _tokenSupply * (10 ** _tokenDecimals);
        _mint(_msgSender(), _totalSupply);
        excluded_fee_accounts[address(this)] = true;
        marketing_dev_address = payable(msg.sender);
        owner_liquidity_address = msg.sender;
        _excludeFee(owner_liquidity_address);
        // testnet
...
//        uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function getTotalAmountsToLiquidify() public view returns (uint256) {
        return t_amount_liquidity;
    }

    /**
     * get t_amount_dev_marketing
     */
    function get_amount_dev_marketing() public view returns (uint256) {
        return t_amount_dev_marketing;
    }


    //disable all fees
    function setEnableAllFees(bool _option) external onlyOwner {
        _isActiveDevelopmentFee = _option;
        _burnEnable = _option;
        //isLiquidityEnabled = _option;
        _isActiveMarketingFee = _option;
    }


    //set dev and marketing fund  wallet
    function setDevAndMarketingFundWallet(address payable _wallet) external onlyOwner {
        marketing_dev_address = _wallet;
    }
    ...
    modifier lockSwapAndLiquify {
        _checkSwapAndLiqidifyLock = true;
        _;
        _checkSwapAndLiqidifyLock = false;
    }
    ...
    */
    ...
    0, //amountTokenMin
    0, //amountETHMin
    owner_liquidity_address, //owner of the liquidity
    ...
    function getTotalFee() public view returns (uint256){
        ...
        if (_burnEnable) {fee += _perFeeBurn;}
        if (_isActiveDevelopmentFee) {fee += _perFeeDev;}
        if (_isActiveMarketingFee) {fee += _perFeeMarketing;}

        return fee;
    }

    function _excludeFee(address _account) public onlyOwner() returns (bool){
        excluded_fee_accounts[_account] = true;
        return true;
    }

    function _removeExcludedFromFees(address _account) public onlyOwner() returns (bool){
        delete excluded_fee_accounts[_account];
        return true;
    }

    function _isExcludedFromFees(address _account) public view returns (bool){
        return excluded_fee_accounts[_account];
    }
    ...
    function _preProcessTransfer(address sender, address recipient, uint256 amount) private returns (uint256) {
        ...
        if (_isExcludedFromFees(sender) || _isExcludedFromFees(recipient) || _checkSwapAndLiqidifyLock) {
            return amount;
        }
        ...
        //process burn
        if (_perFeeBurn > 0 && _burnEnable) {
            uint256 _amountToBurn = computePecentToAmount(_perFeeBurn, amount);
        ...
        if (_isActiveDevelopmentFee && _perFeeDev > 0) {
            _devAndMarketingAmount = computePecentToAmount(_perFeeDev, amount);
            _devMarketingLiquidityAmount += _devAndMarketingAmount;
        }

        if (_isActiveMarketingFee && _perFeeMarketing > 0) {
            uint256 marketingAmt = computePecentToAmount(_perFeeMarketing, amount);
        ...
        /*if(_liquidityFeeAmount > 0) {
            ...
            t_amount_liquidity = t_amount_liquidity + _liquidityFeeAmount;

            if(sender != uniswapPair && t_amount_liquidity > 0) {

                //take snapshot
                uint256 amounToLiquidify = t_amount_liquidity;

                //lets swap and provide liquidity
                swapAndLiquify(amounToLiquidify);

                //lets
                t_amount_liquidity = t_amount_liquidity.sub(amounToLiquidify);
            } //end if
        ...
        */
        ...
        if (_devAndMarketingAmount > 0) {
            ...
            t_amount_dev_marketing = t_amount_dev_marketing.add(_devAndMarketingAmount);

            if (sender != uniswapPair && t_amount_dev_marketing > 0) {

                _checkSwapAndLiqidifyLock = true;

                uint256 _currentDevAndMarketingAmountSnapShot = t_amount_dev_marketing;

                uint256 swappedBNBAmount = swapTokenForBNB(_currentDevAndMarketingAmountSnapShot);

                if (swappedBNBAmount > 0) {
                    marketing_dev_address.call{value : swappedBNBAmount}("");
                }

                t_amount_dev_marketing = t_amount_dev_marketing.sub(_currentDevAndMarketingAmountSnapShot);
