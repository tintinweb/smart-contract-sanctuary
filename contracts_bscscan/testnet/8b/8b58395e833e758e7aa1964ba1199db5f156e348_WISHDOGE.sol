...
contract WISHDOGEDividendTracker is DividendPayingToken, Ownable {
    ...
    function excludeFromWhale(address addr) public onlyOwner()
    {
        _isExcludedFromWhale[addr] = true;    
    }
    

    function isExcludedFromWhale(address addr) public view returns(bool)
    {
        return _isExcludedFromWhale[addr];
    }

    function includeInWhale(address addr) public onlyOwner()
    {
        _isExcludedFromWhale[addr] = false;    
    }
...
contract WISHDOGE is ERC20, Ownable, SafeToken, LockToken {
    ...
    uint256 public maxlimitForAntiWhale = 10000000 * 10**18;
    ...
    mapping (address => uint256) private lastTxnTime;
    ...
    constructor() ERC20("Wish Doge Dragon", "WISHDOGE") {
        ...
        dividendTracker.excludeFromWhale(owner());
        dividendTracker.excludeFromWhale(uniswapV2Pair);
        dividendTracker.excludeFromWhale(address(this));
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function applyFee(address from) private
    {
        uint256 lapse = block.timestamp-getLastTxnTime(from);
        if(lapse<3600)
        {
           extraFeeOnSell = 4;
        }
    ...
    function _transfer(address from, address to, uint256 amount) internal override 
    {
        ...
        if(dividendTracker.isExcludedFromWhale(to)) 
        {
            uint256 walletBalance = balanceOf(to).add(amount);
            require( walletBalance <= maxlimitForAntiWhale, "Wallet Limit exceeding");
        }
    ...
    function getLastTxnTime(address addr) public view returns(uint256)
    {
        return lastTxnTime[addr];
    }

    function setLastTxnTime(address addr) internal
    {
        lastTxnTime[addr] = block.timestamp;
    }
