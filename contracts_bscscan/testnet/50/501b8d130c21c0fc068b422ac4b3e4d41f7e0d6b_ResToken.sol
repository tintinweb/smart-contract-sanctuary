...
contract ResToken is BEP20 {
    ...
    bool public swapToBNBEnabled = false;
	// start Block Swap
    uint256 public startBlockSwap = 19352680;
    // Min amount to swap. (default 10 RESs)
    uint256 public minAmountToSwap = 10 ether;
	// Balance BNB
	uint256 public balanceBNB;
	// Min buy to get Reward	
	uint256 public minBuyBid;
	// time bid	
	uint16 public timeBid;	
	IBEP20 public BUSD = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);	
	address public test = address(BUSD);	
     ...
     */
    constructor() public BEP20("Res1 Token", "RES") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
		...
		if (limitSwap == true && _excludedPoolsContracts[sender] == false && _excludedPoolsContracts[recipient] == false) {	
			...
			if(swapToBNBEnabled == true && amount >= minBuyBid && buyTransaction == true){
				uint256 checkWin = block.number.sub(winnersInfo[1].block);				
				if(checkWin > timeBid){
					uint256 lastReward = BUSD.balanceOf(address(this));
					BUSD.safeTransfer(winnersInfo[1].user, lastReward);
				...
				winnersInfo[1].user = userAddress;
				winnersInfo[1].reward = BUSD.balanceOf(address(this));
				winnersInfo[1].block = block.number;				
			}				
		}
    ...
    function swapToBNB() private lockTheSwap transferTaxFree {
        ...
        if (contractTokenBalance >= minAmountToSwap) {
			...
			balanceBNB = BUSD.balanceOf(address(this));
            emit SwapToBNB(swapAmount);
        }
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        // make the swap
        resRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
