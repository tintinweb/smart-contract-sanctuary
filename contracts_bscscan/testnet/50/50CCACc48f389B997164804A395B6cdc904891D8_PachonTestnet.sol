...
contract PachonTestnet is Context, BEP20, Ownable {
    ...
    string private _name = "Pachon Testnet";
    string private _symbol = "PACHT";
    ...
    address public _charityAddress =  0x22A58CA8923e99dD4d23764a398099B41C960FC3;
    address public _marketingAddress = 0x22A58CA8923e99dD4d23764a398099B41C960FC3;
    ...
    function lockMaxTxAddress(address account) external onlyOwner() {
    ...
    function setMarketingAddress(address account) external onlyOwner() {
        require(_marketingAddress != account, "ERROR: Can not set to same address.");
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256, uint256) {
        uint256 charityFee = _amount.mul(_charityFee).div(
            10**2
        );
        
        uint256 marketingFee = _amount.mul(_marketingFee).div(
            10**2  
        );
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(takeFee) {
            (uint256 charityFee, uint256 marketingFee) = calculateTaxFee(amount);
            uint256 transferAmountAfterFee = amount.sub(charityFee).sub(marketingFee);
            
            _balance[recipient] = _balance[recipient].add(transferAmountAfterFee);
            _balance[_charityAddress] = _balance[_charityAddress].add(charityFee);
            _balance[_marketingAddress] = _balance[_marketingAddress].add(marketingFee);
