...
contract BurnKWTContract is Ownable, SignData {
    ...
    constructor(IBEP20 _tokenKWT) public {
        tokenKWT = _tokenKWT;
        tokenKWT.approve(address(this), uint256(- 1));
    }
    ...
    function config(uint256 _rate, bool _isPause, uint256 _maxBurn) public onlyOwner {
        burnConfig = BurnConfig(_rate, _isPause, _maxBurn);
    }
    ...
    function _burn(address _caller, uint256 _amount) internal {
        uint256 burnT0 = totalBurn.add(_amount);

        require(burnConfig.maxBurn >= burnT0, "total burn max");

        require(burnConfig.rate != 0, "rate burn is zero");

        tokenKWT.safeTransferFrom(_caller, address(this), _amount);

        tokenKWT.burnFrom(address(this), _amount);

        uint256 amountRefund = _amount.mul(burnConfig.rate).div(1e3);
    ...
    function _refundToken(address _caller, uint256 _amount) internal {
        require(burnConfig.pause == true, "is pause");
