...
contract Babylon is IBEP20, Auth {
    ...
    uint256 minHoldingDays = 180 ; //180 days min holding ~ 6 Months
    bool public sellTaxEnabled = true;
    uint256 sellTaxDayFee = 5 ; // 0.05% --> Extra Sales Tax 0% : 9% --> Total Sales Fee from 10% : 19%
    //
    struct BuyRecord {
        uint256 timestamp;
        uint256 amount;
    ...
    function consumeBuyRecord(address holder, uint256 amount)   internal returns (uint256) {
        ...
        if (amount == 0 || _buyRecords[holder].length == 0)
            return minHoldingDays;  //180 days
    ...
    function setSellTaxSettings(bool _sellTaxEnabled, uint256 _minHoldingDays, uint256 _sellTaxDayFee) external authorized {        
        //
        require(_minHoldingDays <= 180 && _sellTaxDayFee <= 5 );        
        //
