...
contract Cashier is ICashier{
    ...
    address token1 = address(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684); // Binance pegged Usdt
    address token2 = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); // BUSD
    ...
    function swapToken2(uint256 bnb) internal {
        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = token2;
    ...
    function rewardBothForQualified(address shareholder, uint256 amount) private {
        ...
        IERC20 t1 = IERC20(token1);
        uint256 token1Before = t1.balanceOf(address(this));
        uint shibAmount = amount.mul(t1Percent).div(100);
        swapToken(shibAmount);
        t1.transfer(shareholder, t1.balanceOf(address(this)) - token1Before);
        
        // Swap half stored BNB for token 2 and send the acquired tokens to shareholder
        
        IERC20 t2 = IERC20(token2);
        uint256 token2Before = t2.balanceOf(address(this));
        uint xrpAmount = amount.mul(t2Percent).div(100);
        swapToken2(xrpAmount);
        t2.transfer(shareholder, t2.balanceOf(address(this)) - token2Before);
    }
...
contract Rafiki is IERC20 {
    ...
    string constant _name = "Rafiki";
    string constant _symbol = "RAF06";
    ...
    uint256 private _totalFee = _liquidityFee + _reflectionFee + _marketingFee + _devFee;
    ...
    uint256 private previousDevFee = _devFee;
    ...
    uint256 private devRatio = _sellDevFee;
    ...
    address payable private _devWallet;// Enter dev wallet here also
    ...
    uint256 private maxWalletPercent = 30; // Less fields to edit
    ...
    constructor (uint256 _snipeBlockAmt) payable {
        ...
        _isDividendExcluded[_devWallet] = true;
    ...
    function setBuyTaxes(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee, uint256 devFee) external onlyOwner {
    ...
    function setSellTaxes(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee, uint256 devFee) external onlyOwner {
        require(liquidityFee <= maxLiquidityFee
                && reflectionFee <= maxReflectionFee
                && marketingFee <= maxMarketingFee
                && devFee <= maxDevFee);
    ...
    function setRatios(uint256 reflection, uint256 liquidity, uint256 marketing, uint256 dev) external onlyOwner {
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        uint256 swapTotalFee = reflectionRatio + liquidityRatio + marketingRatio + devRatio;
    ...
    function transferBNBOut(uint256 amount) internal {
        uint256 amountBNBMarketing = (amount * marketingRatio) / (marketingRatio + devRatio);
    ...
    function burnRaf(uint amount) external onlyOwner returns(bool){
