...
contract ContractA {
    ...
    function cal(uint256 _value, OPE _operator) public returns (uint256 _storedValue) {
        uint8 _ope = uint8(_operator);
        console.logUint(_ope);

        bytes memory data = abi.encodeWithSelector(bytes4(keccak256("cal(uint256,uint8)")), _value, _ope);

        // (bool success, bytes memory resultData) = delegateContract.delegatecall(abi.encodeWithSignature("cal(uint256, uint8)", _value, _ope));
        (bool success, bytes memory resultData) = delegateContract.delegatecall(data);
...
contract ContractB {
    ...
    function cal(uint256 _value, uint8 _operator) public returns (uint256 returnedValue) {
