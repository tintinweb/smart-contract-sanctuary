    ...
    UUPSUpgradeable
{
  ...
  */
    ...
    event FightFinished(
        address player,
        uint256 figtherNftID,
        bool result,
        uint256 amount
    );
    address public aPapyrusAddress;
    //APE ID => BUILDING ID
    mapping(uint256 => uint256) public buildingApeMap;
    // BUILDING ID => # APES
    mapping(uint256 => uint256) public apesBuildingCount;
    ...
    function initialize() public initializer {
        ...
        landEnabled_ = true;
        GnosisSafeVault = payable(
            address(0xfc50B6714482D5cc03cf2d720Dc0b8AE7FC53b2e)
        ); //
    ...
    function setBuildingsBaseRewards(
        uint256 houseReward,
        uint256 farmReward,
        uint256 barrackReward
    ) public onlyOwner {
    ...
    function stackBuilding(uint256 buildingNftID) public {
        ...
        require(
            ...
            "not the owner of this building"
        );
        require(
            buildsAtAddress[buildingNftID] == address(0x000),
            "Building already Stacked"
        );
    ...
    //TODO2: Nuevos fixes
    function stackApe(uint256 apeNftId, uint256 buildingId) public {
        address sender = address(msg.sender);
        IApes apesInstance = IApes(apeContract);
        require(apesInstance.ownerOf(apeNftId) == sender,
                "You're not the owner of this ape");
        require(apesAtAddress[apeNftId] == address(0x000),
                "Ape already Stacked");
        IApeBuilding aBuildInstance = IApeBuilding(buildingsContract);
        require(buildsAtAddress[buildingId] == sender, 
                "Building isn't staked");
        require(aBuildInstance.ownerOf(buildingId) == sender,
                "Building owner error");
        require(apesBuildingCount[buildingId] < 4,
                "Building is full");
        require(apesInstance.typeOfApeById(apeNftId) == 
                aBuildInstance.getTypeIDByNftID(buildingId),
                "Ape on building error");

        apesAtAddress[apeNftId] = sender;
        apesBuildingCount[buildingId] ++;
        buildingApeMap[apeNftId] = buildingId;
        apeStartFarmingTimestamp[apeNftId] = block.timestamp;
        apesInstance.transferFrom(sender, address(this), apeNftId);
    }
    function unstackApe(uint256 apeNftId, uint256 buildingNftId) public {
        address sender = address(msg.sender);
        IApes apesInstance = IApes(apeContract);
        require(apesAtAddress[apeNftId] == sender,
                "APE not staked or owner failed");
        require(buildingApeMap[apeNftId] == buildingNftId,
                "APE on Building error");
        apesAtAddress[apeNftId] = address(0x000);
        apesBuildingCount[buildingNftId] --;
        buildingApeMap[apeNftId] = 0;
        apesInstance.setDeltaStack(apeNftId, block.timestamp -
        apeStartFarmingTimestamp[apeNftId]);
        apeStartFarmingTimestamp[apeNftId] = 0;
        apesInstance.transferFrom(address(this),sender, apeNftId);
    }
    function unStackBuilding(uint256 genericType) public {
        address sender = address(msg.sender);
        require(
            addressToApesType[sender][genericType].length == 0,
            "make claim instead"
        );
        IApeBuilding apeBuilding = IApeBuilding(buildingsContract);
        require(
            addressToBuildingsType[sender][genericType].length > 0,
            "not stacked of this type"
        );
        uint256 nftId = addressToBuildingsType[sender][genericType][
            addressToBuildingsType[sender][genericType].length - 1
        ];
        ...
        uint256 maxSlots = addressToBuildingsType[sender][genericType].length *
            3;
    ...
    function unStackApes(uint256 genericType) public 
    {
        ...
        require(
            addressToApesType[sender][genericType].length > 0,
            "not apes of this type"
        );
        uint256 nftId = addressToApesType[sender][genericType][
            addressToApesType[sender][genericType].length
        ];
  ...
  */
    function claimRewards(uint256 genericType) public 
    {
        ...
        require(
            addressToBuildingsType[sender][genericType].length > 0,
            "no buildings of this type"
        );
        require(
            addressToApesType[sender][genericType].length > 0,
            "no apes of this type"
        );
        ...
        if (((totalApesByType - 1) % 3) == 0) {
            totalRewards +=
                (currentTimestamp -
                    apeStartFarmingTimestamp[
                        addressToApesType[sender][genericType][
                            totalApesByType - 1
                        ]
                    ]) *
            ...
            apeBuilding.transferFrom(
                address(this),
                sender,
                addressToBuildingsType[sender][genericType][
                    addressToBuildingsType[sender][genericType].length - 1
                ]
            );
            buildsAtAddress[
        ...
        apesInstance.setDeltaStack(
            nftID,
            (block.timestamp - apeStartFarmingTimestamp[nftID - 1])
        );
        // adjust rewards
    ...
    function claimRewardsWithoutUnStack(uint256 genericType, address sender) private
    {
        ...
        if (((totalApesByType - 1) % 3) == 0) {
            uint256 buildingDelta = currentTimestamp -
                apeStartFarmingTimestamp[
                    addressToApesType[sender][genericType][totalApesByType - 1]
                ];
            totalRewards +=
    ...
    function getStackedApesIdsByType(address owner, uint256 genericType)
        public
        view
        ...
        returns (uint256)
    {
        ...
        if (addressToBuildingsType[owner][genericType].length <= 0) {
            ...
            uint256 apeDelta = currentTimestamp -
                apeStartFarmingTimestamp[apeId];
            totalRewards += apeDelta * apeTypeBaseRewards[genericType];
        }
        // adjust rewards
        totalRewards = totalRewards / 86400;
    ...
    function fightAgainst(uint256 apeNftID, uint256 enemy)
        public
        returns (bool)
    {
        address sender = address(msg.sender);
        ...
        require(
            aGoldInstance.balanceOf(sender) >= enemies_[enemy].bait,
            "Insufficient aGold to fight"
        );
        require(
            aGoldInstance.allowance(sender, address(this)) >=
                enemies_[enemy].reward,
    ...
    function fight(uint256 percent) private view returns (bool) {
    ...
    function evolve(uint256 nftID) public {
        IAPapyrus aPapyrus = IAPapyrus(aPapyrusAddress);
        require(
