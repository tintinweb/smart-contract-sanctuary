...
contract FarmingV2 is Earning {
    function initialize(
        ...
        bonusEndBlock = type(uint256).max;
...
abstract contract Earning is OwnableUpgradeable, IEarning {
    IERC20 public token; //lptoken
    MinterV2 public minter; //lqfV2Token
    uint256 accDeposit;
    ...
    function setRewardPerBlock(uint256 _rewardPerBlock) external override {
        require(msg.sender == address(minter), "Only minter can change reward");

        updateState();
        rewardPerBlock = _rewardPerBlock;
    }

    function getNumOfBlocks(uint256 _from, uint256 _to) internal view returns (uint256) {
    ...
    function _pendingReward(address _user, uint256 _block) internal view returns (uint256) {
        ...
        uint256 stakedSupply = token.balanceOf(address(this));

        if (_block > pool.lastRewardBlock && stakedSupply != 0) {
            uint256 multiplier = getNumOfBlocks(pool.lastRewardBlock, _block);
            uint256 tokenReward = multiplier * rewardPerBlock;

            accRewardPerShare = accRewardPerShare + (tokenReward * 1e12) / stakedSupply; //TODO experiment with 1e12
        }
    ...
    function updateState() public {
        ...
        uint256 multiplier = getNumOfBlocks(poolInfo.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier * rewardPerBlock;

        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare + (tokenReward * 1e12) / tokenSupply;
        poolInfo.lastRewardBlock = block.number;
    }
...
contract MinterV2 is IMinter, Initializable, OwnableUpgradeable {
    ...
    function setRewardPerBlock(uint256 _newValue) external onlyOwner { //TODO DELETE ME AND FIX TESTS
        rewardPerBlock = _newValue;
    }

    function getStatus(address _farm, address _user, uint256 _block) external view override returns (FarmInfo memory) {
        FarmInfo memory f;
        f.multiplier = farm[_farm].multiplier;
        f.rewardPerBlock = farm[_farm].rewardPerBlock;
    ...
    function getStatusStaking(address _user, uint256 _block) external view override returns (FarmInfo[] memory) {
        ...
        //        if (length == 0)
        //            return new FarmInfo[](0);

        FarmInfo[] memory f = new FarmInfo[](length);

        for (uint256 i = 0; i < length; i++) {
            address addr = address(staking[i]);
            f[i].multiplier = farm[addr].multiplier;
            f[i].rewardPerBlock = farm[addr].rewardPerBlock;
            f[i].token = address(IEarning(addr).getToken());
            f[i].accDeposited = IEarning(addr).getAccDeposit();
            f[i].deposited = IEarning(addr).deposited(_user);
            f[i].earned = IEarning(addr).pendingReward(_user, _block);
            f[i].farm = addr;
        }
        return f;
    }
    ...
    function getStatusFarms(address _user, uint256 _block) external view override returns (FarmInfo[] memory) {
        ...
        for (uint256 i = 0; i < length; i++) {
            address addr = address(farming[i]);
            f[i].multiplier = farm[addr].multiplier;
            f[i].rewardPerBlock = farm[addr].rewardPerBlock;
            f[i].token = address(IEarning(addr).getToken());
            f[i].accDeposited = IEarning(addr).getAccDeposit();
    ...
    function registerFarm(address _farm, uint256 _multiplier) external override {
        ...
        require(farm[_farm].multiplier == 0, "Farming already registered");
    ...
    function registerStaking(address _farm, uint256 _multiplier) external override {
        require(factories[msg.sender] == true, "Only factories can register");
        require(_multiplier > 0, "Multiplier can't be 0");
        require(farm[_farm].multiplier == 0, "Staking already registered");
    ...
    function unregisterFarm(address _farm) external override onlyOwner {
        ...
        require(multiplier != 0, "not registered");
    ...
    function changeMultiplierFarm(address _addr, uint256 _multiplier) external override onlyOwner {
        require(_multiplier > 0, "multiplier 0");
        require(farm[_addr].multiplier != 0, "Not yet added");

        accMultiplierForFarms = accMultiplierForFarms - farm[_addr].multiplier + _multiplier;
        farm[_addr].multiplier = _multiplier;
    }

    function changeMultiplierStaking(address _addr, uint256 _multiplier) external override onlyOwner {
        require(_multiplier > 0, "multiplier 0");
        require(farm[_addr].multiplier != 0, "Not yet added");

        accMultiplierForStakings = accMultiplierForStakings - farm[_addr].multiplier + _multiplier;
        farm[_addr].multiplier = _multiplier;
    }

    function mint(address _to, uint256 _amount) external override {
        require(farm[msg.sender].multiplier != 0, "Not registered");
    ...
    function setXYZ(uint256 _B, uint256 _Z) external override onlyOwner {
        ...
        for (uint256 i = 0; i < staking.length; i++) {
            staking[i].setRewardPerBlock((_X * farm[address(staking[i])].multiplier) / accMultiplierForStakings);
            farm[address(staking[i])].rewardPerBlock = (_X * farm[address(staking[i])].multiplier) / accMultiplierForStakings;
        }

        for (uint256 i = 0; i < farming.length; i++) {
            farming[i].setRewardPerBlock((_Y * farm[address(farming[i])].multiplier) / accMultiplierForFarms);
            farm[address(farming[i])].rewardPerBlock = (_Y * farm[address(farming[i])].multiplier) / accMultiplierForFarms;
        }
    }
}
