...
contract BuyPackage is Ownable , NftLegendCard{
	...
	function changePackage(uint256 _package) public onlyOwner {
		require(_package > 0, 'need package > 0');
		package = _package;
	}
    ...
    function buyCard(uint256 _amount) public {
        ...
        // Random 9C and 1R
        // C --> [1 - 48] R --> [49 - 83] SR --> [84 - 104] SSR --> [105 106 107] 
        ...
        for(uint256 i =0; i < package; i ++){
            uint256 numberCard;
            if(i == (package-1)) {
                uint256 numberMath2 = numberOfPurchases + (package-1);
                numberCard = changeNumber49To83(numberMath2);
            }else{
                uint256 numberMath1 = numberOfPurchases + i;
                numberCard = changeNumber49To83(numberMath1);
            }
    ...
    function changeNumber49To83(uint256 _randNonce) public view returns(uint256){ 
        uint256 numberRandom;     
        uint256 random = getRandomNumber(_randNonce) ;
        uint256 randomEdit = random.mod(35);
        if(randomEdit == 0) {
            //random ==35        
            numberRandom = 35;
        }else if(random == 0) {
    ...
    function changeNumber1To47(uint256 _randNonce) public view returns(uint256){ 
        uint256 numberRandom;     
        uint256 random = getRandomNumber(_randNonce) ;
