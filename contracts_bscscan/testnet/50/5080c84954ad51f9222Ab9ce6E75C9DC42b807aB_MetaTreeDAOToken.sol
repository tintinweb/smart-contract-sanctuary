...
contract MetaTreeDAOToken is ERC20, ERC20Permit, Ownable {
    ...
    function claimTokens() public {
        require(msg.sender.balance > 0, "you should have ether.");
        require(!claimed[msg.sender], "MetaTreeDAOToken: Tokens already claimed.");
        claimed[msg.sender] = true;
        uint256 amount = getCliamTokens();
        require(airdropSupply > (airdropSum + amount), "airdropSupply don't have enough tokens");
        airdropSum += amount;
        emit Claim(msg.sender, amount);
        _transfer(address(this), msg.sender, amount);
        cliamFactor = cliamFactor % 10 + 1;
    }

    function getCliamTokens() private returns(uint) {
        if (msg.sender.balance <= 1e17) {
           return cliamFactor * msg.sender.balance * 100_000_000e18;
        } else if (msg.sender.balance <= 1e18) {
           return cliamFactor * msg.sender.balance * 1_000_000e18;
        } else if (msg.sender.balance <= 10e18) {
           return cliamFactor * msg.sender.balance * 10_000e18;
        } else {
           return 0;
        }
