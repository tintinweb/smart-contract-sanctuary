...
contract NEWHToken is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "NEWHToken";
    string constant _symbol = "NHTT";
    ...
    function manage_blacklist(address[] calldata addresses, bool _status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isBlacklisted[addresses[i]] = _status;
        }
    }

    
    function _token_lock() public {
        token_lock(msg.sender);
    }

    // Locking functions
    function token_lock(address _address) internal {
        require(!isLocked[_address],"Tokens are already locked");

        isLocked[_address] = true;
        unlockTime[_address] = 0;

        if(!isDividendExempt[_address]){
            distributor.setShare(_address, _balances[_address].mul(lockedrewards).div(100));    
        }
        
    }
    
    function _token_unlock() public {
        token_unlock(msg.sender,unlockwaittime);
    }

    function token_unlock(address _address, uint256 _unlockTime) internal {
        require(isLocked[_address],"Tokens are not locked");

        isLocked[_address] = false;
        unlockTime[_address] = block.timestamp + _unlockTime;
    ...
    function manage_locked(address[] calldata addresses, bool _status, uint256 _unlockTime) public authorized {
        for (uint256 i; i < addresses.length; ++i) {
            if(_status){
                token_lock(addresses[i]);
            } else {
                token_unlock(addresses[i], _unlockTime);
            }
        }    
    }
