...
contract MikkoToken_test is Context, IERC20, Ownable {
    ...
    uint256 private _previousautoBurn = _autoBurn;
    ...
    function setautoBurnPercent(uint256 autoBurn) external onlyOwner() {
        _autoBurn = autoBurn;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tCharity = calculateautoBurn(tAmount);
    ...
    function _takeCharity(uint256 tCharity) private {
        uint256 currentRate =  _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[_autoBurnWalletAddress] = _rOwned[_autoBurnWalletAddress].add(rCharity);
        if(_isExcluded[_autoBurnWalletAddress])
            _tOwned[_autoBurnWalletAddress] = _tOwned[_autoBurnWalletAddress].add(tCharity);
    }
    ...
    function calculateautoBurn(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_autoBurn).div(
            10**2
        );
    }
