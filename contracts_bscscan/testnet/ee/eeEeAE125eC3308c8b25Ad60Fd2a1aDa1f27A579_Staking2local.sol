...
contract Staking2local is Ownable {
    ...
    mapping (uint256 => uint256) bonusRateForTime;
            ...
            if (currentTime > user.userRewardInfo[user.lastPaidIndex].canHarvestTimestamp) {
                for (uint256 i = user.lastPaidIndex; i < user.userRewardInfo.length; i++ ) {
                    ...
                    if (currentTime > rewardInfo.canHarvestTimestamp) {
                        pending = rewardInfo.amount.add(rewardInfo.amount.mul(rewardInfo.bonusRate).div(100));
    ...
    function deposit(uint256 _pid, uint256 _amount, uint256 _days) public {
        ...
        if(_amount > 0) {
            ...
            user.userRewardInfo.push(UserRewardInfo({
                amount: realAmount,
                canHarvestTimestamp: block.timestamp.add(_days * 1 seconds),
