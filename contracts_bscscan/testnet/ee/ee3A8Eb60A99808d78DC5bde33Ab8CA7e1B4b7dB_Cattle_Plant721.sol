...
contract Cattle_Plant721 is Ownable, ERC721Enumerable, ERC721URIStorage {
    // for inherit
    ...
    mapping(uint => uint) public plantIdMap;
    ...
    function newCard(string calldata name_, uint type_, uint maxAmount_, string calldata tokenURI_, bool tradable_) public onlyOwner {
        require(type_ != 0 && plantInfo[type_].plantType == 0, "wrong plantType");

        plantInfo[type_] = PlantInfo({
        plantType : type_,
        name : name_,
        currentAmount : 0,
        burnedAmount : 0,
    ...
    function editCard(string calldata name_, uint type_, uint maxAmount_, string calldata tokenURI_, bool tradable_) public onlyOwner {
        require(type_ != 0 && plantInfo[type_].plantType == type_, "wrong plantType");

        plantInfo[type_] = PlantInfo({
        plantType : type_,
        name : name_,
        currentAmount : plantInfo[type_].currentAmount,
        burnedAmount : plantInfo[type_].burnedAmount,
        maxAmount : maxAmount_,
        tradable : tradable_,
        tokenURI : tokenURI_
        });
    ...
    function mint(address player_, uint type_, bool uriInTokenId_) public returns (uint256) {
        require(type_ != 0 && plantInfo[type_].plantType != 0, " wrong plantType");
        ...
        require(plantInfo[type_].currentAmount < plantInfo[type_].maxAmount, "cattle: amount out of limit");
        plantInfo[type_].currentAmount += 1;

        uint tokenId = currentID;
        currentID ++;
        plantIdMap[tokenId] = type_;
    ...
    function mintWithId(address player_, uint id_, uint tokenId_, bool uriInTokenId_) public returns (bool) {
        require(id_ != 0 && plantInfo[id_].plantType != 0, "wrong plantType");
        ...
        require(plantInfo[id_].currentAmount < plantInfo[id_].maxAmount, "cattle: amount out of limit");
        plantInfo[id_].currentAmount += 1;

        plantIdMap[tokenId_] = id_;
    ...
    function burn(uint tokenId_) public returns (bool){
        ...
        uint plantType = plantIdMap[tokenId_];
        plantInfo[plantType].burnedAmount += 1;
        burned += 1;
    ...
    function transferFrom(
        ...
        require(plantInfo[tokenId].tradable, 'can not transfer This Plant');
    ...
    function tokenURI(uint256 tokenId_) override(ERC721URIStorage, ERC721) public view returns (string memory) {
        ...
        string memory tURI = super.tokenURI(tokenId_);
        string memory cURI = plantInfo[plantIdMap[tokenId_]].tokenURI;
    ...
    function checkUserPlant(address player) public view returns (uint[] memory){
