...
contract DarkMatter is ERC20("DarkMatter", "KDM"), Ownable {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "KDM::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "KDM::delegateBySig: invalid nonce");
        require(now <= expiry, "KDM::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "KDM::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "KDM::_writeCheckpoint: block number exceeds 32 bits");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 KDMReward = multiplier.mul(KDMPerblock).mul(pool.allocPoint).div(totalAllocPoint);
        KDM.mint(devaddr, KDMReward.div(10));
    ...
    function safeEggTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > KDMbal) {
            KDM.transfer(_to, KDMbal);
        } else {
            KDM.transfer(_to, _amount);
        }
    }
