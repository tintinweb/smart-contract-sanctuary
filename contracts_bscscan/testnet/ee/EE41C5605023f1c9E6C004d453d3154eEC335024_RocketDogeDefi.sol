...
import "./RocketDogeOwnable.sol";
import "./RocketDogeToken.sol";
import "./RocketDogeProofOfStake.sol";

interface IMigratorChef {
    function migrate(IBEP20 token) external returns (IBEP20);
}

contract RocketDogeDefi is RocketDogeOwnable, Initializable {
    ...
    RocketDogeToken public rocketdoge;
    RocketDogeProofOfStake public postoken;
    ...
    function initialize(
        RocketDogeToken _rocketdoge,
        RocketDogeProofOfStake _rocketdogepos,
        address _owner,
        address _devaddr
    ) public initializer {
        initOwner(_owner);
        ...
        rocketdoge = _rocketdoge;
        postoken = _rocketdogepos;
        emissionRate = 100e18;
    ...
    function setPosAdr(RocketDogeProofOfStake posAdr) public onlyOwner {
        postoken = posAdr;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 reward = emissionRate.mul(block.number.sub(pool.lastRewardBlock))
            .mul(pool.allocPoint).div(totalAllocPoint);
        // rocketdoge.mintTo(address(postoken), reward.mul(116).div(100));
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public {
        require (_pid != 0, 'deposit ROCKETDOGE by staking');
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        require (_pid != 0, 'withdraw ROCKETDOGE by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Stake ROCKETDOGE tokens to MasterChef
    ...
    // Withdraw ROCKETDOGE tokens from STAKING.
    ...
    function setSellContract(address _sellContract) public onlyOwner {
      rocketdoge.setSellContract(_sellContract);
    }
}
...
contract RocketDogeProofOfStake is BEP20 {
    /// @dev owner => next valid nonce to submit with permit()
    mapping(address => uint) public _nonces;
    bytes32 public constant PERMIT_TYPEHASH = keccak256(
        'Permit(address owner,address spender,uint256 value,uint256 nonce,uint256 deadline)'
    ...
    constructor(RocketDogeToken _rocketdoge) public BEP20('RocketDoge Proof Of Stake', 'ROCKETDOGEPOS', 1e15 * 1e18) {
        rocketdoge = _rocketdoge;
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                DOMAIN_TYPEHASH,
    ...
    function sendReward(address _to, uint256 _amount) public onlyOwner {
        ...
        safeTransfer(_to, _amount - lockedAmount);
    }
    ...
    function release(address _account, uint256 _amount) public {
        ...
        uint256 balance = rocketdoge.balanceOf(address(this));
        rocketdoge.approve(address(controller), balance);
    ...
    function _safeTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBal = rocketdoge.balanceOf(address(this));
        if (_amount > tokenBal) {
            rocketdoge.transfer(_to, tokenBal);
        } else {
            rocketdoge.transfer(_to, _amount);
        }
    }
