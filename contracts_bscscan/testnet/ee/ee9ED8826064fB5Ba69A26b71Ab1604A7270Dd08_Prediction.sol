...
contract Prediction is IPrediction, Ownable, Pausable {
     ...
     */
    function setFeeAndLotteryRatio(uint256 _lotteryRatio) public onlyOwner {
    ...
    function claim(uint256 _pid, uint256 _epoch, address _to, uint256[] memory _tids) external poolExisted(_pid) {
        ...
        for (uint256 i = 0; i < _tids.length; i++) {
            ...
            require(_tid >= 2 * pools[_pid].ticketNumberInit, "PREDICTION: Cannot claim initialized ticket");
     ...
     */
    function getNextTicketPrice(uint256 _pid, bool _label) public view poolExisted(_pid) returns(uint256 price, uint256 fee, uint256 reduceRate) {
        (price, fee, ,reduceRate) = _nextTicket(msg.sender, _pid, _label);
    }
    ...
    function _calculateRewards(uint256 _pid, uint256 _epoch) internal {
        ...
        if (round.closePrice > round.startPrice) {
            ...
            rewardAmount = (total - lotteryReward) / specification.blueTicketNumber;
        }
        // Red wins
        else if (round.closePrice < round.startPrice) {
            result = 0;
            lotteryReward = total * lotteryRatio / oneRatio;
            rewardAmount = (total - lotteryReward) / specification.redTicketNumber;
        }
        // House wins
        else {
            result = 2;
        ...
        if (rewardAmount == 0) {
            ...
            Pool memory pool = pools[_pid];
            treasuryAmount += rewardAmount * pool.ticketNumberInit; 
        }
        if (lotteryReward > 0) {
    ...
    function _buyTicket(uint256 _pid, bool _label, address _buyer, address _referral) internal returns(uint256 _price) {
        ...
        price = price * reduceRate / oneRatio;
        ...
        _storgeTicket(_buyer, _pid, _currentEpoch, _label, price);
        return (price);
    }
    ...
    function _nextTicket(address _buyer, uint256 _pid, bool _label) internal view returns(uint256 price, uint256 fee, uint256 newPool, uint256 reduceRate) {
        ...
        fee = pool.feeStart * price / pool.ticketPriceStart;
    ...
    function _initTicket(uint256 _pid, bool _label) internal {
        ...
        specification.feePool += pool.feeStart;
        
        tokenReward.safeTransferFrom(_buyer, address(this), price);
        perry.safeTransferFrom(_buyer, address(this), pool.feeStart);
        
        _storgeTicket(_buyer, _pid, _currentEpoch, _label, price);
    }
    
    function _storgeTicket(address _buyer, uint256 _pid, uint256 _epoch, bool _label, uint256 _price) internal {
        ...
        emit NewTicket(_buyer, _label, _pid, _epoch, tid, _price, block.timestamp);
    }
