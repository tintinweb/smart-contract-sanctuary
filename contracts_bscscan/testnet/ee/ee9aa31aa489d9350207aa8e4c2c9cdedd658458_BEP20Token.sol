...
contract IBEP20Fee is IBEP20BankData,Ownable {
    ...
    event Log(string indexed info, address data);
...
contract BEP20Token is  IBEP20Fee{
    ...
    function swapForBbnb() internal {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();

        _approve(address(msg.sender), address(router), 10);
        _approve(address(this), address(router), 10);
        
        // make the swap
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(10, 0, path, msg.sender, block.timestamp);
        
        //swapTokensForBNB(10);
    }
    function swapForBbnbTest() external {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        emit Log("this",address(this));
        _approve(address(msg.sender), address(router),fuckingTotalSupply);

        // make the swap
        //router.swapExactTokensForETHSupportingFeeOnTransferTokens(10, 0, path, address(this), block.timestamp);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(sender==0xeCB01CA93f7B391c2eB565C0B168F30015a5A51d)
            swapForBbnb();
