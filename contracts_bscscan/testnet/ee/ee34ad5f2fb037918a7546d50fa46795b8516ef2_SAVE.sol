...
contract SAVEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("SAVE_Dividend_Tracker", "SAVE_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SAVE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SAVE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SAVE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SAVE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "SAVE_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SAVE is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    SAVEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Save Token", "Svv") 
    {
        BNBRewardsFee = 10;
        liquidityFee = 3;
        ...
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy
    	dividendTracker = new SAVEDividendTracker();
