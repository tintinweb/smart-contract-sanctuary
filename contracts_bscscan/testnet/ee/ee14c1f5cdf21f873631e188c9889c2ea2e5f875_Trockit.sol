...
/*
 .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------. 
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| |  _________   | || |  _______     | || |     ____     | || |     ______   | || |  ___  ____   | || |     _____    | || |  _________   | |
| | |  _   _  |  | || | |_   __ \    | || |   .'    `.   | || |   .' ___  |  | || | |_  ||_  _|  | || |    |_   _|   | || | |  _   _  |  | |
| | |_/ | | \_|  | || |   | |__) |   | || |  /  .--.  \  | || |  / .'   \_|  | || |   | |_/ /    | || |      | |     | || | |_/ | | \_|  | |
| |     | |      | || |   |  __ /    | || |  | |    | |  | || |  | |         | || |   |  __'.    | || |      | |     | || |     | |      | |
| |    _| |_     | || |  _| |  \ \_  | || |  \  `--'  /  | || |  \ `.___.'\  | || |  _| |  \ \_  | || |     _| |_    | || |    _| |_     | |
| |   |_____|    | || | |____| |___| | || |   `.____.'   | || |   `._____.'  | || | |____||____| | || |    |_____|   | || |   |_____|    | |
| |              | || |              | || |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------' 
Website: www.trockit.com, BSC scan address: 
...
contract Pausable is Ownable {
    ...
    */
    modifier whenNotPaused() {
        require(!paused, "Trade is paused.");
        _;
    }
    ...
    */
    modifier whenPaused() {
        require(paused, "Trade is not paused.");
        _;
    }
...
contract Trockit is Context, IERC20, Ownable, Pausable 
{
    ...
    string private _name = "Trockit";
    string private _symbol = "TRK";
    ...
    address payable public marketingWallet =  payable(0x5ecF3f9e5131957c196c4772EDA6b0fBa9a38989);
    ...
    uint256 public maxLimitPercentage = 6;
    ...
    constructor () 
    {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //main net 
    ...
    {
        uint256 balZeroWallet = balanceOf(0x0000000000000000000000000000000000000000);
        uint256 balDeadWallet = balanceOf(0x000000000000000000000000000000000000dEaD);
        uint256 circulatingSupply  =  totalSupply().sub(balZeroWallet).sub(balDeadWallet);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap 
    {
        ...
        marketingWallet.transfer(newBalance.sub(ethForLiquidity));
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private 
    {
        ...
        uint256 maxLimit = (totalCirculatingSupply()).div(100).mul(maxLimitPercentage);
    ...
    function  setMaxWalletPercentage(uint256 amount) public onlyOwner 
    {
            maxLimitPercentage = amount;
            emit Log("maxLimitPercentage changed", amount);
    }
