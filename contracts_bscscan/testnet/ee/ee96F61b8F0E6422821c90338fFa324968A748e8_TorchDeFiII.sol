...
contract TorchDeFiII is LpVault{
    ...
    function changeDynamicToClaim_g(address user, uint amount) external onlyOwner{
        userInfo[user].dynamicToClaim_g = amount;
    }

    function changeDynamicToClaim_d(address user, uint amount) external onlyOwner{
        userInfo[user].dynamicToClaim_d = amount;
    }

    
    function changeDynamicClaimed_g(address user, uint amount) external onlyOwner{
        userInfo[user].dynamicClaimed_g = amount;
    }

    function changeDynamicClaimed_d(address user, uint amount) external onlyOwner{
        userInfo[user].dynamicClaimed_d = amount;
    }
    ...
    function changeReferLevels(address user, uint lev, uint n) external{
        userReferLevels[user][lev-1] = n;
    }
    
    function setSpecialLev(address user, uint lev) external{
    ...
    function silverBullet(
        ...
        changeUserMulti( user, inv,dt,newLp,refer_n,amount,stakeTotal);
    ...
    function changeUserStakeInfo(address user,uint[3][5] calldata data) internal{
                                    for(uint i=1;i<=5;++i){
        ...
        if(l > userAccount[_msgSender()].level){
            ...
            for(uint j =0; j<20; j++){
                if(temp_user2==address(0) || temp_user2 == address(this) || temp_user2 == oldContract){break;}
                    ...
                    uint l2 = getLevel(temp_user2);
                    if(userAccount[temp_user2].level < l2){
                      userAccount[temp_user2].level = l2;  
                    } 
    ...
    function updateGrade() external {
        uint l_now = userAccount[_msgSender()].level;
        uint l_new = getLevel(_msgSender());
        
        address temp_user =  userAccount[_msgSender()].invitor;
        
        if(l_now < l_new){
            userAccount[_msgSender()].level = l_new;
            for(uint i=0; i<20; i++){
                if(temp_user == address(0) || temp_user == address(this) || temp_user == oldContract){break;}
                else{
                    userReferLevels[temp_user][l_new-1] += 1;
                }
                temp_user = userAccount[temp_user].invitor;
            }
        }         
