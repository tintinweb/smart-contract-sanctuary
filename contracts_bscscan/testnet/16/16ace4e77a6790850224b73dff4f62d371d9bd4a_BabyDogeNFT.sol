    ...
    ReentrancyGuard
{
    ...
    uint256 internal constant ITERATION_PERIOD = 120;//4 weeks;
    ...
    function withdrawAndLock() public onlyOwner {
        require(withdrawIsLocked == false,'withdrawIsLocked is true');
    ...
    function mintDoge(uint256 numberOfTokens) public payable {
        require(saleIsActive,'saleIsActive is false');
        require(numberOfTokens <= maxDogePurchase,'numberOfTokens must be less than maxDogePurchase');
        require(totalSupply().add(numberOfTokens) <= MAX_DOGES,'numberOfTokens must be less than MAX_DOGES');
        require(dogePrice.mul(numberOfTokens) <= msg.value,'Invalid price');
    ...
    function claimPool(uint256[] calldata nftIDs) external nonReentrant {
        require(iterationToClaim[currentIteration.current()] > 0,'iterationToClaim is 0');
        uint256 claimable;
        uint256 balance;
        if (ethPayout) {
        ...
        for (uint256 i = 0; i < nftIDs.length; i++) {
            require(ownerOf(nftIDs[i]) == msg.sender,'Invalid caller - 1 ');
            require(
                iterationClaimed[currentIteration.current()][nftIDs[i]] == false,'iterationToClaim is claimed for this'
            );
        ...
        require(claimable > 0,'not claimable');
        if (ethPayout) {
    ...
    function claimLotto(uint256 _id) external nonReentrant {
        require(ownerOf(_id) == msg.sender,'Invalid caller - 2 ');
        uint256 prize;
