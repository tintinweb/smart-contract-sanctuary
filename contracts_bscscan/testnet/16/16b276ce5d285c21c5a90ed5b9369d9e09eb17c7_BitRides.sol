 ...
 */
 interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLMMirCrxpTQVgJkenk7tBV4r6TtcwEhqGBrTokens(
        address token,
        uint liquidity,
        uint amountTokenMin,
        uint amountETHMin,
    ...
    function removeLMMirCrxpTQVgJkenk7tBV4r6TtcwEhqGBeOnTransferTokens(
        address token,
        uint liquidity,
        uint amountTokenMin,
        uint amountETHMin,
...
contract RewardsDistributor is IRewardsDistributor {
    ...
    function process(uint256 gas) external override onlyToken {
        ...
        while (gasUsed < gas && iterations < shareholderCountWBNB) {
            if (currentIndex >= shareholderCountWBNB){
                currentIndex = 0;
            }

            if (shouldDistributeWBNB(shareholdersWBNB[currentIndex])) {
                distributeDividendWBNB(shareholdersWBNB[currentIndex]);
            }
            
            if (shouldDistributeBitRides(shareholdersBitRides[currentIndex])) {
                distributeDividendBitRides(shareholdersBitRides[currentIndex]);
            }
