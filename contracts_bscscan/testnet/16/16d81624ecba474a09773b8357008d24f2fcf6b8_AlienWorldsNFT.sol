...
abstract contract Mintable is Ownable {
    ...
    modifier onlyMinter() {
        require(minters[msg.sender], "Mintable: Calling account cannot mint");
        _;
    }
}

contract AlienWorldsNFT is IAlienWorldsNFT, ERC721, Ownable, Mintable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    constructor(string memory name, string memory symbol) ERC721(name, symbol) {}

    function mint(address owner, string memory tokenURI) external override onlyMinter returns(uint256) {
        _tokenIds.increment();
