...
contract Woolf is IWoolf, ERC721Enumerable, Ownable, Pausable {
    ...
    mapping(address=>uint8) public whiteNumMap;
    //1 white 2 public 3 erc20 mint
    uint8 public override STATUS=1; 
    uint256 public TOTAL_PAY_TOKENS;
 ...
 function nextStatus() external{
   require(msg.sender==cSigner,"no owner");
   require(STATUS<3," status over ");
    if(STATUS==1){
        STATUS=2;
      }
    if(STATUS==2){
        STATUS=3;
        TOTAL_PAY_TOKENS=minted;
      }

 }
    /** EXTERNAL */
   function mint(uint256 amountV, bytes32 r, bytes32 s , bool stake)   external payable whenNotPaused{
       ...
       uint8 v = uint8(amountV >> 248);
        ...
        if(STATUS == 1){
          require((whiteNumMap[msg.sender]+amount)<3,"exceed the three limit");
        //   require((minted+amount) <WHITE_MAX_TOKENS,"exceed white max");
          whiteNumMap[msg.sender]++;
        }
        // if(STATUS == 2){
        //    require((minted+amount) <PAID_TOKENS,"exceed public max");
        // }
        innerMint(amount, stake);
     ...
     */
    function innerMint(uint256 amount, bool stake) private {
        require(tx.origin == _msgSender(), "Only EOA");
        require(minted + amount <= MAX_TOKENS, "All tokens minted");
        require(amount > 0 && amount <= 10, "Invalid mint amount");
        require(amount * payMintCost() == msg.value, "Invalid payment amount");
    ...
    function payMintCost() public view returns (uint256){
        if(STATUS==1){
          return .5 ether;
        }
        if(STATUS==2){
    ...
    function mintCost(uint256 tokenId) public view returns (uint256) {
      if(STATUS<3){
        ...
        if (tokenId <= TOTAL_PAY_TOKENS) return 0;
        // if (tokenId <= TOTAL_PAY_TOKENS+10000) return 20000 ether;
        // if (tokenId <=TOTAL_PAY_TOKENS+30000) return 40000 ether;
        // return 80000 ether;
        if (tokenId <= TOTAL_PAY_TOKENS+10000) return 2 ether;
        if (tokenId <=TOTAL_PAY_TOKENS+30000) return 4 ether;
        return 8 ether;
    }
