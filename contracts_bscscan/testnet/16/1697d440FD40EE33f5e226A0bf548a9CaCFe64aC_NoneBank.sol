...
contract NoneBank {
    ...
    uint256 public constant REWARD = 1; // 0.01%
    uint256 public constant FEE = 1; // 0.01%
        ...
        returns (bool)
    {
        for (uint256 i = 0; i < _interestRates.length; i++) {
            if (
                _interestRates[i] < 0.01 ether || _interestRates[i] > 99 ether
            ) {
                return false;
            }
        }
    ...
    function repay(
        ...
        app.loanAmount = app.loanAmount.add(principal);

        uint256 reward = amount.mul(REWARD).div(10000);
    ...
    function liquidate(uint256 bid) public {
        ...
        uint256 fee = toOwner.mul(FEE).div(10000);
        uint256 prize = liquidatorPrize.add(devFees).add(fee);
        ...
        if (liquidatorPrize > 0) {
            uint256 reward = liquidatorPrize.mul(REWARD).div(10000);
            gildToken.mint(app.owner, reward);
            gildToken.mint(b.owner, reward);
            gildToken.mint(msg.sender, reward);
            gildToken.mint(dev, reward);
            IERC20(app.loanToken).transfer(
                address(msg.sender),
                liquidatorPrize.sub(fee)
            );
            IERC20(app.loanToken).transferFrom(address(this), profitShare, fee);
        }
...
contract GildToken is ERC20, Ownable {
    ...
    constructor() ERC20("Gild stock token", "GILD") {
        _mint(msg.sender, 1 ether); // mint 1 token for making GILD-BUSD LP Token
    }
