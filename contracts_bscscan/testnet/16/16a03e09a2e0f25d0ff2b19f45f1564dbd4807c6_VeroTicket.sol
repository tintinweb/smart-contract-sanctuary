...
contract ERC20 is Context, IERC20, Ownable {
    ...
    function modifyWhiteListSender(address[] memory newWhiteList, address[] memory removedWhiteList) public onlyOwner {
    ...
    function modifyWhiteListReceiver(address[] memory newWhiteList, address[] memory removedWhiteList) public onlyOwner {
        for(uint256 index; index < newWhiteList.length; index++) {
    ...
    function transferToken(address coinAddress, uint256 value, address payable to) public onlyOwner {
...
contract VeroTicket is ERC20 {
    using SafeMath for uint256;

    uint256 public maxSupply = 1000 * 10**6;

    constructor()
    {
        _initialize("VERO TICKET", "VeroTicket", 0, maxSupply);
    }
    ...
    function _transfer(
        ...
        if (
            (!whiteListSender[sender] || !whiteListReceiver[recipient]) && antiBotEnabled
        ) {
            revert("Anti Bot");
        }       
