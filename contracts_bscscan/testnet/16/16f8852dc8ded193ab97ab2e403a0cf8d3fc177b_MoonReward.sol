...
interface IDEXRouter {
    ...
    function WMOVR() external pure returns (address);
    ...
    ) external payable returns (uint amountToken, uint amountMOVR, uint liquidity);
    ...
    function swapExactMOVRForTokensSupportingFeeOnTransferTokens(
    ...
    function swapExactTokensForMOVRSupportingFeeOnTransferTokens(
...
contract DividendDistributor is IDividendDistributor {
 ...
 IERC20 RWRD = IERC20(0xB3FB48bF090bEDFF4f6F93FFb40221742E107db7); 

  address WMOVR = 0x98878B06940aE243284CA214f92Bb71a2b032B8A;
    ...
    function deposit() external payable override {
        ...
        router.swapExactMOVRForTokensSupportingFeeOnTransferTokens{value: msg.value}(
    ...
    event Deposit(uint256 amountMOVR, uint256 amountRWRD);
...
contract MoonReward is IERC20, Auth {
    ...
    using SafeMath for uint256;
     ...
     address RWRD = 0xB3FB48bF090bEDFF4f6F93FFb40221742E107db7;  
    ...
    string constant _name = "MoonReward";

    string constant _symbol = "mREWARD";
    ...
    uint256 swapThreshold = _totalSupply / 2000;  
    ...
    constructor () Auth(msg.sender) {
        ...
        pair = IDEXFactory(router.factory()).createPair(WMOVR, address(this));
    ...
    function swapBack() internal swapping {
        ...
        if(liquidityFeeAccumulator >= swapThreshold && autoLiquifyEnabled){
            ...
            router.swapExactTokensForMOVRSupportingFeeOnTransferTokens(
            ...
            uint256 amountMOVR = address(this).balance.sub(balanceBefore);



            router.addLiquidityMOVR{value: amountMOVR}(
            ...
            emit AutoLiquify(amountMOVR, amountToLiquify);
            ...
            uint256 amountMOVRReflection = amountMOVR.mul(reflectionFee).div(totalFee);

            uint256 amountMOVRMKT = amountMOVR.mul(MKTFee).div(totalFee);

            uint256 amountMOVRDev = amountMOVR.mul(devFee).div(totalFee);
            ...
            (bool success, ) = payable(MKTFeeReceiver).call{value: amountMOVRMKT, gas: 30000}("");

            if(success){ MKTFees = MKTFees.add(amountMOVRMKT); }



            (success, ) = payable(devFeeReceiver).call{value: amountMOVRDev, gas: 30000}("");

            if(success){ devFees = devFees.add(amountMOVRDev); }



            emit SwapBack(amountToSwap, amountMOVR);
    ...
    function buybackMOVR(uint256 amount) external authorized {
    ...
    function buyTokens(uint256 amount, address to) internal swapping {
        ...
        router.swapExactMOVRForTokensSupportingFeeOnTransferTokens{value: amount}(
    ...
    function setFees(
        ...
        totalFee = buybackFee.add(reflectionFee).add(MKTFee).add(devFee);
        ...
        emit FeesUpdated(_enabled, _liquidityFee, _buybackFee, _reflectionFee, _MKTFee, _devFee, _feeDenominator);

    }



    function setFeeReceivers(address _autoLiquidityReceiver, address _MKTFeeReceiver, address _devFeeReceiver) external authorized {
        ...
        emit FeeReceiversUpdated(_autoLiquidityReceiver, _MKTFeeReceiver, _devFeeReceiver);
    ...
    event AutoLiquify(uint256 amountMOVR, uint256 amountToken);

    event SwapBack(uint256 amountToken, uint256 amountMOVR);

    event Buyback(uint256 amountMOVR);
    ...
    event FeesUpdated(bool enabled, uint256 liquidityFee, uint256 buybackFee, uint256 reflectionFee, uint256 MKTFee, uint256 devFee, uint256 feeDenominator);

    event FeeReceiversUpdated(address autoLiquidityReceiver, address MKTFeeReceiver, address devFeeReceiver);
