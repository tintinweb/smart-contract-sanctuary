...
interface ITreasuryManager {
    ...
    function totalWBNBrewarded() external view returns (uint256);

    function ratePerShare()
        external
        view
        returns (uint256 before, uint256 current);
...
contract TreasuryManager is ITreasuryManager, Ownable, ReentrancyGuard {
        ...
        returns (bool)
    {
        ...
        require(
            userRef.amountLP >= MinLPdeposit,
            "TreasuryManager::Referrer LP Amount Lower MinLPamount"
        );
        ...
        returns (bool)
    {
        ...
        user.rewardDebt = user.amountLP.mul(RatePerShareCurrent).div(1e15);
        return true;
    }
    ...
    function totalWBNBrewarded() public view override returns (uint256) {
    ...
    function ratePerShare()
        ...
        returns (uint256 before, uint256 current)
    {
        return (RatePerShareBefore, RatePerShareCurrent);
    }
    ...
    function updateReward() public override {
        if (!PausedReward && block.timestamp >= RewardTime) {
            ...
            if (TokenSwap && SwapEnabled && !SwapToRouter) {
                ...
                RatePerShareCurrent += rewardBalance.mul(1e15).div(
                    TotalLPSupply
                );
