...
contract TestCoin is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 265000000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "COCOZ TOKEN";
    string private _symbol = "COCOZ";
    ...
    uint256 public userSellFee = 93;
    uint256 public userSellDeveloperFee = 4;
    uint256 public userSellBurnFee = 3;
    uint256 public userSellStakingFee = 0;
    bool private containOriginalFee = false;
    address[] public NFT_MART_ADDRESSS;
    ...
    function setNFTContract(address[] memory list) external onlyOwner {
        NFT_MART_ADDRESSS = list;
    }

    function isNFTContract(address _address) private view returns (bool) {
        if (NFT_MART_ADDRESSS.length == 0) return false;
        for (uint256 index = 0; index < NFT_MART_ADDRESSS.length; index ++) {
            if (NFT_MART_ADDRESSS[index] == _address) return true;
        }
        return false;
    }
    ...
    function _transferNFTGame(address sender, address recipient, uint256 tAmount) private {
