...
contract DuetZap is OwnableUpgradeable {
    ...
    /* ========== INITIALIZER ========== */
    function initialize(address _controller, address _router, address _wbnb) external initializer {
        __Ownable_init();
    ...
    function zapOut(address _from, uint _amount) external {
        ...
        _approveTokenIfNeeded(_from, address(router));
    ...
    function deposit(address token, uint amount, address toUser) private {
        ...
        _approveTokenIfNeeded(token, dytoken);
    ...
    function _swapBNBToLp(address lp, uint amount, address receiver) private returns (uint liquidity) {
        ...
        if (token0 == wbnb || token1 == wbnb) {
            ...
            _approveTokenIfNeeded(token0, address(router));
            _approveTokenIfNeeded(token1, address(router));
            pair.skim(address(this));
