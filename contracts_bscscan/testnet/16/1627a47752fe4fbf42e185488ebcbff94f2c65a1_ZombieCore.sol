...
contract zombieFactory is Ownable{
    ...
    function randMod(uint _modulus, uint _targetCount) public returns(uint){
        randNonce++;
        return modulus = (uint(keccak256(abi.encodePacked(_modulus+1,block.timestamp,_msgSender(),randNonce))) % _targetCount)+1;
    }
    ...
    function _getTypeId(uint32 _rare) public   returns(uint32,string memory){
        ...
        if(_rare ==1){
            _typeId = zombieTypesRareOne[randMod(modulus,zombieTypesRareOne.length)-1].typeId;
            _typeName = zombieTypesRareOne[randMod(modulus,zombieTypesRareOne.length)-1].typeName;
        }
        if(_rare ==2){
            _typeId = zombieTypesRareTwo[randMod(modulus,zombieTypesRareTwo.length)-1].typeId;
            _typeName = zombieTypesRareTwo[randMod(modulus,zombieTypesRareTwo.length)-1].typeName;
        }
        if(_rare ==3){
            _typeId = zombieTypesRareThree[randMod(modulus,zombieTypesRareThree.length)-1].typeId;
            _typeName = zombieTypesRareThree[randMod(modulus,zombieTypesRareThree.length)-1].typeName;
        }
        if(_rare ==4){
            _typeId = zombieTypesRareFour[randMod(modulus,zombieTypesRareFour.length)-1].typeId;
            _typeName = zombieTypesRareFour[randMod(modulus,zombieTypesRareFour.length)-1].typeName;
        }
        if(_rare ==5){
            _typeId = zombieTypesRareFive[randMod(modulus,zombieTypesRareFive.length)-1].typeId;
            _typeName = zombieTypesRareFive[randMod(modulus,zombieTypesRareFive.length)-1].typeName;
        }
        return (_typeId,_typeName);
    }
    function _createZombies(uint _modulus) public  {
        require(typeCount>0);
        uint _zombieId = zombies.length;
        uint32 _rand = uint32(randMod(_modulus,1000));
        uint32 _rare = uint32(_getRare(_rand));
        uint32 _typeID ;
        uint32 _attributeID = uint32(randMod(_modulus,5));
        //uint32 _rare = uint32(zombieTypes[_zombieId].rare);
        uint32 _energy = uint32(randMod(_modulus,_rare*100)+_rare*100);
        string memory _typeName ;
        (_typeID,_typeName)= _getTypeId(_rare);
        zombies.push(Zombie(_zombieId,_typeID,_attributeID,_rare,_energy,0,0,0,0,0,0,"Noname",_typeName));
...
contract zombieHelper is zombieFactory{
    ...
    function fusionRare(uint32 _rareOne,uint32 _rareTwo) public returns(uint32){
        ...
        uint32 _rare;
        uint32 _rand =uint32(randMod(modulus,100));
    ...
    function multiply(uint _zombieId, uint _targetId) public multiplyOnlyOwnerOf(_zombieId,_targetId) {
        ...
        uint32 _typeID;
        uint32 _attributeID = uint32(randMod(modulus,5));
        uint32 _energy = uint32(randMod(modulus,_rare*100)+_rare*100);
        string memory _typeName ;
        (_typeID,_typeName)= _getTypeId(_rare);
