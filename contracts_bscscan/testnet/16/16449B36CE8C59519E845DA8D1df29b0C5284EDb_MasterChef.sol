...
contract BecoToken is BEP20 {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "BECO::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "BECO::delegateBySig: invalid nonce");
        require(now <= expiry, "BECO::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "BECO::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "BECO::_writeCheckpoint: block number exceeds 32 bits");
...
// 
// MasterChef is the master of Beco. He can make Beco and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Beco is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BECO
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBecoPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBecoPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Beco to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Beco distribution occurs.
        uint256 accBecoPerShare;   // Accumulated Beco per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 harvestInterval;  // Harvest interval in seconds
    }

    // The BECO TOKEN!
    BecoToken public beco;
    // Dev address.
    address public devAddress;
    // Deposit Fee address
    address public feeAddress;
    // Beco tokens created per block.
    uint256 public becoPerBlock;
    // Bonus muliplier for early beco makers.
    ...
    // The block number when BECO mining starts.
    uint256 public startBlock;
    // Total locked up rewards
    uint256 public totalLockedUpRewards;

    // Beco referral contract address.
    IBecoReferral public becoReferral;
    ...
    constructor(
        BecoToken _beco,
        uint256 _startBlock,
        uint256 _becoPerBlock
    ) public {
        beco = _beco;
        startBlock = _startBlock;
        becoPerBlock = _becoPerBlock;
    ...
    // Update the given pool's BECO allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending BECO on frontend.
    function pendingBeco(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBecoPerShare = pool.accBecoPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 becoReward = multiplier.mul(becoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accBecoPerShare = accBecoPerShare.add(becoReward.mul(1e12).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accBecoPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 becoReward = multiplier.mul(becoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        beco.mint(devAddress, becoReward.div(10));
        beco.mint(address(this), becoReward);
        pool.accBecoPerShare = pool.accBecoPerShare.add(becoReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for BECO allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (_amount > 0 && address(becoReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            becoReferral.recordReferral(msg.sender, _referrer);
        }
        payOrLockupPendingBeco(_pid);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(beco)) {
                uint256 transferTax = _amount.mul(beco.transferTaxRate()).div(10000);
    ...
    // Pay or lockup pending BECO.
    function payOrLockupPendingBeco(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeBecoTransfer(msg.sender, totalRewards);
    ...
    function updateEmissionRate(uint256 _becoPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, becoPerBlock, _becoPerBlock);
        becoPerBlock = _becoPerBlock;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(becoReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = becoReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                beco.mint(referrer, commissionAmount);
                becoReferral.recordReferralCommission(referrer, commissionAmount);
