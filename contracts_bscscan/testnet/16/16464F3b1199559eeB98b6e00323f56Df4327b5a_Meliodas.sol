...
contract Meliodas is Context, IERC20, IERC20Extented, Ownable {
    ...
    uint256 private constant _tTotal = 60000000000 * 10**18; // 60 billion
    ...
    uint256 public _marketingFee = 60; // divided by 1000
    uint256 private _previousMarketingFee = _marketingFee;
    uint256 public _triviaFee = 10; // divided by 1000
    uint256 private _previousTriviaFee = _triviaFee;
    uint256 public _devFee = 10; // divided by 1000
    uint256 private _previousDevFee = _devFee;
    uint256 public _burnFee = 0; // divided by 1000
    uint256 private _previousBurnFee = _burnFee;
    
    uint256 private _marketingPercent = 750;
    uint256 private _triviaPercent = 125;
    uint256 private _devPercent = 125;
    ...
    address payable private _triviaAddress = payable(0x5cCaA2b9f967019FE5ea59AC572407dBe0858cbE);
    address payable private _devAddress = payable(0xBD92de26245518A172732222749e662Fb28b4674);
    ...
    event PercentsUpdated(uint256 _marketingPercent, uint256 _triviaPercent, uint256 _devPercent);
    event FeesUpdated(uint256 _marketingFee, uint256 _liquidityFee, uint256 _triviaFee, uint256 _devFee, uint256 _burnFee);
    ...
    function removeAllFee() private {
        if (_marketingFee == 0 && _liquidityFee == 0 && _triviaFee == 0 && _devFee == 0 && _burnFee == 0) return;
        _previousMarketingFee = _marketingFee;
        _previousLiquidityFee = _liquidityFee;
        _previousTriviaFee = _triviaFee;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && !presale && from != address(this) && to != address(this) && from != bridge && to != bridge) {
            ...
            if (!inSwap && from != uniswapV2Pair) { //sells, transfers
                ...
                if (contractTokenBalance > 0 && from != address(uniswapV2Router)) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 autoLPamount = _liquidityFee.mul(contractTokenBalance).div(_marketingFee.add(_triviaFee).add(_devFee).add(_liquidityFee));
    ...
    function sendETHToFee(uint256 amount) private {
        ...
        _triviaAddress.transfer(amount.mul(_triviaPercent).div(1000));
        _devAddress.transfer(amount.mul(_devPercent).div(1000));
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        ...
        fees.tTrivia = amount.mul(_triviaFee).div(1000);
        fees.tDev = amount.mul(_devFee).div(1000);
        fees.tBurn = amount.mul(_burnFee).div(1000);
        
        fees.tAmount = amount.sub(fees.tMarketing).sub(fees.tLiquidity).sub(fees.tTrivia).sub(fees.tDev).sub(fees.tBurn);
        
        _balances[sender] = _balances[sender].sub(amount);
        _balances[recipient] = _balances[recipient].add(fees.tAmount);
        _balances[address(this)] = _balances[address(this)].add(fees.tMarketing.add(fees.tLiquidity).add(fees.tTrivia).add(fees.tDev));
        _balances[_burnAddress] = _balances[_burnAddress].add(fees.tBurn);
        
        emit Transfer(sender, recipient, fees.tAmount);
        emit Transfer(sender, address(this), fees.tMarketing.add(fees.tLiquidity).add(fees.tTrivia).add(fees.tDev));
    }
    ...
    function setTaxes(uint256 marketingFee, uint256 liquidityFee, uint256 triviaFee, uint256 devFee, uint256 burnFee) external onlyOwner() {
        uint256 totalFee = marketingFee.add(liquidityFee).add(triviaFee).add(devFee).add(burnFee);
        require(totalFee.div(10) < 50, "Sum of fees must be less than 50");

        _marketingFee = marketingFee;
        _liquidityFee = liquidityFee;
        _triviaFee = triviaFee;
        _devFee = devFee;
        _burnFee = burnFee;
        ...
        uint256 totalBNBfees = _marketingFee.add(_triviaFee).add(_devFee);
        
        _marketingPercent = (_marketingFee.mul(1000)).div(totalBNBfees);
        _triviaPercent = (_triviaFee.mul(1000)).div(totalBNBfees);
        _devPercent = (_devFee.mul(1000)).div(totalBNBfees);
        
        emit FeesUpdated(_marketingFee, _liquidityFee, _triviaFee, _devFee, _burnFee);
    }
    ...
    function updateTriviaAddress(address payable triviaAddress) external onlyOwner() {
        _triviaAddress = triviaAddress;
    }
