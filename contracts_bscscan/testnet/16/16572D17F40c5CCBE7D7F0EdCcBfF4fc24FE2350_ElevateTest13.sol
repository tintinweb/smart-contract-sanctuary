...
contract ElevateTest13 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("ElevateTest13", "ELEVATE") {

    	marketingWallet = 0x9f890987b17100AFF0a0ab94778504AF5513282b;
    ...
    function _transfer(
        ...
        if (tradingIsEnabled &&
            ...
            if (rewardsEnabled) {
                ...
                totalFees = totalFees.add(rewardPortion);
                super._transfer(from, rewardsWallet, rewardPortion);
            }
            
            if (marketingEnabled) {
                uint256 marketingPortion = fees.mul(buyMarketingFee).div(totalBuyFees);
                totalFees = totalFees.add(marketingPortion);
                super._transfer(from, marketingWallet, marketingPortion);
            }

            if (buyBackEnabled) {
                uint256 buyBackPortion = fees.mul(buyBuyBackFee).div(totalBuyFees);
                totalFees = totalFees.add(buyBackPortion);
                super._transfer(from, address(this), buyBackPortion);
            }   
            ...
            if (marketingEnabled) {
                uint256 marketingPortion = fees.mul(sellMarketingFee).div(totalSellFees);
                totalFees = totalFees.add(marketingPortion);
                super._transfer(from, marketingWallet, marketingPortion);
            }

            if (buyBackEnabled) {
                uint256 buyBackPortion = fees.mul(sellBuyBackFee).div(totalSellFees);
                totalFees = totalFees.add(buyBackPortion);
                super._transfer(from, address(this), buyBackPortion);
            }  
        ...
        if (swapping && 
            ...
            block.timestamp.sub(lastBuyBack) > (10 minutes)) {

            swapTokensForBNB(contractTokenBalance);
            uint256 contractBalance = address(this).balance;            

            if (buyBackEnabled) {
                buyBackBalance = contractBalance.mul(buyBackPercent).div(100);
                buyBackAndBurn(buyBackBalance);

                uint256 afterSwap = address(this).balance;

                transferToWallet(payable(marketingWallet), afterSwap);

                buyBackBalance = 0;
            } else {
                buyBackAndBurn(contractBalance);
            }
