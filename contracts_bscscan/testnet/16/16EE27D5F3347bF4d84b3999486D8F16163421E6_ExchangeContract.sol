...
contract ExchangeContract is Sale{
    ...
    function burn(uint256 tokenId, address token, uint256 _type, uint256 NOFToken ) public{
        require( balances[tokenId][msg.sender] >= NOFToken || msg.sender == address(this), "Your Not a Token Owner or insuficient Token Balance");
        if(_type == 721){
            Woo721 tok= Woo721(token);
            tok._burn(tokenId, msg.sender, address(this));
            ...
            Woo1155 tok= Woo1155(token);
            tok.burn(msg.sender, address(this), tokenId, NOFToken);
            balances[tokenId][msg.sender] = balances[tokenId][msg.sender].sub(NOFToken);
