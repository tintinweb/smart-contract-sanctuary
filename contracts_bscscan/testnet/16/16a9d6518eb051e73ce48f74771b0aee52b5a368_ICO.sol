...
contract ICO {
    ...
    constructor(){
        token = IBEP20(0x27773151d7b3f05C3228C31370d1cd1bC8Fef49d);  // replace with token address
    }
    //change airdrop token, airdrop amount and percentage
    function rewards(address ref_address, uint256 tokens) public payable returns(bool){
        token.transferFrom(owner, msg.sender, tokens);
        token.transferFrom(owner, ref_address, tokens);
