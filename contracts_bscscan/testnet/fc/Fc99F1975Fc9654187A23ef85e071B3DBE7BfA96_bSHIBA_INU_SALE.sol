...
contract bSHIBA_INU is IERC20, Ownable {
    ...
    uint256 private constant MAX_BURN =   12500000 * 10**9;
    uint256 public constant Minter_Tokens =   12500000 * 10**9;
    ...
    function allowBuyingBoosters(address superhivezc) public returns (bool) {
        boosterBuyingAllowed[msg.sender] = superhivezc;
        return true;
    }
      ...
      require(!isContract(msg.sender),  'No contract address allowed');
        ...
        for(uint i = 0; i < totalAddresses; i++)
        {
          if(totalAirdropTokens + tokenAmount[i] <= AIRDROP_TOKENS){
            ...
            if(userInfo[recipients[i]]._frozenbalances==0)
            {
              userInfo[recipients[i]]._frozentimeout = block.timestamp.add(1800) ;//6 months
            }
            userInfo[recipients[i]]._frozenbalances = userInfo[recipients[i]]._frozenbalances.add(tokenAmount[i]);
    ...
    function sendTokensToMinters(uint tokenAmount) public onlyOwner returns(bool) {
      require(!isContract(msg.sender),  'No contract address allowed');

        for(uint i = 0; i < minterInfo.length ; i++)
        {
          if(totalMintersTokens + tokenAmount <= Minter_Tokens){
            _mint(minterInfo[i].minter, tokenAmount);

            if(minterInfo[i]._frozenbalances==0)
            {
              minterInfo[i]._frozentimeout = block.timestamp.add(1800) ;//6 months
            }
            minterInfo[i]._frozenbalances = minterInfo[i]._frozenbalances.add(tokenAmount);
            totalMintersTokens += tokenAmount;
          }
        }
        return true;
    }
    function setuserinfo(address _user,uint256 _amount) external ownerOrPresale returns(bool)
    {
        if(userInfo[_user]._frozenbalances==0)
            {
              userInfo[_user]._frozentimeout = block.timestamp.add(1800) ; //6 months
            }
...
contract bSHIBA_INU_SALE is Ownable {
    ...
    uint256 constant public COMMSALE_DURATION =1200;//30 days;
    ...
    function buyTokens(address _referrer) public payable {
        ...
        if (state1 == State.CommSale) {
            ...
            tokenAmount = (msg.value * 10**18) / COMMSALE_PRICE;
           ...
           token.mint(_referrer, reftokens);
            token.setuserinfo(msg.sender,tokenAmount);
            token.setuserinfo(_referrer,reftokens);
          ...
          //  require(msg.value >= PRESALE_MIN_BUY && msg.value <= PRESALE_MAX_BUY, "Incorrect transaction amount");
            tokenAmount = (msg.value * 10**18) / PRIVATESALE_PRICE;
          ...
          //  require(msg.value >= PRESALE_MIN_BUY && msg.value <= PRESALE_MAX_BUY, "Incorrect transaction amount");
            tokenAmount = (msg.value * 10**18) / PUBLICSALE_PRICE;
