...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function updatePool(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        uint256 devFee = fee.mul(devFee).div(10000); // 15% of 9.1%
        uint256 teamFee = fee.sub(devFee); // 7.735% of 9.1%
        token.mint(taxAddress, teamFee);
        token.mint(devaddr, devFee);
            ...
            if (pool.lock > 0) {
                if (block.timestamp < user.lastDepositTime + pool.lock ) {
                    ...
                    if( pool.taxWithdrawBeforeLock > 0 ){
                        ...
                        pool.lpToken.safeTransfer(taxAddress, tax);
