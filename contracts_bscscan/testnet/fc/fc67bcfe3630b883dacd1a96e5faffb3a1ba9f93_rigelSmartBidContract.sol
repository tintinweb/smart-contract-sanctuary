...
constructor(
    ...
    function resetBiddingProcess(uint256 _pid, uint _bidTimeOut, uint256 _bidAmount) public onlyAdmin {
        ...
        for (uint256 j = 0; j < pushTopBidders.length; j++) {
            address wallet = pushTopBidders[j];
            delete(_TopUsersReward[_pid][wallet]);
        }
    }
    ...
    function otherQualifyBidders(uint256 pid) external onlyAdmin{
    ...
    function shareBTWTopBidders(uint256 pid) public {
        CreateBid storage bid = createBid[pid];
        
        require(_msgSender() != _TopUsersReward[pid][_msgSender()], "RIGEL: you cant claim reward twice");
        require(block.timestamp > bid.timeOut, "RGP: BIDDING IS STILL IN PROGRESS");
        ...
        if (user1 == _msgSender()) {
            _TopUsersReward[pid][_msgSender()] = _msgSender();
            pushTopBidders.push(_msgSender());
            ...
            bid.totalBidding = bid.totalBidding.sub(_positionOne);
        } else if (user2 == _msgSender()) {
            _TopUsersReward[pid][_msgSender()] = _msgSender();
            pushTopBidders.push(_msgSender());
            ...
            bid.totalBidding = bid.totalBidding.sub(_positionTwo);
        } else if (user3 == _msgSender()) {
            _TopUsersReward[pid][_msgSender()] = _msgSender();
            pushTopBidders.push(_msgSender());
            ...
            bid.totalBidding = bid.totalBidding.sub(_positionThree);
        }
    ...
    function withdrawBidToken(uint256 _pid, address _to, uint256 _amount) external onlyOwner{
        CreateBid storage bid = createBid[_pid];
