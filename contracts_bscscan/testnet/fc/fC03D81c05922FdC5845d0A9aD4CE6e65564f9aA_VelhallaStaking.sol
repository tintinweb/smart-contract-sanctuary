...
contract VelhallaStaking is Ownable, Pausable {
    using SafeMath for uint256;
    ...
    // SCAR tokens reward per block.
    uint256 public scarPerBlock;
    ...
    constructor(
        ...
        scar = _scar;
        vht = _vht;
        totalReward = _totalreward;  // total SCAR token reward for 240 days
        // totalReward = 20000000 * 1e18;  // Prepare 20000000 SCAR for reward
        scarPerBlock = totalReward.div(240*24*60*60).mul(3);  // total SCAR token for 240 days, where 3 second/block
    ...
    // Update the given pool's SCAR allocation point. Can only be called by the owner.
    ...
    function pendingScar(address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = pool.lpToken.balanceOf(address(this)).sub(totalReward);
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 scarReward = multiplier.mul(scarPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accScarPerShare = accScarPerShare.add(scarReward.mul(1e31).div(lpSupply));
        }
    ...
    function updatePool() public whenNotPaused{
        ...
        if(totalReward.sub(scarReward) < 0) // Stop giving out rewards when VelhallaStaking contract run out of funds. Keep pending without modifying pool.lastRewardBlock until injecting funds.
        {
            return;
        }
        
        scar.transfer(address(vht), scarReward);
        totalReward = totalReward.sub(scarReward);

        pool.accScarPerShare = pool.accScarPerShare.add(scarReward.mul(1e31).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }


    // Stake SCAR tokens to VelhallaStaking
    function enterStaking(uint256 _amount) internal whenNotPaused{
        PoolInfo storage pool = poolInfo[0];
    ...
    function emergencyWithdraw() external whenNotPaused{
        require(!emergencyWithdrawLock == false, "The emergency withdraw is locked");
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[0][msg.sender];
    ...
    function getTotalStakeValue() public view returns (uint256) {
        return scar.balanceOf(address(this)).sub(totalReward);
    }
    ...
    function setLeaveStakingLock(bool _leaveStakingLock) external onlyOwner{
