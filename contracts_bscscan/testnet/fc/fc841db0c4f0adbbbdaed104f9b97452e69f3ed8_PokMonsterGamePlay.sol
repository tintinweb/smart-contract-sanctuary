    ...
    ReentrancyGuard
{
    ...
    function initialize(
        ...
        nft = _nft;
        stats = _stats;
        rewardPerSecond = 753424657534246575; // 0.753424657534246575 pok/seconds
   ...
   function setRewardPerSecond(uint256 _rewardPerSecond)
        ...
        onlyOwner
    {
        require(_rewardPerSecond <= 1 ether, "insane high rate");
        massUpdatePools();
        ...
        returns (uint256)
    {
        ...
        uint256 _totalReward = (user.amount * accpokPerShare) / 1e18;
    ...
    function deposit(uint256 _pid, uint256 _tokenId)
        public
        notContract
        nonReentrant
    {
        BRC721 _nft = BRC721(nft);
        ...
        uint256 _userAmount = getBoostedPercent(_tokenId);
        if(_userAmount>0){
        ...
        user.rewardDebt = (user.amount * pool.accpokPerShare) / 1e18;
    ...
    function _withdraw(
        ...
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_account];
        require(user.tokenId > 0, "withdraw: not good");
        updatePool(_pid);
        if (user.amount > 0) {
            _harvestReward(_pid, _account);
        }
        BRC721(nft).safeTransferFrom(address(this), msg.sender, user.tokenId);
        user.tokenId=0;
        user.amount=0;
        user.rewardDebt = (user.amount * pool.accpokPerShare) / 1e18;
    }
