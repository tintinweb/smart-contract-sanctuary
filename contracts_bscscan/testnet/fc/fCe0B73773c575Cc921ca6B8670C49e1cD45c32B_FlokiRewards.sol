...
// : Unlicense
contract FlokiRewards is ERC20, Ownable {
    using SafeERC20 for IERC20;
    ...
    uint256 public flokiFee;
    uint256 public shibaFee;
    ...
    constructor(
    ...
    ) ERC20("FlokiRewards", "FLOKIRDS") Ownable() {

        
        IUniswapV2Router02 router_ = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        IERC20 floki_ = IERC20(0x2B3F34e9D4b127797CE6244Ea341a83733ddd6E4);
        IERC20 shiba_ = IERC20(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D);
        address marketingWallet_ = 0xA0132e8558c23986E0F387ad3e873592B40da74c;
        ...
        dividendCurrencies[0] = address(floki);
        dividendCurrencies[1] = address(shiba);
    ...
    function shibaDividendOf(address account) external view returns (uint256) {
        return tracker.dividendOf(account, address(shiba));
    }

    function flokiDividendOf(address account) external view returns (uint256) {
        return tracker.dividendOf(account, address(floki));
    }
    ...
    function setMarketingFee(uint256 fee) external onlyOwner {
        require(fee + flokiFee + shibaFee + liquidityFee <= MAX_TOTAL_FEE, "Total fees can not exceed MAX_TOTAL_FEE");
        marketingFee = fee;
    }

    function setFlokiFee(uint256 fee) external onlyOwner {
        require(marketingFee + fee + shibaFee + liquidityFee <= MAX_TOTAL_FEE, "Total fees can not exceed MAX_TOTAL_FEE");
        flokiFee = fee;
    }

    function setShibaFee(uint256 fee) external onlyOwner {
        require(marketingFee + flokiFee + fee + liquidityFee <= MAX_TOTAL_FEE, "Total fees can not exceed MAX_TOTAL_FEE");
        shibaFee = fee;
    }

    function setLiquidityFee(uint256 fee) external onlyOwner {
        require(marketingFee + flokiFee + shibaFee + fee <= MAX_TOTAL_FEE, "Total fees can not exceed MAX_TOTAL_FEE");
        liquidityFee = fee;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (isExcludedFromFee[from] || isExcludedFromFee[to]) {
            ...
            uint256 totalFeeAmount = amount * (marketingFee + flokiFee + shibaFee + liquidityFee) / 100;
    ...
    function _swapAndDistribute() internal lockTheSwap {
        uint256 totalFees = liquidityFee + marketingFee + flokiFee + shibaFee;
        ...
        uint256 amountForFloki = balanceOf(address(this)) * flokiFee / totalFees;
        uint256 amountForShiba = balanceOf(address(this)) - amountForLiquidity - amountForMarketing - amountForFloki;
        ...
        uint256 flokiAmountEth = totalBalance * amountForFloki / totalToSwap;
        uint256 shibaAmountEth = totalBalance * amountForShiba / totalToSwap;
        uint256 liquidityAmountEth = totalBalance - marketingAmountEth - flokiAmountEth - shibaAmountEth;

        payable(marketingWallet).transfer(marketingAmountEth);

        _swapETHForTokens(address(floki), flokiAmountEth);
        uint256 flokiBalance = floki.balanceOf(address(this));
        floki.safeTransfer(address(tracker), flokiBalance);
        tracker.distributeDividends(address(floki), flokiBalance);

        _swapETHForTokens(address(shiba), shibaAmountEth);
        uint256 shibaBalance = shiba.balanceOf(address(this));
        shiba.safeTransfer(address(tracker), shibaBalance);
        tracker.distributeDividends(address(shiba), shibaBalance);
