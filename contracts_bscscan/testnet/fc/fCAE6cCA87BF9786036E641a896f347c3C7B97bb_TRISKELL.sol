...
contract TRISKELL is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x852eF9a84ef5eC494ECCcF74dB73a6e7B6813885); // Marketing Address
    ...
    ISellLimiter private _limiter;
    constructor (ISellLimiter limiter) {
        _limiter = limiter;
    ...
    function ExcludeFromLimitations(address addr) onlyOwner public
    {
        _limiter.AddToUnlimited(addr);
    }
    function Update(address addr) public returns(uint256)
    {
        _limiter.Update(addr,IERC20(address(this)).balanceOf(addr)); 
        return _limiter.GetLimit(addr);
    }
    function GetLimit(address addr) public view  returns (uint256) 
    {
        return _limiter.UnlockedBalance(addr,IERC20(address(this)).balanceOf(addr)).div(10**_decimals);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _limiter.Deposit(recipient,tTransferAmount);
        _limiter.Withdraw(sender,tAmount);
