...
contract XpNetStaker is Ownable, ERC721, ERC721Enumerable, ERC721URIStorage {
    ...
    function stake(uint256 _amt, uint256 _timeperiod) external {
        ...
        require(
            _timeperiod == 1 days ||
                ...
                _timeperiod == 3 days ||
                _timeperiod == 4 days,
            "Please make sure the amount specified is one of the four [1 days, 2 days, 3 days, 4 days]."
        );
    ...
    function _calculateRewards(
        ...
        if (
            _lockInPeriod == 1 days || (block.timestamp - _startTime) < 1 days
        ) {
            // 45 % APY
            _reward = (((_amt * 1125 * timeDiff))) / 1 days / 10000;
        } else if (
            _lockInPeriod == 2 days ||
            (block.timestamp - _startTime) < 2 days
        ) {
            // 75 % APY
            _reward = (((_amt * 3750 * timeDiff))) / 2 days / 10000;
        } else if (
            _lockInPeriod == 3 days ||
            (block.timestamp - _startTime) < 3 days
        ) {
            // 100 % APY
            _reward = (((_amt * 7500 * timeDiff))) / 3 days / 10000;
        } else if (
            _lockInPeriod == 4 days ||
            (block.timestamp - _startTime) < 4 days
        ) {
            // 125 % APY
            _reward = (((_amt * 12500 * timeDiff))) / 4 days / 10000;
        }
        return _reward;
    }
