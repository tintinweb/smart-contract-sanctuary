...
contract XaiTimelockERC721 {
    ...
    mapping(address => uint256) public depositedItemID;
    
    function deposit(address minter, uint256 itemID) public returns (uint256) {
        ERC721 _minter = ERC721(minter);
        // check via minter if msg.sender has itemID
        require(_minter.ownerOf(itemID) == msg.sender, "Sender is not the owner of itemID");
        ...
        depositedItemID[msg.sender] = itemID;
        return depositedTime[msg.sender];
    }
    
    function withdraw() public view returns (uint256) {
        // if currentTime >= withdraw time (deposited time + 7 days)
        require(depositedTime[msg.sender] != 0, "No deposit to withdrawn");
        require(getCurrentTime() >= getWithdrawTime(), "Timelock not complete");
        return getWithdrawTime();
    }
    
    function getWithdrawTime() public view returns (uint256) {
        require(depositedTime[msg.sender] != 0, "No deposit to withdrawn");
        return SafeMath.add(depositedTime[msg.sender], 604800);
    }
