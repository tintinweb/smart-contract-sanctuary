...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v3.4/contracts/GSN/Context.sol
...
*/
contract MShroom is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 MSHROOM-CAKE LP -> MSHROOM pool
    uint256 public constant INITIAL_MSHROOM_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the MSHROOM ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("MSHROOM", "MSHROOM") {
        // Mints 1 MSHROOM to contract creator for initial pool setup
    ...
    function _getMShroomPrice() internal view returns (uint256 _mshroomPrice) {
        try IOracle(mshroomOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("MShroom: failed to fetch MSHROOM price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _mshroomPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_mshroomPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMshroomOracle(address _mshroomOracle) public onlyOperatorOrTaxOffice {
        require(_mshroomOracle != address(0), "oracle address cannot be 0 address");
        mshroomOracle = _mshroomOracle;
    }
    ...
    /**
     * @notice Operator mints MSHROOM to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MSHROOM to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMshroomPrice = _getMShroomPrice();
            currentTaxRate = _updateTaxRate(currentMshroomPrice);
            if (currentMshroomPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        address _mshroomPool,
        address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_mshroomPool != address(0), "!_mshroomPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_mshroomPool, INITIAL_MSHROOM_POOL_DISTRIBUTION);
