...
contract ZOY is BEP20, BEP20Detailed {
  ...
  constructor () BEP20Detailed("ZoolPay", "ZOY", 8) {
      _owner = msg.sender;
...
contract crowdsale is Ownable{
 ...
 address public thisAddress;
  constructor(ZOY _token, address payable _wallet, uint256 _rate) {
  ...
  function buyTokens(address _beneficiary) public payable {
    ...
    time[_beneficiary] = block.timestamp;
  ...
  function _deliverTokens(
    ...
    internal
  {
      if(block.timestamp >= time[_beneficiary] + 15 days)
    {token.transfer(_beneficiary, tokenHolders[_beneficiary]);}
  }
  ...
  function sendTokensBack(address addressToQuery) public onlyOwner
  {
    ...
    token.transfer(wallet, contractBalance);
  }
