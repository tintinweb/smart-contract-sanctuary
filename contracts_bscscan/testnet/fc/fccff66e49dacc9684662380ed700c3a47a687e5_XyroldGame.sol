...
contract XyroldGame {  
    ...
    uint256[] private OpenGamesID;
    uint256[] private ClosedGamesID;
    ...
    */
    ...
    function getOwnRoundChoices(uint _gameId, address _player) view private returns (uint256 ID,  string memory round1, string memory round2, string memory round3) {
        Game storage _getGame = GamesPool[_gameId];
        return (_getGame.ID,  getchoicename(_getGame.details[_player].round1), getchoicename(_getGame.details[_player].round2),  getchoicename(_getGame.details[_player].round3));
    }
