...
contract Sale is Ownable{
    ...
    function _acceptBId(address token,address from, address admin, uint256 amount, uint256 tokenId) internal{
        ...
        require(balances[tokenId][msg.sender] > 0, "Is Not a Owner");
        (uint256 _adminfee, uint256 netamount) = calc(amount, _royal[tokenId], serviceValue);
        BEP20 t = BEP20(token);
    ...
    function calc(uint256 amount, uint256 royal, uint256 _serviceValue) internal pure returns(uint256, uint256){
