...
contract HumpBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public immutable HUMP; // token given as payment for bond
    ...
    struct Bond {
        uint payout; // HUMP remaining to be paid
    ...
    function deposit( 
        ...
        uint payout = payoutFor( value ); // HUMP payout to bonder is computed
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( HUMP ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            IERC20( HUMP ).approve( staking, _amount );
            IStaking( staking ).stake( _recipient, _amount, false );
        }
        return _amount;
    }
