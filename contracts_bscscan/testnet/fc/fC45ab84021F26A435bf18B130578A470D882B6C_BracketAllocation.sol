...
contract BracketAllocation is Ownable{
    ...
    /**
     *
     * @dev Brackets struct store all the included brackets
     * 
    ...
    Brackets[] public bracketInfo; //Bracket storage
    address private crowdFunding; //Crowdfunding contract address

    bool public isEnabled; //Enable/Disable use of bracket

    constructor(DecubateStaking _stakingContract, address _crowdFunding) public {
    ...
    function setBracket(
        ...
        bracketInfo[bracketId].maxCount = _maxCount;
        bracketInfo[bracketId].allowedAmount = _allowedAmount;
        bracketInfo[bracketId].minLimit = _minLimit;
        bracketInfo[bracketId].maxLimit = _maxLimit;
    ...
    function getBracketUsers(uint8 bracket) external view returns(address[] memory alist){
        return bracketInfo[bracket].addrList;
    }

    /**
     *
     * @dev Get bracket tier of a user
     *
     * @param addr Address of the user
     *
    ...
    function getTierOfUser(address addr) public view returns(bool flag, uint8 pos) {
        uint256 len = bracketInfo.length;
        uint256 totalDeposit = getTotalDeposit(addr);

        for(uint8 i = 0; i < len; i++){
            if(totalDeposit >= bracketInfo[i].minLimit && totalDeposit <= bracketInfo[i].maxLimit){
                pos = i;
                flag = true;
                break;
            }
     ...
     *
     * @param addr Address of the user
     * @param amount Deposit amount of user
     *
     * @return bool Status of addition
     *
     */
    function setAddress(address addr, uint256 amount) public returns(bool){
        require(msg.sender == crowdFunding,"Only crowdfunding contract");

        (bool flag, uint8 pos) = getTierOfUser(addr);

        Brackets storage bracket = bracketInfo[pos];

        require(flag,"User does not belong to any bracket");
        require(bracket.allowedAmount >= amount,"Amount exceeds brackets allowance");
