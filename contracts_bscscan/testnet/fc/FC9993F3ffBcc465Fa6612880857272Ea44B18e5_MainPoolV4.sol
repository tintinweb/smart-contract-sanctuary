...
contract MainPoolV4 is OwnableUpgradeable {
    ...
    event PoolTypeUpdated(
        ...
        event TCTokenClaim(address tokenAddress, uint256 tokenAmount);
        event Pools(uint256 poolsId);
        event PtPoolType(bool IsIDOL, uint256 startFee, uint256 endFee, uint256 claimPeriod);
        event IDIsIDOL(bool IsIDOL);
        event LastClaimBlock(uint256 lastClaimBlock);
        event BlockPassed(uint256 blockPassed);
        event ClaimAmount(uint256 claimAmount);
        event FeeAmount(uint256 feeAmount);
        event FinalClaimAmount(uint256 finalClaimAmount);
        event FeeRate(uint256 feeRate);
        event DiffFee(uint256 diffFee);
        event ReduceFee(uint256 reduceFee);
    ...
    function claim(address _tokenAddress, uint256 _amount) external {
        ...
        PoolType memory poolType = poolTypes[pools[_tokenAddress]];
        emit Pools(pools[_tokenAddress]);
        emit PtPoolType(poolType.isIDOL, poolType.startFee, poolType.endFee, poolType.claimPeriod);
        TokenClaim[] storage tokenClaims = creatorBal[msg.sender];
        
        for (uint256 i = 0; i < tokenClaims.length; i++ ) {
            TokenClaim storage tokenClaim = tokenClaims[i];
            emit TCTokenClaim(tokenClaim.tokenAddress, tokenClaim.amount);
            uint256 feeRate = 0;
            uint256 feeAmount = 0;
            uint256 claimAmount = 0;
            uint256 finalClaimAmount = 0;
            ...
            if (_tokenAddress == tokenClaim.tokenAddress) {
                ...
                if (poolType.isIDOL) {
                    // slow release
                    emit IDIsIDOL(poolType.isIDOL);
                    emit LastClaimBlock(tokenClaim.lastClaimBlock);
                    blockPassed = block.number.sub(tokenClaim.lastClaimBlock);
                    emit BlockPassed(blockPassed);
                    if (blockPassed / poolType.claimPeriod >= 1) {
                        claimAmount = _amount;
                    } else {
                     ...
                     if (poolType.endFee == 0) {
                        feeAmount = 0;
                        emit FeeAmount(feeAmount);
                        finalClaimAmount = claimAmount;
                        emit FinalClaimAmount(finalClaimAmount);
                        feeRate = poolType.endFee;
                    ...
                    if (poolType.endFee == 0) {
                        ...
                        if (blockPassed.div(poolType.claimPeriod) >= 1) {
                            ...
                            uint256 diffFee = poolType.endFee.sub(poolType.startFee);
                            emit DiffFee(diffFee);
                            uint256 reduceFee = (blockPassed.mul(diffFee)).div(
                                poolType.claimPeriod
                            );
                            emit ReduceFee(reduceFee);
                            feeRate = poolType.endFee.sub(reduceFee);
                            emit FeeRate(feeRate);
                        }
                        // calculate final amount
                        claimAmount = _amount;
                        emit ClaimAmount(claimAmount);
                        feeAmount = (claimAmount.mul(feeRate)).div(10 ** 18);
                        emit FeeAmount(feeAmount);
                        finalClaimAmount = claimAmount.sub(feeAmount);
                        emit FinalClaimAmount(finalClaimAmount);
