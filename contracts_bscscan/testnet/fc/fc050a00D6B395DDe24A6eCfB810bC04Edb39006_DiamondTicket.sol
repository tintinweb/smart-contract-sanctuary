...
contract DiamondTicket is ERC165, IERC721, IERC721Metadata ,IERC721Enumerable,Ownable {
    using Address for address;
    using Strings for uint256;
    // Token name
    string private _name="DiamondTicket";
    // Token symbol
    ...
    function OwnerSetValueOfTicket(uint256 value) public onlyOwner{
        currentValue=value;
    }
    uint256 public currentValue=1000000000000000;
    uint8 priceIncrease=3;//3% price Increase per buy;
    uint256 public maxCount=100;
    struct NFTData{
        string Name;
        uint256 ID;
        string URI;
    ...
    function OwnerSetMaxCount(uint256 count) public onlyOwner{
     require(count>=NFTHolders.length);
     maxCount=count;
    }
    function OwnerRemoveBNB() public onlyOwner{
        (bool sent,)=owner().call{value:address(this).balance}("");
        require(sent);
    }
    

    function OwnerAirdropTickets(address[] memory recipients, bool allowDoubles) public onlyOwner{
        for(uint256 i=0; i<recipients.length; i++)
        {
            address current=recipients[i];
            currentValue=currentValue*(100+priceIncrease)/100;
            if(!allowDoubles&&_tokenOfOwner[current].length>0) continue;
            _mint(current);
        }
    }
    
    function ChangeToken(uint256 ID,string memory newURI, string memory newName) public{
        require(_isApprovedOrOwner(msg.sender, ID)||
        msg.sender==owner(),"Not approved or Owner");
        require(!hidden[ID]||msg.sender==owner(), "NFT is hidden");
        NFTData memory data=NFTs[ID];   
        data.URI=newURI;
        data.Name=newName;
        NFTs[ID]=data;
    }
    function ChangeLock(uint256 ID, bool locked)public{
        require(_isApprovedOrOwner(msg.sender,ID));
        unlocked[ID]=!locked;
    }
    
    function isLocked(uint256 NFTID) public view returns (bool){
        require(_exists(NFTID),"NFT doesn't exist");
        return !unlocked[NFTID]; 
    }

    
    function BuyNFT(uint256 ID) public payable{
        //Locked NFTs can't be traded
        require(!isLocked(ID),"NFT is Locked");

        uint256 TaxedValue=currentValue*(100+priceIncrease)/100;
        address oldOwner=ownerOf(ID);
        bool sent;
        (sent,)=oldOwner.call{value:currentValue}("");
        require(sent);
        _transfer(oldOwner,msg.sender,ID);
        
        if(msg.value>TaxedValue){
            //transfer back excess funds
            (sent,)=msg.sender.call{value: msg.value-TaxedValue}("");
            require(sent);
        }
    }
    receive() external payable {
        ...
        uint256 TaxedValue=currentValue*(100+priceIncrease)/100;
        currentValue=TaxedValue;
        require(msg.value>=TaxedValue);
        
        _mint(msg.sender);
        if(msg.value>TaxedValue){
            //transfer back excess funds
            (bool sent,)=msg.sender.call{value: msg.value-TaxedValue}("");
            require(sent);
        }        
  	}
  	
    
    function getNFTInfo(uint256 ID) public view returns(string memory name_, string memory uri_, address holder_){
        require(_exists(ID));
        NFTData memory data=NFTs[ID];
        return(data.Name, data.URI, NFTHolders[ID]);
    }

  	 
  	 function _mint(address to) private{
        uint256 ID=NFTHolders.length;
        require(ID<maxCount, "no Tickets left");
        NFTData memory data=NFTData("DiamondTicket",ID,"NO URI DEFINED");
        NFTs.push(data);
        unlocked.push(false);
        hidden.push(false);
        ownerID.push(0);
