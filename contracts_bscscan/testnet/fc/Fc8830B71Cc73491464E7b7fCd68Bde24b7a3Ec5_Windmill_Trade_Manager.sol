...
event GasLog_With_DAOAddress(uint256 line, uint256 gas);
    address public DAOAddress;

    function setDAOAddress(address _DAOAddress) external onlyRole(DAO_ROLE){
emit GasLog_With_DAOAddress(14, gasleft());
        DAOAddress = _DAOAddress;
emit GasLog_With_DAOAddress(15, gasleft());
    }
}// Version: 1.0.0
...
event GasLog_Windmill_Trade_Manager(uint256 line, uint256 gas);
    ////
    ////
    ////
    //////////////// Public variables ////////////////
...
emit GasLog_Windmill_Trade_Manager(70, gasleft());
        require(denominator>0, "Windmill_Trade_Manager: Denominator cannot be null");
emit GasLog_Windmill_Trade_Manager(71, gasleft());
        
        maxLeverage.numerator = numerator;
emit GasLog_Windmill_Trade_Manager(73, gasleft());
        maxLeverage.denominator = denominator;
emit GasLog_Windmill_Trade_Manager(74, gasleft());
    }
    
    function setMinimumBUSDToTrade(uint256 _minimumBUSDToTrade) external onlyRole(DAO_ROLE){
emit GasLog_Windmill_Trade_Manager(77, gasleft());
        minimumBUSDToTrade = _minimumBUSDToTrade;
emit GasLog_Windmill_Trade_Manager(78, gasleft());
    }
    
    function setTraderLevel(address addr, uint8 level) external onlyRole(DAO_ROLE){
emit GasLog_Windmill_Trade_Manager(81, gasleft());
        if (level == 0){
emit GasLog_Windmill_Trade_Manager(82, gasleft());
            maxEnergyTotalSupply -= maxEnergy[addr];
emit GasLog_Windmill_Trade_Manager(83, gasleft());
            maxEnergy[addr] = 0;
emit GasLog_Windmill_Trade_Manager(84, gasleft());
        }else if (level == 1){
emit GasLog_Windmill_Trade_Manager(85, gasleft());
            maxEnergyTotalSupply += juniorTraderDefaultMaxEnergy - maxEnergy[addr];
emit GasLog_Windmill_Trade_Manager(86, gasleft());
            maxEnergy[addr] = juniorTraderDefaultMaxEnergy;
emit GasLog_Windmill_Trade_Manager(87, gasleft());
        }else{
emit GasLog_Windmill_Trade_Manager(88, gasleft());
            maxEnergyTotalSupply += seniorTraderDefaultMaxEnergy - maxEnergy[addr];
emit GasLog_Windmill_Trade_Manager(89, gasleft());
            maxEnergy[addr] = seniorTraderDefaultMaxEnergy;
emit GasLog_Windmill_Trade_Manager(90, gasleft());
        }
    }
    
    function getMaxEnergy(address addr) public returns (uint256){
emit GasLog_Windmill_Trade_Manager(94, gasleft());
        return maxEnergy[addr];
emit GasLog_Windmill_Trade_Manager(95, gasleft());
    }
    
    function getRemainingEnergy(address addr) public returns (uint256){
emit GasLog_Windmill_Trade_Manager(98, gasleft());
        uint256 m = maxEnergy[addr];
emit GasLog_Windmill_Trade_Manager(99, gasleft());
        uint256 u = usedEnergy[addr];
emit GasLog_Windmill_Trade_Manager(100, gasleft());
        
        if (u >= m){
emit GasLog_Windmill_Trade_Manager(102, gasleft());
            return 0;
emit GasLog_Windmill_Trade_Manager(103, gasleft());
        }
        return m - u;
emit GasLog_Windmill_Trade_Manager(105, gasleft());
    }
    
    function getEnergyValueBUSD() public returns (uint256){
emit GasLog_Windmill_Trade_Manager(108, gasleft());
        return (fund.getFundBUSD() * maxLeverage.numerator) / (maxEnergyTotalSupply * maxLeverage.denominator);
emit GasLog_Windmill_Trade_Manager(109, gasleft());
    }
    
    function getMinimumEnergyToTrade() public returns (uint256){
emit GasLog_Windmill_Trade_Manager(112, gasleft());
        return 1 + (minimumBUSDToTrade-1) / getEnergyValueBUSD();
emit GasLog_Windmill_Trade_Manager(113, gasleft());
    }
    
    function addTradeDeployer(I_Windmill_Trade_Deployer_Abstract deployer) external onlyRole(DAO_ROLE){
emit GasLog_Windmill_Trade_Manager(116, gasleft());
...
emit GasLog_Windmill_Trade_Manager(117, gasleft());
        
        DeployerData memory data;
emit GasLog_Windmill_Trade_Manager(119, gasleft());
        data.deployer = deployer;
emit GasLog_Windmill_Trade_Manager(120, gasleft());
        data.enabled = true;
emit GasLog_Windmill_Trade_Manager(121, gasleft());
        
        deployer.setBUSDToken(BUSDToken);
emit GasLog_Windmill_Trade_Manager(123, gasleft());
        
        deployers.push(data);
emit GasLog_Windmill_Trade_Manager(125, gasleft());
    }
    
    function disableTradeDeployer(uint256 deployerId) external onlyRole(DAO_ROLE){
emit GasLog_Windmill_Trade_Manager(128, gasleft());
        require(deployerId < deployers.length, "Windmill_Trade_Manager: Trade deployer not found.");
emit GasLog_Windmill_Trade_Manager(129, gasleft());
        
        deployers[deployerId].enabled = false;
emit GasLog_Windmill_Trade_Manager(131, gasleft());
    }
    
    function getNbTradeDeployers() external returns (uint256){
emit GasLog_Windmill_Trade_Manager(134, gasleft());
        return deployers.length;
emit GasLog_Windmill_Trade_Manager(135, gasleft());
    }
    
    function getNbTrades() external returns (uint256){
emit GasLog_Windmill_Trade_Manager(138, gasleft());
        return trades.length;
emit GasLog_Windmill_Trade_Manager(139, gasleft());
    }
    
    function getTrade(uint256 id) external returns (TradeData memory){
emit GasLog_Windmill_Trade_Manager(142, gasleft());
        require(id < trades.length, "Windmill_Trade_Manager: Trade id not found.");
emit GasLog_Windmill_Trade_Manager(143, gasleft());
        return trades[id];
emit GasLog_Windmill_Trade_Manager(144, gasleft());
    }
    
    function getNbOpenTrades() external returns (uint){
emit GasLog_Windmill_Trade_Manager(147, gasleft());
        return openTradeId.length;
emit GasLog_Windmill_Trade_Manager(148, gasleft());
    }
    
    function getOpenTrade(uint openId) external returns (TradeData memory, uint256){
emit GasLog_Windmill_Trade_Manager(151, gasleft());
        require(openId < openTradeId.length, "Windmill_Trade_Manager: Open trade id not found.");
emit GasLog_Windmill_Trade_Manager(152, gasleft());
        uint256 id = openTradeId[openId];
emit GasLog_Windmill_Trade_Manager(153, gasleft());
        return (trades[id], id);
emit GasLog_Windmill_Trade_Manager(154, gasleft());
    }
    
    function getTradeDeployer(uint256 id) external returns (DeployerData memory){
emit GasLog_Windmill_Trade_Manager(157, gasleft());
        require(id < deployers.length, "Windmill_Trade_Manager: Deployer id not found.");
emit GasLog_Windmill_Trade_Manager(158, gasleft());
        
        return deployers[id];
emit GasLog_Windmill_Trade_Manager(160, gasleft());
    }
    
    function startTrade(uint256 deployerId, uint256 nbEnergy) external returns (uint256){
emit GasLog_Windmill_Trade_Manager(163, gasleft());
        require(nbEnergy <= getRemainingEnergy(msg.sender), "Windmill_Trade_Manager: Not enough remaining energy.");
emit GasLog_Windmill_Trade_Manager(164, gasleft());
        require(nbEnergy >= getMinimumEnergyToTrade(), "Windmill_Trade_Manager: Not enough energy to start a trade.");
emit GasLog_Windmill_Trade_Manager(165, gasleft());
        require(deployerId < deployers.length, "Windmill_Trade_Manager: Trade deployer not found.");
emit GasLog_Windmill_Trade_Manager(166, gasleft());
        
        DeployerData storage deployer = deployers[deployerId];
emit GasLog_Windmill_Trade_Manager(168, gasleft());
        
        require(deployer.enabled, "Windmill_Trade_Manager: Trade deployer disabled.");
emit GasLog_Windmill_Trade_Manager(170, gasleft());
        
        uint256 totalBUSD = fund.getFundBUSD();
emit GasLog_Windmill_Trade_Manager(172, gasleft());
        uint256 nbBUSD = getEnergyValueBUSD() * nbEnergy;
emit GasLog_Windmill_Trade_Manager(173, gasleft());
...
emit GasLog_Windmill_Trade_Manager(175, gasleft());
        
        trade.setTradeId(trades.length);
emit GasLog_Windmill_Trade_Manager(177, gasleft());
        withdrawer.grantRole(withdrawer.TRADE_ROLE(), address(trade));
emit GasLog_Windmill_Trade_Manager(178, gasleft());
        
        TradeData memory data;
emit GasLog_Windmill_Trade_Manager(180, gasleft());
        data.owner = msg.sender;
emit GasLog_Windmill_Trade_Manager(181, gasleft());
        data.trade = trade;
emit GasLog_Windmill_Trade_Manager(182, gasleft());
        data.energy = nbEnergy;
emit GasLog_Windmill_Trade_Manager(183, gasleft());
        data.isActive = true;
emit GasLog_Windmill_Trade_Manager(184, gasleft());
        data.blockStart = block.number;
emit GasLog_Windmill_Trade_Manager(185, gasleft());
        data.percentPWRLockedNumerator =  nbBUSD * 1e20 / totalBUSD;
emit GasLog_Windmill_Trade_Manager(186, gasleft());
        
        
        TradeIdToOpenId[trades.length] = openTradeId.length;
emit GasLog_Windmill_Trade_Manager(189, gasleft());
        openTradeId.push(trades.length);
emit GasLog_Windmill_Trade_Manager(190, gasleft());
        trades.push(data);
emit GasLog_Windmill_Trade_Manager(191, gasleft());
        
        usedEnergy[msg.sender] += nbEnergy;
emit GasLog_Windmill_Trade_Manager(193, gasleft());
        
        fund.sendBUSDToTrade(trade, nbBUSD);
emit GasLog_Windmill_Trade_Manager(195, gasleft());
        
        return trades.length - 1;
emit GasLog_Windmill_Trade_Manager(197, gasleft());
    }
    
    function endTrade(uint256 tradeId) external{
emit GasLog_Windmill_Trade_Manager(200, gasleft());
        require(tradeId < trades.length, "Windmill_Trade_Manager: Trade not found.");
emit GasLog_Windmill_Trade_Manager(201, gasleft());
        
        TradeData storage data = trades[tradeId];
emit GasLog_Windmill_Trade_Manager(203, gasleft());
        
        require(data.owner == msg.sender, "Windmill_Trade_Manager: Only the owner can end the trade.");
emit GasLog_Windmill_Trade_Manager(205, gasleft());
        require(data.isActive, "Windmill_Trade_Manager: Trade already ended.");
emit GasLog_Windmill_Trade_Manager(206, gasleft());
        
        data.isActive = false;
emit GasLog_Windmill_Trade_Manager(208, gasleft());
        data.trade.endTrade();
emit GasLog_Windmill_Trade_Manager(209, gasleft());
        
        withdrawer.revokeRole(withdrawer.TRADE_ROLE(), address(data.trade));
emit GasLog_Windmill_Trade_Manager(211, gasleft());
        
        uint256 i = TradeIdToOpenId[tradeId];
emit GasLog_Windmill_Trade_Manager(213, gasleft());
        uint256 lastI = openTradeId.length - 1;
emit GasLog_Windmill_Trade_Manager(214, gasleft());
        if (lastI != i){
emit GasLog_Windmill_Trade_Manager(215, gasleft());
            uint256 lastTradeId = openTradeId[lastI];
emit GasLog_Windmill_Trade_Manager(216, gasleft());
            openTradeId[i] = lastTradeId;
emit GasLog_Windmill_Trade_Manager(217, gasleft());
            TradeIdToOpenId[lastTradeId] = i;
emit GasLog_Windmill_Trade_Manager(218, gasleft());
        }
        openTradeId.pop();
emit GasLog_Windmill_Trade_Manager(220, gasleft());
        
        if (usedEnergy[data.owner] < data.energy){
emit GasLog_Windmill_Trade_Manager(222, gasleft());
            usedEnergy[data.owner] = 0;
emit GasLog_Windmill_Trade_Manager(223, gasleft());
        }else{
emit GasLog_Windmill_Trade_Manager(224, gasleft());
            usedEnergy[data.owner] -= data.energy;
emit GasLog_Windmill_Trade_Manager(225, gasleft());
        }
    }
