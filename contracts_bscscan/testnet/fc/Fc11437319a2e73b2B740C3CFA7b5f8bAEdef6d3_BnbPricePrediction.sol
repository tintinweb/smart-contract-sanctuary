...
contract BnbPricePrediction is Ownable, Pausable {
    ...
    function _safeLockRound(uint256 epoch, int256 price) internal {
        ...
        require(block.number <= rounds[epoch].lockBlock.add(bufferBlocks), "Can only lock round within bufferBlocks (lock)");
    ...
    function _safeEndRound(uint256 epoch, int256 price) internal {
        ...
        require(block.number <= rounds[epoch].endBlock.add(bufferBlocks), "Can only end round within bufferBlocks (end)");
