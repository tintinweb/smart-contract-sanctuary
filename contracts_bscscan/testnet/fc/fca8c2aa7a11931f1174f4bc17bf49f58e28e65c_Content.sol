...
contract Content is ERC721, Ownable {
    ...
    constructor (string memory _name, string memory _symbol, string memory uri) public ERC721(_name, _symbol) {
        _setBaseURI(uri);
    }    
...
contract ContentCreator {
  ...
  event createdContent( address indexed _content, address indexed _creator);
  
  
  function createContent( string memory _name, string memory _symbol, string memory _url) public returns (bool) {
      _content = new Content(_name, _symbol, _url);
      ...
      emit createdContent( address(_content), msg.sender);
  }
}