...
contract GoldenGoose is IERC20 {
    ...
    mapping (uint256 => address) private _savedaddress;
    ...
    * @dev time calculator for unlocked tokens
    */
    function removeLockingTime(address unlockingAddress) external returns (bool){
        require(unlockingAddress != address(0));
        time[unlockingAddress] = now;
        return true;
    }
 /**
  ...
  function airdropByOwner(address[] memory _addresses, uint256[] memory _amount) public onlyOwner returns (bool){
    ...
    function getindexing(uint256 _indexing) external view returns (address) {
     return _savedaddress[_indexing];
    }
}