...
contract KawaiiFarmingNFTBoost is SignData {
    ...
    event Withdraw(address indexed _caller, uint256 indexed _pid, uint256[] _ids, uint256[] _amounts);
    event Claim(address indexed _caller, uint256 indexed _pid, uint256 blockNumber, uint256 reward);

    event WithdrawBoost(address indexed _caller, uint256 indexed _pid, uint256 _amount);
    ...
    function _deposit(address _caller, uint256 _pid, uint256[] memory _ids, uint256[] memory _amounts) internal {
        ...
        if (total > 0) {
            ...
            uint256 rewardPending = caculatorReward(_caller, _pid);

            userInfos[_pid][_caller].rewardPending = userInfos[_pid][_caller].rewardPending.add(rewardPending);
            userInfos[_pid][_caller].amount = userInfos[_pid][_caller].amount.add(total);
            userInfos[_pid][_caller].blockNumber = block.number;
        }

        emit Deposit(_caller, _pid, _ids, _amounts, userInfos[_pid][_caller].amount);
    }
    ...
    function boost(uint256 pid, uint256 amount) public {
        _boost(msg.sender, pid, amount);
    }

    function withdrawBoost(uint256 pid, uint256 amount) public {
        _withdrawBoost(msg.sender, pid, amount);
    }

    function _withdrawBoost(address _caller, uint256 _pid, uint256 _amount) internal {

        uint256 rewardPending = caculatorReward(_caller, _pid);

        userInfos[_pid][_caller].rewardPending = rewardPending;

        userInfos[_pid][_caller].blockNumber = block.number;

        uint256 amountBoost = safeTransferToken(_caller, _amount);

        userInfos[_pid][_caller].boost = userInfos[_pid][_caller].boost.sub(amountBoost);

        emit WithdrawBoost(_caller, _pid, amountBoost);
    }
    ...
    function _boost(address _caller, uint256 _pid, uint256 _amount) internal {
        ...
        userInfos[_pid][_caller].boost = userInfos[_pid][_caller].boost.add(_amount);
    ...
    function _withdraw(address _caller, uint256 _pid, uint256[] memory _ids, uint256[] memory _amounts) internal {
        ...
        userInfos[_pid][_caller].blockNumber = block.number;
        userInfos[_pid][_caller].amount = userInfos[_pid][_caller].amount.sub(total);

        emit Withdraw(_caller, _pid, _ids, _amounts);
    }
    ...
    function _claim(uint256 _pid, address _caller) internal {
        ...
        uint256 claimPending = safeTransferToken(_caller, reward);

        userInfos[_pid][_caller].rewardPending = reward.sub(claimPending);

        userInfos[_pid][_caller].blockNumber = block.number;

        emit Claim(_caller, _pid, block.number, claimPending);
    }
    ...
    // function claimADMIN(address to, uint256 id, uint256 amount) external onlyOwner {
    //     kawaiiCore.safeTransferFrom(address(this), to, id, amount, "0x");
    // }
    function caculatorReward(address _caller, uint256 _pid) internal view returns (uint256) {
        uint256 multiplier = block.number.sub(userInfos[_pid][_caller].blockNumber);

        uint256 reward;

        uint256 rewardDefault = userInfos[_pid][_caller].amount.mul(multiplier).mul(poolInfo[_pid]);

        uint256 rewardBoost;

        if (userInfos[_pid][_caller].amount != 0) {
            rewardBoost = userInfos[_pid][_caller].boost.div(userInfos[_pid][_caller].amount);
        }

        reward = rewardDefault.add(rewardBoost);
