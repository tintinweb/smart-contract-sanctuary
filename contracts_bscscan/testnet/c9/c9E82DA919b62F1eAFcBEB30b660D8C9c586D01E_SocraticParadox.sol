...
/**  
    ...
    SCOPX  - SCOP Community                                                                                                    
   Socratic Paradox (SCOP) Version 2.0
   
   Total Supply:      1,000,000,000,000,000.00
   Burned Supply:       900,000,000,000,000.00
   Circulating Supply:  100,000,000,000,000.00
    
   Two token features:
   6% fee autoswap to BNB and add to the liquidity pool to locked forever when selling
   2% of each transaction is converted to BNB and used for marketing
   2% fee auto swap to BNB and distribute to all holders
   
   This SCOP token created and Govern by SCOP Community.  
   Code edited by @LemanCrypto	
   KitajagaKita AllahJagaKita
...
contract SocraticParadox is Context, IBEP20, Ownable {
    ...
    string private _name = "SocraticParadox";
    string private _symbol = "SCOP";
    ...
    uint256 private _previousLiquidityFee = taxFee;
   
    uint256 public marketingFee    = 2;
    uint256 private _previousMarketingFee = taxFee;
	

	// This is the total fees for what should be sold into BNB.
	// (Marketing + Liquidity+ Tax).
    uint256 private totalLiqFee = liquidityFee.add(marketingFee).add(taxFee);
    ...
    constructor (address payable marketingWallet, address pancakeSwapRouter) {
		...
		// BSC testnet2: 0x9ac64cc6e4415144c455bd8e4837fea55603e5c3
    ...
    function setLiquidityFeePercent(uint256 LiquidityFee) external onlyOwner() {
        ...
        totalLiqFee = liquidityFee.add(marketingFee).add(taxFee);
    }
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        require(maxTxPercent > 0.000, "% must be greater than 0.000");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 ethForTax = balanceDiff.mul(taxFee).div(totalLiqFee);
        uint256 ethForLiquidity = balanceDiff.sub(ethForMarketing).sub(ethForTax);
