...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 piece = contractTokenBalance.div(totalFees); //
        uint256 tokensForLiquidity =  piece.mul(liquidityFee).div(4); //
        uint256 swapPortion = contractTokenBalance.sub(tokensForLiquidity); //
    ...
    //    uint256 toSwap = contractTokenBalance.sub(tokensToAddLiquidityWith);
        ...
        // swap tokens for ETH //update by me
        swapTokensForBnb(swapPortion, address(this)); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        uint256 deltaBalance = address(this).balance-initialBalance;

        // take worthy amount bnb to add liquidity
        // worthyBNB = deltaBalance * liquidity/(2totalFees - liquidityFee)
        uint256 bnbToAddLiquidityWith = deltaBalance.mul(tokensForLiquidity).div(totalFees);
        
        // add liquidity to uniswap
        addLiquidity(tokensForLiquidity, bnbToAddLiquidityWith);
        ...
        if(success) {
   	 		emit SendDividends(swapPortion-tokensToAddLiquidityWith, dividends);
        }
    ...
    function buyBackAndBurn(uint256 amount) private {
      ...
      // make the swap
        ...
        _burn(address(this), swappedBalance);
