...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function add(
        ...
        _add(_allocPoint, _lpToken, _taxWithdraw, _taxWithdrawBeforeLock,
            _withdrawLockPeriod, _lock, _depositFee, _withUpdate, _cake_pid, _harvestFee);
    }
    ...
    function setReserveFee(uint16 val) external onlyOwner {
        reserveFee = val;
    }
    ...
    function adminSetReserveAddr(address payable _addr) external onlyOwner {
        emit SetReserveAddr(reserveAddress, _addr);
        reserveAddress = _addr;
    }

    function adminSetTaxLpAddress(address payable _addr) external onlyOwner {
        taxLpAddress = _addr;
    }

    function adminSetTaxAddr(address payable _addr) external onlyOwner {
    ...
    function adminSetTokenTaxAddr(address payable _taxTo) external onlyOwner {
    ...
    function setHarvestProcessProfitMode(uint16 mode) external onlyOwner {
    ...
    function setBlaclisted(address addr, bool status) external operator {
    ...
    function adminProcessReserve() external operator {
    ...
    function adminEmergencyWithdraw(uint256 _pid) external operator {
        mc.emergencyWithdraw(poolInfo[_pid].cake_pid);
    }

    function panicAll() external operator {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            panic(pid);
        }
    ...
    function panic(uint256 pid) public operator {
    ...
    function init_testnet() public operator {
        address _router = address(0xf946634f04aa0eD1b935C8B876a0FD535F993D43);
        IUniswapV2Router02 router = IUniswapV2Router02(_router);
        IUniswapV2Factory factory = IUniswapV2Factory(router.factory());
        address busd = address(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee);
        address eth = address(0xd66c6B4F0be8CE5b39D52E0Fd1344c389929B378);

        address token_wbnb = factory.getPair(router.WETH(), address(token));
        if (token_wbnb == address(0x0))
            token_wbnb = factory.createPair(router.WETH(), address(token));
        require(token_wbnb != address(0x0), "!token_wbnb");

        address token_busd = factory.getPair(address(token), busd);
        if (token_busd == address(0x0))
            token_busd = factory.createPair(address(token), busd);
        require(token_busd != address(0x0), "!token_busd");

        address wbnb_busd = factory.getPair(router.WETH(), busd);
        address wbnb_eth = factory.getPair(router.WETH(), eth);

        _add(1000, token_wbnb, 500, 2500, 604800, 604800, 500, true, 0, 500);
        _add(1000, token_busd, 500, 2500, 604800, 604800, 500, true, 0, 500);

        _add(500, wbnb_busd, 0, 500, 259200, 259200, 400, true, 1, 0);
        _add(200, wbnb_eth, 0, 500, 259200, 259200, 400, true, 2, 0);
        _add(200, router.WETH(), 0, 200, 259200, 259200, 400, true, 3, 0);
        _add(200, busd, 0, 500, 259200, 259200, 400, true, 4, 0);
        _add(200, eth, 0, 500, 259200, 259200, 400, true, 0, 0);
