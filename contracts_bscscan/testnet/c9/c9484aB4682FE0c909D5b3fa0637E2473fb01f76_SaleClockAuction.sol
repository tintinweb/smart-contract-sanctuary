...
contract ClockAuctionBase {
    ...
    function _addAuction(uint256 _tokenId, Auction memory _auction) internal {
        require(_auction.duration >= 1 minutes, "TIME_TOO_SHORT");
        ...
        returns (uint256)
    {
        ...
        if (price > 0) {
            ...
            require(IERC20(tokenAddress).transfer(seller, sellerProceeds));
        }

        uint256 bidExcess = _bidAmount - price;

        require(IERC20(tokenAddress).transfer(msg.sender, bidExcess));
...
contract ClockAuction is Pausable, ClockAuctionBase, IClockAuction {
    ...
    ///  between 0-10,000.
    constructor(address _tokenAdress , address _nftAddress, uint256 _cut) public {
        require(_cut <= 10000 ,"CUT_BIGGER_10000");
        ownerCut = _cut;

        nonFungibleContract = IERC721(_nftAddress);
        // require(nonFungibleContract.supportsInterface(InterfaceSignature_ERC721), "ERC721_INVALID");
    ...
    function bid(uint256 _tokenId, uint256 _amount)
        ...
        whenNotPaused
    {
        // require(IERC20(tokenAddress).transfer(address(this), _amount));
...
contract SaleClockAuction is ClockAuction {
    ...
    constructor(address _tokenAddress, address _nftAddr, uint256 _cut) public
        ClockAuction(_tokenAddress,_nftAddr, _cut) {}
