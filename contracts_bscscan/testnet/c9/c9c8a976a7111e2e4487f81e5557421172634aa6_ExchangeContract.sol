...
contract Sale{
    ...
    function _acceptBId(address token,address from, address admin, uint256 amount, uint256 tokenId) internal{
        ...
        uint originalamount = or_am;
        payforRoyalty(originalamount, tokenId);
    }
    ...
    function _saleToken(address payable from, address payable admin,uint256 tokenId, uint256 amount) internal{
        ...
        uint256 ser=fee.div(2);
        uint256 or_am = amount.sub(ser);
        uint256 roy = or_am.mul(totalroyalty).div(100);
        uint256 serfee = ser.add(roy);
        uint256 netamount = or_am.sub(serfee);
...
contract ExchangeContract is Sale{
    ...
    function mint(address token ,string memory tokenuri, uint256 value, uint256 tokenId, uint256 royal, 
    uint256 _type, uint256 supply, address[] memory royaltyaddr, uint[] memory royaltypercentage) public{
    ...
    function changeroyaltypercentage(uint256 percentage) external {
    ...
    function burn(address from, address admin, uint256 tokenId, address token, uint256 _type, uint256 NOFToken ) public{
