...
contract PLTLottery is ReentrancyGuard, IPLTLottery, Ownable {
    ...
    uint256 public pltNFTFreeTicketId;
     ...
     */
    function buyTickets(uint256 _lotteryId, uint32[] calldata _ticketNumbers, bool _useFreeTickets)
        external
        override
        notContract
        nonReentrant {
        ...
        if(amountNftToTransfer > 0) {
            pltNFT.safeTransferFrom(address(this), address(msg.sender), pltNFTFreeTicketId, amountNftToTransfer, "");
        }
    ...
    function claimTickets(
        ...
        if(rewardInPLTNftToTransfer > 0) {
            ...
            pltNFT.safeTransferFrom(address(this), address(msg.sender), pltNFTFreeTicketId, rewardInPLTNftToTransfer, "");
        }
     ...
     * @param _pltNFT: address Poolloto NFT
     */
     ...
     * @param _ticketRewardsBreakdown: breakdown of rewards free tickets 
    ...
    function recoverWrongNFT(address _tokenAddress, uint256 _id, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(pltNFT), "Cannot be poolloto NFT");

        IERC1155(_tokenAddress).safeTransferFrom(address(this), address(msg.sender), _id, _tokenAmount, "");
    ...
    function _calculateTotalPriceForBulkTickets(
        ...
        if(_useFreeTickets) {
            ...
            if(freeTicketsBalance >= _numberTickets) {
                return (0, (freeTicketsBalance - _numberTickets));
            } else {
