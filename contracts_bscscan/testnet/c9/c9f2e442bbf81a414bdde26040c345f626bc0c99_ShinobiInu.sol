...
contract ShinobiInu is Context, IERC20, Ownable {
    ...
    string private constant _name = "Shinobi Inu2";
    string private constant _symbol = "SHIN2";
    ...
    address public _marketWalletAddress;
    ...
    uint256 public marketFeesCollected;
    uint256 public numTokensForMarketSell = 2e5 * 10**9;
    ...
    uint256 public numTokensSellToAddToLiquidity = 4e5 * 10**9;
    
    // Events
    event SwapAndLiquifyEnabledUpdated(bool enabled);
    event SwapAndLiquify(uint256 tokensSwapped,uint256 ethReceived,uint256 tokensIntoLiquidity);
    event numTokensSellToAddToLiquidityUpdated(uint256 numTokensSellToAddToLiquidity);
    event numTokensForMarketSellUpdated(uint256 numTokensForMarketSell);
    ...
    constructor(address devWallet, address liqWallet, address marketWallet) {

        //testnet:
        _setFeeToken(0x0000000000000000000000000000000000000000); 
        _setRouter(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); 


        //_setFeeToken(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);   // BUSD Mainnet
        //_setRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);     // Pancakeswap Router v2 Mainnet

        _rOwned[owner()] = _rTotal;
        _devWalletAddress = devWallet;
        _liqWalletAddress = liqWallet;
        _marketWalletAddress = marketWallet;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarket, uint256 tdev) = _getValues(tAmount);
    ...
    function setBuyTaxFeePercent(uint256 taxFee) external onlyOwner() {
        _buyTaxFee = taxFee;
        require(_buyTaxFee + _buyDevFee + _buyLiquidityFee + _buyMarketFee <= 200, "Total fees exceed 20%");
    }
    ...
    function setBuyMarketFeePercent(uint256 marketFee) external onlyOwner() {
    ...
    function setSellTaxFeePercent(uint256 taxFee) external onlyOwner() {
        _sellTaxFee = taxFee;
        require(_sellTaxFee + _sellDevFee + _sellLiquidityFee + _sellMarketFee <= 200, "Total fees exceed 20%");
    }
    ...
    function setSellMarketFeePercent(uint256 marketFee) external onlyOwner() {
    ...
    function setMarketingWalletAddress(address _address) external onlyOwner() {
        ...
        _marketWalletAddress = _address;
        emit SetMarketWalletAddress(_address);
    }
    ...
    function setNumTokensForMarketSell(uint256 _amount) external onlyOwner() {
        require(_amount > 0);
        numTokensForMarketSell = _amount;
        emit numTokensForMarketSellUpdated(_amount);
    }
    ...
    function _setFeeToken(address _address) internal {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tValues.tFee, tValues.tLiquidity, tValues.tMarket, tValues.tdev, _getRate());
        return (rAmount, rTransferAmount, rFee, tValues.tTransferAmount, tValues.tFee, tValues.tLiquidity, tValues.tMarket, tValues.tdev);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (TValues memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tMarket).sub(tdev);
        return TValues(tTransferAmount, tFee, tLiquidity, tMarket, tdev);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarket, uint256 tdev, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rMarket = tMarket.mul(currentRate);
        uint256 rdev = tdev.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rMarket).sub(rdev);
    ...
    function _transfer(
        ...
        uint256 contractLiquifyTokenBalance = contractTokenBalance.sub(marketFeesCollected);
        
        if(contractLiquifyTokenBalance >= _maxTxAmount) {
            contractLiquifyTokenBalance = _maxTxAmount;
        }

        if(!inSwapAndLiquify && _isPair[to] && !_isExcludedFromFee[from]) {
            if(swapAndLiquifyEnabled && (contractLiquifyTokenBalance >= numTokensSellToAddToLiquidity)) {
                swapAndLiquify(numTokensSellToAddToLiquidity);
            } else if(marketFeesCollected >= numTokensForMarketSell) {
                swapForFee();
            }
        }
    ...
    function swapForFee() private lockTheSwap {
        if(balanceOf(address(this)) < numTokensForMarketSell) {
            return;
        }
        swapTokensForTokens(numTokensForMarketSell, _marketWalletAddress);
        emit SwapForFee(numTokensForMarketSell);
        marketFeesCollected = marketFeesCollected.sub(numTokensForMarketSell);
    }
    ...
    function setMarketFeesCollected(uint256 _marketFeesCollected) external onlyOwner() {
        marketFeesCollected = _marketFeesCollected;
    }
