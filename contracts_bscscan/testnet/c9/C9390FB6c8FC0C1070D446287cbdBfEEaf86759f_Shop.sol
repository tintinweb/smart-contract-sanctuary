...
contract Shop is IShop, PancakeSwapHelper, EmergencyWithdrawable {
    ...
    function buyItem(uint256 id, uint256 quantity, uint256 param) external payable notUnauthorizedContract nonReentrant notPaused {
        ShopItem storage item = items[id];
    ...
    function buyItemWithXLD(uint256 id, uint256 quantity, uint256 amount, uint256 param) external notUnauthorizedContract nonReentrant notPaused {
        ShopItem storage item = items[id];
        require(id > 0 && item.id == id, "Shop: Item does not exist");
