...
contract STAKEtest {
    ...
    function stake(uint256 amount, uint256 timeperiod) public {
        ...
        uint256 RLQVAL = getPriceinUSD();
        uint256 rewardtokenPrice = (amount.mul(RLQVAL)).div(1e9);
    ...
    function harvest(uint256 index) public {
        ...
        stakersRecord[msg.sender][index].lastharvesttime = stakersRecord[msg.sender][index].lastharvesttime.add(rewardTillNow);
    ...
    function getPriceinUSD() public view returns (uint256){
        ...
        address BUSD_WBNB = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; //BUSD_WBNB pancake pool address
        ...
        address RLQ_WBNB = 0xE2bFcCae469b43b6040B202cbE4544F25bA280a0; // Realliq_WBNB pancake pool address
        IERC20 RLQTOKEN = IERC20(0x1476A75A2226aacbbf5d4A7907e5eBE6c9cf75d9); // Realliq Token address 

        uint256 WBNBSUPPLYINRLQ_WBNB =(WBNBTOKEN.balanceOf(RLQ_WBNB));
        uint256 RLQSUPPLYINRLQ_WBNB = (RLQTOKEN.balanceOf(RLQ_WBNB));

        uint256 RLQUSDVAL = (((WBNBSUPPLYINRLQ_WBNB.mul(1e9)).div((RLQSUPPLYINRLQ_WBNB))).mul(BNBPRICE)).div(1e18);
        return RLQUSDVAL;
    }

    function realtimeRewardPerBlock(address user, uint256 blockno) public view returns (uint256) {
        uint256 ret;
            ...
            if (
                ...
                uint256 val;
                uint256 tempharvesttime;
                if(stakersRecord[user][blockno].lastharvesttime == 0){
                    tempharvesttime = stakersRecord[user][blockno].staketime;
                }
                val = block.timestamp - tempharvesttime;
                val = val.mul(stakersRecord[user][blockno].persecondreward);
                if (val < stakersRecord[user][blockno].reward) {
                    ret += val;
                } else {
                    ret += stakersRecord[user][blockno].reward;
                }
            }
        return ret;
    }
    ...
    function realtimeReward(address user) public view returns (uint256) {
        ...
        for (uint256 i; i < Stakers[user].stakeCount; i++) {
            if (
                ...
                uint256 val;
                uint256 tempharvesttime;
                if(stakersRecord[user][i].lastharvesttime == 0){
                    tempharvesttime = stakersRecord[user][i].staketime;
                }
                val = block.timestamp - tempharvesttime;
