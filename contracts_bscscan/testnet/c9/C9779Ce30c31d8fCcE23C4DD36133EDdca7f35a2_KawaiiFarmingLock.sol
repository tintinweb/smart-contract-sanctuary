...
contract KawaiiFarmingLock is SignData, Ownable, Pausable {
    using SafeMath for uint256;
    ...
    function init(address _kawaiiToken) public {
    ...
    function withdrawAll(address _caller) public whenNotPaused {
        UserData storage userData = userDatas[_caller];
        EpochData memory epochUser = epochConfigs[userData.epoch];

        uint256 reward;

        if (userData.amount != 0 && epochUser.end < block.timestamp) {
            (reward,) = pending(_caller);

            reward = reward.add(userData.amount);
        ...
        uint256 totalAmount = reward.add(userData.rewardPending);

        require(totalAmount != 0, "reward is zero");
