...
contract GEMUNILending is PermissionGroup {
    ...
    function requestLending(uint passId, bytes memory sig) external {
        ...
        uint mortgagePrice = geniPassSale.getPricePass(passId, 0, IGENIPass.PriceType.GENI) * mortgagePercent / 100;
    ...
    function getInterestPayable(uint passId, address user) public returns (uint){
        ...
        interestPayable = geniPassSale.getPricePass(passId, 0, IGENIPass.PriceType.GENI) * BASE_APY / 100 * (block.timestamp - lendingInf.startLending) / ONE_YEAR_IN_SECONDS;
