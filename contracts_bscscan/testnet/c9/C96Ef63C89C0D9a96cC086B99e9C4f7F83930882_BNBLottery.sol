 ...
 */
contract BNBLottery is ReentrancyGuard, IBNBLottery, Ownable {
    ...
    uint256 public maxPriceTicketInBNB = 10 ether;
    uint256 public minPriceTicketInBNB = 0.001 ether;
     ...
     */
    constructor(address _randomGeneratorAddress, address _treasuryAddress) {
        ...
        nonReentrant
    {
        ...
        require(msg.value == amountBNBToTransfer, "Insufficient BNB");
     ...
     */
    function injectFunds(uint256 _lotteryId) external override payable onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        if(_tokenAddress == address(0)){
            payable(treasuryAddress).transfer(_tokenAmount);
        }
        else {
            IERC20(_tokenAddress).safeTransfer(treasuryAddress, _tokenAmount);
        }
