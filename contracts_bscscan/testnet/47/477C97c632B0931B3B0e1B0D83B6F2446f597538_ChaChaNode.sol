...
contract ChaChaNode is  Ownable {
    ...
    uint256 public batchAmount = 2;
    bool public publicSaleActivated = false;
    bool public paymentTokenActivated = true;
    address[] public tokenRecipients = [0x5B38Da6a701c568545dCfcB03FcB875f56beddC4,0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2,0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db];
    ...
    @dev setAddress
     */
    function setAddress(address _payToken,address _treasury) public onlyOwner {
   ...
   /**
    ...
    function setTokenRecipients(address[] memory _tokenRecipients)  onlyOwner external {
    ...
    function buyNode() external payable {
        ...
        if(totalPublicSale.mod(batchAmount)==0&&tokenRecipients.length>0){
            ...
            if(totalPublicSale.div(batchAmount)<=tokenRecipients.length){
                recipientIndex = totalPublicSale.div(batchAmount).sub(1);
                IERC20(payToken).transfer(tokenRecipients[recipientIndex], IERC20(payToken).balanceOf(address(this)));
            }else{
                recipientIndex = totalPublicSale.sub(batchAmount.mul(tokenRecipients.length)).div(batchAmount).sub(1);
                IERC20(payToken).transfer(tokenRecipients[recipientIndex], IERC20(payToken).balanceOf(address(this)));
            }
        }
