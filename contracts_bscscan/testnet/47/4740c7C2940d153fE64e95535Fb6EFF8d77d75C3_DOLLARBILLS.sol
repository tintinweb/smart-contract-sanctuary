...
contract DOLLARBILLS is ERC20, Ownable {
    ...
    bool private sendUsdtInTx = true;
    bool private sendBusdInTx = true;

    BusdDividendTracker private busdDividendTracker;
    UsdtDividendTracker private usdtDividendTracker;
    ...
    uint256 private busdDividendPriority = 80;
    ...
    function clearStuckBalance(uint256 amountPercentage) internal onlyOwner {
    ...
    function updateUsdtDividendToken(address _newContract) internal onlyOwner {
        usdtDividendToken = _newContract;
        usdtDividendTracker.setDividendTokenAddress(_newContract);
    }

    function updateMinUsdtBeforeSendDividend(uint256 _newAmount) internal onlyOwner {
        usdtDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }

    function updateBusdDividendToken(address _newContract) internal onlyOwner {
        busdDividendToken = _newContract;
        busdDividendTracker.setDividendTokenAddress(_newContract);
    }

    function updateMinBusdBeforeSendDividend(uint256 _newAmount) internal onlyOwner {
        busdDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }

    function getMinUsdtBeforeSendDividend() internal view returns (uint256) {
        return usdtDividendTracker.minTokenBeforeSendDividend();
    }

    function getMinBusdBeforeSendDividend() internal view returns (uint256) {
        return busdDividendTracker.minTokenBeforeSendDividend();
    }

    function setSendUsdtInTx(bool _newStatus) internal onlyOwner {
        sendUsdtInTx = _newStatus;
    }

    function setSendBusdInTx(bool _newStatus) internal onlyOwner {
        sendBusdInTx = _newStatus;
    }

    function setBusdDividendPriority(uint256 _newAmount) internal onlyOwner {
    ...
    function withdrawableBusdDividendOf(address account) internal view returns(uint256) {
        return busdDividendTracker.withdrawableDividendOf(account);
    }

    function withdrawableUsdtDividendOf(address account) internal view returns(uint256) {
        return usdtDividendTracker.withdrawableDividendOf(account);
    }

    function busdDividendTokenBalanceOf(address account) internal view returns (uint256) {
        return busdDividendTracker.balanceOf(account);
    }

    function usdtDividendTokenBalanceOf(address account) internal view returns (uint256) {
