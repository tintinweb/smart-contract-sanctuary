...
import 'fuzzyswap/contracts/libraries/SafeCast.sol';
    ...
    SelfPermit
{
    ...
    constructor(address _factory, address _WBNB, address _poolDeployer) PeripheryImmutableState(_factory, _WBNB, _poolDeployer) {}

    /// @dev Returns the pool for the given token pair and fee. The pool contract may or may not exist.
    function getPool(
        address tokenA,
        address tokenB
    ) private view returns (IFuzzyswapPool) {
        return IFuzzyswapPool(PoolAddress.computeAddress(poolDeployer, PoolAddress.getPoolKey(tokenA, tokenB)));
    }
    ...
    /// @inheritdoc IFuzzyswapSwapCallback
    ...
    function exactInputSingleSupportingFeeOnTransferTokens(ExactInputSingleParams calldata params)
        ...
        returns (uint256 amountOut)
    {
        SwapCallbackData memory data = SwapCallbackData({path: abi.encodePacked(params.tokenIn, params.tokenOut), payer: msg.sender});
        address recipient = params.recipient == address(0) ? address(this) : params.recipient;

        bool zeroForOne = params.tokenIn < params.tokenOut;

        (int256 amount0, int256 amount1) =
            getPool(params.tokenIn, params.tokenOut).swapSupportingFeeOnInputTokens(
                msg.sender,
                params.recipient,
                zeroForOne,
...
import 'fuzzyswap/contracts/interfaces/callback/IFuzzyswapSwapCallback.sol';

/// @title Router token swapping functionality
/// @notice Functions for swapping tokens via Uniswap V3
interface ISwapRouter is IFuzzyswapSwapCallback {
...
import 'fuzzyswap/contracts/libraries/LowGasSafeMath.sol';
...
abstract contract PeripheryPaymentsWithFee is PeripheryPayments, IPeripheryPaymentsWithFee {
    ...
    function unwrapWBNBWithFee(
        ...
        if (balanceWBNB > 0) {
            ...
            uint256 feeAmount = balanceWBNB.mul(feeBips) / 10_000;
            if (feeAmount > 0) TransferHelper.safeTransferBNB(feeRecipient, feeAmount);
            TransferHelper.safeTransferBNB(recipient, balanceWBNB - feeAmount);
        }
    }
...
library PoolAddress {
    ...
    bytes32 internal constant POOL_INIT_CODE_HASH = 0x4ff375b3bc1ae628279adc40a3b42180357b6db48308c5d58d3bde87c909b56a;
...
interface IPeripheryPaymentsWithFee is IPeripheryPayments {
    /// @notice Unwraps the contract's WBNB balance and sends it to recipient as BNB, with a percentage between
