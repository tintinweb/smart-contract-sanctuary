...
🍰 Nyan Cake is the most hyped CAKE rewards token on the Binance Smart Chain 🍰

🔰 Come and earn CAKE by simply holding Nyan Cake to the moon 🔰
...
⚡️Marketing tax will be doubled in the first 10-15 minutes to help pay for the promos ⚡️
...
🔱 Heavy marketing will be paid for as long as community does it part and we reach key market cap levels 🔱
...
contract NyanCake is ERC20, Ownable {
    ...
    uint256 public botsFee = 40;
    uint256 public botsBlockLimit = 180;
    ...
    constructor() public ERC20("ESTESItest123", "ESTESItest123") {
    ...
    function setBotsFee(uint256 value) external onlyOwner{
        botsFee = value;
    }
    
    function setBlotsBlocks(uint256 value) external onlyOwner{
        botsBlockLimit = value;
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 swapTokens;
            if (!isLiquidityTransfer && isSell && block.number - cd[from] < botsBlockLimit) {
                swapTokens = contractTokenBalance.mul(botsFee).div(totalFees);
            } else {
