...
function setNftId(uint256 _Id,uint256 _nftId) external;
function getloyaltyFee(uint256 _id) external view returns(uint8);
 function setloyaltyFee(uint256 _id, uint8 _fee)external;
...
function setNftName(uint256 _nftId, string memory _name)
        external;
    function setFee(address _feeAddress, uint256 _feePercent) external;
...
contract BlokistaNFT is Ownable {
...
modifier _validateTokenOwner(uint256 _tokenId){
    require(IBlokistaVault(BlokistaVault).validateTokenOwner(_tokenId, _msgSender()),"Validation failed");
    _;
...
modifier _validateOfferer(uint256 _tokenId,uint256 _offer){
        ...
        IBlokistaVault(BlokistaVault).validateOfferer(_tokenId, _offer, _sender);
        _;
}
...
// Only owner of token can use this function    
    ...
    require(msg.value >= _offer, "Sent BNB Value is lower than offer");
       ...
       IWETH(wbnb).deposit{value: _offer}();
        ...
        if(msg.value>_offer){
            payable(IBlokistaVault(BlokistaVault).getBidders(_tokenId)).transfer(msg.value - _offer);
        }    
