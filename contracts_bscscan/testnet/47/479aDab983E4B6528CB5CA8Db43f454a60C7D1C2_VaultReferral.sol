...
contract VaultReferral is IVaultReferral, Ownable {
    ...
    address public UNA = 0x7477Db287A40e90097109EAbAfCf9f5dC8380D64;
    ...
    function multiLevelGift(address _referrer, uint256 _amount,uint256 _count) internal {
        if(_count <= levelGift){
            recordTokenRewards(_referrer,_amount.mul(levelGiftRatio[_count]).div(100));
            _count += 1;
    ...
    function getLuckyTicket(address _user, address _referrer) public {
    ...
    function getReferralTicket(address _user, address _referrer) public {
        require(_user != _referrer,'Referral address invalid');
    ...
    function randomizedIndex(uint256 _externalRandomNumber) public {
    ...
    /// @dev tested to record BNB rewards
    ...
    /// @dev Tested to record token rewards
    ...
    /// @dev tested return countdown time left for next Check in tested
    ...
    /// @dev tested return countdown time left for next balance check in tested
    ...
    /// @dev tested 
    // Owner can withdraw tokens that are sent here by mistake
