...
contract LevelStorage is Ownable {
    ...
    function addFighter(uint256 _level, uint256 _monsterId) external onlyAuth returns(bool) {
       return EnumerableSet.add(levelFighters[_level], _monsterId);
    }

    function removeFighter(uint256 _level, uint256 _monsterId) external onlyAuth returns (bool){
       return EnumerableSet.remove(levelFighters[_level], _monsterId);
    }
    ...
    function getRandomMonsterFromLevel(uint256 _level) external onlyAuth returns(uint256 _monsterId, bool _isLevelRequired){
        bool _levelReq = true;
        if(EnumerableSet.length(levelFighters[_level]) <= 1){
            _level = _level.sub(1);
            _levelReq = false;
        }
        ...
        index = index.mod(EnumerableSet.length(levelFighters[_level]));
        return (EnumerableSet.at(levelFighters[_level],index),_levelReq);
    }
