...
constructor(
    ...
    function add(uint256 _bidAmount, IERC20 bidToken,  uint _bidTimeOut) public onlyAdmin {
        uint bidTimeOut = block.timestamp.add(_bidTimeOut);
        CreateBid storage _newStorage = store;
    ...
    function getChainlinkRandomNumber(uint256 _pid) public returns (bytes32 _requestID){
        bytes32 reqID = requestRandomness(keyHash, fee);
        allBiddersInEachBid[_pid][reqID] = allBiddersInEachBid[_pid][reqID];
    ...
    function fulfillRandomness( bytes32 requestId, uint256 randomness) internal override {
        ...
        // CreateBid storage bid = createBid[_pid];
        // address[] memory transaction = allBiddersInEachBid[0][requestId];
        // uint256 _amount = bid.totalBidding.mul(10E18).div(100E18);
        // address luckyWinner = allBiddersInEachBid[randomness][requestId];
        // bid.transfer(luckyWinner, _amount);
        // randomness.mod(projBidders[0].length)
    ...
    function outChainRandom(uint256 _pid, uint256 _number) public onlyOwner returns(bool) {
        CreateBid storage bid = createBid[_pid];
        uint256 _amount = bid.totalBidding.mul(10E18).div(100E18);
        address _luckWinner = projBidders[_pid][_number];
        bid.token.transfer(_luckWinner, _amount);
    ...
    // function listAllBidders() public view returns (address[] memory) {
    //     return tBidders;
    // }
    ...
    function getBalance(uint _pid) public view returns(uint) {
        CreateBid storage bid = createBid[_pid];
        return bid.token.balanceOf(address(this));
    }
    
    function WBNBSafeWithdraw(uint256 _pid, address _to, uint256 _amount) onlyOwner public {
        CreateBid storage bid = createBid[_pid];
    ...
    function sendLink(address _to, uint256 _amount) public onlyOwner returns(bool) {
    ...
    function setDev( address _devAddress) public onlyOwner () {
