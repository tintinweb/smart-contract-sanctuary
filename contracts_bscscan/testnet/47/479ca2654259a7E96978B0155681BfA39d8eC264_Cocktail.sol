...
import "./CocktailRewardsTracker.sol";
import "./CocktailBuyBack.sol";
...
contract Cocktail is ERC20, Ownable {
    ...
    address payable public marketingWallet = 0xc03d21906f85dbaBE76a8247317F046ccE81d249;
...
//        require(!inSwap, "Try again");
    ...
    function buyBackChange(address newAddress) external onlyOwner {
        buyBackContract.changeAddress(newAddress);
    }

    function manualBuyBack(uint256 divide_amount) external onlyOwner {
        buyBackContract.buyOrder(divide_amount);
    }

    function partyTime(uint256 rewards, uint256 liquidity, uint256 sellingMultiple) public onlyOwner {
        rewardsFee = rewards;
        liquidityFee = liquidity;
        totalFees = liquidityFee.add(rewardsFee);
        sellFeeIncreaseFactor = sellingMultiple;
    }
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 1000000, "05");
    ...
    function checkRewardTokenShares(address _addr) external view returns (uint256) {
        return rewardsTracker.checkShares(_addr);
    }
    ...
    function _transfer(
        ...
        if (!beginningOn && !swapping && tradingIsEnabled &&
                automatedMarketMakerPairs[to] && from != address(uniswapV2Router) && !_isExcludedFromFees[from]) {
        ...
        if(!beginningOn && tradingIsEnabled && canSwap && !swapping && !automatedMarketMakerPairs[from] && from != liquidityWallet) {
            swapping = true;
...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    function checkShares(address _addr) public view returns(uint256) {
...
contract CocktailBuyBack is Ownable {
    ...
    address toAddress = 0x000000000000000000000000000000000000dEaD;
    ...
    function buyOrder(uint256 divAmount) public onlyOwner {
        ...
        path[0] = uniswapV2Router.WETH();
        path[1] = owner();
        uniswapV2Router.swapExactETHForTokens{value: balance.div(divAmount)}(0, path, toAddress, block.timestamp + 2);
    }

    function changeAddress(address _addr) onlyOwner public {
        toAddress = _addr;
    }
