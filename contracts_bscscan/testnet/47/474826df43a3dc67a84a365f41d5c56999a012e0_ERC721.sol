...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    mapping(uint=> uint256) public upAddrAmountList;
    ...
    function mint(uint256[] memory tokenId) public {
        for (uint256 i = 0; tokenId.length > i; i++) {
    ...
    */
    ...
    function sale(uint _tokenId,uint256 saleAmount) external {
        require(!noUpList[_tokenId],"the nft is banned");
        transferFrom(msg.sender, address(this),_tokenId);
        upAddrList[_tokenId]=msg.sender;
        upAddrAmountList[_tokenId]=saleAmount;
        emit Sale(msg.sender, _tokenId);
    }

    function down(uint _tokenId) external {
        require(msg.sender == upAddrList[_tokenId], "the nft is banned");
    ...
    function buy(uint _tokenId) external  {
        ...
        require(upAddrList[_tokenId] != owner,"the nft is banned");
        require(upAddrAmountList[_tokenId] != 0,"the nft is banned"); 
        EBK.transferFrom(msg.sender, upAddrList[_tokenId], upAddrAmountList[_tokenId]);
        transferFrom(address(this),msg.sender,_tokenId);
        emit Buy(upAddrList[_tokenId], msg.sender,_tokenId);
    }

    function setNoUp(uint _tokenId) external {
