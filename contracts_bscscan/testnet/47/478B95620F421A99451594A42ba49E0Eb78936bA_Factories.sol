...
library BBLibrary {
    ...
    struct PageInfo {
        uint8 maxDeployPerFactory;
        uint8 maxFactories;
        uint8 burnReward;
        uint8[] buildTypes;
        uint8[] buildModels;
        uint256 factoryGrowReward;
        uint256 playRelationsBase;
    }
    ...
    struct AppPrices {
        uint256 factoryPrice;
        uint256 employeePrice;
        uint256 playRelationsBase;
        uint256 resetDeploysPrice;
    }
}
...
contract Factories is Context, AccessControl, ERC721Enumerable {
    ...
    mapping(uint256 => BBLibrary.FactoryNFT) private factories;
    ...
    function getFactory(uint256 id)
        public
        view
        returns (BBLibrary.FactoryNFTData memory)
    {
        return BBLibrary.FactoryNFTData(factories[id], tokenURI(id));
    }
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        string memory union = string(
            abi.encodePacked(
                ...
                ";multiplier:",
                Strings.toString(factories[id].multiplier)
            )
        );
    ...
    function mint(
        ...
        factories[_count] = BBLibrary.FactoryNFT(build, model, multiplier);
    ...
    function getMultiplier(uint256 id) public view virtual returns (uint64) {
