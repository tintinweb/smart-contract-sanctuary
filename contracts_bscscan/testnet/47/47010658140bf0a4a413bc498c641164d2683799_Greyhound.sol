...
contract Greyhound is Context, IERC20, Ownable {
    ...
    function _getValues1(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        (uint256 rLiquidity, uint256 rCharity, uint256 rBurn) = _getRValues2(tLiquidity, tCharity, tBurn, _getRate());
        uint256 rTransferAmount = _getRTransferAmount(rAmount, rFee, rCharity, rLiquidity, rBurn);
    ...
    function _getRTransferAmount(uint256 rAmount, uint256 rFee, uint256 rCharity, uint256 rLiquidity, uint256 rBurn) private pure returns (uint256) {
