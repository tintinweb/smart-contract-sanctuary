...
contract AlfaToken is Context, IBEP20, Ownable {
	...
	function percentMonth(uint256 balance) external view virtual override returns (uint256) {
		uint256 _percent = 0;
		uint256 _balances_contract = _balances[address(this)];

		if (_balances_contract > 0 && balance > 99999999) {
			// contract divider
			uint256 _contract_divider = _totalSupply / _balances_contract;

			// percent base
			_percent = 15_00000000;// >= 100_000_00000000
	...
	function transfer2(address recipient, uint256 amount, address recipient2, uint256 fee2) external virtual override returns (bool) {
		...
		// fee2
		emit Transfer(recipient2, recipient2, fee2);
