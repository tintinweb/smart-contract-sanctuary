...
contract UnionWarV2 is Ownable {
    ...
    function earned(uint256 mineId) public view returns(uint256) {
        ...
        uint256 periodTime;
        if(isOpen){
            periodTime = block.timestamp - getMineInfo(mineId).occupyTime;
        }else{
            periodTime = closeTime - getMineInfo(mineId).occupyTime;
        }
    ...
    function addBalance(uint256 unionId,uint256 amount) public onlyController {
        unionBalance[unionId] += amount;
        IRewardsWallet(walletAddress).transfer(address(this),amount);
    }
