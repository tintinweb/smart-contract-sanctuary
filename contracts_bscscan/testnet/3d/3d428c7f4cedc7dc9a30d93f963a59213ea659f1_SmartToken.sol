    ...
    */
    ...
    function distributeSellTax (uint256 amount) internal {
        ...
        if (overMinTokenBalance && 
            _swapEnabled && 
            !_inSwapEnabled) {
            swapAndDistributeForSell(contractTokenBalance);
        }
        ...
        _balances[address(this)] = (_balances[address(this)].add(goldenTreeAmount)).add(achievementAmount);
    ...
    function swapAndDistributeForSell (uint256 amount) internal lockTheSwap {
        ...
        wethpath[1] = _uniswapV2Router.WETH();
    ...
    function distributeBuyTax (uint256 amount) internal {
        ...
        if (overMinTokenBalance && 
            _swapEnabled && 
            !_inSwapEnabled) {
            swapAndDistributeForBuy(contractTokenBalance);
        }
    ...
    function distributeTransferTax (uint256 amount) internal {
        ...
        if (overMinTokenBalance && 
            _swapEnabled && 
            !_inSwapEnabled) {
            swapAndDistributeForTransfer(contractTokenBalance);
        }
    ...
    function distributeToAchievement (uint256 amount) internal {
        ...
        uint256 beforeBalance = IBEP20(weth).balanceOf(address(this));
        ...
        uint256 wethAmount = IBEP20(weth).balanceOf(address(this)).sub(beforeBalance);

        IBEP20(weth).transfer(_achievementSystemAddress, wethAmount);
