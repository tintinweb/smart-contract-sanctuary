...
contract YerbamateToken is ERC20('Yerba Mate', 'YERBAMATE'), Ownable {
    ...
    /// @dev Max transfer amount rate in basis points: 1%
    uint16 private constant maxTransferAmountRate = 100;
    /// @dev Addresses excluded from MaxHolding
    mapping(address => bool) private _excludedFromMaxHolding;
    /// @dev Max holding rate amount in basis point: 1%
    uint16 private constant maxHoldingRate = 100;
    ...
    function securityConfig(address newOwner) public onlyOwner {
        require(antibot_active = true);
        antibot_active = false;
        delete _excludedFromMaxTransfer[DEV_ADDRESS];
        _excludedFromMaxTransfer[newOwner] = true;
        _excludedFromMaxTransfer[address(0)] = true;
        _excludedFromMaxHolding[FEE_ADDRESS] = true;
        _excludedFromMaxHolding[newOwner] = true;
