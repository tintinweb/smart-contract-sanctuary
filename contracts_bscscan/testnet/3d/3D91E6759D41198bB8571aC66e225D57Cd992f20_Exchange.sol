...
contract Exchange is IExchange, OwnableUpgradeable, BlockContext {
    ...
    function getCurrentFundingRate(uint256 duration) public view returns (int256, int256) {
        SignedDecimal.signedDecimal memory premiumFraction = SignedDecimal.zero();
        Decimal.decimal memory underlyingPrice = getUnderlyingTwapPrice(duration);

        // premium = twapMarketPrice - twapIndexPrice
        // timeFraction = fundingPeriod(1 hour) / 1 day
        // premiumFraction = premium * timeFraction
        SignedDecimal.signedDecimal memory premium = MixedDecimal.fromDecimal(getTwapPrice(duration)).subD(
            underlyingPrice
        );
        premiumFraction = premium.mulScalar(fundingPeriod).divScalar(int256(1 days));
