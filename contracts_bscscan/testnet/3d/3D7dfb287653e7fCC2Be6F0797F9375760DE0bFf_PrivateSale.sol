...
contract PrivateSale {
  ...
  function buyAllocation(uint256 _busdAmount) public rightAmount(_busdAmount) {
    require(block.timestamp >= saleStart, "Private sale not started yet");
    require(block.timestamp <= saleEnd, "Private sale is closed");
    
    uint256 tokenAmount = tokenConversion(_busdAmount);
    
    require(_busdAmount.add(raised) <= maxCap, "Amount requested exceeds max cap");
    
    require(tokenAmount.add(_allocatedTokens[msg.sender]) <= maxAllocation, "Amount sent exceeds maxAllocation");
    
    require(busdToken.allowance(msg.sender, address(this)) >= _busdAmount, "Insufficient approved amount");
    require(busdToken.balanceOf(msg.sender) >=_busdAmount, "Insufficient funds");
