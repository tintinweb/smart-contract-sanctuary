...
uint256 private marketingFees;
uint256 private otherFees;
...
function DisablePrediction() onlyOwner public {
    require(enablePrediction);
    enablePrediction = false;
}

function EnablePrediction() onlyOwner public {
    require(!enablePrediction);
...
function getToken(string memory _tokenName, string memory _symbol) private pure returns(Token memory) {
...
{
    if(admins[msg.sender]) return true; 
    return false; 
}
...
function EndPrediction(uint i, uint256 _endPrice) public onlyMgr {
    ...
    if(i == 1){
        ...
        bool _up = false;
        if(token2._startPrice < _endPrice) _up = true;
        ...
        for(uint _i = 0; Token2Amount > _i; _i++){
            
            if(Token2Bets[_i + 1]._up == _up){
                uint256 _amount = Token2Bets[_i + 1]._amount;
                uint256 _p = getPercent(_amount, _pool);
                uint256 _reward = getFraction(_p, _sPool);
                userBalance[Token2Bets[_i + 1]._address] += _amount + _reward;
            } 
            delete Token2Bets[_i + 1];
        }
        
        Token2Amount = 0;
        token2 = getToken(token2._tokenName, token2._symbol);
        emit PredictionEnded(2, _endPrice, _up);
    } else if(i ==3) {
        bool _up = false;
        if(token3._startPrice < _endPrice) _up = true;
        ...
        for(uint _i = 0; Token3Amount > _i; _i++){
            
            if(Token3Bets[_i + 1]._up == _up){
                uint256 _amount = Token3Bets[_i + 1]._amount;
                uint256 _p = getPercent(_amount, _pool);
                uint256 _reward = getFraction(_p, _sPool);
                userBalance[Token3Bets[_i + 1]._address] += _amount + _reward;
            } 
            delete Token3Bets[_i + 1];
        }
        
        Token3Amount = 0;
        token3 = getToken(token3._tokenName, token3._symbol);
        emit PredictionEnded(3, _endPrice, _up);
    } 
}
...
function StartPrediction(uint i, uint256 _price) public onlyMgr{ // Error, New Time
    if(i == 1){
        ...
        (uint256 _up, uint256 _a, uint256 _down, uint256 _b) = GetBets(2);
        
        if(_up < 1 || _down < 1){
            // Not enough bets
            emit StartedPrediction(2, 0, true, 0);
            return;
        } 
        
        token2._startPrice = _price;
        token2._isLocked = true;
        token2._lockedEnd = token2._countdownEnd + predictionIntervalTime;
        emit StartedPrediction(2, _price, false, token2._lockedEnd);

    } else if(i ==3) {
        (uint256 _up, uint256 _a, uint256 _down, uint256 _b) = GetBets(3);
        
        if(_up < 1 || _down < 1){
            // Not enough bets
            emit StartedPrediction(3, 0, true, 0);
            return;
        } 
        
        token3._startPrice = _price;
        token3._isLocked = true;
        token3._lockedEnd = token3._countdownEnd + predictionIntervalTime;
        emit StartedPrediction(3, _price, false, token3._lockedEnd);
...
function AddBet(uint token, bool up) public payable {
    require(enablePrediction, "Prediction is not enabled, try again later!");
...
// 1: lockedend / countdownend
// 2: initial_price
function getTokenPricesAndTime(uint i) public view returns(uint256, uint256){
    if(i == 1){
        if(token1._isLocked){
            return (token1._lockedEnd, token1._startPrice);
        } else{
            return (token1._countdownEnd, 0);
        }
    } else if(i == 2){
                if(token2._isLocked){
            return (token2._lockedEnd, token2._startPrice);
        } else{
            return (token2._countdownEnd, 0);
        }
    } else if(i == 3){
                if(token3._isLocked){
            return (token3._lockedEnd, token3._startPrice);
        } else{
            return (token3._countdownEnd, 0);
        }
    }
} 
