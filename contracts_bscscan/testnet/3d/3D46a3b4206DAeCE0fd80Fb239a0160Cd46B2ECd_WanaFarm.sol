...
contract WanaFarm is Ownable, Initializable, ITokenFarm {
    ...
    uint256 public MAGIC_NUMBER = 1e12;
    ...
    function initialize(
        ...
        tokenAddress = ITokenVault(_tokenVault).token();
    ...
    function _getMultiplierFromStart(uint256 _block) internal view returns (uint256) {
        ...
        if (roundPassed == 0) {
            return (_block - startBlock) * firstCycleRate * MAGIC_NUMBER;
        } else {
            uint256 multiplier = reducingCycle * firstCycleRate * MAGIC_NUMBER;
            uint256 i = 0;
            for (i = 0; i < roundPassed - 1; i++) {
                multiplier =
                    multiplier +
                    ((MAGIC_NUMBER * initRate * reducingRate**i) / 100**i) *
                    reducingCycle;
            }
    ...
    function pendingToken(address _user) external view override returns (uint256) {
        ...
        uint256 _tokenSupply = IERC20(tokenAddress).balanceOf(address(this));
        if (block.number > lastRewardBlock && _tokenSupply != 0 && isActive == true) {
            ...
            uint256 _tokenReward = (_multiplier * tokenPerBlock) / MAGIC_NUMBER;
            _accWanaPerShare = _accWanaPerShare + ((_tokenReward * MAGIC_NUMBER) / _tokenSupply);
        }
        return ((user.amount * _accWanaPerShare) / MAGIC_NUMBER) - user.rewardDebt;
    }
    ...
    function updatePool() public mustActive {
        ...
        uint256 _multiplier = getMultiplier(lastRewardBlock, block.number);
        uint256 _tokenReward = (_multiplier * tokenPerBlock) / MAGIC_NUMBER;
        accWanaPerShare = accWanaPerShare + ((_tokenReward * MAGIC_NUMBER) / _tokenSupply);
        lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) external override mustActive {
        ...
        if (_amount > 0) {
            IERC20(tokenAddress).safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function withdraw(uint256 _amount) external override {
        ...
        if (_amount > 0) {
            user.amount = user.amount - _amount;
            IERC20(tokenAddress).safeTransfer(address(msg.sender), _amount);
        }
        user.rewardDebt = (user.amount * accWanaPerShare) / MAGIC_NUMBER;
    ...
    function emergencyWithdraw() external override {
        UserInfo storage user = userInfo[msg.sender];
        IERC20(tokenAddress).safeTransfer(address(msg.sender), user.amount);
