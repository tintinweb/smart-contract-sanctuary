...
import "./interfaces/IDmiraiMLM.sol";
...
/**
 * @notice DmiraiVaultsBank contract:
 * - Users can:
 *   # Deposit token
 *   # Deposit BNB
 *   # Withdraw assets
...
contract DmiraiVaultsBank is ReentrancyGuard, Ownable {
     ...
     * @param allocPoint: How many allocation points assigned to this pool. DMIRAI to distribute per block
    ...
    uint256 public constant BUY_AND_BURN_DMIRAI = 10;
    ...
    constructor(
        uint256 _startBlock,
        address _dmirai,
        uint256 _dmiraiRewardRate
    ){
        require(block.number < _startBlock, "DmiraiVaultsBank: Start block must have a bigger value");

        startBlock = _startBlock;
        rewardPoolInfo = RewardPoolInfo({rewardToken: _dmirai, rewardPerBlock: _dmiraiRewardRate});
    ...
    modifier onlyOnDmiraiMLM() {
        require(IDmiraiMLM(relationship).isOnDmiraiMLM(msg.sender), "DmiraiVaultsBank: Don't have relationship");
        _;
    }
    ...
    function resetStrategy(uint256 _pid, address _strategy) external onlyOwner {
        ...
        require(
            ...
            "DmiraiVaultsBank: Strategy not empty"
        );
    ...
    function migrateStrategy(uint256 _pid, address _newStrategy) external onlyOwner {
        require(
            ...
            "DmiraiVaultsBank: New strategy not empty"
        );
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function deposit(
        ...
        IDmiraiMLM(relationship).addDmiraiMLM(msg.sender, _referrerId);
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function _deposit(
        ...
        if (_wantAmt > 0) {
            ...
            IDmiraiMLM(relationship).distributeRewards(_wantAmt, address(pool.want), msg.sender);

            pool.want.safeTransfer(buyBack, (_wantAmt * BUY_AND_BURN_DMIRAI) / 100);

            IBuyBack(buyBack).buyAndBurnToken(
                address(pool.want),
                (_wantAmt * BUY_AND_BURN_DMIRAI) / 100,
    ...
    function _withdraw(uint256 _pid, uint256 _wantAmt) private {
        ...
        require(user.shares > 0, "DmiraiVaultsBank: user.shares is 0");
        require(sharesTotal > 0, "DmiraiVaultsBank: sharesTotal is 0");
...
interface IDmiraiMLM {
    function isOnDmiraiMLM(address) external view returns (bool);

    function addDmiraiMLM(address, uint256) external;
