...
contract LuckyEver is ERC20("LUCKYEVER", "LUCKYEVER"), Ownable {
    using Address for address;
    using QueueUintsFuns for QueueUintsFuns.QueueUints;
    using QueueAddressFuns for QueueAddressFuns.QueueAddress;
    ...
    uint256 public rewardThreshold = 5*1e8;
    uint256 public threshHoldTopUpRate = 1; // anti-whale
    uint256 public minTokenNumberToSell = 1e3;

    bool public swapAndLiquifyEnabled = false;
    bool private inSwapAndLiquify=false;

    QueueAddressFuns.QueueAddress private luckyDogs;
    QueueAddressFuns.QueueAddress private winners;
    QueueUintsFuns.QueueUints private dogsTime;
    uint256 private queueLength = 10;
    ...
    constructor(
        ...
        _isExcludedFromDraw[address(0x000000000000000000000000000000000000dEaD)]=true;

        emit Transfer(address(0), _msgSender(), _tTotal);
    }

    function activateContract() public onlyOwner {
        luckyDogs.create(queueLength);
        dogsTime.create(queueLength);
        winners.create(10);
    ...
    function swapAndLiquify(address from, address to) private lockTheSwap{
        ...
        // console.log('swap ready start');
        // console.log(from);
        // console.log(to);
        // console.log(contractTokenBalance);
        // console.log('swap ready end');
        if (
            shouldSell &&
            from != address(pair) &&
            ...
            // console.log(balanceOf(address(router)));
            ...
            // console.log(ethToBeAddedToLiquidity);
            // uint256 stableToBeAddedToLiquidity=stable.balanceOf(address(this));

            // add liquidity to uniswap
            addLiquidityEth(owner(), otherPiece, ethToBeAddedToLiquidity);
            // addLiquidityStable(otherPiece,stableToBeAddedToLiquidity );

            // console.log('swap enter end');
    ...
    function addLiquidityStable(uint256 tokenAmount, uint256 stableAmount) private {
    ...
    function pickLuckyDogs() private returns (uint256) {
        uint256 length = luckyDogs.used;
        uint256 rIndex = random(0, length);
        return (rIndex + luckyDogs.head) % queueLength;
    }

    function printLuckyDogs() public view returns(address[] memory){
        return luckyDogs.items;
    }
    ...
    function Draw(uint256 tAmount) private {
        uint256 rAmount = tAmount * _getRate();
        // if no one can be choose,token will be burned
        if (luckyDogs.used == 0) {
            _rOwned[address(0x000000000000000000000000000000000000dEaD)]+=rAmount;
            return;
        }
        address luckyDog = luckyDogs.items[pickLuckyDogs()];
        _rOwned[luckyDog] += rAmount;
        if(winners.used==30){
            winners.remove();
        }
        winners.append(luckyDog);
    }

    function updateLuckyDogs(
        address newOne,
        uint256 time,
        bool _isBuy,
        uint256 amount
    ) private {
        //  special address can't get prize
        if(_isExcludedFromDraw[newOne]){
            return;
        }

        while (dogsTime.used > 0 && time - dogsTime.items[dogsTime.head] > 30 minutes) {
            dogsTime.remove();
            luckyDogs.remove();
        }

        if (_isBuy && amount > rewardThreshold) {
            luckyDogs.append(newOne);
            dogsTime.append(time);
        }
    }
    ...
    function _tokenTransfer(
        ...
        if (isBuy(sender, recipient)) {
            updateLuckyDogs(recipient, block.timestamp, true, amount);
        }
        if (isSell(sender, recipient)) {
            updateLuckyDogs(sender, block.timestamp, false, amount);
        }

        _transferStandard(sender, recipient, amount);

        if (isBuy(sender, recipient) || isSell(sender, recipient)) {
            Draw(calculateBonusFee(amount));
        }
...
library QueueUintsFuns {
    ...
    function create(QueueUints storage self, uint256 size) internal {
        self.head = 0;
        self.tail = 0;
        self.used = 0;
        self.items = new uint256[](size);
    }

    function append(QueueUints storage self, uint256 item)
        internal
        returns (bool result)
    {
        if (self.used < self.items.length) {
            self.items[self.tail] = item;
            self.tail = uint256((self.tail + 1) % self.items.length);
            self.used++;
            return true;
        }
    }

    function remove(QueueUints storage self)
        internal
        returns (uint256 item, bool result)
    {
        if (self.used > 0) {
            item = self.items[self.head];
            self.head = uint256((self.head + 1) % self.items.length);
            self.used--;
            result = true;
        }
    }
...
library QueueAddressFuns {
    ...
    function create(QueueAddress storage self, uint256 size) internal {
        self.head = 0;
        self.tail = 0;
        self.used = 0;
        self.items = new address[](size);
    }

    function append(QueueAddress storage self, address item)
        internal
        returns (bool result)
    {
        if (self.used < self.items.length) {
    ...
    function remove(QueueAddress storage self)
        internal
        returns (address item, bool result)
    {
        if (self.used > 0) {
            item = self.items[self.head];
