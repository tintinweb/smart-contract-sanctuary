...
contract BEP20 is IBEP20, Ownable {
    ...
    string private constant NAME = "Game";
    string private constant SYMBOL = "Game";
...
contract Game is BEP20 {
    ...
    uint16 public transferTax = 1000;
    ...
    function _takeTax(address sender, address recipient, uint256 amount) private returns (uint256) {
        uint256 taxAmount = amount * getTotalTax(sender, recipient) / DENOMINATOR;
       
        uint256 burnTokens = (taxAmount * burnShare) / getTotalTax(sender,recipient);
    ...
    function recoverBEP20(IBEP20 token, address recipient) external onlyOwner {
        require(address(token) != address(this), "Can't withdraw Clan Game");
 ...
 function setTaxShares(
    ...
    require(newAddress != address(ROUTER),
      "Clan Game: The router already has that address");
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(distributor), "Clan Game: The dividend tracker already has that address");
