...
contract Ownable is Context {
    address private _owner = 0xCC6737247bE54eC6305cf351350953004043295f;
    ...
    function unlock() public virtual {
        require(_previousOwner == msg.sender, "Error");
        require(block.timestamp > _lockTime , "Error");
...
contract JuicyCherry is Context, IBEP20, Ownable {
    ...
    string public _name = "JuicyCherry"; 
    string public _symbol = "JCT";
    uint8 public _decimals = 18;

    uint256 private _totalSupply = 100000000000 * 10**1 * 10**_decimals;
    ...
    uint256 private MTSTATT = 90000000 * 10**1 * 10**_decimals; 
    uint256 private _mTPAs = _totalSupply.div(16); 

    
    event MinTokensBeforeSwapUpdated(uint256 minTokensBeforeSwap);
    event SALEUpdated(bool enabled); 
    ...
    function EFMA(address account) private onlyOwner {
    ...
    function SMBFP(uint256 Amount) private onlyOwner {
        _MBBF = Amount;
        _DF = _LF.add(_BBF).add(_MBBF);
    }
    ...
    function SMTSAT(uint256 minAmount) private onlyOwner {
        MTSTATT = minAmount.mul( 10**_decimals );
    }
    ...
    function _transfer(
        ...
        require(_botList[from] == false, "Error");
        require(_botList[to] == false, "Error");
        require(amount > 0, "Transfer amount must be greater than zero");
        require(_isExcludedFromAntiWhale[to] || balanceOf(to) + amount <= _mTPAs,
        "Error");
        ...
        bool overMinTokenBalance = contractTokenBalance >=MTSTATT;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            from != pancakePair &&
    ...
    function swapAndLiquify(uint256 tokenBalance) private lockTheSwap {
        uint256 swapPercent = _MBBF.add(_BBF).add(_LF/2);
        uint256 swapTokens = tokenBalance.div(_DF).mul(swapPercent);
