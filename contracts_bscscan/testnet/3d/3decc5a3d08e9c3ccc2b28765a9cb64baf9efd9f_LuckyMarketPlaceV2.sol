...
interface IBidNFT {
    ...
    function readyToBidToken(uint256 _tokenId, uint256 _price, uint256 _step, uint256 _startDate, uint256 _endDate) external;
...
contract LuckyMarketPlaceV2 is IBidNFT ,ISwapNFT, ERC721Holder, Ownable, Pausable {
    using SafeMath for uint256;
   ...
   struct ListNFT {
    ...
    EnumerableMap.UintToUintMap private _bidFromDateMap;  
    EnumerableMap.UintToUintMap private _bidEndDateMap;      
    ...
    mapping(uint256 =>Offer[]) public _offerMap;
    mapping(address =>OfferOwner[]) public _offerOwnerMap;
    ...
    event OfferNFTs(address indexed user, uint256 indexed tokenId,uint256 indexed tokenTargrt, uint256 price);
    ...
    function readyToBidToken(uint256 _tokenId, uint256 _price, uint256 _step, uint256 _startDate, uint256 _endDate) public override whenNotPaused {
        require(_msgSender() == nft.ownerOf(_tokenId), 'Only Token Owner can sell token');
        require(_price != 0, 'Price must be granter than zero');
        ...
        _bidFromDateMap.set(_tokenId, _startDate);
        _bidEndDateMap.set(_tokenId, _endDate);        
    ...
    function revokeBidToken(uint256 _tokenId) public override whenNotPaused {
        ...
        _bidFromDateMap.set(_tokenId, 0);
        _bidEndDateMap.set(_tokenId, 0);            
    ...
    function setTokenNFT(address _nftAddress) public onlyOwner {
            nft = IERC721(_nftAddress);
    }
    ...
    function bidToken(uint256 _tokenId, uint256 _price) public override whenNotPaused {
        ...
        require(_bidsMap.contains(_tokenId), 'Token not in bid book');
         ...
         uint256 endDateBid = _bidEndDateMap.get(_tokenId);
        if(endDateBid != 0 ){
            require(block.timestamp<=endDateBid, 'Expired Bid');
        }
   ...
   function updateBidPrice(uint256 _tokenId, uint256 _price) public override whenNotPaused {
    ...
    function cancelBidToken(uint256 _tokenId) public override whenNotPaused {
        ...
        (,uint256 startDate, uint256 endDate) = getInfoBidByTokenId(_tokenId);
        
        if(startDate !=0 && endDate !=0){
            require(block.timestamp > endDate, 'Auction in progress, can not cancel');
        }
    ...
    function getStepByTokenId(uint256 _tokenId)public view returns(uint256) {
     ...
     function offerNFT(uint256 _tokenId,uint256 _tokenTarget, uint256 _price) public override whenNotPaused  {
         if(_price>0){
             quoteErc20.safeTransferFrom(address(_msgSender()), address(this), _price);
         }
         nft.safeTransferFrom(address(_msgSender()), address(this), _tokenId);
        _offerMap[_tokenTarget].push(Offer({isCancel:false, tokenId: _tokenId, price: _price, user:address(_msgSender())}));
      
      
        _offerOwnerMap[address(_msgSender())].push(OfferOwner({tokenId: _tokenTarget, price: _price}));
        emit OfferNFTs(_msgSender(), _tokenId, _tokenTarget, _price);
    }
    
    
    function swapNFT(uint256 _nftA,uint256 _nftB, address _userB) public  override whenNotPaused {
        (Offer memory offerEntry, uint256 _index) = getOfferByTokenIdAndAddressSwap(_nftA, _userB);
        ...
        // if user offer have Token then transfer to user A
        if(price>0){
             quoteErc20.transfer(address(_msgSender()), price);
         }
        ...
        //status update to cancel old offers
        updateStatusOffersCurrent(_nftA);
        
        // remove user B in list offer
        delOfferByTokenIdAndIndex(_nftA, _index);

        (,uint256 _indexOwner) = getOfferOnwerByTokenId(_nftA);        
        delOfferOwnerByTokenIdAndIndex(_indexOwner);        
    ...
    function cancelListNFT(uint256 _tokenId) public  override whenNotPaused {
        ...
        nft.safeTransferFrom(address(this), address(_msgSender()), _tokenId);
    ...
    function cancelOfferNFT(uint256 _tokenId) public  override whenNotPaused {
        ...
        // remove owner offer
        (,uint256  _indexOwner) = getOfferOnwerByTokenId(_tokenId);    
        delOfferOwnerByTokenIdAndIndex( _indexOwner);       
        emit CancelOfferNFT(_msgSender(), _tokenId);
    }

    function delOfferByTokenIdAndIndex(uint256 _tokenId, uint256 _index) internal {
    ...
    function delOfferOwnerByTokenIdAndIndex(uint256  _indexOwner) internal {
        uint256 len = _offerOwnerMap[address(_msgSender())].length;
        for (uint256 i = _indexOwner; i < len - 1; i++) {
            _offerOwnerMap[address(_msgSender())][i] = _offerOwnerMap[address(_msgSender())][i + 1];
        }
        _offerOwnerMap[address(_msgSender())].pop();
    }        
    function updateStatusOffersCurrent(uint256 _tokenId) internal {
        uint256 len = _offerMap[_tokenId].length;
        for (uint256 i = 0; i < len; i++) {
            _offerMap[_tokenId][i].isCancel = true;
        }
    }    
            ...
            returns (Offer memory,uint256)
        {
            ...
            for (uint256 i = 0; i < len; i++) {
                if (_address == offerEntries[i].user) {
                    _index = i;
                    offerEntry = Offer({user: offerEntries[i].user, price: offerEntries[i].price,isCancel:offerEntries[i].isCancel, tokenId:offerEntries[i].tokenId});
                    break;
                }
            }
            return (offerEntry,_index);
    }
    function getOfferOnwerByTokenId(uint256 _tokenId)
            public
            view
            returns (OfferOwner memory,uint256)
        {
            // find the index of the offer
            OfferOwner[] memory offerEntries = _offerOwnerMap[address(_msgSender())];
            uint256 len = offerEntries.length;
            uint256 _index;
            OfferOwner memory offerEntry;
            for (uint256 i = 0; i < len; i++) {
                if (_tokenId == offerEntries[i].tokenId) {
                    _index = i;
                    offerEntry = OfferOwner({ price: offerEntries[i].price, tokenId:offerEntries[i].tokenId});
                    break;
                }
            }
            return (offerEntry,_index);
   ...
   function getOffersLength(uint256 _tokenId) public view returns (uint256) {
    ...
    function getOffersByOwner(address _owner) public view returns (OfferOwner[] memory) {
        uint256 len = _offerOwnerMap[address(_owner)].length;        
        OfferOwner[] memory offers = new OfferOwner[](len);
        for (uint256 i = 0; i < len; ++i) {
            offers[i] = OfferOwner({tokenId: _offerOwnerMap[address(_owner)][i].tokenId, price:  _offerOwnerMap[address(_owner)][i].price});
        }
        return offers;            
    }  
    
    function getInfoBidByTokenId(uint256 _tokenId) public view returns (address, uint256, uint256) {
        uint256 startDate = _bidFromDateMap.get(_tokenId);
        uint256 endDate = _bidEndDateMap.get(_tokenId);    
        address bidder =_tokenBidders[_tokenId];        
        return (bidder,startDate,endDate);
    }       
