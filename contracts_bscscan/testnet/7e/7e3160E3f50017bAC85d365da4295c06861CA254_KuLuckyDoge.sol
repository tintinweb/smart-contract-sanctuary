...
contract KuLuckyDoge is Context, IERC20, IERC20Metadata {
    ...
    uint256 public minDepositWalletBalanceForWithdrawals;

    address public depositWallet;
    mapping(address => uint256) public depositAmountTotal;
    ...
    mapping(address => bool) public isBannedForManipulation;
    mapping(address => uint256) public lastBlockFlippedCoin;
    //////////////////////////// COIN FLIP VARS ////////////////////////////
    ...
    constructor () {
        ...
        // DEPLOYMENT GAMEPLAN
        // deposit 20% into deposit wallet - 2000000000000000000000


        nameOfToken = "KuLuckyDoge";
        symbolOfToken = "KLD";
        decimalsOfToken = 9;
        decimalsMultiplier = 10**9;
        totalSupplyOfToken = 10**15 * decimalsMultiplier;    // 1 Quad
        ...
        // PancakeSwap
        // routerAddressForDEX = 0xc0fFee0000C824D24E0F280f1e4D21152625742b;       // CHANGEIT - change this to real pancakeswap router
        ...
        depositWallet = 0x7777777777777777777777777777777777777777;
        maxBetAmount = totalSupplyOfToken.div(100000);   // 0.001%
        minBetAmount = totalSupplyOfToken.div(10**15);   // set to 1 KLD
        maxDepositAmount = totalSupplyOfToken.div(100);   // 1%
        minDepositAmount = totalSupplyOfToken.div(10**15);   // set to 1 KLD
        minWithdrawAmount = totalSupplyOfToken.div(10**15);   // set to 1 KLD
        minDepositWalletBalanceForWithdrawals = totalSupplyOfToken.div(10);   // 10%
        ...
        reflectBalance[directorAccount] = reflectTokensTotalSupply.div(100).mul(80);        // TODO - figure out distribution
    ...
    function SetBannedForManipulation(address addressToBanOrUnBan, bool isBanned) external OnlyDirector() {
        isBannedForManipulation[addressToBanOrUnBan] = isBanned; 
    }

    function FlipCoin(uint256 betAmount, bool isHeads) external returns (bool) {
        

        address flipperAddress = _msgSender();

        uint256 lastBlockFlippedCoinResult = lastBlockFlippedCoin[flipperAddress];
        lastBlockFlippedCoin[flipperAddress] = block.number;
        require(lastBlockFlippedCoinResult != block.number,"You can only play once per block, please wait a little bit to play again. Thank you.");
        ...
        require(depositAmountTotal[flipperAddress] > 0, "You have no deposit, please deposit more KLD");
        require(depositAmountTotal[flipperAddress] >= betAmount, "You do not have enough KLD in the Deposit, please deposit more KLD");

        require(!isBannedForManipulation[flipperAddress], "You need to appeal your ban. You may have been caught manipulated the system in some way. Please appeal in Telegram or Discord.");
        ...
        if(isHeads == isResultHeads){   // win
            winStreak[flipperAddress] = winStreak[flipperAddress].add(1);
            if(winStreak[flipperAddress] >= 10){
                // manipulation detected, ban them, manually remove ban if appealed.
                isBannedForManipulation[flipperAddress] = true;
            }
            lossStreak[flipperAddress] = 0;
            depositAmountTotal[flipperAddress] = depositAmountTotal[flipperAddress].add(betAmount);
            return true;
        }
        else{   // lose
            lossStreak[flipperAddress] = lossStreak[flipperAddress].add(1);
            winStreak[flipperAddress] = 0;
    ...
    function GetHeadsOrTails() internal view returns (uint256) {
        ...
        return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, randomNumberCounter, balanceOf(depositWallet)))).mod(headsOrTails);
    }
    ...
    function DepositKuLuckyDoge(uint256 depositAmount) external {
    ...
    function WithdrawKuLuckyDoge(uint256 withdrawAmount) external {

        // TODO - require that the depositWallet has enough funds in it, maybe something like 500 trillion
        ...
        require(balanceOfDepositWallet >= minDepositWalletBalanceForWithdrawals, "There is currently not enough in the deposit wallet, contact KuLuckyDoge Support Team in Telegram or Discord.");
