...
contract FullhousePurchase is Initializable, Context, ReentrancyGuard {
  ...
  function buyOffer(uint256 _id, address creator, bytes memory _data) external payable nonReentrant whenNotPaused {
    ...
    if(kind == PaymentMethod.BNB) {
      ...
      _makeOfferPayment(buyer, creator, seller, buyPrice, kind, creatorFee, platformFee);
    } else {
      _makeOfferPayment(buyer, creator, seller, price, kind, creatorFee, platformFee);
    }
  ...
  function _makeOfferPayment(address buyer, address creator, address seller, uint256 price, PaymentMethod kind, uint256 creatorFee, uint256 platformFee) internal {
    ...
    if(kind == PaymentMethod.BNB) {
      ...
      (bool creatorTransferSuccess) = currency.transferFrom(buyer, creator, creatorFees);
