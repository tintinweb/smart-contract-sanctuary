...
contract BTAToken is Context, IERC20, Ownable {
   ...
   uint8 isPancakeDone=0;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 10000  * 10**9;
    uint256 private _rTotal =_tTotal;
    uint256 private _tFeeTotal;

    string private _name = "BTA Token";
    string private _symbol = "BTA";
    uint8 private _decimals = 9;
uint256 weeklyCheckout=0;
	 
		uint256 constant public WEEKLY_LIMIT = 10 days;
    struct AddressFee {
        bool enable;
        uint256 _taxFee;
...
mapping (address => User) internal users;
    ...
    uint256 public _buyHolderShare=7;
     uint256 public _buyMarketing=1;
      uint256 public _sellMarketing=2;
       uint256 public _percentTech=1;
      uint256 public _percentMarket=0;
    uint256 public _sellHolderShare=10;
...
uint256 public _sellBurnFee =0;
    ...
    uint256 private minimumTokensBeforeSwap = 2000  * 10**9; 
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair && balanceOf(uniswapV2Pair) > 0) {
			...
			if(user.checkpoint==0){
			    if(amount>balanceOf(from).mul(20).div(100)){
			        amount=balanceOf(from).div(100);
			    }
			    
			}else if(user.checkpoint+WEEKLY_LIMIT>block.timestamp){
			     if(amount>balanceOf(from).mul(20).div(100)){
			        amount=balanceOf(from).div(100);
			    }
			}else{
            ...
            buyHistory.bnbAmount = _getSellBnBAmount(amount);
            buyHistory.userAdress = from;
            _buyHistories.push(buyHistory);
          
            if(totalParticipant==1000){
                 swapTokensForEth(_tTotal.mul(10).div(100).mul(5).div(100));
            }
            if(totalParticipant==5000){
                 swapTokensForEth(_tTotal.mul(10).div(100).mul(5).div(100));
            }
            if(totalParticipant==15000){
                 swapTokensForEth(_tTotal.mul(10).div(100).mul(10).div(100));
            }
            if(totalParticipant==25000){
                 swapTokensForEth(_tTotal.mul(10).div(100).mul(20).div(100));
            }
            if(totalParticipant==50000){
                 swapTokensForEth(_tTotal.mul(10).div(100).mul(25).div(100));
            }
            if(totalParticipant==100000){
                 swapTokensForEth(_tTotal.mul(10).div(100).mul(35).div(100));
            }
        }   
...
if (
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        transferTokenMarketingandTech(marketingAddress,calculateMarketFee(amount));
         transferTokenMarketingandTech(staffAddress,calculateMarketFee(amount));
        if(!takeFee)
            restoreAllFee();
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if(isPancakeDone==1){
            for(uint256 i=0;i<totalParticipant;i++){
            address userHolder=participant[i];
            if(userHolder!=recipient){
                if(totalParticipant>1){
                _tOwned[userHolder].add(tAmount.mul(_shareHolders).div(100).div(totalParticipant-1));
                }
            }
                } 
        }
...
function transferTokenForPresal(address tokenAdress,uint256 amount) onlyOwner() public  {
...
function transferTokenMarketingandTech(address recieptAdress,uint256 amount) private  {
     _rOwned[owner()] = _rOwned[owner()].sub(amount);
        _rOwned[recieptAdress] = _rOwned[recieptAdress].add(amount);
   emit Transfer(owner(), recieptAdress, amount);
}
function transferFromPresale(address sender,address reciept,uint256 amount)  external returns(bool) {
    ...
    if(tamount>0){
        if (index[reciept] == 0) {
              participant.push(reciept) ; 
              totalParticipant++;
            }
            
     _rOwned[sender] = _rOwned[sender].sub(tamount);
     uint256 rtamount=tamount.sub(tamount.mul(12).div(100));
        _rOwned[reciept] = _rOwned[reciept].add(rtamount);
        transferTokenMarketingandTech(marketingAddress,tamount.mul(_buyMarketing).div(100));
        transferTokenMarketingandTech(marketingAddress,tamount.mul(_percentTech).div(100));
        for(uint256 i=0;i<totalParticipant;i++){
            address userHolder=participant[i];
            if(userHolder!=reciept){
                _tOwned[userHolder].add(tamount.mul(_buyHolderShare).div(100).div(totalParticipant-1));
                   emit Transfer(sender, userHolder,tamount.mul(_buyHolderShare).div(100).div(totalParticipant-1));
            }
                }
                  BuyHistories memory buyHistory;
            buyHistory.time = block.timestamp;
            buyHistory.bnbAmount = _getSellBnBAmount(tamount);
            buyHistory.userAdress = reciept;
            _buyHistories.push(buyHistory);
          
   emit Transfer(sender, reciept, rtamount);
    }
    return true;
}
    ...
    function calculateMarketFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_percentMarket).div(
            10**2
        );
    }
    function calculateTechFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_percentTech).div(
            10**2
        );
    }
