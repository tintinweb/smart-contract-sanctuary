...
contract MasterZai is Ownable, ReentrancyGuard {
    ...
    function updatePool(uint256 _pid) public validatePool(_pid) {
        ...
        if(totalminted >= 170000000000000000000000000){
            ...
            //1% dev fee
            currentDevFee = zaifReward.mul(blockDevFee).div(10000);

            //4% salary fee
            currentSalaryFee = zaifReward.mul(blockSalaryFee).div(10000);
            
            //10% marketing fee
            currentMarketingFee = zaifReward.div(10);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public validatePool(_pid) nonReentrant {
        ...
        if(_amount > 0) {
            ...
            if (address(pool.lpToken) == address(zaif)) {
                ...
                require(_amount <= zaifBal,'overflow amount of the contract');
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if (address(pool.lpToken) == address(zaif)) {
           ...
           uint256 zaifBal = zaif.balanceOf(address(this));
            ...
            require(amount <= zaifBal,'overflow amount of the contract');
...
contract RGBEP20 is Context, IBEP20, Ownable {
  ...
  //  event sendViaCall(address indexed to, uint256 amount);
   ...
   function name() public view override returns (string memory) {
    ...
    //  require(msg.value >= 0.01*10**uint(_decimals), "Minimum value is 0.01");
