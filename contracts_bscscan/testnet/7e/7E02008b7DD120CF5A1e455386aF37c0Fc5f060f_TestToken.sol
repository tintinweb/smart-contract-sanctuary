...
contract TestToken is Context, IERC20, Ownable {
    ...
    uint256 public _ttknFee = 12;
    uint256 private _previousttknFee = _ttknFee;
    
    uint256 public _maxTxAmount = 1000 * 10**9 * 10**9;
    uint256 private minimumTokensBeforeSwap = 3 * 10**6 * 10**9; 
    
    address payable public ttknWallet = 0x97aF543f8dbCB7383f5697b63A57A0a68000943D; 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _tttknTotal = _tttknTotal.add(address(this).balance);
        TransferCharityETH(ttknWallet, address(this).balance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_ttknFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _ttknFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousttknFee = _ttknFee;
    ...
    function setttknFeePercent(uint256 ttknFee) external onlyOwner() {
        _ttknFee = ttknFee;
    }
