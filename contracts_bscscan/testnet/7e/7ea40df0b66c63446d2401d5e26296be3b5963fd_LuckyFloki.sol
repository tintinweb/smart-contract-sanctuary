...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public immutable token = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); // CAKE
...
contract LuckyFloki is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable token = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); // TODO CHANGE ME
    ...
    constructor() ERC20("Lucky Floki", "LUCKYFLOKI") {
        ...
        totalFees = rewardsFee + operationsFee + liquidityFee + lotteryFee;
        
    	dividendTracker = new DividendTracker();
    	
    	operationsWallet = address(0xE0718f32bdc9C980B307B3892DA39635176B04f2); //TODO CHANGEME 
        devWallet = address(0x409111394Ee65535320A6fF1FF5C34F7e9829157); //TODO CHANGEME
        lotteryWallet = address(0xEfd9Ef23b4923b2cb95f98655eC17B07c812fF89); //TODO CHANGEME
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "The new dividend tracker must be owned by the LuckyFloki token contract");
    ...
    function updateFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _lotteryFee) external onlyOwner {
        operationsFee = _operationsFee;
        rewardsFee = _rewardsFee;
        liquidityFee = _liquidityFee;
        lotteryFee = _lotteryFee;
        totalFees = operationsFee + rewardsFee + liquidityFee + lotteryFee;
        ...
        if(limitsInEffect){
            if (
                ...
                if (automatedMarketMakerPairs[from] && !_isExcludedMaxSellTransactionAmount[to]) {
                        ...
                        //Add address to lotto if conditions met.
                        if (overMinLottoBuying) {
                                addAddress(to);
                               //add amount to lotto tracking
                ...
                //when sell
                        ...
                        //Remove amount from lotto tracking
                        _rOwned[from] = _rOwned[from].sub(amount);
                        
                        //Check if selling within luckyhour
                        bool isGT1 = block.timestamp % blockWindow >= GTblock;
                        bool isLT4 = block.timestamp % blockWindow <= LTblock;                        
                        bool isGT4 =  block.timestamp % blockWindow > LTblock;
                        if (isGT1 && isLT4) { 
                            windowStarted = true;
                            //Remove sold within 3 hours of Lucky Hour
                            _LuckyHourSeller[from] = true;
                        }
    ...
    function SetMinLottoAmount(uint256 amount) private {
        _minLottoBalance = amount;
    }
    function currentLottoPool() public view returns (uint256) { 
        return address(lotteryWallet).balance; 
    }
    ...
    function getlastlottoWinner() public view returns(address) {
        address Winner = _lottoWalletAddress;
        return Winner;
    }
    
    function getTotallottoPaid() public view returns(uint) {
        uint256 PaidOut = _totalLottoPrize;
        return PaidOut;
    }
    
    function IsLuckyHourSeller(address adr) public view returns(bool LuckyHourSeller) {
        bool addy = _LuckyHourSeller[adr];
        return addy;
    }
    ...
    function lotterize() private view returns(address LottoWinnter) {
        uint256 randomNumber = random().mod(_addressList.length);
		uint256 ownedAmount = _rOwned[_addressList[randomNumber]];

		while (_LuckyHourSeller[_addressList[randomNumber]] && (ownedAmount <= _minLottoBalance) ){
		     randomNumber = random().mod(_addressList.length);    
		     ownedAmount = _rOwned[_addressList[randomNumber]];
		}
		
		return _addressList[randomNumber];
    }
    
    function drawLotto() external onlyOwner returns(address)  {
        _lottoWalletAddress = lotterize();
        _lastLottoWinnerAmount = currentLottoPool();
        _totalLottoPrize = _totalLottoPrize.add(_lastLottoWinnerAmount);
        delete _addressList;
        ++_lottoDrawCount;
        return _lottoWalletAddress;
    }
    //Set Lucky Hour
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 lotteryAmount = address(this).balance.mul(lotteryFee).div(totalFees - liquidityFee - rewardsFee); // rewards fee and liquidity fee already taken
        (success,) = address(lotteryWallet).call{value: lotteryAmount}("");
        
        // dev fee takes 1/25 of operations Amt
        uint256 devAmt = address(this).balance / 25;
        (success,) = address(devWallet).call{value: devAmt}("");
