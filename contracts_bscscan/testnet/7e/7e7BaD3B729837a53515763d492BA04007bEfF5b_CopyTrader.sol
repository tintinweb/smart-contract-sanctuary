...
contract CopyTrader is Ownable {
    ...
    function withdrawFeeBalance(uint256 amount, address payable to) public onlyOwner {
        require(feeBalance >= amount, "CopyTrader: insufficient feeBalance");
        require(amount > 0, "CopyTrader: amount must be greater than 0");
        require(to == clientAddress || to == MONITOR_ADDRESS, "CopyTrader: not allowed address");
