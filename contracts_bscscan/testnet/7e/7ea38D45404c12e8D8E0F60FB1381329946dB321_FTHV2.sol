...
// This contract is brought to you by: Fuck Them Heggies Token. If you're not fuken heggies you're doing it wrong. Welcome to our V2 super contract. 
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
 ...
 contract FTHV2 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0xB83B963b63A42ce8B1c278Ec3F54B2c0647AbdF5); // Marketing Address
    address payable public charityAddress = payable(0xbFCCbc3C7D1C3D5A6E1bFDe218D97CEe7121624d); // Charity Address
    ...
    string private _name = "Fuck Them Heggies Token";
    string private _symbol = "FTH";
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                if (overMinimumTokenBalance && _startTimeForSwap + _intervalMinutesForSwap <= block.timestamp) {
                    ...
                    uint256 liquidityTokens = contractTokenBalance.div(100).mul(_buyAddLiquidityFee);
                    swapAndLiquify(liquidityTokens);
                    uint256 walletTokens = contractTokenBalance.sub(liquidityTokens);
                    swapTokens(walletTokens);    
                }  
