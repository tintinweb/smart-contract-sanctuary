...
interface IDividendDistributor {
    function setDistributionCriteria(uint256 _minDistribution, uint256 _annualMulipler, uint256 penaltyPercentage) external;
    ...
    function getInfo(bool hideDecimal) external view returns(uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256); 
    function getShareholderInfo(address shareholder, bool hideDecimal) external view returns(uint256, uint256, uint256, uint256, uint256, uint256);
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    function setDistributionCriteria(uint256 _minDistribution, uint256 _annualMulipler, uint256 _penaltyPercentage) external override onlyToken {
    ...
    function unlockTokensWithBreak(address shareholder) external override onlyToken {
        require(locker[shareholder].tokensUnlocked > 0 || locker[shareholder].tokensLocked > 0, "No tokens on the smart contract");
        ...
        if(locker[shareholder].unlockTime > block.timestamp && locker[shareholder].tokensLocked > 0){
            ...
            locker[shareholder].tokensLocked -= penaltyAmount;
    ...
    function deposit() external payable override onlyToken {
        ...
        dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(amount).div(getTotalShares()));
    }
    ...
    function shouldDistribute(address shareholder) internal view returns (bool) {
        ...
        && getUnpaidEarnings(shareholder) > 5 * (10 ** 17); // distribute from 0.5 BUSD
    }
    ...
    function distributeDividend(address shareholder) internal {
        ...
        //dividendsPerShare = totalDividends.mul(dividendsPerShareAccuracyFactor).div(getTotalShares());
    ...
    function getShareholderInfo(address shareholder, bool hideDecimal) public view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    function getInfo(bool hideDecimal) public view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        uint256 shareholderCount = shareholders.length;
        if(hideDecimal){ return (getTotalShares().div(10 ** 18), getTotalLockedTokens().div(10 ** 18), totalDividends.div(10 ** 18), totalDistributed.div(10 ** 18), dividendsPerShare.div(10 ** 18), minDistribution.div(10 ** 18), annualMulipler, penaltyPercentage, shareholderCount); } 
        else return (getTotalShares(), getTotalLockedTokens(), totalDividends, totalDistributed, dividendsPerShare, minDistribution, annualMulipler, penaltyPercentage, shareholderCount);
    }
...
contract endd is IBEP20, Auth {
    ...
    bool public autoDividendDitribution = true;    
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[sender] && recipient == distributorAddress){ try distributor.setMultipleShare(sender, amountReceived) {} catch {} }        
        
        checkMarketCondition();

        if(autoDividendDitribution){ try distributor.process(distributorGas) {} catch {} }
...
//        emit Transfer(sender, recipient, amount);
    ...
    function setAutoDividendDistribution(bool _autoDividendDitribution) external authorized {
        autoDividendDitribution = _autoDividendDitribution;
    }    
    ...
    function setDistributionCriteria(uint256 _minDistribution, uint256 _annualMulipler, uint256 _penaltyPercentage) external authorized {
        // the minimum number of tokens to receive a share in the dividend
        // set the annual share multiplier 
        distributor.setDistributionCriteria(_minDistribution * (10 ** _decimals), _annualMulipler, _penaltyPercentage); 
    }
    ...
    function getDistributorInfo(bool hideDecimal) public view returns (uint256 totalShares, uint256 totalLockedTokens, uint256 totalDividends, uint256 totalDistributed, uint256 dividendsPerShare, uint256 minDistribution, uint256 annualMulipler, uint256 penaltyPercentage, uint256 shareholderCount) {
        return distributor.getInfo(hideDecimal); 
    }

    function getShareholderInfo(address shareholder, bool hideDecimal) public view returns (uint256 shareholderShares, uint256 shareholderDividend, uint256 shareholderUnpaidDividend, uint256 tokensLocked, uint256 unlockTime, uint256 tokensUnlocked) {
        return distributor.getShareholderInfo(shareholder, hideDecimal); 
    }   
