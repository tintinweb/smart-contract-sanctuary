...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, xOX }
    ...
    address public xOX;
    uint public xOXQueue; // Delays change to xOX address
    ...
    /**
        @notice allow approved address to deposit an asset for AlphaOX
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint AlphaOX needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( AlphaOX ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn AlphaOX for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ITEMERC20( AlphaOX ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( xOX ).balanceOf( msg.sender ); // Can only borrow against xOX held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with AlphaOX
        @param _amount uint
     */
    function repayDebtWithTEM( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ITEMERC20( AlphaOX ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, AlphaOX, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( AlphaOX ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( AlphaOX ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns AlphaOX valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match AlphaOX decimals
            value_ = _amount.mul( 10 ** IERC20( AlphaOX ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.xOX ) { // 9
