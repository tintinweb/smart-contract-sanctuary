...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SDIOS }
    ...
    address public sDIOS;
    uint public sDIOSQueue; // Delays change to sDIOS address
    ...
    /**
        @notice allow approved address to deposit an asset for DIOS
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint DIOS needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( DIOS ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn DIOS for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IDIOSERC20( DIOS ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sDIOS ).balanceOf( msg.sender ); // Can only borrow against sDIOS held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with DIOS
        @param _amount uint
     */
    function repayDebtWithDIOS( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IDIOSERC20( DIOS ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, DIOS, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( DIOS ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( DIOS ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns DIOS valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match DIOS decimals
            value_ = _amount.mul( 10 ** IERC20( DIOS ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SDIOS ) { // 9
            sDIOSQueue = block.number.add( blocksNeededForQueue );
        } else return false;
    ...
    function getTotalReserves() public view returns(uint) {
        return totalReserves;
    }

    function getTotalDebt() public view returns(uint) {
        return totalDebt;
    }
}