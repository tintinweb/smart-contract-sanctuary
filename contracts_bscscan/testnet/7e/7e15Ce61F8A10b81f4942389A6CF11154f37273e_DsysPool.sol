...
interface IDsysStablecoin {
  ...
  function dsys_eth_oracle_address() external view returns (address);
  function dsys_info() external view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256);
  function pools(address ) external view returns (bool);
  function pools_array(uint256 ) external view returns (address);
  function dusd_price() external view returns (uint256);
  ...
  function dsys_price() external view returns (uint256);
  function genesis_supply() external view returns (uint256);
  function getRoleAdmin(bytes32 role ) external view returns (bytes32);
  function getRoleMember(bytes32 role, uint256 index ) external view returns (address);
  function getRoleMemberCount(bytes32 role ) external view returns (uint256);
...
contract DsysPool is SafeOwnable {
    ...
    IDsysStablecoin immutable private DUSD;
    IDsysShares immutable private DSYS;
    ...
    uint256 public redeem_price_threshold = 0; 
    uint256 public mint_price_threshold = 0; 
    ...
    constructor (
        IDsysStablecoin _DSYS,
        IDsysShares _DSYSShares,
    ...
    function getDUSDPrice() public view returns (uint256) {
        return DUSD.dusd_price();
    }

    function getDSYSPrice() public view returns (uint256) {
        return DUSD.dsys_price();
    }

    // Returns the FRAX value in collateral tokens
    function getDSYSInCollateral(uint256 col_idx, uint256 dusd_amount) public view returns (uint256) {
        return dusd_amount.mul(PRICE_PRECISION).div(10 ** missing_decimals[col_idx]).div(collateral_prices[col_idx]);
    }
    ...
    // Returns dollar value of collateral held in this DUSD pool, in E18
    ...
    function buybackAvailableCollat() public view returns (uint256) {
        uint256 total_supply = DUSD.totalSupply();
        uint256 global_collateral_ratio = DUSD.global_collateral_ratio();
        uint256 global_collat_value = DUSD.globalCollateralValue();
    ...
    function recollatTheoColAvailableE18() public view returns (uint256) {
        uint256 frax_total_supply = DUSD.totalSupply();
        uint256 effective_collateral_ratio = DUSD.globalCollateralValue().mul(PRICE_PRECISION).div(frax_total_supply); // Returns it in 1e6
        
        uint256 desired_collat_e24 = (DUSD.global_collateral_ratio()).mul(frax_total_supply);
    ...
    function recollatAvailableFxs() public view returns (uint256) {
        uint256 fxs_price = getDSYSPrice();
    ...
    function mintDUSD(
        ...
        if (mint_price_threshold > 0) {
            require(getDUSDPrice() >= mint_price_threshold, "DUSD price too low");
        }
        ...
        if (one_to_one_override || global_collateral_ratio >= PRICE_PRECISION) { 
            ...
            collat_needed = getDSYSInCollateral(col_idx, dusd_amt);
            dsys_needed = 0;
        } else if (global_collateral_ratio == 0) { 
            // Algorithmic
            collat_needed = 0;
            dsys_needed = dusd_amt.mul(PRICE_PRECISION).div(getDSYSPrice());
        } else { 
            // Fractional
            uint256 dusd_for_collat = dusd_amt.mul(global_collateral_ratio).div(PRICE_PRECISION);
            uint256 dusd_for_dsys = dusd_amt.sub(dusd_for_collat);
            collat_needed = getDSYSInCollateral(col_idx, dusd_for_collat);
            dsys_needed = dusd_for_dsys.mul(PRICE_PRECISION).div(getDSYSPrice());
        }

        // Subtract the minting fee
        total_dusd_mint = (dusd_amt.mul(PRICE_PRECISION.sub(minting_fee[col_idx]))).div(PRICE_PRECISION);
        // Checks
        require((dusd_out_min <= total_dusd_mint), "FRAX slippage");
        require(freeCollatBalance(col_idx).add(collat_needed) <= pool_ceilings[col_idx], "Pool ceiling");

        // Take the FXS and collateral first
        console.log("dsys_needed: ", dsys_needed);
        console.log("collat_needed: ", collat_needed);
        console.log("total_dusd_mint: ", total_dusd_mint);
        DSYS.pool_burn_from(msg.sender, dsys_needed);
        SafeERC20.safeTransferFrom(IERC20(collateral_addresses[col_idx]), msg.sender, address(this), collat_needed);

        // Mint the FRAX
        DUSD.pool_mint(msg.sender, total_dusd_mint);
    }

    function redeemDUSD(
        uint256 col_idx, 
        uint256 dusd_amount, 
        uint256 dsys_out_min, 
        uint256 col_out_min
    ) external collateralEnabled(col_idx) returns (
        uint256 collat_out, 
        uint256 dsys_out
        ...
        if (redeem_price_threshold > 0) {
            require(getDUSDPrice() <= redeem_price_threshold, "DUSD price too high");
        }

        uint256 global_collateral_ratio = DUSD.global_collateral_ratio();
        uint256 frax_after_fee = (dusd_amount.mul(PRICE_PRECISION.sub(redemption_fee[col_idx]))).div(PRICE_PRECISION);
        ...
        if(global_collateral_ratio >= PRICE_PRECISION) { 
            ...
            dsys_out = frax_after_fee
                            ...
                            .div(getDSYSPrice()); // PRICE_PRECISIONS CANCEL OUT
        }
        ...
        require(dsys_out >= dsys_out_min, "FXS slippage");
        ...
        redeem_dsys_shares_balances[msg.sender] = redeem_dsys_shares_balances[msg.sender].add(dsys_out);
        unclaimed_pool_dsys_shares = unclaimed_pool_dsys_shares.add(dsys_out);

        last_redeemed[msg.sender] = block.number;
        console.log("dusd_amount: ", dusd_amount);
        console.log("collat_out: ", collat_out);
        console.log("dsys_out: ", dsys_out);
        DUSD.pool_burn_from(msg.sender, dusd_amount);
        DSYS.pool_mint(address(this), dsys_out);
    }
    ...
    function collectRedemption(uint256 col_idx) external returns (uint256 dsys_amount, uint256 collateral_amount) {
        require(redeemPaused[col_idx] == false, "Redeeming is paused");
        ...
        if(redeem_dsys_shares_balances[msg.sender] > 0){
            dsys_amount = redeem_dsys_shares_balances[msg.sender];
            redeem_dsys_shares_balances[msg.sender] = 0;
            unclaimed_pool_dsys_shares = unclaimed_pool_dsys_shares.sub(dsys_amount);
            sendDSYSShares = true;
        }
        ...
        // Send out the tokens
        if(sendDSYSShares){
            SafeERC20.safeTransfer(IERC20(address(DSYS)), msg.sender, dsys_amount);
        }
        if(sendCollateral){
    ...
    function buyBackDSYS(uint256 col_idx, uint256 dsys_amount, uint256 col_out_min) external collateralEnabled(col_idx) returns (uint256 col_out) {
        ...
        uint256 fxs_dollar_value_d18 = dsys_amount.mul(fxs_price).div(PRICE_PRECISION);
    ...
    function recollateralize(uint256 col_idx, uint256 collateral_amount, uint256 fxs_out_min) external collateralEnabled(col_idx) returns (uint256 fxs_out) {
        ...
        DSYS.pool_mint(msg.sender, fxs_out);
