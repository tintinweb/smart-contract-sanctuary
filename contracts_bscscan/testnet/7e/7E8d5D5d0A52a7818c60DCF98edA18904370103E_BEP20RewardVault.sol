...
contract BEP20RewardVault is Ownable {
    ...
    function withdraw(uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            // IndicatorInfo storage referral = indicatorInfo[user.indicator];
            // if (referral.amount >= _amount) {
            //     if (referral.amount == _amount) {
            //         pendingRewardAmountIndicator = referral
            //             .amount
            //             .mul(referral.accRewardTokenPerShare)
            //             .div(1e30)
            //             .sub(referral.rewardDebt);
            //         safeTransferReward(
            //             address(user.indicator),
            //             pendingRewardAmountIndicator
            //         );
            //         referral.amount = referral.amount.sub(_amount);
            //         referral.rewardDebt = referral
            //             .amount
            //             .mul(referral.accRewardTokenPerShare)
            //             .div(1e30);
            //     } else {
