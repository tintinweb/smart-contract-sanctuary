...
contract SSDoge is IERC20 {
    ...
    uint256 private _RewardsFee = 0; // Adjusted by buys and sells.
    uint256 private _totalFee = _liquidityFee + _marketingFee + _RewardsFee;
    uint256 public masterTaxDivisor = 10000;

    uint256 public _buymarketingFee = 1000;
    uint256 public _buyliquidityFee = 300;
    uint256 public _buyRewardsFee = 500;

    uint256 public _sellmarketingFee = 1100;
    uint256 public _sellliquidityFee = 400;
    uint256 public _sellRewardsFee = 600;

    uint256 constant public maxmarketingFee = 2000;
    uint256 constant public maxliquidityFee = 1500;
    uint256 constant public maxRewardsFee = 1500;
    ...
    address payable private _marketingWallet = payable(0xa88c292FE603b737de024A6c716416616CecCaBE);
...
//===============================================================================================================
    ...
    function setBuyTaxes(uint256 liquidityFee, uint256 marketingFee, uint256 RewardsFee) external onlyOwner {
        require(liquidityFee <= maxliquidityFee
                && marketingFee <= maxmarketingFee
                && RewardsFee <= maxRewardsFee
                );
        require(liquidityFee + marketingFee + RewardsFee <= 5000);
        _buyliquidityFee = liquidityFee;
        _buymarketingFee = marketingFee;
        _buyRewardsFee = RewardsFee;
    }

    function setSellTaxes(uint256 liquidityFee, uint256 marketingFee, uint256 RewardsFee) external onlyOwner {
        require(liquidityFee <= maxliquidityFee
                && marketingFee <= maxmarketingFee
                && RewardsFee <= maxRewardsFee
                );
    ...
    function adjustTaxes(address from, address to) internal {
        if (lpPairs[to]) {
            ...
            _RewardsFee = _sellRewardsFee;
        } else if (lpPairs[from]) {
            _marketingFee = _buymarketingFee;
            _liquidityFee = _buyliquidityFee;
            _RewardsFee = _buyRewardsFee;
        } else {
            _marketingFee = 0;
            _liquidityFee = 0;
            _RewardsFee = 0;
    ...
    function getTotalFee() public view returns (uint256) {
        return _marketingFee + _liquidityFee + _RewardsFee;
    }
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        ...
        uint256 amountBNBReflection = amountBNB * _marketingFee / totalBNBFee;
