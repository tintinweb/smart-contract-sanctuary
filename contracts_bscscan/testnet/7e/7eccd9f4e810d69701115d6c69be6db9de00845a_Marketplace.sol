...
/** 
 *  SourceUnit: c:\Projects\realrealm\smart-contracts\contracts\marketplace\Marketplace.sol
*/
...
contract Marketplace is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event BoxContractUpdated(address addr);
    ...
    event BoxConverted(address erc1155, uint256 tokenIdFrom, address erc721, uint256 tokenIdTo, address user);
    ...
    function setBoxContract(IMysterBox box)
        public
        onlyOwner
    {
        boxContract = box;

        emit BoxContractUpdated(address(box));
    }
        ...
        onlyOwner
    {
        ...
        require(length > 0, "Marketplace: erc20 list is required");
    ...
    function setSalePriceForErc1155(address erc1155, address erc20, uint256 tokenId, uint256 price)
        public
        whenNotPaused
        nonReentrant
        inWhitelist(erc1155, erc20)
    {
        address msgSender = _msgSender();

        require(price > 0, "Marketplace: price must be greater than 0");

        IERC1155(erc1155).safeTransferFrom(msgSender, 0x000000000000000000000000000000000000dEaD, tokenId, 1, "");

        uint256 boxId = boxContract.totalSupply() + 1;

        boxContract.mint(boxId, address(this));

        address erc721 = address(boxContract);

        asks[erc721][boxId] = Ask(erc20, msgSender, price);

        emit AskCreated(erc721, erc20, msgSender, price, boxId);

        emit BoxConverted(erc1155, tokenId, erc721, boxId, msgSender);
    }
