...
contract Treasury is AccessControlEnumerable {
	...
	function setContractionIndex(int256 _contractionIndex) public {
		require(_contractionIndex < 10000, 'Contraction too large');
		contractionIndex = _contractionIndex;
	}

	function setExpansionIndex(int256 _expansionIndex) public {
	...
	function setDollarOracle(address _dollarOracle) external {
		dollarOracle = _dollarOracle;
	}

	function setDollarPriceCeiling(uint256 _dollarPriceCeiling) external {
		require(
	...
	function _sendToBoardRoom(uint256 _amount) internal {
		if (_amount > 0) {
			uint256 boardroomCount = IBoardroomAllocation(boardroomAllocation).boardroomInfoLength();
			IBasisAsset(dollar).mint(address(this), _amount);

			for (uint i=0; i < boardroomCount; i++) {
				(address boardroom, bool isActive, uint256 allocationPoints) = IBoardroomAllocation(boardroomAllocation).boardrooms(i);
				//IERC20(dollar).safeApprove(boardroomAddress, _amount.mul());
	...
	function allocateSeigniorage() external {
		...
		int256 supplyDelta = computeSupplyDelta(dollarPrice, dollarPriceOne);
		...
		if (dollarPrice > dollarPriceCeiling) {
			...
			if (
				...
				if (
					_seigniorage.mul(1000).div(10000) <=
					bondSupply.sub(seigniorageSaved)
				) {
					_savedForBond = _seigniorage.mul(1000).div(10000);
					_savedForBoardRoom = _seigniorage.sub(_savedForBond);
				} else {
					_savedForBond = bondSupply.sub(seigniorageSaved);
					_savedForBoardRoom = _seigniorage.sub(_savedForBond);
				}
			}

			if (_savedForBoardRoom > 0) {
				_savedForDevs = _savedForBoardRoom.mul(devPercentage).div(
					10000
				);
		...
		returns (int256)
	{
		...
		if (rate < targetRate) {
			supply = IERC20(dollar).totalSupply().toInt256Safe();
		}
		return
