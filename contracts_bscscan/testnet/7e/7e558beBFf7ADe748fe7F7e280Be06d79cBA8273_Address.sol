...
contract LotteryNFT is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Floki Zilla", "FlokiZilla") {}
...
contract Lottery is LotteryOwnable, Initializable {
    ...
    function reset() external onlyAdmin {
        ...
        olymp.transfer(dead, olymp.balanceOf(address(this)) - totalAmount);
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external inDrawingPhase {
        ...
        olymp.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external inDrawingPhase {
        ...
        olymp.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if (reward > 0) {
            olymp.safeTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if (totalReward > 0) {
            olymp.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        olymp.safeTransfer(address(msg.sender), _amount);
