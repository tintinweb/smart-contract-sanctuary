...
import "./interfaces/IBaseStakingPool.sol";
...
contract KabyHero is IKabyHero, ERC721, AcceptedToken, ReentrancyGuard {
    ...
    /**
     * @dev Minter request to mint hero with specific star to an account
     * @param versionId id of version 
     * @param amount amount of hero want to mint
     * @param account address of user will receive 
     * @param star star of hero want to mint
     */
    function mintHeroWithStar(uint versionId, uint amount, address account, uint star) external onlyMinter override {
        require(versionId <= getLatestVersion(), "KabyHero: invalid version");
        Version storage version = versions[versionId];

        require(star <= maxStar, "KabyHero: invalid star");
        require(amount > 0 && amount <= 50, "KabyHero: amount out of range");
        require(block.timestamp >= version.startTime, "KabyHero: Sale has not started");
        require(amount + version.currentReserve + version.currentSell <= version.maxSupply, "KabyHero: sold out, cannot mint");

        for (uint i = 0; i < amount; i++) {
            _heros.push(Hero(star, 0, 0, 0, 0, 0));
            uint heroId = _heros.length - 1;
            emit SetStar(heroId, star, account);
            emit HeroCreated(heroId, star, account);
            _safeMint(account, heroId);
        }
    ...
    function list(uint heroId, uint price) external override onlyHeroOwner(heroId) {
        ...
        if(stakingPoolAddress != address(0)){
            bool isStaking = IBaseStakingPool(stakingPoolAddress).isCurrentStaking(msg.sender, heroId);
            require(isStaking == false, "KabyHero: hero is current staking");
        }
    ...
    /**
     * @dev Check a hero is on sale or not
     */
    function isOnSale(uint heroId) external view override returns (bool) {
        return herosOnSale[heroId] != 0 ? true : false;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if(stakingPoolAddress != address(0)){
            bool isStaking = IBaseStakingPool(stakingPoolAddress).isCurrentStaking(msg.sender, tokenId);
            require(isStaking == false, "KabyHero: hero is current staking");
        }
    }
}
...
interface IBaseStakingPool {
    /**
     * @notice Check a hero is current staking or not
     */
    function isCurrentStaking(address account, uint heroId) external view returns (bool);
}
...
interface IKabyHero {
     ...
     * @notice Mint Heros from Minter to user.
     */
    function mintHeroWithStar(uint versionId, uint amount, address account, uint star) external;
    ...
    function getHeroStar(uint heroId) external view returns (uint);
     ...
     /**
     * @notice Check a hero is on sale or not
     */
    function isOnSale(uint heroId) external view returns (bool);
