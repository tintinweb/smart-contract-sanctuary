...
//SPDX-License-Identifier: KRAKOVIA
...
// constructor
    constructor() ERC20("Trampa", "TRP") {
        ...
        if(testnet) {
            ...
            uniswapV2Pair = address(0x13B6e448eB51fa9289EaBeB67C775f755652652A);
            automatedMarketMakerPairs[address(0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2)] = true;
        }
        // mappings
        ...
        // tx limits
            ...
            if(automatedMarketMakerPairs[from]) {
                ...
                if(!excludedFromFees[to]) {
                    // tx limit
                    if(limitBuys) {
                        require(amount <= maxBuyTxAmount, "TX Limit Exceeded");
                    }
                    // multi-buy limit
                    if(blockMultiBuys) {
            ...
            //sell - da testare
                ...
                trade_type = 2;
                userLastTradeData[from].amountSell += amount;
                ...
                // limits
                if(!excludedFromFees[from] && limitSells) {
                    require(amount <= maxSellTxAmount, "TX Limit Exceeded");
                    require(
                        userLastTradeData[from].lastSellTime
                        + sellSecondsLimit <= block.timestamp
                        && userLastTradeData[from].amountSell
                        <= maxSellTxAmount
                        );
