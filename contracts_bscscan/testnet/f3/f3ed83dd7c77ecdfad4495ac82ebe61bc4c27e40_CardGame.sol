...
using SafeMath for uint256;
  ...
  event URIAddedToCat(string name, string rarity, string uri, uint256 count);
 ...
 function init1() public {
  ...
  function test(string[] memory test) public{
    test2 = test;
  }
  ...
  function addUriToCategory(string memory name,string memory rarityInput, string memory uris, uint256 cost, uint256 attack, uint256 toughness, uint256 abilityCount, string memory element) adminAdd public {
    ...
    Category[name].rarityBasedUris[rarityInput].attributes[Category[name].count].abilityCount = abilityCount;
    ...
    emit URIAddedToCat(name, rarityInput, uris,Category[name].rarityBasedUris[rarityInput].count);
