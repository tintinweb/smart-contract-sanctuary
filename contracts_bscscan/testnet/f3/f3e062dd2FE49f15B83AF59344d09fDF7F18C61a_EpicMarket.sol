...
contract EpicMarket is AccessControl {
    ...
    function buyItem(uint256 marketId, uint256 amount) public onlyExistItem(marketId) {
        ...
        uint256 price = itemData._price * amount;
        ERC20.transferFrom(msg.sender, itemData._owner, price - (price * rateFee / 100));
        ERC20.transferFrom(msg.sender, _wallet, price * rateFee / 100);
        if(itemData._itemType == TokenType.ERC1155) 
    ...
    function acceptOffer(uint256 marketId, uint256 offerId) public onlyExistItem(marketId){
        ...
        uint256 finalPrice = offerData._price * offerData._amount;
        ERC20.transferFrom(offerData._buyer, itemData._owner, finalPrice  - (finalPrice * rateFee / 100));
        ERC20.transferFrom(offerData._buyer, _wallet, finalPrice * rateFee / 100);
    ...
    function setFee(uint256 _fee, uint256 _rateFee) public onlyRole(DEFAULT_ADMIN_ROLE) returns (bool){
        require(_rateFee < 100, "Rate fee isIncorrect");
        fee = _fee;
        rateFee = _rateFee;
        return true;
    }
