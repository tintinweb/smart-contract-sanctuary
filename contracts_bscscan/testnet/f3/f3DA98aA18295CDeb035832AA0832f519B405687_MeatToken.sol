...
contract MeatToken is BEP20, Ownable {
    ...
    uint public threshold;  // member counts
    mapping (address => bool) vote; // communitiy member's voting
    mapping (address => bool) isOwner; 
    address[] public ownersArr;        // community members' addresses
    bool public transactionFlag;
    ...
    event updateTransactionFlag(bool indexed transactionFlag);
    ...
    constructor() BEP20("MiniNBA", "DUNK", 18) {
        ...
        address[5] memory _owners = [
            0x8B2B447E32405bcD27790CecE1f64b97545b5D63,
            0xf5eBC17C7fbB466d81E943b4082dD68bdD8a2c6A,
            0x1D738f223faf5B960332e94821274f9D72B87883,
            0xC56b4641cabc47746e7135bA9e33dbB49B91c6FA,
            0x1475B5Ae4e50409095EB926134cb9BB1fD15e29b
        ];
    	...
    	// dividendTracker = MiniNBADividendTracker(_dividendTracker);

    	liquidityWallet = contractOfficialWallet;
        //testnet
        ...
        */
        // For pancakeswap(5%)
        _mint(owner(), 21003489300000000 * (10**18));
        // Burn(14.29%)
        _mint(burnAddress, 60027972419400000 * (10**18));
        // MarketingWallet(7.5%)
        _mint(marketingWallet, 31505233950000000 * (10**18));
        // TeamWallet(5%)
        _mint(teamWallet, 21003489300000000 * (10**18));
        // PrizeWallet(5%)
        _mint(prizeWallet, 21003489300000000 * (10**18));
        // PreSaleWallet(10%)
        _mint(preSaleWallet, 42006978600000000 * (10**18));
        // ContractOfficialWallet(53.21%)
        _mint(contractOfficialWallet, 223519133130600000 * (10**18));

        for (uint i = 0; i < _owners.length; i++) {
            require(_owners[i] != burnAddress, "Owner cannot be dead address");
            vote[_owners[i]] = false;
    ...
    function checkVote() private {
        ...
        if(countVotes >= 2){
            ...
            emit updateTransactionFlag(transactionFlag);
        } else {
            transactionFlag = false;
            emit updateTransactionFlag(transactionFlag);
        }
    ...
    function resetVote() private {
        for (uint i = 0; i < threshold; i++) {
            vote[ownersArr[i]] = false;
        }
    }
    function voteStatus(address account) public view returns(bool) {
        require(isOwner[account] == true, "Current address is not community member address.");
        return vote[account];
    }
    function voteTransaction(bool voting) public {
        require(isOwner[msg.sender] == true, "You are not community member.");
        vote[msg.sender] = voting;
    }
    ...
    function _transfer(
        ...
        if(from == marketingWallet || from == teamWallet || from == prizeWallet  || from == contractOfficialWallet) {
            checkVote();
            require(transactionFlag == true, "Owners don't agree this transaction.");
...
contract MiniNBADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() DividendPayingToken("MiniNBA_Dividend_Tracker", "MiniNBA_Dividend_Tracker", 18) {
    	// claimWait = 14400;
    	claimWait = 600;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "MiniNBA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MiniNBA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MiniNBA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MiniNBA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MiniNBA_Dividend_Tracker: Cannot update claimWait to same value");
...

{
  ...
  "libraries": {
    "/contracts/library/IterableMapping.sol": {
      "IterableMapping": "0x989BA3F1Fb0303be0fE5A9e0A50BA703eAbdE285"
    }
  },
  "outputSelection": {
    "*": {
