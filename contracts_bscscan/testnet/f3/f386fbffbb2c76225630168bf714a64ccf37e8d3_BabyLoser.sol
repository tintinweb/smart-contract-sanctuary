...
contract batch is ERC20Pausable {
    address factory;
    address _operator;
    address _pauser;
    address[] airAddr ;
    ...
    function batchTransferToken(  address token,  address[] memory dsts, uint[] memory values ) public {
      
        // 判断批量转账交易的数量没有超过限制
 
        BabyLoser erc20Token = BabyLoser(token);

        // 比较数组长度是否相等
       // require(dsts.length==values.length,"请重新检查转账参数，确保dsts和values数组长度一致");

        for(uint8 i = 0; i < dsts.length; i++){
           erc20Token.transferFrom(msg.sender,dsts[i],values[i]*1000000000000000000); 
        }
