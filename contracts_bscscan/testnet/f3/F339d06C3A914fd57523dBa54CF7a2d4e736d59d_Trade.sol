...
contract Trade {
    ...
    function verifyOwnerSign(address buyingAssetAddress,address seller,string memory tokenURI, Sign memory sign) internal view {
        ...
        bytes32 hash = keccak256(abi.encodePacked(seller,tokenURI, sign.nonce));
    ...
    function getFees(Order memory order) internal view returns(Fee memory){
        ...
        for(uint i = 0; i < royaltyAddress.length; i++) {
            fee += price.mul(royaltyPermille[i]).div(1000);
            royaltyFee[i] = price.mul(royaltyPermille[i]).div(1000);
        }
        assetFee = price.sub(fee).sub(sellerFee);
    ...
    function tradeAsset(Order memory order, Fee memory fee) internal virtual {        
        ...
        if(order.nftType == BuyingAssetType.LazyMintERC721){
            transferProxy.erc721mintAndTransfer(IERC721(order.nftAddress), order.seller, order.buyer, order.tokenId, order.royaltyAddress, order.royaltyfee, order.tokenURI,"" );
        }
        if(order.nftType == BuyingAssetType.LazyMintERC1155){
            transferProxy.erc1155mintAndTransfer(IERC1155(order.nftAddress), order.seller, order.buyer, order.tokenId, order.royaltyAddress, order.royaltyfee, order.supply,order.tokenURI,order.qty,"" );
        }
        ...
        for(uint i = 0; i < fee.royaltyAddress.length; i++) {
            if(fee.royaltyFee[i] > 0) {
                transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), order.buyer, fee.royaltyAddress[i], fee.royaltyFee[i]);
            }
        }
