...
contract AdvensturesSlot is Context, PartnerOwnable { 
    ...
    function bet(uint[] calldata bets,uint domainId, uint tokensId) external  
        ...
        if( amount > 0 ) { //win
            wins.wins_result = amount;
            require(amount>0,'AdvensturesSlot: bet_win_amount_error'); 
            TransferHelper.safeTransfer(FRUIT, msg.sender, amount);  
        } else if(amount==0) { //tie
            wins.wins_result = 0; 
        } else { //loss
            amount = tot_bet - win_tot - fee1; 
            wins.wins_result = amount;
            require(amount>0,'AdvensturesSlot: bet_loss_amount_error'); 
