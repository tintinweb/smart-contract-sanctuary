...
contract LinearPoolV2 is Ownable, ReentrancyGuard {
    ...
    event LinearPoolCreated(uint256 indexed poolId);
    event LinearDeposit(
    ...
    struct LinearPoolInfo {
        ...
        mapping(address => APRInfo[]) aprInfo;
    ...
    /**
     * @notice Return address of reward token and its APR staked in a pool
     * @param _poolId id of the pool
     * @param _tid reward token id 
     */
    function linearGetRewardTokenInfo(uint256 _poolId, uint256 _tid) 
        public 
        view
        linearValidatePoolById(_poolId) 
        returns(address rewardToken, uint256 APR, uint256 APRUpdateTime) 
    {
        LinearPoolInfo storage pool = linearPoolInfo[_poolId];
        rewardToken = pool.rewardTokens[_tid];
        uint256 lengthAPRInfo = pool.aprInfo[rewardToken].length;
        APR = pool.aprInfo[rewardToken][lengthAPRInfo - 1].apr;
        APRUpdateTime = pool.aprInfo[rewardToken][lengthAPRInfo - 1].aprUpdateTime;
    }

    /**
     * @notice Return user's rewar in a pool
     * @param _poolId id of the pool
     * @param _account address of account
     * @param _rewardToken address of reward token
     */
    function linearGetReward(uint256 _poolId, address _account, address _rewardToken) 
        public
        view 
        linearValidatePoolById(_poolId)
        returns (uint256 _reward)
    {
        _reward = linearPendingReward(_poolId, _account, _rewardToken);
    }


    /**
     * @notice Add a new reward token to a pool. Can only be called by the owner.
     * @param _poolId id of the pool
     * @param _rewardToken address of the reward token
     * @param _apr APR of the reward token
     */
    function linearAddRewardToken(uint256 _poolId, address _rewardToken, uint256 _apr) 
        external 
        onlyOwner 
        linearValidatePoolById(_poolId) 
    {
        require(!linearIsTokenExisted(_poolId, _rewardToken), "LSP: reward token is existed");
        linearPoolInfo[_poolId].rewardTokens.push(_rewardToken);
        linearSetAPR(_poolId, _rewardToken, _apr);
        emit AddNewRewardToken(_poolId, _rewardToken, _apr);
    }

    /**
     * @notice Remove a reward token from a pool. Can only be called by the owner.
     * @param _poolId id of the pool
     * @param _rewardToken address of the reward token
     */
    function linearRemoveRewardToken(uint256 _poolId, address _rewardToken) 
        external 
        linearValidatePoolById(_poolId)
        onlyOwner
    {
        require(linearIsTokenExisted(_poolId, _rewardToken), "LSP: reward token is not existed");
        linearSetAPR(_poolId, _rewardToken, 0);
    }

    /**
     * @notice Update the given reward token's APR in a pool. Can only be called by the owner.
     * @param _poolId id of the pool
     * @param _rewardToken address of the reward token
     * @param newAPR new APR of the reward token
     */
    function linearSetAPR(uint256 _poolId, address _rewardToken, uint256 newAPR) 
        public 
        linearValidatePoolById(_poolId) 
        onlyOwner 
    {
        require(linearIsTokenExisted(_poolId, _rewardToken), "LSP: reward token is not existed");
        LinearPoolInfo storage pool =  linearPoolInfo[_poolId];
        pool.aprInfo[_rewardToken].push(APRInfo({
            aprUpdateTime: block.timestamp,
            apr: newAPR
        }));
        emit APRChanged(_poolId, _rewardToken, block.timestamp, newAPR);
    }

    function linearIsTokenExisted(uint256 _poolId, address _rewardToken) 
        public 
        view 
        linearValidatePoolById(_poolId) 
        returns (bool isExisted) 
    {
        for(uint256 i = 0; i < linearPoolInfo[_poolId].rewardTokens.length; i++){
            if (linearPoolInfo[_poolId].rewardTokens[i] == _rewardToken) {
                isExisted = true;
            }
        }
    } 
    ...
    function linearAddPool(
        ...
        require(
            ...
            "LSP: invalid end join time"
        );
        require(
            _delayDuration <= LINEAR_MAXIMUM_DELAY_DURATION,
            "LSP: delay duration is too long"
        );

        require(
            _rewardToken.length == _apr.length,
            "LSP: the number of reward tokens is not same as of APR"
        );

        LinearPoolInfo storage newPool = linearPoolInfo.push();
        newPool.cap = _cap;
        newPool.totalStaked = 0;
        newPool.minInvestment = _minInvestment;
        newPool.maxInvestment= _maxInvestment; 
        newPool.lockDuration= _lockDuration;
        newPool.delayDuration = _delayDuration;
        newPool.startJoinTime = _startJoinTime;
        newPool.endJoinTime = _endJoinTime;
        newPool.rewardTokens = _rewardToken;
        for(uint256 i = 0; i < _apr.length; i++) {
            newPool.aprInfo[_rewardToken[i]].push(APRInfo({
                aprUpdateTime: block.timestamp,
                apr: _apr[i]
            }));
        }
        newPool.aprInfo[address(0)].push(APRInfo({
            aprUpdateTime: block.timestamp,
            apr: 0
        }));
        emit LinearPoolCreated(linearPoolInfo.length - 1);
    }
        ...
        onlyOwner
    {
        require(
            ...
            "LSP: flexible lock duration is too long"
        );
        ...
        onlyOwner
    {
        require(
            ...
            "LSP: invalid reward distributor"
        );
    ...
    function linearApproveSelfDistributor(uint256 _amount) external onlyOwner {
        require(
            ...
            "LSP: distributor is difference pool"
        );
        linearAcceptedToken.safeApprove(linearRewardDistributor, _amount);
    }

    function linearApproveRewardSelfDistributor(uint256 _poolId, uint256 _amount) external onlyOwner {
        require(
            linearRewardDistributor == address(this),
            "LSP: distributor is difference pool"
        );
        ...
        for (uint256 i = 0; i < pool.rewardTokens.length; i++) {
            if(IERC20(pool.rewardTokens[i]).allowance(address(this), linearRewardDistributor) == 0){
                IERC20(pool.rewardTokens[i]).safeApprove(linearRewardDistributor, _amount);
            }   
        }
    }
        ...
        linearValidatePoolById(_poolId)
    {
        ...
        require(amount > 0, "LSP: nothing is currently pending");
        require(
            pending.applicableAt <= block.timestamp,
            "LSP: not released yet"
        );
     ...
     */
    function linearPendingReward(uint256 _poolId, address _account, address _rewardToken) 
        public
        view
        linearValidatePoolById(_poolId)
        returns (uint256 reward)
    {
        ...
        if(endTime > startTime) {
            (bool existed, uint256 id) = search(_poolId, startTime, _rewardToken);
            if (existed) {
                uint256 initInterests = (pool.aprInfo[_rewardToken][id + 1].aprUpdateTime - startTime) * pool.aprInfo[_rewardToken][id].apr;
                totalInterests = calculateTotalInterests(_poolId, id + 1, endTime, initInterests, _rewardToken);
            } else {
                if (id == 0) {
                    totalInterests = calculateTotalInterests(_poolId, 0, endTime, 0, _rewardToken);
                } else {
                    totalInterests = (endTime - startTime) * pool.aprInfo[_rewardToken][pool.aprInfo[_rewardToken].length - 1].apr;
                }
            }
        }

        uint256 pendingReward = (stakingData.balance *
            totalInterests) / ONE_YEAR_IN_SECONDS / 100;

        reward = stakingData.reward[_rewardToken] + pendingReward;
    }

    function calculateTotalInterests(uint256 _poolId, uint256 startId, uint256 endTime, uint256 initInterests, address rewardToken) 
        internal 
        view 
        returns (uint256 total) 
    {
        LinearPoolInfo storage pool = linearPoolInfo[_poolId];
        uint256 lengthAPRInfo = pool.aprInfo[rewardToken].length;
        total = initInterests + (endTime - pool.aprInfo[rewardToken][lengthAPRInfo - 1].aprUpdateTime) * pool.aprInfo[rewardToken][lengthAPRInfo - 1].apr;
        for (uint256 i = startId; i < lengthAPRInfo - 1; i++) {
            total = (pool.aprInfo[rewardToken][i + 1].aprUpdateTime - pool.aprInfo[rewardToken][i].aprUpdateTime) * pool.aprInfo[rewardToken][i].apr + total;
        }
        return total;
    }

    function search(uint256 _poolId, uint256 startTime, address _rewardToken) 
        internal
        view
        returns (bool existed, uint32 id) 
    {
        LinearPoolInfo storage pool = linearPoolInfo[_poolId];
        uint32 low;
        uint32 high = uint32(pool.aprInfo[_rewardToken].length - 1);
        uint32 mid;
        if(startTime < pool.aprInfo[_rewardToken][0].aprUpdateTime){
            id = 0;
        } else if (startTime >= pool.aprInfo[_rewardToken][high].aprUpdateTime) {
            id = high;
        } else {
            while (low < high && !existed) {
                mid = low + (high - low) / 2;
                if (pool.aprInfo[_rewardToken][mid].aprUpdateTime <= startTime && pool.aprInfo[_rewardToken][mid + 1].aprUpdateTime > startTime) {
                    existed = true;
                    id = mid;
                } else if (pool.aprInfo[_rewardToken][mid].aprUpdateTime > startTime) {
                    high = mid;
                } else if (pool.aprInfo[_rewardToken][mid + 1].aprUpdateTime <= startTime) {
                    low = mid + 1;
                }
            }
        }
        ...
        linearValidatePoolById(_poolId)
    {
        require(
            ...
            "LSP: emergency withdrawal is not allowed yet"
        );
        ...
        for (uint256 i; i < pool.rewardTokens.length; i++) {
            stakingData.reward[pool.rewardTokens[i]] = 0;
        }
    ...
    function _linearClaimReward(uint256 _poolId) internal { 
        ...
        for (uint256 i; i < pool.rewardTokens.length; i++) {
            if (stakingData.reward[pool.rewardTokens[i]] > 0) {
                require(
                    linearRewardDistributor != address(0),
                    "LSP: invalid reward distributor"
                );
                uint256 reward = stakingData.reward[pool.rewardTokens[i]];
                stakingData.reward[pool.rewardTokens[i]] = 0;
                IERC20(pool.rewardTokens[i]).safeTransferFrom(
                    linearRewardDistributor,
                    account,
                    reward
                );
                emit LinearRewardsHarvested(_poolId, account, pool.rewardTokens[i], reward);
            }
        }
    }
    ...
    function _linearHarvest(uint256 _poolId, address _account) private {  
        ...
        for (uint256 i; i < pool.rewardTokens.length; i++) {
            stakingData.reward[pool.rewardTokens[i]] = linearPendingReward(_poolId, _account, pool.rewardTokens[i]);
        }
        stakingData.updatedTime = block.timestamp;
    }
