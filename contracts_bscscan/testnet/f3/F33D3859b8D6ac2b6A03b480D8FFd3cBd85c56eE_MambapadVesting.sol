...
contract MambapadVesting is Auth {
  ...
  function initialDepositVestingAmounts() external onlyOwner {
    ...
    totalVestingAmount = totalVestingAmount.add(teamTotalVestingAmount.mul(4)).add(privateSaleTotalVestingAmount).add(preSaleTotalVestingAmount).add(stakeTotalVestingAmount);
  ...
  function monthWithdraw(address _to) external onlyOwner returns(bool) {
    ...
    MambaContract.transfer(recipient, 1000);
