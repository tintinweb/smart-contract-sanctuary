...
contract MoonBearTokenSale is Ownable {
    ...
    modifier ongoingSale() {
        require(
            block.timestamp >= startTime && block.timestamp <= tokenSaleEndTime() && initialized,
     ...
     * @param _salePrice BNB per token, multiplied by 1e6
    ...
    function calculatePurchaseAmount(uint256 purchaseAmountWei) public view returns (uint256) {
        return (purchaseAmountWei * 1e6) / salePrice / 1e9;
    }
     ...
     */
    function withdrawTokens(uint256 amount) public afterRelease {
