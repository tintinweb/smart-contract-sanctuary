...
contract MasterHunter is Ownable {
    ...
    function pendingLeon(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 leonReward = multiplier.mul(leonPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accLeonPerShare = accLeonPerShare.add(leonReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accLeonPerShare = pool.accLeonPerShare.add(leonReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function _deposit(uint256 _pid, uint256 _amount) internal {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accLeonPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
