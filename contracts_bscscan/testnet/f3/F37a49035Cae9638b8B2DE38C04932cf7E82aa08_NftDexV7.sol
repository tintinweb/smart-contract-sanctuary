...
contract NftDexV7 is ERC721Full, ERC721MetadataMintable, Ownable {
 ...
 // events
  ...
  function claimAuction(uint256 _tokenId) public {
    ...
    if(_royal[_tokenId] > 0) {
      mainPerecentage = SafeMath.sub(SafeMath.sub(1000,_royal[tokenId]),platformPerecentage); //50
      authorPercentage = _royal[_tokenId];
    } else {
      mainPerecentage = 950;
      authorPercentage = 25;
    }
  ...
  function buyNFT(address nft_a,uint256 tokenId, address buyer) public returns (bool) {
        ...
        if(_royal[tokenId] > 0) {
          mainPerecentage = SafeMath.sub(SafeMath.sub(1000,_royal[tokenId]),platformPerecentage); //50
          authorPercentage = _royal[tokenId];
        } else {
          mainPerecentage = 950;
          authorPercentage = 25;
        }
