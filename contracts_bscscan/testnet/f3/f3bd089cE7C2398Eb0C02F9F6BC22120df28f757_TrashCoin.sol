...
contract TrashCoin is Context, IBEP20, Ownable {
    ...
    string private _name = "Trash Coin";
    string private _symbol = "Trash ";
    ...
    uint256 public minTokenToSwap = 10 * 1e9; 
    ...
    uint256 public WalletDfee = 30; // 3% will be added to the buyback address
    uint256 public previousWalletDfee = WalletDfee;
    uint256 public WalletRfee = 40; // 4% will be added to the market address
    uint256 public previousWalletRfee = WalletRfee;
    uint256 public WalletMfee = 30; // 3% will be added to the development address
    uint256 public previousWalletMfee = WalletMfee; 

    uint256 bnbDPercent = 300; // 40% of total tax collected
    uint256 bnbMPercent = 400; // 30% of total tax collected
    uint256 bnbRPercent = 300; // 30% of total tax collected
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "Trash: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "Trash: decreased allowance below zero"
            )
        );
        return true;
    }
    ...
    function includeOrExcludeInwhiteList(address [] memory whitelist)
        external
        onlyOwner
    {
    ...
    function startTrading() external onlyOwner {
        require(!_tradingOpen, "Trash: Already enabled");
        _tradingOpen = true;
    ...
    function _approve(
        ...
        require(owner != address(0), "Trash: approve from the zero address");
        require(spender != address(0), "Trash: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "Trash: transfer from the zero address");
        require(to != address(0), "Trash: transfer to the zero address");
        require(amount > 0, "Trash: Amount must be greater than zero");
        ...
        if (
            ...
            require(amount <= maxTxAmount, "Trash: amount exceeded max limit");

            if ( from == pancakePair && !_tradingOpen )
            {
               require( _iswhitelisted[to], " Sale is open just for whitelisted " );
            }

            else if (!_tradingOpen) {
                require(
                    from != pancakePair && to != pancakePair,
                    "Trash: Trading is not enabled yet"
                );
            }
            if (
            ...
            require( balanceOf(to).add(amount)  <= maxHoldingAmount, "Trash:  amount exceed max holding limit");
             }
