...
contract Groove is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public charityWallet = payable(0x4a2e68c87c6255DbCcB2af84cbB6c6B9C252B7Ac); // charity Address
    ...
    {
        uint256 totalFee = _liquidityFee.add(_marketingFee).add(_charityFee);
        ...
        transferToAddressETH(charityWallet, ethForCharity);
    ...
    {
        if(_charityFee==0 && _marketingFee==0) {  return(tTransferAmount, rTransferAmount); }
        uint256 tCharity = tAmount.div(100).mul(_charityFee.add(_marketingFee));
