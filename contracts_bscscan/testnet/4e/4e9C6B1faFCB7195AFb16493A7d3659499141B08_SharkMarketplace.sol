...
contract SharkMarketplace is Pausable, AccessControl, Utils {
    ...
    event AuctionSuccessful(string indexed _orderId, uint256 indexed _tokenId, address _seller, address _buyer, address _token, uint256 _totalPrice);
    event BreedingSuccessful(string indexed _orderId, uint256 indexed _tokenId, uint256 _sireId, uint256 _matronId, uint256 _genes);
        ...
        internal
    {
        ...
        emit AuctionSuccessful(_orderId, _tokenId, _seller, _msgSender(), token, amount);
    }
    ...
    function _officialIncomeBonus(address token, uint256 amount, address[] memory inviters) private {
        ...
        // 20归团队
        // uint256 _toTeamAmount = amount * 20 / 100;
        uint256 _toTeamAmount = amount - _toStakingAmount - _toFissionAmount;
        ...
        internal
    {
        ...
        // 10% 给团队
        uint256 _toTeamAmount = _daoTokenAmount - _toStakingAmount2 - _toActivityAmount;
        DaoToken.transferFrom(_msgSender(), teamContract, _toTeamAmount);

        emit BreedingSuccessful(_orderId, _tokenId, _sireId, _matronId, _genes);
    }
