...
library Constants {
    ...
    /* Oracle */
    address private constant usdc = address(0xd6CE1E6Ce0C7a862DD98111AFb3Ab10B80EfB83d); // usdc Coin
...
contract Oracle is IOracle {
    ...
    constructor(address SGO, address priceconsumer) public {
        _dao = msg.sender;
        _SGO = SGO;
        USDCPrice = IUSDCPrice(priceconsumer);
    }

    function setup() public override onlyDao {
        _pair = IUniswapV2Pair(IUniswapV2Factory(PANCAKE_FACTORY).createPair(_SGO, usdc()));

        (address token0, address token1) = (_pair.token0(), _pair.token1());
        _index = _SGO == token0 ? 0 : 1;

        require(_index == 0 || _SGO == token1, "SGO not found");
    }

    function fetchSGOPriceInDollar() public view returns(uint256){
        (uint112 reserve0, uint112 reserve1, ) = _pair.getReserves();
        uint256 usdcPrice = uint256(USDCPrice.getLatestPrice());
        uint256 price = _index == 0? (reserve1.mul(10**8)).div(reserve0): (reserve0.mul(10**8)).div(reserve1);
        
        return(price.mul(usdcPrice));
    }
