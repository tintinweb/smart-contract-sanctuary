...
contract shpadStaking {
    ...
    */
    ...
    address public contractAddr = 0x5876dccff89B8D4596B8A62cC9a7A4B1b0AF553c;
    ...
    function stakeTransfer(uint amt) external {
        address sender = msg.sender;
        BEP20 token = BEP20(stakeTokenAddr);
            
        require(token.balanceOf(sender) >= amt, "Insufficient balance of user");
        require(amt >= 1 * 10**18, "Minimun stake is 1 token");
        token.transfer(contractAddr, amt);
    }
