...
/** 
    ...
    @notice Version v1.0.3  date: oct 12, 2021
*/
...
contract PresaleTest is ReentrancyGuard, Ownable, Pausable {
   ...
   modifier claimDisabled {
    ...
    /// @param msg.sender
    /** @dev create claims
     *  to be excecuted after the presale by the admin in order to create
     *  records for investor to claim their tokens 
     */
    function createClaims() external onlyOwner claimDisabled whenFinished { // 
       ...
       for(uint _i = 0 ; _i < userAdresses.length ; _i++) {
            ...
            _userClaim =((_swapped / hardCap ) * tokenSupply)  / (10**18); 
    ...
    /// @param msg.sender
    /** @dev create vesting claims
     *  to be excecuted after the presale by the admin in order to create
     *  records for investor to claim their tokens 
     *  vesting has an extra future to distributed the tokens over more records and scheduled future transfers
     */
     ...
     *  _vestNumber = 0 is the 1st record and default used for no-vesting sales
     */
