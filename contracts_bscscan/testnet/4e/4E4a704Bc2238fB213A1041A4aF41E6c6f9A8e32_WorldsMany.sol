...
contract HolderRewarderDistributor is IHolderRewarderDistributor {
  ...
  function calculateShareOfShareHolder(address shareholderAddress, bool excludeIfRewarded) private view returns (uint256, uint256) {
    ...
    for (uint256 i = 0; i < _shareHoldersTxShares[shareholderAddress].length; i++) {
      ...
      if (txDateTime + 5 minutes < blockTimestampLast) {
        ...
        if (txDateTime + 10 minutes < blockTimestampLast)
        {
          longTermHolds += share.amount;
          //_shareHoldersTxShares[shareholderAddress][i].rewardDateTime = blockTimestampLast;
        }
        else
        {
          midTermHolds += share.amount;
          //_shareHoldersTxShares[shareholderAddress][i].rewardDateTime = blockTimestampLast;
        }
      }
    }
  ...
  {
      if (longTermHolds > 0) {
        uint256 busdBalance = BUSD.balanceOf(_longAddress);
        uint256 perc = longTermHolds.div(_snapshotLongTotalShares);
        BUSD.approve(_longAddress, busdBalance);
        BUSD.transferFrom(_longAddress, shareholder, (busdBalance.mul(perc)));
      }
  }
  ...
  function distributeMidDividend(address shareholder, uint256 midTermHolds) internal onlyMid {
      if (midTermHolds > 0) {
        uint256 busdBalance = BUSD.balanceOf(_midAddress);
  ...
  function getTotalShares(address shareholder) external view returns (uint256) {
    ...
    (uint256 shareHolderLongTermHold, uint256 shareHolderMidTermHold) = calculateShareOfShareHolder(shareholder, false);
    amount += shareHolderLongTermHold;
    amount += shareHolderMidTermHold;
    return amount;
  }
...
contract WorldsMany is IBEP20, Authorization {
  using SafeMath for uint256;
  ...
  uint256 private _initalFragmentsSupply = 50000 * 10**_decimals;
  uint256 private _maxTxAmount = 500 * 10**_decimals;
  uint256 private _numTokensSellToAddToLiquidity = 25 * 10**_decimals;

  string private _symbol = "WRLD"; 
  string private _name = "Worlds Many";
  ...
  address public _operationsAddress = (0x2a45BcEf8f9429ea40E427f325bea33b9CE866dA);
  address public _longTermAddress = (0xF978756addF2eC2A3942D344635b99ccD05109CE);
  address public _midTermAddress = (0x6559e3c6C6b0c41717271754eA0FB5f8b37F3d73);
  address public _lottoAddress = (0x3bD80135d5EDC56B82377117f773Dfc4f8717768);
  // IBEP20 BUSD = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
  ...
  modifier onlyRewards() {
      require(msg.sender == _midTermAddress || msg.sender == _longTermAddress); _;
  }
  ...
  constructor() Authorization(msg.sender) {
    ...
    _isExcludedFromFee[_longTermAddress] = true;
    _isExcludedFromFee[_midTermAddress] = true;
    _isExcludedFromFee[_lottoAddress] = true;
    _isExcluded[_operationsAddress] = true;
    _isExcluded[_longTermAddress] = true;
    _isExcluded[_midTermAddress] = true;
  ...
  * @dev Total shares of address within holders shares
  */
  function getSnapshotMidTermRewards() public view returns (uint256) {
    (uint256 longTerm, uint256 midTerm) = distributor.getSnapshotTotalShares();
    return midTerm;
  }
  ...
  * @dev Total shares of address within holders shares
  */
  function getSnapshotLongTermRewards() public view returns (uint256) {
    (uint256 longTerm, uint256 midTerm) = distributor.getSnapshotTotalShares();
    return longTerm;
  }
  ...
  * @dev Unlocks the address this passed through
  */
  function excludeInFee(address account) public onlyOwner {
  ...
  */
  ...
  function depositWallets() external onlyOwner {
    uint256 longTermBalance = balanceOf(_longTermAddress);
    uint256 midTermBalance = balanceOf(_midTermAddress);
    uint256 lottoBalance = balanceOf(_lottoAddress);
    uint256 totalAmount = 
        longTermBalance.add(midTermBalance).add(lottoBalance);
    if (totalAmount > 0) {
        ...
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: address(this).balance.sub(balanceBefore)}(
              0,
              path2,
              address(this),
              block.timestamp
        ...
        if (busdAmount > 0) { 
          if (longTermBalance > 0) {
            BUSD.transfer(
              _longTermAddress
              , longTermBalance.div(totalAmount).mul(busdAmount));
            _balances[_longTermAddress] = _balances[_longTermAddress].sub(_balances[_longTermAddress]);
          }
          if (midTermBalance > 0) {
            BUSD.transfer(
              _midTermAddress
              , midTermBalance.div(totalAmount).mul(busdAmount));
            _balances[_midTermAddress] = _balances[_midTermAddress].sub(_balances[_midTermAddress]);
          }
          if (lottoBalance > 0) {
            BUSD.transfer(
              _lottoAddress
              , lottoBalance.div(totalAmount).mul(busdAmount));
            _balances[_lottoAddress] = _balances[_lottoAddress].sub(_balances[_lottoAddress]);
          }
        }
    }
  }
  ...
  function processRewards() public onlyRewards {
  ...
  function setLottoAddress(address _address) public onlyOwner() {
