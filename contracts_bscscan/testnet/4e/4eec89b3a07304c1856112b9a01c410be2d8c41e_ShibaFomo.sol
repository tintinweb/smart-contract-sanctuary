...
contract ShibaFomo is BEP20, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;
    bool public bpEnabled;
    uint256 public maxSupply =  1 * 10** 6 * 10 ** 18; //1M token
    

    uint256 public buyOperationFee = 5;
    uint256 public DecreaseFee = 2;
    ...
    constructor(string memory _name, string memory _symbol, address poolMarketingAddress) BEP20(_name, _symbol) 
    {
    ...
    function _transfer( address sender, address recipient, uint256 amount ) internal virtual override {
        ...
        if(
            ...
            if(recipient == uniswapV2Pair){
                _sellfee = amount.mul(sellfee).div(100);
                _operationFee = amount.mul(DecreaseFee).div(100);
            } else {
                _operationFee = amount.mul(buyOperationFee).div(100);
            }
            if(_sellfee > 0 && _operationFee > 0){
                amount = amount.sub(_sellfee.add(_operationFee));
    ...
    function setBuyFee(uint256 operationFee, uint256 sellfee) external onlyOwner {
    ...
    function removeExcludeFromFee( address[] memory users) external onlyOwner {
     ...
     function SetDecreaseFee(uint256 operationFee, uint256 sellfee) external onlyOwner {
        DecreaseFee = operationFee;
        sellfee = sellfee;
    }
