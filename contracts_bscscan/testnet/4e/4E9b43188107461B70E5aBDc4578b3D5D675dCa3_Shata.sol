 ...
 /**   
	...
	**/
        ...
        returns (RoundInfo memory info)
    {
    	...
    	return RoundInfo({
    		rid: diffMonth,
    		startTime: releaseTime.add(valid.mul(diffMonth).add(validFormat)),
    		endTime: releaseTime.add(valid.mul(diffMonth.add(1)).add(validFormat)),
    	...
    	returns (bool)
	{
		...
		Deposited storage dep = depositedOf[rinfo.rid.sub(valid).add(1)][msg.sender];
		/**
		 * 检查本周期是否已结束
		 * 
		 **/
		 ...
		 * 实现投资目标 结算收益
		 **/
		if(member >= tabulation[rinfo.rid.sub(valid).add(1)].length.mul(valid)){
			// 根据投资轮次用户列表进行结算

			for(uint256 i=0;i<tabulation[rinfo.rid.sub(valid).add(1)].length; i.add(1)){
				address Sender = tabulation[rinfo.rid.sub(valid).add(1)][i];
				uint256 total = _totalReward(Sender, rinfo.rid.sub(valid).add(1)); // 计算参与轮次的投入数量
				/**
				 * 当存量资金不足以支付时，报错回滚
				 *
				 **/
				...
				delete depositedOf[rinfo.rid.sub(valid).add(1)][Sender];
				delete tabulation[rinfo.rid.sub(valid).add(1)][i];
				member = member.sub(1);
			}
			...
			for(uint256 i=0;i<tabulation[rinfo.rid.sub(valid).add(1)].length; i.add(1)){
				address Sender = tabulation[rinfo.rid.sub(valid).add(1)][i];
				uint256 amount = 0;
				for(uint256 p = 0; p < depositedOf[rinfo.rid.sub(valid).add(1)][Sender].records.length; p.add(1)){
					amount = amount.add(depositedOf[rinfo.rid.sub(valid).add(1)][Sender].records[p].amount);
				}
				/**
				 * 当存量资金不足以支付时，报错回滚
				 *
		...
		returns (uint256 total)
	{
		...
		for(uint256 i=rid; i < valid; i.add(1)){
