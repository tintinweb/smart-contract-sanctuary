...
contract BME is ERC20, Ownable {
    ...
    uint256 public  RewadTokens;
    uint256 public  swapTokens;
    uint256 public  sellTokens;
    ...
    function _transfer(
        ...
        if(antiWhaleRestriction[from].add(oneDayRestriction) >= block.timestamp && from != liquidityWallet && automatedMarketMakerPairs[to] ){
        ...
        if(
            ...
            RewadTokens =  SellFee.mul(TokenRewardFee.div(SelltotalFees)).add(BuyFee.mul(TokenRewardFee).div(BuytotalFees));
            lasttokens = lasttokens.add(RewadTokens);
            
            
            swapTokens = SellFee.mul(SellliquidityFee).div(SelltotalFees).add(BuyFee.mul(BuyliquidityFee).div(BuytotalFees));
            swapAndLiquify(swapTokens);
            
            
            sellTokens = SellFee.mul(BTCRewardsFee.div(SelltotalFees)).add(BuyFee.mul(BTCRewardsFee).div(BuytotalFees));
    ...
    function calculetBMEDividends() private {
        ...
        uint256 rewardPerToken = lasttokens.mul(magnitude).div(totalSupply());
                lasttokens = 0;
            if(rewardPerToken > 0){
