...
contract BEP20PHB is Context, iBEP20, Ownable {
  ...
  PHBOld oldPHB = PHBOld(0xF09f5E21F86692C614D2D7B47E3b9729DC1C436F);
 ...
 /**
  ...
  /** @dev Burn `amount` old PHB tokens and mint `amount` divide 100  new PHB tokens. 
   *
   * Requirements
   *
   * - `amount` must be more than zero and mode 1000000000000 equal to zero.
   */
  function transferToBurn(uint256 amount) public{
    require(amount > 0 , "BEP20: transferToBurn amount not more than zero");
    require(amount.mod(1000000000000)==0, "BEP20: transferToBurn amount mod 1000000000000 not equal to zero");
    oldPHB.burnFrom(_msgSender(), amount);
    _mint(_msgSender(), amount.div(1000000000000));
  }
  ...
  function inflation() public onlyOwner{
    ...
    uint time = nowTime.sub(_lastInflationTime);
    uint n = time.div(86400).add(1);
    uint day_inflation= _totalSupply.mul(_inflationRate).div(100).div(365);
    uint n_day_inflation =  day_inflation.mul(n);
