...
contract FundRaise is Ownable, ReentrancyGuard {
    ...
    modifier onlyCreator{
        require(isCreator[msg.sender],"caller is not a pool creator");
        _;
    }
    ...
    event CreatePool(PoolInfo indexed poolInfo);
    event SetCreator(address owner, address indexed Creator, bool indexed status);
    event updatePool(address account, uint256 indexed poolID, uint256 maxUsers, uint256 indexed minDeposit, uint256 indexed maxDeposit);
    ...
    function setCreator(address _account, bool _status) external onlyOwner {
        isCreator[_account] = _status;
        emit SetCreator(msg.sender, _account, _status);
    }
    
    function createPool( PoolInfo memory _poolDetails) external onlyCreator {
        ...
        emit CreatePool(_poolDetails);
    }
    
    function stopBlock(uint256 _poolID) external onlyCreator{
        require(poolInfo[_poolID].endBlock < block.number,"Pool already ended");
        poolInfo[_poolID].endBlock = block.number;
    }
    
    function setPool(uint256 _poolID, uint256 _maxUsers, uint256 _minDeposit, uint256 _maxDeposit) external onlyCreator{
    ...
    function deposit(uint256 _poolID, uint256 _amount) external payable {
        ...
        require(pool.tokenLimit >= rewards[_poolID].depositTokens,"Deposit :: deposit amount reached" );
        require(pool.userLimit >= rewards[_poolID].members,"Deposit :: pool depositor reached");
        require(pool.startBlock <= block.number && pool.endBlock >= block.number,"Deposit time exceed");
        ...
        if(address(pool.depositToken) == address(0x0)){
            ...
            rewards[_poolID].depositTokens += msg.value;
        } else {
    ...
    function claim(uint256 _poolID) external onlyCreator {
        PoolInfo storage pool = poolInfo[_poolID];
    ...
    function transferToHolders(IBEP20 _token, address[] calldata recipients, uint256[] calldata amounts) external onlyCreator {
