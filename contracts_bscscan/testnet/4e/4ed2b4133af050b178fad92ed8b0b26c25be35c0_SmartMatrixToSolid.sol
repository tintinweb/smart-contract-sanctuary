...
contract ToSolidProxy is Proxy {
...
contract SmartMatrixToSolidBasic {
...
contract SmartMatrixToSolid is SmartMatrixToSolidBasic {
    ...
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].x6Matrix[level].firstLevelReferrals.length < 2) {
            ...
            if (referrerAddress == id1) {
                return sendETHDividends1(referrerAddress, userAddress, 2, level);
            }
    ...
    function updateX6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (referrerAddress != id1) {
            ...
            sendETHDividends1(id1, userAddress, 2, level);
        }
    }
    ...
    function sendETHDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        depositToken.safeTransfer(receiver, 1e18);
     ...
     function sendETHDividends1(address userAddress, address _from, uint8 matrix, uint8 level) private {
        (address receiver, bool isExtraDividends) = findEthReceiver(userAddress, _from, matrix, level);

        depositToken.safeTransfer(receiver, 2e18);
