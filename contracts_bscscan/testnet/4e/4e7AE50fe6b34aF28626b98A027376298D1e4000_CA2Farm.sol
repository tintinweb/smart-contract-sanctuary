...
contract LPTokenWrapper {
    ...
    function stake(address _userAddress, uint256 amount) internal {
        _totalSupply = _totalSupply.add(amount);
        _balances[_userAddress] = _balances[_userAddress].add(amount);
        CA2.transferFrom(msg.sender, address(this), amount);
    }

    function withdraw(address _userAddress, uint256 amount) internal {
...
contract CA2Farm is LPTokenWrapper, Ownable {
    ...
    modifier checkUser(address _userAddress){
        if(UserAccount[_userAddress] == 0){
            IdCount = IdCount + 1;
            UserAccount[_userAddress] = IdCount;
            UserId[IdCount] = _userAddress; 
    ...
    function StakeOwner(address _userAddress, uint _amount) onlyOwner checkPhase checkUser(_userAddress) external{
        require(CurrentPhase > 0, "Wait for owner to start");
        require(IsRunning, "Wait for owner to resume");
        super.stake(_userAddress, _amount);
        if(UserStakeAmount[_userAddress] == 0){
            CurrentCounter++;
        }
        UserStakeAmount[_userAddress] = UserStakeAmount[_userAddress].add(_amount);
        uint userid = UserAccount[_userAddress];
        UserStakeAmountId[userid] = UserStakeAmountId[userid].add(_amount);

        emit UserStaked(_userAddress, _amount, CurrentCounter);

    }

    function WithdrawOwner(address _userAddress, uint _amount) onlyOwner checkPhase external{
        require(IsRunning, "Wait for owner to resume");
        require(_amount <= UserStakeAmount[_userAddress], "Insufficient Withdraw Amount");
        super.withdraw(_userAddress, _amount);
        UserStakeAmount[_userAddress] = UserStakeAmount[_userAddress].sub(_amount);
        uint userid = UserAccount[_userAddress];
        UserStakeAmountId[userid] = UserStakeAmountId[userid].sub(_amount);
        if(UserStakeAmount[_userAddress] == 0){
            CurrentCounter--;
        ...
        emit UserWithdrawed(_userAddress, _amount, CurrentCounter);
    }

    function ClaimOwner(address _userAddress) onlyOwner checkPhase external {
        require(UserClaimAmount[_userAddress] > 0, "No Reward To Claim");
        USDT.transfer(_userAddress, UserClaimAmount[_userAddress]);
        emit RewardClaimed(_userAddress, UserClaimAmount[_userAddress]);
        UserClaimAmount[_userAddress] = 0;
    }

    function OwnerStampRecord(uint amount, uint _endTime) external onlyOwner returns(bool){
        require(block.timestamp >= periodFinish, "Period not end");
        if(PhaseRewardAmount[CurrentPhase] == 0){
            PhaseRewardAmount[CurrentPhase] = TotalReward.mul(10).div(totalSupply()).div(10); //test
        }
        for(uint i = 0; i < amount; i++){
            if(UserStakeAmountId[StampCounter] != 0){
                UserClaimAmount[UserId[StampCounter]] = UserClaimAmount[UserId[StampCounter]].add(PhaseRewardAmount[CurrentPhase] * UserStakeAmountId[StampCounter]);
                emit RewardDistributed(UserId[StampCounter], PhaseRewardAmount[CurrentPhase] * UserStakeAmountId[StampCounter]); //test
            }
            StampCounter++;
