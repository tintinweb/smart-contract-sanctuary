...
interface I_Windmill_Deployer is I_With_PancakeRouter, I_With_BUSDToken, I_With_WBNBToken{
    struct Windmill{
        ...
        I_Windmill_Competition competition;
...
interface I_Windmill_Trade_Deployer_Long is I_Windmill_Trade_Deployer_Swap{
}
// Version: 1.0.0
...
contract Windmill_Deployer is Base, Adressable, Ownable, With_PancakeRouter, With_BUSDToken, With_WBNBToken, I_Windmill_Deployer{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    function deployUpdater(bytes memory bytecode) external onlyOwner returns (address payable){
        address payable addr = _deployBytecode(bytecode);
    ...
    function deployCompetition(bytes memory bytecode) external onlyOwner returns (address payable){
        address payable addr = _deployBytecode(bytecode);
        
        windmill.competition = I_Windmill_Competition(addr);
    ...
    function initWindmill(uint256[16] calldata data,
                          uint256[] calldata proposalLevel,
        ...
        windmill.PWR.grantRole(role, address(windmill.competition));
        ...
        //Setup Competition
        windmill.competition.setPWRToken(windmill.PWR);
        windmill.competition.setFund(fundAddress);
        windmill.competition.setTradeManager(tradeManagerAddress);
        
        windmill.competition.grantRole(tradeManagerRole, tradeManagerAddress);
        
        role = windmill.competition.UPDATER_ROLE();
        windmill.competition.grantRole(role, updaterAddress);
        
        windmill.competition.grantRole(DAORole, DAOAddress);
        windmill.competition.revokeRole(DAORole, thisAddr);
        ...
        windmill.tradeManager.setCompetition(address(windmill.competition));
        ...
        windmill.updater.setCompetition(address(windmill.competition));
        ...
        windmill.DAO.setCompetition(address(windmill.competition));
        windmill.DAO.init(data, proposalLevel);
    ...
    ////
    //////////////// Private functions ////////////////
    function _deployBytecode(bytes memory bytecode) internal returns (address payable){
