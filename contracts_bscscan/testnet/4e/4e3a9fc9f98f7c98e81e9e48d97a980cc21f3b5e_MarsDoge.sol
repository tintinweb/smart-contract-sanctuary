...
contract MarsDoge is Context, IBEP20, Ownable {
    ...
    /**
     */
    function burnDeposit(uint256 amount) private {
        burnAmount = burnAmount.add(amount);
        emit BurnAmount(200, burnAmount, amount);
    }

    function burnMyself() private {
        // todo
        emit BurnEnabled(100, shouldBurn(), burnAmount);
        if (shouldBurn() && burnAmount > 0) {
            // swapDogeForTokens(burnAmount);
    ...
    function swapDogeForTokens2(uint256 tokenAmount) external {
        address[] memory path = new address[](3);
        path[0] = DOGE;
        path[1] = pancakeRouter.WETH();
        path[2] = address(this);
