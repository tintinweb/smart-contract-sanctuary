...
contract MembershipPool is ReentrancyGuard,Authorizable{
    ...
    constructor(address[N_COINS] memory _tokens,address _sportBettingAddress,address _controlledToken,address _insuranceWallet, address _strategy) public  {
    ...
    function changeMiniLevelLimit(uint256 _miniLevelLimit) public onlyOwner() {
        miniLevelAmount = _miniLevelLimit;
    }
    ...
    function checkTerminat(address _add,uint _amount) internal view returns(bool) {
        ...
        if(availableAmount < miniLevelAmount){
            return true;
        }
        return false;
    }
    ...
    function userDeposit(uint256 amount,uint256 _index) public nonReentrant() validAmount(amount){
        ...
        if(members[msg.sender].totalAmount == 0){
            require(amount >= miniLevelAmount ,"MembershipPool: amount is less for membership");
        }
    ...
    function terminateMembership(bool _payFee) public nonReentrant(){
        uint256[N_COINS] memory _amounts;
        uint _total;
        _amounts = members[msg.sender].tokensAmount;
        require(checkValidArray(_amounts),"user amount is zero");
        if(_payFee){
            uint256[N_COINS] memory _withdrawAmountsFromPool;
