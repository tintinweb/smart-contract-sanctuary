...
contract NFTRouter is Ownable{
    ...
    uint256 public evolveDroidBotFee;
    ...
    function createDroidBot(uint256 _pandoBoxId) external {
        if (pandoBox.ownerOf(_pandoBoxId) == msg.sender) {
            ...
            droidBot.create(msg.sender, _info.level);
            uint256 _jNumber = dataStorage.getJDroidBotCreating(_info.level);
            jackpot.enter(msg.sender, _jNumber);
        }
    }

    function evolveDroidBot(uint256 _droidBot0Id, uint256 _droidBot1Id) external{
        if (droidBot.ownerOf(_droidBot0Id) == msg.sender && droidBot.ownerOf(_droidBot1Id) == msg.sender) {
            if (evolveDroidBotFee > 0) {
                ERC20Burnable(address(PSR)).burnFrom(msg.sender, evolveDroidBotFee);
            }
            NFTLib.info memory _droidBot0Info = droidBot.info(_droidBot0Id);
            NFTLib.info memory _droidBot1Info = droidBot.info(_droidBot1Id);
            droidBot.burn(_droidBot0Id);
            droidBot.burn(_droidBot1Id);
            uint256 _p0lv = _droidBot0Info.level;
            uint256 _p1lv = _droidBot1Info.level;
            if (_p0lv < _p1lv) {
                uint256 c = _p0lv;
                _p0lv = _p1lv;
                _p1lv = _p0lv;
            }
            droidBot.evolve(msg.sender, _p0lv, _p1lv);
            uint256 _jNumber = dataStorage.getJDroidBotEvolving(_p0lv, _p1lv);
            jackpot.enter(msg.sender, _jNumber);
        }
    }
    ...
    function setEvolveDroidBotFee(uint256 _newFee) external onlyOwner {
        evolveDroidBotFee = _newFee;
    }

    function setJackpotAddress(address _addr) external onlyOwner {
        jackpot = IJackpot(_addr);
    }
...
interface IPandoBox is IERC721 {
    function create(address receiver) external;
