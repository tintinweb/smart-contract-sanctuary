...
contract KsharkIDO is Ownable, ReentrancyGuard {
  ...
  bool public isClaimAll = false; // Bool is claim all when end season ( case season 1 - 2 - 3... not claim )
  uint256 public totalLockTime; // time lock token after sold
  uint256 public timeClaimDistance; // Time distance per season claim
  uint256 public season = 1; // Number season now
  uint256 public totalSeason; // Total season (totalLockTime / timeClaimDistance)
  mapping(address => mapping(uint256 => bool)) public seasonUserClaims; // List user claim per season open
  mapping(address => uint256) public userAmountPerTime; // Amount user can claim per season;
  mapping(address => uint256) public amountLeftOfUser; // Amount token left when user claim per time
  ...
  string public linkTelegram;
  string public linkTwitter;
  string public linkGithub;
  ...
  function setStringInfo(
    string calldata _saleTitle,
    string calldata _linkTelegram,
    string calldata _linkGithub,
    string calldata _linkTwitter,
  ...
  // Set Time lock Token sold and distance claim
  function setTimeLockAndDistanceClaim(
    uint256 _totalLockTime,
    uint256 _timeClaimDistance
  ) external onlyDev {
    totalLockTime = _totalLockTime;
    timeClaimDistance = _timeClaimDistance;

    totalSeason = _totalLockTime.div(_timeClaimDistance);
  }
  ...
  // Increase season
  function increaseSeasonClaim() external onlyDev {
    require(season.add(1) <= totalSeason, "This end season!");
    ...
    onlyClaimAllowed
  {
    ...
    if (season == 1) {
      uint256 totalTokenUserCanClaim = getTokenAmount(investments[user]);
      amountLeftOfUser[user] = totalTokenUserCanClaim;

      // Amount user can claim per time season
      uint256 amountCanClaimPerSeasonOfUser = totalTokenUserCanClaim.div(
        totalSeason
      );
      userAmountPerTime[user] = amountCanClaimPerSeasonOfUser;
    }

    // Case allow user claim all token
    if (isClaimAll) {
      token.transfer(user, amountLeftOfUser[user]);
      return;
    }

    require(
      seasonUserClaims[user][season] != true,
      "You're has claim this season!"
    );

    require(userAmountPerTime[user] >= 0, "Balance Insufficient");

    seasonUserClaims[user][season] = true;

    token.transfer(user, userAmountPerTime[user]);

    // sub token left of user after claim
    amountLeftOfUser[user] = amountLeftOfUser[user].sub(
      userAmountPerTime[user]
    );
  }
