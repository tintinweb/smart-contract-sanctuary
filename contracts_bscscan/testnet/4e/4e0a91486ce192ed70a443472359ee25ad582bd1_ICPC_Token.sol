...
contract Air_Drop is Ownable {
    ICPC_Token public _tokenAirdrop;
    uint256 public _airdropAmount;
    uint256 public _totalAirdrop;
    mapping(address => uint256) public claimedAirdrop;
    
    constructor() public {
		_totalAirdrop = 10**5 * 2000000*10**18;
	}
	
	function setAirdrop(address _tokenAddress, uint256 _amount) public onlyOwner {
		_tokenAirdrop = ICPC_Token(_tokenAddress);
		_airdropAmount = _amount;
    }
    
    function claimAirdrop() public {
        require(claimedAirdrop[msg.sender] == 0, "user had claimed airdrop");
        bool success = _tokenAirdrop.mintFrozenTokens(msg.sender, _airdropAmount);
        if(success)
            claimedAirdrop[msg.sender] = _airdropAmount;
        emit Deposit(msg.sender, _airdropAmount);
    }
