...
contract StakingRewards {
	...
	uint256 public lockedTime = 1209600; // 14 days
	uint256 public initialTime = 604800; // 7 days
	...
	function rewardPerToken() public view returns (uint256) {
		...
		return rewardPerTokenStored + (((block.timestamp - lastUpdateTime) * rewardRate * 1e6) / _totalSupply);
	}

	function earned(address account) public view returns (uint256) {
		return ((_balances[account] * (rewardPerToken() - userRewardPerTokenPaid[account])) / 1e6) + rewards[account];
	}
