...
contract TokenFT is Context, IERC20, Ownable {
    ...
    Tokenomics:
        ...
        - Max TX: 1,000,000 (1%)
        - 1% fee - automatically redistributed to all holders
        - 3% fee - liquidity pool
        - 3% fee - marketing & development
        - 1% fee - tribute / charity
        - 1% fee - marketing bonus
        Total fees = 9%
    */
    ...
    string private constant _name = "TokenFT";
    ...
    uint256 public _maxTxAmount = 10 * 10**5 * 10**18; // 0.01
...
//    uint256 private constant _TIMELOCK = 31536000; // 1 year
    ...
    function setTributeFeePercent(uint256 tributeFee) external onlyOwner() onlyUnlocked(){
        require(tributeFee <= 100, "Cannot set percentage over 1.00%");
    ...
    function setMarketingFeePercent(uint256 marketingFee) external onlyOwner() onlyUnlocked(){
        require(marketingFee <= 100, "Cannot set percentage over 1.00%");
    ...
    function setMaintenanceFeePercent(uint256 maintenanceFee) external onlyOwner() onlyUnlocked(){
        require(maintenanceFee <= 300, "Cannot set percentage over 3.00%");
