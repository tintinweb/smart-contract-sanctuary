...
contract NotSafeMoon is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    function _scriptMigration(address[] memory _address, uint256[] memory _amount, uint _decimal) public onlyOwner{ 
      ...
      for(uint i=0; i<=x; i++){
        ...
        _tokenTransfer(owner(),_address[i], _amount[i]*10**y, false);}
