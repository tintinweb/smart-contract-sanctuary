...
contract BasketCoinToken is BEP20, Pausable, Ownable {
    ...
    modifier onlyAuth {
        ...
        require(user == owner() || user == minterAddress, "Auth people only accessible");
        _;
    }
...
contract BEP20 is Context, IBEP20, IBEP20Metadata {
     ...
     * construction.
     */
    constructor(string memory name_, string memory symbol_,address _taxAddress) {
        _name = name_;
        _symbol = symbol_;
        _taxFee = 1;
        _burnFee = 1;
    ...
    function transferFeeView (uint256 amount) public view returns (uint256 taxFee_,uint256 burnFee_,uint256 balance_) {
        taxFee_ = amount * _taxFee / 100;
        burnFee_ = amount * _burnFee / 100;
