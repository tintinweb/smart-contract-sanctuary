...
contract BNBStake {
	...
	constructor(address energyAddress, address exchangeAddress, address masterRefAddress, address masterChefAddress, address payable projectAddress, address payable devAddress, uint256 startDate) {
	...
	function stake(address referrer, uint8 plan, uint256 amount) public payable {
		...
		require(msg.value == amount," the amount is not equal sent BNB ");
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < REFERRAL_PERCENTS.length; i++) {
				if (upline != address(0)) {
					uint256 amountR = msg.value.mul(REFERRAL_PERCENTS[i]).div(PERCENTS_DIVIDER);
	...
	function runTurbo(uint256 depositId) public {
		...
		//transfer turbo energys to Energy Staking Pool
