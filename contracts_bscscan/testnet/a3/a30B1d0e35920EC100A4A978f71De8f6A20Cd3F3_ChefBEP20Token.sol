...
contract ChefBEP20Token is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 rewardbps; // percentage of the share
    ...
    function canWithdraw(uint256 pid, address to) public view returns (bool) {
        ...
        uint256 _pendingBEP20Token = user.amount.mul(block.number.sub(user.lastRewardBlock).mul(pool.rewardbps).div(1e12)).add(user.rewardDebt);

        if (pool.lastRewardBlock == 999999999999) {
            if (user.unlocked == false) {
                return false;
            }
        ...
        returns (uint256)
    {
        ...
        uint256 reward = user.rewardDebt.add(user.amount.mul(block.number.sub(user.lastRewardBlock)).mul(pool.rewardbps).div(1e12));
        
        return reward;
    }
    
    // Update the status of the withdraw whitelist
    function updateWithdrawWhitelist(uint256 _pid, address _user, bool _status) external onlyOwner {
        UserInfo storage user = userInfo[_pid][_user];
        user.unlocked = _status;
    }

    // Deposit LP tokens to MasterChef for BEP20Token allocation.
    function deposit(uint256 _pid, uint256 _amount, address _user) public onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        ...
        if (_amount > 0) {
            ...
            //if this is not the first time, need to update 
            if(user.amount != 0)
            {
                user.rewardDebt = user.rewardDebt.add(user.amount.mul(block.number.sub(user.lastRewardBlock)).mul(pool.rewardbps).div(1e12));
                user.lastRewardBlock = block.number;
            }  
            user.amount = user.amount.add(_amount);
            if (user.fundedBy == address(0)) user.fundedBy = _user;
        }
        ...
        internal
    {
        ...
        safeBEP20TokenTransfer(to, _pendingBEP20Token);
