...
contract NftStakingV2 is ERC1155TokenReceiver, Auth {
    ...
    function setrequirements(address Token, address NFT, uint32 cycleLength, uint16 periodLength) external authorized {
        whitelistedNftContract = IERC1155721Transferrable(NFT);
        rewardsTokenContract = IBEP20(Token);
        cycleLengthInSeconds = cycleLength;
        periodLengthInCycles = periodLength;
    }
