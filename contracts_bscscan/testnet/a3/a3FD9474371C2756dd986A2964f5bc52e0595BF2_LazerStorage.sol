...
contract LazerStorage is LazerStorageOwners, ILazerStorageSchema {
    ...
    mapping(uint256 => RecordIndex) public merchantStakeRecordIndex;

    Wallet[] private _wallets;
    StakingRecord[] public _stakingRecords;

    function doesStakeRecordExist(uint256 merchantId) public view returns (bool) {
        return merchantStakeRecordIndex[merchantId].exists;
    }

    function getRecordIndex(uint256 merchantId) public view returns (uint256) {
        require(doesStakeRecordExist(merchantId), "The merchant does not have a record");
        return merchantStakeRecordIndex[merchantId].index;
    }
        ...
        )
    {
        ...
        return (
            ...
            record.derivativeWithdrawn,
            record.derivativeDeposits,
            record.underlyingWithdrawn,
            record.underlyingBalance,
            record.stakeToken,
            record.walletId,
            record.exists
        );
    ...
    function createStakeRecord(
        ...
        RecordIndex memory recordIndex = merchantStakeRecordIndex[merchantId];
        //Create a new record
        require(!doesStakeRecordExist(merchantId), "The merchant already has a record");
        ...
        merchantStakeRecordIndex[merchantId] = recordIndex;
