...
contract StakeMaster is Adminable {
    using SafeMath for uint256;
    using SafeERC20 for ERC20Burnable;
    using SafeERC20 for IERC20;

    IStakeFactory public stakeFactory;
    IStakeFactory public stakePenaltyFactory;
    ...
    uint256 public defaultProfitSharingBP;
    mapping(address => uint256) public profitSharingBP;
    ...
    event StakingPenaltyPoolCreated(
        address owner,
        address pool,
        address stakingToken,
        address poolToken,
    ...
    event SetStakeFactory(IStakeFactory stakeFactory);
    event SetStakePenaltyFactory(IStakeFactory stakePenaltyFactory);
    ...
    constructor(
        ...
        require(_burnPercent <= 100, "Burn percent must be less than divider");
        require(_defaultProfitSharingBP <= MAX_BPS, "Invalid profit sharing BP"); // TODO: 100% limit?

        feeToken = _feeToken;
        feeAmount = _feeAmount;
        feeWallet = _feeWallet;
        burnPercent = _burnPercent;
        defaultProfitSharingBP = _defaultProfitSharingBP;
        divider = 100;
    }

    // address(0) is base asset (eth)
    ...
    function setProfitSharingBP(address _stakingPool, uint256 _profitSharingBP) external onlyOwner {
        require(_profitSharingBP <= MAX_BPS, "Invalid profit sharing BP"); // TODO: 100% limit?
        profitSharingBP[_stakingPool] = _profitSharingBP;
        emit SetProfitSharingBP(_stakingPool, _profitSharingBP);
    }

    function setDefaultProfitSharingBP(uint256 _defaultProfitSharingBP) external onlyOwner {
        require(_defaultProfitSharingBP <= MAX_BPS, "Invalid profit sharing BP"); // TODO: 100% limit?
        defaultProfitSharingBP = _defaultProfitSharingBP;
        emit SetDefaultProfitSharingBP(_defaultProfitSharingBP);
    }

    function setStakeFactory(IStakeFactory _stakeFactory) external onlyOwner {
        stakeFactory = _stakeFactory;

        emit SetStakeFactory(_stakeFactory);
    }

    function setStakePenaltyFactory(IStakeFactory _stakePenaltyFactory) external onlyOwner {
        stakePenaltyFactory = _stakePenaltyFactory;

        emit SetStakePenaltyFactory(_stakePenaltyFactory);
    }
    ...
    function createStakingPool(
        ...
        require(address(stakeFactory) != address(0), "createStakingPool: stakeFactory hasn't been set");

        // transfer fee
        chargeFee();

        address stakingPool = stakeFactory.createStakingPool(
            _stakingToken,
            _poolToken,
            _startTime,
            _finishTime,
        ...
        require(_poolToken.balanceOf(stakingPool) == _poolTokenAmount, "Unsupported token");

        profitSharingBP[stakingPool] = defaultProfitSharingBP;
    ...
    function createStakingPenaltyPool(
        IERC20 _stakingToken,
        IERC20 _poolToken,
        uint256 _startTime,
        uint256 _finishTime,
        ...
        require(address(stakePenaltyFactory) != address(0), "createStakingPool: stakeFactory hasn't been set");

        // transfer fee
        chargeFee();

        address stakingPenaltyPool = stakePenaltyFactory.createStakingPool(
            _stakingToken,
            _poolToken,
            _startTime,
            _finishTime,
        ...
        require(_poolToken.balanceOf(address(stakingPenaltyPool)) == _poolTokenAmount, "Unsupported token");

        profitSharingBP[address(stakingPenaltyPool)] = defaultProfitSharingBP;

        emit StakingPenaltyPoolCreated(
            msg.sender,
            stakingPenaltyPool,
            address(_stakingToken),
            address(_poolToken),
    ...
    function chargeFee() public payable {
        if (feeAmount > 0) {
            ...
            if (address(feeToken) != address(0)) {
                ...
                // feeToken is base asset (eth)
            } else {
                require(msg.value == feeAmount, "Value is not equal to the required fee");
                feeWallet.transfer(feeAmount); // limited gas tx
            }
        }
    }
