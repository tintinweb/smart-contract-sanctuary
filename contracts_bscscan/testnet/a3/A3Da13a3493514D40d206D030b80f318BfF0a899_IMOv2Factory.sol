...
import "./lib/IMOFactory.sol";

contract IMOv2Factory is IMOFactory{
}
...
abstract contract IMOFactory is IIMOFactory, Initializable, OwnableUpgradeable{
    uint256 public override constant VERSION = 0;
    address private immutable implementation;
    address[] public override imoAddresses;
    address private _parent;

    modifier onlyOwnerOrParent(){
        require(address(msg.sender) == owner() ||
                    address(msg.sender) == Parent(),
                "Owner or Parent IMO only.");
        _;
    }
    ...
    function imoAddressesLength() public override view returns(uint256){
    ...
    function createIMO(
    ...
    ) public override onlyOwnerOrParent returns(address){
        require(_hardCap.length == 5, "Invalid hardcap array length.");
        address clone = Clones.clone(implementation);        
        IMOClonable imoClonable = IMOClonable(clone);
        imoClonable.initialize(
            _offerToken,
            _fundToken,
            IIMO(Parent()),
        ...
        imoClonable.transferOwnership(owner());
...
contract IMOClonable is IIMOClonable, Initializable, OwnableUpgradeable{
    ...
    IERC20 public override offerToken;
    IERC20 public override fundToken;
    // Parent Launchpad
    IIMO public imo;

    uint256 public override startCommitTimestamp;
    uint256 public override endCommitTimestamp;
    uint256 public override claimTimestamp;
    uint256 public override withdrawTimestamp;

    uint256 public override raiseAmount;
    uint256 public override offerAmount;

    uint256 public override totalFund;
    ...
    function setOfferToken(IERC20 token) external onlyOwner{
        offerToken = token;
    }

    function setFundToken(IERC20 token) external onlyOwner{
        fundToken = token;
    }

    function setStartCommitTimestamp(uint256 time) external onlyOwner{
        startCommitTimestamp = time;
    }

    function setEndCommitTimestamp(uint256 time) external onlyOwner{
        endCommitTimestamp = time;
    }

    function setClaimTimestamp(uint256 time) external onlyOwner{
        claimTimestamp = time;
    }

    function setWithdrawTimestamp(uint256 time) external onlyOwner{
        withdrawTimestamp = time;
    }

    function setRaiseAmount(uint256 amount) external onlyOwner{
        raiseAmount = amount;
    }

    function setOfferAmount(uint256 amount) external onlyOwner{
        offerAmount = amount;
    }

    function setHardCap(uint256[] memory _hardCap) external onlyOwner{
    ...
    // Tier can be 1 - 5
    function setHardCapByTier(uint256 _hardCap, uint32 tier) external onlyOwner{
        require((tier >= 1 && tier <= 5), "Tier must be 1-5.");
        hardCap[tier-1] = _hardCap;
    }

    function setIMOAddress(IIMO _imo) external onlyOwner{
        imo = _imo;
    }
    ...
    function getHardcapByTier(uint32 tier) public override view returns(uint256){
    ...
    function getUserAllocation(address _user) public override view returns(uint256){
        if(totalFund == 0)return 0;
        return userCommit[_user].commitAmount.mul(1e18).div(totalFund);
    }

    function getOfferAmount(address _user) public override view returns(uint256){
        if(totalFund > raiseAmount){
            uint256 allocation = getUserAllocation(_user);
            return offerAmount.mul(allocation).div(1e18);
        }else{
            return userCommit[_user].commitAmount.mul(offerAmount).div(raiseAmount);
        }
    }

    function getRefundAmount(address _user) public override view returns(uint256){
        if(totalFund <= raiseAmount){
            return 0;
        }
        uint256 allocation = getUserAllocation(_user);
        uint256 amount = raiseAmount.mul(allocation).div(1e18);
        return userCommit[_user].commitAmount.sub(amount);
    }

    function getParticipantLength() public override view returns(uint256){
        return participants.length;
    }

    function getCommitLimit(address _user) public override view returns(uint256){
    ...
    function getCommitUserTier(address _user) public override view returns(uint32){
    ...
    function commit(address _user, uint256 amount) external override onCommitEvent onlyIMO returns(uint256){
    ...
    function _claim(address _user) private returns(uint256, uint256){
        ...
        require(userCommit[_user].claimed == 0, "You have claimed the offer already.");
        ...
        userCommit[_user].claimed = 1;
        emit Claim(_user, offerToken, _offerAmount, fundToken, _refundAmount, userCommit[_user].tier);
        return (_offerAmount, _refundAmount);
    }

    function claim(address _user) external override onClaimEvent onlyIMO returns(uint256, uint256){
        return _claim(_user);
    }

    function claim() public onClaimEvent returns(uint256, uint256){
