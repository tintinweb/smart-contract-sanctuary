...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 NYCAKE = IERC20(0x814230622fB157460dd13eB9Ca7d1e2e9a957938);
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = NYCAKE.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(NYCAKE);
        ...
        uint256 amount = NYCAKE.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            NYCAKE.transfer(shareholder, amount);
...
contract $NYCAKE is IERC20, Ownable {
    using SafeMath for uint256;

    address NYCAKE = 0x814230622fB157460dd13eB9Ca7d1e2e9a957938;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "NYCAKE";
    string constant _symbol = "NYCAKE";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100000000000 * (10 ** _decimals);
    uint256 _maxTxAmount = (_totalSupply * 1) / 1; //5% of total supply
    uint256 _maxWalletSize = (_totalSupply * 1) / 1; // 2% of total supply
    ...
    function setSellMultiplier(uint256 multiplier) external onlyOwner() {
        require(multiplier <= 6 && multiplier * totalFee <= 10000, "Multiplier too high");
