    ...
    AccessControlUpgradeable
{
    ...
    function addListings(address _address, uint256[] memory _idArray, uint8[][] memory _categoryValuesArray) public {
        require(_idArray.length == _categoryValuesArray.length);
        for (uint i = 0; i < _categoryValuesArray.length; ++i) {
            addListing(_address, _idArray[i], _categoryValuesArray[i]);
        }
    }
    ...
    function getCount() public view returns(uint256) {
        ...
        for (uint i = 0; i < tokens.length(); ++i) {
            count += listingSets[tokens.at(i)].length();
        }
        return count;
    }

    function getCountOfToken(address _address) public view returns(uint256) {
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint categoryIndex = _categoryValues.length; categoryIndex < 5; ++categoryIndex) {
            categoryMaxValuesPadded[categoryIndex] = 1;
        }
    ...
    function getListingsCount(address _address, uint8[] memory _categoryValues, bool[] memory _categoryFiltersActive)
        public
        view
        returns (uint256)
    {
    ...
    function isListed(address _address, uint256 _id) public view returns(bool) {
        require(tokens.contains(_address));
        return listingSets[_address].contains(_id);
    }
}