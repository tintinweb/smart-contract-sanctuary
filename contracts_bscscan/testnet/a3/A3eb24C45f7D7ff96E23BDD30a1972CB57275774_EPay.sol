...
contract DividendDistributor is IDividendDistributor {
    ...
    function transferAndBurnRewardDust(address winner) external onlyToken returns (uint256,uint256){
        ...
        if(rotatingDust>rotatingThreshold)
        {
            ...
            remainingDust = rotatingDust.sub(rewardAmountRotating);
            IERC20(TOK2).transfer(DEAD, remainingDust.div(rewardRateDivider).mul(rewardRateMultiplier));
            rotatingDust = rotatingDust.sub(remainingDust.div(rewardRateDivider).mul(rewardRateMultiplier));
        }
        if(defaultDust>defaultThreshold)
        {
            rewardAmount = defaultDust.div(rewardRateDivider).mul(rewardRateMultiplier);
            IERC20(TOK).transfer(winner, rewardAmount);
            remainingDust = defaultDust.sub(rewardAmount);
            IERC20(TOK).transfer(DEAD, remainingDust.div(rewardRateDivider).mul(rewardRateMultiplier));
            defaultDust = defaultDust.sub(remainingDust.div(rewardRateDivider).mul(rewardRateMultiplier));
        }
...
contract EPay is IERC20, Context, Ownable, TokenRecover {
    ...
    */
    ...
    function sendToBlackHole(uint256 tokenAmount) external returns(uint256, uint256, uint256){
        ...
        return (dustAllocation, rewardAmount, rewardAmountRotating);
    }
