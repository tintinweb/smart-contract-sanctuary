...
contract SpintopToken is Ownable, BEP20Pausable, Blacklist {
    ...
    // uint256 constant hardcap = 1000000000000000000000000000;
    uint8 	constant numDecimals = 18;
    uint256 private _circulatingSupply;
    uint256 private _nextUnlockAt;
    uint256 private _lastUnlockAt;
    address[] private _myWallets = [address1, address2, address3, address4, address5, address6, address7, address8];

   //* Addresses to mint initial token supply */////////
    //address constant address1 = 0xd93365a477F4a35D4F59Ff6472071429ad80F21c; //Staking&Liquidity
    //address constant address2 = 0xE6d8dd93c0FAa9263FD3569D6c5C1b58796b3824; //Treasury
    //address constant address3 = 0xC1193acc76F22cEE3153498d57959148cdeB1CBd; //Marketing
    //address constant address4 = 0x6407AfFBeCe77A5cbbDBA51dC9D079Cdcb6Ad43e; //Team
    //address constant address5 = 0x00a2753508abe1d45A967A73ce11c59C29914911; //IDO&StrategicSale
    //address constant address6 = 0x98bd91a8bE0f451f0A06f88Dd06f85D192042173; //Seed Round
    //address constant address7 = 0xF0a693994b40a645DD99D83844BEBc29370e6126; //Initial Liquidity
    //address constant address8 = 0x5C7FfB85031E025Ba80EB450dCdf6fa1e536F5aC; //Airdrop&Bounty
    ...
    uint256[8][] private _monthlyMintingAmounts;
    // Minting vars end //
    
    //Initial Amount to Supply
    uint256 constant _mintingAmount1 = 6500000000000000000000000;  //6.5million tokens
    uint256 constant _mintingAmount2 = 500000000000000000000000; //500thousands tokens
    uint256 constant _mintingAmount3 = 500000000000000000000000; //500thousands tokens
    uint256 constant _mintingAmount4 = 1000000000000000000; //1 tokens
    uint256 constant _mintingAmount5 = 8000000000000000000000000; //8million tokens
    uint256 constant _mintingAmount6 = 3600000000000000000000000; //3.6million tokens
    uint256 constant _mintingAmount7 = 15000000000000000000000000; //15million tokens
    uint256 constant _mintingAmount8 = 1000000000000000000; //1 tokens
    
     // Testing Addresses //
     address constant address1 = 0xEa2ED5Db0E74f198e56ceD89a014867a59D15C23;
     address constant address2 = 0x5FC61B62a957646C1cF01f21aFeA57BF63e3C8AE;
     address constant address3 = 0x85CbC73f02f69a3A05D1dCB3879F0CbFefA755Bf; 
     address constant address4 = 0x8A7F0BB09Ef723c582375211ff2aE63B5fd8a8C9;
     address constant address5 = 0x78b2c40ed1a57c4f0307851F50a21f59227723a9;
     address constant address6 = 0x09F12B460360AAbC8dBD26cb42ce326Cf44B9144;
     address constant address7 = 0xF0a693994b40a645DD99D83844BEBc29370e6126; //Initial Liquidity
     address constant address8 = 0x5C7FfB85031E025Ba80EB450dCdf6fa1e536F5aC; //Airdrop&Bounty

    constructor() Ownable() BEP20('Spintop-Mock','SPIN10')
    {
        _setupDecimals(numDecimals);
        setupMintingDatesandAmounts();
        _mint(address1, _mintingAmount1); //6.5million tokens minted.
        _mint(address2, _mintingAmount2);
        _mint(address3, _mintingAmount3);
        _mint(address4, _mintingAmount4);
        _mint(address5, _mintingAmount5);
        _mint(address6, _mintingAmount6);
        _mint(address7, _mintingAmount7);
        _mint(address8, _mintingAmount8);
        _circulatingSupply = _mintingAmount1 + _mintingAmount2 + _mintingAmount3 + _mintingAmount4 +_mintingAmount5 + _mintingAmount6 + _mintingAmount7 + _mintingAmount8;
    }
    ...
    /// @dev Setups minting dates and amounts, will be called only on initialization
        function setupMintingDatesandAmounts() internal {
        uint256 nextMintingAt = 1637139600; // Sun Nov 17 2021 09:00:00 GMT+0000  MODIFY HERE
        for (uint8 i = 0; i < 3; i++) {
            _mintingDates[i] = nextMintingAt;
            nextMintingAt = nextMintingAt + 5 minutes;
            if (i==0){
                uint256 mintingAmount1 = 6500000000000000000000000;
                uint256 mintingAmount2 = 500000000000000000000000;
                uint256 mintingAmount3 = 500000000000000000000000;
                uint256 mintingAmount4 = 1000000000000000000;
                uint256 mintingAmount5 = 8000000000000000000000000;
                uint256 mintingAmount6 = 3600000000000000000000000;
                uint256 mintingAmount7 = 15000000000000000000000000;
                uint256 mintingAmount8 = 1000000000000000000;
                _monthlyMintingAmounts.push([mintingAmount1, mintingAmount2, mintingAmount3, mintingAmount4, 
                mintingAmount5, mintingAmount6, mintingAmount7, mintingAmount8]);
                _remainingMintingAmount = _remainingMintingAmount+mintingAmount1+mintingAmount2+mintingAmount3+mintingAmount4+mintingAmount5+mintingAmount6+mintingAmount7+mintingAmount8;
            }
            else if (i==1) {
                uint256 mintingAmount1 = 6500000000000000000000000;
                uint256 mintingAmount2 = 500000000000000000000000;
        ...
        function mintingDates() external view returns (uint256[] memory) {
            uint256[] memory returnDates = new uint[](36);
            for(uint8 i = 0; i<3; i++) {
        ...
        function mint() external onlyOwner {
            ...
            //require(_latestMintRound < 3, "Minting is over");
            for(uint8 i = 0; i<8; i++){
                 super._mint(_myWallets[i], _monthlyMintingAmounts[i][currentMintRound()]);
            }
            //_mint(_myWallets[0], _mintingAmounts[currentMintRound()]);
            //_remainingMintingAmount -= _mintingAmounts[currentMintRound()];
            //_circulatingSupply +=_mintingAmounts[currentMintRound()];
            _latestMintRound++;
        }
