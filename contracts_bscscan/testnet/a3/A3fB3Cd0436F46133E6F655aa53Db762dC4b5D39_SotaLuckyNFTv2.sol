...
contract SotaLuckyNFTv2 is Ownable {
    ...
    function getAddressByIndexAndRound(uint256 _roundId, uint256 _number) public view returns (address) {
        ...
        require(_number > 0 && round.addressDraw.length >= _number, "Invalid-number");
        return (round.addressDraw[_number]);
    }
    ...
    function operatorLuckyDraw(uint256 _start, uint256 _end, address[] memory _addressDraw) external {
        ...
        newRound.start = _start;
        newRound.end = _end;
        newRound.addressDraw = _addressDraw;
        newRound.drawTime = now;
        for (uint256 index = 0; index < _addressDraw.length; index++) {
            lotteryNumbers[keccak256(abi.encodePacked(totalRound, _addressDraw[index]))].push(index + 1);
        }
