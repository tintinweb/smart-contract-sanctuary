...
contract SkippyKAR is Context, IERC20, Ownable {
    ...
    function _getCalculatedFees(uint256 amount) internal view returns(uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 deductedAmount = amount.sub(fee.burnFee.add(fee.charityFee).add(fee.liquidityFee).add(fee.rAndDFee).add(fee.marketingFee).add(fee.rewardFee));
    ...
    function deposit(address[] memory beneficiary, uint[] memory amount, uint sum, uint[] memory releaseTime) public payable onlyOwner returns(bool success) {
        ...
        for(uint256 i = 0; i < beneficiary.length; i++) {
            ...
            l.balance = amount[i];
            l.releaseTime = releaseTime[i].mul(86400);
