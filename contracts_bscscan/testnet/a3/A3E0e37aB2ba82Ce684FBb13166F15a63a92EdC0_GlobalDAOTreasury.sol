...
contract GlobalDAOTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SGLBD }
    ...
    address public sGLBD;
    uint public sGLBDQueue; // Delays change to sGLBD address
    ...
    constructor (
       ...
       isLiquidityToken[ _GLBDBUSD ] = true;
       liquidityTokens.push( _GLBDBUSD );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for GLBD
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint GLBD needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( GLBD ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn GLBD for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IGLBDERC20( GLBD ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sGLBD ).balanceOf( msg.sender ); // Can only borrow against sGLBD held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with GLBD
        @param _amount uint
     */
    function repayDebtWithGLBD( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IGLBDERC20( GLBD ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, GLBD, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( GLBD ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( GLBD ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns GLBD valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match GLBD decimals
            value_ = _amount.mul( 10 ** IERC20( GLBD ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SGLBD ) { // 9
            sGLBDQueue = block.number.add( blocksNeededForQueue );
        } else return false;
