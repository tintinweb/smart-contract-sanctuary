...
contract TOAX_NFT is BEP721, BEP721Enumerable, BEP721URIStorage, Pausable, Ownable {

    uint256 public mintFee;
    address public feeWallet;
    event SetMintFee(address indexed owner, uint256 mintFee);
    event SetFeeWallet(address indexed owner, address newFeeWallet);

    constructor(uint256 _mintFee, address _feeWallet) BEP721("WHEELS", "WHEELS") {
        mintFee = _mintFee;
    ...
    function updateMintFee(uint256 _mintFee) public onlyOwner{
        mintFee = _mintFee;
        emit SetMintFee(msg.sender, _mintFee);
    }
    ...
    function safeMint(address to,  string memory uri)
        ...
        payable
    {
        require(msg.value >= mintFee || msg.sender == owner(),"Invalid MintFee");
        uint256 tokenId =  _allTokens.length + 1;
        require(10000 >= tokenId,"Minting Token reached");
        _safeMint(to, tokenId);
