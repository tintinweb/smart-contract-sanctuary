...
import "../interfaces/IERC20UpgradeableShibaMusk.sol";
import "../libs/SafeERC20UpgradeableShibaMusk.sol";

contract ShibaMuskICO is Initializable, ContextUpgradeable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;

    using SafeERC20Upgradeable for IERC20Upgradeable;
    IERC20Upgradeable public tokenBUSD;

    using SafeERC20UpgradeableShibaMusk for IERC20UpgradeableShibaMusk;
    IERC20UpgradeableShibaMusk public tokenShibaMusk;
    ...
    function initialize(
        ...
        address _shibaMuskContractAddress,
        ...
        require(
            _shibaMuskContractAddress != address(0),
        ...
        tokenShibaMusk = IERC20UpgradeableShibaMusk(_shibaMuskContractAddress);
    ...
    function buyICOByBUSD(uint256 amount) external payable {
        ...
        tokenShibaMusk.transferLockToken(sender, buyAmountToken);
    }
    ...
    function claimToken() external onlyAdmin {
        address sender = _msgSender();
        uint256 remainAmountToken = tokenShibaMusk.balanceOf(address(this));
        tokenShibaMusk.safeTransfer(sender, remainAmountToken);
    }

    function setContractToken(address contractToken) external onlyOwner {
        tokenShibaMusk = IERC20UpgradeableShibaMusk(contractToken);
    }
 ...
 */
library SafeERC20UpgradeableShibaMusk {
    using AddressUpgradeable for address;

    function safeTransfer(
        IERC20UpgradeableShibaMusk token,
        address to,
        uint256 value
    ) internal {
        _callOptionalReturn(
 ...
 * @dev Interface of the ERC20 standard as defined in the EIP.
 */
interface IERC20UpgradeableShibaMusk is IERC20Upgradeable, ITransferLockToken {
