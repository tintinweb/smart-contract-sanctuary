...
/**
 *Submitted for verification at hecoinfo.com on 2021-08-15
*/
...
contract MYDA is Context, AFTS {
    ...
    constructor () {
        ...
        _stakeConfig[4].collateral = 40000*1e18;
        _stakeConfig[5].collateral = 80000*1e18;
        _stakeConfig[1].lockTime = 2592000;
        _stakeConfig[2].lockTime = 7776000;
        _stakeConfig[3].lockTime = 15552000;
        _stakeConfig[4].lockTime = 20000000;
        _stakeConfig[5].lockTime = 25552000;
        _stakeConfig[1].reward = 8.25*1e18;
        _stakeConfig[2].reward = 33*1e18;
        _stakeConfig[3].reward = 99*1e18;
        _stakeConfig[4].reward = 120*1e18;
        _stakeConfig[5].reward = 150*1e18;
        _general._isPaused = false;
        _general._stakeAddress = 0xCeF5580136251D687DA50D0A1Ca23Ef76DFE24Eb;
    ...
    function generalDetails() public view virtual override returns(GeneralDetails memory){
        ...
        gendet.t4 = _stakeConfig[4];
        gendet.t5 = _stakeConfig[5];
    ...
    function userDetails(address account) public view virtual override returns(UserDetails memory){
        ...
        userdet.t4 = _stake[account][4];
        userdet.t5 = _stake[account][5];
        return userdet;
    }  
    ...
    function _stakeProcess(address recipient, uint256 tier, uint256 qty, address refAdd, uint256 method) internal virtual {
        ...
        require(method >= 0 && method < 6, "Invalid Method");
    ...
    function _voteProcess(address sender, string memory config) internal {
        ...
        if(_vote[cycle][config].voteCount >= ((_general._activeStakers * _genConfig["voteApproveRate"]) / 100)){
            ...
            } else if(keccak256(bytes(config)) == keccak256(bytes("t4Collateral"))){
                _stakeConfig[4].collateral = _vote[cycle][config].value;
            } else if(keccak256(bytes(config)) == keccak256(bytes("t5Collateral"))){
                _stakeConfig[5].collateral = _vote[cycle][config].value;
            ...
            } else if(keccak256(bytes(config)) == keccak256(bytes("t4Reward"))){
                _stakeConfig[4].reward = _vote[cycle][config].value;
            } else if(keccak256(bytes(config)) == keccak256(bytes("t5Reward"))){
                _stakeConfig[5].reward = _vote[cycle][config].value;
            ...
            } else if(keccak256(bytes(config)) == keccak256(bytes("t4LockTime"))){
                _stakeConfig[4].lockTime = _vote[cycle][config].value;
            } else if(keccak256(bytes(config)) == keccak256(bytes("t5LockTime"))){
                _stakeConfig[5].lockTime = _vote[cycle][config].value;
    ...
    function protocolUpdate(uint256[] memory newConfig, address account, address stakeAddress, string memory info, uint256 status) public virtual override returns (bool) {
        ...
        if(newConfig.length == 16){
            ...
            _stakeConfig[4].collateral = newConfig[15];
            _stakeConfig[4].lockTime = newConfig[16];
            _stakeConfig[4].reward = newConfig[17];
            _stakeConfig[5].collateral = newConfig[18];
            _stakeConfig[5].lockTime = newConfig[19];
            _stakeConfig[5].reward = newConfig[20];
            _general._changeDate = newConfig[21];
        }
