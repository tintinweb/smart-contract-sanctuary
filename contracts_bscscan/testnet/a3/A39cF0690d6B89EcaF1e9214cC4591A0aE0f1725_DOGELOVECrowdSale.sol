...
contract DOGELOVE is Context, IERC20, Ownable {
    ...
    string private _name = "DOGE LOVE";
    string private _symbol = "DOGELOVE";
...
library SafeERC20 {
    ...
    function safeTransfer(DOGELOVE token, address to, uint256 value) internal {
        callOptionalReturn(token, abi.encodeWithSelector(token.transfer.selector, to, value));
    }

    function safeTransferFrom(DOGELOVE token, address from, address to, uint256 value) internal {
        callOptionalReturn(token, abi.encodeWithSelector(token.transferFrom.selector, from, to, value));
    }

    function safeApprove(DOGELOVE token, address spender, uint256 value) internal {
    ...
    function safeIncreaseAllowance(DOGELOVE token, address spender, uint256 value) internal {
        uint256 newAllowance = token.allowance(address(this), spender).add(value);
        callOptionalReturn(token, abi.encodeWithSelector(token.approve.selector, spender, newAllowance));
    }

    function safeDecreaseAllowance(DOGELOVE token, address spender, uint256 value) internal {
     ...
     */
    function callOptionalReturn(DOGELOVE token, bytes memory data) private {
...
contract CROWDSALE is Context, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for DOGELOVE;

    // The token being sold
    DOGELOVE private _token;
     ...
     */
    constructor ( uint256 rate, address payable wallet, DOGELOVE token)  {
        admin = msg.sender;
     ...
     */
    function token_() public view returns (DOGELOVE) {
        return _token;
    }
...
contract AllowanceCrowdsale is CROWDSALE {
     ...
     */
    constructor (address tokenWallet,uint256 rate, address payable wallet, DOGELOVE token) CROWDSALE(rate,wallet,token)   {
...
contract DOGELOVECrowdSale is AllowanceCrowdsale,Ownable {
    uint256 private _minimumContribution;

    constructor(address tokenWallet,uint256 rate, address payable wallet, DOGELOVE token,uint256 minimumContribution) AllowanceCrowdsale(tokenWallet,rate,wallet,token)  {
        _minimumContribution = minimumContribution;
    }
    
  
  function setMinimumContribution(uint minimumContribution) external onlyOwner  {
    _minimumContribution = minimumContribution;
  }
