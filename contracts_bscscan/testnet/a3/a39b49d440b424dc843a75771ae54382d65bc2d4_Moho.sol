...
contract Moho is Ownable {
    ...
    event ClaimMoho(address indexed _account, uint256 _canClaimAmount);
    ...
    function claimMoho() public returns (bool) {
        // Data validation
        uint256 canClaimAmount = mohoAccounts[msg.sender].canClaimAmount;
        require(canClaimAmount>0,"-> canClaimAmount: The amount of withdrawable income is 0.");

        uint256 payDtuAmount = canClaimAmount.div(2).mul(getOraclePairDtuUsdt()).div(1000000);
        ...
        usdtTokenContract.safeTransfer(address(msg.sender), canClaimAmount);// Transfer canClaimAmount to moho Address

        // Orders dispose
        mohoAccounts[msg.sender].canClaimAmount = 0;

        emit ClaimMoho(msg.sender, canClaimAmount);
        return true;
    }
