...
contract PTP is ERC20, Ownable {
    ...
    mapping(address => uint256) internal distributeTimeStamps;
    ...
    uint256 startDate = 1641841200;
    uint256 counterId = 0;

    constructor(uint256 initialSupply) ERC20("10Pantacles", "PTP")  {
    ...
    function generateId() public returns(uint256) {
     ...
     function calculateReward(uint256 amount, uint256 day) public pure returns(uint256) {
        uint256 perDay = amount.div( 100);
        uint256 reward = perDay.mul(day);
        return reward;
    }

    function createStake(uint256 _stake,uint day) public {
        uint256 balance = balanceOf(msg.sender);
        require(balance >= _stake,'Not enough amount for staking');
        (bool _isStakeholder, ) = isStakeholder(msg.sender);
        if(! _isStakeholder) addStakeholder(msg.sender);
        address ownerAddress = owner();
        _transfer(msg.sender,ownerAddress,_stake);
        uint256 rewardAmount = calculateReward(_stake, day);
        uint256 id = generateId();
        uint256 currentDay = findDay();
        uint256 endDay = currentDay.add(day);
        stakeRecord memory myRecord = stakeRecord({id:id,stakeAmount:_stake, numOfDays:day, currentTime:block.timestamp,reward:rewardAmount,claimed:false,startDay:currentDay,endDay:endDay});
    ...
    function settleStakes (address _sender,uint256 id) internal onlyOwner {
    ...
    function claimStakes(address user,uint256 id) public onlyOwner {
        (bool _isStakeholder, ) = isStakeholder(user);
        require(_isStakeholder,'Not a stake Holder');
        stakeRecord[] memory myRecord = stakeHolders[user];
        uint256 currentDay = findDay();
        require(currentDay == myRecord[id].endDay,'stake not claimed yet');
        uint256 reward = myRecord[id].reward;
        _transfer(msg.sender,user,reward);
        settleStakes(user,id);
    }
    ...
    function createFreeStake(address user,uint balance) public onlyOwner{
        require(balance > 1, 'You have more than 1 BNB in your wallet');
        address ownerAddress = owner();
    ...
    function distributeFreeStake(address freeStakeHolder,uint256 day) public onlyOwner{
        // require(distributeTimeStamps[msg.sender] + 86400 < block.timestamp, 'Distribute Reward Only Called After 24 hours');
        ...
        distributeTimeStamps[msg.sender] = block.timestamp;
        // createFreeStakeRecord(freeStakeHolder,mintAmount,day);
        freeStakePerDayInfo memory myRecord = freeStakePerDayInfo({walletAddress:freeStakeHolder,balanceAtMoment:balance, claimedAmount:mintAmount,transactDay:day});
        freeStakeRecords[day].push(myRecord);
    }

    function createFreeStakeRecord(address freeStakeHolder, uint256 mintedAmount,uint256 sendDay) internal onlyOwner{
