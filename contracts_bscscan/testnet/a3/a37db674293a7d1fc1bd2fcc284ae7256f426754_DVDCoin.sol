...
contract DVDCoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DVDCoinDividendTracker public dividendTracker;

    address public liquidityWallet;

    address payable public operations1Address =
        payable(0x364cdB09FEe0E50F849100Cf83949648e448C0F0);
    address payable public operations2Address =
        payable(0x80FC63D420948de1f9bEe13d58ded17F08b8c1c6);

    uint256 public maxSellTransactionAmount = 0;
    uint256 public swapTokensAtAmount = 3000000 * (10**19);

    uint256 public devFees = 10;
    uint256 public liquidityFee = 3;
    uint256 public USDCRewardsBuyFee = 10;
    uint256 public USDCRewardsSellFee = 10;
    ...
    event UpdateFees(
        uint256 dev,
        uint256 liquidity,
        uint256 USDCRewardsBuy,
        uint256 USDCRewardsSell
    );
    ...
    constructor() ERC20("DVDCoin", "DVDC") {
        dividendTracker = new DVDCoinDividendTracker(payable(this));
        ...
        totalSellFees = USDCRewardsSellFee.add(liquidityFee).add(devFees);
        totalBuyFees = USDCRewardsBuyFee.add(liquidityFee).add(devFees);
    ...
    function updateMaxAmount(uint256 newNum) public onlyOwner {
        ...
        require(
            maxSellTransactionAmount == 0 ||
                maxSellTransactionAmount >= 1000000 * (10**19),
            "max sell amount must either be disabled or greater than 500 billion"
        );
        maxSellTransactionAmount = newNum * (10**19);
    }

    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker));

        DVDCoinDividendTracker newDividendTracker = DVDCoinDividendTracker(
            payable(newAddress)
        );
    ...
    function updateFees(
        ...
        require(
            USDCRewardsBuy >= 5 && USDCRewardsBuy <= 25,
            "USDC reward fees must be between 5 and 25"
        );
        require(
            USDCRewardsSell >= 5 && USDCRewardsSell <= 25,
            "USDC reward fees must be between 5 and 25"
        );

        devFees = dev;
        liquidityFee = liquidity;
        USDCRewardsBuyFee = USDCRewardsBuy;
        USDCRewardsSellFee = USDCRewardsSell;

        totalSellFees = USDCRewardsSellFee.add(liquidityFee).add(devFees);
        totalBuyFees = USDCRewardsBuyFee.add(liquidityFee).add(devFees);

        emit UpdateFees(dev, liquidity, USDCRewardsBuy, USDCRewardsSell);
    }
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 USDCRewardsFee;

            if (isSelling) {
                USDCRewardsFee = USDCRewardsSellFee;
            } else {
                USDCRewardsFee = USDCRewardsBuyFee;
            }

            uint256 totalFees = USDCRewardsFee.add(liquidityFee).add(devFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividendsFromBuy = address(this)
            ...
            .mul(buyAmount)
            .div(totalAmount)
            .mul(USDCRewardsBuyFee)
            .div(USDCRewardsBuyFee.add(devFees));

        uint256 dividendsFromSell = address(this)
            .balance
            .mul(sellAmount)
            .div(totalAmount)
            .mul(USDCRewardsSellFee)
            .div(USDCRewardsSellFee.add(devFees));
...
contract DVDCoinDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "DVDCoin_Dividend_Tracker",
            "DVDCoin_Dividend_Tracker"
        )
    {
        dvdCoin = DVDCoin(mainContract);
        minimumTokenBalanceForAutoDividends = 1000 * (10**19);
        minimumTokenBalanceForDividends = 1000 * (10**19);
    ...
    function _transfer(
        ...
        require(false, "DVDCoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DVDCoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DVDCoin contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(dvdCoin);

            uint256 prevBalance = dvdCoin.balanceOf(address(this));
            ...
            {
                uint256 received = dvdCoin.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    dvdCoin.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            // if no tokenAddress assume USDC payout
            if (
                !allowCustomTokens ||
