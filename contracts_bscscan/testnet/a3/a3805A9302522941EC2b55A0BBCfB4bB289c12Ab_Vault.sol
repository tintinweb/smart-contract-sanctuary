...
contract Vault is ReentrancyGuard, Exponential, TokenErrorReporter {
    ...
    function repay(uint256 bid, uint256 amount) public payable nonReentrant {
        ...
        VaultLib.transferTokenOrBNB(profitShare, app.loanToken, profit);
...
library VaultLib {
    ...
    function swap(
        ...
        address[] memory path
    ) internal returns (uint256 colleteralIn, uint256 amountOut) {
        uint256[] memory result;
        if (path[0] == wbnb) {
            result = IPancakeRouter02(router).swapETHForExactTokens(
                amountLoanOut,
    ...
    function interestAmount(
        ...
        uint256 timePast = block.number.sub(startBlock);
        uint256 totalInterest = amount.mul(interestRate).div(1e18).div(100);
        uint256 currentInterest = totalInterest.mul(timePast).div(duration);
        return currentInterest;
    }
