...
contract IDOAllowance is IDO {
    ...
    function initialize(IDOParams memory params) public override {
        ...
        require(
            (params._totalAllocationAmount / allowanceCount) >=
                params._maxDeposit
        );
...
import "../Treasury.sol";
import "../interfaces/ITokensData.sol";
    ...
    ITokensData
{
    ...
    function initialize(IDOParams memory params) public virtual initializer {
        ...
        token = params._token;
        minDeposit = params._minDeposit;
        maxDeposit = params._maxDeposit;
        softCap = params._softCap;
        totalAllocationAmount = params._totalAllocationAmount;

        isMainTokenAllowed = params._pricePerCurrency == 0 ? false : true;

        startDepositTime = params._startDepositTime;
        endDepositTime = params._endDepositTime;
        stratClaimTime = params._startClaimTime;
        ...
        for (uint256 index = 0; index < tokensLength; index++) {
            addToken(
                IERC20(params._tokens[index]._address),
                params._tokens[index]._price
            );
        }
        ...
        if (isMainTokenAllowed) {
            tokenInfo.push(
                TokenInfo({
                    pricePerOne: params._pricePerCurrency,
    ...
    function deposit() public payable virtual whenNotPaused {
        ...
        _deposirRequire(msg.sender, msg.value, 0);

        allowanceToUserInfo[msg.sender].deposit = msg.value;
        currentAllocationAmount =
            currentAllocationAmount +
            (msg.value * tokenInfo[0].pricePerOne);
        ...
        whenNotPaused
    {
        ...
        _deposirRequire(msg.sender, amount, tokenInfoId);
        ...
        currentAllocationAmount =
            currentAllocationAmount +
            (amount * tokenInfo[tokenInfoId].pricePerOne);
    ...
    function claim() public virtual whenNotPaused {
        ...
        require(block.timestamp > stratClaimTime, "Claim not started");
        require(
            allowanceToUserInfo[msg.sender].claimedOrRefunded == false,
        ...
        allowanceToUserInfo[msg.sender].claimedOrRefunded = true;
        claimOrRefundCount++;
    ...
    function _deposirRequire(
        ...
        uint256 currentAllocationAmountValue = currentAllocationAmount +
            (amount * tokenInfo[tokenInfoId].pricePerOne);
