...
contract StakingMP {
  ...
  function withdrawn(uint256 id) public {
    ...
    if((users[msg.sender].deposits[idDep].tw).add(users[msg.sender].deposits[idDep].accrualPeriod) <= time){
      uint256 interval;
      if((users[msg.sender].deposits[idDep].timestamp).add(users[msg.sender].deposits[idDep].term) < time){
        interval = (users[msg.sender].deposits[idDep].timestamp).add(users[msg.sender].deposits[idDep].term).sub(users[msg.sender].deposits[idDep].tw);
      }else{
  ...
  function getTimeClose(uint256 id) public view returns(uint256) {
    return (users[msg.sender].deposits[id].timestamp).add(users[msg.sender].deposits[id].term);
  }
      ...
      if(users[userAddress].deposits[i].investplan == id){
        if(users[userAddress].deposits[i].status == 1){
          if((users[userAddress].deposits[i].tw).add(users[userAddress].deposits[i].accrualPeriod) < time){
            uint256 interval;
            uint256 timeClose = (users[msg.sender].deposits[i].timestamp).add(users[msg.sender].deposits[i].term);
            if(timeClose < time){
