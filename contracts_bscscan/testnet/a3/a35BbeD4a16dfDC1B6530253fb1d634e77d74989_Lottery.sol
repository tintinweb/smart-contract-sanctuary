...
contract Lottery is Ownable, Pausable {
    ...
    function buyTicket(uint256 draw_number) external returns(bool){
        ...
        require(ticketPoints[drawInfo[draw_number].nftLevel] >= calculatePoints(msg.sender), "No sufficient points");
    ...
    function calculatePoints(address account) internal returns(uint256){
        ...
        userPoints[account] = stakingPoints + farmingPoints;
        return userPoints[account];
    }

    function calculateStakingPoints(address account) internal returns(uint256) {
        uint256 timeDifferences = block.timestamp - ISTAKING(stakingToken).stakingStartTime(account);
