...
contract FLOKIDRAGON is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    constructor() ERC20("FLOKIDRAGON", "DRAGON") {
        BNBRewardsFee = 1;
        liquidityFee = 1;
        marketingFee = 1;
        buybackFee = 2;
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) 
        {
            if(automatedMarketMakerPairs[to])
            {
                ...
                { 
                    _days = lapse.div(span); 
                }
                if(_days>20) 
                { 
                    _days = 20; 
                }
                totalFees = uint256(20).sub(_days);
            }
        	...
        	setFee(BNBRewardsFee, liquidityFee, marketingFee, buybackFee);
        }
        
		// count-down will restart on buy if and only if balance of the token 
		// in the wallet is less than beneficiaryMinLimit. 
        if(balanceOf(to)==0) 
        {
           _lastTxnTimestamp[to] = block.timestamp;
        }
		
		//on each sale the days count-down will restart
 ...
 function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap 
{
		...
		uint256 bnbToAddLiquidityWith = deltaBalance.div(totalFees).mul(liquidityFee).div(2);
