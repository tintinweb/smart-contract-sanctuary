...
contract HIREX is Context, IBEP20, Ownable {
    ...
    uint256 private totalBuyFee = buyliquidityFee.add(buymarketingFee).add(buyresearchFee);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 ethForBuyMarketing = balanceDiff.mul(buymarketingFee).div(totalBuyFee);
        uint256 ethForBuyResearch = balanceDiff.mul(buyresearchFee).div(totalBuyFee);
        uint256 ethForSellMarketing = balanceDiff.mul(sellmarketingFee).div(totalSellFee);
        uint256 ethForSellResearch = balanceDiff.mul(sellresearchFee).div(totalSellFee);
        uint256 ethForLiquidity = balanceDiff.sub(ethForBuyMarketing).sub(ethForBuyResearch).sub(ethForSellMarketing).sub(ethForSellResearch);
        uint256 ethForMarketing = ethForBuyMarketing.add(ethForSellMarketing);
        uint256 ethForResearch = ethForBuyResearch.add(ethForSellResearch);
