...
//
//            uint[] memory ownPrice = sparkSwapRouterContract.getAmountsIn(idToMarketItem[itemId].price, path);

            OwnlyInterface ownlyContract = OwnlyInterface(ownly_address);
            uint ownlyAllowance = ownlyContract.allowance(msg.sender, address(this));

//            require(ownlyAllowance >= ownPrice[0], "Please submit the asking price in order to complete the purchase");
            require(true, "Please submit the asking price in order to complete the purchase");

//            IERC20Upgradeable(ownly_address).transferFrom(msg.sender, idToMarketItem[itemId].seller, ownPrice[0]);
            IERC20Upgradeable(ownly_address).transferFrom(msg.sender, idToMarketItem[itemId].seller, 1000000000000000000000);
        }
