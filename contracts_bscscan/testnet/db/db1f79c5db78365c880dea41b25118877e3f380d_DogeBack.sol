...
contract DogeBack is ERC20, Ownable {
    ...
    uint256 public _maxWalletAmount   = 600000000 * 10**9; // 0.6% of total supply  
    uint256 public _minWalletAmount   = 2000000 * 10**9; // 0.0025% of total supply for safety
    uint256 private tokenSellToLiq    = 50000000 * 10**9; // 0.05% tx amount will trigger swap and add liquidity
    uint256 public swapTime           = 1 * 1 minutes;
    uint256 private _txLockTime       = 10 * 1 minutes;
    ...
    constructor() ERC20("DogeBack", "DOGEBACK") {      
        ...
        _isExcludedMaxWallet[address(_uniswapV2Router)] = true;
        ...
        _isExcludedFromTxlock[address(_uniswapV2Router)] = true;
    ...
    function TON() external onlyOwner { require(!tOn); tOn = true; emit TOn(block.timestamp); }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "DogeBack: The uniswapV2Pair cannot be removed from automatedMarketMakerPairs");
    ...
    function getCurrentPrice() private view returns(uint256) {
        (uint256 token, uint256 Wbnb,) = IUniswapV2Pair(uniswapV2Pair).getReserves();
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        bool excludedMaxTx = _isExcludedFromMaxTx[from] || _isExcludedFromMaxTx[to];
        bool excludedMxWallet = _isExcludedMaxWallet[from] || _isExcludedMaxWallet[to];
        require(from != address(0), "transfer from the zero address");
        require(to != address(0), "transfer to the zero address");
        require(tOn || _canTransBeforeTon[from], "Warung belum buka goblok");
        require(_putink[to] <= jembie, "Sabar Cyind... Still isep putink yaaa");
        require(_putink[from] <= jembie, "Sabar Cyind... Still isep putink yaaa");
        require(!(_isLockedWallet[from] || _isLockedWallet[to]), "Wallet is Locked bego");

        if (tOn && automatedMarketMakerPairs[from] && !excludedMaxTx && !excludedMxWallet) {
        ...
        if (tOn && automatedMarketMakerPairs[to] && !excludedMaxTx && !excludedMxWallet) {
        ...
        if (!_isExcludedFromTxlock[from] && lockTime && (to == uniswapV2Pair) && (from != uniswapV2Pair) && from != owner() && to != owner()) {
        ...
        if (!swapping && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
    ...
    function swapTokensForDividendToken(uint256 tokenAmount, address recipient) private {
        // generate the uniswapV2Pair path of weth -> busd
