...
contract PlayingCard is ERC721, Authorizable {
    ...
    function setCardCounter(uint _counter) public onlyOwner(){
    ...
    function mintCard(address _to,uint256 _cardType) public onlyAuthorized() {
        require(_to != address(0),"PlayingCard: _to address not a valid");
        totalSupply += 1;
        _holderTokens[_to].add(counters[_cardType]);
        idCardType[counters[_cardType]] = _cardType;
        _safeMint(_to,counters[_cardType]);
        counters[_cardType]++;
    }

    function batchMintCard(address _to,uint256 _cardType,uint256 _numberOfToken) public onlyAuthorized(){
        require(_numberOfToken > 0,"NFT4Play: use valid token amount");
        for (uint256 i = 0; i < _numberOfToken; i++) {
            mintCard(_to,_cardType);
        }
    }
    
    function batchMintCard(address[] memory _addresses,uint256 _cardType,uint256[] memory _numberOfTokens) external onlyAuthorized() {
        ...
        for (uint256 i = 0; i < _addresses.length; i++) {
            batchMintCard(_addresses[i],_cardType, _numberOfTokens[i]);
        }
    }
    
    function burn(address _address,uint _tokenId) public {
...
contract Factory {
    ...
    address[] public gameCardAddresses;

    function deployNewGameCardContract(string memory _name, string memory _symbol,string memory _baseURI,uint[] memory _counters)
        public
        returns (address)
    {
        require(bytes(_name).length != 0,"Factory: Use valid _name");
        require(bytes(_symbol).length != 0,"Factory: Use valid _symbol");
        PlayingCard playCard = new PlayingCard(_name, _symbol,_baseURI,_counters);
