...
contract GoldenTreat is Context, AccessControl, IERC20, IERC20Metadata {
    ...
    function _tokenVestingUpdate(
        ...
        // If wallet has prior transactions
            ...
            if (tokenVestingDataStorage.time > block.timestamp)
                coolDownRemaining = tokenVestingDataStorage.time.sub(
                    block.timestamp
                );
            ...
            uint256 weightTransaction;
            uint256 weightBalance;
            uint256 weight;
            // Taking the vesting percent.
            _amount = _amount.mul(vestingPercentage).div(10000);
            if (tokenVestingDataStorage.amount != 0 || _amount != 0) {
                if (tokenVestingDataStorage.amount > _amount) {
                    weight =
                        10000 -
                        (((_amount * 10**18) /
                            (_amount + tokenVestingDataStorage.amount)) *
                            10000) /
                        10**18;
                    weightBalance = weight;
                    weightTransaction = 10000 - weight;
                } else {
                    weight =
                        10000 -
                        (((tokenVestingDataStorage.amount * 10**18) /
                            (_amount + tokenVestingDataStorage.amount)) *
                            10000) /
                        10**18;

                    weightTransaction = weight;
                    weightBalance = 10000 - weight;
                }
                uint256 daysPassed = vestingLockAfterPurchase -
                    coolDownRemaining;

                uint256 DaysLocked = ((weightTransaction *
                    vestingLockAfterPurchase) / 1e4) +
                    ((weightBalance * (vestingLockAfterPurchase - daysPassed)) /
                        1e4);

                tokenVestingDataStorage.time = block.timestamp.add(DaysLocked);
                tokenVestingDataStorage.amount = tokenVestingDataStorage
                    .amount
                    .add(_amount);
            }
        ...
        returns (uint256 locked, uint256 coolDownRemaining)
    {
        ...
        if (transactionlocked.time == 0) {
            ...
            if (transactionlocked.time > block.timestamp) {
                coolDownRemaining = transactionlocked.time.sub(block.timestamp);
            }
