...
import "./RewardEnabledUpgradable.sol";

contract ATrollCity is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, RandomnessEnabledUpgradable, RewardEnabledUpgradable, IATrollCity {
    ...
    function initialize(
        ...
        __RandomnessEnabled_init(maxSupply);
        __RewardEnabled_init();
        __Pausable_init();
        __Ownable_init();
    ...
    function airDrop(address assigned, uint256 quantity) external override onlyOwner whenNotPaused {
        require(isSaleActive, "Sale must be active to mint NFT");
        require(totalSupply() < maxSupply, "Max supply exceeded");
        require(totalSupply() < getSupplyCapForRound(round), "Supply cap exceeded");
        if (round == 0)
    ...
    function transferReflection() internal virtual nonReentrant {
        ...
        for (uint256 i = 0; i < totalSupply(); i++){
            ...
            updateDividend(dividend, addressTokenOld);
        }
    ...
    function setSaleActive(bool isActive) external override onlyOwner whenNotPaused{
    ...
    function originalOwnerOf(uint256 tokenId) external view virtual override whenNotPaused returns (address) {
    ...
    function checkAvailableReward() external view override returns (uint256) {
        return _checkAvailableReward();
    }

    function setBuyBackPoolAddress (address _buyBackPoolAddress) external override onlyOwner whenNotPaused{
        buyBackPool = _buyBackPoolAddress;
    }

    function setLPPoolAddress (address _lpPoolAddress) external override onlyOwner whenNotPaused{
        lpPool = _lpPoolAddress;
    }

    function getBuyBackPoolAddress() external view override onlyOwner returns (address) {
        return buyBackPool;
    }

    function getLPPoolAddress() external view override onlyOwner returns (address) {
        return lpPool;
    }

    function withdraw() external override onlyOwner nonReentrant whenNotPaused{
        uint256 contractBalance = address(this).balance;
        require(contractBalance > liability(), "Contract only has liability amount");
        uint256 eligibleWithdrawAmount = contractBalance.sub(liability());
...
contract RewardEnabledUpgradable is Initializable {
    ...
    function __RewardEnabled_init() internal onlyInitializing {
        __RewardEnabled_init_unchained();
    }

    function __RewardEnabled_init_unchained() internal onlyInitializing {
    }
    ...
    function updateDividend(uint256 dividend ,address addressTokenOld) public {
    ...
    function _checkAvailableReward() internal view returns (uint256) {
        return _dividendAmount[msg.sender];
    }

    function liability() internal view returns (uint256) {
        return _liability;
    }
}
...
contract RandomnessEnabledUpgradable is Initializable {
    ...
    function __RandomnessEnabled_init(uint256 _pendingCount) internal onlyInitializing {
        __RandomnessEnabled_init_unchained(_pendingCount);
    }

    function __RandomnessEnabled_init_unchained(uint256 _pendingCount) internal onlyInitializing {
...
/**
 * @dev Interface of the ATrollCity implementation.
 */

interface IATrollCity {

    event SetBaseURI (string newURI);

    event SetNewWhiteList ();
    ...
    function setSaleActive(bool isActive) external;

    function baseURI() external view returns (string memory);

    function setBaseURI(string memory newUri) external;

    function setWhitelist(address[] calldata newAddresses) external;

    function removeWhitelist(address[] calldata currentAddresses) external;

    function startNextRoundSale() external;

    function originalOwnerOf(uint256 tokenId) external view returns (address);

    function mint() external payable;

    function airDrop(address assigned, uint256 amount) external;

    function claimReward() external;

    function checkAvailableReward() external view returns (uint256);

    function withdraw() external;

    function setBuyBackPoolAddress (address _buyBackPoolAddress) external;

    function setLPPoolAddress (address _lpPoolAddress) external;

    function getBuyBackPoolAddress() external view returns (address);

    function getLPPoolAddress() external view returns (address);
}
