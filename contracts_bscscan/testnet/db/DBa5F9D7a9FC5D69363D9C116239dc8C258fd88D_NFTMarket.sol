...
contract NFTMarket is ReentrancyGuard, Ownable {
  ...
  function purchaseMarketListing(
    ...
    _idToMarketListing[listingId].owner = payable(msg.sender);
  ...
  function cancelMarketListing(
    ...
    require(item.seller == msg.sender, "Must be seller to cancel market listing");
    ...
    emit MarketListingCancelled(listingId, item.nftContract, item.tokenId, msg.sender);
  }
  ...
  function fetchMarketListings(uint page, uint limit) public view returns (MarketListing[] memory) {
    ...
    for (uint i = 0; i < itemCount && currentIndex < actualLimit; i++) {
      if (_idToMarketListing[i + 1].seller != address(0)) {
