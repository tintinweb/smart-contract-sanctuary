...
contract TTM is ERC721Enumerable, Ownable {
    ...
    constructor(string memory baseURI) ERC721("TTM", "TTM") {
        setBaseURI(baseURI);
        _contractOwner = payable(msg.sender);
    }

    function reserveNFTs(uint amount) public onlyOwner {
    ...
    function listingAllNFT() public {
        for(uint i = 0; i < _tokenIds.current(); i++)
        {
            updateListingStatus(i, true);
        }
    }
