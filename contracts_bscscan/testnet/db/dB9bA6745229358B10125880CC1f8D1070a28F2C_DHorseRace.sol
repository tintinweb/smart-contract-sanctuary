...
// File: contracts/lib/IDHorseNft.sol

pragma solidity ^0.8.0;



interface IDHorseNft is IERC721{
...
contract DHorseRace is Ownable {
    ...
    struct race {
        ...
        uint32[12] winners; // The winning horseIds.
    ...
    // All the needed info about horse
    struct horse {
        uint256 rating;
        uint256 time; // After this horse can participte in other race
        // initially 0
    }
    mapping(uint256 => race) public raceInfo;
    // HorseId => rating and time
    mapping(uint256 => horse) public horseInfo;
    ...
    modifier isHorseEligible(uint256 _tokenId) {
        require(nftToken.ownerOf(_tokenId) == msg.sender, "Invalid Horse");
        require(
            (horseInfo[_tokenId].time == 0) ||
                (horseInfo[_tokenId].time < block.timestamp),
    ...
    function enterRace(
        ...
        uint256 _index,
        uint256 _tokenId
    ) external isRaceExist(_raceId) isHorseEligible(_tokenId) {
        ...
        horseInfo[_tokenId].time = raceInfo[_raceId].startTime;
...
// 6.(initial level) 5.class5 (0-20) 4.class4 (21-40) 3.class3 (41-60)
        ...
        returns (bool)
    {
        if (_level == 0) {
            ...
            if (horseInfo[_tokenId].rating > 81) {
                return true; 
            ...
            if (
                (horseInfo[_tokenId].rating >= 61) &&
                (horseInfo[_tokenId].rating <= 80)
            ) {
                return true;
            } else {
                return false;
            ...
            if (
                (horseInfo[_tokenId].rating >= 41) &&
                (horseInfo[_tokenId].rating <= 60)
            ) {
                return true;
            } else {
                return false;
            ...
            if( (horseInfo[_tokenId].rating >= 21) &&
                (horseInfo[_tokenId].rating <= 40) ) {
                return true;
            } else {
                return false;
            }
        } else if (_level == 5) {
            if( (horseInfo[_tokenId].rating > 0) &&
                (horseInfo[_tokenId].rating <= 20) ) {
                return true;
            } else {
                return false;
            }
    ...
    function announceRaceResult(uint256 _raceId,uint32[12] memory result) external isOwnerOrOperator {
        require(
            (raceInfo[_raceId].startTime < block.timestamp),
            "Invalid time"
        );
    ...
    function initialLevel(uint256 _raceId) internal {
        ...
        for (uint256 i = 0; i < 12; i++) {
            ...
            horseInfo[tokenId].rating = genRating(tokenId);
    ...
    function updateRating(uint256 _raceId) internal {
        ...
        horseInfo[result[0]].rating = horseInfo[result[0]].rating + 6;
        horseInfo[result[1]].rating = horseInfo[result[1]].rating + 4;
        horseInfo[result[2]].rating = horseInfo[result[2]].rating + 2;
       
        for(uint i=6; i<12; i++){
           
            if(horseInfo[result[i]].rating < points){
              
                horseInfo[result[i]].rating = 0;
            }else {
               
                horseInfo[result[i]].rating = horseInfo[result[i]].rating - points;
            }
        }
    }
    ...
    {
        (, , , , generation,,) = nftToken.getHorse(_tokenId);
    }
