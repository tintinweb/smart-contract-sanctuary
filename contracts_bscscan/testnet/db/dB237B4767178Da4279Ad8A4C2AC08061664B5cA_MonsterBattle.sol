...
contract MonsterBattle is MonsterERC20, ReentrancyGuard {
...
contract MonsterERC20 is Ownable, ERC20 {
    ...
    modifier onlyMON() {
        require(manager.monsterBattle(_msgSender()), "caller is not the evolver");
        _;
    }
    ...
    function setMinTokensBeforeSwap(uint256 _tokenForBosses)
        ...
        onlyOwner
    {
        require(_tokenForBosses < 5 * 10**6 * 10**18, "token box to large");
    ...
    function win(address winner, uint256 reward) external onlyMON {
...
interface ManagerInterface {
    function monsterBattle(address _address) external view returns (bool);
    ...
    function MONBattle() external view returns (uint256);

    function MONEggs() external view returns (uint256);
