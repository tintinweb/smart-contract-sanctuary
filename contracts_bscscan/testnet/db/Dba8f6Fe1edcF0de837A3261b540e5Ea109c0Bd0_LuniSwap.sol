...
// BTCC
 ...
 function addNewEmploy(address newEmploy, uint8 employType) public onlyOwner {
  ...
  function swapLuni(uint256 amountLuni) public returns (uint256, uint256) {
    ...
    uint256 amountToSwap = swapAmount > swapAmountValid ? swapAmountValid : swapAmount;
    uint256 fee = amountToSwap.mul(swapFee).div(100);
    IERC20(LUNI_TOKEN).transferFrom(msg.sender, swapAdmin, amountLuni);
    IERC20(USDT_TOKEN).transfer(msg.sender, amountToSwap.sub(fee));
    employ[msg.sender].amountSwap = employ[msg.sender].amountSwap.add(amountToSwap);
