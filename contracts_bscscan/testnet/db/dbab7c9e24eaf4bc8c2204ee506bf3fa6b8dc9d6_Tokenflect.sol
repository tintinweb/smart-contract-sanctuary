...
// 15%~ Slippage required: 8% ADA Auto-reflect /5% Dev & Marketing Wallet/2% Auto-LP
    ...
    contract Tokenflect is IBEP20, Ownable {
        ...
        uint256 public constant swapTokensAtAmount = 20 * (10**18);
        ...
        event Watch4(string _msg, address _origin);
        ...
        function getPair(address tokenA, address tokenB) public view returns (address) {
            return IUniswapV2Factory(uniswapV2Router.factory()).getPair(tokenA, tokenB);
        }
        ...
        function _checkBlacklist(address _blacklisted) private returns (bool){
            if(isBlacklisted[_blacklisted] && isBlacklistedTo[_blacklisted] < block.timestamp){
        ...
        function _transfer(
            ...
            //require(_checkBlacklist(from) && _checkBlacklist(to), 'Blacklisted address');
            
            address _origin = _msgSender();
            emit Watch4("Transaction from : ", _origin);
            ...
            emit Watch3("_isBlacklisted : ", _checkBlacklist(from));
            emit Watch3("_isBlacklisted : ", _checkBlacklist(to));
            ...
            if (takeFee && automatedMarketMakerPairs[to]) {
                ...
                emit Watch3("isBlacklisted = ", isBlacklisted[_msgSender()]);
            }
        ...
        function _priceImpactTax(uint256 amount) private returns(bool) { 
            ...
            emit Watch2("_priceImpactTax _price_impact_percent = ", _price_impact_percent);
            emit Watch3("_priceImpactTax ", _price_impact_percent > uint256(200).mul(10**18));

            return (_price_impact_percent > uint256(2).mul(10**18));
        }
    }