...
contract Farm is AccountChangable {
  ...
  function addPools(PoolInput[] memory _poolInputs) public onlyRoot {
    require(pools.length == 0, 'POOLS_ALREADY_SET');
  ...
  */
  ...
  function register(address presenter) public {
    address sender = _originalMsgSender();
    require(!isUser(sender), 'ALREADY_REGISTERED');
    createUser(sender, presenter);
  }
