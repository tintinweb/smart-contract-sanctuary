...
contract EvryFactory is IEvryFactory, Ownable {
    ...
    function setFeeTo(address _feeTo) external onlyOwner override {
        feeTo = _feeTo;
    }
    ...
    function setPlatformFee(uint256 feeBasis) external payable onlyOwner override {
...
interface IEvryFactory {
    ...
    function setPlatformFee(uint256) external payable;
...
contract EvryPair is IEvryPair, EvryERC20 {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IEvryFactory(factory).feeTo();
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock override {
        require(amount0Out > 0 || amount1Out > 0, 'Evry: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'Evry: INSUFFICIENT_LIQUIDITY');
        ...
        if (data.length > 0) IEvryCallee(to).evryCall(msg.sender, amount0Out, amount1Out, data);
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this)); 
        }
        //get total fee
        ...
        uint feePlatformBasis = IEvryFactory(factory).feePlatformBasis();
        uint feeToLiquidityBasis = IEvryFactory(factory).feeLiquidityBasis();
        uint totalFee = feePlatformBasis.add(feeToLiquidityBasis);
        if(amount0In > 0) amount0In = amount0In.mul(10000) / (10000-feePlatformBasis);
        if(amount1In > 0) amount1In = amount1In.mul(10000) / (10000-feePlatformBasis);
        uint balance0Adjusted = balance0.mul(10000).add(amount0In.mul(feePlatformBasis)).sub(amount0In.mul(totalFee));
        uint balance1Adjusted = balance1.mul(10000).add(amount1In.mul(feePlatformBasis)).sub(amount1In.mul(totalFee));
...
contract EvryERC20 is IEvryERC20 {
    ...
    string public override constant name = 'Evry.finance LP (EFLP)';
    string public override constant symbol = 'EFLP';
