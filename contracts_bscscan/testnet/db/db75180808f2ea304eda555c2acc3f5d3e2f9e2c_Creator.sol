...
contract priceFrame{
    ...
    function pairInfo() public onlyNotExpiry view returns(string memory, uint256, string memory, uint256, uint256, uint){
        uint totalSupply = tokens.totalSupply();
        
        (uint112 token0v, uint112 token1v,) =  IPancakePair(lpToken).getReserves();

        return (pair0, token0v, pair1, token1v, totalSupply, decimals);
...
contract Creator{
    ...
    payable
  {
    ...
    require(block.timestamp > pairs[tokenAddress].time, 'contract still active');
