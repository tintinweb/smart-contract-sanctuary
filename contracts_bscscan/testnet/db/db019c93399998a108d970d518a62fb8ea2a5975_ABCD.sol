...
contract Ownable is Context {
    ...
    //TODO: not useful. time lock resolve the issue (SSL-13)
...
contract ABCD is Context, IERC20, Ownable {
    ...
    //// ERC-20 init
    string private constant _name = "v3";
    string private constant _symbol = "v3";  //TODO
    uint8 private constant _decimals = 9;
    uint256 private constant _tTotal = 2 * 10**5 * 10**9;  //TODO
    ...
    uint256 public immutable contactReleaseTime;  // make sure the contract is not changeble by owner for certain period for stability and transparency
    ...
    //endweb
    uint256 private constant _level1MinReferredToken = 1 * 10**9;
    uint256 private constant _level2MinReferredToken = 2 * 10**9;
    uint256 private constant _level3MinReferredToken = 8 * 10**9;
    // thousandth
    ...
    //// liquidity pool 
    uint256 private constant numTokensSellToAddToLiquidity = 10**5;  //TODO
    // time lock
    ...
    constructor () {
        ...
        // init time lock
        minReleaseTime = block.timestamp + 60 * 5;  //TODO: start date + 1.5 years
        contactReleaseTime = block.timestamp + 60 * 5;  //TODO: start date + 6 months
        launchReleaseTime = block.timestamp + 60 * 60 * 24;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            //addweb
            uint256 previousMaxTxAmount = _maxTxAmount;
            _maxTxAmount = _tTotal;
            contractTokenBalance = numTokensSellToAddToLiquidity;
            swapAndLiquify(contractTokenBalance);
            _maxTxAmount = previousMaxTxAmount;
            //endweb
        }
    ...
    // get original t token for calculating reflection
    //addweb
    function _updateOriginalTokens(uint256 tAmount, uint256 tTransferAmount, address sender, address recipient) private {
        if (tAmount < originalTToken[sender]) originalTToken[sender] = originalTToken[sender].sub(tAmount);
        else originalTToken[sender] = 0;  // all token left in wallet is coming from reflection or referral reward
        originalTToken[recipient] = originalTToken[recipient].add(tTransferAmount);
    }
    function distributionOfTokens(address account) public view returns (uint256 purchasedToken, uint256 referralToken, uint256 reflectionToken){
        uint256 totalToken = balanceOf(account);
        uint256 _referralToken = referralRewardToken[account];
        uint256 _purchasedToken = originalTToken[account];
        uint256 _reflectionToken = totalToken.sub(_referralToken).sub(_purchasedToken);
        return (_purchasedToken, _referralToken, _reflectionToken);
    }
    //endweb
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        //addweb tdev = tdev original - r referral
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tDev, uint256 tReferrer, uint256 tReferree, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        if (rDev < rReferrer.add(rReferree)){
            rDev = 0;
        } else {
            rDev = rDev.sub(rReferrer).sub(rReferree); // 1000 * 2% = 20
        }
        //endweb
    ...
    function _takeReferralFee(uint256 tReferral, address referralAddr) private {
        ...
        //endweb
        if(_isExcluded.contains(referralAddr))
            _tOwned[referralAddr] = _tOwned[referralAddr].add(tReferral);
    }
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        require(block.timestamp > contactReleaseTime, "Owner can't modify contract until release time");
    ...
    function customizeFeeForReferral(address referrerAddr) private {
        ...
        //addweb  not updating here
        //_devFee = _devFee.sub((_referrerFee.add(_referreeFee)).div(10));  
        //endweb
    }
    ...
    function setdevFeePercent(uint256 newDevFee) external onlyOwner {
        require(block.timestamp > contactReleaseTime, "Owner can't modify contract until release time");
        require(newDevFee < _devFee, "New dev fee must be less than current dev fee");
        // update referral fee
        _referrerFeePony = _referrerFeePony.mul(newDevFee).div(_devFee);
        _referrerFeeHorse = _referrerFeeHorse.mul(newDevFee).div(_devFee);
        _referrerFeeFireHorse = _referrerFeeFireHorse.mul(newDevFee).div(_devFee);
        _referrerFeeUnicorn = _referrerFeeUnicorn.mul(newDevFee).div(_devFee);
        _referreeFeePony = _referreeFeePony.mul(newDevFee).div(_devFee);
        _referreeFeeHorse = _referreeFeeHorse.mul(newDevFee).div(_devFee);
        // update dev fee
        _devFee = newDevFee;
    ...
    function unlockLiquidity() external onlyOwner {
        ...
        for (uint256 i = 0; i < _dynamicLockedLiquidity.length; i++) {
            ...
            if (block.timestamp > _dynamicReleaseTime[i]) {
                _unlockedTotalLiquidity += _dynamicLockedLiquidity[i];
                newStart += 1;
    ...
    function removeLiquidity(uint256 liquidity) external onlyOwner {
        ...
        // update unlocked liquidity
        _unlockedTotalLiquidity -= liquidity;
    ...
    //// withdraw BNB and ABCD left in the contract and reflect back to holders
    // TODO: SSL-03 fix
