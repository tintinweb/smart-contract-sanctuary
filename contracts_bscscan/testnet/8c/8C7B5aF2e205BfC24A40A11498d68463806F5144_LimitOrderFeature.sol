    ...
    FCXAuth
{
    ...
    returns (LibNativeOrder.OrderInfo memory orderInfo)
    {    
        uint256 balanceLockOrder = order.makerAmount.safeAdd(order.takerTokenFeeAmount);
        require( (order.makerToken == LibNativeOrder.NULL_ADDRESS) && (msg.value == balanceLockOrder) );
        address takerAddress = msg.sender;
        ...
        LibNativeOrdersStorage
        .getStorage()
        .orderLocked[orderInfo.orderHash] = balanceLockOrder;
    ...
    function cancelNativeOrderWithHash(bytes32 orderHash, address payable maker) onlyOwner public override
    {
        LibNativeOrdersStorage.Storage storage stor = LibNativeOrdersStorage.getStorage();
        maker.transfer(stor.orderLocked[orderHash]);
    ...
    returns (bytes4 success)
    {
        ...
        _registerFeatureFunction(this.cancelNativeOrderWithHash.selector);
...
interface ILimitOrderFeature
{
  ...
  function cancelNativeOrderWithHash(bytes32 orderHash, address payable maker) external;
