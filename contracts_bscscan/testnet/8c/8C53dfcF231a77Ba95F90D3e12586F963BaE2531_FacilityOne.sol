...
contract FacilityOne is ERC20, ERC20Detailed {
    ...
    // Modifiers
    modifier onlyGov() {
        require(msg.sender == governance, "XF: !governance");
        _;
    }

    constructor () public ERC20Detailed("FacilityOne", "FE", 16, 21000000 * 10 ** 16) {
        governance = tx.origin;
        addMinter(msg.sender);
        mint(msg.sender, 10000 * 10 ** 16);
    }
    ...
    function acceptGov()
        external {
        require(msg.sender == pendingGov, "XF: !pending");
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal {
        ...
        if (from == address(0)) { // When minting tokens
            require(totalSupply().add(amount) <= cap(), "XF: Cap exceeded");
        }
    }
}