...
contract KawaiiCrafting is Signed, Pausable, SignData {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    IBEP20 public kawaiiToken;
    IKawaiiRandomness public kawaiiRandomness;
    address public kawaiiDelivery;
    ...
    constructor(IBEP20 _kawaiiToken, IKawaiiRandomness _kawaiiRandomness, address _kawaiiDelivery, uint256 _minFee, uint256 _maxFee)
    public validAddress(address(_kawaiiToken)) validAddress(address(_kawaiiRandomness)) validAddress(_kawaiiDelivery){
        kawaiiToken = _kawaiiToken;
        kawaiiRandomness = _kawaiiRandomness;
        kawaiiDelivery = _kawaiiDelivery;
    ...
    function setKawaiiRandomness(IKawaiiRandomness _kawaiiRandomness) external validAddress(address(_kawaiiRandomness)) onlyOwner {
        kawaiiRandomness = _kawaiiRandomness;
    }
    ...
    function craftingItem(bytes memory data) external whenNotPaused {
        (IERC1155 _nft1155Address,
        uint256[] memory _inputNFTIds,
        uint256[] memory _inputAmounts,
        uint256[] memory _tokenIds,
        uint256[] memory _weights,
        uint256 _craftingPrice,
        uint256 timestamp,
        address sender,
        bytes memory adminSignedData,
        uint8 v, bytes32 r, bytes32 s) = abi.decode(
            data,
            (IERC1155, uint256[], uint256[], uint256[], uint256[], uint256, uint256, address, bytes, uint8, bytes32, bytes32)
        );
        require(_craftingPrice >= minFee && _craftingPrice <= maxFee, "Invalid fee");
        require(kawaiiDelivery != address(0), "Invalid kawaiidelivery");
        {
            uint256 nonce = nonces[sender]++;
            verify(keccak256(abi.encode(CRAFTING_HASH, keccak256(adminSignedData), nonce)), sender, v, r, s);
            (v, r, s) = abi.decode(adminSignedData, (uint8, bytes32, bytes32));
            address signer = getSigner(
                keccak256(
                    abi.encode(address(this), this.craftingItem.selector, _inputNFTIds, _inputAmounts, _tokenIds, _weights, _craftingPrice, timestamp, nonce)
                ), v, r, s);
            require(isSigner[signer], "Forbidden");
            require(_tokenIds.length == _weights.length, "Invalid input length");
            kawaiiToken.safeTransferFrom(sender, kawaiiDelivery, _craftingPrice);
        }
        ...
        uint256 nftId = _tokenIds[selectItem(_weights, rand)];
        _nft1155Address.mint(sender, nftId, 1);
        emit Crafting(sender, nftId);
    }
