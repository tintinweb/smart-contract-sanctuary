...
abstract contract BaseUpgrading is Auth, EIP712Upgradeable {
  ...
  function isSuccess(string calldata _seed, uint8 _acceptedRatio) internal view returns (bool) {
...
contract ItemUpgrading is BaseUpgrading {
  ...
  event Upgraded(address indexed _user, uint8 _gene, UpgradingInfo upgradingInfo, uint _newTokenId, uint _fee, bool success);
  ...
  function upgradeItem(
    ...
    UpgradingInfo memory upgradingInfo = UpgradingInfo(_itemClass, _materials, _eatherId, _paymentCurrency);
  ...
  function _upgradeItem(
    ...
    if (success) {
      ...
      emit Upgraded(msg.sender, gene, _upgradingInfo, newTokenId, fee, success);
    } else {
      emit Upgraded(msg.sender, gene, _upgradingInfo, _willKeepMaterialIdIfFailed, fee, success);
    }
  }
