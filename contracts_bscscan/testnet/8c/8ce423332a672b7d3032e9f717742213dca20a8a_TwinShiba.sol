...
contract TwinShiba is Context, IERC20, Ownable {
    ...
    uint256 public _sellAutoBurnBalance;
    ...
    function _transfer(
        ...
        uint256 contractTokenBalance = balanceOf(address(this)).sub(devBalance.add(vestedBalance));
                    ...
                    if(vestedBalance > 0){
                        if(_buyBackSellCnt.mod(_vestedBurnInterval) == 0){
                            ...
                            if(vestedBurnAmount > balanceOf(address(this))){
                                ...
                                _tokenTransfer(address(this),devAddress,vestedBurnAmount,false);
                            }
                        }
                    }
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(to == uniswapV2Pair){
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee.add(_sellAutoBurn);
                _sellAutoBurnBalance = _sellAutoBurnBalance.add(amount.mul(_sellAutoBurn).div(100));
            }
