...
contract VelaMarketplace is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => mapping(uint256 => mapping(uint256 => Bid))) public bids; // erc721 address => token id => bid id => bid order
    ...
    function acceptBid(address erc721, uint256 tokenId) public whenNotPaused nonReentrant {
        address msgSender = _msgSender();
        uint256 currentBid = currentBids[msgSender][erc721][tokenId];
        Bid memory bidInfo = bids[erc721][tokenId][currentBid];
        ...
        emit BidAccepted(erc721, bidInfo.erc20, bidInfo.bidder, msgSender, bidInfo.price, tokenId, currentBid);
        delete nfts[erc721][tokenId];
        delete currentBids[bidInfo.bidder][erc721][tokenId];
        delete bids[erc721][tokenId][currentBid];
    }
    ...
    function cancelAuctionListing(address erc721, uint256 tokenId) public whenNotPaused nonReentrant{
        ...
        uint256 currentBid = currentBids[msgSender][erc721][tokenId];
        _cancelBid(erc721, tokenId, msgSender, currentBid);
    ...
    function _cancelBid(address erc721, uint256 tokenId, address bidder, uint256 bidId) internal {
