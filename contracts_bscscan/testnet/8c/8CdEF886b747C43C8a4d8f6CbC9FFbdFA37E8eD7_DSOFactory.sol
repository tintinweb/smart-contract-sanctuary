...
contract DSOFactory is ERC721, AbstractAccessControl {
    // TODO: move reusable structs into an interface folder
    struct Club {
        uint32 id;
        string name;
    }

    // TODO: add club to DSO struct
    struct DSO {
        uint256 id;
        uint64 startDate;
        uint64 endDate;
        uint32 row;
        uint32 column;
        string section; // TODO: move to enum
    }
    ...
    mapping(uint256 => uint256) private dsoToOwner;
    ...
    function createDSO(
        ...
        // TODO: check DSO doesn't exist already
        dsos.push(DSO(newItemId, startDate, endDate, row, column, section));
        // TODO: emit new event 'newDSO'
    }
    ...
    modifier canTransfer(uint256 dsoId, uint256 userId) {
        require(
            dsoToOwner[dsoId] == 0x0 || dsoToOwner[dsoId] == userId,
            "DSOFactory: user is not owner of dso."
        );
        _;
    }

    // TODO: onlyOwner or manager should be able to call this method
    function transferDSO(
        uint256 dsoId,
        uint256 from,
        uint256 to
    ) public canTransfer(dsoId, from) {
        dsoToOwner[dsoId] = to;
        // might be yet unnassigned
        if (userDSOCount[from] > 0) {
            userDSOCount[from]--;
        }
        userDSOCount[to]++;
    }

    function getDSOsByOwner(uint256 userId)
        external
        view
        returns (DSO[] memory)
    {
        DSO[] memory result = new DSO[](userDSOCount[userId]);
        uint256 counter = 0;
        for (uint256 i = 0; i < dsos.length; i++) {
            if (dsoToOwner[dsos[i].id] == userId) {
...
abstract contract AbstractAccessControl is AccessControl {
    constructor() {
        // Grants the contract deployer the default admin role: it will be able to grant and revoke any roles
    ...
    modifier onlyAdmin(address account) {
        require(
            ...
            "AbstractAccessControl: account is not an admin."
        );
        _;
    }
}
