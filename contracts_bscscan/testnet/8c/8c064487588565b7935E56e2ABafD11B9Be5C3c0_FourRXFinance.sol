...
//        totalDepositRewards = totalDepositRewards.add(depositReward);
    ...
    function balanceOf(address _userAddress, uint stakeId) public view returns (uint) {
        ...
        uint rewards =  _calcRewards(user.stakes[stakeId]);

        uint withdrawnPlusPenalty = user.stakes[stakeId].withdrawn.add(user.stakes[stakeId].penalty);

        if (rewards < withdrawnPlusPenalty) { // In case user has withdrawn all his rewards
            return 0;
        }

        return _calcRewards(user.stakes[stakeId]).sub(withdrawnPlusPenalty);
    }
    ...
    function withdraw(uint stakeId) external {
        ...
        uint availableAmount = balanceOf(user.wallet, stake.id);
...
//        stake.holdFrom = uint32(block.timestamp);
    ...
    function exitProgram(uint stakeId) external {
        ...
        uint availableAmount = stake.deposit + (_calcRewards(stake) - stake.withdrawn.add(stake.penalty)) - penaltyAmount; // (deposit - entry fee) + (rewards - withdrawn - old penalties) - exitPenalty
