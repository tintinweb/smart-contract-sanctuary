...
abstract contract AbstractDeflationaryToken is Context, IERC20, CoolOff {
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = maxTxPercent >= 1 && maxTxPercent <= 100 ? _tTotal * (maxTxPercent/100) : _tTotal;
    }
...
contract FeeToAddress is Security {
    ...
    function useConvBNB(bool _state) external onlyOwner {
        conversion = _state;
    }
...
contract FeeToAddrDeflAutoLPToken is DeflationaryAutoLPToken, FeeToAddress {
    ...
    // function _isSell(address reciever) internal view returns(bool) {
    //     // Sender = Wallet Sell
