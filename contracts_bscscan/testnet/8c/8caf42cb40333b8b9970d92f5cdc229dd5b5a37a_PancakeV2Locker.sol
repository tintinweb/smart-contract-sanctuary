...
// ALL RIGHTS RESERVED
// PlaylaunchPad reserves all rights on this code. You may NOT copy these contracts.
...
contract PancakeV2Locker is Ownable, ReentrancyGuard {
  ...
  struct FeeStruct {
    ...
    IERCBurn secondaryFeeToken; // Native token
    uint256 secondaryTokenFee; // optional, native token
   ...
   * @param _lpToken the pancakev2 token address
   ...
   * @param _feeInEth fees can be paid in eth or in a secondary token such as native with a discount on pancakev2 tokens
   * @param _withdrawer the user who can withdraw liquidity once the lock expires.
   */
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlockDate, address payable _referral, bool _feeInEth, address payable _withdrawer) external payable nonReentrant {
    ...
    IPancakeV2Pair lpair = IPancakeV2Pair(address(_lpToken));
    ...
    if (!feeWhitelist.contains(msg.sender)) {
      if (_feeInEth) { // charge fee in eth
    ...
    if (msg.sender != MIGRATION_IN) {
      ...
      if (!_feeInEth && !feeWhitelist.contains(msg.sender)) { // fee discount for large lockers using secondary token
