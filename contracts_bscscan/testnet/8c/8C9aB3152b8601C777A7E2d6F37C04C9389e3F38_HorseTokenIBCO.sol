...
/**
 * @dev Implement Initial Bonding Curve Offering for HorseToken Token.
 */
contract HorseTokenIBCO is Ownable {
    using SafeERC20 for IERC20;

    event Claim(address indexed account, uint256 userShare, uint256 HTKAmount);
    event Deposit(address indexed account, uint256 amount);
    event Withdraw(address indexed account, uint256 amount);

    
    uint256 public constant DECIMALS = 10 ** 18; // HorseToken Token has the same decimals as BNB (18)
    uint256 public constant START = 1639708217; 
    uint256 public constant END = 1639710000; 
    uint256 public constant TOTAL_DISTRIBUTE_AMOUNT = 9900000 * DECIMALS;
    uint256 public constant MINIMAL_PROVIDE_AMOUNT = 0.2 ether;
    uint256 public totalProvided = 0;

    mapping(address => uint256) public provided;
    mapping(address => uint256) private accumulated;

    IERC20 public immutable HORSETOKEN;

    constructor(IERC20 HTK) {
        HORSETOKEN = HTK;
    }
    ...
    function deposit() external payable {
        ...
        require(HORSETOKEN.balanceOf(address(this)) == TOTAL_DISTRIBUTE_AMOUNT, "Insufficient HORSETOKEN token in contract");
    ...
    /**
     * @dev Claims HORSETOKEN token from contract by amount calculated on deposited ETH.
     *
     * Requirement:
    ...
    function claim() external {
        ...
        uint256 HTKAmount = _getEstReceivedToken(msg.sender);
        provided[msg.sender] = 0;

        HORSETOKEN.safeTransfer(msg.sender, HTKAmount);

        emit Claim(msg.sender, userShare, HTKAmount);
    }
    ...
    /**
     * @dev Estimate the amount of $HorseToken that can be claim by user
     */
    ...
    /**
     * @dev Get estimated HORSETOKEN token price
     */
    ...
    /**
     * @dev Get estimated amount of HORSETOKEN token an address will receive
     */
    ...
    /**
     * @dev Withdraw the remaining HORSETOKEN tokens to owner's wallet
     *
     * Requirements:
     * - Only the owner can withdraw.
     * - The offering must have been already ended.
     * - Total HORSETOKEN provided is smaller than MINIMAL_PROVIDE_AMOUNT
     */
    function withdrawRemainedHORSETOKEN() external onlyOwner {
        require(END < block.timestamp, "The offering has not ended");
        require(totalProvided < MINIMAL_PROVIDE_AMOUNT, "Total provided must be less than minimal provided");

        uint256 remainedHorseToken = TOTAL_DISTRIBUTE_AMOUNT -
            ((TOTAL_DISTRIBUTE_AMOUNT * totalProvided) / MINIMAL_PROVIDE_AMOUNT) - 1;
        HORSETOKEN.safeTransfer(owner(), remainedHorseToken);
    }

    /**
     * @dev Withdraw the HORSETOKEN tokens that are unclaimed (YES! They are abandoned!)
     *
     * Requirements:
     * - Only the owner can withdraw.
     * - Withdraw date must be more than 30 days after the offering ended.
     */
    function withdrawUnclaimedHORSETOKEN() external onlyOwner {
        require(END + 30 days < block.timestamp, "Withdrawal is unavailable");
        require(HORSETOKEN.balanceOf(address(this)) != 0, "No token to withdraw");

        HORSETOKEN.safeTransfer(owner(), HORSETOKEN.balanceOf(address(this)));
    }
}
