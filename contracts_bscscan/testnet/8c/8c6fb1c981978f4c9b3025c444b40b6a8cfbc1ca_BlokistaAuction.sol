...
contract BlokistaAuction is Ownable {
...
modifier _validateBuyer(uint256 _tokenId) {
       ...
       require(msg.value >= IBlokistaVault(BlokistaVault).getPrice(_tokenId), "Error, the amount is lower");
        require(IBlokistaVault(BlokistaVault).validateBuyer(_tokenId,_msgSender()),"Validation failed");
        _;
    }
modifier _validateOfferer(uint256 _tokenId,uint256 _offer){
        address _sender=_msgSender();
        require(IBlokistaVault(BlokistaVault).validateOfferer(_tokenId, _offer, _sender),"Validation failed");
        _;
}
...
// Selling without auction can sell multiple copy at once and can even sell any nonrelated nft's at the same time too.
    ...
    function justSell(uint256[] memory _tokenIdler,uint256 _numberOfCopy,uint256[] memory prices) external{
        ...
        require(IBlokistaVault(BlokistaVault).getStatus(_tokenIdler[i])==0,"It is already listed or in auction");
        require(IBlokistaVault(BlokistaVault).validateTokenOwner(_tokenIdler[i],_msgSender()),"Validation Failed");
    ...
    // This function is for buying from justSell option or instant buyer!
      function buy(uint256 _tokenId) external payable _validateBuyer(_tokenId) {
          require(IBlokistaVault(BlokistaVault).getStatus(_tokenId)==2 || IBlokistaVault(BlokistaVault).getInstantSellPrice(_tokenId)>0 ,"You can't buy. This token is on Auction.");
          ...
          if(instPrice>0){
              ...
              emit JustPurchased(_sellerman,_buyerman,instPrice,_tokenId,IERC721Metadata(BlokistaVault).tokenURI(_tokenId));
     ...
     function increaseBid(uint256 _tokenId, uint256 _price) internal returns(uint256){
        ...
        uint256 oldPrice = IBlokistaVault(BlokistaVault).getPrice(_tokenId);
        IBlokistaVault(BlokistaVault).setPrice(_tokenId,_price);

        return oldPrice;
    }

    function updatePriceAsOwner(uint256 _tokenId, uint256 _price) external _validateTokenOwner(_tokenId){
...
// Only owner of token can use this function    
    ...
    function makeOffer(uint256 _tokenId, uint256 _offer) external payable _validateOfferer(_tokenId,_offer) {
        if(IBlokistaVault(BlokistaVault).getDeadline(_tokenId)!=0){
      require(IBlokistaVault(BlokistaVault).getDeadline(_tokenId)>=block.timestamp,"Auction reached to the deadline :/ ");
        }
        ...
        IBlokistaVault(BlokistaVault).setBidders(_tokenId, _msgSender());
       ...
       IWETH(wbnb).deposit{value: _offer}();
        ...
        if (msg.value > _offer) TransferHelper.safeTransferETH(msg.sender, msg.value - _offer);
    emit MadeOffer(_msgSender(),_tokenId,_offer);
    }


    function acceptOffer(uint256 _id) external  _validateTokenOwner(_id){
        require( IBlokistaVault(BlokistaVault).getStatus(_id)==1,"This token is not on an Auction");
        address payable _owner = payable(IERC721(BlokistaVault).ownerOf(_id));
        IBlokistaVault(BlokistaVault).setStatus(_id,0);
   ...
   emit AuctionEnded(_owner,_lastBidder,_lastPrice,_id,IERC721Metadata(BlokistaVault).tokenURI(_id));
...
function _justTrade(uint256 _id) internal {
    ...
    uint256 _sellerValue = (_price.sub(_commissionValue)).sub(_loyaltyValue);
        address payable _creatorArtist=payable(IBlokistaVault(BlokistaVault).getCreatorArtist(_id));
    ...
    // If buyer sent more than price, we send them back their rest of funds
     ...
     function _trade(uint256 _id,uint256 _price) internal {
         
        address payable _owner = payable(IERC721(BlokistaVault).ownerOf(_id));
        address payable _buyer=payable(IBlokistaVault(BlokistaVault).getBidders(_id));
        IBlokistaVault(BlokistaVault).setBidders(_id, address(0));
        address payable _creatorArtist=payable(IBlokistaVault(BlokistaVault).getCreatorArtist(_id));
        uint8 _loyaltyFee=IBlokistaVault(BlokistaVault).getloyaltyFee(_id);
        IBlokistaVault(BlokistaVault).justTransfer(_owner,_buyer, _id);
        uint8 feePercent=IBlokistaVault(BlokistaVault).getFeePercent();
    ...
    uint256 _sellerValue = (_price.sub(_commissionValue)).sub(_loyaltyValue);
       ...
       IERC20(wbnb).safeTransfer(payable(adminFeeAddress),_commissionValue);
        IERC20(wbnb).safeTransfer(payable(_creatorArtist),_loyaltyValue);
     ...
     */
    function burn(uint256 _tokenId) external _validateTokenOwner(_tokenId) {
       
        require(IBlokistaVault(BlokistaVault).getStatus(_tokenId)==0," Remove token from listing");
