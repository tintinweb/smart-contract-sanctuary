...
contract BouiSingle is
    ...
    
 {
    ...
    uint256 public discountMinAmount;
    uint256 public discountMinPercent;
    uint256 public test;
    ...
    function updateDiscountAmount(uint256 _discountamount) public onlyOwner{
        discountMinAmount = _discountamount;
    }

    function updateDiscountPercent(uint256 _discountPercent) public onlyOwner{
        discountMinPercent = _discountPercent;
    }
    ...
    function saleToken(
        ...
        _saleToken(from, tokenId, amount,_type);
    ...
    //         "Insufficent found"
    //     );
        BEP20 t = BEP20(tokentype[_type]);
        uint256 userbalance =  t.balanceOf(msg.sender);
        uint256 discountamt=0;
        uint256 bal=userbalance.div(1e18);
        if(bal>=discountMinAmount){
            test = amount;
            test1=discountMinPercent;
            discountamt = (amount*discountMinPercent).div(1000);
            test2=discountamt;
            amount=amount.sub(discountamt);
            test3=amount;
        }
        uint256 val = pBEPent(amount, serviceValue).add(amount).sub(discountamt/1e18);
        ...
        // require( msg.value == _adminfee.add(roy.add(netamount)), "Insufficient Balance11");
