...
contract NFTArtifacts is Ownable, INFTArtifacts, INFTArtifactsMetadata {
    ...
    mapping(uint256 => uint256[]) private _typeToArtifactIds;
    mapping(uint256 => uint256) private _typeToTotalRarity;
    ...
    mapping(uint256 => mapping(uint256 => RarityInfo))
        private _typeToArtifactIdToRarity;
    ...
    function addArtifact(
        ...
        _typeToArtifactIds[artifactType_].push(artifactId_);
        RarityInfo
            storage _rarityInfoForSpecificType = _typeToArtifactIdToRarity[
                artifactType_
            ][artifactId_];
        _rarityInfoForSpecificType.zeroIndex = _typeToTotalRarity[
            artifactType_
        ];
        _rarityInfoForSpecificType.rarity = rarity_;
        _typeToTotalRarity[artifactType_] += rarity_;
        ...
        returns (uint256 _artifactId)
    {
        ...
        require(
            _typeToTotalRarity[artifactType_] > 0,
        ...
        uint256 _randomNumber = randomNumber_ %
            _typeToTotalRarity[artifactType_];

        for (uint256 i = 0; i < _typeToArtifactIds[artifactType_].length; i++) {
            RarityInfo storage _rarityInfo = _typeToArtifactIdToRarity[
                artifactType_
            ][_typeToArtifactIds[artifactType_][i]];
            if (
                _rarityInfo.zeroIndex <= _randomNumber &&
                _randomNumber < _rarityInfo.zeroIndex + _rarityInfo.rarity
            ) {
                _artifactId = _typeToArtifactIds[artifactType_][i];
                break;
            }
        }
    }
