...
abstract contract DABotSetting is DABotShare {
    ...
    event SettingChanged(uint what, DABotCommon.BotSetting);
...
abstract contract DABotStaking is DABotShare, Context, Ownable {
    ...
    */
    ...
    function certificateOf(IRoboFiToken asset) public view returns(CertToken) {
        return CertToken(_portfolio[asset].certToken);
    }
    ...
    function stake(IRoboFiToken asset, uint amount) external virtual {
        ...
        require(pAsset.certToken != address(0), ERR_INVALID_PORTFOLIO_ASSET);
     ...
     */
    function unstake(CertToken certToken, uint amount) external virtual {
        if (amount == 0) return;
    ...
    function _mintCertificate(IRoboFiToken asset, uint amount) internal {
        DABotCommon.PortfolioAsset storage pAsset = _portfolio[asset];
        asset.transferFrom(_msgSender(), address(pAsset.certToken), amount);
        CertToken token = CertToken(pAsset.certToken);
        ...
        if (duration == 0) {
            ...
            locker = botManager.factory().deploy(warmupLocker, 
                    abi.encode(address(this), _msgSender(), pAsset.certToken, block.timestamp, block.timestamp + duration), true);
    ...
    function _unstake(address account, CertToken certToken, uint amount) internal virtual {
        ...
        address asset = address(assetOf(address(certToken)));
    ...
    function _updatePortfolio(IRoboFiToken asset, uint maxCap, uint iboCap, uint weight) internal {
        ...
        if (address(pAsset.certToken) == address(0)) {
            pAsset.certToken = botManager.deployBotCertToken(address(asset));
            _assets.push(asset);
        }
    ...
    function _removeAsset(IRoboFiToken asset) internal {
        ...
        address certToken = _portfolio[asset].certToken;

        CertToken(certToken).finalize();
 ...
 */
abstract contract DABotBase is DABotSetting, DABotGovernance {
    ...
    function init(bytes calldata data) external virtual payable override {
        ...
        address holder;
        DABotCommon.PortfolioCreationData[] memory portfolio;
        (_botSymbol, _botName, holder, _setting, portfolio) = 
            abi.decode(data, (string, string, address, DABotCommon.BotSetting, DABotCommon.PortfolioCreationData[]));

        require(_setting.iboEndTime() > _setting.iboStartTime(), "DABot: end time < start time");
        require(_setting.initDeposit >= botManager.minCreatorDeposit(), "DABot: insufficient deposit");
        require(_setting.initFounderShare > 0, "DABot: zero founder share");
        ...
        for(uint idx = 0; idx < portfolio.length; idx++) {
            _updatePortfolio(IRoboFiToken(portfolio[idx].asset), portfolio[idx].cap, portfolio[idx].iboCap, portfolio[idx].weight);
        }
    ...
    function botDetails() view external returns(DABotCommon.BotDetail memory output) {
        IDABot template = IDABot(masterContract);

        output.botAddress = address(this);
        output.masterContract = masterContract;
        output.botSymbol = _botSymbol;
        output.botName = _botName;
        output.botType = template.botType();
        output.status = status();
        output.templateName = template.botSymbol();
        output.templateVersion = template.version();
...
contract CEXDABot is DABotBase {
    ...
    function botType() external pure returns(uint8) {
        return 0; /* CEX DABot */
    }
}
