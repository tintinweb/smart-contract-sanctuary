...
contract FarmageddonLottoKeeper is KeeperCompatibleInterface, Ownable {
    ...
    function StartKeeper(uint256 _fCurrentLotteryId, address _lotteryAddress, uint256 _upKeepTime, uint256 _step) external onlyOwner {
        ...
        fl = FarmageddonLottery(_lotteryAddress);
    }
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        if (step == 1) {
            fl.closeLottery(fcurrentLotteryId);
            step = 2;
            upKeepTime += 300;
            ...
            fl.drawFinalNumberAndMakeLotteryClaimable(fcurrentLotteryId, true, fburnDivisor);
            step = 3;
            upKeepTime += 300;
            ...
            fl.startLottery(upKeepTime, fticketPrice, fdiscount, frewardsBreakdown, ftreasuryFee);
            step = 1;
            fcurrentLotteryId += 1;
