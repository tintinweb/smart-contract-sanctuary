...
contract ICO is Ownable, Initializable {
    ...
    function buy() external payable {
        ...
        user[msg.sender].rewardClaim = block.timestamp;
    ...
    function claimRewards( address account) public onlyAuth returns (uint){
        if(user[account].rewardClaim == 0) user[account].lastRewardClaim = user[account].rewardClaim;

        (uint rewards, uint totMonth) = getReward( account);
        user[account].totalRewardClaimed += rewards;
        user[account].lastRewardClaim = 30 days * totMonth;
        return rewards; 
    }

    function getReward( address account) public view returns ( uint, uint) {
        uint totalClaimPeriod = (claimPeriod[0] + (claimPeriod[1] * 2));
        uint lastClaimTime = user[account].lastRewardClaim;
        uint currentTime = block.timestamp;

        if(lastClaimTime >= (user[account].rewardClaim + totalClaimPeriod))
            return (0,0);

        if(currentTime > user[account].rewardClaim + totalClaimPeriod) 
            currentTime = user[account].rewardClaim + totalClaimPeriod;
        
        if(lastClaimTime == 0) lastClaimTime = user[account].rewardClaim;
        
        uint totMonth = (currentTime - lastClaimTime)/30 days;
        uint rewards = user[account].balance * (reward * totMonth) / 100e18;
        return (rewards,totMonth);
    }
