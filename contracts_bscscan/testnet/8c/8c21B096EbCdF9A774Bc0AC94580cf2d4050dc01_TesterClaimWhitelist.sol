...
contract TesterClaimWhitelist is AccessControl {
    using SafeMath for uint;
    using SafeBEP20 for IBEP20;
    IBEP20 public busd;
    bytes32 public constant CREATOR_ADMIN = keccak256("CREATOR_ADMIN");
    uint256 public amountClaim = 5000000000000000000000; 
    event TransferBusd(
        address owner,
        uint256 amount,
        uint256 timeTransfer
    );
    constructor( address minter, address _busd ) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(CREATOR_ADMIN, minter);
        busd = IBEP20(_busd); // Token Heroes
	}
    ...
    function changeAmountClaim(uint256 _amountClaim) public {
        require(hasRole(CREATOR_ADMIN, address(msg.sender)), "Caller is not a admin");
    ...
    function claimBusd() public {
        require(whitelistAddress[msg.sender],"Not found in whitelist");
        safeBUSDTransfer(msg.sender, amountClaim); 
        whitelistAddress[msg.sender] = false;   
    }
    function safeBUSDTransfer(address _to, uint256 _amount) internal {
        ...
        busd.transfer(_to, amountTransfer);
        emit TransferBusd(
            msg.sender,
            amountTransfer,
            block.timestamp
