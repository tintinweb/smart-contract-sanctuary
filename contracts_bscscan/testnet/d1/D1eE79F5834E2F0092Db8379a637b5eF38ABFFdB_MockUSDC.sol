...
contract MockToken is MockERC20 {
    ...
    function mint(
        ...
        uint256 _token_out_min
    ) public {
        uint256 amount = _collateral_amount / price;
        require(amount >= _token_out_min, 'Token: amountOut <= _token_out_min');
        IERC20(usd).transferFrom(msg.sender, address(this), _collateral_amount);
        _mint(msg.sender, amount);
    }
}
