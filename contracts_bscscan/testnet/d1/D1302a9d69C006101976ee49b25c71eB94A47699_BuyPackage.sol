...
contract BuyPackage is Ownable , NftLegendCard{
    ...
    uint256 nflDecimal;
    uint256 amountBuyPackage = 200000000;
    address addressReceive;
	uint256 public package = 10;
	mapping(address => uint256) public packageUser;
    ...
    constructor( address _tokenerc721 ,address _tokennfl,  address _addressReceive, uint256 _nflDecimal) public {
		tokennfl = IBEP20(_tokennfl); // Token NFL
		tokenerc721 = NftLegendCard(_tokenerc721); // Token NFTLegendCard
		nflDecimal = _nflDecimal;
    ...
    function buyEgg(uint256 _amount) public {
        require(_amount == amountBuyPackage, "Invalid quantity");       
        tokennfl.safeTransferFrom(msg.sender, addressReceive , _amount);
		packageUser[msg.sender] = packageUser[msg.sender].add(package);
		emit historyBuyEgg(
			msg.sender,
			block.timestamp,
			block.number,
	...
	function harvestCard(uint256 _tokenId, string memory _tokenURI) public {
		require(packageUser[msg.sender] > 0, 'need number package > 0');
		tokenerc721.mint(msg.sender, _tokenId , _tokenURI);		
		packageUser[msg.sender] = packageUser[msg.sender].sub(1);
		emit historyHarvestCard(
			msg.sender,
			block.timestamp,
			block.number,
			_tokenId,
