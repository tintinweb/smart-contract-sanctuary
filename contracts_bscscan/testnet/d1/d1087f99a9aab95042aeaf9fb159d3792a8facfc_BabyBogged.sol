...
/**
 .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------. 
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| |   ______     | || |      __      | || |   ______     | || |  ____  ____  | || |   ______     | || |     ____     | || |    ______    | || |    ______    | || |  _________   | || |  ________    | |
| |  |_   _ \    | || |     /  \     | || |  |_   _ \    | || | |_  _||_  _| | || |  |_   _ \    | || |   .'    `.   | || |  .' ___  |   | || |  .' ___  |   | || | |_   ___  |  | || | |_   ___ `.  | |
| |    | |_) |   | || |    / /\ \    | || |    | |_) |   | || |   \ \  / /   | || |    | |_) |   | || |  /  .--.  \  | || | / .'   \_|   | || | / .'   \_|   | || |   | |_  \_|  | || |   | |   `. \ | |
| |    |  __'.   | || |   / ____ \   | || |    |  __'.   | || |    \ \/ /    | || |    |  __'.   | || |  | |    | |  | || | | |    ____  | || | | |    ____  | || |   |  _|  _   | || |   | |    | | | |
| |   _| |__) |  | || | _/ /    \ \_ | || |   _| |__) |  | || |    _|  |_    | || |   _| |__) |  | || |  \  `--'  /  | || | \ `.___]  _| | || | \ `.___]  _| | || |  _| |___/ |  | || |  _| |___.' / | |
| |  |_______/   | || ||____|  |____|| || |  |_______/   | || |   |______|   | || |  |_______/   | || |   `.____.'   | || |  `._____.'   | || |  `._____.'   | || | |_________|  | || | |________.'  | |
| |              | || |              | || |              | || |              | || |              | || |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------' 
https://t.me/BabyBoggedToken

Website is still under developement for the time being.

I will do some marketing only if the community decides to hold strong and involve in telegram shilling and other sorts of shilling.

Supply: 100.000.000.000
Decimals: 18 

Tokenomics:
15% Bogged rewards fee
4% Liquidity fee
2% Marketing fee 
*/
...
// File: contracts/BabyBoggedDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable DividendsToken = address(0xB09FE1613fE03E7361319d2a43eDc17422f36B09);
...
contract BabyBogged is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   BabyBoggedDividendTracker public dividendTracker;

    address public liquidityWallet;
    address public marketingWallet = 0x40eA6A1471202FFBa9a051973e6eB75531291F1e;
    ...
    constructor() public ERC20("BabyBogged", "BYBG") {
        uint256 _rewardsFee = 15;
    	...
    	dividendTracker = new BabyBoggedDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyBogged: The dividend tracker already has that address");

        BabyBoggedDividendTracker newDividendTracker = BabyBoggedDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyBogged: The new dividend tracker must be owned by the BabyBogged token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyBogged: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyBogged: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyBogged: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyBogged: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyBogged: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyBogged: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyBogged: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BabyBogged: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of BabyBogged
            path,
            recipient,
            block.timestamp
        );
...
contract BabyBoggedDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BabyBogged_Dividend_Tracker", "BabyBogged_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "BabyBogged_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BabyBogged_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyBogged contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyBogged_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyBogged_Dividend_Tracker: Cannot update claimWait to same value");
