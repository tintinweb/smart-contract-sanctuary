...
contract RaffleWorld is Ownable, VRFConsumerBase {
    ...
    modifier checkTicketsAcquisition(uint256 _raffleId, uint256 _ticketsNo) {
        require(
            ...
            "RaffleWorld: this raffle is canceled!"
        );
        require(
            keccak256(bytes(raffles[_raffleId].status)) !=
                keccak256(bytes("ended")),
    ...
    function _receiveFunds(address _tokenAddress, uint256 _amount) internal {
        if (_tokenAddress == address(0)) {
            require(
                msg.value == _amount,
                "RaffleWorld: you don't have enough funds!"
            );
        } else {
    ...
    */
    function addRaffle(
        ...
        require(
            ...
            "RaffleWorld: raffle type or start date incorrect!"
        );
    ...
    */
    function _cancelRaffle(uint256 _raffleId) internal {
        ...
        if (
            ...
            active_bronze_raffles[
                ...
                rafflesIndices[active_bronze_raffles.length - 1]
        ...
        onlyOwner
    {
        require(
            _prizeAmount > 0,
            "RaffleWorld: the prize should be greater than 0!"
        );
        ...
        onlyOwner
    {
        require(
            _ticketPrice > 0,
            "RaffleWorld: ticket price should be greater than 0!"
        );
        ...
        onlyOwner
    {
        ...
        emit SetRaffleTicketsNumber(_msgSender(), _raffleId, _ticketsNumber);
    }
        ...
        onlyOwner
    {
        require(
            ...
            "RaffleWorld: raffle start date should be in the future!"
        );
        raffles[_raffleId].raffleStartDate = _raffleStartDate;
        emit SetRaffleStartDate(_msgSender(), _raffleId, _raffleStartDate);
    }
    ...
    function getRandomNumber() public returns (bytes32 requestId) {
        return requestRandomness(keyHash, requiredRaffleLink);
    }
        ...
        beforeWithdraw(_raffleId)
    {
        ...
        ticketsPrice = ticketsPrice.sub(ticketsPrice.div(10));
        _giveFunds(
            _msgSender(),
    ...
    function withdrawOwnerFunds(uint256 _raffleId) external onlyOwner {
        require(
            keccak256(bytes(raffles[_raffleId].status)) ==
                keccak256(bytes("ended"))
        );
        uint256 ownerFunds = raffles[_raffleId]
            .ticketPrice
            .mul(raffles[_raffleId].ticketsNumber)
            .sub(raffles[_raffleId].prizeAmount);
        _giveFunds(owner(), raffles[_raffleId].tokenContract, ownerFunds);
    }
}
