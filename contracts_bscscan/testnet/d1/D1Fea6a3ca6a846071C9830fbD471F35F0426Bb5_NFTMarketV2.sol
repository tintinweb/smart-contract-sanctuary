    ...
    AccessControlUpgradeable
{
    ...
    function _isNotListed(IERC721 _tokenAddress, uint256 id) internal view {
        require(
            !listedTokenTypes.contains(address(_tokenAddress)) ||
                !listedTokenIDs[address(_tokenAddress)].contains(id),
    ...
    function _isNotListedOnPack(IERC721 _tokenAddress, uint256 id) internal view {
        require(
            !listedTokenTypes.contains(address(_tokenAddress)) ||
                !packListedTokenIDs[address(_tokenAddress)].contains(id),
            "Token ID must not be listed"
    ...
    function _isHatched(IERC721 _tokenAddress, uint256 _id) internal view {
        require(
    ...
    modifier isMultiHatched(IERC721 _tokenAddress, uint256[] memory ids) {
        for (uint256 i = 0; i < ids.length; i++) {
            _isHatched(_tokenAddress, ids[i]);
        }
        _;
    }
        ...
        isMultiNotListedOnPack(_tokenAddress, _ids)
    {
        ...
        emit NewPackListing(msg.sender, _tokenAddress, packId, _price);
    }
        ...
        isHatched(_tokenAddress, _id)
    {
        ...
        _updateStatesWhenPurchase(_tokenAddress, _id);
    ...
    function purchaseListings(
        IERC721 _tokenAddress,
        uint256[] memory _ids
    )
        public
        userNotBanned
        isMultiListed(_tokenAddress, _ids)
        isMultiHatched(_tokenAddress, _ids)
    {
        uint256 totalTax;
        address[] memory sellers = new address[](_ids.length);
        uint256[] memory finalPrices = new uint256[](_ids.length);

        for (uint256 i = 0; i < _ids.length; i++) {
            uint256 currentId = _ids[i];
            uint256 finalPrice = getFinalPrice(_tokenAddress, currentId);
            finalPrices[i] = finalPrice;

            Listing memory listing = listings[address(_tokenAddress)][currentId];
            require(isUserBanned[listing.seller] == false, "Banned seller");
            uint256 taxAmount = getTaxOnListing(_tokenAddress, currentId);
            totalTax += taxAmount;
            sellers[i] = listing.seller;

            _updateStatesWhenPurchase(_tokenAddress, currentId);
            ...
            _tokenAddress.safeTransferFrom(address(this), msg.sender, currentId);
        ...
        buniToken.safeTransferFrom(msg.sender, taxRecipient, totalTax);

        // emit PurchasedListings(
        //     msg.sender,
        //     sellers,
        //     _tokenAddress,
        //     _ids,
    ...
    function _updateStatesWhenPurchase(IERC721 _tokenAddress, uint256 _id) private {
        delete listings[address(_tokenAddress)][_id];
        listedTokenIDs[address(_tokenAddress)].remove(_id);
        _updateListedTokenTypes(_tokenAddress);
    }
