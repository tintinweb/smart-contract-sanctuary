...
contract LamboStake is Context, Ownable, ReentrancyGuard {
    ...
    function _switchPool(address tokenAddress,uint256 toPool) private {
		...
		require(_lastSwitchPools[tokenAddress] + _getStakePeriodFor(tokenAddress) <= block.timestamp,"Stake period not finished, cannot switch pool now !");
	...
	function checkIfNeedToSwitchPool(address tokenAddress) private {
		if (_poolEnabled[tokenAddress] && _lastSwitchPools[tokenAddress] + _getStakePeriodFor(tokenAddress) <= block.timestamp) {
    ...
    function setPoolTokensByPeriodFor(address tokenAddress,uint256 amount) external onlyAuthorizedCallers nonReentrant {
        _poolTokensByPeriod[tokenAddress] = amount;
    }
    
    function getPoolTokensByPeriodFor(address tokenAddress) public view returns  (uint256) {
        return _poolTokensByPeriod[tokenAddress];
    }
    ...
    function stakeTokens(address tokenAddress,uint256 amount) external isHuman nonReentrant {
        ...
        _userStakes[tokenAddress][msg.sender] = _userStakes[tokenAddress][msg.sender] + amount;
        _totalStaked[tokenAddress] = _totalStaked[tokenAddress] + amount;
        _nextClaimDate[tokenAddress][msg.sender] = block.timestamp + _getStakePeriodFor(tokenAddress);
		emit NextClaimDate(tokenAddress,msg.sender,_nextClaimDate[tokenAddress][msg.sender]);
    ...
    function estimatedRewards(address tokenAddress,address account) public view returns (uint256) {
		...
		if (_nextClaimDate[tokenAddress][account] <= _lastSwitchPools[tokenAddress] + _getStakePeriodFor(tokenAddress)) {
			poolAmount = _currentTotalRewardPools[tokenAddress];
			currentPoolAmount = _currentRewardPools[tokenAddress];
		}		
        bool previous = false;
        if (_nextClaimDate[tokenAddress][account] <= _lastSwitchPools[tokenAddress]) {
            ...
            poolAmount = _previousTotalRewardPools[tokenAddress];
            currentPoolAmount = _previousRewardPools[tokenAddress];
            previous = true;
        }
        if (poolAmount > 0) {
	        ...
	        if (reward > currentPoolAmount) {
				if (_totalStaked[tokenAddress] == stakerBalance) {
					reward = currentPoolAmount;	
				} else {
					reward = currentPoolAmount.mul(rewardPercentage).div(1000000);
				}
	        }
	        return reward;
        } else {
    ...
    function estimatedRewardsIfCouldClaimNow(address tokenAddress,address account) public view returns (uint256) {
        if (!_poolEnabled[tokenAddress]) return 0;
        uint256 poolAmount = _currentTotalRewardPools[tokenAddress];
        if (poolAmount > 0) {
			uint256 stakerBalance = _getStakedAmountFor(tokenAddress,account);
	        uint256 rewardPercentage = stakerBalance.mul(1000000).div(_totalStaked[tokenAddress]);
	        return poolAmount.mul(rewardPercentage).div(1000000);
        } else {
            return 0;
        }
    }
    ...
    function _claimRewards(address tokenAddress,address account) private {
        ...
        if (_nextClaimDate[tokenAddress][account] <= block.timestamp) {
            ...
            if (reward > 0) {
				...
				_nextClaimDate[tokenAddress][account] = block.timestamp + _getStakePeriodFor(tokenAddress);
				emit NextClaimDate(tokenAddress,account,_nextClaimDate[tokenAddress][account]);
		        if (previous) {
