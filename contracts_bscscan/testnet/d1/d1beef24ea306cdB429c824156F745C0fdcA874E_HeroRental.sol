...
contract HeroRental is Ownable {
    ...
    function cancelSignature(
        ...
        require(
            ...
            "HeroRental: invalid heroOwner signature"
        );
    ...
    function rentHero(
        ...
        require(
            ...
            "HeroRental: signature is used or canceled. please send another transaction with new signature"
        );
        ...
        require(
            ...
            "HeroRental: renter doesn't have enough token to rent this item"
        );
        require(
            paymentContract.allowance(_msgSender(), address(this)) >= values[1],
            "HeroRental: renter doesn't approve HeroRental to spend payment amount"
        );

        // lock thetan hero
        ThetanHero nft = ThetanHero(addresses[2]);
        require(
            nft.ownerOf(values[0]) == addresses[0],
            "HeroRental: HeroOwner is not owner of this item now"
        );
        nft.lock(values[0]);
    ...
    function unlockHero(address thetanHero, uint256 tokenId) public onlyOwner {
        ThetanHero nft = ThetanHero(thetanHero);
        nft.unlock(tokenId);
        // Emit rent event
    ...
    function getMessageHash(
        ...
        return
            keccak256(
                abi.encodePacked(_tokenId, _price, _paymentErc20, _saltNonce)
            );
    }
}
