...
contract LandNFTSale is IMidoTokenSale, Whitelist {

    using Address
    for address;

    uint64 public saleRoudIndex;
    uint64 public totalRoud;

    bool public publicSale = false;

    LandNFT public immutable landNFT;

    SaleNFTConfig[] internal _saleConfig;

    mapping(uint64 => SaleRecord) internal _saleRecord;
    mapping(address => mapping(uint64 => UserRecord)) internal _userRecord;
    ...
    /**
     * @dev Call to update status of public sale
     * @param _publicSale is status of public sale
     */
    function startSale(bool _publicSale) external onlyRole(DEFAULT_ADMIN_ROLE) {
        publicSale = _publicSale;
        emit SaleBegins(publicSale);
    }

    /**
     * @dev Call to update the index of current sale round.
     * Can only be called by Owner.
     * @param _saleRoudIndex is index of a sale config list
     */
    function setSaleRound(uint64 _saleRoudIndex) external onlyRole(DEFAULT_ADMIN_ROLE) {
        saleRoudIndex = _saleRoudIndex;
    }
    ...
    function buy(uint64 amount) external payable {
        ...
        require(!buyer.isContract() || hasRole(DEFAULT_ADMIN_ROLE,buyer), 'only EOA or owner');
        require(publicSale, "public sale not started");

        _validateAndUpdateWithBuyAmount(buyer, amount);

        landNFT.mintNFT(amount, buyer);
    }


    /**
     * @dev Add a new sale by the owner only
     * @param _whitelistTime must be greater than @param _publicTime
     * @param _publicTime must be greater than @param _endTime
     * The opening time must be greater than the end time of the previous sale
     * @param _whitelistTime must be greater than end time of the previous sale
     */
    function newSale(
        ...
        uint64 _endTime,
        uint64 _maxSupply,
        uint64 _capPerWhitelistedAddress,
        uint64 _capPerAddress,
        uint256 _salePrice
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_whitelistTime <= _publicTime, 'whitelistTime is greater than publicTime');
        require(_publicTime < _endTime, 'publicTime is greater than or equal to endTime');

        if (_saleConfig.length > 0) {
            SaleNFTConfig memory lastConfig = _saleConfig[saleRoudIndex];
            require(_whitelistTime > lastConfig.endTime, 'The opening time must be greater than the end time of the previous sale');
        }

        _saleConfig.push(SaleNFTConfig({
            whitelistTime: _whitelistTime,
            publicTime: _publicTime,
            endTime: _endTime,
            maxSupply: _maxSupply,
            capPerWhitelistedAddress: _capPerWhitelistedAddress,
            capPerAddress: _capPerAddress,
            salePrice: _salePrice
        }));

        saleRoudIndex = uint64(_saleConfig.length - 1);
        totalRoud = uint64(_saleConfig.length);
        landNFT.updateTotalSupplyWithNewSale(_maxSupply);
    }
     ...
     * @param _saleRoudIndex is index of a sale config list
     */
    function getSaleConfig(uint64 _saleRoudIndex) external view returns(SaleNFTConfig memory config) {
        require(_saleRoudIndex <= saleRoudIndex, '_saleRoudIndex is out of the list sale config');
        config = _saleConfig[_saleRoudIndex];
    }

    /**
     * @dev Return the record, with number of tokens have been sold for different groups
     */
    function getSaleRecord(uint64 _saleRoudIndex) external view returns(SaleRecord memory record) {
        require(_saleRoudIndex <= saleRoudIndex, '_saleRoudIndex is out of the list record');
        record = _saleRecord[_saleRoudIndex];
    }

    /**
     * @dev Return the user record
     */
    function getUserRecord(address user, uint64 _saleRoudIndex) external view returns(UserRecord memory record) {
        require(_saleRoudIndex <= saleRoudIndex, '_saleRoudIndex is out of the list record');
        record = _userRecord[user][_saleRoudIndex];
    }
     ...
     *      - each whitelisted buyer can buy up to capPerWhitelistedAddress tokens with salePrice per token
     *    3. If the buy time is in public buy time:
     *      - each buyer can buy up to total of capPerAddress tokens with salePrice per token
     */
    function _validateAndUpdateWithBuyAmount(address buyer, uint64 amount) internal {
        SaleNFTConfig memory config = _saleConfig[saleRoudIndex];
        ...
        if (config.whitelistTime <= timestamp && timestamp < config.publicTime) {
            ...
            // whitelisted address can buy up to capPerWhitelistedAddress token
            require(totalPaid >= amount * config.salePrice, 'invalid paid value');
            require(
                _userRecord[buyer][saleRoudIndex].whitelistBought + amount <= config.capPerWhitelistedAddress,
                'whitelisted cap reached'
            );
            _userRecord[buyer][saleRoudIndex].whitelistBought += amount;
            _saleRecord[saleRoudIndex].totalWhitelistSold += amount;
            _saleRecord[saleRoudIndex].totalSold += amount;
            emit WhitelistBought(buyer, amount, totalPaid);
            return;
        }

        if (config.publicTime <= timestamp && timestamp < config.endTime) {
            // anyone can buy up to capPerAddress tokens with price of salePrice eth per token
            // it is applied for total of whitelistBought + publicBought
            require(totalPaid == amount * config.salePrice, 'invalid paid value');
            require(
                _userRecord[buyer][saleRoudIndex].publicBought + _userRecord[buyer][saleRoudIndex].whitelistBought + amount <= config.capPerAddress,
                'normal cap reached'
            );
            _userRecord[buyer][saleRoudIndex].publicBought += amount;
            _saleRecord[saleRoudIndex].totalPublicSold += amount;
            _saleRecord[saleRoudIndex].totalSold += amount;
            emit PublicBought(buyer, amount, totalPaid);
        }
    }

    function withdrawSaleFunds(address payable recipient, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
...
interface IMidoTokenSale {
    ...
    struct SaleNFTConfig {
        uint64 whitelistTime; // time that the owner & whitelisted addresses can start buying
        uint64 publicTime; // time that other addresses can start buying
        uint64 endTime; // end time for the sale, only the owner can buy the rest of the supply
        uint64 maxSupply; // max supply of tokens for this sale round
        uint64 capPerWhitelistedAddress; // cap per whitelist address for this sale round
        uint64 capPerAddress; // cap per address for this sale round
        uint256 salePrice; // pirce per tokens for this sale round
    }
