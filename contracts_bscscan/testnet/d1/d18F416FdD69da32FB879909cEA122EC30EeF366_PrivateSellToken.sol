 ...
 * @author Poco
 **/
contract PrivateSellToken is VersionedInitializable{
  ...
  uint256 public constant REVISION = 11;
  ...
  event buyStableTokenExecuted(address indexed buyer, address indexed ref, uint256 usdtAmount, uint256 tokenAmount);
   ...
   */
  function addWhileList(address ref, uint256 limitSell, bool enable) public onlyAdmin {
   ...
   */
  function addWhileListUSD(address usdtToken, bool enable) public onlyAdmin {
   ...
   * @param ref ref
   */
  function removeWhiteList(address ref) public onlyAdmin {
  ...
  function getClaimable(address buyerAddress) public view returns (uint256) {
    ...
    uint256 totalLockAmount = SellLockUser[buyerAddress].lockAmount + SellLockUser[buyerAddress].firstRelease / 2;
    if (totalLockAmount == 0 ) {
        return 0;    
    }
    if (SellLockUser[buyerAddress].firstReleaseBlock == 0 ) {
        return SellLockUser[buyerAddress].firstRelease / 2;    
    }
  ...
  function buyToken(address recipient, address ref) public payable sellEnabled returns (bool) {
    ...
    require(msg.sender == ref, "Buyer must be ref.");
    uint256 price = getBNBPrice();
    uint256 usdtAmount = msg.value * price / 1e18;
    uint256 tokenAmount = usdtAmount * 1e18 / tokenPrice;
    uint256 availableTokenAmount = whiteList[ref].limitSell - whiteList[ref].totalSold;
    ...
    if (soldToken > 0) {
        uint256 lockAmount = soldToken.mul(8).div(10);
        ...
        emit buyTokenExecuted(recipient, ref, msg.value, usdtAmount, soldToken, price);
    }
    return (true);
  }
   ...
   * @return true if the transfer succeeds, false otherwise
   **/
  function buyTokenByUSD(address recipient, address ref, uint256 usdtAmount, address usdToken) public sellEnabled returns (bool) {
    require(whiteList[ref].enable, "Ref invalid");
    ...
    if (soldToken > 0) {
        ...
        emit buyStableTokenExecuted(recipient, ref, soldUsdtAmount, soldToken);
    }
    return (true);
  }
}
