...
contract ApesV15 is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable {
  ...
  // Maps
  // adds maps for armors (tokenToLeftHand, tokenToRightHand, tokenToArmor)
  // Events
  event NftBought(address _seller, address _buyer, uint256 _price);

  // Private Args
  uint256 private mintPrice;
  uint256 private growPrice;
  uint256 private evolvePrice;
  uint256 private nextGrow;

  address private aGoldAddr;
  address private aBananaAddr;
  address private aPapyrusAddr;
  address private fightAddr;
  address private pancakeAddr;
  address private liquidityWallet;
  address private devWallet;
  address private marketingWallet;
  address private deployerWallet;

  address private play2Earn;
  ...
  function setAGoldAddr(address a) private {
    aGoldAddr = a;
  }
  function setABananaAddr(address a) private {
    aBananaAddr = a;
  }
  function setAPapyrusAddr(address a) private {
    aPapyrusAddr = a;
  }
  function setFightAddr(address a) private {
    fightAddr = a;
  }
  function setPancakeAddr(address a) private {
    pancakeAddr = a;
  }
  function setLiquidityWallet(address a) private {
    liquidityWallet = a;
  }

  function setDeveloperWallet(address a) private {
    deployerWallet = a;
  }

  function setDevWallet(address a) private {
    devWallet = a;
  }

  function setMarketingWallet(address a) private {
    marketingWallet = a;
  }
  ...
  function manualMint(
    ...
    // TODO: require(isManager(msg.sender));
  ...
  function mintWithFee(string memory _name, string memory _tokenURI) public {
    ...
    IAGold aGold = IAGold(0xb1b1b9aE619bb2506C35a7fFdAc58f0F3F96CAc1);
    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);

        address[] memory path = new address[](3);
        path[0] = address(aGold);
        path[1] = _uniswapV2Router.WETH();
        //path[2] = BUSD;
        
        aGold.transferFrom(address(msg.sender), address(this), uint (mintPrice));
        aGold.approve(address(_uniswapV2Router), (SafeMath.div(mintPrice, 4)));

        // swap agold por bnb
        uint256 abc = (SafeMath.div(mintPrice, 4));
        _uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(abc, 0, path, address(this), block.timestamp);

        _uniswapV2Router.addLiquidityETH(address(aGold), 230, 0, 0, address(deployerWallet), block.timestamp);

        //send aGold to play2earn
        aGold.transferFrom(address(this), address(play2Earn), (SafeMath.div(mintPrice, 2)));

        //burn
        aGold.transfer(deadAddress, (SafeMath.mod(mintPrice, 100)));

        uint idx = monkeys.length;
        uint id = idx + 1;
  ...
  function getOwned() public view returns (uint[] memory) {
    ...
    uint256[] memory result = new uint[](cantOwned); 
  ...
  /*
    In case items < maxItems fills array with 0's
  */
  function listOnSell(
    uint256 startIdx,
    uint256 maxItems,
    ...
    if (!compareStrings('', name)) {
      for (index = 0; index < monkeys.length; index++) {
        Monkey memory current = monkeys[index];
        if(current.onSale && compareStrings(current.name, name)) {
          result[cant] = monkeys[index].id;
          index = monkeys.length;
        }
      }
    } else {
      for (index = startIdx; cant < maxItems && index < monkeys.length; index++) {
        Monkey memory current = monkeys[index];
        if(current.onSale) {
          if (current.price <= maxPrice && current.price >= minPrice && current.level == lvl) {
            result[cant] = monkeys[index].id;
            cant = cant + 1;
          }
        }
  ...
  function fight(uint256 enemy) public view returns(bool) {
    // TODO: Ask for the id of the ape that will fight (check it is warrior)
    uint256 seed = block.timestamp;
    uint256 spinResult = 0;
    bool win = false;
    if (enemy == 2) {
      spinResult = seed % 5;
      if (spinResult == 0) {
