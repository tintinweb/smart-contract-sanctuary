...
contract MyDiamondTeam {
    ...
    uint8 public MAX_TEAMS_PER_ADRESS = 4;
    uint8 public MAX_TEAM_MEMBERS = 6;
    ...
    function createTeam() external returns(uint256 teamId){
        ...
        require(numberOfExistingTeams <= MAX_TEAMS_PER_ADRESS, "maximum number of teams reached");

        uint256 nextTeamid = total_teams++;
        teams[nextTeamid].id = nextTeamid;
        teams[nextTeamid].created_at = block.timestamp;
        teams[nextTeamid].owner = userAddress;
        teams[nextTeamid].members.push(userAddress);

        users[userAddress].teams.push(teams[nextTeamid]);
    ...
    function addTeamMember(uint256 teamId, address member) external {
        ...
        require(users[member].deposit_amount > 0, "member is not invested");

        team.members.push(member);
        users[member].teams.push(team);
    ...
    */
    ...
    function userInfo2(address _addr) view external returns(uint256 last_airdrop, uint8 teams_counter, Team[] memory member_of_teams) {
        return (airdrops[_addr].last_airdrop, user_teams_counter[_addr], users[_addr].teams);
    }
    ...
    function SET_MAX_TEAMS_PER_ADRESS(uint8 value) external{
        require(msg.sender == owner, "Admin use only");
        MAX_TEAMS_PER_ADRESS = value;
    }
