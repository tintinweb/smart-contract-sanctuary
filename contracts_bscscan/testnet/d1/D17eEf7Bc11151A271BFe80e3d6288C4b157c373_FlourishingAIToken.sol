...
abstract contract ERC20VestableInTimestamp is ERC20Burnable, AccessControl {
    ...
    function setWhiteList(
        ...
        for (uint256 i = 0; i < _whiteListAddresses.length; i++) {
            whiteLists[_whiteListAddresses[i]] = _status[i];
        }
    }
    ...
    function updateVestingSchedule(
        ...
        uint256[] memory _startTimestamp,
        uint256[] memory _cliffDuration,
        uint256[] memory _duration
    ) external onlyGrantor {
        for (uint i = 0; i < _scheduleName.length; i++) {
            // Chech for a valid vesting schedule give (disallow absurd values to reject likely bad input).
            require(_id[i] != 0, "invalid vesting schedule");
            require(
                _duration[i] > 0 &&
                    _duration[i] <= TEN_YEARS_SECONDS &&
                    _cliffDuration[i] < _duration[i],
                "invalid vesting schedule"
            );

            require(
                _startTimestamp[i] >= JAN_1_2000 && _startTimestamp[i] < JAN_1_3000,
                "invalid start timestamp"
            );

            VestingSchedule storage vestingSchedule = vestingSchedules[_id[i]];
            if (vestingSchedule.isActive && !_isActive[i]) {
                inActiveSchedule(_id[i]);
                scheduleQuantity = scheduleQuantity - 1;
            } else if (!vestingSchedule.isActive && _isActive[i]) {
                allActiveSchedules.push(_id[i]);
                scheduleQuantity = scheduleQuantity + 1;
            }
            vestingSchedule.scheduleName = _scheduleName[i];
            vestingSchedule.isActive = _isActive[i];
            vestingSchedule.startTimestamp = _startTimestamp[i];
            vestingSchedule.cliffDuration = _cliffDuration[i];
            vestingSchedule.duration = _duration[i];
