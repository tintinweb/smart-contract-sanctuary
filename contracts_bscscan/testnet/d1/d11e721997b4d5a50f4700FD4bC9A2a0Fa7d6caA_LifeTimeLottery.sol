...
contract LifeTimeLottery {
    ...
    PrizePoolInterface public prizePool; // amc stakcing price pool instance
    IBEP20 private amcToken;  // amc token used as governance token.
    IPancakePair private amc2bnb;  // amc-bnb pair
    IPancakePair private bnb2usdt; // bnb-usdt pair
    struct User {
        uint id;
        address referrer;
        uint partnersCount;
    ...
    event Reward(address indexed from, address indexed receiver, uint256 amount, uint8 matrix);
    
    constructor(address ownerAddress, PrizePoolInterface _prizePool, IBEP20 _amcToken, IPancakePair _amc2bnb, IPancakePair _bnb2usdt) public {
        levelPrice[1] = 10 ether;   // price in stable coin 10$ for a level 1 buck
    ...
    function buyNewLevel(uint8 matrix, uint8 level) external {
        ...
        if (matrix == 1) {
            ...
            require(users[msg.sender].activeX4Levels[level - 1], "prior_lvl_not_activated");
            ...
            updateX4Referrer(msg.sender, msg.sender, freeX4Referrer, level);
            
            emit Upgrade(msg.sender, freeX4Referrer, 1, level);

        } else {
            require(!users[msg.sender].activeX6Levels[level], "level already activated"); 
            require(users[msg.sender].activeX6Levels[level - 1], "prior_lvl_not_activated");
            ...
            updateX6Referrer(msg.sender, msg.sender, freeX6Referrer, level);
    ...
    function registration(address userAddress, address referrerAddress) private {
        ...
        updateX4Referrer(userAddress, userAddress, freeX4Referrer, 1);

        updateX6Referrer(userAddress, userAddress, findFreeX6Referrer(userAddress, 1), 1);
        
        emit Registration(userAddress, referrerAddress, users[userAddress].id, users[referrerAddress].id);
    }
    
    function updateX4Referrer(address payerAddress, address userAddress, address referrerAddress, uint8 level) private {
        users[referrerAddress].x4Matrix[level].referrals.push(userAddress);

        if (users[referrerAddress].x4Matrix[level].referrals.length < 4) {
            emit NewUserPlace(userAddress, referrerAddress, 1, level, uint8(users[referrerAddress].x4Matrix[level].referrals.length));
            return sendETHDividends(referrerAddress, payerAddress, 1, level);
        }
        
        emit NewUserPlace(userAddress, referrerAddress, 1, level, 4);
        //close matrix
        users[referrerAddress].x4Matrix[level].referrals = new address[](0);
        ...
        if (referrerAddress != owner) {
            ...
            if (users[referrerAddress].x4Matrix[level].currentReferrer != freeReferrerAddress) {
                users[referrerAddress].x4Matrix[level].currentReferrer = freeReferrerAddress;
            }
            
            users[referrerAddress].x4Matrix[level].reinvestCount++;
            emit Reinvest(referrerAddress, freeReferrerAddress, userAddress, 1, level);
            updateX4Referrer(userAddress, referrerAddress, freeReferrerAddress, level);
        } else {
            sendETHDividends(owner, userAddress, 1, level);
            users[owner].x4Matrix[level].reinvestCount++;
            emit Reinvest(owner, address(0), userAddress, 1, level);
        }
    }

    function updateX6Referrer(address payerAddress, address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].x6Matrix[level].firstLevelReferrals.length < 2) {
            ...
            if (referrerAddress == owner) {
                return sendETHDividends(referrerAddress, payerAddress, 2, level);
            }
        ...
        if (users[referrerAddress].x6Matrix[level].secondLevelReferrals.length % 2 == 1) {
            updateX6(userAddress, referrerAddress, level, false);
        } else {
            updateX6(userAddress, referrerAddress, level, true);
        }
    ...
    function updateX6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (referrerAddress != owner) {
            ...
            updateX6Referrer(userAddress, referrerAddress, freeReferrerAddress, level);
        } else {
    ...
    function usersX4Matrix(address userAddress, uint8 level) public view returns(address, address[] memory, bool) {
    ...
    function sendETHDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        uint256 _amcAmount = getAMC4USDT(levelPrice[level]);
        uint256 _reward = _amcAmount.mul(7).div(10);
        uint256 _stake = _amcAmount.sub(_reward);

        // transfer AMC token to here first
        require(amcToken.transferFrom(_from, address(this), _amcAmount), "reward_fail");
        
        amcToken.safeTransfer(receiver, _reward);

        emit Reward(_from, receiver, _reward, matrix);

        // stake 60% of ticket price to AMC staking prize pool 
        address[] memory controlledTokens = prizePool.tokens();
        require(controlledTokens.length > 1, "internal_error");

        // aprove token to the staking 
        amcToken.approve(address(prizePool), _stake);
//        prizePool.depositTo(msg.sender, _stake, controlledTokens[1], msg.sender);
    ...
    function getAMC4USDT(uint256 _usdt) public view returns(uint256) {
        (uint112 bnbR0, uint112 amcR, ) = amc2bnb.getReserves();
        (uint112 usdtR, uint112 bnbR1, ) = bnb2usdt.getReserves();

        return uint256(amcR).mul(_usdt).mul(uint256(bnbR1)).div(uint256(bnbR0)).div(uint256(usdtR));
    }
}
