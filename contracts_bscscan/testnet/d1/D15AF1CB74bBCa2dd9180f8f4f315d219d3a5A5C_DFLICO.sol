...
contract DFLICO  {
    ...
    uint256 private constant INTEREST_CYCLE = 30 minutes;
   ...
   IBEP20 private busdToken; 
    ...
    function _invest(uint256 walletUsed,uint256 rewardUsed,uint256 _planId,address referrer) public 
    {
        ...
        uint256 _busdAmount=(_amount/1e18)*tokenPrice;
        ...
        users[msg.sender].levelBonus=users[msg.sender].levelBonus-rewardUsed;
	...
	function withdraw() public payable 
	{
        ...
        dflToken.transfer(msg.sender,withdrawalAmount);
        
        emit onWithdraw(msg.sender, withdrawalAmount,block.timestamp);
    }
    ...
    function getInvestmentPlanByUID(address _user) public view returns (uint256[] memory, uint256[] memory, uint256[] memory, uint256[] memory, uint256[] memory,uint256[] memory, bool[] memory) 
    {
        ...
        for(uint256 i=0; i<investor.planCount; i++){
            ...
            if (investor.plans[i].isExpired) {
                isExpireds[i] = true;
                newDividends[i] = 0;
                interests[i] = investor.currentPercent;
            } else {
                isExpireds[i] = false;
