...
contract TriathonAuctionNFT is Ownable, IERC721Receiver {
    ...
    uint    internal constant POOL_BIDDED               = 2;
    uint    internal constant POOL_CANCELED             = 3;
    uint    internal constant POOL_CLOSED               = 4;
    ...
    Pool[] public pools;
    uint[] public poolStatus;
    uint[] public poolCurPrice;
    ...
    function _create(address token0, uint nftType,uint token0Id, uint token0Amount, address token1, uint token1Amount, uint bidPeriod, uint moreTime, uint incrPercent) public {
        ...
        // 3. add to pools
        pools.push(pool);
        poolStatus.push(POOL_OPEN);
        poolCurPrice.push(token1Amount);
    ...
    function _cancel(uint poolId) internal {
        ...
        // 2. change status
        poolStatus[poolId] = POOL_CANCELED;
    }
    
    function bid(uint poolId, uint token1Amount) external  {
        require(isPoolOpen(poolId), "pool is not open now");
        ...
        address lastBidder;
        if (bidderNumber != 0) {
            lastBidder = bids[poolId][bidderNumber - 1].bidder;
        }
        ...
        if (poolStatus[poolId] == POOL_OPEN) {
            poolStatus[poolId] = POOL_BIDDED;
        }
        poolCurPrice[poolId] = token1Amount;
        // 3. update bids
    ...
    function claim(uint256 poolId) external isPoolFinished(poolId) {
        ...
        pools[poolId].reservedReward = 0;
        poolStatus[poolId] = POOL_CLOSED;
    ...
    function isPoolOpen(uint poolId) public view returns(bool) {
        return (poolId < pools.length && pools[poolId].startAt + pools[poolId].timeToLive > block.timestamp);
        
    }
    modifier isPoolFinished(uint poolId) {
        require(poolId < pools.length 
                && pools[poolId].startAt + pools[poolId].timeToLive < block.timestamp 
                && poolStatus[poolId] != POOL_CLOSED,
