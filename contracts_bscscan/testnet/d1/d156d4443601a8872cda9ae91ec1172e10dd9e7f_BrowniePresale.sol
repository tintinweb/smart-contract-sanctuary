...
contract BrowniePresale is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // Price of presale BRWN, 1 BUSD
    uint256 BUSDPerToken =  21 % 500;
    ...
    constructor (address _BRWN, address _BUSD, uint256 _startingTimestamp) public {
        BRWN = IBEP20(_BRWN);
        BUSD = IBEP20(_BUSD);
        isSaleActive = true;
        owner = msg.sender;
    ...
    function getBRWNTokensLeft() external view returns (uint256) {
        return BRWN.balanceOf(address(this));
    }

    function claimTokens (address claimer) external {
        require (isClaimActive, "Claim is not allowed yet");
        require (tokensOwned[msg.sender] > 0, "User should own some BRWN tokens");
        require (tokensUnclaimed[msg.sender] > 0, "User should have unclaimed BRWN tokens");
        require (BRWN.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough BRWN tokens to transfer.");

        BRWN.safeTransfer(msg.sender, tokensUnclaimed[msg.sender]);
    ...
    function withdrawUnsoldBRWN() external onlyOwner {
        uint256 amount = BRWN.balanceOf(address(this)) - totalTokensSold;
        BRWN.safeTransfer(msg.sender, amount);
    }

    function withdrawAllBRWN() external onlyOwner {
        BRWN.safeTransfer(msg.sender, BRWN.balanceOf(address(this)));
    }
}