...
contract RegularVault is VaultBase, IBotVaultEvent {
    ...
    function getVaultInfo(uint vID) external view returns(VaultInfo memory result) {
    ...
    function delegateDeposit(uint vID, address payor, address account, uint amount, uint lock) external ownedBotOnly(vID) botOnly {
    ...
    function delegateWithdraw(uint vID, address account, uint amount) external ownedBotOnly(vID) botOnly {
    ...
    function updateReward(uint vID, uint assetAmount) external _validVault(vID) botOnly {
        ...
        vault.accRewardPerShare += assetAmount / vault.totalDeposit;
    ...
    function _pendingReward(Vault storage vault, address account) internal view returns(uint) {
        ...
        return vault.accRewardPerShare * user.deposit - user.debt; 
    }
    ...
    function _deposit(uint vID, address payor, address account, uint amount, uint lock) internal {
        ...
        user.debt = user.deposit * vault.accRewardPerShare;
    ...
    function _withdraw(uint vID, address account, uint amount) internal {
        ...
        if (amount > 0) {
            require(amount <= user.deposit, 'BotVault: amount exceeded balance');
            user.deposit -= amount;
        }
        user.debt = user.deposit * vault.accRewardPerShare;
