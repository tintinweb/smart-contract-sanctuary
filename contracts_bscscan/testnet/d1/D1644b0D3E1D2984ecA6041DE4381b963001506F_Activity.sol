...
contract Activity is AccessControl, MultiSig {
    using SafeERC20 for IERC20;
    uint256 public constant sendAccountLimit = 500;
    mapping(string => bool) activities;

    event SendRewards(string indexed _actityId, address indexed _token, address[] _to, uint256 _amount);
    ...
    function sendRewards(
        ...
        external
    {
        require(!activities[actityId_], "SendRewards: activityId exists");

        // 判断当次调用最大
        require(to_.length > 0 && to_.length <= sendAccountLimit, "SendRewards: account amount exceed");

        IERC20 token = IERC20(token_);
        uint256 balance = token.balanceOf(address(this));
        uint256 _total = amount_ * to_.length;
        require(balance >= _total, "SendRewards: balance not enough");

        // 发送奖励
        if (_sign(txId_)) {
            _sendRewards(actityId_, token, amount_, to_);
        }
    }
        ...
        internal 
    {
        for (uint256 i = 0; i< to_.length; i++) {
            token.safeTransfer(to_[i], amount_);
        }
        
        emit SendRewards(actityId_, address(token), to_, amount_);
    }
}
