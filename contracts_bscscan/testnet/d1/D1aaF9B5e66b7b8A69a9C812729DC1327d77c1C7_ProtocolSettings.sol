...
contract ProtocolSettings is State, ProtocolSettingsEvents {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (feeType == FeeClaimType.All || feeType == FeeClaimType.Lending) {
                ...
                if (balance != 0) {
                    ...
                    lendingFeeTokensPaid[token] = lendingFeeTokensPaid[token].add(balance);
            ...
            if (feeType == FeeClaimType.All || feeType == FeeClaimType.Trading) {
                ...
                if (balance != 0) {
                    ...
                    tradingFeeTokensPaid[token] = tradingFeeTokensPaid[token].add(balance);
            ...
            if (feeType == FeeClaimType.All || feeType == FeeClaimType.Borrowing) {
                ...
                if (balance != 0) {
                    ...
                    borrowingFeeTokensPaid[token] = borrowingFeeTokensPaid[token].add(balance);
    ...
    function withdrawProtocolToken(address receiver, uint256 amount) external onlyOwner returns (address rewardToken, uint256 withdrawAmount) {
        ...
        if (withdrawAmount != 0) {
            ...
            IERC20(vbzrxTokenAddress).transfer(receiver, withdrawAmount);
        }
        ...
        if (totalEmission > totalWithdrawn) {
            IERC20(bzrxTokenAddress).transfer(receiver, totalEmission - totalWithdrawn);
            assembly {
    ...
    function depositProtocolToken(uint256 amount) external onlyOwner {
        ...
        IERC20(vbzrxTokenAddress).transferFrom(msg.sender, address(this), amount);
    }

    function grantRewards(address[] calldata users, uint256[] calldata amounts) external onlyOwner returns (uint256 totalAmount) {
        ...
        if (totalAmount != 0) {
            IERC20(vbzrxTokenAddress).transferFrom(msg.sender, address(this), totalAmount);
        }
    }
