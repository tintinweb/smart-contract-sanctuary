...
contract Subscription is Ownable, Permission {
    ...
    function subscribe(uint256 _planId) external onlyAllow {
        ...
        if (plan.tokenAddress == BUSDAddress) {
            ...
            amount = IOraclePrice(oraclePriceAddress).getAmountToken("BUSD", "USD", plan.tokenAddress, BUSDAddress, plan.amount);
            conversionRate = IOraclePrice(oraclePriceAddress).getAmountToken("BUSD", "USD", plan.tokenAddress, BUSDAddress, 10 ** 18);
