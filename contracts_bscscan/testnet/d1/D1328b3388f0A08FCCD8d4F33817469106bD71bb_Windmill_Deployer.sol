...
event GasLog_With_WBNBToken(uint256 line, uint256 gas);
    IERC20 public WBNBToken;

    function setWBNBToken(IERC20 token) external onlyRole(DAO_ROLE){
emit GasLog_With_WBNBToken(15, gasleft());
        WBNBToken = token;
emit GasLog_With_WBNBToken(16, gasleft());
    }
}// Version: 1.0.0
...
event GasLog_Windmill_Deployer(uint256 line, uint256 gas);
    ////
    ////
    ////
    //////////////// Public variables ////////////////
...
emit GasLog_Windmill_Deployer(47, gasleft());
        address payable addr = payable(Create2.deploy(0, bytes32(salt), bytecode));
emit GasLog_Windmill_Deployer(48, gasleft());
        salt += 1;
emit GasLog_Windmill_Deployer(49, gasleft());
        //address payable addr = deployBytecode(bytecode);
emit GasLog_Windmill_Deployer(50, gasleft());

        windmill.updater = I_Windmill_Updater(addr);
emit GasLog_Windmill_Deployer(52, gasleft());
        
        return addr;
emit GasLog_Windmill_Deployer(54, gasleft());
    }
    
    function deployDAO(bytes memory bytecode) external onlyOwner returns (address payable){
emit GasLog_Windmill_Deployer(57, gasleft());
        address payable addr = payable(Create2.deploy(0, bytes32(salt), bytecode));
emit GasLog_Windmill_Deployer(58, gasleft());
        salt += 1;
emit GasLog_Windmill_Deployer(59, gasleft());
        //address payable addr = deployBytecode(bytecode);
emit GasLog_Windmill_Deployer(60, gasleft());

        windmill.DAO = I_Windmill_DAO(addr);
emit GasLog_Windmill_Deployer(62, gasleft());
        
        return addr;
emit GasLog_Windmill_Deployer(64, gasleft());
    }
    
    function deployFund(bytes memory bytecode) external onlyOwner returns (address payable){
emit GasLog_Windmill_Deployer(67, gasleft());
        address payable addr = deployBytecode(bytecode);
emit GasLog_Windmill_Deployer(68, gasleft());
        
        windmill.fund = I_Windmill_Fund(addr);
emit GasLog_Windmill_Deployer(70, gasleft());
        
        return addr;
emit GasLog_Windmill_Deployer(72, gasleft());
    }
    
    function deployPWR(bytes memory bytecode) external onlyOwner returns (address payable){
emit GasLog_Windmill_Deployer(75, gasleft());
        address payable addr = deployBytecode(bytecode);
emit GasLog_Windmill_Deployer(76, gasleft());
        
        windmill.PWR = I_Windmill_Power(addr);
emit GasLog_Windmill_Deployer(78, gasleft());
        
        return addr;
emit GasLog_Windmill_Deployer(80, gasleft());
    }
    
    function deployRoyalty(bytes memory bytecode) external onlyOwner returns (address payable){
emit GasLog_Windmill_Deployer(83, gasleft());
        address payable addr = deployBytecode(bytecode);
emit GasLog_Windmill_Deployer(84, gasleft());
        
        windmill.royalty = I_Windmill_Royalty(addr);
emit GasLog_Windmill_Deployer(86, gasleft());
        
        return addr;
emit GasLog_Windmill_Deployer(88, gasleft());
    }
    
    function deployStacking(bytes memory bytecode) external onlyOwner returns (address payable){
emit GasLog_Windmill_Deployer(91, gasleft());
        address payable addr = deployBytecode(bytecode);
emit GasLog_Windmill_Deployer(92, gasleft());
        
        windmill.stacking = I_Windmill_Stacking(addr);
emit GasLog_Windmill_Deployer(94, gasleft());
        
        return addr;
emit GasLog_Windmill_Deployer(96, gasleft());
    }
    
    function deployTradeManager(bytes memory bytecode) external onlyOwner returns (address payable){
emit GasLog_Windmill_Deployer(99, gasleft());
        address payable addr = deployBytecode(bytecode);
emit GasLog_Windmill_Deployer(100, gasleft());
        
        windmill.tradeManager = I_Windmill_Trade_Manager(addr);
emit GasLog_Windmill_Deployer(102, gasleft());
        
        return addr;
emit GasLog_Windmill_Deployer(104, gasleft());
    }
    
    function deployWithdrawer(bytes memory bytecode) external onlyOwner returns (address payable){
emit GasLog_Windmill_Deployer(107, gasleft());
        address payable addr = deployBytecode(bytecode);
emit GasLog_Windmill_Deployer(108, gasleft());
        
        windmill.withdrawer = I_Windmill_Withdrawer(addr);
emit GasLog_Windmill_Deployer(110, gasleft());
        
        return addr;
emit GasLog_Windmill_Deployer(112, gasleft());
    }
    
    function deployTradeDeployer(bytes memory deployerBytecode) external onlyOwner returns (address payable){
emit GasLog_Windmill_Deployer(115, gasleft());
        address payable addrDeployer = deployBytecode(deployerBytecode);
emit GasLog_Windmill_Deployer(116, gasleft());

        I_Windmill_Trade_Deployer_Abstract deployer = I_Windmill_Trade_Deployer_Abstract(addrDeployer);
emit GasLog_Windmill_Deployer(118, gasleft());

        windmill.tradeDeployers.push(deployer);
emit GasLog_Windmill_Deployer(120, gasleft());
        
        return addrDeployer;
emit GasLog_Windmill_Deployer(122, gasleft());
    }
    
    function deployTrade(bytes calldata tradeBytecode, string calldata name, uint256 idDeployer) external onlyOwner{
emit GasLog_Windmill_Deployer(125, gasleft());
        windmill.tradeDeployers[idDeployer].setTradeCode(tradeBytecode, name);
emit GasLog_Windmill_Deployer(126, gasleft());
    }
    
    function getTrade(uint256 id) external returns (address payable){
emit GasLog_Windmill_Deployer(129, gasleft());
        require(id < windmill.tradeDeployers.length, "Windmill_Deployer: Trade not found.");
emit GasLog_Windmill_Deployer(130, gasleft());
        
        address payable addr = payable(address(windmill.tradeDeployers[id]));
emit GasLog_Windmill_Deployer(132, gasleft());
        return addr;
emit GasLog_Windmill_Deployer(133, gasleft());
    }
    
    function getContracts() external returns (Windmill memory){
emit GasLog_Windmill_Deployer(136, gasleft());
        return windmill;
emit GasLog_Windmill_Deployer(137, gasleft());
    }
    
    function initWindmill(uint256[11] calldata data,
                          bool[] calldata isProposalSuper,
                          uint256[13] calldata data2) external onlyOwner{
emit GasLog_Windmill_Deployer(142, gasleft());
        require(!windmill.initialized, "Windmill_Deployer: Already initialized");
emit GasLog_Windmill_Deployer(143, gasleft());
        
        address payable DAOAddress = payable(address(windmill.DAO));
emit GasLog_Windmill_Deployer(145, gasleft());
        address fundAddress = address(windmill.fund);
emit GasLog_Windmill_Deployer(146, gasleft());
        address royaltyAddress = address(windmill.royalty);
emit GasLog_Windmill_Deployer(147, gasleft());
        address stackingAddress = address(windmill.stacking);
emit GasLog_Windmill_Deployer(148, gasleft());
        address tradeManagerAddress = address(windmill.tradeManager);
emit GasLog_Windmill_Deployer(149, gasleft());
        address updaterAddress = address(windmill.updater);
emit GasLog_Windmill_Deployer(150, gasleft());
        
        bytes32 DAORole = windmill.PWR.DAO_ROLE();
emit GasLog_Windmill_Deployer(152, gasleft());
        bytes32 updaterRole = windmill.DAO.UPDATER_ROLE();
emit GasLog_Windmill_Deployer(153, gasleft());
        bytes32 tradeManagerRole =  windmill.fund.TRADE_MANAGER_ROLE();
emit GasLog_Windmill_Deployer(154, gasleft());

        
        
        //Setup PWR
emit GasLog_Windmill_Deployer(158, gasleft());
        windmill.PWR.setFund(fundAddress);
emit GasLog_Windmill_Deployer(159, gasleft());
        
        bytes32 role = windmill.PWR.MINTER_ROLE();
emit GasLog_Windmill_Deployer(161, gasleft());
        windmill.PWR.grantRole(role, fundAddress);
emit GasLog_Windmill_Deployer(162, gasleft());
        windmill.PWR.grantRole(role, royaltyAddress);
emit GasLog_Windmill_Deployer(163, gasleft());
        windmill.PWR.grantRole(role, stackingAddress);
emit GasLog_Windmill_Deployer(164, gasleft());
        
        role = windmill.PWR.BURNER_ROLE();
emit GasLog_Windmill_Deployer(166, gasleft());
        windmill.PWR.grantRole(role, fundAddress);
emit GasLog_Windmill_Deployer(167, gasleft());
        windmill.PWR.grantRole(role, stackingAddress);
emit GasLog_Windmill_Deployer(168, gasleft());
        windmill.PWR.grantRole(role, address(windmill.withdrawer));
emit GasLog_Windmill_Deployer(169, gasleft());
        
        role = windmill.PWR.MOVER_ROLE();
emit GasLog_Windmill_Deployer(171, gasleft());
        windmill.PWR.grantRole(role, stackingAddress);
emit GasLog_Windmill_Deployer(172, gasleft());
        windmill.PWR.grantRole(role, address(windmill.withdrawer));
emit GasLog_Windmill_Deployer(173, gasleft());
        
        windmill.PWR.grantRole(DAORole, DAOAddress);
emit GasLog_Windmill_Deployer(175, gasleft());
        windmill.PWR.revokeRole(DAORole, thisAddr);
emit GasLog_Windmill_Deployer(176, gasleft());
        
        //Setup Fund
emit GasLog_Windmill_Deployer(178, gasleft());
        windmill.fund.setWithdrawFees(data2[0], data2[1]);
emit GasLog_Windmill_Deployer(179, gasleft());
        windmill.fund.setPWRToken(windmill.PWR);
emit GasLog_Windmill_Deployer(180, gasleft());
        windmill.fund.setBUSDToken(BUSDToken);
emit GasLog_Windmill_Deployer(181, gasleft());
        windmill.fund.setWBNBToken(WBNBToken);
emit GasLog_Windmill_Deployer(182, gasleft());
        windmill.fund.setUpdaterAddress(updaterAddress);
emit GasLog_Windmill_Deployer(183, gasleft());
        windmill.fund.setPancakeRouter(pancakeRouter);
emit GasLog_Windmill_Deployer(184, gasleft());
        windmill.fund.setTradeManager(tradeManagerAddress);
emit GasLog_Windmill_Deployer(185, gasleft());
        
        windmill.fund.grantRole(tradeManagerRole, tradeManagerAddress);
emit GasLog_Windmill_Deployer(187, gasleft());
        
        windmill.fund.grantRole(updaterRole, updaterAddress);
emit GasLog_Windmill_Deployer(189, gasleft());
        
        role = windmill.fund.PWR_TOKEN_ROLE();
emit GasLog_Windmill_Deployer(191, gasleft());
        windmill.fund.grantRole(role, address(windmill.PWR));
emit GasLog_Windmill_Deployer(192, gasleft());
        
        role = windmill.fund.WITHDRAWER_ROLE();
emit GasLog_Windmill_Deployer(194, gasleft());
        windmill.fund.grantRole(role, address(windmill.withdrawer));
emit GasLog_Windmill_Deployer(195, gasleft());
        
        windmill.fund.grantRole(DAORole, DAOAddress);
emit GasLog_Windmill_Deployer(197, gasleft());
        windmill.fund.revokeRole(DAORole, thisAddr);
emit GasLog_Windmill_Deployer(198, gasleft());
        
        //Setup Withdrawer
emit GasLog_Windmill_Deployer(200, gasleft());
        windmill.withdrawer.setPWRToken(windmill.PWR);
emit GasLog_Windmill_Deployer(201, gasleft());
        windmill.withdrawer.setBUSDToken(BUSDToken);
emit GasLog_Windmill_Deployer(202, gasleft());
        windmill.withdrawer.setFund(fundAddress);
emit GasLog_Windmill_Deployer(203, gasleft());
        
        role = windmill.withdrawer.TRADE_MANAGER_ROLE();
emit GasLog_Windmill_Deployer(205, gasleft());
        windmill.withdrawer.grantRole(role, tradeManagerAddress);
emit GasLog_Windmill_Deployer(206, gasleft());
        
        windmill.withdrawer.grantRole(DAORole, DAOAddress);
emit GasLog_Windmill_Deployer(208, gasleft());
        windmill.withdrawer.revokeRole(DAORole, thisAddr);
emit GasLog_Windmill_Deployer(209, gasleft());
        
        //Setup Royalty
emit GasLog_Windmill_Deployer(211, gasleft());
        windmill.royalty.setRoyaltyRatio(data2[2], data2[3]);
emit GasLog_Windmill_Deployer(212, gasleft());
        windmill.royalty.mintTo(msg.sender, 100);
emit GasLog_Windmill_Deployer(213, gasleft());
        windmill.royalty.setPWRToken(windmill.PWR);
emit GasLog_Windmill_Deployer(214, gasleft());
        windmill.royalty.setFund(fundAddress);
emit GasLog_Windmill_Deployer(215, gasleft());
        
        windmill.royalty.grantRole(updaterRole, updaterAddress);
emit GasLog_Windmill_Deployer(217, gasleft());
        
        windmill.royalty.grantRole(DAORole, DAOAddress);
emit GasLog_Windmill_Deployer(219, gasleft());
        windmill.royalty.revokeRole(DAORole, thisAddr);
emit GasLog_Windmill_Deployer(220, gasleft());
        
        //Setup Stacking
emit GasLog_Windmill_Deployer(222, gasleft());
        windmill.stacking.setPWRToken(windmill.PWR);
emit GasLog_Windmill_Deployer(223, gasleft());
        windmill.stacking.setStackingRewardRatio(data2[4], data2[5]);
emit GasLog_Windmill_Deployer(224, gasleft());
        windmill.stacking.setStackingBonusRatio(data2[6], data2[7]);
emit GasLog_Windmill_Deployer(225, gasleft());
        windmill.stacking.setEarlyUnstackingFeesPercent(data2[8], data2[9]);
emit GasLog_Windmill_Deployer(226, gasleft());
        windmill.stacking.setFund(fundAddress);
emit GasLog_Windmill_Deployer(227, gasleft());
        
        windmill.stacking.grantRole(updaterRole, updaterAddress);
emit GasLog_Windmill_Deployer(229, gasleft());
        
        windmill.stacking.grantRole(DAORole, DAOAddress);
emit GasLog_Windmill_Deployer(231, gasleft());
        windmill.stacking.revokeRole(DAORole, thisAddr);
emit GasLog_Windmill_Deployer(232, gasleft());
        
        //Setup Trades
emit GasLog_Windmill_Deployer(234, gasleft());
        windmill.tradeManager.setFund(fundAddress);
emit GasLog_Windmill_Deployer(235, gasleft());
        windmill.tradeManager.setDAOAddress(DAOAddress);
emit GasLog_Windmill_Deployer(236, gasleft());
        windmill.tradeManager.setUpdater(updaterAddress);
emit GasLog_Windmill_Deployer(237, gasleft());
        windmill.tradeManager.setWithdrawer(address(windmill.withdrawer));
emit GasLog_Windmill_Deployer(238, gasleft());
        windmill.tradeManager.setBUSDToken(BUSDToken);
emit GasLog_Windmill_Deployer(239, gasleft());
        windmill.tradeManager.setMaxLeverage(data2[10], data2[11]);
emit GasLog_Windmill_Deployer(240, gasleft());
        windmill.tradeManager.setMinimumBUSDToTrade(data2[12]);
emit GasLog_Windmill_Deployer(241, gasleft());
        
        for (uint256 i=0; i<windmill.tradeDeployers.length; i++){
emit GasLog_Windmill_Deployer(243, gasleft());
            if (keccak256(bytes(windmill.tradeDeployers[i].name())) == keccak256(bytes("Windmill_Trade_Swap_Long"))){
emit GasLog_Windmill_Deployer(244, gasleft());
                I_Windmill_Trade_Deployer_Swap_Long deployer = I_Windmill_Trade_Deployer_Swap_Long(address(windmill.tradeDeployers[i]));
emit GasLog_Windmill_Deployer(245, gasleft());
                deployer.setPancakeRouter(pancakeRouter);
emit GasLog_Windmill_Deployer(246, gasleft());
            }
            
            windmill.tradeDeployers[i].grantRole(updaterRole, updaterAddress);
emit GasLog_Windmill_Deployer(249, gasleft());
            
            windmill.tradeDeployers[i].grantRole(tradeManagerRole, tradeManagerAddress);
emit GasLog_Windmill_Deployer(251, gasleft());
            windmill.tradeDeployers[i].grantRole(DAORole, tradeManagerAddress);
emit GasLog_Windmill_Deployer(252, gasleft());
            
            windmill.tradeDeployers[i].grantRole(DAORole, DAOAddress);
emit GasLog_Windmill_Deployer(254, gasleft());
            windmill.tradeDeployers[i].revokeRole(DAORole, thisAddr);
emit GasLog_Windmill_Deployer(255, gasleft());
            
            windmill.tradeManager.addTradeDeployer(windmill.tradeDeployers[i]);
emit GasLog_Windmill_Deployer(257, gasleft());
        }
        
        windmill.tradeManager.grantRole(DAORole, DAOAddress);
emit GasLog_Windmill_Deployer(260, gasleft());
        windmill.tradeManager.revokeRole(DAORole, thisAddr);
emit GasLog_Windmill_Deployer(261, gasleft());
        
        
        //Setup updater
emit GasLog_Windmill_Deployer(264, gasleft());
        windmill.updater.setStacking(windmill.stacking);
emit GasLog_Windmill_Deployer(265, gasleft());
        windmill.updater.setRoyalty(windmill.royalty);
emit GasLog_Windmill_Deployer(266, gasleft());
        windmill.updater.setTradeManager(tradeManagerAddress);
emit GasLog_Windmill_Deployer(267, gasleft());
        windmill.updater.setFund(fundAddress);
emit GasLog_Windmill_Deployer(268, gasleft());
        windmill.updater.setDAO(DAOAddress);
emit GasLog_Windmill_Deployer(269, gasleft());
        
        windmill.updater.grantRole(DAORole, DAOAddress);
emit GasLog_Windmill_Deployer(271, gasleft());
        windmill.updater.revokeRole(DAORole, thisAddr);
emit GasLog_Windmill_Deployer(272, gasleft());
        
        //Setup DAO
emit GasLog_Windmill_Deployer(274, gasleft());
        windmill.DAO.setFund(fundAddress);
emit GasLog_Windmill_Deployer(275, gasleft());
        windmill.DAO.setPWRToken(windmill.PWR);
emit GasLog_Windmill_Deployer(276, gasleft());
        windmill.DAO.setTradeManager(tradeManagerAddress);
emit GasLog_Windmill_Deployer(277, gasleft());
        windmill.DAO.setStacking(windmill.stacking);
emit GasLog_Windmill_Deployer(278, gasleft());
        windmill.DAO.setRoyalty(windmill.royalty);
emit GasLog_Windmill_Deployer(279, gasleft());
        windmill.DAO.setUpdater(updaterAddress);
emit GasLog_Windmill_Deployer(280, gasleft());
        windmill.DAO.init(data, isProposalSuper);
emit GasLog_Windmill_Deployer(281, gasleft());
        
        windmill.DAO.addUser(msg.sender, 3);
emit GasLog_Windmill_Deployer(283, gasleft());
        
        windmill.DAO.grantRole(updaterRole, updaterAddress);
emit GasLog_Windmill_Deployer(285, gasleft());
        
        windmill.DAO.revokeRole(DAORole, thisAddr);
emit GasLog_Windmill_Deployer(287, gasleft());
...
emit GasLog_Windmill_Deployer(296, gasleft());
        address payable addr = payable(Create2.deploy(0, bytes32(salt), bytecode));
emit GasLog_Windmill_Deployer(297, gasleft());
        salt += 1;
emit GasLog_Windmill_Deployer(298, gasleft());
        
        return addr;
emit GasLog_Windmill_Deployer(300, gasleft());
    }
}