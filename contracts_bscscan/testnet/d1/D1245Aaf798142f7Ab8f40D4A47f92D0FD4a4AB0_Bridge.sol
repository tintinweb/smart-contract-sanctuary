 ...
 *
 * Attributes:
 * - Mints wROOT tokens from an authorized member after a cross-chain migration
 * - Burns wROOT tokens from the bridge and emits a cross-chain fee migration event
 */
contract Bridge is SatelliteMultiBridge {
  constructor(ISatelliteWROOT wROOT, uint256[] memory chainList) public SatelliteMultiBridge(wROOT, chainList) {}
}
...
interface ISatelliteWROOT is IERC20 {
...
import "../interfaces/ISatelliteWROOT.sol";
import "./MultiBridge.sol";

/**
 * wROOT Satellite MultiBridge
 *
 * Attributes:
 * - Mints satellite wROOT tokens to a user from an authorized member after a cross-chain migration
 * - Burns satellite wROOT tokens from a user by an authorized member to initiate a cross-chain migration
 * - Burns satellite wROOT tokens from the bridge and emits a cross-chain fee migration event
 */
contract SatelliteMultiBridge is MultiBridge {
    using SafeERC20 for ISatelliteWROOT;

    // Mintable / Burnable Token
    ISatelliteWROOT private immutable SATELLITE_wROOT;
    ...
    constructor(ISatelliteWROOT SatelliteWROOT, uint256[] memory chainList)
        public
        MultiBridge(SATELLITE_UNLOCK_COST, chainList)
    {
        SATELLITE_wROOT = SatelliteWROOT;
    }
    ...
    function lock(
        ...
        uint256 lockAmount = SATELLITE_wROOT.balanceOf(address(this));
        SATELLITE_wROOT.safeTransferFrom(msg.sender, address(this), amount);
        lockAmount = SATELLITE_wROOT.balanceOf(address(this)).sub(lockAmount);

        SATELLITE_wROOT.burn(lockAmount);
    ...
    function unlock(
        ...
        SATELLITE_wROOT.mint(to, amount);
        if (refundGasPrice != 0)
    ...
    function burn(address from, uint256 amount) external onlyOwner() {
        SATELLITE_wROOT.burn(from, amount);
