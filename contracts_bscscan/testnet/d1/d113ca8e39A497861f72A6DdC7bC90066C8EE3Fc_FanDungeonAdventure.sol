    ...
    AdventureInfo
{
        ...
        internal view returns (Team memory) 
    {
        ...
        uint256 _remainingCrystal = crystalPoolSizeByLevel(_team.dungeonLevel) - dungeon.reservedCrystal;
        uint256 _maxCrystalPerTurn = _dungeonLevelInfo[_team.dungeonLevel].crystalPerSecond * maximumDuration;
        uint256 _maxXpPerTurn = _dungeonLevelInfo[_team.dungeonLevel].xpPerSecond * maximumDuration;

        uint64 durationCrystal = _calculateReduceDuration(_calculateDuration(_remainingCrystal, _maxCrystalPerTurn, _dungeonLevelInfo[_team.dungeonLevel].crystalPerSecond),fanDungeonStage.bonusOf(_team.dungeonId).duration);
        uint64 durationXp = _calculateReduceDuration(_calculateDuration(1000000 ether, _maxXpPerTurn, _dungeonLevelInfo[_team.dungeonLevel].xpPerSecond),fanDungeonStage.bonusOf(_team.dungeonId).duration);
    ...
    function _calculateReduceDuration(uint64 _duration,uint64 _durationReduction) public pure returns(uint64) {
