...
// update 5 = FEE BUY/SELL OK OK, MAX WALLET OK OK, MAX BUY/SELL OK OK, TRNSFR KE BURN AND ANTAR WALET DARI OWNER NO FEE OK OK, LIMIT BY DAY OK OK, TRADING ON/OFF OK OK, CAN TRANSFER BEFORE TRADING ON OK OK
...
contract SUBZEROToken is Context, IBEP20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludeAddressFromMaxTx[to] && !_isExcludedMaxWallet[to] && from != owner() && to != owner() && (from == pancakeSwapV2Pair)) {
        ...
        if (!_isExcludeAddressFromMaxTx[from] && !_isExcludedMinWallet[from] && from != owner() && to != owner() && (to == pancakeSwapV2Pair)) {
