...
contract JPOW is Ownable, ReentrancyGuard {
    ...
    address public WSB = 0x944D45E95cDC7f4b4f34Dc0e3BBA36990b72FE67;
    address public BNB = 0x5dE39e3Fb6997BE9321ded7CD80eE95C7F32baC2;
    address public fundSourceBNB; //source of WSB tokens to pull from
    address public fundSourceWSB;

    address public burnAddress = 0x000000000000000000000000000000000000dEaD;

    //initialize at zero and update later
    uint256 public WSBPerBlock = 0; // WSB tokens distributed per block
    uint256 public BNBPerBlock= 0;
    ...
    function changeBankRollPart(uint _percentage)external onlyOwner{
        require(_percentage<DENOMINATOR, "should be less");
        bankroll=_percentage;
    }
    
    function changeDAOPart(uint _percentage)external onlyOwner{
        require(_percentage<DENOMINATOR, "should be less");
        dao=_percentage;
    }
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && sharesTotal != 0) {
            ...
            uint reReward = WSBReward.mul(95).div(100);
            ...
            accWSBPerShare = accWSBPerShare.add(
                reReward.mul(1e12).div(sharesTotal)
            );
        }
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && sharesTotal != 0) {
            ...
            uint bankrollReward=BNBReward.mul(bankroll).div(DENOMINATOR);
            uint daoReward=BNBReward.mul(dao).div(DENOMINATOR);
            BNBReward=BNBReward.sub(bankrollReward.add(daoReward));    
    ...
    function _updateRewardPerBlock() internal{
        ...
        if(lastRPBlock.add(rPInterval) < block.number){
            ...
            if(increment > 0){
                ...
                lastRPBlock=lastRPBlock.add(rPInterval);
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (user.shares > 0) {
            ...
            if (pendingWSBD > 0) {
                uint poolRewardPart=pendingWSBD.mul(95).div(100);
                uint stakeRewardPart=pendingWSBD.mul(5).div(100);
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (pendingWSBW > 0) {
                uint poolRewardPart=pendingWSBW.mul(95).div(100);
                uint stakeRewardPart=pendingWSBW.mul(5).div(100);
                uint bankrollReward=poolRewardPart.mul(bankroll).div(DENOMINATOR);
                uint daoReward=poolRewardPart.mul(dao).div(DENOMINATOR);
                poolRewardPart=poolRewardPart.sub(bankrollReward.add(daoReward));
                safeWSBTransfer(msg.sender,poolRewardPart);
    ...
    function setBNBPerBlock(uint _BNBPerBlock) external onlyOwner {
        ...
        defaultRewardValue = _BNBPerBlock;
    }
...
contract DarkPool is Ownable {
    ...
    address public WSB = 0x0225647C9B9a941613ba350327f363d166b42A84;
