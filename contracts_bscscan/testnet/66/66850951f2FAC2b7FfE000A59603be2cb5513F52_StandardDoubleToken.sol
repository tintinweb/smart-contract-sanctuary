...
contract StandardDoubleToken is ERC20, Ownable {
    ...
    constructor(
        ...
        uint256 uniswapCount_,
        uint256[2 + 2 + 3] memory buySellShares
        ...
        _buyLiquidityFee = buySellShares[0];
        _buyMarketingFee = buySellShares[1];
        _totalTaxIfBuying = _buyLiquidityFee + _buyMarketingFee;

        _sellLiquidityFee = buySellShares[2];
        _sellMarketingFee = buySellShares[3];
        _totalTaxIfSelling = _sellLiquidityFee + _sellMarketingFee;

        _liquidityShare = buySellShares[4];
        _marketingShare = buySellShares[5];
        _charityShare = buySellShares[6];
