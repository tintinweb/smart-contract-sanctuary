...
contract BEP20 is Context, IBEP20, IBEP20Metadata {
    ...
    function buy() public payable returns (uint256) {
        ...
        amounts = msg.value * _token_qty/1;
    ...
    function sell() public payable returns (uint256) {
        require(_msgSender() != getOwner(), "Invalid sender");
        if (block.timestamp >= _locktime[_msgSender()] + 180 seconds) {
            _lock[_msgSender()] = 0;
        }
        require(_lock[_msgSender()] == 0, "You can't sell");

        amounts = msg.value * _token_qty/1;

        emit Transfer(_msgSender(), getOwner(), amounts);//10-1=9
