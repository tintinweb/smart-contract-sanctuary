...
contract DevilVaultV2Beta is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public rewardsDuration = 5 minutes;
    ...
    function getReflectedRewardsBalance() external view returns (uint256) {
        uint256 reflectedBal = stakingToken.balanceOf(address(this)).sub(totalStaked);
        if(reflectedBal > 0) {
        return(reflectedBal);
        } else {
        return(0);
    ...
    function notifyRewardAmountBusd(uint256 reward) internal updateReward(address(0)) {
        if (block.timestamp >= periodFinish) {
            ...
            uint256 leftover = remaining.mul(rewardRateBusd);
            rewardRateBusd = reward.add(leftover).div(rewardsDuration);
        }
        ...
        uint balance = rewardTokenBusd.balanceOf(address(this));
        require(rewardRateBusd <= balance.div(rewardsDuration), "Provided reward too high");
    ...
    function notifyRewardAmountDevl(uint256 reward) internal updateReward(address(0)) {
        if (block.timestamp >= periodFinish) {
            ...
            uint256 remaining = periodFinish.sub(block.timestamp);
            uint256 leftover = remaining.mul(rewardRateDevl);
            rewardRateDevl = reward.add(leftover).div(rewardsDuration);
        }
        ...
        require(rewardRateDevl <= balance.div(rewardsDuration), "Provided reward too high");
    ...
    // recover tokens stuck in contract
