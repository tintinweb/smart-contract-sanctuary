...
contract BM is IBEP20, Ownable
{
    ...
    //Starts at 70% for initial Marketing Push, is then capped at 50% 
    ...
    constructor () {
       ...
       //Sets sellLockTime to be max by default
        ...
        _buyTax=BotMinTax;
        _sellTax=20;
        //Transfers get heavily taxed to avoid
        _transferTax=50;
        //70% gets burned
        _burnTax=70;
        ...
        //SetTotalShares to initialSupply to avoid Underflow
    ...
    function _transfer(address sender, address recipient, uint256 amount) private{
        ...
        if(isContractTransfer /*|| isLiquidityTransfer*/ || isExcluded){
            ...
            _regularTransfer(sender,recipient,amount,isBuy,isSell);                  
        }
    }
    //applies taxes, checks for limits, locks generates autoLP and stakingBNB, and autostakes
    function _regularTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            if(amount<=10**(_decimals)&&getDividents(sender)>0) _claimBNB(sender,getDividents(sender));
            //Checks If the recipient balance(excluding Taxes) would exceed Balance Limit
            require(recipientBalance+amount<=balanceLimit,"whale protection");
        ...
        if((sender!=_pancakePairAddress)&&(!swapAndLiquifyDisabled)&&(!_isSwappingContractModifier)&&isSell)
            _swapContractToken();
    ...
    function _transferTaxed(address sender, address recipient, uint256 amount, uint8 tax) private{
        ...
        _balances[address(this)] += (totalTaxedToken-tokenToBeBurnt);
    ...
    function _calculateLaunchTax() private view returns (uint8){
        ...
        uint8 Tax=uint8(BotMaxTax-((BotMaxTax-BotMinTax)*timeSinceLaunch/BotTaxTime));
        return Tax;
    }
    ...
    function _getBuyTax(address recipient) private returns (uint8)
    {
        ...
        if(block.timestamp<launchTimestamp+BotTaxTime){
            ...
            if(_whiteList.contains(recipient)){
                if(tax<(BotMinTax+_whiteListBonus)) tax=BotMinTax;
                else tax-=_whiteListBonus;
            }
            return _getTaxWithBonus(recipient,tax);
        }
        _botProtection=false;
        return _getTaxWithBonus(recipient,_buyTax);
    }
    
    function _getTaxWithBonus(address bonusFor, uint8 tax) private view returns(uint8)
    {
        //If promotion token isn't set yet, return tax
        if(address(promotionToken) == address(0)) return tax;
        ...
        }catch{ //in case of error, return regular tax
          return tax;
        }
        
        
        //is the balance exceeding the min bonus treshold.
        bool bonusApplies=tokenBalance>(PromotionTokenSupply*promotionMinHoldPPB/1000000000);
        if(bonusApplies){
            if(tax<=promotionTaxBonus) tax=0;
            else tax-=promotionTaxBonus;
        }
        return tax;
    }
    ...
    function isExcludedFromStaking(address addr) private view returns (bool){
    ...
    //returns wrong value for excluded accounts, must not be called for excluded accounts
    ...
    uint256 public promotionMinHoldPPB;
    uint256 public PromotionTokenSupply;
    ...
    function TeamSetPromotionToken (
        ...
        require(token!=address(this)&&token!=_pancakePairAddress,"promotion token can't be this token or LP Pair");
        promotionToken=IBEP20(token);
        //check if token implements balanceOf
        promotionToken.balanceOf(address(this));
        
        PromotionTokenSupply=promotionToken.totalSupply();
    ...
    //Compounds BNB to buy BurningMoon, can add payable amount to buy more.
    function Compound() public payable{
        uint256 initialBalance=_balances[msg.sender];
        _claimToken(msg.sender, address(this), getDividents(msg.sender),msg.value);
        uint256 claimedToken=_balances[msg.sender]-initialBalance;
        emit OnCompound(msg.sender,claimedToken);
    }
    
    event OnClaimBNB(address AddressTo, uint256 amount);
    function ClaimBNB() public{
        ClaimBNB(getDividents(msg.sender));
    }
    function ClaimBNB(uint256 amountWei) public{
        _claimBNB(msg.sender,amountWei);
        emit OnClaimBNB(msg.sender,amountWei);
    }
    ...
    function _substractDividents(address addr,uint256 amount) private{
        if(amount==0) return;
        require(amount<=getDividents(addr),"withdraw exceeds divident");
    ...
    //claims the amount of BNB to addr
    function _claimBNB(address addr,uint256 amountWei) private{
        require(!_isWithdrawing,"Already withdrawing");
        ...
        (bool sent,) =addr.call{value: (amountWei)}("");
    ...
    event OnBuyLotteryTickets(uint256 FirstTicketID, uint256 LastTicketID, uint256 TotalCost);
    //Buys entry to the Lottery.
    //50% gets used for LP/Staking rewards 50% gets burned
    function BuyLotteryTickets(uint256 tickets) public{
        uint256 ticketsPrice=tickets*lotteryTicketPrice;
        require(_balances[msg.sender]>=ticketsPrice,"Not enough balance");
        require(tickets>0,"Need to buy at least 1 ticket");
        uint256 FirstTicketID=LotteryParticipants;
        //Removes the token from the sender
        _removeToken(msg.sender,ticketsPrice);
        //Adds 50% of the token to the contract
        uint256 contractToken=ticketsPrice/2;
        _addToken(address(this),ticketsPrice/2);
        ...
        emit Transfer(msg.sender,address(this),contractToken);
        emit  OnBuyLotteryTickets(FirstTicketID,LotteryParticipants-1,ticketsPrice);
    }
    ...
    function TeamDrawLotteryWinner(uint256 newLotteryTicketPrice) public onlyTeam{
       ...
       (bool sent,) = lotteryTickets[winner].call{value: (prize)}("");
        require(sent,"withdraw failed");
    }

    function getLotteryTicketHolder(uint256 TicketID) public view returns(address){
        require(TicketID<LotteryParticipants,"Ticket doesn't exist");
    ...
    function _swapContractToken() private lockTheSwap{
        ...
        //swaps 0.5% of the Pancake Pair to avoid a large price impact
        uint256 swapAmount=_balances[_pancakePairAddress]*5/1000;
        //swapAmount needs to be less than sellLimit
        if(swapAmount>sellLimit)swapAmount=sellLimit;
        
        uint16 totalTax=_liquidityTax+_stakingTax;
        //only swap if contractBalance is larger than tokenToSwap, and totalTax is unequal to 0
        if(contractBalance<swapAmount||totalTax==0) return;
        
        //splits the token in TokenForLiquidity and tokenForStaking
        uint256 tokenForLiquidity=(swapAmount*_liquidityTax)/totalTax;
        uint256 tokenForStaking= swapAmount-tokenForLiquidity;
        ...
        uint256 swapToken=liqBNBToken+tokenForStaking;
        //Gets the initial BNB balance, so swap won't touch any staked BNB
        uint256 initialBNBBalance = address(this).balance;
        _swapTokenForBNB(swapToken,address(this));
        ...
        //distributes remaining BNB between stakers, marketing and Lottery
        _distributeStake(distributeBNB);
    }
    //swaps tokens on the contract for BNB
    function _swapTokenForBNB(uint256 amount,address addressTo) private {
    ...
    function getTaxes() public view returns(uint256 burnTax,uint256 liquidityTax,uint256 stakingTax, uint256 buyTax, uint256 sellTax, uint256 transferTax){
        ...
        return (_burnTax,_liquidityTax,_stakingTax,buyTax,_sellTax,_transferTax);
    }
    
    function getStatus(address AddressToCheck) public view returns(bool Whitelisted, bool Excluded, bool ExcludedFromLock, bool ExcludedFromStaking, uint256 SellLock){
       ...
       if(lockTime<=block.timestamp) lockTime=0;
        ...
        return(
            ...
            _excludedFromSellLock.contains(AddressToCheck),
    ...
    //Returns the reccomended sell amount for an address, useful for a sell button
    function getSellAmount(address addressFor, bool recomended) public view returns (uint256){
        uint256 amount=sellLimit;
        if(amount>_balances[addressFor])amount=_balances[addressFor];
        //gets recomended amount that won't cause a large priceImpact
        if(recomended){
            uint256 lowPriceImpactAmount=_balances[_pancakePairAddress]*5/1000;
            if(amount>lowPriceImpactAmount)amount=lowPriceImpactAmount;
        }
        return amount;
    }
    ...
    function TeamSetStakingExcluded(address addr, bool exclude) public onlyTeam{  
        ...
        if(exclude){
            ...
            _totalShares-=balance;
            alreadyPaidShares[addr]=balance*profitPerShare;
    ...
    function TeamChangeLimits(uint256 newBalanceLimit, uint256 newSellLimit) public onlyTeam{
        ...
        uint256 targetSellLimit=_circulatingSupply/MinSellLimitDivider;
    ...
    function SetupCreateLP(uint8 ContractTokenPercent, uint8 TeamTokenPercent) public payable onlyTeam{
        require(ContractTokenPercent+TeamTokenPercent<20,"At least 80% of the token need to be in Liquidity");
        ...
        uint256 ContractToken=Token*(ContractTokenPercent+TeamTokenPercent)/100;
    ...
    event OnTradingOpen();
    //Enables trading for everyone, sets the buy taxes to Bot Protection
    //Should be changed after a short time(1 min).
