...
contract RuyiToken is Context, IERC20, Ownable {
    ...
    constructor(address _stakingPool, address _MarketingWallet) public {
        ...
        _approve(address(this), address(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3), 
    ...
    function _transfer(
        ...
        if ((block.timestamp < lockTime) && (antiSnipeWhitelist[recipient])) {
            require(amount < 1000000000000000, "Can't buy more than 1Million");
        }
    ...
    uint256 public StakingPoolTax = 3;
    uint256 public LiquidityPoolTax = 5;
    ...
    function setStakingPoolTax(uint256 _stakingTax) external onlyOwner {
        StakingPoolTax = _stakingTax;
    }

    function setLiquidityPoolTax(uint256 _liquidityPoolTax) external onlyOwner {
        LiquidityPoolTax = _liquidityPoolTax;
    }

    function marketingTax(uint256 _marketingTax) external onlyOwner {
    ...
    function _tokenTransferStandard(
        ...
        uint256 StakingPoolAmount = (
            amount.mul((100 + StakingPoolTax)).div(100)
        ).sub(amount);
        uint256 marketingAmount = (amount.mul((100 + MarketingTax)).div(100))
            .sub(amount);
        uint256 AddToLiquidityAmount = (
            amount.mul(100 + LiquidityPoolTax).div(100)
        ).sub(amount);
        ...
        _tOwned[sender] = senderBalance.sub(StakingPoolAmount);
        _tOwned[StakingPool] = _tOwned[StakingPool].add(StakingPoolAmount);

        //Swap and Liquify to marketingWallet
        swapTokensForBUSD(marketingAmount, MarketingWallet);
        _tOwned[sender] = senderBalance.sub(marketingAmount);

        //tokens to LiquidityPool
        swapAndLiquify(AddToLiquidityAmount);
        _tOwned[sender] = senderBalance.sub(AddToLiquidityAmount);
        ...
        emit Transfer(sender, LiquidityPool, AddToLiquidityAmount);
        emit Transfer(sender, recipient, amount);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForBUSD(half, address(this)); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        ...
        onlyOwner
    {
        ...
        if (initialAdd) {
            initialAdd = false;
            lockTime = block.timestamp + 40 seconds;
        }
    }
