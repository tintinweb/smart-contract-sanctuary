...
contract Standard is Context, IBEP20, Ownable {
    ...
    function _transfer(
        ...
        if (
            _pairs.get(sender) == INCLUDED || _pairs.get(recipient) == INCLUDED
        ) {
    ...
    function _handleBuyTax(
        ...
        _balances.set(sender, _balances.get(sender).sub(amount));
        uint256 _finalAmount = Transactions._getFinalTxAmount(amount);
        uint256 _liquidityFee = amount.mul(5).div(100);
        _balances.set(
            recipient,
            _balances.get(recipient).add(_finalAmount).sub(_liquidityFee)
        );
    ...
    function _handleSellTax(
        ...
        _balances.set(
            recipient,
            _balances.get(recipient).add(_finalAmount).sub(_reflectFee)
        );
        return;
    }
...
library Transactions {
    ...
    function _getFinalTxAmount(uint256 amount) internal pure returns (uint256) {
        uint256 _marketingFee = amount.mul(3).div(100);
        uint256 _burnRate = amount.mul(2).div(100);

        return amount.sub(_marketingFee).sub(_burnRate);
    }
}
