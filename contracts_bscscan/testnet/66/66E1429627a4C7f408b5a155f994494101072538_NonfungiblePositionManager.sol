    ...
    SelfPermit
{
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        PoolAddress.PoolKey memory poolKey = _poolIdToPoolKey[position.poolId];
        IFuzzyswapPool pool = IFuzzyswapPool(PoolAddress.computeAddress(factory, poolKey));
...
library PoolAddress {
    ...
    bytes32 internal constant POOL_INIT_CODE_HASH =0x36acb35ab90be5e57f6333650c9f12d4cef080ab34a5d73c982484632ed1ee5e;
...
abstract contract PoolInitializer is IPoolInitializer, PeripheryImmutableState {
    ...
    function createAndInitializePoolIfNecessary(
        ...
        pool = IFuzzyswapFactory(factory).getPool(token0, token1, fee);

        if (pool == address(0)) {
            pool = IFuzzyswapFactory(factory).createPool(token0, token1, fee);
            IFuzzyswapPool(pool).initialize(sqrtPriceX96);
        } else {
            (uint160 sqrtPriceX96Existing, , , , , , ) = IFuzzyswapPool(pool).slot0();
