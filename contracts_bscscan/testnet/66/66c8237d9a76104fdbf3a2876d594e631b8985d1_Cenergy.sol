...
contract Cenergy is Context, IERC20, Ownable { 
    ...
    // Non-BUSD fees
    uint256 public _FeeReflection = 0;    // 1% 
    uint256 public _FeeBurnTokens = 0;    // 1%
    // BUSD fees
    uint256 public _FeeLiquidity = 800;     // 3%
    uint256 public _FeeDevTeam = 0;       // 1.75%
    uint256 public _FeeProject = 0;       // 3%
    uint256 public _FeeGenTeam = 0;        // 0.25%
    // Total fees
    ...
    //xxxxxxxxxxx CREATE BUSD PAIR xxxxxxxxxxxxx
        ...
        //uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
          ...
          //  .createPair(address(this), 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); // TEST NET - BUSD
    ...
    */
    ...
    // BUSD TOKEN
        ...
        //IERC20 BUSD = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); // TEST
      ...
      //  address public BUSD = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); //BUSD TEST
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (tryMethod == 1){
        ___swapToBUSD_FEE(firstHalf);} else if (tryMethod == 2){
        ___swapToBUSD_NO_FEE(firstHalf);} else if (tryMethod == 3){
        ___swapToBUSD_3STEP_FEE(firstHalf);} else if (tryMethod == 4){
        ___swapToBUSD_3STEP_NOFEE(firstHalf);}





        uint256 totalBUSD = IERC20(BUSD).balanceOf(address(this)) - balanceBeforeSwap;

        ___addLiquidity(secondHalf,totalBUSD);
        emit SwapAndLiquify(firstHalf,totalBUSD, secondHalf);

    
    }

    uint8 public tryMethod = 1;

    function ___set_tryMethod(uint8 tryMethod_Number) public {
        tryMethod = tryMethod_Number;
    }
    ...
    function ___swapToBUSD_FEE(uint256 tokenAmount) public {
    ...
    function ___swapToBUSD_NO_FEE(uint256 tokenAmount) public {
      ...
      function ___swapToBUSD_3STEP_FEE(uint256 tokenAmount) public {
      ...
      function ___swapToBUSD_3STEP_NOFEE(uint256 tokenAmount) public {
    ...
    function ___addLiquidity(uint256 tokenAmount, uint256 BUSDAmount) public {

        _approve(address(this), address(uniswapV2Router), tokenAmount);
        _approve(BUSD, address(uniswapV2Router), BUSDAmount);
        uniswapV2Router.addLiquidity(
            address(this),BUSD,tokenAmount,BUSDAmount,0,0,owner(), block.timestamp
        );
    }

    /*

      function ___swapTBUSD_3STEP_NOFEE(uint256 tokenAmount) public {
