...
contract test is ERC20, Ownable {
    ...
    // Transfer Limiter    
    mapping (address => uint256) private _transferTimeLog;    
    mapping (address => uint256) private _transferAmountLog;
    uint256 public _transferTimeLimit = 3600;
    uint256 public _transferAmountLimit = 1000 * (10**18);
...
// take fee
        ...
        if(takeFee) {
            ...
            if(_transferTimeLog[from] > block.timestamp){
               ...
               _transferTimeLog[from] = block.timestamp + _transferTimeLimit;
               _transferAmountLog[from] = amount; 
            }
            //
            if(_overLimit){
                fees = amount.mul((totalFees + _transferOverLimitTax)).div(100);
            }
            else{
