...
contract AdvensturesSlot is Context, PartnerOwnable { 
    ...
    function bet(uint[] calldata bets,uint domainId, uint tokensId) external  
        ...
        if( win_tot > cost ) { //win
            wins.wins_result = win_tot - cost; 
            TransferHelper.safeTransfer(FRUIT, msg.sender, wins.wins_result);   
            if(tokensInfo[tokensId].tokens_cake_balance >= wins.wins_result) {
                tokensInfo[tokensId].tokens_cake_balance -= wins.wins_result;
            } else {
            ...
            wins.wins_result = cost - win_tot;
            TransferHelper.safeTransferFrom(FRUIT, msg.sender, address(this), wins.wins_result); 
            tokensInfo[tokensId].tokens_cake_balance += wins.wins_result;
        }
