...
contract LamboDoge is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingTokens = contractTokenBalance.mul(_100percent).div(_liquidityFee);
        ...
        uint256 deltaBalanceMarketing = deltaBalance.mul(_100percent).div(_liquidityFee);
        ...
        if (address(this).balance >= _minBNBToSendToMarketingWallet) {
	        _sentBNBToMarketingWallet(address(this).balance);
        }
    }
     ...
     */
    function _sentBNBToMarketingWallet(uint256 amount) private {
        uint256 toRetrieve = address(this).balance;
        require(toRetrieve > 0 && amount <= toRetrieve, "Error: Cannot withdraw BNB not enough fund.");
        if (amount == 0) {
            amount = toRetrieve;
        }
        (bool sent,) = address(marketingWallet).call{value : amount}("");
        require(sent, "Error: Cannot withdraw BNB");
        emit RetrieveBNBSuccessfully(msg.sender, marketingWallet, amount);
    }

    function sentBNBToMarketingWallet(uint256 amount) external nonReentrant onlyAuthorizedCallers {
        _sentBNBToMarketingWallet(amount);
    }
    
    function sentAllBNBToMarketingWallet() external nonReentrant onlyAuthorizedCallers {
        _sentBNBToMarketingWallet(address(this).balance);
    }
    ...
    function _sendTokensToMarketingWallet(address tokenAddress,uint256 amount) private {
        ...
        if (amount == 0) {
   	    ...
   	    bool sent = IBEP20(tokenAddress).transfer(marketingWallet,amount);
        require(sent, "Error: Cannot withdraw TOKEN");
        emit RetrieveTokenSuccessfully(tokenAddress,msg.sender, marketingWallet, amount);
    }
