...
contract NFTsRealmMarketplace is ReentrancyGuard, Ownable {
    ...
    mapping(uint => string) public paymentTokenMap;
    ...
    function openTrade(uint _id, uint256 _price, string memory paymentType)
    public
    {
    ...
    function buy(uint _id, uint256 _price, string memory paymentType) external payable {
        ...
        if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("KCS")))) {
            ...
            require(IERC20(tokenAddressMap[paymentType]).transferFrom(payable(msg.sender), payable(adminRecoveryAddress), _commissionValue));
        }
        NFTsRealm(nftContract).transferFrom(address(this), msg.sender, _id);
    ...
    function updatePrice(uint _tokenId, uint256 _price, string memory paymentType) public returns (bool) {
        ...
        onlyCreatorOrWinner(tokenId)
    {
        ...
        if(winner == address(0)) {
            ...
            if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("KCS")))) {
                ...
                require(IERC20(tokenAddressMap[paymentType]).transfer(payable(adminRecoveryAddress), _commissionValue));
                if (Creator == creatorMap[tokenId]) {
