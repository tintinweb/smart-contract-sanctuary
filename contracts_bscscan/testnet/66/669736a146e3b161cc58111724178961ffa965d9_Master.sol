...
contract Master is Ownable {
    ...
    // Market oracle provides the GUD/USD exchange rate as an 18 decimal fixed point number.
    ...
    constructor(address _gud) public {
    ...
    function rebase() external {
        ...
        uint256 supplyAfterRebase = gud.rebase(epoch, supplyDelta);
    ...
    function incrementTargetRate() internal {
        targetRate = targetRate.mul(24).div(20);
    }
    
    // set rate to BTC rate + 10%
    function setTargetRateBTC() internal {
        uint256 btcRate = marketOracleBTC.getData();
        targetRate = btcRate.mul(110).div(100);
    }
    ...
    function getRebaseValues() public view returns (uint256, int256) {
        ...
        if (supplyDelta > 0 && gud.totalSupply().add(uint256(supplyDelta)) > MAX_SUPPLY) {
            supplyDelta = (MAX_SUPPLY.sub(gud.totalSupply())).toInt256Safe();
        }
        ...
        returns (int256)
    {
        ...
        return gud.totalSupply().toInt256Safe()
