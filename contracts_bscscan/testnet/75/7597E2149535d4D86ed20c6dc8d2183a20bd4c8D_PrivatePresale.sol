...
contract PrivatePresale {
    ...
    function testDev(address _address) public onlyOwner {
        require(privateOpen,"PrivateRound: Private Round is Closed");
        uint256 _depositAmount = 100*10**18;
        totalCollected += _depositAmount;
        txCount ++;
        inPresale[_address].isWhiteListed = true;
        inPresale[_address].amount += _depositAmount;
        inPresale[_address].tokenAmount += (_depositAmount*10**2)/privatePrice;
        inPresale[_address].newAmount += (_depositAmount*10**2)/privatePrice;
        inPresale[_address].tgeCollect += (((_depositAmount*10**2)/privatePrice)*tgeToCollect)/(1*10**5);
        inPresale[_address].nrmCollect += (((_depositAmount*10**2)/privatePrice)*normalCollect)/(1*10**5);
        inPresale[_address].txCount ++;
        amountOfTokensNeeded += (_depositAmount*10**2)/privatePrice;
        emit DepositSuccessful(_address, _depositAmount, inPresale[_address].amount);
    }
    ...
    function privateRound(uint256 _depositAmount) public {
        ...
        require(_depositAmount > 0,"PrivateRound: You sent 0 BUSD");
        
        // require(inPresale[msg.sender].amount + _depositAmount <= walletLimit, "PrivateRound: Wallet limit has been reached");
        // require(previousCollected + _depositAmount >= depositToken().balanceOf(address(this)), "PrivateRound: Guard Activated");
        
        uint256 _depositAmountCorr = _depositAmount*10**18;
        depositToken().safeTransferFrom(msg.sender, address(this), _depositAmountCorr);
        previousCollected = depositToken().balanceOf(address(this));
        totalCollected += _depositAmountCorr;
        txCount ++;
        inPresale[msg.sender].isWhiteListed = true;
        inPresale[msg.sender].amount += _depositAmountCorr;
        inPresale[msg.sender].tokenAmount += (_depositAmountCorr*10**2)/privatePrice;
        inPresale[msg.sender].newAmount += (_depositAmountCorr*10**2)/privatePrice;
        inPresale[msg.sender].tgeCollect += (((_depositAmountCorr*10**2)/privatePrice)*tgeToCollect)/(1*10**5);
        inPresale[msg.sender].nrmCollect += (((_depositAmountCorr*10**2)/privatePrice)*normalCollect)/(1*10**5);
        inPresale[msg.sender].txCount ++;
        amountOfTokensNeeded += (_depositAmountCorr*10**2)/privatePrice;
    ...
    function isWhitelisted(address _address) public view returns (bool){
        if (inPresale[_address].isWhiteListed)
            return true;
        else    
            return false;
    }

    /// @return amount of tokens left to claim by wallet
    function tokensLeftToClaim(address _address) public view returns(uint256) {
        return inPresale[_address].newAmount;
    }
    
    ///@return initial deposit amount in deposit tokens
    function depositAmount(address _address) public view returns (uint256) {
        require(inPresale[_address].isWhiteListed == true, "PresaleError: Address has not participated");
        return(inPresale[_address].amount);
    }
