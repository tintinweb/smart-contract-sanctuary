...
91.9% automatic liquidity of purchase
...
contract DeAi is ERC20, Ownable {
    ...
    uint256 private _initialTotalSupply = 10**9 * 10**18; // 1 Trillion
    ...
    // Fee 1%
    uint256 private _fee = 100; // 1% = 100 of 10000
    ...
    uint256 liquidityPercent = 9190; // 91.9 % of 10000

    bool private IDOEnabled = false;
    bool private IDOPeriod = true;

    uint256 private IDOThresholdAmount = 260000000; // 26%
    uint256 private IDOUnlockTime = 15000000;
    ...
    constructor () ERC20("Decentralized Ai TEST", "DeAiT02") {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != pancakeswapV2Pair, "DeAi: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function swapAndLiquify(uint256 swapTokenAmount) private lockTheSwap {
        ...
        uint256 newBalance = owner().balance.sub(initialBalance);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        _approve(owner(), address(pancakeswapV2Router), tokenAmount);
    ...
    function _transfer(
        ...
        // Buy token and automatic add liquidity from owner
        if (automatedMarketMakerPairs[from] && to != owner() && swapAndLiquifyEnabled) {
            uint256 liquidityAmount = amount.mul(liquidityPercent).div(10000);
        ...
        if(takeFee) {
            uint256 fees = amount.div(10000).mul(_fee);
