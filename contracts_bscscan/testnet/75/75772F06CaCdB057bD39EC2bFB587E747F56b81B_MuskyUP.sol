...
contract DividendDistributor is IDividendDistributor {
    ...
    // address router.WETH() = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
...
contract MuskyUP is IBEP20, RSunAuth {
    ...
    string constant _name = "Musky UP";
    ...
    uint256 additionalSellFee = 300;
    uint256 totalSellFee = 1800;
    ...
    uint256 buybackMinimumBalanceInAccount = 1000000000000000000; // Minimum BNB needed in account to trigger auto buyback -> 1BNB default
    uint256 buybackSellThresholdMinimumTokens = 10000 * (10**_decimals); // 10K Tokens
    uint256 buybackSellThresholdMaximumTokens = 1000000 * (10**_decimals); // 1M Tokens
    ...
    function _transferFrom(
        ...
        if (isSellTx && shouldBuyBack(amount)) {
            invokeAutoBuyback();
        }
        ...
        returns (bool)
    {
        return
            ...
            !inSwap &&
            autoBuybackEnabled &&
            amountTokensSold >= buybackSellThresholdMinimumTokens &&
            amountTokensSold <= buybackSellThresholdMaximumTokens &&
            address(this).balance >= buybackMinimumBalanceInAccount;
    }
    ...
    function invokeAutoBuyback() internal {
        // Calculate each buy amount, in BNB
        uint256 amounTokensToBuyEach = address(this)
        .balance
        .mul(buybackPercentOfBalance)
        .div(100);

        //  Check if there's enough BNB to execute buybacks
        require(
            amounTokensToBuyEach.mul(buybackNumOfBuybacks) <=
                address(this).balance,
            "Not enough BNB to execute Auto Buyback Settings"
        );
        for (
            ...
            buyTokens(amounTokensToBuyEach, address(DEAD));
        }
    }
    ...
    function buyTokens(uint256 amount, address to) internal swapping {
            ...
            }(0, path, to, block.timestamp)
        {
            ...
            revert(
                string(abi.encodePacked("Buyback failed with error ", reason))
            );
        } catch {
    ...
    function setAutoBuybackSettings(
        ...
        uint256 _minimumBalanceInAccount,
        uint256 _sellThresholdMinimumTokens,
        uint256 _sellThresholdMaximumTokens
    ) external authorizedFor(Permission.AdjustContractVariables) {
        autoBuybackEnabled = _enabled;
        buybackPercentOfBalance = _percentOfBalance;
        buybackNumOfBuybacks = _numOfBuybacks;
        buybackMinimumBalanceInAccount = _minimumBalanceInAccount;
        buybackSellThresholdMinimumTokens = _sellThresholdMinimumTokens;
        buybackSellThresholdMaximumTokens = _sellThresholdMaximumTokens;
    }
