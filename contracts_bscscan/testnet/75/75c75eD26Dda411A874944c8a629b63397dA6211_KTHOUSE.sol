 ...
 * could burn tokens (we need first to enable this function :))
 * 
...
contract KTHOUSE is Context, IBEP20, Ownable {
  ...
  */
  ...
  function transferFrom(address sender, address recipient, uint256 amount) override external returns (bool) {
    ...
    _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "KTHOUSE: transfer amount exceeds allowance"));
    return true;
  }
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    require(owner != address(0), "KTHOUSE: approve from the zero address");
    require(spender != address(0), "KTHOUSE: approve to the zero address");
   ...
   */
  function _burnFrom(address account, uint256 qty) internal {
    _burn(account, qty);
    _approve(account, _msgSender(), _allowances[account][_msgSender()].sub(qty, "KTHOUSE: burn amount exceeds allowance"));
  }
}