...
abstract contract Authorizable is Context, Ownable {
    address private _authorized;

    event AuthorizedTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    constructor () {
        ...
        emit AuthorizedTransferred(address(0), msgSender);
    }
    ...
    modifier onlyAuthorized() {
        require(authorized() == _msgSender(), "Authorized: caller is not authorized");
        _;
    }
     ...
     * thereby removing any functionality that is only available to the owner.
     */
    function renounceAuthorized() public virtual onlyOwner {
        emit AuthorizedTransferred(_authorized, address(0));
     ...
     * Can only be called by the current owner.
     */
    function transferAuthorized(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        emit AuthorizedTransferred(_authorized, newOwner);
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; // WBNB address
...
/////////// Tokens /////////////
////////////////////////////////
contract DoughBack is ERC20, Ownable, Authorizable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public _dividendToken = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; // WBNB address
    ...
    bool public takeFeesEnabled = false;

    DoughBackDividendTracker public dividendTracker;
    ...
    event TakeFeesEnabledUpdated(bool enabled);
    ...
    constructor() ERC20("DoughBack", "DOUGH") {
    	...
    	dividendTracker = new DoughBackDividendTracker();

    	buyBackWallet = 0x4Dca5eC2b8a9Ee8353c8FA19a467eD8A90484559; // BUYBACK address
    ...
    /* Mint by authorized (Farming contract) */
    function authMint(address to, uint256 amount) public onlyAuthorized {
        require(amount > 0, "Amount must be more than 0");
        require(msg.sender == authorized(), "Denied");
        _mint(to, amount);
    }
  	...
  	/*function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "DoughBack: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}*/
  	
  	/*function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "DoughBack: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function setAllEnabled(bool _enabled) public onlyOwner {
        ...
        takeFeesEnabled = _enabled;
        emit TakeFeesEnabledUpdated(_enabled);
    }
    ...
    function setTakeFeesEnabled(bool _enabled) public onlyOwner {
        takeFeesEnabled = _enabled;
        emit TakeFeesEnabledUpdated(_enabled);
    }
    
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "DoughBack: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DoughBack: The dividend tracker already has that address");

        DoughBackDividendTracker newDividendTracker = DoughBackDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DoughBack: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "DoughBack: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DoughBack: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DoughBack: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DoughBack: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DoughBack: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "DoughBack: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DoughBack: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DoughBack: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        bool takeFee = takeFeesEnabled;

        if(takeFee) {
        	uint256 fees = amount.div(100).mul(dividendRewardsFee);

            // if sell, multiply by 1.2
            /* disabled, no extra fees
...
contract DoughBackDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DoughBack_Dividend_Tracker", "DoughBack_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DoughBack_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "DoughBack_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DoughBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DoughBack_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DoughBack_Dividend_Tracker: Cannot update claimWait to same value");
