    ...
    ReentrancyGuard
{
    ...
    function splitfunds(uint256 amount) private {
        //Check percentage
        transferBNBtoAddress(
            developersAddress,
            (amount - (developersPercent / 100))
        );
        transferBNBtoAddress(holdersAddress, (amount - (holdersPercent / 100)));
        transferBNBtoAddress(
            buyBackAddress,
            (amount - (buyBackWalletPercent / 100))
        );
    }

    function swapTokens(uint256 contractTokenBalance) private {
        uint256 initialBalance = address(this).balance;
        uint256 total = address(this).balance - calculateBuyBack();
        splitfunds(calculateBuyBack());
    ...
    function swapBNBForTokens(uint256 amount) private nonReentrant {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
    ...
    function buyBackTokens(uint256 amount) private nonReentrant {
        if (amount > 0) {
            ...
            swapBNBForTokens(total);
        }
    }

    function calculateBuyBack() private view returns (uint256) {
        uint256 value = (buyBackPercent / 100);
        return value;
    }

    function checkHolder(address user) private {
