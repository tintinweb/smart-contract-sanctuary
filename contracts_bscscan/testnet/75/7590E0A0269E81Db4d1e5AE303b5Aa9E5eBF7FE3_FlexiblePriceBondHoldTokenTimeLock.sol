...
contract FlexiblePriceBondHoldTokenTimeLock is PositionBond, HoldToken, TimeLock {
    ...
    {
        for (uint i =0; i < bondPriceRange_.length; i++) {
            bondPriceRange.push(BondStruct.BondPriceRange({
            min : bondPriceRange_[i].min,
            max : bondPriceRange_[i].max,
            price : bondPriceRange_[i].price
            }));
        }
        _issuePrice = BondMath.calculateIssuePriceWithRange(bondPriceRange_, totalSupply_);
    }

    function canPurchase() public view virtual override returns (bool) {
        return TimeLock._isNotLocked() && HoldToken._holdEnoughToken(msg.sender);
    }
    ...
    returns (uint256, uint256)
    {
        ...
        return BondMath.getBondAmountInRange(_bondPriceRange, _currentSupply, amount);
    }
    ...
    IPositionBond
{
    ...
    function active(
        ...
        uint64 _active,
        uint64 _maturity
    ) public override(IPositionBond, BondStatus) onlyIssuer {
        ...
        onlyLiquidated
    {
        ...
        UnderlyingAsset._transferUnderlyingAssetLiquidated(_bondBalance, bondSupply);
    }

    /// @dev see {IPositionBond-claimSoldAmount}
    function claimSoldAmount(uint256 amount) external onlyIssuer onlyActive {
     ...
     */
    function claimFaceValue() external onlyMatured onlyRefunded {
        uint256 _bondBalance = balanceOf(msg.sender);
    ...
    function claimRedundantUnderlyingAsset() external onlyNotOnSale onlyIssuer {
        UnderlyingAsset._transferRedundantUnderlyingAsset(bondSupply, totalSupply());
    }
     ...
     */
    function purchase(uint256 amount) external onlyOnSale purchasable {
    ...
    /// @dev see {IPosititionBond-liquidate}
...
abstract contract UnderlyingAsset {
    ...
    // TODO define param event
        event LiquidatedUnderlyingAssetClaimed();
    ...
    function claimUnderlyingAsset() public virtual {
        _underlyingAsset.transfer(_issuer(), _underlyingAmount);
    }
    ...
    function _transferRedundantUnderlyingAsset(uint256 totalBondSupply, uint256 currentBondSupply) internal {
        uint256 redundantUnderlyingAsset = BondMath.calculateRedundantUnderlyingAsset(totalBondSupply, currentBondSupply, _underlyingAmount);
        _underlyingAsset.transfer(msg.sender, redundantUnderlyingAsset);
    }

    function _transferUnderlyingAssetLiquidated(uint256 bondBalance, uint256 bondSupply) internal {
        uint256 calculatedUnderlyingAsset = BondMath.calculateUnderlyingAsset(bondBalance, bondSupply, _underlyingAmount);
...
abstract contract FaceAsset {
    ...
    event FaceValueRepaid(uint256 amount, uint256 bondAmount, uint256 faceValue);
    ...
    function _transferRepaymentFaceValue(uint256 totalSupply) internal virtual {
        uint256 calculatedFaceValue = BondMath.calculateFaceValue(totalSupply, _faceValue);
    ...
    function _transferFaceValueOut(uint256 bondAmount) internal virtual {
        uint256 calculatedFaceValue = BondMath.calculateFaceValue(bondAmount, _faceValue);
...
abstract contract BondStatus {
    ...
    uint64 private constant liquidationDeadline = 12 hours;
    ...
    modifier onlyNotOnSale() {
        require(!_isOnSale(), "only not on sale");
        _;
    }
    ...
    function getUnderlyingAssetStatus() public view virtual returns (UnderlyingAssetStatus) {
    ...
    function _setUnderlyingAssetStatusLiquidated() internal virtual {
        require(getUnderlyingAssetStatus() == UnderlyingAssetStatus.OnHold, "!OnHold");
    ...
    function _setUnderlyingAssetStatusRefunded() internal virtual {
        require(getUnderlyingAssetStatus() == UnderlyingAssetStatus.ReadyToClaim, "!ReadyToClaim");
...
interface IPositionBond {
    ...
    event BondActivated(uint64 startSale, uint64 active, uint64 maturity);
    ...
    /// @notice When the bond is not on sale, issuer can claim redundant underlying asset
    /// Requirements:
    /// - Bond is not on sale
    function claimRedundantUnderlyingAsset() external;
...
library BondMath {
    ...
    function calculateFaceValue(uint256 bondUnit, uint256 faceValue) internal pure returns (uint256) {
        return bondUnit * faceValue / (10 ** 18);
    }

    function calculateBondAmountWithFlexPrice(uint256 maxOfRange, uint256 priceOfRange, uint256 currentSupply, uint256 amount) internal pure returns (uint256 bondAmount, uint256 paidFaceAmount) {
        uint256 availableAmountInRange = maxOfRange - currentSupply;
        if (amount <= availableAmountInRange) {
            bondAmount = calculateBondAmountWithFixPrice(amount, priceOfRange);
            paidFaceAmount = amount;
        } else {
            bondAmount = calculateBondAmountWithFixPrice(availableAmountInRange, priceOfRange);
            paidFaceAmount = availableAmountInRange;
        }
    }

    function calculateBondAmountWithFixPrice(uint256 amount, uint256 issuePrice) internal pure returns (uint256) {
        return amount * (10**18) / issuePrice;
    }
    
    function calculateUnderlyingAsset(uint256 bondBalance, uint256 bondSupply, uint256 underlyingAmount) internal pure returns (uint256) {
        return bondBalance * underlyingAmount / bondSupply;
    }

    function calculateRedundantUnderlyingAsset(uint256 totalBondSupply, uint256 currentBondSupply, uint256 underlyingAmount) internal pure returns (uint256) {
        return currentBondSupply / totalBondSupply * underlyingAmount;
    }

    function getBondAmountInRange(BondStruct.BondPriceRange[] memory _bondPriceRange, uint256 _currentSupply, uint256 amount) internal pure returns (uint256 bondAmount, uint256 faceAmount) {
        for (uint i = 0; i < _bondPriceRange.length; i++) {
            if (_bondPriceRange[i].min <= _currentSupply && _currentSupply < _bondPriceRange[i].max) {
                (bondAmount, faceAmount)
                = BondMath.calculateBondAmountWithFlexPrice(_bondPriceRange[i].max, _bondPriceRange[i].price, _currentSupply, amount);
    ...
    function calculateIssuePriceWithRange(BondStruct.BondPriceRange[] memory _bondPriceRange, uint256 _totalSupply) internal pure returns (uint256) {
        uint256 totalFaceValue;
        for (uint i =0; i < _bondPriceRange.length; i++) {
            totalFaceValue += (_bondPriceRange[i].max - _bondPriceRange[i].min + 1) * _bondPriceRange[i].price;
        }
