            ...
            for (uint256 j = 0; j < callList.length; j++) {
                if (callList[j].seq == i) {
                    ...
                    if (callList[j].callType == CallType.execRevert) {
                        ...
                        require(
                            callList[j].tokenAmount > 0,
                            "FC: amount missed"
                        );
                        safeReceive(
                            callList[j].targetContract,
                            callList[j].tokenAmount
                        );
...
library Equation {
  ...
  function calculateBool(Node[] memory self, uint256[] memory xValues) internal view returns (bool) {
    return solveBool(self, 0, xValues);
  }
  ...
  function getChildrenCount(uint8 opcode) private pure returns (uint8) {
    ...
    revert("Equation: opcode isn't supported");
  }
    ...
    private view returns (uint256)
  {
    ...
    revert("Equation: illegal math opcode");
  }
    ...
    private view returns (bool)
  {
    ...
    revert("Equation: illegal bool opcode");
  }
}
