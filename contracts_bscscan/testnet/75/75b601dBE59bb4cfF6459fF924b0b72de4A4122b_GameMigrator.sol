 ...
 */
contract GameMigrator is Ownable{
    MemenopolyV2 public memenopolyOld;
    MemenopolyV2 public memenopolyNew;


    event PlayerMigrated(address indexed account);

    constructor (MemenopolyV2 _oldGame, MemenopolyV2 _newGame) public {
        memenopolyOld = _oldGame;
        memenopolyNew = _newGame;

    }

    function setContracts (MemenopolyV2 _oldGame, MemenopolyV2 _newGame) public onlyOwner {
        memenopolyOld = _oldGame;
        memenopolyNew = _newGame;

    }

    function migrateGame() public {
        require(!memenopolyNew.hasMigrated(msg.sender), "already migrated");
...
//        uint256 jailCountV1,
        bool isRollingV1 ) = memenopolyOld.playerInfo(msg.sender);


        memenopolyNew.updatePlayer (
            msg.sender,
            spotIdV1, 
            rentDueV1, 
            lastRollV1,
        ...
        emit PlayerMigrated(msg.sender);
...
interface IMnopVault {
    function giveShares(address to, uint256 amount) external;
}
...
import "./MnopCardPack.sol";
import "./IMnopVault.sol";

contract MemenopolyStoreV2 is Ownable, IERC1155TokenReceiver {
...
//    event SetCardContract(address indexed user, ERC1155Tradable contractAddress);
    ...
    function migrateFromBanker(address _address) public {
        ...
        require(!hasMigrated[_address], "Store-Banker: Account already migrated");
        _migrateV1(_address);
    }
...
//        _checkMigrate(msg.sender);
    ...
    function redeemPack(uint256 _packId) public payable{
        ...
        require(( packs[_packId].maxRedeem == 0 || packs[_packId].totalRedeemed < packs[_packId].maxRedeem) && ( packs[_packId].maxPerAddress == 0 || userTotalByPack[msg.sender][_packId] < packs[_packId].maxPerAddress), "Max packs Redeemed");
...
//        _checkMigrate(msg.sender);
   ...
   function setCardContract(ERC1155Tradable _mnopCardAddress) public onlyOwner {
    ...
    function setVault(IMnopVault _vault, uint256 _vaultPercent) public onlyOwner {
        vault = _vault;
        vaultPercent = _vaultPercent;
    }
...
//    using SafeBEP20 for IBEP20;
    ...
    // mapping(bytes32 => address payable) private rollQueue;
  ...
  //      burnAddress = 0x000000000000000000000000000000000000dEaD;
    ...
    */
    function roll() public payable  nonReentrant returns (bytes32 requestId)  {
        //roll and move
        // bool burnSuccess = false;
...
*/
        ...
        if(gameSettings.rollTokenBurn > 0){
             ...
             gameStats.rollBurn = gameStats.rollBurn.add(gameSettings.rollTokenBurn);
              ...
              // give shares
              contracts.vault.giveShares(msg.sender, gameStats.rollBurn);
              // contracts.mnop.transferFrom(msg.sender, burnAddress, gameSettings.rollTokenBurn);
    ...
    */
     ...
     */
    function _doRoll(uint256 _roll,address payable _player) internal {
...
//            player.jailCount = player.jailCount.add(1);
           ...
           /*  )
            ...
            if(validRug && address(this).balance > 0){
                ...
                payable(address(contracts.vault)).transfer(toVault);
                // the rest to the dev
                devaddr.transfer(address(this).balance.sub(toVault));
            }
        ...
        if(bSpot.spotType == 9 && player.level >= gameSettings.levelLimit && playerTier >= gameSettings.tierLimit){
            ...
            if(gameStats.parkingBalance > 0){
                ...
                if(address(this).balance > 0){
                   ...
                   _player.transfer(address(this).balance.sub(toVault));
                }
...
contract MemenopolyV2Core is Ownable, ReentrancyGuard {
    // using SafeMath for uint256;
    using SafeMathChainlink for uint256;
...
//        // emit PowerUpSet(_puNftId, _puType, _puValue);
    ...
    */
    function payRent() public nonReentrant {
        ...
        require(gameActive && _rentDue > 0 && mnopBal >= _rentDue, "Can't pay");
      ...
      //  uint256 amtStaking;
        ...
        if(gameSettings.shareRent){
           ...
           uint256 amtStaking = stakers.length;
            ...
            //70% to burn
            toBurn = totalToDistribute.mul(75).div(100);
...
//            require(transferSuccess, "transfer failed");
    ...
    */
    ...
    function setContracts(MNOPTokenV2 _mnop, TheBrokerV2 _theBrokerAddress, TheBankerV2 _theBanker, address _migrator, IMnopVault _vault) public onlyOwner{
        contracts.mnop = _mnop;
 ...
 */
contract MnopCardPack is Ownable, Pausable, MinterRole, ReentrancyGuard, VRFConsumerBase, IERC1155TokenReceiver {
...
contract TheBrokerV2 is  Ownable {
    ...
    function stake(uint256[] memory _cardIds) public {
        ...
        require(_isActive(msg.sender), "TheBroker: Farms locked");
        require(maxStake == 0 || _getNumOfNftsStakedByAddress(msg.sender).add(_cardIds.length) <= maxStake, 'Max cards staked');
