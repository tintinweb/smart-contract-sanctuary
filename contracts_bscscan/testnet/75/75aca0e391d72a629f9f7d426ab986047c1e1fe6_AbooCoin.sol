...
contract Ownable is Context {	
    ...
    modifier onlyDev() {	
        require(_owner == _msgSender() || _devWallet == _msgSender(), "Ownable: caller is not the dev");	
        _;	
    }
    ...
    function unlock() public virtual {
        require(_previousOwner == msg.sender, "You are not the previous owner");	
        require(block.timestamp > _lockTime , "The contract cant be unlocked yet");	
...
contract AbooCoin is Context, IBEP20, Ownable {	
    ...
    uint256 private _tTotal = 100 * 1 * 10**6 * 10**9; // 100 quadrillion
    uint256 private _rTotal = (MAX - (MAX % _tTotal));	
    
    uint256 private _tFeeTotal;	
    string private _name = "ABOO";	
    string private _symbol = "ABOO";
    ...
    uint256 public _maxTxAmount = 100 * 1 * 10**6 * 10**9; // 100 quadrillion

    bool    public _isAntiWhaleEnabled = false;
    uint256 public _antiWhaleThreshold =  1 * 1 * 10**6 * 10**9; // 1 quadrillion (%1)
    ...
    function devSetAddress(address payable dev) public onlyDev() {
        _devWallet = dev;
	...
	function devDecreaseFees(uint256 devFee) external onlyDev() {
	    require(devFee < _devFee, "Taxes cannot be increased");
        _devFee = devFee;
    }
    ...
    function devSetRouter(address r) external onlyDev() {
    ...
    function devSetPair(address p) external onlyDev() {
        uniswapV2Pair = p;
        _isExcluded[p] = false;
    ...
    function devToggleAntiDump(bool enabled) public onlyOwner() {
    ...
    function _transfer(	
        ...
        if(from != owner() && to != owner())	
            require(amount <= _maxTxAmount, "Transfer amount must be below the total supply");	
            
        if ( _isAntiWhaleEnabled && !_isExcludedFromAntiWhale[to] ) {
            require(amount <= _antiWhaleThreshold, "Wallet cant hold more than 1 quadrillion tokens");
            require(balanceOf(to).add(amount) <= _antiWhaleThreshold, "Wallet cant hold more than 1 quadrillion tokens");
        }
