...
contract BUSDDividendTracker is DividendPayingToken {
    ...
    function _transfer(
        ...
        require(false, "TKR_Busd_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "TKR_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TKR contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "TKR_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "TKR_Busd_Dividend_Tracker: Cannot update claimWait to same value"
        );
...
contract TokenRugged is ERC20, Ownable {
    ...
    uint256 public _LpFeeSell = 6;
    uint256 public _marketingFeeSell = 2;

    uint256 public busdDividendFeeAmount;
    ...
    event ExcludeFromDividend(address indexed account);
    ...
    constructor() ERC20("Token Rugged", "TKR") {
        marketingWallet = 0xFba1D5ADe6a6b1075Eb48Ec17509D2F0856474Ee;
    ...
    function setWalletBalance(uint256 _newAmount) external onlyOwner {
        maxWalletBalance = _newAmount * (10**18);
    }

    function setMaxBuyTransaction(uint256 _newAmount) external onlyOwner {
        maxBuyTranscationAmount = _newAmount * (10**18);
    }

    function setMaxSellTransaction(uint256 _newAmount) external onlyOwner {
    ...
    function updateMarketingWallet(address _newWallet) external onlyOwner {
        require(
            ...
            "TKR: The marketing wallet is already this address"
        );
        isExcludedFromFees[marketingWallet] = false;
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "TKR: Account is already exluded from fees"
        );
        isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            ...
            "TKR: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            ...
            "TKR: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        require(
            ...
            "TKR: Trading has not started yet"
        );
        ...
        if (!swapping && canSwap && from != uniswapV2Pair) {
            ...
            if (busdDividendEnabled) {
                uint256 busdTokens = busdDividendFeeAmount;
                swapAndSendBusdDividends(busdTokens);
                busdDividendFeeAmount = 0;
            }
        ...
        if (takeFee) {
            ...
            uint256 tmpMarketingFeeAmount;
            uint256 tmpBusdDividendFeeAmount;
            uint256 tmpLpFeeAmount;

            tmpMarketingFeeAmount = amount.mul(marketingFee).div(100);
            tmpBusdDividendFeeAmount = amount.mul(busdDividendRewardsFee).div(
                100
            );
            tmpLpFeeAmount = amount.mul(liquidityFee).div(100);

            fees = tmpMarketingFeeAmount.add(tmpBusdDividendFeeAmount).add(
                tmpLpFeeAmount
            );

            busdDividendFeeAmount = busdDividendFeeAmount.add(
                tmpBusdDividendFeeAmount
            );

            tokensForLpInContract = tokensForLpInContract.add(tmpLpFeeAmount);
