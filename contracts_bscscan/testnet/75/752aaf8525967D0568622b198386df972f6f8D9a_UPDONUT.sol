...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 RWRD = IERC20(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82);
...
contract UPDONUT is IERC20, Auth {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    string constant _name = "UP DONUT";
    ...
    uint256 public sendingbackFee   = 8;
    uint256 public marketingFee    = 2;
  
    uint256 public totalFee        = marketingFee.add(sendingbackFee).add(liquidityFee);
    ...
    modifier onlymainWallet() {
        require(msg.sender == mainWallet || isOwner(msg.sender));
        _;
    }

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    uint256 private constant TokenSupply = 250000000 * 10**_decimals;
    uint256 public swapThreshold = rSupply * 10 / 10000;
    uint256 public _maxTxAmount = TokenSupply ;
    uint256 public _maxWalletToken = TokenSupply ;
    uint256 private constant MAX_UINT256 = ~uint256(0);
    uint256 private constant MAX_SUPPLY = ~uint128(0);
    uint256 private constant rSupply = MAX_UINT256 - (MAX_UINT256 % TokenSupply);

   
 uint256 public rebase_count = 0;
    uint256 public rate;
    uint256 public _totalSupply;
    function rebase_mainWallet(uint256 _percentage_base1000, bool reduce) public onlymainWallet returns (uint256 newSupply){
    ...
    function rebase_owner(uint256 _percentage_base1000, bool reduce) public onlyOwner returns (uint256 newSupply){
        if(reduce){
            newSupply = rebase(0,int(_totalSupply.div(1000).mul(_percentage_base1000)).mul(-1));
        } else{
            newSupply = rebase(0,int(_totalSupply.div(1000).mul(_percentage_base1000)));
    ...
    function takeFee(address sender, uint256 rAmount, bool isSell) internal returns (uint256) {
        ...
        uint256 FeeAmount = rAmount.div(FeeDivisor * 100).mul(totalFee).mul(multiplier);
    ...
    function swapBack() internal swapping {
        uint256 dynamicLiquidityFee = isOverLiquified(targetLiquidity, targetLiquidityDivisor) ? 0 : liquidityFee;
        ...
        uint256 amountBNBsendingback = amountBNB.mul(sendingbackFee).div(totalBNBFee);
    ...
    function setFees(uint256 _liquidityFee, uint256 _sendingbackFee, uint256 _marketingFee, uint256 _FeeDivisor) external authorized {
        liquidityFee = _liquidityFee;
        sendingbackFee = _sendingbackFee;
        marketingFee = _marketingFee;
        
        totalFee = _liquidityFee.add(_sendingbackFee).add(_marketingFee);
        FeeDivisor = _FeeDivisor;
        require(totalFee < FeeDivisor/3, "Fees cannot be more than 33%");
    }
