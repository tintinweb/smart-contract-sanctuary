...
contract Profile {
  ...
  function getBaseTokenPrice() public view returns (uint256) {
   ...
   function updateWeek (address user, uint256 _gameWeek) external{
  
            freeTransfers[user][_gameWeek] =   1 ;
    ...
    function updateProfile (uint256 _freebalance, uint256 _gameWeek) internal{
        ...
        if(_gameWeek > 0 ){
            freeTransfers[msg.sender][_gameWeek] = freeTransfers[msg.sender][_gameWeek] + 1 ;
        }
