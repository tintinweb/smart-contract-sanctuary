...
contract OKNFTSwap {
    ...
    function testCall(bytes memory calldataValue)payable public returns (bool){
        (bool success, bytes memory returnData) = exchange.call(calldataValue);
        return success;
    }

    function swap(MarketRegistry.TradeDetails  memory tradeDetail)  payable public{
        exchange.call{value:msg.value}(tradeDetail.tradeData);
        //exchange.delegatecall(tradeDetail.tradeData);
    }
