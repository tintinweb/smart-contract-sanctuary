...
uint public constant MAX_UNU = 814;
  ...
  mapping(address => bool) public isGiveAwayWhiteListed;
  ...
  function addWhiteListUser(address _goodUser) public onlyOwner {
   ...
   function removeWhiteList(address _clearedUser) public onlyOwner {
    ...
    function addGiveAwayWhiteListUser(address _goodUser) public onlyOwner {
        isGiveAwayWhiteListed[_goodUser] = true;
        emit AddedWhiteList(_goodUser);
    }
    
   function removeGiveAwayWhiteList(address _clearedUser) public onlyOwner {
        isGiveAwayWhiteListed[_clearedUser] = false;
        emit RemovedWhiteList(_clearedUser);
    }
    
     function addMultiWhiteListUser(address[] memory _goodUser) public onlyOwner {
        for(uint i = 0; i < _goodUser.length; i++) {
            isWhiteListed[_goodUser[i]] = true;
            emit AddedWhiteList(_goodUser[i]);
        }
    }
  ...
  function giveAway(address _to, uint256[] calldata token_id, string calldata _uri, string calldata _payload) external {
      require(isGiveAwayWhiteListed[msg.sender] == true, "you are not valid user to giveAway !");
