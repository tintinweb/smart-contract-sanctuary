...
contract Morebnb {
    ...
    //For Test
	uint256 constant public INVEST_MIN_AMOUNT = 1e14;
    uint256 constant public WITHDRAW_MAX_PER_DAY_AMOUNT = 3e14;
    ...
    uint256 constant public TEST_TIME_STEP = 900;
	...
	constructor(IBEP20 _shiva, address payable wallet, address payable dev) {
        // testnet shiva: 0xb5A5F8fce365a3C50B54407438dC8a9A2C86b346
        // mainnet shiva: 0x99BF43a5CB67030CD395d7733D2E0eFAed9B7b2a
		SHIVA = _shiva;
	...
	function withdraw() public {
        ...
        if(block.timestamp - user.firstwithdrawntime <= TEST_TIME_STEP){
            ...
            require(totalAmount < WITHDRAW_MAX_PER_DAY_AMOUNT, "Exceed max withdrawn amount today");
            user.firstwithdrawntime = block.timestamp;
            user.daywithdrawnamount = 0;
        }
    ...
    function canHarvest(address userAddress) public view returns(bool) {
        ...
        if(block.timestamp - user.firstwithdrawntime <= TIME_STEP){
            return(user.daywithdrawnamount.add(totalAmount) < WITHDRAW_MAX_PER_DAY_AMOUNT);
        } else {
            return(totalAmount < WITHDRAW_MAX_PER_DAY_AMOUNT);
        }
    }
