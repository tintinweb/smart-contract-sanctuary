...
contract MysteryBoxNFT is Ownable, ERC1155, ERC1155Burnable, ERC1155Metadata {
    ...
    event Create(address indexed _owner, uint256 indexed _tokenId, uint256 _quantity);

    constructor() ERC1155("https://docs.openzeppelin.com/") {
        maxMintSupply = 20;
    }
    ...
    function createMysteryBox(
        ...
        require(_amount <= maxMintSupply, "Invalid Amount");
        uint256 tokenId = _getNextTokenID();
        _incrementTokenTypeId();
        creators[tokenId] = _recipient;
        tokenURI[tokenId] = _uri;
        if (_amount != 0) _mint(_recipient, tokenId, _amount, "");
        tokenSupply[tokenId] = _amount;
        mappingBoxInfo[tokenId].box = _box;
        mappingBoxInfo[tokenId].pack = _pack;
        emit Create(_recipient, tokenId, _amount);
        return tokenId;
    }

    function getBoxInfo(uint256 _tokenId) public view returns (bytes memory, bytes memory ) {
        MysteryBox memory info =  mappingBoxInfo[_tokenId];
        return (info.box, info.pack);
    }
    ...
    function getUri(uint256 _tokenId) public view returns (string memory) {
        require(_exists(_tokenId), "ERC1155#uri: Non exist token");
        return Strings.strConcat(baseMetadataURI, tokenURI[_tokenId]);
    }
