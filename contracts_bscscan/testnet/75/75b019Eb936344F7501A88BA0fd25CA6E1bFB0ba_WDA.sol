...
contract WDA is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "WDA Token";
    string private _symbol = "WDA";
    ...
    uint256 public _maxTxAmount = _tTotal.mul(4).div(100); // should be 4% percent per transaction
    uint256 minTokenNumberToSell = _tTotal.div(10000000); // 0.00001% max tx amount will trigger swap and add liquidity
    ...
    uint256 public _holderDistribution = 50; // 5% will be distributed among holder as token divideneds
    uint256 private _previousTaxFee = _holderDistribution;
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns (uint256) {
        uint256 percentage = tAmount
            .mul(_holderDistribution.add(_liquidityFee).add(_marketFee))
            .div(1e3);
        return percentage;
    }

    function _reflectFee(uint256 tAmount) private {
        uint256 tFee = tAmount.mul(_holderDistribution).div(1e3);
    ...
    function removeAllFee() private {
        _previousTaxFee = _holderDistribution;
        _previousLiquidityFee = _liquidityFee;
        _previousMarketFee = _marketFee;

        _holderDistribution = 0;
        _liquidityFee = 0;
        _marketFee = 0;
    }

    function restoreAllFee() private {
        _holderDistribution = _previousTaxFee;
    ...
    function _tokenTransfer(
        ...
        if(takeFee && (sender != pancakePair && recipient !=pancakePair)){
            removeAllFee();
        }else if (takeFee && (sender == pancakePair && recipient ==pancakePair)){
            restoreAllFee();
        }
