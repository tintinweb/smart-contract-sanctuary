...
contract Kaby is IKaby, ERC721, AcceptedToken, ReentrancyGuard {
    ...
    function offer(uint heroId, uint offerValue) external override nonReentrant {
        ...
        if (needRefund) {
            ...
            bool success = IERC20(kabyToken).transferFrom(ownerOf(heroId), buyer, returnedValue);
            require(success);
        }
    ...
    function cancelOffer(uint heroId) external override nonReentrant {
        ...
        bool success = IERC20(kabyToken).transferFrom(ownerOf(heroId), sender, offerValue);
        require(success);
    ...
    function _makeTransaction(uint heroId, address buyer, address seller, uint price) private {
        ...
        bool transferToSeller = IERC20(kabyToken).transferFrom(buyer, seller, price - marketFee);
        require(transferToSeller);

        bool transferToTreasury = IERC20(kabyToken).transferFrom(seller, owner(), marketFee);
