...
contract UnionWar is Ownable {
    ...
    event Quitted(uint256 indexed unionId, uint256 difficulty, uint256 mineId);
    event RewardsPaid(uint256 indexed unionId, uint256 rewards);
    ...
    function claimReward(
        ...
        uint256 nowTime = block.timestamp;
        _updateRewards(mapMines[mineKey], nowTime);
        _claimReward(mapMines[mineKey], nowTime);
    }
    ...
    function quitOccupy(
        ...
        uint256 mineKey = _genMineKey(difficulty, mineId);
        uint256 nowTime = block.timestamp;
        _quitOccupy(mapMines[mineKey], nowTime);
    }
    ...
    function _quitOccupy(
        ...
        emit Quitted(mine.unionId, mine.difficulty, mine.mineId);
        _removeOccupy(mine.unionId, mine.difficulty, mine.mineId);
        mine.mineStatus = 1;
        mine.unionId = 0;
        mine.occupyTime = 0;
    ...
    function _claimReward(
        ...
        if (trueRewards > 0) {
            ...
            emit RewardsPaid(mine.unionId, trueRewards);
        }
    }
