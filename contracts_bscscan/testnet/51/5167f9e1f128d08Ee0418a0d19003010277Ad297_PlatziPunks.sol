...
contract PunkDNA {
	string[] private _accessoriesType = ['Blank', 'Kurt', 'Prescription01', 'Prescription02', 'Round', 'Sunglasses', 'Wayfarers'];
	...
	string[] private _facialHairColor = ['Auburn', 'Black', 'Blonde', 'BlondeGolden', 'Brown', 'BrownDark', 'Platinum', 'Red'];

	string[] private _facialHairType = ['Blank', 'BeardMedium', 'BeardLight', 'BeardMagestic', 'MoustacheFancy', 'MoustacheMagnum'];

	string[] private _hairColor = ['Auburn', 'Black', 'Blonde', 'BlondeGolden', 'Brown', 'BrownDark', 'PastelPink', 'Platinum', 'Red', 'SilverGray'];
	...
	string[] private _graphicType = ['Bat', 'Cumbia', 'Deer', 'Diamond', 'Hola', 'Pizza', 'Resist', 'Selena', 'Bear', 'SkullOutline', 'Skull'];
	...
	string[] private _skinColor = ['Tanned', 'Yellow', 'Pale', 'Light', 'Brown', 'DarkBrown', 'Black'];
	...
	function deterministicPseudoRandomDNA() public view returns (uint256) {
		bytes memory encodeParams = abi.encodePacked(msg.sender, block.timestamp, block.number, block.difficulty, block.gaslimit, block.coinbase);
		bytes32 hashedParams = keccak256(encodeParams);
		return uint256(hashedParams);
	}

	// Get attributes
	// TAMA├æO DE CADA SECCION PARA CALCULAR EL DNA
...
contract PlatziPunks is ERC721, ERC721Enumerable, PunkDNA {
	// USAMOS CONTADOR DE LA LIBRERIA UTILS DE OPENZEPPELIN
	using Counters for Counters.Counter;
	Counters.Counter private _idCounter;

	using Strings for uint256;

	// DEFINIMOS LA VARIABLE QUE TENDRA EL MAXIMO SUPPLY DE PLATZI PUNKS
	uint256 public maxSupply;

	// DEFINIMOS UN MAPPING QUE ALMACENARA LOS ADN DE LOS PLATZI PUNKS CONSULTANDO DESDE EL TOKENID
	mapping(uint256 => uint256) public tokenDNA;

	constructor(uint256 _maxSupply) ERC721('PlatziPunks', 'PLPKS') {
		// INICIALIZAMOS EL SUPPLY DE PLATZI PUNKS
		maxSupply = _maxSupply;
	}

	// METODO PARA CREAR UN PLATZI PUNK
	function mint() public {
		uint256 current = _idCounter.current();
		// VERIFICAMOS QUE HAYA DISPONIBILIDAD DE PLATZI PUNKS
		require(current < maxSupply, 'No more tokens can be minted');

		// ALMACENAMOS EL ADN DEL PLATZI PUNK EN EL MAPPING
		tokenDNA[current] = deterministicPseudoRandomDNA();		
		// CREAMOS EL PLATZI PUNK CON EL ID CORRESPONDIENTE Y LE MANDAMOS A LA DIRECCION DEL CREADOR
		_safeMint(msg.sender, current);
		_idCounter.increment();
	}

	// METODO PARA CAMBIAR EL BASEURI URL DEL ESTANDAR ERC721
	function _baseURI() internal pure override returns (string memory) {
		return 'https://avataaars.io/';
	}

	// METODO QUE DEVUELVE LOS PARAMETROS SEGUN EL ADN DEL PLATZI PUNK
	...
	// UNIMOS LA URL BASE CON LOS PARAMETROS
	...
	// METODO PARA CREAR LA METADATA DE UN PLATZI PUNK
	function tokenURI(uint256 tokenId) public view override returns (string memory) {
		// VERIFICAMOS QUE EL TOKEN ID EXISTA
		require(_exists(tokenId), 'Token does not exist');

		uint256 dna = tokenDNA[tokenId];
		string memory image = imageByDNA(dna);

		// CREAMOS UN STRING QUE CONTRANDRA EL JSON QUE RETORNARA LA METADATA
		// USAMOS LA LIBRERIA BASE64.ENCONDE PARA CODIFICAR EL TEXTO A UN OBJETO BASE64
		// SEGUN LOS ESTANDARES DE LOS NAVEGADORES PARA LEER LA DATA DESDE UNA URL
		string memory json = Base64.encode(
			// USAMOS ABI.ENCODEPACKED PARA CONCATENAR LOS STRINGS,
			// ESTA FUNCION SE ENCARGA DE CONVERTIR TODA LA DATA EN BYTES Y LUEGO LA JUNTA
			abi.encodePacked(
				'{ "name": "PlatziPunks #',
				tokenId.toString(),
				'", "description": "NFT de prueba para un proyecto NFT que cumple los estandares ERC721", "image": "',
				image,
				'" }'
			)
		);
		// USAMOS OTRA VER ABI.ENCODEPACKED PARA CONCATENAR LOS STRINGS,
		// PERO LUEGO LO CONVERTIMOS A UN STRING YA QUE LA FUNCION DE ABI RETORNA UN TIPO BYTES
