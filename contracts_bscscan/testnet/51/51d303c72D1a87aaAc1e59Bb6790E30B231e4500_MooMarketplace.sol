  ...
  AccessControlUpgradeable
{
  ...
  bytes32 public constant GOVERNANCE_ROLE =
    keccak256("MOOMONSTER_GOVERNANCE_ROLE");
  ...
  event UpdateMarketplaceFee(uint256 marketplaceFee);
    ...
    initializer
  {
    ...
    emit UpdateMarketplaceFee(marketplaceFee);
  }

  modifier onlySupportedToken(address _tokenAddress) {
    require(
      supportedToken[_tokenAddress],
      "MarketPlace::Token address is not supported"
    );
    _;
  }
  ...
  modifier onlyWhitelistedNFT(address _nftAddress) {
    require(
      ...
      "MarketPlace::NFT address is not whitelisted"
    );
    _;
  }
  ...
  function setMarketplaceFee(uint256 _marketplaceFee)
    external
  ...
  {
    require(
      address(_quoteToken) != address(0),
      "MarketPlace:: Invalid quote token"
    );
    require(_price > 0, "MarketPlace:: Price must greater than zero.");
    ...
    _transferNFT(msg.sender, address(this), _nftAddress, _nftId);
  ...
  function cancelSellingOrder(address _nftAddress, uint256 _orderId)
    external
    whenNotPaused
    onlyWhitelistedNFT(_nftAddress)
  {
    ...
    require(
      ...
      "MarketPlace::Only seller is not allowed to cancel this order."
    );
    require(
      order.isActive && !order.isSold,
      "MarketPlace::This Order is not valid"
    );

    orders[_orderId].isActive = false;
    _transferNFT(address(this), order.seller, order.nftAddress, order.nftId);

    emit OrderCancelled(msg.sender, _nftAddress, _orderId);
  }

  function buyNFT(address _nftAddress, uint256 _orderId)
    external
    whenNotPaused
    onlyWhitelistedNFT(_nftAddress)
  {
    require(_exists(_orderId), "MarketPlace::Selling Order is not exist");
    ...
    require(
      sellingOrder.quoteToken.balanceOf(_msgSender()) >= sellingOrder.price,
    ...
    uint256 tradingFee = marketplaceFee.mul(sellingOrder.price).div(10000);
    ...
    emit OrderSold(msg.sender, _nftAddress, _orderId);
  }
  ...
  function _exists(uint256 _orderId) private view returns (bool) {
    return _orderId <= _orderIds.current();
  }
