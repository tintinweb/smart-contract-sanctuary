...
contract Prediction is Owned, Pausable {
    ...
    event NewTicket(address indexed buyer, bool label, uint256 indexed pid, uint256 indexed epoch, uint8 tid);
    ...
    function _calculateRewards(uint256 _pid, uint256 _epoch) internal {
        ...
        emit RewardsCalculated(_pid, _epoch, rewardAmount, lotteryReward, result);
    }
    ...
    function _buyTicket(uint256 _pid, bool _label, address _buyer, bool _initial, address _referral) internal returns(uint256 _price) {
        ...
        emit NewTicket(_buyer, _label, _pid, _currentEpoch, tid);
        return (price);
    }
