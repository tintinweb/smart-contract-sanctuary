...
contract SyrupBar is BEP20('SyrupBar Token', 'SYRUP') {
    ...
    // Safe wag transfer function, just in case if rounding error causes pool to not have enough WAGYUs.
    function safeWagTransfer(address _to, uint256 _amount) public onlyOwner {
...
interface IMigratorChef {
    // Perform LP token migration from legacy WagSwap to WagSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to WagSwap LP tokens.
    // WagSwap must mint EXACTLY the same amount of WagSwap LP tokens or
    // else something bad will happen. Traditional WagSwap does not
    // do that so be careful!
    function migrate(IBEP20 token) external returns (IBEP20);
}

// MasterChef is the master of Wag. He can make Wag and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of WAGYUs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. WAGYUs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that WAGYUs distribution occurs.
        uint256 accWagPerShare; // Accumulated WAGYUs per share, times 1e12. See below.
    }
    ...
    // View function to see pending WAGYUs on frontend.
