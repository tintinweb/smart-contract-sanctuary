...
contract LootBox is Ownable, Pausable, ReentrancyGuard {
  using SafeMath for uint256;
    ...
    uint256 public OVERALL_PROBABILITY = 10000;
    ...
    // uint256 public totalNftType = 102;

    event Open(uint256[] ids);

    mapping(uint256 => uint256) public nftProbabilitiesmapping;
   ...
   */
    ...
    function _pickRandomNFT(
        ...
        if(nftProbabilitiesmapping[value] == 0){
            return nftProbabilitiesmapping[value];
        }else{
            return 1;
        }
    ...
    function setNFTProbability(uint256[] memory _ids, uint256[] memory _newProbabilities) public onlyOwner{
        ...
        for(uint j = 0 ; j < _ids.length; j ++){
            for( i ; i <= _newProbabilities[j]+lastIndex; i++){
                nftProbabilitiesmapping[i] = _ids[j];
            }
        }
        lastIndex = i;
    }
    ...
    function changeLastIndex(uint256 _index) public onlyOwner{
        lastIndex = _index;
    }
