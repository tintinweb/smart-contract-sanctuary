...
// will be transferred to a governance smart contract once Ruffy is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract RuffyFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Ruffy
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. Ruffy to distribute per block.
        uint256 lastRewardBlock;    // Last block number that Ruffy distribution occurs.
        uint256 accTokenPerShare;   // Accumulated Ruffy per share, times 1e12. See below.
    ...
    // Ruffy tokens created per block.
    uint256 public tokenPerBlock;
    // Bonus muliplier for early token makers.
    uint256 public constant BONUS_MULTIPLIER = 1;
    uint256 public constant CLASS_COUNT = 3;
    uint256[] public userLimitCounts = [~uint256(0), 1500, 350, 150, 50];
    uint256[] public tokenLimitAmounts = [10, 100, 1000, 10000, 100000];
    ...
    // The block number when Ruffy mining starts.
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, uint256 _harvestInterval, bool _withUpdate,
        uint256 _withdrawLockPeriod, uint256 _apr) external onlyOwner {
        ...
        require(_apr >= 100 && _apr <= 600, "add: invalid apr");
    ...
    // Update the given pool's Ruffy allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending Ruffy on frontend.
    function pendingToken(uint256 _pid, address _user) public view returns (uint256) {
        ...
        uint256 rewardPercent = block.timestamp.sub(user.lastDepositTime).mul(pool.apr).div(100);
        uint256 pending = user.amount.mul(rewardPercent).div(1e12).div(365).div(24).div(3600).sub(user.rewardDebt);
        
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest Ruffy.
    ...
    // Deposit LP tokens to MasterChef for Ruffy allocation.
    // function deposit(uint256 _pid, uint256 _amount, address _referrer) external nonReentrant {
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        if (user.amount == 0) {
            uint256 index = _pid / CLASS_COUNT;
            uint256 userCounts = poolInfo[index].userCount + poolInfo[index + 1].userCount + poolInfo[index + 2].userCount;
            require(userCounts < userLimitCounts[index], "enough user!");
            require(_amount >= tokenLimitAmounts[index], "Not enough tokens!");
        }
    ...
    function payOrLockupPendingToken(uint256 _pid) internal {
        ...
        uint256 rewardPercent = block.timestamp.sub(user.lastDepositTime).mul(pool.apr).div(10000);
        uint256 pending = user.amount.mul(rewardPercent).div(1e12).div(365).div(24).div(3600).sub(user.rewardDebt);
    ...
    // Safe token transfer function, just in case if rounding error causes pool to not have enough Ruffy.
