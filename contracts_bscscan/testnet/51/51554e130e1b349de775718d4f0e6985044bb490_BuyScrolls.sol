...
// File: contracts/IBEP721Enumerable.sol
...
// File: contracts/IBEP721Receiver.sol
    ...
    IBEP721Enumerable
{
    ...
    // which can be also obtained as `IBEP721Receiver(0).onERC721Received.selector`
    ...
    /**
     * @dev Internal function to invoke {IBEP721Receiver-onERC721Received} on a target address.
...
contract RandomNumberConsumer is VRFConsumerBase {
    ...
    uint256 public i;
    uint256 scrollNum;
   mapping(address => mapping(uint256 =>uint256) ) public usersRandomNumb;
     ...
     * Requests randomness 
     */
    function getRandomNumber(uint256 scrollNumb) public returns (bytes32 requestId) {
        require(LINK.balanceOf(address(this)) > fee, "Not enough LINK - fill contract with faucet");
        scrollNum = scrollNumb;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        if(i< scrollNum)
        {
             usersRandomNumb[msg.sender][i] = randomResult;
        }
...
contract ScrollNFT is BEP721, Ownable {
        ...
        returns (uint256)
    {
        ...
        numberInfo[newItemId] = _randomNumber;
        return newItemId;
    }
...
contract BuyScrolls is Ownable, RandomNumberConsumer {
    using SafeMath for uint256;
    ...
    mapping(address => mapping(uint256 =>uint256) )public randomNumberMappingAddress;
    ...
    function buy(
        ...
        for (uint256 i = 0; i < _noofScrolls; i++) {         
               
                 id = getRandomNumber(_noofScrolls);
                 randomNumberMappingAddress[msg.sender][i] = randomResult;
                 
                 
                 _randomNumbers[i] = randomResult;
                 usersRandomNum[msg.sender].push(randomResult);
    ...
    function stake(
        ...
        if (stakes.dfcAmount > 0) {
            ...
            (reward,dailyPercent) =  _availableRewards_test();
            stakes.rewardAmount = 
            stakes.rewardAmount.add(reward );
        }
    ...
    function harvest() public returns (uint256 rewards) {
        StakeInfo storage stakes = stakings[msg.sender];
        uint256 dailyPercent;
         (rewards,dailyPercent) = _availableRewards_test();
    ...
    function bonusCalculation() public view returns (uint256) {
        StakeInfo storage stakes = stakings[msg.sender];
    ...
    function _bonusCalculation_test() public view returns (uint256) {
        StakeInfo storage stakes = stakings[msg.sender];
    ...
    function availableRewards() public view returns (uint256) {
        ...
        for (uint256 i = months; i <= months; i--) {
            ...
            tempPercent= tempPercent.add(percent);
        }
        return rewards;
    }

    function _availableRewards_test() internal view returns (uint256, uint256) {
        StakeInfo storage stakes = stakings[msg.sender];
        uint256 interval = now.sub(stakes.lastUpdatedTime);
        ...
        for (uint256 i = months; i <= months; i--) {
            ...
            rewardTemp = rewardTemp.div(365); // for a day
            rewards = rewards + interval.mul(rewardTemp);
            dailyPercent.add(percent);
        }
        return (stakes.rewardAmount.add(rewards), dailyPercent);
    }
