  ...
  // mapping for token creator
    ...
    */
    ...
    /**
        ...
        After the above conditions are met, this function MUST check if `_to` is a smart contract (e.g. code size > 0). If so, it MUST call `onBEP721Received` on `_to` and act appropriately (see "Safe Transfer Rules" section of the standard).
        @param from    Source address
        @param to      Target address
        @param tokenId      ID of the token type.
        @param _data    Additional data with no specified format, MUST be sent unaltered in call to `onBEP7215Received` on `_to`
    */
     ...
     * Deprecated, use {BEP721-_burn} instead.
...
contract Nebula721v1 is BEP721 {
    ...
    constructor (string memory name, string memory symbol, string memory tokenURIPrefix) BEP721 (name, symbol){
        tokenCounter = 1;
        owner = msg.sender;
