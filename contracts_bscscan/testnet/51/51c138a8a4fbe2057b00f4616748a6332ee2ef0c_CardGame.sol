...
using SafeMath for uint256;
  ...
  event test(string rarity, uint256 count);
  // constructor(){
 ...
 function init1() public {
  ...
  function buyBox(address seller, string memory ownerId, string memory name) public {
    ...
    emit test(returnedValue,randomValue);
    }
  ...
  function getUriDetail(string memory name, string memory rarity2, uint256 randomValue) public view returns(string memory,uint256 countBack)
  {
  return (Category[name].rarityBasedUris[rarity2].attributes[randomValue].tokenUris, Category[name].rarityBasedUris[rarity2].count);
  }
