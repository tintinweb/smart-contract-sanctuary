...
contract NewAMasterChef is InitializeOwnable, DEFIToken, Initializable{
        ...
        )
    {
        ...
        uint256 supply = totalSupply + pool.available;
        allocAmount = poolAvalible();
        todayAmount = (allocAmount > 0) ? (allocAmount - supply) : 0;
        alreadyAmount = supply;
        ...
        )
    {
        ...
        expectedClaim = (poolAvalible() > 0) ? (poolAvalible() - supply) : 0;  //Distribute
    ...
    function updatePool() public {
        ...
        if (pool.lastDayAvailable + 1 days < block.timestamp && treasuryUSD > supply + (lastAvalible - supply) && mSupply > 0) {
    ...
    function rebase() public {
        require(lastDayRebase  + 1 days < block.timestamp);
