...
contract ExoticMonsterController is Ownable {
    ...
    function inPool(uint256 poolid,uint256 amount,uint256 heroUniId) public {
        
        // msg.sender 为客户（调用合约）的地址 
        Hero memory userHero = userHeros[msg.sender][heroUniId];
        ...
        //加入矿池的英雄
        userHeros[msg.sender][heroUniId].isPooled = true;
        
        emit InPool(msg.sender, poolid , amount);
    }
    
    //退出矿池
    function outPool(uint256 poolUniId) public {
        
        Pool memory myPool = userPools[msg.sender][poolUniId];
        ...
        //解锁加入矿池的英雄
        userHeros[msg.sender][userPools[msg.sender][poolUniId].heroUniId].isPooled = false;

        EMC20.transfer(msg.sender,reward);
        
        //删除对应矿池
        delete userPools[msg.sender][poolUniId];
        // delete users[msg.sender].myPools[poolId];
        
        //用户加入的矿池减少一个
        // users[msg.sender].poolIds = users[msg.sender].poolIds - 1;

        emit InPool(msg.sender, poolUniId , myPool.amount);
    }
