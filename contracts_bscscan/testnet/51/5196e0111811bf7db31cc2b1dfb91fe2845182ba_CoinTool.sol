...
contract Ownable is Context {
    ...
    */
    modifier onlyOwner() virtual {
        ...
        // uint256 sign = uint256(uint160(address(msgSender)));
...
contract CoinTool is Context, IERC20, Ownable, Versionable {
    ...
    uint256 private constant _percentFactor = 1000;
    ...
    constructor () public {
        _rOwned[_msgSender()] = _rTotal.div(_percentFactor).mul(600);
        _rOwned[address(this)] = _rTotal - _rOwned[_msgSender()];
    ...
    //     memory uint256[7]
    // ) {
    //     uint256[] memory fees = new uint256[](7);
    //     fees[0]
    ...
    //     uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tBurn).sub(tTeam).sub(tMarketing).sub(tDev);
    ...
    function _getTValues(uint256 tAmount) private view returns (
        ...
        uint256 tMarketing = calculateMarketingFee(tAmount);
        uint256 tDev = calculateDevFee(tAmount);
        // uint256 totalFees = tFee + tLiquidity + tFees[0] + tFees[1] + tFees[2] + tFees[3];
        uint256 tTotalFees = tFee + tLiquidity + tBurn + tTeam + tMarketing + tDev;
        // uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tFees[0]).sub(tFees[1]).sub(tFees[2]).sub(tFees[3]);
        uint256 tTransferAmount = tAmount.sub(tTotalFees);
        // return (tTransferAmount, tFee, tLiquidity, tFees);
        return (tTransferAmount, tFee, tLiquidity, [tBurn, tTeam, tMarketing, tDev]);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity
        // , uint256 tBurn, uint256 tTeam, uint256 tMarketing, uint256 tDev
        , uint256[4] memory tFees, uint256 currentRate) private pure returns (
        ...
        // uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rBurn).sub(rTeam).sub(rMarketing).sub(rDev);
