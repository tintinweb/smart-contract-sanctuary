...
interface ILimitrMarketETHToken {
    ...
    /// @param deadline Validity deadline
    function cancelETHOrder(uint256 orderID, uint256 amount, uint256 deadline) external;
    ...
    /// @return cost The amount of counter token spent
    /// @return received The amount of base token received
    function tradeForETHAtAveragePrice(
...
/// @author Limitr
/// @title Trade market contract for Limitr
contract LimitrMarketETHToken is LimitrMarketTokenToken,  ILimitrMarketETHToken {
    ...
    /// @dev Contract constructor
    constructor(address _factory, address _baseToken, address _counterToken)
        LimitrMarketTokenToken(_factory, _baseToken, _counterToken)
    {
        require(
            ILimitrFactory(_factory).weth() == _baseToken,
            'base token is not WETH'
        );
    }
    ...
    /// @param deadline Validity deadline
    /// @return The order ID
    function newETHOrder(uint256 price, address trader, uint256 deadline)
        external payable override
        returns (uint256)
    {
        (uint256 orderID, bool created) = _newETHOrderWithPointer(
            price,
            trader,
            deadline,
            0
    ...
    function _newETHOrderWithPointer(
        uint256 price,
        address trader,
        uint256 deadline,
        uint256 pointer
        ...
        returns (uint256, bool)
    {
        ...
        if (created) {
            _wrapDepositBaseToken(msg.value);
            emit NewOrder(orderID, trader, price, msg.value);
        }
        return (orderID, created);
    }


    function _wrapDepositBaseToken(uint256 amount)
        internal
        postBalanceCheck(baseToken, _expectedBaseBalance + amount)
    {
        _expectedBaseBalance += amount;
        IWETH(baseToken).deposit{value: amount}();
    }
    ...
    /// @param amount The amount to cancel. 0 cancels the total amount
    /// @param deadline Validity deadline
    function cancelETHOrder(uint256 orderID, uint256 amount, uint256 deadline)
        external override
        withinDeadline(deadline)
        ownerOrApproved(orderID)
        lock
    {
        ...
        uint256 _amount = amount != 0 ? amount : _order.amount;
        _cancelOrder(orderID, amount);
        _unwrapWithdrawBaseToken(_order.trader, _amount);
    }

    function _unwrapWithdrawBaseToken(address to, uint256 amount)
        internal
        postBalanceCheck(baseToken, _expectedBaseBalance - amount)
    {
        _expectedBaseBalance -= amount;
        IWETH(baseToken).withdraw(amount);
        payable(to).transfer(amount);
    }
    ...
    function tradeForETHAtMaxPrice(
        ...
        lock
        withinDeadline(deadline)
        returns (uint256 cost, uint256 received)
    {
        Trade memory trade = Trade(0, 0, afterFee(maxCounterIn));
        while (trade.availableCounter > 0) {
            if (!_tradeFirstOrderMaxPriceForETH(maxPrice, trade, receiver)) {
                break;
            }
        }
        require(trade.counter > 0 && trade.base > 0, 'No trade');
    ...
    /// @dev trade the first order at a max price
    function _tradeFirstOrderMaxPriceForETH(
        uint256 maxPrice,
        Trade memory trade,
        address receiver
    ) internal returns (bool) {
        ...
        uint256 maxAmount = returnOf(trade.availableCounter, _order.price);
        // remaining amount of counter token
        return _tradeOrderForETH(orderID, _order, trade, maxAmount, receiver);
    }


    function _tradeOrderForETH(
        uint256 orderID,
        Order memory _order,
        Trade memory trade,
        uint256 maxAmount,
        ...
        uint256 newAmount = _order.amount - amt;
        // transfer the base token to the receiver
        _unwrapWithdrawBaseToken(receiver, amt);
        // transfer counter token to the order trader
        _tokenTransferFrom(counterToken, msg.sender, _order.trader, cost);
        // remove order or update amount
        if (newAmount == 0) { _removeOrder(orderID); }
        ...
        returns (uint256 cost, uint256 received)
    {
        Trade memory trade = Trade(0, 0, afterFee(maxCounterIn));
        while (trade.availableCounter > 0) {
            if (!_tradeFirstOrderAveragePriceForETH(maxAveragePrice, trade, receiver)) {
                break;
            }
        }
        require(trade.counter > 0 && trade.base > 0, 'No trade');
    ...
    /// @dev trade the first order at an average price
    function _tradeFirstOrderAveragePriceForETH(
        uint256 maxAveragePrice,
        Trade memory trade,
        address receiver
    ) internal returns (bool) {
...
contract LimitrDeployerETHToken is ILimitrDeployer {
    function createMarket(
        address factory,
        address baseToken,
        address counterToken
        ...
        returns (address)
    {
        return address(new LimitrMarketETHToken(factory, baseToken, counterToken));
    }
}