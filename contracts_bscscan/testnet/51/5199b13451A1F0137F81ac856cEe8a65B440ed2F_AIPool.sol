...
contract AIPool {
    ...
    event  TransferAI(address indexed src, address indexed dst, uint256 wad, uint percent);
    ...
    function withdraw() public returns (bool) {
        ...
        //if(last_AI_draw[msg.sender] + 1 days < block.timestamp)
        if(last_AI_draw[msg.sender] + 1800 < block.timestamp)
    		{
          ...
          tokenInterface(token_Address).transfer(msg.sender, amount);
          emit TransferAI(address(this),  msg.sender, amount, setpoolperc);
        }
        return true;
    }
