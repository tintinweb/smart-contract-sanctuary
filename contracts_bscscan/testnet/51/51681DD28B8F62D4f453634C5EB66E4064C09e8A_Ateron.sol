...
contract Ateron is IERC20, Ownable {
    ...
    constructor() public {
        ...
        _owned[_lockedWalletForFuturePartner] = futurePartnerSupply;
        emit Transfer(address(0), _lockedWalletForFuturePartner, futurePartnerSupply);
        _isLocked[_lockedWalletForFuturePartner] = true;
        
        uint256 advisorSupply = 15000000 * 10**uint256(_decimals);
        _owned[_lockedWalletForAdvisors] = advisorSupply;
        emit Transfer(address(0), _lockedWalletForAdvisors, advisorSupply);
        _isLocked[_lockedWalletForAdvisors] = true;
        
        uint256 marketingSupply = 20000000 * 10**uint256(_decimals);
        _owned[_walletForMarketing] = marketingSupply.div(2);
        emit Transfer(address(0), _walletForMarketing, marketingSupply.div(2));
        _owned[_lockedWalletForMarketing] = marketingSupply.div(2);
        emit Transfer(address(0), _lockedWalletForMarketing, marketingSupply.div(2));
        _isLocked[_lockedWalletForMarketing] = true;
        
        uint256 playToEarnSupply = 40000000 * 10**uint256(_decimals);
        _owned[_lockedWalletForPlayToEarn] = playToEarnSupply;
        emit Transfer(address(0), _lockedWalletForPlayToEarn, playToEarnSupply);
        _isLocked[_lockedWalletForPlayToEarn] = true;
        
        uint256 teamSupply = 60000000 * 10**uint256(_decimals);
        _owned[_lockedWalletForTeam] = teamSupply;
        emit Transfer(address(0), _lockedWalletForTeam, teamSupply);
        _isLocked[_lockedWalletForTeam] = true;
        
        uint256 dexSupply = 3000000 * 10**uint256(_decimals);
        _owned[_walletForDex] = dexSupply;
        emit Transfer(address(0), _walletForDex, dexSupply);
    }
