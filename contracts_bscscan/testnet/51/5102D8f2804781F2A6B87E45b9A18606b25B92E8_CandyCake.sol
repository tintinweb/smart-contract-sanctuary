...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 CAKE = IERC20(0x9780881Bf45B83Ee028c4c1De7e0C168dF8e9eEF);
...
contract CandyCake is IERC20, Ownable {
    using SafeMath for uint256;

    address CAKE = 0x9780881Bf45B83Ee028c4c1De7e0C168dF8e9eEF;
    address WBNB = 0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "CandyCake";
    string constant _symbol = "CandyCAKE";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1000000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = 1 * (7 ** _decimals);
    uint256 public _maxWalletSize = 2 * (7 ** _decimals);
    ...
    function setTxLimit(uint256 amount) external onlyOwner {
        _maxTxAmount = amount * 1 ** _decimals; //percent
    }
    
    function setMaxWallet(uint256 size) external onlyOwner() {
        _maxWalletSize = size * 1 ** _decimals; //percent
    }
