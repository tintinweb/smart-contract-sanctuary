...
contract BSCFlashloan {
  ...
  function startArbitrage(
    // token0 = WBNB, amount0 = amountBorrow
    // token1 = any with liquidity, amount1 = 0 
    address token0, 
    address token1, 
    uint amount0, 
    uint amount1
  ...
  function transfer() public {
    // transfer all BNB in contract to my wallet
    owner.transfer(contractBalance);
    contractBalance = address(this).balance;
  }
  
  function unwrap(uint amountWBNB) public {
    // token0 = WBNB -> need to unwrap to BNB to transfer out of contract
    // not sure if this function works as intended yet 
    address WBNBContractAddress = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
