 ...
 * checks.
 *
 * Arithmetic s in Solidity wrap on overflow. This can easily result
...
contract IFOByProxy is ReentrancyGuard, Initializable {
    ...
    uint256 public raisingAmount; //10000
    // total amount of offeringToken that will offer
    uint256 public offeringAmount; // 10000000 lucky
    // total amount of raising tokens that have already raised
    uint256 public totalAmount;
    // maximum token community user can deposit
    uint256 public maxTokenForCommunityUser = 100000000000000000000; // 100 LP
    ...
    function addUserToWhiteList(address user, uint256 maxToken)
        public
        onlyAdmin
    {
        if (whitelist[user] == 0) whitelist[user] = maxToken;
    }

    function addUsersToWhitelist(
        address[] memory users,
        uint256[] memory maxToken
    ) public onlyAdmin {
        for (uint256 i = 0; i < users.length; i++) {
            if (whitelist[users[i]] == 0) whitelist[users[i]] = maxToken[i];
        }
    }

    function updateUserInWhitelist(address user, uint256 maxToken)
        public
        onlyAdmin
    {
        whitelist[user] = maxToken;
    }

    function depositWithoutWhitelist(uint256 _amount) public {
        require(
        ...
        require(
            userInfo[msg.sender].amount.add(_amount) <=
                maxTokenForCommunityUser,
            "exceeds Amount for community user"
        );
    ...
    function depositWithWhitelist(uint256 _amount) public {
        require(
            block.number > startBlock && block.number < endBlock,
            "not ifo time"
        );
        ...
        require(
            userInfo[msg.sender].amount.add(_amount) <= whitelist[msg.sender],
            "exceeds amount for whitelist user"
        );
        lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        if (userInfo[msg.sender].amount == 0) {
            addressList.push(address(msg.sender));
    ...
    // function getOfferingAmount(address _user) public view returns(uint256) {
    //  return userInfo[_user].amount.mul(priceToken);
    // }
