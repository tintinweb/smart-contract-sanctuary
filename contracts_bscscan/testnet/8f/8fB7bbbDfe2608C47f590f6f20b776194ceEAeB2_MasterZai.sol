...
contract MasterZai is Ownable, ReentrancyGuard {
    ...
    function getCirculatingSupply() external view returns(uint256) {
        uint256 tSupply = 100000000 * 10 ** 18;
    ...
    //this function make sure even thousands of pool gas fee is still low because each transfer is just 1 time
    function massUpdatePools() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            //Total - 15% fees
            uint256 zaifRewardFees = zaifReward.mul(1500).div(10000);
            uint256 zaifRewardUsers = zaifReward.sub(zaifRewardFees);
            ...
            if(totalminted >= 170000000000000000000){
                ...
                totalReward.add(zaifReward);
            }    

        }
        if(totalReward > 0){

            uint256 totalRewardFees = totalReward.mul(1500).div(10000);
            
            zaif.mint(address(this),totalReward.sub(totalRewardFees));

            //1% dev fee
            zaif.mint(devaddr,totalReward.mul(blockDevFee).div(10000));

            //4% salary fee
            zaif.mint(salaryAddress,totalReward.mul(blockSalaryFee).div(10000));
            
            //10% marketing fee
            zaif.mint(marktAddress,totalReward.div(10));
        }
         
    }
 
    // Update reward variables of the given pool to be up-to-date.
    function updatePool(uint256 _pid) public validatePool(_pid) returns (bool) {
        PoolInfo storage pool = poolInfo[_pid];
        if (block.number <= pool.lastRewardBlock) {
            return false;
        }
