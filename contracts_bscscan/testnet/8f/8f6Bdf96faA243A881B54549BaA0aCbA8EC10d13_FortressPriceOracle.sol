...
/**
  * @title Fortress's InterestRateModel Interface
  * @author Fortress
  */
...
// File: contracts/FTokenInterfaces.sol
...
contract FTokenStorage {
    ...
    /**
     * @notice Contract which oversees inter-fToken operations
     */
    ...
    /**
     * @notice Initial exchange rate used when minting the first FTokens (used when totalSupply = 0)
     */
...
contract FTokenInterface is FTokenStorage {
    /**
     * @notice Indicator that this is a FToken contract (for inspection)
     */
    ...
    /**
     * @notice Event emitted when tokens are redeemed and fee transferred
     */
     ...
     */
    event LiquidateBorrow(address liquidator, address borrower, uint repayAmount, address fTokenCollateral, uint seizeTokens);
     ...
     */
    event TransferReserves(address admin, uint reduceAmount, uint newTotalReserves);
...
contract FBep20Storage {
    /**
     * @notice Underlying asset for this FToken
     */
    address public underlying;
}

contract FBep20Interface is FBep20Storage {
    ...
    function liquidateBorrow(address borrower, uint repayAmount, FTokenInterface fTokenCollateral) external returns (uint);
...
contract FDelegatorInterface is FDelegationStorage {
    /**
...
contract FDelegateInterface is FDelegationStorage {
    /**
...
contract ComptrollerErrorReporter {
    ...
    enum FailureInfo {
        ...
        SET_FAI_MINT_RATE_CHECK,
        SET_FAICONTROLLER_OWNER_CHECK,
        SET_MINTED_FAI_REJECTION,
        FAI_CONTROLLER_MISMATCH,
...
contract TokenErrorReporter {
    ...
    enum FailureInfo {
        ...
        REPAY_FAI_COMPTROLLER_REJECTION,
        REPAY_FAI_FRESHNESS_CHECK,
        FAI_MINT_EXCHANGE_CALCULATION_FAILED,
        FAI_MINT_NEW_ACCOUNT_BALANCE_CALCULATION_FAILED,
...
contract FAIControllerErrorReporter {
    enum Error {
        NO_ERROR,
        UNAUTHORIZED,
        REJECTION,
...
/**
 * @title Fortress's FToken Contract
 * @notice Abstract base for FTokens
 * @author Fortress
 */
contract FToken is FTokenInterface, Exponential, TokenErrorReporter {
    /**
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        (mathErr, srfTokensNew) = subUInt(accountTokens[src], tokens);
        if (mathErr != MathError.NO_ERROR) {
        ...
        accountTokens[src] = srfTokensNew;
     ...
     */
    function getAccountSnapshot(address account) external view returns (uint, uint, uint, uint) {
        uint fTokenBalance = accountTokens[account];
        uint borrowBalance;
        ...
        return (uint(Error.NO_ERROR), fTokenBalance, borrowBalance, exchangeRateMantissa);
    }
    ...
    /**
     * @notice Returns the current per-block borrow interest rate for this fToken
    ...
    /**
     * @notice Returns the current per-block supply interest rate for this fToken
    ...
    /**
     * @notice User supplies assets into the market and receives fTokens in exchange
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
         ...
         *  Note: The fToken must handle variations between BEP-20 and BNB underlying.
         *  `doTransferIn` reverts if anything goes wrong, since we can't be sure if
         *  side-effects occurred. The function returns the amount actually transferred,
         *  in case of a fee. On success, the fToken holds an additional `actualMintAmount`
         *  of cash.
         */
        vars.actualMintAmount = doTransferIn(minter, mintAmount);

        /*
         * We get the current exchange rate and calculate the number of fTokens to be minted:
        ...
        /*
         * We calculate the new total supply of fTokens and minter token balance, checking for overflow:
     ...
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param redeemAmount The amount of underlying to receive from redeeming fTokens
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    ...
    /**
     * @notice User redeems fTokens in exchange for the underlying asset
     * @dev Assumes interest has already been accrued up to the current block
     * @param redeemer The address of the account which is redeeming the tokens
     * @param redeemTokensIn The number of fTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be non-zero)
     * @param redeemAmountIn The number of underlying tokens to receive from redeeming fTokens (only one of redeemTokensIn or redeemAmountIn may be non-zero)
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
         ...
         *  Note: The fToken must handle variations between BEP-20 and BNB underlying.
         *  On success, the fToken has redeemAmount less of cash.
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
         ...
         *  Note: The fToken must handle variations between BEP-20 and BNB underlying.
         *  On success, the fToken borrowAmount less of cash.
         *  doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
         */
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
         ...
         *  Note: The fToken must handle variations between BEP-20 and BNB underlying.
         *  On success, the fToken holds an additional repayAmount of cash.
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, FTokenInterface fTokenCollateral) internal nonReentrant returns (uint, uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
        ...
        error = fTokenCollateral.accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted liquidation failed
        ...
        return liquidateBorrowFresh(msg.sender, borrower, repayAmount, fTokenCollateral);
    }
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, FTokenInterface fTokenCollateral) internal returns (uint, uint) {
        /* Fail if liquidate not allowed */
        uint allowed = comptroller.liquidateBorrowAllowed(address(this), address(fTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
        ...
        /* Verify fTokenCollateral market's block number equals current block number */
        if (fTokenCollateral.accrualBlockNumber() != getBlockNumber()) {
        ...
        (uint amountSeizeError, uint seizeTokens) = comptroller.liquidateCalculateSeizeTokens(address(this), address(fTokenCollateral), actualRepayAmount);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");

        /* Revert if borrower collateral token balance < seizeTokens */
        require(fTokenCollateral.balanceOf(borrower) >= seizeTokens, "LIQUIDATE_SEIZE_TOO_MUCH");
        ...
        if (address(fTokenCollateral) == address(this)) {
            ...
            seizeError = fTokenCollateral.seize(liquidator, borrower, seizeTokens);
        }
        ...
        emit LiquidateBorrow(liquidator, borrower, actualRepayAmount, address(fTokenCollateral), seizeTokens);

        /* We call the defense hook */
        comptroller.liquidateBorrowVerify(address(this), address(fTokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
     ...
     * @dev Will fail unless called by another fToken during the process of liquidation.
     *  Its absolutely critical to use msg.sender as the borrowed fToken and not a parameter.
     * @param liquidator The account receiving seized collateral
     * @param borrower The account having collateral seized
     * @param seizeTokens The number of fTokens to seize
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    ...
    /**
     * @notice Transfers collateral tokens (this market) to the liquidator.
     * @dev Called only during an in-kind liquidation, or by liquidateBorrow during the liquidation of another FToken.
     *  Its absolutely critical to use msg.sender as the seizer fToken and not a parameter.
     * @param seizerToken The contract seizing the collateral (i.e. borrowed fToken)
     * @param liquidator The account receiving seized collateral
     * @param borrower The account having collateral seized
     * @param seizeTokens The number of fTokens to seize
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
    ...
    function _addReservesFresh(uint addAmount) internal returns (uint, uint) {
         ...
         *  Note: The fToken must handle variations between BEP-20 and BNB underlying.
         *  On success, the fToken holds an additional addAmount of cash.
         *  doTransferIn reverts if anything goes wrong, since we can't be sure if side effects occurred.
         *  it returns the amount actually transferred, in case of a fee.
         */
    ...
    /**
     * @notice Accrues interest and reduces reserves by transferring to treasury
     * @param reduceAmount Amount of reduction to reserves
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    ...
    function _transferReservesFresh(uint reduceAmount) internal returns (uint) {
        ...
        // Check caller is treasuryGuardian
        if (msg.sender != IComptroller(address(comptroller)).treasuryGuardian()) {
        ...
        // doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
        doTransferOut(IComptroller(address(comptroller)).treasuryAddress(), reduceAmount);

        emit TransferReserves(IComptroller(address(comptroller)).treasuryAddress(), reduceAmount, totalReservesNew);
...
// File: contracts/FBep20.sol

pragma solidity ^0.5.16;


/**
 * @title Fortress's FBep20 Contract
 * @notice FTokens which wrap an EIP-20 underlying
 * @author Fortress
 */
contract FBep20 is FToken, FBep20Interface {
    /**
    ...
    function initialize(address underlying_,
                        ...
                        string memory symbol_,
                        uint8 decimals_) public {
        // FToken initialize does the bulk of the work
     ...
     */
    function liquidateBorrow(address borrower, uint repayAmount, FTokenInterface fTokenCollateral) external returns (uint) {
        (uint err,) = liquidateBorrowInternal(borrower, repayAmount, fTokenCollateral);
        return err;
    }
...
// File: contracts/Oracle/Chainlink/AggregatorV2V3Interface.sol
...
// File: contracts/Oracle/Umbrella/IChain.sol
...
// File: contracts/Oracle/Umbrella/IRegistry.sol
...
// File: contracts/Oracle/IPancakePair.sol
...
// File: contracts/Oracle/FortressPriceOracle.sol
...
contract FortressPriceOracle is PriceOracle, Exponential {
    ...
    address public constant FTS_ADDRESS = 0x00d1d87E5A64DA9F87464aCB9f15aEE980bf7eA5;

    address public constant FBNB_ADDRESS = 0x4dcb6271b6Cb208943958d3f5D12Ec3dC4CeCD46;
