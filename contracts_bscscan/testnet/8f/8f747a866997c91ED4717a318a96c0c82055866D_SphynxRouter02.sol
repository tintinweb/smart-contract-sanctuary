...
contract SphynxRouter02 is ISphynxRouter02 {
    ...
    function updateSwapFee(uint256 _swapFee) external {
        require(msg.sender == swapfeeSetter, "not-swap-fee-setter");
        swapFee = _swapFee;
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        _swap(amounts, path, to);
        TransferHelper.safeTransferETH(ISphynxFactory(factory).feeTo(), msg.value * swapFee / 1000); // 0.1% swap fee default
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        TransferHelper.safeTransferETH(ISphynxFactory(factory).feeTo(), amounts[amounts.length - 1] * swapFee / 1000); // 0.1% swap fee default
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        assert(IWETH(WETH).transfer(SphynxLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        TransferHelper.safeTransferETH(ISphynxFactory(factory).feeTo(), amounts[0] * swapFee / 1000); // 0.1% swap fee
        // refund dust eth, if any
        ...
        ensure(deadline)
    {
        ...
        TransferHelper.safeTransferETH(ISphynxFactory(factory).feeTo(), amountOut * swapFee / 1000); // 0.1% swap fee default
    }
...
library SphynxLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'6e14ff1efd3aeb61323f2aab7158d8fe9588dc6ec08484f137458602ddbef267'   // INIT_CODE_PAIR_HASH of Sphynx Factory
            ))));
    }
