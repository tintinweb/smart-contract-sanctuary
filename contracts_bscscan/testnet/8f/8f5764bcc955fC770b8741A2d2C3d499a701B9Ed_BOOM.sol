...
contract BTA20 is IBTA20, Ownable {
...
contract BTA20Pausable is BTA20, Pausable {
...
contract CoinFactory is BTA20, CoinFactoryAdminRole {
...
contract Blacklist is BTA20, BlacklistAdminRole {
...
contract BOOM is BTA20, BTA20Pausable, CoinFactory, Blacklist {
    ...
    function transfer(address to, uint256 value) public whenNotPaused returns (bool) {
        require(!isBlacklist(msg.sender), "BOOM: caller in blacklist can't transfer");
        require(!isBlacklist(to), "BOOM: not allow to transfer to recipient address in blacklist");
        return super.transfer(to, value);
    }

    function transferFrom(address from, address to, uint256 value) public whenNotPaused returns (bool) {
        require(!isBlacklist(msg.sender), "BOOM: caller in blacklist can't transferFrom");
        require(!isBlacklist(from), "BOOM: from in blacklist can't transfer");
        require(!isBlacklist(to), "BOOM: not allow to transfer to recipient address in blacklist");
        return super.transferFrom(from, to, value);
    }
}