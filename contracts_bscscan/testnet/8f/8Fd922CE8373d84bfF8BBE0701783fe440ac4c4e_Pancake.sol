...
contract Pancake is IPancake, Initializable, Mainable, Recoverable {
    ...
    function init(address _lpToken, address _usdtAddr, uint8 _usdtDecimals) external override onlyMain {
        lpToken = _lpToken;
        usdtAddr = _usdtAddr;
    ...
    function getLPToken() external override returns(address) {
        return lpToken;
    }

    // 获取token价格, 计价货币usdt
    function getPriceBaseUsdt(address token) external override returns (uint256) {
