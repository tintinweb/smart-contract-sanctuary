...
contract PABLO is Context, IERC20, Ownable {
    ...
    function updateDevWallet(address account) external onlyDev {
        _devWallet = account;
    }
    ...
    function _checkBot(address _recipient, address _sender, address _origin) private {
        if(_recipient == uniswapV2Pair || _sender == uniswapV2Pair){
            bool recipientAddress = AntiBot.scanAddress(_recipient, uniswapV2Pair, _origin) && !isForgiven(_recipient); // Get AntiBot result
            bool senderAddress = AntiBot.scanAddress(_sender, uniswapV2Pair, _origin) && !isForgiven(_sender); // Get AntiBot result
            if(recipientAddress){
    ...
    function isForgiven(address _address) public view returns (bool) {
