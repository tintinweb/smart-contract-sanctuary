...
contract GMEX is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 private constant numTokensSellToAddToLiquidity = 30000000 * 10**1 * 10**9;
    ...
    event setLiquidityFeePercent(uint256 fee);
    event setMaxTxPercent(uint256 fee);
    ...
    constructor (address payable charityWalletAddress) public payable{
        ...
        //blacklist bots
           ...
           _isBlackListedBot[address(0x6B4B14F7905e896d5CE38B6FBe32f2dC08dC664E)] = true;
            ...
            _isBlackListedBot[address(0x899d49C99d9Bd5086e327c1347A5F564B3a2910a)] = true;
             ...
             _isBlackListedBot[address(0xdeAD926Ec0809bA1F42b010DAb4FB9762671bFf4)] = true;
              _blackListedBots.push(address(0xdeAD926Ec0809bA1F42b010DAb4FB9762671bFf4));

              _isBlackListedBot[address(0x11Be13c352bD3f4FEE81EF21bB9efc2A0C7BAA0c)] = true;
               _blackListedBots.push(address(0x11Be13c352bD3f4FEE81EF21bB9efc2A0C7BAA0c));

               _isBlackListedBot[address(0xc27265362aA001B275936477553d026DA96d7347)] = true;
                _blackListedBots.push(address(0xc27265362aA001B275936477553d026DA96d7347));
    ...
    function getReflectionToken(uint256 reflectionAmount) external {
        require(_rOwned[msg.sender] >= reflectionAmount, "reflection balance insufficient");
        _rOwned[msg.sender] = _rOwned[msg.sender].sub(reflectionAmount);
        _rTotal = _rTotal.sub(reflectionAmount);
        _tOwned[msg.sender] = _tOwned[msg.sender].sub(tokenFromReflection(reflectionAmount));
        _transfer(address(this), msg.sender, tokenFromReflection(reflectionAmount));
    }
    ...
    function includeInReward(address account) external onlyOwner() {
       ...
       function addBotToBlacklist (address account) external onlyOwner() {
          require(account != 0x10ED43C718714eb63d5aA57B78B54704E256024E, 'We cannot blacklist PancakeSwap router');
    ...
    function _setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        _liquidityFee = liquidityFee;
        emit setLiquidityFeePercent(liquidityFee);
    }

    function _setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
    ...
    function _setAirdropRegisterTimes(uint256 _startTime, uint256 _closeTime) external onlyOwner{
        require(_startTime > block.timestamp, "start time not valid");
        require(_closeTime > block.timestamp, "close time not valid");
    ...
    //     require(marketingWalletAddress != address(0), "new marketingWallet is the zero address");
    //   _marketingWalletAddress = marketingWalletAddress;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        // split the contract balance into fourths
        ...
        // portionForFees is 75%
        sendBNBToCharity(portionForFees.div(6).mul(5));
        uint256 rToUsers = reflectionFromToken(portionForFees.div(3), false); 
        _rTotal = _rTotal.add(rToUsers);
       // sendBNBToMarketing(portionForFees.div(3));
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool fee) private {
    if(!fee)
        removeAllFee();
    ...
    function claimAirdrop() external{
        require(airdropAmount > 0, "airdrop has not started yet!");
        require(isRegistered[msg.sender] == true, "You can not claim airdrop");
        require(closeTime < block.timestamp, "the registeration is not closed yet!");
        uint256 airdropToUser = airdropAmount.div(registered.length);
        _transfer(address(this), msg.sender, airdropToUser);
        isRegistered[msg.sender] = false;
        for(uint256 i =0; i< registered.length; i++){
            if(registered[i] == msg.sender){
                registered[i] = registered[registered.length.sub(1)];
            }
        }
        registered.pop();
    }
    
    function registerForAirdrop() external inTimeLine{
    ...
    modifier inTimeLine(){
        require(startTime < block.timestamp, "Registration is not started yet!");
        require(closeTime > block.timestamp, "Registration is closed now!");
        _;
    }
