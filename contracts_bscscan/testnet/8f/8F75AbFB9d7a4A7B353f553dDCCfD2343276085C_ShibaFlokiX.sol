...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  constructor(){
      ...
      rewardTokens.push(address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7)); // SHIB- 0
      rewardTokens.push(address(0x8BaBbB98678facC7342735486C851ABD7A0d17Ca)); // FLOKI - 1
...
contract ShibaFlokiX is ERC20, Ownable {
    ...
    constructor() ERC20("test", "TEST"){
        ...
        uint256 _devFee= 2;
        uint256 _rewardsFeeSell = 5;
        uint256 _devFeeSell = 2;
        uint256 _buybackFeeSell = 1;
        ...
        devFee = _devFee;
        totalFees = rewardsFee + marketingFee + liquidityFee + buybackFee + devFee;
        ...
        totalFeesSell = rewardsFeeSell.add(marketingFeeSell).add(devFeeSell).add(buybackFeeSell).add(liquidityFeeSell);

    	dividendTracker = new DividendTracker();
    	
    	marketingWallet = address(owner()); // update to Marketing Wallet if set to Owner... can always be changed later too.
        liquidityWallet = address(0xbb35c9AED2F660f82ECeA56C3eBc698b9AE4a054); // liquidity will go to dead address at first, can update later if needed by owner.
    ...
    function addRewardToken(address _token) public onlyOwner {
        dividendTracker.addRewardToken(_token);
    }
    ...
    function updateFeesBuy(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _devFee, uint256 _feeDivisor) external onlyOwner {
    ...
    function updateFeesSell(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _feeDivisor, uint256 _devFee) external onlyOwner {
        require(_feeDivisor == 100 || _feeDivisor == 1000, "Fee divisor must be 100 or 1000");
        if(_feeDivisor == 100){
            require(_marketingFee <= 8, "Marketing may not be set higher than 8%");
            require(_rewardsFee <= 12, "Rewards fee may not be set higher than 12%");
        ...
        feeDivisorSell = _feeDivisor;
        totalFeesSell = marketingFeeSell + rewardsFeeSell + liquidityFeeSell + buybackFeeSell + devFeeSell;
        if(_feeDivisor == 100){
            require(totalFeesSell <= 20, "Must keep fees at 20% or less");
        } else if(_feeDivisor == 1000) {
            require(totalFeesSell <= 200, "Must keep fees at 20% or less");
        }
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
        	uint256 fees = amount.mul(totalFeesSell).div(feeDivisorSell);
    ...
    function swapBack(uint256 contractTokenBalance, bool sell) internal {
        uint total;
        uint liquidity;
        uint marketing;
        uint dev;
        ...
        if (sell) {
            total = totalFeesSell;
            liquidity = liquidityFeeSell;
            marketing = marketingFeeSell;
            dev = devFeeSell;
            reward = rewardsFeeSell;
        } else {
            total = totalFees;
        ...
        uint256 amountToLiquify = contractTokenBalance.mul(liquidity).div(total).div(2);
        ...
        uint256 totalBNBFee = total.sub(liquidity.div(2));
        
        uint256 amountBNBLiquidity = amountBNB.mul(liquidity).div(totalBNBFee).div(2);
        uint256 amountBNBReflection = amountBNB.mul(reward).div(totalBNBFee);
        uint256 amountBNBMarketing = amountBNB.mul(marketing).div(totalBNBFee);
        uint256 amountBNBDev = amountBNB.mul(dev).div(totalBNBFee);
