...
contract OlyRaceTEST is ERC20, Ownable, ERC20Burnable {
    ...
    string private constant _name = "OlyRace TEST";
    string private constant _symbol = "OLRT";
    ...
    constructor() ERC20("OlyRaceTEST", "OLRT") {
        ...
        if (pancakeLiquidPair != address(0)) {
            if (_sender == pancakeLiquidPair) {
                ...
                if (_amount > maxSell) revert("Sell amount limited ");
                    ...
                    traders[_sender]["SELL"].lastTrade + buyCooldown > curTime
                ) {
                    revert("You are cooldown to next trade!");
                } else {
                    traders[_sender]["SELL"] = TraderInfo({
