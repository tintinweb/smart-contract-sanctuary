...
import "./tokens/BEP20/IBEP20.sol";
import "./libraries/math/SafeMath.sol";
import "./libraries/Address.sol";
import "./tokens/BEP20/utils/SafeBEP20.sol";
import "./utils/Context.sol";
import "./access/Ownable.sol";
import "./security/ReentrancyGuard.sol";
import "./tokens/BEP20/utils/Reserve.sol";
...
contract MultiWithdrawStaking is Ownable {
    ...
    constructor() public {
        ...
        WithdrawFeeInterval[] memory _withdrawFee = new WithdrawFeeInterval[](
            5
        );
        ...
        add(
            108e6,
            IBEP20(0xa1c0bc13962176950b66ae3DCcC10B8afBb4f013),
            IBEP20(0xa1c0bc13962176950b66ae3DCcC10B8afBb4f013),
            0,
            0,
            0,
    ...
    function getWithdrawFeeIntervals(uint256 poolId)
        external
        view
        returns (WithdrawFeeInterval[] memory)
    {
    ...
    function add(
        ...
        require(
            ...
            "WSWAP_STAKING: Invalid Staked token address"
        );
        require(
            address(_rewardToken) != address(0),
            "WSWAP_STAKING: Invalid Reward token address"
        );
        require(poolInfo.length <= 1000, "WSWAP_STAKING: Pool Length Full!");
        require(
            !poolExists[address(_stakedToken)][address(_rewardToken)],
            "WSWAP_STAKING: Pool Already Exists!"
        );
        require(
            _depositFeeBP <= 10000,
            "WSWAP_STAKING: invalid deposit fee basis points"
        );
        require(
            _harvestInterval <= MAXIMUM_HARVEST_INTERVAL,
            "WSWAP_STAKING: invalid harvest interval"
        );
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            accTokenPerShare = accTokenPerShare.add(
                tokenReward.mul(1e12).div(pool.stakedAmount)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 reward = tokenReward > rewardTokenSupply
            ? rewardTokenSupply
            : tokenReward;
        if (reward > 0) {
            pool.rewardSupply -= reward;
            pool.accTokenPerShare = pool.accTokenPerShare.add(
                reward.mul(1e12).div(tokenBalance)
            );
        }
        pool.lastRewardBlock = block.number;
    }
    ...
    function depositRewardToken(uint256 poolId, uint256 amount) external {
        ...
        uint256 initialBalance = _poolInfo.rewardToken.balanceOf(
            address(rewardReserve)
        );
        _poolInfo.rewardToken.safeTransferFrom(
            msg.sender,
            address(rewardReserve),
            amount
        );
        uint256 finalBalance = _poolInfo.rewardToken.balanceOf(
            address(rewardReserve)
        );
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        if (_amount > 0) {
            require(
                _amount >= poolInfo[_pid].minDeposit,
                "WSWAP_STAKING: Not Enough Required Staking Tokens!"
            );
            ...
            if (pool.depositFeeBP > 0) {
                ...
                pool.stakedAmount = pool.stakedAmount.add(delta).sub(
                    depositFee
                );
            } else {
    ...
    function payOrLockupPendingToken(uint256 _pid) internal {
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                rewardReserve.safeTransfer(
                    pool.rewardToken,
                    msg.sender,
                    totalRewards
                );
                rewardDistributions[_pid] = rewardDistributions[_pid].add(
                    totalRewards
                );
            }
        } else if (pending > 0) {
    ...
    function emergencyAdminWithdraw(uint256 _pid) external onlyOwner {
        ...
        require(
            balanceToWithdraw != 0,
            "WSWAP_STAKING: Not enough balance to withdraw!"
        );
