...
contract TTEESSTT is Context, IERC20, Ownable {
    ...
    // address constant BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // BUSD mainnet
    address constant BUSD = address(0x9a01bf917477dD9F5D715D188618fc8B7350cd22); // BUSD testnet
    ...
    function _transfer(
        ...
        if (!_noFee[sender] && !_noFee[recipient] && !swapping) {
            totalFee = amount.mul(_marketingMilipercent + _buyBackMilipercent + _dividendMilipercent).div(100000);
        }
    ...
    function setBuybackFreezeDeadline(uint256 sec) external onlyOwner returns (bool) {
        if (_buybackFreezeDeadline < block.timestamp + sec) {
    ...
    function setMarketingMilipercent(uint256 value) external onlyOwner returns (bool) {
        require(_marketingMilipercent > value, 'Fee must not increase');
    ...
    function setBuybackMilipercent(uint256 value) external onlyOwner returns (bool) {
        require(_buyBackMilipercent > value, 'Fee must not increase');
    ...
    function setDividendMilipercent(uint256 value) external onlyOwner returns (bool) {
        require(_dividendMilipercent > value, 'Fee must not increase');
