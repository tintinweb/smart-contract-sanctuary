    ...
    AdventureInfo
{
    ...
    function _calculatePendingIncompleteRewards(
        ...
        xp = xp - uint64(_calculateRewardPunishment(xp, team_.startTime, team_.endTimeXp,_timestamp));
        ...
        crystal = crystal - _calculateRewardPunishment(crystal, team_.startTime, team_.endTimeCrystal,_timestamp);
    ...
    function _calculateRewardPunishment(uint256 amount, uint256 startTime, uint256 endTime, uint256 currentTime) internal pure returns (uint256) {
        if (currentTime>=endTime) {
            return 0;
        }

        return (amount * 50 / 100) * (endTime - currentTime) / (endTime - startTime);
    }
