...
library Utils {
    ...
    function isLotteryWon(uint256 salty, uint256 winningDoubleRewardPercentage) internal view returns (bool) {
    ...
    function calculateBananaReward(
        uint256 currentBalance,
        uint256 currentBananaPool,
        uint256 winningDoubleRewardPercentage,
        uint256 totalSupply
    ) internal view returns (uint256) {
        uint256 bananaPool = currentBananaPool;
        ...
        uint256 reward = bananaPool.mul(multiplier).mul(currentBalance).div(100).div(totalSupply);
        return reward;
    }
    ...
    function swapTokensForBanana(
        address routerAddress,
        uint256 tokenAmount,
        address bananaAddress
    ) internal {
        IApeRouter02 apeRouter = IApeRouter02(routerAddress);
        // generate the ape pair path of token -> token(banana)
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(bananaAddress);
        // make the swap
...
contract BIGFOOT is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _name = "BIGFOOT";
    string private _symbol = "FOOT";
    uint8 private _decimals = 9;

    IApeRouter02 public immutable apeRouter;
    address public immutable apePair;
    address public bananaAddress;

    bool inSwapAndLiquify = false;

    event SwapAndLiquifyEnabledUpdated(bool enabled);
    event SwapAndLiquify(uint256 tokensSwapped, uint256 bananaReceived, uint256 tokensIntoLiqudity);
    event ClaimBananaSuccessfully(address recipient, uint256 bananaReceived, uint256 nextAvailableClaimDate);

    // Innovation for protocol by Team
    ...
    uint256 public _liquidityFee = 8; // 4% will be added pool, 4% will be converted to Banana
    ...
    constructor (address payable routerAddress, address _bananaAddress) {
        _rOwned[_msgSender()] = _rTotal;
        IApeRouter02 _apeRouter = IApeRouter02(routerAddress);
        bananaAddress = _bananaAddress;
        // Create a ape pair for this new token
        apePair = IApeFactory(_apeRouter.factory()).createPair(address(this), address(bananaAddress));
    ...
    function calculateBananaReward(address ofAddress) public view returns (uint256) {
        uint256 _totalSupply = uint256(_tTotal)
        .sub(balanceOf(address(0)))
        .sub(balanceOf(0x000000000000000000000000000000000000dEaD)) // exclude burned wallet
        .sub(balanceOf(address(apePair)));
        // exclude liquidity wallet
        return Utils.calculateBananaReward(
            balanceOf(address(ofAddress)),
            IBEP20(bananaAddress).balanceOf(address(this)),
    ...
    function checkClaimBananaDate(address _account) public view returns(uint256) {
        return nextAvailableClaimDate[_account];
    }

    function claimBananaReward() isHuman nonReentrant public {
        ...
        uint256 reward = calculateBananaReward(msg.sender);

        // reward threshold
        if (reward >= rewardThreshold) {
            Utils.swapBananaForTokens(
                address(apeRouter),
        ...
        emit ClaimBananaSuccessfully(msg.sender, reward, nextAvailableClaimDate[msg.sender]);

        bool _sent = IBEP20(bananaAddress).transfer(msg.sender, reward);
        // (bool sent,) = address(msg.sender).call{value : reward}("");
        require(_sent, 'Error: Cannot withdraw reward');
    }
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        // also, don't swap & liquify if sender is ape pair.
        ...
        if (
            ...
            !(from == address(this) && to == address(apePair)) // swap 1 time
        ) {
            ...
            uint256 pooledBanana = contractTokenBalance.div(2);
            uint256 piece = contractTokenBalance.sub(pooledBanana).div(2);
            uint256 otherPiece = contractTokenBalance.sub(piece);
            uint256 tokenAmountToBeSwapped = pooledBanana.add(piece);
            uint256 initialBalance = IBEP20(bananaAddress).balanceOf(address(this));

            // now is to lock into staking pool
            Utils.swapTokensForBanana(address(apeRouter), tokenAmountToBeSwapped, bananaAddress);

            // how much Banana did we just swap into?

            // capture the contract's current Banana balance.
            // this is so that we can capture exactly the amount of Banana that the
            // swap creates, and not make the liquidity event include any Banana that
            // has been manually sent to the contract
            uint256 deltaBalance = IBEP20(bananaAddress).balanceOf(address(this)).sub(initialBalance);

            uint256 bananaToBeAddedToLiquidity = deltaBalance.div(3);

            // add liquidity to ape
            Utils.addLiquidity(address(apeRouter), owner(), otherPiece, bananaAddress, bananaToBeAddedToLiquidity);
    ...
    function activateContract() public onlyOwner {
        ...
        // approve contract
        _approve(address(this), address(apeRouter), 2 ** 256 - 1);
    }
}