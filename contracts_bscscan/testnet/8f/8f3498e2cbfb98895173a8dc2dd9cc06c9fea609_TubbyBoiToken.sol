...
contract TubbyBoi is Context, IERC20, Ownable, VRFConsumerBase, Winners {
    ...
    function getPaginatedPastWinners(uint256 offset, uint256 limit) public view returns (address[] memory, uint256[] memory, uint256) {
        require(limit <= 15, "Cannot fetch more than 15 winners per request.");
        Winnings[] memory response;
        address[] memory accounts;
        uint256[] memory amountWon;
        if (offset > _winners.length) return (accounts, amountWon, _winners.length);
        response = new Winnings[]((offset + limit) > _winners.length ? _winners.length - offset : limit);

        for (uint256 i = 0; i < response.length; i++) {
            Winnings memory winnings = _winners[offset + i];
            accounts[i] = winnings.account;
            amountWon[i] = winnings.amountWon;
        }

        return (accounts, amountWon, _winners.length);
    }
    ...
    function setUniswapPair(address _uniswapV2RouterAddress, address _uniswapV2Pair) external onlyOwner() {
    ...
    // Setter in case liquidity starts to dry up, or sucks up too much TUBBY
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH->TUBBY swap when swap+liquify is triggered
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        if (
            ...
            Winnings storage newWinnings;
            newWinnings.account = randomRecipient;
            newWinnings.amountWon = _currentReflection;
            _winners.push(newWinnings);
    ...
    function _tokenTransfer(address from, address to, uint256 amount) private {
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            */
            ...
            if (block.timestamp >= lotteryLastRun + 1 minutes) {
                lockThePayout = false;
            }
