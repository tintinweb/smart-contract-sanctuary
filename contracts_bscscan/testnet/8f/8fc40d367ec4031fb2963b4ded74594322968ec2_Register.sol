...
contract Register is Ownable {
    ...
    constructor () public {
          ...
          spiritFarmContract = SpiritFarm(0x8BDE2197b2DB741Ea4866Bdd5d61d5de9B7C8f4F);
    ...
    function claim(uint256 achievementId) public returns (bool) {
        ...
        // Transfer
        dtuTokenContract.safeTransfer(msg.sender, claimAmount);// Transfer dtu to register address
    ...
    function setSwapSwitchState(bool _switchState) public onlyOwner returns (bool) {
        ...
        if(startTime==0&&switchState){
            ...
            endTime = startTime.add(dayTime.mul(9999));// 28 days in total
        }
