...
contract SmartCake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SmartCakeDividendTracker public dividendTracker;
    address public marketingWallet = 0x74747568dC0b52ec6BDA510a3eD624490FA8a89A;
    address public devWallet = 0xCc5B755A1d019d31cd5A614d84ceB9f13C19691d;
    ...
    uint256 public sellDividendFee = 10;
    ...
    event UpdateWallets(address oldMarketing, address newMarketing, address oldDevFee, address newDevFee);
    ...
    event SetBuyFees(uint256 LiquidityFee, uint256 MarketingFee, uint256 DividendFee);

    event SetSellFees(uint256 SellLiquidityFee, uint256 SellDevFee, uint256 SellMarketingFee, uint256 SellDividendFee);
    ...
    event CalculatedBNBForEachRecipient(uint256 forMarketing, uint256 forDev, uint256 forDividends);
    ...
    constructor() ERC20("SmartCake", "SCAKE") {
        dividendTracker = new SmartCakeDividendTracker();

        // updatePancakeRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        updatePancakeRouter(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); // testnet
        ...
        */
        _mint(owner(), 10**14 * (10**decimals()));
        maxSellAmount = totalSupply().div(200);
        maxHoldingAmount = totalSupply().div(50);
    }
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "SmartCake: The router already has that address");
    ...
    function setBuyFees(uint256 _LiquidityFee, uint256 _MarketingFee, uint256 _DividendFee)  external onlyOwner {
        marketingFee = _MarketingFee;
        liquidityFee = _LiquidityFee;
        DividendFee = _DividendFee;
        uint256 totalFees = liquidityFee + marketingFee + DividendFee;
        require(totalFees < 51, "Too much");
        emit SetBuyFees(_LiquidityFee, _MarketingFee, _DividendFee);
    }

    function setSellFees(uint256 _SellLiquidityFee, uint256 _SellDevFee, uint256 _SellMarketingFee, uint256 _SellDividendFee)  external onlyOwner {
        sellDevFee = _SellDevFee;
        sellLiquidityFee = _SellLiquidityFee;
        sellMarketingFee = _SellMarketingFee;
        sellDividendFee = _SellDividendFee;
        uint256 totalFees = sellLiquidityFee + sellDevFee + sellMarketingFee + sellDividendFee;
        require(totalFees < 51, "Too much");
        emit SetSellFees(_SellLiquidityFee, _SellDevFee, _SellMarketingFee, _SellDividendFee);
    }

    function setWallets(address newMarketingWallet, address newDevWallet) external onlyOwner {
        require(newDevWallet != address(0), 'Buyback Zero address');
        require(newMarketingWallet != address(0), 'Marketing Zero address');
        emit UpdateWallets(marketingWallet, newMarketingWallet, devWallet, newDevWallet);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SmartCake: The dividend tracker already has that address");

        SmartCakeDividendTracker newDividendTracker = SmartCakeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SmartCake: The new dividend tracker must be owned by the SmartCake token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SmartCake: Account is already the value of 'excluded'");
    ...
    function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPeriodLimit[account] != excluded, "SmartCake: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "SmartCake: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SmartCake: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SmartCake: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "SmartCake: This account cannot send tokens until trading is enabled");
        }
        ...
        !_isExcludedFromMaxTxLimit[to] ) {
            require(amount <= maxSellAmount, "Transfer amount exceeds the maxTransactionAmount.");
        }
        ...
        if(takeFee) {
            ...
            if (to == pancakePair) {
                forDev = amount.mul(sellDevFee).div(100);
                forLiquidity = amount.mul(liquidityFee).div(100);
            ...
            uint256 fees = forDev.add(forMarketing).add(forDividends).add(forLiquidity);
    ...
    function swapAndDistributeBNB(uint256 tokens) private {
        ...
        uint256 accTotal = accDevFee.add(accDividendFee).add(accMarketingFee);
        uint256 forMarketing = balance.mul(accMarketingFee).div(accTotal);
        uint256 forDev = balance.mul(accDevFee).div(accTotal);
        uint256 forDividends = balance.sub(forDev).sub(forMarketing);

        emit CalculatedBNBForEachRecipient(forMarketing, forDev, forDividends);
        ...
        (success,) = address(devWallet).call{value: forDev}("");

        if(success) {
            emit SwapAndSendTo(accDevFee, forDev, "ANTI DUMP");
            accDevFee = 1;
        }
...
contract SmartCakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SmartCakeDividendTracker", "Honey_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SmartCakeDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SmartCakeDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main Honey contract.");
    }
