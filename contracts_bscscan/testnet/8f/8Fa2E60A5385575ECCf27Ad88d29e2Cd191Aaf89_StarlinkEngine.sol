...
contract StarlinkEngine is IStarlinkEngine, AccessControlled, EmergencyWithdrawable {
    IXLD public xld;
    IStarlink public starlink;
    ITreasury public treasury;
    IInvestor public investor;

    uint256 minFundsBeforeProcessing = 100000000000000 wei;

    event GasBought(uint256 amount);
    event Donated(address from, uint256 amount);

    constructor(IXLD _xld) {
        setXLD(_xld);
    }
    ...
    function processFunds(uint256 gas) internal {
        uint256 gasLeft = gasleft();
        if (gas > 0 && address(starlink).balance >= minFundsBeforeProcessing) {
            starlink.processFunds(gas);
        }
        
        if (address(treasury).balance >= minFundsBeforeProcessing) {
            treasury.processFunds();
        }
        

        if (address(investor).balance >= minFundsBeforeProcessing) {
            investor.allocateFunds();
        }

        uint256 consumedGas = gasLeft - gasleft();
        if (consumedGas < gas) {
            xld.processRewardClaimQueue(gas - consumedGas);
        }
    }

    function setXLD(IXLD _xld) public onlyOwner {
        require(address(_xld) != address(0), "StarlinkEngine: Invalid address");
        xld = _xld;
    }

    function setStarlink(IStarlink _starlink) public onlyOwner {
        require(address(_starlink) != address(0), "StarlinkEngine: Invalid address");
        starlink = _starlink;
    }

    function setTreasury(ITreasury _treasury) public onlyOwner {
        require(address(_treasury) != address(0), "StarlinkEngine: Invalid address");
        treasury = _treasury;
    }

    function setInvestor(IInvestor _investor) public onlyOwner {
        require(address(_investor) != address(0), "StarlinkEngine: Invalid address");
        investor = _investor;
    }
