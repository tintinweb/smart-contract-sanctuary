...
library Utils {
    ...
    function calculateBNBReward(
        ...
        uint256 reward = bnbPool.mul(currentBalance).div(100).div(totalSupply);
...
contract DemocratInu is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _name = "Democrat Inu";
    string private _symbol = "DEMINU";
    uint8 private _decimals = 9;

    IPancakeRouter02 public pancakeRouter;
    address public immutable pancakePair;
    address payable public lottoWallet;
    ...
    uint256 public _maxHolding = 500 ether;
    uint256 public _maxHoldinginitial = 250 ether;
    uint256 public minTokenNumberToSell = _tTotal.div(100000); // 0.001% max tx amount will trigger swap and add liquidity
    mapping(address => uint256) public nextAvailableClaimDate;
    bool public swapAndLiquifyEnabled = false; // should be true to turn on the bnb pool
    bool public reflectionFeesdiabled = false; // should be false to take fee
    bool inSwapAndLiquify = false;

    uint256 public launchTime ;

    uint256 public _taxFee = 120; // 12% will be distributed among holder as DEMINU divideneds
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _poolFee = 40; // 4% will be added to pool for BNB reward
    uint256 private _previousPoolFee = _poolFee;
    
    uint256 public _lottoFee = 5;  // 2% will go to the market address 
    uint256 private _previousLottoFee = _lottoFee;
    
    uint256 public _devFee = 10;  // 1% will go to the dev address 
    uint256 private _previousDevFee = _devFee;
    
    uint256 public _burnFee = 20;  // 2% will go to the Dead address to deflate DEMINU
    ...
    constructor (
        ...
        address payable _lottoWallet,
        address payable _devWallet
    ) Ownable(_ownerAddress) {
        _rOwned[owner()] = _rTotal;
        lottoWallet = _lottoWallet;
    ...
    function setmaxHoldinginitial(uint256 maxHoldinginitial) public onlyOwner {
        _maxHoldinginitial = _tTotal.mul(maxHoldinginitial).div(10000);
    }
    function setmaxHolding(uint256 maxHolding) public onlyOwner {
        _maxHolding = _tTotal.mul(maxHolding).div(10000);
    }
    ...
    function setRewardCycle(uint256 _newcycle) public onlyOwner {
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_taxFee.add(_poolFee).add(_lottoFee).add(_devFee).add(_burnFee)).div(1e3);
        return percentage;
    }
    ...
    function _takeBnbPoolFee(uint256 tAmount, uint256 currentRate) internal {
    ...
    function _takeLottoFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tLottoFee = tAmount.mul(_lottoFee).div(1e3);
        uint256 rLottoFee = tLottoFee.mul(currentRate);
        _rOwned[lottoWallet] = _rOwned[lottoWallet].add(rLottoFee);
        if (_isExcluded[burnAddress])
            _tOwned[lottoWallet] = _tOwned[lottoWallet].add(tLottoFee);
        emit Transfer(_msgSender(), lottoWallet, tLottoFee);
    }
    ...
    function _transfer(
        ...
        if(_isExcludedFromMaxTx[from] == false && 
            ...
            require(balanceOf(to) + amount < _maxHoldinginitial ,"cant Buy more than 250 Billion Before 30 minutes");
        }else{
            require(balanceOf(to) + amount <= _maxHolding ,"cant Hold more than 500 Billion");
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeBnbPoolFee(tAmount, currentRate);
        _takeLottoFee(tAmount, currentRate);
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        require(balanceOf(msg.sender) >= 0, 'Error: must own DEMINU to claim reward');
