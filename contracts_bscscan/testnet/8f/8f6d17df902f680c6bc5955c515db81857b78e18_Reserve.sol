...
abstract contract ERC20NoUpg is IERC20Metadata, ContextNoUpgrade {
    ...
    */
    ...
    function _screenForFantip(address _fan, address _farm, bool active) internal virtual {
        ...
        (uint regFarm,) = _getBalances(_farm);
        if(active) {
            if(_hodl > 0) {
...
contract Reserve is OwnableNoUpgrade {
    using SafeMath for uint256;
    QfourNoUpg private asset;
    ...
    function lockBalance(string memory pwd, address randAddress, uint nonce, uint256 amount) public onlyOwner {
        bool executed = asset.safeSignedTransfer(owner(), amount, pwd, randAddress, nonce);
        require(executed, "Failed");    
    }

    //Moves the specific balance to locked
    function lockSpecific(uint256 amount) public onlyOwner {
        bool executed = asset.lockBalance(amount);
        require(executed, "Failed");    
    }

    //Moves the aggregate balance to locked
    function unLockBalance(string memory pwd, address randAddress, uint nonce, uint256 amount) public onlyOwner {
        bool executed = asset.unlockBalance(amount, pwd, randAddress, nonce);
        require(executed, "Failed");    
    }
    ...
    function setAddr(QfourNoUpg _asset) public onlyOwner {
        asset = _asset;
    }
}