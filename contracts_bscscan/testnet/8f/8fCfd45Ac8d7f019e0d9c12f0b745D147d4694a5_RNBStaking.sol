...
contract RNBStaking {
    ...
    function modifyLastDepotis(uint256 newLastDeposit) public {
        if(msg.sender==owner){
    ...
    function readStake(string memory txid) public view{
        uint256 stake=0;
        if(msg.sender==stakeTx[txid]){
            stake+=stakedFunds[txid];
        }
    }

    function readTSUnlock(string memory txid) public view {
        uint256 ts=0;
        if(msg.sender==stakeTx[txid]){
            ts+=timestampForUnlock[txid];
        }
    }
