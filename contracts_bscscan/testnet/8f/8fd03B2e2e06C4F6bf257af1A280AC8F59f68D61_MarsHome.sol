...
//        uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function _transfer(
        ...
        if (canSwap && !swapping  && SwapAndLiquidityEnabled) {
            ...
            uint256 marketingAndDevTokens = contractTokenBalance.sub(countLiquidityFees);
            swapTokensForEth(marketingAndDevTokens, payable(address(this)));

            uint256 marketingAndDevFees = sellDevelopmentFee.add(sellMarketingFee);

            uint256 beforeSwap = address(this).balance;
            uint256 marketingFeesBNB = beforeSwap.mul(sellMarketingFee).div(marketingAndDevFees);
            uint256 devFeesBNB=beforeSwap.sub(marketingFeesBNB);

            (bool successMarketing, ) = address(marketingWallet).call{value: marketingFeesBNB}("");

            (bool success, ) = address(developmentWallet).call{value: devFeesBNB}("");
