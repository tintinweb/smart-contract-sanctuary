...
import {VaultAdapterWithIndirection} from "./adapters/VaultAdapterWithIndirection.sol";
import {VaultWithIndirection} from "./libraries/evryloans/VaultWithIndirection.sol";
...
/// @title EvryMix Transmuter

contract EvryMix is Context {
    ...
    address public evToken;
    address public token;

    mapping(address => uint256) public depositedEvTokens;
    ...
    uint256 public totalSupplyEvtokens;
    uint256 public buffer;
    uint256 public lastDepositBlock;

    ///@dev values needed to calculate the distribution of base asset in proportion for evTokens staked
    uint256 public pointMultiplier = 10e18;

    uint256 public totalDividendPoints;
    uint256 public unclaimedDividends;

    /// @dev evryloans addresses whitelisted
    ...
    mapping(VaultAdapterWithIndirection => bool) public adapters;
    ...
    event ActiveVaultUpdated(
        VaultAdapterWithIndirection indexed adapter
    );
    ...
    constructor(address _evToken, address _token, address _governance) {
        require(_governance != ZERO_ADDRESS, "EvryMix: 0 gov");
    ...
    ///@return displays the user's share of the pooled evTokens.
    function dividendsOwing(address account) public view returns (uint256) {
        uint256 newDividendPoints = totalDividendPoints.sub(lastDividendPoints[account]);
        return depositedEvTokens[account].mul(newDividendPoints).div(pointMultiplier);
    }
    ...
    modifier onlyWhitelisted() {
        require(whiteList[msg.sender], "EvryMix: !whitelisted");
        _;
    }

    /// @dev A modifier which checks if caller is a keepr.
    modifier onlyKeeper() {
        require(keepers[msg.sender], "EvryMix: !keeper");
        _;
    }
    ...
    ///
    modifier onlyGov() {
        require(msg.sender == governance, "EvryMix: !governance");
        _;
    }
    ...
    ///@dev Withdraws staked evTokens from the transmuter
    ///
    /// This function reverts if you try to draw more tokens than you deposited
    ///
    ///@param amount the amount of evTokens to unstake
        ...
        updateAccount(msg.sender)
    {
        ...
        address sender = msg.sender;
        require(depositedEvTokens[sender] >= amount,"EvryMix: unstake amount exceeds deposited amount");
        depositedEvTokens[sender] = depositedEvTokens[sender].sub(amount);
        totalSupplyEvtokens = totalSupplyEvtokens.sub(amount);
        IERC20Burnable(evToken).safeTransfer(sender, amount);
        emit EvUsdUnstaked(sender, amount);
    }
    ///@dev Deposits evTokens into the transmuter 
    ///
    ///@param amount the amount of evTokens to stake
        ...
        checkIfNewUser()
    {
        ...
        // requires approval of EvToken first
        address sender = msg.sender;
        //require tokens transferred in;
        IERC20Burnable(evToken).safeTransferFrom(sender, address(this), amount);
        totalSupplyEvtokens = totalSupplyEvtokens.add(amount);
        depositedEvTokens[sender] = depositedEvTokens[sender].add(amount);
        emit EvUsdStaked(sender, amount);
    }
    /// @dev Converts the staked evTokens to the base tokens in amount of the sum of pendingdivs and tokensInBucket
    ///
    /// once the evToken has been converted, it is burned, and the base token becomes realisedTokens which can be recieved using claim()    
    ///
    ...
    function transmute() 
        ...
        if (pendingz > depositedEvTokens[sender]) {
            diff = pendingz.sub(depositedEvTokens[sender]);

            // remove overflow
            pendingz = depositedEvTokens[sender];
        }

        // decrease evtokens
        depositedEvTokens[sender] = depositedEvTokens[sender].sub(pendingz);

        // BURN EVTOKENS
        IERC20Burnable(evToken).burn(pendingz);

        // adjust total
        totalSupplyEvtokens = totalSupplyEvtokens.sub(pendingz);
    ...
    /// @dev Executes transmute() on another account that has had more base tokens allocated to it than evTokens staked.
    ///
        ...
        checkIfNewUser()
    {
        ...
        // check restrictions
        require(
            pendingz > depositedEvTokens[toTransmute],
            "EvryMix: !overflow"
        );
        ...
        uint256 diff = pendingz.sub(depositedEvTokens[toTransmute]);

        // remove overflow
        pendingz = depositedEvTokens[toTransmute];

        // decrease evtokens
        depositedEvTokens[toTransmute] = 0;
    ...
    /// @dev Transmutes and unstakes all evTokens
    ///
    /// This function combines the transmute and unstake functions for ease of use
    function exit() public noContractAllowed() {
        transmute();
        uint256 toWithdraw = depositedEvTokens[msg.sender];
        unstake(toWithdraw);
    }

    /// @dev Transmutes and claims all converted base tokens.
    ///
    /// This function combines the transmute and claim functions while leaving your remaining evTokens staked.
    function transmuteAndClaim() public noContractAllowed() {
        transmute();
        claim();
    }

    /// @dev Transmutes, claims base tokens, and withdraws evTokens.
    ///
    /// This function helps users to exit the transmuter contract completely after converting their evTokens to the base pair.
    ...
    /// @dev Distributes the base token proportionally to all evToken stakers.
    ///
    /// This function is meant to be called by the Evryloans contract for when it is sending yield to the transmuter. 
    ...
    /// @dev Allocates the incoming yield proportionally to all evToken stakers.
    ///
    ...
    function increaseAllocations(uint256 amount) internal {
        if(totalSupplyEvtokens > 0 && amount > 0) {
            totalDividendPoints = totalDividendPoints.add(
                amount.mul(pointMultiplier).div(totalSupplyEvtokens)
            );
    ...
    {
        uint256 _depositedAl = depositedEvTokens[user];
        ...
        uint256 _pendingdivs = _toDistribute.mul(depositedEvTokens[user]).div(totalSupplyEvtokens);
        ...
        returns (address[] memory theUserList, uint256[] memory theUserData)
    {
        ...
        for (uint256 x = 0; x < delta; x += 1) {
            ...
            _theUserData[y] = depositedEvTokens[userList[i]];
            _theUserData[y + 1] = dividendsOwing(userList[i]).add(tokensInBucket[userList[i]]).add(_toDistribute.mul(depositedEvTokens[userList[i]]).div(totalSupplyEvtokens));
            y += 2;
            i += 1;
        }
    ...
    function setPendingGovernance(address _pendingGovernance) external onlyGov() {
        require(_pendingGovernance != ZERO_ADDRESS, "EvryMix: 0 gov");
    ...
    function initialize(VaultAdapterWithIndirection _adapter) external onlyGov {
        require(!initialized, "EvryMix: already initialized");
        require(rewards != ZERO_ADDRESS, "EvryMix: cannot initialize rewards address to 0x0");
    ...
    function migrate(VaultAdapterWithIndirection _adapter) external onlyGov() {
        _updateActiveVault(_adapter);
    }
    ...
    function _updateActiveVault(VaultAdapterWithIndirection _adapter) internal {
        require(_adapter != VaultAdapterWithIndirection(ZERO_ADDRESS), "EvryMix: active vault address cannot be 0x0.");
        require(address(_adapter.token()) == token, "EvryMix.vault: token mismatch.");
    ...
    function recallAllFundsFromVault(uint256 _vaultId) external {
        require(pause && (msg.sender == governance || msg.sender == sentinel), "EvryMix: not paused, or not governance or sentinel");
    ...
    function setSentinel(address _sentinel) external onlyGov() {
        require(_sentinel != ZERO_ADDRESS, "EvryMix: sentinel address cannot be 0x0.");
        sentinel = _sentinel;
    ...
    function setRewards(address _rewards) external onlyGov() {
        ...
        require(_rewards != ZERO_ADDRESS, "EvryMix: rewards address cannot be 0x0.");
    ...
    function migrateFunds(address migrateTo) external onlyGov() {
        ...
        uint256 migratableFunds = totalFunds.sub(totalSupplyEvtokens, "not enough funds to service stakes");
...
library VaultWithIndirection {
  ...
  struct Data {
    VaultAdapterWithIndirection adapter;
