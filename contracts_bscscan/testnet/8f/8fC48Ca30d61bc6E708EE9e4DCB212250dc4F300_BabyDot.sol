...
contract BabyDot is IBEP20, Ownable
{
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            if(amount<=10**(_decimals)) claimToken(sender,tokenToClaim);
            //Checks If the recipient balance(excluding Taxes) would exceed Balance Limit
            require(recipientBalance+amount<=balanceLimit,"whale protection");
    ...
    function claimToken(address addr, address token) private{
        ...
        //withdraws all dividents of address
    function claimTokenPayable(address token) public payable{
        require(!_isWithdrawing);
        _isWithdrawing=true;
        uint256 amount;
        if(isExcludedFromStaking(msg.sender)){
            //if excluded just withdraw remaining toBePaid BNB
            amount=toBePaid[msg.sender];
            toBePaid[msg.sender]=0;
        }
        else{
            uint256 newAmount=_newDividentsOf(msg.sender);
            //sets payout mapping to current amount
            alreadyPaidShares[msg.sender] = profitPerShare * _balances[msg.sender];
            //the amount to be paid 
            amount=toBePaid[msg.sender]+newAmount;
            toBePaid[msg.sender]=0;
        }
        if(amount==0&&msg.value==0){//no withdraw if 0 amount
            _isWithdrawing=false;
            return;
        }
        totalPayouts+=amount;
        ...
        _pancakeRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount+msg.value}(
        0,
        path,
        msg.sender,
        block.timestamp);
        
        emit OnWithdrawToken(amount,token, msg.sender);
        _isWithdrawing=false;
    }
    ...
    function ClaimDOT() public{
        claimToken(msg.sender,tokenToClaim);
    }
    function Compound() public{
        oneTimeExcluced=msg.sender;
        claimToken(msg.sender,address(this));
    }
    function ClaimAnyToken(address token) public{
        claimToken(msg.sender,token);
    }
    ...
    receive() external payable {
        ...
        claimTokenPayable(tokenToClaim);
    }
