...
abstract contract Windmill_Trade_Abstract is Math, Adressable, AccessControlEnumerable, With_DAORole, With_UpdaterRole,
                                             With_TradeManagerRole, With_BUSDToken, With_FundContract, With_TradeManagerContract,
                                             With_WithdrawerContract, I_Windmill_Trade_Abstract{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    ////
    //////////////// Public functions ////////////////
    function initTrade(address _owner, address _manager, address _fund, address _updater, address _withdrawer) public onlyRole(DAO_ROLE) virtual{
        owner = _owner;
    ...
    function setInitBUSDBalance(uint256 _initBUSDBalance) external onlyRole(TRADE_MANAGER_ROLE){
    ...
    function endTrade() public onlyRole(TRADE_MANAGER_ROLE) virtual{
        Fraction memory withdrawFees = fund.getWithdrawFees();
        
        uint256 balance = BUSDToken.balanceOf(thisAddr);
        
        uint256 nbPWRToWithdraw = withdrawer.getNbPWRToWithdraw(tradeId);
        uint256 nbPWRFees = (nbPWRToWithdraw * withdrawFees.numerator) / withdrawFees.denominator;
...
contract Windmill_Trade_Swap_Long is Initializable, Windmill_Trade_Abstract, With_PancakeRouter, I_Windmill_Trade_Swap_Long{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    function initTrade(address _owner, address _manager, address _fund, address _updater, address _withdrawer) public onlyRole(DAO_ROLE) override(I_Windmill_Trade_Abstract, Windmill_Trade_Abstract){
        super.initTrade(_owner, _manager, _fund, _updater, _withdrawer);
    }
    ...
    function setRoute(uint256 routeId) external onlyRole(OWNER_ROLE) onlyNotInitialized{
    ...
    function placeMarketOrder(bool increaseSize, uint256 sizeIn) external onlyRole(OWNER_ROLE) onlyInitialized{
        if (increaseSize){
    ...
    function placeLimitOrder(bool increaseSize, uint256 sizeIn, uint256 triggerPrice) external onlyRole(OWNER_ROLE) onlyInitialized{
    ...
    function updateNeeded() public view override(I_Windmill_Trade_Abstract, Windmill_Trade_Abstract) returns (bool){
    ...
    function estimateBUSDBalance() public view override(I_Windmill_Trade_Abstract, Windmill_Trade_Abstract) returns (uint256){
    ...
    function getTokenPrice(uint256 sizeBUSD) public view onlyInitialized returns (uint256){
    ...
    function getTokenPriceFromTokenSize(uint256 sizeToken) public view onlyInitialized returns (uint256){
    ...
    function update() public override(I_Windmill_Trade_Abstract, Windmill_Trade_Abstract) onlyRole(UPDATER_ROLE){
        super.update();
    ...
    function _swap(uint256 sizeIn, address[] storage _route, IERC20 _token) internal{
        uint256[] memory sizeOut = pancakeRouter.getAmountsOut(sizeIn, _route);
        uint256 sizeOutMin = sizeOut[sizeOut.length-1];
        
        _token.approve(address(pancakeRouter), sizeIn);
