...
contract MultiPadLaunchApp is IBEP20{
    ...
    */
    ...
    function claimPurchasedTokens(uint256 _lockNumber) external validateClaim(msg.sender,_lockNumber) returns (bool){
        uint256 calculation;
        if(_lockNumber == 1)
        calculation  = (_finalSoldAmount[msg.sender]).mul(15).div(100);
        else
        calculation  = (_finalSoldAmount[msg.sender]).mul(17).div(100);
    ...
    modifier validateClaim(address _userAddress, uint256 _lockNumber)
    {
        ...
        require(_lockNumber == 1 || _lockNumber == 2 || _lockNumber == 3 || _lockNumber == 4 || _lockNumber == 5 || _lockNumber == 6, "Invalid Lock Number");
        if(_lockNumber == 1){   //Users will be able to withdraw tokens only after TGE
            require(block.timestamp > saleEndTime + _claimTime1 && reEntrance[_userAddress][_lockNumber] != true, "Insufficient Unlocked Tokens");
        }
        if(_lockNumber == 2){    // 2 month
        ...
        if(_lockNumber == 4){    // 4 month
            require(block.timestamp > saleEndTime + _claimTime1 +  10368000  && reEntrance[_userAddress][_lockNumber] != true , "Insufficient Unlocked Tokens");
        }
        if(_lockNumber == 5){    // 5 month
            require(block.timestamp > saleEndTime + _claimTime1 +  12960000  && reEntrance[_userAddress][_lockNumber] != true , "Insufficient Unlocked Tokens");
        }
        if(_lockNumber == 6){    // 6 month
            require(block.timestamp > saleEndTime + _claimTime1 +  15552000  && reEntrance[_userAddress][_lockNumber] != true , "Insufficient Unlocked Tokens");
        }
        _;
    }
     ...
     */
    function getUserTokensByAdd(address _userAddress) external view returns(uint256 _div1, uint256 _div2, uint256 _div3, uint256 _div4, uint256 _div5, uint256 _div6){
        _div1 = _finalSoldAmount[_userAddress].mul(15).div(100);
        _div2 = _finalSoldAmount[_userAddress].mul(17).div(100);
        _div3 = _finalSoldAmount[_userAddress].mul(17).div(100);
        _div4 = _finalSoldAmount[_userAddress].mul(17).div(100);
        _div5 = _finalSoldAmount[_userAddress].mul(17).div(100);
        _div6 = _finalSoldAmount[_userAddress].mul(17).div(100);
        ...
        if(reEntrance[_userAddress][6] == true){
            _div6 = 0;
        }
        return(_div1,_div2,_div3,_div4,_div5,_div6);
    }
    ...
    function getClaimDates() view external returns(uint256 _d1, uint256 _d2, uint256 _d3, uint256 _d4, uint256 _d5, uint256 _d6){
        _d1 = saleEndTime + _claimTime1;  
        _d2 = saleEndTime + _claimTime1 +  5184000;
        _d3 = saleEndTime + _claimTime1 +  7776000;
        _d4 = saleEndTime + _claimTime1 +  10368000;
        _d5 = saleEndTime + _claimTime1 +  12960000;
        _d6 = saleEndTime + _claimTime1 +  15552000;
        return(_d1, _d2, _d3, _d4,_d5,_d6);
    }
    
    /*
     * @dev Get claimed tokens by user address
     */
    function getClaimedTokensHistory(address _userAddress) view external returns(uint256 r1,uint256 r2, uint256 r3, uint256 r4, uint256 r5, uint256 r6){
        ...
        r6 = _claimedByUser[_userAddress][6];
    ...
    function withdrawPendingTokens(uint256 _amount) external onlyOwner returns(bool){
