...
contract NFTBlindBox is Ownable, Pausable, ReentrancyGuard {
     ...
     */
    function buyDiamond(uint256 _amount) public payable nonReentrant whenNotPaused {
     ...
     * @dev Sale Gold Box
     */
    function buyGold(uint256 _amount) public payable nonReentrant whenNotPaused {
        require(CURRENT_BOX.add(1) <= TOTAL_BOX, "box already sold out");
        uint256 typeBox = 1;
        _buyNFT(typeBox, _amount);
        CURRENT_BOX = CURRENT_BOX.add(1);
     ...
     * @dev Sale Silver Box
     */
    function buySilver(uint256 _amount) public payable nonReentrant whenNotPaused {
        require(CURRENT_BOX.add(1) <= TOTAL_BOX, "box already sold out");
        uint256 typeBox = 2;
        _buyNFT(typeBox, _amount);
        CURRENT_BOX = CURRENT_BOX.add(1);
    ...
    function _buyNFT(uint256 _typeBox, uint256 _amount) internal {
        ...
        uint256 rarePart;
        bool luckyMan = false;
        if (rareRand <= saleBoxs[_typeBox].thirdRarePercent) {
            luckyMan = true;
            rareHero = 1;
        }
        ...
        if(luckyMan) {
            ...
            nft.safeMintNFT(_msgSender(), tokenId);
            NFTItem memory nftHero = NFTItem(
                tokenId,
                "Zuki Hero",
                rareHero,
                block.timestamp
            );
            nft.setNFTFactory(nftHero, tokenId);
            nft.setNFTForUser(nftHero, tokenId, _msgSender());
        }
