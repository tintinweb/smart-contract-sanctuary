...
contract Escrow is IArbitrable, IEvidence, Ownable {
    ...
    function newTransaction(
        ...
        txs.push(
            TX({
                ...
                addedInfo: string(bytes.concat(bytes(_title), bytes(_ipfsAgreementHash)))
            })
        );
    ...
    function reclaimFunds(uint256 _txID) public payable { // to initiate a dispute payer will attempt to reclaim funds
        ...
        if (transaction.status == Status.Reclaimed) {
            if (block.timestamp - transaction.reclaimedAt <= _arbitrationFeeDepositPeriod) {
    ...
    function remainingTimeToDepositArbitrationFee(uint256 _txID) public view returns (uint256) {
        ...
        return
            (block.timestamp - transaction.reclaimedAt) > _arbitrationFeeDepositPeriod
                ? 0
                : (transaction.reclaimedAt + _arbitrationFeeDepositPeriod- block.timestamp);
    }
}