...
contract Issue is AccessControl {
    ...
    uint256 public maxHeroInPack = 30000;
    uint256 public currentHeroInPack = 0;
    ...
    function editMaxHeroInPack(uint256 _maxHeroInPack) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, address(msg.sender)), "Caller is not a owner");
        require(_maxHeroInPack > 0, "Please input > 0");
        maxHeroInPack = _maxHeroInPack;
    }
   ...
   function requestSummon(uint256 _tokenId, uint256 _numberHero, string memory _tier) public{
    ...
    function openPackage(address[] memory _owner, uint256[] memory _tokenId, uint256[] memory _numberHero, string[] memory _tier) public {
        ...
        require(currentHeroInPack.add(_tokenId.length) <= maxHeroInPack,"Sould out");
        for(uint256 i = 0; i < _tokenId.length; i++){
            require(queryTierName(_tier[i]) == 1, "Tier not found");
        ...
        currentHeroInPack = currentHeroInPack.add(_tokenId.length);
    }
    function summon(address[] memory _owner, uint256[] memory _tokenId, uint256[] memory _numberHero, string[] memory _tier) public {
        ...
        for(uint256 i = 0; i < _tokenId.length; i++){
            ...
            require(queryNumberTier(_tier[i]) == 0 || queryNumberTier(_tier[i]) == 1 || queryNumberTier(_tier[i]) == 3, "tier is not allowed to issue");
            require(keccak256(bytes(heroes[_numberHero[i]].name)) != keccak256(bytes(stringNull)), "Heroes not found");
            require( requestHero[_tokenId[i]].Owner == _owner[i] && requestHero[_tokenId[i]].numberHeroes == _numberHero[i] && keccak256(bytes( requestHero[_tokenId[i]].tier )) == keccak256(bytes(_tier[i])), "Please request before issue hero");
            heroesNFT.safeMint(address(_owner[i]), _tokenId[i]);
            heroesNFT.addHeroesNumber(_tokenId[i], _numberHero[i], heroes[_numberHero[i]].name, heroes[_numberHero[i]].race, heroes[_numberHero[i]].class, _tier[i], _tier[i]);
            emit summonhero(
                _owner[i],
