...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accBstablePerShare; 
        uint16 depositFeeBP;
		uint16 withdrawFeeBP;
    }
	
    BSTABLE public bstable;
    uint256 public bstablePerBlock;
    ...
    constructor(BSTABLE _bstable, address _feeAddress, uint256 _startBlock, uint256 _startTime) public {
        bstable = _bstable;
        feeAddress = _feeAddress;
        bstablePerBlock = INITIAL_EMISSION_RATE;
    ...
    function pendingBstable(uint256 _pid, address _user) external view returns (uint256){
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBstablePerShare = pool.accBstablePerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 bstableReward = multiplier.mul(bstablePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accBstablePerShare = accBstablePerShare.add(bstableReward.mul(1e12).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accBstablePerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        bstable.mint(address(this), bstableReward);
        pool.accBstablePerShare = pool.accBstablePerShare.add(bstableReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) external nonReentrant {
		...
		payOrLockupPendingBstable(_pid);

        if (_amount > 0) {			
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(bstable)) {
                uint256 transferTax = _amount.mul(bstable.transferTaxRate()).div(10000);
        ...
        user.rewardDebt = user.amount.mul(pool.accBstablePerShare).div(1e12);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = address(bstable);
        path[1] = bstableRouter.WETH();
        bstable.approve(address(bstableRouter), tokenAmount);
        bstableRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(tokenAmount, 0, path, payable(feeAddress), block.timestamp + 60);
    }

    function payHarvestFee(uint harvestFee) internal {
        if (bstable.balanceOf(address(this)) > harvestFee && harvestFee > 0) {
              swapTokensForEth(harvestFee);
              payable(feeAddress).transfer(address(this).balance);
        }
    }
	
    function payOrLockupPendingBstable(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];

        uint256 pending = user.amount.mul(pool.accBstablePerShare).div(1e12).sub(user.rewardDebt);
        ...
        if (block.timestamp >= startTimeHarvest && block.timestamp <= lastTimeHarvest) {
            if (pending > 0 || user.rewardLockedUp > 0) {        
                ...
                safeBstableTransfer(msg.sender, netRewards);
    ...
    function safeBstableTransfer(address _to, uint256 _amount) internal {
        uint256 bstableBal = bstable.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > bstableBal) {
            transferSuccess = bstable.transfer(_to, bstableBal);
        } else {
            transferSuccess = bstable.transfer(_to, _amount);
        }
        require(transferSuccess, "safeBstableTransfer: transfer failed.");
    }
    ...
    function updateEmissionRate(uint256 _bstablePerBlock) external onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, bstablePerBlock, _bstablePerBlock);
        bstablePerBlock = _bstablePerBlock;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            bstable.mint(referrer, refBonusEarned);
    ...
    function updateBstableRouter(address _router) external onlyOperator {
        bstableRouter = IUniswapV2Router02(_router);
        bstablePair = IUniswapV2Factory(bstableRouter.factory()).getPair(address(bstable), bstableRouter.WETH());
        require(bstablePair != address(0), "BSTABLE::updateBstableRouter: Invalid pair address.");
    }
