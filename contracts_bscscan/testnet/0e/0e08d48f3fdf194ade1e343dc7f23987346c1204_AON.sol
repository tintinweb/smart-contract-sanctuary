...
contract AON is ITRC20, Owend{
    ...
    function _release(address _address) private{
        ...
        _lockReleaseTime[_address]=_lockReleaseTime[_address].add(_cycleTimestamp);
        LockReleaseRecord memory _lockReleaseRecord=LockReleaseRecord(_address,_lockReleaseTime[_address],_totalReleaseAmount);
   ...
   function getLockReleaseRecord(uint _startIndex,uint _length)view public  returns(address[] memory _addressList,uint256 [] memory _timestampList,
    ...
    function pledge(uint256  pledgeMinimumAmount)public {
       ...
       PledgeRecord memory _pledgeRecord= PledgeRecord(msg.sender,timestamp,pledgeMinimumAmount,0,0);
       _pledgeRecords.push(_pledgeRecord);
    ...
    function setPeldegRecord(address _address,uint256 timestamp,uint canRedemption) public onlyOwner{
         for(uint i=0;i<_pledgeRecords.length;i++){
            if(_pledgeRecords[i].account==_address&&_pledgeRecords[i].timestamp==timestamp){
                _pledgeRecords[i].canRedemption=canRedemption;
                break;
            }
        }
    }
    ...
    function findRecord(address account,uint256 timestamp)view public returns(uint isRedemption,uint256 amount,uint canRedemption){
        ...
        for(uint i=0;i<_pledgeRecords.length;i++){
            if(_pledgeRecords[i].account==account&&_pledgeRecords[i].timestamp==timestamp){
                ...
                canRedemption=_pledgeRecords[i].canRedemption;
                break;
            }
   ...
   function getPledgeRecordsByIndex(uint startIndex,uint length)view public  returns(address[] memory pledgeAddressList,
   uint256 [] memory pledgeTimeList,uint256[] memory pledgeAmountList,uint[] memory isRedemption){
        ...
        for(startIndex;startIndex<=endIndex;startIndex++){
            ...
            pledgeAmountList[i]=_pledgeRecords[startIndex].amount;
    ...
    function getPledgeRecords(address _address,uint _timestamp)view public  returns(uint256   pledgeAmount,uint  isRedemption,uint  canRedemption){
        for( uint startIndex;startIndex<=_pledgeRecords.length;startIndex++){
            if(_pledgeRecords[startIndex].account==_address&&_pledgeRecords[startIndex].timestamp==_timestamp){
                pledgeAmount=_pledgeRecords[startIndex].amount;
                isRedemption=_pledgeRecords[startIndex].isRedemption;
                canRedemption=_pledgeRecords[startIndex].canRedemption;
                break;
            }
        }
    }
