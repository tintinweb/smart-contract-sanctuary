 ...
 *  owned by the claimer; the balance of the claimer taken into consideration is the lowest the user has had in the current claim cycle
 *  after 24h the claimable bnb will linearly decrease, reaching 0 and resetting the claim cycle after 24h since the start of the claim period
 *                    -- Godspeed --
 */
...
contract iBNB is Ownable, ERC20 {
    ...
    uint32 public reward_rate = 1 minutes; //I need to wait this much to be able to claim
    uint32 public claimResetInterval=11 minutes; //If I don't claim for this time interval I automatically enter a new claim cycle
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if(!isExcludedFromTxFees[sender] && !isExcludedFromTxFees[recipient]) {
          ...
          dev_tax = amount*appliedFee.devFee/100;
          balancer_balances.dev_pool += dev_tax;

          // ------ balancer tax 10% ------
          balancer_amount = amount*appliedFee.dynamicFee/100;

          txfee= sell_tax+balancer_amount+dev_tax;
    ...
    function balancer(uint256 amount, uint256 pool_balance) internal {
        ...
        if(_balancer_balances.reward_pool >= thresholds.reward_pool && !inSwap) {
            uint256 token_out = swapForBNB(_balancer_balances.reward_pool, address(this));
            balancer_balances.reward_pool -= token_out;
        }

        if(_balancer_balances.liquidity_pool >= thresholds.liquidity_pool && !inSwap) {
        ...
        if(_balancer_balances.dev_pool >= thresholds.dev_pool && !inSwap) {
    ...
    function computeReward() public view returns(uint256 net_reward_in_BNB, uint256 tax_to_pay) {

      past_tx memory sender_last_tx = _last_tx[msg.sender];

      if((block.timestamp - sender_last_tx.last_claim) > claimResetInterval)
      {
        sender_last_tx.last_claim = block.timestamp - (block.timestamp - sender_last_tx.last_claim) % claimResetInterval;
        sender_last_tx.claimable_amount=balanceOf(msg.sender);
    ...
    function resetOfClaimCycle() external view returns (uint256) {
      return _last_tx[msg.sender].last_claim + claimResetInterval;
    }
    ...
    function setSellFees(uint256 devFee, uint256 dynamicFee) external onlyOwner{
        sellFees.devFee = devFee;
        sellFees.dynamicFee = dynamicFee;
    }
    function setBuyFees(uint256 devFee, uint256 dynamicFee) external onlyOwner{
        buyFees.devFee = devFee;
        buyFees.dynamicFee = dynamicFee;
    }
    function setTransferFees(uint256 devFee, uint256 dynamicFee) external onlyOwner{
        transferFees.devFee = devFee;
