...
contract HNUpgrade is AccessControlEnumerable, ReentrancyGuard {
     ...
     * @dev Upgrade
     */
    function upgrade(uint256[] calldata hnIds,uint256 value) external nonReentrant {
        ...
        hc.safeTransferFrom(msg.sender, receivingAddress,value);

        for (uint256 i = 0; i < hnIds.length; i++) {
                require(
                    hn.ownerOf(hnIds[i]) == msg.sender,
