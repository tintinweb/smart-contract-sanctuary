...
abstract contract AccessControl is Context, IAccessControl, ERC165 {
    ...
    constructor() {
        ...
        _setupRole(FORWARDING_ROLE, msg.sender);
    }
...
abstract contract Pausable is Context, AccessControl {
    ...
    function pauseAddress(address addr) public onlyRole(PAUSER_ROLE) {
        adresses[addr] = true;
        emit PausedAddr(_msgSender(), addr);
    }

    function unpauseAddress(address addr) public onlyRole(PAUSER_ROLE) {
        if (adresses[addr]) {
...
abstract contract ForwardingPayment is Context, AccessControl {
    ...
    function setForwardingAddress(address addr) public virtual onlyRole(FORWARDING_ROLE) {
...
abstract contract ERC777 is Context, IERC777, IERC20, AccessControl, Pausable {
    ...
    constructor(
        ...
        address[] memory defaultOperators_ = new address[](0);
