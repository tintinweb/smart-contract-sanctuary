...
import "../interface/IsafeTransferFrom.sol";
...
contract Market is Ownable, ERC721Holder{
    ...
    event UpdateGoodsStatus(uint indexed goodsType, uint indexed tokenId, bool indexed status, uint tradeType, uint price);
    event Exchanged(uint indexed goodsType, uint indexed tokenId, uint indexed tradeType, uint price, address seller, address buyer);


    // ------------------------------  only owner ---------------------------------------
    function newTradeType(uint id_, address bank_) external onlyOwner returns (bool) {
        require(tradeType[id_] == address(0),"Exist trade type");
        tradeType[id_] = bank_;
        return true;
    }
    ...
    function divestFee(address payable payee_, uint value_, uint tradeType_) external onlyOwner returns (bool){
        ...
        IERC20(bank).transfer(payee_, value_);
        return true;
    }
    ...
    function sell(uint goodsType_, uint tokenId_, uint tradeType_, uint price_) public {
        require(price_ >= 100 && price_ <  1e8 * 1e18, "Price");
        ...
        sellingStatus[goodsType_][tokenId_] = true;

        emit UpdateGoodsStatus(goodsType_, tokenId_, true, tradeType_, price_);
    }

    function cancelSell(uint goodsType_, uint index_) public {
        ...
        delete sellingStatus[goodsType_][tokenId];

        emit UpdateGoodsStatus(goodsType_, tokenId, false, 0, 0);
    }

    function mainCoinPurchase(uint goodsType_, uint tokenId_) public payable {
        require(sellingStatus[goodsType_][tokenId_], "Not selling");
        require(sellingGoods[goodsType_][tokenId_].tradeType == 1, "Main coin");
        ...
        purchaseProcess(info.owner, goodsType_, tokenId_, info.tradeType, info.price);
    }

    function erc20Purchase(uint goodsType_, uint tokenId_) public {
        require(sellingStatus[goodsType_][tokenId_], "Not selling");
        require(sellingGoods[goodsType_][tokenId_].tradeType != 1, "erc20");
        ...
        IERC20(banker).transferFrom(_msgSender(), info.owner, amount);
        IERC20(banker).transferFrom(_msgSender(), address(this), handleFee);

        purchaseProcess(info.owner, goodsType_, tokenId_, info.tradeType, info.price);
    }

    function purchaseProcess(address owner_, uint goodsType_, uint tokenId_, uint tradeType_, uint price_) internal {
        uint length = _sellingList[goodsType_][owner_].length;

        if (length > 1) {
            for (uint i = 0; i < length - 1; i++) {
                if (_sellingList[goodsType_][owner_][i] == tokenId_) {
                    _sellingList[goodsType_][owner_][i] = _sellingList[goodsType_][owner_][i + 1];
                    break;
                }
            }
        }
        ...
        delete sellingStatus[goodsType_][tokenId_];
        _sold[goodsType_] += 1;

        emit Exchanged(goodsType_, tokenId_, tradeType_, price_, owner_, _msgSender());
    }
