...
contract GoldenDoge is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    event ContractFee(bool ContractFee, bool isSell);

    event ContractLiqudityAmount(uint256 tLiqudity, uint256 rLiqudity, bool isSell);

    event TransferAddress(address from, address to, address pancakePair);
    ...
    function _takeLiquidity(uint256 tLiquidity, bool isSell) private {
        ...
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        emit ContractLiqudityAmount(tLiquidity, rLiquidity, isSell);
    ...
    function _transfer(
        ...
        if (
            ...
            emit ContractBalance(address(this).balance, contractTokenBalance, _liqudityAmount);
        ...
        emit TransferAddress(from, to, address(pancakePair));

        emit ContractFee(takeFee, isSell);
