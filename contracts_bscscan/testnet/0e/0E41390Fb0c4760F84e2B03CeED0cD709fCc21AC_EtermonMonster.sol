...
contract EtermonMonster is Etermon {
    ...
    function monsterInitailize(address mintFactory_) external {
        require(!lock);
        mintFactory = mintFactory_;
        _addMinter(mintFactory_);
        _addMinter(owner());
        etermonInitailize("EtermonMonster","ETM");
        lock = true;
    }

    function upgradeMonster(uint256[] memory tokenId) external returns (uint256){
        assert(!isBlackListed[msg.sender]);
        require (tokenId.length > 0, "Dont have any Monster");
    ...
    function upgradeMonster(uint256[] memory parentId, uint256 childId) external returns (uint256){
        assert(!isBlackListed[msg.sender]);
        require (parentId.length > 0, "Dont have any Monster");
        
        for(uint256 i; i < parentId.length; i++){
            _burn(parentId[i]);
        }

        MintFactory(mintFactory).mintTo(msg.sender, address(this), childId);
