...
contract MasterERC721Standards is Context {
  ...
  function createTokenERC721(
    ...
    emit ChildrenERC721Standard(address(child), name, symbol, contractURI);
...
contract ERC721Standards is Ownable, ERC721URIStorage {
  ...
  constructor(
    ...
    string memory metadata,
    string[] memory _keyTypes
  ) ERC721(name, symbol, contractURI) {
...
abstract contract Ownable is Context {
   ...
   */
  function setOwners(address[] memory _owners) public virtual groupOwner {
    _setOwners(_owners);
  }
