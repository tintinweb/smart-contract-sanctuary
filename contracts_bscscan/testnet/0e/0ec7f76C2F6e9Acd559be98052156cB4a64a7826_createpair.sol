...
contract createpair {
    ...
    function CreatePair() public{
        ...
        uniswapV2Router = _uniswapV2Router; //Not sure why the interface is renamed. Anyways, this is now what u use to call the uniswap functions.
    }
    ...
    function swapTokensForETH(uint amount) public{
        address[] memory path = new address[](2); //Creates a temporary path
        path[0] = address(this);            //Token address
        path[1] = uniswapV2Router.WETH();   //BNB address
    ...
    function getAmountsOut(uint amountIn) public view returns (uint[] memory amounts){ //Returns ETH value of input token amount
        ...
        address[] memory path2 = new address[](2);   //Creates a temporary path
        path2[0] = token;                            //Token address
        path2[1] = WETH;                             //BNB address
        amounts = uniswapV2Router.getAmountsOut(amountIn,path2);
