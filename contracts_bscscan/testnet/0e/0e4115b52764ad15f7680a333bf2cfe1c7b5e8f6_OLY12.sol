...
to add, every sell, increase by 1% untill 40%

transfer charges greenwalltax
...
contract OLY12 is IBEP20, Auth {
    ...
    string constant _name = "OLY12";
    string constant _symbol = "OLY12";
    ...
    uint256 public liquidityFee = 300; //added public
    uint256 public buybackFee = 300;//added public
    uint256 public reflectionFee = 300;//added public
    uint256 public marketingFee = 400;//added public
    uint256 public lotteryFee = 200;//added public
    uint256 public devFee = 100;//added public
    uint256 public totalBuyFee = 1600;
    uint256 public totalSellFee = 1600;
    ...
    uint256 public greenwallNumerator = 25; //greenwall
  //  uint256 public greenwallDenominator = 1000; //greenwall
    uint256 public greenwallDuration = 2 minutes; //greenwall
    uint256 public greenwallLimit = 1500;
    
    bool public redwallEnabled = true; //redwall
    uint256 public redwallTriggeredAt = 0;//redwall
    uint256 public redwallNumerator = 100; //redwall
 //   uint256 public redwallDenominator = 1000; //redwall
    uint256 public redwallDuration = 10 minutes; //redwall
    uint256 public redwallLimit = 1800;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
     ...
     function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
      ...
      //  address _address = isSell(recipient) ? sender : recipient; //did this to try reduce shouldtakefee to 1 address
        ...
        uint256 amountReceived = shouldTakeFee(sender,recipient) ? takeFee(sender, recipient, amount) : amount; //changed to _address
    ...
    function shouldTakeFee(address sender, address recipient) public view returns (bool) { // original internal //added recipient
    ...
    function greenwall () internal {
        ...
        greenwallBuyFee > greenwallLimit ? greenwallBuyFee = greenwallBuyFee.sub(greenwallNumerator) : greenwallBuyFee = greenwallBuyFee.sub(0);
    }
    
     function redwall () internal {
        redwallTriggeredAt = block.timestamp;
        redwallSellFee < redwallLimit ? redwallSellFee = redwallSellFee.add(redwallNumerator) : redwallSellFee = redwallSellFee.add(0);
    }
   ...
   function setMultiplierSettings(uint256 floppaNum, uint256 bingusNum, uint256 denominator, uint256 length) external authorized{
    ...
    function setFees(uint256 _lotteryFee, uint256 _devFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator, uint256 _totalSellFee) external authorized {
        ...
        devFee = _devFee;
        totalBuyFee = liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee).add(_lotteryFee).add(_devFee);
        totalSellFee = _totalSellFee;
        feeDenominator = _feeDenominator;
        require(totalBuyFee <= feeDenominator * 30 / 100, "Buy fee too high");
        ...
        require(_reflectionFee <= feeDenominator * 10 / 100, "Reward fee too high");
        require(_marketingFee <= feeDenominator * 10 / 100, "Marketing fee too high");
        require(_lotteryFee <= feeDenominator * 10 / 100, "Lottery fee too high");
        require(_devFee <= feeDenominator * 10 / 100, "Dev fee too high");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _lotteryFeeReceiver, address _devFeeReceiver) external authorized {
