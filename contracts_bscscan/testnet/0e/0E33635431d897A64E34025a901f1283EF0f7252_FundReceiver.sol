...
contract FundReceiver is Ownable {
    ...
    // address public mainToken = 0x89Aba1453f58aB08056DA973163A67EFed95A432;
    ...
    event ShareUpdated(address indexed account, uint256 amount);
    ...
    function setShare(address account, uint256 amount) external onlyOwner {
        ...
        emit ShareUpdated(account, amount);
    }
    ...
    //     require(amount <= IERC20(mainToken).balanceOf(address(this)), "Insufficient token balance");

    //     IBurnable(mainToken).burn(amount);
    // }
