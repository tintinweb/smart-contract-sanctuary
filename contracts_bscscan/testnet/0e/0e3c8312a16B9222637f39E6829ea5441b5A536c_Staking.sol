...
interface IStakingPool {
    function addReward(address token, uint256 amount, uint256 numBlocks) external;
}

contract Staking is  AccessControl, MultiSig {
    struct TX {
        address[] tokens;
        uint256[] amounts;
        uint256 numBlocks;
    ...
    event SendReward(uint256 txId, address token, uint256 amount, uint256 numBlocks, address to);
        ...
        external
    {
        ...
        TX memory record = txs[txId];
        if (record.tokens.length == 0) {
            record = TX(tokens, amounts, numBlocks, to);
            txs[txId] = record;
        }

        if (_sign(txId)) {
            _sendRewards(txId, record.tokens, record.amounts, record.numBlocks, record.to);
        }
    }
        ...
        internal
    {
        for (uint i = 0; i < tokens.length; i++) {
            _sendReward(txId, tokens[i], amounts[i], numBlocks, to);
        }
    }
        ...
        internal
    {
        ...
        if (_token.allowance(address(this), to) == 0) {
            _token.approve(to, 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff);
        }
        IStakingPool(to).addReward(token, amount, numBlocks);
        emit SendReward(txId, token, amount, numBlocks, to);
    }
