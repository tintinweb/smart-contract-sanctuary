...
contract BUADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BUA_Dividend_Tracker", "BUA_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "BUA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "BUA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SURF contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "BUA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BUA_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BULLART is ERC20, Ownable {
   
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    BUADividendTracker public dividendTracker;

    address public _marketingWallet = 0x5D4204F52682D49d40E5FE5D8bAD6CBC0b353D74;
    address public _charityWallet = 0x5D4204F52682D49d40E5FE5D8bAD6CBC0b353D74 ;

    bool public restrictWhales = false;

    uint256 public maxTxAmount =  10000000 * (10**18);
    uint256 public walletMax = 10000000  * (10**18);
    uint256 public swapTokensAtAmount = 25000000 * (10**18);
    ...
    constructor() ERC20("BULLART Token", "$BUA") {

        dividendTracker = new BUADividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BUA: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BUA: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BUA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BUA: Cannot update gasForProcessing to same value");
