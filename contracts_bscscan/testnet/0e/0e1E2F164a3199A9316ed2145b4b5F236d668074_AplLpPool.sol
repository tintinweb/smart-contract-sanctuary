...
contract AplLpPool is Ownable {
    ...
    mapping(address => uint256) private lastMolecular;
    mapping(address => uint256) private lastDenominator;
    ...
    function settle() public onlySettle{
        uint256 releaseAplAmount = getAllBlocks().mul(claimPerBlock);
                ...
                for(uint256 j=0; j<stakingList[addressList[n]].length; j++){
                    ...
                    if(timeSpan >= ONE_MONTH_IN_SECONDS){
                        if(stakingList[addressList[n]][j].isLiving){
                            ...
                            settleData2 = stakingList[addressList[n]][j].lastSettleBlockNumber;
                            settleData = totalBlockOfCurrentRound[addressList[n]].add(currentBlock.sub(stakingList[addressList[n]][j].lastSettleBlockNumber));
                            totalBlockOfCurrentRound[addressList[n]] = settleData;
