...
contract KangarooMatrix {
    ...
    function migrationsMatrix621(uint32 id,uint8 level) public view returns(X6_2 memory) {

        (bool success, ) = oldMatrix.staticcall(abi.encodeWithSignature("usersX6Matrix(uint32,uint8)",id,level));
        X6_2 memory x6_2MATRIX;
        assembly {
            //let free_mem_ptr := mload(0x40)
            returndatacopy(x6_2MATRIX, 0, returndatasize())
            switch success
                case 0 {
                    revert(x6_2MATRIX, returndatasize())
                }
                default {
                    return(x6_2MATRIX, returndatasize())
                }  
        }
    }

    function migrationsMatrix4(uint32 id,uint8 level) public view returns(X6_2 memory) {
        ...
        assembly {
            let len := mload(data)
            let _data := add(data, 0x20)
            let _x6_2MATRIX := x6_2MATRIX
            ...
            {
                mstore(_x6_2MATRIX, mload(_data))
                _x6_2MATRIX:=add(_x6_2MATRIX, 0x20)
            }
