...
contract MonsterToken is MonsterERC20, ReentrancyGuard {
    ...
    //delete testnet
    function mint(address _addr, uint256 _am) public  {
        _mint(_addr,_am);
    }
    ...
    function swapToBoss() public nonReentrant {
        ...
        if (contractTokenBalance >= MINIMUM_TOKEN_BOSS) {
            swapTokensForEth(MINIMUM_TOKEN_BOSS);
        }
    }
...
contract MonsterERC20 is Ownable, ERC20 {
    ...
    uint256 public PLAYTOEARN_ALLOCATION = 350 * 10**6 * 10**18;
    uint256 public playToEarnReward;

    IManager public manager;

    uint256 public MINIMUM_TOKEN_BOSS = 2 * 10**6 * 10**18;
    ...
    function setMinTokensBeforeSwap(uint256 _MINIMUM_TOKEN_BOSS)
        public
        onlyOwner
    {
        require(_MINIMUM_TOKEN_BOSS < 20 * 10**6 * 10**18, "not allow over Rate");
        MINIMUM_TOKEN_BOSS = _MINIMUM_TOKEN_BOSS;
    }
    ...
    function win(address winner, uint256 reward) external onlyEvolver {
        ...
        if (playToEarnReward <= PLAYTOEARN_ALLOCATION) _mint(winner, reward);
        else {
            uint256 availableReward = playToEarnReward.sub(PLAYTOEARN_ALLOCATION);
            _mint(winner, availableReward);
            playToEarnReward = PLAYTOEARN_ALLOCATION;
        }
    }
}
