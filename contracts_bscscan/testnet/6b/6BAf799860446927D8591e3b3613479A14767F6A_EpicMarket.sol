...
contract EpicMarket is AccessControl {
    ...
    function getOfferLists() public view returns (Offer[] memory){
        return offers[msg.sender];
    }
    function getOfferById(uint256 offerId) public view returns(bool status, Offer memory){
        for(uint256 i = 0; i < offers[msg.sender].length; i++){
            if(i == offerId) return  (true, offers[msg.sender][i]);
        }
    ...
    function acceptOffer(uint256 marketId, uint256 offerId) public onlyExistItem(marketId){
        ...
        (bool status, Offer memory offerData) = getOfferById(offerId);
        ...
        offers[msg.sender][offerId]._isAccept = true;
        offers[msg.sender][offerId]._active = false;
    }
    function closeOffer(uint256 marketId, uint256 offerId) public onlyExistItem(marketId){
        require(offerId < offers[msg.sender].length, "Invalid offerId");
