...
contract sTrapeza_v2 is IsFIDL, ERC20Permit {
    ...
    event LogContractsUpdated(address stakingContract, address prevsFIDL);
    ...
    IsFIDL public prevsFIDL; // v1 contract 
    ...
    // do this last
    function initialize(address stakingContract_, address prevsFIDL_ ) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");

        require(stakingContract_ != address(0), "Staking");
        stakingContract = stakingContract_;

        require(prevsFIDL_ != address(0), "prev sFIDL Contract");
        prevsFIDL = IsFIDL(prevsFIDL_);

        // require(totalSupply_ != IsFIDL(prevsFIDL_).totalSupply(), "Incorrect totalSupply");
        ...
        emit LogContractsUpdated(stakingContract, prevsFIDL_);
    ...
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(prevsFIDL.circulatingSupply()).add(gFIDL.balanceFrom(IERC20(address(gFIDL)).totalSupply())).add(
