...
contract WastedLands is Context, Ownable, IERC20 {
    ...
    function setArbitragefee(uint _arbitrageFee) external onlyOwner {
        arbitrageFee = _arbitrageFee;
    }

    function withdrawAmountArbitrageFund(uint256 amount, address wallet) public onlyOwner {
    ...
    function _transfer(address fromAddress, address toAddress, uint256 amount) private {
        ...
        if(exceptionAddress[fromAddress] || exceptionAddress[toAddress]) {
            ...
            if ( arbitrageFund > 0 && fromAddress != owner() && toAddress != owner() && !feeWhitelist[fromAddress] && !feeWhitelist[toAddress] ){
    ...
    function _addArbitrageFund(uint256 amount) private returns (uint256) {
    	uint256 arbitrageFundOfTx = amount.mul(arbitrageFee).div(100);
    	
    	_balances[address(this)] = _balances[address(this)].add(arbitrageFundOfTx);
