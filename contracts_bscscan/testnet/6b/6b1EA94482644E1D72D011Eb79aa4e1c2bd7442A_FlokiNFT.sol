...
contract FlokiNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function getTokenIdandURIByAddress(address _owner) public view returns (uint256[] memory, string[] memory) {
        uint256[] memory tokenIdsArray_ = _ownerToTokenIds[_owner];
        string[] memory tokenURIsArray_ = new string[](tokenIdsArray_.length);
        for(uint256 i = 0; i < tokenIdsArray_.length; i++) {
            string memory tokenURI_ = tokenURI(tokenIdsArray_[i]);
            tokenURIsArray_[i] = tokenURI_;
        }
        return (tokenIdsArray_, tokenURIsArray_);
    }
    ...
    function bridgeMint (address _owner, uint256 _tokenId, string memory _tokenURI, string memory _tokenMeta) public onlyAdmin {
