...
contract KabyHero is IKabyHero, ERC721, AcceptedToken, ReentrancyGuard {
    ...
    constructor(
        ...
        address _oraichainRandom
    ) ERC721("KabyHero", "HERO") AcceptedToken(tokenAddress)
    {
        gemContract = gemAddress;
        _uri = baseURI;
        versions.push(Version(0, 0, param.maxSupply, param.maxForSell, param.salePrice, param.startTime, param.endTime, param.provenance, false));
        kabyToken = _kabyToken;
        randomGenerator = IRandomNumberGenerator(_randomGenerator);
        oraichainRandom = IRandomOraichain(_oraichainRandom);
    }
    ...
    modifier onlyRandomNumberGenerator() {
        ...
        modifier onlyOraichainRandom() {
        require(address(oraichainRandom) == msg.sender, "KabyHero: not random number generator");
        _;
    }
    ...
    /**
     * @dev Owner set new randomOraichain contract
     * @param randomAddress new randomOraichain contract address
     */
    function setRandomOraichain(IRandomOraichain randomAddress) external onlyOwner {
        require(address(randomAddress) != address(0));
        oraichainRandom = randomAddress;
        emit UpdateRandomOraichain(address(randomAddress));
    }
     ...
     */
    function setRandomStarByChainlink(uint heroId, uint randomness) external override onlyRandomNumberGenerator {
        require(heroId < _heros.length, "KabyHero: invalid hero");
        Hero storage hero = _heros[heroId];
     ...
     * @param randomness random number is returned from chainlink vrf random
     */
    function setRandomStarByOraichain(uint heroId, uint randomness) external override onlyOraichainRandom {
        require(heroId < _heros.length, "KabyHero: invalid hero");
        Hero storage hero = _heros[heroId];
        uint randomStarIndex = randomness % 100;
    ...
    function randomStar(uint heroId) private returns (uint256 returnStar){
        require(heroId < _heros.length, "KabyHero: invalid hero");
        if ((useOracle == false && address(randomGenerator) == address(0)) || (useOracle == true && address(oraichainRandom) == address(0))){
            ...
            if(useOracle == false){
                ...
                oraichainRandom.requestRandomNumberForStar(heroId, block.timestamp);
            }
