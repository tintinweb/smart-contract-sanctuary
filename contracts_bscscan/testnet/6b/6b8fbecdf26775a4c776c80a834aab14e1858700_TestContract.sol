...
contract TestContract is Context, IBEP20, Ownable {
    ...
    // switch Authorized
    function authorizedWallet(bool _status, address _addr) public onlyOwner {
    ...
    function AuthorizeWallet(address _addr) public view returns(bool) {
    ...
    function swapAndLiquify(uint256 tokenBalance) private lockTheSwap {
        ...
        if(_externalFeeBuy > 0)
        {
            uint256 externalAmount = tokenBalance.mul(_externalFeeBuy);
            externalAmount = externalAmount.div(_totalLiquidityFeeBuy);
