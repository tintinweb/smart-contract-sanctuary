...
contract DAOPrivate is Ownable, ReentrancyGuard, Pausable, SignatureVerifyer {
    ...
    function buyLevel(uint256 levelID) public payable nonReentrant whenNotPaused {
    ...
    function autoLevelUp(
      ...
      uint256 amount
        ...
        emit Payment(
            ...
            upline,
            prepaymentShare,
            TransferType.LEVEL_AUTOPAYMENT
        );
    ...
    function updateEpoch(uint256 userID, uint256 round) private {
      ...
      if(roundEpochs[round][epochNum].users.length == 10) {
        ...
        for(uint256 i = 0; i < roundEpochs[round][epochNum].users.length; i++){
          uint256 userEpochShare = selfRefsCount[roundEpochs[round][epochNum].users[i]] + 1;

          roundEpochs[round][epochNum].share[
            roundEpochs[round][epochNum].users[i]
          ] = userEpochShare;

          fullShare = fullShare.add(userEpochShare);
        }
    ...
    function claimTransitionBonus(uint256 round) external nonReentrant whenNotPaused {
      uint256 userID = userWallets[msg.sender];
      claimTransitionBonusFor(round, userID);
    }

    function claimTransitionBonusFor(uint256 round, uint256 userID) private {
      ...
      require(!roundEpochs[round][epochNum].isClaimed[userID], 'alreadyClaimed');
      ...
      require(rewardAmount != 0, 'nothingToClaim');

      roundEpochs[round][epochNum].isClaimed[userID] = true;
    ...
    function getUserTransitionBonuses(uint256 userID) external view returns (
      uint256[] memory usersCount,
      uint256[] memory amount,
      bool[] memory isClaimable
    ) {
      require(users[userID].exists, '!registered');
      (uint256 maxRound,) = getLevelInfo(users[userID].level);

      usersCount = new uint256[](maxRound);
      amount = new uint256[](maxRound);
      isClaimable = new bool[](maxRound);

      for(uint256 i = 0; i < maxRound; i++) {
        uint256 round = i + 1;
        if(!isUserInRound[userID][round]) {
          break;
        }
        uint256 epochNum = userEpoch[userID][round];
        usersCount[i] = roundEpochs[round][epochNum].users.length;
        isClaimable[i] = roundEpochs[round][epochNum].users.length >= 10
          && !roundEpochs[round][epochNum].isClaimed[userID];
    ...
    function getEpochInfo(uint256 round, uint256 epoch) external view returns (
      uint256 id,
      ...
      return (
        ...
        (roundEpochs[round][epoch].users.length >= 10)
        );
    }

    function getEpochUserShare(uint256 userID, uint256 round, uint256 epoch) external view returns (uint256){
    ...
    function claimRefBonus() external nonReentrant whenNotPaused {
      uint256 userID = userWallets[msg.sender];
      require(users[userID].exists, '!registered');
