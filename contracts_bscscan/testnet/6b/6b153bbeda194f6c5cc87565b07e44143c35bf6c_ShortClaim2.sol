...
contract TokenDividendTracker is DividendPayingToken, Ownable {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 60 && newClaimWait <= 86400, "ShortClaim2_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ShortClaim2_Dividend_Tracker: Cannot update claimWait to same value");
...
contract ShortClaim2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public  buyBackBurnFee = 5;
    uint256 public  totalFees = ETHRewardsFee.add(marketingFee).add(liquidityFee).add(buyBackBurnFee);
    ...
    constructor() public ERC20("AShortClaim2Token", "ShortClaim2") {

        _marketingWallet = 0x333e5922681EE29f515Db739e556c59133329921;
    	dividendTracker = new TokenDividendTracker("ShortClaim2_Dividend_Tracker", "ShortClaim2_Dividend_Tracker", DOGEToken);
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ShortClaim2: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ShortClaim2: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ShortClaim2: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ShortClaim2: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ShortClaim2: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ShortClaim2: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(swapEnabled && !swapping && from != uniswapV2Pair) {
           if (overMinimumTokenBalance) {
                uint256 snlTokens = contractTokenBalance.mul(liquidityFee).div(totalFees);
                swapAndLiquify(snlTokens);

                uint256 marketingTokens = contractTokenBalance.mul(marketingFee).div(totalFees);
                swapAndSendToMarketing(marketingTokens);
                
                uint256 amountForBurn = contractTokenBalance.mul(buyBackBurnFee).div(totalFees);
                swapTokens(amountForBurn);    
