...
// File: contracts/GembleVault.sol

pragma solidity 0.6.12;

contract GembleVault is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(rubyReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 rubyReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(rubyReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
        lastRewardBlock = block.number;
    }
