 ...
 */
interface I_Windmill_Fund is I_Math, I_Payable, IAccessControlEnumerable, I_With_DAORole, I_With_DAOAddress,
 ...
 */
interface I_Windmill_DAO is I_Payable, I_Gas_Refundable, IAccessControlEnumerable, I_With_DAORole, I_With_FundContract,
                            I_With_TradeManagerContract, I_With_PWRToken, I_With_BUSDToken, I_With_WBNBToken, I_With_PancakeRouter{
    /**
     ...
     * - 22 -> Update the Windmill_Royalty contract
     *      (address) -> New royalty contract (DAO_ROLE must have been set correctly)
    ...
    function updateDAONeeded() external view returns (bool);

    function updateDAOCycleNeeded() external view returns (bool);

    function updateOneDAOCycle() external;

    function updateDAOCycle() external;

    function updateRoyaltyCycleNeeded() external view returns (bool);

    function updateOneRoyaltyCycle() external;

    function updateRoyaltyCycle() external;

    function updateTradeNeeded() external view returns (bool);

    function updateOneTrade(uint256 tradeId) external;

    function updateTrade() external;

    function updateOneStackingCycle() external;

    function updateStackingCycle() external;
    
    function updateOneStackingUpdateNeeded(uint256 groupId) external view returns (bool);
    
    function updateStackingUpdateNeeded() external view returns (bool);

    function updateOneStackingUpdate(uint256 groupId) external;
    
    function updateStackingUpdate() external;
    
    function updateOneDAO(uint256 i) external;

    function updateDAO() external;
}
// Version: 1.0.0
...
interface I_Windmill_Deployer is I_With_PancakeRouter, I_With_BUSDToken, I_With_WBNBToken{
    ...
    function deployDAO(bytes memory bytecode) external returns (address payable);
    
    function deployFund(bytes memory bytecode) external returns (address payable);
    
    function deployPWR(bytes memory bytecode) external returns (address payable);
    
    function deployRoyalty(bytes memory bytecode) external returns (address payable);
    
    function deployStacking(bytes memory bytecode) external returns (address payable);

    function deployTrade(bytes memory deployerBytecode, bytes calldata tradeBytecode, string calldata name) external returns (address payable);
    ...
    function initWindmill(uint256[11] calldata data,
                          bool[] calldata _isProposalSuper,
                          uint256[13] calldata data2) external;
}// Version: 1.0.0
...
interface I_Windmill_Trade_Deployer_Swap_Long is I_With_PancakeRouter, I_Windmill_Trade_Deployer_Abstract{
	...
	function getNbAuthorizenTokens() external view returns (uint256);
...
contract Windmill_Deployer is Adressable, Ownable, With_PancakeRouter, With_BUSDToken, With_WBNBToken, I_Windmill_Deployer{
    ...
    function deployTrade(bytes memory deployerBytecode, bytes calldata tradeBytecode, string calldata name) external onlyOwner returns (address payable){
        ...
        deployer.setTradeCode(tradeBytecode, name);
    ...
    function initWindmill(uint256[11] calldata data,
                          ...
                          uint256[13] calldata data2) external onlyOwner{
        ...
        address PWRAddress = address(windmill.PWR);
        ...
        role = windmill.PWR.DAO_ROLE();
        windmill.PWR.grantRole(role, DAOAddress);
        windmill.PWR.revokeRole(role, thisAddr);
        
        //Setup Fund
        windmill.fund.setWithdrawFees(data2[0], data2[1]);
        windmill.fund.setPWRToken(windmill.PWR);
        windmill.fund.setBUSDToken(BUSDToken);
        windmill.fund.setWBNBToken(WBNBToken);
        windmill.fund.setDAOAddress(DAOAddress);
        windmill.fund.setPancakeRouter(pancakeRouter);
        windmill.fund.setTradeManager(windmill.tradeManager);
        
        role =  windmill.fund.TRADE_MANAGER_ROLE();
        windmill.fund.grantRole(role, tradeManagerAddress);
        
        role =  windmill.fund.DAO_ROLE();
        windmill.fund.grantRole(role, DAOAddress);
        windmill.fund.revokeRole(role, thisAddr);
        ...
        role =  windmill.royalty.DAO_ROLE();
        windmill.royalty.grantRole(role, DAOAddress);
        windmill.royalty.revokeRole(role, thisAddr);
        ...
        role =  windmill.stacking.DAO_ROLE();
        windmill.stacking.grantRole(role, DAOAddress);
        windmill.stacking.revokeRole(role, thisAddr);
        
        
        //Setup Trades
        windmill.tradeManager.setFund(windmill.fund);
        ...
        windmill.tradeManager.setMinimumBUSDToTrade(data2[12]);
        ...
        for (uint256 i=0; i<windmill.tradeDeployers.length; i++){
            ...
            role =  windmill.tradeDeployers[i].DAO_ROLE();
            windmill.tradeDeployers[i].grantRole(role, tradeManagerAddress);
            windmill.tradeDeployers[i].grantRole(role, DAOAddress);
            windmill.tradeDeployers[i].revokeRole(role, thisAddr);
            
            windmill.tradeManager.addTradeDeployer(windmill.tradeDeployers[i]);
        }
        
        role =  windmill.tradeManager.DAO_ROLE();
        windmill.tradeManager.grantRole(role, DAOAddress);
        windmill.tradeManager.revokeRole(role, thisAddr);
        
        
        //Setup DAO
        windmill.DAO.init(data, isProposalSuper);
        windmill.DAO.setPWRToken(windmill.PWR);
        windmill.DAO.setBUSDToken(BUSDToken);
        windmill.DAO.setWBNBToken(WBNBToken);
        windmill.DAO.setFund(windmill.fund);
        windmill.DAO.setTradeManager(windmill.tradeManager);
        windmill.DAO.setStackingAddress(windmill.stacking);
        windmill.DAO.setRoyaltyAddress(windmill.royalty);
        windmill.DAO.setPancakeRouter(pancakeRouter);
        
        windmill.DAO.addUser(msg.sender, 3);
        
        role =  windmill.DAO.DAO_ROLE();
        windmill.DAO.revokeRole(role, thisAddr);
    }
    ...
    function testPack(uint256 val) external view returns (uint256, uint256, uint256, uint256[] memory){
        uint256 param = val >> (8+16);
        uint256 deployerProposalId = (val << (256-8-16)) >> (256-8);
        uint256 deployerId = (val << (256-16)) >> (256-16);
        
        return (deployerId, deployerProposalId, (param << (256-4)) >> (256-4), _unpackRoute(param));
    }
    
    function _unpackRoute(uint256 data) internal pure returns (uint256[] memory){
        uint256 size = (data << (256-4)) >> (256-4);
        
        uint256[] memory route = new uint256[](size);
        for (uint i = 0; i<size; i++){
            uint256 tokenId = (data << (256-4-(i+1)*14)) >> (256-14);
            route[i] = tokenId;
        }
