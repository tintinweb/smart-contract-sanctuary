...
// File src/interfaces/TukTukControllerInterface.sol
...
// File src/interest/InterestRateModel.sol
...
// File src/interfaces/EIP20NonStandardInterface.sol
...
// File src/interfaces/TTokenInterfaces.sol
...
// File src/interfaces/EIP20Interface.sol
...
// File src/common/ErrorReporter.sol

pragma solidity ^0.5.16;

contract TukTukControllerErrorReporter {
    enum Error {
        NO_ERROR,
        UNAUTHORIZED,
        TUKTUK_CONTROLLER_MISMATCH,
...
contract TokenErrorReporter {
    enum Error {
        ...
        UNAUTHORIZED,
        BAD_INPUT,
        TUKTUK_CONTROLLER_REJECTION,
        TUKTUK_CONTROLLER_CALCULATION_ERROR,
    ...
    enum FailureInfo {
        ...
        BORROW_TUKTUK_CONTROLLER_REJECTION,
        LIQUIDATE_ACCRUE_BORROW_INTEREST_FAILED,
        LIQUIDATE_ACCRUE_COLLATERAL_INTEREST_FAILED,
        LIQUIDATE_COLLATERAL_FRESHNESS_CHECK,
        LIQUIDATE_TUKTUK_CONTROLLER_REJECTION,
        LIQUIDATE_TUKTUK_CONTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED,
        ...
        LIQUIDATE_SEIZE_TUKTUK_CONTROLLER_REJECTION,
        LIQUIDATE_SEIZE_LIQUIDATOR_IS_BORROWER,
        LIQUIDATE_SEIZE_TOO_MUCH,
        MINT_ACCRUE_INTEREST_FAILED,
        MINT_TUKTUK_CONTROLLER_REJECTION,
        ...
        REDEEM_TUKTUK_CONTROLLER_REJECTION,
        ...
        REPAY_BORROW_TUKTUK_CONTROLLER_REJECTION,
        ...
        SET_COLLATERAL_FACTOR_OWNER_CHECK,
        SET_COLLATERAL_FACTOR_VALIDATION,
        SET_TUKTUK_CONTROLLER_OWNER_CHECK,
        ...
        TRANSFER_TUKTUK_CONTROLLER_REJECTION,
...
// File src/common/CarefulMath.sol
...
// File src/common/ExponentialNoError.sol
...
// File src/common/Exponential.sol
...
/**
 * @title TukTuk's TToken Contract
...
contract TToken is TTokenInterface, Exponential, TokenErrorReporter {
     ...
     */
    function initialize(TukTukControllerInterface tuktukController_,
        ...
        // Set the tuktukController
        uint err = _setTukTukController(tuktukController_);
        require(err == uint(Error.NO_ERROR), "setting tuktukController failed");

        // Initialize block number and borrow index (block number mocks depend on tuktukController being set)
     ...
     * @return Whether or not the transfer succeeded
     */
    function transferTTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        /* Fail if transfer not allowed */
        uint allowed = tuktukController.transferAllowed(address(this), src, dst, tokens);
        if (allowed != 0) {
            return failOpaque(Error.TUKTUK_CONTROLLER_REJECTION, FailureInfo.TRANSFER_TUKTUK_CONTROLLER_REJECTION, allowed);
        }
        ...
        // unused function
        // tuktukController.transferVerify(address(this), src, dst, tokens);
    ...
    function transfer(address dst, uint256 amount) external nonReentrant returns (bool) {
        return transferTTokens(msg.sender, msg.sender, dst, amount) == uint(Error.NO_ERROR);
    }
    ...
    function transferFrom(address src, address dst, uint256 amount) external nonReentrant returns (bool) {
        return transferTTokens(msg.sender, src, dst, amount) == uint(Error.NO_ERROR);
    }
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
        ...
        uint allowed = tuktukController.mintAllowed(address(this), minter, mintAmount);
        if (allowed != 0) {
            return (failOpaque(Error.TUKTUK_CONTROLLER_REJECTION, FailureInfo.MINT_TUKTUK_CONTROLLER_REJECTION, allowed), 0);
        }
        ...
        // unused function
        // tuktukController.mintVerify(address(this), minter, vars.actualMintAmount, vars.mintTokens);
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        ...
        uint allowed = tuktukController.redeemAllowed(address(this), redeemer, vars.redeemTokens);
        if (allowed != 0) {
            return failOpaque(Error.TUKTUK_CONTROLLER_REJECTION, FailureInfo.REDEEM_TUKTUK_CONTROLLER_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        tuktukController.redeemVerify(address(this), redeemer, vars.redeemAmount, vars.redeemTokens);
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
        ...
        uint allowed = tuktukController.borrowAllowed(address(this), borrower, borrowAmount);
        if (allowed != 0) {
            return failOpaque(Error.TUKTUK_CONTROLLER_REJECTION, FailureInfo.BORROW_TUKTUK_CONTROLLER_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        // unused function
        // tuktukController.borrowVerify(address(this), borrower, borrowAmount);
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
        ...
        uint allowed = tuktukController.repayBorrowAllowed(address(this), payer, borrower, repayAmount);
        if (allowed != 0) {
            return (failOpaque(Error.TUKTUK_CONTROLLER_REJECTION, FailureInfo.REPAY_BORROW_TUKTUK_CONTROLLER_REJECTION, allowed), 0);
        }
        ...
        /* We call the defense hook */
        // unused function
        // tuktukController.repayBorrowVerify(address(this), payer, borrower, vars.actualRepayAmount, vars.borrowerIndex);
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, TTokenInterface TTokenCollateral) internal returns (uint, uint) {
        ...
        uint allowed = tuktukController.liquidateBorrowAllowed(address(this), address(TTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
            return (failOpaque(Error.TUKTUK_CONTROLLER_REJECTION, FailureInfo.LIQUIDATE_TUKTUK_CONTROLLER_REJECTION, allowed), 0);
        }
        ...
        (uint amountSeizeError, uint seizeTokens) = tuktukController.liquidateCalculateSeizeTokens(address(this), address(TTokenCollateral), actualRepayAmount);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_TUKTUK_CONTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");
        ...
        /* We call the defense hook */
        // unused function
        // tuktukController.liquidateBorrowVerify(address(this), address(TTokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
     ...
     * @param seizeTToken The contract seizing the collateral (i.e. borrowed TToken)
     * @param liquidator The account receiving seized collateral
     * @param borrower The account having collateral seized
     * @param seizeTokens The number of TTokens to seize
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function seizeInternal(address seizeTToken, address liquidator, address borrower, uint seizeTokens) internal returns (uint) {
        /* Fail if seize not allowed */
        uint allowed = tuktukController.seizeAllowed(address(this), seizeTToken, liquidator, borrower, seizeTokens);
        if (allowed != 0) {
            return failOpaque(Error.TUKTUK_CONTROLLER_REJECTION, FailureInfo.LIQUIDATE_SEIZE_TUKTUK_CONTROLLER_REJECTION, allowed);
        }
        ...
        MathError mathErr;
        uint borroweTTokensNew;
        uint liquidatoTTokensNew;

        /*
         * We calculate the new borrower and liquidator token balances, failing on underflow/overflow:
         *  borroweTTokensNew = accountTokens[borrower] - seizeTokens
         *  liquidatoTTokensNew = accountTokens[liquidator] + seizeTokens
         */
        (mathErr, borroweTTokensNew) = subUInt(accountTokens[borrower], seizeTokens);
        if (mathErr != MathError.NO_ERROR) {
            return failOpaque(Error.MATH_ERROR, FailureInfo.LIQUIDATE_SEIZE_BALANCE_DECREMENT_FAILED, uint(mathErr));
        }

        (mathErr, liquidatoTTokensNew) = addUInt(accountTokens[liquidator], seizeTokens);
        if (mathErr != MathError.NO_ERROR) {
        ...
        /* We write the previously calculated values into storage */
        accountTokens[borrower] = borroweTTokensNew;
        accountTokens[liquidator] = liquidatoTTokensNew;
        ...
        /* We call the defense hook */
        // unused function
        // tuktukController.seizeVerify(address(this), seizeTToken, liquidator, borrower, seizeTokens);
    ...
    function _setTukTukController(TukTukControllerInterface newTukTukController) public returns (uint) {
        ...
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_TUKTUK_CONTROLLER_OWNER_CHECK);
        }

        TukTukControllerInterface oldTukTukController = tuktukController;
        // Ensure invoke tuktukController.isTukTukController() returns true
        require(newTukTukController.isTukTukController(), "marker method returned false");

        // Set market's tuktukController to newTukTukController
        tuktukController = newTukTukController;

        // Emit NewTukTukController(oldTukTukController, newTukTukController)
        emit NewTukTukController(oldTukTukController, newTukTukController);
...
// File src/interfaces/PriceOracle.sol
...
// File src/interfaces/IRewardToken.sol
...
// File src/controller/TukTukDelegateControllerAdminStorage.sol

pragma solidity ^0.5.16;


contract TukTukDelegateControllerAdminStorage {
    /**
    * @notice Administrator for this contract
    */
    ...
    */
    ...
    /**
    * @notice Active brains of TukTukDelegateController
    */
    address public tuktukControllerImplementation;

    /**
    * @notice Pending brains of TukTukDelegateController
    */
    address public pendingTukTukControllerImplementation;
}


// File src/controller/TukTukControllerStorage.sol

pragma solidity ^0.5.16;



contract TukTukControllerStorage is TukTukDelegateControllerAdminStorage {
...
// File src/controller/TukTukDelegateController.sol

pragma solidity ^0.5.16;


/**
 * @title TukTukControllerCore
 * @dev Storage for the tuktukController is at this address, while execution is delegated to the `tuktukControllerImplementation`.
 * CTokens should reference this contract as their tuktukController.
 */
contract TukTukDelegateController is TukTukDelegateControllerAdminStorage, TukTukControllerErrorReporter {

    /**
      * @notice Emitted when pendingTukTukControllerImplementation is changed
      */
    event NewPendingImplementation(address oldPendingImplementation, address newPendingImplementation);

    /**
      * @notice Emitted when pendingTukTukControllerImplementation is accepted, which means tuktukController implementation is updated
      */
    event NewImplementation(address oldImplementation, address newImplementation);
    ...
    function _setPendingImplementation(address newPendingImplementation) public returns (uint) {
        ...
        address oldPendingImplementation = pendingTukTukControllerImplementation;

        pendingTukTukControllerImplementation = newPendingImplementation;

        emit NewPendingImplementation(oldPendingImplementation, pendingTukTukControllerImplementation);

        return uint(Error.NO_ERROR);
    }

    /**
    * @notice Accepts new implementation of tuktukController. msg.sender must be pendingImplementation
    * @dev Admin function for new implementation to accept it's role as implementation
    * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
    */
    function _acceptImplementation() public returns (uint) {
        // Check caller is pendingImplementation and pendingImplementation ≠ address(0)
        if (msg.sender != pendingTukTukControllerImplementation || pendingTukTukControllerImplementation == address(0)) {
            return fail(Error.UNAUTHORIZED, FailureInfo.ACCEPT_PENDING_IMPLEMENTATION_ADDRESS_CHECK);
        }

        // Save current values for inclusion in log
        address oldImplementation = tuktukControllerImplementation;
        address oldPendingImplementation = pendingTukTukControllerImplementation;

        tuktukControllerImplementation = pendingTukTukControllerImplementation;

        pendingTukTukControllerImplementation = address(0);

        emit NewImplementation(oldImplementation, tuktukControllerImplementation);
        emit NewPendingImplementation(oldPendingImplementation, pendingTukTukControllerImplementation);
    ...
    function () payable external {
        ...
        (bool success, ) = tuktukControllerImplementation.delegatecall(msg.data);
...
// File src/controller/TukTukController.sol
...
/**
 * @title TukTuk's TukTukController Contract
 * @author TukTuk
 */
contract TukTukController is TukTukControllerStorage, TukTukControllerInterface, TukTukControllerErrorReporter, ExponentialNoError {
    ...
    event RewardSpeedUpdated(TToken indexed tToken, uint newSpeed);

    /// @notice Emitted when REWARD is distributed to a supplier
    event DistributedSupplierReward(TToken indexed tToken, address indexed supplier, uint rewardDelta, uint rewardSupplyIndex);

    /// @notice Emitted when REWARD is distributed to a borrower
    event DistributedBorrowerReward(TToken indexed tToken, address indexed borrower, uint rewardDelta, uint rewardBorrowIndex);
    ...
    function mintAllowed(address tToken, address minter, uint mintAmount) external returns (uint) {
        ...
        updateRewardSupplyIndex(tToken);
        distributeSupplierReward(tToken, minter);
    ...
    function redeemAllowed(address tToken, address redeemer, uint redeemTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updateRewardSupplyIndex(tToken);
        distributeSupplierReward(tToken, redeemer);
    ...
    function borrowAllowed(address tToken, address borrower, uint borrowAmount) external returns (uint) {
        ...
        updateRewardBorrowIndex(tToken, borrowIndex);
        distributeBorrowerReward(tToken, borrower, borrowIndex);
    ...
    function seizeAllowed(
        ...
        if (TToken(tTokenCollateral).tuktukController() != TToken(tTokenBorrowed).tuktukController()) {
            return uint(Error.TUKTUK_CONTROLLER_MISMATCH);
        }

        // Keep the flywheel moving
        updateRewardSupplyIndex(tTokenCollateral);
        distributeSupplierReward(tTokenCollateral, borrower);
        distributeSupplierReward(tTokenCollateral, liquidator);
     ...
     * @param transferTTokens The number of tTokens to transfer
    ...
    function transferAllowed(address tToken, address src, address dst, uint transferTTokens) external returns (uint) {
        ...
        uint allowed = redeemAllowedInternal(tToken, src, transferTTokens);
        if (allowed != uint(Error.NO_ERROR)) {
            return allowed;
        }
        ...
        // Keep the flywheel moving
        updateRewardSupplyIndex(tToken);
        distributeSupplierReward(tToken, src);
        distributeSupplierReward(tToken, dst);
    ...
    /**
      * @notice Sets a new price oracle for the tuktukController
    ...
    function _setPriceOracle(PriceOracle newOracle) public returns (uint) {
        ...
        // Track the old oracle for the tuktukController
        PriceOracle oldOracle = oracle;

        // Set tuktukController's oracle to newOracle
        oracle = newOracle;
    ...
    function _supportMarket(TToken tToken) external returns (uint) {
        ...
        markets[address(tToken)] = Market({isListed: true, collateralFactorMantissa: 0});
    ...
    function _become(TukTukDelegateController unitroller) public {
    ...
    function adminOrInitializing() internal view returns (bool) {
        return msg.sender == admin || msg.sender == tuktukControllerImplementation;
    }
     ...
     */
    function setRewardSpeedInternal(TToken tToken, uint rewardSpeed) internal {
        uint currentRewardSpeed = rewardSpeeds[address(tToken)];
        if (currentRewardSpeed != 0) {
            ...
            updateRewardSupplyIndex(address(tToken));
            updateRewardBorrowIndex(address(tToken), borrowIndex);
        } else if (rewardSpeed != 0) {
            // Add the REWARD market
            Market storage market = markets[address(tToken)];
            require(market.isListed == true, "market is not listed");

            if (rewardSupplyState[address(tToken)].index == 0 && rewardSupplyState[address(tToken)].block == 0) {
                rewardSupplyState[address(tToken)] = RewardMarketState({
                    index: rewardInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }

            if (rewardBorrowState[address(tToken)].index == 0 && rewardBorrowState[address(tToken)].block == 0) {
                rewardBorrowState[address(tToken)] = RewardMarketState({
                    index: rewardInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }
        ...
        if (currentRewardSpeed != rewardSpeed) {
            rewardSpeeds[address(tToken)] = rewardSpeed;
            emit RewardSpeedUpdated(tToken, rewardSpeed);
        }
    }
     ...
     */
    function updateRewardSupplyIndex(address tToken) internal {
        RewardMarketState storage supplyState = rewardSupplyState[tToken];
        uint supplySpeed = rewardSpeeds[tToken];
        ...
        if (deltaBlocks > 0 && supplySpeed > 0) {
            ...
            rewardSupplyState[tToken] = RewardMarketState({
     ...
     */
    function updateRewardBorrowIndex(address tToken, Exp memory marketBorrowIndex) internal {
        RewardMarketState storage borrowState = rewardBorrowState[tToken];
        uint borrowSpeed = rewardSpeeds[tToken];
        uint blockNumber = getBlockNumber();
        ...
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            ...
            rewardBorrowState[tToken] = RewardMarketState({
                index: safe224(index.mantissa, "new index exceeds 224 bits"),
                block: safe32(blockNumber, "block number exceeds 32 bits")
            });
        } else if (deltaBlocks > 0) {
     ...
     */
    function distributeSupplierReward(address tToken, address supplier) internal {
        RewardMarketState storage supplyState = rewardSupplyState[tToken];
        Double memory supplyIndex = Double({mantissa: supplyState.index});
        Double memory supplierIndex = Double({mantissa: rewardSupplierIndex[tToken][supplier]});
        rewardSupplierIndex[tToken][supplier] = supplyIndex.mantissa;
        ...
        uint supplieTTokens = TToken(tToken).balanceOf(supplier);
        uint supplierDelta = mul_(supplieTTokens, deltaIndex);
        uint supplierAccrued = add_(rewardAccrued[supplier], supplierDelta);
        rewardAccrued[supplier] = supplierAccrued;
        emit DistributedSupplierReward(TToken(tToken), supplier, supplierDelta, supplyIndex.mantissa);
    }
     ...
     */
    function distributeBorrowerReward(address tToken, address borrower, Exp memory marketBorrowIndex) internal {
        RewardMarketState storage borrowState = rewardBorrowState[tToken];
        Double memory borrowIndex = Double({mantissa: borrowState.index});
        Double memory borrowerIndex = Double({mantissa: rewardBorrowerIndex[tToken][borrower]});
        rewardBorrowerIndex[tToken][borrower] = borrowIndex.mantissa;
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            emit DistributedBorrowerReward(TToken(tToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }
     ...
     * @param holder The address to claim REWARD for
     * @param tTokens The list of markets to claim REWARD in
     */
    function claimReward(address holder, TToken[] memory tTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimReward(holders, tTokens, true, true);
    }
     ...
     */
    function claimReward(address[] memory holders, TToken[] memory tTokens, bool borrowers, bool suppliers) public {
        for (uint i = 0; i < tTokens.length; i++) {
            ...
            if (borrowers == true) {
                ...
                for (uint j = 0; j < holders.length; j++) {
                    distributeBorrowerReward(address(tToken), holders[j], borrowIndex);
            ...
            if (suppliers == true) {
                updateRewardSupplyIndex(address(tToken));
                for (uint j = 0; j < holders.length; j++) {
                    distributeSupplierReward(address(tToken), holders[j]);
                    rewardAccrued[holders[j]] = grantRewardInternal(holders[j], rewardAccrued[holders[j]]);
                }
            }
        }
     ...
     * @param rewardSpeed New REWARD speed for market
     */
    function _setRewardSpeed(TToken tToken, uint rewardSpeed) public {
        require(adminOrInitializing(), "only admin can set reward speed");
        setRewardSpeedInternal(tToken, rewardSpeed);
    }
