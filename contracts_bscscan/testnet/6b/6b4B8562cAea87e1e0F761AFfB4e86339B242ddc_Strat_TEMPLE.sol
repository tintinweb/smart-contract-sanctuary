...
import "./solar/ISolarVault.sol";

import "./solar/ISolarRouter01.sol";

import "./solar/ISolarRouter02.sol";
...
abstract contract Strat is Ownable, ReentrancyGuard, Pausable {
    ...
    function _farm() internal virtual {
        ...
        if (isCAKEStaking) {
            ISolarVault(farmContractAddress).enterStaking(wantAmt); // Just for CAKE staking, we dont use deposit()
        } else {
            ISolarVault(farmContractAddress).deposit(pid, wantAmt);
        }
    }

    function _unfarm(uint256 _wantAmt) internal virtual {
        if (isCAKEStaking) {
            ISolarVault(farmContractAddress).leaveStaking(_wantAmt); // Just for CAKE staking, we dont use withdraw()
        } else {
            ISolarVault(farmContractAddress).withdraw(pid, _wantAmt);
        }
    }
    ...
    function earn() public virtual nonReentrant whenNotPaused {
        ...
        if (token0Amt > 0 && token1Amt > 0) {
            ...
            ISolarRouter02(uniRouterAddress).addLiquidity(
                token0Address,
                token1Address,
                token0Amt,
                token1Amt,
    ...
    function _safeSwap(
        ...
        uint256[] memory amounts =
            ISolarRouter02(_uniRouterAddress).getAmountsOut(_amountIn, _path, 25);  // uint fee = 25 on solarRouter
        uint256 amountOut = amounts[amounts.length.sub(1)];

        ISolarRouter02(_uniRouterAddress)
