...
contract DuelEvent is Ownable {
    ...
    uint256 public tokenPerBlock = 1;
    ...
    // Winner pool id.
    uint256 public winnerPid;

    // Boolean to check if winner has been selected, as we can't check winnerPid != 0.
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        require(!winnerSelected || _pid != winnerPid, "DuelEvent: Unable to withdraw from winner pool");
        require(_amount > 0, "DuelEvent: Zero amount");
    ...
    function claimAndExit(uint256 _pid) external {
        ...
        require(_pid == winnerPid, "DuelEvent: Can claim only from winner pool");
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accTokenPerShare).div(1e12).add(user.pendingReward);
    }
    ...
    function setStartBlock(uint256 _block) external onlyOwner {
        require(_block > block.number, "DuelEvent: Event already started");
        ...
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            if (pid != winnerPid) {
                ...
                _pool.lastRewardBlock = startBlock;
            }
        }
    }
    ...
    function setEndBlock(uint256 _block) external onlyOwner {
        require(_block > startBlock, "DuelEvent: Wrong end block");
        endBlock = _block;
    }
...
abstract contract TransactionThrottler is Ownable {
    ...
    modifier transactionThrottler(
        ...
        if (_tradingStart > block.timestamp) {
            require(_isWhitelisted[sender] || _isWhitelisted[recipient], "Protection: Transfers disabled");
