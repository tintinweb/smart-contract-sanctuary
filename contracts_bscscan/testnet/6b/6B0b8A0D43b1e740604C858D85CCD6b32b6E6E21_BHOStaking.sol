...
contract BHOStaking is OwnableUpgradeable {
        ...
        returns (uint256 amount, uint256 duration)
    {
        ...
        if (userInfo.amount > 0) {
            ...
            uint256 pending = _calInterestBHO(userInfo.amount, userInfo.apy);
            if (pending > 0) {
                bho.transfer(user, userInfo.amount.add(pending));
            }
        }
    ...
    function emergencyUnstake() external {
        ...
        uint256 total = userInfo.amount.mul(unstakeFee).div(100);
    ...
    function pendingBHO(address _user) external view returns (uint256) {
        ...
        _calInterestBHO(userInfo.amount, userInfo.apy);
    }

    /******************************************* Internal function below *******************************************/
    function _calInterestBHO(uint256 _amount, uint256 _apy)
        internal
        pure
        returns (uint256)
    {
        uint256 pending = _amount.mul(_apy);
        return pending;
    }
}
