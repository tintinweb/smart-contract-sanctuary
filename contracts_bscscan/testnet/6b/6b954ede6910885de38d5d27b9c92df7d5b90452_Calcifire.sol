...
contract Calcifire is Context, IERC20, Ownable {
    ...
    //token-bnb pair by token symbol - used by Boosts functionality
    mapping (string => address) private _tokenBNBPair;
    ...
    // Set token-bnb pair - used by Boosts functionality
    function setTokenBNBPair(address pairAddress, string memory tokenSymbol) public onlyOperator {
        require(_tokenBNBPair[tokenSymbol] != pairAddress, "setTokenBNBPair: pair address is already set for this token");
        _tokenBNBPair[tokenSymbol] = pairAddress;
    }

    // Get token-usd pair by token symbol
    function getTokenBNBPair(string memory tokenSymbol) public view returns(address) {
        return(_tokenBNBPair[tokenSymbol]);
    }

    // check native pool amount in USD
    function userPoolAmountUSD(uint256 amount, address lpAddress, uint256 lpTotalSupply, string memory quoteToken) public view returns(uint256) {
        require(getTokenUSDPair("BNB") != address(0x0), "checkPoolBoostLimits: BNB-BUSD pair is mandatory");

        uint256 userLPinQuote = 0;
        ...
        if (lpAddress == address(this)) {
          ...
          IERC20 nativeQuoteToken = IERC20(nativePair.token1());
          string memory quoteTokenSymbol = nativeQuoteToken.symbol();

          if (keccak256(bytes(quoteTokenSymbol)) == keccak256(bytes("BUSD"))) {
            nativePriceUSD = getTokenPrice(address(nativePair), 1);
          }
          else if (keccak256(bytes(quoteTokenSymbol)) == keccak256(bytes("WBNB"))) {
            uint256 priceinBNB = getTokenPrice(address(nativePair), 1);
            nativePriceUSD = priceinBNB.div(decimals()).mul(priceBNBinUSD);
          }

          poolAmountInUSD = amount.mul(nativePriceUSD.div(decimals()));
        ...
        IERC20 token1 = IERC20(pair.token1());
        totalTokens = token1.balanceOf(lpAddress);
        userLPinQuote = amount.div(lpTotalSupply);
        userLPinQuote = userLPinQuote.mul(totalTokens).mul(2);

        if (keccak256(bytes(quoteToken)) == keccak256(bytes("BUSD"))) {
          poolAmountInUSD = userLPinQuote;
        }
        else if (keccak256(bytes(quoteToken)) == keccak256(bytes("BNB"))) {

          poolAmountInUSD = userLPinQuote.mul(priceBNBinUSD);
          poolAmountInUSD = poolAmountInUSD.div(decimals());
        }
        else {
          string memory quoteTokenSymbol = token1.symbol();
          uint256 priceQuoteinUSD = 0;
          uint256 priceQuoteinBNB = 0;
          quoteTokenLP = getTokenUSDPair(quoteTokenSymbol);
          if(quoteTokenLP != address(0x0)) {
            priceQuoteinUSD = getTokenPrice(quoteTokenLP, 1);
            poolAmountInUSD = userLPinQuote.mul(priceQuoteinUSD);
            poolAmountInUSD = poolAmountInUSD.div(decimals());
          }
          else {
            quoteTokenLP = getTokenBNBPair(quoteTokenSymbol);
            if(quoteTokenLP != address(0x0)) {
              priceQuoteinBNB = getTokenPrice(quoteTokenLP, 1);
              priceQuoteinUSD = priceQuoteinBNB.mul(priceBNBinUSD.div(decimals()));
