...
contract ArtOnlinePlatform is Context, ERC165, EIP712Base, IERC1155, IERC1155MetadataURI, Pausable, ArtOnlinePlatformStorage {
  ...
  function addItem(string memory name, uint256 cap_, uint256 bonus, uint256 halving) public onlyRole(DEFAULT_ADMIN_ROLE) {
    require(itemIds[name] == 0, 'item exists');
    items.push(name);
    ...
    function _calculateReward(address sender, uint256 id) internal returns (uint256) {
      ...
      if (block.timestamp > startTime + _blockTime) {
        unchecked {
          ...
          _rewards[id][sender] = _rewards[id][sender] + (reward * remainder);
        }
        _startTime[id][sender] = block.timestamp;
      }
