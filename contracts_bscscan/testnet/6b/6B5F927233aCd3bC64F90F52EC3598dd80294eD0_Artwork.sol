...
contract Artwork is ERC721Enumerable, Ownable {
    ...
    uint256 private blindBoxOpenFee;
    uint256 private blindBoxBuyLimit;

    Counters.Counter private _tokenIdTracker;
    constructor() ERC721("Cryptopop Artwork", "Artwork") {
    }

    function _baseURI() internal override pure returns (string memory _newBaseURI) {
        return "https://cryptopop.io/";
    }
    ...
    function pop(address[] storage array, address element) private {
        for (uint256 index = 0; index < array.length; index++) {
            if (array[index] == element) {
    ...
    function random(uint256 _limit) private view returns (uint256) {
        return uint256(
            uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty))) % _limit
        );
    }
    ...
    function mintArtwork() public onlyArtist returns(string memory) {
    ...
    function buyBlindBox(uint256 _quantity) public 
    ...
    modifier onlyArtist() {
        ...
        require(isArtist, "Only artist can call this function");
        _;
    }

    modifier underQuantityLimit(uint256 _quantity) {
        require( _quantity <= blindBoxBuyLimit, "Quantity must be less than or equal to the buy limit");
        _;
    }

    modifier underBlindBoxAmount(uint256 _amount) {
        require(_amount <= blindBox[msg.sender], "Not enough blind boxes to open");
        _;
    }
