...
contract ClockAuctionBase {
    ...
    address public minoWarToken = 0x2C5876eaCE629d49dafa0d797faAfBf70c7Aa178;
    ...
    event AuctionCreated(uint256 tokenId, uint256 startingPrice, uint256 duration, address currentBidder);
        ...
        returns (uint256)
    {
        ...
        require(IERC20(minoWarToken).transferFrom(address(this), auction.currentBidder, _bidAmount));
        auction.currentBidder = msg.sender;
        auction.endingPrice = uint128(_bidAmount);

        require(IERC20(minoWarToken).transferFrom(msg.sender, address(this), _bidAmount));
...
contract ClockAuction is Pausable, ClockAuctionBase, IClockAuction {
    ...
    function withdrawBalance() external onlyOwner {
        ...
        IERC20(minoWarToken).transfer(owner(), getBalance());
    }
    ...
    function getBalance() view public returns(uint256) {
        return IERC20(minoWarToken).balanceOf(address(this));
    }
    ...
    function bid(uint256 _tokenId, uint256 _amount)
        ...
        whenNotPaused
    {
        // require(IERC20(minoWarToken).transfer(address(this), _amount));
...
contract MinoWarClockAuction is ClockAuction {
    ...
    {
        require(IERC20(minoWarToken).transferFrom(_msgSender(), address(this), _amount));
    ...
    function claimNFT(uint256 _tokenId) external {
        Auction storage auction = tokenIdToAuction[_tokenId];
        require(block.timestamp > auction.duration, "waiting auction ended");
        require(_msgSender() == auction.currentBidder, "not the winner");
        uint256 cutAmount = _computeCut(auction.endingPrice);
        require(IERC20(minoWarToken).transfer(auction.seller, cutAmount));
