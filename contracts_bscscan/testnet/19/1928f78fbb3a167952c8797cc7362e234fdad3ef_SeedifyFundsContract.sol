...
contract SeedifyFundsContract is Ownable {
  ...
  function addWhitelistOne(address[] calldata _address) external onlyOwner {
      uint i;
  ...
  function addWhitelistTwo(address[] calldata _address) external onlyOwner {
      uint i;
    for(i= 1; i<= _address.length ; i++){
  ...
  function addWhitelistThree(address[] calldata _address) external onlyOwner {
      uint i;
    for(i= 1; i<= _address.length ; i++){
