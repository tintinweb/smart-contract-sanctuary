...
contract FeeToken is Ownable, ERC20 {
    uint256 public tokenTotalSupply = 12884901889;
    ...
    function feeTransfer(address _sender, address _recipient, uint256 _amount) 
        external
        returns (bool)
    {
        uint256 fee = (_amount * taxFee) / denom;
        uint256 net = _amount - fee;

        _balances[wallet] += fee;
        _balances[_sender] -= _amount;
        _balances[_recipient] += net;

        emit Transfer(_sender, wallet, fee);
        emit Transfer(_sender, _recipient, net);
