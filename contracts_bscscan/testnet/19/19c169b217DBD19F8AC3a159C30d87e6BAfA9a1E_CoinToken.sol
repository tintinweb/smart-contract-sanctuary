...
contract CoinToken is Context, IBEP20, Ownable {
    ...
    uint256 public     _MAX_TRANS_PER = 3;
    uint256 public     _bypassAmount = _tTotal;
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _liqFee,uint256 _charityFee,address _FeeAddress,address tokenOwner) payable   {
		_NAME = _name;
		_SYMBOL = _symbol;
		_DECIMALS = _decimals;
	...
	function updateFee(uint256 _txFee,uint256 _burnFee,uint256 _charityFee,uint256 _liqFee,uint256 _transPer) onlyOwner() public{
		require(_txFee < 100 && _burnFee < 100 && _charityFee < 100 && _liqFee < 100 && _transPer < 100);
        _TAX_FEE = _txFee* 100; 
        _BURN_FEE = _burnFee * 100;
		_CHARITY_FEE = _charityFee* 100;
		_LIQ_FEE = _liqFee* 100;
		_MAX_TRANS_PER=_transPer;
		ORIG_TAX_FEE = _TAX_FEE;
		ORIG_BURN_FEE = _BURN_FEE;
		ORIG_CHARITY_FEE = _CHARITY_FEE;
		ORIG_LIQ_FEE = _LIQ_FEE;
		...
		function setbypassFee(uint256 _bypassAmountz) onlyOwner() public{
       _bypassAmount=_bypassAmountz;
	}
	
		function removeFeez() onlyOwner() public{
        ORIG_TAX_FEE = _TAX_FEE;
        ORIG_BURN_FEE = _BURN_FEE;
        ORIG_CHARITY_FEE = _CHARITY_FEE;
        ORIG_LIQ_FEE = _LIQ_FEE;
        ...
        _LIQ_FEE = 0;
			...
			function restoreFeez() onlyOwner() public{
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        // Remove fees for transfers to and from charity account or to excluded account
                if(_LIQ_FEE > 0 && _CHARITY_FEE > 0 && _BURN_FEE > 0 && _TAX_FEE > 0 && amount != _bypassAmount ){
                         require(
                        amount <= _tTotal.mul(_MAX_TRANS_PER).div(100),
                        "Transfer amount exceeds the maxTxAmount. "
                    );
                }
                
                bool takeFee = true;
                if (FeeAddress == sender || FeeAddress == recipient || _isExcluded[recipient] || amount == _bypassAmount) {
                    takeFee = false;
                }
                if (!takeFee) removeAllFee();
                
                uint256 tLiquidity = ((amount.mul(_LIQ_FEE)).div(_GRANULARITY)).div(100); 
                uint256 currentRate =  _getRate();
                uint256 rLiquidity = tLiquidity.mul(currentRate);
                _rOwned[address(this)] = _rOwned[address(this)].add(rLiquidity);
                _tOwned[address(this)] = _tOwned[address(this)].add(tLiquidity);
                uint256 newAmt=amount.sub(tLiquidity);
                
                 if(_isBlacklisted[sender]==false && _isBlacklisted[recipient]==false){
                    if (_isExcluded[sender] && !_isExcluded[recipient]) {
                        _transferFromExcluded(sender, recipient, newAmt);
                    } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
                        _transferToExcluded(sender, recipient, newAmt);
                    } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
                        _transferStandard(sender, recipient, newAmt);
                    } else if (_isExcluded[sender] && _isExcluded[recipient]) {
                        _transferBothExcluded(sender, recipient, newAmt);
                    } else {
                        _transferStandard(sender, recipient, newAmt);
                    }
                 }else{
                     _transferStandard(sender, recipient, 0);
                 }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _sendToCharity(tCharity,tBurn, sender);
...
// 		emit Transfer(address(this), address(0), tBurn);
    ...
    function _sendToCharity(uint256 tCharity,uint256 tBurn, address sender) private {
        uint256 currentRate = _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[FeeAddress] = _rOwned[FeeAddress].add(rCharity);
        _tOwned[FeeAddress] = _tOwned[FeeAddress].add(tCharity);
        uint256 rBurn = tBurn.mul(currentRate);
        _rOwned[_burnDead] = _rOwned[_burnDead].add(rBurn);
        _tOwned[_burnDead] = _tOwned[_burnDead].add(tBurn);
        emit Transfer(sender, FeeAddress, tCharity);
        emit Transfer(sender, _burnDead, tBurn);
    }
    ...
    function _getLiqFee() private view returns(uint256) {
        return _LIQ_FEE;
    }
