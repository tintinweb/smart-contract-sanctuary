...
contract Contract is ERC20WithReflection, Ownable {
    ...
    function _allowedTxAmount(address from) private view returns(uint256) {
        ...
        } else if(((block.timestamp - userOperations.lastSellDate) > (20 minutes)) || 
            (userOperations.lastSellDate == 0 && ((block.timestamp - userOperations.firstBuyDate) > 20 minutes))){
            return balanceOf(from);
        } else if((block.timestamp - userOperations.lastSellDate) > (10 minutes)){
