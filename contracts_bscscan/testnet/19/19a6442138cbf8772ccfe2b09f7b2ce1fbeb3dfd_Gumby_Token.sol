...
contract Gumby_Token is Context, IERC20, Ownable {
    ...
    */
    ...
    for (uint256 i; i < addresses.length; ++i) {
        while(gasUsed < gasleft()) {
        startGas = gasleft();
        require(!_isWhitelisted[addresses[i]], "Account is already whitelisted");
    ...
    for (uint256 i; i < addresses.length; ++i) {
        while(gasUsed < gasleft()) {
        startGas = gasleft();
        require(_isWhitelisted[addresses[i]], "Account is not whitelisted");
    ...
    for (uint256 i; i < addresses.length; ++i) {
        while(gasUsed < gasleft()) {
        startGas = gasleft();
        require(!_isBlacklisted[addresses[i]], "Account is already blacklisted");
    ...
    for (uint256 i; i < addresses.length; ++i) {
        while(gasUsed < gasleft()) {
        startGas = gasleft();
        require(_isBlacklisted[addresses[i]], "Account is not blacklisted");
