...
import "../../V4/farming/GreenHouseV4.sol";
import "hardhat/console.sol";


contract GreenHouseV5 is GreenHouseV4 {
    using SafeMath for uint256;

    /// @notice request to convert back TREE token to LEAF without lock peroid verify from oracle
    /// @dev only do validation for Tree balance and allowance if sufficient request will be created
    function burnTree(uint256 burnTreeAmount) external {
        require(burnTreeAmount > 0,"GreenHouse: amount must more than zero");
        require(tree.balanceOf(msg.sender) >= burnTreeAmount,"GreenHouse: tree balance is not sufficient");

        uint256 convertBackLeaf = burnTreeAmount.mul(fertilizedTokenPerTree);
        tree.transferFrom(msg.sender, address(this), burnTreeAmount);
        tree.burn(burnTreeAmount);
        fertilizedToken.transfer( msg.sender, convertBackLeaf);
        emit BurnCompleted( msg.sender, burnTreeAmount, convertBackLeaf, 0);
    }
