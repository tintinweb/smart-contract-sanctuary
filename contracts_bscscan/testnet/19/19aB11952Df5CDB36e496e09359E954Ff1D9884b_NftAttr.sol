...
contract NftAttr is Ownable, Context {
  ...
  */
  ...
  function OwnerSafeWithdrawalToken(address token_address, uint256 amount) public onlyOwner{
   ...
   function bulkGetNftInfo(uint256[] calldata tokenIds)public view returns (NftInfo[] memory ret) {
     ret = new NftInfo[](tokenIds.length);
     for(uint256 i = 0; i < tokenIds.length; i++) {
          NftInfo memory n = NftInfoMap[tokenIds[i]];
          ret[i] = NftInfo(
              string(abi.encodePacked(n.uri)),
              n.tokenId,
              n.templateId,
              n.typeId,
              n.atk,
   ...
   function tokenOfOwnerGetAndInfo(address _owner) public view returns (NftInfo[] memory ret){
       uint256[] memory tokenIds = nftContract.tokenOfOwnerGet(_owner);
      uint256 len = nftContract.balanceOf(_owner);
