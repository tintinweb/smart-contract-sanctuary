...
contract IMOv2 is Initializable, OwnableUpgradeable, IMO{
    ...
    function migrateSetUnlockTimestamp(
        address[] memory _users,
        uint256 _unlockTimestamp
    ) public onlyOwner isMigrateable{
        for(uint256 i=0;i<_users.length;i++){
...
abstract contract IMO is IIMO, Initializable, OwnableUpgradeable, PausableUpgradeable{
    ...
    function setStakeToken(IERC20 token) external onlyOwner whenPaused{
        stakeToken = token;
    }

    function setStakeWrapToken(IERC20 token) external onlyOwner whenPaused{
    ...
    function setDepositWrapFee(uint256 fee) external onlyOwner whenPaused{
        depositWrapFee = fee;
    }
    ...
    function setWithdrawWrapFee(uint256 fee) external onlyOwner whenPaused{
        withdrawWrapFee = fee;
    }

    function setPenaltyTime(uint256 time) external onlyOwner whenPaused{
        penaltyTime = time;
    }
    ...
    function setReserveWallet(address _wallet) external onlyOwner whenPaused{
        reserveWallet = _wallet;
    }

    function setMMUSD(address _mmUSD) external onlyOwner whenPaused{
        mmUSD = IERC20(_mmUSD);
    }
    ...
    function _lockPenalty() private whenNotPaused{
    ...
    function depositWrap(uint256 _amount) external whenNotPaused{
        require(_amount > 0, "Amount must be > 0.");
    ...
    function _reserveWithdraw(uint256 _amount) private whenNotPaused returns(uint256){
        uint256 balance = stakeToken.balanceOf(reserveWallet);
        if(balance == 0)return _amount;
        else if(balance >= _amount){
            stakeToken.safeTransferFrom(reserveWallet, msg.sender, _amount);
            return 0;
        }
        else{
            stakeToken.safeTransferFrom(reserveWallet, msg.sender, balance);
    ...
    function withdraw(uint256 _amount) external whenNotPaused{
        ...
        uint256 _amountLeft = _amountAfterFee;
        if(reserveWallet != address(0)){
            _amountLeft = _reserveWithdraw(_amountLeft);
        }
        if(_amountLeft > 0){
            stakeToken.safeTransfer(msg.sender, _amountLeft);
        }
    ...
    function withdrawWrap(uint256 _amount) external whenNotPaused{
        require(_amount > 0, "Amount must be > 0.");
    ...
    // Old commit function using current factoryVersion
    ...
    function commitFromBUSD(uint256 imoIndex, uint256 amount) external whenNotPaused{
        ...
        mmUSD.safeTransfer(msg.sender, mmUSDAmount);
    ...
    // Old normal claim with current Factory Version
    function claim(uint256 imoIndex) external whenNotPaused{
        IIMOClonable imo = getImo(imoIndex);
