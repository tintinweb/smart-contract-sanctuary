...
contract DinoLandNFTUpgradeable is ERC721Upgradeable, DinosAccessControl {
     ...
     function initialize() initializer public {
         __ERC721_init("DinosLandNFT", "DINO");
         //Set CEO
         ceoAddress = msg.sender;
        //Genesis Dino
        dinos.push(Dino(0, block.timestamp, block.timestamp));
        //Create some dinos for testing
        _createDino(1111, msg.sender, block.timestamp); // Normal Novis
        _createDino(1212, msg.sender, block.timestamp); // Rare Aquis
        _createDino(1313, msg.sender, block.timestamp); // Super Rare Terrot
        _createDino(1313, msg.sender, block.timestamp); // Super Rare Terrot
        _createDino(1314, msg.sender, block.timestamp); // Lengendary Terrot
        _createDino(1315, msg.sender, block.timestamp); // Mystic Terrot
     }
    ...
    */
    ...
    //Create new dino from egg genes    
    function createDinoFromEggGenes(uint256 _eggGenes, address _owner) external noContract onlySpawner returns(uint256) {
         uint newGenes = _getRandomDinoGenesFromEgg(_eggGenes);
         uint256 newDinoId = _createDino(newGenes, _owner, block.timestamp);
         return newDinoId;
    }
    
    //Generate random percent
    function _getRandomPercent() private returns(uint _rand) {
        uint rand = uint(keccak256(abi.encodePacked(msg.sender, block.timestamp, block.difficulty, "DinoLand", randomCount++)))%100 + 1;
        return rand;
    }
    
    uint256 public normalRate = 53;
    uint256 public rareRate = 30;
    uint256 public superRareRate = 10;
    uint256 public lengendaryRate = 5;
    uint256 public mysticRate = 2;
    
    //Set hatching Egg rarirty rate - only for CLevel
    function setHatchingEggRate(uint256 _normalRate, uint256 _rareRate, uint256 _superRareRate, uint256 _lengendaryRate, uint256 _mysticRate) external onlyCLevel {
        require(_normalRate + _rareRate + _superRareRate + _lengendaryRate + _mysticRate == 100, "Total percent is not 100");
        normalRate = _normalRate;
        rareRate = _rareRate;
        superRareRate = _superRareRate;
        lengendaryRate = _lengendaryRate;
        mysticRate = _mysticRate;
    }
    
    //Generate dino genes depend on rarirty rate    
    function _getRandomDinoGenesFromEgg(uint _eggGenes) private returns(uint _randomGenes) {
        ...
        if(randRareNumber <= normalRate) {
            return _eggGenes*100 + 11;
        } else if(randRareNumber <= normalRate + rareRate) {
            return _eggGenes*100 + 12;
        } else if(randRareNumber <= normalRate + rareRate + superRareRate) {
            return _eggGenes*100 + 13;
        } else if(randRareNumber <= normalRate + rareRate + superRareRate + lengendaryRate) {
