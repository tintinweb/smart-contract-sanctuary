...
contract OVO  is Context, IERC20, Ownable {
    ...
    string private _name = "OVO";
    string private _symbol = "OVO";
    ...
    uint256 public _platformFee = 18;
...
*/
    ...
    constructor () public {
        ...
        _charityWallet = owner();
        _platformWallet = owner();
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        //(uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tPlatform, uint256 tCharity) = _getValues(tAmount);
        uint256[9] memory values = _getValues(tAmount);
    ...
    function setPlatformAddress(address newAddress) external onlyOwner() {
        _platformWallet = newAddress;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256[9] memory values) {
        // (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tPlatform, uint256 tCharity) = _getTValues(tAmount);
        ...
        // @return rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tBurn, tPlatform, tCharity
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256[6] memory tValues) {
        ...
        uint256 tPlatform = _calculatePlatformFee(tAmount);            // marketing token to collect    
        uint256 tCharity = calculateCharityFee(tAmount);          
        uint256 tempTransferAmount = tAmount.sub(tFee).sub(tLiquidity);
        uint256 tTransferAmount = tempTransferAmount.sub(tBurn).sub(tPlatform).sub(tCharity);
        return [tTransferAmount, tFee, tLiquidity, tBurn, tPlatform, tCharity];
    }
    
    function _getRValues(uint256[6] memory tValues, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        //function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, tBurn, tPlatform, tCharity uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function _takePlatform(uint256 tPlatform) private {
        if(tPlatform == 0) return;
        _transfer(_msgSender(), _platformWallet, tPlatform);
    }
    ...
    function _calculatePlatformFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_platformFee).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _platformFee == 0 && _charityFee == 0 ) return;
