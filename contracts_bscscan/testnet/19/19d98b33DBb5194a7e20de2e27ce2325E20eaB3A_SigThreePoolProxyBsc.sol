...
import "@openzeppelin/contracts/math/Math.sol";
//import "./SigTokenBsc.sol";
import "./CashBackBsc.sol";

contract SigThreePoolProxyBsc {
    ...
    uint256 constant MAX_FEE = 5 * 10 ** 9;        // 0.5% = 0.5*1e10
    uint256 public fee; // 0.024% = 0.00024*1e10 = 2400000,
    // current              24,000,000
    // new fee               2,400,000
    ...
    uint256 public slot_fill_2;
    uint256 public slot_fill_3;

    // following variables aren't part of proxy implementation so they go after all 3pool's variables
    // constants
    address public immutable DELEGATION_TARGET;
    SigTokenBsc public immutable SIG_TOKEN;
    // variables
    address public dutchAuction;
    CashBackBsc public cashBack;
    ...
    // additional variables
        ...
        //
        DELEGATION_TARGET = _delegationTarget;
    ...
    fallback () external {
        _delegate(DELEGATION_TARGET);
    }

    receive () external payable {}

    // this function can be called by anyone, but fees go directly to the auction
    function withdraw_admin_fees_to_auction() public {
        for (uint256 i = 0; i < N_COINS; i++) {
            IERC20 coin = IERC20(coins[i]);
            uint256 value = coin.balanceOf(address(this)) - balances[i];
            if (value > 0) {
                coin.safeTransfer(dutchAuction, value);
            }
        }
    }

    function changeAuction(address newAuction) external {
        require(msg.sender == owner);
        dutchAuction = newAuction;
    }

    function changeCashback(CashBackBsc newCashback) external {
        require(msg.sender == owner);
        cashBack = newCashback;
    }

    // Same as function from proxy implementation
    // def exchange(i: int128, j: int128, dx: uint256, min_dy: uint256):
    // but adds possible cashback
    function exchange2(int128 i, int128 j, uint256 dx, uint256 min_dy) public {
        (bool success, bytes memory result) = DELEGATION_TARGET.delegatecall(
            abi.encodeWithSignature("exchange(int128,int128,uint256,uint256)", i, j, dx, min_dy)
            // signature of exchange(int128,int128,uint256,uint256) - 0x3df02124, but should be optimized anyway
        );
        if (!success) {
            if (result.length > 0) {
        ...
        // pay cashback
        cashBack.payCashbackEther(i, dx);
    }
    ...
    function donate_admin_fees2() external {
        require(msg.sender == owner);
        for (uint256 i = 0; i < N_COINS; i++) {
