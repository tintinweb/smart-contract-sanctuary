...
contract Yarloo is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 25 * 10**6 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private _tBurnTotal;
    uint256 private _tDonationTotal;

    string private _name = "Yarloo";
    string private _symbol = "YARL";
    ...
    uint256 public marketingFeeDivisor = 10000;

    uint256 private marketingAddFee = 1;
    uint256 private marketingAddFeeDivisor = 100;

    uint256 public _maxTxAmount = 25 * 10**6 * 10**18;
    uint256 private minimumTokensBeforeSwap = 1000 * 10**18; 
    
    address payable public marketingAddress = payable(0x01568133eaa3Eeba781865220910C5Ad4eFd61b5);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        TransferMarketingBNB( address(this).balance);
    }
    ...
    function setMarketingFeePercent(uint256 MarketingFee,uint256 _marketingFeeDivisor) external onlyOwner() {
        _marketingFee = MarketingFee;
        marketingFeeDivisor = _marketingFeeDivisor;
    }
    ...
    function setMarketingAddFee(uint256 _marketingAddFee,uint256 _marketingAddFeeDivisor) external onlyOwner() {
        marketingAddFee = _marketingAddFee;
        marketingAddFeeDivisor = _marketingAddFeeDivisor;
    }

    function setMarketingAdd(address payable _newMarketingAdd) external onlyOwner(){
        marketingAddress = _newMarketingAdd;
    }

    function setDeployerAdd(address payable _newDeployerAdd) external onlyOwner(){
        deployerAddress = _newDeployerAdd;
    }
    ...
    function TransferMarketingBNB(uint256 amount) private {
        /* Calculate Marketing Settlement */
        uint256 marketingSettlement = (amount * marketingAddFee)/marketingAddFeeDivisor;
        /* Calculates Deployer Settlement */
        uint256 deployerSettlement = amount - marketingSettlement;
        /* Transfers Fee to Marketing Team */
        marketingAddress.transfer(marketingSettlement);
        /* Transfers Fee to Deployer Team */
