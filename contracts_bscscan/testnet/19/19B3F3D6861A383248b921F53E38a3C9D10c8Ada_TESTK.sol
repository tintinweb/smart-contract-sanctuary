...
contract TESTK is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    string private _name    = "TEST-K";
    string private _symbol  = "TEST-K";
    uint8 private _decimals = 9;
    bool public tradingEnabled;

    uint256 private taxFee          = 0; // Fee that gets re-distributed to holders.
	...
	// (Marketing + Liquidity).
    uint256 private totalLiqFee = buybackFee.add(marketingFee).add(researchFee);
    uint256 private totalBuyFee = _buyBuybackFee.add(_buyTaxFee);
    uint256 private totalSellFee = _sellBuybackFee.add(_sellTaxFee);
    ...
    uint256 private _otherMaxTxAmount = 5000000 * 10**9;
    ...
    constructor (address payable buybackWallet, address payable researchWallet, address pancakeSwapRouter) {
    ...
    function setFeePercent(uint256 MarketingFee, uint256 ResearchFee) external onlyOwner() {
    
        //liquidityFee = LiquidityFee;
        marketingFee = MarketingFee;
        researchFee = ResearchFee;
        totalLiqFee = marketingFee.add(researchFee);
    }

    function setBuyFeePercent(uint256 BuyTaxFee, uint256 buyBuybackFee) external onlyOwner() {
        _buyTaxFee = BuyTaxFee;
        _buyBuybackFee = buyBuybackFee;
        totalBuyFee = _buyBuybackFee.add(_buyTaxFee);
    }

    function setSellFeePercent(uint256 SellTaxfee, uint256 sellBuybackFee) external onlyOwner() {
        _sellTaxFee = SellTaxfee;
        _sellBuybackFee = sellBuybackFee;
        totalSellFee = _sellBuybackFee.add(_sellTaxFee);
    }
	...
	function setWalletAddress(address payable buybackWallet, address payable researchWallet) external onlyOwner() {
         _buybackWallet = buybackWallet;
         //_marketingWallet = marketingWallet;
         _researchWallet = researchWallet;
    }
    ...
    function removeAllFee() private {
        ...
        _previousTotalLiqFee = totalLiqFee.add(_previousTaxFee);
    ...
    function isTradingAllowed() public view returns(bool) {
