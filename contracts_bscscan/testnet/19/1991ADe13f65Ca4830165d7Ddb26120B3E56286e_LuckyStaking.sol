...
contract LuckyStaking {
    ...
    address public owner=0x75053E382513d0514585e913175306B2df3C685a;
    ...
    function getEstimatedPayout(address user) public view returns (uint256){
            ...
            uint256 reward=daysPassed*(((CNR.balanceOf(address(this))-totalDeposits)*poolShare[user])/divideByDays);
    ...
    function claimDividends() public{
        if((lastClaimed[msg.sender]+28800)<block.number){
            ...
            uint256 reward=daysPassed*(((CNR.balanceOf(address(this))-totalDeposits)*poolShare[msg.sender])/divideByDays);
            uint256 rewardPostFees=reward-((reward/100)*(ownerFee+dividendFee));
    ...
    function reinvestDividends() public {
            ...
            uint256 reward=daysPassed*((CNR.balanceOf(address(this))-totalDeposits)*poolShare[msg.sender]);
            totalDeposits+=reward;
            stake[msg.sender]+=reward;
            poolShare[msg.sender]=stake[msg.sender]/totalDeposits;
    ...
    function removeStake(uint256 percentage) public{
        
        uint256 amountToRemove=((percentage/100)*stake[msg.sender]);
        removeStakeHistory.push(amountToRemove);
        removeStakeHistoryAddress.push(msg.sender);
        removeStakeHistoryTS.push(block.number);
        uint256 amountToRemovePostFees=amountToRemove-((amountToRemove/100)*(ownerFee+dividendFee));
        uint256 devFee=((amountToRemove/100)*ownerFee);
        CNR.transfer(owner,devFee);
        CNR.transfer(msg.sender,amountToRemovePostFees);
        totalDeposits-=amountToRemove;
        stake[msg.sender]-=amountToRemove;
        poolShare[msg.sender]=stake[msg.sender]/totalDeposits;
