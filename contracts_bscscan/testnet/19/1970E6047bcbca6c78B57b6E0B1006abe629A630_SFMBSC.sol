...
contract SFMBSC is Context, IERC20, Ownable {
    ...
    uint256 private numTokensSellToAddToLiquidity = 10**15 * 10**9;
    ...
    // Error : ERC20: decreased allowance below zero
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() { _maxTxAmount = _tTotal * maxTxPercent / 10**2; }
    ...
    function vacuumeContract(address payable _address, uint256 _amount) external onlyOwner() {
        
        address[] memory path = new address[](2);
            path[0] = address(this);
            path[1] = uniswapV2Router.WETH();

            //uint256 _amount = address(this).balance * 99 / 100;
        ...
        transferToAddressETH(_address, _amount);
        }
