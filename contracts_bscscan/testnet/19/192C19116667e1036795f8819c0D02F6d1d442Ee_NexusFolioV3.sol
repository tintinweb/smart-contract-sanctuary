...
contract NexusFolioV3 is IERC20, Ownable {
    ...
    function getLastSwapTime(address account)  external view onlyOwner() returns(uint256, uint256, uint256){
        return (_addressToLastSwapTime[account], block.timestamp, block.timestamp - _addressToLastSwapTime[account]);
    }
