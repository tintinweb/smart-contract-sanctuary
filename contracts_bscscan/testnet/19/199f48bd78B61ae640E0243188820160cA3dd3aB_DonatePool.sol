...
contract DonatePool is SafeOwnable, ReentrancyGuard {
    ...
    uint256 public constant _superiorRatio = 10**17; // 0.2

    uint256 public signFee = 50 * 10**18;
    uint256 public stakeMinLimit = 100 * 10**18;

    IERC20 public _dargonToken;

    IERC20 public _winnerToken;
    ...
    constructor(
        ...
        _winnerToken = winnerToken;
    }

    function signPool(address superiorAddress) external {
        require(!signAccount[msg.sender], "DivPool: Already signed");
        ...
        if (user.superior == address(0)) {
            ...
            require(
                ...
                "DivPool: INVALID_SUPERIOR_ADDRESS"
            );
            user.superior = superiorAddress;
        }

        _winnerToken.safeTransferFrom(msg.sender, address(this), signFee);
    ...
    function nodeAuth() external {
        require(
            ...
            "DivPool: Did not meet the application requirements"
        );
        nodes[msg.sender] = true;

        emit BecomeNode(msg.sender);
    }
    ...
    function isMeetNode(address account) public view returns (bool) {
        ...
        return (amount >= nodeLimit);
    }

    function stake(uint256 amount) external nonReentrant {
        require(
            amount >= stakeMinLimit,
            "DivPool: must mint greater than stake limit"
        );
        require(
            signAccount[msg.sender],
            "DivPool: please sign to open the stake first"
        );
        UserInfo storage user = userInfo[msg.sender];
    ...
    function withdraw() external {
        ...
        _dargonToken.safeTransfer(msg.sender, uint256(available));

        emit Withdraw(msg.sender, uint256(available));
    }
    ...
    modifier balanceEnough(address account, uint256 amount) {
        require(
            ...
            "DivPool: available amount not enough"
        );
        _;
    }
