...
contract ElevateTest8 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("ElevateTest8", "ELEVATE") {
    ...
    function _transfer(
        ...
        if (tradingIsEnabled &&
            ...
            if (marketingEnabled) {
                uint256 marketingPortion = fees.mul(buyRewardsFee).div(totalBuyFees);
                fees = fees.sub(marketingPortion);
                super._transfer(from, marketingWallet, marketingPortion);
            }

            if (buyBackEnabled) {
                uint256 buyBackPortion = fees.mul(buyBuyBackFee).div(totalBuyFees);
                fees = fees.sub(buyBackPortion);
            ...
            if (marketingEnabled) {
                uint256 marketingPortion = fees.mul(sellRewardsFee).div(totalBuyFees);
                fees = fees.sub(marketingPortion);
                super._transfer(from, marketingWallet, marketingPortion);
            }

            if (buyBackEnabled) {
                uint256 buyBackPortion = fees.mul(sellBuyBackFee).div(totalBuyFees);
                fees = fees.sub(buyBackPortion);
                super._transfer(from, address(this), buyBackPortion);
            }  
