...
contract JaxAdmin is IJaxAdmin, Initializable, JaxOwnable {
  ...
  function set_fee_blacklist(address[] calldata accounts, bool flag) external onlyGovernor {
  ...
  function set_jtoken(address token, string calldata name, uint jusd_ratio, uint markup_fee, address markup_fee_wallet) external onlyAjaxPrime {
  ...
  function set_wjxn_usd_ratio(uint ratio) external onlyGovernor {
    wjxn_usd_ratio = ratio;
    emit Set_Wjxn_Usd_Ratio(ratio);
  }

  function set_wjax_usd_ratio(uint ratio) external onlyAdmin {
  ...
  function get_wjxn_wjax_ratio(uint withdrawal_amount) public view returns (uint) {
    ...
    return ((10 ** wjax.decimals()) * (wjxn.balanceOf(jaxSwap) - withdrawal_amount) 
        * get_wjxn_jusd_ratio() * 1e8) / (wjax.balanceOf(jaxSwap) * get_wjax_jusd_ratio());
  }
  ...
  function get_wjxn_jusd_ratio() public view returns (uint){
    ...
    if( use_wjxn_usd_dex_pair == 0 ) {
      return wjxn_usd_ratio * 1e18 / 1e8;
    }

    address wjxn_usd_pair = IPancakeFactory(router.factory()).getPair(address(wjxn), address(busd));
    IPancakePair pair = IPancakePair(wjxn_usd_pair);
    (uint Res0, uint Res1,) = pair.getReserves();
    // decimals
    if( pair.token1() == address(wjxn) ){
      return (Res0 * (10 ** wjxn.decimals()) / Res1);
    }
    return (Res1 * (10 ** wjxn.decimals()) / Res0); // return amount of token0 needed to buy token1
  }
  ...
  function get_wjxn_vrp_ratio() public view returns (uint) {
    ...
    if( vrp.totalSupply() == 0 || wjxn.balanceOf(jaxSwap) == 0){
      ...
      wjxn_vrp_ratio = vrp.totalSupply() / wjxn.balanceOf(jaxSwap);
    }
  ...
  function get_vrp_wjxn_ratio() public view returns (uint) {
    ...
    if(wjxn.balanceOf(jaxSwap) == 0 || vrp.totalSupply() == 0) {
        ...
        vrp_wjxn_ratio = wjxn.balanceOf(jaxSwap) * (10 ** (18 * 2)) / vrp.totalSupply();
    }
  ...
  function get_wjax_jusd_ratio() public view returns (uint){
    ...
    if( use_wjax_usd_dex_pair == 0 ) {
        return wjax_usd_ratio * 1e18 / 1e8;
    }

    // Case using pancake ratio

    // Comment the Below part in TestNet
    address wjax_usd_pair = IPancakeFactory(router.factory()).getPair(address(wjax), address(busd));
    IPancakePair pair = IPancakePair(wjax_usd_pair);
    (uint Res0, uint Res1,) = pair.getReserves();
    // decimals
  ...
  function get_jusd_wjax_ratio() public view returns (uint){
    return 1e18 * 1e18 / get_wjax_jusd_ratio();
  }
  ...
  function set_wjax_jusd_markup_fee(uint _wjax_jusd_markup_fee, address _wallet) external onlyGovernor {
    ...
    if( wjax_jusd_markup_fee != 0 ){
      require(wjax_jusd_markup_fee * 105 / 100 >= _wjax_jusd_markup_fee && wjax_jusd_markup_fee * 95 / 100 <= _wjax_jusd_markup_fee, "Change should be under 5%");
    }
  ...
  function setPriceImpactLimit(uint limit) external onlyGovernor {
    require(limit <= 3 * 1e6, "price impact cannot be over 3%");
  ...
  function show_reserves() public view returns(uint, uint, uint){
    ...
    uint wjax_usd_value = wjax_reserves * get_wjax_jusd_ratio() / (10 ** wjax.decimals());
    uint lsc_usd_value = jusd.totalSupply();

    uint jtoken_count = jtoken_addresses.length;
    for(uint i = 0; i < jtoken_count; i++) {
      address addr = jtoken_addresses[i];
      lsc_usd_value += IERC20(addr).totalSupply() * jtokens[addr].jusd_ratio / 1e8;
    }
    uint wjax_lsc_ratio = 1;
    if( lsc_usd_value > 0 ){
      wjax_lsc_ratio = wjax_usd_value * 1e18 / lsc_usd_value;
    }
