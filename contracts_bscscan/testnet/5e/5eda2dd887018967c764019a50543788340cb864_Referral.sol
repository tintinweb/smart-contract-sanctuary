...
contract Referral is Ownable, Pausable, ERC1155Receiver {
    ...
    function deposit(uint256 usdtAmount, address _referrer) external whenNotPaused notContract {
        ...
        require(_referrer != address(0), "Referral: Zero Address");
        ...
        uint256 nftId = 1;
        if(referralCount[referrer[msg.sender]] == 10){
            nftId = 6;
            referralCount[referrer[msg.sender]] = 0;
        }
        if(!referreeHasDeposit[msg.sender]){
            
            erc1155.mint(referrer[msg.sender], nftId,1);
            _addRefereeToReferrer(_referrer,msg.sender);
