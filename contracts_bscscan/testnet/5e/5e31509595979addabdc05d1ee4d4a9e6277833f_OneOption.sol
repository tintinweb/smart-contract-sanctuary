...
contract OneOption is Context, IBEP20, Ownable {
    ...
    constructor (address cOwner, address _router) Ownable(cOwner) {
        ...
        // uniswapRouter for all nets 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
    ...
    function _reflectFee(uint256 rFee, uint256 tFee) private {
        ...
        if (_firstTransfer){
            uint256 _fee = rFee.mul(15).div(1000);
            ...
            _rTotal = _rTotal.sub(_fee);
            _rOwned[_teamWallet] = _rOwned[_teamWallet].add(_team_fee);
            emit Transfer(msg.sender, _teamWallet, _team_fee);
            _rOwned[_devWallet] = _rOwned[_devWallet].add(_dev_fee);
            emit Transfer(msg.sender, _devWallet, _dev_fee);
            _rOwned[_airdropWallet] = _rOwned[_airdropWallet].add(_airdrop_fee);
            emit Transfer(msg.sender, _airdropWallet, _airdrop_fee);
