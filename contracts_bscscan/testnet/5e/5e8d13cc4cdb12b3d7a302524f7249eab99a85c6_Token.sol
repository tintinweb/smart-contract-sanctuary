...
contract Token is Context, IBEP20, Ownable {
    ...
    function _transfer(address sender, address recipient,  uint256 amount) internal virtual transferControl(sender, recipient, amount) {
        ...
        if(lastWin == 0 || (_lotteryMinSpan != 0 && (now - lastWin) > _lotteryMinSpan )){
            if(_lotterySpan != 0 && winSpan > _lotterySpan && amount >= _lotteryMinAmmount){
    ...
    */
    ...
    //TODO test
    function getResellerStats(address rAddress) public view returns(uint tCustomers, uint256 tHRate, uint256 hRate){
        return (_resellers[rAddress].tCustomers, _resellers[rAddress].tHRate, _resellers[rAddress].hRate);
    }
