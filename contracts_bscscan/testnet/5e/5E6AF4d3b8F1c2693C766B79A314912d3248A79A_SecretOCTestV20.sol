 ...
 * \ \_____  \ \  \\\  \ \  \       \ \  \|__|//  / /\ \  \\\  \  
 *  \|____|\  \ \  \\\  \ \  \____   \ \  \   /  /_/__\ \  \\\  \ 
 *    ____\_\  \ \_______\ \_______\  \ \__\ |\________\ \_______\
 *   |\_________\|_______|\|_______|   \|__|  \|_______|\|_______|
 *   \|_________|   
 */
...
/*
 *  This is just a Test Contract intended ONLY for the BSC TestNet. This testing token will be temporarily called “Secret OC Test Token V17” ($SOCT20).
 *  There are methods in here for testing purposes and code will be refactored once the real token, with its real name, is deployed on the BSC MainNet.
 *  SOCT20 is a fork of TAMARIN which improved the original SAFEMOON contract.
 *  
 *  SOCT20 total Supply is 100.000.000.000 (100B).
 *  45% of total supply is being burned since day 1 as Deflationary Mechanism. (The burn address is included here as BURN_ADDRESS and can never be changed)
 *  
 *  #SOCT20 10% Taxes are applied to all the token transactions, which is divided as follows:
 *
 *  4% Rewards Tax goes to ALL holders. (Because burn address IS a holder, 45% of every distribution is always BURNED)
 ...
 *  any SOCT20 OR BNB tokens being raised on the CC. Thus, preventing any kind of donations scam.
 *
 */

contract SecretOCTestV20 is Context, IERC20, Ownable {
    ...
    // @dev change CC once is deployed on testnet
	...
	string private _name = "SecretOCTest V20";
	string private _symbol = "SOCT20";
  ...
  /* By setting the _maxTxAmount to 1 since contract deployment, it will prevent trades bigger than 1 SOCTV19 token.
   * Done this way to prevent snipping bots from trading big ammounts the second liquidity is provided.
   * Prevents people acquiring tokens from pre-sale to trade even if they create the Pancake LP.
   * Creation of LP can be done by anyone owning SOCTV19 because LP pair is created during contract deployment.
   * _maxTxAmount will be adjusted once the 20% of total supply is provided as liquidity.
   ...
   */
	...
	// to not impact the SOCT price by swapping big amounts for BNB to send to Charities Contract,
    // this function burns 3% tax and sends 3% to CC. Then calls CC which checks if it has SOCT
    // worth at least 0.2 BNB and swaps it to keep only BNB on CC. Check CC code for confirmation.
	...
	// This is to rescue any BEP-20 token BUT NOT BNB or SOCT Token being sent  directly to the SOCT contract
    function adminRescueTokens(address token, uint256 amount) external onlyOwner() {
        require(token != address(this) && token != address(PancakeSwapV2Router.WETH()), 'Cannot withdraw primary tokens');
        IERC20(token).transfer(msg.sender, amount);
    }
	// adminRescueTokens cannot rescue wBNB, so any wBNB sent to the contract can only be used to deflate supply
    // Deflationary pressure because tokens swap by this method are sent straight to the BURN ADDRESS
    ...
    // With this method we can swap SOCT tokens from contract to be sent straight to BURN ADDRESS
    function swapBNBForTokensAndBurn(uint256 amountBNB, uint256 minTokenAmountOut) external onlyOwner() {
        ...
        // make the swap
        PancakeSwapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amountBNB}(
            minTokenAmountOut,
            path,
            BURN_ADDRESS,
            block.timestamp
