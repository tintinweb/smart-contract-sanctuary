...
contract MarketPlace is IMarketPlace, ReentrancyGuard, AccessControlEnumerable, IERC721Receiver {
    ...
    uint256 private _totalTradingVolume; // BNB
    ...
    function addTradingInfo(uint256 _tradingVolume) external onlyRole(MARKETPLACE_GOVERNANCE) {
        _totalTradingVolume += _tradingVolume;
        emit OnAddTradingInfo(_tradingVolume);
    }
     ...
     */
    function getTradingInfo() external view returns (uint256) {
     ...
     */
    function claimNiimfer(uint256 versionId, uint256[] memory tokenId) external payable override {
     ...
     */
    function claimNiimfersforAirdrop(uint256 versionId, uint256 amount) external payable override {
        SalesVersion storage version = sales_version[versionId];
    ...
    function _purchase(
        ...
        require(_forSale[tokenId], "This token is not opened for sale");
        nERC721 _nft = nERC721(_nftContract);
        require(_nft.balanceOf(buyer) + 1 <= _purchaseLimit);
        SaleInfo storage saleInfo = idToMarketItem[_toItemId[tokenId]];
...
interface IMarketPlace {
    ...
    event OnAddTradingInfo(uint256 tradingVolume);
