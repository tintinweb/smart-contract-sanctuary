...
contract Sales {
  ...
  function getMarketPrice() external view returns (uint256 badgePriceInBnb, uint256 badgePriceInUsd, uint256 edgePriceInBnb, uint256 edgePriceInUsd) {
    // BADGE Price
    {
      ...
      uint112 badgeReserveInBadgeBnbPool = address(badge) < address(wbnb) ? reserve0 : reserve1;
      uint112 bnbReserveInBadgeBnbPool = address(badge) < address(wbnb) ? reserve1 : reserve0;

      badgePriceInBnb =  uint256(bnbReserveInBadgeBnbPool) * 1e18 / badgeReserveInBadgeBnbPool;
      badgePriceInUsd = convertBnbToUsd(badgePriceInBnb);
    }

    // EDGE Price
    {
      (uint112 reserve0, uint112 reserve1, ) = edgeBnbPair.getReserves();
      uint112 edgeReserveInEdgeBnbPool = address(edge) < address(wbnb) ? reserve0 : reserve1;
      uint112 bnbReserveInBadgeBnbPool = address(edge) < address(wbnb) ? reserve1 : reserve0;

      edgePriceInBnb =  uint256(bnbReserveInBadgeBnbPool) * 1e18 / edgeReserveInEdgeBnbPool;
      edgePriceInUsd = convertBnbToUsd(edgePriceInBnb);
    }
  }

  function getBadgeBnbReserves() external view returns (uint112 reserve0, uint112 reserve1, uint256 price) {
    (reserve0, reserve1, ) = badgeBnbPair.getReserves();
    price = uint256(reserve1) * 1e18 / reserve0;
  }

  function getMarketPriceTest(uint256 _num) external view returns (uint256 price) {
    if (_num == 0) {
      price = 123;
    } else if(_num == 1) {
      (uint112 reserve0, uint112 reserve1, ) = badgeBnbPair.getReserves();
      uint112 badgeReserveInBadgeBnbPool = address(badge) < address(wbnb) ? reserve0 : reserve1;
      uint112 bnbReserveInBadgeBnbPool = address(badge) < address(wbnb) ? reserve1 : reserve0;
      price =  uint256(bnbReserveInBadgeBnbPool) * 1e18 / badgeReserveInBadgeBnbPool;
    } else if (_num == 2) {
      (uint112 badgeReserveInBadgeBnbPool, uint112 bnbReserveInBadgeBnbPool, ) = badgeBnbPair.getReserves();
      uint256 badgePriceInBnb =  uint256(bnbReserveInBadgeBnbPool) * 1e18 / badgeReserveInBadgeBnbPool;
      price = convertBnbToUsd(badgePriceInBnb);
    } else if (_num == 3) {
      (uint112 edgeReserveInEdgeBnbPool, uint112 bnbReserveInEdgeBnbPool, ) = badgeBnbPair.getReserves();
      price =  uint256(bnbReserveInEdgeBnbPool) * 1e18 / edgeReserveInEdgeBnbPool;
    } else if (_num == 4) {
      (uint112 edgeReserveInEdgeBnbPool, uint112 bnbReserveInEdgeBnbPool, ) = badgeBnbPair.getReserves();
      uint256 edgePriceInBnb =  uint256(bnbReserveInEdgeBnbPool) * 1e18 / edgeReserveInEdgeBnbPool;
      price = convertBnbToUsd(edgePriceInBnb);
    } else {
      price = 890;
    }
  }
