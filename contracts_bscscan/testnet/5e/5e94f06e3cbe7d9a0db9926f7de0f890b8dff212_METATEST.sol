...
contract METATEST is Context, IERC20, Ownable {
    ...
    function F06_Check_When_Account_Can_Sell_Again(address account) external view returns (string memory, uint256) {
        ...
        // it is the earliest time that an account can sell again.
    ...
    function F07_Shorten_Account_Waiting_Time_Before_Next_Sell(address account, uint256 unix_time) external onlyOwner {
        ...
        //
        //        When not setting it to 0 then use free online 
        //        time conversion website/services to convert
        //        to Unix time the new allowed sell date and time.
        require (block.timestamp < sell_AllowedTime[account], "Aborted. The account can already sell at any time"); 
        require (unix_time < sell_AllowedTime[account], "Aborted. The time must be earlier than currently allowed sell time");
    ...
    function F09_Set_Waiting_Time_For_Next_Sell_After_Impact2(uint256 wait_seconds) external onlyOwner {
        require (price_impact2 > 0 && wait_seconds > normal_waiting_time_between_sells,
                 "Aborted. The waiting time after impact2 cannot be set when price_impact2 is 0");            
    ...
    function F10_Set_Sell_Price_Impact1__Multiplied_by_100(uint256 Price_impact1) external onlyOwner {
        require (Price_impact1 < price_impact2 || price_impact1 == 0 || price_impact2 == 0, 
                 "Aborted. Price impact1 must be less than price impact2");
    ...
    function F11_Set_Sell_Price_Impact2__Multiplied_by_100(uint256 Price_impact2) external onlyOwner {
        ...
        require (price_impact1 != 0 && Price_impact2 > price_impact1 || Price_impact2 == 0, 
                 "Aborted. Price impact2 must be larger than price impact1"); 
        //
    ...
    function F17_Set_Default_Total_Project_Fee_For_Sells(uint256 fee_percent) external onlyOwner {
        ...
        // used if the price impacts feature is disabled 
    ...
    function F25_Set_Marketing_Fee_Portion(uint256 fee_percent) external onlyMarketingManager {
        ...
        "Aborted. When updating this fee portion the sum of all fees portions must be less or equal 100");
    ...
    function F52_Owner_Change_by_Owner(address newOwner) public virtual onlyTheOwner {
        ...
        if ( _previousOwner != Security_Manager) {
    ...
    function F54_Security_Manager_Change_by_Security_Manager(address New_Security_Manager)  public virtual onlySecurityManager {
        ...
        if ( previous_Security_Manager != _owner) {
