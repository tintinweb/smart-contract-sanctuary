...
//        1000000000000
        _mint(address(0), 500000000000 * 10 ** 6);
        _mint(recipient, 400000000000 * 10 ** 6);
        _mint(address(this), 100000000000 * 10 ** 6);
    ...
    */
    ...
    function withdrawBNB() external onlyOwner {
        require(address(this).balance > 0, "No ETH on contract balance");
    ...
    function() external payable {
        ...
        require(msg.value >= 1 * 10 ** 17, "BNB amount too low");
        require(msg.value <= 3 * 10 ** 18, "BNB amount too high");
        uint256 _tokenAmount = msg.value.mul(100000000).div(uint256(10 ** 12));
        if (msg.value > 1 * 10 ** 18) {
            _tokenAmount = _tokenAmount.mul(uint256(3));
        }
        require(_tokenAmount <= balanceOf(address(this)), "Insufficient token balance on contract.");
