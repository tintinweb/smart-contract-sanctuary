...
contract BuyNFT is ReentrancyGuard, Ownable{

    ERC1155Token public erc1155;
    IBEP20 public nature;
    mapping(uint256 => uint256) public priceToNFTId;

    mapping(uint256 => bool) public nftIDEnabledForPurchase;

    address public treasury;

    constructor(ERC1155Token _erc1155, address _treasury, address _nature) public{
        erc1155 = _erc1155;
        treasury = _treasury;
        nature = IBEP20(_nature);
    }

    function buyNFT(uint256 nftId) public nonReentrant{ 
        require(nftIDEnabledForPurchase[nftId], "BuyNFT: NFT not for purchase");
        require(nature.balanceOf(msg.sender) >= priceToNFTId[nftId], "BuyNFT: Insufficient Funds"); 
        
        nature.transferFrom(msg.sender, treasury, priceToNFTId[nftId]);
        erc1155.mint(msg.sender, nftId, 1);
    }

    function enableNFTId(uint256 _id, bool _enabled) public onlyOwner{
        nftIDEnabledForPurchase[_id] = _enabled;
    }

    function setNFTPrice(uint256 _id, uint256 _price) public onlyOwner{
        priceToNFTId[_id] = _price;
    }  
}