...
interface INonFungibleApesHelper {

    function isTier1(uint16) external view returns (bool);
    function isTier2(uint16) external view returns (bool);
    function isTier3(uint16) external view returns (bool);
    function isTier4(uint16) external view returns (bool);
    function isTier5(uint16) external view returns (bool);
...
import "./INonFungibleApesHelper.sol"; 
...
contract NFAStaking is ReentrancyGuard, Ownable, Initializable {
    ...
    INonFungibleApesHelper public NFA_TIERS;
    // The nfa tier helper contract
    uint8 public TIER;
    // The reward token
    ...
    function initialize(
        ...
        IERC20 _rewardToken,
        INonFungibleApesHelper _nfaTiers,
    ...
    function  validNFt(uint256 _id) public view returns (bool) {
        if (TIER == 1) {
            return NFA_TIERS.isTier1((uint16(_id)));
        } else if (TIER == 2) {
            return NFA_TIERS.isTier2((uint16(_id)));
        }  else if (TIER == 3) {
            return NFA_TIERS.isTier3((uint16(_id)));
        }  else if (TIER == 4) {
            return NFA_TIERS.isTier4((uint16(_id)));
        }  else if (TIER == 5) {
           return  NFA_TIERS.isTier5((uint16(_id)));
        }
        return false;
    }
    ...
    function stakedNfts(address _user) external view returns (uint256[] memory) {
        return userInfo[_user].stakedIds;
    }
    ...
    function deposit(uint256[] calldata _ids) external nonReentrant {
        ...
        if (user.amount > 0) {
         ...
         for (uint256 i=0; i<_ids.length; i++) {
            ...
            user.stakedIds.push(_ids[i]);
            userNfts[_ids[i]] = user.stakedIds.length - 1;
            user.amount++;
            totalStaked++;
        }
    ...
    function emergencyWithdraw(uint256[] calldata _ids) external nonReentrant {
        ...
        for (uint256 i=0; i<_ids.length; i++) {
            ...
            user.amount--;
            totalStaked--;
            removeFromArray(userNfts[_ids[i]], user.stakedIds);
            delete userNfts[_ids[i]];
        }
        user.rewardDebt = 0;
