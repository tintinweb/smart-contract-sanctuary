...
contract LotteryGenerator is Ownable {
    ...
    function createLottery(string memory name, address prizeToken,uint256 maxEntriesPerWallet, uint256 _drawnTime, uint256 _ethToParticipate) public onlyOwner {
    
        require(prizeToken != address(0),'invalid token addraddress');
        require(maxEntriesPerWallet > 0,'max entry per wallet cannot be 0');
        require(_ethToParticipate > 0,'per entry fee cannot be 0');
...
contract Lottery {
    ...
    constructor (string memory name, address _prizeToken,address creator, uint256 maxEntriesPerWallet, uint256 _drawnTime, uint256 _ethToParticipate)  
    {
        ...
        drawnTime = _drawnTime.mul(600).add(block.timestamp);
    ...
    function participate(string memory playerName) public payable {
        ...
        require(msg.value >= ethToParticipate,"Minimum Fee Required!");
