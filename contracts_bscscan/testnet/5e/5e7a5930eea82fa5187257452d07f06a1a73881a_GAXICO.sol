...
contract GAXICO is Context, Ownable {
   ...
   * @dev Start the sale.
   */ 
  function startICO(uint256 _rate) public onlyOwner returns(bool){
        hasStart=true;
        rate = _rate;
        return true;
    }
   ...
   */
   function Inverst() public payable{
       ...
       uint256 numberOfTokens =((msg.value.mul(1e18)).div(rate));
  ...
  function withdrwalTokens(uint256 numberOfTokens) public onlyOwner{
        token.transfer(owner(), numberOfTokens);
    }
