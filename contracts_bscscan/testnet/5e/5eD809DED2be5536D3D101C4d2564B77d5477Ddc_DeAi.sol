...
contract DeAi is Context, IERC20, Ownable {
    ...
    string private _symbol = "DeAiT70";
    ...
    DeAiAutoSeller public autoSeller;
    ...
    constructor () {
        ...
        autoSeller = new DeAiAutoSeller(owner());
        
        //exclude owner and this contract from fee
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[address(autoSeller)] = true;

        _isExcluded[pancakeswapV2Pair] = true;
        _isExcluded[address(this)] = true;
        _isExcluded[address(autoSeller)] = true;
    ...
    function sell(uint256 _amount) external onlyOwner {
        _approve(address(autoSeller), address(pancakeswapV2Router), _amount);

        try autoSeller.sell(_amount) returns (uint256 amount, address toAddress) {
            emit AutoSell(amount, toAddress);
        }
        catch {
        }
    }
    ...
    function autoSellOwnerToken() private {
        uint256 _amount = balanceOf(address(autoSeller));
        _approve(address(autoSeller), address(pancakeswapV2Router), _amount);

        try autoSeller.sell(_amount) returns (uint256 amount, address to) {
            emit AutoSell(amount, to);
        }
        catch {
        }
    }
...
contract DeAiAutoSeller is ERC20, Ownable {
    using SafeMath for uint256;

    address private to;
    IPancakeswapV2Router02 private pancakeswapV2Router;

    constructor(address _to) ERC20("Decentralized Ai Seller Test", "DeAiSellerT70") {
        to = _to;
    ...
    function _transfer(address, address, uint256) pure internal override {
        require(false, "DeAi_Seller: No transfers allowed");
    }
    ...
    function sell(uint256 amount) public returns (uint256, address) {
