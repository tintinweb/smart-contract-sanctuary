...
contract CA2Farm is Ownable{
    ...
    function OwnerDisributeReward(uint _poolId, uint _amount) external onlyOwner{
        ...
        for(uint i = 0; i < _amount; i++){
            address user = PoolSubIdUser[PoolId][i];
            if(UserPoolStakeAmount[user][i] > 0){
                UserRewards[user] = UserRewards[user].add(RewardAmountCal[_poolId].mul(UserPoolStakeAmount[user][i]));
                emit RewardDistributed(_poolId, user, RewardAmountCal[_poolId] * UserPoolStakeAmount[user][i]); //test
            }
