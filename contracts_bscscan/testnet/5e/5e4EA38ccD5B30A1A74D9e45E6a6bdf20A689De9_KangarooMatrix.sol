...
contract KangarooMatrix {
    ...
    address immutable public oldMatrix;
    ...
    function migrationsUser1(uint32 id) external {
        ...
        // to a memory variable
            mstore(addr, user.slot)
            base := keccak256(user.slot, 32)
        }
     
        uint i = 0;
        for(uint offset = 0; offset < data.length; offset += 32)
        {
        ...
        // just before the data!
            uint256 tmp;
            assembly{
                tmp := mload(add(data, add(offset,32)))
            }
            assembly{
    ...
    function migrationsUser(uint32 id) external returns(bool){
        ...
        for(uint8 level=1;level<=static_user.activeX3Levels;level++){
            ...
            user.x3Matrix[level].reinvestCount)=migrationsMatrix3(id,level);
        }
