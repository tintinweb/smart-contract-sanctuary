...
contract BlocksLottery is VRFConsumerBase, Ownable {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        uint256 stage;
        address winner = space.getOwnerOfBlock((randomness % 9 * 100) + randomness % 2);  // %42 %24
        // Select winner that is not blacklisted (3 tries)
        if(blacklist[winner]){
            winner = space.getOwnerOfBlock(((randomness + 69) % 9 * 100) + randomness % 2);
            stage = 1;
            if(blacklist[winner]){
                winner = space.getOwnerOfBlock(((randomness + 96) % 9 * 100) + randomness % 2);
                stage = 2;
            }
        }
