...
interface IAddressRegistry {
    function grapes() external view returns (address);
...
contract Marketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier isListed(
        ...
        require(listing.quantity > 0, "not listed");
        _;
    }
    ...
    modifier validListing(
        ...
        require(_getNow() >= listedItem.startingTime, "not buyable");
        _;
    }
    ...
    function listItem(
        ...
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC721)) {
            IERC721 nft = IERC721(_nftAddress);
            require(nft.ownerOf(_tokenId) == _msgSender(), "not owning");
            require(
    ...
    function _isNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.grapes() == _nftAddress ||
