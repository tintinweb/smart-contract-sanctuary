...
// HakuSwap Dev Team
    ...
    contract OneClickLP is ReentrancyGuard, Ownable, Pausable {
        using SafeMath for uint256;
        using SafeERC20 for IERC20;

        address private constant wavaxAddress = 0x818b425ffEe85a8f85Aa5142b1e17f6E8E025B7A;
        ...
        function zapIn(
            ...
            if (_fromTokenAddress == address(0)) {
                _wrapAVAX();
                _fromTokenAddress = wavaxAddress;
            }
            ...
            if (pair.token0() != _fromTokenAddress)
                token0Amt = _safeSwap(_token0RouterAddress, _fromTokenAddress, halfAmount, _token0Path, address(this));
            else
        ...
        function _pullTokens(
            ...
            if (token == address(0)) {
                ...
                require(msg.value == 0, "AVAX sent with token");
        ...
        function _subtractGoodwill(uint256 amount) internal view returns (uint256 totalGoodwillPortion) {
            if (goodwill > 0) {
                totalGoodwillPortion = amount.mul(goodwill).div(10000);
            }
        }
        ...
        function _wrapAVAX() internal {
            uint256 avaxBal = address(this).balance;
            if (avaxBal > 0) {
                IWAVAX(wavaxAddress).deposit{value: avaxBal}(); // AVAX -> WAVAX
            }
        }
