...
contract MRTOKEN is IERC20, Ownable {
    ...
    function takeFee(
        ...
        if (to == pair) {
            ...
            // allocate fees
            marketingFeeAmount += feeAmount.mul(sellMarketingFee).div(
                sellTotalFees
            );
            devFeeAmount += feeAmount.mul(sellDevFee).div(sellTotalFees);
            rewardFeeAmount += feeAmount.mul(sellDividendRewardsFee).div(
                sellTotalFees
            );
            liquidityFeeAmount += feeAmount.mul(sellLiquidityFee).div(
                sellTotalFees
            );
            if (sellBurnFee > 0) {
            ...
            // allocate fees
            marketingFeeAmount += feeAmount.mul(buyMarketingFee).div(
                buyTotalFees
            );
            devFeeAmount += feeAmount.mul(buyDevFee).div(buyTotalFees);
            rewardFeeAmount += feeAmount.mul(buyDividendRewardsFee).div(
                buyTotalFees
            );
            liquidityFeeAmount += feeAmount.mul(buyLiquidityFee).div(
                buyTotalFees
            );
            if (buyBurnFee > 0) {
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 tokensToSwap = marketingFeeAmount.add(devFeeAmount).add(
            rewardFeeAmount
        );
        // swap the tokens
        ...
        if (rewardFeeAmount > 0) {
            uint256 amountBNBReflection = swappedBnbAmount
                .mul(rewardFeeAmount)
                .div(tokensToSwap);
        ...
        if (marketingFeeAmount > 0) {
            ...
            uint256 totalBnbMarketing = swappedBnbAmount
                .mul(marketingFeeAmount)
                .div(tokensToSwap);

            // send bnb to marketing
            (bool marketingSuccess, ) = payable(marketingFeeReceiver).call{
                value: totalBnbMarketing,
                gas: 30000
            }("");
        ...
        if (devFeeAmount > 0) {
            uint256 totalBnbForDev = address(this).balance;
            (bool devSuccess, ) = payable(devFeeReceiver).call{
                value: totalBnbForDev,
                gas: 30000
            }("");
        ...
        if (liquidityFeeAmount > 0) {
            if (_balances[address(this)] < liquidityFeeAmount) {
                liquidityFeeAmount = _balances[address(this)];
            }
            // add liquidity
    ...
    function swapBackInTokens() internal swapping {
        if (marketingFeeAmount > 0 || devFeeAmount > 0) {
            swapAndSendFees();
            marketingFeeAmount = 0;
            devFeeAmount = 0;
        }
        // if (rewardFeeAmount > 0) {
        //     swapTokensForEth(rewardFeeAmount);
        //     uint256 swappedBnbAmount = address(this).balance;
        //     // send bnb to reward
        //     try
        //         dividendDistributor.deposit{value: swappedBnbAmount}()
        //     {} catch {}
        //     // set reward token amount 0
        //     rewardFeeAmount = 0;
        // }
        // if (liquidityFeeAmount > 0) {
        //     if (_balances[address(this)] < liquidityFeeAmount) {
    ...
    function swapAndSendFees() private {
        uint256 totalTokensToSwap = marketingFeeAmount.sub(devFeeAmount);
        // swap tokens
        swapTokensForTokens(totalTokensToSwap);

        // uint256 currentTokenBalance = IERC20(SWAPTOKEN).balanceOf(
        //     address(this)
        // );
        // uint256 marketingToken = currentTokenBalance
        //     .mul(marketingFeeAmount)
        //     .sub(totalTokensToSwap);
