...
library Utils {
    ...
    function calculatePoolAmounts(uint256 amount) internal pure 
     returns(uint256, uint256, uint256, uint256, uint256){

        return(
            amount.mul(3).div(100),
            amount.mul(1).div(100),
            amount.mul(1).div(100),
            amount.mul(1).div(100),
            amount.mul(1).div(100));
...
contract JokerCoin is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    function _transfer(
        ...
        if(from == pancakePair){
            ...
            require(amount.mul(getCurrentPrice()).div(1e9) >= minBuy,"JOKER: amount less than minimum buy");
        }
    ...
    function setLottery(address user, uint256 amount) internal{
        pool1Count++;
        pool2Count++;
        pool3Count++;
        pool4Count++;
        ...
        if(pool1Count == pool1Lottery){
            Utils.swapTokensForEth(address(pancakeRouter), pool1Amount);
            uint256 transferAmount = getContractBalance().sub(initialBalance);
            payable(user).transfer(transferAmount);
            pool1Amount = 0;
            pool1Count = 0;
            winners.push(user);
            winningAmount[user] = transferAmount;
        }
        if(pool2Count == pool2Lottery){
            Utils.swapTokensForEth(address(pancakeRouter), pool2Amount);
            uint256 transferAmount = getContractBalance().sub(initialBalance);
            payable(user).transfer(transferAmount);
            pool2Amount = 0;
            pool2Count = 0;
        ...
        if(pool3Count == pool3Lottery){
            Utils.swapTokensForEth(address(pancakeRouter), pool3Amount);
            uint256 transferAmount = getContractBalance().sub(initialBalance);
            payable(user).transfer(transferAmount);
            pool3Amount = 0;
            pool3Count = 0;
        ...
        if(pool4Count == pool4Lottery){
            Utils.swapTokensForEth(address(pancakeRouter), pool4Amount);
            uint256 transferAmount = getContractBalance().sub(initialBalance);
            payable(user).transfer(transferAmount);
            pool4Amount = 0;
            pool4Count = 0;
        ...
        if(pool5Count == pool5Lottery){
            Utils.swapTokensForEth(address(pancakeRouter), pool5Amount);
            uint256 transferAmount = getContractBalance().sub(initialBalance);
            payable(user).transfer(transferAmount);
            pool5Amount = 0;
            pool5Count = 0;
