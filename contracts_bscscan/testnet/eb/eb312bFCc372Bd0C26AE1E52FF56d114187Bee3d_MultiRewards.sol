    ...
    function stake(uint256 amount) external nonReentrant notPaused updateReward(msg.sender) {
        ...
        if (actualAmount > 0) {
            ...
            {  
                ...
                if (prev > 1 && block.timestamp == userStakeBalance[msg.sender][prev-1].startingTime)
            ...
            for (uint i = 0; i < rewardTokens.length; i++) {
                ...
                if (prev > 1 && block.timestamp == rewardRatePerToken[rewardTokens[i]][prev-1].startingTime)  // in case same block stake or withdraw
