...
contract VenusPool is BasePool {
    ...
    uint256 private _venusExitRatio;
    constructor( IBEP20 assetToken,
        IVenus venus,
        IEgoToken egoToken,
        uint256 venusExitRatio,
        uint256 initialVEGORate,
        address platformFeeAddress) 
            BasePool(assetToken,
                egoToken,
                initialVEGORate, 1000*1e12)//rate+different in dec numbers
        {
        _platformFeeAddress = platformFeeAddress;
        _venusExitRatio = venusExitRatio;
        _venus = venus;
    ...
    function _unstakeAsset(StakerInfo memory stakerInfo, CelebrityInfo memory celebrityInfo, uint256 amount) internal virtual override {
        ...
        if (avalibleVenusWithdrawal > originalWithExitFee) {
            //witdrawal original
            require(_venus.redeemUnderlying(originalWithExitFee) == 0, "unstake: redeem underlying error");
            celebrityDecreaseVBalance = (originalWithExitFee * DEFAULT_OFFSET) * 1e18 / venusRate / DEFAULT_BASE;
        }
        else {
            // in this case we do not have enough vTokens and staker is going to get less funds than staked. This can happen if exit fee is not covered by the reward
            amount = stakerInfo.assetBalanceInfo.amount;
            //witdrawal max
            require(_venus.redeem(stakerInfo.assetBalanceInfo.vBalance) == 0, "unstake: redeem error");
        ...
        //-----------venus calc---------------
        if( celebrityInfo.assetBalanceInfo.vBalance > celebrityDecreaseVBalance)
            celebrityInfo.assetBalanceInfo.vBalance -= celebrityDecreaseVBalance;
        else
    ...
    {
        uint256 redeemFee =
        (amount * DEFAULT_OFFSET * _venusExitRatio) / DEFAULT_BASE / DEFAULT_BASE;
        return redeemFee;
    }
}
...
abstract contract BasePool is Ownable {
    ...
    //20%
    uint256 constant PLATFORM_FEE = 200000;
    ...
    function stake(
        ...
        uint256 egoAmount = assetStakeAmount * EgoTokenRate / DEFAULT_BASE;
    ...
    function calculateCelebrityEgoClaimAmount(address celebrity) view public returns (ClaimAmount memory result){
        CelebrityInfo memory celebrityInfo = CelerbrityInfo[celebrity];
        return _calculateClaimAmount(celebrityInfo.egoBalanceInfo, CalculatePoolRate());
    }

    function calculateStakerEgoClaimAmount(address owner, address celebrity) view public returns (ClaimAmount memory result) {
        StakerInfo memory stakerInfo = StakersInfo[owner][celebrity];
        return _calculateClaimAmount(stakerInfo.egoBalanceInfoShare, CalculatePoolRate());
    }
    
    function _calculateClaimAmount(BalanceInfo memory currentBalanceInfo, uint256 currentRate) pure public returns (ClaimAmount memory result){
        uint256 total = currentBalanceInfo.vBalance * currentRate / DEFAULT_BASE;
        if (total > currentBalanceInfo.amount)
        {
            result.claimAmount = total - currentBalanceInfo.amount;
            result.vClaimAmount = result.claimAmount / currentRate * DEFAULT_BASE;
        }
    }

    function calculateCelebrityNetAssetReward(address celebrity) view public returns (ClaimAmount memory result){
        CelebrityInfo memory celebrityInfo = CelerbrityInfo[celebrity];
        result = _calculateClaimAmount(celebrityInfo.assetBalanceInfo, _getRate());
    ...
    function calculateCelebrityAssetClaimAmount(address celebrity) view public returns (ClaimAmount memory result){
        CelebrityInfo memory celebrityInfo = CelerbrityInfo[celebrity];
        return _calculateClaimAmount(celebrityInfo.assetBalanceInfo, _getRate());
    }
    ...
    function unstake(address celebrity, uint256 assetStakeAmount) external {
        ...
        uint256 assetOriginal =stakerInfo.egoBalanceInfoOriginal.amount * DEFAULT_BASE / EgoTokenRate;
        uint256 egoAmount = 0;
        if(assetStakeAmount > assetOriginal) {
            assetStakeAmount = stakerInfo.egoBalanceInfoOriginal.amount * DEFAULT_BASE / EgoTokenRate;
            egoAmount = stakerInfo.egoBalanceInfoOriginal.amount;
        }
        else {
            egoAmount = assetStakeAmount * EgoTokenRate / DEFAULT_BASE;
        }
        // update rate inside
        ...
        if( stakerInfo.egoBalanceInfoOriginal.vBalance > vEgoAdded)
            stakerInfo.egoBalanceInfoOriginal.vBalance -= vEgoAdded;
        else {
            stakerInfo.egoBalanceInfoOriginal.vBalance = 0;
            vEgoAdded = stakerInfo.egoBalanceInfoOriginal.vBalance;
        }
        ...
        uint256 shareChange = egoAmount  * (DEFAULT_BASE - stakerInfo.celebrityRewardPercent) / CurrentRate;
        if(stakerInfo.egoBalanceInfoShare.vBalance > shareChange) {
            stakerInfo.egoBalanceInfoShare.vBalance -= shareChange;
        }
        else {
            stakerInfo.egoBalanceInfoShare.vBalance = 0;
        }
        ...
        uint256 celebrityVChange =  vEgoAdded * stakerInfo.celebrityRewardPercent / DEFAULT_BASE;
        ...
        if(_totalVEGOInCirculation > vEgoAdded)
            _totalVEGOInCirculation -= vEgoAdded;
        else
            _totalVEGOInCirculation = 0;
    ...
    function calculateAPR() public view returns (uint256) {
        if (TotalStaked == 0)
            return MAXapr;
        uint256 totalStakedEgo = TotalStaked * EgoTokenRate / DEFAULT_BASE;
