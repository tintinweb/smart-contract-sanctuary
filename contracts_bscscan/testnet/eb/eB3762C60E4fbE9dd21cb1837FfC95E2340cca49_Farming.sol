...
contract Farming is Ownable {
    ...
    constructor(uint256 _emissionRate, IERC20 _lpToken, uint256 _poolLimitPerUser, uint256 _poolCap) public {
    ...
    function stake(uint256 amount) external {
        ...
        if (hasPoolLimit) {
            ...
            require(amount.add(stakedTokenSupply) <= poolCap, "Pool cap reached");
        }

        if (hasUserLimit) {
            require(amount.add(user.amount) <= poolLimitPerUser, "User amount above limit");
        }
