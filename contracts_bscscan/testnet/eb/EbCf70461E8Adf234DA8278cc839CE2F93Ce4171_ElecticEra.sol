...
contract ElecticEra is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 _totalFeePerTx = 50; // 5% by default
    ...
    function _takeLiquidityFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 bothFee = tAmount.mul(_totalFeePerTx).div(1e3);
    ...
    function _reflectFee(uint256 tAmount) private {
        uint256 tFee = tAmount.mul(_holderFee).div(1e3);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        uint256 rAmount = tAmount.add(getTotalFeePerTx(tAmount)).mul(currentRate);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
        uint256 currentRate = _getRate();
        uint256 tTransferAmount = tAmount.add(getTotalFeePerTx(tAmount));
        uint256 rTransferAmount = tAmount.mul(currentRate);
        uint256 rAmount = tAmount.add(getTotalFeePerTx(tAmount)).mul(currentRate);
