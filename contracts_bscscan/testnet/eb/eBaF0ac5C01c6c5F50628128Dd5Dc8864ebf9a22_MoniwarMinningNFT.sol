...
contract MoniwarMinningNFT is Ownable, ReentrancyGuard {
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256)
    {
        ...
        uint256 amount = user.hashRate.mul(accMowaPerShare.div(pool.totalHashRate)).mul(1e12);
        return amount.sub(user.rewardDebt).div(1e12).add(user.rewardLockedUp);
    }
    ...
    function upgrade(uint256 _pid, uint256 uid, uint256 number) public nonReentrant {
        ...
        updatePool(_pid);
        user.rewardDebt = user.hashRate.mul(pool.accMowaPerShare.div(pool.totalHashRate)).mul(1e12);
        payOrLockupPendingMowa(_pid);
    ...
    function harvest(uint256 _pid) public nonReentrant {
        ...
        if (user.hashRate == 0 || block.number >= pool.endBlock) {
            ...
            uint256 amount = user.hashRate.mul(pool.accMowaPerShare.div(pool.totalHashRate)).mul(1e12);
            pending =  amount.sub(user.rewardDebt).div(1e12).add(user.rewardLockedUp);
        }
        if (pending > 0) {
