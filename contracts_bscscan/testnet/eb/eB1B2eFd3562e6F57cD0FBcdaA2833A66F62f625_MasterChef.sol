...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    public validatePoolByPid(_pid) nonReentrant notContract notBlacklisted {
        ...
        if(shouldPayReward(_pid, recipient) )
    ...
    nonReentrant notContract {
        ...
        if (_amount > 0) {
            ...
            if (pool.lock > 0) {
                if (block.timestamp < user.lastDepositTime + pool.lock ) {
        ...
        if(shouldPayReward(_pid, msg.sender) )
            user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(1e12);
        _harvestAll();
    }
    ...
    function adminSetReserveAddr(address _taxAddr) external onlyAdmin {
    ...
    function adminSetTaxAddr(address payable _taxTo) external onlyAdmin {
    ...
    /**
    event PayReward(address user, uint256 pid, bool status, uint256 amount, uint256 pending, uint256 rewardDebt);
    ...
    PendingRewardInfo[] public pendingRewardInfo;
    function getInfo() public view returns(PendingRewardInfo[] memory){
        return pendingRewardInfo;
    }
    */
    function shouldPayReward(uint256 pid, address recipient ) public view returns(bool){
        PoolInfo storage pool = poolInfo[pid];
    ...
    function _payRewardByPid( uint256 pid, address recipient ) public {
        ...
        UserInfo storage user = userInfo[pid][recipient];
        uint256 pending = 0;
        bool status = shouldPayReward(pid, recipient);
        if ( status ) {
            if (user.amount > 0) {
        ...
        /*
        PendingRewardInfo memory info;
        info.user = recipient;
        info.status = status;
        info.pid = pid;
        info.amount = user.amount;
        info.pending = pending;
        info.pendingReward = pendingReward(pid, recipient);
        info.rewardDebt = user.rewardDebt;
        pendingRewardInfo.push(info);
        */
    }
