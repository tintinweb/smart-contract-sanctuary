...
contract PreSale is ReentrancyGuard, Context, Ownable {
    ...
    //Start Pre-Sale
    function startICO(uint startDate, uint endDate, uint _walletSoftCap, uint _walletHardCap, uint _availableTokens, uint256 _softCap, uint256 _hardCap) external onlyOwner icoNotActive() {
    ...
    function setWalletSoftCap(uint256 newWalletSoftCap) public onlyOwner {
        require(newWalletSoftCap > 0, 'Pre-Sale: _walletSoftCap should > 0');
        walletSoftCap = newWalletSoftCap;
    }
    function setWalletHardCap(uint256 newWalletHardCap) public onlyOwner {

        walletHardCap = newWalletHardCap;
    }
    ...
    function setLockDays(uint256 _lockdays) public onlyOwner icoNotActive
    {
        lockdays=_lockdays;
        lastRun = block.timestamp;
    }
    function claimTokenLock()  private
    { 
        uint vestinglock=uint(block.timestamp - lastRun)/day;
        if( vestinglock < lockdays ){
            claimtokenlock= true;
        }
        claimtokenlock= false;
    }
