...
contract ProofPresale is Pausable {
  ...
  function validPurchase() internal  returns (bool) {
    ...
    bool isLate=endTime<block.timestamp;
    return (notSmallAmount && withinCap&&isLate);
  }
  ...
  function getTokenBack()public onlyOwner{
    require(token.balanceOf(address(0))>0);
    token.transfer(msg.sender,token.balanceOf(address(0)));
  }
