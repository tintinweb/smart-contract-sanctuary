...
contract NewHelloWorld is Ownable, ReentrancyGuard {
    ...
    constructor(address _governance, address _validator, address _beneficiary, uint256 _depositFee, uint256 _withdrawFee, uint256 _listingFee, uint256 _foreignChainId) {
    ...
    function withdrawfromBridge(uint256 _amount, address _withdrawERC20, address _depositedERC20, uint256 _homeChainId, uint256 _foreignChainId, uint256 _nonce, bytes32 _txHash, Sig memory signature) external noContractsAllowed nonReentrant {
        ...
        require(homeChainId == _homeChainId, "WithdrawfromBridge:: Invalid Home chainId!");
        require(foreignChainId == _foreignChainId, "WithdrawfromBridge:: Invalid Foreign chainId!");
        require(!claimedWithdrawalsByOtherChainDepositId[_nonce], "WithdrawfromBridge:: Already Withdrawn!");
        require(verifySignature(_amount, _withdrawERC20, _depositedERC20, _homeChainId, _foreignChainId, _nonce, _txHash, signature), "WithdrawfromBridge:: Invalid Signature");
    ...
    function verifySignature(uint256 _amount, address _withdraERC20, address _depositedERC20, uint256 _homeChainId, uint256 _foreignChainId, uint256 _nonce, bytes32 _txHash, Sig memory signature) internal view returns (bool) {
