...
contract TokenName is Context, IERC20, Ownable {
    ...
    address private _marketingWalletAddress = 0x2443e7416b917AdC9998865cC890D545E7D4AAC8;
    ...
    uint256 public _taxFee = 400; // 4% buy, 8% sell
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 100; // 1% buy, 1.5% sell
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _marketingFee = 50; // 0.5% buy, 2% sell
    ...
    function _transfer(
        ...
        if(from == pancakeV2Pair && to != address(pancakeV2Router) && !_isExcludedFromFee[to]) {
                ...
                _taxFee = _taxFee;             // 4%
                _marketingFee = _marketingFee; // 0.5%
                _liquidityFee = _liquidityFee; // 1%

        }

        // sell
        if(!inSwapAndLiquify && from != pancakeV2Pair) {

            _taxFee = _taxFee.mul(2);                        // 8%
            _marketingFee = _marketingFee.mul(4);            // 2%
            _liquidityFee = _liquidityFee.mul(30).div(1000); // 1.5%
