...
contract Staking is Ownable, Pausable{
    ...
    event unStakeEvent(address indexed from,uint256 amount,uint256 reward,uint256 unStakeTime);
    ...
    function cashBackRewardUpdate(uint256 amount) public onlyOwner {
        cashBackReward = amount;
    }
    
    function stake(uint256 stakeType,uint256 amount) public whenNotPaused {
    ...
    function unstake(uint256 stakeId) public whenNotPaused {
        ...
        require(!store.claimStatus, "already claimed");
        
        store.claimStatus = true;
        uint256 reward = pendingRewards(stakeId);
        token.safeTransfer(store.user,store.stakeAmount);
        token.mint(store.user,reward);
        totalRewardEmission = totalRewardEmission.add(reward);
        
        emit unStakeEvent(msg.sender,store.stakeAmount,reward,block.timestamp);
    }
    ...
    function cashBack(uint256 stakeId) public whenNotPaused {
        user storage store = stakeStore[stakeId];
        require(isContains(msg.sender,stakeId), "invalid stake id");
        require(!store.cashBackStatus, "cashback already claimed");
        
        store.cashBackStatus = true;
        token.mint(store.user,store.stakeAmount.mul(cashBackReward).div(100));
    }
