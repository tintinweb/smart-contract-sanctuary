    ...
    ERC721HolderUpgradeable
{
    ...
    //     uint256 _paymentAmount
    // ) {
    //     require(_validAmount(_orderId, _quantity, _paymentAmount));
    //     _;
    // }
    ...
    //     return (_paymentAmount >= buyAmount) ? true : false;
    // }
    ...
    function buy(uint256[] calldata _ordersId) external whenNotPaused returns (bool) {
        ...
        for (uint256 index = 0; index < _ordersId.length; index++) {
            Order memory order = orders[_ordersId[index]];
            uint256 exactPaymentAmount = order.price;
            ...
            emit Buy(_ordersId[index], 1, order.tokenAddress, exactPaymentAmount);
            _match(
                msg.sender,
                order.paymentToken,
                _ordersId[index],
