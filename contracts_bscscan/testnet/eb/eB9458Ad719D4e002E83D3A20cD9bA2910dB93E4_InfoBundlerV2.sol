...
contract Reinvest is Ownable {
    ...
    // PENDING TO WITHDRAW
    mapping(uint256 => uint256) _pending_to_withdraw;
    ...
    function doStuff() public onlyOwner {
        ...
        for (uint256 i = 0; i < pending_to_withdraw_requests.length; i++) {
            ...
            _pending_to_withdraw[_r.pid] += added_to_pools_user[_r.pid][_r.user];
        }
        // Withdraw them
        for (uint256 i = 0; i < pending_to_withdraw_pids.length; i++) {
            uint256 _pid = pending_to_withdraw_pids[i];
            masterchef.withdraw(_pid, _pending_to_withdraw[_pid]);
            added_to_pools[_pid] -= _pending_to_withdraw[_pid];
            delete _pending_to_withdraw[_pid];
        }
        ...
        for (uint256 i = 0; i < pending_to_withdraw_requests.length; i++) {
            ...
            delete is_pending_to_withdraw_user[_r.pid][_r.user];
        }
        delete pending_to_add_requests;
    }
    ...
    function requestLengths() public view returns (uint256 add, uint256 uncake, uint256 withdraw) {
        add = pending_to_add_requests.length;
        uncake = pending_to_uncake_requests.length;
        withdraw = pending_to_withdraw_requests.length;
    }
    ...
    function seePending(uint256 _pid, address _user) public view returns (uint256) {
        if (added_to_pools[_pid] == 0) return 0;
...
contract InfoBundlerV1 {
    ...
    function infoBundleUser(address _user) public view returns (uint256[] memory pending, uint256[] memory added, uint256[] memory pendingToAdd, bool[] memory isPendingToWithdraw, bool pendingToUncake) {
        uint256 _pl = masterchef.poolLength();
        isPendingToWithdraw = new bool[](_pl);
        pendingToAdd = new uint256[](_pl);
        pending = new uint256[](_pl);
        added = new uint256[](_pl);
        for (uint256 i = 0; i < _pl; i++) {
            isPendingToWithdraw[i] = reinvest.is_pending_to_withdraw_user(i, _user);
            pending[i] = reinvest.seePending(i, _user);
            added[i] = reinvest.added_to_pools_user(i, _user);
        }
        (uint256 _pla, uint256 _plu, ) = reinvest.requestLengths();
        for (uint256 i = 0; i < _pla; i++) {
            (address _u, uint256 _pid, uint256 _amount) = reinvest.pending_to_add_requests(i);
            if (_user == _u) pendingToAdd[_pid] += _amount;
        }
        for (uint256 i = 0; i < _plu; i++) {
            address _u = reinvest.pending_to_uncake_requests(i);
            if (_user == _u) pendingToUncake = true;
        }
    }
    function infoBundlePools() public view returns (PoolInfo[] memory pools) {
        uint256 _pl = masterchef.poolLength();
        pools = new PoolInfo[](_pl);
        for (uint256 i = 0; i < _pl; i++) {
            (IBEP20 lpToken, uint256 allocPoint, uint256 lastRewardBlock, uint256 accCakePerShare) = masterchef.poolInfo(i);
            pools[i] = PoolInfo({
                lpToken: lpToken,
    ...
    function infoBundleTotal() public view returns (uint256[] memory pending, uint256[] memory pendingMC, uint256[] memory added, UserInfo[] memory addedMC) {
        uint256 _pl = masterchef.poolLength();
        pending = new uint256[](_pl);
        pendingMC = new uint256[](_pl);
        added = new uint256[](_pl);
        addedMC = new UserInfo[](_pl);
        for (uint256 i = 0; i < _pl; i++) {
            pending[i] = reinvest.seePending(i, address(reinvest));
            pendingMC[i] = masterchef.pendingCake(i, address(reinvest));
            added[i] = reinvest.added_to_pools(i);
            (uint256 amount, uint256 rewardDebt) = masterchef.userInfo(i, address(reinvest));
            addedMC[i] = UserInfo({
                amount: amount,
...
contract InfoBundlerV2 is InfoBundlerV1 {
    function infoBundleUserV2(address _user) public view returns (PoolInfo[] memory pools, uint256[] memory all, uint256[] memory bal, uint256[] memory pending, uint256[] memory added, uint256[] memory pendingToAdd, bool[] memory isPendingToWithdraw, bool pendingToUncake) {
        uint256 _pl = masterchef.poolLength();
        all = new uint256[](_pl);
        bal = new uint256[](_pl);
        (pools) = infoBundlePools();
        for (uint256 i = 0; i < _pl; i++) {
            all[i] = pools[i].lpToken.allowance(_user, address(reinvest));
            bal[i] = pools[i].lpToken.balanceOf(_user);
        }
        (pending, added, pendingToAdd, isPendingToWithdraw, pendingToUncake) = infoBundleUser(_user);
    }
