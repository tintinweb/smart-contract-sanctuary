...
contract BlindBox {
  ...
  function init4(string memory name) adminAdd  public {
    uint256 counter;
  ...
  function getBoxDetail(string memory name, uint256 selectedTime) public view returns(string memory, string memory, uint256,uint256, uint256, bool, uint256){
    return (Category[name].title, Category[name].description, Category[name].price,Category[name].usdPrice,Category[name].countNFT, isBoxSold(selectedTime, name), boxDetails[name].initialize);
  }

  function getCountByTime(uint256 selectedTime, string memory name) public view returns(uint256) {
    uint256 dayCount = (selectedTime > 0 ? selectedTime : block.timestamp  - boxDetails[name].initialize)/(60 * 20);
  ...
  function isBoxSold(uint256 selectedTime, string memory name) public view returns(bool) {
    uint256 dayCount = (selectedTime > 0 ? selectedTime : block.timestamp  - boxDetails[name].initialize)/(60 * 20);
    return boxDetails[name].usedCountByday[dayCount].isSold;
  }
