...
contract Staking is Ownable {
    ...
    constructor(uint256 _startBlock, uint256 _totalTokens, IERC20 _depositToken, IERC20 _rewardToken) {
        ...
        endBlock = startBlock + 1200; // 1 year is approx. 10368000 blocks
        totalTokens = _totalTokens*10**18;
        tokensPerBlock = totalTokens*precisionLevel / 1200; // precision level??
        depositToken = _depositToken;
        rewardToken = _rewardToken;
        lastDepositBlock = endBlock - 800;
    }    
    ...
    function deposit(uint256 _amount) public {
        ...
        require(block.number <= lastDepositBlock, "Staking has finished");
        ...
        stakingWallets[msg.sender].unlockBlock = block.number + 200; // 10 mins
        stakingWallets[msg.sender]._2Claim = stakingWallets[msg.sender].unlockBlock + 100; // 5 mins
        stakingWallets[msg.sender]._3Claim = stakingWallets[msg.sender]._2Claim + 100;
        stakingWallets[msg.sender]._4Claim = stakingWallets[msg.sender]._3Claim + 100;
    ...
    function withdraw() public {
        ...
        require(stakingWallets[msg.sender].claimTx < 5, "You have already claimed everything");
        uint256 withdrawAmount = (stakingWallets[msg.sender].depositAmount/4)*(claimCycle-stakingWallets[msg.sender].claimTx);
        totalDeposited -= withdrawAmount;
        stakingWallets[msg.sender].depositAmount -= withdrawAmount;
        _recalculateRewardForAllUsers();
        stakingWallets[msg.sender].claimTx = claimCycle; 
        withdrawAmount += stakingWallets[msg.sender].rewardDebt;
        stakingWallets[msg.sender].rewardDebt = 0;
        rewToken().safeTransfer(msg.sender, withdrawAmount);
        emit WithdrawalSuccessful(msg.sender, claimCycle, withdrawAmount);
    }

    function getClaimCycle(address _address) internal view returns(uint256) {
        if (block.number > stakingWallets[_address].unlockBlock) {
            if (block.number > stakingWallets[_address]._2Claim) {
                if (block.number > stakingWallets[_address]._3Claim) {
                    if (block.number > stakingWallets[_address]._4Claim) {
                        return 4;
                    } else {
                        return 3;
                    }
    ...
    /// @dev recalculateReward() delete later - needed only for testing
    ...
    function calculateAPR(uint256 _depAmount) public view returns (uint256) {
        uint256 _APR = ((tokensPerBlock/precisionLevel)*_depAmount*10512000)/totalDeposited;
        return _APR;
    }
