...
contract Staking {
    ...
    constructor(
        ...
        require(withdrawStart_ > stakingEnds_, "Staking: withdraw start must be after staking ends");
        ...
        returns (bool)
    {
        ...
        if (block.timestamp > stakingEnds) {
            if(firstWithdraw) {
                reflectedFees = COC(tokenAddress).balanceOf(address(this)).sub(stakedBalance).sub(totalReward);
                firstWithdraw = false;
