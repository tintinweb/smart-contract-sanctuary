...
contract NFTBuySell is ERC1155Holder, ReentrancyGuard {
    ...
    constructor(address _nftContract, address _tokenContract) {
        ...
        nftContract = LoudNft(_nftContract);
    ...
    function createMarketSale(uint256 itemId) public nonReentrant {
        ...
        require(
         ...
         require(
            idToMarketItem[itemId].soldQ < idToMarketItem[itemId].amount,
            "All NFTs sold"
        );
        ...
        uint256 feeAmount = (price / 100) * royaltyFee;

        giveRoyalty(tokenId, feeAmount);

        tokenContract.transferFrom(msg.sender, seller, price-feeAmount);
        nftContract.safeTransferFrom(
            address(this),
            msg.sender,
            tokenId,
    ...
    function fetchMarketItems() public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (idToMarketItem[i + 1].soldQ < idToMarketItem[i + 1].amount) {
                itemCount += 1;
            }
        }
    ...
    function giveRoyalty(uint256 _id, uint256 _amount) internal returns (bool) {
        ...
        tokenContract.transferFrom(address(this), creator, _amount);
        return true;
    }
}