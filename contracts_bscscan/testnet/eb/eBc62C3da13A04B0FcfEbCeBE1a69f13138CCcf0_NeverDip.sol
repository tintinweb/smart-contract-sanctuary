...
contract NeverDip is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    address payable public teamWalletAddress = payable(0xF0acec508CEA081c1B7954CFE0D4b8b4ABFB5DA1); // Team Address
    address payable public marketingWalletAddress = payable(0x503D2300B0E790973E83E30bb451e26394Ed6574); // Marketing Address
    address payable public buybackWalletAddress = payable(0x63F252d5F3b483419b3a559D872fa025Af46a145); // Buyback Address
    ...
    string private _name = "NeverDip";
    string private _symbol = "NEVERDIP";
    uint8 private _decimals = 9;

    uint256 public _teamFee = 2;
    uint256 public _prevTeamFee = 2;
    uint256 public _liquidityFee = 2;
    uint256 public _prevLiquidityFee = 2;
    uint256 public _marketingFee = 2;
    uint256 public _prevMarketingFee = 2;
    uint256 public _buybackFee = 10;
    uint256 public _prevBuybackFee = 10;
    uint256 public _totalTaxPercent = 0;
    uint256 public _prevTotalTaxPercent = 0;
    ...
    constructor () {
        ...
        _totalTaxPercent = _teamFee.add(_liquidityFee).add(_marketingFee).add(_buybackFee);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 forTeam = tAmount.div(_totalTaxPercent).mul(_teamFee);
        uint256 forMarketing = tAmount.div(_totalTaxPercent).mul(_marketingFee);
        uint256 forBuyback = tAmount.div(_totalTaxPercent).mul(_buybackFee);
        ...
        if(forBuyback > 0)
        {
            ...
            uint256 buybackWalletShare = newBalance;

            if(buybackWalletShare > 0)
                transferToAddressETH(buybackWalletAddress, buybackWalletShare);
        }
        if(forTeam > 0)
        {
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(forTeam);
            uint256 newBalance = address(this).balance.sub(initialBalance);

            uint256 teamShare = newBalance;
    ...
    function setPreviousTaxes() private {
        _prevTeamFee = _teamFee;
    ...
    function setTaxes(uint256 newTeamFee, uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newBuybackTax) external onlyOwner() {
        setPreviousTaxes();
        _teamFee = newTeamFee;
    ...
    function setBonusTaxes() external onlyOwner() {
        setPreviousTaxes();
        _teamFee = 3;
        _liquidityFee = 3;
        _marketingFee = 3;
