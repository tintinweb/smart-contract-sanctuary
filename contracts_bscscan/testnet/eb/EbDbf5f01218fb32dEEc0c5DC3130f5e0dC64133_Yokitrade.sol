...
contract Yokitrade is owned {
    ...
    constructor(address _yokiWallet, address _routerAddress, address _lpAddress, address _busdWallet){
        yokiWallet = _yokiWallet;
        routerAddress = _routerAddress;
        lpAddress = _lpAddress;
        busdWallet = _busdWallet;
        _totalSupply = IYokiToken(yokiWallet).totalSupply();
...
// This function calls lp pair and checks for the current price of YOKI token in BUSD
    ...
    function buyTokens(uint256 yokiAmount) external {
        ...
        IBEP20(busdWallet).transferFrom(msg.sender, yokiWallet, busdAmount);     //cut BUSD from user
        IYokiToken(yokiWallet).tradeTransfer(yokiWallet, msg.sender, yokiAmount);    // makes the transfers
    ...
    function sellTokens(uint256 yokiAmount) external {
        ...
        IYokiToken(yokiWallet).tradeTransfer(msg.sender, yokiWallet, yokiAmount);    // makes the transfers
        IBEP20(busdWallet).transferFrom(yokiWallet, msg.sender, busdAmount);        // send BUSD to user
