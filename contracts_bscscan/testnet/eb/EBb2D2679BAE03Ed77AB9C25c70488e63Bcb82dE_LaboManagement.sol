...
contract LaboManagement is Laboratory {
    ...
    function buyPotion(uint256 _potionId) public {
        ...
        require(BZAI.transferFrom(msg.sender, gameAddresses.getPaymentsAddress(), _price));

        IPayments(gameAddresses.getPaymentsAddress()).payOwner(_seller, _price);
        laboratoryRevenues[_fromLab] = laboratoryRevenues[_fromLab].add(_price);
