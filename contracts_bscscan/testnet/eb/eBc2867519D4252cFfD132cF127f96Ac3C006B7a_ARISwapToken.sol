...
function openNFTForBiddingForSpecific(uint256 tokenId, uint256 timedurantion) public onlyNftOwner(tokenId) returns(bool){
    ...
    function acceptOffer(uint256 tokenId) external onlyNftOwner(tokenId){
        require(ownerOf(tokenId) == msg.sender,"acceptOffer:only owner");
        require(_exists(tokenId),"Non-existent tokenId");
        require(biddingInfo[biddingInfo[tokenId].biddingId].winner != address(0));
        super.approve(biddingInfo[biddingInfo[tokenId].biddingId].winner,tokenId);
        super.safeTransferFrom(msg.sender,biddingInfo[biddingInfo[tokenId].biddingId].winner,tokenId,"0x");
        address payable buyer = payable(msg.sender);
        uint256 amountFee=biddingInfo[biddingInfo[tokenId].biddingId].bidPrice.mul(default_Fee.div(100));
        buyer.transfer(biddingInfo[biddingInfo[tokenId].biddingId].bidPrice.sub(amountFee));
        payable(owner()).transfer(amountFee);
        delete biddingInfo[tokenId];
        emit OfferAccepted(tokenId,biddingInfo[biddingInfo[tokenId].biddingId].bidPrice,
        msg.sender,biddingInfo[biddingInfo[tokenId].biddingId].winner);
    }
    ...
    function SellOnFixedPrice(uint256 tokenId,uint256 price)public onlyNftOwner(tokenId) returns(bool){
        Fixedprices[tokenId].price=price;
    ...
    function BuyFixedPriceNFTs(uint256 tokenId)payable public returns(bool){
        require(msg.value>=Fixedprices[tokenId].price, "send lower amount in fixed price");
         super.safeTransferFrom(Fixedprices[tokenId].owner,msg.sender,tokenId,"0x");
         payable(Fixedprices[tokenId].owner).transfer(msg.value);
         return true;
    }
