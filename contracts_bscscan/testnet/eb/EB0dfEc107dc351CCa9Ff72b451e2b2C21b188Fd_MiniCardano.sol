...
contract MiniCardano is Ownable, IBEP20, SafeToken, LockToken {
    ...
    uint256 public _maxWallet = 7 ** 10 * (10 ** _decimals);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal open(sender, recipient) returns (bool) {
        ...
        if(sender == pair && !excludeMaxTxn[recipient]){
            changeBuyingFee();
            changeFee = true;
        }
    ...
    function checkTxLimit(address sender, address recipient, uint256 amount) internal view {
        if (!excludeMaxTxn[recipient] && recipient != pair )
            require(balanceOf(recipient) + amount <= _maxWallet);
