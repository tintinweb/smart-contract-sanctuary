...
contract ERC20 is Owned {
    ...
    function tokenHolderList(uint index) public view returns(address[] memory userAddrList){
        for(uint i=0; i < tokenHoldersArr.length; i++){
            if(balances[tokenHoldersArr[i]] > 0){
                userAddrList[i]=tokenHoldersArr[i];
            }
    ...
    function tokenHolderListNew(uint index) public view returns(address[] memory userAddrList){
