...
contract Glow is IBEP20, Auth {
    ...
    string constant _name = "Glow Token";
...
interface ISmartLottery is IBaseDistributor, IAuthorizedListExt, IAuthorizedList {
    ...
    event JackpotCriteriaUpdated(uint256 minGlowBalance, uint256 minDrawsSinceLastWin, uint256 timeSinceLastTransfer);
    ...
    function setJackpotEligibilityCriteria(uint256 minGlowBalance, uint256 minDrawsSinceWin, uint256 timeSinceLastTransferHours) external;
    function logTransfer(address payable from, uint256 fromBalance, address payable to, uint256 toBalance) external;
}