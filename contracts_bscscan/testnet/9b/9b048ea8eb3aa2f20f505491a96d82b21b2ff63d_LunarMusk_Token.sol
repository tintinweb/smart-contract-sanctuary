...
comments in contract from Jack
needs testing!
change testnet router
...
Twitter: twitter.com/LunarMusk
Telegram: t.me/LunarMusk
...
Tokenomics for the contract

https://t.me/LunarMusk
...
contract LunarMusk_Token is Context, IERC20, Ownable {
    ...
    string private _name = "LunarMusk Token";
    ...
    //fee for the auto LP and the dev wallet 
    ...
    //max wallet holding (set to 2% at launch)
    ...
    uint256 public _numTokensSellToAddToLiquidity = 10000 * 10**6 * 10**9;
    ...
    //slow buying is set to 10 seconds between buys.... to avoid bots buying up too much at a low price
    bool public slowFairBuys = true;
    uint8 public justChillFor = 10;
    ...
    //set fee for the dev (BNB) wallet (X100)
    ...
    //removing multiple addresses from the blacklist - Used if you block somebody by mistake
    ...
    //get ready for presale!
      function __prepareForPreSale() external onlyOwner {
       ...
       removeAllFee();
        ...
        _maxWalletToken = 1000000 * 10**6 * 10**9;
    }
    ...
    function __afterPreSale() external onlyOwner {
        ...
        restoreAllFee();
        _maxTxAmount = 10000 * 10**6 * 10**9;			// 1% of supply is the max transaction
        _maxWalletToken = 20000 * 10**6 * 10**9;		// 2% of supply is the max wallet holding
    }
    ...
    function removeAllFee() private {
        if(_FeeReflection == 0 && _liquidityAndPromoFee == 0 && _FeeBurn == 0) return;
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "Err owner zero address");
        require(spender != address(0), "Err spender zero address");
    ...
    function _transfer(
        ...
        if (to != owner() && to != address(this)  && to != address(0x000000000000000000000000000000000000dEaD) && to != uniswapV2Pair){
            ...
            require((heldTokens + amount) <= _maxWalletToken,"Failed transaction, you are trying to buy more than the max wallet limit.");}
        ...
        require(!_isBlacklisted[from] && !_isBlacklisted[to], "Either the buyer or seller is blacklisted. Transaction reverted.");
        ...
        if (from == uniswapV2Pair &&
            ...
            require(stillWaiting[to] < block.timestamp,"You are buying too fast, please wait a few seconds before trying to buy again.");
        ...
        if(from != owner() && to != owner())
            require(amount <= _maxTxAmount, "Transaction reverted, you are trying to buy more than the max transaction limit allows");
...
//LunarMusk was created by GenTokens.com