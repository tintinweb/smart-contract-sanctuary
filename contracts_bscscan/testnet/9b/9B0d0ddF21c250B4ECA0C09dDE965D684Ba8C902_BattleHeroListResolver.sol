...
contract BattleHeroListResolver{
    ...
    constructor(){
        ...
        _factory = IBattleHeroFactory(0x5119Bb48ead3C0295bF3836d0DbA78DA20e6c920);
    }


    function heroes(address owner, uint256 howMany, uint256 cursor) public view returns(IBattleHeroFactory.Hero[] memory, uint256){
        IBattleHeroFactory.Hero[] memory _heroes = heroesOf(owner);
        uint256 length = howMany;
        if (length > _heroes.length - cursor) {
            length = _heroes.length - cursor;
        }
        IBattleHeroFactory.Hero[] memory _filtered = new IBattleHeroFactory.Hero[](length);
        for (uint256 i = 0; i < length; i++) {
            _filtered[i] = _heroes[cursor + i];
        }
        return (_filtered, cursor + length);
    }

    function heroesOf(address owner) public view returns(IBattleHeroFactory.Hero[] memory){
        uint256[] memory _heroesId = _factory.heroesId();        
        uint256 _length = _heroesId.length;
        IBattleHeroFactory.Hero[] memory _heroes = new IBattleHeroFactory.Hero[](_length);
        uint counter = 0;
        for(uint i = 0; i < _length; i++){
            uint256 _tokenId  = _heroesId[i];        
            IBattleHeroFactory.Hero memory _hero  = _factory.heroeOfId(_tokenId);
            if(_hero.owner == owner){                
                _heroes[counter] = _hero;
                counter = counter + 1;
            }
        }
        return _heroes;
