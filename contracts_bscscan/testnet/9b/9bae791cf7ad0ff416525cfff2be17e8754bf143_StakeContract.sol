...
contract StakeContract is Ownable {
    ...
    uint256 public startBlock = 15243227 ; // set start block
    uint256 public minimum_deposit = 4000*10**18; //minimum deposit
    uint256 public endBlock =   15245227 ;  // set end block
    uint256 public firstMonthRewardperblock = 0.000000014109628 * 10 ** 18; //first month block reward % * 1e18
    uint256 public afterRewardperblock = 0.00000000731625 * 10 ** 18; //after first block reward % * 1e18
    uint256 private firstMonthBlockPeriod = 200; //30days block = 836129.0323, we use 864000
    uint256 private unlockblockperiod = 100; //15days block = 418064.5161, we use 432000
    address public rewardaddress = 0x35c77BEfA581Ae0F3861ceF24e7350aF5239c2c5 ; //rewardaddress
    address public stakeaddress = 0x35c77BEfA581Ae0F3861ceF24e7350aF5239c2c5 ; //stakeaddress
    uint256 public totalpoolstacked = 0;
    uint256 public lastRewardBlock = 15243227;  // should be same as start block at the beginning
    ...
    function rewardBalanceOf(address _user) external view returns (uint256) {
        ...
        if ( lastblock > startBlock && lastRewardBlock < lastblock && endBlock > lastRewardBlock) {
            if (lastblock <= _firstMonthBlock[_user]){
                // within 1 month
                ...
                // after 1 month
                if (lastRewardBlock>=_firstMonthBlock[_user]){
                    ...
                    pendingRewardBlockWithinFirstMonth = _firstMonthBlock[_user].sub(lastRewardBlock);
                    pendingRewardBlockAfterFirstMonth= lastblock.sub(_firstMonthBlock[_user]);
            ...
            if (totalpoolstacked > 0){
                ...
                for (uint id = 0; id < length; ++id) {
                    ...
                    if (_stakedBalance[user] == 0){
                        ...
                        if (lastblock <= _firstMonthBlock[user]){
                            // within 1 month
                            pendingRewardBlockWithinFirstMonth = lastblock.sub(lastRewardBlock);
                            ...
                            // after 1 month
                            if (lastRewardBlock>=_firstMonthBlock[user]){
                                pendingRewardBlockAfterFirstMonth = lastblock.sub(lastRewardBlock);
                                ...
                                pendingRewardBlockWithinFirstMonth = _firstMonthBlock[user].sub(lastRewardBlock);
                                pendingRewardBlockAfterFirstMonth= lastblock.sub(_firstMonthBlock[user]);
