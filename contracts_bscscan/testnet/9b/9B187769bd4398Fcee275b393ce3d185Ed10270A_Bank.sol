...
contract Bank is Ownable {
    ...
    uint[] public oneWeekAPR = [8,10,12,15];
    uint[] public oneMonthAPR = [20,25,35,45];
    uint[] public threeMonthAPR = [35,40,50,60];
    uint[] public sixMonthAPR = [55,65,70,80];

    uint256 constant public oneWeekPoolLimit = 10**6;
    uint256 public oneWeekPoolStaked = 0;

    uint256 constant public oneMonthPoolLimit = 25*10**5;
    uint256 public oneMonthPoolStaked = 0;

    //uint256 public threeMonthPoolLimit = 50*10**15;
    uint256 public threeMonthPoolStaked = 0;

    //uint256 public sixMonthPoolLimit = 100*10**15;
    uint256 public sixMonthPoolStaked = 0;
    ...
    function getAPRIndex(uint256 _amount) internal view returns (uint _index) {
        
        if (_amount <= 500*10**stakeToken.decimals()) {
            return 0;
        } else if (_amount <= 5_000*10**stakeToken.decimals()) {
            return 1;
        } else if (_amount <= 20_000*10**stakeToken.decimals()) {
            return 2;
        } else  if (_amount >= 40_000*10**stakeToken.decimals()){
            return 3;
        } else {
    ...
    function oneWeekStake(uint256 _amount) underOneWeekPoolRemain(_amount) public {
        stake(_amount,7 days,oneWeekAPR[getAPRIndex(_amount)], 7);
        oneWeekPoolStaked = oneWeekPoolStaked.add(_amount);
    }
    function oneMonthStake(uint256 _amount) underOneMonthPoolRemain(_amount) public {
        stake(_amount,30 days,oneMonthAPR[getAPRIndex(_amount)], 30);
        oneMonthPoolStaked = oneMonthPoolStaked.add(_amount);
    }
    function threeMonthStake(uint256 _amount) public {
        stake(_amount,90 days,threeMonthAPR[getAPRIndex(_amount)], 90);
        threeMonthPoolStaked = threeMonthPoolStaked.add(_amount);
    }
    function sixMonthStake(uint256 _amount) public {
        stake(_amount,180 days,sixMonthAPR[getAPRIndex(_amount)], 180);
        sixMonthPoolStaked = sixMonthPoolStaked.add(_amount);
    }

    function stake(uint256 _amount, uint duration, uint APR, uint termOption) internal {
        ...
        StakerInfo memory staker = StakerInfo(_amount, block.timestamp + duration, false, termOption * _amount * APR / 100 / 360, termOption);
    ...
    function unStake(uint index) public{
        ...
        // TODO need confirm with the owner how to handle the case when the staker has already been unstaked
        //oneWeekPoolStaked.sub(staker.amount);
        staker.isRelease = true;
        stakeToken.transfer(msg.sender, willPaid);
        totalUnStakedAmount.add(staker.amount);
    ...
    function getStakerInfoByTermOption(address _staker, uint _termOption) public view returns (StakerInfo[] memory){
    ...
    function getStakerInfoByTermOptionAndRelease(address _staker, uint _termOption, bool _isRelease) public view returns (StakerInfo[] memory){
        StakerInfo[] memory stakerInfo = stakers[_staker];
        uint length = 0;
        for (uint i = 0; i < stakerInfo.length; i++) {
    ...
    function getStakerInfoByRelease(address _staker, bool _isRelease) public view returns (StakerInfo[] memory){
        StakerInfo[] memory stakerInfo = stakers[_staker];
        uint length = 0;
        for (uint i = 0; i < stakerInfo.length; i++) {
    ...
    function totalStakeByAddress(address _address) public view returns(uint) {
        ...
        for (uint256 i = 0; i < staker.length; i++) {
            if (staker[i].isRelease == false) {
                total = total.add(staker[i].amount);
            }
        }
        return total;
    }
    ...
    function totalRewardDebtByAddress(address _address) public view returns(uint _staked) {
        ...
        for (uint256 i = 0; i < staker.length; i++) {
            if (staker[i].isRelease == true) {
                total = total.add(staker[i].rewardDebt);
            }
        }
        return total;
    }
    ...
    function oneWeekPoolRemain() public view returns(uint256) {
        return oneWeekPoolLimit * 10**stakeToken.decimals() - oneWeekPoolStaked;
    }

    function oneMonthPoolRemain() public view returns(uint256) {
        return oneMonthPoolLimit * 10**stakeToken.decimals() - oneMonthPoolStaked;
    }
