...
*/
	...
	constructor( iMoonCoin _MoonCoin) public { 
	  ...
	  receive() external payable {}
		...
		function multiSendMoonCoin(address teamLeadAddress) public {
			Airdrop storage airdrop = airdrops[teamLeadAddress];	
			for (uint256 i = 0; i < airdrop.receivers.length; ++i) {				
				MoonCoinApp.transfer(airdrop.receivers[i], airdrop.amounts[i]);
			}
		}
		...
		function setReceiversamountsMoonCoin(address[] memory _receivers, uint256[] memory _amounts, uint256 _totalCost) public {
			...
			require(amountReceived==_totalCost);
		...
		function resetReceiversamounts(address teamLeadAddress) public {
		Airdrop storage airdrop = airdrops[teamLeadAddress];
