...
contract TestBubs is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "TestBubs";
    string private _symbol = "TestBubs";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0xb289bd090CAbECA807F3BD890221d705d44aF90C); // Marketing Address
    address payable public rewardWalletAddress = payable(0xed344d0b7DA5d31A83c95b73eC5C50936C1d43f1); // xrp Address
    ...
    constructor () {
        ...
        _totalTaxPercent = _liquidityFee.add(_marketingFee).add(_rewardFee);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 tokensForLP = tAmount.div(_totalTaxPercent).mul(_liquidityFee).div(2);
        ...
        uint256 totalBNBFee = _totalTaxPercent.sub(_liquidityFee.div(2));
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        if(_burnFee > 0)
        {
            burnShare = feeAmount.mul(_burnFee).div(100);
