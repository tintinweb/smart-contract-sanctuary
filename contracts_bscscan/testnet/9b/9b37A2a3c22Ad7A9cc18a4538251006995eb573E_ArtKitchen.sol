...
//        return arts[_tokenId].itemName;
        IArtState _state = IArtState(stateAddress);
        return _state.itemNameOf(_tokenId);
    }
    ...
    function create(
        ...
        uint256 _newId = _nft.mint(address(this), _itemName, _tokenURI);
//        arts[newId] = Art({ itemName: _itemName, creator: _msgSender(), tokenURI: _tokenURI });
        IArtState state = IArtState(stateAddress);
        state.createArt(_newId, _itemName, _msgSender(), _tokenURI);
        items[_newId] = Item({ seller: _msgSender(), price: _price });

        emit Create(_msgSender(), _price, _newId, _itemName, _tokenURI);
...
//        delete arts[_tokenId];
        _state.deleteArt(_tokenId);
        _state.deleteState(_tokenId);
