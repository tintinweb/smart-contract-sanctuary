...
contract Manager is Ownable {
  ...
  IERC721Enumerable public stone;
  ...
  constructor(address _nft, address _stone, uint256 _priceEgg, uint256 _priceStone) public {
    nft = ICryptoDogeNFT(_nft);
    stone = IERC721Enumerable(_stone);
    priceEgg = _priceEgg;
  ...
  function getStoneList(address account) external view returns (uint256[] memory) {
    uint256 balance = stone.balanceOf(account);
    uint256[] memory tokenIds = new uint256[](balance);
    for (uint i = 0; i < balance; i++) {
      tokenIds[i] = stone.tokenOfOwnerByIndex(account, i);
    }
    return tokenIds;
  }
