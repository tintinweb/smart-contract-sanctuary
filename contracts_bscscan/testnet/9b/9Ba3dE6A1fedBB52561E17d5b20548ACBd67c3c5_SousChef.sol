...
contract SousChef {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;   // How many STEAK tokens the user has provided.
        ...
        //
        // Whenever a user deposits or withdraws STEAK tokens to a pool. Here's what happens:
    ...
    // Info of each user that stakes Steak tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = steak.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 steakSupply = steak.balanceOf(address(this));
        if (steakSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(steakSupply));
        poolInfo.lastRewardBlock = block.number;
    }


    // Deposit Steak tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        steak.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Steak tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        steak.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        steak.safeTransfer(address(msg.sender), user.amount);
