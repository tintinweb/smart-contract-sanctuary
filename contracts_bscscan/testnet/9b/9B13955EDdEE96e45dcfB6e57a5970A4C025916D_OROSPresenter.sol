  ...
  function pendingReward(address _userAddress) external view returns (uint[] memory) {
    ...
    if(userBalance > 0) {
      for(uint i; i < length; i++){
        ...
        uint share = (userBalance * 1e9)/totalStaked();
        uint userReward = ((share * rewardPool[tokenAddress])/1e9) - userDebt[tokenAddress][_userAddress];
  ...
  function claimAllReward() external {
    ...
    if(userBalance > 0) {
      for(uint i; i < tokens.length; i++){
        address tokenAddress = tokens[i];
        uint share = (userBalance * 1e9)/totalStaked();
        uint userReward = ((share * rewardPool[tokenAddress])/1e9) - userDebt[tokenAddress][msg.sender];
        if(userReward > 0) {
  ...
  function _taxCollection(address _from, address _to, uint256 _amount, bool _isBuy, bool _isSell) internal {
    ...
    for (uint i; i < tokens.length; i++) {
      ...
      if(_isSell == true ) {
        ...
        uint share = (userBalance * 1e9)/totalStaked();
        uint userReward = ((share * rewardPool[info.addr])/1e9) - userDebt[info.addr][recipient];
