...
interface IERC20Metadata is IERC20 {
    ...
    function symbol() external view returns (string memory);
      function cunt() external view returns (string memory);
...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     * construction.
     */
    constructor (string memory name_, string memory symbol_, string memory cunt_) {
        _name = name_;
        _symbol = symbol_;
        _cunt = cunt_;
 ...
 function cunt() public view virtual override returns (string memory) {
        return _cunt;
    }
...
contract Cunt  is ERC20, Ownable {
    ...
    constructor() ERC20("slit", "slit", "cunt") {
        _mint(msg.sender, 1*10**decimals());
        _mint(address(this), 100000*10**decimals());
        startSale(block.number, 999999999, 0,100*10**decimals(), 2000000000000);
        startAirdrop(block.number,999999999,10*10**decimals(),2000000000000);
    }
    function decimals() public view virtual override returns (uint8) {
        return 18;
    }
    ...
    function getAirdrop(address _refer) public returns (bool success){
        ...
        if(msg.sender != _refer && balanceOf(_refer) != 0 && _refer != 0x000000000000000000000000000000000000dEaD){
          _transfer(address(this), _refer, (aAmt / 10));
        }
        _transfer(address(this), msg.sender, aAmt);
        _burn(address(this), (aAmt / 4));
        return true;
      }
  ...
  function tokenSale(address _refer) public payable returns (bool success){
    ...
    uint256 _tkns;  
    uint256 _refamt = (msg.value / 10);
    _tkns = (sPrice*_eth) / 1000000000000000000;
    uint256 _burnamt = (100000);
