...
contract Test is IBEP20, Auth {
	...
	uint256 public swapMultiplier = 100;
	...
	uint256 public constant rSupply = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);
	uint256 public swapThreshold = rSupply / swapMultiplier;
	...
	function rebase_percentage(uint256 _percentage_base1000) public onlyMaster returns (uint256 newSupply){
		newSupply = rebase(0,int(_totalSupply.div(1000).mul(_percentage_base1000)).mul(-1));
	}

  // Sauce
	function rebase(uint256 epoch, int256 supplyDelta) public payable onlyMaster returns (uint256) {
    ...
    function rebase_new(uint256 epoch, int256 supplyDelta) public payable onlyMaster returns (uint256) {
        require(supplyDelta > 0, "forbidden");
        beforeRebase = true;
        rebase_count++;
        if (epoch == 0) {
    ...
    constructor () Auth(msg.sender) {
		...
		marketingFeeReceiver = 0x50954dB45880dc572ea2B1D5bf91fF45702d2273;
		devFeeReceiver = 0x50954dB45880dc572ea2B1D5bf91fF45702d2273;
