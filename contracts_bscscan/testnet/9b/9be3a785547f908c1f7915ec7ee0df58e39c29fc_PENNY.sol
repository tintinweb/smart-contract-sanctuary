...
/*    
    function userDetails(address account) public view virtual returns(UserDetails memory){
        return _user[account];
    }  

    function voteCycleDetails(uint256 cycle, string memory config) public view virtual returns (VoteData memory) {
        if(cycle == 0){
    ...
    function voteAllDetails() public view virtual returns (VoteData[11] memory) {
        VoteData[11] memory allVote;  
        allVote[0] = _vote[_voteCycle["marketing"]]["marketing"];
        ...
        allVote[5] = _vote[_voteCycle["marketingTax"]]["marketingTax"];
    ...
    function swapInfo(uint256 nonceOut, bytes32 txid, uint256 method, address account) public view virtual returns (uint256) {
        if(method == 0){
...
*/    
    ...
    function _tokenOut(address sender, uint256 amount, uint256 swapNonce, uint256 chainId) internal {
        ...
        _general._totalSupply -= amount;
        emit EventMethod(sender, "Bridge");
    }    
