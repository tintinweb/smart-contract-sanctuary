...
contract UnsealMe is Context, Ownable, ReentrancyGuard {
    ...
    function getPayableAmount() public view returns (uint256) {
        return ((address(this).balance) * _entryPricePercentage) / (100);
    }

    function getPayableCreatorAmount() public view returns (uint256) {
        return (getPayableAmount() * _creatorPercentage) / (100);
    }

    // low level function
    function unseal() public nonReentrant payable {
        uint256 payableAmount = ((address(this).balance) * _entryPricePercentage) / (100);
        uint256 payableCreatorAmount = (payableAmount * _creatorPercentage) / (100);
        ...
        emit UnsealCalled(_unsealTimeEnd, _latestUnsealer, ((address(this).balance) * _entryPricePercentage) / (100));
    }
    ...
    function claim() external{
        ...
        uint256 PayableCreatorAmount = (currentBalance * _creatorPercentage) / (100);
