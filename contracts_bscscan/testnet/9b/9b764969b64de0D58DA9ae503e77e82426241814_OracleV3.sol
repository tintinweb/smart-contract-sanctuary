...
contract OracleV3 is IOracleV3, OracleV2 { 
    ...
    event withdrawSeedmonRequested(address requester, uint256 requestId, uint256 tokenId, uint256 provideGas);

    function requestWithdrawSeedmon(uint256 requestId, uint256 tokenId) 
        external 
        payable  
        override
        onlyOpen 
        requireGas 
    {
        ...
        emit withdrawSeedmonRequested(requester, requestId, tokenId, providedGas);
    }

    // =============== Prophet method ================
    function provideWithdrawSeedmon(address requester, uint256 requestId, uint256 tokenId, uint256 exp) 
        external 
        onlyProphet 
    {
        ...
        IClientV3(requester).onWithdrawSeedmonReceived(requestId, tokenId, exp);
