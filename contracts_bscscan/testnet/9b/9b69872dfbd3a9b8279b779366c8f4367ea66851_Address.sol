...
contract BabyTest is IBEP20, Ownable
{
    ...
    address public constant TeamWallet=0xC63A8fD2Af32d892453F943659d68459850be6bB;
    ...
    //Sets Liquidity Release to 100% at a time and prolongs liquidity Lock for a Week after Release. 
    //Should be called once start was successful.
    bool public liquidityRelease100Percent;
    function TeamlimitLiquidityReleaseTo100Percent() public onlyTeam{
        liquidityRelease100Percent=true;
    }
    ...
    function TeamReleaseLiquidity() public onlyTeam {
        ...
        if(liquidityRelease100Percent)
        {
            _liquidityUnlockTime=block.timestamp+DefaultLiquidityLockTime;
            //regular liquidity release, only releases 100% at a time and locks liquidity for another week
            amount=amount*10/10;
        ...
        else
        {
            ...
            //liquidityRelease100Percent should be called once everything is clear
            liquidityToken.transfer(TeamWallet, amount);
        }
    }
    ...
    function TeamRemoveLiquidity(bool addToStaking) public onlyTeam {
        ...
        uint256 amount = liquidityToken.balanceOf(address(this));
        if(liquidityRelease100Percent){
            amount=amount*10/10; //only remove 100% each
        } 
    ...
    //Can only be called 10 days after Liquidity unlocks 
    function TeamRemoveRemainingBNB() public onlyTeam{
        require(block.timestamp >= _liquidityUnlockTime+10 days, "Not yet unlocked");
        _liquidityUnlockTime=block.timestamp+DefaultLiquidityLockTime;
