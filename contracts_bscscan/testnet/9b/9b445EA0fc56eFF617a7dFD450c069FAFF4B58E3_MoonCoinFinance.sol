...
contract MoonCoinFinance  {
	...
	event Custom_RefBonus(uint256 eventId, uint256 timestamp,  address indexed user, address indexed upline, uint256 i, uint256 amount, uint256 promoMultiplier, uint256 tokenBalance, uint256 tokenBalanceFactor);
			...
			for (uint256 i = 0; i < 10; i++) {
				if (upline != address(0) && upline!= msg.sender) {				
					...
					if(tokenBalanceFactor > i+1 || tokenBalanceFactor >= 5) { // token hold good					
						...
						eventId++;
						emit Custom_RefBonus(eventId, block.timestamp, msg.sender,  upline,i, amount, promoMultiplier, tokenBalance, tokenBalanceFactor);
						eventId++;
