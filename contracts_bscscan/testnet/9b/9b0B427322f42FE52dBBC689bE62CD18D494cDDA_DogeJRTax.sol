...
contract DogeJRTax is BEP20 {
    uint256 public constant MAX_SUPPLY = 1000000000 ether; // max supply
    ...
    // Min amount to liquify. (default 250 GOLDs)
    ...
    IUniswapV2Router02 public SwapRouter = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //testnet
    
    // The trading pair
    address public goldSwapPair;
    // In swap and liquify
     ...
     */
    constructor() public BEP20("DogeJr", "DOGEJR") {
        ...
        mint(msg.sender, 150000000 ether); // Mint 15% warchest, initial circulation and lottery pool/ wallet
    }

    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        require(totalSupply().add(_amount) < MAX_SUPPLY, "max supply has been reached");
        _mint(_to, _amount);
    ...
    function _transfer(
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "DogeJR::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "DogeJR::transfer: Tax value invalid");

            super._transfer(sender, BURN_ADDRESS, burnAmount);// change burn to lotto ?
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the GoldSwap pair path of token -> weth
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(
            ...
            "DogeJR::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate."
        );
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "DogeJR::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateSwapPair() public onlyOperator {
        goldSwapPair = IUniswapV2Factory(SwapRouter.factory()).getPair(address(this), SwapRouter.WETH());
        require(goldSwapPair != address(0), "DogeJR::updateSwapPair: Invalid pair address.");
        emit SwapRouterUpdated(msg.sender, address(SwapRouter), goldSwapPair);
    }
    ...
    function delegateBySig(
        ...
        require(now <= expiry, "DogeJR::delegateBySig: signature expired");
