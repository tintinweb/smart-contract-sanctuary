...
contract JurassicPeriod is Ownable {
    ...
    address payable public platform_address = 0xA4A8C6fb91d606c9deAede7661f6Fc8e797C7ee1;
    ...
    uint256[] public animal_opentimes =     [1640361600,1640361600,1641312000,1642176000,1643040000,1643904000];
    ...
    event BuyFeed(address indexed addr,uint256 animal_id);
    ...
    */
        ...
        private
    {
        ...
        // verify the referral animal is feeded or not
        ...
        uint256 _bonus_amount = _amount * _bonus_rate / 100;
        //update referral player bonus data
        ref_animal.referral_bonus += _bonus_amount;
        ref_animal.total_referral_bonus += _bonus_amount;
    }
    ...
    */
    function getUserAnimalList(address _addr)
        ...
        returns(
            ...
            uint256[] memory times, uint256[] memory expires,uint256[] memory endeds,
            uint256[] memory eggs
        )
    {
        //user animal list
        ...
        uint256[] memory _statuses = new uint256[](6);
        uint256[] memory _times = new uint256[](6);
        uint256[] memory _expires = new uint256[](6);
        uint256[] memory _endeds = new uint256[](6);
        uint256[] memory _eggs = new uint256[](6);
        
        for(uint256 i = 0; i < 6; i++){
            _payouts[i] = _animals[i].payout;
            _statuses[i] = _animals[i].status;
            _expires[i] = _animals[i].expire;
            _times[i] = _animals[i].time;
            _endeds[i] = _animals[i].ended;
            _eggs[i] = _animals[i].dividends + _animals[i].referral_bonus;
        }
        return (
            animal_ids,
            _payouts,
    ...
    */
    function getAnimalFeedList()
        view 
        external 
        returns(uint256[] memory ids,uint256[] memory prices,uint256[] memory statuses)
    {
    ...
    */
        ...
        external 
    {
        ...
        PlayerAnimal[] memory animals = _getUserAnimalList(msg.sender);
        PlayerAnimal memory player = animals[_animal_id-1];
        require(player.referral == address(0), "Referral has been set");
        require(_referral != address(0), "Invalid Referral address");
        
        PlayerAnimal[] memory ref_animals = _getUserAnimalList(_referral);
        PlayerAnimal memory ref_player = ref_animals[_animal_id-1];
    ...
    * @dev user do buy animal action,update user animal data
    */
    function buyFeed(uint256 _animal_id) 
        external
        payable
    {
        require(_animal_id >= 1 && _animal_id <= 6 , "Invalid Animal Id");
        ...
        uint256 _payMethod = animal_pays[_animal_id-1];
        uint256 blocktime = uint256(block.timestamp);
        uint256 _feed_price = _getAnimalFeedPrice(_animal_id);
        ...
        if(animal.expire > blocktime){
            ...
            animal.expire = animal.expire.add(86400);
            animal.ended = animal.expire.add(86400*3); //ended of 3 day
        }
        else {
            uint256 _profit_amount = _animal.payout;
            animal.dividends += _profit_amount;
            animal.total_dividends += _profit_amount;
            //update referral payout data
            _referralPayout(msg.sender,_profit_amount,_animal_id);
            //update feed expire time
            animal.last_payout = blocktime;
            animal.expire = blocktime.add(86400);
            animal.ended = animal.expire.add(86400*3); //ended of 3 day
        }
        emit BuyFeed(msg.sender, _animal_id);
    }
    ...
    */
    ...
    /*
    * @dev user do exchange action, update user total havest data, grant rereferral bonus
    */
    function exchange() 
        payable 
        external 
    {
        ...
        //convert the egg to token with the exchange rate
        for (uint8 _animal_id=1;_animal_id <= 6;_animal_id++) 
        {
            PlayerAnimal storage animal = _getStorageAnimal(msg.sender, _animal_id);
            uint256 _dividends = animal.dividends;
            uint256 _referral_bonus = animal.referral_bonus;
            
            //calculate egg exchange data
            _total_dividends += _dividends / animal_exchange_rates1[_animal_id-1] * animal_exchange_rates2[_animal_id-1];
            _total_referral_bonus += _referral_bonus / animal_exchange_rates1[_animal_id-1] * animal_exchange_rates2[_animal_id-1];
            
            //reset profit data
            animal.dividends = 0;
            animal.referral_bonus = 0;
        }
