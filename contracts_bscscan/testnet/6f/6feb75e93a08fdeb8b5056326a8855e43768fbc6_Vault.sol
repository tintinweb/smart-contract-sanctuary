...
contract Vault is Context, Ownable {
	...
	uint256 constant _minAmount = 1000000000000000000; // 10**18
 	IBEP20 _token = IBEP20(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
	...
	function lock(uint256 lockAmount, uint256 lockPeriod) external returns (bool) {
	    require( lockPeriod == 7 || lockPeriod == 30 || lockPeriod == 365, "Invalid Lock Days");
		...
		LockRecord memory lockRecord = LockRecord(lockAmount, unlockAmount, block.timestamp, lockPeriod);
		_userLockedRecords[sender].push(lockRecord);
		emit Lock(sender, lockAmount, lockPeriod);
		return true;
	}
 
	function unlock(uint id) external returns (bool) {
		address sender = _msgSender();
		...
		uint lockAmount = recordToUnlock.lockAmount;
		require(timeElapsed > recordToUnlock.lockPeriod, "Still locked");
		_token.transfer( sender, lockAmount);
