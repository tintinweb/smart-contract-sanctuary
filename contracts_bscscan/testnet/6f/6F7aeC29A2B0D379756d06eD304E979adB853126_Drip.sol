...
// File @openzeppelin/contracts/utils/introspection/[email protected]3.0
...
interface IDrip is IERC721 {
    ...
    function lock(address recipient, IERC20 token, uint256 amount, uint256 startDate, uint256 endDate, uint256 campaignId, bool freezeOwner) external returns (uint256);
...
contract Drip is IDrip, ERC721 {
    ...
    function lock(address _recipient, IERC20 _token, uint256 _amount, uint256 _startDate, uint256 _endDate, uint256 _campaignId, bool _ownerFrozen) external override returns (uint256) {
        require(_startDate <= _endDate, "Start date cannot be later than end date");
        
        uint256 lockId = timelockCount;
        timelockCount += 1;
    ...
    function doClaim(uint256 id) internal {
        ...
        token[id].transfer(msg.sender, unclaimed);
    }
