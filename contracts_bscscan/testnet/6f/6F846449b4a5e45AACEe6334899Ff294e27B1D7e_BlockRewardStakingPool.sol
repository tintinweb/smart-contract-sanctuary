...
contract BlockRewardStakingPool is Ownable {
    ...
    function _deposit(address user, uint256 amount) private {
        require(amount > MIN_INVESTMENT, "NCSStakingPool:Deposit less than min investment");
        if (MAX_INVESTMENT > 0) {
            require(amount < MAX_INVESTMENT, "NCSStakingPool:Deposit exceed max investment");
        }
        if (START_DATE > 0) {
            require(block.timestamp > START_DATE, "NCSStakingPool:Not start yet");
            if (DURATION > 0) {
                require(block.timestamp < START_DATE.add(DURATION), "NCSStakingPool:Pool is over");
            }
        }
