...
import "./interfaces/IBondlyStakingBSC.sol";


contract BondlyStakingBSC is IBondlyStakingBSC, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
	using SafeMathUpgradeable for uint256;
	using SafeERC20Upgradeable for IERC20Upgradeable;

	IERC20Upgradeable public bondlyToken;
	...
	uint256 public bondlyTokensLocked;
	...
	function initialize(address _bondlyToken, address _owner)
		external
		override
		initializer
	{
		bondlyToken = IERC20Upgradeable(_bondlyToken);
	...
	function setRewards(
		...
		uint256 unlockedTokens = _getFutureBondlyTokens();

		rewardPerBlock = _rewardPerBlock;
		firstBlockWithReward = _startingBlock;
		lastBlockWithReward = firstBlockWithReward.add(_blocksAmount).sub(1);

		uint256 lockedTokens = _getFutureBondlyTokens();
		bondlyTokensLocked = bondlyTokensLocked.sub(unlockedTokens).add(lockedTokens);
		require(
			bondlyTokensLocked <= bondlyToken.balanceOf(address(this)).sub(totalStaked),
	...
	function recoverNonLockedBondlyTokens() external override onlyOwner {
		uint256 nonLockedTokens = bondlyToken.balanceOf(address(this)).sub(totalStaked).sub(bondlyTokensLocked);

		bondlyToken.safeTransfer(owner(), nonLockedTokens);
		emit BondlyTokensRecovered(nonLockedTokens);
	}
	...
	function stake(uint256 _amount) external override whenNotPaused nonReentrant updateReward(msg.sender) {
		...
		bondlyToken.safeTransferFrom(msg.sender, address(this), _amount);
	...
	function withdraw(uint256 _amount) public override nonReentrant updateReward(msg.sender) {
		...
		bondlyToken.safeTransfer(msg.sender, _amount);
	...
	function getReward() public override nonReentrant updateReward(msg.sender) {
		...
		if (reward > 0) {
			...
			bondlyToken.safeTransfer(msg.sender, reward);
			bondlyTokensLocked = bondlyTokensLocked.sub(reward);
	...
	function _getFutureBondlyTokens() internal view returns (uint256) {
...
interface IBondlyStakingBSC {
	...
	event BondlyTokensRecovered(uint256 amount);
	...
	function recoverNonLockedBondlyTokens() external;
