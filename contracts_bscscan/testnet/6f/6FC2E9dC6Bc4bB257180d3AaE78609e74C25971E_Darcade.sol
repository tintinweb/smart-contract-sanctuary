...
contract Darcade is ReentrancyGuard, Ownable {
    ...
    */
     ...
     */
    function insertCoin(string memory gameSessionId) public nonReentrant payable {
        address beneficiary = _msgSender();
        uint256 weiAmount = msg.value;
        _preValidatePurchase(beneficiary, weiAmount);

        updateUserGameSession(gameSessionId);
        _forwardFunds();

        emit GameSession(beneficiary, _credits[beneficiary], gameSessionId);
    }

    /**
     * @dev Adds a game in session value
     */
    function updateUserGameSession(string memory gameSessionId) internal {
        userGameSession[gameSessionId] = _msgSender();
    }
