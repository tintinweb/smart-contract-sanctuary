...
contract DHNETWORKTESTS is ERC20, Ownable {
    ...
    function getTimeUntilNextTransfer(address from) external view returns(uint256){
        if(_sellsHistoryPerAddress[from].sellWait < block.timestamp.sub(_sellsHistoryPerAddress[from].sellTime)){
                return 0;
        }
    ...
    function updateAddressLastSellData(address from, uint256 amount) private {
        ...
        _sellsHistoryPerAddress[from].salesAmount += amount;
        if(_sellsHistoryPerAddress[from].sellWait < block.timestamp.sub(_sellsHistoryPerAddress[from].sellTime)){
            _sellsHistoryPerAddress[from].sellWait = 0;
            return;
        }
    ...
    function _maxWalletNotReached(address to, uint256 amount) public view returns (bool) {
