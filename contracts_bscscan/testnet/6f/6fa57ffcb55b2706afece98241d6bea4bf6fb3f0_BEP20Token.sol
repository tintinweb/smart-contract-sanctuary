...
contract TokenRecords {
    ...
    mapping (address => tokenDataStruct) public tokenData; // address -> token details mapping
    address[] public tokens; // Token list
    uint public tokenCount; // Token Count
    ...
    function recordNewToken(address token_address) internal returns (uint) {
        // Create a new token listing
        tokenDataStruct memory new_token;
        // Basic data
        new_token.user_address  = msg.sender;
        new_token.token_address = token_address;
        new_token.dateAdded     = now;
        new_token.blockNumber   = block.number;
        // Get the symbol
        (bool success_symbol, bytes memory _symbol) = token_address.call(abi.encodeWithSignature("symbol()"));
        require(success_symbol==true, "Can't access symbol. Not a BEP20 token.");
        new_token.token_symbol  = abi.decode(_symbol, (string));
        // Get the name
        (bool success_name, bytes memory _name) = token_address.call(abi.encodeWithSignature("name()"));
        require(success_name==true, "Can't access name. Not a BEP20 token.");
        new_token.token_name    = abi.decode(_name, (string));
        // Get the owner
        (bool success_owner, bytes memory _ownerAddr) = token_address.call(abi.encodeWithSignature("getOwner()"));
        require(success_owner==true, "Can't access owner. Not a BEP20 token.");
        new_token.token_owner   = abi.decode(_ownerAddr, (address));
        // Record
        tokenData[token_address] = new_token;
        tokenCount += 1;
 ...
 * BEP20 Token
 */
contract BEP20Token is Context, IBEP20, Ownable, TokenRecords {
    using SafeMath for uint256;
    
    // C: 0x6da73f3e3c3291dfbe6bfe1ae8aeeca759e91fba
    // O: 0x6Fa02ed6248A4a78609368441265a5798ebaFC78
    // W1: 0x6Fa02ed6248A4a78609368441265a5798ebaFC78
    // W2: 0xF80fCFDe462a37D9875e224e1C0180dbFEad81f9
    ...
    // Modifier: Verify the caller has enough tokens
    modifier hasEnoughTokens() {
        require(_balances[msg.sender]>=listingFees, "Not enough coins to list");
        _;
    }
    
    
    function setNewListingFees(uint newFees) external onlyOwner returns (uint) {
        listingFees = newFees;
        return newFees;
    }
