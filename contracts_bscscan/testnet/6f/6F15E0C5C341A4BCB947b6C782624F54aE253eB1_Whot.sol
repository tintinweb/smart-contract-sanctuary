...
struct Match {
    ...
    bool ended;
    // The proposed played card that needs attesting
    uint256 proposedPlay;
    // Indicates that the proposed played card is the last card.
    bool proposedLastCard;
    // Indicates that the last play was attested by the dealer.
    bool lastPlayAttested;
}
...
contract Whot is ERC20Burnable, RandomNumberConsumer, CardEngine {
    ...
    mapping(address => uint256) public dealers;
    mapping(address => uint256) public minters;
    mapping(address => RandReq) public randRequests;
    uint256 public dealersReward;
    mapping(address => PlayStake) public playStake;
    ...
    uint256 public unstakeWaitDuration;
    uint256 public partialMatchReward;
    ...
    modifier onlyDealer() {
        require(dealers[msg.sender] > 0, "Sender must be a dealer");
        _;
    }
    ...
    event SetVRFFee(uint256 fee);
    event SetUnstakeWaitDuration(uint256 fee);
    event SetDeployer(address indexed addr);
    event SetDealer(address indexed addr);
    event RemoveDealer(address indexed addr);
    event AddMinter(address indexed addr);
    event RemoveMinter(address indexed addr);
    event SetShuffleCost(uint256 cost);
    event SetMatchInactiveAfter(uint256 cost);
    event SetRandRequestExpiryAfter(uint256 value);
    event SetPartialMatchReward(uint256 value);
    event SetMatchEntryFee(uint256 value);
    event SetMatchStarterReward(uint256 value);
    event SetMatchWinnerReward(uint256 value);
    event SetMatchDealerReward(uint256 value);
    event PlayerStaked(address indexed staker, uint256 indexed amount);
    event PlayerUnStaked(address indexed staker, uint256 indexed amount);
    event Shuffle(address indexed addr, bytes32 indexed reqId);
    ...
    constructor(
        ...
        uint256 _partialMatchReward,
        uint256 _matchDealerReward,
        uint256 _matchInactiveAfter,
        uint256 _randRequestExpiryAfter,
        uint256 _shuffleCost,
        uint256 _unstakeWaitDuration
    )
        ...
        )
    {
        ...
        partialMatchReward = _partialMatchReward;
        matchInactiveAfter = _matchInactiveAfter;
        randRequestExpiryAfter = _randRequestExpiryAfter;
        shuffleCost = _shuffleCost;
        unstakeWaitDuration = _unstakeWaitDuration;
    ...
    function setDeployer(address newDeployer) public onlyDeployer {
        ...
        emit SetDeployer(newDeployer);
    }

    /// @notice set a new dealer.
    /// @param newDealer is the address of the new dealer.
    function setDealer(address newDealer) public onlyDeployer {
        dealers[newDealer] = block.number;
        emit SetDealer(newDealer);
    }

    /// @notice removes a dealer.
    /// @param addr is the address of the dealer.
    function removeDealer(address addr) public onlyDeployer {
        delete dealers[addr];
        emit RemoveDealer(addr);
    }

    /// @notice set the number of blocks to wait before unstaking.
    /// @param dur is the address of the dealer.
    function setUnstakeWaitDuration(uint256 dur) public onlyDeployer {
        unstakeWaitDuration = dur;
        emit SetUnstakeWaitDuration(dur);
    }

    /// @notice set the cost of a shuffle operation.
    /// @param value is the new value
    function setShuffleCost(uint256 value) public onlyDeployer {
        shuffleCost = value;
        emit SetShuffleCost(value);
    }
    ...
    function setMatchInactiveAfter(uint256 value) public onlyDeployer {
        ...
        emit SetMatchInactiveAfter(value);
    }
    ...
    function setRandRequestExpiryAfter(uint256 value) public onlyDeployer {
        ...
        emit SetRandRequestExpiryAfter(value);
    }

    /// @notice set the partial match reward.
    /// @param value is the new
    function setPartialMatchReward(uint256 value) public onlyDeployer {
        partialMatchReward = value;
        emit SetPartialMatchReward(value);
    }
    ...
    function setMatchStarterReward(uint256 amount) public onlyDeployer {
        ...
        emit SetMatchStarterReward(amount);
    }
    ...
    function setMatchWinnerReward(uint256 amount) public onlyDeployer {
        ...
        emit SetMatchWinnerReward(amount);
    }
    ...
    function setMatchDealerReward(uint256 amount) public onlyDeployer {
        ...
        emit SetMatchDealerReward(amount);
    }
    ...
    /// @notice stake will burn matchEntryFee and create a stake record.
    function stake() external {
        burn(matchEntryFee);

        PlayStake storage ps = playStake[msg.sender];
        if (ps.createdAt == 0) {
            playStake[msg.sender] = PlayStake(
                block.number,
                block.number,
                block.number + unstakeWaitDuration,
                matchEntryFee
            );
        } else {
            ps.updatedAt = block.number;
            ps.unlockAt = block.number + unstakeWaitDuration;
            ps.amount = ps.amount.add(matchEntryFee);
        }

        emit PlayerStaked(msg.sender, matchEntryFee);
    }

    /// @notice unstake a players stake
    function unstake() external {
        PlayStake memory _stake = playStake[msg.sender];
        require(_stake.createdAt > 0, "Must have stake");
        require(_stake.unlockAt <= block.number, "Must reach unlock time");

        delete playStake[msg.sender];
        _mint(msg.sender, _stake.amount);
        emit PlayerUnStaked(msg.sender, _stake.amount);
    }

    /// @notice enter enters the sender in to the matchmaking pool.
    /// Rules:
    /// - The player must not be in the pool.
    /// - The Must be in an active match.
    ...
    function enter() external {
        ...
        require(
            playStake[msg.sender].amount >= matchEntryFee,
            "Stake must be sufficient"
        );
        playerPool.push(Player(block.number, msg.sender, 0));
    ...
    /// @notice requests for random number.
    function shuffle() external {
        ...
        // Burn shuffle cost and credit to dealer reward
        _burn(msg.sender, shuffleCost);
        dealersReward = dealersReward.add(shuffleCost);
        ...
        if (
            ...
            emit Shuffle(msg.sender, randRequests[msg.sender].reqID);
        }
    }
    ...
    function makeMatch() external returns (bool) {
        require(playerPoolIndex[msg.sender] > 0, "Sender must be in pool");
        require(playerPool.length >= 2, "Player pool size must be 2 or more");

        // If there is a pending randomness request, check if result has arrived.
        // If not, return false.
        uint256 randomNum = getVRFResult(randRequests[msg.sender].reqID);
        if (randomNum == 0) {
            return false;
        }
    ...
    function exit() external {
        ...
        delete randRequests[player.addr];
    ...
    function computeRewards(
        ...
        // increment dealer's reward.
        dealersReward = dealersReward.add(matchDealerReward);
    ...
    /// @dev isMatchPlayer checks if an address is a player in the match
    /// @param addr is the address of the player.
    /// @param matchIdx the match index
    function isMatchPlayer(address addr, uint256 matchIdx)
        internal
        view
        returns (bool)
    {
        return
            matches[matchIdx].playerA.addr == addr ||
            matches[matchIdx].playerB.addr == addr;
    }

    /// @dev play allows a player to propose the next play.
    ...
    function play(
        ...
        require(
            isMatchPlayer(msg.sender, matchIdx - 1),
            "Must be a match player"
        );
        ...
        // Must be the turn of the sender.
        if (m.turn != msg.sender) revert("Not player turn");

        // Dealer must have attested to the last play
        // or it must be the first play of the game.
        require(
            (m.proposedPlay == 0) || m.lastPlayAttested,
            "Dealer must attest last play"
        );
        ...
        // Set new match state
        m.proposedPlay = card;
        m.proposedLastCard = lastCard;
        m.lastPlayAttested = false;

        // Emit PlayProposed
        emit PlayProposed(matchIdx, msg.sender, card, lastCard);
    }

    /// @notice attest will attest the correctness of the last proposed
    /// play of a match, share reward or set the next player turn.
    function attest(uint256 matchIdx, bool ok) external onlyDealer {
        require(matchIdx <= matches.length, "Match must exist");
        Match storage m = matches[matchIdx - 1];

        // The match must have a proposed play
        require(m.proposedPlay > 0, "Match must have proposed play");

        // Commit play on approval.
        if (ok) {
            m.lastPlay = m.proposedPlay;
            bool lastCard = m.proposedLastCard;
            m.lastPlayAt = block.number;

            // If this was the last card, end match and mint reward.
            if (lastCard) {
                computeRewards(m.turn, m, matchIdx);
                m.ended = true;
                emit MatchEnded(matchIdx, msg.sender);
            } else {
                // Set next player turn.
                address otherPlayer = m.playerA.addr;
                if (m.turn == m.playerA.addr) otherPlayer = m.playerB.addr;
                m.turn = getNextPlayer(m.lastPlay, m.turn, otherPlayer);
            }
        }

        m.proposedPlay = 0;
        m.proposedLastCard = false;
        m.lastPlayAttested = true;
    }
    ...
    function endMatch(uint256 matchIdx) external {
        require(matchIdx <= matches.length, "Match must exist");
        Match storage m = matches[matchIdx - 1];
        require(!m.ended, "Match must not have ended");
        require(
        ...
        // If a play was recorded, the opponent who missed their turn is the loser.
        // If `turn` is non-zero, return give winner a percentage of match reward.
        ...
        if (m.lastPlay > 0) {
            ...
            // Mint a partial match reward for the winner
            _mint(winner.addr, partialMatchReward);
        }
    ...
    /// @notice mint and withdraw dealers reward
    function withdrawDealersReward(address dest) external onlyDealer {
        require(dealersReward > 0, "Must have non-zero reward");
        _mint(dest, dealersReward);
        dealersReward = 0;
    }

    /// @dev withdraw LINK from contract.
    function withdrawLINK(address to) external onlyDeployer {
...
contract CardEngine {
    ...
    /// @param target is the proposed card.
