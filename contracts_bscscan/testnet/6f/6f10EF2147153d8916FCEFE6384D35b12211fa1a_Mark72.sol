...
contract Mark72 is Ownable {
    ...
    function getOffers(uint sortBy, uint start, uint count, IYoloERC721[] calldata _nftContracts) public view returns (address[] memory contracts, uint[] memory ids){
        IYoloERC721[] memory contractList = whitelist;
        if (_nftContracts.length != 0) {
            contractList = _nftContracts;
        }

        uint totalL;
        uint l = _nftContracts.length;
        for (uint i = 0; i < l; i++) {
